...
/*
 * Verify Input parameters of bcm_dnx_cosq_gport_sched_set()/bcm_dnx_cosq_gport_sched_get() for credit request profilr mapping
 */
static shr_error_e
dnx_iqs_api_queue_to_request_profile_verify(
    int unit,
    bcm_gport_t gport,
    ...
    uint32 is_set)
{
     ...
     */
    /** Verfiy queue id */
    SHR_RANGE_VERIFY(queue_id + cosq, 0, dnx_data_ipq.queues.nof_queues_get(unit) - 1, _SHR_E_PARAM,
                     "queue_id out of bound %d.\n", queue_id + cosq);

    /** Verifty*/
    SHR_IF_ERR_EXIT(dnx_ipq_queue_num_cos_get(unit, core_id, queue_id, &num_cos));
    SHR_RANGE_VERIFY(cosq, 0, num_cos - 1, _SHR_E_PARAM, "queue offset out of bound %d (max is %d).\n",
                     cosq, num_cos - 1);
    ...
    int delay_tolerance_id)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_iqs_api_queue_to_request_profile_verify(unit, gport, cosq, delay_tolerance_id, TRUE));
    ...
    int *delay_tolerance_id)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_iqs_api_queue_to_request_profile_verify
                           (unit, gport, cosq, 0 /* delay_tolerance_id */ , FALSE));
    ...
    bcm_cosq_delay_tolerance_t * delay_tolerance)
{
    ...
    if (delay_tolerance_level != BCM_COSQ_DELAY_TOLERANCE_SET_COMMON_STATUS_MSG)
    {
        ...
        supported_flags =
            BCM_COSQ_DELAY_TOLERANCE_IS_LOW_LATENCY | BCM_COSQ_DELAY_TOLERANCE_PUSH_QUEUE |
            BCM_COSQ_DELAY_TOLERANCE_DELETE;
        ...
        if (!dnx_data_iqs.credit.feature_get(unit, dnx_data_iqs_credit_profile_is_rate_class))
        {
            supported_flags |= BCM_COSQ_DELAY_TOLERANCE_TOLERANCE_OCB_ONLY;
        }
        ...
        if (delay_tolerance->flags & BCM_COSQ_DELAY_TOLERANCE_DELETE)
        {
            int delete_profile_id;
            uint32 delete_profile_enable;
            SHR_IF_ERR_EXIT(dnx_iqs_delete_profile_get(unit, &delete_profile_id, &delete_profile_enable));
            if (delete_profile_enable && delete_profile_id != profile_id)
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "delete profile already exists\n");
            }
        }
        ...
        SHR_RANGE_VERIFY(delay_tolerance->credit_request_hungry_multiplier,
                         (delay_tolerance->flags &
                          (BCM_COSQ_DELAY_TOLERANCE_PUSH_QUEUE | BCM_COSQ_DELAY_TOLERANCE_DELETE) ? 0 :
                          dnx_data_iqs.credit.hungry_mult_min_get(unit)) * max_credit_balance_resolution,
                         dnx_data_iqs.credit.hungry_mult_max_get(unit) * max_credit_balance_resolution, _SHR_E_PARAM,
                         "hungry mult is out of bound.\n");
 ...
 *             BCM_COSQ_DELAY_TOLERANCE_DELETE - Used to clear credit balance in VOQ delete. Note that only one profile can be marked as delete profile.
 * \return
 *   See shr_error_e
 * \remark
 *   * None
    ...
    bcm_cosq_delay_tolerance_t * delay_tolerance)
{
     ...
     */
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_delay_tolerance_level_set_verify(unit, delay_tolerance_level, delay_tolerance));
    ...
    if (delay_tolerance_level != BCM_COSQ_DELAY_TOLERANCE_SET_COMMON_STATUS_MSG)
    {
        ...
        if (delay_tolerance->flags & BCM_COSQ_DELAY_TOLERANCE_DELETE)
        {
            SHR_IF_ERR_EXIT(dnx_iqs_delete_profile_set(unit, profile_id, TRUE));
        }
        else
        {
            int delete_profile_id;
            uint32 delete_profile_enable;
            SHR_IF_ERR_EXIT(dnx_iqs_delete_profile_get(unit, &delete_profile_id, &delete_profile_enable));
            if (profile_id == delete_profile_id && delete_profile_enable == TRUE)
            {
                SHR_IF_ERR_EXIT(dnx_iqs_delete_profile_set(unit, 0, FALSE));
            }
        }
    ...
    bcm_cosq_delay_tolerance_t * delay_tolerance)
{
     ...
     * Additional verification in functionals APIs.
     */
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_delay_tolerance_level_get_verify(unit, delay_tolerance_level, delay_tolerance));
    ...
    if (delay_tolerance_level != BCM_COSQ_DELAY_TOLERANCE_SET_COMMON_STATUS_MSG)
    {
        ...
        if (profile_id == delete_profile_id && delete_profile_enable == TRUE)
        {
            delay_tolerance->flags |= BCM_COSQ_DELAY_TOLERANCE_DELETE;
        }
    }
...
dnx_cosq_delay_tolerance_preset_get_verify(
    ...
    bcm_cosq_delay_tolerance_t * delay_tolerance)
{
    uint32 delete_supported_flags = BCM_COSQ_DELAY_TOLERANCE_PRESET_ATTR_F_DELETE;
    ...
    if (preset_attr->flags & BCM_COSQ_DELAY_TOLERANCE_PRESET_ATTR_F_DELETE)
    {

        SHR_MASK_VERIFY(preset_attr->flags, delete_supported_flags, _SHR_E_PARAM,
                        "Only BCM_COSQ_DELAY_TOLERANCE_PRESET_ATTR_F_DELETE.\n");
    }
    else if (preset_attr->flags & BCM_COSQ_DELAY_TOLERANCE_PRESET_ATTR_F_PUSH_QUEUE)
    {
    ...
    bcm_cosq_delay_tolerance_t * delay_tolerance)
{
    ...
    DNX_ERR_RECOVERY_NOT_NEEDED(unit);
    /** Verify */
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_delay_tolerance_preset_get_verify(unit, preset_attr, delay_tolerance));
    ...
    uint32 credit_size)
{
    ...
    /** Verify */
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_dest_credit_size_set_verify(unit, dest_modid, credit_size));
    ...
    uint32 *credit_size)
{
    ...
    /** Verify */
    SHR_INVOKE_VERIFY_DNXC(dnx_cosq_dest_credit_size_get_verify(unit, dest_modid, credit_size));
    ...
    int priority)
{
     ...
     * Verify priority
     */
    SHR_INVOKE_VERIFY_DNXC(dnx_iqs_queue_priority_set_verify(unit, priority));
    ...
    int *priority)
{
     ...
     * Verify priority
     */
    SHR_INVOKE_VERIFY_DNXC(dnx_iqs_queue_priority_get_verify(unit, priority));
    ...
    int timeout)
{
     ...
     * Verify timeout
     */
    SHR_INVOKE_VERIFY_DNXC(dnx_iqs_queue_flush_timeout_verify(unit, timeout));

    /*
     * Set queue id to flush
     */
    SHR_IF_ERR_EXIT(dnx_iqs_queue_manual_operation_queue_id_set(unit, core, voq));
