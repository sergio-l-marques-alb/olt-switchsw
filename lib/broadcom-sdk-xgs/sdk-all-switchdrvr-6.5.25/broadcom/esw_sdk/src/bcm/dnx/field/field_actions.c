 ...
 * \param [in] fes_mask_id -fes mask id
 * \param [in] action_mask - mask value
 * \return
 ...
 * \param [in] fes_id -fes id
 *   4 bits address                : -XXX
 * \param [in] fes_mask_id - mask id
 * \param [out] action_mask_p -mask value
 * \return
 *   \retval Zero - On success
 *   \retval Error - Identifier as per shr_error_e
 * \remark
 ...
 * \param [in] fes_id -fes_id
 * \param [in] ace_id -ace_id
 ...
 * \param [in] fes_id -fes_id
 * \param [in] fes_mask_id - mask_id
 * \param [in] action_mask - action_mask
 * \return
 *   \retval Zero - On success
 *   \retval Error - Identifier as per shr_error_e
 * \remark
 ...
 *    For which of the PMF blocks the configuration is done (iPMF1/2/3, ePMF).
 * \param [in] fes_id - fes_id
 * \param [in] fes_pgm_id - fes_pgm
 *   selected 'fes_id'
 * \param [in] fes2msb_ndx -fes 2 msb condition
 * \param [in] fes_inst_common_info_fes2msb_p -
 *    Pointer to a structure of type dnx_field_actions_fes_common_info_fes2msb_t. This structure contains all
 *    values to load FES fields except chosen_mask:
 *    ACTION_TYPE, VALID_BITS, SHIFT, TYPE and POLARITY
 ...
 * \param [in] key_select - what to load into fes.
 * \return
 *   \retval Zero - On success
 *   \retval Error - Identifier as per shr_error_e
 * \remark
 ...
 * \param [in] max_nof_bits_on_payload - max size of payload
...
/**
 * \brief
 *    This procedure finds the initializes status of a FES instruction, just as it is initializes.
 *    At the moment this means all fields are zero except the action type which is invalid.
 * \param [in] unit -
 *    Identifier of HW platform.
 * \param [in] field_stage -
 *    For which of the PMF blocks the configuration is done (iPMF1/2/3, ePMF).
 * \param [in] fes2msb_conf_p -
 *    Pointer to be loaded with the initialized FES instruction, not including chosen mask.
 * \param [in] chosen_mask_p
 *    Pointer to be loaded with the initialized FES chosen mask.
 * \param [in] fes_pgm_id_p -
 *    Pointer to be loaded with the initialized FES program ID.
 * \param [in] key_select_p
 *    Pointer to be loaded with the initialized FES key select.
 * \param [in] action_mask_p
 *    Pointer to be loaded with the initialized FES action mask.
 * \return
 *    \retval Zero - On success
 *    \retval Error - Identifier as per shr_error_e
 * \remark
 *    None
 * \see
 *   * dnx_field_actions_fes_array_delete()
 *   * dnx_field_action_fes_array_write_reshuffle()
 */
shr_error_e
    ...
    unsigned int num_fes_change)
{
    ...
    if (alloc_result_p->fes_quartet_change[num_fes_change].dest_is_shared == FALSE)
    {
             ...
             fes2msb_ndx++)
        {
            ...
            /*
             * Check if we need to write a mask.
             */
    ...
    dnx_algo_field_action_fes_alloc_in_t * fes_fg_in_p)
{
    ...
    *polarity_zero_exists_p = FALSE;
    ...
    /*
     * Perform possible allocation for action mask IDs, combining similar masks together in each FES quartet.
     */
    ...
    dnx_field_actions_fes_info_t fes_inst_info[DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_ACTION_PER_GROUP])
{
    ...
    if (fes_fg_in.nof_fes_quartets > 0)
    {
         ...
         * At the moment only gathers the allocations of the context ID we allocate for.
         */
    ...
    dnx_algo_field_action_fes_dealloc_out_t * dealloc_result_p)
{
         ...
         fes_id_ndx++)
    {
        ...
        if (dealloc_result_p->belongs_to_fg[fes_id_ndx])
        {
            ...
            /*
             * Set fes program to point to the zero FES program ID,
 ...
 *    Variable of type 'dnx_field_fem_bit_val_t'.
 ...
 *    Pointer to structure of type 'dnx_field_fem_bit_info_t'
 * \param [out] fem_bit_val_p -
 *    Pointer to 'dnx_field_fem_bit_val_t'.
 *    Nothing
 * \remark
 *    * This procedure verifies its input
 */
 ...
 *    Pointer to array of structures containing all info corresponding to all 'input action's
    ...
    {
        {
            ...
            /*
             * check what fem_map_index is being used by other programs;
             */
                 ...
                 fem_program_index++)
            {
                ...
                {
                    /*
                     * We can continue, but instead we will check as a sanity check that all 
                     * conditions are not mapped
                     */
            ...
            for (fem_map_index = 0; fem_map_index < fem_map_index_max; fem_map_index++)
            {
                ...
                {
                    /*
                     * Deallocate if no other field group uses the extraction.
                     */
        ...
        /*
         * Set initial value to priority type.
         */
    ...
    dnx_field_fem_action_entry_t fem_action_entry[DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_FEM_MAP_INDEX])
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_fem_action_find_action_types(unit, fg_id, fem_encoded_actions, fem_action_entry));
    {
        ...
        {
            /** Verify that if priority isn't position, it is "don't care". */
        ...
        }
        {
             ...
             * In order to conceal the EXEM shift for VMV we shift all offsets accordingly, but leave
             * input offset untouched.
             */
            dnx_field_group_type_e fg_type;
            int template_shift;
            SHR_IF_ERR_EXIT(dnx_field_group_action_buffer_shift_get(unit, fg_id, &template_shift));
    ...
    int *fem_id_can_be_used_p)
{
    ...
    /*
     * Check if we can allocate FEM id 0 or 1. This requires:
     * (a) only 4 bits are specified
     * (b) no 'adder' is specified.
     * Go over all relevant configurations.
     */
    ...
    /*
     * Check if fem_id already used by field group.
     */
