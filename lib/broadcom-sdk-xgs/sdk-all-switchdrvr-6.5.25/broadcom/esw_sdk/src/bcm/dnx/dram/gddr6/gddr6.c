...
/** Actually the module bit is ignored, and we use 2 bits for the channel, i.e. both module and channel are determined according to the channel bits */
...
/** bits 7-22 are the row bits, bits 21-22 are not used and will always be 0 */
...
/** bits 23-29 are the column bits, bit 29 is not used and will always be 0 */
    ...
    uint32 *address)
{
    ...
    /**
     * set base physical address based on logical address and atm:
     * each bit in the base_physical_address is received by AND operation on each bit in logical_address with
