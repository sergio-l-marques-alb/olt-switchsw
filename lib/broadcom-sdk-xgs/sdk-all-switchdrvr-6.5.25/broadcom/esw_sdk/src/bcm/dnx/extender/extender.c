...
#include <bcm_int/dnx/auto_generated/dnx_extender_dispatch.h>
    ...
    bcm_port_extender_mapping_info_t * mapping_info)
{
    ...
    else if (type == bcmPortExtenderMappingTypePortVlan)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_port_sit_port_flag_get(unit, mapping_info->phy_port, &flags_get));
        if (!DNX_PORT_IS_COE_PORT(flags_get))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "This port is not COE type for mapping opeartion!\n");
        }
    }
    else
    {
    ...
    bcm_extender_port_t * extender_port)
{
    ...
    SHR_IF_ERR_EXIT(algo_gpm_gport_l2_forward_info_add(unit,
                                                       _SHR_IS_FLAG_SET(extender_port->flags,
                                                                        BCM_EXTENDER_PORT_REPLACE), FALSE,
    ...
    bcm_extender_port_t * extender_port)
{
    ...
    SHR_IF_ERR_EXIT(algo_gpm_gport_and_encap_to_forward_information
                    (unit, ALGO_GPM_ENCODE_DESTINATION_FLAGS_NONE, extender_port->extender_port_id,
    ...
    bcm_extender_port_t * extender_port)
{
    ...
    if (sub_type_format == EEDB_OUT_AC_SUB_TYPE_FORMAT_EXTENDER_PORT)
    {
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field16_get
    ...
    int is_update)
{
    ...
    outlif_mngr.logical_phase = DNX_ALGO_LOCAL_OUTLIF_LOGICAL_PHASE_8;
    ...
    bcm_extender_port_t * extender_port)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_extender_port_add_verify(unit, extender_port));
    ...
    if (update)
    {
        ...
        if (extender_port->criteria == BCM_EXTENDER_PORT_MATCH_PORT_EXTENDED_PORT_VLAN_STACKED)
        {
            if (extender_port->match_inner_vlan != extender_port_old.match_inner_vlan)
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "Can't change extender port match inner vlan\n");
            }
        }
    ...
    else
    {
        ...
        SHR_IF_ERR_EXIT(dnx_algo_global_lif_allocation_allocate
                        (unit, allocation_flags, direction_flags, NULL, &global_lif));
    ...
    bcm_extender_port_t * extender_port)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_extender_port_get_verify(unit, extender_port));
    ...
    if (is_ingress)
    {
        ...
        if ((gport_hw_resources.inlif_dbal_result_type == DBAL_RESULT_TYPE_IN_AC_INFO_DB_IN_LIF_AC_MP_LARGE)
            || (gport_hw_resources.inlif_dbal_result_type ==
                DBAL_RESULT_TYPE_IN_AC_INFO_DB_IN_LIF_AC_MP_LARGE_GENERIC_DATA_AND_PROTECTION))
        {
    ...
    bcm_gport_t extender_port_id)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_extender_port_delete_verify(unit, extender_port_id));
    ...
    bcm_port_extender_mapping_info_t * mapping_info)
{
    ...
    /** verify */
    SHR_INVOKE_VERIFY_DNXC(dnx_port_extender_mapping_info_set_verify(unit, flags, type, mapping_info));
    ...
    bcm_port_extender_mapping_info_t * mapping_info)
{
    ...
    /** verify */
    SHR_INVOKE_VERIFY_DNXC(dnx_port_extender_mapping_info_get_verify(unit, flags, type, mapping_info));
