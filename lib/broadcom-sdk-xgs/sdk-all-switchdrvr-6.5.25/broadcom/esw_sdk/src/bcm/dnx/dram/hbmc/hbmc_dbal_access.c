/** \file src/bcm/dnx/dram/hbmc/hbmc_dbal_access.c
 *
 *
 *  This file contains the implementation of dbal access
 *  functions of the hbmc module.
 *
 */
...
/**
 * \brief - transform from nanoseconds to clocks
 *
    ...
    uint32 ns)
{
     ...
     *              clk = (ns * frequency * 10^6) / (10^9)
     * however since the numbers are too big, it as done as can be seen below
     */
...
{
    uint32 entry_handle_id;
    /** need to remove const qualifier for low level functions */
    ...
    {
        /** if data_mode=prbs, MISR_mode=1. if data_mode=pattern misr_mode is don't care. Therefore, we set MISR_MODE=0x1 */
        ...
        {
            /** only 20b from each prbs_seed are used, so a mask of 0xfffff is required */
    ...
    /** Data pattern is divided to two tables because of size limitations, first half of the patterns in one table second
     *  in the second table, this is the reason for the below handling */
    ...
    /** patterns 4 - 7, in PART2 table instances 0-3  */
    ...
    hbmc_shmoo_bist_err_cnt_t * error_counters)
{
    ...
    /** Transform info from dbal to error_counters struct */
...
/* see header file */
shr_error_e
dnx_hbmc_dbal_access_parity_errors_get(
    int unit,
    int hbm_index,
    int channel,
...
{
    uint32 entry_handle_id;
    uint32 nof_channel_dwords = dnx_data_dram.hbm.nof_channel_dwords_get(unit);
    uint32 i_dword;
    ...
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_HCC_PARITY_ERR_COUNTERS, &entry_handle_id));
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_HBMC_INDEX, hbm_index);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CHANNEL, channel);
    for (i_dword = 0; i_dword < nof_channel_dwords; ++i_dword)
    {
        dbal_value_field_arr32_request(unit, entry_handle_id, DBAL_FIELD_WRITE_PARITY_DWORD_ERR_CNT, i_dword,
                                       error_counters->write_parity_err_cnt[i_dword]);
        dbal_value_field_arr32_request(unit, entry_handle_id, DBAL_FIELD_READ_PARITY_DWORD_ERR_CNT, i_dword,
                                       error_counters->read_parity_err_cnt[i_dword]);
    }
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));
    ...
    int hbm_index)
{
    ...
    /** enable channels */
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_HBM_PLL_CH_ENABLEB, INST_SINGLE,
                                 dnx_data_dram.apd_phy.pll_get(unit)->ch_enable_bitmap);
    ...
    int hbm_index)
{
    ...
    if (dnx_data_dram.hbm.feature_get(unit, dnx_data_dram_hbm_apd_phy))
    {
        /** take phy power out of reset. should be done before taking phy i/o out of reset */
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_HBM_PHY_POWER_RESET, INST_SINGLE, 0);

        /** take phy i/o out of reset */
        if (!dnx_data_dram.hbm.feature_get(unit, dnx_data_dram_hbm_hbm2e_support))
        {
...
/* see header file */
shr_error_e
dnx_hbmc_dbal_access_phy_reset_set(
    int unit,
    int hbm_index,
    uint32 value)
{
    ...
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_HBMC_INDEX, hbm_index);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_PHY_SBUS_RESET, INST_SINGLE, value);
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    ...
    int enable)
{
    ...
    /** Enable dynamic memory access for all tables under HBMC block */
    SHR_IF_ERR_EXIT(dnx_hbmc_dbal_access_hbmc_dynamic_memory_access_set(unit, hbm_index, enable));
    /** Enable dynamic memory access for all tables under HCC block */
    ...
    int hbm_index)
{
    ...
    uint32 read_parity_enable = dq_read_parity;
    ...
    if (dnx_data_dram.hbm.feature_get(unit, dnx_data_dram_hbm_apd_phy))
    {
        ...
        read_parity_enable = (dq_read_parity == 0 && dq_write_parity == 0) ? 0 : 1;
    }
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_HBM_DQ_WRITE_PARITY, INST_SINGLE,
                                 write_parity_enable);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_HBM_DQ_READ_PARITY, INST_SINGLE, read_parity_enable);
    ...
    {
        /**
         * Read parity should always be enabled, unless both read parity and write parity are disable.
         * As a result, parity latency should be set only in case read_parity=1 and write_parity=0.
         */
        if (dq_read_parity == 1 && dq_write_parity == 0)
        {
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_HBM_PARITY_LATENCY, INST_SINGLE,
                                         dnx_data_dram.general_info.parity_latency_get(unit));
        }
    }
    else
    {
    ...
    if (dnx_data_dram.hbm.feature_get(unit, dnx_data_dram_hbm_apd_phy))
    {
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_RD_DATA_EN_DELAY, INST_SINGLE,
                                     dnx_data_dram.general_info.read_data_enable_delay_get(unit));
    ...
    if (dnx_data_dram.hbm.feature_get(unit, dnx_data_dram_hbm_apd_phy))
    {
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_HBM_T_WR_TO_RDAP, INST_SINGLE,
                                     dnx_data_dram.general_info.write_latency_get(unit) +
                                     dnx_data_dram.hbm.burst_length_get(unit) / 2 +
                                     UTILEX_MAX(dnx_data_dram.general_info.timing_params_get(unit)->twr -
                                                dnx_data_dram.general_info.timing_params_get(unit)->trtpl,
                                                dnx_data_dram.general_info.timing_params_get(unit)->twtrl));
    }
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    ...
    if (dnx_data_dram.hbm.feature_get(unit, dnx_data_dram_hbm_channel_wds_inflight_threshold))
    {
        ...
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_HBMC_INDEX, hbm_index);
        dbal_entry_key_field32_range_set(unit, entry_handle_id, DBAL_FIELD_CHANNEL, DBAL_RANGE_ALL, DBAL_RANGE_ALL);
        /** Stop the WDS 2 entries before it's full */
    ...
    int hbm_index)
{
    ...
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_HBMC_INDEX, hbm_index);
    dbal_entry_key_field32_range_set(unit, entry_handle_id, DBAL_FIELD_CHANNEL, DBAL_RANGE_ALL, DBAL_RANGE_ALL);
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, data_source)
    {
...
/**
 * \brief - transform mode register index to relevant dbal field
 *
 * \param [in] mr_index - mode register index
 *
 *
 * \return
 *   dbal_fields_e - the relevant dbal field or DBAL_FIELD_EMPTY if input was invalid
 *
 * \remark
 *   * None
 * \see
    ...
    uint32 value)
{
    ...
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_HBMC_INDEX, hbm_index);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CHANNEL, channel);
    /** choose mode register dbal field according to mr_index */
...
/* see header file */
shr_error_e
dnx_hbmc_dbal_access_mr_set_all(
    int unit,
    uint32 dram_bitmap[],
    dnx_hbmc_mrs_values_t * mr_values_p)
{
    uint32 entry_handle_id;
    int nof_mrs = dnx_data_dram.general_info.nof_mrs_get(unit);
    int mr_index;
    ...
    SHR_BIT_ITER(dram_bitmap, nof_hbms, hbm_index)
    {
        for (channel_index = 0; channel_index < nof_channels_in_hbm; ++channel_index)
        {
            ...
            {
                dbal_entry_value_field32_set(unit, entry_handle_id, dnx_hbmc_mr_index_to_dbal_field(mr_index),
                                             INST_SINGLE, mr_values_p->mr_val[hbm_index][channel_index][mr_index]);
            }
            SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
            SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_HCC_HBM_MODE_REGISTERS, entry_handle_id));
        }
    }
exit:
    DBAL_FUNC_FREE_VARS;
...
/* see header file */
shr_error_e
dnx_hbmc_dbal_access_mr_get_all(
    int unit,
    uint32 dram_bitmap[],
    dnx_hbmc_mrs_values_t * mr_values_p)
{
    ...
    SHR_BIT_ITER(dram_bitmap, nof_hbms, hbm_index)
    {
        for (channel_index = 0; channel_index < nof_channels_in_hbm; ++channel_index)
        {
            ...
            {
                /** choose mode register dbal field according to mr_index */
                dbal_value_field32_request(unit, entry_handle_id, dnx_hbmc_mr_index_to_dbal_field(mr_index),
                                           INST_SINGLE, &mr_values_p->mr_val[hbm_index][channel_index][mr_index]);
            }
            SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));
            SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_HCC_HBM_MODE_REGISTERS, entry_handle_id));
...
/**
 * \brief - fill dbal entry handle with relevant values for MRs on init.
 *
 * \param [in] unit - unit number
 * \param [in] entry_handle_id - handle id for dbal
 *
 *
 * \return
 *   shr_e_error
    ...
    uint32 entry_handle_id)
{
    ...
    {
                /** loop over mrs and set thier defaults */
    ...
    /** set other sub field values according to dnx data input */
    ...
    SHR_IF_ERR_EXIT(dbal_fields_struct_field_encode(unit, DBAL_FIELD_MR_0, DBAL_FIELD_CMD_PARITY,
                                                    &field_in_struct_val, field_val));
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_MR_0, INST_SINGLE, field_val[0]);
    ...
    SHR_IF_ERR_EXIT(dbal_fields_struct_field_encode(unit, DBAL_FIELD_MR_2, DBAL_FIELD_READ_LATENCY,
                                                    &field_in_struct_val, field_val));
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_MR_2, INST_SINGLE, field_val[0]);
    ...
    SHR_IF_ERR_EXIT(dbal_fields_struct_field_encode(unit, DBAL_FIELD_MR_3, DBAL_FIELD_ACTIVATE_TO_PRECHARGE_RAS,
                                                    &field_in_struct_val, field_val));
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_MR_3, INST_SINGLE, field_val[0]);
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_MR_4, INST_SINGLE, field_val[0]);
    ...
    SHR_IF_ERR_EXIT(dbal_fields_struct_field_encode(unit, DBAL_FIELD_MR_6, DBAL_FIELD_PRE_TRP_VALUE,
                                                    &field_in_struct_val, field_val));
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_MR_6, INST_SINGLE, field_val[0]);
    ...
    uint32 *data_source_id)
{
    ...
    /*
     * The first TDU in each HBMC is mapped to the matching core
     */
...
/*
 * brief - configure write-minus-read leaky bucket for DRAM block
 */
static shr_error_e
    ...
    int unit)
{
    ...
    /** Decrement thresholds are in percentage */
    ...
    {
        /** disable the write-minus-read leaky bucket */
...
/*
 * brief - configure write-plus-read leaky bucket for DRAM block
 */
static shr_error_e
    ...
    int unit)
{
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LEAKY_BUCKET_INCREMENT_SIZE_2, INST_SINGLE, 4);
    /** Increment thresholds are in percentage */
    ...
    {
        /** disable the write-plus-read leaky bucket */
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LEAKY_BUCKET_EN, INST_SINGLE, 0);
    }
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
...
/*
 * brief - configure write leaky bucket for DRAM block
 */
static shr_error_e
    ...
    int unit)
{
    ...
    {
        /** In cases where average-read-inflight mechansim has a log2 based window size, need to configure it separately */
    ...
    int unit)
{
    ...
    /*
     * Configure the data source id of each TDU.
     * If only the second dram is activated, we need to configure the
     * source id for first dram too, otherwise it will not be configred
     * and will result in misconfiugration of that field.
     * Note that in case the dram bitmap has only the second HBM then
     * the data source id is configured the opposite.
     */
    ...
    /** in the dram we have a mechanism which consists of leaky buckets to control when it should be
     * blocked for writes in order not to cause starvation for the reads. each bucket is getting filled
     * and emptied at a configurable rate according to percentage of total traffic to the dram or according
     * to difference in amount of reads and writes. once both buckets passed the assert threshold, the dram is
     * blocked. */
    ...
    const uint32 *atm)
{
    ...
    for (global_hbm_index = 0; global_hbm_index < max_nof_drams; ++global_hbm_index)
    {
        ...
        for (tdu_index = 0; tdu_index < nof_tdus_per_dram; ++tdu_index)
        {
            ...
            /*
             * The TDU block is associated with a corresponding core (data source id), and is responsible to send the packets it gets to the appropriate HBM.
             * For each HBM we have a TDU block which gets packets from the
             * corresponding core (data source id) and send them to the
             * appropriate HBM.
             *
             * Each HBM controller has a TDU block which is associated with a
             * corresponding "data source", and is responsible to send
             * the packets it gets from its "data source" to the appropriate
             * HBM (can also be another HBM).
             *
             * In cases where the TDU and its "data source" are not corresponding
             * to the same HBMC it means the TDU is "fake", i.e. it get its packets
             * from a "real" TDU that resides on a different HBMC and not directly
            ...
            /*
             * "Fake" TDU should have the same translation matrix as "real" TDU
             * except line 0 (module row) which should be zero in order to keep
             * the packet in the local HBM of "fake" TDU.
             */
            ...
            /*
             * All other lines should be configured the same for all TDUs
             */
