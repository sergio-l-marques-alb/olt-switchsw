    ...
    bcm_gport_t * gport)
{
    ...
    else
    {
        ...
        /*
         * When global voq feature is in use, we must allow to map multiple OTM+TC to same flow id.
         * These flow cannot be used by the latency mechanisms.
         */
        if (dnx_data_global_voq.general.feature_get(unit, dnx_data_global_voq_general_is_supported))
        {
            alloc_info.range_end -= dnx_data_cosq_aqm.general.reserved_flows_nof_get(unit);
        }
    }
