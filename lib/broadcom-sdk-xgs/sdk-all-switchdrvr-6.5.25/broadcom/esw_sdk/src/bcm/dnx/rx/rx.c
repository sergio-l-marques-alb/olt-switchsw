    ...
    uint8 device_access_allowed)
{
    ...
    if (do_rx_parser)
    {
        ...
        if (RX_CHAN_CFG(unit, chan).flags & BCM_RX_F_TAKE_PORT_FROM_PARSER)
        {
    ...
    bcm_gport_t * dst_gport)
{
    ...
    int channel_id_i;
    int core_id = dnx_data_adapter.reg_mem_access.swap_core_index_zero_with_core_index_get(unit);
    SHR_FUNC_INIT_VARS(unit);
    ...
    *dst_gport = 0;
    ...
    {
        SHR_IF_ERR_EXIT(dnx_algo_port_nif_phy_to_logical_get(unit, first_lane, 0, 0, &logical_port));
    ...
    else
    {
        ...
        while ((logical_port == DNX_ALGO_PORT_INVALID) && (cosq < 8) && (port_channel >= cosq))
        {
            SHR_IF_ERR_EXIT(dnx_algo_port_interface_and_channel_to_logical_get
                            (unit, core_id, port_info, (port_channel - cosq), TRUE, &logical_port));
            cosq++;
        }
    }
    ...
    void *param)
{
    ...
    buf = sal_alloc(sizeof(*buf) * MAX_PACKET_SIZE_ADAPTER, "Read buffer");
    ...
    next_packet_index = 0;
    sub_unit_id = 0;
    nof_sub_units = dnx_data_adapter.general.nof_sub_units_get(0);

    /*
     * assuming system with multiple units and multiple sub-units is not allowed.
     * find the number of units/sub_units to handle
     */
    toggle_unit = soc_cm_get_num_devices() == 1 ? FALSE : TRUE;
    toggle_sub_unit = (!toggle_unit) && (nof_sub_units > 1);

    INIT_SLEEP;
    while (RX_CONTROL_THREAD_RUNNING_GET())
    {
        if ((dnx_drv_soc_attached(unit) == FALSE) || (dnx_init_is_init_done_get(unit) == FALSE))
        {
            if (toggle_unit)
            {
...
dnx_rx_pool_memory_alloc(
    ...
    char *locus)
{
    return (rxp_memory_source_heap ? sal_alloc(size, locus) : sal_dma_alloc(size, locus));
}
...
dnx_rx_pool_memory_free(
    void *rx_pool_addr)
{
    rxp_memory_source_heap ? sal_free(rx_pool_addr) : sal_dma_free(rx_pool_addr);
}
