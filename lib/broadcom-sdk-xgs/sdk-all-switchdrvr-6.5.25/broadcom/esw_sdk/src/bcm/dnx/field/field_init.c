    ...
    int unit)
{
    ...
    */
    if (dnx_data_field.exem.supported_get(unit) || dnx_data_field.mdb_dt.supported_get(unit))
    {
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_FIELD_PMF_MUX_MDB_INTERFACES, &entry_handle_id));
    }

    if (dnx_data_field.exem.supported_get(unit))
    {
        /*
         * The value of the SEXM MUX is dependant on the SOC property pmf_sexem3_stage.
         */
        sexem_stage = dnx_data_field.exem.pmf_sexem3_stage_get(unit);
    ...
    if (dnx_data_field.mdb_dt.supported_get(unit))
    {
         ...
         */
        map_stage = dnx_data_field.mdb_dt.pmf_map_stage_get(unit);
