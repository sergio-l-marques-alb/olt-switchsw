    ...
    const bcm_port_resource_t * resource)
{
        ...
        dnx_data_nif.oft.feature_get(unit, dnx_data_nif_oft_is_used))
    {
        ...
        for (resource_index = 0; resource_index < nport; ++resource_index)
        {
            if (DNX_ALGO_PORT_TYPE_IS_NIF_ETH(unit, ports_info[resource_index], DNX_ALGO_PORT_TYPE_INCLUDE_L1)
                || DNX_ALGO_PORT_TYPE_IS_FLEXE_MAC(unit, ports_info[resource_index], TRUE))
         ...
         */
        if (_SHR_PBMP_NOT_NULL(ports_with_modified_speed))
        {
    ...
    if (dnx_data_nif.arb.feature_get(unit, dnx_data_nif_arb_is_supported))
    {
        ...
        _SHR_PBMP_ITER(ports_with_modified_speed, logical_port)
        {
        ...
        for (resource_index = 0; resource_index < nport; ++resource_index)
        {
            if (DNX_ALGO_PORT_TYPE_IS_FLEXE_MAC(unit, ports_info[resource_index], FALSE))
            {
    ...
    if (dnx_data_nif.ofr.feature_get(unit, dnx_data_nif_ofr_is_supported))
    {
        ...
        for (resource_index = 0; resource_index < nport; ++resource_index)
        {
            if (DNX_ALGO_PORT_TYPE_IS_NIF_ETH(unit, ports_info[resource_index], DNX_ALGO_PORT_TYPE_INCLUDE(L1, MGMT))
                || DNX_ALGO_PORT_TYPE_IS_FLEXE_MAC(unit, ports_info[resource_index], FALSE)
                || DNX_ALGO_PORT_TYPE_IS_NIF_ILKN(unit, ports_info[resource_index], DNX_ALGO_PORT_TYPE_INCLUDE(L1)))
            {

                SHR_IF_ERR_EXIT(dnx_port_ofr_rx_mem_link_list_set(unit, resource[resource_index].port, TRUE));
