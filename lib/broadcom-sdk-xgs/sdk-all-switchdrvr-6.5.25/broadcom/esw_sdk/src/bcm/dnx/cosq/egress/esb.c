    ...
    uint32 *esb_tdm_or_lp_queue_id)
{
    ...
    */
    ...
    else if (DNX_ALGO_PORT_TYPE_IS_FLEXE_MAC(unit, port_info, 0) || DNX_ALGO_PORT_TYPE_IS_SAR(unit, port_info))
    {
        /**
    ...
    uint8 is_preemption)
{
    ...
    */
    ...
    while (current_queue != ESB_QUEUE_ID_INVALID)
    {
         ...
         */
        if (DNX_ALGO_PORT_TYPE_IS_NIF(unit, port_info, 0) || DNX_ALGO_PORT_TYPE_IS_FLEXE_MAC(unit, port_info, 0))
        {
             ...
             */
            egr_if_to_config =
                (dnx_data_esb.general.feature_get(unit, dnx_data_esb_general_per_priority_queues_mapping) ==
                 1) ? ((current_queue ==
                        esb_tdm_or_lp_queue_id) ? (egr_if_hp +
                                                   dnx_data_egr_queuing.params.first_lp_egr_interface_get(unit) -
                                                   dnx_data_egr_queuing.
                                                   params.egr_if_nif_base_get(unit)) : egr_if_hp) : egr_if_hp;
            ...
            */
             ...
             */
            if (DNX_ALGO_PORT_TYPE_IS_SAR(unit, port_info))
            {
