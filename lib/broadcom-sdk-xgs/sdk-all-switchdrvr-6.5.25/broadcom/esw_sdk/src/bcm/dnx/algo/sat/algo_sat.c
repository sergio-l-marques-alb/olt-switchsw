...
/**
 * \brief
 *    Initialize pkt_header_mem resources.
 *
 * \param [in] unit - Unit-ID
 *
 * \return
 ...
 * \remark
 *    The resource of SAT pkt_header_mem is initialized
 *
 * \see
 *    dnx_algo_res_create
 *
    ...
    int unit)
{
    ...
    sal_memset(&data, 0, sizeof(dnx_algo_res_create_data_t));
    data.flags = 0;
    /** 32*1024/32 = 1024 DWORDs.*/
    data.first_element = 0;
    data.nof_elements = 1024;
    sal_strncpy(data.name, DNX_ALGO_RES_TYPE_SAT_PKT_HEADER_MEM, DNX_ALGO_RES_MNGR_MAX_NAME_LENGTH - 1);

    SHR_IF_ERR_EXIT(algo_sat_db.sat_res_alloc_pkt_header_mem.create(unit, &data, NULL));
exit:
    SHR_FUNC_EXIT;
}
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_pkt_header_mem_init(unit));
exit:
    SHR_FUNC_EXIT;
}
