    ...
    int *template_id)
{
    ...
    if (!
        (sal_strncmp
         (instance_name, "ECMP consistent hashing manager handle", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH)))
    {
        *template_id = L3_ECMP_CONSISTENT_MANAGER;
        SHR_EXIT();
    }

    if (!(sal_strncmp(instance_name, "chm unit testing one", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH)))
    {
        *template_id = CHM_UNIT_TEST_1;
        SHR_EXIT();
    }

    if (!(sal_strncmp(instance_name, "chm unit testing two", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH)))
    {
        *template_id = CHM_UNIT_TEST_2;
        SHR_EXIT();
    }

    if (!(sal_strncmp(instance_name, "chm unit testing three", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH)))
    {
        *template_id = CHM_UNIT_TEST_3;
        SHR_EXIT();
    }

    if (!(sal_strncmp(instance_name, "chm unit testing four", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH)))
    {
        *template_id = CHM_UNIT_TEST_4;
        SHR_EXIT();
    }

    if (!(sal_strncmp(instance_name, "chm unit testing five", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH)))
    {
        *template_id = CHM_UNIT_TEST_5;
        SHR_EXIT();
    }

    if (!(sal_strncmp(instance_name, "chm unit testing six", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH)))
    {
        *template_id = CHM_UNIT_TEST_6;
        SHR_EXIT();
    }

    if (!(sal_strncmp(instance_name, "chm unit testing seven", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH)))
    {
        *template_id = CHM_UNIT_TEST_7;
        SHR_EXIT();
    }

    if (!
        (sal_strncmp
         (instance_name, "trunk consistent hashing manager handle", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH)))
    {
        *template_id = TRUNK_CHM;
        SHR_EXIT();
    }
    ...
    uint32 *chm_handle)
{
    ...
    if (nof_hierarchies == 0)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Invalid value for nof_hierarchies (%d)\n", nof_hierarchies);
    }
    if (nof_banks == 0)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Invalid value for nof_banks (%d)\n", nof_banks);
    }
    if (((nof_banks == 1) && (nof_hierarchies != 1)) || ((nof_banks != 1) && (nof_hierarchies == 1)))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Invalid values for nof_banks(%d) and nof_hierarchies(%d).\n"
                     " if one of them is one they should both be one\n", nof_banks, nof_hierarchies);
    }
    bank_size = nof_resources_to_manage / nof_banks;
    if (!utilex_is_power_of_2(bank_size))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "bank_size (%d) has to be a power of 2\n", bank_size);
    }

    name_len = sal_strnlen(instance_name, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH);
    ...
    /** save number of hierarchies and bank size to sw_state */
    SHR_IF_ERR_EXIT(consistent_hashing_manager.ch_instance_array.
                    nof_hierarchies.set(unit, *chm_handle, nof_hierarchies));
    SHR_IF_ERR_EXIT(consistent_hashing_manager.ch_instance_array.bank_size.set(unit, *chm_handle, bank_size));

    /** create a defragmented chunk instances - there's a chunk for each hierarchy */
    SHR_IF_ERR_EXIT(consistent_hashing_manager.ch_instance_array.def_chunk.alloc(unit, *chm_handle, nof_hierarchies));
    ...
    for (int hierarchy = 0; hierarchy < nof_hierarchies; ++hierarchy)
    {
        SHR_IF_ERR_EXIT(consistent_hashing_manager.ch_instance_array.
                        def_chunk.create(unit, *chm_handle, hierarchy, &chunk_init_info));
    }
...
/**
 * \brief - allocate a piece in relevant defragmented chunk instance
 *          and manage reserved space in others.
 *
 * \param [in] unit - unit number
 * \param [in] chm_handle - consistent hashing manager handle
 * \param [in] hierarchy - hierarchy for which the alloc is needed
 * \param [in] piece_size - size of piece to allocate
 * \param [out] profile_offset - returned offset of allocated piece
 *
 * \return
 *   shr_error_e
 *
 ...
 */
static shr_error_e
dnx_algo_consistent_hashing_manager_def_chunk_piece_alloc(
    int unit,
    uint32 chm_handle,
    uint32 hierarchy,
    uint32 piece_size,
    uint32 *profile_offset)
{
    ...
    /** get nof_hierarchies and bank size */
    SHR_IF_ERR_EXIT(consistent_hashing_manager.ch_instance_array.
                    nof_hierarchies.get(unit, chm_handle, &nof_hierarchies));
    SHR_IF_ERR_EXIT(consistent_hashing_manager.ch_instance_array.bank_size.get(unit, chm_handle, &bank_size));

    /** try to allocate a piece in relevant hierarchy instance of def chunk */
    rv = consistent_hashing_manager.ch_instance_array.def_chunk.piece_alloc(unit, chm_handle, hierarchy, piece_size,
                                                                            (void *) &chm_handle, profile_offset);
    /** if relevant hierarchy is currently full */
    if ((rv == _SHR_E_FULL) || (rv == _SHR_E_NOT_FOUND))
    {
        const int INVALID_HIERARCHY = -1;
        int hierarchy_with_cleared_bank = INVALID_HIERARCHY;
        uint32 cleared_bank_offset;

        /** iterate hierarchies and try to clear a bank to use in requested hierarchy */
        for (int hierarchy_iter = 0; hierarchy_iter < nof_hierarchies; ++hierarchy_iter)
        {
            if (hierarchy_iter == hierarchy)
            {
                /** skip requested hierarchy */
                continue;
            }

            /** defrag hierarchy for a size of a bank */
            rv = consistent_hashing_manager.ch_instance_array.def_chunk.defrag(unit, chm_handle, hierarchy_iter,
                                                                               bank_size, (void *) &chm_handle,
                                                                               &cleared_bank_offset);
            if ((rv == _SHR_E_NOT_FOUND) || (rv == _SHR_E_FULL))
            {
                /** if unsuccessful to clear a bank in this hierarchy try again in the next one */
                continue;
            }
            else
            {
                SHR_IF_ERR_EXIT(rv);
                /** save found hierarchy and break */
                hierarchy_with_cleared_bank = hierarchy_iter;
                break;
            }
        }

        /** if not found a hierarchy */
        if (hierarchy_with_cleared_bank == INVALID_HIERARCHY)
        {
            /** return an error */
            SHR_ERR_EXIT(_SHR_E_FULL, "all hierarchies are full, unable to clear resources for profile\n");
        }

        /** unreserve bank in requested hierarchy */
        SHR_IF_ERR_EXIT(consistent_hashing_manager.ch_instance_array.
                        def_chunk.offset_unreserve(unit, chm_handle, hierarchy, bank_size, cleared_bank_offset));
        /** try to allocate again - this time it should always work */
        SHR_IF_ERR_EXIT(consistent_hashing_manager.ch_instance_array.
                        def_chunk.piece_alloc(unit, chm_handle, hierarchy, piece_size, (void *) &chm_handle,
                                              profile_offset));
    }
    else
    {
    ...
    /** reserve bank in other hierarchies */
    bank_offset = (*profile_offset) / bank_size * bank_size;
    for (int hierarchy_iter = 0; hierarchy_iter < nof_hierarchies; ++hierarchy_iter)
    {
        if (hierarchy_iter == hierarchy)
        {
        ...
        SHR_IF_ERR_EXIT(consistent_hashing_manager.ch_instance_array.
                        def_chunk.offset_reserve(unit, chm_handle, hierarchy_iter, bank_size, bank_offset));
    }
    ...
    void *user_info)
{
    ...
    SHR_IF_ERR_EXIT(consistent_hashing_manager.ch_instance_array.
                    nof_hierarchies.get(unit, chm_handle, &nof_hierarchies));
    if (hierarchy >= nof_hierarchies)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Invalid hierarchy(%d), has to be between 0 and %d\n", hierarchy,
                     nof_hierarchies - 1);
    }
    ...
    t_mngr_key.hierarchy = hierarchy;
    ...
    if (first_reference)
    {
        ...
        rv = dnx_algo_consistent_hashing_manager_def_chunk_piece_alloc(unit, chm_handle, hierarchy, piece_alloc_size,
                                                                       &profile_offset);
        if (rv != _SHR_E_NONE)
        {
    ...
    int profile_type)
{
    ...
    if (last_reference)
    {
        ...
        uint32 nof_resources;
        SHR_IF_ERR_EXIT(consistent_hashing_manager.ch_instance_array.
                        def_chunk.piece_free(unit, chm_handle, profile_key.hierarchy, profile_offset));
    ...
    void *user_info)
{
    ...
    if ((current_nof_members + 1 == profile_key.max_nof_elements_in_profile) && (profile_key.consecutive_profiles == 1))
    {
        ...
        if (first_reference)
        {
            ...
            rv = dnx_algo_consistent_hashing_manager_def_chunk_piece_alloc(unit, chm_handle, profile_key.hierarchy,
                                                                           nof_resources, &new_profile_offset);
            if (rv != _SHR_E_NONE)
            {
                /** need to free profile handle in template manager if defragmented chunk alloc returned an error */
                SHR_IF_ERR_EXIT(dnx_algo_consistent_hashing_manager_template_id_get
    ...
    void *user_info)
{
    ...
    if ((current_nof_members == profile_key.max_nof_elements_in_profile) && (profile_key.consecutive_profiles == 1))
    {
        ...
        /** allocate in defragmented chunk */
        SHR_IF_ERR_EXIT(cbs->nof_resources_per_profile_type_get(unit, profile_key.profile_type, &nof_resources));
        SHR_IF_ERR_EXIT(dnx_algo_consistent_hashing_manager_def_chunk_piece_alloc
                        (unit, chm_handle, profile_key.hierarchy, nof_resources, &new_profile_offset));
    ...
    const void *data)
{
    ...
    SW_STATE_PRETTY_PRINT_ADD_LINE(SW_STATE_PRETTY_PRINT_TYPE_UINT32, "Hierarchy",
                                   t_mngr_key->hierarchy, "Hierarchy", "0x%x");
