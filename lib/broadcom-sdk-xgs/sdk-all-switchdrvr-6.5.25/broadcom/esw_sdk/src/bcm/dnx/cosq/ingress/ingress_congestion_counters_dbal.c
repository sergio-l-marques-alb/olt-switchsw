...
/**
 * \brief - Function to configure the memory access bubble width
 *
 */
static shr_error_e
dnx_ingress_congestion_memory_access_bubble_width_set(
    int unit,
    int core,
    uint32 width)
{
    ...
    /** Taking a handle to DBAL table */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_INGRESS_CONG_CGM_ACCESS_BUBBLE_CONFIG, &entry_handle_id));

    /** Select the core */
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, core);

     /** Get current queue id and its occupancy level   */
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_BUBBLE_WIDTH, INST_SINGLE, width);
...
/**
 * \brief - Function to get the memory access bubble width
 *
 */
static shr_error_e
dnx_ingress_congestion_memory_access_bubble_width_get(
    int unit,
    int core,
    uint32 *width)
{
    ...
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, core);
     ...
     /** Get current queue id and its occupancy level   */
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_BUBBLE_WIDTH, INST_SINGLE, width);
 ...
 */
shr_error_e
dnx_ingress_congestion_vsq_occupancy_iterator_get_next(
    int unit,
    int core,
    ...
    int *is_end)
{
    uint32 default_bubble_width = 0;
    uint32 oversubscription_bubble_width = 0;
    ...
    if (oversubscription)
    {
        /** Set bubble width to enable CPU access under oversubscription */
        oversubscription_bubble_width = dnx_data_ingr_congestion.vsq.oversubscription_bubble_width_get(unit);
        SHR_IF_ERR_EXIT(dnx_ingress_congestion_memory_access_bubble_width_get(unit, core, &default_bubble_width));
        SHR_IF_ERR_EXIT(dnx_ingress_congestion_memory_access_bubble_width_set
                        (unit, core, oversubscription_bubble_width));
    }
    ...
    if (oversubscription)
    {
        /** Restore bubble width value */
        SHR_IF_ERR_EXIT(dnx_ingress_congestion_memory_access_bubble_width_set(unit, core, default_bubble_width));
    }
    ...
    dnx_ingress_congestion_vsq_occupancy_info_t * info)
{
    ...
    SHR_IF_ERR_EXIT(dnx_ingress_congestion_vsq_occupancy_iterator_get_next(unit, core, 0, entry_handle_id, &is_end));
