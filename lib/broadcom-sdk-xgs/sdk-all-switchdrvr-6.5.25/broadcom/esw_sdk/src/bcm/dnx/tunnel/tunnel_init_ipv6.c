    ...
    uint32 srv6_with_dip)
{
    ...
    /** in case of IPv6 which is part of SRv6 tunnel which includes DIP,
     *  it must go to last SID (VPN), ie. Phase 7, unless specified to be in phase 6
     */
    if (((tunnel->type == bcmTunnelTypeSR6) || (tunnel->type == bcmTunnelTypeEthSR6)) && srv6_with_dip)
    {
        ipv6_dip_encap_phase =
            (tunnel->encap_access == bcmEncapAccessTunnel4) ? bcmEncapAccessTunnel4 : bcmEncapAccessArp;
    }
    ...
    uint32 *local_next_outlif)
{
   ...
   /** read DATA_ENTRY table */
    ...
    /**
     * Ipv6 data entry can be in phases 6/7 depending on the configuration
     * If user provided bcmEncapAccessTunnel4, it will be in outlif phase 6, otherwise the default outlif phase 7 is used
     */
    if ((encap_access == bcmEncapAccessTunnel4)
        && ((tunnel->type == bcmTunnelTypeSR6) || (tunnel->type == bcmTunnelTypeEthSR6)))
    {
