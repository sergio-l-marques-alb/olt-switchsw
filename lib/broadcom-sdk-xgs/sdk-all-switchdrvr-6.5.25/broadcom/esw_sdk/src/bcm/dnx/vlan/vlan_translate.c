    ...
    int action_id)
{
    ...
    if (is_ingress == TRUE)
    {
        ...
        /*
         * Don't allow action ID operation, if this action ID was allocated for FHEI size '0'
         */
        last_ivec = dnx_data_vlan.vlan_translate.nof_ingress_vlan_edit_action_ids_get(unit) - 1;
        if (action_id == last_ivec)
        {
            int is_fhei_size_reset;
            SHR_IF_ERR_EXIT(bcm_dnx_switch_control_get(unit, bcmSwitchIngressVlanEditClassNull, &is_fhei_size_reset));
            if (is_fhei_size_reset)
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "Action ID %d can't be destroyed while its FHEI size is being set to '0''\n",
                             action_id);
            }
        }
    }
    ...
    bcm_vlan_port_translation_t * vlan_port_translation)
{
    ...
    if (!_SHR_IS_FLAG_SET(vlan_port_translation->flags, BCM_VLAN_ACTION_SET_INGRESS))
    {
        ...
        {
            gport = vlan_port_translation->gport;
        }
        else
        {
    ...
    bcm_vlan_port_translation_t * vlan_port_translation)
{
    ...
    /*
     * VID_2 isn't supported by all Egress result-types.
     * LIF-Table-Mngr can't be used to deduct this information as virtual entries don't use it
     */
    rv = dbal_entry_handle_value_field32_get(unit, entry_handle_id, DBAL_FIELD_VLAN_EDIT_VID_2, INST_SINGLE,
                                             &vlan_port_translation->new_inner_vlan);
    ...
    bcm_vlan_port_translation_t * vlan_port_translation)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_vlan_port_translation_set_verify(unit, vlan_port_translation));
    ...
    bcm_vlan_port_translation_t * vlan_port_translation)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_vlan_port_translation_get_verify(unit, vlan_port_translation));
    is_ingress = _SHR_IS_FLAG_SET(vlan_port_translation->flags, BCM_VLAN_ACTION_SET_INGRESS) ? TRUE : FALSE;
    /*
     * Handle the Ingress scenario
     */
    ...
    int *action_id)
{
    ...
    SHR_FUNC_INIT_VARS(unit);
    DNX_ERR_RECOVERY_START(unit);
    SHR_INVOKE_VERIFY_DNXC(dnx_vlan_translate_action_id_create_verify(unit, flags, action_id));
    is_ingress = _SHR_IS_FLAG_SET(flags, BCM_VLAN_ACTION_SET_INGRESS) ? TRUE : FALSE;
    ...
    int action_id)
{
    ...
    if (is_ingress)
    {
         ...
         * DNX SW Algorithm, de-allocate ACTION_ID
         */
        SHR_IF_ERR_EXIT(vlan_db.action_id_ingress.free_single(unit, action_id, NULL));
    ...
    else
    {
         ...
         * DNX SW Algorithm, de-allocate ACTION_ID
         */
        SHR_IF_ERR_EXIT(vlan_db.action_id_egress.free_single(unit, action_id, NULL));
    ...
    int action_id)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_vlan_translate_action_id_destroy_verify(unit, flags, action_id));
    is_ingress = _SHR_IS_FLAG_SET(flags, BCM_VLAN_ACTION_SET_INGRESS) ? TRUE : FALSE;
    ...
    uint32 flags)
{
    ...
    SHR_FUNC_INIT_VARS(unit);
    DNX_ERR_RECOVERY_START(unit);
    SHR_INVOKE_VERIFY_DNXC(dnx_vlan_translate_action_id_destroy_all_verify(unit, flags));
    ...
    bcm_vlan_action_set_t * action)
{
    ...
    DBAL_FUNC_INIT_VARS(unit);
    DNX_ERR_RECOVERY_START(unit);
    SHR_INVOKE_VERIFY_DNXC(dnx_vlan_translate_action_id_set_verify(unit, flags, action_id, action));
    is_ingress = _SHR_IS_FLAG_SET(flags, BCM_VLAN_ACTION_SET_INGRESS) ? TRUE : FALSE;
    /*
     * Convert the VLAN Translation action from BCM API format to DNX HW info
     */
    ...
    bcm_vlan_action_set_t * action)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_vlan_translate_action_id_get_verify(unit, flags, action_id, action));
    is_ingress = _SHR_IS_FLAG_SET(flags, BCM_VLAN_ACTION_SET_INGRESS) ? TRUE : FALSE;
    /*
    ...
    bcm_vlan_translate_action_class_t * action_class)
{
    ...
    DBAL_FUNC_INIT_VARS(unit);
    DNX_ERR_RECOVERY_START(unit);
    SHR_INVOKE_VERIFY_DNXC(dnx_vlan_translate_action_class_set_verify(unit, action_class));
    ...
    bcm_vlan_translate_action_class_t * action_class)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_vlan_translate_action_class_get_verify(unit, action_class));
