    ...
    uint32 *extra_enc_hdr_len)
{
    ...
    if (next_is_itmh)
    {
        ...
        /** if lb_tst_info->int_pri is valid, take tc from lb_tst_info->int_pri */
    ...
    {   /* Prepare Ethernet header */
        ...
        *extra_enc_hdr_len = 0;
        ...
        if (DNX_OAMP_IS_V1(unit))
        {
            ...
            {
                /** Get vlan, in case of single TAG VID is stored in inner_vid */
    ...
    else
    {
        ...
        *sys_hdr_len = offset;
        *extra_enc_hdr_len = 14;        /* DA+SA+Ethertype */
    ...
    bcm_oam_tst_rx_t * tst_ptr)
{
    ...
    if (!(tst_ptr->flags & BCM_OAM_TST_RX_UPDATE))
    {
        ...
        ctf_info.identifier_trap_id = trap_id;  /* trap id */
    ...
    for (priority = 0; priority < BCM_SAT_GTF_NUM_OF_PRIORITIES; priority++)
    {
        ...
        for (i = 0; i < BCM_SAT_GTF_PACKET_LENGTH_NUM_OF_PATTERNS; i++)
        {
            ...
            p_config->packet_edit[priority].packet_length[i] = offset - (is_test_tlv ? 1 : 0) + tst_ptr->tlv.tlv_length + 1;    /* add
                                                                                                                                 * end
                                                                                                                                 * tlv */
            min_size = 60 - extra_enc_hdr_len + sys_hdr_len;    /* min size is 64B - 4B CRC - Extra header added by
                                                                 * egress PP + system header */
    ...
    bcm_oam_loopback_t * lb_ptr)
{
    ...
    if (!(lb_ptr->flags & BCM_OAM_LOOPBACK_UPDATE))
    {
         ...
         */
        ctf_info.identifier_session_id = session_id;    /* oam ID */
        ctf_info.identifier_trap_id = trap_id;  /* trap idx 1-3 */
    ...
    bcm_oam_loopback_t * lb_ptr)
{
    ...
    for (priority = 0; priority < BCM_SAT_GTF_NUM_OF_PRIORITIES; priority++)
    {
        for (i2 = 0; i2 < BCM_SAT_GTF_PACKET_LENGTH_NUM_OF_PATTERNS; i2++)
        {
            ...
            /** 
             * SAT packet length must be larger than 64
             * min size is 64B - 4B CRC - Extra header added by egress PP + system header 
            */
            min_size =
                _SHR_IS_FLAG_SET(endpoint_info.flags,
