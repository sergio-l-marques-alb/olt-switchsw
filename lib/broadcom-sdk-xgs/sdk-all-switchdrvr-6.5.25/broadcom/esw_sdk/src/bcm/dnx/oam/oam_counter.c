...
/*
 * Structure for initialization of table for IPPB_OAM_COUNTER_DISABLE_MAP, ETPPC_OAM_COUNTER_DISABLE_MAP
 * for devices without hlm support.
 * The tables maps the keys to counter disable bit
 */
typedef struct
{
     ...
     */
    int dis_cnt_access;
} oam_counter_disable_wo_hlm_table_table_entry_t;
    ...
    uint32 counter_disable)
{
    ...
    /** setting key fields */
    if (dnx_data_oam.general.oam_nof_counter_interfaces_get(unit) > 1)
    {
    ...
    int unit)
{
    ...
    dbal_enum_value_field_mp_type_e mp_type = 0;
    int dis_cnt_access_0, dis_cnt_access_1, dis_cnt_access_2;
    int entry = 0;
    int nof_oam_counter_disable_table_entries = 0;

    SHR_FUNC_INIT_VARS(unit);

    dis_cnt_access_0 = OAM_COUNTER_DIS;
    dis_cnt_access_1 = OAM_COUNTER_DIS;
    dis_cnt_access_2 = OAM_COUNTER_DIS;
