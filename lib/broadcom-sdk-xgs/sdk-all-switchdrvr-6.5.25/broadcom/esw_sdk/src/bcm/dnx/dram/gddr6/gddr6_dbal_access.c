/** \file src/bcm/dnx/dram/gddr6/gddr6_dbal_access.c
 *
 *
 *  This file contains the implementation of dbal access
 *  functions of the gddr6 module.
 *
 */
    ...
    int ignore_vendor)
{
    ...
    /** DramWriteLatency.set(drc_info->real_write_latency); TAKEN FROM SOC PROPERTY*/
    ...
    /** DramDbidcRead.set(drc_info->real_rdbi); TAKEN FROM SOC PROPERTY */
    ...
    /** DramDbidcWrite.set(drc_info->real_wdbi); TAKEN FROM SOC PROPERTY */
    ...
    /** CABI is always on - this is to allow proper init sequence without dealing with init MRs in specific order */
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_DRAM_CABIDC, INST_SINGLE, 1);
    /** DramWrCrc.set(!drc_info->wr_crc ) TAKEN FROM SOC PROPERTY -drc_info->wr_crc 0 is on.*/
    ...
    /** DramRdCrc.set(!drc_info->rd_crc ) TAKEN FROM SOC PROPERTY -drc_info->rd_crc 0 is on.*/
    ...
    /** HbmBankGroupsEn.set(drc_info->bank_groups!=0 ) .*/
    ...
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_DRAM_INDEX, dram_index);
    dbal_entry_key_field32_range_set(unit, entry_handle_id, DBAL_FIELD_CHANNEL, DBAL_RANGE_ALL, DBAL_RANGE_ALL);
    /** CommandPipeExtraDelay.set(drc_info->command_pipe_extra_delay) .*/
    ...
    /** OutputEnableDelay.set(drc_info->real_write_latency - drc_info->command_pipe_extra_delay) .*/
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_OUTPUT_ENABLE_DELAY, INST_SINGLE,
                                 ((dnx_data_dram.general_info.write_latency_get(unit) +
                                   write_latency_vendor_specific_variation) -
                                  dnx_data_dram.gddr6.command_pipe_extra_delay_get(unit)));
    /** OutputEnableLength.set(3) .*/
    ...
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_DRAM_INDEX, dram_index);
    dbal_entry_key_field32_range_set(unit, entry_handle_id, DBAL_FIELD_CHANNEL, DBAL_RANGE_ALL, DBAL_RANGE_ALL);
    /** HbmTwr.set(drc_info->real_write_recovery+drc_info->real_write_latency+`BURST_LENGTH_DIV_4); */
    ...
    /** DramWrToRdap.set(drc_info->real_write_recovery+drc_info->real_write_latency+`BURST_LENGTH_DIV_4);*/
    ...
    /** update dram mode registers when it is changed in controller */
    ...
    /** enable force update per mode registers bitmap - always send a command to the MR, even if values are the same */
    SHR_IF_ERR_EXIT(dnx_gddr6_dbal_access_force_update_per_mode_register_bitmap_set(unit, dram_index, 0xffff));

    /** disable masking update per mode registers bitmap - no need to mask and mode registers from changing */
    SHR_IF_ERR_EXIT(dnx_gddr6_dbal_access_mask_update_per_mode_register_bitmap_set(unit, dram_index, 0x0000));

    /** take MRs out of reset */
    ...
    /** dram bist bank reorder - reorder in that wat to make sure that 2 consecutive transactions are going to different bank groups */
    ...
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_DRAM_INDEX, dram_index);
    dbal_entry_key_field32_range_set(unit, entry_handle_id, DBAL_FIELD_CHANNEL, DBAL_RANGE_ALL, DBAL_RANGE_ALL);
    /** the refiab set to HW should be MIN(trefiab - 100000, dynamic_calibration_period)*/
    field_val =
    ...
    uint32 cke_ui)
{
    ...
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_DRAM_INDEX, dram_index);
    dbal_entry_key_field32_range_set(unit, entry_handle_id, DBAL_FIELD_CHANNEL, DBAL_RANGE_ALL, DBAL_RANGE_ALL);
    /** actual field is CKE_N (CKE not), so need to flip input */
...
/* see header file */
shr_error_e
dnx_gddr6_dbal_access_wck_disable(
    int unit,
    int dram_index,
    int wck_index)
{
    ...
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_DRAM_INDEX, dram_index);
    dbal_entry_key_field32_range_set(unit, entry_handle_id, DBAL_FIELD_CHANNEL, DBAL_RANGE_ALL, DBAL_RANGE_ALL);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_WCK_INDEX, wck_index);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_WCK_DISABLE, INST_SINGLE, 1);
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
...
/*
 * brief - configure average read inflights leaky bucket for DRAM block
 */
static shr_error_e
    ...
    int unit)
{
    ...
    field_count = dnx_data_dram.dram_block.gddr6_average_read_inflights_leaky_bucket_config_params_info_get
        (unit)->key_size[0];
    ...
    {
        field_id = dnx_data_dram.dram_block.gddr6_average_read_inflights_leaky_bucket_config_params_get
            (unit, i)->dbal_field_id;
        field_val = dnx_data_dram.dram_block.gddr6_average_read_inflights_leaky_bucket_config_params_get
            (unit, i)->field_val;
        dbal_entry_value_field32_set(unit, entry_handle_id, field_id, INST_SINGLE, field_val);
    }
    ...
    int unit)
{
    ...
    {
        /** if dram bitmap has only second dram, configure data source id in a different way */
    ...
    int set_init_position)
{
    ...
    /** get VDL step soze and UI size */
    SHR_IF_ERR_EXIT(soc_g6phy16_calculate_step_size(unit, dram_index, &step_size));

    /** configure training bist for dynamic calibration */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_GDDR6_TRAINING_BIST_COMMAND_CONFIG, &entry_handle_id));
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_DRAM_INDEX, dram_index);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CHANNEL, channel);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_DRAM_READ_FIFO_DEPTH, INST_SINGLE,
    ...
    /** configure dynamic calibration parameters */
    ...
    /** set phy vdl register address according to channel */
    for (int byte = 0; byte < dnx_data_dram.gddr6.bytes_per_channel_get(unit); ++byte)
    {
        ...
        if (set_init_position)
        {
            ...
            /**
             * determine starting VDL positions as known working positions by reading current configuration from PHY and
             * normalizing it to single VDL coordinate instead of VDL and UI_SHIFT coordinates
             */
            ...
            /** set normalized VDL position as starting point */
    ...
    if (set_init_position)
    {
        /** toggle Load command to load starting VDL position */
    ...
    uint32 enable)
{
    ...
    /** set dynamic calibration enable state */
    ...
    {
        /**
         * save in sw_state that dynamic calibration was enabled.
         * This is needed to prevent users from running DRAM BIST if the
         * dynamic calibration was enabled any time after the dram tune.
         */
    ...
    uint32 *enable)
{
    ...
    /** get dynamic calibration enable state */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_GDDR6_DYNAMIC_CALIBRATION_CONFIG, &entry_handle_id));
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_DRAM_INDEX, dram_index);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CHANNEL, channel);
    ...
    dnx_gddr6_bist_configuration_t * bist_conf)
{
    ...
    /** BIST_DQ_PATTERN_N is spread over two tables since it is too big. first four patterns are added to part_1, last
     *  four patterns arr added to Part_2 */
    ...
    for (pattern_index = 0; pattern_index < SHMOO_G6PHY16_BIST_NOF_PATTERNS / 2; ++pattern_index)
    {
        ...
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_BIST_DBI_PATTERN_N, pattern_index,
                                     bist_conf->bist_dbi_pattern[pattern_index + 4]);
    }
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    ...
    uint32 entry_handle_id)
{
    ...
    switch (mr_index)
    {
        ...
        case 7:
        {
            ...
            /** DQ Preamble is supported only for Samsung drams */
            field_in_struct_val =
        ...
        case 8:
        {
            ...
            field_in_struct_val = dnx_data_dram.gddr6.ck_termination_get(unit);
            SHR_IF_ERR_EXIT(dbal_fields_struct_field_encode
