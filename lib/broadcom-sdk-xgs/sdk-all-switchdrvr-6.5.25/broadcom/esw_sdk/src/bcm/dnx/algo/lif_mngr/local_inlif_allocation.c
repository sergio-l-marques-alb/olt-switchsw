    ...
    int *element)
{
    ...
    else
    {
        ...
        {
            SHR_IF_ERR_EXIT(dnx_lif_local_inlif_resource_free(unit, module_id, res_tag_bitmap, *element, NULL));
        }
    ...
    void *extra_argument)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_lif_local_inlif_resource_free_verify(unit, entry_size, element));
    ...
    lif_mngr_local_inlif_info_t * inlif_info)
{
    ...
    {
        /*
         * For symmetric core allocation mode - inlif_table_dpc_all_cores is not used 
         */
    ...
    lif_mngr_local_inlif_info_t * inlif_info)
{
    ...
    {
        SHR_IF_ERR_EXIT(lif_mngr_db.local_inlif.
                        inlif_table_shared_by_cores.free_single(unit, inlif_info->local_inlif, NULL));
    }
    else if (physical_table_id == DBAL_PHYSICAL_TABLE_INLIF_2)
    {
        int current_core;
        /*
         * In symmetric core mode - only dpc_all_cores db is used
         */
        if (!DNXCMN_CHIP_IS_MULTI_CORE(unit))
        {
        ...
        else
        {
            ...
            if (free_all_cores)
            {
                 ...
                 */
                SHR_IF_ERR_EXIT(lif_mngr_db.local_inlif.inlif_table_dpc_all_cores.free_several(unit, 2,
                                                                                               inlif_info->local_inlif,
                                                                                               NULL));
            }
        }
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_lif_mngr_nof_inlifs_get(unit, &nof_sbc_lifs, &nof_dpc_lifs));

    /*
     * Allocate local SBC resources
     */
    if (nof_sbc_lifs)
    {
        res_data.first_element = 0;
        res_data.flags = DNX_ALGO_RES_CREATE_USE_ADVANCED_ALGORITHM;
        res_data.nof_elements = nof_sbc_lifs;
        ...
        if (is_std_1)
        {
             ...
             */
            ac_table = DBAL_TABLE_IN_AC_INFO_DB;
            ac_result_type = DBAL_RESULT_TYPE_IN_AC_INFO_DB_IN_LIF_AC_P2P_LARGE;

            SHR_IF_ERR_EXIT(dnx_algo_local_inlif_get_element_size
                            (unit, ac_table, DBAL_RESULT_TYPE_IN_AC_INFO_DB_IN_LIF_AC_P2P_LARGE, &entry_size_indexes));
            ...
            SHR_IF_ERR_EXIT(dnx_lif_mngr_inlif_sw_info_table_id_and_result_type_set
                            (unit, allocated_value, _SHR_CORE_ALL, ac_table, ac_result_type));
        }
    }

    /*
     * Allocate local DPC resources
    ...
    res_data.nof_elements = nof_dpc_lifs - res_data.first_element;
    /*
    ...
    /*
     * For symmetric core allocation mode - all the lifs are allocated on all cores and managed in the same manner, no
     * need to manage the dedicated_per_core db as it won't be used 
     */
    if (DNXCMN_CHIP_IS_MULTI_CORE(unit))
    {
        /*
        ...
        SHR_IF_ERR_EXIT(lif_mngr_db.local_inlif.inlif_table_dpc_all_cores_indication.alloc_bitmap(unit, nof_dpc_lifs));
    }
