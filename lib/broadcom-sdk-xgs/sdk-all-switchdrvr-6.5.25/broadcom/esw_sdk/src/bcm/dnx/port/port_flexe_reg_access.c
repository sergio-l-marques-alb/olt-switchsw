/*
 * $Id: port_flexe_reg_access.c,v 1.30 Broadcom SDK $
 *
 ...
 *  MACROS   *
 *************/
#define FLEXE_ACCESS_LOCK(unit, _lock) do                   \
        {                                                   \
            if (_lock != NULL)                              \
            {                                               \
...
#define FLEXE_ACCESS_UNLOCK(unit, _lock) do     \
        {                                       \
            if (_lock != NULL)                  \
            {                                   \
...
dnx_flexe_reg_read(
    ...
    uint32 *data)
{
    sal_mutex_t flexe_lock = NULL;
    soc_custom_reg_access_t *reg_access;
    SHR_FUNC_INIT_VARS(unit);

    reg_access = dnx_drv_soc_info_custom_reg_access_get(unit);
    flexe_lock = reg_access->flexe_access_lock;
    FLEXE_ACCESS_LOCK(unit, flexe_lock);
    ...
    SHR_IF_ERR_EXIT(_dnx_flexe_reg_read_unlock(unit, reg_addr, data));

exit:
    FLEXE_ACCESS_UNLOCK(unit, flexe_lock);
    SHR_FUNC_EXIT;
}
    ...
    uint32 data)
{
    ...
    SHR_IF_ERR_EXIT(_dnx_flexe_reg_write_unlock(unit, reg_addr, data));
    ...
    uint32 *data)
{
    ...
    SHR_IF_ERR_EXIT(_dnx_flexe_reg_write_unlock(unit, table_base_addr + 0, header));
    SHR_IF_ERR_EXIT(_dnx_flexe_reg_write_unlock(unit, table_base_addr + 1, ram_addr));
    SHR_IF_ERR_EXIT(_dnx_flexe_reg_write_unlock(unit, table_base_addr + 3, entry_idx));

    for (i = width_32; i >= 1; i--)
    {
        if (i == 1)
        {
            SHR_IF_ERR_EXIT(_dnx_flexe_reg_read_unlock(unit, table_base_addr + 0xfd, data));
            break;
        }
        else
        {
            SHR_IF_ERR_EXIT(_dnx_flexe_reg_read_unlock(unit, table_base_addr + 0xfc, data + i - 1));
        }
    }
    ...
    void *entry_data)
{
    ...
    {
        rc = _soc_mem_read_cache_attempt(unit, flags | SOC_MEM_IGNORE_FORCE_READ_THROUGH, mem, copyno, entry_index,
                                         array_index, entry_data, NULL, &cache_consistency_check);
        if (rc == TRUE)
        {
            SHR_EXIT();
        }
