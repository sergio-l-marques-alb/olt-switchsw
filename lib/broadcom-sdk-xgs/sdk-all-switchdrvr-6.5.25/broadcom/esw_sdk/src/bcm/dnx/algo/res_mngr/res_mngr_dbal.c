    ...
    {
        {
            /** Validate that the element is allocated for the requested table */
            int rv;
            rv = dnx_lif_mngr_outlif_sw_info_get(unit, out_lif, &rec_table_id, &rec_result_type, NULL, NULL, NULL,
                                                 NULL);
    ...
    while ((*in_lif) != DNX_ALGO_RES_ILLEGAL_ELEMENT && (!is_found))
    {
        ...
        is_found = FALSE;
            ...
            else
            {
                ...
                {
                    SHR_IF_ERR_EXIT(lif_mngr_db.local_inlif.
                                    inlif_table_dedicated_per_core.get_next(unit, core_id, in_lif));
                }
            }
    ...
    int is_alloc)
{
    ...
    if (is_alloc)
    {
        ...
        SHR_IF_ERR_EXIT(algo_l3_db.source_address_table_allocation.source_address_table.exchange
                        (unit, 0, &source_address_entry, SOURCE_ADDRESS_TABLE_DEFAULT_PROFILE, NULL, &new_profile,
                         &is_first, &is_last));
    }
    else
    {
    ...
    int *index)
{
    ...
    rv = dnx_algo_l3_fec_allocate(unit, &fec_id, flags, fec_db, DBAL_ENUM_FVAL_FEC_RESOURCE_TYPE_NO_PROT_NO_STAT);
    ...
    int result_type)
{
    ...
    SHR_IF_ERR_EXIT(lif_mngr_db.egress_local_lif.free_several(unit, outlif_bank, deallocation_size, index, NULL));
        ...
        case DBAL_FIELD_TYPE_DEF_IN_LIF:
        {
            ...
            SHR_IF_ERR_EXIT(dbal_tables_physical_table_get
                ...
                else if (physical_table_id == DBAL_PHYSICAL_TABLE_INLIF_2)
                {
                    ...
                    {
                        SHR_IF_ERR_EXIT(lif_mngr_db.local_inlif.
                                        inlif_table_dpc_all_cores.free_single(unit, index, NULL));
                    }
                    else
                    {
                        SHR_IF_ERR_EXIT(lif_mngr_db.local_inlif.
