...
#include <src/bcm/dnx/flow/porting/esem_flow.h>
...
#define RING_PROTECTION_MACT_GROUP_IN_JR1_MODE 1

#define MAX_NUMBER_OF_DUMMY_ALLOCATIONS_FOR_PPPOE       (30)

/** VSI max value when enable 16bits wide data, only 15bits*/
#define DNX_VSI_MAX_FOR_16BITS_WIDE_DATA    0x7fff
/**
 * \brief
...
/**
 * \brief
 *   Holds information for egress table ids (outlif) and the
...
typedef struct
{
     ...
     */
    dnx_algo_local_outlif_logical_phase_e outlif_phase;
    /**
     * \brief
    ...
    uint8 *is_allocated)
{
    ...
    *is_allocated = FALSE;
    ...
    if (BCM_GPORT_SUB_TYPE_IS_VIRTUAL_INGRESS_NATIVE(gport))
    {
        ...
        if (dnx_data_flow.vlan_port.feature_get(unit, dnx_data_flow_vlan_port_flow_in_use))
        {
            /** It is checked later on by the FLOW module */
            *is_allocated = TRUE;
            SHR_EXIT();
        }
    ...
    bcm_vlan_port_t * vlan_port)
{
    ...
    switch (vlan_port->criteria)
    {
        case BCM_VLAN_PORT_MATCH_NONE:
            ...
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "Error ! criteria = BCM_VLAN_PORT_MATCH_NONE (%d) but match_ethertype (%d) is not 0! flags = 0x%08X \n",
                             vlan_port->criteria, vlan_port->match_ethertype, vlan_port->flags);
            }
            break;
        ...
        case BCM_VLAN_PORT_MATCH_PORT_VLAN_STACKED:
            ...
            {
                /*
                 * TCAM lookup is not supported for Native -  verify match_ethertype is not set!
                 */
                if (vlan_port->match_ethertype)
                {
                    SHR_ERR_EXIT(_SHR_E_PARAM,
                                 "Error ! Ingress Native does not support Ethernet type match! match_ethertype = %d, flags = 0x%08X, criteria = %d\n",
                                 vlan_port->match_ethertype, vlan_port->flags, vlan_port->criteria);
                }
            }
    ...
    switch (vlan_port->criteria)
    {
        ...
        case BCM_VLAN_PORT_MATCH_PORT_UNTAGGED:
            ...
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "Error ! criteria = %d but match_vlan = %d is not 0! flags = 0x%08X.\n",
                             vlan_port->criteria, vlan_port->match_vlan, vlan_port->flags);
            }

            if (vlan_port->match_inner_vlan != 0)
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "Error ! criteria = %d but match_inner_vlan = %d is not 0! flags = 0x%08X\n",
                             vlan_port->criteria, vlan_port->match_inner_vlan, vlan_port->flags);
            }
        ...
        case BCM_VLAN_PORT_MATCH_NAMESPACE_VSI:
        {
            ...
            SHR_IF_ERR_EXIT(dbal_tables_field_max_value_get
                            (unit, DBAL_TABLE_ESEM_FORWARD_DOMAIN_MAPPING_DB, DBAL_FIELD_VLAN_DOMAIN, TRUE, 0, 0,
        ...
        case BCM_VLAN_PORT_MATCH_NAMESPACE_PORT:
        {
            ...
            SHR_IF_ERR_EXIT(dbal_tables_field_max_value_get
                            (unit, DBAL_TABLE_ESEM_PORT_BASED_SOURCE_ADDRESS_DB, DBAL_FIELD_VLAN_DOMAIN, TRUE, 0, 0,
...
/**
 * \brief - QOS Verify function for dnx_vlan_port_create/replace
 */
static shr_error_e
dnx_vlan_port_create_qos_verify(
    int unit,
    bcm_vlan_port_t * vlan_port)
{
    SHR_FUNC_INIT_VARS(unit);
        ...
        (vlan_port->ingress_qos_model.ingress_ecn != bcmQosIngressEcnModelInvalid))
    {
        ...
        if (_SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_RECYCLE))
        {
            ...
            if ((vlan_port->ingress_qos_model.ingress_ecn != bcmQosIngressEcnModelInvalid) &&
                (vlan_port->ingress_qos_model.ingress_ecn != bcmQosIngressEcnModelEligible))
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "Ingress ECN must be bcmQosIngressEcnModelInvalid or bcmQosIngressEcnModelEligible");
            }
        }
        else
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "Ingress qos model is supported only for Control LIF - flag BCM_VLAN_PORT_RECYCLE (0x%08X) is not set! flags = 0x%08X ingress_phb = %d, ingress_remark = %d, ingress_ttl = %d, ingress_ecn = %d",
                         ...
                         vlan_port->ingress_qos_model.ingress_ecn);
        }
    }
    /**egress qos*/
    ...
    int is_virtual_ac)
{
    ...
    supported_flags =
        ...
        BCM_VLAN_PORT_RECYCLE | BCM_VLAN_PORT_EXTENDED | BCM_VLAN_PORT_IVL;

    if (dnx_data_switch.svtag.supported_get(unit))
    {
        /** Add supported SVTAG flags*/
        supported_flags |= BCM_VLAN_PORT_EGRESS_SVTAG_PORT_ENABLE | BCM_VLAN_PORT_EGRESS_SVTAG_ENABLE;
        if ((_SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_EGRESS_SVTAG_ENABLE)) &&
            (_SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_EGRESS_SVTAG_PORT_ENABLE)))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Error: invalid use of flags, the flags BCM_VLAN_PORT_EGRESS_SVTAG_ENABLE and "
                         "BCM_VLAN_PORT_EGRESS_SVTAG_PORT_ENABLE can't be used together");
        }
    }
    ...
    /*
     * Verify that port is not FEC
     */
    if (BCM_GPORT_IS_FORWARD_PORT(vlan_port->port))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "Error! Unsupported port = 0x%08x is FEC. Use failover_port_id for setting learn information to a FEC\n",
                     vlan_port->port);
    }
    ...
    SHR_IF_ERR_EXIT(dnx_vlan_port_create_qos_verify(unit, vlan_port));
    ...
    /*
     * BCM_VLAN_PORT_EXTENDED flag verification:
     */
    if (_SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_EXTENDED))
    {
        if (_SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_CREATE_INGRESS_ONLY) ||
            _SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_VLAN_TRANSLATION))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "Error, flags = 0x%08X! BCM_VLAN_PORT_EXTENDED can't be used for ingress only AC and virtrual AC!\n",
                         vlan_port->flags);
        }
    }
    ...
    if (_SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_VLAN_TRANSLATION))
    {
        ...
        else
        {
            ...
            else
            {
                ...
                /*
                 * ESEM in a non direct access (lookup) entry thus cannot create two entires with same key!
                 *Verify that if the entry is already created then this call is with REPLACE flag!
                 * Note:
                 * For namespace-vsi, the entry can be created by AC API ro by VXLAN API.
                 * Thus, here we are actualy checking if a gport was created for these keys!
                 * 
                 * Note:
                 * Since the check is heavy (running iterator on large SW DB), only namespace-vsi is checked.
                 * All other cases  will eventually fail when accessing the HW using DBAL.
                 */
                if (vlan_port->criteria == BCM_VLAN_PORT_MATCH_NAMESPACE_VSI)
                {
                    int rv;
                    ...
                    if (rv == _SHR_E_NONE)
                    {
                        SHR_ERR_EXIT(_SHR_E_EXISTS,
                                     "Error! This AC gport = 0x%08X already created! need to call this API with REPLACE (0x%08x) flag! flag = 0x%08X \n",
                                     vlan_port->vlan_port_id, BCM_VLAN_PORT_REPLACE, vlan_port->flags);
                    }
                }
            }
     ...
     */
    if ((ll_app_type == SERVICE_OPTIMIZATION_AC_APPLICATION_TYPE) || (ll_app_type == OPTIMIZATION_AC_APPLICATION_TYPE))
    {
        /*
         * AC Optimization isn't relevant for EGRESS Only, check that BCM_VLAN_PORT_CREATE_EGRESS_ONLY isn't set
         */
        if (is_ingress == FALSE)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "AC Optimization isn't relevant for EGRESS Only,"
                         "Error! BCM_VLAN_PORT_VSI_BASE_VID flag isn't for egress only, can't set BCM_VLAN_PORT_CREATE_EGRESS_ONLY."
                         "flags = 0x%08X\n", vlan_port->flags);
        }
    ...
    if (is_ingress == TRUE)
    {
         ...
         */
        uint32 group_max_value = SAL_UPTO_BIT(dnx_data_l2.dma.flush_group_size_get(unit));
        if (vlan_port->group > group_max_value)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "group exceeded allowed range [0:%d]\n", group_max_value);
        }
    }

    /*
     * MC Group is a learning attribute, therefore not applicable for a Non-Service
     */
    if ((ll_app_type == VLAN_TRANSLATION_AC_APPLICATION_TYPE) || (ll_app_type == OPTIMIZATION_AC_APPLICATION_TYPE))
    {
    ...
    /*
     * Verify protection/learn-destination input parameters
     * In case of FEC Protection verify the FEC gport 
     * Otherwise, the field can accept a phy=port learn destination
     */
    if (vlan_port->failover_port_id)
    {
        if (_SHR_GPORT_IS_FORWARD_PORT(vlan_port->failover_port_id))
        {
            BCM_GPORT_FORWARD_GROUP_TO_L3_ITF_FEC(fec, vlan_port->failover_port_id);
            if (fec == -1)
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "Error: Invalid FEC Gport for FEC protection, failover_port_id = 0x%08X\n",
            ...
            else
            {
                 ...
                 */
                int min_fec_id_value = dnx_data_l3.fec.first_valid_fec_ecmp_id_get(unit);
                SHR_IF_ERR_EXIT(mdb_db_infos.max_fec_id_value.get(unit, &max_fec_id_value));
                fec_id = BCM_L3_ITF_VAL_GET(fec);
                SHR_RANGE_VERIFY(fec_id, min_fec_id_value, max_fec_id_value, _SHR_E_PARAM, "FEC ID is out of bound.\n");
            }
        }
        else if (_SHR_GPORT_IS_MCAST(vlan_port->failover_port_id))
        {
            /*
             * Don't allow a MC gport as a learn destination
             */
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "Error: Invalid Multicast gport for learn destination, failover_port_id = 0x%08X\n",
                         vlan_port->failover_port_id);
        }
        else
        {
            /*
             * Allow a physical port gport as a learn destination, but not a logical gport
             */
            uint8 is_physical_port;
            SHR_IF_ERR_EXIT(dnx_algo_gpm_gport_is_physical(unit, vlan_port->failover_port_id, &is_physical_port));
            if (!is_physical_port)
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "Error: Invalid logical interface gport for learn destination, failover_port_id = 0x%08X\n",
                             vlan_port->failover_port_id);
            }
        }

        /*
         * At the FEC protection/learn-destination case the failover MC group should not be set
         */
        if (vlan_port->failover_mc_group)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "When failover_port_id is set, the failover MC group should not be set. failover_mc_group(%u)\n",
                         vlan_port->failover_mc_group);
        }
    }
    else
    ...
    /*
     * Verify port
     * Remote system port is not supprted.
     * Note: the flag "PP_PORT_IS_MANDATORY" forces pp-port retrive thus for remote system port
     * we will get an error indication.
     * Note:
     * For Native LIF, the port is the gport of the PWE LIFs thus can only check for non-native AC LIF.
     */
    if (!_SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_NATIVE))
    {
        /*
    ...
    if (vlan_port->vsi == 0)
    {
        ...
        int is_egress_esem_namespace_port = (!is_symmetric) && is_egress
            && _SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_VLAN_TRANSLATION) &&
            (vlan_port->criteria == BCM_VLAN_PORT_MATCH_NAMESPACE_PORT);
        int is_esem_match = is_egress && _SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_VLAN_TRANSLATION) &&
            !_SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_DEFAULT);

        if ((!is_symmetric) && (!is_ingress_only) &&
            (!is_esem_default) && (!is_egress_only_non_virtual) && (!is_egress_esem_namespace_port) && (!is_esem_match))
        {
    ...
    {
        if ((vlan_port->group != RING_PROTECTION_MACT_GROUP_IN_JR1_MODE) && (vlan_port->group != 0))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Invalid group:%d at Jericho System header mode, expected value:0 or 1\n",
                         vlan_port->group);
        }

        if (vlan_port->group == RING_PROTECTION_MACT_GROUP_IN_JR1_MODE)
        {
            if (ll_app_type != SERVICE_AC_APPLICATION_TYPE && ll_app_type != SERVICE_OPTIMIZATION_AC_APPLICATION_TYPE)
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "Error, IOP Ring-Protection is Invalid for Non-Service ACs");
            }
            /*
             * In case of FEC and MC Protection, forwarding information is dest_only. 
             * But Ring-Protection needs dest+outlif and it's identify by group = 1 and failover_port_id != 0.
             */
            if (vlan_port->failover_mc_group)
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "Error, Doesn't support Ring-Protection work with ingress_protection");
            }
            if (!(vlan_port->flags & BCM_VLAN_PORT_INGRESS_WIDE) && (vlan_port->failover_port_id == 0))
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "Error, Ring-Protection needs group = 1 and failover_port_id != 0");
            }
    ...
    bcm_vlan_port_t * old_vlan_port)
{
    ...
    supported_flags =
        ...
        BCM_VLAN_PORT_EGRESS_SVTAG_ENABLE | BCM_VLAN_PORT_EXTENDED | BCM_VLAN_PORT_EGRESS_SVTAG_PORT_ENABLE;
    ...
    if ((new_vlan_port->flags & ~supported_flags) != ((old_vlan_port->flags & ~supported_flags)))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Error! VLAN-Port REPLACE: Cannot change flags!\n"
                     ...
                     "BCM_VLAN_PORT_INGRESS_WIDE(0x%08X), BCM_VLAN_PORT_STAT_INGRESS_ENABLE(0x%08X), "
                     "BCM_VLAN_PORT_EGRESS_SVTAG_ENABLE(0x%08X) or BCM_VLAN_PORT_EGRESS_SVTAG_PORT_ENABLE(0x%08X)!\n",
                     ...
                     BCM_VLAN_PORT_EGRESS_SVTAG_ENABLE, BCM_VLAN_PORT_EGRESS_SVTAG_PORT_ENABLE);
    }
    ...
    if (is_ingress)
    {
        ...
        else
        {
            ...
            {
                /*
                 * For non-native: port must be physical.
                 * For native: port is not physical.
                 * Here - port is not physical and the LIF is not native ... something is wrong ...
                 */
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "Error! VLAN-Port REPLACE: Wrong setting! port = 0x%08X is not a physical port and "
                             "vlan_port_id = 0x%08X is not Native AC!\n",
                             new_vlan_port->port, new_vlan_port->vlan_port_id);
            }
        }
         ...
         */
        if ((is_egress) && (!(_SHR_IS_FLAG_SET(old_vlan_port->flags, BCM_VLAN_PORT_VSI_BASE_VID))) &&
 ...
 *
 * Note:
 * created_by_ac is only relevant for NAMESPACE VSI (because the entry can be created by
 * bcm_vxlan_network_domain_config_add)
 *
 */
static shr_error_e
        ...
        case BCM_VLAN_PORT_MATCH_PORT_VLAN:
        {
            ...
            if (is_native)
            {
                ...
                if (is_native_intf_name_space == TRUE)
                {
                    ...
                    /** zero nwk_qos_idx in case it is invalid */
                    nwk_qos_idx = (egress_ac_table_info->nwk_qos_idx == DNX_QOS_INVALID_MAP_ID) ?
                        0 : egress_ac_table_info->nwk_qos_idx;

                    *resultType = DBAL_RESULT_TYPE_ESEM_FORWARD_DOMAIN_MAPPING_PER_INTERFACE_NAMESPACE_DB_ETPS_AC_STAT;
                    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_RESULT_TYPE, INST_SINGLE,
                                                 *resultType);
        ...
        case BCM_VLAN_PORT_MATCH_NAMESPACE_VSI:
        {
            ...
            else if (retVal == _SHR_E_NONE)
            {
                ...
                if (!is_replace)
                {
                    ...
                    /** Only if it is NOT REPLACE, the SW DB is updated so setting it to FALSE only here */
                    *created_by_ac = FALSE;
                }
...
{
    uint32 entry_handle_id;
    bcm_gport_t local_gport = vlan_port->vlan_port_id;
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);
    /*
     * Take table handle
     ...
     * Set key:
     */
    if (dnx_data_flow.algo_gpm.flow_gport_in_use_get(unit))
    {
        SHR_IF_ERR_EXIT(algo_gpm_gport_flow_convert
                        (unit, vlan_port->vlan_port_id, _SHR_GPORT_TYPE_FLOW_LIF, &local_gport));
    }
    ...
    switch (vlan_port->criteria)
    {
        ...
        case BCM_VLAN_PORT_MATCH_NAMESPACE_VSI:
        {
            ...
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_HW_RESULT_TYPE, INST_SINGLE, resultType);
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_CREATED_BY_AC, INST_SINGLE, created_by_ac);
...
/**
 * \brief - Check if the translation from vlan port to forward information is needed.
 *
 * \param [in] unit - the relevant unit.
 ...
 */
static int
vlan_port_gport_to_forward_information_needed(
    int unit,
    bcm_vlan_port_t * vlan_port)
{
    int forward_info_is_needed;

    /*
     * The vlan_port gport need to be translated to forwarding information in following cases:
     * 1) Non native AC;
     * 2) Gliful symmetric Native AC.
     */
    if (!_SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_NATIVE))
    {
        forward_info_is_needed = TRUE;
    }
    else if (dnx_data_l2.feature.feature_get(unit, dnx_data_l2_feature_learn_payload_native_update_enable) &&
             !_SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_VLAN_TRANSLATION) &&
    ...
    {
        forward_info_is_needed = FALSE;
    }

    return forward_info_is_needed;
}
 ...
 * \param [in] system_headers_mode - system headers mode: JR1 or JR2
    ...
    dnx_algo_gpm_forward_info_t * forward_info)
{
    ...
    {
        /*
         * By default Forwarding is done according to destination and Outlif
         */
        if ((vlan_port->group == RING_PROTECTION_MACT_GROUP_IN_JR1_MODE)
        ...
        else if (_SHR_GPORT_IS_FORWARD_PORT(vlan_port->failover_port_id) || vlan_port->failover_mc_group)
        {
            /*
             * In case of FEC and MC Protection Global OutLIF must be 0.
             * In IOP mode, forwarding information couldn't be dest_only because MACT only support Dest+out_LIF, FEC+EEI
             * So dest_only is convert to dest_outlif with outlif is 0. 
             */
            if (system_headers_mode == DBAL_ENUM_FVAL_SYSTEM_HEADERS_MODE_JERICHO_MODE)
            {
    ...
    SHR_IF_ERR_EXIT(algo_gpm_gport_l2_forward_info_add(unit,
                                                       _SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_REPLACE), TRUE,
    ...
    int is_replace)
{
    ...
    if (dnx_data_flow.algo_gpm.flow_gport_in_use_get(unit))
    {
        SHR_IF_ERR_EXIT(algo_gpm_gport_flow_convert(unit, vlan_port_id, _SHR_GPORT_TYPE_FLOW_LIF, &local_gport));
    }
    /*
     * For Native Virtual:
    ...
    dnx_ingress_ac_table_info_t * ingress_ac_table_info)
{
    ...
    {
        /*
         * 1. Contain flush group, always use extend learn info
         * 2. For Wide-data lif, contain protection with VSI more than 15bits, use extend learn info
         */
        if ((vlan_port->group) || DNX_FAILOVER_IS_PROTECTION_ENABLED(vlan_port->ingress_failover_id))
        {
        ...
        {
            /*
             * For wide-data with 28 bits, not contain protection, it use small size of learn field.
             * For wide-data with 16 bits, contain protection, it use small size of learn field + 15bits vsi
             */
            if (DNX_FAILOVER_IS_PROTECTION_ENABLED(vlan_port->ingress_failover_id)
                && (vlan_port->vsi <= DNX_VSI_MAX_FOR_16BITS_WIDE_DATA) && !(vlan_port->group))
            {
        ...
        if ((vlan_port->group == RING_PROTECTION_MACT_GROUP_IN_JR1_MODE) &&
            ...
            && !_SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_INGRESS_WIDE))
        {
            /*
             * Learn information couldn't support group when system header is jericho.
             */
            ingress_ac_table_info->learn_info_type = LIF_LEARN_INFO_REQ_TYPE_BASIC_LIF;
        }
    }
    ...
    dnx_egress_ac_table_info_t * egress_ac_table_info)
{
    ...
    if (_SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_NATIVE))
    {
        egress_ac_table_info->outlif_phase = DNX_ALGO_LOCAL_OUTLIF_LOGICAL_PHASE_3;
    }
    /*
     * Outer AC Database
     */
    ...
    {
        egress_ac_table_info->outlif_phase = DNX_ALGO_LOCAL_OUTLIF_LOGICAL_PHASE_8;
    }
    ...
    int is_native)
{
    ...
    {
        /*
         * If there were no requirements regarding the the learn-info content 
         ...
         */
        if (ingress_ac_table_info->learn_info_type == LIF_LEARN_INFO_REQ_TYPE_NO_REQUIREMENT)
        {
            if ((learn_context == DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_SYMMETRIC_LIF_OPTIMIZED_SVL) ||
                ...
                (learn_context == DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_ASYMMETRIC_LIF_OPTIMIZED_SVL))
            {
                ingress_ac_table_info->learn_info_type = LIF_LEARN_INFO_REQ_TYPE_OPTIMIZED_LIF;
            }
            else
            {
                ingress_ac_table_info->learn_info_type = LIF_LEARN_INFO_REQ_TYPE_BASIC_LIF;
        ...
        switch (ingress_ac_table_info->learn_info_type)
        {
            ...
            case LIF_LEARN_INFO_REQ_TYPE_OPTIMIZED_LIF:
                if (ingress_ac_table_info->dbal_table_id != DBAL_TABLE_PON_DTC_CLASSIFICATION)
                {
            ...
            default:
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "Unsupported learn_info_type - %d\n",
                             ingress_ac_table_info->learn_info_type);
        }
    }
    ...
    int *is_p2p)
{
    ...
    *is_p2p = FALSE;
    ...
    /*
     * initial setting ingress default pcp_dei_extend_profile with explicit map profile 0,
     * unset in replacing.
     * this field will be updated with API bcm_qos_port_map_set().
     */
    if (!_SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_RECYCLE) && !is_replace)
    {
    ...
    if (_SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_INGRESS_WIDE))
    {
        ...
        /*
         * Set LIF_GENERIC_DATA_1 to support 28bits, otherwise 8bits/16bits are supported
         * Available only if the result-type with the Extended Learn-Info isn't required.
         */
        if (ingress_ac_table_info->learn_info_type != LIF_LEARN_INFO_REQ_TYPE_EXTENDED_LIF && !is_native)
        {
            if (*is_p2p)
            {
            ...
            {
                /*
                 * For non-P2P, wide-data supports 16 bits only in DBAL_FIELD_LIF_GENERIC_DATA_0 with protection.
                 */
                if (!has_protection)
                {
                    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LIF_GENERIC_DATA_1, INST_SINGLE, 0);
    ...
    {
        /*
         * Update from wide-data AC to non wide-data AC
         */
        if (is_replace)
        {
            SHR_IF_ERR_EXIT(dnx_lif_table_mngr_is_field_exist_on_dbal_handle
                            (unit, entry_handle_id, DBAL_FIELD_LIF_GENERIC_DATA_0, &is_field_on_handle));
            if (is_field_on_handle)
            {
                dbal_entry_value_field_unset(unit, entry_handle_id, DBAL_FIELD_LIF_GENERIC_DATA_0, INST_SINGLE);
                SHR_IF_ERR_EXIT(dnx_lif_table_mngr_is_field_exist_on_dbal_handle
                                (unit, entry_handle_id, DBAL_FIELD_LIF_GENERIC_DATA_1, &is_field_on_handle));
                if (is_field_on_handle)
                {
                    dbal_entry_value_field_unset(unit, entry_handle_id, DBAL_FIELD_LIF_GENERIC_DATA_1, INST_SINGLE);
                }
            }
        }
    }
    ...
    bcm_vlan_port_t * old_vlan_port)
{
    ...
    if (!is_replace)
    {
        ...
        switch (vlan_port->criteria)
        {
            ...
            case BCM_VLAN_PORT_MATCH_PORT_SVLAN_STACKED:
            {
                result_type = DBAL_RESULT_TYPE_LOCAL_SBC_IN_LIF_MATCH_INFO_SW_MATCH_AC_VLAN_PORT_PORT_SVLAN_SVLAN;
                dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_RESULT_TYPE, INST_SINGLE, result_type);
                dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_GPORT, INST_SINGLE, vlan_port->port);
                dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_OUTER_VLAN_ID, INST_SINGLE,
                                             vlan_port->match_vlan);
            ...
            case BCM_VLAN_PORT_MATCH_PORT_CVLAN_STACKED:
            {
                result_type = DBAL_RESULT_TYPE_LOCAL_SBC_IN_LIF_MATCH_INFO_SW_MATCH_AC_VLAN_PORT_PORT_CVLAN_CVLAN;
                dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_RESULT_TYPE, INST_SINGLE, result_type);
                dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_GPORT, INST_SINGLE, vlan_port->port);
                dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_OUTER_VLAN_ID, INST_SINGLE,
                                             vlan_port->match_vlan);
            ...
            case BCM_VLAN_PORT_MATCH_PORT_UNTAGGED:
            {
                result_type = DBAL_RESULT_TYPE_LOCAL_SBC_IN_LIF_MATCH_INFO_SW_MATCH_AC_VLAN_PORT_PORT_UNTAGGED;
                dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_RESULT_TYPE, INST_SINGLE, result_type);
    ...
    bcm_vlan_port_t * vlan_port)
{
    ...
    if (is_native_ac || is_pon_dtc_ac || dnx_data_lif.in_lif.feature_get(unit, dnx_data_lif_in_lif_phy_db_dpc))
    {
        /*
         * Set table:
         */
    ...
    int local_in_lif)
{
     ...
     * Take table handle and set core key, according to native indication.
     */
    if (is_dpc_ac || dnx_data_lif.in_lif.feature_get(unit, dnx_data_lif_in_lif_phy_db_dpc))
    {
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_LOCAL_DPC_IN_LIF_MATCH_INFO_SW, &entry_handle_id));
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, _SHR_CORE_ALL);
    }
    ...
    uint32 system_port)
{
    ...
    etpp_out_lif_profile_info_t out_lif_profile_info;
    uint8 is_last = 0;
    uint8 is_esem_cmd_field_on_handle;
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);
    ...
    /** Free ESEM CMD */
    SHR_IF_ERR_EXIT(dnx_lif_table_mngr_is_field_exist_on_dbal_handle
                    (unit, entry_handle_id, DBAL_FIELD_ESEM_COMMAND, &is_esem_cmd_field_on_handle));
    if ((dbal_table_id == DBAL_TABLE_EEDB_OUT_AC) && is_esem_cmd_field_on_handle)
    {
    ...
    ac_application_type_e * ll_app_type)
{
    ...
    *ll_app_type = INVALID_AC_APPLICATION_TYPE;
    ...
    {
        /*
         * AC Optimization LIF - service or not
         */
        if (_SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_CREATE_INGRESS_ONLY))
        {
        ...
        {
            *ll_app_type = SERVICE_OPTIMIZATION_AC_APPLICATION_TYPE;
        }
    }
    ...
    dnx_ingress_ac_table_info_t * ingress_ac_table_info)
{
    ...
    int is_ivl = _SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_IVL) ? TRUE : FALSE;
    ...
    if (vlan_port->group)
    {
        if (is_ivl)
        {
            ingress_ac_table_info->learn_context = DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_SYMMETRIC_LIF_EXTENDED_IVL;
        }
        else
        {
            ingress_ac_table_info->learn_context = DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_SYMMETRIC_LIF_EXTENDED_SVL;
        }
    }
    else if (!is_optimized_allowed)
    {
        if (is_ivl)
        {
            ingress_ac_table_info->learn_context = DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_SYMMETRIC_LIF_BASIC_IVL;
        }
        else
        {
            ingress_ac_table_info->learn_context = DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_SYMMETRIC_LIF_BASIC_SVL;
        }
    }
    else
    {
        if (is_ivl)
        {
            ingress_ac_table_info->learn_context = DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_SYMMETRIC_LIF_OPTIMIZED_IVL;
        }
        else
        {
            ingress_ac_table_info->learn_context = DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_SYMMETRIC_LIF_OPTIMIZED_SVL;
        }
    }
    ...
    /*
     * Under JR1 mode, MACT just only support 2 types of result type: Dest+out_LIF; FEC+EEI;
     * So couldn't switch to NO_LIF_xxx learn context.
     */
    if (system_headers_mode == DBAL_ENUM_FVAL_SYSTEM_HEADERS_MODE_JERICHO2_MODE)
    {
        /*
         * Adjust the learn-payload-context to cases where there's no LIF to learn: Dest-Only forward info and
         * Non-Service In-LIFs 
         */
            ...
            || (forward_info->fwd_info_result_type == DBAL_RESULT_TYPE_L2_GPORT_TO_FORWARDING_SW_INFO_DEST_ONLY))
        {
            if ((ingress_ac_table_info->learn_context ==
                 DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_SYMMETRIC_LIF_EXTENDED_SVL)
                || (ingress_ac_table_info->learn_context ==
                    DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_SYMMETRIC_LIF_EXTENDED_IVL))
            {
                ingress_ac_table_info->learn_context = DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_NO_LIF_EXTENDED;
            }
            else if ((ingress_ac_table_info->learn_context ==
                      DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_SYMMETRIC_LIF_BASIC_SVL)
                     || (ingress_ac_table_info->learn_context ==
                         DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_SYMMETRIC_LIF_BASIC_IVL))
            {
                ingress_ac_table_info->learn_context = DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_NO_LIF_BASIC;
            }
            else if ((ingress_ac_table_info->learn_context ==
                      DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_SYMMETRIC_LIF_OPTIMIZED_SVL)
                     || (ingress_ac_table_info->learn_context ==
                         DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_SYMMETRIC_LIF_OPTIMIZED_IVL))
            {
                ingress_ac_table_info->learn_context = DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_NO_LIF_OPTIMIZED;
            }
        }
    }
    else if (is_native == FALSE)
    {
        /*
         * In case of IOP mode + forwarding type is dest_outlif + forwarding_info.outlif is 0,
         * covert SYMMETRIC LIF context to ASYMMETRIC LIF context. So outlif of learned payload could be 0. 
         */
        if ((forward_info->fwd_info_result_type == DBAL_RESULT_TYPE_L2_GPORT_TO_FORWARDING_SW_INFO_DEST_OUTLIF) &&
            (forward_info->outlif == 0))
        {
            /*
             * Needn't support IVL learning in IOP mode.
             * Don't support extend LIF leaning in IOP mode because OLP DSP packet hasn't group field.
             */

            if (ingress_ac_table_info->learn_context == DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_SYMMETRIC_LIF_BASIC_SVL)
            {
                ingress_ac_table_info->learn_context = DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_ASYMMETRIC_LIF_BASIC_SVL;
            }
            else if (ingress_ac_table_info->learn_context ==
                     DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_SYMMETRIC_LIF_OPTIMIZED_SVL)
            {
                ingress_ac_table_info->learn_context =
                    DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_ASYMMETRIC_LIF_OPTIMIZED_SVL;
            }
        }
    }
    ...
    /*
     * Set learn enable by default to TRUE, except for the standard Service-AC where the learn 
     * enable is set after the call to bcm_vswitch_port_add()
     */
...
dnx_vlan_port_create_ingress_non_virtual(
    ...
    bcm_vlan_port_t * vlan_port)
{
    int is_native, is_virtual_ac, old_in_lif_profile;
    ...
    {
        old_in_lif_profile = (int) ingress_ac_fields_info->in_lif_profile;
        SHR_IF_ERR_EXIT(dnx_ingress_inlif_profile_alloc
                        (unit, vlan_port->ingress_network_group_id, native_indexed_mode, old_in_lif_profile,
                         &(ingress_ac_table_info->in_lif_profile), ingress_ac_table_info->dbal_table_id,
                         &last_profile));
    }

    if (vlan_port_gport_to_forward_information_needed(unit, vlan_port))
    {
        /*
        ...
        SHR_IF_ERR_EXIT(vlan_port_gport_to_forward_information_set
                        (unit, vlan_port, ll_app_type, global_lif, system_headers_mode, &forward_info));
    }
    ...
    SHR_IF_ERR_EXIT(dnx_vlan_port_learn_info_fill
                    (unit, &forward_info, ll_app_type, vlan_port, system_headers_mode, ingress_ac_table_info));
...
/**
 * \brief
 * This function sets the ESEM command field for EEDB_OUT_AC table LIFs
 * The function sets HW for ESEM CMD if necessary
 * \param [in] unit - Unit #
 * \param [in] flags - Flags that indicate the configuration of the ESEM CMD (if no flags are found it indicated that
 *                     SVTAG should not be enabled)
 * \param [in] old_esem_cmd_profile - Old ESEM CMD
 * \param [out] esem_cmd_profile_last_reference -
 *                  indicates whether the old ESEM command is no longer in use
 * \param [out] new_esem_cmd - Pointer to the new ESEM CMD
 * \return shr_error_e Standard error handling
 */
static shr_error_e
dnx_vlan_port_esem_cmd_set(
    int unit,
    uint32 flags,
    ...
    int *new_esem_cmd)
{
    ...
    if (_SHR_IS_FLAG_SET(flags, BCM_VLAN_PORT_EGRESS_SVTAG_ENABLE))
    {
        /** enable SVTAG with out-lif as the APP ID */
    ...
    else if (_SHR_IS_FLAG_SET(flags, BCM_VLAN_PORT_EGRESS_SVTAG_PORT_ENABLE))
    {
        /** enable SVTAG with port as the APP ID */
        SHR_IF_ERR_EXIT(dnx_port_esem_cmd_access_add(unit, 0, ESEM_ACCESS_TYPE_SVTAG,
    ...
    {
        /** disable SVTAG by switching to the default ESEM CMD */
        uint32 tmp_esem_cmd;
    ...
    dnx_algo_gpm_gport_hw_resources_t * gport_hw_resources)
{
    ...
    uint8 is_esem_cmd_last_reference = FALSE;
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);
    ...
    if (!is_replace)
    {
            ...
            _SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_EGRESS_SVTAG_PORT_ENABLE))
        {
            ...
            SHR_IF_ERR_EXIT(dnx_vlan_port_esem_cmd_set
                            (unit, vlan_port->flags, old_esem_cmd_profile, &is_esem_cmd_last_reference,
                             &(egress_ac_table_info->esem_cmd)));
        }
    ...
    else
    {
        ...
        egress_ac_table_info->nwk_qos_idx = nwk_qos_idx;
        /*
         * Esem cmd for AC outlif
         */
        if (gport_hw_resources->outlif_dbal_table_id == DBAL_TABLE_EEDB_OUT_AC)
        {
            uint8 is_esem_cmd_field_on_handle;
            dnx_lif_table_mngr_is_field_exist_on_dbal_handle(unit, entry_handle_id, DBAL_FIELD_ESEM_COMMAND,
                                                             &is_esem_cmd_field_on_handle);
            if (is_esem_cmd_field_on_handle)
            {
                int default_profile = dnx_data_esem.access_cmd.no_action_get(unit);

                SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                                (unit, entry_handle_id, DBAL_FIELD_ESEM_COMMAND, INST_SINGLE, &old_esem_cmd_profile));
                if (_SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_EGRESS_SVTAG_ENABLE) ||
                    _SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_EGRESS_SVTAG_PORT_ENABLE))
                {
                    SHR_IF_ERR_EXIT(dnx_vlan_port_esem_cmd_set(unit, vlan_port->flags, old_esem_cmd_profile,
                                                               &is_esem_cmd_last_reference,
                                                               &(egress_ac_table_info->esem_cmd)));
                }
                else
                {
                    if ((old_esem_cmd_profile != default_profile))
                    {
                        SHR_IF_ERR_EXIT(dnx_vlan_port_esem_cmd_set(unit, 0, old_esem_cmd_profile,
                                                                   &is_esem_cmd_last_reference,
                                                                   &(egress_ac_table_info->esem_cmd)));
                    }
                }
    ...
    switch (vlan_port->criteria)
    {
                ...
                if (is_virtual_ac == TRUE)
                {
                    ...
                    if (!is_replace)
                    {
                        SHR_IF_ERR_EXIT(dnx_vlan_port_create_egress_match_esem_sw_state
                                        (unit, vlan_port, DBAL_RESULT_TYPE_ESEM_DEFAULT_RESULT_TABLE_ETPS_AC_STAT, 0));
                    }
                }
            }
            break;
        ...
        case BCM_VLAN_PORT_MATCH_PORT_VLAN:
        {
            ...
            if (is_virtual_ac)
            {
                ...
                SHR_IF_ERR_EXIT(dnx_vlan_port_create_egress_match_esem
                                (unit, vlan_port, egress_ac_table_info, is_replace, &resultType, NULL));
                ...
                if (!is_replace)
                {
                    SHR_IF_ERR_EXIT(dnx_vlan_port_create_egress_match_esem_sw_state(unit, vlan_port, resultType, 0));
                }
            }
            break;
        }
        ...
        {
            uint32 resultType;
            uint32 created_by_ac = 0;
            /*
             * Configure the ESEM entry:
             */
            SHR_IF_ERR_EXIT(dnx_vlan_port_create_egress_match_esem
                            (unit, vlan_port, egress_ac_table_info, is_replace, &resultType, &created_by_ac));
            ...
            {
                SHR_IF_ERR_EXIT(dnx_vlan_port_create_egress_match_esem_sw_state
                                (unit, vlan_port, resultType, created_by_ac));
            }
            break;
        }
        case BCM_VLAN_PORT_MATCH_PORT_UNTAGGED:
    ...
    if (!is_virtual_ac)
    {
        ...
        outlif_info.logical_phase = egress_ac_table_info->outlif_phase;
        outlif_info.global_lif = global_lif;

        if (_SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_EXTENDED))
        {
            outlif_info.table_specific_flags = DNX_ALGO_LIF_TABLE_SPECIFIC_AC_EGRESS_NON_ESEM_BUSTER;
        }
        ...
        /** clear esem cmd profile after allocating and setting HW for the outlif*/
        if (is_esem_cmd_last_reference)
        {
    ...
    bcm_vlan_port_t * vlan_port)
{
    ...
    if (is_ingress && is_egress)
    {
        SHR_IF_ERR_EXIT(dnx_algo_global_lif_allocation_allocate
                        (unit, global_lif_flags, DNX_ALGO_LIF_INGRESS | DNX_ALGO_LIF_EGRESS, NULL, global_lif));
    ...
    {
        SHR_IF_ERR_EXIT(dnx_algo_global_lif_allocation_allocate
                        (unit, global_lif_flags, DNX_ALGO_LIF_INGRESS, NULL, global_lif));
        /*
         * Update vlan_port_id with the allocated new global lif.
         * Encode subtype:
         */
    ...
    {
        SHR_IF_ERR_EXIT(dnx_algo_global_lif_allocation_allocate
                        (unit, global_lif_flags, DNX_ALGO_LIF_EGRESS, NULL, global_lif));
        /*
         * Update vlan_port_id with the allocated new global lif.
         * Encode subtype:
         */
    ...
    uint32 entry_handle_id)
{
     ...
     */
    SHR_IF_ERR_EXIT(dnx_vlan_port_learn_info_fill
                    (unit, &sw_state_forward_info, ll_app_type, &vlan_port, system_headers_mode,
                     &ingress_ac_table_info));
    ...
    dnx_ingress_ac_fields_table_info_get_t * ingress_ac_fields_info)
{
    ...
    if (is_ingress == TRUE)
    {
        ...
        if (DNX_FAILOVER_IS_PROTECTION_ENABLED(vlan_port->ingress_failover_id))
        {
            ...
            BCM_FAILOVER_ID_GET(protection_pointer, vlan_port->ingress_failover_id);
    ...
    if (is_egress == TRUE)
    {
        ...
        if (!is_virtual_ac)
        {
            ...
            if (_SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_EGRESS_PROTECTION))
            {
                ...
                BCM_FAILOVER_ID_GET(egress_ac_table_info.protection_pointer, vlan_port->egress_failover_id);
            }
        }
    ...
    bcm_vlan_port_t * vlan_port)
{
     ...
     */
    SHR_INVOKE_VERIFY_DNXC(dnx_vlan_port_create_replace_verify(unit, vlan_port));

    if (dnx_data_flow.vlan_port.feature_get(unit, dnx_data_flow_vlan_port_flow_in_use) &&
        !_SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_NATIVE) &&
        !_SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_VLAN_TRANSLATION))
    {
        SHR_IF_ERR_EXIT(dnx_vlan_port_flow_create(unit, vlan_port));
    }
    else
    {
        bcm_vlan_port_t old_vlan_port;
    ...
    bcm_vlan_port_t * vlan_port)
{
    ...
    else
    {
         ...
         */
        SHR_IF_ERR_EXIT(dnx_vlan_port_set_ll_app_type(unit, vlan_port, &ll_app_type));
        SHR_INVOKE_VERIFY_DNXC(dnx_vlan_port_create_verify(unit, vlan_port, ll_app_type, is_virtual_ac));

        if (dnx_data_flow.vlan_port.feature_get(unit, dnx_data_flow_vlan_port_flow_in_use))
        {
            if (is_virtual_ac && is_egress && (!is_ingress))
            {
                /** Egress Virtual */
                SHR_IF_ERR_EXIT(dnx_esem_flow_create(unit, vlan_port));
                SHR_EXIT();
            }
            else
            {
                /** AC VLAN-Port (native is TBD) */
                if (!is_native_ac)
                {
                    SHR_IF_ERR_EXIT(dnx_vlan_port_flow_create(unit, vlan_port));
                    SHR_EXIT();
        ...
        if (is_lif)
        {
            ...
            {
                ingress_ac_table_info.protection_path = (vlan_port->ingress_failover_port_id) ? 0 : 1;
                BCM_FAILOVER_ID_GET(protection_pointer[0], vlan_port->ingress_failover_id);
    ...
    int *created_local_lif)
{
    ...
    if (dnx_data_lif.in_lif.feature_get(unit, dnx_data_lif_in_lif_p2p_or_mp_based_on_cs_in_lif_profile))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_in_lif_profile_exchange
                        (unit, &in_lif_profile_info, DNX_IN_LIF_PROFILE_DEFAULT, &new_in_lif_profile, LIF,
                         DBAL_TABLE_IN_AC_INFO_DB, &last_profile));
    ...
    int *created_local_lif)
{
    ...
    else
    {
        if (is_p2p == TRUE)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "default rch lif P2P is not supported");
        }
    }
...
*/
static shr_error_e
dnx_vlan_eth_native_no_tags_default_init(
    int unit)
{
    uint32 entry_handle_id;
    dbal_tables_e dbal_table_id = 0;
    dbal_fields_e field_id = 0;
    ...
    {
        dbal_table_id = DBAL_TABLE_PEMLA_VTT5_DEFAULT_LIF;
        field_id = DBAL_FIELD_VTT5___NATIVE_ETHERNET___0_TAGS_DEFAULT_LIF;
    }
    else
    {
    }
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, field_id, INST_SINGLE, inlif_info.local_inlif);
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
...
*/
static shr_error_e
dnx_vlan_eth_native_service_tagged_miss_init(
    int unit)
{
    uint32 entry_handle_id;
    uint32 trap_dest, snp_strength, fwd_strength, trap_id;
    ...
    {
        dbal_table_id = DBAL_TABLE_PEMLA_VTT5_DEFAULT_LIF;
        field_id = DBAL_FIELD_VTT5___NATIVE_ETHERNET___TRAP_DEFAULT_LIF;
    }
    else
    {
    }
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, field_id, INST_SINGLE, vtt5_default_in_lif);
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
...
{
    uint32 entry_handle_id;
    dbal_fields_e field_id = DBAL_FIELD_INITIAL_DEFAULT_RECYCLE_PORT_VLAN_PORT_IN_LIF;
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_VLAN_PORT_GENERAL_INFO_SW, &entry_handle_id));

    dbal_entry_value_field32_set(unit, entry_handle_id, field_id, INST_SINGLE, initial_default_lif);
    ...
    uint32 *default_recycle_lif)
{
    ...
    dbal_value_field32_request(unit, entry_handle_id, field_id, INST_SINGLE, default_recycle_lif);
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));
exit:
    DBAL_FUNC_FREE_VARS;
    SHR_FUNC_EXIT;
    ...
    int unit)
{
    ...
    uint32 destination;
    lif_mngr_local_inlif_info_t inlif_info;
    uint32 local_lif_to_free_type_0[MAX_NUMBER_OF_DUMMY_ALLOCATIONS_FOR_PPPOE] = { 0 };
    uint32 local_lif_to_free_type_1[MAX_NUMBER_OF_DUMMY_ALLOCATIONS_FOR_PPPOE] = { 0 };
    int num_of_local_lifs_to_free_type_0 = 0;
    int num_of_local_lifs_to_free_type_1 = 0;
    int index;
    int found_valid_pair = FALSE;

    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);

    while (!found_valid_pair)
    {
        /*
         * LIF 0 allocation
         */
        ...
        inlif_info.dbal_result_type = DBAL_RESULT_TYPE_IN_LIF_FORMAT_LIF_SPOOFING_CHECK_IN_LIF_P2P_COMPACT;
        inlif_info.core_id = _SHR_CORE_ALL;
        SHR_IF_ERR_EXIT(dnx_lif_lib_allocate(unit, LIF_MNGR_DONT_ALLOCATE_GLOBAL_LIF, NULL, &inlif_info, NULL));

        while ((inlif_info.local_inlif & 0x3) != 0)
        {
            /*
             * Continue allocate next local lif, and release the previous one, until getting lif with bits[0:1] are 0
             */
            local_lif_to_free_type_0[num_of_local_lifs_to_free_type_0++] = inlif_info.local_inlif;
            if (num_of_local_lifs_to_free_type_0 == MAX_NUMBER_OF_DUMMY_ALLOCATIONS_FOR_PPPOE)
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "too many dummy allocations of type 0 \n");
            }
            sal_memset(&inlif_info, 0, sizeof(inlif_info));
            inlif_info.dbal_table_id = DBAL_TABLE_IN_LIF_FORMAT_LIF_SPOOFING_CHECK;
            inlif_info.dbal_result_type = DBAL_RESULT_TYPE_IN_LIF_FORMAT_LIF_SPOOFING_CHECK_IN_LIF_P2P_COMPACT;
        ...
        sal_memset(&inlif_info, 0, sizeof(inlif_info));
        inlif_info.dbal_table_id = DBAL_TABLE_IN_LIF_FORMAT_LIF_SPOOFING_CHECK;
        inlif_info.dbal_result_type = DBAL_RESULT_TYPE_IN_LIF_FORMAT_LIF_SPOOFING_CHECK_IN_LIF_MPLS_DUMMY;
        inlif_info.core_id = _SHR_CORE_ALL;
        SHR_IF_ERR_EXIT(dnx_lif_lib_allocate(unit, LIF_MNGR_DONT_ALLOCATE_GLOBAL_LIF, NULL, &inlif_info, NULL));

        while ((inlif_info.local_inlif & 0x3) != 2)
        {
            /*
             * Continue allocate next local lif, and release the previous one, until getting lif with bits[0:1] are 2
             */
            local_lif_to_free_type_1[num_of_local_lifs_to_free_type_1++] = inlif_info.local_inlif;
            if (num_of_local_lifs_to_free_type_1 == MAX_NUMBER_OF_DUMMY_ALLOCATIONS_FOR_PPPOE)
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "too many dummy allocations of type 1 \n");
            }
            sal_memset(&inlif_info, 0, sizeof(inlif_info));
            inlif_info.dbal_table_id = DBAL_TABLE_IN_LIF_FORMAT_LIF_SPOOFING_CHECK;
            inlif_info.dbal_result_type = DBAL_RESULT_TYPE_IN_LIF_FORMAT_LIF_SPOOFING_CHECK_IN_LIF_MPLS_DUMMY;
        ...
        {
            local_lif_to_free_type_0[num_of_local_lifs_to_free_type_0++] = local_inlif_0;
            if (num_of_local_lifs_to_free_type_0 == MAX_NUMBER_OF_DUMMY_ALLOCATIONS_FOR_PPPOE)
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "too many dummy allocations of type 0 \n");
            }
            local_lif_to_free_type_1[num_of_local_lifs_to_free_type_1++] = local_inlif_1;
            if (num_of_local_lifs_to_free_type_1 == MAX_NUMBER_OF_DUMMY_ALLOCATIONS_FOR_PPPOE)
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "too many dummy allocations of type 1 \n");
            }
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_RESULT_TYPE, INST_SINGLE,
                                 DBAL_RESULT_TYPE_IN_LIF_FORMAT_LIF_SPOOFING_CHECK_IN_LIF_MPLS_DUMMY);
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    ...
    /*
     * Remove the dummy allocated LIFs that are not used by the PPPoE application
     */
    for (index = 0; index < num_of_local_lifs_to_free_type_0; index++)
    {
        lif_mngr_local_inlif_info_t inlif_to_remove_info;
        sal_memset(&inlif_to_remove_info, 0, sizeof(inlif_to_remove_info));
        inlif_to_remove_info.dbal_table_id = DBAL_TABLE_IN_LIF_FORMAT_LIF_SPOOFING_CHECK;
        inlif_to_remove_info.dbal_result_type = DBAL_RESULT_TYPE_IN_LIF_FORMAT_LIF_SPOOFING_CHECK_IN_LIF_P2P_COMPACT;
        inlif_to_remove_info.core_id = _SHR_CORE_ALL;
        inlif_to_remove_info.local_inlif = local_lif_to_free_type_0[index];
        SHR_IF_ERR_EXIT(dnx_lif_lib_free(unit, LIF_MNGR_INVALID, &inlif_to_remove_info, LIF_MNGR_INVALID));
    }
    for (index = 0; index < num_of_local_lifs_to_free_type_1; index++)
    {
        lif_mngr_local_inlif_info_t inlif_to_remove_info;
        sal_memset(&inlif_to_remove_info, 0, sizeof(inlif_to_remove_info));
        inlif_to_remove_info.dbal_table_id = DBAL_TABLE_IN_LIF_FORMAT_LIF_SPOOFING_CHECK;
        inlif_to_remove_info.dbal_result_type = DBAL_RESULT_TYPE_IN_LIF_FORMAT_LIF_SPOOFING_CHECK_IN_LIF_MPLS_DUMMY;
        inlif_to_remove_info.core_id = _SHR_CORE_ALL;
        inlif_to_remove_info.local_inlif = local_lif_to_free_type_1[index];
        SHR_IF_ERR_EXIT(dnx_lif_lib_free(unit, LIF_MNGR_INVALID, &inlif_to_remove_info, LIF_MNGR_INVALID));
    }
    ...
    int unit)
{
     ...
     *                 set the default in-lif for the port.
     */
    SHR_IF_ERR_EXIT(dnx_vlan_port_ingress_initial_recycle_lif_create(unit, FALSE, &initial_recycle_vlan_port));
    /*
     * Save the initial bridge In-LIF gport:
     */
    SHR_IF_ERR_EXIT(dnx_vlan_port_ingress_initial_default_recycle_port_lif_set(unit, FALSE, initial_recycle_vlan_port));

    /** if the p2p indication is part of a profile, we will create two LIFs (one for each profie) */
    if (dnx_data_lif.in_lif.feature_get(unit, dnx_data_lif_in_lif_p2p_or_mp_based_on_cs_in_lif_profile))
    {
        SHR_IF_ERR_EXIT(dnx_vlan_port_ingress_initial_recycle_lif_create(unit, TRUE, &initial_recycle_vlan_port));
        SHR_IF_ERR_EXIT(dnx_vlan_port_ingress_initial_default_recycle_port_lif_set
                        (unit, TRUE, initial_recycle_vlan_port));
    }

    /*
     * initialize PWE tagged mode default entry
     */
    SHR_IF_ERR_EXIT(dnx_vlan_eth_native_service_tagged_miss_init(unit));
    /*
     * initialize PWE/VXLAN without tags mode default entry
     */
    SHR_IF_ERR_EXIT(dnx_vlan_eth_native_no_tags_default_init(unit));
    /*
     * Create VTT5 PPPoE spoofing check In-LIF pair:
     */
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(dnx_vlan_port_create_egress_match_esem_sw_state
                    (unit, &esem_default_vlan_port, DBAL_RESULT_TYPE_ESEM_DEFAULT_RESULT_TABLE_ETPS_AC_STAT, 0));
    ...
    uint32 *out_lif_is_zero)
{
    ...
    SHR_IF_ERR_EXIT(algo_gpm_gport_and_encap_to_forward_information
                    (unit, ALGO_GPM_ENCODE_DESTINATION_FLAGS_NONE, vlan_port->vlan_port_id,
                     BCM_FORWARD_ENCAP_ID_INVALID, &forward_info));
    if ((forward_info.fwd_info_result_type != DBAL_RESULT_TYPE_L2_GPORT_TO_FORWARDING_SW_INFO_DEST_OUTLIF)
        && (forward_info.fwd_info_result_type != DBAL_RESULT_TYPE_L2_GPORT_TO_FORWARDING_SW_INFO_DEST_ONLY))
    {
    ...
    /*
     * returning learning information depends on whether the destination is MC or differs from the stored port. 
     * A difference is due to a user entered learn destination - A Protection FEC-ID or a phy-port. 
     */
    ...
    else
    {
        ...
        if (encoded_dest_port != forward_info.destination)
        {
            vlan_port->failover_port_id = destination;
        }
    }
    ...
    /*
     * Note: vlan_port->port was already retrieved by SW DB (IN_LIF_MATCH_INFO_SW)
...
exit:
 ...
 /*
  * This function returns the native local-lif value of the port ISEM entry
  * \see
  * bcm_dnx_vlan_port_find
  */
static shr_error_e
dnx_vlan_port_find_ingress_native_match_port(
    int unit,
    bcm_gport_t port,
    int *local_in_lif)
{
    ...
    bcm_gport_t * vlan_port_id)
{
    ...
    if (dnx_data_flow.algo_gpm.flow_gport_in_use_get(unit))
    {
        /** convert the flow gport back to legacy if dnx data flow_gport_in_use=1 */
        SHR_IF_ERR_EXIT(algo_gpm_gport_flow_convert(unit, gport, BCM_GPORT_VLAN_PORT, vlan_port_id));
    }
    else
    {
        *vlan_port_id = gport;
...
/*
 * This function returns the local-lif value of the port X tunnel ISEM entry
 * \see
 * bcm_dnx_vlan_port_find
 */
static shr_error_e
dnx_vlan_port_find_ingress_match_port_tunnel_unmatched(
    int unit,
    bcm_gport_t port,
    bcm_vlan_t tunnel_id,
    int *local_in_lif)
 ...
 */
static shr_error_e
dnx_vlan_port_find_ingress_match_port_tunnel_untagged(
    int unit,
    bcm_gport_t port,
    bcm_vlan_t tunnel_id,
    int *local_in_lif)
...
/*
 * This function returns the local-lif value of the port X tunnel X svlan ISEM entry
 * \see
 * bcm_dnx_vlan_port_find
 */
static shr_error_e
dnx_vlan_port_find_ingress_match_port_tunnel_svlan(
    int unit,
    bcm_gport_t port,
    bcm_vlan_t tunnel_id,
    bcm_vlan_t match_vlan,
...
/*
 * This function returns the local-lif value of the port X tunnel X cvlan ISEM entry
 * \see
 * bcm_dnx_vlan_port_find
 */
static shr_error_e
dnx_vlan_port_find_ingress_match_port_tunnel_cvlan(
    int unit,
    bcm_gport_t port,
    bcm_vlan_t tunnel_id,
    bcm_vlan_t match_vlan,
...
exit:
 ...
 /*
  * This function returns the local-lif value of the port X tunnel X svlan X cvlan ISEM entry
  * \see
  * bcm_dnx_vlan_port_find
  */
static shr_error_e
dnx_vlan_port_find_ingress_match_port_tunnel_svlan_cvlan(
    int unit,
    bcm_gport_t port,
    bcm_vlan_t tunnel_id,
    bcm_vlan_t match_vlan,
...
/*
* This function returns the local-lif value of the port X tunnel X svlan X svlan ISEM entry
* \see
* bcm_dnx_vlan_port_find
*/
static shr_error_e
dnx_vlan_port_find_ingress_match_port_tunnel_svlan_svlan(
    int unit,
    bcm_gport_t port,
    bcm_vlan_t tunnel_id,
    bcm_vlan_t match_vlan,
...
/*
* This function returns the local-lif value of the port X tunnel X cvlan X cvlan ISEM entry
* \see
* bcm_dnx_vlan_port_find
*/
static shr_error_e
dnx_vlan_port_find_ingress_match_port_tunnel_cvlan_cvlan(
    int unit,
    bcm_gport_t port,
    bcm_vlan_t tunnel_id,
    bcm_vlan_t match_vlan,
    ...
    dnx_ingress_ac_fields_table_info_get_t * ingress_ac_fields_info)
{
    ...
    if ((is_field_on_handle) && (tmp32 != DBAL_ENUM_FVAL_VTT_LIF_SERVICE_TYPE_P2MP))
    {
    ...
    dnx_ingress_ac_fields_table_info_get_t * ingress_ac_fields_info)
{
    ...
    uint32 dbal_struct_data = 0;
    in_lif_profile_info_t in_lif_profile_info;
    int is_native, is_native_non_virtual;
    uint8 has_vsi, is_wide, has_stat, is_field_on_handle = 0;
    lif_table_mngr_inlif_info_t lif_info;
    ...
    if (is_field_on_handle)
    {
        if (failover_id[0] != dnx_data_failover.path_select.ing_no_protection_get(unit))
        {
            ...
            BCM_FAILOVER_SET(vlan_port->ingress_failover_id, failover_id_decoded[0], BCM_FAILOVER_TYPE_INGRESS);
        }
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
     ...
     * Note: protection info is not set for Native In-LIFs.
     */
    if (_SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_NATIVE) == FALSE)
    {
    ...
    if (_SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_RECYCLE))
    {
         ...
         */
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                        (unit, entry_handle_id, DBAL_FIELD_PROPAGATION_PROFILE, INST_SINGLE, &prop_prof));
        ...
        {
            vlan_port->ingress_qos_model.ingress_phb = bcmQosIngressModelInvalid;
            vlan_port->ingress_qos_model.ingress_remark = bcmQosIngressModelInvalid;
            vlan_port->ingress_qos_model.ingress_ttl = bcmQosIngressModelInvalid;
        }
    ...
    /*
     * Retrieve learn context information:
     */
    SHR_IF_ERR_EXIT(dnx_lif_table_mngr_uint32_field_on_dbal_handle_get
                    (unit, entry_handle_id, DBAL_FIELD_LEARN_PAYLOAD_CONTEXT, &is_field_on_handle, &dbal_struct_data));
    if (is_field_on_handle)
    {
        /*
         * In IOP mode, It's ring-protection if the learning is FEC & the learn-payload-context basic_symmetric
         */
        if (_SHR_GPORT_IS_FORWARD_PORT(vlan_port->failover_port_id) &&
            (dnx_data_headers.system_headers.system_headers_mode_get(unit) ==
             DBAL_ENUM_FVAL_SYSTEM_HEADERS_MODE_JERICHO_MODE))
        {
            if (dbal_struct_data == DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_SYMMETRIC_LIF_BASIC_SVL)
            {
                /*
                 * In IOP mode, dest_only forwarding information is converted to dest_outlif with outlif == 0.
                 * So needn't set ring protected group for this case. 
                 */
                if (!out_lif_is_zero)
                {
                    vlan_port->group = RING_PROTECTION_MACT_GROUP_IN_JR1_MODE;
                }
            }
        }

        if ((dbal_struct_data == DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_SYMMETRIC_LIF_BASIC_IVL)
            || (dbal_struct_data == DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_SYMMETRIC_LIF_EXTENDED_IVL)
            || (dbal_struct_data == DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_ASYMMETRIC_LIF_BASIC_IVL)
            || (dbal_struct_data == DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_ASYMMETRIC_LIF_EXTENDED_IVL)
            || (dbal_struct_data == DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_SYMMETRIC_LIF_OPTIMIZED_IVL)
            || (dbal_struct_data == DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_ASYMMETRIC_LIF_OPTIMIZED_IVL))
        {
            vlan_port->flags |= BCM_VLAN_PORT_IVL;
        }
    }
    ...
    uint8 not_found_log_enable)
{
            ...
            if (_SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_NATIVE))
            {
                ...
                SHR_IF_ERR_EXIT(dbal_iterator_value_field_arr32_rule_add
                                (unit, entry_handle_id, DBAL_FIELD_VSI, INST_SINGLE, DBAL_CONDITION_EQUAL_TO,
                                 &fodo_vsi, NULL));
                SHR_IF_ERR_EXIT(dbal_iterator_value_field_arr32_rule_add
                                (unit, entry_handle_id, DBAL_FIELD_IS_LIF_SCOPE, INST_SINGLE, DBAL_CONDITION_EQUAL_TO,
                                 &is_native_intf_name_space, NULL));
    ...
    if (is_end == TRUE)
    {
        /*
         * No entry found, return "not found"
         * Don't print msg in some cases since the result is expected.
         */
        if (not_found_log_enable)
        {
            SHR_ERR_EXIT(_SHR_E_NOT_FOUND,
    ...
    {
        /** convert the flow gport back to legacy if dnx data flow_gport_in_use=1 */
        SHR_IF_ERR_EXIT(algo_gpm_gport_flow_convert(unit, gport, BCM_GPORT_VLAN_PORT, &vlan_port->vlan_port_id));
    }
    else
    {
        vlan_port->vlan_port_id = gport;
    ...
    bcm_vlan_port_t * vlan_port)
{
    ...
    dbal_entry_key_field32_set(unit, sw_table_handle_id, DBAL_FIELD_GPORT, local_gport);
    ...
    bcm_vlan_port_t * vlan_port)
{
    ...
    uint32 result_type;
    uint32 sub_type_format = 0;
    uint32 egress_tunnel_value = 0;
    etpp_out_lif_profile_info_t out_lif_profile_info;
     ...
     */
    if (gport_hw_resources->logical_phase == DNX_ALGO_LOCAL_OUTLIF_LOGICAL_PHASE_3)
    {
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
    ...
    {
        BCM_FAILOVER_SET(vlan_port->egress_failover_id, failover_id, BCM_FAILOVER_TYPE_ENCAP);
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
    ...
    SHR_IF_ERR_EXIT(dnx_lif_mngr_outlif_sw_info_get
                    (unit, gport_hw_resources->local_out_lif, NULL, &result_type, NULL, NULL, NULL, NULL));
    ...
    if (result_type == DBAL_RESULT_TYPE_EEDB_OUT_AC_ETPS_AC_TRIPLE_TAG)
    {
        ...
        if (sub_type_format == EEDB_OUT_AC_SUB_TYPE_FORMAT_PON_PORT)
        {
            /**
             * sub type format
             */
            SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                            (unit, entry_handle_id, DBAL_FIELD_ECID_OR_PON_TAG, INST_SINGLE, &egress_tunnel_value));
            vlan_port->egress_tunnel_value = egress_tunnel_value;
        }
    }
    /** check indication for BCM_VLAN_PORT_EGRESS_SVTAG_ENABLE flag */
    ...
    if (rv != _SHR_E_NOT_FOUND)
    {
        ...
        if (svtag_present)
        {
            if (app_db_id == DBAL_ENUM_FVAL_ESEM_APP_DB_ID_OUT_LIF)
            {
                vlan_port->flags |= BCM_VLAN_PORT_EGRESS_SVTAG_ENABLE;
            }
            else if (app_db_id == DBAL_ENUM_FVAL_ESEM_APP_DB_ID_OUT_TM_PORT)
            {
                vlan_port->flags |= BCM_VLAN_PORT_EGRESS_SVTAG_PORT_ENABLE;
            }
            else
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "Unexpected error occured:"
                             " indicated esem command with svtag that is not triggered by port or outlif");
            }
        }
    }
    if (_SHR_IS_FLAG_SET(outlif_info.table_specific_flags, DNX_ALGO_LIF_TABLE_SPECIFIC_AC_EGRESS_NON_ESEM_BUSTER))
    {
...
{
    int is_ingress, is_egress, is_native_ac, is_virtual_ac;
    int is_pon_enable = 0, is_pon_dtc_enable = 0, is_dpc_in_lif = 0;
    SHR_FUNC_INIT_VARS(unit);
    /*
    ...
    if (dnx_data_flow.vlan_port.feature_get(unit, dnx_data_flow_vlan_port_flow_in_use))
    {
        ...
        {
            /** Egress Virtual */
            SHR_IF_ERR_EXIT(dnx_esem_flow_get(unit, vlan_port));
            SHR_EXIT();
        }
        else
        {
    ...
    if (is_ingress == TRUE)
    {
        ...
        switch (vlan_port->criteria)
        {
            case BCM_VLAN_PORT_MATCH_PORT:
            {
                ...
                {
                    SHR_IF_ERR_EXIT(dnx_vlan_port_find_ingress_native_match_port(unit, vlan_port->port, &local_in_lif));
                }
                break;
            }
            case BCM_VLAN_PORT_MATCH_PORT_VLAN:
            ...
            case BCM_VLAN_PORT_MATCH_PORT_TUNNEL:
            {
                SHR_IF_ERR_EXIT(dnx_vlan_port_find_ingress_match_port_tunnel_unmatched
                                (unit, vlan_port->port, vlan_port->match_tunnel_value, &local_in_lif));
                break;
            }
            case BCM_VLAN_PORT_MATCH_PORT_TUNNEL_VLAN:
            {
                SHR_IF_ERR_EXIT(dnx_vlan_port_find_ingress_match_port_tunnel_svlan
            ...
            case BCM_VLAN_PORT_MATCH_PORT_TUNNEL_CVLAN:
            {
                SHR_IF_ERR_EXIT(dnx_vlan_port_find_ingress_match_port_tunnel_cvlan
                                (unit, vlan_port->port, vlan_port->match_tunnel_value, vlan_port->match_vlan,
                                 &local_in_lif));
                break;
            }
            case BCM_VLAN_PORT_MATCH_PORT_TUNNEL_VLAN_STACKED:
            {
                SHR_IF_ERR_EXIT(dnx_vlan_port_find_ingress_match_port_tunnel_svlan_cvlan
                                (unit, vlan_port->port, vlan_port->match_tunnel_value,
                                 vlan_port->match_vlan, vlan_port->match_inner_vlan, &local_in_lif));
                break;
            }
            case BCM_VLAN_PORT_MATCH_PORT_TUNNEL_CVLAN_STACKED:
            {
                SHR_IF_ERR_EXIT(dnx_vlan_port_find_ingress_match_port_tunnel_cvlan_cvlan
                                (unit, vlan_port->port, vlan_port->match_tunnel_value,
                                 vlan_port->match_vlan, vlan_port->match_inner_vlan, &local_in_lif));
                break;
            }
            case BCM_VLAN_PORT_MATCH_PORT_TUNNEL_SVLAN_STACKED:
            {
                SHR_IF_ERR_EXIT(dnx_vlan_port_find_ingress_match_port_tunnel_svlan_svlan
                                (unit, vlan_port->port, vlan_port->match_tunnel_value,
                                 vlan_port->match_vlan, vlan_port->match_inner_vlan, &local_in_lif));
                break;
            }
            case BCM_VLAN_PORT_MATCH_PORT_TUNNEL_UNTAGGED:
            {
                SHR_IF_ERR_EXIT(dnx_vlan_port_find_ingress_match_port_tunnel_untagged
                                (unit, vlan_port->port, vlan_port->match_tunnel_value, &local_in_lif));
                break;
            }
        ...
        {
            /*
             * The local in-lif is not "DTC", retrieve it's gport using the GPM
             */
            if (is_dpc_in_lif == FALSE)
            {
                /*
    ...
    else if (is_egress == TRUE)
    {
         ...
         */
        SHR_IF_ERR_EXIT(dnx_vlan_port_find_egress_match_esem_sw_state(unit, vlan_port, FALSE, 0, TRUE));
    }
    ...
    bcm_vlan_port_t * vlan_port)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_vlan_port_find_verify(unit, vlan_port));
    ...
    /**
     *  For LIF Flow and "find by lookup", vlan_port already done inside dnx_vlan_port_find_by_lookup
     *  dnx_vlan_port_find_by_lookup calls dnx_vlan_port_flow_get
     */
    if (!(dnx_data_flow.vlan_port.feature_get(unit, dnx_data_flow_vlan_port_flow_in_use)
...
/**
 * \brief - Clears egress Namespace-VSI at ESEM
 * \see
 *  bcm_dnx_vlan_port_destroy
 * Note:
 * if created_by_ac - entry is deleted, 
 * else - deleting the entry is done by bcm_vxlan_network_domain_config_remove, here, only set fields to defualt value.
 */
static shr_error_e
dnx_vlan_port_destroy_egress_esem_namespace_vsi_info_clear(
    int unit,
    bcm_gport_t gport,
    bcm_vlan_t vsi,
    uint32 vlan_domain,
    ...
    uint32 created_by_ac)
{
    ...
    {
        /*
         * In case VLAN translation parameters were set, need to zero them:
         */
        ...
        /*
         * Note:
         * Deleting the entry is done by bcm_vxlan_network_domain_config_remove, here, only set fields to defualt value.
         */
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_OUT_LIF_PROFILE, INST_SINGLE,
                                     DNX_OUT_LIF_PROFILE_DEFAULT_ORIENTATION_PROFILE);
    ...
    bcm_vlan_port_t * vlan_port)
{
    ...
    SHR_IF_ERR_EXIT(dnx_vlan_port_destroy_ingress_lif_match_info_clear
                    (unit, is_dpc_in_lif, gport_hw_resources->local_in_lif));
    /*
    ...
    bcm_gport_t gport)
{
    ...
    if (dnx_data_flow.algo_gpm.flow_gport_in_use_get(unit))
    {
        SHR_IF_ERR_EXIT(algo_gpm_gport_flow_convert(unit, gport, _SHR_GPORT_TYPE_FLOW_LIF, &local_gport));
    }
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_INGRESS_GPORT_TO_LOCAL_IN_LIF_SW, &entry_handle_id));
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_GPORT, local_gport);
    SHR_IF_ERR_EXIT(dbal_entry_clear(unit, entry_handle_id, DBAL_COMMIT));
    ...
    bcm_vlan_port_t * vlan_port)
{
     ...
     */
    sw_handle = BCM_GPORT_SUB_TYPE_VIRTUAL_INGRESS_NATIVE_GET(vlan_port->vlan_port_id);
    SHR_IF_ERR_EXIT(vlan_db.vlan_port_ingress_virtual_gport_id.free_single(unit, sw_handle, NULL));
    ...
    bcm_vlan_port_t * vlan_port)
{
     ...
     */
    SHR_IF_ERR_EXIT(algo_port_pp_db.esem_default_result_profile.free_single(unit, esem_default_handle, NULL));
exit:
    DBAL_FUNC_FREE_VARS;
    SHR_FUNC_EXIT;
}
    ...
    bcm_vlan_port_t * vlan_port)
{
     ...
     */
    esem_handle = BCM_GPORT_SUB_TYPE_VIRTUAL_EGRESS_MATCH_GET(vlan_port->vlan_port_id);
    SHR_IF_ERR_EXIT(vlan_db.vlan_port_egress_virtual_gport_id.free_single(unit, esem_handle, NULL));
    /*
    ...
    bcm_vlan_port_t * vlan_port)
{
    ...
    SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                    (unit, sw_table_handle_id, DBAL_FIELD_CREATED_BY_AC, INST_SINGLE, &created_by_ac));
    ...
    /*
     * 2. Clear the ESEM table content:
     * Note:
     * if created_by_ac - entry is deleted, 
     * else - deleting the entry is done by bcm_vxlan_network_domain_config_remove, here, only set fields to defualt value.
     */
    SHR_IF_ERR_EXIT(dnx_vlan_port_destroy_egress_esem_namespace_vsi_info_clear
                    (unit, vlan_port->vlan_port_id, vlan_port->vsi, vlan_domain, 0, created_by_ac));
    ...
    bcm_gport_t gport)
{
    ...
    SHR_FUNC_INIT_VARS(unit);
    DNX_ERR_RECOVERY_NOT_NEEDED(unit);
    SHR_INVOKE_VERIFY_DNXC(dnx_vlan_port_destroy_verify(unit, gport));
    bcm_vlan_port_t_init(&vlan_port);
    ...
    if (BCM_GPORT_SUB_TYPE_IS_VIRTUAL_EGRESS_MATCH(gport))
    {
        ...
        if (dnx_data_flow.vlan_port.feature_get(unit, dnx_data_flow_vlan_port_flow_in_use))
        {
            SHR_IF_ERR_EXIT(dnx_esem_flow_delete(unit, gport));
            SHR_EXIT();
        }
    ...
    else
    {
        ...
        else
        {
            ...
            if (dnx_data_flow.vlan_port.feature_get(unit, dnx_data_flow_vlan_port_flow_in_use))
            {
                SHR_IF_ERR_EXIT(dnx_vlan_port_flow_delete(unit, gport));
                SHR_EXIT();
            }
    ...
    void *user_data)
{
    ...
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_PON_DTC_CLASSIFICATION, &entry_handle_id));
    SHR_IF_ERR_EXIT(dbal_iterator_init(unit, entry_handle_id, DBAL_ITER_MODE_GET_ALL_EXCEPT_DEFAULT));
        ...
        else
        {
            ...
            if (resultType == DBAL_RESULT_TYPE_ESEM_FORWARD_DOMAIN_MAPPING_DB_ETPS_AC_STAT)
            {
                ...
                if (egress_last_layer)
                {
                    ...
                    SHR_IF_ERR_EXIT(dnx_vlan_port_find_egress_match_esem_sw_state
                                    (unit, &vlan_port, TRUE, vlan_domain, TRUE));
                }
                else
                {
                    skip_callback = TRUE;
    ...
    void *user_data)
{
    ...
    while (!is_end)
    {
        ...
        if (dnx_data_flow.algo_gpm.flow_gport_in_use_get(unit))
        {
            /** convert the flow gport back to legacy if dnx data flow_gport_in_use=1 */
            SHR_IF_ERR_EXIT(algo_gpm_gport_flow_convert
                            (unit, vlan_port_id, BCM_GPORT_VLAN_PORT, &vlan_port.vlan_port_id));
        }
        else
        {
    ...
    void *user_data)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_vlan_port_traverse_verify(unit, additional_info, trav_fn));

    if (dnx_data_flow.vlan_port.feature_get(unit, dnx_data_flow_vlan_port_flow_in_use))
    {
        SHR_IF_ERR_EXIT(dnx_vlan_port_flow_traverse(unit, additional_info, trav_fn, user_data));
        SHR_EXIT();
    }
    ...
    uint32 new_local_in_lif_id)
{
     ...
     */
    if ((dnx_data_lif.in_lif.feature_get(unit, dnx_data_lif_in_lif_phy_db_dpc)) &&
        (inlif_dbal_table_id == DBAL_TABLE_IN_AC_INFO_DB))
    {
        algo_flags = DNX_ALGO_GPM_GPORT_HW_RESOURCES_DPC_LOCAL_LIF_INGRESS;
    }
    else
    {
