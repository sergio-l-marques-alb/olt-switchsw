...
#define DNX_PORT_ADD_SUPPORTED_FLAGS (BCM_PORT_ADD_BASE_Q_PAIR_WITH_ID | BCM_PORT_ADD_CGM_AUTO_ADJUST_DISABLE | \
                                      ...
                                      BCM_PORT_ADD_FLR_FIRMWARE_SUPPORT)
    ...
    uint32 flags)
{
   ...
   /** 
    ...
    if (((!DNX_ALGO_PORT_TYPE_IS_NIF(unit, port_info, DNX_ALGO_PORT_TYPE_INCLUDE(L1, FLEXE, MGMT))) &&
         (!DNX_ALGO_PORT_TYPE_IS_RCY_MIRROR(unit, port_info)) && (!DNX_ALGO_PORT_TYPE_IS_RCY(unit, port_info))
         && (!DNX_ALGO_PORT_TYPE_IS_FLEXE(unit, port_info, TRUE, TRUE))
    ...
    bcm_port_mapping_info_t * mapping_info)
{
             ...
             DNX_ALGO_PORT_TYPE_IS_E2E_SCH(unit, port_info))
    {
        ...
        if (DNX_ALGO_PORT_TYPE_IS_FLEXE(unit, port_info, TRUE, FALSE))
        {
            if (mapping_info->channel != 0 && mapping_info->channel != -1)
            {
                            ...
                            (unit, interface_id_h0, interface_id_h1, &master_port));
            {
                SHR_IF_ERR_EXIT(dnx_egr_queuing_interface_alloc(unit,
                                                                ...
                                                                _SHR_IS_FLAG_SET(flags,
                                                                                 BCM_PORT_ADD_CONFIG_CHANNELIZED),
                                                                &(tm_info->egress_interface)));

                SHR_IF_ERR_EXIT(dnx_algo_port_out_tm_port_ref_count_get(unit, mapping_info->core, tm_info->out_tm_port,
                                                                        &out_tm_valid));
                ...
                if (!out_tm_valid)
                {
                    ...
                    if (flags & BCM_PORT_ADD_BASE_Q_PAIR_WITH_ID)
                    {
                        SHR_IF_ERR_EXIT(dnx_ps_db_base_qpair_alloc_with_id
                                        (unit, port, tm_info->egress_interface, common_info->is_channelized,
                                         mapping_info->core, tm_info->num_priorities, mapping_info->base_q_pair));
                    ...
                    else
                    {
                        SHR_IF_ERR_EXIT(dnx_ps_db_base_qpair_alloc
                                        (unit, port, tm_info->egress_interface, common_info->is_channelized,
                                         mapping_info->core, tm_info->num_priorities,
                                         _SHR_IS_FLAG_SET(flags, BCM_PORT_ADD_SCH_PRIORITY_PROPAGATION_EN),
                                         &tm_info->base_q_pair));
                    }
                }
                tm_info->is_egress_interleave = _SHR_IS_FLAG_SET(flags, BCM_PORT_ADD_EGRESS_INTERLEAVING_ENABLE)
                    ? 1 : 0;
...
static const dnx_port_add_remove_step_t port_add_remove_sequence[] = {
    ...
    {"IngressCongestion", dnx_cosq_ingress_port_add_handle,         dnx_cosq_ingress_port_remove_handle,         NULL,    device_specific_skip},
    /** MirrorRcy must come after IngressCongestion */
    {"MirrorRcy",         dnx_mirror_rcy_port_add_handle,           dnx_mirror_rcy_port_remove_handle,           NULL,    device_specific_skip},
    ...
    {"ImbPath",           dnx_port_imb_path_port_add_handle,        dnx_port_imb_path_port_remove_handle,        NULL,    device_specific_skip},
    /** SchPort must come after EgqQueuing */
    {"SchPort",           dnx_sch_port_add_handle,                  dnx_sch_port_remove_handle,                  NULL,    device_specific_skip},
    ...
    {"Disable&Flash",     NULL,                                     dnx_egr_queuing_port_disable,                NULL,    device_specific_skip},
