/** \file src/bcm/dnx/dram/gddr6/gddr6_cb.c
 *
 *
 *  This file contains implementation of call back functions for GDDR6 DRAM PHY shmoo
 *
 */
    ...
    uint32 mask)
{
    ...
    /** verify data and mask can fit mode register */
    ...
    uint32 *data)
{
    ...
    /*
     * indirection to GDDR6 HW is that phy registers are mapped to a memory and only 12 bits of the address are relevant
     */
    ...
    int enable)
{
    ...
    {
        /** if running with disabled read crc is not allowed change value to enable */
        LOG_VERBOSE(BSL_LOG_MODULE,
                    (BSL_META_U
                     (unit,
    ...
    int dram_index)
{
    ...
    /** toggle reset bit for read FIFO */
    ...
    const g6phy16_bist_info_t * info)
{
    ...
    /** check dram_index and channel are valid  */
    SHR_INVOKE_VERIFY_DNXC(dnx_gddr6_index_verify(unit, dram_index));
    LOG_VERBOSE(BSL_LOG_MODULE,
                (BSL_META_U(unit, "%s(): unit=%d, dram_index=%d\n"), FUNCTION_NAME(), unit, dram_index));

    /** Check if need to override existing BIST configuration */
    ...
    {
        /** No need to override BIST configuration, get out! */
        SHR_EXIT();
    }

    /** Init BIST configuration structure either with defaults or with input configuration according to field */
    ...
    /** due to a problem in Q2A A0 column mode has to be incremental and not PRBS */
    ...
    /** due to a problem in Q2A A0 row mode has to be incremental and not PRBS */
    ...
    const g6phy16_bist_info_t * info)
{
    ...
    else
    {
        ...
        {
            /** Both number of actions and timer are 0, this means infinite BIST which is NOT a valid configuration for BIST CB */
    ...
    g6phy16_vendor_info_t * vendor_info)
{
    ...
    /** this value is true to all dram vendors */
    ...
    g6phy16_training_bist_info_t * info)
{
    ...
    /*
     * DBAL_COMMIT_OVERRUN flag is used here, 
     * since this access writes the whole HW register.
     * read is not necessary in this case
     */
    ...
    {
        /*
         * This code is disabled as dbal performs many accesses for this table
         * Flat dbal above replaces this
         */
    ...
    if (info->training_bist_mode != SHMOO_G6PHY16_TRAINING_BIST_ADDR_TRAINING)
    {
        SHR_IF_ERR_EXIT(dnx_gddr6_load_precharge_all_command(unit, dram_index, 1 /* cabi enabled */ ));
    }
    ...
    g6phy16_training_bist_err_cnt_t * info)
{
    ...
    {
        /** concat together the results of both channels, the shifts below are corresponding to the length of the fields */
        SHR_IF_ERR_EXIT(dnx_gddr6_dbal_access_bist_err_cnt_get(unit, dram_index, channel, &bist_err_cnt));
        ...
        {
            /** read training command counter is equal to read data count and read edc count */
            /** this means that the number of read command issued was the same as the number of reads and edc
             * reads that were actually done - as should be the case */
            /*
             * Clearing the bits is redundant since they were already cleared.
             * We do it in order not to have an empty 'if' clause.
             */
        ...
        {
            /*
             * bist_sanity_err_bitmap is a bitmap which indicates
             * sanity error per byte.
             * Since we work here in channel resolution we mark that sanity
             * error occurred on all the bytes in the channel.
             */
