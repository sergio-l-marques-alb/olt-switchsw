    ...
    const dnx_oam_lif_entry_key_t * oam_lif_entry_key)
{
    ...
    DNXCMN_CORE_VALIDATE(unit, oam_lif_entry_key->core_id, TRUE);
    ...
    dnx_oam_lif_entry_value_t * oam_lif_entry_value)
{
    ...
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_oam_lif_get_verify(unit, oam_lif_entry_key, oam_lif_entry_value));
    ...
    const dnx_oam_lif_entry_value_t * oam_lif_entry_value)
{
    ...
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_oam_lif_add_verify(unit, oam_lif_entry_key, oam_lif_entry_value));
    ...
    const dnx_oam_lif_entry_key_t * oam_lif_entry_key)
{
    ...
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_oam_lif_verify(unit, oam_lif_entry_key));
    ...
    uint32 *acc_mp_profile)
{
    ...
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_oam_acc_mep_get_verify(unit, core_id, flags, oam_lif, mdl, mep_id, acc_mp_profile));
    ...
    int acc_mp_profile)
{
    ...
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_oam_acc_mep_add_verify(unit, core_id, flags, oam_lif, mdl, mep_id, acc_mp_profile));
    ...
    int mdl)
{
    ...
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_acc_mep_verify(unit, core_id, flags, oam_lif, mdl));
    ...
    const oam_action_content_t * action_content)
{
    ...
    /** Verify parameters */
    SHR_INVOKE_VERIFY_DNXC(dnx_oam_action_key_verify(unit, flags, action_key));
    SHR_INVOKE_VERIFY_DNXC(dnx_oam_action_content_verify(unit, flags, action_content));
    ...
    oam_lif_configured_lif_t * configured_lif)
{
    ...
    for (index = 0; index <= DNX_OAM_MAX_MDL; index++)
    {
        ...
        if (configured_lif->counter_base[index] > 0)
        {
            configured_lif->nof_lm_meps++;
        }
    }
