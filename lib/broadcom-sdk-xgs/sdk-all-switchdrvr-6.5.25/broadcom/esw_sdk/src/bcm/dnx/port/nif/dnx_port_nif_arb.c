    ...
    bcm_port_t logical_port)
{
    ...
    if (DNX_ALGO_PORT_TYPE_IS_NIF_ETH(unit, port_info, DNX_ALGO_PORT_TYPE_INCLUDE(STIF, L1)))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_port_arb_calendar_ids_get(unit, logical_port, &calendars_to_be_set));
    ...
    int enable)
{
    ...
    /*
     * In case of ETH L1 keep the datapath disabled, traffic will be dropped (used only for monitoring)
     */
