...
/**
 * \brief - 
 * convert rate to DRM internal representation
 */
static shr_error_e
    ...
    uint32 *internal)
{
    ...
    else
    {
        ...
        /** Divide and round by rate */
        COMPILER_64_ADD_32(u64_calc, rate / 2);
        COMPILER_64_UDIV_32(u64_calc, rate);

        *internal = COMPILER_64_LO(u64_calc);
    }


    if (*internal != 0)
    {
        /** Get maximal value of the rate field */
...
/**
 * \brief - 
 * convert DRM internal representation to rate
 */
static shr_error_e
