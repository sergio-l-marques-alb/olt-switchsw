 ...
 */
shr_error_e
dnx_multicast_group_sar_port_set(
    int unit,
    bcm_multicast_t group,
    bcm_port_t port)
{
    uint32 entry_handle_id = 0;
    bcm_core_t core_id = BCM_CORE_ALL;
    uint32 channel_id = 0;
    SHR_FUNC_INIT_VARS(unit);
    ...
    {
        /** Need to clear the existing SAR channel ID that is mapped to the group using the max possible value in the DBAL */
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_TDM_SAR_CHANNEL_ID_MULTICAST, &entry_handle_id));
        SHR_IF_ERR_EXIT(dbal_fields_predefine_value_get
                        (unit, entry_handle_id, DBAL_FIELD_SAR_CHANNEL, 0, DBAL_PREDEF_VAL_MAX_VALUE, &channel_id));
    }
    else
    {
        /** Get the port's core and SAR channel ID */
        SHR_IF_ERR_EXIT(dnx_algo_port_core_get(unit, port, &core_id));
        SHR_IF_ERR_EXIT(dnx_algo_port_flexe_channel_get(unit, port, ((int *) &channel_id)));
    }

    /** Write the channel to HW */
    SHR_IF_ERR_EXIT(dnx_multicast_sar_group_channel_set(unit, core_id, group, channel_id));
