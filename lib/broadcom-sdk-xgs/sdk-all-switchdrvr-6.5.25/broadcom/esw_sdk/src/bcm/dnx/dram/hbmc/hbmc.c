...
/**
 * \brief - HBM PLL configurations for APD PHY.
 *
 * \param [in] unit - unit number
 * \param [in] hbm_index - hbm index
 *
...
/**
 * \brief - steps that has to be done for HBMC block out of reset.
 *
 * \param [in] unit - unit number
 * \param [in] hbm_index - hbm index
 *
...
/**
 * \brief
 *   Configuration sequence of the APD PHY PLL.
 * \param [in] unit -
 *   The unit number.
 * \return
 *   See \ref shr_error_e
    ...
    int unit)
{
    ...
    SHR_BIT_ITER(dram_bitmap, nof_hbms, hbm_index)
    {
        /** Put all EMI blocks into soft reset */
        LOG_VERBOSE(BSL_LOG_MODULE,
        ...
        /** Put all EMI blocks into soft init */
        LOG_VERBOSE(BSL_LOG_MODULE,
        ...
        /*
         * Take all TDU blocks out of soft reset.
         * This is needed even if we work in single HBM mode.
         */
        LOG_VERBOSE(BSL_LOG_MODULE,
        ...
        /** Take HBMC block out of soft reset */
        LOG_VERBOSE(BSL_LOG_MODULE,
        ...
        /** Take TDU blocks out of soft init */
        LOG_VERBOSE(BSL_LOG_MODULE,
...
/**
 * \brief
 *   Initialization sequence of the APD PHY.
 * \param [in] unit -
 *   The unit number.
 * \return
 *   See \ref shr_error_e
 * \remark
 *   Should be run only after dnx_hbmc_apd_phy_pll_init.
 * \see
 *   None.
 */
shr_error_e
    ...
    int unit)
{
    ...
    SHR_BIT_ITER(dram_bitmap, nof_hbms, hbm_index)
    {
        /** allow access to HBMC dynamic memories */
        LOG_VERBOSE(BSL_LOG_MODULE,
        ...
        /** Take HBMC block out of soft init */
        LOG_VERBOSE(BSL_LOG_MODULE,
        ...
        /** allow access to HCC dynamic memories */
        LOG_VERBOSE(BSL_LOG_MODULE,
    ...
    {
        /** Take HRC block out of soft reset */
        LOG_VERBOSE(BSL_LOG_MODULE,
...
/**
 * \brief - Get the HBM Mode Registers values according to predefined values and soc properties
 *
 * \param [in] unit - unit number
 * \param [in] is_hbm2e - is DRAM HBM2E standard
 * \param [out] mrs - mode registers values
 *
 * \return
 *   shr_error_e
 *
    ...
    dnx_hbmc_mode_registers_t * mrs)
{
    ...
    /** HW field value is (driver_strength-6)/3 according to spec */
    ...
    /** HW field value is read_latency-2 according to spec */
    ...
    /** HW field value is write_latency-1 according to spec */
    ...
    /** Actually only burst_length=4 is supported, so need to always set to 1 according to spec */
     ...
     * MR6 info 
     */
    /** HW field value is trp-2 according to spec */
    uint32 trp_hw_val = dnx_data_dram.general_info.timing_params_get(unit)->trp - 2;

    /*
     * MR15 info 
     */
    /** internal_vref = 0.46 * VDDQ, which is '1' according to spec */
...
/**
 * \brief - Init HBM Channels Controllers
 *
 * \param [in] unit - unit number
 * \param [in] hbm_index - hbm index
 *
...
/**
 * \brief - Init HRC block (HBM Remote Controller)
 *
 * \param [in] unit - unit number
 *
 * \return
...
/**
 * \brief - enable/disable refresh per HBM
 *
 * \param [in] unit - unit number
 * \param [in] hbm_index - hbm index
 * \param [in] refresh_enable - enable/disable refresh
 *
 * \return
 *   shr_error_e
 * \remark
 ...
 * \param [in] unit - unit number
 * \param [in] hbm_ndx - hbm index
 * \param [in] is_quiet - should print error in case a mandatory param cannot be found in SOC properties
    ...
    int *shmoo_action_p)
{
    ...
    switch (action)
    {
        ...
        {
            /*
             * Restore tune parameters from otp. If not found, perform tune
...
/**
 * \brief - enable mechanism to ensure DRAM coherence (read-write ordering) in a single channel
 * \param [in] unit - unit number
 * \param [in] hbm_index - hbm index
...
/**
 * \brief - enable mechanism to ensure DRAM coherence (read-write ordering)
 * \param [in] unit - unit number
 *
 * \return
 *   shr_error_e
    ...
    int unit)
{
    ...
    /** Iterate on dram bitmap and set coherence enforcement for each enabled dram */
    SHR_BIT_ITER(dram_bitmap, nof_hbms, hbm_index)
    {
    ...
    int unit)
{
    ...
    {
        /** reading vendor_info is disabled since it can cause CPU2TAP problems on this device */
        SHR_EXIT();
    }
    ...
    int unit)
{
    ...
    /** If dram swstate is already inited then get out */
    ...
    /** set to SW-State that power to dram is off. Will be set to on in dnx_hbmc_init (when if will be called) */
    SHR_IF_ERR_EXIT(dnx_dram_db.power_down_callback.power_on.set(unit, 0));

    if (dnx_data_dram.hbm.feature_get(unit, dnx_data_dram_hbm_apd_phy))
    {
        /**
         * Create mutex to synchronize firmware access.
         * Every function that initiate firmware interrupts (by calling dnx_hbmc_firmware_interrupt_access)
         * should take this mutex.
         */
    ...
    int unit)
{
    ...
    /** set to SW-State that dram is initialized */
    ...
    int unit)
{
    ...
    for (global_hbm_index = 0; global_hbm_index < max_nof_drams; ++global_hbm_index)
    {
        ...
        /** Take HRC block out of soft init */
        LOG_VERBOSE(BSL_LOG_MODULE,
        ...
        if (is_valid)
        {
            /**
             * HBC init.
             * Taking HBM channels out of soft init is done inside.
             */
            LOG_VERBOSE(BSL_LOG_MODULE,
    ...
    /*
     * Enable read-write coherence enforcement 
     */
    if (dnx_data_dram.hbm.feature_get(unit, dnx_data_dram_hbm_is_coherency_enforcer_supported))
    {
        LOG_VERBOSE(BSL_LOG_MODULE, (BSL_META_U(unit, "%s(): Enable HBM coherence enforcer BIST\n"), FUNCTION_NAME()));
    ...
    int unit)
{
    ...
    /** if HBM APD PHY is not available, get out */
    if (!dnx_data_dram.hbm.feature_get(unit, dnx_data_dram_hbm_apd_phy))
    {
        SHR_EXIT();
    }
    ...
    for (global_hbm_index = 0; global_hbm_index < max_nof_drams; ++global_hbm_index)
    {
        ...
        if (is_valid)
        {
            ...
            /** Reset PHY and HBM after running MMT */
            LOG_VERBOSE(BSL_LOG_MODULE,
...
/**
 * \brief - check if temperature monitor should be created
 *
 */
static int
    ...
    int unit)
{
    ...
    return
        ...
        ((dnx_data_dram.general_info.dram_info_get(unit)->dram_bitmap != 0) &&
         /** temp monitor is relevant only for HBM architectures */
    ...
    int unit)
{
    ...
    /** check if temp monitor is relevant, 
     *  if it is not - get out */
 ...
 *   This function saves the MRs values.
 *
 * See .h file
 */
shr_error_e
 ...
 *   Due to HW overlook soft init also clears the MRs in the controller side,
 *   so after soft init need to write them back
 *   This function restore the MRs values.
 *
 * See .h file
 */
shr_error_e
    ...
    dnx_hbmc_mrs_values_t * mr_values_p)
{
    ...
    /*
     * we need to write the previously saved MRs only to the controller,
     * and not to the HBM, since they are already updated there.
     */
    /** set that HBM will NOT be updated on mode register change */
