...
/**
* \brief
*    Check if a GTF ID is in an EVC pair.
* \par
*  \param [in] unit  -  Relevant unit.
*  \param [in] gtf_id  - GTF ID.
*  \param [out] is_paired  - Returns whether the GTF ID is in an EVC pair.
* \retval
*  * Zero if no error was detected
*  * Negative if error was detected. See shr_error_e
* \remark
*  * None
*/
static shr_error_e
...
/**
* \brief
*    For a GTF ID, set whether the corresponding EVC pair is enabled.
* \par
*  \param [in] unit  -  Relevant unit.
*  \param [in] gtf_id  - GTF ID.
*  \param [in] is_paired - paired or not.
* \retval
*  * Zero if no error was detected
*  * Negative if error was detected. See shr_error_e
* \remark
...
/**
* \brief
* To verify SAT gtf ID and priority .
* \par
* \param [in] unit  - Relevant unit.
* \param [in] gtf_id  - GTF id
    ...
    bcm_sat_gtf_t * gtf_id)
{
    ...
    if (flags & BCM_SAT_GTF_WITH_ID)
    {
        ...
        {
            /** only GTF 0~15 can be paired, the GTF ID given must be even */
            if (!DNX_SAT_GTF_ID_IS_VALID_EVC_ID(*gtf_id))
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "Error: invalid GTF ID\n");
            }

            /** If even GTF ID is not allocated, then check odd GTF ID as well */
            if (!is_allocated)
            {
                SHR_IF_ERR_EXIT(algo_sat_db.sat_res_alloc_gtf_id.is_allocated(unit, *gtf_id + 1, &is_allocated));
            }
    ...
    bcm_sat_gtf_t * gtf_id)
{
    ...
    if (flags & BCM_SAT_GTF_ALLOC_PAIR)
    {
        ...
        /*
         * The first GTF ID in an EVC pair is even.
         * The second GTF ID in an EVC pair is odd.
         * Always allocate the second GTF ID WITH_ID.
         */
    ...
    bcm_sat_gtf_t gtf_id)
{
    ...
    /*
     * Get packet header length from template
     */
    ...
    bcm_sat_gtf_t gtf_id)
{
    ...
    /** For GTF ID 0~15, check if GTF is paired */
    if (DNX_SAT_GTF_ID_IS_VALID_EVC_ID(gtf_id))
    {
        SHR_IF_ERR_EXIT(dnx_sat_gtf_is_paired_get(unit, gtf_id, &is_paired));
    }
    ...
    if (is_paired)
    {
        /** If GTF is paired, destroy odd GTF first */
...
/**
* \brief
*     Get GTF or EVC bandwidth from HW.
* \par
* \param [in] unit - Relevant unit.
* \param [in] gtf_id -   gtf_id. When get for EVC, this is the even flow ID in the EVC
* \param [in] priority -  element of the gtf 
    ...
    bcm_sat_gtf_bandwidth_t * bw)
{
    ...
    if (crdt_per_pkt)
    {
        ...
        /*
         * Coverity:
         *     clocks_per_cycle can't be 0, otherwise hardware won't work properly
         */
    ...
    bcm_sat_gtf_bandwidth_t * bw)
{
     ...
     * Should not set priority to a value other than bcmSatGtfPriCir if GTF is not paired.
     */
    ...
    bcm_sat_gtf_bandwidth_t * bw)
{
    ...
    /*
     * If  not configuring EVC then save bw to SW DB
     */
...
* \par
* \param [in] unit - Relevant unit.
* \param [in] pkt_header_base -  offset to the packet header base in dwords.
    ...
    dnx_sat_gtf_pkt_hdr_tbl_info_t * config)
{
    ...
    /** Check if the header spreads into two entries in the memory */
...
* \param [in] unit - Relevant unit.
* \param [in] pkt_header_base -  offset to the packet header base in dwords.
* \param [in] pkt_header_len - length of the packet header.
* \retval
* * Zero if no error was detected
* * Negative if error was detected. See \ref
*   shr_error_e
...
* \par
* \param [in] unit - Relevant unit.
* \param [in] header_profile -  header profile id.
* \param [in] config -  sat packet header information.
* \retval
*   * Zero if no error was detected
*   * Negative if error was detected. See \ref
    ...
    bcm_sat_gtf_packet_config_t * config)
{
    ...
    /*
     * PktHeaderBase[ii] represent bits [11:02] of the PktHeader memory's address The header is 4B aligned, think of the
    ...
    if (config->offsets.seq_number_offset != DNX_SAT_GTF_SEQ_NUM_DISABLE)
    {
        ...
        /*
         * sequence_period:
         *     0 - sequence number is incremented every single packet
         *     1 - sequence number is incremented 2 single packets
         *     2 - sequence number is incremented 4 single packets
         *     3 - sequence number is incremented 8 single packets
         */
    ...
    /*
     * Get SEQ number wrap-around value
     */
    ...
    bcm_sat_gtf_packet_config_t * config)
{
    ...
    /*
     * Only a single priority is supported.
     */
    priority = 0;
    ...
    /*
     * Save pkt_header_base mapping
     */
    SHR_IF_ERR_EXIT(sat_pkt_header_info.pkt_header_base.get(unit, old_profile, &saved_pkt_header_base));

    /*
     * pkt_header_base inited to saved_pkt_header_base, it will be updated if a new profile is allocated
     */
    ...
    {
        /*
         * 2. Allocate packet header in PKT_HEADER_MEM
         */
        ...
        /*
         * 3. Save profile-> pkt_header_base mapping
         */
    ...
    bcm_sat_gtf_pri_t priority)
{
    ...
    {
        /*
         * Enable odd flow in an EVC pair
         */
    ...
    bcm_sat_gtf_pri_t priority)
{
    ...
    {
        /*
         * disable odd flow in an EVC pair
         */
...
/**
* \brief
*     Get EVC rate pattern from HW.
* \par
* \param [in] unit - Relevant unit.
* \param [in] gtf_id -  gtf id.
* \param [out] config -  sat EVC rate pattern .
* \retval
*   * Zero if no error was detected
*   * Negative if error was detected. See \ref
*     shr_error_e
