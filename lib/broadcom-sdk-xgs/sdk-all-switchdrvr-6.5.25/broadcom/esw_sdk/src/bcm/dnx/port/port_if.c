    ...
    for (ii = 0; ii < nof_prio_groups; ++ii)
    {
        ...
        if (priority_config->priority_groups[ii].sch_priority != bcmPortNifSchedulerTDM)
        {
            ...
            if (soc_is(unit, J2X_DEVICE))
            {
                ...
                per_priority_exist[1 - priority_config->priority_groups[ii].sch_priority] = 1;
            }
            else
            {
                /** TDM priority does not require reassembly context */
        ...
        if (total_speed_Mbps[core] > max_speed_Mbps)
        {
            ...
            else
            {
                LOG_WARN(BSL_LOG_MODULE,
                         (BSL_META_U
                          (unit,
                           "total speed in the device for core%d (%dMbps) exceed the egress TM limitation(%dMbps) It is user's respons is responsiblity to ensure traffic does not exceed bandwidth limit in practice."),
                          core, total_speed_Mbps[core], max_speed_Mbps));
            }
    ...
    for (resource_index = 0; resource_index < nport; ++resource_index)
    {
            ...
            && (resource[resource_index].speed != 0) && ports_info[resource_index].is_tx_speed_change)
        {
            ...
            if ((!DNX_ALGO_PORT_TYPE_IS_NIF(unit, ports_info[resource_index].port_info,
                                            DNX_ALGO_PORT_TYPE_INCLUDE(ELK, STIF, L1, FLEXE, MGMT)))
    ...
    if (!soc_is(unit, J2X_DEVICE))
    {
        ...
        for (resource_index = 0; resource_index < nport; ++resource_index)
        {
            ...
            if (DNX_ALGO_PORT_TYPE_IS_NIF_ETH(unit, ports_info[resource_index].port_info,
                                              DNX_ALGO_PORT_TYPE_INCLUDE(L1, MGMT))
    ...
    bcm_port_resource_t * resource)
{
...
#ifdef BCM_DNX2_SUPPORT
    _SHR_PBMP_CLEAR(ports_with_modified_speed);
#endif
    ...
    for (resource_index = 0; resource_index < nport; ++resource_index)
    {
        ...
        {
            /*
             * Save to bitmap the ports which their speed was changed in order to calculate relevant calendars in arbiter.
             */
            speed = 0;
            SHR_IF_ERR_EXIT(dnx_algo_port_has_speed(unit, resource[resource_index].port, 0, &has_speed));
            if (has_speed)
            {
                SHR_IF_ERR_EXIT(dnx_algo_port_speed_get(unit, resource[resource_index].port, 0, &speed));
            }

            if (resource[resource_index].speed != speed
                && (DNX_ALGO_PORT_TYPE_IS_NIF_ETH(unit, ports_info[resource_index].port_info, 0)
                    || DNX_ALGO_PORT_TYPE_IS_FLEXE_MAC(unit, ports_info[resource_index].port_info, 0)))
            {
                _SHR_PBMP_PORT_ADD(ports_with_modified_speed, resource[resource_index].port);
            }
        }
#endif
    }
    ...
    SHR_IF_ERR_EXIT(imb_common_post_resource_multi_set(unit, nport, ports_with_modified_speed, resource));
