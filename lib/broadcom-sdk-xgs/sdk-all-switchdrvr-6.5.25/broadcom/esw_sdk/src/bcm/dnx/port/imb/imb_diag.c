...
static int imb_diag_nif_phy_rate_calculation_get(
    int unit,
    bcm_port_t port,
    int is_rx,
    soc_dnxc_port_phy_measure_t * phy_measure);

static int imb_diag_nif_synce_phy_measure_get(
    int unit,
    bcm_port_t port,
    int is_rx,
    soc_dnxc_port_phy_measure_t * phy_measure);
    ...
    imb_diag_synce_config_t * synce)
{
    ...
    if (dnx_data_synce.general.feature_get(unit, dnx_data_synce_general_synce_in_nanosync))
    {
         ...
         * alloc DBAL table handle
         */
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_SYNCE_CLK_COUNTER_CTRL, &entry_handle_id));
        /*
         * set key fields
         */
         ...
         * set value fields
         */
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_COUNTER_CLK_SEL, INST_SINGLE, synce->clk_sel);
    ...
    else
    {
        ...
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LINK_VALID_SEL, INST_SINGLE,
                                     synce->link_valid_sel);
        ...
        table_id =
            (synce_clk_blk_id <
             dnx_data_nif.eth.cdu_nof_get(unit)) ? DBAL_TABLE_SYNCE_REF_CLK_NIF_CDU_LANE_SEL :
    ...
    imb_diag_synce_config_t * synce)
{
    ...
    if (dnx_data_synce.general.feature_get(unit, dnx_data_synce_general_synce_in_nanosync))
    {
        ...
        dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_COUNTER_CLK_SEL, INST_SINGLE, &synce->clk_sel);
        /*
         * commit request
         */
        SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));
    ...
    else
    {
        ...
        dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_LINK_VALID_SEL, INST_SINGLE,
                                   &synce->link_valid_sel);
 ...
 * \param [in] unit - chip unit id
 * \param [in] core - core id
 * \param [in] synce_clk_blk_id - synce clk block Id
...
{
    int is_signal_lock;
    int phy_rate_calculation = FALSE;
    int loopback;
    SHR_FUNC_INIT_VARS(unit);

    if (!dnx_data_synce.general.feature_get(unit, dnx_data_synce_general_measure_rate_on_down_port_is_supported))
    {
        SHR_IF_ERR_EXIT(bcm_dnx_port_link_status_get(unit, port, &is_signal_lock));
        SHR_IF_ERR_EXIT(bcm_dnx_port_loopback_get(unit, port, &loopback));
        if (!is_signal_lock || loopback == BCM_PORT_LOOPBACK_MAC)
        {
            phy_rate_calculation = TRUE;
        }
    }
    if (phy_rate_calculation)
    {
        SHR_IF_ERR_EXIT(imb_diag_nif_phy_rate_calculation_get(unit, port, is_rx, phy_measure));
    }
    else
    {
        SHR_IF_ERR_EXIT(imb_diag_nif_synce_phy_measure_get(unit, port, is_rx, phy_measure));
    }
...
/**
 * \brief - Calculation the serdes rate using ref clock, pll div, osmode.
 */
static int
imb_diag_nif_phy_rate_calculation_get(
    int unit,
    bcm_port_t port,
    int is_rx,
    soc_dnxc_port_phy_measure_t * phy_measure)
{
    ...
    SHR_IF_ERR_EXIT(phymod_phy_speed_config_t_init(&phy_speed_config));

    PHYMOD_DEVICE_OP_MODE_PCS_BYPASS_SET(phy_access.device_op_mode);
    SHR_IF_ERR_EXIT(phymod_phy_speed_config_get(&phy_access, &phy_speed_config));
    /*
     ...
     * Calc serdes freq
     */
    phy_measure->serdes_freq_int = phy_speed_config.data_rate / 1000;
    phy_measure->serdes_freq_remainder = phy_speed_config.data_rate % 1000;
...
/**
 * \brief - Measure the serdes rate using SyncE.
 */
static int
imb_diag_nif_synce_phy_measure_get(
    int unit,
    bcm_port_t port,
    int is_rx,
    soc_dnxc_port_phy_measure_t * phy_measure)
{
     ...
     */
    SHR_IF_ERR_EXIT(dnx_nif_synce_enable_get(unit, ethu_info.core, is_mgu, &synce_enable_default));
    synce_state_read = 1;
    /*
     * Enable syncE
     */
    SHR_IF_ERR_EXIT(dnx_nif_synce_enable_set(unit, ethu_info.core, is_mgu, 1));
    /*
     * Save syncE configuration to restore at end of function
     */
    ...
    if (dnx_data_synce.general.feature_get(unit, dnx_data_synce_general_synce_in_nanosync))
    {
        block_type = SOC_BLK_CDPM;
        instance = ethu_info.ethu_id;
    }
    else
    {
        block_type = SOC_BLK_NMG;
        instance = ethu_info.core;
    }
    SHR_IF_ERR_EXIT(dnx_gtimer_set(unit, IMB_DIAG_GTIMER_NOF_CYCLES, block_type, instance));
    /*
     ...
     * Read syncE counter
     */
    SHR_IF_ERR_EXIT(imb_diag_synce_counter_get(unit, ethu_info.core, ethu_info.ethu_id, &synce_cnt));
    /*
...
exit:
    if (unit < SOC_MAX_NUM_DEVICES)
    {
        ...
        if (synce_state_read)
        {
            if ((_func_rv = dnx_nif_synce_enable_set(unit, ethu_info.core, is_mgu, synce_enable_default)) < 0)
            {
