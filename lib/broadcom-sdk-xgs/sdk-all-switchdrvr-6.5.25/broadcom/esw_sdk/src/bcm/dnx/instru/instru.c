...
/**
 * \brief -
 * Set the counters init register (used for resetting) off and on
 *
 * \param [in] unit -  Unit-ID
 * \param [in] source_type -  Type of Synchronous Counters mechanism
 * \param [in] set_to_on - If TRUE set the counters to work. if FALSe, set to 0 (resetting the counters).
 * \return
 *   See shr_error_e
 * \remark
 *   * None
 ...
 */
static shr_error_e
dnx_instru_synced_counters_config_hw_init_reset_set(
    int unit,
    bcm_instru_synced_counters_source_type_t source_type,
    uint8 set_to_on)
{
    uint32 entry_handle_id_ctrl;
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);

    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_INST_SYNCED_COUNTERS_CTRL, &entry_handle_id_ctrl));
    dbal_entry_key_field32_set(unit, entry_handle_id_ctrl, DBAL_FIELD_SOURCE_TYPE, source_type);
    ...
    switch (source_type)
    {
        ...
        {
            int bring_back_up = set_to_on;
            SHR_IF_ERR_EXIT(imb_instru_counters_reset_all(unit, bring_back_up));
            break;
        }
        default:
        {
    ...
    dbal_entry_value_field8_set(unit, entry_handle_id_ctrl, DBAL_FIELD_ENABLE, INST_SINGLE, set_to_on);
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id_ctrl, DBAL_COMMIT));
 ...
 */
static shr_error_e
dnx_instru_synced_counters_config_receive_hw_set(
    int unit,
    bcm_instru_synced_counters_source_type_t source_type,
    ...
    uint8 is_continuous)
{
    ...
    switch (source_type)
    {
        case bcmInstruSyncedCountersTypeIcgm:
        {
            uint32 entry_handle_id_init;
            uint32 entry_handle_id_config;

            SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC
                            (unit, DBAL_TABLE_INST_SYNCED_COUNTERS_SETTING_ICGM, &entry_handle_id_init));
            dbal_entry_value_field8_set(unit, entry_handle_id_init, DBAL_FIELD_SYNC_COUNTERS_INIT, INST_SINGLE, 1);
            SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id_init, DBAL_COMMIT));

            SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC
                            (unit, DBAL_TABLE_INST_SYNCED_COUNTERS_SETTING_ICGM, &entry_handle_id_config));
            dbal_entry_value_field16_set(unit, entry_handle_id_config, DBAL_FIELD_SYNC_COUNTERS_NOF_INTERVALS,
            ...
            dbal_entry_value_field8_set(unit, entry_handle_id_init, DBAL_FIELD_SYNC_COUNTERS_INIT, INST_SINGLE, 0);
            SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id_init, DBAL_COMMIT));
            break;
        }
        case bcmInstruSyncedCountersTypeNif:
        {
            SHR_IF_ERR_EXIT(imb_instru_counters_reset_all(unit, FALSE));
...
/**
 * \brief -  Set Synced counters configuration to HW on the ECI (sending the pulse signals.)
 */
static shr_error_e
dnx_instru_synced_counters_config_eci_hw_set(
    int unit,
    bcm_instru_synced_counters_source_type_t source_type,
    uint8 enable,
    uint64 start_time,
 ...
 */
static shr_error_e
dnx_instru_synced_counters_config_eci_hw_get(
    int unit,
    bcm_instru_synced_counters_source_type_t source_type,
    uint8 *enable,
    uint64 *start_time,
    ...
    int is_start)
{
    ...
    SHR_IF_ERR_EXIT(dnx_instru_synced_counters_config_hw_init_reset_set(unit, source_type, is_start));
    ...
    bcm_instru_synced_counters_config_t * config)
{
    ...
    if (enable == TRUE)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_instru_synced_counters_config_eci_hw_set
                        (unit, source_type, enable, start_time, interval_period, nof_intervals, is_continuous));
        SHR_IF_ERR_EXIT(dnx_instru_synced_counters_config_receive_hw_set
                        (unit, source_type, nof_intervals, is_continuous));
        /** Reenable the counters. and set the SW state. */
    ...
    bcm_instru_synced_counters_config_t * config)
{
    ...
    SHR_IF_ERR_EXIT(dnx_instru_synced_counters_config_eci_hw_get
                    (unit, source_type, &enable, &start_time, &interval_period, &nof_intervals, &is_continuous));
