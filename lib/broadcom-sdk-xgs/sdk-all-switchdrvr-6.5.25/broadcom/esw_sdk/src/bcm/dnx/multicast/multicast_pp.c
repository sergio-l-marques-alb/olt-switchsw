    ...
    bcm_if_t * encap_extension_array)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_multicast_encap_extension_create_verify
    ...
    if (!_SHR_IS_FLAG_SET(flags, BCM_MULTICAST_ENCAP_EXTENSION_REPLACE))
    {
        if (_SHR_IS_FLAG_SET(flags, BCM_MULTICAST_ENCAP_EXTENSION_WITH_ID))
        {
            ace_flags |= DNX_FIELD_ENTRY_ACE_ADD_FLAG_WITH_ID;
        }
    }
    else
    {
        ace_flags |= DNX_FIELD_ENTRY_ACE_ADD_FLAG_UPDATE;
    }
    ...
    (*multicast_replication_index) = ace_key;
...
shr_error_e
dnx_multicast_pp_jr1_same_interface_init(
    int unit)
{
    dnx_field_entry_payload_t payload;
    ...
    SHR_IF_ERR_EXIT(dnx_field_ace_entry_add
                    (unit, (DNX_FIELD_ENTRY_ACE_ADD_FLAG_WITH_ID | DNX_FIELD_ENTRY_ACE_AVOID_ALLOCATION),
                     DNX_MULTICAST_PPMC_SINGLE_OUTLIF_ACE_ID, &payload, &ace_key));
    ...
    int unit)
{
    ...
    action_info.size = 22; /** Obtain outlif size.*/
    ...
    /*
     * Create three multicast replication tables within the ACE table (ACE formats).
     * One ACE format updated one outlif, another two and another three.
     * The reason we use three different ACE formats and not one ACE format that can update one to three outlifs
     * is to allow two outlif entries to be written to a 60 bit entry.
     */
    ...
    ace_format_info.use_ppmc_range = TRUE;
    /*
    ...
    bcm_if_t * encap_id)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_multicast_l3_encap_get_verify(unit, group, port, intf, encap_id));
    ...
    bcm_if_t * encap_id)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_multicast_vlan_encap_get_verify(unit, group, port, vlan_port_id, encap_id));
    ...
    bcm_if_t * encap_id)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_multicast_vpls_encap_get_verify(unit, group, port, mpls_port_id, encap_id));
    ...
    bcm_if_t * encap_id)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_multicast_vxlan_encap_get_verify(unit, group, port, vxlan_port_id, encap_id));
    ...
    bcm_if_t * encap_id)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_multicast_egress_object_encap_get_verify(unit, group, intf, encap_id));
