    ...
    bcm_stat_counter_interface_t * config)
{
    ...
    int total_range_size = 0, nof_counters_per_entry;
    ...
    while (next_engine_id != DNX_CRPS_MGMT_ENGINE_INVALID)
    {
        ...
        total_range_size += engine_info->nof_entries;
    ...
    total_range_size = (total_range_size * nof_counters_per_entry);
    ...
    bcm_stat_expansion_select_t * expansion_select)
{
    ...
    for (element_idx = 0; element_idx < expansion_select->nof_elements; element_idx++)
    {
        ...
        current_element_nof_bits =
            dnx_data_crps.expansion.source_type_get(unit, src,
                                                    expansion_select->expansion_elements[element_idx].type)->nof_bits;
    ...
    bcm_stat_expansion_select_t * expansion_select)
{
    ...
    if (dnx_data_crps.expansion.source_type_get(unit, interface->interface_source,
                                                DNX_CRPS_EXPANSION_TOTAL_NOF_BITS)->nof_bits <=
    ...
    bcm_stat_counter_database_t * database)
{
    ...
    DNXCMN_CORES_ITER(unit, database->core_id, core_idx)
    {
        ...
        while (current_engine_id != DNX_CRPS_MGMT_ENGINE_INVALID)
        {
             ...
             */
            engine_info = dnx_data_crps.engine.engines_info_get(unit, current_engine_id);
            if (engine_info->nof_entries != C_TYPE_ENGINE_SIZE(unit))
            {
    ...
    int *nof_countrs)
{
    ...
    while (current_engine != DNX_CRPS_MGMT_ENGINE_INVALID)
    {
        SHR_IF_ERR_EXIT(dnx_crps_mgmt_nof_counters_per_engine
                        (unit, current_engine, format_type, &nof_counters_per_engine));
    ...
    /** Verify set's counters amount */
    {
        ...
        DNXCMN_CORES_ITER(unit, database->core_id, core_idx)
        {
            SHR_IF_ERR_EXIT(dnx_crps_mgmt_nof_counters_per_database_get
                            (unit, database->database_id, core_idx, format_type, &nof_counters_in_database));
    ...
    bcm_stat_counter_interface_t * config)
{
    ...
    if ((flags & BCM_STAT_COUNTER_MODIFY_RANGE_ONLY) == FALSE)
    {
        ...
        SHR_INVOKE_VERIFY_DNXC(dnx_crps_mgmt_counter_interface_set_conter_format_verify
                               (unit, database, config->format_type));
    ...
    if (flags & BCM_STAT_COUNTER_MULTI_SETS_DATABASE)
    {
        ...
        SHR_INVOKE_VERIFY_DNXC(dnx_crps_mgmt_counter_interface_set_config_verify
                               (unit, config->format_type, database, config->set_config));
    ...
    bcm_stat_counter_set_map_t * counter_set_map)
{
    ...
    DNXCMN_CORES_ITER(unit, database->core_id, core_idx)
    {
        ...
        if (is_multi_sets_database && _SHR_IS_FLAG_SET(flags, BCM_STAT_COUNTER_MAPPING_FULL_SPREAD))
        {
        ...
        for (expansion_element_id = 0; expansion_element_id < counter_set_map->nof_entries; expansion_element_id++)
        {
                 ...
                 key_id++)
            {
                 ...
                 */
                if (dnx_data_crps.expansion.source_type_get(unit, interface_source,
                                                            counter_set_map->expansion_data_mapping
                                                            [expansion_element_id].key[key_id].type)->nof_bits <= 0)
                {
                    SHR_ERR_EXIT(_SHR_E_PARAM,
                ...
                nof_bits_per_type =
                    dnx_data_crps.expansion.source_type_get(unit, interface_source,
                                                            counter_set_map->expansion_data_mapping
                                                            [expansion_element_id].key[key_id].type)->nof_bits;
    ...
    int engine_id)
{
    ...
    else
    {
        ...
        parallel_engine_id =
            (engine_info->nof_entries ==
             A_TYPE_ENGINE_SIZE(unit)) ? engine_id + nof_crps_ire_channels : engine_id - nof_crps_ire_channels;
    ...
    uint32 arg)
{
    ...
    switch (control)
    {
        ...
        case bcmStatCounterDatabaseRateCalculationEnable:
            if (!dnx_data_crps.engine.feature_get(unit, dnx_data_crps_engine_rate_calculation_is_supported))
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "Unsupported control=%d\n", control);
            }
            if (arg != 0 && arg != 1)
