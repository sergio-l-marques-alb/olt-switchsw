    ...
    uint32 *interval)
{
    ...
    /*
     * internal function gets rate in [KBytes/sec] and credit in [KBytes].
     * rate is passed in [KBits/sec] to avoid division by 8, which will cause loss of accuracy.
     * credit is passed in [Kbits] to accommodate this.
     */
    SHR_IF_ERR_EXIT(dnx_algo_rates_credit_rate_to_interval_get(unit, credit * UTILEX_NOF_BITS_IN_CHAR, rate, interval));
exit:
    SHR_FUNC_EXIT;
}
    ...
    uint32 *rate)
{
     ...
     * credit is passed in [Kbits] to return rate in [Kbits/Sec].
     */
    SHR_IF_ERR_EXIT(dnx_algo_rates_credit_interval_to_rate_get(unit, credit * UTILEX_NOF_BITS_IN_CHAR, interval, rate));
