    ...
    uint32 *tag)
{
    ...
    if (dnx_data_lif.global_lif.global_lif_bta_max_val_get(unit))
    {
        ...
        (*tag) |= ((alloc_info->bta << offset) & bta_mask);
    ...
    if (dnx_data_lif.global_lif.global_lif_intf_max_val_get(unit))
    {
        ...
        (*tag) |= (alloc_info->outlif_intf << offset) & intf_mask;
    ...
    if (dnx_data_lif.global_lif.global_lif_group_max_val_get(unit))
    {
        ...
        (*tag) |= (alloc_info->outlif_group << offset) & group_mask;
    ...
    int global_lif)
{
    ...
    if (alloc_info)
    {
        ...
        if (alloc_info->outlif_intf > dnx_data_lif.global_lif.global_lif_intf_max_val_get(unit))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "INTF value %d is illegal. max possible value %d",
                         alloc_info->outlif_intf, dnx_data_lif.global_lif.global_lif_intf_max_val_get(unit));
        }
    ...
    int *global_lif)
{
    ...
    {
        SHR_IF_ERR_EXIT(lif_mngr_db.global_lif_allocation.
                        ingress_global_lif.allocate_single(unit, DNX_ALGO_RES_ALLOCATE_WITH_ID, internal_alloc_info,
                                                           global_lif));
    }
...
/** for each grain setting the group value in HW */
    ...
    uint32 alloc_flags)
{
    ...
    dnx_global_lif_tag_max_val_get(unit, &extra_create_info.max_tag_value);
    ...
    int *element)
{
    ...
    dnx_global_lif_tag_set(unit, &alloc_info.global_lif_alloc, &res_tag_bitmap_alloc_info.tag);
