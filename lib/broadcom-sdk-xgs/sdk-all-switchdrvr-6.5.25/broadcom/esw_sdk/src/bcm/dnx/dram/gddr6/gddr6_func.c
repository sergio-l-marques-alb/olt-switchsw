    ...
    g6phy16_shmoo_dram_info_t * shmoo_info)
{
    ...
    shmoo_info->ref_clk_bitmap = dnx_data_dram.gddr6.ref_clk_bitmap_get(unit);
...
{
    uint32 pllRefclkSel =
        (dnx_data_dram.gddr6.master_phy_get(unit, dram_index)->master_dram_index != dram_index) ? 1 : 0;
    SHR_FUNC_INIT_VARS(unit);
    /** sanity checks  */
    ...
    gddr6_dram_init_phase_t phase)
{
    ...
    if ((phase == SHMOO_GDDR6_DRAM_INIT_AFTER_CADT) || (phase == SHMOO_GDDR6_DRAM_INIT_ALL))
    {
        ...
        /** if WCK per word granularity, disable WCK 1 */
        if (dnx_data_dram.gddr6.wck_granularity_get(unit) == GDDR6_DRAM_WCK_PER_WORD)
        {
            SHR_IF_ERR_EXIT(dnx_gddr6_dbal_access_wck_disable(unit, dram_index, 1));
        }
