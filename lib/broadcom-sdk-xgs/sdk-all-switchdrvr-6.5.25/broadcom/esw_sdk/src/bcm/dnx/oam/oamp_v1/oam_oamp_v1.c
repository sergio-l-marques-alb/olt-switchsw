 ...
 * $Id$
 *
 * OAM OAMP table access procedures for DNX OAMP generation one.
 *
 */
/*
...
/** Check if the CCM Phase based on MEP memory type and ID and return 1- for odd, and 0 for even */
...
/** Number of bit that could be masked for flexible crc*/
 ...
 *  crc flexibale tcam entry
 */
typedef struct
{

    /************* Flexible CRC TCAM Key *************************/
    /**
     * OAMP MEP PE profile with mask used for flexible CRC check(6 bits)
     */
    ...
    /**
     *  16 bits
     ** BFDoPWE used - for gach_channel
     ** OAM_y1731 used for opcode
     ** Rest cases should be '0'
     */
    ...
    /**
     * 1 bit
     * Opcode/GACH indication
     */
    uint32 opcode_or_gach_select;
    uint32 opcode_or_gach_select_mask;

   /************* Flexible CRC TCAM Result *************************/
    /**
     * Used to identify which crc from extra_data to use(1- bit)
    */
    uint8 crc;

    /**
     * Used to identify which crc mask to use from fflexibe cec mask(3 bits)
    */
...
/**
 *  Structure used for masking the PDU that should be counted in flexible CRC
 *  crc flexible tcam entry
 */
typedef struct
{

    /** Each bit represent one bit in OAM/BFD PDU(0-7 bytes)*/
    uint8 flex_crc_per_bit_mask[BITS2BYTES(DNX_OAM_OAMP_CRC_BIT)];

    /** Each bit represent one byte in OAM/BFD PDU(8-117 bytes)*/
...
/**
 *  Structure used for allocating OAMP generic memory
 */
typedef struct
{
    ...
    /** number of genric memory entries allocated into the entity*/
    int nof_entries;

    /** index of the first genric memory entry allocated into the entity*/
    int entry_index;
...
/**
 * \brief
 * This function gets the number of extra data entries.
 */
shr_error_e
    ...
    int unit)
{
    ...
    /** For frequency less than 600MHZ short entries are not supported */
    ...
    for (i = 0; i < total_num_of_mep_db_banks; i++)
    {
        ...
        /** Set type rate: '0' - normal scan rate(1.67ms)/ '1' - slow scan rate(3.33ms) */
    ...
    uint32 value)
{
    ...
    /*
     * Check if the channel type is set, before deleting.
     * If bfd_cw_valid_dis is supported, CW verifcation is disabled by default. In this case, no need to check value_channel_type.
     */
    ...
    /*
     * GACH_CHAN_TYPE is shared between PweBfd and MplsTpBfd.
     * When deleting PweBfd, setting GACH_CHAN_TYPE to BFD_MPLS_TP_CC_GACH_DEFAULT_CHANNEL
     * When deleting MplsTpBfd, setting GACH_CHAN_TYPE to BFD_PWE_DEFAULT_CHANNEL
     */
    if (index == 0)
    {
        /*
         * bcmOamMplsTpChannelPweBfd 
         */
    ...
    uint32 header_entry_index)
{
    ...
    /** Get the number of extra data entries */
    ...
    dnx_oam_oamp_ccm_endpoint_t * entry_values)
{
    ...
    {
        /** For the RFC6374 types, the extra_data_ptr represents lm_dm_ptr in the HW */
...
/**
 * \brief - Helper function to fill system headers information for mep_db struct.
 *
 * \param [in] unit - Number of hardware unit used.
 * \param [in] endpoint_info - info structure of the requested
 *        MEP
 * \param [in] group_info - Information about the MEP's group
 * \param [in] gport_hw_resources - Information about gport
 ...
 * \param [in] slm_enabled - SLM indication
 *
 * \return
 *   shr_error_e
 *
    ...
    uint8 slm_enabled)
{
    ...
    if (!DNX_OAM_DISSECT_IS_ENDPOINT_UPMEP(endpoint_info))
    {
        ...
        if (mep_db_entry->mep_type == DBAL_ENUM_FVAL_OAMP_MEP_TYPE_ETH_OAM)
        {
            ...
            /*
             * For Jericho1 mode(inter-op) UDH should be added by microcode for Down MEP
             */
    ...
    {
        /** For server up-MEP, gport is supposed to contain the client VLAN gport */
     ...
     * except egress_injection in jericho2 case we need to set counter_ptr and counter_if
     *
     * In case of SLM the counter_pointer and counter_interface will be re-calculated during loss_add.
     */
    if ((!DNX_OAM_DISSECT_IS_ENDPOINT_UPMEP(endpoint_info)) && !slm_enabled)
    {
        ...
        {
            /**
             * On LMM/LMR/CCM piggy-back Down-MEP injection, the counter interface in OAM-TS-DATA field
             * of ASE-OAM extension should match the configured counter interface incremented by 1
             * in order to read and stamp the TX counter.
             */
...
/**
 * \brief - Helper function to fill oam pdu information for mep_db struct.
 *
 * \param [in] unit - Number of hardware unit used.
 * \param [in] endpoint_info - info structure of the requested
 *        MEP
 ...
 * \param [out] mep_db_entry - return filled mep_db entry according endpoint_info
 * \param [in] is_dual_ended_lm - dual ended indication
 * \param [in] is_1dm_enabled - return 1dm indication
 *
 *
 * \return
 *   shr_error_e
    ...
    uint8 is_1dm_enabled)
{
    ...
    if (!DNX_OAM_IS_ENDPOINT_RFC6374(endpoint_info))
    {
        ...
        /*
         * Egress injection mep_pe_profiles
         */
        ...
        {
            /*
             * In case we are Eth Down-MEP with UDH and in Jericho1 mode(inter-op) mep_pe_profile is already set
             */
            if (!((system_headers_mode == DBAL_ENUM_FVAL_SYSTEM_HEADERS_MODE_JERICHO_MODE) &&
            ...
            if (endpoint_info->endpoint_memory_type == bcmOamEndpointMemoryTypeSelfContained)
            {
                ...
                if (DNX_OAM_DISSECT_IS_EXTRA_DATA_PER_ENDPOINT(endpoint_info))
                {
                    ...
                    else
                    {
                        ...
                        {
                            /** Copy MAID48 to extra data entry and keep DMAC 0 until LM/DM entry added*/
                ...
                {
                    /** Clear extra data entries when disabling LM/DM flexible DMAC*/
                ...
                {
                    /** Additional GAL with One-way DM + 48B MAID */
                    SHR_IF_ERR_EXIT(dnx_oamp_pe_mep_pe_profile_sw_get
        ...
        else
        {
            ...
            if (DNX_OAM_DISSECT_IS_EXTRA_DATA_PER_ENDPOINT(endpoint_info))
            {
                ...
                else
                {
                    ...
                    if (DNX_OAM_LMDM_FLEXIBLE_DA_IS_SET_TO_ENABLE(endpoint_info, sw_endpoint_info))
                    {
                        /** Keep DMAC 0 until LM/DM entry added*/
                        mep_db_entry->extra_data.flags = DNX_OAMP_CCM_ENDPOINT_EXTRA_DATA_DMAC;
                        mep_db_entry->extra_data.opcodes_to_prepend =
    ...
    /** Do we need to modify the OAMP trap TCAM table? */
...
/**
 * \brief - Helper function to fill mep_db struct for replacing existing mep.
 *
 * \param [in] unit - Number of hardware unit used.
 * \param [in] endpoint_info - info structure of the requested
 *        MEP
 * \param [in,out] mep_db_entry - return filled mep_db entry according endpoint_info
 * \param [out] slm_enabled - return SLM indication
 * \param [out] is_dual_ended_lm - return dual-ended indication
 * \param [out] is_1dm_enabled - return 1dm indication
 * \param [out] mep_hw_profiles_delete_data - when modifying, return existing  profiles that
 *        should be deleted from hw
 * \param [out] mep_profile - holds information for mep_profile.
 *
 * \return
 *   shr_error_e
 *
    ...
    int is_48B_maid)
{
    ...
    if (endpoint_info->flags & BCM_OAM_ENDPOINT_WITH_ID)
    {
        ...
        {
            /*
             * MPLS-LM-DM endpoints will not reach this point because of the group_name_type parameter.
             */
    ...
    uint8 is_egress_inj)
{
    ...
    */
    ...
    {
        /*
         * For LMR the stamping is done 8 bytes after the LM stamping..
         */
        value += 8;
    }
    return value;
    ...
    int unit)
{
    ...
    /** Get the pp port of oam_gport[0] */
    ...
    dnx_oam_oamp_mep_db_args_t * oamp_params)
{
    ...
    if (search_params.update_entry)
    {
        ...
        if (!_SHR_IS_FLAG_SET(entry_values->flags, DNX_OAMP_OAM_LM_MEP_ADD_STAT_ENTRY))
        {
            ...
            {
                /**
                  * LM_DB entry already exists and is first.
                  * Move existing DM entry to LM_STAT entry index
                  */
    ...
    else
    {
            ...
            (entry_values->flex_lm_entry == MEP_DB_ENTRY_TO_OAM_ID(oamp_params->flex_lm_dm_ptr)))
        {
            /**
              * LM_DB entry already exists and is first.
              * Move existing DM entry to the next bank
              */
    ...
    int unit)
{
     ...
     * use a separate PRT qualifier(OAMP_INJECTION_DOWN) from OAM Up-MEP,
     * so ignore-protection-state can be enabled/disabled based on PRT-qualifier.
     * Otherwise, use same PRT qualifier(OAMP_INJECTION) from OAM Up-MEP.
     */
    ...
    {
        /** Enable the interface in RXI for different cores */
    ...
    /**
    * Version = 0x0
    * Flags = 0x0 (Query, No TC specific)
    * Control-Code = 0x00 (In-band Response Requested)
    * Message Length = 0x0034(bytes)
    */
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_RFC_6374_LM_DW_0, INST_SINGLE, 0x00000034);
    /**
    * Version = 0x0
    * Flags = 0x8 (Response, No TC specific)
    ...
    */
    ...
    /**
    * DFlags = 0x0 (packet counter values)
    * QTF = 0x0 (Null timestamp format)
    * Reserved = 0x000000
    */
    ...
    */
    ...
    /** Should be last: Enable scanner  machine. TX machine is enabled in dnx_stk_sys_traffic_enable_set */
    ...
    int unit)
{
    ...
    /** Setting CRC mask for MAID48(Bytes 10-57)
     *  Actually for flex_crc_mask->flex_crc_per_byte_mask it's 70-117 bits
     *  Offset(70) = 120(num of bytes that could be masked) -
     *                10(starting points in OAM PDU+
     *                8(number of bytes resides in crc mask per bit) -
     *                48(MAID48 length)
     * */
    ...
    /** Initialize 48B MAID CRC on flexible DA enabled*/
    SHR_IF_ERR_EXIT(dnx_oamp_pe_mep_pe_profile_sw_get
    ...
    /** Initializing Source IPv6 check for BFD */
    SHR_IF_ERR_EXIT(dnx_oamp_pe_mep_pe_profile_sw_get
    ...
    /** Setting CRC mask for Source IPv6(Bytes 24-39)
     *  Actually for flex_crc_mask->flex_crc_per_byte_mask it's 80-95 bits
     *  Offset(88) = 120(num of bytes that could be masked) -
     *                24(starting points in OAM PDU +
     *                8(number of bytes resides in crc mask per bit) -
     *                16
     * */
    ...
    dnx_oamp_copy_bit_array(flex_crc_mask->flex_crc_per_byte_mask, 70, mask, 0, BCM_OAM_GROUP_NAME_LENGTH, FALSE);
    /**
     * Setting CRC mask for Signal Degradation bit
     */
    ...
    /** Initialize 48B MAID CRC for addition gal with 48B MAID */
    flex_tcam_entry->oamp_mep_pe_profile_mask = 0x0;
    tcam_index++;
    flex_tcam_entry->opcode_or_gach_channel = DNX_OAM_OAMP_OPCODE_CCM_CCM_LM;
    flex_tcam_entry->opcode_or_gach_channel_mask = 0;
    ...
    /** Initialize 48B MAID CRC for down MEP egress injection with 48B MAID */
    SHR_IF_ERR_EXIT(dnx_oamp_pe_mep_pe_profile_sw_get
    ...
    /** Initialize 48B MAID CRC for Downmep and 48B MAID */
    SHR_IF_ERR_EXIT(dnx_oamp_pe_mep_pe_profile_sw_get
                    (unit, DBAL_ENUM_FVAL_MEP_PE_PROFILE_SW_DOWN_MEP_MAID_48, &mep_pe_profile));
    flex_tcam_entry->oamp_mep_pe_profile = mep_pe_profile;
    flex_tcam_entry->oamp_mep_pe_profile_mask = 0x0;
    ...
    /** Initialize 48B MAID CRC for UP-mep server and 48B MAID */
    SHR_IF_ERR_EXIT(dnx_oamp_pe_mep_pe_profile_sw_get
                    (unit, DBAL_ENUM_FVAL_MEP_PE_PROFILE_SW_UP_MEP_SERVER_MAID_48, &mep_pe_profile));
    flex_tcam_entry->oamp_mep_pe_profile = mep_pe_profile;
    flex_tcam_entry->oamp_mep_pe_profile_mask = 0x0;
    ...
    /** Initialize 48B MAID CRC for downmep injection with 1DM and 48B MAID */
    SHR_IF_ERR_EXIT(dnx_oamp_pe_mep_pe_profile_sw_get
                    (unit, DBAL_ENUM_FVAL_MEP_PE_PROFILE_SW_1DM_MAID_48, &mep_pe_profile));
    flex_tcam_entry->oamp_mep_pe_profile = mep_pe_profile;
    flex_tcam_entry->oamp_mep_pe_profile_mask = 0x0;
    ...
    /** Initialize 48B MAID CRC for DM jumbo TLV and 48B MAID */
    SHR_IF_ERR_EXIT(dnx_oamp_pe_mep_pe_profile_sw_get
                    (unit, DBAL_ENUM_FVAL_MEP_PE_PROFILE_SW_DM_JUMBO_TLV_MAID_48, &mep_pe_profile));
    flex_tcam_entry->oamp_mep_pe_profile = mep_pe_profile;
    flex_tcam_entry->oamp_mep_pe_profile_mask = 0x0;
    ...
    /** Initialize 48B MAID CRC for up MEP DM tod with 48B MAID */
    SHR_IF_ERR_EXIT(dnx_oamp_pe_mep_pe_profile_sw_get
                    (unit, DBAL_ENUM_FVAL_MEP_PE_PROFILE_SW_UP_MEP_DM_TOD_CCM_MAID_48, &mep_pe_profile));
    flex_tcam_entry->oamp_mep_pe_profile = mep_pe_profile;
    flex_tcam_entry->oamp_mep_pe_profile_mask = 0x0;
    ...
    /** Initialize 48B MAID CRC for addition gal with 1DM and 48B MAID */
    SHR_IF_ERR_EXIT(dnx_oamp_pe_mep_pe_profile_sw_get
    ...
    /** Initialize 48B MAID CRC for addition gal with Jumbo TLV and 48B MAID */
    SHR_IF_ERR_EXIT(dnx_oamp_pe_mep_pe_profile_sw_get
    ...
    dnx_oam_oamp_mep_db_args_t * oamp_params)
{
    ...
    /** LM/DM offloaded part 2 entry exists */
    if (second_mep_db_entry != 0)
    {
        /** If not update, initialize the DM values */
    ...
    dnx_oam_mep_profile_t * mep_profile_data)
{
    ...
    if (opcode == OAMP_MEP_TX_OPCODE_OP0 || opcode == OAMP_MEP_TX_OPCODE_OP1)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_oam_oamp_v1_get_transmit_rate_for_opcode
                        (unit, opcode, 0 /* dont-care for CCM rate */ , mep_profile_data, &opcode_rate));
        SHR_IF_ERR_EXIT(dnx_oam_oamp_v1_get_transmit_rate_for_opcode(unit, other_opcode, 0      /* dont-care for CCM
                                                                                                 * rate */ , mep_profile_data,
                                                                     &other_opcode_rate));
    ...
    dnx_oam_mep_profile_t * mep_profile_data)
{
     ...
     */
    SHR_IF_ERR_EXIT(dnx_oam_oamp_v1_get_transmit_rate_for_opcode
                    (unit, opcode, 0 /* dont-care about ccm rate */ , mep_profile_data, &my_rate));
    SHR_IF_ERR_EXIT(dnx_oam_oamp_v1_get_transmit_rate_for_opcode
                    (unit, other_opcode, 0 /* dont-care about ccm rate */ , mep_profile_data, &other_rate));
    ...
    /*
     * Jumbo tlv set the phase
     */
    if (is_jumbo_tlv_dm)
    {
        /*
         *  Split DMM to 8 phases accoding to id 16*4, 32*4, 48*4, 64*4....
         *  In order to have time to send Jumbo frames the steps of meps must be >=16.
         *  DM_COUNT will be: 0,2,4,....14  / 1,3,5...15 accroding to the dmm phase.
         */
        dmm_phase =
        ...
        /** DM is allocated in advanced, don't change it*/
    ...
    dnx_oam_mep_profile_t * mep_profile_data)
{
    ...
    SHR_IF_ERR_EXIT(dnx_oam_oamp_v1_get_transmit_rate_for_opcode
                    (unit, opcode, 0 /* dont-care about ccm rate */ , mep_profile_data, &tx_rate));
...
/**
 * \brief
 * This function gives the scan count for an opcode
 * given the mep_profile_data.
 *
 */
static shr_error_e
...
/**
 * \brief
 * This function calculates the phase counts for Opcodes LMM, DMM and BFD
 * and updates it in the mep_profile_data.
 *
 * LMM/DMM can take phase counts 0/1 or maximum
 * CCM/BFD is always set to maximum count.
 * Based on the period we could have the following limitations.
 * Lets assume that all periods are translated to number of scans.
 *  So 3MS period is the maximum value and it requires 2 scans to transmit, so every second scan will be reserved for it.
 *  If we have one of the period set to 3MS, the other 2 should be always even numbers(2,4,6..),
 *  and also should be modulo of each other. For example 12 and 4 are valid scans, but 4 and 6 are not.
 * Other limitation is that every other provided scan should be modulo of each other.
 *  For example, 4, 6, 12 is not valid combination, but 4, 8, 16 is valid.
 *  Like 4%4 == 0, 8%4 == 0, 16%4 == 0, 16%8 == 0.
 * Note that during the calculations the scan_rate is +1, this is because the actual scan required is always +1.
 *  For example 3MS / 1.67 == 1, so "1" will be set in the HW, but actual required scans to transmit are 2.
 */
shr_error_e
    ...
    dnx_oam_mep_profile_t * mep_profile_data)
{
    ...
    {
        /** Store the actual scans for further calculations */
        scan_rate_arr[0] = my_scan_rate + 1;
        /** If other opcode has valid tx we need to take it into account */
        if (other_cnt_2 == DNX_OAM_OAMP_ETH1731_MEP_PROFILE_MAX_CCM_CNT)
        {
            if (other_cnt != DNX_OAM_OAMP_ETH1731_MEP_PROFILE_MAX_CCM_CNT)
            {
                /** In this only one other opcode has valid tx rate */
                ...
                /** Check that the opcodes are modulo of each other */
            ...
            /** Non of the other two opcodes has valid tx rate */
        }
        else
        {
            SHR_IF_ERR_EXIT(dnx_oam_oamp_v1_get_existing_scan_rate_for_opcode
            ...
            if (other_cnt != DNX_OAM_OAMP_ETH1731_MEP_PROFILE_MAX_CCM_CNT)
            {
                /** At this stage all 3 opcodes has valid tx rate*/
                SHR_IF_ERR_EXIT(dnx_oam_oamp_v1_get_existing_scan_rate_for_opcode
                                (unit, mep_id, other_opcode, mep_profile_data, &other_scan_rate));
                ...
                /** Fist check that we don't have 2 with tx rate 3.33ms */
                if (other_scan_rate2 == 1 && other_scan_rate == 1)
                {
                    /** if 2 opcodes has tx 3.3ms, then it is not allowed 3th to transmit */
                    SHR_ERR_EXIT(_SHR_E_PARAM, "Invalid rate passed %d, two opcodes already has maximum tx rate",
                                 tx_rate);
                }
                /** Now check that each one of them has 0 modulo between each other */
    ...
    {
        /**
        *   After all validity checks passed, assign the phases.
        *   In a perfect case assign 0,1,2 phases per opcode, based on scan_rate.
        *   In case one of the rates is 3ms, assign it to phase 0, and the other to 1 and 3.
        *   In case two rates are 3ms, assign one of them to phase 0, and the other one to phase 1
        */
        if (my_scan_rate == 1)
        {
            /*
             ...
             * what BFD took.
             */
            /** At this stage only one is possible to be at 3.33ms and the other won't have valid tx */
            if (other_scan_rate == 1 || other_scan_rate2 == 1)
            {
                my_cnt = 0;
                /** The faster is the one with max rate, take other phase */
    ...
    {
        /*
         * If my rate is zero, then it does not matter what phase count to maximum value. I need not change other phase count
         * also.
         */
    ...
    dnx_oam_mep_profile_t * mep_profile_data)
{
    ...
    if (opcode != OAMP_MEP_TX_OPCODE_CCM_BFD)
    {
        /** For regular BFD this unnecessary; for MPLS-LM-DM this will cause a failure */
...
/**
 * \brief - Helper function to fill network headers information for mep_db struct.
 *
 * \param [in] unit - Number of hardware unit used.
 * \param [in] endpoint_info - info structure of the requested
 *        MEP
 ...
 * \param [out] mep_hw_profiles_write_data - return new profiles that should be created in hw
 * \param [out] mep_db_entry - return filled mep_db entry according endpoint_info
 * \param [in] is_mpls_lm_dm_without_bfd_endpoint - Indication to create mpls lm dm with new bfd endpoint
 * \param [in] is_dual_ended_lm - dual ended indication
 * \param [in,out] mep_profile - holds information for mep_profile.
 *
 * \return
 *   shr_error_e
 *
    ...
    dnx_oam_mep_profile_t * mep_profile)
{
    ...
    /*
     * For Y.1731 over MPLS-TP, MDL verification is disabled when mpls_tp_mdl_ignore is enabled.
     */
    ...
    mep_profile->lmm_offset =
        dnx_oam_oamp_v1_lm_dm_set_offset(unit, mep_db_entry, is_dual_ended_lm, 0 /** LM type */ , 0 /** LMM */ ,
    ...
    mep_profile->lmr_offset =
        dnx_oam_oamp_v1_lm_dm_set_offset(unit, mep_db_entry, is_dual_ended_lm, 0 /** LM type */ , 1 /** LMR */ ,
                                         (endpoint_info->type == bcmOAMEndpointTypeMplsLmDmLsp),
                                         _SHR_IS_FLAG_SET(endpoint_info->flags2,
                                                          BCM_OAM_ENDPOINT_FLAGS2_EGRESS_INJECTION_DOWN));
    mep_profile->dmm_offset =
        dnx_oam_oamp_v1_lm_dm_set_offset(unit, mep_db_entry, is_dual_ended_lm, 1 /** DM type */ , 0 /** DMM */ ,
                                         (endpoint_info->type == bcmOAMEndpointTypeMplsLmDmLsp),
                                         _SHR_IS_FLAG_SET(endpoint_info->flags2,
                                                          BCM_OAM_ENDPOINT_FLAGS2_EGRESS_INJECTION_DOWN));
    mep_profile->dmr_offset =
        dnx_oam_oamp_v1_lm_dm_set_offset(unit, mep_db_entry, is_dual_ended_lm, 1 /** DM type */ , 1 /** DMR */ ,
                                         (endpoint_info->type == bcmOAMEndpointTypeMplsLmDmLsp),
                                         _SHR_IS_FLAG_SET(endpoint_info->flags2,
                                                          BCM_OAM_ENDPOINT_FLAGS2_EGRESS_INJECTION_DOWN));
    ...
    {
        /**
         *  If the BFD endpoint has not been created, set the CCM cnt
         *  to maximum value in order to prevent the BFD transmission.
         */
    ...
    /** For BFD use only - meaningless for OAM */
    ...
    oam_oamp_profile_hw_data_t * mep_hw_profiles_delete_data)
{
    ...
    if (DNX_OAM_IS_ENDPOINT_RFC6374(endpoint_info) && !is_mpls_lm_dm_without_bfd_endpoint)
    {
        /** An MPLS-LM-DM EP is being added to a BFD EP */
        ...
        if (!_SHR_IS_FLAG_SET(endpoint_info->flags, BCM_OAM_ENDPOINT_REPLACE))
        {
            ...
            /** Reduce a reference to the old profile */
            SHR_IF_ERR_EXIT(algo_oam_db.
   ...
   /** Set entry type */
    ...
    if (_SHR_IS_FLAG_SET(endpoint_info->flags2, BCM_OAM_ENDPOINT_FLAGS2_EGRESS_INJECTION_DOWN))
    {
        ...
        /** Set IVEC address factor for Down-MEP Egress */
        if (_SHR_IS_FLAG_SET
    ...
    /** Setting LM/DM flexible DMAC flag or Signal Degradation flag */
    if (DNX_OAM_DISSECT_IS_EXTRA_DATA_PER_ENDPOINT(endpoint_info))
    {
        if (!DNX_OAM_IS_ENDPOINT_RFC6374(endpoint_info))
        {
    ...
    uint32 gal)
{
    ...
    {
        /** get the label profile of MPLS_LABEL_GAL*/
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_MPLS_SPECIAL_LABEL_PROFILE_MAP, &entry_handle_id));
        ...
        /** set the additional GAL to MPLS_LABEL_GAL to indicate no additional GAL */
    ...
    uint32 gal)
{
    ...
    /*
     * Parser can handle label 14 as GAL, according to VR
     */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_PEMLA_OAM, &entry_handle_id));
    /*
     * enable if and only if label is 14 
     */
    ...
    uint32 network_qos)
{
    ...
    /*
     * For program PROGRAM_ENUM_DOWN_MEP_INJ_SELF_CONTAINED and,
     * DBAL_ENUM_FVAL_OAMP_PE_PROGRAM_ENUM_DOWN_MEP_INJ_SELF_CONTAINED_MAID_48.
     * the format of program variable is,
     *    1, bit 0: indicate if MAID48 is enabled;
     *    2, bit 1-7: unused;
     *    3, bit 8-15: network QoS in PPH;
     *    4, bit 16-31: unused;
     */
    /** set program variable for program DOWN_MEP_INJ_SELF_CONTAINED */
    ...
    /** set program variable for program DOWN_MEP_INJ_SELF_CONTAINED_MAID_48 */
    ...
    else
    {
        ...
        else
        {
                ...
                || (current_counter_pointer != entry_values->slm_counter_pointer))
            {
                ...
                {
                    /*
                     * Update flex_lm_dm_ptr only if it was not pointing to an LM/DM entry
                     */
                    oamp_params->flex_lm_dm_ptr = OAM_ID_TO_MEP_DB_ENTRY(entry_values->flex_lm_entry);
                }
                oamp_params->exclusive.oam_only.oam_lmm_da_profile = entry_values->lmm_dmm_da_profile;
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if (is_mpls_lm_dm_entry_exists)
                {
                    /** Set the last entry bit in the MPLS-LM-DM entry */
        ...
        if (oamp_params->flex_lm_dm_ptr != 0)
        {
                ...
                UTILEX_GET_BIT(oamp_params->flags, OAMP_MEP_LM_STAT_ENABLE))
            {
                    ...
                    UTILEX_GET_BIT(oamp_params->flags, OAMP_MEP_Q_ENTRY))
                {
                    /**
                    * Both the LM/DM offloaded and the Q_ENTRY flags are set. That means
                    * this entry is this MEP's offloaded_2 type entry, so the DM statistics
                    * that it holds must also be reset.
                    */
                    UTILEX_SET_BIT(oamp_params->flags, TRUE, OAMP_MEP_RESET_DM_STATISTICS);
                    UTILEX_SET_BIT(oamp_params->flags, TRUE, OAMP_MEP_UPDATE);
                    SHR_IF_ERR_EXIT(dnx_oam_oamp_v1_mep_db_add(unit, second_mep_db_entry, oamp_params));
    ...
    int unit)
{
    ...
    dnx_oam_oamp_generic_memory_entity_t entity_config[DNX_OAM_OAMP_GENERIC_MEMORY_ENTITY_NOF] = {
        /** entity_id                                       nof_entries,                                                   entry_index */
