...
/** Flow SRv6 APIs */
#include <src/bcm/dnx/flow/porting/srv6_flow.h>
    ...
    int *local_lif)
{
    ...
    *egr_pointed_id = DNX_ALGO_EGR_POINTED_INVALID;
    ...
    if (!(_SHR_IS_FLAG_SET(info->flags, BCM_SRV6_SID_INITIATOR_REPLACE)))
    {
        ...
        outlif_info.dbal_table_id = DBAL_TABLE_EEDB_SRV6_SID;
        outlif_info.dbal_result_type = DBAL_RESULT_TYPE_EEDB_SRV6_SID_ETPS_DATA_128B;
    ...
    int local_lif)
{
    ...
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_EEDB_SRV6_SID, &entry_handle_id));
    ...
    uint32 *nwk_qos_profile)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_global_out_lif_bta_get
                    (unit, _SHR_GPORT_TUNNEL_ID_GET(info->tunnel_id), &info->estimated_encap_size));
    ...
    int value)
{
    ...
    nof_pairs = sizeof(ctxs_pair_list) / (sizeof(uint32) * 2);

    for (jj = 0; jj < nof_pairs; jj++)
    {
        uint32 red_context_val = 0, regular_context_val = 0;
        uint32 context_to_search, context_to_set;
        uint8 context_is_invalid = 0;

        SHR_IF_ERR_EXIT(dbal_fields_is_illegal_value(unit, DBAL_FIELD_ENCAP_1_CONTEXT_ID,
                                                     ctxs_pair_list[jj][0], &context_is_invalid));
        ...
        SHR_IF_ERR_EXIT(dbal_fields_enum_hw_value_get(unit, DBAL_FIELD_ENCAP_1_CONTEXT_ID,
                                                      ctxs_pair_list[jj][0], &regular_context_val));

        SHR_IF_ERR_EXIT(dbal_fields_enum_hw_value_get(unit, DBAL_FIELD_ENCAP_1_CONTEXT_ID,
                                                      ctxs_pair_list[jj][1], &red_context_val));
        if (value)
        {
            context_to_search = regular_context_val;
            context_to_set = red_context_val;
        }
        else
        {
            context_to_search = red_context_val;
            context_to_set = regular_context_val;
        }

        for (ii = 0; ii < 68; ii++)
        {
            int field_etps_valid = 0;
            field_0 = 0;
            SHR_IF_ERR_EXIT(soc_mem_read(unit, ETPPC_ENCAPSULATION_1_CONTEXT_SELECTION_CAMm, -1, ii, data));
            if (dnx_data_dev_init.context.feature_get(unit, dnx_data_dev_init_context_etps_selection_profile))
            {
                field_etps_valid =
                    soc_mem_field32_get(unit, ETPPC_ENCAPSULATION_1_CONTEXT_SELECTION_CAMm, data,
                                        ETPS_TOS_ENTRY_TYPE_PROFILE_MASKf);
            }
            else
            {
                field_etps_valid =
                    soc_mem_field32_get(unit, ETPPC_ENCAPSULATION_1_CONTEXT_SELECTION_CAMm, data,
                                        ETPS_TOS_ENTRY_TYPE_MASKf);
            }
            if ((value == 1) && (field_etps_valid == 0x3f)
                && (ctxs_pair_list[jj][0] == DBAL_ENUM_FVAL_ENCAP_1_CONTEXT_ID_SRV6___ENCAP_SEGMENT_MAIN_PASS))
            {
                continue;
            }

            field_0 = soc_mem_field32_get(unit, ETPPC_ENCAPSULATION_1_CONTEXT_SELECTION_CAMm, data, PROGRAMf);
            if ((field_0 >> 1) == context_to_search)
            {
                field_0 = (context_to_set << 1) | 1;
                soc_mem_field32_set(unit, ETPPC_ENCAPSULATION_1_CONTEXT_SELECTION_CAMm, data, PROGRAMf, field_0);
                SHR_IF_ERR_EXIT(soc_mem_write(unit, ETPPC_ENCAPSULATION_1_CONTEXT_SELECTION_CAMm, -1, ii, data));
            }
        }
    }
