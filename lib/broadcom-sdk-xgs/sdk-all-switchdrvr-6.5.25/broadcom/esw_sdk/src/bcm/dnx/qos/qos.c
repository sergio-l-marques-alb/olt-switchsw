    ...
    bcm_qos_map_t * map_entry)
{
    ...
    if (flags & BCM_QOS_MAP_GTP)
    {
        DNX_QOS_ERROR_IF_OUT_OF_RANGE("BCM_QOS_MAP_GTP, QOS_MAP_DSCP", map_entry->dscp, DNX_QOS_DSCP_MAX_VALUE);
    }
    ...
    else if (DNX_QOS_MAP_IS_EGRESS(map_id))
    {
        ...
        if (flags & BCM_QOS_MAP_REMARK)
        {
            ...
            if ((flags & BCM_QOS_MAP_L3_L2) && (flags & BCM_QOS_MAP_OPCODE))
            {
                int profile = DNX_QOS_MAP_PROFILE_GET(map_id);
                if (profile > DNX_QOS_TERM_STAGE_MAX_PROFILE)
                {
                    SHR_ERR_EXIT(_SHR_E_PARAM, "FWD PLUS remark profile %d must be less than 8", profile);
                }
            }
        }
    ...
    int *map_id)
{
     ...
     */
    SHR_INVOKE_VERIFY_DNXC(dnx_qos_map_create_verify(unit, flags, map_id));
    ...
    int map_opcode)
{
    ...
    if (DNX_QOS_MAP_IS_INGRESS(map_opcode))
    {
        if (DNX_QOS_MAP_IS_PHB(map_opcode))
        {
            SHR_IF_ERR_EXIT(algo_qos_db.ingress_qos_phb_opcode.free_single(unit, opcode_index, NULL));
        }
        else if (DNX_QOS_MAP_IS_REMARK(map_opcode))
        {
            SHR_IF_ERR_EXIT(algo_qos_db.ingress_qos_remark_opcode.free_single(unit, opcode_index, NULL));
        }
        else
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "wrong ingress map type [%x]\n", map_opcode);
        ...
        if (DNX_QOS_MAP_IS_REMARK(map_opcode))
        {
            if (DNX_QOS_MAP_IS_ECN(map_opcode))
            {
                SHR_IF_ERR_EXIT(algo_qos_db.
                                egress_qos_opcode.free_several(unit, DNX_QOS_ECN_BURN_OPCODE_NUM, opcode_index, NULL));
            }
            else
            {
                SHR_IF_ERR_EXIT(algo_qos_db.egress_qos_opcode.free_single(unit, opcode_index, NULL));
            }
        }
        else if (DNX_QOS_MAP_IS_MPLS_PHP(map_opcode))
        {
            SHR_IF_ERR_EXIT(algo_qos_db.egress_qos_php_opcode.free_single(unit, opcode_index, NULL));
    ...
    int map_id)
{
    ...
    if (DNX_QOS_MAP_IS_INGRESS(map_id))
    {
        if (DNX_QOS_MAP_IS_PHB(map_id) && DNX_QOS_MAP_IS_REMARK(map_id))
        {
            SHR_IF_ERR_EXIT(algo_qos_db.ingress_qos.free_single(unit, profile_index, NULL));
            SHR_IF_ERR_EXIT(dnx_qos_ingress_map_control_clear(unit, profile_index));
        }
        else if (DNX_QOS_MAP_IS_VLAN_PCP(map_id))
        {
            SHR_IF_ERR_EXIT(algo_qos_db.ingress_qos_vlan_pcp.free_single(unit, profile_index, NULL));
        }
        else if (DNX_QOS_MAP_IS_POLICER(map_id))
        {
            SHR_IF_ERR_EXIT(algo_qos_db.ingress_qos_policer.free_single(unit, profile_index, NULL));
        }
        else
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "wrong ingress map type [%x]\n", map_id);
    ...
    else if (DNX_QOS_MAP_IS_EGRESS(map_id))
    {
        if (DNX_QOS_MAP_IS_POLICER(map_id))
        {
            SHR_IF_ERR_EXIT(algo_qos_db.egress_qos_policer.free_single(unit, profile_index, NULL));
        }
        else if ((DNX_QOS_MAP_IS_REMARK(map_id)))
        {
            SHR_IF_ERR_EXIT(algo_qos_db.egress_qos.free_single(unit, profile_index, NULL));
        }
        else if ((DNX_QOS_MAP_IS_MPLS_PHP(map_id)))
        {
            SHR_IF_ERR_EXIT(algo_qos_db.egress_qos_mpls_php.free_single(unit, profile_index, NULL));
        }
        else if ((DNX_QOS_MAP_IS_ENCAP_COS(map_id)))
        {
            SHR_IF_ERR_EXIT(algo_qos_db.egress_cos_profile.free_single(unit, profile_index, NULL));
        }
        else if ((DNX_QOS_MAP_IS_OAM_PCP(map_id)))
        {
            SHR_IF_ERR_EXIT(algo_qos_db.egress_qos_oam_pcp_map.free_single(unit, profile_index, NULL));
        }
        else
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "wrong egress map type [%x]\n", map_id);
    ...
    int map_id)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_qos_map_params_verify(unit, flags, map_id, map_entry));
    ...
    int *array_count)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_qos_map_id_verify(unit, flags, map_id, NULL));
    ...
    *array_count = 0;
    ...
    for (idx = 0; idx < dnx_data_qos.qos.nof_ingress_opcode_profiles_get(unit); idx++)
    {
        ...
        if (is_allocated)
        {
            ...
            *(flags_array + count) = BCM_QOS_MAP_INGRESS | BCM_QOS_MAP_OPCODE | BCM_QOS_MAP_PHB;
            count++;
        }
    }
    /** ingress remark opcode profile*/
    for (idx = 0; idx < dnx_data_qos.qos.nof_ingress_opcode_profiles_get(unit); idx++)
    {
        ...
        if (is_allocated)
        {
            ...
            DNX_QOS_INGRESS_MAP_SET(map_id);
            *(map_ids_array + count) = map_id;
            *(flags_array + count) = BCM_QOS_MAP_INGRESS | BCM_QOS_MAP_OPCODE | BCM_QOS_MAP_REMARK;
            count++;
        }
    }
    /** egress opcode profile*/
    for (idx = 0; idx < dnx_data_qos.qos.nof_egress_opcode_profiles_get(unit); idx++)
    {
        ...
        if (is_allocated)
        {
            ...
            *(map_ids_array + count) = map_id;
            *(flags_array + count) = BCM_QOS_MAP_EGRESS | BCM_QOS_MAP_OPCODE | BCM_QOS_MAP_REMARK;
            count++;
        }
    }
    /** egress mpls php opcode profile, 0 is reserved*/
    ...
    *array_count = 0;
    ...
    for (idx = 1; idx < dnx_data_qos.qos.nof_ingress_policer_profiles_get(unit); idx++)
    {
        ...
        if (is_allocated)
        {
            ...
            DNX_QOS_INGRESS_MAP_SET(map_id);
            *(map_ids_array + count) = map_id;
            *(flags_array + count) = BCM_QOS_MAP_INGRESS | BCM_QOS_MAP_POLICER;
            count++;
        }
    }
    /** ingress vlan pcp profile*/
    for (idx = 0; idx < dnx_data_qos.qos.nof_ingress_vlan_pcp_profiles_get(unit); idx++)
    {
        ...
        if (is_allocated)
        {
            ...
            DNX_QOS_INGRESS_MAP_SET(map_id);
            *(map_ids_array + count) = map_id;
            *(flags_array + count) = BCM_QOS_MAP_INGRESS | BCM_QOS_MAP_L2_VLAN_PCP;
            count++;
        }
    }
    /** egress policer profile*/
    ...
    int arg)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_qos_control_verify(unit, flags, type, arg));
    ...
    int arg)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_qos_map_control_verify(unit, map_id, flags));
