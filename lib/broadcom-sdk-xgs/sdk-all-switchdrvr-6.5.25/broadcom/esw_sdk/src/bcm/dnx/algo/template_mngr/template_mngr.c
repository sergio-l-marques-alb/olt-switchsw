...
{
    int max_name_length = DNX_ALGO_TEMPLATE_MNGR_MAX_NAME_LENGTH;
     ...
     * Verify generic data.
     */
    if (sal_strnlen(data->name, sizeof(data->name)) > max_name_length - 1)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Name %s is too long. Must be at most %d characters", data->name,
                     max_name_length - 1);
    }

    if (sal_strnlen(data->name, sizeof(data->name)) < 1)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Template manager must have name\n");
    }
    ...
    if (_SHR_IS_FLAG_SET(data->flags, DNX_ALGO_TEMPLATE_CREATE_USE_ADVANCED_ALGORITHM))
    {
        ...
        if (sal_strnlen(data->advanced_algo_name, sizeof(data->advanced_algo_name)) < 1)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Advanced algorithm must have name\n");
        }
        if (sal_strnlen(data->advanced_algo_name, sizeof(data->advanced_algo_name)) > max_name_length - 1)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Advanced algorithm name %s is too long. Must be at most %d characters\n",
                         data->advanced_algo_name, max_name_length - 1);
        }
    }
    else
    {
    ...
    uint32 alloc_flags)
{
    ...
    sal_strncpy(data->advanced_algo_name, Template_callbacks_map_sw[data->advanced_algorithm].algo_name,
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_algo_template_create_verify(unit, data));
    ...
    uint8 *first_reference)
{
    ...
    SHR_NULL_CHECK(first_reference, _SHR_E_PARAM, "first_reference")
         ...
         */
        SHR_INVOKE_VERIFY_DNXC(dnx_algo_template_verify(unit, module_id, algo_temp));
    ...
    {
        SHR_INVOKE_VERIFY_DNXC(dnx_algo_template_profile_verify(unit, module_id, algo_temp, *profile));
    }
    ...
    uint8 *first_reference)
{
     ...
     * Verify input.
     */
    SHR_INVOKE_VERIFY_DNXC(dnx_algo_template_allocate_verify
    ...
    void *profile_data)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_algo_template_and_profile_verify(unit, module_id, algo_temp, profile));
    ...
    void *profile_data)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_algo_template_profile_data_get_verify
    ...
    uint8 *last_reference)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_algo_template_free_verify(unit, module_id, algo_temp, profile, last_reference));
    ...
    uint8 *last_reference)
{
    ...
    if (_SHR_IS_FLAG_SET(flags, DNX_ALGO_TEMPLATE_ALLOCATE_WITH_ID))
    {
         ...
         */
        SHR_INVOKE_VERIFY_DNXC(dnx_algo_template_profile_verify(unit, module_id, algo_temp, *new_profile));
    ...
    const void *new_profile_data)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_algo_template_replace_data_verify
                           (unit, module_id, algo_temp, profile, new_profile_data));
    ...
    int *profile)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_algo_template_profile_get_verify(unit, module_id, algo_temp, profile_data, profile));
    ...
    int *current_profile)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_algo_template_get_next_verify(unit, module_id, algo_temp, current_profile));
    ...
    int profile)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_algo_template_free_all_verify(unit, module_id, algo_temp, profile));
    ...
    uint8 *last_reference)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_algo_template_exchange_verify
