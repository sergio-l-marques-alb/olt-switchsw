...
#define FLOW_APP_IS_NOT_VALID(_flow_app_info)               (_flow_app_info->is_valid && !_flow_app_info->is_valid(unit))
...
/** special fields enum - for each spcial field should be an enum value and also a struct definition located in dnx_flow_spcial_fields in the same order*/
typedef enum
{
    FLOW_S_F_EMPTY = 0,
    FLOW_S_F_IPV4_DIP,
    ...
    FLOW_S_F_NOF_OF_LAYERS_TO_TERMINATE,
    FLOW_S_F_LAYER_TYPE,
    ...
    FLOW_S_F_GTP_PDU_TYPE,
    FLOW_S_F_IS_INTERMEDIATE_LIF,
...
typedef enum
{
    ...
    /** when set, for lif applications, this field must be set in the create operation and its value cannot be changed or cleared in replace */
    FLOW_SPECIAL_FIELD_IND_LIF_UNCHANGEABLE,

    /** when set, for match applications, this field must be set in the create operation and its value cannot be changed or cleared in replace */
    FLOW_SPECIAL_FIELD_IND_MATCH_UNCHANGEABLE,
...
typedef struct
{
 ...
 /*
  * Generic callback for various operations per aaplication: set hard coded fields etc. current_entry_handle_id is the
 ...
 *  FLOW_APP_TERM_MATCH_IND_MULTIPLE_DBAL_TABLES is set, should map between dbal table and key fields */
typedef int (
...
typedef struct
{
    ...
    /** Second Pass related DBAL table - optional, if exists should have the similar fields like flow_table and in the
     *  valid_flow_flags add also BCM_FLOW_HANDLE_INFO_BUD */
    dbal_tables_e second_pass_table;
     ...
     */
    dnx_flow_is_entry_related_cb is_entry_related;
     ...
     *  key fields. */
    dnx_flow_match_dbal_table_to_key_field_map_cb dbal_table_to_key_field_map;

} dnx_flow_app_config_t;

/** special fields callbacks definitons */
...
typedef struct
{
     ...
     *  of more complex mapping (such as profiles) need to set the related dbal field (for example profile field). when The
     *  mapping is more complex user can use the set/get/delete cb or the app_specific_cb */
    ...
    flow_special_field_replace_cb replace; /** this is optional - only for profiles */
...
typedef struct
{
     ...
     * This callback will be called only in case of replace with valid_elements_set relevant bit is set (if exists. Otherwise - a simple get-set).
     * In case of valid_elemtents_clear relevant bit is set - only the delete cb will be called (if exists, see below)
     */
...
/** special handling for src address indication (this is a profile that was written to another HW) */
shr_error_e flow_special_field_src_address_get(
    int unit,
    uint32 entry_handle_id,
    bcm_flow_handle_info_t * flow_handle_info,
    dnx_flow_app_config_t * flow_app_info,
...
shr_error_e flow_special_field_src_address_destroy(
    int unit,
    uint32 entry_handle_id,
    bcm_flow_handle_info_t * flow_handle_info,
    dnx_flow_app_config_t * flow_app_info,
...
shr_error_e flow_special_field_src_address_set(
    int unit,
    uint32 entry_handle_id,
    bcm_flow_handle_info_t * flow_handle_info,
    dnx_flow_app_config_t * flow_app_info,
...
shr_error_e dnx_flow_initiator_src_address_profile_replace(
    int unit,
    uint32 entry_handle_id,
    uint32 get_entry_handle_id,
    bcm_flow_handle_info_t * flow_handle_info,
...
shr_error_e dnx_flow_init_dedicated_logic_qos_create(
    int unit,
    uint32 entry_handle_id,
    bcm_flow_initiator_info_t * initiator_info,
    bcm_flow_special_fields_t * special_fields);

shr_error_e dnx_flow_init_dedicated_logic_qos_get(
    int unit,
    dnx_flow_app_config_t * flow_app_info,
    uint32 entry_handle_id,
    bcm_flow_initiator_info_t * initiator_info,
...
shr_error_e dnx_flow_init_dedicated_logic_qos_destroy(
    int unit,
    bcm_flow_handle_info_t * flow_handle_info,
    uint32 entry_handle_id,
    dnx_algo_gpm_gport_hw_resources_t * gport_hw_resources);

shr_error_e dnx_flow_init_dedicated_logic_qos_replace(
    int unit,
    bcm_flow_handle_info_t * flow_handle_info,
    dnx_flow_app_config_t * flow_app_info,
    uint32 entry_handle_id,
...
shr_error_e dnx_flow_init_dedicated_logic_qos_validate(
    int unit,
    bcm_flow_handle_info_t * flow_handle_info,
    dnx_flow_app_config_t * flow_app_info,
