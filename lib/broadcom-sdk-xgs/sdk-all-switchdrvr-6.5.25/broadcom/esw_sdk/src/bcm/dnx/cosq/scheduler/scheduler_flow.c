    ...
    if (is_interdigitated)
    {
        ...
        for (cos = 0; cos < num_pairs; cos++)
        {
            SHR_IF_ERR_EXIT(dnx_scheduler_connector_flow_id_calc(unit, core_id,
                                                                 ...
                                                                 TRUE, cur_flow_subflow, &nof_subflows));
    ...
    dnx_sch_element_se_type_e * se_type)
{
    ...
    quartet_type = dnx_data_sch.flow.quartet_type_get(unit, core_id, group_index)->type;
    region_type = dnx_data_sch.flow.region_type_get(unit, core_id, region_index)->type;

    *se_type = dnx_data_sch.sch_alloc.region_get(unit, quartet_type, region_type)->flow_type[quartet_index];
    ...
    int *is_se)
{
    ...
    SHR_IF_ERR_EXIT(dnx_sch_flow_se_type_get(unit, core_id, flow_id, &se_type));
    *is_se = (se_type != DNX_SCH_ELEMENT_SE_TYPE_NONE);
    ...
    int *is_port_hr)
{
    ...
    *is_port_hr = FALSE;
    ...
    SHR_IF_ERR_EXIT(dnx_sch_convert_flow_to_se_id(unit, core_id, flow_id, &se_id));
