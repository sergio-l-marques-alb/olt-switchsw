    ...
    int stp_state)
{
    uint32 entry_handle_id;
    dnx_algo_gpm_gport_phy_info_t gport_info;
    dbal_enum_value_field_ing_stp_state_e ingress_stp_state;
    ...
    switch (stp_state)
    {
        ...
        case BCM_STG_STP_LISTEN:
            ingress_stp_state = DBAL_ENUM_FVAL_ING_STP_STATE_BLOCK;
            break;
        case BCM_STG_STP_LEARN:
            ingress_stp_state = DBAL_ENUM_FVAL_ING_STP_STATE_LEARN;
            break;
        case BCM_STG_STP_DISABLE:
        case BCM_STG_STP_FORWARD:
            ingress_stp_state = DBAL_ENUM_FVAL_ING_STP_STATE_FORWARD;
            break;
        default:
            SHR_ERR_EXIT(BCM_E_PARAM, "Invalid stp state(%d)", stp_state);
    }

    egress_stp_state = (ingress_stp_state == DBAL_ENUM_FVAL_ING_STP_STATE_FORWARD) ?
        DBAL_ENUM_FVAL_EGR_STP_STATE_FORWARD : DBAL_ENUM_FVAL_EGR_STP_STATE_BLOCK;
    ...
    for (pp_port_index = 0; pp_port_index < gport_info.internal_port_pp_info.nof_pp_ports; pp_port_index++)
    {
        ...
        dbal_entry_value_field16_set(unit, entry_handle_id, DBAL_FIELD_ING_STP_STATE, INST_SINGLE, ingress_stp_state);
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    }
    ...
    for (pp_port_index = 0; pp_port_index < gport_info.internal_port_pp_info.nof_pp_ports; pp_port_index++)
    {
        ...
        dbal_entry_key_field16_set(unit, entry_handle_id, DBAL_FIELD_PP_PORT,
                                   gport_info.internal_port_pp_info.pp_port[pp_port_index]);
        dbal_entry_key_field16_set(unit, entry_handle_id, DBAL_FIELD_STP_TOPOLOGY_ID, stp_topology_id);
        dbal_entry_value_field16_set(unit, entry_handle_id, DBAL_FIELD_EGR_STP_STATE, INST_SINGLE, egress_stp_state);
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    }
    ...
    bcm_stg_t stg)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_stg_id_verify(unit, stg, TRUE));
    ...
    bcm_stg_t stg)
{
    ...
    else
    {
        ...
        SHR_IF_ERR_EXIT(algo_stg_db.stg_id.free_single(unit, stg, NULL));
    }
    ...
    int stp_state)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_stg_stp_set_verify(unit, stg, port, stp_state));
    ...
    int *stp_state)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_stg_stp_get_verify(unit, stg, port, stp_state));
