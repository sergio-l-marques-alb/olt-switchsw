 ...
 * $Id$
 *
 * PP initialization procedures for DNX2.
 * Include only DNX2 functions.
 * identical functinality between device families should be implemented in init_pp_common.c
 */
/*
...
#define FTMH_LB_KEY_EXT_SIZE 3
    /** Size of FTMH Stacking Ext in bytes depends on SOC stacking_extension_enable */
#define STACKING_EXT_SIZE 2     /* STACKING_EXT_HEADER_SIZE_IN_BYTES */
    ...
    int unit)
{
    ...
    if (system_headers_mode == SYSTEM_HEADERS_MODE_JERICHO2_MODE)
    {
        ...
        /** Mapped size 5 times the required size STACKING_EXT_SIZE */
    ...
    else
    {
        ...
        /*
         * Initialize virtual registers for JR2 mode system headers parsing.
         */
    ...
    int unit)
{
    ...
    if (system_headers_mode != SYSTEM_HEADERS_MODE_JERICHO2_MODE)
    {
         ...
         * the soc rif_id_max is actually bigger 1
         * than rif_range.
         */
    ...
    if (system_headers_mode != SYSTEM_HEADERS_MODE_JERICHO2_MODE)
    {
        ...
        /*
         * VR used by Up MEP ucode to indicate Stacking extension present, only in JR1 Mode
         */
        dbal_entry_value_field32_set(unit, entry_handle_id,
    ...
    {
        /*
         * VR used by Up MEP ucode to indicate Stacking extension present
         */
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_PEMLA_OAM, &entry_handle_id));
        dbal_entry_value_field32_set(unit, entry_handle_id,
    ...
    int unit)
{
    ...
    /** set interface 2 of exem to work with My MAC */
...
/**
 * \brief - set otmh configuration for tm port header type
 * It configures the PEMLA_TMOTMHFORMATS table
 *
 * \param [in] unit - Relevant unit.
 *
 * \return
...
/**
 * \brief -
 *   Init estimate egress byte_to_add and byte_to_remove configuration
 * \param [in] unit - Relevant unit
 * \return
 *   shr_error_e
 * \remark
    ...
    int unit)
{
    ...
    if (dnx_data_dev_init.general.feature_get(unit, dnx_data_dev_init_general_egress_estimated_bta_btr_config))
    {
        ...
        dbal_entry_key_field32_range_set(unit, entry_handle_id, DBAL_FIELD_TERMINATE_BY_CTX, DBAL_RANGE_ALL,
                                         DBAL_RANGE_ALL);
        /** enable adding system header termination size to estimated btr */
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_TERMINATE_SYSTEM_HEADERS, INST_SINGLE, TRUE);
        /** enable adding network header termination size (up to fwd layer index) to estimated btr */
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_TERMINATE_NETWORK_HEADERS, INST_SINGLE, TRUE);
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));

        /** if JR1 mode, need to consider also UDH size (if exist) in the estimated btr */
        system_headers_mode = dnx_data_headers.system_headers.system_headers_mode_get(unit);
...
/**
 * \brief -
 *   Init ERPP FIFO Almost full threshold configuration
 *
 * \param [in] unit - Relevant unit.
 *
 *
...
/**
 * \brief - Verify that if vtt1_lookup_by_exem1==1 , SEXEM_1 size > 0 and ISEM_1 is zero.
 *  It allows to do bridge lookup in VTT1 in EXEM1 db instead of ISEM1. This was made to free
 *  more entries for other databases (such as GLEM) on the expense of ISEM1. Therefore, ISEM1 size must be zero in such case.
 * \param [in] unit - Relevant unit.
 *
 * \return
 *   Error inidication according to shr_error_e enum
    ...
    int unit)
{
    ...
    {
        /** in adapter the check is not relevant, because it return always capacity > 0 */
    ...
    int unit)
{
    ...
    /*
     * As there is no enabler per FFC, the not used FFC must be directed to zero value.
     * Take it from the SRH, the top bit of the Segment Left (it has 8 bits and we support it up to 22)
     */
    sal_memset(&ecol_ffc, 0, sizeof(ecol_ffc_t));
    ecol_ffc.opcode = 0;
    ecol_ffc.index = 7;
    ...
    int unit)
{
     ...
     * { 0, 0, (144 * 8 - 8 - 8), 7 , 0}
     * op_code: 0: from relative header
     * index: 0 (indicate to take offset of parsing_layer_idx)
     * field offset: Need to go back 64 - 8 - 8 bits to point to the LSB of SRV6.Ext_len
     * output field size: field size + 1
     */
    ecol_ffc.opcode = 0;
    ecol_ffc.index = 0;
    ...
    /*
     * As there is no enabler per FFC, the not used FFC must be directed to zero value.
     * Take it from the SRH, the top bit of the Segment Left (it has 8 bits and it is 0 in that eco-logic context)
     */
    ...
    /*
     * set btc offset shift in bytes = 0, as the parsing offset is right after the the SRH
     */
    ...
    int unit)
{
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ECOL_FFC_INSTRUCTION_2, INST_SINGLE,
                                 BUILD_ECOL_FFC_INSTRUCTION_16(ecol_ffc));
    /*
     * BTK - Parsing_layer_index - 0. the start of hte SID list
     */
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ECOL_BTK_OFFSET_BASE_IDX, INST_SINGLE,
                                 DBAL_ENUM_FVAL_ECOL_OFFSET_BASE_IDX_PARSING_LAYER_OFFSET);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ECOL_BTK_OFFSET_SHIFT_IN_BYTES, INST_SINGLE, 0);
    ...
    /*
     * BTC - Parsing_layer_index - 0  + { (max_sids_1_pass+1) * 16}
     */
