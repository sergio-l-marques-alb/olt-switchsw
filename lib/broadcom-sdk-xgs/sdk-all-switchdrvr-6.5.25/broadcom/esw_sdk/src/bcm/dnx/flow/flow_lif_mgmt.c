    ...
    if (flow_app_info->flow_app_type == FLOW_APP_TYPE_TERM)
    {
        ...
        else
        {
            SHR_IF_ERR_EXIT(lif_table_mngr_db.
                            flow_ingress_virtual_gport_id.is_allocated(unit, decoded_id, is_allocated));
        }
    }
 ...
 */
static shr_error_e
dnx_flow_terminator_virtual_mapping_sw_add(
    int unit,
 ...
 */
static shr_error_e
dnx_flow_terminator_virtual_mapping_sw_clear(
    int unit,
    bcm_gport_t virtual_id)
{
    uint32 sw_entry_handle_id;
 ...
 */
shr_error_e
dnx_flow_terminator_virtual_mapping_sw_local_lif_get(
    int unit,
    int virtual_id,
    int core_idx,
    int *local_in_lif)
 ...
 */
shr_error_e
dnx_flow_terminator_virtual_mapping_sw_gport_get(
    int unit,
    int local_in_lif,
    int core_idx,
    int *virtual_id)
{
    uint32 sw_entry_handle_id;
    int rule_local_in_lif, is_end, instance_id;
    ...
    if (is_end == TRUE)
    {
        ...
        SHR_ERR_EXIT(_SHR_E_NOT_FOUND,
                     "Error! No entry found in INGRESS_GPORT_TO_LOCAL_IN_LIF_SW matching local_in_lif = %d in core %d\n",
                     local_in_lif, instance_id);
    }
    ...
    int local_lif_id)
{
    ...
    inlif_info.local_lif_flags |= (DNX_ALGO_LIF_MNGR_LOCAL_INLIF_RESERVE | DNX_ALGO_LIF_MNGR_LOCAL_INLIF_REPLACE);
        ...
        if ((!entry_reuse) && (!dnx_data_lif.in_lif.feature_get(unit, dnx_data_lif_in_lif_phy_db_dpc)))
        {
            SHR_IF_ERR_EXIT(dnx_flow_lif_mgmt_reserve_new_lif
                            (unit, core_id, dbal_table_id, (lif_table_mngr_inlif_info_t *) lif_info, new_result_type,
                             existing_result_type, local_lif_id));
        }
        else
        {
        ...
        if (is_ingress)
        {
            SHR_IF_ERR_EXIT(dnx_flow_lif_mgmt_reserve_new_lif
                            (unit, core_id, dbal_table_id, (lif_table_mngr_inlif_info_t *) lif_info, result_type,
                             current_result_type, local_lif_id));
        }
        else
        {
            /*
    ...
    uint32 selectable_result_types)
{
        ...
        && is_new_local_lif)
    {
        ...
        else
        {
            ...
            SHR_IF_ERR_EXIT(dnx_flow_terminator_virtual_mapping_sw_clear(unit, gport_hw_resources->virtual_id));
            SHR_IF_ERR_EXIT(dnx_flow_terminator_virtual_mapping_sw_add
    ...
    uint32 selectable_result_types)
{
    ...
    *local_lif_id = local_lifs_array.local_lifs[0];
    ...
    if (!_SHR_IS_FLAG_SET(lif_info->flags, LIF_TABLE_MNGR_LIF_INFO_DONT_UPDATE_GLOBAL_TO_LOCAL_MAPPING))
    {
        ...
        {
            SHR_IF_ERR_EXIT(dnx_flow_terminator_virtual_mapping_sw_add(unit,
                                                                       BCM_GPORT_SUB_TYPE_VIRTUAL_INGRESS_NATIVE_GET
    ...
    uint32 selectable_result_types)
{
    ...
    SHR_IF_ERR_EXIT(dnx_flow_lif_mgmt_terminator_info_symmetric_create
                    (unit, flow_handle_info, entry_handle_id, local_lif_id, lif_info, selectable_result_types));
...
/** See .h file */
shr_error_e
dnx_flow_lif_mgmt_no_local_lif_entry_on_handle_get(
    int unit,
    ...
    uint32 entry_handle_id)
{
    ...
    /** Retrieve the virtual match key from SW and set it on the handler */
    SHR_IF_ERR_EXIT(dnx_flow_virtual_match_sw_get
                    (unit, no_local_lif_gport, entry_handle_id, &virtual_match_type, &result_type));
...
/** See .h file */
shr_error_e
dnx_flow_lif_mgmt_no_local_lif_entry_clear(
    int unit,
    bcm_gport_t no_local_lif_gport,
    dbal_tables_e dbal_table_id)
{
    ...
    SHR_IF_ERR_EXIT(lif_table_mngr_db.valid_fields_per_no_local_lif_egress_htb.delete(unit, &no_local_lif_gport));

    /** Delete the SW mapping entry from no-local-LIF gport to the match keys and the result-type */
    SHR_IF_ERR_EXIT(dnx_flow_virtual_match_sw_clear(unit, no_local_lif_gport));
