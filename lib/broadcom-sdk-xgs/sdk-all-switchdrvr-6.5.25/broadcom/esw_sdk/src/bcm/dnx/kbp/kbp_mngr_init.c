...
#include <src/bcm/dnx/field/map/auto_generated/dbx_pre_compiled_fwd_app_type_auto_generated_xml_parser.h>
    ...
    uint8 *valid_location)
{
    ...
    if (sal_strncmp(result_location, "FWD", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
    {
        *valid_location = DBAL_DEFINE_KBP_RESULT_LOCATION_FWD;
    }
    else if (sal_strncmp(result_location, "RPF", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
    {
...
kbp_mngr_result_info_get(
    ...
    uint32 *large_optimized_result_size,
    uint8 use_large_optimized_result)
{
    SHR_FUNC_INIT_VARS(unit);
    ...
    SHR_NULL_CHECK(large_optimized_result_size, _SHR_E_INTERNAL, "large_optimized_result_size");
        ...
        for (result_type_index = 0; result_type_index < table->nof_result_types; result_type_index++)
        {
                ...
                DNX_KBP_OPTIMIZED_RESULT_TYPE_HW_VALUE)
            {
                ...
                for (result_field_index = 0; result_field_index < res_info->nof_result_fields; result_field_index++)
                {
                    ...
                    if (use_large_optimized_result && !DNX_KBP_FIELD_OMMITTED_FROM_LARGE_OPTIMIZED_RESULT(field_id))
                    {
                        *large_optimized_result_size += res_info->results_info[result_field_index].field_nof_bits;
                    }
                }
            }
            else
            {
                ...
                SHR_IF_ERR_EXIT(dnx_kbp_valid_result_width(unit, table->max_payload_size, regular_result_size));
            }
            ...
            /** Update the result widths to valid values */
            SHR_IF_ERR_EXIT(dnx_kbp_valid_result_width(unit, *optimized_result_size, optimized_result_size));
            SHR_IF_ERR_EXIT(dnx_kbp_valid_result_width
                            (unit, *large_optimized_result_size, large_optimized_result_size));

            /** Check if the optimized and large optimized result sizes are the same */
            if ((*large_optimized_result_size) && ((*large_optimized_result_size) == (*optimized_result_size)))
            {
                /** Result sizes are the same -> no need for the large one */
                *large_optimized_result_size = 0;
            }
        }
    }
    ...
    uint8 *nof_segments)
{
    ...
    *nof_segments = 0;
    str_len = sal_strnlen(segment_string, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH);
    ...
    uint8 is_standard_image)
{
    ...
    SHR_IF_ERR_EXIT(dbal_fields_enum_value_get
                    (unit, DBAL_FIELD_KBP_FORWARD_APP_TYPES, opcode_id, &opcode_id_enum_value));
    ...
    uint32 opcode_id)
{
    ...
    if ((key_size[0] == 0) && (key_size[1] == 0))
    {
        ...
        SHR_ERR_EXIT(_SHR_E_CONFIG,
                     ...
                     dbal_enum_id_type_to_string(unit, DBAL_FIELD_TYPE_DEF_FWD2_CONTEXT_ID, fwd_ctx_id_enum_value),
                     context_index, dbal_enum_id_type_to_string(unit, DBAL_FIELD_TYPE_DEF_KBP_FORWARD_APP_TYPES,
                                                                opcode_id_enum_value), opcode_id);
    }
    ...
    uint32 *nof_contexts)
{
    ...
    void *iter;
    uint32 entry_handle_id;
    uint32 fwd_ctx_enum_id[DNX_DATA_MAX_FIELD_EXTERNAL_TCAM_MAX_FWD_CONTEXT_NUM_FOR_ONE_APPTYPE] = { 0 };
    uint32 fwd_ctx_hw_id[DNX_DATA_MAX_FIELD_EXTERNAL_TCAM_MAX_FWD_CONTEXT_NUM_FOR_ONE_APPTYPE] = { 0 };
    int ctx_index = 0;
    int nof_ctxs = 0;
    char *context_node_name = is_fwd_opcode ? "Context" : "FwdContext";
    char *context_name_attr_name = is_fwd_opcode ? "Name" : "name";

    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);

    /*
     * Different XMLs are being read depending on is_fwd_opcode:
     * Read from KBP Opcodes XML if is_fwd_opcode = TRUE
     * Read from FWD AppTypes XML if is_fwd_opcode = FALSE
     */
    if (is_fwd_opcode)
    {
    ...
    {
        iter_top = opcode_node;
    }

    RHDATA_ITERATOR(iter, iter_top, context_node_name)
    {
        uint8 is_enum_value_illegal = FALSE;
        char ctx_name[DNX_KBP_KEY_SEGMENT_NAME_SIZE_IN_BYTES] = { 0 };

        RHDATA_GET_STR_STOP(iter, context_name_attr_name, ctx_name);
        ...
        SHR_IF_ERR_EXIT(dbal_fields_is_illegal_value(unit, DBAL_FIELD_FWD2_CONTEXT_ID, fwd_ctx_enum_id[ctx_index],
                                                     &is_enum_value_illegal));
        if (is_enum_value_illegal)
        {
            LOG_VERBOSE(BSL_LOG_MODULE, (BSL_META_U(unit, "opcode %s (%d) related to invalid context %s\n"),
                                         dbal_enum_id_type_to_string(unit, DBAL_FIELD_TYPE_DEF_KBP_FORWARD_APP_TYPES,
                                                                     opcode_id), opcode_id, ctx_name));
            continue;
        }
        /*
        ...
        SHR_IF_ERR_EXIT(dbal_fields_enum_hw_value_get
                        (unit, DBAL_FIELD_FWD2_CONTEXT_ID, fwd_ctx_enum_id[ctx_index], &fwd_ctx_hw_id[ctx_index]));

        SHR_IF_ERR_EXIT(kbp_mngr_fwd_context_to_opcode_hw_verify_update
                        (unit, fwd_ctx_hw_id[ctx_index], fwd_ctx_hw_id[ctx_index], opcode_id, is_standard_image));

        LOG_VERBOSE(BSL_LOG_MODULE, (BSL_META_U(unit, "opcode %s (%d) related to context %s (%d) ACL context %d\n"),
                                     dbal_enum_id_type_to_string(unit, DBAL_FIELD_TYPE_DEF_KBP_FORWARD_APP_TYPES,
                                                                 opcode_id), opcode_id, ctx_name,
                                     fwd_ctx_hw_id[ctx_index], fwd_ctx_hw_id[ctx_index]));

        ctx_index++;
    }

    nof_ctxs = ctx_index;
    if (nof_ctxs > DNX_DATA_MAX_FIELD_EXTERNAL_TCAM_MAX_FWD_CONTEXT_NUM_FOR_ONE_APPTYPE)
    {
        SHR_ERR_EXIT(_SHR_E_CONFIG, "Opcode %d is mapped to %d contexts, which is more than the available %d\n",
                     opcode_id, nof_ctxs, DNX_DATA_MAX_FIELD_EXTERNAL_TCAM_MAX_FWD_CONTEXT_NUM_FOR_ONE_APPTYPE);
    }
    ...
    if (disable_fwd)
    {
        ...
        for (ctx_index = 0; ctx_index < nof_ctxs; ctx_index++)
        {
            SHR_IF_ERR_EXIT(kbp_mngr_pd_order_disable(unit, fwd_ctx_hw_id[ctx_index]));
        }
    }
    else if (!SAL_BOOT_PLISIM)
    {
        ...
        for (ctx_index = 0; ctx_index < nof_ctxs; ctx_index++)
        {
            SHR_IF_ERR_EXIT(kbp_mngr_context_lookups_validate(unit, fwd_ctx_hw_id[ctx_index], opcode_id));
        }
    }

    *nof_contexts = nof_ctxs;
    ...
    uint8 key_segment_index[DNX_KBP_MAX_NOF_SEGMENTS_PER_LOOKUP])
{
    ...
    for (index = 0; index < nof_key_segments; index++)
    {
        ...
        {
            if (key_segment_index[index_2] == key_segment_index[index])
            {
    ...
    uint8 is_standard_image)
{
    ...
    SHR_IF_ERR_EXIT(kbp_mngr_opcode_to_context_read
                    (unit, opcode_node, opcode_id, disable_fwd_db, is_standard_image, TRUE, &nof_fwd_contexts));

    if (nof_fwd_contexts == 0)
    {
        LOG_VERBOSE(BSL_LOG_MODULE,
                    (BSL_META_U(unit, "Opcode %s (%d) has no contexts attached\n"), opcode_name, opcode_id));
        SHR_EXIT();
    }
    ...
    if (is_standard_image)
    {
        ...
        if (opcode_id_enum == DBAL_ENUM_FVAL_KBP_FORWARD_APP_TYPES_IPV4_MULTICAST_PRIVATE_W_UC_RPF)
        {
        ...
        if (opcode_id_enum == DBAL_ENUM_FVAL_KBP_FORWARD_APP_TYPES_IPV6_MULTICAST_PRIVATE_W_UC_RPF)
        {
            sal_memset(&fwd_tcam_access_hash_info, 0, sizeof(fwd_tcam_access_hash_info));
            fwd_tcam_access_hash_info.expected_nof_elements =
    ...
    uint8 acl_context)
{
    ...
    /*
     * Check that the opcode has no lookups,
     * but account for the option that a dummy lookup was added for another context.
     */
    if ((result_bmp != 0 && result_bmp != (1 << lookup_result_id)) ||
        (lookup_bmp != 0 && lookup_bmp != (1 << lookup_result_id)))
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL,
    ...
    SHR_IF_ERR_EXIT(dnx_field_kbp_key_acl_to_fwd2_key_id_convert(unit, kbp_kbr_kbp, &kbp_kbr_fwd));

    /*
     * Set the dummy lookup, if not set already.
     */
    if (result_bmp == 0)
    {
    ...
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL,
                     "acl_context %d has no lookup, but first key aligner size isn't zero (%d).\n",
                     acl_context, aligner_size_get);
    }
    ...
    if (ffc_bitmap[0] != 0 || ffc_bitmap[1] != 0)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL,
                     "acl_context %d has no lookup, ffc bitmap isn't zero (0x%x,0x%x).\n",
                     acl_context, ffc_bitmap[0], ffc_bitmap[1]);
    }
    ffc_bitmap[0] |= 1;
    ...
    uint8 *dummy_lookup_added)
{
    ...
    uint32 result_bmp;
    uint32 list_opcode_id_to_add_dummy[DNX_KBP_NOF_FWD_CONTEXTS];
    uint8 list_acl_context_to_add_dummy[DNX_KBP_NOF_FWD_CONTEXTS];
    int nof_contexts_without_lookup = 0;
    int contexts_without_lookup_index;
    int context_already_found;
    int acl_to_add_dummy_index;
    shr_error_e rv;
    ...
    *dummy_lookup_added = FALSE;
    ...
    for (fwd_ctx_index = 0; fwd_ctx_index < fwd_nof_contexts; fwd_ctx_index++)
    {
        ...
        for (acl_context = 0; acl_context < DNX_KBP_NOF_FWD_CONTEXTS; acl_context++)
        {
            ...
            if (rv != _SHR_E_NOT_FOUND)
            {
                ...
                {
                    /*
                     * Add to the list of ACL contexts to potentially add a dummy lookup to,
                     * if acl context not already on the list.
                     */
                    context_already_found = FALSE;
                    for (contexts_without_lookup_index = 0; contexts_without_lookup_index < nof_contexts_without_lookup;
                         contexts_without_lookup_index++)
                    {
                        if (list_acl_context_to_add_dummy[contexts_without_lookup_index] == acl_context)
                        {
                            if (list_opcode_id_to_add_dummy[contexts_without_lookup_index] != derived_opcode)
                            {
                                SHR_ERR_EXIT(_SHR_E_INTERNAL,
                                             "acl_context %d belongs to more than one opcode, %d and %d.\n",
                                             acl_context, list_opcode_id_to_add_dummy[contexts_without_lookup_index],
                                             derived_opcode);
                            }
                            context_already_found = TRUE;
                            break;
                        }
                    }
                    if (context_already_found == FALSE)
                    {
                        list_opcode_id_to_add_dummy[nof_contexts_without_lookup] = derived_opcode;
                        list_acl_context_to_add_dummy[nof_contexts_without_lookup] = acl_context;
                        nof_contexts_without_lookup++;
                    }
                }
                else
                {
    ...
    if ((nof_contexts_without_lookup == 0) && no_opcode_has_lookup)
    {
    ...
    {
        for (acl_to_add_dummy_index = 0; acl_to_add_dummy_index < nof_contexts_without_lookup; acl_to_add_dummy_index++)
        {
            /*
             * Set A new lookup and set a byte to the aligner.
             */
            SHR_IF_ERR_EXIT(kbp_mngr_opcode_without_lookup_add_dummy_lookup
                            (unit, list_opcode_id_to_add_dummy[acl_to_add_dummy_index],
                             list_acl_context_to_add_dummy[acl_to_add_dummy_index]));
...
kbp_mngr_db_size_and_capacity_get(
    ...
    uint8 large_optimized_result_valid,
    uint32 *table_size,
    uint32 *regular_capacity,
    uint32 *optimized_capacity,
    uint32 *large_optimized_capacity)
{
    int set_size;
    ...
    SHR_NULL_CHECK(large_optimized_capacity, _SHR_E_INTERNAL, "large_optimized_capacity");
    ...
    *table_size = set_size;
    ...
    if (large_optimized_result_valid)
    {
        *large_optimized_capacity = DNX_KBP_LARGE_OPTIMIZED_RESULT_SIZE_DEFAULT_CAPACITY;
    }
    ...
    int algo_mode)
{
    ...
    uint32 large_opt_result_width = 0;
    uint8 use_large_optimized_result = FALSE;
    int key_index;
    const dbal_logical_table_t *table;
    ...
    uint32 large_optimized_result_capacity = 0;
    int xl_db = 0;
    int locality = 0;
    enum kbp_device_type device_type;
    ...
    use_large_optimized_result = dnx_data_elk.application.db_property_get(unit, table_id)->use_large_optimized_result;
    /** Check if there is a need to use the optimized result */
    SHR_IF_ERR_EXIT(kbp_mngr_result_info_get
                    (unit, table, &result_width, &opt_result_width, &large_opt_result_width,
                     use_large_optimized_result));
    ...
    dbal_entry_value_field16_set(unit, entry_handle_id, DBAL_FIELD_LARGE_OPTIMIZED_WIDTH, INST_SINGLE,
                                 large_opt_result_width);
    ...
    SHR_IF_ERR_EXIT(kbp_mngr_db_size_and_capacity_get
                    (unit, table_id, initial_capacity, (result_width ? TRUE : FALSE), (opt_result_width ? TRUE : FALSE),
                     (large_opt_result_width ? TRUE : FALSE), &table_size, &regular_result_capacity,
                     &optimized_result_capacity, &large_optimized_result_capacity));
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LARGE_OPTIMIZED_CAPACITY, INST_SINGLE,
                                 large_optimized_result_capacity);
    ...
    /** set app_id to the table only for original DBs and not for clone DBs */
    if (clone_id == 0)
    /** set app_id to the table only for original DBs and not for clone DBs */
        if (clone_id == 0)
        {
    ...
    uint8 *clone_db_id)
{
    ...
    /** Do not allow cloning for ACL DBs */
    SHR_IF_ERR_EXIT(dbal_tables_table_get(unit, dbal_table_id, &table));
    if (table->table_type == DBAL_TABLE_TYPE_TCAM_BY_ID)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "A scenario occurred where an ACL DB needed to be cloned.\n");
    }
    SHR_IF_ERR_EXIT(dbal_entry_handle_value_field_arr8_get
    ...
    SHR_IF_ERR_EXIT(kbp_mngr_db_create_internal
                    (unit, dbal_table_id, original_db_id, counters_enable[0], initial_capacity,
 ...
 *    _SHR_E_UNAVAIL   - Indicates that the DB size/capacity is set to 0 and the DB should not be created.
 * \remark
 *   None
 * \see
 *   shr_error_e
    ...
    char key_name[DNX_KBP_MAX_NOF_SEGMENTS_PER_LOOKUP][DNX_KBP_KEY_SEGMENT_NAME_SIZE_IN_BYTES])
{
    ...
    SHR_IF_ERR_EXIT(dbal_entry_handle_value_field_arr32_get
                    (unit, db_entry_handle_id, DBAL_FIELD_TABLE_SIZE, INST_SINGLE, &db_size));
    if (db_size == 0)
    {
        /** DB needs to be disabled */
    ...
    for (index = 0; index < nof_db_segments; index++)
    {
        ...
        if (db_update == TRUE)
        {
            if (sal_strncmp(db_key_name, DNX_KBP_KEY_SEGMENT_NOT_CONFIGURED, sizeof(db_key_name)) != 0)
            {
        ...
        if (db_found == TRUE)
        {
            if (sal_strncmp(db_key_name, key_name[db_key_index[index]], sizeof(db_key_name)) != 0)
            {
    ...
    while (!is_end)
    {
        ...
        for (lookup_index = 0; lookup_index < DNX_KBP_MAX_NOF_LOOKUPS; lookup_index++)
        {
            ...
            if (!is_hole_lookup)
            {
                ...
                if (res == _SHR_E_UNAVAIL)
                {
                    uint8 result_index = -1;
                    uint32 result_bmp[1] = { 0 };

                    LOG_VERBOSE(BSL_LOG_MODULE, (BSL_META_U(unit, "disable (zero capacity)\n")));

                    /** Delete the result */
                    SHR_IF_ERR_EXIT(dbal_entry_handle_value_field8_get
                                    (unit, lkp_entry_handle_id, DBAL_FIELD_RESULT_ID, INST_SINGLE, &result_index));
                    dbal_entry_key_field8_set(unit, res_entry_handle_id, DBAL_FIELD_OPCODE_ID, opcode_id);
                    dbal_entry_key_field8_set(unit, res_entry_handle_id, DBAL_FIELD_RESULT_ID, result_index);
                    ...
                    /** Update the opcode lookup bitmap */
                    SHR_IF_ERR_EXIT(utilex_bitstream_reset_bit(lookup_bmp, lookup_index));
                    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LOOKUP_BITMAP, INST_SINGLE,
                                                 lookup_bmp[0]);
                    /** Update the opcode result bitmap */
                    SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                                    (unit, entry_handle_id, DBAL_FIELD_RESULT_BITMAP, INST_SINGLE, result_bmp));
                    SHR_IF_ERR_EXIT(utilex_bitstream_reset_bit(result_bmp, result_index));
                    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_RESULT_BITMAP, INST_SINGLE,
                                                 result_bmp[0]);
                    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
                }
                else if (res == _SHR_E_EXISTS)
                {
                    uint8 lookup_type = DBAL_ENUM_FVAL_KBP_LOOKUP_TYPE_FREE;
    ...
    int unit)
{
    ...
    uint32 table_size = 0, regular_capacity = 0, optimized_capacity = 0, large_optimized_capacity = 0;
    uint32 key_width = 0, result_width = 0, opt_result_width = 0, large_opt_result_width = 0;
    uint32 nof_segments = 0;
    ...
    while (!is_end)
    {
        ...
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field_arr32_get
                        (unit, entry_handle_id, DBAL_FIELD_LARGE_OPTIMIZED_WIDTH, INST_SINGLE,
                         &large_opt_result_width));
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field_arr32_get
        ...
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field_arr32_get
                        (unit, entry_handle_id, DBAL_FIELD_LARGE_OPTIMIZED_CAPACITY, INST_SINGLE,
        ...
        if (clone_db == 0)
        {
            ...
            /** Initialize optimized AD DBs according to optimized indication and optimized result size present */
            if (DNX_KBP_USE_OPTIMIZED_RESULT)
            {
                ...
                if (large_opt_result_width)
                {
                    db_handles.large_opt_result_size = large_opt_result_width;
                    DNX_KBP_TRY_PRINT(kbp_ad_db_init
                                      (device, ad_db_id, large_optimized_capacity, large_opt_result_width,
                                       &db_handles.ad_db_p[DNX_KBP_AD_DB_INDEX_LARGE_OPTIMIZED]), "DB %d AD DB %d",
                                      db_id, ad_db_id);
                    DNX_KBP_TRY_PRINT(kbp_db_set_ad
                                      (db_handles.db_p, db_handles.ad_db_p[DNX_KBP_AD_DB_INDEX_LARGE_OPTIMIZED]),
                                      "DB %d AD DB %d", db_id, ad_db_id);
                    ad_db_id++;
                }
            }
        ...
        else
        {
            ...
            if (DNX_KBP_USE_OPTIMIZED_RESULT)
            {
                ...
                if (large_opt_result_width)
                {
                    db_handles.ad_db_p[DNX_KBP_AD_DB_INDEX_LARGE_OPTIMIZED] =
                        cloned_db_handles.ad_db_p[DNX_KBP_AD_DB_INDEX_LARGE_OPTIMIZED];
                }
            }
         ...
         */
        if ((dnx_data_elk.application.ipv4_large_mc_enable_get(unit) ==
             dnx_data_elk.application.large_mc_mode1_get(unit)
         ...
         */
        if ((dnx_data_elk.application.ipv4_large_mc_enable_get(unit) ==
             dnx_data_elk.application.large_mc_mode2_get(unit)
             && dbal_table_id == DBAL_TABLE_KBP_IPV4_MULTICAST_TCAM_FORWARD)
            || (dnx_data_elk.application.ipv6_large_mc_enable_get(unit) ==
                dnx_data_elk.application.large_mc_mode2_get(unit)
                && dbal_table_id == DBAL_TABLE_KBP_IPV6_MULTICAST_TCAM_FORWARD))
        ...
        /*
         * When large multicast mode=3 is enabled, set the following db properties to KBP
         */
        if ((dnx_data_elk.application.ipv4_large_mc_enable_get(unit) ==
             dnx_data_elk.application.large_mc_mode3_get(unit)
             && dbal_table_id == DBAL_TABLE_KBP_IPV4_MULTICAST_TCAM_FORWARD)
            || (dnx_data_elk.application.ipv6_large_mc_enable_get(unit) ==
                dnx_data_elk.application.large_mc_mode3_get(unit)
                && dbal_table_id == DBAL_TABLE_KBP_IPV6_MULTICAST_TCAM_FORWARD))
        {
            DNX_KBP_TRY_PRINT(kbp_db_set_property
                              (db_handles.db_p, KBP_PROP_ALGORITHMIC, DBAL_ENUM_FVAL_ALGO_MODE_NET_ACL),
                              "DB %d Algo mode 3", db_id);
            DNX_KBP_TRY_PRINT(kbp_db_set_property(db_handles.db_p, KBP_PROP_MC_DB, KBP_DB_MC_TYPE3), "DB %d", db_id);
        }
    ...
    int unit)
{
    ...
    for (db_id = 1; db_id < DNX_KBP_NOF_DBS; db_id++)
    {
        ...
        if (db_handles.db_p != NULL)
        {
            ...
            if (db_handles.ad_db_p[DNX_KBP_AD_DB_INDEX_LARGE_OPTIMIZED] != NULL)
            {
                DNX_KBP_TRY_PRINT(kbp_ad_db_refresh_handle
                                  (device, db_handles.ad_db_p[DNX_KBP_AD_DB_INDEX_LARGE_OPTIMIZED],
                                   &db_handles.ad_db_p[DNX_KBP_AD_DB_INDEX_LARGE_OPTIMIZED]),
                                  "DB %d Large optimized AD_DB", db_id);
            }
    ...
    int unit)
{
    ...
    if (is_standard_image)
    {
        sal_snprintf(opcodes_path, DBAL_MAX_PRINTABLE_BUFFER_SIZE,
                     "jericho2_a0/kbp/auto_generated/%s/%s_kbp_opcodes_definition.xml", image_name, image_name);
        catalogue = dbx_pre_compiled_kbp_top_get(unit, opcodes_path, "KbpOpcodesDbCatalogue", CONF_OPEN_PER_DEVICE);
    }
    else
    {
        sal_snprintf(opcodes_path, DBAL_MAX_PRINTABLE_BUFFER_SIZE,
                     "bare_metal/kbp/auto_generated/%s/%s_kbp_opcodes_definition.xml", image_name, image_name);
        catalogue = dbx_pre_compiled_kbp_top_get(unit, opcodes_path, "KbpOpcodesDbCatalogue", 0);
    }
    if (catalogue == NULL)
    {
    ...
    int unit)
{
    ...
    uint32 opcode_id;
    uint32 entry_handle_id;
    char *fwd_app_type_path_format = NULL;
    ...
    SHR_IF_ERR_EXIT(dnx_pp_prgm_default_image_check(unit, &is_standard_image));
    SHR_IF_ERR_EXIT(dnx_pp_prgm_device_image_name_get(unit, &image_name));
    fwd_app_type_path_format = (is_standard_image) ?
        "auto_generated/%s_fwd_app_type.xml" : "bare_metal/auto_generated/fwd_app_type/%s_fwd_app_type.xml";
    sal_snprintf(opcodes_path, DBAL_MAX_PRINTABLE_BUFFER_SIZE, fwd_app_type_path_format, image_name);
    ...
    {
        int rv = _SHR_E_NONE;
        uint32 is_opcode_valid = FALSE;
        uint32 nof_contexts = 0;
        ...
        rv = dbal_enum_type_string_to_id_no_err(unit, DBAL_FIELD_TYPE_DEF_KBP_FORWARD_APP_TYPES, opcode_name,
                                                &opcode_id_enum);
        if (rv == _SHR_E_NOT_FOUND)
        {
        ...
        SHR_IF_ERR_EXIT(rv);
        SHR_IF_ERR_EXIT(dbal_fields_enum_hw_value_get
                        (unit, DBAL_FIELD_KBP_FORWARD_APP_TYPES, opcode_id_enum, &opcode_id));

        LOG_VERBOSE(BSL_LOG_MODULE,
                    (BSL_META_U(unit, "Configuring opcode %s (%d) for ACL only\n"), opcode_name, opcode_id));

        SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_KBP_OPCODE_INFO, entry_handle_id));
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_OPCODE_ID, opcode_id);
        dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_VALID, INST_SINGLE, &is_opcode_valid);
        SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));

        if (is_opcode_valid)
        {
            /** Opcode is already configured for forwarding */
            LOG_VERBOSE(BSL_LOG_MODULE, (BSL_META_U(unit, "Opcode %s (%d) already configured for forwarding\n"),
                                         opcode_name, opcode_id));
            continue;
        }
        ...
        SHR_IF_ERR_EXIT(kbp_mngr_opcode_to_context_read
                        (unit, opcode_node, opcode_id, TRUE, is_standard_image, FALSE, &nof_contexts));
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(kbp_mngr_acl_opcode_info_init(unit));
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_EGW_BACK_PRESSURE, &entry_handle_id));
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_EN_BACK_PRESSURE, INST_SINGLE, 1);
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    ...
    int unit)
{
    ...
    smt_id_start = 0;
    /*
     * In case of single port connect mode, only thread 0 instructions need to be created,
     * regardless whether SMT is enabled or not.
     */
    smt_id_end = ((!DNX_KBP_OP2_IS_SINGLE_PORT_MODE(unit) && (Kbp_app_data[unit]->device_type == KBP_DEVICE_OP2))
                  ? DNX_KBP_MAX_NOF_SMTS : 1);
    ...
    int unit)
{
    ...
    if (dnx_kbp_device_enabled(unit))
    {
        if (!sw_state_is_warm_boot(unit))
        {
            ...
            if (dnx_data_elk.general.feature_get(unit, dnx_data_elk_general_back_pressure_bit_exists))
            {
                SHR_IF_ERR_EXIT(kbp_mngr_egw_back_pressure(unit));
            }
...
shr_error_e kbp_mngr_init_internal(int unit){DNX_KBP_DUMMY_PROTOTYPE_ERR_MSG("kbp_mngr_init_internal")}
/* *INDENT-ON* */
/* } */
