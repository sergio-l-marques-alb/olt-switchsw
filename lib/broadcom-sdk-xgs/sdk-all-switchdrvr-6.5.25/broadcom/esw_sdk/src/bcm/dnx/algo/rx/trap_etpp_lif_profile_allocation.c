    ...
    uint8 *first_reference)
{
    ...
    {
        *profile = DNX_RX_TRAP_ETPP_LIF_PROFILE_DEFAULT;
    ...
    if ((allocation_needed == TRUE) && (*action_gport_p != BCM_GPORT_INVALID))
    {
        if (BCM_GPORT_IS_TRAP(*action_gport_p))
        {
            ...
            for (tmp_profile = DNX_RX_TRAP_ETPP_LIF_PROFILE_STACK_TRAP_1;
                 tmp_profile <= DNX_RX_TRAP_ETPP_LIF_PROFILE_STACK_TRAP_2; tmp_profile++)
            {
            ...
            if (*profile == DNX_RX_TRAP_ETPP_LIF_PROFILE_DEFAULT)
            {
        ...
        else
        {
            ...
            tmp_profile = DNX_RX_TRAP_ETPP_LIF_PROFILE_STACK_MIRROR;
            SHR_IF_ERR_EXIT(multi_set_get_by_index
