    ...
    bcm_port_t logical_port)
{
    ...
    if (!(soc_sand_is_emulation_system(unit) && DNX_FABRIC_FMAC_EMULATION_NOT_SUPPORTED(unit)))
    {
         ...
         */
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_FABRIC_IF_LEAKY_BUCKET, &entry_handle_id));
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_FABRIC_LINK, link);
         ...
         * Configure nof pipes
         */
        if (!dnx_data_fabric.pipes.feature_get(unit, dnx_data_fabric_pipes_global_config_supported))
        {
            nof_pipes = dnx_data_fabric.pipes.nof_pipes_get(unit);
            SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_FABRIC_IF_PIPES, entry_handle_id));
            dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_FABRIC_LINK, link);
        ...
        if (dnx_data_fabric.cell.pcp_enable_get(unit) == 1
            && !dnx_data_fabric.cell.feature_get(unit, dnx_data_fabric_cell_format_always_with_pcp))
        {
            /*
             * Enable PCP
             */
    ...
    bcm_port_t logical_port)
{
    ...
    SHR_IF_ERR_EXIT(dnx_generic_state_journal_entry_new
                    (unit, (uint8 *) &is_add, sizeof(int),
                     ...
                     &dnx_fabric_port_error_recovery_probe_detach_rollback, FALSE));
exit:
    SHR_FUNC_EXIT;
}
/**
    ...
    bcm_pbmp_t * pbmp)
{
    ...
    SHR_IF_ERR_EXIT(dnx_generic_state_journal_entry_new
                    (unit, (uint8 *) journal_entry, sizeof(dnx_fabric_port_er_counter_journal_t),
                     (uint8 *) pbmp, sizeof(bcm_pbmp_t), &dnx_fabric_port_er_counter_rollback, FALSE));
    ...
    int linkscan_paused)
{
    ...
    SHR_IF_ERR_EXIT(dnx_generic_state_journal_entry_new(unit, (uint8 *) &linkscan_paused, sizeof(int),
                                                        ...
                                                        &dnx_fabric_port_er_linkscan_pause_continue_rollback, FALSE));
exit:
    SHR_FUNC_EXIT;
}
/**
    ...
    int need_err_recovery)
{
    ...
    SHR_IF_ERR_CONT(imb_port_add(unit, logical_port, flags));
    ...
    bcm_port_t logical_port)
{
    ...
    DNX_ERR_RECOVERY_SUPPRESS(unit);
    SHR_IF_ERR_CONT(imb_port_remove(unit, logical_port));
    DNX_ERR_RECOVERY_UNSUPPRESS(unit);
    if (SHR_FUNC_ERR())
    {
        SHR_EXIT();
    ...
    bcm_pbmp_t * pbmp)
{
    ...
    {
        bcm_pbmp_t *counter_pbmp_p;
        SHR_IF_ERR_CONT(soc_control_element_address_get(unit, COUNTER_PBMP, (void **) &counter_pbmp_p));
        if (SHR_FUNC_ERR())
        {
            DNX_ERR_RECOVERY_UNSUPPRESS(unit);
            SHR_EXIT();
    ...
    int unit)
{
    ...
    {
        DNX_ERR_RECOVERY_SUPPRESS(unit);
        SHR_IF_ERR_CONT(soc_counter_start(unit, counter_flags, counter_interval, counter_pbmp));
        DNX_ERR_RECOVERY_UNSUPPRESS(unit);
        if (SHR_FUNC_ERR())
        {
            SHR_EXIT();
    ...
    bcm_pbmp_t * okay_pbmp)
{
    ...
    if (broadcast_load_fabric && !BCM_PBMP_IS_NULL(pbmp))
    {
         ...
         */
        SHR_IF_ERR_CONT(soc_dnxc_fabric_broadcast_firmware_loader(unit, &pbmp));
        if (SHR_FUNC_ERR())
        {
            DNX_ERR_RECOVERY_UNSUPPRESS(unit);
            SHR_EXIT();
    ...
    if (!is_init_sequence)
    {
         ...
         */
        {
            ...
            BCM_PBMP_ITER(*okay_pbmp, logical_port)
            {
                SHR_IF_ERR_CONT(_bcm_linkscan_port_init(unit, logical_port));
                if (SHR_FUNC_ERR())
                {
                    DNX_ERR_RECOVERY_UNSUPPRESS(unit);
                    SHR_EXIT();
        ...
        BCM_PBMP_ITER(*okay_pbmp, logical_port)
        {
            ...
            {
                SHR_IF_ERR_EXIT(bcm_dnx_fabric_link_control_set
                                (unit, logical_port, bcmFabricLLFControlType,
                                 dnx_data_fabric.cgm.llfc_type_default_get(unit)));
            }
            else
            {
                SHR_IF_ERR_EXIT(bcm_dnx_fabric_link_control_set(unit, logical_port, bcmFabricLLFControlSource, 1));
            }
        }
    }
...
/** See .h file */
static shr_error_e
dnx_force_traffic_fabric_access(
    int unit,
    int enable)
{
    uint32 entry_handle_id;
    ...
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_FABRIC_IF_FORCE_TRAFFIC_FABRIC, &entry_handle_id));
    ...
    int enable)
{
    ...
    if (dnx_data_fabric.general.connect_mode_get(unit) != DNX_FABRIC_CONNECT_MODE_FE)
    {
        SHR_ERR_EXIT(_SHR_E_UNAVAIL, "fabric force fabric is only supported in FE mode");
    }

    SHR_IF_ERR_EXIT(dnx_force_traffic_fabric_access(unit, enable));
...
shr_error_e
dnx_force_traffic_local_access(
    int unit,
    int core)
{
    uint32 entry_handle_id;
    ...
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_FABRIC_IF_FORCE_TRAFFIC_LOCAL, &entry_handle_id));
    for (core_idx = 0; core_idx < dnx_data_device.general.nof_cores_get(unit); core_idx++)
    {
        /*
         * core == DNX_FABRIC_FORCE_CORE_ALL_CORES means disable ALL cores, valid core index means enable the selected core
         */
        enable = (core_idx == core ? TRUE : FALSE);
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE, core_idx);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ENABLE, INST_SINGLE, enable);
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    }
    ...
    int core)
{
    ...
    if (dnx_data_fabric.general.connect_mode_get(unit) != DNX_FABRIC_CONNECT_MODE_FE)
    {
        SHR_ERR_EXIT(_SHR_E_UNAVAIL, "fabric force local is only supported in FE mode");
    }

    DNXCMN_CORE_VALIDATE(unit, core, TRUE);

    SHR_IF_ERR_EXIT(dnx_force_traffic_local_access(unit, DNX_FABRIC_FORCE_CORE_ALL_CORES));
    ...
    int unit)
{
    ...
    if (dnx_data_tdm.params.feature_get(unit, dnx_data_tdm_params_is_supported))
    {
        ...
        {
            /*
             * Use the access functions in order not to trigger the error checking in SINGLE_FAP mode
             */
            SHR_IF_ERR_EXIT(dnx_force_traffic_local_access(unit, DNX_FABRIC_FORCE_CORE_ALL_CORES));
            SHR_IF_ERR_EXIT(dnx_force_traffic_fabric_access(unit, FALSE));
        }
    }
exit:
    SHR_FUNC_EXIT;
    ...
    uint32 *errored_token_count)
{
    ...
    *link_status = 0;
    ...
    /*
     * Get remote and local fault
     */
    if (dnx_data_fabric.links.feature_get(unit, dnx_data_fabric_links_link_local_remote_fault_support))
    {
        SHR_IF_ERR_EXIT(bcm_dnx_port_fault_get(unit, logical_port, &fault_flags));
        if (fault_flags & BCM_PORT_FAULT_LOCAL)
        {
            *link_status |= BCM_FABRIC_LINK_STATUS_LOCAL_FAULT;
        }
        if (fault_flags & BCM_PORT_FAULT_REMOTE)
        {
            *link_status |= BCM_FABRIC_LINK_STATUS_REMOTE_FAULT;
        }
    }
    /*
    ...
    uint32 *errored_token_count)
{
    ...
    /** get logical port from GPORT */
    SHR_IF_ERR_EXIT(dnx_algo_gpm_gport_phy_info_get
                    (unit, link_id, DNX_ALGO_GPM_GPORT_TO_PHY_OP_LOCAL_IS_MANDATORY, &gport_info));

    SHR_INVOKE_VERIFY_DNXC(dnx_fabric_if_link_status_verify
                           (unit, gport_info.local_port, link_status, errored_token_count));
     ...
     */
    SHR_IF_ERR_EXIT(dnx_algo_port_fabric_link_get(unit, gport_info.local_port, &link_idx));
 ...
 */
shr_error_e
dnx_fabric_if_fmac_symb_error_get(
    int unit,
    bcm_port_t port,
    uint32 *val)
{
    uint32 entry_handle_id;
    int link_id, error_indication;
    uint32 properties = 0;
     ...
     */
    /** alloc DBAL table handle */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_FABRIC_SYMB_ERRORS_PER_FRAME_COUNTERS, &entry_handle_id));
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_FABRIC_LINK, link_id);
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_FMAC_SYMB_ERRORS_PER_FRAME, INST_SINGLE, val);
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));
    /*
     * Since 15T and 5T FEC uncorrectable error indication bits are different in DBAL,
     * need to convert them to the same bit in SW state
     * For 5T FEC uncorrectable error indication, it is in BIT5,
     */
    SHR_IF_ERR_EXIT(portmod_port_encoding_get(unit, port, &properties, &encoding));
    if (encoding == PORTMOD_PCS_64B66B_RS_FEC || encoding == PORTMOD_PCS_64B66B_LOW_LATENCY_RS_FEC)
    {
        error_indication = SHR_BITGET(val, DNX_5T_FEC_UNCORRECTABLE_ERROR_INDICATION_BIT_IN_DBAL);
        if (error_indication)
        {
            SHR_BITCLR(val, DNX_5T_FEC_UNCORRECTABLE_ERROR_INDICATION_BIT_IN_DBAL);
            SHR_BITSET(val, DNX_15T_FEC_UNCORRECTABLE_ERROR_INDICATION_BIT_IN_DBAL);
        }
    }
