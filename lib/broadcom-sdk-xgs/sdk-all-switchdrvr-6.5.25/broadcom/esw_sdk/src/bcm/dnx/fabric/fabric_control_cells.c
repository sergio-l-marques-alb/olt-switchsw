 ...
 */
static shr_error_e
dnx_fabric_ctrl_cells_mesh_topolgy_sync_polling(
    int unit,
    uint32 iter_index,
    int *success)
{
    int stand_alone = 0;
    uint32 entry_handle_id;
    soc_dnxc_fabric_mesh_topology_diag_t mesh_topology_status;
    int fast_sync_done = 0;
    int is_synced = 0;
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);
    ...
    if (iter_index % 2 != 0)
    {
        SHR_IF_ERR_EXIT(dnx_fabric_mesh_topology_get(unit, &mesh_topology_status));
        fast_sync_done = mesh_topology_status.indication_3;
        is_synced = mesh_topology_status.indication_4;

        if (fast_sync_done && !is_synced)
        {
            SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_FABRIC_MESH_TOPOLOGY, &entry_handle_id));
    ...
    int unit)
{
    ...
    {
        /*
         * This polling reads control cells counter. If no cells are seen at the end of polling,
         * the device will stay in stand alone mode.
         */
 ...
 */
shr_error_e
dnx_fabric_ctrl_cells_mesh_topology_sync(
    int unit)
{
    dnx_fabric_ctrl_cells_polling_t data;
    SHR_FUNC_INIT_VARS(unit);
    ...
    /*
     * Polling to see whether control cells init is done and mesh topology is synced.
     */
    sal_memset(&data, 0, sizeof(data));
    data.polling_name = "whether control cells init is done";
    data.polling_func = dnx_fabric_ctrl_cells_mesh_topolgy_sync_polling;
    ...
    uint32 *period)
{
    ...
    COMPILER_64_SET(time.time, 0, dnx_data_fabric.reachability.full_cycle_period_nsec_get(unit));
    SHR_IF_ERR_EXIT(dnxc_time_time_to_clock_cycles_get(unit, &time, &nof_clock_cycles));
    ...
    uint32 *period)
{
    ...
    time.time_units = DNXC_TIME_UNIT_USEC;
 ...
 */
shr_error_e
dnx_fabric_delay_test_ctrl_cells_enable_set(
    int unit,
    int is_soft_reset,
    int enable_fabric_traffic)
{
    ...
    if (enable_fabric_traffic)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_fabric_ctrl_cells_mesh_topology_sync(unit));

        /*
         * Fabric traffic enabling for cells that are preconfigured (not manadatory all to be enabled)
         */
        ...
        if (dnx_data_fabric.links.nof_links_get(unit) != 0)
        {
            ...
            dbal_entry_key_field32_range_set(unit, entry_handle_id, DBAL_FIELD_PIPE_ID, DBAL_RANGE_ALL, DBAL_RANGE_ALL);
    ...
    int is_soft_reset)
{
    ...
    SHR_IF_ERR_EXIT(dnx_fabric_delay_test_ctrl_cells_enable_set(unit, is_soft_reset, 0));
    ...
    /** Always set MESH TOPOLOGY sync to fast in order to reduce sync time. This will be done even after cold boot. */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_FABRIC_MESH_TOPOLOGY, entry_handle_id));
    ...
    /** Put MESH TOPOLOGY sync to normal after sync is finished. */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_FABRIC_MESH_TOPOLOGY, entry_handle_id));
    ...
    int *links_count)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_fabric_reachability_status_get_verify
    ...
    int *link_partner_count)
{
    ...
    SHR_INVOKE_VERIFY_DNXC(dnx_fabric_link_connectivity_status_get_verify
    ...
    bcm_fabric_link_connectivity_t * link_partner_info)
{
     ...
     */
    SHR_IF_ERR_EXIT(dnx_port_logical_verify(unit, link_id));
    SHR_IF_ERR_EXIT(dnx_port_logical_get(unit, link_id, &logical_port));

    SHR_INVOKE_VERIFY_DNXC(dnx_fabric_link_connectivity_status_single_get_verify
                           (unit, logical_port, link_partner_info));
     ...
     */
    SHR_IF_ERR_EXIT(dnx_algo_port_fabric_link_get(unit, logical_port, &link_index));
