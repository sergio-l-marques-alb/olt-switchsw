...
{
    int ret;
    ret = (dnx_drv_soc_flags_get(unit) & dnx_drv_soc_f_all_modules_inited())? TRUE : FALSE;
    return (ret);
}
    ...
    uint32 flags)
{
    ...
    {
        /** Gracefully destroy the temp monitor thread */
        SHR_IF_ERR_EXIT(dnx_hbmc_temp_monitor_deinit(unit));
    }
