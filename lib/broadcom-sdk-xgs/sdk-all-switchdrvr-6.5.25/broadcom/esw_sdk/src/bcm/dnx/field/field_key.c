...
/**
 * Maximum numebr to contain the full DBAL FIELD_KEY ENUM.
 */
...
*  Only done for one FFC at the time.
* \param [in] unit            - Device Id
* \param [in] cascaded_from_context_id - Context ID of iPMF1 to configure the key for
* \param [in] dnx_qual        - Encoded qualifier - This is the qualifier that should be allocated
*                               as initial qual in ipmf1
* \param [in] ffc_index      - FFC index within a qualifer
...
*   on this array is assumed to be DNX_FIELD_KEY_MAX_NOF_FFC_IN_QUAL.
* \return
*   shr_error_e - Error Type
* \remark
*   * None
    ...
    int app_db_id)
{
    ...
    {
        /*
         * No two keys are allowed to use the same app_db_id,
         * paired ID supplied by the TCAM access profile to determine the app_db_id to use for the
         * second key.
         */
...
* \param [in] qual_attach_info_p - info from attach info.
* \param [in] nof_sub_quals_p  - Number of valid elements in dnx_qual_info.
* \param [in] dnx_qual_info    - Full output information for this qualifier.
*                                Array of size DNX_FIELD_KEY_MAX_SUB_QUAL for when we split into sub qualifiers.
* \return
*   shr_error_e               - Error Type
* \remark
*   * None.
    ...
    dnx_field_key_attached_qual_info_t dnx_qual_info[DNX_FIELD_KEY_MAX_SUB_QUAL])
{
    ...
    /*
     * For external input type, verify stage. Also verify we only read from the range of KBP payloads.
     */
    ...
    {
        /*
         * The breaking to multiple parts for user defined qualifier will take place according to attach info.
         */
        SHR_ERR_EXIT(_SHR_E_PARAM,
    ...
    dnx_field_key_attached_qual_info_t dnx_qual_info[DNX_FIELD_KEY_MAX_SUB_QUAL])
{
    ...
    /*
     * Account for based upon qualifiers.
     */
    if (qual_attach_info_p->base_qual != DNX_FIELD_QUAL_TYPE_INVALID)
    {
        ...
        /** Get the values from the base_qual */
        SHR_IF_ERR_EXIT(dnx_field_map_dnx_qual_offset
        ...
        /** If the starting point is beyond the last part, make it from the last part.*/
        if (base_qual_sub_ndx == base_qual_nof_mappings_size)
        {
            base_qual_sub_ndx--;
            remaining_offset += base_qual_sizes[base_qual_sub_ndx];
        ...
        {
            /** Set the offset and size of the first sub qual.*/
            mapped_offsets[0] = remaining_offset + base_qual_mapped_offsets[base_qual_sub_ndx];
            ...
            {
                base_qual_sub_ndx++;
                /** If last sub qual of the base qualifiers, just extend existing sub_qual.*/
                if (base_qual_sub_ndx == base_qual_nof_mappings_size)
                {
    ...
    for (sub_ndx = 0; sub_ndx < nof_mappings; sub_ndx++)
    {
        ...
        {
            /*
             * For qualifiers that are based on other qualifiers, the attach info offset was already used above.
             */
        ...
        switch (qual_attach_info_p->input_type)
        {
            ...
            {
                /*
                 * We cascade from external stage.
                 */
    ...
    uint8 *is_up_to_one_ffc_p)
{
    ...
    /** offset_on_key is not relevant, set to 0. */
    offset_on_key = 0;
...
* \param [in] context_id    - Context ID to configure FFC's for
* \param [in] key_id        - key id
* \param [in] offset_on_key - offset on key.
 ...
 * \param [in] context_id    - Context ID to configure FFC's for
 * \param [in] ffc_index      - FFC index within a qualifer
 * \param [in] dnx_qual_info_p   - Information about the qualifier.
...
/**
* \brief
*  Delete the KBR configuration per context ID and key ID
* \param [in] unit          - Device Id
* \param [in] context_id    - PMF context to clear KBR for
* \param [in] field_stage   - Stage to clear KBR for
* \param [in] key_id        - Key Id to clear KBR for
* * \return
*   shr_error_e             - Error Type
* \remark
*   * None
...
*  Only done for one FFC at the time.
* \param [in] unit          - Device Id
* \param [in] cascaded_from_context_id - Context ID in iPMF1 to configure FFCs for
* \param [in] ffc_index      - FFC index within a qualifer
* \param [in] qualifier_ffc_info_p   - info structure for the current qualifier
    ...
    dnx_field_key_ffc_sharing_info_t context_ffc_sharing_info[DNX_FIELD_KEY_MAX_NOF_FIELD_KEY_ID])
{
    ...
    /** Get the compare starting offsets.*/
    ...
    int *shared_p)
{
    ...
    *shared_p = FALSE;
    ...
    /*
     * Go through all keys in iPMF2. Validates that OTHER FG's do not use this ffc
     */
...
/**
* \brief
*  This function determines if the initial FFC for the given IPMF2 qualifier
...
* \param [in] unit          - Device ID
* \param [in] field_stage   - For which Field stage to Configure HW
* \param [in] cascaded_from_context_id - Context ID for iPMF1, the context of the initial FFCs.
    ...
    int *shared_p)
{
    ...
    *shared_p = FALSE;
    ...
    /*
     * Calculate the ffc instructions, considering initial ffc
     * Note that the offset_on_key in dnx_qual_info_p is irrelevant.
     */
    SHR_IF_ERR_EXIT(dnx_field_key_calc_ffc_instruction_values
...
* \param [in] unit             - unit
* \param [in] field_stage      - The stage where the lookup takes place.
...
* \param [out] is_found_p      - If true lookup type was found.
* \param [out] pmf_lookup_type_p - The returned lookup interface type for the given (fg_type/key_id) pair.
*                                  Relevant only if is_found_p is TRUE.
* \return
*   shr_error_e             - Error Type
* \remark
*   * None
    ...
    dbal_enum_value_field_pmf_lookup_type_e * pmf_lookup_type_p)
{
    ...
    switch (fg_type)
    {
        ...
        {
            /*
             * For Direct Extraction (DE) and Hash Stamp, no lookup interface is defined,
             */
            *is_found_p = FALSE;
            break;
        }
...
* \see
*   * dnx_field_key_lookup_change_state_hw_epmf_external_tcam_set
*/
static shr_error_e
...
* \param [in] unit                    - Device ID
* \param [in,out] nof_mapping_p       - Qualifier info
* \param [in,out] dnx_qual_info       - Qualifier info
* \param [out] second_nof_mapping_p   - Number of sub qualifiers in the second key
* \param [out] second_dnx_qual_info   - Qualifier info regarding the second key
    ...
    uint8 *was_split_p)
{
    ...
    if (dnx_qual_info[0].offset_on_key >= max_single_key_size)
    {
        ...
        /** Sanity check: Verify the qualifier fits in the second key.*/
    ...
    for (sub_ndx_first = 0; sub_ndx_first < *nof_mapping_p; sub_ndx_first++)
    {
        ...
        {
            /*
             * If the sub_qualifiers is in the second key.
             */
        ...
        else if (dnx_qual_info[sub_ndx_first].offset_on_key + dnx_qual_info[sub_ndx_first].size > max_single_key_size)
        {
            ...
            /*
             * Update the second (split) qualifier info to fit the second key - to be the rest of the bits
             */
            ...
            /*
             * Update the original info to fit the first key - to be the residue of the key
             */
            ...
            /*
             * If the qualifier is a const qualifier, split the input argument.
             */
            if (dnx_qual_info[sub_ndx_first].input_type == DNX_FIELD_INPUT_TYPE_CONST)
            {
                ...
                /*
                 * Provide the LSB part of the input argument to the qualifier in the first key
                 * and the MSB part to the qualifier in the second key.
                 */
        ...
        {
            /*
             * If the sub_qualifiers is in the first part.
             */
            /** Sanity check. We assume the sub qualifier are ordered by offset_on_key. */
            if (*was_split_p)
            {
    ...
    /** Initialize the first array after it ends, even though it is not needed.*/
 ...
 * \param [in] key_id_p       - Structure that holds an array of Key Ids, and the starting offsets on the keys.
 * \return
 *   shr_error_e - Error Type
 * \remark
 *   * None
...
/**
 * \brief
 *  General Key handling for a field group with APP_DB_ID:
    ...
    dnx_field_key_id_t *key_id_p)
{
    ...
    {
        /*
         * When key B on ePMF is attached, first clear KBR
         */
...
/**
  ...
  *  It also adds such a key for a double TCAM with FFC on only one key.
  * \param [in] unit               - Device ID
    ...
    dnx_field_key_id_t *key_id_p)
{
    ...
    /*
     * Verify that an ePMF lookup has at least one FFC, if there is a lookup and no lookup enabler.
     */
        ...
        if (has_lookup_enabler == FALSE)
        {
            ...
            if (one_key_has_no_ffc)
            {
                ...
                else if ((nof_keys_on_fg == 2) && (key_in_info_p->fg_type == DNX_FIELD_GROUP_TYPE_TCAM))
                {
                    ...
                    {
                        /*
                         * Check if the first key has an FFC.
                         */
                        if ((key_id_p->id[0] != DNX_FIELD_KEY_ID_INVALID) && (ffc_id[0][0] != DNX_FIELD_FFC_ID_INVALID))
                        {
                            ...
                            int ffc_found = FALSE;
                            /** Note we expect the FG to have the maximum prefix size since it is on only one key. */
                            ...
                            /*
                             * Find an FFC to copy. Must be on the first key, and must not override the prefix,
                             * or any qualifier on the second key (const 0 qualifer, as any other would have an FFF).
                             */
...
/**
  * \brief
  *  In ePMF, every key must use at list one FFC, so we may add to the KBR of one key an FFC from the other.
  *  Remove that FFC.
  * \param [in] unit               - Device ID
  * \param [in] field_stage        - The PMF stage the field group is in.
  * \param [in] context_id         - Context ID ID to perform the key attach for
  * \param [in] fg_type            - The field group type.
  * \param [in] key_id_p           - The keys allocated to the field group.
  * \param [in] nof_ffcs           - Number of valid elements in ffc_ids.
  * \param [in] ffcs               - Array with DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_FFC elements of non-initial FFCs
  *                                   That were deleted.
  * \return
  *   shr_error_e - Error Type
  * \remark
            ...
            for (key_ndx = 0; key_ndx < DNX_DATA_MAX_FIELD_GROUP_NOF_KEYS_PER_FG_MAX; key_ndx++)
            {
                if (key_id_p->id[key_ndx] != DNX_FIELD_KEY_ID_INVALID)
                {
                    ...
                    if (ffc_id[0] != DNX_FIELD_FFC_ID_INVALID)
                    {
                        ...
                        /*
                         * Verify that the extra FFC is amongst the ones that were deleted.
                         */
                        ...
                        {
                            /*
                             * Sanity check: Verify that we did not find the FFC on the key it
                             *               was supposed to have been deleted on.
                             */
    ...
    dbal_enum_value_field_field_key_e key_id)
{
    ...
    /*
     * Set bit on KBR of key B using FFC0 and APP_DB_ID 63 
     */
 ...
 *
 * \param [in] unit          - Device Id
 * \param [in] dnx_qual_info_p - Information about the qualifier, to include qual_tpye, size and offset on key.
    ...
    dnx_field_app_db_id_t * app_db_id_p)
{
         ...
         (key_template_p->key_qual_map[qual_idx].qual_type != DNX_FIELD_QUAL_TYPE_INVALID); qual_idx++)
    {
        ...
        if (field_stage == DNX_FIELD_STAGE_IPMF2)
        {
            /** If the qualifier doesn't use native pbus - go through its FFCs*/
            if (DNX_QUAL_CLASS(key_template_p->key_qual_map[qual_idx].qual_type) != DNX_FIELD_QUAL_CLASS_META2)
            {
                /*
                 * Go through the list of FFCs for the given qualifier and find the related FFC in
                 * the cascaded_from context.
                 */
                for (ffc_idx = 0; (ffc_idx < DNX_FIELD_KEY_MAX_NOF_FFC_IN_QUAL) &&
...
/**
* \brief
*  Delete the FFC configuration per context ID with given Keys. Includes changing the KBR.
* \param [in] unit                  - Device Id
* \param [in] field_stage           - For which Field stage to Configure HW
* \param [in] context_id            - PMF context to configure FFC's for
* \param [in] nof_ffcs              - Number of valid elements in ffcs.
* \param [in] ffcs                  - Array with DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_FFC elements of non-initial FFCs.
* \param [in] nof_initial_ffcs      - Number of valid elements in initial_ffcs.
* \param [in] initial_ffcs          - Array with DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_FFC elements of initial FFCs.
* * \return
*   shr_error_e             - Error Type
* \remark
*   * None
    ...
    dnx_field_key_initial_ffc_info_t initial_ffcs[DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_FFC])
{
    ...
    /** Deallocated initial FFCs. Can be done for iPMF2 only.*/
    {
        for (ffc_idx = 0; ffc_idx < nof_initial_ffcs; ffc_idx++)
        {
            ...
            /*
             * Clean the initial FFCs ONLY if not used by another fg
             */
            if (!is_shared)
            {
                /*
                 * Go through the list of FFCs for the given qualifier and deallocate the valid initial FFCs
                 */
                /** Remove FFC from KBR. */
                SHR_IF_ERR_EXIT(dnx_field_key_kbr_single_ffc_hw_remove
                                (unit, DNX_FIELD_STAGE_IPMF1, cascaded_from_context_id,
...
/**
 * \brief
 *  Takes the per qualifier information in the group FFC info and converts it into two lists of FFCs,
 *  non-initial and initial.
 *  Do not write duplicate ffcs.
 * \param [in] unit              - Device Id
 * \param [in] field_stage       - Field stage.
 * \param [in] group_ffc_info_p  - FFC info of the field group.
 * \param [out] nof_ffcs_p       - Number of valid elements in ffc_ids.
 * \param [out] ffcs             - Array with DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_FFC elements of non-initial FFCs.
 * \param [out] nof_initial_ffcs_p - Number of valid elements in initial_ffc_ids.
 * \param [out] initial_ffcs     - Array with DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_FFC elements of initial FFCs.
 * \return
 *   shr_error_e - Error Type
 * \remark
 *   * None
    ...
    for (qual_idx = 0; qual_idx < DNX_DATA_MAX_FIELD_GROUP_NOF_QUALS_PER_FG; qual_idx++)
    {
             ...
             ffc_in_group_info_idx++)
        {
            ...
            if (ffc_p->ffc_id != DNX_FIELD_FFC_ID_INVALID)
            {
                ...
                /*
                 * Make sure we do not write the same FFC twice.
                 * For non-initial FFCs we have no sharing and we do not expects them to be reused.
                 */
                ...
                {
                    /** Fail instead of continue, as we do not expect non initial FFCs to be reused.*/
 ...
 * \param [in] fg_type     - The type of the field group, if applicabe. If not a field group, use DNX_FIELD_GROUP_TYPE_INVALID.
 * \param [in] key_template_p - key information.
    ...
    dnx_field_key_id_t *key_id_p)
{
    ...
    /** Remove from the KBR and deallocate the FFCs that are listed in the group_ffc_info.*/
    SHR_IF_ERR_EXIT(dnx_field_key_ffc_kbr_deallocate(unit, field_stage, context_id,
                                                     nof_ffcs, ffcs, nof_initial_ffcs, initial_ffcs));

#if defined(BCM_DNX2_SUPPORT)
    /** Handle ePMF without a lookup enabler.*/
 ...
 * \param [in] field_stage - The stage of the context.
 * \param [in] context_id  - Context ID to detach
 * \param [in] fg_type    - The type of the field group. If TCAM direct table, expects to take TCAM as type.
 * \param [in] key_template_p - Key information.
 * \param [in] key_length_type - An enum indicating the size of the TCAM key.
 * \param [in] key_id_p    -
 *   Structure that holds the array of Key Id to deallocate
 *   Sub param of the key_id_p - bit_range     -
 *   Structure that holds all information regarding the bit-range allocated on key
    ...
    dnx_field_key_id_t *key_id_p)
{
    ...
    {
        /*
         * When key B on ePMF is detached, set a bit on its KBR for diagnostics
         */
        if (field_stage == DNX_FIELD_STAGE_EPMF
            && (key_id_p->id[0] == DBAL_ENUM_FVAL_FIELD_KEY_B || key_id_p->id[1] == DBAL_ENUM_FVAL_FIELD_KEY_B))
        {
 ...
 * \param [in] key_length_type - An enum indicating the size of the EXEM key.
 * \param [in] key_id_p    -
 *   Structure that holds the array of Key Id to deallocate
 *   Sub param of the key_id_p - bit_range     -
 *   Structure that holds all information regarding the bit-range allocated on key
    ...
    int unit)
{
    ...
    /*
     * Verify that the invalid value is illegal.
     */
    ...
    /*
     * Verify that all values are under the MAX_NOF.
     * Also verify that the values are continuous.
     */
    ...
    /*
     * Verify that the number of keys fits the number of keys in iPMF1/2.
     */
    if (nof_keys_found !=
