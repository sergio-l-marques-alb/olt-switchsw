/*
 * $Id: compat_530.c,v 1.8 Broadcom SDK $
    ...
    bcm_compat6520_tunnel_terminator_t *to)
{
    ...
    to->egress_if  = 0;
    to->failover_id = 0;
    to->failover_mc_group = 0;
    to->failover_tunnel_id = 0;
    to->fwd_vlan = 0;
    to->if_class = 0;
    to->ingress_failover_id = 0;
    bcm_qos_ingress_model_t_init(&to->ingress_qos_model);
    to->inlif_counting_profile = 0;
    to->multicast_flag = 0;
    to->priority = 0;
    to->qos_map_id = 0;
    to->remote_port = 0;
    to->sci = 0;
    to->subport_pbmp_profile_id = 0;
    to->tunnel_class = 0;
    to->tunnel_id = 0;
    to->tunnel_if = 0;
    to->tunnel_term_ecn_map_id = 0;
    for (ipv6_index = 0; ipv6_index < 16; ipv6_index++) {
        to->dip6[ipv6_index] = 0;
        to->dip6_mask[ipv6_index] = 0;
        to->sip6[ipv6_index] = 0;
        to->sip6_mask[ipv6_index] = 0;
    }
}
...
{
    int		rv;
    bcm_compat6520_tunnel_terminator_t	ninfo;

    if (info == NULL) {
        return BCM_E_PARAM;
    }
    _bcm_compat530in_l3_tunnel_terminator_t(info, &ninfo);
    rv = bcm_compat6520_tunnel_terminator_add(unit, &ninfo);
    return rv;
}
    ...
    bcm_compat530_l3_tunnel_terminator_t *info)
{
    ...
    _bcm_compat530in_l3_tunnel_terminator_t(info, &ninfo);
    rv = bcm_compat6520_tunnel_terminator_delete(unit, &ninfo);
    return rv;
}
    ...
    bcm_compat530_l3_tunnel_terminator_t *info)
{
    ...
    _bcm_compat530in_l3_tunnel_terminator_t(info, &ninfo);
    rv = bcm_compat6520_tunnel_terminator_update(unit, &ninfo);
    return rv;
}
    ...
    bcm_compat530_l3_tunnel_terminator_t *info)
{
    ...
    _bcm_compat530in_l3_tunnel_terminator_t(info, &ninfo);
    rv = bcm_compat6520_tunnel_terminator_get(unit, &ninfo);
    if (rv >= 0) {
