...
bcm_petra_knet_filter_create(int unit, bcm_knet_filter_t *filter)
{
    ...
    if (SOC_IS_JERICHO(unit)) {
        if (filter->match_flags & BCM_KNET_FILTER_M_MIRRORED) {
            if (filter->m_mirrored != 1 && filter->m_mirrored != 0) {
                return BCM_E_PARAM;
            }
        }
    }
    ...
    if (filter->match_flags == 0) {
        ...
        filter_create.filter.oob_data_size = sizeof(dpp_meta_data_t);
    }
...
bcm_petra_knet_filter_get(int unit, int filter_id, bcm_knet_filter_t *filter)
{
    ...
    if (filter_get.filter.oob_data_size)
    {
        ...
        if (SOC_IS_JERICHO(unit)) {
            ...
            {
                /** 2: Mirror to CPU */
                if (meta_data->action_type == 2)
                {
                    filter->match_flags |= BCM_KNET_FILTER_M_MIRRORED;
                    filter->m_mirrored = 1;
                }
            }
        }
