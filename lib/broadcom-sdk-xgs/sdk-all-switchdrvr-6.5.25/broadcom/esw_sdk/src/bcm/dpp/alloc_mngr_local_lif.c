...
_bcm_dpp_am_local_inlif_internal_alloc(int unit, uint8 tag, uint8 bank_id, int flags, int *local_lif){
    ...
    int elem, alloc_elem_id ,elem_in_bank, elem_to_alloc , nof_elem_to_alloc, offset_in_other_pool_bank;
    ...
    *local_lif = _BCM_DPP_AM_INGRESS_LIF_OBJECT_OFFSET_IN_BANK_TO_LOCAL_LIF_ID(elem_in_bank, bank_id, is_wide_lif);
    ...
    /*update corresponding LIF pool to occupy the elem in the other pool.*/
    rv = ALLOC_MNGR_LOCAL_LIF_ACCESS.inlif_info.bank_info.get(unit, bank_id, bank_info);
    BCMDNX_IF_ERR_EXIT(rv);
    if (is_wide_lif)
    {
        ...
        {
            /* Tanslate the elem_in_bank to an offset in the other pool bank.*/
            offset_in_other_pool_bank = (elem_in_bank /2  * 3);

            alloc_flags = BCM_DPP_AM_IN_LIF_FLAG_COMMON | BCM_DPP_AM_IN_LIF_FLAG_ALLOC_WITH_ID;
            elem_to_alloc = bank_id * _BCM_DPP_AM_INGRESS_LIF_NOF_COMMON_LIFS_IN_BANK + offset_in_other_pool_bank;
            ...
            if (nof_elem_to_alloc > _BCM_DPP_AM_INGRESS_LIF_NOF_COMMON_LIFS_IN_BANK - offset_in_other_pool_bank) {
                nof_elem_to_alloc = _BCM_DPP_AM_INGRESS_LIF_NOF_COMMON_LIFS_IN_BANK - offset_in_other_pool_bank;
            }
        ...
        {
            /* Tanslate the elem_in_bank to an offset in the other pool bank.*/
            offset_in_other_pool_bank = (elem_in_bank /3  * 2 );

            alloc_flags = BCM_DPP_AM_IN_LIF_FLAG_WIDE | BCM_DPP_AM_IN_LIF_FLAG_ALLOC_WITH_ID;
            elem_to_alloc = bank_id * _BCM_DPP_AM_INGRESS_LIF_NOF_WIDE_LIFS_IN_BANK + offset_in_other_pool_bank;
            ...
            if (nof_elem_to_alloc > _BCM_DPP_AM_INGRESS_LIF_NOF_WIDE_LIFS_IN_BANK - offset_in_other_pool_bank) {
                nof_elem_to_alloc = _BCM_DPP_AM_INGRESS_LIF_NOF_WIDE_LIFS_IN_BANK - offset_in_other_pool_bank;
            }
            for ( alloc_elem_id = elem_to_alloc ; alloc_elem_id < elem_to_alloc + nof_elem_to_alloc ; alloc_elem_id ++  ) 
            {
                rv = bcm_dpp_am_local_inlif_tag_get(unit,bank_id,alloc_flags,alloc_elem_id,&tag_id);
...
_bcm_dpp_am_local_inlif_is_pack_empty(int unit,uint8 bank_id, uint8 flags, int elem, uint8 *is_pack_empty)
{
    ...
    uint32 rv1, rv2;
    int elem_to_check, elem_in_bank, elem_p1_in_bank;
    uint8 is_wide_lif = ( flags & BCM_DPP_AM_IN_LIF_FLAG_WIDE ) ? TRUE : FALSE ; 
    BCMDNX_INIT_FUNC_DEFS;
    ...
    else 
    {
        if ( elem_in_bank == _BCM_DPP_AM_INGRESS_LIF_NOF_COMMON_LIFS_IN_BANK - 2 ) 
        {
            ...
            {
                elem_p1_in_bank = _BCM_DPP_AM_INGRESS_LIF_OBJECT_OFFSET_IN_BANK((elem+1), is_wide_lif);
                if ((elem_p1_in_bank / 3 * 2) >= _BCM_DPP_AM_INGRESS_LIF_NOF_WIDE_LIFS_IN_BANK)
                {
                    /* The partner elem is not in the range of the other pool, use current elem to release the six-pack.*/
                    *is_pack_empty = TRUE;
                }
            }
        }
        else if ( elem_in_bank == _BCM_DPP_AM_INGRESS_LIF_NOF_COMMON_LIFS_IN_BANK - 1 )
        {
            ...
            if ( rv1 != BCM_E_EXISTS )
            {
                if ((elem_in_bank / 3 * 2) >= _BCM_DPP_AM_INGRESS_LIF_NOF_WIDE_LIFS_IN_BANK)
                {
                    /* Current elem is out of the range of the other pool, not use it to release the six-pack.*/
                    *is_pack_empty = FALSE;
                }
                else
                {
    ...
    for (bank_id_iter = 0 ; ( bank_id_iter < _BCM_DPP_AM_EGRESS_LIF_NOF_TOP_BANKS(unit) ) && (*bank_found == FALSE )  ; bank_id_iter++) 
    {
        ...
        if (top_bank_info.extension_bank_owner == _BCM_DPP_AM_EEDB_BANK_UNASSIGNED)   /*This top bank can be used as an extension*/
        {
            ...
            *ext_bank_id = bank_id_iter;
            ...
            /* check if all the entries in the top bank are not in-use yet.*/
            rv = ALLOC_MNGR_LOCAL_LIF_ACCESS.eedb_info.banks.get(unit, eedb_half_bank, &eedb_bank_info);
            BCMDNX_IF_ERR_EXIT(rv);
            if (eedb_bank_info.valid) {
                ...
                {
                    rv = bcm_dpp_am_local_outlif_is_alloc(unit, eedb_half_bank, 0, alloc_elem_id);
                    if (rv == BCM_E_EXISTS) {
                        *bank_found = FALSE;
                        break;
            ...
            if (*bank_found == TRUE) {
                for (alloc_elem_id = start_elem ; alloc_elem_id < start_elem + _BCM_DPP_AM_EGRESS_LIF_NOF_ENTRIES_PER_BANK(unit) /4  ; alloc_elem_id ++  ) 
                {
                    ...
                    BCMDNX_IF_ERR_EXIT_MSG(rv,(_BSL_BCM_MSG("Error in allocating elem_id %d in top bank %d with start elem_id %d."), alloc_elem_id, eedb_half_bank, start_elem));
                }

                /** The top bank is available, relate it to the bank.*/
