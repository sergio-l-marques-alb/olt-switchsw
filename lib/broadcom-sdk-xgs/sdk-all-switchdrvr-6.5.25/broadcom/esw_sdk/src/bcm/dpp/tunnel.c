 ...
 ) 
{
    ...
    if (info->type == bcmTunnelTypeVxlan || info->type == bcmTunnelTypeL2Gre) {
        if((info->sip_mask != 0 && info->dip_mask == 0) && !SOC_IS_DIP_SIP_VRF_WITH_MY_VTEP_INDEX_ENABLE(is_ipv6_tunnel, unit)){
    ...
    if (SOC_IS_JERICHO(unit)
        && ((SOC_DPP_CONFIG(unit)->pp.ipv4_tunnel_term_bitmap_enable & (SOC_DPP_IP_TUNNEL_TERM_DB_DIP_SIP_VRF)) 
            || SOC_IS_DIP_SIP_VRF_WITH_MY_VTEP_INDEX_ENABLE(is_ipv6_tunnel, unit))
    ...
    bcm_tunnel_terminator_t *info) 
{
    ...
    if (!is_ipv6_tunnel) {
        ...
        if (SOC_IS_DIP_SIP_VRF_WITH_MY_VTEP_INDEX_ENABLE(is_ipv6_tunnel, unit) 
