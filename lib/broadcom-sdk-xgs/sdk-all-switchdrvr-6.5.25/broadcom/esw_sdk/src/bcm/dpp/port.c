...
bcm_petra_port_init(int unit)
{
  ...
  /* configures the port - trap mapping for static forwarding in TM mode. */
  if (!SOC_WARM_BOOT(unit) && !SOC_DPP_PP_ENABLE(unit)) 
  {
...
bcm_petra_port_speed_set(int unit, bcm_port_t port, int speed)
{
    ...
    uint32                  tm_port = 0;
    uint32                  sch_rate, old_sch_rate;
    uint32                  egq_rate, old_egq_rate;
    uint32                  kbits_sec_max, old_kbits_sec_max;
    uint32                  dummy1;
    uint32                  dummy2;
    int                     core = 0;
        ...
        if (SOC_IS_QAX(unit) && 
            ...
            rv = MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_port_speed_set,(unit, port_i, speed));
             ...
             if (!IS_SFI_PORT(unit, port_i) && !SOC_PORT_IS_NOT_VALID_FOR_EGRESS_TM(port_flags)) {
                ...
                BCMDNX_IF_ERR_EXIT(soc_port_sw_db_speed_get(unit, port_i, &old_speed));
                ...
                if ((!is_channelized) && (speed!=old_speed)) {
                    ...
                    old_sch_rate = old_speed + (old_speed / 100) * 5; /* 5% speedup */
                    old_egq_rate = old_speed + (old_speed / 100) * 1; /* 1% speedup */
                    ...
                    BCMDNX_IF_ERR_EXIT(bcm_petra_cosq_gport_bandwidth_get(unit,fap_port,0,&dummy1,&old_kbits_sec_max,&dummy2));
                    /* Add the difference between the old speed and the new speed to the interface bandwidth */
                    kbits_sec_max = old_kbits_sec_max + egq_rate * 1000 - old_egq_rate * 1000;
                    if (kbits_sec_max <= SOC_TMC_IF_MAX_RATE_KBPS_ARAD(unit)) {
                        BCMDNX_IF_ERR_EXIT(bcm_petra_cosq_gport_bandwidth_set(unit,fap_port,0,0,kbits_sec_max,0));
                    } else {
                    ...
                    kbits_sec_max = old_kbits_sec_max + sch_rate * 1000 - old_sch_rate * 1000;
                    if (kbits_sec_max <= SOC_TMC_IF_MAX_RATE_KBPS_ARAD(unit)) {
                        BCMDNX_IF_ERR_EXIT(bcm_petra_cosq_gport_bandwidth_set(unit,fap_port,0,0,kbits_sec_max,0));
                    } else {
                        BCMDNX_IF_ERR_EXIT(bcm_petra_cosq_gport_bandwidth_set(unit,fap_port,0,0,SOC_TMC_IF_MAX_RATE_KBPS_ARAD(unit),0));
                            ...
                            bcm_port_control_t type, int value)
{
    ...
    switch (type)
    {
            ...
            if ((is_interface || is_local_port) && (port < FABRIC_LOGICAL_PORT_BASE(unit)) ) {
                
                {
                    ...
                    if (value == 0)
                    {
                        ...
                        {
                            /*
                             * This is not really required. Added to appease coverity.
                             */
                            BCMDNX_IF_ERR_EXIT(SOC_E_PARAM);
                        }
            ...
            if (SOC_IS_JERICHO(unit)) {
                if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "mixed_qos_model_enable", 0)) {
                    ...
                    if ((SOC_DPP_CONFIG(unit))->pp.routing_preserve_dscp & SOC_DPP_DSCP_PRESERVE_PORT_CTRL_EN) {
                        ...
                        BCM_SAND_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_port_pp_port_get, (unit, core, pp_port, &pp_port_info)));
                        if (pp_port_info.flags & SOC_PRESERVING_DSCP_PORT_PP_PORT) {
                            dscp_preserve_per_port = 1;
                        }
                    }
                    /* Working with bcmPortControlTCPriority */
                    if (dscp_preserve_per_port) {
                        BCMDNX_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_egr_prog_editor_pp_profile_set, (unit, port, (value ? ARAD_PRGE_PP_SELECT_COPY_DSCP_MIXED_QOS_MODEL : ARAD_PRGE_PP_SELECT_PP_COPY_HEADER))));
                    } else {
                        BCMDNX_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_egr_prog_editor_pp_profile_set, (unit, port, (value ? ARAD_PRGE_PP_SELECT_MIXED_QOS_MODEL : ARAD_PRGE_PP_SELECT_PP_COPY_HEADER))));
                    }
                } else {
                    BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("Invalid control")));
                }
        ...
        case bcmPortControlLatencyEcnEnable:
            {
                int ecn_enable = value ? TRUE : FALSE;
                if (!(SOC_IS_JERICHO_PLUS_ONLY(unit)))
                {
                    BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("Invalid control, only support JR+")));
                }

                BCMDNX_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_port_control_ecn_enable_set,(unit, port, ecn_enable)));
            }
            break;
                            ...
                            bcm_port_control_t type, int *value) 
{
    ...
    case bcmPortControlAllowQosInheritance:
        ...
        case bcmPortControlLatencyEcnEnable:
            {
                int ecn_enable = FALSE;
                if (!(SOC_IS_JERICHO_PLUS_ONLY(unit)))
                {
                    BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("Invalid control, only support JR+")));
                }

                BCMDNX_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_port_control_ecn_enable_get,(unit, port, &ecn_enable)));
                *value = ecn_enable ? TRUE : FALSE;
            }
            break;
        case bcmPortControlReflectorL2:
            {
        ...
        case bcmPortControlTrustCustomHeaderPri:
            if (SOC_IS_JERICHO(unit)) {
                if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "mixed_qos_model_enable", 0)) {
                    ...
                    /* Working with bcmPortControlTCPriority */
                    if (dscp_preserve_per_port) {
                        BCMDNX_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_egr_prog_editor_pp_profile_get, (unit, port, &prog_edit_profile)));
                        *value = (prog_edit_profile == ARAD_PRGE_PP_SELECT_COPY_DSCP_MIXED_QOS_MODEL) ? 1 :0;
                    } else {
                        BCMDNX_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_egr_prog_editor_pp_profile_get, (unit, port, &prog_edit_profile)));
                        *value = (prog_edit_profile == ARAD_PRGE_PP_SELECT_MIXED_QOS_MODEL) ? 1 :0;
                    }
                } else {
                    BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("Invalid control")));
                }
...
bcm_petra_port_probe(int unit, pbmp_t pbmp, pbmp_t *okay_pbmp) 
{
    ...
    */
    if (sfi_in_pbmp && !SOC_WARM_BOOT(unit)) {
...
{
    uint32      is_valid, out_port_priority, tm_port = 0, pp_port = 0, first_header_size = 0, first_phy_port, first_phy_port_elk, interface_max_rate;
    uint32      temp_flags = 0;
    ...
    if (_BCM_DPP_PORT_ADD_IS_NOT_MODEM_PORT(flags)) 
    {
        ...
        header_type = SOC_DPP_CONFIG(unit)->arad->init.pp_enable ? BCM_SWITCH_PORT_HEADER_TYPE_ETH : BCM_SWITCH_PORT_HEADER_TYPE_TM;
        if (flags & BCM_PORT_ADD_TDM_PORT) {
    ...
    if (_BCM_DPP_PORT_ADD_IS_NOT_MODEM_PORT(flags)) {
        ...
        if (SOC_IS_QAX(unit) && SOC_DPP_CONFIG(unit)->qax->lbi_en) {
            BCMDNX_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_lb_is_control_port_get, (unit, port, &is_lb_ctrl_port)));
            if (is_lb_ctrl_port) {
                rv = LB_INFO_ACCESS.port_to_lbg.set(unit, port, SOC_TMC_LB_CONTROL);
                BCMDNX_IF_ERR_EXIT(rv);
            }
        }
...
int bcm_petra_port_get(int unit, bcm_port_t port, uint32 *flags, bcm_port_interface_info_t *interface_info, bcm_port_mapping_info_t *mapping_info) 
{
    ...
    *flags = 0;
    ...
    BCMDNX_IF_ERR_EXIT(soc_port_sw_db_flags_get(unit, port, &port_info_flags));
    if (!SOC_PORT_IS_ELK_INTERFACE(port_info_flags))
    {
        /* get pp port and core */
        BCMDNX_IF_ERR_EXIT(soc_port_sw_db_local_to_pp_port_get(unit, port, &mapping_info->pp_port, &mapping_info->core));
    }
...
int bcm_petra_port_remove(int unit, bcm_port_t port, uint32 flags) 
{
    ...
    if (!is_channelized && _BCM_DPP_PORT_REMOVE_IS_NOT_MODEM_PORT(port_flags)) {
    ...
    if (!IS_LBG_PORT(unit, port)) {
        ...
        BCMDNX_IF_ERR_EXIT(_bcm_linkscan_port_deinit(unit, port));
    }
    ...
    if (is_lbg_port) {
        rv = LB_INFO_ACCESS.port_to_lbg.get(unit, port, &lbg_id);
        BCMDNX_IF_ERR_EXIT(rv);

        rv = LB_INFO_ACCESS.lbg_valid_bmp.bit_clear(unit, lbg_id);
        BCMDNX_IF_ERR_EXIT(rv);
    }

    if (is_lbg_port || is_lb_ctrl_port) {
                            ...
                            bcm_port_control_t type, int *value)
{
    ...
    if (SOC_DPP_PP_ENABLE(unit)) {
      ...
      case bcmPortControlUnknownMacDaAction:
        {
            BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("Available for Petra only"))); 
        }
        break;
      case bcmPortControlPrivateVlanIsolate:
        {
    ...
    uint32 flags)
{
   ...
   /* verify paramters Petra does not support vlan member ingress */
    ...
    bcm_gport_t *tunnel_port_id)
{
    ...
    if (SOC_DPP_CONFIG(unit)->pp.pon_port_channelization_enable) {
        nof_channels_bits = utilex_log2_round_up(SOC_DPP_CONFIG(unit)->pp.pon_port_channelization_num);
        offset = _BCM_PPD_GPORT_PON_PP_PORT_CHANNEL_PROFILE_OFFSET(unit, nof_channels_bits);
    } else {
    ...
    if (gport_info.local_port == _BCM_PPD_GPORT_PON_TO_PHY_PORT(unit, gport_info.local_port)) {
        ...
        {
            if (pon_channel_profile >= _BCM_PPD_NOF_PON_CHANNEL_PROFILE(unit, nof_channels_bits)) {
                BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("PON channel profile range is 0~%d"), _BCM_PPD_NOF_PON_CHANNEL_PROFILE(unit, nof_channels_bits)));
            }
    ...
    bcm_gport_t tunnel_port_id)
{
    ...
    if (SOC_DPP_CONFIG(unit)->pp.pon_port_alloc_mode == 0) {
    ...
    } else if (SOC_DPP_CONFIG(unit)->pp.pon_port_alloc_mode == 1) {
    ...
    } else if (SOC_DPP_CONFIG(unit)->pp.pon_port_alloc_mode == 2) {
        if (tunnel_id > ARAD_PORT_MAX_CHANNEL_ID_IN_GPON_MODE(unit)) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Channel-IDs %d-4 can not use"), ARAD_PORT_MAX_CHANNEL_ID_IN_GPON_MODE(unit)));
        }
    } else {
    ...
    BCM_SAND_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_ports_pon_tunnel_info_set,(unit, core, soc_ppd_port, tunnel_id, &info)));
    ...
    bcm_gport_t *tunnel_port_id)
{
    ...
    BCM_SAND_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_ports_pon_tunnel_info_get,(unit, core, soc_ppd_port, tunnel_id, &info)));
   ...
   soc_port_t port)
{
    ...
    if (is_allocated) {         /* set profile data */
