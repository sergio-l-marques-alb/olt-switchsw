 ...
 *  gport -          [OUT] general port
 *  fwd_decsion -    [IN] PPD forwarding decision
 *  gport_is_fec -   [OUT] general port is encoded as FEC ID
 * Returns:
 *    BCM_E_XXX
 */
int 
_bcm_dpp_gport_from_fwd_decision(int unit, bcm_gport_t *gport, SOC_PPC_FRWRD_DECISION_INFO  *fwd_decsion, int *gport_is_fec)
{
    int rv = BCM_E_NONE;
    ...
    /* Additional info present */
        ...
        if (fwd_decsion->type == SOC_PPC_FRWRD_DECISION_TYPE_TRAP) {
            ...
            if (rv == BCM_E_NONE) {
                ...
                rv = _bcm_dpp_local_lif_to_sw_resources(unit, local_in_lif, -1, _BCM_DPP_GPORT_SW_RESOURCES_INGRESS, gport_sw_resources);
                BCMDNX_IF_ERR_EXIT(rv);
                if (gport_sw_resources->in_lif_sw_resources.additional_flags & _BCM_DPP_INFLIF_MATCH_INFO_GPORT_IS_FEC)
                {
                    *gport_is_fec = 1;
                }

                if (gport_sw_resources->in_lif_sw_resources.lif_type == _bcmDppLifTypeMplsPort && (*gport_is_fec == 0)) {
            ...
            if ((out_lif_usage == _bcmDppLifTypeVlan || out_lif_usage == _bcmDppLifTypeMplsPort) && (*gport_is_fec)) {
                /* Set gport as FEC ID encoding */
                rv = _bcm_dpp_local_lif_sw_resources_learn_gport_get(unit, local_in_lif, gport);
                BCMDNX_IF_ERR_EXIT(rv);
            }
            else if ((out_lif_usage == _bcmDppLifTypeMplsPort)) {
                BCM_GPORT_SUB_TYPE_LIF_SET(outlif_encoded, 0, fwd_decsion->additional_info.outlif.val);
                BCM_GPORT_MPLS_PORT_ID_SET(*gport, outlif_encoded);
            }
                                           ...
                                           uint8 force_destination)
{
    ...
    if (fwd_decsion->additional_info.eei.type != SOC_PPC_EEI_TYPE_EMPTY) {
        ...
        if (fwd_decsion->type == SOC_PPC_FRWRD_DECISION_TYPE_FEC) {
            ...
            rv = _bcm_dpp_gport_from_fwd_decision(unit, gport, fwd_decsion, &gport_is_fec);
            BCMDNX_IF_ERR_EXIT(rv);
        }
        else {
            /* destination is phy-port inside gport info, map fwd-physical dest to gport */
            ...
            if ((encap_usage != -1)/* (If api is MACT or CC) */) {
                if (rv != BCM_E_NOT_FOUND) {
                    ...
                    if ((fwd_decsion->additional_info.outlif.type == SOC_PPC_OUTLIF_ENCODE_TYPE_RAW_INVALID) ||
                        ((fwd_decsion->type == SOC_PPC_FRWRD_DECISION_TYPE_FEC) &&
                         (fwd_decsion->additional_info.outlif.type == SOC_PPC_OUTLIF_ENCODE_TYPE_RAW) &&
                         fwd_decsion->additional_info.outlif.val && gport_is_fec)) {
                        /* set encap to include outlif value */
                        rv = _bcm_dpp_encap_from_fwd_decision(unit,encap_id, fwd_decsion, encap_usage);
                        BCMDNX_IF_ERR_EXIT(rv);
                    }
...
_bcm_dpp_gport_from_sand_pp_dest(int unit, bcm_gport_t *gport, SOC_SAND_PP_DESTINATION_ID  *dest_id)
{
    ...
    rv = _bcm_dpp_gport_from_fwd_decision(unit,gport,&fwd_decision, &gport_is_fec);
    BCMDNX_IF_ERR_EXIT(rv);
 ...
 *      gport_type - (IN) port type (MPLS/VLAN)
 *      gport_is_fec - (OUT) gport(vlan/mpls_port_id) is encoded as fec ID
 * Returns: 
 ...
 */
int
_bcm_dpp_gport_l2_intf_calculate_port_id(int unit, int act_fec, SOC_PPC_LIF_ID global_lif_id, void *gport, int gport_type, int *gport_is_fec){
    int gport_id;
    uint8 is_ingress, is_egress;
    int is_protected;
 ...
 )
{
    ...
    *gport_id = 0;
    ...
    if (is_ingress) {
        ...
        if (from_learning) {
            BCMDNX_IF_ERR_EXIT(_bcm_dpp_gport_from_fwd_decision(unit, gport_id, learn_info_ptr, &gport_is_fec));
            if (*gport_id != 0) {
