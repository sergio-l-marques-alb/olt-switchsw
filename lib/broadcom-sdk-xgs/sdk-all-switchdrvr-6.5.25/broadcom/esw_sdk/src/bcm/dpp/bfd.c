...
/***************************************************************/
/***************************************************************/
static bcm_dpp_bfd_tx_rates_t update_bfd_tx_rates;
    ...
    if ((endpoint_info->type == bcmBFDTunnelTypeUdp) || (endpoint_info->type == bcmBFDTunnelTypeMpls)
        ...
        if (mep_db_entry->mep_type == SOC_PPC_OAM_MEP_TYPE_BFD_O_IPV4_M_HOP ||  (endpoint_info->type == bcmBFDTunnelTypeMpls) 
            ...
            if (existing_endpoint_info==NULL) {
                ...
                /*IPV4_SINGLE_HOP_TTL_PROFILE only for bfd ipv4 single hop*/
 ...
 */
int
_bcm_bfd_oamp_rmep_compare(int unit,
    SOC_PPC_OAM_OAMP_RMEP_DB_ENTRY * old_entry_values,
    SOC_PPC_OAM_OAMP_RMEP_DB_ENTRY * new_entry_values)
{
    uint32 rmep_scan_time = 0;
    uint32 new_lifetime = 0;  
    uint32 new_lifetime_units = 0;  
    rmep_scan_time = _ARAD_PP_OAM_RMEP_SCAN(unit,arad_chip_kilo_ticks_per_sec_get(unit));
    if (SOC_SAND_DIV_ROUND_DOWN(new_entry_values->ccm_period, rmep_scan_time)<= 0x3FF){
        new_lifetime_units = _ARAD_PP_OAM_OAMP_RMEP_DB_LIFETIME_UNITS_SCAN_PERIOD;
        new_lifetime = ((new_entry_values->ccm_period/ rmep_scan_time)< 1)? 1 :(new_entry_values->ccm_period / rmep_scan_time);
    } else if (SOC_SAND_DIV_ROUND_DOWN(new_entry_values->ccm_period/100, rmep_scan_time)<= 0x3FF){
        new_lifetime_units = _ARAD_PP_OAM_OAMP_RMEP_DB_LIFETIME_UNITS_SCAN_PERIOD_100;
        new_lifetime = (((new_entry_values->ccm_period/100)/ rmep_scan_time)< 1)? 1 :((new_entry_values->ccm_period/100)/ rmep_scan_time);
    }    else {
        new_lifetime_units = _ARAD_PP_OAM_OAMP_RMEP_DB_LIFETIME_UNITS_SCAN_PERIOD_10000;
        new_lifetime = (((((new_entry_values->ccm_period/1000)))/(rmep_scan_time*10))< 1)? 1 :((((new_entry_values->ccm_period/1000)))/(rmep_scan_time*10));
    }

    if (old_entry_values->rmep_state !=new_entry_values->rmep_state)
    {
        return TRUE;
    }

    if ((old_entry_values->lifetime_units != new_lifetime_units) || (old_entry_values->lifetime != new_lifetime))
    {
        return TRUE;
    }
    ...
    if (old_entry_values->is_state_auto_handle != new_entry_values->is_state_auto_handle)
    {
        return TRUE;
    }

    if (old_entry_values->is_event_mask != new_entry_values->is_event_mask)
    {
        return TRUE;
    }
...
int bcm_petra_bfd_endpoint_create(int unit, bcm_bfd_endpoint_info_t *endpoint_info)
{
    ...
    SOC_PPC_OAM_OAMP_RMEP_DB_ENTRY existing_rmep_db_entry;
    ...
    if (endpoint_info_lcl->flags & BCM_BFD_ENDPOINT_UPDATE) {
        ...
        if (is_endpoint_acc_to_oamp) {
            ...
            */
            ...
            SOC_PPC_OAM_OAMP_RMEP_DB_ENTRY_clear(&existing_rmep_db_entry);
            
            /* get rmep id from sw db of mep->rmep */
            _BCM_OAM_REMOTE_MEP_INDEX_TO_INTERNAL(rmep_index_internal, endpoint_info_lcl->remote_id); 

            /* get  existing rmep db entry*/
            soc_sand_rv = soc_ppd_oam_oamp_rmep_get(unit, rmep_index_internal, &existing_rmep_db_entry);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
            ...
            if (_bcm_bfd_oamp_rmep_compare(unit, &existing_rmep_db_entry, &rmep_db_entry))
            {
...
int bcm_petra_bfd_endpoint_destroy(int unit, bcm_bfd_endpoint_t endpoint) {
    ...
    uint8 found;
    SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY classifier_mep_entry;
    SOC_PPC_OAM_MEP_DEL_INFO mep_del_info;
    SOC_PPC_OAM_OAMP_MEP_DB_ENTRY mep_db_entry;
    int is_last;
    uint32 trap_code;
    uint32 rmep_index_internal;
    ...
    is_default = _BCM_BFD_IS_MEP_ID_DEFAULT(unit, endpoint);
    SOC_PPC_OAM_OAMP_MEP_DB_ENTRY_clear(&mep_db_entry);
    sal_memset(&mep_del_info, 0x0, sizeof(SOC_PPC_OAM_MEP_DEL_INFO));
    ...
    if (!is_oamp_server
        ...
        if(!(classifier_mep_entry.flags & SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY_FLAG_BFD_SEAMLESS_BFD_REFLECOTR)){
            soc_sand_rv = soc_ppd_oam_classifier_mep_delete(unit, endpoint, &classifier_mep_entry, &mep_del_info);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        }
...
bcm_dpp_bfd_endpoint_diag_print(bcm_bfd_endpoint_info_t *endpoint_info) {
    ...
    case bcmBFDTunnelTypeMplsPhp:
        LOG_CLI((BSL_META("BFD over MPLS PHP"))); break;
    case bcmBFDTunnelTypePweRouterAlert:
