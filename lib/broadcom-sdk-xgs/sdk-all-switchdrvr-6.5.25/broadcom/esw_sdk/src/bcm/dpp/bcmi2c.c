 ...
 */
/*
 * Leave some code outside the 'ifdef' to appease the compiler
 ...
 * Parameters:
 *    unit    - device number or I2C bus number
...
int
bcm_petra_i2c_open(int unit, char *devname, uint32 flags, int speed)
{
...
int
bcm_petra_i2c_write(int unit, int fd, uint32 addr, uint8 *data, uint32 nbytes)
{
...
int
bcm_petra_i2c_read(int unit, int fd, uint32 addr, uint8 *data, uint32 * nbytes)
{
    if ((fd < 0) || (fd >= soc_i2c_device_count(unit))) {
    return BCM_E_PARAM;
    }
...
int
bcm_petra_i2c_ioctl(int unit, int fd, int opcode, void *data, int len)
{
    if ((fd < 0) || (fd >= soc_i2c_device_count(unit))) {
    return BCM_E_PARAM;
    }
