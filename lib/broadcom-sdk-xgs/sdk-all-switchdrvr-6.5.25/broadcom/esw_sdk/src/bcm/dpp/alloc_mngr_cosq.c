                                                   ...
                                                   int* flow_id)
{
                   ...
                   (pool_ref_index < pool_ref_max_entries)); ++pool_ref_index) 
    {
        ...
        if (!SOC_WARM_BOOT(unit)) {
            ...
            {
                rc = dpp_am_res_silent_alloc_align_tag(unit, 0, type_id, res_flags, alloc_args->align, alloc_args->offset, tag, count, &pool_element);
            }
            if (rc == BCM_E_RESOURCE) {
                continue;
            }
...
STATIC int
   _bcm_dpp_am_cosq_scheduler_find_valid_flow_id(int unit,
    ...
    while (i < alloc_scheme_arr_len) {
        ...
        if (alloc_scheme_arr[i] != BCM_DPP_AM_COSQ_ALLOC_SCHEME_CONNECTOR_REGION_0)
        {
            /*
             * tag is required for VOQ connector allocation since 8 connectors shares one remote modid.
             * tag is required for connector only regions. It is not required for interdigitated regions.
             */
            tag = NULL;
        }
