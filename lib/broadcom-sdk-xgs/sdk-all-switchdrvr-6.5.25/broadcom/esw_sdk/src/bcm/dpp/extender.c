 ...
 *      gport_is_fec -  (IN) gport is encoded as FEC ID
 * Returns:
 *      BCM_E_XXX
 */
int
_bcm_dpp_in_lif_extender_match_add(int unit, bcm_extender_port_t *extender_port, int in_lif, int gport_is_fec)
{
...
int bcm_petra_extender_port_add(
        ...
        bcm_extender_port_t *extender_port)
{
    int rv = BCM_E_NONE, ret = BCM_E_NONE;
    ...
    if (!update) {
        /* set vlan_port_id */
        rv = _bcm_dpp_gport_l2_intf_calculate_port_id(unit, 0, extender_lif_resources.global_out_lif, extender_port, BCM_GPORT_EXTENDER_PORT, &gport_is_fec);
        BCMDNX_IF_ERR_EXIT(rv);
    }
    ...
    if (!is_remote_lif) {
        if (is_ingress) {
            ...
            rv = _bcm_dpp_in_lif_extender_match_add(unit, extender_port, extender_lif_resources.local_in_lif, gport_is_fec);
            BCMDNX_IF_ERR_EXIT(rv);
        }
    ...
    if (_BCM_DPP_GPORT_INFO_IS_LOCAL_PORT(gport_info) && (!is_remote_lif)) {
        ...
        if (is_ingress) {
            ...
            /**if failed to release SW DB*/
            if (rv != BCM_E_NONE) {
                ret = MBCM_PP_DRIVER_CALL(unit, mbcm_pp_l2_lif_extender_remove, (unit, &in_extender_key, &extender_lif_resources.local_in_lif));
                BCMDNX_IF_ERR_EXIT(ret);
                
                ret = _bcm_dpp_sw_db_hash_vlan_find(unit, (sw_state_htb_key_t) &(extender_port->extender_port_id), 
                                                   (sw_state_htb_data_t) &phy_port, TRUE);        
                BCMDNX_IF_ERR_EXIT(ret);
            }
            BCMDNX_IF_ERR_EXIT(rv);
