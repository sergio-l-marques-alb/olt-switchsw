...
/* OAM */
static bcm_oam_event_cb _g_dpp_oam_event_cb[BCM_MAX_NUM_UNITS][bcmOAMEventCount];
/* OAM LM/DM statistics reports handlers */
static bcm_oam_performance_event_cb _g_dpp_oam_performance_event_cb[BCM_MAX_NUM_UNITS][bcmOAMEventCount];
/* User data */
static void*            _g_dpp_oam_event_ud[BCM_MAX_NUM_UNITS][bcmOAMEventCount];
/* BFD */
static bcm_bfd_event_cb _g_dpp_bfd_event_cb[BCM_MAX_NUM_UNITS][bcmBFDEventCount];
/* User data */
static void*            _g_dpp_bfd_event_ud[BCM_MAX_NUM_UNITS][bcmBFDEventCount];

/* SAT */
static bcm_sat_event_cb _g_dpp_sat_event_cb[BCM_MAX_NUM_UNITS][bcmSATEventCount];
/* User data */
static void *_g_dpp_sat_event_ud[BCM_MAX_NUM_UNITS][bcmSATEventCount];
...
int bcm_petra_oam_event_register(
    ...
    for (event_i = 0; event_i < bcmOAMEventCount; event_i++) {
         if (BCM_OAM_EVENT_TYPE_GET(event_types, event_i)) {
             ...
             if (_g_dpp_oam_event_cb[unit][event_i] && (_g_dpp_oam_event_cb[unit][event_i] != cb)) {
             ...
             _g_dpp_oam_event_cb[unit][event_i] = cb;
             _g_dpp_oam_event_ud[unit][event_i] = user_data;
         }
    }
...
int bcm_petra_oam_event_unregister(
    ...
    for (event_i = 0; event_i < bcmOAMEventCount; event_i++) {
         if (BCM_OAM_EVENT_TYPE_GET(event_types, event_i)) {
             ...
             _g_dpp_oam_event_cb[unit][event_i] = NULL;
             _g_dpp_oam_event_ud[unit][event_i] = NULL;
    ...
    void *user_data)
{
    ...
    for (event_i = 0; event_i < bcmOAMEventCount; event_i++) {
         if (BCM_OAM_EVENT_TYPE_GET(event_types, event_i)) {
             if (_g_dpp_oam_performance_event_cb[unit][event_i] && (_g_dpp_oam_performance_event_cb[unit][event_i] != cb)) {
                 /* A different calblack is already registered for this event. Return error */
                 BCMDNX_ERR_EXIT_MSG(BCM_E_EXISTS, (_BSL_BCM_MSG("EVENT %d already has a registered callback"), event_i));
             }
             _g_dpp_oam_performance_event_cb[unit][event_i] = cb;
             _g_dpp_oam_event_ud[unit][event_i] = user_data;
         }
    }
    ...
    bcm_oam_performance_event_cb cb)
{
    ...
    for (event_i = 0; event_i < bcmOAMEventCount; event_i++) {
         if (BCM_OAM_EVENT_TYPE_GET(event_types, event_i)) {
             ...
             _g_dpp_oam_performance_event_cb[unit][event_i] = NULL;
             _g_dpp_oam_event_ud[unit][event_i] = NULL;
...
int _bcm_petra_bfd_event_register(
    ...
    for (event_i = 0; event_i < bcmBFDEventCount; event_i++) {
         if (BCM_BFD_EVENT_TYPE_GET(event_types, event_i)) {
             if (_g_dpp_bfd_event_cb[unit][event_i] && (_g_dpp_bfd_event_cb[unit][event_i] != cb)) {
                 /* A different calblack is already registered for this event. Return error */
                 BCMDNX_ERR_EXIT_MSG(BCM_E_EXISTS, (_BSL_BCM_MSG("EVENT %d already has a registered callback"), event_i));
             }
             _g_dpp_bfd_event_cb[unit][event_i] = cb;
             _g_dpp_bfd_event_ud[unit][event_i] = user_data;
         }
    }
...
int _bcm_petra_bfd_event_unregister(
    ...
    for (event_i = 0; event_i < bcmBFDEventCount; event_i++) {
         if (BCM_BFD_EVENT_TYPE_GET(event_types, event_i)) {
             ...
             _g_dpp_bfd_event_cb[unit][event_i] = NULL;
             _g_dpp_bfd_event_ud[unit][event_i] = NULL;
...
int _bcm_jer_sat_event_register(
    ...
    for (type = 0; type < bcmSATEventCount; type++) {
        if (type == event_type) {
            _g_dpp_sat_event_cb[unit][type] = cb;
            _g_dpp_sat_event_ud[unit][type] = user_data;
        }
    }
...
int _bcm_jer_sat_event_unregister(
    ...
    for (type = 0; type < bcmSATEventCount; type++) {
        if (type == event_type) {
            _g_dpp_sat_event_cb[unit][type] = NULL;
            _g_dpp_sat_event_ud[unit][type] = NULL;
        }
    }
...
int bcm_oam_sat_ctf_report_process(int unit, uint32 *event_id,uint32 *event_data) {
    ...
    if (_g_dpp_sat_event_cb[unit][type]) {
        _g_dpp_sat_event_cb[unit][type](unit, type, (void *)&report, _g_dpp_sat_event_ud[unit][type]);
    }
...
int _dpp_oam_fifo_interrupt_handle_user_callback(int unit, _bcm_oam_event_and_rmep_info *event_info,shr_htb_data_t really_the_flags ){
    ...
    SOC_PPC_OAM_REPORT_MODE report_mode;
    SOC_PPC_OAM_ETH1731_MEP_PROFILE_ENTRY eth1731_profile_data;
    int dont_care;
    ...
    if (event_info->is_oam) {
        if (_g_dpp_oam_event_cb[unit][event_info->event_number]) {
            ...
            _g_dpp_oam_event_cb[unit][event_info->event_number](unit, flags, event_info->event_number, mep_info.ma_index, rmeb_db_ndx, _g_dpp_oam_event_ud[unit][event_info->event_number]);
        }
        else if (_g_dpp_oam_performance_event_cb[unit][event_info->event_number]) {
            ...
            rv = _bcm_dpp_am_template_oam_eth1731_mep_profile_get(unit, event_info->mep_or_rmep_indx, &dont_care, &eth1731_profile_data);
            BCMDNX_IF_ERR_EXIT(rv);

            rv = MBCM_PP_DRIVER_CALL(unit, mbcm_pp_oamp_report_mode_get, (unit, &report_mode));
            BCMDNX_IF_ERR_EXIT(rv);
            ...
            if (event_info->event_number == bcmOAMEventEndpointLmStatistics) { /* LM */
                if (report_mode != SOC_PPC_OAM_REPORT_MODE_RAW) {
            ...
            /*finally, the callback*/
            _g_dpp_oam_performance_event_cb[unit][event_info->event_number](unit, event_info->event_number, mep_info.ma_index, event_info->mep_or_rmep_indx,
                                                                        &report_data, _g_dpp_oam_event_ud[unit][event_info->event_number]);
        }
    } else { /* bfd*/
        if (_g_dpp_bfd_event_cb[unit][event_info->event_number]) {
            _BCM_OAM_REMOTE_MEP_INDEX_FROM_INTERNAL(event_info->mep_or_rmep_indx, rmeb_db_ndx);
            rv = _bcm_dpp_oam_bfd_rmep_info_db_get(unit, rmeb_db_ndx,  &rmep_info, &found);
            BCMDNX_IF_ERR_EXIT(rv);
            if (!found) {
            ...
            _g_dpp_bfd_event_cb[unit][event_info->event_number](unit, flags, event_types, rmep_info.mep_index, _g_dpp_bfd_event_ud[unit][event_info->event_number]);
        }
    }
...
int bcm_petra_oam_init(int unit) {
    ...
    if (port_config == NULL) {
     ...
     BCMDNX_IF_ERR_EXIT(_bcm_dpp_rx_trap_sw_id_to_hw_id(unit, SOC_PPC_TRAP_CODE_SAT_2, &trap_code));
     rv = bcm_petra_rx_trap_type_create(unit, BCM_RX_TRAP_WITH_ID, bcmRxTrapUserDefine, &trap_code);
     BCMDNX_IF_ERR_EXIT(rv);

     /* Configuration trapping to SAT */
     bcm_rx_trap_config_t_init(&trap_config);
     trap_config.flags = BCM_RX_TRAP_UPDATE_DEST | BCM_RX_TRAP_TRAP;
     trap_config.dest_port = (SOC_IS_QAX(unit))?(found?sat_port:oamp_port[0]):(oamp_port[0]);
    ...
    for (oam_event_i = 0; oam_event_i < bcmOAMEventCount; oam_event_i++) {
         _g_dpp_oam_event_cb[unit][oam_event_i] = NULL;
         _g_dpp_oam_performance_event_cb[unit][oam_event_i] = NULL;
         _g_dpp_oam_event_ud[unit][oam_event_i] = NULL;
    }
    /* bfd event handler init */
    for (bfd_event_i = 0; bfd_event_i < bcmBFDEventCount; bfd_event_i++) {
         _g_dpp_bfd_event_cb[unit][bfd_event_i] = NULL;
         _g_dpp_bfd_event_ud[unit][bfd_event_i] = NULL;
    }
...
/**
 * \brief
 *   To set 2nd-pass src_sys_port to be 1st-pass out_pp_port
 ...
 *   * None
 */
STATIC int _bcm_oam_set_src_sys_port_to_1st_out_pp_port_for_up_mep_lbm(int unit, bcm_oam_endpoint_info_t *endpoint_info)
{
    uint32      pp_port;
    int         core;
    ARAD_IHP_TM_PORT_SYS_PORT_CONFIG_TBL_DATA sys_port_config;
    soc_port_t mirror_rcy_port;
    uint32 tm_port = 0;
    uint32 rv;
    ...
    rv = _bcm_dpp_gport_to_phy_port(unit, endpoint_info->gport, _BCM_DPP_GPORT_TO_PHY_OP_LOCAL_IS_MANDATORY, &local_gport_info);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    BCMDNX_IF_ERR_EXIT(MBCM_DPP_SOC_DRIVER_CALL(unit, mbcm_dpp_ports_mirror_to_rcy_port_get,(unit, core, pp_port, &mirror_rcy_port)));
    
    /*get recycle port from forward port */
    rv = soc_port_sw_db_local_to_tm_port_get(unit,mirror_rcy_port,&tm_port,&core);
    BCM_SAND_IF_ERR_EXIT(rv);
    /* Set systerm port to be 1st_pass_out_pp_port(local_port), in order to use it on PTCH-2 as injected*/
    rv = arad_ihp_tm_port_sys_port_config_tbl_get_unsafe(unit, core, tm_port, &sys_port_config);
    BCM_SAND_IF_ERR_EXIT(rv);
    sys_port_config.system_port_value_to_use = 1;
    sys_port_config.system_port_value = pp_port | core << 8;/* The leftmost bit in the local port for Up-MEP represents the core in Jericho.*/
    rv = arad_ihp_tm_port_sys_port_config_tbl_set_unsafe(unit, core, tm_port, &sys_port_config);
    BCM_SAND_IF_ERR_EXIT(rv);
...
STATIC int _bcm_oam_endpoint_local_create(int unit, bcm_oam_endpoint_info_t *endpoint_info_lcl)
{
    ...
    if (is_accelerated) {
        ...
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        /*3-pass LBM: Needs to set 2nd-pass src_sys_port to be 1st-pass out_pp_port*/
        if(SOC_DPP_CONFIG(unit)->pp.upmep_lbm_is_configured && endpoint_info_lcl->type == bcmOAMEndpointTypeEthernet
            && _BCM_OAM_DISSECT_IS_ENDPOINT_UPMEP(endpoint_info_lcl)){
            rv = _bcm_oam_set_src_sys_port_to_1st_out_pp_port_for_up_mep_lbm(unit,endpoint_info_lcl);
            BCMDNX_IF_ERR_EXIT(rv);
        }
    ...
    int is_loss_unset)
{
    ...
    /*support 3-pass LBM on un_accelerated mep, setting 2nd-pass src_sys_port to be 1st-pass out_pp_port */
    if (!is_accelerated && SOC_DPP_CONFIG(unit)->pp.upmep_lbm_is_configured
      && BCM_OAM_OPCODE_GET(*action_lcl, SOC_PPC_OAM_ETHERNET_PDU_OPCODE_LBM) 
      && (action_lcl->destination == BCM_GPORT_INVALID)
      && _BCM_OAM_DISSECT_IS_CLASSIFIER_MEP_UPMEP(&classifier_mep_entry)) {
        /*get recycle port from forward port */
        BCMDNX_IF_ERR_EXIT(MBCM_DPP_SOC_DRIVER_CALL(unit, mbcm_dpp_ports_mirror_to_rcy_port_get,(unit, classifier_mep_entry.port_core, classifier_mep_entry.port, &mirror_rcy_port)));
        core_id = classifier_mep_entry.port_core;
        rv = soc_port_sw_db_local_to_tm_port_get(unit,mirror_rcy_port,&tm_port,&core_id);
        BCM_SAND_IF_ERR_EXIT(rv);
        /* Set systerm port to be 1st_pass_out_pp_port(local_port), in order to use it on PTCH-2 as injected*/
        rv = arad_ihp_tm_port_sys_port_config_tbl_get_unsafe(unit, core_id, tm_port, &sys_port_config);
        BCM_SAND_IF_ERR_EXIT(rv);
        sys_port_config.system_port_value_to_use = 1;
        sys_port_config.system_port_value = classifier_mep_entry.port | classifier_mep_entry.port_core << 8;/* The leftmost bit in the local port for Up-MEP represents the core in Jericho.*/
        rv = arad_ihp_tm_port_sys_port_config_tbl_set_unsafe(unit, classifier_mep_entry.port_core, tm_port, &sys_port_config);
        BCM_SAND_IF_ERR_EXIT(rv);
    }
...
int sat_loopback_add_rx(int unit, bcm_oam_loopback_t *loopback_ptr) {
    ...
    if ((classifier_mep_entry.flags & SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY_FLAG_UPMEP) == 0) { /* Down mep */
        ...
        rv = OAM_ACCESS.trap_info.upmep_trap_ids.get(unit, SOC_PPC_OAM_UPMEP_TRAP_ID_SAT, &lb_trap_id);
        BCMDNX_IF_ERR_EXIT(rv);
        if (SOC_IS_QAX(unit)) {
            /* In QAX, lb trap id should use UPMEP SAT trap */
            BCMDNX_IF_ERR_EXIT(_bcm_dpp_rx_trap_sw_id_to_hw_id(unit,lb_trap_id, &trap_id));
        } else {
    ...
    if (!(loopback_ptr->flags & BCM_OAM_LOOPBACK_UPDATE)) {
        ...
        if(classifier_mep_entry.flags & SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY_FLAG_UPMEP){
            rv = sw_state_access[unit].dpp.soc.arad.pp.oam.lb_tst_up_trap_used_cnt.get(unit, &lb_trap_used_cnt);
            BCMDNX_IF_ERR_EXIT(rv);
        }
        else {
        ...
        lb_trap_used_cnt ++;
        if(classifier_mep_entry.flags & SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY_FLAG_UPMEP){
            rv = sw_state_access[unit].dpp.soc.arad.pp.oam.lb_tst_up_trap_used_cnt.set(unit, lb_trap_used_cnt);
            BCMDNX_IF_ERR_EXIT(rv);
        }
        else {
...
int sat_loopback_add_tx(int unit, bcm_oam_loopback_t *loopback_ptr) {
    ...
    for(i=0; i<6;i++){
        ...
        gtf_info.mac_address[i] = loopback_ptr->peer_da_mac_address[i];
    }
    /*build tst header*/
    ...
    for (i = 0; i < loopback_ptr->num_tlvs; i++) {
        ...
        if ((loopback_ptr->tlvs[i].tlv_type == bcmOamTlvTypeData)) {
            ...
            repeated_data[0] = (loopback_ptr->tlvs[i].four_byte_repeatable_pattern & 0xFF000000) >> 24;
            repeated_data[1] = (loopback_ptr->tlvs[i].four_byte_repeatable_pattern & 0xFF0000) >> 16;
            repeated_data[2] = (loopback_ptr->tlvs[i].four_byte_repeatable_pattern & 0xFF00) >> 8;
            repeated_data[3] = (loopback_ptr->tlvs[i].four_byte_repeatable_pattern & 0xFF);
            /*
             * Repeat Pattern data doesn't always start from DATA[0]
             * If tlv stamped payload offset(from system header) = 4*N, the TLV DATA Start with DATA[0],
             * otherwise DATA starts with DATA[stamped payload offset%4].
            */
            if ((tlv_data_offset % 4) != 0)
            {
                shift_offset = 4 - (tlv_data_offset % 4);
                config->payload.payload_pattern[0] = repeated_data[shift_offset];
                config->payload.payload_pattern[1] = repeated_data[(shift_offset + 1) % 4];
                config->payload.payload_pattern[2] = repeated_data[(shift_offset + 2) % 4];
                config->payload.payload_pattern[3] = repeated_data[(shift_offset + 3) % 4];
                config->payload.payload_pattern[4] = repeated_data[shift_offset];
                config->payload.payload_pattern[5] = repeated_data[(shift_offset + 1) % 4];
                config->payload.payload_pattern[6] = repeated_data[(shift_offset + 2) % 4];
                config->payload.payload_pattern[7] = repeated_data[(shift_offset + 3) % 4];
            }
        }
    ...
    /*
     * Adjust the GTF bandwidth
     */
    config->offsets.packet_length_compensation = loopback_ptr->gtf_packet_length_compensation;
...
int sat_loopback_get(int unit, bcm_oam_loopback_t *loopback_ptr) 
{
    ...
    sal_memcpy(loopback_ptr->peer_da_mac_address, gtf_info.mac_address, sizeof(bcm_mac_t));
...
int bcm_petra_oam_tst_rx_add(int unit,bcm_oam_tst_rx_t *tst_ptr) {
    ...
    else 
    {
        /******  Up MEP Classification    ****   */
        uint64 arg64;
        rv = OAM_ACCESS.trap_info.upmep_trap_ids.get(unit, SOC_PPC_OAM_UPMEP_TRAP_ID_SAT, &tst_trap_id);
        BCMDNX_IF_ERR_EXIT(rv);
        if (SOC_IS_QAX(unit)) {
            /* In QAX, lb trap id is already set with SAT port. use that. */
            BCMDNX_IF_ERR_EXIT(_bcm_dpp_rx_trap_sw_id_to_hw_id(unit,tst_trap_id, &trap_id));
        } else {
    ...
    BCMDNX_IF_ERR_EXIT(rv);
       ...
       if(!(tst_ptr->flags & BCM_OAM_TST_RX_UPDATE)){        
         ...
         if(classifier_mep_entry.flags & SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY_FLAG_UPMEP){
             rv = sw_state_access[unit].dpp.soc.arad.pp.oam.lb_tst_up_trap_used_cnt.get(unit, &tst_trap_used_cnt);
             BCMDNX_IF_ERR_EXIT(rv);
         }
         else {
         ...
         if(classifier_mep_entry.flags & SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY_FLAG_UPMEP){
             rv = sw_state_access[unit].dpp.soc.arad.pp.oam.lb_tst_up_trap_used_cnt.set(unit, tst_trap_used_cnt);
             BCMDNX_IF_ERR_EXIT(rv);
         }
         else {
...
int bcm_petra_oam_tst_tx_add(int unit, bcm_oam_tst_tx_t *tst_ptr)
{
   ...
   if(tst_ptr->flags & BCM_OAM_TST_TX_WITH_ID){  /* user request a specify gtf_id*/
    ...
    for(i=0; i<6;i++){
        ...
        gtf_info.mac_address[i] = tst_ptr->peer_da_mac_address[i];
    }
