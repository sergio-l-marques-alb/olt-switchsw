...
/* NONUCAST Trunk Block Mask table pointer (used by trunk milti set) */
nonucast_trunk_block_mask_entry_t *block_mask_tbl[BCM_MAX_NUM_UNITS] = {NULL};
        ...
        bcm_trunk_member_t *member)
{
    ...
    /* Trunk multi set: lock done outside */
    if (!(trunk_info->flags & BCM_TRUNK_FLAG_MULTI_SET)) {
        TRUNK_LOCK(unit);
    }
    ...
    /* Trunk multi set: unlock done outside */
    if (!(trunk_info->flags & BCM_TRUNK_FLAG_MULTI_SET)) {
        TRUNK_UNLOCK(unit);
    }
        ...
        int member_count, bcm_trunk_member_t *member_array)
{
    ...
    if (soc_feature(unit, soc_feature_epc_linkflap_recover)) {
        ...
        bcm_trunk_member_t *temp_member_array = NULL;
...
/*
 * Function:
 *	_bcm_esw_trunk_block_mask_write
 * Purpose:
 *	DMA write nonucast trunk block mask after multi set.
 * Parameters:
 *      unit - StrataSwitch PCI device unit number (driver internal).
 * Returns:
 *      BCM_E_NONE     Success.
 ...
 */
int
_bcm_esw_trunk_block_mask_write(int unit)
{
    int rv = BCM_E_NONE;
    int table_max;
    soc_mem_t mem;
    ...
    if (mask_table) {
        ...
        table_max = soc_mem_index_max(unit, mem);
        ...
        if (soc_feature(unit, soc_feature_axp) && BCM_SUCCESS(rv)) {
...
/*
 * Function:
 *	bcm_esw_trunk_multi_set
 * Purpose:
 *      Update ports to multiple trunk groups.
 * Parameters:
 *      unit - StrataSwitch PCI device unit number (driver internal).
 *      trunk_coount - Number of trunk groups in trunk_array.
 *      trunk_array - Array of trunk groups.
 *      tot_member_count - Total number of members in member_array from trunk_array.
 *      member_array - Chained members list from trunk_array.
 * Returns:
 *      BCM_E_NONE              Success.
 *      BCM_E_XXX
 * Notes:
 *      This API is supported only for TD/TD2/TD3/TH/TH2/TH3/MV2 SOC_IS_TD_TT() devices.
 *      The existing ports in the trunk groups will be replaced with new ones.
 *      The flags BCM_TRUNK_FLAG_MULTI_SET will be added internally.
 *      The trunk multi set API will update trunk port members in order and
 *        will exit with first error encountered.
 *      The set_result for each trunk will reflect the trunk set result
 *        for each trunk:
 *          BCM_E_NONE for trunk set OK.
 *          BCM_E_FAIL for unfinished trunks after the first trunk failure.
 */
int
bcm_esw_trunk_multi_set(int unit,
    int trunk_count, bcm_trunk_multiset_info_t *trunk_array,
    int tot_member_count, bcm_trunk_member_t *member_array)
{
#ifdef BCM_TRIDENT_SUPPORT
    int rv = BCM_E_NONE;
    int rv1 = BCM_E_NONE;
    int i, mc = 0;
    bcm_trunk_multiset_info_t *p;

    if (!SOC_IS_TD_TT(unit)) {
        return BCM_E_UNAVAIL;
    }

    if ((trunk_count > 0) && (trunk_array == NULL)) {
        return BCM_E_PARAM;
    }

    if ((tot_member_count > 0) && (NULL == member_array)) {
        return BCM_E_PARAM;
    }

    /* validating tot_member_count */
    for (i = 0; i < trunk_count; i++) {
        p = &trunk_array[i];
        mc += p->member_count;
    }

    if (mc != tot_member_count) {
        return BCM_E_PARAM;
    }
    ...
    for (i = 0; i < trunk_count; i++) {
        ...
        /* Add internally MULTI_SET flags */
        p->trunk_info.flags |= BCM_TRUNK_FLAG_MULTI_SET;
        p->set_result = BCM_E_NONE;

#ifdef BCM_TOMAHAWK3_SUPPORT
        if (SOC_IS_TOMAHAWK3(unit)) {
            if (p->trunk_info.psc < 0) {
                p->trunk_info.psc = BCM_TRUNK_PSC_PORTFLOW;
            }
            rv = _bcm_th3_trunk_psc_validate(unit, p->tid, p->trunk_info.psc);
            if (BCM_FAILURE(rv)) {
                p->set_result = rv;
                break;
            }
...
#endif /* BCM_TOMAHAWK3_SUPPORT */
        rv =  _bcm_esw_trunk_set(unit, p->tid, &p->trunk_info,
                                 p->member_count, &member_array[mc]);
        /* exit with first error */
        if (BCM_FAILURE(rv)) {
            p->set_result = rv;
            break;
        }
    ...
    rv1 = _bcm_esw_trunk_block_mask_write(unit);
    ...
    /* set_result = BCM_E_FAIL for unfinished trunks */
    while (++i < trunk_count) {
        p = &trunk_array[i];
        p->set_result = BCM_E_FAIL;
    }
    ...
    int status)
{
    ...
    if (is_local) {
        if (IS_HG_PORT(unit, port) || _bcm_esw_is_hgoe_port(unit, port)) {
