/*
 * $Id: flexflow_encap.c,v 1.0 Broadcom SDK $
              ...
              bcm_flow_port_encap_t *pe_info, int is_network_port)
{
    ...
    uint32 flex_data_count;
    uint32 srv6_handle;
    uint32 srv6_handle_evpn;

    vp = _SHR_GPORT_FLOW_PORT_ID_GET(pe_info->flow_port);
    rv = READ_SOURCE_VPm(unit, MEM_BLOCK_ANY, vp, &svp);
    BCM_IF_ERROR_RETURN(rv);

    srv6_handle = FLOW_INFO(unit)->flow_srv6;
    srv6_handle_evpn = FLOW_INFO(unit)->flow_srv6_evpn;
    ...
    if (!is_network_port) {  /* access port side */
        ...
        if (!BCMI_L3_NH_FLEX_VIEW(data_type)) {
            ...
            if (process_next_ptr) {
                /* Some flows like mplsOverGre sets up in egr_obj creation */
                if ((srv6_handle && (pe_info->flow_handle == srv6_handle)) ||
                    (srv6_handle_evpn &&
                             (pe_info->flow_handle == srv6_handle_evpn))) {
                    action_set = 0;
                    soc_format_field32_set(unit, NEXT_PTR_ACTION_SETfmt,
                          &action_set, NEXT_PTR_TYPEf,
        ...
        int network_port)
{
    ...
    int size_sel, size1_sel, size2_sel;
    ...
    if (network_port) {
        ...
        if (info->valid_elements & BCM_FLOW_ENCAP_FUNCTION_VALID) {

            /* for now, only support SRv6 */
            if (!(soc_feature(unit, soc_feature_srv6_enable) &&
                 (info->flow_handle == FLOW_INFO(unit)->flow_srv6))) {
                return BCM_E_UNAVAIL;
            }

            soc_mem_field32_set(unit, mem, entry,
                     DIRECT_ASSIGNMENT_18_ACTION_SETf, info->function & 0xffff);
            soc_mem_field32_set(unit, mem, entry,
                     DIRECT_ASSIGNMENT_19_ACTION_SETf, info->function >> 16);

            /* select 16bit or 32bit function size */
            BCM_IF_ERROR_RETURN(bcm_esw_switch_control_get(unit,
                   bcmSwitchSrv6FunctionSelect1, &size1_sel));
            BCM_IF_ERROR_RETURN(bcm_esw_switch_control_get(unit,
                   bcmSwitchSrv6FunctionSelect2, &size2_sel));

            if (info->function <= 0xffff) {
                size1_sel &= BCM_SWITCH_SRV6_FUNCTION_OFFSET_64_SIZE_16BIT;
                size2_sel &= BCM_SWITCH_SRV6_FUNCTION_OFFSET_64_SIZE_16BIT;
            } else { /* 32 bit */
                size1_sel &= BCM_SWITCH_SRV6_FUNCTION_OFFSET_64_SIZE_32BIT;
                size2_sel &= BCM_SWITCH_SRV6_FUNCTION_OFFSET_64_SIZE_32BIT;
            }
            size_sel = 0;
            if (!info->function) {
                size_sel = 0;
            ...
            soc_mem_field32_set(unit, mem, entry, EDIT_CTRL_ID_ACTION_SETf,
                                size_sel);
        }
    ...
    if (network_port) {
        ...
        if (soc_feature(unit, soc_feature_srv6_enable) &&
            ...
            ((info->criteria == BCM_FLOW_ENCAP_CRITERIA_VFI) ||
             (info->criteria == BCM_FLOW_ENCAP_CRITERIA_VFI_DVP))) {
            if (SOC_MEM_FIELD_VALID(unit, mem, DIRECT_ASSIGNMENT_18_ACTION_SETf)) {
                info->function = soc_mem_field32_get(unit, mem, entry,
                         DIRECT_ASSIGNMENT_18_ACTION_SETf);
            }
            if (SOC_MEM_FIELD_VALID(unit, mem, DIRECT_ASSIGNMENT_19_ACTION_SETf)) {
                info->function |= soc_mem_field32_get(unit, mem, entry,
                     DIRECT_ASSIGNMENT_19_ACTION_SETf) << 16;
            }
        }
    } else {  /* if (network_port) */
        if (info->criteria == BCM_FLOW_ENCAP_CRITERIA_VFI_DVP_GROUP ||
