                                ...
                                bcm_monterey_port_resource_t *resource, int *op)
{
    ...
    *op = BCM_MN_PORT_RESOURCE_OP_NONE;
    ...
    for (i = 0, pr = &resource[0]; i < nport; i++, pr++) {
        ...
        if (SOC_PBMP_MEMBER(si->pbmp_mmu_lossless,pr->port)) {
            if (pr->mmu_lossless == 0) {
                *op |= BCM_MN_PORT_RESOURCE_OP_PMAP;
            }
        }else {
            if (pr->mmu_lossless == 1) {
                *op |= BCM_MN_PORT_RESOURCE_OP_PMAP;
            }
        }
        if (SOC_PBMP_MEMBER(si->roe_compression,pr->port)) {
            if (pr->roe_compression == 0) {
                *op |= BCM_MN_PORT_RESOURCE_OP_PMAP;
            }
        }else {
            if (pr->roe_compression == 1) {
                *op |= BCM_MN_PORT_RESOURCE_OP_PMAP;
            }
        }
                                  ...
                                  int nport, bcm_monterey_port_resource_t *resource)
{
    ...
    for (i = 0; i < nport; i++) {
        ...
        soc_resource[i].mmu_lossless = resource[i].mmu_lossless;
    }
                                    ...
                                    bcm_port_resource_t *resource)
{
    ...
    for (i = 0; i < nport; i++) {
        ...
        port_resource[i].mmu_lossless = resource[i].mmu_lossless;
    }
