        ...
        switch (key_type) {
            ...
            case bcmVlanTranslateEgressKeyVpnGportGroup:
                *key_type_value = TD2PLUS_DVP_EVLXLT_HASH_KEY_TYPE_GID_VFI;
                break;
            case bcmVlanTranslateEgressKeyVlanPort:
                if (SOC_IS_TRIDENT3X(unit)) { /* not supported in td3 */
            ...
            if (SOC_IS_TRX(unit)) {
                ...
                switch (arg) {
                    ...
                    case bcmVlanTranslateKeyMacPortGroup:
                        ...
                        key_type = VLXLT_HASH_KEY_TYPE_VLAN_MAC_PORT;
                        break;
                    case bcmVlanTranslateKeyPortEtagOuter:
                        use_port = 1; /* Port */
                        key_type = VLXLT_HASH_KEY_TYPE_VIF_VLAN;
                        break;
                    case bcmVlanTranslateKeyPortEtagInner:
                        use_port = 1; /* Port */
                        key_type = VLXLT_HASH_KEY_TYPE_VIF_CVLAN;
                        break;
                    default:
            ...
            if (SOC_IS_TRX(unit)) {
                ...
                case VLXLT_HASH_KEY_TYPE_VIF_VLAN:
                    if (is_niv_port) {
                        ...
                        *arg = bcmVlanTranslateKeyPortEtagOuter;
                    }
                    break;
                case VLXLT_HASH_KEY_TYPE_VIF_CVLAN:
                    if (is_niv_port) {
                        ...
                        *arg = bcmVlanTranslateKeyPortEtagInner;
                    }
                    break;
                case VLXLT_HASH_KEY_TYPE_LLVID:
...
/*
 * Function:
 *      bcm_vlan_cross_connect_control_set
 * Purpose:
 *      Set a field indicated by type parameter in VLAN cross connect entry
 * Parameters:
 *      unit       - Device unit number
 *      outer_vlan - Outer vlan ID
 *      inner_vlan - Inner vlan ID
 *      type       - Which vlan cross-connect entry parameter to set
 *      arg        - parameter value
 * Returns:
 *      BCM_E_NONE - Success.
 *      BCM_E_XXX
 */
int
bcm_esw_vlan_cross_connect_control_set(int unit,
                               bcm_vlan_t outer_vlan, bcm_vlan_t inner_vlan,
                               bcm_vlan_cross_connect_control_t type, int arg)
{
#ifdef BCM_TRX_SUPPORT
    if (SOC_IS_TRX(unit)) {
        return bcmi_tr_l2_cross_connect_control_set(unit, outer_vlan,
                        inner_vlan, type, arg);
    }
#endif
    return BCM_E_UNAVAIL;
...
/*
 * Function:
 *      bcm_vlan_cross_connect_control_get
 * Purpose:
 *      Get a field indicated by type parameter in VLAN cross connect entry
 * Parameters:
 *      unit       - Device unit number
 *      outer_vlan - Outer vlan ID
 *      inner_vlan - Inner vlan ID
 *      type       - Which vlan cross-connect entry parameter to get
 *      arg        - Hold the get value
 * Returns:
 *      BCM_E_NONE - Success.
 *      BCM_E_XXX
 */
int
bcm_esw_vlan_cross_connect_control_get(int unit,
                               bcm_vlan_t outer_vlan, bcm_vlan_t inner_vlan,
                               bcm_vlan_cross_connect_control_t type, int *arg)
{
#ifdef BCM_TRX_SUPPORT
    if (SOC_IS_TRX(unit)) {
        return bcmi_tr_l2_cross_connect_control_get(unit, outer_vlan,
                    inner_vlan, type, arg);
    }
#endif
    return BCM_E_UNAVAIL;
...
/*
 * Function:
 *      bcm_vlan_cross_connect_replace
 * Purpose:
 ...
 * Parameters:
 *      unit       - Device unit number
 *      match_info        - match parameters
 *      match_info_mask   - match parameter mask
 *      replace_info      - replace parameters
 *      replace_info_mask - replace parameter mask
 * Returns:
 *      BCM_E_NONE - Success.
 *      BCM_E_XXX
 */
int
bcm_esw_vlan_cross_connect_replace(int unit,
                   uint32 options,
...
#ifdef BCM_TRX_SUPPORT
    if (SOC_IS_TRX(unit)) {
        return bcmi_tr_l2_vlan_cross_connect_replace(unit, options,
              match_info, match_info_mask, replace_info, replace_info_mask);
    }
#endif
    return BCM_E_UNAVAIL;
}
                                         ...
                                         _bcm_flex_stat_handle_t *vxlt_fsh)
{
    ...
    /* This next step is a bit abusive, but we don't have an accessor
     * which captures the full key description in the VLAN_XLATE table. */
    _BCM_FLEX_STAT_HANDLE_COPY(*vxlt_fsh, vent[0]);
...
#ifdef BCM_TRX_SUPPORT
STATIC int
_bcm_esw_vlan_match_action_multi_get(int unit,
    bcm_vlan_match_t match, bcm_gport_t port, int size,
    bcm_vlan_match_info_t *match_info_array, bcm_vlan_action_set_t *action_set,
    int *count)
{
    int count1 = 0;
#if defined(BCM_TOMAHAWK_SUPPORT)
    ...
    BCM_IF_ERROR_RETURN(_bcm_trx_vlan_match_action_multi_get(unit, match,
            port, size, match_info_array, action_set, &count1));

#if defined(BCM_TOMAHAWK_SUPPORT)
    if (SOC_IS_TOMAHAWKX(unit)) {
        if (size == 0) {
            rv = _bcm_th_vlan_match_action_multi_get(unit, match, port, size,
                                                      NULL, NULL, &valid_count);
            BCM_IF_ERROR_RETURN(rv);

            *count = (count1 + valid_count);
            return BCM_E_NONE;
        }

        size1 = size - count1;
        if (size1 > 0) {
            rv = _bcm_th_vlan_match_action_multi_get(unit, match, port, size1,
                                                      &match_info_array[count1],
                                                      &action_set[count1],
                                                      &count2);
            BCM_IF_ERROR_RETURN(rv);
        }
...
#endif
        {
            return _bcm_esw_vlan_match_action_multi_get(unit, match,
                    BCM_GPORT_INVALID, size, match_info_array,
                    action_set, count);
        }
    }
        ...
        for (i = 0, ctr = start;
            ...
            switch(vlan_match_array[i].match) {
                ...
                case BCM_VLAN_MATCH_MAC_PORT_CLASS:
                    ...
                    match_array->port_class = vlan_match_array[i].port_class;
                    sal_memcpy(match_array->src_mac,
                            vlan_match_array[i].src_mac, sizeof (bcm_mac_t));
                    if (match_array->action != NULL) {
                        sal_memcpy(match_array->action, &action_set[i],
