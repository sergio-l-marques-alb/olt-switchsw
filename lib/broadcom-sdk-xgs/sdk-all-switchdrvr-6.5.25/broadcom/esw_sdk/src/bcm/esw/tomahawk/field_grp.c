                              ...
                              uint16 *qual_info_count)
{
    ...
    for (qual_idx = 0; qual_idx < *qual_info_count; qual_idx++) {
        qual_id = (_bcm_field_internal_qualify_t)((*qual_info_arr)[qual_idx].qual_id);
        if (NULL != f_ht) {
            if (NULL != f_ht->hints) {
...
_field_th_ingress_ext_code_get(int unit, _field_group_add_fsm_t *fsm_ptr)
{
    ...
    bcmi_keygen_qual_info_t *qual_info_arr_prev = NULL;
    uint16 qual_info_count_prev = 0;
    _field_group_t *fg;
    _field_stage_t *stage_fc;
    bcm_field_qset_t *qset_req;
#if defined (BCM_TRIDENT3_SUPPORT)
    ...
    if ((fsm_ptr->flags & _FP_GROUP_QSET_UPDATE) && (fsm_ptr->qset_prev)) {
    
        rv = _field_th_group_qual_info_get(unit, stage_fc, fg,
                                           fsm_ptr->qset_prev,
                                           &qual_info_arr_prev,
                                           &qual_info_count_prev);
        if (BCM_FAILURE(rv)) {
            goto cleanup;
        }
    }
    ...
    if ((fsm_ptr->flags & _FP_GROUP_QSET_UPDATE) && (fsm_ptr->qset_prev)) {
        keygen_cfg.prev_qual_info_arr = qual_info_arr_prev;
        keygen_cfg.prev_qual_info_count = qual_info_count_prev;
...
cleanup:
    ...
    if (qual_info_arr_prev != NULL) {
        sal_free(qual_info_arr_prev);
    }
    if (fsm_ptr->flags & _FP_GROUP_QSET_UPDATE) {
                                      ...
                                      _field_group_t **next_group)
{
    ...
    *next_group = NULL;
    for (; fg != NULL; fg = fg->next) {
        ...
        if ((instance != -1) && (instance != fg->instance)) {
            continue;
        }

        if ((priority == -1) || (fg->priority == priority)) {
            *next_group = fg;
            return BCM_E_NONE;
        }
    }
...
{
    int   rv;            /* Operation return status.    */
    int   p, idx;           /* iteration index.            */
    int parts_count = 0;
    _field_control_t *fc; /* Field Control Structure.   */
    _field_lt_slice_t     *lt_fs_p = NULL;
    int   lt_part_prio = lt_pri_part_prio;
    uint8 is_shared_slice = 0;
    _field_group_t *groups = NULL;
    ...
    if (fg->flags & _FP_GROUP_PRESELECTOR_SUPPORT) {
       ...
       if (lt_fs->next) {
           ...
           is_shared_slice = 0;
           for (groups = fc->groups; groups; groups = groups->next) {
               if (fg->priority == groups->priority) {
                   is_shared_slice = 1;
                   break;
               }
           }
           ...
           for (p = 0; p < parts_count; p++) {
               ...
               for (lt_fs_p = lt_fs->next; lt_fs_p != NULL; lt_fs_p = lt_fs_p->next) {
                   /* LT part prio update */
                   rv = _field_th_ingress_group_expand_slice_install(unit, stage_fc,
                           fg, lt_fs_p->slice_number + p, p, lt_part_prio,
                           is_shared_slice);
                   BCM_IF_ERROR_RETURN(rv);
               }
           }
...
STATIC int
_field_group_used_lt_bmp_get(
    int unit,
    _field_stage_t *stage_fc,
    int instance,
    int priority,
    uint32 *used_ltid_bmp)
{
    int rv;
    uint32 lt_id_bmp = 0;
    _field_group_t *fg_ptr = NULL;
    ...
    *used_ltid_bmp = 0;
    ...
    do {
        rv = _bcm_field_th_priority_group_get_next(unit, instance,
                stage_fc->stage_id,
                priority,
                &fg_ptr, &fg_ptr);
        if (BCM_SUCCESS(rv)) {
                                 ...
                                 uint32 *unused_ltids)
{
    ...
    _field_stage_t *other_stage_fc = NULL;
    ...
    if ((other_stage_fc != NULL) &&
        (stage_fc->oper_mode != other_stage_fc->oper_mode)) {
        int inst = 0;
        uint32 lt_id_bmp = 0;

        /* Retrieve LT IDs allocated to the given pipe stage */
        rv = _field_group_used_lt_bmp_get(unit, stage_fc, fg->instance,
                                          -1, &lt_id_bmp);
        if (BCM_FAILURE(rv)) {
            return BCM_E_INTERNAL;
        }
        ...
        used_ltids = lt_id_bmp;
        rv = _field_group_used_lt_bmp_get(unit, other_stage_fc, inst,
                                          -1, &lt_id_bmp);
        if (BCM_FAILURE(rv)) {
            return BCM_E_INTERNAL;
        }
                            ...
                            _field_group_t *fg, int lt_id)
{
    ...
    if ((other_stage_fc != NULL) &&
        ...
        if (other_stage_fc->oper_mode == bcmFieldGroupOperModePipeLocal) {
            valid_inst = -1;
        } else {
            valid_inst = 0;
        }
        ...
        for (inst = 0; inst < NUM_PIPE(unit); inst++) {
            ...
            if (BCM_PBMP_IS_NULL(PBMP_PIPE(unit, inst))) {
                continue;
            }
#endif
            if ((valid_inst != -1) && (valid_inst != inst)) {
                continue;
            }

            if (valid_inst == fg->instance) {
                break;
            }
                        ...
                        bcm_field_group_config_t *gc)
{
    ...
    if (gc->flags & BCM_FIELD_GROUP_CREATE_SLICE_SHARE_ENABLE) {
        fsm.flags |= _BCM_FP_GROUP_SLICE_SHARE_ENABLE;
    }
...
_bcm_field_th_group_update(int unit, bcm_field_group_t group, bcm_field_qset_t *qset)
{
    ...
    for (part = 0; part < _FP_MAX_ENTRY_WIDTH; part++)  {
        ...
        if (size != 0) {
             ...
             _FP_XGS3_ALLOC(qid_array[part], size * sizeof(uint32), "Qid Array");
                                               ...
                                               _field_lt_slice_t *lt_fs)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_FP, (BSL_META_U(unit,
        ...
        __func__, (void *) fg, fg->gid, (void *) lt_fs, lt_fs->slice_number,
        entry_part));
    ...
    LOG_DEBUG(BSL_LS_BCM_FP, (BSL_META_U(unit, "default_group_expand_install():"
     ...
     (void *) lt_f_entry, lt_f_entry->eid,
     (void *) lt_fs, lt_fs->slice_number, lt_f_entry->index));
    LOG_DEBUG(BSL_LS_BCM_FP, (BSL_META_U(unit,
               "-----------------------------------------------------\n\r")));
                                       ...
                                       uint8 is_shared_slice)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_FP, (BSL_META_U(unit,
        ...
        __func__, (void *) fg, fg->gid,
        (void *) lt_fs, lt_fs->slice_number, entry_part));
    ...
    for (idx = 0; idx < _FP_PRESEL_ENTRIES_MAX_PER_GROUP; idx++) {
        ...
        lt_part_fs = f_presel_p->lt_fs;
        /* For slice sharing groups, all slices needs t be updated. */
        if (is_shared_slice) {
            /* Decrement slice free entry count for primary entries. */
            lt_fs->free_count--;
            /* Assign entry to a slice. */
            lt_fs->p_entries[f_presel_p->hw_index] = f_presel_p;
        } else {
            for (; lt_part_fs != NULL; lt_part_fs = lt_part_fs->next) {
                /* Find the tale of the list in order not to decrement again and again */
                if (lt_part_fs->next == NULL) {
                    /* Decrement slice free entry count for primary entries. */
                    lt_part_fs->free_count--;
                    /* Assign entry to a slice. */
        ...
        LOG_DEBUG(BSL_LS_BCM_FP, (BSL_META_U(unit, "group_expand_install():"
              ...
              (void *) f_presel_p,f_presel_p->presel_id,(void *) lt_fs,lt_fs->slice_number,
              f_presel_p->hw_index));
        LOG_DEBUG(BSL_LS_BCM_FP, (BSL_META_U(unit,
               "-----------------------------------------------------\n\r")));
                                         ...
                                         uint8 is_shared_slice)
{
    ...
    if (fg->flags & _FP_GROUP_PRESELECTOR_SUPPORT) {
       rv = _field_th_ingress_group_expand_install(unit, stage_fc, fg,
                                                   part_index, lt_fs, is_shared_slice);
    } else {
       /* Default case. */
