...
/* NONUCAST Trunk Block Mask buffer pointer */
extern nonucast_trunk_block_mask_entry_t *block_mask_tbl[BCM_MAX_NUM_UNITS];
 ...
 *      bm_info - (IN) New nonucast local port info array
        ...
        uint32 flags)
{
    ...
    nonucast_trunk_block_mask_entry_t *mask_table = block_mask_tbl[unit];
    int        region_size, region, table_size, table_max;
    _esw_trunk_bm_info_t *p;
    ...
    table_size = soc_mem_index_count(unit, mem);
    table_max = table_size - 1;
    region_size = table_size / 4;

    if (!(flags & BCM_TRUNK_FLAG_MULTI_SET) || mask_table == NULL) {
        mask_table = soc_cm_salloc(unit,
                                   sizeof(nonucast_trunk_block_mask_entry_t) * table_size,
        ...
        if ((rv = soc_mem_read_range(unit, mem, MEM_BLOCK_ANY,
                                     0, table_max, mask_table)) < 0) {
            soc_mem_unlock(unit, mem);
            soc_cm_sfree(unit, mask_table);
            mask_table = NULL;
            return rv;
        }
        block_mask_tbl[unit] = mask_table;
    }

    for (region = 0; region < TRUNK_BLOCK_MASK_TYPE_COUNT; region++) {
        p = &bm_info[region];
        index_count = 0;
        imin = region_size * region;
        ...
        for (i = imin; i <= imax; i++) {
            ...
            if (region == TRUNK_BLOCK_MASK_TYPE_IPMC &&
                ...
                if (p->localport_count > 0) {
                     ...
                     */
                    if (SHR_BITGET(p->my_modid_bmap,
                                   ((i % region_size) % p->member_count))) {
                        SOC_PBMP_PORT_REMOVE(
                            pbmp, p->localport_array[index_count % p->localport_count]);
                        index_count++;
                    }
                }
            }
    ...
    if (!(flags & BCM_TRUNK_FLAG_MULTI_SET) && mask_table) {
        rv = soc_mem_write_range(unit, mem, MEM_BLOCK_ANY, 0, table_max, mask_table);
        soc_mem_unlock(unit, mem);
        soc_cm_sfree(unit, mask_table);
        block_mask_tbl[unit] = NULL;
    }
        ...
        trunk_private_t *t_info)
{
    ...
    for (i = 0; i < add_info->num_ports; i++) {
        ...
        SOC_PBMP_PORT_ADD(new_trunk_pbmp, port);
        if (SHR_BITGET(ipmc_tpbm, i)) {
            /* Eligible for IPMC forwarding */
            SHR_BITSET(ipmc_my_modid_bmap, ipmc_localport_count);
            ipmc_localport_array[ipmc_localport_count++] = port;
        }
        if (SHR_BITGET(l2mc_tpbm, i)) {
            /* Eligible for L2MC forwarding */
            SHR_BITSET(l2mc_my_modid_bmap, l2mc_localport_count);
            l2mc_localport_array[l2mc_localport_count++] = port;
        }
        if (SHR_BITGET(bcast_tpbm, i)) {
            /* Eligible for broadcast forwarding */
            SHR_BITSET(bcast_my_modid_bmap, bcast_localport_count);
            bcast_localport_array[bcast_localport_count++] = port;
        }
        if (SHR_BITGET(dlf_tpbm, i)) {
            /* Eligible for DLF forwarding */
            SHR_BITSET(dlf_my_modid_bmap, dlf_localport_count);
            dlf_localport_array[dlf_localport_count++] = port;
        }
    }
    ...
    if (SOC_PBMP_NOT_NULL(new_trunk_pbmp) ||
        ...
        _esw_trunk_bm_info_t bm_info[TRUNK_BLOCK_MASK_TYPE_COUNT];

        bm_info[TRUNK_BLOCK_MASK_TYPE_IPMC].localport_array = ipmc_localport_array;
        bm_info[TRUNK_BLOCK_MASK_TYPE_IPMC].localport_count = ipmc_localport_count;
        bm_info[TRUNK_BLOCK_MASK_TYPE_IPMC].my_modid_bmap = ipmc_my_modid_bmap;
        bm_info[TRUNK_BLOCK_MASK_TYPE_IPMC].member_count = ipmc_localport_count;

        bm_info[TRUNK_BLOCK_MASK_TYPE_L2MC].localport_array = l2mc_localport_array;
        bm_info[TRUNK_BLOCK_MASK_TYPE_L2MC].localport_count = l2mc_localport_count;
        bm_info[TRUNK_BLOCK_MASK_TYPE_L2MC].my_modid_bmap = l2mc_my_modid_bmap;
        bm_info[TRUNK_BLOCK_MASK_TYPE_L2MC].member_count = l2mc_localport_count;

        bm_info[TRUNK_BLOCK_MASK_TYPE_BCAST].localport_array = bcast_localport_array;
        bm_info[TRUNK_BLOCK_MASK_TYPE_BCAST].localport_count = bcast_localport_count;
        bm_info[TRUNK_BLOCK_MASK_TYPE_BCAST].my_modid_bmap = bcast_my_modid_bmap;
        bm_info[TRUNK_BLOCK_MASK_TYPE_BCAST].member_count = bcast_localport_count;

        bm_info[TRUNK_BLOCK_MASK_TYPE_DLF].localport_array = dlf_localport_array;
        bm_info[TRUNK_BLOCK_MASK_TYPE_DLF].localport_count = dlf_localport_count;
        bm_info[TRUNK_BLOCK_MASK_TYPE_DLF].my_modid_bmap = dlf_my_modid_bmap;
        bm_info[TRUNK_BLOCK_MASK_TYPE_DLF].member_count = dlf_localport_count;
                       ...
                       trunk_private_t *t_info)
{
    ...
    if (SOC_PBMP_NOT_NULL(old_ports)) {
        ...
        SOC_PBMP_CLEAR(new_ports);
        sal_memset(bm_info, 0, sizeof(bm_info));
 ...
 */
void
_bcm_th3_trunk_sw_dump(int unit)
{
    int rv;
    int i, j;
    int num_ports;
    ...
    if (NULL != _th3_trunk_swfail[unit]) {
        swf = _th3_trunk_swfail[unit];
        num_ports = sizeof(swf->trunk)/sizeof(swf->trunk[0]);
    ...
    if (soc_feature(unit, soc_feature_port_lag_failover)) {
        ...
        if (NULL != _th3_trunk_hwfail[unit]) {
            LOG_CLI((BSL_META_U(unit,
                            "   Trunk HW failover parameters\n")));
            for (i = tid_min; i <= tid_max; i++) {
                hwft = &(_th3_trunk_hwfail[unit]->hw_tinfo[i]);
    ...
    if (NULL != _th3_trunk_member_info[unit]) {
        LOG_CLI((BSL_META_U(unit,
                            "Trunk membership info:\n")));
        for (i = tid_min; i <= tid_max; i++) {
            if (0 != MEMBER_INFO(unit, i).num_ports) {
                LOG_CLI((BSL_META_U(unit,
                                    "   Trunk %d\n"), i));
                LOG_CLI((BSL_META_U(unit,
                                    "      Port count %d\n"),
                         MEMBER_INFO(unit, i).num_ports));
                for (j = 0; j < MEMBER_INFO(unit, i).num_ports; j++) {
                    LOG_CLI((BSL_META_U(unit,
                                        "      Port 0x%04x\n"),
                             MEMBER_INFO(unit, i).port[j]));
                    LOG_CLI((BSL_META_U(unit,
                                        "      Member flags 0x%x\n"),
                             MEMBER_INFO(unit, i).member_flags[j]));
                }
            }
        }
    } else {
