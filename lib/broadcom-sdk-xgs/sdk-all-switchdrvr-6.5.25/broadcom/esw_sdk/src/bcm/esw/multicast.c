...

{
    ...
    for (i = 0; i < port_count ; i++) {
        if (encap_array[i] == BCM_IF_INVALID) {
            /* Skip encap array if it is invalid.
            This logic handles L3_IPMC l3 bitmap modification */
            continue;
        }
                ...
                for (idx = 0; idx < trunk_local_ports; idx++) {
                    ...
                    if ((soc_feature(unit, soc_feature_channelized_switching)) &&
                        ...
                        if (BCMI_MC_GROUP_PORT_REFCOUNT(unit, mc_index,
                            trunk_local_port) && (BCM_IF_INVALID != encap_id)) {
                    ...
                    if (soc_feature(unit, soc_feature_channelized_switching)) {
                        ...
                        if (BCMI_MC_GROUP_PORT_REFCOUNT(unit, mc_index,
                            port_out) && (BCM_IF_INVALID != encap_id)) {
                            ((BCMI_MC_GROUP_PORT_REFCOUNT(unit, mc_index,
                                port_out))--);
                            if (BCMI_MC_GROUP_PORT_REFCOUNT(unit, mc_index,
                                port_out) > 0) {
    ...
    if (_BCM_MULTICAST_L2_OR_FABRIC(unit, group)) {
        ...
        if (0 == port_count) {
            ...
            if (SOC_IS_HURRICANE4(unit) &&
                    soc_feature(unit, soc_feature_xflow_macsec_svtag)) {
                /* MCASEC ports are valid for replication on HR4 */
                BCM_PBMP_OR(mc_eligible_pbmp, PBMP_MACSEC_ALL(unit));
            }
#endif
...
/*
 * Function:
 *      _bcm_esw_multicast_remap_xflow_macsec_set
 * Purpose:
 *      Set xflow_macsec IPMC remapping mode.
 * Parameters:
 *      unit      - (IN) Unit number.
 *      from      - (IN) Multicast group ID to map from
 ...
 */
STATIC int
_bcm_esw_multicast_remap_xflow_macsec_set(int unit, bcm_multicast_t from, int enable)
{
    int rv = BCM_E_UNAVAIL;
    ...
    if (soc_feature(unit, soc_feature_ipmc_remap)) {
        ...
        rv = bcm_hr4_multicast_remap_xflow_macsec_set(unit, from, enable);
    }
#endif
...
/*
 * Function:
 *      _bcm_esw_multicast_remap_xflow_macsec_get
 * Purpose:
 *      Get xflow_macsec IPMC remapping mode.
 * Parameters:
 *      unit      - (IN) Unit number.
 *      from      - (IN) Multicast group ID being mapped
 ...
 */
STATIC int
_bcm_esw_multicast_remap_xflow_macsec_get(int unit, bcm_multicast_t from, int *enable)
{
    int rv = BCM_E_UNAVAIL;
    ...
    if (soc_feature(unit, soc_feature_ipmc_remap)) {
        ...
        rv = _bcm_esw_multicast_l3_group_check(unit, from, NULL);
        if (BCM_SUCCESS(rv)) {
            rv = bcm_hr4_multicast_remap_xflow_macsec_get(unit, from, enable);
        }
    }
#endif
...
#if defined(INCLUDE_L3)
#if defined(BCM_HURRICANE4_SUPPORT) && defined(INCLUDE_XFLOW_MACSEC)
        case bcmMulticastRemapXflowMacsec:
            rv = _bcm_esw_multicast_remap_xflow_macsec_set(unit, group, arg);
            break;
#endif
#endif
        default:
...
#if defined(BCM_HURRICANE4_SUPPORT) && defined(INCLUDE_XFLOW_MACSEC)
        case bcmMulticastRemapXflowMacsec:
            rv = _bcm_esw_multicast_remap_xflow_macsec_get(unit, group, arg);
            break;
#endif
#endif
