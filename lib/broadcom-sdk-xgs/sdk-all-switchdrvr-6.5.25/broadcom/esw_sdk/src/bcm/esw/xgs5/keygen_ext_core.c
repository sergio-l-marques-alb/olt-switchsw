...
STATIC int
bcmi_keygen_ext_cfg_reserve_for_alignment(int unit,
                                 bcmi_keygen_md_t *keygen_md,
                                 uint8 gran,
                                 bcmi_keygen_ext_cfg_t *in_ext_cfg_l1,
                                 bcmi_keygen_ext_cfg_t *in_ext_cfg_l2)
{
    BCMI_KEYGEN_FUNC_ENTER(unit);

    BCMI_KEYGEN_NULL_CHECK(in_ext_cfg_l1, BCM_E_PARAM);
    BCMI_KEYGEN_NULL_CHECK(in_ext_cfg_l2, BCM_E_PARAM);

    in_ext_cfg_l1->in_use = TRUE;
    SHR_BITSET(in_ext_cfg_l1->ext_attrs.w,
                BCMI_KEYGEN_EXT_ATTR_RESERVED_FOR_ALIGNMENT);

    /* L2 is multigran, */
    in_ext_cfg_l2->in_use = TRUE;
    SHR_BITSET(in_ext_cfg_l2->ext_attrs.w,
                BCMI_KEYGEN_EXT_ATTR_RESERVED_FOR_ALIGNMENT);

    /* Set extractor to requested gran */
    BCMI_KEYGEN_EXT_ID_GRAN_SET(in_ext_cfg_l2->ext_id, gran);
                                ...
                                bcmi_keygen_ext_cfg_t *in_ext_cfg)
{
    ...
    bcmi_keygen_ext_cfg_t *ext_cfg = NULL;
    bcmi_keygen_ext_cfg_t *ext_cfg_temp = NULL;
    bcmi_keygen_ext_cfg_t *last_free_ext_cfg_l1 = NULL;
    bcmi_keygen_ext_cfg_t *last_free_ext_cfg_l2 = NULL;
    bcmi_keygen_ext_field_info_t *finfo_temp = NULL;
    ...
    for (ext_idx = 0, total_offset = 0;
        ...
        if (ext_cfg->in_sec != in_ext_cfg->out_sec) {
            ...
            /* Find coresponding ext_cfg at previous level */
            for (ext_temp_idx = 0;
                 ext_temp_idx < keygen_md->ext_cfg_db->conf_size[level];
                 ext_temp_idx++) {
                ext_cfg_temp =  keygen_md->ext_cfg_db->ext_cfg[level] +
                        ext_temp_idx;

                if (ext_cfg_temp->out_sec == ext_cfg->in_sec) {
                    break;
                }
            }
            /* This should not happen. Precautionary check */
            if (ext_temp_idx == keygen_md->ext_cfg_db->conf_size[level]) {
                BCMI_KEYGEN_RETURN_VAL_EXIT(BCM_E_INTERNAL);
            }

            gran_temp = 0;
            /* Calculate offset assuming only 16b/8b extractors are used */
            if (ext_cfg_temp->in_use == 1) {
                finfo_temp = (bcmi_keygen_ext_field_info_t *) ext_cfg_temp->finfo;

                req_bmp = finfo_temp->req_bmp[0];
                found = FALSE;
                if (finfo_temp->flags & BCMI_KEYGEN_EXT_FIELD_USE_L2_GRAN8) {
                    if (((req_bmp & (~0xFF)) == 0) ||
                            ((req_bmp & (~0xFF00)) == 0)) {
                        gran_temp = 8;
                        found = TRUE;
                ...
                /* It will use 16b gran extractor */
                if (found == FALSE) {
                    gran_temp = 16;
                }
                last_free_ext_cfg_l1 = NULL;
                last_free_ext_cfg_l2 = NULL;
            } else {
                if (SHR_BITGET(ext_cfg->ext_attrs.w,
                            BCMI_KEYGEN_EXT_ATTR_MULTI_GRAN_16)) {
                    gran_temp = 16;
                }
                last_free_ext_cfg_l1 = ext_cfg_temp;
                last_free_ext_cfg_l2 = ext_cfg;
            }
            total_offset += gran_temp;
            continue;
        }
        if ((total_offset % 32) != 0) {
            if (last_free_ext_cfg_l1 != NULL) {
                /* Check if we can use gran=8 to make it modulo 32 */
                if (((total_offset - 8) % 16) == 0) {
                    /* Lets mark previous ext to gran=8 */
                    BCMI_KEYGEN_IF_ERR_EXIT
                        (bcmi_keygen_ext_cfg_reserve_for_alignment(unit,
                                                        keygen_md, 8,
                                                        last_free_ext_cfg_l1,
                                                        last_free_ext_cfg_l2));
                }
                /* Now, this extractor can be used */
                break;
            }
