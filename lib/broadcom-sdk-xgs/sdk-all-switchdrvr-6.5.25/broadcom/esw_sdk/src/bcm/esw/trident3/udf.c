...
soc_field_t src_pkt_offset_cmds[11] = {
                                        ...
                                        CMD_C_1_SRC_PKT_OFFSETf,
                                        CMD_C_2_SRC_PKT_OFFSETf,
                                        CMD_C_3_SRC_PKT_OFFSETf
                                     };
soc_field_t dest_cont_id_cmds[11] = {
                                      ...
                                      CMD_C_1_DEST_CONT_IDf,
                                      CMD_C_2_DEST_CONT_IDf,
                                      CMD_C_3_DEST_CONT_IDf
                                   };
soc_field_t valid_cmds[11] = {
                               CMD_C_4_VALIDf,
                               CMD_C_5_VALIDf,
                               CMD_C_6_VALIDf,
                               CMD_C_7_VALIDf,
...
_bcm_udf_abstr_pkt_format_info_init(int unit)
{
    ...
    _BCM_UDF_ABSTRACT_PKT_FORMAT_CFG_INIT(unit, bcmUdfAbstractPktFormatHiGig3, 3);
    ...
    _BCM_UDF_ABSTRACT_PKT_FORMAT_CFG_INIT(unit, bcmUdfAbstractPktFormatIpv6WithSRv6, 1);
    _BCM_UDF_ABSTRACT_PKT_FORMAT_CFG_INIT(unit, bcmUdfAbstractPktFormatKnownL3Ipv6SRH, 1);
    _BCM_UDF_ABSTRACT_PKT_FORMAT_CFG_INIT(unit, bcmUdfAbstractPktFormatL2InIpv6, 1);
    ...
    _udf_obj_pkt_fmt_hw_cfg_t *cfg_data)
{
   ...
   uint8 start_cmd = 0;
   uint8 max_cmds = 0;
   uint8 num_cmds_required = 0;
   bcm_udf_abstract_pkt_format_t pkt_fmt = bcmUdfAbstractPktFormatLastCount;
   ...
   for (stage = 0; stage < hw_info->num_stages; stage++) {
      if (hw_info->stage_policy_mem[stage] != INVALIDm) {
          ...
          if (hw_info->num_cmds[stage] > _BCM_UDF_MAX_POLICY_CMDS) {
              max_cmds = hw_info->num_cmds[stage];
              start_cmd = 0;
          } else {
              max_cmds = _BCM_UDF_MAX_POLICY_CMDS;
              start_cmd = _BCM_UDF_MAX_POLICY_CMDS - hw_info->num_cmds[stage];
          }
          for (i = start_cmd; i < max_cmds ; i++) {
    ...
    _udf_obj_pkt_fmt_hw_cfg_t *cfg_data)
{
   ...
   for (stage = 0; stage < hw_info->num_stages; stage++) {
        ...
        */
       if (hw_info->parser == _BCM_UDF_PARSER0) {
           if (bcmUdfAbstractPktFormatHiGig3 == pkt_fmt) {
               /* Uses CMD9 only. */
               start_cmd = _BCM_UDF_MAX_POLICY_CMDS - 1;
           } else {
...
STATIC
int _bcm_udf_td3_abstr_pkt_format_info_offset_max_get(
        int unit,
        _bcm_udf_td3_abstr_pkt_fmt_hw_info_t *hw_info,
        uint32 *offset_max)
{
    if ((NULL == hw_info) || (NULL == offset_max)){
       return BCM_E_PARAM;
    }

    if (_BCM_UDF_PARSER0 == hw_info->parser) {
        /* provide data in bits for P0 as Parser 0 in takes bits format */
        *offset_max = 64 * 8;
    } else if (_BCM_UDF_PARSER1 == hw_info->parser) {
        *offset_max = (SOC_IS_HURRICANE4(unit) || SOC_IS_FIREBOLT6(unit)) ? 192 : 144;
    } else {
        *offset_max = 0;
    }
    return BCM_E_NONE;
    ...
    bcm_udf_abstract_pkt_format_info_t *pkt_format_info)
{
    ...
    BCM_IF_ERROR_RETURN
        (_bcm_udf_td3_abstr_pkt_format_info_offset_max_get(unit,
                                                           pkt_fmt_hw_info,
                                                           &(pkt_format_info->offset_max)));

    return BCM_E_NONE;
}

STATIC
_flex_ceh_field_str_t flex_udf_tunnel_type_arr[bcmFieldTunnelTypeCount] = {
            {"GENEVE_OUTER_IPV4"},
            {"GENEVE_OUTER_IPV6"},
            {"GPE_OUTER_IPV4"},
            {"GPE_OUTER_IPV6"},
            {"IPINIPGRE_WITH_OUTER_IPV4"},
            {"IPINIPGRE_WITH_OUTER_IPV6"},
            {"IPINIP_WITH_OUTER_IPV4"},
            {"IPINIP_WITH_OUTER_IPV6"},
            {"L2GRE"},
            {"MIM"},
            {"MPLS"},
            {"MPLS_OVER_GRE"},
...
STATIC
int _bcm_udf_td3_tunnel_type_offset_max_get(
    int unit,
    ...
    uint32 *offset_max)
{
    ...
    if ((NULL == offset_max) || (tunnel_type >= bcmUdfTunnelTypeLastCount)) {
        return BCM_E_PARAM;
    }

    if (sal_strcmp(flex_udf_tunnel_type_arr[tunnel_type].field_str, "\0") != 0) {

        rv = (soc_cancun_ceh_obj_field_get(unit, "PARSER_2_LIMIT",
                               flex_udf_tunnel_type_arr[tunnel_type].field_str,
                               &ceh_info));
        if (SOC_FAILURE(rv)) {
           LOG_ERROR(BSL_LS_BCM_FP,
                   (BSL_META("tunnel_type:%d obj_str:%s field_str:%s\n\r"),
                    tunnel_type, "PARSER_2_LIMIT",
                    flex_udf_tunnel_type_arr[tunnel_type].field_str));
           /*
        ...
        *offset_max = ceh_info.value;

        LOG_DEBUG(BSL_LS_BCM_FP,
                (BSL_META_U(unit, "Type:%d - offset:%d width:%d value:%d \n\r"),
                 tunnel_type, ceh_info.offset, ceh_info.width, ceh_info.value));
...
STATIC
_flex_ceh_field_str_t flex_udf_tunnel_term_type_arr[bcmUdfTunnelTermFlowTypeLastCount] = {
...
/* Tunnel Flow type to CEH string mapping */
STATIC
_flex_ceh_field_str_t flex_udf_tunnel_flow_type_arr[bcmUdfTunnelFlowTypeLastCount] = {
            {"MIM_TERM_PYLD_TYPE_IP"},
            {"MIM_TERM_PYLD_TYPE_NON_IP"},
            {"L2_MPLS_TERM_PYLD_TYPE_IP"},
            {"L2_MPLS_TERM_PYLD_TYPE_NON_IP"},
            ...
            {"VXLAN_TRANSIT"},
            {"L2GRE_TRANSIT"},
            {"SRV6_L2_TUNNEL_TERM_PYLD_TYPE_IP"},
            {"SRV6_L2_TUNNEL_TERM_PYLD_TYPE_NON_IP"},
            {"SRV6_L3_TUNNEL_TERM"},
            {"SRV6_TRANSIT"},
            {"L2_IN_IPV6_TRANSIT"},
            {"L2_IN_IPV6_TERM_PYLD_TYPE_NON_IP"},
            {"L2_IN_IPV6_TERM_PYLD_TYPE_IP"},
};
...
_bcm_udf_td3_field_bus_merge_profile_get(int unit,
                                     bcm_udf_tunnel_term_flow_type_t term_flow_type,
                                     bcm_udf_tunnel_flow_type_t tnl_flow_type,
                                     uint32 *chunk_bmap)
{
    int rv;
    uint32 fmt_buf = 0;
    ...
    if (term_flow_type != bcmUdfTunnelTermFlowTypeLastCount) {
        field_name = flex_udf_tunnel_term_type_arr[term_flow_type].field_str;
        flow_type = term_flow_type;
    } else if (tnl_flow_type != bcmUdfTunnelFlowTypeLastCount){
        field_name = flex_udf_tunnel_flow_type_arr[tnl_flow_type].field_str;
        flow_type = tnl_flow_type;
    } else {
        return BCM_E_INTERNAL;
    }
...
STATIC
int _bcm_udf_tunnel_flow_based_chunk_arrange_set(
        int unit,
        bcm_udf_tunnel_flow_type_t flow_type,
        uint32 chunk_bmap)
{
    if (flow_type >= bcmUdfTunnelFlowTypeLastCount) {
        return BCM_E_PARAM;
    }
    ...
    return _bcm_udf_td3_field_bus_merge_profile_set(unit,
                                  bcmUdfTunnelTermFlowTypeLastCount,
                                  flow_type,
                                  chunk_bmap);
}

STATIC
int _bcm_udf_tunnel_flow_based_chunk_arrange_get(
        int unit,
        bcm_udf_tunnel_flow_type_t flow_type,
        uint32 *chunk_bmap)
{
...
_bcm_udf_td3_functions_init(_bcm_udf_funct_t *functions)
{
    ...
    functions->udf_tunnel_flow_based_chunk_arrange_set =
                                     _bcm_udf_tunnel_flow_based_chunk_arrange_set;
    functions->udf_tunnel_flow_based_chunk_arrange_get =
                                     _bcm_udf_tunnel_flow_based_chunk_arrange_get;
    ...
    functions->udf_range_checker_chunk_info_get =
                                         _bcm_udf_td3_range_checker_chunk_info_get;
    functions->udf_tunnel_type_offset_max_get =
                                         _bcm_udf_td3_tunnel_type_offset_max_get;
