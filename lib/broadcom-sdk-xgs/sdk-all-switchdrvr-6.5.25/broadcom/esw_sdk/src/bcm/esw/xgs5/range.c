...
bcmi_xgs5_range_ctrl_free(int unit, bcmi_xgs5_range_ctrl_t *rangec)
{
    ...
    _range_t    *next = NULL;    /* Ptr to next range being traversed */
    ...
    /* Free all ranges before destroying, range control */
    for (cur = RANGE_CTRL(unit)->ranges; cur != NULL; cur = next) {
        /* Cur has been destroyed. So Cur's next cant be accessed. So store it prior. */
        next = cur->next;
        /* Destroy current range ID */
        BCM_IF_ERROR_RETURN
            (bcmi_xgs5_range_destroy(unit, cur->rid));
    }
...
STATIC int
bcmi_xgs5_range_wb_scache_size_get_2_0(int unit, int *req_scache_size)
{
    int         alloc_size = 0;     /* Allocation size */
    _range_t    range;              /* Range structure */
    ...
    alloc_size += RANGE_CTRL(unit)->max_ranges * sizeof(range.rid);
    /*
     * Assumption that HW index will never exceed 64K depth in any chip. 
     * So HW Index can be fitted in 16 bits.
     */
    alloc_size += RANGE_CTRL(unit)->max_ranges * sizeof(uint16);
    alloc_size += RANGE_CTRL(unit)->max_ranges * sizeof(range.offset);
    alloc_size += RANGE_CTRL(unit)->max_ranges * sizeof(range.width);
    alloc_size += RANGE_CTRL(unit)->max_ranges * sizeof(range.udf_id);
    alloc_size += RANGE_CTRL(unit)->max_ranges * sizeof(range.pipe_instance);

    alloc_size += sizeof(range_ctrl->range_group_used_bitmap);
    alloc_size += sizeof(range_ctrl->range_group_range_bmp);
 ...
 */
STATIC int
bcmi_xgs5_range_wb_scache_size_get(int unit,
                                   int *req_scache_size)
{
    if (BCM_WB_DEFAULT_VERSION >= BCM_WB_VERSION_2_0) {
        BCM_IF_ERROR_RETURN(
                bcmi_xgs5_range_wb_scache_size_get_2_0(unit, req_scache_size));
    } else {
        BCM_IF_ERROR_RETURN(
...
STATIC int bcmi_xgs5_range_wb_realloc(int unit)
{
    int         rv = BCM_E_NONE;    /* Operation return status. */
    int         req_scache_size = 0;    /* Required Scache Size.    */
    int         alloc_size = 0;    /* Required Scache Size.    */
    uint32      alloc_get = 0;
    uint8       *scache_ptr = NULL;        /* Scache ptr.              */
    soc_scache_handle_t scache_handle;  /* Scache handle.       */
    ...
    SOC_SCACHE_ALIGN_SIZE(req_scache_size);
    alloc_size = req_scache_size + SOC_WB_SCACHE_CONTROL_SIZE;

    rv = soc_scache_ptr_get(unit, scache_handle, &scache_ptr, &alloc_get);
    if (BCM_E_NOT_FOUND == rv) {
        /* May be Level1, Cant go further with Scache */
        return BCM_E_NONE;
    }
    BCM_IF_ERROR_RETURN(rv);
...
bcmi_xgs5_range_wb_reinit_1_0(int unit, uint8 **scache_ptr, uint32 *scache_pos)
{
    ...
    int             num_ranges = 0;
    int             tmp_pipe_instance = 0;
    soc_mem_t       pipe_mem[SOC_MAX_NUM_PIPES] = {0};
    ...
    if (RANGE_CTRL(unit)->range_used_by_module ==
        ...
        for (idx = index_min; idx <= index_max; idx ++) {
            ...
            /* Get it for temporary. Dont increment scache pos */
            tmp_pipe_instance = *scache_p;

            if (RANGE_CTRL(unit)->range_oper_mode == bcmRangeOperModePipeLocal) {
                if (tmp_pipe_instance < NUM_PIPE(unit)) {
                    mem = pipe_mem[tmp_pipe_instance];
                } else {
            ...
            if ((range_p = sal_alloc(sizeof (_range_t), "range")) == NULL) {
                return (BCM_E_MEMORY);
            }
            ...
            /*
             * Num ranges was not incremented in previous versions.
             * So sync of the same doesnt work. So increment it explicitly.
             */
            num_ranges += 1;
    ...
    /* Assign the number of ranges recovered in 1.0 recovery */
    if (((int8)(RANGE_CTRL(unit)->num_ranges)) <= 0) {
        RANGE_CTRL(unit)->num_ranges = num_ranges;
    }
    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(0,
                            "S$END:range\n")));
...
STATIC int
bcmi_xgs5_range_group_wb_reinit_2_0(int unit,
                                    uint8 **scache_ptr,
                                    uint32 *scache_pos)
{
    uint8       *scache_p = NULL;  /* Scache pointer */
    int         max_num_pipes = 0;
    int         temp_idx = 0;
    ...
    sal_memcpy(&(RANGE_CTRL(unit)->range_group_range_bmp),
               scache_p,
               sizeof(bcm_range_id_bmp_t) * _RANGE_GROUP_TOTAL);
    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(0,
            "S$BEGIN:range:{offset;length;range_group_range_bmp}\n")));
    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(0,
             "    {0x%x;%d;0x"), (*scache_pos),
             (int) (sizeof(bcm_range_id_bmp_t) * _RANGE_GROUP_TOTAL)));

    temp_idx = (int)(sizeof(bcm_range_id_bmp_t) * _RANGE_GROUP_TOTAL);
    while (temp_idx > 0 ) {
        LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(0,
             "%02x"), (unsigned char)(scache_p[temp_idx-1])));
        temp_idx--;
    ...
    (*scache_pos) += (sizeof(bcm_range_id_bmp_t) * _RANGE_GROUP_TOTAL);

    scache_p += sizeof(bcm_range_id_bmp_t) * _RANGE_GROUP_TOTAL;
...
STATIC int
bcmi_xgs5_range_wb_reinit_2_0(int unit, uint8 **scache_ptr, uint32 *scache_pos)
{
    uint8           *scache_p;          /* Pointer to scache */
    soc_mem_t       mem = RANGE_CTRL(unit)->range_mem;  /* Range Checker Memory ID */
    int             idx;                /* Range iteration index */
    ...
    uint32          type;               /* Range checker type. */
    uint32          tbl_entry[SOC_MAX_MEM_FIELD_WORDS];  /* Range checker entry buffer. */
    _range_t        *range_p = NULL;     /* Pointer to a range. */
    _range_t        *tmp = NULL;         /* Pointer to a range. */
    _range_t        **last_range_p = NULL; /* Pointer to the last range reinit. */
    bcm_port_config_t   pc;             /* Port configuration.      */
    ...
    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(0,
            "S$BEGIN:range:{offset;length;" \
            "pipe_instance;rid;hw_index;udf_id;offset;width}\n")));

    if (RANGE_CTRL(unit)->range_used_by_module == RANGE_USED_BY_RANGE_MODULE) {
        for (idx = 0; idx < RANGE_CTRL(unit)->num_ranges; idx++) {
            range_p = NULL;
            /* Create a new range entry for the list */
            if ((range_p = sal_alloc(sizeof (_range_t), "range")) == NULL) {
                rv = (BCM_E_MEMORY);
            ...
            range_p->ports = pc.all;
            if (RANGE_CTRL(unit)->range_oper_mode == bcmRangeOperModePipeLocal) {
                if (range_p->pipe_instance < NUM_PIPE(unit)) {
                    mem = pipe_mem[range_p->pipe_instance];
                } else {
                    sal_free(range_p);
                    return BCM_E_INTERNAL;
                }
            ...
            /* Get the hw_index the range maps to */
            sal_memcpy(&(range_p->hw_index), scache_p, sizeof(uint16));
            LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(0,
                            ";%u"),range_p->hw_index));
            scache_p += sizeof(uint16);
            (*scache_pos) += sizeof(uint16);
            ...
            rv = soc_mem_read(unit, mem, MEM_BLOCK_ANY, range_p->hw_index, tbl_entry);
            if (BCM_FAILURE(rv)) {
                goto exit;
            }
            ...
            soc_mem_field_get(unit, mem, tbl_entry, ENABLEf, &enable);
            if (enable == 0) {
                /* Something is wrong go and fetch the others*/
                LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(0,
                                "Range ID : %d, hw_idx : %d "
                                "reading failed\n"),
                                range_p->rid,range_p->hw_index));
                continue;
            }
...
exit:
    if (BCM_FAILURE(rv)) {
        /* Delete all ranges recovered and throw error. */
        tmp = range_p = *last_range_p;
        while(range_p) {
            tmp = tmp->next;
            sal_free(range_p);
            range_p = tmp;
...
STATIC
int bcmi_xgs5_range_group_range_bmp_get(
    int unit,
    uint32 range_group_config,
    bcm_range_id_bmp_t *range_bmp)
{
    ...
    pipe_instance = (range_group_id / RANGE_CTRL(unit)->max_groups);
    range_group_id = (range_group_id % RANGE_CTRL(unit)->max_groups);
    ...
    if (BCM_SUCCESS(rv)) {
        ...
        range_per_pipe = (RANGE_CTRL(unit)->max_ranges)/SOC_MAX_NUM_PIPES;

        for (idx = 0; idx < range_per_pipe; idx ++ ) {
            if (SHR_BITGET(range_hw_bmap, idx)) {
                for (tmp = RANGE_CTRL(unit)->ranges; tmp != NULL; tmp = tmp->next) {
                    if (idx == tmp->hw_index) {
                        SHR_BITSET(range_bmp->w, tmp->rid);
                    }
                }
            }
        }
...
bcmi_xgs5_range_wb_reinit_1_1(int unit, uint8 **scache_ptr, uint32 *scache_pos)
{
    ...
    *scache_ptr = scache_p;
    ...
    /* Populate range bmp per Range Group. */
    for (temp_idx = 0; temp_idx < (_RANGE_GROUP_TOTAL); temp_idx++) {
        if (SHR_BITGET(RANGE_CTRL(unit)->range_group_used_bitmap, temp_idx)) {
            BCM_IF_ERROR_RETURN
                (bcmi_xgs5_range_group_range_bmp_get(unit,
                             temp_idx,
                             &(RANGE_CTRL(unit)->range_group_range_bmp[temp_idx])));
        }
    }
...
STATIC int
bcmi_xgs5_range_group_wb_sync_2_0(int unit, uint8 **scache_ptr, uint32 *scache_pos)
{
    uint8       *scache_p;  /* Scache pointer */
    int      temp_idx = 0;
    ...
    sal_memcpy(scache_p,
               &(RANGE_CTRL(unit)->range_group_range_bmp),
               sizeof(RANGE_CTRL(unit)->range_group_range_bmp));
    
    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(0,
            "S$BEGIN:range:{offset;length;range_group_range_bmp}\n")));
    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(0,
             "    {0x%x;%d;0x"), (*scache_pos),
             (int) (sizeof(RANGE_CTRL(unit)->range_group_range_bmp))));
    temp_idx = (sizeof(bcm_range_id_bmp_t) * _RANGE_GROUP_TOTAL);
    while (temp_idx > 0 ) {
        LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(0,
             "%02x"), (unsigned char)(scache_p[temp_idx-1])));
        temp_idx--;
    ...
    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(0,
                            "S$END:range\n")));
    (*scache_pos) += sizeof(RANGE_CTRL(unit)->range_group_range_bmp);

    scache_p += sizeof(RANGE_CTRL(unit)->range_group_range_bmp);
...
STATIC int
bcmi_xgs5_range_wb_sync_2_0(int unit, uint8 **scache_ptr, uint32 *scache_pos)
{
    uint8       *scache_p;  /* Scache pointer */
    _range_t    *range_p;   /* Pointer to a range */
    ...
    *scache_p = RANGE_CTRL(unit)->range_used_by_module;
    ...
    while(range_p) {
        ...
        *scache_p = range_p->pipe_instance;
        ...
        sal_memcpy(scache_p, &(range_p->hw_index), sizeof(uint16));
        LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(0,
             ";%u"), (uint32)(range_p->hw_index)));
        scache_p += sizeof(uint16);
        (*scache_pos) += sizeof(uint16);
...
bcmi_xgs5_range_wb_sync(int unit)
{
    ...
    if (BCM_WB_DEFAULT_VERSION >= BCM_WB_VERSION_2_0) {
        BCM_IF_ERROR_RETURN(bcmi_xgs5_range_wb_sync_2_0(unit, &scache_ptr, &scache_pos));
        BCM_IF_ERROR_RETURN
            (bcmi_xgs5_range_group_wb_sync_2_0(unit, &scache_ptr, &scache_pos));
...
bcmi_xgs5_range_reinit(int unit)
{
    ...
    if (BCM_E_NOT_FOUND == rv) {
        ...
        if (recovered_ver >= BCM_WB_VERSION_2_0) {
            BCM_IF_ERROR_RETURN(bcmi_xgs5_range_wb_reinit_2_0(unit,
                        &scache_ptr, &scache_pos));
            BCM_IF_ERROR_RETURN
                (bcmi_xgs5_range_group_wb_reinit_2_0(unit,
                                                     &scache_ptr, &scache_pos));

        } else {
            if (recovered_ver >= BCM_WB_VERSION_1_0) {
                BCM_IF_ERROR_RETURN(bcmi_xgs5_range_wb_reinit_1_0(unit,
                            &scache_ptr, &scache_pos));
            }
            if (recovered_ver >= BCM_WB_VERSION_1_1) {
                BCM_IF_ERROR_RETURN(bcmi_xgs5_range_wb_reinit_1_1(unit,
                            &scache_ptr, &scache_pos));
            }
        }
        BCM_IF_ERROR_RETURN
            (bcmi_xgs5_range_wb_realloc(unit));
    }
        ...
        bcm_range_config_t *range_config)
{
    ...
    /* Search existing ranges */
    for (tmp = RANGE_CTRL(unit)->ranges; tmp != NULL; tmp = tmp->next) {
        if (pipe_instance != tmp->pipe_instance) {
            continue;
        }
    ...
    tmp->next = RANGE_CTRL(unit)->ranges;
    RANGE_CTRL(unit)->ranges = tmp;
        ...
        bcm_range_t rid)
{
    int         rv;         /* Operation return value */
    _range_t    *cur = NULL;    /* Ptr to Current range being traversed */
    _range_t    *tmp = NULL;    /* Ptr to Current range being traversed */
    ...
    /* Find whether hw_index is shared or not  */
    for (tmp = RANGE_CTRL(unit)->ranges; tmp != NULL; tmp = tmp->next) {
        if(range_tmp.rid == tmp->rid) {
            /* same Range. Continue... */
            continue;
        }

        if ((range_tmp.hw_index == tmp->hw_index)
            && (range_tmp.pipe_instance == tmp->pipe_instance)) {
            idx_shared = 1;
            break;
        }
    }
    ...
    bcm_range_group_config_t *range_group_config)
{
    ...
    for (idx = 0; idx < BCM_RANGE_ID_MAX ; idx ++ ) {
    ...
    if (BCM_SUCCESS(rv)) {
        ...
        sal_memcpy(&(RANGE_CTRL(unit)->range_group_range_bmp[range_group_config->range_group_id]),
                &(range_group_config->range_bmp), sizeof(bcm_range_id_bmp_t));
    }
    ...
    bcm_range_group_config_t *range_group_config)
{
    ...
    sal_memcpy(&(range_group_config->range_bmp),
            &(RANGE_CTRL(unit)->range_group_range_bmp[range_group_id]),
            sizeof(bcm_range_id_bmp_t));
