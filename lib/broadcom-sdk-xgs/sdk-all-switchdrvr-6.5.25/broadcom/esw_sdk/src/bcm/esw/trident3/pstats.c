/*
 * $Id: pstats.c,v 1.0 Broadcom SDK $
...
#if defined(INCLUDE_PSTATS) && defined(BCM_TRIDENT3_SUPPORT)
#include <soc/drv.h>
#include <soc/mem.h>
...
#define TD3_MAX_POOL_COUNT          4

#define UCQ_COUNT(unit)             (SOC_IS_HELIX5(unit) ? 1280 : \
                                     SOC_IS_FIREBOLT6(unit) ? 3680 : 1328)

/* Max Elements Count = UCQ + ING POOL + EGR POOL */
#define TD3_MAX_ELEMENT_COUNT(unit) UCQ_COUNT(unit) + TD3_MAX_POOL_COUNT * 2

#define TD3_PORT_NUM_COS(unit, port)                            \
 ...
 * |   1    |  0  |  1   |
 *  */
#define TD3_XPE_PIPE_INDEX(unit, xpe, pipe)                     \
    (((xpe) == -1) ? (pipe) :                                   \
     ((pipe) == -1) ? (xpe) : (xpe) * NUM_PIPE(unit) + (pipe))

#define TD3_XPE_VALID(unit, xpe, valid)                        \
    if (xpe >  NUM_XPE(unit)) {                                \
        valid = FALSE;                                         \
    }                                                          \
...
typedef struct pstats_control_s {
    ...
    pstats_buffer_t                 *ucq_buffer;
    pstats_buffer_t                 ing_pool_buffer[TD3_MAX_POOL_COUNT];
    pstats_buffer_t                 egr_pool_buffer[TD3_MAX_POOL_COUNT];
...
STATIC int
bcm_td3_pstats_session_create(int unit, int options, int array_count,
                              bcm_pstats_session_element_t  *element_array,
                              bcm_pstats_session_id_t *session_id);
STATIC int
bcm_td3_pstats_session_destroy(int unit, bcm_pstats_session_id_t session_id);
STATIC int
bcm_td3_pstats_session_get(int unit, bcm_pstats_session_id_t session_id,
                           int array_max,
                           bcm_pstats_session_element_t *element_array,
                           int *array_count);
STATIC int bcm_td3_pstats_data_sync(int unit);
STATIC int
bcm_td3_pstats_session_data_get(int unit, bcm_pstats_session_id_t session_id,
                                int sync,
                                bcm_pstats_timestamp_t *timestamp,
                                int array_max,
                                ...
                                int *array_count);
STATIC int
bcm_td3_pstats_session_data_clear(int unit, bcm_pstats_session_id_t session_id);

STATIC int
bcm_td3_pstats_session_traverse(int unit, bcm_pstats_session_traverse_cb cb,
                                void *user_data);

#ifdef BCM_WARM_BOOT_SUPPORT_SW_DUMP
/*
 * Function:
 *     bcm_td3_pstats_sw_dump
 * Purpose:
 ...
 */
STATIC void
bcm_td3_pstats_sw_dump(int unit)
{
...
STATIC void
_bcm_td3_pstats_session_cleanup(int unit, pstats_session_t *session)
{
    session->enable = FALSE;
    if (NULL != session->data_array) {
        sal_free(session->data_array);
    }
    if (NULL != session->element_array) {
        sal_free(session->element_array);
    }
...
_bcm_td3_pstats_session_get(int unit, bcm_pstats_session_id_t session_id)
{
    int                 found = 0;
    pstats_session_t   *cur_session;
...
STATIC void
_bcm_td3_pstats_session_next_id_set(int unit)
{
    pstats_session_t            *cur_session;
...
STATIC int
_bcm_td3_pstats_session_delete(int unit, bcm_pstats_session_id_t session_id)
{
    pstats_session_t            *cur_session;

    cur_session = _bcm_td3_pstats_session_get(unit, session_id);
    if (NULL == cur_session) {
    ...
    _bcm_td3_pstats_session_cleanup(unit, cur_session);
    if (NULL != cur_session) {
        sal_free(cur_session);
    }
    PSTATS_CTRL(unit).session_count--;

    _bcm_td3_pstats_session_next_id_set(unit);

    return BCM_E_NONE;
}

STATIC void
_bcm_td3_pstats_session_insert(int unit, pstats_session_t *session)
{
    pstats_session_t            *cur_session;
...
STATIC void
_bcm_td3_pstats_buffer_deinit(int unit, pstats_buffer_t *pbuf)
{
    if (NULL == pbuf) {
        return;
    }
...
STATIC int
_bcm_td3_pstats_buffer_init(int unit, soc_mem_t mem, soc_mem_t clear_mem,
 ...
 */
void
bcm_td3_pstats_deinit(int unit)
{
...
#ifdef BCM_PSTATS_MEASURE
    _bcm_td3_pstats_buffer_deinit(unit, &PSTATS_CTRL(unit).local_time);
    _bcm_td3_pstats_buffer_deinit(unit, &PSTATS_CTRL(unit).local_time_end);
#endif
    _bcm_td3_pstats_buffer_deinit(unit, &PSTATS_CTRL(unit).utc_time);

    if (PSTATS_CTRL(unit).ucq_buffer != NULL) {
        for (i = 0; i < NUM_XPE(unit) * NUM_PIPE(unit); i++) {
            _bcm_td3_pstats_buffer_deinit(unit, &PSTATS_CTRL(unit).ucq_buffer[i]);
        }
        sal_free(PSTATS_CTRL(unit).ucq_buffer);
    }

    for (i = 0; i < TD3_MAX_POOL_COUNT; i++) {
        _bcm_td3_pstats_buffer_deinit(unit, &PSTATS_CTRL(unit).ing_pool_buffer[i]);
        _bcm_td3_pstats_buffer_deinit(unit, &PSTATS_CTRL(unit).egr_pool_buffer[i]);
    }
...
bcm_td3_pstats_init(int unit)
{
    ...
    PSTATS_CTRL(unit).ucq_buffer = sal_alloc(NUM_PIPE(unit) * NUM_XPE(unit) *
                                   sizeof(pstats_buffer_t), "pstats ucq_buffer");

    sal_memset(PSTATS_CTRL(unit).ucq_buffer, 0,
               NUM_PIPE(unit) * NUM_XPE(unit) * sizeof(pstats_buffer_t));

    /* Initializing Driver functions for PSTATS */
    pstats_driver->session_create = bcm_td3_pstats_session_create;
    pstats_driver->session_destroy = bcm_td3_pstats_session_destroy;
    pstats_driver->session_get = bcm_td3_pstats_session_get;
    pstats_driver->data_sync = bcm_td3_pstats_data_sync;
    pstats_driver->session_data_get = bcm_td3_pstats_session_data_get;
    pstats_driver->session_data_clear = bcm_td3_pstats_session_data_clear;
    pstats_driver->session_traverse = bcm_td3_pstats_session_traverse;
#ifdef BCM_WARM_BOOT_SUPPORT_SW_DUMP
    pstats_driver->pstats_sw_dump = bcm_td3_pstats_sw_dump;
#endif
    ...
    rv = _bcm_td3_pstats_buffer_init(unit, mem, mem, pbuf);
    if (BCM_FAILURE(rv)) {
        goto cleanup;
    }
    mem = MMU_INTFO_TIMESTAMPm;
    ...
    for (pipe = 0; pipe < NUM_PIPE(unit); pipe++) {
        for (xpe = 0; xpe < NUM_XPE(unit); xpe++) {
            ...
            index = TD3_XPE_PIPE_INDEX(unit, xpe, pipe);
            if (index >= NUM_XPE(unit) * NUM_PIPE(unit)) {
                rv = BCM_E_INTERNAL;
                goto cleanup;
            }
            pbuf = &PSTATS_CTRL(unit).ucq_buffer[index];

            rv = _bcm_td3_pstats_buffer_init(unit, mem, clear_mem, pbuf);
            if (BCM_FAILURE(rv)) {
                goto cleanup;
            }
        }
    ...
    for (xpe = 0; xpe < NUM_XPE(unit); xpe++) {
        ...
        index = TD3_XPE_PIPE_INDEX(unit, xpe, -1);
        if (index >= TD3_MAX_POOL_COUNT) {
            rv = BCM_E_INTERNAL;
            goto cleanup;
        }
    ...
    if (SOC_WARM_BOOT(unit)) {
        ...
        rv = soc_trident3_mmu_pstats_mode_get(unit, &flag);
        if (BCM_FAILURE(rv)) {
            goto cleanup;
        }
        ...
        if (flag & _TD3_MMU_PSTATS_HWM_MOD) {
            PSTATS_CTRL(unit).stat_mode = 1;
            if (flag & _TD3_MMU_PSTATS_RESET_ON_READ) {
        ...
        flag |= _TD3_MMU_PSTATS_SYNC | _TD3_MMU_PSTATS_MOR_EN;

        /* Reset pstats mode in warmboot */
        rv = soc_trident3_mmu_pstats_mode_set(unit, flag);
        if (BCM_FAILURE(rv)) {
            goto cleanup;
        }
    } else
...
STATIC int
_bcm_td3_pstats_session_element_check(int unit,
    ...
    if (element.type == bcmPStatsSessionUnicastQueue) {
        ...
        if (BCM_GPORT_IS_SET(gport)) {
            if (BCM_GPORT_IS_LOCAL(gport)) {
                ...
                LOG_INFO(BSL_LS_BCM_PSTATS,
                    (BSL_META_U(unit, "pstats_session_element_check: "
        ...
        if ((cosq < 0) || (cosq >= TD3_PORT_NUM_COS(unit, local_port))) {
            LOG_INFO(BSL_LS_BCM_PSTATS,
                (BSL_META_U(unit, "pstats_session_element_check: "
                            "cosq %d is illegal!\n"), cosq));
            return BCM_E_PARAM;
        ...
        if ((element.pool < 0) || (element.pool >= TD3_MAX_POOL_COUNT)) {
            LOG_INFO(BSL_LS_BCM_PSTATS,
                (BSL_META_U(unit, "pstats_session_element_check: "
                              ...
                              bcm_pstats_session_id_t *session_id)
{
    ...
    /* element check */
    if ((array_count < 0) ||
        (array_count > TD3_MAX_ELEMENT_COUNT(unit))) {
    ...
    for (i = 0; i < array_count; i++) {
        BCM_IF_ERROR_RETURN(
                _bcm_td3_pstats_session_element_check(unit, element_array[i]));
    }

    if (options & BCM_PSTATS_OPTIONS_REPLACE) {
        /* delete original session first */
        id = *session_id;
        BCM_IF_ERROR_RETURN(_bcm_td3_pstats_session_delete(unit, id));
        ...
        cur_session = _bcm_td3_pstats_session_get(unit, id);
        if (NULL != cur_session) {
            LOG_INFO(BSL_LS_BCM_PSTATS,
    ...
    alloc_size = array_count * sizeof(bcm_pstats_session_element_t);
    cur_session->element_array = sal_alloc(alloc_size, "element array");
    if (NULL == cur_session->element_array) {
        rv = BCM_E_MEMORY;
        goto cleanup;
    }
    ...
    _bcm_td3_pstats_session_insert(unit, cur_session);
    _bcm_td3_pstats_session_next_id_set(unit);
...
/*
 * Function:
 *      bcm_td3_pstats_session_destroy
 * Purpose:
 ...
 */
STATIC int
bcm_td3_pstats_session_destroy(int unit,
                               bcm_pstats_session_id_t session_id)
{
    return _bcm_td3_pstats_session_delete(unit, session_id);
}

/*
 * Function:
 *      bcm_td3_pstats_session_get
 * Purpose:
 ...
 */
STATIC int
bcm_td3_pstats_session_get(int unit,
                           bcm_pstats_session_id_t session_id,
                           int array_max,
                           bcm_pstats_session_element_t *element_array,
                           int *array_count)
...
STATIC int
_bcm_td3_pstats_session_data_update(int unit,
                                    bcm_pstats_session_element_t element,
                                    bcm_pstats_data_t *data)
{
    bcm_gport_t                 gport;
    ...
    if (element.type == bcmPStatsSessionUnicastQueue) {
        ...
        for (xpe = 0, i = 0; xpe < NUM_XPE(unit); xpe++) {
            ...
            if (SOC_IS_TRIDENT3(unit) || SOC_IS_MAVERICK2(unit)) {
                /* In TD3, due to physical bank is 332 deep, when read out
                 * index='d82, it will read out 2 valid counter at location 'd328
                 * and 'd329, plus 2 invalid counter at location 'd330 and 'd331
                 */
                if (startq >= 330) {
                    startq += 2;
                }
...
STATIC int
_bcm_td3_pstats_session_update(int unit, pstats_session_t *session)
{
    bcm_pstats_session_element_t    element;
    bcm_pstats_data_t               *data;
    ...
    pentry = soc_mem_table_idx_to_pointer(unit, mem, void*, total_buf, 0);
    ...
    for (i = 0; i < cur_session->element_array_count; i++) {
        ...
        BCM_IF_ERROR_RETURN(
            _bcm_td3_pstats_session_data_update(unit, element, data));
    }
 ...
 */
STATIC int
_bcm_td3_pstats_hw_sync(int unit)
{
    pstats_buffer_t             *pbuf;
    int                         index, pipe, xpe;
    soc_info_t                  *si = &SOC_INFO(unit);
...
/*
 * Function:
 *      _bcm_td3_pstats_session_data_sync
 * Purpose:
 ...
 */
STATIC int
_bcm_td3_pstats_session_data_sync(int unit, pstats_session_t *session)
{
    BCM_IF_ERROR_RETURN(_bcm_td3_pstats_hw_sync(unit));
    BCM_IF_ERROR_RETURN(_bcm_td3_pstats_session_update(unit, session));
    return BCM_E_NONE;
}

/*
 * Function:
 *      bcm_td3_pstats_data_sync
 * Purpose:
 *      Start a pstats sync operation.
 * Parameters:
 *      unit - (IN) StrataSwitch unit number
 ...
 */
STATIC int
bcm_td3_pstats_data_sync(int unit)
{
    pstats_session_t    *cur_session = NULL;
    ...
    while (NULL != cur_session) {
        ...
        BCM_IF_ERROR_RETURN(
                _bcm_td3_pstats_session_update(unit, cur_session));
        cur_session = cur_session->next_session;
    }
    return BCM_E_NONE;
}
                                ...
                                int *array_count)
{
    ...
    if (TRUE == sync) {
        if (mode) {
            BCM_IF_ERROR_RETURN(bcm_td3_pstats_data_sync(unit));
        } else {
            BCM_IF_ERROR_RETURN(
                    _bcm_td3_pstats_session_data_sync(unit, cur_session));
        }
    }
...
STATIC int
_bcm_td3_pstats_session_data_hw_clear(int unit, bcm_pstats_session_element_t element)
{
    bcm_gport_t                 gport;
    bcm_port_t                  local_port;
    bcm_cos_queue_t             cosq;
...
bcm_td3_pstats_session_data_clear(int unit, bcm_pstats_session_id_t session_id)
{
    ...
    if (need_hw_clear == 1) {
        for (i = 0; i < cur_session->element_array_count; i++) {
            element = cur_session->element_array[i];
            BCM_IF_ERROR_RETURN(
                _bcm_td3_pstats_session_data_hw_clear(unit, element));
        }
    }
...
/*
 * Function:
 *      bcm_td3_pstats_session_traverse
 * Purpose:
 *      Traverse through the pstats sessions and run callback at each
 *      session.
 * Parameters:
 *      unit - (IN) StrataSwitch unit number.
...
bcm_td3_pstats_session_traverse(int unit, bcm_pstats_session_traverse_cb cb,
                                void *user_data)
{
    bcm_pstats_session_element_t    *element_array = NULL;
    bcm_pstats_session_id_t         session_id;
    pstats_session_t                *cur_session = NULL;
    int                             array_count;
    int                             rv = BCM_E_NONE;
    ...
    while (NULL != cur_session) {
        ...
        element_array = sal_alloc(array_count *
                sizeof(bcm_pstats_session_element_t), "element array");
        if (NULL == element_array) {
            return BCM_E_MEMORY;
        }
...
#else /* INCLUDE_PSTATS && BCM_TRIDENT3_SUPPORT */
typedef int bcm_esw_trident3_pstats_not_empty; /* Make ISO compilers happy. */
#endif /* INCLUDE_PSTATS && BCM_TRIDENT3_SUPPORT */
