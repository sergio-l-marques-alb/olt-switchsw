                           ...
                           uint16 *check_ft_type_bmp)
{
    ...
    if (options & BCM_FLOWTRACKER_CHECK_SECOND) {
        rv = _bcm_field_ft_tracking_param_type_get(unit,
                bcmFlowtrackerGroupTypeNone, bcmFlowtrackerStageDefault,
                flow_check_info.param, flow_check_info.custom_id,
                _BCM_FIELD_FT_TRACKING_PARAM_TYPE_GET_F_IGNORE_GROUP,
                &param_ft_type_bmp);
    } else {
        rv = _bcm_field_ft_tracking_param_type_get(unit,
                bcmFlowtrackerGroupTypeNone, flow_check_info.stage,
                flow_check_info.param, flow_check_info.custom_id,
                _BCM_FIELD_FT_TRACKING_PARAM_TYPE_GET_F_IGNORE_GROUP,
                &param_ft_type_bmp);
    }
    ...
    rv = _bcm_field_ft_tracking_param_range_get(unit, 0, flow_check_info.param,
                     ...
                     bcm_flowtracker_check_t *flowchecker_id)
{
    ...
    if (options & BCM_FLOWTRACKER_CHECK_SECOND) {
        ...
        BCMI_FT_FLOWCHECKER_INFO(unit, index)->check2.stage =
                                               flow_check_info.stage;
    ...
    */
    ...
    BCMI_FT_FLOWCHECKER_INFO(unit, index)->check1.custom_id =
                                           flow_check_info.custom_id;
    BCMI_FT_FLOWCHECKER_INFO(unit, index)->check1.stage =
                                           flow_check_info.stage;
                             ...
                             bcm_flowtracker_stage_t stage)
{
    ...
    bcm_flowtracker_stage_t check_stage;
    ...
    if (stage != bcmFlowtrackerStageDefault) {
        sal_memset(&fc_info, 0, sizeof(bcmi_flowtracker_flowchecker_info_t));
        BCM_IF_ERROR_RETURN
            (bcmi_ft_flowchecker_get(unit, check_id, &fc_info));
        /* Deduce ft_type for check */
        rv = bcmi_ft_group_check_ft_type_get(unit,
                    BCMI_FT_GROUP_TYPE_GET(id), &fc_info, &ft_type);
        BCM_IF_ERROR_RETURN(rv);

        BCMI_FT_TYPE_BMP_STAGE_GET((1 << ft_type), check_stage);
                              ...
                              bcmi_flowtracker_flowchecker_info_t *fc_info)
{
    ...
    if (fc_info->check1.param != bcmFlowtrackerTrackingParamTypeNone) {
        temp_ft_type_bmp = 0;
        rv = _bcm_field_ft_tracking_param_type_get(unit,
                bcmFlowtrackerGroupTypeNone, fc_info->check1.stage,
...
/*
 * Function:
 *   bcmi_ft_group_check_ft_type_get
 * Purpose:
 *   Get ft_type for given check when added to
 *   given group type
 * Parameters:
 *   unit - (IN) BCM device id
 *   group_type - (IN) Flowtracker Group Type
 *   fc_info - (IN) Flowcheck info
 *   ft_type - (OUT)  Ft type
 * Returns:
 *   None.
 */
                    ...
                    bcmi_ft_type_t *ft_type)
{
    ...
    uint16 ft_type_bmp = 0;
    uint16 group_ft_type_bmp = 0;
    bcm_flowtracker_stage_t stage = bcmFlowtrackerStageDefault;

    /* Get ft_type based on group_type and check ft_type_bmp */
    BCMI_FT_GROUP_FT_TYPE_BMP_GET(group_type, group_ft_type_bmp);
    ft_type_bmp = group_ft_type_bmp & fc_info->check_ft_type_bmp;
    ...
    /* Find final ft_type */
    if (fc_info->check1.param != bcmFlowtrackerTrackingParamTypeNone) {
        /* Get default stage for the param */
        rv = _bcm_field_ft_tracking_param_def_stage_get(unit,
                group_type, fc_info->check1.param, &stage);
        BCM_IF_ERROR_RETURN(rv);

        BCMI_FT_STAGE_FT_TYPE_BMP_GET(stage, ft_type_bmp, ft_type_bmp);
        if (BCMI_FT_TYPE_F_IS_UNIQUE(ft_type_bmp)) {
            BCMI_FT_TYPE_F_UNIQUE_GET(ft_type_bmp, *ft_type);
            return BCM_E_NONE;
        }
        ...
        } else if (ft_type_bmp & BCMI_FT_TYPE_F_MFT) {
            *ft_type = bcmiFtTypeMFT;
        } else { /* (ft_type_bmp & BCMI_FT_TYPE_F_EFT) */
