...
bcmi_xgs4_udf_wb_offset_info_reinit_1_0(int unit, int num_offset_nodes,
                                        uint8 **scache_ptr, uint32 *scache_pos)
{
    int rv;
    int i;
    int val = 0;
    ...
    /* Size of the struct bcmi_xgs4_wb_offset_info_1_0 as defined in the source
     * imgage used to sync the data. During recovery, this is the size of the
     * struct to be read from scache.
     */
    int scache_sizeof_bcmi_xgs4_wb_offset_info_1_0 = 0;
#if defined (BCM_TOMAHAWK_SUPPORT)
    int offset_num, port;
    int pbmp_max_words_to_copy = 0;
    int pbmp_port_max_synced  = 0;
    int pbmp_max_words_synced = 0;
    soc_info_t  *si;
    si = &SOC_INFO(unit);
    /* pbmp_port_max_synced holds the SOC_PBMP_PORT_MAX as used in the source 
     * image used to sync scache
     */
    pbmp_port_max_synced = soc_scache_dictionary_entry_get(unit,
                        ssden_SOC_PBMP_PORT_MAX, SOC_PBMP_PORT_MAX);
    pbmp_max_words_synced =  (pbmp_port_max_synced + 32 -1) / 32;
    /* pbmp_max_words_synced od data has to be read from scache as this is the 
     * amount of data that is sync in source image.
     * pbmp_max_words_to_copy is the actual valid data to be synced and rest
     * of the data has to be read and discarded.
     */
    pbmp_max_words_to_copy = (pbmp_max_words_synced <=_SHR_PBMP_WORD_MAX) ? \
                                 pbmp_max_words_synced : _SHR_PBMP_WORD_MAX;

    scache_sizeof_bcmi_xgs4_wb_offset_info_1_0 =
        sizeof(struct bcmi_xgs4_wb_offset_info_1_0) - sizeof(bcm_pbmp_t) +
        (4*pbmp_max_words_synced);
#else
    scache_sizeof_bcmi_xgs4_wb_offset_info_1_0 =
        sizeof(struct bcmi_xgs4_wb_offset_info_1_0);
#endif
    ...
    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
        "S$BEGIN:udf:{offset;length;num_offset_node" \
        "layer,start,width,flags,num_pkt_formats;udf_id;byte_bmp")));
#if defined (BCM_TOMAHAWK_SUPPORT)
        if (soc_feature(unit, soc_feature_udf_multi_pipe_support)) {
            LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                ";associated_ports;associated_pipe")));
        }
#endif
    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,"}\n")));
    ...
    while (num_offset_nodes > 0) {
        ...
        LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
            "    {0x%x;%d;%d;%d;%d;%d;%d;%d;%d;%d"), (*scache_pos),
            (int)(scache_sizeof_bcmi_xgs4_wb_offset_info_1_0),
            num_offset_nodes, scache_offset_p->layer,
            scache_offset_p->start, scache_offset_p->width,
            scache_offset_p->flags, scache_offset_p->num_pkt_formats, 
            scache_offset_p->id, scache_offset_p->byte_bmap));
#if defined (BCM_TOMAHAWK_SUPPORT)
        if ((soc_feature(unit, soc_feature_udf_multi_pipe_support)) &&
           (recovered_ver >= BCM_WB_VERSION_1_2)) {
            sal_memset(&(udf_info.ports), 0x00, sizeof(bcm_pbmp_t));
            sal_memcpy(&(udf_info.ports),
                       &(scache_offset_p->associated_ports),
                       pbmp_max_words_to_copy*sizeof(uint32));
            LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                ";%d"), udf_info.ports.pbits[0]));
            LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                ";%d"), scache_offset_p->associated_pipe));
        }
#endif
        LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,"}\n")));
        ...
        (*scache_pos) += scache_sizeof_bcmi_xgs4_wb_offset_info_1_0;
        *scache_ptr += scache_sizeof_bcmi_xgs4_wb_offset_info_1_0;
        scache_offset_p = (struct bcmi_xgs4_wb_offset_info_1_0 *)(*scache_ptr);
                                      ...
                                      uint8 **scache_ptr, uint32 *scache_pos)
{
    ...
    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
        "S$BEGIN:udf:{offset;length;num_tcam_nodes" \
        "hw_idx;pkt_format_id;priority;num_udfs")));
#if defined (BCM_TOMAHAWK_SUPPORT)
        if (soc_feature(unit, soc_feature_udf_multi_pipe_support)) {
            LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                ";associated_pipe")));
...
#endif
    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,";udf_id_list}\n")));
    ...
    while (num_tcam_nodes > 0) {
        ...
        LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
            "    {0x%x;%d;%d;%d;%d;%d;%d;0x"), (*scache_pos),
            (int)(sizeof(struct bcmi_xgs4_wb_tcam_info_1_0)),
            num_tcam_nodes, tcam_info->hw_idx, tcam_info->pkt_format_id,  
            tcam_info->priority, tcam_info->num_udfs));
#if defined (BCM_TOMAHAWK_SUPPORT)
        if ((soc_feature(unit, soc_feature_udf_multi_pipe_support)) &&
           (recovered_ver >= BCM_WB_VERSION_1_2) &&
           (bcmi_xgs4_udf_oper_mode[unit] == bcmUdfOperModePipeLocal)) {
            ...
            LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                    "%d;0x"), tcam_info->associated_pipe));
        }
#endif
        /* If h/w Index is invalid, continue */
        if (idx >= MAX_UDF_TCAM_ENTRIES) {
            sal_free(tcam_info);
            tcam_info = NULL;
            (*scache_pos) += sizeof(struct bcmi_xgs4_wb_tcam_info_1_0);
            scache_tcam_p++;
            num_tcam_nodes--;
            continue;
        }
        ...
        if (idx < MAX_UDF_TCAM_ENTRIES) {
            ...
            for (idx = 0; idx < MAX_UDF_OFFSET_CHUNKS; idx++) {
                ...
                if (tcam_info->offset_info_list[idx] != NULL) {
                    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                        "%04x"), (tcam_info->offset_info_list[idx])->udf_id));
                } else {
                    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,"0000")));
                }
            }
                                      ...
                                      uint8 **scache_ptr, uint32 *scache_pos)
{
    ...
    while (num_offset_nodes > 0) {
        ...
        LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
            "    {0x%x;%d;%d;%d;%d;%d;%d;%d;%d;%d"), (*scache_pos),
            (int)(sizeof(struct bcmi_xgs4_wb_offset_info_1_0)),
            num_offset_nodes, scache_offset_p->layer,
            scache_offset_p->start, scache_offset_p->width,
            scache_offset_p->flags, scache_offset_p->num_pkt_formats, 
            scache_offset_p->id, scache_offset_p->byte_bmap));
        ...
        if (soc_feature(unit, soc_feature_udf_multi_pipe_support)) {
            ...
            LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                ";%u"), scache_offset_p->associated_ports.pbits[0]));
            LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                ";%d"), scache_offset_p->associated_pipe));
        }
#endif
        LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,"}\n")));
        num_offset_nodes--;
        (*scache_pos) += sizeof(struct bcmi_xgs4_wb_offset_info_1_0);
        scache_offset_p++;
                                    ...
                                    uint8 **scache_ptr, uint32 *scache_pos)
{
    ...
    while (num_tcam_nodes > 0) {
        ...
        LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
            "    {0x%x;%d;%d;%d;%d;%d;%d;0x"), (*scache_pos),
            (int)(sizeof(struct bcmi_xgs4_wb_tcam_info_1_0)),
            num_tcam_nodes, scache_tcam_p->hw_id,  
            scache_tcam_p->id, scache_tcam_p->priority,
             scache_tcam_p->num_udfs));
#if defined (BCM_TOMAHAWK_SUPPORT)
        if (soc_feature(unit, soc_feature_udf_multi_pipe_support)) {
            scache_tcam_p->associated_pipe = tcam_info->associated_pipe;
            LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                    "%d;0x"), scache_tcam_p->associated_pipe));
        }
#endif
        ...
        for (idx = 0; idx < MAX_UDF_OFFSET_CHUNKS; idx++) {
            ...
            LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                    "%04x"), scache_tcam_p->udf_id_list[idx]));
        }
        LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,"}\n")));
        (*scache_pos) += sizeof(struct bcmi_xgs4_wb_tcam_info_1_0);
        scache_tcam_p++;
                             ...
                             uint32 *scache_pos)
{
    ...
    while (num_offset_nodes > 0) {
        ...
        sal_memset(&(udf_info.ports), 0x00, sizeof(bcm_pbmp_t));
        sal_memcpy(&(udf_info.ports), &(scache_offset_p->associated_ports),
                   pbmp_max_words_to_copy*sizeof(uint32));
       ...
       /* Compute byte offset again and store */
        ...
        LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
            ...
            scache_offset_p->id, scache_offset_p->byte_bmap));
        LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
            ";%u"), udf_info.ports.pbits[0]));
        LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
            ";%d}\n"), scache_offset_p->associated_pipe));
        (*scache_pos) += scache_sizeof_bcmi_xgs4_wb_offset_info_1_0;
        *scache_ptr += scache_sizeof_bcmi_xgs4_wb_offset_info_1_0;
        scache_offset_p = (struct bcmi_xgs4_wb_offset_info_1_0 *)(*scache_ptr);
        num_offset_nodes--;
...
/* clear invalid udf_offset cache recovery */
   ...
   for(i=0;i<offset_num;i++){
    ...
    rv = bcmi_xgs4_udf_wb_tcam_info_reinit_1_0(unit,
                UDF_CTRL(unit)->num_pkt_formats, scache_ptr, scache_pos);
    BCM_IF_ERROR_RETURN(rv);
                            ...
                            )
{
    ...
    int max_num_pipes_synced = 0;
    int max_num_pipes_to_copy = 0;

    max_num_pipes_synced = soc_scache_dictionary_entry_get(unit,
                                                ssden_SOC_MAX_NUM_PIPES,
                                                SOC_MAX_NUM_PIPES);
    max_num_pipes_to_copy = (max_num_pipes_synced <= SOC_MAX_NUM_PIPES) ? \
                             max_num_pipes_synced : SOC_MAX_NUM_PIPES;

    if (recovered_ver >= BCM_WB_VERSION_1_3) {
        LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
            "S$BEGIN:udf:{offset;length;pipe;chunk;" \
            "per_pipe_flags;per_pipe_grp_id;per_pipe_num_pkt_formats}\n")));
        for (pipe = 0; pipe < max_num_pipes_synced; pipe++) {
            if (pipe >= max_num_pipes_to_copy ) {
                for (i = 0; i < max_chunks; i++) {
                    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                        "    {0x%x;3;%d;%d;0;0;0}\n"), (*scache_pos), pipe, i));
                    (*scache_pos) += sizeof(struct bcmi_xgs4_wb_offset_entry_1_0);
                    offset_entry_scache_p++;
                }
                continue;
            }
            for (i = 0; i < max_chunks; i++) {
                ...
                LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                    "    {0x%x;3;%d;%d;%d;%d;%d}\n"), (*scache_pos),pipe,i,
                    offset_entry->flags, offset_entry->grp_id,
    ...
    *scache_ptr = (uint8 *)offset_entry_scache_p;
    ...
    rv = bcmi_xgs4_udf_wb_offset_info_reinit_1_0(unit,
                 UDF_CTRL(unit)->num_udfs, scache_ptr, scache_pos);
    BCM_IF_ERROR_RETURN(rv);
...
STATIC int
bcmi_xgs4_udf_wb_reinit_1_0(int unit, uint8 **scache_ptr, uint32 *scache_pos)
{
    int rv;
    int i;
    int max_chunks;
    ...
    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
            "S$BEGIN:udf:{offset;length;num_udfs}\n")));
    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
             "    {0x%x;4;%u}\n"), (*scache_pos),
              (uint32)(UDF_CTRL(unit)->num_udfs)));
    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                            "S$END:udf\n")));
    (*scache_pos) += sizeof(uint32);
    u32_scache_p++;

    /* Number of Packet formats created */
    UDF_CTRL(unit)->num_pkt_formats = *u32_scache_p;
    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
            "S$BEGIN:udf:{offset;length;num_pkt_formats}\n")));
    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
             "    {0x%x;4;%u}\n"), (*scache_pos),
              (uint32)(UDF_CTRL(unit)->num_pkt_formats)));
    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                            "S$END:udf\n")));
    (*scache_pos) += sizeof(uint32);
    u32_scache_p++;
    ...
    if ((SOC_IS_TOMAHAWKX(unit)) &&
        ...
        /* Pass Operation Mode */
        LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
            "S$BEGIN:udf:{offset;length;udf_oper_mode}\n")));
        LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
             "    {0x%x;4;%u}\n"), (*scache_pos),
              (uint32)(bcmi_xgs4_udf_oper_mode[unit])));
        LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                            "S$END:udf\n")));
        (*scache_pos) += sizeof(uint32);
        u32_scache_p++;
...
#endif
    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
            "S$BEGIN:udf:{offset;length;chunk;" \
            "flags;grp_id;num_pkt_formats}\n")));
    for (i = 0; i < max_chunks; i++) {
...
STATIC int
bcmi_xgs4_udf_wb_reinit_1_1(int unit, uint8 **scache_ptr, uint32 *scache_pos)
{
    uint32 *u32_scache_p;
    ...
    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
        "S$BEGIN:udf:{offset;length;udf_used_by_module}\n")));
    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
        "    {0x%x;4;%u}\n"), (*scache_pos),
        (uint32)(UDF_CTRL(unit)->udf_used_by_module)));
    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,"S$END:udf\n")));
    (*scache_pos) += sizeof(uint32);
    u32_scache_p++;
...
STATIC int
bcmi_xgs4_udf_wb_reinit_1_3(int unit, uint8 **scache_ptr, uint32 *scache_pos)
{
    bcmi_xgs4_udf_tcam_entry_t *tcam_entry_arr; /* Tcam entries array.  */
    int num_tcam_nodes;
    ...
    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
        "S$BEGIN:udf:{offset;length;num_tcam_nodes;" \
        "hw_idx;class_id;associated_pipe}\n")));
    while (num_tcam_nodes > 0) {
        if (bcmi_xgs4_udf_oper_mode[unit] == bcmUdfOperModePipeLocal) {
            tcam_entry_arr = 
        ...
        if ((scache_tcam_p->hw_idx < MAX_UDF_TCAM_ENTRIES) &&
            (tcam_entry_arr[scache_tcam_p->hw_idx].tcam_info != NULL)) {
            tcam_entry_arr[scache_tcam_p->hw_idx].tcam_info->class_id = scache_tcam_p->class_id;
        }
        LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                    "    {0x%x;%d;%d;%d;%d;%d}\n"), (*scache_pos),
                    (int)(sizeof(struct bcmi_xgs4_wb_tcam_info_1_3)),
                    num_tcam_nodes, scache_tcam_p->hw_idx, 
                    scache_tcam_p->class_id, scache_tcam_p->associated_pipe));
        (*scache_pos) += sizeof(struct bcmi_xgs4_wb_tcam_info_1_3);
        scache_tcam_p++;
        num_tcam_nodes--;
    }
    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit, "S$END:udf\n")));
...
STATIC int
bcmi_xgs4_udf_wb_reinit_1_4(int unit, uint8 **scache_ptr, uint32 *scache_pos)
{
    int idx;
    int pipe;
    int hw_idx;
    ...
    uint8 *scache_rec_ptr;
    int max_num_pipes_synced = 0;
    int max_num_pipes_to_copy = 0;
    int size_to_incr = 0;
    int udf_id_list_to_copy = 0;
    int max_udf_id_list = 0;
    struct bcmi_xgs4_wb_tcam_info_1_4 scache_tcam_info_read;
    ...
    if (soc_feature(unit, soc_feature_udf_multi_pipe_support) &&
        ...
        max_num_pipes_to_copy = (max_num_pipes_synced <= SOC_MAX_NUM_PIPES) ? \
                                 max_num_pipes_synced : SOC_MAX_NUM_PIPES;
        max_udf_id_list = max_num_pipes_synced * MAX_UDF_OFFSET_CHUNKS; 
        udf_id_list_to_copy = max_num_pipes_to_copy * MAX_UDF_OFFSET_CHUNKS;
        size_to_incr = (2 * sizeof(uint16) * max_num_pipes_synced);
        size_to_incr += (2 * sizeof(uint32) * max_num_pipes_synced);
        size_to_incr += (sizeof(uint16) * MAX_UDF_OFFSET_CHUNKS \
                         * max_num_pipes_synced);
        
        scache_rec_ptr = *scache_ptr;
        while (tcam_info) {
            /* copy the valid data for the current build from scache
             * scache ptr is incremented by amount of data synced in
             * source image
             */
            sal_memcpy(scache_tcam_info_read.hw_idx,
                   scache_rec_ptr,
                   sizeof(uint16) * max_num_pipes_to_copy);
            scache_rec_ptr += sizeof(uint16) * max_num_pipes_synced;

            sal_memcpy(scache_tcam_info_read.num_udfs,
                   scache_rec_ptr,
                   sizeof(uint16) * max_num_pipes_to_copy);
            scache_rec_ptr += sizeof(uint16) * max_num_pipes_synced;

            sal_memcpy(scache_tcam_info_read.offset_bmap,
                   scache_rec_ptr,
                   sizeof(uint32) * max_num_pipes_to_copy);
            scache_rec_ptr += sizeof(uint32) * max_num_pipes_synced;

            sal_memcpy(scache_tcam_info_read.associated_pipes,
                   scache_rec_ptr,
                   sizeof(uint32) * max_num_pipes_to_copy);
            scache_rec_ptr += sizeof(uint32) * max_num_pipes_synced;

            sal_memcpy(scache_tcam_info_read.udf_id_list, \
                   scache_rec_ptr,                        \
                   sizeof(uint16) * max_num_pipes_to_copy \
                       * MAX_UDF_OFFSET_CHUNKS);
            scache_rec_ptr += (sizeof(uint16) * MAX_UDF_OFFSET_CHUNKS \
                               * max_num_pipes_synced);
            LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                "S$BEGIN:udf:{offset;length;pipe;hw_idx_per_pipe;" \
                "num_udfs_per_pipe;offset_bmap_per_pipe;" \
                "associated_pipes}\n")));
            for (idx = 0; idx < max_num_pipes_synced; idx++) {
                if (idx >= max_num_pipes_to_copy ) {
                    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                    "    {0x%x;12;%d;0;0;0;0}\n"), (*scache_pos), idx));
                    continue;
                }
                tcam_info->hw_idx_per_pipe[idx] =
                    scache_tcam_info_read.hw_idx[idx];
                tcam_info->num_udfs_per_pipe[idx] =
                     scache_tcam_info_read.num_udfs[idx];
                tcam_info->offset_bmap_per_pipe[idx] =
                     scache_tcam_info_read.offset_bmap[idx];
                tcam_info->associated_pipes[idx] =
                     scache_tcam_info_read.associated_pipes[idx];

                pipe = tcam_info->associated_pipes[idx];
                hw_idx = tcam_info->hw_idx_per_pipe[idx];
                LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                    "    {0x%x;12;%d;%d;%d;%u;%u}\n"), (*scache_pos), idx,
                    hw_idx,tcam_info->num_udfs_per_pipe[idx],
                    tcam_info->offset_bmap_per_pipe[idx], 
                    pipe));
                ...
                BCM_IF_ERROR_RETURN(
                    soc_mem_read(unit, tcam_mem, MEM_BLOCK_ALL,
                                 hw_idx, &tcam_info->hw_buf));
            ...
            LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                            "S$END:udf\n")));
            LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                "S$BEGIN:udf:{offset;length;udf_idx;udf_id_list}\n")));
            for (idx = 0; idx < max_udf_id_list; idx++) {
                if (idx >= udf_id_list_to_copy) {
                    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                    "    {0x%x;2;%d;0}\n"), ((*scache_pos)+12+(2*idx)), idx));
                    continue;
                }
                tcam_info->udf_id_list[idx] =
                    scache_tcam_info_read.udf_id_list[idx];
                LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                    "    {0x%x;2;%d;%d}\n"), ((*scache_pos)+12+(2*idx)), idx,
                    scache_tcam_info_read.udf_id_list[idx]));
            }
            LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                            "S$END:udf\n")));
            (*scache_pos) += size_to_incr;
            tcam_info = tcam_info->next;
        }
        *scache_ptr = scache_rec_ptr;
...
STATIC int
bcmi_xgs4_udf_wb_reinit_1_5(int unit, uint8 **scache_ptr, uint32 *scache_pos)
{
    int idx;
    ...
    int max_num_pipes_to_copy = 0;
    int size_to_incr = 0;
    struct bcmi_xgs4_wb_tcam_info_1_5 scache_tcam_info_read;
    ...
    max_num_pipes_to_copy = (max_num_pipes_synced <= SOC_MAX_NUM_PIPES) ? \
                             max_num_pipes_synced : SOC_MAX_NUM_PIPES;
    size_to_incr += (sizeof(uint32) * max_num_pipes_synced) + (sizeof(int));
    scache_rec_ptr = *scache_ptr;

    sal_memcpy(&(scache_tcam_info_read.udf_field_bmp),
               scache_rec_ptr,
               sizeof(uint32) * max_num_pipes_to_copy);
    scache_rec_ptr += sizeof(uint32) * max_num_pipes_synced;

    sal_memcpy(&(scache_tcam_info_read.max_chunks),
               scache_rec_ptr,
               sizeof(int));

    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
        "S$BEGIN:udf:{offset;length;pipe;udf_field_bmp}\n")));

    for (idx = 0; idx < max_num_pipes_synced; idx++) {
        if (idx >= max_num_pipes_to_copy) {
            LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
            "    {0x%x;4;%d;0}\n"), ((*scache_pos) +(idx * 4)), idx));
            continue;
        }
        sal_memcpy(UDF_CTRL(unit)->udf_field_bmp[idx].w,
                   &(scache_tcam_info_read.udf_field_bmp[idx]),
                   sizeof(uint32));
        LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
            "    {0x%x;4;%d;%u}\n"), ((*scache_pos) +(idx * 4)), idx,
            (uint32)(scache_tcam_info_read.udf_field_bmp[idx])));
    }
    UDF_CTRL(unit)->max_chunks = scache_tcam_info_read.max_chunks;
    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                            "S$END:udf\n")));
    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
        "S$BEGIN:udf:{offset;length;max_chunks}\n")));
    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
        "    {0x%x;4,%d;}\n"),
        (int)((*scache_pos) + (sizeof(uint32)*max_num_pipes_to_copy)),
        scache_tcam_info_read.max_chunks));
    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                            "S$END:udf\n")));
    (*scache_pos) += size_to_incr;
    *scache_ptr += size_to_incr;


    return BCM_E_NONE;
}
STATIC int
bcmi_xgs4_udf_wb_pkt_format_update(int unit)
{
    int num_pkt_fmts = 0;
    bcmi_xgs4_udf_tcam_info_t *tmp = NULL;
    ...
    UDF_CTRL(unit)->num_pkt_formats = num_pkt_fmts;
...
STATIC int
bcmi_xgs4_udf_wb_sync_1_0(int unit, uint8 **scache_ptr, uint32 *scache_pos)
{
    int rv;
    int i;
    int max_chunks;
...
STATIC int
bcmi_xgs4_udf_wb_sync_1_1(int unit, uint8 **scache_ptr, uint32 *scache_pos)
{
    uint32 *u32_scache_p;
...
STATIC int
bcmi_xgs4_udf_wb_sync_1_3(int unit, uint8 **scache_ptr, uint32 *scache_pos)
{
    int num_tcam_nodes;
    bcmi_xgs4_udf_tcam_info_t *tcam_info;
    struct bcmi_xgs4_wb_tcam_info_1_3 *scache_tcam_p;
...
STATIC int
bcmi_xgs4_udf_wb_sync_1_4(int unit, uint8 **scache_ptr, uint32 *scache_pos)
{
    int idx;
    bcmi_xgs4_udf_tcam_info_t *tcam_info;
    ...
    if ((soc_feature(unit, soc_feature_udf_multi_pipe_support)) &&
        ...
        while (tcam_info) {
            ...
            for (idx = 0; idx < SOC_MAX_NUM_PIPES; idx++) {
                ...
                LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                    "    {0x%x;12;%d;%d;%d;%u;%u}\n"), (*scache_pos), idx,
                    scache_tcam_p->hw_idx[idx],scache_tcam_p->num_udfs[idx],
                    scache_tcam_p->offset_bmap[idx], 
                    scache_tcam_p->associated_pipes[idx]));
            }
            LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                            "S$END:udf\n")));
            ...
            for (idx = 0; idx < MAX_UDF_ID_LIST; idx++) {
                ...
                LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                    "    {0x%x;2;%d;%d}\n"), ((*scache_pos)+14+(2*idx)), idx,
                    scache_tcam_p->udf_id_list[idx]));
            }
            LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                            "S$END:udf\n")));
            (*scache_pos) += sizeof(struct bcmi_xgs4_wb_tcam_info_1_4);
            scache_tcam_p++;
            tcam_info = tcam_info->next;
        }
...
STATIC int
bcmi_xgs4_udf_wb_sync_1_5(int unit, uint8 **scache_ptr, uint32 *scache_pos)
{
    int idx;
    ...
    for (idx = 0; idx < SOC_MAX_NUM_PIPES; idx++) {
        ...
        LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
            "    {0x%x;4;%d;%u}\n"), ((*scache_pos) +(idx * 4)), idx,
            (uint32)(scache_tcam_p->udf_field_bmp[idx])));
    }
    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                            "S$END:udf\n")));
    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
    ...
    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
        "    {0x%x;4,%d;}\n"),
        (int)((*scache_pos) + (sizeof(uint32)*SOC_MAX_NUM_PIPES)),
        scache_tcam_p->max_chunks));
    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                            "S$END:udf\n")));
    (*scache_pos) += sizeof(struct bcmi_xgs4_wb_tcam_info_1_5);
...
bcmi_xgs4_udf_wb_sync(int unit)
{
    ...
    BCM_IF_ERROR_RETURN
        (bcmi_xgs4_udf_wb_scache_size_get(unit, &req_scache_size));
    BCM_IF_ERROR_RETURN
...
bcmi_xgs4_udf_reinit(int unit)
{
    ...
    /* Update num packet formats */
    BCM_IF_ERROR_RETURN(bcmi_xgs4_udf_wb_pkt_format_update(unit));
