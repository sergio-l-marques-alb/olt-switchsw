    ...
    uint32 *arg)
{
    ...
    switch (type) {
        ...
        case bcmTsnTafStatusGatMaxBytesLeft:
            ...
            soc_mem_field64_get(unit, TAF_GATE_BYTES_LEFTm, &mem_entry,
                                GATE_BYTES_LEFTf, &regval_64);
            *arg = COMPILER_64_LO(regval_64);

            break;
        case bcmTsnTafStatusGatMaxBytesLeftHigh:
            SOC_IF_ERROR_RETURN(
                soc_mem_read(unit, TAF_GATE_BYTES_LEFTm, MEM_BLOCK_ANY,
                             taf_gate_id, &mem_entry));
    ...
    bcmi_tsn_taf_profile_t **candidate_profile)
{
    ...
    while (NULL != entry_iter) {
        ...
        bcmi_tsn_taf_profile_t *profile = entry_iter->taf_profile;

        BCM_IF_ERROR_RETURN(
            bcmi_gh2_taf_schedule_remove(
                unit, taf_gate_id, entry_iter->taf_profile));
        profile->status = bcmTsnTafProfileExpired;
