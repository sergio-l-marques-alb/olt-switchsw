...
STATIC
soc_reg_t ia_in_enable_regs[4] = { NS_GDPLL_IA_IN_ENABLE0r,
                                   NS_GDPLL_IA_IN_ENABLE1r,
                                   NS_GDPLL_IA_IN_ENABLE2r,
                                   NS_GDPLL_IA_IN_ENABLE3r,
                                 };
STATIC
...
_bcm_esw_gdpll_chan_enable_set(int unit, int chan, int enable)
{
   /* NS_GDPLL_IA_UPDATE_CONFIG */
    ...
    if (enable) {
        ...
        WRITE_NS_REGr(unit, ia_in_enable_regs[chan/32], 0, (1<<(chan%32)));
    }
...
int
_bcm_esw_gdpll_LoopFilter_set(int unit, int chan, int64 LoopFilter)
{
    int rv = BCM_E_NONE;
    ...
    bsl_printf("\n _bcm_esw_gdpll_LoopFilter_set: PrevLF[0x%llx]\n",
    ...
    bsl_printf("\n _bcm_esw_gdpll_LoopFilter_set: RevLF[0x%llx]\n",
                (long long unsigned int)dpllParam.dpll_state.loop_filter);
...
gdpll_intr(int unit, uint32 int_status, uint32 int_enable)
{
    ...
    while (count < 4) {
        if (soc_reg_field_get(unit, NS_TS_INT_STATUSr, int_status, gdpll_ia_status[count])) {
            ...
            READ_NS_REGr(unit, ia_in_enable_regs[count], 0, &in_enable);
            ...
            while (chan_idx < 32) {
                if ( pGdpllContext->dpll_chan[chan_num+chan_idx].enable &&
                    ...
                    WRITE_NS_REGr(unit, ia_in_enable_regs[count], 0, in_enable);
                          ...
                          bcm_gdpll_chan_t *pGdpllChan, int *pChan)
{
    ...
    if(flags & BCM_GDPLL_EVENT_CONFIG_FB) {
        if (pCurrentGdpllChan->event_fb.input_event == bcmGdpllInputEventPORT) {
            if (pCurrentGdpllChan->event_fb.port_event_type == bcmGdpllPortEventTXPI) {
                ...
                rv = _bcm_esw_gdpll_portmap_get(unit, pCurrentGdpllChan->event_fb.port, &gdpll_port, use_rx_lanemap);
                if (rv != BCM_E_NONE) {
                    goto exit;
                }
                ...
                rv = _bcm_esw_gdpll_portmap_get(unit, pCurrentGdpllChan->event_fb.port, &gdpll_port, 1);
                if (rv != BCM_E_NONE) {
                    goto exit;
                }
            }
        ...
        if(_bcm_esw_gdpll_event_config_set(unit, &(pCurrentGdpllChan->event_fb),
    ...
    if(flags & BCM_GDPLL_EVENT_CONFIG_REF) {
        if (pCurrentGdpllChan->event_ref.input_event == bcmGdpllInputEventPORT) {
            if (pCurrentGdpllChan->event_ref.port_event_type == bcmGdpllPortEventTXPI) {
                rv = _bcm_esw_gdpll_portmap_get(unit, pCurrentGdpllChan->event_ref.port, &gdpll_port, 1);
                if (rv != BCM_E_NONE) {
                    goto exit;
                }
        ...
        if (!inUse) {
            ...
            if(_bcm_esw_gdpll_event_config_set(unit, &(pCurrentGdpllChan->event_ref), eventId_ref, &iaAddr_ref) != BCM_E_NONE) {
                rv = BCM_E_INTERNAL;
                goto exit;
            }
    ...
    if(flags & BCM_GDPLL_CHAN_OUTPUT_CONF) {
        if (pCurrentGdpllChan->out_event == bcmGdpllOutputEventTXPI) {
            ...
            if(_bcm_esw_gdpll_chan_out_txpi_set(unit, chan, pCurrentGdpllChan->port) != BCM_E_NONE) {
                rv = BCM_E_INTERNAL;
                goto exit;
            }
        } else {
            if(_bcm_esw_gdpll_chan_out_misc_set(unit, chan, pCurrentGdpllChan->out_event) != BCM_E_NONE) {
                rv = BCM_E_INTERNAL;
                goto exit;
            }
        }
...
_bcm_esw_gdpll_debug_dump (int unit, dpll_dump_t *pChanDump)
{
    ...
    chan = soc_iproc_m7_read (unit, dumpAddr + SAL_OFFSETOF(dpll_dump_t, chan));
    if (chan != 0xDEAD) {
    ...
    rv = BCM_E_TIMEOUT;
    chan = pChanDump->chan;
    soc_iproc_m7_write(unit, dumpAddr + SAL_OFFSETOF(dpll_dump_t, chan), chan);
    while (poll_period_interval > 0) {
        /* Check response from M7 */
        if (0xDEAD == soc_iproc_m7_read (unit, dumpAddr + SAL_OFFSETOF(dpll_dump_t, chan))) {
