    ...
    for (ix = 0; ix < num_bins; ix++) {
        ...
        switch (bin_cmd[ix].obj_id) {
            ...
            case BCM_HASH_OBJ_INNER_MAC_DA:
                ...
                break;
            /* IPv4 Header 16 bytes. Order is reversed. The offset is from
             * the end. Covert user offset from start of IPv4 header to
             * The offset from end of IPv4 header.
             */
            case BCM_HASH_OBJ_OUTER_IPV4_HEADER:
                offset_val =
                  cmd_offset[BCM_HASH_OBJ_OFFSET_BIT124] + 1 -  /* at the end */
                  offset_val -  /* user specified offset from start */    
                  cmd_offset[BCM_HASH_OBJ_OFFSET_BIT16]; /* length of field */
            default:
                break;
        }
