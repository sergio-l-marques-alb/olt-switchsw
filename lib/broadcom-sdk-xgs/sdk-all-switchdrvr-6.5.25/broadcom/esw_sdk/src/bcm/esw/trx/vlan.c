...
/*
 * Function:
 *      _bcm_th_vlan_match_key_fields_set
 * Description   :
 *      Set Key Fields in VLAN_XLATE entry hw buffer for different view match.
 * Parameters   :
 *      unit       (IN) BCM unit number
    ...
    uint32 *vment)
{
    ...
    bcm_port_t port_out;
    soc_mem_t mem = VLAN_XLATEm;
    int kt = TD2_VLXLT_HASH_KEY_TYPE_VIF_VLAN;
    ...
    switch(match_info->match) {
        case BCM_VLAN_MATCH_PORT_ETAG_VLAN:
            soc_mem_field32_set(unit, mem, vment, VIF__SRC_VIFf,
                                match_info->etag_vlan);
            soc_mem_field32_set(unit, mem, vment, VIF__VLANf,
                                match_info->outer_vlan);
            ...
            if (BCM_GPORT_IS_SET(match_info->port)) {
                ...
                if (BCM_GPORT_IS_TRUNK(match_info->port)) {
                    ...
                    source_type = 1;
                    soc_mem_field32_set(unit, mem, vment, VIF__Tf, 1);
                    soc_mem_field32_set(unit, mem, vment, VIF__TGIDf, trunk_id);
                } else if BCM_GPORT_IS_MODPORT(match_info->port) {
                    if ((-1 == mod_out) || (-1 == port_out)) {
                        return BCM_E_PORT;
                    }
                    source_type = 1;
                    soc_mem_field32_set(unit, mem, vment, VIF__PORT_NUMf,
                                        port_out);
                    soc_mem_field32_set(unit, mem, vment, VIF__MODULE_IDf,
                                        mod_out);
                } else {
                    if (gport_id == -1) {
                        return BCM_E_PORT;
                    ...
                    source_type = 2;
                    soc_mem_field32_set(unit, mem, vment, VIF__SOURCE_FIELDf,
                                        gport_id);
                }
            } else {
                return BCM_E_PORT;
        ...
        case BCM_VLAN_MATCH_PORT_CLASS_ETAG_VLAN:
            ...
            max_bits = soc_mem_field_length(unit, mem, VIF__SOURCE_FIELDf);
    ...
    soc_mem_field32_set(unit, mem, vment, SOURCE_TYPEf, source_type);

    return rv;
}

STATIC int
_bcm_th_vlan_match_action_entry_get(
    int unit,
    void *vment,
    ...
    bcm_vlan_action_set_t *action_set)
{
    ...
    switch(soc_mem_field32_get(unit, mem, vment, SOURCE_TYPEf)) {
        case 1: /* Port or Trunk */
            if (kt == TD2_VLXLT_HASH_KEY_TYPE_VIF_VLAN) {
                match_array->match = BCM_VLAN_MATCH_PORT_ETAG_VLAN;
            }
            if (soc_mem_field32_get(unit, mem, vment, VIF__Tf)) {
                trunk_id = soc_mem_field32_get(unit, mem, vment, VIF__TGIDf);
                BCM_GPORT_TRUNK_SET(match_array->port, trunk_id);
            } else {
                port_in = soc_mem_field32_get(unit, mem, vment, VIF__PORT_NUMf);
                mod_in = soc_mem_field32_get(unit, mem, vment, VIF__MODULE_IDf);
                BCM_GPORT_MODPORT_SET(match_array->port, mod_in, port_in);
            }
            if (match_array->match == BCM_VLAN_MATCH_PORT_ETAG_VLAN) {
                match_array->etag_vlan = soc_mem_field32_get(unit, mem, vment, VIF__SRC_VIFf);
                match_array->outer_vlan = soc_mem_field32_get(unit, mem, vment, VIF__VLANf);
            }
            break;
#if defined(INCLUDE_L3)
        case 2: /* Virtual Port */
            ...
            vp = soc_mem_field32_get(unit, mem, vment, VIF__SOURCE_FIELDf);
        ...
        case 3: /* Port Group */
            ...
            match_array->port_class = soc_mem_field32_get(unit, mem, vment, VIF__SOURCE_FIELDf);
    ...
    /* Read Data Fields */
    action_set->new_outer_vlan = soc_mem_field32_get(unit, mem, vment, VIF__NEW_OVIDf);
    action_set->new_inner_vlan = soc_mem_field32_get(unit, mem, vment, VIF__NEW_IVIDf);
    if (soc_feature(unit, soc_feature_vlan_pri_cfi_action)) {
        action_set->priority = soc_mem_field32_get(unit, mem, vment, VIF__NEW_OPRIf);
        action_set->new_outer_cfi = soc_mem_field32_get(unit, mem, vment, VIF__NEW_OCFIf);
        action_set->new_inner_pkt_prio = soc_mem_field32_get(unit, mem, vment, VIF__NEW_IPRIf);
        action_set->new_inner_cfi = soc_mem_field32_get(unit, mem, vment, VIF__NEW_ICFIf);
    } else {
        action_set->priority = soc_mem_field32_get(unit, mem, vment, VIF__OPRIf);
    }

    profile_idx = soc_mem_field32_get(unit, mem, vment, VIF__TAG_ACTION_PROFILE_PTRf);
...
/*
 * Function:
 *      int _bcm_th_vlan_match_action_add
 * Description   :
 *      Add association from PORT, VLAN, ETAG to VLAN.
 *      If the entry already exists, update the action.
 * Parameters   :
 *      unit      (IN) BCM unit number
    ...
    bcm_vlan_action_set_t *action_set)
{
    ...
    bcm_port_t port_out;
    uint32 profile_idx;
    uint32 vxent[SOC_MAX_MEM_WORDS];
    uint32 res_vxent[SOC_MAX_MEM_WORDS];
    soc_mem_t mem = VLAN_XLATEm;
    int kt = TD2_VLXLT_HASH_KEY_TYPE_VIF_VLAN;
    ...
    sal_memset(&vxent, 0, sizeof(vxent));
    sal_memset(&res_vxent, 0, sizeof(res_vxent));

    /* Set Key Fields */
    BCM_IF_ERROR_RETURN
        (_bcm_th_vlan_match_key_fields_set(unit, match_info, vxent));
    soc_mem_lock(unit, mem);
    rv = soc_mem_search(unit, mem, MEM_BLOCK_ALL, &idx,
                        &vxent, &res_vxent, 0);
    ...
    if (rv == BCM_E_NONE) {
        svp_valid = FALSE;
        if (soc_mem_field32_get(unit, mem, &vxent, VIF__SVP_VALIDf)) {
            svp_valid = TRUE;
        }

        /* Return BCM_E_EXISTS directly if SVP not match */
    ...
    soc_mem_field32_set(unit, mem, &vxent, KEY_TYPEf, kt);

    /* Set SVP Port */
    if (BCM_GPORT_IS_SET(port)) {
        BCM_IF_ERROR_RETURN(_bcm_esw_gport_resolve(unit,
                    port, &mod_out, &port_out, &trunk_id, &gport_id));
        if (SOC_MEM_FIELD_VALID(unit, mem, VIF__SOURCE_VPf)) {
            soc_mem_field32_set(unit, mem, &vxent, VIF__SVP_VALIDf, 1);
            soc_mem_field32_set(unit, mem, &vxent, VIF__SOURCE_VPf, gport_id);
        }
    }
    ...
    if (action_set != NULL) {
        ...
        /* Set Data Fields */
        soc_mem_field32_set(unit, mem, &vxent, VIF__NEW_OVIDf, action_set->new_outer_vlan);
        soc_mem_field32_set(unit, mem, &vxent, VIF__NEW_IVIDf, action_set->new_inner_vlan);

        if (soc_feature(unit, soc_feature_vlan_pri_cfi_action)) {
            soc_mem_field32_set(unit, mem, &vxent, VIF__NEW_OPRIf, action_set->priority);
            soc_mem_field32_set(unit, mem, &vxent, VIF__NEW_OCFIf, action_set->new_outer_cfi);
            soc_mem_field32_set(unit, mem, &vxent, VIF__NEW_IPRIf, action_set->new_inner_pkt_prio);
            soc_mem_field32_set(unit, mem, &vxent, VIF__NEW_ICFIf, action_set->new_inner_cfi);
        } else {
            if (action_set->priority >= BCM_PRIO_MIN &&
                    action_set->priority <= BCM_PRIO_MAX) {
                soc_mem_field32_set(unit, mem, &vxent, VIF__OPRIf, action_set->priority);
            }
        }

        soc_mem_field32_set(unit, mem, &vxent, VIF__TAG_ACTION_PROFILE_PTRf, profile_idx);

        if (SOC_MEM_FIELD_VALID(unit, mem, VIF__VLAN_ACTION_VALIDf)) {
            soc_mem_field32_set(unit, mem, &vxent, VIF__VLAN_ACTION_VALIDf, 1);
        }
    }
    soc_mem_field32_set(unit, mem, &vxent, VALIDf, 1);

    /* Insert to VLAN_XLATE */
    rv = soc_mem_insert_return_old(unit, mem,
            MEM_BLOCK_ALL, &vxent, &res_vxent);
    if (rv == SOC_E_EXISTS) {
        /* Delete the old vlan action profile entry */
        if (soc_mem_field32_get(unit, mem, &res_vxent, VIF__VLAN_ACTION_VALIDf)) {
            profile_idx = soc_mem_field32_get(unit, mem, &res_vxent, VIF__TAG_ACTION_PROFILE_PTRf);
            rv = _bcm_trx_vlan_action_profile_entry_delete(unit, profile_idx);
        }
    }
...
/*
 * Function:
 *      _bcm_th_vlan_match_action_delete
 * Purpose:
    ...
    bcm_vlan_match_info_t *vlan_match_info)
{
    ...
    /* Set Key fields */
    BCM_IF_ERROR_RETURN
        (_bcm_th_vlan_match_key_fields_set(unit, vlan_match_info, vxent));
    ...
    /* Evaluate SVP */
    matched = FALSE;
    svp_valid = soc_mem_field32_get(unit, mem, &res_vxent, VIF__SVP_VALIDf);
    if ((port != BCM_GPORT_INVALID) && (svp_valid == TRUE)) {
        vp = soc_mem_field32_get(unit, mem, &res_vxent, VIF__SOURCE_VPf);
    ...
    if (rv == BCM_E_NONE &&
       (soc_mem_field32_get(unit, mem,
                     &vxent, VIF__VLAN_ACTION_VALIDf))) {
            /* Read action profile data. */
            profile_idx = soc_mem_field32_get(unit, mem,
                    &vxent, VIF__TAG_ACTION_PROFILE_PTRf);
...
/*
 * Function:
 *      _bcm_th_vlan_match_action_delete_all
 * Purpose:
    ...
    bcm_gport_t port)
{
    ...
    uint32 profile_idx;
    uint8 *vmtab = NULL; /* VLAN_XLATE DMA buffer */
    void *vment;
    int key_type_value, key_type;
    bcm_vlan_match_info_t match_info;
    soc_mem_t mem = VLAN_XLATEm;
    ...
    vmtab = soc_cm_salloc(unit, nent * vmbytes, "VLAN_XLATE");
    ...
    soc_mem_lock(unit, mem);
    for(i = 0; i < nent; i++) {
        vment = soc_mem_table_idx_to_pointer(unit, mem, void *, vmtab, i);

        if (!soc_mem_field32_get(unit, mem, vment, VALIDf)) {
            continue;
        }
        ...
        switch(key_type) {
            case VLXLT_HASH_KEY_TYPE_VIF_VLAN:
                ...
                (void) _bcm_th_vlan_match_action_entry_get(unit,
                ...
                /* Evaluate SVP */
                matched = FALSE;
                svp_valid = soc_mem_field32_get(unit, mem, vment, VIF__SVP_VALIDf);
                if ((port != BCM_GPORT_INVALID) && (svp_valid == TRUE)) {
                    vp = soc_mem_field32_get(unit, mem, vment, VIF__SOURCE_VPf);
                    matched = _bcm_esw_port_vp_gport_check(port, vp);
                } else if ((port == BCM_GPORT_INVALID) && (svp_valid == FALSE)) {
                    matched = TRUE;
                } else {
        ...
        if (soc_mem_field32_get(unit, mem, vment, VIF__VLAN_ACTION_VALIDf)) {
            /* Read action profile data. */
            profile_idx = soc_mem_field32_get(unit, mem, vment, VIF__TAG_ACTION_PROFILE_PTRf);
            _bcm_trx_vlan_action_profile_entry_delete(unit, profile_idx);
        }
    ...
    bcm_vlan_action_set_t *action_set)
{
    ...
    uint32 vxent[SOC_MAX_MEM_WORDS];     /* Lookup key hw buffer.      */
    uint32 res_vxent[SOC_MAX_MEM_WORDS]; /* Lookup result buffer.      */
    uint32 profile_idx;                 /* Vlan action profile index. */
    int rv;                             /* Operation return status.   */
    int idx = 0;                        /* Lookup result entry index. */
    soc_mem_t mem = VLAN_XLATEm;
    ...
    if (soc_mem_field32_get(unit, mem, &res_vxent, VIF__VLAN_ACTION_VALIDf)) {
        ...
        action_set->new_outer_vlan = soc_mem_field32_get(unit, mem, &res_vxent, VIF__NEW_OVIDf);
        action_set->new_inner_vlan = soc_mem_field32_get(unit, mem, &res_vxent, VIF__NEW_IVIDf);
        if (soc_feature(unit, soc_feature_vlan_pri_cfi_action)) {
            action_set->priority = soc_mem_field32_get(unit, mem, &res_vxent, VIF__NEW_OPRIf);
            action_set->new_outer_cfi = soc_mem_field32_get(unit, mem, &res_vxent, VIF__NEW_OCFIf);
            action_set->new_inner_pkt_prio = soc_mem_field32_get(unit, mem, &res_vxent, VIF__NEW_IPRIf);
            action_set->new_inner_cfi = soc_mem_field32_get(unit, mem, &res_vxent, VIF__NEW_ICFIf);
        } else {
            action_set->priority = soc_mem_field32_get(unit, mem, &res_vxent, VIF__OPRIf);
        }
...
#if defined(BCM_TOMAHAWK_SUPPORT)
    if ((SOC_IS_TOMAHAWKX(unit))&&
        (match_info->match == BCM_VLAN_MATCH_PORT_ETAG_VLAN ||
         match_info->match == BCM_VLAN_MATCH_PORT_CLASS_ETAG_VLAN)) {
        return _bcm_th_vlan_match_action_add(unit, options, port, match_info,
                                              action_set);
    }
#endif
    ...
    bcm_vlan_action_set_t *action_set)
{
    ...
    if ((SOC_IS_TOMAHAWKX(unit))&&
        (match_info->match == BCM_VLAN_MATCH_PORT_ETAG_VLAN ||
         match_info->match == BCM_VLAN_MATCH_PORT_CLASS_ETAG_VLAN)) {
        return _bcm_th_vlan_match_action_get(unit, port, match_info,
                                              action_set);
    }
#endif
...
#if defined(BCM_TOMAHAWK_SUPPORT)
    if ((SOC_IS_TOMAHAWKX(unit))&&
        (vlan_match_info->match == BCM_VLAN_MATCH_PORT_ETAG_VLAN ||
         vlan_match_info->match == BCM_VLAN_MATCH_PORT_CLASS_ETAG_VLAN)) {
        return  _bcm_th_vlan_match_action_delete(unit, port, vlan_match_info);
    }
#endif
                ...
                bcm_vlan_match_t match, bcm_gport_t port)
{
    ...
    if ((match != BCM_VLAN_MATCH_PORT_ETAG_VLAN) &&
         (match != BCM_VLAN_MATCH_PORT_CLASS_ETAG_VLAN)) {
        if (!SOC_MEM_IS_VALID(unit, VLAN_MACm)) {
            return BCM_E_UNAVAIL;
        }
...
#if defined(BCM_TOMAHAWK_SUPPORT)
    if ((SOC_IS_TOMAHAWKX(unit))&&
        ((match == BCM_VLAN_MATCH_PORT_ETAG_VLAN) ||
         (match == BCM_VLAN_MATCH_PORT_CLASS_ETAG_VLAN) ||
         (match == BCM_VLAN_MATCH_INVALID))) {
        rv = _bcm_th_vlan_match_action_delete_all(unit, match, port);
    }
#endif
