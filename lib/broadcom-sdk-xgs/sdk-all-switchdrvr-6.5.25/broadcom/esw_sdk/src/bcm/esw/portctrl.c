                                               ...
                                               int *valid_abilities)
{
    ...
    for (i = 0; i < num_ability; i++) {
        ...
        if (valid >= max_num_abilities) {
            ...
            port_abilities[valid].fec_type =
                (bcm_port_phy_fec_t) portmod_abilities[i].fec_type;
            port_abilities[valid].medium = portmod_abilities[i].medium;
            port_abilities[valid].pause =
                (bcm_port_phy_pause_t) portmod_abilities[i].pause;
            port_abilities[valid].channel =
                (bcm_port_phy_channel_t) portmod_abilities[i].channel;
            /* coverity[mixed_enums:FALSE] */
            port_abilities[valid].an_mode =
                (bcm_port_autoneg_mode_t) portmod_abilities[i].an_mode;
        }
                                               ...
                                               portmod_port_speed_ability_t *portmod_abilities)
{
    ...
    for (i = 0; i < num_ability; i++) {
        ...
        portmod_abilities[i].fec_type =
            (portmod_fec_t) port_abilities[i].fec_type;
        portmod_abilities[i].medium = port_abilities[i].medium;
        portmod_abilities[i].pause =
            (portmod_port_phy_pause_t) port_abilities[i].pause;
        portmod_abilities[i].channel =
            (portmod_port_phy_channel_t) port_abilities[i].channel;
        /* coverity[mixed_enums:FALSE] */
        portmod_abilities[i].an_mode =
            (portmod_port_phy_control_autoneg_mode_t) port_abilities[i].an_mode;
    }
}
                                       ...
                                       portmod_speed_config_t *sp_cfg)
{
    ...
    rv = portmod_port_enable_get(unit, port, PORTMOD_PORT_ENABLE_MAC, &enable);
    ...
    rv = portmod_port_speed_config_set(unit, pport, sp_cfg);
    if (PORTMOD_FAILURE(rv)) {
        goto exit;
    }
...
#ifdef BCM_TRIDENT3_SUPPORT
        if (soc_feature(unit, soc_feature_hgoe_line_card_mode)) {
    ...
    if (pm_type == portmodDispatchTypePm8x50) {
        if (an_status.enabled && an_status.locked) {
            ...
            if (num_advert && num_remote) {
                pause_local = (portmod_port_phy_pause_t) local_advert[0].pause;
                pause_remote =
                    (portmod_port_phy_pause_t) remote_advert[0].pause;
                if (
...
bcmi_esw_portctrl_fault_get(int unit, bcm_gport_t port, uint32 *flags)
{
...
#ifdef BCM_TOMAHAWK3_SUPPORT
    portmod_port_fault_status_t dual_link_status;
#endif
    ...
    if (SOC_IS_TOMAHAWK3(unit)) {
        PORTMOD_IF_ERROR_RETURN(
            portmod_port_local_fault_control_get(unit, pport, &local_control));
        PORTMOD_IF_ERROR_RETURN(
            portmod_port_remote_fault_control_get(unit, pport, &remote_control));
        if (local_control.enable || remote_control.enable) {
            rv = portmod_port_faults_status_get(unit, pport, &dual_link_status);
            if (PORTMOD_SUCCESS(rv)) {
                if (local_control.enable) {
                    local_fault = dual_link_status.local;
                }
                if (remote_control.enable) {
                    remote_fault = dual_link_status.remote;
                }
            }
        }
    } else
                                             ...
                                             portctrl_pport_t pport, int speed)
{
    ...
    if (sys_interface != SOC_PORT_IF_COUNT) {
        ...
        bcmi_esw_portctrl_phy_control_set(unit, gport,
            _SHR_PORT_PHY_CONTROL_INTERFACE, sys_interface);
    }
                            ...
                            bcm_port_resource_t *resource, bcm_pbmp_t *pbmp)
{
    ...
    sp_config.fec = (portmod_fec_t) resource->fec_type;
                                  ...
                                  bcm_port_resource_t *resource)
{
    ...
    for (i = 0; i < nport; i++) {
        ...
        if (pm_type == portmodDispatchTypePm8x50) {
            ...
            sp_cfg.fec = (portmod_fec_t) resource[i].fec_type;
...
/*
 * Function:
 *      bcmi_esw_portctrl_phy_multicast_set
 * Description:
 *      General PHY register write to multiple buses. This is used to
 *      broadcast write transaction to a specific set of MDIO bus which
 *      is set in mdio_buses parameter.
 * Parameters:
 *      unit - Device number
 *      port - Port number or PHY MDIO address (refer BCM_PORT_PHY_NOMAP).
 *             This parameter is used to get the PHY address which will
 *             be set in MDIO command.
 *.     mdio_buses - Bitmap value for MDIO buses. All the buses listed
 *             in this parameter will receive the same MDIO write command.
 *             Bit 0 represents bus 0, bit 1 represents bus 1, and so on.
 *      flags - Logical OR of one or more of the following flags:
 *              BCM_PORT_PHY_INTERNAL
 *                      Address internal SERDES PHY for port
 *              BCM_PORT_PHY_NOMAP
 ...
 */
int
bcmi_esw_portctrl_phy_multicast_set(int unit, bcm_port_t port, uint32 mdio_buses,
                    uint32 flags, uint32 phy_reg_addr, uint32 phy_data)

{
    ...
    LOG_INFO(BSL_LS_BCM_PORT,
             (BSL_META_U(unit,
                         "portctrl_phy_multicast_set: u=%d p=%d buses=0x%.4x flags=0x%08x "
                         "phy_reg=0x%08x phy_data=0x%08x\n"),
                         unit, port, mdio_buses, flags, phy_reg_addr, phy_data));
    ...
    /* Set mdio_buses in the first 16 bits of phy_id */
    phy_id |= (uint32)(mdio_buses << 16);
    flags |= (BCM_PORT_PHY_NOMAP | BCM_PORT_PHY_BROADCAST_WRITE);

    return bcmi_esw_portctrl_phy_set(unit, phy_id, flags, phy_reg_addr, phy_data);
#else  /* PORTMOD_SUPPORT */
    return BCM_E_UNAVAIL;
#endif /* PORTMOD_SUPPORT */
}
...
#if defined(BCM_TRIDENT3_SUPPORT)
    if (SOC_IS_TRIDENT3X(unit) &&
        soc_feature(unit, soc_feature_higig3) &&
        bcm_td3_is_hg3_enabled_port(unit, port) &&
        mode == BCM_PORT_ENCAP_IEEE) {
        xport_swap = TRUE;
    }

    if (soc_feature(unit, soc_feature_hgoe_line_card_mode) &&
        _bcm_esw_is_hgoe_port(unit, port) &&
        mode == BCM_PORT_ENCAP_IEEE) {
        xport_swap = TRUE;
    }
#endif
...
bcmi_esw_portctrl_encap_set(int unit, bcm_gport_t port, int mode, int force)
{
    ...
    if (soc_feature(unit, soc_feature_hgoe_line_card_mode)) {
        if (mode == BCM_PORT_ENCAP_HIGIG_OVER_ETHERNET) {
            ...
            config.encap = mode;
            return (bcm_esw_port_encap_config_set(unit, port, &config));
        }
    }
#endif
    BCM_IF_ERROR_RETURN(
...
bcmi_esw_portctrl_link_get(int unit, bcm_gport_t port, int hw, int *up)
{
    ...
    if (hw) {
        ...
        *up = PBMP_MEMBER(hw_linkstat, port);
         ...
         */
        if (!(*up) && !soc_feature(unit, soc_feature_cmicx)) {
                                          ...
                                          int value)
{
    ...
    if (SOC_IS_FIREBOLT6(unit)) {
        /* For firebolt6, pass the value user passed. pfc_enable will also
         * hold that value for pfc_control.stats_en below */
        pfc_enable = value;

        SOC_IF_ERROR_RETURN
            (soc_firebolt6_mmu_pfc_tx_config_set(unit, port,
                                                pfc_enable));
    }
#endif
#if defined(BCM_HELIX5_SUPPORT)
    if (SOC_IS_HELIX5(unit)) {
        /* For helix5, pass the value user passed. pfc_enable will also
         * hold that value for pfc_control.stats_en below */
        pfc_enable = value;

        SOC_IF_ERROR_RETURN
            (soc_helix5_mmu_pfc_tx_config_set(unit, port,
                                                pfc_enable));
    }
#endif
#if defined(BCM_MAVERICK2_SUPPORT)
    if (SOC_IS_MAVERICK2(unit)) {
        /* For maverick2, pass the value user passed. pfc_enable will also
         * hold that value for pfc_control.stats_en below */
        pfc_enable = value;

        SOC_IF_ERROR_RETURN
            (soc_maverick2_mmu_pfc_tx_config_set(unit, port,
                                                pfc_enable));
    }
#endif
#if defined(BCM_TRIDENT3_SUPPORT)
    if (SOC_IS_TRIDENT3(unit)) {
        /* For trident3, pass the value user passed. pfc_enable will also
         * hold that value for pfc_control.stats_en below */
        pfc_enable = value;

        SOC_IF_ERROR_RETURN
            (soc_trident3_mmu_pfc_tx_config_set(unit, port,
                                                pfc_enable));
    }
#endif
    return BCM_E_NONE;
                                           ...
                                           int value)
{
    ...
    if (SOC_IS_TOMAHAWK2(unit)) {
        SOC_IF_ERROR_RETURN
            (bcm_th2_mmu_pfc_tx_config_set(unit, port, value));
    }
#endif
#if defined(BCM_TRIDENT3_SUPPORT)
    if (SOC_IS_TRIDENT3(unit)) {
                                         ...
                                         int value)
{
    ...
    for (idx = 0; idx < sizeof(def_regs) / sizeof(def_regs[0]); idx++) {
        ...
        SOC_BLOCKS_ITER(unit, blk, SOC_REG_INFO(unit, def_regs[idx]).block) {
 ...
 */
    ...
    case bcmPortControlTxStall:
        ...
        if (SOC_IS_TRIDENT2(unit) || SOC_IS_TRIDENT3X(unit)
            ...
            || SOC_IS_TOMAHAWK3(unit) || SOC_IS_FIRELIGHT(unit)) {
            if (value) {
                /* Enabling tx stall */
                BCM_IF_ERROR_RETURN(_bcmi_tx_stall_set_port_rate_egress_settings(
                                    unit, port, 1));
            } else {
                /* Disabling tx stall */
                BCM_IF_ERROR_RETURN(_bcmi_tx_stall_set_port_rate_egress_settings(
    ...
    case bcmPortControlPortmodLinkRecoveryEnable:
        PORT_LOCK(unit);
        rv = portmod_port_link_recovery_enable_set(unit, pport, value);
        PORT_UNLOCK(unit);
        break;
 ...
 */
    ...
    case bcmPortControlTxStall:
        ...
        if (SOC_IS_TRIDENT2(unit) || SOC_IS_TRIDENT3X(unit)
            || SOC_IS_TOMAHAWK(unit) || SOC_IS_TOMAHAWK2(unit)
            || SOC_IS_TOMAHAWK3(unit) || SOC_IS_FIRELIGHT(unit)) {
            uint32 refresh, thd_sel;

            BCM_IF_ERROR_RETURN(
                _bcmi_port_tx_stall_egress_meter_hw_get(unit, port, &refresh,
                                    &thd_sel));

            /* if tx stall is enabled, set value to 1 */
            if ((0 == refresh) && (1 == thd_sel)) {
                *value = 1;
            } else {
                *value = 0;
            }
    ...
    case bcmPortControlPortmodLinkRecoveryEnable:
        PORT_LOCK(unit);
        rv = portmod_port_link_recovery_enable_get(unit, pport, value);
        PORT_UNLOCK(unit);
        break;
                                 ...
                                 bcm_port_phy_tx_t *tx)
{
    ...
    soc_phy_tx_copy.signalling_mode =
        (_shr_port_phy_signalling_mode_t) tx->signalling_mode;
    soc_phy_tx_copy.tx_tap_mode = (_shr_port_phy_tx_tap_mode_t) tx->tx_tap_mode;
                                 ...
                                 bcm_port_phy_tx_t *tx)
{
    ...
    tx->pre2 = phy_tx.pre2;
    tx->signalling_mode =
        (bcm_port_phy_signalling_mode_t) phy_tx.signalling_mode;
    tx->tx_tap_mode = (bcm_port_phy_tx_tap_mode_t) phy_tx.tx_tap_mode;
...
bcmi_esw_portctrl_post_ber_proj_get(int unit, bcm_gport_t port,
    ...
    args.ber_proj_fec_type = (_shr_port_phy_fec_t) ber_proj->fec_type;
...
/*
 * Function:
 *      bcmi_esw_portctrl_flr_set
 * Purpose:
 *      Set flr for a given port
 * Parameters:
 *      unit - Unit #
 *      port - GPORT #
 *      enable - flr enable
 *      timeout- flr timeout time
 * Returns:
 *      BCM_E_NONE
 *      BCM_E_XXX
 */
int
bcmi_esw_portctrl_flr_set(int unit, bcm_port_t port, int enable, bcm_port_flr_timeout_t timeout)
{
#ifdef PORTMOD_SUPPORT
    int rv;
    portctrl_pport_t pport;
    ...
    PORT_LOCK(unit);
    switch (timeout) {
        case bcmPortFlrTimeoutInvaild:
            rv = portmod_port_flr_config_set(unit, pport, enable,
                                             portmodFlrTimeoutInvaild);
            if (BCM_SUCCESS(rv)) {
                rv = soc_linkscan_flr_enable_set(unit, port, enable,
                                                 socLinkscanFlrTimeoutInvalid);
            }
            break;
        case bcmPortFlrTimeout50ms:
            rv = portmod_port_flr_config_set(unit, pport, enable,
                                             portmodFlrTimeout50ms);
            if (BCM_SUCCESS(rv)) {
                rv = soc_linkscan_flr_enable_set(unit, port, enable,
                                                 socLinkscanFlrTimeout50ms);
            }
            break;
        case bcmPortFlrTimeout75ms:
            rv = portmod_port_flr_config_set(unit, pport, enable,
                                             portmodFlrTimeout75ms);
            if (BCM_SUCCESS(rv)) {
                rv = soc_linkscan_flr_enable_set(unit, port, enable,
                                                 socLinkscanFlrTimeout75ms);
            }
            break;
        case bcmPortFlrTimeout100ms:
            rv = portmod_port_flr_config_set(unit, pport, enable,
                                             portmodFlrTimeout100ms);
            if (BCM_SUCCESS(rv)) {
                rv = soc_linkscan_flr_enable_set(unit, port, enable,
                                                 socLinkscanFlrTimeout100ms);
            }
            break;
        case bcmPortFlrTimeout150ms:
            rv = portmod_port_flr_config_set(unit, pport, enable,
                                             portmodFlrTimeout150ms);
            if (BCM_SUCCESS(rv)) {
                rv = soc_linkscan_flr_enable_set(unit, port, enable,
                                                 socLinkscanFlrTimeout150ms);
            }
            break;
        case bcmPortFlrTimeout250ms:
            rv = portmod_port_flr_config_set(unit, pport, enable,
                                             portmodFlrTimeout250ms);
            if (BCM_SUCCESS(rv)) {
                rv = soc_linkscan_flr_enable_set(unit, port, enable,
                                                 socLinkscanFlrTimeout250ms);
            }
            break;
        case bcmPortFlrTimeout500ms:
            rv = portmod_port_flr_config_set(unit, pport, enable,
                                             portmodFlrTimeout500ms);
            if (BCM_SUCCESS(rv)) {
                rv = soc_linkscan_flr_enable_set(unit, port, enable,
                                                 socLinkscanFlrTimeout500ms);
            }
            break;
        default:
            rv = BCM_E_PARAM;
    ...
    LOG_INFO(BSL_LS_BCM_PORT,
             (BSL_META_UP(unit, port,
                          "flr enable set: u=%d p=%d enable=%d timeout=%d rv=%d\n"),
              unit, port, enable, timeout, rv));
...
/*
 * Function:
 *     bcmi_esw_portctrl_pmd_rx_lock_status_get
 * Purpose:
 *     Get PMD RX lock status
 * Parameters:
 *      unit - (IN) Unit number.
 *      port - (IN) Port.
 *      rx_lock_bmp - (OUT) The lane bitmap indicates PMD RX lock status
 *      rx_lock_change_bmp - (OUT) The lane bitmap indicates PMD RX lock status has changed since last read
 * Returns:
 *      BCM_E_xxx
 * Notes:
 */
int
bcmi_esw_portctrl_pmd_rx_lock_status_get(int unit, bcm_port_t port,
                                                     uint32* rx_lock_bmp, uint32* rx_lock_change_bmp)
{
#ifdef PORTMOD_SUPPORT
    int rv;
    portctrl_pport_t pport;
    ...
    PORT_LOCK(unit);
    rv = portmod_port_pmd_rx_lock_status_get(unit, pport, rx_lock_bmp, rx_lock_change_bmp);;
    PORT_UNLOCK(unit);
