...
/* The Group flags and group_event_types sync info for wb recovery */
typedef struct _kt2_oam_group_event_types_sync_info_s {
    uint32                flags;              /* Group flags */
    bcm_oam_event_types_t group_event_types;  /* Group event types */
} _kt2_oam_group_event_types_sync_info_t;

/* The rmep flags2 and rmep_event_types sync info for wb recovery */
typedef struct _kt2_oam_rmep_event_types_sync_info_s {
    bcm_oam_endpoint_t    ep_id;             /* Endpoint ID     */
    uint32                flags2;            /* Endpoint flags2 */
    bcm_oam_event_types_t rmep_event_types;  /* Endpoint rmep_event_types */
} _kt2_oam_rmep_event_types_sync_info_t;
...
/* Endpoint flags2 info indexed with ma index */
#define _BCM_KT2_OAM_RX_DOWNMEP_EP_FLAGS2_SCACHE_SIZE \
                (sizeof(uint32) * (oc->ma_idx_count))
#define _BCM_KT2_OAM_RX_DOWNMEP_EP_FLAGS2_SCACHE_LOCATION(mep_index) \
                (sizeof(uint32) * mep_index)
#define _BCM_KT2_OAM_RX_UPMEP_EP_FLAGS2_SCACHE_SIZE \
                (sizeof(uint32) * (oc->egr_ma_idx_count))
#define _BCM_KT2_OAM_RX_UPMEP_EP_FLAGS2_SCACHE_LOCATION(mep_index) \
                (sizeof(uint32) * (oc->ma_idx_count + mep_index))
/* Group's group_event_type and flags info with group index */
#define _BCM_KT2_OAM_GROUP_EVENT_TYPES_INFO_SCACHE_SIZE \
                (sizeof(_kt2_oam_group_event_types_sync_info_t) * (oc->group_count))
#define _BCM_KT2_OAM_GROUP_EVENT_TYPES_INFO_SCACHE_LOCATION(group_index) \
                (sizeof(_kt2_oam_group_event_types_sync_info_t) * (group_index))
/* Endpoint's rmep_event_type and flags2 info with rmep index */
#define _BCM_KT2_OAM_RMEP_EVENT_TYPES_INFO_SCACHE_SIZE \
                (sizeof(_kt2_oam_rmep_event_types_sync_info_t) * (oc->rmep_count))
#define _BCM_KT2_OAM_RMEP_EVENT_TYPES_INFO_SCACHE_LOCATION(rmep_index) \
                (sizeof(_kt2_oam_rmep_event_types_sync_info_t) * (rmep_index))

#define _BCM_KT2_OAM_OPCODE_GROUP_BMP_SIZE \
                (sizeof(uint32) * (8))
...
_bcm_kt2_oam_profile_tables_init(int unit, _bcm_oam_control_t *oc)
{
    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        for (pri = 0; pri < BCM_OAM_INTPRI_MAX; pri++)
        {
            ...
            if (BCM_FAILURE(rv)) {
                LOG_ERROR(BSL_LS_BCM_OAM,
                        (BSL_META_U(unit,
                                    "OAM Error: ing service pri map ref count "
                                    "increment failed - %s.\n"), bcm_errmsg(rv)));
                return (rv);
            }
        }
    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        for (pri = 0; pri < BCM_OAM_INTPRI_MAX; pri++)
        {
            rv = soc_profile_mem_reference(unit, &oc->egr_service_pri_map,
                    pri, 1);
            if (BCM_FAILURE(rv)) {
                LOG_ERROR(BSL_LS_BCM_OAM,
                        (BSL_META_U(unit,
                                    "OAM Error: egr service pri map ref count "
                                    "increment failed - %s.\n"), bcm_errmsg(rv)));
                return (rv);
            }
        }
    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                    (BSL_META_U(unit,
                                "OAM Error: oam opcode control profile ref count "
                                "increment failed - %s.\n"), bcm_errmsg(rv)));
            return (rv);
        }
    }
    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                    (BSL_META_U(unit,
                                "OAM Error: egr oam opcode control profile ref count "
                                "increment failed - %s.\n"), bcm_errmsg(rv)));
            return (rv);
        }
    }
    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                    (BSL_META_U(unit,
                                "OAM Error: ing oam dglp profile ref count "
                                "increment failed - %s.\n"), bcm_errmsg(rv)));
            return (rv);
        }
    }
    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                    (BSL_META_U(unit,
                                "OAM Error: egr oam dglp profile ref count "
                                "increment failed - %s.\n"), bcm_errmsg(rv)));
            return (rv);
        }
    }
...
_bcm_kt2_oam_handle_interrupt(int unit, soc_field_t field)
{
    ...
    _bcm_oam_group_data_t   *g_info_p = NULL;
    _bcm_oam_hash_data_t    *h_data_p = NULL;
    int                     group_cb_enabled;
    int                     rmep_cb_enabled;
    int                     event_notify;
    ...
    for (intr = _kt2_oam_interrupts; intr->status_register != INVALIDr;
        ...
        if ((1 == intr_cur_status) && (intr_count > 0)) {
            group_cb_enabled = FALSE;
            rmep_cb_enabled  = FALSE;
            flags = 0;
            ...
            if (grp_index != BCM_OAM_GROUP_INVALID) {
                g_info_p = &oc->group_info[grp_index];
                if (g_info_p->flags & BCM_OAM_GROUP_CALLBACK_ENABLE_EVENTS) {
                    group_cb_enabled = TRUE;
                }
            }

            if (remote_ep_index != BCM_OAM_ENDPOINT_INVALID) {
                h_data_p = &oc->oam_hash_data[remote_ep_index];
                if (h_data_p->flags2 & BCM_OAM_ENDPOINT_FLAGS2_CALLBACK_ENABLE_EVENTS) {
                    rmep_cb_enabled = TRUE;
                }
            }
            ...
            for (e_handler = oc->event_handler_list_p; e_handler != NULL;
                ...
                if (SHR_BITGET(e_handler->event_types.w, intr->event_type)) {
                    ...
                    if ((intr->event_type == bcmOAMEventEndpointInterfaceUp) ||
                        ...
                        switch (rmep_last_interface_status) {
                            case _BCM_OAM_TLV_INTERFACE_UP:
                                intr->event_type = bcmOAMEventEndpointInterfaceUp;
                                break;
                    ...
                    if ((group_cb_enabled) &&
                        (SHR_BITGET(g_info_p->group_event_types.w, intr->event_type))) {
                        event_notify = TRUE;
                    }

                    if ((rmep_cb_enabled) &&
                        (SHR_BITGET(h_data_p->rmep_event_types.w, intr->event_type))) {
                        event_notify = TRUE;
                    }

                    if ((group_cb_enabled || rmep_cb_enabled) && !event_notify) {
                        if (flags & BCM_OAM_EVENT_FLAGS_MULTIPLE) {
                            flags |= BCM_OAM_EVENT_FLAGS_IGNORE_GROUP_RMEP_ID;
                        } else {
                            continue;
                        }
                    }
                       ...
                       bcm_oam_group_info_t *group_info)
{
    ...
    if (group_p[group_index].flags & BCM_OAM_GROUP_CALLBACK_ENABLE_EVENTS) {
        group_info->flags |=  BCM_OAM_GROUP_CALLBACK_ENABLE_EVENTS;
    }

    group_info->group_event_types = group_p[group_index].group_event_types;
                                    ...
                                    _bcm_oam_hash_data_t *h_data_p)
{
    ...
    if ((h_data_p->trunk_id != BCM_TRUNK_INVALID)) {
        ...
        rv = (bcm_esw_trunk_get(unit, h_data_p->trunk_id,
                               NULL, 0, NULL, &member_count));

        if ((BCM_FAILURE(rv)) && !(h_data_p->flags2 & BCM_OAM_ENDPOINT_FLAGS2_TRUNK_WITHOUT_MEMBERS)) {
            return rv;
        }

        if (0 == member_count) {
            if (!(h_data_p->flags2 & BCM_OAM_ENDPOINT_FLAGS2_TRUNK_WITHOUT_MEMBERS)) {
                /* No members have been added to the trunk group yet */
                LOG_ERROR(BSL_LS_BCM_OAM,
                          (BSL_META_U(unit,
                                      "OAM Error: No local members have been added to "
                                      "the trunk group yet - %s.\n"), bcm_errmsg(rv)));
                return BCM_E_PARAM;
            } else if ((h_data_p->flags2 & BCM_OAM_ENDPOINT_FLAGS2_TRUNK_WITHOUT_MEMBERS)
                       && ((h_data_p->vlan == 0) && (h_data_p->inner_vlan == 0))) {
                /* For PORT based mep this feature can not be supported */
                LOG_ERROR(BSL_LS_BCM_OAM,
                          (BSL_META_U(unit,
                           "OAM(unit %d) Error: No support for port based mep to "
                           "create endpoint trunk without members - %s.\n"), unit, bcm_errmsg(rv)));
                return BCM_E_PARAM;
            }
        }
 ...
 * Parameters:
 *     unit        - (IN) BCM device number
 *     flags2      - (IN) endpoints flags2
 *     is_port_mep - (IN) PORT based MEP flag
 ...
 */
STATIC int
_bcm_kt2_oam_trunk_resolve(int unit, uint32 flags2, uint8 is_port_mep, bcm_trunk_t trunk_id, int trunk_index,
                           ...
                           uint32 *src_glp, uint32 *dst_glp, bcm_gport_t *tx_gport)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_esw_trunk_get(unit, trunk_id, NULL, 0,
                                          NULL, &member_count));
    if (member_count == 0) {
        if (!(flags2 & BCM_OAM_ENDPOINT_FLAGS2_TRUNK_WITHOUT_MEMBERS)) {
            return BCM_E_PARAM;
        } else if ((flags2 & BCM_OAM_ENDPOINT_FLAGS2_TRUNK_WITHOUT_MEMBERS)
                   && (is_port_mep)) {
            /* For PORT based mep this feature can not be supported */
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                                    ...
                                    bcm_gport_t *tx_gport)
{
    ...
    if (BCM_GPORT_IS_TRUNK(ep_info_p->gport)) {
        ...
        BCM_IF_ERROR_RETURN(
              _bcm_kt2_oam_trunk_resolve(unit, ep_info_p->flags2, is_port_mep, *trunk_id, ep_info_p->trunk_index,
...
/*
 * Function:
 *     _bcm_kt2_oam_endpoint_rmep_event_types_validate
 * Purpose:
 *     Validate an endpoint rmep event types.
 * Parameters:
 *     rmep_event_types - (IN) rmep event types.
 * Retruns:
 *     BCM_E_XXX
 */
STATIC int
_bcm_kt2_oam_endpoint_rmep_event_types_validate(bcm_oam_event_types_t rmep_event_types)
{
    bcm_oam_event_type_t e_type;    /* Event type */

    for (e_type = 0; e_type < bcmOAMEventCount; ++e_type) {
       /* Following are Group level events, these events has MA index and no RMEP index.
          These events should not be set to rmep event types*/
        if (SHR_BITGET(rmep_event_types.w, e_type)) {
            if ((e_type == bcmOAMEventGroupCCMxcon) ||
...
/*
 * Function:
 *     _bcm_kt2_oam_group_event_types_validate
 * Purpose:
 *     Validate group event types.
 * Parameters:
 *     group_event_types - (IN) group event types.
 * Retruns:
 *     BCM_E_XXX
 */
STATIC int
_bcm_kt2_oam_group_event_types_validate(bcm_oam_event_types_t group_event_types)
{
    bcm_oam_event_type_t e_type;    /* Event type */

    for (e_type = 0; e_type < bcmOAMEventCount; ++e_type) {
        /* Following are RMEP level events, these events has both MA index and RMEP index.
           These events should not be set to group event types */
        if (SHR_BITGET(group_event_types.w, e_type)) {
            if ((e_type == bcmOAMEventGroupRemote) ||
                (e_type == bcmOAMEventGroupCCMTimeout) ||
                (e_type == bcmOAMEventEndpointPortDown) ||
...
_bcm_kt2_oam_sw_dump(int unit)
{
    ...
    LOG_CLI((BSL_META_U(unit,"OAM opcode group bitmap\n")));
    for (idx = 0; idx < 8; ++idx) {
        LOG_CLI((BSL_META_U(unit,"Group bit map = %d\r\n"),oc->opcode_grp_bmp[idx]));
    }
    LOG_CLI((BSL_META_U(unit,"OAM opcode group1 bitmap\n")));
    for (idx = 0; idx < 8; ++idx) {
        LOG_CLI((BSL_META_U(unit,"Group bit map = %d\r\n"),oc->opcode_grp_1_bmp[idx]));
    }
    LOG_CLI((BSL_META_U(unit,"OAM opcode group2 bitmap\n")));
    for (idx = 0; idx < 8; ++idx) {
        LOG_CLI((BSL_META_U(unit,"Group bit map = %d\r\n"),oc->opcode_grp_2_bmp[idx]));
    }
...
_bcm_kt2_oam_sync(int unit)
{
    ...
    /* Allocate memory to store UpMEP and DownMEP EP info */
    alloc_size += (_BCM_KT2_OAM_RX_DOWNMEP_EP_FLAGS2_SCACHE_SIZE);
    alloc_size += (_BCM_KT2_OAM_RX_UPMEP_EP_FLAGS2_SCACHE_SIZE);

    /* Allocate memory to store all Group event types info */
    alloc_size += (_BCM_KT2_OAM_GROUP_EVENT_TYPES_INFO_SCACHE_SIZE);

    /* Allocate memory to store all RMEP  event types info */
    alloc_size += (_BCM_KT2_OAM_RMEP_EVENT_TYPES_INFO_SCACHE_SIZE);
    ...
    /*   Following is scache allocation for EPs flags2 info */
    sal_memset(oam_scache, 0, (_BCM_KT2_OAM_RX_UPMEP_EP_FLAGS2_SCACHE_SIZE
                               + _BCM_KT2_OAM_RX_DOWNMEP_EP_FLAGS2_SCACHE_SIZE));
    ...
    for (grp_idx = 0; grp_idx < oc->group_count; ++grp_idx)
    {
        if (group_p[grp_idx].in_use == 1) {
            ...
            while (ep_list_p != NULL) {
                sal_memcpy(&ep_data, ep_list_p->ep_data_p, sizeof(_bcm_oam_hash_data_t));
                if ((ep_data.type == bcmOAMEndpointTypeEthernet)
                    && (ep_data.local_rx_enabled)) {
                    if (ep_data.flags & BCM_OAM_ENDPOINT_UP_FACING) {
                        _bcm_kt2_oam_ma_index_offset_get(unit, &ep_data, &ma_offset);
                        ep_data.local_rx_index = ep_data.ma_base_index + ma_offset;
                        rxmep_scache = oam_scache +
                                        (_BCM_KT2_OAM_RX_UPMEP_EP_FLAGS2_SCACHE_LOCATION(ep_data.local_rx_index));
                        sal_memcpy(rxmep_scache, &(ep_data.flags2), sizeof(uint32));
                        rxmep_scache += sizeof(uint32);
                    } else {
                        _bcm_kt2_oam_ma_index_offset_get(unit, &ep_data, &ma_offset);
                        ep_data.local_rx_index = ep_data.ma_base_index + ma_offset;
                        rxmep_scache = oam_scache +
                                        (_BCM_KT2_OAM_RX_DOWNMEP_EP_FLAGS2_SCACHE_LOCATION(ep_data.local_rx_index));
                        sal_memcpy(rxmep_scache,  &(ep_data.flags2), sizeof(uint32));
                        rxmep_scache += sizeof(uint32);
                    }
                }
     ...
     oam_scache +=  _BCM_KT2_OAM_RX_UPMEP_EP_FLAGS2_SCACHE_SIZE;
     oam_scache +=  _BCM_KT2_OAM_RX_DOWNMEP_EP_FLAGS2_SCACHE_SIZE;

    sal_memcpy(oam_scache, &oc->opcode_grp_bmp, _BCM_KT2_OAM_OPCODE_GROUP_BMP_SIZE);
    oam_scache += _BCM_KT2_OAM_OPCODE_GROUP_BMP_SIZE;

    sal_memcpy(oam_scache, &oc->opcode_grp_1_bmp, _BCM_KT2_OAM_OPCODE_GROUP_BMP_SIZE);
    oam_scache += _BCM_KT2_OAM_OPCODE_GROUP_BMP_SIZE;

    sal_memcpy(oam_scache, &oc->opcode_grp_2_bmp, _BCM_KT2_OAM_OPCODE_GROUP_BMP_SIZE);
    oam_scache += _BCM_KT2_OAM_OPCODE_GROUP_BMP_SIZE;

    /* Following is scache allocation for Groups flags and group_event_types info */
    sal_memset(oam_scache, 0, _BCM_KT2_OAM_GROUP_EVENT_TYPES_INFO_SCACHE_SIZE);
    ...
    for (grp_idx = 0; grp_idx < oc->group_count; ++grp_idx)
    {
        ...
        if (BCM_E_EXISTS
            ...
            group_scache = oam_scache + (_BCM_KT2_OAM_GROUP_EVENT_TYPES_INFO_SCACHE_LOCATION(grp_idx));
            sal_memcpy(group_scache, &(group_p[grp_idx].flags), sizeof(uint32));
            group_scache += sizeof(uint32);
            sal_memcpy(group_scache, &(group_p[grp_idx].group_event_types), sizeof(bcm_oam_event_types_t));
            group_scache += sizeof(bcm_oam_event_types_t);
        }
    }
    oam_scache +=  _BCM_KT2_OAM_GROUP_EVENT_TYPES_INFO_SCACHE_SIZE;

    /* Following is scache allocation for RMEPs flags2 and rmep_event_types info */
    sal_memset(oam_scache, 0, _BCM_KT2_OAM_RMEP_EVENT_TYPES_INFO_SCACHE_SIZE);
    ...
    for (grp_idx = 0; grp_idx < oc->group_count; ++grp_idx)
    {
        if (group_p[grp_idx].in_use == 1) {
            ...
            while (ep_list_p != NULL) {
                sal_memcpy(&ep_data, ep_list_p->ep_data_p, sizeof(_bcm_oam_hash_data_t));
                if ((ep_data.type == bcmOAMEndpointTypeEthernet) && (ep_data.is_remote)) {
                    rmep_scache = oam_scache + (_BCM_KT2_OAM_RMEP_EVENT_TYPES_INFO_SCACHE_LOCATION(ep_data.remote_index));
                    sal_memcpy(rmep_scache, &(ep_data.ep_id), sizeof (bcm_oam_endpoint_t));
                    rmep_scache += sizeof(bcm_oam_endpoint_t);
                    sal_memcpy(rmep_scache, &(ep_data.flags2), sizeof(uint32));
                    rmep_scache += sizeof(uint32);
                    sal_memcpy(rmep_scache, &(ep_data.rmep_event_types), sizeof(bcm_oam_event_types_t));
                    rmep_scache += sizeof(bcm_oam_event_types_t);
                }
                ep_list_p = ep_list_p->next;
            }
        }
    }
    oam_scache +=  _BCM_KT2_OAM_RMEP_EVENT_TYPES_INFO_SCACHE_SIZE;
...
/*
 * Function:
 *    _bcm_kt2_oam_wb_group_event_types_info_recover
 * Purpose:
 *     Recover OAM group configuration.
 * Parameters:
 *     unit          - (IN) BCM device number
 *     group_scache  - (IN) Pointer to group scache address pointer.
 * Returns:
 *     BCM_E_XXX
 */
STATIC int
_bcm_kt2_oam_wb_group_event_types_info_recover(int unit, uint8 **group_scache)
{
    int                    index;                    /* Hw table index.     */
    _bcm_oam_group_data_t  *group_p;                 /* Group info pointer. */
    _bcm_oam_control_t     *oc;                      /* Pointer to Control  */
                                                     /* structure.          */
    uint8                  *group_event_types_info;  /* Pointer to RMEP Event Types Info */
    uint32                 flags;                    /* Group flags         */
    bcm_oam_event_types_t  event_types;              /* Group event types   */
    ...
    for (index = 0; index < oc->group_count; index++) {
        ...
        group_p = &oc->group_info[index];
        if (1 == group_p->in_use) {
            /* Recover group entry sync info from group_scache */
            group_event_types_info = *group_scache +
                 (_BCM_KT2_OAM_GROUP_EVENT_TYPES_INFO_SCACHE_LOCATION(index));
            /* Get the group flags and group_event_types from stored info. */
            sal_memcpy(&flags, group_event_types_info, sizeof(uint32));
            group_p->flags = flags;
            group_event_types_info += sizeof(uint32);
            sal_memcpy(&event_types, group_event_types_info, sizeof(bcm_oam_event_types_t));
            group_p->group_event_types = event_types;
            group_event_types_info += sizeof(bcm_oam_event_types_t);
        }
    }
    return (BCM_E_NONE);
}
...
bcm_kt2_oam_scache_alloc(int unit) {
    ...
    /* Allocate memory to store all Rx UpMEP DownMEP EP FLAGS2 */
    alloc_sz += (_BCM_KT2_OAM_RX_UPMEP_EP_FLAGS2_SCACHE_SIZE);
    alloc_sz += (_BCM_KT2_OAM_RX_DOWNMEP_EP_FLAGS2_SCACHE_SIZE);

    /* Allocate memory to store global OAM opcode bit maps */
    alloc_sz += (_BCM_KT2_OAM_OPCODE_GROUP_BMP_SIZE);
    alloc_sz += (_BCM_KT2_OAM_OPCODE_GROUP_BMP_SIZE);
    alloc_sz += (_BCM_KT2_OAM_OPCODE_GROUP_BMP_SIZE);

    /* Allocate memory to store Group flags and group_event_types info */
    alloc_sz += (_BCM_KT2_OAM_GROUP_EVENT_TYPES_INFO_SCACHE_SIZE);

    /* Allocate memory to store RMEP flags2 and rmep_event_types info */
    alloc_sz += (_BCM_KT2_OAM_RMEP_EVENT_TYPES_INFO_SCACHE_SIZE);
...
/*
 * Function:
 *    _bcm_kt2_oam_wb_rmep_event_types_info_recover
 * Purpose:
 *     Recover OAM remote endpoints event_types configuration.
 * Parameters:
 *     unit           - Device unit number.
 *     rmep_scache    - Pointer to rmep entry scache.
 * Returns:
 ...
 */
STATIC int
_bcm_kt2_oam_wb_rmep_event_types_info_recover(int unit,
                                              uint8 **rmep_scache)
{
    _bcm_oam_control_t      *oc = NULL;             /* Pointer to control structure.     */
    _bcm_oam_hash_data_t    *h_data_p = NULL;
    uint8                   *rmep_event_types_info; /* Pointer to RMEP Event Types Info */
    uint32                  flags2 = 0;
    int                     ep_id;                  /* Endpoint ID.                     */
    int                     idx = 0;
    ...
    for (idx = 0; idx < oc->ep_count; idx++) {
        ...
        if (ETH_TYPE(h_data_p->type)
            ...
            /* Recover rmep entry sync info from rmep_scache */
            rmep_event_types_info = *rmep_scache +
                 (_BCM_KT2_OAM_RMEP_EVENT_TYPES_INFO_SCACHE_LOCATION(h_data_p->remote_index));
            /* Recover EP ID from rmep_scache */
            sal_memcpy(&ep_id, rmep_event_types_info, sizeof(bcm_oam_endpoint_t));

            if (ep_id != h_data_p->ep_id) {
                LOG_ERROR(BSL_LS_BCM_OAM,
                    (BSL_META_U(unit,
                                "OAM Error: (RMEP EP =%d) Event_types Info recovery failed.\n"),
                                 h_data_p->ep_id));
                return (BCM_E_FAIL);
            }

            rmep_event_types_info += sizeof(bcm_oam_endpoint_t);
            /* Recover flags2 from rmep_scache */
            sal_memcpy(&flags2, rmep_event_types_info, sizeof(uint32));
            h_data_p->flags2 = flags2;
            rmep_event_types_info += sizeof(uint32);
            /* Recover rmep_event_types from rmep_scache */
            sal_memcpy(&h_data_p->rmep_event_types, rmep_event_types_info, sizeof(bcm_oam_event_types_t));
            rmep_event_types_info += sizeof(bcm_oam_event_types_t);
...
/*
 * Function:
 *    _bcm_kt2_oam_lmep_upmep_rx_endpoint_flags2_update
 * Purpose:
 *     Recover OAM local Rx endpoints flags2 info for UpMEPS
 * Parameters:
 *     unit        - (IN) BCM device number
 *     egr_mp_grp_entry - (IN) Egress MP group table entry pointer.
 * Returns:
 ...
 */
STATIC int
_bcm_kt2_oam_lmep_upmep_rx_endpoint_flags2_update(int unit,
                                                  egr_mp_group_entry_t *egr_mp_grp_entry,
                                                  uint8 **scache)
{
    _bcm_oam_hash_data_t    *h_data_p = NULL; /* Endpoint hash data pointer. */
    ...
    for (mdl = 0; mdl < _BCM_OAM_EP_LEVEL_COUNT; mdl++) {
        ...
        if (mdl_bitmap & (1 << mdl)) {
            ...
            /* Recover EP flags2 from scache and update the endpoint */
            ep_flags2 = *scache + (_BCM_KT2_OAM_RX_UPMEP_EP_FLAGS2_SCACHE_LOCATION(ma_base_idx + ma_offset));

            sal_memcpy(&flags2, ep_flags2, sizeof(uint32));

            if (flags2 & BCM_OAM_ENDPOINT_FLAGS2_TRUNK_WITHOUT_MEMBERS) {
                ep_id = _bcm_kt2_oam_ma_idx_to_ep_id_mapping_get(oc,
                                                                 _BCM_KT2_OAM_MA_IDX_TO_EP_ID_MAPPING_IDX
                                                                  (ma_base_idx, ma_offset, 1));
                h_data_p = &oc->oam_hash_data[ep_id];
                if (ETH_TYPE(h_data_p->type)
                    && (h_data_p->in_use)) {
                    h_data_p->flags2 |= BCM_OAM_ENDPOINT_FLAGS2_TRUNK_WITHOUT_MEMBERS;
                }
            }
            ma_offset++;
        }
...
/*
 * Function:
 *    _bcm_kt2_oam_lmep_downmep_rx_endpoint_flags2_update
 * Purpose:
 *     Recover OAM local Rx endpoints flags2 for DownMEPS
 * Parameters:
 *     unit        - (IN) BCM device number
 *     l3_entry    - (IN) L3 table entry pointer.
 * Returns:
 *     BCM_E_XXX
 */
STATIC int
_bcm_kt2_oam_lmep_downmep_rx_endpoint_flags2_update(int unit,
                                                    l3_entry_1_entry_t *l3_entry,
                                                    uint8 **scache)
{
    _bcm_oam_hash_data_t    *h_data_p = NULL; /* Endpoint hash data pointer. */
   ...
   /* recover DownMEPs */
    ...
    for (mdl = 0; mdl < _BCM_OAM_EP_LEVEL_COUNT; mdl++) {
        ...
        if (mdl_bitmap & (1 << mdl)) {
           /* Recover EP flags2 from scache and update the endpoint */
            ep_flags2 = *scache + (_BCM_KT2_OAM_RX_DOWNMEP_EP_FLAGS2_SCACHE_LOCATION(ma_base_idx + ma_offset));
...
/*
 * Function:
 *     _bcm_kt2_oam_wb_rx_endpoints_flags2_recover
 * Purpose:
 *     Recover OAM Rx MEP endpoints flags configuration.
 * Parameters:
 *     unit        - Device unit number.
 *     rxmep_scache    - Pointer to rxmep entry scache.
 * Returns:
 *     BCM_E_XXX
 */
STATIC int
_bcm_kt2_oam_wb_rx_endpoints_flags2_recover(int unit,
                                            uint8 **rxmep_scache)
{
    int                     index;        /* Hardware index.                  */
    ...
    for (index = 0; index < entry_count; index++) {
        ...
        if (soc_L3_ENTRY_IPV4_UNICASTm_field32_get(unit, &l3_entry, VALIDf)) {
            ...
            if (SOC_MEM_KEY_L3_ENTRY_LMEP == soc_L3_ENTRY_IPV4_UNICASTm_field32_get(unit, &l3_entry,
                                                                       KEY_TYPEf)) {
                rv = _bcm_kt2_oam_lmep_downmep_rx_endpoint_flags2_update(unit,
                                                                         &l3_entry, rxmep_scache);
                if (BCM_FAILURE(rv)) {
                    LOG_ERROR(BSL_LS_BCM_OAM,
                              (BSL_META_U(unit,
                                          "OAM(unit %d) Error: Local endpoint DownMEP"
                                          " (index=%d) flags2 reconstruct failed  - %s.\n"),
    ...
    for (index = 0; index < entry_count; index++) {
        ...
        if (soc_EGR_MP_GROUPm_field32_get(unit, &egr_mp_grp_entry, VALIDf)) {
            rv = _bcm_kt2_oam_lmep_upmep_rx_endpoint_flags2_update(unit,
                                                                   &egr_mp_grp_entry,
                                                                   rxmep_scache);
            if (BCM_FAILURE(rv)) {
                LOG_ERROR(BSL_LS_BCM_OAM,
...
_bcm_kt2_oam_reinit(int unit)
{
    ...
    if (recovered_ver >= BCM_WB_VERSION_1_7) {
        rv = _bcm_kt2_oam_wb_rx_endpoints_flags2_recover(unit, &oam_scache);
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                                  "OAM Error: Rx Endpoint's flags2 recovery failed  - %s.\n"),
                                  bcm_errmsg(rv)));
            goto cleanup;
        }
        oam_scache += (_BCM_KT2_OAM_RX_UPMEP_EP_FLAGS2_SCACHE_SIZE);
    ...
    if (recovered_ver >= BCM_WB_VERSION_1_8) {
        sal_memcpy(&oc->opcode_grp_bmp, oam_scache, _BCM_KT2_OAM_OPCODE_GROUP_BMP_SIZE);
        oam_scache += _BCM_KT2_OAM_OPCODE_GROUP_BMP_SIZE;

        sal_memcpy(&oc->opcode_grp_1_bmp, oam_scache, _BCM_KT2_OAM_OPCODE_GROUP_BMP_SIZE);
        oam_scache += _BCM_KT2_OAM_OPCODE_GROUP_BMP_SIZE;

        sal_memcpy(&oc->opcode_grp_2_bmp, oam_scache, _BCM_KT2_OAM_OPCODE_GROUP_BMP_SIZE);
        oam_scache += _BCM_KT2_OAM_OPCODE_GROUP_BMP_SIZE;
    }

    if (recovered_ver >= BCM_WB_VERSION_1_9) {
        /* Group's group_event_types and flags2 recover and update the group entry */
        rv = _bcm_kt2_oam_wb_group_event_types_info_recover(unit, &oam_scache);
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                                  "OAM Error: Group Event Types info recovery failed  - %s.\n"),
                                  bcm_errmsg(rv)));
            goto cleanup;
        }
        oam_scache += (_BCM_KT2_OAM_GROUP_EVENT_TYPES_INFO_SCACHE_SIZE);

        /* RMEP's rmep_event_types and flags2 recover and update the rmep entry */
        rv = _bcm_kt2_oam_wb_rmep_event_types_info_recover(unit, &oam_scache);
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "OAM Error: RMEP Event Types info recovery failed  - %s.\n"),
                              bcm_errmsg(rv)));
            goto cleanup;
        }
        oam_scache += (_BCM_KT2_OAM_RMEP_EVENT_TYPES_INFO_SCACHE_SIZE);
    ...
    */
    if (!SOC_WARM_BOOT_SCACHE_IS_LIMITED(unit) && (stable_size > 0)) {
        ...
        if (recovered_ver < BCM_WB_VERSION_1_7) {
            realloc_size += (_BCM_KT2_OAM_RX_UPMEP_EP_FLAGS2_SCACHE_SIZE);
            realloc_size += (_BCM_KT2_OAM_RX_DOWNMEP_EP_FLAGS2_SCACHE_SIZE);
        }

        if (recovered_ver < BCM_WB_VERSION_1_8) {
            realloc_size += (_BCM_KT2_OAM_OPCODE_GROUP_BMP_SIZE);
            realloc_size += (_BCM_KT2_OAM_OPCODE_GROUP_BMP_SIZE);
            realloc_size += (_BCM_KT2_OAM_OPCODE_GROUP_BMP_SIZE);
        }

        if (recovered_ver < BCM_WB_VERSION_1_9) {
            realloc_size += (_BCM_KT2_OAM_GROUP_EVENT_TYPES_INFO_SCACHE_SIZE);
            realloc_size += (_BCM_KT2_OAM_RMEP_EVENT_TYPES_INFO_SCACHE_SIZE);
        }
...
bcm_kt2_oam_init(int unit)
{
    ...
    oc->bhh_endpoint_count = soc_property_get(unit, spn_BHH_NUM_SESSIONS, 50);
...
bcm_kt2_oam_group_create(int unit, bcm_oam_group_info_t *group_info)
{
    ...
    /* Check if callback event flag is set */
    if (group_info->flags & BCM_OAM_GROUP_CALLBACK_ENABLE_EVENTS) {
        ma_group->flags |= BCM_OAM_GROUP_CALLBACK_ENABLE_EVENTS;
    }

    /* Check if an event is set for register in the events bitmap. */
    SHR_BITTEST_RANGE(group_info->group_event_types.w, 0, bcmOAMEventCount, event_bmp);
    if (0 != event_bmp) {
        rv = _bcm_kt2_oam_group_event_types_validate(group_info->group_event_types);
        if (BCM_FAILURE(rv)) {
            _BCM_OAM_UNLOCK(oc);
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                                  "OAM Error: Invalid group event type set (GID=%d) \n"),
                                   group_info->id));
            return (rv);
        }
    }
    ma_group->group_event_types = group_info->group_event_types;
...
/*
 * Function:
 *     _bcm_kt2_oam_endpoint_update_fields_mask
 * Purpose:
 *     Zero out all fields that can be hitlessly updated and transient fields
 *     in the endpoint_info structure.
 * Parameters:
 *     ep_info - (IN/OUT) Pointer to endpoint information buffer whose
 *                        fields need to be masked.
 *     tx_add - (IN) Indicates that the Tx params for a LMEP is being added
 *
 * Returns:
 *      None
 */
STATIC void
_bcm_kt2_oam_endpoint_update_fields_mask(bcm_oam_endpoint_info_t *ep_info, int tx_add)
{
    ep_info->flags &= ~(BCM_OAM_ENDPOINT_WITH_ID |
                        BCM_OAM_ENDPOINT_REPLACE);
    ep_info->flags2 &= ~(BCM_OAM_ENDPOINT_FLAGS2_GET_FAULTS_ONLY);
    ep_info->ccm_period = 0;

    if (!(ep_info->flags & BCM_OAM_ENDPOINT_REMOTE)) {
        ep_info->flags &= ~(BCM_OAM_ENDPOINT_PORT_STATE_UPDATE |
                            BCM_OAM_ENDPOINT_INTERFACE_STATE_UPDATE |
                            BCM_OAM_ENDPOINT_PORT_STATE_TX |
                            BCM_OAM_ENDPOINT_INTERFACE_STATE_TX);

        ep_info->trunk_index = 0;
        ep_info->pkt_pri = 0;
        ep_info->inner_pkt_pri = 0;
        ep_info->int_pri = 0;
        ep_info->faults = 0;
        ep_info->persistent_faults = 0;
        ep_info->clear_persistent_faults = 0;
        ep_info->port_state = 0;
        ep_info->interface_state = 0;
    }

    if (tx_add) {
        /* If Tx is being added, Tx only parameters can change. */
        sal_memset(ep_info->src_mac_address, 0, sizeof(bcm_mac_t));
        sal_memset(ep_info->dst_mac_address, 0, sizeof(bcm_mac_t));
        ep_info->outer_tpid = 0;
        ep_info->inner_tpid = 0;
    }
}

/*
 * Function:
 *     bcm_kt2_oam_endpoint_remote_update
 * Purpose:
 *     Update a remote endpoint hitlessly.
 * Parameters:
 *     unit    - (IN)  BCM device number
 *     old     - (IN)  Old state of the endpoint
 *     new     - (IN)  New state of the endpoint
 * Returns:
 *      BCM_E_XXX
 */
STATIC int
_bcm_kt2_oam_endpoint_remote_update(int unit, bcm_oam_endpoint_info_t *old,
                                    bcm_oam_endpoint_info_t *new)
{
    _bcm_oam_control_t *oc = NULL;
    _bcm_oam_hash_data_t *h_data_p = NULL;
    l3_entry_1_entry_t l3_entry;
    ...
    /* Get the hash data pointer. */
    h_data_p = &oc->oam_hash_data[new->id];
    if(h_data_p->in_use == 0) {
        return BCM_E_NOT_FOUND;
    }

    if (new->ccm_period != old->ccm_period) {
        h_data_p->period = new->ccm_period;

        sal_memset(&l3_entry, 0, sizeof(l3_entry));
        _bcm_kt2_oam_rmep_key_construct(unit, h_data_p, &l3_entry);
        BCM_IF_ERROR_RETURN
            (soc_mem_search(unit, L3_ENTRY_IPV4_UNICASTm, MEM_BLOCK_ANY,
                            &eindex, &l3_entry, &l3_entry, 0));
        /* Set the CCM interval. */
        soc_L3_ENTRY_IPV4_UNICASTm_field32_set(unit, &l3_entry, RMEP__CCMf,
                                               _bcm_kt2_oam_ccm_msecs_to_hw_encode(h_data_p->period));
        BCM_IF_ERROR_RETURN(soc_mem_write(unit, L3_ENTRY_IPV4_UNICASTm,
                                          MEM_BLOCK_ALL, eindex, &l3_entry));
    }

    if (new->flags2 & BCM_OAM_ENDPOINT_FLAGS2_CALLBACK_ENABLE_EVENTS) {
        h_data_p->flags2  |= BCM_OAM_ENDPOINT_FLAGS2_CALLBACK_ENABLE_EVENTS;
    } else {
        h_data_p->flags2 &= ~(BCM_OAM_ENDPOINT_FLAGS2_CALLBACK_ENABLE_EVENTS);
    }

    SHR_BITTEST_RANGE(new->rmep_event_types.w, 0, bcmOAMEventCount, event_bmp);
    if (0 != event_bmp) {
        /* Validate RMEP Event types */
        if (BCM_E_NONE != _bcm_kt2_oam_endpoint_rmep_event_types_validate(new->rmep_event_types)) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                       "OAM Error: Invalid rmep event type set to EP:%d \n"), new->id));
            return BCM_E_PARAM;
        }
    }

    h_data_p->rmep_event_types = new->rmep_event_types;
...
/*
 * Function:
 *     bcm_kt2_oam_endpoint_local_update
 * Purpose:
 *     Update a local endpoint hitlessly.
 * Parameters:
 *     unit    - (IN)  BCM device number
 *     old     - (IN)  Old state of the endpoint
 *     new     - (IN)  New state of the endpoint
 ...
 */
STATIC int
_bcm_kt2_oam_endpoint_local_update(int unit, bcm_oam_endpoint_info_t *old,
                                    bcm_oam_endpoint_info_t *new)
{
    _bcm_oam_control_t *oc = NULL;
    _bcm_oam_hash_data_t *h_data_p = NULL;
    ...
    h_data_p->period = new->ccm_period;
    h_data_p->int_pri = new->int_pri;
    h_data_p->trunk_index = new->trunk_index;

    if ((old->ccm_period == BCM_OAM_ENDPOINT_CCM_PERIOD_DISABLED) &&
        (new->ccm_period != BCM_OAM_ENDPOINT_CCM_PERIOD_DISABLED)) {
        /* Tx configuration is being created. */
        BCM_IF_ERROR_RETURN
            (shr_idxres_list_alloc(oc->lmep_pool,
                                   (shr_idxres_element_t *)&h_data_p->local_tx_index));

        h_data_p->local_tx_enabled = 1;
        BCM_IF_ERROR_RETURN(_bcm_oam_kt2_local_tx_mep_hw_set(unit, new));
    } else if ((old->ccm_period != BCM_OAM_ENDPOINT_CCM_PERIOD_DISABLED) &&
               (new->ccm_period == BCM_OAM_ENDPOINT_CCM_PERIOD_DISABLED)) {
        /* Tx configuration is being deleted. */
        sal_memset(&lmep_entry, 0, sizeof(lmep_entry_t));
        sal_memset(&lmep_1_entry, 0, sizeof(lmep_1_entry_t));

        BCM_IF_ERROR_RETURN
            (WRITE_LMEPm(unit, MEM_BLOCK_ALL,
                         h_data_p->local_tx_index, &lmep_entry));

        BCM_IF_ERROR_RETURN
            (WRITE_LMEP_1m(unit, MEM_BLOCK_ALL,
                           h_data_p->local_tx_index, &lmep_1_entry));
    ...
    } else if (new->ccm_period != BCM_OAM_ENDPOINT_CCM_PERIOD_DISABLED) {
        BCM_IF_ERROR_RETURN
            (_bcm_kt2_oam_endpoint_gport_resolve(unit, new, &sglp,
                                                 &dglp, &src_pp_port, &dst_pp_port,
                                                 &svp, &trunk_id, &is_vp_valid,
                                                 &tx_gport));
        h_data_p->src_pp_port = src_pp_port;
        h_data_p->dst_pp_port = dst_pp_port;
        if (new->flags2 & BCM_OAM_ENDPOINT_FLAGS2_VLAN_VP_UP_MEP_IN_HW) {
            up_mep_tx_port = _BCM_KT2_SUBPORT_PORT_ID_GET(new->tx_gport);
            h_data_p->src_pp_port = up_mep_tx_port;
            pp_port = h_data_p->src_pp_port;
        }

        /* Program LMEP entry. */
        BCM_IF_ERROR_RETURN
            (READ_LMEPm(unit, MEM_BLOCK_ANY, h_data_p->local_tx_index,
                        &lmep_entry));
        soc_LMEPm_field32_set(unit, &lmep_entry, CCM_INTERVALf,
                              _bcm_kt2_oam_ccm_msecs_to_hw_encode(new->ccm_period));
        soc_LMEPm_field32_set(unit, &lmep_entry, PRIORITYf, new->pkt_pri);
        soc_LMEPm_field32_set(unit, &lmep_entry, CVLAN_TAGf,
                              ((new->inner_pkt_pri << 13) | new->inner_vlan));

        /* Set Port status TLV in CCM Tx packets. */
        if (new->flags & BCM_OAM_ENDPOINT_PORT_STATE_UPDATE) {
            if (new->port_state > BCM_OAM_PORT_TLV_UP) {
                return BCM_E_PARAM;
            }
            soc_LMEPm_field32_set(unit, &lmep_entry, PORT_TLVf,
                                  (new->port_state == BCM_OAM_PORT_TLV_UP)
                                  ? 1 : 0);
        }

        if ((new->flags & BCM_OAM_ENDPOINT_PORT_STATE_TX) ||
            (new->flags & BCM_OAM_ENDPOINT_INTERFACE_STATE_TX)) {
            soc_LMEPm_field32_set(unit, &lmep_entry, INSERT_TLVf, 1);
        } else {
            soc_LMEPm_field32_set(unit, &lmep_entry, INSERT_TLVf, 0);
        }
        ...
        /* Program LMEP_1 entry. */
        BCM_IF_ERROR_RETURN
            (READ_LMEP_1m(unit, MEM_BLOCK_ANY, h_data_p->local_tx_index,
                          &lmep_1_entry));
        soc_LMEP_1m_field32_set(unit, &lmep_1_entry, PP_PORTf, pp_port);
        soc_LMEP_1m_field32_set(unit, &lmep_1_entry, INT_PRIf,
                                new->int_pri);
        if (new->flags & BCM_OAM_ENDPOINT_INTERFACE_STATE_UPDATE) {
            if ((new->interface_state < BCM_OAM_INTERFACE_TLV_UP) ||
                (new->interface_state > BCM_OAM_INTERFACE_TLV_LLDOWN)) {
                return BCM_E_PARAM;
            }
            soc_LMEP_1m_field32_set(unit, &lmep_1_entry, INTERFACE_TLVf,
                                    new->interface_state);
        }
...
/*
 * Function:
 *     bcm_kt2_oam_endpoint_update
 * Purpose:
 *     Update an endpoint hitlessly, failing which inform the caller to destroy
 *     and recreate the endpoint.
 * Parameters:
 *     unit          - (IN)  BCM device number
 *     endpoint_info - (IN)  Pointer to endpoint information buffer.
 *     destroy       - (OUT) 1 - If the endpoint could not be hitlessly updated,
 *                           caller needs to destroy and recreate the endpoint.
 * Returns:
 *      BCM_E_XXX
 */
STATIC int
_bcm_kt2_oam_endpoint_update(int unit,
                             bcm_oam_endpoint_info_t *endpoint_info,
                             int *destroy)
{
    bcm_oam_endpoint_info_t *start = NULL, *end = NULL, *copy = NULL;
    int tx_add;
    int rv = BCM_E_NONE;

    if (endpoint_info->flags & BCM_OAM_ENDPOINT_INTERMEDIATE) {
        /* Only destructive update supported. */
        *destroy = 1;
        return BCM_E_NONE;
    }
    ...
    *destroy = 0;
    ...
    _BCM_OAM_ALLOC(start, bcm_oam_endpoint_info_t,
                   sizeof(bcm_oam_endpoint_info_t),
                   "Endpoint update - start");
    if (start == NULL) {
        rv = BCM_E_MEMORY;
        goto exit;
    }
    _BCM_OAM_ALLOC(copy, bcm_oam_endpoint_info_t,
                   sizeof(bcm_oam_endpoint_info_t),
                   "Endpoint update - copy");
    if (copy == NULL) {
        rv = BCM_E_MEMORY;
        goto exit;
    }
    _BCM_OAM_ALLOC(end, bcm_oam_endpoint_info_t,
                   sizeof(bcm_oam_endpoint_info_t),
                   "Endpoint update - end");
    if (end == NULL) {
        rv = BCM_E_MEMORY;
    ...
    bcm_oam_endpoint_info_t_init(start);
    bcm_oam_endpoint_info_t_init(end);

    /* Get the current state of the endpoint. */
    rv = bcm_kt2_oam_endpoint_get(unit, endpoint_info->id, start);
    if (rv != BCM_E_NONE) {
        goto exit;
    }

    /* Get the final state of the endpoint. */
    sal_memcpy(end, endpoint_info, sizeof(bcm_oam_endpoint_info_t));

    /* Take a copy of the old state. */
    sal_memcpy(copy, start, sizeof(bcm_oam_endpoint_info_t));

    /* Check if Tx is being added as part of the change. */
    tx_add = 0;
    if (!(endpoint_info->flags & BCM_OAM_ENDPOINT_REMOTE)) {
        if ((start->ccm_period == BCM_OAM_ENDPOINT_CCM_PERIOD_DISABLED) ||
            (end->ccm_period != BCM_OAM_ENDPOINT_CCM_PERIOD_DISABLED)) {
            tx_add = 1;
        }
    }

    /*
     * Mask out all fields that are either transient or can be hitlessly
     * updated.
     */
    _bcm_kt2_oam_endpoint_update_fields_mask(start, tx_add);
    _bcm_kt2_oam_endpoint_update_fields_mask(end, tx_add);

    if (sal_memcmp(start, end, sizeof(bcm_oam_endpoint_info_t)) != 0) {
        /*
         * Some parameter that cannot be hitlessly updated has changed, destroy
         * the endpoint.
         */
        *destroy = 1;
        goto exit;
    ...
    /* Do hitless update. */
    if (endpoint_info->flags & BCM_OAM_ENDPOINT_REMOTE) {
        rv = _bcm_kt2_oam_endpoint_remote_update(unit, copy, endpoint_info);
    } else {
        rv = _bcm_kt2_oam_endpoint_local_update(unit, copy, endpoint_info);
    }
    if (rv != BCM_E_NONE) {
        goto exit;
    }
...
bcm_kt2_oam_endpoint_create(int unit, bcm_oam_endpoint_info_t *endpoint_info)
{
    ...
    if (endpoint_info->flags & BCM_OAM_ENDPOINT_REPLACE) {
        /* Check if hitless update is possible. */
        if (endpoint_info->type == bcmOAMEndpointTypeEthernet) {
            rv = _bcm_kt2_oam_endpoint_update(unit, endpoint_info, &destroy);
            if (BCM_FAILURE(rv)) {
                LOG_ERROR(BSL_LS_BCM_OAM,
                          (BSL_META_U(unit,
                                      "OAM(unit %d) Error: Endpoint update (EP=%d) - %s.\n"),
                           unit, endpoint_info->id, bcm_errmsg(rv)));
                _BCM_OAM_UNLOCK(oc);
                return (rv);
            }
        } else {
            /* Hitless update is only supported for Ethernet OAM endpoints. */
            destroy = 1;
        }

        if (destroy) {
            /*
             * User is requesting the update of some attribute that cannot be
             * done hitlessly, destroy and recreate the endpoint.
             */
    ...
    /* Check if an callback event flag is set to non-RMEPs. */
    if ((endpoint_info->flags & BCM_OAM_ENDPOINT_FLAGS2_CALLBACK_ENABLE_EVENTS) &&
        !(endpoint_info->flags & BCM_OAM_ENDPOINT_REMOTE)) {
        _BCM_OAM_UNLOCK(oc);
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "OAM Error: Callback enable events flag can be set only for RMEPs (EP:%d)\n"),
                               endpoint_info->id));
        return (BCM_E_PARAM);
    }

    /* Check if an event is set for register in the events bitmap. */
    SHR_BITTEST_RANGE(endpoint_info->rmep_event_types.w, 0, bcmOAMEventCount, event_bmp);
    if (0 != event_bmp) {
        if (!(endpoint_info->flags & BCM_OAM_ENDPOINT_REMOTE)) {
            _BCM_OAM_UNLOCK(oc);
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                                  "OAM Error: rmep_event_type can be set only for RMEPs (EP:%d)\n"),
                                   endpoint_info->id));
        ...
        /* Validate RMEP Event types */
        rv =_bcm_kt2_oam_endpoint_rmep_event_types_validate(endpoint_info->rmep_event_types);
        if (BCM_FAILURE(rv)) {
            _BCM_OAM_UNLOCK(oc);
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
    ...
    hash_data->rmep_event_types = endpoint_info->rmep_event_types;
                         ...
                         bcm_oam_endpoint_info_t *endpoint_info)
{
    ...
    if (bcmOAMEndpointTypeEthernet == h_data_p->type) {
        if (1 == h_data_p->is_remote) {
            ...
            if (h_data_p->flags2 & BCM_OAM_ENDPOINT_FLAGS2_CALLBACK_ENABLE_EVENTS) {
                endpoint_info->flags2 |= BCM_OAM_ENDPOINT_FLAGS2_CALLBACK_ENABLE_EVENTS;
            }

            endpoint_info->rmep_event_types = h_data_p->rmep_event_types;
...
/*
 * Function:
 *      _bcm_kt2_eoam_endpoint_faults_multi_get
 * Purpose:
 *      Function to get faults for multiple ETH OAM endpoints
 *      in a single call.
 * Parameters:
 *      unit                (IN) BCM device number
 *      flags               (IN) Flags is kept unused as of now.
 ...
 *      BCM_E_XXXX          Error
 */
int _bcm_kt2_eoam_endpoint_faults_multi_get(
        int unit,
        uint32 flags,
        uint32 max_endpoints,
        bcm_oam_endpoint_fault_t *faults,
            ...
            while (NULL != cur) {
                ...
                if (NULL == h_data_p) {
                    LOG_ERROR(BSL_LS_BCM_OAM,
                         (BSL_META_U(unit,
                                  "OAM Error: Group=%d endpoints access failed -"
                                   " %s.\n"), grp_idx, bcm_errmsg(BCM_E_INTERNAL)));
                    _BCM_OAM_UNLOCK(oc);
                    return (BCM_E_INTERNAL);
                }
                bcm_oam_endpoint_info_t_init(&ep_info);
                ...
                if ((ep_info.flags & BCM_OAM_ENDPOINT_REMOTE) &&
                    (ep_info.faults)) {
                    faults_temp->endpoint_id = ep_info.id;
                    faults_temp->faults = ep_info.faults;
                    faults_temp++;
                    ep_count++;
                }
...

{
    ...
    switch(endpoint_protocol) {
        ...
        case bcmOamProtocolEth:
            *endpoint_count = 0;
            rv = _bcm_kt2_eoam_endpoint_faults_multi_get(
                    unit,
                    flags,
                    max_endpoints,
                    faults,
    ...
    for (i = 0; i < max_ports; i++) {
        ...
        if (add) {
            if (h_data_p->oam_domain == _BCM_OAM_DOMAIN_PORT) {
                ...
                BCM_IF_ERROR_RETURN(_bcm_kt2_oam_port_table_key_update1(unit, PORT_TABm, pp_port, h_data_p));
                BCM_IF_ERROR_RETURN(_bcm_kt2_oam_port_table_key_update1(unit, EGR_PORTm, pp_port, h_data_p));
            }
        } else {
            if (h_data_p->oam_domain == _BCM_OAM_DOMAIN_PORT) {
                ...
                */
                ...
                BCM_IF_ERROR_RETURN(_bcm_kt2_oam_decrement_key_ref_count1(unit, pp_port, h_data_p->dst_pp_port, h_data_p));
            }
        }
    }
    if (h_data_p->oam_domain == _BCM_OAM_DOMAIN_PORT) {
