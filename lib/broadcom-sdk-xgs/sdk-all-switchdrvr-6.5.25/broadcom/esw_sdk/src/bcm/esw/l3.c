...
#define BCM_XGS3_TH3_MPATH_UNDERLAY_EGRESS_IDX_MIN(unit) \
     (BCM_XGS3_MPATH_EGRESS_IDX_MIN(unit) + BCMI_L3_ECMP_OVERLAY_ENTRIES(unit))
#endif
/*
...
#define BCM_WB_VERSION_1_21               SOC_SCACHE_VERSION(1,21)
#define BCM_WB_DEFAULT_VERSION            BCM_WB_VERSION_1_21
...
_bcm_esw_l3_sync(int unit)
{
    ...
    int rv;
#if defined(BCM_TOMAHAWK2_SUPPORT) || defined(BCM_TRIDENT3_SUPPORT) \
    || defined(BCM_TOMAHAWK3_SUPPORT) || defined(BCM_TOMAHAWK_SUPPORT)
    int min_rh_dgm_idx = 0;
#endif
    ...
    int max_rh_dgm_ecmp_groups;

    max_rh_dgm_ecmp_groups = BCM_XGS3_L3_ECMP_MAX_GROUPS(unit);
#endif

#if defined(BCM_TOMAHAWK3_SUPPORT)
    /* In TH3, DGM and RH groups are in underlay only */
    if (soc_feature(unit, soc_feature_l3_ecmp_hier_tbl) &&
        (BCM_XGS3_L3_TBL(unit, ecmp_info).ecmp_mode ==
                                         ecmp_mode_hierarchical)) {
        min_rh_dgm_idx = BCMI_L3_ECMP_OVERLAY_ENTRIES(unit);
    } else if (SOC_IS_TOMAHAWK3(unit)) {
        max_rh_dgm_ecmp_groups -= soc_mem_index_count(unit, ECMP_GROUP_HIERARCHICALm);
    }
#endif
   ...
   if (soc_feature(unit, soc_feature_trill)) {
    ...
    if (soc_feature(unit, soc_feature_ecmp_resilient_hash_optimized)) {
        ...
        alloc_sz += sizeof(uint16) *
                    (max_rh_dgm_ecmp_groups - min_rh_dgm_idx);
...
#define TH3_IS_HECMP_GRP_IDX(unit, idx) \
    ...
    if (soc_feature(unit, soc_feature_l3) && !soc_feature(unit, soc_feature_l3_no_ecmp)) {
        ...
        if (soc_feature(unit, soc_feature_l3_ecmp_hier_tbl)) {
             if (BCM_XGS3_L3_TBL(unit, ecmp_info).ecmp_mode == ecmp_mode_hierarchical) {
                ...
                for (i = 0; i <= hecmp_tbl_ptr->idx_max; i++) {
            ...
            for (i = 0; i <= ecmp_tbl_ptr->idx_max; i++) {
                if (BCM_XGS3_L3_ENT_REF_CNT(ecmp_tbl_ptr, i) > 0) {
                    ...
                    if (BCM_XGS3_L3_TBL(unit, ecmp_info).ecmp_mode ==
                                            ecmp_mode_hierarchical) {
                        if (i < (ecmp_tbl_ptr->idx_max+1)/2) {
                            idx1 = i + BCM_TH3_UNDERLAY_ONLY_OFFSET(unit);
                        }
                    }
                    SHR_BITSET(ecmp_flags, idx1);
                }
            }
        } else
#endif
    ...
    if (soc_feature(unit, soc_feature_l3) && !soc_feature(unit, soc_feature_l3_no_ecmp)) {
        ...
        if ((soc_feature(unit, soc_feature_l3_ecmp_hier_tbl)) &&
             ...
             for (i = 0; i <= ecmp_tbl_ptr->idx_max; i++) {
                if (i < (ecmp_tbl_ptr->idx_max+1)/2) {
                   int idx = i + BCM_TH3_UNDERLAY_ONLY_OFFSET(unit);
                   if (BCM_XGS3_L3_ENT_REF_CNT(ecmp_tbl_ptr, i) > 0 &&
                       (BCM_XGS3_L3_ECMP_GROUP_FLAGS_ISSET(unit, idx,
                               BCM_L3_ECMP_PATH_NO_SORTING))) {
                        SHR_BITSET(ecmp_flags, idx);
                   }
                } else {
                   if (BCM_XGS3_L3_ENT_REF_CNT(ecmp_tbl_ptr, i) > 0 &&
                       (BCM_XGS3_L3_ECMP_GROUP_FLAGS_ISSET(unit, i,
    ...
    if (soc_feature(unit, soc_feature_l3) &&
        ...
        if (!(soc_feature(unit, soc_feature_l3_ecmp_hier_tbl))) {
            ...
            if (BCM_XGS3_L3_TBL(unit, ecmp_info).ecmp_mode ==
                                     ecmp_mode_hierarchical) {
                _bcm_l3_tbl_t *hecmp_tbl_ptr = BCM_XGS3_L3_TBL_PTR(unit, hecmp_grp);
                for (j = 0; j <= hecmp_tbl_ptr->idx_max; j++) {
                    if (BCM_XGS3_L3_ENT_REF_CNT(hecmp_tbl_ptr, j) > 0) {
                        SHR_BITSET(ecmp_l1_flags, j);
                    }
                }
                for (j = 0; j <= ecmp_tbl_ptr->idx_max; j++) {
                    if (BCM_XGS3_L3_ENT_REF_CNT(ecmp_tbl_ptr, j) > 0) {
                        if (j < (ecmp_tbl_ptr->idx_max+1)/2) {
                            SHR_BITSET(ecmp_l2_flags, j + BCM_TH3_UNDERLAY_ONLY_OFFSET(unit));
                        } else {
                            SHR_BITSET(ecmp_l2_flags, j);
                        }
                    }
...
#if defined(BCM_TOMAHAWK2_SUPPORT) || defined(BCM_TRIDENT3_SUPPORT)
    if (soc_feature(unit, soc_feature_dgm)) {
        SOC_SCACHE_DUMP_DATA_BEGIN("l3_ecmp_grp_idx;l3_ecmp_alternate_cnt");
        for (idx = min_rh_dgm_idx; idx < max_rh_dgm_ecmp_groups; idx++) {
            sal_memcpy(l3_scache_ptr,
...
bcm_esw_l3_host_add(int unit, bcm_l3_host_t *info)
{
    ...
    */
    ...
    if (info_local.l3a_flags & BCM_L3_IP6) {
        ...
        if (SOC_IS_TRIDENT2X(unit) || SOC_IS_TOMAHAWKX(unit)) {
            if (BCM_IP6_MULTICAST(info_local.l3a_ip6_addr)) {
                return BCM_E_PARAM;
            }
        }
        ...
        if (SOC_IS_TRIDENT2X(unit) || SOC_IS_TOMAHAWKX(unit)) {
            if (BCM_IP4_MULTICAST(info_local.l3a_ip_addr)) {
                return BCM_E_PARAM;
            }
        }
        /* Add host entry - no prior entry found */
...
{ 
    ...
    ing_l3_next_hop_entry_t in_entry, count_entry;
    int old_queue_id = 0;
    ...
    min = soc_mem_index_min(unit, ING_L3_NEXT_HOPm);
    max = soc_mem_index_max(unit, ING_L3_NEXT_HOPm);
    ...
    if (options & BCM_L3_REPLACE) {
        if (SOC_MEM_FIELD_VALID(unit, mem, EH_QUEUE_TAGf)) {
            ...
            if ((old_queue_id > 0) && (old_queue_id != queue_id)) {
                for( i = min; i < max; i++) {
                    sal_memset(&count_entry, 0, BCM_XGS3_L3_ENT_SZ(unit, nh));
                    /* Read ingress next hop entry. */
                    BCM_IF_ERROR_RETURN(BCM_XGS3_MEM_READ(unit, mem, i, &count_entry));

                    if (old_queue_id == soc_mem_field32_get(unit, mem, &count_entry,
                                EH_QUEUE_TAGf)) {
                        queue_count++;
                    }
                }
    ...
    if (SOC_IS_XGS3_SWITCH(unit) && soc_feature(unit, soc_feature_l3)) {
        ...
        if (BCMI_L3_ECMP_IS_MULTI_LEVEL(unit)) {
            ...
            if (SOC_IS_TRIDENT3X(unit) &&
                (BCM_XGS3_L3_ECMP_GROUP_FLAGS_ISSET(unit,
        ...
        if (soc_feature(unit, soc_feature_hierarchical_ecmp)) {
            ...
            if (BCM_XGS3_L3_ECMP_GROUP_FLAGS_ISSET(unit,
                      (ecmp->ecmp_intf - BCM_XGS3_MPATH_EGRESS_IDX_MIN(unit)),
                       BCM_L3_ECMP_UNDERLAY_ONLY)) {
                ecmp->ecmp_group_flags |= BCM_L3_ECMP_UNDERLAY_ONLY;
            } else if (BCM_XGS3_L3_ECMP_GROUP_FLAGS_ISSET(unit,
                      (ecmp->ecmp_intf - BCM_XGS3_MPATH_EGRESS_IDX_MIN(unit)),
                       BCM_L3_ECMP_UNDERLAY)) {
                ecmp->ecmp_group_flags |= BCM_L3_ECMP_UNDERLAY;
            }
             ...
             !(ecmp->ecmp_group_flags & BCM_L3_ECMP_UNDERLAY)) {
               if ((ecmp->flags & BCM_L3_REPLACE) &&
                   ...
                   if (BCM_XGS3_L3_MPATH_EGRESS_IDX_VALID(unit,
                       ...
                       ecmp_group_flags |= BCM_L3_ECMP_UNDERLAY;
               } else if (ecmp->ecmp_group_flags & BCM_L3_ECMP_UNDERLAY_ONLY) {
                       ecmp_group_flags |= BCM_L3_ECMP_UNDERLAY;
               } else {
                   return BCM_E_PARAM;
               }
...
#ifdef BCM_TOMAHAWK3_SUPPORT
        /* For empty dlb groups allocate a zero bitmap */
        if ((dlb_member_bitmap == NULL) &&
             soc_feature(unit, soc_feature_th3_style_dlb) &&
             (ecmp->flags & BCM_L3_WITH_ID) &&
             (BCM_TH2_ECMP_IS_DLB(unit, ecmp->dynamic_mode))) {
            _bcm_l3_tbl_t *ecmp_tbl_ptr = BCM_XGS3_L3_TBL_PTR(unit, ecmp_grp);
            grp_idx = ecmp->ecmp_intf - BCM_XGS3_MPATH_EGRESS_IDX_MIN(unit);
            if ((BCM_XGS3_L3_ENT_REF_CNT(ecmp_tbl_ptr, grp_idx) > 0) &&
                (grp_idx >= BCM_TH3_L3_ECMP_DLB_START(unit)) &&
                (grp_idx <= ecmp_tbl_ptr->idx_max)) {
                ...
                dlb_member_bitmap = member_bitmap;
            }
        }
#endif
...
/*
 * Function:
 *      bcm_esw_l3_ecmp_dlb_trunk_port_add
 * Purpose:
 *      Add a member port to an DLB ECMP group.
 * Parameters:
 *      unit          - (IN) bcm device.
 *      ecmp_group_id - (IN) L3 interface ID pointing to Egress
 *                           ECMP forwarding object
 *      tid           - (IN) Trunk ID
 *      port          - (IN) port to be added.
 * Returns:
 *      BCM_E_XXX
 */
int
bcm_esw_l3_ecmp_dlb_trunk_port_add(int unit,
                             bcm_if_t ecmp_intf,
                             bcm_trunk_t tid,
                             bcm_port_t port)
{
...
#if defined(BCM_TRIDENT3_SUPPORT)
    rv = bcm_td3_l3_ecmp_dlb_port_add(unit, ecmp_intf -
                                BCM_XGS3_MPATH_EGRESS_IDX_MIN(unit), tid, port);
#endif
    return rv;
}
...
/*
 * Function:
 *      bcm_esw_l3_ecmp_dlb_trunk_port_delete
 * Purpose:
 *      Delete a member port from an existing DLB ECMP group.
 * Parameters:
 *      unit          - (IN) bcm device.
 *      ecmp_group_id - (IN) L3 interface ID pointing to Egress
 *                           ECMP forwarding object
 *      tid           - (IN) Trunk ID
 *      port          - (IN) port to be deleted.
 * Returns:
 *      BCM_E_XXX
 */
int
bcm_esw_l3_ecmp_dlb_trunk_port_delete(int unit,
                                bcm_if_t ecmp_intf,
                                bcm_trunk_t tid,
                                bcm_port_t port)
{
...
#if defined(BCM_TRIDENT3_SUPPORT)
    rv = bcm_td3_l3_ecmp_dlb_port_delete(unit, ecmp_intf -
                                BCM_XGS3_MPATH_EGRESS_IDX_MIN(unit), tid, port);
#endif
    return rv;
}
                           ...
                           int agm_pool_id, int agm_index)
{
    ...
    if (soc_feature(unit, soc_feature_l3_ecmp_hier_tbl) &&
        (ecmp_mode_hierarchical ==
         BCM_XGS3_L3_TBL(unit, ecmp_info).ecmp_mode)) {
        if ((BCMI_L3_ECMP_OVERLAY_ENTRIES(unit)) >
    ...
    if (soc_feature(unit, soc_feature_dgm) &&
        ...
        if (SOC_SUCCESS(rv) &&
            ...
            if (0 == agm_index) { /* detach */
                if (soc_feature(unit, soc_feature_l3_ecmp_hier_tbl)) {
                    count -= BCM_XGS3_L3_ALTERNATE_CNT_PTR(unit)[ecmp_group_id];
                } else {
            ...
            if (!SHR_BITNULL_RANGE(group_bitmap_ptr, 0,
                ...
                && ((!SOC_IS_TOMAHAWK3(unit)) ||
                    (BCM_XGS3_L3_TBL(unit, ecmp_info).ecmp_mode !=
                       ecmp_mode_hierarchical)
#ifdef BCM_TOMAHAWK3_SUPPORT
                    || (SOC_IS_TOMAHAWK3(unit) && (ecmp_group_idx >=
                       BCMI_L3_ECMP_OVERLAY_ENTRIES(unit)))
#endif
               )) {
...
/*
 * Function:
 *      bcm_esw_l3_ecmp_dlb_trigger_reassignment
 * Purpose:
 *      This API provides mechanism to trigger the reassignment of certain
 *      percentage of the flows assigned to a particular egress port in a
 *      DLB ecmp group.
 * Parameters:
 *      unit :        (IN) Device's unit number
 *      ecmp_intf :   (IN) DLB ecmp group number
 *      port        : (IN) Port number on which reassignment has to happen
 *      percentage  : (IN) Percentage of flows to be reassigned
 * Returns:
 *      BCM_E_XXX
 * Notes:
 *    Property spn_DLB_FLOW_MONITOR_EN must be enabled to use this API
 ...
 */
int
bcm_esw_l3_ecmp_dlb_trigger_reassignment(int unit, bcm_if_t dlb_ecmp_group_num,
                                         bcm_port_t port, int percentage)
{
    bcm_l3_egress_ecmp_t dlb_ecmp;
    int dlb_ecmp_count = 0, i = 0;
    int index;
    int num_blocks, total_entries;
    int num_entries_per_block;

    int entry_base_ptr;
    int index_min = 0, index_max = 0;
    int port_member, valid;
    int rv = 0;
    int match_array_alloc_size;
    int *entry_arr = NULL;
    int *rand_num_arr = NULL;
    int count = 0, flows = 0, rand_num = 0;
    ...
    if (percentage == 0) {
        /* Nothing needs to be invalidated so
           we return */
        return BCM_E_NONE;
    }
    /* Retrieve the DLB ecmp group info */
    bcm_l3_egress_ecmp_t_init(&dlb_ecmp);
    dlb_ecmp.ecmp_intf = dlb_ecmp_group_num;
    BCM_IF_ERROR_RETURN(
    _bcm_esw_l3_egress_ecmp_get(unit, &dlb_ecmp, 0, NULL,
                                &dlb_ecmp_count, TRUE));

    if (dlb_ecmp.dynamic_mode != BCM_L3_ECMP_DYNAMIC_MODE_NORMAL) {
        return BCM_E_PARAM;
    }

#if defined(BCM_TOMAHAWK3_SUPPORT)
    if (SOC_IS_TOMAHAWK3(unit)) {
        if (_bcm_th3_mpath_validate(unit, dlb_ecmp.ecmp_intf) == BCM_E_PARAM) {
            goto error;
        }
        ecmp_group_idx = dlb_ecmp.ecmp_intf -
                         BCM_XGS3_TH3_MPATH_UNDERLAY_EGRESS_IDX_MIN(unit);
    } else
#endif
    {
        ecmp_group_idx = dlb_ecmp.ecmp_intf -
...
#endif
    {
        SOC_IF_ERROR_RETURN(READ_ECMP_DLB_CONTROLm(unit, MEM_BLOCK_ANY,
    ...
    /* Calculate the size of the DLB ecmp group */
    num_blocks = dlb_ecmp.dynamic_size >> _BCM_ECMP_DLB_FLOWSET_BLOCK_SIZE_SHIFT;
    num_entries_per_block = 1 << _BCM_ECMP_DLB_FLOWSET_BLOCK_SIZE_SHIFT;
    total_entries = num_blocks * num_entries_per_block;

    match_array_alloc_size = sizeof(int) * total_entries;
    entry_arr = soc_cm_salloc(unit, match_array_alloc_size,
                                  "Matched ECMP DLB Group Entries");
    rand_num_arr = soc_cm_salloc(unit, match_array_alloc_size,
                                  "Array for Random numbers generated");
    if (entry_arr == NULL || rand_num_arr == NULL) {
        rv = BCM_E_MEMORY;
        goto error;
    }
    sal_memset(entry_arr, 0, match_array_alloc_size);
    sal_memset(rand_num_arr, 0, match_array_alloc_size);
    /* Iterate through the ecmp group and identify the flows matching the
       egress port passed from the user. Store the index of the matching
       entries in an array to reuse it later */
    index_min = entry_base_ptr;
    index_max = index_min + dlb_ecmp.dynamic_size - 1;

    for (index = index_min; index <= index_max; index++) {
        SOC_IF_ERROR_RETURN
            (soc_mem_read(unit, DLB_ECMP_FLOWSETm, MEM_BLOCK_ANY,
                          index, &flowset_entry));
        port_member = soc_DLB_ECMP_FLOWSETm_field32_get(unit, &flowset_entry,
                                                        PORT_MEMBER_ASSIGNMENTf);
        valid = soc_DLB_ECMP_FLOWSETm_field32_get(unit, &flowset_entry,
                                                  VALIDf);

        if (port_member == port && valid == 1) {
            entry_arr[count] = index;
            count++;
        }
    }

    /* if no matching entries found related to the egress port, we return with
       E_NOT_FOUND */
    if (count == 0) {
        return BCM_E_NOT_FOUND;
    }

    /* Calculate the number of flows to be reassigned based on the percentage */
    flows = (count *  percentage) / 100;

    /* Iterate through the matched entry array populated before and
       trigger the reassignment by invalidating the flow entries
    */

     while(i < count ) {
        rand_num = sal_rand() % (count);
        if (rand_num_arr[rand_num] == 0 ) {
            rand_num_arr[rand_num] = 1;
            i++;
        }  else {
            continue;
        }
        ...
        SOC_IF_ERROR_RETURN
        (soc_mem_read(unit, mem, MEM_BLOCK_ANY, entry_arr[rand_num], &flowset_entry));

        soc_DLB_ECMP_FLOWSETm_field32_set(unit, &flowset_entry,
                                          VALIDf, 0);
        rv = soc_mem_write(unit, mem, MEM_BLOCK_ALL,
                           entry_arr[rand_num], &flowset_entry);
    ...
    if (rand_num_arr) {
        soc_cm_sfree(unit, rand_num_arr);
    }
    return rv;
...
int bcmi_esw_l3_iif_update_cb_register(int unit,
                                       bcmi_l3_iif_update_callback_t callback,
                                       void *user_data)
{
    if (callback == NULL) {
        return BCM_E_PARAM;
    ...
    L3_INIT(unit);
    BCM_L3_IIF_UPDATE_LOCK(unit);

    if (BCM_L3_IIF_UPDATE_CALLBACK(unit) == callback &&
        BCM_L3_IIF_UPDATE_USER_DATA(unit) == user_data) {
        BCM_L3_IIF_UPDATE_UNLOCK(unit);
        return BCM_E_NONE;
    }

    if (BCM_L3_IIF_UPDATE_CALLBACK(unit) != NULL) {
        BCM_L3_IIF_UPDATE_UNLOCK(unit);
        return BCM_E_RESOURCE;
    }

    BCM_L3_IIF_UPDATE_CALLBACK(unit) = callback;
    BCM_L3_IIF_UPDATE_USER_DATA(unit) = user_data;
...
int bcmi_esw_l3_iif_update_cb_unregister(int unit)
{
    L3_INIT(unit);
    BCM_L3_IIF_UPDATE_LOCK(unit);

    if (BCM_L3_IIF_UPDATE_CALLBACK(unit) != NULL) {
        BCM_L3_IIF_UPDATE_CALLBACK(unit) = NULL;
        BCM_L3_IIF_UPDATE_USER_DATA(unit) = NULL;
    }
...
int bcmi_esw_l3_iif_update_callback(int unit, bcm_if_t intf_id, bcm_vrf_t vrf)
{
    int rv = BCM_E_NONE;
    ...
    if (BCM_L3_IIF_UPDATE_CALLBACK(unit)) {
        rv = BCM_L3_IIF_UPDATE_CALLBACK(unit)(unit, intf_id, vrf,
                                        BCM_L3_IIF_UPDATE_USER_DATA(unit));
    }
