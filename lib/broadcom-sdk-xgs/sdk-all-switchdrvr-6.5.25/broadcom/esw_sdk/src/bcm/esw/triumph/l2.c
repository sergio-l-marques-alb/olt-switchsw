        ...
        if (SOC_MEM_IS_VALID(unit, mem[j])) {
            ...
            if (SOC_WARM_BOOT(unit)) {
                ...
                if (soc_feature(unit, soc_feature_mysta_profile)
                    && my_station_config && j == 1) {
                    int min = 0, max = 0, l2x_size = 0;
                    l2x_entry_t *l2x_entry, *l2x_table;

                    min = soc_mem_index_min(unit, L2Xm);
                    ...
                    if (soc_mem_read_range(unit, L2Xm, MEM_BLOCK_ANY, min, max,
                                           l2x_table) < 0) {
                    ...
                    for (i = min; i <= max; i++) {
                        ...
                        if (soc_feature(unit, soc_feature_base_valid)) {
                            idx = soc_L2Xm_field32_get(unit, l2x_entry,
                                                       BASE_VALIDf);
                        } else {
                            idx = soc_L2Xm_field32_get(unit, l2x_entry, VALIDf);
                        }
                        if (idx == 0) {
                            continue;
                        }

                        idx = soc_mem_field32_dest_get(unit, L2Xm, l2x_entry,
                                                       DESTINATIONf,
                                                       &dest_type);
                        if (dest_type != SOC_MEM_FIF_DEST_MYSTA) {
                            continue;
    ...
    if (soc_feature(unit, soc_feature_l2_mac_move_monitoring)) {
        ...
        if ((soc_l2_mac_move_mon_feature_state_get(unit, &state) !=
             ...
             (state & SOC_L2_MAC_MOVE_MON_STATE_INITED)) {
                   (void)soc_l2_mac_move_mon_entry_delete(unit, mac, vid);
        }
    }
#endif /* BCM_XGS_SWITCH_SUPPORT */
...
STATIC int
_vlan_xc_param_validate(int unit,
                        soc_field_t field, int param, char *param_name)
{
    int field_len;
    int field_max;

    field_len = soc_mem_field_length(unit, L2Xm, field);
    field_max = (1 << field_len) - 1;
    if (param < 0 || param > field_max) {
        LOG_ERROR(BSL_LS_BCM_L2, (BSL_META_U(unit,
            "Error: %s value %d exceeds field max value %d\n"),
             param_name, param, field_max));
        return BCM_E_PARAM;
    }
    return BCM_E_NONE;
}
...
/*
 * Function:
 *      bcmi_tr_l2_cross_connect_control_set
 * Purpose:
 *      Set additional fields not accessible from bcm_vlan_cross_connect_add
 * Parameters:
 *      unit       - Device unit number
 *      outer_vlan - Outer vlan ID. Key to identify the entry
 *      inner_vlan - Inner vlan ID. Key to identify the entry
...
int
bcmi_tr_l2_cross_connect_control_set(int unit,
                               bcm_vlan_t outer_vlan, bcm_vlan_t inner_vlan,
                               bcm_vlan_cross_connect_control_t type, int arg)
{
    l2x_entry_t  l2x_entry;
    ...
    if ((outer_vlan < BCM_VLAN_DEFAULT) || (outer_vlan > BCM_VLAN_MAX)) {
        ...
        if (SOC_MEM_FIELD_VALID(unit, L2Xm, VLAN__IVIDf)) {
            soc_L2Xm_field32_set(unit, &l2x_entry, VLAN__IVIDf, inner_vlan);
        } else {
    ...
    if (SOC_MEM_FIELD_VALID(unit, L2Xm, VLAN__OVIDf)) {
        ...
        soc_L2Xm_field32_set(unit, &l2x_entry, OVIDf, outer_vlan);
    }

    soc_mem_lock(unit, L2Xm);
    /* See if the entry already exists */
    rv = soc_mem_search(unit, L2Xm, MEM_BLOCK_ANY, &l2_index, &l2x_entry,
                        &l2x_lookup, 0);
    if (BCM_FAILURE(rv)) {
        soc_mem_unlock(unit, L2Xm);
        return rv;
    ...
    switch (type) {
        case bcmVlanCrossConnectControlStatic:
            soc_L2Xm_field32_set(unit, &l2x_lookup, VLAN__STATIC_BITf, arg? 1:0);
            break;
        case bcmVlanCrossConnectControlOpaqueValue:
            if (SOC_MEM_FIELD_VALID(unit, L2Xm, VLAN__RESERVEDf)) {
                rv = _vlan_xc_param_validate(unit, VLAN__RESERVEDf, arg,
                           "OpaqueValue");
                if (BCM_SUCCESS(rv)) {
                    soc_L2Xm_field32_set(unit, &l2x_lookup, VLAN__RESERVEDf,
                            arg);
                }
            } else if (SOC_MEM_FIELD_VALID(unit, L2Xm, VLAN__RESERVED_1f)) {
                rv = _vlan_xc_param_validate(unit, VLAN__RESERVED_1f, arg,
                           "OpaqueValue");
                if (BCM_SUCCESS(rv)) {
                    soc_L2Xm_field32_set(unit, &l2x_lookup, VLAN__RESERVED_1f,
                            arg);
                }
            } else if (SOC_MEM_FIELD_VALID(unit, L2Xm, VLAN__RESERVED_52_30f)) {
                rv = _vlan_xc_param_validate(unit, VLAN__RESERVED_52_30f, arg,
                           "OpaqueValue");
                if (BCM_SUCCESS(rv)) {
                    soc_L2Xm_field32_set(unit, &l2x_lookup,
                               VLAN__RESERVED_52_30f, arg);
                }
            } else {
                rv = BCM_E_UNAVAIL;
            }
            break;
        case bcmVlanCrossConnectControlClassId:
            if (SOC_MEM_FIELD_VALID(unit, L2Xm, VLAN__CLASS_IDf)) {
                rv = _vlan_xc_param_validate(unit, VLAN__CLASS_IDf, arg,
                           "ClassId");
                if (BCM_SUCCESS(rv)) {
                    soc_L2Xm_field32_set(unit, &l2x_lookup, VLAN__CLASS_IDf,
                           arg);
                }
            } else {
                rv = BCM_E_UNAVAIL;
    ...
    if (BCM_SUCCESS(rv)) {
        rv = soc_mem_write(unit, L2Xm, MEM_BLOCK_ALL, l2_index, &l2x_lookup);
    }
    soc_mem_unlock(unit, L2Xm);
    BCM_IF_ERROR_RETURN(rv);
    return rv;
...
/*
 * Function:
 *      bcmi_tr_l2_cross_connect_control_get
 * Purpose:
 *      get a field value programmed by _control_set function
 * Parameters:
 *      unit       - Device unit number
 *      outer_vlan - Outer vlan ID. Key to identify the entry
 *      inner_vlan - Inner vlan ID. Key to identify the entry
 *      type       - Field identifier
 *      arg        - Field value pointer
 * Returns:
 *      BCM_E_NONE - Success.
 *      BCM_E_XXX
 */
int
bcmi_tr_l2_cross_connect_control_get(int unit,
                               bcm_vlan_t outer_vlan, bcm_vlan_t inner_vlan,
                               bcm_vlan_cross_connect_control_t type, int *arg)
{
    l2x_entry_t  l2x_entry;
    ...
    switch (type) {
        case bcmVlanCrossConnectControlStatic:
            *arg = soc_L2Xm_field32_get(unit, &l2x_lookup, VLAN__STATIC_BITf);
            break;
        case bcmVlanCrossConnectControlOpaqueValue:
            if (SOC_MEM_FIELD_VALID(unit, L2Xm, VLAN__RESERVEDf)) {
                *arg = soc_L2Xm_field32_get(unit, &l2x_lookup, VLAN__RESERVEDf);
            } else if (SOC_MEM_FIELD_VALID(unit, L2Xm, VLAN__RESERVED_1f)) {
                *arg = soc_L2Xm_field32_get(unit, &l2x_lookup,
                            VLAN__RESERVED_1f);
            } else if (SOC_MEM_FIELD_VALID(unit, L2Xm, VLAN__RESERVED_52_30f)) {
                *arg = soc_L2Xm_field32_get(unit, &l2x_lookup,
                            VLAN__RESERVED_52_30f);
            } else {
                rv = BCM_E_UNAVAIL;
            }
            break;
        case bcmVlanCrossConnectControlClassId:
            if (SOC_MEM_FIELD_VALID(unit, L2Xm, VLAN__CLASS_IDf)) {
                *arg = soc_L2Xm_field32_get(unit, &l2x_lookup, VLAN__CLASS_IDf);
            } else {
                rv = BCM_E_UNAVAIL;
            }
            break;
...
_bcm_td_l2_bulk_replace_modport(int unit, _bcm_l2_replace_t *rep_st)
{
    ...
    if (!(rep_st->flags & BCM_L2_REPLACE_IGNORE_PENDING)) {
...
/*
 * _vlan_xc_replace_entry_setup
 * Purpose:
 *    Set up the L2_BULK match or replace hardware entry based on user
 *  provided information.
 *
 * Parameters:
 *     unit   device number
 *     info   user provided match or replace information
 *     entry  match or replace hardware entry
 * Return:
 *     BCM_E_XXX
 */

STATIC int
_vlan_xc_replace_entry_setup(int unit,
                   bcm_vlan_cross_connect_info_t *info,
                   l2x_entry_t *entry)
{
    bcm_module_t        modid;
    bcm_trunk_t         tgid = BCM_TRUNK_INVALID;
    ...
    sal_memset(entry, 0, sizeof(l2x_entry_t));
    if (soc_feature(unit, soc_feature_base_valid)) {
        soc_mem_field32_set(unit, L2Xm, entry, BASE_VALIDf, 1);
    } else {
        soc_mem_field32_set(unit, L2Xm, entry, VALIDf, 1);
    }

    /* validate parameters */
    BCM_IF_ERROR_RETURN(_vlan_xc_param_validate(unit,
                     VLAN__OVIDf, info->outer_vlan, "outer_vlan"));
    BCM_IF_ERROR_RETURN(_vlan_xc_param_validate(unit,
                     VLAN__IVIDf, info->inner_vlan, "inner_vlan"));
    BCM_IF_ERROR_RETURN(_vlan_xc_param_validate(unit,
                     VLAN__PRIf, info->int_pri, "int_pri"));
    soc_mem_field32_set(unit, L2Xm, entry, VLAN__OVIDf, info->outer_vlan);
    soc_mem_field32_set(unit, L2Xm, entry, VLAN__IVIDf, info->inner_vlan);
    soc_mem_field32_set(unit, L2Xm, entry, VLAN__PRIf, info->int_pri);
    if (SOC_MEM_FIELD_VALID(unit, L2Xm, VLAN__RESERVEDf)) {
        BCM_IF_ERROR_RETURN(_vlan_xc_param_validate(unit,
                     VLAN__RESERVEDf, info->opaque_value, "opaque_value"));
        soc_mem_field32_set(unit, L2Xm, entry, VLAN__RESERVEDf,
                        info->opaque_value);
    } else if (SOC_MEM_FIELD_VALID(unit, L2Xm, VLAN__RESERVED_1f)) {
        BCM_IF_ERROR_RETURN(_vlan_xc_param_validate(unit,
                     VLAN__RESERVED_1f, info->opaque_value, "opaque_value"));
        soc_mem_field32_set(unit, L2Xm, entry, VLAN__RESERVED_1f,
                        info->opaque_value);
    } else if (SOC_MEM_FIELD_VALID(unit, L2Xm, VLAN__RESERVED_52_30f)) {
        BCM_IF_ERROR_RETURN(_vlan_xc_param_validate(unit,
                 VLAN__RESERVED_52_30f, info->opaque_value, "opaque_value"));
        soc_mem_field32_set(unit, L2Xm, entry, VLAN__RESERVED_52_30f,
                        info->opaque_value);
    }

    BCM_IF_ERROR_RETURN(_vlan_xc_param_validate(unit,
                 VLAN__CLASS_IDf, info->class_id, "class_id"));
    soc_mem_field32_set(unit, L2Xm, entry, VLAN__CLASS_IDf, info->class_id);
    if (info->flags & BCM_VLAN_CROSS_CONNECT_INFO_STATIC) {
        soc_mem_field32_set(unit, L2Xm, entry, VLAN__STATIC_BITf, 1);
    }
    if (BCM_GPORT_IS_SET(info->port_1)) {
        BCM_IF_ERROR_RETURN(_bcm_esw_gport_resolve(unit, info->port_1,
             &modid, &port, &tgid, &id));
    } else {
        port = info->port_1;
        modid = 0;
    ...
    if (soc_feature(unit, soc_feature_generic_dest)) {
        BCM_IF_ERROR_RETURN(_vlan_xc_param_validate(unit,
                 ...
                 modid << SOC_MEM_FIF_DGPP_MOD_ID_SHIFT_BITS | port,
                 "port_1"));
        soc_mem_field32_dest_set(unit, L2Xm, entry,
            VLAN__DESTINATIONf, SOC_MEM_FIF_DEST_DGPP,
            modid << SOC_MEM_FIF_DGPP_MOD_ID_SHIFT_BITS | port);
    } else {
        BCM_IF_ERROR_RETURN(_vlan_xc_param_validate(unit,
                 VLAN__MODULE_IDf, modid, "module id1"));
        BCM_IF_ERROR_RETURN(_vlan_xc_param_validate(unit,
                 VLAN__PORT_NUMf, port, "port1"));
        soc_mem_field32_set(unit, L2Xm, entry, VLAN__MODULE_IDf, modid);
        soc_mem_field32_set(unit, L2Xm, entry, VLAN__PORT_NUMf, port);
    }

    if (BCM_GPORT_IS_SET(info->port_2)) {
        BCM_IF_ERROR_RETURN(_bcm_esw_gport_resolve(unit, info->port_2,
             &modid, &port, &tgid, &id));
    } else {
        port = info->port_2;
        modid = 0;
    ...
    if (soc_feature(unit, soc_feature_generic_dest)) {
        ...
        BCM_IF_ERROR_RETURN(_vlan_xc_param_validate(unit,
                 VLAN__MODULE_ID_1f, modid, "module id2"));
        BCM_IF_ERROR_RETURN(_vlan_xc_param_validate(unit,
                 VLAN__PORT_NUM_1f, port, "port2"));
        soc_mem_field32_set(unit, L2Xm, entry, VLAN__MODULE_ID_1f, modid);
        soc_mem_field32_set(unit, L2Xm, entry, VLAN__PORT_NUM_1f, port);
    }
    return BCM_E_NONE;
}

/*
 * bcmi_tr_l2_vlan_cross_connect_replace
 * Purpose:
 *    Perform vlan cross-connect entry replacement or deletion for the
 *  matched entries with L2_BULK operation. Notice it can match either
 *  single cross-connect entries or double cross-connect entries, but not
 *  both at the same time. Replacement can only happens to the policy data
 *  fields not the key fields, i.e. outer vlan and inner vlan.
 *
 * Parameters:
 *   unit:    device number
 *   options: specify replace or delete action; single or double cross connect
 *   match_info: user provided match information
 *   match_info_mask: user provided match information
 *   entry:  match or replace hardware entry
 * Return:
 *     BCM_E_XXX
 */
...
int
bcmi_tr_l2_vlan_cross_connect_replace(int unit,
                   uint32 options,
                   bcm_vlan_cross_connect_info_t *match_info,
                   bcm_vlan_cross_connect_info_t *match_info_mask,
                   bcm_vlan_cross_connect_info_t *replace_info,
                   bcm_vlan_cross_connect_info_t *replace_info_mask)
{
    int rv;
    int field_len;
    _bcm_l2_replace_t rep_st;

    if (match_info == NULL || match_info_mask == NULL) {
        return BCM_E_PARAM;
    }
    if (!(options & BCM_VLAN_CROSS_CONNECT_REPLACE_DELETE)) {
        if (replace_info == NULL || replace_info_mask == NULL) {
            return BCM_E_PARAM;
        }
    }
    ...
    /* set up match entry */
    rv = _vlan_xc_replace_entry_setup(unit, match_info, &rep_st.match_data);
    BCM_IF_ERROR_RETURN(rv);
    if (options & BCM_VLAN_CROSS_CONNECT_REPLACE_DOUBLE_TAGS) {
        soc_mem_field32_set(unit, L2Xm, &rep_st.match_data, KEY_TYPEf,
                            TR_L2_HASH_KEY_TYPE_DOUBLE_CROSS_CONNECT);
    } else {
        soc_mem_field32_set(unit, L2Xm, &rep_st.match_data, KEY_TYPEf,
                            TR_L2_HASH_KEY_TYPE_SINGLE_CROSS_CONNECT);
    ...
    /* set up mask entry */
    rv = _vlan_xc_replace_entry_setup(unit, match_info_mask,
               &rep_st.match_mask);
    BCM_IF_ERROR_RETURN(rv);
    field_len = soc_mem_field_length(unit, L2Xm, KEY_TYPEf);
    soc_mem_field32_set(unit, L2Xm, &rep_st.match_mask, KEY_TYPEf,
          (1 << field_len) - 1);

    if (!(options & BCM_VLAN_CROSS_CONNECT_REPLACE_DELETE)) {
        /* set up replace entry */
        rv = _vlan_xc_replace_entry_setup(unit, replace_info, &rep_st.new_data);
        BCM_IF_ERROR_RETURN(rv);

        /* set up replace mask entry */
        rv = _vlan_xc_replace_entry_setup(unit, replace_info_mask,
                                &rep_st.new_mask);
        BCM_IF_ERROR_RETURN(rv);
    } else {
