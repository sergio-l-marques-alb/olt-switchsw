...
{
    int ing_port = 0, add_signature = 0;
    uint32 mod = 0;
    ...
    */
    ...
    PBMP_PORT_ITER(unit, ing_port) {
        add_signature = soc_feature(unit, soc_feature_hgoe_line_card_mode) ?
                        (_bcm_esw_is_hgoe_port(unit, ing_port)) :
                        IS_HG_PORT(unit, ing_port);
...
/*
 * Function:
 *      bcmi_num_hg_lkup_ports(internal function)
 * Purpose:
 *      Provide the number of ports eligible for HG lookup.
 * Parameters:
 *      unit - (IN) Unit number.
 *      num_ports - (OUT) number of ports eligible for HG lookup.
 * Returns:
 *      BCM_E_xxx
 */
STATIC int
bcmi_num_hg_lkup_ports(int unit, int *num_ports)
{
    int port = 0, i = 0;
    int is_hg_lkup_port = 0;

    PBMP_PORT_ITER(unit, port) {
        is_hg_lkup_port = soc_feature(unit, soc_feature_hgoe_line_card_mode) ?
...
{
    int index = 0, is_hg_lkup_port = 0;
    int i = 0;
    int ing_port = 0;
    int hg_size;
    int *hg_array = NULL;

    bcmi_num_hg_lkup_ports(unit, &hg_size);
...
bcmi_proxy_server_wb_alloc(int unit)
{
    ...
    bcmi_num_hg_lkup_ports(unit, &port_size);
                          ...
                          bcm_proxy_mode_t mode, int enable)
{
    ...
    if (IS_HG_PORT(unit, server_port) ||
        IS_CPU_PORT(unit, server_port) ||
        (soc_feature(unit, soc_feature_hgoe_line_card_mode) &&
        _bcm_esw_is_hgoe_port(unit, server_port))) {
                              ...
                              int enable)
{
    ...
    if (!(IS_HG_PORT(unit, local_ingress_port) ||
          ...
          (soc_feature(unit, soc_feature_hgoe_line_card_mode) &&
          _bcm_esw_is_hgoe_port(unit, local_ingress_port)))) {
