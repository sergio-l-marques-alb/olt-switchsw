...
/*
 * Function:
 *      bcm_esw_flowtracker_cfg_init
 * Purpose:
 *      Re-Initialize the Flowtracker subsystem with provided resource configurations.
 *      It would be the end user responsibility to clear all the existing flowtracker
 *      pipeline flow programming that is present in the system before invoking this API.
 * Parameters:
 *      unit         - (IN) BCM device number
 *      cfg          - (IN) Flowtracker resource configurations.
 * Returns:
 *      BCM_E_XXX   - BCM error code.
 */
int bcm_esw_flowtracker_cfg_init(int unit, bcm_flowtracker_cfg_t *cfg)
{
    int result = BCM_E_UNAVAIL;
...
/*
 * Function:
 *      bcm_esw_flowtracker_cfg_get
 * Purpose:
 *      Get the Flowtracker subsystem resource configurations.
 * Parameters:
 *      unit         - (IN) BCM device number
 *      cfg          - (OUT) Flowtrackr resource configurations.
 * Returns:
 *      BCM_E_XXX   - BCM error code.
 */
int bcm_esw_flowtracker_cfg_get(int unit, bcm_flowtracker_cfg_t *cfg)
{
    int result = BCM_E_UNAVAIL;
    ...
    if (soc_feature(unit, soc_feature_flex_flowtracker_ver_1)) {
        /* Take lock */
        BCM_IF_ERROR_RETURN(bcm_esw_flowtracker_lock(unit));
        result = bcmi_ft_cfg_get(unit, cfg);
        /* Release lock */
        BCM_IF_ERROR_RETURN(bcm_esw_flowtracker_unlock(unit));
    }
#endif
...
/*
 * Function:
 *  bcm_esw_flowtracker_tracking_param_type_info_get
 * Purpose:
 *  To get Flowtracker Tracking param type information.
 * Parameters:
 *  unit          - (IN) BCM device number
 *  group_type    - (IN) Flowtracker Group Type
 *  param         - (IN) Flowtracker Ttrackng param
 *  info          - (OUT) Tracking Param Type information
 * Returns:
 *  BCM_E_XXX   - BCM error code.
 */
int bcm_esw_flowtracker_tracking_param_type_info_get(
    int unit,
    bcm_flowtracker_group_type_t group_type,
    bcm_flowtracker_tracking_param_type_t param,
    bcm_flowtracker_tracking_param_type_info_t *info)
{
    ...
    if (soc_feature(unit, soc_feature_flex_flowtracker_ver_1)) {
        ...
        result = bcmi_ft_tracking_param_type_info_get(unit,
