...
if (BCM_GPORT_IS_MODPORT(gport)) {
    ...
    if (!SHR_BITGET(vlan_info[unit].vp_mode, vlan) &&
        ...
        if (BCM_GPORT_IS_VLAN_PORT(gport) ||
            BCM_GPORT_IS_NIV_PORT(gport) || BCM_GPORT_IS_EXTENDER_PORT(gport)) {

            if (!bc_do_not_add) {
                /* Add the VP to the BC group */
                BCM_IF_ERROR_RETURN
                  (_bcm_tr_multicast_ipmc_group_type_get(unit, bc_idx, &group));
                if (BCM_GPORT_IS_VLAN_PORT(gport)) {
                    BCM_IF_ERROR_RETURN
                        (bcm_esw_multicast_vlan_encap_get(unit, group,
                                                          phy_port_trunk,
                                                          gport, &encap_id));
                } else if (BCM_GPORT_IS_NIV_PORT(gport)) {
                    BCM_IF_ERROR_RETURN
                        (bcm_esw_multicast_niv_encap_get(unit, group,
                                                         phy_port_trunk,
                                                         gport, &encap_id));
                } else {
                    BCM_IF_ERROR_RETURN
                        (bcm_esw_multicast_extender_encap_get(unit, group,
                                                             phy_port_trunk,
                                                             gport, &encap_id));
                }
                /* coverity[stack_use_overflow : FALSE] */
                BCM_IF_ERROR_RETURN
                    (bcm_esw_multicast_egress_add(unit, group,
                                                  phy_port_trunk, encap_id));
            }

            if (!umc_do_not_add) {
                /* Add the VP to the UMC group, if different from BC group */
                BCM_IF_ERROR_RETURN
                 (_bcm_tr_multicast_ipmc_group_type_get(unit, umc_idx, &group));
                if (BCM_GPORT_IS_VLAN_PORT(gport)) {
                    BCM_IF_ERROR_RETURN
                        (bcm_esw_multicast_vlan_encap_get(unit, group,
                                                          phy_port_trunk,
            ...
            if (!uuc_do_not_add) {
                /* Add the VP to the UUC group, if different from BC/UMC grp */
                BCM_IF_ERROR_RETURN
                 (_bcm_tr_multicast_ipmc_group_type_get(unit, uuc_idx, &group));
                if (BCM_GPORT_IS_VLAN_PORT(gport)) {
                    BCM_IF_ERROR_RETURN
                        (bcm_esw_multicast_vlan_encap_get(unit, group,
                                                          phy_port_trunk,
    ...
    if (flags & BCM_VLAN_GPORT_ADD_VP_VLAN_MEMBERSHIP) {
        ...
        if (soc_feature(unit, soc_feature_vlan_vfi_membership)) {
            ...
            rv = bcm_td2p_vp_vlan_member_get(
