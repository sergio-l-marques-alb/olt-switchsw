                     ...
                     alpm_lib_trie_traverse_state_t *state, void *user_data)
{
    ...
    /* at least bkt fits one fmt */
    if (fmt > ACB_FMT_CNT(acb, vrf_id)) {
        ALPM_FATAL(("_alpm_bkt_fmt_req_cb: unexpected alpm bkt fmt %d\n", fmt));
    }
                       ...
                       int merge_req)
{
    ...
    if (merge_req == MERGE_REQ_COVER_BKT) {
         ...
         */
        rv = alpm_bkt_pfx_shrink(u, acb, gprt_pvt, &shrink_empty_only, NULL);
        ALPM_IEG(rv);
                 ...
                 alpm_lib_trie_traverse_state_t *state, void *info)
{
    ...
    if (!(PRT_IS_PAYLOAD(prt_type) && pptrie == ptrie) &&
        !(PRT_IS_INTERNAL(prt_type) && lc_bkt != NULL)) {
        ALPM_ERR(("alpm_l3_merge_cb prt_type=%d\n", prt_type));
    }
    ...
    if (l1_pvt_node == NULL) {
        ALPM_ERR(("alpm_l3_merge_cb: l1_pvt_node is NULL\n"));
    }
    ...
    if (BCM_FAILURE(rv)) {
        ...
        int shrink_bnks = 0;
        /* Bucket prefix shrink if no mergeable */
        rv = alpm_bkt_pfx_shrink(u, acb, chd_pvt, &shrink_empty_only, &shrink_bnks);
        ALPM_IER(rv);
        if (shrink_bnks > 0) {
            ++merge_ctrl->l3_shrink_cnt;
                         ...
                         alpm_lib_trie_traverse_state_t *state, void *info)
{
    ...
    if (l1_pvt_node == NULL) {
        ALPM_ERR(("alpm_l3_merge_payload_cb: l1_pvt_node is NULL\n"));
    }
                 ...
                 alpm_lib_trie_traverse_state_t *state, void *info)
{
    ...
    if(!(PRT_IS_PAYLOAD(prt_type) && pptrie == ptrie) &&
       !(PRT_IS_INTERNAL(prt_type) && lc_pvt != NULL)) {
        ALPM_ERR(("alpm_l2_merge_cb: prt_type=%d\n", prt_type));
    }
 ...
 *     l2_shrink_cnt - (OUT) L2 bucket shrink merge counter achieved during this defrag API call.
 *     l3_shrink_cnt - (OUT) L3 bucket shrink merge counter achieved during this defrag API call.
 * Returns:
 *     BCM_E_XXX:
...
bad:
    ...
    if (l2_shrink_cnt != NULL) {
        *l2_shrink_cnt = merge_ctrl.l2_shrink_cnt;
    }
    if (l3_shrink_cnt != NULL) {
        *l3_shrink_cnt = merge_ctrl.l3_shrink_cnt;
    }
