/*
 * $Id: flexflow_match.c,v 1.0 Broadcom SDK $
...
#define _OPAQUE_OBJECT_1  1
#define _OPAQUE_OBJECT_2  2
#define _OPAQUE_OBJECT_3  3
#define _OPAQUE_OBJECT_4  4
...
static int _gbp_handle_option_check(int unit,
        uint32 handle,
        uint32 option,
        int *result)
{
    ...
    struct {
        char *handle;
        char *option;
    } gbp_name[2] = {{"GBP",      "ASSIGN_GBP_SID"},
                     {"VXLAN_GBP","ASSIGN_GBP_SID"}
                    };

    *result = FALSE;

    for (ix = 0; ix < COUNTOF(gbp_name); ix++) {
        rv = bcm_esw_flow_handle_get(unit,gbp_name[ix].handle, &gbp_handle);
        if (BCM_SUCCESS(rv)) {
            rv = bcm_esw_flow_option_id_get(unit, gbp_handle,
                           gbp_name[ix].option, &gbp_option);
            if (BCM_SUCCESS(rv)) {
                if (gbp_handle == handle && gbp_option == option) {
                    *result = TRUE;
                    break;
                }
            }
        ...
        int clear_action)
{
    ...
    if (SOC_MEM_FIELD_VALID(unit, sgpp_view_id, SVP_PROFILE_INDEXf)) {
        svp_profile = soc_mem_field32_get(unit, sgpp_view_id, entry,
                                            SVP_PROFILE_INDEXf);
    }
    ...
    if (gport_id == -1) { /* not a vp */
        ...
        rv = soc_mem_read(unit, mem, MEM_BLOCK_ANY, gport_id, entry);
    }
    if (SOC_SUCCESS(rv)) {
        if (SOC_MEM_FIELD_VALID(unit, sgpp_view_id, CTRL_PROFILE_IDX_1f)) {
            if (clear_action) {
        ...
        if (SOC_MEM_FIELD_VALID(unit, sgpp_view_id, SVP_PROFILE_INDEXf)) {
            if (clear_action) {
                if (_SHR_GPORT_IS_MPLS_PORT(info->port)) {
                    svp_profile = 1;  /* clear to default */
                } else {
                    svp_profile = 0;  /* clear to default */
                }
            }
            soc_mem_field32_set(unit, sgpp_view_id, entry, SVP_PROFILE_INDEXf,
                                svp_profile);
        }
        if (info->valid_elements & BCM_FLOW_MATCH_FLEX_DATA_VALID) {
            if (SOC_MEM_FIELD_VALID(unit, sgpp_view_id, IDf)) {

                /* TD3 only, not include MV2 and HX5 */
                if (SOC_IS_TRIDENT3(unit)) {
                    rv = _gbp_handle_option_check(unit, info->flow_handle,
                                 info->flow_option,
                                 &result);
                    if (BCM_FAILURE(rv)) {
                        soc_mem_unlock(unit,mem);
                    ...
                    if (result) {
                        soc_fld = 0;
                        if (gbp_sel == _OPAQUE_OBJECT_1) {
                            soc_fld = LOWER_OPAQUE_1_STRENGTH_FROM_IFPf;
                        } else if (gbp_sel == _OPAQUE_OBJECT_2) {
                            soc_fld = LOWER_OPAQUE_2_STRENGTH_FROM_IFPf;
                        } else if (gbp_sel == _OPAQUE_OBJECT_3) {
                            soc_fld = LOWER_OPAQUE_3_STRENGTH_FROM_IFPf;
                        } else if (gbp_sel == _OPAQUE_OBJECT_4) {
                            soc_fld = LOWER_OPAQUE_4_STRENGTH_FROM_IFPf;
                        }
                        if (soc_fld) {
                            rv = soc_reg_field32_modify(unit, ING_FLEX_CONFIGr,
                                     REG_PORT_ANY, soc_fld, clear_action? 0: 1);
                            if (BCM_FAILURE(rv)) {
                                soc_mem_unlock(unit,mem);
                                return rv;
                            }
        ...
        if (gport_id == -1) { 
            ...
            rv = soc_mem_write(unit, mem, MEM_BLOCK_ANY, gport_id, entry);
            soc_mem_unlock(unit,mem);
            BCM_IF_ERROR_RETURN(rv);
            return rv;
        }
    ...
    char **option_str)
{
    *option_str = NULL;
    ...
    } else if (info->criteria == BCM_FLOW_MATCH_CRITERIA_SIP_DIP) {
        if (info->valid_elements & (BCM_FLOW_MATCH_SIP_VALID |
                                    BCM_FLOW_MATCH_DIP_VALID |
                                    BCM_FLOW_MATCH_FLOW_PORT_VALID)) {
            *option_str = "LOOKUP_IPV4_SIP_DIP_ASSIGN_SVP";
        }
    }
}
        ...
        int *replaced_vp)
{
    ...
    if (info->valid_elements & BCM_FLOW_MATCH_VPN_VALID) {
        int vfi = 0;
        if (_BCM_MPLS_VPN_IS_VPLS(info->vpn)) {
            _BCM_MPLS_VPN_GET(vfi, _BCM_MPLS_VPN_TYPE_VPLS, info->vpn);
        } else {
            BCM_IF_ERROR_RETURN(
        ...
        uint32 *entry)
{
    ...
    if (_BCM_FLOW_IS_FLEX_VIEW(info)) {
        ...
        if (SOC_MEM_FIELD_VALID(unit, mem, DESTINATION_ACTION_SETf)) {
            dest = soc_mem_field32_dest_get(unit, mem, entry,
            ...
            } else if (dest_type == SOC_MEM_FIF_DEST_ECMP) {
                info->intf_id = dest + BCM_XGS3_MPATH_EGRESS_IDX_MIN(unit);
                info->valid_elements |= BCM_FLOW_MATCH_IIF_VALID;
            }
        }
    } else {
...
STATIC
int _bcm_flow_match_entry_srv6_key_set(int unit,
    bcm_flow_match_config_t *info,
    soc_mem_t mem_id,
    uint32 *entry)
{
    ...
    int fwd_to_nh;
    struct _srv6_tnl_action_map {
        bcm_flow_tunnel_srv6_action_t action;
        char *name1;
        int  svp_profile;
    } map[] = {
        {bcmFlowTunnelSrv6ActionTransitStdSid,       "SRV6_NEXT_SEGMENT"},
        {bcmFlowTunnelSrv6ActionTransitGsid,         "SRV6_CSID_PREFIX1"},
        {bcmFlowTunnelSrv6ActionTransitGsidPrefix2,  "SRV6_CSID_PREFIX2"},
        {bcmFlowTunnelSrv6ActionTransitUsid,         "SRV6_CSID_PREFIX1"},
        {bcmFlowTunnelSrv6ActionTransitUsidPrefix2,  "SRV6_CSID_PREFIX2"},
        {bcmFlowTunnelSrv6ActionPsp,                 "SRV6_SRH_LAYER_REMOVAL"}
    };
    ...
    if ((!(info->valid_elements & BCM_FLOW_MATCH_TNL_ACTION_VALID)) ||
        (!(info->valid_elements & BCM_FLOW_MATCH_DIP_V6_VALID))) {
        return BCM_E_PARAM;
    }
    flow_info = FLOW_INFO(unit);
    for (ix = 0; ix < COUNTOF(map); ix++) {
        if (map[ix].action == info->tunnel_action) {
            rv = soc_cancun_ceh_obj_field_get(unit,
                     "UAT_EXTRACTION_CTRL_ID_ACTION_SET",
                     map[ix].name1, &ceh_info);
            BCM_IF_ERROR_RETURN(rv);
            action_value = ceh_info.value;
            break;
        }
    }

    soc_mem_field32_set(unit, mem_id, entry,
                UAT_EXTRACTION_CTRL_ID_ACTION_SETf, action_value);
    fwd_to_nh = FALSE;
    if (info->valid_elements & BCM_FLOW_MATCH_IIF_VALID) {
        if (BCM_XGS3_L3_MPATH_EGRESS_IDX_VALID(unit, info->intf_id) ||
            BCM_XGS3_L3_EGRESS_IDX_VALID(unit, info->intf_id)) {
            fwd_to_nh = TRUE;
        }
    }
    if (fwd_to_nh == FALSE) {
        if (info->tunnel_action == bcmFlowTunnelSrv6ActionTransitStdSid) {
            soc_mem_field32_set(unit, mem_id, entry,
                        SVP_ACTION_SETf, (flow_info->svp_next_seg) << 1);
        } else if (info->tunnel_action == bcmFlowTunnelSrv6ActionTransitGsid ||
                   info->tunnel_action == bcmFlowTunnelSrv6ActionTransitUsid) {
            soc_mem_field32_set(unit, mem_id, entry,
                        SVP_ACTION_SETf, (flow_info->svp_csid_prefix1) << 1);
        } else if (info->tunnel_action == bcmFlowTunnelSrv6ActionTransitGsidPrefix2 ||
                   info->tunnel_action == bcmFlowTunnelSrv6ActionTransitUsidPrefix2) {
            soc_mem_field32_set(unit, mem_id, entry,
                        SVP_ACTION_SETf, (flow_info->svp_csid_prefix2) << 1);
        }
    } else {
        /* SVP must be zero */
        soc_mem_field32_set(unit, mem_id, entry, SVP_ACTION_SETf, 0);
    }
    rv = bcm_esw_flow_logical_field_id_get(unit, flow_info->flow_srv6,
                     "SRV6_DIP", &lfield.id);
    BCM_IF_ERROR_RETURN(rv);
    soc_mem_ip6_addr_set(unit, mem_id, entry, lfield.id, info->dip6,
                         SOC_MEM_IP6_UPPER_96BIT);

    if (info->valid_elements & BCM_FLOW_MATCH_NEXT_HDR_VALID) {
        rv = bcm_esw_flow_logical_field_id_get(unit, flow_info->flow_srv6,
                     "SRH_NEXT_HDR", &lfield.id);
        BCM_IF_ERROR_RETURN(rv);
        soc_mem_field32_set(unit, mem_id, entry, lfield.id, info->next_hdr);
    }
    if (info->valid_elements & BCM_FLOW_MATCH_CLASS_ID_VALID) {
        soc_mem_field32_set(unit, mem_id, entry,
                   CLASS_ID_ACTION_SETf, info->class_id);
    }
    if (info->valid_elements & BCM_FLOW_MATCH_VLAN_VALID) {
        rv = bcm_esw_flow_logical_field_id_get(unit, flow_info->flow_srv6,
                     "PAYLOAD_OVID", &lfield.id);
        BCM_IF_ERROR_RETURN(rv);
        soc_mem_field32_set(unit, mem_id, entry, lfield.id, info->vlan);
    }
    return BCM_E_NONE;
}
    ...
    uint32 *entry)
{
    ...
    if (info->criteria == BCM_FLOW_MATCH_CRITERIA_PORT_VLAN ) {
        ...
        soc_mem_field32_set(unit,mem_id, entry, OTAGf, info->vlan);
               ...
               (info->criteria == BCM_FLOW_MATCH_CRITERIA_SIP_DIP)) {
        if (_BCM_FLOW_IS_FLEX_VIEW(info)) {
            ovid_f = OVIDf;
        ...
        /* Ensure the needed switch control is valid.
         * Also ensure only SIP is valid for BCM_FLOW_MATCH_CRITERIA_SIP and
         * both SIP and DIP are valid for BCM_FLOW_MATCH_CRITERIA_SIP_DIP
         */
        rv = _bcmi_esw_flex_match_dip_sip_based_svp_get(unit, &value);
        ...
        if (info->criteria == BCM_FLOW_MATCH_CRITERIA_SIP_DIP) {
            if (!SOC_MEM_FIELD_VALID(unit, mem_id, dip_f)) {
                return BCM_E_PARAM;
            }
            soc_mem_field32_set(unit,mem_id, entry, dip_f, info->dip);
        }
    }  else if (info->criteria == BCM_FLOW_MATCH_CRITERIA_VN_ID ||
    ...
    } else if (info->criteria == BCM_FLOW_MATCH_CRITERIA_DIP ||
               info->criteria == BCM_FLOW_MATCH_CRITERIA_DIP_NHDR ||
               info->criteria == BCM_FLOW_MATCH_CRITERIA_DIP_NHDR_OVID) {
        rv = _bcm_flow_match_entry_srv6_key_set(unit, info, mem_id, entry);

        /* for now, only valid for SRv6 */
        BCM_IF_ERROR_RETURN(rv);
        ...
        if (info->valid_elements & BCM_FLOW_MATCH_VLAN_VALID) {
            if (!SOC_MEM_FIELD_VALID(unit,mem_id,ovid_f)) {
                if (SOC_MEM_FIELD_VALID(unit,mem_id,OUTER_VLANf)) {
                    ovid_f = OUTER_VLANf;
                } else {
                    return SOC_E_PARAM;
                }
    ...
    bcm_flow_logical_field_t *field)
{
    ...
    if (info->criteria == BCM_FLOW_MATCH_CRITERIA_PORT) {
        ...
        if (info->valid_elements & BCM_FLOW_MATCH_IIF_VALID) {
            if (info->intf_id && (info->intf_id < num_iif)) {
                flow_info->iif_ref_cnt[info->intf_id]++;
            }
            if (replaced_iif && (replaced_iif < num_iif)) {
...
/*
 * Function:
 *      _bcm_flow_match_sip_dip_based_svp_view_id_get
 * Description   :
 *      Based on the key_type and data_type, return the mem_view_id that has
 *      the IPV4__DIPf defined, the caller uses this to distinguish between
 *      BCM_FLOW_MATCH_CRITERIA_SIP vs BCM_FLOW_MATCH_CRITERIA_SIP_DIP,
 *      since both of these use the same key_type in HW
 * Returns:
 *      The param 'sip_dip__mem_view_id' points to the sip_dip.
 *
 */
int
_bcm_flow_match_sip_dip_based_svp_view_id_get(int unit,
                                        soc_mem_t mem,
                                        int key_type,
                                        int data_type,
                                        uint32 *sip_dip__mem_view_id)
{
    int rv = 0, jx = 0, num_views = 10;
    soc_mem_t view_id_array[10];

    /* Get the multiple views associated */
    rv = soc_flow_db_mem_to_view_id_multi_get (unit,
                                               MPLS_ENTRY_SINGLEm,
                                               key_type,
                                               data_type,
                                               0,
                                               NULL,
                                               num_views,
                                               (uint32 *)view_id_array);

    SOC_IF_ERROR_RETURN(rv);

    /* Run through each view and look for the 'IPV4__DIPf' field that
     * distinguishes between a SIP vs SIP_DIP view
     */
    for (jx = 0; jx < num_views; jx++) {
        if (!soc_flow_db_mem_view_is_valid(unit, view_id_array[jx])) {
            continue;
        }
        if (SOC_MEM_FIELD_VALID(unit, view_id_array[jx], IPV4__DIPf)) {
            /* Return the mem_view_id of this SIP_DIP view */
            *sip_dip__mem_view_id = view_id_array[jx];
            return BCM_E_NONE;
        }
    }
                             ...
                             bcm_flow_logical_field_t *field)
{
    ...
    if (info->criteria == BCM_FLOW_MATCH_CRITERIA_SIP) {
        ...
        /* In HW, the SIP only vs SIP_DIP view use the same key_type, we
         * hence need to get the mem_view_id related to the SIP_DIP vie,
         * extract the DIP field and if it has a valid value, override
         * the match-criteria to SIP_DIP
         */
        if (soc_feature(unit, soc_feature_vxlan_sip_dip_based_svp_assignment)) {
            rv = _bcm_flow_match_sip_dip_based_svp_view_id_get(unit, mem,
                                  key_type, data_type, &sip_dip_mem_view_id);
            SOC_IF_ERROR_RETURN(rv);

            if ((info->dip = soc_mem_field32_get(unit, sip_dip_mem_view_id, entry, dip_f))) {
                /* Since the DIP is valid, mark DIP as a valid element and
                 * override the SIP criteria with SIP_DIP and
                 */
