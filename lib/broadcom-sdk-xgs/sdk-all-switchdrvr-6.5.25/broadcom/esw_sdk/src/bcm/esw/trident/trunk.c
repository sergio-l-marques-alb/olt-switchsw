...
STATIC int _bcm_trident_trunk_block_mask(int unit,
    ...
    if (_trident_trunk_swfail[unit]) {
        if (_trident_trunk_swfail[unit]->tinfo) {
            ...
            for (i = 0; i < trunk_num_groups; i++) {
                ...
                if (_trident_trunk_swfail[unit]->tinfo[i].member_flags) {
                    sal_free(_trident_trunk_swfail[unit]->tinfo[i].member_flags);
                    _trident_trunk_swfail[unit]->tinfo[i].member_flags = NULL;
                }
            }
        ...
        pbmp_t new_trunk_pbmp)
{
    ...
    if (num_ports > 0) {
        ...
        bm_info[TRUNK_BLOCK_MASK_TYPE_IPMC].member_count = ipmc_member_count;
        ...
        bm_info[TRUNK_BLOCK_MASK_TYPE_L2MC].member_count = l2mc_member_count;
        ...
        bm_info[TRUNK_BLOCK_MASK_TYPE_BCAST].member_count = bcast_member_count;
        ...
        bm_info[TRUNK_BLOCK_MASK_TYPE_DLF].member_count = dlf_member_count;
        ...
        bcm_pbmp_t ports_status)
{
    ...
    PBMP_ITER(ports_active, port) {
        ...
        if (tid >= 1 && tid <= chip_info.trunk_group_count) {
            ...
            /* higig trunk */
            rv = _bcm_trident_trunk_fabric_swfailover_trigger
                 (unit,
                  port,
        ...
        uint32 flags)
{
    ...
    for (region = 0; region < TRUNK_BLOCK_MASK_TYPE_COUNT; region++) {
        p = &bm_info[region];
        all_local = FALSE;
        if ((p->localport_count > 0) && (p->localport_count == p->member_count)) {
            all_local = TRUE;
        }
        index_count = 0;
        ...
        for (i = imin; i <= imax; i++) {
            ...
            if (region == TRUNK_BLOCK_MASK_TYPE_IPMC &&
                ...
                if (p->localport_count > 0) {
                    if (all_local || (p->member_count > BCM_SWITCH_TRUNK_MAX_PORTCNT)) {
                        ...
                        if (SHR_BITGET(p->my_modid_bmap, ((i % 16) % p->member_count))) {
                            SOC_PBMP_PORT_REMOVE(
                                pbmp, p->localport_array[index_count % p->localport_count]);
                            index_count++;
                        }
...
#if defined(BCM_TRIUMPH3_SUPPORT) && defined(INCLUDE_L3)
        if (soc_feature(unit, soc_feature_axp) &&
            !(add_info->flags & BCM_TRUNK_FLAG_MULTI_SET)) {
            rv = _bcm_tr3_wlan_trunk_block_mask_update(unit);
            if (BCM_FAILURE(rv)) {
                _BCM_TRIDENT_TRUNK_FP_SET_FREE_MEM_RETURN(rv);
            }
