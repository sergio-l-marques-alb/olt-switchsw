...
#if defined(BCM_TOMAHAWK3_SUPPORT)
        if (SOC_IS_TOMAHAWK3(unit) &&
            soc_property_get(unit, spn_QOS_DEFAULT_MAP, 0) == 1) {
            /* Program default L3 ingress QoS mapping */
            int i;
            for (i = 0; i < _BCM_QOS_MAP_CHUNK_DSCP; i++) {
                soc_DSCP_TABLEm_field32_set(unit, &dscp_table[i], DSCPf, i);
            }
        } 
#endif /* BCM_TOMAHAWK3_SUPPORT */
...
_bcm_rqe_queue_offset_map_table_compare(int unit, uint32 *entry, int id, int *profile_offset)
{
    ...
    while ((i >= 0) && (i < _BCM_QOS_MAP_LEN_RQE_QUEUE_OFFSET_MAP)) {
        if (!_BCM_QOS_RQE_QUEUE_OFFSET_MAP_USED_GET(unit, i)) {
            i++;
            continue;
        }
...
bcm_tr2_qos_map_add(int unit, uint32 flags, bcm_qos_map_t *map, int map_id, int internal)
{
    ...
    case _BCM_QOS_MAP_TYPE_EGR_DSCP_TABLE:
        if (!_BCM_QOS_EGR_DSCP_TABLE_USED_GET(unit, id)) {
...
#endif
            {
                ...
                /* Reference count of zero means go legacy way of handling:
                 * check for external references, delete old profile and add
                 * new profile entry
                 */
                ref_cnt = 0;

                if (soc_property_get(unit,
                        spn_QOS_MAP_EGR_DSCP_NONSHARABLE_MODE, 0)) {
                    /* Get the reference count of the profile entry */
                    rv = _bcm_egr_dscp_table_entry_ref_count_get(unit,
                                                                 index,
                                                                 &ref_cnt);
                    if (BCM_FAILURE(rv)) {
                        /* If reference count cannot be retrieved, then perform
                         * legacy way of handling by explicilty setting ref cnt
                         * to 0.
                         */
                        ref_cnt = 0;
                    }
                ...
                /* check for external references before delete
                 * If the reference count is one then no problem as it is not a
                 * shared entry, it can be updated in-place. However if it is
                 * not one, it could be shared one hence need to check ext ref.
                 */
                if (soc_feature(unit, soc_feature_egr_dscp_map_ext_ref_check) &&
                    (1 == internal) && (1 != ref_cnt)) {
                    bmp_len = _BCM_QOS_MAP_LEN_EGR_DSCP_TABLE;
                    temp_bmp = sal_alloc(SHR_BITALLOCSIZE(bmp_len), "temp_bmp");
                    if (NULL == temp_bmp) {
                        QOS_UNLOCK(unit);
                ...
                if (1 == ref_cnt) {
                    /* If no one else is sharing
                     * this entry then just do in-place update
                     */
                    rv = _bcm_egr_dscp_table_entry_set(unit,
                                                       entries, index);
                    if (BCM_SUCCESS(rv)) {
                        int mode;
                        rv = _bcm_egr_dscp_table_entry_write_mode_get(unit,
                                                                      index,
                                                                      &mode);
                        /* Reference count increment occurs in entry_set only
                         * in dma mode, hence decrease the profile ref_count
                         * increased by entry_set by deleting the entry once
                         */
                        if (BCM_SUCCESS(rv) &&
                            SOC_PROFILE_MEM_WRITE_DMA_MODE == mode) {
                            (void)_bcm_egr_dscp_table_entry_delete(unit, index);
                        }
                    }
                } else {
                    /* Perform legacy way
                     * Others are sharing this entry too.
                     * Delete the old profile chunk
                     */
                    ...
                    /* Add the new entry and get the new profile index */
    ...
    default:
        ...
        if (SOC_IS_HURRICANE4(unit) &&
            ...
            (flags & BCM_QOS_MAP_SUBPORT)) {
            rv = soc_hr4_xflow_macse_svtag_dot1p_map_add(unit, map);
        } else
...
bcm_tr2_qos_map_delete(int unit, uint32 flags, bcm_qos_map_t *map, int map_id)
{
    ...
    default:
        ...
        if (SOC_IS_HURRICANE4(unit) &&
            soc_feature(unit, soc_feature_xflow_macsec) &&
            (flags & BCM_QOS_MAP_SUBPORT)) {
            rv = soc_hr4_xflow_macse_svtag_dot1p_map_delete(unit, map);
        } else
#endif /* defined(INCLUDE_XFLOW_MACSEC) */
        {
            sal_memcpy(&clr_map, map, sizeof(clr_map));
