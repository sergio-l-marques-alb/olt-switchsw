...
#define DPC_OWNER(_u, _fn) (((char *)(unsigned long)_fn) + _u)
...
_bcm_th3_pfc_deadlock_cb_update(int unit, int arg)
{
    ...
    if (arg == 0) {
        ...
        PFC_UNLOCK(unit);
        if (dd_enabled == 0) {
            sal_dpc_cancel(DPC_OWNER(unit, _th3_pfc_deadlock_cb));
...
/*
 * Function:
 *      _bcm_th3_pfc_deadlock_port_enable_get
 * Purpose:
 *      Returns pfc deadlock recovery enabled priorities for the entire
 *      port. Same as call to bcm_tomahawk3_cosq_pfc_deadlock_control_get with
 *      bcmCosqPFCDeadlockDetectionAndRecoveryEnable but returns the entire 32
 *      bit value rather than whether a pfc_priority is enabled.
 * Parameters:
 *      unit             - (IN) device id.
 *      port             - (IN) logical port number.
 *      enabled_priority - (OUT) enabled_priority for the port.
 * Returns:
 *      SOC_E_XXX
 */
int
_bcm_th3_pfc_deadlock_port_enable_get(int unit, bcm_port_t port,
                            uint32 *enabled_priority)
{
    _bcm_th3_pfc_deadlock_port_config_t *port_config = NULL;

    if (!SOC_PORT_VALID(unit, port) || enabled_priority == NULL) {
        return BCM_E_PARAM;
    }

    PFC_LOCK(unit);
    port_config = _BCM_TH3_PFC_DEADLOCK_CONFIG(unit, port);
    *enabled_priority = port_config->enabled_priority;
    PFC_UNLOCK(unit);
    ...
    int *arg)
{
    ...
    switch (type) {
        case bcmCosqPFCDeadlockDetectionAndRecoveryEnable:
            PFC_LOCK(unit);
            port_config = _BCM_TH3_PFC_DEADLOCK_CONFIG(unit, port);
            *arg = (port_config->enabled_priority & (1U << pfc_priority)) > 0 ? 1 : 0;
            PFC_UNLOCK(unit);
            break;
        case bcmCosqPFCDeadlockDetectionTimer:
        case bcmCosqPFCDeadlockRecoveryTimer:
...
_bcm_th3_pfc_deadlock_reinit(int unit, uint8 **scache_ptr, uint16 recovered_ver)
{
    ...
    soc_reg_t int_mask_reg = MMU_INTFI_DD_TIMER_INT_MASKr;
    soc_field_t mask_field = PFC_PRIORITY_INT_MASKf;
    int port, max_num_ports = 0, min_max_num_ports = 0;
    uint16 *u16_scache_p;
    uint32 *u32_scache_p;
    ...
    max_num_ports = soc_scache_dictionary_entry_get(unit,
                                     ...
                                     SOC_MAX_NUM_PORTS);
    min_max_num_ports = SOC_SCACHE_SIZE_MIN(max_num_ports, SOC_MAX_NUM_PORTS);
    ...
    *scache_ptr += (max_num_ports * 8 * sizeof(uint32));
    ...
    for (port = 0; port < min_max_num_ports; port++) {
    ...
    if (recovered_ver >= BCM_WB_VERSION_1_2) {
        /* Recover the pfc deadlock recovery enable for each port */
        u32_scache_p = (uint32 *)(*scache_ptr);
        PBMP_PORT_ITER(unit, port) {
            port_config = &(dd_control->port_config[port]);
            port_config->enabled_priority |= *u32_scache_p++;
        }
        *scache_ptr += (max_num_ports * sizeof(uint32));
    }
...
_bcm_th3_pfc_deadlock_recovery_reset(int unit)
{
    ...
    if (dd_enabled == 1) {
        ...
        if (dd_control->cb_enabled == FALSE) {
            ...
            _th3_pfc_deadlock_cb(DPC_OWNER(unit, _th3_pfc_deadlock_cb),
                                 INT_TO_PTR(_BCM_TH3_PFC_DEADLOCK_CB_INTERVAL(unit)),
                                 INT_TO_PTR(unit), 0, 0);
        }
    }
