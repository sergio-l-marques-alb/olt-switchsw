                               ...
                               bcm_ifa_cc_config_t *config_data)
{
            ...
            if (!soc_iproc_m0ssq_reset_ucore_get(unit, i, &reset)) {
                if (reset) {
                    soc_uc_mem_write(unit,
                                     (sram_base_addr + BCM_IFA_CC_M0_EAPP_STATUS_OFFSET),
                                     (uint32)0);
                    soc_uc_mem_write(unit, (sram_base_offset + ifa_cc_offset(app_status)),
                                     (uint32)0);
                    soc_uc_mem_write(unit, (sram_base_offset + ifa_cc_offset(app_status) + 4),
                                     (uint32)0);
                    continue;
                }
            } else {
    ...
    for (j = 0; j < num_core; j++) {
        ...
        if (SOC_IS_MAVERICK2(unit)) {
            soc_uc_mem_write(unit,
                    (sram_base_offset + ifa_cc_offset(num_pipes)),
                    (uint32)_MV2_PIPES_PER_DEV);
            soc_uc_mem_write(unit,
                    (sram_base_offset + ifa_cc_offset(queues_per_pipe)),
                    (uint32)(SOC_MV2_NUM_UC_QUEUES_PER_PIPE - 10));

            soc_uc_mem_write(unit,
                    (sram_base_offset + ifa_cc_offset(queues_per_port)),
                    (uint32)SOC_MV2_NUM_UCAST_QUEUE_PER_PORT);

            soc_uc_mem_write(unit,
                    (sram_base_offset + ifa_cc_offset(ports_per_pipe)),
                    (uint32)_MV2_PORTS_PER_PIPE);
        }
        ...
        if (config_data->flag & BCM_IFA_CC_IFA2) {
                /* Key is formed as below:
                 * KEY_TYPE<7:3> = 29 (Index to KEY_ATTRIBUTES table)
                 * DGPP<23:8>
                 * INT_PRI<27:24>
                 * I2E_CLASS_ID_5_4<29:28>
                 */
                index = 0;
                sal_memset(bufp, 0, sizeof(uint32) * SOC_MAX_MEM_FIELD_WORDS);
                bufp[0]   =  (29 << 3);
                ...
                soc_uc_mem_write(unit,
                        (sram_base_offset + ifa_cc_offset(egr_vlan_xlate_tbl_offset1) + (i * 4)),
                        (uint32)index);

                soc_uc_mem_write(unit,
                        (sram_base_offset + ifa_cc_offset(egr_vlan_xlate_tbl_data1) + (i * 16)),
                        (uint32)bufp[0]);
                soc_uc_mem_write(unit,
                        (sram_base_offset + ifa_cc_offset(egr_vlan_xlate_tbl_data1) + (i * 16) + 4),
                        (uint32)bufp[1]);

                soc_uc_mem_write(unit,
                        (sram_base_offset + ifa_cc_offset(egr_vlan_xlate_tbl_data1) + (i * 16) + 8),
                        (uint32)bufp[2]);

                soc_uc_mem_write(unit,
                        (sram_base_offset + ifa_cc_offset(egr_vlan_xlate_tbl_data1) + (i * 16) + 12),
                        (uint32)bufp[3]);
            }
            newnode = temp;
            temp = temp->next;
