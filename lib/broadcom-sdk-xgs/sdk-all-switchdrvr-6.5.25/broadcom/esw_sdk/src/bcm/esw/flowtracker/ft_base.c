...
/* For 6.5.24, Added sync/recv for src_ts in group alu info */
#define BCM_WB_VERSION_1_5                SOC_SCACHE_VERSION(1,5)
/* For 6.5.21, Added sync/recv for param/check stage */
 ...
 */
int
bcmi_ft_init(int unit, bcm_flowtracker_cfg_t *cfg)
{
    ...
    result = bcmi_ft_alu_load_init(unit, cfg);
    if (BCM_FAILURE(result)) {
        goto cleanup;
    }
    ...
    if (soc_feature(unit, soc_feature_flex_flowtracker_ver_3) &&
        ...
        result = soc_mem_read(unit, BSC_DG_ECC_CORRUPT_CONTROLm, MEM_BLOCK_ALL,
                                                    0, &ft_ts_u16b_entry);
        if (BCM_FAILURE(result)) {
            goto cleanup;
        }
 ...
 *   bcmi_ft_cfg_get
 * Purpose:
 *   Get Flowtracker configuration.
 * Parameters:
 *   unit - (IN) BCM device id
 *   cfg  - (OUT) flowtracker resource config
 * Returns:
 *   None.
 */
int
bcmi_ft_cfg_get(int unit, bcm_flowtracker_cfg_t *cfg)
{
    if (cfg == NULL) {
        return BCM_E_PARAM;
    }
    ...
    sal_memset(cfg, 0, sizeof(bcm_flowtracker_cfg_t));

    cfg->chip_debug_enable = BCMI_FT_CHIP_DEBUG_ENABLE(unit);

#ifdef BCM_FLOWTRACKER_V2_SUPPORT
    if (soc_feature(unit, soc_feature_flex_flowtracker_ver_2)) {
        BCM_IF_ERROR_RETURN
            (bcmi_ftv2_resource_cfg_get(unit, cfg));
    }
#endif /* BCM_FLOWTRACKER_V2_SUPPORT */
    ...
    if (soc_feature(unit, soc_feature_flex_flowtracker_ver_3)) {
        BCM_IF_ERROR_RETURN
            (bcmi_ftv3_resource_cfg_get(unit, cfg));
    }
#endif /* BCM_FLOWTRACKER_V3_SUPPORT */
                            ...
                            int *alloc_size_p)
{
    ...
    for (i=0; i < length; i++) {
        ...
        if (BCM_WB_DEFAULT_VERSION >= BCM_WB_VERSION_1_4) {
            alloc_size += sizeof(bcm_flowtracker_stage_t);
        }
    }
        ...
        uint8 **p_ft_scache_ptr)
{
    ...
    for (i=0; i < length; i++) {
        ...
        sal_memcpy(ft_scache_ptr, &t_info->mask,
                sizeof(bcm_flowtracker_tracking_param_mask_t));
...
STATIC int
bcmi_ft_sync_ver_1_4(int unit, uint8 **p_ft_scache_ptr)
{
    int i = 0, j = 0, num_groups = 0;
    int num_flowcheckers = 0, length = 0;
    uint8 *ft_scache_ptr = NULL;
    ...
    /* Sync tracking param stage information per group. */
    for(i = 0; i < num_groups; i++) {
        BCM_FLOWTRACKER_GROUP_ID_SET(id, bcmFlowtrackerGroupTypeNone, i);
        if (bcmi_ft_group_is_invalid(unit, id)) {
            continue;
        ...
        t_info = BCMI_FT_GROUP_TRACK_PARAM(unit, id);
        for (j = 0; j < length; j++, t_info++) {
            /* Sync stage in tracking param info */
            sal_memcpy(ft_scache_ptr, &t_info->stage,
                    sizeof(bcm_flowtracker_stage_t));
            ft_scache_ptr += sizeof(bcm_flowtracker_stage_t);
        }
    }
    ...
    /* Sync stage in check information */
    for (i = 0; i < num_flowcheckers; i++) {
        if (!(SHR_BITGET(BCMI_FT_FLOWCHECKER_BITMAP(unit), i))) {
            /* If the particular id is not set then do not save state. */
            continue;
        ...
        sal_memcpy(ft_scache_ptr,
                &BCMI_FT_FLOWCHECKER_INFO(unit, i)->check1.stage,
                sizeof(bcm_flowtracker_stage_t));
        ft_scache_ptr += sizeof(bcm_flowtracker_stage_t);

        sal_memcpy(ft_scache_ptr,
                &BCMI_FT_FLOWCHECKER_INFO(unit, i)->check2.stage,
                sizeof(bcm_flowtracker_stage_t));
        ft_scache_ptr += sizeof(bcm_flowtracker_stage_t);
    }
...
STATIC int
bcmi_ft_sync_ver_1_5(int unit, uint8 **p_ft_scache_ptr)
{
    int i = 0, j = 0, num_groups = 0;
    int length = 0;
    uint8 *ft_scache_ptr;
    ...
    /* Sync Change in src_ts in group_alu_info per group */
    for(i = 0; i < num_groups; i++) {
        BCM_FLOWTRACKER_GROUP_ID_SET(id, bcmFlowtrackerGroupTypeNone, i);
        if (bcmi_ft_group_is_invalid(unit, id)) {
            continue;
        ...
        for (j = 0; j < length; j++, temp++) {
            ...
            /* Sync src_ts */
            sal_memcpy(ft_scache_ptr, &(temp->src_ts),
                    sizeof(bcm_flowtracker_timestamp_source_t));
            ft_scache_ptr += sizeof(bcm_flowtracker_timestamp_source_t);
        }
    }
...
bcmi_ft_sync(int unit)
{
    ...
    /* Version 1_4 Sync */
    BCM_IF_ERROR_RETURN(bcmi_ft_sync_ver_1_4(unit, &ft_scache_ptr));
    /* Version 1_5 Sync */
    BCM_IF_ERROR_RETURN(bcmi_ft_sync_ver_1_5(unit, &ft_scache_ptr));
...
STATIC int
bcmi_ft_recover_ver_1_4(int unit, uint8 **p_ft_scache_ptr)
{
    int i = 0, j = 0;
    int num_groups = 0;
    uint8 *ft_scache_ptr;
    ...
    for(i = 0; i < num_groups; i++) {
        ...
        t_info = BCMI_FT_GROUP_TRACK_PARAM(unit, id);
        for (j = 0; j < length; j++, t_info++) {
            sal_memcpy(&t_info->stage, ft_scache_ptr,
                        sizeof(bcm_flowtracker_stage_t));
            ft_scache_ptr += sizeof(bcm_flowtracker_stage_t);
        }
    }
    ...
    for (i = 0; i < num_flowcheckers; i++) {
        ...
        sal_memcpy(&BCMI_FT_FLOWCHECKER_INFO(unit, i)->check1.stage,
                ft_scache_ptr, sizeof(bcm_flowtracker_stage_t));
        ft_scache_ptr += sizeof(bcm_flowtracker_stage_t);
        sal_memcpy(&BCMI_FT_FLOWCHECKER_INFO(unit, i)->check2.stage,
                ft_scache_ptr, sizeof(bcm_flowtracker_stage_t));
        ft_scache_ptr += sizeof(bcm_flowtracker_stage_t);
    }
...
STATIC int
bcmi_ft_recover_ver_1_5(int unit, uint8 **p_ft_scache_ptr)
{
    int i = 0, j = 0;
    int num_groups = 0;
    uint8 *ft_scache_ptr;
    ...
    for(i = 0; i < num_groups; i++) {
        ...
        for (j = 0; j < length; j++, temp++) {
            ...
            sal_memcpy(&(temp->src_ts), ft_scache_ptr, sizeof(bcm_flowtracker_timestamp_source_t));
            ft_scache_ptr += sizeof(bcm_flowtracker_timestamp_source_t);
        }
    }
...
bcmi_ft_recover(int unit)
{
    ...
    if (recovered_ver >= BCM_WB_VERSION_1_4) {
        bcmi_ft_recover_ver_1_4(unit, &ft_scache_ptr);
    }
    if (recovered_ver >= BCM_WB_VERSION_1_5) {
        bcmi_ft_recover_ver_1_5(unit, &ft_scache_ptr);
    }
                                  ...
                                  bcmi_ft_alu_load_type_t alu_load_type)
{
    ...
    if ((alu_load_type == bcmiFtAluLoadTypeLoad16) ||
        ...
        for (i = 0; i < total_loads_mem; i++) {
            ...
            /* Set alu_ft_type */
            if (local->alu_ft_type == bcmiFtTypeNone) {
        ...
        if (info->alu_ft_type == bcmiFtTypeNone) {
    ...
    int arg)
{
    ...
    switch(type) {
        case bcmFlowtrackerControlTcpBidirEstablishedDoNotLearn:
             ...
             * b2 = SYN
             * b3 = ACK
             * Learn disable = 1, for b3b2b1b0 = 0001, 0011, 1001, 1011
             */
            ...
            if (arg != 0) {
                ...
                soc_mem_field32_set(unit, mem, (uint32 *)&kmap2, KMAP_INPUT_SELECT_1f, 6);
                ...
                soc_mem_field32_set(unit, mem, (uint32 *)&kmap2, KMAP_CONTROLf, 0x0a0a);
            }
...
/*
 * Function:
 *  bcmi_ft_tracking_param_type_info_get
 * Purpose:
 *  Get Tracking params type information for given group type
 * Parameters:
 *  unit          - (IN) BCM device number
 *  group_type    - (IN) Flowtracker group type
 *  param         - (IN) Tracking param
 *  info          - (OUT) Tracking param type info
 * Returns:
 *  BCM_E_XXX   - BCM error code.
 */
int
...
{
    return _bcm_field_ft_tracking_param_type_info_get(unit,
            group_type, param, info);
}
