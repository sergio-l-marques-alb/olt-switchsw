...
_bcm_modules_init(int unit)
{
    ...
    if (SOC_IS_TRIDENT3X(unit)) {
        /*
         * TD3 b870-hgoe.1.0.0 CANCUN supports fabric mode only.
         * Needs update to disable unsupported features.
         */
        ...
        if (soc_feature(unit, soc_feature_higig3)) {
            soc_td3_hg3_feature_update(unit);
        }
    }
#endif
#if defined(BCM_TRIDENT3_SUPPORT)
...
bcm_esw_info_get(int unit, bcm_info_t *info)
{
    ...
    if (SOC_IS_TOMAHAWK3(unit)) {
        if (dev_id == BCM56983_DEVICE_ID) {
            info->num_pipes = NUM_PIPE(unit)/2;
            info->num_pp_pipes = NUM_PIPE(unit)/2;
            info->num_buffers = NUM_ITM(unit);
        } else {
            info->num_pp_pipes = NUM_PIPE(unit);
            info->num_buffers = NUM_ITM(unit);
        }
    }
#endif
