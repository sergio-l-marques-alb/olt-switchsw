                                      ...
                                      _field_action_t *fa)
{
    ...
    switch (action) {
       case bcmFieldActionGroupClassSet:
            ...
            break;
       case bcmFieldActionDropMaskControlProfile:
            if (fa->param[0] >= bcmFieldDropMaskControlLastCount) {
                return BCM_E_PARAM;
            }
            break;
                               ...
                               _field_action_t *fa, uint32 *buf)
{
    ...
    switch (fa->action) {
       ...
       case  bcmFieldActionDropMaskControlProfile:
            switch (fa->param[0]) {
                case bcmFieldDropMaskControlProfile0:
                    val = 0;
                    break;
                case bcmFieldDropMaskControlProfile1:
                    val = 1;
                    break;
                default:
                    return BCM_E_INTERNAL;
            }
            soc_mem_field32_set(unit, data_mem, buf, DROP_VECTOR_MASK_SELECTf,
                                val);
            break;
       case bcmFieldActionGroupLookupDisable:
                                            ...
                                            _field_group_t *fg)
{
    ...
    int rv;                         /* Operation return status.         */
    _field_control_t    *fc;        /* Field control structure.     */
    _field_group_t      *fg_ptr;    /* Field group structure.       */
    int lt_id_bmp = 0;
    ...
    fg_ptr = NULL;
    /* Retrieve LT IDs allocated to the given pipe stage */
    do {
        rv = _bcm_field_th_priority_group_get_next(unit, fg->instance,
                                                   fg->stage_id,
                                                   -1,
                                                   &fg_ptr, &fg_ptr);
        if (BCM_SUCCESS(rv)) {
            lt_id_bmp |= 1 << fg_ptr->lt_id;
        }
    } while(fg_ptr != NULL);
...
_bcm_field_th_presel_entry_hw_remove(int unit, _field_presel_entry_t *f_presel)
{
    ...
    _field_presel_entry_t *f_presel_p;       /* Presel Part entry.          */
     _field_lt_slice_t     *lt_fs_p = NULL;
   ...
   /* Input parameters check. */
    ...
    for (part = 0; part < parts_count; part++) {
        ...
        if (f_presel_p->lt_fs->next) {
            for (lt_fs_p = f_presel_p->lt_fs->next; (lt_fs_p); lt_fs_p = lt_fs_p->next) {
                /* Given entry part number, get the entry TCAM index. */
                BCM_IF_ERROR_RETURN(_bcm_field_presel_entry_tcam_idx_get(unit,
                                    f_presel_p, lt_fs_p, &tcam_idx));
...
_bcm_field_max_lt_part_prio_get(int unit)
{
    ...
    if (soc_feature(unit, soc_feature_utt)) {
        ...
        if (soc_feature(unit, soc_feature_td3x1_reduced_utt)) {
            if (soc_feature(unit, soc_feature_field_slices7)) {
                return 7;
            } else if (soc_feature(unit, soc_feature_field_slices6)) {
                return 6;
            } else if (soc_feature(unit, soc_feature_field_slices5)) {
                return 5;
