...
/* fixed SVP_PROFILE_INDEXs for SRv6 transit flow */
#define SRV6_NEXT_SEGMENT  5
#define SRV6_CSID_PREFIX1  6
#define SRV6_CSID_PREFIX2  7
#define SRV6_TRANSIT_NUM_VP 3  /* number of fixed VPs for transit flow */

extern int _bcm_xgs3_l3_ingress_interface_add(int unit, _bcm_l3_ingress_intf_t *info);
extern int _bcm_xgs3_l3_ingress_interface_delete(int unit, bcm_if_t intf_id);
/*
 * Function:
...
#define BCM_WB_VERSION_2_1                SOC_SCACHE_VERSION(2, 1)
#define BCM_WB_DEFAULT_VERSION            BCM_WB_VERSION_2_1
...
_bcm_flow_wb_recover(int unit)
{
    ...
    if (flow_scache_ptr != NULL) {
        ...
        extra_soft_tnl = soc_mem_index_count(unit, EGR_L3_NEXT_HOPm);
        sz = sizeof(uint16);
        if (recovered_ver >= BCM_WB_VERSION_2_1) {
            /* recover additional init_tunnel state */
            for (i = 0; i < extra_soft_tnl; i++) {
                sal_memcpy(&flow_info->init_tunnel[i + num_soft_tnl].idx,
                      flow_scache_ptr, sz);
                flow_scache_ptr += sz;
            }
        } else {
            additional_scache_size += extra_soft_tnl * sz;
        }
...
_bcm_flow_wb_alloc(int unit)
{
    ...
    /* increase the size of soft_tnl from BCM_WB_VERSION_2_1 */
    alloc_sz += soc_mem_index_count(unit, EGR_L3_NEXT_HOPm) * sizeof(uint16);
...
_bcm_esw_flow_sync(int unit)
{
    ...
    /* save additional init_tunnel state */
    extra_soft_tnl = soc_mem_index_count(unit, EGR_L3_NEXT_HOPm);
    sz = sizeof(uint16);
    for (i = 0; i < extra_soft_tnl; i++) {
        sal_memcpy(flow_scache_ptr,
                   &flow_info->init_tunnel[i + num_soft_tnl].idx, sz);
        flow_scache_ptr += sz;
    }
    return rv;
}
 ...
 *      _bcm_flow_srv6_init
 * Purpose:
 *      Populate SRv6 flow information, allocate fixed SVP resource and
 *   enable SRv6 flow based on configuration.
 *
 * Parameters:
 *      unit     - Device Number
 * Returns:
 ...
 */
int
_bcm_flow_srv6_init(int unit)
{
    uint32 flow_option;
    _bcm_vp_info_t vp_info;
    int vp, num_vp;
    _bcm_flow_bookkeeping_t *flow_info;
    _bcm_l3_ingress_intf_t intf_info;
    ...
    if (soc_property_get(unit, spn_L3_SRV6_ENABLE, 0)) {

        SOC_IF_ERROR_RETURN(soc_flow_db_flow_handle_get(unit, "SRV6",
                          &flow_info->flow_srv6));
        SOC_IF_ERROR_RETURN(soc_flow_db_flow_handle_get(unit,
                          "SRV6_NON_CANONICAL",
                          &flow_info->flow_srv6_non_canonical));
        SOC_IF_ERROR_RETURN(soc_flow_db_flow_handle_get(unit, "SRV6_EVPN",
                          &flow_info->flow_srv6_evpn));
        SOC_IF_ERROR_RETURN(soc_flow_db_flow_option_id_get(unit,
                          flow_info->flow_srv6,
                          "L3_ASSIGN_SEGMENT_LIST_1_AND_NEXT_PTR",
                          &flow_option));
        SOC_IF_ERROR_RETURN(soc_flow_db_ffo_to_mem_view_id_get(unit,
                          flow_info->flow_srv6, flow_option,
                          SOC_FLOW_DB_FUNC_EGRESS_OBJ_ID,
                          &flow_info->nh2_mem_view));
        SOC_IF_ERROR_RETURN(soc_flow_db_flow_option_id_get(unit,
                          flow_info->flow_srv6,
                          "L3_ASSIGN_SIP_AND_SEGMENT_LIST_2",
                          &flow_option));
        SOC_IF_ERROR_RETURN(soc_flow_db_ffo_to_mem_view_id_get(unit,
                      flow_info->flow_srv6, flow_option,
                      SOC_FLOW_DB_FUNC_TUNNEL_INITIATOR_ID,
                      &flow_info->tnl_l3_mem_view));
        SOC_IF_ERROR_RETURN(soc_flow_db_flow_option_id_get(unit,
                      flow_info->flow_srv6,
                      "L2_ASSIGN_SIP_SRH_BASE_SEGMENT_LIST_2_AND_1",
                       &flow_info->option_id_l2tnl));
        SOC_IF_ERROR_RETURN(soc_flow_db_flow_option_id_get(unit,
                      flow_info->flow_srv6,
                      "L3_ASSIGN_SIP_AND_SEGMENT_LIST_2",
                       &flow_info->option_id_l3tnl));
        SOC_IF_ERROR_RETURN(soc_flow_db_flow_option_id_get(unit,
                      flow_info->flow_srv6,
                      "ADAPT_1_LOOKUP_DIP_ASSIGN_SVP_L3_IIF_AND_TUNNEL_TERM",
                       &flow_info->option_term_adapt1));
        flow_info->option_transit_adapt1 = flow_info->option_term_adapt1;
        SOC_IF_ERROR_RETURN(soc_flow_db_flow_option_id_get(unit,
                      flow_info->flow_srv6,
                      "ADAPT_2_LOOKUP_DIP_ASSIGN_SVP_L3_IIF_AND_TUNNEL_TERM",
                       &flow_info->option_term_adapt2));
        flow_info->option_transit_adapt2 = flow_info->option_term_adapt2;
        SOC_IF_ERROR_RETURN(soc_flow_db_flow_option_id_get(unit,
                      flow_info->flow_srv6,
                      "LOOKUP_DIP_ASSIGN_SVP_L3_IIF",
                       &flow_info->option_transit_adapt4));
        SOC_IF_ERROR_RETURN(soc_flow_db_flow_option_id_get(unit,
                    flow_info->flow_srv6,
                    "ADAPT_1_PSP_LOOKUP_DIP_ASSIGN_SVP_L3_IIF_AND_TUNNEL_TERM",
                     &flow_info->option_psp_adapt1));
        SOC_IF_ERROR_RETURN(soc_flow_db_flow_option_id_get(unit,
                    flow_info->flow_srv6,
                    "ADAPT_2_PSP_LOOKUP_DIP_ASSIGN_SVP_L3_IIF_AND_TUNNEL_TERM",
                     &flow_info->option_psp_adapt2));
        SOC_IF_ERROR_RETURN(soc_flow_db_flow_option_id_get(unit,
                    flow_info->flow_srv6,
                    "PSP_LOOKUP_DIP_ASSIGN_SVP_L3_IIF",
                     &flow_info->option_psp_adapt4));

        SOC_IF_ERROR_RETURN(soc_reg_field32_modify(unit, ING_FLEX_CONFIGr,
                       REG_PORT_ANY, SRV6_ENABLEf, 1));

        /* allocate three vp objects for srv6 transit flow usage */
        num_vp = soc_mem_index_count(unit, SOURCE_VPm);
        _bcm_vp_info_init(&vp_info);
        vp_info.vp_type = _bcmVpTypeFlow;
        if (!flow_info->svp_next_seg) {
            BCM_IF_ERROR_RETURN(_bcm_vp_alloc(unit, 1, (num_vp - 1),
                SRV6_TRANSIT_NUM_VP, SOURCE_VPm, vp_info, &vp));
            flow_info->svp_next_seg = vp;
            flow_info->svp_csid_prefix1 = vp + 1;
            flow_info->svp_csid_prefix2 = vp + 2;
            SOC_IF_ERROR_RETURN(soc_mem_field32_modify(unit, SOURCE_VPm,
                vp, SVP_PROFILE_INDEXf, SRV6_NEXT_SEGMENT));
            SOC_IF_ERROR_RETURN(soc_mem_field32_modify(unit, SOURCE_VPm,
                vp + 1, SVP_PROFILE_INDEXf, SRV6_CSID_PREFIX1));
            SOC_IF_ERROR_RETURN(soc_mem_field32_modify(unit, SOURCE_VPm,
                vp + 2, SVP_PROFILE_INDEXf, SRV6_CSID_PREFIX2));
        }
        if (!flow_info->direct_fwd_iif) {
            sal_memset(&intf_info, 0, sizeof(_bcm_l3_ingress_intf_t));
            intf_info.flags = BCM_L3_INGRESS_GLOBAL_ROUTE;
            intf_info.vrf = BCM_L3_VRF_INVALID;
            BCM_IF_ERROR_RETURN(_bcm_xgs3_l3_ingress_interface_add(unit, &intf_info));
            flow_info->direct_fwd_iif = intf_info.intf_id;
        }

    } else {  /* srv6 disable */

        SOC_IF_ERROR_RETURN(soc_reg_field32_modify(unit, ING_FLEX_CONFIGr,
                       REG_PORT_ANY, SRV6_ENABLEf, 0));
        if (flow_info->svp_next_seg) {
            BCM_IF_ERROR_RETURN(_bcm_vp_free(unit, _bcmVpTypeFlow,
                    SRV6_TRANSIT_NUM_VP, flow_info->svp_next_seg));
        }
        if (flow_info->direct_fwd_iif) {
            BCM_IF_ERROR_RETURN(_bcm_xgs3_l3_ingress_interface_delete(unit,
                                flow_info->direct_fwd_iif));
        }
        flow_info->direct_fwd_iif = 0;
        flow_info->svp_next_seg = 0;
        flow_info->svp_csid_prefix1 = 0;
        flow_info->svp_csid_prefix2 = 0;
        flow_info->flow_srv6 = 0;
        flow_info->flow_srv6_non_canonical = 0;
        flow_info->flow_srv6_evpn = 0;
        flow_info->option_id_l2tnl = 0;
        flow_info->option_id_l3tnl = 0;
        flow_info->nh2_mem_view = 0;
        flow_info->tnl_l3_mem_view = 0;
        flow_info->option_term_adapt1 = 0;
        flow_info->option_term_adapt2 = 0;
        flow_info->option_transit_adapt1 = 0;
        flow_info->option_transit_adapt2 = 0;
        flow_info->option_transit_adapt4 = 0;
        flow_info->option_psp_adapt1 = 0;
        flow_info->option_psp_adapt2 = 0;
        flow_info->option_psp_adapt4 = 0;
    }
    return BCM_E_NONE;
}
...
bcmi_esw_flow_init(int unit)
{
    ...
    /* next_hop type L3 tunnel associated with egr_obj, soft tunnel index:
     * max l2 tunnel + sizeof(EGR_L3_INTFm) + EGR_L3_NEXT_HOP index.
     */
    num_tnl += soc_mem_index_count(unit, EGR_L3_NEXT_HOPm);
