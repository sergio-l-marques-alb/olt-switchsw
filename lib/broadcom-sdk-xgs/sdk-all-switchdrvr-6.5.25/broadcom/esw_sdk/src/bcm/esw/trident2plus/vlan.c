...
bcm_td2p_vlan_vp_group_get(int unit, int vlan_vpn, int egress, int vp_group, int *enable)
{
    ...
    /* safety check */
    field_len = soc_mem_field_length(unit, mbrship_mem, VP_GROUP_BITMAPf);
    if (vp_group >= field_len || vp_group < 0) {
        return BCM_E_PARAM;
    }
    if (field_len > sizeof(vp_group_bitmap) * 8) {
        return BCM_E_INTERNAL;
...
#if defined(BCM_TRIDENT3_SUPPORT)
        if (egress && soc_feature(unit,soc_feature_flex_flow)) {
            if (soc_mem_field_valid(unit, port_mem, DATA_TYPEf)) {
                int data_type = 0;
                int rv = 0;
                uint32 view_id = 0;
                data_type = soc_mem_field32_get(unit, port_mem,
                        &entry, DATA_TYPEf);
                ...
                if (rv == BCM_E_NONE) {
                    /* View is flex view */
                    if (SOC_MEM_FIELD_VALID(unit, view_id, L2_MEMBERSHIP_CONTROLS_ACTION_SETf)) {
                        filter_field = L2_MEMBERSHIP_CONTROLS_ACTION_SETf;
                        port_mem = view_id;
                        *filter = soc_mem_field32_get(unit, port_mem, &entry, filter_field);
                        *vp_group  = (*filter) & 0x3f;

                        /*EN_EFILTER in L2_MEMBERSHIP_CONTROLS_ACTION_SETf is from bit position 8:7 */
                        *filter >>= 7;
                    } else {
...
bcm_td2p_vp_vlan_member_set(int unit, bcm_gport_t gport, uint32 flags)
{
    ...
    /* efilter may not be available for td3 flexflow */
    rv = bcm_td2p_vp_group_port_filter_get(
