/*
 * $Id: dnxf/fabric.c,v 1.23 Broadcom SDK $
 *
    ...
    soc_dnxc_fabric_pipe_t arg)
{
    ...
    /*
     * validation
     * if enable then all  bits must be on
     */
    ...
    int unit)
{
    ...
    {
        /**
         * Skip this if device support only cell format with PCP field included
         */
    ...
    if (!SOC_WARM_BOOT(unit))
    {
        if (dnxf_data_fabric.multicast.priority_map_enable_get(unit) == 1)      /* set all valid multicast ID's to low
                                                                                 * priority */
        {
...
/**
 * \brief - set fabric control attributes.
 *
 * \param [in] unit - Unit number
 * \param [in] type - Fabric control parameter
 * \param [in] arg - Argument whose meaning is dependent on type
 * \return:
 *      _SHR_E_xxx
 * \remark:
 */
    ...
    bcm_port_t * links_array)
{
    ...
    {
        /** Skip invalid ports (virtual links support) */
        ...
        else
        {
            ...
            else
            {
                if (BCM_FABRIC_MODID_IS_LOCAL(destid_array[i]))
                {
                    ...
                    {
                        /*
                         * for direct mode use the actual modid because the local modid to actual modid mapping may not
                         * be 1:1, i.e. it may be scrambled. BUT the multicast table must be configured 1:1, it needs
                         * to correlate with the actual FAP modid
                         */
...
/*
 * \brief
 *   Common function to get attributes of multiple groups.
 ...
 *      unit       - (IN) Unit number.
 *      flags      - (IN) Flags.
 *      ngroups    - (IN) Number of MC-IDs.
 *      groups     - (IN) MC-IDs.
 *      dest_array - (IN) mod-id participating in the MC group.
 * Returns:
 *      _SHR_E_xxx
 * Notes:
 */
...
/*
 * \brief
 *   Common function to set attributes of multiple groups.
    ...
    int *enable)
{
    ...
    /*
     * Parameters are required only for set stage
     */
    if (!flags || (flags & _SHR_FABRIC_MULTICAST_SET_ONLY))
    {
        SHR_NULL_CHECK(groups, _SHR_E_PARAM, "groups");
    ...
    bcm_fabric_link_control_t type)
{
    ...
    /*
     * link can be -1 (but not smaller) for
     * bcmFabricLinkCableSwapMasterMode and
     * bcmFabricLinkCableSwapEnable
     */
...
/**
 * \brief -
 *      Set fabric-link attribute (fifo type) per link. Each link might have two fifo types; one per RX and one per TX
 * \param [in] unit
 *      Unit number
    ...
    int arg)
{
    ...
    switch (type)
    {
        case bcmFabricLinkCellFormat:
            SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_fabric_link_cell_format_set,
                                                  (unit, link, arg, 0 /** not init */ )));
            break;
    ...
    int set_get)
{
    ...
    /*
     * Make sure that dnxf_data won't be accessed with invalid key
     */
 ...
 *      _bcm_dnxf_tx
 * Purpose:
 *      Send destination routed cell from the FE with a valid packet inside carrying the payload
 * Parameters:
 *      unit            - (IN)  Unit number.
 *      tx_pkt          - (IN/OUT) packet to be encapsulated by a dest routed cell
 *      cookie        - (IN) cookie
 * Returns:
 *      _SHR_E_XXX
 */
    ...
    void *cookie)
{
    ...
    /**
     * Get FE modid as this will be source modid field of the cell
     */
    SHR_IF_ERR_EXIT(bcm_dnxf_stk_modid_get(unit, (int *) &(cell.header.source_modid)));
    cell.header.dest_modid = tx_pkt->dest_mod;

    /**
     * Currently implementation supports 1 cell of 256B or 512B.
     * However this includes the 13B system headers and the 2 bytes of CRC
     * so max payload data size is (max cell size) - 15.
     * We assume we always send 1 cell in maximum supported format
     *
     */
    ...
    /**
     * Calculate CRC and append to the payload
     */
    ...
    /**
     * Set cell data size to be system_headers + data + crc
     */
    ...
    bcm_fabric_system_upgrade_target_t target)
{
    ...
    /** Check if the system upgrade is in process */
