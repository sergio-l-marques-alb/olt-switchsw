...
#if defined(BCM_EDK_SUPPORT) && defined(EDKHLIB)
...
_bcm_th3_threads_shutdown(int unit)
{
    ...
    if (soc_l2_mac_move_mon_thread_running(unit)) {
        rv = soc_l2_mac_move_mon_thread_stop(unit);
    }

    _THREAD_STOP_CHECK_ERR(rv, "macmovemon");
...
_bcm_th3_modules_init(int unit)
{
    ...
    if (SOC_FAST_REBOOT(unit)) {
        ...
        /* Fetch the port driver functions */
        BCM_IF_ERROR_RETURN(bcm_esw_port_fast_reboot_port_drv_init(unit));
    ...
    if (SOC_WARM_BOOT(unit)) {
        ...
        /* Warmboot EDK-Host */
        if (edk_main(2, argv_list) != BCM_E_NONE) {
            cli_out("edk_init:: dnx_edk_state_init ERROR in edk_main()\n");
        }
    }
#endif  /* BCM_WARM_BOOT_SUPPORT */
#endif
...
#if defined(BCM_TOMAHAWK_SUPPORT)
    if (SOC_WARM_BOOT(unit) && soc_feature(unit, soc_feature_advanced_flex_counter)) {
        _BCM_MOD_INIT_CER(unit, _bcm_esw_stat_flex_done, SHR_BPROF_BCM_STAT_DONE, TRUE);
    }
#endif
