...
static uint32 _cmicm_debug_flags[BCM_MAX_NUM_UNITS] = {0};
/* Global variables */
    ...
    int message_len)
{
    ...
    if (status != _BCM_MBOX_MS_EMPTY) {
        LOG_ERROR(BSL_LS_BCM_COMMON,
                  (BSL_META_U(unit,
                              "TOP message buffer in not empty, re-attempt\n")));
    }
...
#endif /* BCM_CMICM_SUPPORT */
int
_bcm_mbox_debug_flag_set(int unit, uint32 flags)
{
    _cmicm_debug_flags[unit] = flags;
...
int
_bcm_mbox_debug_flag_get(int unit, uint32 *flags)
{
    *flags = _cmicm_debug_flags[unit];
