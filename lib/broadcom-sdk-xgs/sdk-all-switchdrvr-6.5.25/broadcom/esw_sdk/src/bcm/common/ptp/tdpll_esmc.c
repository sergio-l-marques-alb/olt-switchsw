    ...
    void **unused)
{
    ...
    if (objdata.tx_enable[*dpll_index]
        ...
        if (PTP_CLOCK_IS_G8275P2_PROF(clock_info) &&
            (!g_apts_enabled ||
             ((g_apts_current_op_mode != bcmPtpClockAptsModeSyncEPtp) &&
              (g_apts_current_op_mode != bcmPtpClockAptsModeSyncEGps) &&
              (g_apts_current_op_mode != bcmPtpClockAptsModeSyncEHoldover)))) {
            /* Override only when APTS is disabled Or APTS freq mode is not SyncE. */
