    ...
    bcm_tdpll_input_clock_l1mux_t *l1mux)
{
    ...
    /*  findout logical port number from lane-swaped port number rcvd from fw */
    if (SOC_IS_FIRELIGHT(unit)) {
        ...
        port_sel = l1mux->port;
        phy_port = bcmi_esw_time_fl_phy_port_get (unit, port_sel);
        if (phy_port > 0 ) {
    ...
    bcm_tdpll_input_clock_l1mux_t *l1mux)
{
    ...
    if (SOC_IS_FIRELIGHT(unit)) {
        ...
        port_sel = bcmi_esw_time_fl_synce_L1_rcvd_clk_port_get(unit, phy_port);
        if (port_sel <  0 ) {
            PTP_ERROR_FUNC("bcmi_esw_time_fl_synce_L1_rcvd_clk_port_get()");
            return BCM_E_PARAM;
        }
        l1mux->port = port_sel;
    }
