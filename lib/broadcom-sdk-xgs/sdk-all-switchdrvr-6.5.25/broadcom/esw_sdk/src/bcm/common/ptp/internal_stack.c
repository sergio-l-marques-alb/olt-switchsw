...
{
    int rv = BCM_E_NONE;
    bcm_rx_chan_t chan_id = 0;
    int num_cosq = 0;
    int min_cosq, max_cosq, ptp_cosq = 0;
    int i;
    int rx_channel = 1;  /* For now, PTP is always on uC by itself, so always use Rx channel 1 */
    int cmc;

    COMPILER_REFERENCE(num_cosq); COMPILER_REFERENCE(max_cosq);
    COMPILER_REFERENCE(ptp_cosq); COMPILER_REFERENCE(i);
    COMPILER_REFERENCE(rx_channel); COMPILER_REFERENCE(chan_id);
    COMPILER_REFERENCE(min_cosq);

#if defined(BCM_CMICX_SUPPORT)

#define BCM_PTP_CMICX_PKTRX_CORE1_DEF_CHANNEL_NO    (13)

#if defined(BCM_FIRELIGHT_SUPPORT)
    if (SOC_IS_FIRELIGHT(unit)){

        /* Firelight default cosq configuration -
         * num_queues_pci=6; num_queues_uc0=2; num_queues_uc1=0
         * ptp_cosq = 7
         */

        /* Get PTP COSq soc property */
        ptp_cosq = soc_property_get(unit, spn_PTP_COSQ, 7);

        /* PKT RX - PKTDMA1 CH5 */
        rv = _bcm_common_rx_queue_channel_set(unit, ptp_cosq, BCM_PTP_CMICX_PKTRX_CORE1_DEF_CHANNEL_NO);
        if (!BCM_SUCCESS(rv)) {
...
#endif /* defined(BCM_CMICX_SUPPORT) */

    /* Non - CMICX devices - !defined(BCM_CMICX_SUPPORT) */
    /*
     * PTP COS Queue
     *
    ...
    LOG_VERBOSE(BSL_LS_BCM_PTP, (BSL_META_U(unit,
                    "\t ptp_cosq:%d min_cosq:%d max_cosq:%d cmc:%d uc_num:%d\n"),
                    ptp_cosq, min_cosq, max_cosq, cmc, uc_num));

#ifdef BCM_CMICX_SUPPORT
    if (soc_feature(unit, soc_feature_cmicx)) {
        chan_id = (BCM_CMICX_RX_CHANNELS * (SOC_ARM_CMC(unit, uc_num))) +
                  (uc_num * 4) + rx_channel;
        LOG_VERBOSE(BSL_LS_BCM_PTP, (BSL_META_U(unit,
                "cmc:%d chan_id:%d\n"), (SOC_ARM_CMC(unit, uc_num)), chan_id));
    } else
#endif
    {
        /*
