...
_bcm_ptp_construct_signaling_msg(
    ...
    int *udp_payload_len)
{
    sal_memset(udp_payload, 0, 34);  /* blank PTP header */
    ...
    while(queue->head != -1 && (tsList + queue->head)->data != NULL)
    {
        ...
        switch ((_bcm_ptp_ext_servo_event_t)event_type) {
...
#if 0
            {
                _bcm_ptp_timestamp_node_t *node =NULL;
                int idx = 0;
            ...
            case _bcm_ptp_ext_servo_master_change_event:
                ...
                if (g_apts_enabled)
                {
                    ...
                    if (_bcm_ptp_peer_portid_compare(&best_master_portId, &clk_portid)) {
                        ...
                        _bcm_ptp_apts_update_current_mode(unit, ptp_id);
            ...
            case _bcm_ptp_ext_servo_master_unavail_event:
                ...
                sal_memset(master_portaddr.address, 0, BCM_PTP_MAX_NETW_ADDR_SIZE);
                sal_memcpy(master_portaddr.address, ptr+28, BCM_PTP_MAX_NETW_ADDR_SIZE);
                ...
                if (event_type == _bcm_ptp_ext_servo_master_avail_event) {
                    ...
                    if (BCM_SUCCESS(rv = _bcm_ptp_ext_servo_master_table_entry_get(&master_portId, NULL))) {
            ...
            case _bcm_ptp_ext_servo_gps_unavail_event:
                ...
                if ((g_apts_enabled) && (!g_apts_servo_assist_gps)) {
            ...
            case _bcm_ptp_ext_servo_gps_ts_event:
                ...
                if (source_avail & _bcm_ptp_apts_avail_gps)
                {
                    ...
                    first_TS.nanoseconds = (temp_ts%1000000000);
                    ...
                    for (i = 0; i < num_TS; i++)
                    {
                        gps_diff_ts[i] = _bcm_ptp_int64_read(ptr) - (((uint64)(temp_ts + servo_phase_offset + g_apts_gps_offset)) % ((uint64)1 << 48));
