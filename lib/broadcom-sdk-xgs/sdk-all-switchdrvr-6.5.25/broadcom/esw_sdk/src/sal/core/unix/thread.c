...
sal_thread_create(char *name, int ss, int prio, void (f)(void *), void *arg)
{
    ...
    {   //PTIn modified
        ...
        //else
        //if (0==strcmp("Desc DMA Timeout",name))     {pri=83; tslice=1;}
        //else
        //if (0==strncmp("socdmadesc.", name, strlen("socdmadesc.")))
        //                                            {pri=83; tslice=1;}
        //else
        ...
        //else
        {pri=L7_DEFAULT_TASK_PRIORITY; tslice=L7_DEFAULT_TASK_SLICE;}

        salTaskHandle = osapiTaskCreate(name, f, arg, 1,
                                        (L7_uint32) ss,
                                        pri,
                                        tslice);
    }
...
/* print information about the process with the give process ID */
void print_process_info(int pid)
{
    FILE* f = NULL;
    char filename[32], *line;
    size_t line_size = 0;
    /* set the name of the process file containing the command line */
    int ret_value = snprintf(filename, sizeof(filename), "/proc/%d/cmdline", pid);

    /* open the process file containing the process command line, and print its content */
    if (ret_value > 0 && ret_value < sizeof(filename) &&
        (f = fopen(filename, "rb")) != NULL) {
        printf("process %d:", pid);
        for (;;) {
            line = NULL;
            if (getdelim(&line, &line_size, 0, f) >= 0 && line != NULL) {
                printf(" %s", line);
                free(line);
            } else {
                break;
