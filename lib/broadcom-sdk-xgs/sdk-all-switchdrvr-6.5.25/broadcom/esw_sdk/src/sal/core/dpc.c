...
/*
 * Function:
 * 	sal_dpc_owner_is_active
 * Purpose:
 *	Get status of 'sal_dpc_owner'.
 *      Indicate whether 'dpc owner' is active under thread "bcmDPC".
 *      Used for suppressing the 'get' operation under tsan.
 * Parameters:
 *	void *xx - Identifier of DPC owner
 * Returns:
 *	int - If 'non-zero' then specified DPC owner is active.
 */
STATIC int sal_dpc_owner_is_active(void *xx)
{
    int ret;
    ret = (sal_dpc_owner_stat[PTR_TO_INT(xx)] & SAL_DPC_STATUS_ACTIVE);
    return (ret);
}
/*
 * Function:
 * 	sal_dpc_q_get
 * Purpose:
 *	Get value of the static/global 'sal_dpc_q'. Used
 *      for suppressing the 'get' operation under tsan.
 *      Applied only on code which is NOT protected by 'spinlock'.
 * Parameters:
 *	None
 * Returns:
 *	sal_dpc_t * - Current value of sal_dpc_q.
 */
STATIC sal_dpc_t *sal_dpc_q_get(void)
{
    sal_dpc_t *ret;
    ret = sal_dpc_q;
    return (ret);
 ...
 * 	sal_dpc_q_set
 * Purpose:
 *	Set value of the static/global 'sal_dpc_q'. Used
 *      for potentially controlling the 'set' operation under tsan.
 *      Applied only on code which is NOT protected by 'spinlock'.
 * Parameters:
 *	sal_dpc_t *value -
 *        Value to load into sal_dpc_q.
 * Returns:
 *	void
 */
STATIC void sal_dpc_q_set(sal_dpc_t *value)
{
    sal_dpc_q = value;
    return;
}
 ...
 * 	sal_dpc_q_sd_t_get
 * Purpose:
 *	Get value of the static/global 'sal_dpc_q->sd_t'. Used
 *      for suppressing the 'get' operation under tsan.
 *      Applied only on code which is NOT protected by 'spinlock'.
 * Parameters:
 *	None
 * Returns:
 *	unit32 sal_dpc_q->sd_t - Current value of sal_dpc_q->sd_t.
 */
STATIC uint32 sal_dpc_q_sd_t_get(void)
{
    uint32 ret;
    ret = sal_dpc_q->sd_t;
    return (ret);
}
#define SAL_DPC_OWNER_IS_ACTIVE(xx) \
    sal_dpc_owner_is_active(xx)
#define SAL_DPC_Q_GET() \
    sal_dpc_q_get()
#define SAL_DPC_Q_SET(value) \
    sal_dpc_q_set(value)
#define SAL_DPC_Q_SD_T_GET() \
    sal_dpc_q_sd_t_get()
/* } */
#else
/* { */
#define SAL_DPC_OWNER_IS_ACTIVE(xx) \
    (sal_dpc_owner_stat[PTR_TO_INT(xx)] & SAL_DPC_STATUS_ACTIVE)
#define SAL_DPC_Q_GET() \
    sal_dpc_q
#define SAL_DPC_Q_SET(value) \
    sal_dpc_q = value
...
sal_dpc_thread(void *arg)
{
	...
	if (SAL_DPC_Q_GET() == NULL) {
	    (void)sal_sem_take(sal_dpc_sem, sal_sem_FOREVER);
	} else {
	    int t = SAL_USECS_SUB(SAL_DPC_Q_SD_T_GET(), sal_time_usecs());
	    if (t > 0) {
