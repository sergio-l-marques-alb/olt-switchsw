...
char* autocomplete_print(int unit, char* input, char* prompt) {
   ...
   /* get the first token */
   token = strtok(str_cpy, separator);
              ...
              if (sal_strncasecmp(node->keyword, token, MIN(assignment - token + 1, sal_strlen(node->keyword))) == 0) {
                  ...
                  else if (node->ac_option_cb != 0)
                  {
                      ...
                      else if (num_of_suggestions > 1)
                      {
                          ...
                          ac_option = strtok(ac_full_string, separator);
                          /*
                           ...
                           */
                          common_prefix_length = strcaseindex(ac_option, ac_substring) - ac_option;
                          prefix = ac_substring;
                          ...
                          while (ac_option != NULL)
                          {
                              ...
                              ac_option = strtok(NULL, separator);
                          }
                          /*
   ...
   /* no suggestions */
   if (token_idx != node_depth && strtok(NULL, separator)) {
   ...
   if (num_of_suggestions > 1) {
       ...
       sal_printf("\n");
       token = strtok(str_to_print, separator);
       while( token != NULL )
       {
