...
int vlan_membership_filter_after_ive = 0;
...
int l2_basic_bridge_run(int unit,  l2_basic_bridge_s *param,int step){
    ...
    /* For ESEM updating VSI value to be lower value*/
    if (step == 3) {
    ...
    if( step == 3) {
        ...
        vlan_port.vsi = 0;
        if (vlan_membership_filter_after_ive) {
    ...
    /* IVL context outer_vid is part of modid of l2_addr */
    if (step == 5) {
        l2_addr.modid = (is_jericho2) ? new_outer_vid : g_l2_basic_bridge.outer_vid;
    }
...
/*
 * Create an egress Esem objet and set the action_profile to DROP
 */
int l2_basic_vlan_membership_filter_after_ive_set(
    int unit,
    int port,
    int vsi,
    int vlan)
{
    ...
    /* set tag format for tags after IVE*/
    bcm_port_tpid_class_t port_tpid_class;
    bcm_port_tpid_class_t_init(&port_tpid_class);
    port_tpid_class.tpid1 = 0x88a8;
    port_tpid_class.tpid2 = 0x8100;
    port_tpid_class.tag_format_class_id = g_l2_basic_bridge.egress_tag_format[1].tag_format;
    port_tpid_class.flags = BCM_PORT_TPID_CLASS_EGRESS_ONLY | BCM_PORT_TPID_CLASS_OUTER_C;
    port_tpid_class.port =  g_l2_basic_bridge.out_port;
    rv = bcm_port_tpid_class_set(unit, &port_tpid_class);
    if (rv != BCM_E_NONE)
    {
        printf("Error, bcm_port_tpid_class_set in l2_basic_vlan_membership_filter_after_ive_set\n");
        return rv;
    }

    /* Create an ESEM entry with port + vsi + ctag for filter */
    bcm_vlan_port_t vlan_port;
    bcm_vlan_port_t_init(&vlan_port);
    vlan_port.flags = BCM_VLAN_PORT_CREATE_EGRESS_ONLY | BCM_VLAN_PORT_VLAN_TRANSLATION;
    vlan_port.criteria = BCM_VLAN_PORT_MATCH_PORT_VLAN;
    ...
    rv = bcm_vlan_port_create(unit, &vlan_port);
    if (rv != BCM_E_NONE) {
        printf("Error, bcm_vlan_port_create in l2_basic_vlan_membership_filter_after_ive_set!\n");
        return rv;
    }

    /* Set the action to drop for the ESEM entry.*/
    ...
    lif_config.lif_gport = vlan_port.vlan_port_id;
    lif_config.action_gport = action_gport;
    rv = bcm_rx_trap_lif_set(unit, 0, &lif_config);
    if (rv != BCM_E_NONE) {
        printf("Error, bcm_rx_trap_lif_set in l2_basic_vlan_membership_filter_after_ive_set!\n");
        return rv;
    }

    printf("Successfully set action_profile with trap gport 0x%08X for vlan_port 0x%08X, port = %d, vsi = %d, vlan = %d!\n",
           action_gport, vlan_port.vlan_port_id, port, vsi, vlan);
