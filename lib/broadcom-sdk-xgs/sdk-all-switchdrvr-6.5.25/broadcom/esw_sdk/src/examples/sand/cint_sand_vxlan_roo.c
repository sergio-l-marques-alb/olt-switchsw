    ...
    int unit)
{
    ...
    {
        routed_learning = *(dnxc_data_get(unit, "l3", "feature", "routed_learning", NULL));
        if (routed_learning)
        {
 ...
 */
int
qos_map_egress_remark_set(int unit, int dscp_map)
{
    int rv;    
    int flags = 0;
 ...
 */
int
qos_map_egress_remark_clear(int unit, int dscp_map)
{
    int rv;
    bcm_qos_map_t l3_eg_map;
    int dscp, tc, dp;
...
int vxlan_roo_eve_action(int unit, int out_port, int out_vlan, int vsi, int is_native)
{
    ...
    printf("vxlan_roo_eve_action: unit=%d out_port=%d out_vlan=%d, vsi=%d is_native=%d \n", unit, out_port, out_vlan, vsi ,is_native);
    ...
    if (is_device_or_above(unit, JERICHO2)) {
        ...
        if (is_native) {
            if (cint_vxlan_basic_info.egress_native_virtual_ac != 0) {
                /* the entry was created to configure orientation. See vxlan_split_horizon_set */
        ...
        printf("vxlan_roo_eve_action: vlan_port_id=0x%08X \n", vlan_port_id);
