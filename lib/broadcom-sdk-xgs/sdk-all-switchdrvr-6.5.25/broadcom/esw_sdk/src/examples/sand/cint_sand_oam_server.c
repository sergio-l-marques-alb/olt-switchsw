...
/* Globals */
  ...
  bcm_oam_endpoint_info_t ep_server_down, ep_server_down2, ep_server_up, ep_server_up2, ep_client_down, ep_client_down2, ep_client_up, ep_client_up2, rmep;
...
int oamp_server_set_server_endpoints(int server_unit, int client_unit, int server_port1, int server_port2, int client_port1, int client_port2, int is_server_client) {
    ...
    if(is_device_or_above(client_unit, JERICHO2))
    {
        ...
        {
            int client_device_type = *(dnxc_data_get(client_unit, "oam", "oamp", "dev_type_for_server_tests", NULL));
            int server_device_type = *(dnxc_data_get(server_unit, "oam", "oamp", "dev_type_for_server_tests", NULL));
            if(server_device_type != client_device_type)
            {
                /**
                 * When the devices are of different types (JR2/J2C, JR2/J2P,
                 * J2C/J2P) the recycle port has a different number
                 */
...
int oamp_server_set_multi_server_endpoints_down(int server_unit, int client_unit, int server_port1, int server_port2, int client_port1, int client_port2) {
    ...
    rv = bcm_oam_profile_id_get_by_type(client_unit, acc_ingress_profile, bcmOAMProfileIngressAcceleratedEndpoint, &flags, &acc_ingress_profile);
    if (rv != BCM_E_NONE) {
      printf("bcm_oam_profile_id_get_by_type(accelerated ingress) \n");
      return rv;
    }

    rv = bcm_oam_profile_id_get_by_type(client_unit, acc_egress_profile, bcmOAMProfileEgressAcceleratedEndpoint, &flags, &acc_egress_profile);
    if (rv != BCM_E_NONE) {
      printf("bcm_oam_profile_id_get_by_type(accelerated egress) \n");
      return rv;
    }
    ...
    bcm_oam_endpoint_info_t_init(ep_server_down);
    sal_memcpy(ep_server_down.src_mac_address, src_mac, 6);
    BCM_GPORT_TRAP_SET(ep_server_down.remote_gport, trap_code, 7, 0);
    rv = port_to_system_port(client_unit, client_port1, &ep_server_down.tx_gport);
    if (rv != BCM_E_NONE) {
        printf("(%s) \n", bcm_errmsg(rv));
        return rv;
    }
    ...
    bcm_oam_endpoint_info_t_init(ep_server_down2);
    sal_memcpy(ep_server_down2.src_mac_address, src_mac_1, 6);
    BCM_GPORT_TRAP_SET(ep_server_down2.remote_gport, trap_code, 7, 0); 
    rv = port_to_system_port(client_unit, client_port2, &ep_server_down2.tx_gport);
    if (rv != BCM_E_NONE) {
        printf("(%s) \n", bcm_errmsg(rv));
        return rv;
    }
    ep_server_down2.type = bcmOAMEndpointTypeEthernet;
    ep_server_down2.gport = BCM_GPORT_INVALID;
    ep_server_down2.acc_profile_id = BCM_OAM_PROFILE_INVALID;
    ep_server_down2.group = group_info_short_ma_server.id;
    ep_server_down2.level = down_mdl;
    ep_server_down2.name = 102;
    ep_server_down2.ccm_period = BCM_OAM_ENDPOINT_CCM_PERIOD_100MS;
    ep_server_down2.opcode_flags = BCM_OAM_OPCODE_CCM_IN_HW;
    ep_server_down2.vlan = 22;
    ep_server_down2.outer_tpid = 0x8100;
    rv = bcm_oam_endpoint_create(client_unit, &ep_server_down2);
    if (rv != BCM_E_NONE) {
        printf("(%s) \n", bcm_errmsg(rv));
        return rv;
    }
    printf("Down MEP created on server (unit %d) with id %d, tx_gport is: %d\n", client_unit, ep_server_down2.id, ep_server_down2.tx_gport);

    /** Set the RMEP entry associated with the above endpoint.*/
    bcm_oam_endpoint_info_t_init(rmep);
    rmep.name = 202;
    rmep.local_id = ep_server_down2.id;
    rmep.type = bcmOAMEndpointTypeEthernet;
    rmep.ccm_period = BCM_OAM_ENDPOINT_CCM_PERIOD_100MS;
    rmep.flags |= BCM_OAM_ENDPOINT_REMOTE | BCM_OAM_ENDPOINT_WITH_ID;
    rmep.loc_clear_threshold = 1;
    rmep.id = ep_server_down2.id;
    rv = bcm_oam_endpoint_create(client_unit, &rmep);
    if (rv != BCM_E_NONE) {
        printf("(%s) \n", bcm_errmsg(rv));
        return rv;
    }
    ...
    ep_client_down.local_id = server_unit;
    rv = bcm_oam_endpoint_create(client_unit, &ep_client_down);
    if (rv != BCM_E_NONE) {
        printf("(%s) \n", bcm_errmsg(rv));
        return rv;
    ...
    bcm_oam_endpoint_info_t_init(ep_client_down2);
    ep_client_down2.flags2 |= BCM_OAM_ENDPOINT2_HW_ACCELERATION_SET;
    ep_client_down2.acc_profile_id = acc_ingress_profile;
    ep_client_down2.gport = client_vp2.vlan_port_id;
    ep_client_down2.flags = BCM_OAM_ENDPOINT_WITH_ID;
    ep_client_down2.type = bcmOAMEndpointTypeEthernet;
    ep_client_down2.group = group_info_short_ma_client.id;
    ep_client_down2.level = down_mdl;
    ep_client_down2.lm_counter_base_id = 0;
    ep_client_down2.id = ep_server_down2.id;
    ep_client_down2.local_id = client_unit;
    rv = bcm_oam_endpoint_create(client_unit, &ep_client_down2);
    if (rv != BCM_E_NONE) {
        printf("(%s) \n", bcm_errmsg(rv));
        return rv;
    }
    printf("Server-client down-MEP created on server (unit %d) with id %d\n", client_unit, ep_client_down2.id);
...
int oamp_server_set_multi_server_endpoints_up(int server_unit, int client_unit, int server_port1, int server_port2, int client_port1, int client_port2) {
    ...
    bcm_oam_endpoint_info_t_init(ep_server_up);
    bcm_oam_endpoint_info_t_init(ep_server_up2);
    sal_memcpy(ep_server_up.src_mac_address, src_mac, 6);
    sal_memcpy(ep_server_up2.src_mac_address, src_mac_1, 6);
    if(is_device_or_above(client_unit, JERICHO2))
    {
        /**
         * remote_gport is used for reception: this trap
        ...
        ep_server_up2.remote_gport = ep_server_up.remote_gport;
        ep_server_up2.tx_gport = ep_server_up.tx_gport;
         ...
         * PP port must be provided from the client
         */
        rv = bcm_port_get(client_unit, client_port1, &flags, &interface_info, &mapping_info);
        if (rv != BCM_E_NONE) {
            printf("bcm_port_get fail \n");
            return rv;
        }
        ...
        rv = bcm_port_get(client_unit, client_port2, &flags, &interface_info, &mapping_info);
        if (rv != BCM_E_NONE) {
            printf("bcm_port_get fail \n");
            return rv;
        }
        ep_server_up2.gport = mapping_info.pp_port; 
    }
    ...
    ep_server_up.group = group_info_short_ma_server.id;
    ep_server_up.level = up_mdl;
    ep_server_up.name = 101;
    ep_server_up.ccm_period = BCM_OAM_ENDPOINT_CCM_PERIOD_100MS;
    ep_server_up.flags = BCM_OAM_ENDPOINT_UP_FACING;
    ep_server_up.opcode_flags = BCM_OAM_OPCODE_CCM_IN_HW;
    ep_server_up.vlan = 21;
    ep_server_up.outer_tpid = 0x8100;
    rv = bcm_oam_endpoint_create(server_unit, &ep_server_up);
    if (rv != BCM_E_NONE) {
        printf("(%s) \n", bcm_errmsg(rv));
    ...
    ep_server_up2.type = bcmOAMEndpointTypeEthernet;
    ep_server_up2.acc_profile_id = BCM_OAM_PROFILE_INVALID;
    ep_server_up2.group = group_info_short_ma_server.id;
    ep_server_up2.level = up_mdl;
    ep_server_up2.name = 102;
    ep_server_up2.ccm_period = BCM_OAM_ENDPOINT_CCM_PERIOD_100MS;
    ep_server_up2.flags = BCM_OAM_ENDPOINT_UP_FACING;
    ep_server_up2.opcode_flags = BCM_OAM_OPCODE_CCM_IN_HW;
    ep_server_up2.vlan = 22;
    ep_server_up2.outer_tpid = 0x8100;
    rv = bcm_oam_endpoint_create(client_unit, &ep_server_up2);
    if (rv != BCM_E_NONE) {
        printf("(%s) \n", bcm_errmsg(rv));
        return rv;
    }
    printf("Up MEP created on server (unit %d) with id %d, tx_gport is: %d, remote_gport is: %d\n", client_unit, ep_server_up2.id, ep_server_up2.tx_gport, ep_server_up2.remote_gport);

    /** Set the RMEP entry associated with the above endpoint.*/
    bcm_oam_endpoint_info_t_init(rmep);
    rmep.name = 202;
    rmep.local_id = ep_server_up2.id;
    rmep.type = bcmOAMEndpointTypeEthernet;
    rmep.ccm_period = BCM_OAM_ENDPOINT_CCM_PERIOD_100MS;
    rmep.flags |= BCM_OAM_ENDPOINT_REMOTE | BCM_OAM_ENDPOINT_WITH_ID;
    rmep.loc_clear_threshold = 1;
    rmep.id = ep_server_up2.id;
    rv = bcm_oam_endpoint_create(client_unit, &rmep);
    if (rv != BCM_E_NONE) {
        printf("(%s) \n", bcm_errmsg(rv));
        return rv;
    ...
    ep_client_up.flags2 |= BCM_OAM_ENDPOINT2_HW_ACCELERATION_SET;
    ep_client_up.acc_profile_id = acc_egress_profile;
    ep_client_up.gport = client_vp1.vlan_port_id;
    ep_client_up.flags = BCM_OAM_ENDPOINT_WITH_ID | BCM_OAM_ENDPOINT_UP_FACING;
    ep_client_up.type = bcmOAMEndpointTypeEthernet;
    ep_client_up.group = group_info_short_ma_client.id;
    ep_client_up.level = up_mdl;
    ...
    ep_client_up.local_id = server_unit;
    rv = bcm_oam_endpoint_create(client_unit, &ep_client_up);
    if (rv != BCM_E_NONE) {
        printf("(%s) \n", bcm_errmsg(rv));
        return rv;
    ...
    bcm_oam_endpoint_info_t_init(ep_client_up2);
    ep_client_up2.flags2 |= BCM_OAM_ENDPOINT2_HW_ACCELERATION_SET;
    ep_client_up2.acc_profile_id = acc_egress_profile;
    ep_client_up2.gport = client_vp2.vlan_port_id;
    ep_client_up2.flags = BCM_OAM_ENDPOINT_WITH_ID | BCM_OAM_ENDPOINT_UP_FACING;
    ep_client_up2.type = bcmOAMEndpointTypeEthernet;
    ep_client_up2.group = group_info_short_ma_client.id;
    ep_client_up2.level = up_mdl;
    ep_client_up2.lm_counter_base_id = counter_base_id_upmep;
    ep_client_up2.id = ep_server_up2.id;
    ep_client_up2.local_id = client_unit;
    rv = bcm_oam_endpoint_create(client_unit, &ep_client_up2);
    if (rv != BCM_E_NONE) {
        printf("(%s) \n", bcm_errmsg(rv));
        return rv;
    }
    printf("Server-client up-MEP created on server (unit %d) with id %d\n", client_unit, ep_client_up2.id);
...
int oamp_server_create_traps(int server_unit, int client_unit) {
    ...
    if(is_device_or_above(client_unit, JERICHO2))
    {
        ...
        rv = bcm_rx_trap_type_get(client_unit,0,bcmRxTrapOamEthAccelerated,&stamped_trap_code);
        BCM_IF_ERROR_RETURN(rv);
        bcm_rx_trap_config_t_init(&trap_to_server_oamp);
        BCM_GPORT_TRAP_SET(trap_to_server_oamp.cpu_trap_gport, trap_code_up_ing, 15, 0);
        trap_to_server_oamp.stamped_trap_code = stamped_trap_code;
    }
...
int oamp_multi_servers_clients_example(int server_unit, int client_unit, int server_port1, int server_port2, int client_port1, int client_port2, int is_up) {
    ...
    if (is_up) {
         ...
         * by up MEP.
         */
        client_vp3.criteria = BCM_VLAN_PORT_MATCH_PORT_VLAN;
        bcm_vlan_port_t_init(&client_vp3);
        client_vp3.criteria = BCM_VLAN_PORT_MATCH_PORT_CVLAN;
        client_vp3.flags = 0;
        client_vp3.port = client_port2;
        client_vp3.match_vlan = 100;
        /*client_vp3.egress_vlan = is_device_or_above(client_unit, JERICHO2) ? 0 : 100;*/
        rv = bcm_vlan_port_create(client_unit,&client_vp3);
        if (rv != BCM_E_NONE) {
            printf("(%s) \n",bcm_errmsg(rv));
            return rv;
        }
        printf("Vlan-port created on client (unit %d) with id %d\n", client_unit, client_vp3.vlan_port_id);

        /** Config coross connection between client_vp1 and client_vp3 */
        bcm_vswitch_cross_connect_t_init(&client_cross_connect);
        client_cross_connect.port1 = client_vp1.vlan_port_id;
        client_cross_connect.port2 = client_vp3.vlan_port_id;
        rv = bcm_vswitch_cross_connect_add(client_unit, &client_cross_connect);
        if (rv != BCM_E_NONE) {
            printf("(%s) \n",bcm_errmsg(rv));
            return rv;
        ...
        /** Config coross connection between client_vp1 and client_vp3 */
        bcm_vswitch_cross_connect_t_init(&client_cross_connect);
        client_cross_connect.port1 = client_vp2.vlan_port_id;
        client_cross_connect.port2 = client_vp3.vlan_port_id;
        rv = bcm_vswitch_cross_connect_add(client_unit, &client_cross_connect);
        if (rv != BCM_E_NONE) {
            printf("(%s) \n",bcm_errmsg(rv));
        ...
        rv = bcm_vlan_gport_add(client_unit, 100, client_port2, 0);
        if (rv != BCM_E_NONE)
        {
            printf("(%s) - bcm_vlan_gport_add \n", bcm_errmsg(rv));
            return rv;
        ...
        rv = oamp_server_set_multi_server_endpoints_up(server_unit, client_unit, server_port1, server_port2, client_port1, client_port2);
        if (rv != BCM_E_NONE) {
            printf("(%s) \n",bcm_errmsg(rv));
            return rv;
        }
    ...
    else 
    {
        ...
        rv = oamp_server_set_multi_server_endpoints_down(server_unit, client_unit, server_port1, server_port2, client_port1, client_port2);
        if (rv != BCM_E_NONE) {
            printf("(%s) \n",bcm_errmsg(rv));
            return rv;
        }
