...
vswitch_vpls_hvpls_ingress_mpls_port_create(int unit){
    ...
    for (is_primary=0; is_primary<=1; is_primary++) {
        ...
        if (is_device_or_above(unit, JERICHO2) && !is_jericho2_mode && !j1_learn) {
            /*
             * IOP mode, JR2 MACT creates refresh events with TRANSPLANT command when learned destination is FEC.
             * Because JR1/ARAD destination encoding supports only 15 bits, the FEC used for learning shall be 15 bits wide. 
             */
            mpls_port_ingress.failover_port_id = vswitch_vpls_hvpls_info.pwe_fec & 0x7FFF; /* Learning - PWE FEC */
        } else {
...
vswitch_vpls_hvpls_fec_mpls_port_create(int unit){
    ...
    if (is_device_or_above(unit, JERICHO2)) {
        ...
        if ((soc_property_get(unit, "system_headers_mode", 1) == 0) && (mpls_port_eei == 1))
        {
...
vswitch_vpls_hvpls_run(int unit){
    ...
    if (is_device_or_above(unit, JERICHO2) && (soc_property_get(unit, "system_headers_mode", 1) == 0)
        && (mpls_port_eei == 0)) 
    {
        create_l3_egress_params.l3_flags |= BCM_L3_ENCAP_SPACE_OPTIMIZED;
    }
...
int vswitch_vpls_mc_in_fec_run_with_defaults_dvapi(int unit,int acP, int pweP)
{
    ...
    if (is_device_or_above(unit, JERICHO2)) {
        ...
        if (!is_jericho2_mode) {
            rv = fec_id_alignment_field_add(unit, acP, BCM_L3_ITF_VAL_GET(vswitch_vpls_hvpls_info.pwe_fec));
            if (rv != BCM_E_NONE){
                 printf("Error, fec_id_alignment_field_add\n");
                 return rv;
            }
        }
    }
