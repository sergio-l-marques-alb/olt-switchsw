    ...
    int pwe_port_2)
{
    ...
    pwe_enc->mpls_port_id = soc_property_get(0, "max_rif_id", 8192) + 0x4000;
    ...
    pwe_enc->label = 3457;
    pwe_enc->label_flags |= BCM_MPLS_EGRESS_LABEL_TTL_DECREMENT;
    pwe_enc->mpls_port_id = soc_property_get(0, "max_rif_id", 8192) + 0x4001;
    /** In JR1, encap_id is needed in case of EGRESS_ONLY*/
    if (!is_device_or_above(unit, JERICHO2)) {
        pwe_enc->encap_id = pwe_enc->mpls_port_id;
        pwe_enc->flags |= BCM_MPLS_PORT_ENCAP_WITH_ID;
    ...
    int pwe_port_2)
{
    ...
    if (is_device_or_above(unit, JERICHO2))
    {
        ...
        if (!is_jericho2_mode) {
            rv = fec_id_alignment_field_add(unit, ac_port_1, BCM_L3_ITF_VAL_GET(vswitch_vpls_params.pwe_params[0].pwe_term.egress_tunnel_if));
            if (rv != BCM_E_NONE)
            {
    ...
    bcm_vpn_t vpn_id)
{
    ...
    rv = bcm_mpls_vpn_id_get(unit, vpn_id, &info);
    if (rv == BCM_E_NONE)
    {
        /*
        ...
        if (rv != BCM_E_NONE)
        {
            printf("Error(%d) in bcm_mpls_vpn_id_destroy  vpn_id=%d\n", rv, vpn_id);
            return rv;
        }
    } else if (rv != BCM_E_NOT_FOUND)
    {
        printf("Error(%d) in bcm_mpls_vpn_id_get  vpn_id=%d\n", rv, vpn_id);
        return rv;
    }
