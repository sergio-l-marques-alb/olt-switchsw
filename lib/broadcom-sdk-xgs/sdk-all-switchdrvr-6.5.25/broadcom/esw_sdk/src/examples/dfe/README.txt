 $Id: README.txt,v 1.55 Broadcom SDK $
 $Copyright: (c) 2021 Broadcom.
 Broadcom Proprietary and Confidential. All rights reserved.$                                                        
	 
 ~ Name:	 
+-------------------------+                                                        
|    Snake Application    |                                                         
+-------------------------+                                                       
 ~ Description:
	The test is a stress test on the BCM88950 (FE3200) links. The test validates the links performance under a burst of cells.                                                                                                   
                                                                              
 ~ File Reference:                                                              
    cint_snake_test.c                                                              
	
 ~ Name:   
+-----------------+
|    Warm Boot    |
+-----------------+
 ~ Description:
	Test for BCM88950 (FE3200) warm reboot.

 ~ File Reference: 
	cint_warmboot.c
 
 
 ~ Name:   
+--------------------+
|    Flow Control    |
+--------------------+
 ~ Description:
	This example demonstrates and tests the FIFO threshold sequence.

 ~ File Reference:
	cint_fifos.c

	  
 ~ Name:   
+---------------------------+
|    Flow Control FE3200    |
+---------------------------+
 ~ Description:
	This example demonstrates and tests the FIFO threshold pipe_set/get API for BCM88770 (FE3600, previously known as FE3200).

 ~ File Reference:
	cint_flow_control_fe3200.c
    
	  
 ~ Name:   	  
+----------------------+
|    FE3200 Interop    |
+----------------------+
 ~ Description:
	This example demonstrate the configuration required when connecting BCM88770 (FE3600, previously known as FE3200) to legacy devices.

 ~ File Reference:
	cint_fe3200_interop.c
