/*
 * $Id: cint_mpls_extended_term_flow_field.c, Exp $
 $Copyright: (c) 2021 Broadcom.
 Broadcom Proprietary and Confidential. All rights reserved.$ File: cint_mpls_5_label_term_field.c  
 */
...
bcm_field_group_t mpls_extended_term_fg_id = 0;
       ...
       bcm_if_t rcy_extended_term_encap_id)
 {
    ...
    if (mpls_extended_term_fg_id == 0)
    {
        ...
        rv = bcm_field_group_add(unit, 0, &fg_info, &mpls_extended_term_fg_id);
        if (rv != BCM_E_NONE)
        {
        ...
        rv = bcm_field_group_context_attach(unit, 0, mpls_extended_term_fg_id, context_id, &attach_info);
        if (rv != BCM_E_NONE)
        {
    ...
    rv = bcm_field_entry_add(unit, 0, mpls_extended_term_fg_id, &entry_info, &entry_handle);
    if (rv != BCM_E_NONE)
    {
