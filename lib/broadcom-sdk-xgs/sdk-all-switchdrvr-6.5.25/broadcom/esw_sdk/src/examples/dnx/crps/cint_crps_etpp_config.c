    ...
    int database_enable)
{
    ...
    rv = crps_sal_to_format_type_convert(&(cnt_interface.format_type));
    if(rv != BCM_E_NONE)
    {
        printf("Error, incorrect counter_format_sel variable\n");
        return rv;
    }
    ...
    crps_etpp_config_database_t create_db)
{
    ...
    rv = crps_sal_to_format_type_convert(&format_type);
    if(rv != BCM_E_NONE)
    {
        printf("Error, incorrect counter_format_sel variable\n");
        return rv;
...
crps_etpp_ace_stat_meter_delete(int unit, int pmf_group_id, int pp_field_entry_id, int pp_profile_id){
    ...
    rv = bcm_field_group_context_detach(unit,pmf_group_id,cint_attach_context_id_eth);
    if(rv != BCM_E_NONE)
    {
