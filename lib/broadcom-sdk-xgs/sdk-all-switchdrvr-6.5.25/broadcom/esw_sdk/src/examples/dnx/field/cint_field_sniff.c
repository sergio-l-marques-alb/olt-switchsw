...
int cint_field_sniff_ingress_main(int unit, bcm_field_context_t ipmf1_context_id)
{
    ...
    rv = bcm_field_group_add(unit, 0, &cinf_field_sniff_ingress_fg_info, &cint_field_sniff_ingress_fg_id);
    if (rv != BCM_E_NONE)
    {
...
int cint_field_sniff_egress_main(int unit, bcm_field_context_t epmf_context_id)
{
    ...
    rv = bcm_field_group_add(unit, 0, &cinf_field_sniff_egress_fg_info, &cint_field_sniff_egress_fg_id);
    if (rv != BCM_E_NONE)
    {
        printf("Error (%d), in bcm_field_group_add\n", rv);
        return rv;
