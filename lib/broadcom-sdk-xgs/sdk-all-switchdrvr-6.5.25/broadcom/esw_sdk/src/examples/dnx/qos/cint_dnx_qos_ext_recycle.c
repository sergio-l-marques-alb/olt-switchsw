/* $Id: cint_dnx_qos_ext_recycle.c,v 1.0 2021/01/20 10:28:20 Frank  $
...
*      BCM> cint src/examples/dnx/cint_dnx_qos_ext_recycle.c
*      BCM> cint
*      cint> dnx_qos_ext_recycle_service(unit,in_Port,out_Port,is_pipe,mode);
*/
...
int dnx_qos_ext_recycle_main(int unit, int in_port, int out_port, int rcy_port, int qos_model)
{
    ...
    if (rv != BCM_E_NONE) {
        printf("error in ARP bcm_l3_egress_create()\n");
        printf("(%s)\n", bcm_errmsg(rv));
        return rv;
    }
    ...
    if (rv != BCM_E_NONE) {
        printf("error in second pass tunnel bcm_mpls_tunnel_initiator_create()\n");
        printf("(%s)\n", bcm_errmsg(rv));
        return rv;
    }
    ...
    l3eg.port = out_port;
    l3eg.flags = 0;
    rv =  bcm_l3_egress_create(unit, BCM_L3_INGRESS_ONLY, &l3eg, &l3egid);
    if (rv != BCM_E_NONE) {
        printf("error in fec bcm_l3_egress_create()\n");
        printf("(%s)\n", bcm_errmsg(rv));
        return rv;
    }
    fec_id2 = l3egid;
    ...
    if (rv != BCM_E_NONE) {
        printf("error in recycle bcm_l2_egress_create()\n");
        printf("(%s)\n", bcm_errmsg(rv));
        return rv;
    }
    ...
    rv = bcm_mpls_tunnel_initiator_create(unit, 0, num_labels, label_array);
    if (rv != BCM_E_NONE) {
        printf("error in first pass tunnel bcm_mpls_tunnel_initiator_create()\n");
        printf("(%s)\n", bcm_errmsg(rv));
        return rv;
    }
    ...
    rv = bcm_vlan_gport_add(unit,in_vid,in_port,0);
    if (rv != BCM_E_NONE) {
   ...
   /*In RIF*/
    ...
    if (rv != BCM_E_NONE) {
         printf("error in qos ext recycle bcm_l3_intf_create()\n");
         printf("(%s)\n", bcm_errmsg(rv));
         return rv;
    }
    ...
    if (rv != BCM_E_NONE) {
         printf("error in qos ext recycle bcm_l3_ingress_get()\n");
         printf("(%s)\n", bcm_errmsg(rv));
         return rv;
    }
    ...
    if (rv != BCM_E_NONE) {
         printf("error in qos ext recycle bcm_l3_ingress_create()\n");
         printf("(%s)\n", bcm_errmsg(rv));
         return rv;
    }
     ...
     * 7. Add Route entry
     */
    rv = add_route_ipv4(unit, route, mask, 0, fec_id1);
    if (rv != BCM_E_NONE)
    {
    ...
    if(rv != BCM_E_NONE) {
        printf("dnx_qos_ext_recycle_main: Error in bcm_field_context_create context %d\n", context_id);
        return rv;
    }
    ...
    if(rv != BCM_E_NONE) {
        printf("dnx_qos_ext_recycle_main: Error in bcm_field_presel_set context %d\n", context_id);
        return rv;
    }
    ...
    if (rv != BCM_E_NONE)
    {
        printf("Error (%d), in bcm_field_entry_add on contex_id = %d \n", rv, fg_id);
        return rv;
    }
    return rv;
}
