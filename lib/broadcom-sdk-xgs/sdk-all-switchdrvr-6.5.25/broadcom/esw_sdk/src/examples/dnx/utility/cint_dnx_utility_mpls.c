    ...
    for (port_idx = 0; port_idx < nof_ports; port_idx++)
    {
        if (l2_port[port_idx].port != BCM_PORT_INVALID)
        {
            ...
            else
            {
                ...
                /* Ingress */
                bcm_vlan_port_t_init(&vlan_port);
                vlan_port.port = l2_port[port_idx].port;
                vlan_port.vsi = *l2_port[port_idx].eth_rif_id;
    ...
    int nof_fecs)
{
    ...
    for (fec_idx = 0; fec_idx < nof_fecs; fec_idx++)
    {
        if (fecs[fec_idx].fec_id != MPLS_UTIL_INVALID_VALUE)
        {
            ...
            dnx_utils_l3_fec_s_common_init(unit, allocation_flags, 0, &fec_entry, fecs[fec_idx].fec_id, fecs[fec_idx].flags, 0, destination, *fecs[fec_idx].tunnel_gport, *fecs[fec_idx].tunnel_gport2);
