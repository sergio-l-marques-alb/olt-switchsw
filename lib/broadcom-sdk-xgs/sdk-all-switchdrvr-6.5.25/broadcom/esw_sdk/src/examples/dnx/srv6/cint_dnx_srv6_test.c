                            ...
                            int is_arp_plus_ac)      /** Indication if it's ARP+AC, or AC coming from ESEM */
{
    ...
    rv = l3__egress_only_fec__create_inner(unit, cint_srv6_tunnel_info.tunnel_fec_id[INGRESS_SRH_FEC_ID], srv6_basic_lif_encode, 0, gport, 0, 0,
                                           0, 0,&encoded_fec1);
    ...
    int skip_rif)
{
    ...
    if(skip_rif)
    {
        rv = l3__egress_only_fec__create_inner(unit, fec, 0 , encap_id_itf, gport, 0, 0,
                                               0, 0,&encoded_fec);
        if (rv != BCM_E_NONE)
        {
    ...
    {
        rv = l3__egress_only_fec__create_inner(unit, fec, intf_out , encap_id, gport, 0, 0,
                                               0, 0,&encoded_fec);
        if (rv != BCM_E_NONE)
        {
            printf("%s(): Error, create egress object FEC only\n",proc_name);
                            ...
                            int is_p2p)
{
    ...
    if (is_p2p)
    {
        ...
        rv = l3__egress_only_fec__create(unit, fec_id, pwe_encap.encap_id | 0x40000000, 0, gport, flags, 0);
        if (rv != BCM_E_NONE)
        {
            printf("%s(): Error, create access FEC\n",proc_name);
            return rv;
    ...
    int out_port)
{
     ...
     */
    rv = l3__egress_only_fec__create(ingress_unit, cint_srv6_tunnel_info.tunnel_fec_id[END_POINT_1_FEC_ID], cint_srv6_tunnel_info.eth_rif_intf_out[END_POINT_1], cint_srv6_tunnel_info.tunnel_arp_id[END_POINT_1], out_port, 0, 0);
    if (rv != BCM_E_NONE)
    {
        printf
                            ...
                            int is_eth_fwd)
{
    ...
    int encoded_fec1;
    BCM_L3_ITF_SET(srv6_basic_lif_encode, BCM_L3_ITF_TYPE_LIF, cint_srv6_tunnel_info.tunnel_global_lif[INGRESS_IP_TUNNEL_GLOBAL_LIF]);
    rv = l3__egress_only_fec__create_inner(ingress_unit, cint_srv6_tunnel_info.tunnel_fec_id[INGRESS_SRH_FEC_ID], srv6_basic_lif_encode, 0, out_port, 0, 0,
                                           0, 0,&encoded_fec1);
                            ...
                            int is_eth_fwd)
{
     ...
     * 6. Create FEC and set its properties
     */
    rv = l3__egress_only_fec__create(ingress_unit, cint_srv6_tunnel_info.tunnel_fec_id[EGRESS_FEC_ID], cint_srv6_tunnel_info.eth_rif_intf_out[EGRESS], cint_srv6_tunnel_info.tunnel_arp_id[EGRESS], out_port, 0, 0);
    if (rv != BCM_E_NONE)
    {
    ...
    int max_nof_terminated_usp_sids_1pass = *(dnxc_data_get (ingress_unit, "srv6", "termination", "max_nof_terminated_sids_usp_1pass", NULL));
    int nof_2pass_recycle_entries = max_nof_terminated_usp_sids - max_nof_terminated_usp_sids_1pass;
