    ...
    int base_engine)
{
    ...
    const uint32 nof_dual_buckets_per_entry = *dnxc_data_get(unit, "meter", "mem_mgmt", "nof_dual_buckets_per_entry", NULL);
    ...
    else
    {
        ...
        engine_size = (*dnxc_data_1d_get(unit, "crps", "engine", "engines_info", "nof_entries", base_engine));
        engine_size = 2 * engine_size;

        /** verify the given total_nof_meters given can be fitted  */
        max_nof_meters = (*dnxc_data_get(unit, "crps", "engine", "nof_big_engines", NULL)) * engine_size * nof_dual_buckets_per_entry;
 ...
 * \param [in] is_ingress -refer to ingress or egress policer
 * \param [in] database_id -database_id (0..2 for ingress, 0..1 for egress). database id is actually the command id or the statistic_interface that is used.
 * \param [in] engine_ids -list of engines to detach.
 * \param [in] section -list of sections, for single bucket meters in big engine, bcmPolicerEngineSectionAll will case detuch from bouth
 *                      bcmPolicerEngineSectionLow and bcmPolicerEngineSectionHigh
 *             NOTE: policer APIs do not support bcmPolicerEngineSectionAll for big engine contains single bucket meters
 * \return
 *   error indication.
 * \remark
 *   NONE
 ...
 */
int
policer_generic_meter_multi_engines_database_destroy(
    int unit,
    bcm_core_t core,
    int is_ingress,
    int database_id,
    int *engine_ids,
    bcm_policer_engine_section_t *sections,
    int single_bucket_mode,
    int nof_engines)
{

    int database_handle, rv, curr_engine, i;
    int flags = 0;
    bcm_policer_engine_t engine;
    bcm_policer_engine_section_t curr_sections;
    uint32 ingress_special_engine, egress_special_engine, special_engine;

    bcm_policer_engine_t_init(&engine);
    printf
       ("policer_generic_meter_database_destroy called: unit=%d, core=%d, is_ingress=%d, database_id=%d\n",
        unit, core, is_ingress, database_id);
    ...
    {
        engine.core_id = core;
        engine.engine_id = engine_ids[i];
        engine.section = sections[i];

        ingress_special_engine = *(dnxc_data_get(unit, "meter", "mem_mgmt", "ingress_single_bucket_engine", NULL));
        egress_special_engine = *(dnxc_data_get(unit, "meter", "mem_mgmt", "egress_single_bucket_engine", NULL));

        special_engine = ((engine.engine_id != ingress_special_engine)&&(engine.engine_id != egress_special_engine)) ? 0 : 1;

        /** in case of big engine contain single bucket meters, detach each section separately */
        if(single_bucket_mode && !special_engine &&
    ...
    int profile)
{
    ...
    if(rv != BCM_E_NONE)
   {
    ...
    rv = bcm_field_group_add(unit, 0, &epmf_fg_info, &epmf_fg_id);
    if(rv != BCM_E_NONE)
    {
        printf("Error (%d), in bcm_field_group_add\n", rv);
        return rv;
...
/**
* \brief
*  destroy egress meter pointer in the etpp.
*  meter_pointer=base_meter_ptr+Qos_offset.
*  Qos_offset is mapping of {Qos_meter_profile,tc}
*/
int
policer_egress_pointer_destroy(
    int unit,
    int core_id,
    int base_meter_ptr,
    int database_id,
    ...
    int vlan)
{
    ...
    if (rv != BCM_E_NONE)
    {
        printf("Error (%d), in bcm_qos_map_destroy\n", rv);
        return rv;
    }

   /** 2. destroy ETPP object */
    bcm_vlan_port_t_init(&out_vlan_port);
    out_vlan_port.flags = BCM_VLAN_PORT_CREATE_EGRESS_ONLY | BCM_VLAN_PORT_VLAN_TRANSLATION | BCM_VLAN_PORT_STAT_EGRESS_ENABLE;
    out_vlan_port.criteria    = BCM_VLAN_PORT_MATCH_PORT_VLAN;
    out_vlan_port.match_vlan  = 0;
    ...
    rv = (bcm_vlan_port_find(unit, &out_vlan_port));
    if (rv != BCM_E_NONE)
    {
    ...
    rv = bcm_vlan_port_destroy(unit, out_vlan_port.vlan_port_id);
    if (rv != BCM_E_NONE)
    {
    ...
    int out_port)
{
    ...
    engine_size = * dnxc_data_1d_get(unit, "crps", "engine", "engines_info", "nof_entries", first_big_engine_id);
    engine_size = 2 * engine_size;
