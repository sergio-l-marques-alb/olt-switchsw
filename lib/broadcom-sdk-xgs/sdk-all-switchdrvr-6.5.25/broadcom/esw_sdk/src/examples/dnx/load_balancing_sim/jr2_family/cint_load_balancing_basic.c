...
/*
 * Global structure that holds the PMF information information (see structure description for more information) .
 */
load_blancing_sim_pmf_cfg_info_t pmf_cfg_info;
/*
 * Global variable to check PMF hashing validation is needed 
 */
int validate_pmf = 0;
...
load_balancing_sim_example()
{
    ...
    */
    ...
    pkt_headers_info.header_offsets[1].header_type = LB_SIM_HEADER_IPV4_OUTER;
    ...
    load_balancing_sim_hw_cfg.device = DEVICE_TYPE_JERICHO2_B0;

    load_balancing_sim_hw_cfg.parser_seed = 0x00000000;
    load_balancing_sim_hw_cfg.clients_polynomial_seeds[0] = 0;
    load_balancing_sim_hw_cfg.clients_polynomial_seeds[1] = 0;
    load_balancing_sim_hw_cfg.clients_polynomial_seeds[2] = 0;
    load_balancing_sim_hw_cfg.clients_polynomial_seeds[3] = 0;
    load_balancing_sim_hw_cfg.clients_polynomial_seeds[4] = 0;

    load_balancing_sim_hw_cfg.clients_polynomial[0]= 0x015d;
    load_balancing_sim_hw_cfg.clients_polynomial[1]= 0x00d7;
    load_balancing_sim_hw_cfg.clients_polynomial[2]= 0x0039;
    load_balancing_sim_hw_cfg.clients_polynomial[3]= 0x0ac5;
    load_balancing_sim_hw_cfg.clients_polynomial[4]= 0x09e7;

    load_balancing_sim_hw_cfg.clients_16_crc_selection[0] = 0x00;
    load_balancing_sim_hw_cfg.clients_16_crc_selection[1] = 0x00;
    load_balancing_sim_hw_cfg.clients_16_crc_selection[2] = 0x00;
    load_balancing_sim_hw_cfg.clients_16_crc_selection[3] = 0x00;
    load_balancing_sim_hw_cfg.clients_16_crc_selection[4] = 0x00;



    uint32 lb_keys[5];

    rv = load_balancing_sim_main(packet_header,packet_size, &pkt_headers_info, &load_balancing_sim_hw_cfg, lb_keys);
    if(rv != 0)
    {
...
load_balancing_sim_validate(int unit, int core)
{
    ...
    uint32 lag_key;
    uint32 nwk_key;
    /** some of the signals are different between device and adapter */
    uint8 is_adapter = !(*(dnxc_data_get(unit, "device", "general", "hw_support", NULL)));
    char* block_name = "IRPP";
    char* key_from_name = (is_adapter) ? "ipmf2" : "ipmf1";
    char* key_to_name = "fer";
    char* header_from_name = (is_adapter) ? "VTT1" : "NIF";
    char* fwd_index_from_name = "VTT5";
    char* fwd_index_to_name = "FWD1";
    char* packet_signal_name = (is_adapter) ? "packet_header" : "header";
    ...
    if (validate_pmf) {
        ...
        key.size = 64;
        key.data[0] = pmf_cfg_info.qual_info_b.input_arg;;
        key.data[1] = pmf_cfg_info.qual_info_a.input_arg;
        rv = load_balancing_pmf_hash_sim_main(unit, packet_header, byte_count, &headers_info, &load_balancing_sim_hw_cfg, key, lb_keys);
    } else {
        rv = load_balancing_sim_main(packet_header, byte_count, &headers_info, &load_balancing_sim_hw_cfg, lb_keys);
    }
    if (rv != BCM_E_NONE) {
        printf("load_balancing_sim_main failed \n");
    }
    ...
    rv = cint_dnx_switch_lb_offline_sim_destroy(unit);
    if(rv != BCM_E_NONE)
    {
        printf("cint_dnx_switch_lb_offline_sim_destroy failed \n");
    }
