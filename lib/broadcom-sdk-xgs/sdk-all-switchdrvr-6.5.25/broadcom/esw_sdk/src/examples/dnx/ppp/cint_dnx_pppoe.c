    ...
    int is_wide)
{
    ...
    if (is_mp)
    {
        ...
        /** Convert tunnel's GPort ID to intf ID */
        BCM_GPORT_TUNNEL_TO_L3_ITF_LIF(intf_id, ppp_gport);
    ...
    int nni_port)
{
    ...
    if (cint_pppoe_basic_info.is_egress_mc)
    {
        ...
        rv = add_ipv6_ipmc(unit, cint_pppoe_basic_info.mc_dip6, dip6_mask, cint_pppoe_basic_info.mc_sip6, sip6_mask, 0, cint_pppoe_basic_info.vrf, 0, 0, cint_pppoe_basic_info.egress_mc_group, 0,0);
        if (rv != BCM_E_NONE)
        {
            printf("Error, in function add_ipv4_ipmc: vrf = %d, mc group = %d\n", cint_pppoe_basic_info.vrf, cint_pppoe_basic_info.egress_mc_group);
            return rv;
    ...
    else
    {
        ...
        rv = bcm_multicast_create(unit, flags, &cint_pppoe_basic_info.ingress_mc_group);
        if (rv != BCM_E_NONE)
        {
            printf("Error, in bcm_multicast_create \n");
            return rv;
        ...
        if (ibch1_supported)
        {
            bcm_multicast_replication_t_init(&rep_array);
            rep_array.port = uni_port;
            rep_array.encap1 = BCM_GPORT_TUNNEL_ID_GET(cint_pppoe_basic_info.initiator_id);
            rv = bcm_multicast_add(unit, cint_pppoe_basic_info.ingress_mc_group, BCM_MULTICAST_INGRESS_GROUP, 1, &rep_array);
            if (rv != BCM_E_NONE)
            {
                printf("Error, in bcm_multicast_add Port= %d\n", uni_port);
                return rv;
        ...
        if (rv != BCM_E_NONE)
        {
   		...
   		if(!compressed_vrf)
        {   
            rv = add_ipv6_ipmc(unit, cint_pppoe_basic_info.mc_dip6, dip6_mask, cint_pppoe_basic_info.mc_sip6, sip6_mask, 0, cint_pppoe_basic_info.vrf, 0, 0, cint_pppoe_basic_info.ingress_mc_group, 0,0);
            if (rv != BCM_E_NONE)
            {
                printf("Error, in function add_ipv4_ipmc: vrf = %d, mc group = %d\n", cint_pppoe_basic_info.vrf, cint_pppoe_basic_info.ingress_mc_group);
                return rv;
