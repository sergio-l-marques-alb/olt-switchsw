...
/*
 *
 * $Id: cint_l2_fast_flush.c,v 1.4 Broadcom SDK $
...
int ring_protection_fast_flush(int unit, int flush_group_id)
{
    ...
    printf("Performed flush(bcm_l2_replace) for unit %d, flush_group - %d\n", unit, flush_group_id);
...
int ring_protect_fast_flush_by_dest_only(int unit, bcm_gport_t gport)
{
    ...
    if (rv != BCM_E_NONE) {
        printf("Error in bcm_l2_replace with gport(0x%x) - %s\n", l2_replace_addr.port, bcm_errmsg(rv));
        return rv;
    }
...
int ring_protect_with_jr1_mode_create(int unit, int inP, int outP)
{
    ...
    if (rv != BCM_E_NONE) {
        printf("Error, bcm_vlan_port_find - %s\n", bcm_errmsg(rv));
        return rv;
    }

    /* Compare group value and failover_port_id */
    if (vlan_port_get.group != vlan_port.group)
    {
        printf("%s(): Comparison failed for gport 0x%08x, expected group - 0x%08x, received group - 0x%08x\n",
               vlan_port.vlan_port_id, vlan_port.group, vlan_port_get.group);
        return BCM_E_FAIL;
    }
