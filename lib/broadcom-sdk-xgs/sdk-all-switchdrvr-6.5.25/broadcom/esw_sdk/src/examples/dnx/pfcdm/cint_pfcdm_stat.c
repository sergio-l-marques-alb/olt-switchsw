...
const int PFCDM_DATA_MAPPING_ARRAY_MAX = 10;
    ...
    int *engine_id_arr)
{
    ...
    bcm_stat_expansion_data_mapping_t data_mapping_array[PFCDM_DATA_MAPPING_ARRAY_MAX];
    int counter_set_size;
    ...
    int database_id)
{
    ...
    for (index = 0; index < nof_engines; index ++)
    {
        ...
        engine_size = (*dnxc_data_1d_get(unit, "crps", "engine", "engines_info", "nof_entries", engine_arr[0])) *
                (*dnxc_data_1d_get(unit, "crps", "engine", "counter_format_types", "nof_counters_per_entry", bcmStatCounterFormatPacketsAndBytes));
