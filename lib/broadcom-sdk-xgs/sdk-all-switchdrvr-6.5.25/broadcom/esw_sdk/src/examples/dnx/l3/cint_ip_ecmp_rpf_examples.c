    ...
    int vsi_none)
{
    ...
    rv = l3__egress_only_fec__create(unit, fec_id, intf_out, encap_id, out_port, 0, 0);
    if (rv != BCM_E_NONE)
    {
    ...
    for (iter = 0; iter < NOF_OF_MEMBERS_ECMP_GROUP; iter++)
    {
         ...
         */
        rv = l3__egress_only_fec__create(unit, fec_id_ecmp, intf_out, encap_index, out_port, 0, 0);
        if (rv != BCM_E_NONE)
        {
            printf("Error, create egress object FEC only, intf_out\n");
            return rv;
    ...
    for (mid = 0; mid < nof_rpf_modes; mid++)
    {
        rv = l3__egress_only_fec__create(unit, fec_id_rpf[mid], intf_out, encap_id, out_port, 0, 0);
        if (rv != BCM_E_NONE)
        {
            printf("Error, create egress object FEC only, intf_out\n");
            return rv;
