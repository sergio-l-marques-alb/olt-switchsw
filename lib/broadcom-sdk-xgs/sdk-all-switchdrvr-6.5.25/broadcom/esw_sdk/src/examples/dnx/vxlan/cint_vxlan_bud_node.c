/*
 * $Id: cint_vxlan_bud_node.c,v 1.0 2017/12/6 10:54:52 sk019289 Exp $
    ...
    int * recycle_entry_encap_id) 
{
    ...
    rv = vxlan_bud_node_create_control_lif(unit, &control_lif_gport);
    if (rv != BCM_E_NONE)
    {
        printf("Error, create_control_lif \n");
        return rv;
    }
    rv = vxlan_bud_node_create_recycle_entry(unit, control_lif_gport, recycle_entry_encap_id);
    {
        printf("Error, vxlan_bud_node_create_recycle_entry \n");
        return rv;
    }
    return rv; 
}
    ...
    int recycle_port)
{
    ...
    rv = vxlan_bud_node_create_recycle_entry_with_control_lif(unit, &recycle_entry_encap_id); 
    if (rv != BCM_E_NONE)
    {
