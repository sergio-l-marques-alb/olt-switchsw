    ...
    bcm_field_context_t context_id)
{
    ...
    rv = bcm_field_group_add(unit, 0, &fg_info, &policer_fg_id);
    if (rv != BCM_E_NONE)
    {
    ...
    rv = bcm_field_group_context_attach(unit, 0, policer_fg_id, context_id, &attach_info);
    if (rv != BCM_E_NONE)
    {
...
int policer_acl_rule_destroy(int unit, bcm_field_context_t context_id)
{
    int rv = BCM_E_NONE;
    ...
    rv = bcm_field_group_context_detach(unit, policer_fg_id, context_id);
    if (rv != BCM_E_NONE)
    {
