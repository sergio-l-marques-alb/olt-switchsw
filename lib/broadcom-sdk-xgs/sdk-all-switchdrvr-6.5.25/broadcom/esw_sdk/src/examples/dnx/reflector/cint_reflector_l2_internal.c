    ...
    bcm_field_context_t epmf_context_id)
{
    ...
    bcm_port_mapping_info_t mapping_info, mapping_info_rcy;
    ...
    if (is_use_dedicated_rcy_port) {
        rv = bcm_port_get(unit, rcy_port, &port_flags, &interface_info, &mapping_info_rcy);
        if (rv != BCM_E_NONE)
        {
    ...
    rv = cint_reflector_l2_internal_fp_entry_add(unit, fg_id, &fg_info, out_tm_port, mapping_info_rcy.tm_port, ace_entry_handle, encap_id);
    if (rv != BCM_E_NONE)
    {
    ...
    bcm_field_context_t epmf_context_id)
{
    ...
    out_tm_port = mapping_info.tm_port;
    rv = cint_reflector_l2_internal_fp_entry_add(unit, fg_id, &fg_info, out_tm_port, 0, ace_entry_handle, encap_id);
    if (rv != BCM_E_NONE)
    {
        printf("cint_reflector_l2_internal_fp_entry_add failed\n");
        return rv;
        ...
        bcm_field_context_t epmf_context_id)
{
    ...
    rv = cint_reflector_l2_internal_mc_create(unit, egress_port, rcy_port, my_mac, epmf_context_id);
    if (rv != BCM_E_NONE)
    {
    ...
    int egress_port)
{
    ...
    /**config for IBCH1.PTC as RCY_port.PTC*/
    
    char cmd[300] = {0};
    sprintf(cmd, "mod ETPPB_TRAP_PER_PP_DSP %d 1 PP_DSP_STAMP_VALUE=9", egress_port);
    bshell(unit, cmd);
        ...
        bcm_field_context_t epmf_context_id)
{
    ...
    bcm_field_group_info_t fg_info;
    bcm_port_interface_info_t interface_info;
    bcm_port_mapping_info_t mapping_info_egress, mapping_info_rcy;
    uint32 flags;
    bcm_mirror_port_to_rcy_map_info_t rcy_map_info, rcy_map_info_clear;
    ...
    if (is_use_dedicated_rcy_port) {
        rv = bcm_port_get(unit, rcy_port, &flags, &interface_info, &mapping_info_rcy);
        if (rv != BCM_E_NONE)
        {
            printf("bcm_port_get failed for rcy_port\n");
            return rv;
    ...
    rv = cint_reflector_l2_internal_uc_fp_entry_add(unit, fg_id, &fg_info, in_port, mapping_info_egress.tm_port, mapping_info_rcy.tm_port, ace_entry_handle, is_ip_swap, is_udp_swap, reflector_encap_id);
    if (rv != BCM_E_NONE)
    {
        printf("cint_reflector_l2_internal_fp_entry_add failed\n");
        return rv;
        ...
        bcm_field_context_t epmf_context_id)
{
    ...
    rv = cint_reflector_l2_internal_uc_fp_entry_add(unit, fg_id, &fg_info, in_port, mapping_info_egress.tm_port, 0, ace_entry_handle, is_ip_swap, is_udp_swap, reflector_encap_id);
    if (rv != BCM_E_NONE)
    {
        printf("cint_reflector_l2_internal_fp_entry_add failed\n");
        return rv;
