    ...
    int presel_index)
{
    ...
    uint32 presel_qual_value[Cint_field_st_multi_nof_presels] = {fwd_layer, bcmTagStructTypeCTag};
 ...
 * \param [in]  state_table_atomic_rmw - If the device supports state table read modify write.
 * \return
 *   shr_error_e - Type of Error
 * \remark
 *   * None
    ...
    int unit)
{
    ...
    *******************************************************************************/
    ...
    rv = bcm_field_group_add(unit, 0, &fg_info, &Cint_field_st_multi_fg_id[2]);
    if (rv != BCM_E_NONE)
    {
      printf("Error (%d), in bcm_field_group_add\n", rv);
      return rv;
 ...
 * \param [in]  unit               - Device ID
 * \param [in]  fwd_layer          - The forwarding layer.
 * \param [in]  state_table_atomic_rmw - If the device supportd state table read modify write.
 * \return
 *   shr_error_e - Type of Error
 * \remark
 *   * None
    ...
    int state_table_atomic_rmw)
{
     ...
     */
    rv = cint_field_st_multi_cfg(unit, fwd_layer, 0, state_table_atomic_rmw);
    if (rv != BCM_E_NONE)
    {
     ...
     */
    rv = cint_field_st_multi_cfg(unit, fwd_layer, 1, state_table_atomic_rmw);
    if (rv != BCM_E_NONE)
    {
        printf("Error (%d), in cint_field_st_multi_cfg %d\n", rv);
        return rv;
 ...
 *
 * \param [in]  unit               - Device ID
 * \param [in]  state_table_atomic_rmw - If the device supports read mify write in state table.
 * \return
 *   shr_error_e - Type of Error
 * \remark
 *   * None
