    ...
    int compare_id)
{
    ...
    rv = bcm_field_group_add(unit, 0, &fg_info, &Cint_field_compare_single_fg_id_ipmf1);
    if (rv != BCM_E_NONE)
    {
    ...
    bcm_field_context_t * context_id_p)
{
    ...
    /** Create FG without ID. */
    rv = bcm_field_group_add(unit, 0, &fg_info, &Cint_field_compare_single_fg_id_ipmf2);
    if (rv != BCM_E_NONE)
    {
        printf("%s Error (%d), in bcm_field_group_add\n", proc_name, rv);
        return rv;
