/*
 * $Id: cint_dnx_tunnel_l2_encap_external_cpu.c, Exp $
 ...
 * ./bcm.user
 * cd ../../../../regress/bcm
 * bcm shell "config add tm_port_header_type_out_200.0=ENCAP_EXTERNAL_CPU"
 * bcm shell "config add tm_port_header_type_out_201.0=ENCAP_EXTERNAL_CPU"
 ...
 *
 * bcm shell "config delete tm_port_header_type_out_200.0"
 * bcm shell "config delete tm_port_header_type_out_201.0"
 * bcm shell "0: tr 141 NoInit=1"
 * bcm shell "config add custom_feature_init_verify=1"
 * bcm shell "config delete custom_feature_multithread_en*"
 * bcm shell "config delete custom_feature_kbp_multithread_en*"
    ...
    int unit)
{
    ...
    else
    {
        ...
        rv = l3__egress_only_fec__create_inner(unit, l2_external_cpu_info.fec_id_enc+2, 1, BCM_L3_ITF_VAL_GET(l2_external_cpu_info.l2_remote_arp_intf[0]), gport, 0,
        ...
        rv = add_route_ipv4(unit, l2_external_cpu_info.dip_fwd, l2_external_cpu_info.dip_fwd_mask, l2_external_cpu_info.vrf, encoded_fec);
        if (rv != BCM_E_NONE)
        {
    ...
    int unit)
{
    ...
    rv = l3__egress_only_fec__create(unit, l2_external_cpu_info.fec_id_enc, 0, 0, mc_gport, 0, BCM_L3_FLAGS2_FWD_ONLY);
    if (rv != BCM_E_NONE)
    {
    ...
    {
        rv = add_route_ipv4(unit,l2_external_cpu_info.dip_mc_group, l2_external_cpu_info.dip_fwd_mask, l2_external_cpu_info.vrf, l2_external_cpu_info.fec_id_enc);
        if (rv != BCM_E_NONE)
        {
            printf("Error, in function add_route_ipv4\n");
            return rv;
    ...
    int unit)
{
    ...
    rv = get_first_fec_in_range_which_not_in_ecmp_range(unit, 0, 0, &cint_ipv4_tunnel_info.tunnel_fec_id);
    if (rv != BCM_E_NONE)
    {
     ...
     * Create FEC
     */
    rv = l3__egress_only_fec__create(unit, cint_ipv4_tunnel_info.tunnel_fec_id, l3_intf.l3a_intf_id, 0, l2_external_cpu_info.remote_port1, 0, 0);
    if (rv != BCM_E_NONE)
    {
    ...
    uint8 create_ipv4_tunnel)
{
    ...
    rv = get_first_fec_in_range_which_not_in_ecmp_range(unit, 0, 0, &l2_external_cpu_info.fec_id_enc);
    if (rv != BCM_E_NONE)
    {
        printf("Error, get_first_fec_in_range_which_not_in_ecmp_range\n");
        return rv;
    ...
    if (!create_ipv4_tunnel)
    {
      ...
      */
      ...
      if (rv != BCM_E_NONE)
      {
          printf("Error, l2_external_cpu_mc \n");
          print rv;
          return rv;
      }
    }
    ...
    bcm_port_t out_port2)
{
    ...
    rv = l2_external_cpu_uc_l2_forwarding(unit);
    if (rv != BCM_E_NONE) {
        printf("Error, l2_external_cpu_uc_l2_forwarding \n");
        return rv;
    }
    ...
    rv = l2_external_cpu_mc_l2_forwarding(unit);
    if (rv != BCM_E_NONE) {
        printf("Error, l2_external_cpu_mc_l2_forwarding \n");
        return rv;
    }
