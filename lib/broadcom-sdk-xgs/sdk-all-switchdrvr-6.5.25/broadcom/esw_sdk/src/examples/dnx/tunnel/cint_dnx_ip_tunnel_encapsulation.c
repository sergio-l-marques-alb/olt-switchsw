/*
 * $Id: cint_dnx_ip_tunnel_encapsulation.c,v 1.15 Broadcom SDK $
 $Copyright: (c) 2021 Broadcom.
 Broadcom Proprietary and Confidential. All rights reserved.$File: cint_dnx_ip_tunnel_encapsulation.c Purpose: Various examples for IPTunnels in J2        .
 *                                                                                                                     .
 ...
 * cint
 * ip_tunnel_encap_mpls_fwd(0,202,203);
 * exit;
 * 
 * 
 * Traffic: 
...
ip_tunnel_encap(
    ...
    int rv = BCM_E_NONE;
    char *proc_name = "ip_tunnel_encap";
...
int ip_tunnel_encap_udp(
    ...
    int rv = BCM_E_NONE;
    char *proc_name = "ip_tunnel_encap_udp";
    cint_ip_tunnel_basic_info.tunnel_type = bcmTunnelTypeUdp;
    cint_ip_tunnel_basic_info.udp_dst_port = udp_dst_port;
    rv = ip_tunnel_encap(unit, access_port, provider_port);
    if (rv != BCM_E_NONE)
    {
        printf("%s(): Error, ip_tunnel_encap\n", proc_name);
        return rv;
    }
    return rv;
}
...
/*
 * Main function for basic MPLS and ip tunnel encapsulation
 * The function implements MPLS tunnel into GRE4 IP tunnel encapsulation of IPv4oETH packet
 * Inputs: unit - relevant unit;
 *         access_port - incoming port;
 *         provider_port - outgoing port;
 */
int
mpls_with_ip_tunnel_gre_encap(
    int unit,
    int access_port,
    int provider_port)
{
    ...
    proc_name = "mpls_with_ip_tunnel_gre_encap";
     ...
     * Create GRE IP tunnel
     */
    rv = tunnel_initiator_create(unit, cint_ip_tunnel_basic_info.tunnel_arp_id, &cint_ip_tunnel_basic_info.tunnel_intf, 0);
    if (rv != BCM_E_NONE)
    {
        printf("%s(): Error, tunnel_initiator_create\n", proc_name);
        return rv;
    ...
    /*
     * IPv4 routing into MPLS tunnel
     */
    rv = add_host_ipv4(unit, cint_ip_tunnel_basic_info.route_ipv4_dip, cint_ip_tunnel_basic_info.access_vrf,
                       cint_ip_tunnel_basic_info.tunnel_fec_id, 0, 0);
    if (rv != BCM_E_NONE)
    {
    ...
    int gre_lb_key_enable)
{
    ...
    if (cint_ip_tunnel_basic_info.svtag_lookup) {
        ...
        tunnel_info.egress_qos_model.egress_ttl = bcmQosEgressModelUniform;
        tunnel_info.egress_qos_model.egress_qos = bcmQosEgressModelUniform;
    }
    if (*tunnel_intf > 0)
    {
