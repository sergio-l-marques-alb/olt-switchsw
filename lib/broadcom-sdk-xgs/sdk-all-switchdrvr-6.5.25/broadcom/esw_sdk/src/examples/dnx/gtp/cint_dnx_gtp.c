...
struct cint_gtp_basic_info_s
{
    ...
    int tunnel_ttl;                     /* ttl */
    int ingress_qos_map_id;          /* gtp tunnel termination qos profile*/
    int egress_qos_map_id;           /* gtp tunnel initiator qos profile*/
    int dscp;                        /* gtp qfi */
    int pdu_type;
};
...
/*update IP tunnel with QOS map id*/
int
ip_tunnel_initiator_update_qos(
    int unit,
    int qos_map_id)
{
    int rv;
    ...
    bcm_tunnel_initiator_t_init(&tunnel_info);
    bcm_l3_intf_t_init(&l3_intf);
    l3_intf.l3a_intf_id = cint_gtp_basic_info.ip_tunnel_outlif;
    rv = bcm_tunnel_initiator_get(unit,&l3_intf,&tunnel_info);
    if (rv != BCM_E_NONE)
    {
        printf("Error, bcm_tunnel_initiator_get %d\n", rv);
        return rv;
    }

    tunnel_info.qos_map_id = qos_map_id;
    tunnel_info.egress_qos_model.egress_qos = bcmQosEgressModelUniform;
    tunnel_info.flags |= (BCM_TUNNEL_WITH_ID | BCM_TUNNEL_REPLACE);
    rv = bcm_tunnel_initiator_create(unit, &l3_intf, tunnel_info);
    if (rv != BCM_E_NONE)
    {
    ...
    bcm_gport_t *tunnel_id)
{
    ...
    tunnel_term.ingress_qos_model.ingress_phb = bcmQosIngressModelPipe;
    tunnel_term.ingress_qos_model.ingress_remark = bcmQosIngressModelPipe;
    tunnel_term.ingress_qos_model.ingress_ttl = bcmQosIngressModelPipe;
...
{    
    ...
    BCM_IF_ERROR_RETURN(bcm_flow_handle_get(unit, "GTP_TERMINATOR",&flow_handle));
    ...
    *terminator_id = flow_handle_info.flow_id;
    ...
    BCM_IF_ERROR_RETURN(bcm_flow_handle_get(unit, "GTP_TERM_MATCH", &flow_handle));
    flow_handle_info.flow_handle = flow_handle;
    ...
    bcm_flow_initiator_info_t info)
{
    ...
    if ((info.valid_elements_set & BCM_FLOW_INITIATOR_ELEMENT_QOS_EGRESS_MODEL_VALID) && (info.egress_qos_model.egress_qos != bcmQosEgressModelUniform))
    {
        ...
        special_fields.special_fields[nof_spec_fields].shr_var_uint32 = cint_gtp_basic_info.dscp;
        nof_spec_fields++;
        ...
        BCM_IF_ERROR_RETURN(bcm_flow_logical_field_id_get(unit, flow_handle, "QOS_DSCP", &field_id));
        special_fields.special_fields[nof_spec_fields].field_id = field_id;
        special_fields.special_fields[nof_spec_fields].is_clear = TRUE;
        nof_spec_fields++;
    }
    special_fields.actual_nof_special_fields++;
    BCM_IF_ERROR_RETURN(bcm_flow_logical_field_id_get(unit, flow_handle, "GTP_PDU_TYPE", &field_id));
    special_fields.special_fields[nof_spec_fields].field_id = field_id;
    special_fields.special_fields[nof_spec_fields].shr_var_uint32 = cint_gtp_basic_info.pdu_type;
...
/* 
 * GTP ingress map:
 * Incoming-QFI-> nwk_qos
 *         QFI -> (QFI + 1) % 64
  * Incoming-QFI->    TC,           DP
 *       QFI    -> (QFI + 1) % 8,   1
 */
int
gtp_qos_map_ingress_profile(int unit, int remark_opcode, int phb_opcode)
{
    bcm_qos_map_t in_map;
    int flags = 0;
    int dscp;
    int rv = BCM_E_NONE;
    int opcode = remark_opcode;

    /** Clear structure */
    bcm_qos_map_t_init(&in_map);

    /** Create QOS profile */
    rv = bcm_qos_map_create(unit, BCM_QOS_MAP_INGRESS | BCM_QOS_MAP_PHB | BCM_QOS_MAP_REMARK | BCM_QOS_MAP_WITH_ID,
                            &cint_gtp_basic_info.ingress_qos_map_id);
    if (rv != BCM_E_NONE)
    {
        printf("error in ingress profile()\n");
        printf("rv is: $rv \n");
        printf("(%s)\n", bcm_errmsg(rv));
        return rv;
    }
    printf("allocated qos map id: %x\n", cint_gtp_basic_info.ingress_qos_map_id);
    ...
    if (rv != BCM_E_NONE)
    {
        printf("error in ingress remark opocde()\n");
        printf("rv is: $rv \n");
        printf("(%s)\n", bcm_errmsg(rv));
        return rv;
    }
     ...
     */
    flags = BCM_QOS_MAP_GTP | BCM_QOS_MAP_REMARK | BCM_QOS_MAP_OPCODE;
    rv = bcm_qos_map_add(unit, flags, &in_map, cint_gtp_basic_info.ingress_qos_map_id);
    if (rv != BCM_E_NONE)
    {
    ...
    printf("allocated Remark opcode: %x\n", opcode);
    for (dscp = 0; dscp < 64; dscp++)
    {
         ...
         * Incoming (dscp) -> nwk_qos
         */
        in_map.dscp = dscp;
        in_map.remark_int_pri = (dscp + 1) % 64 ;

        flags = BCM_QOS_MAP_GTP | BCM_QOS_MAP_REMARK;
        rv = bcm_qos_map_add(unit, flags, &in_map, opcode);
        if (rv != BCM_E_NONE)
        {
    ...
    printf("allocated PHB opcode: %x\n", opcode);
    /** Clear structure */
    bcm_qos_map_t_init(&in_map);
    in_map.opcode = opcode;
    flags = BCM_QOS_MAP_GTP | BCM_QOS_MAP_PHB | BCM_QOS_MAP_OPCODE;
    rv = bcm_qos_map_add(unit, flags, &in_map, cint_gtp_basic_info.ingress_qos_map_id);
    if (rv != BCM_E_NONE)
    {
    ...
    for (dscp = 0; dscp < 64; dscp++)
    {
         ...
         */
        in_map.dscp = dscp;
        in_map.int_pri = (dscp + 1) % 8;
        in_map.color = 1;

        flags = BCM_QOS_MAP_GTP | BCM_QOS_MAP_PHB;
        rv = bcm_qos_map_add(unit, flags, &in_map, opcode);
        if (rv != BCM_E_NONE)
        {
            printf("error in l3 ingress bcm_qos_map_add()\n");
...
/* MAp: COLOR, INT_PRI(NWK_WOS)-> DSCP (QFI)*/
int 
gtp_qos_map_egress_profile(int unit, int opcode)
{
    bcm_qos_map_t gtp_eg_map;
    int flags = 0;
    int gtp_qfi;
    int remark_opcode = opcode;
    int rv;
    int color;
    int nwk_qos;

    /** Clear structure */
    bcm_qos_map_t_init(&gtp_eg_map);

    /** Create QOS profile */
    rv = bcm_qos_map_create(unit, BCM_QOS_MAP_EGRESS | BCM_QOS_MAP_REMARK | BCM_QOS_MAP_WITH_ID, &cint_gtp_basic_info.egress_qos_map_id);
    if (rv != BCM_E_NONE)
    {
        printf("(%s)\n", bcm_errmsg(rv));
        return rv;
    }
    printf("allocated qos map id: %x\n", cint_gtp_basic_info.egress_qos_map_id);
        /** Create QOS opcode */
     ...
     */
    flags = BCM_QOS_MAP_GTP | BCM_QOS_MAP_REMARK | BCM_QOS_MAP_OPCODE;
    gtp_eg_map.opcode = remark_opcode;
    rv = bcm_qos_map_add(unit, flags, &gtp_eg_map, cint_gtp_basic_info.egress_qos_map_id);
    if (rv != BCM_E_NONE)
    {
        printf("(%s)\n", bcm_errmsg(rv));
        return rv;
    ...
    /** COLOR, INT_PRI(NWK_WOS) -> DSCP (QFI) */
    
    for (nwk_qos = 0; nwk_qos < 256; nwk_qos++)
    {
        for (color = 0; color < 4; color++)
        {
            bcm_qos_map_t_init(&gtp_eg_map);

            gtp_eg_map.color = color;
            gtp_eg_map.int_pri = nwk_qos;
            /*
             * Egress mapping is:  qos_var -> qos_var+1
             */
            gtp_eg_map.dscp = (nwk_qos + 1) % 64;
            flags = BCM_QOS_MAP_GTP | BCM_QOS_MAP_REMARK;
            rv = bcm_qos_map_add(unit, flags, &gtp_eg_map, remark_opcode);
            if (rv != BCM_E_NONE)
            {
                printf("(%s)\n", bcm_errmsg(rv));
                return rv;
     ...
     * COLOR, INT_PRI(NWK_WOS) -> remark_int_pri (new NWK_QOS)
     */
    bcm_qos_map_t_init(&gtp_eg_map);
    for (nwk_qos = 0; nwk_qos < 256; nwk_qos++)
    {
        for (color = 0; color < 4; color++)
        {
            ...
            gtp_eg_map.int_pri = nwk_qos;
            /*
             * Egress inheritance mapping is 1:1
             */
            gtp_eg_map.remark_color = 1;
            gtp_eg_map.remark_int_pri = nwk_qos ;
