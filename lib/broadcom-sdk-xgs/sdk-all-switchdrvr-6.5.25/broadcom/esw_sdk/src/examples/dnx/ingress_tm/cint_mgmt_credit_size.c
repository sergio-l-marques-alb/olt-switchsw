...
int cint_bcm_mgmt_delay_tolerenace_level_create(int fap_unit, int dest_fap_unit, bcm_port_t dest_port, int credit_size)
{
    ...
    /** create delay tolerance profile according to speed and credit size */
    {
        ...
        if (profile_is_rate_class)
        {
            ...
            rv = bcm_cosq_gport_rate_class_create(fap_unit, rate_class_gport, flags, &create_info);
            if (BCM_E_NONE != rv) {
                printf("bcm_cosq_gport_rate_class_create failed \n");
                return rv;
            }
        }
        else
    ...
    /** map designated voq to the created delay tolerance profile */
    {
        ...
        for (cos = 0; cos < 8; cos++)
        {
            if (profile_is_rate_class)
            {
                ...
                rv = bcm_cosq_profile_mapping_set(fap_unit, voq_gport, cos, 0, profile_mapping);
                if (BCM_FAILURE(rv))
                {
                    printf("Error, bcm_cosq_profile_mapping_set (base_voq=%d, cos=%d, profile=%d) \n", base_voq, cos, profile);
                    return rv;
                }
            }
            else
