    ...
    if (device_type < device_type_jericho2) {
      ...
      if (BCM_E_NONE != rv) {
       ...
       if (rv != BCM_E_NONE)
       {
        ...
        rv = cint_field_bfd_echo_entry_add(unit, 0x7fffff03, NULL, bfd_local_discr & 0xFFFF, bfd_local_discr & 0xFFFF);
        if (rv != BCM_E_NONE) {
...
void sbfd_reflector_expected_checksum_get( int unit,uint16 *expected_checksum, int is_mpls) 
{
    ...
    int checksum = 0;
    int i = 0;
    printf("before change ip_field[4]=0x%x\n",ip_field[4]); 
    if(sbfd_rand_ip_ttl != -1)
    {
        ip_field[4] = sbfd_rand_ip_ttl << 8 | (ip_field[4] & 0xff);     
    }
    for(i=0; i<10; i++)
    {
...
/**
* Define global entry and group id of Cascade PMF for SBFD IP checksum calculation and trap packet to recycle port
*/
bcm_field_entry_t ent_a_global;
bcm_field_entry_t ent_b_global;
bcm_field_group_t group_a_global = 12;
bcm_field_group_t group_b_global = 13;

/**
* Remove entry and group of Cascade PMF for SBFD IP checksum calculation and trap packet to recycle port
*/
void sbfd_reflector_src_ip_pmf_remove(int unit)
{
    bcm_field_entry_remove(unit,ent_a_global);
    bcm_field_entry_remove(unit,ent_b_global);
    bcm_field_group_remove(unit,group_a_global);
    bcm_field_group_remove(unit,group_b_global);
}

/**
* Re-install entry and group of Cascade PMF for SBFD IP checksum calculation and trap packet to recycle port
*/
void sbfd_reflector_src_ip_pmf_reconf(int unit)
{
    bcm_field_entry_install(unit,ent_a_global);
    bcm_field_entry_install(unit,ent_b_global);
    bcm_field_group_install(unit,group_a_global);
    bcm_field_group_install(unit,group_b_global);
}
...
int sbfd_reflector_src_ip_pmf_config( int unit,bcm_port_t recycle_port, int dummy_lif, int is_mpls) 
{
    ...
    bcm_gport_t gport;
    bcm_field_group_t group_a = group_a_global;
    bcm_field_group_t group_b = group_b_global;
