...
int pon_gport_num = 1;
int pon_gports_with_same_vsi = 0;
...
int pon_anti_spoofing_init(int unit, bcm_port_t pon_port, bcm_port_t nni_port)
{
    ...
    if (pon_gports_with_same_vsi)
    {
        pon_gport_num = 2;
    }
    if (pon_gport_num > 2)
    {
        printf("pon_gport_num:%d is bigger than 2\n", pon_gport_num);
        return BCM_E_PARAM;
    }
    ...
    if (pon_gports_with_same_vsi)
    {
        /* src_bind[5] -- ipv4 static subnet */
        /* 11.10.10.10/24 */
        src_bind[index].ip = 0x01010101;
        src_bind[index].ip_mask = 0xFFFFFF00;/*255.255.255.0*/
        src_bind[index].flags = BCM_L3_SOURCE_BIND_USE_MASK;
        index++;

        /* src_bind[6] -- ipv6 static subnet */
        src_bind[index].flags = (BCM_L3_SOURCE_BIND_IP6 | BCM_L3_SOURCE_BIND_USE_MASK);
        src_bind[index].ip6[15]= 0x11;
        src_bind[index].ip6[0] = 0x55;
        src_bind[index].ip6_mask[0] = 0xFF;
        src_bind[index].ip6_mask[1] = 0xFF;
        src_bind[index].ip6_mask[2] = 0xFF;
        src_bind[index].ip6_mask[3] = 0xFF;
        src_bind[index].ip6_mask[4] = 0xFF;
        src_bind[index].ip6_mask[5] = 0xFF;
        src_bind[index].ip6_mask[6] = 0xFF;
        src_bind[index].ip6_mask[7] = 0xFF;
        index++;
    }
...
int pon_anti_spoofing_app(int unit, bcm_port_t pon, bcm_port_t nni)
{
    ...
    for (i = 0; i < pon_gport_num; i++)
    {
        ...
        rv = pon_lif_create(unit, 
                            ...
                            match_otag,
                            0, 0,
                            (pon_anti_spoofing_info.tunnel_id + i),
                            (pon_anti_spoofing_info.tunnel_id + i),
                            pon_anti_spoofing_info.pon_cvlan,
                            0, 0, 0,
                            &(pon_anti_spoofing_info.pon_gport[i]));
        if (rv != BCM_E_NONE)
        {
    ...
    {
        src_bind[i].port = pon_anti_spoofing_info.pon_gport[0];
    }
    ...
    for (i = 0; i < pon_gport_num; i++)
    {
        ...
        if (rv != BCM_E_NONE)
        {
            printf("Error, bcm_l3_source_bind_enable_set returned %s pon_gport:%d\n", 
                    bcm_errmsg(rv), pon_anti_spoofing_info.pon_gport[i]);
            return rv;
        }
    }
...
/*********************************************************************************
* L3 source binding entry get test.
*/
int pon_anti_spoofing_get_test(int unit)
{
    int rv = 0;
    int i = 0;
    int j = 0;
    bcm_l3_source_bind_t source_bind_get;
    int exp_rv = BCM_E_NONE;


    /* We have added 4 bind configuration(IPV4 DHCP, IPV4 STATIC, IPV6 DHCP and IPV6 STATIC) to PON gport1
     */
    for (i = 0; i < pon_gport_num; i++)
    {
        for (j = 0; j < bind_entries_num; j++)
        {
            bcm_l3_source_bind_t_init(&source_bind_get);
            sal_memcpy(&source_bind_get, &src_bind[j], sizeof(source_bind_get));
            source_bind_get.port = pon_anti_spoofing_info.pon_gport[i];
            rv = bcm_l3_source_bind_get(unit, &source_bind_get);
            if (i == 1)
            {
                exp_rv = BCM_E_NOT_FOUND;
            }

            if (rv != exp_rv)
            {
                rv = BCM_E_FAIL;
                printf("Error, bcm_l3_source_bind_get returned %s src_bind[%d] in gport:%x\n", bcm_errmsg(rv), i, pon_anti_spoofing_info.pon_gport[i]);
                return rv;
            }
            else
            {
...
int pon_anti_spoofing_app_clean(int unit)
{
    ...
    if (pon_anti_spoofing_info.grp.group)
    {
    ...
    for (i = 0; i < pon_gport_num; i++)
    {
        rv = vswitch_delete_port(unit, pon_anti_spoofing_info.vsi, pon_anti_spoofing_info.pon_gport[i]);
        if (rv != BCM_E_NONE) 
        {
...
int pon_anti_spoofing_with_dscp_remark_dvapi(int unit, int pon_port, int nni_port) {
    ...
    pon_anti_spoofing_info.pon_gport[0] = pon_gport = vlan_port.vlan_port_id;
...
int pon_anti_spoofing_with_dscp_remark_dvapi_clearn_up(int unit) {
    ...
    bcm_vlan_port_destroy(unit, pon_anti_spoofing_info.pon_gport[0]);
