 ...
 *              - ecmp_resolution - ECMP resolution.
 */
 ...
 *    uint32_bit_unpack
 * Purpose:
 *    Extracting a sequence of bits from a given uint32.
 * Parameters: 
 *    number_in_big_endian    -- uint32 value.
 *    bit_offset              -- the offset is counted from the MSB.
 *    num_bits                -- length of the bit sequence.
 * Returns:
 *  value in big-endian byte order
 * Notes:
 *  operation >> or << is based on value, don't worry endian here
 */
uint32 uint32_bit_unpack(uint32 number_in_big_endian, uint32 bit_offset, uint32 num_bits) {
    uint32 res = 0;

    if ((bit_offset + num_bits) > 32)
    {
        printf("integer_bit_unpack error, bit_offset + num_bits > 32\n");
    }
    res = (number_in_big_endian >> (32 - bit_offset - num_bits)) & ((1 << num_bits) - 1);
 ...
 *  bit_tobel
 * Purpose:
    Convert a 32-bit value from uint32 buffer to value in big-endian byte order.
 * Parameters:
 *  a - Start address of uint32 buffer
 * Returns:
 *  value in big-endian byte order
 * Notes:
 *
...
bit_tobel(uint32 *a)
{
    uint8 byte_stream[4] = {0};
    uint32 number_in_big_endian = 0;

    if (a == NULL)
    {
        return number_in_big_endian;
    }

    sal_memcpy(byte_stream, a, sizeof(byte_stream));
    number_in_big_endian  = byte_stream[0] << 24;
    number_in_big_endian |= byte_stream[1] << 16;
    number_in_big_endian |= byte_stream[2] << 8;
    number_in_big_endian |= byte_stream[3];
 ...
 *    bit_offset    -- the bit offset is counted from the MSB of header[0] (=SOP-Start Of Packet).
 *    num_bits      -- length of the bit sequence (<=32).
 */
uint32 bit_unpack(uint32 *header, uint32 bit_offset, uint32 num_bits) {
    uint32 header_index;
    uint32 modulo;
    uint32 res;
    uint32 tmp;
    ...
    if (header == NULL)
    {
       printf("bit_unpack error, header is NULL\n");
    }
    if (num_bits > 32)
    {
       printf("bit_unpack error, num_bits > 32\n");
    }

    header_index = (bit_offset / 32);
    modulo = bit_offset % 32;

    /** Convert the first uint32 to value in big endian */
    tmp = bit_tobel(header + header_index);

    if (modulo != 0)
    {
        res = uint32_bit_unpack(tmp, modulo, 32 - modulo);
        /** left shift and clear modulo bits */
        res = (res << modulo) & (((1 << 32) -1) - ((1 << modulo) - 1));
        /** Convert the next uint32 to value in big endian */
        tmp = bit_tobel(header + header_index + 1);
        res += uint32_bit_unpack(tmp, 0, modulo);
    }
    else
    {
        res = tmp;
    ...
    res = uint32_bit_unpack(res, 0, num_bits);
    return (res);
}
...
uint32 fer_lb_key(
   ...
   if (!is_device_or_above(unit,ARAD_PLUS)) {
	  printf("This cint is compatible with Arad+ only!");
   }
