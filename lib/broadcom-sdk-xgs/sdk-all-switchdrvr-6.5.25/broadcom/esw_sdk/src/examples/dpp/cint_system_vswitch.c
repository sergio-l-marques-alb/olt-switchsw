/*
 * $Id: cint_system_vswitch.c,v 1.15 Broadcom SDK $
...
system_vlan_port_lif_only_create(
    ...
    *gport = vp->vlan_port_id;  
    ...
    if (vsi > 0) {
        ...
        if (vp->failover_port_id) {
            bcm_vlan_port_t vp_get;
            bcm_vlan_port_t_init(&vp_get);
            vp_get.vlan_port_id = *gport;
            rv = bcm_vlan_port_find(unit, &vp_get);
            if (rv != BCM_E_NONE) {
                printf("Error, bcm_vlan_port_find\n");
                return rv;
            }
            if (vp_get.failover_port_id != vp->failover_port_id) {
                rv = BCM_E_FAIL;
                printf("Invalid failover_port_id in vlan port(0x%08x)getting, expected:0x%08x current:0x%08x in unit %d\n",*gport, vp_get.failover_port_id, vp.failover_port_id, unit);
                return rv;
            }
        }
