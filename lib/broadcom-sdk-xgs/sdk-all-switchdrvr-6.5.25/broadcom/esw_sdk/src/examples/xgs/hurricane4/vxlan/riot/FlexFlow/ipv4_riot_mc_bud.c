                 ...
                 int l3,bcm_multicast_t acc_mc_group,bcm_gport_t *acc_flow_port)
{
    ...
    if(l3){
        ...
        printf("Access: create OL L3 interface done, acc_intf_ol = %d\n", acc_intf_ol);
                     ...
                     bcm_flow_handle_t flow_handle)
{
    ...
    printf("Network: create UL L3 interface net_intf_ul = %d\n ", net_intf_ul);
    ...
    rv = vxlan_create_l3_mc_ingress(unit,0,vxlan_cfg->vrf,
    ...
    if (bud_mode) {
        ...
        rv = vxlan_create_l3_ingress(unit,0,vxlan_cfg->vrf,
                                     net_intf_ol,&net_ol_ing_obj, vxlan_net->net_ol_vlan);
        if (BCM_FAILURE(rv)) {
            printf("\nError in vxlan_create_l3_ingress: %s\n", bcm_errmsg(rv));
            return rv;
        }
