...
/*
 * Feature  : TD3(56870) Vxlan Bridging , IPv4 access to network tunnel initiation flow, validating MTU checks on network DVP port
 *
 * Usage    : BCM.0> cint ipv4_AtoN_MTU_check.c
 *
 * config   : config_td3_flex.bcm
 *
 * Log file : ipv4_AtoN_MTU_check_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
...
verify(int unit)
{
	...
	snprintf(str, 512, "tx 1 pbm=%d data=0x00000000AAAA00000000BBBB810000150800450000500001000040117497010101010202020200350035003C32F674657374746573747465737474657374746573747465737474657374746573747465737474657374746573747465737474657374112233445566778899112233445566778899; sleep quiet 1", ingress_port);
...
config_vxlan_network(int unit, vxlan_cfg_t *vxlan_cfg, vxlan_net_cfg_t *vxlan_net, bcm_gport_t *net_flow_port, bcm_flow_handle_t flow_handle)
{
	...
	peinfo.valid_elements = BCM_FLOW_PORT_ENCAP_PORT_VALID |
		BCM_FLOW_PORT_ENCAP_EGRESS_IF_VALID | BCM_FLOW_PORT_ENCAP_MTU_VALID;

        // DVP mtu check will be applied on the final frame length after the encapsulation
        peinfo.mtu = 160;
	rv = vxlan_flow_port_encap_set(unit, peinfo);
	if (BCM_FAILURE(rv)) {
