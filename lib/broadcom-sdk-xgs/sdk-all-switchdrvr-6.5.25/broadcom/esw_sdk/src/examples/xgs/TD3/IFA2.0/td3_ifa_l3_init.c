 ...
 *
 * Usage    : BCM.0> cint td3_ifa_l3_init.c
 *
 * config   : td3_ifa_init_transit_config.bcm
 *
 * Log file : td3_ifa_l3_init_log.txt
 *
 * Test Topology :
 *                      +-------------------+
 *                      |                   |
 ...
 * Summary:
 * ========
 * cint script to demonstrate IFA init for l3 flow
 *
 ...
 *     b) Do the necessary IFA init 2-pass configurations for l3 flow
...
{
    BCM_IF_ERROR_RETURN(bcm_port_control_set(unit, gport, bcmPortControlPacketProcssingPort, 1));
...
create_egr_obj(int unit, bcm_if_t l3_if, bcm_mac_t mac, bcm_gport_t gport, bcm_if_t *egr_if)
{
    ...
    l3_egr.flags2 = BCM_L3_FLOW_ONLY;
    l3_egr.intf = l3_if;
    l3_egr.port = gport;
...
int
route_config(int unit, bcm_gport_t gport)
{
    bcm_error_t rv;
    ...
    rv = create_egr_obj(unit, intf_id_network_ingress, remote_mac_network_ingress, gport, &egr_obj_network_ingress);
    if (BCM_FAILURE(rv)) {
...
int test_setup(int unit)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_port_gport_get(unit, egress_port, gport1));
...
int test_verify(int unit)
{
    ...
    snprintf(str, 512, "tx 1 pbm=%d  data=00000000BBBB00000000AAAA8100006408004500006A000000000B11976F0B0B01000A0A02001F001F690056D8CD000102030405060708090A0B0C0D0E0F101112131415161718191A1B1C1D1E1F202122232425262728292A2B2C2D2E2F303132333435363738393A3B3C3D3E3F404142434445464748494A4B4C4D;sleep 5", ingress_port);
    printf("%s\n", str);
    bshell(unit, str);
