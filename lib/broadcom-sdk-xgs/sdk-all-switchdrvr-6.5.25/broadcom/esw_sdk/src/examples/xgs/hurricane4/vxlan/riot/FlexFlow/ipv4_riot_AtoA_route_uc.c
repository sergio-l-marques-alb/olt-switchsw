...
/*
 * Feature  : Vxlan Riot, IPv4 access to access unicast flow, L3 routing is based on route table entry(L3_DEEFIP)
 *
 * Usage    : BCM.0> cint ipv4_riot_AtoA_route_uc.c
 *
 * config   : BCM56275_A1-RIOT-FLEXFLOW.bcm
 *
 * Log file : ipv4_riot_AtoA_route_uc_log.txt
 *
 * Test Topology :
 *
 *
 *        +----+                       +---------+                   +---+
 *        |PC1 | ---> Access VFI 1     |  switch |  Access VFI 2 --> |PC2|
 *        +----+                       +---------+                   +---+
                     ...
                     bcm_flow_handle_t flow_handle)
{
    ...
    bcm_if_t      net_intf_ol = vxlan_net->net_ol_vlan;
    ...
    rv = create_l3_interface(unit,BCM_L3_WITH_ID,vxlan_net->net_ol_mac,vxlan_net->net_vpn,
                             &net_intf_ol);
    if (BCM_FAILURE(rv)) {
        printf("\nError in create_l3_interface: %s\n", bcm_errmsg(rv));
        return rv;
    }
    printf("Network: create OL L3 interface, net_intf_ol =%d\n", net_intf_ol);
    ...
    rv = vxlan_create_l3_ingress(unit,0,vxlan_cfg->vrf,
                                 net_intf_ol,&net_ing_obj, vxlan_net->net_vpn);
    if (BCM_FAILURE(rv)) {
        printf("\nError in vxlan_create_l3_ingress: %s\n", bcm_errmsg(rv));
        return rv;
    }
...
config_vxlan_ipv4_vtep(int unit)
{
    ...
    vxlan_acc_cfg_t vxlan_acc = {
        ...
        10,                      /*access vlan*/
        "00:00:00:22:22:22",     /*src mac*/
        "00:00:00:11:11:11",     /*dst mac*/
        0,                       /*access vpn*/
