...
/*
 * Feature  : HR4 Vxlan Bridging, IPv4 network to access tunnel termination flow, Validating MTU checks on access DVP port
 *
 * Usage    : BCM.0> cint ipv4_NtoA_MTU_check.c
 *
 * config   : BCM56277_A1-PORT-FLEXFLOW.bcm
 *
 * Log file : ipv4_NtoA_MTU_check_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
...
verify(int unit)
{
    ...
    snprintf(str, 1184, "tx 1 pbm=%d data=; sleep quiet 1", ingress_port);
    bshell(unit, str);
    bshell(unit, "show c");
    bshell(unit, "l2 show");

    print("\nsending the larger size encap packet to validate programmed MTU size(110Byte) on access port and we expect packet will be dropped on egress port with TL2_MTU as a tx drop reason\n");
        snprintf(str1, 64, "cstat set %d tx 9 TL2_MTU; sleep quiet 1", egress_port);
        snprintf(str2, 64, "cstat %d; sleep quiet 1", egress_port);
    snprintf(str, 1184, "tx 1 pbm=%d data=; sleep quiet 1", ingress_port);
...
config_vxlan_access(int unit, vxlan_cfg_t *vxlan_cfg, vxlan_acc_cfg_t *vxlan_acc, bcm_gport_t *acc_flow_port)
{
    ...
    peinfo.valid_elements |= (BCM_FLOW_PORT_ENCAP_FLAGS_VALID |
            ...
            BCM_FLOW_PORT_ENCAP_VLAN_VALID | BCM_FLOW_PORT_ENCAP_MTU_VALID);
    ...
    /* DVP(access port) mtu check will be applied on the final frame length after the decapsulation */
    peinfo.mtu = 110;
