...
/*
 * Feature  : UTT Mode B configuration
 *
 * Usage    : BCM.0> cint utt_mode_b_configuration.c
 *
 * config   : utt_mode_b_config.bcm
 *
 * Log file : utt_mode_b_configuration_log.txt
 *
 * Test Topology :
 *
 *                  +------------------------------+
 ...
 *   BCM APIs. Also, it just shows that how UTT modes are configured based on
 *   config property variables - lpm_mem_size, ifp_mem_size, ifp_lookup_depth
 *   Mode B: IFP TCAMs - 54, LPM TCAMS - 40
 *
 ...
 *        route in LPM table and does the necessary configurations of vlan,
 ...
 *        Also, dump the UTT related tables to check the configurations as it's
 *        nothing to do with functionality!
 *        UTT_BANK_DECODE, IFP_POLICY_UTT_ADDR_XLATE, L3_DEFIP_DATA_UTT_ADDR_XLATE
 *        and L3_DEFIP_PAIR_128_HIT_UTT_ADDR_XLATE
 *
 *        Please note, we have done very simple LPM route entry functionality
...
verify(int unit)
{
    ...
    bshell(unit, "config show ifp");
    bshell(unit, "config show lpm");

    bshell(unit, "sleep 2");
    print "Dump the UTT_BANK_DECODE, this provides the Bank decode for UTT TCAMs!";
    bshell(unit, "d chg UTT_BANK_DECODE");

    bshell(unit, "sleep 2");
    print "Dump the IFP_POLICY_UTT_ADDR_XLATE, this provides the Adddress be forwarded to Policy Table!";
    bshell(unit, "d chg IFP_POLICY_UTT_ADDR_XLATE");

    bshell(unit, "sleep 2");
    print "Dump the L3_DEFIP_DATA_UTT_ADDR_XLATE, this provides the Adddress be forwarded to Policy Tables!";
    bshell(unit, "d chg L3_DEFIP_DATA_UTT_ADDR_XLATE");

    bshell(unit, "sleep 2");
    print "Dump the L3_DEFIP_PAIR_128_HIT_UTT_ADDR_XLATE, this provides the Adddress be forwarded to Policy Tables!";
    bshell(unit, "d chg L3_DEFIP_PAIR_128_HIT_UTT_ADDR_XLATE");
