 ...
 *                        Underlay forming an ECMP group.
 *
 * Usage    : BCM.0> cint ipv4_riot_AtoN_uc_ECMP_UL.c
 *
 * config   : config_fb6_flex_riot_ecmp.bcm
 *
 * Log file : ipv4_riot_AtoN_uc_ECMP_UL_log.txt
 *
 * Test Topology :
 *            ACCESS                             NETWORK
 *                                                                    DMAC=00::aa:aa vlan=11
 *                        +----------------+                          SMAC=00::bb:bb
 *                        |   Firebolt6    |                         +---------->  egress_port[0]
 *                        |                |                         |
 *                        |                |                         | DMAC=00::aa:ab vlan=12
 *                        |                | IP=192.168.20.0/24      | SMAC=00::bb:bc
 *              +--------->                +------------------------------------>  egress_port[1]
 *            ingress_port|                |                     .   |
 *                        |                | DMAC=00::02:02:02   .   | DMAC=00::aa:ac vlan=13
 *      +------>          |                | SMAC=00::01:01:01   .   | SMAC=00::bb:bd
 * DMAC=00::22:22:22      |                |     VLAN=14         .   +---------->  egress_port[2]
 * SMAC=00::11:11:11      |                |                     .
 * VLAN=10                |                |                 Tunnel SIP=10.10.10.1
 *                        |                |                 Tunnel DIP=20.20.20.1
 * SIP=192.168.10.X       |                |
 * DIP=192.168.20.1       |                |                              Underlay
 *                        |                |                               ECMP
 *                        |                |
 *                        |                |
 *                        |                |
...
Ingress Packets: (No. of packets = 3)
======
...
Header Info
  ...
  ###[ IP ]###
        version= 4
        proto= udp
        src= 192.168.10.1, 192.168.10.3, 192.168.10.4 <== Changing contents in 3 packets
        dst= 192.168.20.1
  ###[ UDP ]###
                     ...
                     bcm_flow_handle_t flow_handle)
{
    ...
    rv = vxlan_create_egr_obj(unit, 0, net_intf_ol, vxlan_net->net_ol_nh_mac,
                    vxlan_net->net_gport, vxlan_net->net_vpn, &net_egr_obj_ol);
    if (BCM_FAILURE(rv)) {
        printf("\nError in configuring egress obj: %s.\n", bcm_errmsg(rv));
        return rv;
    }
    ...
    rv = l3_route_add(unit,0,vxlan_acc->acc_dest_ip,mask,vxlan_cfg->vrf,
                        net_egr_obj_ol);
    if (BCM_FAILURE(rv)) {
        printf("\nError in l3_route_add: %s\n", bcm_errmsg(rv));
        return rv;
