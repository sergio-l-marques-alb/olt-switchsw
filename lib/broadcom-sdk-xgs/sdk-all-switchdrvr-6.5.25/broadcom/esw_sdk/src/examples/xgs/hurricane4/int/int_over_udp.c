 ...
 *  
 *  config   : BCM56275_A1-PORT-INT.bcm
 *  
 ...
 * 1) Step1 - Test Setup
 * ============================================
 * a) Connect xe0 and xe1 with traffic generator.
 * b) Make sure xe0 and xe1 link up with traffic generator.
 * 
 * 2) Step2 - Configuration
 * ============================================
 * ./bcm.user
 * config add flow_init_mode=1
 * rc
 * c int_over_udp.c
 * 
...
bcm_vrf_t vrf = 10;
bcm_vrf_t vrf_ta = 20;
bcm_port_t port_ta = 0x41;   /* loopback port */
...
/* VRF initialized to 10 & 20, check if device supports VRF
  if not, change it to vrf 0 */
print bcm_l3_info(unit, &l3_info_get);
print l3_info_get;
...
bcm_error_t ConfigureINT(int unit)
{ 
...
bcm_error_t execute()
{
	...
	if( (rrv = ConfigureINT(unit)) != BCM_E_NONE )
	{
