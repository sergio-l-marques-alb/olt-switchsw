/*
 * Feature  : vxlan acc to acc mc/bc/dlf
 *
 * Usage    : cint acc_to_acc_mc.c
 *
 * config   : BCM56275_A1-BRIDGE-LEGACY.bcm
 *
 * Log file : acc_to_acc_mc_log.txt
 * Feature  : VxLAN access_to_access non-uc script
 *
 * Test Topology :
 *
 *                  +------------------------------+
 ...
 *
 *    2) Step2 - Configuration (Done in vxlan_access_to_access_multicast())
 *    =============================================================================
 ...
 *       
 *       BCM.0> l2 show
 *       mac=01:00:5e:01:01:14 vlan=28688 GPORT=0x0 modid=0 port=0/cpu0 Static CPU MCast=201326594
 *       
 *       --> Packet in access port 1(the script will send the pkt automatically):
 *       
 ...
 *       
 *       0000 0000 0003 0000 0000 0001 8100 000b
 ...
 *       --> Packet in access port 1(the script will send the pkt automatically)::
 *       
 *       01005E0101140000000000018100000A
 *       08004500002E0000000040FF84B10A0A
 *       0A01E001011400010203040506070809
 *       0A0B0C0D0E0F10111213141516171819
 *       FCE77104
 *       
 *       Expected Result:
 *       Captured on Out access port 2:
 *       
 *       0100 5e01 0114 0000 0000 0001 8100 000b
 *       0800 4500 002e 0000 0000 40ff 84b1 0a0a 
 *       0a01 e001 0114 0001 0203 0405 0607 0809 
 *       0a0b 0c0d 0e0f 1011 1213 1415 1617 1819 
 *       efc8 298b
 *       
 *       Captured on access port 3:
 *       
 *       0100 5e01 0114 0000 0000 0001 8100 000c
 *       0800 4500 002e 0000 0000 40ff 84b1 0a0a 
 *       0a01 e001 0114 0001 0203 0405 0607 0809 
 *       0a0b 0c0d 0e0f 1011 1213 1415 1617 1819 
 *       1409 4391
...
bcm_error_t vxlan_access_to_access_multicast(int unit)
{
  ...
  bcm_mac_t known_mc_mac = "01:00:5e:01:01:14";

  /* Multicast group for known MC */
  ...
  rv = bcm_multicast_vxlan_encap_get(unit, known_mc_group, access_gport_1, vxlan_port_a, &encap_a);
  if (BCM_E_NONE != rv)
  {
      printf("bcm_multicast_vxlan_encap_get vxlan_port_a failed %s\n", bcm_errmsg(rv));		
      return rv;	
  }

  rv = bcm_multicast_vxlan_encap_get(unit, known_mc_group, access_gport_2, vxlan_port_b, &encap_b);
  if (BCM_E_NONE != rv)
  {
      printf("bcm_multicast_vxlan_encap_get vxlan_port_a failed %s\n", bcm_errmsg(rv));		
      return rv;	
  ...
  rv = bcm_multicast_vxlan_encap_get(unit, known_mc_group, access_gport_3, vxlan_port_c, &encap_c);
  if (BCM_E_NONE != rv)
  {
      printf("bcm_multicast_vxlan_encap_get vxlan_port_a failed %s\n", bcm_errmsg(rv));		
      return rv;	
  ...
  rv = bcm_multicast_egress_add(unit, known_mc_group, vxlan_port_a, encap_a);
  if (BCM_E_NONE != rv)
  {
      printf("bcm_multicast_egress_add vxlan_port_a failed %s\n", bcm_errmsg(rv));		
      return rv;	
  ...
  rv = bcm_multicast_egress_add(unit, known_mc_group, vxlan_port_b, encap_b);
  if (BCM_E_NONE != rv)
  {
      printf("bcm_multicast_egress_add vxlan_port_b failed %s\n", bcm_errmsg(rv));		
      return rv;	
  ...
  rv = bcm_multicast_egress_add(unit, known_mc_group, vxlan_port_c, encap_c);
  if (BCM_E_NONE != rv)
  {
      printf("bcm_multicast_egress_add vxlan_port_c failed %s\n", bcm_errmsg(rv));		
      return rv;	
  ...
  printf("vxlan_access_to_access_multicast SUCCESS \n"); 	
...
void verify(int unit)
{
	...
	*	   
	*/ 
	/* sent known mc packet */  
	snprintf(str, 512, "tx 1 pbm=%d DATA=0x01005E0101140000000000018100000A08004500002E0000000040FF84B10A0A0A01E0010114000102030405060708090A0B0C0D0E0F10111213141516171819", access_port_1);
	bshell(unit, str);		
	bshell(0, "sleep 1");
	bshell(unit, "l2 show");
	bshell(unit, "show c");
...
bcm_error_t execute()
{
  ...
  if (BCM_FAILURE((rv = vxlan_access_to_access_multicast(unit)))) {
