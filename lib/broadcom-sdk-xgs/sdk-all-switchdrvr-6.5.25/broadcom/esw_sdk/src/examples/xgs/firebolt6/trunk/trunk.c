...
int dlb_trunk_create(int unit, int *port, int nport, bcm_trunk_t *tid)
{
    ...
    *tid = 1025;
    ...
    for (i = 0; i < nport; i++) {
        ...
        p_trunk_member[i].dynamic_load_weight = 1;
        p_trunk_member[i].dynamic_scaling_factor = 100; // For 100G port
    }
    ...
    if (BCM_FAILURE(rv)) {
        printf ("failed, error code: %s\n", bcm_errmsg(rv));
        printf(">>> Please use the correct config where HG ports are 100G <<<<\n");
        if( rv == BCM_E_UNAVAIL ) {
            printf(">>> Ensure that config has dlb_hgt_lag_selection=1 <<<<\n");
        }
        return rv;
    }
    printf("Done \n");
