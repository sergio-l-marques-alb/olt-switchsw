...
/*
 * Feature  : TD3(56870) Vxlan Bridging , IPv4 access to network tunnel initiation Flow with outer/inner vlan as a flow match criteria(BCM_FLOW_MATCH_CRITERIA_PORT_VLAN_STACKED)
 *
 * Usage    : BCM.0> cint ipv4_AtoN_MatchPortOuterInnerVlan.c
 *
 * config   : BCM56275_A1-BRIDGE-FLEXFLOW.bcm 
 *
 * Log file : ipv4_AtoN_MatchPortOuterInnerVlan_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
...
verify(int unit)
{
	...
	printf("setting 0x9100 as a inner tpid on access port from its default value 0x8100\n");
	rv = bcm_port_inner_tpid_set(0, ingress_port, 0x9100);
	if (BCM_FAILURE(rv)) {
                printf("\nError in bcm_port_inner_tpid_set(): %s.\n", bcm_errmsg(rv));
                return rv;
        }

	printf("\nSending known unicast packet to ingress_port access side:%d\n", ingress_port);
	snprintf(str, 512, "tx 1 pbm=%d data=0x00000000AAAA00000000BBBB81000015910000640800450000500001000040117497010101010202020200350035003C32F674657374746573747465737474657374746573747465737474657374746573747465737474657374746573747465737474657374; sleep quiet 1", ingress_port);
	bshell(unit, str);
	bshell(unit, "show c");
...
vxlan_access_port_config(int unit, bcm_gport_t gport_acc)
{
	...
	BCM_IF_ERROR_RETURN(bcm_vlan_control_port_set(unit, ingress_port, bcmVlanPortTranslateKeyFirst, bcmVlanTranslateKeyPortDouble));
	return BCM_E_NONE;
}
...
config_vxlan_access(int unit, vxlan_cfg_t *vxlan_cfg, vxlan_acc_cfg_t *vxlan_acc, bcm_gport_t *acc_flow_port)
{
	...
	printf("Access: Add flow match (match: port, outer/inner vlan criteria).\n");
	bcm_flow_match_config_t_init(&match_info);
	match_info.criteria  = BCM_FLOW_MATCH_CRITERIA_PORT_VLAN_STACKED;
	match_info.port      = vxlan_acc->phy_Gport;
	match_info.vlan      = vxlan_acc->acc_vlan;
	match_info.inner_vlan = 0x64;
