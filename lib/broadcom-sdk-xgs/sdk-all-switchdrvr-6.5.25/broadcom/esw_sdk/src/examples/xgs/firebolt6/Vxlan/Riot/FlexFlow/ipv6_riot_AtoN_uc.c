...
/*
 * Feature  : FB6(56470) Vxlan Riot, IPv6 access to network tunnel initiation flow
 *
 * Usage    : BCM.0> cint ipv6_riot_AtoN_uc.c
 *
 * config   : config_fb6_flex_riot.bcm
 *
 * Log file : ipv6_riot_AtoN_uc_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
...
Egress Packet:
=============
    0000 0000 aaaa 0000 0000 bbbb 8100 000b
    86dd 6240 0000 0050 113f 0101 0101 3500
    7000 0000 1009 0807 0605 0202 0202 3600
    7000 0000 0102 0304 0506 ffff 2118 0050
    0000 0000 0000 0123 4500 0000 0002 0202
    0000 0001 0101 0800 4500 002e 0000 0000
    3fff db7e c0a8 0a01 c0a8 1401 0000 0000
    0000 0000 0000 0000 0000 0000 0000 0000
    0000 0000 0000 0000 0000 fd9b a5d2
...
/* Create L3 interface */
bcm_error_t
create_l3_interface(int unit, uint32 flags, bcm_mac_t local_mac, int vid, bcm_flow_handle_t flow, bcm_flow_option_id_t flow_option,
                     bcm_if_t *intf_id)
{
...
vxlan_create_egr_obj(int unit, uint32 flag, int l3_if, bcm_mac_t nh_mac,
                     bcm_gport_t gport, int vid, bcm_flow_handle_t flow,bcm_flow_option_id_t flow_option, bcm_if_t *egr_obj_id)
{
                    ...
                    bcm_gport_t *acc_flow_port)
{
    ...
    rv = vxlan_create_egr_obj(unit,BCM_L3_FLOW_ONLY,BCM_IF_INVALID,
                        dummy_mac_addr,vxlan_acc->phy_Gport,vxlan_acc->acc_vlan,0,0,
                              &acc_egr_obj_ul);
    if (BCM_FAILURE(rv)) {
    ...
    rv = create_l3_interface(unit,0,vxlan_acc->acc_ol_mac, vxlan_acc->acc_vpn,0,0,
                            &acc_intf_ol);
    if (BCM_FAILURE(rv)) {
    ...
    rv = vxlan_create_egr_obj(unit,0,acc_intf_ol,vxlan_acc->acc_ol_nh_mac,
                      vxlan_acc->acc_gport,vxlan_acc->acc_vlan, 0, 0, &acc_egr_obj_ol);
    if (BCM_FAILURE(rv)) {
        printf("\nError in vxlan_create_egr_obj: %s\n", bcm_errmsg(rv));
        return rv;
    }
    ...
    rv = bcm_port_control_set(unit, *acc_flow_port, bcmPortControlIP6, TRUE);
    if (BCM_FAILURE(rv)) {
        printf("Error in bcm_port_control_set: %s\n", bcm_errmsg(rv));
        return rv;
    }
...

{
    ...
    rv = create_l3_interface(unit,0,vxlan_net->net_ul_mac,
                             vxlan_net->net_ul_vlan, vxlanV6FlowInfo.flow_handle,vxlanV6FlowInfo.intf_flow_option, &net_intf_ul);
    if (BCM_FAILURE(rv)) {
        printf("\nError in create_l3_interface: %s\n", bcm_errmsg(rv));
        return rv;
    }

    vxlan_net->intf_id=net_intf_ul;
    ...
    rv = vxlan_create_egr_obj(unit,BCM_L3_FLOW_ONLY,net_intf_ul,
                              ...
                              vxlan_net->net_ul_vlan, vxlanV6FlowInfo.flow_handle,vxlanV6FlowInfo.egr_flow_option, &net_egr_obj_ul);
    if (BCM_FAILURE(rv)) {
        printf("\nError in vxlan_create_egr_obj: %s\n", bcm_errmsg(rv));
        return rv;
    }

    vxlan_net->egr_id=net_egr_obj_ul;
    ...
    rv = create_l3_interface(unit,0,vxlan_net->net_ol_mac,vxlan_net->net_vpn,0,0,
                             &net_intf_ol);
    if (BCM_FAILURE(rv)) {
        printf("\nError in create_l3_interface: %s\n", bcm_errmsg(rv));
        return rv;
    ...
    rv = vxlan_create_egr_obj(unit,0,net_intf_ol,vxlan_net->net_ol_nh_mac,
                              vxlan_net->net_gport,vxlan_net->net_vpn,0,0, &net_egr_obj_ol);
    if (BCM_FAILURE(rv)) {
        printf("\nError in vxlan_create_egr_obj: %s\n", bcm_errmsg(rv));
        return rv;
    }
    ...
    rv = bcm_port_control_set(unit, *net_flow_port, bcmPortControlIP6, TRUE);
    if (BCM_FAILURE(rv)) {
        printf("Error in bcm_port_control_set: %s\n", bcm_errmsg(rv));
        return rv;
    }
    ...
    einfo.flow_port  = vxlan_net->net_gport;
    ...
    peinfo.flow_port = vxlan_net->net_gport;
...
execute(void)
{
    ...
    print "~~~ #2) config_vxlan_ipv6_vtep(): ** start **";
