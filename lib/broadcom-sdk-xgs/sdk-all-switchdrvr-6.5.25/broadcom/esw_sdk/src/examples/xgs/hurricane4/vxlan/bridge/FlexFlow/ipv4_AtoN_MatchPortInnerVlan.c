...
/*
 * Feature  : TD3(56870) Vxlan Bridging, IPv4 access to network tunnel initiation Flow with inner vlan as a flow match criteria(BCM_FLOW_MATCH_CRITERIA_PORT_INNER_VLAN)
 *
 * Usage    : BCM.0> cint ipv4_AtoN_MatchPortInnerVlan.c
 *
 * config   : BCM56275_A1-BRIDGE-FLEXFLOW.bcm 
 *
 * Log file : ipv4_AtoN_MatchPortInnerVlan_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
...
verify(int unit)
{
	...
	snprintf(str, 512, "tx 1 pbm=%d data=0x00000000AAAA00000000BBBB81000015910000050800450000500001000040117497010101010202020200350035003C32F674657374746573747465737474657374746573747465737474657374746573747465737474657374746573747465737474657374; sleep quiet 1", ingress_port);
	bshell(unit, str);
	bshell(unit, "show c");
...
config_vxlan_access(int unit, vxlan_cfg_t *vxlan_cfg, vxlan_acc_cfg_t *vxlan_acc, bcm_gport_t *acc_flow_port)
{
	...
	printf("Access: Add flow match (match: inner vlan criteria).\n");
	bcm_flow_match_config_t_init(&match_info);
	match_info.criteria  = BCM_FLOW_MATCH_CRITERIA_PORT_INNER_VLAN;
	match_info.port      = vxlan_acc->phy_Gport;
	match_info.vlan      = vxlan_acc->acc_vlan;
	match_info.inner_vlan = 0x5;
