...
/*
 * Feature  : Vxlan Riot, IPv4 network to access tunnel termination flow,
 *            igmp packet as a payload in vxlan packet
 *
 * Usage    : BCM.0> cint ipv4_riot_NtoA_igmp_protocol.c
 *
 * config   : config_fb6_flex_riot.bcm
 *
 * Log file : ipv4_riot_NtoA_igmp_protocol_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
...
NETWORK TO ACCESS:-
Decap-case: packet sent from network port to access
...
======
    ...
    0000 0123 4500 0100 5e00 0001 0000 0000
    0400 0800 4500 001c 0000 0000 4002 5cc3
    1400 0014 0a00 000a 1164 e491 0a00 000a
    0001 0203 0405 0607 0809 0a0b 0c0d 0e0f
    1011 aa6f d0f9 4f33 5e69
...
======
    ...
    [bcmPW.0]Packet[1]: data[0030]: 0000 0123 4500 0100 5e00 0001 0000 0000
    [bcmPW.0]Packet[1]: data[0040]: 0400 0800 4500 001c 0000 0000 4002 5cc3
    [bcmPW.0]Packet[1]: data[0050]: 1400 0014 0a00 000a 1164 e491 0a00 000a
    [bcmPW.0]Packet[1]: data[0060]: 0001 0203 0405 0607 0809 0a0b 0c0d 0e0f
    [bcmPW.0]Packet[1]: data[0070]: 1011 aa6f d0f9 4f33 5e69
    [bcmPW.0]Packet[1]: length 122 (122). rx-port 5. cos 0. prio_int 0. vlan 11. reason 0x4001. Outer tagged.
    [bcmPW.0]Packet[1]: dest-port 0. dest-mod 0. src-gport -1342177278. opcode 0.  matched 0. classification-tag 0.
    [bcmPW.0]Packet[1]: reasons: FilterMatch
    [bcmPW.0]Packet[1]: reasons: Protocol   ====> copied to cpu
...
verify(int unit)
{
    ...
    bshell(unit, "clear c");
    printf("\nSending vxlan packet with unknown igmp payload on ingress network side:%d\n",
                                                                ingress_port);
    snprintf(str, 512, "tx 1 pbm=%d data=0x00000000bbbb00000000aaaa8100000b080045000064000000003f113f6a141414010a0a0a01ffff2118003a0000080000000123450001005E00000100000000040008004500001C0000000040025CC3140000140A00000A1164E4910A00000A000102030405060708090A0B0C0D0E0F1011AA6FD0F9; sleep quiet 1", ingress_port);
    bshell(unit, str);
    bshell(unit, "show c");
...
create_vxlan_vpn(int unit,bcm_multicast_t *vxlan_mc_group,bcm_vpn_t *vxlan_vpn)
{
    ...
    vpn_info.protocol_pkt.arp_request_action = BCM_VLAN_PROTO_PKT_TOCPU_ENABLE |
                                             BCM_VLAN_PROTO_PKT_DROP_ENABLE;
    vpn_info.protocol_pkt.igmp_report_leave_action = BCM_VLAN_PROTO_PKT_TOCPU_ENABLE |
                                             BCM_VLAN_PROTO_PKT_DROP_ENABLE;
    vpn_info.protocol_pkt.igmp_query_action = BCM_VLAN_PROTO_PKT_TOCPU_ENABLE |
                                             BCM_VLAN_PROTO_PKT_DROP_ENABLE;
    vpn_info.protocol_pkt.igmp_unknown_msg_action = BCM_VLAN_PROTO_PKT_TOCPU_ENABLE |
                                             BCM_VLAN_PROTO_PKT_DROP_ENABLE;
...
/*
    printf("Network: overlay l2 station entry for termination\n");
    rv = l2_station_add(unit,vxlan_net->net_ol_mac,vxlan_net->net_vpn,0,
                         BCM_L2_STATION_IPV4_MCAST | BCM_L2_STATION_IPV4,
                        &l2_station_net_ol);
    if (BCM_FAILURE(rv)) {
        printf("\nError in l2_station_add: %s\n", bcm_errmsg(rv));
        return rv;
...
config_vxlan_ipv4_vtep(int unit)
{
    ...
    vxlan_net_cfg_t vxlan_net = {
        ...
        "00:00:00:01:01:01",     /*overlay payload dst mac*/
        "00:00:00:02:02:02",     /*overlay payload src mac*/
        11,                      /*tunnel vid*/
    ...
    printf("Setup switch control for ARP termination\n");
    BCM_IF_ERROR_RETURN(bcm_switch_control_set(unit,
                                           bcmSwitchIngParseL2TunnelTermArp,1));
    BCM_IF_ERROR_RETURN(bcm_switch_control_set(unit,
                                          bcmSwitchIngParseL2TunnelTermRarp,1));
