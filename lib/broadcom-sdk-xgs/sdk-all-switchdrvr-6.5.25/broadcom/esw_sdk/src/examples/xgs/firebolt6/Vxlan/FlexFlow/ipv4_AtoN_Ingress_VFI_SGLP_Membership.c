...
/*
 * Feature  : FB6(56470) Vxlan Bridging , IPv4 access to network tunnel initiation Flow, Verifying Port membership check based on VFI (VFI,SGLP).
 *
 * Usage    : BCM.0> cint ipv4_AtoN_Ingress_VFI_SGLP_Membership.c
 *
 * config   : config_fb6_flex.bcm
 *
 * Log file : ipv4_AtoN_Ingress_VFI_SGLP_Membership_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
...
verify(int unit)
{
	...
	printf("\n\n Enabling membership check on ingress port and we will see packet get dropped on ingress port with reason as RxVlanMemberMismatch\n");
	if (BCM_E_NONE != bcm_port_vlan_member_set(unit, ingress_phy_gport, BCM_PORT_VLAN_MEMBER_INGRESS) ) {
    	    printf("bcm_port_vlan_member_set() failed\n");
	    return -1;
	}

	snprintf(str1, 64, "cstat set %d rx 7 RxVlanMemberMismatch; sleep quiet 1", ingress_port);
	snprintf(str2, 64, "cstat %d; sleep quiet 1", ingress_port);

	bshell(unit, str1);
	bshell(unit, str);
	bshell(unit, str2);

	printf("\n\n Now Adding ingress port for VFI based membership check and packet get egress on network port\n");
        if (BCM_E_NONE != bcm_vlan_gport_add(unit, vpn, ingress_phy_gport, BCM_VLAN_GPORT_ADD_INGRESS_ONLY) ) {
	    prinf("bcm_vlan_gport_add() failed \n");
	    return -1;
	}
	bshell(unit, str);
	bshell(unit, "show c");
