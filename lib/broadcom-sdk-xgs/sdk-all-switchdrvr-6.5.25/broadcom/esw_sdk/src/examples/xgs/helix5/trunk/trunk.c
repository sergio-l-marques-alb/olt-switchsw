 ...
 *   1. Create a trunk group with two members
 ...
 *
 *  NOTE: UDF hashing is not supported in Maverick2
 */
int trunk_udf_hash_enable(int unit)
{
    ...
    if( 0 ) // UDF hashing is not supported in Maverick2
    {
...
bcm_error_t execute()
{
    ...
    if( opt_TestTypeDlb )
    {
        if( (rv = example_run_dlb()) != BCM_E_NONE )
        {
    ...
    {
        if( (rv = example_run_trunk()) != BCM_E_NONE )
        {
