...
/*
 * Feature  : FB6(56470) Vxlan Bridging , IPv4 access to network tunnel initiation Flow, Verifying SVP membership check based on VFI (VFI,VP).
 *
 * Usage    : BCM.0> cint ipv4_AtoN_Ingress_VFI_VP_Membership.c
 *
 * config   : config_fb6_flex.bcm
 *
 * Log file : ipv4_AtoN_Ingress_VFI_VP_Membership_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
...
verify(int unit)
{
	...
	printf("\n\n Enabling membership check on SVP and we will see packet get dropped on ingress port with reason as RxVlanMemberMismatch\n");
	if (BCM_E_NONE != bcm_port_vlan_member_set(unit, acc_vp, BCM_PORT_VLAN_MEMBER_INGRESS) ) {
	...
	printf("\n\n Now Adding SVP for VFI based membership check and packet get egress on network port\n");
        if (BCM_E_NONE != bcm_vlan_gport_add(unit, vpn, acc_vp, BCM_VLAN_GPORT_ADD_INGRESS_ONLY) ) {
