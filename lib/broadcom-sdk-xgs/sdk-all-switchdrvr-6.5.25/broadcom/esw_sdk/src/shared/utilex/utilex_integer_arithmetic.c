    ...
    uint32 *log2_round_val)
{
    ...
    if (utilex_log2_round_up(x) < SAL_UINT32_NOF_BITS)
    {
        ...
        if (average_val < x)
        {
            *log2_round_val = utilex_log2_round_up(x);
        }
        else
        {
            *log2_round_val = utilex_log2_round_down(x);
        }
    }
    else
    {
        /**
         * Round-up value is going out of bounds of UINT32
         */
        rv = _SHR_E_PARAM;
    }
    return rv;
    ...
    uint32 len)
{
    ...
    UTILEX_64_UDIV_32(accum_u64, len);
    ...
    uint32 *x)
{
    ...
    COMPILER_UINT64 tmp_result, result;
    ...
    /*
     * eq_const_multi * (mnt + * eq_const_mnt_inc) 
     */
    COMPILER_64_SET(tmp_result, 0, eq_const_multi);
    COMPILER_64_UMUL_32(tmp_result, (mnt + eq_const_mnt_inc));

    COMPILER_64_COPY(result, tmp_result);
    UTILEX_64_UDIV_32(result, (1 << exp));

    if (COMPILER_64_HI(result)) /* overflow */
    {
        *x = 0xFFFFFFFF;
        ex = _SHR_E_PARAM;
        goto exit;
...
{
    shr_error_e ex = _SHR_E_NONE;
    COMPILER_UINT64 tmp_result, tmp_result_2, result;
    uint32 tmp_res;
    ...
    /*
     * eq_const_multi * 2^exp 
     */
    COMPILER_64_SET(tmp_result, 0, eq_const_multi);
    COMPILER_64_UMUL_32(tmp_result, (1 << exp));

    /*
     * check if the multiply result is > uint32_max 
     */
    tmp_res = COMPILER_64_LO(tmp_result);
    if (COMPILER_64_HI(tmp_result))
    {
        /*
         * Enter for overflow case
         */
        if (round_to_closest)
        {
            /*
             * assumption: (1 << * rev_exp) * * eq_const_div < 2^32 
             */
            COMPILER_64_ADD_32(tmp_result, (((1 << rev_exp) * eq_const_div) / 2));
        }
        /*
         * divide first and then continue multiplying 
         */
        /*
         * tmp_result / (2^rev_exp * eq_const_div) 
         */
        COMPILER_64_COPY(tmp_result_2, tmp_result);
        UTILEX_64_UDIV_32(tmp_result_2, (1 << rev_exp) * eq_const_div);

        tmp_res = COMPILER_64_LO(tmp_result_2);
        if (COMPILER_64_HI(tmp_result_2))
        {
            /*
             * Enter in case of overflow.
             */
            ex = _SHR_E_PARAM;
            goto exit;
        }
        ...
        /*
         * tmp_res * (mnt + eq_const_mnt_inc) 
         */
        COMPILER_64_SET(result, 0, tmp_res);
        COMPILER_64_UMUL_32(result, (mnt + eq_const_mnt_inc));
    }
    else
    {
        /*
         ...
         * tmp_res * (mnt + * eq_const_mnt_inc) 
         */
        COMPILER_64_SET(tmp_result, 0, tmp_res);
        COMPILER_64_UMUL_32(tmp_result, (mnt + eq_const_mnt_inc));
         ...
         */
        COMPILER_64_COPY(result, tmp_result);
        UTILEX_64_UDIV_32(result, (1 << rev_exp) * eq_const_div);
    }
    ...
    uint32 *exp)
{
    ...
    uint32
        bin_search_beg,
        bin_search_end, bin_search_num_elements, bin_search_mid, bin_search_low_val, bin_search_high_val, exp_man, res,
        tmp;
    COMPILER_UINT64 tmp_64, tmp_res_64, tmp_val, quotient_mul_dividor_64, remainder, divisor_half;
    ...
    COMPILER_64_SET(tmp_64, 0, eq_const_multi);
    COMPILER_64_UMUL_32(tmp_64, (max_mnt + eq_const_mnt_inc));

    COMPILER_64_COPY(tmp_res_64, tmp_64);
    UTILEX_64_UDIV_32_WITH_REMAINDER(tmp_res_64, eq_const_div, res);
    res = COMPILER_64_HI(tmp_res_64);
    if (res)
    {
        /*
         * overflow
    ...
    exp_man = COMPILER_64_LO(tmp_res_64);
    ...
    if (x <= exp_man)
    {
        *exp = 0;
        COMPILER_64_SET(tmp_64, 0, eq_const_multi);
        COMPILER_64_UMUL_32(tmp_64, eq_const_mnt_inc);

        COMPILER_64_COPY(tmp_res_64, tmp_64);
        UTILEX_64_UDIV_32_WITH_REMAINDER(tmp_res_64, eq_const_div, res);
        if (COMPILER_64_HI(tmp_res_64))
        {
            /*
             * overflow
             */
        ...
        else
        {
             ...
             */
            COMPILER_64_SET(tmp_64, 0, eq_const_div);
            COMPILER_64_UMUL_32(tmp_64, x);

            COMPILER_64_COPY(tmp_res_64, tmp_64);
            UTILEX_64_UDIV_32_WITH_REMAINDER(tmp_res_64, eq_const_multi, res);
            if (COMPILER_64_HI(tmp_res_64))
            {
                /*
                 * overflow
             ...
             */
            COMPILER_64_SET(quotient_mul_dividor_64, 0, tmp);
            COMPILER_64_UMUL_32(quotient_mul_dividor_64, eq_const_multi);

            COMPILER_64_COPY(remainder, tmp_64);
            ...
            COMPILER_64_SUB_64(remainder, quotient_mul_dividor_64);
            if (COMPILER_64_BITTEST(remainder, 63))
            {
                /*
                 * Enter if 'remainder' turned out negative (i.e., 'quotient_mul_dividor_64'
                 * was found larger than the original 'remainder'.
                 * In that case, set 'remainder' to zero, as in utilex_u64_subtract_u64().
                 */
                COMPILER_64_SET(remainder, 0, 0);
            }
            else
            {
                /*
                 * (remainder = (x * eq_const_div) - (eq_const_multi * quotient)) >= 0 
                 */
                COMPILER_64_SET(divisor_half, 0, (eq_const_multi / 2));
                if (COMPILER_64_GT(remainder, divisor_half) && (tmp - eq_const_mnt_inc < max_mnt))
                {
                    /*
                     * if (remainder > (eq_const_multi / 2))
                     * and we're not exceeding the maximum mantissa take the round up
     ...
     * if (x >= exp_man * (1 << max_exp)) 
     */
    COMPILER_64_SET(tmp_val, 0, exp_man);
    COMPILER_64_UMUL_32(tmp_val, (uint32) (1 << max_exp));

    tmp = COMPILER_64_LO(tmp_val);
    if ((COMPILER_64_HI(tmp_val) == 0) && (x >= tmp))
    {
        /*
         * Enter if no overflow and (x >= tmp)
         */
        *exp = max_exp;
        *mnt = max_mnt;
        goto exit;
    ...
    bin_search_beg = 0;
    bin_search_end = max_exp;
    bin_search_num_elements = max_exp;
    bin_search_mid = bin_search_beg + bin_search_num_elements / 2;
    bin_search_low_val = exp_man * (1 << bin_search_mid);
    bin_search_high_val = exp_man * (1 << (bin_search_mid + 1));

    while ((bin_search_beg <= bin_search_end) && !((x > bin_search_low_val) && (x <= bin_search_high_val)))
    {
        if (x <= bin_search_low_val)
        {
            bin_search_end = bin_search_mid - 1;
            bin_search_num_elements = bin_search_num_elements / 2;
            bin_search_mid = bin_search_beg + bin_search_num_elements / 2;
        }
        else
        {
            bin_search_beg = bin_search_mid + 1;
            bin_search_num_elements = bin_search_num_elements / 2;
            bin_search_mid = bin_search_beg + bin_search_num_elements / 2;
        }
        bin_search_low_val = exp_man * (1 << bin_search_mid);
    ...
    *exp = bin_search_mid + 1;
    /*
     * (x * eq_const_div) / (eq_const_multi * (1 << *exp)) should be equal to eq_const_mnt_inc + mnt 
     */
    COMPILER_64_SET(tmp_64, 0, x);
    COMPILER_64_UMUL_32(tmp_64, eq_const_div);

    COMPILER_64_COPY(tmp_res_64, tmp_64);
    UTILEX_64_UDIV_32_WITH_REMAINDER(tmp_res_64, eq_const_multi, res);
    COMPILER_64_COPY(tmp_64, tmp_res_64);
    UTILEX_64_UDIV_32_WITH_REMAINDER(tmp_64, (1 << *exp), res);
    tmp = COMPILER_64_LO(tmp_64);
    if (COMPILER_64_HI(tmp_val))
    {
        /*
         * Enter in case of overflow.
         */
    ...
    uint32 *exp)
{
    ...
    COMPILER_UINT64 max_val_mult_u64, abs_val_numerator_u64;
    ...
    COMPILER_64_SET(max_val_mult_u64, 0, max_val);
    COMPILER_64_UMUL_32(max_val_mult_u64, abs_val_denominator);

    COMPILER_64_SET(abs_val_numerator_u64, 0, abs_val_numerator);

    if (COMPILER_64_GT(abs_val_numerator_u64, max_val_mult_u64))
    {
    ...
    uint32 *abs_val_denominator)
{
    shr_error_e ex;
    uint32 tmp_num, tmp_exp, real_max_val;
    COMPILER_UINT64 real_max_val_mult_u64;
    ...
    *abs_val_numerator = 0;
    if (tmp_num)
    {
        ...
        COMPILER_64_SET(real_max_val_mult_u64, 0, real_max_val);
        COMPILER_64_UMUL_32(real_max_val_mult_u64, *abs_val_denominator);
        COMPILER_64_UDIV_32(real_max_val_mult_u64, tmp_num);
        if (COMPILER_64_HI(real_max_val_mult_u64))
        {
            /*
             * Enter in case of overflow.
             */
        ...
        *abs_val_numerator = COMPILER_64_LO(real_max_val_mult_u64);
    }
