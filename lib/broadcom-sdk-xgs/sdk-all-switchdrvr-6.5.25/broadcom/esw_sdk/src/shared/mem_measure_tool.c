...
int memory_consumption_start_measurement(char *str)
{
    ...
    for(idx = 0;idx < memory_measurement_tool.count;idx++) { 
        if(sal_strncmp(memory_measurement_tool.elements[idx].id, str, sizeof(memory_measurement_tool.elements[idx].id)) == 0) {
...
int memory_consumption_pause_measurement(char *str)
{
    ...
    int fnd = 0;
    for(idx = 0;idx < memory_measurement_tool.count;idx++) {
        if((sal_strncmp(memory_measurement_tool.elements[idx].id, str, sizeof(memory_measurement_tool.elements[idx].id)) == 0) && (memory_measurement_tool.elements[idx].thread_id == sal_thread_self())) {
            fnd = 1;
...
int memory_consumption_start_measurement_dnx(char *str)
{
    ...
    for(idx = 0;idx < memory_measurement_tool_dnx.count;idx++) { 
        if(sal_strncmp(memory_measurement_tool_dnx.elements[idx].id, str, sizeof(memory_measurement_tool_dnx.elements[idx].id)) == 0) {
...
int memory_consumption_end_measurement_dnx(char *str)
{
    ...
    int fnd = 0;
    for(idx = 0;idx < memory_measurement_tool_dnx.count;idx++) {
        if((sal_strncmp(memory_measurement_tool_dnx.elements[idx].id, str, sizeof(memory_measurement_tool_dnx.elements[idx].id)) == 0) && (memory_measurement_tool_dnx.elements[idx].thread_id == sal_thread_self())) {
            fnd = 1;
