...
error:
    ...
    if (ns != NULL)
	xmlFree(ns);
    /* The allocation of the variable name may be used or freed on the call to xmlNewNsPropEatName */
...
xmlSAX2StartElement(void *ctx, const xmlChar *fullname, const xmlChar **atts)
{
	...
	if (ctxt->input != NULL) {
		...
		ret->line = (short) ctxt->input->line;
	        ...
	        /* The function xmlAddChild doesn't free ret on any case, so probably it's ok */
	        /* coveirty[deref_after_free:FALSE] */
	        ret->line = 65535;
	}
    }
     ...
     */
    /* The function xmlAddSibling doesn't free the second parameter on all cases, so probably it's ok */
...
xmlSAX2Characters(void *ctx, const xmlChar *ch, int len)
{
	...
	if ((coalesceText) && (ctxt->nodemem != 0)) {
		...
		size *= 2;
                newbuf = (xmlChar *) xmlRealloc(lastChild->content, size/2, size);
		if (newbuf == NULL) {
		    xmlSAX2ErrMemory(ctxt, "xmlSAX2Characters");
		    return;
		}
