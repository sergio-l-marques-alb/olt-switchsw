 ...
 */
int
xmlParserInputGrowRv(xmlParserInputPtr in, int len) {
    int ret;
    size_t indx;
...
/**
 * xmlParserInputGrow:
 * Same as xmlParserInputGrowRv.
 * Doesn't return value
 */
void
xmlParserInputGrow(xmlParserInputPtr in, int len) {
    /* We don't check the returned value on purpose */
    /* coverity[check_return:FALSE] */
    xmlParserInputGrowRv(in, len);
}
...
xmlParserInputShrink(xmlParserInputPtr in) {
    ...
    /* xmlParserInputBufferRead is a void function */
    /* coverity[check_return:FALSE] */
...
xmlNextChar(xmlParserCtxtPtr ctxt)
{
    ...
    if ((*ctxt->input->cur == 0) &&
        (xmlParserInputGrowRv(ctxt->input, INPUT_CHUNK) <= 0)) {
        return;
    }
    ...
    /* Otherwise, we need to add new node to buffer */
	    ...
	    (ctxt->node_seq.buffer == NULL)) {
            ...
            if (ctxt->node_seq.buffer == NULL)
                ...
                tmp_buffer =
                    (xmlParserNodeInfo *) xmlRealloc(ctxt->node_seq.buffer, (sizeof(*ctxt->node_seq.buffer) * (2 * ctxt->node_seq.maximum)),
                                                     byte_size);
