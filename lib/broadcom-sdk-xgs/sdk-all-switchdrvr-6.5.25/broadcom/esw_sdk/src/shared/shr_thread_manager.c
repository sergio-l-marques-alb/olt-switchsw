...
typedef struct shr_thread_manager_object_s {
    ...
    int interval;                                   /* interval provided by the user (see shr_thread_manager.h)*/
...
int shr_thread_manager_stop(shr_thread_manager_handler_t h, int dont_wait)
{
    ...
    soc_timeout_init(&to, obj->stop_timeout_usec, 100);

    while (is_thread_created(h)) {
        if (soc_timeout_check(&to)) {
            LOG_WARN(obj->bsl_module, (BSL_META("THREAD_MANAGER[%s]: Failed to gracefully turn off thread. \n"), obj->name));
            rv = _SHR_E_TIMEOUT;
            break;
        }
    }
...
int shr_thread_manager_join(shr_thread_manager_handler_t* h, uint32 timeout)
{
    ...
    {
        if (soc_timeout_check(&to)) {
            LOG_WARN(obj->bsl_module, (BSL_META("THREAD_MANAGER[%s]: Failed to join the thread. \n"), obj->name));
