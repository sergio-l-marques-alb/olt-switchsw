...
int sw_state_get_nof_elements(int unit, void *ptr, uint32 size, uint32* from, uint32 *nof_elements) {
    ...
    if (ptr == NULL) {
        ...
        *from = 0;
        ...
        _SOC_IF_ERR_EXIT(shr_sw_state_allocated_size_get(unit, (uint8*)ptr, nof_elements));
        *nof_elements /= size;
...
void shr_sw_state_dump_update_stride(int unit, uint64 value, uint32 size, char* str) {
    ...
    sal_strncpy_s(sw_state_stride_dump[unit].last_string[*cnt_idx], str, SW_STATE_STRIDE_DUMP_FILE_NAME_LEN);
}
...
void shr_sw_state_dump_print_stride(int unit, uint8 is_broken) {
  ...
  for (i = 0; i < nof_members; i++) {
      ...
      sal_strncat_s(str_to_print, tmp_string, SW_STATE_STRIDE_DUMP_MAX_STRING_LEN);
      sal_strncat_s(str_to_print, close_par, SW_STATE_STRIDE_DUMP_MAX_STRING_LEN);
      ...
      sal_strncat_s(str_to_print, value_str, SW_STATE_STRIDE_DUMP_MAX_STRING_LEN);
      shr_sw_state_dump_print(unit, str_to_print);
  }
  for (i = 0; i < nof_members && is_broken && nof_members > 1; i++) {
      ...
      if (sw_state_stride_dump[unit].same_diff_cnt[i] == min_same_diff) {
          ...
          sal_snprintf(tmp_str, SW_STATE_STRIDE_DUMP_TEMP_STR_LEN, "%llx\n",(LLU) (sw_state_stride_dump[unit].last_val[i]));
...
void shr_sw_state_dump_attach_file(int unit, char* filename) {
    ...
    sal_strncpy_s(sw_state_stride_dump_file_name[unit], filename, SW_STATE_STRIDE_DUMP_FILE_NAME_LEN);
