...
exit:
    /*
     * The function xmlDocSetRootElement use the data allocated on doc 
     */
    ...
    char *filepath)
{
    ...
    if (node && node->doc)
    {
        ...
        err = xmlSaveFormatFile(filepath, node->doc, 1);
        if (err < 0)
        {
            cli_out("dbx_xml_top_save(): procedure xmlSaveFormatFile() reported error %d\n", err);
            goto exit;
        }
    }
exit:
