 ...
 */
void
shr_util_format_uint32(char *buf, uint32 n, int min_digits, int base)
{
    static char *digit_char = "0123456789abcdef";
    unsigned int tmp;
    int digit, needed_digits = 0;
...
/*!
 * \brief Duplicate a specified string.
 *
 * The duplicated string can be tracked by the specified ID \c s.
 * Use \c sal_free to release the duplicated string.
 *
 * \param [in] str The string to be duplicated.
 * \param [in] s Text string for tracking purposes.
 *
 * \return Pointer to a new string which is a duplicate of \c str,
 *         or NULL on failure.
 */
char *
shr_util_strdup(const char *str, char *s)
{
    int len = sal_strlen(str) + 1;
    char *rc = sal_alloc(len, s);
...
/*!
 * \brief Convert hex digit to hex character.
 *
 * Convert hex digit to hex character.
 *
 * \param [in] digit The integer digit to be converted.
 *
 * \return hex character.
 */
int
shr_i2xdigit(int digit)
{
    digit &= 0xf;
...
/*!
 * \brief Convert MAC address type to standard MAC address string.
 *
 * shr_format_macaddr requires a buffer of 18 bytes minimum.
 * It does not use sal_sprintf so it can be called from an interrupt context.
 *
 * \param [out] buf Character array to hold output string.
 * \param [in] MAC address in the format of sal_mac_addr_t (uint8 array).
 *
 */
void
shr_format_macaddr(char buf[SAL_MACADDR_STR_LEN], sal_mac_addr_t macaddr)
{
    int i;

    for (i = 0; i <= 5; i++) {
        *buf++ = shr_i2xdigit(macaddr[i] >> 4);
        *buf++ = shr_i2xdigit(macaddr[i]);
        *buf++ = ':';
    }
...
/*!
 * \brief Convert IPv4 address type to standard IPv4 address string.
 *
 * shr_format_ipaddr requires a buffer of 16 bytes minimum.
 * It uses sal_sprintf so it CANNOT be called from an interrupt context.
 *
 * \param [out] buf Character array to hold output string.
 * \param [in] IPv4 address in the format of sal_ip_addr_t
 *
 */
void
shr_format_ipaddr(char buf[SAL_IPADDR_STR_LEN], sal_ip_addr_t ipaddr)
{
...
/*!
 * \brief Convert IPv6 address type to standard IPv6 address string.
 *
 * shr_format_ipaddr requires a buffer of 64 bytes minimum.
 * It uses sal_sprintf so it CANNOT be called from an interrupt context.
 *
 * \param [out] buf Character array to hold output string.
 * \param [in] IPv6 address in the format of sal_ip6_addr_t
 *
 */
void
shr_format_ip6addr(char buf[SAL_IP6ADDR_STR_LEN], sal_ip6_addr_t ipaddr)
{
