...
/*
 * SAL_THREAD_STKSZ is adjusted to include the possibility of up to 10
 * recursive calls to cint_variable_print, each of which results in allocation
 * of upto 3 buffers of size CINT_CONFIG_MAX_STACK_PRINT_BUFFER on the stack
 *
 * The user can also set the stack size for the logger thread via SOC
 * property api_logger_thread_stksz. See api_logging* in propdata.pl
 */
#define CINT_LOGGER_THREAD_STKSZ_DEFAULT    (SAL_THREAD_STKSZ)
...
typedef struct cint_logger_global_cfg_data_s {
    ...
    /*
     * parameters related to replay
     *
     * Holds name of the CINT filter function which is invoked by the Replay
     * handler. This function receives auto-magically the arguments provided
     * to the API during replay, along with the values in the original logs.
     * See cint_logger_replay_function_arguments
     */
    char *cint_replay_filter_fn;
...
/*
 * Logger internal utility to set/clear CONST indication on members of
 * cint_logger_call_ctxt_t, depending on mode being Replay or not.
 */
extern void cint_logger_cint_const_fields(int set);
