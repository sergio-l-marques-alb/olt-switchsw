/*
 * $Id: cint_builtins.c,v 1.45 Broadcom SDK $
...
static cint_parameter_desc_t __cint_struct_members__cint_time_t[] = {
    { "time_t" , "tv_sec"  , 0, 0 },
    { "long"   , "tv_usec" , 0, 0 },
    { NULL },
};
...
static void*
__cint_maddr__cint_time_t (void* p, int mnum, cint_struct_type_t* parent)
{
    void* rv;
    cint_time_t* s = p;

    switch(mnum) {
    case  0: rv = &(s->tv_sec); break;
    case  1: rv = &(s->tv_usec); break;
    default: rv = NULL; break;
    }
...
static cint_parameter_desc_t __cint_struct_members__cint_tm_t[] = {
    { "int", "tm_sec"   , 0, 0 },
    { "int", "tm_min"   , 0, 0 },
    { "int", "tm_hour"  , 0, 0 },
    { "int", "tm_mday"  , 0, 0 },
...
static void*
__cint_maddr__cint_tm_t (void *p, int mnum, cint_struct_type_t* parent)
{
    void *rv;
    cint_tm_t *s = p;

    switch (mnum) {
    case  0: rv = &(s->tm_sec);   break;
    case  1: rv = &(s->tm_min);   break;
    case  2: rv = &(s->tm_hour);  break;
    case  3: rv = &(s->tm_mday);  break;
    case  4: rv = &(s->tm_mon);   break;
    case  5: rv = &(s->tm_year);  break;
    case  6: rv = &(s->tm_wday);  break;
    case  7: rv = &(s->tm_yday);  break;
    case  8: rv = &(s->tm_isdst); break;
    default: rv = NULL; break;
    }
...
#if CINT_CONFIG_INCLUDE_POSIX_TIMER == 1
        CINT_STRUCT_TYPE_DEFINE(cint_time_t),
        CINT_STRUCT_TYPE_DEFINE(cint_tm_t),
#endif
        { NULL }
    };
...
/*
 * Get system time of the day
 *
 * Parameters
 *   [out] tv
 *     Pointer to cint_time_t which is populated with time of the day
 *
 * Uses the clock_gettime or gettimeofday POSIX APIs. Return value of those
 * APIs are returned.  See the manual page for those APIs.
 */
int
cint_gettime (cint_time_t *tv)
{
    int ret;
#if defined(CLOCK_REALTIME) && !defined(CINT_USE_GETTIMEOFDAY)
    struct timespec ltv;
    if ((ret = clock_gettime(CLOCK_REALTIME, &ltv) == 0)) {
        tv->tv_sec  = ltv.tv_sec;
        tv->tv_usec = (ltv.tv_nsec / 1000);
    }
#else
    struct timeval ltv;
    if ((ret = gettimeofday(&ltv, NULL) == 0)) {
        tv->tv_sec  = ltv.tv_sec;
...
/*
 * Wrapper around the POSIX localtime API.
 *
 * Parameters and Return values
 *
 * cint_tm_t is the same as struct tm; See manual page for localtime
 */
cint_tm_t *
cint_localtime (time_t *tp)
{
    return localtime(tp);
}

/*
 * Wrapper around the POSIX localtime_r API. This is the thread-safe version of
 * POSIX localtime
 *
 * Parameters and Return values
 *
 * cint_tm_t is the same as struct tm; See manual page for localtime_r
 */
cint_tm_t *
cint_localtime_r (time_t *tp, cint_tm_t *result)
{
    return localtime_r(tp,result);
}

/*
 * Wrapper around the POSIX strftime API.
 *
 * Parameters and Return values
 *
 * cint_tm_t is the same as struct tm; See manual page for strftime
 */
int
cint_strftime (char *str, int maxlen, char *fmt, cint_tm_t *tm)
{
    return strftime(str, maxlen, fmt, tm);
}

/*
 * Compare two time values since epoch
 *
 * This API behaves similar to strcmp. It takes two inputs and returns a value
 * >0, =0, <0 depending on whether the first argument is greater than, equal
 * to, or less than the second argument
 *
 * Parameters
 *   [in] a,b
 *     Time values to be compared
 *
 * Returns
 *   1  if a > b
 *   0  if a = b
 ...
 */
int
cint_timercmp (cint_time_t *a, cint_time_t *b)
{
...
/*
 * Add two cint_time_t values and return the result
 *
 * Parameters
 *   [in] a,b
 *     The source time values which need to be added
 *   [out] res
 *     Result of a+b is stored here.
 *
 * Routine takes care to restrict tv_usec to 0 - 999999
 */
void
cint_timeradd (cint_time_t *a, cint_time_t *b, cint_time_t *res)
{
...
/*
 * Subtract two cint_time_t values and return the result. Uses cint_timercmp to
 * compare the two inputs; result is populated only if a>=b.
 *
 * Parameters
 *   [in] a,b
 *     The source time values which need to be subtracted (a-b)
 *   [out] res
 *     Result of a-b is stored here.
 *
 * Callers must ensure a>=b before using the result.
 *
 * Routine takes care to restrict res.tv_usec to 0 - 999999 and adjusts
 * res.tv_sec accordingly.
 */
void
cint_timersub (cint_time_t *a, cint_time_t *b, cint_time_t *res)
{
    if (cint_timercmp(a,b) >= 0) {
...
CINT_FWRAPPER_CREATE2_RP1(int,int,0,0,
                          cint_gettime,
                          cint_time_t*, cint_time_t, tv, 1, 0, CINT_PARAM_OUT);

CINT_FWRAPPER_CREATE2_RP1(cint_tm_t*,cint_tm_t,1,0,
                          cint_localtime,
                          time_t*, time_t, tp, 1, 0, CINT_PARAM_IN);

CINT_FWRAPPER_CREATE2_RP2(cint_tm_t*,cint_tm_t,1,0,
                          cint_localtime_r,
                          time_t*, time_t, tp, 1, 0, CINT_PARAM_IN,
                          cint_tm_t*, cint_tm_t, result, 1, 0, CINT_PARAM_OUT);

CINT_FWRAPPER_CREATE2_RP4(int,int,0,0,
                          cint_strftime,
                          char*, char, str, 1, 0, CINT_PARAM_OUT,
                          int, int, maxlen, 0, 0, CINT_PARAM_IN,
                          char*, char, fmt, 1, 0, CINT_PARAM_IN,
                          cint_tm_t*, cint_tm_t, tm, 1, 0, CINT_PARAM_IN);

CINT_FWRAPPER_CREATE2_VP3(cint_timeradd,
                          cint_time_t*, cint_time_t,   a, 1, 0, CINT_PARAM_IN,
                          cint_time_t*, cint_time_t,   b, 1, 0, CINT_PARAM_IN,
                          cint_time_t*, cint_time_t, res, 1, 0, CINT_PARAM_OUT);

CINT_FWRAPPER_CREATE2_VP3(cint_timersub,
                          cint_time_t*, cint_time_t,   a, 1, 0, CINT_PARAM_IN,
                          cint_time_t*, cint_time_t,   b, 1, 0, CINT_PARAM_IN,
                          cint_time_t*, cint_time_t, res, 1, 0, CINT_PARAM_OUT);
...
static cint_function_t __cint_builtin_functions[] = 
        {
                ...
                CINT_FWRAPPER_ENTRY(cint_gettime),
                CINT_FWRAPPER_ENTRY(cint_localtime),
                CINT_FWRAPPER_ENTRY(cint_localtime_r),
                CINT_FWRAPPER_ENTRY(cint_strftime),
                CINT_FWRAPPER_ENTRY(cint_timeradd),
                CINT_FWRAPPER_ENTRY(cint_timersub),
                CINT_FWRAPPER_ENTRY(cint_timercmp),
#endif 
