/*
 * $Id: cint_operators.c,v 1.37 Broadcom SDK $
...
__cint_operator_OpenBracket(cint_ast_t* ast, cint_operand_type_t optype, cint_variable_t* left, cint_variable_t* right)
{
    ...
    {   
        ...
        if(left->cached_num_dimensions <= 1) {
            ...
            /*
             * A new variable with one dimension removed (on account of array
             * subscript) is being created below.
             *
             * Previously, the variable's underlying datatype was being
             * adjusted here after creating it with cint_variable_create.
             *
             * Since the initial datatype was a pointer (desc.pcount = 1)
             * during creation, the newly created variable's size was computed
             * to be the same as that of a pointer; this became inconsistent
             * with the adjusted datatype for the case of pointers to array. An
             * object derived by subscripting a pointer to array is an array
             * and no longer a pointer.
             *
             * The computed size is also returned by the sizeof() operator -
             * see __cint_operator_Sizeof - thus returning incorrect size to
             * any CINT which uses sizeof() operator on such variables.
             *
             * Therefore, desc is being adjusted here to the variable's
             * intended final datatype prior to cint_variable_create.
             */
            desc.pcount--;
            desc.num_dimensions = left->cached_num_dimensions - 1;
            for(dim_index = 0;
...
__cint_operator_Dereference(cint_ast_t* ast, cint_operand_type_t optype, cint_variable_t* left, cint_variable_t* right)
{
    ...
    /*
     * It is meaningless to dereference a pointer with basetype void and one
     * indirection (void*).
     */
    else if ((right->dt.desc.pcount == 1) && !CINT_STRCMP(right->dt.desc.basetype, "void")) {
