    ...
    int unit)
{
    ...
    /*
     *  Interrupts force unmask on init
     */
    table_info = dnx_data_intr.general.force_unmask_on_init_info_get(unit);
    for (i = 0; i < table_info->key_size[0]; i++)
    {
        event.event_id = dnx_data_intr.general.force_unmask_on_init_get(unit, i)->intr;
        event.index = BCM_CORE_ALL;
        event.action = bcmSwitchEventForceUnmask;
