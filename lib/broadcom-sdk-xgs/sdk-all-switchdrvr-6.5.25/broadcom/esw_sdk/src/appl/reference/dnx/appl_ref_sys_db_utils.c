    ...
    int *value)
{
    ...
    length = sal_strnlen(token, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH);
    if (length == 0)
    {
    ...
    int *value)
{
    ...
    /** convert token to integer */
    if (sal_strncmp(token, "TDM", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
    {
        *value = APPL_PORT_TYPE_TDM;
    }
    else if (sal_strncmp(token, "OLP", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
    {
...
/*** \brief - interpret "appl_param_sys_port_X = <dev>:<port>:<core>:<pp_dsp>:<rate>" appl soc property, see header */
shr_error_e
    ...
    system_port_mapping_t * sys_port_map)
{
    ...
    /** read next token - PP DSP */
    token = sal_strtok_r(NULL, ":", &saveptr);
    SHR_IF_ERR_EXIT(appl_dnx_sys_db_utils_token_interpret(unit, token, &value));
    sys_port_map->pp_dsp = value;
