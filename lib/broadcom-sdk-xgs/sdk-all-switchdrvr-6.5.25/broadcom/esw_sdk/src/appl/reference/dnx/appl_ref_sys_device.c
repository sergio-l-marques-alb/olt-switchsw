 ...
 * appl_param_sys_port5 = <dev>:<port>:<core>:<pp_dsp>:<rate>:<port_type>                 :: map system port to device, local port, core, pp dsp and rate
 *
    ...
    appl_dnx_sys_device_t * sys_params)
{
    ...
    /** Number of ports (PP_DSP's) per modid, by default maximal amount is assumed (256) */
    sys_params->max_nof_ports_per_modid =
        soc_property_suffix_num_get_only_suffix(unit, -1, spn_APPL_PARAM, "nof_ports_per_modid",
                                                APPL_SAND_NOF_PORTS_PER_MODID);

    LOG_VERBOSE_EX(BSL_LOG_MODULE, "   Max number of ports per modid = %d.\n%s%s%s",
                   sys_params->max_nof_ports_per_modid, EMPTY, EMPTY, EMPTY);

    /** get rcy_mirror ports range */
    ...
    /** make sure that max nof cores is at least 2 (even for systems with single core devices only) */
    sys_params->max_nof_cores = 2;
    ...
    for (int device_index = 0, total_nof_modids = 0; device_index < sys_params->nof_devices; ++device_index)
    {
        ...
        /** set max nof cores in the system */
        if (sys_params->devices_array[device_index].nof_cores > sys_params->max_nof_cores)
        {
            sys_params->max_nof_cores = sys_params->devices_array[device_index].nof_cores;
        }
    ...
    else
    {
        ...
        /** for now ASSUME SYMMETRIC CONFIGURATION!!! */
        /**  asymmetric configuration for MGMT devices will be delivered later per demand */
        int local_port;
        bcm_pbmp_t pbmp;
        bcm_port_config_t port_config;
        ...
        BCM_PBMP_ITER(pbmp, local_port)
        {
            ...
            for (int device_index = 0; device_index < sys_params->nof_devices; ++device_index)
            {
                ...
                sys_port_map.system_port_id = system_port_id;
...
/**
 * \brief - Convert FAP index + PP DSP to
 * modid + ftmh_pp_dsp
 *
 */
shr_error_e
appl_dnx_fap_and_pp_dsp_to_modport_convert(
    int unit,
    int fap_index,
    int core_id,
    int pp_dsp,
    ...
    int *ftmh_pp_dsp)
{
    ...
    local_index = pp_dsp / APPL_SAND_NOF_PORTS_PER_MODID;
    /** Get base modid */
    base_modid = device_info->base_modids[local_index];
    /** Get FTMH pp dsp (currently only 8b) */
    *ftmh_pp_dsp = pp_dsp % APPL_SAND_NOF_PORTS_PER_MODID;
    /** Calculate modid */
 ...
 */
shr_error_e
appl_dnx_fap_and_pp_dsp_to_modid_convert(
    int unit,
    int fap_index,
    int core_id,
    int pp_dsp,
    int *modid)
{
    ...
    SHR_IF_ERR_EXIT(appl_dnx_fap_and_pp_dsp_to_modport_convert(unit, fap_index, core_id, pp_dsp, modid, &ftmh_pp_dsp));
exit:
    SHR_FUNC_EXIT;
}
    ...
    void *local_port_verify_void_ptr)
{
    ...
    if ((sys_port_map_info->pp_dsp != mapping_info.pp_dsp) ||
        (sys_port_map_info->core != mapping_info.core) || (sys_port_map_info->rate != port_resource.speed))
    {
        LOG_ERROR(BSL_LOG_MODULE,
                  (BSL_META("system mapping for system port %d: local_port=%d, core=%d, pp_dsp=%d, rate=%d\n"),
                   system_port_id, sys_port_map_info->local_port, sys_port_map_info->core, sys_port_map_info->pp_dsp,
                   sys_port_map_info->rate));
        LOG_ERROR(BSL_LOG_MODULE,
                  (BSL_META("local mapping for local port %d :                core=%d, pp_dsp=%d, rate=%d\n"),
                   local_port, mapping_info.core, mapping_info.pp_dsp, port_resource.speed));
    ...
    int unit)
{
    ...
    if (is_j1_mode)
    {
        j1_learning = dnx_data_field.init.j1_learning_get(unit);
    }
    ...
    if (!SOC_WARM_BOOT(unit))
    {
        ...
        {
            if (j1_learning && is_j1_mode)
            {
                if ((logical_port == LEARN_RCY_PORT1) || (logical_port == LEARN_RCY_PORT2))
                {
                    continue;
                }
            }
