    ...
    system_port_mapping_t * sys_port_map_info)
{
    ...
    if (system_port_db->entries[system_port_id].valid_entry == 0)
    {
        SHR_ERR_EXIT(_SHR_E_EXISTS, "system port %d is not in use\n", system_port_id);
    }

    *sys_port_map_info = system_port_db->entries[system_port_id].sys_port_map_info;

    /** Set logical port to invalid value on non local system ports to double check they are not used */
    SHR_IF_ERR_EXIT(appl_dnx_sys_device_params_get(unit, &sys_params));
    if (sys_port_map_info->device_index != sys_params->unit_to_device_index[unit])
    {
        sys_port_map_info->local_port = -1;
    }
