    ...
    sh_sand_control_t * sand_control)
{
    ...
    int core = 0;
    uint32 max_nof_fgs;
    char bank_entries_count_buff[DIAG_DNX_FIELD_UTILS_STR_SIZE];
    char bank_entries_count_buff_full_line[DIAG_DNX_FIELD_UTILS_STR_SIZE];
    ...
    for (bank_id_iter = bank_id_lower; bank_id_iter <= bank_id_upper; bank_id_iter++)
    {
        ...
        for (fg_id_iter = 0; fg_id_iter < allocated_fg_iter; fg_id_iter++)
        {
            ...
            sal_strncpy_s(bank_entries_count_buff, "", sizeof(bank_entries_count_buff));
            DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core)
            {
                ...
                if (sal_strncmp(bank_entries_count_buff, "", sizeof(bank_entries_count_buff)))
                {
                    DIAG_DNX_FIELD_UTILS_STR_INPUT_VERIFY(bank_entries_count_buff, "%s", "\n");
                    sal_strncat_s(bank_entries_count_buff, "\n", sizeof(bank_entries_count_buff));
                }
                DIAG_DNX_FIELD_UTILS_STR_INPUT_VERIFY(bank_entries_count_buff, "core_%d: %d", core,
                                                      bank_entries_count[bank_id_iter]);
                sal_snprintf(bank_entries_count_buff_full_line, sizeof(bank_entries_count_buff_full_line),
                             "core_%d: %d", core, bank_entries_count[bank_id_iter]);
                sal_strncat_s(bank_entries_count_buff, bank_entries_count_buff_full_line,
                              sizeof(bank_entries_count_buff));
            }
            ...
            PRT_CELL_SET("%s", bank_entries_count_buff);
        }
        if (!do_display)
        {
            PRT_COMMITX;
