    ...
    int release)
{
    ...
    *add_soc_prop = FALSE;
    ...
    {
        if ((label_list = utilex_str_split(label, "|", 32, &num_of_labels)) == NULL)
        {
    ...
    else
    {
        ...
        label_list[0] = sal_alloc(sal_strnlen(label, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) + 1, "label");
        sal_memcpy(label_list[0], label, sal_strnlen(label, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH));
        num_of_labels = 1;
    }
                ...
                if (table->values[value_index].property.name != NULL && table->values[value_index].property.doc != NULL)
                {
                    ...
                    SHR_IF_ERR_EXIT(diag_dnxc_data_man_property_add
                                    (unit, submodule_node, depth, &table->values[value_index].property,
                                     table->keys[0].name, table->values[value_index].default_val, label, release,
                                     &submodule_nof_prop));
                }
            }
        }       /* Dump table */
    ...
    for (define_index = 0; define_index < submodule->nof_defines; define_index++)
    {
        ...
        if (dump)
        {
            ...
            if (define->property.name != NULL && define->property.doc != NULL)
            {
                ...
                SHR_IF_ERR_EXIT(diag_dnxc_data_man_property_add
                                (unit, submodule_node, depth, &define->property, NULL, default_val_str, label, release,
                                 &submodule_nof_prop));
            }
        }       /* Dump numeric */
    }   /* numeric iterator */
    ...
    for (submodule_index = 0; submodule_index < module->nof_submodules; submodule_index++)
    {
        *should_print = 0;
        ...
        SHR_IF_ERR_EXIT(diag_dnxc_data_submodule_man
                        (unit, args, sand_control, module_node, module, submodule_index, 1, should_print, label,
                         release, &module_nof_prop));
