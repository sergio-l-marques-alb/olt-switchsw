...
static shr_error_e
diag_dnx_field_range_init(
    int unit)
{
    bcm_field_range_info_t range_info;
    int range_id_src_dst = 5;
    int range_id_src = 15;
    int range_id_dst = 21;
    int range_id_invalid = 0;
    int range_id_out_vport = 1;
    int range_id_header_size = 2;

    SHR_FUNC_INIT_VARS(unit);

    /** A range with both src and dest.*/
    ...
    SHR_IF_ERR_EXIT(bcm_field_range_set(unit, 0, bcmFieldStageIngressPMF1, range_id_src_dst, &range_info));
    SHR_IF_ERR_EXIT(bcm_field_range_set(unit, 0, bcmFieldStageEgress, range_id_src_dst + 2, &range_info));
    bcm_field_range_info_t_init(&range_info);
    range_info.range_type = bcmFieldRangeTypeL4DstPort;
    range_info.min_val = 200;
    range_info.max_val = 2001;
    SHR_IF_ERR_EXIT(bcm_field_range_set(unit, 0, bcmFieldStageIngressPMF1, range_id_src_dst, &range_info));
    SHR_IF_ERR_EXIT(bcm_field_range_set(unit, 0, bcmFieldStageEgress, range_id_src_dst + 2, &range_info));

    /** A range with src only.*/
    bcm_field_range_info_t_init(&range_info);
    range_info.range_type = bcmFieldRangeTypeL4SrcPort;
    range_info.min_val = 4;
    range_info.max_val = 5;
    SHR_IF_ERR_EXIT(bcm_field_range_set(unit, 0, bcmFieldStageIngressPMF1, range_id_src, &range_info));
    SHR_IF_ERR_EXIT(bcm_field_range_set(unit, 0, bcmFieldStageEgress, range_id_src + 2, &range_info));

    /** A range with dst only.*/
    bcm_field_range_info_t_init(&range_info);
    range_info.range_type = bcmFieldRangeTypeL4DstPort;
    range_info.min_val = 3;
    range_info.max_val = 5;
    SHR_IF_ERR_EXIT(bcm_field_range_set(unit, 0, bcmFieldStageIngressPMF1, range_id_dst, &range_info));
    SHR_IF_ERR_EXIT(bcm_field_range_set(unit, 0, bcmFieldStageEgress, range_id_dst + 2, &range_info));

    /** A range with a never hit range.*/
    bcm_field_range_info_t_init(&range_info);
    range_info.range_type = bcmFieldRangeTypeL4DstPort;
    range_info.min_val = 5;
    range_info.max_val = 3;
    SHR_IF_ERR_EXIT(bcm_field_range_set(unit, 0, bcmFieldStageIngressPMF1, range_id_invalid, &range_info));
    SHR_IF_ERR_EXIT(bcm_field_range_set(unit, 0, bcmFieldStageEgress, range_id_invalid + 2, &range_info));

    /** An out vport range.*/
    bcm_field_range_info_t_init(&range_info);
    range_info.range_type = bcmFieldRangeTypeOutVport;
    range_info.min_val = 10;
    range_info.max_val = 20;
    SHR_IF_ERR_EXIT(bcm_field_range_set(unit, 0, bcmFieldStageIngressPMF1, range_id_out_vport, &range_info));
    SHR_IF_ERR_EXIT(bcm_field_range_set(unit, 0, bcmFieldStageIngressPMF3, range_id_out_vport + 1, &range_info));

    /** A header size range.*/
    bcm_field_range_info_t_init(&range_info);
    range_info.range_type = bcmFieldRangeTypePacketHeaderSize;
    range_info.min_val = 20;
    range_info.max_val = 30;
    SHR_IF_ERR_EXIT(bcm_field_range_set(unit, 0, bcmFieldStageIngressPMF1, range_id_header_size, &range_info));
...
static shr_error_e
diag_dnx_field_range_extended_init(
    int unit)
{
    bcm_field_range_info_t range_info;
    ...
    if (dnx_data_field.features.extended_l4_ops_get(unit))
    {
        ...
        SHR_IF_ERR_EXIT(bcm_field_range_type_config_set(unit, 0, bcmFieldStageIngressPMF1, 0, 0, &range_qual_info));

        range_qual_info.qual_type = bcmFieldQualifyInVPort0;
        range_qual_info.qual_info.input_type = bcmFieldInputTypeMetaData;
        SHR_IF_ERR_EXIT(bcm_field_range_type_config_set(unit, 0, bcmFieldStageIngressPMF1, 0, 1, &range_qual_info));

        SHR_IF_ERR_EXIT(bcm_switch_control_set(unit, bcmSwitchRangeOperator, bcmSwitchRangeOperatorNone));
        ...
        key.type = bcmSwitchRangeTypeSelect;
        key.index = 1;
        value.value = bcmFieldRangeTypeUserDefined1High;
        SHR_IF_ERR_EXIT(bcm_switch_control_indexed_set(unit, key, value));
        ...
        key.index = 0;
        /*
         * Coverity explanation: different types take different values/enums.
         */
         ...
         */
         /* coverity[mixed_enums:FALSE]  */
        value.value = bcmSwitchRangeResultMapThirdnForth;
        SHR_IF_ERR_EXIT(bcm_switch_control_indexed_set(unit, key, value));
        ...
        SHR_IF_ERR_EXIT(bcm_field_range_set(unit, 0, bcmFieldStageIngressPMF1, 0, &range_info));

        bcm_field_range_info_t_init(&range_info);
        range_info.range_type = bcmFieldRangeTypeUserDefined1High;
        range_info.min_val = 0;
        range_info.max_val = 5;
        SHR_IF_ERR_EXIT(bcm_field_range_set(unit, 0, bcmFieldStageIngressPMF1, 1, &range_info));

        bcm_field_range_info_t_init(&range_info);
        range_info.range_type = bcmFieldRangeTypeInTTL;
        range_info.min_val = 0;
        range_info.max_val = 100;
        SHR_IF_ERR_EXIT(bcm_field_range_set(unit, 0, bcmFieldStageIngressPMF1, 2, &range_info));
        range_info.min_val = 100;
        range_info.max_val = 200;
        SHR_IF_ERR_EXIT(bcm_field_range_set(unit, 0, bcmFieldStageIngressPMF1, 3, &range_info));

        bcm_field_range_info_t_init(&range_info);
        range_info.range_type = bcmFieldRangeTypeL4OpsPacketHeaderSize;
        range_info.min_val = 90;
        range_info.max_val = 260;
        SHR_IF_ERR_EXIT(bcm_field_range_set(unit, 0, bcmFieldStageIngressPMF1, 4, &range_info));
    }
...
static sh_sand_man_t Field_range_list_man = {
    ...
    .examples = ""
                "\n" "type=L4_SRC stage=ipmf1",
    .init_cb =  diag_dnx_field_range_init,
};
...
static sh_sand_man_t Field_range_extended_man = {
    ...
    .synopsis = NULL,
    .examples = NULL,
    .init_cb =  diag_dnx_field_range_extended_init,
};
...
static sh_sand_man_t Field_range_extended_ffc_man = {
    ...
    .examples = ""
                "\n" "index=RANGE_CONFIG_TYPE_LOWEST-RANGE_CONFIG_TYPE_HIGHEST",
    .init_cb =  diag_dnx_field_range_extended_init,
};
    ...
    sh_sand_control_t * sand_control)
{
         ...
         range_type_index++)
    {
            ...
            for (range_id = 0; range_id < max_range_id; range_id++)
            {
                ...
                for (stage_index = stage_lower; stage_index <= stage_upper; stage_index++)
                {
                         ...
                         range_type_internal <= DNX_FIELD_RANGE_TYPE_L4_DST_PORT; range_type_internal++)
                    {
                        ...
                        range_map_legacy_p = &range_map_legacy[range_type_internal][field_stage];
        ...
        else
        {
             ...
             */
            range_map_legacy_p = &range_map_legacy[range_type][DNX_FIELD_STAGE_IPMF1];
            PRT_TITLE_SET("%s Range", range_map_legacy_p->name);
            PRT_COLUMN_ADD_FLEX(PRT_FLEX_ASCII, "Range ID");
            PRT_COLUMN_ADD_FLEX(PRT_FLEX_ASCII, "Range Field Stage");
            PRT_COLUMN_ADD_FLEX(PRT_FLEX_ASCII, "Range min");
            PRT_COLUMN_ADD_FLEX(PRT_FLEX_ASCII, "Range max");
            ...
            for (range_id = 0; range_id < max_range_id; range_id++)
            {
                ...
                for (stage_index = stage_lower; stage_index <= stage_upper; stage_index++)
                {
                    ...
                    /** If the stage is not supported for the given range_type continue. */
                    range_map_legacy_p = &range_map_legacy[range_type][field_stage];
                    if (range_map_legacy_p->table_id == DBAL_TABLE_EMPTY)
                    {
                        continue;
                    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (range_type_index = range_type_lower; range_type_index <= range_type_upper; range_type_index++)
    {
        ...
        for (index = 0; index < nof_valid_range_indexes; index++)
        {
            ...
            sal_snprintf(indexes_per_range_type_buff, sizeof(indexes_per_range_type_buff), "%d", range_indexes[index]);
            if (sal_strncmp(indexes_per_range_type_string, "", sizeof(indexes_per_range_type_string)))
            {
                DIAG_DNX_FIELD_UTILS_STR_INPUT_VERIFY(indexes_per_range_type_string, "%s", ", ");
                sal_strncat_s(indexes_per_range_type_string, ", ", sizeof(indexes_per_range_type_string));
            }
    ...
    sh_sand_control_t * sand_control)
{
         ...
         range_config_type_index++)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_field_map_dnx_qual_size
                        (unit, DNX_FIELD_STAGE_L4_OPS, dnx_qual_info.dnx_qual, NULL, NULL, &dnx_qual_size));
        ...
        else if (dnx_qual_info.qual_info.input_type == DNX_FIELD_INPUT_TYPE_EXTERNAL)
        {
            PRT_CELL_SET("%s", "-");
            PRT_CELL_SET("%d", dnx_qual_info.qual_info.offset);
        }
