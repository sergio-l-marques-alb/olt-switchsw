...
static sh_sand_man_t Field_fem_info_by_fg_man = {
    ...
    .full = "'FEM' display for a range of fems. Range is specified via 'group' and 'fem id'\r\n"
      ...
      "If 'valid' is not specified or is set to 'yes' then only group combinations, with at"
    ...
    .synopsis = "[group=<lower value>-<upper value>]"
                ...
                " [valid=<YES | NO>]",
    .examples = ""
                "\n" "valid=YES group=0-20 active_actions=ignore id=min-max",
    .init_cb =  diag_dnx_field_fem_init,
};
...
static sh_sand_man_t Field_fem_info_by_context_man = {
    ...
    .synopsis = "[context=<<Lowest context number>-<Highest context number>>]"
                ...
                " [valid=<YES | NO>]",
    .examples = ""
                "\n" "valid=YES context=0-20 id=min-max",
    .init_cb =  diag_dnx_field_fem_init,
};
    ...
    sh_sand_control_t * sand_control)
{
    ...
    LOG_DEBUG_EX(BSL_LOG_MODULE, "fg_id: from %d to %d%s%s\r\n", fg_id_lower, fg_id_upper, EMPTY, EMPTY);
    ...
    for (fem_id_index = fem_id_lower; fem_id_index <= fem_id_upper; fem_id_index++)
    {
        ...
        for (fem_program_index = 0; fem_program_index < fem_program_max; fem_program_index++)
        {
            ...
            if ((fg_id == DNX_FIELD_GROUP_INVALID) || ((fg_id >= fg_id_lower) && (fg_id <= fg_id_upper)))
            {
                 ...
                 * "fem_id", "fg_id", "input offset", "ignore actions'
                 */
                SHR_IF_ERR_EXIT(appl_dnx_action_swstate_fem_prg_display
                                (unit, fem_id_index, fem_program_index, 4, sand_control, prt_ctr));
                do_display = TRUE;
            }
        }
    }
