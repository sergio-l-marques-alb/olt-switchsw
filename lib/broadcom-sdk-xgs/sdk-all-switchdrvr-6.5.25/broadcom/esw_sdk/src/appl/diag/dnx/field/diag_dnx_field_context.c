    ...
    char *presel_qual_arg_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_map_cs_qual_map_get_dispatch(unit, field_stage, qual_data.qual_type, &cs_qual_map));

    nof_layer_records = dnx_data_field.stage.stage_info_get(unit, field_stage)->nof_layer_records;

    if (cs_qual_map.nof == 1 || cs_qual_map.index_shift == 0)
    {
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (field_stage == DNX_FIELD_STAGE_EXTERNAL)
    {
         ...
         */
#if defined(BCM_DNX2_SUPPORT) && defined(INCLUDE_KBP)
        int device_locked, external_tcam_exists;
        SHR_IF_ERR_EXIT(dnx_field_map_is_device_locked(unit, &device_locked));
        SHR_IF_ERR_EXIT(dnx_field_map_is_external_tcam_available(unit, &external_tcam_exists));
        if (external_tcam_exists && (device_locked == FALSE))
        {
