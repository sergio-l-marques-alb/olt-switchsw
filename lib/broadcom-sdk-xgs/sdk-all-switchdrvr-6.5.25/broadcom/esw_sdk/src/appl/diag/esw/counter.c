/*
 * $Id: counter.c,v 1.48 Broadcom SDK $
...
do_show_counters(int unit, soc_reg_t ctr_reg, soc_pbmp_t pbmp, int flags)
{
    ...
    if (soc_feature(unit, soc_feature_configure_loopback_port)) {
    ...
    if (soc->counter_interval == 0) {
	    ...
	    cli_out("NOTE: DMA counter collection is not active for ports %s\n",
...
lookup miss during tunnel decapsulation"},
    {"TxAdaptLookupMiss", "packets dropped due to egress adapt lookup miss"},
    {"TxSvpEsDrop", "packets dropped due to enabling ethernet segment \
filtering based on source virtual port"},
    {"TxL3EgressOrIntfRangeError", "packets dropped due to overlay and \
underlay L3 next hop and interface indices do not comply with configured \
allocation bitmap"},
    {"RxLinkDownDrop", "packets dropped due to link down or destination port \
disabled"},
    {"RxPartialLinkDownDrop", "non-UC packets dropped partially due to \
link down or destination port disabled"},
    {"RxVxlanVfiAssignFailDrop", "packets dropped due to VXLAN \
packets with VFI assignment failure"},
    {"RxVxlanTunnelErrorDrop", "packets dropped due to VXLAN tunnel \
termination errors"}
};
...
get_counter(char *c, int rx_tx, int *counter)
{
    ...
    if (rx_tx == RDBGC) {
        ...
        /* Specifically checking for snmpBcmCustomTransmit15 as it's not continuous enum */
        if (n == 15) {
            n = snmpBcmCustomTransmit15;
        } else {
            n += snmpBcmCustomTransmit0;
        }
        if (n < snmpBcmCustomTransmit0 ||
            ((n >= (snmpBcmCustomTransmit14 + 1)) &&
              n !=snmpBcmCustomTransmit15)) {
            return -1;
        }
    }
    ...
    if (rx_tx == RDBGC || rx_tx == BOTH) {
        ...
        for (trig_typ = 0, loop = 0; trig_typ < bcmDbgCntNum; trig_typ++) {
            if (BCM_SUCCESS(bcm_stat_custom_check(unit, dummy_port,
                                                  ...
                                                  trig_typ, &res))) {
                if (!about) {
                    cli_out("%-18s %s", _trig_names[trig_typ].name,
                            !((loop + 1) % 4) ? "\n  " : "");
                } else {
...
display_val(uint64 val, int count_index)
{
    ...
    if ((count_index >= snmpBcmCustomTransmit0 &&
        ...
        (count_index == snmpBcmCustomTransmit15)) {
        if (count_index == snmpBcmCustomTransmit0) {
            typ_s = "TX";
        }
        ...
        if (count_index == snmpBcmCustomTransmit15) {
            count_index = 15;
        } else {
 ...
 *      unit  - StrataSwitch PCI device unit number.
 *      port  - port number.
 *      index - Custom stat index
 *      mid   - Middle of the table to separate RX and TX display
 *      value - value to be displayed
 * Returns:
 *      None.
 */
static void
list_table_display(int unit, int port, int index, int mid, uint64 value)
{
    ...
    if (index >= snmpBcmCustomTransmit0 && mid) {
        cli_out("%s", line);
        mid = 0;
    }
    val_f = 1;
    ...
    for (trig = 0, i = 0; trig < bcmDbgCntNum; trig++) {
        conf = 0;
        rv = bcm_stat_custom_check(unit, port, index, trig, &conf);
        if (BCM_SUCCESS(rv) && conf) {
            if (val_f) {
                display_val(value, index);
                val_f = 0;
            }
            if (!((i + 1) % COL_SZ)) {
...
list_table(int unit, bcm_pbmp_t pbmp, int show_zero)
{
    ...
    DPORT_BCM_PBMP_ITER(unit, pbmp, dport, port) {
        ...
        for (n = snmpBcmCustomReceive0; n <= snmpBcmCustomReceive8; n++) {
            ...
            list_table_display(unit, port, n, 0, value);
        }
        ...
        for (n = snmpBcmCustomTransmit0; n <= snmpBcmCustomTransmit14; n++) {
            ...
            list_table_display(unit, port, n, mid, value);
            mid = 0;
        }
        /* snmpBcmCustomTransmit15 enum is not continuos with snmpBcmCustomTransmit14 */
        rv = bcm_stat_get(unit, port,snmpBcmCustomTransmit15, &value);
        if (BCM_SUCCESS(rv) && (COMPILER_64_IS_ZERO(value))) {
            list_table_display(unit, port, snmpBcmCustomTransmit15, 0, value);
        }
        if (end) {
            cli_out("%s", line);
            end = 0;
