...
static shr_error_e
diag_dnx_field_action_user_init(
    int unit)
{
...
static sh_sand_man_t Field_action_user_list_man = {
    ...
    .examples = ""
                "\n" "id=0-20 stage=IPMF1-IPMF3",
    .init_cb =  diag_dnx_field_action_user_init,
};
    ...
    prt_control_t * prt_ctr)
{
    ...
    for (i_act = 0; i_act < dnx_action_nof; i_act++)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_field_map_action_dnx_to_bcm_int
                        (unit, stage, dnx_action_list[i_act], &bcm_action, &bcm_action_found));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (stage_index = stage_lower; stage_index <= stage_upper; stage_index++)
    {
        ...
        SHR_IF_ERR_EXIT(diag_dnx_field_map_action_dnx_print(unit, match_n, field_stage, prt_ctr));
    }
    ...
    prt_control_t * prt_ctr)
{
    ...
    const dnx_field_action_map_t *action_map_p = NULL;
    shr_error_e rv;
    ...
    for (bcm_action = 0; bcm_action < bcmFieldActionCount; bcm_action++)
    {
        ...
        rv = dnx_field_map_action_bcm_to_dnx_int(unit, stage, FALSE, bcm_action, &dnx_action, &action_map_p);
        ...
        if (show_desc == TRUE)
        {
            PRT_CELL_SET("%s", bcm_action_description[bcm_action]);
        }
        PRT_CELL_SET("%d", size);
        ...
        else
        {
            ...
            for (bcm_action_raw = 0; bcm_action_raw < bcmFieldActionCount; bcm_action_raw++)
            {
                ...
                SHR_IF_ERR_EXIT(rv);
                if (BCM_TO_DNX_IS_BAISC_OBJ(action_map_p->flags) && (dnx_action == dnx_action_raw))
                {
             ...
             * the macro BCM_TO_DNX_IS_BAISC_OBJ and in such case we present current BCM action.
             */
            if (!raw_action_found)
            {
                PRT_CELL_SET("%s", bcm_action_n);
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (stage_index = stage_lower; stage_index <= stage_upper; stage_index++)
    {
        ...
        SHR_IF_ERR_EXIT(diag_dnx_field_map_action_bcm_print(unit, match_n, field_stage, show_desc, prt_ctr));
    }
    ...
    prt_control_t * prt_ctr)
{
    ...
    for (bcm_vw_act_id = action_id_lower; bcm_vw_act_id <= action_id_upper; bcm_vw_act_id++)
    {
        ...
        shr_error_e rv =
            dnx_field_map_action_bcm_to_dnx_int(unit, stage, FALSE, bcm_vw_act_id, &dnx_vw_action, &action_map_entry_p);
        if (rv == _SHR_E_PARAM || rv == _SHR_E_NOT_FOUND)
        {
            continue;
        }
        ...
        SHR_IF_ERR_EXIT(dnx_field_map_vw_action_info_get(unit, vw_act_id, &vw_action_info));
        SHR_IF_ERR_EXIT(dnx_field_map_per_vw_action_info_get(unit, vw_act_id, &per_vw_action_info));
        base_dnx_action = per_vw_action_info.base_dnx_action_per_stage[vw_action_info.action_index_within_vw][stage];
        ...
        PRT_CELL_SET("%d", vw_action_info.action_index_within_vw);
