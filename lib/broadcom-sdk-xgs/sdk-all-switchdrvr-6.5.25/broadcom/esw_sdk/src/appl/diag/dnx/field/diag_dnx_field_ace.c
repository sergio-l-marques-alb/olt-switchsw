    ...
    int unit)
{
    ...
    sal_strncpy_s((char *) (ace_format_info.name), "ACE_ENT_DIAG_INIT", sizeof(ace_format_info.name));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    char fes_mask_string_all_dashes[DNX_DATA_MAX_FIELD_ACE_NOF_FES_INSTRUCTION_PER_CONTEXT + 1] = { 0 };
    int fes_inst_iter;
    int fes_mask_usage_iter;
    int total_fes_mask_usage_iter;
    int fes_masks_not_presented;
    int fes_id_iter;
    int nof_fes_instructions_per_context = dnx_data_field.ace.nof_fes_instruction_per_context_get(unit);
    ...
    for (fes_id_iter = 0; fes_id_iter < nof_fes_instructions_per_context; fes_id_iter++)
    {
        fes_mask_string_all_dashes[fes_id_iter] = '-';
    }
    fes_mask_string_all_dashes[fes_id_iter] = '\0';
    ...
    sal_strncpy_s(total_fes_mask_string, fes_mask_string_all_dashes, sizeof(total_fes_mask_string));
    sal_strncpy_s(not_present_fes_mask_string, fes_mask_string_all_dashes, sizeof(not_present_fes_mask_string));
    ...
    for (format_id = format_id_lower; format_id <= format_id_upper; format_id++)
    {
        ...
        sal_strncpy_s(fes_mask_string, fes_mask_string_all_dashes, sizeof(fes_mask_string));
        for (fes_inst_iter = 0; fes_inst_iter < ace_format_id_info->actions_fes_info.nof_fes_instr; fes_inst_iter++)
        {
            /** Sanity check.*/
            if (ace_format_id_info->actions_fes_info.fes_instr[fes_inst_iter].fes_id >=
                nof_fes_instructions_per_context)
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "Reached EFES ID %d above maximum %d\n",
                             ace_format_id_info->actions_fes_info.fes_instr[fes_inst_iter].fes_id,
                             nof_fes_instructions_per_context - 1);
            }
            /**
    ...
    sh_sand_control_t * sand_control)
{
    ...
    PRT_COLUMN_ADD_FLEX(PRT_FLEX_ASCII, "ACE Format ID");
    PRT_COLUMN_ADD_FLEX(PRT_FLEX_ASCII, "Name");
    PRT_COLUMN_ADD_FLEX(PRT_FLEX_ASCII, "Entry Range");
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (fes_inst_iter = 0; fes_inst_iter < ace_format_info_p->actions_fes_info.nof_fes_instr; fes_inst_iter++)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_field_map_action_type_to_predef_dnx_action(unit, DNX_FIELD_STAGE_ACE,
                                                                       ace_format_info_p->
                                                                       actions_fes_info.fes_instr[fes_inst_iter].
                                                                       action_type, &dnx_action));
