...
#define PCIEFW_MEASURE_END(testname)
    ...
    if (!in) {
        ...
        cli_out("Error opening file: %s. PCIE firmware load failed.\n", file_str);
        rv = _SHR_E_EXISTS;
        goto error;
    }

    if(sal_fseek(in, 0, SEEK_END) != 0 || (len = sal_ftell(in)) == -1) {
        rv = _SHR_E_INTERNAL;
        goto error;
    }
...
_pcie_phy_fw_info(int unit, args_t *args, pciephy_access_t *sa)
{
    ...
    if(fwhdr.magic == PCIE_FW_MAGIC_NUM) {
        ver_maj = fwhdr.ldrver >>16;
        ver_min = fwhdr.ldrver & 0xffff;
...
#if defined(BCM_CMICX_SUPPORT)
            addr = soc_iproc_percore_membase_get(unit, 2) + TRACE_ADDR;
            /* Dump raw log. */
            if (dump_raw) {
...
_pcie_phy_fw_version(int unit, pciephy_access_t *sa)
{
    ...
    if(fwhdr.magic == PCIE_FW_MAGIC_NUM) {
        cli_out("\tPCIe FW loader version: %u.%u\n\tPCIe FW version: %s\n",
                                                  fwhdr.ldrver >>16,
                                                  fwhdr.ldrver & 0xffff,
                                                  fwhdr.version);
    }
