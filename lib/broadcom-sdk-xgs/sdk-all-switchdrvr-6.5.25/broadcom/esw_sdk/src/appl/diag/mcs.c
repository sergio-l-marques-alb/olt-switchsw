...
mcs_file_load(int unit, FILE *fp, int ucnum, uint32 *lowaddr, int stage, uint32 twostageaddr)
{
    ...
    while (NULL != sal_fgets(input, sizeof(input) - 1, fp)) {
        cp = &input[0];
        if (input[0] == 'S') {
        ...
        if ((stage == 0) && (addr >= twostageaddr)) {
            /* No Need to parse the remaining now... */
            return CMD_OK;
        }
    ...
    defined(BCM_FIREBOLT6_SUPPORT)
                    ...
                    if (SOC_IS_DNX(unit)) {
                        if (soc_feature(unit, soc_feature_iproc_17)) {
                            if (uc < 2) {
                                addr = 0x50800000 + 0x00060000 * uc;
                            } else {
                                addr = 0x51800000 + 0x00060000 * (uc-2);
                            }
                        } else {
                            addr = 0x01100000 + 0x00060000 * uc;
                        }
