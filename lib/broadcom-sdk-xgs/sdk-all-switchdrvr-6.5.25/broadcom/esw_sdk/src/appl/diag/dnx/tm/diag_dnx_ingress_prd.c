 ...
 * \param [in] ignore_drop - if true then dont return drop count
 *
 * \return
 *   int - see _SHR_E_*
 *
    ...
    int ignore_drop)
{
    ...
    uint64 fifo_size_64;
    uint64 port_64;
    uint32 rmc_fifo_resolution, max_threshold;
    ...
    if (ignore_drop)
    {
        sal_sprintf(drop_str, "GLOBAL TABLE BELOW");
    }
    else
    {
    ...
    prt_control_t * prt_ctr)
{
     ...
     */
    SHR_IF_ERR_EXIT(sh_dnx_ingress_port_drop_add_row(unit, port, 0 /* hrf_idx */ , src_prio, sch_prio, hrf_size, drop_count, header_str, prt_ctr,       /* ignore_drop 
                                                                                                                                                         */ 0));
    ...
    prt_control_t * prt_ctr)
{
    ...
    uint32 src_prio, fifo_size, ignore_drop;
    uint64 drop_count = COMPILER_64_INIT(0, 0);
    bcm_port_nif_scheduler_t sch_prio;
    char header_str[DNX_DIAG_PORT_PRD_MAX_STR_LEN] = "\0";
    ...
    /*
     * if OFR is supported then don't get prd drop count
     */
    ignore_drop = dnx_data_nif.ofr.feature_get(unit, dnx_data_nif_ofr_is_supported) ? 1 : 0;
