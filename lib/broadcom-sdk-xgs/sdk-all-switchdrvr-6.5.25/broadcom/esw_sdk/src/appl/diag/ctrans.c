...
{
    int payload_len, len, len_str, dstlen;
    uint32 pkt_flags = 0;
    ...
    if (alloc_hdr_here) {
        dstlen = len - CPUTRANS_HEADER_BYTES - ECHO_STR_OFFSET;
    } else {
        dstlen = len - ECHO_STR_OFFSET;
    }

    sal_strlcpy((char *)&payload_buf[ECHO_STR_OFFSET], str, dstlen);
