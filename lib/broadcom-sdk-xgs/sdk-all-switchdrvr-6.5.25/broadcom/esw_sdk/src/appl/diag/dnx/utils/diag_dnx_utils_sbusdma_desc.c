    ...
    sh_sand_control_t * sand_control)
{
    ...
    else
    {
        ...
        /*
         * Sleep to allow the timeout thread to self-terminate
         */
        sal_usleep(10000);
    }
...
static sh_sand_option_t dnx_sbusdma_desc_timeout_options[] = {
    {"ENAble", SAL_FIELD_TYPE_BOOL, "Enable timeout thread utilization", "yes"},
    {NULL}
};

static sh_sand_man_t dnx_sbusdma_desc_timeout_man = {
    .brief = "Enable or disable the descriptor DMA timeout mechanism.",
    .full =
        "To enable/disable descriptor DMA timeout mechanism, respectively call with enable=yes/no."
        "Note this does not allocate/free resources, simply avoids interacting with mutexes and sempahores.",
    .synopsis = "descDMA timeout [ENAble=<yes/no>]",
    .examples = "enable=no\n" "enable=yes"
};

/**
* \brief
* Change the desriptor DMA timeout enabled configuration
*****************************************************/
static shr_error_e
cmd_sbusdma_desc_timeout_set(
    int unit,
    args_t * args,
    sh_sand_control_t * sand_control)
{
    ...
    SHR_IF_ERR_EXIT(dnx_sbusdma_desc_timeout_enable(unit, enable));

    LOG_INFO(BSL_LOG_MODULE,
             (BSL_META_U(unit, "Descriptor DMA timeout mechanism successfully %s\n"),
...
/**
* \brief
* Print the descriptor DMA configuration and statistics
*****************************************************/
static shr_error_e
    ...
    sh_sand_control_t * sand_control)
{
    ...
    PRT_COLUMN_ADD("Timeout enabled");
    ...
    PRT_CELL_SET("%d", desc_stats.timeout_enabled);
...
sh_sand_cmd_t sh_dnx_utils_sbusdma_desc_cmds[] = {
    ...
    *************************************************************************************************************************************/
    {"ENAble", cmd_sbusdma_desc_enable_set, NULL, dnx_sbusdma_desc_enable_options, &dnx_sbusdma_desc_enable_man, NULL,
     NULL, SH_CMD_NO_XML_VERIFY * SH_CMD_SKIP_MEMORY_CHECK}
    ,
    {"timeout", cmd_sbusdma_desc_timeout_set, NULL, dnx_sbusdma_desc_timeout_options, &dnx_sbusdma_desc_timeout_man,
     NULL,
     NULL, SH_CMD_NO_XML_VERIFY * SH_CMD_SKIP_MEMORY_CHECK}
    ,
