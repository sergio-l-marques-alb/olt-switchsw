    ...
    if (is_acc)
    {
        ...
        if (!is_rfc_6374)
        {
                ...
                endpoint_info.flags2 & BCM_OAM_ENDPOINT_FLAGS2_EGRESS_INJECTION_DOWN_IVEC_DELETE_TWO_VLAN_TAGS)
            {
                ...
                sal_sprintf(display_str, "%d - Delete two VLAN tags", (int) ivec);
                PRT_ROW_ADD(PRT_ROW_SEP_NONE);
                     ...
                     endpoint_info.flags2 & BCM_OAM_ENDPOINT_FLAGS2_EGRESS_INJECTION_DOWN_IVEC_DELETE_ONE_VLAN_TAG)
            {
                ...
                sal_sprintf(display_str, "%d - Delete one VLAN tag", (int) ivec);
                PRT_ROW_ADD(PRT_ROW_SEP_NONE);
                PRT_CELL_SET("%s", "Down Egress Inj IVEC");
                PRT_CELL_SET("%s", display_str);
            }
    ...
    int *num_of_counters)
{
    ...
    SHR_IF_ERR_EXIT(dnx_crps_mgmt_nof_counters_per_engine
                    (unit, 0 /* A's engine id */ , format_type, &nof_counters_in_a_engine));
    SHR_IF_ERR_EXIT(dnx_crps_mgmt_nof_counters_per_engine
                    (unit, first_b_engine_id, format_type, &nof_counters_in_b_engine));
    SHR_IF_ERR_EXIT(dnx_crps_mgmt_nof_counters_per_engine
                    (unit, first_c_engine_id, format_type, &nof_counters_in_c_engine));
 ...
 * Otherwise, valid bits of pkt counter is 34
 */
#define CPY_COUNTER_VALUE_FROM_HW(_DST_CNT, _ENGINE_ID, _INNER_HW_IDX, _PCNT_VAL)                    \
{                                                                                                    \
    ...
    if (_ENGINE_ID >= first_c_engine_id && _INNER_HW_IDX == 3) {                                     \
        COMPILER_64_ADD_32(_DST_CNT, ((_PCNT_VAL[_INNER_HW_IDX] >> (_INNER_HW_IDX*2)) & 0x3FFFF));   \
    } else {                                                                                         \
        COMPILER_64_ADD_32(_DST_CNT, (_PCNT_VAL[_INNER_HW_IDX] >> (_INNER_HW_IDX*2)));               \
    ...
    nof_c_engines = dnx_data_crps.engine.nof_big_engines_get(unit);

    SHR_IF_ERR_EXIT(dnx_crps_mgmt_nof_counters_per_engine
                    (unit, first_c_engine_id, bcmStatCounterFormatPackets, &nof_of_counter_c_engine));
    ...
    ing_entry_index = counter_info.ing_index >> 2;
    eg_entry_index = counter_info.eg_index >> 2;
    ...
    if (counter_info.ing_engine_id >= first_c_engine_id)
    {
        ...
        ing_array_index = (counter_info.ing_index >= nof_of_counter_c_engine) ? (ing_engine * 2 + 1) : (ing_engine * 2);
        eg_array_index = (counter_info.eg_index >= nof_of_counter_c_engine) ? (eg_engine * 2 + 1) : (eg_engine * 2);
        if (soc_is(unit, JERICHO2_ONLY_DEVICE))
        {
            ing_array_index =
                (counter_info.ing_index >= nof_of_counter_c_engine) ? (ing_engine + nof_c_engines) : (ing_engine);
            eg_array_index =
                (counter_info.eg_index >= nof_of_counter_c_engine) ? (eg_engine + nof_c_engines) : (eg_engine);
        }
    }
    else
    {
    ...
    inner_hw_index = endpoint_info.lm_counter_base_id % 4;
    ...
    if (oam_cntrs_results->endpoint_results.is_pcp)
    {
        for (pcp_index = 1; pcp_index < 4; pcp_index++)
        {
            CPY_COUNTER_VALUE_FROM_HW(oam_cntrs_results->endpoint_results.ing_counter_value[pcp_index],
                                      counter_info.ing_engine_id, pcp_index, ing_entry_above_64);
            CPY_COUNTER_VALUE_FROM_HW(oam_cntrs_results->endpoint_results.eg_counter_value[pcp_index],
                                      counter_info.ing_engine_id, pcp_index, eg_entry_above_64);
        }
        SHR_IF_ERR_EXIT(soc_mem_array_read
        ...
        {
            CPY_COUNTER_VALUE_FROM_HW(oam_cntrs_results->endpoint_results.ing_counter_value[pcp_index + 4],
                                      counter_info.ing_engine_id, pcp_index, ing_entry_above_64);
            CPY_COUNTER_VALUE_FROM_HW(oam_cntrs_results->endpoint_results.eg_counter_value[pcp_index + 4],
                                      counter_info.ing_engine_id, pcp_index, eg_entry_above_64);
        }
    }
exit:
