    ...
    uint32 *data_size)
{
    int lsnibble;
    int word;
    char data_str_local[SH_SAND_MAX_TOKEN_SIZE];
    SHR_FUNC_INIT_VARS(unit);

    remove_spaces_from_str(data_str, data_str_local);
    *data_size = 0;
    lsnibble = sal_strlen(data_str_local);
    for (word = 0; word < PPARSE_MAX_DATA_WORDS; word++)
    {
        ...
        {
            int nibval = nibble_value(data_str_local[msnibble]);
            if (nibval < 0)
            {
    ...
    sh_sand_control_t * sand_control)
{
    ...
    uint32 max_packet_size = 16380; /** 16,380 Bytes for Jumbo packets */
    uint32 raw_header_size = 0;

    PRT_INIT_VARS;
    SHR_FUNC_INIT_VARS(unit);

    SHR_ALLOC_SET_ZERO_ERR_EXIT(raw_header, max_packet_size * sizeof(char), "Raw_header, allocating max header length",
                                "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
    LOG_CLI(("\n"));
    ...
    RHITERATOR(obj, pit)
    {
        ...
        /*
         * Accumulate raw header string:
         * Assuming all headers are bytes aligned, and no padding in each header print
         * It is than possible to concatenate the headers string to a raw string
         */
        data_str_len = sal_strlen(obj->print_value);
        sal_strncpy(&raw_header[raw_header_size], obj->print_value, data_str_len + 1);
        raw_header_size += data_str_len;
    }

    PRT_INFO_ADD("Total headers stack size: %dB", (size_acc >> 3));
    PRT_INFO_ADD("Packet Raw data = %s", raw_header);
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SH_SAND_GET_ITERATOR(sand_arg)
    {
        ...
        pparse_packet_info_t packet_info = { {0} };
...
#ifdef BCM_DNX_SUPPORT
        if ((SOC_IS_DNX(unit) && dnx_data_sat.general.feature_get(unit, dnx_data_sat_general_is_sat_supported) == 0))
        {
            SHR_CLI_EXIT(_SHR_E_NONE, "SAT is not support on this device\n");
        }
#endif
