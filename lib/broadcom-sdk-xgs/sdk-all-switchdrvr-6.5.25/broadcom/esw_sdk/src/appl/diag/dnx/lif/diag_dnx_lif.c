...
typedef struct alloc_sim_prop_s
{
    ...
    int res_type;
    dnx_algo_local_outlif_logical_phase_e phase_id;
    uint8 with_ll;
    ...
    sh_sand_control_t * sand_control)
{
    ...
    int nof_outlifs_per_eedb_bank, nof_allocated_elements, eedb_bank_size;
    ...
    PRT_COLUMN_ADD("EEDB banks used for data (entries used)");

    eedb_bank_size = dnx_data_mdb.dh.macro_type_info_get(unit, MDB_EEDB_BANK)->nof_rows;
    ...
    for (outlif_bank = 0; outlif_bank < nof_outlif_banks; outlif_bank++)
    {
        ...
        if (assigned == DNX_ALGO_OUTLIF_BANK_ASSIGNED_TO_EEDB)
        {
                 ...
                 current_eedb_bank_array_index++)
            {
                ...
                {
                    sal_snprintf(eedb_banks, strn_size, "%s%d (%d/%d) ", eedb_banks, current_eedb_bank,
                                 nof_allocated_elements, eedb_bank_size);
                }
            }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    uint32 phase_id, first_bank_for_phase = 0, last_bank_for_phase = 0, first_bank_for_phase_no_ll =
    ...
    SHR_IF_ERR_EXIT(local_outlif_info.logical_phase_info.
                    first_no_ll_bank.get(unit, phase_id, &first_bank_for_phase_no_ll));
    SHR_IF_ERR_EXIT(local_outlif_info.logical_phase_info.
                    last_no_ll_bank.get(unit, phase_id, &last_bank_for_phase_no_ll));
    ...
    for (current_bank = first_bank_for_phase_no_ll; current_bank <= last_bank_for_phase_no_ll; current_bank++)
    {
        SHR_IF_ERR_EXIT(local_outlif_info.outlif_bank_info.in_use.get(unit, current_bank, &in_use));
        if (!in_use)
        {
        ...
        if (bank_logical_phase == phase_id)
        {
                 ...
                 current_eedb_bank_array_index++)
            {
                ...
                if (current_eedb_bank != (uint8) DNX_ALGO_LOCAL_OUTLIF_INVALID_BANK)
                {
                    ...
                    SHR_IF_ERR_EXIT(lif_mngr_db.egress_local_lif.nof_allocated_elements_in_range_get(unit, current_bank,
                                                                                                     OUTLIF_BANK_FIRST_INDEX
                                                                                                     (unit,
                                                                                                      current_bank) +
                                                                                                     nof_outlifs_per_eedb_bank
    ...
    if (last_bank_for_phase >= first_bank_for_phase)
    {
        PRT_INFO_ADD("Cluster mapped outlif banks available to phase %d: {%d-%d}", (phase_id + 1),
                     first_bank_for_phase, last_bank_for_phase);
    }
    if (last_bank_for_phase_no_ll >= first_bank_for_phase_no_ll)
    {
        PRT_INFO_ADD("Cluster mapped outlif banks above 512k (no link list) available to phase %d: {%d-%d}",
                     (phase_id + 1), first_bank_for_phase_no_ll, last_bank_for_phase_no_ll);
    }
    if ((last_bank_for_phase < first_bank_for_phase) && (last_bank_for_phase_no_ll < first_bank_for_phase_no_ll))
    {
        /*
         * If first_bank_for_phase > last_bank_for_phase - that's the lif manager indication that there're no available 
         * cluster-based resources for this phase
         */
        PRT_INFO_ADD("Cluster mapped outlif banks available to phase %d: None", (phase_id + 1));
    }
    ...
    alloc_sim_prop_t * alloc_sim_prop)
{
    ...
    if ((alloc_sim_prop->phase_id < DNX_ALGO_LOCAL_OUTLIF_LOGICAL_PHASE_FIRST)
        || (alloc_sim_prop->phase_id >= DNX_ALGO_LOCAL_OUTLIF_LOGICAL_PHASE_COUNT))
    {
    ...
    if (!found)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Illegal combination: %s, %s, for logical phase #%d",
    ...
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "LL mismatch for combination %s, %s, for logical phase #%d",
                     dbal_logical_table_to_string(unit, alloc_sim_prop->table_id), dbal_result_type_to_string(unit,
                                                                                                              alloc_sim_prop->table_id,
                                                                                                              alloc_sim_prop->res_type),
                     alloc_sim_prop->phase_id + 1);
    ...
    int *success)
{
    ...
    for (comb_idx = 0; comb_idx < num_requested_comb; comb_idx++)
    {
        ...
        outlif_info.logical_phase = alloc_sim_prop[comb_idx].phase_id;
    ...
    int *is_full_match)
{
    ...
    *is_full_match = 0;
    ...
    for (iter = curr_table_id; iter < dnx_data_dbal.table.nof_dynamic_and_static_tables_get(unit); iter++)
    {
        ...
        if (!sal_strncasecmp(substr_match, table->table_name, sizeof(table->table_name)))
        {
            *is_full_match = 1;
            *table_id = iter;
            break;
    ...
    sh_sand_control_t * sand_control)
{
    ...
    while (!is_end)
    {
        ...
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                        (unit, entry_handle_id, DBAL_FIELD_NOF_REQUESTED_LIFS, INST_SINGLE, &nof_requested_lifs));
        SHR_IF_ERR_EXIT(dbal_iterator_get_next(unit, entry_handle_id, &is_end));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SH_SAND_GET_ENUM("direction", dir);

    if (dnx_data_lif.global_lif.global_lif_bta_max_val_get(unit))
    {
        if ((dir == (DNX_ALGO_LIF_INGRESS | DNX_ALGO_LIF_EGRESS)) || (dir == 0))
        {
            LOG_CLI((BSL_META_U(unit, "\nGlobal lif symmetric info\n")));
            LOG_CLI((BSL_META_U(unit, "=========================\n")));
            SHR_IF_ERR_EXIT(lif_mngr_db.global_lif_allocation.symmetric_global_lif.print(unit));
        }

        if ((dir == (DNX_ALGO_LIF_INGRESS)) || (dir == 0))
        {
            LOG_CLI((BSL_META_U(unit, "\nGlobal lif ingress info\n")));
            LOG_CLI((BSL_META_U(unit, "=======================\n")));
            SHR_IF_ERR_EXIT(lif_mngr_db.global_lif_allocation.ingress_global_lif.print(unit));
        }

        if ((dir == (DNX_ALGO_LIF_EGRESS)) || (dir == 0))
        {
            LOG_CLI((BSL_META_U(unit, "\nGlobal lif egress info\n")));
            LOG_CLI((BSL_META_U(unit, "======================\n")));
            SHR_IF_ERR_EXIT(lif_mngr_db.global_lif_allocation.egress_global_lif.print(unit));
        }
    }

    LOG_CLI((BSL_META_U(unit, "\n\n")));

    PRT_TITLE_SET("Valid range per LIF type");
    PRT_COLUMN_ADD("LIF type");
    PRT_COLUMN_ADD("Range");

    PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE);

    PRT_CELL_SET("%s", "RIF_GLOBAL_LIF");
    PRT_CELL_SET("%d - %d", 0, dnx_data_l3.rif.nof_rifs_get(unit));
    PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE);

    PRT_CELL_SET("%s", "L2_GPORT_GLOBAL_LIF");
    PRT_CELL_SET("%d - %d", dnx_data_l3.rif.nof_rifs_get(unit),
                 dnx_data_lif.global_lif.nof_global_l2_gport_lifs_get(unit));
    PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE);

    PRT_CELL_SET("%s", "PHP_GPORT_GLOBAL_LIF");
    if (dnx_data_lif.global_lif.nof_global_in_lifs_get(unit) - dnx_data_mpls.general.nof_php_gport_lif_get(unit) > 0)
    {
        PRT_CELL_SET("%d - %d",
    ...
    {
        PRT_CELL_SET("no applicable range");
    }
    PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE);
    ...
    {
        range_start = dnx_data_lif.global_lif.nof_global_l2_gport_lifs_get(unit);
    }
    PRT_CELL_SET("%s", "No type");
    PRT_CELL_SET("%d - %d", range_start,
                 dnx_data_lif.global_lif.nof_global_in_lifs_get(unit) -
                 dnx_data_mpls.general.nof_php_gport_lif_get(unit));
...
static sh_sand_enum_t sh_dnx_global_lif_direction_enum_table[] = {
    {"ingress", DNX_ALGO_LIF_INGRESS, "Ingress"},
    {"egress", DNX_ALGO_LIF_EGRESS, "Egress"},
    {"symmetric", DNX_ALGO_LIF_EGRESS | DNX_ALGO_LIF_INGRESS, "Symmetric"},
    {"all", 0, "All directions"},
    {NULL}
};

static sh_sand_option_t sh_dnx_global_lif_options[] = {
    {"Direction", SAL_FIELD_TYPE_ENUM, "Global Lif direction", "all",
     (void *) sh_dnx_global_lif_direction_enum_table},
    {NULL}
};

static sh_sand_man_t sh_dnx_global_lif_man = {
    .brief = "Display global LIF allocation info.",
    .examples = "Direction=ingress"
};

static sh_sand_cmd_t sh_dnx_global_lif_cmds[] = {
    /*
     * keyword, action, command, options, man
     */
    {"status", dnx_global_lif_dump_cmd, NULL, sh_dnx_global_lif_options, &sh_dnx_global_lif_man},
...
sh_sand_cmd_t sh_dnx_lif_cmds[] = {
    ...
    {"GlobalLIF", NULL, sh_dnx_global_lif_cmds, NULL, &sh_dnx_global_lif_man, NULL, NULL, SH_CMD_NO_XML_VERIFY},
    {NULL}
};
