...
STATIC int             pw_recover(int unit);
#endif
 ...
 */
{
    ...
    int cos_bmp = 0;
#if defined(BCM_PETRA_SUPPORT) || defined(BCM_DNX_SUPPORT)
    int chan_count = 0; /* Counter of DMA channels for RX */
    int cos_ctrl_index;
    uint32 cos_ctrl_value; /* Record DMA cos value per channel */
    dma_chan_t rx_chan[CMICX_N_DMA_CHAN] = {0}; /* Record RX channel ID from bitmap pu->pu_channel */
#endif
    ...
    for (i = 0; i < dma_chan; i++) {
#if defined(BCM_PETRA_SUPPORT) || defined(BCM_DNX_SUPPORT)
        /* Clear DMA cos on each RX channel */
        if (SOC_IS_ARAD(unit) || SOC_IS_DNX(unit))
        {
            soc_dma_chan_cos_ctrl_set(unit, i, 1, 0);
            soc_dma_chan_cos_ctrl_set(unit, i, 2, 0);
        }
#endif
        if (pu->pu_channel & PU_CHANNEL(i)) {
            ...
            if (SOC_IS_ARAD(unit) || SOC_IS_DNX(unit))
            {
                /* Record the rx channel ID */
                rx_chan[chan_count] = i;
                /* Record total number of rx channel */
                chan_count += 1;
            }
#endif
        } else if (tx == -1) {
    ...
    if (SOC_IS_ARAD(unit) || SOC_IS_DNX(unit))
    {
        /* Traverse DMA cos, make sure it's enabled on a RX channel */
        if (chan_count)
        {
            for (cos_ctrl_index = 0; cos_ctrl_index < 64; cos_ctrl_index ++)
            {
                /* Choose a RX channel */
                i = cos_ctrl_index % chan_count;

                if (cos_ctrl_index < 32)
                {
                    soc_dma_chan_cos_ctrl_get(unit, rx_chan[i], 1, &cos_ctrl_value);
                    cos_ctrl_value |= (1 << cos_ctrl_index);
                    soc_dma_chan_cos_ctrl_set(unit, rx_chan[i], 1, cos_ctrl_value);
                }
                else
                {
                    soc_dma_chan_cos_ctrl_get(unit, rx_chan[i], 2, &cos_ctrl_value);
                    cos_ctrl_value |= (1 << (cos_ctrl_index - 32));
                    soc_dma_chan_cos_ctrl_set(unit, rx_chan[i], 2, cos_ctrl_value);
                }
            }
        }
    }
    ...
    for (i = 0; i < dma_chan; i++) {
        ...
        if (pu->pu_channel & PU_CHANNEL(i)) {
            ...
            if (cfg->chan_cfg[i].chains == 0) {
                ...
                if (SOC_IS_ARAD(unit) || SOC_IS_DNX(unit))
                {
                    /* Set number of chains to minimum value due to limitation of RX pool size in the case the multiple RX chains */
                    cfg->chan_cfg[i].chains = 1;
                }
                else
#endif
                {
    ...
    if ((tx == -1) ||
        ((rv = soc_dma_chan_config(unit, tx, DV_TX, SOC_DMA_F_INTR |SOC_DMA_F_DEFAULT)) < 0)) {
...
/*
 * Function:   pw_recover
 * Purpose:    recover origianl CPU RX.
 * Parameters: unit - unit to configure.
 * Returns:    0 - success
 *             <0 - failed.
 */
...
#if defined(BCM_PETRA_SUPPORT)
    if (SOC_IS_ARAD(unit)) {
        if ((rv = appl_dcmn_rx_activate(unit, 0)) < 0) {
 ...
 */
{
    ...
    while ((c = ARG_GET(a)) != NULL) {
        if (!sal_strcasecmp(c, "start")) {
            ...
            if (pu->pu_flags & PU_F_RUN) {
                cli_out("%s: Unable to reset configuration while running\n", pw_name[unit]);
                return(CMD_FAIL);
            } else {
                pu->init_done = FALSE;
                pw_init(unit);
            ...
            if (ARG_CNT(a) == 0) {
                ...
                cli_out("%s: Unable to configure DMA channels while running\n", pw_name[unit]);
                return(CMD_FAIL);
            }
            ...
            if (ARG_CNT(a)) {
                while ((c = ARG_CUR(a)) != NULL && !parse_mask(c, pw_report_table, &pu->pu_report)) {
                    ARG_NEXT(a); /* Bump arg if matched report */
                }
            } else { /* Print values */
 ...
 * Parameters:  unit - unit #
 *              n - number of packets to dump, -n means last "n" packets.
 * Returns:     CMD_OK - success
 */
{
    pwu_t *pu = &pw_units[unit];
