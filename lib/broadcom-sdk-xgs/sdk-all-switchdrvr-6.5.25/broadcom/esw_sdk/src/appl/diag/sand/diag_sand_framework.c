...
sh_sand_option_list_process_recursive(
    ...
    int flags)
{
    sh_sand_option_t *option_p, *tmp_options;

    SHR_FUNC_INIT_VARS(unit);

    for (option_p = sh_sand_cmd->options;
         (option_p != NULL) && ((option_p->keyword != NULL) || option_p->inherited_options != NULL); option_p++)
    {
        if (option_p->inherited_options)
        {
             ...
             */
            tmp_options = sh_sand_cmd->options;
            sh_sand_cmd->options = option_p->inherited_options;
            SHR_CLI_EXIT_IF_ERR(sh_sand_option_list_process_recursive(unit, core, args, sh_sand_cmd, sand_ctr, flags),
                                "");
            sh_sand_cmd->options = tmp_options;
        }
        else
        {
    ...
    int flags)
{
    ...
    sh_sand_option_list_process_recursive(unit, core, args, sh_sand_cmd, sand_ctr, flags);
    ...
    int prefix_lookup)
{
    ...
    if ((sh_sand_cmd != NULL) && SHR_FUNC_VAL_IS(_SHR_E_NONE))
    {
        ...
        if (utilex_is_prime_flag_set(sh_sand_cmd->flags, SH_CMD_CONDITIONAL) && (sh_sand_cmd->invoke_cb != NULL) &&
            ...
            ((SOC_FLAGS_GET(unit) & SOC_F_ALL_MODULES_INITED) && sh_sand_cmd->invoke_cb(unit, NULL) != _SHR_E_NONE))
        {
