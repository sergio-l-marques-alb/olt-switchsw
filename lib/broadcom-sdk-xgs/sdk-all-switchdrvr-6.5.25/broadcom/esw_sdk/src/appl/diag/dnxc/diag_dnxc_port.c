...
#ifdef BCM_DNX_SUPPORT
    if (SOC_IS_DNX(unit) && !dnx_init_is_init_done_get(unit))
    {
        SHR_EXIT();
    }
#endif
    ...
    sh_sand_control_t * sand_control)
{
    ...
    BCM_PBMP_ITER(pbm, port)
    {
        ...
        pname = lnstr + sal_strnlen(lnstr, sizeof(lnstr));
        /*
         * Append first phy ID
         */
        sal_snprintf(pname, sizeof(lnstr) - sal_strnlen(lnstr, sizeof(lnstr)) + 1, "/%d", first_phy);
    ...
    sal_usecs_t start_time)
{
    ...
    if (is_lane_port && BCM_PHY_GPORT_IS_LANE(port))
    {
...
#ifdef BCM_DNX_SUPPORT
    SH_SAND_IS_PRESENT("direction", is_dt_present);
    /*
     * For J2P, use tx direction to measure the serdes rate by default.
     */
    if (SOC_IS_DNX(unit) && !is_dt_present &&
        !dnx_data_synce.general.feature_get(unit, dnx_data_synce_general_measure_rate_on_down_port_is_supported))
    {
        is_rx = 0;
        ...
        else
        {
            ...
            if (DNXC_PORT_PHY_SPEED_IS_PAM4(lane_rate))
            {
                    ...
                    && !(BCM_PORT_RESOURCE_PHY_LANE_CONFIG_FORCE_ES_GET(resource->phy_lane_config)))
                {
                    ...
                    cli_out
                        ("WARNING! Either NS or ES should be enabled when link training is disabled with PAM4 modulation. Therefore => enabling NS on port %d so we can disable CL72\n",
                         port);
                }
            }
        }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    int interval = 0;
#ifdef BCM_DNX_SUPPORT
    int port, is_over_fabric;
    int nof_aggregated_pms;
    portmod_pm_identifier_t controlled_pms[PORTMOD_MAX_ILKN_AGGREGATED_PMS];
#endif
    int rv;
    ...
    BCM_PBMP_ITER(pbmp, port)
    {
        if (IS_IL_PORT(unit, port))
        {
            ...
            SHR_IF_ERR_EXIT(dnx_algo_port_ilkn_core_aggregated_pms_get
                            (unit, port, is_over_fabric, &nof_aggregated_pms, controlled_pms));
            if (nof_aggregated_pms > 1)
            {
                SHR_CLI_EXIT(_SHR_E_PARAM, "BER projection doesn't support working on ILKN(%d) across 2 PMs.\n", port);
            }
        }
    }
#endif
    ...
    sh_sand_control_t * sand_control)
{
    ...
    bcm_port_t port;
    portmod_access_get_params_t params;
    phymod_linkCAT_op_mode_t linkcat_mode;
    phymod_linkcat_config_t linkcat_config;
    phymod_phy_access_t phy_access[DIAG_DNXC_MAX_CORE_ACCESS_STRUCTURES_PER_PORT];
    ...
    BCM_PBMP_ITER(logical_ports, port)
    {
        ...
        phymod_linkcat_config_t_init(&linkcat_config);
        linkcat_config.mode = linkcat_mode;
        SHR_IF_ERR_EXIT(phymod_phy_linkcat(phy_access, linkcat_config));
    }
exit:
    if ((_func_rv = dnxc_phy_diag_linkscan_recover(unit, interval)) < 0)
    {
...
static sh_sand_man_t dnxc_phy_cl72_man = {
    ...
    .full     = "Turn on or Turn off PHY CL72 for given ports.\n"
                ...
                "CL72 status.\n"
                "Setting cl72 to \'on\' will use \'auto detect\' for ES/NS selection.\n"
                "Setting cl72 to \'off\' will force NS channel.",
#ifdef BCM_DNX_SUPPORT
#ifdef INCLUDE_CTEST
...
static sh_sand_man_t dnxc_phy_raw_c45_man = {
    ...
    .full     = "Set or Get phy registers via clause 45 MDIO. "
                "If option \"data\" is not present, it a read register operation.\n"
                "Note: If the phy register is per pll or per lane, user should first configure which pll id or lane id will be used in the next access. "
                "In blackhawk pmd, this is done by setting the relevant bit in regad=0xffde, according to: <0:7-lane | 8-pll_index>\n"
                "So for example, in order to access pll1 of phy_id 0xa0 (PMD only), user should first execute: \'phy raw c45 phy_id=0xa0 devad=1 regad=0xffde data=0x100\'.",
...
static sh_sand_option_t dnxc_phy_raw_c45_options[] = {
    /*name           type                  desc          default    ext*/
    {"phy_id",    SAL_FIELD_TYPE_UINT32, "PHY ID (according to column \"ADDR\" of \"phy info\" diagnostic)", NULL, NULL},
    {"dev_addr",  SAL_FIELD_TYPE_UINT32, "Device Address (pmd=1/PCS=0)", NULL, NULL},
...
static sh_sand_option_t dnxc_phy_raw_sbus_options[] = {
    /*name           type                  desc          default    ext*/
    {"pm_id",     SAL_FIELD_TYPE_UINT32, "PM ID (according to column \"PM ID\" of \"port portmod info\" diagnostic)", NULL, NULL},
    {"dev_addr",  SAL_FIELD_TYPE_UINT32, "Device Address (pmd=1/PCS=0)", NULL, NULL},
    {"reg_addr",  SAL_FIELD_TYPE_UINT32, "Register Address", NULL, NULL},
...
static sh_sand_man_t dnxc_phy_ber_proj_man = {
    ...
    .full     = "This diag is used to calculate Post FEC BER projection. "
                ...
                "3. The option \'hist_errcnt_threshold\' is meaningless for Blackhawk 7nm devices.\n"
                "4. BER projection doesn't support working on ILKN across 2 PMs.",
...
static sh_sand_enum_t phy_linkcat_mode_option_enum_table[] = {
    {"rx",   phymodlinkCAT_RX_MODE, "LinkCAT mode: RX."},
    {"lpbk", phymodlinkCAT_LPBK_MODE, "LinkCAT mode: lPBK."},
    {"tx",   phymodlinkCAT_TX_MODE, "LinkCAT mode: TX."},
    {NULL}
};
