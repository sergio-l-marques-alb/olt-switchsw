  ...
  )
{
    ...
    if ((socLbObjModem == lb_key->obj_type) || (socLbObjAll == lb_key->obj_type)) {
        if ((lb_key->modem_id != -1) && (lb_key->modem_id > SOC_TMC_LB_MODEM_MAX)) {
  ...
  )
{
    ...
    sal_snprintf(counter_name, sizeof(counter_name), "RLBPKT");
    _dpp_lb_counter_item_print(counter_name, print_none_zero, rx_global_counter->lb_pkts);
    sal_snprintf(counter_name, sizeof(counter_name), "RBypassPKT");
    _dpp_lb_counter_item_print(counter_name, print_none_zero, rx_global_counter->bypass_pkts);
  ...
  )
{
    ...
    sal_snprintf(counter_name, sizeof(counter_name), "TCtrlPKT");
    _dpp_lb_counter_item_print(counter_name, print_none_zero, tx_global_counter->control_pkts);
    sal_snprintf(counter_name, sizeof(counter_name), "TCtrlBYT");
    _dpp_lb_counter_item_print(counter_name, print_none_zero, tx_global_counter->control_octets);
  ...
  )
{
    soc_error_t
        rv = SOC_E_NONE;
    soc_reg_t
        reg = ILB_SENT_PACKETS_CNT_REGr;
    uint64
        reg_val64 = COMPILER_64_INIT(0, 0);

    rv = soc_reg64_get(unit, reg, REG_PORT_ANY, 0, &reg_val64);
    if (rv != SOC_E_NONE){
        cli_out("Error: failed to get reg in _dpp_lb_ing_global_stat_get\n");
        return rv;
    }

    *lb_pkts = soc_reg64_field32_get(unit, reg, reg_val64, ILB_SENT_SOP_CNTf);
    *bypass_pkts = soc_reg64_field32_get(unit, reg, reg_val64, BYPASS_SENT_SOP_CNTf);

    return rv;
}

soc_error_t
  _dpp_lb_egr_control_stat_get(
    int        unit,
    uint64    *pkts_value,
    uint64    *octets_value
  )
{
    ...
    rv = soc_reg64_get(unit, reg, REG_PORT_ANY, 0, &reg_val64);
    if (rv != SOC_E_NONE){
        cli_out("Error: failed to get reg in _dpp_lb_egr_control_stat_get\n");
        return rv;
    }

    *pkts_value = soc_reg64_field_get(unit, reg, reg_val64, REG_0220f);
    ...
    *octets_value = soc_reg64_field_get(unit, reg, reg_val64, REG_02E2f);
  ...
  )
{
    ...
    uint64 stat_val2 = COMPILER_64_INIT(0, 0);
    uint32 stat_32_val = 0, stat_32_val2 = 0;
    uint8 bit_val = 0;
    int rv = 0;
    ...
    if ((socLbDirectionRx == lb_key->direction) || (socLbDirectionBoth == lb_key->direction))
    {
        if ((socLbObjGlobal == lb_key->obj_type) || (socLbObjAll == lb_key->obj_type)) {
            ...
            rv = _dpp_lb_ing_global_stat_get(unit, &stat_32_val, &stat_32_val2);
            if (rv != BCM_E_NONE) {
                return CMD_FAIL;
            }
            COMPILER_64_SET(lb_counter->rx_counter.rx_global_counter.lb_pkts, 0, stat_32_val);
            COMPILER_64_SET(lb_counter->rx_counter.rx_global_counter.bypass_pkts, 0, stat_32_val2);
        }
    ...
    if ((socLbDirectionTx == lb_key->direction) || (socLbDirectionBoth == lb_key->direction))
    {
        if ((socLbObjGlobal == lb_key->obj_type) || (socLbObjAll == lb_key->obj_type)) {
            ...
            rv = _dpp_lb_egr_control_stat_get(unit, &stat_val, &stat_val2);
            if (rv != BCM_E_NONE) {
                return CMD_FAIL;
            }
            lb_counter->tx_counter.tx_global_counter.control_pkts = stat_val;
            lb_counter->tx_counter.tx_global_counter.control_octets = stat_val2;
        }
