...
typedef struct sh_dnx_counter_register_s
{
    ...
    uint32 fld_num;
    uint32 is_err;
    dnx_diag_graphical_block_mapping_e mapping;
    ...
    else
    {
        ...
        for (reg_idx = 0; reg_idx < nof_regs; reg_idx++)
        {
            ...
            for (instance_id = 0; instance_id < nof_instance; instance_id++)
            {
                ...
                if (blk_group[*blk_idx].counter_table->mapping == dnx_block_mapping_core_to_numel_idx)
                {
                    cnt_reg_port = 0;
                    cnt_reg_index = instance_id;
                }
                else if (blk_group[*blk_idx].counter_table->mapping == dnx_block_mapping_half_cores_to_block_idx)
                {
                    /**
                     * This mapping type is used where the nof block instances is half of the nof_cores.
                     * For example core0 is accessed in port 0, index 0; core 1 is port 0, index 1; core 2 is port 1, index 0; core 2 is port 1, index 1
                     */
                    cnt_reg_port = instance_id / (nof_instance / 2);
                    cnt_reg_index = instance_id % (nof_instance / 2);
    ...
    for (row = 0; row < dnx_data_graphical.diag.num_of_rows_get(unit); row++)
    {
        ...
        for (index = 0; index < dnx_data_graphical.diag.num_of_index_get(unit); index++)
        {
            if (dnx_data_graphical.diag.blocks_left_get(unit, row, index)->valid)
            {
                ...
                group_left[left_blks].counter_table->mapping =
                    dnx_data_graphical.diag.blocks_left_get(unit, row, index)->mapping;
        ...
        for (index = 0; index < dnx_data_graphical.diag.num_of_index_get(unit); index++)
        {
            if (dnx_data_graphical.diag.blocks_right_get(unit, row, index)->valid)
            {
                ...
                group_right[right_blks].counter_table->mapping =
                    dnx_data_graphical.diag.blocks_right_get(unit, row, index)->mapping;
