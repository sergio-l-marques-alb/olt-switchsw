...
char cmd_knet_ctrl_usage[] =
    ...
    "          [CpuCos=<cosq>]      - Match specified CPU COS\n"
    "          [MirrorSts=<val>]    - Match on Mirror status\n"
    "                                 1:mirrored copy 0:switched copy\n"
...
STATIC char *knet_netif_type[] = {
    "unknown", "Vlan", "Port", "Meta", NULL
};

/* Note:  See knet.h, BCM_KNET_DEST_T_xxx */
STATIC char *knet_filter_dest_type[] = {
    "Null", "NetIF", "RxAPI", "CallBack", NULL
};

STATIC char *knet_reason_str[bcmRxReasonCount+1] = BCM_RX_REASON_NAMES_INITIALIZER;

STATIC void
knet_show_netif(int unit, bcm_knet_netif_t *netif)
{
    char *type_str = "?";
    char *port_str = "n/a";
    ...
    case BCM_KNET_NETIF_T_TX_META_DATA:
        type_str = knet_netif_type[netif->type];
        break;
...
STATIC int
knet_trav_netif(int unit, bcm_knet_netif_t *netif, void *user_data)
{
    int *count = (int *)user_data;

    knet_show_netif(unit, netif);
...
STATIC void
knet_show_filter(int unit, bcm_knet_filter_t *filter)
{
    char *dest_str = "?";
    char proto_str[16];
    int idx, edx;
    ...
    case BCM_KNET_DEST_T_CALLBACK:
        dest_str = knet_filter_dest_type[filter->dest_type];
        break;
    default:
        break;
    }
...
STATIC int
knet_trav_filter(int unit, bcm_knet_filter_t *filter, void *user_data)
{
    int *count = (int *)user_data;

    knet_show_filter(unit, filter);
...
cmd_knet_ctrl(int unit, args_t *args)
{
    ...
    if (sal_strcasecmp(subcmd, "netif") == 0) {
        ...
        if (sal_strcasecmp(subcmd, "create") == 0) {
            ...
            count = 0;
            if (bcm_knet_netif_traverse(unit, knet_trav_netif, &count) < 0) {
                return CMD_FAIL;
            }
            if (count == 0) {
        ...
        if (sal_strcasecmp(subcmd, "create") == 0) {
            ...
            parse_table_add(&pt, "CpuCos", PQ_DFL|PQ_INT, 0, &pf_cpu_cos, 0);
            ...
            count = 0;
            if (bcm_knet_filter_traverse(unit, knet_trav_filter, &count) < 0) {
                return CMD_FAIL;
            }
            if (count == 0) {
                cli_out("<no filters>\n");
