...
extern stage_t ikleap_stages_info[KLEAP_NOF_STAGES_MAX];
...
#define DIAG_IKLEAP_SKIP_STAGE                                                     \
    (all_stages_flag && diag_dnx_kleap_stage_is_pmf(unit, STAGE_INFO.kleap_stage_name) /* skip PMF */ ) ||       \
...
#if defined(BCM_DNX2_SUPPORT) && defined(ADAPTER_SERVER_MODE)
#define DIAG_IKLEAP_SKIP_KBR_KBP_CHECK                          \
{                                                               \
...
{                                                                           \
    ...
    {                                                                       \
        ...
        {                                                                   \
            if ((sal_strncmp(STAGE_INFO.kleap_stage_name, fixed_key_lookup[fixed_key_i].stage_name, KLEAP_STAGE_NAME_LEN) == 0) && \
...
{                                                                               \
    ...
    else if (diag_dnx_kleap_stage_is_pmf(unit, STAGE_INFO.kleap_stage_name))    \
    {                                                                           \
...
{                                                                               \
    ...
    context_hw_val = 0;                                                         \
    nasid = 0;                                                                  \
    if (sal_strncmp(STAGE_INFO.kleap_stage_name, "VTT1", KLEAP_STAGE_NAME_LEN) != 0)                                         \
    {                                                                           \
    ...
    {                                                                           \
        ...
        if (diag_dnx_kleap_stage_is_vtt(unit, STAGE_INFO.kleap_stage_name) || diag_dnx_kleap_stage_is_fwd(unit, STAGE_INFO.kleap_stage_name))\
        {\
            SHR_IF_ERR_EXIT(dbal_fields_enum_value_get(unit, STAGE_INFO.dbal_stage_context_id[sub_stage], context_hw_val, &context_enum_val));\
        }\
...
#define SIGNALS_MATCH_EXTRA_FLAGS 0
#else
#define SIGNALS_MATCH_EXTRA_FLAGS SIGNALS_MATCH_ASIC
#endif
...
dnx_ikleap_stages_get(
    ...
    int stage_index)
{
    return ikleap_stages_info[stage_index];
}

stage_t
dnx_ikleap_stages_get_by_name(
    int unit,
    char *stage_name)
{
    int stage_index, sub_stage;
    stage_t dummy_stage;

    sal_memset(&dummy_stage, 0, sizeof(stage_t));
    for (stage_index = 0; stage_index < dnx_data_kleap_stage_info.kleap_stage_info.nof_kleap_stages_get(unit);
         stage_index++)
    {
        stage_t STAGE_INFO = dnx_ikleap_stages_get(unit, stage_index);
        for (sub_stage = 0; sub_stage <= STAGE_INFO.stage_type; sub_stage++)
        {
            if (sal_strncmp(stage_name, STAGE_INFO.kleap_sub_stages_names[sub_stage], KLEAP_STAGE_NAME_LEN) == 0)
            {
                return STAGE_INFO;
            }
        }
...
static uint8
diag_dnx_ikleap_is_kaps_or_lpm(
    int unit,
...
diag_dnx_kleap_stage_is_pmf(
    ...
    char *stage_name)
{
    if (sal_strncmp(stage_name, "IPMF", 4) == 0 || sal_strncmp(stage_name, "EPMF", 4) == 0)
    {
        return TRUE;
    }
    return FALSE;
...
static uint8
diag_dnx_kleap_stage_is_vtt(
    int unit,
    char *stage_name)
{
    if (sal_strncmp(stage_name, "VTT", 3) == 0)
    {
        return TRUE;
    }
    return FALSE;
...
static uint8
diag_dnx_kleap_stage_is_fwd(
    int unit,
    char *stage_name)
{
    if (sal_strncmp(stage_name, "FWD", 3) == 0)
    {
        return TRUE;
    }
    return FALSE;
...
/*
 * \brief - Check if core_in has a legal value
 */
static shr_error_e
diag_kleap_check_core_in(
    int unit,
    int core_in)
{
    SHR_FUNC_INIT_VARS(unit);
     ...
     * Case of CORES_ALL
     */
    if (core_in == BCM_CORE_ALL)
    {
        SHR_EXIT();
    }

    /*
     * In adapter mode there is only one core, 0 or 3
     */
#ifdef ADAPTER_SERVER_MODE
    if (dnx_data_adapter.reg_mem_access.swap_core_index_zero_with_core_index_get(unit) != core_in)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Illegal Core %d, Only legal core is: %d\n", core_in,
...
#else
    if (core_in < 0 || core_in > dnx_data_device.general.nof_cores_get(unit) - 1)
    {
    ...
    int *is_default_lif)
{
    ...
    dbal_enum_value_field_direct_payload_sizes_e inlif_table_payload_type;
...
/* *INDENT-ON* */
    ...
    SHR_ALLOC_SET_ZERO_ERR_EXIT(lif_arr_by_strength, MAX_SUPPORTED_STRENGTH * NOF_ELEMENTS * sizeof(uint32),
                                "entry_print_info", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);

    SHR_ALLOC_SET_ZERO_ERR_EXIT(res_sig_name, SIGNALS_MAX_NAME_LENGTH + 1, "res_sig_name", "%s%s%s\r\n", EMPTY, EMPTY,
                                EMPTY);
    SHR_ALLOC_SET_ZERO_ERR_EXIT(hit_sig_name, SIGNALS_MAX_NAME_LENGTH + 1, "hit_sig_name", "%s%s%s\r\n", EMPTY, EMPTY,
                                EMPTY);

    if (!diag_dnx_kleap_stage_is_vtt(unit, STAGE_INFO.kleap_stage_name))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "last local lif is valid for VT stages only. given ikleap stage %d is not supported",
                     stage_index);
    }
    else if (sal_strncmp(STAGE_INFO.kleap_stage_name, "VTT1", KLEAP_STAGE_NAME_LEN) == 0)
    {
        tt_pd_idx = 0;
    ...
    if (first_kbr_printed)
    {
        for (kbr_idx = 2; kbr_idx < nof_kbrs; kbr_idx++)
        {
            ...
            {
                sal_strncat_s(res_sig_name, "_", SIGNALS_MAX_NAME_LENGTH + 1);
                sal_strncat_s(res_sig_name, index_as_str[STAGE_INFO.kbr_if_id[kbr_idx]], SIGNALS_MAX_NAME_LENGTH + 1);

                sal_strncat_s(hit_sig_name, "_", SIGNALS_MAX_NAME_LENGTH + 1);
                sal_strncat_s(hit_sig_name, index_as_str[STAGE_INFO.kbr_if_id[kbr_idx]], SIGNALS_MAX_NAME_LENGTH + 1);
            }
            /*
             * Free signal_output - if yet or already NULL - nothing happens
             */
            SHR_IF_ERR_EXIT(sand_signal_output_find(unit, core_id,
                                                    SIGNALS_MATCH_EXTRA_FLAGS, "", phy_db_name, stage_name,
                                                    res_sig_name, &signal_output_result));
            local_lif_value = 0;
            SHR_IF_ERR_EXIT(utilex_bitstream_get_any_field(signal_output_result->value,
                                                           signal_output_result->size - 20, 20, &local_lif_value));

            /*
             * Get the HIT sent signal
             */
            SHR_IF_ERR_EXIT(sand_signal_output_find
                            (unit, core_id, SIGNALS_MATCH_EXTRA_FLAGS, "", phy_db_name, stage_name, hit_sig_name,
    ...
    if (default_supported)
    {
         ...
         */
        SHR_IF_ERR_EXIT(sand_signal_output_find
                        (unit, core_id, SIGNALS_MATCH_EXTRA_FLAGS, "",
                         dnx_data_pp.debug.from_signal_str_get(unit)->in_port, "", "In_Port", &signal_output_port));
    ...
    char **updated_res_string)
{
...
#if defined(BCM_DNX2_SUPPORT)
    int meaningful_lookup_type_string_offset =
        sal_strnlen("KBP_LOOKUP_TYPE_", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH);
#endif
    ...
    if (diag_dnx_ikleap_is_kaps_or_lpm(unit, phyDb))
    {
    ...
    /** Start key-result parsing using DBAL */
    {
        ...
        SHR_ALLOC_SET_ZERO_ERR_EXIT(entry_print_info, sizeof(dbal_printable_entry_t),
                                    "entry_print_info", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
        SHR_ALLOC_SET_ZERO_ERR_EXIT(entry_print_info_kbp, 8 * sizeof(kbp_printable_entry_t),
...
{
    uint32 entry_handle_id;
    uint32 in_port, vt1_context, dbal_enum_vt1_context;
    ...
    SHR_IF_ERR_EXIT(dbal_fields_enum_value_get(unit, DBAL_FIELD_VT1_CONTEXT_ID, vt1_context, &dbal_enum_vt1_context));
    if (((dbal_enum_vt1_context == DBAL_ENUM_FVAL_VT1_CONTEXT_ID_BRIDGE___UNTAGGED)
         || (dbal_enum_vt1_context == DBAL_ENUM_FVAL_VT1_CONTEXT_ID_BRIDGE___UNTAGGED_FRR)))
    {
        /*
         * get default VID based on in_port signal
         */
        SHR_IF_ERR_EXIT(dpp_dsig_read(unit, core_id, "IRPP", "VTT5", "", "In_Port", &in_port, 1));
        ...
        SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_GET_ALL_FIELDS));
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                        (unit, entry_handle_id, DBAL_FIELD_PORT_VID, INST_SINGLE, vid));
    }
    else
    {
        ...
        if (signal_output_header->field_list == NULL || signal_output_header->field_list->top == NULL)
        {
            SHR_ERR_EXIT(_SHR_E_FAIL, "Header signal is not complete!\n");
        }
        else
        {
            first_header_sig = (signal_output_t *) (signal_output_header->field_list->top);
        }
        if (sal_strnstr(first_header_sig->entry.name, "PTCH", sizeof(first_header_sig->entry.name)))
        {
            second_header_sig = (signal_output_t *) (first_header_sig->entry.next);
            if (sal_strnstr(second_header_sig->entry.name, "ETH", sizeof(second_header_sig->entry.name)))
            {
                found_eth = TRUE;
                eth_header_sig = second_header_sig;
            }
        }
        else if (sal_strnstr(first_header_sig->entry.name, "ETH", sizeof(first_header_sig->entry.name)))
        {
            found_eth = TRUE;
        ...
        if (found_eth)
        {
            ...
            {
                if (!sal_strncasecmp(entry->name, "VLAN", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
                {
                    found_vlan = TRUE;
            ...
            if (found_vlan)
            {
                ...
                {
                    if (!sal_strncasecmp(entry->name, "VID", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
                    {
    ...
    uint32 *fodo_value)
{
     ...
     */
    if (sal_strncmp(stage_name, "VTT1", KLEAP_STAGE_NAME_LEN) == 0)
    {
        *fodo_value = 0;
    }
    else
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SHR_ALLOC_SET_ZERO_ERR_EXIT(temp_key_sig_value, DSIG_MAX_SIZE_UINT32 * sizeof(uint32),
                                "temp_key_sig_value value", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
    SHR_ALLOC_SET_ZERO_ERR_EXIT(key_sig_value, DSIG_MAX_SIZE_UINT32 * sizeof(uint32),
                                "key sig value", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
    SHR_ALLOC_SET_ZERO_ERR_EXIT(key_sig_name, SIGNALS_MAX_NAME_LENGTH + 1, "key_sig_name", "%s%s%s\r\n", EMPTY, EMPTY,
                                EMPTY);
    SHR_ALLOC_SET_ZERO_ERR_EXIT(res_sig_name, SIGNALS_MAX_NAME_LENGTH + 1, "res_sig_name", "%s%s%s\r\n", EMPTY, EMPTY,
                                EMPTY);
    SHR_ALLOC_SET_ZERO_ERR_EXIT(app_sig_name, SIGNALS_MAX_NAME_LENGTH + 1, "app_sig_name", "%s%s%s\r\n", EMPTY, EMPTY,
                                EMPTY);
    SHR_ALLOC_SET_ZERO_ERR_EXIT(hit_sig_name, SIGNALS_MAX_NAME_LENGTH + 1, "hit_sig_name", "%s%s%s\r\n", EMPTY, EMPTY,
                                EMPTY);
    SHR_ALLOC_SET_ZERO_ERR_EXIT(err_sig_name, SIGNALS_MAX_NAME_LENGTH + 1, "err_sig_name", "%s%s%s\r\n", EMPTY, EMPTY,
                                EMPTY);
    SHR_ALLOC_SET_ZERO_ERR_EXIT(error_sig_name, SIGNALS_MAX_NAME_LENGTH + 1, "error_sig_name", "%s%s%s\r\n", EMPTY,
                                EMPTY, EMPTY);
    SHR_ALLOC_SET_ZERO_ERR_EXIT(context_name, DIAG_DNX_KLEAP_MAX_STRING_LENGTH, "context_name value", "%s%s%s\r\n",
                                EMPTY, EMPTY, EMPTY);
    ...
    SHR_IF_ERR_EXIT(diag_kleap_check_core_in(unit, core_in));
    ...
    SH_DNX_CORES_ITER(unit, core_in, core_id)
    {
             ...
             stage_index++)
        {
            ...
            for (sub_stage = 0; sub_stage <= STAGE_INFO.stage_type; sub_stage++)
            {
                ...
                PRT_TITLE_SET("%s Context %s(%d), Core %d", stage_name, context_name, context_enum_val, core_id);
                ...
                for (kbr_idx = 2; kbr_idx < nof_kbr; kbr_idx++)
                {
...
#if defined(BCM_DNX2_SUPPORT) && defined(ADAPTER_SERVER_MODE)
                    DIAG_IKLEAP_SKIP_KBR_KBP_CHECK;
#endif
                    ...
                    if (kbr_has_app_db_id)
                    {
                        dbal_tables_e dbal_table_for_lookup = DBAL_TABLE_EMPTY;

                        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                                        (unit, kbr_info_handle_id, DBAL_FIELD_APP_DB_ID, INST_SINGLE, &app_db_id));

                        SHR_IF_ERR_EXIT(dbal_tables_table_by_mdb_phy_get
                                        (unit, kbr2phy_mapping[kbr_idx], app_db_id, &dbal_table_for_lookup));

                        if (dbal_table_for_lookup == DBAL_TABLE_EMPTY)
                        {
                            continue;
                        }
                    }
                    ...
                    {
                        sal_strncat_s(signal_prefix, index_as_str[first_interface_in_key], sizeof(signal_prefix));
                    ...
                    if (kbr_has_app_db_id)
                    {
                         ...
                         */
                        SHR_IF_ERR_EXIT(sand_signal_output_find
                                        (unit, core_id, SIGNALS_MATCH_EXTRA_FLAGS, "", stage_name, phy_db_name,
                    ...
                    for (ii = 0; ii < nof_key_signals_to_concatanate; ii++)
                    {
                        ...
                        if (sal_strncmp(STAGE_INFO.kleap_stage_name, "FWD12", KLEAP_STAGE_NAME_LEN) == 0)
                        {
                            /** Check if the key is ACL or FWD (key is mapped to KBR) */
                            dbal_entry_key_field32_set(unit, kbr_info_handle_id, context_profile_field, context_hw_val);
                            dbal_entry_key_field32_set(unit, kbr_info_handle_id, kbr_idx_field,
                                                       key_sig_fwd12_2kbp_kbr[ii]);
                        ...
                        if (ii != 0)
                        {
                            ...
                            if (kbr2phy_mapping[kbr_idx] == DBAL_PHYSICAL_TABLE_KBP)
                            {
                                ...
                                key_sig_name[sal_strnlen(key_sig_name, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) -
                                             1] = '0' + ii;
                            }
                            else
#endif
                            {
                                key_sig_name[sal_strnlen(key_sig_name, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) -
                                             1] = '0' + first_interface_in_key - ii;
                            }
                        }
                        SHR_IF_ERR_EXIT(sand_signal_output_find
                                        (unit, core_id, SIGNALS_MATCH_EXTRA_FLAGS, "", stage_name, phy_db_name,
                                         key_sig_name, &signal_output_key));
                        if (ii == 0)
                        {
                    ...
                    {
                        SHR_IF_ERR_EXIT(sand_signal_output_find
                                        (unit, core_id, SIGNALS_MATCH_EXTRA_FLAGS, "", "FWD2", "IPMF1",
                                         "Elk_Lkp_Payload", &signal_output_result));
                    ...
                    {
                        SHR_IF_ERR_EXIT(sand_signal_output_find
                                        (unit, core_id, SIGNALS_MATCH_EXTRA_FLAGS, "", phy_db_name, stage_name,
                                         res_sig_name, &signal_output_result));
                        res_sig_value = signal_output_result->value;
                        res_sig_size = signal_output_result->size;
                        ...
                        SHR_IF_ERR_EXIT(sand_signal_output_find
                                        (unit, core_id, SIGNALS_MATCH_EXTRA_FLAGS, "", phy_db_name, stage_name,
                                         hit_sig_name, &signal_output_hit));
                        hit_sig_value = signal_output_hit->value;
                        /*
                         * Get the ERROR sent signal
                         */
                        rv = sand_signal_output_find(unit, core_id, SIGNALS_MATCH_EXTRA_FLAGS, "",
                 ...
                 */
                {
                    ...
                    uint32 vsi_enabler = 0, lif_enabler = 0;
                    {
                        ...
                        if (diag_dnx_kleap_stage_is_vtt(unit, STAGE_INFO.kleap_stage_name))
                        {
                            context_table = STAGE_INFO.dbal_stage_context_properties[sub_stage];
                            context_field = STAGE_INFO.dbal_stage_context_id[sub_stage];
                            is_vt_stage = TRUE;
                        }
                    }
                    if (is_vt_stage)
                    {
                        ...
                        if (bare_metal_disabled && lif_enabler)
                        {
                            SHR_IF_ERR_EXIT(diag_dnx_ikleap_find_last_local_lif
                                            (unit, stage_index, sub_stage, core_id, context_hw_val, first_kbr_printed,
                            ...
                            if (local_in_lif != 0)
                            {
                                ...
                                PRT_CELL_SET("%s(%s%s)", dbal_logical_table_to_string(unit, lif_dbal_table), "INLIF_",
                                             (sal_strncmp(STAGE_INFO.kleap_stage_name, "VTT1", KLEAP_STAGE_NAME_LEN) ==
                                              0) ? "2" : "1");
