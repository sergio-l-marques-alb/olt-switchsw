...
static bcma_ft_export_elements_map_t bcma_ft_export_elements_map[] = {
            ...
            {   BCMI_FT_XLATE_ELEM(EgrClassId, 2 ) },
            {   BCMI_FT_XLATE_ELEM(TcpSeqNumber, 4 ) },
            {   BCMI_FT_XLATE_ELEM(Custom, 0 ) },
            {   BCMI_FT_XLATE_ELEM(PacketTos, 1 ) },
            {   BCMI_FT_XLATE_ELEM(ForwardingOuterVlanTag,      2 )   },
            {   BCMI_FT_XLATE_ELEM(ForwardingInnerVlanTag,      2 )   }
};
                             ...
                             char *elem_name)
{
    ...
    for (ix = 0; ix < COUNTOF(bcma_ft_export_elements_map); ix++) {
        if (bcma_ft_export_elements_map[ix].export_element == info_elem) {
            if (elem_name) {
                sal_strlcpy(elem_name, bcma_ft_export_elements_map[ix].element_name,
