    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (is_std_1)
    {
        LOG_CLI_EX("\r\n" "Virtual wires are not supported on standard_1.%s%s%s%s\r\n\n", EMPTY, EMPTY, EMPTY, EMPTY);
        SHR_EXIT();
    }
        ...
        while (1)
        {
            ...
            if (support_qual)
            {
                ...
                if (bcm_qual != bcmFieldQualifyCount)
                {
                    ...
                    SHR_IF_ERR_EXIT(dnx_field_map_vw_qual_info_get
                                    (unit, (bcm_qual - dnx_data_field.qual.vw_1st_get(unit)), &vw_qual_info));
                    if ((vw_qual_info.stage_bmp & (SAL_BIT(field_stage))) == 0)
                    {
    ...
    for (stage_index = stage_lower; stage_index <= stage_upper; stage_index++)
    {
        ...
        for (signal_index = 0; signal_index < signals_nof; signal_index++)
        {
                 ...
                 action_index++)
            {
                ...
                if (sal_strncmp(bits_covered_by_act_string, "", sizeof(bits_covered_by_act_string)))
                {
                    DIAG_DNX_FIELD_UTILS_STR_INPUT_VERIFY(bits_covered_by_act_string, "%s", ",");
                    sal_strncat_s(bits_covered_by_act_string, ",", sizeof(bits_covered_by_act_string));
                }
