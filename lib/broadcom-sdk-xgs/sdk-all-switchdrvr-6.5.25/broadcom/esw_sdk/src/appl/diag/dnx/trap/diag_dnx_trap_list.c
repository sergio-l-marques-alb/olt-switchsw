    ...
    sh_sand_control_t * sand_control)
{
    ...
    sal_memset(&single_map_entry, 0, sizeof(dnx_rx_trap_map_type_t));
    ...
    /** Sort trap ids from low to high */
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SHR_IF_ERR_EXIT(dnx_rx_trap_map_ingress_ud_alloc_get(unit, alloc_index, &trap_id));
    while (trap_id != DBAL_NOF_ENUM_INGRESS_TRAP_ID_VALUES)
    {
        SHR_IF_ERR_EXIT(dnx_rx_trap_is_trap_id_allocated(unit, trap_id, &is_allocated));
        if (state == 2)
        {
...
sh_sand_cmd_t sh_dnx_trap_list_cmds[] = {
    ...
    ,
    {"USeR", sh_dnx_trap_list_ud_cmd, NULL, dnx_trap_list_ud_options, &dnx_trap_list_ud_man, NULL, NULL,
