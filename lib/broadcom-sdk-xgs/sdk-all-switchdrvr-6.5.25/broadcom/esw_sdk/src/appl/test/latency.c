/*
 * $Id: latency.c,v 1.0 Broadcom SDK $
...
static int time_init(int unit)
{
    int rv;
    bcm_time_ts_counter_t ts1;

    bcm_time_ts_counter_t_init(&ts1);

    ts1.ts_counter = 1;
    rv = bcm_time_ts_counter_get(unit, &ts1);
    if (BCM_FAILURE(rv)) {
        test_error(unit, "Failed fetching ts counter info: rc: %d\n", rv);
        return rv;
    }
    ...
    ts1.enable = 1;
    rv = bcm_time_ts_counter_set(unit, &ts1);
    if (BCM_FAILURE(rv)) {
        test_error(unit, "Failed enabling ts counter: rc: %d\n", rv);
        return rv;
    }
...
static int ts_tod_update(int unit)
{
    int rv;
    bcm_time_tod_t tod;
    ...
    rv = bcm_time_ts_counter_get(unit, &ts1);
    if (BCM_FAILURE(rv)) {
        test_error(unit, "Failed to fetch timestamper info rc: %d\n", rv);
        return rv;
    }
    if (!ts1.enable) {
        return BCM_E_PARAM;
    ...
    tod.time_format = ts1.time_format;
    tod.ts_adjustment_counter_ns = ts1.ts_counter_ns;

    /* Get seconds from the timestamp value */
    secs = ts1.ts_counter_ns;
    ...
    if (BCM_FAILURE(rv)) {
        test_error(unit, "Failed to set tod info rc: %d\n", rv);
        return rv;
    }
...
ts_tod_update_thread(void* up)
{
    ...
    do {
        if (BCM_FAILURE(ts_tod_update(unit))) {
            break;
        }
        sal_usleep(100000);
    } while (latency_p->ts_tod_update_thread_kill != 1);

    latency_p->ts_tod_update_thread_kill = 0;
    sal_thread_exit(0);
...
static int 
ts_tod_update_init(int unit)
{
    char str[80];

    sal_sprintf(str, "TR507_TS_UPDATE");
    sal_thread_create(str, 32 * 1024 * 1024, 200,
                      ts_tod_update_thread, INT_TO_PTR(unit));

    LOG_INFO(BSL_LS_APPL_TESTS,
             (BSL_META_U(unit, "background thead created for ts-tod update %24s\n"), str));

    return BCM_E_NONE;
}

static int port_ts_pcs_config(int unit, int port)
{
    int rv;
    uint64 am_norm_mode;
    uint64 ts_offset;
    bcm_port_phy_timesync_config_t phy_ts_config;
    ...
    if (BCM_FAILURE(rv)) {
        test_error(unit, "Failed configuring bcmTimesyncTimestampingMode48bit rc: %d\n", rv);
        return rv;
    }


    /* API to configure the PCS - Not required for TH3. */
    COMPILER_64_SET(ts_offset, 0, 0x10);
    rv = bcm_port_control_phy_timesync_set(0, port,
                                           bcmPortControlPhyTimesyncTimestampOffset, ts_offset);
    if (BCM_FAILURE(rv)) {
        test_error(unit, "Failed configuring bcmPortControlPhyTimesyncTimestampOffset rc: %d\n", rv);
        return rv;
    }
    /* Adjustment mode (deskew config) */
    COMPILER_64_SET(am_norm_mode, 0, bcmPortPhyTimesyncCompensationModeNone);
    rv = bcm_port_control_phy_timesync_set(0, port,
                                           bcmPortControlPhyTimesyncTimestampAdjust,
                                           am_norm_mode);
    if (BCM_FAILURE(rv)) {
        test_error(unit, "Failed configuring bcmPortControlPhyTimesyncTimestampAdjust rc: %d\n", rv);
        return rv;
    }

    bcm_port_phy_timesync_config_t_init(&phy_ts_config);
    phy_ts_config.flags = (BCM_PORT_PHY_TIMESYNC_ENABLE | BCM_PORT_PHY_TIMESYNC_ONE_STEP_ENABLE);
    rv = bcm_port_phy_timesync_config_set(0, port, &phy_ts_config);
    if (BCM_FAILURE(rv)) {
        test_error(unit, "Failed configuring bcm_port_phy_timesync_config_set rc: %d\n", rv);
        return rv;
    }
...
#if 0
        start_cmicx_timesync(unit);
        READ_NS_TIMESYNC_COUNTER_CONFIG_SELECTr(unit, &rval);
        soc_reg_field_set(unit, NS_TIMESYNC_COUNTER_CONFIG_SELECTr, &rval,
                                    ENABLE_COMMON_CONTROLf, 1);
        WRITE_NS_TIMESYNC_COUNTER_CONFIG_SELECTr(unit, rval);
#else
        BCM_IF_ERROR_RETURN(time_init(unit));

        BCM_IF_ERROR_RETURN(port_ts_pcs_config(unit, latency_p->timestamp_port1_param));
        BCM_IF_ERROR_RETURN(port_ts_pcs_config(unit, latency_p->timestamp_port2_param));

        BCM_IF_ERROR_RETURN(ts_tod_update_init(unit));
#endif
    } else
#endif
    {
...
latency_set_up_streams(int unit)
{
    ...
    if (SOC_IS_TOMAHAWK3(unit)) {
        ...
        action.new_outer_vlan = XLATE_VLAN1;
        bcm_vlan_port_egress_default_action_set(unit, latency_p->timestamp_port1_param, &action);
...
latency_test(int unit, args_t *a, void *pa)
{
    ...
    if (SOC_IS_TOMAHAWK3(unit)) {
        ...
        bcm_vlan_action_set_t_init(&action);
        action.ot_outer = bcmVlanActionReplace;
        action.new_outer_vlan = XLATE_VLAN0;
        bcm_vlan_port_egress_default_action_set(unit, latency_p->swirl_port_param, &action);

        sal_usleep(1);
        /* Setting this here so that the pkts on port1 with XLATE_VLAN1 vlan
           are replaced with CPU_VLAN
        */
        bcm_vlan_action_set_t_init(&action);
        action.ot_outer = bcmVlanActionReplace;
        action.new_outer_vlan = CPU_VLAN;
        bcm_vlan_port_egress_default_action_set(unit, latency_p->timestamp_port1_param, &action);
    } else
#endif
    {
...
latency_test_cleanup(int unit, void *pa)
{
    ...
    latency_p->ts_tod_update_thread_kill = 1;
