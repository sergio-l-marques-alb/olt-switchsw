/*
 * $Id: diag_pp_test.c,v 1.18 Broadcom SDK $
...
diag_pp_parsing_info_get(int unit, int core_id, diag_pp_parsing_info_fields_t *pars_info)
{
    ...
    sal_memcpy(pars_info->hdrs_stack, prm_pars_info.hdrs_stack, sizeof(SOC_PPC_DIAG_PARSING_HEADER_INFO)*SOC_PPC_DIAG_MAX_NOF_HDRS);
...
diag_pp_vlan_edit_info_get(int unit, int core_id, diag_pp_vlan_edit_info_fields_t *vlan_edit_info)
{
    ...
    sal_memcpy(&vlan_edit_info->cmd_info, &prm_vlan_edit_info.cmd_info, sizeof(SOC_PPC_LIF_ING_VLAN_EDIT_COMMAND_INFO));
    sal_memcpy(&vlan_edit_info->ac_tag, &prm_vlan_edit_info.ac_tag, sizeof(SOC_SAND_PP_VLAN_TAG));
    sal_memcpy(&vlan_edit_info->ac_tag2, &prm_vlan_edit_info.ac_tag2, sizeof(SOC_SAND_PP_VLAN_TAG));
...
diag_pp_pkt_tm_info_get(int unit, int core_id, diag_pp_pkt_tm_info_fields_t *pkt_tm_info)
{
    ...
    sal_memcpy(&pkt_tm_info->frwrd_decision, &prm_pkt_tm_info.frwrd_decision, sizeof(SOC_PPC_FRWRD_DECISION_INFO));
...
diag_pp_lif_lkup_info_get(int unit, int core_id, diag_pp_lif_lkup_info_fields_t *lif_lkup_info, SOC_PPC_DIAG_DB_USE_INFO *db_info)
{
    ...
    sal_memcpy(&lif_lkup_info->key, &prm_lif_lkup_info.key, sizeof(SOC_PPC_DIAG_LIF_KEY));
    lif_lkup_info->found = prm_lif_lkup_info.found;
    sal_memcpy(&lif_lkup_info->value, &prm_lif_lkup_info.value, sizeof(SOC_PPC_DIAG_LIF_VALUE));
...
diag_pp_termination_info_bridge_test(int unit, int core_id) {
    ...
    diag_pp_termination_info_fields_t expected_term_info = { 0 };
...
diag_pp_lif_lkup_info_bridge_test(int unit, int core_id) {
    ...
    sal_memset(&expected_lif_lkup_info, 0, sizeof(diag_pp_lif_lkup_info_fields_t));
