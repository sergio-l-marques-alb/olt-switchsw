...
test_accessible_dnx2(
    ...
    /* 
        * Check if a feature field type configured with family exclusive of DNX2 is set and accessible for a DNX2 device.
        */
    ...
    if (feature_val != 1)
    {
        LOG_ERROR(BSL_LOG_MODULE, (BSL_META_U(unit, "feature_family_dnx2 - feature typed field configured with family exclusive of DNX2 \nError - expected that feature_family_dnx2 to be set\n")));
        res = 1;
    }
    /* 
    ...
    /* 
        * Check if a feature field type that it's submodule configured with family exlcusive of DNX2 is accessible thorugh a DNX2 device.
        */
...
compile_flags_test(
    ...
    uint32 feature_val;
    const dnx_data_module_testing_compileflags_submodule_standard_table_flags_t *table_flags;
    const dnx_data_module_testing_compileflags_submodule_flags_table_no_flags_t *table_no_flags;
    /* 
     ...
     */
    if (dnx_data_module_testing.compileflags_submodule_standard.define_flags_get(unit) != 1) {
        LOG_ERROR(BSL_LOG_MODULE, (BSL_META_U(unit, "Module: 'module_testing' submodule: 'compileflags_submodule_standard' Field: 'define_flags' should be set.\n")));
        res = 1;
    }
    if (dnx_data_module_testing.compileflags_submodule_standard.numeric_flags_get(unit)  != 1) {
        LOG_ERROR(BSL_LOG_MODULE, (BSL_META_U(unit, "Module: 'module_testing' submodule: 'compileflags_submodule_standard' Field: 'numeric_flags' should be set.\n")));
        res = 1;
    }
    feature_val = dnx_data_module_testing.compileflags_submodule_standard.feature_get(unit, dnx_data_module_testing_compileflags_submodule_standard_feature_flags);
    if (feature_val != 1)
    {
        LOG_ERROR(BSL_LOG_MODULE, (BSL_META_U(unit, "Module: 'module_testing' submodule: 'compileflags_submodule_standard' Field: 'feature_flags' should be set.\n")));
        res = 1;
    }
    table_flags = dnx_data_module_testing.compileflags_submodule_standard.table_flags_get(unit, 0);
    {
        if (table_flags == NULL)
        {
            LOG_ERROR(BSL_LOG_MODULE,
                    (BSL_META_U(unit, "Module: 'module_testing' submodule: 'compileflags_submodule_standard' Field: 'table_flags' should be set.\n")));
            res = 1;
        }
        else if (table_flags->table_flags_value != 1)
        {
            LOG_ERROR(BSL_LOG_MODULE,
                    (BSL_META_U(unit, "Module: 'module_testing' submodule: 'compileflags_submodule_standard' Field: 'table_flags' get incorrect value for key 1 : %x\n"),
     ...
     */
    if (dnx_data_module_testing.compileflags_submodule_flags.define_no_flags_get(unit) != 1) {
        LOG_ERROR(BSL_LOG_MODULE, (BSL_META_U(unit, "Module: 'module_testing' submodule: 'compileflags_submodule_flags' Field: 'define_no_flags' should be set.\n")));
        res = 1; 
    }
    if (dnx_data_module_testing.compileflags_submodule_flags.numeric_no_flags_get(unit) != 1) {
        LOG_ERROR(BSL_LOG_MODULE, (BSL_META_U(unit, "Module: 'module_testing' submodule: 'compileflags_submodule_flags' Field: 'numeric_no_flags' should be set.\n")));
        res = 1; 
    }
    feature_val = dnx_data_module_testing.compileflags_submodule_flags.feature_get(unit, dnx_data_module_testing_compileflags_submodule_flags_feature_no_flags);
    if (feature_val != 1)
    {
        LOG_ERROR(BSL_LOG_MODULE, (BSL_META_U(unit, "Module: 'module_testing' submodule: 'compileflags_submodule_flags' Field: 'feature_no_flags' should be set.\n")));
        res = 1;
    }
    table_no_flags = dnx_data_module_testing.compileflags_submodule_flags.table_no_flags_get(unit, 0);
    {
        if (table_no_flags == NULL)
        {
            LOG_ERROR(BSL_LOG_MODULE,
                    (BSL_META_U(unit, "Module: 'module_testing' submodule: 'compileflags_submodule_flags' Field: 'table_no_flags' should be set.\n")));
            res = 1;
        }
        else if (table_no_flags->table_no_flags_value != 1)
        {
            LOG_ERROR(BSL_LOG_MODULE,
                    (BSL_META_U(unit, "Module: 'module_testing' submodule: 'compileflags_submodule_flags' Field: 'table_no_flags' get incorrect value for key 1 : %x\n"),
    ...
    if (diag_dnxc_data_is_field_exist(unit, "module_testing", "compileflags_submodule_standard", "define_flags") == 1) {
        LOG_ERROR(BSL_LOG_MODULE, (BSL_META_U(unit, "Error - Expecting module_testing.compileflags_submodule_standard.define_flags to be inaccessible\n")));
        res = 1;
    }  
    if (diag_dnxc_data_is_field_exist(unit, "module_testing", "compileflags_submodule_standard", "numeric_flags") == 1) {
        LOG_ERROR(BSL_LOG_MODULE, (BSL_META_U(unit, "Error - Expecting module_testing.compileflags_submodule_standard.numeric_flags to be inaccessible\n")));
        res = 1;
    }  
    if (diag_dnxc_data_is_field_exist(unit, "module_testing", "compileflags_submodule_standard", "feature_flags") == 1) {
        LOG_ERROR(BSL_LOG_MODULE, (BSL_META_U(unit, "Error - Expecting module_testing.compileflags_submodule_standard.feature_flags to be inaccessible\n")));
        res = 1;
    }
    if (diag_dnxc_data_is_field_exist(unit, "module_testing", "compileflags_submodule_standard", "table_flags") == 1) {
        LOG_ERROR(BSL_LOG_MODULE, (BSL_META_U(unit, "Error - Expecting module_testing.compileflags_submodule_standard.table_flags to be inaccessible\n")));
        res = 1;
    }  
    ...
    if (diag_dnxc_data_is_field_exist(unit, "module_testing", "compileflags_submodule_flags", "define_no_flags") == 1) {
        LOG_ERROR(BSL_LOG_MODULE, (BSL_META_U(unit, "Error - Expecting module_testing.compileflags_submodule_flags.define_no_flags to be inaccessible\n")));
        res = 1;
    }  
    if (diag_dnxc_data_is_field_exist(unit, "module_testing", "compileflags_submodule_flags", "numeric_no_flags") == 1) {
        LOG_ERROR(BSL_LOG_MODULE, (BSL_META_U(unit, "Error - Expecting module_testing.compileflags_submodule_flags.numeric_no_flags to be inaccessible\n")));
        res = 1;
    }    
    if (diag_dnxc_data_is_field_exist(unit, "module_testing", "compileflags_submodule_flags", "feature_no_flags") == 1) {
        LOG_ERROR(BSL_LOG_MODULE, (BSL_META_U(unit, "Error - Expecting module_testing.compileflags_submodule_flags.feature_no_flags to be inaccessible\n")));
        res = 1;
    }  
    if (diag_dnxc_data_is_field_exist(unit, "module_testing", "compileflags_submodule_flags", "table_no_flags") == 1) {
        LOG_ERROR(BSL_LOG_MODULE, (BSL_META_U(unit, "Error - Expecting module_testing.compileflags_submodule_flags.table_no_flags to be inaccessible\n")));
        res = 1;
    }    
#endif
    return res;
