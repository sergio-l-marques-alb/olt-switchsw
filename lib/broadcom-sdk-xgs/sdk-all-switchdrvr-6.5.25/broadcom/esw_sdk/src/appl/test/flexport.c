...
flexport_by_config_file(int unit, char *fname)
{
    ...
    for (i=0; i < MAX_TSC_NUM; i++) { hg_bmp[i] = 0; }
    ...
    while (sal_fgets(line, sizeof(line), fp)) {
        if (sal_strstr(line, "#") != line) {
            ...
            if (sal_strstr(line, dic_higig)) {
                ...
                /* Potentially 65 bit, so cannot just strtol entire string */
                j = 0;
                for (i = (strlen(port_data[1]) - 1); i >= 0; i--) {
                    if ((j * 4) < MAX_TSC_NUM) {
                        convert_hex2bin(port_data[1][i], bmp_4bit);
                        hg_bmp[j * 4    ] = bmp_4bit[0];
                        if ((j * 4 + 1) < MAX_TSC_NUM) {
                            hg_bmp[j * 4 + 1] = bmp_4bit[1];
                            hg_bmp[j * 4 + 2] = bmp_4bit[2];
                            hg_bmp[j * 4 + 3] = bmp_4bit[3];
                            j++;
                        }
                    }
                }
