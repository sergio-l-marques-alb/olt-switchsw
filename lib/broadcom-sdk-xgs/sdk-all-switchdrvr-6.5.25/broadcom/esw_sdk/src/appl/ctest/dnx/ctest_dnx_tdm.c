    ...
    sh_sand_control_t * sand_control)
{
    ...
    else
    {
        if (get_ingress_context->fabric_ports_count != 0)
        {
            ...
            if (sal_strncmp(set_fabric_ports_str, get_fabric_ports_str, sizeof(set_fabric_ports_str)))
            {
                is_equal = FALSE;
                PRT_CELL_SET("Failed");
            }
    ...
    uint32 *err_pkt_tx_from_dst_port_p)
{
    ...
    if (src_port > 0)
    {
        ...
        if (DNX_ALGO_PORT_TYPE_IS_NIF_ETH(unit, src_port_info, DNX_ALGO_PORT_TYPE_INCLUDE(L1)))
        {
        ...
        else if (DNX_ALGO_PORT_TYPE_IS_NIF_ILKN(unit, src_port_info, DNX_ALGO_PORT_TYPE_INCLUDE(L1)))
        {
    ...
    if (dst_port > 0)
    {
        ...
        if (DNX_ALGO_PORT_TYPE_IS_NIF_ETH(unit, dst_port_info, DNX_ALGO_PORT_TYPE_INCLUDE(L1)))
        {
        ...
        else if (DNX_ALGO_PORT_TYPE_IS_NIF_ILKN(unit, dst_port_info, DNX_ALGO_PORT_TYPE_INCLUDE(L1)))
        {
            SHR_IF_ERR_EXIT(dnx_mib_ilkn_counter_get
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (i_context = 0; i_context < context_count; i_context++)
    {
         ...
         * Set TDM editing context configuration
         */
        context_id = (i_context + 1) * context_step;
    ...
    sh_sand_control_t * sand_control)
{
    ...
    PRT_ROW_ADD(PRT_ROW_SEP_NONE);
    PRT_CELL_SET("WIde Stream Enable");
    PRT_CELL_SET("%d", set_interface_config->stream_id_wide_enable);
    PRT_CELL_SET("%d", get_interface_config->stream_id_wide_enable);
    if (set_interface_config->stream_id_wide_enable != get_interface_config->stream_id_wide_enable)
    {
        is_equal = FALSE;
        PRT_CELL_SET("Failed");
    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SHR_IF_ERR_EXIT(bcm_dnx_port_get(unit, port, &flags, &interface_info, &mapping_info));
    ...
    set_interface_config.stream_id_wide_enable=0;
...
exit:
    ...
    if (SHR_FUNC_ERR())
    {
        ...
        LOG_CLI_EX("stream_id_key_size %d, stream_id_key_offset %d stream_id_enable %d stream_id_wide_enable %d\r\n",
                   set_interface_config.stream_id_key_size,set_interface_config.stream_id_key_offset,
                   set_interface_config.stream_id_enable, set_interface_config.stream_id_wide_enable);
    }
    else
    {
        LOG_CLI_EX("\r\n" "Test '%s' completed SUCCESSFULLY. %s%s%s\r\n", test_name, EMPTY, EMPTY, EMPTY);
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (i_stream = 0; i_stream < count; i_stream++)
    {
        ...
        context_ids[i_stream] = set_stream_context;
    }
    ...
    if (to_clean == TRUE)
    {
        for (i_stream = 0; i_stream < count; i_stream++)
        {
             ...
             * Set TDM editing context configuration
             */
            SHR_IF_ERR_EXIT(bcm_tdm_ingress_context_destroy(unit, 0, context_ids[i_stream]));
        }
    }
    if (is_equal == FALSE)
    {
...
static sh_sand_option_t dnx_tdm_bypass_test_arguments[] = {
    ...
    {"wide_sid",      SAL_FIELD_TYPE_BOOL,  "Wide Stream id", "No"},
    {"src_port",      SAL_FIELD_TYPE_PORT,  "Source Port for TDM Bypass Traffic", "eth1"},
    {"dst_port",      SAL_FIELD_TYPE_PORT,  "Destination Port for Unicast Context or one of multicast group", "eth13"},
    ...
    {"blocked_channel_id",    SAL_FIELD_TYPE_UINT32,"Channel id to apply for the blocked port ", "MIN", (void *) dnx_tdm_interface_channel_id_enum_table, "MIN-MAX"},
    {NULL}
};

static sh_sand_option_t dnx_tdm_egress_bypass_test_arguments[] = {
    {"src_port",      SAL_FIELD_TYPE_PORT,  "Source Port for TDM Bypass Traffic", "eth1"},
    {"dst_port",      SAL_FIELD_TYPE_PORT,  "Destination Port for Unicast Context or for multicast (sar is the only out port even in mc setup)", "eth13"},
    {"multicast_id",  SAL_FIELD_TYPE_UINT32,"Multicast ID for Multicast Context, should not be present for unicast scenario", "1025"},
    {"cpu_port",      SAL_FIELD_TYPE_PORT,  "CPU Port to be used in order to send traffic to TDM loopback port", "201"},
    {"clean",         SAL_FIELD_TYPE_BOOL,  "Clean after test", "Yes"},
    {"packet",        SAL_FIELD_TYPE_STR,   "Packet name from XML (TestCase.xml)", "tdm_01"},
    {NULL}
...
static sh_sand_man_t dnx_tdm_bypass_wide_sid_test_man = {
    "Test Ingress Bypass Single Packet TDM Traffic with wide sid"
};
static sh_sand_man_t dnx_tdm_egress_bypass_test_man = {
    "Test Engress Bypass Single Packet TDM Traffic"
};
/**
...
static sh_sand_invoke_t dnx_tdm_bypass_tests[] = {
    ...
    {"src_ilkn_with_2_channels",            "src_port=il6 dst_port=eth13 base=100 channel_id=8 blocked_channel_id=9"         },
    {NULL}
};

/**
 * Regression tests to run for 'bypass_wide_sid' mode. See 'dnx_tdm_bypass_test_cmd()'
 */
static sh_sand_invoke_t dnx_tdm_bypass_wide_sid_tests[] = {
     /** multicast_id is taken from testcases.xml, so this multicast_id=0 is only an idication the test is sending multicast traffic*/
    {"stream_mcast_stamp_sid_with_wide",     "stream_enable=yes base=0 multicast_id=0 bits_in_key=18 offset=158 stamp_mcid_with_sid=yes wide_sid=yes"},
    {NULL}
};

/**
 * Regression tests to run for 'dual_bypass' mode. See 'dnx_tdm_bypass_test_cmd()'
 */
static sh_sand_invoke_t dnx_tdm_egress_bypass_tests[] = {
        
        {"unicast",                      "src_port=eth1"           },
        {"multicast",                    "src_port=eth1 multicast_id=11111"            },
        {NULL}
};

/**
 *  CLI parameters for 'dnx_tdm_failover_drop_test_cmd()'.
 */
...
static sh_sand_option_t dnx_tdm_hybrid_test_arguments[] = {
    ...
    {"dst_port_data", SAL_FIELD_TYPE_PORT,  "Destination Port for DATA traffic", "eth17"},
    {"clean",         SAL_FIELD_TYPE_BOOL,  "Clean after test", "Yes"},
    {"cpu_port",      SAL_FIELD_TYPE_PORT,  "CPU Port to be used in order to send traffic to TDM destination port", "201"},
    {NULL}
};
    ...
    uint32 *context_id_p)
{
    ...
    *context_id_p = DEFAULT_CONTEXT_PER_STREAM;
     ...
     */
    if (context_id != DNX_TDM_INGRESS_INVALID_CONTEXT)
    {
    ...
    int *ingress_context_id_p)
{
    ...
    if (is_not_mc != FALSE)
    {
        ...
        /*
         * map logical port to sysport_gport
         */
        appl_dnx_logical_port_to_sysport_get(unit, 0 /* base_modid */ , dst_port, &system_port_id);
        BCM_GPORT_SYSTEM_PORT_ID_SET(sysport_gport, system_port_id);
        /*
         * map sysport_gport to modport_gport
         */
        bcm_stk_sysport_gport_get(unit, sysport_gport, &modport_gport);

        ingress_context.destination_port = modport_gport;
        ingress_context.is_mc = FALSE;
    }
    else
    {
    ...
    ingress_context.is_wide_stream_id = set_interface_config_p->stream_id_wide_enable;
 ...
 *   of replications.
 * \param [in] mcast_port -
 *   Array of bcm_port_t. Ports to add to multicast group(s). Number of elemnts on this
    ...
    bcm_port_t dst_port)
{
    ...
    }
    {
        ...
        for (ii = 0; ii < MAX_NUM_MCAST_PORTS; ii++)
        {
            ...
            if (ii < num_members_in_multicast_group)
            {
    ...
    sh_sand_control_t * sand_control)
{
    ...
    }
    {
         ...
         */
        char expected_value[DSIG_MAX_SIZE_STR];
        uint8 result[DSIG_MAX_SIZE_STR];
        int size;
        /*
...
exit:
    ...
    if (SHR_FUNC_ERR())
    {
        ...
        LOG_CLI_EX("==> wide_stream_enable %d %s%s%s\r\n",
                   set_interface_config.stream_id_wide_enable, EMPTY, EMPTY, EMPTY);
...
static shr_error_e
dnx_tdm_port_port_to_interface_mapping_info(
    int unit,
    bcm_port_t port,
    ...
    char *description)
{
    ...
    LOG_CLI_EX("\r\n %s %s %s %s", description, EMPTY, EMPTY, EMPTY);
    LOG_CLI_EX("\r\n "
               "adding port: interface_info.interface_id %d. mapping_info.channel %d. \r\n"
               "mapping_info.core %d. mapping_info.tm_port %d. \r\n\n",
               interface_info.interface_id, mapping_info.channel, mapping_info.core, mapping_info.tm_port);
    *nif_interface_id = interface_info.interface_id;
...
static shr_error_e
dnx_tdm_egress_bypass_test_cmd(
    int unit,
    args_t * args,
    sh_sand_control_t * sand_control)
{
    char *test_name = "dnx_tdm_egress_bypass_test_cmd";
    char *packet_n, *tdm_mode_str, *ucode_port_type;
    char ucode_port_name_src[RHNAME_MAX_SIZE], ucode_port_name_all_src[RHNAME_MAX_SIZE];
    char ucode_port_value_src[RHNAME_MAX_SIZE], egress_bypass_soc_property[RHNAME_MAX_SIZE];
    char ucode_port_name_dst[RHNAME_MAX_SIZE], ucode_port_name_all_dst[RHNAME_MAX_SIZE];
    char ucode_port_value_dst[RHNAME_MAX_SIZE];
    char gen_string[RHNAME_MAX_SIZE];
    rhhandle_t ctest_soc_set_h = NULL, packet_h = NULL;

    int multicast_scanario = FALSE, expect_fail_on_is_tdm = FALSE, is_ilkn = FALSE, to_clean = FALSE;
    int nif_interface_id, speed, core_id, core_id_dst, ingress_context_id, multicast_id;
    uint32 tm_port, channel_id_src, channel_id_dst;
    bcm_port_t dst_port, src_port, cpu_port;
    bcm_pbmp_t logical_ports;
    ctest_soc_property_t ctest_soc_property[MAX_NUM_SOC_PROPERTIES];
    uint32 octets_rx_from_src_port = 0, octets_tx_from_dst_port = 0, err_pkt_rx_from_src_port =
        0, err_pkt_tx_from_dst_port = 0;
    uint64 tdm_packet_counter, fdt_egq_tdm_packet_counter;
    uint32 tdm_dropped_packet_counter, err_tdm_packet_counter;

    SHR_FUNC_INIT_VARS(unit);

    tdm_mode_str = "TDM_OPTIMIZED";
    SH_SAND_GET_STR("packet", packet_n);
    SH_SAND_IS_PRESENT("multicast_id", multicast_scanario);
    ...
    sal_snprintf(egress_bypass_soc_property, RHNAME_MAX_SIZE, "%s.0", "custom_feature_tdm_egress_bypass_enable");
    /**Egress bypass configuration*/
    SHR_IF_ERR_EXIT(dnx_ctest_soc_add(unit, ctest_soc_property, egress_bypass_soc_property, "1"));
    ...
    SHR_IF_ERR_EXIT(dnx_tdm_get_first_and_only_port(unit, logical_ports, &src_port));
    {
        SHR_IF_ERR_EXIT(dnx_tdm_port_port_to_interface_mapping_info(unit, src_port, &nif_interface_id,
                                                                    &channel_id_src, &tm_port, &core_id, &speed,
                                                                    "Source port"));
        SHR_IF_ERR_EXIT(dnx_tdm_convert_speed_to_port_type_str(unit, speed, &is_ilkn, &ucode_port_type));

        sal_snprintf(ucode_port_name_src, RHNAME_MAX_SIZE, "ucode_port_%d", src_port);
        sal_snprintf(ucode_port_name_all_src, RHNAME_MAX_SIZE, "%s.*", ucode_port_name_src);

        sal_snprintf(ucode_port_value_src, RHNAME_MAX_SIZE, "%s%d:core_%d.%d:%s:tdm", ucode_port_type,
                     nif_interface_id, core_id, tm_port, "if_tdm_only");
    ...
    SHR_IF_ERR_EXIT(dnx_tdm_get_first_and_only_port(unit, logical_ports, &dst_port));
    {
        ...
        {
            SHR_IF_ERR_EXIT(dnx_tdm_port_port_to_interface_mapping_info(unit, dst_port, &nif_interface_id_dst,
                                                                        &channel_id_dst, &tm_port_dst, &core_id_dst,
                                                                        &speed, "Destination port"));
        }
        SHR_IF_ERR_EXIT(dnx_tdm_convert_speed_to_port_type_str(unit, speed, FALSE, &ucode_port_type_dst));
        sal_snprintf(ucode_port_name_dst, RHNAME_MAX_SIZE, "ucode_port_%d", dst_port);
        sal_snprintf(ucode_port_name_all_dst, RHNAME_MAX_SIZE, "%s.*", ucode_port_name_dst);
        sal_snprintf(ucode_port_value_dst, RHNAME_MAX_SIZE, "%s%d:core_%d.%d:if_tdm_only:tdm", ucode_port_type_dst,
                     nif_interface_id_dst, core_id_dst, tm_port_dst);
    ...
    {
        bcm_tdm_interface_config_t set_interface_config;
        bcm_tdm_ingress_editing_type_t type = 0;
        bcm_port_t *fabric_ports_p;
        int fabric_ports_count;
        ...
        SHR_IF_ERR_EXIT(dnx_tdm_ire_configure
                        (unit, TRUE, src_port, channel_id_src, !multicast_scanario, dst_port, type,
                         FALSE, multicast_id, &set_interface_config, fabric_ports_p, fabric_ports_count,
                         &ingress_context_id));
    }
    ...
    if (!multicast_scanario)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_tdm_ire_configure
                        (unit, do_not_override_context, dst_port, channel_id_dst, !multicast_scanario, cpu_port, type,
                         loc_stamp_mcid_with_sid, 0, &set_interface_config, fabric_ports_p,
...
exit:
    ...
    if (SHR_FUNC_ERR())
    {
        ...
        LOG_CLI_EX("\r\n"
                   "Test '%s' has FAILED with the following parameters:\r\n"
                   " src_port %d %s%s\r\n", test_name, src_port, EMPTY, EMPTY);
        LOG_CLI_EX("==> cpu_port %d, Packet_name %s, clean %d dst_port %d, \r\n", cpu_port, packet_n, to_clean,
                   dst_port);
    }
    else
    {
    ...
    sh_sand_control_t * sand_control)
{
    ...
    bcm_port_t src_port, cpu_port;
    bcm_port_t dst_port;
    bcm_port_t blocked_port;

    bcm_port_t mcast_port[MAX_NUM_MCAST_PORTS];
    int dst_present;
    int blocked_channel_present;
    ...
    uint32 channel_id_src;
    uint32 tm_port;
    uint32 channel_id_blocked = -1;
    uint32 tm_port_blocked;
    char ucode_port_name_src[RHNAME_MAX_SIZE];
    char ucode_port_name_all_src[RHNAME_MAX_SIZE];
    char ucode_port_value_src[RHNAME_MAX_SIZE];
    char ucode_port_name_blocked[RHNAME_MAX_SIZE];
    char ucode_port_name_all_blocked[RHNAME_MAX_SIZE];
    char ucode_port_value_blocked[RHNAME_MAX_SIZE];
    char ucode_port_name_dst[RHNAME_MAX_SIZE];
    char ucode_port_value_dst[RHNAME_MAX_SIZE];
    char ucode_port_name_all_dst[RHNAME_MAX_SIZE];
    char ucode_port_name_mcast[MAX_NUM_MCAST_PORTS][RHNAME_MAX_SIZE];
    ...
    ctest_soc_property_t ctest_soc_property[MAX_NUM_SOC_PROPERTIES];
    uint32 octets_rx_from_src_port, octets_tx_from_dst_port;
    uint32 octets_rx_from_blocked_port;
    uint32 err_pkt_rx_from_src_port, err_pkt_tx_from_dst_port;
    uint32 err_pkt_rx_from_blocked_port;
    int is_ilkn_src;
    int is_ilkn_dst;
    int is_ilkn_mcast[MAX_NUM_MCAST_PORTS];
    uint32 channel_id_dst;
    bcm_multicast_t multicast_id;
    int multicast_cud;
    int multicast_id_present, multicast_cud_present;
    uint32 vid = 0, wide_sid;
    int expect_fail_on_is_tdm;
    ...
    int stream_id_enable, vid_stream_enable, wide_sid_enable;
    int stream_id_base, stream_id_key_size;
    int num_members_in_multicast_group;
    uint64 tdm_packet_counter;
    uint32 tdm_dropped_packet_counter;
    ...
    uint32 ftmh_opt_enabled;
    /*
     * chosen because those ports do not exist in the config.bcm file for j2c and q2a
     */
    const int BLOCKED_PORT = 12;
    ...
    SH_SAND_GET_BOOL("wide_sid", wide_sid_enable);

    if ((stamp_mcid_with_sid == FALSE) && ((vid_stream_enable != FALSE) || (wide_sid_enable != FALSE)))
    {
        SHR_ERR_EXIT(_SHR_E_FAIL,
                     "If VID stamping mode is enabled then 'stamp_mcid_with_sid' must be set as well.\r\n");
    }
    if ((wide_sid_enable == TRUE)
        && !dnx_data_tdm.params.feature_get(unit, dnx_data_tdm_params_is_sid_wide_mode_supported))
    {
        SHR_ERR_EXIT(_SHR_E_UNAVAIL,
                     "wide mode stream id is not supported on this device, which makes the test unsupported.\r\n");
    }
    ...
    SH_SAND_IS_PRESENT("blocked_channel_id", blocked_channel_present);
    {
        /*
         * add blocked channel in case it was provided in the cli
         */
        if (blocked_channel_present != FALSE)
        {
            SH_SAND_GET_UINT32("blocked_channel_id", channel_id_blocked);
            if (!is_ilkn_src)
            {
                SHR_ERR_EXIT(_SHR_E_FAIL, "\r\n blocked channel id scenario is expected on ilkn port");
            }
            tm_port_blocked = BLOCKED_PORT;
            blocked_port = BLOCKED_PORT;
            sal_snprintf(ucode_port_name_blocked, RHNAME_MAX_SIZE, "ucode_port_%d", blocked_port);
            sal_snprintf(ucode_port_name_all_blocked, RHNAME_MAX_SIZE, "%s.*", ucode_port_name_blocked);
            sal_snprintf(ucode_port_value_blocked, RHNAME_MAX_SIZE, "%s%d.%d:core_%d.%d:%s:tdm", ucode_port_type,
                         nif_interface_id, channel_id_blocked, core_id, tm_port_blocked,
                         sh_sand_enum_value_text(sand_control, "if_type", if_type));
            SHR_IF_ERR_EXIT(dnx_ctest_soc_add(unit, ctest_soc_property, ucode_port_name_all_blocked, NULL));
            SHR_IF_ERR_EXIT(dnx_ctest_soc_add
                            (unit, ctest_soc_property, ucode_port_name_blocked, ucode_port_value_blocked));
    ...
    SHR_IF_ERR_EXIT(dnx_tdm_get_first_and_only_port(unit, logical_ports, &dst_port));
    {
        ...
        if (dst_present == FALSE)
        {
            ...
            int mcast_index;
            uint32 channel_id_mcast[MAX_NUM_MCAST_PORTS];
            ...
            {
                SHR_IF_ERR_EXIT(dnx_tdm_port_port_to_interface_mapping_info
                                (unit, mcast_port[mcast_index], &nif_interface_id_mcast[mcast_index],
                                 &channel_id_mcast[mcast_index], &tm_port_mcast[mcast_index],
                                 &core_id_mcast[mcast_index], &speed_mcast[mcast_index], "Multicast port"));
     ...
     */
    {
        ...
        SH_SAND_GET_INT32("offset", set_interface_config.stream_id_key_offset);
        set_interface_config.stream_id_enable = stream_id_enable;
        set_interface_config.stream_id_wide_enable = wide_sid_enable;
        SH_SAND_GET_ENUM("context_type", type);
        do_not_override_context = TRUE;
        ...
        SHR_IF_ERR_EXIT(dnx_tdm_ire_configure
                        (unit, do_not_override_context, src_port, channel_id_src, dst_present, dst_port, type,
                         ...
                         &ingress_context_id));
        if (wide_sid_enable && (ingress_context_id != DNX_TDM_INGRESS_CONTEXT_FOR_WIDE_STREAM_ID))
        {
            SHR_ERR_EXIT(_SHR_E_BADID,
                         "\r\n wide stream id mode is on so the context id must be DNX_TDM_INGRESS_CONTEXT_FOR_WIDE_STREAM_ID");
        }
    }
    {
        /*
    ...
    }
    {
        ...
        /**
        * get multicast id value when working in wide_sid mode, this will be later used
        * as the expected stream id
        */
        if (wide_sid_enable)
        {
            SHR_IF_ERR_EXIT(diag_sand_packet_proto_get_uint_field(unit, packet_n, "ETH2", "Type", NULL, &wide_sid));
        }
    ...
    }
    {
         ...
         * Also, make sure to reserve space for ending NULL.
         */
        char header_to_add_expected_value[DSIG_MAX_SIZE_STR];

        uint8 header_to_add_result[DSIG_MAX_SIZE_STR];
        ...
        if (use_optimized_ftmh)
        {
            ...
            else
            {
                ...
                if (egress_edit_type == bcmTdmEgressEditingStampMulticastEncapId)
                {
                     ...
                     */
                    char expected_cud[DSIG_MAX_SIZE_STR];
                    uint8 result_cud[DSIG_MAX_SIZE_STR];
                ...
                else if (stamp_mcid_with_sid == TRUE)
                {
                    ...
                    else if (wide_sid_enable)
                    {

                        if (ftmh_stamped_value != stream_id_base + wide_sid)
                        {
                            SHR_ERR_EXIT(_SHR_E_FAIL,
                                         "\r\n"
                                         "using wide stream id, and the stamped value (%d) is not as expected (%d) "
                                         "base should be 0 and it is %d Quit.\r\n\n",
                                         ftmh_stamped_value, multicast_id, stream_id_base);
                        }
                    }
                    else
                    {
    ...
    {
        /*
         * now starts the blocked port testing. we send traffic through a port with no tdm defined
         * traffic expected to be dropped.
         * should be done only after the src port checks were done.
         */
        LOG_CLI_EX("\r\n" "------------Blocked Source Port testing begins------------%s%s%s%s\r\n", EMPTY, EMPTY, EMPTY,
                   EMPTY);
        /*
         * Close loopback on blocked_port, so that traffic injected by CPU to this port will enter as tdm
         * this will enter the traffic through the nif!
         */
        SHR_IF_ERR_EXIT(bcm_port_loopback_set(unit, blocked_port, BCM_PORT_LOOPBACK_MAC));

        SHR_IF_ERR_EXIT(bcm_port_force_forward_set(unit, cpu_port, blocked_port, 1));
        {
            /*
             * Make sure to take counters' values before starting the test.
             */
            octets_tx_from_dst_port = 0;
            octets_rx_from_blocked_port = 0;
            err_pkt_tx_from_dst_port = 0;
            err_pkt_rx_from_blocked_port = 0;

            SHR_IF_ERR_EXIT(dnx_tdm_load_src_dst_counters
                            (unit, TRUE, blocked_port, dst_port, &octets_rx_from_blocked_port, &octets_tx_from_dst_port,
                             &err_pkt_rx_from_blocked_port, &err_pkt_tx_from_dst_port));
        }
        {
            SHR_IF_ERR_EXIT(diag_sand_packet_send(unit, cpu_port, packet_h, SAND_PACKET_RESUME));
            sal_msleep(1000);
        ...
        {
            LOG_CLI_EX("\r\n" "Blocked Source Port %s%s%s%s\r\n", EMPTY, EMPTY, EMPTY, EMPTY);
            sal_snprintf(gen_string, RHNAME_MAX_SIZE, "show counter full port=%d", blocked_port);
            SHR_IF_ERR_EXIT(sh_process_command(unit, gen_string));

        }
        {
            /*
             * Get counters info on received/transmitted packets for blocked port
             */
            SHR_IF_ERR_EXIT(dnx_tdm_load_src_dst_counters
                            (unit, FALSE, blocked_port, dst_port, &octets_rx_from_blocked_port,
                             &octets_tx_from_dst_port, &err_pkt_rx_from_blocked_port, &err_pkt_tx_from_dst_port));
        }
        /*
         * At this point, the following holds:
         *   octets_rx_from_blocked_port is the number of bytes received from blocked port. none zero as we did forward traffic
         *   err_pkt_rx_from_blocked_port is the number of error packets detected on reception from blocked port - should not be zero.
         *
         *   the counters on dest port should be equal to zero:
         *   octets_tx_from_dst_port is the number of bytes transmitted through destination port
         *   err_pkt_tx_from_dst_port is the number of error packets detected on transmission through destination port
         */
        if (octets_rx_from_blocked_port == 0)
        {
            SHR_ERR_EXIT(_SHR_E_FAIL,
                         "\r\n"
                         "traffic from cpu didn't reach the blocked source port. issue with the test. octets_rx_from_blocked_port %d \r\n\n",
                         octets_rx_from_blocked_port);
        }
        if (octets_tx_from_dst_port != 0)
        {
            SHR_ERR_EXIT(_SHR_E_FAIL,
                         "\r\n" "traffic did pass on the non-tdm-defined port: octets_tx_from_dst_port %d\r\n\n",
                         octets_tx_from_dst_port);
        }
        if ((err_pkt_rx_from_blocked_port != 0) || (err_pkt_tx_from_dst_port != 0))
        {
            SHR_ERR_EXIT(_SHR_E_FAIL,
                         "\r\n"
                         "blocked port is %d. Destination port is %d\r\n"
                         "There should not be any errors on the blocked port (err_pkt_rx_from_blocked_port = %d) or on dst_port (err_pkt_tx_from_dst_port = %d) . Quit.\r\n\n",
                         blocked_port, dst_port, err_pkt_rx_from_blocked_port, err_pkt_tx_from_dst_port);
        }
    }
    ...
    bcm_port_t port)
{
    ...
    bcm_gport_t destination_gport;
    bcm_module_t egress_modid;
    int profile_is_rate_class = dnx_data_iqs.credit.feature_get(unit, dnx_data_iqs_credit_profile_is_rate_class);
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SHR_IF_ERR_EXIT(dnx_tdm_get_first_and_only_port(unit, logical_ports, &dst_port));
    {
        ...
        SHR_IF_ERR_EXIT(bcm_dnx_port_get(unit, dst_port, &flags, &interface_info, &mapping_info));
...
/** can't use dnx data at this stage.
 *  _SHR_E_NOT_FOUND is marking this diag is not supported
 *  diag = shell commands
 *   */
static shr_error_e
ctest_dnx_tdm_bypass_wide_sid_support_check(
    int unit,
    rhlist_t * invoke_list)
{
    SHR_FUNC_INIT_VARS(unit);
    ...
    if (SHR_GET_CURRENT_ERR() == _SHR_E_NONE)
    {
        /** Reach here if unit is a real 'device'. */
        if (!dnx_data_tdm.params.feature_get(unit, dnx_data_tdm_params_is_sid_wide_mode_supported))
        {
 ...
 *   */
static shr_error_e
ctest_dnx_tdm_egress_bypass_support_check(
    int unit,
    rhlist_t * invoke_list)
{
    SHR_FUNC_INIT_VARS(unit);
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (dnx_data_headers.optimized_ftmh.feature_get(unit, dnx_data_headers_optimized_ftmh_ftmh_opt_parse))
    {
        ...
        /** send packets and verify result*/
        {
            ...
            {
                uint8 result_str[DSIG_MAX_SIZE_STR];
                char dummy_expected_value[DSIG_MAX_SIZE_STR] = { 0 };
                int size;
    ...
    sh_sand_control_t * sand_control)
{
    ...
    num_members_in_multicast_group = 1;
    ...
    /*
     * Default ports were changed and, now, they are not the same for J2C and Q2A. So
     * we apply them, hard coded, and remove the previous parametric implementation.
     *
    ...
    /*
     * Get 'mcast_port_2' whether it is or is not present on the command line.
     * If it is not present, then get its defualt value.
     */
    SH_SAND_GET_PORT("mcast_port_2", logical_ports);
    SHR_IF_ERR_EXIT(dnx_tdm_get_first_and_only_port(unit, logical_ports, &mcast_port[1]));
...
sh_sand_cmd_t dnx_tdm_test_cmds[] = {
    ...
    {"bypass_mode_wide_sid", dnx_tdm_bypass_test_cmd, NULL, dnx_tdm_bypass_test_arguments, &dnx_tdm_bypass_wide_sid_test_man, NULL,
                    dnx_tdm_bypass_wide_sid_tests, SH_CMD_CONDITIONAL * SH_CMD_NO_XML_VERIFY, ctest_dnx_tdm_bypass_wide_sid_support_check},
    {"egress_bypass_mode", dnx_tdm_egress_bypass_test_cmd, NULL, dnx_tdm_egress_bypass_test_arguments, &dnx_tdm_egress_bypass_test_man, NULL,
                    dnx_tdm_egress_bypass_tests, SH_CMD_CONDITIONAL * SH_CMD_NO_XML_VERIFY, ctest_dnx_tdm_egress_bypass_support_check},
