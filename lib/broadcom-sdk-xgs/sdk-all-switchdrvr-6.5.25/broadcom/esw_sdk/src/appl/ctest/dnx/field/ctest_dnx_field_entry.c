...
sh_sand_option_t dnx_field_entry_options[] = {
    ...
    {DNX_DIAG_FIELD_ENTRY_OPTION_TEST_TYPE,   SAL_FIELD_TYPE_ENUM,    "Type (level) of test (bcm)",                  "BCM", (void *)Field_level_enum_table},
...
sh_sand_man_t sh_dnx_field_entry_man = {
    ...
    "ctest field entry type=<BCM> stage=<IPMF1 | IPMF2 | IPMF3 | EPMF>",
    "type=BCM stage=IPMF1\r\n",
};
...
static dnx_field_group_t field_group[NUM_FGS] = { 0 };
    ...
    uint32 cur_fg_entry)
{
    ...
    for (entry_itterator = 0; entry_itterator <= cur_fg_entry; entry_itterator++)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_field_map_qual_bcm_to_dnx(unit, dnx_field_stage,
                                                      bcm_qual_array2[dnx_field_stage][entry_itterator], &dnx_qual));
        SHR_IF_ERR_EXIT(dnx_field_map_dnx_qual_size(unit, dnx_field_stage, dnx_qual, NULL, NULL, &qual_size));
 ...
 */
static shr_error_e
ctest_dnx_field_entry_semantic(
    int unit,
    args_t * args,
    ...
    if (field_entry_test_type_name == 0)
    {
        for (fg_index = 0; fg_index < NUM_FGS; fg_index++)
        {
            ...
            SHR_IF_ERR_EXIT(bcm_field_group_add
                            (unit, 0, &bcm_fg_info, (bcm_field_group_t *) & (field_group[fg_index])));
        ...
        for (fg_index = 0; fg_index < NUM_FGS; fg_index++)
        {
            for (cur_fg_entry = 0; cur_fg_entry < NUM_ENTRIES; cur_fg_entry++)
            {
                SHR_IF_ERR_EXIT(bcm_field_entry_delete
                                (unit, field_group[fg_index], NULL, bcm_entry_handle_array[fg_index][cur_fg_entry]));
            }
            SHR_IF_ERR_EXIT(bcm_field_group_delete(unit, field_group[fg_index]));
        }
    }
    else
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "test type not supoprted (%d)\r\n", field_entry_test_type_name);
    }
exit:
    SHR_FUNC_EXIT;
}
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (count_iter = 0; count_iter < count; count_iter++)
    {
        ...
        SHR_IF_ERR_EXIT(ctest_dnx_field_entry_semantic(unit, args, sand_control));
