    ...
    bcm_vlan_t vsi)
{
    ...
    for (mac_ind = 0; mac_ind < *entries_count; mac_ind++)
    {
        ...
        result = bcm_l2_addr_add(unit, &l2addr);
        /*
         * *update entries_count to be the number of entries successfully added.
         */
        if (result != _SHR_E_NONE)
        {
            if (result == _SHR_E_FULL)
            {
                break;
            }
            else
...
/*
 * Call back to print entry including the group
 */
volatile int call_back_done = FALSE;
int call_back_user_data[2];
    ...
    void *ud)
{
    int print;
    int user_info;
    int *user_data = (int *) ud;
    user_info = user_data[0];
    print = user_data[1];

    if (print)
    {
        sal_printf("call_back_get_block: user_data = %d.\n", user_info);

        if (l2e == NULL)
        {
            sal_printf("call_back_get_block: finished, no more data.\n");
            call_back_done = TRUE;
        }
        else
        {
            sal_printf("| %02x:%02x:%02x:%02x:%02x:%02x |",
                       l2e->mac[0], l2e->mac[1], l2e->mac[2], l2e->mac[3], l2e->mac[4], l2e->mac[5]);
            sal_printf(" VLAN=0x%04x | PORT=0x%08x |", l2e->vid, l2e->port);
            sal_printf("GROUP=0x%x|\n", l2e->group);
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (non_blocking)
    {
        ...
        call_back_user_data[0] = vsi;
        call_back_user_data[1] = print;
        call_back_counter = 0;
        ...
        usec2 = sal_time_usecs() - usec;
        LOG_CLI((BSL_META("traverse commit done took %u us\n"), usec2));

        while (call_back_done == FALSE && print)
        {
        ...
        usec = sal_time_usecs() - usec;
        LOG_CLI((BSL_META("match traverse the entries took %u us\n"), usec));
        LOG_CLI((BSL_META("Call-back finished, call_back_counter = %d.\n"), call_back_counter));
        if (print && (call_back_counter != mac_entries_count))
        {
...
static sh_sand_man_t dnx_l2_flush_cleanup_man = {
    "L2 Flush buffer cleanup Test",
};

/*
 * \brief
 * This callback iterates over a specified nof entries and return an error once it reached the number
 * \param [in] unit - unit id
 * \param [in] l2e - pointer to the entry to perform the callback on.
 * \param [in] ud - pointer to the user data, this used data is parsed as int[2]:
 *                  ud[0]- is a counter (for the entries)
 *                  ud[1]- is the nof entries that we want to reach before returning an error, if given zero it will
 *                        traverse all the enries.
 */
int
call_back_traverse_and_abort(
    int unit,
    bcm_l2_addr_t * l2e,
    void *ud)
{
    ...
    user_data[0]++;
    if (user_data[1] && (user_data[0] == user_data[1]))
    {
        return BCM_E_FAIL;
    }
    return BCM_E_NONE;
...
/*
 * \brief
 * Verifies that the buffers of the flush machine are cleared in case that one of the
 * callbackes returns an error.
 *  The following stages are performed in non-blocking mode
 *      1. The following stages are performed for different number of initial number of entries in the MACT
 *          1. Add entries to the MACT with 2 different vlan IDs
 *          2. Traverse the entries and return an error in the middle of traversing
 *          3. Delete by vlan half the MACT entries
 *          4. Traverse the entries with the deleted vlan and make sure that there is no entry left in the buffer
 */
static shr_error_e
dnx_l2_flush_cleanup_cmd(
    int unit,
    args_t * args,
    sh_sand_control_t * sand_control)
{
    int iter;
    uint32 entries_count[2] = { 100, 500 };
    bcm_vlan_t vsi = 1;
    bcm_vlan_t vsi_to_remove = 2;
    rhhandle_t ctest_soc_set_h = NULL;
    SHR_FUNC_INIT_VARS(unit);

    /**
     * Reduce the flush buffer size, so the test will not be too time consuming
     */
    {
        ctest_soc_property_t ctest_soc_property[] = { {"l2_flush_buffer_nof_entries", "1000"}, {NULL} };
    ...
    for (iter = 0; iter < 2; iter++)
    {
        uint32 flags;
        bcm_l2_addr_t match_addr;
        bcm_mac_t mac_addr = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };

        /*
         * Fill the MACT with entries of 2 different VSIs
         */
        SHR_IF_ERR_EXIT(dnx_l2_add_entries(unit, &(entries_count[iter]), TRUE, vsi));
        SHR_IF_ERR_EXIT(dnx_l2_add_entries(unit, &(entries_count[iter]), TRUE, vsi_to_remove));

        /*
         * traverse the MACT entries and return an error (callback should return an error while the flush is in progress
         */
        bcm_l2_addr_t_init(&match_addr, mac_addr, vsi);
        match_addr.flags = BCM_L2_STATIC;
        flags = BCM_L2_TRAVERSE_IGNORE_PENDING;
        /*
         * we use the used data as follows:
         * call_back_user_data[0] is a counter for the entries that we have traversed
         * call_back_user_data[1] is the nof entries we want to go through before returning an error
         */
        call_back_user_data[0] = 0;
        call_back_user_data[1] = 20;
        SHR_IF_ERR_EXIT(bcm_l2_matched_traverse
                        (unit, flags, &match_addr, call_back_traverse_and_abort, &call_back_user_data));

        /*
         * wait for flush work to be done
         */
        {
            int flush_done = 0;
            while (!flush_done)
        ...
        /*
         * traverse and remove entries by VSI, vsi_to_remove, from the HW
         */
        flags = BCM_L2_DELETE_STATIC | BCM_L2_DELETE_NO_CALLBACKS;
        SHR_IF_ERR_EXIT(bcm_l2_addr_delete_by_vlan(unit, vsi_to_remove, flags));

        /*
         * traverse and verify that the memory is sufficient
         */
        flags = BCM_L2_TRAVERSE_MATCH_VLAN | BCM_L2_TRAVERSE_MATCH_STATIC;
        bcm_l2_addr_t_init(&match_addr, mac_addr, vsi_to_remove);
        /*
         * we use the used data as follows:
         * call_back_user_data[0] is a counter for the entries that we have traversed
         * call_back_user_data[1] is the nof entries we want to go through before returning an error, 0- don't return error at all
         */
        call_back_user_data[0] = 0;
        call_back_user_data[1] = 0;
        SHR_IF_ERR_EXIT(bcm_l2_matched_traverse
                        (unit, flags, &match_addr, call_back_traverse_and_abort, &call_back_user_data));
        SHR_VAL_VERIFY(call_back_user_data[0], 0, _SHR_E_FAIL, "Entries from the removed VSI were   retrieved");
        ...
        SHR_IF_ERR_EXIT(bcm_l2_replace(unit, flags, &match_addr, 0, 0, 0));
    }
    ...
    sh_sand_control_t * sand_control)
{
     ...
     */
    SHR_IF_ERR_EXIT(ctest_dnx_l2_send_packet_stream(unit, port2, vlan, mac_addr2, mac_addr1, 1));
    SHR_IF_ERR_EXIT(dnx_algo_port_out_tm_port_get(unit, port1, &core1, &out_tm_port));
    SHR_IF_ERR_EXIT(sand_signal_verify
                    (unit, core1, "ETPP", "PRP2", "", "Out_TM_Port", &out_tm_port, 1, NULL, return_value, 0));

    dnx_visibility_resume(unit, BCM_CORE_ALL,
                          BCM_INSTRU_CONTROL_FLAG_DIRECTION_INGRESS | BCM_INSTRU_CONTROL_FLAG_DIRECTION_EGRESS);

    SHR_IF_ERR_EXIT(ctest_dnx_l2_send_packet_stream(unit, port1, vlan, mac_addr1, mac_addr2, 1));
    SHR_IF_ERR_EXIT(dnx_algo_port_out_tm_port_get(unit, port2, &core2, &out_tm_port));
    SHR_IF_ERR_EXIT(sand_signal_verify
                    (unit, core2, "ETPP", "PRP2", "", "Out_TM_Port", &out_tm_port, 1, NULL, return_value, 0));
...
static sh_sand_option_t dnx_l2_flush_arguments[] = {
    ...
    {"print", SAL_FIELD_TYPE_INT32, "Show flush machine prints", "1"},
    {NULL}
};
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (i_cycle = 0; i_cycle < control.aging_cycles + 1;)
    {
        ...
        if (tx_flag == TRUE)
        {
            ...
            {
                SHR_IF_ERR_EXIT(sand_signal_verify
                                (unit, core1, "ETPP", "PRP2", "", "Out_TM_Port", &out_tm_port, 1, NULL,
                                 return_value, 0));
            }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    uint32 average_time[3] = { 100, 100, 120 };
    bcm_stg_t stg;
    ...
    sal_printf("AVERAGE time for bcm_vlan_create - %u us, expected approximately %u\n",
               (total_time[0] / nof_entries), average_time[0]);
    sal_printf("AVERAGE time for bcm_stg_vlan_add - %u us, expected approximately %u\n",
               (total_time[1] / nof_entries), average_time[1]);
    sal_printf("AVERAGE time for bcm_mpls_vpn_id_create - %u us, expected approximately %u\n",
    ...
    if (system_headers_mode != DBAL_ENUM_FVAL_SYSTEM_HEADERS_MODE_JERICHO_MODE)
    {
        ...
        SHR_IF_ERR_EXIT(sand_signal_verify
                        (unit, mapping_info_out_port.core, "ERPP", "Fabric", "", "FTMH_Base.FTMH_PP_DSP",
                         &ftmh_pp_dsp, 1, NULL, return_value, 0));
    }
    SHR_IF_ERR_EXIT(sand_signal_verify
...
static sh_sand_invoke_t dnx_l2_sw_hw_count_tests[] = {
    {"l2_mact_entry_count_sw_hw_check", "", CTEST_POSTCOMMIT},
    {NULL}
};

static sh_sand_man_t dnx_l2_sw_hw_count_man = {
    "MACT entry count check",
};

static sh_sand_option_t dnx_l2_sw_hw_count_arguments[] = {
    {"count", SAL_FIELD_TYPE_UINT32, "Number of addresses to add", "3"},
    {NULL}
};
/**
* \brief
* Add only STATIC entries in the MACT.
* Call delete API to delete dynamic entries.
* Verify the MACT entry count in SW was not decreased.
* Verify the MACT entry count in SW is equal to the entry count in HW.
*/
static shr_error_e
dnx_l2_sw_hw_count_cmd(
    int unit,
    args_t * args,
    sh_sand_control_t * sand_control)
{
    ...
    bcm_vlan_t vsi = 1;
    uint32 mac_entries_count;
    uint32 hw_entry_count = 0;
    int sw_entry_count = 0;

    SHR_FUNC_INIT_VARS(unit);

    SH_SAND_GET_UINT32("count", mac_entries_count);

    /** Add static MAC entries */
    SHR_IF_ERR_EXIT(dnx_l2_add_entries(unit, &mac_entries_count, is_static, vsi));

    /** Check the number of MACT entries in HW and in SW.*/
    SHR_IF_ERR_EXIT(sh_dnx_l2_get_mact_count(unit, &hw_entry_count));

#if defined BCM_DNX2_SUPPORT
    SHR_IF_ERR_EXIT(mdb_db_infos.
                    em_entry_count.get(unit, DBAL_PHYSICAL_TABLE_LEM, MDB_EM_ENTRY_ENCODING_HALF, &sw_entry_count));
    sal_printf("The MACT entry count in SW is %d, in HW is %d\n", sw_entry_count, hw_entry_count);
#endif

    if (sw_entry_count != hw_entry_count)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "MACT entry count in SW is not equal to MACT entry count in HW!");
    }
    /** Calling bcm_l2_addr_delete_by_vlan to delete dynamic MACT entries*/
    SHR_IF_ERR_EXIT(bcm_l2_addr_delete_by_vlan(unit, vsi, BCM_L2_DELETE_NO_CALLBACKS));
    ...
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "MACT entry count in SW is not equal to MACT entry count in HW after calling the delete API!");
    }

    sal_printf("MACT entry count test passed!\n");
    /** Clean up*/
    SHR_IF_ERR_EXIT(bcm_l2_addr_delete_by_vlan(unit, vsi, BCM_L2_DELETE_STATIC));
exit:
    SHR_FUNC_EXIT;
}
...
sh_sand_cmd_t dnx_l2_test_cmds[] = {
    ...
    {"flush_cleanup", dnx_l2_flush_cleanup_cmd,   NULL,                 NULL,                         &dnx_l2_flush_cleanup_man,NULL, NULL,                     CTEST_POSTCOMMIT * SH_CMD_ROLLBACK * SH_CMD_CONDITIONAL, sh_cmd_is_device},
    ...
    {"count",         dnx_l2_sw_hw_count_cmd,     NULL,                 dnx_l2_sw_hw_count_arguments, &dnx_l2_sw_hw_count_man,  NULL, dnx_l2_sw_hw_count_tests, CTEST_POSTCOMMIT },
    {NULL}
};
/* *INDENT-ON* */
