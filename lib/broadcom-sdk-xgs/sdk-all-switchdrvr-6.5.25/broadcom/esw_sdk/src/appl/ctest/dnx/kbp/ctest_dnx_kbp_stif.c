    ...
    int speed)
{
    ...
    uint32 flags;
    int nof_lanes, ethu_id, stif_unit_first_lane, stif_unit_nof_lanes, current_first_phy, current_core_first_phy,
        current_core_nof_phys;
    ...
    SHR_IF_ERR_EXIT(dnx_algo_port_nif_phys_first_phy_per_core_get(unit, core_id, &current_first_phy));
    ...
    for (i = 0; i < nof_ports; i++)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_algo_port_nif_phys_first_phy_per_core_get(unit, core_id, &current_core_first_phy));
        SHR_IF_ERR_EXIT(dnx_algo_port_nif_phys_nof_phys_per_core_get(unit, core_id, &current_core_nof_phys));
        if (current_first_phy >= current_core_first_phy + current_core_nof_phys)
        {
    ...
    int dp)
{
    ...
    switch (dp)
    {
        case BCM_FIELD_COLOR_GREEN:
            sal_strncpy_s((char *) (fg_info.name), "DpG", sizeof(fg_info.name));
            break;
        case BCM_FIELD_COLOR_YELLOW:
            sal_strncpy_s((char *) (fg_info.name), "DpY", sizeof(fg_info.name));
            break;
        case BCM_FIELD_COLOR_RED:
            sal_strncpy_s((char *) (fg_info.name), "DpR", sizeof(fg_info.name));
            break;
        case BCM_FIELD_COLOR_BLACK:
            sal_strncpy_s((char *) (fg_info.name), "DpB", sizeof(fg_info.name));
            break;
    }
...
static shr_error_e
dnx_kbp_stif_ing_egr_config_tab_db(
    int unit)
{
    int db_id, attr_value, counter_offset;
    kbp_status status;
    ...
    int profile)
{
    ...
    SHR_IF_ERR_EXIT(dnx_kbp_stif_acl_create(unit, 50, bcmFieldStageEgress, &cint_crps_context_id_epmf));
    ...
    int to_clean)
{
    ...
    static ctest_soc_property_arr_t soc_arr[MAX_NUM_SOC_PROPERTIES];
    bcm_port_config_t port_config;
    bcm_port_t port, stat_port, port_base = 21;
    ...
    ctest_soc_property_t ctest_soc_property[MAX_NUM_SOC_PROPERTIES] = {
        ...
        {"appl_enable_l2", "0"},
        {"stat_if_report_multicast_single_copy", "0"},
        {"port_init_cl72_ext_stat0", NULL},
        {"port_init_cl72_ext_stat1", NULL},
        {"port_init_cl72_ext_stat2", NULL},
        {"port_init_cl72_ext_stat3", NULL},
        /*
         * port_init_cl72_ext_stat configure all OP2_stat_ports with same value.
         */
        {"port_init_cl72_ext_stat", link_training_enable},
        /*
         * Configure all lanes that belong to stat ports with the same TXFIR values
         * by using serdes_tx_taps_ext_stat=txfir_config
         */
    ...
    sal_memset(soc_arr, 0, sizeof(soc_arr[0]));

    id = 0;
    SHR_IF_ERR_EXIT(bcm_port_config_get(unit, &port_config));
    BCM_PBMP_ITER(port_config.stat, stat_port)
    {
        sal_snprintf(soc_arr[id].property, RHNAME_MAX_SIZE, "%s%d", "port_init_cl72_", stat_port);
        SHR_IF_ERR_EXIT(ctest_dnxc_soc_add(unit, ctest_soc_property, soc_arr[id].property, link_training_enable));
        id++;
    }
    ...
    {
        /*
         * Disable other stat port
         */
        SHR_IF_ERR_EXIT(ctest_dnxc_soc_add(unit, ctest_soc_property, "port_init_speed_ext_stat2", NULL));
    }

    /*
     * Remove non stat nif port
     */
    BCM_PBMP_ITER(port_config.port, port)
    {
        if (BCM_PBMP_MEMBER(port_config.stat, port) || BCM_PBMP_MEMBER(port_config.sfi, port))
        {
            continue;
        }
        sal_snprintf(soc_arr[id].property, RHNAME_MAX_SIZE, "%s%d%s", "ucode_port_", port, ".*");
        SHR_IF_ERR_EXIT(ctest_dnxc_soc_add(unit, ctest_soc_property, soc_arr[id].property, NULL));
        id++;
    }
    BCM_PBMP_CLEAR(reserved_phy_pbmp);

    /*
     * Add port per core on free pms including cdu and clu(if supported)
     */
    ethu_type = -1;
    port = port_base;
    lanes = 2;
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core_id)
    {
        int nof_port;
        int pm_id;
        int pm_first_phy, pm_last_phy, port_first_phy;
        bcm_pbmp_t free_pm_pbmp;
        int port_index;

        SHR_IF_ERR_EXIT(ctest_dnx_port_free_pms_get
                        (unit, ethu_type, core_id, port_config.stat, reserved_phy_pbmp, &free_pm_pbmp));
        nof_port = 0;
        BCM_PBMP_ITER(free_pm_pbmp, pm_id)
        {
            int add_port_done_on_core;
            add_port_done_on_core = 0;
            ctest_dnx_pm_first_last_phy_get(unit, pm_id, &pm_first_phy, &pm_last_phy);
            for (port_first_phy = pm_first_phy; port_first_phy <= pm_last_phy; port_first_phy += lanes)
            {
                port_index = port_first_phy / lanes;
                sal_snprintf(soc_arr[id].property, RHNAME_MAX_SIZE, "%s%d", "ucode_port_", port);
                sal_snprintf(soc_arr[id].value, RHNAME_MAX_SIZE, "%s%d%s%d%s%d", "XLGE2_", port_index, ":core_",
                             core_id, ".", port);
                SHR_IF_ERR_EXIT(ctest_dnxc_soc_add(unit, ctest_soc_property, soc_arr[id].property, soc_arr[id].value));
                to_port[core_id][nof_port] = port;
                id++;
                port++;
                /*
                 * Skip stif port IDs
                 */
                while (BCM_PBMP_MEMBER(port_config.stat, port))
                {
                    port++;
                    if (nof_cores == 1)
                        break;
                ...
                {
                    /*
                     * Add 14 ports per core
                     */
                    add_port_done_on_core = 1;
                    break;
                }
            }
            if (add_port_done_on_core)
            {
                break;
            }
        }
    ...
    if (nof_cores > 1)
    {
        source.core = 1;
        source.src_type = bcmStatStifSourceIngressEnqueue;
        SHR_IF_ERR_EXIT(bcm_stat_stif_source_mapping_set(unit, 0, source,
   ...
   /** Lock device configuration */
    ...
    SHR_IF_ERR_EXIT(dnx_kbp_stif_acl_create(unit, 10, bcmFieldStageIngressPMF1, &context_id));
    sal_msleep(5000);

    BCM_PBMP_ITER(port_config.sat, port)
    {
        SHR_IF_ERR_EXIT(bcm_port_get(unit, port, &flags, &interface_info, &mapping_info));
        if (mapping_info.core <= 1)
        {
            sat_port[mapping_info.core] = port;
        }
    }
    ...
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core_id)
    {
        uint64 sat_send_count, packet_count, bytes_count;
        /** map in port to TC and DP */
        SHR_IF_ERR_EXIT(dnx_kbp_stif_acl_run(unit, context_id, sat_port[core_id], dp[core_id]));
        SHR_IF_ERR_EXIT(bcm_port_force_forward_set(unit, sat_port[core_id], to_port[core_id][0], 1));
        /*
         * sanke in order to get to 200G traffic rate
         */
        for (int i = 0; i < (DNX_KBP_STIF_VOQ_DB_TEST_NOF_PORTS - 1); i++)
        {
            bcm_port_loopback_set(unit, to_port[core_id][i], BCM_PORT_LOOPBACK_MAC);
            bcm_port_force_forward_set(unit, to_port[core_id][i], to_port[core_id][i + 1], 1);
        }
        SHR_IF_ERR_EXIT(bcm_port_get(unit, sat_port[core_id], &flags, &interface_info, &mapping_info));
        ...
        SHR_IF_ERR_EXIT(diag_sand_packet_send(unit, sat_port[core_id], packet_h, 0));
        SHR_IF_ERR_EXIT(diag_sand_packet_stream_get_stats(unit, packet_h, &bytes_count, &packet_count));
        LOG_CLI((BSL_META("SAT generated {0x%x,0x%x} Packets {0x%x,0x%x} Bytes from port=%d core=%d \n"),
                 COMPILER_64_HI(packet_count), COMPILER_64_LO(packet_count),
                 COMPILER_64_HI(bytes_count), COMPILER_64_LO(bytes_count), sat_port[core_id], core_id));
        COMPILER_64_COPY(sat_send_count, packet_count);
         ...
         */
        sal_msleep(5000);
        SHR_IF_ERR_EXIT(sh_process_command(unit, "nif sts"));
        for (int j = 0; j < DNX_KBP_STIF_VOQ_DB_TEST_NOF_PORTS; j++)
        {
            ...
            for (int i = 0; i < set_size; i++)
            {
                    ...
                    (i == 7 && core_id == 1 && j == 0) || (i == 0 && core_id == 0 && j > 0))
                {
                    ...
                    if (core_id == 0 && j == DNX_KBP_STIF_VOQ_DB_TEST_NOF_PORTS - 1)
                    {
                        /*
                         * Verify the packet counter on last port equal to sat generated value 
                         */
                        if (COMPILER_64_NE(sat_send_count, packet_count))
                        {
                            SHR_CLI_EXIT(_SHR_E_FAIL,
                                         "Counter Verification Failed on port %d: pkts sent by sat {0x%X,0x%X} pkts, pkts stat on port {0x%X,0x%X} pkts.\n",
                                         to_port[core_id][j], COMPILER_64_HI(sat_send_count),
                                         COMPILER_64_LO(sat_send_count), COMPILER_64_HI(packet_count),
                                         COMPILER_64_LO(packet_count));
                        }
                    }
 ...
 */
static shr_error_e
dnx_kbp_stif_egr_record_format_config(
    int unit,
    ...
    int packet_size)
{
    ...
    kbp_status status;
    int hole_len, idx;
    struct kbp_device *device_th;
    struct kbp_tap_record *rec_th;
    char name[DNX_KBP_STIF_ING_EGR_TEST_NOF_DB][32] = { "ING_ACL_PTR", "ING_QUEUE_ID", "EGR_ACE_PTR" };
    int qnum_mask = dnx_data_ipq.queues.queue_id_bits_get(unit);
    int ptr_len[DNX_KBP_STIF_ING_EGR_TEST_NOF_DB] = { 20, qnum_mask, 20 };

    SHR_FUNC_INIT_VARS(unit);

    for (idx = 0; idx < nof_threads; idx++)
    {

        device_th = kbp_mngr_smt_enabled(unit) ? kbp_stif_config.device_th[idx] : kbp_stif_config.device;

        status = kbp_tap_instruction_init(device_th, (idx * 2 + 1), KBP_TAP_INSTRUCTION_TYPE_EGRESS,
                                          &(kbp_stif_config.inst_th[idx * 2 + 1]));
        if (status != KBP_OK)
        {
            SHR_CLI_EXIT(_SHR_E_FAIL, "kbp_tap_instruction_init failed: %s\n", kbp_get_status_string(status));
        }
        status = kbp_tap_record_init(device_th, KBP_TAP_RECORD_TYPE_EGRESS, &rec_th);
        if (status != KBP_OK)
        {
            SHR_CLI_EXIT(_SHR_E_FAIL, "kbp_tap_record_init failed: %s\n", kbp_get_status_string(status));
        }
 ...
 */
static shr_error_e
dnx_kbp_stif_ing_record_format_config(
    int unit,
    unsigned int nof_threads,
    int packet_size)
{
    ...
    for (idx = 0; idx < nof_threads; idx++)
    {
        ...
        status =
            kbp_tap_instruction_init(device_th, (idx * 2), KBP_TAP_INSTRUCTION_TYPE_INGRESS,
                                     &(kbp_stif_config.inst_th[idx * 2]));

        if (status != KBP_OK)
        {
            SHR_CLI_EXIT(_SHR_E_FAIL, "kbp_tap_instruction_init failed: %s\n", kbp_get_status_string(status));
        }
        status = kbp_tap_record_init(device_th, KBP_TAP_RECORD_TYPE_INGRESS, &rec_th);
        if (status != KBP_OK)
        {
            SHR_CLI_EXIT(_SHR_E_FAIL, "kbp_tap_record_init failed: %s\n", kbp_get_status_string(status));
        }
...
static shr_error_e
dnx_kbp_stif_ing_egr_config_example_ports_configuration(
    int unit,
    int nof_ports)
{
    int first_port, nof_cores, core_id, flags;
    bcm_stat_stif_source_t source;
    SHR_FUNC_INIT_VARS(unit);

    nof_cores = dnx_data_device.general.nof_cores_get(unit);

    /** connect each source to two ports*/
    if (dnx_data_stif.config.feature_get(unit, dnx_data_stif_config_map_source_to_two_stif_ports))
    {
        DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core_id)
        {
            first_port = (core_id == 0) ? 33 : 35;

            flags = nof_ports == 4 ? BCM_STAT_SOURCE_MAPPING_FIRST_PORT : 0;

            source.core = core_id;
            source.src_type = bcmStatStifSourceIngressEnqueue;
            SHR_IF_ERR_EXIT(bcm_stat_stif_source_mapping_set(unit, flags, source, first_port));
            source.src_type = bcmStatStifSourceEgressDequeue;
            SHR_IF_ERR_EXIT(bcm_stat_stif_source_mapping_set(unit, flags, source, first_port));

            if (nof_ports == 4)
            {
                flags = BCM_STAT_SOURCE_MAPPING_SECOND_PORT;
                source.src_type = bcmStatStifSourceIngressEnqueue;
                SHR_IF_ERR_EXIT(bcm_stat_stif_source_mapping_set(unit, flags, source, first_port + 1));
                source.src_type = bcmStatStifSourceEgressDequeue;
                SHR_IF_ERR_EXIT(bcm_stat_stif_source_mapping_set(unit, flags, source, first_port + 1));
            }
        }
    ...
    else
    {
        ...
        SHR_IF_ERR_EXIT(bcm_stat_stif_source_mapping_set(unit, 0, source, (nof_ports == 1) ? 33 : 34));
    }
exit:
    SHR_FUNC_EXIT;
}
    ...
    int nof_stif_ports)
{
    ...
    SHR_IF_ERR_EXIT(bcm_stk_module_enable(unit, 0, 0, 0));
     ...
     /** mapping source to stif port*/
    SHR_IF_ERR_EXIT(dnx_kbp_stif_ing_egr_config_example_ports_configuration(unit, nof_stif_ports));
    ...
    status = dnx_kbp_stif_ing_egr_config_tab_db(unit);

    if (status != KBP_OK)
    {
        SHR_CLI_EXIT(_SHR_E_FAIL, "dnx_kbp_stif_ing_egr_config_tab_db failed: %s\n", kbp_get_status_string(status));
    }
    ...
    nof_threads = kbp_mngr_smt_enabled(unit) ? 2 : 1;
     ...
     /**configure ingress's record format in kbp side*/
    status = dnx_kbp_stif_ing_record_format_config(unit, nof_threads, packet_size);

    if (status != KBP_OK)
    {
        SHR_CLI_EXIT(_SHR_E_FAIL, "dnx_kbp_stif_ing_record_format_config failed: %s\n", kbp_get_status_string(status));
    }

     /**configure egress's record format in kbp side*/
    status = dnx_kbp_stif_egr_record_format_config(unit, nof_threads, packet_size);

    if (status != KBP_OK)
    {
        SHR_CLI_EXIT(_SHR_E_FAIL, "dnx_kbp_stif_egr_record_format_config failed: %s\n", kbp_get_status_string(status));
    }
    ...
    int unit)
{
    ...
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core_id)
    {
        ...
        /*
         * sending from SAT 80G. packet size=72 bytes. ==> 60G/72*(96+64 bits)=133.33G rate in the stif port
         */
        SHR_IF_ERR_EXIT(diag_sand_packet_stream_create(unit, core_id, packet_h[core_id], 72, 1,
                                                       BCM_SAT_GTF_RATE_IN_BYTES, 60000000, 1000,
                                                       SAT_GRANULARITY_SET_BY_DIAG));
    }
    status = kbp_device_scrub_tap_dma_buffer(kbp_stif_config.device);
    if (status != KBP_OK)
...
/**
* \brief - to config kbp stif soc properties and init soc.
* \param [in] unit                  - Device ID
* \param [in] speed                 - stif port speed
* \param [in] nof_lanes             - nof lanes per stif port
* \param [in] ext_tcam_connect_mode - connect mode
* \param [in] link_training_enable  - is lint training enable
* \param [out] ctest_soc_property   - output soc property array
* \return
*   int - Error Type
*/
static shr_error_e
dnx_kbp_stif_soc_properties_config(
    int unit,
    int speed,
    int nof_lanes,
    char *ext_tcam_connect_mode,
    char *link_training_enable,
    ctest_soc_property_t * ctest_soc_property)
{
    ...
    /*
     * Common soc config for stif test
     */
    SHR_IF_ERR_EXIT(ctest_dnxc_soc_add(unit, ctest_soc_property, "stat_if_enable", "1"));
    SHR_IF_ERR_EXIT(ctest_dnxc_soc_add(unit, ctest_soc_property, "stat_if_report_mode", "BILLING"));
    SHR_IF_ERR_EXIT(ctest_dnxc_soc_add(unit, ctest_soc_property, "stat_if_report_size_ingress", "96"));
    SHR_IF_ERR_EXIT(ctest_dnxc_soc_add(unit, ctest_soc_property, "stat_if_report_size_egress", "64"));
    SHR_IF_ERR_EXIT(ctest_dnxc_soc_add(unit, ctest_soc_property, "stat_if_idle_reports_present", "1"));
    ...
    if (speed != -1)
    {
        ...
        /*
         * Config soc items like port_init_speed_ext_stat0 ~ port_init_speed_ext_stat3 to NULL to remove them
         * Then port_init_speed_ext_stat takes effect for all the stat port ids.
         */
        SHR_IF_ERR_EXIT(ctest_dnxc_soc_add(unit, ctest_soc_property, "port_init_speed_ext_stat0", NULL));
        SHR_IF_ERR_EXIT(ctest_dnxc_soc_add(unit, ctest_soc_property, "port_init_speed_ext_stat1", NULL));
        SHR_IF_ERR_EXIT(ctest_dnxc_soc_add(unit, ctest_soc_property, "port_init_speed_ext_stat2", NULL));
        SHR_IF_ERR_EXIT(ctest_dnxc_soc_add(unit, ctest_soc_property, "port_init_speed_ext_stat3", NULL));

        sal_snprintf(soc_arr[id].property, RHNAME_MAX_SIZE, "%s", "port_init_speed_ext_stat");
        sal_snprintf(soc_arr[id].value, RHNAME_MAX_SIZE, "%d%s%d%s", speed, ".", nof_lanes, "lanes");
        SHR_IF_ERR_EXIT(ctest_dnxc_soc_add(unit, ctest_soc_property, soc_arr[id].property, soc_arr[id].value));
        id++;
        SHR_IF_ERR_EXIT(ctest_dnx_port_ucode_port_type_group_get
                        (unit, speed, nof_lanes, &ucode_port_type, &port_group));

        /*
         * Configure uncode_port for stif ports
         */
        BCM_PBMP_ITER(port_config.stat, port)
        {
            /** add ucode port soc property */
            int first_phy, last_phy;
            bcm_core_t core;
            SHR_IF_ERR_EXIT(ctest_dnx_port_first_last_phy_get(unit, port, &first_phy, &last_phy));
            port_index = first_phy / nof_lanes;
            SHR_IF_ERR_EXIT(dnx_algo_port_nif_phys_phy_to_core_id_get(unit, first_phy, &core));
            sal_snprintf(soc_arr[id].property, RHNAME_MAX_SIZE, "%s%d", "ucode_port_", port);
            sal_snprintf(soc_arr[id].value, RHNAME_MAX_SIZE, "%s%d%s%d%s", ucode_port_type, port_index, ":core_", core,
                         ":stat");
            SHR_IF_ERR_EXIT(ctest_dnxc_soc_add(unit, ctest_soc_property, soc_arr[id].property, soc_arr[id].value));
            id++;
        }
    ...
    /*
     * Configure all the stat ports in both sides to get the same link training configuration
     */
    SHR_IF_ERR_EXIT(ctest_dnxc_soc_add(unit, ctest_soc_property, "port_init_cl72_ext_stat0", NULL));
    SHR_IF_ERR_EXIT(ctest_dnxc_soc_add(unit, ctest_soc_property, "port_init_cl72_ext_stat1", NULL));
    SHR_IF_ERR_EXIT(ctest_dnxc_soc_add(unit, ctest_soc_property, "port_init_cl72_ext_stat2", NULL));
    SHR_IF_ERR_EXIT(ctest_dnxc_soc_add(unit, ctest_soc_property, "port_init_cl72_ext_stat3", NULL));
    /** configure all the stat ports with same property */
    SHR_IF_ERR_EXIT(ctest_dnxc_soc_add(unit, ctest_soc_property, "port_init_cl72_ext_stat", link_training_enable));
    BCM_PBMP_ITER(port_config.stat, port)
    {
        sal_snprintf(soc_arr[id].property, RHNAME_MAX_SIZE, "%s%d", "port_init_cl72_", port);
        sal_snprintf(soc_arr[id].value, RHNAME_MAX_SIZE, "%s", link_training_enable);
        SHR_IF_ERR_EXIT(ctest_dnxc_soc_add(unit, ctest_soc_property, soc_arr[id].property, soc_arr[id].value));
        id++;
    }

    if (!ISEMPTY(ext_tcam_connect_mode))
    {
        SHR_IF_ERR_EXIT(ctest_dnxc_soc_add(unit, ctest_soc_property, "ext_tcam_connect_mode", ext_tcam_connect_mode));
    }

    /*
     * The test is using DispositionIsDrop, which is not suppoerted when STIF is
     */
    /*
     * configured to count all MC copies as one on some devices.
     */
    if (!dnx_data_stif.config.feature_get(unit, dnx_data_stif_config_stif_report_disp_is_drop_with_mc_single_copy))
    {
        SHR_IF_ERR_EXIT(ctest_dnxc_soc_add(unit, ctest_soc_property, "stat_if_report_multicast_single_copy", "0"));
    }
    ...
    int to_clean)
{
    ...
    SHR_FUNC_INIT_VARS(unit);
    sal_memset(ctest_soc_property, 0, (MAX_NUM_SOC_PROPERTIES * sizeof(ctest_soc_property[0])));

    /*
     * config soc properties list, use the default speed and nof lanes
     */
    SHR_IF_ERR_EXIT(dnx_kbp_stif_soc_properties_config
                    (unit, -1 /* speed */ , -1 /* nof lanes */ , ext_tcam_connect_mode,
                     link_training_enable, ctest_soc_property));
    SHR_IF_ERR_EXIT(ctest_dnxc_soc_add(unit, ctest_soc_property, "serdes_tx_taps_ext_stat", txfir_config));
    ...
    int to_clean)
{
    ...
    int ing_rep_size = soc_is(unit, Q2A_DEVICE) ? 64 : 96;
    char ing_rep_size_str[3];
    ...
    sal_snprintf(ing_rep_size_str, 3, "%d", ing_rep_size);
    SHR_IF_ERR_EXIT(ctest_dnxc_soc_add(unit, ctest_soc_property, "stat_if_report_size_ingress", ing_rep_size_str));
    if (!dnx_data_stif.config.feature_get(unit, dnx_data_stif_config_stif_report_disp_is_drop_with_mc_single_copy))
    {
        SHR_IF_ERR_EXIT(ctest_dnxc_soc_add(unit, ctest_soc_property, "stat_if_report_multicast_single_copy", "0"));
    }
    ...
    /** 0.75 factors is received due to the fact that SAT is not considering IPG and PREEAMBLE into account. 72 / 72 + 8 + 12 */
    sat_rate =
        0.75 * (dnx_data_device.general.core_clock_khz_get(unit) / DNX_KBP_STIF_ING_EGR_ONE_PORT_TEST_NOF_PORTS) *
    ...
    ing_rec_size = ing_rep_size;
    egr_rec_size = 80;
    null_rec_size = ing_rep_size;
   ...
   /** Lock device configuration */
    ...
    SHR_IF_ERR_EXIT(diag_sand_packet_stream_create(unit, 0, packet_h, sat_packet_size, 1,
                                                   BCM_SAT_GTF_RATE_IN_BYTES, sat_rate, 1000,
                                                   SAT_GRANULARITY_SET_BY_DIAG));
...
/**
* \brief - to run kbp stif ingress and egress test on different nof stif ports with diffrent lanes.
* \param [in] unit                  - Device ID
* \param [in] speed                 - stif port speed
* \param [in] nof_ports             - nof stif ports
* \param [in] nof_lanes             - nof lanes per stif port
* \param [in] ext_tcam_connect_mode - connect mode
* \param [in] to_clean              - if to clean all configurations
* \return
*   int - Error Type
*/
static shr_error_e
dnx_kbp_stif_ing_egr_port_test(
    int unit,
    int speed,
    int nof_ports,
    int nof_lanes,
...
{
    rhhandle_t ctest_soc_set_h = NULL;
    char *link_training_enable = "1";
    int is_test_support = 1;
    ctest_soc_property_t ctest_soc_property[MAX_NUM_SOC_PROPERTIES];
    ...
    {
        is_test_support = 0;
        SHR_ERR_EXIT(_SHR_E_NONE, "The test case is applicable for only OP2 device\n");
    }

    SHR_IF_ERR_EXIT(dnx_kbp_stif_soc_properties_config
                    (unit, speed, nof_lanes, ext_tcam_connect_mode, link_training_enable, ctest_soc_property));
    SHR_IF_ERR_EXIT(ctest_dnxc_set_soc_properties(unit, ctest_soc_property, &ctest_soc_set_h));
    /*
     * Waiting for ports link up, the link of PAM4 port (speed per lane >=50G) needs up to 18s to be up
     */
    if (speed / nof_lanes < 50000)
    {
        sal_msleep(6000);
    }
    else
    ...
    SHR_IF_ERR_EXIT(sh_process_command(unit, "nif sts"));
    SHR_IF_ERR_EXIT(dnx_kbp_stif_ing_egr_config_example(unit, nof_ports));
    SHR_IF_ERR_EXIT(bcm_switch_control_set(unit, bcmSwitchExternalTcamSync, 0));
    SHR_IF_ERR_EXIT(dnx_kbp_stif_ing_egr_send_and_check(unit));
    SHR_IF_ERR_EXIT(dnx_kbp_stif_tap_destroy(unit,
                                             kbp_stif_config.db,
                                             kbp_stif_config.nof_db,
                                             kbp_stif_config.inst_th, kbp_stif_config.nof_instructions));

    LOG_CLI((BSL_META("TEST PASS!!! \n")));
exit:
    if (is_test_support && to_clean == TRUE)
    {
        ctest_dnxc_restore_soc_properties(unit, ctest_soc_set_h);
    }
    SHR_FUNC_EXIT;
    ...
    sh_sand_control_t * sand_control)
{
    ...
    char *ext_tcam_connect_mode = NULL;
    ...
    SH_SAND_GET_STR("ext_tcam_connect_mode", ext_tcam_connect_mode);

    ext_tcam_connect_mode = ISEMPTY(ext_tcam_connect_mode) ? NULL : ext_tcam_connect_mode;
    ...
    {
        case 0:
            SHR_IF_ERR_EXIT(dnx_kbp_stif_voq_db_test(unit, txfir_str, link_training_enable, to_clean));
            break;
        case 1:
            SHR_IF_ERR_EXIT(dnx_kbp_stif_ing_egr_two_ports_test
                            (unit, txfir_str, link_training_enable, ext_tcam_connect_mode, to_clean));
            break;
        case 2:
            ...
            SHR_IF_ERR_EXIT(dnx_kbp_stif_ing_egr_one_port_test(unit, to_clean));
            break;
        case 3:
            ...
            SHR_IF_ERR_EXIT(dnx_kbp_stif_ing_egr_max_dbs_one_port_test(unit, to_clean));
            break;
        default:
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SH_SAND_GET_UINT32("lanes", nof_lanes);
    SH_SAND_GET_UINT32("nof_ports", nof_ports);
    SH_SAND_GET_STR("ext_tcam_connect_mode", ext_tcam_connect_mode);
    SH_SAND_GET_BOOL("clean", to_clean);
     ...
     * parameters check
     */
    if (speed != -1 && speed != 100000 && speed != 200000)
    {
        SHR_CLI_EXIT(_SHR_E_FAIL, "The speed %d is invalid for stif port\n", speed);
    }
    if (nof_lanes != 2 && nof_lanes != 4)
    {
        SHR_CLI_EXIT(_SHR_E_FAIL, "The number of lanes %d is invalid for stif ports\n", nof_lanes);
    }
    if (nof_ports > dnx_data_stif.port.max_nof_instances_get(unit))
    {
        SHR_CLI_EXIT(_SHR_E_FAIL, "The number of ports %d is invalid for this test\n", nof_ports);
    }

    LOG_CLI((BSL_META("Start test with speed %d, nof_ports %d, nof_lanes %d, ext_tcam_connect_mode %s \n"),
             speed, nof_ports, nof_lanes, ext_tcam_connect_mode));
    SHR_IF_ERR_EXIT(dnx_kbp_stif_ing_egr_port_test(unit, speed, nof_ports, nof_lanes, ext_tcam_connect_mode, to_clean));
    LOG_CLI((BSL_META("TEST PASS!!! \n")));
...
static sh_sand_option_t dnx_kbp_stif_test_list_options[] = {
    ...
    {"ext_tcam_connect_mode", SAL_FIELD_TYPE_STR, "ext tcam connect mode soc", ""},
    {NULL}
};

static sh_sand_option_t dnx_kbp_stif_port_test_list_options[] = {
    {"clean", SAL_FIELD_TYPE_BOOL, "Clean after test", "Yes"},
    {"speed", SAL_FIELD_TYPE_INT32, "speed of stif port, like 200000 or 100000, -1 for not speicify", "-1"},
    {"nof_ports", SAL_FIELD_TYPE_UINT32, "number of stif ports, not over max number of stif port instances", "2"},
    {"lanes", SAL_FIELD_TYPE_UINT32, "number of lanes per stif port, like 2 or 4", "4"},
    {"ext_tcam_connect_mode", SAL_FIELD_TYPE_STR, "ext tcam connect mode soc", ""},
    {NULL}
};
...
static sh_sand_man_t dnx_kbp_stif_port_test_man = {
    .brief = "test kbp and dnx devices with different nof ports and lanes using statistic interface",
    .full = NULL,
    .synopsis = NULL,
    .examples = "speed=100000 nof_ports=2 lanes=4 ext_tcam_connect_mode=SINGLE_PORT_SMT\n" "nof_ports=1 lanes=4"
};
...
sh_sand_cmd_t sh_dnx_kbp_stif_test_cmds[] = {
    ...
    {"port", dnx_kbp_stif_port_test, NULL, dnx_kbp_stif_port_test_list_options, &dnx_kbp_stif_port_test_man, NULL, NULL,
     SH_CMD_SKIP_EXEC,.label = "kbp"},
