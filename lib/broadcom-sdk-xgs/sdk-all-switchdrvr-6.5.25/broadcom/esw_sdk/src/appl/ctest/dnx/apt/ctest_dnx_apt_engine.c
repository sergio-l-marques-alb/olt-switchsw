...
static dnx_apt_soc_properties_def_t g_soc_properties;
...
/*
 * API indicating whether API performance test validation is supported for the compilation
 * Returns TRUE if Intel-GTS-OPT.
 * Returns FALSE otherwise.
 */
static uint8
dnx_apt_is_validation_supported_by_compilation(
    void)
{
...
/*
 * Enable threads and error recovery
 */
static void
dnx_apt_engine_threads_and_recovery_enable(
    int unit)
{
...
/*
 * Disable threads and error recovery
 */
static void
dnx_apt_engine_threads_and_recovery_disable(
    int unit)
{
    /** Disable any prints in the following block to reduce unnecessary output */
    bsl_severity_t severity;
 ...
 *      Initialize required data and SOC property structures
 */
static shr_error_e
dnx_apt_engine_init(
    int unit,
    ...
    const dnx_apt_object_def_t * test_object)
{
    ...
    sal_memset(&g_soc_properties, 0x0, sizeof(dnx_apt_soc_properties_def_t));
    ...
    const dnx_apt_object_def_t * test_object)
{
    ...
    /** Perform validation only on the supported compilation (Intel-GTS_OPT). */
    if (dnx_apt_is_validation_supported_by_compilation()
        && (test_object->validation_method != DNX_APT_VALIDATION_METHOD_SKIP))
    {
        /** If time threshold is 0 the validation will pass */
        fail_compare_sign = test_object->time_threshold(unit) ?
            (validation_time <= test_object->time_threshold(unit) ? FALSE : TRUE) : FALSE;
    ...
    else
    {
        ...
        /** Print results */
        LOG_CLI((BSL_META
                 ("Test \"%s\" results:\n\tAverage execution time %.3fus / rate %d calls per second (Validation skipped).\n"),
                 test_object->test_name, validation_time, DNX_APT_TIME_TO_RATE(validation_time)));
    }

    /** Print rate (entries per second) to the regression DB */
    dnxc_regression_utils_advanced_print("APT", "E %d", DNX_APT_TIME_TO_RATE(validation_time));
    ...
    const dnx_apt_object_def_t * test_object)
{
    ...
    int are_soc_properties_set = FALSE;
    int is_test_init_executed = FALSE;
    int are_threads_and_error_recovery_disabled = FALSE;
    ...
    /** SOC properties initialization */
    if (test_object->soc_properties_set)
    {
        SHR_IF_ERR_EXIT(ctest_dnxc_set_soc_properties(unit, test_object->soc_properties_set, &ctest_soc_set_h));
    }
    are_soc_properties_set = TRUE;
    ...
    /** Disable threads and error recovery */
    dnx_apt_engine_threads_and_recovery_disable(unit);
    are_threads_and_error_recovery_disabled = TRUE;
    ...
    for (run = 0; run < test_object->number_of_runs; run++)
    {
        ...
        if (test_object->ddma_table != SBUSDMA_DESC_MODULE_ENABLE_NOF_MODULES
            && dnx_sbusdma_desc_is_enabled(unit, test_object->ddma_table) == TRUE)
        {
            dnx_sbusdma_desc_wait_done(unit);
        }
...
exit:
    /** Enable threads and error recovery */
    if (are_threads_and_error_recovery_disabled)
    {
        dnx_apt_engine_threads_and_recovery_enable(unit);
    }
    ...
    if (test_object->soc_properties_set && are_soc_properties_set)
    {
        LOG_CLI((BSL_META("Restore SOC properties\n")));
