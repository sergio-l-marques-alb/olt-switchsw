...
/**
 * \brief
 *   This function checks configuration of enable/disable MTU check on context.
 *   Currently only done for context ETPP_FORWARDING_WO_ETPS_POP.
 *
 * \param [in] unit             - The unit number.
 *
 * \return
 ...
 */
static shr_error_e
ctest_dnx_rx_trap_mtu_ctx_enable_pos(
    int unit)
{
    bcm_switch_control_key_t key;
    bcm_switch_control_info_t info, info_get;

    SHR_FUNC_INIT_VARS(unit);

    key.type = bcmSwitchForwardingLayerMtuFilter;
    key.index = 0;

    /** Enabler should be TRUE on init */
    info.value = TRUE;

    SHR_IF_ERR_EXIT(bcm_switch_control_indexed_get(unit, key, &info_get));

    SHR_IF_ERR_EXIT(ctest_dnx_rx_trap_compare_values(unit, &info.value, &info_get.value,
                                                     sizeof(info.value), "enabler_value"));

    /** Disable MTU on context */
    info.value = FALSE;
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (sal_strncasecmp
        (test_type, CTEST_DNX_RX_TRAPS_MTU_OPTION_TEST_MODE_PORT, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
    {
        ...
        SHR_IF_ERR_EXIT(ctest_dnx_rx_trap_mtu_get_compare(unit, &mtu_value, &mtu_key, &mtu_config));
             ...
             (test_type, CTEST_DNX_RX_TRAPS_MTU_OPTION_TEST_MODE_RIF,
              SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
    {
        ...
        SHR_IF_ERR_EXIT(ctest_dnx_rx_trap_mtu_get_compare(unit, &mtu_value, &mtu_key, &mtu_config));
             ...
             (test_type, CTEST_DNX_RX_TRAPS_MTU_OPTION_TEST_MODE_LIF,
              SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
    {
        LOG_INFO(BSL_LS_APPL_COMMON,
                 (BSL_META_U(unit, "--------------------------------------------------------------- \n")));
    ...
    /** Check enable/disable MTU check on context */
    SHR_IF_ERR_EXIT(ctest_dnx_rx_trap_mtu_ctx_enable_pos(unit));
...
/**
 * \brief
 *   This function checks errors of enable/disable MTU check on context.
 *   Currently only done for context ETPP_FORWARDING_WO_ETPS_POP.
 *
 * \param [in] unit             - The unit number.
 *
 ...
 */
static shr_error_e
ctest_dnx_rx_trap_mtu_ctx_enable_neg(
    int unit)
{
    int rv = BCM_E_NONE;
    bcm_switch_control_key_t key;
    ...
    LOG_INFO(BSL_LS_APPL_COMMON,
             (BSL_META_U
              (unit, "bcmSwitchForwardingLayerMtuFilter: SUPPLYING invalid key.index to bcm_switch_control_key_t \n")));
    LOG_INFO(BSL_LS_APPL_COMMON,
             (BSL_META_U
              (unit, "-------------------------------------------------------------------------------------------\n")));
    ...
    LOG_INFO(BSL_LS_APPL_COMMON,
             (BSL_META_U
              (unit,
               "bcmSwitchForwardingLayerMtuFilter: SUPPLYING invalid info.value to bcm_switch_control_info_t\n")));
    LOG_INFO(BSL_LS_APPL_COMMON,
             (BSL_META_U
              (unit,
               "--------------------------------------------------------------------------------------------\n")));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (dnx_data_trap.etpp.feature_get(unit, dnx_data_trap_etpp_mtu_layer_to_compressed_layer_mapping_support))
    {
        ...
        LOG_INFO(BSL_LS_APPL_COMMON,
                 (BSL_META_U
                  (unit, "--------------- bcm_switch_control_indexed_set/get TESTING STARTS HERE ----------\n")));
        LOG_INFO(BSL_LS_APPL_COMMON,
                 (BSL_META_U
                  (unit, "---------------------------------------------------------------------------------\n")));
        ...
        LOG_INFO(BSL_LS_APPL_COMMON,
                 (BSL_META_U
                  (unit,
                   "bcmSwitchLinkLayerMtuFilter: SUPPLYING bcmFieldLayerTypeCount to bcm_switch_control_key_t \n")));
        LOG_INFO(BSL_LS_APPL_COMMON,
                 (BSL_META_U
                  (unit,
                   "------------------------------------------------------------------------------------------\n")));
        ...
        if (rv != BCM_E_PARAM)
        {
            LOG_ERROR(BSL_LS_APPL_COMMON,
                      (BSL_META_U
                       (unit, "bcm_switch_control_indexed_set should fail with BCM_E_PARAM! . Error:%d (%s)\n"), rv,
                       bcm_errmsg(rv)));
            return rv;
        }
        ...
        LOG_INFO(BSL_LS_APPL_COMMON,
                 (BSL_META_U
                  (unit, "bcmSwitchLinkLayerMtuFilter: SUPPLYING value > 3b to bcm_switch_control_info_t \n")));
        LOG_INFO(BSL_LS_APPL_COMMON,
                 (BSL_META_U
                  (unit, "-------------------------------------------------------------------------------\n")));
        ...
        if (rv != BCM_E_PARAM)
        {
            LOG_ERROR(BSL_LS_APPL_COMMON,
                      (BSL_META_U
                       (unit, "bcm_switch_control_indexed_get should fail with BCM_E_PARAM! . Error:%d (%s)\n"), rv,
                       bcm_errmsg(rv)));
            return rv;
        }

        SHR_IF_ERR_EXIT(ctest_dnx_rx_trap_mtu_ctx_enable_neg(unit));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (sal_strncasecmp
        (test_type, CTEST_DNX_RX_TRAPS_MTU_TEST_TYPE_POSITIVE, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
    {
        ...
        SHR_EXIT();
             ...
             (test_type, CTEST_DNX_RX_TRAPS_MTU_TEST_TYPE_NEGATIVE, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
    {
        ...
        SHR_EXIT();
             ...
             (test_type, CTEST_DNX_RX_TRAPS_MTU_TEST_TYPE_EXHAUSTIVE,
              SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
    {
