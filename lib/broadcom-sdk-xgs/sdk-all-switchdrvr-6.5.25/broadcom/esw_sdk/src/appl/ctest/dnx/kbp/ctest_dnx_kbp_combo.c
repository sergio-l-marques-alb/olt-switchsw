    ...
    dnx_kbp_combo_group_info_t * group_info)
{
    ...
    DNX_KBP_COMBO_API_EXECUTE_NO_ERR_EXIT(dbal_tables_table_create
                                          (unit, DBAL_ACCESS_METHOD_KBP, group_info->dbal_table_type,
                                           ...
                                           res_field_info, 0, group_info->name, &(group_info->dbal_table_id)),
                                          group_info, "failed DBAL table creation",
    ...
    dnx_kbp_combo_opcode_info_t * opcode_info)
{
    ...
    DNX_KBP_COMBO_API_EXECUTE_NO_ERR_EXIT(dbal_enum_string_to_id
                                          (unit, DBAL_FIELD_KBP_FORWARD_APP_TYPES, tmp_string, &source_opcode),
                                          opcode_info, "failed XML read: invalid source opcode",
                                          DNX_KBP_COMBO_OPCODE_STATUS_FAILED_XML_READ);
    ...
    uint8 configure)
{
    ...
    if (sal_strncmp(output, "stdout", sal_strnlen(output, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH)))
    {
        /** Output to file */
