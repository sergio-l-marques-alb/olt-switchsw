    ...
    sh_sand_control_t * sand_control)
{
    ...
    BCM_GPORT_SYSTEM_PORT_ID_SET(gport, out_port);
    SHR_IF_ERR_EXIT(bcm_stk_sysport_gport_get(unit, gport, &modport_gport));
    ...
    SHR_CLI_EXIT_IF_ERR(sand_signal_verify
                        (unit, mapping_info_in_port.core, "IRPP", dnx_data_pp.debug.from_signal_str_get(unit)->in_port,
                         "", "In_Port", &mapping_info_in_port.pp_port, 1, NULL, return_value, 0),
                        "Test failed , In_Port \n");
    SHR_CLI_EXIT_IF_ERR(sand_signal_verify
    ...
    if (system_headers_mode != DBAL_ENUM_FVAL_SYSTEM_HEADERS_MODE_JERICHO_MODE)
    {
        ...
        SHR_CLI_EXIT_IF_ERR(sand_signal_verify
                            (unit, mapping_info_out_port.core, "ERPP", "Fabric", "", "FTMH_PP_DSP", &ftmh_pp_dsp, 1,
