...
#include "../../../diag/dnx/dbal/diag_dnx_dbal.h"
#include <soc/dnx/dnx_err_recovery_manager.h>
#include <appl/ctest/dnxc/ctest_dnxc_utils.h>
...
test_dnx_dbal_partial_access(
    ...
    uint8 is_rand)
{
    uint32 key_field_val_arr[DBAL_TABLE_MAX_NUM_OF_KEY_FIELDS][DBAL_FIELD_ARRAY_MAX_SIZE_IN_WORDS];
    ...
    {
        if (SHR_BITGET(table->keys_info[field_index].field_indication_bm, DBAL_FIELD_IND_IS_FIELD_ENUM))
        {
            int enum_val = INVALID_ENUM;
            if (table->keys_info[field_index].min_value > 0)
            {
                enum_val = table->keys_info[field_index].min_value - 1;
            }
            /** getting the first valid enum value */
            SHR_IF_ERR_EXIT(dbal_fields_enum_next_enum_value_get(unit, table->keys_info[field_index].field_id,
                                                                 enum_val, key_field_val_arr[field_index]));
        }
        else
        {
            key_field_val_arr[field_index][0] = table->keys_info[field_index].min_value;
        }

        if (dbal_table_is_in_lif(table) && (table->keys_info[field_index].field_id == DBAL_FIELD_IN_LIF))
        {
            key_field_val_arr[field_index][0] = DIAG_DBAL_MIN_IN_LIF_TO_ALLOCATE(unit);
        }
            ...
            (!dbal_table_is_allcator_not_needed(unit, table_id)))
        {
            ...
            SHR_IF_ERR_EXIT(diag_dbal_alloc_dealloc_value(unit, table->keys_info[field_index].field_id,
                                                          table_id, 0, core_val, key_field_val_arr[field_index][0], 1));
        }
    ...
    for (field_index = 0; field_index < table->multi_res_info[0].nof_result_fields; field_index++)
    {
        ...
        dbal_entry_value_field_arr32_set(unit, entry_handle_wr,
                                         table->multi_res_info[0].results_info[field_index].field_id,
                                         table->multi_res_info[0].results_info[field_index].inst_idx,
                                         field_val[field_index]);
    }
    ...
    if (nof_result_fields_to_use < 2)
    {
        ...
        for (field_index = 0; field_index < table->nof_key_fields; field_index++)
        {
                ...
                (!dbal_table_is_allcator_not_needed(unit, table_id)))
            {
                int core_val = table->core_mode == DBAL_CORE_MODE_DPC ? 0 : _SHR_CORE_ALL;
                SHR_IF_ERR_EXIT(diag_dbal_alloc_dealloc_value(unit, table->keys_info[field_index].field_id,
                                                              table_id, 0, core_val, key_field_val_arr[field_index][0],
                                                              0));
            }
        }
        SHR_EXIT();
    ...
    do
    {
        ...
        else
        {
             ...
             */
            uint32 max_value;
            if ((table->multi_res_info[0].results_info[field_index].field_id == DBAL_FIELD_KBP_FORWARD_PMF_GENERIC_DATA)
            ...
            {
                max_value = table->multi_res_info[0].results_info[field_index].max_value;
            }
            ...
            SHR_IF_ERR_EXIT(dbal_fields_get_value_for_field(unit,
                                                            ...
                                                            results_info[field_index].min_value, max_value,
                                                            table->multi_res_info[0].
    ...
    for (field_index = 0; field_index < table->multi_res_info[0].nof_result_fields; field_index++)
    {
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field_arr32_get(unit, entry_handle_rd,
                                                                ...
                                                                table->multi_res_info[0].
                                                                results_info[field_index].inst_idx,
    ...
    for (field_index = 0; field_index < table->nof_key_fields; field_index++)
    {
            ...
            (!dbal_table_is_allcator_not_needed(unit, table_id)))
        {
            int core_val = table->core_mode == DBAL_CORE_MODE_DPC ? 0 : _SHR_CORE_ALL;
            SHR_IF_ERR_EXIT(diag_dbal_alloc_dealloc_value(unit, table->keys_info[field_index].field_id,
                                                          table_id, 0, core_val, key_field_val_arr[field_index][0], 0));
        }
    }
    ...
    for (field_index = 0; field_index < table->nof_key_fields; field_index++)
    {
        for (update_index = 0; update_index < MAX_NUM_UPDATE_BEFORE_COMMIT; update_index++)
        {
            SHR_IF_ERR_EXIT(dbal_fields_get_value_for_field(unit, table->keys_info[field_index].field_id,
                                                            ...
                                                            TRUE, 0, 0, key_field_val_arr[field_index]));
    ...
    dbal_tables_e table_id)
{
    ...
    /*
     * Clear the handle before validation
     */
 ...
 * \brief:
 * Clear the handle
 * Then set key and result fields for one entry.
 */
static shr_error_e
dnx_dbal_ut_prepare_entry(
    int unit,
    dbal_tables_e table_id,
    CONST dbal_logical_table_t * table,
    ...
    int res_type)
{
    ...
    for (field_index = 0; field_index < table->nof_key_fields; field_index++)
    {
        if (table->keys_info[field_index].field_type == DBAL_FIELD_TYPE_DEF_L2_MAC)
        {
            bcm_mac_t mac = { 0x11, 0x22, 0x33, 0x44, 0x55, 0x66 };
            dbal_entry_key_field_arr8_set(unit, entry_handle_id, table->keys_info[field_index].field_id, mac);
        }
        else
        {
            dbal_entry_key_field32_set(unit, entry_handle_id, table->keys_info[field_index].field_id, 1);
        }
    }
    ...
    for (field_index = 0; field_index < table->multi_res_info[res_type].nof_result_fields; field_index++)
    {
        if (table->multi_res_info[res_type].results_info[field_index].field_id == DBAL_FIELD_RESULT_TYPE)
        {
            continue;
        }
        dbal_entry_value_field32_set(unit, entry_handle_id,
                                     table->multi_res_info[res_type].results_info[field_index].field_id, INST_SINGLE,
                                     1);
    }
 ...
 */
static shr_error_e
dnx_dbal_ut_prepare_updated_entry(
    int unit,
    CONST dbal_logical_table_t * table,
    uint32 entry_handle_id,
    int res_type)
{
    ...
    {
        /*
         * Update only regular values, not boolean or result type
         */
        if (table->multi_res_info[res_type].results_info[field_index].field_id == DBAL_FIELD_RESULT_TYPE ||
            table->multi_res_info[res_type].results_info[field_index].field_type == DBAL_FIELD_TYPE_DEF_BOOL)
        {
            continue;
        }
        dbal_entry_value_field32_set(unit, entry_handle_id,
...
/**
 * \brief
 * Enters an entry to properly configure VSI_LEARNING_PROFILE_INFO table,
 * because in case ltt runs for this table runs, it can make its values illegal
 * and cause FWD_MACT not to function.
 */
static shr_error_e
dnx_dbal_configure_vsi_learning_table(
    int unit)
{
    uint32 entry_handle_id = DBAL_SW_NOF_ENTRY_HANDLES;
    SHR_FUNC_INIT_VARS(unit);
    ...
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_VSI_LEARNING_PROFILE, 0);

    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LEARN_LIMIT, INST_SINGLE, 2097151);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_MESSAGE_ENABLE, INST_SINGLE, 1);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_INTERRUPT_ENABLE, INST_SINGLE, 1);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_EVENT_FORWARDING_PROFILE, INST_SINGLE, 0);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_AGE_PROFILE, INST_SINGLE, 0);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_CYCLE_PERIOD, INST_SINGLE, 0);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_CYCLE_MASK, INST_SINGLE, 0);
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
...
/**
 * \brief
 * Enters an entry to properly configure DBAL_TABLE_MACT_APP_DB_CONFIGURATION table,
 * because in case ltt runs for this table runs, it can make its values illegal
 * and cause FWD_MACT not to function.
 */
static shr_error_e
dnx_dbal_configure_mact_app_db(
    int unit)
{
    uint32 entry_handle_id = DBAL_SW_NOF_ENTRY_HANDLES;
    ...
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_LEM_APP_DB, 2);
    ...
    SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_MACT_APP_DB_CONFIGURATION, entry_handle_id));
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_LEM_APP_DB, 3);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_IS_LARGE_EM, INST_SINGLE, 1);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_IS_MACT, INST_SINGLE, 1);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_MACT_APP_DB_INDEX, INST_SINGLE,
    ...
    {
        SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_MACT_APP_DB_CONFIGURATION, entry_handle_id));
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_LEM_APP_DB, 16);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_IS_LARGE_EM, INST_SINGLE, 1);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_IS_MACT, INST_SINGLE, 1);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_MACT_APP_DB_INDEX, INST_SINGLE,
...
/**
 * \brief
 * This test commits entries with DBAL_COMMIT_HW_ONLY flag.
 * It checks that in that case, entry was not written to SW
 */
shr_error_e
dnx_dbal_ut_commit_hw_only(
    int unit,
    dbal_tables_e table_id)
{
    ...
    if (table_id != DBAL_TABLE_L2_FORWARD_UNKNOWN_DA_PROFILE && table_id != DBAL_TABLE_IPV4_UNICAST_PRIVATE_HOST
        && table_id != DBAL_TABLE_FWD_MACT && table_id != DBAL_TABLE_IN_AC_INFO_DB)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "This test can not run with %s table\n",
                     dbal_logical_table_to_string(unit, table_id));
    }
    ...
    {
        SHR_IF_ERR_EXIT(dnx_dbal_configure_vsi_learning_table(unit));
    }
    else if (table_id == DBAL_TABLE_IPV4_UNICAST_PRIVATE_HOST)
    {
        /*
         * Check hw_only feature for table dynamically enabled for learning
         */
        dbal_tables_indication_set(unit, DBAL_TABLE_IPV4_UNICAST_PRIVATE_HOST, DBAL_TABLE_IND_IS_LEARNING_EN, TRUE);
    }
    ...
    /** Set entry */
    SHR_IF_ERR_EXIT(dnx_dbal_ut_prepare_entry(unit, table_id, table, entry_handle_id, res_type));

    /*
     * Negative Test, check commit hw_only returns internal error in case of non learning table
     */
    if (!SHR_IS_BITSET(table->indications_bm, DBAL_TABLE_IND_IS_LEARNING_EN))
    {
        diag_dbal_pre_action_logger_close(unit);
        rv = dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT | DBAL_COMMIT_HW_ONLY);
        diag_dbal_post_action_logger_restore(unit);
        SHR_IF_NOT_EXCEPT_ERR_EXIT(rv, _SHR_E_INTERNAL);
        SHR_SET_CURRENT_ERR(_SHR_E_NONE);
        SHR_EXIT();
    }

    /*
     * Scenario 1: 1. Commit HW only 2. Get HW only. Expected: OK 3. Commit HW only again, expected: error
     * 4. Do regular get, Expected: OK (MDB EM tables searches SW then HW) 5. Clear HW only 6. Get HW only, expected: not found
     */
     ...
     * 1
     */
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT | DBAL_COMMIT_HW_ONLY));
    ...
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT | DBAL_COMMIT_HW_ONLY));
    ...
    for (field_index = 0; field_index < table->multi_res_info[res_type].nof_result_fields; field_index++)
    {
        ...
        if (get_field_val[field_index][0] != 1)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "table %s wrong value, expected 1, received %d\n",
                         dbal_logical_table_to_string(unit, table_id), get_field_val[field_index][0]);
        }
    }
    /*
     * 3
     */
    diag_dbal_pre_action_logger_close(unit);
    SHR_IF_NOT_EXCEPT_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT | DBAL_COMMIT_HW_ONLY),
                               _SHR_E_EXISTS);
    diag_dbal_post_action_logger_restore(unit);

    /*
     * 4
     * For regular get (APPLY ALL), when the table supports learning, dbal_entry_get
     * does first a SW get, and if not found, it does a HW get.
     */
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));
     ...
     * 5
     */
    SHR_IF_ERR_EXIT(dbal_entry_clear(unit, entry_handle_id, DBAL_COMMIT | DBAL_COMMIT_HW_ONLY));

    /*
     * 6
     */
    diag_dbal_pre_action_logger_close(unit);
    SHR_IF_NOT_EXCEPT_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT | DBAL_COMMIT_HW_ONLY),
                               _SHR_E_NOT_FOUND);
    diag_dbal_post_action_logger_restore(unit);

    /*
     * Scenario 2 Update: 1. Update HW only (before commit), expected not found 2. commit HW only 3. commit update HW only
     * 4. get HW only 5. Do regular get, expected: found in learning table, not found in not learning table
     * 6. Clear regular, expected OK 7. get HW only, expected: not found
     */
     ...
     */
    diag_dbal_pre_action_logger_close(unit);
    SHR_IF_NOT_EXCEPT_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT_UPDATE | DBAL_COMMIT_HW_ONLY),
                               _SHR_E_NOT_FOUND);
    diag_dbal_post_action_logger_restore(unit);
    ...
    SHR_IF_ERR_EXIT(dnx_dbal_ut_prepare_updated_entry(unit, table, entry_handle_id, res_type));

    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT_UPDATE | DBAL_COMMIT_HW_ONLY));
    ...
    for (field_index = 0; field_index < table->multi_res_info[res_type].nof_result_fields; field_index++)
    {
        ...
        else if (table->multi_res_info[res_type].results_info[field_index].field_type == DBAL_FIELD_TYPE_DEF_BOOL)
        {
            expected_value = 1;
        }
        else
        ...
        if (get_field_val[field_index][0] != expected_value)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "table %s wrong value, expected %d, received %d\n",
                         dbal_logical_table_to_string(unit, table_id), expected_value, get_field_val[field_index][0]);
        }
    }
    ...
    /*
     * Scenario 3: 1. Commit regular 2. Clear HW only, expected: exists (in SW) 3. Get HW only. Expected: OK
     * 4. Commit update HW only, expected: OK. 5. Get and check updated values
     */
    ...
    /*
     * 2
     * Regular commit writes the entry to HW and SW, then HW_ONLY clear should fail and returns EXIST
     */
    diag_dbal_pre_action_logger_close(unit);
    SHR_IF_NOT_EXCEPT_ERR_EXIT(dbal_entry_clear(unit, entry_handle_id, DBAL_COMMIT | DBAL_COMMIT_HW_ONLY),
                               _SHR_E_EXISTS);
    diag_dbal_post_action_logger_restore(unit);
    ...
    if (table_id == DBAL_TABLE_IPV4_UNICAST_PRIVATE_HOST)
    {
        dbal_tables_indication_set(unit, DBAL_TABLE_IPV4_UNICAST_PRIVATE_HOST, DBAL_TABLE_IND_IS_LEARNING_EN, FALSE);
    }
    ...
    dbal_tables_e table_id)
{
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_SOME_FIELD_1, INST_SINGLE, peer_gport_val);
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    ...
    SHR_IF_ERR_EXIT(dbal_entry_handle_value_field_arr32_get(unit, entry_handle_id, DBAL_FIELD_SOME_FIELD_1, INST_SINGLE,
                                                            field_val_int));
    if (field_val_int[0] != peer_gport_val)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Comparison error: field_id[%s], get_field_val[%d] != expected_val[%d],\n",
                     dbal_field_to_string(unit, DBAL_FIELD_SOME_FIELD_1), field_val_int[0], peer_gport_val);
    }

     /** negative test, checking field that is not exists */
    diag_dbal_pre_action_logger_close(unit);

    rv = dbal_entry_handle_value_field_arr32_get(unit, entry_handle_id, DBAL_FIELD_ESEM_COMMAND, INST_SINGLE,
                                                 field_val_int);
    diag_dbal_post_action_logger_restore(unit);
    if (rv == 0)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "negative test passed. field %s should not be found in entry,\n",
                     dbal_field_to_string(unit, DBAL_FIELD_ESEM_COMMAND));
    }
    ...
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_SOME_FIELD_1, INST_SINGLE, &ret_peer_gport_val);
    ...
    if (ret_peer_gport_val != peer_gport_val)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Known res type, Comparison error: field %s, ret_val %d != expected_val %d,\n",
                     dbal_field_to_string(unit, DBAL_FIELD_SOME_FIELD_1), ret_peer_gport_val, peer_gport_val);
    }
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ESEM_COMMAND, INST_SINGLE, esem_access_val);
    ...
    SHR_IF_ERR_EXIT(dbal_entry_handle_value_field_arr32_get
                    (unit, entry_handle_id, DBAL_FIELD_ESEM_COMMAND, INST_SINGLE, field_val_int));
    if (field_val_int[0] != esem_access_val)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Comparison error: field_id[%s], get_field_val[%d] != expected_val[%d],\n",
                     dbal_field_to_string(unit, DBAL_FIELD_ESEM_COMMAND), field_val_int[0], esem_access_val);
    }
    ...
    /** negative test, checking field that is not exists  */
    diag_dbal_pre_action_logger_close(unit);
    rv = dbal_entry_handle_value_field_arr32_get(unit, entry_handle_id, DBAL_FIELD_SOME_FIELD_1, INST_SINGLE,
                                                 field_val_int);
    diag_dbal_post_action_logger_restore(unit);
    if (rv == 0)
    {
    ...
    uint8 is_rand)
{
    ...
    for (field_index = 0; field_index < table->nof_key_fields; field_index++)
    {
        DIAG_DBAL_RANDOM_FIELD_VAL(key_field_val_arr[field_index], table->keys_info[field_index].field_nof_bits, TRUE);
        dbal_entry_key_field_arr32_set(unit, entry_handle_wr, table->keys_info[field_index].field_id,
                                       key_field_val_arr[field_index]);
    }
    ...
    dbal_tables_e table_id)
{
    ...
    if (!is_end)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "clear range did not clear all entries 2\n");
    }
 ...
 * INST_ALL flag. then check all instances values
 */
shr_error_e
    ...
    dbal_tables_e table_id)
{
    ...
    uint32 core_id_val = DBAL_MAX_NUM_OF_CORES - 1;
    ...
    for (ii = 0; ii < nof_counter_inst; ii++)
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "wrong value for counter %d, single inst set step\n", ii);
        }
    }

    /** set all counters in an entry with mutual value  */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, table_id, entry_handle_id));
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_PP_PORT, pp_port_val);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, core_id_val);
...
/*
 * \brief
 * The test checks functionality of a partially mapped table.
 * Such table has some field which under specific condition, is not
 * mapped to physical location, and trying to commit entry there is
 * legal.
 */
shr_error_e
dnx_dbal_ut_asymmetric_table(
    int unit,
    dbal_tables_e table_id)
{
    uint32 entry_handle_id;
    uint32 res_type, res_field;
    int nof_entries = 0;
    int is_end;
     ...
     * Verify table ID, This test is a special test that can run only
     */
    if (table_id != DBAL_TABLE_EXAMPLE_TABLE_WITH_ASYMMETRIC_MAPPING)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "This test can not run with %s table\n",
                     dbal_logical_table_to_string(unit, table_id));
    }
     ...
     */
    /*
     * 1.1 Add entry to illegal location (RT0), expect getting _SHR_E_ACCESS 
     */
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, 1);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_ETHU_ID, 4);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_RMC, 2);

    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_RESULT_TYPE, INST_ALL, 0);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_SOME_RESULT_FIELD_0, INST_ALL, 456);

    diag_dbal_pre_action_logger_close(unit);
    SHR_IF_NOT_EXCEPT_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT), _SHR_E_ACCESS);
    diag_dbal_post_action_logger_restore(unit);

    /*
     * 1.2 Add entry to illegal location (RT1), expect getting _SHR_E_ACCESS 
     */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, table_id, entry_handle_id));
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, 0);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_ETHU_ID, 4);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_RMC, 3);

    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_RESULT_TYPE, INST_ALL, 1);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_SOME_RESULT_FIELD_1, INST_ALL, 460);
     ...
     */
    /*
     * 2.1.1 Add entry to legal location (RT0) 
     */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, table_id, entry_handle_id));
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, 0);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_ETHU_ID, 4);
    ...
    /*
     * 2.1.2 Get the entry and verify 
     */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, table_id, entry_handle_id));
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, 0);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_ETHU_ID, 4);
    ...
    SHR_IF_ERR_EXIT(dbal_entry_handle_value_field_arr32_get(unit, entry_handle_id, DBAL_FIELD_RESULT_TYPE, INST_SINGLE,
                                                            &res_type));
    SHR_IF_ERR_EXIT(dbal_entry_handle_value_field_arr32_get
                    (unit, entry_handle_id, DBAL_FIELD_SOME_RESULT_FIELD_0, INST_SINGLE, &res_field));
    if ((res_type != 0) || (res_field != 456))
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Received value is not as expected");
    }

    /*
     * 2.2.1 Add entry to legal location (RT)
     */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, table_id, entry_handle_id));
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, 1);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_ETHU_ID, 3);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_RMC, 3);
    ...
    /*
     * 2.2.2 Get the entry and verify 
     */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, table_id, entry_handle_id));
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, 1);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_ETHU_ID, 3);
    ...
    SHR_IF_ERR_EXIT(dbal_entry_handle_value_field_arr32_get(unit, entry_handle_id, DBAL_FIELD_RESULT_TYPE, INST_SINGLE,
                                                            &res_type));
    SHR_IF_ERR_EXIT(dbal_entry_handle_value_field_arr32_get
                    (unit, entry_handle_id, DBAL_FIELD_SOME_RESULT_FIELD_1, INST_SINGLE, &res_field));

    if ((res_type != 1) || (res_field != 460))
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Received value is not as expected");
    }
    ...
    if (nof_entries != 2)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Expected 2 entries, found %d", nof_entries);
    }

    /*
     * 4. Clear table by iterator 
     */
    SHR_IF_ERR_EXIT(dbal_iterator_init(unit, entry_handle_id, DBAL_ITER_MODE_GET_ALL_EXCEPT_DEFAULT));
    SHR_IF_ERR_EXIT(dbal_iterator_get_next(unit, entry_handle_id, &is_end));
    while (!is_end)
    ...
    if (nof_entries > 0)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Expected no entry, found %d", nof_entries);
    }

    /*
     * 4. Get from illegal location Here we simulate case of a table with one result type. That is why we set the
     * result type. In such case, we expect to get access error. 
     */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, table_id, entry_handle_id));
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, 0);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_ETHU_ID, 4);
    ...
    diag_dbal_pre_action_logger_close(unit);
    SHR_IF_NOT_EXCEPT_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_GET_ALL_FIELDS), _SHR_E_ACCESS);
    diag_dbal_post_action_logger_restore(unit);

    diag_dbal_pre_action_logger_close(unit);
    SHR_IF_NOT_EXCEPT_ERR_EXIT(dbal_entry_clear(unit, entry_handle_id, DBAL_COMMIT), _SHR_E_ACCESS);
    diag_dbal_post_action_logger_restore(unit);
    ...
    dbal_tables_e table_id)
{
    ...
    for (ii = 0; ii < table->nof_key_fields; ii++)
    {
        ...
        {
            field_value = UTILEX_MIN(table->keys_info[ii].max_value, field_value);
        }
...
{
    uint32 entry_handle_id;
    uint32 result_fields_val[2][2];
    CONST dbal_logical_table_t *table;
    dbal_fields_e key_field_id = DBAL_FIELD_EMPTY; /** the key field ID other than core_id to be set */
    dbal_fields_e value_field_id_0 = DBAL_FIELD_EMPTY, value_field_id_1 = DBAL_FIELD_EMPTY; /** the value field IDs to validate */
    ...
    if (table_id == DBAL_TABLE_INGRESS_PTC_PORT)
    {
        ...
        value_field_id_0 = DBAL_FIELD_PRT_QUALIFIER_RAW;
        value_field_id_1 = DBAL_FIELD_IRPP_1ST_PARSER_PARSER_CONTEXT;
        key_field_id = DBAL_FIELD_PTC;
    }
    else if (table_id == DBAL_TABLE_INGRESS_PP_RECYCLE_COMMAND)
    {
        /** Both are mapped to IPPE_PRT_RECYCLE_INFOm */
        value_field_id_0 = DBAL_FIELD_IN_PP_PORT;
        value_field_id_1 = DBAL_FIELD_SOURCE_SYSTEM_PORT;
        {
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, value_field_id_0, INST_SINGLE, 0);
    dbal_entry_value_field32_set(unit, entry_handle_id, value_field_id_1, INST_SINGLE, 0);

    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));

    /** adding entry to core DBAL_MAX_NUM_OF_CORES both fields 1 */

    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, DBAL_MAX_NUM_OF_CORES - 1);
    dbal_entry_key_field32_set(unit, entry_handle_id, key_field_id, 0);

    dbal_entry_value_field32_set(unit, entry_handle_id, value_field_id_0, INST_SINGLE, 1);
    ...
    /** setting value 2 for field_1 for both cores */
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, DBAL_CORE_ALL);
    dbal_entry_key_field32_set(unit, entry_handle_id, key_field_id, 0);

    dbal_entry_value_field32_set(unit, entry_handle_id, value_field_id_0, INST_SINGLE, 2);
    ...
    dbal_value_field32_request(unit, entry_handle_id, value_field_id_0, INST_SINGLE, &result_fields_val[0][0]);
    dbal_value_field32_request(unit, entry_handle_id, value_field_id_1, INST_SINGLE, &result_fields_val[0][1]);
    ...
    dbal_value_field32_request(unit, entry_handle_id, value_field_id_0, INST_SINGLE, &result_fields_val[1][0]);
    dbal_value_field32_request(unit, entry_handle_id, value_field_id_1, INST_SINGLE, &result_fields_val[1][1]);

    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));

    if (result_fields_val[0][0] != 2)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "error validation 1 val=%d, expected 2\n", result_fields_val[0][0]);
    }

    if (result_fields_val[1][0] != 2)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "error validation 2 val=%d, expected 2\n", result_fields_val[1][0]);
    }

    if (result_fields_val[0][1] != 0)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "error validation 3 val=%d, expected 0\n", result_fields_val[0][1]);
    }

    if (result_fields_val[1][1] != 1)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "error validation 4 val=%d, expected 1\n", result_fields_val[1][1]);
    }
...
/**
 * \brief
 * This test sets and gets entries from DPC table using
    ...
    dbal_tables_e table_id)
{
    ...
    /** Commit #1: All key fields are 0, core 0, all result fields is 1 */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, table_id, &entry_handle_id));
    for (ii = 0; ii < table->nof_key_fields; ii++)
    {
    ...
    /** Commit #2: All key fields are 0, core 1, all result fields is 1 */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, table_id, entry_handle_id));
    for (ii = 0; ii < table->nof_key_fields; ii++)
    {
        dbal_fields_e field_id = table->keys_info[ii].field_id;
        ...
        {
            dbal_entry_key_field32_set(unit, entry_handle_id, field_id, DBAL_MAX_NUM_OF_CORES - 1);
        }
        else
        {
            dbal_entry_key_field32_set(unit, entry_handle_id, field_id, 0);
    ...
    /** get both entries using get core DEFAULT (0), and verify all
     *  results are 1 */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, table_id, entry_handle_id));
    for (ii = 0; ii < table->nof_key_fields; ii++)
    {
    ...
    /** Commit #3: All key fields are 0, core ALL, all result fields is 0   */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, table_id, entry_handle_id));
    for (ii = 0; ii < table->nof_key_fields; ii++)
    {
        dbal_fields_e field_id = table->keys_info[ii].field_id;
    ...
    dbal_tables_e table_id)
{
    ...
    switch (table_id)
    {
        ...
        case DBAL_TABLE_IN_LIF_PROFILE_TABLE:
            ...
            res_field = DBAL_FIELD_CS_IN_LIF_VTT_TO_VTT_PROFILE;
            field_value_set = 0x2;
            break;
...
static shr_error_e
dnx_dbal_ut_count_and_compare(
    int unit,
    uint32 entry_handle_id,
    uint8 nof_expected_entries)
{
    int is_end = 0;
    int entries_count;
    ...
    if (entries_count != nof_expected_entries)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Expected %d entries, but found %d.\n", nof_expected_entries, entries_count);
    }
exit:
    DBAL_FUNC_FREE_VARS;
    SHR_FUNC_EXIT;
 ...
 * Relevant for multi-core only
 */
shr_error_e
dnx_dbal_ut_multi_core_iterator(
    int unit,
    dbal_tables_e table_id)
{
    uint32 entry_handle_id;
    int ii, nof_committed_entries;

    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);

    if (table_id != DBAL_TABLE_INNER_ETHERNET_VLAN_CLASSIFICATION_LIF_SCOPED_VLAN_0_VLANS)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "This test can not run with %s table\n",
                     dbal_logical_table_to_string(unit, table_id));
    }
    ...
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, table_id, &entry_handle_id));
    /*
     * 1. Commit entry to all the cores 
     */
    nof_committed_entries = 0;
    for (ii = 0; ii < DBAL_MAX_NUM_OF_CORES; ++ii)
    {
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, ii);
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_GLOB_IN_LIF, 7);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_IN_LIF, INST_SINGLE, 3);
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
        SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, table_id, entry_handle_id));
        ++nof_committed_entries;
    }

    /** Iterate to retrieve all the entries */
    SHR_IF_ERR_EXIT(dnx_dbal_ut_count_and_compare(unit, entry_handle_id, nof_committed_entries));

    SHR_IF_ERR_EXIT(dbal_table_clear(unit, table_id));

    /*
     * 2. Commit entry to first and last core 
     */
    nof_committed_entries = 0;
    ...
    /*
     * 3. Commit entry to core 1 and last core (relevant for devices with more than 2 cores: 4 and above...)
     */
    nof_committed_entries = 0;
    ...
    /*
     * In dual core, last core is #1, so only one entry was committed. We let it that way, so test is general and suits 
     * all the devices 
     */
    if (1 < DBAL_MAX_NUM_OF_CORES - 1)
    {
        ++nof_committed_entries;
    }
    ...
    dbal_tables_e in_table_id)
{
    ...
    SHR_IF_ERR_EXIT(dbal_tables_table_create(unit, DBAL_ACCESS_METHOD_MDB, DBAL_TABLE_TYPE_TCAM_BY_ID,
                                             ...
                                             results_info, 0, "ctest_dyn_tbl", &table_id));
    ...
    for (iter = (first_table_id - DBAL_NOF_TABLES); iter < dnx_data_dbal.table.nof_dynamic_tables_get(unit); iter++)
    {
        SHR_IF_ERR_EXIT(dbal_tables_table_create(unit, DBAL_ACCESS_METHOD_MDB, DBAL_TABLE_TYPE_TCAM_BY_ID,
                                                 DBAL_CORE_MODE_SBC, nof_key_fields, keys_info, nof_result_fields,
                                                 results_info, 0, NULL, &table_id));
    }

    diag_dbal_pre_action_logger_close(unit);

    rv = dbal_tables_table_create(unit, DBAL_ACCESS_METHOD_MDB, DBAL_TABLE_TYPE_TCAM_BY_ID,
                                  DBAL_CORE_MODE_SBC, nof_key_fields, keys_info, nof_result_fields, results_info, 0,
                                  NULL, &wrong_table_id);
    ...
    dbal_tables_e table_id)
{
    ...
    for (jj = 0; jj < nof_res_types_to_test; jj++)
    {
        ...
        for (tt = 0; tt < ENTRIES_PER_RES_TYPE; tt++)
        {
            ...
            {
                uint32 val = 1;
                uint8 inst_idx = table->multi_res_info[jj].results_info[ii].inst_idx;
                ...
                dbal_entry_value_field32_set(unit, entry_handle_id, field_id, inst_idx, val);
            }
    ...
    dbal_tables_e table_id)
{
    ...
    SHR_IF_ERR_EXIT(dnx_dbal_configure_vsi_learning_table(unit));
    SHR_IF_ERR_EXIT(dnx_dbal_configure_mact_app_db(unit));
    ...
    dbal_tables_e table_id)
{
    ...
    uint32 entry_handle_id;
    CONST dbal_logical_table_t *table;
    bcm_mac_t mac = { 0x12, 0xc1, 0xd3, 0x44, 0x55, 0x66 };
    int result_type = 5;
    ...
    if (table_id != DBAL_TABLE_FWD_MACT && table_id != DBAL_TABLE_FWD_MACT_IVL)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "The age rule iterator test is written to run over FWD_MACT"
                     " and FWD_MACT_IVL tables only\n");
    }
    ...
    for (ii = 0; ii < nof_entries_age1; ii++)
    {
        ...
        {
            dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_VID, ii);
        }
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_RESULT_TYPE, INST_SINGLE, result_type);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_DESTINATION, INST_SINGLE, ii);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_MAC_STRENGTH, INST_SINGLE, 1);
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT | DBAL_COMMIT_HW_ONLY));
        SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, table_id, entry_handle_id));
    }
...
/**
 * \brief
 * Some fields can use a given field type and override the size/offset
 * In such case, mask needs to be used with same size/offset.
 * This tests validates right behavior in such situation.
 * It uses the table IPV6_VRF_DB_TCAM, field IPV6_SIP, which overrides sides to 96
 * instead of 128, and uses offset 32.
 */
shr_error_e
dnx_dbal_ut_non_aligned_data_mask_validation(
    int unit,
    dbal_tables_e table_id)
{
    uint32 entry_handle_id;
    uint32 access_id;
    uint8 data[SAL_IP6_ADDRLEN] =
        { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F };
    uint8 mask[SAL_IP6_ADDRLEN] =
        { 0xFF, 0xFF, 0x00, 0x00, 0xFF, 0xFF, 0xFF, 0xFF, 0x00, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF };
    CONST dbal_logical_table_t *table;
    uint8 field_val_get[DBAL_FIELD_ARRAY_MAX_SIZE_IN_WORDS] = { 0 };
    uint8 mask_val_get[DBAL_FIELD_ARRAY_MAX_SIZE_IN_WORDS] = { 0 };
    dbal_table_field_info_t *ipv6_sip_key_info;
    int ii;

    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);

    if (table_id != DBAL_TABLE_IPV6_VRF_DB_TCAM)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Test valid only for IPV6_VRF_DB_TCAM table.\n");
    }
    ...
    SHR_IF_ERR_EXIT(dbal_entry_attribute_set(unit, entry_handle_id, DBAL_ENTRY_ATTR_PRIORITY, 7));

    dbal_entry_key_field_arr8_masked_set(unit, entry_handle_id, DBAL_FIELD_IPV6_SIP, data, mask);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_VRF, INST_SINGLE, 3);
    ...
    SHR_IF_ERR_EXIT(dbal_entry_handle_key_field_arr8_masked_get(unit, entry_handle_id, DBAL_FIELD_IPV6_SIP,
                                                                field_val_get, mask_val_get));

    for (ii = 0; ii < table->nof_key_fields; ++ii)
    {
        if ((table->keys_info[ii].field_id) == DBAL_FIELD_IPV6_SIP)
        {
            break;
        }
    }

    if (ii == table->nof_key_fields)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Could not find field DBAL_FIELD_IPV6_SIP\n");
    }

    ipv6_sip_key_info = &table->keys_info[ii];
    for (ii = 0; ii < BITS2BYTES(ipv6_sip_key_info->field_nof_bits); ++ii)
    {
        if ((field_val_get[ii] != (data[ii] & mask[ii])) || (mask_val_get[ii] != mask[ii]))
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "Received value is not as expected\nIndex = %d, field_val_get = %#04x, "
                         "mask_val_get = %#04x, input value = %#04x, input mask = %#04x", ii, field_val_get[ii],
                         mask_val_get[ii], data[ii], mask[ii]);
        }
    }
...
#define MAX_NUM_RESULTS 5
    ...
    sal_memset(&key_field_val_arr[0][0], 0, sizeof(key_field_val_arr));
    ...
    {
        field_id = table->keys_info[ii].field_id;
        dbal_entry_key_field_arr32_set(unit, set_entry_handle_id, field_id, key_field_val_arr[ii]);
        dbal_entry_key_field_arr32_set(unit, upd_entry_handle_id, field_id, key_field_val_arr[ii]);
        dbal_entry_key_field_arr32_set(unit, get_entry_handle_id, field_id, key_field_val_arr[ii]);
    }
    ...
    dbal_tables_e table_id)
{
    ...
    /** Verify table id */
    if ((table_id != DBAL_TABLE_BFD_ENDPOINT_SW_INFO)
        && (table_id != DBAL_TABLE_BRIDGE_IPV4_MULTICAST_SOURCE_SPECIFIC_IVL))
    {
...
/**
 * \brief
 * Validate distinct field to field type per family.
 */
shr_error_e
dnx_dbal_ut_field_to_field_type_per_family(
    int unit)
{
    dbal_fields_e field_id = DBAL_FIELD_DUMMY_FIELD1;
    dbal_field_types_defs_e actual_field_type = DBAL_FIELD_TYPE_DEF_EMPTY;
    dbal_field_types_defs_e expected_field_type = DBAL_FIELD_TYPE_DEF_DUMMY_FIELD_TYPE1;
    SHR_FUNC_INIT_VARS(unit);

    SHR_IF_ERR_EXIT(dbal_fields_field_type_get(unit, field_id, &actual_field_type));
    if (actual_field_type != expected_field_type)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Unexpected field type. Was expecting %s, but actual was %s \n",
...
#define VALID_IND_MAX_NUM_RESULTS 4
    ...
    /*
     * not related to dnx_dbal_ut_field_valid_indication.
     * Validiation on field to field type per family.
     * Not worth its own ut
     */
    SHR_IF_ERR_EXIT(dnx_dbal_ut_field_to_field_type_per_family(unit));
...
#define TABLE_RESTORE_MAX_NUM_RESULTS 4
    ...
    SHR_IF_ERR_EXIT(dbal_tables_table_create(unit, DBAL_ACCESS_METHOD_MDB, DBAL_TABLE_TYPE_TCAM_BY_ID,
                                             ...
                                             TABLE_RESTORE_MAX_NUM_RESULTS, results_info, 0, NULL, &table_id));
...
#define MAX_NUM_RESULT_FIELDS 10
    ...
    for (ii = 0; ii < table->multi_res_info[0].nof_result_fields; ii++)
    {
        dbal_value_field_arr32_request(unit,
                                       get_entry_handle_id,
                                       table->multi_res_info[0].results_info[ii].field_id, INST_SINGLE, get_values[ii]);
    }
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, get_entry_handle_id, DBAL_COMMIT));
    for (ii = 0; ii < table->multi_res_info[0].nof_result_fields; ii++)
    {
    ...
    dbal_tables_e table_id)
{
    ...
    /*
     * Commit Force action - for update
     * Set first result to 2
     * Verify counter does not change
     * Relevant only for LPM and EM - otherwise error will be returned from commit function
     */
    if ((table->table_type == DBAL_TABLE_TYPE_EM) || (table->table_type == DBAL_TABLE_TYPE_LPM))
    {

        field_id = table->multi_res_info[0].results_info[0].field_id;

        dbal_entry_value_field32_set(unit, entry_handle_id, field_id, INST_SINGLE, 2);
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT_FORCE));
        SHR_IF_ERR_EXIT(DBAL_SW_STATE_TBL_PROP.nof_entries.get(unit, table_id, &nof_entries));
        if (entries_counter != nof_entries)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL,
                         "After Commit Force/Update action, expected Entries count: %u, but actual is: %d. Table: %s\n",
                         entries_counter, nof_entries, dbal_logical_table_to_string(unit, table_id));
        }
    }

    /*
     * Commit Force action - for commit
     * Set first key to 0 (new entry) and first result to 1
    ...
    if ((table->table_type == DBAL_TABLE_TYPE_EM) || (table->table_type == DBAL_TABLE_TYPE_LPM))
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL,
                         "After Commit Force/Commit action, expected Entries count: %u, but actual is: %d. Table: %s\n",
                         entries_counter, nof_entries, dbal_logical_table_to_string(unit, table_id));
        }
    }
...
{
    int ii;
    uint32 entry_handle_id, inst_idx;
    CONST dbal_logical_table_t *table;
    dbal_fields_e field_id;
    ...
    {
        field_id = table->multi_res_info[0].results_info[ii].field_id;
        inst_idx = table->multi_res_info[0].results_info[ii].inst_idx;

        dbal_entry_value_field32_set(unit, entry_handle_id, field_id, inst_idx, 1);
    }
    ...
    for (ii = 0; ii < table->multi_res_info[0].nof_result_fields; ii++)
    {
        ...
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field_arr32_get(unit, entry_handle_id, field_id, inst_idx,
                                                                field_val_int));
        if (ii == 0)
        {
    ...
    DBAL_FUNC_INIT_VARS(unit);
    {
        if (table_id == DBAL_TABLE_EXAMPLE_FOR_HL_MULTIPLE_RESULT)
        {
            ...
            for (ii = 0; ii < NOF_ITERATIONS; ii++)
            {
                ...
                /** For single core devices core ID is always 0 change expected result accordingly */
                if (dnx_data_device.general.nof_cores_get(unit) == 1)
                {
...
#define NOF_ITERATIONS 4
    ...
    else if (table_id == DBAL_TABLE_IPV4_UNICAST_PRIVATE_HOST)
    {
        ...
        SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, table_id, entry_handle_id));
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_VRF, 3);
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_IPV4, 3133);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_RESULT_TYPE, INST_SINGLE,
                                     DBAL_RESULT_TYPE_IPV4_UNICAST_PRIVATE_HOST_FWD_DEST);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_PORT_ID, INST_SINGLE, 0x5);
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
        ...
        for (ii = 0; ii < NOF_ITERATIONS; ii++)
        {
            ...
            int jj;
            /** Rules parameters per iteration */
            int expected_counter[NOF_ITERATIONS] = { 4, 3, 1, 0 };
            dbal_condition_types_e rule_val_condition[NOF_ITERATIONS] =
        ...
        /*
         * Check that we can define rule according to subfield 
         */
        {
            int is_end = 0;
            int entries_counter = 0;
            uint32 rule_field_val = 1;
            SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, table_id, entry_handle_id));
            SHR_IF_ERR_EXIT(dbal_iterator_init(unit, entry_handle_id, DBAL_ITER_MODE_GET_ALL_EXCEPT_DEFAULT));
            SHR_IF_ERR_EXIT(dbal_iterator_value_field_arr32_rule_add
                            /*
                             * DBAL_FIELD_PORT_ID is a subfield of DBAL_FIELD_DESTINATION 
                             */
                            (unit, entry_handle_id, DBAL_FIELD_PORT_ID, 0, DBAL_CONDITION_BIGGER_THAN,
                             &rule_field_val, NULL));
            ...
            {
                SHR_ERR_EXIT(_SHR_E_FAIL, "Wrong number of counted entries: Expected 1, Actual %d\n", entries_counter);
            }
        }
        /** Clear Table */
        SHR_IF_ERR_EXIT(dbal_table_clear(unit, table_id));
 ...
 */
shr_error_e
dnx_dbal_ut_iterator_update_get_mact(
    int unit,
    dbal_tables_e table_id)
{
    uint32 entry_handle_id;
    ...
    uint32 rule_value = 0;
    bcm_mac_t mac = { 0x12, 0xc1, 0xd3, 0x44, 0x55, 0x66 }, mac2 =
    {
    0x12, 0xc1, 0xd3, 0x44, 0x55, 0x67};
    uint32 expected_nof_entries = 0;
    ...
    /** Clear Table */
    SHR_IF_ERR_EXIT(dbal_table_clear(unit, table_id));
    /** Entry 1: flush compatible, FODO=1, RT=1, DESTINATION=1 */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, table_id, &entry_handle_id));
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_FODO, 1);
    dbal_entry_key_field_arr8_set(unit, entry_handle_id, DBAL_FIELD_L2_MAC, mac);
    if (table_id == DBAL_TABLE_FWD_MACT_IVL)
    {
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_VID, 1);
    }
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_RESULT_TYPE, INST_SINGLE, 1);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_DESTINATION, INST_SINGLE, 1);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_MAC_STRENGTH, INST_SINGLE,
                                 DBAL_ENUM_FVAL_MAC_STRENGTH_2);
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    ++expected_nof_entries;

    /** Entry 2: non-flush compatible, FODO=0, RT=0, DESTINATION=0 */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, table_id, entry_handle_id));
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_FODO, 0);
    dbal_entry_key_field_arr8_set(unit, entry_handle_id, DBAL_FIELD_L2_MAC, mac);
    if (table_id == DBAL_TABLE_FWD_MACT_IVL)
    {
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_VID, 0);
    }
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_RESULT_TYPE, INST_SINGLE, 0);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_DESTINATION, INST_SINGLE, 4);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_MAC_STRENGTH, INST_SINGLE,
                                 DBAL_ENUM_FVAL_MAC_STRENGTH_2);
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    ++expected_nof_entries;

    /** Entry 3: non-flush compatible, FODO=2, RT=2, DESTINATION=2 */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, table_id, entry_handle_id));
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_FODO, 2);
    dbal_entry_key_field_arr8_set(unit, entry_handle_id, DBAL_FIELD_L2_MAC, mac);
    if (table_id == DBAL_TABLE_FWD_MACT_IVL)
    {
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_VID, 0);
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_RESULT_TYPE, INST_SINGLE, 2);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_DESTINATION, INST_SINGLE, 2);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_MAC_STRENGTH, INST_SINGLE,
                                 DBAL_ENUM_FVAL_MAC_STRENGTH_2);
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    ++expected_nof_entries;

    /** Entry 4: non-flush compatible, FODO=3, RT=2, DESTINATION=2 */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, table_id, entry_handle_id));
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_FODO, 3);
    dbal_entry_key_field_arr8_set(unit, entry_handle_id, DBAL_FIELD_L2_MAC, mac);
    if (table_id == DBAL_TABLE_FWD_MACT_IVL)
    {
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_VID, 0);
    ...
    /** Entry 5: flush compatible, FODO=0, RT=3, DESTINATION=2 */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, table_id, entry_handle_id));
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_FODO, 0);
    dbal_entry_key_field_arr8_set(unit, entry_handle_id, DBAL_FIELD_L2_MAC, mac2);
    if (table_id == DBAL_TABLE_FWD_MACT_IVL)
    {
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_VID, 0);
    }
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_RESULT_TYPE, INST_SINGLE, 3);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_DESTINATION, INST_SINGLE, 2);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_MAC_STRENGTH, INST_SINGLE,
                                 DBAL_ENUM_FVAL_MAC_STRENGTH_2);
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    ...
    /** Entry 6: flush compatible, FODO=2, RT=3, DESTINATION=2 */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, table_id, entry_handle_id));
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_FODO, 2);
    dbal_entry_key_field_arr8_set(unit, entry_handle_id, DBAL_FIELD_L2_MAC, mac2);
    if (table_id == DBAL_TABLE_FWD_MACT_IVL)
    ...
    SHR_IF_ERR_EXIT(dbal_iterator_entries_count_get(unit, entry_handle_id, &entries_counter));
    if (entries_counter != expected_nof_entries)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Expected %d entries but found %d.\n", expected_nof_entries, entries_counter);
    }

    /** Update entries with result type 3, to a new DBAL_FIELD_DESTINATION value
     * The test is done is 1 step: iterate over entries (update is done in flush at first call of get next, then SW
     * is updated at each iteration)
     * In this test, all the entries are in a one result type */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, table_id, entry_handle_id));
    SHR_IF_ERR_EXIT(dbal_iterator_init(unit, entry_handle_id, DBAL_ITER_MODE_GET_ALL_EXCEPT_DEFAULT));
    rule_value = 3;
    SHR_IF_ERR_EXIT(dbal_iterator_value_field_arr32_rule_add
    ...
    SHR_IF_ERR_EXIT(dbal_iterator_field_arr32_action_add
                    (unit, entry_handle_id, DBAL_ITER_ACTION_UPDATE, DBAL_FIELD_DESTINATION, 0, &update_res_field,
                     NULL));

    /** count only 2 entries and validate the destination field */
    is_end = 0;
    entries_counter = 0;
    SHR_IF_ERR_EXIT(dbal_iterator_get_next(unit, entry_handle_id, &is_end));
    while (!is_end)
    {
        ...
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                        (unit, entry_handle_id, DBAL_FIELD_DESTINATION, 0, &get_res_field));
        if (get_res_field != update_res_field)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL,
                         "Expected destination %d, read destination %d\n", update_res_field, get_res_field);
        }
        SHR_IF_ERR_EXIT(dbal_iterator_get_next(unit, entry_handle_id, &is_end));
    }
    SHR_IF_ERR_EXIT(dbal_iterator_entries_count_get(unit, entry_handle_id, &entries_counter));
    ...
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Expected 2 entries but counter is %d.\n", entries_counter);
    }

    /*
     * At this step we have 3 entries with destination value 2
     */
    /** Update entries with DBAL_FIELD_DESTINATION = 2, to a new DBAL_FIELD_DESTINATION value (7)
     * The test is done is 1 step: iterate over entries (update is done in flush at first call of get next, then SW
     * is updated at each iteration)
     * In this test, all the entries are in a two different result types */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, table_id, entry_handle_id));
    SHR_IF_ERR_EXIT(dbal_iterator_init(unit, entry_handle_id, DBAL_ITER_MODE_GET_ALL_EXCEPT_DEFAULT));
    rule_value = 2;
    SHR_IF_ERR_EXIT(dbal_iterator_value_field_arr32_rule_add
                    (unit, entry_handle_id, DBAL_FIELD_DESTINATION, 0, DBAL_CONDITION_EQUAL_TO, &rule_value, NULL));
    update_res_field = 7;
    SHR_IF_ERR_EXIT(dbal_iterator_field_arr32_action_add
                    (unit, entry_handle_id, DBAL_ITER_ACTION_UPDATE, DBAL_FIELD_DESTINATION, 0, &update_res_field,
                     NULL));

    /** count only 3 entries and validate the destination field */
    is_end = 0;
    entries_counter = 0;
    SHR_IF_ERR_EXIT(dbal_iterator_get_next(unit, entry_handle_id, &is_end));
    while (!is_end)
 ...
 */
shr_error_e
dnx_dbal_ut_iterator_rules_actions_mact(
    int unit,
    dbal_tables_e table_id)
{
    shr_error_e rv = _SHR_E_NONE;
    ...
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_DESTINATION, INST_SINGLE, &read_field_value);
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));
    if (read_field_value != 1)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Value expected: 1, read: %d.\n", read_field_value);
    }

    /** Delete a entries by rule RT = 1, FODO = 1 (from flush) */
    SHR_IF_ERR_EXIT(dbal_iterator_init(unit, entry_handle_id, DBAL_ITER_MODE_GET_ALL_EXCEPT_DEFAULT));
    rule_value = 1;
    SHR_IF_ERR_EXIT(dbal_iterator_key_field_arr32_rule_add
                    (unit, entry_handle_id, DBAL_FIELD_FODO, DBAL_CONDITION_EQUAL_TO, &rule_value, NULL));
    SHR_IF_ERR_EXIT(dbal_iterator_value_field_arr32_rule_add
                    (unit, entry_handle_id, DBAL_FIELD_RESULT_TYPE, 0, DBAL_CONDITION_EQUAL_TO, &rule_value, NULL));
    ...
    /*
     * After deletion from flush, check entry was cleared from shadow too
     */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, table_id, entry_handle_id));
    SHR_IF_ERR_EXIT(dbal_entry_handle_get_internal(unit, entry_handle_id, &entry_handle));
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_FODO, 1);
    ...
    if (rv != _SHR_E_NOT_FOUND)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Entry not deleted from shadow\n");
        SHR_EXIT();
    }

    /*========================================================================================*/

    /** Delete a entries by rule RT = 2, destination = 2 (non flush) */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, table_id, entry_handle_id));
    SHR_IF_ERR_EXIT(dbal_iterator_init(unit, entry_handle_id, DBAL_ITER_MODE_GET_ALL_EXCEPT_DEFAULT));
    rule_value = 2;
    SHR_IF_ERR_EXIT(dbal_iterator_value_field_arr32_rule_add
    ...
    SHR_IF_ERR_EXIT(dbal_iterator_entry_action_add(unit, entry_handle_id, DBAL_ITER_ACTION_DELETE));
    SHR_IF_ERR_EXIT(dbal_iterator_action_commit(unit, entry_handle_id));
    expected_nof_entries -= 2;

    /** Valid count of entries */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, table_id, entry_handle_id));
    SHR_IF_ERR_EXIT(dbal_iterator_init(unit, entry_handle_id, DBAL_ITER_MODE_GET_ALL_EXCEPT_DEFAULT));
    is_end = 0;
    entries_counter = 0;
    ...
    /** Update entries with result type 3, to a new DBAL_FIELD_DESTINATION value
     * The test is done is 2 steps: 1 - commit, 2 - iterate over entries
     */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, table_id, entry_handle_id));
    SHR_IF_ERR_EXIT(dbal_iterator_init(unit, entry_handle_id, DBAL_ITER_MODE_GET_ALL_EXCEPT_DEFAULT));
    rule_value = 3;
    ...
    /** Get entries by rule at DESTINATION (from flush and non flush) */
    SHR_IF_ERR_EXIT(dbal_iterator_init(unit, entry_handle_id, DBAL_ITER_MODE_GET_ALL_EXCEPT_DEFAULT));
    rule_value = update_res_field;
    SHR_IF_ERR_EXIT(dbal_iterator_value_field_arr32_rule_add
                    (unit, entry_handle_id, DBAL_FIELD_DESTINATION, 0, DBAL_CONDITION_EQUAL_TO, &rule_value, NULL));
    ...
    dbal_tables_e table_id)
{
    ...
    for (iter = 0; iter < expect_cnt; iter++)
    {
        ...
        {
            case DBAL_FIELD_TYPE_DEF_QOS_PROFILE:
                SHR_IF_ERR_EXIT(algo_qos_db.ingress_qos.free_single(unit, alloc_resource[iter], NULL));
                break;
            case DBAL_FIELD_TYPE_DEF_ECN_QOS_PROFILE:
                SHR_IF_ERR_EXIT(algo_qos_db.ingress_qos_ecn.free_single(unit, alloc_resource[iter], NULL));
                break;
            default:
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "Add relevant case for the relevant resource.\n");
                break;
    ...
    dbal_tables_e table_id)
{
     ...
     */
    if ((table_id != DBAL_TABLE_MDB_33_LEM) && (table_id != DBAL_TABLE_HCC_BIST_RUN))
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "This test can not run with %s table\n",
                     dbal_logical_table_to_string(unit, table_id));
    }
     ...
     */
    if (table_id == DBAL_TABLE_MDB_33_LEM)
    {
        SHR_EXIT();
        dbal_entry_value_field32_set(unit, entry_handle_wr, DBAL_FIELD_MDB_ITEM_1, INST_SINGLE, 1);
    }
    ...
    dbal_tables_e table_id)
{
    ...
    if (table_field == NULL)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "table_field is NULL. Illegal \n");
    }
    ...
    /**for enum need to compare the hw values */
    ...
    dbal_tables_e table_id)
{
    ...
    int nof_key_rules = DNX_DATA_MAX_L2_DMA_FLUSH_DB_NOF_DMA_RULES;

    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);

    /*
     * In case the flush can support bigger entries (Size ONE)
     * Add half entries, as each entry will create two FLush rules (1 per entry size)
     */
    if (dnx_data_mdb.em.flush_max_supported_payload_get(unit) > 64)
    {
        nof_key_rules /= 2;
    }
    /*
     ...
     * per result type add NOF_ENTRIES_PER_RT entries:
     */
    cli_out("Step1. Adding nof_key_rules(%d) + 1 entries of all result types\n", nof_key_rules);

    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, table_id, &entry_handle_id));
    ii = 0;
    nof_entries = 0;
    while (nof_entries < nof_key_rules + 1)
    {
        int payload_size = table->multi_res_info[ii % table->nof_result_types].entry_payload_size;

        if (payload_size > dnx_data_mdb.em.flush_max_supported_payload_get(unit))
        {
            ii++;
            continue;
        }
    ...
    /*
     * Iterate table and count entries - should be nof_key_rules+1
     */
    cli_out("Step2. Iterate table and count entries\n");
    {
        int is_end = FALSE;
        ...
        expected_entries_count = nof_key_rules + 1;
        SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, table_id, entry_handle_id));
        SHR_IF_ERR_EXIT(dbal_iterator_init(unit, entry_handle_id, DBAL_ITER_MODE_GET_ALL_EXCEPT_DEFAULT));
        SHR_IF_ERR_EXIT(dbal_iterator_get_next(unit, entry_handle_id, &is_end));
        while (!is_end)
    ...
    /*
     * Delete nof_key_rules entries with key rule on vsi only
     */
    ...
    cli_out("Step3. Delete nof_key_rules entries with key rule on vsi only\n");
    {
        for (ii = 0; ii < nof_key_rules; ii++)
        {
            SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, table_id, entry_handle_id));
            SHR_IF_ERR_EXIT(dbal_iterator_init(unit, entry_handle_id, DBAL_ITER_MODE_GET_ALL_EXCEPT_DEFAULT));
     ...
     * with key VSI=vsi+nof_key_rules
     */
    cli_out("Step4. Iterate table and validate a single entry is found, with key VSI=vsi+nof_key_rules\n");
    {
        int is_end = FALSE;
        int entries_counter = 0, expected_entries_count = 0;
        ...
        SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, table_id, entry_handle_id));
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_VSI, vsi + nof_key_rules);
        dbal_entry_key_field_arr8_set(unit, entry_handle_id, DBAL_FIELD_L2_MAC, mac_address);
        if (table_id == DBAL_TABLE_FWD_MACT_IVL)
        {
            dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_VID, 0);
    ...
    dbal_tables_e table_id)
{
    ...
    int nof_rules = DNX_DATA_MAX_L2_DMA_FLUSH_DB_NOF_DMA_RULES;
     ...
     * per result type add NOF_ENTRIES_PER_RT entries:
     */
    cli_out("Step1. Adding nof_rules(%d) + 1 entries of all result types\n", nof_rules);

    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, table_id, &entry_handle_id));
    ii = 0;
    nof_entries = 0;
    while (nof_entries < nof_rules)
    {
        int payload_size = table->multi_res_info[ii % table->nof_result_types].entry_payload_size;
    ...
    /*
     * Iterate table and count entries - should be nof_rules
     */
    cli_out("Step2. Iterate table and count entries\n");
    {
        int is_end = FALSE;
        ...
        expected_entries_count = nof_rules;
        SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, table_id, entry_handle_id));
        SHR_IF_ERR_EXIT(dbal_iterator_init(unit, entry_handle_id, DBAL_ITER_MODE_GET_ALL_EXCEPT_DEFAULT));
        SHR_IF_ERR_EXIT(dbal_iterator_get_next(unit, entry_handle_id, &is_end));
        while (!is_end)
     ...
     */
    cli_out("Step4. Iterate table and validate a nof_rules -1 entries are found, only one deleted\n");
    {
        int is_end = FALSE;
        int entries_counter = 0, expected_entries_count = 0;

        expected_entries_count = nof_rules - 1;
        SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, table_id, entry_handle_id));
        SHR_IF_ERR_EXIT(dbal_iterator_init(unit, entry_handle_id, DBAL_ITER_MODE_GET_ALL_EXCEPT_DEFAULT));
        SHR_IF_ERR_EXIT(dbal_iterator_get_next(unit, entry_handle_id, &is_end));
        while (!is_end)
    ...
    cli_out("Step4. Iterate table and validate half of original entries are removed\n");
    {
        ...
        expected_entries_count = nof_rules / 2;
        SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, table_id, entry_handle_id));
        SHR_IF_ERR_EXIT(dbal_iterator_init(unit, entry_handle_id, DBAL_ITER_MODE_GET_ALL_EXCEPT_DEFAULT));
        SHR_IF_ERR_EXIT(dbal_iterator_get_next(unit, entry_handle_id, &is_end));
        while (!is_end)
     ...
     * with key VSI=vsi+nof_rules
     */
    cli_out("Step6. Iterate table and validate all of original entries are removed\n");
    {
        int is_end = FALSE;
        int entries_counter = 0, expected_entries_count = 0;
    ...
    dbal_tables_e table_id)
{
    ...
    /*
     * Verify table ID, This test is valid only for specific table
     */
    ...
    dbal_tables_e table_id)
{
    ...
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "checker thread didn't perform action %d \n", protection_sync_value);
    }
    ...
    dbal_tables_e table_id)
{
    ...
    diag_dbal_pre_action_logger_close(unit);
    {
        dbal_result_type_t invalid_result_type;
        SHR_IF_ERR_EXIT(dbal_tables_table_nof_res_type_get
                        (unit, DBAL_TABLE_ING_VSI_INFO_DB, (int *) &invalid_result_type));
        invalid_result_type += 1;
        rv = dbal_entry_result_type_update(unit, entry_handle_id, invalid_result_type,
...
/**
 * \brief
 *    This function loads and runs the cint functions to configures the device in order to generate hitbit for
 *    the xxx test
 * \param [in]
 *    unit- Relevant unit
 * \param [in]
    ...
    int32 vrf)
{
    ...
    /** Load relevant cints */
    SHR_CLI_EXIT_IF_ERR(ctest_dnxc_cint_load(unit, "sand/cint_ip_route_basic.c"), "CINT Load Failed\n");
    SHR_CLI_EXIT_IF_ERR(ctest_dnxc_cint_load(unit, "sand/utility/cint_sand_utils_global.c"), "CINT Load Failed\n");
    SHR_CLI_EXIT_IF_ERR(ctest_dnxc_cint_load(unit, "sand/cint_mpls_encapsulation_basic.c"), "CINT Load Failed\n");
    SHR_CLI_EXIT_IF_ERR(ctest_dnxc_cint_load(unit, "sand/cint_mpls_termination_basic.c"), "CINT Load Failed\n");
    SHR_CLI_EXIT_IF_ERR(ctest_dnxc_cint_load(unit, "dnx/internal/cint_mpls_l3vpn_test_jr2.c"), "CINT Load Failed\n");

    /** Config arguments for the cint function */
    /** in port */
    cint_arguments[0].type = SAL_FIELD_TYPE_INT32;
    cint_arguments[0].value.value_int32 = inP;
    /** out port */
    cint_arguments[1].type = SAL_FIELD_TYPE_INT32;
    cint_arguments[1].value.value_int32 = outP;
    /** vrf */
    cint_arguments[2].type = SAL_FIELD_TYPE_INT32;
    cint_arguments[2].value.value_int32 = vrf;

    /** Run cint function */
    SHR_CLI_EXIT_IF_ERR(ctest_dnxc_cint_run(unit, "mpls_l3vpn_decapsulation_ipv4_4lbl", cint_arguments, 3),
                        "CINT Run Failed\n");
    /** Reset cint */
...
/**
 * \brief
 *    This function sends an MPLS packet in order to hit the entry configured for
 *    the iterator_hitbit_retrieved test
 * \param [in]
 *    unit- Relevant unit
 * \param [in]
 *    inP- In port
    ...
    int inP)
{
    ...
    uint32 flags = 0;
    char *smac = "00:11:00:00:01:12";
    char *dmac = "00:0c:00:02:00:00";
    char dip[RHNAME_MAX_SIZE] = "160.161.161.162";
    char sip[RHNAME_MAX_SIZE] = "160.0.0.17";
    uint32 vc_label = 5000;
    uint32 lsp_label1 = 3000;
    uint32 lsp_label2 = 2000;
    uint32 lsp_label3 = 1000;

    SHR_FUNC_INIT_VARS(unit);
    SHR_IF_ERR_EXIT(bcm_port_get(unit, inP, &flags, &interface_info, &mapping_info));
    ...
    /** config packet of format IPv4oMPLSoMPLSoMPLSoMPLSoETH1 */
    ...
    SHR_IF_ERR_EXIT(diag_sand_packet_field_add_uint32(unit, packet_tx, "MPLS.LABEL", &lsp_label1, 16));
    SHR_IF_ERR_EXIT(diag_sand_packet_proto_add(unit, packet_tx, "MPLS"));
    SHR_IF_ERR_EXIT(diag_sand_packet_field_add_uint32(unit, packet_tx, "MPLS.LABEL", &lsp_label2, 16));
    SHR_IF_ERR_EXIT(diag_sand_packet_proto_add(unit, packet_tx, "MPLS"));
    SHR_IF_ERR_EXIT(diag_sand_packet_field_add_uint32(unit, packet_tx, "MPLS.LABEL", &lsp_label3, 16));
    SHR_IF_ERR_EXIT(diag_sand_packet_proto_add(unit, packet_tx, "MPLS"));
    SHR_IF_ERR_EXIT(diag_sand_packet_field_add_uint32(unit, packet_tx, "MPLS.LABEL", &vc_label, 16));
    ...
    /** send packet */
    SHR_IF_ERR_EXIT(diag_sand_packet_send(unit, inP, packet_tx, SAND_PACKET_RX | SAND_PACKET_RESUME));
exit:
    SHR_FUNC_EXIT;
}
/**
 * \brief
 *    This function init iterator with action to retrieve hit-bit and verifies that that hit-bit is retrieved correctly
 *    as part of the iterator_hitbit_retrieved test
 * \param [in]
 *    unit- Relevant unit
 * \param [in]
 *    inP- In port
 ...
 */
static shr_error_e
init_iterator_and_check_hitbit(
    int unit,
    int core_id,
    dbal_tables_e table_id)
{
    ...
    /** Init iterator and add an action to retrieve hit bit */
    SHR_IF_ERR_EXIT(dbal_iterator_init(unit, entry_handle_id, DBAL_ITER_MODE_GET_ALL_EXCEPT_DEFAULT));
    ...
    /** Iterate over the entries of the table and validate the retrieved hit bit */
    while (!is_end)
    {
        ...
        /** Get core ID */
        SHR_IF_ERR_EXIT(dbal_entry_handle_key_field_arr32_get
                        (unit, entry_handle_id, DBAL_FIELD_CORE_ID, &entry_core_id));

        /** Get hit bit value */
        SHR_IF_ERR_EXIT(dbal_entry_attribute_get(unit, entry_handle_id, DBAL_ENTRY_ATTR_HIT_GET, &hitbit));

        /** Validate that hit retrieved only from current core */
        if (hitbit)
        {
            hitbit_occurred = TRUE;
            if (entry_core_id != core_id)
            {
                SHR_ERR_EXIT(_SHR_E_FAIL, "Received hit at core %d.", entry_core_id);
            }
        }
        SHR_IF_ERR_EXIT(dbal_iterator_get_next(unit, entry_handle_id, &is_end));
    }

    /** Verify that a hit bit indeed occur */
    if (!hitbit_occurred)
    {
        SHR_ERR_EXIT(_SHR_E_FAIL, "No hit was retrieved at core %d.", core_id);
    }
...
/**
 * \brief
 *    This test tests the dbal iterator hit-bit retrieved value.
 *    The test validates that the hit bit is retrieved on all cores
 */
shr_error_e
dnx_dbal_ut_iterator_hitbit_retrieved(
    int unit,
    dbal_tables_e table_id)
{
    ...
    if (SAL_BOOT_PLISIM)
    {
        SHR_EXIT_WITH_LOG(_SHR_E_NONE, "This test is not supported by adapter!%s%s%s\n", EMPTY, EMPTY, EMPTY);
    }

    if (table_id != DBAL_TABLE_MPLS_TERMINATION_SINGLE_LABEL_DB)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "The iterator hit bit retrieved test is written to run over MPLS_TERMINATION_SINGLE_LABEL_DB only\n");
    }

    /** Iterate over the cores of the device */
    SH_DNX_CORES_ITER(unit, core_in, core_id)
    {
        /** Get CPU port for the specified core */
        SHR_IF_ERR_EXIT(dnx_algo_port_logicals_get(unit, core_id, DNX_ALGO_PORT_LOGICALS_TYPE_CPU, 0, &logical_ports));
        _SHR_PBMP_FIRST(logical_ports, in_port);

        /** Start roll-back journal (for cleanup) */
        SHR_IF_ERR_EXIT(dnx_rollback_journal_start(unit));

        /** run cint */
        SHR_IF_ERR_EXIT(load_cint_mpls_hitbit(unit, in_port, 201, 70));

        /** config and send packet */
        SHR_IF_ERR_EXIT(send_packet_mpls_for_hitbit(unit, in_port));

        /** init iterator with action to retrieve hit-bit to check that hit-bit is retrieved correctly */
        SHR_IF_ERR_EXIT(init_iterator_and_check_hitbit(unit, core_id, table_id));

        /** Cleanup; End roll-back journal */
 ...
 */
shr_error_e
dnx_dbal_ut_iterator_hitbit_over_tables(
    int unit,
    dbal_tables_e table_id)
{
    dbal_entry_handle_t *entry_handle;
    ...
    if (table_id != DBAL_TABLE_IPV6_TT_MP_EM_16 && table_id != DBAL_TABLE_SRV6_TERMINATED_LIF)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "The iterator hit bit retrieved test is written to run over IPV6_TT_MP_EM_16 only\n");
    }

    table = entry_handle->table;

    /*
     * Do the test for each physical table related to this table
     */
    for (jj = 0; jj < table->nof_physical_tables; ++jj)
    {
         ...
         * DPC table
         */
        if (table_id == DBAL_TABLE_IPV6_TT_MP_EM_16)
        {
            for (ii = 0; ii < DBAL_MAX_NUM_OF_CORES; ++ii)
            {
                /*
                 * Commit an entry in core ii
                 */
                SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, table_id, entry_handle_id));
                dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, ii);
                dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_IPV6_ADDRESS_16, 65535);
                dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_IPV6_MP_TUNNEL_IDX, 30);
                dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_IN_LIF, INST_SINGLE, 32);
                SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
            }
        }
        /*
         * SBC table
         */
        else if (table_id == DBAL_TABLE_SRV6_TERMINATED_LIF)
        {
            SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, table_id, entry_handle_id));
            dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_SRH_LAST_ENTRY, 30);
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_IN_LIF, INST_SINGLE, 32);
            SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
        }

        /*
         * Set hitbit on in the jjth phy DB related to this table
         */
        phy_entry = &entry_handle->phy_entry;
        phy_entry->hitbit |= DBAL_PHYSICAL_KEY_HITBIT_WRITE;

        SHR_IF_ERR_EXIT(mdb_em_entry_get(unit, table->physical_db_id[jj], entry_handle->table->app_id, phy_entry, 0));
        ...
        /** Iterate over the entries of the table and check hitbit is found */
        while (!is_end)
        {
            /** Get hit bit value */
            SHR_IF_ERR_EXIT(dbal_entry_attribute_get(unit, entry_handle_id, DBAL_ENTRY_ATTR_HIT_GET, &hitbit));

            /** Validate hit was retrieved */
            if (hitbit)
            {
                break;
            }
