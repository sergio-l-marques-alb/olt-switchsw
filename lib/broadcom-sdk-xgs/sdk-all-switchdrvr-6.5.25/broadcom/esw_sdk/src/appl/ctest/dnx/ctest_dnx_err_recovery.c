...
/*
 * \brief - pre/post rollback swstate variable snapshots (per thread), used for the multithreading test
 */
uint32 pre_rb_swstate_var[DNX_ERR_RECOVERY_MAX_NOF_ACTIVE_TRANSACTION] = { 0 };
uint32 post_rb_swstate_var[DNX_ERR_RECOVERY_MAX_NOF_ACTIVE_TRANSACTION] = { 0 };
    ...
    uint32 flag)
{
    ...
    if (er_thread_id != -1)
    {
        return dnx_err_recovery_common_flag_is_on(unit, er_thread_id, mngr_id, flag);
    }
    else
    {
        LOG_CLI((BSL_META
                 ("err recovery ERROR: The current transcion's ID does not match any of the ER thread IDs.\n")));
        assert(0);
    }
    return FALSE;
}
    ...
    int unit)
{
    ...
    if (er_thread_id != -1)
    {
        return ((0 == dnx_err_recovery_manager[unit][er_thread_id].is_suppressed_counter)
                &&
                (err_recovery_flag_is_on
    ...
    LOG_CLI((BSL_META("4. Check if access_info_set for EM table is rollbackable.\n")));
    {
        ...
        SHR_IF_ERR_EXIT(dbal_tables_table_create
                        (unit, DBAL_ACCESS_METHOD_MDB, DBAL_TABLE_TYPE_EM, DBAL_CORE_MODE_SBC, nof_key_fields,
                         keys_info_p, nof_result_fields, results_info_p, 0, NULL, &table_id));
        /*
         * Save the table info. 
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SHR_IF_ERR_EXIT(dbal_entry_handle_access_id_get(unit, entry_handle_id, &access_ids[0]));
    ...
    for (jj = 2; jj < 4; jj++)
    {
        ...
        SHR_IF_ERR_EXIT(dbal_entry_attribute_set(unit, entry_handle_id, DBAL_ENTRY_ATTR_PRIORITY, 7));
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
        SHR_IF_ERR_EXIT(dbal_entry_handle_access_id_get(unit, entry_handle_id, &access_ids[access_id_counter]));
    ...
    uint8 should_have_er)
{
    ...
    if (er_thread_id == -1)
    {
        SHR_ERR_EXIT(_SHR_E_FAIL, "No thread valid er thread id was found!\n");
    }
    /*
     * pretend this we are inside of a BCM API
     */
    dnx_err_recovery_manager[unit][er_thread_id].api_counter = 1;
#endif /* DNX_ERR_RECOVERY_VALIDATION */
...
#ifdef DNX_ERR_RECOVERY_VALIDATION_BASIC
    if (dnx_err_recovery_manager[unit][er_thread_id].api_counter != 0)
    {
        dnx_err_recovery_manager[unit][er_thread_id].api_counter = 0;
    }
#endif /* DNX_ERR_RECOVERY_VALIDATION */
...
{
    int er_thread_id;
    int transaction_registered_in_test = FALSE;
    SHR_FUNC_INIT_VARS(unit);

    /*
     * register transaction if an er_thread_id does not exist already
     */
    er_thread_id = dnx_er_threading_transaction_get(unit);
    if (er_thread_id == -1)
    {
        dnx_er_threading_transaction_register(unit);
        transaction_registered_in_test = TRUE;
    }
    else
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "An er_thread_id already exists, while it shouldn't.\n");
    }
...
exit:
     ...
     * free er_thread_id
     */
    if (transaction_registered_in_test == TRUE)
    {
    ...
    dnx_err_recovery_test_thread_t data)
{
    ...
    if (data.opts_in)
    {
         ...
         * get thread id
         */
        er_thread_id = dnx_er_threading_transaction_get(data.unit);
        if (er_thread_id == -1)
        {
            SHR_ERR_EXIT(_SHR_E_FAIL, "No valid er thread id was found!\n");
        }

        if (dnx_err_recovery_manager[data.unit][er_thread_id].entry_counter != 0)
        {
            SHR_ERR_EXIT(_SHR_E_FAIL, "Entries exist at the beginning of the the tranasction.\n");
        }
        /*
         * Saving initial swstate values, before any changes are made
         */
        SHR_IF_ERR_EXIT(example.dynamic_array.get(data.unit, er_thread_id, &pre_rb_swstate_var[idx]));
    }
    else
    {
        LOG_CLI((BSL_META("NO SUPPORT BEGIN.\n")));
        dnx_err_recovery_transaction_no_support_counter_inc(data.unit, __func__);
        er_thread_id = dnx_er_threading_transaction_get(data.unit);
        if (er_thread_id == -1)
        {
            SHR_ERR_EXIT(_SHR_E_FAIL, "No valid er thread id was found!\n");
    ...
    LOG_CLI((BSL_META("Entry count in transaction %u\n"),
             dnx_err_recovery_manager[data.unit][er_thread_id].entry_counter));
    LOG_CLI((BSL_META("Changing swstate %u times...\n"), changes_count));

    /**Save the pre-rollback value for the current thread */
    SHR_IF_ERR_EXIT(example.dynamic_array.get(data.unit, er_thread_id, &pre_rb_swstate_var[er_thread_id]));

    for (idx = 0; idx < changes_count; idx++)
    {
        SHR_IF_ERR_EXIT(example.dynamic_array.set(data.unit, er_thread_id, idx));
    }
    LOG_CLI((BSL_META("Entry count in transaction %u\n"),
             dnx_err_recovery_manager[data.unit][er_thread_id].entry_counter));
     ...
     */
    if (data.opts_in && dnx_err_recovery_manager[data.unit][er_thread_id].entry_counter != changes_count)
    {
...
exit:
    ...
    if (data.opts_in)
    {
        ...
        if (er_thread_id != -1)
        {
            dnx_err_recovery_transaction_end(data.unit, bcmModuleCount, TRUE);
            /*
             * Save the post-rollback value for the current thread.
             */
            SHR_IF_ERR_EXIT(example.dynamic_array.get(data.unit, er_thread_id, &post_rb_swstate_var[er_thread_id]));
        }
    ...
    void *data_ptr)
{
    ...
    data = ((dnx_err_recovery_test_thread_t *) data_ptr)[0];

    /*
     * no need to inc / dec the API counters. BCM API verification is not being enforced.
     */
    SHR_IF_ERR_EXIT(err_recovery_thread_test_dummy_dnx_api(data));
    ...
    dnx_err_recovery_test_thread_t bcmERTest_data)
{
    ...
    shr_thread_manager_handler_t er_tid = NULL;
    ...
    thread_config_info.name = "bcmERTest";
    thread_config_info.bsl_module = BSL_LOG_MODULE;
    thread_config_info.type = SHR_THREAD_MANAGER_TYPE_RUN_TO_COMPLETION;
    thread_config_info.callback = err_recovery_thread_cb;
    thread_config_info.user_data = &bcmERTest_data;
    thread_config_info.thread_priority = thread_priority;
    thread_config_info.joinable = TRUE;
    SHR_IF_ERR_EXIT(shr_thread_manager_create(main_data.unit, &thread_config_info, &er_tid));

    SHR_IF_ERR_EXIT(err_recovery_thread_test_dummy_dnx_api(main_data));

    SHR_IF_ERR_EXIT(shr_thread_manager_join(&er_tid, 5 * SECOND_USEC));
    /*
     * Check if the swstate variable has been rollbacked correctly.
     */
    if (main_data.opts_in && bcmERTest_data.opts_in)
    {
        for (int er_thread_id = 0; er_thread_id < DNX_ERR_RECOVERY_MAX_NOF_ACTIVE_TRANSACTION; er_thread_id++)
        {
            if (post_rb_swstate_var[er_thread_id] != pre_rb_swstate_var[er_thread_id])
            {
                LOG_CLI((BSL_META_U(main_data.unit, "pre rb swstate array: %u for thread %d\n"),
                         pre_rb_swstate_var[er_thread_id], er_thread_id));
                LOG_CLI((BSL_META_U(main_data.unit, "post rb swstate array: %u for thread %d\\n"),
                         post_rb_swstate_var[er_thread_id], er_thread_id));
                SHR_ERR_EXIT(_SHR_E_FAIL, "Pre/post rollback values do not match. Rollback was unsuccessfull\n");
            }
        }
    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SHR_IF_ERR_EXIT(example.dynamic_array.alloc(unit, DNX_ERR_RECOVERY_MAX_NOF_ACTIVE_TRANSACTION));
    ...
    LOG_CLI((BSL_META("\n3. Transaction on both threads.\n")));
    {
        ...
        /*
         * main thread API opts-in of ER
         * wait 2 seconds for "bcmERTest thread" to finish "[bcmERTest] TRANSACTION END" before executing "TRANSACTION END"
         */
        main_data.unit = unit;
        main_data.opts_in = TRUE;
        main_data.sec_sleep_start = 0;
        ...
        /*
         * bcmERTest thread API opts-in of ER,
         * wait 1 second for "main thread" to finish "TRANSATION START" before executing "[bcmERTest] TRANSACTION START"
         */
        bcmERTest_data.unit = unit;
        bcmERTest_data.opts_in = TRUE;
        bcmERTest_data.sec_sleep_start = 1;
        ...
        /*
         * main thread API opts-in of ER,
         * wait 2 seconds for "bcmERTest thread" to finish "TRANSACTION START" before executing "TRANSACTION END"
         */
        main_data.unit = unit;
        main_data.opts_in = TRUE;
        main_data.sec_sleep_start = 0;
        ...
        /*
         * bcmERTest thread API opts-in of ER,
         * wait 1 second for "main thread" to finish "TRANSATION START" before executing "[cmERTest] TRANSACTION START"
         * wait 2 second for "main thread" to finish "TRANSACTION END" before executing "[cmERTest] TRANSACTION END"
         */
        bcmERTest_data.unit = unit;
        bcmERTest_data.opts_in = TRUE;
        bcmERTest_data.sec_sleep_start = 1;
    ...
    sh_sand_control_t * sand_control)
{
    ...
    int value1 = 10;
    uint8 success = 0;
    int er_thread_id, transaction_registered_in_test;

    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);

    transaction_registered_in_test = FALSE;
    ...
    LOG_CLI((BSL_META("Running ECMP error recovery scenario...\n")));
    {
        ...
        SHR_IF_ERR_EXIT(algo_l3_db.fec_db_info.
                        fec_db.get(unit, DNX_L3_FEC_DIRECTION_NA, DBAL_ENUM_FVAL_HIERARCHY_LEVEL_HIERARCHY_LEVEL_1,
                                   &fec_db));
        SHR_IF_ERR_EXIT(mdb_init_fec_ranges_allocation_info_get(unit, fec_db, &fec, &fec_range_size));
        /*
