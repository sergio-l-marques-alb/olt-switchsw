    ...
    bcm_gport_t gport_sub_type)
{
    ...
    bcm_if_t rif = 0;
    bcm_gport_t lif = 0, default_gport = BCM_GPORT_INVALID;
    ...
    /** Check calling get function returns the default gport */
    SHR_IF_ERR_EXIT(bcm_rx_trap_lif_get(unit, &lif_config_get));

    SHR_IF_ERR_EXIT(ctest_dnx_rx_trap_compare_values(unit, &default_gport, &lif_config_get.action_gport,
                                                     sizeof(bcm_gport_t), "default_action_gport"));
    ...
    int unit)
{
    ...
    LOG_INFO(BSL_LS_APPL_COMMON, (BSL_META_U(unit, "-------------------------------------------------------------\n")));
    LOG_INFO(BSL_LS_APPL_COMMON,
             (BSL_META_U(unit, "SUPPLYING non-irpp trap for LifTypeInLif to bcm_rx_trap_lif_set \n")));
    LOG_INFO(BSL_LS_APPL_COMMON, (BSL_META_U(unit, "-------------------------------------------------------------\n")));

    bcm_rx_trap_lif_config_t_init(&lif_config);

    lif_config.lif_gport = lif;
    BCM_GPORT_TRAP_SET(lif_config.action_gport, BCM_RX_TRAP_EG_TX_TRAP_ID_DROP, 7, 0);
    lif_config.lif_type = bcmRxTrapLifTypeInLif;
    ...
    LOG_INFO(BSL_LS_APPL_COMMON, (BSL_META_U(unit, "-------------------------------------------------------------\n")));
    LOG_INFO(BSL_LS_APPL_COMMON,
             (BSL_META_U(unit, "SUPPLYING non-etpp trap for LifTypeOutLif to bcm_rx_trap_lif_set \n")));
    LOG_INFO(BSL_LS_APPL_COMMON, (BSL_META_U(unit, "-------------------------------------------------------------\n")));
    ...
    int unit)
{
    ...
    LOG_INFO(BSL_LS_APPL_COMMON, (BSL_META_U(unit, "-------------------------------------------------------------\n")));
    LOG_INFO(BSL_LS_APPL_COMMON, (BSL_META_U(unit, "                ETPP ARP outlif                         \n")));
    LOG_INFO(BSL_LS_APPL_COMMON, (BSL_META_U(unit, "-------------------------------------------------------------\n")));

    bcm_l3_egress_t_init(&l3eg);

    l3eg.encap_id = 0;
    BCM_L3_ITF_LIF_TO_GPORT_TUNNEL(gport, l3eg.encap_id);

    l3eg.vlan = 20;
    flags = BCM_L3_EGRESS_ONLY | BCM_L3_WITH_ID;

    SHR_IF_ERR_EXIT(bcm_l3_egress_create(0, flags, &l3eg, &l3egid_null));
    ...
    SHR_IF_ERR_EXIT(bcm_rx_trap_lif_set(0, 0, &lif_config));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (sal_strncasecmp
        (test_type, CTEST_DNX_RX_TRAPS_LIF_TEST_TYPE_POSITIVE, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
    {
        ...
        else
        {
            SHR_IF_ERR_EXIT(appl_dnx_rx_trap_lif_pos(unit, test_mode, BCM_GPORT_SUB_TYPE_LIF));
             ...
             (test_type, CTEST_DNX_RX_TRAPS_LIF_TEST_TYPE_NEGATIVE, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
    {
        SHR_IF_ERR_EXIT(appl_dnx_rx_trap_lif_neg(unit));
    }
    else if (sal_strncasecmp
             (test_type, CTEST_DNX_RX_TRAPS_LIF_TEST_TYPE_EXHAUSTIVE,
