    ...
    bcm_gport_t lif_gport)
{
    ...
    for (config_iter = 0; config_iter < 2; config_iter++)
    {
        ...
        {
            SHR_IF_ERR_EXIT(dnx_rx_trap_type_map_info_get(unit, trap_type_list[trap_iter], &predefined_trap_info));
            ...
            {
                trap_configured_on_init[trap_iter] = TRUE;
                continue;
            }
    ...
    LOG_INFO(BSL_LS_APPL_COMMON, (BSL_META_U(unit, "INGRESS PREDEFINED TRAP CONFIGURATION Create ALL!\n")));

    for (trap_iter = 0; trap_iter < nof_traps; trap_iter++)
    {
        SHR_IF_ERR_EXIT(dnx_rx_trap_type_map_info_get(unit, trap_type_list[trap_iter], &predefined_trap_info));
        dnx_rx_trap_is_trap_id_allocated(unit, predefined_trap_info.trap_id, &is_alloc);
        if (trap_configured_on_init[trap_iter] == TRUE)
        {
            continue;
        }
    ...
    /** Count number of free user defined traps available */
    for (trap_iter = 0; trap_iter < nof_ud_traps; trap_iter++)
    {
        SHR_IF_ERR_EXIT(dnx_rx_trap_map_ingress_ud_alloc_get(unit, trap_iter, &trap_id));
    ...
    LOG_INFO(BSL_LS_APPL_COMMON, (BSL_META_U(unit, "INGRESS USERDEFINED TRAP CONFIGURATION Create ALL!\n")));
    ...
    LOG_INFO(BSL_LS_APPL_COMMON, (BSL_META_U(unit, "INGRESS TRAP CONFIGURATION MC destination!\n")));

    SHR_IF_ERR_EXIT(bcm_rx_trap_type_create(unit, 0, bcmRxTrapLinkLayerSaEqualsDa, &trap_id));

    /** Check multicast destination configuration */
    bcm_rx_trap_config_t_init(&trap_config);
    trap_config.flags = BCM_RX_TRAP_UPDATE_DEST | BCM_RX_TRAP_DEST_MULTICAST;
    trap_config.dest_group = 123;
    trap_config.trap_strength = 5;
    SHR_IF_ERR_EXIT(bcm_rx_trap_set(unit, trap_id, &trap_config));

    /** dest_group always returns encoded as MCAST gport */
    BCM_GPORT_MCAST_SET(trap_config.dest_group, trap_config.dest_group);
    SHR_IF_ERR_EXIT(bcm_rx_trap_get(unit, trap_id, &trap_config_get));
    SHR_IF_ERR_EXIT(ctest_dnx_rx_trap_ingress_compare(unit, trap_id, &trap_config, &trap_config_get));
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(dnx_rx_trap_type_map_info_get(unit, bcmRxTrapMplsUnexpectedBos, &predefined_trap_info));
    ...
    rv = (bcm_error_t) dnx_rx_trap_type_to_name(unit, (bcmRxTrapCount + 5), trap_name);
    if (rv != BCM_E_PARAM)
    {
        LOG_ERROR(BSL_LS_APPL_COMMON,
    ...
    rv = (bcm_error_t) dnx_rx_trap_type_to_name(unit, bcmRxTrapReserved, trap_name);
    if (rv != BCM_E_UNAVAIL)
    {
        LOG_ERROR(BSL_LS_APPL_COMMON,
                  (BSL_META_U(unit, "dnx_rx_trap_type_to_name should fail with BCM_E_PARAM! . Error:%d (%s)\n"), rv,
...
/**
 * \brief
 *   Negative testing for user defined traps
 *
 * \param [in] unit - The unit number.
 *
 * \return
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (sal_strncasecmp
        (test_type, CTEST_DNX_RX_TRAPS_INGRESS_OPTION_TEST_POSITIVE, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
    {
        if (sal_strncasecmp
            (test_mode, CTEST_DNX_RX_TRAPS_INGRESS_OPTION_MODE_PREDEFINED,
             SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
        {
            SHR_IF_ERR_EXIT(appl_dnx_rx_trap_ingress_sem_predefined(unit, lif_gport));
                 ...
                 (test_mode, CTEST_DNX_RX_TRAPS_INGRESS_OPTION_MODE_OAM,
                  SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
        {
            SHR_IF_ERR_EXIT(appl_dnx_rx_trap_ingress_sem_oam(unit, lif_gport));
        }
        else if (sal_strncasecmp
                 (test_mode, CTEST_DNX_RX_TRAPS_INGRESS_OPTION_MODE_GET,
                  SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
        {
        ...
        else
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Mode indicated is unsupported\n Supported modes (mode=): %s, %s, %s\n ",
             ...
             (test_type, CTEST_DNX_RX_TRAPS_INGRESS_OPTION_TEST_NEGATIVE,
              SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
    {
        if (sal_strncasecmp
            (test_mode, CTEST_DNX_RX_TRAPS_INGRESS_OPTION_MODE_PREDEFINED,
             SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
        {
            SHR_IF_ERR_EXIT(appl_dnx_rx_trap_ingress_sem_neg_predefined(unit));
                 ...
                 (test_mode, CTEST_DNX_RX_TRAPS_INGRESS_OPTION_MODE_USERDEFINED,
                  SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
        {
            SHR_IF_ERR_EXIT(appl_dnx_rx_trap_ingress_sem_neg_userdefined(unit));
        }
        else if (sal_strncasecmp
                 (test_mode, CTEST_DNX_RX_TRAPS_INGRESS_OPTION_MODE_ACTIONS,
                  SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
        {
...
exit:
    ...
    {
        if (BCM_GPORT_IS_MPLS_PORT(lif_gport))
        {
            SHR_IF_ERR_EXIT(ctest_dnx_rx_trap_lif_destroy_example(unit, &lif_gport));
        }
    }
