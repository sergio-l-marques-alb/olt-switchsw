    ...
    char *property_value)
{
    ...
    if ((ISEMPTY(cur_value) && ISEMPTY(property_value)) ||
        ((cur_value != NULL) && (property_value != NULL)
         && !sal_strncasecmp(cur_value, property_value, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH)))
    {
        SHR_EXIT();
    }
    ...
    char *soc_value)
{
     ...
     */
    while (id < MAX_NUM_SOC_PROPERTIES && !ISEMPTY(ctest_soc_property_p[id].property))
    {
        id++;
    }
    ctest_soc_property_p[id].property = soc_name;
    ctest_soc_property_p[id].value = soc_value;
    id++;

    if (id >= MAX_NUM_SOC_PROPERTIES)
    {
        SHR_ERR_EXIT(_SHR_E_MEMORY,
                     "Number of soc properties to change (%d) is larger than num elements on container (%d). Quit\r\n",
                     id, MAX_NUM_SOC_PROPERTIES);
    }
    ctest_soc_property_p[id].property = NULL;
exit:
    SHR_FUNC_EXIT;
}
/* } */
    ...
    rhhandle_t ctest_soc_property_set_h)
{
    ...
    SHR_CLI_EXIT_IF_ERR(ctest_dnxc_reset_soc_properties_no_init(unit, ctest_soc_property_set_h), "");
...
shr_error_e
ctest_dnxc_reset_soc_properties_no_init(
    int unit,
    rhhandle_t ctest_soc_property_set_h)
{
    ctest_soc_entry_t *property_entry;
    ...
    char *cint_name)
{
    ...
    if (!dbx_file_exists(cint_name))
    {
         ...
         */
        err = dbx_file_find_sdk_path(sdk_location);
        if (err != _SHR_E_NONE)
        {
            if (err == _SHR_E_NOT_FOUND)
            {
                SHR_CLI_EXIT(err, "No SDK directory was found while looking for %s\n", cint_name);
            }
            SHR_CLI_EXIT(err, "dbx_file_find_sdk_path returned error:'%s'\n", _SHR_ERRMSG(err));
        }
        sal_strncat_s(sdk_location, "/", sizeof(sdk_location));

        sal_strncat_s(sdk_location, "src/examples/", sizeof(sdk_location));
        sal_strncat_s(sdk_location, cint_name, sizeof(sdk_location));
        if (!dbx_file_exists(sdk_location))
        {
            ...
            if (!dbx_file_exists(sdk_location))
            {
                ...
                dbx_file_find_sdk_path(sdk_location);
                sal_strncat_s(sdk_location, "/examples/", sizeof(sdk_location));
                sal_strncat_s(sdk_location, cint_name, sizeof(sdk_location));
                if (!dbx_file_exists(sdk_location))
                {
    ...
    int nof_arguments)
{
    ...
    {
        str_len = sal_strnlen(cint_str, sizeof(cint_str));
