 ...
 */
static void
diag_dnx_oamp_v1_test_generate_ccm_params(
    int unit,
 ...
 */
static void
diag_dnx_oamp_test_generate_bfd_oamp_v1_params(
    int unit,
    int for_modification,
    dnx_bfd_oamp_v1_endpoint_t * bfd_entry_data,
    diag_dnx_oamp_semantic_test_struct * test,
    uint32 *entry_id)
{
...
static shr_error_e
diag_dnx_oamp_test_compare_bfd_oamp_v1_info_structs(
    int unit,
    dnx_bfd_oamp_v1_endpoint_t * bfd_entry_data,
    dnx_bfd_oamp_v1_endpoint_t * bfd_entry_data2)
{
    SHR_FUNC_INIT_VARS(unit);
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (DNX_OAMP_IS_V1(unit))
    {
        ...
        exp_op0_cnt = 0;
        exp_op1_cnt = 0;
        rv = dnx_oam_oamp_v1_scan_count_calc(unit, endpoint_id, 0, is_short_entry, ccm_rate, ccm_rx_without_tx, opcode,
                                             &mep_profile_data);
    ...
    sh_sand_control_t * sand_control)
{
    ...
    uint8 *rmep_type = NULL;
    dnx_oam_oamp_ccm_endpoint_t *ccm_entry_data = NULL, *ccm_entry_data2 = NULL;
    dnx_oam_oamp_rmep_db_entry_t *oam_oamp_rmep_entry_data = NULL, *oam_oamp_rmep_entry_data2 = NULL;
#ifdef BCM_DNX2_SUPPORT
/* { */
    dnx_bfd_oamp_v1_endpoint_t *bfd_oamp_v1_entry_data = NULL, *bfd_oamp_v1_entry_data2 = NULL;
...
/* } */
    ...
    if (entry_type == ENTRY_TYPE_CCM)
    {
        {
#ifdef BCM_DNX2_SUPPORT
            SHR_ALLOC_SET_ZERO_ERR_EXIT(ccm_entry_data, sizeof(dnx_oam_oamp_ccm_endpoint_t),
                                        "Struct for creating OAMP CCM entries", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
            SHR_ALLOC_SET_ZERO_ERR_EXIT(ccm_entry_data2, sizeof(dnx_oam_oamp_ccm_endpoint_t),
                                        "Struct for reading OAMP CCM entries", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
#endif
        }

        if (_SHR_IS_FLAG_SET(flags, OAM_WITH_LM_ENTRY))
        {
            SHR_ALLOC_SET_ZERO_ERR_EXIT(lm_entry_data, sizeof(dnx_oam_oamp_endpoint_lm_params_t),
                                        "Struct for creating OAMP LM entries", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
            SHR_ALLOC_SET_ZERO_ERR_EXIT(lm_entry_data2, sizeof(dnx_oam_oamp_endpoint_lm_params_t),
                                        "Struct for reading OAMP LM entries", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
        }

        if (_SHR_IS_FLAG_SET(flags, OAM_WITH_DM_ENTRY))
        {
            SHR_ALLOC_SET_ZERO_ERR_EXIT(dm_entry_data, sizeof(dnx_oam_oamp_endpoint_dm_params_t),
                                        "Struct for creating OAMP DM entries", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
            SHR_ALLOC_SET_ZERO_ERR_EXIT(dm_entry_data2, sizeof(dnx_oam_oamp_endpoint_dm_params_t),
...
#ifdef BCM_DNX2_SUPPORT
/* { */
                SHR_ALLOC_SET_ZERO_ERR_EXIT(bfd_oamp_v1_entry_data, sizeof(dnx_bfd_oamp_v1_endpoint_t),
                                            "Struct for creating OAMP BFD entries", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
                SHR_ALLOC_SET_ZERO_ERR_EXIT(bfd_oamp_v1_entry_data2, sizeof(dnx_bfd_oamp_v1_endpoint_t),
                                            "Struct for reading OAMP BFD entries", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
/* } */
#endif
            }
        }
        ...
        {
            if (entry_type == ENTRY_TYPE_RMEP)
            {
#ifdef BCM_DNX2_SUPPORT
                if (DNX_OAMP_IS_V1(unit))
                {
                    /*
                     * No RMEP test for OAMP V1 
                     */
                    SHR_EXIT();
                }
#endif
                SHR_ALLOC_SET_ZERO_ERR_EXIT(oam_oamp_rmep_entry_data, sizeof(dnx_oam_oamp_rmep_db_entry_t),
                                            "Struct for creating OAMP RMEP entries", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
                SHR_ALLOC_SET_ZERO_ERR_EXIT(oam_oamp_rmep_entry_data2, sizeof(dnx_oam_oamp_rmep_db_entry_t),
                                            "Struct for reading OAMP RMEP entries", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
                SHR_ALLOC_SET_ZERO_ERR_EXIT(rmep_type, num_entries, "Array of RMEP types", "%s%s%s\r\n", EMPTY, EMPTY,
                                            EMPTY);
            }
        }
    }

    SHR_ALLOC_SET_ZERO_ERR_EXIT(entry_id, num_entries * sizeof(uint32), "Array of entry IDs", "%s%s%s\r\n", EMPTY,
                                EMPTY, EMPTY);


    SHR_ALLOC_SET_ZERO_ERR_EXIT(test, sizeof(diag_dnx_oamp_semantic_test_struct), "Struct for test variables",
                                "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
        ...
        {
            {
                 ...
                 */
                SHR_NULL_CHECK(ccm_entry_data, _SHR_E_INTERNAL, "ccm_entry_data");
                ccm_entry_data->flags = 0;
                diag_dnx_oamp_v1_test_generate_ccm_params(unit, FALSE, ccm_entry_data, test, &entry_id[entry_index]);
                ...
                SHR_CLI_EXIT_IF_ERR(dnx_oam_oamp_v1_mep_db_ccm_endpoint_get
                                    (unit, entry_id[entry_index], 0, ccm_entry_data2),
                ...
                ccm_entry_data->flags = DNX_OAMP_OAM_CCM_MEP_UPDATE |
                    (ccm_entry_data->flags &
                     (DNX_OAMP_OAM_CCM_MEP_LM_DM_OFFLOADED | DNX_OAMP_OAM_CCM_MEP_Q_ENTRY |
                      DNX_OAMP_OAM_CCM_MEP_IS_UPMEP));
                diag_dnx_oamp_v1_test_generate_ccm_params(unit, TRUE, ccm_entry_data, test, &entry_id[entry_index]);
...
#ifdef BCM_DNX2_SUPPORT
            if (DNX_OAMP_IS_V1(unit) && _SHR_IS_FLAG_SET(flags, OAM_WITH_LM_ENTRY))
            {
                ...
                /** Create LM entry */
                SHR_CLI_EXIT_IF_ERR(dnx_oamp_v1_mep_db_lm_endpoint_set(unit, entry_id[entry_index], lm_entry_data),
                                    "Test failed.  Could not create LM entry\n");
                LOG_CLI((BSL_META("Added LM entry #%d\n"), entry_index + 1));

                /** Read entry parameters and compare   */
                SHR_CLI_EXIT_IF_ERR(dnx_oamp_v1_mep_db_lm_endpoint_get(unit, entry_id[entry_index], lm_entry_data2,
            ...
            if (DNX_OAMP_IS_V1(unit) && _SHR_IS_FLAG_SET(flags, OAM_WITH_DM_ENTRY))
            {
                ...
                /** Create DM entry */
                SHR_CLI_EXIT_IF_ERR(dnx_oamp_v1_mep_db_dm_endpoint_set(unit, entry_id[entry_index], dm_entry_data),
                                    "Test failed.  Could not create DM entry\n");
                LOG_CLI((BSL_META("Added DM entry #%d\n"), entry_index + 1));

                /** Read entry parameters and compare   */
                dm_entry_data2->lmm_dmm_da_profile = 0;
                SHR_CLI_EXIT_IF_ERR(dnx_oamp_v1_mep_db_dm_endpoint_get
...
/* { */
                    ...
                    diag_dnx_oamp_test_generate_bfd_oamp_v1_params(unit, FALSE, bfd_oamp_v1_entry_data, test,
                                                                   &entry_id[entry_index]);

                    endpoint_info.id = entry_id[entry_index];
                    /** Create BFD entry */
                    SHR_CLI_EXIT_IF_ERR(dnx_bfd_oamp_v1_mep_db_endpoint_set
                                        (unit, &endpoint_info, bfd_oamp_v1_entry_data),
                                        "Test failed.  Could not create BFD entry\n");
                    LOG_CLI((BSL_META("Added BFD entry #%d\n"), entry_index + 1));

                    /** Read entry parameters and compare   */
                    SHR_CLI_EXIT_IF_ERR(dnx_bfd_oamp_v1_mep_db_endpoint_get
                                        (unit, entry_id[entry_index], bfd_oamp_v1_entry_data2),
                                        "Test failed.  BFD entry could not be read.\n");

                    LOG_CLI((BSL_META("Checking values for BFD entry #%d\n"), entry_index + 1));
                    SHR_CLI_EXIT_IF_ERR(diag_dnx_oamp_test_compare_bfd_oamp_v1_info_structs
                                        (unit, bfd_oamp_v1_entry_data, bfd_oamp_v1_entry_data2),
                                        "Test failed.  Parameters read did not match parameters written.\n");
                    LOG_CLI((BSL_META("Read BFD entry #%d\n"), entry_index + 1));

                    /** Modify BFD entry   */
                    bfd_oamp_v1_entry_data->flags |= DNX_OAMP_BFD_MEP_UPDATE;
                    diag_dnx_oamp_test_generate_bfd_oamp_v1_params(unit, TRUE, bfd_oamp_v1_entry_data, test,
                                                                   &entry_id[entry_index]);
        ...
        {
            {
                ...
                if (_SHR_IS_FLAG_SET(flags, OAM_WITH_LM_ENTRY))
                {
                    SHR_CLI_EXIT_IF_ERR(dnx_oamp_v1_mep_db_lm_endpoint_clear(unit, entry_id[entry_index], 0),
                                        "Test failed.  Could not delete LM entry.\n");
                    LOG_CLI((BSL_META("Deleted LM entry #%d\n"), entry_index + 1));
                }
                if (_SHR_IS_FLAG_SET(flags, OAM_WITH_DM_ENTRY))
                {
                    SHR_CLI_EXIT_IF_ERR(dnx_oamp_v1_mep_db_dm_endpoint_clear(unit, entry_id[entry_index], 0),
...
#ifdef BCM_DNX2_SUPPORT
/* { */ uint8 is_ipv6 = 0;
                    SHR_CLI_EXIT_IF_ERR(dnx_bfd_oamp_v1_mep_db_endpoint_clear
                                        (unit, entry_id[entry_index], is_ipv6),
...
#ifdef BCM_DNX2_SUPPORT
/* { */
    SHR_FREE(bfd_oamp_v1_entry_data);
    SHR_FREE(bfd_oamp_v1_entry_data2);
/* } */
#endif
    ...
    SHR_FREE(entry_id);
    SHR_FREE(rmep_type);
    SHR_FREE(oam_oamp_rmep_entry_data);
    SHR_FREE(oam_oamp_rmep_entry_data2);
    SHR_FREE(test);
    SHR_FUNC_EXIT;
}
...
static sh_sand_invoke_t diag_dnx_oamp_sem_tests[] = {
    ...
    {"eps_4_rmep", "Number=4 Type=RMEP FLaGs=NONE"},
    {"eps_6_rmep", "Number=6 Type=RMEP FLaGs=NONE"},
    {NULL}
};
