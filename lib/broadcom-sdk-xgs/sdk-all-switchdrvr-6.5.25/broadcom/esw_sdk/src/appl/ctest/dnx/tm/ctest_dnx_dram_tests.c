...
#define DNX_DRAM_TEST_STUB_ARR_LEN                 (512)
...
static const ctest_dnx_dram_operation_buffer_params_t Default_buffer_params = {
    ...
    .expected_rate_deviation_in_percents = {7, 7, 7, 7, 7, 7, 0},
 ...
 *   CGE  ==> 100G
 *
 * Parameters that are different between devices are configured in diag_dnx_dram_rate_defaults_load.
 *
 * The expected dram rate results are empirical.
 * The results may vary according to number of ports and ports
 * type.
 *
 * We run a snake of 98.5G between ports, and try to reach the maximum
 * bandwidth the dram can hold (if we will overload the dram we will
 * encounter in "dram block" which will reduce the bandwidth to a lower
 * number).
 * We use 200G ports in the snake even though we run only 98.5G, because
 * the SAT is currently bursty and we don't want it to affect us.
 */
static const ctest_dnx_dram_operation_rate_params_t Default_rate_params = {
    .ctest_dnx_dram_operation_rate_cycles = 1,
    .ctest_dnx_dram_num_port_in_snake = 0,
    .ctest_dnx_num_phys_ports_in_one_combined_port = 4,
    .ctest_dnx_text_ports = NULL,
    .ctest_dnx_use_dram_only = {TRUE},
    .ctest_dnx_apply_debug_severity = FALSE,
    .sat_injection_rate_kbps = {98500000},
    .sat_packet_size_bytes = {512},
    .sat_burst_size = {1000},
    .sat_granularity_size = {2000000},
    .delay_before_inspect_in_milli = {10000},
    .num_sec_to_wait_for_rate = {20},
    .expected_rate_in_mbps = {0},
    .expected_rate_deviation_in_percents = {6},
    .verify_clear_at_end_of_cycle = {FALSE},
    .expect_dram_to_be_used = {TRUE},
    .expected_dram_rate_in_mbps = {0},
    .expected_dram_rate_deviation_in_percents = {5},
    .expect_enq_deq_dram = {TRUE}
};
    ...
    ctest_dnx_dram_operation_rate_params_t * rate_params_p)
{
    ...
    *rate_params_p = Default_rate_params;
    ...
    rate_params_p->ctest_dnx_dram_num_port_in_snake = dnx_data_dram.ctests.rate_get(unit)->nof_lanes_in_snake;
    rate_params_p->ctest_dnx_text_ports = dnx_data_dram.ctests.rate_get(unit)->snake_ports_type;
    rate_params_p->expected_rate_in_mbps[0] = dnx_data_dram.ctests.rate_get(unit)->expected_rate_in_mbps;
    rate_params_p->expected_dram_rate_in_mbps[0] = dnx_data_dram.ctests.rate_get(unit)->expected_dram_rate_in_mbps;
    ...
    dbal_enum_value_field_dram_use_mode_e dram_use_mode)
{
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_DRAM_BOUND_VOQ_SRAM_WORDS_QSIZE_TH, INST_SINGLE,
                                 (dram_use_mode == DBAL_ENUM_FVAL_DRAM_USE_MODE_DRAM_ONLY) ? 0 : 0x18);
...
/**
 * \brief
 *   Retrieve rate class of a port.
 * \param [in] unit -
 *   HW identifier of unit.
 * \param [in] port -
 *   Port in question.
 * \param [out] rate_class -
 *   Rate class associated with the required port.
 * \return
 *   Error code (as per 'shr_error_e').
 * \remark
 *   * None.
 ...
 */
static shr_error_e
ctest_dnx_dram_rate_class_from_port_get(
    int unit,
    bcm_port_t port,
    int *rate_class)
{
     ...
     * Get VOQ from port 
     */
    SHR_IF_ERR_EXIT(bcm_stk_gport_sysport_get(unit, port, &sysport));
    SHR_IF_ERR_EXIT(bcm_cosq_sysport_ingress_queue_map_get(unit, 0, sysport, &base_queue));
    BCM_GPORT_UNICAST_QUEUE_GROUP_SET(voq_gport, base_queue);

    /*
     * Get rate class from VOQ 
     */
    ...
    *rate_class = profile_mapping.mapped_profile;
...
/**
 * \brief
 *   Make rate_calss's dram bound always active.
 * \param [in] unit -
 *   HW identifier of unit.
 * \param [in] rate_class_id -
 *   Rate class to set dram bound configuration upon.
 * \return
 *   Error code (as per 'shr_error_e').
 * \remark
 *   * Set dram bound params to minimum.
 * \see
 *   shr_error_e
 */
static shr_error_e
ctest_dnx_dram_bound_rate_class_set(
    int unit,
    int rate_class_id)
{
    ...
    SHR_IF_ERR_EXIT(bcm_cosq_gport_fadt_threshold_set(unit, 0, &fadt_info, &fadt_threshold));
    ...
    SHR_IF_ERR_EXIT(bcm_cosq_gport_threshold_set(unit, rate_class_gport, 0, &dram_bound_recovery_threshold));
...
/**
 * \brief
 *   Make port's dram bound always active.
 * \param [in] unit -
 *   HW identifier of unit.
 * \param [in] port -
 *   Port to set dram bound configuration upon.
 * \return
 *   Error code (as per 'shr_error_e').
 * \remark
 *   * Set dram bound params to minimum.
    ...
    bcm_port_t port)
{
    ...
    /*
     * Get rate class from port 
     */
    SHR_IF_ERR_EXIT(ctest_dnx_dram_rate_class_from_port_get(unit, port, &rate_class));

    /*
     * Configure dram bound on the rate class 
     */
    SHR_IF_ERR_EXIT(ctest_dnx_dram_bound_rate_class_set(unit, rate_class));
...
/**
 * \brief
 *   Set rate_class's dram bound thresholds.
 * \param [in] unit -
 *   HW identifier of unit.
 * \param [in] rate_class_id -
 *   Rate class to set dram bound information.
 * \param [in] fadt_threshold -
 *   FADT threshold to set.
 * \param [in] dram_bound_recovery_threshold -
 *   Dram bound recovery threshold to set.
 * \return
 *   Error code (as per 'shr_error_e').
 * \remark
 *   * Used in order to restore the saved dram bound params after changing them.
 * \see
 *   shr_error_e
 */
static shr_error_e
ctest_dnx_dram_bound_rate_class_default_set(
    int unit,
    int rate_class_id,
    bcm_cosq_fadt_threshold_t * fadt_threshold,
    bcm_cosq_threshold_t * dram_bound_recovery_threshold)
...
/**
 * \brief
 *   Get rate_class's dram bound thresholds.
 * \param [in] unit -
 *   HW identifier of unit.
 * \param [in] rate_class_id -
 *   Rate class to get dram bound information.
 * \param [out] fadt_threshold -
 *   FADT threshold to get.
 * \param [out] dram_bound_recovery_threshold -
 *   Dram bound recovery threshold to get.
 * \return
 *   Error code (as per 'shr_error_e').
 * \remark
 *   * Used in order to save the dram bound params before changing them.
 * \see
 *   shr_error_e
 */
static shr_error_e
ctest_dnx_dram_bound_rate_class_default_get(
    int unit,
    int rate_class_id,
    bcm_cosq_fadt_threshold_t * fadt_threshold,
    bcm_cosq_threshold_t * dram_bound_recovery_threshold)
{
    ...
    fadt_info.thresh_type = bcmCosqFadtDramBound;
    fadt_info.resource_type = bcmResourceOcbPacketDescriptors;
    SHR_IF_ERR_EXIT(bcm_cosq_gport_fadt_threshold_get(unit, 0, &fadt_info, fadt_threshold));

    /** configure dram bound recovery info */
    dram_bound_recovery_threshold->type = bcmCosqThresholdBytes;
    dram_bound_recovery_threshold->flags = BCM_COSQ_THRESHOLD_INGRESS | BCM_COSQ_THRESHOLD_QSIZE_RECOVERY;
    SHR_IF_ERR_EXIT(bcm_cosq_gport_threshold_get(unit, rate_class_gport, 0, dram_bound_recovery_threshold));
...
/**
 * \brief
 *   Set port's dram bound thresholds.
 * \param [in] unit -
 *   HW identifier of unit.
 * \param [in] port -
 *   Port to set dram bound information.
 * \param [in] fadt_threshold -
 *   FADT threshold to set.
 * \param [in] dram_bound_recovery_threshold -
 *   Dram bound recovery threshold to set.
 ...
 */
static shr_error_e
ctest_dnx_dram_bound_port_default_set(
    int unit,
    bcm_port_t port,
    bcm_cosq_fadt_threshold_t * fadt_threshold,
    bcm_cosq_threshold_t * dram_bound_recovery_threshold)
{
     ...
     * Configure dram bound on the rate class 
     */
    SHR_IF_ERR_EXIT(ctest_dnx_dram_bound_rate_class_default_set
                    (unit, rate_class, fadt_threshold, dram_bound_recovery_threshold));
...
/**
 * \brief
 *   Get port's dram bound thresholds.
 * \param [in] unit -
 *   HW identifier of unit.
 * \param [in] port -
 *   Port to get dram bound information.
 * \param [in] fadt_threshold -
 *   FADT threshold to get.
 * \param [in] dram_bound_recovery_threshold -
 *   Dram bound recovery threshold to get.
 ...
 */
static shr_error_e
ctest_dnx_dram_bound_port_default_get(
    int unit,
    bcm_port_t port,
    bcm_cosq_fadt_threshold_t * fadt_threshold,
    bcm_cosq_threshold_t * dram_bound_recovery_threshold)
{
    ...
    /*
     * Get dram bound of the rate class 
     */
    SHR_IF_ERR_EXIT(ctest_dnx_dram_bound_rate_class_default_get
                    (unit, rate_class, fadt_threshold, dram_bound_recovery_threshold));
    ...
    *sram_enq_rate_in_mega_bits_per_sec_p = *sram_deq_rate_in_mega_bits_per_sec_p = 0;
    {
        ...
        *dram_read_rate_in_mega_bits_per_sec_p = *dram_write_rate_in_mega_bits_per_sec_p = 0;
        ...
        SHR_IF_ERR_EXIT(sh_dnxc_diag_block_gtimer_start
                        (unit, tdu_nof_block, tdu_block_ids, gtimer_period_in_sec, DIAG_DNXC_TIME_UNIT_SEC));
    ...
    }
    {
        ...
        *sram_enq_rate_in_mega_bits_per_sec_p = *sram_deq_rate_in_mega_bits_per_sec_p = 0;
        ...
        SHR_IF_ERR_EXIT(sh_dnxc_diag_block_gtimer_start
                        (unit, cgm_nof_block, cgm_block_ids, gtimer_period_in_sec, DIAG_DNXC_TIME_UNIT_SEC));
    ...
    sh_sand_control_t * sand_control)
{
     ...
     */
    {
        ...
        SHR_ALLOC_ERR_EXIT(from_port_p, sizeof(*from_port_p) * nof_cores, "DRAM testing", "%s%s%s", "from_port_p",
                           EMPTY, EMPTY);
        ...
        SHR_ALLOC_ERR_EXIT(to_port_p, sizeof(*to_port_p) * maximal_potential_num_phys, "DRAM testing", "%s%s%s",
                           "to_port_p", EMPTY, EMPTY);
        SHR_ALLOC_ERR_EXIT(to_port_core_p, sizeof(*to_port_core_p) * maximal_potential_num_phys, "DRAM testing",
                           "%s%s%s", "to_port_core_p", EMPTY, EMPTY);
        ...
        {
            sal_snprintf(bcm_command_line, sizeof(bcm_command_line) - 1, "config delete ucode_port_%d.*", nif_port);
             ...
             index_phys_port++, index_ucode_port++)
        {
            ...
            {
                SHR_IF_ERR_EXIT(dnx_algo_port_nif_phys_phy_to_core_id_get(unit, index_phys_port, &core_id));
                sal_snprintf(bcm_command_line, sizeof(bcm_command_line) - 1,
        ...
        sal_snprintf(bcm_command_line, sizeof(bcm_command_line) - 1, "config add port_init_speed_xe%s=10000",
                     collective_name);
        SHR_CLI_EXIT_IF_ERR(sh_process_command(unit, bcm_command_line),
                            "Failed on sh_process_command(), config add port\r\n");
        LOG_CLI_EX("%s %s%s%s\r\n", bcm_command_line, EMPTY, EMPTY, EMPTY);
        sal_snprintf(bcm_command_line, sizeof(bcm_command_line) - 1, "config add appl_enable_l2%s=0", collective_name);
        SHR_CLI_EXIT_IF_ERR(sh_process_command(unit, bcm_command_line),
    ...
    sh_sand_control_t * sand_control)
{
    ...
    bcm_pbmp_t nif_ports_bitmap;
    bcm_port_t nif_port;
    bcm_cosq_fadt_threshold_t original_fadt_threshold;
    bcm_cosq_threshold_t original_dram_bound_recovery_threshold;
    ...
    }
    {
        ...
        SHR_CLI_EXIT_IF_ERR(sh_process_command(unit, "tr 141"), "failed on tr 141\r\n");
        {
            ...
            LOG_DEBUG_EX(BSL_LOG_MODULE,
                         "Force connect ports %d->%d on core %d %s\r\n", from_port_p[core_under_test], port_under_test,
                         core_under_test, EMPTY);
            SHR_IF_ERR_EXIT(ctest_dnx_dram_bound_port_default_get
                            (unit, port_under_test, &original_fadt_threshold, &original_dram_bound_recovery_threshold));
            /*
             * Config for 'snake' in order to activate all ports
             * we assume the number of active ports is larger than '1'
             */
                 ...
                 cycle_index++)
            {
                ...
                {
                    /*
                     * Set system to work with dram only, and lower dram bound
                     * thershold to 0, so all traffic will run through dram.
                     */
                    ...
                    SHR_IF_ERR_EXIT(ctest_dnx_dram_bound_port_set(unit, port_under_test));
                }
                else
                {
                    ...
                    SHR_IF_ERR_EXIT(ctest_dnx_dram_bound_port_default_set
                                    (unit, port_under_test, &original_fadt_threshold,
                                     &original_dram_bound_recovery_threshold));
                }
                SHR_IF_ERR_EXIT(bcm_port_get
                                (unit, from_port_p[core_under_test], &flags, &interface_info, &mapping_info));
                SHR_IF_ERR_EXIT(diag_sand_packet_create(unit, &packet_h));
...
exit:
    ...
    /*
     * Protection: Note that it is possible to get here before 'to_port_p' has been loaded by
     * meaningful inf.
     */
    if (to_port_p != NULL)
    {
        SHR_IF_ERR_CONT(ctest_dnx_dram_bound_port_default_set
                        (unit, to_port_p[0], &original_fadt_threshold, &original_dram_bound_recovery_threshold));
    }
    diag_sand_packet_free(unit, packet_h);
    SHR_FREE(from_port_p);
    SHR_FREE(to_port_core_p);
     ...
     * Re-enable L2.
     */
    sal_snprintf(bcm_command_line, sizeof(bcm_command_line) - 1, "config add appl_enable_l2%s=1", collective_name);
    SHR_IF_ERR_CONT(sh_process_command(unit, bcm_command_line));
    LOG_CLI_EX("%s %s%s%s\r\n", bcm_command_line, EMPTY, EMPTY, EMPTY);
    /*
     * Run Deinit-Init DNX DATA - The SoC properties, above, are loaded upon init.
     */
    SHR_IF_ERR_CONT(sh_process_command(unit, "tr 141"));
    SHR_FUNC_EXIT;
}
    ...
    sh_sand_control_t * sand_control)
{
    ...
    create_info.rate = port_resource.speed / 1000;      /* convert Mbps to Gbps */
...
static sh_sand_invoke_t ctest_dnx_dram_operation_buffer_tests[] = {
    {"dram_buffer_1", "",  CTEST_POSTCOMMIT},
    {NULL}
};
/**
 * \brief
...
static sh_sand_invoke_t diag_dnx_dram_bound_rc_tests[] = {
    ...
    {"bound_on_res_type_buffers", "port=14 enable=1 rate_class=46 resource=bcmResourceOcbBuffers"},
    {NULL}
};
