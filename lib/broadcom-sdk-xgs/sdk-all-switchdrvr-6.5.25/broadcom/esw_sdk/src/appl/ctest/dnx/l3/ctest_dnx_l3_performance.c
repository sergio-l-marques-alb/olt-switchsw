 ...
 * {
 */
/** An enum for describing the run actions carried out in tests */
typedef enum dnx_l3_performance_run_type_e
{
    DNX_L3_PERF_RUN_TYPE_INSERT,
    DNX_L3_PERF_RUN_TYPE_UPDATE,
    DNX_L3_PERF_RUN_TYPE_DELETE,
    DNX_L3_PERF_RUN_NOF_TYPES
} dnx_l3_performance_run_type_t;
/*
 * }
 */
/*
 ...
 * {
 */
extern shr_error_e dnx_l3_performance_fec_ids_generate(
    int unit,
    int *fec_ids,
    uint32 nof_entries,
    int *hierarchy);
...
{
    int index;
    int mint = run_time[DNX_L3_PERF_RUN_TYPE_INSERT], maxt = run_time[DNX_L3_PERF_RUN_TYPE_INSERT], avgt =
        run_time[DNX_L3_PERF_RUN_TYPE_INSERT];
...
/** The API is currently supporting only LPM tables. */
static shr_error_e
    ...
    int run_time[3][100])
{
    ...
    if (is_ipmc)
    {
        ...
        use_optimized = TRUE;
        /** Support this API for ACL tables as well */
    ...
    for (run_index = 0; run_index < nof_runs; run_index++)
    {
        ...
        run_time[DNX_L3_PERF_RUN_TYPE_INSERT][run_index] = sal_time_usecs() - start_time;
    }
    ...
    if (action[1])
    {
        ...
        for (run_index = 0; run_index < nof_runs; run_index++)
        {
            ...
            run_time[DNX_L3_PERF_RUN_TYPE_UPDATE][run_index] = sal_time_usecs() - start_time;
        }
    ...
    if (action[2])
    {
        ...
        for (run_index = 0; run_index < nof_runs; run_index++)
        {
            ...
            run_time[DNX_L3_PERF_RUN_TYPE_DELETE][run_index] = sal_time_usecs() - start_time;
        }
    ...
    if (_is_ipmc_) { \
        ...
        if (_is_tcam_) { \
            ...
            if (_is_external_) { SHR_IF_ERR_EXIT(dbal_entry_attribute_set(_unit_, _entry_handle_id_, DBAL_ENTRY_ATTR_PRIORITY, 0)); } \
                ...
                else { SHR_IF_ERR_EXIT(dbal_entry_attribute_set(unit, _entry_handle_id_, DBAL_ENTRY_ATTR_PRIORITY, 0)); } \
            } \
        } \
    } \
    ...
    int *fec_ids_array)
{
    ...
    for (counter = 0; counter < nof_entries; counter++)
    {
        ...
        fec_entries[counter].flags2 |= BCM_L3_FLAGS2_SKIP_HIT_CLEAR;
...
/**
 * \brief
 *   A function that is used to test the performance for FEC add, update and delete actions
 *   using DBAL string APIs.
 * \param [in] unit - The unit number.
 * \param [in] nof_entries - The number of entries which will be added, updated and deleted.
 * \param [in] timers_group - The unique ID of the timer group
 * \param [in] fec_ids - The list of FEC IDs which will be added, updated and deleted
 * \param [in] entries_array - The list of FEC structures describing each FEC's data
 * \param [in] run - The index of the consecutive run of the procedure (0 to 100)
 * \param [out] run_time - The result of timing each procedure.
 * \return
 *   \retval Zero if no error was detected
 *   \retval Negative if error was detected. See \ref shr_error_e
 */
shr_error_e
dnx_l3_performance_fec_dbal(
    int unit,
    uint32 nof_entries,
    uint32 timers_group,
    int *fec_ids,
    bcm_l3_egress_t * entries_array,
    int run,
    int run_time[3][100])
{
    uint32 entry_handle_id;
    ...
    int rv;
    uint32 out_port = 14;
    uint32 out_lif = dnx_data_l3.rif.nof_rifs_get(unit);
    uint32 start_time;
    dbal_tables_e dbal_table = DBAL_TABLE_SUPER_FEC_1ST_HIERARCHY;
    mdb_physical_table_e fec_db;
    uint32 alloc_flags = DNX_ALGO_L3_FEC_WITH_ID;
    uint32 hierarchy;
    ...
    {
        bcm_l3_egress_t egr;
        int fec_id = fec_ids[iter];
        sal_memcpy(&egr, &entries_array[iter], sizeof(bcm_l3_egress_t));
        hierarchy = DNX_L3_FEC_HIERARCHY_FROM_FLAGS_GET(egr.flags);

        /** Allocate FEC (fec_index) */
        SHR_IF_ERR_EXIT(algo_l3_db.fec_db_info.fec_db.get(unit, DNX_L3_FEC_DIRECTION_NA, hierarchy, &fec_db));
    ...
    for (iter = 0; iter < nof_entries; iter++)
    {
        uint32 fec_entry[2] = { 0, 0 };
        uint32 super_fec_val = fec_ids[iter] >> 1;
        uint32 instance = fec_ids[iter] & 0x1;
        dbal_table = L3_EGRESS_GET_FEC_HIERARCHICAL_TABLE(entries_array[iter].flags);
        ...
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_SUPER_FEC_ID, super_fec_val);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_RESULT_TYPE, INST_SINGLE,
                                     DBAL_RESULT_TYPE_SUPER_FEC_1ST_HIERARCHY_SUPER_FEC_NO_PROTECTION);
        SHR_IF_ERR_EXIT(dbal_fields_struct_field_encode(unit, DBAL_FIELD_FEC_DESTINATION_LIF0, DBAL_FIELD_DESTINATION,
                                                        &out_port, fec_entry));
        SHR_IF_ERR_EXIT(dbal_fields_struct_field_encode(unit, DBAL_FIELD_FEC_DESTINATION_LIF0, DBAL_FIELD_GLOB_OUT_LIF,
                                                        &out_lif, fec_entry));
        dbal_entry_value_field_arr32_set(unit, entry_handle_id, DBAL_FIELD_FEC_DESTINATION_LIF0, instance, fec_entry);
        rv = dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT);
        if (rv != BCM_E_NONE)
        {
            sal_printf("Error in dbal_entry_commit \n");
        }
        /** Stop timer for action create */
        utilex_ll_timer_stop(timers_group, 1);
    }
    run_time[DNX_L3_PERF_RUN_TYPE_INSERT][run] = sal_time_usecs() - start_time;
    start_time = sal_time_usecs();

    out_lif = dnx_data_l3.rif.nof_rifs_get(unit) + 2;
    for (iter = 0; iter < nof_entries; iter++)
    {
        uint32 fec_entry[2] = { 0, 0 };
        uint32 super_fec_val = fec_ids[iter] >> 1;
        ...
        if (rv != BCM_E_NONE)
        {
            sal_printf("Error in dbal_entry_commit UPDATE \n");
        }
        /** Stop timer for action create */
        utilex_ll_timer_stop(timers_group, 2);
        out_lif++;
    }
    run_time[DNX_L3_PERF_RUN_TYPE_UPDATE][run] = sal_time_usecs() - start_time;
    start_time = sal_time_usecs();
    for (iter = 0; iter < nof_entries; iter += 2)
    {
        uint32 super_fec_val = fec_ids[iter] >> 1;

        /** Set timer for action create */
        utilex_ll_timer_set("FEC delete", timers_group, 3);
        SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, dbal_table, entry_handle_id));
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_SUPER_FEC_ID, super_fec_val);
        rv = dbal_entry_clear(unit, entry_handle_id, DBAL_COMMIT);
        /** Stop timer for action delete */
        utilex_ll_timer_stop(timers_group, 3);
        if (rv != BCM_E_NONE)
        {
            sal_printf("Error in dbal_entry_clear \n");
        }
    }
    run_time[DNX_L3_PERF_RUN_TYPE_DELETE][run] = sal_time_usecs() - start_time;

    for (iter = 0; iter < nof_entries; iter++)
    {
        SHR_IF_ERR_EXIT(dnx_algo_l3_fec_deallocate(unit, fec_ids[iter]));
    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    sal_printf("Generating %d nof entries per hierarchy \n", nof_entries);
    ...
    for (run = 0; run < nof_runs; run++)
    {
        switch (layer_id)
        {
            ...
            case CTEST_DNX_L3_PERFORMANCE_LAYER_ID_DBAL:
            {
                SHR_IF_ERR_EXIT(dnx_l3_performance_fec_dbal
                                (unit, nof_entries, timers_group, fec_ids, entries_array, run, run_time));
                break;
            }
            default:
            {
...
static sh_sand_cmd_t dnx_l3_performance_host_cmds[] = {
    {"IPv4", dnx_l3_performance_host_ipv4_cmd, NULL, dnx_l3_performance_cmd_options, &dnx_l3_performance_host_ipv4_man, NULL, NULL, CTEST_UM},
    {"IPv6", dnx_l3_performance_host_ipv6_cmd, NULL, dnx_l3_performance_cmd_options, &dnx_l3_performance_host_ipv6_man, NULL, NULL, CTEST_UM},
    {NULL}
};
...
static sh_sand_cmd_t dnx_l3_performance_route_cmds[] = {
    {"IPv4", dnx_l3_performance_route_ipv4_cmd, NULL, dnx_l3_performance_cmd_options, &dnx_l3_performance_route_ipv4_man, NULL, NULL, CTEST_UM},
    {"IPv6", dnx_l3_performance_route_ipv6_cmd, NULL, dnx_l3_performance_cmd_options, &dnx_l3_performance_route_ipv6_man, NULL, NULL, CTEST_UM},
    {NULL}
};
...
static sh_sand_cmd_t dnx_l3_performance_ipmc_cmds[] = {
    {"IPv4", dnx_l3_performance_ipmc_ipv4_cmd, NULL, dnx_l3_performance_cmd_options, &dnx_l3_performance_ipmc_ipv4_man, NULL, NULL, CTEST_UM},
    {"IPv6", dnx_l3_performance_ipmc_ipv6_cmd, NULL, dnx_l3_performance_cmd_options, &dnx_l3_performance_ipmc_ipv6_man, NULL, NULL, CTEST_UM},
    {NULL}
};
