    ...
    sh_sand_control_t * sand_control)
{
    ...
    const uint32 timer_compare[3][2] = { {1, 2}, {0, 2}, {1, 2} };

#ifndef ADAPTER_SERVER_MODE
    uint32 threshold_time_per_timer[4] = { 600, 600, 600, 400 };/** Very loose threshold, to pass all CPUs and compilation */
    ...
    {
        SHR_IF_ERR_EXIT(utilex_ll_timer_set("MPLS In Lif tunnel add1", timers_group, timer_indx));
    ...
    timer_indx++;
    for (; ii < nof_entries * 2; ii++)
    {
        SHR_IF_ERR_EXIT(utilex_ll_timer_set("MPLS In Lif tunnel add2", timers_group, timer_indx));
        SHR_IF_ERR_EXIT(mpls_prfm_tst_add_rxlsp(unit, rxlsp_match_label + ii, rx_hw_id + ii));
        SHR_IF_ERR_EXIT(utilex_ll_timer_stop(timers_group, timer_indx));
    }
    timer_indx++;

    for (; ii < nof_entries * 3; ii++)
    {
        SHR_IF_ERR_EXIT(utilex_ll_timer_set("MPLS In Lif tunnel add3", timers_group, timer_indx));
        SHR_IF_ERR_EXIT(mpls_prfm_tst_add_rxlsp(unit, rxlsp_match_label + ii, rx_hw_id + ii));
        SHR_IF_ERR_EXIT(utilex_ll_timer_stop(timers_group, timer_indx));
    }
    timer_indx++;

    for (ii = 0; ii < nof_entries * 3; ii++)
    {
    ...
    for (timer_indx = 0; timer_indx < nof_timers; timer_indx++)
    {
        ...
        timer_value[timer_indx] = total_time / nof_hits;
#ifndef ADAPTER_SERVER_MODE
...
#endif
    for (timer_indx = 0; timer_indx < 3; timer_indx++)
    {
        for (ii = 0; ii < 2; ii++)
        {
            uint32 compare_value = timer_value[timer_compare[timer_indx][ii]];
            if (((compare_value * 0.7) > timer_value[timer_indx]) || ((compare_value * 1.3) < timer_value[timer_indx]))
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "Timer value changes with adding more entries\n");
            }
        }
    }
    SHR_IF_ERR_EXIT(utilex_ll_timer_clear_all(timers_group));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    int mpls_tunnel_encap_id = 0x4002;
    int arp_id = 0;
    int ii;
    uint32 timers_group, nof_timers, timer_indx;
    char *timers_group_name;
