    ...
    bcm_rx_trap_config_t * trap_config_p)
{
    ...
    trap_config_p->flags2 = (sal_rand() % 1) ? 0 : BCM_RX_TRAP_FLAGS2_MIRROR_CODE_CLEAR;
}

/**
* \brief
*   Gets the configuration of drop profile and compares with set configuration.
* \param [in] unit - unit ID
...
*/
shr_error_e
ctest_dnx_rx_trap_etpp_drop_get_and_compare(
    int unit,
    bcm_rx_trap_config_t * trap_config_p)
{
    ...
    trap_config_p->flags = BCM_RX_TRAP_UPDATE_DEST;
    trap_config_p->dest_port = BCM_GPORT_BLACK_HOLE;

    SHR_IF_ERR_EXIT(bcm_rx_trap_get(unit, BCM_RX_TRAP_EG_TX_TRAP_ID_DROP, &trap_config_get));
    ...
    SHR_IF_ERR_EXIT(ctest_dnx_rx_trap_compare_values(unit, &trap_config_p->flags2, &trap_config_get.flags2,
                                                     sizeof(trap_config_get.flags2), "flags"));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    bcm_rx_trap_config_t_init(&trap_config[0]);
    trap_config[0].flags2 = BCM_RX_TRAP_FLAGS2_INT_STAT_DISABLE;
    SHR_IF_ERR_EXIT(bcm_rx_trap_set(unit, BCM_RX_TRAP_EG_TX_TRAP_ID_DROP, &trap_config[0]));
    SHR_IF_ERR_EXIT(ctest_dnx_rx_trap_etpp_drop_get_and_compare(unit, &trap_config[0]));

    bcm_rx_trap_config_t_init(&trap_config[0]);
    trap_config[0].pp_drop_reason = (sal_rand() % 3);
    SHR_IF_ERR_EXIT(bcm_rx_trap_set(unit, BCM_RX_TRAP_EG_TX_TRAP_ID_DROP, &trap_config[0]));
    SHR_IF_ERR_EXIT(ctest_dnx_rx_trap_etpp_drop_get_and_compare(unit, &trap_config[0]));
    ...
    LOG_INFO(BSL_LS_APPL_COMMON,
             (BSL_META_U(unit, "--------------------------------------------------------------- \n")));
    LOG_INFO(BSL_LS_APPL_COMMON, (BSL_META_U(unit, "     DROP profile get, change config ans set          \n")));
    LOG_INFO(BSL_LS_APPL_COMMON,
             (BSL_META_U(unit, "--------------------------------------------------------------- \n")));

    SHR_IF_ERR_EXIT(bcm_rx_trap_get(unit, BCM_RX_TRAP_EG_TX_TRAP_ID_DROP, &trap_config[0]));
    trap_config[0].pp_drop_reason = 1;
    SHR_IF_ERR_EXIT(bcm_rx_trap_set(unit, BCM_RX_TRAP_EG_TX_TRAP_ID_DROP, &trap_config[0]));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    int trap_iter = 0, config_iter = 0, mirror_cmd = 0;
    ...
    mirror_cmd = BCM_GPORT_MIRROR_GET(mirror_dest.mirror_dest_id);
    BCM_GPORT_TRAP_SET(trap_gport, mirror_cmd, 0, 7);
    ...
    sh_sand_control_t * sand_control)
{
    ...
    LOG_INFO(BSL_LS_APPL_COMMON,
             (BSL_META_U(unit, "-------------------------------------------------------------------------\n")));
    LOG_INFO(BSL_LS_APPL_COMMON,
             (BSL_META_U(unit, "  Trying to set BCM_RX_TRAP_FLAGS2_INT_STAT_DISABLE for non drop trap ID \n")));
    LOG_INFO(BSL_LS_APPL_COMMON,
             (BSL_META_U(unit, "-------------------------------------------------------------------------\n")));

    bcm_rx_trap_config_t_init(&trap_config);
    trap_config.flags2 = BCM_RX_TRAP_FLAGS2_INT_STAT_DISABLE;
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (sal_strncasecmp
        (test_type, CTEST_DNX_RX_TRAP_ETPP_TEST_TYPE_POSITIVE, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
    {
        if (!dnx_data_trap.egress.feature_get(unit, dnx_data_trap_egress_egress_divided_to_erpp_and_etpp))
        {
        ...
        if (sal_strncasecmp
            (test_mode, CTEST_DNX_RX_TRAP_ETPP_TEST_MODE_RCY_CMD, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
        {
            SHR_IF_ERR_EXIT(appl_dnx_rx_trap_fwd_rcy_cmd_pos(unit, args, sand_control));
        }
        else if (sal_strncasecmp
                 (test_mode, CTEST_DNX_RX_TRAP_ETPP_TEST_MODE_ACTIONS,
        ...
        else
        {
            SHR_IF_ERR_EXIT(appl_dnx_rx_trap_etpp_appl_pos(unit, args, sand_control));
             ...
             (test_type, CTEST_DNX_RX_TRAP_ETPP_TEST_TYPE_NEGATIVE, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
    {
        if (sal_strncasecmp
            (test_mode, CTEST_DNX_RX_TRAP_ETPP_TEST_MODE_RCY_CMD, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
        {
