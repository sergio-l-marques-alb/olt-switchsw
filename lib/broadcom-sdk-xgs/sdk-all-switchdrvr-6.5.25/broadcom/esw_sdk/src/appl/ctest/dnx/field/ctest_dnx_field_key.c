 ...
 * Enum key value
 */
static sh_sand_enum_t Field_key_enum_table[] = {
    {"A", DBAL_ENUM_FVAL_FIELD_KEY_A, "Key-A"},
    {"B", DBAL_ENUM_FVAL_FIELD_KEY_B, "Key-B"},
    {"C", DBAL_ENUM_FVAL_FIELD_KEY_C, "Key-C"},
    {"D", DBAL_ENUM_FVAL_FIELD_KEY_D, "Key-D"},
    {"E", DBAL_ENUM_FVAL_FIELD_KEY_E, "Key-E"},
    {"F", DBAL_ENUM_FVAL_FIELD_KEY_F, "Key-F"},
    {"G", DBAL_ENUM_FVAL_FIELD_KEY_G, "Key-G"},
    {"H", DBAL_ENUM_FVAL_FIELD_KEY_H, "Key-H"},
    {"I", DBAL_ENUM_FVAL_FIELD_KEY_I, "Key-I"},
    {"J", DBAL_ENUM_FVAL_FIELD_KEY_J, "Key-J"},
    {NULL}
};
    ...
    dnx_field_qual_t ** qual_type)
{
    ...
    switch (key_size)
    {
        ...
        case DNX_DATA_MAX_FIELD_TCAM_KEY_SIZE_SINGLE:
        {
            ...
            {
                *qual_type[0] = DNX_QUAL(DNX_FIELD_QUAL_CLASS_META, DNX_FIELD_STAGE_IPMF3, DNX_FIELD_QUAL_EEI);

                *qual_type[1] =
                    DNX_QUAL(DNX_FIELD_QUAL_CLASS_META, DNX_FIELD_STAGE_IPMF3, DNX_FIELD_QUAL_EGRESS_LEARN_ENABLE);

                *qual_type[2] = DNX_QUAL(DNX_FIELD_QUAL_CLASS_META, DNX_FIELD_STAGE_IPMF3, DNX_FIELD_QUAL_FWD_PAYLOAD);
            }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if ((field_stage == DNX_FIELD_STAGE_IPMF1) || (field_stage == DNX_FIELD_STAGE_EPMF))
    {
        ...
        key_in_info.key_length_type = DNX_FIELD_KEY_LENGTH_TYPE_SINGLE;

    }
    else if (field_stage == DNX_FIELD_STAGE_IPMF3)
    {
        field_group.dnx_quals[0] =
            DNX_QUAL(DNX_FIELD_QUAL_CLASS_META, DNX_FIELD_STAGE_IPMF3, DNX_FIELD_QUAL_SNOOP_STRENGTH);
        SHR_IF_ERR_EXIT(dnx_field_qual_attach_info_t_init(unit, &(key_in_info.qual_info[0])));
        ...
        field_group.dnx_quals[1] =
            DNX_QUAL(DNX_FIELD_QUAL_CLASS_META, DNX_FIELD_STAGE_IPMF3, DNX_FIELD_QUAL_RPF_DST_VALID);
        SHR_IF_ERR_EXIT(dnx_field_qual_attach_info_t_init(unit, &(key_in_info.qual_info[1])));
        key_in_info.qual_info[1].input_type = DNX_FIELD_INPUT_TYPE_META_DATA;
        key_in_info.qual_info[1].offset = DNX_FIELD_ATTACH_INFO_FIELD_OFFSET_DEFAULT;

        field_group.dnx_quals[2] = DNX_QUAL(DNX_FIELD_QUAL_CLASS_META, DNX_FIELD_STAGE_IPMF3, DNX_FIELD_QUAL_RPF_DST);
        SHR_IF_ERR_EXIT(dnx_field_qual_attach_info_t_init(unit, &(key_in_info.qual_info[2])));
    ...
    else if (field_stage == DNX_FIELD_STAGE_IPMF2)
    {
        ...
        key_in_info.key_length_type = DNX_FIELD_KEY_LENGTH_TYPE_DOUBLE;
    }
    /*
     ...
     */
    field_group.dnx_actions[0] = DNX_ACTION(DNX_FIELD_ACTION_CLASS_STATIC, field_stage, DNX_FIELD_ACTION_DP);
     ...
     * Create Key template
     * */
    SHR_IF_ERR_EXIT(dnx_field_key_template_create(unit, field_group.field_stage, field_group.dnx_quals, &key_template));
    ...
    if (key_test_clean == TRUE)
    {
        ...
        for (context_idx = 0; context_idx < DNX_DIAG_KEY_CONTEXT_NOF; context_idx++)
        {
            CTEST_DNX_FIELD_UTIL_ERR(dnx_field_key_detach
                                     (unit, key_in_info.field_stage, context_id[context_idx], key_in_info.fg_type,
                                      &(key_in_info.key_template), key_in_info.key_length_type, &key_id[context_idx]));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    key_id.id[0] = key_id.id[1] = DNX_FIELD_KEY_ID_INVALID;
    if (key_test_allocate)
    {
        ...
        LOG_CLI_EX("Bit range allocation: Allocated %d bits at offset %d of key '%c' %s\r\n",
                   (int) num_bits_in_bit_range, key_id.offset_on_first_key, 'A' + key_id.id[0], EMPTY);
    }
    else
    {
        /*
         ...
         */
        uint32 bit_lsb_uint32;

        SH_SAND_GET_INT32(DNX_DIAG_KEY_OPTION_TEST_OFFEST, bit_lsb_uint32);
        ...
        LOG_CLI_EX("Bit range free: Try to free key %c at offset %d. for stage %s, fg_type %s",
                   ('A' + key_id.id[0]), bit_lsb_uint32, dnx_field_stage_e_get_name(field_stage),
                   dnx_field_group_type_e_get_name(fg_type));
        LOG_CLI_EX(" context %d %s%s%s\r\n", (int) context_lower, EMPTY, EMPTY, EMPTY);
        key_id.offset_on_first_key = bit_lsb_uint32;
        if (key_id.offset_on_first_key != bit_lsb_uint32)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Offset %d is too large.\r\n", bit_lsb_uint32);
        }
        SHR_IF_ERR_EXIT(dnx_algo_field_key_id_bit_range_free(unit, field_stage, fg_type, context_lower, &key_id));
        LOG_CLI_EX("Bit range free: Completed 'free' of key '%c' at offset %d %s%s\r\n", 'A' + key_id.id[0],
                   key_id.offset_on_first_key, EMPTY, EMPTY);
    }
exit:
    SHR_FUNC_EXIT;
}
    ...
    dnx_field_group_t * fg_id_p)
{
    ...
    sal_snprintf(fg_info.name, sizeof(fg_info.name), "SHARE_FFC_FG_%d", *fg_id_p);

    fg_info.dnx_quals[0] = DNX_QUAL(DNX_FIELD_QUAL_CLASS_META, field_stage, DNX_FIELD_QUAL_DP);
    ...
    dnx_field_group_type_e fg_type)
{
     ...
     */
    SHR_IF_ERR_EXIT(appl_dnx_fg_add(unit, field_stage, fg_type, DNX_FIELD_ACTION_DP, fg_id_1_p));
    LOG_CLI_EX("Created fg %d %s%s%s\r\n", *fg_id_1_p, EMPTY, EMPTY, EMPTY);

    SHR_IF_ERR_EXIT(appl_dnx_fg_add(unit, field_stage, fg_type, DNX_FIELD_ACTION_TC, fg_id_2_p));
        ...
        {
            {
                ...
                SHR_IF_ERR_EXIT(dnx_algo_field_key_id_free
                                (unit, fg_type[free_test_step], field_stage, key_length[free_test_step],
                                 context_id[free_test_step], &(key_id[free_test_step])));
            }
            {
                /*
        ...
        {
            {
                ...
                key_id[alloc_test_step].offset_on_first_key = key_offset_bits[alloc_test_step];
                key_id[alloc_test_step].nof_bits[0] = key_length_bits[alloc_test_step];
        ...
        if ((field_stage != DNX_FIELD_STAGE_IPMF1) && (field_stage != DNX_FIELD_STAGE_EPMF))
        {
            ...
            }
            {
                ...
                LOG_INFO_EX(BSL_LOG_MODULE, "Key allocation request - Half key for direct extraction:\r\n"
                            ...
                            dnx_field_group_type_e_get_name(fg_type[alloc_test_step]), fg_type[alloc_test_step]);
                LOG_INFO_EX(BSL_LOG_MODULE,
                            " context_id %d%s%s%s\r\n", context_id[alloc_test_step], EMPTY, EMPTY, EMPTY);

                SHR_IF_ERR_EXIT(dnx_algo_field_key_id_bit_range_allocate
                                (unit, field_stage, fg_type[alloc_test_step], context_id[alloc_test_step],
                                 key_length_bits[alloc_test_step], &align_info, FALSE, &key_id[alloc_test_step]));

                key_offset_bits[alloc_test_step] = key_id[alloc_test_step].offset_on_first_key;
