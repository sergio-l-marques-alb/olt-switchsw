 ...
 */
#define DNX_CTEST_EXEM_OPTION_TEST_CONTEXT      "context"
/**
 * \brief
...
/**
 * The Presel IDs to use.
 */
#define DNX_CTEST_EXEM_PRESEL_ID (50)
 ...
 *   soc property pmf_sexem3_stage, which by default chooses iPMF2.
 */
sh_sand_invoke_t Sh_dnx_field_exem_tests[] = {
    {"DNX_field_exem_1",         "stage=ipmf1 count=6",              CTEST_POSTCOMMIT}
    ,
    {"DNX_field_exem_2",         "stage=ipmf2 count=6",              CTEST_POSTCOMMIT}
    ,
    {"DNX_field_exem_3",         "stage=ipmf3 count=6",              SH_CMD_SKIP_EXEC}
    ,
    ...
    ,
    {"DNX_field_exem_2_traffic", "stage=ipmf2 traffic=yes count=6",  CTEST_POSTCOMMIT}
    ,
    {"DNX_field_exem_3_traffic", "stage=ipmf3 traffic=yes count=6",  SH_CMD_SKIP_EXEC}
    ,
...
sh_sand_option_t Sh_dnx_field_exem_options[] = {
    ...
    {DNX_CTEST_EXEM_OPTION_TEST_TRAFFIC,    SAL_FIELD_TYPE_BOOL,   "Will the test include traffic test or not",  "No"},
    {DNX_CTEST_EXEM_OPTION_TEST_CONTEXT,    SAL_FIELD_TYPE_UINT32, "The context ID",                             "20"},
...
/** The DNX_QUAL to be used for each stage.*/
static dnx_field_qual_t Dnx_quals_array[CTEST_DNX_FIELD_EXEM_NUM_STAGES][CTEST_DNX_FIELD_EXEM_NUM_QUALS] = {
    ...
    {DNX_QUAL(DNX_FIELD_QUAL_CLASS_META, DNX_FIELD_STAGE_IPMF2, DNX_FIELD_QUAL_SNOOP_STRENGTH),
     DNX_QUAL(DNX_FIELD_QUAL_CLASS_META2, DNX_FIELD_STAGE_IPMF2, DNX_FIELD_QUAL_PMF1_EXEM_ACTION)},
    {DNX_QUAL(DNX_FIELD_QUAL_CLASS_META, DNX_FIELD_STAGE_IPMF3, DNX_FIELD_QUAL_FWD_ACTION_DP),
     DNX_QUAL(DNX_FIELD_QUAL_CLASS_META, DNX_FIELD_STAGE_IPMF3, DNX_FIELD_QUAL_WEAK_TM_PROFILE)},
...
static int Quals_input_arg[CTEST_DNX_FIELD_EXEM_NUM_STAGES][CTEST_DNX_FIELD_EXEM_NUM_QUALS] = {
    {1, 1},
    {0, 0},
    {0, 0},
    {1, 1},
...
static int Quals_offset[CTEST_DNX_FIELD_EXEM_NUM_STAGES][CTEST_DNX_FIELD_EXEM_NUM_QUALS] = {
    {0, 0},
    {0, 0},
    {0, 0},
    {0, 0},
...
/**
 * \brief
 *   This function checks that two dnx_field_entry_t structures are identical.
 * \param [in] unit - 
 *    The unit number.
 * \param [in] field_stage -
 *    The PMF stage for which we run the test (IPMF1, IPMF2, IPMF3, EPMF).
...
appl_dnx_exem_tester(
    ...
    dnx_field_context_t exem_test_context,
    int exem_test_clean)
{
    ...
    uint8 context_exists;
    dnx_field_stage_e field_stage_for_context;
    ...
    /*
     * Handle context, create if needed.
     */
    ...
    context_id = exem_test_context;
    SHR_IF_ERR_EXIT(dnx_field_context_id_is_allocated(unit, field_stage_for_context, context_id, &context_exists));
    if (context_exists == FALSE)
    {
        /*
         * Create the context and a catch all presel.
         */
        SHR_IF_ERR_EXIT(dnx_field_context_mode_t_init(unit, &context_mode));
        SHR_IF_ERR_EXIT(dnx_field_context_create
                        (unit, DNX_FIELD_CONTEXT_FLAG_WITH_ID, field_stage_for_context, &context_mode, &context_id));
        dnx_field_presel_entry_id_t_init(unit, &p_id);
        dnx_field_presel_entry_data_t_init(unit, &p_data);
        p_id.presel_id = DNX_CTEST_EXEM_PRESEL_ID;
        p_id.stage = field_stage_for_context;
    ...
    if (fg_info_out.group_full_info.action_length_type != DNX_FIELD_ACTION_LENGTH_TYPE_EXEM)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL,
                     "Group get returned with action_length_type %d, expected %d.\r\n",
                     fg_info_out.group_full_info.action_length_type, DNX_FIELD_ACTION_LENGTH_TYPE_EXEM);
    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SH_SAND_GET_UINT32(DNX_CTEST_EXEM_OPTION_TEST_CONTEXT, exem_test_context);
    SH_SAND_GET_BOOL(DNX_CTEST_EXEM_OPTION_TEST_CLEAN, exem_test_clean);

    SHR_IF_ERR_EXIT(appl_dnx_exem_tester(unit, field_stage, exem_test_traffic, exem_test_context, exem_test_clean));
