 ...
 *    3. If the allocation is at its limits try to allocate one more to receive the 'table full' error
 *
 * \param [in]
 *    unit- Relevant unit
 * \param [in]
...
/**
 * \brief
 *    This test tests that allocation fails in the following case:
 *    allocating an already allocated data s.t the profile ID is out side of the range
 *
 * \param [in]
 *    unit- Relevant unit
 * \param [in]
    ...
    char *test_name)
{
     ...
     * this should result in an error because the data we are trying to allocate already exists and
    ...
    SHR_VAL_VERIFY(rv, _SHR_E_PARAM, _SHR_E_FAIL, "Allocation manager did not fail in contradiction to expectation");
 ...
 */
shr_error_e
ctest_dnx_lif_esem_cmd_allocation_even_distribution_semantic_test(
    int unit,
    args_t * args,
    ...
    sh_sand_control_t * sand_control)
{
    SHR_FUNC_INIT_VARS(unit);
    {
        SHR_IF_ERR_EXIT(ctest_dnx_lif_esem_cmd_allocation_even_distribution_semantic_test(unit, args, sand_control));
    }
exit:
    SHR_FUNC_EXIT;
