...
/** EM rate */
#define DNX_APT_MPLS_INGRESS_INSERT_RATE        6600
#define DNX_APT_MPLS_EGRESS_INSERT_RATE         12000
#define DNX_APT_MPLS_EGRESS_UPDATE_RATE         9800
#define DNX_APT_MPLS_EGRESS_UPDATE_DBAL_RATE    60000
#define DNX_APT_MPLS_INGRESS_PORT_INSERT_RATE   6000
#define DNX_APT_MPLS_EGRESS_PORT_INSERT_RATE    11800

#define DNX_APT_MPLS_INGRESS_INSERT_RATE_JR2_A0         6000
#define DNX_APT_MPLS_EGRESS_INSERT_RATE_JR2_A0          10000
#define DNX_APT_MPLS_EGRESS_UPDATE_RATE_JR2_A0          8000
#define DNX_APT_MPLS_EGRESS_UPDATE_DBAL_RATE_JR2_A0     60000
...
typedef struct
{
    ...
    dnx_apt_mpls_info_t *mpls_info_array;
} dnx_apt_flow_mpls_info_t;
...
static double
dnx_apt_mpls_egress_update_time_threshold(
    int unit)
{
    return DNX_APT_MPLS_TIME_THRESHOLD(unit, EGRESS, UPDATE);
}
static double
dnx_apt_mpls_egress_update_dbal_time_threshold(
    int unit)
{
    return DNX_APT_MPLS_TIME_THRESHOLD(unit, EGRESS, UPDATE_DBAL);
}
static double
...
static shr_error_e
dnx_apt_mpls_egress_update_optional_threshold_info(
    int unit,
    char info[DNX_APT_MAX_DESCRIPTION_LENGTH])
{
    DNX_APT_MPLS_OPTIONAL_THRESHOLD_INFO(info, EGRESS, UPDATE);
    return _SHR_E_NONE;
}
static shr_error_e
dnx_apt_mpls_egress_update_dbal_optional_threshold_info(
    int unit,
    char info[DNX_APT_MAX_DESCRIPTION_LENGTH])
{
    DNX_APT_MPLS_OPTIONAL_THRESHOLD_INFO(info, EGRESS, UPDATE_DBAL);
    return _SHR_E_NONE;
}
static shr_error_e
    ...
    int number_of_egress_labels)
{
    ...
    for (iter = 0; iter < number_of_entries; iter++)
    {
        if (is_port)
        {
            ...
            if (is_egress)
            {
                mpls_info[iter].port.flags |= BCM_MPLS_PORT_EGRESS_TUNNEL;
                mpls_info[iter].port.flags2 |= BCM_MPLS_PORT2_EGRESS_ONLY;
                mpls_info[iter].port.encap_id = 0;
        ...
        else
        {
            if (is_egress)
            {
                ...
                for (label_iter = 0; label_iter < number_of_egress_labels; label_iter++)
                {
                    ...
                    mpls_info[iter].egress_label_array[label_iter].flags = BCM_MPLS_EGRESS_LABEL_TTL_DECREMENT;
                    /** if estimate bta/btr feature enable, allocate the lif without id since */
                    /** the feature creates restrictions on the global lif allocation */
...
/**
 * \brief A function that is used to create MPLS tunnel initiator entries for update and delete performance tests.
 */
static shr_error_e
dnx_apt_mpls_egress_pre_update(
    int unit,
    int number_of_calls,
    void *custom_data)
{
    int idx;
    int label_id;
    int label_size = dnx_data_mpls.general.nof_bits_mpls_label_get(unit);

    SHR_FUNC_INIT_VARS(unit);
    for (idx = 0; idx < number_of_calls; idx++)
    {
        dnx_apt_mpls_info_t mpls_info = ((dnx_apt_mpls_info_t *) custom_data)[idx];
        mpls_info.number_of_egress_labels = 1;
        for (label_id = 0; label_id < mpls_info.number_of_egress_labels; label_id++)
        {
            mpls_info.egress_label_array[label_id].flags = BCM_MPLS_EGRESS_LABEL_TTL_DECREMENT;
            /** if estimate bta/btr feature enable, allocate the lif without id since */
            /** the feature creates restrictions on the global lif allocation */
            if (!dnx_data_lif.out_lif.global_lif_allocation_estimated_encap_size_enable_get(unit))
            {
                mpls_info.egress_label_array[label_id].flags |= BCM_MPLS_EGRESS_LABEL_WITH_ID;
            }

        }
        SHR_IF_ERR_EXIT(bcm_mpls_tunnel_initiator_create(unit, 0,
                                                         mpls_info.number_of_egress_labels,
                                                         mpls_info.egress_label_array));

        for (label_id = 0; label_id < mpls_info.number_of_egress_labels; label_id++)
        {
            BCM_L3_ITF_LIF_TO_GPORT_TUNNEL(mpls_info.tunnel_gport[label_id],
                                           mpls_info.egress_label_array[label_id].tunnel_id);
            mpls_info.egress_label_array[label_id].label =
                dnx_apt_unique_random_get(idx) & UTILEX_BITS_MASK(label_size - 1, 0);
            mpls_info.egress_label_array[label_id].flags =
                BCM_MPLS_EGRESS_LABEL_WITH_ID | BCM_MPLS_EGRESS_LABEL_TTL_DECREMENT | BCM_MPLS_EGRESS_LABEL_REPLACE;
        }
        ((dnx_apt_mpls_info_t *) custom_data)[idx] = mpls_info;
    }
exit:
    SHR_FUNC_EXIT;
}
...
/**
 * \brief A function that updates a single MPLS tunnel initiator entry using DBAL APIs.
 * The entry must be previously created using dnx_apt_mpls_egress_pre_update.
 */
static shr_error_e
dnx_apt_mpls_egress_dbal_update_exec(
    int unit,
    int call_iter,
    void *custom_data)
{
    ...
    uint32 result_type = DBAL_RESULT_TYPE_EEDB_MPLS_TUNNEL_ETPS_MPLS_1;
    dnx_apt_mpls_info_t mpls_info;
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);
    mpls_info = ((dnx_apt_mpls_info_t *) custom_data)[call_iter];
    SHR_IF_ERR_EXIT(dnx_algo_gpm_gport_to_hw_resources
                    (unit, mpls_info.tunnel_gport[0], DNX_ALGO_GPM_GPORT_HW_RESOURCES_LOCAL_LIF_EGRESS,
                     &gport_hw_resources));
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_MPLS_LABEL, INST_SINGLE,
                                 mpls_info.egress_label_array[0].label);
...
/***********************************************************************************************************************
 * MPLS tunnel initiator create API that uses the Flow framework
 **********************************************************************************************************************/
...
const dnx_apt_object_def_t dnx_apt_mpls_egress_update_object = {
    /** Test name */
    "MPLS_Egress_Update",
    /** Test description */
    "Measures the performance of bcm_mpls_tunnel_initiator_create() when updating random egress labels using BCM API.",
    /** Support flags */
    0,
    /** ctest flags */
    CTEST_POSTCOMMIT,
...
const dnx_apt_object_def_t dnx_apt_mpls_egress_update_dbal_object = {
    /** Test name */
    "MPLS_Egress_Update_DBAL",
    /** Test description */
    "Measures the performance of bcm_mpls_tunnel_initiator_create() when updating random egress labels using DBAL APIs.",
    /** Support flags */
    0,
    /** ctest flags */
    CTEST_POSTCOMMIT,
