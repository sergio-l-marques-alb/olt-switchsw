    ...
    int *local_lif_array)
{
    ...
    global_lif_info.global_lif = global_lif_array[*current_index];
    SHR_IF_ERR_EXIT(dnx_lif_lib_allocate(unit, 0, &global_lif_info, lif_info, NULL));

    local_lif_array[*current_index] = lif_info->local_inlif;
    global_lif_array[*current_index] = global_lif_info.global_lif;
    (*current_index)++;
exit:
    SHR_FUNC_EXIT;
}
    ...
    int *local_lif_array[NOF_RESOURCES_TO_COMPARE])
{
    ...
    {
        /*
         * The most likely next common inlif is the next inlif that is divisible by the size of the n-pack. Since it might be occupied - we go 2 n-packs further, just to be sure.
         */
        target_inlif =
            UTILEX_ALIGN_UP(target_inlif,
                            dnx_data_lif.in_lif.inlif_resource_tag_size_get(unit)) +
            2 * (dnx_data_lif.in_lif.inlif_resource_tag_size_get(unit));
    ...
    int *local_lif_array[NOF_RESOURCES_TO_COMPARE])
{
    ...
    for (current_resource = 0; current_resource < NOF_RESOURCES_TO_COMPARE; current_resource++)
    {
         ...
         */
        /** expect 1 in lif because we expect all cores have the same local in lif value,  */
        /** and the only one index returned with core_id=_SHR_CORE_ALL */
        SHR_VAL_VERIFY(gports_hw_res[current_resource].local_in_lif_per_core.nof_in_lifs, nof_cores,
                       _SHR_E_FAIL, "Mismatching nof_in_lifs");
        SHR_VAL_VERIFY(gports_hw_res[current_resource].local_in_lif_per_core.core_id[0],
                       lif_info[current_resource]->core_id, _SHR_E_FAIL, "Mismatching core_id");
        ...
        SHR_VAL_VERIFY(gports_hw_res[current_resource].local_in_lif_per_core.local_in_lif[0],
                       lif_info[current_resource]->local_inlif, _SHR_E_FAIL, "Mismatching local inlif");
        ...
        local_lif_info.phy_table = lif_mapping_info.phy_table;
        local_lif_info.local_lif = lif_mapping_info.local_lif;
        SHR_IF_ERR_EXIT(dnx_algo_lif_mapping_local_to_global_get
                        (unit, DNX_ALGO_LIF_INGRESS, &local_lif_info, &(global_lifs[current_resource])));
    ...
    int should_be_reused)
{
    ...
    if (!can_be_reused)
    {
        ...
        local_lif_payload.local_lifs_array.local_lifs[0] = local_inlif_info->local_inlif;
        local_lif_payload.phy_table = DBAL_PHYSICAL_TABLE_INLIF_2;
        SHR_IF_ERR_EXIT(dnx_algo_lif_mapping_create(unit, DNX_ALGO_LIF_INGRESS, global_lif, &local_lif_payload, TRUE));
    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SHR_IF_ERR_EXIT(dnx_lif_lib_allocate(unit, 0, &global_lif_info, &local_inlif_info, NULL));

    for (ii = 0; ii < 2; ii++)
    {
        if (local_inlif_info.local_inlif % 6)
        {
            break;
        }
        SHR_IF_ERR_EXIT(dnx_lif_lib_allocate(unit, 0, &global_lif_info, &local_inlif_info, NULL));
    }
    /**Add two extra entry, to ensure non-empty 6th-pack*/

    /*
     * Test case 1: Replace entry of 120b to entry of 120b, expect re-use of the same entry.
     */
    SHR_IF_ERR_EXIT(ctest_dnx_local_inlif_allocation_replace(unit, &local_inlif_info, global_lif_info.global_lif,
    ...
    SHR_IF_ERR_EXIT(ctest_dnx_local_inlif_allocation_replace(unit, &local_inlif_info, global_lif_info.global_lif,
                                                             ...
                                                             2, (local_inlif_info.local_inlif % 6) ? FALSE : TRUE));
    ...
    SHR_IF_ERR_EXIT(ctest_dnx_local_inlif_allocation_replace(unit, &local_inlif_info, global_lif_info.global_lif,
                                                             ...
                                                             3, (local_inlif_info.local_inlif % 6) ? FALSE : TRUE));

exit:
    /*
     * Now free the outlif.
     */
    rv = dnx_lif_lib_free(unit, global_lif_info.global_lif, &local_inlif_info, LIF_MNGR_INVALID);
