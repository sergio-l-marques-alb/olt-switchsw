...
extern char group_name[UTILEX_LL_TIMER_MAX_NOF_CHARS_IN_TIMER_GROUP_NAME];
extern uint32 timers_group;
extern uint32 timer_idx[NOF_FLOW_TIMERS];
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (obj_index = 0; obj_index < number_of_objects - 1; obj_index++)
    {
        ...
        if (dnx_apt_objects[obj_index]->time_threshold == NULL)
        {
            SHR_ERR_EXIT(_SHR_E_CONFIG, "Test \"%s\" does not have time_threshold callback configured.\n",
        ...
        for (search_index = obj_index + 1; search_index < number_of_objects; search_index++)
        {
            ...
            if ((dnx_apt_objects[obj_index]->support_flags == dnx_apt_objects[search_index]->support_flags) &&
                ...
                && (dnx_apt_objects[obj_index]->number_of_calls == dnx_apt_objects[search_index]->number_of_calls))
            {
                SHR_ERR_EXIT(_SHR_E_CONFIG,
                             "Tests \"%s\" and \"%s\" have identical flags, functions and number of calls.\n",
                             dnx_apt_objects[obj_index]->test_name, dnx_apt_objects[search_index]->test_name);
            }
        }
    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    /*
     * Initialize counters for FLOW performance measurement
     */
    if (FLOW_TIMERS_MODE)
    {
        int idx;

        sal_strncpy(group_name, "FLOW Performance", UTILEX_LL_TIMER_MAX_NOF_CHARS_IN_TIMER_GROUP_NAME - 1);
        SHR_IF_ERR_EXIT(utilex_ll_timer_group_allocate(group_name, &timers_group));
        utilex_ll_timer_clear_all(timers_group);
        for (idx = 0; idx < NOF_FLOW_TIMERS; idx++)
        {
            timer_idx[idx] = idx;
        }
    }
    ...
    for (obj_index = iter_start; obj_index < iter_end; obj_index++)
    {
        ...
        if (!execute_all && (utilex_is_prime_flag_set(dnx_apt_objects[obj_index]->ctest_flags, SH_CMD_SKIP_EXEC)))
        {
    ...
    /*
     * Prints counters from FLOW performance measurement
     */
    if (FLOW_TIMERS_MODE)
    {
        int is_active, idx;
        ...
        for (idx = 0; idx < NOF_FLOW_TIMERS; idx++)
        {
            SHR_IF_ERR_EXIT(utilex_ll_timer_get_info
                            (timers_group, timer_idx[idx], &is_active, &name, &hits, &total_time));
            avrg = (double) total_time / (double) hits;

            if (hits)
            {

                sal_printf("timing average for %s (%d) - %f\n", name, idx, avrg);
            }
        }
    ...
    rhlist_t * test_list)
{
    ...
    /** Do not create dynamic test list simulation sessions except when deliberately including adapter unsupported tests */
    if (SAL_BOOT_PLISIM && !test_list->include_unsupported)
    {
        SHR_EXIT();
    }
    ...
    for (obj_index = 0; obj_index < number_of_objects; obj_index++)
    {
        ...
        if (rv != _SHR_E_NONE && !test_list->include_unsupported)
        {
            /** The test is not supported on the current device configuration. */
            continue;
        }

        sal_strncat_s(test_args, dnx_apt_objects[obj_index]->test_name, sizeof(test_args));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (iter = 0; iter < number_of_objects; iter++)
    {
        ...
        PRT_CELL_SET("%s%s%s",
                     (utilex_is_prime_flag_set(dnx_apt_objects[iter]->ctest_flags, CTEST_PRECOMMIT) ? "PRECOMMIT " :
                      ""), (utilex_is_prime_flag_set(dnx_apt_objects[iter]->ctest_flags,
                                                     CTEST_POSTCOMMIT) ? "POSTCOMMIT " : ""),
                     (utilex_is_prime_flag_set(dnx_apt_objects[iter]->ctest_flags, SH_CMD_SKIP_EXEC) ? "SKIPPED " :
                      ""));
        if (dump_all)
        {
...
sh_sand_cmd_t dnx_apt_cmds[] = {
    {"TeST", dnx_apt_test_cmd, NULL, dnx_apt_test_options, &dnx_apt_test_man, NULL, NULL, CTEST_PASS * CTEST_SKIPPABLE * CTEST_ADAPTER_UNSUPPORTED, dnx_apt_test_dyncamic_tests_creation_cmd},
    {"DuMP", dnx_apt_dump_cmd, NULL, dnx_apt_dump_options, &dnx_apt_dump_man, NULL, NULL, SH_CMD_SKIP_EXEC},
    {"Validate", dnx_apt_validate_cmd, NULL, dnx_apt_validate_options, &dnx_apt_validate_man, NULL, NULL, CTEST_PRECOMMIT * CTEST_SKIPPABLE},
    {NULL}
};
/* *INDENT-ON* */
/*
