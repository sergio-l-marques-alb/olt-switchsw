/**
  * \file ctest_dnx_switch_load_balancing.c
  *
    ...
    sh_sand_control_t * sand_control)
{
     ...
     */
    bsl_severity_t original_severity_lb;
    int nof_mpls_protocols_plus_one;
    int mpls_iter = 0;
    SHR_FUNC_INIT_VARS(unit);
    /**
     * Since this procedure includes negative tests, error logs are expected.
     ...
     */
    SHR_GET_SEVERITY_FOR_MODULE_SPECIFIC(BSL_LS_BCMDNX_LB, original_severity_lb);
    sal_memset(set_mpls_identification.mpls_identification, 0,
               sizeof(set_mpls_identification.mpls_identification[0]) * SWITCH_MPLS_IDENTIFICATION_SIZE_IN_UINT32);
    set_mpls_identification.mpls_identification[DBAL_ENUM_FVAL_LAYER_TYPES_MPLS / SAL_UINT32_NOF_BITS] =
        SAL_BIT(DBAL_ENUM_FVAL_LAYER_TYPES_MPLS % SAL_UINT32_NOF_BITS);
    ...
    }
    {
         ...
         */
        SHR_SET_SEVERITY_FOR_MODULE_SPECIFIC(BSL_LS_BCMDNX_LB, bslSeverityFatal);
        nof_mpls_protocols_plus_one = dnx_data_switch.load_balancing.num_valid_mpls_protocols_get(unit) + 1;
        if (nof_mpls_protocols_plus_one <
            SAL_BIT(dnx_data_switch.load_balancing.nof_bits_mpls_protocol_identifier_get(unit)))
        {
            while (nof_mpls_protocols_plus_one > SAL_UINT32_NOF_BITS)
            {
                set_mpls_identification.mpls_identification[mpls_iter++] = UTILEX_BITS_MASK(SAL_UINT32_NOF_BITS - 1, 0);
                nof_mpls_protocols_plus_one -= SAL_UINT32_NOF_BITS;
            }
            set_mpls_identification.mpls_identification[mpls_iter] = SAL_UPTO_BIT(nof_mpls_protocols_plus_one);
            SHR_IF_NOT_EXCEPT_ERR_EXIT(dnx_switch_load_balancing_mpls_identification_set
                                       (unit, &set_mpls_identification), _SHR_E_PARAM);
        }
        else
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL,
                         "Unable to perform negative test, nof valid mpls protocol plus one is %d bigger than nof mpls protocols %d",
                         nof_mpls_protocols_plus_one,
                         SAL_BIT(dnx_data_switch.load_balancing.nof_bits_mpls_protocol_identifier_get(unit)));
        }
        /**
         *  Restore the original severity after the end of Negative test.
