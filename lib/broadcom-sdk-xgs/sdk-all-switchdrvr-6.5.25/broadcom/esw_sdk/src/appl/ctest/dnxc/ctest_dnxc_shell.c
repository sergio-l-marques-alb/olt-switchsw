...
shell_input_option_cb(
    ...
    void **ext_ptr_p)
{
    if (!sal_strncasecmp(keyword, "test", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
    {
        if (type_p != NULL)
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if(SOC_IS_DNX(unit))
    {
        DNXCMN_CORE_VALIDATE(unit, value_int32, TRUE);
    }
#endif
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (i_key = 0; sh_sand_keywords[i_key].keyword != NULL; i_key++)
    {
        ...
        if (i_key != 0)
        {
            if (sal_strncasecmp(sh_sand_keywords[i_key].keyword, sh_sand_keywords[i_key - 1].keyword, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) < 0)
            {
                                                        ...
                                                        !(sh_sand_keywords[i_key].flags & SH_SAND_KEYWORD_ALLOW_DOUBLE))
        {
            ...
            {
                if (sal_strncasecmp(sh_sand_keywords[i_key].short_key, sh_sand_keywords[k_key].short_key, sizeof(sh_sand_keywords[k_key].short_key)) == 0)
                {
        ...
        if (!(sh_sand_keywords[i_key].flags & SH_SAND_KEYWORD_SINGLE_CHARACTER_SHORTCUT) &&
                                                                (sal_strnlen(sh_sand_keywords[i_key].short_key,sizeof(sh_sand_keywords[i_key].short_key)) == 1))
        {
...
static sh_sand_cmd_t sh_sand_prt_test_cmds[] = {
    {"plural", plural_prt_cmd, NULL, NULL, &plural_prt_man, NULL, NULL, CTEST_PRECOMMIT * CTEST_SKIPPABLE},
    {"combine", combine_prt_cmd, NULL, NULL, &plural_prt_man, NULL, NULL, CTEST_PRECOMMIT * CTEST_SKIPPABLE},
    {"info", info_prt_cmd, NULL, NULL, &info_prt_man, NULL, NULL, CTEST_PRECOMMIT * CTEST_SKIPPABLE},
    {NULL}
};
sh_sand_invoke_t dnxc_input_tests[] = {
    {"first", "name=example",    CTEST_PRECOMMIT * CTEST_SKIPPABLE},
    {"ipv4",  "ip4=10.11.12.13", CTEST_PRECOMMIT * CTEST_SKIPPABLE},
    {"free_variables",  "status_string 3", CTEST_PRECOMMIT * CTEST_SKIPPABLE},
    {"range", "range=100-300",   CTEST_PRECOMMIT * SH_CMD_SKIP_TEST_WB * CTEST_SKIPPABLE},
    {NULL}
};
    ...
    rhlist_t * test_list)
{
    ...
    int flags = (test_list->include_unsupported == TRUE) ? CTEST_ALL : 0;
    ...
    int flags)
{
    ...
    int exclude_unsupported_tests = TRUE;
    int test_add_flags;
    ...
    /*
     * Normally, tests with SH_CMD_NO_XML_VERIFY flag are skipped.
     * However, if in adapter and test has CTEST_ADAPTER_UNSUPPORTED flag, don't skip.
     */
    exclude_unsupported_tests = !utilex_is_prime_flag_set(sh_sand_cmd->flags, CTEST_ADAPTER_UNSUPPORTED);
#endif

    /*
     * Skip commands marked by SH_CMD_NO_XML_VERIFY flag
     */
    if (utilex_is_prime_flag_set(sh_sand_cmd->flags, SH_CMD_NO_XML_VERIFY) && exclude_unsupported_tests)
    {
        SHR_EXIT();
    }
    ...
    /*
     * If the test name is empty (an array of zeros) it should not be added to the list
     */
    if (test_name[0] == 0)
    {
        SHR_EXIT();
    ...
    test_add_flags = sh_sand_cmd->flags;
    utilex_prime_flag_set(&test_add_flags, CTEST_VERIFY);
    SHR_CLI_EXIT_IF_ERR(sh_sand_test_add(unit, test_list, test_name, test_arguments,
                                         test_add_flags), "Add shell test for:%s failed\n", test_name);
    ...
    rhlist_t * test_list)
{
    ...
    SHR_CLI_EXIT_IF_ERR(sh_sand_cmd_traverse(unit, NULL, sh_sand_device_a, shell_verify_test_add, (void *) test_list, flags, NULL       /* label 
                                                                                                                                         */ ), "");
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (log_flag == TRUE)
    {
        sal_strncat_s(full_command, " lg", sizeof(full_command));
    }
...
sh_sand_cmd_t dnxc_framework_test_cmds[] = {
    ...
    {"resource",    shell_resource_cmd,        NULL,   shell_resource_options, &shell_resource_man,        NULL,                  NULL,             CTEST_PRECOMMIT * SH_CMD_SKIP_TEST_WB * CTEST_SKIPPABLE},
    {"print",       NULL,                      sh_sand_prt_test_cmds},
    {"usagetest",   shell_screen_cmd,          NULL,   shell_test_arguments,   &shell_screen_man,          NULL,                  NULL,             CTEST_POSTCOMMIT * CTEST_SKIPPABLE, shell_test_list_create},
    {"shell",       shell_exec_cmd,            NULL,   shell_test_arguments,   &shell_exec_man,            NULL,                  NULL,             CTEST_PASS * CTEST_SKIPPABLE, shell_test_list_create},
    {"write",       shell_write_cmd,           NULL,   shell_test_arguments,   &shell_write_man,           NULL,                  NULL,             CTEST_POSTCOMMIT * CTEST_SKIPPABLE, shell_test_list_create},
    {"verify",      shell_verify_cmd,          NULL,   shell_verify_arguments, &shell_verify_man,          NULL,                  NULL,             CTEST_POSTCOMMIT * CTEST_SKIPPABLE * CTEST_VERIFY, shell_verify_list_create},
    {"regress",     shell_regress_cmd,         NULL,   shell_test_arguments,   &shell_regress_man,         NULL,                  NULL,             CTEST_SKIPPABLE},
    /*
     * Callback at the end is for command eligibility and not for test creation, which is marks using SH_CMD_CONDITIONAL flag
     */
    {"cint",     shell_cint_cmd,     NULL, NULL,                   &shell_cint_man,     NULL,                  NULL,             CTEST_PRECOMMIT * SH_CMD_CONDITIONAL * SH_CMD_ROLLBACK * CTEST_SKIPPABLE, sh_cmd_is_dnx},
    {"cint_ext", shell_cint_ext_cmd, NULL, NULL,                   &shell_cint_man,     NULL,                  NULL,             CTEST_PRECOMMIT * SH_CMD_CONDITIONAL * SH_CMD_ROLLBACK * CTEST_SKIPPABLE, sh_cmd_is_dnx},
    {"property", test_property_cmd,  NULL, NULL,                   &test_property_man,  NULL,                  NULL,             CTEST_POSTCOMMIT * CTEST_SKIPPABLE},
    {NULL}
};
/* *INDENT-ON* */
