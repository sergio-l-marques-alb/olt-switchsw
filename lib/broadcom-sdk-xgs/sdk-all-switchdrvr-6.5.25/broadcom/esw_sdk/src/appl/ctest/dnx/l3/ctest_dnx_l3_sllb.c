    ...
    uint32 result_nwk_lb_key)
{
    ...
    SHR_CLI_EXIT_IF_ERR(sand_signal_verify
                        (unit, core, "IRPP", "LBP", "ITMR", "UDH_Base", (uint32 *) &udh_base, 1, NULL, return_value_str,
                         0), "Test failed , UDH_Base from LBP core %d expecting %d return value %s\n", core, udh_base,
                        return_value_str);
    ...
    if (udh_server_fec_id != result_fec_id)
    {
        SHR_ERR_EXIT(_SHR_E_FAIL,
                     "Test failed , Server FEC_ID stamped on UDH2 (%d) does not match expected Server FEC_ID (%d)\n",
    ...
    {
        SHR_ERR_EXIT(_SHR_E_FAIL,
                     "Test failed , ECMP3-Lb-Key stamped on UDH2 (%d) does not match expected ECMP3-Lb-Key (%d)\n",
    ...
    {
        SHR_ERR_EXIT(_SHR_E_FAIL,
                     "Test failed , NWK-LB-Key stamped on UDH2 (%d) does not match expected NWK-LB-Key (%d)\n",
    ...
    uint32 basic_route_fec_id)
{
    ...
    for (l3_test_type = CTEST_SLLB_L3_TYPE_IPV4; l3_test_type <= CTEST_SLLB_L3_TYPE_IPV6; l3_test_type++)
    {
        ...
        for (l4_test_type = CTEST_SLLB_L4_TYPE_UDP; l4_test_type <= CTEST_SLLB_L4_TYPE_NONE; l4_test_type++)
        {
            ...
            SHR_CLI_EXIT_IF_ERR(sand_signal_verify
                                (unit, core, "IRPP", "FWD1", "FWD2", "Fwd_Action_Dst", (uint32 *) &result_fec_id, 2,
                                 NULL, return_value_str, 0),
                                "Test failed , Fwd_Action_Dst from FWD1 core %d expecting %d return value %s\n", core,
                                result_fec_id, return_value_str);
            ...
            result_fec_id = basic_route_fec_id;
            SHR_CLI_EXIT_IF_ERR(sand_signal_verify
                                (unit, core, "IRPP", "FWD2", "IPMF1", "Fwd_Action_Dst", (uint32 *) &result_fec_id, 2,
                                 NULL, return_value_str, 0),
    ...
    for (l3_test_type = CTEST_SLLB_L3_TYPE_IPV4; l3_test_type <= CTEST_SLLB_L3_TYPE_IPV6; l3_test_type++)
    {
        ...
        for (l4_test_type = CTEST_SLLB_L4_TYPE_UDP; l4_test_type <= CTEST_SLLB_L4_TYPE_TCP; l4_test_type++)
        {
            ...
            for (i_pkt = 0; i_pkt < CTEST_SLLB_BASIC_PKT_COUNT; i_pkt++, dst_port++)
            {
                ...
                sand_signal_verify(unit, core, "IRPP", "FWD2", "IPMF1", "Fwd_Action_Dst", (uint32 *) &result_fec_id, 4,
                                   NULL, return_value_str, 0);
            ...
            sand_signal_verify(unit, core, "IRPP", "FWD1", "FWD2", "ECMP_LB_Keys.Hierarchy_3",
                               (uint32 *) &result_ecmp_lb_key, 2, NULL, return_value_str, 0);
            sscanf(return_value_str, "%x", &result_ecmp_lb_key);
            result_nwk_lb_key = 0;      /* Dummy value - we don't know the expected value */
            sand_signal_verify(unit, core, "IRPP", "FWD1", "FWD2", "Nwk_LB_Key", (uint32 *) &result_nwk_lb_key, 2, NULL,
                               return_value_str, 0);
            ...
            sand_signal_verify(unit, core, "IRPP", "FWD1", "FWD2", "LAG_LB_Key", (uint32 *) &result_lag_lb_key, 2, NULL,
                               return_value_str, 0);
            ...
            SHR_CLI_EXIT_IF_ERR(sand_signal_verify
                                (unit, core, "IRPP", "IPMF1", "FER", "Snoop_Qualifier", (uint32 *) &result_snoop_qual,
                                 1, NULL, return_value_str, 0),
                                "Test failed , Snoop_Qualifier from IPMF1 core %d expecting %d return value %s\n", core,
                                result_snoop_qual, return_value_str);
             ...
             */
            SHR_IF_ERR_EXIT(dnx_l3_sllb_basic_udh_verify
                            (unit, core, vip_id, result_server_fec_id, result_ecmp_lb_key, result_nwk_lb_key));
            ...
            {
                SHR_ERR_EXIT(_SHR_E_FAIL,
                             "Test failed , FWD2 result 0x%08x did not match the PCC State table entry FEC ID 0x%08x\n",
            ...
            if (0 /* l4_test_type == CTEST_SLLB_L4_TYPE_TCP */ )
            {
                ...
                SHR_CLI_EXIT_IF_ERR(sand_signal_verify
                                    (unit, core, "IRPP", "IPMF1", "FER", "Snoop_Qualifier",
                                     (uint32 *) &result_snoop_qual, 1, NULL, return_value_str, 0),
                                    "Test failed , Snoop_Qualifier from IPMF1 core %d expecting %d return value %s\n",
            ...
            if (kbp_pcc_enabled)
            {
                ...
                {
                    SHR_ERR_EXIT(_SHR_E_FAIL,
                                 "Test failed , FWD2 result 0x%08x did not match the KPB PCC State table entry FEC ID 0x%08x\n",
                                 result_server_fec_id, pcc_fec_id);
                }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SHR_CLI_EXIT_IF_ERR(ctest_dnxc_cint_run(unit, "sllb_field_example", NULL, 0), "sllb_field_example Run Failed\n");

    SHR_IF_ERR_EXIT(bcm_port_get(unit, in_port, &flags, &interface_info, &mapping_info));

    SHR_IF_ERR_EXIT(dnx_l3_sllb_basic_vip_route_test
                    (unit, mapping_info.core, mapping_info.pp_port, mapping_info.tm_port, vip_id, fec_config.start,
                     sllb_table_size, kbp_pcc_enabled));

    SHR_IF_ERR_EXIT(dnx_l3_sllb_basic_direct_route_test
                    (unit, mapping_info.core, mapping_info.pp_port, mapping_info.tm_port, fec_config.start));
    ...
    int *vip_ecmp_fec_max_id)
{
    ...
    SHR_IF_ERR_EXIT(bcm_switch_control_set(unit, bcmSwitchVIPEcmpTableSize, arg_value_set));

    /*
     * Get bcmSwitchVIPEcmpTableSize and compare to set value
     */
    SHR_IF_ERR_EXIT(bcm_switch_control_get(unit, bcmSwitchVIPEcmpTableSize, &arg_value_get));
    ...
    *sllb_table_size = arg_value_get;
    ...
    rv = bcm_switch_control_set(unit, bcmSwitchVIPL3EgressIDMin, arg_value_set);
    if (rv != BCM_E_PARAM)
    {
        SHR_ERR_EXIT(_SHR_E_FAIL,
    ...
    if (rv != BCM_E_PARAM)
    {
        SHR_ERR_EXIT(_SHR_E_FAIL,
                     ...
                     arg_value_set, (arg_value_set + nof_vip_ecmp - 1), (nof_fecs - nof_vip_ecmp + 1), rv);
    }

    /*
     * 2.c test arg = nof_fecs - nof_vip_ecmp 
     */
    SHR_IF_ERR_EXIT(bcm_switch_control_get(unit, bcmSwitchVIPL3EgressIDMin, &arg_value_get));

    if (arg_value_get == 0)
    {
        arg_value_set = nof_fecs - nof_vip_ecmp;
        SHR_IF_ERR_EXIT(bcm_switch_control_set(unit, bcmSwitchVIPL3EgressIDMin, arg_value_set));
    ...
    *vip_ecmp_fec_min_id = arg_value_get;
    ...
    SHR_IF_ERR_EXIT(bcm_switch_control_get(unit, bcmSwitchVIPL3EgressIDMax, &arg_value_get));
    if (arg_value_get != arg_value_set)
    {
        SHR_ERR_EXIT(_SHR_E_FAIL,
    ...
    *vip_ecmp_fec_max_id = arg_value_get;
    ...
    SHR_IF_ERR_EXIT(bcm_switch_control_set(unit, bcmSwitchSLLBDestinationNotFound, arg_value_set));

    BCM_GPORT_LOCAL_SET(arg_value_set, 0);
    SHR_IF_ERR_EXIT(bcm_switch_control_set(unit, bcmSwitchSLLBDestinationNotFound, arg_value_set));

    SHR_IF_ERR_EXIT(bcm_switch_control_set(unit, bcmSwitchSLLBDestinationNotFound, BCM_GPORT_LOCAL_CPU));
    ...
    SHR_IF_ERR_EXIT(bcm_switch_control_set(unit, bcmSwitchSLLBDestinationNotFound, BCM_GPORT_BLACK_HOLE));
    ...
    int vip_ecmp_fec_max_id)
{
    ...
    if (rv != BCM_E_PARAM)
    {
        SHR_ERR_EXIT(_SHR_E_FAIL, "Test failed. Should throw E_PARAM for ecmp NULL pointer.\n");
    }

    rv = bcm_l3_egress_ecmp_create(unit, &ecmp, sllb_table_size, NULL);
    if (rv != BCM_E_PARAM)
    {
        SHR_ERR_EXIT(_SHR_E_FAIL, "Test failed. Should throw E_PARAM for intf_array NULL pointer.\n");
    }
    ...
    if (rv != BCM_E_PARAM)
    {
        SHR_ERR_EXIT(_SHR_E_FAIL, "Test failed. Should throw E_PARAM for unsupported flags (BCM_L3_2ND_HIERARCHY).\n");
    }
    ...
    if (rv != BCM_E_PARAM)
    {
        SHR_ERR_EXIT(_SHR_E_FAIL, "Test failed. Should throw E_PARAM for invalid dynamic mode.\n");
    }
    ...
    /*
     * 1. Test ecmp group not in VIP ECMP range -- valid ecmp group id
     */
    bcm_l3_egress_ecmp_t_init(&ecmp);
    ...
    {
        SHR_ERR_EXIT(_SHR_E_FAIL,
                     "Test failed. Should throw E_NOT_FOUND if ecmp.ecmp_intf (%d) is not in VIP ECMP range.\n",
                     ecmp.ecmp_intf);
    }
    ...
    {
        SHR_ERR_EXIT(_SHR_E_FAIL,
                     "Test failed. Should throw E_NOT_FOUND if ecmp.ecmp_intf (%d) is not allocated.\n",
                     ecmp.ecmp_intf);
    }
    ...
    int pcc_fec_id)
{
    ...
    if (rv != _SHR_E_PARAM)
    {
        SHR_ERR_EXIT(_SHR_E_FAIL, "Test failed. Should throw E_PARAM for missing flags.\n");
    }
    ...
    for (table_index = 0; table_index < CTEST_NUM_PCC_TABLES; table_index++)
    {
        ...
        for (flags_index = 0; flags_index < CTEST_NUM_MATCH_FLAGS_COMBO; flags_index++)
        {
            ...
            SHR_IF_ERR_EXIT(bcm_l3_egress_ecmp_resilient_replace
                            (unit, flags, &match_entry, &num_entries, &replace_entry));
        ...
        for (flags_index = 0; flags_index < CTEST_NUM_MATCH_FLAGS_COMBO; flags_index++)
        {
            ...
            {
                SHR_ERR_EXIT(_SHR_E_FAIL, "Test failed. Should throw E_NOT_FOUND for unmatched key field ecmp_intf");
            }
        }
    ...
    for (table_index = 0; table_index < CTEST_NUM_PCC_TABLES; table_index++)
    {
        ...
        if (rv != BCM_E_NOT_FOUND)
        {
            SHR_ERR_EXIT(_SHR_E_FAIL, "Test failed. Should throw E_NOT_FOUND for unmatched key fields");
        }
        ...
        if (rv != BCM_E_NOT_FOUND)
        {
            SHR_ERR_EXIT(_SHR_E_FAIL, "Test failed. Should throw E_NOT_FOUND for unmatched key field hash_key");
        }
     ...
     */
    {
        for (table_index = 0; table_index < CTEST_NUM_PCC_TABLES; table_index++)
        {
            ...
            fec_id = start_fec_id;
            for (vip_ecmp_fec_id = start_vip_ecmp_fec_id;
                 vip_ecmp_fec_id < (start_vip_ecmp_fec_id + num_vip_ecmp_fec_id); vip_ecmp_fec_id++)
            {
            ...
            for (flags_index = 0; flags_index < CTEST_NUM_MATCH_FLAGS_COMBO; flags_index++)
            {
                ...
                SHR_IF_ERR_EXIT(dnx_l3_egress_ecmp_pcc_resilient_traverse
                                (unit, flags, &match_entry, dnx_l3_sllb_resilient_trav_cb, &count));
                if (count != expected_count)
                {
                    SHR_ERR_EXIT(_SHR_E_FAIL,
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SHR_IF_ERR_EXIT(dnx_l3_sllb_switch_control_sem_tests(unit,
                                                         ...
                                                         nof_ecmp,
                                                         nof_vip_ecmp,
                                                         &sllb_table_size, &vip_ecmp_fec_min_id, &vip_ecmp_fec_max_id));

    /*
     * bcm_l3_egress_ecmp_* semantic tests 
     */
    SHR_IF_ERR_EXIT(dnx_l3_sllb_ecmp_sem_tests(unit,
                                               sllb_table_size, nof_ecmp, vip_ecmp_fec_min_id, vip_ecmp_fec_max_id));

    /*
     * bcm_l3_egress_ecmp_resilient* semantic tests 
     */
    SHR_IF_ERR_EXIT(dnx_l3_sllb_resilient_sem_tests(unit, vip_ecmp_fec_min_id, vip_ecmp_fec_max_id, nof_ecmp));
