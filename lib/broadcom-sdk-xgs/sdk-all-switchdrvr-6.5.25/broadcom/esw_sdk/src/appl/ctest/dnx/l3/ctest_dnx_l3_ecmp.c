...
#include <src/bcm/dnx/mdb/mdb_init_internal.h>
...
/** ECMP visibility FER forward test details */
...
/** Custom user profile ECMP test details */
static sh_sand_man_t sh_dnx_ecmp_custom_user_profile_man = {
    "ECMP custom user profile testing",
    "Covers basic ECMP custom user profile testing, as well as Capacity and defragmentation testing"
};

/** Consistent ECMP test details */
static sh_sand_man_t sh_dnx_ecmp_memory_capacity_man = {
    "ECMP tables memory capacity test",
    "Tests that the maximum capacity of the different sized tables in the memory is as expected."
};
/*
 * }
 */
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(algo_l3_db.fec_db_info.
                    fec_db.get(unit, DNX_L3_FEC_DIRECTION_NA, DBAL_ENUM_FVAL_HIERARCHY_LEVEL_HIERARCHY_LEVEL_1,
                               &fec_db));
    SHR_IF_ERR_EXIT(mdb_init_fec_ranges_allocation_info_get(unit, fec_db, &base_fec, &fec_range));
    ...
    for (test_type = 0; test_type < DIAG_DNX_ECMP_CONST_NOF_TBLS_TEST; test_type++)
    {
             ...
             table_iter < DBAL_NOF_ENUM_ECMP_CONSISTENT_TABLE_VALUES; table_iter++)
        {
            ...
            switch (test_type)
            {
                ...
                {
                    ecmp.max_paths =
                        SAL_BIT(dnx_data_l3.ecmp.consistent_tables_info_get(unit, table_iter)->entry_size_in_bits);
                    intf_count =
                        SAL_BIT(dnx_data_l3.ecmp.consistent_tables_info_get(unit, table_iter)->entry_size_in_bits);
                    break;
                }
    ...
    for (intf_iter = 0; intf_iter < DNX_DATA_MAX_L3_ECMP_MAX_GROUP_SIZE; intf_iter++)
    {
        SHR_IF_ERR_EXIT(bcm_l3_egress_destroy(unit, intf_array_sent[intf_iter]));
    }
exit:
    ...
    sh_sand_control_t * sand_control)
{
    ...
    int intf_array_copy[3][DNX_DATA_MAX_L3_ECMP_MAX_GROUP_SIZE];
    int idx;
    ...
    int max_group_profiles = 0;
    int fec_dir = DNX_L3_FEC_DIRECTION_NA;
    ...
    uint32 hier_enums[3] =
        { DBAL_ENUM_FVAL_HIERARCHY_LEVEL_HIERARCHY_LEVEL_1, DBAL_ENUM_FVAL_HIERARCHY_LEVEL_HIERARCHY_LEVEL_2,
    ...
    max_group_size = 1 << dnx_data_l3.ecmp.group_size_multiply_and_divide_nof_bits_get(unit);
    max_group_profiles = 1 << dnx_data_l3.ecmp.profile_id_size_get(unit);

    for (hier_id = 0, idx = 0; hier_id < nof_hierarchies; hier_id++)
    {
        SHR_IF_ERR_EXIT(algo_l3_db.fec_db_info.fec_db.get(unit, fec_dir, hier_enums[hier_id], &fec_db));
        SHR_IF_ERR_EXIT(mdb_init_fec_ranges_allocation_info_get(unit, fec_db, &base_fec, &range_size));

        for (intf_iter = 0; intf_iter < max_group_size; intf_iter++)
        {
            fec_id = base_fec + intf_iter;
            bcm_l3_egress_t_init(&egr_intf);
            egr_intf.port = 200;
            egr_intf.flags = hier_flags[hier_id];
            SHR_IF_ERR_EXIT(bcm_l3_egress_create(unit, fec_flags, &egr_intf, &fec_id));
            intf_array[intf_iter] = fec_id;
        }
        sal_memcpy(&intf_array_copy[hier_id], intf_array, sizeof(int) * max_group_size);
        base_fec = 0;
    }
    sal_printf("Allocating groups with ID and different group profile \n");
    for (hier_id = 0, idx = 0; hier_id < nof_hierarchies; hier_id++)
    {
        int ecmp_start_temp = ecmp_intf_start + (hier_id * 0x2000);
        for (intf_iter = 0; intf_iter < max_group_size - intf_count; intf_iter++, idx++)
        {
            if (intf_iter == max_group_profiles)
            {
                sal_printf("All available profiles have been allocated!\n");
                break;
            }
            bcm_l3_egress_ecmp_t_init(&ecmp);
            ...
            SHR_IF_ERR_EXIT(bcm_l3_egress_ecmp_create(unit, &ecmp, intf_count + intf_iter, intf_array_copy[hier_id]));
            utilex_ll_timer_stop(timer_group, 1);
            ecmp_ids[idx] = ecmp.ecmp_intf;
        }
    }

    sal_printf("Allocating groups without ID and same group profile \n");
    for (hier_id = 0; hier_id < nof_hierarchies; hier_id++)
    {
        for (intf_iter = 0; intf_iter < max_group_size; intf_iter++, idx++)
        {
            bcm_l3_egress_ecmp_t_init(&ecmp);
            ecmp.dynamic_mode = BCM_L3_ECMP_DYNAMIC_MODE_DISABLED;
            ...
            SHR_IF_ERR_EXIT(bcm_l3_egress_ecmp_create(unit, &ecmp, intf_count, intf_array_copy[hier_id]));
    ...
    for (hier_id = 0; hier_id < nof_hierarchies; hier_id++)
    {
        for (intf_iter = 0; intf_iter < max_group_size; intf_iter++)
        {
            SHR_IF_ERR_EXIT(bcm_l3_egress_destroy(unit, intf_array_copy[hier_id][intf_iter]));
        }
    }
    ...
    sal_printf("AVERAGE time for bcm_l3_egress_ecmp_create WITH ID - %u us, expected approximately %d us\n",
               (total_time[0] / hits[0]), average_time[0]);
    sal_printf("AVERAGE time for bcm_l3_egress_ecmp_create without ID - %u us, expected approximately %d us\n",
    ...
    sh_sand_control_t * sand_control)
{
     ...
     */
    SHR_IF_ERR_EXIT(algo_l3_db.fec_db_info.
                    fec_db.get(unit, fec_dir, DBAL_ENUM_FVAL_HIERARCHY_LEVEL_HIERARCHY_LEVEL_1, &fec_db));
    SHR_IF_ERR_EXIT(mdb_init_fec_ranges_allocation_info_get(unit, fec_db, &base_fec, &range_size));
    ...
    bcm_l3_egress_ecmp_t * ecmp)
{
    ...
    int ecmp_start_temp = ecmp_intf_start + (hierarchy * dnx_data_l3.ecmp.ecmp_bank_size_get(unit));
    SHR_FUNC_INIT_VARS(unit);
...
/**
 * \brief
 *  custom profile entry init and create with the given parameters
 * \param [in] unit- Relevant unit
 * \param [in] udep_ecmp - the user profile ecmp structure to initialize
 * \param [in] ecmp_group_flags - the user profile ecmp_group_flags that needed to be added to the udep_ecmp structure
 * \param [in] intf_count - the user profile number of offsets in the intf_array
 * \param [in] intf_array - the array of offsets that the user profile is getting
 * \return
 *    shr_error_e
 */
shr_error_e
custom_profile_entry_init_and_create(
    int unit,
    bcm_l3_egress_ecmp_t * udep_ecmp,
    int ecmp_group_flags,
    int intf_count,
    bcm_if_t * intf_array,
    int user_profile_id)
{
    SHR_FUNC_INIT_VARS(unit);
    bcm_l3_egress_ecmp_t_init(udep_ecmp);
    udep_ecmp->ecmp_group_flags = ecmp_group_flags;
    udep_ecmp->dynamic_mode = BCM_L3_ECMP_DYNAMIC_MODE_RESILIENT;
    if (user_profile_id != 0)
    {
        udep_ecmp->user_profile = user_profile_id;
        udep_ecmp->flags = BCM_L3_WITH_ID;
    }
    else
    {
        udep_ecmp->flags = 0;
    }
    SHR_IF_ERR_EXIT(bcm_l3_egress_ecmp_user_profile_create(unit, udep_ecmp, intf_count, intf_array));
exit:
    SHR_FUNC_EXIT;
}
...
/**
 * \brief
 *  custom profile entry init and update with the given parameters
 * \param [in] unit- Relevant unit
 * \param [in] udep_ecmp - the user profile ecmp structure to initialize
 * \param [in] ecmp_group_flags - the user profile ecmp_group_flags that needed to be added to the udep_ecmp structure
 * \param [in] intf_count - the user profile number of offsets in the intf_array
 ...
 */
shr_error_e
custom_profile_entry_init_and_update(
    int unit,
    bcm_l3_egress_ecmp_t * udep_ecmp,
    int ecmp_group_flags,
    int intf_count,
    ...
    int user_profile_id)
{
    ...
    udep_ecmp->dynamic_mode = BCM_L3_ECMP_DYNAMIC_MODE_RESILIENT;
    udep_ecmp->user_profile = user_profile_id;
    udep_ecmp->flags = BCM_L3_WITH_ID | BCM_L3_REPLACE;
    SHR_IF_ERR_EXIT(bcm_l3_egress_ecmp_user_profile_create(unit, udep_ecmp, intf_count, intf_array));
exit:
    SHR_FUNC_EXIT;
}
 ...
 */
shr_error_e
consistent_table_entry_offset_get(
    int unit,
    uint32 user_profile_id,
    uint32 *table_offset)
{
    dnx_l3_ecmp_user_profile_t ecmp_profile_data;
    dbal_enum_value_field_ecmp_consistent_table_e consistent_table_type;
    SHR_FUNC_INIT_VARS(unit);
    sal_memset(&ecmp_profile_data, 0x0, sizeof(dnx_l3_ecmp_user_profile_t));
    SHR_IF_ERR_EXIT(dnx_algo_l3_ecmp_user_profile_data_get(unit, user_profile_id, &ecmp_profile_data));
    SHR_IF_ERR_EXIT(dnx_l3_ecmp_consistent_table_type_from_ecmp_mode_get
                    (unit, ecmp_profile_data.ecmp_mode, &consistent_table_type));
    *table_offset =
        ecmp_profile_data.consistent_members_table_addr * L3_ECMP_CONSISTENT_NOF_RESOURCES_TAKEN(unit,
...
/**
 * \brief
      Testing user profile capacity and defragmantation
      first we test full capacity - creating max nof user profiles and expecting to fail at the max_nof+1 user profile
      than we test defragmanation - creating max nof user profile with small consistent tables and than deleting some in
      order to create user profile with medium table and one user profile with large table.
 * \param [in]
 *    unit- Relevant unit
 * \param [in]
 *    args - The args to pass to the diagnostic function
 ...
 */
shr_error_e
sh_dnx_ecmp_custom_user_profile_cmd(
    int unit,
    args_t * args,
    sh_sand_control_t * sand_control)
{
    int table_size;
    int num_of_consistent_tables;
    int table_iter;
    int intf_count[DBAL_NOF_ENUM_ECMP_CONSISTENT_TABLE_VALUES];
    bcm_l3_egress_ecmp_t
        udep_ecmp[DBAL_NOF_ENUM_ECMP_CONSISTENT_TABLE_VALUES][L3_ECMP_CONSISTENT_TABLE_MAX_NOF_ENTRIES + 1];
    bcm_if_t intf_array[DBAL_NOF_ENUM_ECMP_CONSISTENT_TABLE_VALUES][L3_ECMP_CONSISTENT_TABLE_MAX_NOF_ENTRIES + 1] =
        { {0} };
    bcm_if_t intf_array_update[DBAL_NOF_ENUM_ECMP_CONSISTENT_TABLE_VALUES][L3_ECMP_CONSISTENT_TABLE_MAX_NOF_ENTRIES +
                                                                           1] = { {0} };
    int table_size_flags[DBAL_NOF_ENUM_ECMP_CONSISTENT_TABLE_VALUES] =
        { 0, BCM_L3_ECMP_MEDIUM_TABLE, BCM_L3_ECMP_LARGE_TABLE };
    bsl_severity_t original_severity_sw_state, original_res_mngr, original_l3, consistent_hashing, defrag_chunk;
    SHR_FUNC_INIT_VARS(unit);

    for (table_size = DBAL_ENUM_FVAL_ECMP_CONSISTENT_TABLE_SMALL_SIZE;
         table_size <= DBAL_ENUM_FVAL_ECMP_CONSISTENT_TABLE_LARGE_SIZE; table_size++)
    {
        intf_count[table_size] = dnx_data_l3.ecmp.consistent_tables_info_get(unit, table_size)->nof_entries;
    }
    /*
     * Try to allocate for each table size all the range of tables and expect to fail at the MAX+1 table.
     */
    {
        for (table_size = DBAL_ENUM_FVAL_ECMP_CONSISTENT_TABLE_SMALL_SIZE;
             table_size <= DBAL_ENUM_FVAL_ECMP_CONSISTENT_TABLE_LARGE_SIZE; table_size++)
        {
            ...
            for (table_iter = 0; table_iter < num_of_consistent_tables; table_iter++)
            {
                SHR_IF_ERR_EXIT(custom_profile_entry_init_and_create
                                (unit, &udep_ecmp[table_size][table_iter], table_size_flags[table_size],
                                 intf_count[table_size], intf_array[table_size], 0));
            }

            {
                /** Turn off expected error prints */
                ...
                SHR_GET_SEVERITY_FOR_MODULE_SPECIFIC(BSL_LS_BCMDNX_TEMPLATEMNGR, original_res_mngr);
                SHR_SET_SEVERITY_FOR_MODULE_SPECIFIC(BSL_LS_BCMDNX_TEMPLATEMNGR, bslSeverityFatal);
                SHR_GET_SEVERITY_FOR_MODULE_SPECIFIC(BSL_LS_BCMDNX_L3, original_l3);
                SHR_SET_SEVERITY_FOR_MODULE_SPECIFIC(BSL_LS_BCMDNX_L3, bslSeverityFatal);
                SHR_GET_SEVERITY_FOR_MODULE_SPECIFIC(BSL_LS_BCMDNX_UTILS, consistent_hashing);
                SHR_SET_SEVERITY_FOR_MODULE_SPECIFIC(BSL_LS_BCMDNX_UTILS, bslSeverityFatal);
                SHR_GET_SEVERITY_FOR_MODULE_SPECIFIC(BSL_LS_SHAREDSWDNX_DEFRAGCHUNKDNX, defrag_chunk);
                SHR_SET_SEVERITY_FOR_MODULE_SPECIFIC(BSL_LS_SHAREDSWDNX_DEFRAGCHUNKDNX, bslSeverityFatal);
            }
            SHR_IF_NOT_EXCEPT_ERR_EXIT(custom_profile_entry_init_and_create
                                       (unit, &udep_ecmp[table_size][table_iter + 1], table_size_flags[table_size],
                                        intf_count[table_size], intf_array[table_size], 0), _SHR_E_FULL);
            {
                /** Reset prints configuration to default */
                SHR_SET_SEVERITY_FOR_MODULE_SPECIFIC(BSL_LS_SWSTATEDNX_GENERAL, original_severity_sw_state);
                SHR_SET_SEVERITY_FOR_MODULE_SPECIFIC(BSL_LS_BCMDNX_TEMPLATEMNGR, original_res_mngr);
                SHR_SET_SEVERITY_FOR_MODULE_SPECIFIC(BSL_LS_BCMDNX_L3, original_l3);
                SHR_SET_SEVERITY_FOR_MODULE_SPECIFIC(BSL_LS_BCMDNX_UTILS, consistent_hashing);
                SHR_SET_SEVERITY_FOR_MODULE_SPECIFIC(BSL_LS_SHAREDSWDNX_DEFRAGCHUNKDNX, defrag_chunk);
            }
            for (table_iter = 0; table_iter < num_of_consistent_tables; table_iter++)
            {
                SHR_IF_ERR_EXIT(bcm_l3_egress_ecmp_user_profile_destroy(unit, &udep_ecmp[table_size][table_iter]));
            }
        }
        sal_printf("ECMP custom user profile full capacity test PASSED!\n");
    }

    /*
     * Defragmentation testing
     * 1. allocate the MAX number of UDEP with small tables (all should have unique members in the consistent hashing).
     * 2. allocate the MAX number of UDEP with small tables (all should have unique members in the consistent hashing).
     * 3. Destroy 8 of them (not consecutive) and place a large table
     * 4. Destroy another 4 of them (not consecutive) and place a medium table.
     * 5. Verify that all UDEP have the expected content of the consistent member table
     */
    {
        int remove_indices[DNX_DATA_MAX_L3_ECMP_NOF_CONSISTENT_RESOURCES + 1];
        int remove_indices_bitmap[DNX_DATA_MAX_L3_ECMP_NOF_CONSISTENT_RESOURCES + 1] = { 0 };
        int indice_iter;
        int nof_entries_small_table = 0;
        int entry_iter;
        int udep_iter;
        int nof_udep_to_create;
        int nof_indices_to_delete;
        bcm_if_t defrag_intf_array[DNX_DATA_MAX_L3_ECMP_NOF_CONSISTENT_RESOURCES +
                                   1][L3_ECMP_CONSISTENT_TABLE_MAX_NOF_ENTRIES] = { {0} };
        /*
         * Initializing the indices array in valid indices numbers in range [0..DNX_DATA_MAX_L3_ECMP_NOF_CONSISTENT_RESOURCES]
         */
        nof_indices_to_delete =
            L3_ECMP_CONSISTENT_NOF_RESOURCES_TAKEN(unit,
                                                   DBAL_ENUM_FVAL_ECMP_CONSISTENT_TABLE_MEDIUM_SIZE) +
            L3_ECMP_CONSISTENT_NOF_RESOURCES_TAKEN(unit, DBAL_ENUM_FVAL_ECMP_CONSISTENT_TABLE_LARGE_SIZE);
        nof_udep_to_create = dnx_data_l3.ecmp.nof_consistent_resources_get(unit);
        for (udep_iter = 0; udep_iter < DNX_DATA_MAX_L3_ECMP_NOF_CONSISTENT_RESOURCES; udep_iter++)
        {
            remove_indices[udep_iter] = udep_iter;
        }
        /*
         * Generating 12 indices from the indice range for deleting them
         */
        for (udep_iter = 0; udep_iter < nof_indices_to_delete; udep_iter++)
        {
            int cur_indice;
            int temp;
            cur_indice = rand() % (nof_udep_to_create - udep_iter);
            temp = remove_indices[nof_udep_to_create - udep_iter];
            remove_indices[nof_udep_to_create - udep_iter] = remove_indices[cur_indice];
            remove_indices[cur_indice] = temp;
            remove_indices_bitmap[cur_indice] = 1;
        }

        nof_entries_small_table =
            dnx_data_l3.ecmp.consistent_tables_info_get(unit,
                                                        DBAL_ENUM_FVAL_ECMP_CONSISTENT_TABLE_SMALL_SIZE)->nof_entries;
        if (nof_entries_small_table >
            dnx_data_l3.ecmp.consistent_tables_info_get(unit,
                                                        DBAL_ENUM_FVAL_ECMP_CONSISTENT_TABLE_SMALL_SIZE)->nof_entries)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "nof entries in consistent small table %d is bigger than max defined %d",
                         nof_entries_small_table, dnx_data_l3.ecmp.consistent_tables_info_get(unit,
                                                                                              DBAL_ENUM_FVAL_ECMP_CONSISTENT_TABLE_SMALL_SIZE)->nof_entries);
        }
        /*
         * Initializing the members array of all small consistent tables in unique numbers
         */
        for (table_iter = 1; table_iter <= nof_udep_to_create; table_iter++)
        {
            for (entry_iter = 0; entry_iter < nof_entries_small_table; entry_iter++)
            {
                defrag_intf_array[table_iter][entry_iter] =
                    rand() %
                    (SAL_BIT
                     (dnx_data_l3.ecmp.consistent_tables_info_get(unit,
        ...
        {
            SHR_IF_ERR_EXIT(custom_profile_entry_init_and_create
                            (unit, &udep_ecmp[DBAL_ENUM_FVAL_ECMP_CONSISTENT_TABLE_SMALL_SIZE][table_iter],
                             table_size_flags[DBAL_ENUM_FVAL_ECMP_CONSISTENT_TABLE_SMALL_SIZE],
                             intf_count[DBAL_ENUM_FVAL_ECMP_CONSISTENT_TABLE_SMALL_SIZE], defrag_intf_array[table_iter],
                             table_iter));
        }
        /*
         * Deleting 4 small tables and creating medium consistent table
         */
        for (indice_iter = 0;
             indice_iter < L3_ECMP_CONSISTENT_NOF_RESOURCES_TAKEN(unit,
                                                                  DBAL_ENUM_FVAL_ECMP_CONSISTENT_TABLE_MEDIUM_SIZE);
             indice_iter++)
        {
            SHR_IF_ERR_EXIT(bcm_l3_egress_ecmp_user_profile_destroy
                            (unit,
                             &udep_ecmp[DBAL_ENUM_FVAL_ECMP_CONSISTENT_TABLE_SMALL_SIZE][remove_indices
                                                                                         [nof_udep_to_create -
                                                                                          indice_iter]]));
        }
        SHR_IF_ERR_EXIT(custom_profile_entry_init_and_create
                        (unit, &udep_ecmp[DBAL_ENUM_FVAL_ECMP_CONSISTENT_TABLE_MEDIUM_SIZE][0],
                         table_size_flags[DBAL_ENUM_FVAL_ECMP_CONSISTENT_TABLE_MEDIUM_SIZE],
                         intf_count[DBAL_ENUM_FVAL_ECMP_CONSISTENT_TABLE_MEDIUM_SIZE],
                         intf_array[DBAL_ENUM_FVAL_ECMP_CONSISTENT_TABLE_MEDIUM_SIZE], 0));
        /*
         * Deleting 4 small tables and creating large consistent table
         */
             ...
             indice_iter++)
        {
            SHR_IF_ERR_EXIT(bcm_l3_egress_ecmp_user_profile_destroy
                            (unit,
                             &udep_ecmp[DBAL_ENUM_FVAL_ECMP_CONSISTENT_TABLE_SMALL_SIZE][remove_indices
                                                                                         [nof_udep_to_create -
                                                                                          L3_ECMP_CONSISTENT_NOF_RESOURCES_TAKEN
                                                                                          (unit,
                                                                                           DBAL_ENUM_FVAL_ECMP_CONSISTENT_TABLE_MEDIUM_SIZE)
                                                                                          - indice_iter]]));
        }
        SHR_IF_ERR_EXIT(custom_profile_entry_init_and_create
                        (unit, &udep_ecmp[DBAL_ENUM_FVAL_ECMP_CONSISTENT_TABLE_LARGE_SIZE][0],
                         table_size_flags[DBAL_ENUM_FVAL_ECMP_CONSISTENT_TABLE_LARGE_SIZE],
                         intf_count[DBAL_ENUM_FVAL_ECMP_CONSISTENT_TABLE_LARGE_SIZE],
                         intf_array[DBAL_ENUM_FVAL_ECMP_CONSISTENT_TABLE_LARGE_SIZE], 0));
        /*
         * compare members of all small table
         */

        for (table_iter = 1; table_iter <= nof_udep_to_create; table_iter++)
        {
            if (remove_indices_bitmap[table_iter] == 0)
            {
                int intf_count_get;
                bcm_l3_egress_ecmp_t get_ecmp;
                bcm_if_t intf_array_get[L3_ECMP_CONSISTENT_TABLE_MAX_NOF_ENTRIES + 1] = { 0 };
                get_ecmp.user_profile = table_iter;
                get_ecmp.flags = BCM_L3_WITH_ID;
                SHR_IF_ERR_EXIT(bcm_l3_egress_ecmp_user_profile_get
                                (unit, &get_ecmp, nof_entries_small_table, intf_array_get, &intf_count_get));
                if (intf_count_get != nof_entries_small_table)
                {
                    SHR_ERR_EXIT(_SHR_E_PARAM, "intf_count from get %d is not equal to original intf_count %d \n",
                                 intf_count_get, intf_count[DBAL_ENUM_FVAL_ECMP_CONSISTENT_TABLE_SMALL_SIZE]);
                }
                for (entry_iter = 0; entry_iter < nof_entries_small_table; entry_iter++)
                {
                    if (intf_array_get[entry_iter] != defrag_intf_array[table_iter][entry_iter])
                    {
                        SHR_ERR_EXIT(_SHR_E_PARAM,
                                     "offset array member %d in entry %d is not equal to original member %d in entry %d of table %d\n",
                                     intf_array_get[entry_iter], entry_iter, defrag_intf_array[table_iter][entry_iter],
                                     entry_iter, table_iter);
                    }
                }
                SHR_IF_ERR_EXIT(bcm_l3_egress_ecmp_user_profile_destroy
                                (unit, &udep_ecmp[DBAL_ENUM_FVAL_ECMP_CONSISTENT_TABLE_SMALL_SIZE][table_iter]));
        ...
        SHR_IF_ERR_EXIT(bcm_l3_egress_ecmp_user_profile_destroy(unit,
                                                                &udep_ecmp
                                                                [DBAL_ENUM_FVAL_ECMP_CONSISTENT_TABLE_MEDIUM_SIZE][0]));
        SHR_IF_ERR_EXIT(bcm_l3_egress_ecmp_user_profile_destroy
                        (unit, &udep_ecmp[DBAL_ENUM_FVAL_ECMP_CONSISTENT_TABLE_LARGE_SIZE][0]));

        sal_printf("ECMP custom user profile defragmantation test PASSED!\n");
    }

    {
        /** Update UDEP testing */
        uint32 table_offset_create = 0, table_offset_update = 0;
        int user_profile_id = 0;
        sal_printf("ECMP custom user profile update test run!\n");
        for (table_size = DBAL_ENUM_FVAL_ECMP_CONSISTENT_TABLE_SMALL_SIZE;
             table_size <= DBAL_ENUM_FVAL_ECMP_CONSISTENT_TABLE_LARGE_SIZE; table_size++)
        {
            num_of_consistent_tables =
            ...
            for (table_iter = 0; table_iter < num_of_consistent_tables; table_iter++)
            {
                user_profile_id = table_iter + 1;
                SHR_IF_ERR_EXIT(custom_profile_entry_init_and_create
                                (unit, &udep_ecmp[table_size][table_iter], table_size_flags[table_size],
                                 intf_count[table_size], intf_array[table_size], user_profile_id));
            }
            table_iter--;
            /** Get the table offset of the last created entry */
            SHR_IF_ERR_EXIT(consistent_table_entry_offset_get(unit, user_profile_id, &table_offset_create));
            sal_memcpy(intf_array_update[table_size], intf_array[table_size], sizeof(uint32) * intf_count[table_size]);
            intf_array_update[table_size][0]++;
            /** Update the last entry that was created in place */
            SHR_IF_ERR_EXIT(custom_profile_entry_init_and_update
                            (unit, &udep_ecmp[table_size][table_iter],
                             table_size_flags[table_size] | BCM_L3_ECMP_RESILIENT_REPLACE, intf_count[table_size],
                             intf_array_update[table_size], user_profile_id));
            SHR_IF_ERR_EXIT(consistent_table_entry_offset_get(unit, user_profile_id, &table_offset_update));
            if (table_offset_create != table_offset_update)
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL,
                             "Expected that after in place update, the table offset will not be updated\n");
            }
            {
                /** Turn off expected error prints */
                SHR_GET_SEVERITY_FOR_MODULE_SPECIFIC(BSL_LS_SWSTATEDNX_GENERAL, original_severity_sw_state);
            ...
            /** Safely update the last entry that was created - impossible because all entries are taken */
            SHR_IF_NOT_EXCEPT_ERR_EXIT(custom_profile_entry_init_and_update
                                       (unit, &udep_ecmp[table_size][table_iter], table_size_flags[table_size],
                                        intf_count[table_size], intf_array[table_size], user_profile_id), _SHR_E_FULL);
            {
                /** Reset prints configuration to default */
                SHR_SET_SEVERITY_FOR_MODULE_SPECIFIC(BSL_LS_SWSTATEDNX_GENERAL, original_severity_sw_state);
            ...
            /** Destroy one entry in order to have space for update */
            SHR_IF_ERR_EXIT(bcm_l3_egress_ecmp_user_profile_destroy(unit, &udep_ecmp[table_size][0]));

            /** Safely update the last created entry */
            SHR_IF_ERR_EXIT(custom_profile_entry_init_and_update
                            (unit, &udep_ecmp[table_size][table_iter], table_size_flags[table_size],
                             intf_count[table_size], intf_array[table_size], user_profile_id));

            /** Get the last created entry and validate that the table offset is changed */
            SHR_IF_ERR_EXIT(consistent_table_entry_offset_get(unit, user_profile_id, &table_offset_update));
            if (table_offset_create == table_offset_update)
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "Expected that after safe update, the table offset will be updated");
            }

            /** Delete all created entries */
            for (table_iter = 1; table_iter < num_of_consistent_tables; table_iter++)
            {
                SHR_IF_ERR_EXIT(bcm_l3_egress_ecmp_user_profile_destroy(unit, &udep_ecmp[table_size][table_iter]));
            }
        }
        sal_printf("ECMP custom user profile update test PASSED!\n");
    }
exit:
    SHR_FUNC_EXIT;
}
...
/**
 * \brief
 *    This function creates an ECMP according to the parameters.
 * \param [in]
 *    unit- Relevant unit
 * \param [in]
 *    flags- flags to be added to the bcm_l3_egress_ecmp_t struct that is passed as an argument to
 *    bcm_l3_egress_ecmp_create
 * \param [in]
 *    ecmp_intf- The ECMP ID
 * \param [in]
 *    max_paths- The max nof paths in the ECMP group that is passed as an argument to
 *    bcm_l3_egress_ecmp_create
 * \param [in]
 *    ecmp_group_flags- BCM_L3_ECMP_xxx flags
 * \param [in]
 *    dynamic_mode - Dynamic load balancing mode. See BCM_L3_ECMP_DYNAMIC_MODE_xxx definitions.
 * \param [in]
 *    intf_count - The ECMP current number of members in the group
 * \param [in]
 *    intf_array - Array of the ECMP members either FEC-IDs or ECMP-IDs
 * \return
 *    shr_error_e
 *
 * \remark
 *    See bcm_dnx_l3_egress_ecmp_create
 *
 * \see
 *    sh_dnx_pp_vis_fer_cmd
 */
static inline shr_error_e
sh_dnx_ecmp_create_ecmp(
    int unit,
    uint32 flags,
    bcm_if_t ecmp_intf,
    int max_paths,
    ...
    bcm_if_t * intf_array)
{
    ...
    ecmp.flags = flags;
    ecmp.ecmp_group_flags = ecmp_group_flags;
...
/**
 * \brief
 *    This test tests the capacity of the allocation of the ECMP tables in the memory
 *
 * \return
 *    shr_error_e
 *
 ...
 */
static shr_error_e
sh_dnx_ecmp_memory_capacity_cmd(
    int unit,
    args_t * args,
    sh_sand_control_t * sand_control)
{
    int table_size, ecmp_iter, fec_iter;
    int table_size_flags[] = { 0, BCM_L3_ECMP_MEDIUM_TABLE, BCM_L3_ECMP_LARGE_TABLE };
    ...
    for (fec_iter = 0; fec_iter < intf_count; fec_iter++)
    {
        bcm_l3_egress_t_init(&l3eg);
        l3eg.intf = arp_id;
        l3egid = fec_config.start + fec_iter;
         ...
         table_size <= DBAL_ENUM_FVAL_ECMP_CONSISTENT_TABLE_LARGE_SIZE; table_size++)
    {
        ...
        for (ecmp_iter = 0; ecmp_iter < (L3_ECMP_TOTAL_NOF_CONSISTENT_RESOURCES(unit) /
                                         L3_ECMP_CONSISTENT_NOF_RESOURCES_TAKEN(unit, table_size)); ecmp_iter++)
        {
            SHR_IF_ERR_EXIT(sh_dnx_ecmp_create_ecmp(unit, BCM_L3_WITH_ID, ecmp_count++,
                                                    SAL_BIT(dnx_data_l3.ecmp.consistent_tables_info_get(unit,
                                                                                                        table_size)->
                                                            entry_size_in_bits), table_size_flags[table_size],
                                                    BCM_L3_ECMP_DYNAMIC_MODE_RESILIENT, intf_count, fecs));
        }

        SHR_IF_NOT_EXCEPT_ERR_EXIT(sh_dnx_ecmp_create_ecmp(unit, BCM_L3_WITH_ID, ecmp_count++,
                                                           SAL_BIT(dnx_data_l3.ecmp.consistent_tables_info_get(unit,
                                                                                                               table_size)->entry_size_in_bits),
                                                           table_size_flags[table_size],
                                                           BCM_L3_ECMP_DYNAMIC_MODE_RESILIENT, intf_count, fecs),
                                   _SHR_E_FULL);
    ...
    sal_printf("\nECMP memory capacity test PASSED!\n");
...
sh_sand_cmd_t dnx_ecmp_test_cmds[] = {
    ...
    {"memory_capacity", sh_dnx_ecmp_memory_capacity_cmd, NULL, NULL, &sh_dnx_ecmp_memory_capacity_man, NULL, NULL, CTEST_POSTCOMMIT},
    {"ecmp_custom_user_profile", sh_dnx_ecmp_custom_user_profile_cmd, NULL, NULL, &sh_dnx_ecmp_custom_user_profile_man, NULL, NULL, CTEST_POSTCOMMIT * CTEST_UM},
    {NULL}
};
/* *INDENT-ON* */
