    ...
    sh_sand_control_t * sand_control)
{
    ...
    SHR_IF_ERR_EXIT(dnx_crps_mgmt_nof_counters_per_engine(unit, engine_id, format_type, &nof_counters));

    if (sw_counter_id >= nof_counters * DNX_CRPS_MGMT_SUB_COUNT)
    {
        SHR_CLI_EXIT(_SHR_E_PARAM, "counter=%d, nof_counters=%d\n", sw_counter_id,
                     nof_counters * DNX_CRPS_MGMT_SUB_COUNT);
    }
    ...
    DNXCMN_CORES_ITER(unit, core_id, core_idx)
    {
        SHR_IF_ERR_EXIT(dnx_crps_ctrs_db_counter_set
                        (unit, DNX_CRPS_CTRS_OVERWRITE_COUNTER_VALUE, core_idx, engine_id,
                         sw_counter_id / DNX_CRPS_MGMT_SUB_COUNT, (sw_counter_id & 0x1), format_type, value64));
    }
