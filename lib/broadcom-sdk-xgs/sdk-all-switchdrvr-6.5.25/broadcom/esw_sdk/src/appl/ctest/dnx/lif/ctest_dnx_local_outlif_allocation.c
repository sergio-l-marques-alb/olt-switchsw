...
lif_mngr_local_outlif_info_t stress_test_local_outlif_info_per_phase[DNX_ALGO_LOCAL_OUTLIF_LOGICAL_PHASE_COUNT] = {
     ...
     */
    {DBAL_TABLE_EEDB_RIF_BASIC, DBAL_RESULT_TYPE_EEDB_RIF_BASIC_ETPS_ETH_RIF, DNX_ALGO_LOCAL_OUTLIF_LOGICAL_PHASE_1},
    {DBAL_TABLE_EEDB_ARP, DBAL_RESULT_TYPE_EEDB_ARP_ETPS_ARP_AC, DNX_ALGO_LOCAL_OUTLIF_LOGICAL_PHASE_2},
    ...
    {DBAL_TABLE_EEDB_ARP, DBAL_RESULT_TYPE_EEDB_ARP_ETPS_ARP, DNX_ALGO_LOCAL_OUTLIF_LOGICAL_PHASE_7},
    {DBAL_TABLE_EEDB_OUT_AC, DBAL_RESULT_TYPE_EEDB_OUT_AC_ETPS_AC_STAT_PROTECTION, DNX_ALGO_LOCAL_OUTLIF_LOGICAL_PHASE_8}
};
    ...
    int target_nof_outlifs)
{
    ...
    for (current_lif_index = 0; current_lif_index < nof_outlifs; current_lif_index++)
    {
        ...
        rv = (dnx_lif_lib_allocate(unit, 0, &global_lif_info, NULL, local_outlif_info));
        ...
        stress_test_allocated_global_lifs[logical_phase][outlif_array_start_index + current_lif_index] =
            global_lif_info.global_lif;
    }
    ...
    int fail_if_external_memories_are_unused)
{
    ...
    for (current_logical_phase = DNX_ALGO_LOCAL_OUTLIF_LOGICAL_PHASE_FIRST;
         current_logical_phase < DNX_ALGO_LOCAL_OUTLIF_LOGICAL_PHASE_COUNT; current_logical_phase++)
    {
        cli_out("Logical Phase %d: 0x%x outlifs were allocated.\n", current_logical_phase + 1,
    ...
    rhlist_t * invoke_list)
{
    ...
    /*
     * If the eedb traffic lock feature is in effect, then the test becomes completely irrelevant
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if ((sal_strncmp
         ("balanced", mdb_profile_name,
          sal_strnlen(mdb_profile_name, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH)) == 0)
        ||
        (sal_strncmp
         ("balanced-exem", mdb_profile_name,
          sal_strnlen(mdb_profile_name, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH)) == 0))
    {
    ...
    sh_sand_control_t * sand_control)
{
    ...
    local_outlif_info->logical_phase = DNX_ALGO_LOCAL_OUTLIF_LOGICAL_PHASE_2;
    ...
    local_outlif_info->logical_phase = DNX_ALGO_LOCAL_OUTLIF_LOGICAL_PHASE_5;
    local_outlif_info->local_outlif = local_outlif_2;
    SHR_IF_ERR_EXIT(dnx_lif_lib_allocate(unit, 0, &global_lif_info, NULL, local_outlif_info));

    SHR_IF_ERR_EXIT(dnx_lif_mngr_outlif_sw_info_get
                    (unit, local_outlif_1, &dbal_table_id, NULL, &logical_phase, NULL, NULL, NULL));
    if (logical_phase != DNX_ALGO_LOCAL_OUTLIF_LOGICAL_PHASE_2)
    {
        SHR_ERR_EXIT(_SHR_E_FAIL, "Received logical phase %d, expected logical phase %d for local outlif %d",
                     logical_phase, DNX_ALGO_LOCAL_OUTLIF_LOGICAL_PHASE_2, local_outlif_1);
    }

    SHR_IF_ERR_EXIT(dnx_lif_mngr_outlif_sw_info_get
                    (unit, local_outlif_2, &dbal_table_id, NULL, &logical_phase, NULL, NULL, NULL));
    if (logical_phase != DNX_ALGO_LOCAL_OUTLIF_LOGICAL_PHASE_5)
    {
        SHR_ERR_EXIT(_SHR_E_FAIL, "Received logical phase %d, expected logical phase %d for local outlif %d",
                     logical_phase, DNX_ALGO_LOCAL_OUTLIF_LOGICAL_PHASE_5, local_outlif_2);
    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (current_iteration = 0; current_iteration < 2; current_iteration++)
    {
        ...
        for (current_outlif = 0; current_outlif < CTEST_DNX_LOCAL_OUTLIF_SEMANTIC_TEST_NOF_OUTLIFS; current_outlif++)
        {
            ...
            local_outlif_info = &(semantic_test_local_outlif_info_per_ll_and_size[use_ll][entry_size_index]);
            local_outlif_info->logical_phase = stress_test_local_outlif_info_per_phase[current_phase].logical_phase;
            local_outlif_info->local_lif_flags = DNX_ALGO_LIF_MNGR_LOCAL_OUTLIF_IGNORE_PHASE_ALLOC_MISMATCH;
            local_outlif_info->local_outlif = 0;

            (global_lif_info.global_lif) = global_lif_array[current_phase][use_ll][entry_size_index];
            SHR_IF_ERR_EXIT(dnx_lif_lib_allocate(unit, 0, &global_lif_info, NULL, local_outlif_info));

            global_lif_array[current_phase][use_ll][entry_size_index] = (global_lif_info.global_lif);
        ...
        for (current_phase = 0; current_phase < DNX_ALGO_LOCAL_OUTLIF_LOGICAL_PHASE_COUNT; current_phase++)
        {
            for (use_ll = 0; use_ll < SEMANTIC_TEST_NOF_LL_OPTIONS; use_ll++)
            {
                ...
                if (!(dnx_data_mdb.feature.feature_get(unit, dnx_data_mdb_feature_eedb_bank_traffic_lock) && use_ll))
                {
                    /*
                     * If we have the eedb bank traffic lock feature, then the outlifs with and without linked list will
    ...
    int should_be_reused)
{
    ...
    if (!can_be_reused)
    {
        ...
        local_lif_payload.local_lifs_array.local_lifs[0] = local_outlif_info->local_outlif;
    ...
    sh_sand_control_t * sand_control)
{
    ...
    /**
     * First create a 120b entry.
     */
    sal_memset(&local_outlif_info, 0, sizeof(lif_mngr_local_outlif_info_t));
    local_outlif_info.dbal_table_id = DBAL_TABLE_EEDB_MPLS_TUNNEL;
    local_outlif_info.dbal_result_type = DBAL_RESULT_TYPE_EEDB_MPLS_TUNNEL_ETPS_MPLS_1_STAT;
    local_outlif_info.logical_phase = DNX_ALGO_LOCAL_OUTLIF_LOGICAL_PHASE_5;
    local_outlif_info.local_lif_flags = DNX_ALGO_LIF_MNGR_LOCAL_OUTLIF_IGNORE_PHASE_ALLOC_MISMATCH;
    local_outlif_info.local_outlif = 0;
     ...
     */
    SHR_IF_ERR_EXIT(ctest_dnx_local_outlif_allocation_replace(unit, &local_outlif_info, global_lif_info.global_lif,
                                                              DBAL_RESULT_TYPE_EEDB_MPLS_TUNNEL_ETPS_MPLS_1, 1, TRUE));
...
exit:
     ...
     * Now free the outlif.
     */
    rv = dnx_lif_lib_free(unit, global_lif_info.global_lif, NULL, local_outlif_info.local_outlif);

    cli_out("Local outlif semantic test - %s\n", (SHR_FUNC_ERR() || rv) ? "failed :(" : "success!");
...
/**
 * \brief
 *    Test that validates correct mdb phase mapping and un-mapping:
 *    1. Read bank_allocation_size from dnx data.
 *    2. Allocate #bank_allocation_size lifs on the same logical
 *    phases.
 *    3. Read mdb_phase_map memory.
 *    4. Delete #bank_allocation_size lifs.
 *    5. Validate clearance of the correct mdb phase map
 *    entries.
 */
shr_error_e
    ...
    sh_sand_control_t * sand_control)
{
    ...
    int num_lifs_in_phase, lif_idx, is_end, num_current_rows, mdb_phase_map_key, mdb_phase_map_value,
    ...
    dnx_algo_local_outlif_logical_phase_e logical_phase_idx;
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);

    local_outlifs = sal_alloc(sizeof(int) * eedb_bank_size, "eedb_bank_size");

    outlif_info.dbal_table_id = DBAL_TABLE_EEDB_IPV6_DIP;
    ...
    for (logical_phase_idx = DNX_ALGO_LOCAL_OUTLIF_LOGICAL_PHASE_FIRST;
         logical_phase_idx < DNX_ALGO_LOCAL_OUTLIF_LOGICAL_PHASE_COUNT; logical_phase_idx++)
    {
        num_lifs_in_phase = 0;
        outlif_info.logical_phase = logical_phase_idx;
        rv = _SHR_E_NONE;
        ...
        sal_printf("\nAllocated %d lifs in logical phase #%d. Now delete them\n", num_lifs_in_phase,
                   logical_phase_idx + 1);
        ...
        while (!is_end)
        {
            ...
            {
                SHR_ERR_EXIT(_SHR_E_FAIL,
                             "Redundant mapping found - [key, val] == [0x%x, 0x%x] is a leftover for logical phase #%d \n",
                             mdb_phase_map_key, mdb_phase_map_value, logical_phase_idx + 1);
            }
            num_current_rows++;
            SHR_IF_ERR_EXIT(dbal_iterator_get_next(unit, entry_handle_id, &is_end));
        }
...
ctest_dnx_local_outlif_allocation_capacity_test(
    ...
    sh_sand_control_t * sand_control)
{
    int total_outlifs = 0, rv, lif_idx, lifs_in_allocation_bank;
    uint32 num_clear_lifs = 0;
    int clear_lifs_w_ll_per_stage[DNX_ALGO_LOCAL_OUTLIF_LOGICAL_PHASE_COUNT] = { 0 };
    int clear_lifs_wo_ll_per_stage[DNX_ALGO_LOCAL_OUTLIF_LOGICAL_PHASE_COUNT] = { 0 };
    lif_mngr_local_outlif_info_t outlif_info = { 0 };
    int num_lifs_per_phase[DNX_ALGO_LOCAL_OUTLIF_LOGICAL_PHASE_COUNT] = { 0 };
    char *dbal_table;
    ...
    else if (num_clear_lifs > 0)
    {
        ...
        clear_lifs_wo_ll =
            sal_alloc(sizeof(int) * num_clear_lifs * DNX_ALGO_LOCAL_OUTLIF_LOGICAL_PHASE_COUNT, "eedb_bank_size");
        clear_lifs_with_ll =
            sal_alloc(sizeof(int) * num_clear_lifs * DNX_ALGO_LOCAL_OUTLIF_LOGICAL_PHASE_COUNT, "eedb_bank_size");
    }
         ...
         logical_phase_idx < DNX_ALGO_LOCAL_OUTLIF_LOGICAL_PHASE_COUNT; logical_phase_idx++)
    {
        ...
        for (lif_idx = 0; lif_idx < lifs_in_allocation_bank; lif_idx++)
        {
            ...
            num_lifs_per_phase[logical_phase_idx]++;
            if ((num_clear_lifs) && (!clear_all_lifs))
            {
                if (clear_lifs_w_ll_per_stage[logical_phase_idx]++ < num_clear_lifs)
                {
         ...
         logical_phase_idx < DNX_ALGO_LOCAL_OUTLIF_LOGICAL_PHASE_COUNT; logical_phase_idx++)
    {
        ...
        for (lif_idx = 0; lif_idx < lifs_in_allocation_bank; lif_idx++)
        {
            ...
            if ((num_clear_lifs) && (!clear_all_lifs))
            {
                if (clear_lifs_wo_ll_per_stage[logical_phase_idx]++ < num_clear_lifs)
                {
         ...
         logical_phase_idx < DNX_ALGO_LOCAL_OUTLIF_LOGICAL_PHASE_COUNT; logical_phase_idx++)
    {
        ...
        total_outlifs += num_lifs_per_phase[logical_phase_idx];
        sal_printf("Capacity for logical phase #%d reached: %d.\n", logical_phase_idx + 1,
                   num_lifs_per_phase[logical_phase_idx]);
    }
