...
/*
 * Number of 'mpls labels' which are loaded into the DBAL_TABLE_MPLS_SKIP_TCAM_DB table
 * before calling 'traverse' to delete all of them.
 */
#define NUM_ENTRIES_FOR_TRAVERSE    20
/*
 * First label loaded into the the DBAL_TABLE_MPLS_SKIP_TCAM_DB table
    ...
    void *user_data_p)
{
    ...
    LOG_INFO_EX(BSL_LOG_MODULE,
                ...
                __func__, __LINE__, label_info_p->label_value, *label_type_p);
    ...
    sh_sand_control_t * sand_control)
{
    ...
    LOG_INFO_EX(BSL_LOG_MODULE,
                ...
                __func__, __LINE__, label, (label + count - 1));
...
static sh_sand_man_t Dnx_mpls_tunnel_frr_traverse_test_man = {
    ...
    /** Full */
    "Semantic test of MPLS TUNNEL FRR traverse API\n"
        "  This test loads a fixed number of entries into the DBAL_TABLE_MPLS_SKIP_TCAM_DB\r\n"
        ...
        for (ii = 0; ii < count; ii++)
        {
            ...
            if (shr_error != _SHR_E_NONE)
            {
                LOG_INFO_EX(BSL_LOG_MODULE,
                            ...
                            __func__, __LINE__, label_info.label_value, EMPTY);
            }
            else
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL,
            ...
            if (shr_error != _SHR_E_NONE)
            {
                if (shr_error == _SHR_E_NOT_FOUND)
                {
                    LOG_INFO_EX(BSL_LOG_MODULE,
                                ...
                                __func__, __LINE__, label_info_get.label_value, EMPTY);
                }
                else
                {
                    SHR_ERR_EXIT(_SHR_E_INTERNAL,
...
static sh_sand_man_t Dnx_mpls_tunnel_frr_add_test_man = {
    ...
    "Semantic test of MPLS TUNNEL_FRR add/get/delete API\n"
        "  'label' indicates the special FRR MPLS label to be added to the MPLS_SKIP_TCAM_DB table\n"
        "  'count' indicates the number of consequent labels to be added to the MPLS_SKIP_TCAM_DB table\n"
        "  'clean' indicates whether to delete entries added to the MPLS_SKIP_TCAM_DB table, during this test\n"
