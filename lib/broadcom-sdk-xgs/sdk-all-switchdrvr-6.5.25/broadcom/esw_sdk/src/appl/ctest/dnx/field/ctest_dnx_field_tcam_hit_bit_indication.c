...
static dnx_field_utils_packet_info_t Ctest_tcam_hit_bit_indication_packet_info = {
    {"PTCH_2", "PTCH_2.PP_SSP", "27", "ETH1", "IPv4", "IPv4.SIP", "0.0.0.2", "IPv4.DIP", "0.0.1.2", ""}
};
    ...
    bcm_field_entry_t * entry_handle_p)
{
    ...
    SHR_IF_ERR_EXIT(bcm_field_group_add(unit, 0, &fg_info, fg_id_p));
 ...
 * \brief
 *  This function is creating TCAM FG (attaching it
 *  to the default context). Adding 16 entries to the FG. All config is done
 *  on IPMF1 stage.
 *
 * \param [in] unit - Device ID
 * \param [out] fg_id_p - Created Field Group id
 ...
 */
static shr_error_e
ctest_dnx_field_tcam_hit_bit_indication_multi_entries_config(
    int unit,
    bcm_field_group_t * fg_id_p,
    bcm_field_entry_t * entry_handles)
{
    bcm_field_group_info_t fg_info;
    bcm_field_group_attach_info_t attach_info;
    bcm_field_entry_info_t entry_info;
    ...
    for (entry_iter = 0; entry_iter < CTEST_DNX_FIELD_TCAM_HIT_BIT_INDICATION_NOF_ENTRIES; entry_iter++)
    {
        /** Add entry to the created field group. */
        SHR_IF_ERR_EXIT(bcm_field_entry_add(unit, 0, *fg_id_p, &entry_info, &entry_handles[entry_iter]));
        entry_info.entry_qual[0].value[0] += 1;

        LOG_INFO_EX(BSL_LOG_MODULE, "Entry ID 0x%X (%d) was added to Field Group ID %d. \n%s", entry_handles[entry_iter], entry_handles[entry_iter], *fg_id_p, EMPTY);
    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    bcm_field_entry_t entry_handles[CTEST_DNX_FIELD_TCAM_HIT_BIT_INDICATION_NOF_ENTRIES];
    int entry_iter;
    ...
    LOG_INFO_EX(BSL_LOG_MODULE,
                    "%s(), line %d. Tesing 'HIT BIT indication GET/CLEAR flags'. %s%s\r\n", __func__, __LINE__, EMPTY, EMPTY);
    ...
    if (hit_bit == FALSE)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL,
                     "Entry HIT BIT indication is FALSE (0x%x), when the entry must match! FG ID %d and ENTRY ID %d.\n",
    ...
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL,
                     "Entry HIT BIT indication is FALSE (0x%x), when it shouldn't be cleared after DBAL_ENTRY_ATTR_HIT_CLEAR! FG ID %d and ENTRY ID %d.\n",
                     hit_bit, fg_id, entry_handle);
    }
    ...
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL,
                     "Entry HIT BIT indication is 0x%x, when it should be FALSE after CLEAR--->GET operations! FG ID %d and ENTRY ID %d.\n",
                     hit_bit, fg_id, entry_handle);
    }
    ...
    LOG_INFO_EX(BSL_LOG_MODULE,
                    "%s(), line %d. Tesing 'HIT BIT indication GET/CLEAR flags finished successfully'. %s%s\r\n", __func__, __LINE__, EMPTY, EMPTY);

    LOG_INFO_EX(BSL_LOG_MODULE,
                    "%s(), line %d. Tesing 'HIT BIT indication multi entries'. %s%s\r\n", __func__, __LINE__, EMPTY, EMPTY);

    SHR_IF_ERR_EXIT(ctest_dnx_field_tcam_hit_bit_indication_multi_entries_config(unit, &fg_id, entry_handles));
    SHR_IF_ERR_EXIT(dnx_field_group_dbal_table_id_get(unit, fg_id, &dbal_table_id));
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, dbal_table_id, &entry_handle_id));
    for (entry_iter = 1; entry_iter <= CTEST_DNX_FIELD_TCAM_HIT_BIT_INDICATION_NOF_ENTRIES; entry_iter++)
    {
        sal_snprintf(Ctest_tcam_hit_bit_indication_packet_info.header_info[6], DNX_FIELD_UTILS_STR_SIZE, "0.0.0.%d", entry_iter);
        SHR_IF_ERR_EXIT(ctest_dnx_field_utils_packet_tx(unit, 200, &Ctest_tcam_hit_bit_indication_packet_info));

        hitbit_flags = DBAL_ENTRY_ATTR_HIT_GET;
        SHR_IF_ERR_EXIT(dbal_entry_handle_access_id_set(unit, entry_handle_id, entry_handles[(entry_iter - 1)]));
        SHR_IF_ERR_EXIT(dbal_entry_attribute_request(unit, entry_handle_id, hitbit_flags, &hit_bit));
        /*
         * Dummy core set, real core_id is resolved from access_id at the access layer 
         */
        ...
        if (hit_bit == FALSE)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL,
                         "Entry HIT BIT indication is FALSE (0x%x), when the entry must match! FG ID %d and ENTRY ID %d.\n",
                         hit_bit, fg_id, entry_handles[(entry_iter - 1)]);
        }
    }

    /** Clear first 8 entries hit indications. */
    for (entry_iter = 0; entry_iter < (CTEST_DNX_FIELD_TCAM_HIT_BIT_INDICATION_NOF_ENTRIES / 2); entry_iter++)
    {
        SHR_IF_ERR_EXIT(bcm_field_entry_hit_flush(unit, 0, entry_handles[entry_iter]));
    }

    /** Hit first entry with SIP == 0.0.0.1 then check all other entries. */
    sal_snprintf(Ctest_tcam_hit_bit_indication_packet_info.header_info[6], DNX_FIELD_UTILS_STR_SIZE, "0.0.0.%d", 1);
    SHR_IF_ERR_EXIT(ctest_dnx_field_utils_packet_tx(unit, 200, &Ctest_tcam_hit_bit_indication_packet_info));
    for (entry_iter = 0; entry_iter < CTEST_DNX_FIELD_TCAM_HIT_BIT_INDICATION_NOF_ENTRIES; entry_iter++)
    {
        hitbit_flags = DBAL_ENTRY_ATTR_HIT_GET;
        SHR_IF_ERR_EXIT(dbal_entry_handle_access_id_set(unit, entry_handle_id, entry_handles[entry_iter]));
        SHR_IF_ERR_EXIT(dbal_entry_attribute_request(unit, entry_handle_id, hitbit_flags, &hit_bit));
        /*
         * Dummy core set, real core_id is resolved from access_id at the access layer 
         */
        ...
        /** Check that entries with array indexes 1-7 in entry_handles[] are not hit. */
        if ((entry_iter > 0) && (entry_iter < (CTEST_DNX_FIELD_TCAM_HIT_BIT_INDICATION_NOF_ENTRIES / 2)))
        {
            if (hit_bit != FALSE)
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL,
                             "Entry HIT BIT indication is 0x%x, when it should be FALSE! FG ID %d and ENTRY ID %d.\n",
                             hit_bit, fg_id, entry_handles[entry_iter]);
            }
        }
        else
        {
    ...
    for (entry_iter = 0; entry_iter < CTEST_DNX_FIELD_TCAM_HIT_BIT_INDICATION_NOF_ENTRIES; entry_iter++)
    {
        SHR_IF_ERR_EXIT(bcm_field_entry_delete(unit, fg_id, NULL, entry_handles[entry_iter]));
    }
    SHR_IF_ERR_EXIT(bcm_field_group_context_detach(unit, fg_id, BCM_FIELD_CONTEXT_ID_DEFAULT));
    SHR_IF_ERR_EXIT(bcm_field_group_delete(unit, fg_id));

    LOG_INFO_EX(BSL_LOG_MODULE,
                    "%s(), line %d. Tesing 'HIT BIT indication multi entries finished successfully'. %s%s\r\n", __func__, __LINE__, EMPTY, EMPTY);
