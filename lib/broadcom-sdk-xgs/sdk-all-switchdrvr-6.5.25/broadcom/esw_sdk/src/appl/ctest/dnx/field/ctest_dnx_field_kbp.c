...
#define KBP_FG_ID                                   14
#define KBP_CTX_ID                                  DBAL_ENUM_FVAL_KBP_FORWARD_APP_TYPES_NOP
    ...
    dnx_field_group_t * fg_id_p)
{
    ...
    fg_info.fg_type = DNX_FIELD_GROUP_TYPE_EXTERNAL_TCAM;
    sal_strncpy_s(fg_info.name, "KBP_FG_1", sizeof(fg_info.name));

    fg_info.dnx_quals[0] = DNX_QUAL(DNX_FIELD_QUAL_CLASS_META, DNX_FIELD_STAGE_EXTERNAL, DNX_FIELD_QUAL_PP_PORT);
    fg_info.dnx_quals[1] = DNX_QUAL(DNX_FIELD_QUAL_CLASS_META, DNX_FIELD_STAGE_EXTERNAL, DNX_FIELD_QUAL_DP);
    ...
    uint32 *entry_handle)
{
         ...
         (key_template.key_qual_map[qual_ndx].qual_type != DNX_FIELD_QUAL_TYPE_INVALID); qual_ndx++)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_field_map_dnx_qual_size
                        (unit, DNX_FIELD_STAGE_EXTERNAL, entry_info.key_info.qual_info[qual_ndx].dnx_qual,
                         NULL, NULL, &qual_size));
        /*
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (sal_strncasecmp(field_kbp_test_type_name, TEST_TYPE_KBP_SIMPLE, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) ==
        0)
    {
        ...
        SHR_IF_ERR_EXIT(appl_dnx_kbp_simple(unit, field_kbp_test_clean));
             ...
             (field_kbp_test_type_name, TEST_TYPE_KBP_TIMING, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
    {
