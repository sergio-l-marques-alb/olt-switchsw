    ...
    uint32 nof_mc_ids)
{
    ...
    rv = (shr_error_e) sh_process_command(unit, "debug bcmdnx multicast -");
    if (rv != _SHR_E_NONE)
    {
...
/**
 * \brief - Set ot add replications to multicast groups in random order and with random CUD, verify that they were successfully added/set
 *
 * \par DIRECT_INPUT:
 *   \param [in] unit - Unit ID
 *   \param [in] flags - flags for the multicast groups
 ...
 *   \param [in] nof_repl - number of replications to be set, can be bigger that the gport_size. In that case some of the gports will be repeated with different CUDs
    ...
    for (mc_id_index = 0; mc_id_index < nof_mc_ids; ++mc_id_index)
    {
        ...
        for (repl_index = 0; repl_index < nof_repl; repl_index++)
        {
            ...
            else
            {
                 ...
                 */
                cud = repl_index % DNX_MULTICAST_MAX_CUD;
                repl_in[repl_index].encap1 = cud;
            }
        }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (sal_strncasecmp(mc_type_name, "ing", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
    {
        is_ing = TRUE;
    }
    else if (sal_strncasecmp(mc_type_name, "egr", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
    {
        is_ing = FALSE;
    }
    else if (sal_strncasecmp(mc_type_name, "all", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
    {
        is_ing = -1;
    }
    else
    ...
    if (sal_strncasecmp(test_type_name, "single", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
    {
    ...
    else if (sal_strncasecmp(test_type_name, "multiple", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
    {
        if (sal_strncasecmp(mc_type_name, "all", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) != 0)
        {
    ...
    else if (sal_strncasecmp(test_type_name, "full", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
    {
        LOG_CLI((BSL_META
