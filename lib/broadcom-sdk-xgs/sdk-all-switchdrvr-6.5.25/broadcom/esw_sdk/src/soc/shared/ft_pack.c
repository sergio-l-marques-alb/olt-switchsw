...
shr_ft_msg_ctrl_collector_create_pack(uint8* buf, shr_ft_msg_ctrl_collector_create_t *msg)
{
    ...
    if (ft_msg_version >= FT_MSG_VERSION_7) {
    ...
    if (ft_msg_version >= FT_MSG_VERSION_6) {
        _SHR_PACK_U32(buf, msg->domain_id);
    }
    return buf;
}
...
shr_ft_msg_ctrl_collector_create_unpack(uint8* buf, shr_ft_msg_ctrl_collector_create_t *msg)
{
    ...
    if (ft_msg_version >= FT_MSG_VERSION_7) {
        _SHR_UNPACK_U16(buf, msg->num_records);
    }
                                         ...
                                         shr_ft_msg_ctrl_group_flow_data_get_t *msg)
{
    ...
    if (ft_msg_version >= FT_MSG_VERSION_8) {
        for(i = 0; i < 16; i++) {
            _SHR_PACK_U8(buf, msg->src_ipv6_addr[i]);
        }
        for(i = 0; i < 16; i++) {
            _SHR_PACK_U8(buf, msg->dst_ipv6_addr[i]);
        }
        for(i = 0; i < 6; i++) {
            _SHR_PACK_U8(buf, msg->src_mac_addr[i]);
        }
        for(i = 0; i < 6; i++) {
            _SHR_PACK_U8(buf, msg->dst_mac_addr[i]);
        }
        _SHR_PACK_U16(buf, msg->outer_vlan);
        _SHR_PACK_U16(buf, msg->ether_type);
    }
    return buf;
}
                                          ...
                                          shr_ft_msg_ctrl_group_flow_data_get_t *msg)
{
    ...
    if (ft_msg_version >= FT_MSG_VERSION_8) {
        for(i = 0; i < 16; i++) {
            _SHR_UNPACK_U8(buf, msg->src_ipv6_addr[i]);
        }
        for(i = 0; i < 16; i++) {
            _SHR_UNPACK_U8(buf, msg->dst_ipv6_addr[i]);
        }
        for(i = 0; i < 6; i++) {
            _SHR_UNPACK_U8(buf, msg->src_mac_addr[i]);
        }
        for(i = 0; i < 6; i++) {
            _SHR_UNPACK_U8(buf, msg->dst_mac_addr[i]);
        }
        _SHR_UNPACK_U16(buf, msg->outer_vlan);
        _SHR_UNPACK_U16(buf, msg->ether_type);
    }
    return buf;
}
                                      ...
                                      shr_ft_msg_ctrl_stats_export_t *msg)
{
    ...
    if (ft_msg_version >= FT_MSG_VERSION_7) {
        _SHR_PACK_U32(buf, msg->num_pkt_export_fails);
    }
                                        ...
                                        shr_ft_msg_ctrl_stats_export_t *msg)
{
    ...
    if (ft_msg_version >= FT_MSG_VERSION_7) {
        _SHR_UNPACK_U32(buf, msg->num_pkt_export_fails);
    }
