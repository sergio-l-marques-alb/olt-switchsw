...
/* Sense Resistors Configuration Table for Trident3x1 56175/56176 */
static i2c_ltc_t sense_resistor_config_56175[TD3X1_MAX_VS_CONFIG]={
    { 0, "Core",     I2C_LTM4676A_40, I2C_LTM4676A_CH0,     1,  0 },
};
#endif
...
ltc388x_board_resistor_match(int unit, int in_offset)
{
    ...
    if ((SOC_CONTROL(unit)->board_type == 0xCD) ||
        (SOC_CONTROL(unit)->board_type == 0xCE)) {
        return SOC_E_NONE;
    }
#endif
...
ltc388x_setmin_max(int unit, int devno, int len)
{
    ...
    if ((SOC_CONTROL(unit)->board_type == 0xCD) ||
        (SOC_CONTROL(unit)->board_type == 0xCE)) {
        max_vs_config = sizeof(sense_resistor_config_56175)/sizeof(i2c_ltc_t);
    }
#endif
...
#if defined(BCM_SABER2_SUPPORT) || defined (BCM_HURRICANE4_SUPPORT)
        if(!SOC_IS_SABER2(unit) && !SOC_IS_HURRICANE4(unit)) {
#endif
                                                        ...
                                                        unsigned short voltage)
{
    ...
    /* LTM4676A does not support command POWER_GOOD_ON/POWER_GOOD_OFF */
    if ((soc_i2c_devtype(unit, devno) != LTC3884_DEVICE_TYPE) &&
        (soc_i2c_devtype(unit, devno) != LTM4676A_DEVICE_TYPE)) {
...
#if defined (BCM_APACHE_SUPPORT) || defined (BCM_HELIX5_SUPPORT) || defined (BCM_HURRICANE4_SUPPORT) || defined (BCM_FIREBOLT6_SUPPORT)
    if ((SOC_CONTROL(unit)->board_type == 0x94) ||
        (SOC_CONTROL(unit)->board_type == 0x96) ||
        ...
        (SOC_CONTROL(unit)->board_type == 0xCD) ||
        (SOC_CONTROL(unit)->board_type == 0xCE)) {
...
#endif /* #if defined (BCM_APACHE_SUPPORT) || defined (BCM_HELIX5_SUPPORT) || defined (BCM_HURRICANE4_SUPPORT) */
...
ltc388x_read_current(int unit, int devno, void* data, int ch)
{
    ...
    i2cbus = I2CBUS(unit);
    for( i=0; i < max_vs_config; i++ ) {
#if defined(BCM_FIREBOLT6_SUPPORT) || defined (BCM_HURRICANE4_SUPPORT)
        if(SOC_IS_FIREBOLT6(unit) || SOC_IS_HURRICANE4(unit)) {
            continue;
        }
#endif
...
ltc388x_ioctl(int unit, int devno, int opcode, void* data, int len)
{
    ...
    case I2C_LTC_IOC_SET_CONFIG:
        ...
        if ((SOC_CONTROL(unit)->board_type == 0xCD) ||
            (SOC_CONTROL(unit)->board_type == 0xCE)) {
            sense_resistor_config = sense_resistor_config_56175;
            rv = ltc388x_setmin_max(unit, devno, len);
            break;
        }
#endif
   ...
   case I2C_LTC_IOC_NOMINAL:
           ...
           if ((SOC_CONTROL(unit)->board_type == 0xCD) ||
            ...
            for (iter = 0; iter < 100; iter++) {
                if (soc_i2c_addr(unit, devno) == I2C_LTC3884_SADDR42) {
                    ...
                    fval += fval1;
                } else if (soc_i2c_addr(unit, devno) == I2C_LTM4676A_SADDR40) {
                    if ((SOC_CONTROL(unit)->board_type == 0xCD) ||
                        (SOC_CONTROL(unit)->board_type == 0xCE)) {
                        /* For Trident3x1 phase support */
                        no_ch = I2C_BOTH_CH;
                        rv = ltc388x_read_current(unit, devno, &fval, no_ch);
                    } else {
            ...
            for (iter = 0; iter < 100; iter++) {
                if (soc_i2c_addr(unit, devno) == I2C_LTC3884_SADDR42) { 
                    ...
                    if ((SOC_CONTROL(unit)->board_type == 0xCD) ||
                        ...
                        /* For Trident3x1 phase support */
                        no_ch = I2C_BOTH_CH;
                        rv = ltc388x_read_power(unit, devno, &fval, no_ch);
                    } else {
    ...
    case I2C_LTC_IOC_SET_VOUT:
        ...
        if (DEV_DAC_PARAMS(dev)) {
             ...
             *
             * For Trident3x1 2 Phase support
             * LTC4676A at 0x40: Channel 0,1 = VDD_Core
             */
            if ((SOC_CONTROL(unit)->board_type == 0x94) ||
                (SOC_CONTROL(unit)->board_type == 0x96) ||
                (SOC_CONTROL(unit)->board_type == 0xB0) ||
                ...
                if (soc_i2c_addr(unit, devno) == I2C_LTC3884_SADDR42) {
                    ...
                    if ((SOC_CONTROL(unit)->board_type == 0xCD) ||
                        ...
                        no_ch = I2C_BOTH_CH;
                        SOC_IF_ERROR_RETURN
                            (ltc388x_dac_set_ch_voltage(unit, devno, no_ch, voltage));
                    } else {
                        SOC_IF_ERROR_RETURN
                            (ltc388x_dac_set_ch_voltage(unit, devno, ch, voltage));
                    }
...
ltc388x_init(int unit, int devno, void* data, int len)
{
    ...
    if (devtype == LTC3880_DEVICE_TYPE) {
        ...
        soc_i2c_devdesc_set(unit, devno, "Linear Tech LTM4676 Step_Down DC/DC "
            "Controller");
    } else if (devtype == LTM4676A_DEVICE_TYPE) {
        soc_i2c_devdesc_set(unit, devno, "Linear Tech LTM4676A Step_Down DC/DC "
                "Controller");
    } else {
...
STATIC int
ltm4676A_init(int unit, int devno, void* data, int len)
{
    SOC_IF_ERROR_RETURN(
            ltc388x_check_device_id(unit, devno, LTC_MFR_ID, LTM_4676A_MFR_MODEL));
    return ltc388x_init(unit, devno, data, len);
}
...
/* ltm 4676A voltage control Chip Driver callout */
i2c_driver_t _soc_i2c_ltm4676A_driver = {
    0x0, 0x0, /* System assigned bytes */
    LTM4676A_DEVICE_TYPE,
    ltc388x_read,
    ltc388x_write,
    ltc388x_ioctl,
    ltm4676A_init,
