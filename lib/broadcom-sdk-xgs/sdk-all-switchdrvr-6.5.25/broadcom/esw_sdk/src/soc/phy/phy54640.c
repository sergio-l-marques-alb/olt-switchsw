/*
 * $Id: phy54640.c,v 1.104 Broadcom SDK $
...
STATIC int
_phy_bcm54640_serdes_update(int unit, soc_port_t port)
{
    phy_ctrl_t  *pc;
    ...
    if ( (!(pc->copper.enable)) ||
        ...
        uint16 serdes_mode = (pc->fiber.force_speed == 100)
                           ? PHY_BCM54640_EXT_SERDES_FX : 0;
        /* External Serdes Control Reg., DIGX_SHD_1C_14, RDB_0x234 */
        SOC_IF_ERROR_RETURN
            (MODIFY_PHY54640_EXT_SERDES_CTRLr(unit, pc, serdes_mode,
                                        PHY_BCM54640_EXT_SERDES_FX_MASK));
    }
    return SOC_E_NONE;
}
                           ...
                           soc_phy_config_t  *cfg)
{
    ...
    SOC_IF_ERROR_RETURN
        (_phy_bcm54640_serdes_update(unit, port));
...
_phy_54640_reset_setup(int unit, soc_port_t port)
{
    ...
    if ( soc_phy_primary_and_offset_get(unit, port, &primary_port, &index)
        ...
        if ( index & 0x80 ) {
            PHY54640_FLAGS(pc) |= ( PHY54640_PHYA_REV);    /* reverse order */
        } else {
            PHY54640_FLAGS(pc) &= (~PHY54640_PHYA_REV);    /* normal  order */
        }
    } else {
        ...
        index        = pc->phy_id & ( PHY54640_DEV_PHY_SLICE_MASK);
    ...
    PHY54640_DEV_PHY_ID_ORIG(pc) = pc->phy_id;
    PHY54640_DEV_PHY_SLICE(pc)   = index;
    PHY54640_DEV_SET_BASE_ADDR(pc);   /* phy address of port 0 of this chip */

    SOC_IF_ERROR_RETURN
        (phy_54640_control_set( unit, port, SOC_PHY_CONTROL_PORT_PRIMARY,
                                            primary_port));
    SOC_IF_ERROR_RETURN
        (phy_54640_control_set( unit, port, SOC_PHY_CONTROL_PORT_OFFSET,
                                            index ));
    if ( index == 0 ) {
    ...
    if ( ECD_ENABLE && ((PHY_IS_BCM54640E(pc))
                    && ((pc->phy_rev & 0x7) > 0x2)) ) {
        SOC_IF_ERROR_RETURN(
...
_phy_54640_no_reset_setup(int unit, soc_port_t port)
{
    ...
    if (pc->fiber.enable && (pc->automedium || pc->fiber.preferred)) {
        ...
        /* SGMII AutoNeg Advertis Reg., Fiber Reg. 0x4 */
        SOC_IF_ERROR_RETURN
            (MODIFY_PHY54640_1000X_MII_ANAr(unit, pc, MII_ANA_C37_FD, 
                                    (MII_ANA_C37_HD | MII_ANA_C37_FD)));
        /* SGMII Slave Reg.,  Reg. 0x1C, shadow 0x15 */
        SOC_IF_ERROR_RETURN
            (MODIFY_PHY54640_SGMII_SLAVEr(unit, pc, 0, 0x1));
...

{
    ...
    rv = pc->write(unit, pc_port3->phy_id, 0x17, 0x0F0B);
    if (SOC_FAILURE(rv)) {
        INT_MCU_UNLOCK(unit);
        return rv;
    }
...

{
    ...
    rv = pc->write(unit, pc_port1->phy_id, 0x1c, 0xAC00);
    if (SOC_FAILURE(rv)) {
        INT_MCU_UNLOCK(unit);
        return rv;
    }
...
phy_54640_probe(int unit, phy_ctrl_t *pc)
{
    ...
    pc->size = sizeof(PHY54640_DEV_DESC_t);
    return rv;
}
...
phy_54640_link_up(int unit, soc_port_t port)
{
    ...
    if (speed == 100) {
        ...
        if(value & (1U << 13)) {
                SOC_IF_ERROR_RETURN
                    (PHY54640_REG_MODIFY(unit, pc, 0x00, 0x0FF5,0x15, 0x0<<10, 0x3<<10)); /* exp reg f5 */
        } else {
                SOC_IF_ERROR_RETURN
                    (PHY54640_REG_MODIFY(unit, pc, 0x00, 0x0FF5,0x15, 0x1<<10, 0x3<<10)); /* exp reg f5 */
        }
    }
...
phy_54640_timesync_config_set(int unit, soc_port_t port, soc_port_phy_timesync_config_t *conf)
{
    ...
    offset = PHY54640_DEV_PHY_SLICE(pc);
    ...
    do {
        if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_FLAGS) {
            ...
            SOC_IF_ERROR_BREAK
                (MODIFY_PHY54640_SGMII_SLAVEr(unit, pc, 0x02, 0x02), rv);

            SOC_IF_ERROR_BREAK
                (MODIFY_PHY54640_EXT_SERDES_CTRLr(unit, pc, 0x00, 0x01), rv);

            SOC_IF_ERROR_BREAK
                (PHY54640_REG_MODIFY(unit, pc, 0x00, 0x0FF5,0x15, 1U<<0, 1U<<0), rv); /* exp reg f5 */
            } else {

            SOC_IF_ERROR_BREAK
                (PHY54640_REG_MODIFY(unit, pc, 0x00, 0x0FF5,0x15, 0, 1U<<0), rv); /* exp reg f5 */

            }

            /* pc->phy_id =  PHY54640_SLICE_ADDR(pc, 0); */
            pc->phy_id =  PHY54640_DEV_PHY_ID_BASE(pc);

            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_modify(unit, pc, 0, 0x10, en_control_reg, 
                           ((1U << (offset + 8)) | (1U << offset))), rv);

            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x10, &en_control_reg), rv);
            ...
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_modify(unit, pc, 0, 0x12, tx_capture_en_reg, 
                           (1U << offset)), rv);

            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_modify(unit, pc, 0, 0x13, rx_capture_en_reg, 
                           (1U << (offset + 8))), rv);
            ...
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_modify(unit, pc, 0, 0x4f, nse_nco_6, 
                           (1U << 13)), rv);
            ...
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_write(unit, pc, 0, 0x34, rx_crc_control_reg), rv);
            ...
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_write(unit, pc, 0, 0x32, tx_control_reg), rv);
                              
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_write(unit, pc, 0, 0x33, rx_control_reg), rv);
            ...
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_modify(unit, pc, 0, 0x47, nse_nco_2c, 
                               (1U << 14)), rv);
        }
        ...
        if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_ITPID) {
            /* VLAN TAG register */
            SOC_IF_ERROR_BREAK
            (_phy_54640e_blk_top_lvl_reg_write(unit, pc, 0, 0x35, conf->itpid), rv);
        }

        if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_OTPID) {
            /* OUTER VLAN TAG register */
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_write(unit, pc, 0, 0x36, conf->otpid), rv);
        }

        if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_OTPID2) {
            /* INNER VLAN TAG register */
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_write(unit, pc, 0, 0x52, conf->otpid2), rv);
        }

        if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_TS_DIVIDER) {
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_write(unit, pc, 0, 0x4b, conf->ts_divider & 0xfff), rv);
        }


        if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_RX_LINK_DELAY) {
            pc->phy_id =  PHY54640_DEV_PHY_ID_ORIG(pc);

            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_write(unit, pc, 0, 0x6f, conf->rx_link_delay & 0xffff), rv);
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_write(unit, pc, 0, 0x70, (conf->rx_link_delay >> 16) & 0xffff), rv);
        ...
        if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_ORIGINAL_TIMECODE) {
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_write(unit, pc, 0, 0x25, (uint16)(conf->original_timecode.nanoseconds & 0xffff)), rv);

            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_write(unit, pc, 0, 0x26, (uint16)((conf->original_timecode.nanoseconds >> 16) & 0xffff)), rv);

            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_write(unit, pc, 0, 0x27, (uint16)(COMPILER_64_LO(conf->original_timecode.seconds) & 0xffff)), rv);

            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_write(unit, pc, 0, 0x28, (uint16)((COMPILER_64_LO(conf->original_timecode.seconds) >> 16) & 0xffff)), rv);

            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_write(unit, pc, 0, 0x29, (uint16)(COMPILER_64_HI(conf->original_timecode.seconds) & 0xffff)), rv);
        }
        mask = 0;
        ...
        if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_FRAMESYNC_MODE) {
            ...
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_write(unit, pc, 0, 0x51, length_threshold), rv);
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_write(unit, pc, 0, 0x50, event_offset), rv);
        }
        ...
        if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_SYNCOUT_MODE) {
            ...
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_write(unit, pc, 0, 0x48, interval & 0xffff), rv);
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_write(unit, pc, 0, 0x49, 
                    ((pulse_2_length & 0x3) << 14) | ((interval >> 16) & 0x3fff)), rv);
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_write(unit, pc, 0, 0x4a, 
                    ((pulse_1_length & 0x1ff) << 7) | ((pulse_2_length >> 2) & 0x7f)), rv);

            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_write(unit, pc, 0, 0x4e, 
                    (uint16)(COMPILER_64_LO(conf->syncout.syncout_ts) & 0xffff)), rv);
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_write(unit, pc, 0, 0x4d, 
                    (uint16)((COMPILER_64_LO(conf->syncout.syncout_ts) >> 16) & 0xffff)), rv);
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_write(unit, pc, 0, 0x4c, 
                    (uint16)(COMPILER_64_HI(conf->syncout.syncout_ts) & 0xffff)), rv);
        }

        SOC_IF_ERROR_BREAK
            (_phy_54640e_blk_top_lvl_reg_modify(unit, pc, 0, 0x4f, (gmode << 14) |
        ...
        if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_TX_TIMESTAMP_OFFSET) {
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_write(unit, pc, 0, 0x15 + offset, (uint16)(conf->tx_timestamp_offset & 0xffff)), rv);

            if (offset < 4 ) {
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_modify(unit, pc, 0, 0x63, 
                    ((conf->tx_timestamp_offset & 0xf0000) >> ((4 - offset) << 2)), 0xf << (offset << 2)), rv);
            } else {
            uint16 offset1 = offset - 4;
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_modify(unit, pc, 0, 0x64, 
        ...
        if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_RX_TIMESTAMP_OFFSET) {
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_write(unit, pc, 0, 0x1d + offset, (uint16)(conf->rx_timestamp_offset & 0xffff)), rv);

            if (offset < 4 ) {
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_modify(unit, pc, 0, 0x65, 
                    ((conf->rx_timestamp_offset & 0xf0000) >> ((4 - offset) << 2)), 0xf << (offset << 2)), rv);
            } else {
            uint16 offset1 = offset - 4;
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_modify(unit, pc, 0, 0x66, 
        ...
        SOC_IF_ERROR_BREAK
            (_phy_54640e_blk_top_lvl_reg_modify(unit, pc, 0, 0x11, value, mask), rv); 
        ...
        SOC_IF_ERROR_BREAK
            (_phy_54640e_blk_top_lvl_reg_modify(unit, pc, 0, 0x12, value << 8, mask << 8), rv); 
        ...
        if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_PHY_1588_DPLL_REF_PHASE) {
            ...
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_write(unit, pc, 0, 0x38, (uint16)(COMPILER_64_LO(conf->phy_1588_dpll_ref_phase) & 0xffff)), rv);

            /* Initial ref phase [31:16]  */
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_write(unit, pc, 0, 0x39, (uint16)((COMPILER_64_LO(conf->phy_1588_dpll_ref_phase) >> 16) & 0xffff)), rv);

            /*  Initial ref phase [47:32] */
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_write(unit, pc, 0, 0x3a, (uint16)(COMPILER_64_HI(conf->phy_1588_dpll_ref_phase) & 0xffff)), rv);
        }

        if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_PHY_1588_DPLL_REF_PHASE_DELTA) {
            /* Ref phase delta [15:0] */
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_write(unit, pc, 0, 0x3b, (uint16)(conf->phy_1588_dpll_ref_phase_delta & 0xffff)), rv);

            /* Ref phase delta [31:16]  */
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_write(unit, pc, 0, 0x3c, (uint16)((conf->phy_1588_dpll_ref_phase_delta >> 16) & 0xffff)), rv);
        }
        ...
        SOC_IF_ERROR_BREAK
            (_phy_54640e_blk_top_lvl_reg_modify(unit, pc, 0, 0x3d, (conf->phy_1588_dpll_k1 & 0xff)<<8 | (conf->phy_1588_dpll_k2 & 0xff), mask ), rv);

        if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_PHY_1588_DPLL_K3) {
            /* DPLL K3 */
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_write(unit, pc, 0, 0x3e, conf->phy_1588_dpll_k3 & 0xff), rv);
        }
        ...
        if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_PHY_1588_DPLL_LOOP_FILTER) {
            ...
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_write(unit, pc, 0, 0x3f, (uint16)(COMPILER_64_LO(conf->phy_1588_dpll_loop_filter) & 0xffff)), rv);

            /* Initial loop filter[31:16]  */
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_write(unit, pc, 0, 0x40, (uint16)((COMPILER_64_LO(conf->phy_1588_dpll_loop_filter) >> 16) & 0xffff)), rv);

            /*  Initial loop filter[47:32] */
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_write(unit, pc, 0, 0x41, (uint16)(COMPILER_64_HI(conf->phy_1588_dpll_loop_filter) & 0xffff)), rv);

            /* Initial loop filter[63:48]  */
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_write(unit, pc, 0, 0x42, (uint16)((COMPILER_64_HI(conf->phy_1588_dpll_loop_filter) >> 16) & 0xffff)), rv);
        }
...
phy_54640_timesync_config_get(int unit, soc_port_t port, soc_port_phy_timesync_config_t *conf)
{
    ...
    do {
        ...
        SOC_IF_ERROR_BREAK
            (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x4f, &nse_nco_6), rv);
        ...
        if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_FLAGS) {
            ...
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x12, &tx_capture_en_reg), rv); 
            ...
            SOC_IF_ERROR_BREAK
            (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x34, &rx_crc_control_reg), rv);
            ...
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x32, &tx_control_reg), rv);
            ...
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x47, &nse_nco_2c), rv);
        ...
        if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_FRAMESYNC_MODE) {
            ...
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x51, &temp1), rv);
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x50, &temp2), rv);
        ...
        if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_SYNCOUT_MODE) {
            ...
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x48, &temp1), rv);
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x49, &temp2), rv);
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x4a, &temp3), rv);
            ...
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x4e, &temp1), rv);
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x4d, &temp2), rv);
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x4c, &temp3), rv);
        ...
        if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_ITPID) {
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x35, &conf->itpid), rv);
        }

        if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_OTPID) {
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x36, &conf->otpid), rv);
        }

        if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_OTPID2) {
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x52, &conf->otpid2), rv);
        }

        if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_TS_DIVIDER) {
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x4b, &conf->ts_divider), rv);
        }
        ...
        if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_RX_LINK_DELAY) {
            ...
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x6f, &temp1), rv);
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x70, &temp2), rv);
        ...
        if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_ORIGINAL_TIMECODE) {
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x25, &temp1), rv);

            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x26, &temp2), rv);

            conf->original_timecode.nanoseconds = ((uint32)temp2 << 16) | temp1; 

            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x27, &temp1), rv);

            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x28, &temp2), rv);

            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x29, &temp3), rv);
        ...
        if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_TX_TIMESTAMP_OFFSET) {
            ...
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x15 + offset, &temp1), rv);
            if (offset < 4 ) {
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x63, &temp2), rv);
            temp32 = (temp2 & (0xf << (offset << 2))) << ((4 - offset) << 2);
            } else {
            uint16 offset1 = offset - 4;
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x64, &temp2), rv);
        ...
        if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_RX_TIMESTAMP_OFFSET) {
            ...
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x1d + offset, &temp1), rv);
            if (offset < 4 ) {
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x65, &temp2), rv);
            temp32 = (temp2 & (0xf << (offset << 2))) << ((4 - offset) << 2);
            } else {
            uint16 offset1 = offset - 4;
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x66, &temp2), rv);
            temp32 = (temp2 & (0xf << (offset1 << 2))) << ((4 - offset1) << 2);
            }
        ...
        SOC_IF_ERROR_BREAK
            (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x11, &value), rv); 
        ...
        SOC_IF_ERROR_BREAK
            (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x12, &value), rv); 
        ...
        if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_PHY_1588_DPLL_REF_PHASE) {
            /* Initial ref phase [15:0] */
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x38, &temp1), rv);

            /* Initial ref phase [31:16]  */
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x39, &temp2), rv);

            /*  Initial ref phase [47:32] */
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x3a, &temp3), rv);
        ...
        if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_PHY_1588_DPLL_REF_PHASE_DELTA) {
            /* Ref phase delta [15:0] */
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x3b, &temp1), rv);

            /* Ref phase delta [31:16]  */
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x3c, &temp2), rv);
        ...
        /* DPLL K1 & K2 */
        SOC_IF_ERROR_BREAK
            (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x3d, &temp1 ), rv);
        ...
        if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_PHY_1588_DPLL_K3) {
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x3e, &temp1), rv);
        ...
        if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_PHY_1588_DPLL_LOOP_FILTER) {
            /* Initial loop filter[15:0] */
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x3f, &temp1), rv);

            /* Initial loop filter[31:16]  */
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x40, &temp2), rv);

            /*  Initial loop filter[47:32] */
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x41, &temp3), rv);

            /* Initial loop filter[63:48]  */
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x42, &temp4), rv);
...
phy_54640_timesync_control_set(int unit, soc_port_t port, soc_port_control_phy_timesync_t type, uint64 value)
{
    ...
    do {
        ...
        case SOC_PORT_CONTROL_PHY_TIMESYNC_NCOADDEND:
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_write(unit, pc, 0, 0x44, (uint16)COMPILER_64_LO(value)), rv);
            break;

        case SOC_PORT_CONTROL_PHY_TIMESYNC_FRAMESYNC:
           SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x4f, &temp1), rv);
            temp2 = ((temp1 | (0x3 << 14) | (0x1 << 12)) & ~(0xf << 2)) | (0x1 << 5);
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_write(unit, pc, 0, 0x4f, temp2), rv);
            sal_udelay(1);
            temp2 &= ~((0x1 << 5) | (0x1 << 12));
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_write(unit, pc, 0, 0x4f, temp2), rv);
            sal_udelay(1);
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_write(unit, pc, 0, 0x4f, temp1), rv);
            break;

        case SOC_PORT_CONTROL_PHY_TIMESYNC_LOCAL_TIME:
            COMPILER_64_SHR(value, 4);
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_write(unit, pc, 0, 0x45, (uint16)(COMPILER_64_LO(value) & 0xffff)), rv);
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_write(unit, pc, 0, 0x46, (uint16)((COMPILER_64_LO(value) >> 16) & 0xffff)), rv);
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_modify(unit, pc, 0, 0x47, (uint16)(COMPILER_64_HI(value) & 0xfff), 0xfff), rv);
            break;
        ...
        case SOC_PORT_CONTROL_PHY_TIMESYNC_LOAD_CONTROL:
            ...
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_write(unit, pc, 0, 0x2e, temp1), rv);
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_write(unit, pc, 0, 0x2f, temp2), rv);
            break;
        ...
        case SOC_PORT_CONTROL_PHY_TIMESYNC_INTERRUPT_MASK:
            ...
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_write(unit, pc, 0, 0x30, temp1), rv);
            break;
        ...
        case SOC_PORT_CONTROL_PHY_TIMESYNC_TX_TIMESTAMP_OFFSET:
            ...
            /* tx_timestamp_offset */
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_write(unit, pc, 0, 0x15 + offset, (uint16)(value0 & 0xffff)), rv);
        ...
        case SOC_PORT_CONTROL_PHY_TIMESYNC_RX_TIMESTAMP_OFFSET:
            ...
            /* rx_timestamp_offset */
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_write(unit, pc, 0, 0x1d + offset, (uint16)(value0 & 0xffff)), rv);
...
phy_54640_timesync_control_get(int unit, soc_port_t port, soc_port_control_phy_timesync_t type, uint64 *value)
{
    ...
    do {
        ...
        case SOC_PORT_CONTROL_PHY_TIMESYNC_HEARTBEAT_TIMESTAMP:
            SOC_IF_ERROR_BREAK
                (WRITE_PHY54640_EXP_READ_START_END_CTRLr(unit, pc, 0x1), rv);

            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x56, &value0), rv);
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x57, &value1), rv);
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x58, &value2), rv);

            SOC_IF_ERROR_BREAK
                (WRITE_PHY54640_EXP_READ_START_END_CTRLr(unit, pc, 0x2), rv);
            SOC_IF_ERROR_BREAK
                (WRITE_PHY54640_EXP_READ_START_END_CTRLr(unit, pc, 0x0), rv);
        ...
        case SOC_PORT_CONTROL_PHY_TIMESYNC_CAPTURE_TIMESTAMP:
            SOC_IF_ERROR_BREAK
                (WRITE_PHY54640_EXP_READ_START_END_CTRLr(unit, pc, 0x4), rv);

            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x53, &value0), rv);
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x54, &value1), rv);
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x55, &value2), rv);

            SOC_IF_ERROR_BREAK
                (WRITE_PHY54640_EXP_READ_START_END_CTRLr(unit, pc, 0x8), rv);
            SOC_IF_ERROR_BREAK
                (WRITE_PHY54640_EXP_READ_START_END_CTRLr(unit, pc, 0x0), rv);
        ...
        case SOC_PORT_CONTROL_PHY_TIMESYNC_NCOADDEND:
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x44, &value0), rv);
            /* *value = value0; */
        ...
        case SOC_PORT_CONTROL_PHY_TIMESYNC_LOCAL_TIME:
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x45, &value0), rv);
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x46, &value1), rv);
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x47, &value2), rv);
        ...
        case SOC_PORT_CONTROL_PHY_TIMESYNC_LOAD_CONTROL:
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x2e, &value1), rv);
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x2f, &value2), rv);
            value0 = 0;
            if (value1 & (1U << 11)) {
        ...
        case SOC_PORT_CONTROL_PHY_TIMESYNC_INTERRUPT:
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x31, &value1), rv);
            value0 = 0;
            if (value1 & (1U << 1)) {
        ...
        case SOC_PORT_CONTROL_PHY_TIMESYNC_INTERRUPT_MASK:
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x30, &value1), rv);
            value0 = 0;
            if (value1 & (1U << 1)) {
        ...
        /* tx_timestamp_offset */
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x15 + offset, &value1), rv);
            if (offset < 4 ) {
                SOC_IF_ERROR_BREAK
                    (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x63, &value2), rv);
                temp32 = (value2 & (0xf << (offset << 2))) << ((4 - offset) << 2);
            } else {
                uint16 offset1 = offset - 4;
                SOC_IF_ERROR_BREAK
                    (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x64, &value2), rv);
        ...
        /* rx_timestamp_offset */
            SOC_IF_ERROR_BREAK
                (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x1d + offset, &value1), rv);
            if (offset < 4 ) {
                SOC_IF_ERROR_BREAK
                    (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x65, &value2), rv);
                temp32 = (value2 & (0xf << (offset << 2))) << ((4 - offset) << 2);
            } else {
                uint16 offset1 = offset - 4;
                SOC_IF_ERROR_BREAK
                    (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 0x66, &value2), rv);
                temp32 = (value2 & (0xf << (offset1 << 2))) << ((4 - offset1) << 2);
            }
            /* *value = temp32 | value1; */
            COMPILER_64_SET((*value), 0, (uint32)(temp32 | value1));
...
typedef int _soc_phy_54640_not_empty; /* Make ISO compilers happy. */
