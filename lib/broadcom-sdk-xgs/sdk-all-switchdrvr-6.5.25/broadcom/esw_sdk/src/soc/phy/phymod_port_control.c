...
control_handler_medium_type_get(int unit, phymod_phy_access_t *phy, uint32 ref_clk, uint32 *value)
{
    ...
    if (PHYMOD_INTF_MODES_FIBER_GET(&config)) {
        ...
        *value = SOC_PORT_MEDIUM_COPPER; 
    } else if (PHYMOD_INTF_MODES_BACKPLANE_GET(&config)) {
        *value = SOC_PORT_MEDIUM_BACKPLANE;
    } else {
...
control_handler_linkdown_transmit_set(int unit, phymod_phy_access_t *phy,  uint32 value, uint32 param2) {
    ...
    if (phy->type == phymodDispatchTypeViper) {
        return(phymod_phy_linkdown_transmit_set(phy, value));
    }
#endif
...
#ifdef PHYMOD_VIPER_SUPPORT
    if (phy->type == phymodDispatchTypeViper) {
        return(phymod_phy_linkdown_transmit_get(phy, value));
    }
#endif
...
STATIC int
control_handler_codec_mode_set(int unit,
                                   phymod_phy_access_t *phy,
                                   uint32_t value1,
                                   uint32_t value2)
{
    phymod_phy_codec_mode_t codec_mode;
    int mode = value2;

    if (phy == NULL) return SOC_E_INTERNAL;

    switch (mode) {
        case SOC_PHY_CONTROL_CODEC_MODE_CL49:
            codec_mode = phymodCodecCL49;
            break;
        case SOC_PHY_CONTROL_CODEC_MODE_CL82:
            codec_mode = phymodCodecCL82;
            break;
        case SOC_PHY_CONTROL_CODEC_MODE_REDUCED_IPG:
            codec_mode = phymodCodecReducedIPG;
            break;
        default:
            codec_mode = phymodCodecCL49;
            break;
    ...
    return (phymod_phy_codec_mode_set(phy, codec_mode));
}

STATIC int
control_handler_codec_mode_get(int unit,
                                   phymod_phy_access_t *phy,
                                   uint32_t param,
                                   uint32_t *value)
{
    ...
    SOC_IF_ERROR_RETURN(phymod_phy_codec_mode_get(phy, &codec_mode));

    switch (codec_mode) {
        case phymodCodecCL49:
            *value = SOC_PHY_CONTROL_CODEC_MODE_CL49;
            break;
        case phymodCodecCL82:
            *value = SOC_PHY_CONTROL_CODEC_MODE_CL82;
            break;
        case phymodCodecReducedIPG:
            *value = SOC_PHY_CONTROL_CODEC_MODE_REDUCED_IPG;
            break;
        default:
            *value = SOC_PHY_CONTROL_CODEC_MODE_CL49;
            break;
...
STATIC int
control_handler_an_fail_inhibit_timer_set(int unit, phymod_phy_access_t *phy,
                                          uint32 value1, uint32 value2)
{
    SOC_IF_ERROR_RETURN(phymod_phy_an_timer_set(phy, phymodAnTimerLinkFailInhibitLtPam4, value1));
    return SOC_E_NONE;
}

STATIC int
control_handler_an_fail_inhibit_timer_get(int unit,
                                          phymod_phy_access_t *phy,
                                          uint32_t param,
                                          uint32_t *value)
{
    SOC_IF_ERROR_RETURN(phymod_phy_an_timer_get(phy, phymodAnTimerLinkFailInhibitLtPam4, value));
    return SOC_E_NONE;
}
...
int soc_port_control_get_wrapper(int unit, phymod_ref_clk_t ref_clock, int is_lane_control, phymod_phy_access_t *phys, int nof_phys, soc_phy_control_t control, uint32 *value)
{
    ...
    if (((int)control < 0) || (control >= SOC_PHY_CONTROL_COUNT)) {
        return (SOC_E_PARAM);
    }
    ...
    *value = 0;
    ...
    case SOC_PHY_CONTROL_CODEC_MODE_CL82:
    case SOC_PHY_CONTROL_CODEC_MODE_REDUCED_IPG:
        handler = control_handler_codec_mode_get;
        lane_control_support = TRUE;
        param = control;
        break;
    case SOC_PHY_CONTROL_AN_LINK_FAIL_INHIBIT_TIMER_LT_PAM4:
        handler = control_handler_an_fail_inhibit_timer_get;
        lane_control_support = TRUE;
        break;
    default:
        return SOC_E_UNAVAIL;
...
int soc_port_control_set_wrapper(int unit, phymod_ref_clk_t ref_clock, int is_lane_control, phymod_phy_access_t *phys, int nof_phys, soc_phy_control_t control, uint32 value)
{
    ...
    case SOC_PHY_CONTROL_CODEC_MODE_CL82:
    case SOC_PHY_CONTROL_CODEC_MODE_REDUCED_IPG:
        handler = control_handler_codec_mode_set;
        lane_control_support = TRUE;
        param2 = control;
        break;
    case SOC_PHY_CONTROL_AN_LINK_FAIL_INHIBIT_TIMER_LT_PAM4:
        handler = control_handler_an_fail_inhibit_timer_set;
        lane_control_support = TRUE;
        break;
    default:
        return SOC_E_UNAVAIL;
