...
_phy_wc40_firmware_force_cl72_config(int unit, soc_port_t port, int enable)
{
    ...
    for (i = lane_start; i <= lane_end; i++) {
        ...
        PMD_IEEE9BLK_TENGBASE_KR_PMD_CONTROL_REGISTER_150_TRAINING_ENABLE_MASK;
        data16 = enable? mask16 : 0;
        if (!(SOC_IS_KATANA2(unit)) && (!(lane_start == lane_end) ||
            ((lane_start == lane_end) && (DEV_CFG_PTR(pc)->line_intf & WC40_IF_KR)))) {/* Skiped for HG[11] */
            SOC_IF_ERROR_RETURN
...
_phy_wc40_force_cl72_config(int unit, soc_port_t port, int enable)
{
    ...
    for (i = lane_start; i <= lane_end; i++) {
        ...
        SOC_IF_ERROR_RETURN
         ...
         /* Enable IEEE reg for cl72 training */
    ...
    *speed_val = 0;
    ...
    case 10000:  /* 10G_XFI, 10G_SFI, 10G dxgxs, 10G dxgxs hig */
        ...
        if (IS_DUAL_LANE_PORT(pc)) {
           ...
           } else if ((DEV_CFG_PTR(pc)->line_intf & WC40_IF_KR)) {
               *speed_val = SERDESDIGITAL_FORCE_SPEED_dr_10G_KR;
...
_phy_wc40_ind_speed_set(int unit, soc_port_t port, int speed)
{
    ...
    */
    ...
    /* Configure 10G via IEEE PMA/PMD registers.
     * Set pma_pmd_forced_speed_enc_en(0x8039.5) bit to 1.
     * SpeedSelection0 = 1 (PMD_ieee0Blk_PMD_ieeeControl1.6)
     * SpeedSelection1 = 1 (PMD_ieee0Blk_PMD_ieeeControl1.13)
     * SpeedSelection3 = 0 (PMD_ieee0Blk_PMD_ieeeControl1[5:2])
     * Set PMA_PMD_typeSel = 0xB(PMD_ieee0Blk_PMD_ieeeControl2 [3:0])
     */
    if (speed_val == SERDESDIGITAL_FORCE_SPEED_dr_10G_KR) {
        SOC_IF_ERROR_RETURN
            (MODIFY_WC40_SERDESDIGITAL_MISC2r(unit, pc, 0x00,
                           SERDESDIGITAL_MISC2_PMA_PMD_FORCED_SPEED_ENC_EN_MASK,
                           SERDESDIGITAL_MISC2_PMA_PMD_FORCED_SPEED_ENC_EN_MASK));
