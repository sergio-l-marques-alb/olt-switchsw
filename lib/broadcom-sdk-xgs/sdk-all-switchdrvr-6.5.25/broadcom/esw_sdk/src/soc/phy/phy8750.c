...
/*
 * $Id: phy8750.c,v 1.17 Broadcom SDK $
...
phy_8750_init(int unit, soc_port_t port)
{
    ...
    phy_ctrl_t     *pc;
    uint32         phy_ext_rom_boot, phy_long_xfi, force_fw_load = 0; 
    ...
    if (chip_id == 0x8754) {
        force_fw_load = soc_property_port_get(unit, port, spn_PHY_FORCE_FIRMWARE_LOAD, 0);
        if (!force_fw_load) {
            ...
            if (ucode_ver >= 0x0301) {
                ...
                if (data16 != 0x600D || SOC_FAILURE(rv)) {
                    LOG_INFO(BSL_LS_SOC_PHY,
                     (BSL_META_U(unit, "8750: Invalid Checksum:0x%x\n"),data16));
                } else {
                    return SOC_E_NONE;
                }
            } else {
                LOG_INFO(BSL_LS_SOC_PHY,
                     (BSL_META_U(unit, "8750: Invalid Ucode:0x%x\n"),ucode_ver));
            }
            LOG_INFO(BSL_LS_SOC_PHY,
                     (BSL_META_U(unit,
                                 "8750: u=%d port%d Ucode ver: 0x%x: init.\n"),
                                 unit, port, ucode_ver));
        }
