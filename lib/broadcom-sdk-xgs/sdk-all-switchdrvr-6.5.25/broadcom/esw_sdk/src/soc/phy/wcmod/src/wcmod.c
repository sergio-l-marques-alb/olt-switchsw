...
_init_wcmod_st(wcmod_st *pc)
{
      ...
      pc->aer_bcst_ofs_strap = 0x1ff; /* good for TR3  */
...

    {
        ...
        if (!_wcmod_chip_init_done(unit,pc->chip_num,pc->phy_mode)) {
            ...
            if (SOC_INFO(unit).port_num_lanes[pc->port] == 1) { 
                ...
                /* Reset value for Auto_det is 1, need to disable 1000X / SGMII Autodetect for QSGMII */
                /* coverity[value_overwrite] */
                qsgmii_ws->lane_select = WCMOD_LANE_BCST;
                SOC_IF_ERROR_RETURN
                   (MODIFY_WC40_SERDESDIGITAL_CONTROL1000X1r(qsgmii_ws->unit, qsgmii_ws, 0,
...
_phy_wcmod_ind_init_common(wcmod_st *ws, int fw_load_method) {
    ...
    /* configure and initialize the resource shared by all 4 lanes*/
    if ((SOC_IS_ARAD(ws->unit) && IS_SFI_PORT(ws->unit, ws->port)))
    {
        SOC_IF_ERROR_RETURN
