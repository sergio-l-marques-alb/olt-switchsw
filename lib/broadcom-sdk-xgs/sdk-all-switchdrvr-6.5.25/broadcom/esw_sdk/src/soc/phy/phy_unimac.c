/*
 * $Id: phy_unimac.c,v 1.3 Broadcom SDK $
...
phy_unimac_mac_init(phy_mac_ctrl_t *mmc, int dev_port)
{
    ...
    BLMI_E_IF_ERROR_RETURN(
        UNIMAC_WRITE_CMD_CFG_REG(mmc, dev_port, cmd_cfg));
    cmd_cfg = UNIMAC_REG_GPORT_RSV_ERROR_MASK;
    BLMI_E_IF_ERROR_RETURN(
        UNIMAC_WRITE_GPORT_RSV_MASK_REG(mmc, dev_port, cmd_cfg));
#endif
...
_mac_duplex_gateway_set(phy_mac_ctrl_t *mmc, int port,
                        ...
                        int ipg, int max_frame, int pause, int line_side)
{
    uint32  cmd_reg;
    ...
    BMACSEC_MAC_REG_FIELD_SET(cmd_reg, 0, UNIMAC_REG_CMD_CFG_PAUSE_FWD_MASK,
                                          UNIMAC_REG_CMD_CFG_PAUSE_FWD_SHIFT);
    /* Line and System side TX pause, RX pause are enabled in full duplex mode
     * to terminate and generate pause control frame in split pause domain <PHY-4048> */
    BMACSEC_MAC_REG_FIELD_SET(cmd_reg, ((line_side && (!pause || !fullduplex)) ? 0 : 1),
                              ...
                              phy_mac_duplex_gateway_t *dgw)
{
    ...
    BLMI_E_IF_ERROR_RETURN(
        _mac_duplex_gateway_set(mmc, dgw->lport,
                        speed, dgw->duplex, dgw->ipg, dgw->max_frame,dgw->pause, 1) );
    /* config switch-side MAC, always full-duplex */
    BLMI_E_IF_ERROR_RETURN(
        _mac_duplex_gateway_set(mmc, dgw->sport,
                        speed, 1, dgw->ipg, dgw->max_frame,dgw->pause, 0) );
