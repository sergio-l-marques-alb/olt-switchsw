...
*/
STATIC
int _pm8x50_flexe_gen2_vco_to_pll_get(phymod_ref_clk_t ref_clock, portmod_vco_type_t vco, uint32_t* pll)
{
    int rv = SOC_E_NONE;
    SOC_INIT_FUNC_DEFS;
                                         ...
                                         phymod_phy_pll_state_t *new_pll_state)
{
    ...
    _SOC_IF_ERR_EXIT(_pm8x50_flexe_gen2_vco_to_pll_get(PM_8x50_FLEXE_GEN2_INFO(pm_info)->ref_clk, required_vco, &required_pll_div));
                                          ...
                                          uint32 value)
{
    ...
    switch(type) {
        ...
        case  portmodPreemptionControlFinalFragSizeRx:
            _SOC_EXIT_WITH_ERR(SOC_E_UNAVAIL,
              (_SOC_MSG(" preemption control type(%d) is not supported"), type));
            break;
        default:
            _SOC_EXIT_WITH_ERR(SOC_E_PARAM,
              (_SOC_MSG("Invalid preemption control type(%d)"), type));
