...
#ifdef BCM_CMICX_SUPPORT
uint32 sw_prog_intr = SW_PROG_INTR;
#endif
...
static uint32 _soc_cmic_uc_msg_base(int unit)
{
    ...
    if (SOC_IS_DNX(unit)) {
        if (soc_feature(unit, soc_feature_iproc_17)) {
            return 0x51afe000;
        } else {
            return 0x012ff000;
        }
...
int _soc_cmic_uc_msg_process_status(int unit, int uC) {
    ...
    cur_status_in = soc_uc_mem_read(unit, CMICM_ADDR(area_in, status));
    ...
    for (i = MOS_MSG_STATUS_SENT_INDEX(soc->uc_msg_prev_status_in[uC]);
        ...
        mos_msg_data_t msg;
        msg.words[0] = soc_uc_mem_read(unit,CMICM_DATA_WORD0_BASE(area_in) +
                                         MOS_MSG_DATA_SIZE * i);
        msg.words[1] = soc_uc_mem_read(unit,CMICM_DATA_WORD1_BASE(area_in) +
                                         MOS_MSG_DATA_SIZE * i);
    ...
    if (out_status) {
        /*  We have determined the new status out - write it and notify
            ...
            interrupting the uC*/
        soc_uc_mem_write(unit, CMICM_ADDR(area_out, status),
                           soc->uc_msg_prev_status_out[uC]);
        /* coverity[result_independent_of_operands] */
#ifdef BCM_CMICX_SUPPORT
        if (soc_feature(unit, soc_feature_cmicx_v4)) {
            CMICX2_SW_INTR_SET(unit, CMICM_SW_INTR_UC(uC));
...
soc_cmic_uc_msg_active_wait(int unit, uint32 uC) {
    ...
    if (!sal_sem_take(soc->uc_msg_active[uC], 10000000)) {
        ...
        LOG_VERBOSE(BSL_LS_SOC_COMMON,
                    (BSL_META_U(unit,
                                "UC%d msg active wait timed out\n"), uC));
    }
    ...
    while (1) {
        ...
        if (rsubclass == MOS_MSG_SUBCLASS_SYSTEM_LOG) {
            ...
            if (rlen == 0) {
                ...
                send.s.len = ((*(uint32 *)endian_buf) == 0x01020304) ? 1 : 0;
                send.s.mclass = MOS_MSG_CLASS_SYSTEM;
...
#ifdef BCM_CMICX_SUPPORT
        if (soc_feature(unit, soc_feature_cmicx)) {
            soc_uc_mem_write(unit, CMICX_ADDR(area_in, status), 0);
        } else
#endif
        {
...
#ifdef BCM_CMICX_SUPPORT
            if (soc_feature(unit, soc_feature_cmicx)) {
                cur_status_in = soc_uc_mem_read(unit, CMICX_ADDR(area_in, status));
            } else
#endif
            {
...
#ifdef BCM_CMICX_SUPPORT
            if (soc_feature(unit, soc_feature_cmicx)) {
                soc_uc_mem_write(unit, CMICX_ADDR(area_out, status),
                                   soc->uc_msg_prev_status_out[uC]);
                if (soc_feature(unit, soc_feature_cmicx_v4)) {
                    CMICX2_SW_INTR_SET(unit, uC_intr);
                } else {
...
soc_cmicx_sw_intr(int unit, void *data)
{
    ...
    if (soc_feature(unit, soc_feature_cmicx_v4)) {
#ifndef BCM_EDK_SUPPORT
        /* SW interrupt bits reserved as - 0:PCI, 1-4:M0, 5-8:mHost */
        READ_PAXB_0_SW_PROG_INTR_ENABLEr(unit, &intr_enable);
        READ_PAXB_0_SW_PROG_INTR_STATUSr(unit, &intr_status);

        /* Process only enabled interrupts */
        intr_status = intr_status & intr_enable;

        if (intr_status & (0x1 << 5)) {
            mHost0_val = 1;
        }
        if (intr_status & (0x1 << 6)) {
            mHost1_val = 1;
        }
        if (intr_status & (0x1 << 7)) {
            mHost2_val = 1;
        }
        if (intr_status & (0x1 << 8)) {
            mHost3_val = 1;
        }
#endif
    } else {
        
        /* Disabling mHost interrupt handling if EDK is enabled */
#ifndef BCM_EDK_SUPPORT
    ...
    if (soc_feature(unit, soc_feature_cmicx_v4)) {
        ...
        WRITE_PAXB_0_SW_PROG_INTR_CLRr(unit, intr_status);
    } else {
        if (mHost0_val)
        {
    ...
    if (mHost2_val)
    {
        if (soc->swIntr[CMICM_SW_INTR_UC(2)]) {
            sal_sem_give(soc->swIntr[CMICM_SW_INTR_UC(2)]);
        }
    }
    if (mHost3_val)
    {
        if (soc->swIntr[CMICM_SW_INTR_UC(3)]) {
            sal_sem_give(soc->swIntr[CMICM_SW_INTR_UC(3)]);
        }
    }
#endif
    if (m0_u0_sw_prg_intr) {
...
#if defined(BCM_DNX_SUPPORT)
    if (soc_feature(unit, soc_feature_cmicx_v4)) {
        sw_prog_intr = IPROC17_SW_PROG_INTR;

        /* Enable the SW interrupts from all 4 cores */
        WRITE_PAXB_0_SW_PROG_INTR_ENABLEr(unit, 0xF<<5);
    }
#endif
...
soc_cmic_uc_msg_uc_start(int unit, int uC) {
    ...
    sal_thread_t thread_id_uc_system_msg;
     ...
     defined(BCM_TRIDENT2_SUPPORT) || defined(BCM_TIME_LOCAL_TOD_SUPPORT))
    uint32 stages = 0;
    bcm_time_tod_t tod;
#endif
    ...
    if (SOC_IS_JERICHO2(unit)) {
        /* In case of JR2, hold the lock a little longer,
         * until after handshaking is complete */
        if (soc_cmic_uc_msg_active_wait(unit, uC) != SOC_E_NONE) {
            LOG_ERROR(BSL_LS_SOC_COMMON,
                    (BSL_META_U(unit,
                                "uC-%d Msg handshaking not completed\n"), uC));
...
#endif
    {
     ...
     defined(BCM_TOMAHAWK2_SUPPORT) || defined(BCM_MAVERICK2_SUPPORT) || \
     defined(BCM_TRIDENT2_SUPPORT) || defined(BCM_TIME_LOCAL_TOD_SUPPORT))
    /* Set initial ref time in FW */
#ifdef COMPILER_HAS_LONGLONG
    tod.ts.seconds = sal_time();
#else
    COMPILER_64_SET(tod.ts.seconds, 0, sal_time());
#endif
    stages = BCM_TIME_STAGE_BTE;

    if (SOC_E_NONE != bcm_esw_time_tod_set(unit, stages, &tod)) {
        LOG_VERBOSE(BSL_LS_SOC_COMMON,
                  (BSL_META_U(unit,
                              "failed to set initial ref time (uC - %d)\n"), uC));
    }
#endif
    return (SOC_E_NONE);
}
...
int soc_cmic_uc_msg_send(int unit, int uC, mos_msg_data_t *msg,
    ...
    while (1) {                 /* Retry loop */
        ...
        if (sal_mutex_take(soc->uc_msg_control, soc->uc_msg_ctl_timeout) == 0) {
            ...
            area_out = MSG_AREA_HOST_TO_ARM(msg_base, uC);
            soc_uc_mem_write(unit, CMICM_DATA_WORD0_BASE(area_out) +
                               MOS_MSG_DATA_SIZE * index, 
                               omsg.words[0]);
            soc_uc_mem_write(unit, CMICM_DATA_WORD1_BASE(area_out) +
                               MOS_MSG_DATA_SIZE *index,
                               omsg.words[1]);
            soc_uc_mem_write(unit, CMICM_ADDR(area_out,status),
                               soc->uc_msg_prev_status_out[uC]);
                          ...
                          soc_cmic_uc_appl_cb_t *shutdown_cb, void *cb_data)
{
    ...
    if(msg_class == MOS_MSG_CLASS_IFA2) {
        ifa2_firmware_version = soc_ntohl(rdata);
    }
#endif /* INCLUDE_IFA */
#if defined(INCLUDE_TELEMETRY)
