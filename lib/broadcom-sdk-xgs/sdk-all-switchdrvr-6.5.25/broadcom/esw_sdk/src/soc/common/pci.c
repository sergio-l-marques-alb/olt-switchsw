...
/* If SOC_PCI_DEBUG or INCLUDE_CPU_I2C not defined, then these functions are inlined in cmic.h */
...
{
	uint32 addr32 = addr;
#if defined(SOC_PCI_DEBUG) && defined(IPROC_ACCESS_DEBUG) && (defined(BCM_DNX_SUPPORT) || defined(BCM_DNXF_SUPPORT))
...
#endif /* defined(SOC_PCI_DEBUG) && defined(IPROC_ACCESS_DEBUG) && (defined(BCM_DNX_SUPPORT) || defined(BCM_DNXF_SUPPORT)) */
...
#endif /* INCLUDE_CPU_I2C || SOC_PCI_DEBUG */
 ...
 *      soc_bar_read_reg
 * Purpose:
 *      Read a register through a device BAR
 * Parameters:
 *      unit - unit number
 *      reg - the register to read
 *      index - array index to use (should be 0 if the register is not an array)
 *      data - output value
 */
void soc_bar_read_reg32(int unit, soc_reg_t reg, int index, uint32 *data)
{
    soc_regtype_t regtype = SOC_REG_TYPE(unit, reg);
    uint32 addr = soc_reg_addr(unit, reg, REG_PORT_ANY, index);
    if (regtype == soc_cpureg) {
        /* Read PCI register value. */
        soc_pci_getreg(unit, addr, data);
#ifdef BCM_IPROC_SUPPORT
    } else if (regtype == soc_iprocreg) {
        *data = soc_cm_iproc_read(unit, addr);
#endif
 ...
 *      soc_bar_write_reg
 * Purpose:
 *      Write a register through a device BAR
 ...
 *      index - array index to use (should be 0 if the register is not an array)
 */
void soc_bar_write_reg32(int unit, soc_reg_t reg, int index, uint32 data)
{
    soc_regtype_t regtype = SOC_REG_TYPE(unit, reg);
    uint32 addr = soc_reg_addr(unit, reg, REG_PORT_ANY, index);
    if (regtype == soc_cpureg) {
