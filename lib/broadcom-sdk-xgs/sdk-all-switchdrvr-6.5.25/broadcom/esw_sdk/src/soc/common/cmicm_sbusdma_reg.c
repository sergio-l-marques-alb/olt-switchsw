/*
 * $Id: cmicm_sbusdma_reg.c,v 1.0 Broadcom SDK $
...
_cmicm_sbusdma_reg_array_read(int unit, soc_sbusdma_reg_param_t  *param)
{
   ...
   if (param->copyno == COPYNO_ALL) {
    ...
    /*
     * Cache flush/invalidate before DMA start to mitigate
     * cache coherency issue in some processors e.g. ARM with IOMMU.
     */
    if (soc->sbusdma_flush_cache_enable) {
        soc_cm_sinval(unit, (void *)param->buffer,
           WORDS2BYTES(data_beats) * count);
    }
    schan_msg_clear(&msg);
...
retry:
    ...
    if (rv != SOC_E_NONE) {
        if (rv != SOC_E_TIMEOUT) {
            ...
            /* Check the PCI error flag */
            (void)cmicm_sbusdma_ch_pci_err_get(unit, cmc, ch, &pci_err);
            if (pci_err) {
                (void)cmicm_sbusdma_ch_pci_err_set(unit, cmc, ch, 0);
                 rv = SOC_E_INTERNAL;
            }
        } else { /* Timeout cleanup */
            /* Abort Table DMA */
            rv = cmicm_sbusdma_ch_abort(unit, cmc, ch, soc->tableDmaTimeout);
            if (rv == SOC_E_TIMEOUT) {
                LOG_ERROR(BSL_LS_SOC_DMA,
                          (BSL_META_U(unit,
                                      "Fatal error: CMC %d channel %d abort failed, "
                                      "cold boot might be needed\n"),
                           cmc, ch));
            }

            /* Abort successfully and retry the current operation */
            if (rv == SOC_E_NONE && retry_times--) {
                goto retry;
            }
            ...
            LOG_ERROR(BSL_LS_SOC_DMA,
                      (BSL_META_U(unit,
                                  ...
                                  "addr 0x%x (index %d-%d), opcode 0x%x\n"),
                       count, data_beats, start_addr,
                       param->index_begin, param->index_end,
                       msg.dwords[0]));

            /* Generate switch event */
            sal_dpc(cmicm_sbusdma_event_dpc, INT_TO_PTR(unit),
                    INT_TO_PTR(SOC_SWITCH_EVENT_IO_ERROR_SBUSDMA_TIMEOUT),
                    INT_TO_PTR(cmc), INT_TO_PTR(ch), NULL);
        }
    }
        ...
        if (!param->flags || (param->flags & SOC_MEM_WRITE_STATUS_ONLY)) {
            ...
            if (rv < 0) {
                if (rv != SOC_E_TIMEOUT) {
                    ...
                    /* Abort Slam DMA */
                    rv = cmicm_sbusdma_ch_abort(unit, cmc, ch, soc->tslamDmaTimeout);
                    if (rv == SOC_E_TIMEOUT) {
                        LOG_ERROR(BSL_LS_SOC_DMA,
                                  (BSL_META_U(unit,
                                              "Fatal error: CMC %d channel %d abort failed, "
...
int cmicm_sbusdma_reg_init(int unit, soc_sbusdma_reg_drv_t *drv)
{
    ...
    soc->sbusdma_flush_cache_enable =
        soc_property_get(unit, spn_SBUSDMA_FLUSH_CACHE_ENABLE, 0);
