...
soc_cm_get_name(int dev)
{
    ...
    static char _cm_dev_name[SOC_MAX_NUM_DEVICES][16];
        ...
        if ((NULL!=SOC_CONTROL(dev)) &&
               (SOC_FLAGS_GET(dev) & SOC_F_INITED)) {
    ...
    {
#ifdef BCM_FIRELIGHT_SUPPORT
        if (SOC_IS_FIRELIGHT(dev)) {
            dev_id = CMDEV(dev).dev.dev_id;
            if ((dev_id == BCM56071_DEVICE_ID) && SOC_BOND_INFO_FEATURE_GET(dev, socBondInfoFeatureNoMacsec)) {
                sal_memcpy(_cm_dev_name[dev], CMDEV(dev).dev.info->dev_name, 8);
                 _cm_dev_name[dev][8] = 'N';
                sal_memcpy(&(_cm_dev_name[dev][9]), &(CMDEV(dev).dev.info->dev_name[8]),
                sal_strlen(CMDEV(dev).dev.info->dev_name)-8);
                return _cm_dev_name[dev];
            } else {
...
soc_cm_config_str_get(int dev, const char *name)
{
    ...
    if (dev == SOC_UNIT_NONE_SAL_CONFIG) {
        int dev_idx;
        char *s;
        for (dev_idx = 0; dev_idx < SOC_MAX_NUM_DEVICES; dev_idx++) {
 ...
 */
int
soc_cm_config_str_delete(int dev, const char *name)
{
#if defined(BCM_LTSW_SUPPORT) || defined(BCM_CTSW_SUPPORT)
    int rv = 0;
    int dev_num = 0;
 ...
 */
int
soc_cm_config_port_str_delete(int dev, int port, const char *name)
{
#if defined(BCM_LTSW_SUPPORT) || defined(BCM_CTSW_SUPPORT)
    int rv = 0;
    int dev_num = 0;
...
{
#if defined(BCM_LTSW_SUPPORT) || defined(BCM_CTSW_SUPPORT)
    return soc_ltsw_ha_mem_init(dev, pool_alloc, pool_free, pool_ctx,
                                pool_ptr, pool_size);
#else
    return 0;
#endif
 ...
 */
int
soc_cm_ha_mem_cleanup(int dev)
{
#if defined(BCM_LTSW_SUPPORT) || defined(BCM_CTSW_SUPPORT)
    return soc_ltsw_ha_mem_cleanup(dev);
#else
    return 0;
#endif
}
 ...
 */
int
soc_cm_ha_mem_reset(int dev)
{
#if defined(BCM_LTSW_SUPPORT) || defined(BCM_CTSW_SUPPORT)
    return soc_ltsw_ha_mem_reset(dev);
#else
    return 0;
#endif
}
