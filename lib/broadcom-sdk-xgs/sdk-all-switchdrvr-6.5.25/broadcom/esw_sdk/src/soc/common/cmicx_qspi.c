/*
 * $Id: cmicx_qspi.c,v 1.00 Broadcom SDK $
...
#endif
#ifndef IPROC_BSPI_QUAD_READ_CMD
#define IPROC_BSPI_QUAD_READ_CMD 0xeb
#endif
              ...
              uint32 mode)
{
    ...
    if ((SOC_IS_DNX(unit) || SOC_IS_DNXF(unit)) &&
        (IPROC_BSPI_DATA_LANES != 1 || IPROC_BSPI_ADDR_LANES != 1)) {
        ...
        if (IPROC_BSPI_ADDR_LANES == 4) {
            /* In current code this condition will always be true */
            val = soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "qspi_fast_read_cmd", (SOC_IS_DNX(unit) || SOC_IS_DNXF(unit)) ? IPROC_BSPI_QUAD_READ_CMD : IPROC_BSPI_READ_CMD);
        } else {
            val = soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "qspi_fast_read_cmd", IPROC_BSPI_READ_CMD);
        }
        ...
        if (IPROC_BSPI_ADDR_LANES == 4) {
            soc_cm_iproc_read(unit, 0x1c002000);
        }
    }
    /* MSPI: Basic hardware initialization */
    /* set the delay after transfer to 256 * 32 *n, where n is the system clock period*/
    IPROC_QSPI_WRITE(unit, QSPI_MSPI_SPCR1_LSBr, 0);
    /* set the delay from PCS valid to SCK transition. to 256 * n, where n is the system clock period*/
    IPROC_QSPI_WRITE(unit, QSPI_MSPI_SPCR1_MSBr, 0);
    /* set new queue pointer value, this register holds the address of the first MSPI queue entry*/
    IPROC_QSPI_WRITE(unit, QSPI_MSPI_NEWQPr, 0);
    /* When the MSPI completes the transfer, the value of endqp determines whether it is the last entry to be completed. after completing each command
     * the MSPI compares the current pointer value with the value of endqp. if the two value match. the SPIF bit in the MSPI STATUS register is set.
     * and if the wrap around mode is not enabled the MSPI turns it self off by clearing the spe bit in SPCR2 */
    IPROC_QSPI_WRITE(unit, QSPI_MSPI_ENDQPr, 0);
    IPROC_QSPI_WRITE(unit, QSPI_MSPI_SPCR2r, 0);
    /*TBD: MSPI: SCK configuration */
    /* In master mode the MSPI generates the baud rate for SCK. the frequency of which is determined by the modulus counter. the minimum that should be programmed is 8 */
    ...
    /* configure SPI is configured to be master*/
    /* Transfer 8 bits per word */
    /* configure clock phase and clock polarity CPOL / CPHA */
    conf->mspi_16bit = 0;
    val = 0x80 |                      /* Master */
         (8 << 2) |                  /* 8 bits per word */
...
/* Workaround releasing QSPI interface.
 * In PCIE FW loader which version is older than v2.10, R5_0 is holding the
 * QSPI interface in the pen-holder loop to wait for loading and start an eAPP.
 * If no eAPP was started, the R5_0 would be always holding the QSPI interface
 * and SDK QSPI driver could not get QSPI interface.
 * In this workaround, when QSPI timeout happen, check PEN_HOLDER content. If
 * it points to the flash space, reset R5_0 to release the QSPI interface.
 * This operation is safe if it happens NOT during R5_0 eAPP start sequence.
 * If it happens in abnormal case, the user should re-do eAPP start sequence.
 */
static void
war_r5_0_hold_qspi_in_idle(int unit)
{
    soc_timeout_t to;
    uint32 val, penholder = 0;
    uint32 rst_control = 0;

    if (soc_feature(unit, soc_feature_uc_mhost)) {
#ifdef INCLUDE_CPU_I2C
        iproc_qspi_read(unit, 0xFFFF0FE8, &penholder);
#else
        soc_iproc_getreg(unit, 0xFFFF0FE8, &penholder);
#endif
        if ((penholder & 0xFF000000) == 0x1C000000) {
...
enable_mspi(int unit, cmicx_qspi_conf_t *conf)
{
    ...
    if ((val & 0x01) == 0) {
        ...
        while(1) {
            ...
            if (soc_timeout_check(&to)) {
                if (SOC_IS_ESW(unit)) {
                    war_r5_0_hold_qspi_in_idle(unit);
                }
                break;
            }
        }
