/*
 * $Id: cmicx_sbusdma_common.c,v 1.00 Broadcom SDK $
...
cmicx_sbusdma_intr_wait(int unit, int cmc, int ch, int timeout, uint32 do_not_perform_start_operation)
{
    ...
    if (sal_sem_take(soc->sbusDmaIntrs[cmc][ch], timeout) < 0) {
        LOG_VERBOSE(BSL_LS_SOC_COMMON,
                    (BSL_META_U(unit, "Interrupt Timeout: cmc = %d, ch = %d\n"),
                     cmc, ch));
    }
    sal_spinlock_lock(_cmicx_sbusdma_ch.lock[unit]);
    _cmicx_sbusdma_ch.wait[unit][cmc] &= ~(0x01 << ch);
    ...
    if (rv != SOC_E_NONE) {
        ...
        if (rv == SOC_E_TIMEOUT) {
            soc_event_generate(unit, SOC_SWITCH_EVENT_IO_ERROR,
                               SOC_SWITCH_EVENT_IO_ERROR_SBUSDMA_TIMEOUT,
                               cmc, ch);
        }
    }
...
cmicx_sbusdma_ch_done(int unit, void *data)
{
    ...
    uint8 flag;
    uint8 mask =  0x01 << in_d->ch;

    if ((soc->sbusDmaIntrEnb) || (soc->tslamDmaIntrEnb) ||
               (soc->tableDmaIntrEnb)) {
        sal_spinlock_lock(_cmicx_sbusdma_ch.lock[unit]);
        flag = _cmicx_sbusdma_ch.wait[unit][in_d->cmc];
        sal_spinlock_unlock(_cmicx_sbusdma_ch.lock[unit]);
        if (flag & mask) {
            sal_sem_give(soc->sbusDmaIntrs[in_d->cmc][in_d->ch]);
        } else {
            LOG_VERBOSE(BSL_LS_SOC_COMMON,
                (BSL_META_U(unit, " Spurious SBUSDMA Interrupt:" \
                                  " cmc = %d, ch = %d, flag = 0x%x, mask = 0x%x\n"),
                                  in_d->cmc, in_d->ch, flag, mask));
        }
    }
}
