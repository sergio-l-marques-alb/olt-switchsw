...
soc_port_phy_eyescan_counter_register(int unit, soc_port_phy_eyscan_counter_t counter, soc_port_phy_eyescan_counter_cb_t* cf)
{
    ...
    if(counter >= socPortPhyEyescanNofCounters || (int)counter < 0) {
        LOG_ERROR(BSL_LS_SOC_PHY,
                  (BSL_META_U(unit,
    ...
    soc_port_phy_eyescan_results_t* results /*array of result per port*/)
{
    ...
    if(params->counter >= socPortPhyEyescanNofCounters || (int)(params->counter) < 0) {
        LOG_ERROR(BSL_LS_SOC_PHY,
                  (BSL_META_U(unit,
                              "Counter %d isn't supported\n"), params->counter));
        return SOC_E_PARAM;
            ...
            while ((under_threshold >= params->nof_threshold_links) && (params->sample_time <= params->time_upper_bound)) {
                ...
                for (j = 0; (j < nof_ports); j++) {
                    ...
                    if (first_run) {
                        ...
                        if ((flags & SRD_EYESCAN_FLAG_DSC_PRINT)){
                            rv = soc_phyctrl_control_set(unit, local_ports[j], SOC_PHY_CONTROL_DUMP, 1);
                            if (SOC_FAILURE(rv)) {
                                LOG_ERROR(BSL_LS_SOC_PHY,
                                          (BSL_META_U(unit,
                                                      "%s\n"), soc_errmsg(rv)));
...

{
    ...
    int     i, pm8x50 = 0, pm8x50_fabric = 0, pm8x50_ilkn = 0;
            ...
            for(idx = 0 ; idx < nof_phys ; idx++) {
                ...
                if(pm_type == portmodDispatchTypePmOsILKN_50G) {
                    pm8x50_ilkn = 1;
                }
#endif
                ...
                if (line_rates) {
                    if(pm8x50 == 1) {
                        ...
                        SOC_IF_ERROR_RETURN(rv);
                    } else if (pm8x50_fabric || pm8x50_ilkn) {
...
_eye_margin_diagram_cal(int unit, EYE_DIAG_INFOt *pInfo, soc_port_phy_eyescan_results_t* results)
{
    ...
    int beta_max=0;
    COMPILER_DOUBLE ber_conf_scale[20];
    int start_n;
    int stop_n;
    int low_confidence;
    int loop_index;
