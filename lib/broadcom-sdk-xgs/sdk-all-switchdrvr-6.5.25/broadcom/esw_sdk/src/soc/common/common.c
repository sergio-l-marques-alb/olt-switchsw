...
soc_reg_sprint_addr(int unit, char *bp, soc_regaddrinfo_t *ainfo)
{
    ...
    if (!ainfo->valid) {
        sal_memcpy(bp, "Invalid Address", sizeof("Invalid Address"));
        return;
    }
    if (ainfo->reg != INVALIDr && SOC_REG_IS_VALID(unit, ainfo->reg)) {
        ...
        sal_memcpy(bp, "Reserved", sizeof("Reserved"));
        return;
    }
    ...
    if (((reginfo->regtype != soc_portreg) &&
        ...
        *bp++ = '.';
        if ((reginfo->regtype == soc_xpereg) &&
            ...
            sal_memcpy(bp, SOC_BLOCK_NAME(unit, ainfo->block), (len + 1));
        }
        UPDATE_BP(bp);
    }
        ...
        *bp++ = '.';
        ...
        sal_memcpy(bp, SOC_PORT_NAME(unit, ainfo->port), (len + 1));
        UPDATE_BP(bp);
    }
    if (finfop) {
#ifdef SOC_NO_NAMES
