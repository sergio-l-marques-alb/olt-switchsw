...
if (SOC_IS_DNX(unit)) { \
    if (cpu2tap_mutexes[unit] == NULL) { /* If a cpu2tap mutex was not created for the device */ \
        ...
        soc_dnxc_set_in_cpu2tap(unit, 1); \
    } \
}
...
if (_func_rv != _SHR_E_NONE && SOC_IS_DNX(unit) && cpu2tap_mutexes[unit]) { \
    if (SOC_IS_DNX(unit)) { \
        soc_dnxc_set_cpu2tap_had_error(unit, 1); \
        soc_dnxc_set_in_cpu2tap(unit, 0); \
    } \
...
if (SOC_IS_DNX(unit)) { \
    if (_func_rv != SOC_E_NONE)  { \
        soc_dnxc_set_cpu2tap_had_error(unit, 1); \
    }  \
    if (cpu2tap_mutexes[unit] == NULL || (soc_dnxc_set_in_cpu2tap(unit, 0), sal_mutex_give(cpu2tap_mutexes[unit])) != SOC_E_NONE) { \
...
)
{
    ...
    if (mutex != NULL) {
        ...
        sal_mutex_destroy(mutex); /* destroy the mutex */
    }
exit:
    SHR_FUNC_EXIT;
}
...
/* function to initialize the CPU2TAP mechanism. */
...
/* function to initialize the CPU2TAP mechanism, first with enable+reset, then only enable */
...
/* function to de-initialize the CPU2TAP mechanism */
...
{
    /* Do not report done in SER testing iteration end unless asked for */
...
/*
 * De-initialize the CPU2TAP mechanism, not resetting the CPU2TAP interface.
...
/*
 * De-initialize the CPU2TAP mechanism after running MBIST, resetting the CPU2TAP interface.
 * We assume the MBIST scripts leave regular memory access working.
 * Requires a hard reset to later be performed.
 */
...
{
    /* Do not log in SER testing iteration end unless asked for */
    unsigned log_level = BSL_LOG_MODULE | (dynamic->log_level ? dynamic->log_level : (dynamic->ser_test_delay ? BSL_DEBUG : BSL_INFO));
    SHR_FUNC_INIT_VARS(unit);
