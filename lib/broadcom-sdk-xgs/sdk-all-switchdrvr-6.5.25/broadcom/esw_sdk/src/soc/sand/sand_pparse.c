...
#ifdef BCM_DNX_SUPPORT
/*
 * This function is adding 'dynamic' next_header conventions (i.e. local SDK conventions for next header,
 * we don't add them to NetworkHeadersObjects.xml file because they can be changed),
 * Unlike this, the pparse_pdb_next_headers_add function is adding the general next-headers protocols from NetworkHeadersObjects.xml file.
 * For now the only dynamic next header protocol is RCH_Next_Protocol.
 */
static shr_error_e
    ...
    parsing_db_t * pdb)
{
    ...
    /*
     * Add a RCH_Next_Protocol
     */
    {
        /*
         * Add a RCH_Next_Protocol node to next_headers_lst
         */
        ...
        /*
         * Fill the new RCH_Next_Protocol node with name-value list as in CURRENT_PROTOCOL_TYPE enum
         */
    ...
    rhlist_t * pit)
{
    ...
    while (NULL != (next = pparse_list_get_next(unit, parse_list)))
    {
        ...
        /*
         * Get details from the parsing DB for parsing the header
         */
        ...
        if (next->pdb_entry->size > 0)
        {
...
#ifdef BCM_DNX_SUPPORT
            /*
             * Start to parse the header fields in details:
             *
            ...
            if (port_skip_support && met_real_header == FALSE)
            {
                ...
                /** check that signal Recycle_Cmd is valid (the signal is found + its value is not N/A) */
        ...
        /*
         * Add extensions(next protocol) if there are and they meet the conditions to add them
         */
...
/**
 * \brief
 *  Get the port header type of the packet
 */
static shr_error_e
packet_src_port_get(
    int unit,
    ...
    uint32 *enabled)
{
    ...
    *enabled = FALSE;
    ...
    /** The analysis is per core. If core_id is not given, do nothing.*/
    ...
    /** Find out the mpls_unterm layer index */
    ...
    for (layer_protocol_offset = 0; layer_protocol_offset < sig_layer_protocol_p->size;)
    {
        ...
        {
            /** It's layer_types_system. MPLS_UNTERM is impossible.*/
            SHR_EXIT();
        }
    ...
    /** Check if all mpls headers are terminated.*/
    rv = sand_signal_output_find(unit, core, SIGNALS_MATCH_NO_FETCH_ALL, "IRPP",
            dnx_data_pp.debug.from_signal_str_get(unit)->ingress_layers, "FWD1", "Fwd_Layer_Index", &sig_fwd_index_p);
    if (rv != _SHR_E_NONE)
    {
        /** Generally, the signal will be found. If not, e.g. no traffic on the core, do not go ahead.*/
        SHR_EXIT();
    }
    ...
    {
        /** Not all mpls are terminated, do nothing.*/
        SHR_EXIT();
    }

    /** Check if all mpls headers are terminated before VT5.*/
    ...
    /** Check if VT5 is also used for MPLS termination.*/
    ...
    {
        /** The global inLIF is not expected, do nothing*/
        SHR_EXIT();
    }

    /** To avoid error log, we check if the global inLIF is valid first.*/
    {
