...
* \param [in] unit - Unit #
* \param [in] input_buffer - A bitstream contains all signals of the stage.
*             \Within the bitstream array, the least significant word comes first.
*             \However, each word itself is stored in the platform native byte order.
* \param [in] start_bit
* \param [in] nof_bits - nof_bits to copy into debug_signal->adapter_value
* \param [in] to - Name of dest stage or DB
* \param [in] from - Name of source stage or DB
* \param [in] attr - Signal name
...
* Parses a packet depicted by packet_info and populates the result in the parsed info tree pit.
* \param [in] unit - Unit #
* \param [in] mdb_type - 0-direst, 1-EM, 2-EEDB, 3-LPM, TCAM-4
* \param [in] memory_id - db_id as in mdb_adapter_mapping table.
* \param [out] db_id - db_id.
* \return shr_error_e Standard error handeling
*/
       ...
       int *db_id)
{
    ...
    /*
     *       Explanation: We treat TCAM differently because memory_id in TCAM is the actual tcam-id was accessed,
     *       while in mdb_adapter_mapping_get TCAM is 0.
     */
...
/**
* \brief
* Allows us to skip signals we don't want to add.
* \param [in] unit - Unit #
* \param [in] mdb_type - 0-direst, 1-EM, 2-EEDB, 3-LPM, TCAM-4
* \param [in] adapter_stage_index
* \param [in] kbr_index
* \param [out] boolean: 1-skip signal, 0-add signal.
* \return int: 1-skip signal, 0-add signal.
*/
...
/**
* \brief
* Get app_db size and encoding, app_db has 3 cases of encoding.
* \param [in] unit - Unit #
* \param [in] mdb_type - 0-direst, 1-EM, 2-EEDB, 3-LPM, TCAM-4
* \param [in] db_id
* \param [in] key_size
* \param [out] app_db_size
* \param [out] is_app_db_part_of_key: 0-no, 1-yes.
* \return void
*/
static void
...
/**
* \brief
* Calculate res_size and make adjust res in terms of shifting and 0 filling.
* \param [in] unit - Unit #
* \param [in] mdb_type - 0-direst, 1-EM, 2-EEDB, 3-LPM, TCAM-4
* \param [in] db_id
* \param [in] db_name
* \param [in] adapter_value - A bitstream contains all signals of the stage.
 *             \Within the bitstream array, the least significant word comes first.
 *             \However, each word itself is stored in the platform native byte order.
* \param [in] data_index - index of curr data in adapter_value
* \param [in] data_size - Size of result we got from adapter (size of data starting on data_index, this is
* \param [out] res_size - Actual final size of res.
       ...
       uint32 *res)
{
    ...
    /*
     * In case of no data, create a default 0 Result.
     */
    if ( data_size == 0 )
    {
    ...
    else if (sal_strncmp(db_name, "KAPS", sal_strlen("KAPS")) == 0 )
    {
        (*res_size) = DEFAULT_RESULT_SIZE;
        /*
         * Remove strength bits (But not shifting, therefore it will have a 000 in MSB).
         */
    ...
    char *res)
{
    ...
    /*
     * In cases interface info is passed in format of 1000*stage+kbr
     */
    ...
    /*
     * In cases caller_id is passed from adapter (instead of format of 1000*stage+kbr)
     * We want to use a map of caller_id->interface
     */
    {
        if (kbr_index != 0 )
        {
...
/**
 * \brief
 *  Parse MDB signals and add them to stage signals.
 *
 *  Signals are coded as follow:
 *   --------------------------------------------------------------------------------------------
 *      ms_id(16) caller_interface(16) mdb_type(1b) db_id(16b) keys payloads.hit (8b) error (8b)
 *   --------------------------------------------------------------------------------------------
 *     Where keys is as follow:
 *   --------------------------------------------------------------------------------------------
 *      nof_keys(1b, up to 4 keys) key1_size(16b) key1 data ... keyN_size keyN data
 *   --------------------------------------------------------------------------------------------
 *     Payloads is as follow
 *   --------------------------------------------------------------------------------------------
 *     nof_payloads(1b, up to 8) payload1_size(16b) payload1_data ... payloadN_size payloadN_data
 *   --------------------------------------------------------------------------------------------
 *
 * \param [in] unit - Unit #
 * \param [in] signal_size - Length of adapter_value (nof bits).
 * \param [in] adapter_value - A bitstream contains all signals of the stage.
 *             \Within the bitstream array, the least significant word comes first.
 *             \However, each word itself is stored in the platform native byte order.
 * \param [out] cur_pp_stage - New signals will be added to cur_pp_stage->stage_debug_signals
 * \return shr_error_e Standard error handeling
 */
shr_error_e
    ...
    pp_stage_t *cur_pp_stage)
{
    ...
    while ( data_index > 0)
    {
        ...
        /*
         * Read & Parse caller_interf.
         * caller_interf = adapter_stage*1000+kbr_index, therefor:
         * caller_interf/1000 is the adapter_stage (i.e. FWD1=6)
         * we translate it into IKLEAP stage (i.e. FWD1=3)
         * caller_interf%1000 is kbr_index.
         */
        data_index-=16;
        ...
        {
            /*
             * Get Appdb size and type of coding, and in 1 case get the Appdb itself.
             *
             * Appdb field parsing is inconsistent, since it has 3 different options it can be coded:
             * 1. Appdb external from the key: In this case the appdb is coded as a separate key (nof_keys=2
             *     instead of 1), this case is only when db_type is TCAM (but not all TCAMs are coded this way).
             * 2. Appdb as part of the key coding, but we must separate the appdb from the key.
             * 3. Appdb is the first a few bits of the key i.e. you read these bits twice first time as appdb
             *     and second as the key.
             *
             */
        ...
        /*
         * Read nof_data_payloads(Up to 8)
         */
        data_index-=8;
        ...
        int flags)
{
    ...
    for(i_sig = 0; i_sig < adapter_info.nof; i_sig++)
    {
        ...
        /*
         * In case it is an MDB access signal, no need to add it, just parse its parts
         */
