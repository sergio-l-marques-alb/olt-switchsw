  ...
  )
{
    ...
    /* When using I2C access, use indirect write to fill the table */
    ...
    /*
     * in emulation we want to continue in case of memory init failures,
     * to not loose the long startup time we spent
     */
...
/*
 * Initialize indirect access data structures.
 */
...
/*
 * De-initialize indirect access data structures.
 */
...
/*
 * Lock the given block instance for indirect memory access.
 * broadcast/multicast blocks need to lock instead all of the instances written to.
 */
...
/*
 * Unlock the given block instance for indirect memory access.
 * broadcast/multicast blocks need to lock instead all of the instances written to.
 */
...
int soc_sand_cache_table_update_all(int unit)
{
    ...
    {
        /** Skip memory which not protected by ECC/Parity */
...
/*
 *  Check if given memory cache disabled or enabled by specific
 */
int
...
* \param [in] address - address in block memory address space to start writing at
...
/**
* \brief - Fill (write the same value to) continuous entries in a memory, using the indirect write mechanism of its block.
*          Does not work for wide memories.
*          Does not perform locking of the needed locking of the block instance.
*          Does not perform broadcast writes, and instead writes to each instance separately.
*
* \param [in] unit - unit #
* \param [in] mem - memory to fill #
* \param [in] copyno - memory block instance to fill
* \param [in] array_index_start - first array index to fill
* \param [in] array_index_end - last array index to fill
* \param [in] index_start - first (entry) index to fill
* \param [in] index_end - last (entry) index to fill
* \param [in] entry - value of the entries to write
* \return
*   See shr_error_e
*/
int
   ...
   if (entry == NULL) {
    ...
    if (SOC_MEM_IS_ARRAY(unit, mem) && (maip = SOC_MEM_ARRAY_INFOP(unit, mem)) != NULL) {
        ...
        if (array_index_end < array_index_start) {
            ...
            uint32_index = maip->numels + maip->first_array_index; /* max array index + 1 */
    ...
    /* Is this a memory array where we can write using continuous addressed between continuous array indices */
    ...
    /* write the entries not exceeding MAX_ENTRIES_IN_WRITE entries per write */
    for (array_i = array_index_start; array_i <= array_index_end; ++array_i) {
        ...
        for (entry_i = index_start; entry_i <= index_end ; entry_i += MAX_ENTRIES_IN_WRITE) {
            nof_entries = index_end + 1 - entry_i; /* calculate number of entries to write at this iteration */
            if (nof_entries > MAX_ENTRIES_IN_WRITE) {
                nof_entries = MAX_ENTRIES_IN_WRITE;
            }
            /* write the same entries in every block instance in parallel */
    ...
    /* foreach block instance, verify that the last write is done, and update all the cachewrite the same entries in every block instance in parallel */
    for (blk_inst = blk_inst_start; blk_inst <= blk_inst_end; ++blk_inst) {
        /*_soc_mem_write_cache_update_range(unit, mem, array_index_start, array_index_end, blk_inst, index_start, index_end, entry);*/
