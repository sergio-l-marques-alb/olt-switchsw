    ...
    xml_node nodeTop)
{
    ...
    RHDATA_ITERATOR(cur, curSubTop, "structure")
    {
        ...
        /* check if requested to override existing structure or just add similar one over it */
        if(override && (utilex_rhlist_name_exists(struct_list, name, &temp) == _SHR_E_NONE))
        {
            ...
            /* allow override only if name is unique */
        ...
        if(!ISEMPTY(name))
        {
            ...
            else  if (SOC_IS_DNX(unit))
            {
                ...
                else if(!sal_strcasecmp(name, "pmf_context"))
                {
                    sigstruct->expansion_cb = pmf_context_decode;
                }
            }
    ...
    device_t * device)
{
    ...
    /*
     * Check if there are translated signals from user expected to device specific
     */
    if ((curTop = dbx_pre_compiled_common_top_get(unit, "TranslatedDebugSignals.xml", "signals", CONF_OPEN_NO_ERROR_REPORT)) != NULL)
    {
        ...
        RHDATA_ITERATOR(cur, curTop, "UserSignal")
        {
            ...
            {
                RHDATA_GET_STR_CONT(curDevice, "chip", chip_n);
                RHDATA_GET_INT_DEF(curDevice, "adapter", is_adapter, -1);
                if(!sal_strcasecmp(this_chip_n, chip_n) &&
                    ((is_adapter == -1) || (this_is_adapter == is_adapter)))
                {
                    char signal_n[RHNAME_MAX_SIZE] = {0};
                    /*
                     * Add this element to the list
                     */
                    RHDATA_GET_STR_DEF_NULL(cur, "name", signal_n);
    ...
    device_t * device)
{
    ...
    RHDATA_ITERATOR(curBlock, curSubTop, "block")
    {
        ...
        SHR_IF_ERR_EXIT(sand_signal_get_stages(unit, cur_pp_block, curBlock));
        {
            ...
            for (i_stage = 0; i_stage < cur_pp_block->stage_num; i_stage++)
            {
                if (cur_pp_block->stages[i_stage].stage_debug_signals == NULL)
                {
                    if((cur_pp_block->stages[i_stage].stage_debug_signals = 
                        utilex_alloc(sizeof(debug_signal_t) * DNX_MAX_NOF_DEBUG_SIGNALS)) == NULL)
                    {
                        SHR_ERR_EXIT(_SHR_E_MEMORY, "Memory allocation of:%zu failed for debug signals\n",
                                                    sizeof(debug_signal_t) * DNX_MAX_NOF_DEBUG_SIGNALS);
                    }
                }
                sal_memset(cur_pp_block->stages[i_stage].stage_debug_signals, 0, sizeof(debug_signal_t) * DNX_MAX_NOF_DEBUG_SIGNALS);
                cur_pp_block->stages[i_stage].stage_signal_num = 0;
            }
