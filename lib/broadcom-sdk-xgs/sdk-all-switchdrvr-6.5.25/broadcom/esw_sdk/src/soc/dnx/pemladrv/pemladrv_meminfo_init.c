...
int parse_meminfo_definition_file(int unit, int restore_after_reset, uint32 use_file, const char *rel_file_path, const char *file_name)
{
    ...
    if(use_file)
    {
        ...
        {
            /*sal_printf("Can't open file '%s'. Exit.\n", file_name);*/
            return UINT_MAX;
        }
  ...
  /* Read one line per iteration and parse it */
    ...
    /* Handle comments */
    if (sal_strnstr(line, END_COMMENT, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) != 0)    { in_comment = 0; ++line_no; continue;} /* Note that this match also one-line-comment */
