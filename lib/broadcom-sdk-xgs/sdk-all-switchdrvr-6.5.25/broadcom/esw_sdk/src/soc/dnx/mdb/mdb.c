    ...
    int *capacity)
{
    mdb_physical_table_e mdb_physical_table_id =
        dnx_data_mdb.pdbs.dbal_pdb_info_get(unit, dbal_physical_table)->logical_to_physical;
    uint32 row_width;
    int temp_capacity = 0;
    int cluster_idx;
    int nof_clusters;
    mdb_physical_table_e mdb_physical_table_id_other;
    int table_id_shift = dnx_data_mdb.pdbs.dbal_pdb_info_get(unit, dbal_physical_table)->db_subtype;
    ...
    mdb_physical_table_id = MDB_PHYSICAL_TABLE_BIG_KAPS_BB_1 + table_id_shift;
    mdb_physical_table_id_other =
        MDB_PHYSICAL_TABLE_BIG_KAPS_BB_2 + table_id_shift -
        (MDB_LPM_DBS_IN_PAIR * (table_id_shift % MDB_LPM_DBS_IN_PAIR));
    ...
    if (temp_capacity == 0)
    {
        ...
        if ((nof_clusters_other == 0) || (requested_kaps_cfg == MDB_LPM_KAPS_CFG_SMALL_KAPS_ENABLE * (db_idx + 1)))
        {
            ...
            temp_capacity = num_of_small_bbs *
                (dnx_data_mdb.kaps.db_info_get(unit, db_idx)->nof_rows_in_small_bb) *
                dnx_data_mdb.kaps.max_prefix_in_bb_row_get(unit);
        }
    }
    ...
    int *capacity)
{
    ...
    dbal_enum_value_field_mdb_db_type_e mdb_db_type =
        dnx_data_mdb.pdbs.dbal_pdb_info_get(unit, dbal_physical_table)->db_type;
    ...
    if (dnx_data_mdb.global.adapter_stub_enable_get(unit) && (mdb_db_type != DBAL_ENUM_FVAL_MDB_DB_TYPE_DIRECT))
    {
        temp_capacity = dnx_data_mdb.pdbs.mdb_adapter_mapping_get(unit, dbal_physical_table)->capacity;
    }
    else
#endif

    {
        ...
        {
            SHR_IF_ERR_EXIT(mdb_get_capacity_kaps(unit, dbal_physical_table, &temp_capacity));
        }
...
#ifdef ADAPTER_SERVER_MODE
            temp_capacity = MDB_EM_LPM_ADAPTER_SIZE;
#endif
        }
        else if (mdb_db_type == DBAL_ENUM_FVAL_MDB_DB_TYPE_EEDB)
        {
            ...
            SHR_IF_ERR_EXIT(mdb_eedb_calc_capacity(unit, dbal_physical_table, &temp_capacity));
        }
        else if (mdb_db_type == DBAL_ENUM_FVAL_MDB_DB_TYPE_DIRECT)
        {
            ...
            if (payload_type != DBAL_NOF_ENUM_DIRECT_PAYLOAD_SIZES_VALUES)
            {
                ...
                if (nof_clusters != 0)
                {
                    temp_capacity = UTILEX_DIV_ROUND_UP((row_width * (max_end_address - min_start_address)),
            ...
            {
                if (temp_capacity > dnx_data_mdb.pdbs.mdb_adapter_mapping_get(unit, dbal_physical_table)->capacity)
                {
                    temp_capacity = dnx_data_mdb.pdbs.mdb_adapter_mapping_get(unit, dbal_physical_table)->capacity;
                }
            }
...
#endif
    {
        if (mdb_db_type == DBAL_ENUM_FVAL_MDB_DB_TYPE_EM)
        {
            ...
            SHR_IF_ERR_EXIT(mdb_em_get_entry_encoding
                            (unit, dbal_physical_table_id, key_size, payload_size, app_id, app_id_size,
                             &entry_encoding));
            ...
            *capacity_estimate = capacity / (UTILEX_DIV_ROUND_UP(payload_size, table_entry_size));
            ...
            if (dnx_data_mdb.global.adapter_stub_enable_get(unit))
            {
                if (*capacity_estimate >
                    dnx_data_mdb.pdbs.mdb_adapter_mapping_get(unit, dbal_physical_table_id)->capacity)
                {
                    *capacity_estimate =
                        dnx_data_mdb.pdbs.mdb_adapter_mapping_get(unit, dbal_physical_table_id)->capacity;
                }
        ...
        for (macro_idx = 0; macro_idx < max_macro; macro_idx++)
        {
            for (cluster_idx = 0; cluster_idx < max_cluster; cluster_idx++)
            {
                SHR_IF_ERR_EXIT(mdb_init_set_cluster_assoc
                                (unit, macro_type, macro_idx, cluster_idx, MDB_NOF_PHYSICAL_TABLES));
            }
        }
    }
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(mdb_em_db.init(unit));
    ...
    SHR_IF_ERR_EXIT(mdb_db_infos.fec_db_info.fec_db_map.alloc(unit));
    SHR_IF_ERR_EXIT(mdb_db_infos.fec_db_info.fec_id_range_start.alloc(unit));
    SHR_IF_ERR_EXIT(mdb_db_infos.fec_db_info.fec_id_range_size.alloc(unit));
...
mdb_get_dh_block(
    ...
    int *blk)
{
    int block_index = dnx_data_mdb.dh.block_info_get(unit, global_macro_index)->block_index;
    ...
    switch (dnx_data_mdb.dh.block_info_get(unit, global_macro_index)->block_type)
    {
        case DBAL_ENUM_FVAL_MDB_BLOCK_TYPES_DDHA:
            *blk = DDHA_BLOCK(unit, block_index);
            break;
        case DBAL_ENUM_FVAL_MDB_BLOCK_TYPES_DDHB:
            *blk = DDHB_BLOCK(unit, block_index);
            break;
        case DBAL_ENUM_FVAL_MDB_BLOCK_TYPES_DHC:
            *blk = DHC_BLOCK(unit, block_index);
            break;
        default:
    ...
    uint8 *is_allocated)
{
    ...
    mdb_physical_table = dnx_data_mdb.pdbs.dbal_pdb_info_get(unit, dbal_phy_table_id)->logical_to_physical;
    ...
    uint8 *is_mdb)
{
    ...
    if ((access_method != DBAL_ACCESS_METHOD_MDB)
        ||
        (sal_strnstr
         (dbal_logical_table_to_string(unit, dbal_logical_table_id), "EXAMPLE_",
          SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) != NULL))
    {
        *is_mdb = FALSE;
    }
    else
