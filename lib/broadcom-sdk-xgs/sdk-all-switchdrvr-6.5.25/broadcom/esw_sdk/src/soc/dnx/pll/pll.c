    ...
    DNX_PLL1_TYPE pll1_type)
{
    ...
    {
        refclk_source_sel =
            (dnx_data_pll.general.feature_get(unit, dnx_data_pll_general_ts_refclk_source_internal) == 0);
    }
    else
    {
    ...
    DNX_PLL4_TYPE pll4_type)
{
    ...
    {
        refclk_source_sel = dnx_data_pll.pll4.config_get(unit, pll4_type)->ref_clk_src_sel;
    }
    ...
    /**
     * In fractional mode also need to configure the Np and Nq dividers
     */
    if (dnx_data_pll.pll4.feature_get(unit, dnx_data_pll_pll4_pll_in_frac_mode))
    {
        if (dnx_data_pll.pll4.config_get(unit, pll4_type)->ndiv_frac_mode != -1)
        {
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_NDIV_FRAC_MODE, INST_SINGLE,
                                         dnx_data_pll.pll4.config_get(unit, pll4_type)->ndiv_frac_mode);
        }
        if (dnx_data_pll.pll4.config_get(unit, pll4_type)->ndiv_p != -1)
        {
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_NDIV_P, INST_SINGLE,
                                         dnx_data_pll.pll4.config_get(unit, pll4_type)->ndiv_p);
        }
        if (dnx_data_pll.pll4.config_get(unit, pll4_type)->ndiv_q != -1)
        {
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_NDIV_Q, INST_SINGLE,
                                         dnx_data_pll.pll4.config_get(unit, pll4_type)->ndiv_q);
        }
    }
 ...
 * \param [out] enable -
 *   SyncE enable. 1 power up; 0 is power down
 * \return
 *   See \ref shr_error_e
 * \remark
 *   None.
 ...
 */
int
soc_dnx_synce_pll_power_enable_get(
    int unit,
    int synce_index,
    uint32 *enable)
{
    ...
    /*
     * request the power state
     */
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_SYNCE_PLL_PWRDN, INST_SINGLE, enable);
    /*
     * get the value
     */
...
/**
 * \brief
 *   SyncE PLL power up or down
 * \param [in] unit -
 *   The unit number.
 * \param [in] synce_index -
 *   Master or Slave SyncE.
 * \param [in] enable -
 *   SyncE power up or down.
 * \return
 *   See \ref shr_error_e
 * \remark
 *   None.
 ...
 */
int
soc_dnx_synce_pll_power_enable_set(
    int unit,
    int synce_index,
    int enable)
{
     ...
     * set value field
     */
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_SYNCE_PLL_PWRDN, INST_SINGLE, !enable);
    ...
    int time_stamp_index)
{
    ...
    SHR_IF_ERR_EXIT(soc_dnx_pll_ts_freq_ctrl_high_set
                    (unit, time_stamp_index, dnx_data_pll.general.ts_phase_initial_hi_get(unit)));
    SHR_IF_ERR_EXIT(soc_dnx_pll_ts_freq_ctrl_low_set
                    (unit, time_stamp_index, dnx_data_pll.general.ts_phase_initial_lo_get(unit)));
    ...
    uint64 *timestamp_ns)
{
    ...
    int right_shift_to_ns = dnx_data_pll.general.ts_nof_bits_right_shift_ns_get(unit);
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);
    ...
    /*
     * In HW the value is separated into two parts - high and low bits.
     * We also want to convert that value to nanoseconds, as the HW value may have
     * granularity of less than nanoseconds.
     */
...
/**
 * \brief
 *  PLLs type 1 init function.
 *  Used to init all PLLs of type 1 on the current device
 * \param [in] unit -
 *   The unit number.
 * \return
 *   See \ref shr_error_e
...
/**
 * \brief
 *  PLLs type 3 init function.
 *  Used to init all PLLs of type 3 on the current device
 * \param [in] unit -
 *   The unit number.
 * \return
 *   See \ref shr_error_e
...
/**
 * \brief
 *  PLLs type 4 init function.
 *  Used to init all PLLs of type 4 on the current device
 * \param [in] unit -
 *   The unit number.
 * \return
 *   See \ref shr_error_e
    ...
    int unit)
{
     ...
     * Init All PLL1s
     */
    SHR_IF_ERR_EXIT(soc_dnx_pll_1_init_all(unit));

    /**
     * Init All PLL3s
     */
    SHR_IF_ERR_EXIT(soc_dnx_pll_3_init_all(unit));

    /**
     * Init All PLL4s
     */
    SHR_IF_ERR_EXIT(soc_dnx_pll_4_init_all(unit));
    ...
    /*
     * Needed FMAC Soft Reset after the PLL configuration for J2C/J2P/J2X because of the FAB PLL
     */
    if (SOC_IS_J2C(unit) || SOC_IS_J2P(unit) || SOC_IS_J2X(unit))
    {
        ...
        if (SOC_IS_J2P(unit))
        {
            soc_reg_above_64_field_set(unit, ECI_BLOCKS_SOFT_RESETr, reg_above_64_val, BLOCKS_SOFT_RESET_FMAC_12f,
                                       field_val);
            soc_reg_above_64_field_set(unit, ECI_BLOCKS_SOFT_RESETr, reg_above_64_val, BLOCKS_SOFT_RESET_FMAC_13f,
                                       field_val);
            soc_reg_above_64_field_set(unit, ECI_BLOCKS_SOFT_RESETr, reg_above_64_val, BLOCKS_SOFT_RESET_FMAC_14f,
                                       field_val);
            soc_reg_above_64_field_set(unit, ECI_BLOCKS_SOFT_RESETr, reg_above_64_val, BLOCKS_SOFT_RESET_FMAC_15f,
                                       field_val);
            soc_reg_above_64_field_set(unit, ECI_BLOCKS_SOFT_RESETr, reg_above_64_val, BLOCKS_SOFT_RESET_FMAC_16f,
                                       field_val);
            soc_reg_above_64_field_set(unit, ECI_BLOCKS_SOFT_RESETr, reg_above_64_val, BLOCKS_SOFT_RESET_FMAC_17f,
                                       field_val);
            soc_reg_above_64_field_set(unit, ECI_BLOCKS_SOFT_RESETr, reg_above_64_val, BLOCKS_SOFT_RESET_FMAC_18f,
                                       field_val);
            soc_reg_above_64_field_set(unit, ECI_BLOCKS_SOFT_RESETr, reg_above_64_val, BLOCKS_SOFT_RESET_FMAC_19f,
                                       field_val);
            soc_reg_above_64_field_set(unit, ECI_BLOCKS_SOFT_RESETr, reg_above_64_val, BLOCKS_SOFT_RESET_FMAC_20f,
                                       field_val);
            soc_reg_above_64_field_set(unit, ECI_BLOCKS_SOFT_RESETr, reg_above_64_val, BLOCKS_SOFT_RESET_FMAC_21f,
                                       field_val);
            soc_reg_above_64_field_set(unit, ECI_BLOCKS_SOFT_RESETr, reg_above_64_val, BLOCKS_SOFT_RESET_FMAC_22f,
                                       field_val);
            soc_reg_above_64_field_set(unit, ECI_BLOCKS_SOFT_RESETr, reg_above_64_val, BLOCKS_SOFT_RESET_FMAC_23f,
                                       field_val);
            soc_reg_above_64_field_set(unit, ECI_BLOCKS_SOFT_RESETr, reg_above_64_val, BLOCKS_SOFT_RESET_FMAC_24f,
                                       field_val);
            soc_reg_above_64_field_set(unit, ECI_BLOCKS_SOFT_RESETr, reg_above_64_val, BLOCKS_SOFT_RESET_FMAC_25f,
                                       field_val);
            soc_reg_above_64_field_set(unit, ECI_BLOCKS_SOFT_RESETr, reg_above_64_val, BLOCKS_SOFT_RESET_FMAC_26f,
                                       field_val);
            soc_reg_above_64_field_set(unit, ECI_BLOCKS_SOFT_RESETr, reg_above_64_val, BLOCKS_SOFT_RESET_FMAC_27f,
                                       field_val);
            soc_reg_above_64_field_set(unit, ECI_BLOCKS_SOFT_RESETr, reg_above_64_val, BLOCKS_SOFT_RESET_FMAC_28f,
                                       field_val);
            soc_reg_above_64_field_set(unit, ECI_BLOCKS_SOFT_RESETr, reg_above_64_val, BLOCKS_SOFT_RESET_FMAC_29f,
                                       field_val);
            soc_reg_above_64_field_set(unit, ECI_BLOCKS_SOFT_RESETr, reg_above_64_val, BLOCKS_SOFT_RESET_FMAC_30f,
                                       field_val);
            soc_reg_above_64_field_set(unit, ECI_BLOCKS_SOFT_RESETr, reg_above_64_val, BLOCKS_SOFT_RESET_FMAC_31f,
                                       field_val);
            soc_reg_above_64_field_set(unit, ECI_BLOCKS_SOFT_RESETr, reg_above_64_val, BLOCKS_SOFT_RESET_FMAC_32f,
                                       field_val);
            soc_reg_above_64_field_set(unit, ECI_BLOCKS_SOFT_RESETr, reg_above_64_val, BLOCKS_SOFT_RESET_FMAC_33f,
                                       field_val);
            soc_reg_above_64_field_set(unit, ECI_BLOCKS_SOFT_RESETr, reg_above_64_val, BLOCKS_SOFT_RESET_FMAC_34f,
                                       field_val);
            soc_reg_above_64_field_set(unit, ECI_BLOCKS_SOFT_RESETr, reg_above_64_val, BLOCKS_SOFT_RESET_FMAC_35f,
                                       field_val);
            soc_reg_above_64_field_set(unit, ECI_BLOCKS_SOFT_RESETr, reg_above_64_val, BLOCKS_SOFT_RESET_FMAC_36f,
                                       field_val);
            soc_reg_above_64_field_set(unit, ECI_BLOCKS_SOFT_RESETr, reg_above_64_val, BLOCKS_SOFT_RESET_FMAC_37f,
                                       field_val);
            soc_reg_above_64_field_set(unit, ECI_BLOCKS_SOFT_RESETr, reg_above_64_val, BLOCKS_SOFT_RESET_FMAC_38f,
                                       field_val);
            soc_reg_above_64_field_set(unit, ECI_BLOCKS_SOFT_RESETr, reg_above_64_val, BLOCKS_SOFT_RESET_FMAC_39f,
                                       field_val);
            soc_reg_above_64_field_set(unit, ECI_BLOCKS_SOFT_RESETr, reg_above_64_val, BLOCKS_SOFT_RESET_FMAC_40f,
                                       field_val);
            soc_reg_above_64_field_set(unit, ECI_BLOCKS_SOFT_RESETr, reg_above_64_val, BLOCKS_SOFT_RESET_FMAC_41f,
                                       field_val);
            soc_reg_above_64_field_set(unit, ECI_BLOCKS_SOFT_RESETr, reg_above_64_val, BLOCKS_SOFT_RESET_FMAC_42f,
                                       field_val);
            soc_reg_above_64_field_set(unit, ECI_BLOCKS_SOFT_RESETr, reg_above_64_val, BLOCKS_SOFT_RESET_FMAC_43f,
                                       field_val);
            soc_reg_above_64_field_set(unit, ECI_BLOCKS_SOFT_RESETr, reg_above_64_val, BLOCKS_SOFT_RESET_FMAC_44f,
                                       field_val);
            soc_reg_above_64_field_set(unit, ECI_BLOCKS_SOFT_RESETr, reg_above_64_val, BLOCKS_SOFT_RESET_FMAC_45f,
                                       field_val);
            soc_reg_above_64_field_set(unit, ECI_BLOCKS_SOFT_RESETr, reg_above_64_val, BLOCKS_SOFT_RESET_FMAC_46f,
                                       field_val);
            soc_reg_above_64_field_set(unit, ECI_BLOCKS_SOFT_RESETr, reg_above_64_val, BLOCKS_SOFT_RESET_FMAC_47f,
                                       field_val);
        }
