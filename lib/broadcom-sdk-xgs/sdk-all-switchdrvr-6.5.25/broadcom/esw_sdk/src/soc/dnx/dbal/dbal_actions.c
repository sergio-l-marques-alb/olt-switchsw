...
#define DBAL_FIELD_SET_ERROR_MSG "Cannot perform action. There is an issue with setting value to field"
/**
 *  \brief
 *  Propagate post access error to dbal actions, if error exists.
 */
...
/** ACTION NAME        | PRINTS                  | VALIDATIONS                                        | LEGAL FLAGS  */
    ...
    {"COMMIT",         PRE_ACCESS_PRINTS_ENABLED, ALL_KEY_FIELDS_MUST_EXIST | RESULT_FIELD_MUST_BE_SET | VALIDATE_ALLOCATOR_FIELD, COMMON_LEGAL_FLAGS | DBAL_COMMIT_OVERRIDE_DEFAULT | DBAL_COMMIT_OVERRUN | DBAL_COMMIT_HW_ONLY},

    /** DBAL_ACTION_ENTRY_COMMIT_UPDATE */
    {"COMMIT UPDATE",  PRE_ACCESS_PRINTS_ENABLED, ALL_KEY_FIELDS_MUST_EXIST | RESULT_FIELD_MUST_BE_SET | VALIDATE_ALLOCATOR_FIELD, COMMON_LEGAL_FLAGS | DBAL_COMMIT_UPDATE | DBAL_COMMIT_OVERRIDE_DEFAULT | DBAL_COMMIT_HW_ONLY},

    /** DBAL_ACTION_ENTRY_COMMIT_FORCE */
    {"COMMIT FORCE",   PRE_ACCESS_PRINTS_ENABLED, ALL_KEY_FIELDS_MUST_EXIST | RESULT_FIELD_MUST_BE_SET | VALIDATE_ALLOCATOR_FIELD, COMMON_LEGAL_FLAGS | DBAL_COMMIT_FORCE | DBAL_COMMIT_OVERRIDE_DEFAULT | DBAL_COMMIT_HW_ONLY},

    /** DBAL_ACTION_ENTRY_COMMIT_RANGE */
    {"COMMIT RANGE",   PRE_ACCESS_PRINTS_ENABLED, ALL_KEY_FIELDS_MUST_EXIST | RESULT_FIELD_MUST_BE_SET | VALIDATE_ALLOCATOR_FIELD, COMMON_LEGAL_FLAGS | DBAL_COMMIT_OVERRUN},

    /** DBAL_ACTION_ENTRY_GET */
    {"GET",            POST_ACCESS_PRINTS_ENABLED, ALL_KEY_FIELDS_MUST_EXIST | RESULT_FIELD_MUST_BE_SET | VALIDATE_ALLOCATOR_FIELD | CORE_ID_MUST_BE_SET_TO_ONE_CORE_ONLY, COMMON_LEGAL_FLAGS | DBAL_COMMIT_VALIDATE_OTHER_CORE | DBAL_COMMIT_HW_ONLY},

    /** DBAL_ACTION_ENTRY_GET_ALL */
    {"GET ALL FIELDS", POST_ACCESS_PRINTS_ENABLED, ALL_KEY_FIELDS_MUST_EXIST| VALIDATE_ALLOCATOR_FIELD | CORE_ID_MUST_BE_SET_TO_ONE_CORE_ONLY, COMMON_LEGAL_FLAGS | DBAL_GET_ALL_FIELDS | DBAL_COMMIT_HW_ONLY},
    ...
    {"ENTRY CLEAR",    PRE_ACCESS_PRINTS_ENABLED, ALL_KEY_FIELDS_MUST_EXIST, COMMON_LEGAL_FLAGS | DBAL_COMMIT_HW_ONLY},
 ...
 * will be dirty (for the dbal user) and if he will re-commit an entry need to merge the prime entry again with
...
/**
 *  \brief
 *  performing clear \ commit to multiple entries that are in a range one by one according to the action.
 *  non-consecutive mode - iterate over all entries in range
 */
shr_error_e
    ...
    dbal_actions_e action)
{
    ...
    /** setting the first entry (low range values already on the handle) */
    ...
    /** retreiving the handle to the original state: setting back the ranges, and default values on the handle incase the
     *  user will want to use another range action on the table.  */
    for (ii = 0; ii < entry_handle->nof_key_fields; ii++)
    {
    ...
    dbal_entry_handle_t * entry_handle)
{
         ...
         dbal_tables_is_merge_entries_supported(unit, entry_handle->table_id)) || entry_handle->er_flags)
    {
        ...
        /*
         * If not previously looked up or if looked up and exist, do get 
         */
        if (rv == _SHR_E_NONE)
        {
            /** need to get the entry if it exists or was not looked-up in the optimize */
        ...
        /** if entry exists, and we are doing error recovery we need to update get_entry_copy for later to add to journal
         *  otherwise it is OK that get_entry_copy equal to NULL. */
        if ((rv != _SHR_E_NOT_FOUND) && entry_handle->er_flags)
        {
            int is_default = 0;

            /** get_entry_handle is the entry that we received from HW to use in the error recovery journal. In case the
        ...
        /** if needed merging the entry with the existing entry in the HW  */
        if (DBAL_HANDLE_IS_MERGE_NEEDED(entry_handle))
        {
    ...
    SHR_IF_ERR_EXIT_WITH_MSG_EXCEPT_IF(dbal_actions_access_entry_commit(unit, entry_handle, get_entry_copy),
                                       _SHR_E_ACCESS);
...
static void
dbal_actions_mdb_action_apply_set(
    int unit,
    dbal_entry_handle_t * entry_handle,
    dbal_entry_handle_t * get_entry_handle)
{
    ...
    {
        /*
         * If refresh is supported, no need to validate entry existence (since refresh forces
         * the insertion anyway), get is only done for ER or merging needs, so doing SW get
         * is enough (in such case, if entry exists only in HW it won't be retrieved).
         * If refresh is not supported, need to validate entry existence before insertion, then
         * we cannot modify the mdb action apply.
         */
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    if (DBAL_TABLE_IS_NONE_DIRECT(entry_handle->table))
    {
        /** in-case that non direct mode is optimized no need to perform get - we trust the input.. */
                ...
                || entry_handle->er_flags))
        {
            ...
            {
                /** glem table is the only table that uses the iterator optimization, this allow fast validation for entry existence */
                int is_exists = FALSE;
                SHR_IF_ERR_EXIT(dbal_actions_optimized_is_exists(unit, entry_handle, &is_exists));
                if (!is_exists)
                {
            ...
            {
                /** Needs to perform get and validate that the entry doesn't exist */
                dbal_actions_mdb_action_apply_set(unit, entry_handle, get_entry_handle);
                rv = dbal_actions_access_entry_get(unit, entry_handle, get_entry_handle);
                SHR_IF_ERR_EXIT_EXCEPT_IF(rv, _SHR_E_NOT_FOUND);
            }

            /** In case of non direct table - verify entry does not exist yet.
             * If exists need to use commit update/force. */
            if (rv == _SHR_E_NONE)
            {
                /** Entry exists */
            ...
            {
                /**
                 *  In case of non-direct and the entry was already merged before,
                 *  merging the entry with the default entry, to clear
                 *  unneeded values from previous merging on the handle.
                 *  (In case the entry was merged before with different values)
                 */
                SHR_IF_ERR_EXIT(dbal_merge_entries(unit, entry_handle, get_entry_handle));
            }
        }
    ...
    {
        SHR_IF_ERR_EXIT_WITH_MSG_EXCEPT_IF(dbal_entry_commit_direct(unit, entry_handle), _SHR_E_ACCESS);
    }
    ...
    dbal_entry_handle_t * entry_handle)
{
          ...
          !(DBAL_HANDLE_IS_MERGE_NEEDED(entry_handle))) || entry_handle->er_flags)
    {
        ...
        if (entry_handle->phy_entry.mdb_action_apply != DBAL_MDB_ACTION_APPLY_HW_ONLY)
        {
            dbal_actions_mdb_action_apply_set(unit, entry_handle, get_entry_handle);
            rv = dbal_actions_access_entry_get(unit, entry_handle, get_entry_handle);
        }
        else    /* entry_handle->phy_entry.mdb_action_apply = DBAL_MDB_ACTION_APPLY_HW_ONLY */
        {
            /*
             * Update HW only => Do a SW get, and delete value if found
             */
            get_entry_handle->phy_entry.mdb_action_apply = DBAL_MDB_ACTION_APPLY_SW_SHADOW;
            rv = dbal_actions_access_entry_get(unit, entry_handle, get_entry_handle);
            if (rv == _SHR_E_NONE)
            {
                 ...
                 * Clear from SW 
                 */
                SHR_IF_ERR_EXIT(dbal_actions_access_entry_clear(unit, get_entry_handle));
                get_entry_handle->phy_entry.mdb_action_apply = DBAL_MDB_ACTION_APPLY_ALL;
            }
            else        /* if it was found in SW no need to look for it in HW, it needs to be there */
            {
                /*
                 * Do a HW get and make sure entry exists
                 */
                get_entry_handle->phy_entry.mdb_action_apply = DBAL_MDB_ACTION_APPLY_HW_ONLY;
                rv = dbal_actions_access_entry_get(unit, entry_handle, get_entry_handle);
            }
        }
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    /*
     * For non-direct optimized tables (KBP or KAPS LPM tables).
     * If all result fields were set it is possible to just commit the entry.
     ...
     * It can only be done when error recovery is disabled, since for error recovery it is needed to get the existing
     * entry for rollback.
     */
    if (SHR_IS_BITSET(entry_handle->table->indications_bm, DBAL_TABLE_IND_IS_NONE_DIRECT_OPTIMIZED) &&
        !(DBAL_HANDLE_IS_MERGE_NEEDED(entry_handle)) && !(entry_handle->er_flags))
    ...
    else if (entry_handle->phy_entry.mdb_action_apply == DBAL_MDB_ACTION_APPLY_HW_ONLY)
    {
        SHR_ALLOC_SET_ZERO(get_entry_handle, sizeof(dbal_entry_handle_t), "Handle Alloc", "%s%s%s\r\n",
                           EMPTY, EMPTY, EMPTY);

        /*
         * If HW only, look for the entry in shadow, and deletes if it exists 
         */
        get_entry_handle->phy_entry.mdb_action_apply = DBAL_MDB_ACTION_APPLY_SW_SHADOW;
        rv = dbal_actions_access_entry_get(unit, entry_handle, get_entry_handle);
        if (rv == _SHR_E_NONE)
        ...
        /*
         * Indicates to MDB layer, the entry needs to be committed anyway 
         */
        entry_handle->phy_entry.indirect_commit_mode = DBAL_INDIRECT_COMMIT_MODE_FORCE;
    }
    else
    {
        ...
        else if (rv == _SHR_E_NOT_FOUND)
        {
            ...
            {
                /**
                 *  In case of non-direct merging the entry with the default entry, to clear unneeded info from previous merging on the
                 *  entry handle (in case the entry was merged before with different values)
                 */
                SHR_IF_ERR_EXIT(dbal_merge_entries(unit, entry_handle, get_entry_handle));
            }
    ...
    dbal_entry_action_flags_e flags)
{
    ...
    if (flags & DBAL_COMMIT_HW_ONLY)
    {
        if (!SHR_IS_BITSET(entry_handle->table->indications_bm, DBAL_TABLE_IND_IS_LEARNING_EN))
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL,
                         "Flags DBAL_COMMIT_HW_ONLY was used for table %s, "
                         "but is for tables supporting learning only\n", table->table_name);
        }
    }
    ...
    if (entry_handle->error_info.error_exists)
    {
        ...
        if (SHR_IS_BITSET(entry_handle->table->indications_bm, DBAL_TABLE_IND_HAS_RESULT_TYPE))
        {
            SHR_ERR_EXIT(entry_handle->error_info.error_exists,
                         DBAL_FIELD_SET_ERROR_MSG " %s table %s res type %s\n",
                         dbal_field_with_inst_to_string(unit, entry_handle->error_info.field_id,
                                                        entry_handle->error_info.inst_idx, field_string),
                         table->table_name, table->multi_res_info[res_type].result_type_name);
        }
        else
        {
            SHR_ERR_EXIT(entry_handle->error_info.error_exists,
                         DBAL_FIELD_SET_ERROR_MSG " %s table %s\n",
                         dbal_field_with_inst_to_string(unit, entry_handle->error_info.field_id,
                                                        entry_handle->error_info.inst_idx, field_string),
                         table->table_name);
        }
    ...
    if (!(actions_info->action_validation_flags & CORE_COULD_BE_NOT_INITIALIZED))
    {
        ...
        if ((table->core_mode == DBAL_CORE_MODE_DPC) && (entry_handle->core_id == DBAL_CORE_NOT_INITIALIZED))
        {
        ...
        (actions_info->action_validation_flags & CORE_ID_MUST_BE_SET_TO_ONE_CORE_ONLY))
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "table %s is DPC, but core in handle is not initialized, core_id = %d\n",
                         table->table_name, entry_handle->core_id);
        }
    }
    ...
    if (actions_info->action_validation_flags & ALL_KEY_FIELDS_MUST_EXIST)
    {
        ...
        if (!DBAL_TABLE_IS_TCAM(table) && (table->table_type != DBAL_TABLE_TYPE_LPM))
        {
            ...
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL,
                             "illegal number of key fields for action %s in table %s, expected %d set %d\n",
...
/** validations of the allocator is done only in the internal code */
    ...
    if (actions_info->action_validation_flags & RESULT_FIELD_MUST_BE_SET)
    {
        if ((entry_handle->nof_result_fields == 0) && !(entry_handle->phy_entry.attr_flags & DBAL_ATTR_FLAG_NO_PAYLOAD))
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL,
                         "No value fields where set (or requested for get operation), cannot perform the action %s on table %s\n",
                         actions_info->name, table->table_name);
        }
    }

    /** setting the core_id to max core */
        ...
        if (dbal_logger_is_enable(unit, entry_handle->table_id))
        {
                ...
                (logger_info->logger_mode == DBAL_LOGGER_MODE_WRITE_ONLY && action >= DBAL_ACTION_ENTRY_CLEAR))
            {
                ...
                if (log_enabled || (logger_info->dbal_file != NULL))
                {
                    ...
                    if (DBAL_TABLE_IS_TCAM(entry_handle->table) && !(DBAL_TABLE_IS_TCAM_BY_KEY(entry_handle->table)))
                    {
    ...
    dbal_actions_e * action)
{
    ...
    if ((*entry_handle)->nof_ranged_fields > 0)
    {
        if (!(((table->access_method == DBAL_ACCESS_METHOD_KBP)
              ) && (table->table_type == DBAL_TABLE_TYPE_TCAM_BY_ID)))
        {
    ...
    dbal_actions_e * action)
{
    ...
    /** for not STD1 there could be cases that no result fields were set */
    ...
    /** key not mapped after init dumping error to console during init ignoring */
    ...
    dbal_actions_e * action)
{
    ...
    /*
     * Action resolution part - according to the general action and flags, we set the final dbal action 
     */
    ...
    /*
     * MDB action is ALL, except if user wants HW only operation
     * In case of pure get operation: for non learning tables it is made SW in dbal MDB layer
     * For commit/update/clear operations, it is Hw if user defined it so, else ALL (those operations cannot be SW only)
     * For non pure get operations (get from other operations), default is ALL, changed by DBAL MDB to SW for non learning tables
     * or HW/SW if explicitly set
     */
    if (!(flags & DBAL_COMMIT_HW_ONLY))
    {
...
/*
 * get next table entry key using bitmap optimization for better iterator performance.
 */
static shr_error_e
    ...
    uint8 *entry_found)
{
    ...
    *entry_found = FALSE;
    ...
    while (!*entry_found && !iterator_info->is_end)
    {
        ...
        if (optimized_iterator_bitmap_val != 0)
        {
            ...
            for (bit_index = bit_offset; bit_index < 32; bit_index++)
            {
                ...
                {
                    *entry_found = TRUE;
                    /**Check if there's a rule that affects this entry*/
                ...
                {
                    /*
                     * No more valid bits in this bitmap move to the next bitmap word
                     */
            ...
            {
                /*
                 * we didn't find an entry in current bitmap but we are pointing to the next key, dec the key value by one.
                 */
                entry_handle->phy_entry.key[0]--;
            }
        }
    ...
    if (*entry_found)
    {
        /**Lifs result type must be set for entry get*/
        /**Handle outlifs*/
        if ((dbal_table_is_out_lif(entry_handle->table)))
        {
            rv = dnx_lif_mngr_outlif_sw_info_get
                (unit, *entry_handle->phy_entry.key, NULL, (uint32 *) &entry_handle->cur_res_type,
                 NULL, NULL, NULL, NULL);
            if (rv != _SHR_E_NONE)
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "Table %s key %d is not allocated in resource",
                             dbal_logical_table_to_string(unit, entry_handle->table_id),
                             entry_handle->phy_entry.key[0]);
            }
        }
        ...
        {
            int core_id = entry_handle->core_id;
            rv = dnx_lif_mngr_inlif_sw_info_table_id_and_result_type_get
                (unit, *entry_handle->phy_entry.key, core_id,
                 entry_handle->table->physical_db_id[0], NULL, (uint32 *) &entry_handle->cur_res_type);
            if (rv != _SHR_E_NONE)
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "Table %s key %d core %d is not allocated in resource",
                             dbal_logical_table_to_string(unit, entry_handle->table_id), entry_handle->phy_entry.key[0],
                             core_id);
            }
        }
    }
...
/*
 * Clear set bit map entry, used for optimized iterator implementation,
 * the function support both single entry and range of entries
 */
static void
    ...
    int *is_entry_exists)
{
    ...
    if (!SHR_IS_BITSET(entry_handle->table->indications_bm, DBAL_TABLE_IND_IS_ITERATOR_OPTIMIZED))
    {
        ...
        SHR_EXIT();
        /** supported only for table that has DBAL_TABLE_IND_IS_ITERATOR_OPTIMIZED */
    }
    ...
    if (!(bitmap_val & ((uint32) 0x1 << bit_offset)))
    {
    ...
    int num_of_entries)
{
    ...
    /** support consecutive mode for MDB/SW/PEMLA  */
    switch (table->access_method)
    {
        case DBAL_ACCESS_METHOD_MDB:
            /** SHR_IF_ERR_EXIT(dbal_mdb_table_range_entry_add(unit, entry_handle, num_of_entries));*/
...
/*
 * The function assumes the table has a result type and that the RT is not
 * already initialized
 */
shr_error_e
    ...
    dbal_entry_handle_t * get_handle)
{
    ...
    switch (table->access_method)
    {
        ...
        case DBAL_ACCESS_METHOD_HARD_LOGIC:
            SHR_IF_ERR_EXIT_WITH_MSG_EXCEPT_IF(dbal_hl_entry_set(unit, entry_handle), _SHR_E_ACCESS);
            break;
    ...
    dbal_entry_handle_t * get_entry_handle)
{
    ...
    dbal_logical_table_t *table = entry_handle->table;
    dbal_mdb_action_apply_type_e action_bkp;

    SHR_FUNC_INIT_VARS(unit);

    /**  when get_entry_handle != NULL means that the get operation called from DBAL and not directly from the user
     *   for example for error recovery or validations */
    if (get_entry_handle != NULL)
    {
        int size_to_copy = 0;
        action_bkp = get_entry_handle->phy_entry.mdb_action_apply;
        /**size_to_copy = all the handle but payload and p_mask buffers */
        size_to_copy = (sizeof(dbal_entry_handle_t) - (2 * DBAL_PHYSICAL_RES_SIZE_IN_BYTES));

        is_get_on_new_entry = 1;

        /** Create a copy of the handle, copying all the information but the payload. */
        sal_memcpy(get_entry_handle, entry_handle, size_to_copy);
        /*
         * Restore MDB action 
         */
        get_entry_handle->phy_entry.mdb_action_apply = action_bkp;

        /** making sure that the payload is clean. */
        ...
        {
            /** in case we are getting an entry for most of the cases we need to resolve the result type the usual way. because the
             *  result type not must to be equal to the added entry result type. for LIF MDB tables we don't allow to change the
             *  result type, so it means that the result is equal to the added entry or the entry doesn't exists. it is only
    ...
    if (!SHR_FUNC_ERR())
    {
        ...
        if (!SHR_FUNC_ERR())
        {
            /** when doing get on a new handle updating the value field IDs on the handle. */
    ...
    dbal_entry_handle_t * entry_handle)
{
        ...
        || ((table->nof_result_types > 1) && (entry_handle->cur_res_type == DBAL_RESULT_TYPE_NOT_INITIALIZED)))
    {
        ...
        /*
         * For non HW operations, just check the values exists in shadow, delete if exists,
        ...
        if (!(entry_handle->phy_entry.mdb_action_apply == DBAL_MDB_ACTION_APPLY_HW_ONLY))
        {
            ...
            else
            {
                ...
                /** get_entry_handle is the entry that we received from HW in case that the entry is default no need to delete the entry */
                SHR_IF_ERR_EXIT(dbal_entry_direct_is_default(unit, get_entry_handle, &is_default));
                if (is_default)
                {
                    SHR_EXIT();
        ...
        /*
         * For HW_ONLY operation:
         * if entry !found in shadow, delete
         * else if value is found in shadow, returns error because it tried to clear from HW an entry present in SW too.
         * */
        else    /* if hw_only */
        {
            /*
            ...
            rv = dbal_actions_access_entry_get(unit, entry_handle, get_entry_handle);
            /*
             * Restore get handle mdb action value
             */
            get_entry_handle->phy_entry.mdb_action_apply = DBAL_MDB_ACTION_APPLY_ALL;
            if (rv == _SHR_E_NOT_FOUND)
            {
                get_entry_handle->phy_entry.mdb_action_apply = DBAL_MDB_ACTION_APPLY_HW_ONLY;
                /*
                 * Entry was not found in SW, so we check in HW
                 */
                SHR_IF_ERR_EXIT(dbal_actions_access_entry_get(unit, entry_handle, get_entry_handle));
            }
            else
            {
                SHR_ERR_EXIT(_SHR_E_EXISTS, "Entry exists in Shadow, cannot remove from HW only");
            }
        }
    ...
    if (DBAL_TABLE_IS_NONE_DIRECT(entry_handle->table))
    {
        /** limit per LIF counters. Payload should be added to the delete command of MACT entries */
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    if (SHR_IS_BITSET(entry_handle->table->indications_bm, DBAL_TABLE_IND_IS_ITERATOR_OPTIMIZED))
    {
        ...
        {
            /** currently for EM tables only global rif is supported for iterator optimized */
            if (entry_handle->table_id == DBAL_TABLE_GLOBAL_RIF_EM)
            {
...
/** if rules or actions exists on the entry need to update the entry handle from the iterator info */
static shr_error_e
    ...
    dbal_iterator_info_t * iterator_info)
{
    ...
    {
        /*
         * FWD_MACT and FWD_MACT_IVL are running through flush machine, so setting the get hit rule only when flush
         * machine is ended
         */
    ...
    /*
     * in case age_rule is valid, always set the attribute request, as those entries are blocked in flush machine
     * All entries are read from SE iterator
     */
    ...
    /** in case hit_bit_action get is valid, set the action on the handle */
    ...
    /** in case hit_bit_action clear is valid, set the action on the handle */
    ...
    uint8 *continue_iterating)
{
    ...
    if (SHR_IS_BITSET(table->indications_bm, DBAL_TABLE_IND_IS_ITERATOR_OPTIMIZED))
    {
        ...
        if (entry_found)
        {
            SHR_IF_ERR_EXIT(dbal_actions_access_entry_get(unit, entry_handle, NULL));
        }
    }
    else
    {
    ...
    if (!iterator_info->is_end)
    {
        ...
        /*
         * mark handle's state as DBAL_HANDLE_STATUS_ACTION_PREFORMED 
         */
        ...
        if ((((iterator_info->nof_val_rules > 0) &&
             ...
             /*
              * If entry was received from flush, we know value rules are filled, no need to check again
              * Relevant only for MDB (LEM) tables, which use flush machine
              */
             ((table->access_method != DBAL_ACCESS_METHOD_MDB) ||
              !iterator_info->mdb_iterator.iterate_in_flush_machine ||
              iterator_info->mdb_iterator.start_non_flush_iteration)))
        {
        ...
        if (is_value_rules_pass && iterator_attrib_info->age_rule_valid)
        {
            ...
            {
                is_value_rules_pass = 0;
            }
        }

        if (is_value_rules_pass && iterator_attrib_info->hit_bit_rule_valid)
        {
            uint32 attr_val;

            /*
             * Get value for required hit bit (rule type) 
             */
            SHR_IF_ERR_EXIT(dbal_entry_attribute_get(unit, entry_handle->handle_id,
                                                     iterator_attrib_info->hit_bit_rule_type, &attr_val));

            /*
             * If hit_bit_rule_is_hit == 0, it means no hit is expected
             */
            if ((iterator_attrib_info->hit_bit_rule_is_hit == 0)
                &&
                (((iterator_attrib_info->hit_bit_rule_type & DBAL_ENTRY_ATTR_HIT_PRIMARY) &&
                  (((attr_val & DBAL_PHYSICAL_KEY_PRIMARY_HITBIT_ACCESSED_A) != 0) ||
                   ((attr_val & DBAL_PHYSICAL_KEY_PRIMARY_HITBIT_ACCESSED_B) != 0)))
                 ||
                 ((iterator_attrib_info->hit_bit_rule_type & DBAL_ENTRY_ATTR_HIT_SECONDARY) &&
                  (((attr_val & DBAL_PHYSICAL_KEY_SECONDARY_HITBIT_ACCESSED_A) != 0) ||
                   ((attr_val & DBAL_PHYSICAL_KEY_SECONDARY_HITBIT_ACCESSED_B) != 0)))))
            {
                is_value_rules_pass = 0;
            }
            /*
             * If hit_bit_rule_is_hit == 1, we check if hit occurred as expected, according to
             * the hitbit flags defined in the rule
             */
            if (is_value_rules_pass &&
                ((iterator_attrib_info->hit_bit_rule_is_hit != 0)
                 &&
                 !(((iterator_attrib_info->hit_bit_rule_type & DBAL_ENTRY_ATTR_HIT_PRIMARY) &&
                    ((attr_val & DBAL_PHYSICAL_KEY_PRIMARY_HITBIT_ACCESSED_A) ||
                     (attr_val & DBAL_PHYSICAL_KEY_PRIMARY_HITBIT_ACCESSED_B)))
                   ||
                   ((iterator_attrib_info->hit_bit_rule_type & DBAL_ENTRY_ATTR_HIT_SECONDARY) &&
                    ((attr_val & DBAL_PHYSICAL_KEY_SECONDARY_HITBIT_ACCESSED_A) ||
                     (attr_val & DBAL_PHYSICAL_KEY_SECONDARY_HITBIT_ACCESSED_B))))))
            {
                is_value_rules_pass = 0;
            }
        }
