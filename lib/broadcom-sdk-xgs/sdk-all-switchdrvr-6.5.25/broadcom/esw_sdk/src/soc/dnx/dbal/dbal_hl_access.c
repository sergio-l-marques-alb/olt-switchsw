 ...
 *  The functions receives field id encoded with its instance index.
 */
shr_error_e
    ...
    int *num_of_blocks)
{
    ...
    {/** in this case a specific block should be set */
        SHR_IF_ERR_EXIT(dbal_hl_offset_calculate(unit, entry_handle, curr_l2p_info, 0, block_index_info, block));
        (*num_of_blocks) = 1;
    }
    else
    {
    ...
    int logger_enable)
{
    ...
    if (offset_in_mem >= 0)
    {
        /**Suppose the field data is always 0 if it's in the outside of the memory entry.*/
            ...
            if (is_non_standard)
            {
                ...
                SHR_IF_ERR_EXIT(dbal_hl_reg_block_calc
                                (unit, entry_handle, entry_handle->core_id, reg, &(curr_l2p_info->block_index_info),
                                 curr_l2p_info, &block, &nof_blocks));
...
/**
 *  \brief
 *  Determines if the entry is default for direct table
 */
static shr_error_e
...
/**
 * \brief
 * For table with unmapped field, check if the current key combination fills
 * one of the unmapped cases, and return ACCESS error is it does.
 */
shr_error_e
    ...
    int res_type_get)
{
    ...
    /** updating the entry result type valid in case of result_type_get != DBAL_INVALID_PARAM */
    ...
    for (iter = 0; iter < l2p_hl_info->num_of_access_fields; ++iter)
    {
        ...
        if (is_key_comb_unmapped)
        {
            ...
            entry_handle->error_info.field_id = l2p_hl_info->l2p_fields_info[iter].mapping_condition->field_id;
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    if (SHR_IS_BITSET(table->indications_bm, DBAL_TABLE_IND_IS_HOOK_ACTIVE))
    {
        ...
        {
            /**
             * If the hook returns false, exit.
             * If the hook returns true, nothing was done.
             * Continue as if there is not hook.
             */
            SHR_EXIT();
        }
    }
 ...
 *  only requested fields are received from HW (in case of get all fields is set all fields are requested).
    ...
    int num_of_entries)
{
    ...
    for (ii = 0; ii < nof_group_elements; ii++)
    {
        ...
        SHR_IF_ERR_EXIT(dbal_hl_mem_block_calc(unit, entry_handle, core_id, memory,
                                               &(l2p_info->block_index_info), l2p_info, &block, &num_of_blocks));
    ...
    int num_of_entries)
{
    ...
    {
        /** If DPC table, need to perform action per core */
        core_id = 0;
        start_iteration = 0;
    ...
    for (core_iteration = start_iteration; core_iteration < end_iteration; core_iteration++)
    {
        ...
        for (iter = 0; iter < l2p_hl_info->num_of_access_fields; iter++)
        {
            ...
            if (is_condition_pass)
            {
                /** indicate that access occurred, remove the error indication */
                    ...
                    (last_mem_offset_used != mem_entry_offset) || (last_mem_array_offset_used != mem_array_offset))
                {
                    ...
                    if (last_memory_used != INVALIDm)
                    {
                        ...
                        {
                            /*
                             * if alias offset exists we need to copy the data to the original data (data_for_alias)
                    ...
                    if (curr_l2p_info->alias_memory != INVALIDm)
                    {
                        ...
                        if (alias_offset != 0)
                        {
                            ...
                            /** copying the relevant part of the memory (from alias_offset) to beginning of data */
                            SHR_IF_ERR_EXIT(utilex_bitstream_get_any_field(data_for_alias, alias_offset,
                                                                           curr_l2p_info->
                                                                           alias_data_offset_info.internal_inparam,
                                                                           data));
        ...
        for (ii = 1; ii < DBAL_MAX_NUMBER_OF_HW_ELEMENTS; ii++)
        {
            ...
            if (l2p_info->reg[ii] != INVALIDr)
            {
                SHR_IF_ERR_EXIT(dbal_hl_reg_block_calc(unit, entry_handle, entry_handle->core_id, reg,
                                                       &(l2p_info->block_index_info), l2p_info, &block, &nof_blocks));
                for (block_indx = 0; block_indx < nof_blocks; block_indx++)
                {
                    if ((l2p_info->array_offset_info.formula == NULL) && SOC_REG_IS_ARRAY(unit, reg))
                    {
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    else if (table->table_type == DBAL_TABLE_TYPE_DIRECT)
    {
        ...
        if (SHR_IS_BITSET(table->indications_bm, DBAL_TABLE_IND_IS_HOOK_ACTIVE))
        {
            ...
            if (!continue_regular_process)
            {
                /** If hook does something, exit.  Otherwise, continue */
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    else if (table->table_type == DBAL_TABLE_TYPE_DIRECT)
    {
        ...
        /*
         * Go reverse to first check there is no unmapped field 
         */
        for (access_type = 0; access_type < DBAL_NOF_HL_ACCESS_TYPES; ++access_type)
        {
            if (table->hl_mapping_multi_res[entry_handle->cur_res_type].l2p_hl_info[access_type].num_of_access_fields >
 ...
 *  fields with the default  values of the entry. eventually
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    else if (table->table_type == DBAL_TABLE_TYPE_DIRECT)
    {
        ...
        {
            /*
             * Reset the error which could occur in a previous
             * invalid entry (can happen when iterating over
            ...
            if (is_valid_entry == DBAL_KEY_IS_VALID)
            {
                ...
                {
                    /*
                     * Unmapped combination, just skip it 
                     */
                    continue;
                }
