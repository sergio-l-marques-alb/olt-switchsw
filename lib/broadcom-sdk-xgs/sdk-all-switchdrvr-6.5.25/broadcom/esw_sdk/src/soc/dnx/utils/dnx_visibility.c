...
/*
 * This table holds memory register name per ASIC block, the options are:
 * mem_array[unit][block_id][0] 'internal' name, example: IPPA_MEM_F000000m used in some devices
 * mem_array[unit][block_id][1] informative name, example: IPPA_IPPA_DEBUG_UNITm used in others
 * Device hilds array at dnx_data_debug.mem.mem_array and the index held in dnx_data_debug.mem.mem_array_index.
 */
    ...
    int unit)
{
    ...
    /*
     * Init dnx_visibility_block_map table from DNX DATA
     */
    ...
    /*
     * Init mem_array table from DNX DATA
     */
    ...
    int unit)
{
    ...
    /*
     * Free dnx_visibility_block_map table
     */
 ...
 * \param [in] asic_block - ASIC Block ID
 * \param [in] line_index - Memory line inside ASIC block
 * \param [out] reset_val - The reset value
 *
 * \return
 *      error - per standard shr_error_e
 */
...
#ifdef ADAPTER_SERVER_MODE
    /*
     * With adapter, the visibility is global.
     * it is enough to write the reset to one of the blocks only.
     * To make sure that the action is finished before sending a packetm read it afterwards.
     */
...
/**
 * \brief - Enable port visibility per pp dsp - Egress
 *
 * \param [in] unit - Unit id
 * \param [in] core - Core id
 * \param [in] pp_dsp - pp dsp id 8b
...
/**
 * \brief - Verify if the port is trunk member and return an error if it is
 *
 * \param [in] unit - Relevant unit
 * \param [in] gport - Logical port, trunk gport or trunk member
 *
 * \return
 *   shr_error_e
 *
    ...
    int unit)
{
        ...
        dnx_data_debug.general.has_etpp_visibility_enabler_get(unit))
    {
        ...
        if (dnx_data_trap.egress.feature_get(unit, dnx_data_trap_egress_egress_divided_to_erpp_and_etpp))
        {
            /** Disable visibility and unforce for all erpp ports */
        ...
        /** Disable visibility and unforce for all etpp ports */
