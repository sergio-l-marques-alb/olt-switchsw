 ...
 * \param [in] core_id    - The core ID to be checked (Must be specified)
    ...
    dnx_field_tcam_access_fg_params_t * fg_params)
{
    ...
    if (!dnx_data_field.tcam.feature_get(unit, dnx_data_field_tcam_mixed_ratio_supported))
    {
        ...
        /*
         * Check if bank is allocated for a different key size
         */
 ...
 * \param[in] core_ids           - Array of core IDs per bank ID
 *
 * \return
 *  \retval _SHR_E_NONE  - Success
 *  \retval _SHR_E_PARAM - 1) Given bank_id exceeds limit
    ...
    bcm_core_t core_id)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_tcam_access_sw.fg_params.get(unit, handler_id, &fg_params));
    /*
     * Add the handler id to the active_handlers_id linked list of this bank and core
     */
...
/**
 * \brief
 *  Allocates the given bank to the given handler_id
 *
 * \param [in] unit       - Device ID
 * \param [in] handler_id - Handler ID to allocate the bank for
 * \param [in] bank_id    - The bank ID to be checked
 * \param [in] core_id    - The core ID to be checked (Must be specified)
 *
 * \return
    ...
    uint32 handler_id)
{
    ...
    for (bank_id = 0; bank_id < dnx_data_field.tcam.nof_banks_get(unit); bank_id++)
    {
        DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core_iter)
        {
            ...
            {
                /*
                 * If handler was found, free prefix if relevant
                 */
                if (!fg_params.direct_table &&
