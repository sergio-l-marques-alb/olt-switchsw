...
#if defined(BCM_DNX2_SUPPORT)
#include <src/soc/dnx/mdb/auto_generated/standard_1_dbx_pre_compiled_vmv_mdb_xml_parser.h>
#endif
_INTERNAL_AUTO_GENERATED_INCLUDES
...
void*
dbx_pre_compiled_mdb_vmv_top_get(
    int unit,
    char *filepath,
    char *topname,
    int flags)
{
    ...
    for(i = 0; i < 4; i++)
    {
        ...
        if(is_image)
        {
            curTop = dbx_xml_top_get_internal(_filepath, topname, flags, pre_compiled_mdb_vmv_standard_1_xml_to_buf);
            if(curTop)
            {
                return curTop;
            }
