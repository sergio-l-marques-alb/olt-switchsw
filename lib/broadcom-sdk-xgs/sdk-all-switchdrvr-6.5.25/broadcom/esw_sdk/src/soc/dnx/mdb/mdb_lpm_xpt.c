...
#define MDB_LPM_MAX_NUM_DB_PER_CORE 4



shr_error_e
mdb_lpm_xpt_big_kaps_enabled(
    int unit,
    uint8 db_id,
    int *big_kaps_enabled)
{
    ...
    int big_kaps_enabled_temp = FALSE;

    SHR_FUNC_INIT_VARS(unit);

    mdb_table_id = dnx_data_mdb.kaps.db_info_get(unit, db_id)->mdb_bb_id;

    
    if (mdb_table_id != MDB_NOF_PHYSICAL_TABLES)
    {
        SHR_IF_ERR_EXIT(mdb_db_infos.db.number_of_clusters.get(unit, mdb_table_id, &nof_clusters));

        if (nof_clusters > 0)
        {
            big_kaps_enabled_temp = TRUE;
        }
    }

    *big_kaps_enabled = big_kaps_enabled_temp;
    ...
    uint8_t * bytes)
{
    ...
    if ((offset == 0) && (nbytes == 4) && (bytes != NULL))
    {
        ...
        int big_kaps_enabled_arr[DNX_DATA_MAX_MDB_KAPS_NOF_DBS];
        int db_id_iter;

        

        for (db_id_iter = 0; db_id_iter < dnx_data_mdb.kaps.nof_dbs_get(unit); db_id_iter++)
        {
            rv = mdb_lpm_xpt_big_kaps_enabled(unit, db_id_iter, &big_kaps_enabled_arr[db_id_iter]);
            if (rv != _SHR_E_NONE)
            {
                LOG_ERROR(BSL_LOG_MODULE,
    ...
    struct kaps_search_result * kaps_result)
{
    ...
    core = 0;
    mdb_db_idx = search_interface;

    sal_memset(kaps_result, 0, sizeof(*kaps_result));

    sal_memset(&entry, 0, sizeof(entry));

    mdb_lpm_kaps_buffer_kaps_to_dbal(dnx_data_mdb.kaps.key_width_in_bits_get(unit), key, entry.key);

    SHR_IF_ERR_EXIT(mdb_lpm_xpt_entry_search
                    (unit, core, dnx_data_mdb.kaps.db_info_get(unit, mdb_db_idx)->dbal_id, 0x0, &entry));

    mdb_lpm_kaps_buffer_dbal_to_kaps(entry.payload_size, entry.payload_size, entry.payload, kaps_result->ad_value);

    kaps_result->match_len = entry.key_size;
    kaps_result->ad_width_1 = entry.payload_size;
...
STATIC kaps_status
mdb_lpm_xpt_translate_blk_func_offset_to_mem_reg(
    int unit,
    uint8 blk_id,
    uint32 func,
    uint32 offset,
...
shr_error_e
mdb_lpm_xpt_is_cluster_allocated(
    int unit,
...
int
mdb_lpm_xpt_physical_table_is_bb(
    int unit,
    mdb_physical_table_e mdb_physical_table_id)
{
    
    if ((mdb_physical_table_id < MDB_NOF_PHYSICAL_TABLES) &&
        (dnx_data_mdb.pdbs.mdb_pdb_info_get(unit, mdb_physical_table_id)->db_type == DBAL_ENUM_FVAL_MDB_DB_TYPE_KAPS))
    {
        {
            return TRUE;
        }
...
kaps_status
mdb_lpm_xpt_map_bb_big_kaps_blk_id(
    int unit,
    uint8 blk_id,
    soc_mem_t * mem,
    int *blk,
    ...
    int *macro_index)
{
    ...
    global_macro_index =
        dnx_data_mdb.kaps.big_bb_blk_id_mapping_get(unit, 0,
                                                    blk_id - MDB_LPM_SECOND_BB_FIRST_BLK_ID)->global_macro_index;
    ...
    soc_mem_t * mem_hitbit_b)
{
    ...
    rv = mdb_lpm_xpt_map_bb_big_kaps_blk_id(unit, blk_id, mem, blk, &macro_type, &macro_index);
    if (rv != KAPS_OK)
    {
        LOG_ERROR(BSL_LOG_MODULE,
    ...
    if (rv == KAPS_OK)
    {
        ...
        mdb_physical_table_e assoc_mdb_physical_table_id_low =
            MDB_NOF_PHYSICAL_TABLES, assoc_mdb_physical_table_id_high = MDB_NOF_PHYSICAL_TABLES;
        ...
        kaps_is_alloc_low = mdb_lpm_xpt_physical_table_is_bb(unit, assoc_mdb_physical_table_id_low);
        kaps_is_alloc_high = mdb_lpm_xpt_physical_table_is_bb(unit, assoc_mdb_physical_table_id_high);
...
kaps_status
mdb_lpm_xpt_write_command_memory(
    int unit,
    soc_mem_t mem,
    int array_index,
    int blk,
...
kaps_status
mdb_lpm_xpt_write_command_big_kaps(
    int unit,
    uint8 blk_id,
    uint32 cmd,
    uint32 func,
    ...
    uint8 *bytes)
{
    ...
    else if ((blk_id >= MDB_LPM_SECOND_BB_FIRST_BLK_ID) && (blk_id <= MDB_LPM_SECOND_BB_LAST_BLK_ID))
    {
        ...
        rv = mdb_lpm_xpt_map_bb_big_kaps(unit, blk_id, func, offset, &mem, &bucket_index, &blk, &row_in_cluster, NULL,
                                         NULL);
        if (rv != _SHR_E_NONE)
        {
        ...
        rv = mdb_lpm_xpt_write_command_memory(unit, mem, bucket_index, blk, row_in_cluster, row_data);
...
kaps_status
mdb_lpm_xpt_write_command_small_kaps(
    int unit,
    uint8 blk_id,
    uint32 cmd,
    uint32 func,
    ...
    uint8 *bytes)
{
    ...
    rv = mdb_lpm_xpt_translate_blk_func_offset_to_mem_reg(unit, blk_id, func, offset, &mem, &reg, &array_index, &blk);
    if (rv != KAPS_OK)
    {
        LOG_ERROR(BSL_LOG_MODULE,
    ...
    if (mem != INVALIDm)
    {
        ...
        DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core)
        {
            if (mem == KAPS_RPB_TCAM_CPU_COMMANDm)
            {
                ...
                int bubble_not_synced =
                    dnx_data_mdb.feature.feature_get(unit, dnx_data_mdb_feature_kaps_rpb_bubble_not_synced);
                uint32 valid_bits;
                ...
                {
                    int tcam_inst_iter;
                    int tcam_inst_iter_max;

                    
                    if (bubble_not_synced == TRUE)
                    {
                        tcam_inst_iter_max = 2;
                    }
                    else
                    {
                        tcam_inst_iter_max = 1;
                    }

                    for (tcam_inst_iter = 0; tcam_inst_iter < tcam_inst_iter_max; tcam_inst_iter++)
                    {
                        ...
                        rv = mdb_lpm_xpt_write_command_memory(unit, mem, array_index, KAPS_BLOCK(unit, core),
                        ...
                        if ((ecc_valid_shared == TRUE) && (valid_bits != 0) && (row_iter == 0))
                        {
                            ...
                            rv = mdb_lpm_xpt_write_command_memory(unit, mem, array_index, KAPS_BLOCK(unit, core),
                                                                  offset + 1,
                                                                  &(mem_array[MDB_LPM_RPB_MAX_UINT32_WIDTH]));
            ...
            {
                rv = mdb_lpm_xpt_write_command_memory(unit, mem, array_index, KAPS_BLOCK(unit, core), offset,
                                                      mem_array);
    ...
    uint8 *bytes)
{
    ...
    if ((blk_id >= MDB_LPM_RPB_FIRST_BLK_ID) && (blk_id <= MDB_LPM_BB_LAST_BLK_ID))
    {
        ...
        rv = mdb_lpm_xpt_write_command_small_kaps(unit, blk_id, cmd, func, offset, nbytes, bytes);
        if (rv != _SHR_E_NONE)
        {
            LOG_ERROR(BSL_LOG_MODULE,
    ...
    else if ((blk_id >= MDB_LPM_SECOND_ADS_FIRST_BLK_ID) && (blk_id <= MDB_LPM_SECOND_BB_LAST_BLK_ID))
    {
        ...
        rv = mdb_lpm_xpt_write_command_big_kaps(unit, blk_id, cmd, func, offset, nbytes, bytes);
        if (rv != _SHR_E_NONE)
        {
            LOG_ERROR(BSL_LOG_MODULE,
...
kaps_status
mdb_lpm_xpt_big_kaps_ads_depth(
    int unit,
    ...
    uint8 *bytes)
{
    ...
    if (mem != INVALIDm)
    {
        ...
        DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core)
        {
            ...
            else
            {
                ...
                uint32 mem_array_xor[MDB_LPM_MAX_UINT32_WIDTH];

                sal_memcpy(mem_array_xor, mem_array_temp, sizeof(mem_array_temp));

                rv = utilex_bitstream_xor(mem_array_xor, mem_array, MDB_LPM_MAX_UINT32_WIDTH);
                if (rv != _SHR_E_NONE)
                {
                    LOG_ERROR(BSL_LOG_MODULE,
                              (BSL_META_U(unit, "%s(), utilex_bitstream_xor failed.\n"), FUNCTION_NAME()));
                    return KAPS_FATAL_TRANSPORT_ERROR;
                }

                if (utilex_bitstream_have_one_in_range
                    (mem_array_xor, 0  , MDB_LPM_MAX_UINT32_WIDTH * SAL_UINT32_NOF_BITS - 1))
                {
                    int uint32_iter;

                    LOG_ERROR(BSL_LOG_MODULE,
                              (BSL_META_U(unit, "%s(), core compare failed, core %d does not match core 0, .\n"),
                               FUNCTION_NAME(), core));
                    LOG_ERROR(BSL_LOG_MODULE,
                              (BSL_META_U(unit, "%s():  blk_id: %d, cmd: %d, func: %d, offset: %d, nbytes: %d\n"),
                               FUNCTION_NAME(), blk_id, cmd, func, offset, nbytes));

                    LOG_ERROR(BSL_LOG_MODULE, (BSL_META_U(unit, "%s() core 0: 0x"), FUNCTION_NAME()));
                    for (uint32_iter = 0; uint32_iter < MDB_LPM_MAX_UINT32_WIDTH; uint32_iter++)
                    {
                        LOG_ERROR(BSL_LOG_MODULE, (BSL_META_U(unit, "%08X"), mem_array[uint32_iter]));
                    }
                    LOG_ERROR(BSL_LOG_MODULE, (BSL_META_U(unit, "\n")));

                    LOG_ERROR(BSL_LOG_MODULE, (BSL_META_U(unit, "%s() core %d: 0x"), FUNCTION_NAME(), core));
                    for (uint32_iter = 0; uint32_iter < MDB_LPM_MAX_UINT32_WIDTH; uint32_iter++)
                    {
                        LOG_ERROR(BSL_LOG_MODULE, (BSL_META_U(unit, "%08X"), mem_array[uint32_iter]));
                    }
        ...
        if ((mem == KAPS_RPB_TCAM_CPU_COMMANDm)
            && (dnx_data_mdb.kaps.feature_get(unit, dnx_data_mdb_kaps_tcam_read_valid_bits_lsbits)
#ifdef ADAPTER_SERVER_MODE
                && FALSE
#endif
            ))
        {
            int nof_valid_bits = dnx_data_mdb.kaps.rpb_valid_bits_get(unit);
            int key_width_in_bits = dnx_data_mdb.kaps.key_width_in_bits_get(unit);
            int key_lsn_bits = dnx_data_mdb.kaps.key_lsn_bits_get(unit);
            uint8 valid_bits;
            int uint32_iter;

            valid_bits = mem_array[0] & ((1 << nof_valid_bits) - 1);

            
            for (uint32_iter = 0; uint32_iter < MDB_LPM_RPB_MAX_UINT32_WIDTH; uint32_iter++)
            {
                mem_array[uint32_iter] =
                    ((mem_array[uint32_iter + 1] & ((1 << nof_valid_bits) - 1)) << (32 - nof_valid_bits)) |
                    (mem_array[uint32_iter] >> nof_valid_bits);
            }

            
            uint32_iter = BITS2WORDS(key_width_in_bits + key_lsn_bits) - 1;
            mem_array[uint32_iter] = mem_array[uint32_iter] | (valid_bits << key_lsn_bits);
        }
             ...
             && func == KAPS_FUNC0)
    {
        ...
        mdb_physical_table_e mdb_physical_table = MDB_PHYSICAL_TABLE_ADS_1 + (blk_id - MDB_LPM_SECOND_ADS_FIRST_BLK_ID);
        ...
        rv = mdb_lpm_xpt_big_kaps_ads_depth(unit, mdb_physical_table, &reg_val);
        if (rv != _SHR_E_NONE)
        {
            LOG_ERROR(BSL_LOG_MODULE,
    ...
    else if (blk_id >= MDB_LPM_SECOND_BB_FIRST_BLK_ID && blk_id <= MDB_LPM_SECOND_BB_LAST_BLK_ID && func == KAPS_FUNC0)
    {
        ...
        if (offset == KAPS2_BIG_CONFIG_REG_ADDR)
        {
            ...
            if (!SOC_WARM_BOOT(unit))
            {
                ...
                int kaps_allocation[MDB_PHYSICAL_TABLE_BIG_KAPS_BB_2 - MDB_PHYSICAL_TABLE_BIG_KAPS_BB_1 + 1] = { 0 };

                

                for (mdb_physical_table_id = MDB_PHYSICAL_TABLE_BIG_KAPS_BB_1;
                     mdb_physical_table_id <= MDB_PHYSICAL_TABLE_BIG_KAPS_BB_2; mdb_physical_table_id++)
                {
                    int bucket_index;
                    for (bucket_index = 0;
                         bucket_index < dnx_data_mdb.dh.nof_buckets_in_macro_get(unit); bucket_index++)
                    {
                        ...
                        rv = mdb_lpm_xpt_is_cluster_allocated(unit, mdb_physical_table_id, macro_type, macro_index,
                                                              bucket_index *
                        ...
                        kaps_allocation[mdb_physical_table_id - MDB_PHYSICAL_TABLE_BIG_KAPS_BB_1] += is_allocated;
                    }
                }
                ...
                if ((kaps_allocation[0] != 0) && (kaps_allocation[1] != 0))
                {
                    ...
                    rv = mdb_lpm_xpt_is_cluster_allocated(unit, MDB_PHYSICAL_TABLE_BIG_KAPS_BB_1,
                                                          macro_type, macro_index, 0, &is_allocated);
                    if (rv != _SHR_E_NONE)
                    {
                        LOG_ERROR(BSL_LOG_MODULE,
                                  (BSL_META_U(unit, "%s(), mdb_lpm_is_cluster_allocated failed.\n"), FUNCTION_NAME()));
                    ...
                    if (is_allocated != TRUE)
                    {
                        LOG_ERROR(BSL_LOG_MODULE,
                                  (BSL_META_U
                                   (unit,
                                    "%s(), A480_B480 requires the lower bucket to be allocated to the first DB.\n"),
                                   FUNCTION_NAME()));
                        return KAPS_FATAL_TRANSPORT_ERROR;
                    }
                ...
                else
                {
                    ...
                    {
                        mdb_physical_table = MDB_PHYSICAL_TABLE_ADS_1;
                    ...
                    else if (ads_depth == MDB_LPM_ADS_DEPTH_IN_MACRO_A_UNITS(unit, 1))
                    {
                        if (macro_type == MDB_MACRO_A)
                        {
                            if (dnx_data_mdb.kaps.feature_get(unit, dnx_data_mdb_kaps_flex_bb_row))
                            {
                                if (allocation == 1)
                                {
                                    reg_val = regular_alloc;
    ...
    uint8_t * bytes)
{
    ...
    {
        case KAPS_CMD_READ:
            rv = mdb_lpm_xpt_read_command(xpt, blk_nr, cmd, func, row_nr, nbytes, bytes);
            break;

        case KAPS_CMD_WRITE:
            rv = mdb_lpm_xpt_write_command(xpt, blk_nr, cmd, func, row_nr, nbytes, bytes);
            break;
    ...
    int unit)
{
    ...
    {
        int tcam_row;
        int nof_tcam_rows = dnx_data_mdb.kaps.db_info_get(unit, blk_id - MDB_LPM_RPB_FIRST_BLK_ID)->nof_rows_in_rpb;
        ...
        for (tcam_row = 0; tcam_row < nof_tcam_rows; tcam_row++)
        {
            DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core)
            {
                ...
                rv = mdb_lpm_xpt_write_command_memory(unit, KAPS_RPB_TCAM_CPU_COMMANDm,
                                                      blk_id - MDB_LPM_RPB_FIRST_BLK_ID, KAPS_BLOCK(unit, core),
                                                      tcam_row, mem_array);
    ...
    {
        for (db_id = 0; db_id < dnx_data_mdb.kaps.nof_dbs_get(unit); db_id++)
        {
            int hitbit_row_idx_max = dnx_data_mdb.kaps.db_info_get(unit, db_id)->nof_rows_in_small_rpb_hitbits;
    ...
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core) for (bb_id = 0; bb_id < MDB_LPM_BB_NOF_BLK_ID; bb_id++)
    {
        {
            int hitbit_row_idx;
            int hitbit_row_idx_max = dnx_data_mdb.kaps.db_info_get(unit, 0)->nof_rows_in_small_bb;
...
{
    uint32 field;
    soc_mem_t ads_mem = dnx_data_mdb.kaps.db_access_info_get(unit, db_id)->small_ads_mem;

    SHR_FUNC_INIT_VARS(unit);

    {
        soc_mem_field_get(unit, ads_mem, raw_ads, RPB_ADS_BDATAf, &field);
        LOG_CLI((BSL_META_U(unit, " BDATA:0x%x"), field));

        soc_mem_field_get(unit, ads_mem, raw_ads, RPB_ADS_BLENGTHf, &field);
        LOG_CLI((BSL_META_U(unit, " BLENGTH:0x%x"), field));

        soc_mem_field_get(unit, ads_mem, raw_ads, RPB_ADS_ROFSf, &field);
        LOG_CLI((BSL_META_U(unit, " ROFS:0x%x"), field));

        soc_mem_field_get(unit, ads_mem, raw_ads, RPB_ADS_FM_0f, &field);
        LOG_CLI((BSL_META_U(unit, " FM_0:0x%x"), field));

        soc_mem_field_get(unit, ads_mem, raw_ads, RPB_ADS_FM_1f, &field);
        LOG_CLI((BSL_META_U(unit, " 1:0x%x"), field));

        soc_mem_field_get(unit, ads_mem, raw_ads, RPB_ADS_FM_2f, &field);
        LOG_CLI((BSL_META_U(unit, " 2:0x%x"), field));

        soc_mem_field_get(unit, ads_mem, raw_ads, RPB_ADS_FM_3f, &field);
        LOG_CLI((BSL_META_U(unit, " 3:0x%x"), field));

        soc_mem_field_get(unit, ads_mem, raw_ads, RPB_ADS_FM_4f, &field);
        LOG_CLI((BSL_META_U(unit, " 4:0x%x"), field));

        soc_mem_field_get(unit, ads_mem, raw_ads, RPB_ADS_FM_5f, &field);
        LOG_CLI((BSL_META_U(unit, " 5:0x%x"), field));

        soc_mem_field_get(unit, ads_mem, raw_ads, RPB_ADS_FM_6f, &field);
        LOG_CLI((BSL_META_U(unit, " 6:0x%x"), field));

        soc_mem_field_get(unit, ads_mem, raw_ads, RPB_ADS_FM_7f, &field);
        LOG_CLI((BSL_META_U(unit, " 7:0x%x"), field));

        soc_mem_field_get(unit, ads_mem, raw_ads, RPB_ADS_FM_8f, &field);
        LOG_CLI((BSL_META_U(unit, " 8:0x%x"), field));

        soc_mem_field_get(unit, ads_mem, raw_ads, RPB_ADS_FM_9f, &field);
        LOG_CLI((BSL_META_U(unit, " 9:0x%x"), field));

        soc_mem_field_get(unit, ads_mem, raw_ads, RPB_ADS_FM_10f, &field);
        LOG_CLI((BSL_META_U(unit, " 10:0x%x"), field));

        soc_mem_field_get(unit, ads_mem, raw_ads, RPB_ADS_FM_11f, &field);
        LOG_CLI((BSL_META_U(unit, " 11:0x%x"), field));

        soc_mem_field_get(unit, ads_mem, raw_ads, RPB_ADS_FM_12f, &field);
        LOG_CLI((BSL_META_U(unit, " 12:0x%x"), field));

        soc_mem_field_get(unit, ads_mem, raw_ads, RPB_ADS_FM_13f, &field);
        LOG_CLI((BSL_META_U(unit, " 13:0x%x"), field));

        soc_mem_field_get(unit, ads_mem, raw_ads, RPB_ADS_FM_14f, &field);
        LOG_CLI((BSL_META_U(unit, " 14:0x%x"), field));

        soc_mem_field_get(unit, ads_mem, raw_ads, RPB_ADS_FM_15f, &field);
        LOG_CLI((BSL_META_U(unit, " 15:0x%x"), field));

        soc_mem_field_get(unit, ads_mem, raw_ads, RPB_ADS_KSHIFTf, &field);
        LOG_CLI((BSL_META_U(unit, " KSHIFT:0x%x"), field));

        if (small_kaps)
        {
            soc_mem_field_get(unit, ads_mem, raw_ads, RPB_ADS_LBUCKETf, &field);
        }
        else
        {
            ...
            uint32 lbucket_start_bit = 104;
            SHR_BITCOPY_RANGE(&field, 0, raw_ads, lbucket_start_bit,
    ...
    int *nof_small_bbs)
{
    ...
    for (blk_id = MDB_LPM_BB_FIRST_BLK_ID; blk_id <= MDB_LPM_BB_LAST_BLK_ID; blk_id++)
    {
        ...
        rv = mdb_lpm_xpt_read_command(&xpt, blk_id, KAPS_CMD_READ, KAPS_FUNC0, MDB_LPM_BB_GLOBAL_CONFIG_OFFSET,
                                      sizeof(uint32), bytes);
        if (rv != KAPS_OK)
        {
    ...
    mdb_physical_table_e big_kaps_bb)
{
    ...
    blk_id = MDB_LPM_RPB_FIRST_BLK_ID + small_kaps_db_idx;

    LOG_CLI((BSL_META_U(unit, "Small KAPS RPB:\n")));

    nof_rows_in_rpb = dnx_data_mdb.kaps.db_info_get(unit, small_kaps_db_idx)->nof_rows_in_rpb;
    for (offset = 0; offset < nof_rows_in_rpb; offset++)
    {
        uint8 bytes[DNX_DATA_MAX_MDB_KAPS_RPB_BYTE_WIDTH];
        ...
        {
            rv = mdb_lpm_xpt_read_command(&xpt, blk_id, KAPS_CMD_READ, KAPS_FUNC1, offset,
        ...
        mdb_lpm_kaps_buffer_kaps_to_dbal(BYTES2BITS(dnx_data_mdb.kaps.rpb_byte_width_get(unit)), bytes, words);
        ...
        {
            uint32 words_cache[MDB_LPM_MAX_UINT32_WIDTH] = { 0 };

            SHR_IF_ERR_EXIT(mdb_lpm_cache_tcam_get(unit, small_kaps_db_idx, 0, offset, 0, words_cache));

            
            SHR_IF_ERR_EXIT(utilex_bitstream_xor
                            (words_cache, words, BYTES2WORDS(DNX_DATA_MAX_MDB_KAPS_RPB_BYTE_WIDTH)));

            if (utilex_bitstream_have_one_in_range
                (words_cache, 0  , BYTES2BITS(DNX_DATA_MAX_MDB_KAPS_RPB_BYTE_WIDTH) - 1))
            {
                int uint32_idx;
                ...
                LOG_CLI((BSL_META_U(unit, " Error! cache contains different contents:")));
                LOG_CLI((BSL_META_U(unit, "  Row %d: 0x"), offset));
                for (uint32_idx = 0; uint32_idx < BYTES2WORDS(DNX_DATA_MAX_MDB_KAPS_RPB_BYTE_WIDTH); uint32_idx++)
                {
                    LOG_CLI((BSL_META_U(unit, "%08x"),
                             words_cache[BYTES2WORDS(DNX_DATA_MAX_MDB_KAPS_RPB_BYTE_WIDTH) - 1 - uint32_idx]));
                }
                LOG_CLI((BSL_META_U(unit, "\n")));
    ...
    LOG_CLI((BSL_META_U(unit, "\nSmall KAPS ADS:\n")));
    nof_rows_in_small_ads = dnx_data_mdb.kaps.db_info_get(unit, small_kaps_db_idx)->nof_rows_in_small_ads;
    for (offset = 0; offset < nof_rows_in_small_ads; offset++)
    {
...

        {
            ...
            rv = mdb_lpm_xpt_read_command(&xpt, blk_id, KAPS_CMD_READ, KAPS_FUNC4, offset,
        ...
        mdb_lpm_kaps_buffer_kaps_to_dbal(BYTES2BITS(dnx_data_mdb.kaps.ads_byte_width_get(unit)), bytes, words);

        if (utilex_bitstream_have_one_in_range
            (words, 0  , BYTES2BITS(dnx_data_mdb.kaps.ads_byte_width_get(unit)) - 1))
        {
            LOG_CLI((BSL_META_U(unit, "  Row %d: 0x"), offset));
            ...
            SHR_IF_ERR_EXIT(mdb_lpm_xpt_parse_ads(unit, small_kaps_db_idx, words, TRUE));
            LOG_CLI((BSL_META_U(unit, "\n")));
        }
    }
    ...
    LOG_CLI((BSL_META_U(unit, "\nSmall KAPS BBS:\n")));
    nof_rows_in_small_bb = dnx_data_mdb.kaps.db_info_get(unit, small_kaps_db_idx)->nof_rows_in_small_bb;
    for (blk_id = MDB_LPM_BB_FIRST_BLK_ID; blk_id <= MDB_LPM_BB_LAST_BLK_ID; blk_id++)
    {
        uint8 bytes[DNX_DATA_MAX_MDB_KAPS_BB_BYTE_WIDTH];
        int blk_id_valid;
...

        {
            ...
            blk_id_valid = (rpb_sel == small_kaps_db_idx) ? TRUE : FALSE;
        }

        if (blk_id_valid == TRUE)
        {
            LOG_CLI((BSL_META_U(unit, "  Small KAPS BB %d:\n"), blk_id - MDB_LPM_BB_FIRST_BLK_ID));

            for (offset = 0; offset < nof_rows_in_small_bb; offset++)
            {
                int byte_index;
                uint32 words[MDB_LPM_MAX_UINT32_WIDTH];

                {
                    rv = mdb_lpm_xpt_read_command(&xpt, blk_id, KAPS_CMD_READ, KAPS_FUNC2, offset,
                ...
                mdb_lpm_kaps_buffer_kaps_to_dbal(BYTES2BITS(dnx_data_mdb.kaps.bb_byte_width_get(unit)), bytes, words);
    ...
    for (cluster_idx = 0; cluster_idx < nof_clusters; cluster_idx++)
    {
        ...
        dbal_physical_table_id = dnx_data_mdb.kaps.db_info_get(unit, small_kaps_db_idx)->dbal_id;
        ...
        for (offset = cluster_info.start_address; offset < cluster_info.end_address; offset++)
        {
            ...
            if (utilex_bitstream_have_one_in_range
                (data_entry.payload, 0  , MDB_NOF_CLUSTER_ROW_BITS - 1))
            {
                int uint32_idx;
                ...
                SHR_IF_ERR_EXIT(mdb_lpm_xpt_parse_ads(unit, small_kaps_db_idx, data_entry.payload, FALSE));
                LOG_CLI((BSL_META_U(unit, "\n")));
            }
        }
    }
    ...
    for (cluster_idx = 0; cluster_idx < nof_clusters; cluster_idx++)
    {
        ...
        if (cluster_info.cluster_index % dnx_data_mdb.dh.nof_bucket_clusters_get(unit) == 0)
        {
            ...
            for (offset = 0; offset < cluster_rows; offset++)
            {
                ...
                {
                    uint32 row_data_cache[MDB_ENTRY_BANK_SIZE_IN_BITS / SAL_UINT32_NOF_BITS];
                    int bucket_cluster_iter;

                    for (bucket_cluster_iter = 0; bucket_cluster_iter < dnx_data_mdb.dh.nof_bucket_clusters_get(unit);
                         bucket_cluster_iter++)
                    {
                        SHR_IF_ERR_EXIT(mdb_lpm_cache_big_bb_get
                                        (unit, cluster_info.macro_type, cluster_info.macro_index,
                                         cluster_info.cluster_index + bucket_cluster_iter, offset,
                                         bucket_cluster_iter * MDB_NOF_CLUSTER_ROW_BITS, row_data_cache));
                    }

                    
                    SHR_IF_ERR_EXIT(utilex_bitstream_xor
                                    (row_data_cache, row_data, MDB_ENTRY_BANK_SIZE_IN_BITS / SAL_UINT32_NOF_BITS));

                    if (utilex_bitstream_have_one_in_range(row_data_cache, 0  ,
                                                           BYTES2BITS(dnx_data_mdb.kaps.bb_byte_width_get(unit)) - 1))
                    {
                        int uint32_idx;
                        ...
                        {
                            LOG_CLI((BSL_META_U(unit, "%08x"),
                                     row_data_cache[BYTES2WORDS(dnx_data_mdb.kaps.bb_byte_width_get(unit)) - 1 -
                                                    uint32_idx]));
                        }
                        LOG_CLI((BSL_META_U(unit, "\n")));
    ...
    if (dump_error == TRUE)
    {
        SHR_ERR_EXIT(_SHR_E_FAIL, "HW dump contents different from cache contents, see error prints above.\n");
    }
    ...
    uint8_t * bytes)
{
    ...
    sal_memset(bytes, 0, MDB_LPM_HB_BYTES * sizeof(uint8_t));
    ...
    else if ((blk_nr >= MDB_LPM_SECOND_BB_FIRST_BLK_ID) && (blk_nr <= MDB_LPM_SECOND_BB_LAST_BLK_ID))
    {
        ...
        rv = mdb_lpm_xpt_map_bb_big_kaps(unit, blk_nr, KAPS_FUNC2, row_nr, &mem, &bucket_index, &blk,
    ...
    void **xpt)
{
    ...
    SHR_ALLOC_SET_ZERO(*xpt, sizeof(MDB_LPM_XPT), "kaps_xpt", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
    if (*xpt == NULL)
    {
        SHR_ERR_EXIT(_SHR_E_MEMORY, " Error:  SHR_ALLOC_SET_ZERO for xpt Failed\n");
    }
    ...
    {
        xpt_p->mdb_lpm_xpt.kaps_search = mdb_lpm_xpt_search;
        xpt_p->mdb_lpm_xpt.kaps_register_read = mdb_lpm_xpt_register_read;
        xpt_p->mdb_lpm_xpt.kaps_command = mdb_lpm_xpt_command;
        xpt_p->mdb_lpm_xpt.kaps_register_write = mdb_lpm_xpt_register_write;

        xpt_p->mdb_lpm_xpt.kaps_hb_read = mdb_lpm_xpt_hb_read;
        xpt_p->mdb_lpm_xpt.kaps_hb_write = mdb_lpm_xpt_hb_write;

        if (!SOC_WARM_BOOT(unit))
        {
            SHR_IF_ERR_EXIT(mdb_lpm_xpt_init_overrides(unit));
    ...
    uint32 *dst_array)
{
    ...
    uint32 nbytes = DNX_DATA_MAX_MDB_KAPS_BB_BYTE_WIDTH;
    uint8 uint8_data[WORDS2BYTES(MDB_LPM_MAX_UINT32_WIDTH)];
    uint32 uint32_data[MDB_LPM_MAX_UINT32_WIDTH];
    ...
    for (blk_nr = MDB_LPM_SECOND_BB_FIRST_BLK_ID; blk_nr <= MDB_LPM_SECOND_BB_LAST_BLK_ID; blk_nr++)
    {
        soc_mem_t mem;
        mdb_macro_types_e macro_type_temp;
        int macro_index_temp;
        int soc_blk;

        SHR_IF_ERR_EXIT(mdb_lpm_xpt_map_bb_big_kaps_blk_id
                        (unit, blk_nr, &mem, &soc_blk, &macro_type_temp, &macro_index_temp));

        if ((macro_type_temp == macro_type) && (macro_index_temp == macro_index))
        {
            break;
        }
    }

    if (blk_nr > MDB_LPM_SECOND_BB_LAST_BLK_ID)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL,
                     "Unexpected macro_type (%s) and macro_index (%d) for LPM BB data.\n",
                     dnx_data_mdb.dh.macro_type_info_get(unit, macro_type)->name, macro_index);
    }

    mdb_kaps_db.big_kaps_large_bb_mode.get(unit, blk_nr - MDB_LPM_SECOND_BB_FIRST_BLK_ID, &blk_nr_mode);

    if ((blk_nr_mode == KAPS_LARGE_BB_WIDTH_A480_B000) || (blk_nr_mode == KAPS_LARGE_BB_WIDTH_A000_B480))
    {
        
        int bucket_is_bb[DNX_DATA_MAX_MDB_DH_NOF_BUCKETS_IN_MACRO];
        int bucket_iter;
        ...
        for (bucket_iter = 0; bucket_iter < DNX_DATA_MAX_MDB_DH_NOF_BUCKETS_IN_MACRO; bucket_iter++)
        {
            mdb_physical_table_e assoc_mdb_physical_table_id;

            SHR_IF_ERR_EXIT(mdb_init_get_cluster_assoc
                            (unit, macro_type, macro_index, bucket_iter * DNX_DATA_MAX_MDB_DH_NOF_BUCKET_CLUSTERS,
                             &assoc_mdb_physical_table_id));
            bucket_is_bb[bucket_iter] = mdb_lpm_xpt_physical_table_is_bb(unit, assoc_mdb_physical_table_id);
        }

        if ((bucket_is_bb[0] == TRUE) && (bucket_is_bb[1] == TRUE)
            && (cluster_index >= DNX_DATA_MAX_MDB_DH_NOF_BUCKET_CLUSTERS))
        {
            
            row_nr += dnx_data_mdb.dh.macro_type_info_get(unit, macro_type)->nof_rows;
        }
    }
    else if ((blk_nr_mode == KAPS_LARGE_BB_WIDTH_A960_B000) || (blk_nr_mode == KAPS_LARGE_BB_WIDTH_A000_B960) ||
             (blk_nr_mode == KAPS_LARGE_BB_WIDTH_A480_B480))
    {
        
        int bucket_idx = cluster_index / DNX_DATA_MAX_MDB_DH_NOF_BUCKET_CLUSTERS;
        ...
        {
            func_num = KAPS_FUNC10;
        }
        else
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "Unexpected cluster_index (%d)  for LPM BB data.\n", cluster_index);
        }
    }
    else
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Unexpected BB mode (%d)  for LPM BB data.\n", blk_nr_mode);
    }

    DNX_KAPS_TRY(kaps_device_get_ser_data(device, blk_nr, row_nr, func_num, nbytes, uint8_data));

    mdb_lpm_kaps_buffer_kaps_to_dbal(BYTES2BITS(nbytes), uint8_data, uint32_data);

    SHR_BITCOPY_RANGE(dst_array, dst_offset_bits, uint32_data,
                      MDB_NOF_CLUSTER_ROW_BITS * (cluster_index % DNX_DATA_MAX_MDB_DH_NOF_BUCKET_CLUSTERS),
    ...
    uint32 *dst_array)
{
    ...
    uint32 nbytes = DNX_DATA_MAX_MDB_KAPS_RPB_BYTE_WIDTH * 2;
    uint8 uint8_data[WORDS2BYTES(MDB_LPM_MAX_UINT32_WIDTH)];
    uint32 uint32_data[MDB_LPM_MAX_UINT32_WIDTH] = { 0 };
    uint32 field_val;

    SHR_FUNC_INIT_VARS(unit);

    
    blk_nr = MDB_LPM_RPB_FIRST_BLK_ID + db_id;
    row_nr = row_index;
    func_num = KAPS_FUNC1;
    ...
    mdb_lpm_kaps_buffer_kaps_to_dbal(BYTES2BITS(DNX_DATA_MAX_MDB_KAPS_RPB_BYTE_WIDTH),
                                     &uint8_data[(row_index % MDB_LPM_RPB_ROWS_NUM) *
                                                 DNX_DATA_MAX_MDB_KAPS_RPB_BYTE_WIDTH], uint32_data);

    
    field_val = 1;
    if (instance_id == 0)
    {
        soc_mem_field_set(unit, KAPS_RPB_TCAM_CPU_COMMANDm, uint32_data, RPB_TCAM_CPU_COMMAND_INST_Af, &field_val);
    }
    else if (instance_id == 1)
    {
        soc_mem_field_set(unit, KAPS_RPB_TCAM_CPU_COMMANDm, uint32_data, RPB_TCAM_CPU_COMMAND_INST_Bf, &field_val);
    }
    else
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Unexpected instance_id (%d), expected 0 (A) or 1 (B).\n", instance_id);
    }

    SHR_BITCOPY_RANGE(dst_array, dst_offset_bits, uint32_data, 0, WORDS2BITS(MDB_LPM_RPB_MAX_UINT32_WIDTH));
    ...
    dbal_physical_entry_t * entry)
{
    ...
    search_cmd_mem = dnx_data_mdb.kaps.db_access_info_get(unit, mdb_db_idx)->search_cmd_mem;

    SHR_IF_ERR_EXIT(mdb_lpm_xpt_big_kaps_enabled(unit, mdb_db_idx, &big_kaps_enabled));

    {
        ...
        mem_field = 0x1;
        soc_mem_field_set(unit, search_cmd_mem, mem_cmd, RPB_TCAM_CPU_COMMAND_INST_Af, &mem_field);
        ...
        mem_blk = KAPS_BLOCK(unit, core);
        reg_blk = MDB_BLOCK(unit);
#endif 
    }

    SHR_IF_ERR_EXIT(soc_mem_array_write(unit, search_cmd_mem, mdb_db_idx, mem_blk, 0, mem_cmd));

    
    SHR_IF_ERR_EXIT(soc_mem_array_read(unit, search_cmd_mem, mdb_db_idx, mem_blk, 0, mem_reply));
    ...
    if (big_kaps_enabled == FALSE)
    {
        ...
        entry->payload[0] = (mem_reply[1]) & ((1 << (entry->payload_size)) - 1);
    ...
    else
    {
        ...
        soc_field_t payload_field, prefix_length_field, result_found_field, result_error_field;
        ...
        result_reg = dnx_data_mdb.kaps.kaps_lookup_result_get(unit, mdb_db_idx, core)->lookup_result;

        SHR_IF_ERR_EXIT(soc_reg_above_64_get(unit, result_reg, reg_blk, 0, reg_val));

        {
            payload_field = ITEM_0_19f;
            prefix_length_field = ITEM_20_27f;
            result_found_field = ITEM_28_28f;
            result_error_field = ITEM_29_29f;

            payload_size = mdb_lpm_ad_size_enum_to_bits[MDB_LPM_AD_SIZE_20];
        }

        found = soc_reg_above_64_field32_get(unit, result_reg, reg_val, result_found_field);
        error = soc_reg_above_64_field32_get(unit, result_reg, reg_val, result_error_field);

        if (error != 0)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL,
                         " Search interface has returned error indication. found: %d, error: %d, prefix length: %d\n",
                         found, error, soc_reg_above_64_field32_get(unit, result_reg, reg_val, prefix_length_field));
        }

        if (found != 0)
        {
            soc_reg_above_64_field_get(unit, result_reg, reg_val, payload_field, entry->payload);

            entry->prefix_length = soc_reg_above_64_field32_get(unit, result_reg, reg_val, prefix_length_field);
