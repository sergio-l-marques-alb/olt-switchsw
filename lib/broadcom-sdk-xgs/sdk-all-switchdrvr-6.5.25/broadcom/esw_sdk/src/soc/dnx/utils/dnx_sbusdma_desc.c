...
/* Holds the descriptor chain creation time */
...
/* Allows activating and de-activating the timeout mechanism during runtime */
    ...
    int unit)
{
    ...
    /*
     * Destroy the timeout thread if it is active, destroy mutex afterwards.
     */
    SHR_IF_ERR_EXIT(shr_thread_manager_is_active_get(dnx_sbusdma_desc_timeout_tid[unit], &is_active));
    ...
    uint32 timeout_usec)
{
    ...
    /*
     * Only allocate mutex and activate timeout thread if the timeout param is non-zero
     */
    if (timeout_usec != 0)
    {
        shr_error_e rv;
    ...
    uint32 enable)
{
    ...
    /*
     * Commit any pending transactions before changing the timeout behavior 
     */
