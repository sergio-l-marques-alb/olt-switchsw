/** \file j2x_a0_data_pll.c
 * 
 * DEVICE DATA - PLL
 * 
 * Device Data
 * SW component that maintains per device data
 * For additional details about Device Data Component goto 'dnxc_data_mgmt.h'
 *        
 *     
 * 
 * AUTO-GENERATED BY AUTOCODER!
 * DO NOT EDIT THIS FILE!
 */
/* *INDENT-OFF* */
/*
 * $Copyright: (c) 2021 Broadcom.
 * Broadcom Proprietary and Confidential. All rights reserved.$
 */
#ifdef BSL_LOG_MODULE
#error "BSL_LOG_MODULE redefined"
#endif
#define BSL_LOG_MODULE BSL_LS_BCMDNX_PLL
/*
 * INCLUDE FILES:
 * {
 */
#include <soc/dnx/dnx_data/auto_generated/dnx_data_internal_pll.h>
#include <soc/dnx/pll/pll.h>
/*
 * }
 */

/*
 * FUNCTIONS:
 * {
 */
/*
 * Submodule: general
 */

/*
 * Features
 */
/**
 * \brief Pointer to function (per device) which set values for feature
 * 
 * \param [in] unit - Unit #
 * 
 * \return
 *     err - see shr_error_e
 * \remark
 *   * Auto-generated using device data autocoder
 * \see
 *   * None
 */
static shr_error_e
j2x_a0_dnx_data_pll_general_ts_freq_lock_set(
    int unit)
{
    dnxc_data_feature_t *feature;
    int module_index = dnx_data_module_pll;
    int submodule_index = dnx_data_pll_submodule_general;
    int feature_index = dnx_data_pll_general_ts_freq_lock;
    SHR_FUNC_INIT_VARS(unit);

    feature = &_dnxc_data[unit].modules[module_index].submodules[submodule_index].features[feature_index];
    /* Set default value */
    feature->default_data = 1;

    /* Set value */
    feature->data = 1;

    /* Set data flags as supported */
    feature->flags |= DNXC_DATA_F_SUPPORTED;
    /* Property */
    feature->property.name = spn_PHY_1588_DPLL_FREQUENCY_LOCK;
    feature->property.doc = 
        "\n"
        "IEEE1588 DPLL mode\n"
        "Supported values: 0 - phase lock, 1 - frequency lock\n"
        "\n"
    ;
    feature->property.method = dnxc_data_property_method_enable;
    feature->property.method_str = "enable";
    SHR_IF_ERR_EXIT(dnxc_data_mgmt_property_read(unit, &feature->property, -1, &feature->data));
    /* Set data flags as property */
    feature->flags |= (DNXC_DATA_F_PROPERTY);


    /* Override data with dynamic data (if required) */
    SHR_IF_ERR_EXIT(dnxc_data_dyn_loader_feature_value_set(unit, feature));

exit:
    SHR_FUNC_EXIT;
}

/*
 * Defines
 */
/*
 * Tables
 */
/**
 * \brief device level function which set values for table nif_pll
 * Module - 'pll', Submodule - 'general', table - 'nif_pll'
 * NIF PLL reference and output frequencies
 * The function set relevant table structure in _dnx_data
 * 
 * \param [in] unit - Unit #
 * 
 * \return
 *     err - See shr_error_e
 * \remark
 *   * Auto-generated using device data autocoder
 * \see
 *   * None
 */
static shr_error_e
j2x_a0_dnx_data_pll_general_nif_pll_set(
    int unit)
{
    int pll_index_index;
    dnx_data_pll_general_nif_pll_t *data, *default_data;
    dnxc_data_table_t *table;
    int module_index = dnx_data_module_pll;
    int submodule_index = dnx_data_pll_submodule_general;
    int table_index = dnx_data_pll_general_table_nif_pll;
    SHR_FUNC_INIT_VARS(unit);

    table = &_dnxc_data[unit].modules[module_index].submodules[submodule_index].tables[table_index];
    /* Set data flags as supported */
    table->flags |= DNXC_DATA_F_SUPPORTED;

    /*
     * Set key sizes
     */
    table->keys[0].size = 2;
    table->info_get.key_size[0] = 2;

    /* Info - default values */
    table->values[0].default_val = "DNX_SERDES_REF_CLOCK_156_25";
    table->values[1].default_val = "DNX_SERDES_REF_CLOCK_156_25";
    /* Exit the function if the table size is zero */
    if (table->keys[0].size == 0 || table->info_get.key_size[0] == 0)
    {
        SHR_EXIT();
    }

    /* Allocate data buffer */
    DNXC_DATA_ALLOC(table->data, dnx_data_pll_general_nif_pll_t, (1 * (table->keys[0].size) + 1 /* to store default value */ ), "data of dnx_data_pll_general_table_nif_pll");

    /* Store Default Values */
    default_data = (dnx_data_pll_general_nif_pll_t *) dnxc_data_mgmt_table_data_get(unit, table, -1, -1);
    default_data->in_freq = DNX_SERDES_REF_CLOCK_156_25;
    default_data->out_freq = DNX_SERDES_REF_CLOCK_156_25;
    /* Set Default Values */
    for (pll_index_index = 0; pll_index_index < table->keys[0].size; pll_index_index++)
    {
        data = (dnx_data_pll_general_nif_pll_t *) dnxc_data_mgmt_table_data_get(unit, table, pll_index_index, 0);
        sal_memcpy(data, default_data, table->size_of_values);
    }
    /*
     * Set Values - Entries
     */
    if (0 < table->keys[0].size)
    {
        data = (dnx_data_pll_general_nif_pll_t *) dnxc_data_mgmt_table_data_get(unit, table, 0, 0);
        data->in_freq = DNX_SERDES_REF_CLOCK_156_25;
        data->out_freq = DNX_SERDES_REF_CLOCK_156_25;
    }

    /* Override data with dynamic data (if required) */
    SHR_IF_ERR_EXIT(dnxc_data_dyn_loader_table_value_set(unit, table));


exit:
    SHR_FUNC_EXIT;
}

/**
 * \brief device level function which set values for table fabric_pll
 * Module - 'pll', Submodule - 'general', table - 'fabric_pll'
 * Fabric PLL reference and output frequencies
 * The function set relevant table structure in _dnx_data
 * 
 * \param [in] unit - Unit #
 * 
 * \return
 *     err - See shr_error_e
 * \remark
 *   * Auto-generated using device data autocoder
 * \see
 *   * None
 */
static shr_error_e
j2x_a0_dnx_data_pll_general_fabric_pll_set(
    int unit)
{
    int pll_index_index;
    dnx_data_pll_general_fabric_pll_t *data, *default_data;
    dnxc_data_table_t *table;
    int module_index = dnx_data_module_pll;
    int submodule_index = dnx_data_pll_submodule_general;
    int table_index = dnx_data_pll_general_table_fabric_pll;
    SHR_FUNC_INIT_VARS(unit);

    table = &_dnxc_data[unit].modules[module_index].submodules[submodule_index].tables[table_index];
    /* Set data flags as supported */
    table->flags |= DNXC_DATA_F_SUPPORTED;

    /*
     * Set key sizes
     */
    table->keys[0].size = 2;
    table->info_get.key_size[0] = 2;

    /* Info - default values */
    table->values[0].default_val = "DNX_SERDES_REF_CLOCK_156_25";
    table->values[1].default_val = "DNX_SERDES_REF_CLOCK_156_25";
    /* Exit the function if the table size is zero */
    if (table->keys[0].size == 0 || table->info_get.key_size[0] == 0)
    {
        SHR_EXIT();
    }

    /* Allocate data buffer */
    DNXC_DATA_ALLOC(table->data, dnx_data_pll_general_fabric_pll_t, (1 * (table->keys[0].size) + 1 /* to store default value */ ), "data of dnx_data_pll_general_table_fabric_pll");

    /* Store Default Values */
    default_data = (dnx_data_pll_general_fabric_pll_t *) dnxc_data_mgmt_table_data_get(unit, table, -1, -1);
    default_data->in_freq = DNX_SERDES_REF_CLOCK_156_25;
    default_data->out_freq = DNX_SERDES_REF_CLOCK_156_25;
    /* Set Default Values */
    for (pll_index_index = 0; pll_index_index < table->keys[0].size; pll_index_index++)
    {
        data = (dnx_data_pll_general_fabric_pll_t *) dnxc_data_mgmt_table_data_get(unit, table, pll_index_index, 0);
        sal_memcpy(data, default_data, table->size_of_values);
    }
    /*
     * Set Values - Entries
     */
    if (0 < table->keys[0].size)
    {
        data = (dnx_data_pll_general_fabric_pll_t *) dnxc_data_mgmt_table_data_get(unit, table, 0, 0);
        data->in_freq = DNX_SERDES_REF_CLOCK_156_25;
        data->out_freq = DNX_SERDES_REF_CLOCK_156_25;
    }

    /* Override data with dynamic data (if required) */
    SHR_IF_ERR_EXIT(dnxc_data_dyn_loader_table_value_set(unit, table));


exit:
    SHR_FUNC_EXIT;
}

/*
 * Submodule: pll1
 */

/*
 * Features
 */
/*
 * Defines
 */
/**
 * \brief device level function which set define nof_pll1
 * define info:
 * Number of PLL1
 * 
 * \param [in] unit - Unit #
 * 
 * \return
 *     err - see shr_error_e
 * \remark
 *   * Auto-generated using device data autocoder
 * \see
 *   * None
 */
static shr_error_e
j2x_a0_dnx_data_pll_pll1_nof_pll1_set(
    int unit)
{
    dnxc_data_define_t *define;
    int module_index = dnx_data_module_pll;
    int submodule_index = dnx_data_pll_submodule_pll1;
    int define_index = dnx_data_pll_pll1_define_nof_pll1;
    SHR_FUNC_INIT_VARS(unit);

    define = &_dnxc_data[unit].modules[module_index].submodules[submodule_index].defines[define_index];
    /* Set default value */
    define->default_data = 0;

    /* Set value */
    define->data = 0;

    /* Set data flags as supported */
    define->flags |= DNXC_DATA_F_SUPPORTED;

    /* Override data with dynamic data (if required) */
    SHR_IF_ERR_EXIT(dnxc_data_dyn_loader_define_value_set(unit, define));


exit:
    SHR_FUNC_EXIT;
}

/*
 * Tables
 */
/*
 * Submodule: pll3
 */

/*
 * Features
 */
/*
 * Defines
 */
/**
 * \brief device level function which set define nof_pll3
 * define info:
 * Number of PLL3
 * 
 * \param [in] unit - Unit #
 * 
 * \return
 *     err - see shr_error_e
 * \remark
 *   * Auto-generated using device data autocoder
 * \see
 *   * None
 */
static shr_error_e
j2x_a0_dnx_data_pll_pll3_nof_pll3_set(
    int unit)
{
    dnxc_data_define_t *define;
    int module_index = dnx_data_module_pll;
    int submodule_index = dnx_data_pll_submodule_pll3;
    int define_index = dnx_data_pll_pll3_define_nof_pll3;
    SHR_FUNC_INIT_VARS(unit);

    define = &_dnxc_data[unit].modules[module_index].submodules[submodule_index].defines[define_index];
    /* Set default value */
    define->default_data = 0;

    /* Set value */
    define->data = 0;

    /* Set data flags as supported */
    define->flags |= DNXC_DATA_F_SUPPORTED;

    /* Override data with dynamic data (if required) */
    SHR_IF_ERR_EXIT(dnxc_data_dyn_loader_define_value_set(unit, define));


exit:
    SHR_FUNC_EXIT;
}

/*
 * Tables
 */
/*
 * Submodule: pll4
 */

/*
 * Features
 */
/**
 * \brief Pointer to function (per device) which set values for feature
 * 
 * \param [in] unit - Unit #
 * 
 * \return
 *     err - see shr_error_e
 * \remark
 *   * Auto-generated using device data autocoder
 * \see
 *   * None
 */
static shr_error_e
j2x_a0_dnx_data_pll_pll4_pll_in_frac_mode_set(
    int unit)
{
    dnxc_data_feature_t *feature;
    int module_index = dnx_data_module_pll;
    int submodule_index = dnx_data_pll_submodule_pll4;
    int feature_index = dnx_data_pll_pll4_pll_in_frac_mode;
    SHR_FUNC_INIT_VARS(unit);

    feature = &_dnxc_data[unit].modules[module_index].submodules[submodule_index].features[feature_index];
    /* Set default value */
    feature->default_data = 1;

    /* Set value */
    feature->data = 1;

    /* Set data flags as supported */
    feature->flags |= DNXC_DATA_F_SUPPORTED;

    /* Override data with dynamic data (if required) */
    SHR_IF_ERR_EXIT(dnxc_data_dyn_loader_feature_value_set(unit, feature));

exit:
    SHR_FUNC_EXIT;
}

/*
 * Defines
 */
/**
 * \brief device level function which set define nof_pll4
 * define info:
 * Number of PLL4
 * 
 * \param [in] unit - Unit #
 * 
 * \return
 *     err - see shr_error_e
 * \remark
 *   * Auto-generated using device data autocoder
 * \see
 *   * None
 */
static shr_error_e
j2x_a0_dnx_data_pll_pll4_nof_pll4_set(
    int unit)
{
    dnxc_data_define_t *define;
    int module_index = dnx_data_module_pll;
    int submodule_index = dnx_data_pll_submodule_pll4;
    int define_index = dnx_data_pll_pll4_define_nof_pll4;
    SHR_FUNC_INIT_VARS(unit);

    define = &_dnxc_data[unit].modules[module_index].submodules[submodule_index].defines[define_index];
    /* Set default value */
    define->default_data = 7;

    /* Set value */
    define->data = 7;

    /* Set data flags as supported */
    define->flags |= DNXC_DATA_F_SUPPORTED;

    /* Override data with dynamic data (if required) */
    SHR_IF_ERR_EXIT(dnxc_data_dyn_loader_define_value_set(unit, define));


exit:
    SHR_FUNC_EXIT;
}

/**
 * \brief device level function which set numeric ts_pll4_id
 * numeric info:
 * Index of the TS PLL4, used for DBAL access
 * 
 * \param [in] unit - Unit #
 * 
 * \return
 *     err - see shr_error_e
 * \remark
 *   * Auto-generated using device data autocoder
 * \see
 *   * None
 */
static shr_error_e
j2x_a0_dnx_data_pll_pll4_ts_pll4_id_set(
    int unit)
{
    dnxc_data_define_t *define;
    int module_index = dnx_data_module_pll;
    int submodule_index = dnx_data_pll_submodule_pll4;
    int define_index = dnx_data_pll_pll4_define_ts_pll4_id;
    SHR_FUNC_INIT_VARS(unit);

    define = &_dnxc_data[unit].modules[module_index].submodules[submodule_index].defines[define_index];
    /* Set default value */
    define->default_data = dnx_data_pll.pll4.config_get(unit, DNX_PLL4_TYPE_TS)->pll4_id;

    /* Set value */
    define->data = dnx_data_pll.pll4.config_get(unit, DNX_PLL4_TYPE_TS)->pll4_id;

    /* Set data flags as supported */
    define->flags |= DNXC_DATA_F_SUPPORTED;

    /* Override data with dynamic data (if required) */
    SHR_IF_ERR_EXIT(dnxc_data_dyn_loader_define_value_set(unit, define));


exit:
    SHR_FUNC_EXIT;
}

/**
 * \brief device level function which set numeric bs_pll4_id
 * numeric info:
 * Index of the BS PLL4, used for DBAL access
 * 
 * \param [in] unit - Unit #
 * 
 * \return
 *     err - see shr_error_e
 * \remark
 *   * Auto-generated using device data autocoder
 * \see
 *   * None
 */
static shr_error_e
j2x_a0_dnx_data_pll_pll4_bs_pll4_id_set(
    int unit)
{
    dnxc_data_define_t *define;
    int module_index = dnx_data_module_pll;
    int submodule_index = dnx_data_pll_submodule_pll4;
    int define_index = dnx_data_pll_pll4_define_bs_pll4_id;
    SHR_FUNC_INIT_VARS(unit);

    define = &_dnxc_data[unit].modules[module_index].submodules[submodule_index].defines[define_index];
    /* Set default value */
    define->default_data = dnx_data_pll.pll4.config_get(unit, DNX_PLL4_TYPE_BS)->pll4_id;

    /* Set value */
    define->data = dnx_data_pll.pll4.config_get(unit, DNX_PLL4_TYPE_BS)->pll4_id;

    /* Set data flags as supported */
    define->flags |= DNXC_DATA_F_SUPPORTED;

    /* Override data with dynamic data (if required) */
    SHR_IF_ERR_EXIT(dnxc_data_dyn_loader_define_value_set(unit, define));


exit:
    SHR_FUNC_EXIT;
}

/**
 * \brief device level function which set numeric nif_pll4_id
 * numeric info:
 * Index of the NIF PLL4, used for DBAL access
 * 
 * \param [in] unit - Unit #
 * 
 * \return
 *     err - see shr_error_e
 * \remark
 *   * Auto-generated using device data autocoder
 * \see
 *   * None
 */
static shr_error_e
j2x_a0_dnx_data_pll_pll4_nif_pll4_id_set(
    int unit)
{
    dnxc_data_define_t *define;
    int module_index = dnx_data_module_pll;
    int submodule_index = dnx_data_pll_submodule_pll4;
    int define_index = dnx_data_pll_pll4_define_nif_pll4_id;
    SHR_FUNC_INIT_VARS(unit);

    define = &_dnxc_data[unit].modules[module_index].submodules[submodule_index].defines[define_index];
    /* Set default value */
    define->default_data = dnx_data_pll.pll4.config_get(unit, DNX_PLL4_TYPE_NIF)->pll4_id;

    /* Set value */
    define->data = dnx_data_pll.pll4.config_get(unit, DNX_PLL4_TYPE_NIF)->pll4_id;

    /* Set data flags as supported */
    define->flags |= DNXC_DATA_F_SUPPORTED;

    /* Override data with dynamic data (if required) */
    SHR_IF_ERR_EXIT(dnxc_data_dyn_loader_define_value_set(unit, define));


exit:
    SHR_FUNC_EXIT;
}

/**
 * \brief device level function which set numeric fab_pll4_id
 * numeric info:
 * Index of the FSRD PLL4, used for DBAL access
 * 
 * \param [in] unit - Unit #
 * 
 * \return
 *     err - see shr_error_e
 * \remark
 *   * Auto-generated using device data autocoder
 * \see
 *   * None
 */
static shr_error_e
j2x_a0_dnx_data_pll_pll4_fab_pll4_id_set(
    int unit)
{
    dnxc_data_define_t *define;
    int module_index = dnx_data_module_pll;
    int submodule_index = dnx_data_pll_submodule_pll4;
    int define_index = dnx_data_pll_pll4_define_fab_pll4_id;
    SHR_FUNC_INIT_VARS(unit);

    define = &_dnxc_data[unit].modules[module_index].submodules[submodule_index].defines[define_index];
    /* Set default value */
    define->default_data = dnx_data_pll.pll4.config_get(unit, DNX_PLL4_TYPE_FAB)->pll4_id;

    /* Set value */
    define->data = dnx_data_pll.pll4.config_get(unit, DNX_PLL4_TYPE_FAB)->pll4_id;

    /* Set data flags as supported */
    define->flags |= DNXC_DATA_F_SUPPORTED;

    /* Override data with dynamic data (if required) */
    SHR_IF_ERR_EXIT(dnxc_data_dyn_loader_define_value_set(unit, define));


exit:
    SHR_FUNC_EXIT;
}

/**
 * \brief device level function which set numeric nif_tsc_pll4_id
 * numeric info:
 * Index of the NIF TSC PLL4, used for DBAL access
 * 
 * \param [in] unit - Unit #
 * 
 * \return
 *     err - see shr_error_e
 * \remark
 *   * Auto-generated using device data autocoder
 * \see
 *   * None
 */
static shr_error_e
j2x_a0_dnx_data_pll_pll4_nif_tsc_pll4_id_set(
    int unit)
{
    dnxc_data_define_t *define;
    int module_index = dnx_data_module_pll;
    int submodule_index = dnx_data_pll_submodule_pll4;
    int define_index = dnx_data_pll_pll4_define_nif_tsc_pll4_id;
    SHR_FUNC_INIT_VARS(unit);

    define = &_dnxc_data[unit].modules[module_index].submodules[submodule_index].defines[define_index];
    /* Set default value */
    define->default_data = dnx_data_pll.pll4.config_get(unit, DNX_PLL4_TYPE_NIF_TSC)->pll4_id;

    /* Set value */
    define->data = dnx_data_pll.pll4.config_get(unit, DNX_PLL4_TYPE_NIF_TSC)->pll4_id;

    /* Set data flags as supported */
    define->flags |= DNXC_DATA_F_SUPPORTED;

    /* Override data with dynamic data (if required) */
    SHR_IF_ERR_EXIT(dnxc_data_dyn_loader_define_value_set(unit, define));


exit:
    SHR_FUNC_EXIT;
}

/**
 * \brief device level function which set numeric framer_pll4_id
 * numeric info:
 * Index of the Framer PLL4, used for DBAL access
 * 
 * \param [in] unit - Unit #
 * 
 * \return
 *     err - see shr_error_e
 * \remark
 *   * Auto-generated using device data autocoder
 * \see
 *   * None
 */
static shr_error_e
j2x_a0_dnx_data_pll_pll4_framer_pll4_id_set(
    int unit)
{
    dnxc_data_define_t *define;
    int module_index = dnx_data_module_pll;
    int submodule_index = dnx_data_pll_submodule_pll4;
    int define_index = dnx_data_pll_pll4_define_framer_pll4_id;
    SHR_FUNC_INIT_VARS(unit);

    define = &_dnxc_data[unit].modules[module_index].submodules[submodule_index].defines[define_index];
    /* Set default value */
    define->default_data = dnx_data_pll.pll4.config_get(unit, DNX_PLL4_TYPE_FRAMER)->pll4_id;

    /* Set value */
    define->data = dnx_data_pll.pll4.config_get(unit, DNX_PLL4_TYPE_FRAMER)->pll4_id;

    /* Set data flags as supported */
    define->flags |= DNXC_DATA_F_SUPPORTED;

    /* Override data with dynamic data (if required) */
    SHR_IF_ERR_EXIT(dnxc_data_dyn_loader_define_value_set(unit, define));


exit:
    SHR_FUNC_EXIT;
}

/**
 * \brief device level function which set numeric sar_pll4_id
 * numeric info:
 * Index of the SAR PLL4, used for DBAL access
 * 
 * \param [in] unit - Unit #
 * 
 * \return
 *     err - see shr_error_e
 * \remark
 *   * Auto-generated using device data autocoder
 * \see
 *   * None
 */
static shr_error_e
j2x_a0_dnx_data_pll_pll4_sar_pll4_id_set(
    int unit)
{
    dnxc_data_define_t *define;
    int module_index = dnx_data_module_pll;
    int submodule_index = dnx_data_pll_submodule_pll4;
    int define_index = dnx_data_pll_pll4_define_sar_pll4_id;
    SHR_FUNC_INIT_VARS(unit);

    define = &_dnxc_data[unit].modules[module_index].submodules[submodule_index].defines[define_index];
    /* Set default value */
    define->default_data = dnx_data_pll.pll4.config_get(unit, DNX_PLL4_TYPE_SAR)->pll4_id;

    /* Set value */
    define->data = dnx_data_pll.pll4.config_get(unit, DNX_PLL4_TYPE_SAR)->pll4_id;

    /* Set data flags as supported */
    define->flags |= DNXC_DATA_F_SUPPORTED;

    /* Override data with dynamic data (if required) */
    SHR_IF_ERR_EXIT(dnxc_data_dyn_loader_define_value_set(unit, define));


exit:
    SHR_FUNC_EXIT;
}

/*
 * Tables
 */
/**
 * \brief device level function which set values for table config
 * Module - 'pll', Submodule - 'pll4', table - 'config'
 * PLL4 configuration parameters
 * The function set relevant table structure in _dnx_data
 * 
 * \param [in] unit - Unit #
 * 
 * \return
 *     err - See shr_error_e
 * \remark
 *   * Auto-generated using device data autocoder
 * \see
 *   * None
 */
static shr_error_e
j2x_a0_dnx_data_pll_pll4_config_set(
    int unit)
{
    int pll4_type_index;
    dnx_data_pll_pll4_config_t *data, *default_data;
    dnxc_data_table_t *table;
    int module_index = dnx_data_module_pll;
    int submodule_index = dnx_data_pll_submodule_pll4;
    int table_index = dnx_data_pll_pll4_table_config;
    SHR_FUNC_INIT_VARS(unit);

    table = &_dnxc_data[unit].modules[module_index].submodules[submodule_index].tables[table_index];
    /* Set data flags as supported */
    table->flags |= DNXC_DATA_F_SUPPORTED;

    /*
     * Set key sizes
     */
    table->keys[0].size = DNX_PLL4_TYPE_COUNT;
    table->info_get.key_size[0] = DNX_PLL4_TYPE_COUNT;

    /* Info - default values */
    table->values[0].default_val = "-1";
    table->values[1].default_val = "-1";
    table->values[2].default_val = "-1";
    table->values[3].default_val = "-1";
    table->values[4].default_val = "-1";
    table->values[5].default_val = "-1";
    table->values[6].default_val = "-1";
    table->values[7].default_val = "-1";
    table->values[8].default_val = "-1";
    table->values[9].default_val = "-1";
    table->values[10].default_val = "-1";
    table->values[11].default_val = "-1";
    table->values[12].default_val = "-1";
    table->values[13].default_val = "-1";
    table->values[14].default_val = "-1";
    table->values[15].default_val = "-1";
    table->values[16].default_val = "-1";
    table->values[17].default_val = "-1";
    /* Exit the function if the table size is zero */
    if (table->keys[0].size == 0 || table->info_get.key_size[0] == 0)
    {
        SHR_EXIT();
    }

    /* Allocate data buffer */
    DNXC_DATA_ALLOC(table->data, dnx_data_pll_pll4_config_t, (1 * (table->keys[0].size) + 1 /* to store default value */ ), "data of dnx_data_pll_pll4_table_config");

    /* Store Default Values */
    default_data = (dnx_data_pll_pll4_config_t *) dnxc_data_mgmt_table_data_get(unit, table, -1, -1);
    default_data->pll4_id = -1;
    default_data->valid = -1;
    default_data->ref_clk_src_sel = -1;
    default_data->ref_clk = -1;
    default_data->dll_ref_pdiv = -1;
    default_data->dll_post_en = -1;
    default_data->aux_post_en = -1;
    default_data->pdiv = -1;
    default_data->ndiv_frac_mode = -1;
    default_data->ndiv_p = -1;
    default_data->ndiv_q = -1;
    default_data->ndiv_int = -1;
    default_data->d2c_en = -1;
    default_data->ch0_mdiv = -1;
    default_data->ch1_mdiv = -1;
    default_data->ch2_mdiv = -1;
    default_data->ch3_mdiv = -1;
    default_data->ch6_mdiv = -1;
    /* Set Default Values */
    for (pll4_type_index = 0; pll4_type_index < table->keys[0].size; pll4_type_index++)
    {
        data = (dnx_data_pll_pll4_config_t *) dnxc_data_mgmt_table_data_get(unit, table, pll4_type_index, 0);
        sal_memcpy(data, default_data, table->size_of_values);
    }
    /*
     * Set Values - Entries
     */
    if (DNX_PLL4_TYPE_TS < table->keys[0].size)
    {
        data = (dnx_data_pll_pll4_config_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_PLL4_TYPE_TS, 0);
        data->pll4_id = 0;
        data->valid = 1;
        data->ref_clk_src_sel = 1;
        data->ref_clk = 50;
        data->dll_ref_pdiv = 1;
        data->dll_post_en = 15;
        data->aux_post_en = 15;
        data->pdiv = 1;
        data->ndiv_int = 120;
        data->d2c_en = 1;
        data->ch0_mdiv = 6;
        data->ch1_mdiv = 8;
        data->ch2_mdiv = 24;
        data->ch3_mdiv = 48;
        data->ch6_mdiv = 3;
    }
    if (DNX_PLL4_TYPE_BS < table->keys[0].size)
    {
        data = (dnx_data_pll_pll4_config_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_PLL4_TYPE_BS, 0);
        data->pll4_id = 1;
        data->valid = 1;
        data->ref_clk_src_sel = 0;
        data->ref_clk = 50;
        data->dll_ref_pdiv = 1;
        data->dll_post_en = 15;
        data->aux_post_en = 15;
        data->pdiv = 1;
        data->ndiv_int = 120;
        data->d2c_en = 1;
        data->ch0_mdiv = 200;
        data->ch1_mdiv = 200;
        data->ch6_mdiv = 60;
    }
    if (DNX_PLL4_TYPE_NIF < table->keys[0].size)
    {
        data = (dnx_data_pll_pll4_config_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_PLL4_TYPE_NIF, 0);
        data->pll4_id = 2;
        data->valid = 1;
        data->ref_clk_src_sel = 1;
        data->ref_clk = 156;
        data->dll_ref_pdiv = 1;
        data->dll_post_en = 15;
        data->aux_post_en = 15;
        data->pdiv = 1;
        data->ndiv_int = 32;
        data->d2c_en = 1;
        data->ch0_mdiv = 50;
        data->ch2_mdiv = 25;
        data->ch3_mdiv = 4;
    }
    if (DNX_PLL4_TYPE_FAB < table->keys[0].size)
    {
        data = (dnx_data_pll_pll4_config_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_PLL4_TYPE_FAB, 0);
        data->pll4_id = 3;
        data->valid = 1;
        data->ref_clk_src_sel = 1;
        data->ref_clk = 156;
        data->dll_ref_pdiv = 1;
        data->dll_post_en = 15;
        data->aux_post_en = 13;
        data->pdiv = 1;
        data->ndiv_int = 29;
        data->d2c_en = 1;
        data->ch0_mdiv = 5;
        data->ch1_mdiv = 29;
    }
    if (DNX_PLL4_TYPE_NIF_TSC < table->keys[0].size)
    {
        data = (dnx_data_pll_pll4_config_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_PLL4_TYPE_NIF_TSC, 0);
        data->pll4_id = 4;
        data->valid = 1;
        data->ref_clk_src_sel = 1;
        data->ref_clk = 156;
        data->dll_ref_pdiv = 1;
        data->dll_post_en = 15;
        data->aux_post_en = 15;
        data->pdiv = 1;
        data->ndiv_frac_mode = 1;
        data->ndiv_p = 32;
        data->ndiv_q = 125;
        data->ndiv_int = 32;
        data->d2c_en = 1;
        data->ch0_mdiv = 3;
    }
    if (DNX_PLL4_TYPE_FRAMER < table->keys[0].size)
    {
        data = (dnx_data_pll_pll4_config_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_PLL4_TYPE_FRAMER, 0);
        data->pll4_id = 5;
        data->valid = 1;
        data->ref_clk_src_sel = 1;
        data->ref_clk = 155;
        data->dll_ref_pdiv = 1;
        data->dll_post_en = 15;
        data->aux_post_en = 15;
        data->pdiv = 1;
        data->ndiv_frac_mode = 1;
        data->ndiv_p = 17;
        data->ndiv_q = 27;
        data->ndiv_int = 29;
        data->d2c_en = 1;
        data->ch0_mdiv = 4;
    }
    if (DNX_PLL4_TYPE_SAR < table->keys[0].size)
    {
        data = (dnx_data_pll_pll4_config_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_PLL4_TYPE_SAR, 0);
        data->pll4_id = 6;
        data->valid = 1;
        data->ref_clk_src_sel = 1;
        data->ref_clk = 155;
        data->dll_ref_pdiv = 1;
        data->dll_post_en = 15;
        data->aux_post_en = 15;
        data->pdiv = 1;
        data->ndiv_int = 28;
        data->d2c_en = 1;
        data->ch0_mdiv = 6;
        data->ch1_mdiv = 14;
    }

    /* Override data with dynamic data (if required) */
    SHR_IF_ERR_EXIT(dnxc_data_dyn_loader_table_value_set(unit, table));


exit:
    SHR_FUNC_EXIT;
}

/*
 * Submodule: synce_pll
 */

/*
 * Features
 */
/**
 * \brief Pointer to function (per device) which set values for feature
 * 
 * \param [in] unit - Unit #
 * 
 * \return
 *     err - see shr_error_e
 * \remark
 *   * Auto-generated using device data autocoder
 * \see
 *   * None
 */
static shr_error_e
j2x_a0_dnx_data_pll_synce_pll_present_set(
    int unit)
{
    dnxc_data_feature_t *feature;
    int module_index = dnx_data_module_pll;
    int submodule_index = dnx_data_pll_submodule_synce_pll;
    int feature_index = dnx_data_pll_synce_pll_present;
    SHR_FUNC_INIT_VARS(unit);

    feature = &_dnxc_data[unit].modules[module_index].submodules[submodule_index].features[feature_index];
    /* Set default value */
    feature->default_data = 0;

    /* Set value */
    feature->data = 0;

    /* Set data flags as supported */
    feature->flags |= DNXC_DATA_F_SUPPORTED;

    /* Override data with dynamic data (if required) */
    SHR_IF_ERR_EXIT(dnxc_data_dyn_loader_feature_value_set(unit, feature));

exit:
    SHR_FUNC_EXIT;
}

/*
 * Defines
 */
/*
 * Tables
 */
/*
 * Device attach func
 */
/**
 * \brief Attach device to module - attach set function to data structure
 * 
 * \param [in] unit - Unit #
 * 
 * \return
 *     err - 
 * \remark
 *   * Auto-generated using device data autocoder
 * \see
 *   * None
 */
shr_error_e
j2x_a0_data_pll_attach(
    int unit)
{
    dnxc_data_module_t *module = NULL;
    dnxc_data_submodule_t *submodule = NULL;
    dnxc_data_define_t *define = NULL;
    dnxc_data_feature_t *feature = NULL;
    dnxc_data_table_t *table = NULL;
    int module_index = dnx_data_module_pll;
    int submodule_index = -1, data_index = -1;
    SHR_FUNC_INIT_VARS(unit);

    COMPILER_REFERENCE(define);
    COMPILER_REFERENCE(feature);
    COMPILER_REFERENCE(table);
    COMPILER_REFERENCE(submodule);
    COMPILER_REFERENCE(data_index);
    COMPILER_REFERENCE(submodule_index);
    module = &_dnxc_data[unit].modules[module_index];
    /*
     * Attach submodule: general
     */
    submodule_index = dnx_data_pll_submodule_general;
    submodule = &module->submodules[submodule_index];

    /*
     * Attach defines: 
     */

    /*
     * Attach features: 
     */
    data_index = dnx_data_pll_general_ts_freq_lock;
    feature = &submodule->features[data_index];
    feature->set = j2x_a0_dnx_data_pll_general_ts_freq_lock_set;

    /*
     * Attach tables: 
     */
    data_index = dnx_data_pll_general_table_nif_pll;
    table = &submodule->tables[data_index];
    table->set = j2x_a0_dnx_data_pll_general_nif_pll_set;
    data_index = dnx_data_pll_general_table_fabric_pll;
    table = &submodule->tables[data_index];
    table->set = j2x_a0_dnx_data_pll_general_fabric_pll_set;
    /*
     * Attach submodule: pll1
     */
    submodule_index = dnx_data_pll_submodule_pll1;
    submodule = &module->submodules[submodule_index];

    /*
     * Attach defines: 
     */
    data_index = dnx_data_pll_pll1_define_nof_pll1;
    define = &submodule->defines[data_index];
    define->set = j2x_a0_dnx_data_pll_pll1_nof_pll1_set;

    /*
     * Attach features: 
     */

    /*
     * Attach tables: 
     */
    /*
     * Attach submodule: pll3
     */
    submodule_index = dnx_data_pll_submodule_pll3;
    submodule = &module->submodules[submodule_index];

    /*
     * Attach defines: 
     */
    data_index = dnx_data_pll_pll3_define_nof_pll3;
    define = &submodule->defines[data_index];
    define->set = j2x_a0_dnx_data_pll_pll3_nof_pll3_set;

    /*
     * Attach features: 
     */

    /*
     * Attach tables: 
     */
    /*
     * Attach submodule: pll4
     */
    submodule_index = dnx_data_pll_submodule_pll4;
    submodule = &module->submodules[submodule_index];

    /*
     * Attach defines: 
     */
    data_index = dnx_data_pll_pll4_define_nof_pll4;
    define = &submodule->defines[data_index];
    define->set = j2x_a0_dnx_data_pll_pll4_nof_pll4_set;
    data_index = dnx_data_pll_pll4_define_ts_pll4_id;
    define = &submodule->defines[data_index];
    define->set = j2x_a0_dnx_data_pll_pll4_ts_pll4_id_set;
    data_index = dnx_data_pll_pll4_define_bs_pll4_id;
    define = &submodule->defines[data_index];
    define->set = j2x_a0_dnx_data_pll_pll4_bs_pll4_id_set;
    data_index = dnx_data_pll_pll4_define_nif_pll4_id;
    define = &submodule->defines[data_index];
    define->set = j2x_a0_dnx_data_pll_pll4_nif_pll4_id_set;
    data_index = dnx_data_pll_pll4_define_fab_pll4_id;
    define = &submodule->defines[data_index];
    define->set = j2x_a0_dnx_data_pll_pll4_fab_pll4_id_set;
    data_index = dnx_data_pll_pll4_define_nif_tsc_pll4_id;
    define = &submodule->defines[data_index];
    define->set = j2x_a0_dnx_data_pll_pll4_nif_tsc_pll4_id_set;
    data_index = dnx_data_pll_pll4_define_framer_pll4_id;
    define = &submodule->defines[data_index];
    define->set = j2x_a0_dnx_data_pll_pll4_framer_pll4_id_set;
    data_index = dnx_data_pll_pll4_define_sar_pll4_id;
    define = &submodule->defines[data_index];
    define->set = j2x_a0_dnx_data_pll_pll4_sar_pll4_id_set;

    /*
     * Attach features: 
     */
    data_index = dnx_data_pll_pll4_pll_in_frac_mode;
    feature = &submodule->features[data_index];
    feature->set = j2x_a0_dnx_data_pll_pll4_pll_in_frac_mode_set;

    /*
     * Attach tables: 
     */
    data_index = dnx_data_pll_pll4_table_config;
    table = &submodule->tables[data_index];
    table->set = j2x_a0_dnx_data_pll_pll4_config_set;
    /*
     * Attach submodule: synce_pll
     */
    submodule_index = dnx_data_pll_submodule_synce_pll;
    submodule = &module->submodules[submodule_index];

    /*
     * Attach defines: 
     */

    /*
     * Attach features: 
     */
    data_index = dnx_data_pll_synce_pll_present;
    feature = &submodule->features[data_index];
    feature->set = j2x_a0_dnx_data_pll_synce_pll_present_set;

    /*
     * Attach tables: 
     */

    SHR_FUNC_EXIT;
}
#undef BSL_LOG_MODULE
/* *INDENT-ON* */
