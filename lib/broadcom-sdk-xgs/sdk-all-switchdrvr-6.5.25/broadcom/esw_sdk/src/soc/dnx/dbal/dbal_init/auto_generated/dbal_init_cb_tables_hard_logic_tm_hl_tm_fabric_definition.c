/** \file dbal_init_cb_tables_hard_logic_tm_hl_tm_fabric_definition.c
 * Logical DB enums \n 
...
{
    dbal_entry_handle_t *  eh = (dbal_entry_handle_t *) entry_handle;
    uint32 key_value__fabric_load_balancing_context;
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FORMULA_CB_GET_KEY_VALUE(unit, eh, DBAL_FIELD_FABRIC_LOAD_BALANCING_CONTEXT, key_value__fabric_load_balancing_context);
    *offset = key_value__fabric_load_balancing_context;
    SHR_EXIT();
exit:
    SHR_FUNC_EXIT;
}
...
uint32 * offset)
{
    uint32 result_size__weight = 0;
    uint32 instance_idx;
    SHR_FUNC_INIT_VARS(unit);
    result_size__weight += 7;
    DBAL_FORMULA_CB_GET_INSTANCE_IDX(unit, current_mapped_field_id, instance_idx);
    *offset = result_size__weight*instance_idx;
    SHR_EXIT();
exit:
    SHR_FUNC_EXIT;
}
