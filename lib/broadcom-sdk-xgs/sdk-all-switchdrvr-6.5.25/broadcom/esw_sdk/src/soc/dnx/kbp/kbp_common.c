...
/*
 * Utility macro for checking and updating the indirect commit mode that is returned back to the DBAL layer.
 * The only purpose of this marco is to minimize the code footprint and make it more readable.
 * The purpose of the code is to update the commit mode from FORCE to NORMAL or UPDATE in order for DBAL code to correctly
 * increment the table entry counter for new entries and not increment it for updated entries.
 */
    ...
    uint32 *op2_payload_size)
{
    ...
    if (op2_payload_size && op2_kbp_handles)
    {
        ...
        {
            /** Payload is retrieved from large optimized AD DB */
...
/*
 * Verify current KBP state and the provided DBAL table ID.
 * Returns the related KBP DB ID and the KBP handles info.
 */
static shr_error_e
...
/**
 * \brief
 *   Clear all cached entries from the DB, which corresponds to the provided handles.
 *   \param [in] unit - Relevant unit.
 *   \param [in] op2_kbp_handles - The provided DB handles.
 *  \return
    ...
    uint8 is_rollback)
{
    ...
    /** Delete all DB entries that are cached and not installed. */
    ...
    dbal_tables_e dbal_table_id)
{
    ...
    {
        /** Perform cache commit. In case of error, caching indication still needs to be set to disabled. */
        SHR_SET_CURRENT_ERR(dnx_kbp_cache_db_commit_internal(unit, op2_kbp_handles));
        /** Set caching indication to disabled */
    ...
    uint32 nof_entries_to_clear)
{
    ...
    SHR_IF_ERR_EXIT(dnx_kbp_cache_db_verify(unit, dbal_table_id, &db_id, &op2_kbp_handles));
    /** Perform cache clear. In case of error, caching indication still needs to be set to disabled. */
    SHR_SET_CURRENT_ERR(dnx_kbp_cache_db_clear_internal(unit, op2_kbp_handles, FALSE));
    /** Set caching indication to disabled */
    SHR_IF_ERR_EXIT(KBP_ACCESS.db_handles_info.caching_enabled.set(unit, db_id, FALSE));
    /** Update the DBAL entry counter */
