 ...
 *   The TCAM access layer is responsible for writing/reading raw data to TCAM HW.
 */
/**
...
/**
 * \brief
 *  Removes entry (identified by the given key/mask/position) from SW hash
 *
 * \param [in] unit     - Device ID
 * \param [in] core     - Core ID
 * \param [in] key_size - Size of entry's key
 * \param [in] position - The index to delete from hash
 *
 * \return
 *  \retval _SHR_E_NONE - success
 */
...
/**
 * \brief
 *  Writes the given encoded key and key_mask to the given position.
 *  Doesn't update shadow
 *
 * \param [in] unit        - Device ID
 ...
 * \param [in] bank_offset - The offset inside the TCAM_BANK to read from, by counting Single sized entries.
 *                           Equal to the TCAM bank offset, twice the action bank offset.
 * \param [in] valid_bits  - The valid bit
 * \param [in] key         - The key buffer to write
 * \param [in] mask        - The key_mask buffer to write
 * \param [in] payload     - The payload buffer to write
 *
 * \return
 *   \retval _SHR_E_NONE success
 *
...
/**
 * \brief
 *   Copies line content of 'line_from' address to 'line_to' address
 *
 * \param [in] unit        - Device ID
 * \param [in] core        - Core ID
 * \param [in] line_from   - Address of line to copy from
 * \param [in] line_to     - Address of line to copy to
 *
 * \return
 *  \retval _SHR_E_NONE - success
 */
 ...
 * \param [in] unit              - Device ID
 * \param [in] core              - Core ID
 * \param [in] app_id            - The app ID that the entry belongs to
 * \param [in] bank_id           - The TCAM_BANK ID to read from
 * \param [in] bank_offset       - The offset inside the TCAM_BANK to read from
 ...
 * \param [in] part              - Which parts of the key should be written
 * \param [in] valid             - Specifies whether entry is valid or not
 * \param [in] is_move_operation - TRUE when this function is called as part of a move procedure
 *
 * \return
 *  \retval _SHR_E_NONE - success
 */
    ...
    if (part != DNX_FIELD_TCAM_ACCESS_PART_ALL)
    {
        ...
        if (entry_in_use)
        {
                ...
                && !dnx_data_field.tcam.feature_get(unit, dnx_data_field_tcam_tcam_80b_traffic_safe_write))
            {
                ...
                /*
                 * Make a copy of line at position
                 */
 ...
 * \param [in] unit              - Device ID
 * \param [in] core              - Core ID
 * \param [in] app_id            - The handler ID that the entry belongs to
 * \param [in] bank_id           - The TCAM_BANK ID to read from
 * \param [in] bank_offset       - The offset inside the TCAM_BANK to read from
 * \param [in] key               - Buffer of the key to write
 * \param [in] key_mask          - Buffer of the key mask to write
 ...
 * \param [in] unit        - Device ID
 * \param [in] core        - Core ID
 * \param [in] app_id      - Handler ID that the entry to invalidate belongs to
 * \param [in] position    - The position to invalidate
 * \param [in] part        - Part to invalidate
 * \param [in] is_special  - Whether the invalidate operation is for the special (single_key/half_payload) DB or not
 *
 * \return
 *   \retval _SHR_E_NONE success
 *
    ...
    uint32 flags)
{
    ...
    /*
     * Priority for TCAM is actually passed using this "prefix_length" param 
     */
    ...
    /*
     * entry->age actually indicates whether to INVALIDATE/VALIDATE an entry. TRUE when invalidation is
     * requested
     */
 ...
 *   This function uses SWSTATE structs to search for the key by hashing the key and looking
 *   for the hash in a hash_table.
 *
 * \param [in] unit       - unit
 * \param [in] core       - Core ID (should be specific)
 ...
 * \param [in] is_move_operation - True when this function is being invoked during entry move operation
 *
 * \return
 *  \retval _SHR_E_NONE - success
 */
    ...
    uint8 is_move_operation)
{
    ...
    {
        /**
         * Delete entry from SW hash (not performed during move, since we already delete key from hash before re-adding it at new position)
         */
        SHR_IF_ERR_EXIT(dnx_field_tcam_access_sw_hash_key_remove(unit, core, db_info.key_size, position));
    }
    ...
    uint32 flags)
{
    ...
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core_iter)
    {
        ...
        {
            SHR_IF_ERR_EXIT(dnx_field_tcam_access_entry_delete_aux(unit, core_iter, app_id, position, FALSE     /* is_move_operation 
                                                                                                                 */ ));
    ...
    /*
     *  If entry delete is invoked while cache installation is in progress then it means that an error
     *  has occured, and that entry_delete is called to clear entries from HW, but we shouldn't free access IDs
     *  as entries will still be kept in cache.
     */
    SHR_IF_ERR_EXIT(dnx_field_tcam_access_sw.cache_install_in_progress.get(unit, &cache_install_in_progress));
    ...
    uint32 position_to)
{
    ...
    {
        /**
         *  Delete entry from SW hash before moving the entry
         */
