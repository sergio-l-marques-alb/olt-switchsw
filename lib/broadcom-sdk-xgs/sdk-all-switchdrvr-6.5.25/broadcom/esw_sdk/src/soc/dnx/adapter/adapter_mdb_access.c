...
/* Logical memory for adapter implementation */
...
/**
 * \brief - calculate cmic HW block representation of MDB block
 */
static int
...
/**
 * Write everything to the buffer
 * This buffer configuration is for isem1a only
 *
 * write_index is used both for input and outputs
 */
shr_error_e
    ...
    uint8 buffer[ADAPTER_MDB_ACCESS_MAX_BUFFER_SIZE_U8])
{
    ...
    /*
     * In case of GLEM in ERPP lookup payload doesn't include vmv 
     */
    ...
    dbal_physical_entry_t * entry)
{
    ...
    if (dnx_data_mdb.pdbs.dbal_pdb_info_get(unit, dbal_physical_table_id)->db_subtype == MDB_EM_TYPE_GLEM)
    {
        ...
        /*
         * in case we are using MDB stub with two interfaces, we need to broadcast the entry to third GLEM intf as it exists in the adapter.
         * since we broadcast in the DBAL layer but only to two GLEM interfaces, writing to GLEM2 will also write to GLEM3
         */
