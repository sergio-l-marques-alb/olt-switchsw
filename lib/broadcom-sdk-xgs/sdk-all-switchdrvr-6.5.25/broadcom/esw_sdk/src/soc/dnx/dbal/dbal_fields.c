    ...
    uint32 *sw_field_val)
{
    ...
    if (ii == nof_enum_vals)
    {
        /** no matching enum value found - return error */
    ...
    if (ret_val == _SHR_E_UNAVAIL)
    {
      ...
      *  for key fields also need to set the handle status to ignore the action TBD. */
        ...
        if (entry_handle && is_key)
        {
             ...
             *  the handle error is updated  to _SHR_E_UNAVAIL and the field Id to the not mapped key field id. (this happen outside
...
/** returns the sub_field_info for a specific field, if not found returns e_not_found */
shr_error_e
    ...
    uint32 *sub_field_val)
{
    ...
    /** looking for subfield that fits to the HW value, meaning that decoding and encoding will return the same value.. */
