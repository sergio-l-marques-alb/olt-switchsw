...
/** 
 *  \brief
 *  after initialized holds the start indexes in dbal_field_strings[] of each alphabet letter
 ...
 * also need the update corresponding parm1
    ...
    uint8 *inst_idx)
{
    ...
    while (str[ii] != 0)
    {
        ...
        {
            /*
             * ++ii to point after the # and get the index 
             */
            *inst_idx = sal_atoi(&str[++ii]);
            /*
             * (ii - 2) for ending string before _# 
             */
            str[ii - 2] = 0;
            break;
        }
...
/** \brief
 *  initialize the accelerator for dbal_field_string_to_id, update for string the related offsets in the
