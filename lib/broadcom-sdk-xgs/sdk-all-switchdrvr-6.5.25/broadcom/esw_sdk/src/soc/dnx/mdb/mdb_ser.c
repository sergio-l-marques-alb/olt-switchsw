    ...
    mdb_physical_table_e mdb_phy_table_id)
{
    ...
    if (mdb_phy_table_id < MDB_NOF_PHYSICAL_TABLES)
    {
        return dnx_data_mdb.pdbs.mdb_pdb_info_get(unit, mdb_phy_table_id)->name;
    }
    ...
    mdb_physical_table_e * mdb_phy_table_id)
{
    ...
    for (ii = 0; ii < MDB_NOF_PHYSICAL_TABLES; ii++)
    {
        mdb_table_name = mdb_physical_table_to_string(unit, ii);
        if (sal_strncasecmp(mdb_table_name, str, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
        {
            *mdb_phy_table_id = (mdb_physical_table_e) ii;
            SHR_EXIT();
        }
    }
    ...
    int detail)
{
    ...
    dbal_physical_table_id = dnx_data_mdb.pdbs.mdb_pdb_info_get(unit, mdb_phy_table_id)->physical_to_logical;
    ...
    db_type = dnx_data_mdb.pdbs.mdb_pdb_info_get(unit, mdb_phy_table_id)->db_type;
    ...
    if (detail)
    {
        for (cluster_idx = 0; cluster_idx < nof_clusters; cluster_idx++)
        {
            ...
            sal_snprintf(str_info, sizeof(str_info), "%s-%d-%d",
                         ...
                         cluster_info.macro_index, cluster_info.cluster_index);
    ...
    int unit)
{
    ...
    {
        macro_idx_max = dnx_data_mdb.dh.macro_type_info_get(unit, macro_type_iter)->nof_macros;
        cli_out("MDB_%s: macro_idx_max=%d\n", dnx_data_mdb.dh.macro_type_info_get(unit, macro_type_iter)->name,
                macro_idx_max);
        for (macro_idx = 0; macro_idx < macro_idx_max; macro_idx++)
        {
            SHR_IF_ERR_EXIT(mdb_direct_calc_macro_mem_blk(unit, macro_type_iter, macro_idx, &mem, NULL, NULL, &blk));
    ...
    uint32 *mem_index)
{
    ...
    SHR_IF_ERR_EXIT(mdb_dh_get_cluster_granularity(unit, macro_type, macro_index, cluster_idx, &cluster_granularity));
    ...
    *mem_index = row_idx & MDB_SER_ADDR_OFFSET_MASK(unit, macro_type);
    ...
    for (cluter_iter = 0; cluter_iter < cluster_granularity; cluter_iter++)
    {
        int cluster_index = cluster_off - (cluster_off % cluster_granularity) + cluter_iter;

         
        SHR_IF_ERR_EXIT(utilex_bitstream_reset_bit_range
                        (mem_index,
                         MDB_SER_ADDR_OFFSET_OFFSET(unit, macro_type) + MDB_SER_ADDR_OFFSET_BITS * cluster_index,
                         MDB_SER_ADDR_OFFSET_OFFSET(unit, macro_type) + MDB_SER_ADDR_OFFSET_BITS * cluster_index + 1));
    }
    ...
    uint32 ecc)
{
    ...
    SHR_IF_ERR_EXIT(mdb_dh_get_cluster_granularity
                    (unit, macro_type, macro_index,
                     (bucket_idx * dnx_data_mdb.dh.nof_bucket_clusters_get(unit)) + cluster_off, &cluster_granularity));
    ...
    uint32 enable)
{
    ...
    {
        mdb_scan_conf_reg = MDB_REG_174r;
        mdb_scan_pause_field = ITEM_0_0f;
    }
    else
    {
    ...
    for (ii = 0; ii < row_index_max; ii++)
    {
        ...
        if (utilex_bitstream_have_one_in_range(entry_data, 0, MDB_SER_ENTRY_DATA_SIZE_IN_BITS - 1))
        {
            sal_snprintf(print_msg, sizeof(print_msg), "    %s-%d, cluster=%d, index=%-6d: ",
                         dnx_data_mdb.dh.macro_type_info_get(unit, macro_type)->name, macro_idx, cluster_idx, ii);
    ...
    uint32 cluster_idx)
{
    ...
    SHR_IF_ERR_EXIT(mdb_ser_map_cluster_to_dbal_phy_table
                    (unit, macro_type, macro_idx, cluster_idx, &dbal_id, &db_type));
    ...
    cli_out("    %s, index-%d, cluster-%d, dbal-table=%s, db_type=%s\n",
            dnx_data_mdb.dh.macro_type_info_get(unit, macro_type)->name,
    ...
    dbal_enum_value_field_mdb_db_type_e * db_type)
{
    ...
    SHR_IF_ERR_EXIT(mdb_init_get_cluster_assoc(unit, macro_type, macro_index, cluster_index, &mdb_physical_table_id));
    ...
    if (mdb_physical_table_id != MDB_NOF_PHYSICAL_TABLES)
    {
        ...
        *db_type = dnx_data_mdb.pdbs.mdb_pdb_info_get(unit, mdb_physical_table_id)->db_type;
    }
    else
    {
        *dbal_physical_table_id = DBAL_NOF_PHYSICAL_TABLES;
        *db_type = DBAL_NOF_ENUM_MDB_DB_TYPE_VALUES;
    }

    if (*dbal_physical_table_id == DBAL_NOF_PHYSICAL_TABLES)
    {
        LOG_VERBOSE(BSL_LOG_MODULE,
                    (BSL_META_U(unit, "Can't map macro_type %d macro_index %d, cluster_index %d to DBAL\n"), macro_type,
                     macro_index, cluster_index));
    }
    ...
    uint32 entry_idx)
{
    ...
    for (sp_index = 0; sp_index < sp_banks_num; sp_index++)
    {
        ...
        {
            rc = mdb_lpm_cache_big_bb_get(unit, macro_type, macro_index, cluster_index, correct_index, 0, entry_data);
        }
        else
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "ERROR: Unrecognized db_type (%d) for direct/eedb/lpm db 2b sync.\n", db_type);
        }
...
mdb_ser_ecc_2b_em_sync_cache(
    ...
    dbal_physical_tables_e dbal_physical_table_id)
{
    uint32 iterator_app_id = MDB_APP_ID_ITER_ALL;
    ...
    if (dnx_data_mdb.pdbs.dbal_pdb_info_get(unit, dbal_physical_table_id)->row_width ==
        (2 * DNX_DATA_MAX_MDB_DH_CLUSTER_ROW_WIDTH_BITS))
    {
    ...
    SHR_IF_ERR_EXIT(mdb_em_iterator_init(unit, dbal_physical_table_id, iterator_app_id, &physical_entry_iterator));
    iterator_init = TRUE;

    SHR_IF_ERR_EXIT(mdb_em_iterator_get_next
                    (unit, dbal_physical_table_id, iterator_app_id, &physical_entry_iterator, &entry, &is_end));
    while (is_end == FALSE)
    {
        int entry_app_id = physical_entry_iterator.app_id;
        ...
        if (severity >= bslSeverityVerbose)
        {
            cli_out("mdb_ser_ecc_2b_em_sync_cache: dbal_physical_table is %s, app_id=%d\n",
                    dbal_physical_table_to_string(unit, dbal_physical_table_id), entry_app_id);
            mdb_ser_dump_dbal_em_entry(&entry);
        }

        entry.mdb_action_apply = DBAL_MDB_ACTION_APPLY_HW_ONLY;
        sal_memcpy(&entry_get, &entry, sizeof(entry));
        rc = mdb_em_entry_get(unit, dbal_physical_table_id, entry_app_id, &entry_get, 0);
        if (rc == _SHR_E_NOT_FOUND)
        {
            cli_out("EM entry was not found at HW, re-write it: dbal_physical_table is %s, app_id=%d\n",
                    dbal_physical_table_to_string(unit, dbal_physical_table_id), entry_app_id);
            SHR_IF_ERR_EXIT(mdb_ser_em_entry_add(unit, dbal_physical_table_id, entry_app_id, &entry));

            
            if (dnx_data_mdb.em.feature_get(unit, dnx_data_mdb_em_entry_counter_decrease_support))
            {
                SHR_IF_ERR_EXIT(mdb_em_hw_entry_count_decrease(unit, dbal_physical_table_id));
            }
        }
...
exit:
    ...
    {
        SHR_IF_ERR_CONT(mdb_em_iterator_deinit
                        (unit, dbal_physical_table_id, iterator_app_id, &physical_entry_iterator));
    }
    SHR_FUNC_EXIT;
}
    ...
    uint32 entry_idx)
{
    ...
    SHR_IF_ERR_EXIT(mdb_ser_map_cluster_to_dbal_phy_table
                    (unit, macro_type, macro_index, cluster_index, &dbal_physical_table_id, &db_type));
    if (dbal_physical_table_id == DBAL_NOF_PHYSICAL_TABLES)
    {
    ...
    if ((db_type == DBAL_ENUM_FVAL_MDB_DB_TYPE_DIRECT)
        || (db_type == DBAL_ENUM_FVAL_MDB_DB_TYPE_EEDB) || (db_type == DBAL_ENUM_FVAL_MDB_DB_TYPE_KAPS))
    {
        SHR_IF_ERR_EXIT(mdb_ser_ecc_2b_direct_sync_cache(unit, db_type, blk, mem, array_idx, entry_idx));
    }
    ...
    else if (db_type == DBAL_ENUM_FVAL_MDB_DB_TYPE_TCAM)
    {
