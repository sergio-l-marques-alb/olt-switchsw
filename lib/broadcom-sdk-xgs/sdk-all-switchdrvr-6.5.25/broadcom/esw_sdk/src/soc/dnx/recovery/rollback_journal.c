 ...
 */
static dnx_rollback_journal_pool_t
    dnx_rollback_journal_pool[SOC_MAX_NUM_DEVICES][DNX_ERR_RECOVERY_MAX_NOF_ACTIVE_TRANSACTION];

/**
 * \brief - flag that determines if logging is enabled (relevant for all threads).
 */
static uint8 dnx_rollback_journal_logging_enabled[SOC_MAX_NUM_DEVICES][DNX_ROLLBACK_JOURNAL_JOURNAL_MAX_NUM];

/**
 * \brief - flag that determines if logging init restriction (logging is generally disallowed at init) is bypassed (relevant for all threads).
 */
 ...
 */
#define DNX_ROLLBACK_JOURNAL_VALIDATE_HANDLE(_handle, _er_thread_id)\
do {\
    if((DNX_ROLLBACK_JOURNAL_INVALID_HANDLE == (_handle))\
     ||((_handle) >= DNX_ROLLBACK_JOURNAL_JOURNAL_MAX_NUM)\
     ||(NULL == dnx_rollback_journal_pool[unit][_er_thread_id].entries[(_handle)])) {\
 ...
 */
#define DNX_ROLLBACK_JOURNAL_GET_JOURNAL_PTR(_unit, _er_thread_id, journal_ptr, _handle)   \
do {                                                                  \
    DNX_ROLLBACK_JOURNAL_VALIDATE_HANDLE((_handle), _er_thread_id);                 \
    journal_ptr = dnx_rollback_journal_pool[_unit][_er_thread_id].entries[(_handle)];\
} while(0)

#ifdef DNX_ROLLBACK_JOURNAL_OPT_CODE
/**
 * \brief - get the 8 bytes aligned size
 */
#define DNX_ROLLBACK_JOURNAL_GET_ALIGNED_SIZE(size) ((size+0x7)&~0x7)
#else
#define DNX_ROLLBACK_JOURNAL_GET_ALIGNED_SIZE(size) (size)
#endif /* DNX_ROLLBACK_JOURNAL_OPT_CODE */

/**
 * \brief - get the size of dnx_rollback_journal_mem_chunk_t struct
 */
...
/**
 * \brief - get the size of dnx_rollback_journal_entry_t struct
 */
    ...
    dnx_rollback_journal_entry_t ** entry)
{
    ...
    *entry = NULL;
    ...
    /*
     * Pad entry to avoid misalignments in memory
     */
    ...
    dnx_rollback_journal_handle_t handle)
{
    ...
    if ((handle >= DNX_ROLLBACK_JOURNAL_JOURNAL_MAX_NUM)
        || (NULL == dnx_rollback_journal_pool[unit][er_thread_id].entries[handle])
        || (NULL == dnx_rollback_journal_pool[unit][er_thread_id].entries[handle]->cbs.is_on))
    {
        return FALSE;
    }
     ...
     */
    if (dnx_rollback_journal_pool[unit][er_thread_id].entries[handle]->is_journaled_disabled_counter > 0)
    {
        return FALSE;
    }

    return dnx_rollback_journal_pool[unit][er_thread_id].entries[handle]->cbs.is_on(unit, er_thread_id);
}
    ...
    dnx_rollback_journal_print_handler print_handler)
{
     ...
     */
    DNX_ROLLBACK_JOURNAL_GET_JOURNAL_PTR(unit, er_thread_id, journal, handle);
    ...
    entry->seq_id = (NULL == journal->cbs.seq_id_get) ? 0 : journal->cbs.seq_id_get(unit, er_thread_id);
    entry->stamp = NULL;
    entry->stamp_size = 0;
    ...
    dnx_rollback_journal_handle_t * handle)
{
    ...
    *handle = DNX_ROLLBACK_JOURNAL_INVALID_HANDLE;
    ...
    if (dnx_rollback_journal_pool[unit][er_thread_id].count >= DNX_ROLLBACK_JOURNAL_JOURNAL_MAX_NUM)
    {
        SHR_IF_ERR_EXIT_WITH_LOG(SOC_E_INTERNAL,
    ...
    DNX_ROLLBACK_JOURNAL_ALLOC(unit,
                               dnx_rollback_journal_pool[unit][er_thread_id].entries[dnx_rollback_journal_pool[unit]
                                                                                     [er_thread_id].count],
                               dnx_rollback_journal_t *, sizeof(dnx_rollback_journal_t), "rollback journal alloc", type,
                               subtype);

    
    journal =
        dnx_rollback_journal_pool[unit][er_thread_id].entries[dnx_rollback_journal_pool[unit][er_thread_id].count];
    ...
    dnx_rollback_journal_logging_enabled[unit][dnx_rollback_journal_pool[unit][er_thread_id].count] = FALSE;
     ...
     */
    *handle = dnx_rollback_journal_pool[unit][er_thread_id].count++;
    ...
    dnx_rollback_journal_handle_t * handle)
{
    ...
    {
        DNX_ROLLBACK_JOURNAL_VALIDATE_HANDLE(*handle, er_thread_id);

        /*
         * clear all entries in the journal
         */
        SHR_IF_ERR_EXIT(dnx_rollback_journal_clear(unit, er_thread_id, FALSE, *handle));
        sal_free(dnx_rollback_journal_pool[unit][er_thread_id].entries[*handle]);
        dnx_rollback_journal_pool[unit][er_thread_id].entries[*handle] = NULL;

        dnx_rollback_journal_pool[unit][er_thread_id].count--;
    }
    ...
    uint8 is_unsafe)
{
     ...
     */
    if (NULL == dnx_rollback_journal_pool[unit][er_thread_id].entries[handle])
    {
        SHR_EXIT();
    }
    ...
    if (!is_unsafe
        && (NULL == dnx_rollback_journal_pool[unit][er_thread_id].entries[handle]->cbs.is_on
            || !dnx_rollback_journal_pool[unit][er_thread_id].entries[handle]->cbs.is_on(unit, er_thread_id)))
    {
        SHR_EXIT();
    }
    ...
    dnx_rollback_journal_handle_t handle)
{
     ...
     */
    dnx_rollback_journal_pool[unit][er_thread_id].is_any_journal_rolling_back = TRUE;
    ...
    while (journal->entry_counter > 0)
    {
        ...
        if (DNX_ROLLBACK_JOURNAL_ENTRY_STATE_IS_VALID(entry->state))
        {
            rv_rollback = entry->rollback_handler(unit, er_thread_id, metadata, payload, entry->stamp);
    ...
    dnx_rollback_journal_pool[unit][er_thread_id].is_any_journal_rolling_back = FALSE;
...
exit:
    if (dnx_rollback_journal_pool[unit][er_thread_id].is_any_journal_rolling_back)
    {
        dnx_rollback_journal_pool[unit][er_thread_id].is_any_journal_rolling_back = FALSE;
    }
    SHR_FUNC_EXIT;
    ...
    dnx_rollback_journal_is_distinct_cb is_distinct_cb)
{
    ...
    while (entry != NULL)
    {
        ...
        while (trav_entry != NULL)
        {
             ...
             */
            SHR_IF_ERR_EXIT(dnx_rollback_journal_entry_get_merged_metadata_payload_ptr(unit, journal, trav_entry,
...
dnx_rollback_journal_is_any_journal_rolling_back(
    ...
    int er_thread_id)
{
    return dnx_rollback_journal_pool[unit][er_thread_id].is_any_journal_rolling_back;
}
    ...
    dnx_rollback_journal_handle_t handle)
{
    ...
    if (dnx_rollback_journal_is_on(unit, er_thread_id, handle))
    {
...
dnx_rollback_journal_get_current_alloc_memory(
    ...
    int er_thread_id)
{
    uint32 total_bytes_allocated = 0;
    if (er_thread_id != -1)
    {
        for (int idx = 0; idx < DNX_ROLLBACK_JOURNAL_JOURNAL_MAX_NUM; idx++)
        {
            if (dnx_rollback_journal_pool[unit][er_thread_id].entries[idx] != NULL)
            {
                total_bytes_allocated +=
                    dnx_rollback_journal_pool[unit][er_thread_id].entries[idx]->mem_pool.total_bytes_allocated;
            }
        }
    }
