...
#include <shared/dbx/dbx_xml.h> /* For RHDATA_* */
#include <shared/dbx/dbx_file.h>        /* For dbx_file_get_xml_top() */
#include <sal/core/libc.h>      /* For sal_strncmp */
#include <soc/dnxc/dnxc_regression_utils.h>

/*
 * \brief
 * stats_syntax.xml file path relative to dbx_file_get_xml_top() which starts from $SDK/tools/sand/db/
 */
...
/*
 * \brief
 * Max number of digits after the dot
 */
#define STATS_DECIMAL_PRECISION                 3

/*
 * \brief
 * Stat Family with dimension "Date" has special handling
 */
#define STATS_DATE                          "Date"

/**
 * \brief
 * Store Stat Family information parsed from stats_syntax.xml
 */
typedef struct
{
...
/*
 * \brief
 * Check input of single fill API
 */
static shr_error_e
...
/*
 * \brief
 * Check input of multiple fill API
 */
static shr_error_e
...
/*
 * \brief
 * Add a sample to a dimension in stat_family. If dimension is not in stat_family add it.
 */
static shr_error_e
...
/*
 * \brief
 * Remove trailing zeroes from a decimal number. E.g. "4.230" -> "4.23"
 */
static void
    ...
    stat_family_xml_t * stat_family_xml)
{
    ...
    /*
     * dbx_file_get_xml_top path is relative to $SDK/tools/sand/db/
     */
...
/*
 * \brief
 * Return 1 if Stat Family contains "Date" dimension otherwise return 0
 */
static int
...
/*
 * \brief
 * Check that the number of dimensions in stat_family is according to the Stat Family defined in the XML.
 */
static shr_error_e
...
/*
 * \brief
 * Check that nof samples equals across all dimensions.
 * If "Date" dimension only a single sample per dimension is allowed except for "Date" which should not be filled.
 */
static shr_error_e
...
/*
 * \brief
 * Fill stat_family dimensions_parsing_name according to dimension_parsing_name in stat_family_xml.
 * Validates that dimensions names in stat_family match dimension names in stat_family_xml.
 */
static shr_error_e
...
/*
 * \brief
 * Perform several validations of stat_family according to the Stat Family definition in xml.
 * Copy dimensions parsing name from stat_family_xml to stat_family
 */
static shr_error_e
...
/*
 * \brief
 * Commit data stored in stat_family to DB
 */
static void
stats_data_commit(
    int unit,
    arrakis_stat_family_t * stat_family)
{
    ...
    for (dim_idx = 0; dim_idx < stat_family->dimensions_nof; dim_idx++)
    {
        ...
        /**
         * dimension_str format: "<dimension name in DB><delimiter><first sample><delimiter><second sample>...".
         * E.g. "D 1 2 3 4"
         */
