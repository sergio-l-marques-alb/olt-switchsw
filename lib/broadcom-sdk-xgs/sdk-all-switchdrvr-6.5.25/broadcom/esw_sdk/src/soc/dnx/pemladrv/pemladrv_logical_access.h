/** \file src/soc/dnx/pemladrv/pemladrv_logical_access.h
 *
...
#define pemladrv_calloc(ARG, ARG2, ARG3)           sal_alloc(((ARG2)* (ARG3)), ""); \
...
#define pemladrv_calloc(ARG, ARG2, ARG3)      calloc((size_t)ARG2, ARG3)
...
typedef struct RegFieldMapper {
  /* name info for the sake of knowing the register name.*/
...
typedef struct AppletInfo {
  ...
  AppletRegMemInfo        applet_reg_info[PEM_CFG_API_NOF_CORES][2];   /*register info for trigerring applets writg*/
  AppletRegMemInfo        applet_mem_info[PEM_CFG_API_NOF_CORES][2];   /*memory to write applets to*/
  unsigned int            nof_pending_applets[PEM_CFG_API_NOF_CORES][2];    /*how many */
  unsigned int*           applet_mem_cache[PEM_CFG_API_NOF_CORES][2][APPLET_MEM_NOF_ENTRIES]; /*2 memories as one for ingress and one for ingress*/
  int                     entry_found_for_read_lately[PEM_CFG_API_NOF_CORES][2];     /*usually doing read-modify-write. maybe entry found in applets mem. if so keep the entry for faster access when writing*/
} AppletInfo;
...
int  start_writing_applets(int unit);                                           /*call when configuration through applets is desired*/
void flush_applets_to_single_mem(int unit,  const int core, const int is_ingress0_egress1);
int  flush_applets(int unit);                                                   /*call to flush the applets memory and write to PEM*/
