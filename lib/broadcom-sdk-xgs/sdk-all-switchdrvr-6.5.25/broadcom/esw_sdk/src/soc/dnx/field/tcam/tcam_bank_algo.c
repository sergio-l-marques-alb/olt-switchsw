 ...
 *
 * \param [in] unit         - Device ID
 * \param [in] fg_params    - FG paramerters for which bank is to be allocated for
 * \param [in] bank_id      - bank_id to be checked
 * \param [in] handler_id   - handler_id to be checked if it's suitable to be added to the given bank
 * \param [in] core_id      - core_id to be checked if it's suitable to be added to the given bank (Must be specified)
 * \param [out] can_be_used - TRUE if given bank can be used for given handler id, FALSE otherwise
 *
 * \return
 *  \retval _SHR_E_NONE  - Success
    ...
    uint8 *can_be_used)
{
    ...
    *can_be_used = TRUE;
    ...
    /*
     * Check if handler is already in bank and core
     */
    ...
    /*
     * Check if bank is not full
     * Skip odd banks availability check for double-sized key handlers
     */
    ...
    int *bank_id)
{
    ...
    if (fg_params.bank_allocation_mode != DNX_FIELD_TCAM_BANK_ALLOCATION_MODE_AUTO)
    {
         ...
         * i.e this function shouldnt be reached with mode != auto
         * */
        SHR_IF_ERR_EXIT_NO_MSG(_SHR_E_DISABLED);
    }

    /*
     * Iterater over all banks to check which is suitable to be used for given handler_id and core
     */
    for (*bank_id = 0; *bank_id < dnx_data_field.tcam.nof_banks_get(unit);)
    {
        ...
        /*
         * In case of DT allocation, bank_id must be the same for all cores
         */
    ...
    uint8 *contains_half_key_handler)
{
    ...
    /*
     * First assume no half key handler in bank 
     */
    *contains_half_key_handler = FALSE;
    /*
     * Iterate over the list to check if bank contains half-key handler
     */
    SHR_IF_ERR_EXIT(dnx_field_tcam_bank_sw.tcam_banks.active_handlers_id.get_first(unit, bank_id, core_id, &node));
    while (DNX_SW_STATE_LL_IS_NODE_VALID(node))
    {
