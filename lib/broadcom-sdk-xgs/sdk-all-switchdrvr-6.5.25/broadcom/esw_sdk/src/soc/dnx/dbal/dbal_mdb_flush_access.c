    ...
    dnx_l2_action_type_t l2_action_type)
{
    ...
    /** get the dbal physical table according to the l2 action type (fwd or learn) **/
 ...
 *
 * \param [in] unit - unit id
 * \param [in] l2_action_type - Indicates the L2 action type (fwd/learn), in order to determine the relevant flush
 *
 * \return
 *   shr_error_e
 *
...
/**
 * \brief - Calculate the flush buffer max size.
 *
 * \param [in] unit - unit id.
 * \param [in] *flush_buffer_entries_max - flush buffer size - used as output for the calculated buffer size.
 *
 * \return
 *   shr_error_e
 *
    ...
    int *flush_buffer_entries_max)
{
    ...
    *flush_buffer_entries_max = dnx_data_l2.age_and_flush_machine.flush_buffer_nof_entries_get(unit);
    ...
    {
        /*
         * Get the max number of MACT entries for current MDB profile
         */
...
/**
 * \brief - Copy entries from flush DMA to flush_buffer
 *
 * \param [in] unit - unit id
 *
 * \return
    ...
    int unit)
{
    ...
    /*
     * Repeat the loop as long as flush is still working and at least two loops after flush is done and reply fifo is
     * empty to clear any wrap-arounds 
     */
    while ((flush_in_work) || (entries_count != 0) || (reply_fifo_empty_counter < 2))
    {
        /** Read entries from the DMA's host memory. Wrap-around is considered internally */
        SHR_IF_ERR_EXIT(soc_dnx_fifodma_num_entries_get
        ...
        {
            /*
             * If flush is last known to be still working, check its current status,
             * otherwise check the reply fifo count, it's possible FIFO DMA HW hasn't begun copying from the reply FIFO despite the flush being done
             */
            if (flush_in_work)
            {
            ...
            {
                /*
                 * Check the MACT reply FIFO to verify the FIFO DMA has copied all of the remaining entries after the flush is done.
                 * After MACT reply FIFO is empty, repeat the loop once more to process all remaining FIFO DMA entries.
                 * No need to read MACT reply FIFO count again after it is reported empty, since it is only checked after flush is done,
                 * therefore it is unexpected to be filled again.
                 */
                if (reply_fifo_empty_counter == 0)
                {
                    ...
                    /*
                     * Count the number of iterations after reply_fifo_count is zero 
                     */
        ...
        else
        {
            ...
            /*
             * Copy retrieved entries from DMA buffer to flush buffer 
             */
            ...
            /*
             * update buffer's entries count 
             */
            
            flush_buffer_entries_count[unit][0] += entries_to_copy;

            /** Release the memory of the copied DMA entries */
            SHR_IF_ERR_EXIT(soc_dnx_fifodma_set_entries_read(unit, dma_channel_id, entries_count));

            /*
             * Sleep between FIFODMA "set_read" and "entries_get" to prevent a rare race of DMA "entries counter" and actual copied entries buffer
             */
            sal_usleep(10);
        }
    }
    ...
    if (bulk_commit || !flush_in_bulk)
    {
        ...
        else
        {
            ...
            while (flush_in_work && hw_accelerated && !non_blocking)
            {
                ...
                /** In case the flush is not done, sleep 1 millisecond */
                if (flush_in_work)
                {
                    /** Sleep 1 millisecond */
                    sal_usleep(1000);
