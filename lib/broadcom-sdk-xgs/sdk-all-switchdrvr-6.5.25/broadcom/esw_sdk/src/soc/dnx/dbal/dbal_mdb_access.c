...
/**Following macros apply for grem optimization mode*/

/**Number of erpp profiles in entire glem entry*/
#define DBAL_MDB_ACCESS_NUM_ERPP_PROFILES_IN_ENTRY 4
/**Zero padding required to match grem width to glem width*/
...
/**Get the relevant payload from the glem entry*/
...
/**Set the payload to its correct position inside the glem entry*/
...
/** Holds the MDB tables actions per DB type */
...
/**Get ll indication and outlif phase from sw table. If no link list and link list exists on RT - reduce link list field size from payload*/
static shr_error_e
    ...
    uint8 *is_eedb_ll)
{
    ...
    {
        /**Get ll allocation indication from the outlif sw table*/
...
/*
 * \brief
 * Update the DBAL tables with the MAX and MIN values of the FEC tables that was calculated during the pre init stage.
 * This values are updated here as the warm boot doesn't update these values.
 */
static shr_error_e
    ...
    int unit)
{
    ...
    for (fec_db_iter = 0; fec_db_iter < dnx_data_mdb.direct.nof_fec_dbs_get(unit); fec_db_iter++)
    {
        ...
        /*
         * In case the FEC range size is 0 set both the min and max values to 0.
         */
        if (range_size > 0)
        {
    ...
    int unit)
{
    ...
    for (table_idx = 0; table_idx < DBAL_NOF_PHYSICAL_TABLES; table_idx++)
    {
        ...
        /** Skip TCAM, KBP and DBs not supported on current device */
    ...
    uint32 *total_capacity)
{
    ...
    /*
     * get capacity from the first physical table
     */
    ...
    for (jj = 1; jj < table->nof_physical_tables; jj++)
    {
        ...
        {
            /*
             * If the table is SBC, the max capacity is the minimum of
             * the physical tables capacities it is mapped to.
             */
    ...
    int *result_type)
{
    ...
    for (ii = 0; ii < DBAL_NOF_TABLES; ii++)
    {
        ...
        {
            /*
             * In case Result type has don't care bits, e.g. its size is smaller than the ETPP_ETPS format size, need to shift.
             */
            ...
            {
                /*
                 * If the found result-type has don't-care bits, save it temporarily and continue to try to find a full
                 * match result-type. If there is not a full match result type, return the saved one.
                 * Note: thre is a risk to return the full macth result type improperly in case it indeed has don't-care bits.
                 */
    ...
    uint8 resolve_from_sw)
{
    ...
    /**Assumption: result types for lif tables are always provided by the dbal user*/
    ...
    while ((curr_res_type < table->nof_result_types) && (result_type_resolved == 0))
    {
        ...
        else
        {
            ...
            /** The MDB returns the value aligned to the biggest result type this is the reason why
             *  we need to read this value with the offset from max payload size */
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    else
    {
        ...
        /** updating the max_num_of_iterations */
    ...
    int nof_flush_rules)
{
    ...
    for (ii = 0; ii < nof_result_types; ii++)
    {
        ...
        {
            /*
             * In case there are only rules or actions on result fields with fixed location,
             * we need a rule per entry size in MDB
             */
            ...
            /*
             * This if prevents to run the for loop when:
             * 1) entry_encoding was already identified as forbidden rule
             * 2) entry_encoding was already set as a rule
             */
            ...
            {
                /*
                 * Check if one of the other result type with same encoding size
                 * is non flush compatible.
                 * If found, we cannot set flush rule for this entry encoding
                 */
        ...
        /** Set action bitmap */
        flush_shadow[rule_counter].action_types_btmp = actions_bitmap & 0xF; /** Only first 4 actions are valid for flush machine */
        flush_shadow[rule_counter].value_action_size = entry_handle->table->max_payload_size;
        /** In case having an action of read entries to DMA, need to set the ACTION_GET bit */
        ...
        {
            /*
             * In case result rule and actions are on aligned fields only, no need to have a rule per RT
             * So need to set don't care bits on result type field, and single rule is enough
             */
...
/**
 *  \brief Returns if possible to use flush at all.
 *         Updates the result_types_in_flush_bitmap and result_types_in_non_flush_bitmap bitmaps
 *         Updates nof_flush_rules and nof_non_flush_rules
 *  \param [in] unit
 *  \param [in] entry_handle
 *  \param [out] use_flush_out - indication if iterator is done via flush (does NOT mean ONLY flush machine is used)
 *  \param [out] result_types_in_flush_bitmap - Bitmap of result types which are iterated via flush machine
 *  \param [out] nof_flush_rules - number of rules in flush machine, each rule represents a result type
 */
static shr_error_e
...
/**
 *  \brief commit an entry to the rif part of the glem table.
 *  The data passed to the mdb is 20 bits long (same as global lif) while the rif is 4 bits long.
 *  This function places the key and data in the correct place and calls mdb apis
 */
static shr_error_e
    ...
    uint32 entry_flags)
{
    ...
    /**HW entry size is 20 bits*/
    tmp_entry_handle->phy_entry.payload_size = dnx_data_lif.out_lif.glem_result_get(unit);

    /**In case of optimization enabled - first check if one of the 4 entries on the 20 bits entry already exists. If yes - need to read-modify-write it.
     * Else - no need to read from hw (table is registered as optimized, allocated entries are stored in a bitmap), just add a new entry*/
    if (dnx_data_lif.global_lif.glem_rif_optimization_enabled_get(unit))
    {
        ...
        else
        {
            ...
            /**Entry found - place the data at the correct place inside the buffer*/
            ...
            /** setting the  new value in the payload   */
            tmp_entry_handle->phy_entry.payload[0] |= (payload << (DBAL_MDB_ACCESS_ALIGNED_KEY(global_rif)));
        }
    }
    else
    {
        /**Align the data to the correct place inside the 20 bits*/
...
/**
 *  \brief get an entry from the rif part of the glem table.
 *  The data retreived from the mdb is 20 bits long (same as global lif) while the rif is 4 bits long.
 *  This function places the key place and calls mdb get api and returns the relevant data
 */
static shr_error_e
    ...
    uint32 entry_flags)
{
    ...
    /**First - make sure that the entry exists*/
 ...
 *  \brief get an entry from the rif part of the glem table.
 *  The data retreived from the mdb is 20 bits long (same as global lif) while the rif is 4 bits long.
 *  This function places the key place and calls mdb get api and deletes the relevant data
 */
static shr_error_e
    ...
    uint32 entry_flags)
{
    ...
    /**For optimization enabled - 4 entries can exist in the same 20 bits glem entry.
     * In case there're more than one entry present - need to remove only the relevant one.
     * Else - remove the entire 20 bits entry*/
    if (dnx_data_lif.global_lif.glem_rif_optimization_enabled_get(unit))
    {
        ...
        /**How many entries on this specific entry*/
        for (idx = 0; idx < 4; idx++)
        {
            tmp_entry_handle.phy_entry.key[0] = entry_base + idx;
            SHR_IF_ERR_EXIT(dbal_actions_optimized_is_exists(unit, &tmp_entry_handle, &entry_exists));
        ...
        /**Only this profile on entry, remove the entire entry*/
        if (num_entries == 1)
        {
            SHR_IF_ERR_EXIT(physical_table_actions->entry_delete(unit, physical_db_id, app_id, &entry_handle->phy_entry,
                                                                 entry_flags));
        }
        /**More than one profile, update the existing entry*/
        else
        {
            /**Key of the whole mdb entry*/
    ...
    dbal_iterator_info_t * iterator_info)
{
    ...
    while (!entry_found && !iterator_info->is_end)
    {
        ...
        {
            /** case where there is an allocator field, using resource manager to find the next entry */
...
/**
 * \brief
 * Copy the values set by user in the MDB iterator
 * from src to dst
 * \param [in] unit
 * \param [in] src - MDB iterator to be copied
 * \param [out] dst - MDB iterator to copy to
 */
static shr_error_e
...
/*
 * \brief In SBC table mapped to more than one physical table, we iterate over all
 * the physical tables to find the eventual hits
 * Start iterating from next table after current_physical_table.
 */
static shr_error_e
    ...
    uint32 app_id)
{
    ...
    SHR_FUNC_INIT_VARS(unit);
    /*
     * In SBC table mapped to more than one physical table, we iterate over all the physical tables to find the
     * eventual hits
     */
           ...
           && (physical_table_iter < table->nof_physical_tables))
    {
        ...
        /*
         * entry needs to be found in all the physical tables since we are in SBC table
         */
        SHR_IF_ERR_EXIT(physical_table_actions->entry_get(unit,
                                                          table->physical_db_id[physical_table_iter],
                                                          app_id, phy_entry, 0));
        /*
         * OR together results from the different tables 
         */
...
/*
 * brief
 * Internal function getting next valid entry from SW
 * \param [in] unit
 * \param [in] entry_handle
 * \param [out] continue_calling_mdb_get_next
 * This flag indicates to the higher level that the entry we got is not valid,
 * so we need to continue getting next entry.
 */
static shr_error_e
    ...
    int *continue_calling_mdb_get_next)
{
    ...
    *continue_calling_mdb_get_next = FALSE;
    ...
    /*
     * Iterator got to end. In case the table is DPC and mapped to more
     * than 1 physical table, we configure it to next physical table, and
     * run over it.
     */
    if (((!SHR_IS_BITSET(table->indications_bm, DBAL_TABLE_IND_RESULT_TYPE_MAPPED_TO_SW))
         && (table->core_mode == DBAL_CORE_MODE_DPC) && (DBAL_MAX_NUM_OF_CORES > 1)))
    {
        while (iterator_info->is_end && (iterator_info->physical_db_index < (table->nof_physical_tables - 1)))
        {
            ...
            /*
             * backup and restore the info set by user in MDB physical entry iterator
             */
    ...
    {
        dbal_key_value_validity_e is_valid_entry = DBAL_KEY_IS_INVALID;
        /** In case there were no flush rules or there is already a result type rule - No need to
         * add non flush rules for result type
         */
        ...
        /*
         * - Leave default core id for DPC tables in single core devices
         * - core_id is updated when the entry was retrieved from the HW only
         */
        ...
        /*
         * this block is entered for a result type already treated in flush machine
         */
        else if (!utilex_bitstream_test_bit(&result_types_to_find, entry_handle->cur_res_type))
        {
            /*
             * If we are in a perform action flow (update, delete), we want to update shadow entries only (because HW
             * was already updated by flush).
             * But this is true only if we don't do "get" too, because in this case (action + get), the entry is received from
             * flush and action apply is updated there.
             */
        ...
        *continue_calling_mdb_get_next = FALSE;
        /*
         * If the found entry is retrieved in SW (not flush), the action needs to be performed HW and Shadow TBD: check
         * if/why this line is not needed, because DBAL_MDB_ACTION_APPLY_ALL is already the default
         */
        ...
        {
            /*
             * In SBC (or single core DPC) table, if mapped to more than one physical table,
             * we iterate over the other physical tables to find the eventual hits
             */
...
/*
* \brief
* Getting entry from flush is relevant only if the caller wants the entry back (get action),
* or if we explicitly want to read from DMA (read entries from another thread after trigger).
* Else if no get is required, it means some action was committed to flush, and is performed there
*/
static shr_error_e
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    if (!iterator_info->is_end)
    {
        /** The flush can not retrieve hit bit so we perform entry get again.
         * The hit bit request is already on the on the handle,
         * just need to perform get again to receive */
        ...
        /*
         * When action is "get + update", the next entry we get from flush needs is returned
         * with action apply set to shadow, so the action can be performed in shadow too (as HW part
         * already performed) on this same handle, before it is returned to user.
         *
         */
        entry_handle->phy_entry.mdb_action_apply = DBAL_MDB_ACTION_APPLY_SW_SHADOW;
#if !defined(ADAPTER_SERVER_MODE) && !defined(PLISIM)
...
/*
 * brief
 * Internal function getting next valid entry from flush and SW
 * \param [in] unit
 * \param [in] entry_handle
 * \param [out] continue_calling_mdb_get_next
 */
    ...
    int *continue_calling_mdb_get_next)
{
    ...
    *continue_calling_mdb_get_next = FALSE;
    ...
    /*
     * non-blocking mode is relevant only for the first call to get next, when not reading from DMA (when reading from
     * DMA, it is always blocking)
     */
    ...
    /*
     * Getting entry from flush is relevant only if the caller wants the entry back (get action), or if we explicitly
     * want to read from DMA (read entries from another thread after trigger). Else if no get is required, it means
     * some action was committed to flush, and is performed there
     */
    ...
    /*
     * This blocks gets next_entry from SW (not flush): it is accessed only when we are in blocked mode, and
     * not specifically requested to read entries from DMA only
     * Plus:
     * if is_end is true: means no more entry was found in flush => continue to SW
     * or if not in a get action (performing some action) => we need to get entries from SW and update there too
     * or if non flush iteration has started
     */
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    {
        /** for LPM there is no way to get from HW, the only valid action apply is ALL */
    ...
    {
        /** for TCAM no need to change the action apply */
        SHR_EXIT();
    }
    ...
    if (phy_entry->mdb_action_apply == DBAL_MDB_ACTION_APPLY_ALL)
    {
        ...
        if (skip_shadow)
        {
            /** If the flag to Skip shadow is set, we read from HW, as this is probably for debug usage */
        ...
        else
        {
            ...
            {
                /** for tables which don't support learning we can perform get from SW since SW and HW are synced. */
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    /**In case of inlif - align to max possible payload for mdb access*/
    ...
    uint8 is_eedb_ll)
{
    ...
    if (SHR_IS_BITSET(entry_handle->table->indications_bm, DBAL_TABLE_IND_RESULT_TYPE_MAPPED_TO_SW))
    {
        ...
        int curr_res_type = entry_handle->cur_res_type;
          ...
          * when the result type is in SW no need to align since the from HW perspective all entries has the same size..*/
    ...
    {
        /**In case there's a link list on the result type and it is not used, need to align the retrieved data 30 bits to compensate for it*/
    ...
    uint8 is_eedb_ll)
{
    ...
    /** in case that the table is TCAM by key there is a need to first get the access ID and with the access ID perform
     *  the get */
    ...
    /** getting the entry from the MDB, the entry is received aligned to the max_payload_size. (this way it is
     *  easy to identify the result type value it is in the same offset for all) */
    if (entry_handle->table_id == DBAL_TABLE_GLOBAL_RIF_EM)
    {
        /**Special treatment for global rif table*/
    ...
    else
    {
             ...
             && (entry_handle->phy_entry.hitbit & DBAL_PHYSICAL_KEY_HITBIT_GET)))
        {
            /*
             * In SBC (or single core DPC) table, if mapped to more than one physical table,
             * we iterate over all the physical tables to find the eventual hits
             */
            SHR_IF_ERR_EXIT(dbal_mdb_entry_hitbit_get_over_physical_tables
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    if (dbal_table_is_out_lif(table))
    {
        ...
        /**Validate ll existence and update payload*/
...

    {
        DBAL_ITERATE_OVER_CORES(entry_handle, core_id)
        {
            ...
            SHR_IF_ERR_EXIT_WITH_MSG_EXCEPT_IF(dbal_mdb_access_get
                                               (unit, app_id, physical_table, entry_handle, is_eedb_ll),
                                               _SHR_E_NOT_FOUND);
...
/**
 * calls entry delete from MDB. if needed delete the entry also from the SW shadow.
 */
static shr_error_e
    ...
    uint8 is_eedb_ll)
{
    ...
    if (SHR_IS_BITSET(entry_handle->table->indications_bm, DBAL_TABLE_IND_RESULT_TYPE_MAPPED_TO_SW))
    {
        ...
        {
            /** for devices with one core, there are cases that two phy_dbs should be updated.
             *  in the SW there is only one entry that corresponds to the two phy DBs, this is the reason why we don't need to
             *  delete twice */
            sw_access_needed = 0;
        }
        if (sw_access_needed)
    ...
    dbal_entry_handle_t * entry_handle)
{
...

    {
        ...
        DBAL_ITERATE_OVER_CORES(entry_handle, core_id)
        {
            ...
            /** Align the valid physical_tables to the lower indices */
...
/**
 * calls entry add from MDB. if needed add the entry also to the SW shadow.
 */
static shr_error_e
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    if (dbal_table_is_out_lif(table))
    {
        ...
        if ((!is_eedb_ll) && (rt_has_link_list))
        {
            /**If there's no link list but it exists on the result type - need to shift it*/
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    physical_tables[0] = physical_db_id; /** only first element relevant for now */
