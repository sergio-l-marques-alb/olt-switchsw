...
/**
 * \brief
 *  Init function for a dnx_field_tcam_access_fg_params_t structure.
 *
 * \param [in] unit                        - Device ID
 * \param [out] fg_params_p                - The structure to be initialized
 *
 * \return
    ...
    dnx_field_tcam_entry_t * entry)
{
    ...
    else
    {
        ...
        /*
         * Verify that entry ID does not exist 
         */
        DNXCMN_CORES_ITER(unit, core, core_iter)
        {
    ...
    uint32 handler_id)
{
     ...
     * 3. Clear fg_params SW state
     * 4. free the handler id
     */
    ...
    dnx_field_tcam_entry_t * entry)
{
    ...
    {
        /*
         * Check whether a new bank needs to be add
         */
        ...
        if (banks_full)
        {
            ...
            {
               /**
                * We do not allocate bank automatice for non auto mode
                * means all SELECTED banks are full and there is no more room to add new entry
                * */
    ...
    /*
     * Find and allocate locations per core for new entry 
     */
    DNXCMN_CORES_ITER(unit, core, core_iter)
    {
    ...
    uint32 entry_id)
{
    ...
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core_iter)
    {
        ...
        /*
         * Check nof entries left for AUTO BAM
         */
    ...
    /*
     * Check nof entries left for AUTO BAM DT
     */
    if (fg_params.bank_allocation_mode == DNX_FIELD_TCAM_BANK_ALLOCATION_MODE_AUTO && fg_params.direct_table)
    {
        DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core_iter)
        ...
        /*
         * When no entries left on both cores for AUTO BAM, remove bank from handler's allocated banks list
         */
        if (entries_left == 0)
        {
 ...
 * \param [in] core_ids     - Array of the core IDs to evacuate bank from
 * \return
 *  \retval _SHR_E_NONE  - success
 */
static shr_error_e
    ...
    bcm_core_t core_ids[])
{
    ...
    for (bank_idx = 0; bank_idx < nof_banks; bank_idx++)
    {
        ...
        {
            /*
             * Check that bank allocated for given handler on given core
             */
