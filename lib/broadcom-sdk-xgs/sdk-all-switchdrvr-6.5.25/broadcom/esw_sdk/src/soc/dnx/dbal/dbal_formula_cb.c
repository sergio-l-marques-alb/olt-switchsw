/*
 * $Id: dbal_formula_cb.c,v 1.13 Broadcom SDK $
    ...
    uint32 *offset)
{
    ...
    /** Get first lane of first PM  in ETHU*/
    ...
    /** Subtract the first lane in ETHU from the global lane */
    ...
    uint32 *offset)
{
    ...
    /** Get the type index from ETHU properties */
    type_index = dnx_data_nif.eth.ethu_properties_get(unit, ethu_id)->type_index;

    /** If the type is CDU we need to take into account CDUMs */
    ...
    uint32 *offset)
{
    ...
    /** Get the ETHU ID (total, not in core) */
    ...
    uint32 *offset)
{
    ...
    *offset =
    ...
    /*
     * Handle case when NIF ports order is ETH, FlexE, ILKN
     */
...
/**
 *  ILLEGAL VALUES CALLBACKS PART
 *  -----------------------------
 *
 *  this part consist of all the callback for illegal values per field types.
 *  user can assign a callback to a field type incase the illegal values are dynamic and cannot be wrriten in XML.
 *
 *  To support such a callback tehere are two steps:
 *
 *  1. Define the function form the type dnx_dbal_field_type_illegal_value_cb
 *  2. Add the callback to the dbal_field_types_init_illegal_value_cb_assign to the corresponding field type.
 *
 */

/**
   \brief
 * a simple example of a callback to be used
 *
 */
shr_error_e
...
/**
 * \brief - clu logical port illegal value callback
 *
 */
static shr_error_e
...
#ifdef INCLUDE_XFLOW_MACSEC
/**
 * \brief - macsec id illegal value callback
 *
 */
static shr_error_e
    ...
    uint8 *is_illegal)
{
    ...
    /*
     * dbal tables are accessed before sw-state is initiated, and the indication is not yet available
     * Need to enable access in that case
     */
...
/**
 * \brief
 * API assign illegal value callback function to a specific field type.
 * the callback should be from dnx_dbal_field_type_illegal_value_cb
 *
 * callback should be from the type: dnx_dbal_field_type_illegal_value_cb
 *
 */
shr_error_e
