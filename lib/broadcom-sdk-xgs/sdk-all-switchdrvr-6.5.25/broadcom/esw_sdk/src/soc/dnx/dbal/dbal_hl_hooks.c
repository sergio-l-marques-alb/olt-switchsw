    ...
    dbal_entry_handle_t * entry_handle_hook)
{
    ...
    /** In case that pbmp_pp_dsp is empty, exit from the function (relevant for trunk members) */
    if (BCM_PBMP_IS_NULL(pbmp_pp_dsp))
    {
        entry_handle_hook->error_info.error_exists = 0;
        SHR_EXIT();
    }
    /** Iterate over all PP DSP (dsp_pp ports) in order to set all the entries according to them */
        ...
        if (is_condition_pass)
        {
            ...
            {   /** need to read new memory from HW */
                ...
                while (correct_data_is_read != TRUE)
                {
                    ...
                    if (fieldinfo != NULL)
                    {
                        /** From the data get the ECC field of the memory */
...
exit:
    /** Enable ECC error check for the relevant HW by unmasking interrupt
     *  This needs to be done even in case of an error, or else the interrupt will be left masked
     *  and in case of a real ECC error interrupt handler will not be called and there will be no soft reset.
     *  Start from index-1 dow to ensure we only write to registers that were shenged in the beggining of the
     *  function. */
    index -= 1;
    while (index >= 0)
    {
