    ...
    dbal_physical_tables_e dbal_physical_table_id)
{
    ...
    uint8 direct_sub_type;

    db_type = dnx_data_mdb.pdbs.dbal_pdb_info_get(unit, dbal_physical_table_id)->db_type;
    direct_sub_type = dnx_data_mdb.pdbs.dbal_pdb_info_get(unit, dbal_physical_table_id)->db_subtype;
    ...
    int unit)
{
        ...
        && (!(dnx_data_sbusdma_desc.global.feature_get(unit, dnx_data_sbusdma_desc_global_force_enable))))
    {
             ...
             dbal_physical_table_id++)
        {
            ...
            if (dnx_sbusdma_desc_is_enabled(unit, mdb_direct_dbal_table_to_dma_enum(unit, dbal_physical_table_id)) ==
                TRUE)
            {
         ...
         mdb_physical_table_id_iter++)
    {
        if ((dnx_data_mdb.pdbs.mdb_pdb_info_get(unit, mdb_physical_table_id_iter)->db_type ==
            ...
            || ((dnx_data_mdb.pdbs.mdb_pdb_info_get(unit, mdb_physical_table_id_iter)->db_type ==
                ...
                && (mdb_eedb_table_to_mem_type(unit, mdb_physical_table_id_iter) == MDB_EEDB_MEM_TYPE_DATA_CLUSTER)))
        {
            alloc_size =
                ...
                dnx_data_mdb.dh.macro_type_info_get(unit,
                                                    dnx_data_mdb.pdbs.mdb_pdb_info_get(unit,
                                                                                       mdb_physical_table_id_iter)->macro_granularity)->
                nof_rows;
    ...
    uint32 cluster_index_val)
{
    ...
    alloc_range_size =
        dnx_data_mdb.dh.macro_type_info_get(unit,
                                            dnx_data_mdb.pdbs.mdb_pdb_info_get(unit,
                                                                               mdb_physical_table_id)->macro_granularity)->
        nof_rows;
         ...
         address_iter < physical_address_end / alloc_range_size; address_iter++)
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Error. Address 0x%x is already set for mdb_physical_table_id %s.\n",
                         address_iter, dnx_data_mdb.pdbs.mdb_pdb_info_get(unit, mdb_physical_table_id)->name);
        }
    ...
    int physical_address_end)
{
    ...
    if ((dnx_data_mdb.pdbs.mdb_pdb_info_get(unit, mdb_physical_table_id)->db_type != DBAL_ENUM_FVAL_MDB_DB_TYPE_DIRECT)
        && (mdb_eedb_table_to_mem_type(unit, mdb_physical_table_id) != MDB_EEDB_MEM_TYPE_DATA_CLUSTER))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Error. MDB physical table (%d) is not direct.\n", mdb_physical_table_id);
    }
...
mdb_direct_table_calc_index_address(
    int unit,
    mdb_physical_table_e physical_table,
    uint32 entry_index,
    int entry_size,
    uint32 row_width,
    soc_mem_t * mem_entry,
    ...
    uint8 *is_end)
{
    ...
    int requested_payload_size = entry->payload_size;
    ...
    {
        entry->payload_size = requested_payload_size;
    }
...
mdb_direct_table_clear(
    ...
    uint32 app_id)
{
    int dbal_physical_table_id_iter;

    SHR_FUNC_INIT_VARS(unit);

    
    for (dbal_physical_table_id_iter = 0; dbal_physical_table_id_iter < DBAL_MAX_NUM_OF_PHY_DB_IN_TABLE;
         dbal_physical_table_id_iter++)
    {
        dbal_physical_tables_e dbal_physical_table_id = physical_tables[dbal_physical_table_id_iter];
        if (dbal_physical_table_id != DBAL_PHYSICAL_TABLE_NONE)
        {
            ...
            SHR_IF_ERR_EXIT(mdb_direct_table_iterator_init
            ...
            SHR_IF_ERR_EXIT(mdb_direct_table_iterator_deinit
                            (unit, dbal_physical_table_id, app_id, &physical_entry_iterator));
        }
    }
...
mdb_direct_table_entry_add(
    ...
    uint32 flags)
{
    mdb_physical_table_e mdb_physical_table_arr[DBAL_MAX_NUM_OF_PHY_DB_IN_TABLE];
    mdb_cluster_direct_entry_info_t cluster_info;
    dbal_enum_value_field_direct_payload_sizes_e payload_type;
    int remainder_size;
    soc_mem_t mem;
    soc_reg_t overflow_reg;
    uint32 row_data[BITS2WORDS(MDB_ENTRY_BANK_SIZE_IN_BITS)] = { 0 };
    ...
    for (table_iter = 0; table_iter < tables_num; table_iter++)
    {
        mdb_physical_table_arr[table_iter] =
            dnx_data_mdb.pdbs.dbal_pdb_info_get(unit, dbal_physical_table_id_arr[table_iter])->logical_to_physical;

        if (mdb_physical_table_arr[table_iter] == MDB_NOF_PHYSICAL_TABLES)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
    ...
    row_width = dnx_data_mdb.pdbs.dbal_pdb_info_get(unit, dbal_physical_table_id_arr[0])->row_width;
    ...
    for (table_iter = 0; table_iter < tables_num; table_iter++)
    {
        ...
        if (use_overflow_reg)
        {
            ...
            if ((dnx_sbusdma_desc_is_enabled
                 (unit, mdb_direct_dbal_table_to_dma_enum(unit, dbal_physical_table_id_arr[table_iter])) == TRUE)
                && (cache_enabled == TRUE))
            {
    ...
    uint32 flags)
{
    ...
    int macro_index;
    bsl_severity_t severity;
    uint8 cache_enabled = TRUE;
    int hitbit_enabled = TRUE;

    SHR_FUNC_INIT_VARS(unit);

#if defined(ADAPTER_SERVER_MODE)
    hitbit_enabled = FALSE;
#endif
...
#endif
    {
        if (dnx_data_mdb.pdbs.dbal_pdb_info_get(unit, dbal_physical_table_id)->logical_to_physical ==
        ...
        if ((hitbit_enabled == TRUE) && (entry->hitbit & DBAL_PHYSICAL_KEY_HITBIT_ACTION))
        {
...
#if defined(ADAPTER_SERVER_MODE) && defined(BCM_DNX2_SUPPORT)
int
mdb_map_table_names(
    int unit,
    ...
    char *adapter_table_name)
{
    ...
    const char *adapter_table_names[] = {
        "InLif1", "InLif2", "InLif3", "Ivsd", "Map", "Fec1", "Fec2", "Fec3", "Eedb1", "Eedb2", "Eedb3", "Eedb4", "Evsd"
    };
    int nof_elements;
    ...
    for (i = 0; i < nof_elements; i++)
    {
        ...
        {
            sal_strlcpy(adapter_table_name, adapter_table_names[i], SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH);
    ...
    int full_time)
{
    ...
    entry_size = dnx_data_mdb.pdbs.dbal_pdb_info_get(unit, dbal_physical_table_id)->max_payload_size;
                                                                               ...
                                                                               * MDB_DIRECT_BASIC_ENTRY_SIZE)))
            {
                ...
                if (op_iter == 1)
                {
                    ...
                    SHR_IF_ERR_EXIT(mdb_direct_table_entry_get
                                    (unit, dbal_physical_table_id, 0  , &entry, 0));
                    if (full_time == FALSE)
                    {
...
#if defined(ADAPTER_SERVER_MODE) && defined(BCM_DNX2_SUPPORT)
                    if ((adapter_table_name[0] != 0) && (!dnx_data_mdb.global.adapter_stub_enable_get(unit)))
                    {
                        ...
                        SHR_IF_ERR_EXIT(control_get_table_data
                                        (unit, &(adapter_table_name[0]), "mdb", entry.key[0],
                ...
                if (op_iter == 3)
                {
                    ...
                    if (utilex_bitstream_have_one_in_range(entry.payload, 0  , entry.payload_size - 1))
                    {
                        LOG_INFO(BSL_LOG_MODULE, (BSL_META_U(unit, "\n")));
                        LOG_INFO(BSL_LOG_MODULE, (BSL_META_U(unit, "Payload size: %d\n"), entry.payload_size));
                        LOG_INFO(BSL_LOG_MODULE, (BSL_META_U(unit, "entry key: %d\n"), entry.key[0]));
                        LOG_INFO(BSL_LOG_MODULE, (BSL_META_U(unit, "entry_counter: %d\n"), entry_counter));
    ...
    if (mode == MDB_TEST_CAPACITY)
    {
        SHR_IF_ERR_EXIT(mdb_direct_table_clear(unit, physical_tables, 0  ));
    }
    ...
    dbal_enum_value_field_direct_payload_sizes_e * payload_type)
{
    ...
    {
        if (mdb_eedb_table_to_mem_type(unit, mdb_physical_table_id) == MDB_EEDB_MEM_TYPE_LL)
        {
    ...
    {
        *payload_type = dnx_data_mdb.pdbs.mdb_pdb_info_get(unit, mdb_physical_table_id)->direct_payload_type;
    }
exit:
    SHR_FUNC_EXIT;
}
    ...
    int *basic_size)
{
    ...
    if (dnx_data_mdb.pdbs.mdb_pdb_info_get(unit, mdb_physical_table_id)->db_type == DBAL_ENUM_FVAL_MDB_DB_TYPE_EEDB
        || dnx_data_mdb.pdbs.mdb_pdb_info_get(unit,
                                              mdb_physical_table_id)->db_type == DBAL_ENUM_FVAL_MDB_DB_TYPE_DIRECT)
    {
        dbal_enum_value_field_direct_payload_sizes_e payload_type;
    ...
    else
    {
        ...
        *basic_size = dnx_data_mdb.pdbs.mdb_pdb_info_get(unit, mdb_physical_table_id)->row_width;
    }
exit:
    SHR_FUNC_EXIT;
}
...
mdb_direct_table_get_basic_size_using_logical_mdb_table(
    ...
    int *basic_size)
{
    mdb_physical_table_e physical_mdb_table;
    SHR_FUNC_INIT_VARS(unit);
    switch (logic_table_id)
    {
        case DBAL_PHYSICAL_TABLE_EEDB_1:
            physical_mdb_table = MDB_PHYSICAL_TABLE_EEDB_1_DATA;
            break;
        case DBAL_PHYSICAL_TABLE_EEDB_2:
            physical_mdb_table = MDB_PHYSICAL_TABLE_EEDB_2_DATA;
            break;
        case DBAL_PHYSICAL_TABLE_EEDB_3:
            physical_mdb_table = MDB_PHYSICAL_TABLE_EEDB_3_DATA;
            break;
        case DBAL_PHYSICAL_TABLE_EEDB_4:
            physical_mdb_table = MDB_PHYSICAL_TABLE_EEDB_4_DATA;
            break;
        case DBAL_PHYSICAL_TABLE_EEDB_5:
            physical_mdb_table = MDB_PHYSICAL_TABLE_EEDB_5_DATA;
            break;
        case DBAL_PHYSICAL_TABLE_EEDB_6:
            physical_mdb_table = MDB_PHYSICAL_TABLE_EEDB_6_DATA;
            break;
        case DBAL_PHYSICAL_TABLE_EEDB_7:
            physical_mdb_table = MDB_PHYSICAL_TABLE_EEDB_7_DATA;
            break;
        case DBAL_PHYSICAL_TABLE_EEDB_8:
            physical_mdb_table = MDB_PHYSICAL_TABLE_EEDB_8_DATA;
            break;
        default:
            physical_mdb_table = dnx_data_mdb.pdbs.dbal_pdb_info_get(unit, logic_table_id)->logical_to_physical;
    }
