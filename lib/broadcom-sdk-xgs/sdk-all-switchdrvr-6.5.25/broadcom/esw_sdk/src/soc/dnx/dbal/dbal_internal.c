    ...
    uint8 operation)
{
    ...
    /** only if table has const field need to set the const field value before working on the handle */
    ...
    uint32 *field_mask)
{
    ...
    /** in case of subfield the mask should be affect the encoding part suffix/prefix. in those cases the suffix/prefix
     *  mask should be all "1" */
    ...
    dbal_pointer_type_e pointer_type)
{
    ...
    if (field_mask && DBAL_TABLE_IS_KEY_MASK_REQUIRED(table))
    {
        ...
        {
            /** Copy the relevant part of the field mask */
    ...
    uint32 *field_mask)
{
    ...
    /** in case of valid indication (and all other encoding that adds bits to the field the user can use a smaller pointer
    ...
    dbal_pointer_type_e pointer_type)
{
    ...
    /*
     *  in case of result type we can easily return it from the
    ...
    else
    {
        ...
        /** in case the result type is superset, if the field was not set before the call it means that it was not added */
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    for (iter = 0; iter < table->multi_res_info[entry_handle->cur_res_type].nof_result_fields; iter++)
    {
        ...
        /*
         * Once we updated the pointer, we clean the requested field to make sure we don't reuse it 
         */
        entry_handle->value_field_ids[iter] = DBAL_FIELD_EMPTY;
        entry_handle->nof_result_fields--;
    }
...
/**
 * \brief - Format an array of uint32s as a block,
 *
 * \param [in] unit
 * \param [out] buffer_dst - out destination buffer to print
 * \param [in] buffer_src - value array
 * \param [in] prefix_str - string to print before
 * \param [in] suffix_str - string to print after
 * \param [in] separator_str - string to print between the blocks
 * \param [in] size - nof bits to print
 * \param [out] total_char_count - total number of printed char
 *
 * Example: {0x01234567, 0x89abcdef}:
 * If prefix_str = "" and separator_str = "",  it will be displayed 0x0123456789abcdef
 * If prefix_str = "(" and suffix_str = ")" and separator_str = ", ",  it will be displayed (0x01234567, 0x89abcdef)
 * \return
 *   shr_error_e
 */
static shr_error_e
    ...
    int *total_char_count)
{
    ...
    *total_char_count += cur_char_count;
    ...
    /*
     * If there are no separator, print "0x" once before the hex value
     */
    ...
    {
        /*
         * If there is a separator, print "0x" before each block 
         */
        if (sal_strncmp(separator_str, "", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) != 0)
        {
            cur_char_count = sal_snprintf(buffer_dst, DBAL_MAX_STRING_LENGTH, "0x");
    ...
    /*
     * Print termination character (suffix) 
     */
    ...
    char buffer_to_print[DBAL_MAX_PRINTABLE_BUFFER_SIZE])
{
    ...
    /*
     * For uint32 values, if logger is configured as print=hex, just print the value in hexa 
     */
        ...
        if (condition.field_id != DBAL_FIELD_EMPTY)
        {
            ...
            else
            {
                ...
                *is_passed = 0;
                ...
                /*
                 * In case rule was defined on subfield, it can happen the field for current entry is not for this
                 * subfield, which causes this error. So we simply skip the entry. 
                 */
    ...
    dbal_key_value_validity_e * is_valid)
{
    ...
    *is_valid = DBAL_KEY_IS_VALID;
    ...
    /*
     * We check if this key combination is unmapped for all RT.
     * If it is valid for at least one RT, entry is valid.
     */
    if ((entry_handle->table->access_method == DBAL_ACCESS_METHOD_HARD_LOGIC) &&
        ...
        SHR_IS_BITSET(entry_handle->table->indications_bm, DBAL_TABLE_IND_IS_ASYMMETRIC))
    {
        ...
        /*
         * When we get here, it means we received _SHR_E_ACCESS for
         * all the result types.
         * Obviously the most common case for this to happen is when
         * the table has only one RT, which is unmapped for the
    ...
    dbal_key_value_validity_e * is_valid_entry)
{
    ...
    if (is_allocator)
    {
        ...
        {
            /** if the field size is smaller than the actual resource need to validate that the entry is not exceeded  */
...
/** Dummy definitions for KBP access APIs. */
