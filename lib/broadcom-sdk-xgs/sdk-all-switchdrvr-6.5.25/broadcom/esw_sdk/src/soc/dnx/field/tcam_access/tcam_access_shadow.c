...
dnx_field_tcam_access_entry_key_read_shadow(
    ...
    uint8 *valid_bits)
{
    uint8 shadow_data[BITS2BYTES(DNX_DATA_MAX_FIELD_TCAM_ENTRY_SIZE_SINGLE_KEY_SHADOW)] = { 0 };
    uint32 shadow_data_32[BITS2WORDS(DNX_DATA_MAX_FIELD_TCAM_ENTRY_SIZE_SINGLE_KEY_SHADOW)] = { 0 };
    uint32 entry_half_key_size;
    /** Valid bits are an array instead of a pointer due to coverity error of bitrange treating it as an array.*/
    ...
    uint32 *key_mask)
{
    ...
    {
        /*
         * Remove mode bits from shadow data
         */
    ...
    uint8 *equal)
{
    ...
    *equal = FALSE;     /* Assume entry not equal to pair, until proven wrong */
    ...
    {
        SHR_EXIT();     /* Entry not in use, equal is false */
    }
    ...
    if (key_size == DOUBLE_KEY_BIT_SIZE)
    {
        ...
        LOG_DEBUG_EX(BSL_LOG_MODULE, "Double-key comparision, comparing second half in odd bank ID%s%s%s%s\n", EMPTY,
                     EMPTY, EMPTY, EMPTY);
        SHR_IF_ERR_EXIT(dnx_field_tcam_access_entry_equal_shadow(unit, core, key + SINGLE_KEY_WORD_SIZE, key_mask + SINGLE_KEY_WORD_SIZE, key_size / 2, bank_id + 1, bank_offset, TRUE  /* Skip 
                                                                                                                                                                                         * entry 
                                                                                                                                                                                         * in 
                                                                                                                                                                                         * use 
                                                                                                                                                                                         * check 
        ...
        /*
         * Now proceed to compare first half in even bank ID 
         */
        key_size /= 2;
    }
    ...
    {
        /*
         * Compare second half for odd offsets 
         */
        /*
    ...
    {
        /*
         * sal_memcmp isn't endian-safe, therefore clear msb and compare whole key 
         */
