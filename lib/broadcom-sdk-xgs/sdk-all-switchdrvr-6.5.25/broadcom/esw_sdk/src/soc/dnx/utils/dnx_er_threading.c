...
/*
 * This function has dual usage
 * 1. mark thread as active and get an ER id for it that will be used as its index in db
 * 2. mark thread as temporarily excluded from ER activity (even if it's not ER active yet)
 */
static shr_error_e
    ...
    int is_tmp_expluded_thread)
{
    ...
    /*
     * make a greedy attempt to +1 an already existing id and avoid taking the mutex
     */
    for (i = 0; i < DNX_ERR_RECOVERY_MAX_NOF_ACTIVE_TRANSACTION; i++)
    {
...
/*
 * This function has dual usage
 * 1. mark thread as not active anymore and free ER id for it in case it's not used anymore (counter is 0)
 * 2. unmark thread as temporarily excluded from ER activity (even if it's not ER active yet)
 */
static shr_error_e
    ...
    int unit)
{
    ...
    /*
     * Assert if error as this is used by dispatcher and must succeed 
     */
 ...
 */
int
dnx_er_threading_id_leak_verification(
    int unit)
{
    int er_thread_id;
