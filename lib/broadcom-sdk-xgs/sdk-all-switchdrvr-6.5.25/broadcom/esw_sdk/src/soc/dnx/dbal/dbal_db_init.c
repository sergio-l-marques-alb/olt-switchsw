...
/** Precondition. field_nof_bits, offset_in_logical_field have been set to field */
...
/** Precondition. field_indication_bm with const valid indication has been set to field */
...
/** Precondition. field_id, field_type have been set to field */
...
/** Precondition.has result type, and nof_result_types has been set to table */
...
/** skip set allocator fields when:
 *  - mdb table with multiple key fields (core ID is not counted)
 *  - allocator has already been found and set to the table
 *  Preconditions: table has been initialized with access method and interface keys */
    ...
    int unit)
{
    ...
    for (table_index = 0; table_index < nof_tables; table_index++)
    {
        ...
        {
            /** we are allocating kbp_handles for all dynamic tables in advance since we can't know which table belongs to which
             *  access type. this is done only if there is a KBP in the system.. (alloc is not allowed after init) */
...
/**
 * \brief
 * The function parses the DNX DATA reference from xml in its
 * new format (as used in SDK) to dnx data components
 * New formats:
 * 1. size="dnx_data_mdb_app_db.fields.numeric_mdb_field___1_get(unit)"
 * 2. size="dnx_data_module.submodule.data_get(unit, key)->member"
 * 3. size="dnx_data_module.submodule.data_get(unit, key1, key2)->member"
 * 4. size="dnx_data_module.submodule.feature_get(unit, dnx_data_module_submodule_feature)"
 */
static shr_error_e
    ...
    dbal_parsed_db_dnx_data_struct_t * int_or_dnx_data)
{
    ...
    /** first word is dnx_data_<module_name> - get the module name */
    ...
    if (dnx_data_label != NULL)
    {
        if (!sal_strncmp(int_or_dnx_data->data, DB_INIT_DNX_DATA_FEATURE, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
        {
            ...
            /*
             * if we have feature_get, we need to parse the feature name (see format at the function head), and remove
             * from it module and submodule +1 to remove the '_' after the submodule 
             */
    ...
    /** Parse key 2, note it may not exist, so the value returned by atoi is irrelevant */
    dnx_data_label = sal_strtok_r(NULL, dnx_data_delimiters, &saveptr);
    if (dnx_data_label != NULL)
    {
...
/**
 * \brief
 * The function parse the read DNX DATA reference from xml
 * in its old DBAL format to dnx data components
 * Old format: Size="DNX_DATA.mdb_app_db.fields.numeric_mdb_field___1"
 */
static shr_error_e
    ...
    uint8 allow_dynamic_fields)
{
    ...
    str_len = sal_strnlen(str_formula, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH);
    {
        ...
        while (index < str_len)
        {
            ...
            /*
             * '-' can be subtract sign but also part of '->' 
             */
    ...
    dbal_field_types_basic_info_t * fld_type_ent)
{
    ...
    for (ii = 0; ii < fld_type_prm->nof_enum_vals; ii++)
    {
        ...
        {
            /*
             * update enum value with invalid when empty name An enum value name is empty when it has not been
             * initialized. It happens for enum value from a different family, it will be marked as invalid 
             */
        ...
        {
            /** In case that the field is invalid */
        ...
        /** Consider replacing value_from_mapping to fld_type_ent->enum_val_info[ii].value */
    ...
    dbal_field_types_basic_info_t * fld_type_ent)
{
    ...
    /** Indicates field type with this name was initialized once */
    ...
    if (fld_type_ent->encode_info.encode_mode == DBAL_VALUE_FIELD_ENCODE_VALID_IND)
    {
        ...
        fld_type_ent->max_value = ((uint32) 1 << (fld_type_ent->max_size - 1)) - 1;     /* 2^(fld_type_ent->max_size-1)-1 
                                                                                         */
    }
    ...
    dbal_logical_table_t * table)
{
    ...
    {
        /*
         * in case we have a formula for entry offset, logical capacity will be smaller
         * than physical capacity.
         * In order to find it, we run over the physical capacity, and apply the formula
         * for each index (access id). We break when the result exceeds the physical capacity.
         *
         * NB: This works only when entry offset depends on the access id only.
         * If in the future we have a case where entry offset depends on another variable,
    ...
    table_db_struct_t * table_params)
{
    ...
    switch (table->access_method)
    {
        ...
        case DBAL_ACCESS_METHOD_MDB:
            /** the max capacity for MDB should be updated according to HW table capacity*/
            table->max_capacity = 0;
            break;
        ...
        {
            {
                ...
                {
                    /**  max capacity is calculated by dbal_sw_table_nof_entries_calculate */
                    table->max_capacity = 0;
                }
            }
            break;
...
/** Precondition. field_id has been set to field */
...
/** Precondition. field_id, field_nof_bits, offset_in_logical_field have been set to field
 *  set const value. Set max and min with const if valid */
static shr_error_e
...
/** Precondition. field_id, field_nof_bits, offset_in_logical_field have been set to field
 *  table_name has been set to table */
static shr_error_e
...
/** Precondition. field_id, have been set to field */
static shr_error_e
...
/** Precondition. field_id, field_type, field_nof_bits, offset_in_logical_field have been set to field */
static shr_error_e
...
/** Precondition. nof_instances, field_id, field_type, access_method have been set to field */
static shr_error_e
    ...
    uint8 is_validate_nof_instances)
{
    ...
    for (jj = 1; jj < field_info->nof_instances; jj++)
    {
        ...
        /*
         * Set field id to basic field id
         */
...
/** Precondition: nof_key_field has been set to table */
void
...
/**
 * Initializes the conditions for a given access
 */
static shr_error_e
    ...
    uint8 *has_valid_interface_field)
{
    ...
    /** for TCAM tables, the field is always considered as valid since tcam doesn't use fields it uses KEY/RESULT/MASK  */
    else
    {
...
/** get number of access per access type (mem/reg/sw)
    count the num of access for each access type, for dynamic memory allocation 
    check if the field is valid in the interface (otherwise no need to allocate memory for it 
*/
static shr_error_e
...
/** alloc and init with default values l2p fields */
static shr_error_e
...
/** get the nof access per access types.
 *  alloc and init with default values l2p fields */
static shr_error_e
    ...
    table_db_struct_t * table_params)
{
    ...
    if (cs_tcam_db_stage_info.supports_half_entries)
    {
        ...
        /*
         * Verify that the table really is used as half entry.
         */
    ...
    dbal_logical_table_t * table_info)
{
...

    {
        ...
        {
            /** for ETM_PP_DESCRIPTOR_EXPANSION table there is a need to initialize since !std_1 is not handled in FP. */
            if (table_id != DBAL_TABLE_ETM_PP_DESCRIPTOR_EXPANSION)
            {
                SHR_EXIT();
            }
    ...
    dbal_logical_table_t * table_entry)
{
    ...
    switch (table_entry->access_method)
    {
        ...
        case DBAL_ACCESS_METHOD_HARD_LOGIC:
            ...
            /*
             * sal_memset(cur_table_param->hl_access, 0x0, table_entry->nof_result_types *
             * sizeof(table_db_hl_access_struct_t));
             */
            break;
        case DBAL_ACCESS_METHOD_SW_STATE:
            /** unlike other access method, the don't init a sw_access
             *  because indirect_table_size is shared with other
    ...
    multi_res_info_t * multi_res_info)
{
    ...
    /** keep the pointer of the results_info */
...
/**
* extracted from dbal_db_init_logical_tables
**/
shr_error_e
    ...
    dbal_access_method_e access_method)
{
    ...
    /** in-case that the access method of the table is KBP and KBP is not enabled, no need to initialize the table */
...
/**
 * Init field param struct with default values for optional field properties.
 */
void
...
/**
 * \brief The function adds a new field to the superset field 
 *        array. The relevant field attributes are copied from
 *        the attributes of the added field.
 *  
 * \param [in] unit - unit id
 * \param [out] superset_res_type - DBAL definitions for the
 *         superset result type that gets modified.
 * \param [in] field_idx - An index to the field IDs array
 *         within the result type struct. That's the location of
 *        the added field.
 * \param [in] field_info - A pointer to the attributes of the
 *          added field.
 * \return
 *   void
 * \see
...
/**
 * \brief The function modifies the attributes of a specified
 *        field within the superset result type. This is
 *        performed whenever there's an additional appearance of
 *        the same field in another result type or when a
 *        subfield is merged into a parent field.
 * \param [in] unit - unit id
 * \param [out] superset_res_type - DBAL static definitions for
 *        the superset result type that may get modified.
 * \param [in] field_idx - An index to the field IDs array
 *        within the result type struct. That's the location of
 *        the existing appearance of the field.
 * \param [in] field_info - A pointer to the new field appearance
 *        that is added.
 * \return
 *   shr_error_e
 * \see
    ...
    dbal_table_field_info_t * field_info)
{
    ...
    /*
     * Adjust max size attributes to the higher value between the existing 
     * field attribiutes and those of the add field apperance. 
     * Adjust min size attributes to the lowest value.
     */
...
/**
 * \brief The function merges subfield attributes in a superset
 *        result type onto attributes of the parent field. The
 *        function then removes the merged subfield from its
 *        location in the superset fields array.
 * \param [in] unit - unit id
 * \param [out] superset_res_type - DBAL definitions for the 
 *        superset result type that gets modified.
 * \param [in] field_idx - An index to the field IDs array
 *        within the result type struct. That's the location of
 *        the parent field.
 * \param [in] nof_merge_sub_fields - The number of subfield
 *        merges to perform.
 * \param [in] merge_sub_field_idx - A list of subfield
 *        locations within the superset field array.
 * \return
 *   shr_error_e
 * \see
 */
    ...
    int *merge_sub_field_idx)
{
    ...
    /*
     * Traverse the list of subfields and merge them all
     */
    for (child_idx = 0; child_idx < nof_merge_sub_fields; child_idx++)
    {
        /*
         * Modify the parent field attributes with subfield's attributes 
         * Adjust the given subfield index in the array as the array decreases 
         * in size as the subfield is merged. 
         */
        ...
        /*
         * Remove the subfield from the superset by shifting all the succeeding fields 
         * in the array and clearing the last field. 
         */
 ...
 * field with the given field ranges that exists in one of the
 ...
 * In the case of Parent and Child fields defined in the same 
 * table, the Child fields are merged into the Parent field, and 
 * only the Parent field is left.
 * \param [in] unit - unit id
 * \param [in] superset_res_type - A DBAL pointer to structure 
 *        that stores the Superset result type attributes for a
 *        specific DBAL table.
 * \param [in] field_info - A pointer to a static field
 *        information in the context of a specific result type.
 * \return
 *   shr_error_e
 * \see
    ...
    dbal_table_field_info_t * field_info)
{
     ...
     * In case of a parent field, all encountered subfields are arrnaged in an array 
     * In caes of a subfield, it's merged to a parent if one exists in the array 
     */
    ...
    {
        /*
         * Check whether the updated field is a subfield of the field in the superset array or 
         * the exact field id was found 
         */
        ...
        /*
         * Check whether the updated field is a parent of the field in the superset array. 
         * In that case store the index of the subfield in an array of subfields
         */
    ...
    {
        /*
         * In case the new field is a Parent field add it at the start of the array, 
         * right after the result type field, as we need the Parent fields to appear 
         * before the child fields.
         * Make the necessary adjustments to the array
         */
        if (SHR_BITGET(field_info->field_indication_bm, DBAL_FIELD_IND_IS_PARENT_FIELD))
        {
            field_idx = 1;
        ...
        /*
         * Initialize field location with the relevant field attributes
         */
        dbal_superset_result_type_field_add(unit, superset_res_type, field_idx, field_info);

        /*
         * In case of a newly added Parent field, merge any existing subfields
         */
        if (SHR_BITGET(field_info->field_indication_bm, DBAL_FIELD_IND_IS_PARENT_FIELD))
        {
    ...
    dbal_logical_table_t * table)
{
    ...
    switch (table_id)
    {
        ...
        /** those tables does multiple get operations during init, adding them to here improve performance */
    ...
    dbal_logical_table_t * table)
{
    ...
    if (alloc_bit_num > 0)
    {
        ...
        /*
         * Allocate iterator optimized bitmap per core only in DPC,
         * in SBC, allocates only table->iterator_optimized[0]
         */
...
/** update indication bm for key & result field */
static shr_error_e
    ...
    dbal_logical_table_t * table)
{
     ...
     *  3. Calculate the total size of the key
     */
    ...
    dbal_logical_table_t * table)
{
     ...
     *  3. Calculate the total size of the payload
     *  4. Keep the max payload size and max num of result fields for all result types
     *  5. updating superset result type
     */
    max_payload_size = 0;
    ...
    /*
     * currently only a single static result type is supported for ETM_PP_DESCRIPTOR_EXPANSION since this table has also dynamic result types.
     */
    if ((table_id == DBAL_TABLE_ETM_PP_DESCRIPTOR_EXPANSION)
...
/** \brief
 * reverse instance position (bit offset in buffer) for a field with multiple instance */
static void
dbal_db_init_table_reverse_instance_order(
    dbal_table_field_info_t * field_info)
{
    /*
     * instance low designate the 1st part of the instance fields which position is swapped with the 2nd part of
     * instance fields (instance_high) 
     */
...
/** \brief
 * reverse instance position (bit offset in buffer) for multiple instance fields */
static void
...
/** \brief
 * reverse instance position (bit offset in buffer) for multiple instance key/result fields */
static void
    ...
    dbal_logical_table_t * table)
{
    ...
    /** fields order is rearranged - Invert the order of result buffer   */
    for (qq = 0; qq < table->nof_result_types; qq++)
    {
        result_type_set = &(table->multi_res_info[qq]);
                ...
                switch (hl_access_type)
                {
                    ...
                    case DBAL_HL_ACCESS_SW:
                        ...
                        curr_l2p_field->offset_in_interface =
                            ...
                            result_type_set->results_info[field_in_table].bits_offset_in_buffer;
                        /*
                         * we reuse access_offset variable to specify the exact bit offset in sw buffer
                         */
    ...
    dbal_logical_table_t * tables_info)
{
    ...
    for (table_id = 0; table_id < DBAL_NOF_STATIC_TABLES; table_id++)
    {
        ...
        /** skip validation for incompatible image tables. DBAL doesn't performs actions on this tables */
        if (table_status == DBAL_TABLE_INCOMPATIBLE_IMAGE)
        {
            continue;
        }
        ...
        if (!is_wb)
        {
            /** Mark that table was initialized */
