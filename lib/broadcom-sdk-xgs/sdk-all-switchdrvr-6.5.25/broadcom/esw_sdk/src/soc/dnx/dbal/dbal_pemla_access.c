...
/**
 * \brief 
 * Initialize dbal pemla register tables mappings according to the current
 * ucode mappings for table ID's, fieldID's and field sizes.
 * Table which is not found in Application DB (Pemla ucode db),
 * cannot be accessed using the pemla driver, so this table is set
    ...
    int unit)
{
    ...
    for (table_id = 0; table_id < DBAL_NOF_TABLES; table_id++)
    {
        ...
        /*
         * Init Pemla tables mapped to registers only (no key fields) 
         */
        ...
        {
            /*
             * Table info not found in Pemla driver ucode DB so we cannot access it using pemla driver
             * Table is disabled
             */
            table->maturity_level = DBAL_MATURITY_LOW;
...
/**
 * \brief
 * Initialize dbal pemla direct tables mappings according to the current
 * ucode mappings for table ID's, fieldID's and field sizes.
 * Table which is not found in Application DB (Pemla ucode db),
 * cannot be accessed using the pemla driver, so this table is set
 * to ML=0, and an Error status is set.
    ...
    int unit)
{
    ...
    for (table_id = 0; table_id < DBAL_NOF_TABLES; table_id++)
    {
        ...
        /*
         * Init Pemla tables mapped to memory only (at least one key field) 
         */
        ...
        {
            /*
             * Table info not found in Pemla driver ucode DB so we cannot access it using pemla drive
             * disable table.
             */
            table->maturity_level = DBAL_MATURITY_LOW;
            SHR_IF_ERR_EXIT(dbal_tables_update_hw_error(unit, table_id));
        ...
        for (res_type_index = 0; res_type_index < table->nof_result_types; res_type_index++)
        {
            ...
            for (field_index = 0; field_index < multi_res_info->nof_result_fields; field_index++)
            {
                ...
                /*
                 * Pem table has only one field including all the dbal fields After the field division is done on PEMLA 
                 * side, we can put this code back, for mapping from dbal to pem fields 
                 */
                for (pem_field_index = 0; pem_field_index < pemla_mem_info[pem_mem_index].nof_fields; pem_field_index++)
                {
                    ...
                    /*
                     * TBD no field_name in the direct struct 
                     */
                    if ((sal_strncasecmp
                         (field_name, pem_field_info->field_name, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0))
                    {
                ...
                /*
                 * This code to should be moved back 
                 */
...
/** ****************************************************
* \brief
* Set entry to default value (0)
*
*  \par DIRECT INPUT:
*    \param [in] unit
*    \param [in] entry_handle -
*      dbal entry handle - holds the entry information
*    \param [out] pemla_drv_mem_out - returns the
*       filled pemla structure
*  \par DIRECT OUTPUT:
*    shr_error_e - \n
*    Error code
*****************************************************/
    ...
    pemladrv_mem_t ** pemla_drv_mem_out)
{
    ...
    *pemla_drv_mem_out = NULL;
    ...
    {
        /*
         * For now pem table data is not divided by field, so we see it as raw data, and simply clear it all 
         */
        nof_results_field = 1;
    }
...
/** ****************************************************
* \brief
* The function is used to clear the entry in a DBAL PEMLA table of
* type pemla_reg. Writes 0 for each result field.
*
*  \par DIRECT INPUT:
*    \param [in] unit
*    \param [in] entry_handle - \n
...
* \brief
* The function is used to clear the entry in a DBAL PEMLA table of
* type pemla_memory. Writes 0 for each result field.
*
*  \par DIRECT INPUT:
*    \param [in] unit
*    \param [in] entry_handle - \n
    ...
    switch (entry_handle->table->table_type)
    {
        ...
        case DBAL_TABLE_TYPE_DIRECT:
            ...
            else if (entry_handle->table->pemla_mapping.access_type == DBAL_HL_ACCESS_MEMORY)
            {
                ...
                /*
                 * SHR_IF_ERR_EXIT(dbal_pemla_table_pemla_mem_entry_set(unit, entry_handle)); 
                 */
            }
            else
            {
...
* type pemla_mem
* The function assumes it reads all fields from hw
    ...
    switch (entry_handle->table->table_type)
    {
        ...
        case DBAL_TABLE_TYPE_DIRECT:
            ...
            else if (entry_handle->table->pemla_mapping.access_type == DBAL_HL_ACCESS_MEMORY)
            {
                ...
                /*
                 * SHR_IF_ERR_EXIT(dbal_pemla_table_pemla_mem_entry_get(unit, entry_handle)); 
                 */
            }
            else
            {
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    switch (entry_handle->table->table_type)
    {
        ...
        case DBAL_TABLE_TYPE_DIRECT:
            ...
            {
                /*
                 * dbal_pemla_table_pemla_reg_entry_clear is not functional, using 
                 * dbal_pemla_table_pemla_reg_clear_entry instead.
                 */
                /*
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    while ((!entry_found) && (!iterator_info->is_end))
    {
        ...
        if (is_valid_entry == DBAL_KEY_IS_VALID)
        {
            ...
            {
                /*
                 * if the entry is not equal to the default entry it is returned.
                 */
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    /*
     * dbal_pemla_table_pemla_reg_entry_clear is not functional,
     * using dbal_pemla_table_pemla_reg_clear_entry instead.
     */
    /*
     * SHR_IF_ERR_EXIT(dbal_pemla_table_pemla_reg_entry_clear(unit, entry_handle));
     */
