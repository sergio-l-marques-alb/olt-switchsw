    ...
    pemladrv_mem_t * data)
{
  ...
  if (api_info[unit].db_direct_container.db_direct_info_arr == NULL || api_info[unit].db_direct_container.db_direct_info_arr[db_id].result_chunk_mapper_matrix_arr == NULL) {
    /*sal_printf("Warning: PEM Db-id %d was NOT mapped during the application run. Unable to write.\n", db_id);*/
    return 0;
}
    ...
    const pemladrv_mem_t * data)
{
  ...
  if (cam_info->result_chunk_mapper_matrix_arr == NULL || cam_info->result_chunk_mapper_matrix_arr->db_chunk_mapper == NULL) {
    /*sal_printf("Warning: PEM Db was NOT mapped during the application run. Unable to write.\n");*/
    return 0;
  }
    ...
    int *index)
{
  ...
  if (key->nof_fields != nof_fields_in_key) {
    /*sal_printf("ERROR. One or More fields are missing while trying to write key. Exact-Match - #%d id\n", table_id); */
    sal_free(virtual_key_data);
  ...
  if (next_free_index == -1) {
    /*sal_printf("The DB is already full, Can't write or add keys.\n");*/
    sal_free(virtual_key_data);
  ...
  *index = dnx_pemladrv_does_key_exists_in_physical_db(unit, &api_info[unit].db_em_container.db_em_info_arr[table_id].logical_em_info, api_info[unit].db_em_container.db_em_info_arr[table_id].em_cache.physical_entry_occupation, api_info[unit].db_em_container.db_em_info_arr[table_id].em_cache.em_key_entry_arr, virtual_key_data);
  if (*index != -1) {
    /*sal_printf("Key already exists. Key index is %d\n", *index);*/
    sal_free(virtual_key_data);
    ...
    int *index)
{
  ...
  *index = dnx_pemladrv_find_physical_entry_index_by_key(unit,&api_info[unit].db_em_container.db_em_info_arr[table_id].logical_em_info, api_info[unit].db_em_container.db_em_info_arr[table_id].em_cache.physical_entry_occupation, api_info[unit].db_em_container.db_em_info_arr[table_id].em_cache.em_key_entry_arr, key);
  if (*index == -1) {
    /*sal_printf("Key doesn't exist in DB\n");*/
    ...
    pemladrv_mem_t * result)
{
  ...
  if (api_info[unit].db_em_container.db_em_info_arr[table_id].em_cache.physical_entry_occupation[index] == 1) {
    /*sal_printf("Error. Entry is already occupied.\n");*/
    ...
    int *index)
{
  ...
  if(uint_len_in_bits > api_info[unit].db_lpm_container.db_lpm_info_arr[table_id].logical_lpm_info.total_key_width) {
    /*sal_printf("ERROR. Wrong prefix in: Exact-Match - #%d id\n", table_id);*/
    sal_free(virtual_key_data);
  ...
  *index = dnx_pemladrv_does_key_exists_in_physical_lpm_db(unit, &api_info[unit].db_lpm_container.db_lpm_info_arr[table_id].logical_lpm_info, uint_len_in_bits, &api_info[unit].db_lpm_container.db_lpm_info_arr[table_id].lpm_cache, virtual_key_data);
  if (*index != -1) {
    /*sal_printf("Warning - Key already exists. Key index is %d. OVERWRITING.\n", *index);*/
    sal_free(virtual_key_data);
    ...
    pemladrv_mem_t ** result)
{
  ...
  /*check id db was mapped in the application*/
  if (api_info[unit].db_direct_container.db_direct_info_arr == NULL || api_info[unit].db_direct_container.db_direct_info_arr[db_id].result_chunk_mapper_matrix_arr == NULL) {
    /*sal_printf("Warning: PEM Db-id %d was NOT mapped during the application run.\n", db_id);*/
    return 1;
  }
    ...
    pemladrv_mem_t ** result)
{
  ...
  if (api_info[unit].db_tcam_container.db_tcam_info_arr[db_id].result_chunk_mapper_matrix_arr == NULL ||
    ...
    /*sal_printf("Warning: PEM Db was NOT mapped during the application run\n")*/;
    return 1;
  }
...
int dnx_pemladrv_pem_mem_virtual_bist(int unit){
  ...
  if (ret_val){
    /*sal_printf("PEM virtual MBIST FAILED!\n");*/
  }
  return ret_val;
}
    ...
    const unsigned int *value)
{
  ...
  /*sal_printf("%s: address=32'h%X, value=%d'h%s, row_index=%d\n", prefix, address, length_in_bits, hex_value, row_ndx);*/
  sal_free (hex_value);
}
    ...
    void* entry_data)
{
    ...
    {
        /*
         * Assuming Industrial TCAM CS has 128 entries
         */
        ...
        /**
         * As ucode also calls that function for odd lines,
         * need to change key and mask indexes in case of odd line.
         */
        ...
        /* get the data and the valid bits from the replied data */
        ...
        /* get the data from the replied data */
        ...
        /**
         *  Build the full entry key+mask
         *  WR command in bit 0 = 1
         *  VALID bits in bits 3-4 = valid_bits
         *  KEY data in bits 5-204 = key data
         *  MASK data in bits 205-404 = mask data
         *
         */
    ...
    void *entry_data)
{
  ...
  int entry_in_cache = -1;
    ...
    if (!currently_writing_applets)
    {
        ...
        {
            /*
             * Read industrial TCAM according to TCAM interface.
             * Assuming that the code here will only hit CS tables.
             */
