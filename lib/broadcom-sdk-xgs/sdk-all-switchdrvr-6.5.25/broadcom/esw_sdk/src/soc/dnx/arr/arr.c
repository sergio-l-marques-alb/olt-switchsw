...
/**
 *  \brief update arr_history history state
 */
static shr_error_e
...
/**
 *  \brief if history present update table_id and result_type hw value to parse data entries
 */
shr_error_e
    ...
    char to_n[RHNAME_MAX_SIZE])
{
    ...
    for (field_idx = 0; field_idx < table->multi_res_info[result_type].nof_result_fields; field_idx++)
    {
        ...
        {
            /*
             * skip counting result_type for ETPS data entries as the field is not present in HW
             */
