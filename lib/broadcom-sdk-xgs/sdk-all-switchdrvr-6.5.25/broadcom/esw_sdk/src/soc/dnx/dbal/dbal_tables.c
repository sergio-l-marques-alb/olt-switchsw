...
/** Array of pointers, to be allocated dynamically for each unit */
...
#if defined(BCM_WARM_BOOT_SUPPORT)
    /*
     * Skip alloc if warmboot test mode on
     */
    ...
    switch (access_method)
    {
        ...
        case DBAL_ACCESS_METHOD_KBP:
            ...
            if (table->kbp_handles)
            {
                ...
                /*
                 * For KBP it is not required to write the handles to DBAL table access info
                 * and use them to restore the tables.
                 * Restoring the tables will be performed in KBP manager.
                 */
            }
            else
            {
    ...
    xml_node top_node)
{
    ...
    RHDATA_ITERATOR(appdb_node, top_node, "AppDB")
    {
        ...
        RHDATA_ITERATOR(results_node, multiple_results_node, "Result")
        {
            ...
            /*
             * Check if the current result type is valid 
             */
    ...
    dbal_tables_e table_id)
{
    ...
    if (table_status == DBAL_TABLE_NOT_INITIALIZED)
    {
    /** table ETM_PP_DESCRIPTOR_EXPANSION was not initialized no need to restore the result types */
        SHR_EXIT();
    }
    ...
    dbal_tables_e table_id)
{
    ...
    /*
     * Delete table info and update status.
     */
    SHR_IF_ERR_EXIT(dbal_tables_dbal_logical_table_t_clear(unit, table));
    ...
    dbal_tables_e * next_table_id)
{
    ...
    for (table_counter = table_id + 1; table_counter < nof_tables; table_counter++)
    {
        ...
        {
            /*
             * Skip the table if none of the associated physical DBs match the requested physical DB 
             */
    ...
    dbal_fields_e * next_field_id)
{
    ...
    {
        /**
         * for pemla tables, there are some fields that can be invalid, so need to return next valid field 
         */
    ...
    else
    {
        for (fields_counter = 0; fields_counter < nof_fields; fields_counter++)
        {
            ...
            {
                /** return the next field or DBAL_FIELD_EMPTY if it is the last field */
    ...
    if (!field_found)
    {
        /** expect that the field_id will be found in table */
    ...
    int *app_db_size)
{
    ...
    {
        /** Return NOT_FOUND error if app ID is 0 (not set) */
        SHR_SET_CURRENT_ERR(_SHR_E_NOT_FOUND);
        SHR_EXIT();
    }
...
/** returns the max key value for key size that is not bigger than 32 bit. Max key size is the calculated without the
 *  core_id */
shr_error_e
    ...
    uint32 *key_buffer)
{
    ...
    for (ii = 0; ii < table->nof_key_fields; ii++)
    {
        ...
        {
            /** for enums, we are looking for the MAX value of the HW value. */
            int enum_iter;
            dbal_field_types_basic_info_t *field_type_info;
            key_field_max_val = 0;
...
/**
 * \brief
 * Updates the entries counters (physical and logical)
 */
shr_error_e
    ...
    uint8 is_add_operation)
{
     ...
     *  is only indication that the table is ACL table and not really direct table. */
    ...
    if (table->access_method == DBAL_ACCESS_METHOD_MDB)
    {
        for (jj = 0; jj < table->nof_physical_tables; jj++)
        {
            ...
            {
                /*
                 * Get nof logical entries, and decrement it only (we don't want to damage other tables mapped to
    ...
    /**negative number set to Zero (means that the operation is table clear)*/
    if (num_of_entries_to_update < 0)
    {
