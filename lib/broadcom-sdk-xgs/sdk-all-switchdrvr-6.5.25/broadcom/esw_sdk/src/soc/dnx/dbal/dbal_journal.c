...
/*
 * \brief - notify the managers when entry is attempted to be inserted in journal.
 */
...
/**
 * \brief check if dbal entry should be excluded
 */
static uint8
...
{
    /*
     * list of permanently excluded tables for comparison journal
     */
    dbal_tables_e comparison_perm_excluded[] = {
        ...
        /*
         * MEP Profile is exchange instead of clear by bcm_oam_delay_delete() and bcm_oam_loss_delete() 
         */
        ...
        /*
         * Exclude below 2 FlexE tables as they won't restore to default setting
         */
        DBAL_TABLE_TINY_MAC_RX_LSS_CTRL,
        DBAL_TABLE_NIF_FEU_TINY_MAC_CTRL,
        /*
         * Unique case where the following two tables are excluded due to being mapped to the same register fields.
         */
    ...
    /*
     * list of permanently excluded tables for rollback journal
     */
    ...
    if (!is_comparison)
    {
        ...
        /*
         * check permanently excluded table ids
         */
...
/**
 * \brief - rollback a dbal journal snapshot entry
 */
static shr_error_e
...
/**
 * \brief - dbal journal entry rollback callback
 */
static shr_error_e
    ...
    char *stamp)
{
    ...
    /*
     * Sanitizer flags if handle was journaled with any flags being set
     */
...
/**
 * \brief - masks "trigger"and read-only values field data in dbal handle payload.
 */
static shr_error_e
    ...
    uint8 *ptr_compare_payload)
{
    ...
    for (result_type_id = 0; result_type_id < table->nof_result_types; result_type_id++)
    {
        ...
        for (result_id = 0; result_id < result_set.nof_result_fields; result_id++)
        {
            ...
            /*
             * Mask the field in case it is a trigger or readonly readonly is masked as it cannot change directly by
             * dbal commit and some times these fields are clear-on-read which can cause false positive comparison
             * errors 
             */
            ...
            /*
             * we have found at least one comparable field
             */
    ...
    char *stamp)
{
    ...
    /*
     * Sanitize er flags if handle was journaled with any flags being set
     */
    ptr_payload->er_flags = 0x0;
        ...
        if (is_macsec_table)
        {
            ...
            if (SHR_FAILURE(local_rc))
            {
                LOG_ERROR(BSL_LOG_MODULE,
                          (BSL_META_U
                           (unit, "Error in dbal comparison journal. table %s. Error %d during restore power down\n"),
    ...
    /*
     * mask "trigger" value field data might be changed by HW.
     * introduces issues for comparison journal, therefore omitted.
     */
    SHR_IF_ERR_EXIT(dnx_dbal_journal_handle_trigger_value_mask
                    (unit, ptr_payload, get_entry_handle, &is_compare_payload));

    /*
     * check if entry doesn't exist or it is a 'default' entry
     */
    ...
    /*
     * get_entry_handle - can be empty if it doesn't exist ( "default entry", not always zeroes )
     */
    ...
    {
        /*
         * the entry should be in the same state as it was prior
        ...
        /*
         * compare keys and key types for TCAM entries
         */
        ...
        /*
         * print both entries and return an error
         */
        if (!entries_match)
        {
            LOG_CLI((BSL_META("\n################################################################################\n")));
...
/**
 * \brief - check if two dbal journal comparison entries are distinct
 */
static shr_error_e
    ...
    uint8 *result)
{
    ...
    /**
     * Entries are considered to be the same if:
     * 1. table ids match
     * 2. contents of key fields match
     * 3. core_id match:
     *      - core_ids are equal or
     *      - second dbal entry core_id is DBAL_CORE_ALL
     */
    ...
    /*
     * special unsupported matching case
     * NOTE: In case of issues with this check, it can be avoided by splitting CORE_ALL actions into multiple
     *       single core entries. This will ensure that cases like asymmetrical commit/clear of dbal entries
     *       will not trigger an error, even though it is logically correct.
     */
    if (entries_match
    ...
    /*
     * entries are distinct if they are not the same
     */
...
/**
 * \brief - determine default entry state based on the access method of a dbal table associated with the entry.
 */
static shr_error_e
    ...
    dnx_rollback_journal_entry_state * entry_state)
{
    ...
    {
        /*
         * For HL/TCAM_CS/SW mark the entry as valid and non-invalidatable.
         * Entry WILL NOT be invalidated by an error in the dbal access function which produced that entry.
         * Entry WILL be attempted to be rolled-back in case of failure on the dbal access layer.
         *
         * No custom mechanism for rolling back failures in dbal access for these access types required.
         */
    ...
    {
        /*
         * For MDB/KBP/PEMLA mark the entry as valid and invalidatable.
         * If this is last entry inserted into the dbal journal,
         * it WILL be invalidated by an error in the dbal access function which produced that entry.
         * DBAL journal invalidated entries ARE NOT rolled-back, just destroyed.
         *
         * Custom mechanism for rolling back failures in dbal access for these access types required.
         * (NOT IN THE DBAL JOURNAL) - This was dropped for the time being and DBAL
         * Errors are considered non-recoverable.
         */
    ...
    {
        /*
         * New dbal access methods need to take under consideration the behavior of dbal journal.
         * The access method should either invalidate (or not) the last entry inserted in to the dbal journal in case of an error
         * during dbal access entry add / clear.
         */
...
/**
 * \brief - dbal journal entry insert
 */
static shr_error_e
    ...
    uint8 is_dbal_error)
{
    ...
    /*
     * infer entry state based on table access method
     */
    ...
    if (is_er && !dnx_dbal_journal_is_table_excluded(unit, er_thread_id, ptr_handle->table_id, FALSE)
        /*
         * errors that have occurred in the DBAL are not journaled except a few specific cases described in
         * dnx_dbal_rollback_journal_entry_state_get for which the journal entry state is set to ALWAYS_VALID and the
         * operation is journaled even if an error occured. 
         */
    ...
    {
        /*
         * comparison journal entries are currently always invalidatable.
         */
        SHR_IF_ERR_EXIT(dnx_rollback_journal_entry_new(unit,
                                                       er_thread_id,
    ...
    uint8 is_comparison)
{
    ...
    for (idx = 0; idx < DNX_ERR_RECOVERY_MAX_NOF_ACTIVE_TRANSACTION; idx++)
    {
        ...
        /*
         * clear all exclusions from previous comparison
         */
        if (is_comparison)
        {
    ...
    uint8 is_comparison)
{
    ...
    if (is_comparison)
    {
        SHR_IF_ERR_EXIT(dnx_rollback_journal_clear
                        (unit, er_thread_id, only_head_clear, DNX_DBAL_COMPARISON_JOURNAL_HANDLE));
    }
    else
    {
        SHR_IF_ERR_EXIT(dnx_rollback_journal_clear
                        (unit, er_thread_id, only_head_clear, DNX_DBAL_ROLLBACK_JOURNAL_HANDLE));
    }
    ...
    uint8 is_comparison)
{
    ...
    SHR_IF_ERR_EXIT(dnx_dbal_journal_clear_internal(unit, er_thread_id, only_head_clear, is_comparison));
    ...
    uint8 is_comparison)
{
    ...
    if (is_comparison)
    {
        ...
        distinct_rv = dnx_rollback_journal_distinct
            (unit, er_thread_id, DNX_DBAL_COMPARISON_JOURNAL_HANDLE, &dnx_dbal_comparison_journal_entries_are_distinct);

        rollback_rv = dnx_rollback_journal_rollback
            (unit, er_thread_id, only_head_rollback, DNX_DBAL_COMPARISON_JOURNAL_HANDLE);

        /*
         * exit with the relevant error code, depending on which part of the comparison check failed - distinct/rb 
         */
    ...
    uint32 table_id)
{
    ...
    /*
     * mark table as unsupported if error during table status retrieval
     */
    ...
    return (    /* 
                 * <--- end of HARD_LOGIC gold tables --->
                 */
               /*
                * <--- end of MDB gold tables --->
                */
               /*
                * <--- end of TCAM_CS gold tables --->
                */
               /*
                * <--- end of MDB tables ---->
                */
               /*
                * <--- end of HARD_LOGIC tables ---->
                */
               /*
                * <--- end of TCAM_CS  --->
                */
               ...
               /*
                * <---- end of tables that are too big, takes too long ---->
                */
               || (DBAL_TABLE_HBMC_INTERRUPTS == table_id)
               /*
                * <----- end of b0 adapter failures ----->
                */
               /*
                * <----- end of tables that never passed ----->
                */
               /*
                * <--- end of Permanently excluded --->
                */
        );
}
    ...
    uint32 table_id)
{
    ...
    /*
     * check if table contains any TRIGGER / READONLY / WRITEONLY fields,
     * for such cases we currently skip test creation
     */
    ...
    /*
     * check if the table is unsupported
     */
    return !dnx_dbal_journal_is_unsupported_table(unit, table_id);
}

/**
 * /brief - log an entry in the dbal journal
 */
static shr_error_e
    ...
    uint8 is_dbal_error)
{
    ...
    {
        /*
         * dont journal unsupported tables, skip error
         */
    ...
    /*
     * It is possible for the dbal phy entry key to be masked or unmasked while attempted to be inserted in journal.
     * This results in an issue for comparison journal which compares full entry keys.
     * Workaround is to always apply the key mask to the key for relevant table types and access methods.
     */
    ...
    uint32 *seq_id)
{
    ...
    SHR_IF_ERR_EXIT(dnx_dbal_journal_get_head_seq_id_internal(unit, er_thread_id, is_comparison, seq_id));
...
/**
 * \brief - update the access id of a handle, in case access id is available after DBAL commit.
 * Currently available for KBP TCAM DIRECT tables.
 */
static inline shr_error_e
...
/**
 * \brief - assumes that journal is on.
 */
static inline shr_error_e
    ...
    uint8 is_dbal_error)
{
    ...
    {
        /*
         * when access id is available after DBAL commit, need to just raise the flag.
         * don't need to update the access id or insert entry in the rollback journal if dbal error occured.
         */
        if (!is_dbal_error)
        {
    ...
    {
        /*
         * For MDB entries we cannot retrieve the payload mask from hardware.
         * Copy the the 'full' payload mask from the entry_handle instead.
         */
        ...
        /*
         * when access id is available after DBAL commit, copy the access id to the get handle.
         * don't need to update the access id or insert entry in the rollback journal if dbal error occured.
         */
        if (!is_dbal_error)
        {
    ...
    uint8 is_dbal_error)
{
    ...
    /*
     * at this point er_thread_id may not be set yet, if that's the case logging in not needed 
     */
    if (er_thread_id == -1)
    {
        SHR_EXIT();
    ...
    uint8 is_comparison)
{
    ...
    /*
     * Exclude table:
     * 1. find dbal table id by the given stamp,
     * 2. add it to the excluded tables in the current transaction and increment the number of excluded tables
     */
