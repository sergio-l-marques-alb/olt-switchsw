/** \file j2x_a0_data_ingr_reassembly.c
 * 
    ...
    int unit)
{
    ...
    DNXC_DATA_ALLOC(table->data, dnx_data_ingr_reassembly_context_context_map_t, (1 * (table->keys[0].size) * (table->keys[1].size) + 1 /* to store default value */ ), "data of dnx_data_ingr_reassembly_context_table_context_map");
    ...
    if (DNX_ALGO_PORT_TYPE_FLEXE_MAC < table->keys[0].size && 0 < table->keys[1].size)
    {
        data = (dnx_data_ingr_reassembly_context_context_map_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_ALGO_PORT_TYPE_FLEXE_MAC, 0);
