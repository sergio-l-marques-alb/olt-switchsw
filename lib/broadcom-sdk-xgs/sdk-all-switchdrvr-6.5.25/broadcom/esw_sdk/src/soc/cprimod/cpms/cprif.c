...
int cprif_encap_header_config_get( int unit, int port, uint32 queue_num, 
                                   cprimod_cpri_roe_frame_format_t frame_format,
                                   cprimod_encap_header_config_t* config_info)
{
    cprif_encap_header_entry_t entry;
    SOC_INIT_FUNC_DEFS;
    ...
    config_info->roe_frame_format       = frame_format;

   if (frame_format == CPRIMOD_CPRI_ROE_FRAME_FORMAT_IEEE1914) {
    ...
    } else if (frame_format == CPRIMOD_CPRI_ROE_FRAME_FORMAT_RSVD3) {
        config_info->version  = ((entry.ver & 0x03)<<2) | ((entry.pkttype & 0x30)>>4);
        config_info->flow_id  = entry.flow_id | ((entry.pkttype & 0x0f)<<8);

    } else if (frame_format == CPRIMOD_CPRI_ROE_FRAME_FORMAT_ECPRI8) {
        config_info->ecpri_msg_type = entry.roe_opcode;
        config_info->ecpri_pc_id    = ((entry.ver & 0x03) << 14) |
                                      ((entry.pkttype & 0x3F) << 8) |
                                      entry.flow_id;
    } else if (frame_format == CPRIMOD_CPRI_ROE_FRAME_FORMAT_ECPRI12) {
        config_info->ecpri_msg_type         = entry.roe_opcode;
        config_info->ecpri_pc_id            = entry.flow_id;
...
int cprif_tx_framer_tgen_enable_set(int unit, int port, uint32 enable)
{
    SOC_INIT_FUNC_DEFS;

    _SOC_IF_ERR_EXIT
        (cprif_drv_tx_framer_tgen_enable_set(unit, port, enable));
...
int cprif_tx_framer_tgen_enable_get(int unit, int port, uint32 *enable)
{
    SOC_INIT_FUNC_DEFS;

    _SOC_IF_ERR_EXIT
        (cprif_drv_tx_framer_tgen_enable_get(unit, port, enable));
...
int cprif_header_compare_entry_for_flow_get ( int unit, int port,
                                              uint8 flow_id,
                                              cprimod_flow_type_t flow_type,
                                              cprimod_header_compare_entry_t *entry)
{
    cprimod_header_compare_entry_t local_entry;
    int index;
        ...
        if (local_entry.valid == 1) {
            ...
            if ((local_entry.flow_id   == flow_id) &&
                (local_entry.flow_type == flow_type)) {
                    ...
                    entry->match_data   = local_entry.match_data;
                    entry->mask         = local_entry.mask;
                    return SOC_E_NONE;
            }
        }
    }
...
int cprif_port_rsvd4_rx_interface_config_set(int unit, int port, const phymod_phy_access_t* phy,
    ...
    SOC_IF_ERROR_RETURN(cprif_drv_rsvd4_rxpcs_speed_set(unit, port, phy,
                                                        config->speed));
exit:
    SOC_FUNC_RETURN;
...
STATIC
int _cpri_core_firmware_load(const phymod_core_access_t* core, phymod_firmware_load_method_t load_method, cprimod_firmware_loader_f fw_loader)
{
    SOC_INIT_FUNC_DEFS;
    /*
                    ...
                    const cprimod_port_init_config_t* port_config)
{
    ...
    sal_memset(&serdes_firmware_config, 0, sizeof(serdes_firmware_config));
    sal_memcpy(&phy_access_copy, phy, sizeof(phy_access_copy));
    phy_access_copy.access.pll_idx = 0;
...
int cprif_cpri_port_cw_fast_eth_config_set(int unit, int port, const cprimod_fast_eth_config_info_t* config_info)
{
    ...
    if (config_info->min_packet_size > 0xFF) {
        _SOC_EXIT_WITH_ERR(SOC_E_PARAM, ("Min packet size %d is out of range. max is 0xFF.\n",
                                         config_info->min_packet_size));
    }
...
int cprif_rx_tag_config_get(int unit, int port, uint32 *default_tag, uint32 *no_match_tag)
{
    SOC_INIT_FUNC_DEFS;

    _SOC_IF_ERR_EXIT
        (cprif_drv_rx_tag_config_get( unit, port, default_tag, no_match_tag));
...
int cprif_rsvd4_tx_control_flow_group_member_get( int unit, int port, uint32 group_id, uint32 priority,
                                                  uint32 *queue_num,
                                                  cprimod_control_msg_proc_type_t *proc_type)

{
    ...
    if (entry.valid_mask & (0x1 << priority)) {
        *queue_num  = entry.queue_num[priority];
        _cprif_to_cprimod_proc_type(entry.proc_type[priority], proc_type);
    } else {
        return SOC_E_NOT_FOUND;
    }
...
int cprif_rsvd4_tx_control_flow_group_member_get_all ( int unit, int port, uint32 group_id,
                                                       cprimod_rsvd4_control_group_entry_t* group_config)
{
    cprif_control_group_entry_t entry;
    int priority;
    SOC_INIT_FUNC_DEFS;
    ...
    group_config->valid_mask = entry.valid_mask;
    for (priority = 0; priority < CPRIF_TX_CONTROL_MAX_MEMBER; priority++) {
        group_config->queue_num[priority]   = entry.queue_num[priority];
        group_config->type[priority]        = entry.proc_type[priority];
    }
...
int cprif_rsvd4_tx_single_tunnel_message_config_get(int unit, int port, cprimod_cpri_crc_option_t *crc_option)
{
    uint32 local_crc_option;
    SOC_INIT_FUNC_DEFS;

    _SOC_IF_ERR_EXIT
        (cprif_drv_tx_control_single_tunnel_msg_config_get(unit, port, &local_crc_option));

    if (local_crc_option == 1) {
        *crc_option = CPRIMOD_CPRI_CRC_REGENERATE;
    } else {
        *crc_option = CPRIMOD_CPRI_CRC_USE_ROE;
    }
exit:
    SOC_FUNC_RETURN;
...
int cprif_rsvd4_tx_control_flow_config_get(int unit, int port, uint32 flow_id, cprimod_rsvd4_tx_config_info_t *config_info)
{
    ...
    uint32 queue_num;
    cprif_cpri_tx_fast_eth_config_t tx_fe_config;
    SOC_INIT_FUNC_DEFS;
    ...
    if(config_info->proc_type == CPRIMOD_CONTROL_MSG_PROC_TYPE_FE){
        sal_memset(&tx_fe_config, 0, sizeof(cprif_cpri_tx_fast_eth_config_t));
        _SOC_IF_ERR_EXIT
            (cprif_drv_cpri_port_tx_cw_fast_eth_config_get(unit, port, &tx_fe_config));
        config_info->crc_mode = tx_fe_config.crc_mode;
    }
...
int cprif_rsvd4_fast_eth_config_get(int unit, int port, cprimod_cpri_fast_eth_config_t* config_info)
{
    cprif_cpri_rx_fast_eth_config_t rx_config;
    SOC_INIT_FUNC_DEFS;
    ...
    _SOC_IF_ERR_EXIT
        (cprif_drv_cpri_port_rx_cw_fast_eth_config_get(unit, port, &rx_config));
    config_info->queue_num          = rx_config.queue_num ; /* Not used for for RSVD4 */
    config_info->no_fcs_err_check   = rx_config.ignore_fcs_err;
    config_info->min_packet_drop    = rx_config.min_packet_drop;
    config_info->max_packet_drop    = rx_config.max_packet_drop;
    config_info->min_packet_size    = rx_config.min_packet_size;
...
int cprif_cpri_port_tx_enable_set(int unit, int port, int val)
{
    ...
    _SOC_IF_ERR_EXIT(cprif_drv_tx_frame_optional_config_set(unit, port,
                     CPRIMOD_TX_CONFIG_TX_PMD_DISBALE_OVERRIDE_VAL, (val&1)^1));
    _SOC_IF_ERR_EXIT(cprif_drv_tx_frame_optional_config_set(unit, port,
                     CPRIMOD_TX_CONFIG_TX_PMD_DISBALE_OVERRIDE_EN, (val&1)^1));
...
int cprif_cpri_port_tx_enable_get(int unit, int port, int* val)
{
    ...
    _SOC_IF_ERR_EXIT(cprif_drv_tx_frame_optional_config_get(unit, port,
                     CPRIMOD_TX_CONFIG_TX_PMD_DISBALE_OVERRIDE_VAL, &tmpval0));
    _SOC_IF_ERR_EXIT(cprif_drv_tx_frame_optional_config_get(unit, port,
                     CPRIMOD_TX_CONFIG_TX_PMD_DISBALE_OVERRIDE_EN, &tmpval1));
    /* make val ^1 to convert disable to enable */
...
int cprif_cpri_port_clock_disable_set(int unit, int port, int rst)
{
    SOC_INIT_FUNC_DEFS;

    _SOC_IF_ERR_EXIT
        (cprif_drv_disable_glas_clk_set(unit, port, rst));
    _SOC_IF_ERR_EXIT
        (cprif_drv_disable_rx_clk_set(unit, port, rst));
    _SOC_IF_ERR_EXIT
        (cprif_drv_disable_tx_clk_set(unit, port, rst));
    _SOC_IF_ERR_EXIT
        (cprif_drv_disable_cip_clk_set(unit, port, rst));
...
cprimod_cpri_ecpri_header_field_t _cprif_drv_to_cprimod_ecpri_header_field(uint32 cprif_drv_field)
{
    ...
    switch (cprif_drv_field) {
        ...
        case CPRIF_DECAP_HEADER_FIELD_ID_HDR_BYTE_2:
            cprimod_field = CPRIMOD_CPRI_ECPRI_HEADER_FIELD_BYTE_2;
            break;

        case CPRIF_DECAP_HEADER_FIELD_ID_HDR_BYTE_3:
            cprimod_field = CPRIMOD_CPRI_ECPRI_HEADER_FIELD_BYTE_3;
            break;

        case CPRIF_DECAP_HEADER_FIELD_ID_HDR_BYTE_4:
            cprimod_field = CPRIMOD_CPRI_ECPRI_HEADER_FIELD_BYTE_4;
            break;

        case CPRIF_DECAP_HEADER_FIELD_ID_HDR_BYTE_5:
            cprimod_field = CPRIMOD_CPRI_ECPRI_HEADER_FIELD_BYTE_5;
            break;

        case CPRIF_DECAP_HEADER_FIELD_ID_HDR_BYTE_6:
            cprimod_field = CPRIMOD_CPRI_ECPRI_HEADER_FIELD_BYTE_6;
            break;

        case CPRIF_DECAP_HEADER_FIELD_ID_HDR_BYTE_7:
            cprimod_field = CPRIMOD_CPRI_ECPRI_HEADER_FIELD_BYTE_7;
            break;
        ...
        case CPRIF_DECAP_HEADER_FIELD_ID_HDR_BYTE_9:
            cprimod_field = CPRIMOD_CPRI_ECPRI_HEADER_FIELD_BYTE_9;
            break;

        case CPRIF_DECAP_HEADER_FIELD_ID_HDR_BYTE_10:
            cprimod_field = CPRIMOD_CPRI_ECPRI_HEADER_FIELD_BYTE_10;
            break;

        case CPRIF_DECAP_HEADER_FIELD_ID_HDR_BYTE_11:
            cprimod_field = CPRIMOD_CPRI_ECPRI_HEADER_FIELD_BYTE_11;
            break;
...
int cprif_cpri_port_tx_pipeline_clear(int unit, int port)
{
    ...
    /* Disable TGEN */
    _SOC_IF_ERR_EXIT
        (cprif_tx_framer_tgen_enable_set(unit, port, FALSE));
...

{
    ...
    if (port_config->interface == cprimodCpri) {
        ...
        if (table == cprimod_basic_frame_table_0) {
            _SOC_IF_ERR_EXIT
                (cprif_basic_frame_parser_active_table_set(unit, port, cprimod_basic_frame_table_1, FALSE));
        } else {
            _SOC_IF_ERR_EXIT
                (cprif_basic_frame_parser_active_table_set(unit, port, cprimod_basic_frame_table_0, FALSE));
        }
        _SOC_IF_ERR_EXIT
            (cprif_basic_frame_assembly_active_table_get(unit, port, &table));
        if (table == cprimod_basic_frame_table_0) {
            _SOC_IF_ERR_EXIT
                (cprif_basic_frame_assembly_active_table_set(unit, port,cprimod_basic_frame_table_1, FALSE));
        } else {
            _SOC_IF_ERR_EXIT
                (cprif_basic_frame_assembly_active_table_set(unit, port,cprimod_basic_frame_table_0, FALSE));
        }
    } else {
        _SOC_IF_ERR_EXIT
            (cprif_basic_frame_parser_switch_active_table(unit, port));
    ...
    /* disable TGEN. */
    cprif_drv_tx_framer_tgen_enable_set(unit, port, 0);
    ...
    if (info->clear_supported) {
        if (info->reg != INVALIDr) {
            ...
            if (info->clear_field != INVALIDf) {
                COMPILER_64_SET(f_val, 0, 1);
                soc_reg64_field_set(unit, info->reg, &reg_data, info->clear_field, f_val);
                _SOC_IF_ERR_EXIT
                    (soc_reg_set(unit, info->reg, port, 0, reg_data));
            }
        } else if (info->mem != INVALIDm) {
...
int cprif_cpri_port_lane_map_get(int unit, int port,
                                 uint32 flags, phymod_lane_map_t* lane_map)
{
    uint32_t lane = 0;
    int map_lane = 0;
    SOC_INIT_FUNC_DEFS;
    ...
    for (lane = 0; lane < CPRI_NOF_LANES_IN_CORE; lane++){
        _SOC_IF_ERR_EXIT
            (cprif_drv_lane_swap_get(unit, port, lane, CPRIMOD_DIR_TX,
                                     &map_lane));
        lane_map->lane_map_tx[lane] = map_lane;
        _SOC_IF_ERR_EXIT
            (cprif_drv_lane_swap_get(unit, port, lane, CPRIMOD_DIR_RX,
                                     &map_lane));
        lane_map->lane_map_rx[lane] = map_lane;
    }

    lane_map->num_of_lanes = CPRI_NOF_LANES_IN_CORE;
