 ...
 *      max_entry_size                  - (IN)  size of entry array in bytes
 *      nof_fifo_dma_buf_entries        - (IN)  num of entries to copy from _soc_dfe_fifo_dma[unit][channel].buffer to entry
 *      entry                           - (OUT) data out
 * Returns:
 *      SOC_E_XXX
 */
...
soc_dfe_fifo_dma_channel_entry_get(int unit, int channel, uint32  max_entry_size, uint32 nof_fifo_dma_buf_entries, uint8 *entry)
{
    ...
    if (_soc_dfe_fifo_dma[unit][channel].nof_unread_entries > 0)
    {
        ...
        _soc_dfe_fifo_dma[unit][channel].current_entry_id = (_soc_dfe_fifo_dma[unit][channel].current_entry_id + nof_fifo_dma_buf_entries) % _soc_dfe_fifo_dma[unit][channel].config.max_entries; /*Cyclic buffer*/
