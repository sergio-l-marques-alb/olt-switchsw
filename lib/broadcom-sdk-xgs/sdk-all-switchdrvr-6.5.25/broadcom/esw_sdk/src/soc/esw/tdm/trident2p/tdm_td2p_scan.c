...
**/
int
tdm_td2p_legacy_which_tsc(unsigned short port, int **tsc)
{
...
{
	int i, j, idx_s=0, idx_e=0, port_tsc=TD2P_NUM_PHY_PM, type=BOOL_TRUE, tsc[TD2P_NUM_PHY_PM][TD2P_NUM_PM_LNS];
	...
	if (_tdm_s->_core_data.vars_pkg.port<65) {
		idx_s = 1;
		idx_e = 64;
	}
	else {
	...
	for (i=idx_s; i<=idx_e; i++) {
		port_tsc = tdm_td2p_scan_which_tsc(i,tsc);
		if (port_tsc<TD2P_NUM_PHY_PM){
			if (_tdm_s->_chip_data.soc_pkg.speed[i]!=SPEED_0 && _tdm_s->_chip_data.soc_pkg.soc_vars.td2p.pm_encap_type[port_tsc]==PM_ENCAP__HIGIG2) {
				type=BOOL_FALSE;
				break;
			}
		}
...
**/
int
tdm_td2p_slice_size_local(unsigned short idx, int *tdm, int lim)
{
	int i, slice_size=(-1);
	if (tdm[idx]!=TD2P_OVSB_TOKEN && tdm[idx]!=TD2P_NUM_EXT_PORTS) {
		for (i=idx; i>=0; i--) {
			if (tdm[i]!=TD2P_OVSB_TOKEN && tdm[i]!=TD2P_NUM_EXT_PORTS) {slice_size++;}
			else {break;}
		}
		for (i=idx; i<lim; i++) {			
			if (tdm[i]!=TD2P_OVSB_TOKEN && tdm[i]!=TD2P_NUM_EXT_PORTS) {slice_size++;}
		...
		for (i=idx; i>=0; i--) {
			if (tdm[i]==TD2P_OVSB_TOKEN) {slice_size++;}
			else {break;}
		}
		for (i=idx; i<lim; i++) {
			if (tdm[i]==TD2P_OVSB_TOKEN) {slice_size++;}
...
**/
int
tdm_td2p_slice_size(unsigned short port, int *tdm, int lim)
{
...
**/
int
tdm_td2p_slice_idx(unsigned short port, int *tdm, int lim)
{
...
**/
int
tdm_td2p_slice_prox_local(unsigned short idx, int *tdm, int lim, int **tsc)
{
...
/**
@name: tdm_td2p_scan_slice_min
@param:

Given port number, returns the MIN size of port slices in an array
**/
int
tdm_td2p_scan_slice_min(unsigned short port, int *tdm, int lim, int *slice_start_idx, int pos)
{
	...
	if(pos>=0 && pos<lim){
		...
		TD2P_TOKEN_CHECK(port) {
			for (i=0; i<lim; i++) {
				...
				TD2P_TOKEN_CHECK(tdm[idx0]) {
					k = tdm_td2p_scan_slice_size_local(idx0, tdm, lim, &idx_start);
		...
		/* oversub */
		else if (port==TD2P_OVSB_TOKEN){
			for (i=0; i<lim; i++) {
				idx0 = ((i+pos)<lim)?(i+pos):(i+pos-lim);
				if (tdm[idx0]==TD2P_OVSB_TOKEN) {
					k = tdm_td2p_scan_slice_size_local(idx0, tdm, lim, &idx_start);
					if(k>0 && k<slice_size_min){
						slice_size_min = k;
						slice_idx = idx_start;
		...
		/* idle */
		else if (port==TD2P_IDL1_TOKEN || port==TD2P_IDL2_TOKEN ){
			for (i=0; i<lim; i++) {
				idx0 = ((i+pos)<lim)?(i+pos):(i+pos-lim);
				if (tdm[idx0]==TD2P_IDL1_TOKEN || tdm[idx0]==TD2P_IDL2_TOKEN) {
					k = tdm_td2p_scan_slice_size_local(idx0, tdm, lim, &idx_start);
					if(k>0 && k<slice_size_min){
						slice_size_min = k;
						slice_idx = idx_start;
...
/**
@name: tdm_td2p_scan_slice_max
@param:

Given port number, returns the MAX size of port slices in an array
**/
int
tdm_td2p_scan_slice_max(unsigned short port, int *tdm, int lim, int *slice_start_idx, int pos)
{
...
/**
@name: tdm_td2p_scan_slice_size_local
@param:

Given index, returns the largest size of local slice
**/
int
tdm_td2p_scan_slice_size_local(unsigned short idx, int *tdm, int lim, int *slice_start_idx)
{
	...
	if(idx<lim){
		...
		TD2P_TOKEN_CHECK(tdm[idx]){
			for (i=idx; i>=0; i--) {
				TD2P_TOKEN_CHECK(tdm[i]) {slice_size++; idx_start=i;}
				else {break;}
			}
			for (i=idx; i<lim; i++) {
				TD2P_TOKEN_CHECK(tdm[i]) {slice_size++;}
				else {break;}
			}
		}
		/* ovsb */
		if (tdm[idx]==TD2P_OVSB_TOKEN) {
			for (i=idx; i>=0; i--) {
				if (tdm[i]==TD2P_OVSB_TOKEN) {slice_size++; idx_start=i;}
				else {break;}
			}
			for (i=idx; i<lim; i++) {
				if (tdm[i]==TD2P_OVSB_TOKEN) {slice_size++;}
		...
		/* idle */
		else if (tdm[idx]==TD2P_IDL1_TOKEN || tdm[idx]==TD2P_IDL2_TOKEN) {
			for (i=idx; i>=0; i--) {
				if (tdm[i]==TD2P_IDL1_TOKEN || tdm[i]==TD2P_IDL2_TOKEN) {slice_size++; idx_start=i;}
				else {break;}
			}
			for (i=idx; i<lim; i++) {
				if (tdm[i]==TD2P_IDL1_TOKEN || tdm[i]==TD2P_IDL2_TOKEN) {slice_size++;}
				else {break;}
			}
		}
	}
...
/**
@name: tdm_td2p_slice_size_min
@param:

Given port number, returns the MIN size of port slices (mixed with ANCL) in an array
**/
int
tdm_td2p_scan_mix_slice_min(unsigned short port, int *tdm, int lim, int *slice_start_idx, int pos)
{
	int i, k=0, idx0, slice_size_min=256, slice_idx=-1 , idx_start;
	if (pos>=0 && pos<lim) {
		/* linerate */
		TD2P_TOKEN_CHECK(port) {
			for (i=0; i<lim; i++) {
				idx0 = ((i+pos)<lim)?(i+pos):(i+pos-lim);
				TD2P_TOKEN_CHECK(tdm[idx0]) {
					k = tdm_td2p_scan_mix_slice_size_local(idx0, tdm, lim, &idx_start);
					if(k>0 && k<slice_size_min){
						slice_size_min = k;
						slice_idx= idx_start;
					}
...
/**
@name: tdm_td2p_scan_mix_slice_max
@param:

Given port number, returns the MAX size of port slices (mixed with ANCL) in an array
**/
int
tdm_td2p_scan_mix_slice_max(unsigned short port, int *tdm, int lim, int *slice_start_idx, int pos)
{
	int i, k=0, idx0, slice_size_max=0, slice_idx=-1 , idx_start;
	if (pos>=0 && pos<lim) {
		/* linerate */
...
/**
@name: tdm_td2p_scan_mix_slice_size_local
@param:

Given index, returns the largest size of local slice (mixed with ANCL)
**/
int
tdm_td2p_scan_mix_slice_size_local(unsigned short idx, int *tdm, int lim, int *slice_start_idx)
{
	int i, slice_size=(-1), idx_start=(-1);
	...
	if(idx<lim){
		...
		TD2P_TOKEN_CHECK(tdm[idx]){
			for (i=idx; i>=0; i--) {
				TD2P_TOKEN_CHECK(tdm[i]) {slice_size++; idx_start=i;}
				else if (tdm[i]==TD2P_ANCL_TOKEN) {slice_size++; idx_start=i;}
				else {break;}
			}
			for (i=idx; i<lim; i++) {
				TD2P_TOKEN_CHECK(tdm[i]) {slice_size++;}
				else if (tdm[i]==TD2P_ANCL_TOKEN) {slice_size++;}
				else {break;}
			}
		}
		/* ancl mix linerate */
		else if (tdm[idx]==TD2P_ANCL_TOKEN){
			for (i=idx; i>=0; i--) {
				TD2P_TOKEN_CHECK(tdm[i]) {slice_size++; idx_start=i;}
				else if (tdm[i]==TD2P_ANCL_TOKEN) {slice_size++; idx_start=i;}
				else {break;}
...
/**
@name: tdm_td2p_check_slot_swap_cond
@param:
...
**/
int
tdm_td2p_check_slot_swap_cond(int idx, int *tdm_tbl, int tdm_tbl_len, int **tsc, enum port_speed_e *speed)
{
	...
	if (check_pass==BOOL_TRUE){
		TD2P_TOKEN_CHECK(tdm_tbl[idx_x]){
			idx0 = idx_x;
		...
		TD2P_TOKEN_CHECK(tdm_tbl[idx_y]){
			idx0 = idx_y;
...
/**
@name: tdm_td2p_check_shift_cond_pattern
@param:
...
**/
int
tdm_td2p_check_shift_cond_pattern(unsigned short port, int *tdm_tbl, int tdm_tbl_len, int **tsc, int dir)
{
	...
	if (shift_cond_pass==BOOL_TRUE) {
		...
		if (dir==DN) {
			for (i=0; i<(tdm_tbl_len-1); i++) {
				if (tdm_tbl[i]==port && tdm_tbl[i+1]!=TD2P_OVSB_TOKEN && tdm_tbl[i+1]!=TD2P_ANCL_TOKEN) {
	...
	if(shift_cond_pass==BOOL_TRUE){
		port_tsc = tdm_td2p_legacy_which_tsc(port,tsc);
		/* Downward pattern: _x_ovsb_..._x_ovsb_..._x_ovsb_ */
		if (dir==DN){
			for (i=0; i<(tdm_tbl_len-1); i++) {
				if (tdm_tbl[i]==port) {
...
/**
@name: tdm_td2p_check_shift_cond_local_slice
@param:
...
**/
int
tdm_td2p_check_shift_cond_local_slice(unsigned short port, int *tdm_tbl, int tdm_tbl_len, int **tsc, int dir)
{
	...
	os_clump_max_last = tdm_td2p_scan_slice_max(ovsb_token,tdm_tbl,tdm_tbl_len, &slice_idx, 0);
	lr_clump_max_last = tdm_td2p_scan_mix_slice_max(1,tdm_tbl,tdm_tbl_len, &slice_idx, 0);
	lr_clump_min_last = tdm_td2p_scan_mix_slice_min(1,tdm_tbl,tdm_tbl_len, &slice_idx, 0);
	...
	if ( (lr_clump_max_last<=1) || (lr_clump_max_last==2 && lr_clump_min_last==1) ) {
		...
		for (i=0; i<tdm_tbl_len; i++) {
			...
			if (tdm_tbl[idx0]==ovsb_token){
				os_clump_local_above = tdm_td2p_scan_slice_size_local(idx0,tdm_tbl,tdm_tbl_len, &slice_idx);
			}
			if (tdm_tbl[idx1]==ovsb_token){
				os_clump_local_below = tdm_td2p_scan_slice_size_local(idx1,tdm_tbl,tdm_tbl_len, &slice_idx);
			}
			...
			lr_clump_local_last = tdm_td2p_scan_mix_slice_size_local(i,tdm_tbl,tdm_tbl_len, &slice_idx);
			...
			if (dir==DN){
				...
				if (tdm_tbl[idx0]!=ovsb_token){
					for (j=0; j<(tdm_tbl_len-2); j++){
						...
						if (tdm_tbl[idx1]==ovsb_token) {
							lr_clump_local_curr = 1 + tdm_td2p_scan_mix_slice_size_local(idx0,tdm_tbl,tdm_tbl_len, &slice_idx);
							break;
						}
						else if (tdm_tbl[idx1]==filter_port) {
							lr_clump_local_curr = tdm_td2p_scan_mix_slice_size_local(idx0,tdm_tbl,tdm_tbl_len, &slice_idx);
							break;
						}
					}
				}
...
**/
int
tdm_td2p_scan_which_tsc( int port, int tsc[TD2P_NUM_PHY_PM][TD2P_NUM_PM_LNS] )
{
	int result=TD2P_NUM_EXT_PORTS;
	tdm_mod_t *_tdm_s;
	
	TD2P_TOKEN_CHECK(port){
		_tdm_s = tdm_chip_td2p_shim__scan_which_tsc_alloc(port, tsc);
		if (_tdm_s != NULL) {
			result = tdm_td2p_which_tsc(_tdm_s);
			tdm_chip_td2p_shim__scan_which_tsc_free(_tdm_s);
		}
	}
...
/**
@name: tdm_td2p_check_pipe_ethernet
@param:

Returns BOOL_TRUE or BOOL_FALSE depending on if pipe of the given port has traffic entirely Ethernet
**/
int
tdm_td2p_check_pipe_ethernet( int port, tdm_mod_t *_tdm)
{
	int port_tmp, result=BOOL_TRUE;
	
	port_tmp = _tdm->_core_data.vars_pkg.port;
	_tdm->_core_data.vars_pkg.port = port;
	result = tdm_td2p_check_ethernet(_tdm);
