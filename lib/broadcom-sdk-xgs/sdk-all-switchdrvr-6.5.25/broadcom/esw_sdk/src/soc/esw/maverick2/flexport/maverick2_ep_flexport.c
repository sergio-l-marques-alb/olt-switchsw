...
static const soc_mv2_ep_core_cfg_t
    soc_mv2_ep_core_cfg_tbl[] = {
    /* L2_lat & L3_lat not used in MV2
	 * speed  ep2mmu(1.616) ep2mmu(2.155) L2_lat   L3 lat  full_lat
     * LR  3:2 2:1   LR  3:2 2:1   LR  3:2 2:1
     */
    {    -1,  0,  0, {15, 19, 20}, {15, 19, 20}, {15, 19, 20} },   /* SAF     */
    { 10000, 11, 11, {49, 56, 59}, {49, 56, 59}, {49, 56, 59} },   /* 10GE    */
    { 11000, 11, 11, {49, 56, 59}, {49, 56, 59}, {49, 56, 59} },   /* HG[11]  */
    { 20000, 13, 16, {59, 68, 72}, {59, 68, 72}, {59, 68, 72} },   /* 20GE    */
    { 21000, 13, 16, {59, 68, 72}, {59, 68, 72}, {59, 68, 72} },   /* HG[21]  */
    { 25000, 18, 20, {92,101,105}, {92,101,105}, {92,101,105} },   /* 25GE    */
    { 27000, 18, 20, {92,101,105}, {92,101,105}, {92,101,105} },   /* HG[27]  */
    { 40000, 19, 25, {30, 37, 40}, {30, 37, 40}, {30, 37, 40} },   /* 40GE    */
    { 42000, 19, 25, {30, 37, 40}, {30, 37, 40}, {30, 37, 40} },   /* HG[42]  */
    { 50000, 22, 26, {30, 37, 40}, {30, 37, 40}, {30, 37, 40} },   /* 50GE    */
    { 53000, 22, 26, {30, 37, 40}, {30, 37, 40}, {30, 37, 40} },   /* HG[53]  */
    {100000, 38, 41, {30, 37, 40}, {30, 37, 40}, {30, 37, 40} },   /* 100GE   */
    {106000, 38, 41, {30, 37, 40}, {30, 37, 40}, {30, 37, 40} }    /* HG[106] */
};


/*! @fn int soc_maverick2_ep_flexport_get_max_ovs_ratio(int unit,
 ...
 */
int
soc_maverick2_ep_flexport_get_max_ovs_ratio(
...
/*! @fn int soc_maverick2_ep_get_ep2mmu_credit(int unit, 
    ...
    int unit, soc_port_schedule_state_t *port_schedule_state, int logical_port)
{
    ...
    ct_class = soc_maverick2_get_ct_class(speed);
	asf_mode = port_schedule_state->cutthru_prop.asf_modes[logical_port];
	credit_incr = ((asf_mode == _SOC_ASF_MODE_SAME_SPEED) ||
               (asf_mode == _SOC_ASF_MODE_SLOW_TO_FAST) ||
               (asf_mode == _SOC_ASF_MODE_FAST_TO_SLOW)) ? 2 : 0;

    /*
     * PFC optimized value can be used only when FREQ is 1525MHz and ovs_ratio less than 1.61; 
     */
    if ((new_max_ovs_ratio <= 15) && (port_schedule_state->frequency == 1525)) {
        return (soc_mv2_ep_core_cfg_tbl[ct_class].egr_mmu_credit_1616 + credit_incr);
    } else {
        return soc_mv2_ep_core_cfg_tbl[ct_class].egr_mmu_credit_2155;
    }
}


/*! @fn int soc_maverick2_ep_get_ct_xmit_start_cnt(int ct_class, int latency_mode,
 *              int ovs_ratio)
 *  @param unit Chip unit number.
 *  @param port_schedule_state Pointer to a soc_port_schedule_state_t struct 
 *         variable
 ...
 */
int
soc_maverick2_ep_get_ct_xmit_start_cnt(int ct_class, int latency_mode,
                                       int ovs_ratio)
{
    int xmit_cnt;
    ...
    /* FULL latency */
    case 0:
        if (ovs_ratio <= 15) {     /* OVS 1.616 */
            xmit_cnt =
                soc_mv2_ep_core_cfg_tbl[ct_class].xmit_cnt_full_latency.
                oversub_1616;
        } else if (ovs_ratio <= 20) {     /* OVS 2.00 */
            xmit_cnt =
                soc_mv2_ep_core_cfg_tbl[ct_class].xmit_cnt_full_latency.
                oversub_2000;
        } else {     /* OVS 2.155 */
    ...
    case 1:
        if (ovs_ratio <= 15) {     /* OVS 1.616 */
            xmit_cnt =
                soc_mv2_ep_core_cfg_tbl[ct_class].xmit_cnt_l3_latency.
                oversub_1616;
        } else if (ovs_ratio <= 20) {     /* OVS 2.00 */
            xmit_cnt =
                soc_mv2_ep_core_cfg_tbl[ct_class].xmit_cnt_l3_latency.
    ...
    case 2:
        if (ovs_ratio <= 15) {     /* OVS 1.616 */
            xmit_cnt =
                soc_mv2_ep_core_cfg_tbl[ct_class].xmit_cnt_l2_latency.
                oversub_1616;
        } else if (ovs_ratio <= 20) {     /* OVS 2.00 */
            xmit_cnt =
                soc_mv2_ep_core_cfg_tbl[ct_class].xmit_cnt_l2_latency.
...
/*! @fn int soc_maverick2_ep_get_xmit_start_count(int unit,
 *              soc_port_schedule_state_t *port_schedule_state,
 ...
 */
int
soc_maverick2_ep_get_xmit_start_count(
    int  unit, soc_port_schedule_state_t *port_schedule_state,
    int  logical_port, int *xmit_start_cnt)
{
    ...
    cookie = port_schedule_state->cookie;
    speed = cookie->exact_out_log_port_speed[logical_port];
    dst_ct_class = soc_maverick2_get_ct_class(speed);
    ...
    saf_xmit_start_cnt = soc_maverick2_ep_get_ct_xmit_start_cnt(
                                      0, latency_mode, new_max_ovs_ratio);                                                            /* 0 means SAF */
    dst_ct_xmit_start_cnt = soc_maverick2_ep_get_ct_xmit_start_cnt(
...
/*! @fn int soc_maverick2_ep_flexport_sft_rst_pm_intf(int unit,
 ...
 */
int
soc_maverick2_ep_flexport_sft_rst_pm_intf(
    int unit, soc_port_schedule_state_t *port_schedule_state, int rst_on)
{
    ...
    uint32 pm_rst_values[MAVERICK2_PIPES_PER_DEV][MAVERICK2_PBLKS_PER_PIPE];

    uint64 rval64;
    soc_reg_t reg;
    static const soc_reg_t edb_sft_rst_regs[MAVERICK2_PIPES_PER_DEV] = {
        EGR_PORT_BUFFER_SFT_RESET_0_PIPE0r
    };


    /* pipe_map indicates which pipe has flexing ports */
    _soc_mv2_tdm_get_pipe_map(unit, port_schedule_state, &pipe_map);
    ...
    if (1 == rst_on) {
        for (i = 0; i < port_schedule_state->nport; i++) {
            ...
            pm_num = (physical_port - 1) / MAVERICK2_PORTS_PER_PBLK; /* device pm_num*/
            pm_indx = (pm_num % MAVERICK2_PBLKS_PER_PIPE);
            /* pm_indx is pipe local PM indx and swapped for odd pipes */
            pm_indx =
                ((pipe % 2) == 0) ? pm_indx : (MAVERICK2_PBLKS_PER_PIPE-1 - pm_indx);
            subport = (physical_port - 1) % MAVERICK2_PORTS_PER_PBLK;
            switch (subport) {
            case 0:
            case 1:
                if (num_lanes == 4) {
        ...
        for (pipe = 0; pipe < MAVERICK2_PIPES_PER_DEV; pipe++) {
            for (pm_indx = 0; pm_indx < MAVERICK2_PBLKS_PER_PIPE; pm_indx++) {
    ...
    for (pipe = 0; pipe < MAVERICK2_PIPES_PER_DEV; pipe++) {
        ...
        soc_reg64_field32_set(unit, reg, &rval64, PM10_RESETf,
                            pm_rst_values[pipe][10]);
        soc_reg64_field32_set(unit, reg, &rval64, PM11_RESETf,
                            pm_rst_values[pipe][11]);
        soc_reg64_field32_set(unit, reg, &rval64, PM12_RESETf,
                            pm_rst_values[pipe][12]);
        soc_reg64_field32_set(unit, reg, &rval64, PM13_RESETf,
                            pm_rst_values[pipe][13]);
        soc_reg64_field32_set(unit, reg, &rval64, PM14_RESETf,
                            pm_rst_values[pipe][14]);
        soc_reg64_field32_set(unit, reg, &rval64, PM15_RESETf,
                            pm_rst_values[pipe][15]);
        soc_reg64_field32_set(unit, reg, &rval64, PM16_RESETf,
                            pm_rst_values[pipe][16]);
        soc_reg64_field32_set(unit, reg, &rval64, PM17_RESETf,
                            pm_rst_values[pipe][17]);
        soc_reg64_field32_set(unit, reg, &rval64, PM18_RESETf,
                            pm_rst_values[pipe][18]);
        soc_reg64_field32_set(unit, reg, &rval64, PM19_RESETf,
                            pm_rst_values[pipe][19]);
...
/*! @fn int soc_maverick2_ep_flexport_sft_rst_ports(int unit,
 *              soc_port_schedule_state_t *port_schedule_state)
 *  @param unit Chip unit number.
 ...
 */
int
soc_maverick2_ep_flexport_sft_rst_ports(
    int unit, soc_port_schedule_state_t *port_schedule_state, int rst_on)
{
    int i;
    ...
    uint32 memfld;
    soc_mem_t mem;
    int port_rst_serviced[MAVERICK2_PHY_PORTS_PER_DEV];
...
/*! @fn int soc_maverick2_ep_enable_disable(int unit,
 *              soc_port_schedule_state_t *port_schedule_state,
 ...
 */
int
soc_maverick2_ep_enable_disable(int                        unit,
                                soc_port_schedule_state_t *port_schedule_state,
                                int                        logical_port,
                                int                        down_or_up)
{
...
/*! @fn int soc_maverick2_ep_set_egr_mmu_credit(int unit,
 *              soc_port_schedule_state_t *port_schedule_state
 *              int logical_port, int down_or_up)
 *  @param unit Chip unit number.
 *  @param port_schedule_state Pointer to a soc_port_schedule_state_t struct 
 ...
 */
int
soc_maverick2_ep_set_egr_mmu_credit(
    int unit, soc_port_schedule_state_t *port_schedule_state,
    int logical_port, int down_or_up)
{
    uint32 entry[SOC_MAX_MEM_WORDS];
    ...
    if (0 == down_or_up ) { /* port down */
        ...
        memfld = soc_maverick2_ep_get_ep2mmu_credit(unit, port_schedule_state,
                                                    logical_port);
        physical_port =
            port_schedule_state->out_port_map.port_l2p_mapping[logical_port];
    }
...
/*! @fn int soc_maverick2_ep_set_dev_to_phy(int unit,
 *              soc_port_schedule_state_t *port_schedule_state
 *              int logical_port, int down_or_up)
 *  @param unit Chip unit number.
 *  @param port_schedule_state Pointer to a soc_port_schedule_state_t struct
 ...
 */
int
soc_maverick2_ep_set_dev_to_phy(int                        unit,
                                soc_port_schedule_state_t *port_schedule_state,
                                int                        logical_port,
                                int                        down_or_up)
{
...
/*! @fn int soc_maverick2_ep_set_edb_1dbg_b(int unit,
 *              soc_port_schedule_state_t *port_schedule_state
 *              int logical_port, int down_or_up)
 *  @param unit Chip unit number.
 *  @param port_schedule_state Pointer to a soc_port_schedule_state_t struct
 ...
 */
int
soc_maverick2_ep_set_edb_1dbg_b(int                        unit,
                                soc_port_schedule_state_t *port_schedule_state,
                                int                        logical_port,
                                int                        down_or_up)
{
...
/*! @fn int soc_maverick2_ep_flexport_extra_holding_set(int unit,
 ...
 */
int
soc_maverick2_ep_flexport_extra_holding_set(
    int unit, soc_port_schedule_state_t *port_schedule_state,
    int cell_counts)
{
    int pipe;
    ...
    uint32 pipe_map;
    soc_reg_t reg;
    static const soc_reg_t extr_hold_regs[MAVERICK2_PIPES_PER_DEV] = {
...
/*! @fn int soc_maverick2_ep_flexport_xmit_cnt_set(int unit,
 *              soc_port_schedule_state_t *port_schedule_state, int logical_port)
 *  @param unit Chip unit number.
 *  @param port_schedule_state Pointer to a soc_port_schedule_state_t struct
 *         variable
 ...
 */
int
soc_maverick2_ep_flexport_xmit_cnt_set(
    int unit, soc_port_schedule_state_t *port_schedule_state, int logical_port)
{
    soc_mem_t mem;
    ...
    static const soc_mem_t ep_xmit_cnt_mems[MAVERICK2_PIPES_PER_DEV] = {
    ...
    soc_maverick2_ep_get_xmit_start_count(unit, port_schedule_state,
                                          logical_port,
                                          xmit_start_cnt);
    for (class = 0; class < 13; class++) {
        sal_memset(entry, 0, sizeof(uint32) *
                   soc_mem_entry_words(unit, EGR_XMIT_START_COUNT_PIPE0m));
        mem_index = (logical_port % MAVERICK2_DEV_PORTS_PER_PIPE) * 16 + class;
...
/*! @fn int soc_maverick2_ep_flexport_xmit_cnt_set_all_ports(int unit,
 *              soc_port_schedule_state_t *port_schedule_state)
 *  @param unit Chip unit number.
 *  @param port_schedule_state Pointer to a soc_port_schedule_state_t struct
 *         variable
 ...
 */
int
soc_maverick2_ep_flexport_xmit_cnt_set_all_ports(
    int unit, soc_port_schedule_state_t *port_schedule_state)
{
    int old_max_ovs_ratio, new_max_ovs_ratio;
    ...
    if (new_max_ovs_ratio != old_max_ovs_ratio) {
        for (logical_port = 0; logical_port < MAVERICK2_DEV_PORTS_PER_DEV;
             logical_port++) {
            ...
            if ((port_schedule_state->out_port_map.log_port_speed[logical_port]
                 > 0 ) &&
                (is_ovs == 1)) {
                soc_maverick2_ep_flexport_xmit_cnt_set(unit,
                                                       port_schedule_state,
                                                       logical_port);
            }
        }
...
/*! @fn int soc_maverick2_ep_flexport_ct_class_set(int unit,
 *              soc_port_schedule_state_t *port_schedule_state, int logical_port)
 *  @param unit Chip unit number.
 *  @param port_schedule_state Pointer to a soc_port_schedule_state_t struct
 *         variable
 ...
 */
int
soc_maverick2_ep_flexport_ct_class_set(
    int unit, soc_port_schedule_state_t *port_schedule_state, int logical_port)
{
    soc_mem_t mem;
    uint32 memfld;
    ...
    cookie = port_schedule_state->cookie;
    speed  = cookie->exact_out_log_port_speed[logical_port];
    memfld = soc_maverick2_get_ct_class(speed);
...
/*! @fn int soc_maverick2_ep_flexport_drain_port(int unit,
 *              soc_port_schedule_state_t *port_schedule_state, int logical_port)
 *  @param unit Chip unit number.
 *  @param port_schedule_state Pointer to a soc_port_schedule_state_t struct
 *         variable
 ...
 */
int
soc_maverick2_ep_flexport_drain_port(
    int unit, soc_port_schedule_state_t *port_schedule_state, int logical_port)
{
    int pipe;
    ...
    static const soc_reg_t misc_ctrl_regs[MAVERICK2_PIPES_PER_DEV] = {
    ...
    while (iter++ < timeout) {
        SOC_IF_ERROR_RETURN
            (soc_mem_read(unit, mem, MEM_BLOCK_ALL, physical_port, entry));
...
/*! @fn int soc_maverick2_flex_ep_port_down(int unit,
 *              soc_port_schedule_state_t *port_schedule_state)
 *  @param unit Chip unit number.
 *  @param port_schedule_state Pointer to a soc_port_schedule_state_t struct
 *         variable
 ...
 */
int
soc_maverick2_flex_ep_port_down(int                        unit,
                                soc_port_schedule_state_t *port_schedule_state)
{
    ...
    for (i = 0; i < port_schedule_state->nport; i++) {
        if (port_schedule_state->resource[i].physical_port == -1) {
            logical_port = port_schedule_state->resource[i].logical_port;
            SOC_IF_ERROR_RETURN(soc_maverick2_ep_flexport_drain_port(unit,
    ...
    for (i = 0; i < port_schedule_state->nport; i++) {
        ...
        if (-1 == physical_port) { /* that is, port DOWN */
            ...
            soc_maverick2_ep_enable_disable(unit, port_schedule_state,
                                            logical_port,
                                            down_or_up);
        }
    }
    ...
    rst_on = 1;
    SOC_IF_ERROR_RETURN(soc_maverick2_ep_flexport_sft_rst_ports(unit,
                                                                port_schedule_state,
                                                                rst_on));
...
/*! @fn int soc_maverick2_flex_ep_reconfigure_remove(int unit,
 *              soc_port_schedule_state_t *port_schedule_state)
 *  @param unit Chip unit number.
 *  @param port_schedule_state Pointer to a soc_port_schedule_state_t struct
 *         variable
    ...
    int unit, soc_port_schedule_state_t *port_schedule_state)
{
    ...
    for (i = 0; i < port_schedule_state->nport; i++) {
        ...
        if (-1 == physical_port) { /* that is, port DOWN */
             ...
             * EGR_DEVICE_TO_PHYSICAL_PORT_NUMBER_MAPPINGr
             */
            soc_maverick2_ep_set_dev_to_phy(unit, port_schedule_state,
                                            logical_port,
                                            down_or_up);
            /* Reset EP2MMU credits to 0 : write EGR_MMU_CELL_CREDITm */
            soc_maverick2_ep_set_egr_mmu_credit(unit, port_schedule_state,
                                                logical_port,
                                                down_or_up);
            /* Reset edb_1dbg_b : write EDB_1DBG_Bm */
            soc_maverick2_ep_set_edb_1dbg_b(unit, port_schedule_state,
                                            logical_port,
                                            down_or_up);
        }
    }
    ...
    SOC_IF_ERROR_RETURN(soc_maverick2_ep_flexport_extra_holding_set(unit,
                            port_schedule_state, 8)); /* 8 as per MV2 Chip Uarch Delta Spec */

    return SOC_E_NONE;
}


/*! @fn int soc_maverick2_flex_ep_reconfigure_add(int unit,
 *              soc_port_schedule_state_t *port_schedule_state)
 *  @param unit Chip unit number.
 *  @param port_schedule_state Pointer to a soc_port_schedule_state_t struct
 *         variable
    ...
    int unit, soc_port_schedule_state_t *port_schedule_state)
{
    ...
    for (i = 0; i < port_schedule_state->nport; i++) {
        ...
        if (-1 != physical_port) { /* that is, port UP */
            ...
            soc_maverick2_ep_flexport_ct_class_set(unit, port_schedule_state,
                                                   logical_port);

            /* EGR_PORT_MIN_PKT_SIZE needs to be 76 for hg2 ports */
            if (port_schedule_state->resource[i].encap == SOC_ENCAP_HIGIG2) {
                SOC_IF_ERROR_RETURN(
                        soc_trident3_egr_port_min_pkt_size_set(
                            unit, logical_port, 76));
            } else {
                SOC_IF_ERROR_RETURN(
                        soc_trident3_egr_port_min_pkt_size_set(
                            unit, logical_port, 64));
            }
        }
    }
     ...
     * write EGR_XMIT_START_COUNT_PIPE0/1m
     */
    SOC_IF_ERROR_RETURN(soc_maverick2_ep_flexport_xmit_cnt_set_all_ports(unit,
                            port_schedule_state));
...
/*! @fn int soc_maverick2_flex_ep_port_up(int unit,
 *              soc_port_schedule_state_t *port_schedule_state)
 *  @param unit Chip unit number.
 *  @param port_schedule_state Pointer to a soc_port_schedule_state_t struct
 *         variable
 ...
 */
int
soc_maverick2_flex_ep_port_up(int                        unit,
                              soc_port_schedule_state_t *port_schedule_state)
{
    int i, rst_on, down_or_up;
    int logical_port, physical_port;
 ...
 */
int
soc_maverick2_ep_display_xmit_cnt(
    int unit, soc_port_schedule_state_t *port_schedule_state)
{
    ...
    for (ovs_ratio = 3; ovs_ratio < 6; ovs_ratio++) {
        LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit, "\n\nOVS=%d\n"), 5*ovs_ratio));
        for (ct_class=0; ct_class < 13; ct_class++) {
            LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit,
                                "ct_class=%d\tep2mmu_1616=%d\tep2mmu_max_2155=%d\t"),
                     ct_class, soc_mv2_ep_core_cfg_tbl[ct_class].egr_mmu_credit_1616,
                     soc_mv2_ep_core_cfg_tbl[ct_class].egr_mmu_credit_2155));
            for (lat=2; lat>=0; lat--) {
                xmit_cnt = soc_maverick2_ep_get_ct_xmit_start_cnt(ct_class, lat,
                                                                  5*ovs_ratio);
    ...
    for (port = 1; port < SOC_MAX_NUM_PORTS; port++) {
        if (port_schedule_state->out_port_map.log_port_speed[port] > 0 ) {
            ep2mmu_cred = soc_maverick2_ep_get_ep2mmu_credit(
...
int
soc_maverick2_ep_flexport_remove_ports_shim(
    int unit, soc_port_schedule_state_t *port_schedule_state)
{

    LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit,
                        "soc_maverick2_ep_flexport_remove_ports_shim(): "
                        "calling soc_maverick2_flex_start")));
    SOC_IF_ERROR_RETURN(soc_maverick2_flex_start(unit, port_schedule_state));
    LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit,
                        "soc_maverick2_ep_flexport_remove_ports_shim(): "
                        "calling soc_maverick2_tdm_calculation_flexport")));
    SOC_IF_ERROR_RETURN(soc_maverick2_tdm_calculation_flexport(unit,
                                                               port_schedule_state));
    LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit,
                        "soc_maverick2_ep_flexport_remove_ports_shim(): "
                        "calling soc_maverick2_flex_ep_port_down")));
    SOC_IF_ERROR_RETURN(soc_maverick2_flex_ep_port_down(unit,
                                                        port_schedule_state));
    LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit,
                        "soc_maverick2_ep_flexport_remove_ports_shim(): "
                        "calling soc_maverick2_ep_flexport_reconfigure_remove")));
    SOC_IF_ERROR_RETURN(soc_maverick2_flex_ep_reconfigure_remove(unit,
                            port_schedule_state));
...
/* Comment out these functions from soc_maverick2_ep_flexport_add_ports function
 * 1. soc_maverick2_ep_flexport_ct_class_set
 * 2. soc_maverick2_ep_flexport_xmit_cnt_set_all_ports
 * 3. soc_maverick2_ep_enable_disable
 */
int
soc_maverick2_ep_flexport_add_ports_shim(
    int unit, soc_port_schedule_state_t *port_schedule_state)
{
    LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit,
                        "soc_maverick2_ep_flexport_add_ports_shim(): "
                        "calling soc_maverick2_ep_flexport_reconfigure_add")));
    SOC_IF_ERROR_RETURN(soc_maverick2_flex_ep_reconfigure_add(unit,
                                                              port_schedule_state));
    LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit,
                        "soc_maverick2_ep_flexport_add_ports_shim(): "
                        "calling soc_maverick2_ep_flexport_add_ports")));
    SOC_IF_ERROR_RETURN(soc_maverick2_flex_ep_port_up(unit, port_schedule_state));
...
int
soc_maverick2_ep_flexport_enable_ports_shim(
    int unit, soc_port_schedule_state_t *port_schedule_state)
{
