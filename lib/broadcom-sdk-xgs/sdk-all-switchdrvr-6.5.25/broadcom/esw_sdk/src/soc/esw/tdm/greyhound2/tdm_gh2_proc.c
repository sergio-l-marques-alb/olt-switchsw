 ...
 */
	...
	#include <tdm_top.h>
	#include <tdm_gh2_vec.h>
#else
	#include <soc/tdm/core/tdm_top.h>
	#include <soc/tdm/greyhound2/tdm_gh2_vec.h>
#endif


/**
@name: tdm_gh2_proc_cal_tdm5
@param:

**/
int
tdm_gh2_proc_cal_tdm5( tdm_mod_t *_tdm )
{
	int j, v, w;
	...
	for (j=0; j<(TDM_AUX_SIZE); j++) {
        lr1[j]   = GH2_NUM_EXT_PORTS;
        lr10[j]  = GH2_NUM_EXT_PORTS;
        lr20[j]  = GH2_NUM_EXT_PORTS;
        lr25[j]  = GH2_NUM_EXT_PORTS;
        lr40[j]  = GH2_NUM_EXT_PORTS;
        lr50[j]  = GH2_NUM_EXT_PORTS;
        lr100[j] = GH2_NUM_EXT_PORTS;
        _tdm->_core_data.vars_pkg.m_tdm_vmap_alloc.os1[j]  = GH2_NUM_EXT_PORTS;
        _tdm->_core_data.vars_pkg.m_tdm_vmap_alloc.os10[j] = GH2_NUM_EXT_PORTS;
        _tdm->_core_data.vars_pkg.m_tdm_vmap_alloc.os20[j] = GH2_NUM_EXT_PORTS;
        _tdm->_core_data.vars_pkg.m_tdm_vmap_alloc.os25[j] = GH2_NUM_EXT_PORTS;
        _tdm->_core_data.vars_pkg.m_tdm_vmap_alloc.os40[j] = GH2_NUM_EXT_PORTS;
        _tdm->_core_data.vars_pkg.m_tdm_vmap_alloc.os50[j] = GH2_NUM_EXT_PORTS;
        _tdm->_core_data.vars_pkg.m_tdm_vmap_alloc.os100[j]= GH2_NUM_EXT_PORTS;
	}	
	
	for (j=0; j<TDM_AUX_SIZE; j++) {
		if ((_tdm->_core_data.vars_pkg.lr_buffer[j] != GH2_NUM_EXT_PORTS) && ( (_tdm->_chip_data.soc_pkg.state[_tdm->_core_data.vars_pkg.lr_buffer[j]-1] == PORT_STATE__LINERATE)||(_tdm->_chip_data.soc_pkg.state[_tdm->_core_data.vars_pkg.lr_buffer[j]-1] == PORT_STATE__LINERATE_HG) )) {
		...
		else if (_tdm->_core_data.vars_pkg.lr_buffer[j] != GH2_NUM_EXT_PORTS && _tdm->_core_data.vars_pkg.lr_buffer[j] != GH2_OVSB_TOKEN) {
		...
		if ((_tdm->_core_data.vars_pkg.os_buffer[j] != GH2_NUM_EXT_PORTS) && ( (_tdm->_chip_data.soc_pkg.state[_tdm->_core_data.vars_pkg.os_buffer[j]-1] == PORT_STATE__OVERSUB)||(_tdm->_chip_data.soc_pkg.state[_tdm->_core_data.vars_pkg.os_buffer[j]-1] == PORT_STATE__OVERSUB_HG) )) {
		...
		else if (_tdm->_core_data.vars_pkg.os_buffer[j] != GH2_NUM_EXT_PORTS) {
	...
	if (_tdm->_core_data.vars_pkg.m_tdm_vmap_alloc.num_40g==3 && _tdm->_core_data.vars_pkg.m_tdm_vmap_alloc.num_100g==4 && _tdm->_chip_data.soc_pkg.clk_freq==GH2_CLK_850MHZ) {
	...
	if(_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.y4 > 0){
		if (_tdm->_core_data.vars_pkg.HG4X106G_3X40G) {
			if (tdm_core_postalloc_vmap(param_vmap, param_freq, 107, &_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.yy, &(_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.y4), param_lr_limit, lr100, TOKEN_100G, "100", GH2_NUM_EXT_PORTS, param_vmap_wid, param_vmap_len)!=PASS) {
				TDM_ERROR0("Critical error in 100G time vector preallocation\n");
				return 0;
			}
		}
		else if (_tdm->_core_data.vars_pkg.HG1X106G_xX40G_OVERSUB) {
			if (tdm_core_postalloc_vmap(param_vmap, param_freq, 108, &_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.yy, &(_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.y4), param_lr_limit, lr100, TOKEN_100G, "100", GH2_NUM_EXT_PORTS, param_vmap_wid, param_vmap_len)!=PASS) {
				TDM_ERROR0("Critical error in 100G time vector preallocation\n");
				return 0;
			}
		}
		else if (_tdm->_core_data.vars_pkg.HGXx120G_Xx100G) {
			if (tdm_core_postalloc_vmap(param_vmap, param_freq, 109, &_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.yy, &(_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.y4), param_lr_limit, lr100, TOKEN_100G, "100", GH2_NUM_EXT_PORTS, param_vmap_wid, param_vmap_len)!=PASS) {
				TDM_ERROR0("Critical error in 100G time vector preallocation\n");
				return 0;
			}
		}
		else{
			if (tdm_gh2_check_pipe_ethernet(lr100[_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.y4],_tdm) && _tdm->_chip_data.soc_pkg.soc_vars.gh2.higig_mgmt==BOOL_FALSE && _tdm->_chip_data.soc_pkg.soc_vars.gh2.cal_hg_en==BOOL_FALSE) {
				if (tdm_core_postalloc_vmap(param_vmap, param_freq, 100, &_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.yy, &(_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.y4), param_lr_limit, lr100, TOKEN_100G, "100", GH2_NUM_EXT_PORTS, param_vmap_wid, param_vmap_len)!=PASS) {
					TDM_ERROR0("Critical error in 100G time vector preallocation\n");
					return 0;
				}
			}
			else {
				if (tdm_core_postalloc_vmap(param_vmap, param_freq, 106, &_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.yy, &(_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.y4), param_lr_limit, lr100, TOKEN_100G, "100", GH2_NUM_EXT_PORTS, param_vmap_wid, param_vmap_len)!=PASS) {
					TDM_ERROR0("Critical error in 100G time vector preallocation\n");
					return 0;
				}
			}
	...
	if(_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.y5 > 0){
		for (v=_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.y5; v>0; v--) {
			if (tdm_gh2_legacy_which_tsc(lr50[v+1],_tdm->_chip_data.soc_pkg.pmap)==tdm_gh2_legacy_which_tsc(lr50[v],_tdm->_chip_data.soc_pkg.pmap) ||
				tdm_gh2_legacy_which_tsc(lr50[v-1],_tdm->_chip_data.soc_pkg.pmap)==tdm_gh2_legacy_which_tsc(lr50[v],_tdm->_chip_data.soc_pkg.pmap)) {
		...
		for (v=_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.y5; v>0; v--) {
			_tdm->_core_data.vars_pkg.m_tdm_vmap_alloc.tri_chk = tdm_gh2_legacy_which_tsc(lr50[v],_tdm->_chip_data.soc_pkg.pmap);
		...
		if (tdm_gh2_check_pipe_ethernet(lr50[_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.y5],_tdm) && _tdm->_chip_data.soc_pkg.soc_vars.gh2.higig_mgmt==BOOL_FALSE && _tdm->_chip_data.soc_pkg.soc_vars.gh2.cal_hg_en==BOOL_FALSE) {
			if (tdm_core_postalloc_vmap(param_vmap, param_freq, 50, &_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.yy, &(_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.y5), param_lr_limit, lr50, TOKEN_50G, "50", GH2_NUM_EXT_PORTS, param_vmap_wid, param_vmap_len)!=PASS) {
				TDM_ERROR0("Critical error in 50G time vector preallocation\n");
				return 0;
			}
		}
		else {
			if (tdm_core_postalloc_vmap(param_vmap, param_freq, 53, &_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.yy, &(_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.y5), param_lr_limit, lr50, TOKEN_50G, "50", GH2_NUM_EXT_PORTS, param_vmap_wid, param_vmap_len)!=PASS) {
				TDM_ERROR0("Critical error in 50G time vector preallocation\n");
				return 0;
			}
		}
	...
	if (_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.y3 > 0) {
		for (v=_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.y3; v>0; v--) {
			if (tdm_gh2_legacy_which_tsc(lr40[v+1],_tdm->_chip_data.soc_pkg.pmap)==tdm_gh2_legacy_which_tsc(lr40[v],_tdm->_chip_data.soc_pkg.pmap) ||
				tdm_gh2_legacy_which_tsc(lr40[v-1],_tdm->_chip_data.soc_pkg.pmap)==tdm_gh2_legacy_which_tsc(lr40[v],_tdm->_chip_data.soc_pkg.pmap)) {
		...
		for (v=_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.y3; v>0; v--) {
			_tdm->_core_data.vars_pkg.m_tdm_vmap_alloc.tri_chk = tdm_gh2_legacy_which_tsc(lr40[v],_tdm->_chip_data.soc_pkg.pmap);
			if ((_tdm->_chip_data.soc_pkg.pmap[_tdm->_core_data.vars_pkg.m_tdm_vmap_alloc.tri_chk][0]!=_tdm->_chip_data.soc_pkg.pmap[_tdm->_core_data.vars_pkg.m_tdm_vmap_alloc.tri_chk][2] && _tdm->_chip_data.soc_pkg.pmap[_tdm->_core_data.vars_pkg.m_tdm_vmap_alloc.tri_chk][1]==_tdm->_chip_data.soc_pkg.pmap[_tdm->_core_data.vars_pkg.m_tdm_vmap_alloc.tri_chk][3]) ||
			    (_tdm->_chip_data.soc_pkg.pmap[_tdm->_core_data.vars_pkg.m_tdm_vmap_alloc.tri_chk][0]==_tdm->_chip_data.soc_pkg.pmap[_tdm->_core_data.vars_pkg.m_tdm_vmap_alloc.tri_chk][2] && _tdm->_chip_data.soc_pkg.pmap[_tdm->_core_data.vars_pkg.m_tdm_vmap_alloc.tri_chk][1]!=_tdm->_chip_data.soc_pkg.pmap[_tdm->_core_data.vars_pkg.m_tdm_vmap_alloc.tri_chk][3]) ){
		...
		if (_tdm->_core_data.vars_pkg.HG4X106G_3X40G==BOOL_TRUE) {
			if (_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.y3==3) {
				if (tdm_core_postalloc_vmap(param_vmap, param_freq, 43, &_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.yy, &(_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.y3), param_lr_limit, lr40, TOKEN_40G, "40", GH2_NUM_EXT_PORTS, param_vmap_wid, param_vmap_len)!=PASS) {
					TDM_ERROR0("Critical error in 40G time vector preallocation\n");
					return 0;
				}
			}
			else if (_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.y3==2) {
				if (tdm_core_postalloc_vmap(param_vmap, param_freq, 44, &_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.yy, &(_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.y3), param_lr_limit, lr40, TOKEN_40G, "40", GH2_NUM_EXT_PORTS, param_vmap_wid, param_vmap_len)!=PASS) {
					TDM_ERROR0("Critical error in 40G time vector preallocation\n");
					return 0;
				}
			}
			else if (_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.y3==1) {
				if (tdm_core_postalloc_vmap(param_vmap, param_freq, 45, &_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.yy, &(_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.y3), param_lr_limit, lr40, TOKEN_40G, "40", GH2_NUM_EXT_PORTS, param_vmap_wid, param_vmap_len)!=PASS) {
					TDM_ERROR0("Critical error in 40G time vector preallocation\n");
					return 0;
				}
			}
			...
			if (tdm_gh2_check_pipe_ethernet(lr40[_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.y3],_tdm) && _tdm->_chip_data.soc_pkg.soc_vars.gh2.higig_mgmt==BOOL_FALSE && _tdm->_chip_data.soc_pkg.soc_vars.gh2.cal_hg_en==BOOL_FALSE) {
				if (tdm_core_postalloc_vmap(param_vmap, param_freq, 40, &_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.yy, &(_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.y3), param_lr_limit, lr40, TOKEN_40G, "40", GH2_NUM_EXT_PORTS, param_vmap_wid, param_vmap_len)!=PASS) {
					TDM_ERROR0("Critical error in 40G time vector preallocation\n");
					return 0;
				}
			}
			else {
				if (tdm_core_postalloc_vmap(param_vmap, param_freq, 42, &_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.yy, &(_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.y3), param_lr_limit, lr40, TOKEN_40G, "40", GH2_NUM_EXT_PORTS, param_vmap_wid, param_vmap_len)!=PASS) {
					TDM_ERROR0("Critical error in 40G time vector preallocation\n");
					return 0;
				}
			}
	...
	if (_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.y6 > 0) {
		if (tdm_gh2_check_pipe_ethernet(lr25[_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.y6],_tdm) && _tdm->_chip_data.soc_pkg.soc_vars.gh2.higig_mgmt==BOOL_FALSE && _tdm->_chip_data.soc_pkg.soc_vars.gh2.cal_hg_en==BOOL_FALSE) {
			if (tdm_core_postalloc_vmap(param_vmap, param_freq, 25, &_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.yy, &(_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.y6), param_lr_limit, lr25, TOKEN_25G, "25", GH2_NUM_EXT_PORTS, param_vmap_wid, param_vmap_len)!=PASS) {
				TDM_ERROR0("Critical error in 25G time vector preallocation\n");
				return 0;
			}
		}
		else {
			if (tdm_core_postalloc_vmap(param_vmap, param_freq, 27, &_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.yy, &(_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.y6), param_lr_limit, lr25, TOKEN_25G, "25", GH2_NUM_EXT_PORTS, param_vmap_wid, param_vmap_len)!=PASS) {
				TDM_ERROR0("Critical error in 25G time vector preallocation\n");
				return 0;
			}
		}
	...
	if (_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.y2 > 0) {
		for (v=_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.y2; v>0; v--) {
			_tdm->_core_data.vars_pkg.m_tdm_vmap_alloc.tri_chk = tdm_gh2_legacy_which_tsc(lr20[v],_tdm->_chip_data.soc_pkg.pmap);
			if ((_tdm->_chip_data.soc_pkg.pmap[_tdm->_core_data.vars_pkg.m_tdm_vmap_alloc.tri_chk][0]!=_tdm->_chip_data.soc_pkg.pmap[_tdm->_core_data.vars_pkg.m_tdm_vmap_alloc.tri_chk][2] && _tdm->_chip_data.soc_pkg.pmap[_tdm->_core_data.vars_pkg.m_tdm_vmap_alloc.tri_chk][1]==_tdm->_chip_data.soc_pkg.pmap[_tdm->_core_data.vars_pkg.m_tdm_vmap_alloc.tri_chk][3]) ||
			    (_tdm->_chip_data.soc_pkg.pmap[_tdm->_core_data.vars_pkg.m_tdm_vmap_alloc.tri_chk][0]==_tdm->_chip_data.soc_pkg.pmap[_tdm->_core_data.vars_pkg.m_tdm_vmap_alloc.tri_chk][2] && _tdm->_chip_data.soc_pkg.pmap[_tdm->_core_data.vars_pkg.m_tdm_vmap_alloc.tri_chk][1]!=_tdm->_chip_data.soc_pkg.pmap[_tdm->_core_data.vars_pkg.m_tdm_vmap_alloc.tri_chk][3]) ){
		...
		if (tdm_gh2_check_pipe_ethernet(lr20[_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.y2],_tdm) && _tdm->_chip_data.soc_pkg.soc_vars.gh2.higig_mgmt==BOOL_FALSE && _tdm->_chip_data.soc_pkg.soc_vars.gh2.cal_hg_en==BOOL_FALSE) {
			if (tdm_core_postalloc_vmap(param_vmap, param_freq, 20, &_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.yy, &(_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.y2), param_lr_limit, lr20, TOKEN_20G, "20", GH2_NUM_EXT_PORTS, param_vmap_wid, param_vmap_len)!=PASS) {
				TDM_ERROR0("Critical error in 20G time vector preallocation\n");
				return 0;
			}
		}
		else {
			if (tdm_core_postalloc_vmap(param_vmap, param_freq, 21, &_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.yy, &(_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.y2), param_lr_limit, lr20, TOKEN_20G, "20", GH2_NUM_EXT_PORTS, param_vmap_wid, param_vmap_len)!=PASS) {
				TDM_ERROR0("Critical error in 20G time vector preallocation\n");
				return 0;
			}
		}
	...
	if (_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.y1 > 0) {
		if (tdm_gh2_check_pipe_ethernet(lr10[_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.y1],_tdm) && _tdm->_chip_data.soc_pkg.soc_vars.gh2.higig_mgmt==BOOL_FALSE && _tdm->_chip_data.soc_pkg.soc_vars.gh2.cal_hg_en==BOOL_FALSE) {
			if (tdm_core_postalloc_vmap(param_vmap, param_freq, 10, &_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.yy, &(_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.y1), param_lr_limit, lr10, TOKEN_10G, "10", GH2_NUM_EXT_PORTS, param_vmap_wid, param_vmap_len)!=PASS) {
				TDM_ERROR0("Critical error in 10G time vector preallocation\n");
				return 0;
			}
		}
		else {
			if (tdm_core_postalloc_vmap(param_vmap, param_freq, 11, &_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.yy, &(_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.y1), param_lr_limit, lr10, TOKEN_10G, "10", GH2_NUM_EXT_PORTS, param_vmap_wid, param_vmap_len)!=PASS) {
				TDM_ERROR0("Critical error in 10G time vector preallocation\n");
				return 0;
			}
		}
	...
	if (_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.y8 > 0) {
		if (tdm_core_postalloc_vmap(param_vmap, param_freq, 1, &_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.yy, &(_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.y8), param_lr_limit, lr1, TOKEN_1G, "1", GH2_NUM_EXT_PORTS, param_vmap_wid, param_vmap_len)!=PASS) {
...
/**
@name: tdm_gh2_proc_cal_ancl
@param:

Allocate ancillary slots with matched speed rates accordingly
**/
int
tdm_gh2_proc_cal_ancl(tdm_mod_t *_tdm)
{
...
/**
@name: tdm_gh2_proc_cal_idle
@param:

Allocate idle slots with matched speed rates accordingly
**/
int
tdm_gh2_proc_cal_idle( tdm_mod_t *_tdm )
{
    int i, result=PASS;
    int param_lr_limit, param_ancl_num, param_cal_len, 
...
tdm_gh2_proc_cal_lr( tdm_mod_t *_tdm )
{
    ...
    /* allocate Linerate slots (step-2): vmap allocation */
        ...
        if (_tdm->_core_data.vars_pkg.m_tdm_vmap_alloc.num_40g==3 &&
            ...
            _tdm->_chip_data.soc_pkg.clk_freq==GH2_CLK_850MHZ) {
            if (tdm_gh2_check_pipe_ethernet(param_lr_buff[0],_tdm)!=BOOL_TRUE||
