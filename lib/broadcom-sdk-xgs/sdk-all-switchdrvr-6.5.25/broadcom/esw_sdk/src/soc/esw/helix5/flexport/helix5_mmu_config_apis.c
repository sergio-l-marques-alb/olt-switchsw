...
/* ASF core config */
/*! @struct soc_hx5_mmu_asf_core_cfg_s
 *  @brief
 */
typedef struct soc_hx5_mmu_asf_core_cfg_s {
    int speed;
...
} soc_hx5_mmu_asf_core_cfg_t;

/* ASF Core Config Table  */
static const soc_hx5_mmu_asf_core_cfg_t
    soc_hx5_mmu_asf_cfg_tbl[] = {
...
/*! @struct soc_hx5_mmu_thdi_pg_hdrm_cfg_s
 *  @brief
 */
typedef struct soc_hx5_mmu_thdi_pg_hdrm_cfg_s {
    int speed;
    uint32 line_rate_cells;
    uint32 oversub_cells;
} soc_hx5_mmu_thdi_pg_hdrm_cfg_t;

static const soc_hx5_mmu_thdi_pg_hdrm_cfg_t soc_hx5_mmu_thdi_pg_hdrm_cfg_tbl[]
    = {
    {1000,  118, 0},
    {2500,  120, 0},
    {5000,  160, 194},
...
/*! @fn int soc_helix5_mmu_thdi_get_pg_hdrm_setting(int speed, int line_rate,
 *              int* setting)
 ...
 */
int
soc_helix5_mmu_thdi_get_pg_hdrm_setting(int unit, int speed, int line_rate,
                                           uint32* setting)
{
    int index;
    int found = 0;
    for(index=0; index<HX5_MMU_NUM_SPEEDS; index++) {
        if (soc_hx5_mmu_thdi_pg_hdrm_cfg_tbl[index].speed == speed) {
            *setting =
                line_rate ? soc_hx5_mmu_thdi_pg_hdrm_cfg_tbl[index].
                line_rate_cells
                : soc_hx5_mmu_thdi_pg_hdrm_cfg_tbl[index].oversub_cells;
            found = 1;
            break;
        }
    }
    if(!found) {
        LOG_CLI((BSL_META_U(unit,
                       "In soc_helix5_mmu_thdi_get_pg_hdrm_setting"
...
/*! @fn int soc_helix5_mmu_get_num_l0_nodes_per_port(int unit, int lcl_port,
 ...
 */
int
soc_helix5_mmu_get_num_l0_nodes_per_port(int unit, int lcl_port, int pipe,
                                            int *number_l0_nodes)
{
    if (lcl_port < HX5_MMU_NUM_UP_PORTS) {
        *number_l0_nodes = HX5_MMU_NUM_L0_NODES_PER_HSP_UP_PORT;
    }
    else if (lcl_port < HX5_MMU_CPU_PORT_NUM) {
        *number_l0_nodes = HX5_MMU_NUM_L0_NODES_PER_HSP_DL_PORT;
    }
    else if (HX5_MMU_IS_CPU_PORT(pipe, lcl_port)) {
        *number_l0_nodes = HX5_MMU_NUM_L0_NODES_CPU_PORT;
    }
    else if (HX5_MMU_IS_LPBK_PORT(lcl_port)) {
        *number_l0_nodes = HX5_MMU_NUM_L0_NODES_LPBK_PORT;
    }
    else {
...
/*! @fn int soc_helix5_mmu_get_num_l1_nodes_per_port(int unit, int lcl_port,
 ...
 */
int
soc_helix5_mmu_get_num_l1_nodes_per_port(int unit, int lcl_port, int pipe,
                                            int *number_l1_nodes)
{
    if (lcl_port < HX5_MMU_NUM_UP_PORTS) {
        *number_l1_nodes = HX5_MMU_NUM_L1_NODES_PER_HSP_UP_PORT;
    }
    else if (lcl_port < HX5_MMU_CPU_PORT_NUM) {
        *number_l1_nodes = HX5_MMU_NUM_L1_NODES_PER_HSP_DL_PORT;
    }
    else if (HX5_MMU_IS_CPU_PORT(pipe, lcl_port)) {
        *number_l1_nodes = HX5_MMU_NUM_L1_NODES_CPU_PORT;
    }
    else if (HX5_MMU_IS_LPBK_PORT(lcl_port)) {
        *number_l1_nodes = HX5_MMU_NUM_L1_NODES_LPBK_PORT;
    }
    else {
        LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META_U(unit,
...
/*! @fn int soc_helix5_mmu_get_num_l2_nodes_per_port(int unit, int lcl_port,
 ...
 */
int
soc_helix5_mmu_get_num_l2_nodes_per_port(int unit, int lcl_port, int pipe,
                                            int *number_l2_nodes)
{
    if (lcl_port < HX5_MMU_NUM_UP_PORTS) {
        *number_l2_nodes = HX5_MMU_NUM_L2_NODES_PER_HSP_UP_PORT;
    }
    else if (lcl_port < HX5_MMU_CPU_PORT_NUM) {
        *number_l2_nodes = HX5_MMU_NUM_L2_NODES_PER_HSP_DL_PORT;
    }
    else if (HX5_MMU_IS_CPU_PORT(pipe, lcl_port)) {
        *number_l2_nodes = HX5_MMU_NUM_L2_NODES_CPU_PORT;
    }
    else if (HX5_MMU_IS_LPBK_PORT(lcl_port)) {
        *number_l2_nodes = HX5_MMU_NUM_L2_NODES_LPBK_PORT;
    }
    else {
        LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META_U(unit,
...
/*! @fn int soc_helix5_mmu_get_num_l3_uc_nodes_per_port(int unit,
 ...
 */
int
soc_helix5_mmu_get_num_l3_uc_nodes_per_port(int unit, int lcl_port, int pipe,
                                               int *number_l3_uc_nodes)
{
    if (lcl_port < HX5_MMU_NUM_UP_PORTS) {
        *number_l3_uc_nodes = HX5_MMU_NUM_UCQ_PER_HSP_UP_PORT;
    }
     else if (lcl_port < HX5_MMU_CPU_PORT_NUM) {
        *number_l3_uc_nodes = HX5_MMU_NUM_UCQ_PER_HSP_DL_PORT;
    }
    else {
        LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META_U(unit,
...
/*! @fn int soc_helix5_mmu_get_num_l3_mc_nodes_per_port(int unit,
 ...
 */
int
soc_helix5_mmu_get_num_l3_mc_nodes_per_port(int unit, int lcl_port, int pipe,
                                               int *number_l3_mc_nodes)
{
    if (lcl_port < HX5_MMU_NUM_UP_PORTS) {
        *number_l3_mc_nodes = HX5_MMU_NUM_MCQ_PER_HSP_UP_PORT;
    }
		else if (lcl_port < HX5_MMU_CPU_PORT_NUM) {
        *number_l3_mc_nodes = HX5_MMU_NUM_MCQ_PER_HSP_DL_PORT;
    }
    else if (lcl_port == HX5_MMU_CPU_PORT_NUM) {
        *number_l3_mc_nodes = HX5_MMU_NUM_L3_NODES_CPU_PORT;
    }
    else if (lcl_port == HX5_MMU_LPBK_PORT_NUM) {
        *number_l3_mc_nodes = HX5_MMU_NUM_L3_NODES_LPBK_PORT;
    }
    else {
        LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META_U(unit,
...
/*! @fn int soc_helix5_mmu_get_valid_ipipes_for_xpe(int unit, int xpe,
 ...
 */
int
soc_helix5_mmu_get_valid_ipipes_for_xpe(int unit, int xpe,
                                           int *ipipes)
{
    int i;
...
/*! @fn int soc_helix5_mmu_get_valid_epipes_for_xpe(int unit, int xpe,
 ...
 */
int
soc_helix5_mmu_get_valid_epipes_for_xpe(int unit, int xpe,
                                           int *epipes)
{
    int i;
    for (i=0; i<SOC_MAX_NUM_PIPES; i++)
...
/*! @fn int soc_helix5_mmu_get_l0_base_offset_for_port(int unit,
 ...
 */
int
soc_helix5_mmu_get_l0_base_offset_for_port(int unit, int lcl_port,
                                              int *l0_offset)
{
    *l0_offset = 0;
    if (lcl_port < HX5_MMU_NUM_UP_PORTS) {
        *l0_offset = lcl_port * HX5_MMU_NUM_L0_NODES_PER_HSP_UP_PORT;
    }
		else if (lcl_port < HX5_MMU_CPU_PORT_NUM) {
        *l0_offset = HX5_MMU_NUM_UP_PORTS*HX5_MMU_NUM_L0_NODES_PER_HSP_UP_PORT + 
										 (lcl_port-HX5_MMU_NUM_UP_PORTS)*HX5_MMU_NUM_L0_NODES_PER_HSP_DL_PORT;
    }
		else if (lcl_port == HX5_MMU_CPU_PORT_NUM) {
        *l0_offset = HX5_MMU_CPU_PORT_L0_START;
    }
		else if (lcl_port == HX5_MMU_LPBK_PORT_NUM) {
        *l0_offset = HX5_MMU_LPBK_PORT_L0_START;
    }
    else {
        LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META_U(unit,
...
/*! @fn int soc_helix5_mmu_get_l1_base_offset_for_port(int unit,
 ...
 */
int
soc_helix5_mmu_get_l1_base_offset_for_port(int unit, int lcl_port,
                                              int *l1_offset)
{
    *l1_offset = 0;
    if (lcl_port < HX5_MMU_NUM_UP_PORTS) {
        *l1_offset = lcl_port * HX5_MMU_NUM_L1_NODES_PER_HSP_UP_PORT;
    }
		else if (lcl_port < HX5_MMU_CPU_PORT_NUM) {
        *l1_offset = HX5_MMU_NUM_UP_PORTS*HX5_MMU_NUM_L1_NODES_PER_HSP_UP_PORT + 
										 (lcl_port-HX5_MMU_NUM_UP_PORTS)*HX5_MMU_NUM_L1_NODES_PER_HSP_DL_PORT;
    }
		else if (lcl_port == HX5_MMU_CPU_PORT_NUM) {
        *l1_offset = HX5_MMU_CPU_PORT_L1_START;
    }
		else if (lcl_port == HX5_MMU_LPBK_PORT_NUM) {
        *l1_offset = HX5_MMU_LPBK_PORT_L1_START;
    }
    else {
        LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META_U(unit,
...
/*! @fn int soc_helix5_mmu_get_l2_base_offset_for_port(int unit,
 ...
 */
int
soc_helix5_mmu_get_l2_base_offset_for_port(int unit, int lcl_port,
                                              int *l2_offset)
{
    *l2_offset = 0;
    if (lcl_port < HX5_MMU_NUM_UP_PORTS) {
        *l2_offset = lcl_port * HX5_MMU_NUM_L2_NODES_PER_HSP_UP_PORT;
    }
		else if (lcl_port < HX5_MMU_CPU_PORT_NUM) {
        *l2_offset = HX5_MMU_NUM_UP_PORTS*HX5_MMU_NUM_L2_NODES_PER_HSP_UP_PORT + 
										 (lcl_port-HX5_MMU_NUM_UP_PORTS)*HX5_MMU_NUM_L2_NODES_PER_HSP_DL_PORT;
    }
		else if (lcl_port == HX5_MMU_CPU_PORT_NUM) {
        *l2_offset = HX5_MMU_CPU_PORT_L2_START;
    }
		else if (lcl_port == HX5_MMU_LPBK_PORT_NUM) {
        *l2_offset = HX5_MMU_LPBK_PORT_L2_START;
    }
    else {
        LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META_U(unit,
...
/*! @fn int soc_helix5_mmu_get_l3_base_offset_for_port(int unit,
 ...
 */
int
soc_helix5_mmu_get_l3_base_offset_for_port(int unit, int lcl_port,
                                              int uc_bit,
                                              int *l3_offset)
{
    *l3_offset = 0;
    if (uc_bit == 1) {
        if (lcl_port < HX5_MMU_NUM_UP_PORTS) 
				{
            *l3_offset = lcl_port * HX5_MMU_NUM_UCQ_PER_HSP_UP_PORT;
        }
        else if (lcl_port < HX5_MMU_CPU_PORT_NUM) 
				{
            *l3_offset =HX5_MMU_NUM_UP_PORTS*HX5_MMU_NUM_UCQ_PER_HSP_UP_PORT + 
										 (lcl_port-HX5_MMU_NUM_UP_PORTS)*HX5_MMU_NUM_UCQ_PER_HSP_DL_PORT; 
        } 
				else {
            return SOC_E_FAIL;
        }
        ...
        if (lcl_port < HX5_MMU_NUM_UP_PORTS) {
            *l3_offset = HX5_MMU_MCQ_NUMBER_START +
                         (lcl_port * HX5_MMU_NUM_MCQ_PER_HSP_UP_PORT);
        }
				else if (lcl_port < HX5_MMU_CPU_PORT_NUM) {
            *l3_offset = HX5_MMU_MCQ_NUMBER_START + (HX5_MMU_NUM_UP_PORTS*HX5_MMU_NUM_MCQ_PER_HSP_UP_PORT + (lcl_port-HX5_MMU_NUM_UP_PORTS)*HX5_MMU_NUM_MCQ_PER_HSP_DL_PORT);            
        }
        else if (lcl_port == HX5_MMU_LPBK_PORT_NUM) {
            *l3_offset = HX5_MMU_LPBK_PORT_L3_START;
        }
        else if (lcl_port == HX5_MMU_CPU_PORT_NUM) {
            *l3_offset = HX5_MMU_CPU_PORT_L3_START;
        }
        else {
            LOG_ERROR(BSL_LS_SOC_COMMON,
                      (BSL_META_U(unit,
...
/*! @fn int soc_helix5_mmu_return_dip_delay_amount(int unit, int port_speed,
 *              int *dip_amount)
 *  @param unit Device number
 ...
 */
int
soc_helix5_mmu_return_dip_delay_amount(int unit, int port_speed,
                                          uint64 *dip_amount)
{
    int rand_num;
    unsigned int seed=0;
...
/*! @fn int soc_helix5_get_min_max_src_ct_speed(int unit, int speed_decode,
 *              int *min_speed, int *max_speed)
 *  @param unit Device number
 *  @param speed_decode Decoded speed value based on the function
 *         soc_helix5_mmu_decode_speed_setting
 ...
 *         is done in the soc_hx5_mmu_asf_cfg_tbl struct
 */
int
soc_helix5_get_min_max_src_ct_speed(int unit, int speed_decode,
    ...
    if ((ct_profile == _SOC_ASF_MEM_PROFILE_SIMILAR) ||
        ...
        if (ct_mode == _SOC_ASF_MODE_SAME_SPEED) {
            ...
            COMPILER_64_SET(*min_speed, 0,
                            soc_hx5_mmu_asf_cfg_tbl[speed_decode].min_sp);
            COMPILER_64_SET(*max_speed, 0,  speed_decode);
        }
            ...
            if (ct_profile == _SOC_ASF_MEM_PROFILE_SIMILAR) {
                ...
                COMPILER_64_SET(*max_speed, 0,
                                (soc_hx5_mmu_asf_cfg_tbl[speed_decode].max_sp));
            }
        }
    }
...
/*! @fn int soc_helix5_get_asf_xmit_start_count(int unit, int dst_decode,
 ...
 */
int
soc_helix5_get_asf_xmit_start_count(int unit, int dst_decode, int ct_mode,
                                       uint64 *asf_count)
{
    if (dst_decode > HX5_MMU_NUM_CT_SPEED_GROUPS) {
        return SOC_E_FAIL;
    }
    ...
    if (ct_mode == _SOC_ASF_MODE_SAME_SPEED || ct_mode ==
        ...
        COMPILER_64_SET(*asf_count, 0,
                        soc_hx5_mmu_asf_cfg_tbl[dst_decode].mmu_prebuffer);
    }
    else {
        COMPILER_64_SET(*asf_count, 0, 0);
    }
...
/*! @fn int soc_helix5_get_ct_fifo_threshold_depth(int unit, int dst_decode,
 ...
 */
int
soc_helix5_get_ct_fifo_threshold_depth(int unit, int dst_decode, int ct_mode,
                                          int oversub,
    ...
    if (oversub == 0) {
        if (ct_mode == _SOC_ASF_MODE_SAME_SPEED) {
            COMPILER_64_SET(*ct_fifo_thresh, 0,
                soc_hx5_mmu_asf_cfg_tbl[dst_decode].ct_fifo_threshold_lr);
            COMPILER_64_SET(*ct_fifo_depth, 0,
                soc_hx5_mmu_asf_cfg_tbl[dst_decode].ct_fifo_depth_lr);
        }
        else if (ct_mode == _SOC_ASF_MODE_FAST_TO_SLOW) {
            COMPILER_64_SET(*ct_fifo_thresh, 0,
                soc_hx5_mmu_asf_cfg_tbl[dst_decode].ct_fifo_threshold_lr);
            COMPILER_64_SET(*ct_fifo_depth, 0,
                (soc_hx5_mmu_asf_cfg_tbl[dst_decode].ct_fifo_depth_lr+
                 soc_hx5_mmu_asf_cfg_tbl[dst_decode].fast_to_slow_accum));
            COMPILER_64_ZERO(temp64);
            COMPILER_64_ADD_32(temp64,
                soc_hx5_mmu_asf_cfg_tbl[dst_decode].ct_fifo_depth_lr);
            COMPILER_64_ADD_32(temp64, HX5_MAX_NUM_CELLS);
        ...
        if (ct_mode == _SOC_ASF_MODE_SAME_SPEED) {
            COMPILER_64_SET(*ct_fifo_thresh, 0,
                soc_hx5_mmu_asf_cfg_tbl[dst_decode].ct_fifo_threshold_os);
            COMPILER_64_SET(*ct_fifo_depth, 0,
                soc_hx5_mmu_asf_cfg_tbl[dst_decode].ct_fifo_depth_os);
...
/*! @fn int soc_helix5_mmu_get_bst_mode(int unit, int *bst_mode)
 *  @param unit Device number
 *  @param *bst_mode Return value bst mode
 *  @brief API to return the bst mode programmed
 */
int
soc_helix5_mmu_get_bst_mode(int unit, int *bst_mode)
{
    uint64 mmu_rval64;
    uint32 mmu_rval32;
...
/*! @fn int soc_helix5_mmu_get_pktstat_mode(int unit, int *pktstat_mode)
 *  @param unit Device number
 *  @brief API to return the pktstat mode programmed
 */
int
soc_helix5_mmu_get_pktstat_mode(int unit, int *pktstat_mode)
{
    uint64 mmu_rval64;
    uint32 mmu_rval32;
...
/*! @fn int soc_helix5_mmu_get_ct_ep_credit_low_high_threshold(int unit,
 ...
 */
int
soc_helix5_mmu_get_ct_ep_credit_low_high_threshold(int  unit,
                                                      int  speed_decode,
                                                      int  frequency,
                                                      int  oversub_ratio,
                                                      int  oversub,
                                                      ...
                                                      uint64 *ep_credit_hi)
{						      
    if (speed_decode > HX5_MMU_NUM_CT_SPEED_GROUPS) {
        return SOC_E_FAIL;
    }
    if (frequency< 1525 ||
        ((frequency == 1525) && (oversub ==1) && (oversub_ratio > 15))) {
        COMPILER_64_SET(*ep_credit_low, 0,
                        soc_hx5_mmu_asf_cfg_tbl[speed_decode].ep_credit_low);
        COMPILER_64_SET(*ep_credit_hi, 0,
                        soc_hx5_mmu_asf_cfg_tbl[speed_decode].ep_credit_hi);
    }
    else {
        COMPILER_64_SET(*ep_credit_low, 0,
                   soc_hx5_mmu_asf_cfg_tbl[speed_decode].ep_credit_low_1dot5);
        COMPILER_64_SET(*ep_credit_hi, 0,
                   soc_hx5_mmu_asf_cfg_tbl[speed_decode].ep_credit_hi_1dot5);
    }
...
/*! @fn int soc_helix5_mmu_get_pipe_flexed_status(int unit,
 ...
 */
int
soc_helix5_mmu_get_pipe_flexed_status(int                         unit,
