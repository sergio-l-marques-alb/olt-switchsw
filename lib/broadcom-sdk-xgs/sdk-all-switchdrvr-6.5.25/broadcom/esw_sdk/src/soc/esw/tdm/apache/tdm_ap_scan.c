...
/**
@name: tdm_ap_which_tsc
@param:

Returns the TSC to which the input port belongs given pointer to transcribed pmap
**/
int
tdm_ap_which_tsc( tdm_mod_t *_tdm_s )
{
	AP_TOKEN_CHECK(_tdm_s->_core_data.vars_pkg.port) {
...
/**
@name: tdm_ap_legacy_which_tsc
@param:

Returns the TSC to which the input port belongs given pointer to transcribed pmap
**/
int
tdm_ap_legacy_which_tsc(unsigned short port, int **tsc)
{
...
tdm_ap_check_ethernet( tdm_mod_t *_tdm_s)
{
	...
	for (i=0; i<AP_NUM_PHY_PM; i++) {
		...
		for (i=1; i<73; i++) {
		  port_tsc = tdm_ap_scan_which_tsc(i,tsc);     
          if (port_tsc<_tdm_s->_chip_data.soc_pkg.pm_num_phy_modules) {
            /*
             * COVERITY
             *
             * The value of the above variable "port_tsc" is guaranteed
             * less than 18 once the program going into this block, because
             * "_tdm_s->_chip_data.soc_pkg.pm_num_phy_modules" is
             * initialized to 18 (AP_NUM_PHY_PM).
             *
             */
            /* coverity[overrun-local] */
			if (_tdm_s->_chip_data.soc_pkg.speed[i]!=SPEED_0 && _tdm_s->_chip_data.soc_pkg.soc_vars.ap.pm_encap_type[port_tsc]==PM_ENCAP__HIGIG2) {
				type=BOOL_FALSE;
				break;
			}
          }
...
/**
@name: tdm_ap_check_same_port_dist_dn
@param:
...
**/
int
tdm_ap_check_same_port_dist_dn(int idx, int *tdm_tbl, int lim)
{
	int j, dist=1, slot;
...
/**
@name: tdm_ap_check_same_port_dist_up
@param:
...
**/
int
tdm_ap_check_same_port_dist_up(int idx, int *tdm_tbl, int lim)
{
	int j, dist=1, slot;
...
/**
@name: tdm_ap_check_same_port_dist_dn_port
@param:
...
**/
int
tdm_ap_check_same_port_dist_dn_port(int port, int idx, int *tdm_tbl, int lim)
{
	int j, dist=1, slot;
...
/**
@name: tdm_ap_check_same_port_dist_up_port
@param:
...
**/
int
tdm_ap_check_same_port_dist_up_port(int port, int idx, int *tdm_tbl, int lim)
{
	int j, dist=1, slot;
...
/**
@name: tdm_ap_slice_size_local
@param:

Given index, returns size of largest contiguous slice
**/
int
tdm_ap_slice_size_local(unsigned short idx, int *tdm, int lim)
{
	int i, slice_size=(-1);
...
/**
@name: tdm_ap_slice_size
@param:

Given port number, returns size of largest slice
**/
int
tdm_ap_slice_size(unsigned short port, int *tdm, int lim)
{
...
/**
@name: tdm_ap_slice_idx
@param:

Given port number, returns index of largest slice
**/
int
tdm_ap_slice_idx(unsigned short port, int *tdm, int lim)
{
...
/**
@name: tdm_ap_slice_prox_dn
@param:

Given port number, checks min spacing in a slice in down direction
**/
int
tdm_ap_slice_prox_dn(int slot, int *tdm, int lim, int **tsc, enum port_speed_e *speed)
{
	...
	wc=(tdm[slot]==AP_ANCL_TOKEN)?(tdm[slot]):(tdm_ap_legacy_which_tsc(tdm[slot],tsc));
	if (slot<=(lim-5)) {
		if ( wc==tdm_ap_legacy_which_tsc(tdm[slot+1],tsc) ||
		     wc==tdm_ap_legacy_which_tsc(tdm[slot+2],tsc) ||
		     wc==tdm_ap_legacy_which_tsc(tdm[slot+3],tsc) ||
			 wc==tdm_ap_legacy_which_tsc(tdm[slot+4],tsc) ) {
			slice_prox=FAIL;
		}
	}
	else {
		while (idx<lim) {
			if (wc==tdm_ap_legacy_which_tsc(tdm[idx],tsc)) {
				slice_prox=FAIL;
				break;
			}
			idx++; cnt++;
		...
		for (i=(lim-slot-cnt-1); i>=0; i--) {
			if (wc==tdm_ap_legacy_which_tsc(tdm[i],tsc)) {
				slice_prox=FAIL;
				break;
			}
		}
...
/**
@name: tdm_ap_slice_prox_up
@param:

Given port number, checks min spacing in a slice in up direction
**/
int
tdm_ap_slice_prox_up(int slot, int *tdm, int **tsc, enum port_speed_e *speed)
{
...
/**
@name: tdm_ap_check_fit_smooth
@param:

Inside of table array, returns number of nodes inside a port vector that clump with other nodes of the same type
**/
int
tdm_ap_check_fit_smooth(int *tdm_tbl, int port, int lr_idx_limit, int clump_thresh)
{
	int i, cnt=0;

	for (i=0; i<lr_idx_limit; i++) {
		if ( (tdm_tbl[i]==port) && (tdm_ap_slice_size_local(i,tdm_tbl,lr_idx_limit)>=clump_thresh) ) {
			cnt++;
		}
	}
...
/**
@name: tdm_ap_check_lls_flat_up
@param:

Checks LLS scheduler min spacing in tdm array, up direction only, returns dist
**/
int
tdm_ap_check_lls_flat_up(int idx, int *tdm_tbl, enum port_speed_e *speed)
{
...
/**
@name: tdm_ap_slice_prox_local
@param:

Given index, checks min spacing of two nearest non-token ports
**/
int
tdm_ap_slice_prox_local(unsigned short idx, int *tdm, int lim, int **tsc)
{
	int i, prox_len=0, wc=AP_NUM_EXT_PORTS;
	
	/* Nearest non-token port */
	AP_TOKEN_CHECK(tdm[idx]) {
		wc=tdm_ap_legacy_which_tsc(tdm[idx],tsc);
	}
	else {
		for (i=1; (idx-i)>=0; i++) {
			AP_TOKEN_CHECK(tdm[i]) {
				wc=tdm_ap_legacy_which_tsc(tdm[idx-i],tsc);
				break;
			}
		}
	}
	for (i=1; (idx+i)<lim; i++) {
		if (tdm_ap_legacy_which_tsc(tdm[idx+i],tsc)!=wc) {
			prox_len++;
		}
		else {
			break;
...
/**
@name: tdm_ap_num_lr_slots
@param:

**/
int
tdm_ap_num_lr_slots(int *tdm_tbl)
{
	int i, cnt=0;
...
/**
@name: tdm_ap_scan_slice_min
@param:

Given port number, returns the MIN size of port slices in an array
**/
int
tdm_ap_scan_slice_min(unsigned short port, int *tdm, int lim, int *slice_start_idx, int pos)
{
		...
		AP_TOKEN_CHECK(port) {
			for (i=0; i<lim; i++) {
				...
				AP_TOKEN_CHECK(tdm[idx0]) {
					k = tdm_ap_scan_slice_size_local(idx0, tdm, lim, &idx_start);
...
/**
@name: tdm_ap_scan_slice_max
@param:

Given port number, returns the MAX size of port slices in an array
**/
int
tdm_ap_scan_slice_max(unsigned short port, int *tdm, int lim, int *slice_start_idx, int pos)
{
...
/**
@name: tdm_ap_scan_slice_size_local
@param:

Given index, returns the largest size of local slice
**/
int
tdm_ap_scan_slice_size_local(unsigned short idx, int *tdm, int lim, int *slice_start_idx)
{
...
/**
@name: tdm_ap_slice_size_min
@param:

Given port number, returns the MIN size of port slices (mixed with ANCL) in an array
**/
int
tdm_ap_scan_mix_slice_min(unsigned short port, int *tdm, int lim, int *slice_start_idx, int pos)
{
	int i, k=0, idx0, slice_size_min=256, slice_idx=-1 , idx_start;
	if (pos>=0 && pos<lim) {
		/* linerate */
		AP_TOKEN_CHECK(port) {
			for (i=0; i<lim; i++) {
				idx0 = ((i+pos)<lim)?(i+pos):(i+pos-lim);
				AP_TOKEN_CHECK(tdm[idx0]) {
					k = tdm_ap_scan_mix_slice_size_local(idx0, tdm, lim, &idx_start);
					if(k>0 && k<slice_size_min){
						slice_size_min = k;
						slice_idx= idx_start;
					}
...
/**
@name: tdm_ap_scan_mix_slice_max
@param:

Given port number, returns the MAX size of port slices (mixed with ANCL) in an array
**/
int
tdm_ap_scan_mix_slice_max(unsigned short port, int *tdm, int lim, int *slice_start_idx, int pos)
{
	int i, k=0, idx0, slice_size_max=0, slice_idx=-1 , idx_start;
	if (pos>=0 && pos<lim) {
		/* linerate */
...
/**
@name: tdm_ap_scan_mix_slice_size_local
@param:

Given index, returns the largest size of local slice (mixed with ANCL)
**/
int
tdm_ap_scan_mix_slice_size_local(unsigned short idx, int *tdm, int lim, int *slice_start_idx)
{
	int i, slice_size=(-1), idx_start=(-1);
...
/**
@name: tdm_ap_check_slot_swap_cond
@param:
...
**/
int
tdm_ap_check_slot_swap_cond(int idx, int *tdm_tbl, int tdm_tbl_len, int **tsc, enum port_speed_e *speed)
{
...
/**
@name: tdm_ap_check_shift_cond_pattern
@param:
...
**/
int
tdm_ap_check_shift_cond_pattern(unsigned short port, int *tdm_tbl, int tdm_tbl_len, int **tsc, int dir)
{
	...
	if (shift_cond_pass==BOOL_TRUE) {
		...
		if (dir==DN) {
			for (i=0; i<(tdm_tbl_len-1); i++) {
				if (tdm_tbl[i]==port && tdm_tbl[i+1]!=AP_OVSB_TOKEN && tdm_tbl[i+1]!=AP_ANCL_TOKEN) {
	...
	if(shift_cond_pass==BOOL_TRUE){
		port_tsc = tdm_ap_legacy_which_tsc(port,tsc);
		/* Downward pattern: _x_ovsb_..._x_ovsb_..._x_ovsb_ */
		if (dir==DN){
			for (i=0; i<(tdm_tbl_len-1); i++) {
				if (tdm_tbl[i]==port) {
...
/**
@name: tdm_ap_check_shift_cond_local_slice
@param:
...
**/
int
tdm_ap_check_shift_cond_local_slice(unsigned short port, int *tdm_tbl, int tdm_tbl_len, int **tsc, int dir)
{
	...
	os_clump_max_last = tdm_ap_scan_slice_max(ovsb_token,tdm_tbl,tdm_tbl_len, &slice_idx, 0);
	lr_clump_max_last = tdm_ap_scan_mix_slice_max(1,tdm_tbl,tdm_tbl_len, &slice_idx, 0);
	lr_clump_min_last = tdm_ap_scan_mix_slice_min(1,tdm_tbl,tdm_tbl_len, &slice_idx, 0);
	...
	if ( (lr_clump_max_last<=1) || (lr_clump_max_last==2 && lr_clump_min_last==1) ) {
		...
		for (i=0; i<tdm_tbl_len; i++) {
			...
			if (tdm_tbl[idx0]==ovsb_token){
				os_clump_local_above = tdm_ap_scan_slice_size_local(idx0,tdm_tbl,tdm_tbl_len, &slice_idx);
			}
			if (tdm_tbl[idx1]==ovsb_token){
				os_clump_local_below = tdm_ap_scan_slice_size_local(idx1,tdm_tbl,tdm_tbl_len, &slice_idx);
			}
			...
			lr_clump_local_last = tdm_ap_scan_mix_slice_size_local(i,tdm_tbl,tdm_tbl_len, &slice_idx);
			...
			if (dir==DN){
				...
				if (tdm_tbl[idx0]!=ovsb_token){
					for (j=0; j<(tdm_tbl_len-2); j++){
						...
						if (tdm_tbl[idx1]==ovsb_token) {
							lr_clump_local_curr = 1 + tdm_ap_scan_mix_slice_size_local(idx0,tdm_tbl,tdm_tbl_len, &slice_idx);
							break;
						}
						else if (tdm_tbl[idx1]==filter_port) {
							lr_clump_local_curr = tdm_ap_scan_mix_slice_size_local(idx0,tdm_tbl,tdm_tbl_len, &slice_idx);
							break;
						}
					}
				}
...
**/
int
tdm_ap_scan_which_tsc( int port, int tsc[AP_NUM_PHY_PM][AP_NUM_PM_LNS] )
{
	int result=AP_NUM_EXT_PORTS;
	tdm_mod_t *_tdm_s;
	
	AP_TOKEN_CHECK(port){
		_tdm_s = tdm_chip_ap_shim__which_tsc_alloc(port, tsc);
		if (_tdm_s != NULL) {
			result = tdm_ap_which_tsc(_tdm_s);
			tdm_chip_ap_shim__which_tsc_free(_tdm_s);
		}
	}
