/*
 * $Id: maverick2_tdm.c, Exp $
 * $Copyright: (c) 2021 Broadcom.
 * Broadcom Proprietary and Confidential. All rights reserved.$
 *
 * File:        maverick2_tdm.c
 * Purpose:
 * Requires:
 */
...
/*! @fn void _soc_mv2_tdm_print_port_map(
        int unit,
 ...
 */
void
_soc_mv2_tdm_print_port_map(int unit, soc_port_map_type_t *port_map)
{
    int port;
    ...
    for (port = 0; port < MV2_NUM_EXT_PORTS; port++) {
...
/*! @fn void _soc_mv2_tdm_print_tdm_info(
        int unit,
 ...
 */
void
_soc_mv2_tdm_print_tdm_info(int unit, soc_tdm_schedule_t *tdm_info,
                            int hpipe, int is_ing)
{
    ...
    if (hpipe == 0) {
        for (cal_len = tdm_info->cal_len; cal_len > 0; cal_len--) {
            if (tdm_info->linerate_schedule[cal_len - 1] != MV2_NUM_EXT_PORTS) {
                break;
            }
        }
    ...
    for (j=0; j< tdm_info->num_ovs_groups; j++) {
        ...
        for (m=0; m< tdm_info->ovs_group_len; m++) {
            if (tdm_info->oversub_schedule[j][m] != MV2_NUM_EXT_PORTS) {
    ...
    if (is_ing == 1) {
        ...
        for (j=0; j<tdm_info->pkt_sch_or_ovs_space_len; j++) {
            ...
            if (tdm_info->pkt_sch_or_ovs_space[0][j] != MV2_NUM_EXT_PORTS) {
                LOG_VERBOSE(BSL_LS_SOC_COMMON, (BSL_META_U(unit, "\t%d"),
...
/*! @fn void _soc_mv2_tdm_print_port_resource(
        int unit,
 ...
 */
void
_soc_mv2_tdm_print_port_resource(int unit,
                                 soc_port_resource_t *port_info,
                                 int entry_num)
{
    int is_hg2, lane;
...
/*! @fn void _soc_mv2_tdm_print_schedule_state(
        int unit,
 ...
 */
void
_soc_mv2_tdm_print_schedule_state(int unit,
                                  soc_port_schedule_state_t *sch_info)
{
    ...
    LOG_VERBOSE(BSL_LS_SOC_COMMON, (BSL_META_U(unit,
                        "\nPRINTING soc_port_schedule_state_t"
                        "::[in_port_map]\n")));
    _soc_mv2_tdm_print_port_map(unit, &sch_info->in_port_map);

    if (sch_info->is_flexport == 1) {
        LOG_VERBOSE(BSL_LS_SOC_COMMON, (BSL_META_U(unit,
                            "\nPRINTING soc_port_schedule_state_t"
                            "::[out_port_map]\n")));
        _soc_mv2_tdm_print_port_map(unit, &sch_info->out_port_map);
    }
    ...
    if (print_tdm == 1) {
        for (pipe = 0; pipe< NUM_PIPE(unit); pipe++) {
            ...
            for (hpipe = 0; hpipe< MAX_SCH_SLICES; hpipe++) {
                ...
                _soc_mv2_tdm_print_tdm_info(unit, tdm_schedule, hpipe, 1);
            }
            LOG_VERBOSE(BSL_LS_SOC_COMMON, (BSL_META_U(unit,
            ...
            for (hpipe = 0; hpipe< MAX_SCH_SLICES; hpipe++) {
                tdm_schedule = &(sch_info->tdm_egress_schedule_pipe[
                                 pipe].tdm_schedule_slice[hpipe]);
                _soc_mv2_tdm_print_tdm_info(unit, tdm_schedule, hpipe, 0);
            }
        }
    }
    ...
    if ((sch_info->is_flexport == 1) && (sch_info->nport > 0)) {
        ...
        for (i = 0; i < sch_info->nport; i++) {
            _soc_mv2_tdm_print_port_resource(unit, &sch_info->resource[i], i);
        }
    }
}


/*! @fn void soc_mv2_tdm_get_port_ratio(
        int unit,
        soc_port_schedule_state_t *sch_info,
        int pm_num,
        int *pm_mode,
...
/*
 * #1   single: 100  -   x  -   x  -   x  SOC_MV2_PORT_RATIO_SINGLE
 * #2   single:  40  -   x  -   x  -   x  SOC_MV2_PORT_RATIO_TRI_023_2_1_1
 * #3   dual:    50  -   x  -  50  -   x  SOC_MV2_PORT_RATIO_DUAL_1_1
 * #4   dual:    40  -   x  -  40  -   x  SOC_MV2_PORT_RATIO_DUAL_1_1
 * #5   dual:    20  -   x  -  20  -   x  SOC_MV2_PORT_RATIO_DUAL_1_1
 * #6   dual:    40  -   x  -  20  -   x  SOC_MV2_PORT_RATIO_DUAL_2_1
 * #7   dual:    20  -   x  -  40  -   x  SOC_MV2_PORT_RATIO_DUAL_1_2
 * #8   tri:     50  -   x  - 25/x - 25/x SOC_MV2_PORT_RATIO_TRI_023_2_1_1
 * #9   tri:     20  -   x  - 10/x - 10/x SOC_MV2_PORT_RATIO_TRI_023_2_1_1
 * #10  tri:     40  -   x  - 10/x - 10/x SOC_MV2_PORT_RATIO_TRI_023_4_1_1
 * #11  tri:    25/x - 25/x -  50  -   x  SOC_MV2_PORT_RATIO_TRI_012_1_1_2
 * #12  tri:    10/x - 10/x -  20  -   x  SOC_MV2_PORT_RATIO_TRI_012_1_1_2
 * #13  tri:    10/x - 10/x -  40  -   x  SOC_MV2_PORT_RATIO_TRI_012_1_1_4
 * #14  quad:   25/x - 25/x - 25/x - 25/x SOC_MV2_PORT_RATIO_QUAD
 * #15  quad:   10/x - 10/x - 10/x - 10/x SOC_MV2_PORT_RATIO_QUAD
 */
void
soc_mv2_tdm_get_port_ratio(int unit,
                            soc_port_schedule_state_t *sch_info,
                            int pm_num, int *pm_mode, int prev_or_new)
{
    int port, phy_port_base, lane, mode = 0;
    int num_lanes[MAVERICK2_TDM_PORTS_PER_PBLK];
    int speed_max[MAVERICK2_TDM_PORTS_PER_PBLK];
    soc_port_map_type_t *port_map;
    int l_speed = 0, l_num_lanes = 0;

    sal_memset(num_lanes, 0, MAVERICK2_TDM_PORTS_PER_PBLK*sizeof(int));
    sal_memset(speed_max, 0, MAVERICK2_TDM_PORTS_PER_PBLK*sizeof(int));
    ...
    phy_port_base = 1 + (pm_num * MAVERICK2_TDM_PORTS_PER_PBLK);
    for (lane = 0; lane < MAVERICK2_TDM_PORTS_PER_PBLK; lane += 2) {
        port = port_map->port_p2l_mapping[phy_port_base + lane];
        if ( (port == -1) || (port_map->log_port_speed[port]==0) ) {
            num_lanes[lane] = -1;
            speed_max[lane] = -1;
        } else {
            soc_mv2_tdm_get_log_port_speed(unit, sch_info, port,
    ...
    if (num_lanes[0] == 4) {
        mode = SOC_MV2_PORT_RATIO_SINGLE;
    } else if (num_lanes[0] == 2 && num_lanes[2] == 2) {
        if (speed_max[0] == speed_max[2]) {
            mode = SOC_MV2_PORT_RATIO_DUAL_1_1;
        } else if (speed_max[0] > speed_max[2]) {
            mode = SOC_MV2_PORT_RATIO_DUAL_2_1;
        } else {
            mode = SOC_MV2_PORT_RATIO_DUAL_1_2;
        }
...
/*! @fn void _soc_mv2_tdm_get_pipe_map(
        int unit,
        soc_port_schedule_state_t *sch_info,
        uint32 *pipe_map)
    @param unit Chip unit number.
 ...
 */
void
_soc_mv2_tdm_get_pipe_map(int unit,
                          soc_port_schedule_state_t *sch_info,
                          uint32 *pipe_map)
{
    ...
    if (sch_info->is_flexport == 0) { /* Init */
        for (port = 0; port < MAVERICK2_TDM_PHY_PORTS_PER_DEV; port++) {
            if (sch_info->out_port_map.log_port_speed[port] > 0 ) {
                pipe = port / MAVERICK2_TDM_PHY_PORTS_PER_PIPE;
                pipe_bmap |= 1 << pipe;
            }
        }
    } else { /* FlexPort */
...
/*! @fn void _soc_mv2_tdm_get_ovs_pipe_map(
        int unit,
        soc_port_schedule_state_t *sch_info,
        uint32 *ovs_pipe_map)
    @param unit Chip unit number.
 ...
 */
void
_soc_mv2_tdm_get_ovs_pipe_map(int unit,
                              soc_port_schedule_state_t *sch_info,
                              uint32 *ovs_pipe_map)
{
...
/*! @fn int _soc_mv2_tdm_get_speed_ovs_class(
        int unit,
        int speed)
    @param unit   Chip unit number.
    @param speed Speed
 ...
 */
int
_soc_mv2_tdm_get_speed_ovs_class(int unit, int speed)
{
    int speed_class = 0;
    uint16 dev_id = 0;
    uint8  rev_id = 0;
    ...
    if (speed >= 40000) {
        ...
        if (speed >= 25000) {
            ...
            /* take 1G speed as 10G in MV2 for ovsb port */
            speed_class = 1;
        }
    }

    soc_cm_get_id(unit, &dev_id, &rev_id);
    if (((dev_id & TDM_MV2_DEV_ID_MASK) ==
...
/*! @fn int _soc_mv2_tdm_check_is_hsp_port_e(
        int unit,
        int speed)
    @param unit   Chip unit number.
    @param speed Speed
 ...
 */
STATIC int
_soc_mv2_tdm_check_is_hsp_port_e(int unit, int speed)
{
    int speed_class, port_is_hsp = 0;

    speed_class = _soc_mv2_tdm_get_speed_ovs_class(unit, speed);
    switch (speed_class) {
        case 2: /* 20G */
        case 4: /* 40G */
        case 5: /* 50G */
...
/*! @fn int _soc_mv2_tdm_check_mgmt_enable(
        int unit,
        soc_port_schedule_state_t *sch_info)
    @param unit   Chip unit number.
    @param sch_info Pointer to a soc_port_schedule_state_t struct variable.
 ...
 */
STATIC int
_soc_mv2_tdm_check_mgmt_enable(int unit,
                               soc_port_schedule_state_t *sch_info)
{
    int port;
    int mgmt_port_cnt = 0;
...
/*! @fn int _soc_mv2_tdm_check_2nd_mgmt_enable(
        int unit,
        soc_port_schedule_state_t *sch_info)
    @param unit   Chip unit number.
    @param sch_info Pointer to a soc_port_schedule_state_t struct variable.
 ...
 */
STATIC int
_soc_mv2_tdm_check_2nd_mgmt_enable(int unit,
                                   soc_port_schedule_state_t *sch_info)
{
    int port;
    int mgmt_port_cnt = 0;
...
/*! @fn int _soc_mv2_tdm_get_same_port_spacing_e(
        int unit,
        int speed)
    @param unit   Chip unit number.
    @param speed Speed
 ...
 */
int
_soc_mv2_tdm_get_same_port_spacing_e(int unit, int speed)
{
    int port_same_spacing = 0;

    if (_soc_mv2_tdm_check_is_hsp_port_e(unit, speed) == 1) {
        port_same_spacing = 8;
    } else {
...
/*! @fn int _soc_mv2_tdm_calculation(
        int unit,
        soc_port_schedule_state_t *sch_info)
    @param unit Chip unit number.
    @param sch_info Pointer to a soc_port_schedule_state_t struct variable.
 ...
 */
int
_soc_mv2_tdm_calculation(int unit, soc_port_schedule_state_t *sch_info)
{
    ...
    tdm_soc_t _chip_pkg;
    tdm_mod_t *_tdm_pkg;
    uint32 port_speeds[MAVERICK2_TDM_PHY_PORTS_PER_DEV];
    uint32 port_states[MAVERICK2_TDM_PHY_PORTS_PER_DEV];
    ...
    soc_cm_get_id(unit, &dev_id, &rev_id);
    LOG_VERBOSE(BSL_LS_SOC_COMMON, (BSL_META_U(unit,
                                    "mv2_tdm_dev_id 0x%0x\n"),
                                    dev_id));

    sal_memset(port_speeds, 0, MAVERICK2_TDM_PHY_PORTS_PER_DEV*sizeof(uint32));
    sal_memset(port_states, 0, MAVERICK2_TDM_PHY_PORTS_PER_DEV*sizeof(uint32));

    /* Remap input speeds to ETH bitrates */
    soc_maverick2_port_schedule_speed_remap(unit, sch_info);

    /* Set port states and speeds */
    for (lgc_port = 0; lgc_port < MAVERICK2_TDM_PHY_PORTS_PER_DEV; lgc_port++) {
        ...
        if (phy_port < MAVERICK2_TDM_PHY_PORTS_PER_DEV) {
            if (sch_info->in_port_map.log_port_speed[lgc_port] > 0 ) {
                ...
                soc_mv2_tdm_get_log_port_speed(unit, sch_info, lgc_port,
                    prev_or_new, &l_speed, &l_num_lanes);
                for (lane = 1; lane < l_num_lanes; lane++) {
                    phy_port_tmp = phy_port + lane;
                    if (phy_port_tmp < MAVERICK2_TDM_PHY_PORTS_PER_DEV) {
    ...
    if (_soc_mv2_tdm_check_mgmt_enable(unit, sch_info) == 1) {
    ...
    if (LOG_CHECK(BSL_LS_SOC_TDM | BSL_INFO)) {
        ...
        LOG_VERBOSE(BSL_LS_SOC_COMMON, (BSL_META_U(unit,
                            "%8s%8s%8s\n"),
                            "port", "speed", "state"));
        for (phy_port = 0; phy_port < MAVERICK2_TDM_PHY_PORTS_PER_DEV;
             phy_port++) {
            LOG_VERBOSE(BSL_LS_SOC_COMMON, (BSL_META_U(unit,
                                "%8d%8d%8d\n"),
    ...
    */
    ...
    _chip_pkg.num_ext_ports = MV2_NUM_EXT_PORTS;
    ...
    for (index = 0; index < _chip_pkg.num_ext_ports &&
                    index < MAVERICK2_TDM_PHY_PORTS_PER_DEV; index++) {
    ...
    _chip_pkg.soc_vars.mv2.mgmt_pm_hg= mgmt_pm_hg;
    _chip_pkg.soc_vars.mv2.mgmt_mode = mgmt_mode;
    ...
    if (_tdm_pkg == NULL) {
        ...
        for (pipe = 0; pipe < MAVERICK2_TDM_PIPES_PER_DEV; pipe++) {
            ...
            if (tdm_pipe_ovs_group == NULL ||
                ...
                for (i = 0; i < tdm_pipe_grp_num; i++) {
                    ...
                    hpipe = i / MAVERICK2_TDM_OVS_GROUPS_PER_HPIPE;
                    group = i % MAVERICK2_TDM_OVS_GROUPS_PER_HPIPE;
                ...
                for (hpipe = 0; hpipe < 2; hpipe++) {
                    for (j = 0; j < MV2_SHAPING_GRP_LEN; j++) {
                        ...
                        tdm_pipe_ovs_group[hpipe+MV2_SHAPING_GRP_IDX_0][j];
                    }
                }
            }
        }
        ...
        for (pm_num = 0; pm_num < MV2_NUM_PHY_PM; pm_num++) {
            for (lane = 0; lane < MAVERICK2_TDM_PORTS_PER_PBLK; lane++) {
                sch_info->out_port_map.port_p2PBLK_inst_mapping[
                (pm_num * MAVERICK2_TDM_PORTS_PER_PBLK) + 1 + lane] =
    ...
    if (LOG_CHECK(BSL_LS_SOC_TDM | BSL_INFO)) {
        for (pipe = 0; pipe < MAVERICK2_TDM_PIPES_PER_DEV; pipe++) {
            ...
            LOG_VERBOSE(BSL_LS_SOC_COMMON, (BSL_META_U(unit, "\n")));
            /* Oversub groups */
            for (hpipe = 0; hpipe < MAVERICK2_TDM_HPIPES_PER_PIPE; hpipe++) {
                LOG_VERBOSE(BSL_LS_SOC_COMMON, (BSL_META_U(unit,
                            "Pipe %d Half-Pipe %d [IDB TDM Oversub Groups]\n"),
                            pipe, hpipe));
                for (group = 0; group < MAVERICK2_TDM_OVS_GROUPS_PER_HPIPE;
                     group++) {
                    LOG_VERBOSE(BSL_LS_SOC_COMMON, (BSL_META_U(unit,
                                "oversub group %d: "), group));
                    for (index = 0; index < MAVERICK2_TDM_OVS_GROUP_LENGTH;
                         index++) {
                        LOG_VERBOSE(BSL_LS_SOC_COMMON, (BSL_META_U(unit, " %3d"),
                                    sch_info->tdm_ingress_schedule_pipe[pipe].
    ...
    */
    ...
    _soc_mv2_tdm_print_schedule_state(unit, sch_info);
    return SOC_E_NONE;
}


/*! @fn int _soc_mv2_tdm_set_idb_calendar(
        int unit,
        soc_port_schedule_state_t *sch_info)
    @param unit Chip unit number.
    @param sch_info Pointer to a soc_port_schedule_state_t struct variable.
    @brief API to initialize the MV2 IDB main calendar.
 */
int
_soc_mv2_tdm_set_idb_calendar(int unit, soc_port_schedule_state_t *sch_info)
{
    uint32 pipe_map;
    uint32 cal_len;
    soc_mem_t mem;
    ...
    uint32 mem_indx;
    static const soc_mem_t idb_tdm_cal_mem[MAVERICK2_TDM_PIPES_PER_DEV][2] = {
        {IS_TDM_CALENDAR0_PIPE0m, IS_TDM_CALENDAR1_PIPE0m}/* ,
        {IS_TDM_CALENDAR0_PIPE1m, IS_TDM_CALENDAR1_PIPE1m} */
    };
    static const soc_reg_t idb_tdm_cfg_reg[MAVERICK2_TDM_PIPES_PER_DEV] = {
    ...
    _soc_mv2_tdm_get_pipe_map(unit, sch_info, &pipe_map);
    ...
    for (pipe = 0; pipe < NUM_PIPE(unit); pipe++) {
        ...
        for (slot = 0; slot < length; slot += 2) {
            ...
            if (phy_port == MV2_OVSB_TOKEN) {
                idb_port = MAVERICK2_TDM_IDB_OVSB_TOKEN;
            } else if (phy_port == MV2_IDL1_TOKEN) {
                idb_port = MAVERICK2_TDM_IDB_SBUS_TOKEN;
            } else if (phy_port == MV2_IDL2_TOKEN) {
                idb_port = MAVERICK2_TDM_IDB_UNUSED_TOKEN;
            } else if (phy_port == MV2_NULL_TOKEN) {
                idb_port = MAVERICK2_TDM_IDB_NULL_TOKEN;
            } else if (phy_port >= MV2_NUM_EXT_PORTS) {
                idb_port = MAVERICK2_TDM_IDB_UNUSED_TOKEN;
            } else if (phy_port == MV2_MGM1_TOKEN) {
                /* TDM code always allocates slots for management port.
                 * If mgmt ports are omitted from config, this mapping
                 * is not set up.
                 */
                idb_port = MAVERICK2_TDM_IDB_MGMT1_TOKEN;
            } else {
            ...
            if (idb_port > 63) { /* Not a general front panel port */
                ...
                id = (phy_port - 1) / MAVERICK2_TDM_PORTS_PER_PBLK;
            }
        ...
        if (_soc_mv2_tdm_check_2nd_mgmt_enable(unit, sch_info) == 1 &&
            pipe == (MAVERICK2_TDM_PIPES_PER_DEV - 1)) {
...
/*! @fn int _soc_mv2_tdm_set_idb_hsp(
        int unit,
        soc_port_schedule_state_t *sch_info)
    @param unit Chip unit number.
    @param sch_info Pointer to a soc_port_schedule_state_t struct variable.
    @brief API to initialize the MV2 IDB HSP.
 */
int
_soc_mv2_tdm_set_idb_hsp(int unit, soc_port_schedule_state_t *sch_info)
{
    soc_reg_t reg;
    uint32 pipe_map;
    int pipe;
    ...
    static const soc_reg_t idb_tdm_hsp[MAVERICK2_TDM_PIPES_PER_DEV][2] = {
        {IS_TDM_HSP_0_PIPE0r, IS_TDM_HSP_1_PIPE0r}/* ,
        {IS_TDM_HSP_0_PIPE1r, IS_TDM_HSP_1_PIPE1r} */
    };
...
/*! @fn int _soc_mv2_tdm_set_idb_ppe_credit(
        int unit,
    ...
    @brief API to initialize the MV2 IDB PPE Credit Config register.
           Regs/Mems configured:PPE_CREDIT_CONFIG_PIPE<pipe>
 */
int
_soc_mv2_tdm_set_idb_ppe_credit(int unit,
                                soc_port_schedule_state_t *sch_info,
                                int full_credit_threshold_0,
                                int opp_credit_threshold_0,
                                int full_credit_threshold_1,
                                int opp_credit_threshold_1 )
{
    ...
    int pipe;
    uint32 rval = 0;
    static const soc_reg_t idb_ppe_credit[MAVERICK2_TDM_PIPES_PER_DEV] = {
...
/*! @fn int _soc_mv2_tdm_set_idb_opportunistic(
        int unit,
        soc_port_schedule_state_t *sch_info,
        int cpu_op_en,
        int lb_opp_en,
    ...
    @brief API to initialize the MV2 IDB Opportunistic related registers
           Regs/Mems configured: IS_CPU_LB_OPP_CFG_PIPE<pipe>
                                 IS_OPP_SCHED_CFG_PIPE<pipe>
 */
int
_soc_mv2_tdm_set_idb_opportunistic(int unit,
                                   soc_port_schedule_state_t *sch_info,
                                   int cpu_op_en,
                                   int lb_opp_en,
                                   int opp1_port_en,
                                   ...
                                   int opp_ovr_sub_en)
{
    ...
    int pipe;
    uint32 rval = 0;
    static const soc_reg_t idb_cpu_lb_opp_cfg[MAVERICK2_TDM_PIPES_PER_DEV] = {
        IS_CPU_LB_OPP_CFG_PIPE0r/* , IS_CPU_LB_OPP_CFG_PIPE1r */
    };
    static const soc_reg_t idb_opp_sched_cfg[MAVERICK2_TDM_PIPES_PER_DEV] = {
...
/*! @fn int soc_mv2_tdm_set_idb_dpp_ctrl(
        int unit,
        soc_port_schedule_state_t *sch_info,
        int credits)
    @param unit Chip unit number.
    @param sch_info Pointer to a soc_port_schedule_state_t struct variable.
    @param credits
    @brief API to initialize the MV2 IDB_DPP_CTRL register.
 */
int
soc_mv2_tdm_set_idb_dpp_ctrl(int unit,
                             soc_port_schedule_state_t *sch_info,
                             int credits)
{
    soc_reg_t reg;
    ...
    int pipe;
    uint32 rval = 0;
    static const soc_reg_t idb_dpp_ctrl[MAVERICK2_TDM_PIPES_PER_DEV] = {
...
/*! @fn int _soc_mv2_tdm_set_idb_pkt_calendar(
        int unit,
        soc_port_schedule_state_t *sch_info)
    @param unit Chip unit number.
    @param sch_info Pointer to a soc_port_schedule_state_t struct variable.
    @brief API to initialize the MV2 IDB Packet Shaper calendar.
           Regs/Mems configured: IDB PKT_SCH_CALENDAR
 */
int
_soc_mv2_tdm_set_idb_pkt_calendar(int unit,
                                  soc_port_schedule_state_t *sch_info)
{
    uint32 pipe_map;
    soc_mem_t mem;
    ...
    uint32 memfld;
    uint32 entry[SOC_MAX_MEM_WORDS];
    static const soc_mem_t idb_pkt_shaper_mems[MAVERICK2_TDM_PIPES_PER_DEV][2] = {
    ...
    for (pipe = 0; pipe < MAVERICK2_TDM_PIPES_PER_DEV; pipe++) {
        ...
        for (hpipe = 0; hpipe < MAVERICK2_TDM_HPIPES_PER_PIPE; hpipe++) {
            ...
            for (slot = 0; slot < MV2_SHAPING_GRP_LEN; slot ++) {
                phy_port = sch_info->tdm_ingress_schedule_pipe[pipe].
                tdm_schedule_slice[hpipe].pkt_sch_or_ovs_space[0][slot];
                if ((phy_port >= MV2_NUM_EXT_PORTS) || (phy_port <= 0)) {
                    idb_port = MAVERICK2_TDM_IDB_UNUSED_TOKEN;
                } else {
...
/*! @fn int _soc_mv2_tdm_set_ovs_group_sel(
        int unit,
        soc_port_schedule_state_t *sch_info,
        int set_idb,
        int set_mmu)
    ...
    @brief API to initialize the MV2 MMU &/or IDB OVS tables.
            Regs/Mems configured: IDB HPIPE0/1_OVR_SUB_GRP_CFG,
                                  HPIPE0/1_OVR_SUB_GRP_CFG
                                  PBLK<n>_CALENDAR
 */
int
_soc_mv2_tdm_set_ovs_group_sel(int unit,
                               soc_port_schedule_state_t *sch_info,
                               int set_idb, int set_mmu)
{
    ...
    int pblks[MAVERICK2_TDM_PIPES_PER_DEV][2][MAVERICK2_TDM_PBLKS_PER_HPIPE];
    ...
    static const soc_reg_t idb_grp_cfg_regs[MAVERICK2_TDM_PIPES_PER_DEV][2] = {
    ...
    static const soc_reg_t idb_grp_ovs_regs[MAVERICK2_TDM_PIPES_PER_DEV][2][6] = {
        {
    ...
    static const soc_reg_t idb_pblk_cal_regs[MAVERICK2_TDM_PIPES_PER_DEV][2][10] = {
        {
    ...
    _soc_mv2_tdm_get_pipe_map(unit, sch_info, &pipe_map);
    _soc_mv2_tdm_get_ovs_pipe_map(unit, sch_info, &ovs_pipe_map);
    ...
    for (pipe = 0; pipe < MAVERICK2_TDM_PIPES_PER_DEV; pipe++) {
        ...
        for (hpipe = 0; hpipe < MAVERICK2_TDM_HPIPES_PER_PIPE; hpipe++) {
            for (group = 0; group < MAVERICK2_TDM_OVS_GROUPS_PER_HPIPE; group++) {
                ...
                if (MAVERICK2_TDM_OVS_GROUP_LENGTH == slot) {
                    port_speed_class = 0;
                    port_same_spacing = 0;
                    ...
                    port_speed_class =
                        _soc_mv2_tdm_get_speed_ovs_class(unit, speed_max);
                    port_same_spacing =
                        _soc_mv2_tdm_get_same_port_spacing_e(unit, speed_max);
    ...
    sal_memset(pblks, -1, sizeof(int) * MAVERICK2_TDM_PIPES_PER_DEV * 2 *
               MAVERICK2_TDM_PBLKS_PER_HPIPE);
    for (pipe = 0; pipe < MAVERICK2_TDM_PIPES_PER_DEV; pipe++) {
        if (!(ovs_pipe_map & (1 << pipe))) {
            continue;
        }
            ...
            for (group = 0; group < MAVERICK2_TDM_OVS_GROUPS_PER_HPIPE; group++) {
                for (slot = 0; slot < MAVERICK2_TDM_OVS_GROUP_LENGTH; slot++) {
                    phy_port = sch_info->tdm_ingress_schedule_pipe[pipe].
                                         ...
                                         oversub_schedule[group][slot];
                    if (phy_port < MV2_NUM_EXT_PORTS) {
                        pm_num = (phy_port - 1) / MAVERICK2_TDM_PORTS_PER_PBLK;
                        pblk_cal_idx = sch_info->out_port_map.
                                       port_p2PBLK_inst_mapping[phy_port];
                        if (pblk_cal_idx >= 0 &&
                            pblk_cal_idx < MAVERICK2_TDM_PBLKS_PER_HPIPE) {
    ...
    for (pipe = 0; pipe < MAVERICK2_TDM_PIPES_PER_DEV; pipe++) {
        ...
        inst = pipe | SOC_REG_ADDR_INSTANCE_MASK;
        for (hpipe = 0; hpipe < MAVERICK2_TDM_HPIPES_PER_PIPE; hpipe++) {
            for (pblk_cal_idx = 0; pblk_cal_idx < MAVERICK2_TDM_PBLKS_PER_HPIPE;
                    pblk_cal_idx++) {
                pm_num = pblks[pipe][hpipe][pblk_cal_idx];
                phy_port_base = 1 + (pm_num * MAVERICK2_TDM_PORTS_PER_PBLK);
                /* Get port ratio for this Port Macro */
                soc_mv2_tdm_get_port_ratio(unit, sch_info, pm_num, &mode, 1);
                 ...
                 */
                if ((pm_num < 0) || (pm_num > MAVERICK2_TDM_PBLKS_PER_DEV)) {
                    ...
                    for (slot = 0; slot < 6; slot++) {
                        ...
                        if (lane == -1) {
                            ...
                            prev_or_new = 1;
                            soc_mv2_tdm_get_log_port_speed(unit, sch_info,
                                port, prev_or_new, &l_speed, &l_num_lanes);
                            speed_max = 25000 * l_num_lanes;
                            if (speed_max > l_speed) speed_max = l_speed;

                            port_speed_class =
                                _soc_mv2_tdm_get_speed_ovs_class(unit,
                                                                 speed_max);
                            pblk_spacing =
                                _soc_mv2_tdm_get_same_port_spacing_e(unit,
                                                                     speed_max);
                        }

                        soc_reg_field_set(unit, reg, &rval, VALIDf, pblk_valid);
                        soc_reg_field_set(unit, reg, &rval, SPACINGf, 8);
                        soc_reg_field_set(unit, reg, &rval, PORT_NUMf,
                                                            idb_port);
...
/*! @fn int _soc_mv2_tdm_set_ovs_group(
        int unit,
        soc_port_schedule_state_t *sch_info)
    @param unit Chip unit number.
    @param sch_info Pointer to a soc_port_schedule_state_t struct variable.
    @brief API to initialize the MV2 MMU & IDB OVS tables.
            Regs/Mems configured: IDB HPIPE0/1_OVR_SUB_GRP_CFG
                                  HPIPE0/1_OVR_SUB_GRP_CFG
                                  PBLK<n>_CALENDAR
 */
int
_soc_mv2_tdm_set_ovs_group(int unit, soc_port_schedule_state_t *sch_info)
{
    return (_soc_mv2_tdm_set_ovs_group_sel(unit, sch_info, 1, 1));
}


/*! @fn int _soc_mv2_tdm_set_mmu_calendar(
        int unit,
        soc_port_schedule_state_t *sch_info)
    @param unit Chip unit number.
    @param sch_info Pointer to a soc_port_schedule_state_t struct variable.
    @brief API to initialize the MV2 MMU main calendar.
            Regs/Mems configured: MMU TDM_CALENDAR
                                  TDM_CONFIG
 */
int
_soc_mv2_tdm_set_mmu_calendar(int unit, soc_port_schedule_state_t *sch_info)
{
    uint32 pipe_map;
    uint32 cal_len;
    soc_mem_t mem;
    ...
    for (pipe = 0; pipe < MAVERICK2_TDM_PIPES_PER_DEV; pipe++) {
        ...
        mem = calendar_mems[(calendar_id * MAVERICK2_TDM_PIPES_PER_DEV) + pipe];
        cal_end_field = calendar_end_fields[calendar_id];
        ...
        for (length = cal_len; length > 0; length--) {
            if (sch_info->tdm_egress_schedule_pipe[pipe].
                          ...
                          linerate_schedule[length - 1] != MV2_NUM_EXT_PORTS) {
                break;
            }
        }
        for (slot = 0; slot < length; slot += 2) {
            ...
            if (phy_port == MV2_OVSB_TOKEN) {
                mmu_port = MAVERICK2_TDM_MMU_OVSB_TOKEN;
            } else if (phy_port == MV2_IDL1_TOKEN) {
                mmu_port = MAVERICK2_TDM_MMU_PURG_TOKEN;
            } else if (phy_port == MV2_IDL2_TOKEN) {
                mmu_port = MAVERICK2_TDM_MMU_UNUSED_TOKEN;
            } else if (phy_port == MV2_NULL_TOKEN) {
                mmu_port = MAVERICK2_TDM_MMU_NULL_TOKEN;
            } else if (phy_port >= MV2_NUM_EXT_PORTS) {
                mmu_port = MAVERICK2_TDM_MMU_UNUSED_TOKEN;
            } else if (phy_port == MV2_MGM1_TOKEN) {
                /* TDM code always allocates slots for management port.
                 ...
                 * is not set up.
                 */
                mmu_port = MAVERICK2_TDM_MMU_MGMT1_TOKEN;
            } else {
                /* [0,69] && 127 */
...
/*! @fn int _soc_mv2_tdm_set_mmu_hsp(
        int unit,
        soc_port_schedule_state_t *sch_info)
    @param unit Chip unit number.
    @param sch_info Pointer to a soc_port_schedule_state_t struct variable.
    @brief API to initialize the MV2 MMU HSP.
            Regs/Mems configured: TDM_HSP_PIPE
 */
int
_soc_mv2_tdm_set_mmu_hsp(int unit,
                         soc_port_schedule_state_t *sch_info)
{
    soc_reg_t reg;
    uint32 pipe_map;
    ...
    uint32 port_map[MAVERICK2_TDM_PIPES_PER_DEV][2];
    uint32 rval = 0;
    int l_speed = 0, l_num_lanes = 0, prev_or_new = 0;
    ...
    for (port = 0; port < MAVERICK2_TDM_PHY_PORTS_PER_DEV; port++) {
        if (sch_info->out_port_map.log_port_speed[port] > 0) {
            soc_mv2_tdm_get_log_port_speed(unit, sch_info, port,
                prev_or_new, &l_speed, &l_num_lanes);
            if (_soc_mv2_tdm_check_is_hsp_port_e(unit, l_speed) == 1) {
                phy_port = sch_info->out_port_map.port_l2p_mapping[port];
                if ((phy_port >= 1) &&
                    (phy_port <= MAVERICK2_TDM_GPORTS_PER_DEV)) {
                    pipe = (phy_port - 1) / MAVERICK2_TDM_GPORTS_PER_PIPE;
                    hpipe = ((phy_port - 1) & 0x3f) /
                            MAVERICK2_TDM_GPORTS_PER_HPIPE;
                    mmu_port =
                        sch_info->out_port_map.port_p2m_mapping[phy_port] &
                        0x1f;
                    if (pipe < MAVERICK2_TDM_PIPES_PER_DEV && hpipe < 2) {
...
/*! @fn int _soc_mv2_tdm_set_mmu_opportunistic(
        int unit,
        soc_port_schedule_state_t *sch_info,
        int cpu_op_en,
        int lb_opp_en,
    ...
    @param opp2_port_en
    @param opp_ovr_sub_en
    @brief API to initialize the MV2 MMU Opportunistic related registers
            Regs/Mems configured: CPU_LB_OPP_CFG
                                  OPP_SCHED_CFG
 */
int
_soc_mv2_tdm_set_mmu_opportunistic(int unit,
                                   soc_port_schedule_state_t *sch_info,
                                   int cpu_op_en,
                                   int lb_opp_en,
                                   int opp1_port_en,
 ...
 */
int soc_mv2_tdm_init(int unit, soc_port_schedule_state_t *sch_info)
{
    SOC_IF_ERROR_RETURN(_soc_mv2_tdm_calculation(unit, sch_info));

    SOC_IF_ERROR_RETURN(_soc_mv2_tdm_set_idb_calendar(unit, sch_info));
    SOC_IF_ERROR_RETURN(_soc_mv2_tdm_set_idb_hsp(unit, sch_info));
    SOC_IF_ERROR_RETURN(_soc_mv2_tdm_set_idb_pkt_calendar(unit, sch_info));
    SOC_IF_ERROR_RETURN(_soc_mv2_tdm_set_idb_ppe_credit(unit, sch_info,
                                                        20, 9, 20, 9));
    /* SOC_IF_ERROR_RETURN(soc_mv2_tdm_set_idb_dpp_ctrl(unit, sch_info, 32)); */
    /* IDB opp1_port_en should always be 1 to enable sbus access to the rest of IPIPE*/
    SOC_IF_ERROR_RETURN(_soc_mv2_tdm_set_idb_opportunistic(unit, sch_info,
                                                           1, 1, 1, 1, 1));

    SOC_IF_ERROR_RETURN(_soc_mv2_tdm_set_ovs_group(unit, sch_info));

    SOC_IF_ERROR_RETURN(_soc_mv2_tdm_set_mmu_calendar(unit, sch_info));
    SOC_IF_ERROR_RETURN(_soc_mv2_tdm_set_mmu_hsp(unit, sch_info));
    SOC_IF_ERROR_RETURN(_soc_mv2_tdm_set_mmu_opportunistic(unit, sch_info,
                                                           1, 1, 1, 1, 1));

    return SOC_E_NONE;
}

/*! @fn int soc_maverick2_port_speed_higig2eth(int speed)
 *  @param int speed
 *  @brief Map port speed to ETH bitrates
 */
static int
soc_maverick2_port_speed_higig2eth(int speed)
{
    switch (speed) {
    case 11000:
        /* 10G */
...
/*! @fn void soc_maverick2_port_schedule_speed_remap(int unit,
 ...
 */
void
soc_maverick2_port_schedule_speed_remap(
    int  unit,
    soc_port_schedule_state_t *port_schedule_state)
{
    int i, port;
    ...
    for (port=0; port < SOC_MAX_NUM_PORTS; port++) {
        port_schedule_state->in_port_map.log_port_speed[port] =
            soc_maverick2_port_speed_higig2eth(
                port_schedule_state->in_port_map.log_port_speed[port]);
    }
...
/*! @fn int soc_mv2_tdm_get_log_port_speed(int unit,
 *   soc_port_schedule_state_t *port_schedule_state,
 *  int log_port,
 *  int prev_or_new,
 *  int *speed,
 ...
 */
int
soc_mv2_tdm_get_log_port_speed(
    int unit,
    soc_port_schedule_state_t *port_schedule_state,
    int log_port,
    int prev_or_new,
