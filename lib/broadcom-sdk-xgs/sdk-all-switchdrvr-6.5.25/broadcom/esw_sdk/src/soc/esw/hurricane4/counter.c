...
int
soc_counter_hurricane4_generic_get_info(int unit, soc_ctr_control_info_t
        ...
        case SOC_CTR_INSTANCE_TYPE_PORT:
            ...
            switch (id) {
                ...
                case SOC_COUNTER_NON_DMA_EGR_PERQ_XMT_BYTE_UC:
                    return soc_counter_hurricane4_get_info(unit, port, id,
                    ...
                    *num_entries = non_dma->entries_per_port;
                    ...
                    if (IS_CPU_PORT(unit, port)) {
                        ...
                        *base_index += pipe_offset +
                                        (mmu_port & SOC_HR4_MMU_PORT_STRIDE);
                    }
                    *base_index *= _HR4_MMU_NUM_PG;
                    break;
...
int
soc_counter_hurricane4_get_info(int unit, soc_port_t port, soc_reg_t id,
                            int *base_index, int *num_entries)
{
    ...
    case SOC_COUNTER_NON_DMA_EGR_PERQ_XMT_BYTE:
         ...
         */
        *base_index  = si->port_cosq_base[port] + SOC_HR4_NUM_MAX_UC_QUEUES_PER_PIPE;
        *num_entries = si->port_num_cosq[port];
        break;
    case SOC_COUNTER_NON_DMA_EGR_PERQ_XMT_PKT_UC:
    case SOC_COUNTER_NON_DMA_EGR_PERQ_XMT_BYTE_UC:
 ...
 */
int
soc_counter_hurricane4_non_dma_init(int unit, int nports,
                                ...
                                int *non_dma_entries)
{
    ...
    *non_dma_entries += non_dma0->num_entries;
    ...
    if (soc_feature(unit, soc_feature_flex_flowtracker_ver_1)) {
        ...
        non_dma0->entries_per_port = 0;
        non_dma0->num_entries =
            NUM_PIPE(unit) * soc_mem_index_count(unit, BSC_KG_FLOW_EXCEED_COUNTERm);
        non_dma0->mem = BSC_KG_FLOW_EXCEED_COUNTERm;
        non_dma0->reg = INVALIDr;
        non_dma0->field = FLOW_COUNTf;
        non_dma0->cname = "GROUP_FLOW_EXCEED_COUNT";

        num_entries = soc_mem_index_count(unit, BSC_KG_FLOW_EXCEED_COUNTERm);
        entry_words = soc_mem_entry_words(unit, BSC_KG_FLOW_EXCEED_COUNTERm);
        ...
        *non_dma_entries += non_dma0->num_entries;
        ...
        non_dma0->entries_per_port = 0;
        non_dma0->num_entries =
            NUM_PIPE(unit) * soc_mem_index_count(unit, BSC_KG_FLOW_LEARNED_COUNTERm);
        non_dma0->mem = BSC_KG_FLOW_LEARNED_COUNTERm;
        non_dma0->reg = INVALIDr;
        non_dma0->field = FLOW_COUNTf;
        non_dma0->cname = "GROUP_FLOW_LEARNED_COUNT";

        num_entries = soc_mem_index_count(unit, BSC_KG_FLOW_LEARNED_COUNTERm);
        entry_words = soc_mem_entry_words(unit, BSC_KG_FLOW_LEARNED_COUNTERm);
        ...
        *non_dma_entries += non_dma0->num_entries;
        ...
        non_dma0->entries_per_port = 0;
        non_dma0->num_entries =
            NUM_PIPE(unit) * soc_mem_index_count(unit, BSC_KG_FLOW_AGE_OUT_COUNTERm);
        non_dma0->mem = BSC_KG_FLOW_AGE_OUT_COUNTERm;
        non_dma0->reg = INVALIDr;
        non_dma0->field = FLOW_COUNTf;
        non_dma0->cname = "GROUP_AGE_OUT_COUNT";

        num_entries = soc_mem_index_count(unit, BSC_KG_FLOW_AGE_OUT_COUNTERm);
        entry_words = soc_mem_entry_words(unit, BSC_KG_FLOW_AGE_OUT_COUNTERm);
        ...
        *non_dma_entries += non_dma0->num_entries;
        ...
        non_dma0->entries_per_port = 0;
        non_dma0->num_entries =
            NUM_PIPE(unit) * soc_mem_index_count(unit, BSC_KG_FLOW_MISSED_COUNTERm);
        non_dma0->mem = BSC_KG_FLOW_MISSED_COUNTERm;
        non_dma0->reg = INVALIDr;
        non_dma0->field = FLOW_COUNTf;
        non_dma0->cname = "GROUP_FLOW_MISSED_COUNT";

        num_entries = soc_mem_index_count(unit, BSC_KG_FLOW_MISSED_COUNTERm);
        entry_words = soc_mem_entry_words(unit, BSC_KG_FLOW_MISSED_COUNTERm);
        ...
        *non_dma_entries += non_dma0->num_entries;
        ...
        non_dma0->entries_per_port = 0;
        non_dma0->num_entries =
            NUM_PIPE(unit) * soc_mem_index_count(unit, BSC_DG_FLOW_METER_EXCEED_COUNTERm);
        non_dma0->mem = BSC_DG_FLOW_METER_EXCEED_COUNTERm;
        non_dma0->reg = INVALIDr;
        non_dma0->field = FLOW_COUNTf;
        non_dma0->cname = "GROUP_METER_EXCEED_COUNT";

        num_entries = soc_mem_index_count(unit, BSC_DG_FLOW_METER_EXCEED_COUNTERm);
        entry_words = soc_mem_entry_words(unit, BSC_DG_FLOW_METER_EXCEED_COUNTERm);
