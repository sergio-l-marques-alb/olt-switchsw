...
static egr_vlan_xlate_1_hash_control_entry_t       egr_vlan_1_hash_control;
static egr_vlan_xlate_2_hash_control_entry_t       egr_vlan_2_hash_control;
static egr_vp_vlan_membership_hash_control_entry_t egr_vp_vlan_mbm_hash_control;
static exact_match_hash_control_entry_t            exact_match_hash_control;
static ing_vp_vlan_membership_hash_control_entry_t ing_vp_vlan_mbm_hash_control;
static l2_entry_hash_control_entry_t               l2_entry_hash_control;
static l3_entry_hash_control_entry_t               l3_entry_hash_control;
static mpls_entry_hash_control_entry_t             mpls_entry_hash_control;
static vlan_xlate_1_hash_control_entry_t           vlan_1_hash_control;
static vlan_xlate_2_hash_control_entry_t           vlan_2_hash_control;
static uft_shared_banks_control_entry_t            uft_shared_banks_control;
static ip_uat_shared_banks_control_entry_t         ip_uat_shared_banks_control;
static ep_uat_shared_banks_control_entry_t         ep_uat_shared_banks_control;
static ft_key_ss0_shared_banks_control_entry_t     ft_key_ss0_shared_banks_control;
static ft_key_ss1_shared_banks_control_entry_t     ft_key_ss1_shared_banks_control;
...
{
    soc_utt_agent_t aid;
    uint8 max_tcams = FB6_UTT_MAX_TCAMS;

    uint8 ifp_min_tcams = FB6_UTT_IFP_MIN_TCAMS;
    uint8 ifp_max_tcams = FB6_UTT_IFP_MAX_TCAMS;

    uint8 lpm_min_tcams = FB6_UTT_LPM_MIN_TCAMS;
    uint8 lpm_max_tcams = FB6_UTT_LPM_MAX_TCAMS;

    if (soc_feature(u, soc_feature_td3x4_ax_reduced_utt)) {
        max_tcams = 72;
    ...
    SOC_UTT_MAX_TCAMS(u)  = max_tcams;
    SOC_UTT_MAX_AGENTS(u) = 2;

    /* IFP */
    aid = SOC_UTT_AGENT_IFP;
    SOC_UTT_AGENT_MIN_TCAMS(u, aid)            = ifp_min_tcams;
    SOC_UTT_AGENT_MAX_TCAMS(u, aid)            = ifp_max_tcams;
    ...
    SOC_UTT_AGENT_DEF_TABLE_SIZE(u, aid)       = ifp_max_tcams * FB6_UTT_TCAM_DEPTH;
    ...
    SOC_UTT_AGENT_MIN_TCAMS(u, aid)            = lpm_min_tcams;
    SOC_UTT_AGENT_MAX_TCAMS(u, aid)            = lpm_max_tcams;
    ...
    SOC_UTT_AGENT_DEF_TABLE_SIZE(u, aid)       = lpm_min_tcams * FB6_UTT_TCAM_DEPTH;
...
soc_firebolt6_mem_config(int unit)
{
     ...
     * bank 10-11 are dedicated L3 banks (16k entries per bank)
     * bank 2-9 are shared banks (32k L2/L3 or 16k FPEM entries
     *                              or 48k ALPM entries per bank)
     */
    if (soc_feature(unit, soc_feature_td3x4_ax_uft_shared_8_banks_16k)) {
        /* BCM56474  UFT BANK SIZING
         * Dedicated banks size unchanged, Shared banks size reduced to half.
         * bank 0-1 are dedicated L2 banks (16k entries per bank)
         * bank 10-11 are dedicated L3 banks (16k entries per bank)
         * bank 2-9 are shared banks (16k L2/L3 or 8k FPEM entries
    ...
    if (l2_entries < fixed_l2_entries) {
        ...
        def_val = (32 * 1024)/div_factor;
        l2_entries -= fixed_l2_entries;
        shared_l2_banks = (l2_entries + (def_val - 1)) / (def_val);
        l2_entries = fixed_l2_entries + shared_l2_banks * def_val;
    }
    ...
    if (l3_entries < fixed_l3_entries) {
        ...
        def_val = (32 * 1024)/div_factor;
        l3_entries -= fixed_l3_entries;
        shared_l3_banks = (l3_entries + (def_val - 1)) / (def_val);
        l3_entries = fixed_l3_entries + shared_l3_banks * def_val;
    }

    def_val = (16 * 1024)/div_factor;
    fpem_entries = soc_property_get(unit, spn_FPEM_MEM_ENTRIES, 0);
    shared_fpem_banks = (fpem_entries + (def_val - 1)) / (def_val);
    ...
    fpem_entries = shared_fpem_banks * def_val;
    ...
    sop->memState[L2_ENTRY_LPm].index_max = (((4 * 2 * 1024) +
                                             (shared_l2_banks * 8 * 1024))/div_factor) - 1;
    ...
    sop->memState[L3_ENTRY_LPm].index_max = (((7 * 2* 1024) +
                                            (shared_l3_banks * 14 * 1024))/div_factor) - 1;
    ...
    sop->memState[EXACT_MATCH_LPm].index_max = (((shared_fpem_banks * 6 * 1024)/div_factor) - 1);
    sop->memState[EXACT_MATCH_LP_PIPE0m].index_max = (((shared_fpem_banks * 6 * 1024)/div_factor) - 1);
    ...
    if (alpm_enable) {
        sop->memState[L3_DEFIP_ALPM_RAWm].index_max =
             sop->memState[L3_DEFIP_ALPM_RAWm].index_max / div_factor;
        sop->memState[L3_DEFIP_ALPM_IPV4m].index_max =
             sop->memState[L3_DEFIP_ALPM_IPV4m].index_max / div_factor;
        sop->memState[L3_DEFIP_ALPM_IPV4_1m].index_max =
             sop->memState[L3_DEFIP_ALPM_IPV4_1m].index_max / div_factor;
        sop->memState[L3_DEFIP_ALPM_IPV6_64m].index_max =
             sop->memState[L3_DEFIP_ALPM_IPV6_64m].index_max / div_factor;
        sop->memState[L3_DEFIP_ALPM_IPV6_64_1m].index_max =
             sop->memState[L3_DEFIP_ALPM_IPV6_64_1m].index_max / div_factor;
        sop->memState[L3_DEFIP_ALPM_IPV6_128m].index_max =
             sop->memState[L3_DEFIP_ALPM_IPV6_128m].index_max / div_factor;
        sop->memState[L3_DEFIP_ALPM_HIT_ONLYm].index_max =
             sop->memState[L3_DEFIP_ALPM_HIT_ONLYm].index_max / div_factor;
        sop->memState[L3_DEFIP_ALPM_ECCm].index_max =
             sop->memState[L3_DEFIP_ALPM_ECCm].index_max / div_factor;
    ...
    */
    ...
    /* No OTP bit for NHop in FB6 X4-AX hence reduced NHop by soc feature */
    if (soc_feature(unit, soc_feature_td3x4_ax_reduced_nhop)) {
    ...
    /* No OTP bit for IFP meter table in FB6 X4-AX hence reduced it by soc feature */
    if (soc_feature(unit, soc_feature_td3x4_ax_reduced_ifp_meter_table)) {
        sop->memState[IFP_METER_TABLEm].index_max = 6143;
        sop->memState[IFP_METER_TABLE_PIPE0m].index_max = 6143;
    }

    /* NO OTP bit to adjust ingress Flex Counters in FB6 X4-AX hence reduced through soc feature */
    if (soc_feature(unit, soc_feature_td3x4_ax_reduced_ing_flex_ctr)) {
      ...
      sop->memState[ING_FLEX_CTR_COUNTER_TABLE_8m].index_max = 1023;
      sop->memState[ING_FLEX_CTR_COUNTER_TABLE_9m].index_max = 1023;
      sop->memState[ING_FLEX_CTR_COUNTER_TABLE_10m].index_max = 1023;
      sop->memState[ING_FLEX_CTR_COUNTER_TABLE_11m].index_max = 1023;
      sop->memState[ING_FLEX_CTR_COUNTER_TABLE_12m].index_max = 1023;
      sop->memState[ING_FLEX_CTR_COUNTER_TABLE_13m].index_max = 1023;
      sop->memState[ING_FLEX_CTR_COUNTER_TABLE_14m].index_max = 1023;
      sop->memState[ING_FLEX_CTR_COUNTER_TABLE_15m].index_max = 1023;
      sop->memState[ING_FLEX_CTR_COUNTER_TABLE_16m].index_max = 1023;
      sop->memState[ING_FLEX_CTR_COUNTER_TABLE_17m].index_max = 1023;
      sop->memState[ING_FLEX_CTR_COUNTER_TABLE_18m].index_max = 1023;
      sop->memState[ING_FLEX_CTR_COUNTER_TABLE_19m].index_max = 1023;
      sop->memState[ING_FLEX_CTR_COUNTER_TABLE_0_PIPE0m].index_max = 2047;
      sop->memState[ING_FLEX_CTR_COUNTER_TABLE_1_PIPE0m].index_max = 2047;
      sop->memState[ING_FLEX_CTR_COUNTER_TABLE_2_PIPE0m].index_max = 2047;
      sop->memState[ING_FLEX_CTR_COUNTER_TABLE_3_PIPE0m].index_max = 2047;
      sop->memState[ING_FLEX_CTR_COUNTER_TABLE_4_PIPE0m].index_max = 2047;
      sop->memState[ING_FLEX_CTR_COUNTER_TABLE_5_PIPE0m].index_max = 2047;
      sop->memState[ING_FLEX_CTR_COUNTER_TABLE_6_PIPE0m].index_max = 2047;
      sop->memState[ING_FLEX_CTR_COUNTER_TABLE_7_PIPE0m].index_max = 2047;
      sop->memState[ING_FLEX_CTR_COUNTER_TABLE_8_PIPE0m].index_max = 1023;
      sop->memState[ING_FLEX_CTR_COUNTER_TABLE_9_PIPE0m].index_max = 1023;
      sop->memState[ING_FLEX_CTR_COUNTER_TABLE_10_PIPE0m].index_max = 1023;
      sop->memState[ING_FLEX_CTR_COUNTER_TABLE_11_PIPE0m].index_max = 1023;
      sop->memState[ING_FLEX_CTR_COUNTER_TABLE_12_PIPE0m].index_max = 1023;
      sop->memState[ING_FLEX_CTR_COUNTER_TABLE_13_PIPE0m].index_max = 1023;
      sop->memState[ING_FLEX_CTR_COUNTER_TABLE_14_PIPE0m].index_max = 1023;
      sop->memState[ING_FLEX_CTR_COUNTER_TABLE_15_PIPE0m].index_max = 1023;
      sop->memState[ING_FLEX_CTR_COUNTER_TABLE_16_PIPE0m].index_max = 1023;
      sop->memState[ING_FLEX_CTR_COUNTER_TABLE_17_PIPE0m].index_max = 1023;
      sop->memState[ING_FLEX_CTR_COUNTER_TABLE_18_PIPE0m].index_max = 1023;
      sop->memState[ING_FLEX_CTR_COUNTER_TABLE_19_PIPE0m].index_max = 1023;
    }
...
int
soc_firebolt6_egr_index_min_pkt_size_set(int unit, int index, uint8 min)
{
    egr_port_min_pkt_size_entry_t egr_port_mem_pkt_size_ent;

    /* Register file mentions 50 as the lower bounds of the range. Field
     * is 7 bits, so error on overflow */
    if (min < 50 || min > 127) {
        return SOC_E_PARAM;
    }

    SOC_IF_ERROR_RETURN
        (READ_EGR_PORT_MIN_PKT_SIZEm(unit, MEM_BLOCK_ALL, index,
                                    &egr_port_mem_pkt_size_ent));
    soc_mem_field32_set(unit, EGR_PORT_MIN_PKT_SIZEm,
                       &egr_port_mem_pkt_size_ent, MIN_PKT_SIZEf, min);
    SOC_IF_ERROR_RETURN
        (WRITE_EGR_PORT_MIN_PKT_SIZEm(unit, MEM_BLOCK_ALL, index,
                                     &egr_port_mem_pkt_size_ent));
    return SOC_E_NONE;
}

int
_soc_firebolt6_egr_indexes_min_pkt_size_set(int unit)
{
    int index;
    ...
    for (index = 0; index < soc_mem_index_count(unit,
        ...
        /* Index is physical port (for Non-channelized applications)
         * EGR_PORT_MIN_PKT_SIZE needs to be 76 for hg, cpu, and lb ports */
        if (IS_HG_PORT(unit, index) || IS_CPU_PORT(unit, index)
                                    || IS_LB_PORT(unit, index)) {
            SOC_IF_ERROR_RETURN
                (soc_firebolt6_egr_index_min_pkt_size_set(unit, index, 76));
        } else {
            SOC_IF_ERROR_RETURN
                (soc_firebolt6_egr_index_min_pkt_size_set(unit, index, 64));
        }
    }
...
soc_fb6_flexport_scache_recovery(int unit)
{
    ...
    soc_info_t *si = &SOC_INFO(unit);
    int pbmp_port_max, pbmp_scache_sz;
    ...
    pbmp_port_max = soc_scache_dictionary_entry_get(unit,
                                                    ...
                                                    SOC_PBMP_PORT_MAX);
    pbmp_scache_sz = SHR_BITALLOCSIZE(pbmp_port_max);
    /*Physical to logical port mapping */
    var_size = (sizeof(int) * FIREBOLT6_PHY_PORTS_PER_DEV);
    ...
    sal_memcpy(&si->subtag_allowed_pbm,
               &flexport_scache_ptr[scache_offset],
               SOC_SCACHE_SIZE_MIN(pbmp_scache_sz, sizeof(pbmp_t)));
    scache_offset += pbmp_scache_sz;
...
_soc_firebolt6_misc_init(int unit)
{
    ...
    SOC_IF_ERROR_RETURN(_soc_firebolt6_egr_indexes_min_pkt_size_set(unit));
    ...
    soc_mem_field32_set(unit, FC_CONFIG_1m, &entry, PFC_OPCODEf, 0x0101);
    soc_mem_field32_set(unit, FC_CONFIG_1m, &entry, E2ECC_OPCODEf, 0x0);
    ...
    soc_mem_field32_set(unit, FC_CONFIG_2m, &entry, PFC_OPCODEf, 0x0101);
    soc_mem_field32_set(unit, FC_CONFIG_2m, &entry, E2ECC_OPCODEf, 0x0);
...
int
soc_firebolt6_mmu_pfc_tx_config_set(int unit, bcm_port_t port, int pfc_enable)
{
    int pipe, obm_id, lane;
    soc_reg_t reg;
    uint64 rval64;
    soc_info_t *si;
    soc_reg_t obm_fc_config_regs[] = {
    ...
    reg = SOC_REG_UNIQUE_ACC(unit, obm_fc_config_regs[obm_id])[pipe];
