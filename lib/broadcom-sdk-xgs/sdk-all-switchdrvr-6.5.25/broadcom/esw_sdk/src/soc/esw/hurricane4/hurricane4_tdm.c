/*
 * $Id: hurricane4_tdm.c, Exp $
 * $Copyright: (c) 2021 Broadcom.
 * Broadcom Proprietary and Confidential. All rights reserved.$
 *
 * File:        hurricane4_tdm.c
 * Purpose:
 * Requires:
 */
...
/*! @fn void _soc_hr4_tdm_print_port_map(
        int unit,
 ...
 */
void
_soc_hr4_tdm_print_port_map(int unit, soc_port_map_type_t *port_map)
{
    int port;
    ...
    for (port = 0; port < HR4_NUM_EXT_PORTS; port++) {
...
/*! @fn void _soc_hr4_tdm_print_tdm_info(
        int unit,
 ...
 */
void
_soc_hr4_tdm_print_tdm_info(int unit, soc_tdm_schedule_t *tdm_info,
                            int hpipe, int is_ing)
{
    ...
    if (hpipe == 0) {
        for (cal_len = tdm_info->cal_len; cal_len > 0; cal_len--) {
            if (tdm_info->linerate_schedule[cal_len - 1] != HR4_NUM_EXT_PORTS) {
                break;
            }
        }
    ...
    for (j=0; j< tdm_info->num_ovs_groups; j++) {
        ...
        for (m=0; m< tdm_info->ovs_group_len; m++) {
            if (tdm_info->oversub_schedule[j][m] != HR4_NUM_EXT_PORTS) {
    ...
    if (is_ing == 1) {
        ...
        for (j=0; j<tdm_info->pkt_sch_or_ovs_space_len; j++) {
            ...
            if (tdm_info->pkt_sch_or_ovs_space[0][j] != HR4_NUM_EXT_PORTS) {
                LOG_VERBOSE(BSL_LS_SOC_COMMON, (BSL_META_U(unit, "\t%d"),
...
/*! @fn void _soc_hr4_tdm_print_port_resource(
        int unit,
 ...
 */
void
_soc_hr4_tdm_print_port_resource(int unit,
                                 soc_port_resource_t *port_info,
                                 int entry_num)
{
    int is_hg2, lane;
...
/*! @fn void _soc_hr4_tdm_print_schedule_state(
        int unit,
 ...
 */
void
_soc_hr4_tdm_print_schedule_state(int unit,
                                  soc_port_schedule_state_t *sch_info)
{
    ...
    LOG_VERBOSE(BSL_LS_SOC_COMMON, (BSL_META_U(unit,
                        "\nPRINTING soc_port_schedule_state_t"
                        "::[in_port_map]\n")));
    _soc_hr4_tdm_print_port_map(unit, &sch_info->in_port_map);

    if (sch_info->is_flexport == 1) {
        LOG_VERBOSE(BSL_LS_SOC_COMMON, (BSL_META_U(unit,
                            "\nPRINTING soc_port_schedule_state_t"
                            "::[out_port_map]\n")));
        _soc_hr4_tdm_print_port_map(unit, &sch_info->out_port_map);
    }
    ...
    if (print_tdm == 1) {
        for (pipe = 0; pipe< NUM_PIPE(unit); pipe++) {
            ...
            for (hpipe = 0; hpipe< MAX_SCH_SLICES; hpipe++) {
                ...
                _soc_hr4_tdm_print_tdm_info(unit, tdm_schedule, hpipe, 1);
            }
            LOG_VERBOSE(BSL_LS_SOC_COMMON, (BSL_META_U(unit,
            ...
            for (hpipe = 0; hpipe< MAX_SCH_SLICES; hpipe++) {
                tdm_schedule = &(sch_info->tdm_egress_schedule_pipe[
                                 pipe].tdm_schedule_slice[hpipe]);
                _soc_hr4_tdm_print_tdm_info(unit, tdm_schedule, hpipe, 0);
            }
        }
    }
    ...
    if ((sch_info->is_flexport == 1) && (sch_info->nport > 0)) {
        ...
        for (i = 0; i < sch_info->nport; i++) {
            _soc_hr4_tdm_print_port_resource(unit, &sch_info->resource[i], i);
        }
    }
}


/*! @fn void soc_hr4_tdm_get_port_ratio(
        int unit,
        soc_port_schedule_state_t *sch_info,
        int pm_num,
        int *pm_mode,
 ...
 */
void
soc_hr4_tdm_get_port_ratio(int unit,
                            soc_port_schedule_state_t *sch_info,
                            int pm_num, int *pm_mode, int prev_or_new)
{
    int port, phy_port_base, lane, mode = 0;
    int num_lanes[HURRICANE4_TDM_PORTS_PER_PBLK];
    int speed_max[HURRICANE4_TDM_PORTS_PER_PBLK];
    soc_port_map_type_t *port_map;

    sal_memset(num_lanes, 0, HURRICANE4_TDM_PORTS_PER_PBLK*sizeof(int));
    sal_memset(speed_max, 0, HURRICANE4_TDM_PORTS_PER_PBLK*sizeof(int));
   ...
   /* phy_port_base = 1 + (pm_num * HURRICANE4_TDM_PORTS_PER_PBLK); */
     phy_port_base = _soc_hr4_tdm_get_phy_port_base(pm_num);
    for (lane = 0; lane < HURRICANE4_TDM_PORTS_PER_PBLK; lane += 2) {
...
/*! @fn void _soc_hr4_tdm_get_pipe_map(
        int unit,
        soc_port_schedule_state_t *sch_info,
        uint32 *pipe_map)
    @param unit Chip unit number.
 ...
 */
void
_soc_hr4_tdm_get_pipe_map(int unit,
                          soc_port_schedule_state_t *sch_info,
                          uint32 *pipe_map)
{
    ...
    if (sch_info->is_flexport == 0) { /* Init */
        for (port = 0; port < HURRICANE4_TDM_PHY_PORTS_PER_DEV; port++) {
            if (sch_info->out_port_map.log_port_speed[port] > 0 ) {
                pipe = port / HURRICANE4_TDM_PHY_PORTS_PER_PIPE;
                pipe_bmap |= 1 << pipe;
            }
        }
    } else { /* FlexPort */
...
/*! @fn int _soc_hr4_tdm_get_mport_2_hp_num(
        uint32 phy_num
        )
    @param phy_num
 ...
 */
int
 _soc_hr4_tdm_get_mport_2_hp_num(uint32 mmu_port_num
                          )
{
    if (mmu_port_num<= 7)
        return((mmu_port_num));
...
/*! @fn int _soc_hr4_tdm_get_cal_id(
        uint32 phy_num
        )
    @param phy_num
    @brief Returns PM ID.Needed correction as 68/4 results 17 pm instead 8
 * Description:
        Returns PM ID
 */
int
_soc_hr4_tdm_get_cal_id(uint32 phy_num
                          )
{
    ...
    if (phy_num < 24)
        ...
        return(((phy_num-24)/16) + 3);
    }else if(phy_num < 56) {
        return(((phy_num-40)/16) + 4);
    }else if(phy_num < 60) {
        return(( (phy_num - 56)/4)+5);
    } else if(phy_num <64) {
        return(( (phy_num - 60)/4)+6);
    } else if(phy_num <68) {
        return(( (phy_num - 64)/4)+7);
    }
    else
        return(HR4_NUM_EXT_PORTS);
}

/*! @fn int _soc_hr4_tdm_get_phy_port_base (
        uint32 pm_num
        )
    @param phy_num
    @brief Returns PM ID.Needed correction as 76/4 results 19 pm instead 10
 ...
 */
int
_soc_hr4_tdm_get_phy_port_base (uint32 pm_num)
{
    if (pm_num < 2)
        return((pm_num*12)+1);
    else if(pm_num >= 2 && pm_num < 3) {
        return((pm_num*12)+1);
        }
    else if(pm_num >= 3 && pm_num < 4) {
        return((pm_num*12)+5);
        }
    else if(pm_num >= 4 && pm_num < 5) {
        return((pm_num*14)+1);
        }
    else if(pm_num >= 5 && pm_num < 6) {
        return((pm_num*12)+1);
        }
    else
        return(HR4_NUM_EXT_PORTS);
...
/*! @fn void _soc_hr4_tdm_get_ovs_pipe_map(
        int unit,
        soc_port_schedule_state_t *sch_info,
        uint32 *ovs_pipe_map)
    @param unit Chip unit number.
 ...
 */
void
_soc_hr4_tdm_get_ovs_pipe_map(int unit,
                              soc_port_schedule_state_t *sch_info,
                              uint32 *ovs_pipe_map)
{
...
/*! @fn int _soc_hr4_tdm_get_speed_ovs_class(
        int unit,
        int speed)
    @param unit   Chip unit number.
    @param speed Speed
 ...
 */
int
_soc_hr4_tdm_get_speed_ovs_class(int unit, int speed)
{
    int speed_class = 0;
   /*uint16 dev_id = 0;*/
   /*uint8  rev_id = 0;*/
    ...
    if (speed >= 40000) {
        ...
        if (speed >= 25000) {
            ...
            /* take 1G speed as 10G in HR4 for ovsb port */
            speed_class = 1;
        }
    }

/* In 56874 10G & 25G mixed is allowed and 1G, 10G & 25G speed port */
/* always treated as 25G speed port*/
/* soc_cm_get_id(unit, &dev_id, &rev_id);*/
/* if (((dev_id & TDM_HR4_DEV_ID_MASK) ==*/
/*     (TDM_HR4_DEV_ID_BCM56370 & TDM_HR4_DEV_ID_MASK)) &&*/
/*     (speed_class == 1)) {*/
/*     speed_class = 3;*/
/* }*/
/**/
...
/*! @fn int _soc_hr4_tdm_check_is_hsp_port_e(
        int unit,
        int speed)
    @param unit   Chip unit number.
    @param speed Speed
 ...
 */
STATIC int
_soc_hr4_tdm_check_is_hsp_port_e(int unit, int speed)
{
    int speed_class, port_is_hsp = 0;

    speed_class = _soc_hr4_tdm_get_speed_ovs_class(unit, speed);
    switch (speed_class) {
        case 1: if(speed>= 10000)   /* 10G */
                  port_is_hsp = 1;
                else
                  port_is_hsp = 0;
                break;
...
/*! @fn int _soc_hr4_tdm_check_mgmt_enable(
        int unit,
        soc_port_schedule_state_t *sch_info)
    @param unit   Chip unit number.
    @param sch_info Pointer to a soc_port_schedule_state_t struct variable.
 ...
 */
STATIC int
_soc_hr4_tdm_check_mgmt_enable(int unit,
                               soc_port_schedule_state_t *sch_info)
{
    int port;
    int mgmt_port_cnt = 0;
...
#if 0
/*! @fn int _soc_hr4_tdm_check_2nd_mgmt_enable(
        int unit,
        soc_port_schedule_state_t *sch_info)
    @param unit   Chip unit number.
    @param sch_info Pointer to a soc_port_schedule_state_t struct variable.
 ...
 */
STATIC int
_soc_hr4_tdm_check_2nd_mgmt_enable(int unit,
                                   soc_port_schedule_state_t *sch_info)
{
    int port;
    int mgmt_port_cnt = 0;
...
/*! @fn int _soc_hr4_tdm_get_same_port_spacing_e(
        int unit,
        int speed)
    @param unit   Chip unit number.
    @param speed Speed
 ...
 */
int
_soc_hr4_tdm_get_same_port_spacing_e(int unit, int speed)
{
    int port_same_spacing = 0;

    /*if (_soc_hr4_tdm_check_is_hsp_port_e(unit, speed) == 1) {*/
        port_same_spacing = 6;
    /*} else {
        port_same_spacing = 6;
    }*/
...
/*! @fn int _soc_hr4_tdm_get_sis_port_spacing_e(
        int unit,
        int speed)
    @param unit   Chip unit number.
    @param speed Speed
 ...
 */
int
_soc_hr4_tdm_get_sis_port_spacing_e(int unit, int phy_port){
    int port_sis_spacing = 0;

    if (phy_port >0 && phy_port <57)
    {
        port_sis_spacing = 4;
    } else {
        port_sis_spacing = 2;
...
/*! @fn int _soc_hr4_tdm_calculation(
        int unit,
        soc_port_schedule_state_t *sch_info)
    @param unit Chip unit number.
    @param sch_info Pointer to a soc_port_schedule_state_t struct variable.
 ...
 */
int
_soc_hr4_tdm_calculation(int unit, soc_port_schedule_state_t *sch_info)
{
    ...
    tdm_soc_t _chip_pkg;
    tdm_mod_t *_tdm_pkg;
    uint32 port_speeds[HURRICANE4_TDM_PHY_PORTS_PER_DEV];
    uint32 port_states[HURRICANE4_TDM_PHY_PORTS_PER_DEV];


  /*  soc_cm_get_id(unit, &dev_id, &rev_id); */
  /*  LOG_VERBOSE(BSL_LS_SOC_COMMON, (BSL_META_U(unit, "hr4_tdm_dev_id %0d\n"), dev_id)); */

    sal_memset(port_speeds, 0, HURRICANE4_TDM_PHY_PORTS_PER_DEV*sizeof(uint32));
    sal_memset(port_states, 0, HURRICANE4_TDM_PHY_PORTS_PER_DEV*sizeof(uint32));


    /* Remap input speeds to ETH bitrates */
    soc_hurricane4_port_schedule_speed_remap(unit, sch_info);

    /* Set port states and speeds */
    for (lgc_port = 0; lgc_port < HURRICANE4_TDM_PHY_PORTS_PER_DEV; lgc_port++) {
        phy_port = sch_info->in_port_map.port_l2p_mapping[lgc_port];
        if (phy_port < HURRICANE4_TDM_PHY_PORTS_PER_DEV) {
            if (sch_info->in_port_map.log_port_speed[lgc_port] > 0 ) {
                ...
                for (lane = 1; lane < port_lanes; lane++) {
                    ...
                    if (phy_port_tmp < HURRICANE4_TDM_PHY_PORTS_PER_DEV) {
    ...
    if (_soc_hr4_tdm_check_mgmt_enable(unit, sch_info) == 1) {
    ...
    if (LOG_CHECK(BSL_LS_SOC_TDM | BSL_INFO)) {
        ...
        LOG_VERBOSE(BSL_LS_SOC_COMMON, (BSL_META_U(unit,
                            "%8s%8s%8s\n"),
                            "port", "speed", "state"));
        for (phy_port = 0; phy_port < HURRICANE4_TDM_PHY_PORTS_PER_DEV;
             phy_port++) {
            LOG_VERBOSE(BSL_LS_SOC_COMMON, (BSL_META_U(unit,
                                "%8d%8d%8d\n"),
   ...
   /* _chip_pkg.dev_id        = dev_id; */
    _chip_pkg.num_ext_ports = HR4_NUM_EXT_PORTS;
    ...
    for (index = 0; index < _chip_pkg.num_ext_ports &&
                    index < HURRICANE4_TDM_PHY_PORTS_PER_DEV; index++) {
    ...
    _chip_pkg.soc_vars.hr4.mgmt_pm_hg= mgmt_pm_hg;
    _chip_pkg.soc_vars.hr4.mgmt_mode = mgmt_mode;

    /* _chip_pkg.soc_vars.hr4.tdm_chk_en = 1; */
    ...
    if (_tdm_pkg == NULL) {
        LOG_CLI((BSL_META_U(unit,
                            "Unable to configure TDM (l-1), "
                            "please contact your Field Applications Engineer "
                            "or Sales Manager for additional assistance.\n")));
        return SOC_E_FAIL;
    } else {
        ...
        for (pipe = 0; pipe < HURRICANE4_TDM_PIPES_PER_DEV; pipe++) {
            ...
            if (tdm_pipe_ovs_group == NULL ||
                ...
                LOG_CLI((BSL_META_U(unit,
                                    "Unable to configure TDM (l-2), "
                                    "please contact your Field Applications "
                                    "Engineer or Sales Manager for "
                                    "additional assistance.\n")));
                tdm_hr4_main_free(_tdm_pkg);
                sal_free(_tdm_pkg);
                return SOC_E_FAIL;
            } else {
                ...
                for (i = 0; i < tdm_pipe_grp_num; i++) {
                    ...
                    hpipe = i / HURRICANE4_TDM_OVS_GROUPS_PER_HPIPE;
                    group = i % HURRICANE4_TDM_OVS_GROUPS_PER_HPIPE;
                ...
                for (hpipe = 0; hpipe < 2; hpipe++) {
                    for (j = 0; j < HR4_SHAPING_GRP_LEN; j++) {
                        ...
                        tdm_pipe_ovs_group[hpipe+HR4_SHAPING_GRP_IDX_0][j];
                    }
                }
            }
        }
        ...
        for (pm_num = 0; pm_num < HR4_NUM_PHY_PM; pm_num++) {
            for (lane = 0; lane < HURRICANE4_TDM_PORTS_PER_PBLK; lane++) {
                sch_info->out_port_map.port_p2PBLK_inst_mapping[
                (pm_num * HURRICANE4_TDM_PORTS_PER_PBLK) + 1 + lane] =
    ...
    if (LOG_CHECK(BSL_LS_SOC_TDM | BSL_INFO)) {
        for (pipe = 0; pipe < HURRICANE4_TDM_PIPES_PER_DEV; pipe++) {
            ...
            LOG_VERBOSE(BSL_LS_SOC_COMMON, (BSL_META_U(unit, "\n")));
            /* Oversub groups */
            for (hpipe = 0; hpipe < HURRICANE4_TDM_HPIPES_PER_PIPE; hpipe++) {
                LOG_VERBOSE(BSL_LS_SOC_COMMON, (BSL_META_U(unit,
                            "Pipe %d Half-Pipe %d [IDB TDM Oversub Groups]\n"),
                            pipe, hpipe));
                for (group = 0; group < HURRICANE4_TDM_OVS_GROUPS_PER_HPIPE;
                     group++) {
                    LOG_VERBOSE(BSL_LS_SOC_COMMON, (BSL_META_U(unit,
                                "oversub group %d: "), group));
                    for (index = 0; index < HURRICANE4_TDM_OVS_GROUP_LENGTH;
                         index++) {
                        LOG_VERBOSE(BSL_LS_SOC_COMMON, (BSL_META_U(unit, " %3d"),
                                    sch_info->tdm_ingress_schedule_pipe[pipe].
    ...
    */
    ...
    _soc_hr4_tdm_print_schedule_state(unit, sch_info);
    return SOC_E_NONE;
}


/*! @fn int _soc_hr4_tdm_set_idb_calendar(
        int unit,
        soc_port_schedule_state_t *sch_info)
    @param unit Chip unit number.
    @param sch_info Pointer to a soc_port_schedule_state_t struct variable.
    @brief API to initialize the HR4 IDB main calendar.
 */
int
_soc_hr4_tdm_set_idb_calendar(int unit, soc_port_schedule_state_t *sch_info)
{
    uint32 pipe_map;
    uint32 cal_len;
    soc_mem_t mem;
    ...
    uint32 mem_indx;
    static const soc_mem_t idb_tdm_cal_mem[HURRICANE4_TDM_PIPES_PER_DEV][2] = {
        {IS_TDM_CALENDAR0_PIPE0m, IS_TDM_CALENDAR1_PIPE0m}/* ,
        {IS_TDM_CALENDAR0_PIPE1m, IS_TDM_CALENDAR1_PIPE1m} */
    };
    static const soc_reg_t idb_tdm_cfg_reg[HURRICANE4_TDM_PIPES_PER_DEV] = {
    ...
    _soc_hr4_tdm_get_pipe_map(unit, sch_info, &pipe_map);
    ...
    for (pipe = 0; pipe < HURRICANE4_TDM_PIPES_PER_DEV; pipe++) {
        ...
        for (slot = 0; slot < length; slot += 2) {
            ...
            if (phy_port == HR4_OVSB_TOKEN) {
                idb_port = HURRICANE4_TDM_IDB_OVSB_TOKEN;
            } else if (phy_port == HR4_IDL1_TOKEN) {
                idb_port = HURRICANE4_TDM_IDB_SBUS_TOKEN;
            } else if (phy_port == HR4_IDL2_TOKEN) {
                idb_port = HURRICANE4_TDM_IDB_OPT2_TOKEN;
            } else if (phy_port == HR4_NULL_TOKEN) {
                idb_port = HURRICANE4_TDM_IDB_NULL_TOKEN;
            } else if (phy_port == HR4_FAE_TOKEN) {
                idb_port = HURRICANE4_TDM_IDB_FAE_TOKEN;
            } else if (phy_port >= HR4_NUM_EXT_PORTS) {
                idb_port = HURRICANE4_TDM_IDB_UNUSED_TOKEN;
            } /* else if (phy_port == HR4_MGM1_TOKEN) { */
                /* TDM code always allocates slots for management port.
                 * If mgmt ports are omitted from config, this mapping
                 * is not set up.
                 */
               /* idb_port = HURRICANE4_TDM_IDB_MGMT1_TOKEN; }*/
	    else {
                port = sch_info->out_port_map.port_p2l_mapping[phy_port];
                idb_port = sch_info->out_port_map.port_l2i_mapping[port] & 0x7f;
            }
            if (idb_port > 68 || idb_port == 64) { /* Not a general front panel port */
                id = 0xf;
            } else {
              /*  id = (phy_port - 1) / HURRICANE4_TDM_PORTS_PER_PBLK; */
	        id = _soc_hr4_tdm_get_cal_id((phy_port-1));
            }
   ...
   *      if (_soc_hr4_tdm_check_2nd_mgmt_enable(unit, sch_info) == 1 &&
   *         pipe == (HURRICANE4_TDM_PIPES_PER_DEV - 1)) {
...
/*! @fn int _soc_hr4_tdm_set_idb_hsp(
        int unit,
        soc_port_schedule_state_t *sch_info)
    @param unit Chip unit number.
    @param sch_info Pointer to a soc_port_schedule_state_t struct variable.
    @brief API to initialize the HR4 IDB HSP.
 */
int
_soc_hr4_tdm_set_idb_hsp(int unit, soc_port_schedule_state_t *sch_info)
{
    soc_reg_t reg;
    uint32 pipe_map;
    int port, idb_port, phy_port;
    int port_speed;
    uint32 port_map[3];
    uint32 rval = 0;
    ...
    for (port=0; port< 70 ; port++) { /* mmu logical ports */
        ...
        if (port_speed > 0) {
            /*if (_soc_hr4_tdm_check_is_hsp_port_e(unit, port_speed) == 1) {*/
                phy_port = sch_info->out_port_map.port_l2p_mapping[port];
                if ((phy_port >= 1) &&
                    (phy_port <= 68)) {
                    idb_port = sch_info->out_port_map.port_l2i_mapping[port] & 0x7f;
                    if (idb_port < 32) {
                        port_map[0] |= 1 << idb_port;
                    }
                    else if (idb_port < 64) {
                        port_map[1] |= 1 << (idb_port - 32);
                    }
                    else if (idb_port > 63) {
                        port_map[2] |= 1 << (idb_port - 64);
                    }
                }
            /*}*/
        }
...
/*! @fn int _soc_hr4_tdm_set_idb_ppe_credit(
        int unit,
    ...
    @param unit Chip unit number.
    @param sch_info Pointer to a soc_port_schedule_state_t struct variable.
    @brief API to initialize the HR4 sister port spacing
    @param full_credit_threshold_0
    @param opp_credit_threshold_0
    @param full_credit_threshold_1
    @param opp_credit_threshold_1
           Regs/Mems configured:IS_TDM_CONFIG_SISPORT_SPACING_CHECK_PIPE0r
 */
int
_soc_hr4_tdm_set_idb_ppe_credit(int unit,
                                soc_port_schedule_state_t *sch_info,
                                int full_credit_threshold_0,
                                int opp_credit_threshold_0,
                                int full_credit_threshold_1,
                                int opp_credit_threshold_1 )
{
    ...
    uint32 rval  = 0;*/
    uint32 rval1 = 0;
    static const soc_reg_t is_tdm_cfg_sisport_spacing_chk
                  = IS_TDM_CONFIG_SISPORT_SPACING_CHECKr;
    /*static const soc_reg_t idb_ppe_credit[HURRICANE4_TDM_PIPES_PER_DEV] = {
    ...
    rval1 = 0;
    /* IDB reg programming   */
    soc_reg_field_set(unit, is_tdm_cfg_sisport_spacing_chk,
        &rval1, ENABLE_1IN2_CHECKf, 0x00e0);
    SOC_IF_ERROR_RETURN(soc_reg32_set(unit, is_tdm_cfg_sisport_spacing_chk,
...
/*! @fn int _soc_hr4_tdm_set_idb_opportunistic(
        int unit,
        soc_port_schedule_state_t *sch_info,
        int cpu_op_en,
        int lb_opp_en,
    ...
    @brief API to initialize the HR4 IDB Opportunistic related registers
           Regs/Mems configured: IS_CPU_LB_OPP_CFG_PIPE<pipe>
                                 IS_OPP_SCHED_CFG_PIPE<pipe>
 */
int
_soc_hr4_tdm_set_idb_opportunistic(int unit,
                                   soc_port_schedule_state_t *sch_info,
                                   int cpu_op_en,
                                   int lb_opp_en,
                                   int opp1_port_en,
                                   ...
                                   int opp_ovr_sub_en)
{
    ...
    int pipe;
    uint32 rval = 0;
    static const soc_reg_t idb_cpu_lb_opp_cfg[HURRICANE4_TDM_PIPES_PER_DEV] = {
        IS_CPU_LB_OPP_CFG_PIPE0r/* , IS_CPU_LB_OPP_CFG_PIPE1r */
    };
    static const soc_reg_t idb_opp_sched_cfg[HURRICANE4_TDM_PIPES_PER_DEV] = {
...
/*! @fn int soc_hr4_tdm_set_idb_dpp_ctrl(
        int unit,
        soc_port_schedule_state_t *sch_info,
        int credits)
    @param unit Chip unit number.
    @param sch_info Pointer to a soc_port_schedule_state_t struct variable.
    @param credits
    @brief API to initialize the HR4 IDB_DPP_CTRL register.
 */
int
soc_hr4_tdm_set_idb_dpp_ctrl(int unit,
                             soc_port_schedule_state_t *sch_info,
                             int credits)
{
    soc_reg_t reg;
    ...
    int pipe;
    uint32 rval = 0;
    static const soc_reg_t idb_dpp_ctrl[HURRICANE4_TDM_PIPES_PER_DEV] = {
...
/*! @fn int _soc_hr4_tdm_set_idb_pkt_calendar(
        int unit,
        soc_port_schedule_state_t *sch_info)
    @param unit Chip unit number.
    @param sch_info Pointer to a soc_port_schedule_state_t struct variable.
    @brief API to initialize the HR4 IDB Packet Shaper calendar.
           Regs/Mems configured: IDB PKT_SCH_CALENDAR
 */
int
_soc_hr4_tdm_set_idb_pkt_calendar(int unit,
                                  soc_port_schedule_state_t *sch_info)
{
    uint32 pipe_map;
    soc_mem_t mem;
    ...
    uint32 memfld;
    uint32 entry[SOC_MAX_MEM_WORDS];
    static const soc_mem_t idb_pkt_shaper_mems[HURRICANE4_TDM_PIPES_PER_DEV][2] = {
    ...
    for (pipe = 0; pipe < HURRICANE4_TDM_PIPES_PER_DEV; pipe++) {
        ...
        for (hpipe = 0; hpipe < HURRICANE4_TDM_HPIPES_PER_PIPE; hpipe++) {
            ...
            for (slot = 0; slot < HR4_SHAPING_GRP_LEN; slot ++) {
                phy_port = sch_info->tdm_ingress_schedule_pipe[pipe].
                tdm_schedule_slice[hpipe].pkt_sch_or_ovs_space[0][slot];
                if ((phy_port >= HR4_NUM_EXT_PORTS) || (phy_port <= 0)) {
                    idb_port = HURRICANE4_TDM_IDB_UNUSED_TOKEN;
                } else {
...
/*! @fn int _soc_hr4_tdm_set_ovs_group_sel(
        int unit,
        soc_port_schedule_state_t *sch_info,
        int set_idb,
        int set_mmu)
    ...
    @brief API to initialize the HR4 MMU &/or IDB OVS tables.
            Regs/Mems configured: IDB HPIPE0/1_OVR_SUB_GRP_CFG,
                                  HPIPE0/1_OVR_SUB_GRP_CFG
                                  PBLK<n>_CALENDAR
 */
int
_soc_hr4_tdm_set_ovs_group_sel(int unit,
                               soc_port_schedule_state_t *sch_info,
                               int set_idb, int set_mmu)
{
    ...
    int pblks[HURRICANE4_TDM_PIPES_PER_DEV][2][HURRICANE4_TDM_PBLKS_PER_HPIPE];
    int pm_num, pblk_cal_idx;
    int pblk_valid, pblk_spacing;
    int port_speed_class, port_sister_spacing;

    static const soc_reg_t idb_grp_cfg_regs[HURRICANE4_TDM_PIPES_PER_DEV][2] = {
    ...
    static const soc_reg_t idb_grp_ovs_regs[HURRICANE4_TDM_PIPES_PER_DEV][2][6] = {
        {
    ...
    static const soc_reg_t idb_pblk_cal_regs[HURRICANE4_TDM_PIPES_PER_DEV][2][10] = {
        {
    ...
    _soc_hr4_tdm_get_pipe_map(unit, sch_info, &pipe_map);
    _soc_hr4_tdm_get_ovs_pipe_map(unit, sch_info, &ovs_pipe_map);
        ...
        for (hpipe = 0; hpipe < HURRICANE4_TDM_HPIPES_PER_PIPE; hpipe++) {
			...
			mmu_port = _soc_hr4_tdm_get_mport_2_hp_num(mmu_port) ;
                        id = sch_info->out_port_map.port_p2PBLK_inst_mapping[
                            phy_port] & 0xf;
	        	 /* id = _soc_hr4_tdm_get_cal_id((phy_port-1)) & 0xf; */
                ...
                if (HURRICANE4_TDM_OVS_GROUP_LENGTH == slot) {
                    port_speed_class = 0;
                   /* port_same_spacing = 0; */
                    ...
                    port_speed_class =
                        _soc_hr4_tdm_get_speed_ovs_class(unit, speed_max);
                   /* port_same_spacing =
                    *    _soc_hr4_tdm_get_same_port_spacing_e(unit, speed_max);
										*/
                    port_sister_spacing =
                        _soc_hr4_tdm_get_sis_port_spacing_e(unit, port);
                }
                if (set_idb == 1) {
                    rval = 0;
    ...
    sal_memset(pblks, -1, sizeof(int) * HURRICANE4_TDM_PIPES_PER_DEV * 2 *
               HURRICANE4_TDM_PBLKS_PER_HPIPE);
    for (pipe = 0; pipe < HURRICANE4_TDM_PIPES_PER_DEV; pipe++) {
        if (!(ovs_pipe_map & (1 << pipe))) {
            continue;
        }
            ...
            for (group = 0; group < HURRICANE4_TDM_OVS_GROUPS_PER_HPIPE; group++) {
                for (slot = 0; slot < HURRICANE4_TDM_OVS_GROUP_LENGTH; slot++) {
                    phy_port = sch_info->tdm_ingress_schedule_pipe[pipe].
                                         ...
                                         oversub_schedule[group][slot];
                    if (phy_port < HR4_NUM_EXT_PORTS) {
                      /*  pm_num = (phy_port - 1) / HURRICANE4_TDM_PORTS_PER_PBLK; */
		           pm_num = _soc_hr4_tdm_get_cal_id((phy_port - 1));
                        pblk_cal_idx = sch_info->out_port_map.
                                       port_p2PBLK_inst_mapping[phy_port];
                        if (pblk_cal_idx >= 0 &&
                            pblk_cal_idx < HURRICANE4_TDM_PBLKS_PER_HPIPE) {
    ...
    for (pipe = 0; pipe < HURRICANE4_TDM_PIPES_PER_DEV; pipe++) {
        ...
        inst = pipe | SOC_REG_ADDR_INSTANCE_MASK;
        for (hpipe = 0; hpipe < HURRICANE4_TDM_HPIPES_PER_PIPE; hpipe++) {
            for (pblk_cal_idx = 0; pblk_cal_idx < HURRICANE4_TDM_PBLKS_PER_HPIPE;
                    pblk_cal_idx++) {
                pm_num = pblks[pipe][hpipe][pblk_cal_idx];
                /*phy_port_base = 1 + (pm_num * HURRICANE4_TDM_PORTS_PER_PBLK); */
		phy_port_base = _soc_hr4_tdm_get_phy_port_base(pm_num);
                /* Get port ratio for this Port Macro */
                soc_hr4_tdm_get_port_ratio(unit, sch_info, pm_num, &mode, 1);
                 ...
                 */
                if ((pm_num < 0) || (pm_num > HURRICANE4_TDM_PBLKS_PER_DEV)) {
			    ...
			    mmu_port = _soc_hr4_tdm_get_mport_2_hp_num(mmu_port) ;
                            ...
                            port_speed_class =
                                _soc_hr4_tdm_get_speed_ovs_class(unit,
                                                                 speed_max);
                            pblk_spacing =
                                _soc_hr4_tdm_get_same_port_spacing_e(unit,
                                                                     speed_max);
                        }
...
/*! @fn int _soc_hr4_tdm_set_ovs_group(
        int unit,
        soc_port_schedule_state_t *sch_info)
    @param unit Chip unit number.
    @param sch_info Pointer to a soc_port_schedule_state_t struct variable.
    @brief API to initialize the HR4 MMU & IDB OVS tables.
            Regs/Mems configured: IDB HPIPE0/1_OVR_SUB_GRP_CFG
                                  HPIPE0/1_OVR_SUB_GRP_CFG
                                  PBLK<n>_CALENDAR
 */
int
_soc_hr4_tdm_set_ovs_group(int unit, soc_port_schedule_state_t *sch_info)
{
    return (_soc_hr4_tdm_set_ovs_group_sel(unit, sch_info, 1, 1));
}


/*! @fn int _soc_hr4_tdm_set_mmu_calendar(
        int unit,
        soc_port_schedule_state_t *sch_info)
    @param unit Chip unit number.
    @param sch_info Pointer to a soc_port_schedule_state_t struct variable.
    @brief API to initialize the HR4 MMU main calendar.
            Regs/Mems configured: MMU TDM_CALENDAR
                                  TDM_CONFIG
 */
int
_soc_hr4_tdm_set_mmu_calendar(int unit, soc_port_schedule_state_t *sch_info)
{
    uint32 pipe_map;
    uint32 cal_len;
    soc_mem_t mem;
    ...
    for (pipe = 0; pipe < HURRICANE4_TDM_PIPES_PER_DEV; pipe++) {
        ...
        mem = calendar_mems[(calendar_id * HURRICANE4_TDM_PIPES_PER_DEV) + pipe];
        cal_end_field = calendar_end_fields[calendar_id];
        ...
        for (length = cal_len; length > 0; length--) {
            if (sch_info->tdm_egress_schedule_pipe[pipe].
                          ...
                          linerate_schedule[length - 1] != HR4_NUM_EXT_PORTS) {
                break;
            }
        }
        for (slot = 0; slot < length; slot += 2) {
            ...
            if (phy_port == HR4_OVSB_TOKEN) {
                mmu_port = HURRICANE4_TDM_MMU_OVSB_TOKEN;
            } else if (phy_port == HR4_IDL1_TOKEN) {
                mmu_port = HURRICANE4_TDM_MMU_PURG_TOKEN;
            } else if (phy_port == HR4_IDL2_TOKEN) {
                mmu_port = HURRICANE4_TDM_MMU_UNUSED_TOKEN;
            } else if (phy_port == HR4_NULL_TOKEN) {
                mmu_port = HURRICANE4_TDM_MMU_NULL_TOKEN;
            } else if (phy_port >= HR4_NUM_EXT_PORTS) {
                mmu_port = HURRICANE4_TDM_MMU_UNUSED_TOKEN;
            }
             else {
...
/*! @fn int _soc_hr4_tdm_set_mmu_hsp(
        int unit,
        soc_port_schedule_state_t *sch_info)
    @param unit Chip unit number.
    @param sch_info Pointer to a soc_port_schedule_state_t struct variable.
    @brief API to initialize the HR4 MMU HSP.
            Regs/Mems configured: TDM_HSP_PIPE
 */
int
_soc_hr4_tdm_set_mmu_hsp(int unit,
                         soc_port_schedule_state_t *sch_info)
{
    soc_reg_t reg;
    uint32 pipe_map;
    ...
    uint32 rval1 = 0;
    static const soc_reg_t is_tdm_cfg_sisport_spacing_chk
                  = TDM_CONFIG_SISPORT_SPACING_CHECKr;
    ...
    /* MMU reg programming for HR4 newly added register  */
    soc_reg_field_set(unit, is_tdm_cfg_sisport_spacing_chk,
        &rval1, ENABLE_1IN2_CHECKf, 0x00e0);
    SOC_IF_ERROR_RETURN(soc_reg32_set(unit, is_tdm_cfg_sisport_spacing_chk,
             REG_PORT_ANY, 0, rval1));
    ...
    for (port=0; port< 70 ; port++) { /* mmu logical ports */
        ...
        if (port_speed > 0) {
            if (_soc_hr4_tdm_check_is_hsp_port_e(unit, port_speed) == 1) {
                phy_port = sch_info->out_port_map.port_l2p_mapping[port];
                if ((phy_port >= 1) &&
                    (phy_port <= HURRICANE4_TDM_GPORTS_PER_DEV)) {
...
/*! @fn int _soc_hr4_tdm_set_mmu_opportunistic(
        int unit,
        soc_port_schedule_state_t *sch_info,
        int cpu_op_en,
        int lb_opp_en,
    ...
    @param opp2_port_en
    @param opp_ovr_sub_en
    @brief API to initialize the HR4 MMU Opportunistic related registers
            Regs/Mems configured: CPU_LB_OPP_CFG
                                  OPP_SCHED_CFG
 */
int
_soc_hr4_tdm_set_mmu_opportunistic(int unit,
                                   soc_port_schedule_state_t *sch_info,
                                   int cpu_op_en,
                                   int lb_opp_en,
                                   int opp1_port_en,
 ...
 */
int soc_hr4_tdm_init(int unit, soc_port_schedule_state_t *sch_info)
{
    SOC_IF_ERROR_RETURN(_soc_hr4_tdm_calculation(unit, sch_info));

    SOC_IF_ERROR_RETURN(_soc_hr4_tdm_set_idb_calendar(unit, sch_info));
    SOC_IF_ERROR_RETURN(_soc_hr4_tdm_set_idb_hsp(unit, sch_info));
   /* SOC_IF_ERROR_RETURN(_soc_hr4_tdm_set_idb_pkt_calendar(unit, sch_info)); */
    SOC_IF_ERROR_RETURN(_soc_hr4_tdm_set_idb_ppe_credit(unit, sch_info, 20, 6, 20, 6));
    /* SOC_IF_ERROR_RETURN(soc_hr4_tdm_set_idb_dpp_ctrl(unit, sch_info, 32)); */
    /* IDB opp1_port_en should always be 1 to enable sbus access to the rest of IPIPE*/
    SOC_IF_ERROR_RETURN(_soc_hr4_tdm_set_idb_opportunistic(unit, sch_info,
                                                           1, 1, 1, 1, 1));

    /*SOC_IF_ERROR_RETURN(_soc_hr4_tdm_set_ovs_group(unit, sch_info)); */

    SOC_IF_ERROR_RETURN(_soc_hr4_tdm_set_mmu_calendar(unit, sch_info));
    SOC_IF_ERROR_RETURN(_soc_hr4_tdm_set_mmu_hsp(unit, sch_info));
    SOC_IF_ERROR_RETURN(_soc_hr4_tdm_set_mmu_opportunistic(unit, sch_info,
                                                           1, 1, 1, 1, 1));

    return SOC_E_NONE;
}

/*! @fn int soc_hurricane4_port_speed_higig2eth(int unit, int speed)
 *  @param int unit
 *  @param int speed
 *  @brief Map port speed to ETH bitrates
 */
static int
soc_hurricane4_port_speed_higig2eth(int unit, int speed)
{
    switch (speed) {
    case 11000:
        /* 10G */
        return 10000;
    case 21000:
        /* 20G/21G based on enhanced tdm support for x1 */
        if (soc_feature(unit, soc_feature_td3x1_enhanced_tdm)) {
            return 21000;
        } else {
            return 20000;
        }
...
/*! @fn void soc_hurricane4_port_schedule_speed_remap(int unit,
 ...
 */
void
soc_hurricane4_port_schedule_speed_remap(
    int  unit,
    soc_port_schedule_state_t *port_schedule_state)
{
    int i, port;
    ...
    for (port=0; port < SOC_MAX_NUM_PORTS; port++) {
        port_schedule_state->in_port_map.log_port_speed[port] =
            soc_hurricane4_port_speed_higig2eth(unit,
                port_schedule_state->in_port_map.log_port_speed[port]);
    }
