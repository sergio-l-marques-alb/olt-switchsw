...
#ifndef SOC_TRIDENT3_FLEXPORT_C
#define SOC_TRIDENT3_FLEXPORT_C


#include <soc/flexport/trident3/trident3_flexport.h>

int (*td3_flexport_phases[MAX_FLEX_PHASES]) (int unit, 
soc_port_schedule_state_t *port_schedule_state) = {
  &soc_trident3_flex_start,
  &soc_trident3_flex_top_port_down,
  &soc_trident3_tdm_calculation_flexport,
  &soc_trident3_flex_ep_reconfigure_remove,
  &soc_trident3_flex_mmu_reconfigure_phase1,
  &soc_trident3_flex_mmu_reconfigure_phase2,
  &soc_trident3_flex_idb_reconfigure,
  &soc_trident3_flex_ep_reconfigure_add,
  &soc_trident3_flex_top_port_up,
  &soc_trident3_flex_end,
  NULL,
  NULL,
  NULL,
  NULL,
...
void *soc_trident3_port_lane_info_alloc (
  void 
  ) {
...
int soc_trident3_set_port_lane_info (
  int unit,
...
int soc_trident3_get_port_lane_info (
  int unit,
  const void *lane_info_ptr,
  int *port_idx,
  int *pgw,
...
int soc_trident3_reconfigure_ports (
  int unit,
  soc_port_schedule_state_t *port_schedule_state
  ) {
  int i, j, rv;
  ...
  for (i = 0; i < MAX_FLEX_PHASES; i++) {
      if (td3_flexport_phases[i] != NULL) {
          rv = td3_flexport_phases[i](unit, port_schedule_state);
          if (rv != SOC_E_NONE) {
...
int soc_trident3_flex_start (
  int unit,
  soc_port_schedule_state_t *port_schedule_state
  ) {
  soc_trident3_flex_scratch_t *cookie;

  cookie = (soc_trident3_flex_scratch_t *) sal_alloc(
					sizeof(soc_trident3_flex_scratch_t), "");
...
/*! @fn int soc_trident3_flex_top_port_down(int unit, soc_port_schedule_state_t
 *               *port_schedule_state)
    ...
    )
{
    ...
    SOC_IF_ERROR_RETURN(soc_trident3_flex_dis_forwarding_traffic(unit,
                                                   port_schedule_state));
    SOC_IF_ERROR_RETURN(soc_trident3_flex_mac_rx_port_down(unit,
                                                   port_schedule_state));
    SOC_IF_ERROR_RETURN(soc_trident3_flex_idb_port_down(unit,
                                                   port_schedule_state));
    SOC_IF_ERROR_RETURN(soc_trident3_flex_mmu_port_down(unit,
                                                   port_schedule_state));
    SOC_IF_ERROR_RETURN(soc_trident3_flex_ep_port_down(unit,
                                                   port_schedule_state));
    SOC_IF_ERROR_RETURN(soc_trident3_flex_mac_tx_port_down(unit,
                                                   port_schedule_state));

    return SOC_E_NONE;
}

/*! @fn int soc_trident3_flex_top_port_up(int unit, soc_port_schedule_state_t
 *               *port_schedule_state)
 *   @param unit Chip unit number.
 *   @param port_schedule_state Pointer to data holding all required information
 *          for FlexPort opeations.
...
{
	LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit, 
				"START soc_trident3_flex_top_port_up\n")));
    SOC_IF_ERROR_RETURN(soc_trident3_flex_mmu_port_up(unit,
                                                       port_schedule_state));
    SOC_IF_ERROR_RETURN(soc_trident3_flex_ep_port_up(unit, 
													   port_schedule_state));
    SOC_IF_ERROR_RETURN(soc_trident3_flex_idb_port_up(unit,
                                                       port_schedule_state));
    SOC_IF_ERROR_RETURN(soc_trident3_flex_mac_port_up(unit,
                                                       port_schedule_state));
    SOC_IF_ERROR_RETURN(soc_trident3_flex_en_forwarding_traffic(unit,
                                                       port_schedule_state));

	LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit, 
				"END soc_trident3_flex_top_port_up\n")));
    return SOC_E_NONE;
}

/*! @fn int soc_trident3_get_ct_class(int speed)
 *  @param speed Speed
 *  @brief Helper function to get CT class
 */
int
soc_trident3_get_ct_class(int speed)
{
    int ct_class = 0;
    switch (speed) {
