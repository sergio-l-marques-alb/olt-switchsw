...
/**
@name: tdm_hx5_filter_chk_slot_shift_cond
@param:

**/
int
tdm_hx5_filter_chk_slot_shift_cond(int slot, int dir, int cal_len, int *cal_main, tdm_mod_t *_tdm)
{
    ...
    if (slot < cal_len) {
        ...
        idx     = src;
        port    = cal_main[idx];
        port_pm = tdm_hx5_cmn_get_port_pm(port, _tdm);
        if (port >= param_phy_lo && port <= param_phy_hi) {
            ...
            for (k=1; k<=param_space_sister; k++) {
                ...
                if (port_k >= param_phy_lo && port_k <= param_phy_hi) {
                    if (tdm_hx5_cmn_get_port_pm(port_k, _tdm) == port_pm) {
                        result = FAIL;
                        break;
                    }
                }
...
/**
@name: tdm_hx5_filter_lr_jitter
@param:

**/
int
tdm_hx5_filter_lr_jitter(tdm_mod_t *_tdm, int min_speed)
{
    ...
    if (param_lr_en==BOOL_TRUE && (min_speed==SPEED_10G || min_speed==SPEED_25G || min_speed==SPEED_40G || min_speed==SPEED_50G || min_speed==SPEED_100G)) {
        for (i=0; i<param_cal_len; i++) {
            ...
            port_speed = tdm_hx5_cmn_get_port_speed(port, _tdm);
            ...
            if (port < param_ext_phy_num) {
                ...
                port_bmp[port] = 1;
                tdm_hx5_cmn_get_speed_jitter(port_speed, param_cal_len,
                                             &space_min, &space_max);
                for (j=0; j<param_cal_len; j++) {
                    k = (i+param_cal_len-j)%param_cal_len;
                    if (param_cal_main[k] == port) {
                        idx_curr = k;
                        dist_up = tdm_hx5_cmn_get_same_port_dist(idx_curr,
                                    TDM_DIR_UP, param_cal_main, param_cal_len);
                        dist_dn = tdm_hx5_cmn_get_same_port_dist(idx_curr,
                        ...
                        if (dist_up > space_max || dist_dn < space_min) {
                            ...
                            for (m=1; m<=dist_mv; m++) {
                                ...
                                y = (x + 1) % param_cal_len;
                                chk_x = tdm_hx5_filter_chk_slot_shift_cond(x, TDM_DIR_DN,
                                            param_cal_len, param_cal_main, _tdm);
                                chk_y = tdm_hx5_filter_chk_slot_shift_cond(y, TDM_DIR_UP,
                                            param_cal_len, param_cal_main, _tdm);
                                /* shift port */
                        ...
                        /* filter port if space violation is detected: down */
                            ...
                            for (m=1; m<=dist_mv; m++) {
                                ...
                                chk_x = tdm_hx5_filter_chk_slot_shift_cond(x, TDM_DIR_UP,
                                            param_cal_len, param_cal_main, _tdm);
                                chk_y = tdm_hx5_filter_chk_slot_shift_cond(y, TDM_DIR_DN,
                                            param_cal_len, param_cal_main, _tdm);
                                /* shift port */
                                if (chk_x == PASS && chk_y == PASS) {
                                    param_cal_main[y] = param_cal_main[x];
...
/**
@name: tdm_hx5_filter_lr
@param:

**/
int
tdm_hx5_filter_lr(tdm_mod_t *_tdm)
{
    if (tdm_hx5_filter_lr_jitter(_tdm, SPEED_10G) == 0){
        return PASS;
    }
    if (tdm_hx5_filter_lr_jitter(_tdm, SPEED_20G) == 0) {
        return PASS;
    }
    if (tdm_hx5_filter_lr_jitter(_tdm, SPEED_40G) == 0) {
        return PASS;
    }
    if (tdm_hx5_filter_lr_jitter(_tdm, SPEED_25G) == 0) {
        return PASS;
    }
    if (tdm_hx5_filter_lr_jitter(_tdm, SPEED_50G) == 0) {
        return PASS;
    }
    return PASS;
}
...
void
tdm_hx5_tdm_print(int *tdm_tbl, int length) 
{
        int i;
        for (i=0; i<length; i++) {      
...
/**
@name: tdm_hx5_osub_filter
@param:

**/
int
tdm_hx5_osub_filter( tdm_mod_t *_tdm, int dont_chk_1n6 )
{
   ...
   int lr_idx, ovs_idx, cur_idx, swap_idx, num_10g_port;
    ...
    for (i=0; i<param_cal_len; i++){
/*TDM_PRINT2(" param_cal_main[%0d] speed is %d \n", i, tdm_hx5_cmn_get_port_speed(param_cal_main[i],_tdm));*/
...
TDM_PRINT2("os_ratio=%0d, lr_ratio=%0d\n", os_ratio, lr_ratio);
	...
	*/
       for (cur_idx=0; cur_idx<param_cal_len; cur_idx++) {
            ...
            *ANC token slot or port with speed==1G (applicable only for HX5/HRCN4)
            */
...
tdm_hx5_tdm_print(param_cal_main, param_cal_len);*/
                     break;
                  }
               }
           }
	...
	*/
       for (cur_idx=0; cur_idx<param_cal_len; cur_idx++) {
           ...
           while(param_cal_main[lr_idx]!=param_token_ovsb && lr_idx<param_cal_len) {
               if ((param_speeds[param_cal_main[lr_idx]]==SPEED_50G) || (param_speeds[param_cal_main[lr_idx]]==SPEED_100G)) { 
                     break; 
               }
               lr_idx++;
            }
...
if (dont_chk_1n6==0) {
  if (os_ratio!=1 ) { /* The below filter is not applicable for 57110 and 5722 */
	num_violations = tdm_hx5_same_port_1n6_filter(_tdm);
  	while(num_violations!=0) {
	 /*TDM_PRINT2("tdm_hx5_same_port_1n6_filter:: iteration = %0d num_violations = %0d\n", num_iter, num_violations);*/
...
/**
@name: tdm_hx5_same_port_1n6_filter
@param:

**/
int
tdm_hx5_same_port_1n6_filter( tdm_mod_t *_tdm )
{
   int swap_idx_start, ovs_idx=0, swap_idx,port;
    ...
    int num_os_slot_rqd = 0;
    int num_os_slot = 0;
    enum port_state_e *param_states;
    int param_phy_lo , param_phy_hi ,port_state, port_speed = 0;
...
num_os_slot = (hp0_idx+hp1_idx)*10 ;
TDM_PRINT2("num_of_os_slots =%0d, num_os_slot_rqd=%0d\n", num_os_slot,num_os_slot_rqd);
tdm_hx5_tdm_print(hp0, hp0_idx);

if(num_os_slot_rqd == 0) /* prevent divide by zero */
   num_os_slot_rqd = 1;

if((num_os_slot/num_os_slot_rqd)> 11)
{
TDM_FREE(hp0);
TDM_FREE(hp1);
...
if (hp0_num_os_port<=3) {
    ...
    if ((param_cal_len-hp0[hp0_idx-hp0_num_os_port]+hp0[0])<6) {
      ...
      for (swap_idx=hp0[0]+swap_idx_start;swap_idx<hp1[0];swap_idx++) { 
        if ((param_cal_main[swap_idx]==param_token_ancl) || (param_speeds[(param_cal_main[swap_idx])] == SPEED_1G) || 
              ((/*param_speeds[(param_cal_main[swap_idx])] == SPEED_10G ||*/(param_speeds[(param_cal_main[swap_idx])] == SPEED_10G  && has_10g_port>4)||param_speeds[(param_cal_main[swap_idx])] == SPEED_2p5G )  )) {
...
tdm_hx5_tdm_print(hp0, hp0_idx);
tdm_hx5_tdm_print(hp1, hp1_idx);
if (hp1_num_os_port<=3) {
    for (i=hp1_num_os_port;i<hp1_idx;i++) {
      if (hp1[i] - hp1[i-hp1_num_os_port]<6) { 
        TDM_PRINT2("Noticed violations between idx - %0d and %0d. need to space them.\n", hp1[i], hp1[i-hp1_num_os_port]) ;
    ...
    if ((param_cal_len-hp1[hp1_idx-hp1_num_os_port]+hp1[0])<6) {
      ...
      for (swap_idx=hp1[0]+swap_idx_start;swap_idx<hp0[0];swap_idx++) { 
        if ((param_cal_main[swap_idx]==param_token_ancl) || (param_speeds[(param_cal_main[swap_idx])] == SPEED_1G) || 
              ((/*param_speeds[(param_cal_main[swap_idx])] == SPEED_10G ||*/(param_speeds[(param_cal_main[swap_idx])] == SPEED_10G && has_10g_port>4) ||param_speeds[(param_cal_main[swap_idx])] == SPEED_2p5G )  )) {
	   if (has_100g_port) { 
...
tdm_hx5_filter( tdm_mod_t *_tdm )
{
    ...
    tdm_hx5_filter_lr(_tdm);

    /* Smooth oversub port jitter */

    if (param_os_en==BOOL_TRUE && param_lr_en==BOOL_TRUE){
        tdm_hx5_osub_filter(_tdm,0);
        tdm_hx5_osub_filter(_tdm,0); /*Second time call to further smooth out OS clump that can occur with tdm_hx5_same_port_1n6_filter call */
    }
