...
_soc_buffer_ser_en_info_t _soc_buffer_ser_info[] = {
    { "CMIC BUFFER", IARB_SER_CONTROLr, CMIC_BUF_ECC_ENf, IARB_EN_COR_ERR_RPTr, CMIC_BUFFER_1BIT_ERROR_REPORTf },
    { "IARB LEARN FIFO", IARB_SER_CONTROLr, LEARN_FIFO_ECC_ENf, IARB_EN_COR_ERR_RPTr, LEARN_FIFO_1BIT_ERROR_REPORTf },
...
STATIC int
_soc_td2_reg_is_dyn(int unit, soc_reg_t reg)
{
                                  ...
                                  sal_usecs_t detect_time)
{
    ...
    if (reg != INVALIDr) {
        skip_pop = _soc_td2_reg_is_dyn(unit, reg) ||
...
_soc_trident2_process_ser_fifo(int unit, soc_block_t blk, int pipe, char *prefix_str)
{
    ...
    case SOC_BLK_IPIPE:
        ...
        if (SOC_IS_TD2P_TT2P(unit)) {
            ...
            ser_reg_info_table = _soc_bcm56860_a0_ip_reg_ser_info;
        } else
#endif
        {
            mask = pipe ? 0x15540 : 0xAAA0; /* All IP bits based upon pipe */
            ser_reg_info_table = ser_info_table = _soc_td2_ip_ser_info;
        }
    ...
    case SOC_BLK_EPIPE:
        ...
        if (SOC_IS_TD2P_TT2P(unit)) {
            ...
            ser_reg_info_table = _soc_bcm56860_a0_ep_reg_ser_info;
        } else
#endif
        {
            mem = pipe ? EGR_SER_FIFO_Ym : EGR_SER_FIFO_Xm;
            ser_reg_info_table = ser_info_table = _soc_td2_ep_ser_info;
        }
        ...
        if (soc_mem_field32_get(unit, mem, entry, VALIDf)) {
            ...
            if (regmem == _SOC_TD2_SER_MEM) {
                ...
                if (non_sbus == 0) {
                    ...
                    if (spci.mem != INVALIDm &&
                        SOC_MEM_SER_CORRECTION_TYPE(unit, spci.mem) != SOC_MEM_FLAG_SER_SPECIAL) {
                        spci.flags |= SOC_SER_LOG_WRITE_CACHE;
                    }
                    spci.log_id = _soc_td2_populate_ser_log(unit,
                                              parity_enable_reg,
                              ...
                              soc_field_t *ser_enable_field)
{
    ...
    _soc_td2_fifo_ser_info_t *fifo_ser_info = NULL;
#ifdef BCM_TRIDENT2PLUS_SUPPORT
    _soc_reg_ser_en_info_t *ser_en_info = NULL;
#endif
    ...
    if (SOC_IS_TD2P_TT2P(unit) || SOC_IS_APACHE(unit)) {
        ser_en_info = (_soc_reg_ser_en_info_t*)ser_en_info_list;
        for (i = 0; ser_en_info[i].reg != INVALIDr; i++) {
            if (ser_en_info[i].reg == reg) {
                *ser_control_reg = ser_en_info[i].en_ctrl.ctrl_type.en_reg;
                *ser_enable_field = ser_en_info[i].en_ctrl.en_fld;
                break;
            }
...
#endif
    {
        fifo_ser_info = (_soc_td2_fifo_ser_info_t*)ser_en_info_list;
        for (i = 0; fifo_ser_info[i].enable_reg != INVALIDr; i++) {
            if (fifo_ser_info[i].reg == reg) {
                *ser_control_reg = fifo_ser_info[i].enable_reg;
                *ser_enable_field = fifo_ser_info[i].enable_field;
                break;
            }
...
#endif /* (BCM_56850_A0) || (BCM_56860_A0) */
...
soc_trident2_chip_reset(int unit)
{
     ...
     * warmboot case taken care in _soc_trident2_misc_init() function
     */
...
STATIC int
soc_td2_ledup_chain_set(int unit)
{
    int phy_port;
    int pgw;
    int pos, uc;
    ...
    /* Build a LED chain hardware descriptor for TD2. */
    ...
    port_max = (TRIDENT2_PHY_PORT_MAX > (SOC_MAX_NUM_PORTS - 1)) ?
               (SOC_MAX_NUM_PORTS - 1) : TRIDENT2_PHY_PORT_MAX;

    for (phy_port = 1; phy_port <= port_max; phy_port++) {

        /* According to scan chain, set remap to reverse port order. */
        uc = (phy_port - 1) / 64;
        pos = (63 - ((phy_port - 1) % 64));
        ...
        if (pgw % 2) {
            ...
            if (lport >= 0 && si->port_num_lanes[lport] >= 10) {
                 pos += 4;
            } else if ((pos % 16) < 4) {
                 pos += 12;
            } else if ((pos % 16) < 8) {
 ...
 */
STATIC int
soc_td2p_ledup_chain_set(int unit)
{
    return soc_td2_ledup_chain_set(unit);
}
...
_soc_td2p_ledup_remap_setup(int unit, int init)
{
    ...
    return soc_td2p_ledup_chain_set(unit);
}
#endif /* BCM_TRIDENT2PLUS_SUPPORT */
...
{
#if defined(TD2P_LED_FLEXPORT_SUPPORT)
    if (SOC_IS_TD2P_TT2P(unit)) {
...
_soc_trident2_misc_init(int unit)
{
 ...
 #ifdef BCM_WARM_BOOT_SUPPORT
    /* Restoring MDIO mapping functions after warmboot */
    if (SOC_WARM_BOOT(unit)) {
        if (soc_property_get(unit, "td2_sbus_mdio", SOC_IS_TD2P_TT2P(unit))) {
            SOC_FUNCTIONS(unit)->soc_sbus_mdio_read = _soc_trident2_mdio_reg_read;
            SOC_FUNCTIONS(unit)->soc_sbus_mdio_write = _soc_trident2_mdio_reg_write;
...
STATIC int
soc_trident2_ser_single_bit_report_for_bus_buf_get(int unit, int *enable)
{
    int i;
    soc_reg_t ecc_reg;
    soc_field_t ecc_fld;
...
STATIC int
soc_trident2p_ser_single_bit_report_for_bus_buf_get(int unit, int *enable)
{
#ifdef BCM_56860_A0
    int i;
    soc_reg_t ecc_reg;
    ...
    bus_ser_info = _soc_ip_bus_ser_info;
...
int soc_td2x_ser_single_bit_report_for_bus_buf_get(int unit, int *enable)
{
    int rv = SOC_E_NONE;

#ifdef BCM_TRIDENT2PLUS_SUPPORT
    if (SOC_IS_TD2P_TT2P(unit)) {
        rv = soc_trident2p_ser_single_bit_report_for_bus_buf_get(unit, enable);
    } else
#endif
    {
        rv = soc_trident2_ser_single_bit_report_for_bus_buf_get(unit, enable);
    }
    return rv;
}
