...
/**
@name: tdm_td3_ovsb_gen_pm2pblk_map_flex
@param:
@desc: [chip_specific]
...
**/
static int
tdm_td3_ovsb_gen_pm2pblk_map_flex(tdm_mod_t *_tdm)
{
    int pm, pm_lo, pm_hi, spipe, pblk;
    int *pm2pblk_map = NULL, *pm2spipe_map = NULL;
    int prev_pm2pblk_map[TD3_NUM_PM_MOD];
    int pm_done[TD3_NUM_PM_MOD]= {0};
    int pm_en[TD3_NUM_PM_MOD]= {0};
    int prev_pm_en[TD3_NUM_PM_MOD]= {0};
    int prt, subp, spd, prt_lo, prt_hi;
    int phy_port, grp, grp_lo, grp_hi, pos, os_en;
    int pblk_idx_en[TD3_NUM_SPIPE][TD3_PBLK_CAL_LEN] = {{0}};
    int pblk_idx_cnt[TD3_NUM_SPIPE][TD3_PBLK_CAL_LEN] = {{0}};
    int param_token_empty;
    int prt_state;
    enum port_state_e *param_states_prev;
    tdm_calendar_t *cal;
    ...
    for (prt = prt_lo; prt <= prt_hi; prt++) {
        pm = tdm_td3_cmn_get_port_pm(_tdm, prt);
        spd = tdm_td3_cmn_get_port_speed(_tdm, prt);
        os_en = tdm_td3_cmn_chk_port_is_os(_tdm, prt);
        if (spd > 0 && os_en == BOOL_TRUE) {
            pm_en[pm] = 1;
        }
    }
    ...
    for (prt = prt_lo; prt <= prt_hi; prt++) {
        pm = tdm_td3_cmn_get_port_pm(_tdm, prt);
        prt_state = param_states_prev[prt - 1];
        if (prt_state == PORT_STATE__FLEX_OVERSUB_CH ||
            prt_state == PORT_STATE__FLEX_OVERSUB_DN) {
            prev_pm_en[pm] = 1;
        } else if (prt_state == PORT_STATE__FLEX_OVERSUB_UP) {
            for (subp = 0; subp < TD3_NUM_PM_LNS; subp++) {
                phy_port = pm * TD3_NUM_PM_LNS + subp + 1;
                spd = tdm_td3_cmn_get_port_speed(_tdm, phy_port);
                os_en = tdm_td3_cmn_chk_port_is_os(_tdm, phy_port);
                prt_state = param_states_prev[phy_port - 1];
                if (spd > 0 && os_en == BOOL_TRUE &&
                    prt_state != PORT_STATE__FLEX_OVERSUB_UP) {
                    prev_pm_en[pm] = 1;
                    break;
                }
            }
    ...
    TDM_PRINT1("TDM: [pm2pblk_map:flex] cal_id=%0d {pm:curr_pm_en} ",
               _tdm->_core_data.vars_pkg.cal_id);
    for (pm = pm_lo; pm <= pm_hi && pm < TD3_NUM_PM_MOD; pm++) {
        TDM_PRINT2("%2d:%2d ", pm, pm_en[pm]);
    }
    TDM_PRINT0("\n");

    TDM_PRINT1("TDM: [pm2pblk_map:flex] cal_id=%0d {pm:prev_pm_en} ",
               _tdm->_core_data.vars_pkg.cal_id);
    for (pm = pm_lo; pm <= pm_hi && pm < TD3_NUM_PM_MOD; pm++) {
        TDM_PRINT2("%2d:%2d ", pm, prev_pm_en[pm]);
    }
    TDM_PRINT0("\n\n");

    /* set pm2pblk_map[] for special PMs: ANY-TO-DOWN, and DOWN-TO-UP */
    TDM_PRINT1("TDM: [pm2pblk_map:flex] cal_id=%0d [before] {pm:pblk} ",
               _tdm->_core_data.vars_pkg.cal_id);
    for (pm = pm_lo; pm <= pm_hi && pm < TD3_NUM_PM_MOD; pm++) {
        TDM_PRINT2("%2d:%2d ", pm, pm2pblk_map[pm]);
    }
    TDM_PRINT0("\n");
    for (pm = pm_lo; pm <= pm_hi && pm < TD3_NUM_PM_MOD; pm++) {
        if (prev_pm_en[pm] == 0 && pm_en[pm] == 0) {
            pm2pblk_map[pm] = -1;
            TDM_PRINT1("TDM: [pm2pblk_map:flex] PM dn2dn, pm=%0d (non-flex)\n", pm);
        } else if (prev_pm_en[pm] == 1 && pm_en[pm] == 0) {
            pm2pblk_map[pm] = -1;
            TDM_PRINT1("TDM: [pm2pblk_map:flex] PM up2dn, pm=%0d\n", pm);
        } else if (prev_pm_en[pm] == 0 && pm_en[pm] == 1) {
            pm2pblk_map[pm] = -1;
            TDM_PRINT1("TDM: [pm2pblk_map:flex] PM dn2up, pm=%0d\n", pm);
        }
    }
    TDM_PRINT1("TDM: [pm2pblk_map:flex] cal_id=%0d [middle] {pm:pblk} ",
               _tdm->_core_data.vars_pkg.cal_id);
    for (pm = pm_lo; pm <= pm_hi && pm < TD3_NUM_PM_MOD; pm++) {
        TDM_PRINT2("%2d:%2d ", pm, pm2pblk_map[pm]);
    }
    ...
    /* record prev_pm2pblk_map[] */
    for (pm = pm_lo; pm <= pm_hi && pm < TD3_NUM_PM_MOD; pm++) {
        prev_pm2pblk_map[pm] = pm2pblk_map[pm];
    }

    /* change-port */
    for (prt = prt_lo; prt <= prt_hi; prt++) {
        if (param_states_prev[prt - 1] == PORT_STATE__FLEX_OVERSUB_CH) {
            pm = tdm_td3_cmn_get_port_pm(_tdm, prt);
            pm_done[pm] = 1;
            TDM_PRINT2("TDM: [pm2pblk_map:flex] port ch, pm=%0d, prt=%0d\n",
                       pm, prt);
        }
    }
    ...
    for (prt = prt_lo; prt <= prt_hi; prt++) {
        pm = tdm_td3_cmn_get_port_pm(_tdm, prt);
        if (param_states_prev[prt - 1] == PORT_STATE__FLEX_OVERSUB_DN) {
            if (prev_pm_en[pm] == 1 && pm_en[pm] == 0) {
                pm_done[pm] = 1;
            }
            TDM_PRINT2("TDM: [pm2pblk_map:flex] port dn, pm=%0d, prt=%0d\n",
                       pm, prt);
        }
    }
    ...
    for (prt = prt_lo; prt <= prt_hi; prt++) {
        pm = tdm_td3_cmn_get_port_pm(_tdm, prt);
        if (param_states_prev[prt - 1] == PORT_STATE__FLEX_OVERSUB_UP) {
            if (prev_pm_en[pm] == 1 && pm_en[pm] == 1) {
                pm_done[pm] = 1;
            }
            TDM_PRINT2("TDM: [pm2pblk_map:flex] port up, pm=%0d, prt=%0d\n",
                       pm, prt);
        }
    }
    for (spipe = 0; spipe < TD3_NUM_SPIPE; spipe++) {
        ...
        for (grp = grp_lo; grp <= grp_hi; grp++) {
            for (pos = 0; pos < cal->grp_len; pos++) {
                ...
                if (prt != param_token_empty) {
                    pm = tdm_td3_cmn_get_port_pm(_tdm, prt);
                    pblk = pm2pblk_map[pm];
                    if (pblk >= 0 && pblk < TD3_PBLK_CAL_LEN) {
                        pblk_idx_en[spipe][pblk] = 1;
                    }
                }
            }
        }
    ...
    for (prt = prt_lo; prt <= prt_hi; prt++) {
        if (param_states_prev[prt - 1] == PORT_STATE__FLEX_OVERSUB_UP) {
            pm = tdm_td3_cmn_get_port_pm(_tdm, prt);
            if (pm_done[pm] == 1) continue;
            TDM_PRINT2("TDM: [pm2pblk_map:flex] PM up, pm=%0d, prt=%0d\n",
                       pm, prt);
            spipe = pm2spipe_map[pm];
            if (spipe >= 0 && spipe < TD3_NUM_SPIPE) {
                for (pblk = 0; pblk < TD3_PBLK_CAL_LEN; pblk++) {
                    if (pblk_idx_en[spipe][pblk] == 0) {
                        pm2pblk_map[pm] = pblk;
                        pm_done[pm] = 1;
                        pblk_idx_en[spipe][pblk] = 1;
                        break;
    ...
    TDM_PRINT1("TDM: [pm2pblk_map:flex] cal_id=%0d [after ] {pm:pblk} ",
               _tdm->_core_data.vars_pkg.cal_id);
    for (pm = pm_lo; pm <= pm_hi && pm < TD3_NUM_PM_MOD; pm++) {
        TDM_PRINT2("%2d:%2d ", pm, pm2pblk_map[pm]);
    }
    ...
    /* self check */
    for (pm = pm_lo; pm <= pm_hi && pm < TD3_NUM_PM_MOD; pm++) {
        if (prev_pm2pblk_map[pm] >= 0 && pm2pblk_map[pm] >= 0) {
            if (prev_pm2pblk_map[pm] != pm2pblk_map[pm]) {
                TDM_ERROR4("%s pm=%0d, prev_pblk=%0d, curr_pblk=%0d\n",
                          "TDM: [pm2pblk_map:flex] pblk index shift.",
                          pm, prev_pm2pblk_map[pm], pm2pblk_map[pm]);
            }
        }
    }
    for (pm = pm_lo; pm <= pm_hi && pm < TD3_NUM_PM_MOD; pm++) {
        ...
        pblk = pm2pblk_map[pm];
        if (spipe >= 0 && spipe < TD3_NUM_SPIPE &&
            pblk >=0 && pblk < TD3_PBLK_CAL_LEN) {
            pblk_idx_cnt[spipe][pblk]++;
            if (pblk_idx_cnt[spipe][pblk] > 1) {
                TDM_ERROR5("%s hpipe=%0d, pm=%0d, pblk=%0d, pblk_cnt=%0d\n",
                           "TDM: [pm2pblk_map:flex] invalid pblk index.",
                           spipe, pm, pm2pblk_map[pm],
                           pblk_idx_cnt[spipe][pblk]);
            }
        }
    }
...
/**
@name: tdm_td3_ovsb_gen_pm2pblk_map_init
@param:
@desc: [chip_specific]
...
**/
int
tdm_td3_ovsb_gen_pm2pblk_map_init(tdm_mod_t *_tdm)
{
    ...
    TDM_PRINT1("TDM: [pm2pblk_map:init] cal_id=%0d [before] {pm:pblk} ",
               _tdm->_core_data.vars_pkg.cal_id);
    for (pm = pm_lo; pm <= pm_hi && pm < TD3_NUM_PM_MOD; pm++) {
        TDM_PRINT2("%2d:%2d ", pm, pm2pblk_map[pm]);
    }
    ...
    TDM_PRINT1("TDM: [pm2pblk_map:init] cal_id=%0d [after ] {pm:pblk} ",
               _tdm->_core_data.vars_pkg.cal_id);
    for (pm = pm_lo; pm <= pm_hi && pm < TD3_NUM_PM_MOD; pm++) {
        TDM_PRINT2("%2d:%2d ", pm, pm2pblk_map[pm]);
    }
...
{
    if (_tdm->_chip_data.soc_pkg.flex_port_en == 1) {
        return (tdm_td3_ovsb_gen_pm2pblk_map_flex(_tdm));
    } else {
        return (tdm_td3_ovsb_gen_pm2pblk_map_init(_tdm));
    }
