...
#define SOC_PBMP_PORT_256_MAX 256
#define SOC_PBMP_WIDTH_256      (((SOC_PBMP_PORT_256_MAX + 32 - 1)/32)*32)
#define SOC_PBMP_WORD_256_MAX           \
        ((SOC_PBMP_WIDTH_256 + _SHR_PBMP_WORD_WIDTH-1) / _SHR_PBMP_WORD_WIDTH)

/* port bitmap 256 bits wide for legacy code */
typedef struct _shr_pbmp_256 {
        uint32  pbits[SOC_PBMP_WORD_256_MAX];
} pbmp_256_t;

#undef SOC_PBMP_PORT_256_MAX
#undef SOC_PBMP_WIDTH_256
#undef SOC_PBMP_WORD_256_MAX

typedef struct _soc_td3_asf_ctrl_256_s {
    uint8                         init;
    pbmp_256_t                    asf_ports;
    pbmp_256_t                    pause_restore;
    pbmp_256_t                    asf_ss;
    pbmp_256_t                    asf_sf;
    pbmp_256_t                    asf_fs;
    soc_td3_asf_mem_profile_e asf_mem_profile;
} _soc_td3_asf_ctrl_256_t;
...
/* Legacy ASF Warmboot */
typedef struct _soc_td3_asf_wb_256_s {
    int unit;
    _soc_td3_asf_ctrl_256_t asf_256_ctrl;
} _soc_td3_asf_wb_256_t;
#endif
    ...
    uint8   ca_thresh)
{
    ...
    uint32 idb_obm_ca_ct_ctrl_reg[][2] = {
           ...
           ,{IDB_OBM16_CA_CT_CONTROL_PIPE0r, -1},
           {IDB_OBM17_CA_CT_CONTROL_PIPE0r, -1},
           {IDB_OBM18_CA_CT_CONTROL_PIPE0r, -1},
           {IDB_OBM19_CA_CT_CONTROL_PIPE0r, -1}
#endif
        };
    ...
    if (SOC_IS_MAVERICK2(unit)) {
          SOC_IF_ERROR_RETURN
            (soc_maverick2_port_obm_info_get(unit, port, &obm, &lane));
       if ((obm >= _MV2_PBLKS_PER_PIPE) ||
              (lane < 0 || lane >= _MV2_PORTS_PER_PBLK)) {
              return SOC_E_PARAM;
       }
    } else
#endif
...
#ifdef BCM_MAVERICK2_SUPPORT
           ,{IDB_OBM16_CONTROL_PIPE0r, -1},
           {IDB_OBM17_CONTROL_PIPE0r, -1},
           {IDB_OBM18_CONTROL_PIPE0r, -1},
           {IDB_OBM19_CONTROL_PIPE0r, -1}
#endif
        };
...
#ifdef BCM_MAVERICK2_SUPPORT
           ,{IDB_OBM16_CT_THRESHOLD_PIPE0r, -1},
           {IDB_OBM17_CT_THRESHOLD_PIPE0r, -1},
           {IDB_OBM18_CT_THRESHOLD_PIPE0r, -1},
           {IDB_OBM19_CT_THRESHOLD_PIPE0r, -1}
#endif
        };
...
#ifdef BCM_MAVERICK2_SUPPORT
           ,{IDB_OBM16_DBG_A_PIPE0r, -1},
           {IDB_OBM17_DBG_A_PIPE0r, -1},
           {IDB_OBM18_DBG_A_PIPE0r, -1},
           {IDB_OBM19_DBG_A_PIPE0r, -1}
#endif
        };
    ...
    uint16            scache_ver)
{
    ...
    *mem_sz = 0;
    ...
    /* scache_ver is passed in from port, thus it depends on the port
     * wb version
     */
    if (scache_ver <= BCM_WB_VERSION_1_13 ) {
        /* The contents of prior versions of scache for asf were stored in
         * a structure, so recover it this time */
        *mem_sz += sizeof(_soc_td3_asf_wb_256_t);
    } else {
        /* Get the scache dictionary value for the max number of ports
         * in a port bitmap. The default value, if it is not found, is
         * SOC_PBMP_PORT_MAX
         */
        pbmp_port_max = soc_scache_dictionary_entry_get(unit,
                ssden_SOC_PBMP_PORT_MAX,
                SOC_PBMP_PORT_MAX);
        pbmp_size_bytes = SHR_BITALLOCSIZE(pbmp_port_max);

        /* Going forward, the contents of scache for asf will be stored
         * individually, not in structure format */
        *mem_sz += sizeof(int);     /* _soc_td3_asf_wb_t.unit */
        *mem_sz += sizeof(uint8);   /* _soc_td3_asf_wb_t.asf_ctrl.init */
        *mem_sz += pbmp_size_bytes; /* _soc_td3_asf_wb_t.asf_ctrl.asf_ports */
        *mem_sz += pbmp_size_bytes; /* _soc_td3_asf_wb_t.asf_ctrl.pause_restore */
        *mem_sz += pbmp_size_bytes; /* _soc_td3_asf_wb_t.asf_ctrl.asf_ss */
        *mem_sz += pbmp_size_bytes; /* _soc_td3_asf_wb_t.asf_ctrl.asf_sf */
        *mem_sz += pbmp_size_bytes; /* _soc_td3_asf_wb_t.asf_ctrl.asf_fs */
                                    /* _soc_td3_asf_wb_t.asf_mem_profile */
        *mem_sz += sizeof(soc_td3_asf_mem_profile_e);
    }
    ...
    IN_OUT uint8* const wb_data)
{
    ...
    /* Sync each element of the struct individually */
    sal_memcpy(scache, &unit, sizeof(int));
    scache += sizeof(int);
    sal_memcpy(scache, &_soc_td3_asf_ctrl[unit]->init,
                    sizeof(_soc_td3_asf_ctrl[unit]->init));
    scache += sizeof(_soc_td3_asf_ctrl[unit]->init);
    sal_memcpy(scache, &_soc_td3_asf_ctrl[unit]->asf_ports,
                    sizeof(_soc_td3_asf_ctrl[unit]->asf_ports));
    scache += sizeof(_soc_td3_asf_ctrl[unit]->asf_ports);
    sal_memcpy(scache, &_soc_td3_asf_ctrl[unit]->pause_restore,
                    sizeof(_soc_td3_asf_ctrl[unit]->pause_restore));
    scache += sizeof(_soc_td3_asf_ctrl[unit]->pause_restore);
    sal_memcpy(scache, &_soc_td3_asf_ctrl[unit]->asf_ss,
                    sizeof(_soc_td3_asf_ctrl[unit]->asf_ss));
    scache += sizeof(_soc_td3_asf_ctrl[unit]->asf_ss);
    sal_memcpy(scache, &_soc_td3_asf_ctrl[unit]->asf_sf,
                    sizeof(_soc_td3_asf_ctrl[unit]->asf_sf));
    scache += sizeof(_soc_td3_asf_ctrl[unit]->asf_sf);
    sal_memcpy(scache, &_soc_td3_asf_ctrl[unit]->asf_fs,
                    sizeof(_soc_td3_asf_ctrl[unit]->asf_fs));
    scache += sizeof(_soc_td3_asf_ctrl[unit]->asf_fs);
    sal_memcpy(scache, &_soc_td3_asf_ctrl[unit]->asf_mem_profile,
                    sizeof(_soc_td3_asf_ctrl[unit]->asf_mem_profile));
    ...
    uint16       scache_ver)
{
     ...
     */
    if (scache_ver <= BCM_WB_VERSION_1_13 ) {
        /* There is a struct in scache. Copy it into the current data */
        _soc_td3_asf_wb_256_t *asf_wb_256 = (_soc_td3_asf_wb_256_t *)wb_data;

        if (asf_wb_256->unit == unit) {
            if (!(_soc_td3_asf_ctrl[unit] =
                        sal_alloc(sizeof(_soc_td3_asf_ctrl_t), "TD3 ASF Ctrl Area"))) {
                return SOC_E_MEMORY;
            }

            /* The passed in scache ponter points to a structure of type
             * _soc_td3_asf_wb_256_t, which is how it was written to scache, so copy
             * each element of that structure into the currently sized structure */
            sal_memcpy(&(_soc_td3_asf_ctrl[unit]->init),
                       &(asf_wb_256->asf_256_ctrl.init),
                       sizeof(asf_wb_256->asf_256_ctrl.init));
            sal_memcpy(&(_soc_td3_asf_ctrl[unit]->asf_ports),
                       &(asf_wb_256->asf_256_ctrl.asf_ports),
                       sizeof(asf_wb_256->asf_256_ctrl.asf_ports));
            sal_memcpy(&(_soc_td3_asf_ctrl[unit]->pause_restore),
                       &(asf_wb_256->asf_256_ctrl.pause_restore),
                       sizeof(asf_wb_256->asf_256_ctrl.pause_restore));
            sal_memcpy(&(_soc_td3_asf_ctrl[unit]->asf_ss),
                       &(asf_wb_256->asf_256_ctrl.asf_ss),
                       sizeof(asf_wb_256->asf_256_ctrl.asf_ss));
            sal_memcpy(&(_soc_td3_asf_ctrl[unit]->asf_sf),
                       &(asf_wb_256->asf_256_ctrl.asf_sf),
                       sizeof(asf_wb_256->asf_256_ctrl.asf_sf));
            sal_memcpy(&(_soc_td3_asf_ctrl[unit]->asf_fs),
                       &(asf_wb_256->asf_256_ctrl.asf_fs),
                       sizeof(asf_wb_256->asf_256_ctrl.asf_fs));
            sal_memcpy(&(_soc_td3_asf_ctrl[unit]->asf_mem_profile),
                       &(asf_wb_256->asf_256_ctrl.asf_mem_profile),
                       sizeof(asf_wb_256->asf_256_ctrl.asf_mem_profile));
        }
    } else {
        /* Scache was written individually.  Recover each element individually,
         * based on the sizes referenced in the scache dictionary */
        int pbmp_port_max;
        int pbmp_size_bytes;
        int recovered_unit;
        uint8 *scache = wb_data;
        ...
        sal_memcpy(&recovered_unit, scache, sizeof(int));
        scache += sizeof(int);

        if (recovered_unit == unit) {
            if (!(_soc_td3_asf_ctrl[unit] =
                        sal_alloc(sizeof(_soc_td3_asf_ctrl_t), "TD3 ASF Ctrl Area"))) {
                return SOC_E_MEMORY;
            }
            ...
            scache += sizeof(_soc_td3_asf_ctrl[unit]->init);
            sal_memcpy(&(_soc_td3_asf_ctrl[unit]->asf_ports),
                       scache, SOC_SCACHE_SIZE_MIN(sizeof(pbmp_t), pbmp_size_bytes));
            scache += pbmp_size_bytes;
            sal_memcpy(&(_soc_td3_asf_ctrl[unit]->pause_restore),
                       scache, SOC_SCACHE_SIZE_MIN(sizeof(pbmp_t), pbmp_size_bytes));
            scache += pbmp_size_bytes;
            sal_memcpy(&(_soc_td3_asf_ctrl[unit]->asf_ss),
            ...
            scache += pbmp_size_bytes;
            sal_memcpy(&(_soc_td3_asf_ctrl[unit]->asf_mem_profile),
                       scache, sizeof(_soc_td3_asf_ctrl[unit]->asf_mem_profile));
        }
    }
