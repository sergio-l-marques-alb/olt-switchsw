...
int tdm_hx5_cmn_get_port_pm_chk(int port_token, tdm_mod_t *_tdm);
/**
@name: tdm_hx5_chk_get_speed_slots_5G
@param:

Return the number of slots (5G per slot) of the given speed
**/
int 
tdm_hx5_chk_get_speed_slots_5G(enum port_speed_e port_speed)
{
    int slot_num;
    
    slot_num = tdm_hx5_cmn_get_speed_slots(port_speed);
    if (slot_num > 1) {
        slot_num /= 2;
    }
...
/**
@name: tdm_hx5_chk_get_pipe_token_cnt
@param:

Return number of slots of the given token in selected calendar
**/
int 
tdm_hx5_chk_get_pipe_token_cnt(tdm_mod_t *_tdm, int cal_id, int port_token)
{
    int i, cnt=0;
    int param_cal_len;
    int *param_cal_main=NULL;
    
    param_cal_len = tdm_hx5_cmn_get_pipe_cal_len(cal_id, _tdm);
...
/**
@name: tdm_hx5_chk_get_port_lanes
@param:

Return number of lanes of the given port
**/
int 
tdm_hx5_chk_get_port_lanes(tdm_mod_t *_tdm, int port)
{
    ...
    if (port >= param_phy_lo && port <= param_phy_hi) {
        ...
        if (param_speeds[port] == SPEED_40G || param_speeds[port] == SPEED_42G) {
            port_start = ((port - 1) / HX5_NUM_PM_LNS) * HX5_NUM_PM_LNS + 1;
            if (port_start == port &&
...
/**
@name: tdm_hx5_chk_struct_os_ratio
@param:

Check oversub ratio for each pipe and/or half-pipe
**/
int 
tdm_hx5_chk_struct_os_ratio(tdm_mod_t *_tdm, int cal_id, int os_ratio_limit)
{
        ...
        for (port_phy=phy_lo; port_phy<=phy_hi; port_phy++) {
            ...
            if (port_state==PORT_STATE__OVERSUB    ||
                ...
                if (port_phy>port_phy_mid) {
                    hp0_os_bw_req += tdm_hx5_cmn_get_speed_slots(port_speed);
                } else {
                    hp1_os_bw_req += tdm_hx5_cmn_get_speed_slots(port_speed);
                }
            }
        }
        /* os_bw_max */
...
/**
@name: tdm_hx5_chk_struct_lr_bw
@param:

Check overall bandwidth for linerate ports for the given pipe (for DV only)
**/
int 
tdm_hx5_chk_struct_lr_bw(tdm_mod_t *_tdm, int cal_id)
{
    int port_speed, port_state, result=PASS;
    int port_phy, phy_lo, phy_hi;
    int lr_slot_req = 0, lr_slot_limit;
    int param_phy_lo, param_phy_hi, param_mgmt_mode;
    enum port_speed_e *param_speeds;
    enum port_state_e *param_states;
    
    param_phy_lo    = _tdm->_chip_data.soc_pkg.soc_vars.fp_port_lo;
    param_phy_hi    = _tdm->_chip_data.soc_pkg.soc_vars.fp_port_hi;
    param_mgmt_mode = _tdm->_chip_data.soc_pkg.soc_vars.hx5.mgmt_mode;
    param_speeds    = _tdm->_chip_data.soc_pkg.speed;
    param_states    = _tdm->_chip_data.soc_pkg.state;

    COMPILER_REFERENCE(param_mgmt_mode);
    tdm_hx5_cmn_get_pipe_port_range(cal_id, &phy_lo, &phy_hi);
    if (phy_lo>=param_phy_lo && phy_hi<=param_phy_hi) {
        for (port_phy=phy_lo; port_phy<=phy_hi; port_phy++) {
            port_state = param_states[port_phy-1];
            ...
            if (port_state==PORT_STATE__LINERATE    ||
                ...
                lr_slot_req += tdm_hx5_cmn_get_speed_slots(port_speed);
            }
        }
        if (lr_slot_req>0){
            lr_slot_limit = tdm_hx5_cmn_get_pipe_cal_len(cal_id, _tdm) - HX5_NUM_ANCL ;
...
**/
int
tdm_hx5_chk_struct(tdm_mod_t *_tdm)
{
    ...
    for (cal_id=0; cal_id<8; cal_id++){
        if (cal_id == HX5_IDB_PIPE_2_CAL_ID ||
            cal_id == HX5_IDB_PIPE_3_CAL_ID ||
            cal_id == HX5_IDB_PIPE_1_CAL_ID ||
            cal_id == HX5_MMU_PIPE_1_CAL_ID || 
            cal_id == HX5_MMU_PIPE_2_CAL_ID || 
            cal_id == HX5_MMU_PIPE_3_CAL_ID) {
            continue;
        }
        cal_len_act = tdm_hx5_cmn_get_pipe_cal_len_max(cal_id, _tdm);
        cal_len_limit = tdm_hx5_cmn_get_pipe_cal_len(cal_id, _tdm);
        TDM_SEL_CAL(cal_id, param_cal_main);
    ...
    if (PASS!=tdm_hx5_chk_struct_os_ratio(_tdm, HX5_IDB_PIPE_0_CAL_ID, 2) ||   /* IDB pipe 0 */
        PASS!=tdm_hx5_chk_struct_os_ratio(_tdm, HX5_MMU_PIPE_0_CAL_ID, 2))  {  /* MMU pipe 0 */
        result = FAIL;
    }
    /* linerate bandwidth */
    if (PASS!=tdm_hx5_chk_struct_lr_bw(_tdm, HX5_IDB_PIPE_0_CAL_ID)) {   /* IDB pipe 0 */
        result = FAIL;
    }
     /* linerate bandwidth */
    if (PASS!=tdm_hx5_chk_struct_lr_bw(_tdm, HX5_MMU_PIPE_0_CAL_ID)) {   /* MMU  pipe 0 */
        result = FAIL;
    }
    ...
    for (port_phy=param_phy_lo; port_phy<=param_phy_hi; port_phy++) {
        ...
        if ((port_speed > SPEED_0) && 
            (port_state == PORT_STATE__LINERATE    || 
             ...
             port_state == PORT_STATE__OVERSUB_HG  ||
             port_state == PORT_STATE__MANAGEMENT ) ) {
            port_tsc = tdm_hx5_cmn_get_port_pm(port_phy, _tdm);
            if (port_tsc<param_pm_num) {
                port_lanes_exp = tdm_hx5_cmn_get_speed_lanes(port_speed);
                port_lanes = 0;
...
/**
@name: tdm_hx5_chk_sister
@param:

Check sister port spacing constraint
**/
int 
tdm_hx5_chk_sister(tdm_mod_t *_tdm)
{
    ...
    for (cal_id=0; cal_id<8; cal_id++){
        ...
        for (i=0; i<param_cal_len; i++){
            ...
            if (port_i>=param_phy_lo && port_i<=param_phy_hi) {
                tsc_i = tdm_hx5_cmn_get_port_pm_chk(port_i, _tdm);
		/* just setting parameters for falcon to be 2 and eagle as 4 */
		if(tsc_i < 16)
		param_sister_min = HX5_MIN_SPACING_SISTER_EAGLE_PORT;
		else
		param_sister_min = HX5_MIN_SPACING_SISTER_FALCON_PORT;

                for (j=1; j<param_sister_min; j++){
                    k = (i+j)%param_cal_len;
                    port_k = param_cal_main[k];
                    if (port_k>=param_phy_lo && port_k<=param_phy_hi) {
                        tsc_k = tdm_hx5_cmn_get_port_pm_chk(port_k, _tdm);
                        if (tsc_i == tsc_k &&
...
/**
@name: tdm_hx5_chk_same (Version 2.0)
@param:

Check same port spacing constraint
**/
int 
tdm_hx5_chk_same(tdm_mod_t *_tdm)
{
    ...
    for (cal_id=0; cal_id<8; cal_id++) {
        ...
        for (i=0; i<param_cal_len; i++) {
            ...
            if (port_i>=param_phy_lo && port_i<=param_phy_hi) {
                ...
                if((cal_id == HX5_MMU_PIPE_0_CAL_ID  
                    ) &&
                   (port_speed == SPEED_10G ||
                    port_speed == SPEED_11G ||
                    port_speed == SPEED_25G ||
                    port_speed == SPEED_27G)) {
                    param_same_min = HX5_MIN_SPACING_SAME_PORT_LSP;
                } else {
                    param_same_min = HX5_MIN_SPACING_SAME_PORT_HSP;
                }
...
/**
@name: tdm_hx5_chk_bandwidth_ancl
@param:
...
**/
int 
tdm_hx5_chk_bandwidth_ancl(tdm_mod_t *_tdm)
{
    int cal_id, slot_req, slot_cnt, slot_token,
        slot_cpu,  token_cpu,
        slot_lpbk, token_lpbk,
        slot_opt1, token_opt1,token_fae,
        slot_null, token_null,
        slot_idle, token_idle,slot_fae,
        result=PASS;
    int param_token_cmic, param_token_null,
        param_token_idl1, param_token_idl2, 
        param_token_lpb0, param_token_fae,
        param_token_ext, param_mgmt_mode;
    enum port_speed_e *param_speeds;
    
    param_token_cmic= HX5_CMIC_TOKEN;
    param_token_idl1= HX5_IDL1_TOKEN;
    param_token_idl2= HX5_IDL2_TOKEN;
    param_token_null= HX5_NULL_TOKEN;
    param_token_lpb0= HX5_LPB0_TOKEN;
    param_token_fae   = HX5_FAE_TOKEN ;
    param_token_ext = _tdm->_chip_data.soc_pkg.num_ext_ports;
    param_mgmt_mode = _tdm->_chip_data.soc_pkg.soc_vars.hx5.mgmt_mode;
    param_speeds    = _tdm->_chip_data.soc_pkg.speed;
    
    COMPILER_REFERENCE(param_speeds);
    COMPILER_REFERENCE(param_mgmt_mode);
    
    for (cal_id=0; cal_id<8; cal_id++) {
        if (cal_id == HX5_IDB_PIPE_0_CAL_ID) {
            /* IDB 0 */
            slot_cpu  = HX5_NUM_ANCL_CPU;      token_cpu  = param_token_cmic;
            slot_lpbk = HX5_NUM_ANCL_LPBK;     token_lpbk = param_token_lpb0;
            slot_fae = HX5_NUM_ANCL_FAE  ;     token_fae = param_token_fae;
            slot_opt1 = HX5_NUM_ANCL_IDB_OPT1; token_opt1 = param_token_idl1;
            slot_null = HX5_NUM_ANCL_IDB_NULL; token_null = param_token_null;
            slot_idle = HX5_NUM_ANCL_IDB_IDLE; token_idle = param_token_idl2;
        }  else if (cal_id == HX5_MMU_PIPE_0_CAL_ID) {
            /* MMU 0 */
            slot_cpu  = HX5_NUM_ANCL_CPU;      token_cpu  = param_token_cmic;
            slot_lpbk = HX5_NUM_ANCL_LPBK;     token_lpbk = param_token_lpb0;
            slot_fae = HX5_NUM_ANCL_FAE  ;     token_fae = param_token_fae;
            slot_opt1 = HX5_NUM_ANCL_MMU_OPT1; token_opt1 = param_token_ext;
            slot_null = HX5_NUM_ANCL_MMU_NULL; token_null = param_token_null;
            slot_idle = HX5_NUM_ANCL_MMU_IDLE; token_idle = param_token_idl2;
        }
	 else {
            slot_cpu  = 0; token_cpu  = param_token_ext;
	    slot_fae = 0; token_fae    = param_token_fae;
        ...
        if (slot_cpu>0) {
            ...
            slot_req   = slot_cpu;
            slot_cnt   = tdm_hx5_chk_get_pipe_token_cnt(_tdm, cal_id, slot_token);
...
/**
@name: tdm_hx5_chk_bandwidth_lr_pipe
@param:

Check bandwidth for linerate ports for the given pipe
**/
int 
tdm_hx5_chk_bandwidth_lr_pipe(tdm_mod_t *_tdm, int cal_id)
{
    ...
    if (phy_lo>=param_phy_lo && phy_hi<=param_phy_hi) {
        for (port_phy=phy_lo; port_phy<=phy_hi; port_phy++) {
            ...
            if (port_state==PORT_STATE__LINERATE    ||
                port_state==PORT_STATE__LINERATE_HG ){
                slot_req = tdm_hx5_cmn_get_speed_slots(port_speed);
                slot_cnt = tdm_hx5_chk_get_pipe_token_cnt(_tdm, cal_id, port_phy);
                if (slot_cnt<slot_req){
                    result = FAIL;
...
/**
@name: tdm_hx5_chk_bandwidth_os_pipe
@param:

Check bandwidth for Oversub ports for the given pipe
**/
int 
tdm_hx5_chk_bandwidth_os_pipe(tdm_mod_t *_tdm, int cal_id)
{
            ...
            for (j=1; j<param_cal_grp_len; j++){
                ...
                if (port_phy>=param_phy_lo && port_phy<=param_phy_hi) {
                    ...
                    if (grp_speed == SPEED_0) {
                        ...
                        grp_speed_slots = tdm_hx5_cmn_get_speed_slots(grp_speed);
                    }
                    if ( (grp_speed==SPEED_20G  || grp_speed==SPEED_40G) && 
                        ...
                        port_speed_slots = tdm_hx5_cmn_get_speed_slots(port_speed);
...
**/
int 
tdm_hx5_chk_bandwidth_lr(tdm_mod_t *_tdm)
{
    int result=PASS;
    
    /* IDB pipe 0 */
    if (tdm_hx5_chk_bandwidth_lr_pipe(_tdm, HX5_IDB_PIPE_0_CAL_ID) != PASS) {
        result = FAIL;
    }
    /* IDB pipe 1 
    if (tdm_hx5_chk_bandwidth_lr_pipe(_tdm, HX5_IDB_PIPE_1_CAL_ID) != PASS) {
        result = FAIL;
    }*/
    /* MMU pipe 0 */
    if ( tdm_hx5_chk_bandwidth_lr_pipe(_tdm, HX5_MMU_PIPE_0_CAL_ID) != PASS) {
        result = FAIL;
    }
    /* MMU pipe 1 
    if (tdm_hx5_chk_bandwidth_lr_pipe(_tdm, HX5_MMU_PIPE_1_CAL_ID) != PASS) {
        result = FAIL;
    }
    */
    return result;
...
**/
int 
tdm_hx5_chk_bandwidth_os(tdm_mod_t *_tdm)
{
    int result=PASS;
    
    /* IDB pipe 0 */
    if (tdm_hx5_chk_bandwidth_os_pipe(_tdm, HX5_IDB_PIPE_0_CAL_ID) != PASS) {
        result = FAIL;
    }
    /* IDB pipe 1 
    if (tdm_hx5_chk_bandwidth_os_pipe(_tdm, HX5_IDB_PIPE_1_CAL_ID) != PASS) {
        result = FAIL;
    }
    */
...
/**
@name: tdm_hx5_chk_bandwidth
@param:

Verifies IDB/MMU calendars allocating enough slots for port bandwidth
**/
int 
tdm_hx5_chk_bandwidth(tdm_mod_t *_tdm)
{
    int result=PASS;
    
    /* Ancillary ports */
    if (tdm_hx5_chk_bandwidth_ancl(_tdm)!=PASS){
        result = FAIL;
    }
    /* Linerate ports */
    if(tdm_hx5_chk_bandwidth_lr(_tdm)!=PASS){
        result = FAIL;
    }
    /* Oversub ports */
    if(tdm_hx5_chk_bandwidth_os(_tdm)!=PASS){
        result = FAIL;
    }
...
/**
@name: tdm_hx5_chk_jitter_cmic
@param:

Checks distribution of cmic port for the given pipe
**/
int
tdm_hx5_chk_jitter_cmic(tdm_mod_t *_tdm, int cal_id)
{
    ...
    param_cal_len = tdm_hx5_cmn_get_pipe_cal_len(cal_id, _tdm);
	TDM_SEL_CAL(cal_id, param_cal_main);
	cmic_num = tdm_hx5_chk_get_pipe_token_cnt(_tdm, cal_id, HX5_CMIC_TOKEN);
	switch(cmic_num) {
		case 4: 
			port_speed = SPEED_10G;
			break;
    ...
    tdm_hx5_cmn_get_speed_jitter(port_speed, param_cal_len, &space_min, &space_max);
	for(i = 0; i < param_cal_len; i++) {
		if(param_cal_main[i] != HX5_CMIC_TOKEN) {
            continue;
        }
		for(j=1; j< param_cal_len;j++) {
			if(param_cal_main[(i+j)%param_cal_len] == HX5_CMIC_TOKEN) {
...
/**
@name: tdm_hx5_chk_jitter_lr_pipe
@param:

Checks distribution of linerate slots of the same port for the given pipe
**/
int 
tdm_hx5_chk_jitter_lr_pipe(tdm_mod_t *_tdm, int cal_id)
{
                ...
                for (k=1; k<param_cal_len; k++) {
                    ...
                    if (param_cal_main[pos_curr] == port_phy) {
                        ...
                        if (port_space > space_max) {
                            if (port_speed<=SPEED_25G) { /* Low speed port: 10G, 20G, 25G */
                                ...
                                if (port_space > (space_max+HX5_LR_CHK_TRHD)) {
                                    result = FAIL;
...
/**
@name: tdm_hx5_chk_jitter_lr
@param:

Checks distribution of linerate tokens for IDB/MMU calendars
**/
int 
tdm_hx5_chk_jitter_lr(tdm_mod_t *_tdm)
{
    int result=PASS;
    
    /* IDB pipe 0 */
    if (tdm_hx5_chk_jitter_lr_pipe(_tdm, HX5_IDB_PIPE_0_CAL_ID) != PASS) {
        result = FAIL;
    }
    /* IDB pipe 1 
    if (tdm_hx5_chk_jitter_lr_pipe(_tdm, HX5_IDB_PIPE_1_CAL_ID) != PASS) {
        result = FAIL;
    }
    */
    /* MMU pipe 0 */
    if (tdm_hx5_chk_jitter_lr_pipe(_tdm, HX5_MMU_PIPE_0_CAL_ID) != PASS) {
        result = FAIL;
    }
    /* MMU pipe 1 
    if (tdm_hx5_chk_jitter_lr_pipe(_tdm, HX5_MMU_PIPE_1_CAL_ID) != PASS) {
        result = FAIL;
    }
    */
...
tdm_hx5_chk_os_halfpipe(tdm_mod_t *_tdm, int cal_id)
{
    ...
    COMPILER_REFERENCE(param_states);
    COMPILER_REFERENCE(param_phy_hi);
    COMPILER_REFERENCE(param_phy_lo);
    switch(cal_id){
        case HX5_IDB_PIPE_0_CAL_ID: 
            param_cal_grp_num = _tdm->_chip_data.cal_0.grp_num;
            param_cal_grp_len = _tdm->_chip_data.cal_0.grp_len;
    ...
    for (i=0; i<param_cal_grp_num; i++) {
        for (j=0; j<param_cal_grp_len; j++) {
            port_phy = param_cal_grp[i][j];
            if (port_phy>=49 && port_phy<=76) {
                port_speed = param_speeds[port_phy];
		if ((port_phy<=72 ) && (port_phy >= 69 )) { /* to include first halfpipe first falcon */
                    hp0_port_cnt++;
                    hp0_bw += port_speed;
                }
    ...
    if (hp0_port_cnt==0 && hp1_port_cnt==0) {
        ...
        for (i=0; i<param_cal_grp_num; i++) {
            for (j=0; j<param_cal_grp_len; j++) {
                port_phy = param_cal_grp[i][j];
	/* hx5 is a fixed unbalanced half pipe */	
	/* hp0 egl0 egl1 egl2 falcon 0:- pm's  49,53,57,65 */
	/* hp1 falcon 1 , 2 & egl3 :- pm's 61,69,73 */
                port_hp_id = (port_phy <(49)) ? (0) : 
		             ((port_phy>= 61) && (port_phy <= 64) ) ? (0) : 
		             ((port_phy>= 69) && (port_phy <= 72) ) ? (0) : (1);
                    port_pm = tdm_hx5_cmn_get_port_pm(port_phy, _tdm);
...
/**
@name: tdm_hx5_chk_pkt_sched_print
@param:

Print out PKT scheduler calender
**/
int
tdm_hx5_chk_pkt_sched_print(tdm_mod_t *_tdm, int cal_id, int hpipe_id)
{
    int i, j, x, y, port_phy, port_space, slot_cnt, 
        hp_id, result=PASS;
    int dist_max[HX5_NUM_PHY_PORTS_PER_HPIPE],
        dist_min[HX5_NUM_PHY_PORTS_PER_HPIPE];
    int port_cnt = 0, port_buff[HX5_NUM_PHY_PORTS_PER_HPIPE];
    int max_tmp, min_tmp,
        max_10g = 0, max_20g = 0, max_25g = 0,
        max_40g = 0, max_50g = 0, max_100g = 0,
        min_10g = HX5_SHAPING_GRP_LEN, min_20g = HX5_SHAPING_GRP_LEN,
        min_25g = HX5_SHAPING_GRP_LEN, min_40g = HX5_SHAPING_GRP_LEN,
        min_50g = HX5_SHAPING_GRP_LEN, min_100g= HX5_SHAPING_GRP_LEN;
    int param_phy_lo, param_phy_hi,
        param_cal_grp_num, param_cal_grp_len;
    int *param_pkt_cal;
    int **param_cal_grp=NULL;
    ...
    for (i = 0; i < HX5_NUM_PHY_PORTS_PER_HPIPE; i++) {
        port_buff[i]= HX5_NUM_EXT_PORTS;
        dist_max[i] = 0;
        dist_min[i] = HX5_SHAPING_GRP_LEN;
    }
    ...
    switch (hpipe_id) {
        case 0: param_pkt_cal = param_cal_grp[HX5_SHAPING_GRP_IDX_0]; break;
        case 1: param_pkt_cal = param_cal_grp[HX5_SHAPING_GRP_IDX_1]; break;
    ...
    for (i=0; i<param_cal_grp_num; i++) {
        ...
        for (j=0; j<param_cal_grp_len; j++){
            port_phy = param_cal_grp[i][j];
            if (port_phy >= param_phy_lo && port_phy <= param_phy_hi) {
                if (port_cnt < HX5_NUM_PHY_PORTS_PER_HPIPE) {
    ...
    for (i = 0; i < port_cnt; i++) {
        ...
        for (j=0; j <HX5_SHAPING_GRP_LEN; j++) {
            if (port_phy == param_pkt_cal[j]) {
                port_space = 0;
                for (x = 0; x < HX5_SHAPING_GRP_LEN; x++) {
                    y = (x + j + 1) % HX5_SHAPING_GRP_LEN;
                    if (param_pkt_cal[y] == HX5_NUM_EXT_PORTS) {
                        continue;
                        ...
                        slot_cnt = (slot_cnt + 1) % HX5_NUM_PKT_SLOTS_PER_PM;
                        break;
                    } else {
                        port_space++;
                    }
    ...
    if (min_10g == HX5_SHAPING_GRP_LEN && max_10g == 0) {
        TDM_PRINT4("%4s%12s%12s%12s\n", "10G", "--", "--", "--");
    } else {
        TDM_PRINT4("%4s%12d%12d%12d\n", "10G", min_10g, max_10g, max_10g - min_10g);
    }
    if (min_20g == HX5_SHAPING_GRP_LEN && max_20g == 0) {
        TDM_PRINT4("%4s%12s%12s%12s\n", "20G", "--", "--", "--");
    } else {
        TDM_PRINT4("%4s%12d%12d%12d\n", "20G", min_20g, max_20g, max_20g - min_20g);
    }
    if (min_25g == HX5_SHAPING_GRP_LEN && max_25g == 0) {
        TDM_PRINT4("%4s%12s%12s%12s\n", "25G", "--", "--", "--");
    } else {
        TDM_PRINT4("%4s%12d%12d%12d\n", "25G", min_25g, max_25g, max_25g - min_25g);
    }
    if (min_40g == HX5_SHAPING_GRP_LEN && max_40g == 0) {
        TDM_PRINT4("%4s%12s%12s%12s\n", "40G", "--", "--", "--");
    } else {
        TDM_PRINT4("%4s%12d%12d%12d\n", "40G", min_40g, max_40g, max_40g - min_40g);
    }
    if (min_50g == HX5_SHAPING_GRP_LEN && max_50g == 0) {
        TDM_PRINT4("%4s%12s%12s%12s\n", "50G", "--", "--", "--");
    } else {
        TDM_PRINT4("%4s%12d%12d%12d\n", "50G", min_50g, max_50g, max_50g - min_50g);
    }
    if (min_100g == HX5_SHAPING_GRP_LEN && max_100g == 0) {
        TDM_PRINT4("%4s%12s%12s%12s\n", "100G", "--", "--", "--");
    } else {
...
/**
@name: tdm_hx5_chk_pkt_sched
@param:

Checks same port jitter for PKT scheduler calender
**/
int
tdm_hx5_chk_pkt_sched(tdm_mod_t *_tdm, int cal_id, int hpipe_id)
{
	int i, j, k, port_phy, port_speed, slot_req, slot_cnt,
        hp_id, result=PASS;
    int dist_max[HX5_NUM_PKT_SLOTS_PER_PM], dist_min[HX5_NUM_PKT_SLOTS_PER_PM];
    int x, y, port_space, pkt_jitter, slot_pos_base, slot_pos,
        port_lanes, pos_step;
    int port_cnt = 0, port_buff[HX5_NUM_PKT_SLOTS_PER_PM];
    int param_phy_lo, param_phy_hi,
        param_cal_grp_num, param_cal_grp_len;
    int *param_pkt_cal;
    int **param_cal_grp=NULL;
    ...
    for (i = 0; i < HX5_NUM_PKT_SLOTS_PER_PM; i++) {
        dist_max[i] = 0;
        dist_min[i] = HX5_SHAPING_GRP_LEN;
        port_buff[i]= HX5_NUM_EXT_PORTS;
    }
    ...
    for (i=0; i<param_cal_grp_num; i++) {
        ...
        for (j=0; j<param_cal_grp_len; j++){
            port_phy = param_cal_grp[i][j];
            if (port_phy >= param_phy_lo && port_phy <= param_phy_hi) {
                if (port_cnt < HX5_NUM_PKT_SLOTS_PER_PM) {
                    port_buff[port_cnt++] = port_phy;
                }
            }
        }
    ...
    for (i = 0; i < port_cnt; i++) {
        port_phy = port_buff[i];
        port_speed   = param_speeds[port_phy];
        slot_req = tdm_hx5_chk_get_speed_slots_5G(port_speed);
        slot_cnt = 0;
        for (k=0; k <HX5_SHAPING_GRP_LEN; k++) {
    ...
    for (i = 0; i < port_cnt; i++) {
        ...
        slot_pos_base = 0;
        port_speed = param_speeds[port_phy];
        port_lanes = tdm_hx5_chk_get_port_lanes(_tdm, port_phy);
        switch (port_lanes) {
        ...
        for (k=0; k <HX5_SHAPING_GRP_LEN/8; k++) {
            /* quad mode (lanes == 1) */
            ...
            slot_pos = (slot_pos_base + k * 8) % HX5_SHAPING_GRP_LEN;
            if(param_pkt_cal[slot_pos] != port_phy &&
               param_pkt_cal[slot_pos] != HX5_NUM_EXT_PORTS) {
...
/*             TDM_PRINT5("%s port %3d, speed %dG, jitter/limit %d/%d\n",
    ...
    tdm_hx5_chk_pkt_sched_print(_tdm, cal_id, hpipe_id);
...
tdm_hx5_chk(tdm_mod_t *_tdm)
{
    ...
    if (tdm_hx5_chk_struct(_tdm) != PASS) {
        result = FAIL;
    }
    TDM_SML_BAR
    ...
    if (tdm_hx5_chk_transcription(_tdm) != PASS) {
        result = FAIL;
    }
    TDM_SML_BAR
    ...
    if (tdm_hx5_chk_sister(_tdm) != PASS) {
        result = FAIL;
    }
    TDM_SML_BAR
    ...
    if (tdm_hx5_chk_bandwidth(_tdm) != PASS) {
         result = FAIL;
    }
    TDM_SML_BAR
    ...
    if (tdm_hx5_chk_jitter_lr(_tdm) != PASS) {
        result = FAIL;
    }
    TDM_SML_BAR
    ...
    if (tdm_hx5_chk_pkt_sched(_tdm, HX5_IDB_PIPE_0_CAL_ID, 0) != PASS ||
        tdm_hx5_chk_pkt_sched(_tdm, HX5_IDB_PIPE_0_CAL_ID, 1) != PASS ||
        tdm_hx5_chk_pkt_sched(_tdm, HX5_IDB_PIPE_1_CAL_ID, 0) != PASS ||
        tdm_hx5_chk_pkt_sched(_tdm, HX5_IDB_PIPE_1_CAL_ID, 1) != PASS ) {
        result = FAIL;
    }
    */
    TDM_SML_BAR
