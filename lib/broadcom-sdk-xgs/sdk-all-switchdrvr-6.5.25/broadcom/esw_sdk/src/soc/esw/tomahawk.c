...
int
soc_tomahawk_sbus_ring_map_dump(int unit)
{
    ...
    if (soc_feature(unit, soc_feature_cmicm)) {
        ...
        LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META_U(unit, "CMIC_SBUS_RING_MAP_0_7:0x%08x\n"), rval));

        SOC_IF_ERROR_RETURN(READ_CMIC_SBUS_RING_MAP_8_15r(unit, &rval));
        LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META_U(unit, "CMIC_SBUS_RING_MAP_8_15:0x%08x\n"), rval));

        SOC_IF_ERROR_RETURN(READ_CMIC_SBUS_RING_MAP_16_23r(unit, &rval));
        LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META_U(unit, "CMIC_SBUS_RING_MAP_16_23:0x%08x\n"), rval));

        SOC_IF_ERROR_RETURN(READ_CMIC_SBUS_RING_MAP_24_31r(unit, &rval));
        LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META_U(unit, "CMIC_SBUS_RING_MAP_24_31:0x%08x\n"), rval));

        SOC_IF_ERROR_RETURN(READ_CMIC_SBUS_RING_MAP_32_39r(unit, &rval));
        LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META_U(unit, "CMIC_SBUS_RING_MAP_32_39:0x%08x\n"), rval));

        SOC_IF_ERROR_RETURN(READ_CMIC_SBUS_RING_MAP_40_47r(unit, &rval));
        LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META_U(unit, "CMIC_SBUS_RING_MAP_40_47:0x%08x\n"), rval));

        SOC_IF_ERROR_RETURN(READ_CMIC_SBUS_RING_MAP_48_55r(unit, &rval));
        LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META_U(unit, "CMIC_SBUS_RING_MAP_48_55:0x%08x\n"), rval));

        SOC_IF_ERROR_RETURN(READ_CMIC_SBUS_RING_MAP_56_63r(unit, &rval));
        LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META_U(unit, "CMIC_SBUS_RING_MAP_56_63:0x%08x\n"), rval));

        SOC_IF_ERROR_RETURN(READ_CMIC_SBUS_TIMEOUTr(unit, &rval));
        LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META_U(unit, "CMIC_SBUS_TIMEOUT:0x%08x\n"), rval));
    }
#endif
...
_soc_tomahawk_port_mapping_init(int unit)
{
    ...
    while (0 <= port) {
        if (!SOC_PORT_VALID(unit, port)) {
            /* Initialize an invalid port used for black hole MTP. */
            soc_mem_field32_set(unit, mem, &entry, DEVICE_PORT_NUMBERf, port);
            SOC_IF_ERROR_RETURN
                (soc_mem_write(unit, mem, MEM_BLOCK_ALL, port, &entry));
            break;
...
STATIC int
_soc_th_ledup_chain_set(int unit) {
    ...
    int pos, uc;
    int rv;
    cmicd_led_chain_t *chain = NULL;

    /* Build a LED chain hardware descriptor for TH. */
    chain = sal_alloc(sizeof(cmicd_led_chain_t), "CmicdLedChain");
    if (chain == NULL) {
        LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META_U(unit,
                  "ERROR: CMICd LED chain memory not allocated")));
        return SOC_E_MEMORY;
    }

    sal_memset(chain, 0, sizeof(cmicd_led_chain_t));
    for (phy_port = 1; phy_port <= 132; phy_port++) {
        if (phy_port <= 32) {
            uc = 0;
            pos = 32 - phy_port;
        } else if (phy_port <= 96) {
            uc = 1;
            pos = (3 - ((phy_port - 33) & 0x3)) + ((phy_port - 33) & ~(0x3)) ;
        } else if (phy_port <= 128) {
            uc = 0;
            pos = (32 - (phy_port - 97)) + 31;
        } else {
            uc = 2;
            pos = 3 - (phy_port - 129);
        }
        chain->valid[phy_port] = 1;
        chain->uc[phy_port] = uc;
        chain->pos[phy_port] = pos;
    }

    /* Register the LED chain descriptor and enable LED driver. */
    rv = soc_cmicd_led_chain_set(unit, chain);
...
_soc_th_ledup_init(int unit)
{
    ...
    return _soc_th_ledup_chain_set(unit);
}
...
#define DEFAULT_PG_NUM 7  /* Default Priority Group Num */
    ...
    SOC_PBMP_ITER(si->lb_pbm, port) {
        ...
        for (idx = 0; idx < numq; idx++) {
            ...
            lb_q_min += buf_queue->guarantee;
        }
    }
    lb_q_min = lb_q_min >> 1; /* Taking half, 2 pipe = 1 xpe */
...
soc_error_t
soc_th_tdm_recover(int unit, int initTime)
{
    soc_control_t *soc = SOC_CONTROL(unit);
    _soc_tomahawk_tdm_t *tdm;
    int rv;
    ...
    tdm = soc->tdm_info;
    /* Calculate TDM calendars */
    rv = _soc_tomahawk_tdm_calculation(unit, initTime, tdm);
    SOC_IF_ERROR_RETURN(_soc_tomahawk_tdm_calculation_free_memory(unit));
    if (SOC_FAILURE(rv)){
        LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META_U(unit,
