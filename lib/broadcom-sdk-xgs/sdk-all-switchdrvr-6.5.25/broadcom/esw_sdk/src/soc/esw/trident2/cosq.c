            ...
            if (SOC_IS_TD2P_TT2P(unit)) {
                if (lvl == SOC_TD2_NODE_LVL_L1) {
                    if (IS_CPU_PORT(unit, in_port)) {
                        ...
                        /*
                         * COVERITY
                         * Caller will assure offset >= 0, in turn base >= 0.
                         */
...
int
soc_td2_cosq_threshold_dynamic_change_check(int unit, int new_threshold,
                                            soc_reg_t ctr_reg,
                                            soc_port_t port, int ar_idx,
                                            uint8 sync_mode,
                                            uint8 *allowed)
{
    uint64 val64;
    ...
    *allowed = FALSE;
    ...
    do {
        ...
        /* Retrieve the use count. */
        sal_memset(&val64, 0, sizeof(val64));
        if (sync_mode) {
            SOC_IF_ERROR_RETURN
                (soc_counter_sync_get(unit, port, ctr_reg, ar_idx, &val64));
        } else {
            SOC_IF_ERROR_RETURN
                (soc_counter_get(unit, port, ctr_reg, ar_idx, &val64));
        }

        /* Use count is greater or equal than new threshold. */
        if (COMPILER_64_HI(val64)
            || COMPILER_64_LO(val64) >= new_threshold) {
            check_pass = FALSE;
        }
