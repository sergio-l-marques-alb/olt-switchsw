...
_soc_apache_idb_port_init(int unit, int reset, int lossless, soc_port_t port)
{
    ...
    */ 
    ...
    soc_reg_field_set(unit, reg, &rval32, obm_oversub_fields[subport], 1);
    SOC_IF_ERROR_RETURN(soc_reg32_set(unit, reg, pgw_inst, 0, rval32));
                                    ...
                                    char *prefix_str)
{
    ...
    SOC_PBMP_ITER(pbmp, mmu_port) {
        ...
        log_generic.parity_type = SOC_PARITY_TYPE_START_ERR;
                         ...
                         char *prefix_str)
{
    ...
    for (info_index = 0; ; info_index++) {
        ...
        case _SOC_PARITY_TYPE_GENERIC:
            ...
            log_generic.boot_count = soc_ser_log_get_boot_count(unit);
            log_generic.block_type = SOC_BLOCK_INFO(unit, block_info_idx).type;
            log_generic.parity_type = SOC_PARITY_TYPE_GENERIC;
...
_soc_apache_process_ser_fifo (int unit, soc_block_t blk, char *prefix_str)
{
    ...
    case SOC_BLK_IPIPE:
        ...
        ser_reg_info_table = _soc_bcm56560_a0_ip_reg_ser_info;
        sal_sprintf(blk_str, "IPIPE");
        break;
    case SOC_BLK_EPIPE:
        mem = EGR_SER_FIFOm;
        ser_info_table = _soc_bcm56560_a0_ep_mem_ser_info;
        ser_reg_info_table = _soc_bcm56560_a0_ep_reg_ser_info;
        ...
        if (_apache_ser_status_bus_fmt_get(entry,VALIDf)) {
            ...
            if (regmem == _SOC_APACHE_SER_MEM) {
                ...
                if (non_sbus == 0) {
                    ...
                    spci.detect_time = sal_time_usecs();
                    spci.acc_type = _SOC_ACC_TYPE_PIPE_ANY;
                    soc_regaddrinfo_extended_get(unit, &ainfo, sblk,
                                                 spci.acc_type, address);
                    spci.reg = ainfo.reg;
                    _soc_td2_ser_reg_control_get(unit, ser_reg_info_table,
                                                 spci.reg,
                                                 &parity_enable_reg,
                                                 &parity_enable_field);

                    spci.log_id = _soc_td2_populate_ser_log_for_reg(unit,
                                                                    parity_enable_reg,
                                                                    parity_enable_field,
                                                                    spci.reg,
                                                                    ainfo.idx >= 0 ? ainfo.idx : 0,
                                                                    ainfo.port,
                                                                    0, spci.acc_type,
                                                                    spci.detect_time);
                    rv = soc_ser_correction(unit, &spci);
