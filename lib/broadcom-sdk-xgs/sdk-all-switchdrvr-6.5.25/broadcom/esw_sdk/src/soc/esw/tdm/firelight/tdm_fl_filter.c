...
/**
@name: tdm_fl_filter_chk_slot_shift_cond
@param:

**/
int
tdm_fl_filter_chk_slot_shift_cond(int slot, int dir, int cal_len, int *cal_main, tdm_mod_t *_tdm)
{
    ...
    if (slot < cal_len) {
        ...
        idx     = src;
        port    = cal_main[idx];
        port_pm = tdm_fl_cmn_get_port_pm(port, _tdm);
        if (port >= param_phy_lo && port <= param_phy_hi) {
            ...
            for (k=1; k<=param_space_sister; k++) {
                ...
                if (port_k >= param_phy_lo && port_k <= param_phy_hi) {
                    if (tdm_fl_cmn_get_port_pm(port_k, _tdm) == port_pm) {
                        result = FAIL;
                        break;
                    }
                }
...
/**
@name: tdm_fl_filter_lr_jitter
@param:

**/
int
tdm_fl_filter_lr_jitter(tdm_mod_t *_tdm, int min_speed)
{
    ...
    if (param_lr_en==BOOL_TRUE && min_speed>=SPEED_10G) {
        for (i=0; i<param_cal_len; i++) {
            ...
            if (port < param_ext_phy_num) {
                ...
                port_bmp[port] = 1;
                tdm_fl_cmn_get_speed_jitter(port_speed, param_cal_len,
                                             &space_min, &space_max);
                for (j=0; j<param_cal_len; j++) {
                    k = (i+param_cal_len-j)%param_cal_len;
                    if (param_cal_main[k] == port) {
                        idx_curr = k;
                        dist_up = tdm_fl_cmn_get_same_port_dist(idx_curr,
                                    TDM_DIR_UP, param_cal_main, param_cal_len);
                        dist_dn = tdm_fl_cmn_get_same_port_dist(idx_curr,
                        ...
                        if (dist_up > space_max || dist_dn < space_min) {
                            ...
                            for (m=1; m<=dist_mv; m++) {
                                ...
                                y = (x + 1) % param_cal_len;
                                chk_x = tdm_fl_filter_chk_slot_shift_cond(x, TDM_DIR_DN,
                                            param_cal_len, param_cal_main, _tdm);
                                chk_y = tdm_fl_filter_chk_slot_shift_cond(y, TDM_DIR_UP,
                                            param_cal_len, param_cal_main, _tdm);
                                /* shift port */
                        ...
                        /* filter port if space violation is detected: down */
                            ...
                            for (m=1; m<=dist_mv; m++) {
                                ...
                                chk_x = tdm_fl_filter_chk_slot_shift_cond(x, TDM_DIR_UP,
                                            param_cal_len, param_cal_main, _tdm);
                                chk_y = tdm_fl_filter_chk_slot_shift_cond(y, TDM_DIR_DN,
                                            param_cal_len, param_cal_main, _tdm);
                                /* shift port */
                                if (chk_x == PASS && chk_y == PASS) {
                                    param_cal_main[y] = param_cal_main[x];
...
/**
@name: tdm_fl_filter_lr
@param:

**/
int
tdm_fl_filter_lr(tdm_mod_t *_tdm)
{
    tdm_fl_filter_lr_jitter(_tdm, SPEED_40G);
