/*
 * $Id: tomahawk3.c,v 1.1.10.25 Broadcom SDK $
...
static int parity_enable[SOC_MAX_NUM_DEVICES];
...
soc_th3_bypass_unused_pm(int unit)
{
    ...
    /* If the user has decided to allow disabling PMs, check which
     * ones are in use */
    if (soc_property_get(unit, "disable_unused_port_blocks", FALSE)) {
        ...
        /* In 56982, only certain PMs are enabled so we are setting the
           pm maps directly instead of relying on the config or port mappings
         */
        for (pm = 0; pm < _TH3_PBLKS_PER_DEV; pm++) {
            if ((pm == 2 || pm == 3 || pm == 4 || pm == 5 ||
              pm == 6 || pm == 9 || pm == 10 || pm == 11 ||
              pm == 12 || pm == 13 || pm == 18 || pm == 19 ||
              pm == 20 || pm == 21 || pm == 22 || pm == 25 ||
              pm == 26 || pm == 27 || pm == 28 || pm == 29)) {
                used_pm_map[pm] = 1;
                si->active_pm_map[pm] = 1;
            } else {
                used_pm_map[pm] = 0;
                si->active_pm_map[pm] = 0;
            }
        }
    } else {
        /* Otherwise, mark all PMs as enabled */
        for (pm = 0; pm < _TH3_PBLKS_PER_DEV + 1; pm++) {
            si->active_pm_map[pm] = 1;
        }
        /* Return because there won't be any changes to TOP_TSC_ENABLE,
         * which will only happen if a PM in used_pm_map == 0. The above
         * for loop would have set them all to 1. */
        return SOC_E_NONE;
    }
...
_soc_th3_pp_pll_config(int unit, int freq)
{
    ...
    /* Fref,       Ndiv,   Pdiv, Mdiv,          pp_clk,          VCO */
      ...
      {110010,      70,     1,    4,             875,   (uint32)3.5},
      {110010,      60,     1,    4,             750,             3},
      {110010,      53,     1,    4,   (uint32)662.5,  (uint32)2.65},
      {110010,      50,     1,    4,             625,   (uint32)2.5},
      {110010,      40,     1,    4,             500,   (uint32)2.0}
    };
    int pp_pll_len;
...
soc_tomahawk3_chip_reset_fast_reboot(int unit)
{
    ...
    parity_enable[unit] = soc_property_get(unit, spn_PARITY_ENABLE, TRUE);
    if (parity_enable[unit]) {
...
#ifdef BCM_CMICX_SUPPORT
STATIC int
_soc_th3_linkscan_flr_init(int unit)
{
    soc_pbmp_t support;
    int pport;
    ...
    if (soc_feature(unit, soc_feature_cmicx) && !(SAL_BOOT_PLISIM || SAL_BOOT_BCMSIM)) {
        ...
        /* Only enable FLR support on TH3 Blackhawk(physical port 1 ~ 256). */
        SOC_PBMP_CLEAR(support);
        for (pport = 1; pport <= 256; pport++) {
            if (pport < SOC_PBMP_PORT_MAX) {
                SOC_PBMP_PORT_ADD(support, pport);
            }
        }

        SOC_IF_ERROR_RETURN(soc_linkscan_flr_support_set(unit, support));
    }
    return SOC_E_NONE;
}
...
_soc_tomahawk3_ipep_default_init (int unit)
{
    ...
    soc_pbmp_t pbmp;
    uint64 rval64;
    static int rtag7_field_width[] = { 16, 16, 6, 16, 0, 0, 16, 16 };
...
_soc_tomahawk3_port_mib_reset (int unit) 
{
    ...
    SOC_BLOCK_ITER(unit, block_info_idx, SOC_BLK_CDPORT) {
        ...
        for (port = phy_port_base; port < phy_port_base + _TH3_PORTS_PER_PBLK; port++) {
            SOC_IF_ERROR_RETURN
                (soc_reg32_rawport_set(unit, CDMAC_MIB_COUNTER_CTRLr,
                                       port, 0, rval));
        }
    }
    sal_usleep(1);
...
_soc_tomahawk3_cmic_mdio_freq_config (int unit)
{
    ...
    ext_divisor = soc_property_get(unit, spn_RATE_EXT_MDIO_DIVISOR, RATE_EXT_MDIO_DIVISOR_DEF(unit));
...
void soc_th3_lp_intr_handler(int unit, void *data)
{
    ...
    (void)soc_iproc_getreg(unit, soc_reg_addr(unit, reg[0], REG_PORT_ANY, 0),
                           &regval);
    if ((regval & SOC_TH3_SER_REG0_INTR_MASK) && parity_enable[unit]) {
    ...
    (void)soc_iproc_getreg(unit, soc_reg_addr(unit, reg[2], REG_PORT_ANY, 0),
                           &regval);
    if ((regval & SOC_TH3_SER_REG2_INTR_MASK) && parity_enable[unit]) {
    ...
    (void)soc_iproc_getreg(unit, soc_reg_addr(unit, reg[3], REG_PORT_ANY, 0),
                           &regval);
    if ((regval & SOC_TH3_SER_REG3_INTR_MASK) && parity_enable[unit]) {
    ...
    (void)soc_iproc_getreg(unit, soc_reg_addr(unit, reg[1], REG_PORT_ANY, 0),
                           &regval);
    if ((regval & SOC_TH3_SER_REG1_INTR_MASK) && parity_enable[unit]) {
...
_soc_tomahawk3_misc_init(int unit)
{
    ...
    */
    ...
    {
        SOC_IF_ERROR_RETURN(_soc_th3_ledup_init(unit));
        SOC_IF_ERROR_RETURN(_soc_th3_linkscan_flr_init(unit));
    }
#endif /* BCM_CMICX_SUPPORT */
