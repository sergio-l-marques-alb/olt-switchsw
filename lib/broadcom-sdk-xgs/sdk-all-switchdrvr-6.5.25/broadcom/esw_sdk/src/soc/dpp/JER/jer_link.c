...
_soc_jer_ports_phy_to_cmic_bitmap_map(int unit, pbmp_t hw_mii_pbm, soc_pbmp_t  *cmic_bitmap)
{
    ...
    PBMP_ITER(hw_mii_pbm, port)
    {
        ...
        else
        {
            ...
            /* The nif ports mapping to CMIC_MIIM_SCAN_PORTS are starting
             * from CMIC_MIIM_SCAN_PORTS_2 on qax.It means CMIC_MIIM_SCAN_PORTS_0/1
             * are unuseful. Here we add 64 offset since the with of the
             * CMIC_MIIM_SCAN_PORTS are 32bit.
             */
...
/*
 * Function:
 *     soc_jer_linkcan_fault_to_cmiclink_enable_set
 * Purpose:
 *     Enable/disable external Local/Remote Fault signal to indicate link
 *     down to CMIC
 * Parameters:
 *     unit - Device unit number
 *     pbm  - Bitmap of ports to operate on
 *     remote_fault_enable  - enable remote fault signal to indicate link down to CMIC or not
 *     local_fault_enable   - enable local fault signal to indicate link down to CMIC or not
 * Returns:
 *     BCM_E_XXX
 */
int
