...
/* 120G Synchronous Packer interface */
 ...
 * Virtual-Port[3:0] = PON-Port, Virtual-Port[5:4] = Channel-ID
 */
  ...
  SOC_DPP_CORES_ITER(SOC_CORE_ALL, core) {
      for (pp_port = 0; pp_port < MAX_PORTS_IN_CORE ; ++pp_port)
      {
          ...
          if (PBMP_MEMBER(pon_port_bm,port)) {
              ...
              {
                  /*to support 16*4 PON-TAG[13:12] = channel_id; PON-TAG[11:0] = gem_id
                  * for FEM, header_data[0]=8100, header_data[1]=13e9,channel_id = header_data[1]{21:20}
                  * we need to build fem_key[32:16]=header_data[1],
                  * and IHP_PTC_VIRTUAL_PORT_CONFIG.OFFSET should be 1, fem_field_select_5 = 21, fem_field_select_4=20
                  */
    ...
    if( nof_replications == 0 ) 
    {
        ...
        if( SOC_DPP_IS_MODID_AND_BASE_MODID_ON_SAME_FAP(unit, fap_id_add_lag_member, sys_fap_id_self) == 1 )
        {
            ...
            if (SOC_IS_JERICHO(unit) && ARAD_IS_HQOS_MAPPING_ENABLE(unit)
                ...
                /**if not enable on src side, need to set master lag sysport*/
                if (!soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "hqos_update_src_sys_port_once", 0)) {
                    if (master_lag_sysport == ARAD_SW_DB_MODPORT2SYSPORT_INVALID_SYSPORT) {
                        ...
                        /** this port is in master LAG, update it*/
                        if (lag_id == lag_ndx) {
                            SOCDNX_SAND_IF_ERR_EXIT(arad_sw_db_modport2master_lagsysport_set(unit,fap_id_add_lag_member,port_id_add_lag_member,sys_logic_port_id));
                        }
                    }
            ...
            if (SOC_IS_JERICHO(unit) && ARAD_IS_HQOS_MAPPING_ENABLE(unit)
                ...
                if (master_lag_sysport == ARAD_SW_DB_MODPORT2SYSPORT_INVALID_SYSPORT) {
                    /* 
                     * this port is not added to any LAG, take it as master LAG.
                     */
        ...
        if (nof_replications == 1) 
        {
            ...
            if ( SOC_DPP_IS_MODID_AND_BASE_MODID_ON_SAME_FAP(unit, fap_id_removed_lag_member, sys_fap_id_self) == 1)
            {
                ...
                if (SOC_IS_JERICHO(unit) && ARAD_IS_HQOS_MAPPING_ENABLE(unit)
                    && soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "hqos_update_src_sys_port_once", 0)) {
                    /* 
                     * this port is in master LAG, need to remove
                     */
                    if ((master_lag_sysport != ARAD_SW_DB_MODPORT2SYSPORT_INVALID_SYSPORT) && (lag_id == lag_ndx)) {
                ...
                if (SOC_IS_JERICHO(unit) && ARAD_IS_HQOS_MAPPING_ENABLE(unit)
                    ...
                    /**if not enable on src side, need to remove master lag sysport*/
                    if (!soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "hqos_update_src_sys_port_once", 0)) {
                        /* 
                         * this port is in master LAG, need to remove
                         */
            ...
            if(( moved_lag_member_nof_replications == 1) || ( (moved_lag_member_first_replication_index != -1) && (moved_lag_member_first_replication_index > removed_member_index) ))
            {
                ...
                {                    
                    ...
                    if (SOC_IS_JERICHO(unit) && ARAD_IS_HQOS_MAPPING_ENABLE(unit)
                        ...
                        if (master_lag_sysport == ARAD_SW_DB_MODPORT2SYSPORT_INVALID_SYSPORT) {
                            /* 
                             * this port is not added to any LAG, forbidden here.
                             */
                            ...
                            if (lag_id == lag_ndx) {
                                /* 
                                 * this port is in master LAG, need to re-align
                                 */
        ...
        {                    
            ...
            if(current_member_first_replication_index == current_member_index)
            {
                ...
                if (SOC_IS_JERICHO(unit) && ARAD_IS_HQOS_MAPPING_ENABLE(unit)
                    ...
                    if (master_lag_sysport == ARAD_SW_DB_MODPORT2SYSPORT_INVALID_SYSPORT) {
                        /* 
                         * this port is not added to any LAG, just update to origin phy-port
                         */
                        ...
                        /*if master_lag_id == lag_ndx, this is opeartion in master LAG, update to origin phy-port */
                        if (lag_id == lag_ndx) {
                            SOCDNX_SAND_IF_ERR_EXIT(arad_port_src_sys_port_set_unsafe(unit, core_id_current_lag_member, port_id_current_lag_member, org_sys_port));
                        }
                    }
    ...
    for(current_member_index = 0; current_member_index < nof_members_in_lag; ++current_member_index)
    {
        ...
        {                    
            ...
            {                
                ...
                /*
                 *  Under Hqos mode, dont't update if master LAG already set
                 *  Check this lag_ndx == master_lag_ndx
                 */
                SOCDNX_SAND_IF_ERR_EXIT(arad_sw_db_modport2master_lagsysport_get(unit, fap_id_current_lag_member, port_id_current_lag_member, &master_lag_sysport));
                SOCDNX_SAND_IF_ERR_EXIT(arad_ports_logical_sys_id_parse(unit, master_lag_sysport, &is_lag_not_phys, &lag_id, &lag_member_id, &sys_phys_port_id));
                ...
                if (SOC_IS_JERICHO(unit) && ARAD_IS_HQOS_MAPPING_ENABLE(unit)
                    ...
                    if (master_lag_sysport == ARAD_SW_DB_MODPORT2SYSPORT_INVALID_SYSPORT) {
                        /* 
                         * this port is not added to any LAG, this is master_LAG
                         */
                        ...
                        /*if master_lag_id == lag_ndx, update it, don't take care about member id */
                        if (lag_id == lag_ndx) {
                            SOCDNX_SAND_IF_ERR_EXIT(arad_port_src_sys_port_set_unsafe(unit, core_id_current_lag_member, port_id_current_lag_member, sys_logic_port_id));
                            SOCDNX_SAND_IF_ERR_EXIT(arad_sw_db_modport2master_lagsysport_set(unit,fap_id_current_lag_member,port_id_current_lag_member,sys_logic_port_id));
                        }
    ...
    SOC_SAND_OUT uint32     enable)
{
    ...
    if(enable) 
    {
        ...
        if (SOC_IS_JERICHO(unit)) {
            ...
            /* Before switch to ignore mode, set all bits first to not ignore all received FC */
            ...
            /* In force mode, clear port related EGQ bits to ignore received FC */
