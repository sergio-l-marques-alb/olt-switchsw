...
/* The table is PPDB_B_LARGE_EM_AGE_AGING_MODE.
...
/* Offset of dynamic configuration in PPDB_B_LARGE_EM_AGE_AGING_MODE register*/
#define ARAD_PP_FRWRD_BMACT_AGE_AGING_DYNAMIC_CONF_OFFSET ((ARAD_PP_FRWRD_BMACT_AGE_AGING_DB_INDEX << 3) | (1 << 2))
/* Number of bits per dynamic/static configuration in PPDB_B_LARGE_EM_AGE_AGING_MODE register*/
  ...
  )
{
  ...
  uint8 dyn_age_pointer_default[ARAD_PP_FRWRD_BMACT_NOF_VSI_AGING_PROFILES]; /* Default mapping between dynamic aging profile and aging pointer */
  ...
  */
  ...
  /* Aging for BMACT-DB, for dynamic entries.
     *   The table is PPDB_B_LARGE_EM_AGE_AGING_MODE.
     *   The key is {Entry.DB-Index(2bit),Entry.Is-Dynamic(1bit),Entry.FID-Aging-Profile(2bit)}.
     *   The entry format is {LargeEmAgeOwnSystemLagPorts(1), LargeEmAgeOwnSystemPhysicalPorts(1), LargeEmAgeAgingCfgPtr(2)}.
     */
  ...
  for (; age_key_ndx < last_age_key_ndx; ++age_key_ndx)
  {
    ...
    fld_val = dyn_age_pointer_default[age_key_ndx - ARAD_PP_FRWRD_BMACT_AGE_AGING_DYNAMIC_CONF_OFFSET]; /* Set the default dynamic aging pointers in the bitmap. */
   ...
   )
{
  ...
  if (bmac_key->flags & SOC_PPC_BMACT_ENTRY_TYPE_LEARN) { 
      ...
      /*
       * the key in test2 is only vsi&sa
       */
  ...
  )
{
  ...
  /* If aging is static, all fields are 0 */
