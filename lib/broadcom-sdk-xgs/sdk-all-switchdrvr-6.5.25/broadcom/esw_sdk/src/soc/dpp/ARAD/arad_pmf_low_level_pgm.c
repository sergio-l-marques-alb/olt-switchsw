...
/* } */
  ...
  if(SOC_IS_JERICHO(unit))
  {
      ...
      /* Configure header_profile 15 - for Learn extension disable : same configuration like Ethernet  without PPH Learn extension */
      header_profile = 0;
...
/* } */
  ...
  if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
      ...
      if (SOC_IS_ARAD_B0_AND_ABOVE(unit)) {
          ...
          LOG_ERROR(BSL_LS_SOC_FP,(BSL_META_U(unit,"Unit %d Unsupported architecture\n\r"),unit));
          SOC_SAND_SET_ERROR_CODE(ARAD_UNSUPPORTED_ERR, 15, exit);
      }
