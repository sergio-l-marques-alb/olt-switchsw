...
#define QAX_ITM_NOF_TOTAL_BYTES(unit)      ((SOC_DPP_CONFIG(unit)->jer->dbuffs.dbuffs_bdries.mnmc_0.size) /* Number of buffers */ \
...
/* When there are no DRAMs nof_words==nof_sram_words */
    ...
    )
{
    ...
    data2 = (SOC_DPP_CONFIG(unit)->jer->tm.truncate_delta_in_pp_counter[1] == 0) ? 0xE : 0xF;
     /* For ITM,all compensation activities are active, except Header_truncate, which is determine by config file */
  ...
  )
{
    ...
    SOC_DPP_CORES_ITER(SOC_CORE_ALL, core_index) {
        ...
        ingress_congestion_mgmt.global[SOC_TMC_INGRESS_THRESHOLD_SRAM_PDS].total = QAX_ITM_NOF_SRAM_PDS(unit) - 1; /* table's field has only 15 bits */
    ...
    /* Init default flow control VSQ-E index */
...
int qax_itm_fc_vsq_e_defaults_set(int unit)
{
    ...
    for (core_id = 0; core_id < SOC_DPP_CONFIG(unit)->core_mode.nof_active_cores; core_id++)
    {
        for (src_pp_port = 0; src_pp_port < SOC_DPP_DEFS_GET(unit, nof_pp_ports_per_core); src_pp_port++){
            /* Disable VSQ E/F mapping */
            SOCDNX_IF_ERR_EXIT
                (qax_itm_src_vsqs_mapping_set(unit, core_id, src_pp_port, 0/* vsq_index */, 0/* pg_base */, 0 /* disable */));
        }
    }
...
int qax_itm_drop_global_thresholds_init(int unit)
{
    ...
    SOC_DPP_CORES_ITER(SOC_DPP_CORE_ALL(unit), core_index) {
        ...
        /* Reject thresholds based on shared resource occupancy of Pools by the source based VSQs - per DP. */
        for (rsrc_type = 0; rsrc_type < SOC_TMC_INGRESS_THRESHOLD_NOF_TYPES; ++rsrc_type) {
            /* Set reject threshold to maximum */
...
int qax_itm_ingress_latency_init(int unit)
{
    ...
    for(i = 0; i < QAX_ITM_NOF_MAX_LATENCY_MEASUREMENTS; ++i) { /*8 such registers */
    ...
    /* configure the latency measurement flow id selector to use latency flow-id mode.*/
    /* it means that the value in the latency flow-id action (using PMF) will be written as the latency-flow-id value on the OAM-TS-HEADER */
    ...
    /* configure the accuracy of the IRPP stamping as as Jericho, meanings bits 31:5 from the mac-Ts (Time-stamp = Time-Stamp-from-MAC[31:0]>>Cfg-Packet-Latency[2:0]) */
    SOCDNX_IF_ERR_EXIT(READ_IHB_LBP_GENERAL_CONFIG_0r(unit, 0, &reg64Val));
    ...
    /* the IRPP timeStamp is in nano seconds, but only bits 31:5. (as configured above) */
    /* Meaning the 5 LSB bits are masked. therefore, need to set the accuracy of the IPT TS the same. */
  ...
  )
{
    ...
    *c1 = soc_sand_log2_round_down(calc);
    ...
    /*
     * According to definition of WRED, C2 should be calculated as the following:
     * C2 = FACTOR * max-prob * min-th / (max-th - min-th)
     *
     * This formula is valid, when WRED parameters have full (real) precision.
     * When WRED parameters have integer precision, 
     * this formula is not useful.
     * 
     * Actual probability is defined by the following
     *
     * P = (Ignore_pkt_size == 0) --> [(2^C1 * avrg_size - C2)  / 2^C3] * pkt_size.
     *     (Ignore_pkt_size == 1) --> [(2^C1 * avrg_size - C2)  / 2^C3]
     * 
     * C2 can't be bigger than (2^C1 * min_th), since then we get negative drop probability.
     * C2 can't be smaller than (2^C1 * min_th), 
     * since than drop probability descreases when max probability increases 
     * (when the two max probabilities produce the same C1, the bigger max probability produces 
     * smaller C2).
     * 
     * So, C2 must be equal to 2^C1 * min_th
     */
  ...
  )
{
    ...
    /*VSQE mapping*/
    /*In-PP-Port => VSQ-E index, default value of src_port_vsq_index should be invalid NIF port instead of valid port "0" */
  ...
  )
{
    ...
    *enable = 0;
    /* 
     * validate that the '0' value of src_port_vsq_index and pg_base
     * is indeed the value and not just the field's reset value 
     */
  ...
  )
{
    ...
    if (SOC_IS_QUX(unit) && (pg_vsq_id >= (SOC_TMC_NOF_TRAFFIC_CLASSES * 8))) {
        /* For Viper single lane ports on QUX, only 2 priorities are supported */
...
/*!
 * \brief
 *  Verify that the given global reject threshold is in valid ranges.
 *
*/
STATIC
int
  ...
  )
{
    ...
    /*
     * Convert max threshold to mantissa-exponent representation and back
     * to avoid false error due to mantissa-exponent inaccuracy
     */
...
/*!
 * \brief
 *  Verify that the given global reject threshold is in valid range
 * and write it to HW.
 *
*/
STATIC int
  ...
  )
{
    ...
    /* Set Global VSQ shared pools resources reject thresholds */
    is_dram_thresh = 0;
    for (rsrc_type = 0; rsrc_type < SOC_TMC_INGRESS_THRESHOLD_NOF_TYPES; ++rsrc_type) {
        for (color = 0; color < SOC_TMC_NOF_DROP_PRECEDENCE; ++color) {
            /* Set thresholds for pool 0 */
            /* pool 0 entries occupied the first half of the table */
            entry_offset = color;
            ...
            /* Set thresholds for pool 1 */
            /* pool 1 entries occupied the last half of the table */
    ...
    /* Until the end of the function rsrc_type is used only for not deviding by Words size */
  ...
  )
{
    ...
    /* Get Global VSQ shared pools resources reject thresholds */
    for (rsrc_type = 0; rsrc_type < SOC_TMC_INGRESS_THRESHOLD_NOF_TYPES; ++rsrc_type) {
        for (color = 0; color < SOC_TMC_NOF_DROP_PRECEDENCE; ++color) {
            /* Get thresholds for pool 0 */
            /* pool 0 entries occupied the first half of the table */
            entry_offset = color;
  ...
  ) 
{
    ...
    for (rsrc_type = 0; rsrc_type < SOC_TMC_INGRESS_THRESHOLD_NOF_TYPES; ++rsrc_type) {
        ...
        /*
         * VSQ resources are Total Bytes, Sram Buffers and Sram Pds
         * The only one which needs scaling is Total Bytes
         */
  ...
  )
{
    ...
    SOCDNX_IF_ERR_EXIT(soc_reg_get(unit, CGM_REJECT_STATUS_BITMAPr, SOC_CORE_ALL, 0, &val)); 
  ...
  )
{
    ...
    itm_mantissa_exp_threshold_info voq_sys_red_range_th_mant_exp = {
        ...
        8, /* mantissa_bits */
        5, /* exp_bits */
        QAX_ITM_WORDS_RESOLUTION /* factor */
    };
