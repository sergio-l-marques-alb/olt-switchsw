/* $Id: jer_pp_kaps_diag.c, Broadcom SDK Exp $
                            ...
                            JER_KAPS_SEARCH_RESULT *return_cmp_rslt)
{
    int rv = BCM_E_NONE;
    uint32 soc_sand_rv;
    JER_KAPS_SEARCH_RESULT cmp_rslt, *cmp_rslt_p;
    ...
    /* RPB indices 2(search_id is 1) and 3(search_id is 2) have switched for Jericho Plus only */
    ...
    if ((key[key_index][0] >> (SOC_SAND_NOF_BITS_IN_BYTE - JER_KAPS_TABLE_PREFIX_LENGTH)) == JER_KAPS_IPV4_UC_AND_NON_IP_TABLE_PREFIX) {
        /* IPV4 UC is used for dynamic tables */
        ...
        /* IPV4 MC is used for dynamic tables */
    ...
    for(i = 0; i<search_cfg.valid_tables_num; i++) {
        ...
        if ((key_offset != i) && (return_cmp_rslt != NULL)) {
            sal_memcpy(return_cmp_rslt->assoc_data[i], cmp_rslt_p->assoc_data[key_offset], sizeof(uint8) * KAPS_INSTRUCTION_MAX_AD_BYTES);
...
int jer_kaps_sw_search_test(int unit, uint32 tbl_id, uint8 *key, uint8 *payload, int32 *prefix_len, int32 *is_matched)
{
    ...
    JER_KAPS_ENTRY *entry_p = NULL;
    JER_KAPS_AD *ad_entry_p = NULL;
    ...
    res = kaps_db_search(table_cfg_p.tbl_p, key, &entry_p, &index, prefix_len);
    if(JER_KAPS_TO_SOC_RESULT(res) != SOC_SAND_OK) {
       LOG_ERROR(BSL_LS_SOC_TCAM,
                 (BSL_META_U(unit,
                             "Error in %s(): kaps_db_search failed with : %s!\n"),
                             FUNCTION_NAME(),
    ...
    if (entry_p) {
        *is_matched = TRUE;
        res = kaps_entry_get_ad(table_cfg_p.tbl_p, entry_p, &ad_entry_p);
        if(JER_KAPS_TO_SOC_RESULT(res) != SOC_SAND_OK) {
            LOG_ERROR(BSL_LS_SOC_TCAM,
                (BSL_META_U(unit,
                        "Error in %s(): kaps_entry_get_ad failed: %s\n"),
                        FUNCTION_NAME(), kaps_get_status_string(res)));
            SOC_SAND_SET_ERROR_CODE(SOC_SAND_GEN_ERR, 20, exit);
        }
        res = kaps_ad_db_get(ad_db_p, ad_entry_p, payload);
        if(JER_KAPS_TO_SOC_RESULT(res) != SOC_SAND_OK) {
            LOG_ERROR(BSL_LS_SOC_TCAM,
                (BSL_META_U(unit,
                        "Error in %s(): kaps_ad_db_get failed: %s\n"),
                        FUNCTION_NAME(), kaps_get_status_string(res)));
...
int jer_kaps_hw_search_test(int unit, uint32 search_id, uint8 *master_key, JER_KAPS_SEARCH_RESULT *cmp_rslt)
{
    ...
    jer_kaps_search_config_get(unit, search_id, &search_cfg);
    res = kaps_instruction_search(search_cfg.inst_p, master_key, 0, cmp_rslt);
    if(JER_KAPS_TO_SOC_RESULT(res) != SOC_SAND_OK) {
       LOG_ERROR(BSL_LS_SOC_TCAM,
                 (BSL_META_U(unit,
                             "Error in %s(): kaps_instruction_search failed with : %s!\n"),
                             FUNCTION_NAME(),
                  kaps_get_status_string(res)));
...
int jer_kaps_parse_print_entry(int unit, uint32 dbal_table_id, uint32 prio_len, uint8 *data_bytes, uint8 *ad_8, uint8 header_flag) {
    ...
    res = arad_pp_dbal_kaps_buffer_to_entry_key(unit, &dbal_table, prio_len, data_bytes, qual_vals);
...
int jer_kaps_show_table(int unit, uint32 dbal_table_id, uint32 print_entries) {
    ...
    for (j=0; j<2; j++) {
        ...
        if ((table_cfg_p.clone_of_tbl_id == JER_KAPS_IP_NOF_TABLES)
            ...
            /*initialize iterator*/
            KAPS_TRY(kaps_db_entry_iter_init(tbl_p, &iter));
            num_of_entries = 0;
            /*traverse entries*/
            do {
                KAPS_TRY(kaps_db_entry_iter_next(tbl_p, iter, &kaps_e));

                if (kaps_e == NULL)
                    break;

                KAPS_TRY(kaps_entry_get_info(tbl_p, kaps_e, &kaps_e_info));
                /* Verify that the table prefix is correct - dynamic tables share the KAPS table but do not necessarily share the dbal prefix*/
                if (kaps_e_info.data[0] >> (SOC_SAND_NOF_BITS_IN_BYTE - dbal_table.db_prefix_len) != dbal_table.db_prefix) {
                    continue;
                }
            ...
            KAPS_TRY(kaps_db_entry_iter_destroy(tbl_p, iter));
            ...
            if (print_entries) {
                ...
                do {
                    ...
                    if ((ad_db_p != NULL) && (kaps_e_info.ad_handle != NULL)) {
                            KAPS_TRY(kaps_ad_db_get(ad_db_p, kaps_e_info.ad_handle, ad_8));
                    } else {
                        sal_memset(ad_8, 0, BITS2BYTES(JER_KAPS_AD_WIDTH_IN_BITS));
                    }

                    res = jer_kaps_parse_print_entry(unit, dbal_table_id, kaps_e_info.prio_len, kaps_e_info.data, ad_8, header_flag);
                    SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
...
int jer_kaps_show_db_stats( int unit, uint32 dbal_table_id ) {
    ...
    for ( j = 0; j < 2 ; j++ ) {
        ...
        if ( ( tbl_p != NULL ) && ( dbal_table.is_table_initiated ) ) {
            ...
            res = kaps_db_stats( tbl_p, &stats );
            if ( JER_KAPS_TO_SOC_RESULT( res ) != SOC_SAND_OK ) {
                LOG_ERROR( BSL_LS_SOC_TCAM, ( BSL_META_U( unit, "Error in %s(): DB: kaps_db_stats with failed: %s!\n" ), FUNCTION_NAME(), kaps_get_status_string( res ) ) );
            }
...
int jer_pp_kaps_tcam_bist(int unit)
{
    ...
    /*Initialize IBC in case KAPS is not initialized*/
    ...
    for (blk_id = JER_KAPS_RPB_BLOCK_INDEX_START; blk_id <= JER_KAPS_RPB_BLOCK_INDEX_END; blk_id++) {
        for(cam_index = 0; cam_index < cam_index_max; cam_index++) {
            ...
            if (SOC_IS_JERICHO_PLUS(unit)) {
                ...
                /* coverity[overrun-buffer-val:FALSE] Coverity accesses the function with the wrong nbytes */
            ...
            if (SOC_IS_JERICHO_PLUS(unit)) {
                ...
                /* Attempt to read the register multiple times to allow the BIST to complete */
                do
                {
                    ...
                    attempt_counter++;
                } while ((attempt_counter < ARAD_MIN_POLLS) && (sal_memcmp(res_data, expected_data, 4) != 0));
            }
...
int jer_pp_kbp_sdk_ver_test(int unit)
{
    ...
    const char* kbp_sdk_ver_arr[] =       {"KBP SDK 1.5.16"};
...
/*
 * KAPS RPB TCAM SER test: 1bit error auto-correction.
 */
int jer_pp_kaps_ser_test(int unit)
{
    ...
    /*ECC is 16b */
    uint8 ecc_data[8]={0};
    int is_ecc, flip_bit, flip_byte,flip_b;
    uint8 flip_data;
    int tcam_rows = 4096;
    int tcam_row_iter;
    ...
    if (!SOC_IS_JERICHO_PLUS(unit)) {
        ...
        for (blk_id = JER_KAPS_RPB_BLOCK_INDEX_START; blk_id <= JER_KAPS_RPB_BLOCK_INDEX_END; blk_id++) {
            ...
            {
                /*
                only need check entry 166 bist: 160 bits data + 4 bits search_lsn + 2 bits of valid.
                loop flip bit: entry bit 0; ... entry bit 165; ecc bit 0; ... ecc bit 15;
                */
                flip_bit = tcam_row_iter%182;
                if (flip_bit<166) {
                    is_ecc =0;
                } else {
                    is_ecc =1;
                    flip_bit=flip_bit-166;
                }
                flip_byte = flip_bit/8; /*need update flip_byte byte */
                flip_b = flip_bit%8;  /*revert flip_b bit */


                if (is_ecc==0) {
                    /*read ecc and save in ecc_data*/
                    sal_memset(ecc_data, 0, sizeof(ecc_data));
                    res = jer_pp_kaps_read_command(&xpt_p,
                                                   blk_id,
                                                   KAPS_CMD_READ,
                                                   KAPS_FUNC5,
                    ...
                    res = jer_pp_kaps_read_command(&xpt_p,
                                                   ...
                                                   data);
                    SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
                    flip_data= 0x1<<flip_b;
                    data[flip_byte] = data[flip_byte] ^ flip_data;
                    res = jer_pp_kaps_write_command(&xpt_p,
                                                   blk_id,
                                                   KAPS_CMD_WRITE,
                                                   KAPS_FUNC1,
                                                   ...
                                                   data);
                    SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
                    /*write back the original ecc data*/
                    res = jer_pp_kaps_write_command(&xpt_p,
                                                   blk_id,
                                                   KAPS_CMD_WRITE,
                                                   KAPS_FUNC5,
                    ...
                    flip_data= 1<<flip_b;
                    ecc_data[flip_byte] = ecc_data[flip_byte] ^ flip_data;
                    res = jer_pp_kaps_write_command(&xpt_p,
                                                   blk_id,
                                                   KAPS_CMD_WRITE,
                                                   KAPS_FUNC5,
...
exit:
    SOC_SAND_EXIT_AND_SEND_ERROR( "error in jer_pp_kaps_ser_test()", 0, 0);
}
...
int jer_pp_kaps_search_test(int unit, uint32 add_entries, uint32 search_entries, uint32 delete_entries, uint32 loop_test, uint32 cache_test, uint32 ser_test)
{
    ...
    JER_KAPS_SEARCH_RESULT cmp_rslt;
    ...
    do {
        ...
        for (k=0; k < 2; k++) { /* Second pass runs with all expected zero after a table clear*/
            ...
            if (ser_test==1 && k==0) {
                cli_out("\n-----------------------------\nSER Test- Flip one bit for all entries.\n-----------------------------\n");
                jer_pp_kaps_ser_test(unit);
            ...
            if (search_entries && ((k==0 && add_entries) || (k==1 && delete_entries))) {
                for (j=0; j < JER_KAPS_NOF_SEARCHES; j++) {
                    ...
                    for (i=0; i < search_cfg.valid_tables_num; i++) {
                        ...
                        res = arad_pp_dbal_entry_key_to_kaps_buffer(unit, &table, JER_KAPS_KEY_BUFFER_NOF_BYTES, qual_vals, &prefix_len, key[i]);
                        if (res != SOC_SAND_OK) {
