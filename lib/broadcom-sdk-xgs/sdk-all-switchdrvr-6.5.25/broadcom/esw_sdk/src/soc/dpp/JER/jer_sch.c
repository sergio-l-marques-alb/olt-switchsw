...
*/
#define JER_SCH_CREDIT_WORTH_DIV_MIN (1)  /*ASSIGNED/REBOUNDED_CREDIT_WORTH min*/
#define JER_SCH_CREDIT_WORTH_DIV_MAX (0x1fff) /*ASSIGNED/REBOUNDED_CREDIT_WORTH MAX, value: ~CSST/PSST_max_burst/4*/
    ...
    )
{
    ...
    SOC_DPP_CORES_ITER(SOC_CORE_ALL, core)
    {
        ...
        {
            /*
             * Since QAX does not allow to remove offset in Credit Messages, 
             * in order to avoid interrupt in IPS, we mask out the bit #16 (64K)
             * Note that if there would be a system where QAX is connected to non QAX, 
             * non QAX would get IPS interrupt, since the flow id does not match.
             * (Credits would arrive fine. The interrupt is just annoying.)
             */
    ...
    )
{
    ...
    if (is_master) {
        ...
        {
            SOC_SAND_SET_BIT(egress_offset, 0, 0); /* handle fast ports first */
            end++;
	    }
        ...
        uint32 *max_bucket)
{
    ...
    for (index = 0; index < JER_SCH_MAX_BUCKET_TABLE_NOF_INDEXES; index++)
    {
        ...
        else if (slow_rate < (slow_rate_max_bucket[index+1].rate * 1000 /* kbps*/ ))
        {
            /*
...
/* This function should be called only during init sequence */
    ...
    /* config csst and psst tables and chose new divider using calculate_new_calendars function
       we call jer_sch_rates_calculate_new_calendars with increasing_maximum_rate = 0
    ...
    */
     SOCDNX_IF_ERR_EXIT(_jer_sch_rates_calculate_new_calendars(unit, core, /*increasing_maximum_rate*/0, &is_new_rate_configured));
...
/*
 * this function is called when configuring 1-2 priority port
 * to unset TCG specific configuration    
 */
int
...

{
    ...
    {
        /** we can't use ARAD_SCH_PORT_TCG_ID_GET since base_q_pair is not 8-priority base */
        ...
        if (valid)
        {
            SOCDNX_IF_ERR_EXIT(_jer_sch_rates_calendar_index_get(unit, core, offset, 0, &index));
            /*
             * set corresponding entries in SCH_CIR_SHAPERS_STATIC_TABEL_CSST to default value
             * in lower and upper calendar
             */
            ...
            /** unset this Q-pair in the SW db, to avoid overwriting when switching calendars */
