  ...
  )
{
  ...
  case SOC_PPC_FRWRD_DECISION_TYPE_UC_PORT:
  case SOC_PPC_FRWRD_DECISION_TYPE_UC_LAG:
      if ((fwd_decision->type == SOC_PPC_FRWRD_DECISION_TYPE_UC_LAG) && BCM_DPP_TRUNK_MEMBER_IS_SET(fwd_decision->dest_id)) {
          sys_port_id = BCM_DPP_TRUNK_MEMBER_GET(fwd_decision->dest_id);
      } else {
  ...
  )
{
  ...
  {
    if (!BCM_DPP_TRUNK_MEMBER_IS_SET(fwd_decision->dest_id)) {
...
arad_pp_init_orientation_maps(int unit){ 
    ...
    /** In mode 3, logical_port_mim_out is not supported.*/
