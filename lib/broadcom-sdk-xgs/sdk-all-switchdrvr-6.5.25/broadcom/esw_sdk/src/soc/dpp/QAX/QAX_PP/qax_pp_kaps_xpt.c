/* $Id: qax_pp_kaps_xpt.c, Broadcom SDK Exp $
...
kaps_status qax_pp_kaps_search(void *xpt,
                                     uint8_t *key,
                                     ...
                                     struct kaps_search_result *kaps_result)
{
    ...
    rv = soc_mem_array_write(unit, KAPS_RPB_TCAM_CPU_COMMANDm, search_interface, KAPS_BLOCK(unit, 0), 0 /*offset*/, mem_array);
    if (rv != SOC_SAND_OK) {
    ...
    rv = soc_mem_array_read(unit, KAPS_RPB_TCAM_CPU_COMMANDm, search_interface, KAPS_BLOCK(unit, 0), 0 /*offset*/, mem_array);
    if (rv != SOC_SAND_OK) {
...
kaps_status qax_kaps_register_read(void *xpt, uint32_t offset, uint32_t nbytes, uint8_t *bytes)
{
    int rv = KAPS_OK,
        i, unit;
    uint32 reg_val = 0;
...
kaps_status qax_kaps_register_write(void *xpt, uint32_t offset, uint32_t nbytes, uint8_t *bytes)
{
...
STATIC
kaps_status qax_kaps_translate_blk_func_offset_to_mem_reg(int unit,
                                                         uint8 blk_id,
                                                         uint32 func,
                                                         uint32 offset,
                                                         soc_mem_t *mem,
...
kaps_status qax_pp_kaps_write_command(void *xpt,
                                     uint8 blk_id,
                                     uint32 cmd,
                                     uint32 func,
                                     uint32 offset,
                                     ...
                                     uint8 *bytes)
{
    ...
    if (mem != INVALIDm) {
        ...
        if ((func == KAPS_FUNC5) && ((mem == KAPS_BUCKET_MEMORYm) || (mem == KAPS_BBS_BUCKET_MEMORYm))) {
            ...
            if(nbytes == 15) {
                /* Enter here for Jericho_plus, where the KAPS DMA entry buffer is 120-bit*/
                byte_idx = 0;
                for (word_idx = 0; word_idx < BYTES2WORDS(nbytes); word_idx++) {
                    if (word_idx == 0) {
                        /* in case we have only 15 bytes instead of 16, the LSByte is missing */
   ...
   kaps_status qax_pp_kaps_read_command(void *xpt,
                                           uint32 blk_id,
                                           uint32 cmd,
                                           uint32 func,
                                           uint32 offset,
                                           ...
                                           uint8 *bytes)
{
    ...
    if (mem != INVALIDm) {
        ...
        if ((func == KAPS_FUNC5) && ((mem == KAPS_BUCKET_MEMORYm) || (mem == KAPS_BBS_BUCKET_MEMORYm))) {
            ...
            if(nbytes == 15) {
                /* For Jericho+, we read 16 bytes, but take the LSB 15 Bytes*/
                byte_idx = 0;
                for (word_idx = 0; word_idx < BYTES2WORDS(nbytes); word_idx++) {
                ...
                /* Shift the whole result array 1 byte to the left, to omit the MSB byte*/
                for(byte_idx = 0; byte_idx < nbytes; byte_idx++) {
                    bytes[byte_idx] = bytes[byte_idx + 1];
                }
            } else {
                /* We start from i=1 since the MSB is in the BMM */
                byte_idx = 1;
                for (word_idx = 0; word_idx < BYTES2WORDS(nbytes); word_idx++) {
...
kaps_status qax_pp_kaps_command(void *xpt,
                               enum kaps_cmd cmd,
                               enum kaps_func func,
                               uint32_t blk_nr,
                               uint32_t row_nr,
