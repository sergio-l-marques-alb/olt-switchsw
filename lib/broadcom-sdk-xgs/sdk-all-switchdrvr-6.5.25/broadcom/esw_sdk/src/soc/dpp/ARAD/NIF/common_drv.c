/*
 * $Id: common_drv.c,v 1.8 Broadcom SDK $
 *
...
/*
 * Function:
 *      soc_common_drv_bypass_mode_set
 * Purpose:
 *      Configure bypass mode
 * Parameters:
 *      unit        - (IN) Unit number.
 *      port        - (IN) Port number.
 *      bypass_mode - (IN) Is bypass mode enable.
 * Returns:
 *      SOC_E_xxx
 */
soc_error_t 
...
/*
 * Function:
 *      soc_common_drv_hdr_mode_val_get
 * Purpose:
 *      Get the hdr mode due to the encap mode
 * Parameters:
 *      unit        - (IN) Unit number.
 *      port        - (IN) Port number.
 *      mac_mode_reg -(IN) the reg to write e.g CLP_CMAC_MODEr
 *      reg_val      -(OUT) the reg value
 * Returns:
 *      SOC_E_xxx
 */
soc_error_t
...
/*
 * Function:
 *      soc_common_link_status_clear
 * Purpose:
 *      Clear current link latch status.
 * Parameters:
 *      unit - unit number.
 *      port - port number.
 * Returns:
...
soc_common_link_status_clear(int unit, soc_port_t port)
{
    ...
    /* get default register value in order to reset default value
     * This is important in case PORT_LINKSTATUS_DOWN_CLEAR is selected to begin with
     */
