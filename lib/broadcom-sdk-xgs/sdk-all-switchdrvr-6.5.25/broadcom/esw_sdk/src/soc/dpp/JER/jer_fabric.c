...
/*
 * Function:
 *      jer_fabric_pcp_mode_init
 * Purpose:
 *      Perform relevant tasks for fabric_pcp_mode operations:
 *          - Set relevant registers to set fabric pcp mode to no_packing based on soc_property
 * Parameters:
 *      unit -  unit number
 *
 * Returns:
  ...
  )
{
    ...
    /* Configure packet cell packing mode */
    ...
    SOC_SAND_OUT int             *divider)
{
    ...
    if (!SOC_IS_QAX_WITH_FABRIC_ENABLED(unit)) {
        ...
        *divider = soc_reg64_field_get(unit, FSRD_SYNC_E_SELECTr, reg64_val, SYNC_E_CLK_DIV_PHASE_1f) + 1; /*phase1 is configured as (divider-1)*/
    } else {
