  ...
  )
{
  ...
  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  120,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, IPS_STORED_CREDITS_USAGE_CONFIGURATIONr, SOC_CORE_ALL, 0, MUL_PKT_DEQ_BYTESf,  16));
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  150, exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, ECI_GLOBAL_2r, REG_PORT_ANY, 0, ISP_QNUM_LOWf,  0x17fef)); /* 96K-17 to allow last 16 queues for others */
  ...
  )
{
  ...
  /*
   * Total guaranteed size(s) are set in arad_init.c
   */
  ...
  )
{
  ...
  if (hw_q_type == ARAD_SW_DB_QUEUE_TYPE_NOT_AVAILABLE) {
      ...
      if (!SOC_IS_ARADPLUS_AND_BELOW(unit)) {
          ...
          /* Select between max command credits profiles (regular profile and high bandwidth profile) */
      ...
      if (hw_q_type != SOC_TMC_ITM_QT_PUSH_Q_NDX) {
          ...
          if (!SOC_IS_ARADPLUS_AND_BELOW(unit)) {
              ...
              SOC_SAND_SET_ERROR_CODE(ARAD_UNSUPPORTED_ERR, 405, exit); /* Unsupported architecture */
          }
  ...
  )
{
  ...
  if (!SOC_IS_ARADPLUS_AND_BELOW(unit)) {
      ...
      /* Get max command credits profile */
      credit_selection_reg = (SOC_IS_QAX(unit)) ? (IPS_MAX_S_DEQ_COMMAND_CREDIT_SELECTIONr) : (IPS_MAX_DEQ_COMMAND_CREDIT_SELECTIONr);
      credit_selection_field = (SOC_IS_QAX(unit)) ? (MAX_SDQ_CMD_SELf) : (MAX_DQ_CMD_SELf);
      SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res, 440, exit, ARAD_REG_ACCESS_ERR, 
              soc_reg32_get(unit, credit_selection_reg, REG_PORT_ANY, 0, &reg32));
...
uint32
  arad_itm_queue_is_ocb_only_set(
    ...
    if (line_shift >= 32) {
        /** line_shift should always be in the range of 0 - 31, since the field size is 32.*/
        SOCDNX_IF_ERR_EXIT(SOC_E_INTERNAL);
    }
    ...
    )
{
    ...
    uint32 res;
    int need_workaround = (SOC_IS_ARADPLUS_A0(unit) );
    SOC_SAND_INIT_ERROR_DEFINITIONS(0);
    if (!SOC_IS_QAX(unit)) {
        if (alpha >= 0) {
...
/* } */
#endif /* of #if efined(BCM_88660_A0) */
