    ...
    for (key=0; key<_QAX_PP_OAM_HLM_ING_KEY_SELECT_REG_KEY_SIZE; key++) {
        ...
        if (!_QAX_PP_OAM_OEM1_KEY_SEL_INGRESS_KEY_OAM_LIF_OUTER_VALID(key)) {
            ...
            if (!_QAX_PP_OAM_OEM1_KEY_SEL_INGRESS_KEY_OAM_LIF_INNER_VALID(key)) {
                ...
                /* For JR+ out_LIF will be selcted as Inner key for counting 
                 * as only one lookup possible in OEM1 table, to do the hierarcvhical counting.
                 * Default MEP will be selected for packet trapping based on profile 0.
                 */
...
soc_error_t soc_qax_pp_oam_oem1_key_select_egress_init(int unit)
{
    ...
    for (key=0; key<_QAX_PP_OAM_HLM_EG_KEY_SELECT_REG_KEY_SIZE; key++) {
        key_sel_inner   = 0;
        key_sel_outer   = 0;
        mp_profile_sel  = 1; /* Always set Mp-Profile_sel at egress */
        your_disc_inner = 0;
        your_disc_outer = 0;
...
soc_error_t soc_qax_pp_oam_classifier_oem1_entry_set_for_hlm(
    ...
    /* pre_count_set means previous OEM1 counter already set and valid on the same lif
       new_count_set means new counter ID is valid */
    if(SOC_IS_QUX(unit)){
...
/*
* This function use to update pe_profile of MEP's DB when user want to count Y1711-LM packet.
* @author Xiao (13/3/2017)
*
* @param unit
* @param mep_type:  Y1711_MPLS or Y1711_PWE
* @param mep_index
* @param cnt_enable:  Whether enable Y1711-LM packet count or not
*
* @return soc_error_t
*
*/
  ...
  )
{
    ...
    uint32     reg_val = 0,reg32_field_get;
   ...
   if((sd_sf_entry->ccm_tx_rate> 0x7)||(sd_sf_entry->thresh_profile> 15)){
    ...
    if(dbflag == 1){
        ...
        SHR_BITCOPY_RANGE(entry, 0, &reg_val, 0, 2);
         /*SLIDING_WND_CNTR & SUM_OF_CNTRS cleard by init*/
        
        reg_val = 0; /*SUM_OF_CNTRSf */
...
/* Set OAMP_MEP_PROFILE[1].UDP_SRC_PORT_CHECK_DIS = 0/1 based on enable/disable.
 *
 * NOTE: the BFD module should correctly update the MEP_PROFILE in the corresponding
 * OAMP_MEP_DB entry as profile 1.
 */
...
soc_error_t soc_qax_pp_oam_slm_set(int unit, int is_slm) {
    ...
    /*
     * DEPRECATED
     * Replaced with per-LIF implementation.
     */
    soc_error_t res = SOC_E_NONE;
    soc_reg_above_64_val_t reg_data;
                                                                           ...
                                                                           SOC_SAND_INOUT _oam_oam_a_b_table_buffer_t  *oamb_buffer)
{
    ...
    for (is_my_cfm_mac = 0; is_my_cfm_mac <=1; is_my_cfm_mac++) {
        ...
        for (internal_opcode = 1; internal_opcode < SOC_PPC_OAM_OPCODE_MAP_COUNT; internal_opcode++) {
            ...
            /* subtype was changed to 0 not to select the CCM UP MEP RX program */
...
/*The function sets the mep_pe_profile in case the gpoup is maid_48
  The mep_pe_profile depends on the mep_type
  The function returns the mep_pe_profile*/
    ...
    SOC_SAND_OUT     uint32            *mep_pe_profile)
{
    ...
    /*In case double pointer or egress injection feature,the feature depends on the mep_type*/
    if(out_lif != 0) {
        /*In case double pointer feature*/
        ...
        /*In case egress_injection feature*/
        ...
        *mep_pe_profile = reg_val;
        ...
        if(is_upmep) {
            ...
            /*In case of down mep, check the user_header_size*/
            if(user_header_size){
