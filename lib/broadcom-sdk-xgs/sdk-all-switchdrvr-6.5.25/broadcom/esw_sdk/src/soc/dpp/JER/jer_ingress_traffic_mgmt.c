...
/*
 * Despite the description in Jericho register file,
 * System Red boudary threshold is
 * 7 bits mantissa and 5 bits exponent, 
 * like it is in Arad
 */
  ...
  )
{
    ...
    BCM_DPP_CORES_ITER(BCM_CORE_ALL, core_index) {
        ...
        SOCDNX_IF_ERR_EXIT(res);
        /** get BDs threshold that was configured to global BD threshold - take the one configured for DP3 - that is the lower one.
         *  this is required to allow reserved traffic of all DPs to flow regardless to the DP of the regular traffic */
        ...
        if (core_index == 0)
        {
            ...
            /** dram total size should be determined by the minimal value between the BD global threshold and the mnmc size.
             *  this is required to prevent a corner case of threshold colision of reserved resources. this is under the assumption
             *  that the global BDs threshold was not changed after init */
            ...
            guaranteed_q_resource.dram.total = total_dram_buffers_for_reserved_resources_calculations; /* DRAM accounts only for MNMC buffers for unicast traffic */
        }
        else
        {
            /* core 1 */
  ...
  )
{
    ...
    SOCDNX_IF_ERR_EXIT(tmc_itm_mantissa_exp_field_set(unit,
                                                      ...
                                                      mantissa_bits,
                                                      soc_mem_field_length(unit, IQM_VSQ_PG_PRMm, FADT_FC_OFFSETf) - mantissa_bits, /* exp_bits */
                                                      1, /*factor */
                                                      1, /* round_up */
                                                      &data,
    ...
    SOCDNX_IF_ERR_EXIT(tmc_itm_mantissa_exp_field_set(unit,
                                                      ...
                                                      mantissa_bits,
                                                      soc_mem_field_length(unit, IQM_VSQ_PG_PRMm, FADT_FC_FLOORf) - mantissa_bits, /* exp_bits */
                                                      1, /*factor */
                                                      1, /* round_up */
                                                      &data,
       ...
       )
{
    ...
    /*OCB only is an attribute of the PG to be configured in IQM_VSQ_PG_IS_OCB_ONLY and does not selects a VSQ like in the other VSQ types*/
...
int jer_itm_ingress_latency_init(int unit)
{
    ...
    /* configure the accuracy of the IRPP stamping */
    /* In jericho, Time-stamp = Time-Stamp-from-MAC[31:0]>>5, therefore, IPT should get the same accuracy, 
       in order to shift left back the value of the IRPP TS. 
       In Jer+, (Time-stamp = Time-Stamp-from-MAC[31:0]>>Cfg-Packet-Latency), since Cfg-Packet-Latency=0, also the accuracy=0 */    
    ...
    /* In JR+, remove above limitation, then the Latency-Flow-ID comes from PMF action bcmFieldActionLatencyFlowId*/
...
int jer_itm_max_latency_pkts_get(int unit, int max_count, SOC_TMC_MAX_LATENCY_PACKETS_INFO *max_latency_packets, int *actual_count)
{
    ...
    /** must read entire table. HW freeze table when reading entry zero and un-freeze when reading entry 7 */
...
*   PROC
* FUNCTION:
*     This procedure triggers VOQ flush
* INPUT:
*   int unit
*   int core_id - device core index
*   int queue_id - voq index
*   int timeout - timeout for the flush operation [usecs]
* REMARKS:
*     None.
*********************************************************************/
soc_error_t
