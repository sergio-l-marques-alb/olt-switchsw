  ...
  )
{
 ...
 /*
  * success or overwriting exist entry only when not Key_only operation
  */
  ...
  )
{
  ...
  if ((vsid_ndx < SOC_DPP_CONFIG(unit)->l3.nof_rifs) || ((soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "mim_spb_enable", 0)) && _BCM_DPP_VLAN_IS_BVID(unit, vsid_ndx))) { /* low VSI with its own RIF */
    rif_id = vsid_ndx;
  } else { /* high VSI with default RIF */
    res = READ_IHP_HIGH_VSI_CONFIGr(unit, &reg32_val);
    SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 10, exit);
