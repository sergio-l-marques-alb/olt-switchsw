...
typedef enum
{
    ...
    #define ARAD_PMF_OAM_MIRROR_WA_NOF_STATIC_PROG_SEL_LINES(unit) \
        ...
        (SOC_PPC_TRAP_CODE_OAM_FTMH_MIRROR_TRAP_CODES_NUM + 1 + (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "egress_snooping_advanced", 0) > 0)) : 0)
#else
  ...
  )
{
    ...
    ARAD_PMF_LINE_INFO_clear(&group_line);
  ...
  )
{
  ...
  if (SOC_DPP_CONFIG(unit)->pp.oam_enable && (stage == SOC_PPC_FP_DATABASE_STAGE_INGRESS_PMF)) {
      ...
      for (prog_selection_line_indx = 0; prog_selection_line_indx < (SOC_PPC_TRAP_CODE_OAM_FTMH_MIRROR_TRAP_CODES_NUM + is_oam_wa_snoop_enabled + is_lb_sat_support); prog_selection_line_indx++) {
          /* Translate the PFG info to the table */
          ...
          /* Add one line to support UP mep LB packet*/
          if(prog_selection_line_indx == (ARAD_PMF_OAM_MIRROR_WA_NOF_STATIC_PROG_SEL_LINES(unit)-1))
          {
              trap_code_ndx = SOC_PPC_TRAP_CODE_SAT_2;
          }
...
#ifdef ARAD_PMF_OAM_MIRROR_WA_SNOOP_OUT_LIF_IN_DSP_EXT_ENABLED
          if (is_oam_wa_snoop_enabled && (prog_selection_line_indx == SOC_PPC_TRAP_CODE_OAM_FTMH_MIRROR_TRAP_CODES_NUM)) {
