  ...
  )
{
  int
    ret ;
  ret = 0 ;
  /* coverity explanation: overrun on the array Soc_sand_error_desc_element.
     However, there is no overrun; When we reach to "error_pool_size = sizeof(SOC_ERROR_DESC_ELEMENT) * ui";
     ui has the index of the last elemnt of the array, so we get the right size of the array, and when we copy with this size it's ok.
     */
  ...
  )
{
  int
    ret ;
  ret = 0 ;
  /* coverity[overrun-buffer-val:FALSE] legacy code */
