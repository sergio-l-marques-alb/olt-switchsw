  ...
  )
{
    ...
    /* SOC_PPC_FRWRD_FCF_ROUTE_LOCAL_DOMAIN --> add to sem :: prefix: 8 or 24 */
    ...
    if (route_key->flags & SOC_PPC_FRWRD_FCF_ROUTE_DOMAIN || route_key->flags & SOC_PPC_FRWRD_FCF_ROUTE_DOMAIN_NPORT) {
        if (SOC_IS_JERICHO(unit)) {
            ...
            payload = (payload + (1 << 17)); /* FEC encoding in PP destination */
    ...
    /* SOC_PPC_FRWRD_FCF_ROUTE_HOST --> add to LEM, ::prefix has to 8 or 24  */
  ...
  )
{
  ...
  if (route_key->flags & SOC_PPC_FRWRD_FCF_ROUTE_DOMAIN || route_key->flags & SOC_PPC_FRWRD_FCF_ROUTE_DOMAIN_NPORT) {
      if (SOC_IS_JERICHO(unit)) {
            ...
            if(route_key->flags == SOC_PPC_FRWRD_FCF_ROUTE_DOMAIN_NPORT){
                qual_vals[0].type = SOC_PPC_FP_QUAL_HDR_FWD_IPV4_SIP;
                /* coverity[overrun-buffer-val : FALSE] Type (void*) is parsed according to physical db type */
                res = arad_pp_dbal_entry_get(unit, SOC_DPP_DBAL_SW_TABLE_ID_FCOE_NPORT_KAPS, qual_vals, &payload, 0/*priority*/, 0/*hit_bit*/,found);
            }else {
                /* coverity[overrun-buffer-val : FALSE] Type (void*) is parsed according to physical db type */
                res = arad_pp_dbal_entry_get(unit, SOC_DPP_DBAL_SW_TABLE_ID_FCOE_KAPS, qual_vals, &payload, 0/*priority*/, 0/*hit_bit*/, found);
            }
            SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
  ...
  )
{
    ...
    if (soc_property_get(unit, spn_FCOE_NPV_BRIDGE_MODE, 0)) {
        arad_pp_dbal_vtt_fcoe_npv_mode_no_my_mac_set(unit, enable); /* enable/disable the NPV my_mac mode in the VTT stage*/
