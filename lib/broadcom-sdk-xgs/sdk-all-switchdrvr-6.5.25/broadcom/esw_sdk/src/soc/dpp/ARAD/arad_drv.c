...
soc_driver_t soc_driver_acp = {
    ...
    /* mem_array_info         */    NULL,
    /* block_info             */    NULL,
    /* nof_instances_per_device */  0,
    /* invalid_block_instances*/    NULL,
    /* block_instances        */    NULL,
    /* format_info          */ NULL,
    /* dop_info               */    NULL,
    /* port_info              */    NULL,
    /* counter_maps           */    NULL,
...
soc_arad_default_config_meter_get(int unit)
{
    ...
    /* Frequency need to be converted to hrz. After the devition round up is needed */
...
soc_arad_default_config_general_get(int unit)
{
    ...
    /* custom_feature_ipmc_l2_ssm_mode_lem==1 && local_switching_enable==1 : ingress lif cos is [32: 63] 
           custom_feature_ipmc_l2_ssm_mode_lem==1 && local_switching_enable==0 : ingress lif cos is [0: 31] */
    ...
    /* for L2TP only DIP and DIP_SIP are supported, DIP_SIP_VRF is done through L2TP termination config */
    ...
    if (SOC_IS_ARAD_B1_AND_BELOW(unit)){
        ...
        /* Use LEM to classify  IPv4 BFD packet after label termination for BFD over MPLS */
...
soc_arad_str_prop_nif_elk_fwd_table_size_get(int unit, ARAD_INIT_ELK *elk)
{
    ...
    /* In Jericho and above is mandatory to use IPv6 UC with RPF */
...
soc_arad_deinit(int unit)
{
    ...
    /* disconnect interrupts at last, make sure interrupt assert signal can be serviced */
    if (SOC_IS_JERICHO(unit)) {
...
soc_arad_info_config(int unit)
{
    ...
    if ((propval == NULL) || (sal_strcmp(propval, "SHARED") == 0) || SOC_IS_QAX(unit)){ /*elij - I prefer  SOC_DPP_DEFS_GET(unit, nof_cores) == 1) but this might change Arad behavior*/
    ...
    /* OAM statistics and STACKING (for ingress + egress multicast WA) must disable xgs PMF program */
...
/* } */
    ...
    if (dpp_arad->init.pp_enable) {
      ...
      /* Verify udp_tunnel misconfigurations - not supported with bfd multi hop */
      if (SOC_DPP_CONFIG(unit)->pp.udp_tunnel_enable &&
          (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "multi_hop_bfd_extra_dip_support", 0))) {
...
soc_arad_init(int unit, int reset_action)
{
   ...
   if (dpp->arad == NULL) {
    ...
    /* set tcam entry cache enable for arad after phase1 */
