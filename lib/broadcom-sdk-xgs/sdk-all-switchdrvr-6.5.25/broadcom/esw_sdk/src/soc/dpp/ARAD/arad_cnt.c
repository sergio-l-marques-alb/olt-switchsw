...
/* 
 * in Arad the encoding is Threshold = Th[9:0]<<Th[14:10], Jericho is the encoding is Threshold = Th[61:0]
 * in Arad 2^22, 2^24 in Jericho 
 */
#define ARAD_CNT_PKT_TRESH(unit)     (SOC_IS_JERICHO(unit) ? (1 << 24) : (0x5801))
/* 
 * in Arad the encoding is Threshold = Th[9:0]<<Th[14:10], Jericho is the encoding is Threshold = Th[61:0]
 * in Arad 2^29, 2^32 in Jericho
 */
...
STATIC uint32
    ...
    /* For Qax, CRPS_CNT_PROCESSOR_READr length is 17, so there is no special register for the "small" engine*/
  ...
  )
{
  ...
  for (cnt_proc_ndx = 0; cnt_proc_ndx < ARAD_CNT_NOF_PROCESSOR_IDS(unit); cnt_proc_ndx++) {
      ...
      {
        /* discconnect all sources from counter proccessor */
  ...
  if (SOC_IS_JERICHO(unit) && !SOC_IS_QAX(unit)) {
    ...
    if(SOC_IS_QAX(unit))
    {
        /** Note: we update IRPP but not TM, because there is no sence of counting queues only for first copy, */
        /** because each copy goes to different queue */
      ...
      /*Mark DMA channel as used for CRPS
        ...
        */
        if (crps_fifo <= 0x3) {
            ...
            selected_applicaion = dma_fifo_channel_src_reserved; /*Return to reset-value*/
        } else {
            SOCDNX_IF_ERR_EXIT(BCM_E_PARAM);
        }
     ...
     SOC_SAND_OUT uint8*                          crps_fifo) 
{
    ...
    if (SOC_IS_JERICHO(unit)) {
        ...
        if (selected_applicaion <= 0x3) {
            /*0x0 - CRPS 0-3 ,0x1 - CRPS 4-7 ,0x2 - CRPS 8-11 ,0x3 - CRPS 12-15,*/
            *crps_fifo = selected_applicaion;
        } else if (selected_applicaion <= 0x7) {
            /*0x4 - OAMP Status ,0x5 - OAMP Event ,0x6 - OLP ,0x7 - PPDB CPU REPLY.*/
...
if (!SAL_BOOT_PLISIM ) {
  ...
  rv = _soc_mem_sbus_fifo_dma_start_memreg((unit), channel, FALSE /*is_mem*/, 0, reg_crps_cache_rd_results, MEM_BLOCK_ANY, 0, ARAD_CNT_CACHE_LENGTH(unit), host_buff);
  if (SOC_FAILURE(rv)) {
  ...
  /* Configure Cannels endianess according to the CPU */
       ...
       SOC_SAND_IN  ARAD_CNT_COUNTERS_INFO *info)
{
   ...
   if (info->src_type == ARAD_CNT_SRC_TYPE_INGRESS_OAM || info->src_type == ARAD_CNT_SRC_TYPE_EGRESS_OAM || info->src_type == ARAD_CNT_SRC_TYPES_IPT_LATENCY){
       ...
       if(group_size_val != ARAD_CNT_BMAP_OFFSET_COUNT) {
            ...
            /* if set size 8 - disable the filter field - set it to 8*/
 ...
 *      stif_counter_id - relevanto only for QAX/J+
 *      is_double_entry - relevant only for QAX/J+
 *      engine_range_offset - relevant only for QAX
 * Returns:
 *      SOC_E_xxx
 */
uint32 
   ...
   )
{
    ...
    if (source == SOC_TMC_CNT_SRC_TYPE_EPNI || 
        ...
        /* determine the offset added to the pointer. */
        base_offset_field_val =  
            (((info->num_counters*format_scale) - ((base_val * info->custom_mode_params.set_size) % (info->num_counters*format_scale))) % (info->num_counters*format_scale) /*in case base_val is zero the offset is zero*/) / info->custom_mode_params.set_size;
        /* determine the we_val. In case of PKS_AND_PKS, there is x2 counters, so the we_val is affected by it. (also the pointer offset) */            
        base_counter_we_val = 
            ((base_val * info->custom_mode_params.set_size) / ((info->num_counters)*format_scale));

        if (source == SOC_TMC_CNT_SRC_TYPE_EPNI) 
        {
            if(SOC_IS_JERICHO_PLUS(unit)) /* include also QAX */
            {
    ...
    {    
        if (source == SOC_TMC_CNT_SRC_TYPE_VOQ) 
        {
            ...
            if (!SOC_IS_JERICHO(unit)) {
                /* for Arad devices, calc the max possible queue to count in the engine */                
...
/*****************************************************************
 *      Configure the EPNI global related register
 *      to enable statistics command
 *      if value is 0xD means enable
 *      if value is 0x0 means disable
 *****************************************************************/
uint32 
 ...
 *     crps_config - if FALSE, means statistic interface
*********************************************************************/
uint32 
      ...
      SOC_SAND_IN int                     crps_config)
{
    ...
    if (eg_mode_type == ARAD_CNT_MODE_EG_TYPE_TM || 
        ...
        if ((SOC_IS_JERICHO(unit)) || (SOC_IS_ARAD_B0_AND_ABOVE(unit))) {
            ...
            LOG_ERROR(BSL_LS_BCM_CNT,
                  (BSL_META_U(unit, "unit %d: Unsupported architecture: BCM_88650_A0 \n"), unit));
            goto exit;
        }
    ...
    for(pp_port_ndx = 0;  pp_port_ndx < SOC_TMC_NOF_FAP_PORTS_PER_CORE;  pp_port_ndx++)
    {
        ...
        {
            /* for STIF, there are two different counters */
    ...
    if (!SOC_IS_ARADPLUS_AND_BELOW(unit) && !SOC_IS_JERICHO_PLUS(unit)) {
        ...
        */
        ...
        /* if statistic interface and if it type=out_lif and if Jericho device (a0/b0), need to set the range of the command id to MAX range (in order to keep backward compatability) */
        /* STIF do not use the interface with the profile manager, therefore, it configure the MAX range of LIFs*/
        /* From the other hand, since it capture all of the LIF range, it is not allowed to configure OUT LIF counting in the CRPS on the other command id, because it will cause overlapping ranges */
    ...
    /* if statistic interface and type EPNI - enable the epni statistics*/
...
/* } */
 ...
 SOC_SAND_CHECK_FUNC_RESULT(res, 4, exit);
  ...
  if ((info->src_type == ARAD_CNT_SRC_TYPE_VOQ) || (info->src_type == ARAD_CNT_SRC_TYPE_STAG) || 
      ...
      /* in qax, value 1, mean to take the external statistics ptr that was produce by irpp/pmf */
...
/* } */
  ...
  /* check if latency source - enable/disable latency */
  if(SOC_IS_JERICHO(unit) && info->src_type == SOC_TMC_CNT_SRC_TYPES_IPT_LATENCY) {
      ...
      /* connect/disconnect the IPT to the CRPS if latency source */
      if(info->src_core == 0) {
          if(SOC_IS_QAX(unit)) {
              ...
              /* Configure the CRPS to count latency from SRAM */
  ...
  /* Oam works in double entry mode, the counters are small and may overflow. In such case, the counter need to wrap-arround.
        It is acceptable by the Oam standard and handled by the protocol */
  ...
  /* set source as the last stage of the engine configuration */
  if (SOC_IS_QAX(unit))
  {
      if(src_field_name != INVALIDf)
      {
  ...
  /* if QAX and OAM, need to configure IHP_A and IHP_B the same if multiple_sources=TRUE */
  if(SOC_IS_QAX(unit) && (info->src_type == ARAD_CNT_SRC_TYPE_INGRESS_OAM || info->src_type == ARAD_CNT_SRC_TYPE_EGRESS_OAM) && (info->multiple_sources == TRUE))
  {
    ...
    SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 1253, exit);
    /*configure relevent mode */
    res = soc_reg_above_64_field32_modify(unit, CRPS_CRPS_CNT_SRC_MODEr, REG_PORT_ANY, 0, crps_cnt_src_mode_fiels, (info->format != SOC_TMC_CNT_FORMAT_PKTS_AND_PKTS));
    SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 1255, exit);                
  }
  /* EPNI source - enable counting statistics */
...
/* } */
  ...
  if (ARAD_CNT_SRC_TYPE_IS_EGR_PP_FLD(unit,fld_val)) {
      ...
      /* warning!!! - HW always connected to ingress_Oam, even if user selected EgressOam. therefore, the return src_type will be Ingress_oam
           *  but, user who configure it as EgressOam in the BCM API will get a missmatch between set and get. 
           *  In order overcome it, need to use the SW state in the bcm level in order to know what was the user configuration.
           */
  ...
  if (info->src_type == ARAD_CNT_SRC_TYPE_EPNI) {
      ...
      if(SOC_IS_JERICHO_PLUS(unit))
      {
          /* assume src_type was already set */
          switch (fld_val)
          {
          ...
          case QAX_CNT_MODE_EG_TYPE_VSI_FLD_VAL:
            ...
            info->mode_eg.base_val = fld_val2*2048; /* 2K base val resolution */
            break;
          }                
      }
      else
...
/* check if configurable mode*/
  if (SOC_IS_ARADPLUS(unit)){
    ...
    if(do_not_count_field != INVALIDf){
        ...
        /* if do_not_count_val is equal to the invalid val or if the set size is 8 - configurable mode*/
  ...
  )
{
 ...
 if (direction == SOC_TMC_PORT_DIRECTION_INCOMING) {
     ...
     /** General value will return EGQ value */
 ...
 * Purpose:
 *      set offset field in register EGQ_COUNTER_PROFILE_OFFSET
 *      depending on the src_type and the eg_type - 1-ACE/ 2-OUTLIF/ 3-VSI
 *      
 * Parameters:
 *  SOC_SAND_IN int                                                             unit, 
    ...
    )
{
    ...
    if(src_type != SOC_TMC_CNT_SRC_TYPE_EGR_PP) {
        ...
        if(eg_type == SOC_TMC_CNT_MODE_EG_TYPE_PMF) { /* COUNTER_PROFILE_OFFSET_1 - ACE pointer*/
        ...
        else if(eg_type == SOC_TMC_CNT_MODE_EG_TYPE_OUTLIF) { /* COUNTER_PROFILE_OFFSET_2 - OUTLIF */
        ...
        else if(eg_type == SOC_TMC_CNT_MODE_EG_TYPE_VSI){ /* COUNTER_PROFILE_OFFSET_3 - VSI */
 ...
 * Purpose:
 *      get offset field in register EGQ_COUNTER_PROFILE_OFFSET
 *     depending on the src_type and the eg_type - from which field to read - 1-ACE/ 2-OUTLIF/ 3-VSI
 *      
 * Parameters:
 *  SOC_SAND_IN int                                                             unit, 
 *  SOC_SAND_IN SOC_TMC_CNT_SRC_TYPE                                            src_type,
    ...
    )
{
    ...
    if(src_type == SOC_TMC_CNT_SRC_TYPE_EGR_PP && eg_type == SOC_TMC_CNT_MODE_EG_TYPE_PMF) { /* COUNTER_PROFILE_OFFSET_1 - ACE pointer*/
    ...
    else if(src_type == SOC_TMC_CNT_SRC_TYPE_EGR_PP && eg_type == SOC_TMC_CNT_MODE_EG_TYPE_OUTLIF) { /* COUNTER_PROFILE_OFFSET_2 - OUTLIF */
    ...
    else if(src_type == SOC_TMC_CNT_SRC_TYPE_EGR_PP && eg_type == SOC_TMC_CNT_MODE_EG_TYPE_VSI){ /* COUNTER_PROFILE_OFFSET_3 - VSI */
 ...
 * Purpose:
 *      wrap to the function arad_pmf_counter_set, in order to configure the ingress pp counters for statistics interface purpose.
 *      
 * Parameters:
 *  SOC_SAND_IN  int                                                            unit,
 *  SOC_SAND_IN  int                                                            command_id,
...
/* } */
#endif /* of #if defined(BCM_88660_A0) */
