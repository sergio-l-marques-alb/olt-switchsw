...
/*this priority is just between frist and second*/
    ...
    )
{
    ...
    for(ce_idx = 0; ce_idx < nof_options; ce_idx++)
    {
         ...
         * For KAPS split issue, try to allocate two of 16,
         * only if failed try to allocate a 32-bit CE.
         */
...
/*
 * Given a qualifier type and ce list, get the qual ce info accordingly
 *  
 * return Value:
 *  res - OK or ERROR.
 *  
 ...
 *  qual_type - qualifier list
 *  ce_id - list of ce ids
 *   
 * SOC_SAND_OUT: 
      ...
      )
{
    ...
    /*
     * Break the header/internal qualifier into several CEs according to CE size
     * Then for each CE calculate the lost bits accordingly 32/16 bit CE and for internal
    ...
    while(tmp_size > 0)
    {
        ...
        if(is_header)
        {
            ...
            res = arad_pmf_ce_nof_real_bits_compute_header(
                    ...
                    ce_is_32,    /* is_32b_ce - 0 - CE-16, 1 - CE-32*/
                    &bits_for_ce
                    );
        ...
        else
        {
             ...
             * it in the internal table types
             */
            ...
            if ((flags & ARAD_PP_FP_KEY_ALLOC_CE_FLP_NO_SPLIT) && (qual_type == SOC_PPC_FP_QUAL_IRPP_ALL_ZEROES)) {
                ce_lost_bits = 0; /* no lost bits in special FLP case byte-aligned */
            }
            else {
        ...
        /* The lost_bits of CEs of each qualifier */
...
arad_pp_fp_key_alloc_specific_ce_constrain_calc(
        ...
        SOC_SAND_IN       SOC_PPC_FP_DATABASE_STAGE   stage,
        SOC_SAND_IN       uint32                      flags,
        SOC_SAND_IN       SOC_PPC_FP_QUAL_TYPE        qual_types[ARAD_PP_QUAL_CHAIN_MAX], /* qualifiers*/
        SOC_SAND_IN       uint32                      max_nof_ppc_qual,
        SOC_SAND_IN       int                         ce_id[BCM_FIELD_NOF_CE_ID_PER_QUAL],
        SOC_SAND_OUT      ARAD_PP_FP_CE_CONSTRAINT    ce_const[ARAD_PMF_LOW_LEVEL_NOF_CE_IN_PROG_MAX_ALL_LEVELS], /* CE constraints*/
        ...
        )
{
                                          ...
                                          ; qual_indx++)
    {
        ...
        {
            if(total_ce_indx >= ARAD_PMF_LOW_LEVEL_NOF_CE_IN_PROG_MAX_ALL_LEVELS)
            {
                LOG_ERROR(BSL_LS_SOC_FP,
                          (BSL_META_U(unit,
      ...
      )
{
    ...
    *success = TRUE;
    ...
    /*
    * For QUX large direct lookup only:
    *     Large direct lookup key bits [13:12] are not used for lookup by hardware.
    *     As a result, a qualifier is split into 2 CEs if it takes bits above 13 in the key.
    *     Those two bits hole is compensated by copying additional two bits in CE 0,
    *     minus the lost bits in CE 1.
    */
  ...
  else
  {
      if (SOC_IS_JERICHO_PLUS(unit) 
        ...
        && (place_cons == ARAD_PP_FP_KEY_CE_PLACE_HASH_KEY)) {
          /*
           * CE for cascadey key(7) and hash_key(12~15) is const
           */
  ...
  )
{
    ...
    *fit_ce = arad_pp_fp_key_get_available_ce_under_constraint(unit, stage, place_cons,size_cons,group_cons);
    ...
    /* 32 bit CE index is higher that 16 bit CE, 
     * make sure 32 bits CE index is low than 16 bit CE index
     */
    if ((flags & ARAD_PP_FP_KEY_ALLOC_ELK_RANGE) && (place_cons == ARAD_PP_FP_KEY_CE_LOW)) {
        if (size_cons != ARAD_PP_FP_KEY_CE_SIZE_16) {
            *fit_ce = *fit_ce & 0x00f0; /*index should be 4~7*/
        } else {
            *fit_ce = *fit_ce & 0x0f00; /*index should be 8~11*/
        }
    }
  ...
  )
{
    ...
    for(ce_indx = 0; ce_indx < alloc_info->nof_ce; ++ce_indx) {
        ...
        if (!(flags&ARAD_PP_FP_KEY_ALLOC_CHECK_ONLY))
        {
            if (flags & ARAD_PP_FP_KEY_ALLOC_CE_WITH_ID)
            {
                /* In case this is a shared ce we need to add the new db_id to the already existing configuration,
                   instead of creating a new one. Thus, load the information about the previous installations */
                res = sw_state_access[unit].dpp.soc.arad.tm.pmf.pgm_ce.get(
                        unit,
                        stage,
                        prog_ndx,
                        alloc_info->cycle, /* lookup num*/
                        alloc_info->ce[ce_indx], /* ce in lsb/msb*/
                        &sw_db_ce
                      );
                /* This ce id was not used in a field group, thus we need to init the db_ids array.
                   In case the ce_id was already used, the is_ce_shared would be true and the db_id array would alredy include 
                   relevant ids.*/
        ...
        if(is_header) {
          ...
          /* fill cur header offset */
            ...
            sw_db_ce.msb -= sw_db_ce.msb_padding; /* exclude padding bits */
            ...
            if (!(flags&ARAD_PP_FP_KEY_ALLOC_CHECK_ONLY)) {
                if (!(SOC_IS_JERICHO_PLUS(unit) 
                    ...
                    /* In JER+/QAX, there's no need to allocate CE for cascaded key or SLB hash key, since
        ...
        if (!(flags&ARAD_PP_FP_KEY_ALLOC_CHECK_ONLY)) {
            ...
            if (flags & ARAD_PP_FP_KEY_ALLOC_CE_WITH_ID)
            {
                ...
                /* Find an empty spot iside the ce_sharing_info */
                for (ii = 0; ii < 8; ii++)
                {
                ...
                || is_equal || is_slb_hash_in_quals))
        {
            ...
            if (is_cascaded) 
            {
                ...
                if (SOC_IS_JERICHO_PLUS(unit)) 
                {
                    ...
                    /*For cascaded on JR+/QAX, should first create 2nd cycle DB, then the 1st cycle DB */
                    ...
                    {
                        /*
                         * When configuring 2nd cycle FG, configure CE at update key stage to copy first TCAM lookup result for place holder.
                         * When configuring 1st cycle FG, configure CE at update key stage to copy from 1st cycle TCAM lookup result.
                         */
                        if (lookup_id == 0) {                            
                    ...
                    /* get cascaded key for 2nd cycle */
                    if (lookup_id != 0) {
                        cascaded_key_id = alloc_info->key_id[0];
                    }

                    /* write to CE HW
                     * hash key is consuming CEs 4, 5, and 6
                     */
                    res = arad_pmf_ce_internal_info_entry_set_unsafe(
                            unit,
                            stage,
                        ...
                        for (qual_index=0; qual_index<SOC_PPC_FP_NOF_QUALS_PER_DB_MAX; ++qual_index) {
                            ...
                            if (qual_types[qual_index] == SOC_PPC_FP_QUAL_KEY_AFTER_HASHING) {
                                ...
                                if (SOC_PPC_FP_IS_QUAL_TYPE_USER_DEFINED(qual_types[qual_index])) {
                                    /* Only copy necessary number of bits */
                            ...
                            if (is_slb_hash_found) {
                                /* FIXEM: replace the code with arad_pmf_ce_internal_info_entry_set_unsafe */
                                SOC_DPP_CORES_ITER(SOC_CORE_ALL, core) {
                                ...
                                /* Assume that we only have one UDF of SLB hashing key */
                                break;
                            }
                        }
                    } else {
          ...
          for (cycle = cycle_first; cycle <= cycle_last; ++cycle) {
              ...
              for(ce_index = 0; ce_index  < nof_ce_const; ++ce_index) {
                  ...
                  res = arad_pp_fp_key_alloc_ce(
                           ...
                           prog_ndx,
                           cycle,
                           alloc_place_tmp,/* qual constraint & my constraint */
  ...
  )
{
  ...
  *key_alloced = 0;
  ...
  if ((stage == SOC_PPC_FP_DATABASE_STAGE_INGRESS_FLP) && (!(flags & ARAD_PP_FP_KEY_ALLOC_CE_CHECK_ONLY))) {
      ...
      /*
      * In case of ELK ACL, pad key segments to be byte-aligned.
      */
    ...
    )
{
  ...
  /*
  * If nof_quals is less than 2, return directly
  */
  if (nof_quals < 2) {
            ...
            *found = TRUE;
            ...
            for(qual_indx = 0;  (qual_indx < nof_quals) && (*found); ++qual_indx) 
            {
              ...
              for(ce_indx = 0; ce_indx < ce_info[qual_indx].nof_ce; ++ce_indx) 
              {
                ...
                /* 
                 * For JR+: slb hash always select CE 12~15 in updated Key, match it. 
                 */
  ...
  )
{
    ...
    /* Check whether there is a hash value qualifier (data from SLB 74b LEM key). */
    res = arad_pp_fp_is_qual_in_qual_arr(unit, 
                                             qual_types, 
                                             ARAD_PMF_LOW_LEVEL_NOF_CE_IN_PROG_MAX_ALL_LEVELS, 
                                             SOC_PPC_FP_QUAL_KEY_AFTER_HASHING, 
                                             &is_slb_hash_in_quals);
    ...
    if ((is_cascaded && (lookup_id == 0)) || is_equal || is_slb_hash_in_quals) {
        if (SOC_IS_ARADPLUS(unit) && !SOC_IS_JERICHO_PLUS(unit)) {
            uint32 *key_bits[4];
            int i = 0;
...
{    
    ...
    if (is_for_kbp) {		 
        if (is_from_dbal) {
            ...
            if (fp_database_info_l->flags & SOC_PPC_FP_DATABASE_INFO_FLAGS_EXTENDED_DATABASES) {
                frwrd_table_id = fp_database_info_l->internal_table_id; /* the table ID is updated in the BCM layer according to the table in the KBP*/
            } else {
                frwrd_table_id = (ARAD_KBP_ACL_TABLE_ID_OFFSET + db_id);
            }
        }

        /* if kbp flexible mode is set, get tables size only if table is valid */
    ...
    /* 
     * Check for share qual
     * Get share qual data from LSB of entry data 
     */
    if (is_for_kbp && (direction == ARAD_PP_FP_KEY_BUFFER_DIRECTION_BUFFER_TO_KEY)) {
        ...
        if (dip_sip_sharing_enabled) {
            ...
            /* get data for share qual */
            /*
             * get share qual first 
             * Share qual position in share zone:
             *        MSB----------------------------------->LSB
             *  IPv4: SIP-4B----DIP-4B
             *  IPv6: SIP-8B----SIP-8B----DIP-8B----DIP-8B
             */
            ...
            /* put date into share qual structure */
    ...
    for (key_ndx = 0; key_ndx <= is_key_320b; key_ndx++) {
        ...
        for(ce_indx = 0; ce_indx < nof_ces_to_handle; ++ce_indx) {
            ...
            if(!qual_found) {
                ...
                buf_msb -= (sw_db_ce.msb_padding + sw_db_ce.msb + 1); /* sw_db_ce.lsb + sw_db_ce.msb - sw_db_ce.lsb + 1;*/
                if (moved_to_msb == 0) {
                    nof_lsb[key_ndx] = (ARAD_PP_FP_TCAM_ENTRY_SIZE*32) - buf_msb - first_key_size;
                }
                LOG_DEBUG(BSL_LS_SOC_FP,
