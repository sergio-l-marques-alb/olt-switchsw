...
soc_error_t soc_jer_oamp_rfc6374_lm_dm_config(int unit, uint8 is_ilm, uint8 timestamp_format){
    ...
    {
      /** 
      * Set the RFC DM timestamp format
...
/* Configure the RFC6374 LM channel type */
soc_error_t soc_jer_pp_oam_rfc_6374_lm_config(int unit, uint32 exp_field_value) {
  ...
  /* For optimization write only if it's requried */
...
/* Configure the RFC6374 DM QTF, RTF, RPTF fields */
soc_error_t soc_jer_pp_oam_rfc_6374_dm_config(int unit, uint32 exp_field_value) {
  ...
  SOCDNX_IF_ERR_EXIT(rv);
  /** Since the fields are always set the same, check only the first one */
...
/* Adds init static entries to for LM/DM RFC6374 
...
soc_error_t soc_jer_pp_oam_rfc_6374_lm_dm_config_init(int unit) {
  ...
  /**
  * Version = 0
  * Flags = 0000b (Query, No TC specific)
  * Control-Code =  0 (In-band Response Requested)
  ...
  */
  ...
  /**
  * Version = 0
  * Flags = 1000b (Response, No TC specific)
  * Control-Code = 1 (operation successful)
  * Message Length = 52 (bytes)
  */
  ...
  */
  ...
  /** 
  *  Version = 0
  *  Flags = 1000b (Query, No TC specific)
  *  Control-Code = 1 (In-band Response Requested)
  *  Message Length = 44(bytes)
  */
...
soc_error_t soc_jer_pp_oam_oem1_mep_add(
    ...
    /* Disallow only MIP->MEP case, MEP->MIP case and Active->Active or Passive->Passive or MIP->MIP cases */
    if (!update && ((prev_mdl_mp_type==_JER_PP_OAM_MDL_MP_TYPE_MIP && is_mep) ||
        ...
        /* To allow valid scenarios of mep,mip or mip,mip, we are checking here for fake passive match for above mdl values */
    ...
    if ((is_active) || (!(SOC_PPC_OAM_IS_MEP_TYPE_Y1731(classifier_mep_entry->mep_type) &&
        ...
        /* Do not modify in case you are having an active match already and you would want to
         * change it to passive match for the new MEP - Two MEPs on same LIF, same mdl but different direction case */
    ...
    if (SOC_IS_QUX(unit)) {
        /* direction bit must be statically set on counter pointer for ingress OAM classification.
        QUX only, hw issue. */
        ...
        if ((is_upmep && !is_active) || (!is_upmep && is_active)) {
            /* Ingress classification (Down MEP active / Up MEP passive) */
    ...
    *new_mp_type_vector = oem1_payload.mp_type_vector;
    ...
    if (is_first_real_upper_is_mip) {
        ...
        if (change_to_passive) {
                /* change to passive (instead of No MP) in order to do passive filtering
                 * for the other MEP on same level same lif but different
                 * direction */
                JERICHO_PP_OAM_SET_MDL_MP_TYPE_VECTOR_BY_LEVEL(*new_mp_type_vector, _JER_PP_OAM_MDL_MP_TYPE_PASSIVE_MATCH, classifier_mep_entry->md_level);
        } else {
...
/* Set OAMP_BFD_IPV4_CONTROLr.BFD_IPV4_CHECK_UDP_SRC_PORT 
 * to 0 or 1 based on enable/disable */
soc_error_t
  ...
  )
{
    ...
    if (SOC_IS_QAX(unit)) {
        ...
        /* In QAX, we could have disabled discr check or UDP src port check. 
         * So get those values so that we do not un-intentionally zero them
         * out due to OAM */
   ...
   )
{
    ...
    if (ts_format == bcmOAMTimestampFormatNTP) {
        ...
        /* write value to leap_sec_control register  */
...
soc_error_t soc_jer_pp_oam_init_response_table(int unit) {
      ...
      for (i = 0; i < sizeof(valid_opcodes) / sizeof(int); ++i) {
          ...
          /* The table OAMP_RSP_TCAM has been changed to dynamic table in QUX. */
          if (SOC_IS_QUX(unit)) {
     ...
     SOC_SAND_OUT int *header_offset)
{
    ...
    if((!lb_tst_info->is_up_mep) && (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "injection_with_user_header_enable", 0 /*DEFAULT VALUE*/) == 1))
    {
                                                                           ...
                                                                           SOC_SAND_INOUT _oam_oam_a_b_table_buffer_t  *oamb_buffer)
{
    ...
    /* in order to differentiate between injected CCMs and Passive side trapped CCMs.
     *  injected ccm will use 0x64, then use PG to modify it to 1*/
    ...
    for (internal_opcode = 1; internal_opcode < SOC_PPC_OAM_OPCODE_MAP_COUNT; internal_opcode++) {
        ...
        for(cur_mp_type = 0; cur_mp_type < (sizeof(mp_types_qmx) / sizeof(uint32)); ++cur_mp_type){
            ...
            /* iterate over all possible mp-profiles.*/
 ...
 *   PROC
 * FUNCTION:
 *   Set OAMP RFC-6374 LM query/response header's control code
 * INPUT:
 *   int unit
 *   uint8 is_response
 *   uint32 rfc6374_lm_control_code
 * REMARKS:
 *   None.
 * RETURNS:
 *   OK or ERROR indication.
...
soc_error_t soc_jer_oamp_rfc6374_lm_control_code_set(int unit, uint8 is_response, uint32 rfc6374_lm_control_code) {
   ...
   SOCDNX_IF_ERR_EXIT(rv);
    ...
    if (is_response) {
        /* Read RFC_6374_LM_RSP_DW_0f value */
 ...
 *   PROC
 * FUNCTION:
 *   Get OAMP RFC-6374 LM query/response header's control code
 * INPUT:
 *   int unit
 *   uint8 is_response
 *   uint32 *rfc6374_lm_control_code
 ...
 *   PROC
 * FUNCTION:
 *   Set OAMP RFC-6374 DM query/response header's control code
 * INPUT:
 *   int unit
 *   uint8 is_response
 *   uint32 rfc6374_dm_control_code
 * REMARKS:
 *   None.
 * RETURNS:
 *   OK or ERROR indication.
...
soc_error_t soc_jer_oamp_rfc6374_dm_control_code_set(int unit, uint8 is_response, uint32 rfc6374_dm_control_code) {
   ...
   SOCDNX_IF_ERR_EXIT(rv);
    ...
    if (is_response) {
        /* Read RFC_6374_DM_RSP_DW_0f value */
 ...
 *   PROC
 * FUNCTION:
 *   Get OAMP RFC-6374 DM query/response header's control code
 * INPUT:
 *   int unit
 *   uint8 is_response
 *   uint32 *rfc6374_dm_control_code
...
/*********************************************************************
* NAME:
 *   soc_jer_oamp_rfc6374_rx_trap_codes_set
 * TYPE:
 *   PROC
 * FUNCTION:
 *   Set OAMP RFC-6374 LM/DM RX trap code
 * INPUT:
 *   int unit
 *   uint8 is_rfc6374_lm
 *   uint8 is_rfc6374_dm
...
/*********************************************************************
* NAME:
 *   soc_jer_oamp_rfc6374_rx_filter_table_set
 * TYPE:
 *   PROC
 * FUNCTION:
 *   Set OAMP RFC-6374 query/response action per control code
 * INPUT:
 *   int unit
 *   int control_code
 *   uint8 is_response
...
soc_error_t soc_jer_oamp_rfc6374_rx_filter_table_set(int unit, int control_code, uint8 is_response, uint32 punt_2_ignore_1_pass_0) {
    ...
    {
        /** Control Punt/Ingnore for control_code[2:0] == 0 */
        case 0:
        {
          ...
          break;
        /** Control Punt/Ingnore for control_code[2:0] == 1 */
        case 1:
        {
            if (punt_2_ignore_1_pass_0 == 0) /* Enable the packet */
            {
          ...
          break;
        /** Control Punt/Ingnore for control_code[2:0] == 2 */
        case 2:
        {
            if (punt_2_ignore_1_pass_0 == 0) /* Enable the packet */
            {
          ...
          break;
        /** Control Punt/Ingnore for control_code[2:0] == 3 */
        case 3:
        {
            if (punt_2_ignore_1_pass_0 == 0) /* Enable the packet */
            {
...
/*********************************************************************
* NAME:
 *   soc_jer_oamp_rfc6374_rx_filter_table_get
 * TYPE:
 *   PROC
 * FUNCTION:
 *   Get OAMP RFC-6374 query/response action per control code
 * INPUT:
 *   int unit
 *   int control_code
 *   uint8 is_response
...
soc_error_t soc_jer_oamp_rfc6374_rx_filter_table_get(int unit, int control_code, uint8 is_response, uint32 * punt_2_ignore_1_pass_0) {
    ...
    switch (control_code & 0x3)
    {
        /** Control Punt/Ingnore for control_code[1:0] == 0 */
        case 0:
        {
