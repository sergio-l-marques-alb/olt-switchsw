    ...
    for (i = 0; i < retries; ++i) {
        ...
        if (option == 1) {
            ...
            for (k = 0; k < nof_rop_tests; ++k) {
                /** coverity: If option is "1", core is never set to 2 */
                /* coverity[overrun-call] */
                rv += arad_kbp_init_rop_test(unit, core); 
            }
        } else if (option == 2) {
            /** coverity: If option is "2", core is never set to 2 */
            /* coverity[overrun-call] */
            rv = arad_kbp_blk_lut_set(unit, core);
        } else if (option == 3) {
            /** coverity: If option is "3", arad_kbp_rop_write can handle broadcast scenario */
            /* coverity[overrun-call] */
            rv = arad_kbp_rop_write(unit, core, (arad_kbp_rop_write_t *)buf);
        } else if (option == 4) {
            /** coverity: If option is "4", core is never set to 2 */
            /* coverity[overrun-call] */
