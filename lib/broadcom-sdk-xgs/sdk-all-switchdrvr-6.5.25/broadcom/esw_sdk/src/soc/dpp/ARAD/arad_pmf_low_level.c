...
CONST STATIC
    ...
    /* KeyA - = {SYSTEM_PORT_ID_PREFIX=001,DSP_EXTENSION[15:0] */
    {2, 1,  0, SOC_PPC_FP_QUAL_IRPP_ALL_ZEROES,                 2,  0,  SOC_SAND_BIT(ARAD_PMF_KEY_A)}, /* 2 bits zeroes */
    {2, 1,  1, SOC_PPC_FP_QUAL_IRPP_ALL_ONES,                   1,  0,  SOC_SAND_BIT(ARAD_PMF_KEY_A)}, /* 1 bit one */
/* Copied before:{2, 1,  2, SOC_PPC_FP_QUAL_HDR_DSP_EXTENSION_AFTER_FTMH,   16,  0,  SOC_SAND_BIT(ARAD_PMF_KEY_A)}, *//* DSP_EXTENSION - inverted with Out-LIF if custom_feature_egress_snooping_advanced = 1*/
    {2, 1,  3, SOC_PPC_FP_QUAL_HDR_STACKING_EXT_AFTER_DSP_EXT, 16,  0,  SOC_SAND_BIT(ARAD_PMF_KEY_A)}, /* STACKING_EXTENTION*/

    /* KeyB - = {MCID_PREFIX=10,MCID[16:0] */
    {2, 0,  0, SOC_PPC_FP_QUAL_IRPP_ALL_ONES,                   1,  0,  SOC_SAND_BIT(ARAD_PMF_KEY_B)}, /* 1 bit one*/
    {2, 0,  1, SOC_PPC_FP_QUAL_IRPP_ALL_ZEROES,                 1,  0,  SOC_SAND_BIT(ARAD_PMF_KEY_B)}, /* 1 bits zeroes */
    {2, 0,  2, SOC_PPC_FP_QUAL_HDR_FTMH,                        1, 51,  SOC_SAND_BIT(ARAD_PMF_KEY_B)}, /* MCID[16] */
    {2, 0,  3, SOC_PPC_FP_QUAL_HDR_FTMH,                       16, 52,  SOC_SAND_BIT(ARAD_PMF_KEY_B)}, /* MCID[15:0] */

#ifdef ARAD_PMF_CPU_TO_CPU_WA_STACKING_ENABLED
    /* CPU to CPU wa imp. */
    {2, 1,  4, SOC_PPC_FP_QUAL_IRPP_SRC_PP_PORT           ,      9, 0,  SOC_SAND_BIT(ARAD_PMF_KEY_B)}, /* Mapped-pp-port which carries core information, instead of PTC port in ARAD */
    {2, 1,  5, SOC_PPC_FP_QUAL_HDR_STACKING_EXT_AFTER_DSP_EXT,   8, 0,  SOC_SAND_BIT(ARAD_PMF_KEY_B)}, /* FTMH Stacking-Extension[15:8] */
    {2, 1,  6, SOC_PPC_FP_QUAL_IRPP_PMF_PGM_KEY_GEN_VAR,         4, 8,  SOC_SAND_BIT(ARAD_PMF_KEY_B)}, /* ProgramKeyGenVar with the new Header-Profile */
    {2, 1,  7, SOC_PPC_FP_QUAL_HDR_STACKING_EXT_AFTER_DSP_EXT,   8, 0,  SOC_SAND_BIT(ARAD_PMF_KEY_B)}, /* FTMH Stacking-Extension[15:8] */
#endif /* ARAD_PMF_CPU_TO_CPU_WA_STACKING_ENABLED */
...
/* Defined in Key-C {2, 0, 2, SOC_PPC_FP_QUAL_PACKET_FORMAT_QUALIFIER0,    1, 0,  SOC_SAND_BIT(ARAD_PMF_KEY_B) | SOC_SAND_BIT(ARAD_PMF_KEY_C)},   */
    ...
    {2, 0, 9, SOC_PPC_FP_QUAL_IRPP_PKT_HDR_TYPE,         1, 0, SOC_SAND_BIT(ARAD_PMF_KEY_B)}, /* PFC0 of IS */
    ...
    {2, 0,  13, SOC_PPC_FP_QUAL_HDR_ITMH,                  17, 31-16, SOC_SAND_BIT(ARAD_PMF_KEY_C)}, /* ITMH[16:0]. Extracted to UDH2 and later on stamped by IPT as outlif */
    {2, 0,  14, SOC_PPC_FP_QUAL_IRPP_PKT_HDR_TYPE,         3, 0, SOC_SAND_BIT(ARAD_PMF_KEY_C)}, /* PFC[2:0] */
    ...
    {2, 0, 8, SOC_PPC_FP_QUAL_IRPP_ALL_ZEROES,          6, 0, SOC_SAND_BIT(ARAD_PMF_KEY_C) | SOC_SAND_BIT(ARAD_PMF_KEY_D)}, /* 6 zeroes */
    ...
    {2, 0,12, SOC_PPC_FP_QUAL_IRPP_PARSER_LEAF_CONTEXT,     0, 0, SOC_SAND_BIT(ARAD_PMF_KEY_D)}, /* Parser-Leaf-Context */
    ...
    {2, 1,11, SOC_PPC_FP_QUAL_IRPP_PARSER_LEAF_CONTEXT,     1, 0, SOC_SAND_BIT(ARAD_PMF_KEY_D)}, /* Parser-Leaf-Context bit 0 */
...
/* Defined in Key-C {2, 0, 2, SOC_PPC_FP_QUAL_PACKET_FORMAT_QUALIFIER0,    1, 0,  SOC_SAND_BIT(ARAD_PMF_KEY_B) | SOC_SAND_BIT(ARAD_PMF_KEY_C)},   */
    ...
    {2, 0,11, SOC_PPC_FP_QUAL_IRPP_PARSER_LEAF_CONTEXT,     0, 0, SOC_SAND_BIT(ARAD_PMF_KEY_D)}, /* Parser-Leaf-Context */
    ...
    {2, 1,15, SOC_PPC_FP_QUAL_IRPP_PARSER_LEAF_CONTEXT,     1, 0, SOC_SAND_BIT(ARAD_PMF_KEY_D)}, /* Parser-Leaf-Context bit 0 */
};
  ...
  )
{
  ...
  /* Check if using oam statistics with stacking */
  ...
  for (pmf_pgm_type_ndx = 0; pmf_pgm_type_ndx < (ARAD_PMF_PGM_TYPE_TM + is_oam_snoop_ftmh_pmf_pgm_to_build); pmf_pgm_type_ndx++)
  {
      if (soc_property_suffix_num_get(unit, -1, spn_FIELD_INGRESS_DEFAULT_PGM_LOAD_DISABLE, default_pgm_string[pmf_pgm_type_ndx], 0)) {
          /* using XGS program for oam statistics with stacking feature */
  ...
  )
{
  ...
  soc_IHB_PMF_PROGRAM_SELECTION_CAMm_field32_set(unit, pgm_sel_tbl_data, MASK_PACKET_FORMAT_CODEf, pfc_pmf_mask); /* Only MSB valid mask */
  ...
  )
{
  ...
  /*
  * For OAM snoop WA, program selection is added in arad_pmf_prog_select_eth_init.
  * This program selection entry is added for stacking program.
  */
  ...
  if (headers_mode == ARAD_PP_SYSTEM_HEADERS_MODE_ARAD || headers_mode == ARAD_PP_SYSTEM_HEADERS_MODE_JERICHO)
  {
      ...
      if ((SOC_DPP_CONFIG(unit)->pp.oam_statistics) && !(is_for_oam_snoop_was) && !is_oam_stats_with_stacking) {
          /* Last 3rd and front 4 CEs re-used by OAM statistics. ce_ndx points to the last 3rd entry in the array. */
          ce_ndx = pmf_key_construction_info_length - 1;
          /* Skip last two entries */ 
          ce_ndx -= 2;
          ...
          /* feature two_pass_mc_dest_flow uses 2 pass FLP lookup destination result instead of FTMH header */
          if (SOC_IS_JERICHO(unit) && soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "two_pass_mc_dest_flow", 0)){
              for (i = 0; i < pmf_fes_construction_info_length; i++) {
                  if (pmf_fes_construction_info[i].fes_info.action_type == SOC_PPC_FP_ACTION_TYPE_DEST) {
                      /* FES 15 replace FES 12 and FES 14 to invalid original DEST action */
      ...
      if (is_for_oam_snoop_was && (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "egress_snooping_advanced", 0))) {
          ...
          if (SOC_IS_JERICHO(unit)) {
              ARAD_PMF_KEY_CONSTR  outlif_lookup_key =
                  {2, 1, 15, SOC_PPC_FP_QUAL_LEM_1ST_LOOKUP_RESULT,          21, 16,  SOC_SAND_BIT(ARAD_PMF_KEY_C)};    /* global outlif from lem lookup result*/
              ARAD_PMF_KEY_CONSTR  fwd_code_key[2] = {                                                                  /* fwd_code 0xc(1100) */
                  {2, 1, 14, SOC_PPC_FP_QUAL_IRPP_ALL_ZEROES,                2,   0,  SOC_SAND_BIT(ARAD_PMF_KEY_C)},        /* 2 bits zero */
                  {2, 1, 13, SOC_PPC_FP_QUAL_IRPP_ALL_ONES,                  2,   0,  SOC_SAND_BIT(ARAD_PMF_KEY_C)}};       /* 2 bits one */
  ...
  if (SOC_IS_JERICHO(unit) && is_for_oam_snoop_was) {
      /*
       *  1. Remove 11 bytes, including ftmh and dsp extension built in PRGE in 1st pass
       *  2. Re-build new ftmh header in LBP
       */
      ...
      pgm_info.bytes_to_rmv.nof_bytes = 11;
       ...
       /*
       *  1. Remove 11 bytes, including ftmh and dsp extension built in PRGE in 1st pass
       *  2. Re-build new ftmh header, not including PPH header, in LBP
       */
      pgm_info.header_profile = SOC_TMC_PMF_PGM_HEADER_PROFILE_TM_UNICAST;
      pgm_info.bytes_to_rmv.header_type = SOC_TMC_PMF_PGM_BYTES_TO_RMV_HDR_1ST; /* As HW */
      pgm_info.bytes_to_rmv.nof_bytes = 11;
  } else {
...
/*
 * oam statistics when a stacking port is configured is enabled PMF Program
 */
uint32
  ...
  )
{
  ...
  */
  ...
  /* Set the Value and mask of InPortProfile and PacketFormatCode - others are masked */
  soc_IHB_PMF_PROGRAM_SELECTION_CAMm_field32_set(unit, pgm_sel_tbl_data, IN_PORT_PROFILEf, ARAD_PMF_PORT_PROFILE_XGS_TM); /* FTMH */
  soc_IHB_PMF_PROGRAM_SELECTION_CAMm_field32_set(unit, pgm_sel_tbl_data, MASK_IN_PORT_PROFILEf, 0x0); /* 3'b000 */
  soc_IHB_PMF_PROGRAM_SELECTION_CAMm_field32_set(unit, pgm_sel_tbl_data, PTC_PROFILEf, 0); /* Always PTC-Profile = 0 */
  soc_IHB_PMF_PROGRAM_SELECTION_CAMm_field32_set(unit, pgm_sel_tbl_data, MASK_PTC_PROFILEf, 0x0); /* 3b valid mask */
  ...
  res = dpp_parser_pfc_get_pmf_by_sw(unit, DPP_PFC_ANY_TM, &pfc_pmf, &pfc_pmf_mask);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 50, exit);
  /* printf("pfc_pmf: %d, pfc_pmf_mask: %d\n", pfc_pmf, pfc_pmf_mask); */
