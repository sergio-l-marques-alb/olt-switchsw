...
uint32 soc_jer_irr_tbls_init(int unit)
{
    ...
    SOCDNX_IF_ERR_EXIT(sand_fill_table_with_entry(unit, IRR_DESTINATION_TABLEm, MEM_BLOCK_ANY, m)); /* fill table with the entry */
...
uint32 soc_jer_ihp_tbls_init(int unit)
{
    ...
    /** port infor: use_dei true, default trap code 0xF7*/
...
uint32 soc_jer_iqm_tbls_init( int unit)
{
    ...
    SOCDNX_IF_ERR_EXIT(sand_fill_table_with_entry(unit, IQM_PQWQm, IQM_BLOCK(unit, SOC_CORE_ALL),  table_default)); /* fill table with the entry */
...
uint32 soc_jer_egq_tbls_init(int unit)
{
    ...
    SOCDNX_IF_ERR_EXIT(sand_fill_table_with_entry(unit, EDB_MAP_OUTLIF_TO_DSPm, MEM_BLOCK_ALL, data)); /* map all CUDs to a none valid port */
...
int soc_jer_static_tbls_reset (int unit)
{
    ...
    for (mem_iter = 0; mem_iter < NUM_SOC_MEM; mem_iter++) 
    {
        if (SOC_MEM_IS_VALID(unit, mem_iter) && /* Memory must be valid for the device */
            ...
            /* the dma not work for this 2 edb tables so we use slow sbus transactions.*/
...
static soc_mem_t jerplus_tbls_excluded_mem_list[] = {
    ...
    ILKN_PML_PORT_0_CPU_ACCESSm,
    ILKN_PML_PORT_1_CPU_ACCESSm,
    SCH_SCHEDULER_INITm,        /** not a real table */
...
STATIC soc_reg_t jer_plus_dynamic_mem_disable_regs[] = {
    ...
    /* Access to dynamic memories may be needed for EGQ, EPNI */
    FCR_ENABLE_DYNAMIC_MEMORY_ACCESSr,
    FDT_ENABLE_DYNAMIC_MEMORY_ACCESSr,
    IDR_ENABLE_DYNAMIC_MEMORY_ACCESSr,
    /* Access to dynamic memories needed for IHB */
    /* Access to dynamic memories may be needed for IHP */
    ILKN_PMH_ENABLE_DYNAMIC_MEMORY_ACCESSr,
    ILKN_PML_ENABLE_DYNAMIC_MEMORY_ACCESSr,
    IPST_ENABLE_DYNAMIC_MEMORY_ACCESSr,
    IPS_ENABLE_DYNAMIC_MEMORY_ACCESSr,
    IPT_ENABLE_DYNAMIC_MEMORY_ACCESSr,
    /* Access to dynamic memories needed for IQMT */
    IQM_ENABLE_DYNAMIC_MEMORY_ACCESSr,
    IRE_ENABLE_DYNAMIC_MEMORY_ACCESSr,
    IRR_ENABLE_DYNAMIC_MEMORY_ACCESSr,
    MMU_ENABLE_DYNAMIC_MEMORY_ACCESSr,
    MRPS_ENABLE_DYNAMIC_MEMORY_ACCESSr,
    MTRPS_EM_ENABLE_DYNAMIC_MEMORY_ACCESSr,
    /* Access to dynamic memories may be needed for OAMP */
    OCB_ENABLE_DYNAMIC_MEMORY_ACCESSr,
    /* Access to dynamic memories may be needed for OLP */
    /* Access to dynamic memories needed for PPDB_{A,B} */
    RTP_ENABLE_DYNAMIC_MEMORY_ACCESSr,
    SCH_ENABLE_DYNAMIC_MEMORY_ACCESSr,
    /* Access to dynamic memories needed for KAPS */
    /* Access to dynamic memories needed for KAPS_BBS */
    INVALIDr
};
...
static uint32  soc_jerplus_idr_tbls_init (int unit){
    ...
    /** entries 48 - 63 are representing entries in which IDR_IRDB indicated on FMC usage. for thise entries we would like to use FMC as well */
...
static uint32 soc_jerplus_kaps_tbls_init(int unit)
{
    ...
    SOCDNX_INIT_FUNC_DEFS;
    SOC_REG_ABOVE_64_ALLONES(data);
    /* WA for Jericho+: indirect_wr_mask register masking mechanism is opposite for KAPS_TCAM_HIT_INDICATION table */
...
int soc_jerplus_tbls_init(int unit)
{
    ...
    SOCDNX_IF_ERR_EXIT(soc_sand_control_dynamic_mem_writes(unit, jer_plus_dynamic_mem_enable_regs, 1)); /* enable dynamic memory writes */
    ...
    SOCDNX_IF_ERR_EXIT(soc_jer_ips_tbls_init(unit)); /* IPS block */
    ...
    SOCDNX_IF_ERR_EXIT(soc_jerplus_kaps_tbls_init(unit)); /* KAPS block */
    ...
    SOCDNX_IF_ERR_EXIT(soc_sand_control_dynamic_mem_writes(unit, jer_plus_dynamic_mem_disable_regs, 0)); /* disable dynamic memory writes */
