...
/*SOC_DPP_DEFS_GET((unit), nof_flp_key_zone_bits)*/
...
typedef struct
{
    ...
    uint8                  private_db_id;
    JER_KAPS_ENTRY_ITER    *private_table_iterator;
    JER_KAPS_DB            *public_table_p;
    uint8                   public_db_id;
    JER_KAPS_ENTRY_ITER    *public_table_iterator;
} DBAL_KAPS_ITERATOR;
#endif
...
typedef struct
{
    ...
    /* Parameters used for lem_get_block */
    ...
    arad_pp_dbal_next_available_ce_get(int unit, int qual_nof_bits, uint8 use_ce_32, uint32 last_unchecked_ce, SOC_PPC_FP_DATABASE_STAGE stage, uint32 ce_rsrc, SOC_DPP_DBAL_CE_SEARCH_MODE mode, int is_msb, ARAD_PP_KEY_CE_ID* free_ce)
{
    ...
    *ce_rsrc_lcl = ce_rsrc;
    ...
    if((int)ce_ndx < low_limit) { /* no available CE */
    ...
    arad_pp_dbal_next_available_key_get(int unit, SOC_PPC_FP_DATABASE_STAGE stage, uint32 prog_id, SOC_DPP_DBAL_PHYSICAL_DB_TYPES db_type, SOC_DPP_DBAL_PROGRAM_KEYS* key_id)
{
    ...
    for (curr_key = SOC_DPP_DBAL_PROGRAM_KEY_A; curr_key < ARAD_PMF_LOW_LEVEL_NOF_KEYS; curr_key++) {
        ...
        if (!SOC_IS_JERICHO(unit) && db_type == SOC_DPP_DBAL_PHYSICAL_DB_TYPE_TCAM) {
            curr_key = SOC_DPP_DBAL_PROGRAM_KEY_C; /* in arad+, the only available key for TCAM is C  */
        }
    ...
    arad_pp_dbal_tcam_table_init(int unit, SOC_DPP_DBAL_SW_TABLE_IDS table_id, SOC_PPC_FP_DATABASE_STAGE stage)
{
    ...
    case SOC_DPP_DBAL_ATI_NONE: /* create tcam DB (dynamic allocation) */
    ...
    case SOC_DPP_DBAL_ATI_TCAM_DUMMY: /* NO need to create tcam DB */
        break;

    case SOC_DPP_DBAL_ATI_TCAM_STATIC: /*  create tcam DB only if it wasn't already created (is valid)*/
        ...
        /* if is valid db already created */
    ...
    /* update table information (db_prefix && ATI)*/
    ...
    /* key_size + prefix_size shouldn't larger than entry_size */
                                  ...
                                  SOC_DPP_DBAL_QUAL_INFO  qual_info[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX], char* table_name, SOC_DPP_DBAL_SW_TABLE_IDS* table_id)
{
    ...
    switch (physical_db) {
    case SOC_DPP_DBAL_PHYSICAL_DB_TYPE_TCAM:
        additional_info = SOC_DPP_DBAL_ATI_NONE; /* by setting it to 0 the tcam_db_id and tcam resources will be allocated */
        db_prefix_len = 0;
        break;

    case SOC_DPP_DBAL_PHYSICAL_DB_TYPE_LEM:
        additional_info = SOC_DPP_DBAL_ATI_NONE; /* by setting it to 0 the app_type will be allocated */
        db_prefix_len = ARAD_PP_LEM_ACCESS_KEY_PREFIX_SIZE_IN_BITS;
        break;

    case SOC_DPP_DBAL_PHYSICAL_DB_TYPE_SEM_A:
    case SOC_DPP_DBAL_PHYSICAL_DB_TYPE_SEM_B:
        additional_info = SOC_DPP_DBAL_ATI_NONE; /* always zero - value not used */
...
arad_pp_dbal_table_clear_lem(int unit, SOC_DPP_DBAL_SW_TABLE_IDS table_id)
{
  ...
  if(SOC_DPP_IS_LEM_SIM_ENABLE(unit)){
     SOCDNX_IF_ERR_EXIT(arad_pp_dbal_table_lem_delete_by_iterator(unit, table_id));
      ...
      key.type = table.additional_table_info; /*ARAD_PP_LEM_ACCESS_KEY_TYPE_ILM;*/
      key.prefix.nof_bits = table.db_prefix_len;
      key.prefix.value = table.db_prefix;

      key_mask.type = table.additional_table_info;/*  ARAD_PP_LEM_ACCESS_KEY_TYPE_ILM*/
...
/* printing all entries that belong to the relevant DBAL table Id. in case of TCAM, it will print all entries related to the tcam prefix */
uint32
...
/* deleting all entries for LEM and shadow */
uint32
                           ...
                           SOC_PPC_FP_QUAL_VAL *qual_vals_array, void* payload,  uint32 *nof_entries)
{
    ...
    while ( !SOC_SAND_HASH_TABLE_ITER_IS_END(&iter) && (entry_index < block_range_key->start.payload.arr[0])); /*the last entry ID */
    ...
    do {
        ...
        if (table_id == entry_tbl_id)
        {
            ...
            if (found) {
                if (is_vrf_mode) {
                    /* find the VRF qualifier inside the Key*/
                    vrf_qual_index = 0;
                    ...
                    /* it means that no VRF in the qualifiers, cannot be in VRF mode */
                    if ((vrf_qual_index == SOC_PPC_FP_NOF_QUALS_PER_DB_MAX)){
                        SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG("no VRF in Table!!! %d"), table_id));
                    }
                }

                /*only if entry has the required vrf or not in VRF mode will save the existing entry */
...
#if defined(BCM_88675_A0)
/* this function can retrieve the next block by VRF or normal mode according to input */
uint32
    arad_pp_dbal_block_get_kaps(int unit, SOC_PPC_IP_ROUTING_TABLE_RANGE *block_range_key, SOC_DPP_DBAL_SW_TABLE_IDS table_id,
                           SOC_PPC_FP_QUAL_VAL *qual_vals_array/*SOC_PPC_FP_NOF_QUALS_PER_DB_MAX per entry*/, void* payload,  uint32 *nof_entries)
{
    SOC_DPP_DBAL_TABLE_INFO table;
    ...
    /*if working in VRF mode, retrieve vrf and reinitialize the qual_val from the first entry in the qual_vals_array */
    if (qual_vals_array[0].type != SOC_PPC_FP_QUAL_IRPP_VRF) {
        vrf_ndx = -1;
    }else{
        vrf_ndx = qual_vals_array[0].val.arr[0];
...
uint32 arad_pp_dbal_table_kaps_hitbit_set(int unit, SOC_DPP_DBAL_SW_TABLE_IDS table_id, uint8 enable) {
    ...
    /*Clear table to make sure no memory leaks when switching KAPS table hitbit modes*/
    ...
    arad_pp_dbal_qualifier_to_instruction(int unit, SOC_PPC_FP_DATABASE_STAGE stage, uint8 is_32b_ce, uint32 nof_bits, SOC_PPC_FP_QUAL_TYPE qual_type, int is_msb, uint32* ce_instr_encoding)
{
    ...
    uint32 qual_lsb = 0; /* indicates from where to start point */
    uint32 lost_bits = 0; /* if qualifier is not align how many bits are lost*/
    ...
    if (found) {
        ...
        if (nof_bits == 0) {/* take full qualifier */
        ...
        /* TBD - header qualifiers support */
        /*int32 offset;
        ARAD_PMF_CE_HEADER_QUAL_INFO    header_qual_info;
        uint32 resolution_ce_in_bits;
    ...
    arad_pp_dbal_flp_hw_based_key_enable(int unit, int program_id, SOC_DPP_HW_KEY_LOOKUP hw_key_based_lookup_enable)
{
    ...
    case SOC_DPP_HW_KEY_LOOKUP_IN_LEM_1ST:
        if ((flp_lookups_tbl.lem_1st_lkp_valid == 1) && (flp_lookups_tbl.lem_1st_lkp_key_select != ARAD_PP_FLP_LKP_KEY_SELECT_FID_FWD_MAC_KEY_HW_VAL)){
            ...
            /* it is a legal action, but to prevent miss configuration this error is sent
               user should clean the lem_1st_lkp_valid before he re-configuring */
        }
    ...
    arad_pp_dbal_flp_lookup_update(int unit, DBAL_PROGRAM_INFO table_program, SOC_DPP_DBAL_SW_TABLE_IDS table_id, uint8 is_to_remove, uint8* lookup_used, int is_msb)
{
    ...
    case SOC_DPP_DBAL_PHYSICAL_DB_TYPE_LEM:
        ...
        if((table_id == SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_ROUTE_SCALE_LONG_LEM)
            && SOC_DPP_CONFIG(unit)->pp.enhanced_fib_scale_prefix_length_ipv6_long == 64) {
                /* The logical prefix value is 10xxx, lem prefix take 2 bits from logical prefix */
    ...
    case SOC_DPP_DBAL_PHYSICAL_DB_TYPE_KBP:
        {
            ...
            switch(key_used_in_kbp){
                ...
                case SOC_DPP_DBAL_PROGRAM_KBP_MULTI_KEY:
                    /* In case of multi key, key B is full*/
                    flp_lookups_tbl.elk_key_b_valid_bytes = 10;
                                             ...
                                             SOC_DPP_DBAL_KEY_TO_TABLE keys_to_table_id[SOC_DPP_DBAL_PROGRAM_NOF_KEYS],uint8 use_32_bit_ce[SOC_DPP_DBAL_PROGRAM_NOF_KEYS] [SOC_PPC_FP_NOF_QUALS_PER_DB_MAX], int nof_valid_keys)
{
    ...
    for (curr_key_indx = 0; curr_key_indx < nof_valid_keys; curr_key_indx++){
        ...
        if ((table.physical_db_type == SOC_DPP_DBAL_PHYSICAL_DB_TYPE_KBP) &&
            ...
            continue; /* for KBP if the table is LOOKUP only - no need to configure the key construction */
        }
        ...
        for (curr_qual_id = 0; curr_qual_id < table.nof_qualifiers; curr_qual_id++){
            ...
            /* last_unchecked_ce can't be 134217727 as the coverity claims. It can be only if stage is not between 0 and SOC_PPC_NOF_FP_DATABASE_STAGES - 1.
                                                      ...
                                                      int nof_valid_keys)
{
    ...
    for (curr_key_indx = 0; curr_key_indx < nof_valid_keys; curr_key_indx++){
        ...
        if ((table.physical_db_type == SOC_DPP_DBAL_PHYSICAL_DB_TYPE_KBP) && (table.additional_table_info == SOC_DPP_DBAL_ATI_KBP_LOOKUP_ONLY)) {
            continue; /* for KBP if the table is LOOKUP only - not need to configure the key construction */
        }

        curr_key = keys_to_table_id[curr_key_indx].key_id;
        if ((curr_key >  SOC_DPP_DBAL_PROGRAM_KEY_D) &&
            ((table.physical_db_type == SOC_DPP_DBAL_PHYSICAL_DB_TYPE_KAPS) || (table.physical_db_type == SOC_DPP_DBAL_PHYSICAL_DB_TYPE_KBP))) {
            curr_key = curr_key - SOC_DPP_DBAL_PROGRAM_NOF_COMPLETE_KEYS;
            is_msb = TRUE;
        }
        ...
        table.table_programs[nof_table_programs].key_id = curr_key;
  ...
  arad_pp_dbal_entry_add_lem(int unit, SOC_DPP_DBAL_SW_TABLE_IDS table_id, SOC_PPC_FP_QUAL_VAL  qual_vals[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX], void* payload, SOC_SAND_SUCCESS_FAILURE *success)
{
    ...
    /* according to actions we need to decode the action*/
    ...
    arad_pp_dbal_entry_add_tcam(int unit, SOC_DPP_DBAL_SW_TABLE_IDS table_id, SOC_PPC_FP_QUAL_VAL qual_vals[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX], void* payload, uint32 priority,  SOC_SAND_SUCCESS_FAILURE *success)
{
    ...
    uint8  stream[ARAD_PP_SW_DB_HASH_KEY_LEN_BYTES] = {0}; /* the stream consist of value mask table Id */
    SOC_SAND_HASH_TABLE_PTR hash_tbl;
    ...
    } else { /* in FLP stage the result is uint32[2] */
    ...
    arad_pp_dbal_entry_add_kaps(int unit, SOC_DPP_DBAL_SW_TABLE_IDS table_id, SOC_PPC_FP_QUAL_VAL qual_vals[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX], void* payload, uint32 priority,  SOC_SAND_SUCCESS_FAILURE *success)
{
    ...
    SOCDNX_SAND_IF_ERR_EXIT(arad_pp_dbal_entry_key_to_kaps_buffer(unit, &table, JER_KAPS_KEY_BUFFER_NOF_BYTES, qual_vals, &prefix_len, data_bytes));
...
{
    /* Commented code is old implementation, once the new code is verified, delete commented code lines.
     *        To revert new code uncomment all commented code lines and delete jer_pp_kaps_dbal_key_encode function call.
     */
    uint32  table_size_in_bytes, table_payload_in_bytes;
    uint8   elk_data[ARAD_PP_FRWRD_IP_ELK_FWD_MAX_KEY_LENGTH_IN_BYTES];
    ...
    arad_pp_dbal_entry_delete_kaps(int unit, SOC_DPP_DBAL_SW_TABLE_IDS table_id, SOC_PPC_FP_QUAL_VAL qual_vals[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX], SOC_SAND_SUCCESS_FAILURE *success)
{
    ...
    /* KBPSDK does not support deleting the entries while traversing */
    ...
    SOCDNX_SAND_IF_ERR_EXIT(jer_pp_kaps_dbal_table_id_translate(unit, table_id, qual_vals, -1/*private_table*/, &kaps_table_id));
    SOCDNX_SAND_IF_ERR_EXIT(arad_pp_dbal_entry_key_to_kaps_buffer(unit, &table, JER_KAPS_KEY_BUFFER_NOF_BYTES, qual_vals, &prefix_len, data_bytes));
    SOCDNX_IF_ERR_EXIT(jer_pp_kaps_entry_remove_hw(unit, kaps_table_id, prefix_len, table.additional_table_info, data_bytes));
    ...
    arad_pp_dbal_entry_get_kbp(int unit, SOC_DPP_DBAL_SW_TABLE_IDS table_id, SOC_PPC_FP_QUAL_VAL qual_vals[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX], void* payload, uint32* priority, uint8* found)
{
    ...
    if (*found) {
        SOCDNX_SAND_IF_ERR_EXIT(arad_pp_kbp_route_payload_decode(unit,table.db_prefix,0 /* lookup_id */,elk_ad_value,(ARAD_PP_LEM_ACCESS_PAYLOAD*)payload));
    }
...
/*See SOC_DPP_DBAL_HITBIT_FLAG_X for HITBIT related flags, currently used for KAPS*/
uint32
    ...
    arad_pp_dbal_iterator_get_next_lem(int unit, SOC_DPP_DBAL_SW_TABLE_IDS table_id, uint32 flags, SOC_PPC_FP_QUAL_VAL qual_vals[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX], ARAD_PP_LEM_ACCESS_PAYLOAD* payload, uint32* priority, uint8* hit_bit, uint8* found)
{
    ...
    if ((table_iterator->read_keys == NULL) || (table_iterator->read_vals == NULL)) {
        *found = FALSE;
        ...
        /* Check whether another block read is necessary */
            ...
            if (SOC_SAND_TBL_ITER_IS_END(&table_iterator->block_range.iter)) {
                *found = FALSE;
                ...
                SOCDNX_SAND_IF_ERR_EXIT(arad_pp_frwrd_lem_get_block_unsafe(unit, &table_iterator->key, &table_iterator->key_mask,
                                                                        NULL,0/*access_only*/, &table_iterator->block_range, table_iterator->read_keys,
    ...
    arad_pp_dbal_iterator_get_next_kaps(int unit, SOC_DPP_DBAL_SW_TABLE_IDS table_id, uint32 flags, SOC_PPC_FP_QUAL_VAL qual_vals[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX], void* payload, uint32* priority, uint8* hit_bit, uint8* found)
{
    ...
    JER_KAPS_ENTRY *kaps_e;
    JER_KAPS_ENTRY_INFO kaps_e_info;
    JER_KAPS_AD_DB *ad_db_p;

    DBAL_KAPS_ITERATOR *table_iterator;

    JER_KAPS_DB    *table_p;
    JER_KAPS_ENTRY_ITER *kaps_iterator;
    int i;
    uint8 db_id;
    ...
    *found = FALSE;
    ...
    /* Attempt to get the entry first from the public table and then from the private */
    for (i=0; i<2; i++) {
        if (i==0) {
#if defined(INCLUDE_KBP)
            if(SOC_DPP_CONFIG(unit)->arad->init.elk.tcam_dev_type != ARAD_NIF_ELK_TCAM_DEV_TYPE_NONE) {
                /* if KBP is enabled, KAPS is used for KBP default routes, they should all be in private DB.
                 * skip this iteration
                 */
                continue;
            }
        ...
        if ((kaps_iterator != NULL) && (*found == FALSE)) {
            ...
            while (TRUE) { /* Loop until we find an entry with the correct table prefix */
                KAPS_TRY(kaps_db_entry_iter_next(table_p, kaps_iterator, &kaps_e));

                if (kaps_e == NULL) {
                    KAPS_TRY(kaps_db_entry_iter_destroy(table_p, kaps_iterator));
                    if (i==0) {
                    ...
                    KAPS_TRY(kaps_entry_get_info(table_p, kaps_e, &kaps_e_info));

                    /* Verify that the table prefix is correct - dynamic tables share the KAPS table but do not necessarily share the dbal prefix*/
                    if (kaps_e_info.data[0] >> (SOC_SAND_NOF_BITS_IN_BYTE - table.db_prefix_len) != table.db_prefix) {
                        continue;
                    }

                    jer_kaps_ad_db_by_db_id_get(unit, db_id, &ad_db_p);

                    if ((ad_db_p != NULL) && (kaps_e_info.ad_handle != NULL)) {
                            KAPS_TRY(kaps_ad_db_get(ad_db_p, kaps_e_info.ad_handle, ad_bytes));
                            SOCDNX_SAND_IF_ERR_EXIT(jer_pp_kaps_route_to_kaps_payload_buffer_decode(unit, ad_bytes, payload));
                            ...
                            if (SOC_IS_JERICHO_PLUS(unit)) {
                                ...
                                if (table.additional_table_info != 0) {
                                    ...
                                    SOCDNX_SAND_IF_ERR_EXIT(jer_pp_kaps_get_hit_bit(unit, kaps_table_id, kaps_e, clear_on_read, hit_bit));
                                }
                            }
                    }

                    SOCDNX_IF_ERR_EXIT(arad_pp_dbal_kaps_buffer_to_entry_key(unit, &table, kaps_e_info.prio_len, kaps_e_info.data, qual_vals));
                    *found = TRUE;
                    break;
                }
            }
    ...
    arad_pp_dbal_iterator_get_next_kbp(int unit, SOC_DPP_DBAL_SW_TABLE_IDS table_id, uint32 flags, SOC_PPC_FP_QUAL_VAL qual_vals[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX], void* payload, uint32* priority, uint8* hit_bit, uint8* found)
{
    ...
    *found = FALSE;
	...
	while(TRUE) {
		/* Loop until we find an entry with the correct prefix len */
	...
	if (*found) {
		...
		/* Translate the actions in two steps:
		 * 1. TCAM ACTION -> LEM PAYLOAD
		 * 2. LEM PAYLOAD -> ROUTE INFO
		 */
    ...
    arad_pp_dbal_iterator_init_kaps(int unit, SOC_DPP_DBAL_SW_TABLE_IDS table_id)
{
    ...
    /* initialize both private and public iterators */
    kaps_table_iterator = (DBAL_KAPS_ITERATOR *)table.table_iterator;

    SOCDNX_IF_ERR_EXIT(jer_pp_kaps_dbal_table_id_translate(unit, table_id, NULL/*qual_vals*/, 1/*private_table*/, &kaps_table_id/*kaps*/));
    ...
    SOCDNX_IF_ERR_EXIT(jer_pp_kaps_dbal_table_id_translate(unit, table_id, NULL/*qual_vals*/, 0/*private_table*/, &kaps_table_id/*kaps*/));
    jer_kaps_table_config_get(unit, kaps_table_id, &kaps_table_cfg_p);
    ...
    if (JER_KAPS_ENABLE_PUBLIC_DB(unit)) {
        KAPS_TRY(kaps_db_entry_iter_init(kaps_table_iterator->public_table_p, &kaps_table_iterator->public_table_iterator));
    }

    if (JER_KAPS_ENABLE_PRIVATE_DB(unit)) {
        KAPS_TRY(kaps_db_entry_iter_init(kaps_table_iterator->private_table_p, &kaps_table_iterator->private_table_iterator));
    }
    ...
    arad_pp_dbal_iterator_deinit_kaps(int unit, SOC_DPP_DBAL_SW_TABLE_IDS table_id)
{
    ...
    if (kaps_table_iterator != NULL) {
        if (kaps_table_iterator->public_table_iterator != NULL) {
            KAPS_TRY(kaps_db_entry_iter_destroy(kaps_table_iterator->public_table_p, kaps_table_iterator->public_table_iterator));
            kaps_table_iterator->public_table_iterator = NULL;
        }

        if (kaps_table_iterator->private_table_iterator != NULL) {
            KAPS_TRY(kaps_db_entry_iter_destroy(kaps_table_iterator->private_table_p, kaps_table_iterator->private_table_iterator));
            kaps_table_iterator->private_table_iterator = NULL;
        }
    }
  ...
  arad_pp_dbal_entry_key_to_lem_buffer(int unit, SOC_PPC_FP_QUAL_VAL qual_vals[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX], uint32 table_id, ARAD_PP_LEM_ACCESS_KEY *key)
{
    ...
    for (i = 0; i < table.nof_qualifiers; i++) {
        ...
        /*  Adjust qual_offset to always represent the offset from the LSB
         *  use nof_bits from qual_offset towards the MSB
         */
        ...
        if ((qual_type != SOC_PPC_FP_QUAL_IRPP_ALL_ZEROES) && (qual_type != SOC_PPC_FP_QUAL_IRPP_ALL_ONES)) {
            /* the table qualifier in the user provided qual_vals */
            user_qual_idx = 0;
            ...
            if (user_qual_idx == SOC_PPC_FP_NOF_QUALS_PER_DB_MAX) {
                ...
                if ((qual_offset + table.qual_info[i].qual_nof_bits > 32) && (qual_offset < 32)) {
                    /* Copy bits both from val.arr[1] and val.arr[0] */
  ...
  arad_pp_dbal_lem_buffer_to_entry_key(int unit, ARAD_PP_LEM_ACCESS_KEY *key, uint32 table_id, SOC_PPC_FP_QUAL_VAL qual_vals[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX])
{
    ...
    for (i = 0; i < key->nof_params; i++) {
        ...
        if ((qual_type != SOC_PPC_FP_QUAL_IRPP_ALL_ZEROES) && (qual_type != SOC_PPC_FP_QUAL_IRPP_ALL_ONES)) {
            /* the table qualifier in the user provided qual_vals */
            user_qual_idx = 0;
            /* Find the qual or an empty qual */
  ...
  arad_pp_dbal_entry_lem_buffer_to_key(int unit, uint32 table_id, uint32 buffer[ARAD_PP_FP_TCAM_ENTRY_SIZE], SOC_PPC_FP_QUAL_VAL qual_vals[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX])
{
    ...
    /* updating lem key info by using same algorithm used when adding entries */
    key.type = table.additional_table_info;
    ...
    /* updating lem key from the input buffer */
    curr_bit_lsb = 0;
    ...
    /* transforming the LEM params to qualifiers */
    offset_fix = 0;
    for (i = 0; i < table.nof_qualifiers; i++) {
        if(i > 0){
            if(table.qual_info[i].qual_type == table.qual_info[i-1].qual_type) {
                                       ...
                                       uint32 *prefix_len, uint8 *data_bytes)
{
        ...
        while (qual_remaining_bits > 0)
        {
            ...
            if (qual_type != SOC_PPC_FP_QUAL_IRPP_ALL_ZEROES) {
                ...
                if (qual_type == SOC_PPC_FP_QUAL_IRPP_ALL_ONES) {
                    ...
                    if (byte_prefix_len) {/*Do not add zeros into length if they are followed by empty qualifiers*/
    ...
    /*check if all the dont care qualifiers are not valid, if one of them has a valid value then return error*/
    for (i = 0; i < qual_idx; i++) {
        ...
        if (qual_type != SOC_PPC_FP_QUAL_IRPP_ALL_ZEROES && qual_type != SOC_PPC_FP_QUAL_IRPP_ALL_ONES){
            ...
            if (user_qual_idx != SOC_PPC_FP_NOF_QUALS_PER_DB_MAX){  /*found user qualifier*/
                if  (qual_vals[user_qual_idx].is_valid.arr[0] == 0 && qual_vals[user_qual_idx].is_valid.arr[1] == 0){
                    continue;
                }
                else{
                    /* Check if this qualifier has been partially used in a previous table qualifier */
                    continue_flag = 0;
...
#if defined(BCM_88675_A0)
uint32
  arad_pp_dbal_entry_key_to_kaps_buffer(int unit, SOC_DPP_DBAL_TABLE_INFO *dbal_table, uint32 table_size_in_bytes, SOC_PPC_FP_QUAL_VAL qual_vals[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX],
                                       uint32 *prefix_len, uint8 *data_bytes)
{
    uint8
        nof_quals = dbal_table->nof_qualifiers,
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_pp_dbal_entry_key_to_kaps_buffer()",0,0);
}
#endif
                                       ...
                                       SOC_PPC_FP_QUAL_VAL qual_vals[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX])
{
    ...
    for (i = 0; i < nof_quals; i++)
    {
        ...
        if ((qual_type != SOC_PPC_FP_QUAL_IRPP_ALL_ZEROES) && (qual_type != SOC_PPC_FP_QUAL_IRPP_ALL_ONES)) {
            /*find an available user_qual_idx*/
            user_qual_idx = 0;
            while ((user_qual_idx < SOC_PPC_FP_NOF_QUALS_PER_DB_MAX) && (qual_type != qual_vals[user_qual_idx].type)) {
        ...
        while (qual_remaining_bits > 0 && prefix_remain > 0)
        {
            ...
            /*Sometimes prefix_remain != 0; check your prefix_len and trailing all zeroes and all ones*/
            ...
            if ((qual_type != SOC_PPC_FP_QUAL_IRPP_ALL_ZEROES) && (qual_type != SOC_PPC_FP_QUAL_IRPP_ALL_ONES)) {
                ...
                /*this check should not be done, unless these all zeroes and all ones are not included in the prefix_len*/
...
#if defined(BCM_88675_A0)
uint32
  arad_pp_dbal_kaps_buffer_to_entry_key(int unit, const SOC_DPP_DBAL_TABLE_INFO *dbal_table, uint32 prefix_len, uint8 *data_bytes,
                                       SOC_PPC_FP_QUAL_VAL qual_vals[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX])
{
    uint8
        nof_quals = dbal_table->nof_qualifiers,
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_pp_dbal_kaps_buffer_to_entry_key()",0,0);
}
#endif
    ...
    arad_pp_dbal_entry_dump(int unit, SOC_PPC_FP_DATABASE_STAGE stage, SOC_DPP_DBAL_SW_TABLE_IDS table_id, SOC_PPC_FP_QUAL_VAL in_qual_vals[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX], uint32 in_data[ARAD_PP_FP_TCAM_ENTRY_SIZE])
{
...
#if defined(INCLUDE_KBP)
        {
            /* suggested immplementation if needed
			  uint32  prefix_len;
              JER_KAPS_IP_TBL_ID kaps_table_id;
              uint8   data_bytes[JER_KAPS_KEY_BUFFER_NOF_BYTES];

              SOCDNX_SAND_IF_ERR_EXIT(jer_pp_kaps_dbal_table_id_translate(unit, table_id, in_qual_vals, -1, &kaps_table_id));
              SOCDNX_SAND_IF_ERR_EXIT(arad_pp_dbal_entry_key_to_kbp_buffer(unit, table, JER_KAPS_KEY_BUFFER_NOF_BYTES, in_qual_vals, &prefix_len, data_bytes));*/
        }
#endif /*defined(INCLUDE_KBP) */
        SOCDNX_FUNC_RETURN;
        break;
    ...
    case SOC_DPP_DBAL_PHYSICAL_DB_TYPE_LEM:
        if (buffer_to_qualifier_mode) {
            ...
            no_buffer_print = 1;
            /* SOCDNX_SAND_IF_ERR_EXIT(arad_pp_dbal_entry_key_to_lem_buffer(unit, in_qual_vals, table_id, &(request.key))); */
        }
...
/* this function is used only in debug mode (during development)
   to validate that all the input qualifiers exists in the DB
 */
uint32
	...
	arad_pp_dbal_diag_entry_manage(int unit, SOC_DPP_DBAL_DIAG_ENTRY_MANAGE_MODE mode, int table_id, args_t* arg)
{
    ...
    /* prepare payload parameters and parse payload*/
    if (mode == SOC_DPP_DBAL_DIAG_ENTRY_MANAGE_ADD) {
        ...
        LOG_CLI((BSL_META("\nPayload:\n")));
		/*Coverity doesn't like using payload_array as an array.*/
    ...
    case SOC_DPP_DBAL_DIAG_ENTRY_MANAGE_ADD:
        ...
        break;
    default:/*SOC_DPP_DBAL_DIAG_ENTRY_MANAGE_DELETE*/
	...
	arad_pp_dbal_diag_input_parse(int unit, const char *input, uint32 payload_nof_words, uint32 *payload_value, uint32 *input_nof_words)
{
	...
	/* count words for multiple words value */
	while (tmp_token && i < payload_nof_words) {
		i++;
		tmp_token++; /*skip current delim*/
	...
	*input_nof_words = i;
	...
	input_copy = sal_strdup(input); /*strtok will destroy original*/
    ...
    arad_pp_dbal_ce_per_program_dump(int unit, int program_id, SOC_PPC_FP_DATABASE_STAGE stage)
{
    ...
    for (ce_ndx = 0; ce_ndx <= last_ce_ndx; ce_ndx++) {
        ...
        /* ce_ndx can't be 134217727 as the coverity claims. It can be only if stage is not between 0 and SOC_PPC_NOF_FP_DATABASE_STAGES - 1.
         * We do such check above
         */
        /* coverity[overrun-local:FALSE] */
...
/* find the related table to the prefix. in case of multiple tables, take the 1st table unless there is a table that as
    ...
    for (stage = 0; stage < NUM_OF_STAGES; stage++) {
        ...
        if (prog_id[stage] == -1) { /* no program invoked continue to next stage */
            ...
            if (stage == stage_flp) {
                ...
                /* FLP use program id instead of cam_line */
...
STATIC SOC_DPP_SIGNAL_MNGR_SIGNAL_INFO arad_pp_signal_mngr_arad_signals_table[SOC_DPP_SIGNAL_MNGR_SIGNAL_TYPE_NOF_TYPES] = {
    ...
    {/*FLP_2_TCAM_KEY     		*/ TRUE,  ARAD_IHB_ID,  15,       1,     235,   76,     160,        0},
    {/*TCAM_2_FLP_RES     		*/ TRUE,  ARAD_IHB_ID,  16,       0,     124,   85,      40,        0},
    {/*TCAM_2_FLP_HIT     		*/ TRUE,  ARAD_IHB_ID,  16,       0,     125,  125,       1,        0},
    {/*FLP_2_KBP_OPCODE   		*/ TRUE,  ARAD_IHB_ID,   3,       0,       7,    0,       8,        0},
    {/*FLP_2_KBP_KEY_PART1		*/ TRUE,  ARAD_IHB_ID,   3,       0,     255,    8,     248,        0},
    {/*FLP_2_KBP_KEY_PART2		*/ TRUE,  ARAD_IHB_ID,   3,       1,     255,    0,     256,        0},
    {/*FLP_2_KBP_KEY_PART3		*/ TRUE,  ARAD_IHB_ID,   3,       2,     255,    0,     256,        0},
    {/*FLP_2_KBP_KEY_PART4		*/ TRUE,  ARAD_IHB_ID,   3,       3,     255,    0,     256,        0},
    {/*FLP_2_KBP_KEY_PART5		*/ TRUE,  ARAD_IHB_ID,   3,       4,       7,    0,       8,        0},
	{/*KBP_2_FLP_RES_LSB_PART1	*/ FALSE, ARAD_IHP_ID,   0,       0,       0,    0,       0,        0},
	{/*KBP_2_FLP_RES_LSB_PART2	*/ FALSE, ARAD_IHP_ID,   0,       0,       0,    0,       0,        0},
    {/*KBP_2_FLP_RES      		*/ TRUE,  ARAD_IHB_ID,   4,       0,     128,    1,     128,        0},
    {/*KBP_2_FLP_ERR      		*/ TRUE,  ARAD_IHB_ID,   4,       0,       0,    0,       1,        0},
...
STATIC SOC_DPP_SIGNAL_MNGR_SIGNAL_INFO arad_pp_signal_mngr_jericho_signals_table[SOC_DPP_SIGNAL_MNGR_SIGNAL_TYPE_NOF_TYPES] = {
    ...
    {/*LEM1_2_FLP_RES     		*/ TRUE,  ARAD_IHP_ID,  27,       0,      92,   48,      45,        0},
    {/*LEM2_2_FLP_RES     		*/ TRUE,  ARAD_IHP_ID,  27,       0,      44,    0,      44,        0},
    ...
    {/*FLP_2_KBP_KEY_PART2		*/ TRUE,  ARAD_IHP_ID,  32,       1,     255,    0,     256,        0},
    {/*FLP_2_KBP_KEY_PART3		*/ TRUE,  ARAD_IHP_ID,  32,       2,     255,    0,     256,        0},
    {/*FLP_2_KBP_KEY_PART4		*/ TRUE,  ARAD_IHP_ID,  32,       3,     255,    0,     256,        0},
    {/*FLP_2_KBP_KEY_PART5		*/ TRUE,  ARAD_IHP_ID,  32,       4,       7,    0,       8,        0},
	{/*KBP_2_FLP_RES_LSB_PART1	*/ TRUE,  ARAD_IHP_ID,   4,       8,     255,  167,      89,        0},   /*signal is taken form the flp2pmf*/
	{/*KBP_2_FLP_RES_LSB_PART2	*/ TRUE,  ARAD_IHP_ID,   4,       9,      38,    0,      39,        0},   /*signal is taken form the flp2pmf*/
    {/*KBP_2_FLP_RES      		*/ TRUE,  ARAD_IHP_ID,   4,       9,     166,   39,     128,        0},   /*signal is taken form the flp2pmf*/
    {/*KBP_2_FLP_ERR      		*/ TRUE,  ARAD_IHP_ID,   4,       9,     167,  167,       1,        0},   /*signal is taken form the flp2pmf*/
    ...
    {/*VT_2_TCAM_KEY_LSB  		*/ TRUE,  ARAD_IHP_ID,  24,       0,     255,  172,      84,        0},
    {/*VT_2_TCAM_KEY_MSB  		*/ TRUE,  ARAD_IHP_ID,  24,       1,      75,    0,      76,        0},
...
STATIC SOC_DPP_SIGNAL_MNGR_SIGNAL_INFO arad_pp_signal_mngr_qax_signals_table[SOC_DPP_SIGNAL_MNGR_SIGNAL_TYPE_NOF_TYPES] = {
    /*  TYPE             		| is_valid |  block  |addr_high|addr_low| msb | lsb | num_of_bits| offset*/
    {/*FLP_2_LEM1_KEY     		*/ TRUE,  ARAD_IHP_ID,  28,       0,     205,  126,      80,        0},
    {/*FLP_2_LEM2_KEY     		*/ TRUE,  ARAD_IHP_ID,  28,       0,      79,    0,      80,        0},
    {/*LEM1_2_FLP_RES     		*/ TRUE,  ARAD_IHP_ID,  29,       0,      44,    0,      44,        0},
    {/*LEM2_2_FLP_RES     		*/ TRUE,  ARAD_IHP_ID,  29,       0,      92,   48,      45,        0},
    {/*LEM1_2_FLP_HIT     		*/ TRUE,  ARAD_IHP_ID,  29,       0,      95,   95,       1,        0},
    {/*LEM2_2_FLP_HIT     		*/ TRUE,  ARAD_IHP_ID,  29,       0,      47,   47,       1,        0},
    {/*FLP_2_LPM1_KEY     		*/ TRUE,  ARAD_IHP_ID,  32,       1,     255,  252,       4,        0},
    {/*FLP_2_LPM1_KEY_2   		*/ TRUE,  ARAD_IHP_ID,  32,       2,     155,    0,     156,        0},
    {/*FLP_2_LPM2_KEY     		*/ TRUE,  ARAD_IHP_ID,  32,       0,     166,    7,     160,        0},
    {/*FLP_2_LPM2_KEY_2   		*/ TRUE,  ARAD_IHP_ID,  32,       1,     255,  174,     160,       78},
    {/*LPM1_2_FLP_RES     		*/ TRUE,  ARAD_IHP_ID,  33,       0,     119,  100,      20,        0},
    {/*LPM2_2_FLP_RES     		*/ TRUE,  ARAD_IHP_ID,  33,       0,      59,   40,      20,        0},
    {/*LPM1_2_FLP_HIT     		*/ TRUE,  ARAD_IHP_ID,  33,       0,       9,    8,       2,        0},
    {/*LPM2_2_FLP_HIT     		*/ TRUE,  ARAD_IHP_ID,  33,       0,      99,  100,       2,        0},
    {/*FLP_2_TCAM_KEY     		*/ TRUE,  ARAD_IHP_ID,  30,       0,     255,  166,     160,        0},
    {/*TCAM_2_FLP_RES     		*/ TRUE,  ARAD_IHP_ID,  31,       0,      98,   51,      48,        0},
    {/*TCAM_2_FLP_HIT     		*/ TRUE,  ARAD_IHP_ID,  31,       0,      99,   99,       1,        0},
    {/*FLP_2_KBP_OPCODE   		*/ TRUE,  ARAD_IHP_ID,  34,       0,       7,    0,       8,        0},
    {/*FLP_2_KBP_KEY_PART1		*/ TRUE,  ARAD_IHP_ID,  34,       0,     255,    8,     248,        0},
    {/*FLP_2_KBP_KEY_PART2		*/ TRUE,  ARAD_IHP_ID,  34,       1,     255,    0,     255,        0},
    {/*FLP_2_KBP_KEY_PART3		*/ TRUE,  ARAD_IHP_ID,  34,       2,     255,    0,     255,        0},
    {/*FLP_2_KBP_KEY_PART4		*/ TRUE,  ARAD_IHP_ID,  34,       3,     255,    0,     255,        0},
    {/*FLP_2_KBP_KEY_PART5		*/ TRUE,  ARAD_IHP_ID,  34,       4,       7,    0,       8,        0},
	{/*KBP_2_FLP_RES_LSB_PART1	*/ TRUE,  ARAD_IHP_ID,   4,       8,     255,  232,      24,        0},   /*signal is taken form the flp2pmf*/
	{/*KBP_2_FLP_RES_LSB_PART2	*/ TRUE,  ARAD_IHP_ID,   4,       9,     103,    0,     104,        0},   /*signal is taken form the flp2pmf*/
    {/*KBP_2_FLP_RES      		*/ TRUE,  ARAD_IHP_ID,   4,       9,     231,  104,     128,        0},   /*signal is taken form the flp2pmf*/
    {/*KBP_2_FLP_ERR      		*/ TRUE,  ARAD_IHP_ID,   4,       9,     232,  232,       1,        0},   /*signal is taken form the flp2pmf*/
    {/*VT_2_SEM_A_KEY     		*/ TRUE,  ARAD_IHP_ID,  24,       0,      99,   50,      50,        0},
    {/*VT_2_SEM_A_RES     		*/ TRUE,  ARAD_IHP_ID,  25,       0,      33,   18,      16,        0},
    {/*VT_2_SEM_A_HIT     		*/ TRUE,  ARAD_IHP_ID,  25,       0,      35,   35,       1,        0},
    {/*VT_2_SEM_B_KEY     		*/ TRUE,  ARAD_IHP_ID,  26,       0,      99,   50,      50,        0},
    {/*VT_2_SEM_B_RES     		*/ TRUE,  ARAD_IHP_ID,  27,       0,      33,   18,      16,        0},
    {/*VT_2_SEM_B_HIT     		*/ TRUE,  ARAD_IHP_ID,  27,       0,      35,   35,       1,        0},
    {/*VT_2_TCAM_KEY_LSB  		*/ TRUE,  ARAD_IHP_ID,  14,       0,     255,  166,      90,        0},
    {/*VT_2_TCAM_KEY_MSB  		*/ TRUE,  ARAD_IHP_ID,  14,       1,      69,    0,      70,        0},
    {/*VT_2_TCAM_RES      		*/ TRUE,  ARAD_IHP_ID,  15,       0,      98,   51,      48,        0},
    {/*VT_2_TCAM_HIT      		*/ TRUE,  ARAD_IHP_ID,  15,       0,      99,   99,       1,        0},
    {/*TT_2_SEM_A_KEY     		*/ TRUE,  ARAD_IHP_ID,  24,       0,      49,    0,      50,        0},
    {/*TT_2_SEM_A_RES     		*/ TRUE,  ARAD_IHP_ID,  25,       0,      15,    0,      17,        0},
    {/*TT_2_SEM_A_HIT     		*/ TRUE,  ARAD_IHP_ID,  25,       0,      17,   17,       1,        0},
    {/*TT_2_SEM_B_KEY     		*/ TRUE,  ARAD_IHP_ID,  26,       0,      49,    0,      50,        0},
    {/*TT_2_SEM_B_RES     		*/ TRUE,  ARAD_IHP_ID,  27,       0,      15,    0,      16,        0},
    {/*TT_2_SEM_B_HIT     		*/ TRUE,  ARAD_IHP_ID,  27,       0,      17,   17,       1,        0},
    {/*TT_2_TCAM_KEY      		*/ TRUE,  ARAD_IHP_ID,  14,       0,     159,    0,     160,        0},
    {/*TT_2_TCAM_RES      		*/ TRUE,  ARAD_IHP_ID,  15,       0,      48,    1,      48,        0},
    {/*TT_2_TCAM_HIT      		*/ TRUE,  ARAD_IHP_ID,  15,       0,      49,   49,       1,        0},
    {/*RIF                		*/ TRUE,  ARAD_IHP_ID,   3,       2,      46,   32,      15,        0},
    {/*RIF_PRofile        		*/ TRUE,  ARAD_IHP_ID,   3,       2,      31,   26,       6,        0},
};
...
STATIC SOC_DPP_SIGNAL_MNGR_SIGNAL_INFO arad_pp_signal_mngr_jericho_plus_signals_table[SOC_DPP_SIGNAL_MNGR_SIGNAL_TYPE_NOF_TYPES] = {
    ...
    {/*FLP_2_LEM1_KEY           */ TRUE,  ARAD_IHP_ID,  26,       0,     205,  126,      80,        0},
    {/*FLP_2_LEM2_KEY           */ TRUE,  ARAD_IHP_ID,  26,       0,      79,    0,      80,        0},
    {/*LEM1_2_FLP_RES           */ TRUE,  ARAD_IHP_ID,  27,       0,      44,    0,      45,        0},
    {/*LEM2_2_FLP_RES           */ TRUE,  ARAD_IHP_ID,  27,       0,      92,   48,      45,        0},
    {/*LEM1_2_FLP_HIT           */ TRUE,  ARAD_IHP_ID,  27,       0,      95,   95,       1,        0},
    {/*LEM2_2_FLP_HIT           */ TRUE,  ARAD_IHP_ID,  27,       0,      47,   47,       1,        0},
    {/*FLP_2_LPM1_KEY           */ TRUE,  ARAD_IHP_ID,  30,       1,     255,  252,       4,        0},
    {/*FLP_2_LPM1_KEY_2         */ TRUE,  ARAD_IHP_ID,  30,       2,     155,    0,     156,        4},
    {/*FLP_2_LPM2_KEY           */ TRUE,  ARAD_IHP_ID,  30,       0,     166,    7,     160,        0},
    {/*FLP_2_LPM2_KEY_2         */ TRUE,  ARAD_IHP_ID,  30,       1,     255,  252,       4,        0},
    {/*LPM1_2_FLP_RES           */ TRUE,  ARAD_IHP_ID,  31,       0,     119,  100,      20,        0},
    {/*LPM2_2_FLP_RES           */ TRUE,  ARAD_IHP_ID,  31,       0,      59,   40,      20,        0},
    {/*LPM1_2_FLP_HIT           */ TRUE,  ARAD_IHP_ID,  31,       0,       9,    8,       2,        0},
    {/*LPM2_2_FLP_HIT           */ TRUE,  ARAD_IHP_ID,  31,       0,      99,   98,       2,        0},
    {/*FLP_2_TCAM_KEY           */ TRUE,  ARAD_IHP_ID,  28,       0,     255,  166,      90,        0},
    {/*TCAM_2_FLP_RES           */ TRUE,  ARAD_IHP_ID,  29,       0,      98,   51,      48,        0},
    {/*TCAM_2_FLP_HIT           */ TRUE,  ARAD_IHP_ID,  29,       0,      99,   99,       1,        0},
    {/*FLP_2_KBP_OPCODE         */ TRUE,  ARAD_IHP_ID,  32,       0,       7,    0,       8,        0},
    {/*FLP_2_KBP_KEY_PART1      */ TRUE,  ARAD_IHP_ID,  32,       0,     255,    8,     248,        0},
    ...
    {/*FLP_2_KBP_KEY_PART4      */ TRUE,  ARAD_IHP_ID,  32,       3,     255,    0,     256,        0},
    {/*FLP_2_KBP_KEY_PART5      */ TRUE,  ARAD_IHP_ID,  32,       4,       7,    0,       8,        0},
    {/*KBP_2_FLP_RES_LSB_PART1  */ TRUE,  ARAD_IHP_ID,   4,       8,     255,  237,      19,        0},   /*signal is taken form the flp2pmf*/
    {/*KBP_2_FLP_RES_LSB_PART2  */ TRUE,  ARAD_IHP_ID,   4,       9,     108,    0,     109,        0},   /*signal is taken form the flp2pmf*/
    {/*KBP_2_FLP_RES            */ TRUE,  ARAD_IHP_ID,   4,       9,     236,  109,     128,        0},   /*signal is taken form the flp2pmf*/
    {/*KBP_2_FLP_ERR            */ TRUE,  ARAD_IHP_ID,   4,       9,     237,  237,       1,        0},   /*signal is taken form the flp2pmf*/
    {/*VT_2_SEM_A_KEY           */ TRUE,  ARAD_IHP_ID,  22,       0,      99,   50,      50,        0},
    {/*VT_2_SEM_A_RES           */ TRUE,  ARAD_IHP_ID,  23,       0,      35,   19,      17,        0},
    {/*VT_2_SEM_A_HIT           */ TRUE,  ARAD_IHP_ID,  23,       0,      37,   37,       1,        0},
    {/*VT_2_SEM_B_KEY           */ TRUE,  ARAD_IHP_ID,  24,       0,      99,   50,      50,        0},
    {/*VT_2_SEM_B_RES           */ TRUE,  ARAD_IHP_ID,  25,       0,      35,   19,      17,        0},
    {/*VT_2_SEM_B_HIT           */ TRUE,  ARAD_IHP_ID,  25,       0,      37,   37,       1,        0},
    {/*VT_2_TCAM_KEY_LSB        */ TRUE,  ARAD_IHP_ID,  12,       0,     255,  166,      90,        0},
    {/*VT_2_TCAM_KEY_MSB        */ TRUE,  ARAD_IHP_ID,  12,       1,      69,    0,      70,       90},
    {/*VT_2_TCAM_RES            */ TRUE,  ARAD_IHP_ID,  13,       0,      98,   51,      48,        0},
    {/*VT_2_TCAM_HIT            */ TRUE,  ARAD_IHP_ID,  13,       0,      99,   99,       1,        0},
    {/*TT_2_SEM_A_KEY           */ TRUE,  ARAD_IHP_ID,  22,       0,      49,    0,      50,        0},
    {/*TT_2_SEM_A_RES           */ TRUE,  ARAD_IHP_ID,  23,       0,      16,    0,      17,        0},
    {/*TT_2_SEM_A_HIT           */ TRUE,  ARAD_IHP_ID,  23,       0,      17,   17,       1,        0},
    {/*TT_2_SEM_B_KEY           */ TRUE,  ARAD_IHP_ID,  24,       0,      49,    0,      50,        0},
    {/*TT_2_SEM_B_RES           */ TRUE,  ARAD_IHP_ID,  25,       0,      16,    0,      17,        0},
    {/*TT_2_SEM_B_HIT           */ TRUE,  ARAD_IHP_ID,  25,       0,      18,   18,       1,        0},
    {/*TT_2_TCAM_KEY            */ TRUE,  ARAD_IHP_ID,  12,       0,     159,    0,     160,        0},
    {/*TT_2_TCAM_RES            */ TRUE,  ARAD_IHP_ID,  13,       0,      48,    1,      48,        0},
    {/*TT_2_TCAM_HIT            */ TRUE,  ARAD_IHP_ID,  13,       0,      49,   49,       1,        0},
    {/*RIF                      */ TRUE,  ARAD_IHP_ID,   3,       2,      35,   30,       6,        0},
    {/*RIF_PRofile              */ TRUE,  ARAD_IHP_ID,   3,       2,      35,   30,       6,        0},
};
...
{
    uint32 val_aligned[ARAD_PP_DIAG_DBG_VAL_LEN] = {0}; /* used only if the value is not aligned*/
