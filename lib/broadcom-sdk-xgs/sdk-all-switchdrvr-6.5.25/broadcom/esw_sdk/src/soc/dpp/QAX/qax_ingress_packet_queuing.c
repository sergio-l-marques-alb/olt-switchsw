...
/*
 * Initialization of Ingress Packet Queueing.
 */
int
  qax_ipq_init(
  ...
  ) 
{
    ...
    SOCDNX_IF_ERR_EXIT(soc_reg_get(unit, IPS_IPS_GENERAL_CONFIGURATIONSr, core, 0, &reg64));
    ...
    SOCDNX_IF_ERR_EXIT(soc_reg_set(unit, IPS_IPS_GENERAL_CONFIGURATIONSr, core, 0, reg64));
    ...
    if (dpp_arad->init.drc_info.dram_num == 0) { /* ocb only mode */
        ...
        /* 
         * Configuration for regular queues profile. Currently set to hw 
         * default values, until AEs will fine tune them - AEs feedback 
         */
        ...
        /* 
         * Configuration for high bandwidth queues profile. Currently set to hw 
         * default values, until AEs will fine tune them - AEs feedback 
         */
        reg32 = 0;
   ...
   ) 
{
    ...
    * Per flow, a profile is got to set its TC-mapping and its Base-Flow
