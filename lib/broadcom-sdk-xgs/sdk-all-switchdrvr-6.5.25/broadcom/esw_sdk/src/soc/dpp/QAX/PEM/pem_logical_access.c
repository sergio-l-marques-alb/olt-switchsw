...
int pem_logical_db_tcam_write(int unit, tcam_id_t tcam_id, uint32 row_index, pem_mem_access_t *key, pem_mem_access_t *mask, pem_mem_access_t *valid, pem_mem_access_t *data) {
  ...
  unsigned int chunk_matrix_row      = row_index / PEM_CFG_API_CAM_TCAM_CHUNK_N_ENTRIES;          /*Should always be 0*/
...
int pem_logical_db_tcam_read(int unit,  tcam_id_t tcam_id, uint32 row_index, pem_mem_access_t *key, pem_mem_access_t *mask, pem_mem_access_t *valid, pem_mem_access_t *result) {
  ...
  unsigned int total_key_width_in_bits    = 2 * qax_api_info.db_tcam_container.db_tcam_info_arr[tcam_id].total_key_width + 1;  /*Key + Mask + Valid*/
...
int qax_pem_reginfo_access(int unit, table_id_t reg_id, pem_mem_access_t *data) { /*, enum action_e action) {*/
...
extern int qax_parse_meminfo_definition_file(const char *file_name); /* This one is implemented in pem_meminfo_init.c */
...
STATIC void qax_build_virtual_db_mask_and_data_from_key_fields(const int key_width, const unsigned int total_width, const FieldBitRange* field_bit_range_arr, const pem_mem_access_t* key, const pem_mem_access_t* mask, const pem_mem_access_t* valid, unsigned int **virtual_key_mask, unsigned int **virtual_key_data) {
  ...
  *virtual_key_mask = (unsigned int*)calloc(ceil((1.0f*total_width) / UINT_WIDTH_IN_BITS), sizeof(uint32));
  ...
  qax_build_virtual_db_mask(0,             field_bit_range_arr,              key,       *virtual_key_mask);  /*key - offset is 0 */                                                                                                                                                   
  qax_build_virtual_db_mask(key_width,     field_bit_range_arr,              mask,      *virtual_key_mask);  /*Done for Mask. includes the offset of the key*/
  qax_build_virtual_db_mask(2 * key_width, valid_field_bit_range_arr,        valid,     *virtual_key_mask);  /*Done for Valid. includes the offset of the key and the mask*/

  /*DATA*/
  *virtual_key_data = (unsigned int*)calloc(ceil((1.0f*total_width) / UINT_WIDTH_IN_BITS), sizeof(uint32));
                                                                                                                                                                                      
  qax_build_virtual_db_data(          0, field_bit_range_arr,              key,       *virtual_key_data);  /*key - offset is 0 */                                           
  qax_build_virtual_db_data(  key_width, field_bit_range_arr,              mask,      *virtual_key_data);  /*Done for Mask. includes the offset of the key*/
  qax_build_virtual_db_data(2*key_width, valid_field_bit_range_arr,        valid,     *virtual_key_data);  /*Done for Valid. includes the offset of the key and the mask*/
...
STATIC void qax_build_virtual_db_mask_and_data_from_result_fields(const unsigned int total_width, const FieldBitRange* field_bit_range_arr, const pem_mem_access_t* result, unsigned int **virtual_result_mask, unsigned int **virtual_result_data) {
  ...
  *virtual_result_mask = (unsigned int*)calloc(ceil((1.0f*total_width) / UINT_WIDTH_IN_BITS), sizeof(uint32));
  ...
  qax_build_virtual_db_mask(0, field_bit_range_arr, result, *virtual_result_mask);       /*offset for RESULT is 0 */

  /*DATA*/
  *virtual_result_data = (unsigned int*)calloc(ceil((1.0f*total_width) / UINT_WIDTH_IN_BITS), sizeof(uint32));
                                                                                                                                                                                      
  qax_build_virtual_db_data(0, field_bit_range_arr, result,  *virtual_result_data);     /*offset for RESULT is 0 */
  return;
}
...
STATIC PhysicalWriteInfo* qax_find_or_allocate_and_read_physical_data(const DbChunkMapper* chunk_mapper, const unsigned int row_index, PhysicalWriteInfo** target_physical_memory_entry_data_list) {
  ...
  target_physical_memory_entry_data_new_element->entry_data = (unsigned int*)calloc(1, sizeof(unsigned int));      /* EDI ----> MULTIPLY SIZEOF() BY MEM_WIDTH/UINT ?? */
...
STATIC void qax_set_pem_mem_accesss_fldbuf(const unsigned int offset, const unsigned int* virtual_db_data_array,const FieldBitRange* field_bit_range_arr, pem_mem_access_t* mem_access) {
  ...
  for (field_index = 0; field_index < mem_access->nFields; ++field_index) {
    ...
    qax_set_bits(virtual_db_data_array, field_bit_range_arr[field_id].lsb + offset,                   0, field_width, mem_access->fields[field_id].fldbuf);    /*KEY*/
  }
}
...
/* The following is a EM info structure
 * last_index - indicate last index we search. It supposed to be (in most cases) the next index I look for.
 * entry_busy - indicating if row 'r' in  is free or occupied 
 * nof_entries - number of rows in this EM.
 * key_size_in_bits
 * result_size_in_bits
 */
int   qax_em_index;     /* next EM db index */
typedef struct {
  int  last_index;
  char *entry_busy;
  int  nof_entries;
...
/* The following describes an LPM entry:
 * prefix_length - length in bit of prefix
 * key_size_in_bite    - size (in bits) of the LPM key (and mask) 
 * result_size_in_bite - size (in bits) of the LPM result
 *
 * valid - valid bit data
 * key   - key data
 * mask  - mask data
 * result- result data
 */
...
/* The following describes one LPM DB:
 * tcam_id          - TCAM ID that actually implement this LPM DB
 * nof_free_entries - Number of free rows in this LPM DB
 * nof_lpm_entries  - Size (no. of rows) of this LPM
 * entries          - array (of size == nof_lpm_entries) contains LPM entries
 */
...
lpm_db_t* qax_lpm_info;  /* Array of all LPM DBs */
