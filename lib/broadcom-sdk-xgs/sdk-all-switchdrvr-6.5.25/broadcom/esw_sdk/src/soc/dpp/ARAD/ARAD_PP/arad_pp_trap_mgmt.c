...
#define ARAD_PP_TRAP_MGMT_COUNTER_ID_MAX                         (0x100000 - 1) /* 19 full bits*/
...
Arad_pp_trap_mgmt_ud_start[] ={
    ...
    (SOC_PPC_TRAP_CODE_INTERNAL_IHP_TIMNA_PREFIX|0) /* Last traps used for ITMH.Snoop field */
};
...
Arad_pp_trap_mgmt_ud_end[] ={
    ...
    (SOC_PPC_TRAP_CODE_INTERNAL_IHP_FLP4_PREFIX|15),
    (SOC_PPC_TRAP_CODE_INTERNAL_IHP_TIMNA_PREFIX|15) /* Last traps used for ITMH.Snoop field */
};
  ...
  )
{
  ...
  /* Configure filter action to drop packets - strength must be set to 1 and forward enable should be set to 0 */
  if (!SOC_WARM_BOOT(unit)) {
      if (SOC_IS_JERICHO(unit)) {
          ...
          for (tbl_line_ndx = 0; tbl_line_ndx < ARAD_PP_TRAP_MGMT_EPNI_PMF_MIRROR_PROFILE_TABLE_INDEX_MAX; tbl_line_ndx++) {
              ...
              if(tbl_line_ndx == 0) /* First profile should be Forward-Enabled */
              {
              ...
              soc_mem_field32_set(unit, EPNI_PMF_MIRROR_PROFILE_TABLEm, &tbl_entry, FWD_STRENGTHf, 0); /* Strength = 0 since it is the default and first egress filter in the pipe */
  ...
  {
      /* Enable default EGRESS forwarding - former called VSI-Membership */

      /* Membership_type = 1 for VSI Filtering (Backward Compatibility). Membership_type = 0 is for VLAN filtering (Supported since QAX).*/
      ...
      /* Enable Egress Membership check per outlif profile */
...
/*****************************************************************************
* Function:  arad_pp_trap_mgmt_trap_code_to_internal
* Purpose:   Convert PPC trap code to internal trap code (index at IHB_FWD_ACT_PROFILE)
* Params:
* unit      (IN)         - Device Number
* trap_code_ndx   (IN)   - PPC trap code (SW id)
* trap_code_internal (OUT) - internal trap code (HW id)
* strength_fld_fwd (OUT)  - Filed of register to fill frw strentgth
* strength_fld_snp  (OUT) -  Filed of register to fill snoop strentgth
* Return:    (uint32)
*******************************************************************************/
uint32
  ...
  )
{
  ...
  case  SOC_PPC_TRAP_CODE_MPLS_TERM_TTL_0: /* for mates configuration (configures FLP and VTT blocks) */
  case  SOC_PPC_TRAP_CODE_MPLS_TUNNEL_TERMINATION_TTL0: /* for independent VTT configuration */
    *trap_code_internal = SOC_PPC_TRAP_CODE_INTERNAL_VTT_MPLS_TTL_0;
    ...
    break;
  case  SOC_PPC_TRAP_CODE_MPLS_TERM_TTL_1: /* for mates configuration (configures FLP and VTT blocks) */
  case  SOC_PPC_TRAP_CODE_MPLS_TUNNEL_TERMINATION_TTL1: /* for independent VTT configuration */
  ...
  case  SOC_PPC_TRAP_CODE_IPV4_CHECKSUM_ERROR:
    *trap_code_internal = SOC_PPC_TRAP_CODE_INTERNAL_FLP_IPV4_CHECKSUM_ERROR;
    /* The field names are different in Jericho*/
  ...
  case  SOC_PPC_TRAP_CODE_IPV6_MULTICAST_DESTINATION:
    *trap_code_internal = SOC_PPC_TRAP_CODE_INTERNAL_FLP_IPV6_MULTICAST_DESTINATION;
    ...
    break;
  case  SOC_PPC_TRAP_CODE_MPLS_TTL0: /* for mates configuration (configures FLP and VTT blocks) */
  case  SOC_PPC_TRAP_CODE_MPLS_FORWARDING_TTL0: /* for independent FLP configuration */
    *trap_code_internal = SOC_PPC_TRAP_CODE_INTERNAL_FLP_MPLS_TTL0;
    ...
    break;
  case  SOC_PPC_TRAP_CODE_MPLS_TTL1: /* for mates configuration (configures FLP and VTT blocks) */
  case  SOC_PPC_TRAP_CODE_MPLS_FORWARDING_TTL1: /* for independent FLP configuration */
...
/* Arad not supported
  case  SOC_PPC_TRAP_CODE_MC_USE_SIP_RPF_FAIL:
    *trap_code_internal = SOC_PPC_TRAP_CODE_INTERNAL_FER_MC_USE_SIP_RPF_FAIL;
  ...
  case  SOC_PPC_TRAP_CODE_MC_USE_SIP_ECMP:
    *trap_code_internal = SOC_PPC_TRAP_CODE_INTERNAL_FER_MC_USE_SIP_ECMP;
 ...
 */
  ...
  case  SOC_PPC_TRAP_CODE_USER_ETH_OAM_UP_ACCELERATED:
    *trap_code_internal = SOC_PPC_TRAP_CODE_INTERNAL_USER_ETHERNET_OAM_UP_ACCELERATED;
    break;
    /*Jericho only - It was taken care to return an error if the device is not Jericho at the BCM level*/
  ...
  case  SOC_PPC_TRAP_CODE_INNER_ETHERNET_MY_MAC_IP_DISABLED:
    *trap_code_internal = SOC_PPC_TRAP_CODE_INTERNAL_INNER_ETHERNET_MY_MAC_IP_DISABLED;
    break;
  default:
    /* SOC_PPC_TRAP_CODE_RESERVED_MC_0-7 */
    ...
    /* SOC_PPC_TRAP_CODE_1588_PACKET_0-7 */
    ...
    /* SOC_PPC_TRAP_CODE_USER_DEFINED_0-59 */
...
/*****************************************************************************
* Function:  arad_pp_trap_mgmt_tunnel_termination_trap_code_to_reg
* Purpose:   Convert PPC trap code to tunnel termination trap registers
* Params:
* unit      (IN)         - Device Number
* trap_code_ndx   (IN)   - PPC trap code (SW id)
* strength_fld_fwd (OUT)  - Filed of register to fill frw strentgth
...
/*****************************************************************************
* Function:  arad_pp_trap_mgmt_check_user_define
* Purpose:   Check if the trap was defined as user defined,
                    if yes fills to/from which registers are the FWD Strength and SNP  Strength for that trap
* Params:
* unit      (IN)         - Device Number
* trap_code_ndx   (IN)   - PPC trap code (SW id)
* strength_fld_fwd (OUT)  - Filed of register to fill frw strentgth
  ...
  )
{
  ...
  case  SOC_PPC_TRAP_CODE_INTERNAL_VTT_MPLS_TTL_0:
    /* If SOC_PPC_TRAP_CODE_MPLS_TUNNEL_TERMINATION_TTL0 is allocated the case is independent configuration,
       *  meaning only the VTT is configured.
       * Otherwise the case is mates configuration, meaning FLP and VTT are configured the same.
       * Return the suitable SOC PPC trap code. */
  ...
  case  SOC_PPC_TRAP_CODE_INTERNAL_VTT_MPLS_TTL_1:
    /* If SOC_PPC_TRAP_CODE_MPLS_TUNNEL_TERMINATION_TTL1 is allocated the case is independent configuration,
       *  meaning only the VTT is configured.
       * Otherwise the case is mates configuration, meaning FLP and VTT are configured the same.
       * Return the suitable SOC PPC trap code. */
  ...
  case  SOC_PPC_TRAP_CODE_INTERNAL_FLP_MPLS_TTL0:
    /* If SOC_PPC_TRAP_CODE_MPLS_TTL0 is allocated the case is mates configuration,
     *  meaning FLP and VTT configured the same.
     * Otherwise the case is independent configuration, meaning only FLP configuration.
     * Return the suitable SOC PPC trap code. */
  ...
  case  SOC_PPC_TRAP_CODE_INTERNAL_FLP_MPLS_TTL1:
    /* If SOC_PPC_TRAP_CODE_MPLS_TTL1 is allocated the case is mates configuration,
     *  meaning FLP and VTT configured the same.
     * Otherwise the case is independent configuration, meaning only FLP configuration.
     * Return the suitable SOC PPC trap code. */
...
/* Arad not supported
  case  SOC_PPC_TRAP_CODE_INTERNAL_FER_MC_USE_SIP_RPF_FAIL:
    *trap_code = SOC_PPC_TRAP_CODE_MC_USE_SIP_RPF_FAIL;
    break;
  case  SOC_PPC_TRAP_CODE_INTERNAL_FER_MC_USE_SIP_ECMP:
    *trap_code = SOC_PPC_TRAP_CODE_MC_USE_SIP_ECMP;
    break;
 */
  ...
  default:
    ...
    /* SOC_PPC_TRAP_CODE_UNKNOWN_DA_0-7 */
  ...
  )
{
  ...
  is_routed = ((fld_val == ARAD_PP_TRAP_MGMT_IPV4_UC_ROUTE_VAL) ||
    ...
    SOC_SAND_CHECK_FUNC_RESULT(res, 60, exit);
     ...
     /* Advance header ptr to end of internal */
    ...
    {
        /*For non-trapped packets*/
        curr_dnx_header->internal.trap_id = -1;
        
        /*For snooped or mirrored packets get CPU trap code qualifier */ 
    ...
    /* Advance header ptr to end of internal ext, if exists */
    if (e_ext)
    {
  ...
  )
{
  ...
  curr_dnx_header = &(dnx_pkt->dnx_header_stack[dnx_pkt->dnx_header_count]);
   ...
   /* Advance header ptr to end of FTMH. Note that during this functions, header_ptr
     always points to the beginning of the next header (FTMH ot other), since it used as
  ...
  if (has_internal) {
      /* Both ptr's now points to the internal */
      header_ptr = packet_info->ntwrk_header_ptr;
   ...
   /* OAM DM TOD header, following UDH if UDH exists */
   if (oam_dm_tod_exist) {
       packet_info->ntwrk_header_ptr += 4;
       /* dnx_pkt won't be updated here, as it's only used to dump by now */
   }
      ...
      )
  {
      ...
      if (user_header_size && !packet_info->is_punted) {
          /* Punt packets do not have UDH in the inner header */
  ...
  )
{
  ...
  /*Some traps may be implemented as user define
        Need to check before writing strength to registers*/
  if((trap_code_ndx >= SOC_PPC_TRAP_CODE_USER_DEFINED_0) &&
      (trap_code_ndx <= SOC_PPC_TRAP_CODE_USER_DEFINED_LAST) &&
      (profile_info->strength > 0)) /*If its not actually a user define trap but one thats use it place*/
  {
  ...
  if (strength_fld_fwd.reg != INVALIDr) 
  {
    ...
    {
        SOC_REG_ABOVE_64_CLEAR(fld_val);
        /* UNKNOWN L3 protocol hardware trap code */
  ...
  )
{
  ...
  /* config per core section - Jericho and above */
...
/* is_ingress==1 iff trap_code_internal is a jericho only ingress trap*/
int
arad_pp_trap_is_jericho_ingress_trap(/*in*/  SOC_SAND_IN  int32 trap_code_internal,
                                     /*out*/ uint8 *is_ingress)
...
int
arad_pp_trap_base_q_pair_to_port(/*in*/SOC_SAND_IN int unit,SOC_SAND_IN  int core,SOC_SAND_IN  uint32 base_q_pair, /*out*/ SOC_SAND_OUT  uint32 *tm_port)
{
...
/* get the first cpu port of spcified core. BCM_E_FAIL will be returned if no such port has been found*/
int
arad_pp_trap_get_first_cpu_port(/*in*/ SOC_SAND_IN int unit, SOC_SAND_IN int core, /*out*/ SOC_SAND_OUT  uint32 *cpu_port_tm )
{
    uint32 port_i;
    int core_lcl;
    soc_error_t rv;
...
/* defined_on_all_cores == 1 iff there is a cpu port on all of the device's core */
int
arad_pp_trap_cpu_ports_on_all_cores(/*in*/ SOC_SAND_IN int unit, /*out*/ SOC_SAND_OUT uint8 *defined_on_all_cores)
{
    int rv;
    int core;
    uint32 port;
...
/* This function sets the out_tm port in tbl_data according to the tm_port and the given profile_info */
int 
arad_pp_trap_action_profile_set_tm_port(/*in*/ SOC_SAND_IN int unit,SOC_SAND_IN int core, SOC_SAND_IN SOC_PPC_TM_PORT tm_port, SOC_SAND_IN SOC_PPC_TRAP_EG_ACTION_PROFILE_INFO *profile_info, /*out*/ SOC_SAND_OUT ARAD_PP_EGQ_ACTION_PROFILE_TABLE_TBL_DATA *tbl_data)
{
    ...
    /* Get the right core of out_tm port */
...
/* This function gets the out_tm port and hdr data from tbl_data according to the tm_port */
int 
arad_pp_trap_action_profile_get_tm_port_and_hdr_data(/*in*/ SOC_SAND_IN int unit, SOC_SAND_IN int core, SOC_SAND_IN ARAD_PP_EGQ_ACTION_PROFILE_TABLE_TBL_DATA *tbl_data,/*out*/  SOC_SAND_OUT SOC_PPC_TM_PORT *tm_port, SOC_SAND_OUT SOC_PPC_TRAP_EG_ACTION_HDR_DATA    *profile_hdr_data, SOC_SAND_OUT SOC_PPC_TRAP_EG_ACTION_PROFILE_INFO *profile_info)
{
