...
/**
 * Generic fields of the FTMH base header
 * NOTE! Start bit is calculated on base of  uint32 operations and 80bit size of FTMH base header
 */
 ...
 */
/**
 * Generic fields of the transmited self(destination) routed data VSC256 cell
 */
 ...
 */
/**
 * Generic fields of the transmited self(destination) routed data VSC512 cell
 */
 ...
 * Purpose:
 *      Build FTMH base header which is the minimum system header requirement for a packet to be accepted by the egress
 * Parameters:
 *      unit                    - (IN)  Unit number.
 *      pp_destination_port     - (IN) pp_destination_port
 *      paket_size              - (IN) paket_size
 *      buf                        - (OUT) 10B of FTMH header result
 * Returns:
 *      _SHR_E_XXX
 */
shr_error_e
 ...
 * Parameters:
 *      unit            - (IN)  Unit number.
 *      cell            - (IN/OUT) dest routed cell info
 *      buf_size        - (IN) buffer size
 *      buf             - (OUT) pointer to a uint32 array where the header will be configured
 * Returns:
 *      _SHR_E_XXX
 */
shr_error_e
    ...
    uint32 *buf)
{
    ...
    /**
     * Assuming we send 1 cell using the largest supported cell format
     */
    switch (nof_pipes)
    {
        case 1:
    ...
    /**
     * Predefined values for current implementations
     * Currently it is supported only sending of 1 packet at a time with data together with the cell and headers to 256B
     */
    cell->header.last_cell_of_paket = 1;
    cell->header.fragment_number = 1;
    /**
     * PCP has to be enabled for simple packing
     * Value of 1 means cell is not simple packed (either unpacked, or cell is continues packed)
     */
    ...
    /*
     * Go over the table and populate the relevant bits with the values from the cell info
     */
    line = 0;
    while (table_pointer[line].dest_id != soc_dnxc_fabric_cell_field_invalid)
    {
