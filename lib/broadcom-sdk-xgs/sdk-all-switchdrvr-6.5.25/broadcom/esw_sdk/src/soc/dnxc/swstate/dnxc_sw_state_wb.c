    ...
    uint32 sw_state_max_size)
{
    ...
    switch (init_mode) {
        ...
        case socDnxcSwStateDataBlockRestoreAndOveride:
            ...
            /* make scache_ptr 8 bytes aligned */
...
int dnxc_sw_state_data_block_header_init(int unit, uint32 size) {
    ...
    /* decrease from the buffer size the block used for alignment */
...
void dnxc_sw_state_wb_module_size_update(int unit, uint32 module_id, uint32 allocated_size, uint32 flags){
    ...
    /* check if we are increasing (flags = 1) or decreasing */
    if (flags == 0)
    {
