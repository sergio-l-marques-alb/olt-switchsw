...
#include <soc/dnxc/dnxc_data/init_internal/dnxc_data_init_internal.h>
...
#ifndef BCM_DNX_INTERNAL_INIT
shr_error_e
dnxc_data_internal_init_load_dispatch(
    int unit)
{
...
/**
 * brief - Return dynamic data file path for internal SKU revision (device ID + revision ID)
 */
...
/**
 * brief - Return dynamic data file path for internal SKU
 */
...
/**
 * brief - Return dynamic data file path for adapter (device ID)
 */
shr_error_e dnxc_data_dyn_loader_adapter_file_path_get(
    int unit,
    char file_path[RHFILE_MAX_SIZE]);

/**
 * brief - Return dynamic data file path for adapter revision (device ID + revision ID)
 */
...
#if defined(BCM_DNX_SUPPORT)
/**
 * brief - Return memory profile file path
 */
static shr_error_e dnx_memory_profile_file_path_get(
    int unit,
    char memory_profile_file[RHFILE_MAX_SIZE]);
#endif
/*
 * }
 */
...
static dnxc_data_dyn_file_path_get_f dnx_data_dyn_files[] = {
    ...
    dnx_memory_profile_file_path_get,
#ifdef ADAPTER_SERVER_MODE
...
static dnxc_data_dyn_file_path_get_f dnxf_data_dyn_files[] = {
    ...
    dnxc_data_dyn_loader_internal_sku_revision_file_path_get
#ifdef ADAPTER_SERVER_MODE
        , dnxc_data_dyn_loader_adapter_file_path_get,
    dnxc_data_dyn_loader_adapter_revision_file_path_get
#endif /* ADAPTER_SERVER_MODE */
#if defined(BCM_DNX_SUPPORT)
        , dnx_memory_profile_file_path_get
    ...
    dnxc_data_property_t * property_info)
{
    ...
    if (!sal_strncasecmp(method, "", sizeof(method)))
    {
    ...
    dnxc_data_dyn_table_data_type * data_type)
{
     ...
     */
    if (!sal_strncasecmp(type_name, "bcm_pbmp_t", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
    {
        *data_type = dnxc_data_dyn_table_data_type_pbmp;
    }
    /** handle int type */
    else if (!sal_strncasecmp(type_name, "int", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
    {
        *data_type = dnxc_data_dyn_table_data_type_int;
    }
    /** handle uint32 type */
    else if (!sal_strncasecmp(type_name, "uint32", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
    {
        *data_type = dnxc_data_dyn_table_data_type_uint32;
    }
    /** handle int array type */
    else if (!sal_strncasecmp(type_name, "int[", sal_strnlen("int[", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH)))
    {
        *data_type = dnxc_data_dyn_table_data_type_int_array;
    }
    /** handle uint32 array type */
    else if (!sal_strncasecmp
             (type_name, "uint32[", sal_strnlen("uint32[", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH)))
    {
        *data_type = dnxc_data_dyn_table_data_type_uint32_array;
    }
    /** handle uint8 type */
    else if (!sal_strncasecmp(type_name, "uint8", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
    {
        *data_type = dnxc_data_dyn_table_data_type_uint8;
    }
    /** handle strings */
    else if (!sal_strncasecmp(type_name, "char *", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
    {
    ...
    dnxc_data_define_t * numeric_info)
{
    ...
    if (numeric_info->dynamic_data_node != NULL)
    {
        ...
        RHDATA_GET_INT_EXIT_IF_NOT_FOUND(numeric_info->dynamic_data_node, "value", numeric_default_value);
    ...
    char file_path[RHFILE_MAX_SIZE])
{
    ...
    if (cur_top == NULL)
    {
        SHR_IF_ERR_EXIT(dnxc_data_internal_init_load_dispatch(unit));
        SHR_EXIT();
    }
    ...
    if (sal_strncasecmp(module_name, "", sizeof(module_name)))
    {
    ...
    char file_path[RHFILE_MAX_SIZE])
{
    ...
    sal_strncat_s(file_name, "_data.xml", sizeof(file_name));

    /** add file name to the relative path */
    sal_strncat_s(file_path, file_name, RHFILE_MAX_SIZE);
...
#ifdef ADAPTER_SERVER_MODE
/**
 * \brief - construct a file path relative to db directory of the current chip adapter revision (device ID + revision ID)
 */
shr_error_e
dnxc_data_dyn_loader_adapter_file_path_get(
    int unit,
    char file_path[RHFILE_MAX_SIZE])
{
    ...
    SHR_IF_ERR_EXIT(dnxc_data_dyn_loader_sku_file_path_common_get(unit, DNXC_DATA_DYN_ADAPTER_XML_DB_PATH, file_path));
    ...
    char memory_profile_file[RHFILE_MAX_SIZE])
{
    ...
    char name[RHFILE_MAX_SIZE] = { 0 };
    char base_name[RHFILE_MAX_SIZE] = { 0 };
    char *memory_footprint_profile = NULL;

    SHR_FUNC_INIT_VARS(unit);

    SHR_IF_ERR_EXIT(dbx_file_get_db(unit, name, base_name));
    memory_footprint_profile = soc_property_get_str(unit, "memory_footprint_profile");

    sal_strncat_s(file_name, "/data/internal/", sizeof(file_name));
    sal_strncat_s(file_name, name, sizeof(file_name));
    sal_strncat_s(file_name, "_memory_profile_", sizeof(file_name));
    if (memory_footprint_profile != NULL)
    {
        sal_strncat_s(file_name, memory_footprint_profile, sizeof(file_name));
    }
    sal_strncat_s(file_name, ".xml", sizeof(file_name));
    sal_strncpy(memory_profile_file, file_name, RHFILE_MAX_SIZE - 1);
