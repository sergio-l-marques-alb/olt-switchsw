    ...
    void *data)
{
    ...
    /** De-construct the key */
    SHR_IF_ERR_EXIT(utilex_bitstream_get_field(key_temp, DNX_MULTICAST_HASH_KEY_DESTINATION_START_BIT, DNX_MULTICAST_HASH_KEY_DESTINATION_NOF_BITS, &destination));
    SHR_IF_ERR_EXIT(utilex_bitstream_get_field(key_temp, DNX_MULTICAST_HASH_KEY_REFERENCE_NUM_START_BIT, DNX_MULTICAST_HASH_KEY_REFERENCE_NUM_NOF_BITS, &ref_num));
    SHR_IF_ERR_EXIT(utilex_bitstream_get_field(key_temp, DNX_MULTICAST_HASH_KEY_DESTINATION_TYPE_START_BIT, DNX_MULTICAST_HASH_KEY_DESTINATION_TYPE_NOF_BITS, &destination_type));
    SHR_IF_ERR_EXIT(utilex_bitstream_get_field(key_temp, DNX_MULTICAST_HASH_KEY_ENCAPSULATION_ID_START_BIT, DNX_MULTICAST_HASH_KEY_ENCAPSULATION_ID_NOF_BITS, &cud));
    SHR_IF_ERR_EXIT(utilex_bitstream_get_field(key_temp, DNX_MULTICAST_HASH_KEY_GROUP_ID_START_BIT, DNX_MULTICAST_HASH_KEY_GROUP_ID_NOF_BITS, &group_id));
    SHR_IF_ERR_EXIT(utilex_bitstream_get_field(key_temp, DNX_MULTICAST_HASH_KEY_IS_INGRESS_START_BIT, DNX_MULTICAST_HASH_KEY_IS_INGRESS_NOF_BITS, &is_ingress));
    SHR_IF_ERR_EXIT(utilex_bitstream_get_field(key_temp, DNX_MULTICAST_HASH_KEY_CORE_ID_START_BIT, DNX_MULTICAST_HASH_KEY_CORE_ID_NOF_BITS, &core_id));

    DNX_SW_STATE_PRINT(unit, "Core ID:0x%x Group ID:0x%x Is Ingress:0x%x Encap ID:0x%x Destination Type:0x%x Destination:0x%x Reference:0x%x - MCDB Entry:0x%x", core_id, group_id, is_ingress, cud, destination_type, destination, ref_num, *mydata);
