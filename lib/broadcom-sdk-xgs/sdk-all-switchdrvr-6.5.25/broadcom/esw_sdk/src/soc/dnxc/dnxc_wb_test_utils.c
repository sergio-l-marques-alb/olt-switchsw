...
/* holds the warmboot backoff rate parameter for each unit */
int dnxc_warm_boot_exp_param[SOC_MAX_NUM_DEVICES] = { 0 };
/* holds the number of guaranteed reboots at the beginning of the session for each unit */
    ...
    int *nof_skip)
{
    ...
    /*
     * The reboot is performed under 1 of 2 conditions:
     *  1. The test is one of the first tests (param: dnxc_nof_guaranteed_reboots)
     *  2. The test is a multiple of the highest power of dnxc_warm_boot_exp_param (that is <= to it)
     */
    ...
    else
    {
        ...
        /*
         * nof_skip is set to be the number of tests we skip the reboot for
         * It is only set for tests where the reboot is skipped and immediately following a test where reboot was performed
         * nof_skip is set under 2 circumstances:
         *  1. test_counter is 1 greater than dnxc_nof_guaranteed_reboots (the test immediately following the last of the guaranteed reboots)
         *  2. test_counter is 1 greater than a multiple of the highest power of dnxc_warm_boot_exp_param (that is <= to it)
         */
