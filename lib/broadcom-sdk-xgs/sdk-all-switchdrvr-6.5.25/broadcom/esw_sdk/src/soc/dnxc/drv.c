...
/*
 * Perform a CMIC (only) reset instead of the DMU hard reset.
 */
static shr_error_e
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(READ_CMICX_M0_IDM_IDM_RESET_CONTROLr(unit, &reg32_val));    /* read current reset register value */
    soc_reg_field_set(unit, CMICX_M0_IDM_IDM_RESET_CONTROLr, &reg32_val, RESETf, 1);    /* mark a reset */
    SHR_IF_ERR_EXIT(WRITE_CMICX_M0_IDM_IDM_RESET_CONTROLr(unit, reg32_val));    /* perform the reset */
    SHR_IF_ERR_EXIT(READ_CMICX_M1_IDM_IDM_RESET_CONTROLr(unit, &reg32_val));    /* read current reset register value */
    soc_reg_field_set(unit, CMICX_M1_IDM_IDM_RESET_CONTROLr, &reg32_val, RESETf, 1);    /* mark a reset */
    SHR_IF_ERR_EXIT(WRITE_CMICX_M1_IDM_IDM_RESET_CONTROLr(unit, reg32_val));    /* perform the reset */
    SHR_IF_ERR_EXIT(READ_CMICX_M2_IDM_IDM_RESET_CONTROLr(unit, &reg32_val));    /* read current reset register value */
    soc_reg_field_set(unit, CMICX_M2_IDM_IDM_RESET_CONTROLr, &reg32_val, RESETf, 1);    /* mark a reset */
    SHR_IF_ERR_EXIT(WRITE_CMICX_M2_IDM_IDM_RESET_CONTROLr(unit, reg32_val));    /* perform the reset */
    SHR_IF_ERR_EXIT(READ_CMICX_S0_IDM_IDM_RESET_CONTROLr(unit, &reg32_val));    /* read current reset register value */
    soc_reg_field_set(unit, CMICX_S0_IDM_IDM_RESET_CONTROLr, &reg32_val, RESETf, 1);    /* mark a reset */
    SHR_IF_ERR_EXIT(WRITE_CMICX_S0_IDM_IDM_RESET_CONTROLr(unit, reg32_val));    /* perform the reset */
    sal_usleep(1000);
    SHR_IF_ERR_EXIT(READ_CMICX_M0_IDM_IDM_RESET_CONTROLr(unit, &reg32_val));    /* read current reset * * register
                                                                                 * value */
    soc_reg_field_set(unit, CMICX_M0_IDM_IDM_RESET_CONTROLr, &reg32_val, RESETf, 0);    /* mark a reset */
    SHR_IF_ERR_EXIT(WRITE_CMICX_M0_IDM_IDM_RESET_CONTROLr(unit, reg32_val));    /* perform the reset */
    SHR_IF_ERR_EXIT(READ_CMICX_M1_IDM_IDM_RESET_CONTROLr(unit, &reg32_val));    /* read current reset * * register
                                                                                 * value */
    soc_reg_field_set(unit, CMICX_M1_IDM_IDM_RESET_CONTROLr, &reg32_val, RESETf, 0);    /* mark a reset */
    SHR_IF_ERR_EXIT(WRITE_CMICX_M1_IDM_IDM_RESET_CONTROLr(unit, reg32_val));    /* perform the reset */
    SHR_IF_ERR_EXIT(READ_CMICX_M2_IDM_IDM_RESET_CONTROLr(unit, &reg32_val));    /* read current reset * register value */
    soc_reg_field_set(unit, CMICX_M2_IDM_IDM_RESET_CONTROLr, &reg32_val, RESETf, 0);    /* mark a reset */
    SHR_IF_ERR_EXIT(WRITE_CMICX_M2_IDM_IDM_RESET_CONTROLr(unit, reg32_val));    /* perform the reset */
    SHR_IF_ERR_EXIT(READ_CMICX_S0_IDM_IDM_RESET_CONTROLr(unit, &reg32_val));    /* read current reset register value */
    soc_reg_field_set(unit, CMICX_S0_IDM_IDM_RESET_CONTROLr, &reg32_val, RESETf, 0);    /* mark a reset */
    SHR_IF_ERR_EXIT(WRITE_CMICX_S0_IDM_IDM_RESET_CONTROLr(unit, reg32_val));    /* perform the reset */

exit:
    SHR_FUNC_EXIT;
}
#endif /* USE_SOC_DNXC_CMICX_RESET_USING_IDM */
...
/**
 * \brief
 *   Simulate an ECI reset by writing default values to its registers.
 *   Needs to be run the first thing after configuring the ring map,
 *   to prevent resetting sbus registers that were already configured.
 */
shr_error_e
soc_dnxc_eci_reset(
    int unit)
{
    ...
    /** Set ECI registers to their default values */
    for (reg = 0; reg < NUM_SOC_REG; reg++)
    {
        if (!SOC_REG_IS_VALID(unit, reg) || /** skip registers not in the device */
            !SOC_BLOCK_IS(SOC_REG_INFO(unit, reg).block, SOC_BLK_ECI) || /** skip registers not in ECI */
            (SOC_REG_INFO(unit, reg).flags & /** skip read only/interrupt/counter/signal registers */
             (SOC_REG_FLAG_RO | SOC_REG_FLAG_INTERRUPT | SOC_REG_FLAG_GENERAL_COUNTER | SOC_REG_FLAG_SIGNAL)))
        {
            continue;
        }
        /** We do not want to write to registers who will affect the correct operation of the device. */
        switch (reg)
        {
            /** do not affect the indirect access mechanism. */
            case ECI_INDIRECT_COMMAND_WR_DATAr:
            case ECI_INDIRECT_COMMANDr:
                                       /** would cause memory access failures */
            case ECI_INDIRECT_COMMAND_ADDRESSr:
            /** Do not touch CPU2TAP */
    ...
    int unit)
{
    ...
    SHR_FUNC_INIT_VARS(unit);
    /*
     * Get dnx/dnxf values for mdio
     * internal and external divisor dnx data values
     * internal and external delay dnx data values
     */
    ...
    /**
     * The MDIO has only 1 register got divisor delay
     * so we set the internal as its inital value
     * BUT each time we access through the mdio we change the value
     * depending of if it is external or internal access
     */
...
/*
 * Verify that this the device init succeeded, can be checked in B0 devices.
 */
shr_error_e
...
#ifdef PLISIM
        !SAL_BOOT_PLISIM &&     /* may not be modelled in simulation and emulation */
#endif
        soc_sand_is_emulation_system(unit) == 0)
    {
        SHR_IF_ERR_EXIT(READ_ECI_RESERVED_30r(unit, &val));     /* test if this is a production device */
        if (val == 0)
        {
...
/*
 * We will use register ICFG_GEN_PURPOSE_REG[15] to store state in the deice.
 * This register is set to its default value of 0 on out of system reset, but
 * not in hard resets we perform.
 */
/** values and masks used to mark that we are using the register for this purpose */
#define PERSISTANT_VALUE_MARKING_USED 0xaface8c0
#define PERSISTANT_VALUE_MARKING_MASK 0xfffffff0
/** value used to mark that a system reset is needed due to MBIST or to another planned reason */
#define PERSISTANT_VALUE_NEEDS_SYSTEM_RESET 1
/** bit used to mark that the device had */
#define PERSISTANT_VALUE_IN_CPU2TAP 2
/** bit used to mark the device had an error in a CPU2TAP sequence */
#define PERSISTANT_VALUE_CPU2TAP_ERROR 4

/** value used to test if a system reset was performed */
    ...
    int unit)
{
    ...
    soc_bar_read_reg32(unit, r, r_index, &reg32_val); /** read the register */
    /** returns  true if ADDRf is marked as used and VALIDf==0 and ADDRf is marked as needing a system reset */
...
/**
 * \brief
 *   check if the device is used by another SDK process, if not mark the current process ID as the SDK process.
 *   print a warning message if the device is already used.
 */
void
    ...
    int unit)
{
    ...
    /**   We are not stopping the SDK as the existing process may not be an SDK, in case the SDK existed, perhaps even the OS restarted, and now we have a different process with this ID */
    ...
    /** device is not used by a process that still exists, mark the device as used by the current process */
    soc_bar_write_reg32(unit, r, r_index, pid);
}

/**
 * \brief
 *   check if the device is marked with persistent settings.
...
/**
 * \brief
 *   Sets a flag in a persistent (over system resets) register, marking that this mechanism is in use.
 */
static void
    ...
    uint32 flag_to_set)
{
    ...
    soc_bar_read_reg32(unit, r, r_index, &orig_reg_val); /** read the register */
    /** If the register was not yet used for this mechanism, mark it as being used */
    ...
    new_reg_val &= flag_mask; /** Clear the previous flag value */
    new_reg_val |= flag_to_set; /** set the new flag value */

    if (new_reg_val != orig_reg_val)
    {
        soc_bar_write_reg32(unit, r, r_index, new_reg_val); /** write back the register */
    }
}
...
/**
 * \brief
 *   Sets if the device is marked as being inside a CPU2TAP sequence .
 *   Used at the start and end of a CPU2tap sequences.
 */
void
...
/**
 * \brief
 *   Sets if the device is marked as having an error in a CPU2TAP sequence since the last out of system reset.
 */
void
 ...
 *   This requires that a system reset callback was registered by the applicationm
 ...
 *   Returns SOC_E_NONE if The system reset was performed and it was successful.
 */
int
    ...
    int unit)
{
        ...
        SOC_CONTROL(unit)->system_reset_cb(unit, SOC_SYSTEM_RESET_FLAG_IS_SUPPORTED) == SOC_E_NONE)
    {
        ...
        /** set register to be changed by the system reset */
        ...
        READ_ICFG_PCIE_0_STRAPSr(unit, &straps_val); /** Read straps value for checking later */

        /** mark as needing system reset, this marking will be void by a system reset or power cycle */
        soc_dnxc_set_need_system_reset(unit, 1);

        results = SOC_CONTROL(unit)->system_reset_cb(unit, 0); /** perform system reset */
        if (results != SOC_E_NONE)
        {
            LOG_ERROR(BSL_LOG_MODULE, (BSL_META_U(unit, "Device system reset failed.\n")));
        }
        else
        {   /** check that the system reset was successful */
            READ_ICFG_PCIE_0_STRAPSr(unit, &straps_val2); /** Read straps value */
