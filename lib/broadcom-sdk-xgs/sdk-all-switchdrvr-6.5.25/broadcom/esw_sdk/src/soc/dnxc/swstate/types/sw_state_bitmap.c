    ...
    char *dbg_string)
{
    ...
    /* get the offset to the SHR_BITDCL that contains the first bit */
    ...
    do {
        ...
        /* calculate the range we need to read in this iteration */
        ...
        /* write back to sw state memory and journal (only if it got changed)*/
        ...
        /* only first iteration starts from a non-zero bit */
        bit_offset = 0;

        /* iterate in byte granularity */
        byte_offset += sizeof(SHR_BITDCL);

        /* advance also the src */
        src_offset += calculated_range;

        /* reduce the amount of "range" we read in this iteration */
        range -= calculated_range;

        /* clear the tmp bitmap sandbox */
        tmp_storage[0] = 0;

        /* the range should not get below zero */
        assert(((int) range) >= 0);
