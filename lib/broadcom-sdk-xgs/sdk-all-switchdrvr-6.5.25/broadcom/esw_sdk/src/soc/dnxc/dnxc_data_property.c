    ...
    const char *suffix)
{
    ...
    *fw_load_method = load_firmware_prop_val & DNXC_DATA_PROPERTY_FW_LOAD_METHOD_BITS_MASK;
    ...
    /** Check if fw_load_method is 0 - do not load firmware and exit with error. */
    if (*fw_load_method == 0)
    {
    ...
    else
    {
        ...
        /*
         * coverity explanation: coverity has found that we try to test if unsigned variable is smaller than zero, we
         * want to keep this check although it's redundant in order to be generic Conclusion: It's only redundant check 
         * and not real vulunerability 
         */
