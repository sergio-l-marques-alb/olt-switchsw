    ...
    const char *cint_string)
{
    ...
    sal_strlcpy(cint_name, cint_string, MTA_MAX_CINT_NAME_LENGTH - 1);;
    ...
    int unit)
{
    ...
    for (i = 0; i < mta_db[unit].nof_registered_threads; i++)
    {
        ...
        {
            sal_strlcpy(mta_db[unit].thread[i].name, "main", MTA_MAX_THREAD_NAME_LENGTH - 1);
        }
    ...
    uint8 in_play)
{
    ...
    if (in_play == TRUE)
    {
        int len = sal_strnlen(api_name, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH);
    ...
    char *res_name)
{
    ...
    sal_strlcpy(mta_db[unit].dynamic_resource_ids[resource][mta_db[unit].nof_dynamic_resources[resource]], res_name,
