...
/*undef if you want print skipped ll*/
/*#define SORTED_LL_EXTENDED_PRINT*/
...
#define TRANSLATE_LL_SERVICE_IDX(index)           (index - ll->max_elements) /* used to translate service node to service ll index */
...
STATIC int sw_state_ll_verify_order(int unit, uint32 module_id, sw_state_ll_t ll, uint32 ll_index, sw_state_ll_node_t *created_node){
    ...
    SW_STATE_CB_DB_GET_CB(
     ...
     /* in case of sorted list, the key must be bigger than prev->key and smaller than next->key.
     * In addition, because it is a skipped sorted list, down->key should be equal */
...
* \param [in] down  - the down node, relevant when it is sorted linked list & service node, otherwise - NULL.
* \param [out] created_node  - the created node will be put here.
* \param [in] key    - the content of the new node's key
* \param [in] value    - the content of the new node's value
* \param [in] key_pointer - a pointer for a key in ll->keys array
* \param [in] is_service_node - 1 if it is a service node, 0 regular node (relevant for sorted ll)
...
STATIC int sw_state_ll_create_new_node(int unit, uint32 module_id, sw_state_ll_t ll, uint32 ll_index, sw_state_ll_node_t *created_node, 
    ...
    /* check if there is unused node available and take the first free node*/
    *created_node = SW_STATE_LL_GET_HEAD(0, is_service_node);
    ...
    /*next_tmp is the new head of the free nodes lists (service_node if it's a service node, otherwise - node.*/
    next_tmp = SW_STATE_LL_GET_NEXT(*created_node);

    if(is_service_node && next_tmp == SW_STATE_LL_INVALID){
        next_tmp = next_tmp + ll->max_elements;
    }
    /* update head of free node */
    SW_STATE_LL_SET_HEAD(0, next_tmp);

    /*Deleting the pointers to next & prev & down of the created node -> the node that was taken from free nodes list and will be use for the new node.*/
    if ((*created_node) != SW_STATE_LL_INVALID) {
    ...
    /* set the value if it's not a service node */
    if(!is_service_node) {
        ...
        /* set the key if sorted list and not a service node */
        if (key != NULL) {
    ...
    /* set key pointer in a service node it's sorted list and not a service node */
    else {
...
/**
* \brief
*  The function is an internal function that will find the nodes
*  before the node in each level
* \param [in] unit -       Device Id
* \param [in] module_id -  Module Id.
* \param [in] ll -         linked list to operate on
* \param [in] key -        the content of the new node's key
* \param [out] update -    pointer to the nodes that founded before the node
* \remark
*   * None
* \see
*   * None
...
/**
* \brief
*  The function is an internal function that returning randomly a level between
*  1 to (current level of the list + 1), the function check that there is free nodes in the service_nodes list.
* \param [in] ll -         linked list to operate on
* \param [out] level -     pointer to returned level
* \remark
*   * None
* \see
*   * None
...
static void sw_state_ll_rand_level(sw_state_ll_t ll, int* level) {
    ...
    while (sal_rand() < SAL_RAND_MAX / SW_STATE_SKIP_LIST_LEVEL_PROBABILITY && counter <= ll->curr_level && counter < ll->max_level &&
            /*check that there is free nodes in the service_nodes list*/head != SW_STATE_LL_INVALID){
        counter++;
        head = SW_STATE_LL_GET_NEXT(head);
    }
    *level=counter;
...
STATIC int sw_state_ll_add_internal(int unit, uint32 module_id, sw_state_ll_t ll, uint32 ll_index, sw_state_ll_node_t node_to_add_after, uint8 is_first, const void* key, const void* value) {
    ...
    if (ll->is_sorted) {
        ...
        /* check that service free nodes list not empty*/
        sw_state_ll_rand_level(ll, &level);
        /* update the level of the linked list  */
        if (level > ll->curr_level) {
            ll -> curr_level = level;
        }

        /* creates new instance of the node in each level (according to the random level that got - level) .    */
    ...
    /* if didn't get a node to add after, add at the beginning of list */
...
int sw_state_ll_create_empty(int unit, uint32 module_id, sw_state_ll_init_info_t *init_info, uint32 flags, uint32 key_size,
    ...
    /* allocate extra space for higher levels of skipped linked list and 1st node in lower level*/
    ...
    /* if single head linked list - override nof_heads with 1 */
    ...
    /* allocate values, keys, prevs, nexts, service_downs head and tail arrays */
    ...
    /* initialize the list/s of free cells */
    node = 1;
    SW_STATE_LL_SET_HEAD(0, node);
    tmp_node = elements_to_alloc-1;
    SW_STATE_LL_SET_TAIL(0, tmp_node);
    /*If it's a sorted linked list, two heads needed. One for the regular nodes (that are in the lower level) and one for
     * the service nodes (that are in the higher levels)*/
    if (sw_state_is_flag_on(flags, SW_STATE_LL_SORTED)){
    ...
    if(SOC_IS_DONE_INIT(unit)) {
        /*initializing the free nodes list*/
        ...
        /*If it's sorted ll, initializing the free service nodes list*/
        if (sw_state_is_flag_on(flags, SW_STATE_LL_SORTED)){
    ...
    if (ll->is_sorted) {
        ...
        ll->curr_level = 1;
        /*
         * Initialize first node in each level to have key & value = sw_state_ll_head_defult_val.
         * (first node is the head, last head node, down is null)
         */
...
static int sw_state_ll_sorted_ll_remove_node(int unit, uint32 module_id, sw_state_ll_t ll, uint32 ll_index, sw_state_ll_node_t node) {
    ...
    /* Verifying that the removed node exist */
    ...
    /* if the removed node is the tail node */
...
int sw_state_ll_destroy(int unit, uint32 module_id, sw_state_ll_t *ll_ptr) {
     ...
     * all memory will get freed automatically at detach */
...
*  sorted skipped linked list, with all the levels
* \param [in] unit    - Device Id
* \param [in] ll  - linked list to operate on
* \return
*   int - Error Type
...
/**
* \brief
*  The function will print to the screen the list of the free service nodes in the sorted skipped linked list
* \param [in] unit    - Device Id
* \param [in] ll  - linked list to operate on
* \return
*   int - Error Type
...
int sw_state_ll_print(int unit, sw_state_ll_t ll) {
    ...
    for (ll_index = 1; ll_index < ll->nof_heads+1; ll_index++) {
        ...
        /*
         * We have one element to store the end of the LL
         */
        nof_usable_elements = ll->max_elements - 1;
        if(ll->is_sorted)
        {
            /* one extra node for higher levels of skipped linked list*/
            nof_usable_elements--;
        }
        ...
        /* for sorted linked list should print only the lower layer*/
        if (ll->is_sorted) {
