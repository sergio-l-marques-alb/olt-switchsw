 ...
 ******************/
/*
 * The following 3 macros are copied, as is, from phymod_definitions.h
 * The only difference is that, in case of error, macros either return an 'int'
 * or 'shr_error_e' rather than 'phymod_error_t'. This makes them suitable for
 * using by DNX code.
 * {
 */
    ...
    portmod_pm_create_info_t * pm_info)
{
    ...
    {
#ifdef BCM_DNXF_SUPPORT
        soc_dnxc_init_serdes_ref_clock_t ref_clk = (soc_dnxc_init_serdes_ref_clock_t) phymodRefClkCount;
    ...
    if (SOC_IS_DNX(unit))
    {
        ...
        if (DNX_ALGO_PORT_TYPE_IS_NIF(unit, port_info, DNX_ALGO_PORT_TYPE_INCLUDE(ELK, STIF, L1, FLEXE, MGMT)))
        {
            ...
            *lane_rate =
                ...
                ? resource->speed : (resource->speed / num_lane);
        }
        else
        {
            /*
 ...
 *      unit       - (IN)   Unit number.
 *      port       - (IN)   Port number
 *      is_pam4    - (IN)   Is pam4 speed
 *      tx         - (OUT)  Default TX taps
 * Returns:
 *      _SHR_E_xxx
 * Notes:
 */
    ...
    uint32 phy_data)
{
    ...
    else
    {
        if (flags & BCM_PORT_PHY_NOMAP)
        {
            ...
            rv = soc_cmicx_miim_operation(unit, 1 /* is_write */ , 45,
                                          phy_id, phy_reg, &phy_wr_data);
            SHR_IF_ERR_EXIT(rv);
        }
        else
        {
    ...
    int enable)
{
    ...
    /*
     * Executed only for dynamic procedures
     */
    ...
    int enable)
{
     ...
     * Executed only for dynamic procedures and only if feature is valid
...
/**
 * \brief
 *   Common function used to enable/disable the port inside portmod (FMAC and PHY)
 * \param [in] unit -
 *   The unit id.
 * \param [in] port -
 *   The PM core id.
 ...
 *   See \ref shr_error_e.
 * \remark
 *   Note that this is a generic way to put in/out of reset FMAC and PHY
 *   but per device there might be other registers and procedures which should be called
 *   which should be handled in soc and imb layer for example.
 * \see
 *   None.
 */
shr_error_e
    ...
    int *fmac_inner_link)
{
    ...
    {
#ifdef BCM_DNXF_SUPPORT
        SHR_IF_ERR_EXIT(soc_dnxf_drv_link_to_block_mapping(unit, link, fmac_index, fmac_inner_link, SOC_BLK_FMAC));
    ...
    uint32 nof_tiks)
{
    ...
    SHR_IF_ERR_EXIT(soc_dnxc_port_fmac_index_get(unit, link, &fmac_index, &fmac_inner_link));
    ...
    int *nof_pms)
{
    ...
    if (SOC_IS_DNX(unit))
    {
        ...
        /** added to overcome compiler error the exit label was not used in DNX only compilation */
        SHR_EXIT();
#endif
    }
    else
    ...
    int arg)
{
    ...
    /** setting pm as a master MUST be done when cable swap is disabled */
    if (type == bcmFabricLinkCableSwapMasterMode)
    {
        /** calculate first PM and the amount of PM to check */
        SHR_IF_ERR_EXIT(soc_dnxc_port_link_pms_get(unit, link, &pm_id, &nof_pms));
        /** iterate over all/specific PM */
    ...
    if (SOC_IS_DNX(unit))
    {
        ...
        /** calculate first link and the amount of links to check */
        if (link != -1)
        {
            /** iterate over specific link */
        ...
        /** iterate over the port in same PM/device, based on link value] */
    ...
    else
    {
        ...
        /** calculate which links or PMs are relevant for the configuration */
        if (link != -1)
        {
            SHR_IF_ERR_EXIT(soc_dnxc_port_link_pms_get(unit, link, &pm_id, &nof_pms));
        ...
        {
            /** all active links are relevant */
            SOC_PBMP_ASSIGN(links, PBMP_SFI_ALL(unit));
        }

        /** check if links are disabled before configuring them */
    ...
    int arg)
{
    ...
    SHR_FUNC_INIT_VARS(unit);
    /** Verify that all links on same PM are disabled */
    ...
    int *is_control_type_valid)
{
    ...
    /**
     * Go over all supported FEC type and FEC control type configurations for the device. See if the requested control is
     * supported for the FEC type and if at all available for the device
     */
    if (SOC_IS_DNX(unit))
    {
#ifdef BCM_DNX_SUPPORT
...
/*
 * \brief - Verify if parameters (port speed, pad size etc) are correct for FMAC TX Padding
 *
 * \param [in] unit -     Unit number
 * \param [in] port - fabric logical port
 * \param [in] pad_size - Pad size in bytes
 * \param [in] is_set - Whether checking the get or set API
 * \return
 *      shr_error_e
 * \remark
 *   * None
    ...
    int is_set)
{
    ...
    /*
     * FMAC Tx padding is only relevant in port speeds of 100G or more
     */
