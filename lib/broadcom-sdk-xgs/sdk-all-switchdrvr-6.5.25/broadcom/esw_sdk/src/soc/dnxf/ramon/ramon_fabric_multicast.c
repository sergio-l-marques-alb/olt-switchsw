    ...
    int unit)
{
    ...
    if (!SAL_BOOT_PLISIM)
    {
        ...
        /*
         * Minimum number of links for group reachability. Configuration per FAP/group. Relevant only for FE13. For MC
         * cell with MCID for which local route is enabled: Only if nof links from FE13 towards each one of the
         * FAPs/groups within this MCID is >= this configured value, the MC cell will actually go via local route.
         * Otherwise it will go towards all-reachable 
         */
    ...
    soc_dnxf_fabric_priority_t prio)
{
    ...
    /*
     * Low priority multicast priority mapping. bit 4 - mapping of priority 00 - 1 for low priority, 0 for normal
     * priority. bit 5 - mapping of priority 01 - 1 for low priority, 0 for normal priority. bit 6 - mapping of
     * priority 10 - 1 for low priority, 0 for normal priority. bit 7 - mapping of priority 11 - 1 for low priority, 0
     * for normal priority
     */
    switch (prio)
    {
    ...
    int fap_modid_offset)
{
    ...
    /*
     * map FAP ID to local modid in FLGM. The FAP IDs are between [offset, offset + 192] and local modid is between [0, 
     * 192] 
     */
