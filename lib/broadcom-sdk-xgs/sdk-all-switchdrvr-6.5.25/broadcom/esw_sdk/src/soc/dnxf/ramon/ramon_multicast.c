    ...
    soc_multicast_t group)
{
    ...
    SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL
                    (unit, mbcm_dnxf_fabric_multicast_multi_set, (unit, 0, 1, &group, &dest_array, NULL)));
 ...
 *      port_array     - (OUT) Array of FAP-IDs/LRCs
 *      port_count     - (OUT) Actual number of FAP-IDs/LRCs returned in the array
 * Returns:
 *      _SHR_E_xxx
 * Notes:
 */
 ...
 *      unit           - (IN)  Unit number.
 *      group          - (IN)  Multicast group ID 
 *      port_count     - (IN) Number of FAP-IDs/LRCs in the array
 *      port_array     - (IN) Array of FAP-IDs/LRCs
 * Returns:
 *      _SHR_E_xxx
 * Notes:
 */
