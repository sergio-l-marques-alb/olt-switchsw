    ...
    const dnxc_vsc256_sr_cell_t * cell)
{
    ...
    /*
     * Cell size is (register field value + 1)
     */
    ...
    uint32 *filter_type_val)
{
    ...
    {   /* soc_dnxc_fe2_filtered_cell - do nothing */
    }
    ...
    soc_dnxc_fabric_cell_parse_table_t * parse_table)
{
    ...
    else
    {
        ...
        if (cell_size_bytes <= SOC_RAMON_FABRIC_CAPTURED_CELL_SMALL_CELL_MAX_SIZE)
        {
            ...
            {
                /**
                * If data is in MSB than it should be in (1024:2047) not in (0:2047)
                */
                parse_table[line].src_start_bit += SOC_RAMON_FABRIC_CAPTURED_CELL_DATA_IN_MSB_OFFSET;
            }
            else
            {
                /**
                * If data is not in MSB than it should be in (0:1023) not in (0:2047)
                * The destination needs to be shifted 1024 bits so that the payload would still align correctly.
                * The actual payload is always 256B (64 words) and the the payload is read MSB first.
                * This is why when receiving a small cell it needs to occupy the MSB section of the payload.
                */
 ...
 * Purpose:
 *      Send dest routed cell from the DCH block
 * Parameters:
 *      unit            - (IN)  Unit number.
    ...
    dnxc_dest_routed_cell_t * cell)
{
    ...
    /**
     * Calculate which DCH block holds the links towards the destination
     */
    ...
    for (reg_arr_index = 0; reg_arr_index <= max_reg_arr_index; reg_arr_index++)
    {
        ...
        /**
         * Data is written in big-endian format and array index 0 is the last of the data payload
         */
