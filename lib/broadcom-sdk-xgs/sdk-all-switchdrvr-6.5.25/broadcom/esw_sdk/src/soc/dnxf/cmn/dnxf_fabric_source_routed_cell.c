    ...
    uint32 *data_in)
{
    ...
    sal_memcpy(cell.payload.data, data_in,
               data_in_size <
               DNXC_VSC256_SR_DATA_CELL_PAYLOAD_MAX_LENGTH_U32 ? WORDS2BYTES(data_in_size) :
               WORDS2BYTES(DNXC_VSC256_SR_DATA_CELL_PAYLOAD_MAX_LENGTH_U32));
    ...
    dnxc_sr_cell_link_list_t * sr_link_list)
{
    ...
    rc = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_fabric_link_connectivity_status_get,
                               (unit, route->hop_ids[0], &link_partner));
    SHR_IF_ERR_EXIT(rc);

    /**
     * If destination provided by the user do not rely on reachability information
     */
    if (route->dest_device_type != _SHR_FABRIC_DEVICE_TYPE_UNKNOWN)
    {

        if (route->dest_device_type < _SHR_FABRIC_DEVICE_TYPE_NOF)
        {
            if (route->dest_device_type == _SHR_FABRIC_DEVICE_TYPE_FIP)
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "Device type FIP cannot be a valid destination for source routed cell");
            }
            /**
            * Assume that is user provides type FAP he means FIP
            */
            if (route->dest_device_type == _SHR_FABRIC_DEVICE_TYPE_FAP)
            {
                sr_link_list->dest_entity_type = _SHR_FABRIC_DEVICE_TYPE_FOP;
            }
            else
            {
                sr_link_list->dest_entity_type = route->dest_device_type;
            }

        }
        else
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Invalid device type provided - %d", route->dest_device_type);
        }
        ...
        switch (dnxf_data_fabric.general.device_mode_get(unit))
        {
            ...
            case soc_dnxf_fabric_device_mode_multi_stage_fe2:
                sr_link_list->src_entity_type = _SHR_FABRIC_DEVICE_TYPE_FE2;
                break;
            case soc_dnxf_fabric_device_mode_multi_stage_fe13:
                if (route->dest_device_type == _SHR_FABRIC_DEVICE_TYPE_FOP
                    || route->dest_device_type == _SHR_FABRIC_DEVICE_TYPE_FAP)
                {
                    switch (route->number_of_hops)
                    {
                        case 1:
                            sr_link_list->src_entity_type = _SHR_FABRIC_DEVICE_TYPE_FE3;
                            break;
                        case 3:
                            sr_link_list->src_entity_type = _SHR_FABRIC_DEVICE_TYPE_FE1;
                            break;
                        default:
                            SHR_ERR_EXIT(_SHR_E_PARAM, "number_of_hops %d is out of limit", route->number_of_hops);
                    }
                }
                else if (route->dest_device_type == _SHR_FABRIC_DEVICE_TYPE_FE1)
                {
                    SHR_ERR_EXIT(_SHR_E_PARAM, "FE1 cannot be destination of FE13");
                }
                else if (route->dest_device_type == _SHR_FABRIC_DEVICE_TYPE_FE2)
                {
                    switch (route->number_of_hops)
                    {
                        case 1:
                ...
                else if (route->dest_device_type == _SHR_FABRIC_DEVICE_TYPE_FE3)
                {
                    switch (route->number_of_hops)
                    {
                        case 2:
    ...
    else
    {
        switch (dnxf_data_fabric.general.device_mode_get(unit))
        {
            case soc_dnxf_fabric_device_mode_single_stage_fe2:
            case soc_dnxf_fabric_device_mode_multi_stage_fe2:
                sr_link_list->src_entity_type = (_shr_fabric_device_type_t) bcmFabricDeviceTypeFE2;
                break;
            case soc_dnxf_fabric_device_mode_multi_stage_fe13:
                if (bcmFabricDeviceTypeFE2 == link_partner.device_type)
                {
                    sr_link_list->src_entity_type = (_shr_fabric_device_type_t) bcmFabricDeviceTypeFE1;
                }
                else
                {
                    sr_link_list->src_entity_type = (_shr_fabric_device_type_t) bcmFabricDeviceTypeFE3;
                }
                break;
            default:
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "failed to get source device");
        ...
        switch (route->number_of_hops)
        {
            case 1:
                if (bcmFabricDeviceTypeFE2 == link_partner.device_type)
                    sr_link_list->dest_entity_type = (_shr_fabric_device_type_t) bcmFabricDeviceTypeFE2;
                else if (bcmFabricDeviceTypeFAP == link_partner.device_type
                         || bcmFabricDeviceTypeFOP == link_partner.device_type)
                    sr_link_list->dest_entity_type = (_shr_fabric_device_type_t) bcmFabricDeviceTypeFOP;
                else if (bcmFabricDeviceTypeFE13 == link_partner.device_type
                         || bcmFabricDeviceTypeFE3 == link_partner.device_type)
                    sr_link_list->dest_entity_type = (_shr_fabric_device_type_t) bcmFabricDeviceTypeFE3;
                else
                {
    ...
    for (hop_index = 0; hop_index < route->number_of_hops; hop_index++)
    {
        ...
        {
            case 0:
                if (sr_link_list->src_entity_type == _SHR_FABRIC_DEVICE_TYPE_FE2)
                {
                    sr_link_list->path_links[SOC_DNXF_FABRIC_SRC_VSC256_V1_CELL_PATH_LINK_FE2_SWITCH_POSITION] =
                        (uint8) route->hop_ids[hop_index];
                }
                else if (sr_link_list->src_entity_type == _SHR_FABRIC_DEVICE_TYPE_FE1)
                {
                    sr_link_list->path_links[SOC_DNXF_FABRIC_SRC_VSC256_V1_CELL_PATH_LINK_FE1_SWITCH_POSITION] =
                        (uint8) route->hop_ids[hop_index];
                }
                else if (sr_link_list->src_entity_type == _SHR_FABRIC_DEVICE_TYPE_FE3)
                {
                    sr_link_list->path_links[SOC_DNXF_FABRIC_SRC_VSC256_V1_CELL_PATH_LINK_FE3_SWITCH_POSITION] =
                        (uint8) route->hop_ids[hop_index];
                }
                else
                {
            ...
            case 1:
                ...
                {
                    sr_link_list->path_links[SOC_DNXF_FABRIC_SRC_VSC256_V1_CELL_PATH_LINK_FIP_SWITCH_POSITION] =
                        (uint8) route->hop_ids[hop_index];
                }
                else
                {
    ...
    uint32 *data_out)
{
    ...
    int header_offset = 0, header_field_idx, value = 0;
    ...
    (*data_out_size) =
        data_out_max_size <
        DNXC_VSC256_SR_DATA_CELL_PAYLOAD_MAX_LENGTH_U32 ? data_out_max_size :
        DNXC_VSC256_SR_DATA_CELL_PAYLOAD_MAX_LENGTH_U32;
    ...
    if (flags == _SHR_FABRIC_CELL_RX_HEADER_PREPEND)
    {
        ...
        /*
         * Shift the pointer of the array so the data will start after the header.
         */
