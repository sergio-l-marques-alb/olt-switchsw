    ...
    bcm_module_t module)
{
    ...
    field_val =
        soc_reg_above_64_field32_get(unit, RTP_REACHABILITY_MESSAGE_GENERATOR_CONFIGURATIONr, reg_val_above_64,
                                     MAX_BASE_INDEXf);
    if ((module / dnxf_data_fabric.topology.all_reachable_fap_group_size_get(unit)) > field_val)        /* AlrcMaxBaseIndex 
                                                                                                         * to be
                                                                                                         * configured
                                                                                                         * is >
                                                                                                         * MaxBaseIndex 
    ...
    bcm_module_t module)
{
    ...
    SHR_IF_ERR_EXIT(READ_RTP_ALL_REACHABLE_AND_GROUP_REACHABLE_CFGr(unit, &reg_val32));
    field_val = soc_reg_field_get(unit, RTP_ALL_REACHABLE_AND_GROUP_REACHABLE_CFGr, reg_val32, ALRC_MAX_BASE_INDEXf);
    if (field_val > (module / dnxf_data_fabric.topology.all_reachable_fap_group_size_get(unit)))        /* MaxBaseIndex 
                                                                                                         * to be
                                                                                                         * configured
                                                                                                         * is <
                                                                                                         * AlrcMaxBaseIndex 
    ...
    int arg)
{
    ...
    for (i = 0; i < dnxf_data_fabric.topology.all_reachable_max_nof_ignore_faps_get(unit); i++) /* Loop over all
                                                                                                 * ignoring register */
    {
        ...
        if (valid_field == 1)   /* The register is holding a fap id which is ignored */
        {
            if (id_field == module)
            {
                if (arg == 0)   /* need to clear this register */
                {
                    ...
                    SHR_EXIT(); /* the way we implemented, each fap id will be ignored only once, so we can exit safely 
                                 * without worrying that an additional register will ignore this id */
                }
        ...
        else if (first_empty_cell == -1 && arg == 1)    /* Haven't found a place in the array to configure the module
                                                         * id */
        {
            first_empty_cell = i;
        }
