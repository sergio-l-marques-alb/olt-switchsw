 ...
 */
/**
 * override_nof_data_cells = generate <x+1>*51 data cells : 47 = 2*number_of_links_in_core - 1
 */
    ...
    int unit)
{
    ...
    /**
     * Disable CCells generation
     * Where data_cells_lvl = ctrl_cells_lvl = 0
     * 1 << 4 == 16
     */
    ...
    int unit)
{
    ...
    /**
     * Reconfigure the filters for the disabled cores and
     */
    ...
    uint32 flags)
{
    ...
    /**
     * Enable all disabled FSRDs for the test.
     * HW snake is supported with detached ports but still requires all FSRD blocks to be enabled during the test.
     * Because of that regardless of the device initial port setup enable all FSRD blocks.
     * They are disabled again soc_ramon_cell_snake_test_run
     */
    ...
    /*
     * go over all phy cores of the device and check that either all ports of the core are attached or all ports of the 
     * core are detached 
     */
...
exit:
    /**
     * When doing HW snake with MAC LB we do reset on all blocks including the MAC and the FSRD.
     * If we reset the serdes , we need to re-load the FW.
     */
    if (flags & SOC_DNXF_ENABLE_MAC_LOOPBACK)
    {
...
exit:
    ...
    /**
     * Disable back all FSRD blocks that were initially disabled
     */
 ...
 * Purpose:
 *      By disabling enough ports on the FE we might disable the whole FSRD block in order to save power.
 *      For the HW snake test we need access on all FSRD (FMAC) blcoks so we turn them on in the start of the test
 *      and we turn them off again in the end
 * Parameters:
 *      unit            - (IN)  Unit number.
 *      power_up        - (IN)  power up or down the disabled fsrd blocks .
 *
 * Returns:
 *      _SHR_E_xxx
 * Notes:
