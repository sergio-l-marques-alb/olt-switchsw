    ...
    int is_init_sequence)
{
    ...
    int lane2fmac_lane[DNXF_DATA_MAX_PORT_GENERAL_NOF_LINKS];
    int lane_swap_bound;
    ...
    for (i = 0; i < DNXF_DATA_MAX_PORT_GENERAL_NOF_LINKS; ++i)
    {
        ...
        lane2fmac_lane[i] = SOC_DNXF_LANE_MAP_DB_LANE_TO_SERDES_NOT_MAPPED;
    }
    ...
    for (lane = 0; lane < map_size; ++lane)
    {
        ...
        if ((serdes_rx == SOC_DNXF_LANE_MAP_DB_LANE_TO_SERDES_NOT_MAPPED
             && serdes_tx != SOC_DNXF_LANE_MAP_DB_LANE_TO_SERDES_NOT_MAPPED)
            || (serdes_rx != SOC_DNXF_LANE_MAP_DB_LANE_TO_SERDES_NOT_MAPPED
                && serdes_tx == SOC_DNXF_LANE_MAP_DB_LANE_TO_SERDES_NOT_MAPPED))
        {
        ...
        if ((serdes_rx >= map_size)
            || ((serdes_rx < 0) && (serdes_rx != SOC_DNXF_LANE_MAP_DB_LANE_TO_SERDES_NOT_MAPPED)))
        {
            SHR_ERR_EXIT(_SHR_E_CONFIG, "lane %d: invalid serdes rx id %d\n", lane, serdes_rx);
        }
        if ((serdes_tx >= map_size)
            || ((serdes_tx < 0) && (serdes_tx != SOC_DNXF_LANE_MAP_DB_LANE_TO_SERDES_NOT_MAPPED)))
        {
        ...
        if (serdes_rx != SOC_DNXF_LANE_MAP_DB_LANE_TO_SERDES_NOT_MAPPED)
        {
            ...
            if (serdes_tx_core != lane_core)
            {
                SHR_ERR_EXIT(_SHR_E_CONFIG,
                             "Swap is out of bounds: lane %d, serdes tx id %d. Swap is allowed only within quad bounds\n",
                             lane, serdes_tx);
            }
        ...
        /** RM1 uses simple 1-1 mapping */
        lane2fmac_lane[lane] = lane;
        ...
        if (!is_init_sequence)
        {
            ...
            SHR_IF_ERR_EXIT(dnxf_state.lane_map_db.serdes_map.lane2serdes.get(unit, lane, &prev_lane2serdes));
    ...
    for (i = 0; i < map_size; ++i)
    {
        ...
        SHR_IF_ERR_EXIT(dnxf_state.lane_map_db.serdes_map.lane2serdes.set(unit, i, &lane2serdes[i]));
        /** serdes to lane mapping - rx, tx */
        SHR_IF_ERR_EXIT(dnxf_state.lane_map_db.serdes_map.serdes2lane.set(unit, i, &serdes2lane[i]));
        /** lane to fmac_lane mapping */
        SHR_IF_ERR_EXIT(dnxf_state.lane_map_db.link_to_fmac_lane_map.set(unit, i, lane2fmac_lane[i]));
    }
    ...
    soc_dnxc_lane_map_db_map_t * lane2serdes)
{
    ...
    for (lane = 0; lane < map_size; ++lane)
    {
        SHR_IF_ERR_EXIT(dnxf_state.lane_map_db.serdes_map.lane2serdes.get(unit, lane, &lane2serdes[lane]));
    }
 ...
 */
shr_error_e
soc_ramon_lane_map_db_pm_map_get(
    int unit,
    int pm_index,
    int nof_ports,
    soc_dnxc_lane_map_db_map_t * lane2serdes)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnxf_state.lane_map_db.serdes_map.serdes2lane.get(unit, serdes, &serdes2lane_entry));
    ...
    for (lane = pm_lower_boundry; lane <= pm_upper_boundry; ++lane)
    {
        ...
        SHR_IF_ERR_EXIT(dnxf_state.lane_map_db.serdes_map.lane2serdes.get(unit, lane, &lane2serdes[lane_in_pm]));
 ...
 */
shr_error_e
soc_ramon_lane_map_db_is_lane_mapped_get(
    int unit,
    int lane,
    int *is_lane_mapped)
{
    ...
    SHR_IF_ERR_EXIT(dnxf_state.lane_map_db.serdes_map.lane2serdes.get(unit, lane, &lane2serdes_entry));
