...
/*
 * If possible configure HA required memory
 * and create HA stable file
 */
shr_error_e
soc_dnxf_ha_deinit(
    int unit)
 ...
 * Initialize SW state feature and create initial allocations
 * and set default values to SW State data
 * \param [in] unit - unit
 *
 * \return
 *   See shr_error_e
    ...
    int unit)
{
    ...
    /**
     * Set runtime warmboot supported check
     */
    dnxc_sw_state_is_warmboot_supported_set(unit, warmboot);
    sw_state_max_size = dnxf_data_dev_init.ha.sw_state_max_size_get(unit);

    /**
     * Initialize all HA and WB features like validation, logging and diagnostic
     * Also this is the step which will restore the SW State if under warmboot
     */
    ...
    {
        /**
         * Make the allocation and set the initial default values to all SW State values
         */
...
/**
 * \brief - Deinit allocated SW state resources
 * \param [in] unit - unit
 *
 * \return
 *   See shr_error_e
