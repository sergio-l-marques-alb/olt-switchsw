    ...
    int *divider)
{
    ...
    SHR_IF_ERR_EXIT(soc_reg_get(unit, FSRD_SYNC_E_SELECTr, fsrd_block, 0, &reg_val2));
    *divider = soc_reg64_field_get(unit, FSRD_SYNC_E_SELECTr, reg_val2, SYNC_E_CLK_DIV_PHASE_1f) + 1;   /* phase1 is
                                                                                                         * configured
                                                                                                         * as
                                                                                                         * (divider-1) */
 ...
 * Purpose:
 *      Get blocks enable or not
 * Parameters:
 *      unit          - (IN)     Unit number.
 *      block_info - (IN)     block information, enum.
 *      block        - (IN)     block index.
 *      enable      - (OUT)  the block enabled.
 * Comments:
    ...
    int is_init_sequence)
{
    ...
    SOC_PBMP_ITER(pbmp, port)
    {
        ...
        /*
         * Get lane-mapping relevant to all lanes in the pm of the port: lane -> {serdes rx, serdes tx}. Unmapped lanes 
         * defined by user will be filled with unused serdeses- so that all lanes are mapped, in order to configure
         * FMAC and phymod without holes 
         */
    ...
    SOC_PBMP_ITER(pbmp, port)
    {
        ...
        {
            /*
             * Get lane-mapping relevant to all lanes in the pm of the port: lane -> {serdes rx, serdes tx}. Unmapped
             * lanes defined by user will be filled with unused serdeses- so that all lanes are mapped, in order to
             * configure FMAC and phymod without holes 
             */
            pm_index = port / DNXC_PORT_FABRIC_LANES_PER_CORE;
            SHR_IF_ERR_EXIT(soc_ramon_lane_map_db_pm_map_get
                            (unit, pm_index, DNXC_PORT_FABRIC_LANES_PER_CORE, port_config.lane2serdes));
        ...
        /*
         * Update relevant bitmaps - add to enabled bitmaps, remove from disabled bitmaps. Only when port is dynamic-
         * bitmaps are changed by below code. For static port bitmaps remain the same.
         */
    ...
    int *is_first_link)
{
    ...
    for (i = fsrd_block * dnxf_data_device.blocks.nof_links_in_fsrd_get(unit); i < fsrd_block * dnxf_data_device.blocks.nof_links_in_fsrd_get(unit) + dnxf_data_device.blocks.nof_links_in_fsrd_get(unit); i++) /* create 
                                                                                                                                                                                                                 * a 
                                                                                                                                                                                                                 * mask 
                                                                                                                                                                                                                 * for 
                                                                                                                                                                                                                 * all 
    ...
    int unit)
{
    ...
    /*
     * Enable temporary SW state access because portmod code is not completely covered by the WB (aligned with DNX code)
     */
    ...
    if (!SOC_WARM_BOOT(unit))
    {
        ...
        SHR_IF_ERR_EXIT(soc_ramon_lane_map_db_map_set(unit, nof_links, lane2serdes, 1   /* no need to check if port is
                                                                                         * * in use in init sequence */ ));
    }
    ...
    portmod_loopback_mode_t loopback)
{
    ...
    {
        SHR_IF_ERR_EXIT(soc_dnxf_fabric_link_device_stage_get(unit, port, 0 /* tx */ , &link_mode));
    ...
    soc_pbmp_t disable_pbmp)
{
    ...
    if (!SOC_WARM_BOOT(unit))
    {
        /** must be after update disable blocks, update brdc last in chain */
    ...
    int enable)
{
    ...
    {
        /**
         * The FMAC FMAC registers bellow had to be configured when PHY TX is diabled.
         * After that disable all FMAC and PHY
         */
    ...
    /*
     * Always disable both MAC and PHY, TX and RX
     */
    ...
    uint32 *nof_tiks)
{
    ...
    *burst = soc_reg64_field32_get(unit, FMAC_TX_CELL_LIMITr, reg64_val, CELL_LIMIT_COUNTf) + 2 /* +2 for shaper
                                                                                                 * accuracy */ ;
    ...
    soc_pbmp_t port_bmp)
{
    ...
    /*
     * If at least 1 port is PAM4 wait more This is a time optimization to avoid more loops and still support the
     * ability to provide whatever port bitmap 
     */
