    ...
    int unit)
{
    ...
    /*
     * STAGE 1
     * Set all memories based on generic propertis
     */
    ...
    /*
     * STAGE 2
     * Update some memories per static dnx data properties
     * Some of the tables are dynamically updated depending 
     * on the device mode or because of other features like fallen link
     */
    ...
    int unit)
{
    ...
    for (quad = 0; quad < dnxf_data_port.general.nof_links_get(unit) / nof_links_in_fmac; quad++)
    {
        ...
        /*
         * Check if a given quad is also disabled by a soc_property If so add it to disabled links pbmp
         */
    ...
    {
        /*
         * If port is valid ADD to enabled pbmp and REMOVE from disabled pbmp
         */
        ...
        {
            /*
             * At this stage, link_to_fmac mapping is not yet set for DNXF1 devices.
             * so soc_dnxf_lane_map_db_link_to_fmac_lane_get cannot be called
             */
            fmac_lane = port;
        }

        /*
         * If port is invalid, unmapped or disabled REMOVE from enabled pbmp and ADD to disabled pbmp
         */
...
/**
 * \brief - Basic preliminary procedures needed for device init
 *
 * \param [in] unit - unit
 *
 * \return
...
#ifdef VEDOR_DNX
        /*
         * soc_dnxf_drv_soc_property_serdes_qrtt_read was moved to a later stage
         * There is a suspicion that this steps in needed before configuring SOC layer for Ramon1.
         * It was decided to leave it here for RM1, due to the high risk.
         */
#endif
    ...
    int unit)
{
    ...
    if (!SOC_WARM_BOOT(unit))
    {
        ...
        {
            /*
             * When reloading no actual reset is done in HW
             * Function used only to set the SW state as in cold boot
             */
...
/**
 * \brief -  This function main role is to restore the status to be identical to the status before bcm_dnxf_init.
 *
 * \param [in] unit - unit
 *
 * \return
...
/**
 * \brief -  Map the logical links to relevant HW blocks
 *
 * \param [in] unit - unit
 *
 * \return
...
/**
 * \brief -  Do complete reset and initial configurations on all relevant blocks
 *
 * \param [in] unit - unit
 *
 * \return
    ...
    int unit)
{
    ...
    /*
     * Put the device blocks in reset and set initial configurations
     */
    if (!SOC_WARM_BOOT(unit))
    {
...
/**
 * \brief -  Preliminary steps before SOC starts to be deinited
 *
 * \param [in] unit - unit
 *
 * \return
...
/*
 * \brief
 *  This function is meant to follow exactly the sequence in dnxf_soc_init_deinit_seq
 *  NOTE!
 *     Function is not used in the regular init/deinit but supposedly only for special access/interrupt cases
 *     Currently only can be called from soc_dnxf_init which is called only from soc_init and soc_reset_init which actually not part of attach/init
 *     Given that this handles only part of the init/deinit there are no guarantees about its stability and behavior
 */
int
soc_dnxf_init(
    int unit)
...
/*
 * \brief
 * This function main role is to resotore the status to be identical to the status after soc_dnxf_init_reset.
 * It is meant to follow exactly the sequence in dnxf_soc_init_deinit_seq on backward direction
 * NOTE!
 * Function is not used in the regular init/deinit but supposedly only for special access/interrupt cases
 * Currently only can be called from soc_dnxf_init which is called only from soc_init and soc_reset_init which actually not part of attach/init
 * Given that this handles only part of the init/deinit there are no guarantees about its stability and behavior
    ...
    *aligned_lane_id = lane_id;
        ...
        dnxf_data_device.general.feature_get(unit, dnxf_data_device_general_is_not_all_cores_active))
    {
        ...
        if (!is_core_zero_active)
        {
            ...
            /*
             * For the first half FMAC instances, subtract nof links in FMAC to align the lane
             * For the second half FMAC instances, add nof links in FMAC
             */
    ...
    char *valid)
{
    ...
    {
        /*
         * PCID creates an additional core file using different main function(check pcidappl.c)
         * This application uses soc_dnxf_info_config which uses this function but DNXF data is not inited
         */
    ...
    {
        /*
         * fmac_lanes and fsrd count is 1:1 for all DNXF devices
         */
    ...
    int unit)
{
    ...
    /*
     * De-init idirect memory access mutexes 
     */
    ...
    int unit)
{
    ...
    /** DMA abort should be first thing after interrupt disabling */
    ...
    /*
     * Init idirect memory access mutexes 
     */
    ...
    int *nof_cycles)
{
    ...
    /*
     *              core_clock_khz * 1000 [ticks]    nof_ms [seconds]
     * nof_cycles = ----------------------------- *  ----------------
     *              [seconds]                        1000
     */
...
/**
 * \brief - Common function to set the initial values of the RTP block
 *
 * \param [in] unit - unit
 *
 * \return
    ...
    int unit)
{
    ...
    /*
     * Calculate gpd_rmgr_global_in_clocks
     */
    dnxc_time.time_units = DNXC_TIME_UNIT_USEC;
    ...
    /*
     * Calculate gpd_protect_cnt_th_in_clocks
     */
    dnxc_time.time_units = DNXC_TIME_UNIT_USEC;
    ...
    /*
     * Calculate gpd_cnt_th_in_clocks
     */
...
/**
 * \brief
 * To return bitmap of links that are associated with that instance_id of the requested block type.
 *
 * \param [in] unit -
 *   The unit number.
 * \param [in] block_id -
 *   Block instance id (0-nof_block_instances).
 * \param [in] block_type
 *   input of type SOC_BLK_*.
 * \param [out] links
 *   bitmap of links that are associated with that instance_id of the requested block type.
 * \return
 * \remark
 * \see
 */
    ...
    soc_pbmp_t * links)
{
    ...
    {
        /**
         * During init we create the golbal SFI PBMP at a very early stage after checking which SerDeses are disabled
         * However we should use such functions only the ports were actually probed
         * Current implementations relies on Portmod WB DB to track that so we check if port is attached based on portmod function
         */
...
/**
 * \brief
 * To return bitmap of blocks which belong to a certain FE multistage - FE1 or FE3
 * Function takes into account what type of block is provided
 * RX         - DCH
 * MID,TX     - DCML
 * The assignment of a block to a stage is made on the bases of what type of traffic are they carrying
 * - from FAP(FE1 stage) or from FE2(FE3 stage)
 * If FE2 stage provided return all blocks
 * \param [in] unit -
 *   The unit number.
 * \param [in] block_type -
 *   instance blkock type of type SOC_BLK_*.
 * \param [in] block_stage
 *   soc_dnxf_fabric_device_stage_fe1 or soc_dnxf_fabric_device_stage_fe3.
 * \param [out] block_bmp
 *   bitmap of blocks belonging to this stage block_bmp.
 * \return
 * \remark
 * \see
 */
    ...
    soc_pbmp_t * block_bmp)
{
    ...
    {
        /*
         * Add only the blocks that are valid for the device
         */
...
/**
 * \brief
 * To return link stage(FE1,FE2 or FE3) based on if reffering to link RX or TX
 * traffic and to which DCH it is connected
 * The assignment of a link to a stage is made on the bases of what type of traffic are they carrying
 * - from FAP(FE1 stage) or from FE2(FE3 stage)
 * \param [in] unit -
 *   The unit number.
 * \param [in] port -
 *   port.
 * \param [in] is_rx
 *   indicates if interested in rx traffic or tx traffic on specified link
 * \param [out] link_device_stage
 *   soc_dnxf_fabric_device_stage_fe1, soc_dnxf_fabric_device_stage_fe2 or soc_dnxf_fabric_device_stage_fe3.
 * \return
 * \remark
 * \see
 */
...
/**
 * \brief
 * To return a fmac_lane according to dnx_data/calculations.
 *
 * The opposite function of soc_dnxf_drv_link_to_block_mapping
 *
 * \param [in] unit -
 *   The unit number.
 * \param [in] block_id
 *   index of the block
 * \param [in] inner_link
 *   offset inside that block 'block_id'
 * \param [in] block_type
 *   the block type to refer to.
 * \param [out] fmac_lane
 *   fmac lane according to dnx_data.
 * \return
 * \remark
 * \see
 */
...
/**
 * \brief
 * To return the access related parameters of a certain fmac_lane
 *
 * The opposite function of soc_dnxf_drv_block_to_fmac_lane_mapping
 *
 * \param [in] unit -
 *   The unit number.
 * \param [in] fmac_lane -
...
/**
 * \brief
 * To return bitmap of all active blocks of given types. 
 * Active block is each one that has at least 1 of its relevant ports attached.
 * \param [in] unit -
 *   The unit number.
 * \param [in] block_type -
 *   instance blkock type of type SOC_BLK_*.
 * \param [out] block_bmp
 *   bitmap of active blocks which has at least 1 port attached on them..
 * \return
 * \remark
 * \see
 */
