...
/* Delay before the retimer phase interpolator TX part (usec) */
#define SOC_RAMON_FABRIC_LINKS_RETIMER_DELAY_BEFORE_PI_TX_USEC  (5)
/* Delay after the retimer phase interpolator TX part (usec) */
    ...
    soc_dnxf_fabric_link_remote_pipe_mapping_t * pipe_map)
{
     ...
     */
     /*DCH*/ rv = READ_DCH_PIPES_MAPPING_ENr(unit, dch_instance, reg32_val);
    SHR_IF_ERR_EXIT(rv);
     ...
     */
     /*DCH*/ rv = READ_DCH_TWO_PIPES_BMPr(unit, dch_instance, &reg64_val);
    SHR_IF_ERR_EXIT(rv);
    ...
    if (pipe_map->num_of_remote_pipes >= 1)
    {
         /*DCH*/ rv = READ_DCH_REMOTE_PRI_PIPE_IDXr(unit, dch_instance, reg_above_64_val);
        SHR_IF_ERR_EXIT(rv);
    ...
    if (pipe_map->num_of_remote_pipes >= 2)
    {
         /*DCH*/ rv = READ_DCH_REMOTE_SEC_PIPE_IDXr(unit, dch_instance, reg_above_64_val);
        SHR_IF_ERR_EXIT(rv);
    ...
    soc_port_t port)
{
    ...
    SHR_IF_ERR_EXIT(soc_dnxf_fabric_link_device_stage_get(unit, port, 1 /* rx */ , &device_mode));
    ...
    SOC_PBMP_ITER(pbmp, port_index)
    {
         ...
         */
        aldwp = (3 * max_cell_size * dnxf_data_device.general.core_clock_khz_get(unit)) / (speed / 8    /* bits to *
                                                                                                         * bytes */ );
        /*
         * mili ticks 
         */
    ...
    soc_port_t link_b)
{
    ...
    min_nof_polls = 1;
    /*
     * Set polling timeout to 6 seconds 
     */
 ...
 * Purpose:
 *      poll on Rx FEC lock in 2 steps:
 *      1. consequent polls with no sleeping between them
 *      2. polls with exponential backoff sleep periods between them
 * Parameters:
 *      unit            - (IN) The unit number
 ...
 *      min_nof_polls   - (IN) Nof consequent polls with no sleeping between them (step 1)
 *      usec_timeout    - (IN) The maximum total sleep time between the polls (step 2)
 *      rx_locked       - (OUT) Is Rx FEC locked
 * Returns:
 *      _SHR_E_xxx
 */
static shr_error_e
    ...
    soc_port_t link_b)
{
    ...
    if (link_b != -1)
    {
        ...
        /** Configure phase interpolator RX */
        ...
        /** Configure phase interpolator TX */
