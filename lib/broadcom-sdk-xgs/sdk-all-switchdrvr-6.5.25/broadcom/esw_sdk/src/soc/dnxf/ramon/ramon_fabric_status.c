    ...
    uint32 *errored_token_count)
{
    ...
    /*
     * link status DNXC_FABRIC_LINK_STATUS_CRC_ERROR Non-zero CRC rate DNXC_FABRIC_LINK_STATUS_SIZE_ERROR Non-zero
     * size error-count DNXC_FABRIC_LINK_STATUS_CODE_GROUP_ERROR Non-zero code group error-count
     * DNXC_FABRIC_LINK_STATUS_MISALIGN Link down, misalignment error DNXC_FABRIC_LINK_STATUS_NO_SIG_LOCK Link down,
     * SerDes signal lock error DNXC_FABRIC_LINK_STATUS_NO_SIG_ACCEP Link up, but not accepting reachability cells
     * DNXC_FABRIC_LINK_STATUS_ERRORED_TOKENS Low value, indicates bad link connectivity or link down, based on
     * reachability cells 
     */
    ...
    int *links_count)
{
    ...
    *links_count = 0;
    ...
    /*
     * coverity explanation: BCM_PBMP_ITER(PBMP_SFI_ALL(unit), i) iteration index 
