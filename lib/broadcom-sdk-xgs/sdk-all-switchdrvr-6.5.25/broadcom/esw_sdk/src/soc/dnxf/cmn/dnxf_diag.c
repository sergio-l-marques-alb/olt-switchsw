    ...
    int *temperature_count)
{
    ...
    {
        /*
         * PVTMON temperature readings has some rare outliers value in 16 nm and 7 nm devices.
         * The outliers aren't real temp indication but a false reading, the below algrith to remove such outliers:
         * [*X=5 reads*] and  [*Y=5 oC*].
         * (1) Execute X read, check that all reads are on the same range (Y).
         *    (1a) If yes - return one of the read values (or the average of all read values). Then will exit the thread current iteration.
         *    (1b) If no  - repeat (a), still in the same thread invocation.
         * (2) put timeout to avoid endless loop of 1.b when actual range is always > Y.
         */
 ...
 * Purpose:
 *      Parse relevant register in order to obtain connectivity info
 * Parameters:
 *      unit                - (IN)  Unit number.
 *      reg_val             - (IN)  Value of input register
 *      link_partner        - (OUT) Link partner holding connectivity info
 * Returns:
 *      _SHR_E_xxx
 */
shr_error_e
    ...
    bcm_fabric_link_connectivity_t * link_partner)
{
    ...
    if (reg_val & (0x1 << CCH_CONNECTIVITY_LINK_BIT_VALID))     /* bit 22 is valid indication */
    {
        link_partner->module_id = reg_val & CCH_CONNECTIVITY_LINK_SOURCE_ID_MASK;

        /*
         * COVERITY
         * utilex_bitstream_get_field does not modify data,
         * so no adjacent memory locations will be corrupted.
         * The size of the field called is much lower than the size of uint32, so no risk 
         * of reading memory outside of &reg_val.
         */
        ...
        /*
         * 3'b110:fe1 3'bX11-fe2 3'b010-fe3 3'bX0x-fap
         */
