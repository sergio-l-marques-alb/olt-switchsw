        ...
        if (db->num_algo_levels_in_db > 2)
        {
            ...
            else
            {
                ...
                start_brick_pos = kaps_array_get_first_n_bits_with_value(db->hw_res.db_res->small_bb_brick_bitmap, 1,
                                                     numBricksNeeded, 0,
                                                     db->hw_res.db_res->small_bb_brick_bitmap_num_bytes * KAPS_BITS_IN_BYTE);

                if (start_brick_pos == db->hw_res.db_res->small_bb_brick_bitmap_num_bytes * KAPS_BITS_IN_BYTE)
                {
                    *o_reason = NLMRSC_DBA_ALLOC_FAILED;
                    return NLMERR_FAIL;
                }
                ...
                for (j = 0; j < numBricksNeeded; ++j)
                {
                    ...
                    numEntriesPerBrick = kaps_lsn_mc_calc_max_triggers_in_brick(trie->m_lsn_settings_p, 
                                                        cur_brick_gran);
    ...
    for (i = 0; i < maxNumPools; ++i)
    {
        ...
        if (db->num_algo_levels_in_db == 3)
        {
            for (brickIter = 0; brickIter < device->kaps_shadow->ab_to_small_bb[ab->ab_num].num_bricks; ++brickIter)
            {
                ...
                {
                    uint32_t small_bb_nr = device->kaps_shadow->ab_to_small_bb[ab->ab_num].sub_ab_bricks[brickIter].small_bb_num;
                    uint32_t row_nr = device->kaps_shadow->ab_to_small_bb[ab->ab_num].sub_ab_bricks[brickIter].row_num;

                    uint8_t *bktData = device->kaps_shadow->small_bbs[small_bb_nr].bkt_rows[row_nr].data;
                    uint32_t i;

                    for (i = 0; i < KAPS_BKT_WIDTH_8; ++i) 
                    {
                        if (bktData[i] != 0) {
                            kaps_assert(0, "Bkt Data is not empty while releasing small BB\n");
                            bktData[i] = 0;
                        }
                    }
