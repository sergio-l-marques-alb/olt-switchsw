    ...
    uint32_t it_addr)
{
    ...
    KapsWriteBitsInArray(write_buf, num_bytes, 113, 104, write_ads->bkt_row);
...
kaps_algo_hw_write_rpt_data(
    ...
    uint16_t entry_nr)
{
    KAPS_STRY(kaps_dm_kaps_rpb_write(device, rpt_num, entry_nr, data, mask, 3));
    return KAPS_OK;
    ...
    uint16_t entry_nr)
{
    ...
    kaps_memset(invalid_data, 0x0, KAPS_RPB_WIDTH_8);
    KAPS_STRY(kaps_dm_kaps_rpb_write(device, rpt_num, entry_nr, invalid_data, invalid_data, 0));
