    ...
    kaps_nlm_allocator *alloc)
{
    if (self->m_isAdditionalIxPresent) {
        ...
        tmp_ptr = KAPS_PFX_BUNDLE_GET_ASSOC_PTR(self);
        kaps_memcpy(&cur_ix_chunk, tmp_ptr, sizeof(struct kaps_ix_chunk*));

        ix_mgr = kaps_lsn_mc_get_ix_mgr_for_lmpsofar_pfx(
                                cur_ix_chunk->ad_info->ad_db);
        
        kaps_ix_mgr_free(ix_mgr, cur_ix_chunk);
    }
