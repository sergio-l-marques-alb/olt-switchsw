    ...
    NlmReasonCode * o_reason)
{
    ...
    *o_reason = NLMRSC_REASON_OK;
    ...
    kaps_trie_get_resource_usage(fibTbl, rxcUsage, log_info);
    ...
    struct NlmFibStats *fib_stats)
{
    ...
    if (status != KAPS_OK)
        kaps_assert(0, "kaps_db_stats returned error \n");
    ...
    struct kaps_db *db)
{
    ...
    kaps_fprintf(f, "<h2>RPB %d Capacity %d </h2>\n", db->rpb_id, stats.num_entries);
    ...
    kaps_fib_tbl * fibTbl)
{
    ...
    kaps_trie_calc_trie_lsn_stats(fibTbl->m_trie->m_roots_trienode_p, 0, fibStats);
    ...
    if (fibTbl->m_db->num_algo_levels_in_db == 3) 
    {
        ...
        ads2_id = fibTbl->m_db->rpb_id;

        fibStats->numFreeAds2Slots = 
            device->kaps_shadow->ads2_overlay[ads2_id].num_free_it_slots;

        fibStats->numUsedAds2Slots = 
                device->kaps_shadow->ads2_overlay[ads2_id].it_fbmp.num_access_bits 
                - fibStats->numFreeAds2Slots;
