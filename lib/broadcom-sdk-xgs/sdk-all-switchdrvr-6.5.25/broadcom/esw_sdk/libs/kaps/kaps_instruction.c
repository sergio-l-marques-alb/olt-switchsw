    ...
    struct kaps_complete_search_result * result)
{
    ...
    if (instruction->device->type == KAPS_DEVICE_KAPS)
    {
        ...
        for (j = 0; j < instruction->device->max_num_searches; j++)
        {
            ...
            is_ad_all_zero = 1;
            for (i = 0; i < KAPS_MAX_AD_WIDTH_8; ++i)
            {
            ...
            kaps_memcpy(result->assoc_data[j], kaps_result.ad_value, KAPS_MAX_AD_WIDTH_8);
        }
    ...
    struct kaps_wb_cb_functions * wb_fun)
{
    ...
    struct kaps_instruction_wb wb_info = {0};
    struct kaps_instruction_wb *wb_info_ptr;
    int32_t i;
