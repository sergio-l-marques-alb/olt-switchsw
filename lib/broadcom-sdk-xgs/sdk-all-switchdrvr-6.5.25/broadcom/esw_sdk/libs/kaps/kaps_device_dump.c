    ...
    struct kaps_entry *e)
{
    ...
    if (db->device->flags & KAPS_DEVICE_ISSU)
        status = kaps_entry_get_info(db, KAPS_WB_CONVERT_TO_ENTRY_HANDLE(kaps_entry, e->user_handle), &info);
    else
        status = kaps_entry_get_info(db, e, &info);

    if (status != KAPS_OK)
        kaps_assert(0, "kaps_entry_get_info returned an error \n");
    ...
    FILE * fp)
{
    ...
    kaps_fprintf(fp, "KAPS ID = %d, subtype = %d ",
