...
uint32_t
kaps_seq_num_gen_get_current_pfx_seq_nr(
    void)
{
    return g_pfxSeqNr;
}
...
void
kaps_seq_num_gen_set_current_pfx_seq_nr(
    struct kaps_pfx_bundle *newPfx_p)
{

#ifdef KAPS_LPM_DEBUG
    newPfx_p->m_nSeqNum = kaps_seq_num_gen_get_current_pfx_seq_nr();
#endif
    return;
}
