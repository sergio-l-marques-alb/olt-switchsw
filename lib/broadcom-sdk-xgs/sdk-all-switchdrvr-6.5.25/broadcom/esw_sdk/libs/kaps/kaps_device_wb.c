...
struct kaps_device_res_wb_info
{
    ...
    uint16_t num_dba_sb;
    uint16_t num_ab[KAPS_HW_MAX_NUM_DB_GROUPS];
    uint16_t num_uda;
    ...
    struct kaps_wb_cb_functions *wb_fun)
{
    ...
    *wb_fun->nv_offset = *wb_fun->nv_offset + sizeof(count);
    ...
    while ((e1 = kaps_c_list_iter_next(&it)) != NULL) {
        ...
        if (db->type != KAPS_DB_DMA)
            continue;
        KAPS_STRY(kaps_dma_db_wb_save_info(db, wb_fun));
    }
    ...
    struct kaps_wb_cb_functions *wb_fun)
{
    ...
    *wb_fun->nv_offset = *wb_fun->nv_offset + sizeof(num_db);
    ...
    for (db_no = 0; db_no < num_db; db_no++) {
        KAPS_STRY(kaps_dma_db_wb_restore_info(device, wb_fun));
    }
    ...
    struct kaps_wb_cb_functions * wb_fun)
{
    ...
    while (cascade != NULL)
    {
        ...
        *wb_fun->nv_offset = *wb_fun->nv_offset + sizeof(struct kaps_device_property);
        ...
        for (i = 0; i < KAPS_HW_MAX_NUM_DB_GROUPS; ++i)
        {
            res_ptr->num_ab[i] = cascade->num_ab[i];
        }
    ...
    KAPS_STRY(kaps_device_wb_save_dma(device, wb_fun));
    ...
    struct kaps_wb_cb_functions * wb_fun)
{
    ...
    while (cascade)
    {
        ...
        *wb_fun->nv_offset = *wb_fun->nv_offset + sizeof(*res_ptr);
        ...
        for (i = 0; i < KAPS_HW_MAX_NUM_DB_GROUPS; ++i)
        {
            cascade->num_ab[i] = res_ptr->num_ab[i];
        }
     ...
     */
    KAPS_STRY(kaps_device_wb_restore_dma(device, wb_fun)); 
