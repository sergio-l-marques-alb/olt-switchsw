...
uint32_t g_coherency_search_enable;
...
NlmErrNum_t kaps_trie_merge_ancestor_lsns(
    kaps_lpm_trie * trie_p,
    ...
    kaps_lpm_lpu_brick *curBrick)
{
    ...
    for (k = 0; k < curBrick->m_maxCapacity; k++) {
        kaps_pfx_bundle *pfxBundle = curBrick->m_pfxes[k];

        if (pfxBundle) 
        {
            if (cur_fp)
            {
                kaps_fprintf(cur_fp, "%d, ", pfxBundle->m_nPfxSize);
            }
            else
            {
                kaps_printf("%d, ", pfxBundle->m_nPfxSize);
            }
        } 
        else 
        {
    ...
    kaps_lsn_mc *curLsn)
{
    ...
    if (cur_fp)
    {
        kaps_fprintf(cur_fp, " LSN Id = %d, Depth = %d, Num Pfx = %d, Num Bricks = %d\n",
    ...
    {
        kaps_printf(" LSN Id = %d, Depth = %d, Num Pfx = %d, Num Bricks = %d\n",
            curLsn->m_lsnId, curLsn->m_nDepth, curLsn->m_nNumPrefixes,
            curLsn->m_numLpuBricks);
    }
    ...
    for (j = 0; j < curLsn->m_numLpuBricks; ++j) {
        ...
        kaps_trie_print_brick(cur_fp, curBrick);
        
        curBrick = curBrick->m_next_p;
    }
}

void kaps_trie_print_lsn_split(
    kaps_lpm_trie *trie_p, 
    kaps_lsn_mc *originalLsn)
{
    FILE *cur_fp;
    ...
    if (g_lsnSplitCnt <= 1) {
        cur_fp = kaps_fopen("LsnSplits.txt", "w");
    } else {
        cur_fp = kaps_fopen("LsnSplits.txt", "a");
    }

    kaps_fprintf(cur_fp, "LSN_SPLIT: \n");

    kaps_fprintf(cur_fp, "Submit Rqt = %d, TID = %d\n", g_submitRqtHitCnt,
                trie_p->m_tbl_ptr->m_db->tid);

    kaps_fprintf(cur_fp, "Splitting ");

    kaps_trie_print_lsn(cur_fp, originalLsn);
    ...
    for (i = 0; i < trie_p->m_num_items_in_completed_giveouts; ++i) {
        ...
        kaps_trie_print_lsn(cur_fp, curGiveoutData_p->lsn_cookie);

    }
    kaps_fprintf(cur_fp, "________________________________\n");
...
void kaps_trie_print_trie(
    FILE *cur_fp, 
    kaps_trie_node *cur_node)
{
    if (cur_node) {
        if (cur_node->m_node_type == NLMNSTRIENODE_LP) {
            kaps_trie_print_lsn(cur_fp, cur_node->m_lsn_p);
            kaps_fprintf(cur_fp, "________________________________________\n\n");
        }

        kaps_trie_print_trie(cur_fp, cur_node->m_child_p[0]);
        kaps_trie_print_trie(cur_fp, cur_node->m_child_p[1]);
             
    }
}

void kaps_trie_print_pfx(
    FILE *cur_fp,
    uint8_t *pfx_data,
    uint32_t pfx_len,
    char demarcater,
    ...
    uint8_t print_new_line)
{
    ...
    for (i = 0; i < num_pfx_bytes; ++i)
    {
        ...
        {
            if (cur_fp)
                kaps_fprintf(cur_fp, "%d", pfx_data[i]);
            else   
                kaps_printf("%d", pfx_data[i]);
        }
        else
        {
            if (cur_fp)
            {
                kaps_fprintf(cur_fp, "%c", demarcater);
                kaps_fprintf(cur_fp, "%d", pfx_data[i]);
            }
            else  
            {  
                kaps_printf("%c", demarcater);
                kaps_printf("%d", pfx_data[i]);
            }
        }
    }
    ...
    {
        if (cur_fp)
            kaps_fprintf(cur_fp, "/%d", pfx_len);
        else
            kaps_printf("/%d", pfx_len);
    }
    ...
    uint8_t use_demarcater)
{
    ...
    for (i = 0; i < ad_width_8; ++i) 
    {
        if (i == 0) 
        {
            if (fp)
            {
                kaps_fprintf(fp, "0x%02x", entry->ad_handle->value[i]);    
            }
            else
            {
                kaps_printf("0x%02x", entry->ad_handle->value[i]);
            }
        }
        else 
        {
            ...
            {
                if (fp)
                    kaps_fprintf(fp, "%02x", entry->ad_handle->value[i]);    
                else
                    kaps_printf("%02x", entry->ad_handle->value[i]);
...
void
kaps_trie_log_search_key_record(
    kaps_lpm_trie * trie_p,
    struct kaps_lpm_entry *entry)
{
    ...
    if (g_submitRqtHitCnt == 0)
        fp = kaps_fopen("search_key_log.txt", "w");
    else
        fp = kaps_fopen("search_key_log.txt", "a");
            
    kaps_fprintf(fp, "{");
        
        
    kaps_fprintf(fp, "%d", trie_p->m_tbl_ptr->m_db->rpb_id);
    ...
    kaps_trie_print_pfx(fp, entry->pfx_bundle->m_data, 
    ...
    kaps_trie_print_ad(fp, device, entry, 1);
...
void
kaps_trie_dump_pfx_log_records(
    struct kaps_device *device)
{
    uint32_t i, j;
    FILE *fp;
    
    fp = kaps_fopen("pfx_log.txt", "w");
    ...
    for (i = 0; i < device->cur_num_pfx_log_records; ++i)
    {
        ...
        kaps_fprintf(fp, "DB%d:", device->pfx_log_records[i].db_id);
        
        kaps_trie_print_pfx(fp, device->pfx_log_records[i].pfx_data,
                device->pfx_log_records[i].pfx_len, '.', 0, 0);
                
        kaps_fprintf(fp, "  ");
        
        for (j = 0; j < KAPS_MAX_AD_WIDTH_8; ++j)
        {
            kaps_fprintf(fp, "%02x", device->pfx_log_records[i].pfx_ad[j]);
        }
...
void
kaps_trie_store_pfx_log_records(
    kaps_lpm_trie *trie_p,
    ...
    struct kaps_lpm_entry *entry)
{
        ...
        && device->cur_num_pfx_log_records < device->max_num_pfx_log_records)
    {
        ...
        kaps_memcpy(device->pfx_log_records[i].pfx_data,
                    entry->pfx_bundle->m_data, 
                    num_bytes);
                    
        kaps_memcpy(device->pfx_log_records[i].pfx_ad,
        ...
        {
            kaps_trie_dump_pfx_log_records(device);
        }
    }
}
    ...
    FILE * fp)
{
    ...
    {
        kaps_printf("Submit Rqt = %d, \n", g_submitRqtHitCnt);
        kaps_printf("RPT Node Id = %d, depth = %d, Num Triggers = %d\n",
                     curRptNode->m_trienodeId, curRptNode->m_depth, curRptNode->m_numIptEntriesInSubtrie);        
    }
    ...
    if (db->num_algo_levels_in_db == 3)
    {
        ...
        for (i = 0; i < device->kaps_shadow->ab_to_small_bb[ab->ab_num].num_bricks; ++i)
        {
            ...
            if (fp)
            {
                kaps_fprintf(fp, "(Brick = %d, Small BB Num = %d, Row Num = %d, Gran = %d, Low index = %d, Upper index = %d, Num free slots = %d), \n", i,
                         device->kaps_shadow->ab_to_small_bb[ab->ab_num].sub_ab_bricks[i].small_bb_num,
                         device->kaps_shadow->ab_to_small_bb[ab->ab_num].sub_ab_bricks[i].row_num,
                         device->kaps_shadow->ab_to_small_bb[ab->ab_num].sub_ab_bricks[i].sub_ab_gran,
                         device->kaps_shadow->ab_to_small_bb[ab->ab_num].sub_ab_bricks[i].lower_index,
                         device->kaps_shadow->ab_to_small_bb[ab->ab_num].sub_ab_bricks[i].upper_index,
                         num_free_slots);
            }
            else
            {
                kaps_printf("(Brick = %d, Small BB Num = %d, Row Num = %d, Gran = %d, Low index = %d, Upper index = %d, Num free slots = %d), \n", i,
                         device->kaps_shadow->ab_to_small_bb[ab->ab_num].sub_ab_bricks[i].small_bb_num,
                         device->kaps_shadow->ab_to_small_bb[ab->ab_num].sub_ab_bricks[i].row_num,
                         device->kaps_shadow->ab_to_small_bb[ab->ab_num].sub_ab_bricks[i].sub_ab_gran,
                         device->kaps_shadow->ab_to_small_bb[ab->ab_num].sub_ab_bricks[i].lower_index,
    ...
    else
    {
        ...
        kaps_printf("Cur num DBA entries = %d, Max num DBA entries = %d\n",
                 ipmPoolInfo->m_curNumDbaEntries, ipmPoolInfo->m_maxNumDbaEntries);
    ...
    kaps_lpm_trie * trie_p)
{
    ...
    if (g_rptSplitCnt <= 1)
    {
        fp = kaps_fopen("RptSplit.txt", "w");
    }
    else
    {
        fp = kaps_fopen("RptSplit.txt", "a");
    }
    ...
    kaps_trie_node * curRptNodeToMove)
{
    ...
    if (g_rptMoveCnt <= 1)
    {
        fp = kaps_fopen("RptMove.txt", "w");
    }
    else
    {
        fp = kaps_fopen("RptMove.txt", "a");
    }
...
uint32_t kaps_trie_is_lsn_empty(
    kaps_lpm_trie *trie_p,
    kaps_trie_node *curNode)
{
    ...
    {
        /*If m_strictlyStoreLmpsofarInAds is 1, then the LSN should have 0 prefixes AND 
        also there should be no prefix whose length matches the LSN depth*/
    ...
    for (i = 0; i < numPfx; ++i)
    {
        ...
        for (cur_brick_nr = 0; cur_brick_nr < trie_p->m_iptBrickScratchpad.m_numActiveBricks; ++cur_brick_nr)
        {
                ...
                trie_p->m_iptBrickScratchpad.m_maxNumEntries[cur_brick_nr])
            {
                ...
                numEntriesPerBrick = kaps_lsn_mc_calc_max_triggers_in_brick(
                                            trie_p->m_lsn_settings_p,
                                            cur_trigger_gran);
    ...
    NlmReasonCode * o_reason)
{
    ...
    if (fibTblMgr_p->m_devMgr_p->issu_status == KAPS_ISSU_INIT)
    {
        ...
        errNum = kaps_rpm_add_entry(rpm_p, rptNode_p->m_rpt_prefix_p, dbId, o_reason);
    ...
    errNum = kaps_rpm_write_entry_to_hw(rpm_p, rptNode_p->m_rpt_prefix_p, dbId);
    ...
    kaps_trie_node * node_p)
{
    ...
    if (node_p)
    {
        ...
        {
            kaps_trie_pvt_calc_num_ipt_and_apt(node_p->m_child_p[0]);
        ...
        {
            kaps_trie_pvt_calc_num_ipt_and_apt(node_p->m_child_p[1]);
    ...
    NlmReasonCode * o_reason)
{
    ...
    isLsnEmpty = kaps_trie_is_lsn_empty(trie_p, ancestorIptNode);
    ...
    *isPfxMatchingNewRptNodePresentInAncestor = 0;
    ...
    while (curLpuBrick)
    {
        for (i = 0; i < curLpuBrick->m_maxCapacity; i++)
        {
            ...
            if (pfxBundle_p)
            {
                uint32_t isMoreSpecificOrEqual = kaps_prefix_pvt_is_more_specific_equal(&pfxBundle_p->m_data[byteOffset],
                ...
                if (trie_p->m_lsn_settings_p->m_strictlyStoreLmpsofarInAds)
                {
                    /*Let us say that ancestorIpt is 10/8 and it has the prefix 10.0/16. 
                    Now the newRptNode is also 10.0/16. In this case, if m_strictlyStoreLmpsofarInAds is 1, 
                    we will not mark 10.0/16 to be moved since 10.0/16 will not go into the LSN at depth 16.
                    Later when we process iitLmpsofar, we will move it to newRptNode_p->m_iitLmpsofarPfx_p*/
                    if (isMoreSpecificOrEqual 
                        && pfxBundle_p->m_nPfxSize == newRptNode_p->m_lp_prefix_p->m_nPfxSize)
                    {
                        doMove = 0;
    ...
    uint32_t *numPfxInArray)
{
    ...
    uint32_t indexOfLmpsofarInLsn = KAPS_LSN_NEW_INDEX, indexInLsn;
    struct kaps_ad_db *ad_db = NULL;
    struct kaps_device *device = trie_p->m_lsn_settings_p->m_device;
    ...
    /*Initialize lmpsofarPfxBundle to the lmpsofar of the ancestor IPT node*/
    lmpsofarPfxBundle = ancestorIptNode_p->m_iitLmpsofarPfx_p;

    /*Go through the ancestor LSN and find out if there is a deeper lmpsofar*/
    ancestorLpuBrick = ancestorIptLsn_p->m_lpuList;
    indexInLsn = 0; /*each slot whether hole or prefix contributes to 1 index*/
    while (ancestorLpuBrick)
    {
        for (i = 0; i < ancestorLpuBrick->m_maxCapacity; ++i)
        {
            ...
            if (trie_p->m_lsn_settings_p->m_strictlyStoreLmpsofarInAds)
            {
                /*If m_strictlyStoreLmpsofarInAds is 1, then the prefix whose depth is equal 
                to the new LSN will still be in the ancestor LSN. So use <= for comparison*/
            ...
            {
                /*The prefix whose depth is equal to the depth of the new LSN will be stored in
                the new LSN. The lmpsofar will be shorter than the newLsn. so use < for comparison*/
                if (ancestorPfxBundle->m_nPfxSize < newLsn_p->m_nDepth) 
                {
                    isPfxLenSuitable = 1;
                }
    ...
    if (trie_p->m_lsn_settings_p->m_strictlyStoreLmpsofarInAds
        ...
        && pfxLocationsArray)
    {
        /*If we found the prefix whose depth matches the new LSN, then add the index of the
        prefix to the prefixLocationsArray in SORTED ORDER. prefixLocationsArray will generally contain the 
        location of prefixes that need to be cleared after splitting the IPT ancestor*/
        i = *numPfxInArray;

        while (i > 0)
        {
            /*If the location in array is >= indexOfLmpsofar, shift the elements in the array to the right,
            if the location in array is < indexOfLmpsofar, break out of the loop*/
...
NlmErrNum_t
kaps_trie_split_ipt_ancestor_and_get_pfx_for_new_lsn(
    kaps_trie_node * newRptNode_p,
    kaps_lsn_mc * ancestorIptLsn_p,
    uint32_t pfxLocationsToMove_p[],
    uint16_t numPfxToMove,
        ...
        while (ancestorLpuBrick)
        {
            for (i = 0; i < ancestorLpuBrick->m_maxCapacity; i++)
            {
                if (indexInLsn == pfxLocationsToMove_p[j])
                {
                    ...
                    {
                        /*If m_strictlyStoreLmpsofarInAds is 1 and the prefix length is equal to the depth 
                        of the LSN, then we will not store this prefix in the LSN. We will instead store this prefix 
                        in the ADS. This prefix should not be present in pfxLocationsToMove_p*/
                        kaps_assert(0, "Exactly matching prefix should not be present in pfxLocationsToMove_p\n");


                        /*In release made if the assert is disabled, skip this prefix*/
                        ++j;
                        if (j >= numPfxToMove)
                            break;
    ...
    NlmReasonCode * o_reason)
{
    ...
    errNum = kaps_trie_split_ipt_ancestor_and_get_pfx_for_new_lsn(newRptNode_p, ancestorIptLsn_p,
    ...
    NlmReasonCode * o_reason)
{
    ...
    for (i = 0; i < device->hw_res->total_num_bb[db_group_id] / 2; ++i)
    {
        ...
        if (device->map->bb_map[db_group_id][bb_nr].row->owner &&
            (device->map->bb_map[db_group_id][bb_nr].row->owner == device->map->bb_map[db_group_id][bb_nr + 1].row->owner))
        {
    ...
    if (trie_p->m_lsn_settings_p->m_areIPTHitBitsPresent)
    {
        if (fibtbl->m_db->num_algo_levels_in_db == 2)
        {
            trie_p->m_iptHitBitBuffer =
                kaps_nlm_allocator_calloc(alloc_p, KAPS_RPB_MAX_BLOCK_SIZE, sizeof(uint8_t));
        }
        else
        {
            trie_p->m_iptHitBitBuffer =
                kaps_nlm_allocator_calloc(alloc_p,
                                          KAPS_HW_MAX_NUM_SMALL_BB_PER_DB_GROUP * KAPS_HW_MAX_NUM_ROWS_IN_SMALL_BB *
    ...
    NlmReasonCode * o_reason)
{
    ...
    {
        if (!trie_global->m_curRptNodeToPush_p->m_isRptNode)
            return NLMERR_OK;
        ...
        kaps_trie_pvt_calc_num_ipt_and_apt(trie_global->m_curRptNodeToPush_p);
         ...
         */
        if (newRptPushNode_p->m_iitLmpsofarPfx_p &&
            newRptPushNode_p->m_iitLmpsofarPfx_p->m_nPfxSize == newRptPushNode_p->m_depth)
        {
            /*If the new RPT node already has an iitLmpsofar whose length is equal to depth of the new node
            then don't copy the iitLmpsofar of the RPT node being pushed. So don't do anything. This will happen only if
            m_strictlyStoreLmpsofarInAds is 1. Note that we are not explicitly checking if m_strictlyStoreLmpsofarInAds is 1*/
        }
        else
        {
            /*Copy the iitLmpsofar of the RPT node being pushed into the new RPT node.*/
...
kaps_pfx_bundle *
kaps_trie_handle_exact_strict_lmpsofar_for_delete(
    kaps_lpm_trie *trie_p,
    kaps_trie_node *destnode,
    struct kaps_lpm_entry *entry,
    NlmReasonCode * o_reason)
{
    kaps_trie_node *iternode;
    ...
    /*m_strictlyStoreLmpsofarInAds is 1 and the prefix that we are trying to 
    delete exactly matches with the depth of the destnode. In this case we have to
    find the lmpsofar of the in the ancestor IPT of the destnode*/

    iternode = destnode->m_parent_p;

    while (iternode) 
    {
        /*Find the nearest IPT ancestor*/
        ...
        /*Find if lmpsofar is present in the LSN of the IPT ancestor*/
        lmpsofarPfxBundle_p = kaps_lsn_mc_locate_lpm(iternode->m_lsn_p, pfxData, pfxLen, NULL, NULL, NULL);
        ...
        {
            /*If lmpsofar doesn't exist in the LSN of the IPT ancestor, then assign
            m_iitLmpsofarPfx_p of the ancestor trie node to lmpsofarPfxBundle_p*/
    ...
    /*Update the iitLmpsofar of the destnode and also update the ADS*/
    destnode->m_iitLmpsofarPfx_p = lmpsofarPfxBundle_p;

    errNum = kaps_lsn_mc_update_iit(destnode->m_lsn_p, o_reason);

    if (errNum != NLMERR_OK)
        kaps_assert(0, "Update IIT failed when handling exact strict lmpsofar for delete \n");
    ...
    NlmReasonCode * o_reason)
{
    ...
    kaps_trie_global *trie_global = trie_p->m_trie_global;
    struct kaps_db *db = trie_global->fibtblmgr->m_curFibTbl->m_db;
    struct kaps_trie_node *ancestorIptNodeWithEntry;
    uint32_t list_size;
    ...
    else if (rqtCode == NLM_FIB_PREFIX_DELETE)
    {
        ...
        if (destnode->m_lsn_p->m_pSettings->m_strictlyStoreLmpsofarInAds 
            && destnode->m_depth == entry->pfx_bundle->m_nPfxSize)
        {
            lmpsofarPfxBundle_p = kaps_trie_handle_exact_strict_lmpsofar_for_delete(
                                        trie_p, destnode, entry, o_reason);
            
        }
        else
        {
            /*Check for the longest prefix in the LSN of destnode which matches with the 
    ...
    else if (rqtCode == NLM_FIB_PREFIX_UPDATE_AD)
    {
        ...
        if (destnode->m_lsn_p->m_pSettings->m_strictlyStoreLmpsofarInAds 
            && destnode->m_depth == entry->pfx_bundle->m_nPfxSize)
        {
            /*If m_strictlyStoreLmpsofarInAds is 1 and the prefix exactly matches the destnode
            then we have to update the ADS of trigger corresponding to the destnode*/
    ...
    if (iterTrieNode)
    {
        kaps_trie_node_update_iitLmpsofar(iterTrieNode->m_child_p[0], ancestorIptNodeWithEntry, &lmpsofarPfxBundle_p, entry->pfx_bundle,
                                          rqtCode);
        kaps_trie_node_update_iitLmpsofar(iterTrieNode->m_child_p[1], ancestorIptNodeWithEntry, &lmpsofarPfxBundle_p, entry->pfx_bundle,
                                          rqtCode);
    }
...
NlmErrNum_t
kaps_process_first_lsn_in_path(
    kaps_lpm_trie *trie_p, 
    kaps_trie_node *destnode,
    struct kaps_lpm_entry * entry,
    NlmReasonCode *o_reason)
    ...
    NlmReasonCode * o_reason)
{
    ...
    if (rqtCode == NLM_FIB_PREFIX_INSERT)
    {
        ...
        {
            errNum = kaps_process_first_lsn_in_path(trie_p, destnode, entry, o_reason);
            if (errNum != NLMERR_OK)
            {
                return errNum;
            }
    ...
    else if (rqtCode == NLM_FIB_PREFIX_DELETE)
    {
        ...
        isLsnEmpty = kaps_trie_is_lsn_empty(trie_p, destnode);
        ...
        {
            NLM_STRY(kaps_trie_merge_ancestor_lsns(trie_p, destnode, o_reason));
        }
    }
    else if (rqtCode == NLM_FIB_PREFIX_UPDATE_AD)
    {
    ...
    NlmReasonCode * o_reason)
{
    ...
    /*kaps_trie_store_pfx_log_records(trie_p, rqtCode, entry);*/
    ...
    NlmReasonCode * o_reason)
{
     ...
     * here. The RPT split will be triggered by  issuing a RETRY_WITH_RPT_SPLIT
     */
    ...
    NlmReasonCode * o_reason)
{
    ...
    NlmErrNum_t errNum = NLMERR_OK;
    uint32_t numAncestorIptPfxToMove = 0, numHolesToClear = 0;
    kaps_lsn_mc *ancestorIptLsn_p = NULL;
    kaps_trie_global *trie_global = trie_p->m_trie_global;
    uint32_t *indexBuffer_p = trie_global->m_indexBuffer_p;
    kaps_pool_mgr *poolMgr = trie_p->m_trie_global->poolMgr;
    ...
    if (trie_global->m_curRptNodeToSplit_p)
    {
        ...
        trie_global->useTrigLenForCalcIptApt = 0;
        kaps_trie_pvt_calc_num_ipt_and_apt(trie_global->m_curRptNodeToSplit_p);
        ...
        isPfxMatchingNewRptNodePresentInAncestor = 0;
        kaps_trie_check_split_nearest_ipt_ancestor(trie_p, newRptNode_p, &ancestorIptLsn_p, 
                                        trie_global->m_pfxLocationsToMove,
                                        &numAncestorIptPfxToMove,
                                        &isPfxMatchingNewRptNodePresentInAncestor,
                                        o_reason);

        /*Copy the prefix locations to be moved into a separate array that stores the locations to be cleared
        We are having a separate array for location to be cleared because, if m_strictlyStoreLmpsofarInAds is 1 
        and the prefix that exactly matches the new RPT is present in the ancestor LSN, then it will not be 
        present in m_pfxLocationsToMove (because it will go to the m_iitLmpsofarPfx_p of new RPT node
        and not go to the LSN in new RPT node) but it will later be added to m_pfxLocationsToClear
        (because it still has to be cleared from ancestor)*/
        numHolesToClear = numAncestorIptPfxToMove;
        for (i = 0; i < numAncestorIptPfxToMove; ++i)
        {
            trie_global->m_pfxLocationsToClear[i] = 
                        trie_global->m_pfxLocationsToMove[i];
        }
        ...
        if (shouldSplitNearestIptAncestor)
        {
             ...
             */
            errNum =
                kaps_trie_split_nearest_ipt_ancestor(newRptNode_p, ancestorIptLsn_p, 
                                        trie_global->m_pfxLocationsToMove,
                                        numAncestorIptPfxToMove,
                                        trie_global->m_pfxLocationsToClear,
                                        &numHolesToClear,
                                        o_reason);
        ...
        if (trie_global->m_isNearestIptAncestorSplit || wasLsnForRptLmpsofarAdded)
        {
            ...
            if (trie_global->m_isNearestIptAncestorSplit)
            {
                ...
                NLM_STRY(kaps_trie_clear_holes_in_split_ipt_ancestor
                         (trie_p, ancestorIptNode, newRptNode_p, 
                            trie_global->m_pfxLocationsToClear,
                            numHolesToClear, 
                            o_reason));
            }
    ...
    uint32_t originalPoolId)
{
    ...
    for (cur_brick_nr = 0; cur_brick_nr < num_bricks; ++cur_brick_nr)
    {
        ...
        maxNumEntriesPerBrick = kaps_lsn_mc_calc_max_triggers_in_brick(trie_p->m_lsn_settings_p,
                                                                gran);
    ...
    NlmReasonCode * o_reason)
{
    ...
    while (trie_p->m_num_items_in_pending_giveouts)
    {
        ...
        errNum = kaps_lsn_mc_giveout(originalLsn, lsnSettings_p, fibTbl, curGiveoutData_p, keepData_p, giveData_p,
    ...
    kaps_trie_node * curNode)
{
    ...
    if (curNode->m_isRptNode)
    {
        ...
        kaps_trie_pvt_calc_num_ipt_and_apt(curNode);
...
kaps_trie_verify_iit_lmpsofar(
    ...
    kaps_trie_node * ancestorIptNode_p)
{
    kaps_pfx_bundle *lmp_in_prev_or_cur_lsn, *lmp_in_prev_lsn, *lmp_in_cur_lsn;
    kaps_lpm_trie *trie_p = NULL;
    ...
    if (is_candidate_node)
    {
        ...
        {
            /*Find the lmpsofar in the ancestor LSN*/
            ...
            if (trie_p->m_lsn_settings_p->m_strictlyStoreLmpsofarInAds)
            {
                /*if m_strictlyStoreLmpsofarInAds is 1 and prefix exactly matching the node is stored in
                the node_p->m_iitLmpsofarPfx_p, then update lmp_in_prev_or_cur_lsn */
                if (node_p->m_iitLmpsofarPfx_p 
                    && node_p->m_iitLmpsofarPfx_p->m_nPfxSize == node_p->m_depth)
                {
                    ...
                    if (lmp_in_prev_lsn && lmp_in_prev_lsn->m_nPfxSize >= lmp_in_cur_lsn->m_nPfxSize)
                        kaps_assert(0, "Previous LSN has the prefix that exactly matches Current LSN\n");
                }
            }
            ...
            {
                /*lmp_in_prev_or_cur_lsn and node_p->m_iitLmpsofarPfx_p should match*/
                if (node_p->m_iitLmpsofarPfx_p != lmp_in_prev_or_cur_lsn)
                {
                    kaps_assert(0, "Lmpsofar is not the longest possible\n");
                }
            }
            else
            {
        ...
        else
        {
            if (trie_p->m_lsn_settings_p->m_strictlyStoreLmpsofarInAds)
            {
                /*If node has no ancestor IPT node, and  node->m_m_iitLmpsofarPfx_p is not NULL,
                then lmpsofar length should match with the node depth*/
                if (node_p->m_iitLmpsofarPfx_p &&
                    node_p->m_iitLmpsofarPfx_p->m_nPfxSize != node_p->m_depth)
                {
            ...
            {
                /*node_p->m_iitLmpsofarPfx_p should be NULL*/
        ...
        if (trie_p->m_lsn_settings_p->m_strictlyStoreLmpsofarInAds)
        {
            /*If m_strictlyStoreLmpsofarInAds is 1, then prefix that exactly matches the trie node 
            can't be present in the LSN
            */
            uint32_t matchBrickNum = 0;
            kaps_pfx_bundle *exactMatchPfxBundle;
            
            exactMatchPfxBundle = kaps_lsn_mc_locate_exact(node_p->m_lsn_p, node_p->m_lp_prefix_p->m_data,
                             node_p->m_lp_prefix_p->m_nPfxSize, &matchBrickNum);

            if (exactMatchPfxBundle)
                kaps_assert(0, "Prefix exactly matching the LSN should not be present in LSN \n");
        }

        /*lmpsofar prefix should have m_isLmpsofarPfx set to 1*/
        if (node_p->m_iitLmpsofarPfx_p && !node_p->m_iitLmpsofarPfx_p->m_isLmpsofarPfx)
            kaps_assert(0, "IIT lmpsofar m_isLmpsofarPfx is 0 \n");



        if (trie_p->m_lsn_settings_p->m_strictlyStoreLmpsofarInAds)
        {
            /*the lmpsofar should be <= the node depth*/
        ...
        {
            /*the lmpsofar length should be < than the node depth*/
            if (node_p->m_iitLmpsofarPfx_p && node_p->m_iitLmpsofarPfx_p->m_nPfxSize >= node_p->m_depth)
                kaps_assert(0, "IIT lmpsofar should be less than the node depth\n");
        }


        /*lmpsofar backPtr should be correct*/
        if (node_p->m_iitLmpsofarPfx_p &&
            node_p->m_iitLmpsofarPfx_p->m_backPtr->pfx_bundle != node_p->m_iitLmpsofarPfx_p)
        {
            kaps_assert(0, "IIT lmpsofar backPtr is incorrect\n");
        }
        

        /*lmpsofar prefix should not be a prefix copy*/
        if (node_p->m_iitLmpsofarPfx_p && node_p->m_iitLmpsofarPfx_p->m_isPfxCopy)
            kaps_assert(0, "IIT lmpsofar should not be a prefix copy\n");


        /*entry handle to the lmpsofar prefix should not be NULL*/
    ...
    NlmReasonCode * o_reason)
{
    ...
    if (iptAncestorNode)
    {
        /*If m_strictlyStoreLmpsofarInAds is 1, then if prefix that exactly matches the new IPT node
        is present in the IPT ancestor, don't add this prefix to the locations to be moved since this prefix
        will not be stored in the LSN. */
    ...
    /*Copy the prefixes locations to be moved into the prefix locations to be cleared. We need 
    separate lists for moving and clearing because we will later on be adding the prefix that exactly
    matches the new IPT node into the prefix locations to clear*/
    ...
    if (trie_p->m_lsn_settings_p->m_strictlyStoreLmpsofarInAds)
    {
        /*If trienodes being merged have prefix that exactly match the trienode, then add these
        prefixes to the extra prefix array and increment totalNumExtraPfx*/
    ...
    if (trie_p->m_trie_global->m_isIITLmpsofar)
    {
        if (iptAncestorNode)
        {
            /*Compute the lmpsofar of the newIptTrieNode. If m_strictlyStoreLmpsofarInAds is 1,
            and prefix that matches new IPT trie node is present, then add it to the prefix locations to
            clear*/
            kaps_trie_ipt_ancestor_iit_lmpsofar(newIptTrieNode, 
        ...
        {
            /*ancestor IPT node doesn't exist. */
...
uint32_t
kaps_trie_find_terminating_path(
    ...
    kaps_trie_node *cur_node)
{
    ...
    if (!cur_node->m_child_p[0]) 
    {
        kaps_printf("Found, left child is NULL \n");

        kaps_trie_print_pfx(NULL, cur_node->m_lp_prefix_p->m_data, 
    ...
    if (!cur_node->m_child_p[1])
    {
        kaps_printf("Found, right child is NULL \n");
    ...
    if (cur_node->m_child_p[0])
    {
        ret_val = kaps_trie_find_terminating_path(rpt_node, cur_node->m_child_p[0]);
        if (ret_val)
        {
            return ret_val;
        }
    ...
    if (cur_node->m_child_p[1])
    {
        ret_val = kaps_trie_find_terminating_path(rpt_node, cur_node->m_child_p[1]);
        if (ret_val)
        {
            return ret_val;
        }
    ...
    struct NlmFibStats *stats_p)
{
    ...
    if (node_p->m_isRptNode)
    {
        ...
        {
            kaps_printf("RPT Node Id = %d at depth = %d is not an IPT node and it has %d IPT nodes under it \n", 
               node_p->m_trienodeId,
               node_p->m_depth,
               node_p->m_numIptEntriesInSubtrie);

            kaps_trie_find_terminating_path(node_p, node_p);
    ...
    if (node_p->m_node_type == NLMNSTRIENODE_LP)
    {
        ...
        while (curLpuBrick)
        {
            ...
            if (brickIter == lsn_p->m_numLpuBricks - 1) 
            {
                ...
                && kaps_lsn_mc_find_if_next_brick_is_joined(lsn_p, brickIter)) 
            {
                stats_p->numHolesInLastBrick += numHolesInCurBrick; 
            } 
            else 
    ...
    if (node_p->m_child_p[0])
        kaps_trie_calc_trie_lsn_stats(node_p->m_child_p[0], find_terminating_search_path, 
                                stats_p);

    if (node_p->m_child_p[1])
        kaps_trie_calc_trie_lsn_stats(node_p->m_child_p[1], find_terminating_search_path,
                                stats_p);
}
    ...
    uint32_t doVerify)
{
    ...
    uint32_t maxNumTriggersInBrick, numTriggersInBrick;
    uint32_t db_group_id = fibTbl->m_db->db_group_id;
    ...
    stats->maxBricks = lpm_db->db_info.hw_res.db_res->num_small_bb_per_row * 
                        device->hw_res->num_rows_in_small_bb[db_group_id];
    ...
    for (poolIter = 0; poolIter < poolMgr->m_numActiveIptPools; ++poolIter)
    {
             ...
             ++cur_brick_nr)
        {
            ...
            maxNumTriggersInBrick = fibTbl->m_trie->m_lsn_settings_p->m_maxTriggersInBrickForGranIx[granIx];

            numTriggersInBrick = 0;
            for (i = 0; i < maxNumTriggersInBrick; ++i)
            {
                if (kaps_array_check_bit
            ...
            stats->numHolesForEachGran[granIx] += maxNumTriggersInBrick - numTriggersInBrick;
            stats->numPfxForEachGran[granIx] += numTriggersInBrick;
    ...
    if (doVerify)
    {
        ...
        for (i = 0; i < lpm_db->db_info.hw_res.db_res->small_bb_brick_bitmap_num_bytes; ++i)
        {
    ...
    uint32_t * longestPfxLen_p)
{
    ...
    {
        kaps_trie_node *trienode = NULL;
        void * tmp_ptr = KAPS_PFX_BUNDLE_GET_ASSOC_PTR(curLsn->m_pParentHandle);
    ...
    NlmReasonCode * o_reason)
{
    ...
    if (!settings_p->m_isPerLpuGran)
    {
        ...
        struct kaps_ad_db *ad_db = (struct kaps_ad_db *) settings_p->m_fibTbl->m_db->common_info->ad_info.ad;
        ...
        maxPfxPerBrick = kaps_lsn_mc_calc_max_pfx_in_lpu_brick(settings_p, ad_db, 0, 0xf, longestPfxGran);
    ...
    if (trie_p->m_lsn_settings_p->m_strictlyStoreLmpsofarInAds)
    {
        /*If prefix exactly matching the downStreamNode is present, then add it to m_extraPfxArray*/
     ...
     * Convert the temporary merged LSN into Flat LSN Data
     */
    errNum = kaps_lsn_mc_convert_lsn_to_flat_data(tempMergedLsn, 
    ...
    kaps_lpm_trie * trie_p)
{
    ...
    struct kaps_db *db = trie_p->m_tbl_ptr->m_db;
    uint32_t rpb_id = db->rpb_id;
    uint32_t num_rows_to_read = device->hw_res->num_rows_in_rpb[rpb_id] / KAPS_HB_ROW_WIDTH_1;
    struct kaps_ab_info *ab;
    ...
    if (settings->m_areIPTHitBitsPresent)
    {
        if (db->num_algo_levels_in_db == 2)
        {
            ...
            if (trie_p->m_iptHitBitBuffer)
                kaps_memset(trie_p->m_iptHitBitBuffer, 0, device->hw_res->num_rows_in_rpb[rpb_id]);
        ...
        else
        {
            ...
            if (trie_p->m_iptHitBitBuffer)
            {
                kaps_memset(trie_p->m_iptHitBitBuffer, 0,
                            (device->hw_res->total_small_bb[db_group_id]) * device->hw_res->num_rows_in_small_bb[db_group_id] *
                            KAPS_HB_ROW_WIDTH_1);
            }

            num_rows_to_read = device->hw_res->num_rows_in_small_bb[db_group_id];
    ...
    if (settings->m_areRPTHitBitsPresent)
    {
        ...
        num_rows_to_read = device->hw_res->num_rows_in_rpb[rpb_id] / KAPS_HB_ROW_WIDTH_1;

        if (trie_p->m_rptTcamHitBitBuffer)
            kaps_memset(trie_p->m_rptTcamHitBitBuffer, 0, device->hw_res->num_rows_in_rpb[rpb_id]);
    ...
    *nv_offset += sizeof(num_rpt_entries);
    ...
    while (ad_db) 
    {
        for (i = 0; i < device->hw_res->num_daisy_chains; ++i) {
            ...
            KAPS_STRY(kaps_ix_mgr_wb_save_ix_translate
    ...
    struct kaps_wb_rpt_entry_info * rpt_entry_info)
{
    ...
    struct kaps_wb_ipt_entry_info ipt_node = {0};
    NlmErrNum_t err_num = NLMERR_OK;
    kaps_ipm *ipm_p;
    ...
    *nv_offset += sizeof(num_rpt_entries);
    ...
    while (ad_db) 
    {
        for (i = 0; i < device->hw_res->num_daisy_chains; ++i) 
        {
            ...
            KAPS_STRY(kaps_ix_mgr_wb_restore_ix_translate
    ...
    kaps_trie_node * trie_node)
{
    ...
    if (trie_node->m_isRptNode)
    {
        ...
        kaps_trie_pvt_calc_num_ipt_and_apt(trie_node);
