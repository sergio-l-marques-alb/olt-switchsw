...
static kaps_status
kaps_dm_kaps_rpb_write_tcam_format_1(
    struct kaps_device * device,
    uint32_t blk_num,
    uint32_t row_num,
    uint8_t * data,
...
static kaps_status
kaps_dm_kaps_rpb_write_tcam_format_2(
    struct kaps_device * device,
    uint32_t blk_num,
    uint32_t row_num,
    uint8_t * data,
    ...
    uint8_t is_xy)
{
    ...
    switch (valid_bit)
    {
        case 0:
            ...
            /*Y*/
            KapsWriteBitsInArray(write_buf, write_buf_len_8, 168 + 167, 168 + 166, 0x3);
    ...
    uint8_t valid_bit)
{
    ...
    {
        /*Store the information in Data-Mask Format in the shadow*/
    ...
    if (device->tcam_format == KAPS_TCAM_FORMAT_2)
    {
        status = kaps_dm_kaps_rpb_write_tcam_format_2(device, blk_num, row_num,
                                        data, mask, valid_bit, 0);
    }
    else if (device->tcam_format == KAPS_TCAM_FORMAT_1)
    {
        status = kaps_dm_kaps_rpb_write_tcam_format_1(device, blk_num, row_num,
                                        data, mask, valid_bit, 0);
    }
    else 
    {
        kaps_assert(0, "Incorrect TCAM Format \n");
