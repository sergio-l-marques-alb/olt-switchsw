...
const char *
kaps_get_status_string(
    kaps_status status)
{
    static const char *status_strings[] = {
#define KAPS_INC_SEL(name,string) string,
#include "kaps_error_tbl.def"
#undef KAPS_INC_SEL
        "should never be seen"
    };

    if (((uint32_t) status) >= KAPS_STATUS_LAST_UNUSED)
        return NULL;
    ...
    kaps_status kaps_err)
{
    ...
    switch (kaps_err)
    {
        ...
        case KAPS_OUT_OF_MEMORY:
            ...
            break;
        case KAPS_INVALID_HANDLE:
        case KAPS_PREFIX_NOT_FOUND:
