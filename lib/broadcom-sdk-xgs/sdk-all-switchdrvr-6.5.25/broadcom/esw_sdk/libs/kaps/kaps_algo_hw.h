 ...
 */
    ...
    struct kaps_device_resource
    {
        ...
        uint16_t num_rows_in_rpb[KAPS_HW_MAX_NUM_RPB_BLOCKS];  /**< Number of 160b rows in the RPB, Data and mask together form a single row*/
        ...
        uint16_t total_num_bb[KAPS_HW_MAX_NUM_DB_GROUPS];
                            /**< Total number of Bucket Blocks in the device */
        uint16_t num_bb_in_one_chain;
                             /**< Total number of Bucket Blocks chained to form a cascade. */
        uint16_t bb_width_1; /**< Width of Bucket Blocks in bits.*/
        uint32_t num_rows_in_bb[KAPS_HW_MAX_NUM_DB_GROUPS]; /**< Number of rows in a BB */
        
        uint16_t total_lpus;/**< LPU is an LSN Processing Unit. It is equal to the number of BB chains
                                                ie, total_num_bb / num_bb_in_one_chain*/
        uint16_t max_lpu_per_db; /**< Max LPUs allowed per database */
        
        uint16_t total_small_bb[KAPS_HW_MAX_NUM_DB_GROUPS];
                             /**< Number of Small Bucket Blocks*/
        uint16_t num_rows_in_small_bb[KAPS_HW_MAX_NUM_DB_GROUPS];
                                   /**< Number of rows in the Small Bucket Block. Relevant only for KAPS*/

        uint32_t num_ads2_blks; /**< Number of ADS2 blocks present*/
        uint32_t ads2_depth[KAPS_HW_MAX_NUM_ADS2_BLOCKS];  /**< Depth of the ADS2 for type a database */
        uint32_t max_ads2_x_table_size; /**< Maximum number of slots of the ADS2 translation table*/
        uint32_t ads2_rpt_lmpsofar_virtual_ix_start; /**< Starting virtual index for the RPT Lmpsofar in the ADS2 translation table*/
        ...
        uint32_t num_db_groups:4; 
                        /**< Number of DB groups present in this device*/
        uint32_t num_db_in_one_group:4; /**< Number of databases in one group */
        ...
        uint32_t ads_width_1;  /**< Width of the ADS in bits*/
        uint32_t ads_width_8;  /**< Width of the ADS in bytes*/

        uint32_t num_daisy_chains; /**< Number of daisy chains*/
