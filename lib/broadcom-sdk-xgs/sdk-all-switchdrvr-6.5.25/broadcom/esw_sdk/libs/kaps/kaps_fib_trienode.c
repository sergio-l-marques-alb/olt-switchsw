...
static void kaps_trie_node_traverse_dfs_for_remove(
    kaps_trie_node * node,
    kaps_trie_node_traverse_dfs_visitor_for_remove_t func,
    ...
    NlmNsTrieNode__TraverseArgs * args)
{
    if (start_node)
    {
        kaps_recursion_status ret;
        kaps_lpm_trie *trie = start_node->m_trie_p;
        kaps_trie_node **ary = kaps_trie_get_stack_space(trie);
        uint32_t n = 0;
        kaps_trie_node *left, *right, *cur_node;
        ...
        ary[n] = start_node;
        processing_status_stack[n] = KAPS_UNTRAVERSED_NODE;
        n++;
        ...
        while (n)
        {
            ...
            {
                kaps_trie_node *rptParentOfNodes = args->arg1;
            ...
            if (*processing_status == KAPS_UNTRAVERSED_NODE)
            {
                left = cur_node->m_child_p[NLMNSTRIE_LCHILD];
                *processing_status = KAPS_LEFT_CHILD_EXPANDED;
            ...
            if (*processing_status == KAPS_LEFT_CHILD_EXPANDED)
            {
                right = cur_node->m_child_p[NLMNSTRIE_RCHILD];
                *processing_status = KAPS_RIGHT_CHILD_EXPANDED;
            ...
            if (*processing_status == KAPS_RIGHT_CHILD_EXPANDED) 
            {
                ret = func(cur_node, args);
                n--;

                if (ret == KAPS_RECURSION_ERROR)
                    break;
            }
...
/*
potentialLmpPfx is the lmpsofar prefix bundle which should be propagated down
...
kaps_trie_node_update_iitLmpsofar(
    kaps_trie_node * self,
    kaps_trie_node * ancestorIptNodeWithPotentialLmpPfx,
    kaps_pfx_bundle ** potentialLmpPfx_pp,
    ...
    NlmTblRqtCode rqtCode)
{
    ...
    uint32_t list_size;
    struct kaps_db *db;
    kaps_pfx_bundle *potentialLmpPfx;
    ...
    if (self->m_child_p[0])
        kaps_trie_node_update_iitLmpsofar(self->m_child_p[0], ancestorIptNodeWithPotentialLmpPfx, 
                        potentialLmpPfx_pp, rqtPfx, rqtCode);

    if (self->m_child_p[1])
        kaps_trie_node_update_iitLmpsofar(self->m_child_p[1], ancestorIptNodeWithPotentialLmpPfx, 
                        potentialLmpPfx_pp, rqtPfx, rqtCode);
