...
#ifndef EXCLUDE_PHYMOD_TSCF16_GEN3_SIM
...
#define TSCF16_GEN3_BF_SET(_val, _mask, _shift) _val |= ((_mask) << (_shift))
#define TSCF16_GEN3_BF_GET(_val, _mask, _shift) (((_val) >> (_shift)) & (_mask))

/*
 * Raw 32-bit address consists of AER value in upper 16 bits and
 * clause 45 address in lower 16 bits.
 */
#define TSCF16_GEN3_DEVAD_SHIFT       27
#define TSCF16_GEN3_DEVAD_MASK        0x1f
#define TSCF16_GEN3_DEVAD_GET(_addr) \
    TSCF16_GEN3_BF_GET(_addr, TSCF16_GEN3_DEVAD_MASK, TSCF16_GEN3_DEVAD_SHIFT)
#define TSCF16_GEN3_LANE_SHIFT        16
#define TSCF16_GEN3_LANE_MASK         0x7
#define TSCF16_GEN3_LANE_GET(_addr) \
    TSCF16_GEN3_BF_GET(_addr, TSCF16_GEN3_LANE_MASK, TSCF16_GEN3_LANE_SHIFT)
#define TSCF16_GEN3_REG_SHIFT         0
#define TSCF16_GEN3_REG_MASK          0xffff
#define TSCF16_GEN3_REG_GET(_addr) \
    TSCF16_GEN3_BF_GET(_addr, TSCF16_GEN3_REG_MASK, TSCF16_GEN3_REG_SHIFT)

#define TSCF16_GEN3_ADDR(_devad, _lane, _reg) \
    (((_devad) << TSCF16_GEN3_DEVAD_SHIFT) +  \
     ((_lane) << TSCF16_GEN3_LANE_SHIFT) +    \
     ((_reg) << TSCF16_GEN3_REG_SHIFT))

#define TSCF16_GEN3_AER               TSCF16_GEN3_ADDR(0, 0, 0xffde)
#define TSCF16_GEN3_BLK               TSCF16_GEN3_ADDR(0, 0, 0x001f)

/*
 * The CL45 indicator is used to determine whether the upper 16 bits
 * of the address is an AER value or a clause 45 DEVAD.
 */
#define TSCF16_GEN3_CL45              (0x20 << 16)
#define TSCF16_GEN3_CL45_MASK         (0xe0 << 16)

#define TSCF16_GEN3_ID0                 0x600d
#define TSCF16_GEN3_ID1                 0x8770
#define TSCF16_GEN3_SERDES_ID           0x0324
#define FALCON16_GEN3_TSC_UCODE_IMAGE_CRC 0x696A

#define TSCF16_GEN3_SIM_FW_LOAD_DONE_REG_ADDR (0x800d203)

/* Forward declarations */
STATIC int
tscf16_gen3_sim_write(phymod_sim_data_t *pms_data, uint32_t addr, uint32_t data);
STATIC int
tscf16_gen3_sim_read(phymod_sim_data_t *pms_data, uint32_t addr, uint32_t *data);

STATIC uint32_t
tscf16_gen3_sim_default_data_get(uint32_t addr)
{
    uint32_t reg;

    reg = TSCF16_GEN3_REG_GET(addr);

    /*force FW load success */
    if(addr == TSCF16_GEN3_SIM_FW_LOAD_DONE_REG_ADDR) {
        return 1;
    }

    switch (reg) {
        case 0x0002:
            return TSCF16_GEN3_ID0;
        case 0x0003:
            return TSCF16_GEN3_ID1;
        case 0x900e:
            return TSCF16_GEN3_SERDES_ID;
        case 0xd03d:
            /*
...
STATIC uint32_t
tscf16_gen3_sim_reg_copies_get(uint32_t addr)
{
    uint32_t devad, reg;

    devad = TSCF16_GEN3_DEVAD_GET(addr);
    reg = TSCF16_GEN3_REG_GET(addr);

    if (reg == TSCF16_GEN3_AER || reg == TSCF16_GEN3_BLK) {
        return 1;
    }
...
STATIC int
_tscf16_gen3_sim_write(phymod_sim_data_t *pms_data, uint32_t addr, uint32_t data)
{
    int idx;
    uint32_t mask;
    ...
    for (idx = 0; idx < pms_data->entries_used; idx++) {
        ...
        if (pse->addr == addr) {
            pse->data &= ~mask;
            pse->data |= data;
            DBG_VERB(("_TSCF16_GEN3_sim_write 0x%08"PRIx32" = 0x%04"PRIx32"\n",
                      addr, pse->data));
    ...
    if (data == tscf16_gen3_sim_default_data_get(addr)) {
        return PHYMOD_E_NONE;
    }
    ...
    DBG_VERB(("_TSCF16_GEN3_sim_write 0x%08"PRIx32" = 0x%04"PRIx32" (new)\n",
              addr, pse->data));
...
STATIC uint32_t
tscf16_gen3_sim_write_adjust(phymod_sim_data_t *pms_data, uint32_t addr, uint32_t data)
{
    ...
    if (devad == 0) {
        ...
        case 0xd207:
            ...
            tscf16_gen3_sim_write(pms_data, addr + 0x4, data);
            break;
        case 0xd0e1:
             ...
             * 0xF80 = 0xd0e1 - 0xc161*/
            tscf16_gen3_sim_read(pms_data, (addr - 0xF80) & 0x7ffff, &val);
            val = (val & 0xfffd) | (((data&0x80)==0x80)?0:0x2);
            tscf16_gen3_sim_write(pms_data, (addr - 0xF80) & 0x7ffff, val);
            break;
        case 0xd147:
            /* Sync configured all lanes' PLL_CAL_CTL7 register. Only one PLL/VCO per core */
            /* Write lanes 0, 1, 2 and 3 */
            addr = TSCF16_GEN3_ADDR(devad, 0, reg);
            (void)_tscf16_gen3_sim_write(pms_data, addr, data);

            addr = TSCF16_GEN3_ADDR(devad, 1, reg);
            (void)_tscf16_gen3_sim_write(pms_data, addr, data);

            addr = TSCF16_GEN3_ADDR(devad, 2, reg);
            (void)_tscf16_gen3_sim_write(pms_data, addr, data);

            addr = TSCF16_GEN3_ADDR(devad, 3, reg);
            (void)_tscf16_gen3_sim_write(pms_data, addr, data);
            break;
        case 0xd094:
        case 0xd095:
            ...
            tscf16_gen3_sim_write(pms_data, addr + 0x9f, data);
            break;
        default:
            break;
        }
...
STATIC int
tscf16_gen3_sim_init(phymod_sim_data_t *pms_data,
...
STATIC int
tscf16_gen3_sim_reset(phymod_sim_data_t *pms_data)
{
    uint32_t sim_size;
...
tscf16_gen3_sim_read(phymod_sim_data_t *pms_data, uint32_t addr, uint32_t *data)
{
    ...
    if (addr < TSCF16_GEN3_BLK) {
        /* Assume clause 22 access */
        (void)tscf16_gen3_sim_read(pms_data, TSCF16_GEN3_BLK, &blk);
        /* IEEE bit */
        if (addr & 0x10) {
            blk |= 0x8000;
        } else {
        ...
        if (addr != TSCF16_GEN3_AER && addr != TSCF16_GEN3_BLK) {
            (void)tscf16_gen3_sim_read(pms_data, TSCF16_GEN3_AER, &aer);
            addr |= (aer << 16);
        }
    } else {
        /* Extract devad if clause 45 address format */
        if ((addr & TSCF16_GEN3_CL45_MASK) == TSCF16_GEN3_CL45) {
            devad = (addr >> 16) & 0x1f;
            addr &= 0xffff;
        }
    }
    ...
    if (addr != TSCF16_GEN3_AER && addr != TSCF16_GEN3_BLK) {
        ...
        if (lane > 3) {
            ...
            addr = TSCF16_GEN3_ADDR(TSCF16_GEN3_DEVAD_GET(addr), 0, TSCF16_GEN3_REG_GET(addr));
        }
    }
    ...
    devad = TSCF16_GEN3_DEVAD_GET(addr);
    reg = TSCF16_GEN3_REG_GET(addr);
    copies = tscf16_gen3_sim_reg_copies_get(addr);
    if (copies == 1) {
        lane = 0;
    } else if (copies == 2) {
        lane &= ~0x1;
    }
    addr = TSCF16_GEN3_ADDR(devad, lane, reg);

    /* for DSC_UC_CTRL always succeed */
    if(addr == 0x800d03d) {
        *data = 0x80;
        DBG_VERB(("tscf16_gen3_sim_read 0x%08"PRIx32" = 0x%04"PRIx32"\n",
                      addr, *data));
        return PHYMOD_E_NONE;
    } else if (addr == 0x800d03e) {
        *data = FALCON16_GEN3_TSC_UCODE_IMAGE_CRC;
        return PHYMOD_E_NONE;
    } else if (reg == 0xd104) {
        *data = 0x8271;
        return PHYMOD_E_NONE;
    ...
    *data = tscf16_gen3_sim_default_data_get(addr);
...
tscf16_gen3_sim_write(phymod_sim_data_t *pms_data, uint32_t addr, uint32_t data)
{
    ...
    if (addr != TSCF16_GEN3_AER && addr != TSCF16_GEN3_BLK) {
        ...
        if (lane > 3) {
            ...
            if (lane == 4 || lane == 6) {
                /* Write lanes 0 and 1 */
                addr = TSCF16_GEN3_ADDR(devad, 8, reg);
                (void)tscf16_gen3_sim_write(pms_data, addr, data);
                addr = TSCF16_GEN3_ADDR(devad, 1, reg);
                (void)tscf16_gen3_sim_write(pms_data, addr, data);
            }
            if (lane == 5 || lane == 6) {
    ...
    data = tscf16_gen3_sim_write_adjust(pms_data, addr, data);
    ...
    return _tscf16_gen3_sim_write(pms_data, addr, data);
}

STATIC int
tscf16_gen3_sim_event(phymod_sim_data_t *pms_data, phymod_sim_event_t event)
{
    if (pms_data == NULL || pms_data->entries == NULL) {
        return PHYMOD_E_INIT;
    }
...
phymod_sim_drv_t tscf16_gen3_sim_drv = {
    tscf16_gen3_sim_init,
    tscf16_gen3_sim_reset,
    tscf16_gen3_sim_read,
    tscf16_gen3_sim_write,
    tscf16_gen3_sim_event
};

#endif /* EXCLUDE_PHYMOD_TSCF16_GEN3_SIM */
