 ...
 * for TSCJ PMD Register accesses.
 */
#define TSCJ_REG_PMD_DEVICE (0x00080000)
...
/*
 * This function should be only be invoked to access Jupiter PMD register.
 * Any PCS register access should be treated as error.
 */
int
phymod_tscj_iblk_read(const phymod_access_t *pa, uint32_t addr, uint32_t *data)
{
    int ioerr = 0;
    ...
    addr &= ~(1 << 16);
    ioerr += PHYMOD_BUS_READ(pa, (addr | TSCJ_REG_PMD_DEVICE), data);
...
int
phymod_tscj_iblk_write(const phymod_access_t *pa, uint32_t addr, uint32_t data)
{
    int ioerr = 0;
    uint32_t devad = (addr >> 16) & 0x1;
    uint32_t add;
    ...
    /* Clear devad in the address */
    addr &= ~(1 << 16);
    ioerr += PHYMOD_BUS_WRITE(pa, (addr | TSCJ_REG_PMD_DEVICE), data);
