...
err_code_t osprey7_v2l8p2_INTERNAL_get_BER_string(srds_access_t *sa__, uint16_t time_ms, char *string, uint8_t string_size) {
    ...
    char string_extra[3] = "";
    uint8_t chk_en = 0;
    struct osprey7_v2l8p2_ber_data_st ber_data_local;/* = BER_DATA_ST_INIT;*/
...
uint16_t osprey7_v2l8p2_INTERNAL_eye_to_mV(srds_access_t *sa__, uint8_t var, uint8_t ladder_range) {
    ...
    slicer = LMS_SLICER;
    EFUN(osprey7_v2l8p2_INTERNAL_ladder_setting_to_mV(sa__, var, ladder_range, slicer, &nlmv_val));
...
err_code_t osprey7_v2l8p2_INTERNAL_read_lane_state(srds_access_t *sa__, osprey7_v2l8p2_lane_state_st_t *istate) {
  ...
  istate->laneid = osprey7_v2l8p2_acc_get_lane(sa__);

  EFUN(osprey7_v2l8p2_INTERNAL_pmd_lock_status(sa__,&istate->pmd_lock, &istate->pmd_lock_chg));
  {
      err_code_t err_code = 0;
      istate->stop_state = osprey7_v2l8p2_INTERNAL_stop_micro(sa__,istate->pmd_lock,&err_code);

      if(err_code) USR_PRINTF(("WARNING: Unable to stop microcontroller,  following data is suspect\n"));
  }
  EFUN(osprey7_v2l8p2_get_rx_tuning_status(sa__, &istate->rx_tuning_done));


  {
      osprey7_v2l8p2_osr_mode_st osr_mode;
      ENULL_MEMSET(&osr_mode, 0, sizeof(osprey7_v2l8p2_osr_mode_st));
      EFUN(osprey7_v2l8p2_INTERNAL_get_osr_mode(sa__, &osr_mode));
      istate->osr_mode = osr_mode;
  }

  if(istate->osr_mode.tx_rx != 255) {
      tx= osprey7_v2l8p2_INTERNAL_get_osr_str(istate->osr_mode.tx_rx, GET_OSR_STR_LONG);
      USR_SNPRINTF(istate->tx_osr_mode_str, sizeof(istate->tx_osr_mode_str), "%s", tx);
  } else {
      tx = osprey7_v2l8p2_INTERNAL_get_osr_str(istate->osr_mode.tx, GET_OSR_STR_SHORT);
      rx = osprey7_v2l8p2_INTERNAL_get_osr_str(istate->osr_mode.rx, GET_OSR_STR_SHORT);
      USR_SNPRINTF(istate->tx_osr_mode_str, sizeof(istate->tx_osr_mode_str), "%s", tx);
  ...
  EFUN(osprey7_v2l8p2_INTERNAL_sigdet_status(sa__,&istate->sig_det, &istate->sig_det_chg));
  ESTM(istate->ppm = rd_cdr_integ_reg()/84);

  ESTM(istate->clk90 = rdv_usr_sts_phase_hoffset());

  EFUN(osprey7_v2l8p2_INTERNAL_decode_br_os_mode(sa__, &istate->CDR));
  ...
  EFUN(osprey7_v2l8p2_read_rx_afe(sa__, RX_AFE_PF, &istate->pf_main));
  ESTM(istate->PF_L = rd_rx_pflow_ctrl_val());

  EFUN(osprey7_v2l8p2_read_rx_afe(sa__, RX_AFE_VGA, &istate->VGA));
  EFUN(osprey7_v2l8p2_read_rx_afe(sa__, RX_AFE_FGA, &istate->FGA));

  ESTM(istate->DCO = rd_dc_offset_bin());
  EFUN(osprey7_v2l8p2_read_rx_afe(sa__, RX_AFE_PF3, &istate->pf3_ctrl));
  ESTM(istate->P1mV = rdv_usr_main_tap_est()/32);
  EFUN(osprey7_v2l8p2_INTERNAL_ladder_setting_to_mV(sa__, (int16_t)istate->P1mV, 0, LMS_SLICER, &istate->P1mV));

  if (istate->rx_pam_mode == NRZ) {
      ESTM(istate->TP0  = rd_rx_phase02_status());
      EFUN(osprey7_v2l8p2_INTERNAL_ladder_setting_to_mV(sa__, (int16_t)istate->TP0, 0, LMS_SLICER, &istate->TP0));
  }
  else {
  ...
  EFUN(osprey7_v2l8p2_INTERNAL_get_ffe_enabled(sa__, &istate->ffe_enabled));
  ESTM(istate->RXFFE_g = rd_ams_rx_ffe_gain());
  istate->RXFFE_g = osprey7_v2l8p2_INTERNAL_gray_to_uint8(istate->RXFFE_g);
  if(istate->ffe_enabled) {
      EFUN(osprey7_v2l8p2_INTERNAL_get_rx_ffe_pre(sa__, &istate->RXFFE_n1));
      EFUN(osprey7_v2l8p2_INTERNAL_get_rx_ffe_post(sa__,&istate->RXFFE_p1));
  }
  ...
  if (istate->rx_pam_mode == NRZ) {
      EFUN(osprey7_v2l8p2_INTERNAL_get_rx_dfe1(sa__, &istate->DFE1));
  }

  EFUN(osprey7_v2l8p2_INTERNAL_get_rx_dfe2(sa__, &istate->DFE2));
  EFUN(osprey7_v2l8p2_INTERNAL_get_rx_dfe3(sa__, &istate->DFE3));
  EFUN(osprey7_v2l8p2_INTERNAL_get_rx_dfe4(sa__, &istate->DFE4));
  EFUN(osprey7_v2l8p2_INTERNAL_get_rx_dfe5(sa__, &istate->DFE5));

  EFUN(osprey7_v2l8p2_INTERNAL_get_rx_dfe6(sa__, &istate->dfe6));
  ESTM(istate->DFESidx = rdv_usr_status_eq_debug11());

  EFUN(osprey7_v2l8p2_INTERNAL_get_tx_ppm(sa__, &istate->tx_ppm));
...

  {
      uint8_t tap_num;
      for (tap_num=0; tap_num<TXFIR_ST_NUM_TAPS; ++tap_num) {
          EFUN(osprey7_v2l8p2_INTERNAL_get_tx_tap(sa__, tap_num, &istate->txfir.tap[tap_num]));
      }
  ...
  EFUN(osprey7_v2l8p2_INTERNAL_get_eye_margin_est(sa__, &istate->heye_left, &istate->heye_right, &istate->veye_upper, &istate->veye_lower));
  {
      struct osprey7_v2l8p2_usr_ctrl_disable_functions_st dsu, dss;
      EFUN(osprey7_v2l8p2_get_usr_ctrl_disable_startup(sa__, &dsu));
      EFUN(osprey7_v2l8p2_get_usr_ctrl_disable_steady_state(sa__, &dss));
  ...
  EFUN(osprey7_v2l8p2_INTERNAL_get_link_time(sa__, &istate->link_time));
  {
      ...
      if (istate->pmd_lock != rx_lock_at_end) {
          USR_PRINTF(("WARNING: Lane %d rx_lock status changed while reading lane state (rx_lock_start=%d, rx_lock_chg_start=%d, rx_lock_at_end=%d, rx_lock_chg_at_end=%d) \n",
                      osprey7_v2l8p2_acc_get_lane(sa__), istate->pmd_lock, istate->pmd_lock_chg, rx_lock_at_end, rx_lock_chg_at_end));
      }
  ...
  EFUN(osprey7_v2l8p2_INTERNAL_get_BER_string(sa__, 100, istate->BER, sizeof(istate->BER)));

  {
    const char * const prepend = "    ";

    if(istate->UC_STS != 0) {
        EFUN(osprey7_v2l8p2_decode_uc_sts(sa__, istate->UC_STS, istate->uc_sts_decoded, sizeof(istate->uc_sts_decoded), prepend));
    }

    if(istate->UC_STS_EXT != 0) {
        EFUN(osprey7_v2l8p2_decode_uc_sts_ext(sa__, istate->UC_STS_EXT, istate->uc_sts_ext_decoded, sizeof(istate->uc_sts_ext_decoded), prepend));
    }
  }
