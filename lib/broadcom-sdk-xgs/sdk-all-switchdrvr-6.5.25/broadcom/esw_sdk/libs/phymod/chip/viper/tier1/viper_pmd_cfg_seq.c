...
int viper_tx_squelch_set(const PHYMOD_ST *pa, int enable)
{
    DIG_MISC3r_t misc3;

    DIG_MISC3r_CLR(misc3);
    if (VIPER_CORE_IS_XGXS_10G_028(pa)) {
        READ_DIG_MISC3r(pa, &misc3);
        DIG_MISC3r_LANEDISABLEf_SET(misc3, enable ? 1 : 0);
        PHYMOD_IF_ERR_RETURN(WRITE_DIG_MISC3r(pa, misc3));
    }
...
int viper_tx_squelch_get(const PHYMOD_ST *pa, int *enable)
{
    DIG_MISC3r_t misc3;
    ...
    if (VIPER_CORE_IS_XGXS_10G_028(pa)) {
        READ_DIG_MISC3r(pa, &misc3);
        *enable = DIG_MISC3r_LANEDISABLEf_GET(misc3);
    }
