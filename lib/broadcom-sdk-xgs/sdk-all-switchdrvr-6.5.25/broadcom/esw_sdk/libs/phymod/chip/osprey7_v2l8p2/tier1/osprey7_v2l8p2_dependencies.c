...
err_code_t osprey7_v2l8p2_pmd_mwr_reg(srds_access_t *sa__, uint16_t addr, uint16_t mask, uint8_t lsb, uint16_t val) {
...
uint8_t osprey7_v2l8p2_get_lane(srds_access_t *sa__) {
...
err_code_t osprey7_v2l8p2_delay_ns(srds_access_t *sa__, uint16_t delay_ns) {
    uint32_t delay;
    delay = delay_ns / 1000;
    if (!delay ) {
        delay = 1;
...
err_code_t osprey7_v2l8p2_delay_ms(srds_access_t *sa__, uint32_t delay_ms) {
...
err_code_t osprey7_v2l8p2_pmd_wr_reg(srds_access_t *sa__, uint16_t address, uint16_t val){
...
err_code_t osprey7_v2l8p2_delay_us(srds_access_t *sa__, uint32_t delay_us){
    PHYMOD_USLEEP(delay_us);
    return ( 0 );
}

err_code_t osprey7_v2l8p2_pmd_rdt_reg(phymod_access_t *pa, uint16_t address, uint16_t *val) {
    uint32_t data;
    uint32_t error_code;
...
uint8_t osprey7_v2l8p2_get_core(srds_access_t *sa__) {
    return(0);
}

err_code_t osprey7_v2l8p2_pmd_wr_pram(srds_access_t *sa__, uint8_t val){

    return ERR_CODE_NONE;

}

err_code_t osprey7_v2l8p2_set_lane(srds_access_t *sa__, uint8_t lane_index)
{
...
uint8_t osprey7_v2l8p2_get_pll_idx(srds_access_t *sa__)
{
    uint32_t data;
    data = sa__->pll_idx;
    return (data & 0x1);
...
err_code_t osprey7_v2l8p2_set_pll_idx(srds_access_t *sa__, uint8_t pll_index)
{
    sa__->pll_idx = pll_index;
    return ERR_CODE_NONE;
}

uint8_t osprey7_v2l8p2_get_micro_idx(srds_access_t *sa__)
{
    uint32_t data;
    data = sa__->pll_idx;
    return (data & 0x3);
...
err_code_t osprey7_v2l8p2_set_micro_idx(srds_access_t *sa__, uint8_t micro_index)
{
...
srds_info_t *osprey7_v2l8p2_get_info_table_address(srds_access_t *sa__)
{
