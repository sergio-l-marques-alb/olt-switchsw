  ...
  if ((str_len > 2) && ((USR_STRNCMP(pattern, "0x", 2)) == 0)) {
    ...
    for (i=2; i < str_len; i++) {
      if (pattern[i] != '_') {
        ...
        EFUN(osprey7_v2l8p1_INTERNAL_compute_bin(sa__, pattern[i],bin, sizeof(bin)));
    ...
    for (i=1; i < str_len; i++) {
      if (pattern[i] != '_') {
        ...
        EFUN(osprey7_v2l8p1_INTERNAL_pam4_to_bin(sa__, pattern[i], bin, sizeof(bin)));
        bytes_written = USR_SNPRINTF(patt_mod + mod_count, sizeof(patt_mod) - mod_count, "%s",  bin);
        mod_count += (size_t)bytes_written;
        if(( bytes_written < 0) || (mod_count >= sizeof(patt_mod))) {
          return ERR_CODE_REACHED_BUF_SIZE_LIMIT;
...
err_code_t osprey7_v2l8p1_BER_setup_measurement(srds_access_t *sa__, uint16_t time_ms, struct osprey7_v2l8p1_ber_data_st *ber_data, struct osprey7_v2l8p1_prbs_chk_hw_timer_ctrl_st *prbs_chk_hw_timer_ctrl) {
    uint8_t lcklost = 0;
    uint32_t err_cnt= 0;
    uint8_t clk_gate = 0;
    ...
    return(ERR_CODE_NONE);
} /* osprey7_v2l8p1_BER_setup_measurement() */

err_code_t osprey7_v2l8p1_BER_poll_measurement(srds_access_t *sa__, struct osprey7_v2l8p1_ber_data_st *ber_data, struct osprey7_v2l8p1_prbs_chk_hw_timer_ctrl_st *prbs_chk_hw_timer_ctrl) {
...
err_code_t osprey7_v2l8p1_BER_get_measurement(srds_access_t *sa__, struct osprey7_v2l8p1_ber_data_st *ber_data, struct osprey7_v2l8p1_prbs_chk_hw_timer_ctrl_st *prbs_chk_hw_timer_ctrl) {
    uint8_t  lcklost = 0;
    ...
    return(ERR_CODE_NONE);
} /* osprey7_v2l8p1_BER_get_measurement() */
...
const char* osprey7_v2l8p1_e2s_prbs_mode_enum[22] = {
    " PRBS_7",
    " PRBS_9",
    "PRBS_11",
    "PRBS_15",
...
err_code_t osprey7_v2l8p1_get_prbs_error_analyzer_lane_err_count(srds_access_t *sa__, osprey7_v2l8p1_prbs_err_analyzer_config_status_st *err_analyzer, uint8_t lane) {
    ...
    EFUN(osprey7_v2l8p1_read_dac_pattern_memory(sa__,dacmem2,(uint8_t)(4*(num_lanes+1)),2));
    ...
    for (i=OSPREY7_V2L8P1_PRBS_NUM_OF_ERROR_ANALYZER_COUNTERS; i > 0; i--) {
        if (i!=OSPREY7_V2L8P1_PRBS_NUM_OF_ERROR_ANALYZER_COUNTERS) {
...
/*  ii) Need at least 2 valid data points to generated projected data                                             */
/*----------------------------------------------------------------------------------------------------------------*/
err_code_t osprey7_v2l8p1_prbs_error_analyzer_extrapolate_err_count(srds_access_t *sa__, osprey7_v2l8p1_prbs_err_analyzer_config_status_st *err_analyzer, uint8_t err_cnt_idx ) {
#ifdef SERDES_API_FLOATING_POINT
    INIT_SRDS_ERR_CODE
    uint8_t    delta_n;
    uint8_t    i;
    ...
    if (delta_n >= 2) { /* There are at least 2 points to trace a line for extrapolation */
        ...
        EFUN(osprey7_v2l8p1_INTERNAL_fit_second_order(sa__, delta_n, x, y, a));
    }
