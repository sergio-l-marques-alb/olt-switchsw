...
#define FALCON16_LANE_RAM_BYTE7_BIT5 (1 << 5)  /* to skip the TXFIR initialization in a restart event during the AN link training */
#define FALCON16_LANE_RAM_BYTE7_BIT6 (1 << 6)  /* to initialize TXFIR in 1G mode (CL73 AN) to (0,100,0). */
...
STATIC
int _tscf16_phy_firmware_lane_an_cl72_tx_init_skip_on_restart_set(const phymod_phy_access_t* phy, uint32_t enable)
{
    uint32_t data;

    PHYMOD_IF_ERR_RETURN(falcon16_tsc_cl93n72_frc_byte_get(&phy->access, &data));

    if (enable) {
        data |= (FALCON16_LANE_RAM_BYTE7_BIT5 | FALCON16_LANE_RAM_BYTE7_BIT6);
    } else {
        data &= ~(FALCON16_LANE_RAM_BYTE7_BIT5 | FALCON16_LANE_RAM_BYTE7_BIT6);
    }

    PHYMOD_IF_ERR_RETURN(falcon16_tsc_cl93n72_frc_byte_set(&phy->access, data));

    return PHYMOD_E_NONE;
}

STATIC
int _tscf16_phy_firmware_lane_an_cl72_tx_init_skip_on_restart_get(const phymod_phy_access_t* phy, uint32_t *enable)
{
    uint32_t data;

    PHYMOD_IF_ERR_RETURN(falcon16_tsc_cl93n72_frc_byte_get(&phy->access, &data));

    *enable = data & (FALCON16_LANE_RAM_BYTE7_BIT5 | FALCON16_LANE_RAM_BYTE7_BIT6) ? 1 : 0;
...
int _tscf16_phy_firmware_lane_config_set(const phymod_phy_access_t* phy, phymod_firmware_lane_config_t fw_config)
{
    ...
    for (i = 0; i < num_lane; i++) {
        ...
        if (!is_warm_boot) {
            ...
            PHYMOD_IF_ERR_RETURN(_tscf16_phy_firmware_lane_an_cl72_tx_init_skip_on_restart_set(&phy_copy, fw_config.AnCl72TxInitSkipOnRestart));
...
int tscf16_phy_firmware_lane_config_get(const phymod_phy_access_t* phy, phymod_firmware_lane_config_t* fw_lane_config)
{
    ...
    PHYMOD_IF_ERR_RETURN(_tscf16_phy_firmware_lane_an_cl72_tx_init_skip_on_restart_get(phy, &enable));
    ...
    fw_lane_config->AnCl72TxInitSkipOnRestart = enable;
...
int _tscf16_speed_id_interface_config_get(const phymod_phy_access_t* phy, int speed_id, phymod_phy_inf_config_t* config)
{
    ...
    if (ilkn_set) {
        ...
        case 0x32:
            ...
            break;
        case 0x34:
            config->data_rate = (plldiv_r_val == 0x7) ? 50000 : 53000;
            config->interface_type = phymodInterfaceCR2;
            break;
        case 0x35:
            config->data_rate = (plldiv_r_val == 0x7) ? 50000 : 53000;
...
int _tscf16_core_init_pass1(const phymod_core_access_t* core, const phymod_core_init_config_t* init_config, const phymod_core_status_t* core_status)
{
    ...
    /* Assert micro reset and allocate uc DATA/CODE RAM */
    PHYMOD_IF_ERR_RETURN
        (falcon16_tsc_uc_reset(&core_copy.access, 1));
