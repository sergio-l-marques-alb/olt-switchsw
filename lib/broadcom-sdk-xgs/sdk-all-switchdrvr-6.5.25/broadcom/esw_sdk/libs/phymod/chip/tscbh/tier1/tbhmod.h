...
typedef struct tbhmod_fec_error_mask_s {
...
} tbhmod_fec_error_mask_t;

#define TBHMOD_CL74_EN_FLAG_SET(error_control)           (error_control |= 0x1 << 9)
#define TBHMOD_RSFEC_1XN_EN_FLAG_SET(error_control)      (error_control |= 0x1 << 10)
#define TBHMOD_RSFEC_2XN_EN_FLAG_SET(error_control)      (error_control |= 0x1 << 11)

#define TBHMOD_CL74_EN_FLAG_GET(error_control)           (((error_control) & (0x1 << 9)) ? 1 : 0)
#define TBHMOD_RSFEC_1XN_EN_FLAG_GET(error_control)      (((error_control) & (0x1 << 10)) ? 1 : 0)
#define TBHMOD_RSFEC_2XN_EN_FLAG_GET(error_control)      (((error_control) & (0x1 << 11)) ? 1 : 0)
...
typedef enum {
    TBHMOD_AN_TIMER_NONE = 0,
    TBHMOD_AN_FAIL_INHIBIT_TIMER_LT_PAM4
}tbhmod_an_timer_t;

typedef struct tbhmod_rx_rsfec_sync_status_s {
    uint8_t fec_amps_lock_live_0;
    uint8_t fec_amps_lock_live_1;
} tbhmod_rx_rsfec_sync_status_t;
...
extern int tbhmod_pcs_1588_ts_offset_get(PHYMOD_ST *pc, uint32_t *ns_offset, uint32_t *sub_ns_offset);
...
extern int tbhmod_pcs_rx_ts_update_en(PHYMOD_ST* pc, int en);
...
extern int tbhmod_ts_offset_rx_set(PHYMOD_ST* pc, uint32_t vco, int os_mode, int tbl_ln, uint32_t *table);
...
extern int tbhmod_fec_cl74_correctable_counter_get(PHYMOD_ST* pc, uint32_t* count);
extern int tbhmod_fec_cl74_uncorrectable_counter_get(PHYMOD_ST* pc, uint32_t* count);
...
extern int tbhmod_get_pcs_latched_status_local_fault_reasons (PHYMOD_ST* pc, 
                                                              uint32_t *pcs_link_status_live,
                                                              uint32_t *lf_latch_status,
                                                              uint32_t *deskew_latch_low);
extern int tbhmod_pmd_lock_change(PHYMOD_ST* pc, uint8_t *state);
extern int tbhmod_am_lock_latched_low_get(PHYMOD_ST* pc, int bit_mux_mode, uint32_t *state);
extern int tbhmod_phy_fec_align_latched_low_get(PHYMOD_ST* pc, uint32_t *fec_latch_low);

extern int tbhmod_fec_error_inject_set(PHYMOD_ST *pc,
                                uint16_t error_control_map,
                                tbhmod_fec_error_mask_t bit_error_mask);
extern int tbhmod_fec_error_inject_get(PHYMOD_ST *pc,
                                uint16_t* error_control_map,
                                tbhmod_fec_error_mask_t* bit_error_mask);

extern int tbhmod_an_link_fail_inhibit_timer_set(PHYMOD_ST* pc,
                                                 tbhmod_an_timer_t timer_type,
                                                 uint32_t period);
extern int tbhmod_an_link_fail_inhibit_timer_get(PHYMOD_ST* pc,
                                                 tbhmod_an_timer_t timer_type,
                                                 uint32_t* period);

extern int tbhmod_bip_error_counter_get(PHYMOD_ST* pc, uint32_t* count);
extern int tbhmod_cl49_ber_counter_get(PHYMOD_ST* pc, uint32_t* count);
extern int tbhmod_cl82_ber_counter_get(PHYMOD_ST* pc, uint32_t* count);
extern int tbhmod_tick_time_get(PHYMOD_ST* pc, uint32_t *tick_time);
extern int tbhmod_am_lock_latch_state_get(PHYMOD_ST* pc, int nums_fec_lane, uint32_t *state);
extern int tbhmod_rs_fec_align_live_state_get(PHYMOD_ST* pc, uint32_t *state);
extern int tbhmod_rs_fec_amps_lock_state_get(PHYMOD_ST* pc, int nums_fec_lane, tbhmod_rx_rsfec_sync_status_t* sync_status);
#endif  /*  _tbhmod_H_ */
