                                                     ...
                                                     phymod_phy_speed_config_t *speed_config)
{
    ...
    PHYMOD_IF_ERR_RETURN
        (_tsco_dpll_per_lane_data_rate_get(vco_rate, osr_mode.rx, firmware_lane_config.ForcePAM4Mode, &data_rate_lane));
...
int tsco_dpll_phy_autoneg_set(const phymod_phy_access_t* phy, const phymod_autoneg_control_t* an)
{
    ...
    if (do_lane_config_set) {
        ...
        phy_copy.access.lane_mask = 0x1 << start_lane;
        PHYMOD_IF_ERR_RETURN
            (osprey7_v2l8p2_set_osr_mode(&phy_copy.access,
                                         (enum osprey7_v2l8p2_osr_mode_enum)TSCOMOD_OS_MODE_UNINITIALIZED));
    }
...
int tsco_dpll_intr_handler(const phymod_phy_access_t* phy, phymod_interrupt_type_t type, uint32_t* is_handled)
{
    ...
    switch (type) {
        ...
        case phymodIntrEccRx1588Mpp1:
            if (intr_status.is_1b_err) {
                index = intr_status.err_addr;
                if (index >= TSCOMOD_DPLL_TS_RX_MPP_MEM_SIZE ) {
                    return PHYMOD_E_FAIL;
                }
                PHYMOD_IF_ERR_RETURN
