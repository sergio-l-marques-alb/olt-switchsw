...
int tqmod_lane_info(PHYMOD_ST *pc, int *lane, int *sub_port)
{
    ...
    *lane = i ;
    if(!PHYMOD_ACC_F_QMODE_GET(pc)) {  /* no qmode */
...
*/
int tqmod_port_state_set(PHYMOD_ST* pc, int qmod_state_type, int sub_port, int val)
{
    phymod_access_t pc_copy;
    int all_sub_port_val = 0;
    int start_lane, num_lane;
    DIG_SPARE0r_t  reg_spare0 ;
    DIG_SPARE1r_t  reg_spare1 ;

    DIG_SPARE0r_CLR(reg_spare0);
    DIG_SPARE1r_CLR(reg_spare1);

    PHYMOD_MEMCPY(&pc_copy, pc, sizeof(pc_copy));
    PHYMOD_IF_ERR_RETURN
          (phymod_util_lane_config_get(&pc_copy, &start_lane, &num_lane));

    pc_copy.lane_mask = 0x1 << (start_lane/4);

    switch (qmod_state_type) {
    case TQMOD_PORT_STATE_CONFIGED:
        PHYMOD_IF_ERR_RETURN(READ_DIG_SPARE0r(&pc_copy, &reg_spare0));
        all_sub_port_val = DIG_SPARE0r_SPARE0f_GET(reg_spare0) & 0xf;
        if (val) {
            all_sub_port_val |= 1<< sub_port;
        } else {
            all_sub_port_val = all_sub_port_val & ~(1 << sub_port);
        }
        DIG_SPARE0r_SPARE0f_SET(reg_spare0, all_sub_port_val);
        PHYMOD_IF_ERR_RETURN(WRITE_DIG_SPARE0r(&pc_copy, reg_spare0));
        break;
    case TQMOD_PORT_STATE_TX_SQUELCH:
        PHYMOD_IF_ERR_RETURN(READ_DIG_SPARE1r(&pc_copy, &reg_spare1));
        all_sub_port_val = DIG_SPARE1r_SPARE1f_GET(reg_spare1);
        if (val) {
            all_sub_port_val |= 1<< sub_port;
        } else {
            all_sub_port_val = all_sub_port_val & ~(1 << sub_port);
        }
        DIG_SPARE1r_SPARE1f_SET(reg_spare1, all_sub_port_val);
        PHYMOD_IF_ERR_RETURN(WRITE_DIG_SPARE1r(&pc_copy, reg_spare1));
        break;
    case TQMOD_PORT_STATE_RX_SQUELCH:
        PHYMOD_IF_ERR_RETURN(READ_DIG_SPARE1r(&pc_copy, &reg_spare1));
        all_sub_port_val = DIG_SPARE1r_SPARE1f_GET(reg_spare1);
        if (val) {
...
int tqmod_port_state_get(PHYMOD_ST* pc, int qmod_state_type, int *all_sub_port_val)
{
    phymod_access_t pc_copy;
    int start_lane, num_lane;
    DIG_SPARE0r_t  reg_spare0;
    ...
    case TQMOD_PORT_STATE_TX_SQUELCH:
        PHYMOD_IF_ERR_RETURN(READ_DIG_SPARE1r(&pc_copy, &reg_spare1));
        *all_sub_port_val = DIG_SPARE1r_SPARE1f_GET(reg_spare1) & 0xf;
        break;
    case TQMOD_PORT_STATE_RX_SQUELCH:
        PHYMOD_IF_ERR_RETURN(READ_DIG_SPARE1r(&pc_copy, &reg_spare1));
        *all_sub_port_val = (DIG_SPARE1r_SPARE1f_GET(reg_spare1) >> 4) & 0xf;
        break;
    default:
        return PHYMOD_E_PARAM;
        break;
