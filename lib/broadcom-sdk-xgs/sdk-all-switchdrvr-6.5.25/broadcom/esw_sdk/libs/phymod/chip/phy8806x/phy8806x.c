...
int phy8806x_port_pause_addr_set(const phymod_phy_access_t* phy, phymod_pause_addr_t addr)
{
    uint32_t xmodtxbuff[XMOD_BUFFER_MAX_LEN/4];
    uint8_t *buftxptr;
    int xmodtxlen, rv=PHYMOD_E_NONE;
    xmod_pause_addr_t x_pause_addr;

    *(uint32_t *)&addr.mac[0] = HOST2LE32(*(uint32_t *)&addr.mac[0]);
    *(uint16_t *)&addr.mac[4] = HOST2LE16(*(uint16_t *)&addr.mac[4]);

    PHYMOD_MEMCPY(x_pause_addr.mac, addr.mac, 6);
    x_pause_addr.sys_side = (phy->port_loc == phymodPortLocSys) ? 1 : 0;

    /* write args to xmod buffer */
    buftxptr = (uint8_t *) xmodtxbuff;
    WRITE_XMOD_ARG_BUFF(buftxptr, &x_pause_addr, sizeof(xmod_pause_addr_t));
    xmodtxlen = GET_XMOD_ETHERNET_CMD_IN_LEN(XMOD_PAUSE_ADDR_SET);
    xmodtxlen = GET_XMOD_BUF_WORD_LEN(xmodtxlen);

    /* call xmod */
    rv = _phy_8806x_xmod_command(&phy->access, XMOD_CMD_MODE_ETH(XMOD_PAUSE_ADDR_SET),
                                xmodtxbuff, xmodtxlen, NULL, 0);

    return rv;
}

int phy8806x_port_pause_addr_get(const phymod_phy_access_t* phy, phymod_pause_addr_t* addr)
{
    uint32_t xmodrxbuff[XMOD_BUFFER_MAX_LEN/4];
    uint32_t xmodtxbuff[XMOD_BUFFER_MAX_LEN/4];
    uint8_t *bufrxptr;
    ...
    buftxptr = (uint8_t *)xmodtxbuff;
    WRITE_XMOD_ARG_BUFF(buftxptr, &sys_side, sizeof(uint32_t));
    xmodtxlen = GET_XMOD_ETHERNET_CMD_IN_LEN(XMOD_PAUSE_ADDR_GET);
    xmodtxlen = GET_XMOD_BUF_WORD_LEN(xmodtxlen);

    xmodrxlen = GET_XMOD_ETHERNET_CMD_OUT_LEN(XMOD_PAUSE_ADDR_GET);
    xmodrxlen = GET_XMOD_BUF_WORD_LEN(xmodrxlen);

    rv = _phy_8806x_xmod_command(&phy->access, XMOD_CMD_MODE_ETH(XMOD_PAUSE_ADDR_GET),
                                xmodtxbuff, xmodtxlen, xmodrxbuff, xmodrxlen);

    /* retrieve the argument from the xmod rx buffer */
    bufrxptr = (uint8_t *)xmodrxbuff;
    READ_XMOD_ARG_BUFF(bufrxptr, &x_pause_addr, sizeof(x_pause_addr));

    PHYMOD_MEMCPY(addr->mac, x_pause_addr.mac, 6);
