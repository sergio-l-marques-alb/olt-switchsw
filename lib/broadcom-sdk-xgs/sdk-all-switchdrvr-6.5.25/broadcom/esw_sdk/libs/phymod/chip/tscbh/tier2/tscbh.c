...
#define TSCBH_MAX_NUM_AM_RETRY          3
...
int _tscbh_core_firmware_load(const phymod_core_access_t* core, const phymod_core_init_config_t* init_config)
{
    ...
    if (PHYMOD_CORE_INIT_F_FLR_SUPPORT_GET(init_config)) {
        if (rev_num == TBHMOD_REV_NUM_0) {
            PHYMOD_DEBUG_ERROR(("devad 0x%"PRIx32" lane 0x%"PRIx32": FLR unsupport\n", core->access.addr, core->access.lane_mask));
            return PHYMOD_E_CONFIG;
        } else {
            blackhawk_ucode = blackhawk_flr_ucode_get();
            blackhawk_ucode_len = BLACKHAWKB0_TSC_FLR_UCODE_IMAGE_SIZE;
        }
    } else {
        if (rev_num == TBHMOD_REV_NUM_0) {
...
int _tscbh_phy_firmware_lane_config_set(const phymod_phy_access_t* phy, phymod_firmware_lane_config_t fw_config)
{
    ...
    PHYMOD_IF_ERR_RETURN
        (blackhawk_tsc_INTERNAL_is_flr_supported(&phy_copy.access, &flr_support));
    if (flr_support && fw_config.ForceExtenedReach) {
                                   ...
                                   phymod_phy_pll_state_t* new_pll_state)
{
    ...
    tscbh_spd_id_entry_26 = tscbh_spd_id_entry_26_get();
    tscbh_spd_id_entry_25 = tscbh_spd_id_entry_25_get();
    tscbh_spd_id_entry_20 = tscbh_spd_id_entry_20_get();
        ...
        if ((tvco_pll_div == TBHMOD_PLL_MODE_DIV_170) || (tvco_pll_div == TBHMOD_PLL_MODE_DIV_85))  {
            ...
            PHYMOD_IF_ERR_RETURN
                (phymod_mem_write(&pm_phy_copy.access, phymodMemSpeedIdTable, TSCBH_FORCED_SPEED_ID_OFFSET + start_lane,
                                  (tscbh_spd_id_entry_26 + mapped_speed_id * TSCBH_SPEED_ID_ENTRY_SIZE)));
            ...
            PHYMOD_IF_ERR_RETURN
                (phymod_mem_write(&pm_phy_copy.access, phymodMemSpeedIdTable, TSCBH_FORCED_SPEED_ID_OFFSET + start_lane,
                                  (tscbh_spd_id_entry_25 + mapped_speed_id * TSCBH_SPEED_ID_ENTRY_SIZE)));
        } else {
            /* then load 20G TVCO speed id entry */
            PHYMOD_IF_ERR_RETURN
                (phymod_mem_write(&pm_phy_copy.access, phymodMemSpeedIdTable, TSCBH_FORCED_SPEED_ID_OFFSET + start_lane,
                                  (tscbh_spd_id_entry_20 + mapped_speed_id * TSCBH_SPEED_ID_ENTRY_SIZE)));
        }
    }
...
int tscbh_phy_cl72_set(const phymod_phy_access_t* phy, uint32_t cl72_en)
{
    ...
    PHYMOD_IF_ERR_RETURN
        (blackhawk_tsc_INTERNAL_is_flr_supported(&pm_phy_copy.access, &flr_support));
    if (flr_support) {
...
int _tscbh_core_init_pass1(const phymod_core_access_t* core, const phymod_core_init_config_t* init_config, const phymod_core_status_t* core_status)
{
    ...
    tscbh_am_table_entry = tscbh_am_table_entry_get();
    tscbh_um_table_entry = tscbh_um_table_entry_get();
    tscbh_speed_priority_mapping_table = tscbh_speed_priority_mapping_table_get();
    ...
    if (PHYMOD_CORE_INIT_F_FLR_SUPPORT_GET(init_config)) {
        if (rev_num == TBHMOD_REV_NUM_0) {
            ...
            ucode.stack_size = BLACKHAWKB0_TSC_FLR_UCODE_STACK_SIZE;
            ucode.ucode_size = BLACKHAWKB0_TSC_FLR_UCODE_IMAGE_SIZE;
            blackhawk_ucode_len = BLACKHAWKB0_TSC_FLR_UCODE_IMAGE_SIZE;
            blackhawk_ucode = blackhawk_flr_ucode_get();
        }
    } else {
    ...
    for (i = 0; i < am_table_load_size; i++) {
        PHYMOD_IF_ERR_RETURN
            (phymod_mem_write(&core_copy.access, phymodMemAMTable, i,
                              (tscbh_am_table_entry + i * TSCBH_AM_ENTRY_SIZE)));
    }

    for (i = 0; i < um_table_load_size; i++) {
        PHYMOD_IF_ERR_RETURN
            (phymod_mem_write(&core_copy.access, phymodMemUMTable, i,
                              (tscbh_um_table_entry + i * TSCBH_UM_ENTRY_SIZE)));
    }
...
int _tscbh_core_init_pass2(const phymod_core_access_t* core, const phymod_core_init_config_t* init_config, const phymod_core_status_t* core_status)
{
        ...
        for (i = 0; i < speed_id_load_size; i++) {
            PHYMOD_IF_ERR_RETURN
                (phymod_mem_write(&core_copy.access, phymodMemSpeedIdTable, i,
                                  (tscbh_spd_id_entry_26 + i * TSCBH_SPEED_ID_ENTRY_SIZE)));
        }
    } else if (tvco_rate == TBHMOD_VCO_25G) {
        for (i = 0; i < speed_id_load_size; i++) {
            PHYMOD_IF_ERR_RETURN
                (phymod_mem_write(&core_copy.access, phymodMemSpeedIdTable, i,
                                  (tscbh_spd_id_entry_25 + i * TSCBH_SPEED_ID_ENTRY_SIZE)));
        }
    } else if (tvco_rate == TBHMOD_VCO_20G) {
        for (i = 0; i < speed_id_load_size; i++) {
            PHYMOD_IF_ERR_RETURN
                (phymod_mem_write(&core_copy.access, phymodMemSpeedIdTable, i,
                                  (tscbh_spd_id_entry_20 + i * TSCBH_SPEED_ID_ENTRY_SIZE)));
        }
    }
    ...
    if (init_config->firmware_load_method != phymodFirmwareLoadMethodNone) {
        ...
        if (PHYMOD_CORE_INIT_F_FLR_SUPPORT_GET(init_config)) {
            if (rev_num == TBHMOD_REV_NUM_0) {
                ...
                ucode.stack_size = BLACKHAWKB0_TSC_FLR_UCODE_STACK_SIZE;
                ucode.ucode_size = BLACKHAWKB0_TSC_FLR_UCODE_IMAGE_SIZE;
                ucode.crc_value  = BLACKHAWKB0_TSC_FLR_UCODE_IMAGE_CRC;
            }
        } else {
            if (rev_num == TBHMOD_REV_NUM_0) {
                ucode.stack_size = BLACKHAWK_TSC_UCODE_STACK_SIZE;
    ...
    for (lane = 0; lane < TSCBH_NOF_LANES_IN_CORE; lane++) {
        ...
        PHYMOD_IF_ERR_RETURN
            (blackhawk_tx_rx_polarity_set(&phy_access_copy.access,
                                          tmp_pol.tx_polarity, tmp_pol.rx_polarity));
    ...
    for (lane = 0; lane < TSCBH_NOF_LANES_IN_CORE; lane++) {
        phy_access_copy.access.lane_mask = 1 << lane;
        PHYMOD_IF_ERR_RETURN
            (blackhawk_tsc_set_usr_ctrl_lane_event_log_level(&phy_access_copy.access, 2));
    }
...
int tscbh_timesync_enable_set(const phymod_phy_access_t* phy, uint32_t flags, uint32_t enable)
{
    ...
    if (PHYMOD_TIMESYNC_ENABLE_F_RX_PCS_ENABLE_GET(flags)) {
        PHYMOD_IF_ERR_RETURN
            (tbhmod_pcs_rx_ts_update_en(&phy_copy.access, enable));
    }
...
int tscbh_timesync_offset_get(const phymod_core_access_t* core, uint32_t *ts_offset)
{
    ...
    PHYMOD_IF_ERR_RETURN
        (tbhmod_pcs_1588_ts_offset_get(&core_copy.access, ts_offset, &sub_ns_offset));
...
int tscbh_timesync_adjust_set(const phymod_phy_access_t* phy, uint32_t flags, uint32_t ts_am_norm_mode)
{
    ...
    char intr_str[16];
    ts_table_entry* ts_table_tx_sop;
    ts_table_entry* ts_table_rx_sop;
    int am_processing =1;
    int num_retry = 0;
    int rv = 0;

    ts_table_tx_sop = ts_table_tx_sop_get();
    ts_table_rx_sop = ts_table_rx_sop_get();
    ...
    */
    ...
    PHYMOD_IF_ERR_RETURN
        (tbhmod_pcs_rx_ts_update_en(&phy_copy.access, 1));
    ...
    if (ts_am_norm_mode == 0x1) {
        ...
        while (am_processing && (num_retry < TSCBH_MAX_NUM_AM_RETRY)) {
            ...
            rv = tbhmod_pcs_mod_rx_1588_tbl_val(&phy_copy.access, speed_config_entry.bit_mux_mode,
         ...
         */
        if (num_retry == TSCBH_MAX_NUM_AM_RETRY) {
        ...
        PHYMOD_IF_ERR_RETURN
            (tbhmod_ts_offset_rx_set(&phy_copy.access, current_vco, osr_mode, psll_entry_size, psll_entry));
                       ...
                       uint32_t* is_handled)
{
    ...
    uint32_t ts_table_entry[TBHMOD_TS_ENTRY_SIZE];
    uint32_t *tscbh_am_table_entry;
    uint32_t *tscbh_um_table_entry;
    int index;
    ...
    switch (type) {
                ...
                */
                PHYMOD_IF_ERR_RETURN
                    (phymod_mem_write(&phy_copy.access, phymodMemAMTable, index,
                                      (tscbh_am_table_entry + index * TSCBH_AM_ENTRY_SIZE)));
                *is_handled = 1;
            }
            break;
            ...
            if (intr_status.is_2b_err || intr_status.is_1b_err) {
                ...
                PHYMOD_IF_ERR_RETURN
                    (phymod_mem_write(&phy_copy.access, phymodMemUMTable, index,
                                      (tscbh_um_table_entry + index * TSCBH_UM_ENTRY_SIZE)));
                *is_handled = 1;
            }
        ...
        case phymodIntrEccRx1588400g:
            if (intr_status.is_1b_err) {
                ...
                PHYMOD_IF_ERR_RETURN
                    (phymod_mem_read(&phy_copy.access, phymodMemRxLkup1588400G, index, ts_table_entry));
                PHYMOD_IF_ERR_RETURN
                    (phymod_mem_write(&phy_copy.access, phymodMemRxLkup1588400G, index, ts_table_entry));
            }
            break;
        ...
        case phymodIntrEccTx1588400g:
            if (intr_status.is_1b_err) {
                ...
                PHYMOD_IF_ERR_RETURN
                    (phymod_mem_read(&phy_copy.access, phymodMemTxLkup1588400G, index, ts_table_entry));
                PHYMOD_IF_ERR_RETURN
                    (phymod_mem_write(&phy_copy.access, phymodMemTxLkup1588400G, index, ts_table_entry));
            }
            break;
    ...
    if (!(*is_handled)) {
        ...
        switch (type) {
            ...
            case phymodIntrEccRsFECRbufMpp0:
                PHYMOD_IF_ERR_RETURN
                    (tbhmod_ecc_error_intr_enable_set(&phy_copy.access, type, 0));
                break;
            default:
                break;
...
int tscbh_phy_load_speed_id_entry(const phymod_phy_access_t* phy, uint32_t speed, uint32_t num_lane, phymod_fec_type_t fec_type)
{
    ...
    if ((tvco_pll_div == TBHMOD_PLL_MODE_DIV_170) || (tvco_pll_div == TBHMOD_PLL_MODE_DIV_85))  {
        /* then load 26G TVCO speed id entry */
        PHYMOD_IF_ERR_RETURN
            (phymod_mem_write(&phy_copy.access, phymodMemSpeedIdTable, TSCBH_FORCED_SPEED_ID_OFFSET + start_lane,
                               (tscbh_spd_id_entry_26 + mapped_speed_id * TSCBH_SPEED_ID_ENTRY_SIZE) ));
...
int tscbh_phy_fec_override_set(const phymod_phy_access_t* phy, uint32_t enable)
{
    ...
    uint32_t* tscbh_spd_id_entry_25;
    uint32_t* tscbh_spd_id_entry_26;
    uint32_t* tscbh_spd_id_entry_100g_4lane_no_fec_25;
    uint32_t* tscbh_spd_id_entry_100g_4lane_no_fec_26;

    tscbh_spd_id_entry_25 = tscbh_spd_id_entry_25_get();
    tscbh_spd_id_entry_26 = tscbh_spd_id_entry_26_get();
    tscbh_spd_id_entry_100g_4lane_no_fec_25 = tscbh_spd_id_entry_100g_4lane_no_fec_25_get();
    tscbh_spd_id_entry_100g_4lane_no_fec_26 = tscbh_spd_id_entry_100g_4lane_no_fec_26_get();
        ...
        if (enable) {
            PHYMOD_IF_ERR_RETURN
                (phymod_mem_write(&phy_copy.access, phymodMemSpeedIdTable,
                                  ...
                                  tscbh_spd_id_entry_100g_4lane_no_fec_26));
            PHYMOD_IF_ERR_RETURN
                (phymod_mem_write(&phy_copy.access, phymodMemSpeedIdTable,
            ...
            PHYMOD_IF_ERR_RETURN
                (phymod_mem_write(&phy_copy.access, phymodMemSpeedIdTable,
                                  SPEED_ID_INDEX_100G_4_LANE_CL73_KR4,
                                  (tscbh_spd_id_entry_26 + SPEED_ID_INDEX_100G_4_LANE_CL73_KR4 * TSCBH_SPEED_ID_ENTRY_SIZE)));
            PHYMOD_IF_ERR_RETURN
                (phymod_mem_write(&phy_copy.access, phymodMemSpeedIdTable,
                                  SPEED_ID_INDEX_100G_4_LANE_CL73_CR4,
                                  (tscbh_spd_id_entry_26 + SPEED_ID_INDEX_100G_4_LANE_CL73_CR4 * TSCBH_SPEED_ID_ENTRY_SIZE)));
        }
    } else if ((tvco_pll_div == TBHMOD_PLL_MODE_DIV_165) || (tvco_pll_div == TBHMOD_PLL_MODE_DIV_82P5)) {
        if (enable) {
            PHYMOD_IF_ERR_RETURN
                (phymod_mem_write(&phy_copy.access, phymodMemSpeedIdTable,
                                  SPEED_ID_INDEX_100G_4_LANE_CL73_KR4,
                                  tscbh_spd_id_entry_100g_4lane_no_fec_25));
            PHYMOD_IF_ERR_RETURN
                (phymod_mem_write(&phy_copy.access, phymodMemSpeedIdTable,
                                  SPEED_ID_INDEX_100G_4_LANE_CL73_CR4,
                                  tscbh_spd_id_entry_100g_4lane_no_fec_25));
            ...
            PHYMOD_IF_ERR_RETURN
                (phymod_mem_write(&phy_copy.access, phymodMemSpeedIdTable,
                                  SPEED_ID_INDEX_100G_4_LANE_CL73_KR4,
                                  (tscbh_spd_id_entry_25 + SPEED_ID_INDEX_100G_4_LANE_CL73_KR4 * TSCBH_SPEED_ID_ENTRY_SIZE)));
            PHYMOD_IF_ERR_RETURN
                (phymod_mem_write(&phy_copy.access, phymodMemSpeedIdTable,
                                  SPEED_ID_INDEX_100G_4_LANE_CL73_CR4,
                                  (tscbh_spd_id_entry_25 + SPEED_ID_INDEX_100G_4_LANE_CL73_CR4 * TSCBH_SPEED_ID_ENTRY_SIZE)));
        }
    }
    return PHYMOD_E_NONE;
}
...
STATIC
int _tscbh_phy_rx_am_lock_latched_low_get (const phymod_phy_access_t* phy, uint32_t* am_lock_ll)
{
    int start_lane, num_lane, i;
    int speed_id;
    phymod_phy_access_t pm_phy_copy;
    ...
    *am_lock_ll = 0;
    ...
    PHYMOD_IF_ERR_RETURN
        (tbhmod_speed_id_get(&pm_phy_copy.access, &speed_id));

    PHYMOD_IF_ERR_RETURN
        (phymod_mem_read(&pm_phy_copy.access, phymodMemSpeedIdTable, speed_id, packed_entry));
    spd_ctrl_unpack_spd_id_tbl_entry(packed_entry, &speed_config_entry);
    ...
    for (i = 0; i < num_lane; i++) {
        ...
        PHYMOD_IF_ERR_RETURN
            (tbhmod_am_lock_latched_low_get(&pm_phy_copy.access, speed_config_entry.bit_mux_mode, &tmp));
        *am_lock_ll |= (uint32_t) tmp;
    }
...
int tscbh_phy_local_fault_info_get (const phymod_phy_access_t* phy, 
                                    phymod_phy_local_fault_info_t *local_fault_info)
{
    phymod_phy_access_t pm_phy_copy;

    PHYMOD_MEMCPY(&pm_phy_copy, phy, sizeof(pm_phy_copy));
    PHYMOD_MEMSET(local_fault_info, 0, sizeof(*local_fault_info));

    PHYMOD_IF_ERR_RETURN
        (tbhmod_get_pcs_latched_status_local_fault_reasons(&pm_phy_copy.access, 
                                                           &(local_fault_info->pcs_link_status_live),
                                                           &(local_fault_info->pcs_latched_local_fault),
                                                           &(local_fault_info->pcs_latched_deskew_low)));

    PHYMOD_IF_ERR_RETURN
        (blackhawk_rx_pmd_lock_status_get(&pm_phy_copy.access,
                                          &(local_fault_info->pmd_rx_locked),
                                          &(local_fault_info->pmd_rx_lock_change)));

    PHYMOD_IF_ERR_RETURN
        (_tscbh_phy_rx_am_lock_latched_low_get(phy, &(local_fault_info->am_lock_latched_low)));

    PHYMOD_IF_ERR_RETURN
        (tbhmod_phy_fec_align_latched_low_get(&pm_phy_copy.access, 
                                              &(local_fault_info->fec_align_latched_low)));

    return PHYMOD_E_NONE;
}

/*The unit of period is ms*/
int tscbh_phy_an_timer_set(const phymod_phy_access_t* phy, phymod_an_timer_t timer_type, uint32_t period)
{
    ...
    switch (timer_type) {
        case phymodAnTimerLinkFailInhibitLtPam4:
            ...
            PHYMOD_IF_ERR_RETURN
                (tbhmod_an_link_fail_inhibit_timer_set(&phy_copy.access,
                                                       TBHMOD_AN_FAIL_INHIBIT_TIMER_LT_PAM4,
                                                       period));
...
int tscbh_phy_an_timer_get(const phymod_phy_access_t* phy, phymod_an_timer_t timer_type, uint32_t* period)
{
    phymod_core_access_t  phy_copy;
    ...
    switch (timer_type) {
        case phymodAnTimerLinkFailInhibitLtPam4:
            PHYMOD_IF_ERR_RETURN
                (tbhmod_an_link_fail_inhibit_timer_get(&phy_copy.access,
                                                       TBHMOD_AN_FAIL_INHIBIT_TIMER_LT_PAM4,
                                                       period));
            break;
        default:
...
STATIC
int _tscbh_flr_timeout_phymod_to_blackhawk(phymod_phy_flr_timeout_t timeout_time, enum blackhawk_tsc_flr_timeout_enum *blackhawk_timeout_time)
{
    switch (timeout_time) {
    case phymodFlrTimeout50ms:
        *blackhawk_timeout_time = BLACKHAWK_TSC_FLR_TIMEOUT_50MS;
        break;
    case phymodFlrTimeout75ms:
        *blackhawk_timeout_time = BLACKHAWK_TSC_FLR_TIMEOUT_75MS;
        break;
    case phymodFlrTimeout100ms:
        *blackhawk_timeout_time = BLACKHAWK_TSC_FLR_TIMEOUT_100MS;
        break;
    case phymodFlrTimeout150ms:
        *blackhawk_timeout_time = BLACKHAWK_TSC_FLR_TIMEOUT_150MS;
        break;
    case phymodFlrTimeout250ms:
        *blackhawk_timeout_time = BLACKHAWK_TSC_FLR_TIMEOUT_250MS;
        break;
    case phymodFlrTimeout500ms:
        *blackhawk_timeout_time = BLACKHAWK_TSC_FLR_TIMEOUT_500MS;
        break;
    default:
        PHYMOD_RETURN_WITH_ERR(PHYMOD_E_PARAM, (_PHYMOD_MSG("unsupported timeout time for blackhawk %u"), timeout_time));
        return PHYMOD_E_PARAM;
    }
    return PHYMOD_E_NONE;
}

int tscbh_phy_flr_config_set(const phymod_phy_access_t* phy, uint32_t enable, phymod_phy_flr_timeout_t timeout_time)
{
    int start_lane, num_lane, i;
    phymod_phy_access_t phy_copy;
    uint8_t flr_support = 0;
    uint32_t pcs_enable, lane_reset;
    enum blackhawk_tsc_flr_timeout_enum blackhawk_timeout_time;
    ...
    PHYMOD_IF_ERR_RETURN
        (blackhawk_tsc_INTERNAL_is_flr_supported(&phy_copy.access, &flr_support));
    if (!flr_support) {
        PHYMOD_DEBUG_ERROR(("devad 0x%"PRIx32": FLR unsupport\n", phy_copy.access.addr));
        return PHYMOD_E_INTERNAL;
    }

    if (enable) {
        PHYMOD_IF_ERR_RETURN
            (_tscbh_flr_timeout_phymod_to_blackhawk(timeout_time, &blackhawk_timeout_time));
    } else {
        if (timeout_time == phymodFlrTimeoutInvaild) {
            blackhawk_timeout_time = BLACKHAWK_TSC_FLR_TIMEOUT_50MS;
        } else {
            PHYMOD_IF_ERR_RETURN
                (_tscbh_flr_timeout_phymod_to_blackhawk(timeout_time, &blackhawk_timeout_time));
    ...
    for (i = 0; i < num_lane; i++) {
        ...
        PHYMOD_IF_ERR_RETURN
            (blackhawk_tsc_configure_fast_link_recovery(&phy_copy.access, enable,
                                                        enable, blackhawk_timeout_time));
    }
...
int tscbh_phy_flr_config_get(const phymod_phy_access_t* phy, uint32_t *enable, phymod_phy_flr_timeout_t *timeout_time)
{
    return PHYMOD_E_UNAVAIL;
}

STATIC
int _tscbh_speed_table_entry_to_nums_fec_lane_get(spd_id_tbl_entry_t* speed_config_entry,
                                                  int* nums_fec_lane)
{
    switch (speed_config_entry->bit_mux_mode) {
        case 0:
            *nums_fec_lane = 1;
            break;
        case 1:
        ...
        case 3:
            *nums_fec_lane = 5;
            break;
        default:
            PHYMOD_DEBUG_ERROR(("Unsupported bit mux mode\n"));
            return PHYMOD_E_UNAVAIL;
    }
...
int tscbh_phy_am_loss_align_multi_state_get(const phymod_phy_access_t* phy, uint32_t* am_loss_align_state)
{
    int start_lane, num_lane, speed_id, i, fec_lane;
    int nums_fec_lane = 0;
    uint32_t packed_entry[20] = {0};
    uint32_t fec_align_live_sts = 0, rsfec_amps_lock_sts = 0;
    spd_id_tbl_entry_t speed_config_entry;
    phymod_fec_type_t fec_type;
    phymod_phy_access_t phy_copy;
    tbhmod_rx_rsfec_sync_status_t sync_status;

    PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));
    PHYMOD_IF_ERR_RETURN
        (phymod_util_lane_config_get(&phy->access, &start_lane, &num_lane));
    PHYMOD_MEMSET(&sync_status, 0x0, sizeof(tbhmod_rx_rsfec_sync_status_t));

    /* Get speed id from resolved status */
    PHYMOD_IF_ERR_RETURN
        (tbhmod_speed_id_get(&phy_copy.access, &speed_id));

    /* Read the speed entry  then decode the speed and FEC type */
    phy_copy.access.lane_mask = 1 << 0;
    ...
    /* Just return when port speed is no_fec. */
    PHYMOD_IF_ERR_RETURN
        (tbhmod_fec_arch_decode_get(speed_config_entry.fec_arch, &fec_type));
    if (fec_type == phymod_fec_None) {
        *am_loss_align_state = 0;
        return PHYMOD_E_NONE;
    }

    PHYMOD_IF_ERR_RETURN
        (_tscbh_speed_table_entry_to_nums_fec_lane_get(&speed_config_entry, &nums_fec_lane));

    phy_copy.access.lane_mask = 1 << start_lane;
    PHYMOD_IF_ERR_RETURN
        (tbhmod_rs_fec_align_live_state_get(&phy_copy.access, &fec_align_live_sts));

    if (fec_align_live_sts) {
        *am_loss_align_state = 0;
        return PHYMOD_E_NONE;
    }
    ...
    for (i = 0; i < num_lane; i++) {
        ...
        phy_copy.access.lane_mask = 1 << (start_lane + i);
        PHYMOD_IF_ERR_RETURN
            (tbhmod_rs_fec_amps_lock_state_get(&phy_copy.access, nums_fec_lane, &sync_status));
        for (fec_lane = 0; fec_lane < nums_fec_lane; fec_lane++) {
            if (fec_lane == 0) {
                rsfec_amps_lock_sts = sync_status.fec_amps_lock_live_0;
            } else {
                rsfec_amps_lock_sts &= sync_status.fec_amps_lock_live_1;
            }
        }
        if (!rsfec_amps_lock_sts) {
            break;
        }
    }
    if (!rsfec_amps_lock_sts) {
