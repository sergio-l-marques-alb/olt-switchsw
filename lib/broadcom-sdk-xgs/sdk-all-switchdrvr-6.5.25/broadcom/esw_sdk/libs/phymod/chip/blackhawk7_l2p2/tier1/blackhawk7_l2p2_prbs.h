...
#ifndef BLACKHAWK7_L2P2_API_PRBS_H
#define BLACKHAWK7_L2P2_API_PRBS_H
 ...
 * NOTE: blackhawk7_l2p2_tx_shared_patt_gen_en() API should be called to enable the Pattern generator for that particular lane. \n\n
 *
 ...
 * @b Example
 * @code
    EFUN(blackhawk7_l2p2_config_shared_tx_pattern(sa__, 4, "0101"));
    EFUN(blackhawk7_l2p2_tx_shared_patt_gen(en(sa__, 1, 4));
 * @endcode
 ...
 * Note: The patt_length_bits input to the function should be the value sent to the blackhawk7_l2p2_config_shared_tx_pattern() function
 * @param[in] sa__ is an opaque state vector passed through to device access functions.
 * @param[in] enable Enable shared fixed pattern generator (1 = Enable; 0 = Disable)
 * @param[in] patt_length_bits length of the pattern in bits used in blackhawk7_l2p2_config_shared_tx_pattern()
 ...
 * Once the PRBS generator is configured, to enable PRBS use the blackhawk7_l2p2_tx_prbs_en() API.
 * @param[in] sa__ is an opaque state vector passed through to device access functions.
 ...
 * Once the PRBS checker is configured, use the blackhawk7_l2p2_rx_prbs_en() API to enable the checker.
 * @param[in] sa__ is an opaque state vector passed through to device access functions.
...
struct blackhawk7_l2p2_prbs_chk_hw_timer_ctrl_st {
 ...
 *       To get the BER data, user needs to call blackhawk7_l2p2_get_BER_data_HW_mode() for ber result.
 *       The time between this function call and call to the blackhawk7_l2p2_get_BER_data_HW_mode() can be time_ms long.
 * @param[in] sa__ is an opaque state vector passed through to device access functions.
 ...
 * Note: This function call may take up to time_ms delay to complete after calling the blackhawk7_l2p2_config_BER_HW_mode().
 * @param[in] sa__ is an opaque state vector passed through to device access functions.
...
extern const char* blackhawk7_l2p2_e2s_prbs_mode_enum[22];

/*---------------------------------*/
/*  PRBS Error Analyzer Functions  */
/*---------------------------------*/

#define BLACKHAWK7_L2P2_PRBS_FEC_FRAME_SIZE_MULTIPLIER         320
#define BLACKHAWK7_L2P2_PRBS_FEC_FRAME_SIZE_ROUND_UP           240  /* 3/4th of difference b/w configurable fec_size's (320) */
#define BLACKHAWK7_L2P2_PRBS_ERR_ANALYZER_FEC_SIZE_MIN         ( 6 * BLACKHAWK7_L2P2_PRBS_FEC_FRAME_SIZE_MULTIPLIER)
#define BLACKHAWK7_L2P2_PRBS_ERR_ANALYZER_FEC_SIZE_MAX         (63 * BLACKHAWK7_L2P2_PRBS_FEC_FRAME_SIZE_MULTIPLIER)
#define BLACKHAWK7_L2P2_PRBS_ERR_ANALYZER_ERR_THRESH_MIN        1
#define BLACKHAWK7_L2P2_PRBS_ERR_ANALYZER_ERR_THRESH_MAX       15
#define BLACKHAWK7_L2P2_PRBS_ERR_ANALYZER_HIST_ERR_THRESH_MIN   1
#define BLACKHAWK7_L2P2_PRBS_ERR_ANALYZER_HIST_ERR_THRESH_MAX   8
#define BLACKHAWK7_L2P2_PRBS_ERR_ANALYZER_FRAME_OVERLAP_FACTOR  8
#define BLACKHAWK7_L2P2_PRBS_ERR_ANALYZER_LINEARITY_ADJUST      1.0
#define BLACKHAWK7_L2P2_PRBS_ERR_ANALYZER_NUM_COUNTERS          8

#define BLACKHAWK7_L2P2_PRBS_FEC_SIZE_FRAC_1_1                  0
#define BLACKHAWK7_L2P2_PRBS_FEC_SIZE_FRAC_1_8                  1
#define BLACKHAWK7_L2P2_PRBS_FEC_SIZE_FRAC_1_4                  2
#define BLACKHAWK7_L2P2_PRBS_FEC_SIZE_FRAC_1_2                  3
#define BLACKHAWK7_L2P2_PRBS_NUM_OF_ERROR_ANALYZER_COUNTERS    16
...
typedef struct {
  ...
  uint8_t diag_mode;
  /** prbs_errcnt[i] - Num of FEC frames with >i errors (i = 0 to BLACKHAWK7_L2P2_PRBS_NUM_OF_ERROR_ANALYZER_COUNTERS-1) */
  uint32_t prbs_errcnt[BLACKHAWK7_L2P2_PRBS_NUM_OF_ERROR_ANALYZER_COUNTERS];
...
} blackhawk7_l2p2_prbs_err_analyzer_config_status_st;
...
typedef struct {
  ...
  uint8_t delta_n;
} blackhawk7_l2p2_prbs_err_analyzer_report_st;
...
/** Extrapolate PRBS Error Analyzer error count.
 * @param[in] sa__ is an opaque state vector passed through to device access functions.
 * @param[in] err_analyzer pointer to err_analyzer PRBS Error Analyzer Error Count & Config Settings.
 * @param[in] error_count_index PRBS Error Analyzer Error Counter index.
 * @return Error Code generated by API (returns ERR_CODE_NONE if no errors).
 */
err_code_t blackhawk7_l2p2_prbs_error_analyzer_extrapolate_err_count(srds_access_t *sa__, blackhawk7_l2p2_prbs_err_analyzer_config_status_st *err_analyzer, uint8_t error_count_index);
