/***********************************************************************************
 ***********************************************************************************
 *  File Name     :  merlin_sesto_functions.c                                        *
 *  Created On    :  29/04/2013                                                    *
 *  Created By    :  Kiran Divakar                                                 *
 *  Description   :  APIs for Serdes IPs                                           *
 *  Revision      :  $Id: merlin_sesto_functions.c 869 2015-01-23 00:24:44Z kirand $ *
 *                                                                                 *
...
#include "merlin_sesto_functions.h"
#include "merlin_sesto_dependencies.h"

#include "merlin_sesto_field_access.c"
#include "merlin_sesto_internal.c"
#include "merlin_sesto_dv_functions.c"
...
err_code_t merlin_sesto_display_eye_scan(const phymod_access_t *pa ) {
  ...
  err_code = merlin_sesto_meas_eye_scan_start(pa, EYE_SCAN_HORIZ);
  if (err_code) {
    merlin_sesto_meas_eye_scan_done( pa );
    return (err_code);
  }

  merlin_sesto_display_eye_scan_header(pa, 1);
  /* display stripe */
   for (y = 31;y>=-31;y=y-1) { 
 	err_code = merlin_sesto_read_eye_scan_stripe(pa, &stripe[0], &status);
	if (err_code) {
		merlin_sesto_meas_eye_scan_done( pa );
		return (err_code);
	}

    merlin_sesto_display_eye_scan_stripe(pa, y,&stripe[0]);
	USR_PRINTF(("\n"));  
  }  
  /* stop acquisition */
  err_code = merlin_sesto_meas_eye_scan_done( pa );
  if (err_code) {
	return (err_code);
  }

  merlin_sesto_display_eye_scan_footer(pa, 1);
...
err_code_t merlin_sesto_meas_lowber_eye(const phymod_access_t *pa, const struct merlin_sesto_eyescan_options_st eyescan_options, uint32_t *buffer) {
  ...
  for (y = eyescan_options.vert_max;y>=eyescan_options.vert_min;y=y-eyescan_options.vstep) {   
	for (x=eyescan_options.horz_min;x<=eyescan_options.horz_max;x=x+eyescan_options.hstep) {
	  /* acquire sample */
        EFUN(merlin_sesto_pmd_uc_cmd_with_data(pa, CMD_DIAG_EN,CMD_UC_DIAG_GET_EYE_SAMPLE,((uint16_t)x)<<8 | (uint8_t)y,200));
        /* wait for sample complete */
#if 0
        do {
            merlin_sesto_delay_us(1000);
            ESTM(status=rdv_usr_diag_status());
            USR_PRINTF(("status=%04x\n",status));
        } while((status & 0x8000) == 0); 
#else
        EFUN(merlin_sesto_poll_diag_done(pa, &status, (((uint32_t)timeout)<<7)*10 + 20000));
#endif
        {
            ...
            if(osr_mode.tx_rx == MERLIN_SESTO_OSX1) {
                EFUN(merlin_sesto_prbs_err_count_ll(pa, &errors));
            } else if(osr_mode.tx_rx == MERLIN_SESTO_OSX2) {
  ...
  USR_PRINTF(("\n"));
  EFUN(merlin_sesto_meas_eye_scan_done( pa ));
...
err_code_t merlin_sesto_meas_eye_scan_start(const phymod_access_t *pa, uint8_t direction) {
  ...
  if(direction == EYE_SCAN_VERTICAL) {
	err_code = merlin_sesto_pmd_uc_diag_cmd(pa, CMD_UC_DIAG_START_VSCAN_EYE,200);
  } else {
	  err_code = merlin_sesto_pmd_uc_diag_cmd(pa, CMD_UC_DIAG_START_HSCAN_EYE,200);
  }
  	if (err_code) {
		return (err_code);
	}
...
err_code_t merlin_sesto_read_eye_scan_stripe(const phymod_access_t *pa, uint32_t *buffer,uint16_t *status){
	...
	*status = 0;
	for(i=0;i<32;i++) {
		err_code = merlin_sesto_poll_diag_eye_data(pa,&val[0], &sts, 200);
 		*status |= sts & 0xF000;
		if (err_code) {
			return (err_code);
		}
...
err_code_t merlin_sesto_meas_eye_density_data(const phymod_access_t *pa, const struct merlin_sesto_eyescan_options_st eyescan_options, int32_t *buffer,uint16_t *buffer_size) {
  ...
  err_code = merlin_sesto_pmd_uc_diag_cmd(pa, CMD_UC_DIAG_DENSITY,2000);
  if (err_code) {
    return (err_code);
  }
  i = 0;hzcnt=0;
  ...
  for (y = eyescan_options.vert_max;y>=eyescan_options.vert_min;y=y-eyescan_options.vstep) {   
	...
	for (x=eyescan_options.horz_min;x<=eyescan_options.horz_max;x=x+eyescan_options.hstep) {
         ...
         err_code = merlin_sesto_poll_dsc_state_equals_uc_tune(pa, 2000);
	     if (err_code) 
           return(err_code);
	...
	merlin_sesto_display_eye_scan_header(pa, 1);
        for (y = eyescan_options.vert_max-1;y>=eyescan_options.vert_min;y=y-eyescan_options.vstep) {
          USR_PRINTF(("%6dmV : ",(_ladder_setting_to_mV(pa,y, range) + _ladder_setting_to_mV(pa,y+1, range))/2));    
           ...
           for (x=eyescan_options.horz_min;x<=eyescan_options.horz_max;x=x+eyescan_options.hstep) {
            /* Normalize peak to 15 */
            if (maxval) {
               val = buffer[j] * 16/maxval;
            } else {
               val = 0;
            }
            if (val>15) val = 15;
...
/*eye_scan_status_t merlin_sesto_read_eye_scan_status() */
...
err_code_t merlin_sesto_meas_eye_scan_done(const phymod_access_t *pa ) {
  err_code_t err_code;
  err_code = merlin_sesto_pmd_uc_diag_cmd(pa, CMD_UC_DIAG_DISABLE,200);
...
err_code_t merlin_sesto_start_ber_scan_test(const phymod_access_t *pa, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    ...
    EFUN(merlin_sesto_pmd_uc_cmd(pa, CMD_CAPTURE_BER_START, ber_scan_mode,500));
    return(ERR_CODE_NONE);
}
 ...
 ESTM(sts = rdv_usr_diag_status());
	...
	*cnt = (sts & 0x00FF)/3;
	for(i=0;i < *cnt;i++) {
		...
		err_code = merlin_sesto_pmd_uc_cmd(pa,  CMD_READ_DIAG_DATA_WORD, 0, timeout);
		if(err_code) return(err_code);
		...
		err_code = merlin_sesto_pmd_uc_cmd(pa,  CMD_READ_DIAG_DATA_BYTE, 0, timeout);
		if(err_code) return(err_code);
...
err_code_t merlin_sesto_eye_margin_proj(const phymod_access_t *pa, USR_DOUBLE rate, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
	...
	EFUN(merlin_sesto_start_ber_scan_test(pa, ber_scan_mode, timer_control, max_error_control));
	...
	if(verbose > 5) {
		do {
			EFUN(merlin_sesto_delay_us(2000000));
            ESTM(sts = rdv_usr_diag_status());
		} while ((sts & 0x8000) == 0);
	} 
    /*
    ...
    /* coverity[dead_error_begin] */
        ...
        EFUN(merlin_sesto_poll_diag_done(pa, &sts,timer_control*2000));
	}
	if(verbose > 2) USR_PRINTF(("delay done\n"));

	EFUN(merlin_sesto_read_ber_scan_data(pa,  &errs[0], &time[0], &cnt, 2000));
	
	if(verbose > 2) USR_PRINTF(("read done cnt=%d\n",cnt));

	EFUN(merlin_sesto_pmd_uc_cmd(pa, CMD_CAPTURE_BER_END,0x00,50));
	...
	/*} */
   	...
   	EFUN(merlin_sesto_display_ber_scan_data(pa, rate, ber_scan_mode, &errs[0], &time[0],(uint8_t)_abs(offset_start)));
...
err_code_t merlin_sesto_display_lane_state(const phymod_access_t *pa ) {
  err_code_t err_code;
  err_code = _merlin_sesto_display_lane_state_no_newline( pa );
  USR_PRINTF(("\n"));
  return (err_code);
}
...
err_code_t merlin_sesto_display_core_state_line(const phymod_access_t *pa ) {
  err_code_t err_code;
  err_code = _merlin_sesto_display_core_state_no_newline( pa ); 
  USR_PRINTF(("\n"));
  return (err_code);
}
...
err_code_t merlin_sesto_display_lane_config(const phymod_access_t *pa ) {
  ...
  err_code_t  err_code;
  struct merlin_sesto_uc_lane_config_st lane_cfg;

  USR_MEMSET(&lane_cfg, 0, sizeof(struct merlin_sesto_uc_lane_config_st));

  USR_PRINTF(("\n\n*************************************\n")); 
  USR_PRINTF(("**** SERDES LANE %d CONFIGURATION ****\n",merlin_sesto_get_lane(pa))); 
  USR_PRINTF(("*************************************\n\n")); 

  err_code = merlin_sesto_get_uc_lane_cfg(pa, &lane_cfg);
  if (err_code) {
    return (err_code);
  }
...
err_code_t merlin_sesto_display_core_state(const phymod_access_t *pa ) {
  ...
  ESTM(bin = rdcv_avg_tmon_reg13bit());temp = _bin_to_degC((bin>>3));
...
err_code_t merlin_sesto_display_lane_debug_status(const phymod_access_t *pa ) {
  ...
  /* startup */
  struct merlin_sesto_usr_ctrl_disable_functions_st ds; 
  struct merlin_sesto_usr_ctrl_disable_dfe_functions_st dsd;

  /* steady state */
  struct merlin_sesto_usr_ctrl_disable_functions_st dss;
  struct merlin_sesto_usr_ctrl_disable_dfe_functions_st dssd;

  USR_MEMSET(&ds, 0, sizeof(struct merlin_sesto_usr_ctrl_disable_functions_st));
  USR_MEMSET(&dsd, 0, sizeof(struct merlin_sesto_usr_ctrl_disable_dfe_functions_st));
  USR_MEMSET(&dss, 0, sizeof(struct merlin_sesto_usr_ctrl_disable_functions_st));
  USR_MEMSET(&dssd, 0, sizeof(struct merlin_sesto_usr_ctrl_disable_dfe_functions_st));
  ...
  err_code = merlin_sesto_get_usr_ctrl_disable_startup(pa, &ds);
  if (err_code) {
    return (err_code);
  }
  ...
  err_code = merlin_sesto_get_usr_ctrl_disable_startup_dfe(pa, &dsd);
  if (err_code) {
    return (err_code);
  }
  ...
  err_code = merlin_sesto_get_usr_ctrl_disable_steady_state(pa, &dss);
  if (err_code) {
    return (err_code);
  }
  ...
  err_code = merlin_sesto_get_usr_ctrl_disable_steady_state_dfe(pa, &dssd);
  if (err_code) {
    return (err_code);
  }
...
err_code_t merlin_sesto_stop_uc_lane(const phymod_access_t *pa, uint8_t enable) {
  ...
  if (enable) {
    return(merlin_sesto_pmd_uc_control(pa, CMD_UC_CTRL_STOP_GRACEFULLY,100));
  }
  else {
    return(merlin_sesto_pmd_uc_control(pa, CMD_UC_CTRL_RESUME,50));
  }
}
...
err_code_t merlin_sesto_request_stop_rx_adaptation(const phymod_access_t *pa ) {
  ...
  return(merlin_sesto_pmd_uc_control_return_immediate(pa, CMD_UC_CTRL_STOP_GRACEFULLY));
}
...
err_code_t merlin_sesto_pmd_uc_cmd_return_immediate(const phymod_access_t *pa, enum srds_pmd_uc_cmd_enum cmd, uint8_t supp_info) {
  ...
  err_code = merlin_sesto_poll_uc_dsc_ready_for_cmd_equals_1(pa, 1); /* Poll for uc_dsc_ready_for_cmd = 1 to indicate merlin_sesto ready for command */
  if (err_code) {
  ...
  EFUN(merlin_sesto_pmd_wr_reg(pa,DSC_A_DSC_UC_CTRL, cmddata));         /* This address is same for Eagle, and all Merlin */
...
err_code_t merlin_sesto_pmd_uc_cmd(const phymod_access_t *pa, enum srds_pmd_uc_cmd_enum cmd, uint8_t supp_info, uint32_t timeout_ms) {
  ...
  EFUN(merlin_sesto_pmd_uc_cmd_return_immediate(pa, cmd, supp_info));    /* Invoke merlin_sesto_uc_cmd and return control immediately */

  EFUN(merlin_sesto_poll_uc_dsc_ready_for_cmd_equals_1(pa, timeout_ms)); /* Poll for uc_dsc_ready_for_cmd = 1 to indicate merlin_sesto ready for command */
...
err_code_t merlin_sesto_pmd_uc_cmd_with_data(const phymod_access_t *pa, enum srds_pmd_uc_cmd_enum cmd, uint8_t supp_info, uint16_t data, uint32_t timeout_ms) {
  ...
  EFUN(merlin_sesto_pmd_uc_cmd_with_data_return_immediate(pa, cmd, supp_info, data)); /* Invoke merlin_sesto_uc_cmd and return control immediately */
...
err_code_t merlin_sesto_pmd_uc_control_return_immediate(const phymod_access_t *pa, enum srds_pmd_uc_ctrl_cmd_enum control) {
  return(merlin_sesto_pmd_uc_cmd_return_immediate(pa, CMD_UC_CTRL, (uint8_t) control));
}
...
err_code_t merlin_sesto_pmd_uc_control(const phymod_access_t *pa, enum srds_pmd_uc_ctrl_cmd_enum control, uint32_t timeout_ms) {
  return(merlin_sesto_pmd_uc_cmd(pa, CMD_UC_CTRL, (uint8_t) control, timeout_ms));
}
...
err_code_t merlin_sesto_pmd_uc_diag_cmd(const phymod_access_t *pa, enum srds_pmd_uc_diag_cmd_enum control, uint32_t timeout_ms) {
  return(merlin_sesto_pmd_uc_cmd(pa, CMD_DIAG_EN, (uint8_t) control, timeout_ms));
}
...
uint8_t merlin_sesto_rdbl_uc_var(const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr) {
	uint8_t rddata;
	...
	{
    merlin_sesto_uc_lane_info_st uc_lane_info;
    USR_MEMSET(&uc_lane_info, 0, sizeof(merlin_sesto_uc_lane_info_st));
    EFUN(merlin_sesto_get_uc_ln_info(pa, &uc_lane_info));
	if (uc_lane_info.is_direct_ram_access_avail) {
		EPFUN(merlin_sesto_rdb_ram(pa, &rddata, uc_lane_info.lane_var_base_address+addr, 1));    /* Direct RAM access */
	}
	else {
		...
		EPFUN(merlin_sesto_pmd_uc_cmd(pa, CMD_READ_UC_LANE_BYTE, (uint8_t)addr, 50));          /* Use Serdes FW DSC Command Interface for reading RAM */
...
int8_t merlin_sesto_rdbls_uc_var(const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr) {
  return ((int8_t) merlin_sesto_rdbl_uc_var(pa, err_code_p, addr));
} 

/* Micro RAM Lane Word Read */
uint16_t merlin_sesto_rdwl_uc_var(const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr) {
  uint16_t rddata;
  ...
  EFUN(merlin_sesto_get_uc_ln_info(pa, &uc_lane_info));
  if (uc_lane_info.is_direct_ram_access_avail) {
	  EFUN(merlin_sesto_rdw_ram(pa, &rddata, uc_lane_info.lane_var_base_address+addr, 1));                    /* Direct RAM access */
  }
  else {
	  if (addr >= uc_lane_info.lane_ram_size || addr > 255) {                       /* uC command only support 8-bit addr */                 
		  *err_code_p = ERR_CODE_INVALID_RAM_ADDR;                                  /* Error Code indicating Out of Range access */
	  ...
	  EPFUN(merlin_sesto_pmd_uc_cmd(pa, CMD_READ_UC_LANE_WORD, (uint8_t)addr, 50));                   /* Use Serdes FW DSC Command Interface for reading RAM */
...
int16_t merlin_sesto_rdwls_uc_var(const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr) {
  return ((int16_t) merlin_sesto_rdwl_uc_var(pa, err_code_p, addr));
}
...
err_code_t merlin_sesto_wrbl_uc_var(const phymod_access_t *pa, uint16_t addr, uint8_t wr_val) {
    ...
    EFUN(merlin_sesto_get_uc_ln_info(pa, &uc_lane_info));
	if (uc_lane_info.is_direct_ram_access_avail) {
		EFUN(merlin_sesto_wrb_ram(pa, uc_lane_info.lane_var_base_address+addr, 1, &wr_val));
		return (ERR_CODE_NONE);
	}
	else {
		if (addr >= uc_lane_info.lane_ram_size || addr > 255) {                      /* uC command only support 8-bit addr */                             
		...
		return (merlin_sesto_pmd_uc_cmd_with_data(pa, CMD_WRITE_UC_LANE_BYTE, (uint8_t)addr, (uint16_t)wr_val, 10)); /* Use Serdes FW DSC Command Interface for writing RAM */
	}
    }
}
...
err_code_t merlin_sesto_wrbls_uc_var(const phymod_access_t *pa, uint16_t addr, int8_t wr_val) {
  return (merlin_sesto_wrbl_uc_var(pa, addr, wr_val));
}
...
err_code_t merlin_sesto_wrwl_uc_var(const phymod_access_t *pa, uint16_t addr, uint16_t wr_val) {
    ...
    EFUN(merlin_sesto_get_uc_ln_info(pa, &uc_lane_info));
	if (uc_lane_info.is_direct_ram_access_avail) {
		EFUN(merlin_sesto_wrw_ram(pa, uc_lane_info.lane_var_base_address+addr, 1, &wr_val));
		return (ERR_CODE_NONE);
	}
	else {
		if (addr >= uc_lane_info.lane_ram_size || addr > 255 ) {                   /* uC command only support 8-bit addr */                  
		...
		return (merlin_sesto_pmd_uc_cmd_with_data(pa, CMD_WRITE_UC_LANE_WORD, (uint8_t)addr, (uint16_t)wr_val, 10)); /* Use Serdes FW DSC Command Interface for writing RAM */
	}
    }
}
...
err_code_t merlin_sesto_wrwls_uc_var(const phymod_access_t *pa, uint16_t addr, int16_t wr_val) {
  return (merlin_sesto_wrwl_uc_var(pa, addr,wr_val));
}
...
uint8_t merlin_sesto_rdbc_uc_var(const phymod_access_t *pa, err_code_t *err_code_p, uint8_t addr) {
  ...
  EFUN(merlin_sesto_get_uc_ln_info(pa, &uc_lane_info));
  if (uc_lane_info.is_direct_ram_access_avail) {
	  EPFUN(merlin_sesto_rdb_ram(pa, &rddata, uc_lane_info.core_var_base_address+(uint16_t)addr, 1));                    /* Direct RAM access */
  }
  else {
	  ...
	  EPFUN(merlin_sesto_pmd_uc_cmd(pa, CMD_READ_UC_CORE_BYTE, addr, 10));                   /* Use Serdes FW DSC Command Interface for reading RAM */
	  EPSTM(rddata = (uint8_t)rd_uc_dsc_data());                                            /* Read_Value read from uc_dsc_data field */
  }
  }
...
int8_t merlin_sesto_rdbcs_uc_var(const phymod_access_t *pa, err_code_t *err_code_p, uint8_t addr) {
  return ((int8_t) merlin_sesto_rdbc_uc_var(pa, err_code_p, addr));
}

/* Micro RAM Core Word Read */
uint16_t merlin_sesto_rdwc_uc_var(const phymod_access_t *pa, err_code_t *err_code_p, uint8_t addr) {
  ...
  EFUN(merlin_sesto_get_uc_ln_info(pa, &uc_lane_info));
  if (uc_lane_info.is_direct_ram_access_avail) {
	  EFUN(merlin_sesto_rdw_ram(pa, &rddata, uc_lane_info.core_var_base_address+(uint16_t)addr, 1));                    /* Direct RAM access */
  }
  else {
	  if (addr >= uc_lane_info.lane_ram_size) {         
		  *err_code_p = ERR_CODE_INVALID_RAM_ADDR;                                  /* Error Code indicating Out of Range access */
	  ...
	  EPFUN(merlin_sesto_pmd_uc_cmd(pa, CMD_READ_UC_CORE_WORD, addr, 10));                   /* Use Serdes FW DSC Command Interface for reading RAM */
...
int16_t merlin_sesto_rdwcs_uc_var(const phymod_access_t *pa, err_code_t *err_code_p, uint8_t addr) {
  return ((int16_t) merlin_sesto_rdwc_uc_var(pa, err_code_p, addr));
}
...
err_code_t merlin_sesto_wrbc_uc_var(const phymod_access_t *pa, uint8_t addr, uint8_t wr_val) {
    ...
    EFUN(merlin_sesto_get_uc_ln_info(pa, &uc_lane_info));
	if (uc_lane_info.is_direct_ram_access_avail) {
		EFUN(merlin_sesto_wrb_ram(pa, uc_lane_info.core_var_base_address+(uint16_t)addr, 1, &wr_val));
		return (ERR_CODE_NONE);
	}
	else {
		if (addr >= uc_lane_info.lane_ram_size) {                      
		...
		return (merlin_sesto_pmd_uc_cmd_with_data(pa, CMD_WRITE_UC_CORE_BYTE, addr, (uint16_t)wr_val, 10)); /* Use Serdes FW DSC Command Interface for writing RAM */
	}
    }
} 
...
err_code_t merlin_sesto_wrbcs_uc_var(const phymod_access_t *pa, uint8_t addr, int8_t wr_val) {
  return (merlin_sesto_wrbc_uc_var(pa, addr, wr_val));
}
...
err_code_t merlin_sesto_wrwc_uc_var(const phymod_access_t *pa, uint8_t addr, uint16_t wr_val) {
    ...
    EFUN(merlin_sesto_get_uc_ln_info(pa, &uc_lane_info));
	if (uc_lane_info.is_direct_ram_access_avail) {
		EFUN(merlin_sesto_wrw_ram(pa, uc_lane_info.core_var_base_address+addr, 1, &wr_val));
		return (ERR_CODE_NONE);
	}
	else {
		if (addr >= uc_lane_info.lane_ram_size) {                                     
		...
		return (merlin_sesto_pmd_uc_cmd_with_data(pa, CMD_WRITE_UC_CORE_WORD, addr, wr_val, 10)); /* Use Serdes FW DSC Command Interface for writing RAM */
	}
    }
}
...
err_code_t merlin_sesto_wrwcs_uc_var(const phymod_access_t *pa, uint8_t addr, int16_t wr_val) {
  return(merlin_sesto_wrwc_uc_var(pa, addr,wr_val));
}
...
err_code_t merlin_sesto_set_uc_core_config(const phymod_access_t *pa, struct merlin_sesto_uc_core_config_st struct_val) {
    ...
    if(reset_state < 7) {
        USR_PRINTF(("ERROR: merlin_sesto_set_uc_core_config(pa, ..) called without core_dp_s_rstb=0\n"));
...
err_code_t merlin_sesto_set_uc_lane_cfg(const phymod_access_t *pa, struct merlin_sesto_uc_lane_config_st struct_val) {
    ...
    if(reset_state < 7) {
        USR_PRINTF(("ERROR: merlin_sesto_set_uc_lane_cfg(pa, ..) called without ln_dp_s_rstb=0\n"));
...
err_code_t merlin_sesto_get_uc_core_config(const phymod_access_t *pa, struct merlin_sesto_uc_core_config_st *get_val) {
...
err_code_t merlin_sesto_get_uc_lane_cfg(const phymod_access_t *pa, struct merlin_sesto_uc_lane_config_st *get_val) { 
...
err_code_t merlin_sesto_poll_diag_done(const phymod_access_t *pa, uint16_t *status, uint32_t timeout_ms) {
 ...
 for(loop=0;loop < 100; loop++) {
	 ...
	 if(loop>10) {
		 EFUN(merlin_sesto_delay_us(10*timeout_ms));
	 }
 }
...
err_code_t merlin_sesto_poll_diag_eye_data(const phymod_access_t *pa,uint32_t *data,uint16_t *status, uint32_t timeout_ms) {
 ...
 for(loop=0;loop < 100; loop++) {
	 ...
	 if(((*status & 0x00FF) > 2) || ((*status & 0x8000) > 0)) {
		EFUN(merlin_sesto_pmd_uc_cmd(pa,  CMD_READ_DIAG_DATA_WORD, 0, 200));
...
err_code_t merlin_sesto_poll_uc_dsc_ready_for_cmd_equals_1(const phymod_access_t *pa, uint32_t timeout_ms) {
  ...
  for (loop = 0; loop < 100; loop++) {
    uint16_t rddata;
    EFUN(merlin_sesto_pmd_rdt_reg(pa,DSC_A_DSC_UC_CTRL, &rddata));
	...
	if(loop>10) {
		err_code = merlin_sesto_delay_us(10*timeout_ms);
		if(err_code) return(err_code);
	}
  }
...
#else
  { uint16_t rddata;
      USR_PRINTF(("// ATE_LOG merlin_sesto_poll_uc_dsc_ready_for_cmd_equals_1 begin\n"));
      if(timeout_ms < 100) {
          EFUN(merlin_sesto_delay_us(timeout_ms));
      } else {
          EFUN(merlin_sesto_delay_ms(timeout_ms));
      }
      EFUN(merlin_sesto_pmd_rdt_reg(pa,DSC_A_DSC_UC_CTRL, &rddata));
      if (rddata & 0x0080) {    /* bit 7 is uc_dsc_ready_for_cmd */
          if (rddata & 0x0040) {  /* bit 6 is uc_dsc_error_found   */
          ...
          USR_PRINTF(("// ATE_LOG merlin_sesto_poll_uc_dsc_ready_for_cmd_equals_1 end\n"));
          return (ERR_CODE_NONE);
      }     
      USR_PRINTF(("// ATE_LOG merlin_sesto_poll_uc_dsc_ready_for_cmd_equals_1 timeout\n"));
  }
#endif
...
err_code_t merlin_sesto_reg_dump(const phymod_access_t *pa ) {
  ...
  for (addr = 0x0; addr < 0xF; addr++) {
    EFUN(merlin_sesto_pmd_rdt_reg(pa,addr,&rddata));
...
err_code_t merlin_sesto_uc_core_var_dump(const phymod_access_t *pa ) {
  ...
  for (addr = 0x0; addr < 0xFF; addr++) {  
    rddata = merlin_sesto_rdbc_uc_var(pa, &err_code, addr);
    if (err_code) {
      return (err_code);
    }
...
err_code_t merlin_sesto_uc_lane_var_dump(const phymod_access_t *pa ) {
  ...
  uint16_t    addr;
  err_code_t  err_code = ERR_CODE_NONE;
  merlin_sesto_uc_lane_info_st lane_info;
  ...
  USR_PRINTF(("********************************************\n")); 
    ...
    err_code = merlin_sesto_get_uc_ln_info(pa, &lane_info);
    if (err_code) {
      return (err_code);
    }

    for (addr = 0x0; addr < lane_info.lane_ram_size; addr++) {  
      rddata = merlin_sesto_rdbl_uc_var(pa, &err_code, addr);
      if (err_code) {
        return (err_code);
      }
...
err_code_t merlin_sesto_read_event_log(const phymod_access_t *pa, uint8_t *trace_mem,enum srds_event_log_display_mode_enum display_mode) {
	...
	EFUN(merlin_sesto_event_log_stop( pa ));

	EFUN(merlin_sesto_event_log_readmem(pa, trace_mem));

	EFUN(merlin_sesto_event_log_display(pa, trace_mem, display_mode));
...
err_code_t merlin_sesto_event_log_stop(const phymod_access_t *pa ) {
	...
	EFUN(merlin_sesto_pmd_uc_cmd(pa, CMD_EVENT_LOG_READ, CMD_EVENT_LOG_READ_START, 10));
...
err_code_t merlin_sesto_event_log_readmem(const phymod_access_t *pa, uint8_t *trace_mem) {
	...
	do {
		/* Read Next */
		EFUN(merlin_sesto_pmd_uc_cmd(pa, CMD_EVENT_LOG_READ, CMD_EVENT_LOG_READ_NEXT, 10));
	...
	EFUN(merlin_sesto_pmd_uc_cmd(pa, CMD_EVENT_LOG_READ, CMD_EVENT_LOG_READ_DONE, 10));
		...
		while (addr < trace_mem_size-8) {                                                            /* last 8-byte reserved for trace memory wraparound */
			...
			if (*(trace_mem+addr) == 0xff) {                                                         /* timestamp wraparound event handling */
				...
				if (!is_last_event_timewrap) {                                                         /* display the rest of previous event info */
					USR_PRINTF((","));
					_merlin_display_event(pa, prev_event,(uint8_t)prev_rr,prev_cursor,curr_cursor,post_cursor,supp_info);
                ...
                */
                ...
                merlin_sesto_uc_lane_idx_to_system_id(pa, uc_lane_id_str, cc);
...
err_code_t merlin_sesto_display_state (const phymod_access_t *pa ) {

  err_code_t err_code;
  err_code = ERR_CODE_NONE;

  if (!err_code) err_code = merlin_sesto_display_core_state( pa );
  if (!err_code) err_code = merlin_sesto_display_lane_state_hdr( pa );
  if (!err_code) err_code = merlin_sesto_display_lane_state( pa );

  return (err_code);
}

err_code_t merlin_sesto_display_config (const phymod_access_t *pa ) {

  err_code_t err_code;
  err_code = ERR_CODE_NONE;

  if (!err_code) err_code = merlin_sesto_display_core_config( pa );
  if (!err_code) err_code = merlin_sesto_display_lane_config( pa );
...
err_code_t merlin_sesto_read_die_temperature (const phymod_access_t *pa, int16_t *die_temp) {
    ...
    EFUN(merlin_sesto_pmd_uc_cmd(pa, CMD_UC_DBG,CMD_UC_DBG_DIE_TEMP,50));
...
err_code_t merlin_sesto_read_die_temperature_double (const phymod_access_t *pa, USR_DOUBLE *die_temp) {
    ...
    *die_temp = 410.04-0.48705* ((USR_DOUBLE)die_temp_sensor_reading); 
    ...
    return(ERR_CODE_NONE);        
#else
    USR_PRINTF((" Function 'merlin_sesto_read_die_temperature_double' needs SERDES_API_FLOATING_POINT defined to operate \n"));
    return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
#endif
}
...
err_code_t _merlin_sesto_read_lane_state_define(const phymod_access_t *pa, merlin_sesto_lane_state_define_st *istate) {
  ...
  USR_MEMSET(&state, 0, sizeof(merlin_sesto_lane_state_define_st));
  ...
  USR_MEMSET(&osr_mode, 0, sizeof(merlin_sesto_osr_mode_st));
  EFUN(_merlin_sesto_get_osr_mode(pa, &osr_mode));
      state.osr_mode =  *(merlin_sesto_osr_mode_define_st*)&osr_mode;
  }
