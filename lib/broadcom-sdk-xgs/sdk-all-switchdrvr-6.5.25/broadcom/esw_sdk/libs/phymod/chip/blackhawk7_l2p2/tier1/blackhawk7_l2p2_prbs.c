...
/***********************************************************************************
 ***********************************************************************************
 *  File Name     :  blackhawk7_l2p2_prbs.c                                             *
...
#include "blackhawk7_l2p2_dependencies.h"
...
err_code_t blackhawk7_l2p2_config_shared_tx_pattern(srds_access_t *sa__, uint8_t patt_length_bits, const char pattern[]) {
    INIT_SRDS_ERR_CODE
  ...
  uint8_t    zero_pad_len    = 0; /* suppress warnings, changed by blackhawk7_l2p2_INTERNAL_calc_patt_gen_mode_sel() */
  uint16_t   patt_gen_wr_val = 0;
  uint8_t    mode_sel        = 0; /* suppress warnings, changed by blackhawk7_l2p2_INTERNAL_calc_patt_gen_mode_sel() */

  EFUN(blackhawk7_l2p2_INTERNAL_calc_patt_gen_mode_sel(sa__, &mode_sel,&zero_pad_len,patt_length_bits));
  ...
  if ((str_len > 2) && ((USR_STRNCMP(pattern, "0x", 2)) == 0)) {
    ...
    for (i=2; i < str_len; i++) {
      if (pattern[i] != '_') {
        ...
        if (actual_patt_len > 240) {
          ...
          return (blackhawk7_l2p2_error(sa__, ERR_CODE_CFG_PATT_PATTERN_BIGGER_THAN_MAXLEN));
        }
        EFUN(blackhawk7_l2p2_INTERNAL_compute_bin(sa__, pattern[i],bin, sizeof(bin)));
    ...
    if ((offset_len > 3)  || (actual_patt_len < patt_length_bits)) {
      ...
      return (blackhawk7_l2p2_error(sa__, ERR_CODE_CFG_PATT_LEN_MISMATCH));
    }
    else if (offset_len) {
    ...
    for (i=1; i < str_len; i++) {
      if (pattern[i] != '_') {
        ...
        EFUN(blackhawk7_l2p2_INTERNAL_pam4_to_bin(sa__, pattern[i], bin, sizeof(bin)));
        bytes_written = USR_SNPRINTF(patt_mod + mod_count, sizeof(patt_mod) - mod_count, "%s",  bin);
        mod_count += (size_t)bytes_written;
        if(( bytes_written < 0) || (mod_count >= sizeof(patt_mod))) {
          return ERR_CODE_REACHED_BUF_SIZE_LIMIT;
    ...
    for (i=0; i < str_len; i++) {
      if ((pattern[i] == '0') || (pattern[i] == '1')) {
        ...
        return (blackhawk7_l2p2_error(sa__, ERR_CODE_CFG_PATT_INVALID_PATTERN));
      }
    }
  ...
  for (i=0; i < 15; i++) {
    ...
    for (j=0; j < 4; j++) {
      ...
      bin[4] = '\0';
      EFUN(blackhawk7_l2p2_INTERNAL_compute_hex(sa__,bin, &hex));
    ...
    switch (i) {
      ...
      default: EFUN_PRINTF(("ERROR: Invalid write to patt_gen_seq register\n"));
               return (blackhawk7_l2p2_error(sa__, ERR_CODE_CFG_PATT_INVALID_SEQ_WRITE));
    }
  }
...
err_code_t blackhawk7_l2p2_tx_shared_patt_gen_en(srds_access_t *sa__, uint8_t enable, uint8_t patt_length_bits) {
    INIT_SRDS_ERR_CODE
  uint8_t zero_pad_len = 0; /* suppress warnings, changed by blackhawk7_l2p2_INTERNAL_calc_patt_gen_mode_sel() */
  uint8_t mode_sel     = 0; /* suppress warnings, changed by blackhawk7_l2p2_INTERNAL_calc_patt_gen_mode_sel() */
  ...
  if (enable) {
    if ((mode_sel < 1) || (mode_sel > 6)) {
      return (blackhawk7_l2p2_error(sa__, ERR_CODE_PATT_GEN_INVALID_MODE_SEL));
    }
...
err_code_t blackhawk7_l2p2_config_tx_jp03b_pattern(srds_access_t *sa__, uint8_t enable) {
    INIT_SRDS_ERR_CODE
...
err_code_t blackhawk7_l2p2_config_tx_linearity_pattern(srds_access_t *sa__, uint8_t enable) {
    INIT_SRDS_ERR_CODE
...
err_code_t blackhawk7_l2p2_config_tx_prbs(srds_access_t *sa__, enum srds_prbs_polynomial_enum prbs_poly_mode, uint8_t prbs_inv) {
    INIT_SRDS_ERR_CODE
...
err_code_t blackhawk7_l2p2_get_tx_prbs_config(srds_access_t *sa__, enum srds_prbs_polynomial_enum *prbs_poly_mode, uint8_t *prbs_inv) {
    INIT_SRDS_ERR_CODE
    uint8_t val;
...
err_code_t blackhawk7_l2p2_tx_prbs_en(srds_access_t *sa__, uint8_t enable) {
    INIT_SRDS_ERR_CODE
    if (enable) {
...
err_code_t blackhawk7_l2p2_get_tx_prbs_en(srds_access_t *sa__, uint8_t *enable) {
    INIT_SRDS_ERR_CODE
    if(!enable) {
...
err_code_t blackhawk7_l2p2_tx_prbs_err_inject(srds_access_t *sa__, uint8_t enable) {
    INIT_SRDS_ERR_CODE
  /* PRBS Error Insert.
...
err_code_t blackhawk7_l2p2_config_rx_prbs(srds_access_t *sa__, enum srds_prbs_polynomial_enum prbs_poly_mode, enum srds_prbs_checker_mode_enum prbs_checker_mode, uint8_t prbs_inv) {
    INIT_SRDS_ERR_CODE
  {
    uint8_t dig_lpbk = 0;
  if(prbs_poly_mode >= PCS_PRBS_7) {
...
/* get PRBS Checker */
err_code_t blackhawk7_l2p2_get_rx_prbs_config(srds_access_t *sa__, enum srds_prbs_polynomial_enum *prbs_poly_mode, enum srds_prbs_checker_mode_enum *prbs_checker_mode, uint8_t *prbs_inv) {
    INIT_SRDS_ERR_CODE
    uint8_t val;
...
err_code_t blackhawk7_l2p2_rx_prbs_en(srds_access_t *sa__, uint8_t enable) {
    INIT_SRDS_ERR_CODE
    if (enable) {
...
err_code_t blackhawk7_l2p2_get_rx_prbs_en(srds_access_t *sa__, uint8_t *enable) {
    INIT_SRDS_ERR_CODE
    if(!enable) {
        return(blackhawk7_l2p2_error(sa__, ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
    }
...
err_code_t blackhawk7_l2p2_prbs_chk_lock_state(srds_access_t *sa__, uint8_t *chk_lock) {
    INIT_SRDS_ERR_CODE
  if(!chk_lock) {
    return(blackhawk7_l2p2_error(sa__, ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
  }
...
err_code_t blackhawk7_l2p2_prbs_err_count_ll(srds_access_t *sa__, uint32_t *prbs_err_cnt) {
    INIT_SRDS_ERR_CODE
  uint16_t rddata;
...
err_code_t blackhawk7_l2p2_prbs_err_count_state(srds_access_t *sa__, uint32_t *prbs_err_cnt, uint8_t *lock_lost) {
    INIT_SRDS_ERR_CODE
  ...
  if(!prbs_err_cnt || !lock_lost) {
    ...
    EFUN(blackhawk7_l2p2_prbs_err_count_ll(sa__, prbs_err_cnt));
...
err_code_t blackhawk7_l2p2_get_prbs_chk_hw_timer_ctrl(srds_access_t *sa__, struct blackhawk7_l2p2_prbs_chk_hw_timer_ctrl_st * const prbs_chk_hw_timer_ctrl_bak) {
    INIT_SRDS_ERR_CODE
    ...
    BLACKHAWK7_L2P2_UAPI_INIT_V3     /* if BLACKHAWK7_V3L2P2 */
    ESTM(prbs_chk_hw_timer_ctrl_bak->prbs_chk_new_timer_mode = rd_prbs_chk_new_timer_mode());
    BLACKHAWK7_L2P2_UAPI_NOT_V1_OR_V2_ERR
    return ERR_CODE_NONE;
}

/* Restore prbs hardware timer config registers */
err_code_t blackhawk7_l2p2_set_prbs_chk_hw_timer_ctrl(srds_access_t *sa__, struct blackhawk7_l2p2_prbs_chk_hw_timer_ctrl_st const * const prbs_chk_hw_timer_ctrl_bak) {
    INIT_SRDS_ERR_CODE
...
err_code_t blackhawk7_l2p2_config_prbs_chk_hw_timer(srds_access_t *sa__, uint16_t time_ms, uint16_t *time_ms_adjusted) {
    INIT_SRDS_ERR_CODE
    struct blackhawk7_l2p2_prbs_chk_hw_timer_ctrl_st prbs_chk_hw_timer_ctrl;
    ENULL_MEMSET(&prbs_chk_hw_timer_ctrl, 0, sizeof(struct blackhawk7_l2p2_prbs_chk_hw_timer_ctrl_st));
    EFUN(wr_prbs_chk_burst_err_cnt_en(0));

    /* Figure out timer mode and timer count */
    EFUN(blackhawk7_l2p2_INTERNAL_get_prbs_timeout_count_from_time(time_ms, time_ms_adjusted, &prbs_chk_hw_timer_ctrl));
...
err_code_t blackhawk7_l2p2_display_detailed_prbs_state_hdr(srds_access_t *sa__) {
...
const char* blackhawk7_l2p2_e2s_prbs_mode_enum[22] = {
    " PRBS_7",
    " PRBS_9",
    "PRBS_11",
    "PRBS_15",
...
err_code_t blackhawk7_l2p2_display_detailed_prbs_state(srds_access_t *sa__) {
    INIT_SRDS_ERR_CODE
    uint32_t err_cnt = 0;
    uint8_t lock_lost = 0;
    uint8_t enabled;

    ESTM_PRINTF(("  %d ",blackhawk7_l2p2_acc_get_lane(sa__)));
    ...
    if(enabled) {
        ...
        EFUN(blackhawk7_l2p2_get_tx_prbs_config(sa__, &prbs_poly_mode, &prbs_inv));
        if(prbs_poly_mode == PRBS_UNKNOWN) {
            prbs_string = "UNKNOWN";
        }
        else {
            prbs_string =  (char *)blackhawk7_l2p2_e2s_prbs_mode_enum[prbs_poly_mode];
        }
    ...
    if(enabled) {
        ...
        uint8_t prbs_inv = 0;
        const char *prbs_string;
        EFUN(blackhawk7_l2p2_get_rx_prbs_config(sa__, &prbs_poly_mode, &prbs_checker_mode, &prbs_inv));
        if(prbs_poly_mode == PRBS_UNKNOWN) {
            prbs_string = "UNKNOWN";
        }
        else {
    ...
    EFUN(blackhawk7_l2p2_prbs_err_count_state(sa__,&err_cnt,&lock_lost));
    EFUN_PRINTF(("  %d  %010d ",lock_lost,err_cnt));
    EFUN(blackhawk7_l2p2_INTERNAL_display_BER(sa__,100));
    EFUN_PRINTF(("\n"));
...
err_code_t blackhawk7_l2p2_prbs_error_analyzer_reset(srds_access_t *sa__) {
    INIT_SRDS_ERR_CODE
    EFUN(wrc_tlb_err_analyze_en(0x0)); /* Disables error counting into the memory */
    EFUN(blackhawk7_l2p2_clear_dac_pattern_memory(sa__));  /* Clear DAC pattern memory */
    return (ERR_CODE_NONE);
}

/* Configure PRBS Error Analyzer */
err_code_t blackhawk7_l2p2_prbs_error_analyzer_config(srds_access_t *sa__, blackhawk7_l2p2_prbs_err_analyzer_config_status_st *err_analyzer) {
    INIT_SRDS_ERR_CODE
    uint8_t  fec_size;
    ...
    if ((err_analyzer->prbs_err_fec_size < BLACKHAWK7_L2P2_PRBS_ERR_ANALYZER_FEC_SIZE_MIN) || (err_analyzer->prbs_err_fec_size > BLACKHAWK7_L2P2_PRBS_ERR_ANALYZER_FEC_SIZE_MAX)) {
        return (blackhawk7_l2p2_error(sa__, ERR_CODE_INVALID_PRBS_ERR_ANALYZER_FEC_SIZE));
    }

    fec_size = (uint8_t)((err_analyzer->prbs_err_fec_size + BLACKHAWK7_L2P2_PRBS_FEC_FRAME_SIZE_ROUND_UP) / BLACKHAWK7_L2P2_PRBS_FEC_FRAME_SIZE_MULTIPLIER);

    fec_size_bits = (uint16_t)(fec_size * BLACKHAWK7_L2P2_PRBS_FEC_FRAME_SIZE_MULTIPLIER);
    ...
    switch (err_analyzer->prbs_err_aggregate_mode) {
        case SRDS_IEEE_50GE: /* 50G w/ Bit Muxing 2:1 and No Code Word Interleaving 50GE/100GE 56G BHK */
           ...
           err_analyzer->fec_size_frac = BLACKHAWK7_L2P2_PRBS_FEC_SIZE_FRAC_1_8;
        ...
        case SRDS_BHK_200GE:              /* 200G w/ Bit Muxing 2:1 and 2 Code Word Interleaving 200GE/400GE 56G BHK */
           ...
           err_analyzer->fec_size_frac = BLACKHAWK7_L2P2_PRBS_FEC_SIZE_FRAC_1_4;
           EFUN(wr_tlb_err_symbol_msb_lsb_group(0x1));
           EFUN(wr_tlb_err_fec_msb_en(0x1));
...
err_code_t blackhawk7_l2p2_prbs_error_analyzer_start(srds_access_t *sa__) {
    INIT_SRDS_ERR_CODE
...
err_code_t blackhawk7_l2p2_get_prbs_error_analyzer_config(srds_access_t *sa__, blackhawk7_l2p2_prbs_err_analyzer_config_status_st *err_analyzer) {
    INIT_SRDS_ERR_CODE
    ...
    err_analyzer->prbs_err_fec_size = (uint16_t)(prbs_err_fec_size * BLACKHAWK7_L2P2_PRBS_FEC_FRAME_SIZE_MULTIPLIER);
    err_analyzer->fec_size_frac = (uint8_t) prbs_err_fec_size_frac;
    return (ERR_CODE_NONE);
}

/* Display PRBS Error Analyzer Config */
err_code_t blackhawk7_l2p2_display_prbs_error_analyzer_config(srds_access_t *sa__, blackhawk7_l2p2_prbs_err_analyzer_config_status_st *err_analyzer, uint32_t timeout_s) {
    if(!err_analyzer) {
        return(blackhawk7_l2p2_error(sa__, ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
    }
    ...
    {
        EFUN_PRINTF(("%u %hu %hhu %u %hhu %hhu %hhu %08x %08x\n",   timeout_s, err_analyzer->prbs_err_fec_size, err_analyzer->fec_size_frac, err_analyzer->prbs_err_aggregate_mode, err_analyzer->lanes_active, err_analyzer->diag_mode, blackhawk7_l2p2_get_lane(sa__), err_analyzer->pp_field2, err_analyzer->pp_field1));
    }
    else
    {
        EFUN_PRINTF(("\n ****************************************************************************** \n"));
...
err_code_t blackhawk7_l2p2_clear_dac_pattern_memory(srds_access_t *sa__) {
    INIT_SRDS_ERR_CODE
    uint8_t i;
    ...
    EFUN(blackhawk7_l2p2_INTERNAL_clk4sync_div2_sequence(sa__, 0));
    ...
    EFUN(blackhawk7_l2p2_INTERNAL_clk4sync_div2_sequence(sa__, 1));
    BLACKHAWK7_L2P2_UAPI_NOT_AMS_A0_ERR   /* if not BLACKHAWK7_V1L8P2, print error */

    return (ERR_CODE_NONE);
}

/* Read DAC pattern memory into a 32xN memory */
err_code_t blackhawk7_l2p2_read_dac_pattern_memory(srds_access_t *sa__, uint32_t *dac_mem, uint8_t read_addr, uint8_t num_of_lines) {
    INIT_SRDS_ERR_CODE
...
err_code_t blackhawk7_l2p2_get_prbs_error_analyzer_lane_err_count(srds_access_t *sa__, blackhawk7_l2p2_prbs_err_analyzer_config_status_st *err_analyzer, uint8_t lane) {
    INIT_SRDS_ERR_CODE
    ...
    EFUN(blackhawk7_l2p2_read_dac_pattern_memory(sa__,dacmem,(uint8_t)(4*lane),4));
    ...
    EFUN(blackhawk7_l2p2_read_dac_pattern_memory(sa__,dacmem2,(uint8_t)(4*(num_lanes+1)),2));
    ...
    if (PRBS_VERBOSE > 2) {
        EFUN_PRINTF(("\n\n\t=====> DEBUG INFO (start) [Core = %d, Lane = %d]\n", blackhawk7_l2p2_acc_get_core(sa__), lane));
        EFUN_PRINTF(("\n\t << Frame Errors accumulated in the DAC Pattern Memory >>\n"));
        EFUN_PRINTF(("\t FEC frames with    0 Errors = %u\n",err_analyzer->prbs_frames_all));
        for (i=0; i<BLACKHAWK7_L2P2_PRBS_NUM_OF_ERROR_ANALYZER_COUNTERS; i++) {
            if (i < (BLACKHAWK7_L2P2_PRBS_NUM_OF_ERROR_ANALYZER_COUNTERS-1)) {
                EFUN_PRINTF(("\t FEC frames with   %2d Errors = %u\n",i+1,err_analyzer->prbs_errcnt[i]));
            }
            else {
                EFUN_PRINTF(("\t FEC frames with > %2d Errors = %u\n",(BLACKHAWK7_L2P2_PRBS_NUM_OF_ERROR_ANALYZER_COUNTERS-1),err_analyzer->prbs_errcnt[i]));
            }
        }
    }

    /* Calculating cumulative error count for respective values of "t" */
    for (i=BLACKHAWK7_L2P2_PRBS_NUM_OF_ERROR_ANALYZER_COUNTERS; i > 0; i--) {
        if (i!=BLACKHAWK7_L2P2_PRBS_NUM_OF_ERROR_ANALYZER_COUNTERS) {
...
err_code_t blackhawk7_l2p2_get_prbs_error_analyzer_err_count(srds_access_t *sa__, blackhawk7_l2p2_prbs_err_analyzer_config_status_st *err_analyzer) {
    INIT_SRDS_ERR_CODE
    ...
    ESTM(err_analyzer->pp_field2 = blackhawk7_l2p2_acc_get_lane(sa__));
    EFUN(blackhawk7_l2p2_INTERNAL_get_num_bits_per_ms(sa__, 1, &err_analyzer->pp_field1));

    ESTM(prbs_lock = rd_prbs_chk_lock());
    if(!prbs_lock) {
        EFUN_PRINTF(("\nERROR : PRBS Checker is not locked for Lane %d\n",blackhawk7_l2p2_acc_get_lane(sa__)));
        for (i=0; i<BLACKHAWK7_L2P2_PRBS_NUM_OF_ERROR_ANALYZER_COUNTERS; i++) {
    ...
    EFUN(blackhawk7_l2p2_get_prbs_error_analyzer_lane_err_count(sa__, err_analyzer, lane));

    return (ERR_CODE_NONE);
}

/* Get PRBS Error Analyzer Aggregate Error Count */
err_code_t blackhawk7_l2p2_get_prbs_error_analyzer_aggregate_err_count(srds_access_t *sa__, blackhawk7_l2p2_prbs_err_analyzer_config_status_st *err_analyzer) {
    INIT_SRDS_ERR_CODE
    ...
    for(lane = 0;lane<num_lanes;lane++) {
        if ((err_analyzer->lane_mask_aggregate >> lane) & 0x1) {
            EFUN(blackhawk7_l2p2_acc_set_lane(sa__,lane));
    ...
    EFUN(blackhawk7_l2p2_get_prbs_error_analyzer_lane_err_count(sa__, err_analyzer, num_lanes)); /* For aggregate counters, just pass the num_lanes to point to the correct memory location in the DAC */

    return (ERR_CODE_NONE);
}

static err_code_t _blackhawk7_l2p2_display_fec_frame_error(srds_access_t *sa__, uint8_t errs_per_frame, uint32_t num_frame_errs) {
...
err_code_t blackhawk7_l2p2_display_prbs_error_analyzer_err_count(srds_access_t *sa__, blackhawk7_l2p2_prbs_err_analyzer_config_status_st *err_analyzer) {
    INIT_SRDS_ERR_CODE
    uint8_t i;

    if (err_analyzer->diag_mode)
    {
        for (i=0; i<(BLACKHAWK7_L2P2_PRBS_NUM_OF_ERROR_ANALYZER_COUNTERS-1); i++) {
            EFUN_PRINTF(("%08x,", err_analyzer->prbs_errcnt[i]));
        }
        EFUN_PRINTF(("%08x\n", err_analyzer->prbs_errcnt[BLACKHAWK7_L2P2_PRBS_NUM_OF_ERROR_ANALYZER_COUNTERS-1]));
        EFUN_PRINTF(("%08x\n", err_analyzer->prbs_frames_all));
    }
    else
    {
        ...
        for (i=0; i<BLACKHAWK7_L2P2_PRBS_NUM_OF_ERROR_ANALYZER_COUNTERS; i++) {
            EFUN(_blackhawk7_l2p2_display_fec_frame_error(sa__, i, err_analyzer->prbs_errcnt[i]));
        }
        EFUN_PRINTF((" -------------------------------------------------------------\n"));
    }
...
err_code_t blackhawk7_l2p2_display_prbs_error_analyzer_aggregate_err_count(srds_access_t *sa__, blackhawk7_l2p2_prbs_err_analyzer_config_status_st *err_analyzer) {
    INIT_SRDS_ERR_CODE
    uint8_t i;
...
err_code_t blackhawk7_l2p2_display_prbs_error_analyzer_struct(srds_access_t *sa__, blackhawk7_l2p2_prbs_err_analyzer_config_status_st *err_analyzer, uint32_t timeout_s) {
    INIT_SRDS_ERR_CODE
    if (err_analyzer->diag_mode)
    {
        /* Start of data dump */
    ...
    /* Display PRBS Error Analyzer Config */
    EFUN(blackhawk7_l2p2_display_prbs_error_analyzer_config(sa__, err_analyzer, timeout_s));
    /* Display PRBS Error Analyzer Err_Counts */
    EFUN(blackhawk7_l2p2_display_prbs_error_analyzer_err_count(sa__, err_analyzer));
    if (err_analyzer->diag_mode)
    {
        /* End of data dump */
...
err_code_t blackhawk7_l2p2_prbs_error_analyzer_aggregate_config(srds_access_t *sa__, blackhawk7_l2p2_prbs_err_analyzer_config_status_st *err_analyzer) {
    INIT_SRDS_ERR_CODE
    uint8_t num_lanes, lane;
    /* uint8_t aggr_mode; */
    ...
    for(lane = 0;lane<num_lanes;lane++) {
        if ((err_analyzer->lane_mask_aggregate >> lane) & 0x1) {
            EFUN(blackhawk7_l2p2_acc_set_lane(sa__,lane));
            EFUN(blackhawk7_l2p2_prbs_error_analyzer_config(sa__, err_analyzer));  /* Configure PRBS Error Analyzer */
...
err_code_t blackhawk7_l2p2_display_prbs_error_analyzer_proj(srds_access_t *sa__, uint16_t prbs_err_fec_size, uint32_t timeout_s) {
    INIT_SRDS_ERR_CODE
    ...
    ESTM(lane_mask_aggregate = (uint8_t)(0x1 << (blackhawk7_l2p2_acc_get_lane(sa__))));

    /* Call the aggregate function to display error analyzer projections */
    EFUN(blackhawk7_l2p2_display_aggregate_prbs_error_analyzer_proj(sa__, prbs_err_fec_size, SRDS_IEEE_50GE, lane_mask_aggregate, timeout_s, 0x0));
    return (ERR_CODE_NONE);
}

/* Display Aggregate PRBS Error Analyzer Projection */
err_code_t blackhawk7_l2p2_display_aggregate_prbs_error_analyzer_proj(srds_access_t *sa__, uint16_t prbs_err_fec_size, enum srds_prbs_error_analyzer_aggregate_mode_enum prbs_err_aggregate_mode, uint8_t lane_mask_aggregate, uint32_t timeout_s, uint8_t diag_mode) {
    INIT_SRDS_ERR_CODE
    uint8_t  hrs, mins, secs;
    uint8_t num_lanes, lane;
    uint8_t  prbs_lock;
    uint8_t i;
    blackhawk7_l2p2_prbs_err_analyzer_config_status_st err_analyzer;
    ...
    ENULL_MEMSET(&err_analyzer, 0, sizeof(blackhawk7_l2p2_prbs_err_analyzer_config_status_st));
    ...
    EFUN(blackhawk7_l2p2_prbs_error_analyzer_reset(sa__));  /* Reset PRBS Error Analyzer and clear DAC */
    EFUN(blackhawk7_l2p2_prbs_error_analyzer_aggregate_config(sa__, &err_analyzer));  /* Configure PRBS Error Analyzer Aggregation Mode*/
    ...
    for(lane = 0;lane<num_lanes;lane++) {
        if ((err_analyzer.lane_mask_aggregate >> lane) & 0x1) {
            EFUN(blackhawk7_l2p2_acc_set_lane(sa__,lane));
            EFUN(blackhawk7_l2p2_prbs_error_analyzer_start(sa__));  /* Start PRBS Error Analyzer */
        }
    }

    EFUN(blackhawk7_l2p2_INTERNAL_seconds_to_displayformat(timeout_s, &hrs, &mins, &secs));
    ...
    for(lane = 0;lane<num_lanes;lane++) {
        if ((err_analyzer.lane_mask_aggregate >> lane) & 0x1) {
            ...
            EFUN(blackhawk7_l2p2_get_prbs_error_analyzer_err_count(sa__, &err_analyzer));
            /* Display PRBS Error Analyzer Config */
            EFUN(blackhawk7_l2p2_display_prbs_error_analyzer_struct(sa__, &err_analyzer, timeout_s));
            EFUN(blackhawk7_l2p2_prbs_error_analyzer_compute_proj(sa__, err_analyzer, timeout_s));
        }
    }
    if (err_analyzer.num_of_aggregate_lanes > 1)
    {
       ...
       err_analyzer.prbs_frames_all = 0;
       EFUN(blackhawk7_l2p2_get_prbs_error_analyzer_aggregate_err_count(sa__, &err_analyzer));
       EFUN(blackhawk7_l2p2_display_prbs_error_analyzer_aggregate_err_count(sa__, &err_analyzer));
       EFUN(blackhawk7_l2p2_prbs_error_analyzer_compute_proj(sa__, err_analyzer, timeout_s));
    }
...
/*----------------------------------------------------------------------------------------------------------------*/
err_code_t blackhawk7_l2p2_prbs_error_analyzer_report_proj(srds_access_t *sa__, blackhawk7_l2p2_prbs_err_analyzer_config_status_st err_analyzer, uint32_t timeout_s, blackhawk7_l2p2_prbs_err_analyzer_report_st *err_analyzer_report) {
#ifdef SERDES_API_FLOATING_POINT
    INIT_SRDS_ERR_CODE
    uint8_t    delta_n;
    uint8_t     i;
    ...
    USR_DOUBLE meas_fec_ber[BLACKHAWK7_L2P2_PRBS_NUM_OF_ERROR_ANALYZER_COUNTERS];
    ...
    ENULL_MEMSET(err_analyzer_report, 0, sizeof(blackhawk7_l2p2_prbs_err_analyzer_report_st));
    EFUN(blackhawk7_l2p2_INTERNAL_get_num_bits_per_ms(sa__, 1, &num_bits_per_ms));

    if (err_analyzer.fec_size_frac == BLACKHAWK7_L2P2_PRBS_FEC_SIZE_FRAC_1_8)
        overlap_factor = 8;
    else if (err_analyzer.fec_size_frac == BLACKHAWK7_L2P2_PRBS_FEC_SIZE_FRAC_1_4)
        overlap_factor = 4;
    else if (err_analyzer.fec_size_frac == BLACKHAWK7_L2P2_PRBS_FEC_SIZE_FRAC_1_2)
        overlap_factor = 2;
    else
        overlap_factor = 1;
    ...
    start_idx = 0;
    stop_idx  = 0;
    if (err_analyzer.prbs_errcnt[BLACKHAWK7_L2P2_PRBS_NUM_OF_ERROR_ANALYZER_COUNTERS-1] != 0xFFFFFFFF) {
        stop_idx  = BLACKHAWK7_L2P2_PRBS_NUM_OF_ERROR_ANALYZER_COUNTERS-1;
    }
    for (i=BLACKHAWK7_L2P2_PRBS_NUM_OF_ERROR_ANALYZER_COUNTERS; i > 0; i--) {
    ...
    if (delta_n >= 2) {                                                              /* There are at least 2 points to trace a line for extrapolation */
        for(i = start_idx; i < stop_idx; i++) {
            x    = (USR_DOUBLE) (i);
            y    = pow((-log10(meas_fec_ber[i])),BLACKHAWK7_L2P2_PRBS_ERR_ANALYZER_LINEARITY_ADJUST);
    ...
    if (delta_n >= 2) {
        ...
        proj_ber[15] = pow(proj_ber[15],1.0/BLACKHAWK7_L2P2_PRBS_ERR_ANALYZER_LINEARITY_ADJUST);
...
/*                                                                                                                */
/* Following guidelines are used while computing projected values -                                               */
/*   i) Only "frame_errors" that are in between "10 and 0xffffffff" are considered as valid data points for projection */
/*  ii) Need at least 2 valid data points to generated projected data                                              */
/*----------------------------------------------------------------------------------------------------------------*/
err_code_t blackhawk7_l2p2_prbs_error_analyzer_extrapolate_err_count(srds_access_t *sa__, blackhawk7_l2p2_prbs_err_analyzer_config_status_st *err_analyzer, uint8_t err_cnt_idx ) {
#ifdef SERDES_API_FLOATING_POINT
    INIT_SRDS_ERR_CODE
    uint8_t    delta_n;
    uint8_t    i;
    ...
    if (delta_n >= 2) { /* There are at least 2 points to trace a line for extrapolation */
        ...
        EFUN(blackhawk7_l2p2_INTERNAL_fit_second_order(sa__, delta_n, x, y, a));
    }
...
err_code_t blackhawk7_l2p2_prbs_error_analyzer_compute_proj(srds_access_t *sa__, blackhawk7_l2p2_prbs_err_analyzer_config_status_st err_analyzer, uint32_t timeout_s) {

#ifdef SERDES_API_FLOATING_POINT
    INIT_SRDS_ERR_CODE
    blackhawk7_l2p2_prbs_err_analyzer_report_st err_analyzer_rep;
...
err_code_t blackhawk7_l2p2_get_rx_tuning_status(srds_access_t *sa__, uint8_t *status) {
    INIT_SRDS_ERR_CODE
...
err_code_t blackhawk7_l2p2_config_BER_HW_mode(srds_access_t *sa__, uint16_t time_ms, struct blackhawk7_l2p2_ber_data_st *ber_data, struct blackhawk7_l2p2_prbs_chk_hw_timer_ctrl_st *prbs_chk_hw_timer_ctrl) {
    INIT_SRDS_ERR_CODE
    uint8_t lcklost = 0;
    uint32_t err_cnt= 0;
    uint8_t clk_gate = 0;
    ...
    EFUN(blackhawk7_l2p2_INTERNAL_prbs_chk_cdr_lock_lost(sa__, &ber_data->cdrlcklost));

    /* save prbs hardware timer config registers and prbs mode */
    EFUN(blackhawk7_l2p2_get_prbs_chk_hw_timer_ctrl(sa__, prbs_chk_hw_timer_ctrl));

    /* Configure hardware timer to count prbs errors */
    EFUN(blackhawk7_l2p2_config_prbs_chk_hw_timer(sa__, time_ms, &prbs_chk_hw_timer_ctrl->time_ms_adjusted_bak));
    BLACKHAWK7_L2P2_UAPI_INIT_V3     /* if BLACKHAWK7_V3L2P2 */

    EFUN(blackhawk7_l2p2_prbs_err_count_state(sa__, &err_cnt, &lcklost)); /* Clear prbs error counters and start timer */
    BLACKHAWK7_L2P2_UAPI_ELSE_V1_OR_V2     /* if BLACKHAWK7_V1L2P2 || BLACKHAWK7_V2L2P2  */
    BLACKHAWK7_L2P2_UAPI_ELSE_ERR
    /* Extra delay to account for the synchronization delay in prbs checker */
    EFUN(blackhawk7_l2p2_INTERNAL_hw_timer_sync_delay(sa__));

    return(ERR_CODE_NONE);
}

err_code_t blackhawk7_l2p2_get_BER_data_HW_mode(srds_access_t *sa__, struct blackhawk7_l2p2_ber_data_st *ber_data, struct blackhawk7_l2p2_prbs_chk_hw_timer_ctrl_st *prbs_chk_hw_timer_ctrl) {
    INIT_SRDS_ERR_CODE
    uint8_t lcklost = 0;
    uint32_t err_cnt= 0;
...
/*
                ber_data->num_bits = (uint64_t)num_bits_per_ms*(uint64_t)prbs_chk_hw_timer_ctrl->time_ms_adjusted_bak;
...
*/
    ...
    /* restore prbs hardware timer config registers */
    EFUN(blackhawk7_l2p2_set_prbs_chk_hw_timer_ctrl(sa__, prbs_chk_hw_timer_ctrl));
