...
#define TSCF_REV_LETTER_D        0x3
#define TSCF_REV_NUM_0           0x0
#define TSCF_REV_NUM_1           0x1
#define FALCON_LANE_RAM_BYTE7_BIT5 (1 << 5)  /* to skip the TXFIR initialization in a restart event during the AN link training */
#define FALCON_LANE_RAM_BYTE7_BIT6 (1 << 6)  /* to initialize TXFIR in 1G mode (CL73 AN) to (0,100,0). */
...
STATIC
int _tscf_phy_firmware_lane_an_cl72_tx_init_skip_on_restart_set(const phymod_phy_access_t* phy, uint32_t enable)
{
    uint32_t data;

    PHYMOD_IF_ERR_RETURN(falcon_tsc_cl93n72_frc_byte_get(&phy->access, &data));

    if (enable) {
        data |= (FALCON_LANE_RAM_BYTE7_BIT5 | FALCON_LANE_RAM_BYTE7_BIT6);
    } else {
        data &= ~(FALCON_LANE_RAM_BYTE7_BIT5 | FALCON_LANE_RAM_BYTE7_BIT6);
    }

    PHYMOD_IF_ERR_RETURN(falcon_tsc_cl93n72_frc_byte_set(&phy->access, data));

    return PHYMOD_E_NONE;
}

STATIC
int _tscf_phy_firmware_lane_an_cl72_tx_init_skip_on_restart_get(const phymod_phy_access_t* phy, uint32_t *enable)
{
    uint32_t data;

    PHYMOD_IF_ERR_RETURN(falcon_tsc_cl93n72_frc_byte_get(&phy->access, &data));

    *enable = data & (FALCON_LANE_RAM_BYTE7_BIT5 | FALCON_LANE_RAM_BYTE7_BIT6) ? 1 : 0;
...
int _tscf_phy_firmware_lane_config_set(const phymod_phy_access_t* phy, phymod_firmware_lane_config_t fw_config)
{
    ...
    for (i = 0; i < num_lane; i++) {
        ...
        if(!is_warm_boot) {
            ...
            PHYMOD_IF_ERR_RETURN(_tscf_phy_firmware_lane_an_cl72_tx_init_skip_on_restart_set(&phy_copy, fw_config.AnCl72TxInitSkipOnRestart));
...
int tscf_core_info_get(const phymod_core_access_t* phy, phymod_core_info_t* info)
{
    ...
    if ((serdes_id & 0x3f) == TSCF_GEN2_MODEL) {
        if (rev_letter == TSCF_REV_LETTER_D) {
            info->core_version = phymodCoreVersionTscfD0;
        }
    }
...
int tscf_core_lane_map_get(const phymod_core_access_t* core,
                           phymod_lane_map_t* lane_map)
{
    uint32_t pmd_swap = 0 , pcs_swap = 0, lane; 
    uint32_t pmd_tx_swap = 0;

    if (!PHYMOD_DEVICE_OP_MODE_PCS_BYPASS_GET(core->device_op_mode)) {
        PHYMOD_IF_ERR_RETURN(tefmod_pcs_lane_swap_get(&core->access,
                             &pcs_swap));
    }
    PHYMOD_IF_ERR_RETURN(tefmod_pmd_lane_swap_tx_get(&core->access,
                                                     &pmd_tx_swap));

    if (!PHYMOD_DEVICE_OP_MODE_PCS_BYPASS_GET(core->device_op_mode)) {
        for (lane = 0 ; lane < TSCF_NOF_LANES_IN_CORE ; lane++) {
            pmd_swap |= lane << 4 * ((pmd_tx_swap >> (lane * 4)) & 0xf);
       }
    } else {
    ...
    for( lane = 0 ; lane < TSCF_NOF_LANES_IN_CORE ; lane++){
        ...
        lane_map->lane_map_rx[lane] =
                         (pcs_swap >> (lane*4)) & TSCF_LANE_SWAP_LANE_MASK;
        /*considering the pcs lane swap: tx_map[lane] = pmd_map[pcs_map[lane]]*/
        lane_map->lane_map_tx[lane] =
                                 (pmd_swap >> (lane_map->lane_map_rx[lane]*4)) &
                                              TSCF_LANE_SWAP_LANE_MASK;
    }
...
int tscf_phy_firmware_lane_config_set(const phymod_phy_access_t* phy, phymod_firmware_lane_config_t fw_config)
{
    phymod_phy_access_t phy_copy;
    int start_lane, num_lane, i;
    int port_starting_lane, port_num_lane;
    ...
    PHYMOD_IF_ERR_RETURN
        (tefmod_port_start_lane_get(&phy->access,
                                    &port_starting_lane,
                                    &port_num_lane));
    phy_copy.access.lane_mask = 1 << port_starting_lane;
    /* next we need to toggle the pcs data path reset */
    PHYMOD_IF_ERR_RETURN
        (tefmod_trigger_speed_change(&phy_copy.access));
...
int tscf_phy_firmware_lane_config_get(const phymod_phy_access_t* phy, phymod_firmware_lane_config_t* fw_config)
{
    ...
    PHYMOD_IF_ERR_RETURN(_tscf_phy_firmware_lane_an_cl72_tx_init_skip_on_restart_get(&phy_copy, &enable));
    PHYMOD_MEMSET(fw_config, 0, sizeof(*fw_config));
    ...
    fw_config->AnCl72TxInitSkipOnRestart = enable;
...
{
    PHYMOD_IF_ERR_RETURN(falcon_tsc_rx_pmd_restart(&phy->access));
    return PHYMOD_E_NONE;
}
...
int tscf_phy_interface_config_set(const phymod_phy_access_t* phy, uint32_t flags, const phymod_phy_inf_config_t* config)
{
    ...
    if((current_pll_div != new_pll_div) && !(PHYMOD_INTF_F_DONT_TURN_OFF_PLL & flags)) {
        ...
        if (config->ref_clock == phymodRefClk156Mhz &&
            pll_multiplier == 175) {
            firmware_core_config.VcoRate = 214;
        } else {
...
int tscf_phy_init(const phymod_phy_access_t* phy, const phymod_phy_init_config_t* init_config)
{
    ...
    for (i = 0; i < num_lane; i++) {
        ...
        pm_phy_copy.access.lane_mask = 0x1 << (i + start_lane);
        if (PHYMOD_PHY_INIT_F_AN_CL72_TX_INIT_SKIP_ON_RESTART_GET(init_config)) {
            firmware_lane_config.AnCl72TxInitSkipOnRestart = 1;
        }
        PHYMOD_IF_ERR_RETURN
             (_tscf_phy_firmware_lane_config_set(&pm_phy_copy, firmware_lane_config));
    }
...
int tscf_phy_hg2_codec_control_set(const phymod_phy_access_t* phy, phymod_phy_hg2_codec_t hg2_codec) {
    ...
    uint32_t pcs_lane_enable;
     ...
     PHYMOD_IF_ERR_RETURN
         (tefmod_disable_get(&phy->access, &pcs_lane_enable));
