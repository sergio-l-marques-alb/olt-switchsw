/**************************************************************************************
**************************************************************************************
*  File Name     :  falcon_tsc_dv_functions.c                                        *
*  Created On    :  22/05/2014                                                       *
*  Created By    :  Kiran Divakar                                                    *
*  Description   :  APIs for Serdes IPs ported over from DV                          *
*  Revision      :  $Id: falcon_tsc_dv_functions.c 1565 2017-08-18 23:22:09Z kirand $ *
*                                                                                    *
...
/** @file falcon_tsc_dv_functions.c
...
err_code_t falcon_tsc_read_tx_afe (const phymod_access_t *pa, enum srds_tx_afe_settings_enum param, int8_t *val){
	...
	case TX_AFE_AMP:
		...
		break;
	case TX_AFE_DRIVERMODE:
        EFUN(_get_tx_driver_mode(pa, val));
		break;
	default:
		return _error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT);
	}
...
err_code_t falcon_tsc_loop_timing (const phymod_access_t *pa, uint8_t enable){
	...
	if (enable) {
		...
		if ((osr_mode == FALCON_TSC_OSX16P5) || (osr_mode == FALCON_TSC_OSX20P625))
		...
		EFUN(falcon_tsc_delay_us(25));                  /* Wait for tclk to lock to CDR */
	} else {
...
err_code_t falcon_tsc_rmt_lpbk (const phymod_access_t *pa, uint8_t enable){
	if (enable) {
		EFUN(falcon_tsc_loop_timing (pa, enable));
		...
		EFUN(falcon_tsc_delay_us(50));          /* Wait for rclk and tclk phase lock before expecing good data from rmt loopback */
...
err_code_t falcon_tsc_tx_pi_jitt_gen (const phymod_access_t *pa, uint8_t enable, int16_t freq_override_val, enum srds_tx_pi_freq_jit_gen_enum jit_type, uint8_t tx_pi_jit_freq_idx, uint8_t tx_pi_jit_amp){
	...
	EFUN(falcon_tsc_tx_pi_freq_override (pa, enable, freq_override_val));
...
err_code_t falcon_tsc_uc_reset (const phymod_access_t *pa, uint8_t enable){
	if (enable) {
		...
		EFUN(falcon_tsc_pmd_wr_reg (pa, 0xD226, 0x0000));
		...
		for (index1 = 0; index1 < num_lanes; ++index1) {
			...
			if ((tx_lane_map[index1] != rx_lane_map[index1])
				...
				EFUN_PRINTF(("Warning:  In core %d, TX lane %d is mapped to %d, while RX lane %d is mapped to %d.\n          Digital and remote loopback will not operate as expected.\n          Further warnings are suppressed.\n",
					     EFUN(falcon_tsc_get_core()), index1, tx_lane_map[index1], index1, rx_lane_map[index1]));
...
err_code_t falcon_tsc_core_pwrdn (const phymod_access_t *pa, enum srds_core_pwrdn_mode_enum mode){
	switch (mode) {
	case PWR_ON:
		EFUN(_falcon_tsc_core_clkgate (pa, 0));
		EFUN(wrc_afe_s_pll_pwrdn(0x0));
		EFUN(wrc_core_dp_s_rstb(0x1));
		break;
	case PWRDN:
		EFUN(wrc_core_dp_s_rstb(0x0));
		EFUN(falcon_tsc_delay_ns(   500)); /* wait >50 comclk cycles  */
	...
	case PWRDN_DEEP:
		...
		EFUN(falcon_tsc_delay_ns(   500)); /* wait >50 comclk cycles  */
		EFUN(wrc_afe_s_pll_pwrdn(0x1));
		EFUN(_falcon_tsc_core_clkgate (pa, 1));
		break;
	default:
...
err_code_t falcon_tsc_lane_pwrdn (const phymod_access_t *pa, enum srds_core_pwrdn_mode_enum mode){
	...
	case PWR_ON:
		...
		EFUN(_falcon_tsc_lane_clkgate (pa, 0));
		break;
	case PWRDN:
	...
	case PWRDN_DEEP:
		...
		EFUN(wr_ln_rx_s_pwrdn(0x1));
		EFUN(wr_ln_tx_s_pwrdn(0x1));
		EFUN(_falcon_tsc_lane_clkgate (pa, 1));
...
err_code_t falcon_tsc_isolate_ctrl_pins (const phymod_access_t *pa, uint8_t enable){
	...
	EFUN(falcon_tsc_isolate_lane_ctrl_pins (pa, enable));
	EFUN(falcon_tsc_isolate_core_ctrl_pins (pa, enable));

	return ERR_CODE_NONE;
}

err_code_t falcon_tsc_isolate_lane_ctrl_pins (const phymod_access_t *pa, uint8_t enable){
...
err_code_t falcon_tsc_isolate_core_ctrl_pins (const phymod_access_t *pa, uint8_t enable){
...
/* Micro RAM Byte Read */
err_code_t falcon_tsc_rdblk_uc_ram (const phymod_access_t *pa, uint8_t *mem, uint16_t addr, uint16_t cnt){
	if (!mem)
		return _error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT);
	while (cnt--) {
		err_code_t error_code;
		uint8_t tmp = falcon_tsc_rdb_uc_ram (pa, &error_code, addr++); /* Value read from the required Data RAM Address */
		if (error_code)
...
err_code_t falcon_tsc_pll_drv_hv_disable (const phymod_access_t *pa, uint8_t disable){
...
err_code_t falcon_tsc_configure_fast_link_recovery(const phymod_access_t *pa, uint8_t enable, uint8_t enable_timeout, enum falcon_tsc_flr_timeout_enum timeout_time, uint8_t enable_2ms_yield, uint8_t filter_count) {
    ...
    if(reset_state < 7) {
        EFUN_PRINTF(("ERROR: falcon_tsc_configure_fast_link_recovery(..) called without ln_dp_s_rstb=0 Lane=%d reset_state=%d\n",falcon_tsc_get_lane(pa),reset_state));
        return (_error(ERR_CODE_LANE_DP_NOT_RESET));
    }
#endif

    if (timeout_time_val > 3) {
        ESTM_PRINTF(("timeout_time value for falcon_tsc_configure_fast_link_recovery() is too large:  %d\n", timeout_time_val));
        return (_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
    }
    if (filter_count > 7) {
        ESTM_PRINTF(("filter_count value for falcon_tsc_configure_fast_link_recovery() is too large:  %d\n", filter_count));
        return (_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
    }
    EFUN(wrv_usr_ctrl_fast_link_recovery_byte(  (enable ? 1 : 0)
                                              ...
                                              | (filter_count << 5)));
    EFUN(wrv_usr_flr_enhanced((enable==2) ? 1 : 0));
    return (ERR_CODE_NONE);
}
