...
/*Set timesync adjust*/
int tscbh_fe_gen2_timesync_adjust_set(const phymod_phy_access_t* phy, uint32_t flags, uint32_t ts_am_norm_mode)
{
    return PHYMOD_E_NONE;
}
                                       ...
                                       phymod_phy_pll_state_t* new_pll_state)
{
    ...
    *new_pll_state = *old_pll_state;
    ...
    if (!PHYMOD_DEVICE_OP_MODE_PCS_BYPASS_GET(phy->device_op_mode)) {
        ...
        PHYMOD_IF_ERR_RETURN
        (blackhawk7_l2p2_speed_config_get(speed_config->data_rate, ref_clk_is_156p25, &request_pll_div, &is_pam4, &osr_mode));
