...
err_code_t osprey7_v2l8p1_collect_custom_fields_osprey7_v2l8p1_detailed_lane_status_st(srds_access_t *sa__, osprey7_v2l8p1_detailed_lane_status_st_t *st){
    INIT_SRDS_ERR_CODE
    uint32_t txval;
    int8_t lms_thresh;
    uint8_t tap_num;
    ...
    EFUN(osprey7_v2l8p1_prbs_err_count_state(sa__, &txval, &st->prbs_chk_lock_lost));
    st->prbs_chk_errcnt = txval;

    ESTM(st->dummy22    = rdv_usr_status_eq_debug17());
    EFUN(osprey7_v2l8p1_INTERNAL_get_link_time(sa__, &txval));
    st->link_time = txval;  
    ...
    EFUN(osprey7_v2l8p1_INTERNAL_get_rx_vga(sa__, &tap_num));

    st->vga = tap_num;
    EFUN(osprey7_v2l8p1_INTERNAL_get_rx_fga(sa__, &tap_num));
    st->fga = tap_num;

    EFUN(osprey7_v2l8p1_INTERNAL_get_rx_pf_main(sa__, &tap_num)); 
    st->pf = tap_num;

    EFUN(osprey7_v2l8p1_INTERNAL_get_rx_pf2(sa__, &tap_num)); 
    st->pf2 = tap_num;

    EFUN(osprey7_v2l8p1_INTERNAL_get_rx_pf3(sa__, &tap_num));
    st->pf3 = tap_num;
...
err_code_t osprey7_v2l8p1_collect_custom_fields_osprey7_v2l8p1_lane_state_st(srds_access_t *sa__, osprey7_v2l8p1_lane_state_st_t *st){
    INIT_SRDS_ERR_CODE
    EFUN(osprey7_v2l8p1_INTERNAL_read_lane_state(sa__,st));
    return ERR_CODE_NONE;
