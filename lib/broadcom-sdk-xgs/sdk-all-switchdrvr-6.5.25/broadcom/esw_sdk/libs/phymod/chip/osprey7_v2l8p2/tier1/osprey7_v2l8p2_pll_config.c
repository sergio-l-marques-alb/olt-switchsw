...
err_code_t osprey7_v2l8p2_INTERNAL_configure_pll(srds_access_t *sa__,
    ...
    ESTM(ams_version = rd_ams_tx_version_id() & 0xf0);
    if(ams_version == 0xa0) {
        pll_vco_cutoff_in_khz=VCO_FREQ_51P72G_KHZ;
    }
    else if(ams_version == 0xc0) {
        pll_vco_cutoff_in_khz=VCO_FREQ_53P2G_KHZ;
    }
    /* else keep the default 50G cutoff frequency for B and D versions  */
...
err_code_t osprey7_v2l8p2_INTERNAL_configure_53P125_pll(srds_access_t *sa__,
    ...
    ESTM(ams_version = rd_ams_tx_version_id() & 0xf0);
    EFUN(osprey7_v2l8p2_INTERNAL_configure_lb_53P125_pll(sa__, refclk, srds_div, pll_option)); /* Enable LBVCO */
