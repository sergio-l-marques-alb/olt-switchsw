...
#include <phymod/chip/blackhawk7_l8p2.h>
...
#define BLACKHAWK7_L8P2_MODEL               0x28
#define BLACKHAWK7_L8P2_NOF_LANES_IN_CORE   0x8
#define BLACKHAWK7_L8P2_PHY_ALL_LANES       0xff
#define BLACKHAWK7_L8P2_TX_TAP_NUM          12
#define BLACKHAWK7_L8P2_PMD_CRC_UCODE       1

#define REF_CLOCK_312P5_HZ            312500000
#define REF_CLOCK_156P25_HZ           156250000


#define BLACKHAWK7_L8P2_CORE_TO_PHY_ACCESS(_phy_access, _core_access) \
    do{\
        ...
        (_phy_access)->access.lane_mask = BLACKHAWK7_L8P2_PHY_ALL_LANES; \
    }while(0)
...
{
    info->core_version = phymodCoreVersionBlackhawk7_l8p2;
    info->serdes_id = 0;
    info->phy_id0 = 0;
    info->phy_id1 = 0;
...
int blackhawk7_l8p2_core_lane_map_get(const phymod_core_access_t* core, phymod_lane_map_t* lane_map)
{
    ...
    for (i = 0; i < BLACKHAWK7_L8P2_NOF_LANES_IN_CORE; i++) {
...
STATIC
int _blackhawk7_l8p2_core_firmware_load(const phymod_core_access_t* core, const phymod_core_init_config_t* init_config)
{
...
int blackhawk7_l8p2_phy_firmware_lane_config_get(const phymod_phy_access_t* phy, phymod_firmware_lane_config_t* fw_config)
{
    ...
    for (i = 0; i < num_lane; i++) {
        ...
        if (tx->tap_mode == phymodTxTapMode3Tap) {
            ...
            PHYMOD_PMD_IF_ERR_RETURN
                (blackhawk7_l8p2_apply_txfir_cfg(&phy_copy.access,
                                                 enable_taps,
                                                 0,
                                                 tx->pre,
                                                 tx->main,
...
int blackhawk7_l8p2_core_lane_map_set(const phymod_core_access_t* core, const phymod_lane_map_t* lane_map)
{
    ...
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_l8p2_map_lanes(&core_copy.access,
                                   BLACKHAWK7_L8P2_NOF_LANES_IN_CORE,
                                   pmd_tx_addr,
                                   pmd_rx_addr));
...
static
int _blackhawk7_l8p2_phy_firmware_lane_config_set(const phymod_phy_access_t* phy, phymod_firmware_lane_config_t fw_config)
{
...
int blackhawk7_l8p2_phy_firmware_lane_config_set(const phymod_phy_access_t* phy, phymod_firmware_lane_config_t fw_config)
{
    phymod_phy_access_t phy_copy;
    int start_lane, num_lane, i;
    ...
    for (i = 0; i < num_lane; i++) {
        ...
        phy_copy.access.lane_mask = 1 << (start_lane + i);
        PHYMOD_IF_ERR_RETURN
            (_blackhawk7_l8p2_phy_firmware_lane_config_set(&phy_copy, fw_config));
    }
...
int blackhawk7_l8p2_phy_polarity_get(const phymod_phy_access_t* phy, phymod_polarity_t* polarity)
{
    ...
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_l8p2_tx_rx_polarity_get(&phy_copy.access, &polarity->tx_polarity, &polarity->rx_polarity));
    return PHYMOD_E_NONE;
}
...
int blackhawk7_l8p2_phy_tx_override_set(const phymod_phy_access_t* phy, const phymod_tx_override_t* tx_override)
{
    ...
    for (i = 0; i < num_lane; i++) {
        ...
        phy_copy.access.lane_mask = 1 << (start_lane + i);
        PHYMOD_IF_ERR_RETURN
             (blackhawk7_l8p2_tx_pi_freq_override(&phy_copy.access,
...
int blackhawk7_l8p2_phy_tx_override_get(const phymod_phy_access_t* phy, phymod_tx_override_t* tx_override)
{
    ...
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_l8p2_tx_pi_control_get(&phy_copy.access, &value));
...
int blackhawk7_l8p2_phy_rx_set(const phymod_phy_access_t* phy, const phymod_rx_t* rx)
{
    ...
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_l8p2_signalling_mode_status_get(&phy_copy.access, &signalling_mode));
    ...
    for (i = 0; i < num_lane; i++) {
        ...
        PHYMOD_IF_ERR_RETURN(blackhawk7_l8p2_stop_uc_lane_status(&phy_copy.access, &uc_lane_stopped));
        if (!uc_lane_stopped) {
            PHYMOD_IF_ERR_RETURN(blackhawk7_l8p2_stop_rx_adaptation(&phy_copy.access, 1));
        }

        PHYMOD_PMD_IF_ERR_RETURN
            (blackhawk7_l8p2_write_rx_afe(&phy_copy.access, RX_AFE_VGA, (int8_t) rx->vga.value));

        PHYMOD_PMD_IF_ERR_RETURN
            (blackhawk7_l8p2_write_rx_afe(&phy_copy.access, RX_AFE_PF, (int8_t) rx->peaking_filter.value));

        PHYMOD_PMD_IF_ERR_RETURN
            (blackhawk7_l8p2_write_rx_afe(&phy_copy.access, RX_AFE_PF2, (int8_t) rx->low_freq_peaking_filter.value));

        /* next check the peaking value */
        PHYMOD_PMD_IF_ERR_RETURN
            (blackhawk7_l8p2_write_rx_afe(&phy_copy.access, RX_AFE_PF3, (int8_t) rx->high_freq_peaking_filter.value));
            ...
            switch (j) {
                case 0:
                    if (signalling_mode == phymodSignallingMethodNRZ) {
                        PHYMOD_PMD_IF_ERR_RETURN
                            (blackhawk7_l8p2_write_rx_afe(&phy_copy.access, RX_AFE_DFE1, (int8_t) rx->dfe[j].value));
                    } else {
                        if (rx->dfe[0].enable) {
                ...
                case 1:
                    PHYMOD_PMD_IF_ERR_RETURN
                        (blackhawk7_l8p2_write_rx_afe(&phy_copy.access, RX_AFE_DFE2, (int8_t) rx->dfe[j].value));
                    break;
                case 2:
                    PHYMOD_PMD_IF_ERR_RETURN
                        (blackhawk7_l8p2_write_rx_afe(&phy_copy.access, RX_AFE_DFE3, (int8_t) rx->dfe[j].value));
                    break;
                case 3:
                    PHYMOD_PMD_IF_ERR_RETURN
                        (blackhawk7_l8p2_write_rx_afe(&phy_copy.access, RX_AFE_DFE4, (int8_t) rx->dfe[j].value));
                    break;
                case 4:
                    PHYMOD_PMD_IF_ERR_RETURN
                        (blackhawk7_l8p2_write_rx_afe(&phy_copy.access, RX_AFE_DFE5, (int8_t) rx->dfe[j].value));
                    break;
                case 5:
                    PHYMOD_PMD_IF_ERR_RETURN
                        (blackhawk7_l8p2_write_rx_afe(&phy_copy.access, RX_AFE_DFE6, (int8_t) rx->dfe[j].value));
                    break;
                case 6:
                    PHYMOD_PMD_IF_ERR_RETURN
                        (blackhawk7_l8p2_write_rx_afe(&phy_copy.access, RX_AFE_DFE7, (int8_t) rx->dfe[j].value));
                    break;
                case 7:
                    PHYMOD_PMD_IF_ERR_RETURN
                        (blackhawk7_l8p2_write_rx_afe(&phy_copy.access, RX_AFE_DFE8, (int8_t) rx->dfe[j].value));
                    break;
                case 8:
                    PHYMOD_PMD_IF_ERR_RETURN
                        (blackhawk7_l8p2_write_rx_afe(&phy_copy.access, RX_AFE_DFE9, (int8_t) rx->dfe[j].value));
                    break;
                case 9:
                    PHYMOD_PMD_IF_ERR_RETURN
                        (blackhawk7_l8p2_write_rx_afe(&phy_copy.access, RX_AFE_DFE10, (int8_t) rx->dfe[j].value));
                    break;
                case 10:
                    PHYMOD_PMD_IF_ERR_RETURN
                        (blackhawk7_l8p2_write_rx_afe(&phy_copy.access, RX_AFE_DFE11,(int8_t)  rx->dfe[j].value));
                    break;
                case 11:
                    PHYMOD_PMD_IF_ERR_RETURN
                        (blackhawk7_l8p2_write_rx_afe(&phy_copy.access, RX_AFE_DFE12, (int8_t) rx->dfe[j].value));
                    break;
                case 12:
                    PHYMOD_PMD_IF_ERR_RETURN
                        (blackhawk7_l8p2_write_rx_afe(&phy_copy.access, RX_AFE_DFE13, (int8_t) rx->dfe[j].value));
                    break;
                case 13:
                    PHYMOD_PMD_IF_ERR_RETURN
                        (blackhawk7_l8p2_write_rx_afe(&phy_copy.access, RX_AFE_DFE14, (int8_t) rx->dfe[j].value));
                    break;
                default:
                    return PHYMOD_E_PARAM;
            }
...
int blackhawk7_l8p2_phy_rx_get(const phymod_phy_access_t* phy, phymod_rx_t* rx)
{
    ...
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_l8p2_read_rx_afe(&phy_copy.access, RX_AFE_VGA, &val));
    rx->vga.value = val;
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_l8p2_read_rx_afe(&phy_copy.access, RX_AFE_PF, &val));
    rx->peaking_filter.value = val;
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_l8p2_read_rx_afe(&phy_copy.access, RX_AFE_PF2, &val));
    rx->low_freq_peaking_filter.value = val;
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_l8p2_read_rx_afe(&phy_copy.access, RX_AFE_PF3, &val));
    ...
    for (j = 0 ; j < rx->num_of_dfe_taps ; j++){
        switch (j) {
            case 0:
                if (signalling_mode == phymodSignallingMethodNRZ) {
                    PHYMOD_IF_ERR_RETURN
                        (blackhawk7_l8p2_read_rx_afe(&phy_copy.access, RX_AFE_DFE1, &val));
                    rx->dfe[0].enable = 1;
                } else {
            ...
            case 1:
                PHYMOD_IF_ERR_RETURN
                    (blackhawk7_l8p2_read_rx_afe(&phy_copy.access, RX_AFE_DFE2, &val));
                break;
            case 2:
                PHYMOD_IF_ERR_RETURN
                    (blackhawk7_l8p2_read_rx_afe(&phy_copy.access, RX_AFE_DFE3, &val));
                break;
            case 3:
                PHYMOD_IF_ERR_RETURN
                    (blackhawk7_l8p2_read_rx_afe(&phy_copy.access, RX_AFE_DFE4, &val));
                break;
            case 4:
                PHYMOD_IF_ERR_RETURN
                    (blackhawk7_l8p2_read_rx_afe(&phy_copy.access, RX_AFE_DFE5, &val));
                break;
            case 5:
                PHYMOD_IF_ERR_RETURN
                    (blackhawk7_l8p2_read_rx_afe(&phy_copy.access, RX_AFE_DFE6, &val));
                break;
            case 6:
                PHYMOD_IF_ERR_RETURN
                    (blackhawk7_l8p2_read_rx_afe(&phy_copy.access, RX_AFE_DFE7, &val));
                break;
            case 7:
                PHYMOD_IF_ERR_RETURN
                    (blackhawk7_l8p2_read_rx_afe(&phy_copy.access, RX_AFE_DFE8, &val));
                break;
            case 8:
                PHYMOD_IF_ERR_RETURN
                    (blackhawk7_l8p2_read_rx_afe(&phy_copy.access, RX_AFE_DFE9, &val));
                break;
            case 9:
                PHYMOD_IF_ERR_RETURN
                    (blackhawk7_l8p2_read_rx_afe(&phy_copy.access, RX_AFE_DFE10, &val));
                break;
            case 10:
                PHYMOD_IF_ERR_RETURN
                    (blackhawk7_l8p2_read_rx_afe(&phy_copy.access, RX_AFE_DFE11, &val));
                break;
            case 11:
                PHYMOD_IF_ERR_RETURN
                    (blackhawk7_l8p2_read_rx_afe(&phy_copy.access, RX_AFE_DFE12, &val));
                break;
            case 12:
                PHYMOD_IF_ERR_RETURN
                    (blackhawk7_l8p2_read_rx_afe(&phy_copy.access, RX_AFE_DFE13, &val));
                break;
            case 13:
                PHYMOD_IF_ERR_RETURN
                    (blackhawk7_l8p2_read_rx_afe(&phy_copy.access, RX_AFE_DFE14, &val));
                break;
            default:
                return PHYMOD_E_PARAM;
        }
...
int blackhawk7_l8p2_phy_speed_config_set(const phymod_phy_access_t* phy,
...
int blackhawk7_l8p2_phy_speed_config_get(const phymod_phy_access_t* phy, phymod_phy_speed_config_t* speed_config)
{
    ...
    if (ref_clk_is_156p25) {
        PHYMOD_IF_ERR_RETURN
            (blackhawk7_l8p2_get_vco_from_refclk_div(&phy_copy.access, REF_CLOCK_156P25_HZ, pll_div, &vco_freq_khz, 0));
    } else {
        PHYMOD_IF_ERR_RETURN
            (blackhawk7_l8p2_get_vco_from_refclk_div(&phy_copy.access, REF_CLOCK_312P5_HZ, pll_div, &vco_freq_khz, 0));
    }
    ...
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_l8p2_phy_firmware_lane_config_get(&phy_copy, &firmware_lane_config));
    ...
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_l8p2_channel_loss_get(&phy_copy.access, &channel_loss));
...
int blackhawk7_l8p2_phy_cl72_set(const phymod_phy_access_t* phy, uint32_t cl72_en)
{
    ...
    if (cl72_en) {
        PHYMOD_IF_ERR_RETURN
            (blackhawk7_l8p2_phy_firmware_lane_config_get(phy, &firmware_lane_config));

        firmware_lane_config.ForceNormalReach = 0;
        firmware_lane_config.ForceExtenedReach = 0;

         PHYMOD_IF_ERR_RETURN
            (_blackhawk7_l8p2_phy_firmware_lane_config_set(phy, firmware_lane_config));
    } else {
        ...
        for (i = 0; i < num_lane; i++) {
            ...
            precoder_en = 0;
            PHYMOD_IF_ERR_RETURN
                (blackhawk7_l8p2_phy_tx_pam4_precoder_enable_get(&pm_phy_copy, &precoder_en));
            if (precoder_en) {
                PHYMOD_IF_ERR_RETURN
                    (blackhawk7_l8p2_phy_tx_pam4_precoder_enable_set(&pm_phy_copy, 0));
            }
            PHYMOD_IF_ERR_RETURN
                (blackhawk7_l8p2_phy_firmware_lane_config_get(&pm_phy_copy, &firmware_lane_config));

            if (firmware_lane_config.ForcePAM4Mode) {
                firmware_lane_config.ForceNormalReach = 1;
                firmware_lane_config.ForceExtenedReach = 0;
                PHYMOD_IF_ERR_RETURN
                    (_blackhawk7_l8p2_phy_firmware_lane_config_set(&pm_phy_copy, firmware_lane_config));
            }
        }
    }
    for (i = 0; i < num_lane; i++) {
...
int blackhawk7_l8p2_phy_loopback_set(const phymod_phy_access_t* phy, phymod_loopback_mode_t loopback, uint32_t enable)
{
    ...
    case phymodLoopbackGlobalPMD :
        for (i = 0; i < num_lane; i++) {
            ...
            PHYMOD_IF_ERR_RETURN(blackhawk7_l8p2_pmd_force_signal_detect(&phy_copy.access,  (int) enable, (int) enable));
        }
        break;
...
int blackhawk7_l8p2_phy_rx_pmd_locked_get(const phymod_phy_access_t* phy, uint32_t* pmd_lock)
{
    int start_lane, num_lane, i;
    phymod_phy_access_t pm_phy_copy;
    uint8_t tmp_lock;
...
int blackhawk7_l8p2_phy_rx_pmd_lock_status_get(const phymod_phy_access_t* phy,
                                               uint32_t* pmd_lock,
                                               uint32_t* pmd_lock_change)
{
    ...
    PHYMOD_MEMCPY(&pm_phy_copy, phy, sizeof(pm_phy_copy));
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_l8p2_rx_pmd_lock_status_get(&pm_phy_copy.access,
                                          pmd_lock,
                                          pmd_lock_change));
    return PHYMOD_E_NONE;
}
...
int blackhawk7_l8p2_phy_rx_signal_detect_get(const phymod_phy_access_t* phy, uint32_t* signal_detect)
{
    int start_lane, num_lane, i;
    phymod_phy_access_t pm_phy_copy;
    uint32_t tmp_detect;
...
{
    phymod_phy_access_t pm_phy_copy;
    uint8_t ver_id, afe_ver_a0 = 0, flr_support = 0;

    PHYMOD_MEMCPY(&pm_phy_copy, phy, sizeof(pm_phy_copy));

    PHYMOD_IF_ERR_RETURN(blackhawk7_l8p2_afe_version_get(&pm_phy_copy.access, &ver_id));
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_l8p2_INTERNAL_is_flr_supported(&pm_phy_copy.access, &flr_support));
    if (ver_id == AMS_VERSION_A0) {
       afe_ver_a0 = 1;
       info->ucode_len = BLACKHAWK7_V1L8P2_UCODE_IMAGE_SIZE;
    } else {
        if (flr_support) {
            info->ucode_len = BLACKHAWK7_V2L8P2_FLR_UCODE_IMAGE_SIZE;
        } else {
            info->ucode_len = BLACKHAWK7_V2L8P2_UCODE_IMAGE_SIZE;
        }
    }
    info->ucode_ptr = blackhawk7_l8p2_ucode_get(afe_ver_a0, flr_support);
...
int blackhawk7_l8p2_phy_rx_adaptation_resume(const phymod_phy_access_t* phy)
{
    ...
    for (i = 0; i < num_lane; i++) {
        ...
        PHYMOD_IF_ERR_RETURN(blackhawk7_l8p2_stop_uc_lane_status(&phy_copy.access, &uc_lane_stopped));
        if (uc_lane_stopped) {
            PHYMOD_IF_ERR_RETURN(blackhawk7_l8p2_stop_rx_adaptation(&phy_copy.access, 0));
        }
    }
    return PHYMOD_E_NONE;
}
...
int blackhawk7_l8p2_phy_pll_pwrdn(const phymod_phy_access_t* phy, uint32_t pll_index, uint32_t pwrdn)
{
    ...
    PHYMOD_IF_ERR_RETURN(blackhawk7_l8p2_pll_pwrdn_get(&phy_copy.access, &is_pwrdn));
    if (is_pwrdn != pwrdn) {
        if (pwrdn) {
            PHYMOD_IF_ERR_RETURN(blackhawk7_l8p2_core_pwrdn(&phy_copy.access, PWRDN));
        } else {
            PHYMOD_IF_ERR_RETURN(blackhawk7_l8p2_core_pwrdn(&phy_copy.access, PWR_ON));
        }
    }
...
int blackhawk7_l8p2_phy_tx_phase_lock_set(const phymod_phy_access_t* phy, uint8_t enable)
{
     phymod_phy_access_t phy_copy;
     PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));

     PHYMOD_IF_ERR_RETURN(blackhawk7_l8p2_ext_loop_timing(&phy_copy.access, enable));
     return PHYMOD_E_NONE;
}
...
int blackhawk7_l8p2_phy_eye_margin_est_get(const phymod_phy_access_t* phy, phymod_eye_margin_mode_t eye_margin_mode, uint32_t* value)
{
    ...
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_l8p2_INTERNAL_get_eye_margin_est(&phy_copy.access, &hz_l, &hz_r, &vt_u, &vt_d));
...
int blackhawk7_l8p2_phy_channel_loss_hint_set(const phymod_phy_access_t* phy,  uint32_t channel_loss)
{
    ...
    for (i = 0; i < num_lane; i++) {
        ...
        PHYMOD_IF_ERR_RETURN
            (blackhawk7_l8p2_channel_loss_set(&phy_copy.access, channel_loss));
    }
...
int blackhawk7_l8p2_phy_pmd_info_init(const phymod_phy_access_t* phy)
{
    ...
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_l8p2_init_blackhawk7_l8p2_info(&phy_copy.access));
