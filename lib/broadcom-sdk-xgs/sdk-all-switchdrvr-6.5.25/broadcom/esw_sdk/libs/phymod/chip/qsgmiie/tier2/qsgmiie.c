...
int qsgmiie_phy_power_set(const phymod_phy_access_t* phy, const phymod_phy_power_t* power)
{
    ...
    if (PHYMOD_ACC_F_QMODE_GET(&phy->access)) {
        qmode = 1;
        PHYMOD_IF_ERR_RETURN
            (tqmod_lane_info(&pm_phy_copy.access, &lane_id, &sub_port));
        PHYMOD_IF_ERR_RETURN(tqmod_port_state_get(&pm_phy_copy.access, TQMOD_PORT_STATE_CONFIGED, &subports));
    }

    if ((power->tx == phymodPowerOff) && (power->rx == phymodPowerOff)) {
        if (qmode) {
            PHYMOD_IF_ERR_RETURN(tqmod_port_state_set(&pm_phy_copy.access, TQMOD_PORT_STATE_TX_SQUELCH, sub_port, 1));
            PHYMOD_IF_ERR_RETURN(tqmod_port_state_set(&pm_phy_copy.access, TQMOD_PORT_STATE_RX_SQUELCH, sub_port, 1));

            PHYMOD_IF_ERR_RETURN(tqmod_port_state_get(&pm_phy_copy.access, TQMOD_PORT_STATE_TX_SQUELCH, &sq_value));
            if (sq_value == subports) {
                /* all subports want to squelch */
                PHYMOD_IF_ERR_RETURN(tqmod_tx_squelch_set(&pm_phy_copy.access, 1));
            }
            PHYMOD_IF_ERR_RETURN(tqmod_port_state_get(&pm_phy_copy.access, TQMOD_PORT_STATE_RX_SQUELCH, &sq_value));
            if (sq_value == subports) {
                /* all subports want to squelch */
    ...
    if ((power->tx == phymodPowerOn) && (power->rx == phymodPowerOn)) {
        if (qmode) {
            PHYMOD_IF_ERR_RETURN(tqmod_port_state_set(&pm_phy_copy.access, TQMOD_PORT_STATE_TX_SQUELCH, sub_port, 0));
            PHYMOD_IF_ERR_RETURN(tqmod_port_state_set(&pm_phy_copy.access, TQMOD_PORT_STATE_RX_SQUELCH, sub_port, 0));
        }
...
int qsgmiie_phy_power_get(const phymod_phy_access_t* phy, phymod_phy_power_t* power)
{
    ...
    if (PHYMOD_ACC_F_QMODE_GET(&phy->access)) {
        qmode = 1;
        PHYMOD_IF_ERR_RETURN
            (tqmod_lane_info(&phy->access, &lane_id, &sub_port));
    }
    ...
    /* PMD lane is not squelch */
    if ((!enable) && qmode) {
        PHYMOD_IF_ERR_RETURN(tqmod_port_state_get(&phy->access, TQMOD_PORT_STATE_RX_SQUELCH, &sq_value));
    ...
    /* PMD lane is not squelch */
    if ((!enable) && qmode) {
        PHYMOD_IF_ERR_RETURN(tqmod_port_state_get(&phy->access, TQMOD_PORT_STATE_TX_SQUELCH, &sq_value));
        enable = sq_value & (1 << sub_port);
    }
...
int qsgmiie_phy_tx_lane_control_set(const phymod_phy_access_t* phy, phymod_phy_tx_lane_control_t tx_control)
{
    ...
    if (PHYMOD_ACC_F_QMODE_GET(&phy->access)) {
        ...
        PHYMOD_IF_ERR_RETURN
            (tqmod_lane_info(&phy->access, &lane_id, &sub_port));
        PHYMOD_IF_ERR_RETURN(tqmod_port_state_get(&phy->access, TQMOD_PORT_STATE_CONFIGED, &subports));
    }

    switch (tx_control) {
    case phymodTxTrafficDisable:
        PHYMOD_IF_ERR_RETURN(tqmod_tx_lane_control_set(&phy->access, TQMOD_TX_LANE_TRAFFIC_DISABLE));
        break;
    case phymodTxTrafficEnable:
        PHYMOD_IF_ERR_RETURN(tqmod_tx_lane_control_set(&phy->access, TQMOD_TX_LANE_TRAFFIC_ENABLE));
        break;
    case phymodTxReset:
        PHYMOD_IF_ERR_RETURN(tqmod_tx_lane_control_set(&phy->access, TQMOD_TX_LANE_RESET));
        break;
    case phymodTxSquelchOn:
        if (qmode) {
            PHYMOD_IF_ERR_RETURN(tqmod_port_state_set(&phy->access, TQMOD_PORT_STATE_TX_SQUELCH, sub_port, 1));
            PHYMOD_IF_ERR_RETURN(tqmod_port_state_get(&phy->access, TQMOD_PORT_STATE_TX_SQUELCH, &sq_value));
            if (sq_value == subports) {
                /* all subports want to squelch */
    ...
    case phymodTxSquelchOff:
        if (qmode) {
            PHYMOD_IF_ERR_RETURN(tqmod_port_state_set(&phy->access, TQMOD_PORT_STATE_TX_SQUELCH, sub_port, 0));
        }
...
int qsgmiie_phy_tx_lane_control_get(const phymod_phy_access_t* phy, phymod_phy_tx_lane_control_t* tx_control)
{
    ...
    /* Physical lane is not squelch */
    if ((!enable) && PHYMOD_ACC_F_QMODE_GET(&phy->access)) {
        PHYMOD_IF_ERR_RETURN
            (tqmod_lane_info(&pm_phy_copy.access, &lane_id, &sub_port));
        PHYMOD_IF_ERR_RETURN(tqmod_port_state_get(&pm_phy_copy.access, TQMOD_PORT_STATE_TX_SQUELCH, &sq_value));
        enable = sq_value & (1 << sub_port);
...
int qsgmiie_phy_rx_lane_control_set(const phymod_phy_access_t* phy, phymod_phy_rx_lane_control_t rx_control)
{
    ...
    case phymodRxSquelchOn:
        if (qmode) {
            PHYMOD_IF_ERR_RETURN(tqmod_port_state_set(&phy->access, TQMOD_PORT_STATE_RX_SQUELCH, sub_port, 1));
            PHYMOD_IF_ERR_RETURN(tqmod_port_state_get(&phy->access, TQMOD_PORT_STATE_RX_SQUELCH, &sq_value));
            if (sq_value == subports) {
                /* all subports want to squelch */
                PHYMOD_IF_ERR_RETURN(tqmod_rx_squelch_set(&phy->access, 1));
            }
        } else {
            PHYMOD_IF_ERR_RETURN(tqmod_rx_squelch_set(&phy->access, 1));
        }
    ...
    case phymodRxSquelchOff:
        if (qmode) {
            PHYMOD_IF_ERR_RETURN(tqmod_port_state_set(&phy->access, TQMOD_PORT_STATE_RX_SQUELCH, sub_port, 0));
        }
        PHYMOD_IF_ERR_RETURN(tqmod_rx_squelch_set(&phy->access, 0));
        break;
    default:
        break;
    }
...
int qsgmiie_phy_init(const phymod_phy_access_t* phy, const phymod_phy_init_config_t* init_config)
{
    ...
    if (PHYMOD_ACC_F_QMODE_GET(&phy->access)) {
        PHYMOD_IF_ERR_RETURN
            (tqmod_lane_info(&pm_phy_copy.access, &lane_id, &sub_port));
        PHYMOD_IF_ERR_RETURN(tqmod_port_state_set(&pm_phy_copy.access, TQMOD_PORT_STATE_CONFIGED, sub_port, 1));
    }
