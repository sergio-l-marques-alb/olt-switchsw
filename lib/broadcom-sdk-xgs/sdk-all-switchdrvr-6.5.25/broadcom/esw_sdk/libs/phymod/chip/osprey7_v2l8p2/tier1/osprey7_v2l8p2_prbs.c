...
static const char* osprey7_v2l8p2_e2s_prbs_mode_enum[22] = {
    " PRBS_7",
    " PRBS_9",
    "PRBS_11",
    "PRBS_15",
...
err_code_t osprey7_v2l8p2_display_detailed_prbs_state(srds_access_t *sa__) {
    ...
    if(enabled) {
        ...
        if(prbs_poly_mode == PRBS_UNKNOWN) {
            ...
            prbs_string =  (char *)osprey7_v2l8p2_e2s_prbs_mode_enum[prbs_poly_mode];
        }
...
err_code_t osprey7_v2l8p2_prbs_error_analyzer_reset(srds_access_t *sa__) {
    ...
    EFUN(osprey7_v2l8p2_clear_dac_pattern_memory(sa__));  /* Clear DAC pattern memory */
    return (ERR_CODE_NONE);
}
...
err_code_t osprey7_v2l8p2_prbs_error_analyzer_config(srds_access_t *sa__, osprey7_v2l8p2_prbs_err_analyzer_config_status_st *err_analyzer) {
    ...
    fec_size = (uint8_t)((err_analyzer->prbs_err_fec_size + OSPREY7_V2L8P2_PRBS_FEC_FRAME_SIZE_ROUND_UP) / OSPREY7_V2L8P2_PRBS_FEC_FRAME_SIZE_MULTIPLIER);
...
err_code_t osprey7_v2l8p2_display_prbs_error_analyzer_config(srds_access_t *sa__, osprey7_v2l8p2_prbs_err_analyzer_config_status_st *err_analyzer, uint32_t timeout_s) {
    ...
    {
        EFUN_PRINTF(("%u %hu %hhu %u %hhu %hhu %hhu %08x %08x\n",   timeout_s, err_analyzer->prbs_err_fec_size, err_analyzer->fec_size_frac, err_analyzer->prbs_err_aggregate_mode, err_analyzer->lanes_active, err_analyzer->diag_mode, osprey7_v2l8p2_get_lane(sa__), err_analyzer->pp_field2, err_analyzer->pp_field1));
    }
    else
    {
        EFUN_PRINTF(("\n ****************************************************************************** \n"));
...
err_code_t osprey7_v2l8p2_get_prbs_error_analyzer_aggregate_err_count(srds_access_t *sa__, osprey7_v2l8p2_prbs_err_analyzer_config_status_st *err_analyzer) {
    ...
    EFUN(osprey7_v2l8p2_get_prbs_error_analyzer_lane_err_count(sa__, err_analyzer, num_lanes)); /* For aggregate counters, just pass the num_lanes to point to the correct memory location in the DAC */

    return (ERR_CODE_NONE);
}

static err_code_t _osprey7_v2l8p2_display_fec_frame_error(srds_access_t *sa__, uint8_t errs_per_frame, uint32_t num_frame_errs) {
...
err_code_t osprey7_v2l8p2_display_prbs_error_analyzer_err_count(srds_access_t *sa__, osprey7_v2l8p2_prbs_err_analyzer_config_status_st *err_analyzer) {
    ...
    else
    {
        ...
        for (i=0; i<OSPREY7_V2L8P2_PRBS_NUM_OF_ERROR_ANALYZER_COUNTERS; i++) {
            EFUN(_osprey7_v2l8p2_display_fec_frame_error(sa__, i, err_analyzer->prbs_errcnt[i]));
        }
        EFUN_PRINTF((" -------------------------------------------------------------\n"));
    }
...
err_code_t osprey7_v2l8p2_prbs_error_analyzer_aggregate_config(srds_access_t *sa__, osprey7_v2l8p2_prbs_err_analyzer_config_status_st *err_analyzer) {
    ...
    for(lane = 0;lane<num_lanes;lane++) {
        if ((err_analyzer->lane_mask_aggregate >> lane) & 0x1) {
            EFUN(osprey7_v2l8p2_acc_set_lane(sa__,lane));
            EFUN(osprey7_v2l8p2_prbs_error_analyzer_config(sa__, err_analyzer));  /* Configure PRBS Error Analyzer */
...
err_code_t osprey7_v2l8p2_display_aggregate_prbs_error_analyzer_proj(srds_access_t *sa__, uint16_t prbs_err_fec_size, enum srds_prbs_error_analyzer_aggregate_mode_enum prbs_err_aggregate_mode, uint8_t lane_mask_aggregate, uint32_t timeout_s, uint8_t diag_mode) {
    ...
    EFUN(osprey7_v2l8p2_prbs_error_analyzer_reset(sa__));  /* Reset PRBS Error Analyzer and clear DAC */
    EFUN(osprey7_v2l8p2_prbs_error_analyzer_aggregate_config(sa__, &err_analyzer));  /* Configure PRBS Error Analyzer Aggregation Mode*/
    ...
    for(lane = 0;lane<num_lanes;lane++) {
        if ((err_analyzer.lane_mask_aggregate >> lane) & 0x1) {
            EFUN(osprey7_v2l8p2_acc_set_lane(sa__,lane));
            EFUN(osprey7_v2l8p2_prbs_error_analyzer_start(sa__));  /* Start PRBS Error Analyzer */
        }
    }
