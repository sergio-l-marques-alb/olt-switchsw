...
#define BCM_TRUNK_FLAG_VP_UNDERLAY          0x40000    /* If set, Create a
                                                          virtual port trunk
                                                          group at ECMP level 2. */
...
/* Trunk member block type. */
#define BCM_TRUNK_BLOCK_TYPE_L2MC   1          /* L2MC type. */
#define BCM_TRUNK_BLOCK_TYPE_IPMC   2          /* IPMC type. */
#define BCM_TRUNK_BLOCK_TYPE_BCAST  4          /* Broadcast type. */
#define BCM_TRUNK_BLOCK_TYPE_DLF    8          /* Unknown unicast and unknown
                                                  multicast. */

/* Key of trunk member block. */
typedef struct bcm_trunk_member_block_key_s {
    bcm_trunk_t tid;    /* Trunk ID. */
    uint32 type;        /* Trunk member block type. */
    int index;          /* Index to member block table on a given block type. */
} bcm_trunk_member_block_key_t;

/* Initializes the bcm_trunk_member_block_key_t structure */
extern void bcm_trunk_member_block_key_t_init(
    bcm_trunk_member_block_key_t *trunk_info);

#ifndef BCM_HIDE_DISPATCHABLE

/* Set block port bitmap on a given key. */
extern int bcm_trunk_member_block_set(
    int unit, 
    bcm_trunk_member_block_key_t *key, 
    bcm_pbmp_t block_pbmp);

/* Get block port bitmap on a given key. */
extern int bcm_trunk_member_block_get(
    int unit, 
    bcm_trunk_member_block_key_t *key, 
    bcm_pbmp_t *block_pbmp);

/* 
 * Set all the block port bitmap entries on a given trunk and block
 * types.
 */
extern int bcm_trunk_member_block_set_all(
    int unit, 
    bcm_trunk_member_block_key_t *key, 
    int array_size, 
    bcm_pbmp_t *block_pbmp_array);

/* Get all the block port bitmap entries on a given trunk and block type. */
extern int bcm_trunk_member_block_get_all(
    int unit, 
    bcm_trunk_member_block_key_t *key, 
    int array_size, 
    bcm_pbmp_t *block_pbmp_array, 
