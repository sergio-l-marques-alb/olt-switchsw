...
#define BCM_MIRROR_DEST_REPLACE             (1U << 0)  /* Replace a mirror
                                                          destination. Need to
                                                          additionally detach
                                                          mirror sources from
                                                          the old mirror
                                                          destination and
                                                          re-attach to the new
                                                          mirror destination. */
...
#define BCM_MIRROR_DEST_FLAGS2_BTE_TIMESTAMP_UPDATE (1U << 10) /* Enable timestamp
                                                          update in BTE. */
#define BCM_MIRROR_DEST_FLAGS2_PAD_TO_CROP_SIZE_ENABLE (1U << 11) /* Allow padding to crop
                                                          size */
#define BCM_MIRROR_DEST_FLAGS2_OVERWRITE    (1U << 12) /* Overwrite a mirror
                                                          destination. Don't
                                                          need to detach mirror
                                                          sources from the old
                                                          mirror destination and
                                                          re-attach to the new
                                                          mirror destination. */
...
/* GRE protocol ethertypes for ERSPAN. */
#define BCM_MIRROR_ERSPAN1_GRE_PROTOCOL_TYPE 0x88BE     /* ERSPAN type I and IIs
                                                          ethertype. */
#define BCM_MIRROR_ERSPAN2_GRE_PROTOCOL_TYPE 0x22EB     /* ERSPAN type III
                                                          ethertype. */
...
#define BCM_MIRROR_ADMIT_PROFILES_NOF   8          /* Number of admit profiles. */
...
typedef struct bcm_mirror_destination_s {
    ...
    int ip_proto;                       /* IP Protocol number in ERSPAN IP
                                           encap. */
    uint32 switch_id;                   /* Switch ID in the PSAMP mirror-on-drop
                                           encapsulation header. */
    uint32 fabric_mirror_flow_profile;  /* Fabric mirror flow profile */
    int drop_group_bmp_array[BCM_MIRROR_ADMIT_PROFILES_NOF]; /* Mirror on drop groups bitmap per
                                           admit profile. */
...
typedef struct bcm_mirror_pkt_dnx_pp_header_s {
    uint8 tail_edit_profile;        /* Tail edit profile, used at ETPP for tail
                                       editing */
    uint8 bytes_to_remove;          /* Bytes to remove from the start of the
                                       packet */
    uint8 eth_header_remove;        /* Indication to remove Ethernet header from
                                       mirror copy */
    uint8 force_mirror;             /* Indication to force mirroring processing
                                       packet at egress */
...
typedef struct bcm_mirror_sample_profile_s {
    ...
    uint8 mirror;                       /* Copy sampled packets to all mirror
                                           destinations created with
                                           BCM_MIRROR_PORT_SFLOW flag. */
...
/* Sample statistics enum type. */
typedef enum bcm_mirror_sample_stat_type_e {
    bcmMirrorSampleStatFlexPkts = 0,    /* Total number of packets eligible to
                                           be sampled by Flex sampler enabled by
                                           IFP ingress sampling policy action. */
    bcmMirrorSampleStatFlexSampledPkts = 1, /* Total number of packets sampled by
                                           Flex sampler enabled by IFP ingress
                                           sampling policy action. */
    bcmMirrorSampleStatFlexSnapshotPkts = 2, /* Total number of eligible packets when
                                           the last packet was sampled by Flex
                                           sampler enabled by IFP ingress
                                           sampling policy action. */
    bcmMirrorSampleStatCount = 3        /* The last one, invalid. */
} bcm_mirror_sample_stat_type_t;

#ifndef BCM_HIDE_DISPATCHABLE

/* Set counter value for mirror sample statistic type. */
extern int bcm_mirror_sample_stat_set(
    int unit, 
    int profile_id, 
    bcm_mirror_sample_stat_type_t type, 
    uint64 value);

/* Get counter value for mirror sample statistic type. */
extern int bcm_mirror_sample_stat_get(
    int unit, 
    int profile_id, 
    bcm_mirror_sample_stat_type_t type, 
    uint64 *value);
...
/* 
 * Get counter value for mirror sample statistic type from hardware
 * memory.
 */
extern int bcm_mirror_sample_stat_sync_get(
    int unit, 
    int profile_id, 
    bcm_mirror_sample_stat_type_t type, 
    uint64 *value);
...
/* Mirror source flags. */
#define BCM_MIRROR_SOURCE_INGRESS           (1U << 0)  /* Ingress Mirroring. */
#define BCM_MIRROR_SOURCE_EGRESS            (1U << 1)  /* Egress Mirroring. */
#define BCM_MIRROR_SOURCE_INGRESS_EVENT_ALL (1U << 2)  /* Enable all ingress
                                                          events. */
#define BCM_MIRROR_SOURCE_WITH_PORT         (1U << 3)  /* Apply to specified
                                                          ports. */
...
typedef struct bcm_mirror_source_s {
    ...
    bcm_pbmp_t ports;                   /* Member ports for trace and drop
                                           events. Valid only if
                                           BCM_MIRROR_SOURCE_WITH_PORT is set. */
} bcm_mirror_source_t;
...
/* Mirror encapsulation instance flags. */
#define BCM_MIRROR_ENCAP_INSTANCE_INGRESS   (1U << 0)  /* Ingress Mirroring. */
#define BCM_MIRROR_ENCAP_INSTANCE_EGRESS    (1U << 1)  /* Egress Mirroring. */

/* Mirror encapsulation instance structure. */
typedef struct bcm_mirror_encap_instance_s {
    uint32 flags;               /* See BCM_MIRROR_ENCAP_INSTANCE_xxx flag
                                   definitions. */
    bcm_gport_t mirror_dest_id; /* Mirror destination id. */
} bcm_mirror_encap_instance_t;

/* Initialize mirror encapsulation instance structure. */
extern void bcm_mirror_encap_instance_t_init(
    bcm_mirror_encap_instance_t *instance);

/* Mirror encapsulation information structure. */
typedef struct bcm_mirror_encap_info_s {
    uint32 encap_id;    /* Mirror encapsulation id. */
} bcm_mirror_encap_info_t;

/* Initialize mirror encapsulation information structure. */
extern void bcm_mirror_encap_info_t_init(
    bcm_mirror_encap_info_t *info);

#ifndef BCM_HIDE_DISPATCHABLE

/* Get encapsulation information for a given mirror instance. */
extern int bcm_mirror_encap_info_get(
    int unit, 
    bcm_mirror_encap_instance_t *encap_instance, 
    bcm_mirror_encap_info_t *encap_info);
...
typedef enum bcm_mirror_ingress_mod_event_e {
    ...
    bcmMirrorIngModEventOlUlNhopEifRangeError = 144, /* Enable to select overlay and underlay
                                           ...
                                           range error drop. */
    bcmMirrorIngModEventINTDataplaneTurnaround = 145, /* Enable to select In-band Network
                                           Telemetry Data-plane Probe (INT-DP)
                                           turned around packets copied to CPU. */
    bcmMirrorIngModEventINTDataplaneHopLimitExceeded = 146, /* Enable to select In-band Network
                                           Telemetry Data-plane Probe (INT-DP)
                                           packets with HOP_COUNT no less than
                                           HOP_LIMIT copied to CPU. */
    bcmMirrorIngModEventVxlanVfiAssignFailToCpu = 147, /* Enable to select packet copied due to
                                           VXLAN packets with VFI assignment
                                           failure. */
    bcmMirrorIngModEventVxlanVfiAssignFailDrop = 148, /* Enable to select packet dropped due
                                           to VXLAN packets with VFI assignment
                                           failure. */
    bcmMirrorIngModEventVxlanSvpAssignFailToCpu = 149, /* Enable to select packet copied due to
                                           VXLAN packets with source virtual
                                           port assignment failure. */
    bcmMirrorIngModEventVxlanSvpAssignFailDrop = 150, /*  Enable to select packet dropped due
                                           to VXLAN packets with source virtual
                                           port assignment failure. */
    bcmMirrorIngModEventVxlanTnlErrDrop = 151, /* Enable to select packets dropped due
                                           to VXLAN tunnel errors. */
    bcmMirrorIngModEventLinkDownDrop = 152, /* Enable to select packets dropped due
                                           to link down or destination port
                                           disabled. */
    bcmMirrorIngModEventPartialLinkDownDrop = 153, /* Enable to select non-UC packets
                                           dropped partially due to link down or
                                           destination port disabled. */
    bcmMirrorIngModEventPriNextHopDownDrop = 154, /* Enable to select packets dropped due
                                           to primary next hop down and no
                                           secondary next hop is up or selected. */
    bcmMirrorIngModEventPriSecNextHopDownDrop = 155, /* Enable to select packets dropped due
                                           to primary and secondary next hops
                                           down and no tertiary next hop is up
                                           or selected. */
    bcmMirrorIngModEventCount = 156     /* Must be last. Not a valid value. */
