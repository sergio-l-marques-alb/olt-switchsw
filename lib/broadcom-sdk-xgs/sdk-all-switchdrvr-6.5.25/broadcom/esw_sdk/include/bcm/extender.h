...
/* Key selection for SVP assignment. */
typedef enum bcm_extender_svp_assignment_key_e {
    BCM_EXTENDER_SVP_ASSIGN_KEY_DISABLE         = 0, 
    BCM_EXTENDER_SVP_ASSIGN_KEY_EVID_VXLT       = 1, 
    BCM_EXTENDER_SVP_ASSIGN_KEY_EVID_PTGRP      = 2, 
    BCM_EXTENDER_SVP_ASSIGN_KEY_EVID_PTGRP_NS   = 3 
} bcm_extender_svp_assignment_key_t;
...
typedef struct bcm_extender_port_s {
    ...
    int port_group_id;                  /* Port Group ID to match */
    int ingress_qos_map_id;             /* QOS map ID for ingress ETAG mapping
                                           profile. */
} bcm_extender_port_t;
...
/* Attach counter entries to the given port extender entry. */
extern int bcm_extender_forward_stat_attach(
    int unit, 
    bcm_extender_forward_t *extender_forward_entry, 
    uint32 stat_counter_id);

/* Detach counter entries from the given port extender entry. */
extern int bcm_extender_forward_stat_detach(
    int unit, 
    bcm_extender_forward_t *extender_forward_entry);

/* Get stat counter ID associated with the given port extender entry. */
extern int bcm_extender_forward_stat_id_get(
    int unit, 
    bcm_extender_forward_t *extender_forward_entry, 
    uint32 *stat_counter_id);
