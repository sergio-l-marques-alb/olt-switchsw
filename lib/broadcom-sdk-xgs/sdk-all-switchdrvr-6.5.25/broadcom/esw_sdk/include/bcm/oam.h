...
#define BCM_OAM_GROUP_FLEXIBLE_MAID_40_BYTE 0x400      /* Only 40 first bytes of
                                                          ...
                                                          will be allocated. */
#define BCM_OAM_GROUP_CALLBACK_ENABLE_EVENTS 0x800      /* In case set, without
                                                          events set in
                                                          group_event_types then
                                                          it will disable
                                                          default callback
                                                          notifications for all
                                                          the events.In case
                                                          set, with events set
                                                          in group_event_types
                                                          then it will enable
                                                          the callback
...
#define BCM_OAM_ENDPOINT_FLAGS2_ADDITIONAL_GAL_SPECIAL_LABEL 0x1000000  /* The additional GAL
                                                          ...
                                                          MEP */
#define BCM_OAM_ENDPOINT_FLAGS2_TRUNK_WITHOUT_MEMBERS 0x2000000  /* Allow to create OAM
                                                          endpoint on trunk
                                                          without member ports */
#define BCM_OAM_ENDPOINT_FLAGS2_EGRESS_INJECTION_DOWN_IVEC_DELETE_ONE_VLAN_TAG 0x4000000  /* In case set, egress
                                                          injection down
                                                          endpoint will use
                                                          ingress VLAN edit
                                                          command to delete one
                                                          tag. */
#define BCM_OAM_ENDPOINT_FLAGS2_EGRESS_INJECTION_DOWN_IVEC_DELETE_TWO_VLAN_TAGS 0x8000000  /* In case set, egress
                                                          injection down
                                                          endpoint will use
                                                          ingress VLAN edit
                                                          command to delete two
                                                          tags. */
#define BCM_OAM_ENDPOINT_FLAGS2_CALLBACK_ENABLE_EVENTS 0x10000000 /* In case set, without
                                                          events set in
                                                          rmep_event_types then
                                                          it will disable
                                                          default callback
                                                          notifications for all
                                                          the events. In case
...
#define BCM_OAM_OPCODE_OTHER_DROP           0x10000    /* Drop other OAM opcode
                                                          to CPU */
#define BCM_OAM_OPCODE_LMM_IN_HW            0x20000    /* Endpoint is LM */
#define BCM_OAM_OPCODE_DMM_IN_HW            0x40000    /* Single ended DM */
#define BCM_OAM_OPCODE_SLM_IN_HW            0x80000    /* Endpoint is SLM */
#define BCM_OAM_OPCODE_1DM_IN_HW            0x100000   /* Dual ended DM */
#define BCM_OAM_OPCODE_DUAL_ENDED_LM        0x200000   /* Dual ended LM (CCM
                                                          based) */
#define BCM_OAM_OPCODE_DMM_ONE_WAY_STATS    0x400000   /* One Way stats when
                                                          using single ended DM */
...
/* Egress DB lookup definition */
typedef enum bcm_oam_encap_db_e {
    bcmOamEncapDbRmepCCM = 0,       /* Down MEP CCM, including MPLS/PWE */
    bcmOamEncapDbRmep = 1,          /* BFD, TWAMP */
    bcmOamEncapDbLmDm = 2,          /* LM/DM Down MEP */
    bcmOamEncapDbLmDmPriority = 3,  /* LM/DM with priority Down MEP */
    bcmOamEncapDbSlr = 4,           /* SLR Down MEP */
    bcmOamEncapDbUpMep = 5          /* Up MEP, second pass */
} bcm_oam_encap_db_t;
...
#define BCM_OAM_DELAY_TX_ENABLE             0x0001     /* Enable Delay Packet
                                                          Transmission */
#define BCM_OAM_DELAY_ONE_WAY               0x0002     /* Dual ended DM */
...
#define BCM_OAM_DELAY_ONE_WAY_STATS         0x80       /* One Way stats when
                                                          using single ended DM. */
...
typedef enum bcm_oam_control_type_e {
    ...
    bcmOamControlEthDmacMdlMatchCheck = 36, /* Enable/Disable checking the match
                                           between DMAC LSB and MDL for Ethernet
                                           OAM */
    bcmOamControlEnableBfdPhp = 37,     /* Enable/Disable classifying BFD over
                                           PHP packet */
    bcmOamControlIngressVlanEditClassDeleteOneVlanTag = 38, /* Indicate the ingress VLAN edit
                                           command ID for delete one tag in OAMP
                                           used for DownMEP Egress inject
                                           (PP-injected) OAM packets */
    bcmOamControlIngressVlanEditClassDeleteTwoVlanTags = 39, /* Indicate the ingress VLAN edit
                                           command ID for delete two tags in
                                           OAMP used for DownMEP Egress inject
                                           (PP-injected) OAM packets */
    bcmOamControlOampInjectedNetworkQos = 40, /* Controls priority field on headers
                                           encapsulated through the OAMP outLIF
                                           fields for OAM/BFD */
    bcmOAMControlCount = 41             /* Always Last. Not a usable value. */
...
/* OAM event callback flags */
#define BCM_OAM_EVENT_FLAGS_IGNORE_GROUP_RMEP_ID 0x02       /* Indicates that ignore
                                                          MA group id and RMEP
                                                          id of the event in the
                                                          event callback
                                                          function. */
...
typedef struct bcm_oam_group_info_s {
    ...
    bcm_oam_event_types_t group_event_types; /* A collection of MA group event types
                                           for which the specified callback will
                                           be called */
...
typedef struct bcm_oam_endpoint_info_s {
    ...
    bcm_oam_endpoint_t local_id;        /* Used by remote and client endpoints. */
    ...
    bcm_oam_event_types_t rmep_event_types; /* A collection of RMEP event types for
                                           which the specified callback will be
                                           called */
...
typedef struct bcm_oam_loss_s {
    ...
    uint32 test_id;                     /* Test ID to differentiate each SLM
                                           ...
                                           sessions running between a pair of
                                           MEPs. */
    uint8 inner_pkt_pri;                /* Inner vlan packet priority for the LM
                                           frame. */
} bcm_oam_loss_t;
...
typedef struct bcm_oam_tst_tx_s {
    ...
    int gtf_id; 
    int gtf_packet_length_compensation; /* Adjust the GTF bandwidth */
    uint32 flags; 
...
typedef struct bcm_oam_performance_event_data_s {
    ...
    uint32 Tx_FCf;          /* Tx_FCf. Used for performance events of type
                               bcmOAMEventEndpointLMStatistics. */
    uint32 Rx_FCf;          /* Rx_FCf. Used for performance events of type
                               bcmOAMEventEndpointLMStatistics. */
    uint32 Tx_FCb;          /* Tx_FCb. Used for performance events of type
                               bcmOAMEventEndpointLMStatistics. */
    uint32 Rx_FCb;          /* Rx_FCb. Used for performance events of type
                               bcmOAMEventEndpointLMStatistics. */
    uint64 Tx_Timef;        /* Used for performance events of type
                               bcmOAMEventEndpointDMStatistics. */
    uint64 Rx_Timef;        /* Used for performance events of type
                               bcmOAMEventEndpointDMStatistics. */
    uint64 Tx_Timeb;        /* Used for performance events of type
                               bcmOAMEventEndpointDMStatistics. */
    uint64 Rx_Timeb;        /* Used for performance events of type
                               bcmOAMEventEndpointDMStatistics. */
...
typedef struct bcm_oam_pm_stats_s {
    ...
    uint32 num_loss_measurement_periods; /* Number of elapsed loss measurement
                                           period. */
} bcm_oam_pm_stats_t;
...
/* Get the processed stats for a given loss ID */
extern int bcm_oam_pm_loss_stats_get(
    int unit, 
    int loss_id, 
    bcm_oam_pm_stats_t *stats_ptr);
...
#define BCM_OAM_PROFILE_HIERARCHICAL_LM_DISABLE 0x20       /* Disable Hierarchical
                                                          LM by MD-level. */
#define BCM_OAM_PROFILE_DM_TYPE_NONE        0x40       /* Disable stamping
                                                          timestamp on DMM/DMR
                                                          packets. */
...
typedef enum bcm_oam_match_type_e {
    ...
    bcmOAMMatchTypeLowMDLMIP = 4, 
    bcmOAMMatchTypeCount = 5 
