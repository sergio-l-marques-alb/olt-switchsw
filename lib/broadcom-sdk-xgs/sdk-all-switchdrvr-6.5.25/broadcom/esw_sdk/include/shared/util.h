...
extern int shr_util_popcount(uint32 n);
extern int shr_util_log2(uint32 n);
extern int shr_util_xch2i(int ch);
extern unsigned short shr_ip_chksum(unsigned int length, unsigned char *data);
extern void shr_util_format_uint32(char *buf, uint32 n, int min_digits, int base);
extern void *shr_util_realloc(void *old_ptr, size_t old_size, size_t size, char *str);
extern char *shr_util_strdup(const char *str, char *s);

extern int shr_i2xdigit(int digit);
extern void shr_format_macaddr(char buf[SAL_MACADDR_STR_LEN], sal_mac_addr_t macaddr);
extern void shr_format_ipaddr(char buf[SAL_IPADDR_STR_LEN], sal_ip_addr_t ipaddr);
extern void shr_format_ip6addr(char buf[SAL_IP6ADDR_STR_LEN], sal_ip6_addr_t ipaddr);
