...
/**
 * Find DB location as per DNX-Devices.xml, do not look for default DB location.
 * Do not use for global DB files
 */
#define  CONF_OPEN_CURRENT_DEVICE_ONLY       0x0040
 ...
 * \par DIRECT INPUT
 *   \param [in] node - xml handle for node
 *   \param [in] property - property to be set in the node
 *   \param [in] target - pointer to string value of property to be modified
 *   \param [in] target_size - size for target memory allocation
 * \par DIRECT OUTPUT:
 *   \retval _SHR_E_NONE - success
 *   \retval _SHR_E_INTERNAL - only failure
 */
...
#define RHDATA_GET_INT_EXIT_IF_NOT_FOUND(node_mac, string_mac, target_mac)                                                          \
    SHR_IF_ERR_EXIT_EXCEPT_IF(dbx_xml_property_get_int(node_mac, string_mac, &(target_mac)), _SHR_E_NOT_FOUND)         \
