...
extern int sal_console_init(void) SAL_ATTR_WEAK;
#ifndef	SAL_THREAD_NAME_PRINT_DISABLE
extern int sal_console_thread_name_print_enable(int enable) SAL_ATTR_WEAK;
#endif
extern int sal_console_info_get(sal_console_info_t *info) SAL_ATTR_WEAK;
extern int sal_console_write(const void *buf, int count) SAL_ATTR_WEAK;
extern int sal_console_read(void *buf, int count) SAL_ATTR_WEAK;
extern char *sal_console_gets(char *buf, int size) SAL_ATTR_WEAK;
extern int sal_printf(const char *fmt, ...)
    COMPILER_ATTRIBUTE ((format (printf, 1, 2))) SAL_ATTR_WEAK;
extern int sal_vprintf(const char *fmt, va_list varg)
    COMPILER_ATTRIBUTE ((format (printf, 1, 0))) SAL_ATTR_WEAK;

extern int sal_open(char *name, int oflag, ...) SAL_ATTR_WEAK;
extern int sal_close(int fd) SAL_ATTR_WEAK;

extern FILE *sal_fopen(char *name, char *mode) SAL_ATTR_WEAK;

extern int sal_fclose(FILE *fp) SAL_ATTR_WEAK;

extern int sal_fread(void *buf, int size, int num, FILE *fp) SAL_ATTR_WEAK;
extern int sal_feof(FILE *fp) SAL_ATTR_WEAK;
extern int sal_ferror(FILE *fp) SAL_ATTR_WEAK;
extern int sal_fsize(FILE *fp) SAL_ATTR_WEAK;

extern int sal_homedir_set(char *dir) SAL_ATTR_WEAK;
extern char *sal_homedir_get(char *buf, size_t size) SAL_ATTR_WEAK;

extern int sal_umask(int mask) SAL_ATTR_WEAK;
extern char *sal_getcwd(char *buf, size_t size) SAL_ATTR_WEAK;
extern int sal_ls(char *filename, char *flags) SAL_ATTR_WEAK;
extern int sal_cd(char *filename) SAL_ATTR_WEAK;
extern int sal_remove(char *filename) SAL_ATTR_WEAK;
extern int sal_rename(char *file_old, char *file_new) SAL_ATTR_WEAK;
extern int sal_mkdir(char *path) SAL_ATTR_WEAK;
extern int sal_rmdir(char *path) SAL_ATTR_WEAK;
extern char *sal_dirname(char *path, char *dest) SAL_ATTR_WEAK;
extern char *sal_basename(char *path, char *dest) SAL_ATTR_WEAK;
...
#else
extern size_t sal_fwrite(const void *ptr, size_t size, size_t nmemb, FILE *stream) SAL_ATTR_WEAK;
extern char* sal_fgets(char *s, int size, FILE *fp) SAL_ATTR_WEAK;
...
extern int sal_fgetc(FILE *fp) SAL_ATTR_WEAK;
extern int sal_fputc(int c, FILE *fp) SAL_ATTR_WEAK;
extern int sal_fputs(const char *s, FILE *fp) SAL_ATTR_WEAK;
extern int sal_isatty (int ) SAL_ATTR_WEAK;
extern int sal_fflush(FILE *fp) SAL_ATTR_WEAK;
#endif
...
extern SAL_DIR *sal_opendir(char *dirName) SAL_ATTR_WEAK;
extern int sal_closedir(SAL_DIR *pDir) SAL_ATTR_WEAK;
extern struct sal_dirent *sal_readdir(SAL_DIR *pDir);
extern void sal_rewinddir(SAL_DIR *pDir) SAL_ATTR_WEAK;

/*
 * Command line editor I/O
 */

extern char* sal_readline(char *prompt, char *buf, int bufsize, char *defl) SAL_ATTR_WEAK;
extern void sal_readline_config(char *(*complete)(char *pathname, int *unique),
			    int (*list_possib)(char *pathname, char ***avp)) SAL_ATTR_WEAK;
extern int sal_readchar(const char *prompt) SAL_ATTR_WEAK;

extern int sal_flash_init(int format) SAL_ATTR_WEAK;
extern int sal_flash_boot(char *file) SAL_ATTR_WEAK;
extern int sal_flash_sync(void) SAL_ATTR_WEAK;
