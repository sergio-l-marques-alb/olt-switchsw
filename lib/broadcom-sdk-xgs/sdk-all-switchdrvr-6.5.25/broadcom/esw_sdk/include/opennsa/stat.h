...
typedef enum bcm_stat_val_e {
    ...
    snmpBcmCustomTransmit15 = 289,      /* Broadcom-specific */
    snmpValCount = 290 
} bcm_stat_val_t;
...
typedef enum bcm_custom_stat_trigger_e {
    ...
    bcmDbgCntRxVxlanDecapSrcIpLookupMiss = 205, /* Rx: VXLAN packet dropped due to
                                           ...
                                           decapsulation. */
    bcmDbgCntTxAdaptLookupMiss = 206,   /* Tx: packets dropped due to egress
                                           adapt lookup miss. */
    bcmDbgCntTxSvpEsDrop = 207,         /* Tx: packets dropped due to enabling
                                           ethernet segment filtering based on
                                           source virtual port. */
    bcmDbgCntTxL3EgressOrIntfRangeError = 208, /* Tx: packets dropped due to overlay
                                           and underlay L3 next hop and
                                           interface indices do not comply with
                                           configured allocation bitmap. */
    bcmDbgCntRxLinkDownDrop = 209,      /* Rx: packets dropped due to link down
                                           or destination port disabled. */
    bcmDbgCntRxPartialLinkDownDrop = 210, /* Rx: non-UC packets dropped partially
                                           due to link down or destination port
                                           disabled. */
    bcmDbgCntRxVxlanVfiAssignFailDrop = 211, /* Rx: packets dropped due to VXLAN
                                           packets with VFI assignment failure. */
    bcmDbgCntRxVxlanTunnelErrorDrop = 212, /* Rx: packets dropped due to VXLAN
                                           tunnel termination errors. */
    bcmDbgCntNum = 213                  /* Must be last one. */
...
typedef enum bcm_stat_counter_format_type_e {
    ...
    bcmStatCounterFormatSlimPacketsAndBytes = 6, /* Slim dual counter format and add one
                                           action */
    bcmStatCounterFormatCount = 7       /* Must be last one. */
...
typedef enum bcm_eviction_destination_type_e {
    bcmStatEvictionDestinationLocalHost = 0, /* Configure the counters eviction
                                           destination to be the CPU */
    bcmStatEvictionDestinationNetwork = 1, /* Configure the counters eviction
                                           destination to be the network */
    bcmStatEvictionDestinationSize = 2  /* enum size */
...
typedef enum bcm_stat_control_e {
    ...
    bcmStatControlCounterThreadEnable = 1, /* General enabler for the SW BG thread
                                           ...
                                           engines) */
    bcmStatControlCounterNetworkEvictionPktSize = 2, /* Configure the paket size sents from
                                           CRPS in network eviction */
    bcmStatControlEgressL2FwdPlusOneMetadataEnable = 3, /* General enabler for the egress L2
                                           bridge stat metadata based on fwd
                                           plus one layer */
    bcmStatControlSize = 4              /* enum size */
} bcm_stat_control_t;
...
/* Set configuring eviction for the database */
extern int bcm_stat_counter_database_eviction_set(
    int unit, 
    uint32 flags, 
    bcm_stat_counter_database_t *database, 
    bcm_stat_eviction_t *eviction);

/* Get configuring eviction for the database */
extern int bcm_stat_counter_database_eviction_get(
    int unit, 
    uint32 flags, 
    bcm_stat_counter_database_t *database, 
    bcm_stat_eviction_t *eviction);
