...
#define BCM_TRUNK_FLAG_WEIGHTED             0x4000     /* If set, indicates it
                                                          ...
                                                          powers-of-2. */
#define BCM_TRUNK_FLAG_NONUCAST_TRUNK_BLOCK_MASK_DISABLE 0x8000     /* If set,
                                                          NONUCAST_TRUNK_BLOCK_MASK
                                                          table will not be
                                                          programmed. Once set,
                                                          all subsequent trunk
                                                          operations will not
                                                          touch
                                                          NONUCAST_TRUNK_BLOCK_MASK
                                                          table. */
#define BCM_TRUNK_FLAG_MULTI_SET            0x10000    /* If set, trunk is part
                                                          of trunk_multi_set. */
...
#define BCM_TRUNK_MEMBER_DGM_ALTERNATE      0x0200     /* If set indicating
                                                          ...
                                                          alternate path, else
                                                          within primary path. */
#define BCM_TRUNK_MEMBER_SPA_WITH_ID        0x0400     /* used to indicate
                                                          whether the user
                                                          desire a specific spa
                                                          id to the newly add
                                                          member of the group */
...
typedef struct bcm_trunk_multiset_info_s {
    bcm_trunk_t tid;                /* Trunk ID */
    bcm_trunk_info_t trunk_info;    /* Trunk info structure (see \ref
                                       bcm_trunk_info_t) */
    int member_count;               /* Number of members in this trunk. */
    int set_result;                 /* Trunk set result (returned error) */
} bcm_trunk_multiset_info_t;

/* Trunk ID information structure. */
typedef struct bcm_trunk_id_info_s {
    int pool_index;     /* pool index of trunk_id */
    int group_index;    /* group index of trunk_id */
} bcm_trunk_id_info_t;

/* Flow Aggregation information structure. */
typedef struct bcm_flow_agg_info_s {
    bcm_gport_t base_voq;   /* Base VOQ for the flow aggregate */
    bcm_trunk_t trunk_id;   /* Trunk ID for the flow aggregate to ride on */
    int nof_cosq_levels;    /* number of cosq levels */
} bcm_flow_agg_info_t;
...
typedef struct bcm_trunk_member_s {
    ...
    int dynamic_queue_size_weight;  /* Relative weight of queue size in
                                       determining member quality. */
    uint32 spa_id;                  /* spa_id of this member, provided together
                                       with BDM_TRUNK_MEMBER_SPA_WITH_ID flag */
} bcm_trunk_member_t;
...
/* Structure containing the range (minimum and maximum) of a trunk member */
typedef struct bcm_trunk_member_range_s {
    int min_hash;   /*  Minimum hash */
    int max_hash;   /*  Maximum hash */
} bcm_trunk_member_range_t;

/* Initializes the bcm_trunk_info_t structure */
extern void bcm_trunk_info_t_init(
    bcm_trunk_info_t *trunk_info);

/* Initializes the bcm_trunk_multiset_info_t structure */
extern void bcm_trunk_multiset_info_t_init(
    bcm_trunk_multiset_info_t *trunk_multiset_info);
...
extern int bcm_trunk_multi_set(
    int unit, 
    int trunk_count, 
    bcm_trunk_multiset_info_t *trunk_array, 
