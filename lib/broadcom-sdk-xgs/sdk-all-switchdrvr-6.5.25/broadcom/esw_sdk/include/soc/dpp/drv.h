...
#define SOC_DPP_PPPOE_SESSION_ID_MAX  0xFFFF /* PPPoE session id is 16 bits */

#define SOC_DPP_L2TP_IS_ENABLE(unit) (SOC_IS_JERICHO(unit) && soc_property_get(unit, spn_L2TP_MODE, 0))
#define SOC_DPP_L2TP_VRF_MAX  0xFFF /* we only have 12 bits for vrf lookup in TT phase */
#define SOC_DPP_L2TP_SESSION_ID_MAX  0xFFFF /* PPPoE session id is 16 bits */
#define SOC_DPP_L2TP_TUNNEL_ID_MAX  0xFFFF /* PPPoE session id is 16 bits */
...
typedef struct soc_dpp_tm_crps_config_s {
	...
	uint32 ingress_replicate_packets_all; /** if true, irpp ingress multicast packets will be counted for all copies, otherwise only one time */
...
typedef struct soc_dpp_config_pp_s {
  ...
  SOC_PPC_LIF_ID global_lif_rch; /* Global LIF index used as default port lif for ports with generalized recycle header */
  ...
  uint8   gtp_teid_exists_parsing; /* identify GTP V1 or GTP v2 with TEID flag */
  ...
  uint8   add_ptch2_header;   /* If set allocate the prge program ARAD_EGR_PROG_EDITOR_PROG_MIM_PTCH2.
                               This program does not remove the network headers and adds PTCH to packet */
  uint8   force_bridge_forwarding; /* If set allocate the existing TT program PROG_TT_BRIDGE_STAR_2ND_PROG with program selection- ptc_tt_profile = FORCE_BRIDGE_FWD*/
  ...
  SOC_PPC_LIF_ID ip_lif_dummy_id_vxlan; /* In-LIF-ID of IP-VXLAN-LIF-dummy for termination of IP-Overlay bud multicast traffic */
  SOC_PPC_LIF_ID ip_lif_dummy_id_gre; /* In-LIF-ID of IP-GRE-LIF-dummy for termination of IP-Overlay bud multicast traffic */
  ...
  uint8   pon_port_ptc_size; /* number of ptc size per PON port */  
  uint8   pon_port_alloc_mode;
  ...
  uint8   tunnel_termination_ordered_by_index; /* If set all MPLS label index is used for order */
  ...
  uint8   custom_feature_vrrp_scaling_tcam; /* full range vrrp using tcam - arad+*/
  uint8   vrrp_scaling_tcam; /* full range vrrp using tcam - jerico*/
  uint8   use_small_banks_mode_vrrp; /* TCAM banks mode for vrrp sacling - jericho */
...
#define SOC_DPP_CONFIG_CUSTOM_PP_PORT_IS_VENDOR2() (SOC_DPP_CONFIG(unit)->pp.custom_feature_pp_port == 2)
  ...
  uint8   ipmc_ivl; /* enable lookup with VLAN <fid,vlan,group,sip> in KAPS */
  uint8   test1; 
  ...
  uint32  vxlan_tunnel_term_in_sem_my_vtep_index_nof_bits;  /* This soc property is used in vxlan termination according to DIP SIP VRF, using my-vtep-index, 
                                                  ...
                                                  * Note that number of bits for VRFs + number of bits for my-vtep-index must be <= 15.
                                                  */
  uint32  ip6_tunnel_term_in_tcam_vrf_nof_bits; /* This soc property is used in IPv6 tunnel termination according to DIP VRF Next-Protocol Port-Property, 
                                                  * enabled using bcm886xx_ip6_tunnel_termination_mode = 2.
                                                  * It defines the number of bits available for VRF 
                                                  * in IPv6 tunnel termination lookup: DIP VRF Next-Protocol Port-Property.
                                                  * Max value is number of VRF in the device.
                                                  * Note that  number of bits for DIP VRF Next-Protocol Port-Property must be <= 154.
                                                  */
...
#define SOC_DPP_DSCP_PRESERVE_OUTLIF_SEL_MASK 0x3F
  /* routing_preserve_dscp = {6'outlif_sel, 13'b0, inlif_ctrl_en, outlif_ctrl_en, port_ctrl_en}
  * outlif_sel is an enable bitmap of each outlif profile (first outlif profile~sixth outlif profile),
  * bit 0 = first outlif profile valid, ... bit 5 = sixth outlif profile valid.
  * If more than one bit is enabled, then multiple program selection entries will be added, e.g.,
  *     If first outlif profile & dscp_preserve_bit == 1 then select dscp preserve program
  *     ...
  *     If fourth outlif profile & dscp_preserve_bit == 1 then select dscp preserve program
  */
  uint32 routing_preserve_dscp; /* Per LIF control of preserve DSCP remark after routing */
  ...
  uint8 parser_mode; /* In version 6.5.8 the advance parser option was added, enabled by the parser_mode soc property. */
  /* soc for rx trap parser */
  ...
  uint8 oam_forward_trap_strength;  /* Forward trap strength configured for OAM */
  uint8 oam_default_trap_strength;  /* Default trap strength configured for OAM */
  uint8 ipmc_lookup_model; /* The advance ipmc lookup  option was added, enabled by the ipmc_lookup_model soc property. */
  uint8 rch_port0_out_exists;  /* standardized recycle header type 0 out port exists */
  uint8 rch_port1_out_exists;  /* standardized recycle header type 1 out port exists */
  uint8 rch_port0_in_exists;  /* standardized recycle header type 0 in port exists */
  uint8 rch_port1_in_exists;  /* standardized recycle header type 1 in port exists */
  uint8 oamp_flexible_da; /* Custom feature - custom_feature_oamp_flexible_da. */
  ...
  uint16 vxlan_udp_dest_port; /* VXLAN UDP destination port */
  uint8 bfd_ipv4_flex_iptos; /* 0 - Disabled. 1 - Enabled. Feature used for enabling IP TOS configuration for single hop endpoints. */
  uint8 eth_default_procedure_disable; /* 0 - enabled. 1 - Disabled. Ethernet-Default procedure for MPLS switching/IP-UC routing or not. */
  uint32 sbfd_rcy_port; /* Relevant to Arad,Arad+ and QAX */
  uint32 sbfd_rcy_port_core_0; /* Relevant only to Jericho */
  uint32 sbfd_rcy_port_core_1; /* Relevant only to Jericho */
  uint8 sbfd_reflector_is_configured; /* Relevant only to Jericho */
  uint8 bfd_sbfd_enable; /* 0 - Disabled. 1 - Enabled. Feature used for enabling Seamless BFD */
  uint8 out_ac_ttl_enable; /* 0 - Disabled. 1 - Enabled. Feature used for enabling TTL decrements for Out-AC */
  uint8 mim_bsa_lsb_from_ssp; /* Custom feature - set BSA LSB per SrcSysPort */
  uint8 bfd_mpls_special_ttl_support; /* Custom feature - Support rx BFDoMPls with ttl 255 */
  uint8 ipv6_compressed_sip_enable; /* 0 - Disabled. 1 - Enabled. Feature used for enabling IPv6 SSM in KAPS */
} soc_dpp_config_pp_t;
...
#ifdef BCM_JERICHO_SUPPORT
#define SOC_IS_DIP_SIP_VRF_WITH_MY_VTEP_INDEX_ENABLE(is_ipv6_tunnel, unit) ((SOC_IS_JERICHO(unit) && (SOC_DPP_CONFIG(unit)->pp.ingress_ipv4_tunnel_term_mode == SOC_DPP_IP_TUNNEL_L2_LKUP_MODE_DIP_SIP_VRF_SEM_ONLY_AND_DEFAULT_TCAM)) || \
                                                            (SOC_IS_JERICHO_PLUS(unit) && is_ipv6_tunnel && (SOC_DPP_CONFIG(unit)->pp.ipv6_tunnel_term_bitmap_enable & SOC_DPP_IP_TUNNEL_TERM_DB_DIP_SIP_VRF)))
#else 
#define SOC_IS_DIP_SIP_VRF_WITH_MY_VTEP_INDEX_ENABLE(is_ipv6_tunnel, unit) 0
#endif   
