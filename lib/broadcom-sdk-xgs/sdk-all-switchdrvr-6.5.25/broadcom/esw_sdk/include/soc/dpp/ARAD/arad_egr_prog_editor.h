...
/* valid outlif profile index: 1-6 */
#define ARAD_EGR_PROG_MAX_OUTLIF_PROFILE_INDEX 6
/*
* Intend to set any outlif profile from first outlif profile to sixth outlif profile, if it is relevant.
* A outlif profile is relevant when xxx_out_lif_profile_mask is set to 0 during init.
* For example, if the program needs to be selected based on either first outlif profile or forth outlif profile,
* then during initialization, create two program selection entries:
*     a. if xxx AND first_out_lif_profile_mask=0 then program = prog
*     b. if xxx AND forth_out_lif_profile_mask=0 then program = prog
*
* xxx_out_lif_profile_mask will be set to actual value when arad_egr_prog_editor_prog_sel_outlif_profile_set is called.
* Such programs should not be selected based on more than one outlif profiles
*/
...
typedef enum
{
    ...
    /* ERSPAN = change ERSPAN packet flow / egress editor program to work in ARAD-A0 + B0 with IPv4 routed packet */
    ARAD_EGR_PROG_EDITOR_PROG_ERSPAN,                   /* ERPAN without XGS without change */
    ARAD_EGR_PROG_EDITOR_PROG_ERSPAN_WA,                /* ERPAN without XGS with    change */
    ARAD_EGR_PROG_EDITOR_PROG_ERSPAN_XGS_PE_FROM_FTMH,  /* ERPAN with    XGS with    change */
    ARAD_EGR_PROG_EDITOR_PROG_ERSPAN_XGS_NO_WA,         /* ERPAN with    XGS without change */
    ...
    ARAD_EGR_PROG_EDITOR_PROG_OAM_DM_DOWN_TRAP_2CPU, /* for ARAD+. Uses 1588 time stamping only. */
    ARAD_EGR_PROG_EDITOR_PROG_OAM_DM_DOWN_TRAP_NTP_2CPU, /* for ARAD+. Uses NTP time stamping only. */
    ...
    ARAD_EGR_PROG_EDITOR_PROG_OAM_EGRESS_SNOOP_WITH_OUTLIF_INFO, /* OAM egress snoop for MIP */
    ARAD_EGR_PROG_EDITOR_PROG_OAM_DROP_1ST_ENC, /* Drop 1st encapsulation as it is created by OAMP */
    ARAD_EGR_PROG_EDITOR_PROG_OAM_EGR_INJECTION_WA, /* egress injection CCM , stamp counter */
    ARAD_EGR_PROG_EDITOR_PROG_OAM_EGR_INJECTION_OPCODE, /* egress injection modify Opcode */
    ...
    ARAD_EGR_PROG_EDITOR_PROG_OAM_OAMP_REMOVE_UDH_BFD_ECHO, /* remove UDH from packets coming to OAM and bfd echo */
    ...
    ARAD_EGR_PROG_EDITOR_PROG_PP_COE_ITMH_COPY_HEADER_TDM_COE, /* TDM to COE (inter-connect) */
    ...
    ARAD_EGR_PROG_EDITOR_PROG_RM_SYS_HDR_TDM_COE,   /* COE to TDM (inter-connect) */
    ...
    ARAD_EGR_PROG_EDITOR_PROG_REFLECTOR_IP_AND_ETH, /* L2 fwd, swap DA with SA, SIP with DIP, add PTCH.*/
    ARAD_EGR_PROG_EDITOR_PROG_REFLECTOR_IP_AND_ETH_L3, /* L3 fwd, swap DA with SA, SIP with DIP, add PTCH.*/
    ...
    ARAD_EGR_PROG_EDITOR_PROG_USER_MODE_PTAGGED_MPLS_RAW,       /* ptagged programs with mpls raw */
    ARAD_EGR_PROG_EDITOR_PROG_SERVICE_MODE_PTAGGED_MPLS_RAW,    /* ptagged programs with mpls raw */
    ARAD_EGR_PROG_EDITOR_PROG_USER_MODE_NOT_PTAGGED_MPLS_RAW,   /* ptagged programs with mpls raw */    
    ARAD_EGR_PROG_EDITOR_PROG_LSR_MANIPULATION,      /* MPLS labels manipulating program*/
    ARAD_EGR_PROG_EDITOR_PROG_BFD_ECHO,        /*bfd echo*/
    ARAD_EGR_PROG_EDITOR_PROG_SBFD_REFLECTOR,   /*sbfd reflector*/
    ...
    ARAD_EGR_PROG_EDITOR_PROG_MPLS_NON_BOS_ELI_AT_ENC1_JER,  /*Inserting EL/ELI at non-BoS location B2*/
    ARAD_EGR_PROG_EDITOR_PROG_MPLS_NON_BOS_ELI_AT_ENC2_JER,  /*Inserting EL/ELI at non-BoS location C1*/
    ARAD_EGR_PROG_EDITOR_PROG_MPLS_EXTENDED_JER, /* Segment Routing for jericho only C1 or only D1 */
    ARAD_EGR_PROG_EDITOR_PROG_MPLS_EXTENDED_JER_SR, /* Segment Routing for jericho C3 and D1 all valid */
    ...
    ARAD_EGR_PROG_EDITOR_PROG_IPSEC_ENC_DEC_UNDERSIZE,
    ARAD_EGR_PROG_EDITOR_PROG_IPSEC_ENC_DEC_UNDERSIZE_OTSH_REMOVE,
    ...
    ARAD_EGR_PROG_EDITOR_PROG_IPV4_TUNNEL_TOR, /* IPV4_TUNNEL_TOR PROGRAM */
    /*PPPoE*/
    ...
    /* CH header encapsulation for basic service - L2,IPv4,IPv6,MPLS */
    ARAD_EGR_PROG_EDITOR_PROG_CH_CONN_TO_NP,
    /* CH header encapsulation for L2 1pass Reflector */
    ARAD_EGR_PROG_EDITOR_PROG_CH_CONN_TO_NP_REFLECTOR,
    /* CH header encapsulation for RSPAN */
    ARAD_EGR_PROG_EDITOR_PROG_CH_CONN_TO_NP_RSPAN,
    ARAD_EGR_PROG_EDITOR_PROG_VXLAN_ROO_COPY_DSCP, /* copy DSCP from forwarding header to tunnel IP when ROO into vxlan */
    ARAD_EGR_PROG_EDITOR_PROG_VXLAN_ROO_COPY_DSCP_IPV6, /* copy DSCP from IPv6 forwarding header to tunnel IP when ROO into vxlan */
    /* 20 Byte Header Prefix */
    ...
    /* Copy DSCP from IPv4/IPv6 forwarding header to tunnel IP in Mixed QOS mode */
...
typedef enum
{
    ...
    ARAD_PRGE_PP_SELECT_TDM_COE_INTER_CONN = 5, /* used for TDM and COE inter-connect when LB-ext exist */
    ...
    ARAD_PRGE_PP_SELECT_SBFD_REFLECTOR = 8, /* SBFD REFLECTOR */
...
typedef enum
{
    ...
    ARAD_EGR_PROG_EDITOR_LFEM_ZERO_BIT2, /* Data[15:0] & ~(0x0004) - Clear FTMH.PPH_Type OAM-TS bit */
    ...
    ARAD_EGR_PROG_EDITOR_LFEM_LSHIFT_4_BITOR_RSHIFT_4, /* MPLS EL -Used for construct 16msb of EL with load balance key */
    ...
    ARAD_EGR_PROG_EDITOR_LFEM_IN_DSCP_TO_EXP, /* FHEI.IN-DSCP to MPLS EXP*/
    ARAD_EGR_PROG_EDITOR_LFEM_MPLS_CLEAR_EXP, /* clear MPLS EXP*/
    ...
    ARAD_EGR_PROG_EDITOR_LFEM_SPB_STAMP_LEFT_NIBBLE_8, /* extract TPID's three rightmost nibbles and set the leftmost nibble to 0x8 */
    ...
    ARAD_EGR_PROG_EDITOR_LFEM_GPON_CHAN_JER_PTC5,
    ARAD_EGR_PROG_EDITOR_LFEM_GPON_CHAN_JER_PTC6,
    ARAD_EGR_PROG_EDITOR_LFEM_GPON_CHAN_JER_MAPLE,
    ARAD_EGR_PROG_EDITOR_LFEM_GPON_STAG_PCP,
    ARAD_EGR_PROG_EDITOR_LFEM_GPON_GEM_ID,
    ARAD_EGR_PROG_EDITOR_LFEM_GPON_GEM_ID_EES,

    ARAD_EGR_PROG_EDITOR_LFEM_IPV6_VXLAN_NH_HL, /* extract IPv6 next header and hop limit from IPv4 protocol and TTL */
    ARAD_EGR_PROG_EDITOR_LFEM_IPV6_VXLAN_FL, /* extract IPv6 flow label from LB key */

    ARAD_EGR_PROG_EDITOR_LFEM_BIT_0_M_3, /* bit 0 multiples 3, used to calculate the FHEI-size if FHEI is 3B */
    ARAD_EGR_PROG_EDITOR_LFEM_BIT_1_M_5, /* bit 1 multiples 5, used to calculate the FHEI-size if FHEI is 5B or 8B */
    ARAD_EGR_PROG_EDITOR_LFEM_COE_TAG_TC_DP,
    ARAD_EGR_PROG_EDITOR_LFEM_COE_TAG_TC,    
    ARAD_EGR_PROG_EDITOR_LFEM_BIT_2_M_6, /* bit 2 multiples 6, used to calculate the OAM-TS size */
    ARAD_EGR_PROG_EDITOR_LFEM_BITS_2_17_RSHIFT_2, /* extract bits[17:2] */
    ARAD_EGR_PROG_EDITOR_LFEM_BIT_0_M_6, /* bit 0 multiples 6, used to calculate the MAC size */
    ...
    ARAD_EGR_PROG_EDITOR_LFEM_BIT_0_M_4, /* bit 0 multiples 4, used to indicate MPLS bos and label size, etc. */
    ARAD_EGR_PROG_EDITOR_LFEM_FTMH_SRC_SYS_PORT,
...
typedef struct
{
  ...
  uint8 egr_inj_wa; /* down mep egress injection, stamp OAM counter */
  uint8 egr_inj_opcode; /*modify opcode for packet*/
...
/*
 * Update FEM value for sbfd_reflector_status
 */
uint32
