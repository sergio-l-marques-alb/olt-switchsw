...
#define SOC_TMC_LB_CONTROL                           (SOC_TMC_LB_NOF_LBG)
#define SOC_TMC_LB_LBG_MAX_WITH_CONTROL               SOC_TMC_LB_CONTROL
#define SOC_TMC_LB_NOF_LBG_WITH_CONTROL               (SOC_TMC_LB_NOF_LBG+1)
...
/* Max speed of one LBG is 30G bps */
...
#define _SOC_LB_SEGMENT_MODE_MSG_INIT        {                            \
        "128",                           /* socLbSegmentMode128 */         \
        "192",                           /* socLbSegmentMode192 */         \
        "192Enhanced",                   /* socLbSegmentMode192Enhanced */ \
        "128Enhanced",                   /* socLbSegmentMode128Enhanced */ \
        "Unknown"                        /* socLbSegmentModeCount */       \
        }

#define SOC_TMC_LB_MODEM_SHARPER_CONFIG_OP_FLAG_SHAPER            0x1
#define SOC_TMC_LB_MODEM_SHARPER_CONFIG_OP_FLAG_HDR_COMPENSATION  0x2
...
typedef struct soc_lb_cfg_rx_glb_info_s {
    ...
    int nof_total_shared_buffer;           /* only relevant to QUX */

    int single_buffer_size;                /* only relevant to QUX */
} soc_lb_cfg_rx_glb_info_t;

typedef struct soc_lb_cfg_tx_glb_info_s {

    soc_lb_segment_mode_t segment_mode;    /* only relevant to QAX */

    uint16                vlan_tpid;

    uint16                lbg_tpid;

    uint32                is_pkt_crc_ena;  /* only relevant to QAX */
...
typedef struct soc_lb_cfg_rx_lbg_info_s {
    ...
    int        nof_guaranteed_buffer;       /* only relevant to QUX */
...
typedef struct soc_lb_cfg_tx_lbg_info_s {
    ...
    soc_lb_segment_mode_t segment_mode;  /* only relevant to QUX */

    uint32                is_pkt_crc_ena;  /* only relevant to QUX */
