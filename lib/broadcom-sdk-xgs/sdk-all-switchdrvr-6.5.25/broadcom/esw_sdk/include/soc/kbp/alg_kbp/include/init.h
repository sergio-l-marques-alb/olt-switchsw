...
struct kbp_device_config {
    ...
    int32_t npu_type;   /* Flag for internal use */
 ...
 * @param config Configuration structure initialized appropriately.
 * @param flags The device configuration flags
 * @param port_number Input Port number
 * @param enable enable or disable, enable: 1, disable: 0
 * @param is_last_port flag to clear errors on all ports
 * @param phmf_tog flag to toggle PHMF FIFO
 * @param fec_tog flag to toggle FEC,  disable/enable (toggle) FEC: 1, no action: 0
 *
 * @return KBP_OK on success or an error code.
 */
