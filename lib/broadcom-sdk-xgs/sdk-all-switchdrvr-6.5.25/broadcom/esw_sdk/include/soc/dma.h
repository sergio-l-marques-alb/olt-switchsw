...
typedef enum soc_dma_poll_type_e {
    ...
    SOC_DMA_POLL_ADDR_DECODE_ERR = 3
} soc_dma_poll_type_t;
...
typedef struct soc_cmic_dma_drv_s {
    ...
    int (*chan_restart)(int unit, sdc_t *sc);
    ...
    int (*pci_timeout_handle)(int unit, int cmc);
    int (*chan_cur_op_datails)(int unit, sdc_t *sc);
} soc_cmic_dma_drv_t;
...
extern int soc_cmicm_dma_active(int unit, int cmc);
...
extern int      soc_dma_pci_timeout_handle(int unit, int cmc);
