/*
 * $Id: dnxf_cgm.h,v 1.5 Broadcom SDK $
 *
...
_shr_error_t soc_dnxf_cgm_pipe_link_type_decode(
    int unit,
    _shr_dnxf_threshold_id_t threshold_id,
    int *link,
    int *pipe);
...
{
    soc_dnxf_cgm_reg_structure_1,       /* Common case where we have in each reg field for each dimensions indexes
                                         * (pipe/level/cast/prio) and profile in the reg numels */
    soc_dnxf_cgm_reg_structure_2,       /* Special case where pipe is in the register index and profile in the reg
                                         * numels */
    soc_dnxf_cgm_reg_structure_3,       /* Special case where profile id is in the register index , pipe in the reg
                                         * numels and level in the reg fields */
    soc_dnxf_cgm_reg_structure_4,       /* Special case where pipe is in the register index and profile in the reg *
                                         * fields (relevant for LLFC) */
    soc_dnxf_cgm_reg_structure_5        /* Special case where access is per FMAC link and not profile dependent
                                         * (relevant for Credit based LLFC) */
