/*
 * $Id: dnxf_diag.h,v 1.11 Broadcom SDK $
 *
...
{
    /** for each DCH, save the above info */
...
/**
 * DNXF threshold stages options.
 * This order - RX, MID, TX, SHARED is also used as default diag display order
 */
typedef enum
{
...
void soc_dnxf_counters_info_init(
    soc_dnxf_counters_info_t * fe_counters_info);

void soc_dnxf_queues_info_init(
    int unit,
    soc_dnxf_queues_info_t * fe_queues_info);

shr_error_e soc_dnxf_diag_fabric_cell_snake_test_interrupts_name_get(
    int unit,
    const soc_dnxf_diag_flag_str_t ** intr_names);

shr_error_e soc_dnxf_diag_cell_pipe_counter_get(
    int unit,
    int pipe,
    uint64 *counter);

shr_error_e soc_dnxf_diag_fabric_traffic_profile_get(
    int unit,
    int source_id,
    int dest_id,
    soc_dnxf_diag_fabric_traffic_profile_t * traffic_info);

shr_error_e soc_dnxf_diag_connectivity_status_reg_parse(
    int unit,
    uint32 reg_val,
    bcm_fabric_link_connectivity_t * link_partner);

#endif /*_SOC_DNXF_DIAG_H_*/
