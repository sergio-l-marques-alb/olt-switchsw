...
#define DSIG_MAX_SIZE_STR           SHR_PP_SIGNAL_PRINT_VALUE_MAX_SIZE_STR /* Maximal signal value transfered as a string */
...
/** Is substitute means that part of its signals are from other ASIC stage */
#define SIGNAL_STAGE_IS_SUBSTITUTE       0x0001
/** Have substitute means that stage has no native signals on adapter and they are taken from substitue stage */
#define SIGNAL_STAGE_HAS_SUBSTITUTE      0x0002
/** Stage signals ignore port setting, because it is before port resolution */
#define SIGNAL_STAGE_PORT_VIS_IGNORE     0x0004
/** Stage always record previous packet */
...
/*
 * Signal expansion cb includes from as its sigstruct above,
 * since sometimes expansion call back needs to read signals and
 * understand in which stage its being called from
 */
typedef shr_error_e(
    *sand_expansion_cb_t) (
    int unit,
 ...
 *  extra_flags - Use additonal flags that will be added to the ones implicitly set by this routine
 ...
 *  signal_ouptut_p - pointer to signal output structure with value, print_value,size and fields
 ...
 * Notes
 *  1. Use "sig get" to verify existence and usage of signals
 *  2. This routine frees the content of signal_ouptut_p if not NULL
 */
shr_error_e
sand_signal_output_find(
    int unit,
...
/**
 * \brief - Decode Contextvalue according to NASID Signal so that the context
 *          is parsed by the relevant DBAL field of the context of the next active stage
 *
 * \param [in] unit - Unit ID
 * \param [in] core - Core ID
 * \param [in] desc - Name of the object to decode
 * \param [in] data -  Data of the context signal to decode
...
/**
 * \brief - Decode Context value in PMF stages
 *
 * \param [in] unit - Unit ID
 * \param [in] core - Core ID
 * \param [in] desc - Name of the object to decode
 ...
 *   * None
 */
shr_error_e pmf_context_decode(
    int unit,
    int core,
    char *desc,
    uint32 *data,
 ...
 *  sand_signal_verify
 * Purpose:
 *  Compare signal value with either uint32 array or string expected value
 * Parameters:
 *  unit, core - unit id and core id
 *  block - name of PP block, one of IRPP, ERPP, ETPP. If NULL all blocks will be searched for match
 *  from  - name of stage signal comes from, If null any from stages will be searched for match
 ...
 *  flags      - Additional flags that are required from the set [SIGNALS_MATCH_*]
 * Returns:
 *  _SHR_E_NONE   - if signal was found and values successfully obtained
 *  _SHR_E_PARAM  - Bad parameters, like NULL pointer for value
 *  _SHR_E_MEMORY - Memory allocation problems
 *  _SHR_E_NOT_FOUND  - Signal was not found
 *  _SHR_E_RESOURCE - Memory for expected signal is less than signal size
 *  _SHR_E_FAIL   - Compare failed
 *  _SHR_E_UNAVAIL - the signal is not present either in adapter or asic, and hence shouldn't be compared or fail the test
 */
shr_error_e
sand_signal_verify(
    int unit,
