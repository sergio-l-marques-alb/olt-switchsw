 ...
 */
#define CDMAC_PREEMPT_TX_INACTIVE             0x0
/*
 * Preemption is enabled, Verify is enabled and its
 * operation is in progress.
 */
#define CDMAC_PREEMPT_TX_VERIFY_IN_PROGRESS   0x1
/*
 * Preemption is enabled and active.
 * if VERIFY_ENABLE = 1, verify is enabled and succeed
 * if VERIFY_ENABLE = 0, preemption is enabled
 */
#define CDMAC_PREEMPT_TX_ACTIVE               0x2

/* MAC preemption verification state */
#define CDMAC_PREEMPT_VERIFY_DISABLED    0x0 /* disabled */
#define CDMAC_PREEMPT_VERIFY_INITIAL     0x1 /* enabled but not started */
#define CDMAC_PREEMPT_VERIFY_VERIFYING   0x2 /* in progress */
#define CDMAC_PREEMPT_VERIFY_SUCCEED     0x3 /* successful */
#define CDMAC_PREEMPT_VERIFY_FAILED      0x4 /* failed */

/*
 * MAC Preemption Tx status flags
 * These macros are used as flags to retrieve the preemption status
 * on the transmit side.
 * Used by function CDMAC_PREEMPTion_tx_status_get() to get either
 * TX, Verify operation status or Number of Verify Messages sent
 * during preemption.
 */
#define CDMAC_PREEMPTION_TX_STATUS          0x1
#define CDMAC_PREEMPTION_VERIFY_STATUS      0x2
#define CDMAC_PREEMPTION_VERIFY_CNT_STATUS  0x3

/* MAC preemption force lost rx EOP flags */
#define CDMAC_PREEMPTION_FORCE_SW_RX_EOP       0x1
#define CDMAC_PREEMPTION_FORCE_AUTO_HW_RX_EOP  0x2
...
int cdmac_tx_threshold_set(int unit, soc_port_t port, int value);
int cdmac_tx_threshold_get(int unit, soc_port_t port, int *value);
...
int cdmac_faults_status_get(int unit, int port,
                            portmod_port_fault_status_t *faults);
int cdmac_drop_tx_data_on_ordered_set_enable(int unit,
                                             int port,
                                             uint32 enable);

int cdmac_preemption_mode_set(int32 unit, soc_port_t port, uint32 enable);
int cdmac_preemption_mode_get(int32 unit, soc_port_t port, uint32 *enable);

int cdmac_preemption_tx_enable_set(int32 unit, soc_port_t port, uint32 enable);
int cdmac_preemption_tx_enable_get(int32 unit, soc_port_t port, uint32 *enable);

int cdmac_preemption_verify_enable_set(int32 unit, soc_port_t port,
                                       uint32 enable);
int cdmac_preemption_verify_enable_get(int32 unit, soc_port_t port,
                                       uint32 *enable);

int cdmac_preemption_verify_time_set(int32 unit, soc_port_t port,
                                     uint32 time_val);
int cdmac_preemption_verify_time_get(int32 unit, soc_port_t port,
                                     uint32 *time_val);

int cdmac_preemption_verify_attempts_set(int32 unit, soc_port_t port,
                                         uint32 num_attempts);
int cdmac_preemption_verify_attempts_get(int32 unit, soc_port_t port,
                                         uint32 *num_attempts);

int cdmac_preemption_tx_status_get(int unit, soc_port_t port,
                                   int flags, uint32 *value);

int cdmac_preemption_force_rx_lost_eop_set(int unit, soc_port_t port,
                                           int flags, uint32 value);
int cdmac_preemption_force_rx_lost_eop_get(int unit, soc_port_t port,
                                           int flags, uint32 *value);

int cdmac_preemption_rx_timeout_info_set(int unit, soc_port_t port,
                                         uint32 enable, uint32 timeout_val);
int cdmac_preemption_rx_timeout_info_get(int unit, soc_port_t port,
                                         uint32 *enable, uint32 *timeout_val);
