 ...
 * \param [in] core_id    - Core ID to check same-context collision on (Must be specified)
 ...
 * \param [in] core_id - The core to search in
 ...
 * \param [in] core_id - The id of the core to check for the handler_id in (Must be specified)
 ...
 * \param [in] core_id    - the core id to search for.
...
/**
 * \brief
 *   Returns contains_half_key_handler=TRUE if bank contains at least one half-key
 *   sized handler
 * \param [in] unit                       - unit
 * \param [in] bank_id                    - The id of the bank to check whether half-key sized handler is in
 * \param [in] core_id                    - The id of the core to check whether half-key sized handler is in
 * \param [out] contains_half_key_handler - TRUE if bank contains at least one half-key sized handler, FALSE otherwise
 * \return
 *  \retval _SHR_E_NONE - success
 * \remark
 * SPECIAL INPUT:
