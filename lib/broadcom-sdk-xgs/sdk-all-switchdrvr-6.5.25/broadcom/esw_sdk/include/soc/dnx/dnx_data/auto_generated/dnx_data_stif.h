...
/**
 * \brief returns define data of ethu_id
 * Module - 'stif', Submodule - 'port', data - 'ethu_id'
 * Ethu id of where sif is used if const
 * 
 * \param [in] unit - unit #
 * 
 * \return
 *     ethu_id - the required data value
 * \remark
 *   * Auto-generated using device data autocoder
 * \see
 *   * None
 */
typedef uint32(
    *dnx_data_stif_port_ethu_id_get_f) (
    int unit);
...
typedef struct
{
    ...
    /**
     * returns define data of ethu_id
     */
    dnx_data_stif_port_ethu_id_get_f ethu_id_get;
    /**
 ...
 */
/**
 * \brief Holds values of submodule regression table supported_modes
 * Table info:
 * The table describes the supported speed for each stif mode
 */
typedef struct
{
    /**
     * valid speed for mode enable/disable
     */
    uint32 valid;
} dnx_data_stif_regression_supported_modes_t;
...
typedef enum
{
     ...
     * Must be last one!
     */
    _dnx_data_stif_regression_feature_nof
} dnx_data_stif_regression_feature_e;
 ...
 */
typedef int(
    *dnx_data_stif_regression_feature_get_f) (
    int unit,
    dnx_data_stif_regression_feature_e feature);

/**
 * \brief get table supported_modes entry 
 * The table describes the supported speed for each stif mode
 * 
 * \param [in] unit - unit #
 * \param [in] mode - The supported stif modes
 * \param [in] speed - The suppoted speed for stst ports
 * 
 * \return
 *     supported_modes - returns the relevant entry values grouped in struct - see dnx_data_stif_regression_supported_modes_t
 * \remark
 *   * Auto-generated using device data autocoder
 * \see
 *   * None
 */
typedef const dnx_data_stif_regression_supported_modes_t *(
    *dnx_data_stif_regression_supported_modes_get_f) (
    int unit,
    int mode,
    int speed);

/*
 * SUBMODULE INTERFACE DNX_DATA_IF_STIF - REGRESSION:
 * {
 */
/**
 * \brief Interface for stif regression data
 */
typedef struct
{
    /**
     * get submodule feature - disabled/enabled
     */
    dnx_data_stif_regression_feature_get_f feature_get;
    /**
     * get table supported_modes entry 
     */
    dnx_data_stif_regression_supported_modes_get_f supported_modes_get;
    /**
     * get general info table about table (for example key size)supported_modes info
     */
    dnxc_data_table_info_get_f supported_modes_info_get;
} dnx_data_if_stif_regression_t;
...
typedef struct
{
    ...
    /**
     * Interface for stif regression data
     */
    dnx_data_if_stif_regression_t regression;
} dnx_data_if_stif_t;
