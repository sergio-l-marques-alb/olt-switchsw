...
/**
 * \brief - suppress expression only, continue if error
 */
...
/**
 * \brief
 *  Test if prerequisites to support error recovery feature are met.
 * \param [in] unit    - Device Id
 * \return
 *   uint8 - boolean value indicating if prerequisites are met
 * \remark
 *   * None
 * \see
 *   * None
...
/**
 * \brief
 *  Temporary turn on or off error recovery feature
 * \param [in] unit    - Device Id
 ...
 * \param [in] unit    - Device Id
 * \param [in] func_name - pointer to a data that stores the caller function name
 * \return
 *   int - Error Type
 * \remark
 *   * None
