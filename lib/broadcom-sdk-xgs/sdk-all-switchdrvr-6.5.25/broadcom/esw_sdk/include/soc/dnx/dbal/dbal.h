...
{                                                                                       \
    ...
    {                                                                                     \
       ...
       /** if no room for another handle in this function exit with err */                \
        ...
        /** search for a free position in the handles DB */                                \
 ...
 * entry of the function. it free all dbal handles used in the
 * function scope.
 ...
 *          section, used also for validating the free macro exist,
 *          otherwise a variable not used warning will appear in
 *          compilation
 *    \param [in] _nof_local_handles - indicate the number of dbal tables used
 *          in this function
 *
 * \par INDIRECT OUTPUT:
 *    \param [in] _dbal_handles - DB for holding
...
/**
 *\brief Clear the payload hold in the handle, and reset result type
 *       to not initialized.
 *       This function is useful when one needs to set result type on
 *       a handle where it is already set, without clearing the whole handle.
 *       This way, the value of the keys won't be cleared.
 *
 * \par DIRECT INPUT:
 *   \param [in]  unit -\n
 *   \param [in]  entry_handle_id - the entry handle\n
 ...
 *   _SHR_E_CONFIG in case it's subfield and the parent field
 *     value encoding doesn;t match the field's encoding.
...
/**
 * \brief get the entry attribute value. this API should be called prior to entry_get (same as value_field_request
 *        APIs)
 *
 * \par DIRECT INPUT:
 *   \param [in]  unit \n
...
/**
 * \brief This API gets a key and payload buffer and updates an allocated handle with those values.
...
/**
 * \brief setting rule on entry attribute to filter the entries that the iterator find.
 *
 * \par DIRECT INPUT:
 *   \param [in]  unit \n
 *   \param [in]  entry_handle_id - the handle to perform the action \n
 *   \param [in]  attr_type - attribute type from DBAL_ENTRY_ATTR_XXX\n
 *   \param [in]  attr_val - attribute value to be find by the iterator
 *      Note: For hitbit rule, the user specifies the type of hitbit to be retrieved
 *      DBAL_ENTRY_ATTR_HIT_PRIMARY or DBAL_ENTRY_ATTR_HIT_SECONDARY (or both)
 *      Specifying 0 in attr_val means we wants to retrieve entries where the specified
 *      hitbit(s) is(are) not hit
 *      Specifying 1 in attr_val means we wants to retrieve entries where the specified
 *      hitbit(s) is(are) hit
 * \par DIRECT OUTPUT:
 *   Non-zero(shr_error_e) in case of an error
 */
 ...
 *   \param [in]  unit -\n
 *   \param [in]  entry_handle_id - handle that used by the iterator \n
 *   \param [out]  entries_counter - entries count of the iterator \n
 * \par DIRECT OUTPUT:
 *   Non-zero(shr_error_e) in case of an error
 */
 ...
 * \par DIRECT OUTPUT:
 *   Non-zero(shr_error_e) in case of an error, in case that the field_id is not parent fields will return _SHR_E_PARAM
 * \par INDIRECT OUTPUT
...
/**
 * \brief return the next valid enum value for enum field,
 *
 * \par DIRECT INPUT:
 *   \param [in] unit
 *   \param [in] field_id - the field ID - must be enum\n
 *   \param [in] enum_value -  the current enum value. use INVALID_ENUM to indicate first enum value \n
 *   \param [in] next_valid_enum_value - pointer to the next valid logical enum value, if not more enum values found returns
 *          INVALID_ENUM
 * \par DIRECT OUTPUT:
 *   Non-zero(shr_error_e) in case of an error
 * \par INDIRECT OUTPUT
 *       filled the next_valid_enum_value pointer \n
 */
shr_error_e
 ...
 *   in case the field size is bigger than 32 bit it will return E_PARAM with print
...
/**
 * \brief return if the result type is valid or not.
 *
 * \par DIRECT INPUT:
 *   \param [in] unit
 *   \param [in] table_id -  the relevant table_id.
    \param [in] result_type -  the result type to look for.
 *   \param [out] is_valid -  indicates if the result type is valid \n
 * \par DIRECT OUTPUT:
 *   Non-zero(shr_error_e) in case of an error
 * \par INDIRECT OUTPUT
 *       filled the is_valid pointer \n
 */
shr_error_e
 ...
 *   \param [in] inst_idx - field instance index\n
 *   \param [in] buffer_val -
 *       the field value that will be used to transform the string. Maximal size for
 *       this field is DBAL_FIELD_ARRAY_MAX_SIZE_IN_WORDS.
 *   \param [in] buffer_mask -
 *       optional, in case no need put NULL. If not NULL, Maximal size for
 *       this field is DBAL_FIELD_ARRAY_MAX_SIZE_IN_WORDS.
 ...
 * \par DIRECT INPUT:
 *   \param [in] unit
 *   \param [in] print_type - see DBAL_LOGGER_PRINT_TYPE_XX
 * \par DIRECT OUTPUT:
 *   Non-zero(shr_error_e) in case of an error
 */
shr_error_e
