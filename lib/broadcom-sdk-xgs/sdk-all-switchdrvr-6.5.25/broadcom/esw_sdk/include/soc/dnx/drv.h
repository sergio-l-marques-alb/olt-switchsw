...
shr_error_e soc_dnx_chip_type_set(
    ...
    uint16 dev_id);
/**
 * \brief - Check for an error in a function called from access initialization, and if an error happened, decide if to return the error and fail the startup.
 *
 * \par DIRECT_INPUT:
 *   \param [in] unit - unit #
 *   \param [in] error_value - return value of the function
 *
 * \par INDIRECT INPUT:
 *   * None
 * \par DIRECT OUTPUT:
