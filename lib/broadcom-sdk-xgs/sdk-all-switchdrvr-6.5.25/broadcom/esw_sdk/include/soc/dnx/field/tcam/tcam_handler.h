...
/**
 * \brief
 *  Returns core for the given entry_id.
 *  Output is undefined for non-existing entry_id.
 *
 * \param [in] unit        - Device ID
 * \param [in] entry_id    - Entry ID to return core for
 * \param [out] core       - Core on which the entry can be found on (BCM_CORE_ALL in case entry exists on all cores)
 *
 * \return
 *   \retval _SHR_E_NONE success
 */
