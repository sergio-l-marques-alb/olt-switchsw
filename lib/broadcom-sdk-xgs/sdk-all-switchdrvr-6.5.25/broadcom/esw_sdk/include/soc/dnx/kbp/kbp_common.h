 ...
 * Convenience macro to wrap function calls, capture and translate error codes and their descriptions.
 *
 * In case of rollback, no error is printed and no error is returned.
 * Otherwise a verbose error is printed when an error occurs.
 *
 * Based on KBP_TRY.
 */
#define DNX_KBP_TRY_ROLLBACK(A, _is_rollback_)                                                                      \
    do                                                                                                              \
    {                                                                                                               \
        kbp_status __tmp_status = A;                                                                                \
        if (!(_is_rollback_) && (__tmp_status != KBP_OK))                                                           \
        {                                                                                                           \
            SHR_IF_ERR_EXIT_WITH_LOG(dnx_kbp_error_translation(__tmp_status),                                       \
                                       "\n"#A" failed: %s%s%s\n",                                                   \
                                       kbp_get_status_string(__tmp_status), EMPTY, EMPTY);                          \
...
/**
 * \brief - Internal KBP function for performing caching on a single DB.
 * \param [in] unit - Relevant unit.
 * \param [in] op2_kbp_handles - pointer to the structure which contains all the KBP Database pointers (KBP only).
 *
 * \return
 ...
 *   * None
 */
shr_error_e dnx_kbp_cache_db_commit_internal(
    int unit,
    kbp_db_handles_t op2_kbp_handles);

/**
 * \brief
 *   Set the caching indication of the KBP DB, related to the provided DBAL table, to enabled.
 *   \param [in] unit - Relevant unit.
 *   \param [in] dbal_table_id - The provided DBAL table ID.
 *  \return
 ...
 *    shr_error_e
 */
shr_error_e dnx_kbp_cache_db_enable(
    int unit,
    dbal_tables_e dbal_table_id);

/**
 * \brief
 *   Install all cached entries to the KBP DB, related to the provided DBAL table and
 *   set the caching indication to disabled.
 *   In case of error, all cached entries that are not installed will be cleared.
 *   \param [in] unit - Relevant unit.
 *   \param [in] dbal_table_id - The provided DBAL table ID.
 *  \return
 *    \retval errors if unexpected behavior. See \ref shr_error_e
 ...
 *    shr_error_e
 */
shr_error_e dnx_kbp_cache_db_commit(
    int unit,
    dbal_tables_e dbal_table_id);

/**
 * \brief
 *   Clear all cached entries from the KBP DB, related to the provided DBAL table and
 *   set the caching indication to disabled.
 *   \param [in] unit - Relevant unit.
 *   \param [in] dbal_table_id - The provided DBAL table ID.
 *   \param [in] nof_entries_to_clear - The number of cleared entries. Used to update the DBAL entry counter.
 *  \return
 *    \retval errors if unexpected behavior. See \ref shr_error_e
 *  \remark
 *    None
 ...
 *    shr_error_e
 */
shr_error_e dnx_kbp_cache_db_clear(
    int unit,
    dbal_tables_e dbal_table_id,
...
#define DNX_KBP_DUMMY_PROTOTYPE_ERR_MSG(__func_name__)                                                      \
