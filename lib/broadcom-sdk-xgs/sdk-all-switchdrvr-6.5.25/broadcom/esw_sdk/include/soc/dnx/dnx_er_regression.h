...
/*
 * \brief - indicates the regression testing should be
 *  skipped for the current BCM API - not protected from in-transaction calls
 */
#define DNX_ERR_RECOVERY_REGRESSION_API_SKIP_TESTING(_unit)\
    dnx_er_regress_sim_skip(_unit)

/*
 * \brief - resets the is_api_excluded flag to its default value (FALSE)
 */
...
/**
 * \brief
 *  Exclude API from error recovery API testing - not protected from in-transaction calls
 * \param [in] unit - Device Id
 * \return
 *   * None
 * \remark
...
/**
 * \brief
 *  Reset is_api_excluded flag value to FALSE
 * \param [in] unit - Device Id
 * \return
 *   * None
 * \remark
