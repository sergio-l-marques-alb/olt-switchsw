...
/**
 * \brief
 *  Enable or disable the timeout thread mechanism,
 *  this does not allocate/free resources,
 *  it only skips the mutex and semaphores under the assumption that the same thread that inserts entries will commit them.
 * \par DIRECT INPUT
 *  \param [in] unit - The unit number.
 *  \param [in] enable - TRUE/FALSE to enable/disable.
 * \par INDIRECT INPUT
 *  See entry above.
 * \par DIRECT OUTPUT:
 *  Non-zero in case of an error
