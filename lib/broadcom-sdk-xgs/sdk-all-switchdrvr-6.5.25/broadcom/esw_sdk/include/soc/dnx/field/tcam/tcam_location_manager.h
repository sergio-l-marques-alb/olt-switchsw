 ...
 * \param [in] core       - Core ID (Should be specific core ID)
...
/**
 * \brief
 *  Returns whether there are free vacant 80b slots that are complementary to the given handler entries.
 *  In other words, this function returns TRUE if the given handler's entries are fragmented. Since
 *  80b handlers can no longer share TCAM lines for their entries due to handling traffic safeness issues
 *  more efficintley, 
 *
 * \param [in] unit           - Device ID
 * \param [in] core           - Core ID
 * \param [in] handler_id     - 80b Handler for for which to check vacant 80b slots
 * \param [in] bank_id        - Bank for for which to check vacant 80b slots
 * \param [out] is_vacant_80b - TRUE if vacant 80b slots are to be found
 *
 * \return
 *   \retval _SHR_E_NONE success
 */
...
/**
 * \brief
 *  Fetches a free line in promixity to the given line to copy, which allows
 ...
 * \param [out] free_line   - Free line location
 *
 * \return
 *   \retval _SHR_E_NONE success
 */
