/** \file dnx/swstate/auto_generated/access/dnx_iqs_access.h
 *
 * sw state functions declarations
 *
 * DO NOT EDIT THIS FILE!
 * This file is auto-generated.
 * Edits to this file will be lost when it is regenerated.
 *
 */
/*
 * $Copyright: (c) 2021 Broadcom.
 * Broadcom Proprietary and Confidential. All rights reserved.$
 */

#ifndef __DNX_IQS_ACCESS_H__
#define __DNX_IQS_ACCESS_H__

#include <soc/dnx/swstate/auto_generated/types/dnx_iqs_types.h>
#include <bcm/cosq.h>
#include <soc/dnx/dnx_data/auto_generated/dnx_data_iqs.h>
/*
 * TYPEDEFs
 */

/**
 * implemented by: dnx_iqs_db_is_init
 */
typedef int (*dnx_iqs_db_is_init_cb)(
    int unit, uint8 *is_init);

/**
 * implemented by: dnx_iqs_db_init
 */
typedef int (*dnx_iqs_db_init_cb)(
    int unit);

/**
 * implemented by: dnx_iqs_db_credit_request_profile_alloc
 */
typedef int (*dnx_iqs_db_credit_request_profile_alloc_cb)(
    int unit);

/**
 * implemented by: dnx_iqs_db_credit_request_profile_valid_set
 */
typedef int (*dnx_iqs_db_credit_request_profile_valid_set_cb)(
    int unit, uint32 credit_request_profile_idx_0, int valid);

/**
 * implemented by: dnx_iqs_db_credit_request_profile_valid_get
 */
typedef int (*dnx_iqs_db_credit_request_profile_valid_get_cb)(
    int unit, uint32 credit_request_profile_idx_0, int *valid);

/**
 * implemented by: dnx_iqs_db_credit_request_profile_thresholds_set
 */
typedef int (*dnx_iqs_db_credit_request_profile_thresholds_set_cb)(
    int unit, uint32 credit_request_profile_idx_0, bcm_cosq_delay_tolerance_t thresholds);

/**
 * implemented by: dnx_iqs_db_credit_request_profile_thresholds_get
 */
typedef int (*dnx_iqs_db_credit_request_profile_thresholds_get_cb)(
    int unit, uint32 credit_request_profile_idx_0, bcm_cosq_delay_tolerance_t *thresholds);

/**
 * implemented by: dnx_iqs_db_credit_watchdog_queue_min_set
 */
typedef int (*dnx_iqs_db_credit_watchdog_queue_min_set_cb)(
    int unit, int queue_min);

/**
 * implemented by: dnx_iqs_db_credit_watchdog_queue_min_get
 */
typedef int (*dnx_iqs_db_credit_watchdog_queue_min_get_cb)(
    int unit, int *queue_min);

/**
 * implemented by: dnx_iqs_db_credit_watchdog_queue_max_set
 */
typedef int (*dnx_iqs_db_credit_watchdog_queue_max_set_cb)(
    int unit, int queue_max);

/**
 * implemented by: dnx_iqs_db_credit_watchdog_queue_max_get
 */
typedef int (*dnx_iqs_db_credit_watchdog_queue_max_get_cb)(
    int unit, int *queue_max);

/**
 * implemented by: dnx_iqs_db_credit_watchdog_enable_set
 */
typedef int (*dnx_iqs_db_credit_watchdog_enable_set_cb)(
    int unit, int enable);

/**
 * implemented by: dnx_iqs_db_credit_watchdog_enable_get
 */
typedef int (*dnx_iqs_db_credit_watchdog_enable_get_cb)(
    int unit, int *enable);

/**
 * implemented by: dnx_iqs_db_credit_watchdog_retransmit_th_set
 */
typedef int (*dnx_iqs_db_credit_watchdog_retransmit_th_set_cb)(
    int unit, int retransmit_th);

/**
 * implemented by: dnx_iqs_db_credit_watchdog_retransmit_th_get
 */
typedef int (*dnx_iqs_db_credit_watchdog_retransmit_th_get_cb)(
    int unit, int *retransmit_th);

/**
 * implemented by: dnx_iqs_db_credit_watchdog_delete_th_set
 */
typedef int (*dnx_iqs_db_credit_watchdog_delete_th_set_cb)(
    int unit, uint32 delete_th_idx_0, int delete_th);

/**
 * implemented by: dnx_iqs_db_credit_watchdog_delete_th_get
 */
typedef int (*dnx_iqs_db_credit_watchdog_delete_th_get_cb)(
    int unit, uint32 delete_th_idx_0, int *delete_th);

/**
 * implemented by: dnx_iqs_db_credit_watchdog_delete_th_alloc
 */
typedef int (*dnx_iqs_db_credit_watchdog_delete_th_alloc_cb)(
    int unit);

/**
 * implemented by: dnx_iqs_db_delete_credit_request_profile_valid_set
 */
typedef int (*dnx_iqs_db_delete_credit_request_profile_valid_set_cb)(
    int unit, uint32 valid);

/**
 * implemented by: dnx_iqs_db_delete_credit_request_profile_valid_get
 */
typedef int (*dnx_iqs_db_delete_credit_request_profile_valid_get_cb)(
    int unit, uint32 *valid);

/**
 * implemented by: dnx_iqs_db_delete_credit_request_profile_id_set
 */
typedef int (*dnx_iqs_db_delete_credit_request_profile_id_set_cb)(
    int unit, int id);

/**
 * implemented by: dnx_iqs_db_delete_credit_request_profile_id_get
 */
typedef int (*dnx_iqs_db_delete_credit_request_profile_id_get_cb)(
    int unit, int *id);

/*
 * STRUCTs
 */

/**
 * This structure holds the access functions for the variable valid
 */
typedef struct {
    dnx_iqs_db_credit_request_profile_valid_set_cb set;
    dnx_iqs_db_credit_request_profile_valid_get_cb get;
} dnx_iqs_db_credit_request_profile_valid_cbs;

/**
 * This structure holds the access functions for the variable thresholds
 */
typedef struct {
    dnx_iqs_db_credit_request_profile_thresholds_set_cb set;
    dnx_iqs_db_credit_request_profile_thresholds_get_cb get;
} dnx_iqs_db_credit_request_profile_thresholds_cbs;

/**
 * This structure holds the access functions for the variable dnx_iqs_credit_request_profile_db_t
 */
typedef struct {
    dnx_iqs_db_credit_request_profile_alloc_cb alloc;
    /**
     * Access struct for valid
     */
    dnx_iqs_db_credit_request_profile_valid_cbs valid;
    /**
     * Access struct for thresholds
     */
    dnx_iqs_db_credit_request_profile_thresholds_cbs thresholds;
} dnx_iqs_db_credit_request_profile_cbs;

/**
 * This structure holds the access functions for the variable queue_min
 */
typedef struct {
    dnx_iqs_db_credit_watchdog_queue_min_set_cb set;
    dnx_iqs_db_credit_watchdog_queue_min_get_cb get;
} dnx_iqs_db_credit_watchdog_queue_min_cbs;

/**
 * This structure holds the access functions for the variable queue_max
 */
typedef struct {
    dnx_iqs_db_credit_watchdog_queue_max_set_cb set;
    dnx_iqs_db_credit_watchdog_queue_max_get_cb get;
} dnx_iqs_db_credit_watchdog_queue_max_cbs;

/**
 * This structure holds the access functions for the variable enable
 */
typedef struct {
    dnx_iqs_db_credit_watchdog_enable_set_cb set;
    dnx_iqs_db_credit_watchdog_enable_get_cb get;
} dnx_iqs_db_credit_watchdog_enable_cbs;

/**
 * This structure holds the access functions for the variable retransmit_th
 */
typedef struct {
    dnx_iqs_db_credit_watchdog_retransmit_th_set_cb set;
    dnx_iqs_db_credit_watchdog_retransmit_th_get_cb get;
} dnx_iqs_db_credit_watchdog_retransmit_th_cbs;

/**
 * This structure holds the access functions for the variable delete_th
 */
typedef struct {
    dnx_iqs_db_credit_watchdog_delete_th_set_cb set;
    dnx_iqs_db_credit_watchdog_delete_th_get_cb get;
    dnx_iqs_db_credit_watchdog_delete_th_alloc_cb alloc;
} dnx_iqs_db_credit_watchdog_delete_th_cbs;

/**
 * This structure holds the access functions for the variable dnx_iqs_credit_watchdog_db_t
 */
typedef struct {
    /**
     * Access struct for queue_min
     */
    dnx_iqs_db_credit_watchdog_queue_min_cbs queue_min;
    /**
     * Access struct for queue_max
     */
    dnx_iqs_db_credit_watchdog_queue_max_cbs queue_max;
    /**
     * Access struct for enable
     */
    dnx_iqs_db_credit_watchdog_enable_cbs enable;
    /**
     * Access struct for retransmit_th
     */
    dnx_iqs_db_credit_watchdog_retransmit_th_cbs retransmit_th;
    /**
     * Access struct for delete_th
     */
    dnx_iqs_db_credit_watchdog_delete_th_cbs delete_th;
} dnx_iqs_db_credit_watchdog_cbs;

/**
 * This structure holds the access functions for the variable valid
 */
typedef struct {
    dnx_iqs_db_delete_credit_request_profile_valid_set_cb set;
    dnx_iqs_db_delete_credit_request_profile_valid_get_cb get;
} dnx_iqs_db_delete_credit_request_profile_valid_cbs;

/**
 * This structure holds the access functions for the variable id
 */
typedef struct {
    dnx_iqs_db_delete_credit_request_profile_id_set_cb set;
    dnx_iqs_db_delete_credit_request_profile_id_get_cb get;
} dnx_iqs_db_delete_credit_request_profile_id_cbs;

/**
 * This structure holds the access functions for the variable dnx_iqs_delete_credit_request_profile_db_t
 */
typedef struct {
    /**
     * Access struct for valid
     */
    dnx_iqs_db_delete_credit_request_profile_valid_cbs valid;
    /**
     * Access struct for id
     */
    dnx_iqs_db_delete_credit_request_profile_id_cbs id;
} dnx_iqs_db_delete_credit_request_profile_cbs;

/**
 * This structure holds the access functions for the variable dnx_iqs_db_t
 */
typedef struct {
    dnx_iqs_db_is_init_cb is_init;
    dnx_iqs_db_init_cb init;
    /**
     * Access struct for credit_request_profile
     */
    dnx_iqs_db_credit_request_profile_cbs credit_request_profile;
    /**
     * Access struct for credit_watchdog
     */
    dnx_iqs_db_credit_watchdog_cbs credit_watchdog;
    /**
     * Access struct for delete_credit_request_profile
     */
    dnx_iqs_db_delete_credit_request_profile_cbs delete_credit_request_profile;
} dnx_iqs_db_cbs;

/*
 * Global Variables
 */

/*
 * Global Variables
 */

extern dnx_iqs_db_cbs dnx_iqs_db;

#endif /* __DNX_IQS_ACCESS_H__ */
