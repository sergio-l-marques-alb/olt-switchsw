 ...
 *   \param [in] field_name -
 *       the new field name. Maximal size for this input is DBAL_MAX_STRING_LENGTH.\n
...
/**
 * \brief
 *   create a dynamic DBAL table, only applicable for specific access methods such as KBP and MDB.\n
 *   those tables are used for ACLs. Dynamic result types are not supported for dynamic tables
 * \par DIRECT INPUT:
 *   \param [in] unit
 ...
 *               and from LSB to MSB for hard logic (but dynamic table for hard logic is not supported at the moment).
 *   \param [in] result_lsb_padding - Increase the result size by shifting the offset of all result fields by this amount.
...
/**
 * \brief Converts the given logical value of the given field_id to its corresponding HW value
 *
 * \par DIRECT INPUT:
 *   \param [in] unit
 *   \param [in] field_id      - the field to get the HW value for\n
 *   \param [in] logical_value - logical value to convert of the field\n
