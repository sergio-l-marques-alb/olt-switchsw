...
typedef enum
{
     ...
     *  One and only one of these flags must appear when committing an entry:
     *  DBAL_COMMIT, DBAL_COMMIT_UPDATE and DBAL_COMMIT_FORCE */
    ...
    /** Allows operation (commit/get/clear) to HW only (no shadow)
     * - supported for MDB EM tables only
     * - Commit normal: Only HW is checked for non existence.
     * If entry exists in SW only, HW commit will still work.
     * - Commit update: Only HW is checked for existence.
     * Even if entry exists in SW, HW update will still work.
     * - Clear entry: Check if entry exists in SW, if it does, return error,
     * if not, check entry exists in HW, then clear entry
     * This way, if an entry exists in SW, it cannot be removed from HW only */
...
typedef enum
{
    /** Returns all the entries including "0" and default entries. Valid only for SW and HL */
    DBAL_ITER_MODE_ALL,

    /** Returns all the entries except default values. If default is not defined, value "0" is used */
...
typedef struct
{
     ...
     * Only relevant if the max_value is smaller than the field's max_value otherwise.
     * Only relevant if field_nof_bits is at most 32 bits.
     * 0 means to ignore this value. */
    uint32 max_value;
