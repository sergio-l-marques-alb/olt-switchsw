...
#define SW_STATE_DEFRAGMENTED_CHUNK_OFFSET_UNRESERVE(module_id, defragmented_chunk, slots_in_offset, piece_offset)\
    SHR_IF_ERR_EXIT(sw_state_defragmented_chunk_offset_unreserve(unit, module_id, defragmented_chunk, slots_in_offset, piece_offset))

#define SW_STATE_DEFRAGMENTED_CHUNK_OFFSET_RESERVE(module_id, defragmented_chunk, slots_in_offset, piece_offset)\
    SHR_IF_ERR_EXIT(sw_state_defragmented_chunk_offset_reserve(unit, module_id, defragmented_chunk, slots_in_offset, piece_offset))

#define SW_STATE_DEFRAGMENTED_CHUNK_DEFRAG(module_id, defragmented_chunk, wanted_size, move_cb_additional_info, piece_offset)\
    SHR_IF_ERR_EXIT(sw_state_defragmented_chunk_defrag(unit, module_id, defragmented_chunk, wanted_size, move_cb_additional_info, piece_offset))
...
typedef struct
{
     ...
     * the value is the exponent to a base of 2 and the resulted power is the size
    ...
    /**
     * move functions cb_db to use in case a defrag procedure is required.
     */
...
/**
 * \brief - reserve a piece starting from given offset
 *
 * \param [in] unit - unit number
 * \param [in] module_id - Module ID.
 * \param [in] defragmented_chunk - Defragmented chunk.
 * \param [in] slots_in_offset - the size that we're trying to reserve, has to be power of 2
 * \param [in] piece_offset - the offset of the piece we're trying to reserve, has to be aligned to (N * slots_in_offset)
 *
 * \return
 *   shr_error_e
 *
 * \remark
 *   * provided range has to be either not used or already reserved. reserved slots are reduced from available free slots
 * \see
 *   * None
 */
...
/**
 * \brief - unreserve a reserved piece starting from given offset
 *
 * \param [in] unit - unit number
 * \param [in] module_id - Module ID.
 * \param [in] defragmented_chunk - Defragmented chunk.
 * \param [in] slots_in_offset - the size that we're trying to unreserve, has to be power of 2
 * \param [in] piece_offset - the offset of the piece we're trying to unreserve, has to be aligned to (N * slots_in_offset)
 *
 * \return
 *   shr_error_e
 *
 * \remark
 *   * provided range has to be already reserved. unreserved slots are added to available free slots
 * \see
 *   * None
 */
