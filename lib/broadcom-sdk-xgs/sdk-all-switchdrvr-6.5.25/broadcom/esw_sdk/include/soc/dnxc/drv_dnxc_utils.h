...
typedef enum
{
    ...
    JERICHO2_A0_DEVICE,
    /*
     *  See SOC_IS_JERICHO2_B0()
     */
    JERICHO2_B0_DEVICE,
    /*
     *  See SOC_IS_JERICHO2_B1()
     */
    JERICHO2_B1_ONLY,
    /*
     *  See SOC_IS_DNX()
...
/*
 * Function:
 *      dnx_drv_soc_flags_get
 * Purpose:
 *      Read value of 'soc_control[unit]->soc_flags' for specified unit.
 * Parameters:
 *      unit - Device unit #
 * Returns:
 *      uint32 - Value of 'soc_control[unit]->soc_flags'. If 'unit' is illegal then
 *      return '-1'
 */
uint32 dnx_drv_soc_flags_get(
    int unit);
/*
 * Function:
 *      dnx_drv_soc_flags_set
 * Purpose:
 *      Set 'value' into 'soc_control[unit]->soc_flags' of specified unit.
 * Parameters:
 *      unit -
 *          Device unit #
 *      value -
            Value to load into 'soc_control[unit]->soc_flags'
 * Returns:
 *      int - If 'unit' is illegal then no action is taken (no memory is written
 *      to) and a negative  value is returned. Otherwise, zero is returned.
 */
int dnx_drv_soc_flags_set(
    int unit,
    uint32 value);
 ...
 *     returned.
 *   * If 'soc_control[unit]' is NULL (has not been loaded) then FALSE is
 *     returned.
 * \see
 *   * SOC_IS_JERICHO
...
/**
 * \brief
 *   Procedure used to hide the PROCEDURE dnx_redo_soft_reset_soft_init(unit, perform_soft_reset, without_fabric, without_ile)
 *   (Configuration of the Soft init)
 */
shr_error_e dnx_drv_soc_redo_soft_reset_soft_init(
    int unit,
