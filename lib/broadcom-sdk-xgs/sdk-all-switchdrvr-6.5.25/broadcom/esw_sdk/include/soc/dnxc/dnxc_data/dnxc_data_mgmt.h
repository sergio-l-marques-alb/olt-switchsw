...
/* Ignore/Reset INIT ONLY feature */
...
*   \param [in] data_flags - pointer to data flags see DNXC_DATA_F_#
...
*   * None
* \remark
*   * We input pointer to the flags as they may get changed as result of running the set_func cb
* \see
*   * None
*/
...
/**
* \brief - component state get
*          Return the DNXC DATA module state.
* \par DIRECT_INPUT:
*   \param [in] unit - unit #
*
* \par INDIRECT INPUT:
 ...
 * Used for shell services to get dnx data table data.
 * Enable to get 'init-only' tables data for tests and diagnostics.
 * \par DIRECT_INPUT:
 *   \param [in] unit - Unit #
 *   \param [in] table - pointer to table structure in database
 *   \param [in] key0 - key1 index - 0 if no index
 *   \param [in] key1 - key2 index - 0 if no index
 *
 * \par INDIRECT INPUT:
