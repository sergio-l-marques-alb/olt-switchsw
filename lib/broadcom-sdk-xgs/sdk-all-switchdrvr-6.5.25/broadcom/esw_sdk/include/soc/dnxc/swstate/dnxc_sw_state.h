...
#define DNX_SW_STATE_MODULE_INIT(unit, module_id, implementation, type, flags, dbg_string)\
    DNXC_SW_STATE_IF_ERR_ASSERT(dnxc_sw_state_dispatcher_module_init(unit, module_id, implementation));\
    DNX_SW_STATE_DIAG_INFO_PRE(unit, module_id);\
    DNXC_SW_STATE_IF_ERR_ASSERT(dnxc_sw_state_dispatcher[unit][module_id].module_init(unit, module_id, sizeof(type), flags, dbg_string));

/**
 * \brief - deinitialize a sw state module
 */
#define DNX_SW_STATE_MODULE_DEINIT(unit, module_id, flags, dbg_string)\
    DNXC_SW_STATE_IF_ERR_ASSERT(dnxc_sw_state_dispatcher[unit][module_id].module_deinit(unit, module_id, flags, dbg_string));\
...
#define DNX_SW_STATE_ALLOC_BASIC(unit, module_id, ptr_location, num_of_elements, size, flags, dbg_string)\
    DNXC_SW_STATE_IF_ERR_ASSERT(dnxc_sw_state_dispatcher[unit][module_id].alloc(unit, module_id, ptr_location, num_of_elements, size, flags, dbg_string));
...
#define DNX_SW_STATE_FREE_BASIC(unit, module_id, ptr_location, flags, dbg_string)\
    DNXC_SW_STATE_IF_ERR_ASSERT(dnxc_sw_state_dispatcher[unit][module_id].free(unit, module_id, ptr_location, flags, dbg_string));
...
#define DNX_SW_STATE_SET(unit, module_id, location, input, type, flags, dbg_string)\
    DNXC_SW_STATE_IF_ERR_ASSERT(dnxc_sw_state_dispatcher[unit][module_id].memcpy(unit, module_id, sizeof(type), (uint8 *)(&location), (uint8 *)(&input), flags, dbg_string));
...
#define DNX_SW_STATE_COUNTER_INC(unit, module_id, location, inc_val, type, flags, dbg_string)\
    DNXC_SW_STATE_IF_ERR_ASSERT(dnxc_sw_state_dispatcher[unit][module_id].counter_inc(unit, module_id, (uint8 *)(&location), inc_val, sizeof(type), flags, dbg_string));
...
#define DNX_SW_STATE_COUNTER_DEC(unit, module_id, location, dec_val, type, flags, dbg_string)\
    DNXC_SW_STATE_IF_ERR_ASSERT(dnxc_sw_state_dispatcher[unit][module_id].counter_dec(unit, module_id, (uint8 *)(&location), dec_val, sizeof(type), flags, dbg_string));

/**
 * \brief - sw state basic memcpy
 */
#define DNX_SW_STATE_MEMCPY_BASIC(unit, module_id, destination, source, size, flags, dbg_string) \
    DNXC_SW_STATE_IF_ERR_ASSERT(dnxc_sw_state_dispatcher[unit][module_id].memcpy(unit, module_id, size, (uint8 *)(destination), (uint8 *)(source), flags, dbg_string));
...
#define DNX_SW_STATE_MEMSET(unit, module_id, location, offset, value, length, flags, dbg_string)\
    DNXC_SW_STATE_IF_ERR_ASSERT(dnxc_sw_state_dispatcher[unit][module_id].memset(unit, module_id, (uint8 *)(location) + offset, value, length, flags, dbg_string));

#define DNX_SW_STATE_MEMWRITE_BASIC(unit, module_id, src, src_offset, dest, dest_offset, length, flags, dbg_string)\
    DNXC_SW_STATE_IF_ERR_ASSERT(dnxc_sw_state_dispatcher[unit][module_id].memcpy(unit, module_id, length, (uint8 *)(dest) + dest_offset, (uint8 *)(src) + src_offset, flags, dbg_string));
...
#define DNX_SW_STATE_MUTEX_CREATE(unit, module_id, mtx, desc)\
    DNXC_SW_STATE_IF_ERR_ASSERT(dnxc_sw_state_dispatcher[unit][module_id].mutex_create(unit, module_id, (&(mtx)), desc, 0));
...
#define DNX_SW_STATE_MUTEX_DESTROY(unit, module_id, mtx)\
    DNXC_SW_STATE_IF_ERR_ASSERT(dnxc_sw_state_dispatcher[unit][module_id].mutex_destroy(unit, module_id, (&(mtx)), 0));

/**
 * \brief - sw state mutex take
 */
#define DNX_SW_STATE_MUTEX_TAKE(unit, module_id, mtx, usec)\
    DNXC_SW_STATE_IF_ERR_ASSERT(dnxc_sw_state_dispatcher[unit][module_id].mutex_take(unit, module_id, &(mtx), usec, 0));
...
#define DNX_SW_STATE_MUTEX_GIVE(unit, module_id, mtx)\
    DNXC_SW_STATE_IF_ERR_ASSERT(dnxc_sw_state_dispatcher[unit][module_id].mutex_give(unit, module_id, &(mtx), 0));

/**
 * \brief - sw state semaphore create
 */
#define DNX_SW_STATE_SEM_CREATE(_unit, _module_id, _sem, _is_binary, _initial_count, _desc)\
    DNXC_SW_STATE_IF_ERR_ASSERT(dnxc_sw_state_dispatcher[_unit][_module_id].sem_create(_unit, _module_id, &(_sem), _is_binary, _initial_count, _desc, 0));
...
/**
 * \brief - sw state semaphore destroy
 */
#define DNX_SW_STATE_SEM_DESTROY(_unit, _module_id, _sem)\
    DNXC_SW_STATE_IF_ERR_ASSERT(dnxc_sw_state_dispatcher[_unit][_module_id].sem_destroy(_unit, _module_id, &(_sem), 0));

/**
 * \brief - sw state semaphore take
 */
#define DNX_SW_STATE_SEM_TAKE(_unit, _module_id, _sem, _usec)\
    DNXC_SW_STATE_IF_ERR_ASSERT(dnxc_sw_state_dispatcher[_unit][_module_id].sem_take(_unit, _module_id, &(_sem), _usec, 0));

/**
 * \brief - sw state semaphore give
 */
#define DNX_SW_STATE_SEM_GIVE(_unit, _module_id, _sem)\
    DNXC_SW_STATE_IF_ERR_ASSERT(dnxc_sw_state_dispatcher[_unit][_module_id].sem_give(_unit, _module_id, &(_sem), 0));
...
/**
 * \brief - initialize a sw state no WB module during WB is on.
 */
#define DNX_SW_STATE_NO_WB_MODULE_INIT(unit, module_id, root, implementation, type, flags, dbg_string)\
    DNXC_SW_STATE_IF_ERR_ASSERT(dnxc_sw_state_dispatcher_module_init(unit, module_id, implementation));\
    DNXC_SW_STATE_IF_ERR_ASSERT(dnxc_sw_state_alloc_plain(unit, module_id, (uint8**)&root, 1, sizeof(type), flags, dbg_string));

/**
 * \brief - deinitialize a sw state no WB module during WB is on
 */
#define DNX_SW_STATE_NO_WB_MODULE_DEINIT(unit, module_id, root, flags, dbg_string)\
    DNXC_SW_STATE_IF_ERR_ASSERT(dnxc_sw_state_free_plain(unit, module_id, (uint8**)&root, flags, dbg_string));\
    DNXC_SW_STATE_IF_ERR_ASSERT(dnxc_sw_state_dispatcher_module_deinit(unit));
/*
 * }
 */
