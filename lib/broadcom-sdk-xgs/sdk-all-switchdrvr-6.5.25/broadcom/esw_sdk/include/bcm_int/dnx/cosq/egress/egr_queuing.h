...
/**
 * \brief - set credit size and init credits for special interfaces
 *
 * \param [in] unit -  Unit-ID
 * \param [in] port -  local port
 * \param [in] speed -  speed of the port
 *
 * \return
 *   shr_error_e
 * \remark
 ...
 *   * None
 */
shr_error_e dnx_egr_queuing_special_if_credit_set(
    int unit,
    bcm_port_t port,
    int speed);
 ...
 */
shr_error_e dnx_egr_queuing_nif_calendar_refresh(
    int unit,
    bcm_core_t core);
 ...
 */
shr_error_e dnx_egr_queuing_nif_calendar_activate(
    int unit,
    bcm_core_t core);
 ...
 *   shr_error_e
 * \remark
 *   * When called for RCY port, this function needs to receive as 
 *     input the total speed of both RCY interfaces, because both
 *     RCY interfaces are represented as a single TXI
 * \see
 *   * None
 */
shr_error_e dnx_egr_queuing_total_egr_if_credits_sw_update(
    int unit,
    bcm_port_t logical_port,
    int speed);
...
/**
 * \brief -
 *   Get correct Egress IF including for RCY interfaces
 * \param [in] unit -
 *   Int. Identifier of HW platform.
 * \param [in] logical_port -
 *   Int. Logical port for which to get the Egress IF
 * \param [out] egr_if -
 *   int. Egress interface ID
 * \return
 *    \retval Zero - On success
 *    \retval Error - Identifier as per shr_error_e
 * \remark
 * Recycle interface is special in a way that it has 2 egress interfaces.
 * but only one RXI - TXI, so in some cases a specific egress IF for RCY1 needs to be used.
 * \see
 */
shr_error_e dnx_egr_queuing_fqp_egr_if_get(
    int unit,
