...
/**
 * \brief In case we're working with max possible pointer size, we remove the outlif profile bit since we're missing 1 bit. In this case, the outlif pointer is capable of accessing a larger space than the eedb banks, this might cause aliasing and is handled throughout the lif manager
 */
...
/**
 * \brief This struct holds the extra info for outlif bank resource create
 */
typedef struct
{
    int granularity;
 ...
 *   \param [in,out] global_lif_info - global lif allocation information such as BTA and a pointer to memory to write output into. \n
...
/**
 * \brief - Given a unit, returns the number of inlifs (DBC, DPC)
 *        in the device.
 *
 * \param [in] unit - Identifier of the device to access.
 * \param [out] nof_sbc_lifs - if not null, filled with the
 *        number of sbc LIFs in the device.
 * \param [out] nof_dpc_lifs - if not null, filled with the
 *        number of dpc LIFs in the device.
 * \return
 *   shr_error_e
 *
 * \remark
...
*          type.
*   \param [out] logical_phase          - Out-LIF logical phase.
...
/**
 * \brief
 * translate logical phase string to logical phase id.
 *
 *  \par DIRECT INPUT:
 *    \param [in] unit
 *    \param [in] logical_phase_str - \n
 *      string to translate
 *    \param [in] logical_phase - \n
 *      pointer to phase id
 *
 *  \par DIRECT OUTPUT:
 *    shr_error_e - \n
...
/**
 * \brief - Converts a BCM API encap_access enum to an EEDB
 *        logical phase.
 *
 * \param [in] unit - Device ID.
 * \param [in] encap_access - The encap_access enum to convert
 * \param [out] logical_phase - Pointer to a converted logical
 *        phase.
 * \return
 *   shr_error_e
 * \see
...
/**
 * \brief - Converts an EEDB logical phase to a BCM API
 *        encap_access enum.
 *
 * \param [in] unit - Device ID.
 * \param [in] logical_phase - The logical phase to convert.
 * \param [out] encap_access - Pointer to a converted
 *        encap_access enum.
 * \return
 *   shr_error_e
 * \see
...
/**
 * \brief
 *  get the bta value per global lif id from HW. if feature not exist, return -1
 * \param [in] unit -unit id
 * \param [in] global_lif - global lif id
 * \param [out] bta - bytes to add HW value
 * \return
 *  shr_error_e
 * \remark
 *   NONE
