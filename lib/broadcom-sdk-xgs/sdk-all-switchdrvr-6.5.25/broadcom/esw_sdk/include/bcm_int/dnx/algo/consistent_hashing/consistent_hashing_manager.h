 ...
 * \param [in] nof_hierarchies - number of hierarchies the CHM needs to manage.
 *        this is relevant if the HW reasource that the CHM uses is in divided
 *        to banks, and the user has multiple hierarchies, and each bank can be
 *        accessed by a single hierarchy at a time.
 * \param [in] nof_banks - number of banks the HW reasources are divided to,
 *        see description in nof_hierarchies. bank size - which id defined by
 *        (nof_resources_to_manage / nof_banks) has to be a power of 2.
 ...
 *   
 * \remark
 *   * nof_hierarchies and nof_banks are relevant only if the HW resources used
 *   can support multiple hierarchies but a bank can be accessed only by a single
 *   hierarchy at a time. in any other case - nof_hierarchies and nof_banks are
 *   irrelevant and should be 1.
 * \see
 *   * None
 */
 ...
 * \param [in] hierarchy - hierarchy that the profile is associated with.
