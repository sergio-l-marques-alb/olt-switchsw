...
/**
 * \brief
 *   List of HBM APD firmware parameter offsets.
 *   Used to indicate which parameter we want to access on a get / set param interrupt.
...
/**
 * \brief
 *   List of HBM APD firmware parameter offsets, for the firmware version the supports HBM2E
 *   Used to indicate which parameter we want to access on a get / set param interrupt.
 *   Can be accessed through the following interrupt codes:
 *     DNX_HBMC_FIRMWARE_INTERRUPT_CODE_GET_PARAMETER_VALUE
 *     DNX_HBMC_FIRMWARE_INTERRUPT_CODE_SET_PARAMETER
 ...
 *
 * \remark
 *   * This function takes a firmware mutex.
 * \see
 *   * none
 */
...
/** Use default value for the AAPL command */
#define DNX_HBMC_FIRMWARE_AAPL_CMD_DEFAULT (-1)

/**
 * \brief
 *   AAPL commands that can be run by dnx_hbmc_firmware_aapl_cmd_run.
 */
typedef enum
{
...
/**
 * \brief - Run AAPL command
 *
 * \param [in] unit - unit id.
 * \param [in] hbmc_index - HBMC index
 * \param [in] aapl_cmd - AAPL command to run
 * \param [in] aapl_params - AAPL params
 *
 * \return
 *   shr_error_e
 *
