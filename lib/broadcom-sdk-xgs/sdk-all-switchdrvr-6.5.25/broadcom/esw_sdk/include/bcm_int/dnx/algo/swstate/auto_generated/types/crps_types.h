...
/**
 * set of parameters unique for each counter set
 */
typedef struct {
    /**
     * Noumber of counters in single counter set, as configured in the API
     */
    int size;
    /**
     * start range (stat-id)
...
typedef struct {
    /**
     * first index that is admmited for an engine. for multi-type db, index + base-stat-id*set-size
     */
    uint32 logical_object_id_first;
    /**
     * last index that is admmited for an engine. for multi-type db, + base-stat-id*set-size
     */
...
typedef struct {
    ...
    /**
     * information about the counter sets configured in the interface set API.
     */
