...
/**
 * \brief
 *   HBM Mode Registers configuration.
 *   See spec in order to see what fields each MR has,
 *   and how each bit is configured.
 */
typedef struct
{
    /** Mode Registers */
    uint32 mr_val[DNX_DATA_MAX_DRAM_GENERAL_INFO_NOF_MRS];
} dnx_hbmc_mode_registers_t;
...
typedef struct hbmc_parity_err_cnt_s
{
    /** Number of write parity errors */
    uint32 write_parity_err_cnt[DNX_DATA_MAX_DRAM_HBM_NOF_CHANNEL_DWORDS][2];
    /** Number of write parity errors */
    uint32 read_parity_err_cnt[DNX_DATA_MAX_DRAM_HBM_NOF_CHANNEL_DWORDS][2];
} hbmc_parity_err_cnt_t;
...
typedef enum
{
     ...
     */
    DNX_HBMC_RESTORE_TUNE_PARAMS_FROM_OTP_OR_TUNE,
...
/** this enum describes PHY firmware generation */
typedef enum
{
    DNX_HBMC_HBM_GENERATION_HBM,
    DNX_HBMC_HBM_GENERATION_HBM2E,
    /** number of types */
    DNX_HBMC_HBM_GENERATION_COUNT
} dnx_hbmc_hbm_generation_e;
 ...
 *   * None
 */
void dnx_hbmc_mode_registers_default_values_get(
    int unit,
    int is_hbm2e,
 ...
 *   * None
 */
shr_error_e dnx_hbmc_cattrip_indication_mask_set(
    int unit,
    int hbm_index,
...
/**
 * \brief
 *   Run HBM PHY BIST in order to validate it's working as expected.
 *
 * \param [in] unit - unit number
 *
 * \return
...
/**
 * \brief - get parity errors counters
 *
 * \param [in] unit - unit number
 * \param [in] hbm_index - hbm index
 * \param [in] channel - channel
 * \param [out] error_counters - parity error counters
 *
 * \return
 *   shr_error_e
 *
 ...
 *   * None
 */
shr_error_e dnx_hbmc_parity_errors_get(
    int unit,
    int hbm_index,
    int channel,
 ...
 *   None.
 */
shr_error_e dnx_hbmc_apd_phy_init(
    int unit);
