 ...
 */
/**
 * For the follwing 4 'defines', see description
 * of 'fes_instruction_info_type' element and of 'fes_instruction_info_polarity' element
 * in dnx_field_fes_instruction_info_t
 */
/**
...
/**
 * Structure containing the required configuration needed for an action
 * to receive FES quartet and action mask allocation and to be written to HW.
 */
typedef struct
{
    /*
...
/**
 *  \brief
 *    Given an IO source and a range of bits within it, get the corresponding fes input data.
 * \param [in] unit -
 *    Identifier of HW platform.
 ...
 * \param [in] action_size - action size
 * \param [in] action_lsb - offset of the action
 ...
 */
shr_error_e dnx_field_actions_fes_key_select_get(
    int unit,
...
/**
 * \brief
 *    When creating a cascaded iPMF2 context, update the added EFESes from the iPMF1 context.
 * \param [in] unit - Identifier of HW platform.
 * \param [in] stage - The field group.
 * \param [in] context_id - The iPMF2 context_id to be updated.
 * \param [in] cascaded_from - The iPMF1 context id cascaded from.
 * \return
 *    \retval Zero - On success
 *    \retval Error - Identifier as per shr_error_e
 * \remark
 ...
 * \param [in] field_stage -
 *    For which of the PMF blocks the configuration is done (iPMF1/2/3, ePMF).
 * \param [in] fes_id - fes id
 * \param [in] fes_pgm_id -fes program
 *   selected 'fes_id'
 * \param [in] fes2msb_ndx -2 bit condition
 ...
 */
shr_error_e dnx_field_actions_fes_2nd_instruction_hw_get(
    int unit,
    dnx_field_stage_e field_stage,
    dnx_field_fes_id_t fes_id,
    dnx_field_fes_pgm_id_t fes_pgm_id,
 ...
 * \param [in] fes_id - fes id
 * \param [out] fes_pgm_id_p - fes program
 ...
 *   * dnx_field_actions_context_fes_info_get()
 */
shr_error_e dnx_field_fes_program_fes_instruction_hw_get(
    int unit,
    dnx_field_stage_e field_stage,
    dnx_field_context_t context_id,
    dnx_field_fes_id_t fes_id,
 ...
 */
/**
 * This remark relates only to adapter on the stage where it is still impossible to
 ...
 * See 'nof_fem_programs' in 'dnx_field.xml'
 */
...
/**
 * \brief
 *   Given an IO source and a range of bits within it, get the corresponding FEM DATA
 * \param [in] unit -
 *   Identifier of HW platform.
 * \param [in] actions_block -
 *   Variable of type dbal_enum_value_field_field_io_e. Identifier of the 'IO' within which
 ...
 * \see
 *   * See dnx_algo_field_key_is_any_key_occupied()
 */
...
/**
 * \brief
 *   Verify that the number of valid bits per FEM.
 * \param [in] unit -
 *   Identifier of HW platform.
 * \param [in] fem_id -
 *   Indicator for which 'FEM id' this operation is intended.
 ...
 * \param [in] unit -
 *   Identifier of HW platform.
 * \param [in] fem_priority -
 *    Indicator for which 'FEM id' this operation is intended, expecting wither position (FEM ID) or "don't care".
 * \param [in] fg_id -
 ...
 * \param [in] fem_condition_ms_bit - the ms bit offset in the input of 4 condition bits
...
shr_error_e dnx_field_fem_action_add(
    int unit,
    dnx_field_action_priority_t fem_priority,
    dnx_field_group_t fg_id,
    uint8 input_offset,
    uint8 input_size,
    dnx_field_group_t second_fg_id,
 ...
 *    Bitmap indicating which of the extractions are being used, ignoring the actual allocation for a specific FEM ID,
 *    ubt for an abstract FEM.
 * \param [out] fem_id_can_be_used_p -
 *    Indication if the FEM ID given can be allocated.
 * \return
 *   \retval Zero - On success
 *   \retval Error - Identifier as per shr_error_e
 * \remark
 ...
 *   * None
 */
shr_error_e dnx_field_action_fem_is_fem_id_available(
    int unit,
    dnx_field_fem_id_t fem_id,
    dnx_field_group_t fg_id,
    dnx_field_group_t second_fg_id,
 ...
 *   Identifier of HW platform.
 * \param [in] fem_id -
 *    Indicator for which 'FEM id'
 * \param [in] fem_program -
 *    Indicator for which 'FEM program'
...
/**
 * \brief
 *   Check if two dnx_field_fem_action_entry_t structures are effectively the same extraction/action.
 * \param [in] unit -
 *   Identifier of HW platform.
 * \param [in] fem_action_entry_1_p -
 *   The first structure to compare.
 * \param [in] fem_action_entry_2_p -
 *   The second structure to compare.
 * \param [out] is_equivalent_p -
 *   If the structure are the same.
 * \return
 *   \retval Zero - On success
 *   \retval Error - Identifier as per shr_error_e
 * \remark
 ...
 */
shr_error_e dnx_field_fem_action_entry_t_compare(
    int unit,
    dnx_field_fem_action_entry_t * fem_action_entry_1_p,
    dnx_field_fem_action_entry_t * fem_action_entry_2_p,
    uint8 *is_equivalent_p);
