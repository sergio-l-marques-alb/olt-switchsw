...
typedef struct dnx_rx_trap_erpp_config_s
{
    ...
    /** when set, drop precedence is valid */
    uint8 dp_valid;
...
typedef struct dnx_rx_trap_etpp_action_s
{
    ...
    /** When set, internal statistics are enabled */
    uint8 is_int_stats_en;

    /** When set, mirror copy is discarded */
    uint8 kill_mirror_copy;
...
typedef struct dnx_rx_parse_headers_s
{
    ...
    /** Time stamp of time sync protocol packets. */
    uint32 rx_timestamp;
    /** Upper 32-bit of 64-bit timestamp */
    uint32 rx_timestamp_upper;
