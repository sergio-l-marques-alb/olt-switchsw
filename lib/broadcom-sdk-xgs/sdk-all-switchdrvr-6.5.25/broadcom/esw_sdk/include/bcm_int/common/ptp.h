...
#if defined(BCM_DNX_SUPPORT)
 #define PTP_CONDITIONAL_IS_JERICHO2(unit) \
            SOC_IS_DNX2(unit)
#else
 #define PTP_CONDITIONAL_IS_JERICHO2(unit) (0)
#endif /* defined(BCM_DNX_SUPPORT) */

#if defined(BCM_FIRELIGHT_SUPPORT)
 #define PTP_CONDITIONAL_IS_FIRELIGHT(unit) (SOC_IS_FIRELIGHT(unit))
#else
 #define PTP_CONDITIONAL_IS_FIRELIGHT(unit) (0)
#endif
...
/* define SOC which support internal stack */
    ...
    defined(BCM_QUX_SUPPORT) || defined(BCM_DNX_SUPPORT) || defined(BCM_FIRELIGHT_SUPPORT)
...
#define SOC_HAS_PTP_INTERNAL_STACK_SUPPORT(unit)  (PTP_CONDITIONAL_IS_KATANA(unit) || PTP_CONDITIONAL_IS_TRIUMPH3(unit) ||    \
                                                   ...
                                                   PTP_CONDITIONAL_IS_MONTEREY(unit) || PTP_CONDITIONAL_IS_JERICHO2(unit) || \
                                                   PTP_CONDITIONAL_IS_FIRELIGHT(unit) \
                                                   )
#else
...
#define PTP_MGMTMSG_ID_APTS_GPS_OFFSET                             (0xdf09)
...
#define PTP_MGMTMSG_ID_PORT_DELAY_ASYMMETRY                        (0xd02f)
...
#define PTP_MGMTMSG_ID_DNX_JR2_HEADERS_CONFIGURATION               (0xd102)

#define PTP_MGMTMSG_ID_APTS_SOURCE_INPUT_PIN_CONFIGS               (0xd103)

#define PTP_MGMTMSG_ID_PACKET_DUMP_LEVEL                           (0xd104)
...
#define PTP_MGMTMSG_PAYLOAD_CLOCK_INSTANCE_SIZE_OCTETS                 (64)
...
/* Management message PORT_DELAY_ASYMMETRY TLV payload size (octets). */
#define PTP_MGMTMSG_PAYLOAD_PORT_DELAY_ASYMMETRY_SIZE_OCTETS           (14) 
...
/* Management message APTS GPS offset size (octets) */
#define PTP_MGMTMSG_PAYLOAD_APTS_GPS_OFFSET_SIZE_OCTETS                (4)
/* Management message PTP CLOCK INFO FLAGS size (octets) */
#define PTP_MGMTMSG_PAYLOAD_CLOCK_INFO_FLAGS_SIZE_OCTETS               (10)
/* Management message PTP JR2 DNX Headers configuration size (octets) */
#define PTP_MGMTMSG_PAYLOAD_DNX_JR2_HEADERS_CONFIGURATION_SIZE_OCTETS    (65)

/* Management message APTS input source configurations size (octets) */
#define PTP_MGMTMSG_PAYLOAD_APTS_INPUT_SOURCE_CONFIGS_SIZE_OCTETS       (40)

/* Management message PACKET_DUMP_LEVEL TLV payload size (octects) */
#define PTP_MGMTMSG_PAYLOAD_PACKET_DUMP_LEVEL_SIZE_OCTETS              (14)
...
typedef enum _bcm_ptp_event_e {
    ...
    _bcm_ptp_pdv_thres_exceed_event= 0x16,
...
/* Enumeration of events for master change  reason code from BMCA */
typedef enum _bcm_ptp_bmca_reason_code_e {
    _bcm_ptp_bmca_reason_code_null_announce = 0,
    _bcm_ptp_bmca_reason_code_better_priority1,
    _bcm_ptp_bmca_reason_code_better_class,
    _bcm_ptp_bmca_reason_code_better_accuracy,
    _bcm_ptp_bmca_reason_code_better_scale_variance,
    _bcm_ptp_bmca_reason_code_better_priority2,
    _bcm_ptp_bmca_reason_code_better_identity,
    _bcm_ptp_bmca_reason_code_better_steps_removed_plus_1,
    _bcm_ptp_bmca_reason_code_better_sender_identity,
    _bcm_ptp_bmca_reason_code_better_topology,
    _bcm_ptp_bmca_reason_code_better_src_port_number,
    _bcm_ptp_bmca_reason_code_better_rx_port_number,
    _bcm_ptp_bmca_reason_code_better_preferred,
    _bcm_ptp_bmca_reason_code_better_signal_fail,
    _bcm_ptp_bmca_reason_code_better_local_priority,
    _bcm_ptp_bmca_reason_code_localclock_grand_master,
    _bcm_ptp_bmca_reason_code_last
} _bcm_ptp_bmca_reason_code_t;
...
extern int _bcm_ptp_telecom_g8265_packet_master_ptsf_loss_announce_set(
    int unit,
    bcm_ptp_stack_id_t ptp_id,
    int clock_num,
    bcm_ptp_clock_port_address_t *address,
    _bcm_ptp_event_t event_type);
...
extern int _bcm_ptp_packet_dump_configure(
    int unit,
    bcm_ptp_stack_id_t ptp_id,
    int clock_num,
    uint32 dump_mask,
...
extern int _bcm_ptp_update_peer_counts(
    ...
    bcm_gport_t *phy_port, unsigned remote_port_num);

extern int _bcm_ptp_update_packet_counts(
        int ptpPortNum,
        bcm_ptp_protocol_t protocol,
        uint32 packets_transmitted,
...
extern int g_apts_servo_assist_gps;
...
extern int g_apts_synce_freq_corr_ppb;

extern void _bcm_ptp_apts_update_current_mode(int unit, int stack_id);
extern int _bcm_ptp_apts_apply_servo_issued_freq_corr(void);
...
#define BCM_PTP_DNX_JER2_UDH_DATA_TYPE_MAX          (4)

/* PPH LIF Ext. 3 bit type */
#define BCM_PTP_DNX_JER2_PPH_LIF_EXT_TYPE_MAX       (8)


/* struct to represent system headers information passed from
 *  host to ukernel
 */
typedef struct  dnx_jr2_system_headers_info_s {
    uint32      ftmh_lb_key_ext_size;
    uint32      ftmh_stacking_ext_size;
    uint32      pph_base_size;
    uint32      pph_lif_ext_size[BCM_PTP_DNX_JER2_PPH_LIF_EXT_TYPE_MAX];
    uint8       system_headers_mode;
    uint8       udh_enable;
    uint8       udh_data_lenght_per_type[BCM_PTP_DNX_JER2_UDH_DATA_TYPE_MAX];
    uint32      cosq_port_cpu_channel;
    uint32      cosq_port_pp_port;
} dnx_jr2_system_headers_info_t;
