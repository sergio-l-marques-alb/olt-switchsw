...
#define _BCM_VXLAN_TUNNEL_TERM_KEEP_INNER_DSCP     0x10
...
#define _BCM_VXLAN_PORT_MATCH_TYPE_PORT_ETAG_VLAN        (1 << 9)
...
/*
 * Software info book keeping for
 * PHB from VXLAN tunnel DSCP during termination.
 */
typedef struct _bcm_td3_vxlan_bookkeeping_s {
    /*
     * PHB_MAPPING_TBL_2 index bitmap, set only
     * if it has any phb ctrl id (PHB_SELECT_TCAM entry)
     * allocated to it.
     */
    SHR_BITDCL *phb_map_bitmap;

    /*
     * And PHB_MAPPING_TBL_2 index to phb ctrl id
     * (PHB_SELECT_TCAM index) mapping
     */
    uint8      *phb_map_hwid_ctrl_id;

    /*
     * Tracks the phb ctrl_id (PHB_SELECT_TCAM entry)
     * alloc/freeing.
     */
    SHR_BITDCL *phb_sel_used;

    /*
     * Multiple tunnels can be mapped to phb map id
     * PHB_MAPPING_TBL_2 index, so which means
     * more than one tunnel can use the
     * phb_ctrl_id. As a single phb_ctrl_id
     * (PHB_SELECT_TCAM) entry will be allocated
     * to given PHB_MAPPING_TBL_2 entry/mapid.
     */
    uint16     *phb_sel_refcount;
} _bcm_td3_vxlan_bookkeeping_t;

extern _bcm_td3_vxlan_bookkeeping_t *_bcm_td3_vxlan_info[BCM_MAX_NUM_UNITS];
#endif
...
extern int _bcm_td2_vxlan_match_get(int unit,
                                    bcm_vxlan_port_t *vxlan_port,
                                    int vp, bcm_vpn_t vpn);
