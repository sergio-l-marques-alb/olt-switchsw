...
/**
 * \brief
 *  Check if error recovery journaling is supported for the current thread. 
 * \return
 *   int - Value indicating if journaling can be started from this thread
 * \remark
 *   * None
 * \see
 *   * None
 */
extern int sal_thread_is_journaling_allowed(void);
#endif
