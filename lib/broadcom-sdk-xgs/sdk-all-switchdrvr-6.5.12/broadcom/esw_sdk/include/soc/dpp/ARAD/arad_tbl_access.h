...
/* ARAD_EPNI_LLVP_TABLE_TBL_DATA: \{VLAN-Processing-Profile(3b), Outer-Tag(2), Outer-Tag-Is-Priority(1), Inner-Tag(2) \} */
...
/* Bits[n+1:n+0] - Number of tags to remove, Bits[n+3:n+2] - Inner PCP DEI source Bits[n+5:n+4] - Outer PCP DEI source 
   ...
   Note: This macro is also used to set native EVEC entry, since they have the same fields, except for packet_is_tagged_after_eve.
        So for native evec entry, set packet_is_tagged_after_eve to 0. 
   */
...
typedef struct
{
  ...
  /* Qax new fields, if set then hit bits bookkeeping is enabled for this lookup */
...
typedef struct
{
  ...
  uint8  custom_rif_profile_bit;   /*can be used to enable ipmc_bridge_v4mc_enable in IPv4mc or enable double capacity for Ipv4uc */
...
typedef struct
{
  ...
  uint32 data1[ARAD_PP_IHB_FEC_SUPER_ENTRY_TBL_ENTRY_SIZE]; /* Relevant for Jericho and above */
...
/* 
 * This function supports porting of ARAD to Jericho code.
 * Only 6 instructions are supported (as in ARAD)        .
 * New FLP programs are expected to use the DBAL mechanism which supports new devices properly.                                                      .
 */
uint32
