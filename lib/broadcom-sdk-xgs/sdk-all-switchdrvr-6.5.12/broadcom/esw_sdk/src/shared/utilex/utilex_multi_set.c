...
/*
 * utilex_multi_set is included
 */
#ifdef BCM_DNX_SUPPORT
#include <soc/dnx/swstate/auto_generated/types/utilex_multi_set_types.h>
#include <soc/dnx/swstate/auto_generated/access/utilex_multi_set_access.h>
#endif
    ...
    uint32 max_nof_multis)
{
    ...
    SHR_IF_ERR_EXIT(UTILEX_MULTI_SET_ACCESS.init(unit));
...
shr_error_e
utilex_multi_set_destroy_all(
    int unit)
{
    uint32 multi_set_index;
    uint8 bit_val;
    uint32 in_use, max_nof_multis;
    UTILEX_MULTI_SET_PTR multi_set;
    ...
    {
        /*
         * If number of multi sets is beyond the maximum then quit
         * with error.
         */
        LOG_CLI((BSL_META_U(unit, "nof multi sets (%d) exceed the amount allowed per unit (%d). "), in_use,
                 max_nof_multis));
        SHR_SET_CURRENT_ERR(_SHR_E_INTERNAL);
        SHR_EXIT();
    }
    /*
     * Run through all occupied multi sets (a set bit in 'occupied_multis') and destroy them one by one
     */
    for (multi_set_index = 0; multi_set_index < max_nof_multis; multi_set_index++)
    {
        SHR_IF_ERR_EXIT(UTILEX_MULTI_SET_ACCESS.occupied_multis.bit_get(unit, multi_set_index, &bit_val));
        ...
        {
            /*
             * 'multi_set_index' is now the index of an occupied multi set.
             */
            UTILEX_MULTI_SET_CONVERT_MULTISET_INDEX_TO_HANDLE(multi_set, multi_set_index);
            SHR_IF_ERR_EXIT(utilex_multi_set_destroy(unit, multi_set));
        }
    }
    SHR_IF_ERR_EXIT(UTILEX_MULTI_SET_ACCESS.occupied_multis.free(unit));
    SHR_IF_ERR_EXIT(UTILEX_MULTI_SET_ACCESS.multis_array.ptr_free(unit));
    /*
    ...
    int unit)
{
    ...
    UTILEX_MULTI_SET_VERIFY_UNIT_IS_LEGAL(unit);
    {
        ...
        for (jj = 0; jj < init_info.max_duplications; jj++)
        {
            for (ii = 1; ii < (num_members_for_test + 1); ii++)
            {
                ...
                key[0] = 100;
                key[1] = ii;
                SHR_IF_ERR_EXIT(utilex_multi_set_member_add
                                (unit, multi_set, &key[0], &data_indx, &first_appear, &success));
                LOG_CLI((BSL_META_U(unit, " %s(): %d : data_indx %d first_appear %d success %d\r\n"), __FUNCTION__, ii,
                         data_indx, (int) first_appear, (int) success));
            }
            clear_on_print = 0;
    ...
    }
    {
        ...
        LOG_CLI((BSL_META_U(unit,
                 ...
                 (int) multi_set, data_indx, (int) first_appear, (int) success));
        if (!success)
        {
            SHR_SET_CURRENT_ERR(_SHR_E_INTERNAL);
            LOG_CLI((BSL_META_U(unit,
        ...
        SHR_IF_ERR_EXIT(utilex_multi_set_member_add_at_index_nof_additions
                        (unit, multi_set, key, data_indx, nof_additions, &first_appear, &success));
        LOG_CLI((BSL_META_U(unit, " %s(): multi_set %d data_indx %d first_appear %d success %d\r\n"), __FUNCTION__,
                 (int) multi_set, data_indx, (int) first_appear, (int) success));
        if (!success)
        {
            SHR_SET_CURRENT_ERR(_SHR_E_INTERNAL);
        ...
        nof_additions = UTILEX_U32_MAX;
        res =
            utilex_multi_set_member_add_at_index_nof_additions(unit, multi_set, key, data_indx, nof_additions,
                                                               &first_appear, &success);
        if (res == _SHR_E_NONE)
        {
            SHR_SET_CURRENT_ERR(_SHR_E_INTERNAL);
            LOG_CLI((BSL_META_U(unit,
    ...
    }
    {
         ...
         * Test no. 5:
         *
         * Create maximal number of multi sets and, then, destroy them all.
         */
        uint32 in_use, max_nof_multis;
        uint32 multi_set_index;
 ...
 */
shr_error_e
utilex_multi_set_dummy_empty_function_to_make_compiler_happy(
    int unit)
{
    SHR_FUNC_INIT_VARS(unit);
    SHR_FUNC_EXIT;
