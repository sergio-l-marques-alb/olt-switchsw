...
static sh_sand_invoke_t packet_tx_invokes[] = {
    {"basic", "PTCH_2 ETH VLAN IPv4", 0},
    {NULL}
};
 ...
 */
static sh_sand_option_t packet_tx_options[] = {
    /*
     * Name 
     *//*
     * Type 
    ...
    else
    {
        ...
        SH_SAND_GET_ITERATOR(sand_arg)
        {
            if (sal_strstr(SH_SAND_GET_NAME(sand_arg), ".") == NULL)
            {
                 ...
                 * Header argument
                 */
                SHR_IF_ERR_EXIT(diag_sand_packet_proto_add
                                (unit, packet_handle, SH_SAND_GET_NAME(sand_arg), &header_handle));
            }
            else
            {
                /*
                ...
                {
                    SHR_ERR_EXIT(_SHR_E_PARAM, "Field (%s) supplied before any header", SH_SAND_GET_NAME(sand_arg));
                }
                SHR_IF_ERR_EXIT(diag_sand_packet_field_add_str(unit, header_handle, SH_SAND_GET_NAME(sand_arg),
                                                               SH_SAND_ARG_STR(sand_arg)));
            }
        }

        SHR_IF_ERR_EXIT(diag_sand_packet_send(unit, packet_handle));
    }
exit:
    ...
    void **ext_ptr_p)
{
    ...
    if (NULL == utilex_rhlist_entry_get_by_name(pdb, keyword))
    {
...
static sh_sand_man_t packet_decode_man = {
     ...
     * brief
     */
    "Decode a packet given the 1st header name and raw data.",
    /*
     * full
     */
    "Use this command to parse a packet given in raw data (either hex string or binary string) by supplying it the "
        "first header hint.\n"
        "The raw data can be given in binary format with prefix '0b', for instance 0b01111000, or in hex format with prefix"
        " '0x', for instance 0x78. When no prefix is given, the format is assumed to be hex.",
    /*
     * synopsis
     */
    "packet decode [first header]=[data]",
    /*
     * exmaples
     */
    "packet decode ftmh=017c0064b322000640\n" "packet decode pph=0b0010011000000110101000"
};

static sh_sand_invoke_t packet_decode_invokes[] = {
    {"basic", "ETH=0x00010203040500060708090a81004001884700000000000001004500000000000000000000000a0b0c010a0b0c02", 0},
    {NULL}
};
    ...
    int shift)
{
    ...
    {
        char *name = ((ISEMPTY(field->expansion)) ? RHNAME(field) : field->expansion);
        PRT_ROW_ADD(PRT_ROW_SEP_NONE);
        PRT_CELL_SET_SHIFT(shift, "%s", name);
    ...
    sh_sand_control_t * sand_control)
{
    ...
    RHITERATOR(obj, pit)
    {
        char *name = ((ISEMPTY(obj->expansion)) ? RHNAME(obj) : obj->expansion);
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SH_SAND_GET_ITERATOR(sand_arg)
    {
        ...
        SHR_IF_ERR_EXIT(pparse_packet_process(unit, pdb, &packet_info, &pit));
    }
    /*
     * Print parsed info
     */
    ...
    void **ext_ptr_p)
{
    ...
    if (NULL == utilex_rhlist_entry_get_by_name(pdb, obj_name))
    {
        SHR_ERR_EXIT(_SHR_E_NOT_FOUND, "Object: %s does not exist\n", obj_name);
    }
...
static sh_sand_man_t packet_compose_man = {
     ...
     * brief
     */
    "Compose a packet according to given headers list.",
    /*
     * full
     */
    "Use this command to generate a raw packet string to use with tx command.\n"
        ...
        "The fields should be supplied with a full \"path\" (see example). Currently, only unsigned 32 bit values are"
        "supported.",
    /*
     * synopsis
     */
    "packet compose <list of headers>",
    /*
     * exmaples
     */
    "packet compose eth eth.type=0x9100 vlan vlan.vid=64 mpls mpls.label=3 mpls mpls.label=4 ipv4 ipv4.ttl=100 udp bfd"
};

static sh_sand_invoke_t packet_compose_invokes[] = {
    {"basic", "PTCH_2 ETH VLAN MPLS MPLS IPv4", 0},
    {NULL}
};
 ...
 */
static sh_sand_option_t packet_compose_options[] = {
    /*
     * Name 
     *//*
     * Type 
    ...
    sh_sand_control_t * sand_control)
{
    ...
    sand_signal_value_to_str(unit, NULL, SAL_FIELD_TYPE_NONE, NULL, 0, data, print_data, size, PRINT_BIG_ENDIAN);
    LOG_CLI(("\n String for tx: 0x%s\n", print_data));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SH_SAND_GET_ITERATOR(sand_arg)
    {
        ...
        if (sal_strstr(SH_SAND_GET_NAME(sand_arg), ".") == NULL)
        {
             ...
             * Header argument
             */
            SHR_IF_ERR_EXIT(utilex_rhlist_entry_add_tail(pkt_list, SH_SAND_GET_NAME(sand_arg), RHID_TO_BE_GENERATED,
                                                         &rhhandle));
            header = rhhandle;
        }
        else
        {
            ...
            SHR_IF_ERR_EXIT(utilex_rhlist_entry_add_tail(header->field_list, SH_SAND_GET_NAME(sand_arg),
                                                         RHID_TO_BE_GENERATED, &rhhandle));
            sig = rhhandle;
            sal_strncpy(sig->print_value, sand_arg->param1.val_str, DSIG_MAX_SIZE_STR - 1);
        }
    }
    ...
    SHR_IF_ERR_EXIT(packet_compose_print(unit, pkt_list, data, size, sand_control));

exit:
    diag_sand_packet_free(unit, pkt_list);
    SHR_FUNC_EXIT;
}
...
static sh_sand_man_t packet_combine_man = {
     ...
     * brief
     */
    "Prints bit offsets from MSB of a specified packet structure",
    /*
     * full
     */
     ...
     * synopsis
     */
    "packet combine name=<packet-name> <list of headers>",
    /*
     * exmaples
     */
    "packet combine name=BFDoTunnel ETH VLAN VLAN MPLS MPLS ETH IPV4 UDP BFD"
};

static sh_sand_invoke_t packet_combine_invokes[] = {
    {"ETHBFDoTunnel", "name=ETHBFDoTunnel ETH VLAN VLAN MPLS MPLS ETH IPV4 UDP BFD", 0},
    {NULL}
};
 ...
 * \brief - Options list for 'packet compose' shell command
 */
static sh_sand_option_t packet_combine_options[] = {
    /*
     * Name 
     *//*
     * Type 
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SH_SAND_GET_ITERATOR(sand_arg)
    {
        if (sand_arg->type == SAL_FIELD_TYPE_BOOL)
        {
            /*
             * Header argument.
             * Add the header to the list and add a header specific prefix
             */
            signal_output_t *header;
            SHR_IF_ERR_EXIT(diag_sand_packet_proto_add
                            (unit, packet_handle, SH_SAND_GET_NAME(sand_arg) /* Signal name */ ,
                             &header_handle));
            header = (signal_output_t *) header_handle;
            /*
             * Add header specific prefix
             */
            sal_snprintf(header->expansion, RHNAME_MAX_SIZE, "Header_%d", header_i++);
        }
    }
    /*
...
static sh_sand_man_t packet_list_man = {
     ...
     * brief
     */
    "List the available objects to parse.",
    /*
     * full
     */
    "List the available objects that can be used in the 'type' option of 'packet decode' command",
    /*
     * synopsis
     */
    "packet list",
...
sh_sand_man_t sh_sand_packet_man = {
     ...
     * full
     */
    "This command parses/composes/sends raw packet data, using an external database for objects in the packet.",
    /*
     * synopsis
     */
    "",
...
sh_sand_cmd_t sh_sand_packet_cmds[] = {
    /*
     * Name | Leaf Action | Node | Options for Leaf | Usage | option resolution 
     */
    {"decode", packet_decode_cmd, NULL, NULL, &packet_decode_man, packet_decode_option_get,
     packet_decode_invokes}
    ,
    {"compose", packet_compose_cmd, NULL, packet_compose_options, &packet_compose_man, packet_compose_option_get,
     packet_compose_invokes}
    ,
    {"combine", packet_combine_cmd, NULL, packet_combine_options, &packet_combine_man, packet_combine_option_get,
     packet_combine_invokes}
    ,
    {"list", packet_list_cmd, NULL, NULL, &packet_list_man}
    ,
    {"tx", packet_tx_cmd, NULL, packet_tx_options, &packet_tx_man, packet_compose_option_get, packet_tx_invokes}
    ,
    {NULL}
};
...
{
    cmd_result_t result;
    SH_SAND_VERIFY(sh_sand_packet_cmds, result);

    sh_sand_act(unit, args, sh_sand_packet_cmds, sh_sand_sys_cmds);
    ARG_DISCARD(args);
    /*
