...
#define PTP_SYNCPHY_USAGE \
    ...
    " PTP SyncPhy PBM=<port bitmap> [FrameSyncPin=<pin#>] [SyncPin=<pin#>] [fw_type=<firmware type>]\n\t"  \
    "       defaults are FrameSyncPin=3, SyncPin=-1 (-1 corresponds to BroadSync Heartbeat),fw_type=0(0->tdpll/bs) \n"
...
cmd_ptp_syncphy(int unit, args_t *a)
{
    ...
    int fw_type = 0; /* For testing 0 -> tdpll/bs, 1-> ptpfull/pp */ 
    int framesync_pin = 3;
    char *pbm_str = 0;
    pbmp_t pbm;
    soc_port_t p, dport;
    ...
    uint64 localStackTime;
    int32_t nsec;
    bcm_ptp_timestamp_t localTime;
    ...
    parse_table_add(&pt, "fw_type",PQ_INT,(void*)0,&fw_type,NULL); /* added for testing ptp without GPIO3 future time programme */
    if (parse_arg_eq(a, &pt) < 0) {
        cli_out("%s: Invalid option %s\n", ARG_CMD(a), ARG_CUR(a));
    }
    ...
    if(freq_pin == -2) { /*BSHB1 timestamp*/
        localTime.seconds = (bshblocalTime.bshb1_time/1000000000);
        localTime.nanoseconds = bshblocalTime.bshb1_time - localTime.seconds*1000000000;
    } else { /*default BSHB0 timestamp */
        localTime.seconds = (bshblocalTime.bshb0_time/1000000000);
        localTime.nanoseconds = bshblocalTime.bshb0_time - localTime.seconds*1000000000;        
    } 

    /* get system stack time */
    if(fw_type) { /* only in case of PTP fw use sys time referenced by BS0 or BS1 4KHz */
       if(freq_pin == -2) { /*BSHB1 sys time */
           localTime.seconds     =  bshblocalTime.systime_bs1_sec;
           localTime.nanoseconds =  bshblocalTime.systime_bs1_nsec;
       } else { /*BSHB0 sys time */
           localTime.seconds     =  bshblocalTime.systime_bs0_sec;
           localTime.nanoseconds =  bshblocalTime.systime_bs0_nsec;
       }
       cli_out("$$$$ System time read from fw %llu.%d \n",(long long)localTime.seconds,localTime.nanoseconds);
    } /* fw_type */
#else
    COMPILER_64_SET(one_billion,0,1000000000);
    if(freq_pin == -2) { /*BSHB1 timestamp*/
        uint64 localTime_nsec_t;
        localTime_sec = bshblocalTime.bshb1_time;
        COMPILER_64_UDIV_64(localTime_sec,one_billion);
        localTime_nsec_t = localTime_sec;
        COMPILER_64_SUB_64(localTime_nsec_t,bshblocalTime.bshb1_time);
        COMPILER_64_TO_32_LO(localTime_nsec,localTime_nsec_t);
        localTime.seconds = localTime_sec;
        localTime.nanoseconds = localTime_nsec;
    } else { /*default BSHB0 timestamp */
        uint64 localTime_nsec_t;
        localTime_sec = bshblocalTime.bshb0_time;
        COMPILER_64_UDIV_64(localTime_sec,one_billion);
        localTime_nsec_t = localTime_sec;
        COMPILER_64_SUB_64(localTime_nsec_t,bshblocalTime.bshb0_time);
        COMPILER_64_TO_32_LO(localTime_nsec,localTime_nsec_t);
        localTime.seconds = localTime_sec;
        localTime.nanoseconds = localTime_nsec;
    }
    ...
    nsec = localTime.nanoseconds;
    localStackTime = localTime.seconds;
    /*COMPILER_64_ADD_32(localStackTime, 1);*/      /*keep 1sec+0.5sec delta offset*/ 
    /*COMPILER_64_UMUL_32(localStackTime, 1000000000);*/
    nsec += 500000000;  
    if (nsec > 1000000000) {
        nsec -= 1000000000;
        localTime.nanoseconds = nsec;
        COMPILER_64_ADD_32(localStackTime, 2);
    } else if(nsec > 0) {
        localTime.nanoseconds = nsec;
        COMPILER_64_ADD_32(localStackTime, 1);
    } else if(nsec < 0) {
        localTime.nanoseconds = nsec+1000000000;
        COMPILER_64_ADD_32(localStackTime, 1);
    } 

    COMPILER_64_UMUL_32(localStackTime, 1000000000);
    /*COMPILER_64_ADD_32(localStackTime, (localTime.nanoseconds + 500000000)); //10msec */
    COMPILER_64_ADD_32(localStackTime, (localTime.nanoseconds )); 
    ...
    DPORT_SOC_PBMP_ITER(unit, pbm, dport, p) {
        ...
        if (configure_phy_sync(unit, p, localStackTime) != BCM_E_NONE) {
            PTP_CLI_RESULT_RETURN(CMD_FAIL);
        }
    }

    sync_input.reference_time = localStackTime;
   ...
   /*Debug logs added here to avoid any latency */
    if(-2 == freq_pin) { /*BSHB1 timestamp*/
        if(0 == fw_type) {
            ...
            cli_out("****BS1 selected for SYNC_IN0 ,PTP systime with BS1 ref :%lld\n",(long long)(localTime.seconds*1000000000+(uint64)localTime.nanoseconds));
        }
        cli_out("REF_PHASE Configured with value : %lld\n",(long long)localStackTime);
    } else { /*default BSHB0 timestamp */
        if(0 == fw_type) {
            cli_out("****BS0 selected for SYNC_IN0 ,BS0 HB Time:%lld\n",(long long)bshblocalTime.bshb0_time);
        } else {
            cli_out("****BS0 selected for SYNC_IN0 ,PTP systime with BS0 ref :%lld\n",(long long)(localTime.seconds*1000000000+(uint64)localTime.nanoseconds));
        }
        cli_out("REF_PHASE Configured with value : %lld\n",(long long)localStackTime);
    }
#endif
...
#define PTP_TODIN_USAGE_2\
    "       PTP ToDIn ISO8601 OffsetSec=1 OffsetNS=0 BaudRate=9600\n" \
...
tdpll_instance_show(int unit, int low_idx, int high_idx)
{
	...
	int num_io = 13, ptp_io_num = 8, alt_ptp_input_idx = 9;
	 ...
	 if (SOC_IS_QUX(unit)) {
		 cli_out("#  GP0  GP1  GP2  GP3  GP4  GP5  SE1  SE2  1588  BS0  BS1  PMLPLL  PMXPLL  BW   PBO\n");
...
cmd_tdpll_instance(int unit, args_t *a)
{
    ...
    } else if (SOC_IS_APACHE(unit) || SOC_IS_QAX(unit)) {
        /* Retain end_index */
    } else {
        end_index = TDPLL_DPLL_INSTANCE_NUM_MAX - 2;
    } 
