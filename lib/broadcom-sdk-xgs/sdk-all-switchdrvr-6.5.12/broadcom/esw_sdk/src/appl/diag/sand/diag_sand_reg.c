...
#if (defined(LINUX) || defined(UNIX)) && (defined(__DUNE_GTO_BCM_CPU__) || defined(__DUNE_WRX_BCM_CPU__) || defined(__DUNE_SLK_BCM_CPU__))
    ...
    args_t * a)
{
    ...
    else if (!sal_strncasecmp(function, "read", strlen(function)))
    {
        ...
        cpu_i2c_read(int_chip_addr, int_data_addr, (CPU_I2C_BUS_LEN) int_alen, &int_value);
    ...
    args_t * a)
{
    ...
    else if (!sal_strncasecmp(function, "read", strlen(function)) || !sal_strncasecmp(function, "r", strlen(function)))
    {
        ...
        int_value = iproc ?
            CMVEC(unit).iproc_read(&CMDEV(unit).dev, int_addr) : CMVEC(unit).read(&CMDEV(unit).dev, int_addr);
...
cmd_result_t
    cmd_sand_pcic_access(int unit, args_t* a)
{
