                            ...
                            bcm_port_control_t type, int value)
{
    ...
    switch (type)
    {
        ...
        case bcmPortControlAllowQosInheritance:
             ...
             */
            if (SOC_IS_JERICHO(unit) && 
                soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "conn_to_np_enable", 0)){
                ...
                if (gport_parse_info.type != _bcmDppGportParseTypeSimple){
                     ...
                     if (lif_entry_info.type == SOC_PPC_LIF_ENTRY_TYPE_PWE) {
                         ...
                         /* LIF is not PWE nor AC */
                         BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (
                             _BSL_BCM_MSG("Unexpected lif id. Must be of type PWE or AC")));
                     }
 ...
 *      unit    - (IN) Unit number
 *      lif_id  - (IN) Lif ID
 *      isid   - (IN) ISID to connect to the LIF
 * Returns:
 *      BCM_E_xxx
 * Notes:
 */
    ...
    uint32 isid) 
{
    ...
    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    if (found) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_EXISTS, (_BSL_BCM_MSG("Trying to add an ISID that already exists - 0x%x"), isid));
    }
    ...
    bcm_port_match_info_t *match)
{
    ...
    if (usage == _bcmDppLifTypeVlan) {
        ...
        if (gport_hw_resources.local_in_lif != _BCM_GPORT_ENCAP_ID_LIF_INVALID) {
            rv = _bcm_mim_port_match_add(unit, gport_hw_resources.local_in_lif, match->isid);
            BCMDNX_IF_ERR_EXIT(rv);
        } else {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("API bcm_petra_port_match_add received invalid port")));
        }
