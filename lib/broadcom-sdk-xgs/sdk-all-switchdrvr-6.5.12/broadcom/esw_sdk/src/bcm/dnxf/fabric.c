    ...
    bcm_module_t modid)
{
    ...
    /*
     * For Multistage FE2, modid to group mapping in HW will be done in
        ...
        else
        {
          ...
          else
          {
               if(BCM_FABRIC_MODID_IS_LOCAL(destid_array[i]))
               {
                   ...
                   {
                       SHR_ERR_EXIT(_SHR_E_PARAM, "Destination id  %d is larger than multicast table entry size %d", BCM_FABRIC_LOCAL_MODID_UNSET(destid_array[i]), mc_table_entry_size);
                   }
                   port_array[i] = BCM_FABRIC_LOCAL_MODID_UNSET(destid_array[i]);
                   rc = bcm_dnxf_modid_local_map_is_valid(unit, port_array[i], &is_valid);
                   SHR_IF_ERR_EXIT(rc);
                   if(!is_valid) {
                        SHR_ERR_EXIT(_SHR_E_PARAM, "Local id %d isn't mapped, failed on index %d",port_array[i], i);
                   }
               }
               else
               {
    ...
    int *local_route_array)
{
    ...
    if (local_route_array != NULL && !SOC_DNXF_IS_FE13(unit)) {
    ...
    switch(type)
    {
       ...
       case bcmFabricLinkRepeaterEnable:
        ...
        case bcmFabricLinkRetimerConnect:
            ...
            {
                SHR_ERR_EXIT(_SHR_E_UNAVAIL, "Retimer is not supported for device %s Type",soc_dev_name(unit)); 
            } else {
    ...
    int *link_partner_count)
{
    ...
    *link_partner_count = 0;
    ...
    max_link_id = dnxf_data_port.general.nof_links_get(unit);
    ...
    */
    switch (dnxf_data_fabric.congestion.thresholds_info_get(unit, threshold_type)->index_type)
    {
        case _shr_dnxf_cgm_index_type_pipe:
            if (threshold_id != -1)
            {
                if (threshold_id < 0 || threshold_id >= dnxf_data_fabric.pipes.nof_pipes_get(unit))
                {
        ...
        case _shr_dnxf_cgm_index_type_priority:
            if (threshold_id != -1)
            {
                if (threshold_id < 0 || threshold_id >= dnxf_data_fabric.congestion.nof_threshold_priorities_get(unit))
                {
