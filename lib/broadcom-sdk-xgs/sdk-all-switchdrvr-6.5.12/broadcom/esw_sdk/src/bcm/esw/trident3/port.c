...
bcmi_td3_port_dev_info_init(int unit)
{
     ...
     *                and 25G port speeds.
     */
    dev_info->speed_class.speed_class_num = _TD3_TDM_SPEED_CLASS_MAX;
...
bcmi_td3_port_fn_drv_init(int unit)
{
   int phase_pos, present;
    ...
    /* BCMI Driver init */
    sal_memset(&bcmi_td3_port_drv, 0, sizeof(bcmi_td3_port_drv));
...
bcmi_td3_port_attach_stack(int unit, int port)
{
    ...
    BCM_IF_ERROR_RETURN
            (_bcm_td2p_port_stack_port_attach(unit, port));
    return BCM_E_NONE;
}
...
bcmi_td3_port_detach_stack(int unit, int port)
{
    ...
    BCM_IF_ERROR_RETURN
            (_bcm_td2p_port_stack_port_detach(unit, port));
    return BCM_E_NONE;
}
                                               ...
                                               soc_profile_mem_t *prof)
{
    ...
    entries[1] = &phb;
    BCM_IF_ERROR_RETURN
        (soc_profile_mem_add(unit, prof, entries,
                             _BCM_TD3_PHB_MAP_TAB1_ENTRIES_PER_PROFILE,
                             (uint32 *)&base));
                             ...
                             int *int_pri, bcm_color_t *color)
{
    ...
    index = ((untagged != 0) << 4) | ((pkt_pri << 1) & 0x7) | (cfi & 0x1);
