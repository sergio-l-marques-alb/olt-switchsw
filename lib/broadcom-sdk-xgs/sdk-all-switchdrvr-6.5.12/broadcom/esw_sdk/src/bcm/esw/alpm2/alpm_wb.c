...
extern int tcam_wb_reinit(int u, int pkm, int vrf_id, int idx, int key_len);
extern int tcam_wb_reinit_done(int u);
extern int alpm_vrf_deinit(int u, int vrf_id, int pkm);

STATIC int
alpm_pvt_bpm_len_cb(int u, _alpm_cb_t *acb,
                    _alpm_pvt_node_t *pvt_node, void *user_data)
{
    trie_t *pfx_trie = (trie_t *)user_data;
    _alpm_pfx_node_t *pfx_node = NULL;
    int rv = BCM_E_NONE;

    /* Find LPM for this pivot key */
    rv = trie_find_lpm(pfx_trie, pvt_node->key,
                       pvt_node->key_len, (trie_node_t **)&pfx_node);
    if (rv == BCM_E_NONE) {
        PVT_BKT_DEF(pvt_node) = pfx_node->bkt_ptr;
        PVT_BPM_LEN(pvt_node) = pfx_node->key_len; /* this is pvt bpm_len */
        ALPM_INFO(("trie_find_lpm() OK rv:%d key:0x%08x key_len:%d bpm_len:%d\n",
                rv, pvt_node->key[1], pvt_node->key_len, PVT_BPM_LEN(pvt_node)));
    } else {
        ALPM_ERR(("trie_find_lpm() Failed rv:%d key:0x%08x key_len:%d v6_key:%d\n",
 ...
 * Purpose:
 *      Update pivot bpm length after prefix trie is obtained from warmboot.
 * Parameters:
 *      u        - Device unit
 * Returns:
 *      void
    ...
    for (i = 0; i < ACB_CNT(u); i++) {
        ...
        for (pkm = 0; pkm < ALPM_PKM_CNT; pkm++) {
            for (vrf_id = 0; vrf_id < ALPM_VRF_ID_MAX(u); vrf_id++) {
                ...
                ALPM_INFO(("LC(%d) VRF:%d PKM:%s\n",
                           i, vrf_id, alpm_util_pkm_str[pkm]));
 ...
 */
STATIC int
alpm_wb_vrf_init(int u, _alpm_cb_t *acb, int vrf_id, int pkm)
{
    int     rv = BCM_E_NONE;
    uint32  max_key_len;
    trie_t  *root = NULL;
    ...
    ALPM_IER(
        trie_init(max_key_len, &ACB_PVT_TRIE(acb, vrf_id, pkm)));
                       ...
                       _alpm_pvt_node_t **pvt_node)
{
    ...
    max_key_len = alpm_util_trie_max_key_len(u, pkm);
    ALPM_IEG(trie_init(max_key_len, &PVT_BKT_TRIE(npvt_node)));
    ...
    rv = trie_insert(pvt_trie, npvt_node->key, NULL, key_len,
                     (trie_node_t *)npvt_node);
bad:
    if (BCM_FAILURE(rv)) {
        if (npvt_node != NULL) {
 ...
 */
STATIC int
alpm_wb_bkt_insert(int u, int vrf_id, int pkm,
                       ...
                       _alpm_bkt_node_t **bkt_node)
{
    ...
    rv = trie_insert(bkt_trie, nbkt_node->key, NULL, key_len,
                     (trie_node_t *)nbkt_node);
bad:
    if (BCM_FAILURE(rv)) {
        if (nbkt_node != NULL) {
 ...
 */
int
alpm_wb_pfx_trie_add(int u, int vrf_id, int pkm,
                     uint32 *key, int key_len, int ent_idx,
                     _alpm_bkt_adata_t *adata,
                     _alpm_bkt_node_t *bkt_node)
{
    ...
    rv = trie_insert(pfx_trie, pfx_node->key, NULL, key_len,
                   ...
                   uint32 *route_cnt, _bcm_l3_trvrs_data_t *trv_data)
{
    ...
    /**** 0) ALPM VRF init ****/
    if (!ACB_VRF_INITED(u, acb, vrf_id, pkm)) {
        ALPM_IER(alpm_wb_vrf_init(u, acb, vrf_id, pkm));
    }

    /**** 1) Pivot trie restore ****/
    ALPM_IER(alpm_wb_pvt_insert(u, acb, vrf_id, pkm,
                           key, key_len, idx, sub_idx, &pvt_node));
    ...
    for (bnk = 0; bnk < ACB_BNK_PER_BKT(acb, vrf_id); bnk++) {
        ...
        /* TD3ALPMTBD: to restore ACB_VRF_DB_TYPE(u, acb, vrf_id, pkm) =
            _th_alpm_adata_full_fmt[ALPM_LVL_TYPE(acb)][fmt]? */
        ...
        for (eid = 0; eid < ACB_FMT_ENT_MAX(acb, fmt); eid++) {
            ...
            ALPM_IER(alpm_wb_bkt_insert(u, vrf_id, pkm, new_key, new_len,
            ...
            if (ACB_HAS_RTE(acb)) {
                ...
                ALPM_IER(alpm_wb_pfx_trie_add(u, vrf_id, pkm, new_key,
        ...
        for (idx = 0; idx < idx_end; idx++) {
            ...
            for (sub_idx = 0; sub_idx < ALPM_TCAM_SUBCNT; sub_idx += step_size) {
                ...
                if (tcam_idx_reinit == FALSE) {
                    tcam_wb_reinit(u, pkm, vrf_id, idx, key_pfx_len);
                ...
                if (!use_alpm) {
                    ...
                    if (!ACB_VRF_INITED(u, acb, vrf_id, pkm)) {
                        (void)alpm_pvt_trie_init(u, vrf_id, pkm);
                    }
                    alpm_trie_key_to_pfx(u, pkm, key, key_pfx_len, lpm_cfg.user_data);
...
alpm_pfx_update_match_cb(_alpm_pfx_node_t *pfx_node, void *user_data)
{
    ...
    alpm_trie_pfx_to_key(u, pkm, pfx_node->key, pfx_node->key_len, key);
    alpm_util_cfg_construct(u, pkm, vrf_id, key, pfx_node->key_len,
                            &adata, &lpm_cfg);
    ...
    /* Dist hitbit */
    if (!ALPM_HIT_SKIP(u) &&
        alpm_rte_hit_get(u, vrf_id, bkt_tbl, idx, ent)) {
        lpm_cfg.defip_flags |= BCM_L3_HIT;
    }

    lpm_cfg.defip_index = alpm_util_ent_phy_idx_get(u, acb, pkm, vrf_id, idx);
    nh_ecmp_idx = lpm_cfg.defip_ecmp_index;
                           ...
                           _bcm_l3_trvrs_data_t *trv_data)
{
    ...
    for (pkm = 0; pkm < ALPM_PKM_CNT; pkm++) {
        ...
        for (vrf_id = 0; vrf_id <= ALPM_VRF_ID_MAX(u); vrf_id++) {
            ...
            (void)bcm_esw_alpm_pfx_traverse(u, vrf_id, pkm,
...
bcm_esw_alpm_update_match(int u, _bcm_l3_trvrs_data_t *trv_data)
{
    ...
    for (idx = 0; idx < def_rte_cnt; idx++) {
        alpm_util_cfg_construct(u, def_rte[idx].pkm, def_rte[idx].vrf_id, key,
