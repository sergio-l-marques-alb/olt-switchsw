...
int run_vsq_gl_example_jer2 (int unit, uint32 set_threshold, uint32 clr_threshold)
{
    ...
    threshold.thresh_set = set_threshold;
    threshold.thresh_clear = clr_threshold;
    ...
    /** Global VSQ per DP threshold for OCB Buffers*/
    threshold_info.gport = vsq;
    threshold_info.threshold_action = bcmCosqThreshActionDrop;
    threshold_info.resource_type = bcmResourceOcbBuffers;
    threshold_info.is_free_resource = 1;

    for (dp = 0; dp < 4; dp++) {
        threshold_info.index = BCM_COSQ_TH_INDEX_DP_SET(dp);
        rc = bcm_cosq_gport_static_threshold_set(unit, 0, &threshold_info, &threshold);
        if (rc != BCM_E_NONE) {
            printf("Error, in bcm_cosq_gport_static_threshold_set\n");
            return rc;
        }
