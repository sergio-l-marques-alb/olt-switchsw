...
int get_device_type(int unit, device_type_t *device_type)
{
    ...
    if ( info.device == 0x8270 ) {
        ...
        *device_type = DEVICE_TYPE_JERICHO2;
    } else {
...
int run_wred_example(int unit,int destination_local_port, int base_queue)
{
    ...
    int max_thresh = (device_type == DEVICE_TYPE_JERICHO2 ? 256 :1);
    int min_thresh = 0;
    int max_prob = 100;
    int gain = 1;
    int cosq = 0;
    ...
    if (device_type >= DEVICE_TYPE_QAX) { /* qax and above */
        ...
        rsrc_flags[1] = BCM_COSQ_GPORT_SIZE_SRAM | (device_type == DEVICE_TYPE_JERICHO2 ? BCM_COSQ_GPORT_SIZE_BUFFERS : BCM_COSQ_GPORT_SIZE_BUFFER_DESC);
