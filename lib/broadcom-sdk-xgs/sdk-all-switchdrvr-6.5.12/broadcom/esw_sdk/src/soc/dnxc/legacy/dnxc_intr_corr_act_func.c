...
static dnxc_xor_mem_info jer2_xor_mem_info[] = {
...
static dnxc_xor_mem_info ramon_xor_mem_info[] = {
    {MCT_MCTm, 2, 13, 1},
    {INVALIDm}
};
...
dnxc_mem_is_em(int unit, soc_mem_t mem)
{
    ...
    for (index = 0; jer2_em_mem[index] != NUM_SOC_MEM; index++) {
        if (jer2_em_mem[index] == mem) {
            return 1;
        }
    }
...
dnxc_xor_mem_info *dnxc_xor_mem_info_get(int unit, soc_mem_t mem)
{
    ...
    xor_mem_info = SOC_IS_RAMON(unit) ? ramon_xor_mem_info : jer2_xor_mem_info;
...
*Low (parity/ecc_2b) - - if memory static and accessible and SW shadow available fix with shadow. Otherwise ignore.     
    ...
    */
    ...
    if (dynamic_mem) {
        ...
        case DNXC_P_3_ECC_ECC1B_DC:
            /*
              There is no EM table for DNXF
             */
            if (mem_is_accessible &&
                (SOC_IS_DNXF(unit) ||SOC_IS_DNX(unit) || dnxc_mem_is_em(unit, mem))) {
            ...
            SHR_EXIT();
        default:
            if (SOC_IS_DNX(unit) && dnxc_mem_is_em(unit, mem)) {
    ...
    char* msg)
{
    ...
    for (current_index = shadow_correct_info_p->min_index ; current_index <= shadow_correct_info_p->max_index ; current_index++) {
        rc = dnxc_get_ser_entry_from_cache(unit,   mem, copyno, shadow_correct_info_p->array_index, current_index, data_entry);
        SHR_IF_ERR_EXIT(rc);
    ...
    uint32 *data_entry)
{
    ...
    if (cache_enable) {
        ...
        rc = soc_mem_array_read(unit, ecc_1b_correct_info_p->mem, ecc_1b_correct_info_p->array_index, 
    ...
    if (!cache_enable && SOC_MEM_FIELD_VALID(unit, ecc_1b_correct_info_p->mem, ECCf) ) {
        ...
        *ecc_field = soc_mem_field32_get(unit, ecc_1b_correct_info_p->mem, data_entry, ECCf);
        ...
        SHR_IF_ERR_EXIT(ecc_correction(unit,ecc_1b_correct_info_p->mem, mem_row_bit_width,xor_flag, data_entry, ecc_field));
    ...
    char* msg)
{
    ...
    if (SOC_MEM_FIELD_VALID(unit, xor_correct_info->mem, ECCf)) {
        ...
        SHR_IF_ERR_EXIT(dnxc_disable_block_ecc_check(unit,interrupt_id, xor_correct_info->mem,block_instance,value,orig_value));
    }
    ...
    if (SOC_MEM_FIELD_VALID(unit, xor_correct_info->mem, ECCf)) {
        SHR_IF_ERR_EXIT(dnxc_disable_block_ecc_check(unit,interrupt_id,xor_correct_info->mem,block_instance,orig_value,NULL));
    }
    ...
    char* msg)
{
    ...
    SOC_REG_ABOVE_64_CLEAR(value);
    SHR_IF_ERR_EXIT(dnxc_disable_block_ecc_check(unit,interrupt_id, ecc_1b_correct_info_p->mem,copyno,value,orig_value));
    ...
    SHR_IF_ERR_EXIT(dnxc_disable_block_ecc_check(unit,interrupt_id,ecc_1b_correct_info_p->mem,copyno,orig_value,NULL));
    ...
    char *msg)
{
    ...
    SHR_FUNC_INIT_VARS(unit);
    LOG_ERROR(BSL_LS_BCM_INTR, (BSL_META_U(unit,
    ...
    char* msg)
{
    ...
    else if(dnxf_data_fabric.topology.load_balancing_mode_get(unit) == soc_dnxf_load_balancing_mode_balanced_input)
    {
        /* Table is used */
        /* reset the scrubber */
        SHR_IF_ERR_EXIT(READ_RTP_REACHABILITY_MESSAGE_GENERATOR_CONFIGURATIONr_REG64(unit, &reg_val64));
        ...
        SHR_IF_ERR_EXIT(WRITE_RTP_REACHABILITY_MESSAGE_GENERATOR_CONFIGURATIONr_REG64(unit, reg_val64));
    ...
    else if(dnxf_data_fabric.topology.load_balancing_mode_get(unit) == soc_dnxf_load_balancing_mode_destination_unreachable)
    {
        /* Table is used */
        /* write max value to the entry */
        score_slsct = SOC_DNXF_DRV_MULTIPLIER_MAX_LINK_SCORE;
    ...
    char* msg)
{
    ...
    LOG_ERROR(BSL_LS_BCM_INTR, (BSL_META_U(unit, "%s"), print_msg));
exit:
    SHR_FUNC_EXIT;
}
...
#endif
        case DNXC_INT_CORR_ACT_XOR_FIX:
            rc = dnxc_interrupt_handles_corrective_action_for_xor(unit, block_instance, interrupt_id,(dnxc_interrupt_mem_err_info*)param1, msg);
            SHR_IF_ERR_EXIT(rc);
            break;
