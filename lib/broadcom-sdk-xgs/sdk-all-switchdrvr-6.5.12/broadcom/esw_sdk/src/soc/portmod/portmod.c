 ...
 */
int portmod_pm_capability_get(int unit, portmod_dispatch_type_t pm_type,
                              portmod_pm_capability_t* pm_cap)
{
    ...
    _SOC_IF_ERR_EXIT(portmod_pm_specific_capability_get(unit, pm_cap));
 ...
 * portmod_pm_vcos_get
 *
 * @brief Get the VCO values for the list of speed ids that
 *        need to be supported
 *
 * @param [in]  unit        - unit id
 * @param [in]  pm_type     - PM type
 * @param [out] vco_select  - struct portmod_pm_vco_select_t, list of
 *                            speed-ids to be populated to retrieve
 *                            available VCOs for the PM
 */
int portmod_pm_vcos_get(int unit, portmod_dispatch_type_t pm_type,
                        portmod_pm_vco_setting_t* vco_select)
{
    ...
    if(NULL !=
        __portmod__dispatch__[__type__]->f_portmod_pm_specific_vcos_get) {

        __rv__ =
            __portmod__dispatch__[__type__]->f_portmod_pm_specific_vcos_get(
                                        unit, &pm_cap, vco_select);
        _SOC_IF_ERR_EXIT(__rv__);
    } else {
        _SOC_EXIT_WITH_ERR(SOC_E_UNAVAIL, ("portmod_pm_specific_vcos_get "
                           "isn't implemented for driver type"));
    }
...
/*!
 * portmod_pm_core_init
 *
 * @brief PM Core init routine
 *
 * @param [in]  unit            - unit id
 * @param [in]  pport           - physical port
 * @param [in]  lport           - physical port
 * @param [in]  flags           - 
 */
int portmod_pm_core_init(int unit, int pport, int lport, uint32 flags, const portmod_port_add_info_t* add_info)
{
    portmod_dispatch_type_t __type__ = portmodDispatchTypeCount;
    pm_info_t pm_info = NULL;
    int __rv__;
    ...
    /* get PM type from the physical port */
    _SOC_IF_ERR_EXIT(portmod_phy_pm_type_get(unit, pport, &__type__)); 
    ...
    for(i = 0 ; i < MAX_PMS_PER_PHY; i++){
        ...
        if(__type__ == _pmm_info[unit]->pms[pm_id].type) {
            pm_info = &_pmm_info[unit]->pms[pm_id];
            break;
        }
    }
    ...
    if(NULL !=
        __portmod__dispatch__[__type__]->f_portmod_port_pm_core_init) {

        __rv__ =
            __portmod__dispatch__[__type__]->f_portmod_port_pm_core_init(
                                        unit, pm_info, pport, lport,
                                        flags, add_info);
        _SOC_IF_ERR_EXIT(__rv__);
    } else {
        _SOC_EXIT_WITH_ERR(SOC_E_UNAVAIL, ("portmod_pm_core_init "
                           "isn't implemented for driver type"));
    }
...
/*!
 * portmod_pm_is_core_initialized
 *
 * @brief Returns if the PortMacro associated with the port
 ...
 */
int portmod_pm_is_core_initialized(int unit, int pport, int lport,
                                   int* is_initialized)
{
    portmod_dispatch_type_t __type__ = portmodDispatchTypeCount;
    pm_info_t pm_info = NULL;
    ...
    if(NULL !=
        __portmod__dispatch__[__type__]->f_portmod_pm_is_initialized) {
        ...
        _SOC_EXIT_WITH_ERR(SOC_E_UNAVAIL, ("portmod_pm_is_core_initialized "
                           "isn't implemented for driver type"));
    }
...
int portmod_pm_supported_vcos_get(int unit, portmod_dispatch_type_t pm_type, 
                                  ...
                                  int size, portmod_dual_vcos_t* dual_vco)
{
    ...
    if(NULL != __portmod__dispatch__[__type__]->f_portmod_port_vcos_get) {
        __rv__ = __portmod__dispatch__[__type__]->f_portmod_port_vcos_get(unit, 0, speed_config_list, size, dual_vco);
        _SOC_IF_ERR_EXIT(__rv__);
    } else {
