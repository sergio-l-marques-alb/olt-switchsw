...
_soc_th_alpm_route_capacity_max(int unit, soc_mem_t mem)
{
    ...
    if (urpf) {
        if (parallel) {
            max_bkts = ALPM_CTRL(unit).num_bkts;
        } else {
            max_bkts = ALPM_CTRL(unit).num_bkts >> 1;
        }
    } else {
        if (!parallel && (v6 || ALPM_CTRL(unit).double_wide[L3_DEFIP_MODE_V4])) {
            max_bkts = ALPM_CTRL(unit).num_bkts >> 1;
        } else {
            max_bkts = ALPM_CTRL(unit).num_bkts;
        }
...
#define ULTIZATION_85(x) ((x) * 85 / 100)
    ...
    case L3_DEFIP_ALPM_IPV4_1m:
        if (urpf) {
            num_pivots = ALPM_CTRL(unit).num_bkts >> 1;
            if (combined) {
#ifdef BCM_TOMAHAWK2_SUPPORT
                if (soc_feature(unit, soc_feature_half_of_l3_defip_ipv4_capacity)) {
            ...
            if (combined && ALPM_CTRL(unit).double_wide[L3_DEFIP_MODE_V4]) {
#ifdef BCM_TOMAHAWK2_SUPPORT
                if (soc_feature(unit, soc_feature_half_of_l3_defip_ipv4_capacity)) {
                    bkt_min = ULTIZATION_66(bkt_cnt);
                } else
            ...
            } else if (tcam_alpm && !ALPM_CTRL(unit).double_wide[L3_DEFIP_MODE_V4]) {
                bkt_min = ULTIZATION_33(bkt_cnt);
            } else {
#ifdef BCM_TOMAHAWK2_SUPPORT
                if (soc_feature(unit, soc_feature_half_of_l3_defip_ipv4_capacity)) {
            ...
            if (parallel || ALPM_CTRL(unit).double_wide[L3_DEFIP_MODE_V4]) {
                num_pivots = ALPM_CTRL(unit).num_bkts >> 1;
            } else {
                num_pivots = ALPM_CTRL(unit).num_bkts;
            }
        }
#ifdef BCM_TOMAHAWK2_SUPPORT
        if (soc_feature(unit, soc_feature_half_of_l3_defip_ipv4_capacity)) {
    ...
    case L3_DEFIP_ALPM_IPV6_64_1m:
        if (urpf) {
            if (combined) {
                ...
                num_pivots = ALPM_CTRL(unit).num_bkts >> 2;
                bkt_min = ULTIZATION_33(bkt_cnt);
            }
        } else {
            if (combined) {
    ...
    case L3_DEFIP_ALPM_IPV6_128m:
        if (urpf) {
            if (combined) {
                ...
                num_pivots = ALPM_CTRL(unit).num_bkts >> 3;
                bkt_min = ULTIZATION_33(bkt_cnt);
            }
        } else {
            if (combined) {
