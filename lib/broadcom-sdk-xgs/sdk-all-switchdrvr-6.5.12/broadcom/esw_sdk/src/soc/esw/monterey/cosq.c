...
typedef struct soc_monterey_fc_map_shadow_memory_s {
    soc_mem_t mem;
    uint32  *mem_shadow;
}soc_monterey_fc_map_shadow_memory_t;

typedef struct soc_monterey_fc_map_shadow_s {
    soc_monterey_fc_map_shadow_memory_t* shadow_array;
    int mem_count;
}soc_monterey_fc_map_shadow_t;
...
static soc_monterey_fc_map_shadow_t soc_monterey_fc_map_shadow[SOC_MAX_NUM_DEVICES];
...
{
    soc_monterey_sched_type_t sched_type;
    uint32 entry[SOC_MAX_MEM_WORDS], mfval = 0, rval = 0, wrr_mask;
    soc_mem_t mem;
    soc_reg_t reg;
...
_soc_monterey_dump_hsp_sched_at(int unit, int port, int level, int offset, int index)
{
    ...
    if (IS_CPU_PORT(unit, port)) {
        l2_max_num = 8; 
    } else if (IS_LB_PORT(unit, port) || IS_MACSEC_PORT(unit, port)) {
        l2_max_num = 10;
    }
...
void
soc_monterey_fc_map_shadow_free(int unit)
{
    int mem_idx;
    soc_monterey_fc_map_shadow_memory_t * shadow_info;
    soc_monterey_fc_map_shadow_t* shadow_ctrl = &soc_monterey_fc_map_shadow[unit];
...
int
soc_monterey_fc_map_shadow_create(int unit)
{
	  int mem_idx, alloc_size, num_entries, entry_size;
	  soc_monterey_fc_map_shadow_memory_t* shadow_info = NULL;
	  uint32* shadow = NULL;
    
    if (soc_monterey_fc_map_shadow[unit].shadow_array == NULL) {
        alloc_size = sizeof(soc_monterey_fc_map_shadow_memory_t) * 
                     COUNTOF(fc_map_mems);
        ...
        soc_monterey_fc_map_shadow[unit].shadow_array = shadow_info;
        ...
        for (mem_idx = 0; mem_idx < COUNTOF(fc_map_mems); mem_idx++) {
            ...
            if (shadow == NULL) {
                soc_monterey_fc_map_shadow_free(unit);
                return SOC_E_MEMORY;
            }
            sal_memset(shadow, 0, alloc_size);
            soc_monterey_fc_map_shadow[unit].shadow_array[mem_idx].mem_shadow = shadow;
            soc_monterey_fc_map_shadow[unit].shadow_array[mem_idx].mem = fc_map_mems[mem_idx];
            soc_monterey_fc_map_shadow[unit].mem_count++;
        }
    }
    return SOC_E_NONE; 
}
...
int
soc_monterey_fc_map_shadow_entry_get (int unit, soc_mem_t mem, int index, 
                                      void* entry_data)
{
    ...
    shadow = soc_monterey_fc_map_shadow[unit].shadow_array[mem_idx].mem_shadow;
...
int
soc_monterey_fc_map_shadow_entry_set (int unit, soc_mem_t mem, int index, 
                                      void* entry_data)
{
    int mem_idx, entry_words;
    uint32 * shadow = NULL;
...
int 
soc_monterey_fc_map_shadow_clear (int unit, soc_mem_t mem)
{
    uint32 * shadow = NULL;
...
int
soc_monterey_fc_map_shadow_size_get (int unit, uint32 * size)
{
...
int
soc_monterey_fc_map_shadow_sync (int unit, uint32 **sync_addr)
{
    ...
    for (mem_idx = 0; mem_idx < COUNTOF(fc_map_mems); mem_idx++) {
        ...
        sal_memcpy(*sync_addr, soc_monterey_fc_map_shadow[unit].shadow_array[mem_idx].mem_shadow, 
...
int
soc_monterey_fc_map_shadow_load (int unit, uint32 **load_addr)
{
    ...
    for (mem_idx = 0; mem_idx < COUNTOF(fc_map_mems); mem_idx++) {
        ...
        tbl_size = entry_size * num_entries;
        sal_memcpy(soc_monterey_fc_map_shadow[unit].shadow_array[mem_idx].mem_shadow, *load_addr,
                   tbl_size * sizeof(uint32));
        *load_addr += tbl_size;
