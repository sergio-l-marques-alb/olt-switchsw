        ...
        soc_mem_view_split_field_info_t *sf_info)   /* split field info */
{
    ...
    sf_info->offset = view_sf_info.offset;
    sf_info->width  = view_sf_info.width;
    for (i = 0; i < 2; i++) {
        sf_info->field[i].field = view_sf_info.field[i].field_id; 
        sf_info->field[i].len   = view_sf_info.field[i].width;
        sf_info->field[i].bp    = view_sf_info.field[i].offset;
        sf_info->field[i].flags = view_sf_info.field[i].flags;
    }
    return SOC_E_NONE;
}
