  ...
  jer_itm_regs_init(int unit)
{
    ...
    BCM_DPP_CORES_ITER(BCM_CORE_ALL, core) {
     ...
     /* stamp snoop or mirror command for fabric multicast mirror and snoop function */
...
{ 
  ...
  if (SOC_IS_QAX(unit)) { /* ingress shaping is disabled in QAX */
    ...
    )
{
    ...
    SOCDNX_IF_ERR_EXIT(tmc_itm_mantissa_exp_field_set(unit,
                                                       ...
                                                       mantissa_bits,
                                                       soc_mem_field_length(unit, IQM_MC_FADTTm, STATIC_THf) - mantissa_bits, /* exp_bits */
                                                       1, /*factor */
                                                       1, /* round_up */
                                                       &data,
    ...
    SOCDNX_IF_ERR_EXIT(tmc_itm_mantissa_exp_field_set(unit,
                                                       ...
                                                       mantissa_bits,
                                                       soc_mem_field_length(unit, IQM_MC_FADTTm, DYNAMIC_MIN_THf) - mantissa_bits, /* exp_bits */
                                                       1, /*factor */
                                                       1, /* round_up */
                                                       &data,
                                                       info->min_threshold,
    ...
    SOCDNX_IF_ERR_EXIT(tmc_itm_mantissa_exp_field_set(unit,
                                                       ...
                                                       mantissa_bits,
                                                       soc_mem_field_length(unit, IQM_MC_FADTTm, DYNAMIC_MAX_THf) - mantissa_bits, /* exp_bits */
                                                       1, /*factor */
                                                       1, /* round_up */
                                                       &data,
                                                       info->max_threshold,
  ...
  )
{
    ...
    /* Disable VSQ A-F Flow Control (set to max) */
    info->bd_size_fc.set = JER_ITM_VSQ_FC_BD_SIZE_MAX;
    info->bd_size_fc.clear = JER_ITM_VSQ_FC_BD_SIZE_MAX;
    ...
    if (enable) {
        ...
        if (SOC_PORT_IF_ILKN == interface_type) {
            ...
            if (JER_ITM_NON_PHY_IF_FOR_VSQ_E_F(interface_type)) {
                /* non PHY port doesn't have CFC LLFC */
                fc_enable = FALSE;
            } else {
                           ...
                           )
{
    ...
    if (enable) {
        ...
        if (JER_ITM_NON_PHY_IF_FOR_VSQ_E_F(interface_type)) {
            /* non PHY port doesn't have CFC PFC */
            pg_enable = FALSE;            
        } else {
            res = soc_port_sw_db_first_phy_port_get(unit, logical_port, &phy_port);
            SOCDNX_IF_ERR_EXIT(res);
            if (SOC_PORT_IF_ILKN == interface_type) {
                ...
                if (SOC_JER_NIF_IS_ILKN_IF_OVER_FABRIC(unit, ilkn_id)) {
                    /* ILKN over fabric doesn't have CFC PFC */
                    pg_enable = FALSE;
                }             
            }
        }
    ...
    SOC_DPP_CORES_ITER(core, core_id){
        ...
        if (reservation_increase) {
            ...
            if (SOC_DPP_CONFIG(unit)->tm.guaranteed_q_mode == SOC_DPP_GUARANTEED_Q_RESOURCE_MEMORY) {
                /* Handle memory size reservation, look at arad_itm_glob_rcs_drop_set_unsafe() and at _bcm_petra_cosq_vsq_gl_drop_threshold_set() */
...
/*
  HW contains a single compensation value for per in pp port and per queue compensation.

  This function computes the offset to the compensation table and
  combined compensation value.
 */
  ...
  ) 
{
    ...
    SOC_DPP_CORES_ITER(core, core_id) {
        ...
        data = 0;
        if (adjust_type == SOC_TMC_ITM_PKT_SIZE_ADJUST_QUEUE || adjust_type == SOC_TMC_ITM_PKT_SIZE_ADJUST_PORT) { /* per queue or per port (credit class) compensation */
...
{
    uint32 data = 0;
    uint32 sync_count = 0x12c; /* reset value of the field */
    int sync_count_field_length;
    uint32 core_freq = SOC_DPP_CONFIG(unit)->arad->init.core_freq.frequency/1000; /* kHz -> MHz*/
