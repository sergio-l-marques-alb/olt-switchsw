            ...
            if (is_tcam_dt) {
              if (phase_ndx == 0) {
                ...
                else if(SOC_IS_JERICHO(unit))
                {
                    ...
                    info->large_direct_lookup_key = dma_offset;
  ...
  arad_pp_fp_direct_table_entry_get(int unit, SOC_PPC_FP_DATABASE_STAGE stage, uint32 db_id_ndx, uint32 entry_id_ndx, SOC_PPC_FP_ENTRY_INFO* info, uint8* is_found, uint32 value_in[ARAD_PP_FP_TCAM_ENTRY_SIZE], uint32 mask_in[ARAD_PP_FP_TCAM_ENTRY_SIZE], ARAD_TCAM_ACTION* action)
{
  ...
  if(is_tcam_dt) {
      ...
      entry_dt_first = entry_id_ndx;
      is_dt_bit_meaningful = ARAD_PP_FP_DIRECT_TABLE_KAPS_NOF_LINES - 1;
  ...
  )
{
    ...
    pmf_key = pmf_db_info.used_key[pmf_pgm_ndx][sw_db_ce.is_second_key];
    res = arad_pmf_ce_for_key_get_unsafe(unit, stage, pmf_pgm_ndx, pmf_key, ce_ndx, sw_db_ce.is_msb, is_second_lookup, info);
    SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);

    if (info->nof_bits > 0) {
        info->lsb = sw_db_ce.lsb;
        info->msb = sw_db_ce.msb;
        info->is_msb = sw_db_ce.is_msb;
        info->key_mask = pmf_key;
