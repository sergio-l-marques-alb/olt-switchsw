   ...
   )
{
    ...
    /* Verify total number of segments in Master key will not exceed max */
    {
        ...
        if((search_format->nof_key_segments + nof_key_segments) > ARAD_KBP_MAX_NOF_KEY_SEGMENTS) {
            LOG_ERROR(BSL_LS_SOC_TCAM,
                    (BSL_META_U(unit,
                            "Error in %s(): Maximum alowed number of key segments exceeded (%d)\n"),
                            FUNCTION_NAME(), ARAD_KBP_MAX_NOF_KEY_SEGMENTS));
    ...
    for (field_idx = 0; field_idx < search_format->nof_key_segments; field_idx++)
    {
        if(search_format->key_segment[field_idx].nof_bytes > ARAD_KBP_MAX_SEGMENT_LENGTH_BYTES) {
            LOG_ERROR(BSL_LS_SOC_TCAM,
                    (BSL_META_U(unit,
                            "Error in %s(): Maximum alowed search key segment size exceeded (%d)\n"),
        ...
        */
        if (SOC_IS_JERICHO(unit)) {
            ...
            if (acl_type == ARAD_KBP_ACL_IN_MASTER_KEY_LSB_ONLY) {
                ...
                /* Add all range segments to the master key */
       ...
       SOC_SAND_OUT SOC_SAND_SUCCESS_FAILURE    *success)
{
    ...
    for (pgm_ndx = pgm_ndx_min; pgm_ndx < pgm_ndx_max; ++pgm_ndx)
    {
        ...
        if(SOC_IS_JERICHO(unit)){
            ...
            /* Initialize search format */
			/* A. Get the master-key status (its current length and segment's division).
             *    Why is it important?
            ...
            */
            ...
            search_format.nof_key_segments = search_format.nof_key_segments + nof_shared_segments + nof_range_quals;
            ...
            /* The range qualifiers are located on the MSB of the key.
             * The qualifiers are 16 bits in size. No other size is supported.
             * Nevertheless use the provided info.
             */
            new_number_of_segments = (segment_idx + nof_range_quals + (acl_type == ARAD_KBP_ACL_IN_MASTER_KEY_LSB_MSB ? 2 : 1));

            /* validation */
            if(new_number_of_segments >= ARAD_KBP_MAX_NOF_KEY_SEGMENTS) {
                LOG_ERROR(BSL_LS_SOC_TCAM,
                        (BSL_META_U(unit,
                                "Error in %s(): Maximum alowed number of key segments exceeded (%d). New number of segments %d.\n"),
                                FUNCTION_NAME(), ARAD_KBP_MAX_NOF_KEY_SEGMENTS, new_number_of_segments));
            ...
            if(acl_type == ARAD_KBP_ACL_IN_MASTER_KEY_LSB_MSB) {
                acl_s0_size = lsb_free_in_bytes - range_quals_size_in_bytes;
                acl_s1_size = key_size_in_bytes - lsb_free_in_bytes;
            } else {
                acl_s0_size = key_size_in_bytes - range_quals_size_in_bytes;
                acl_s1_size = 0;
            }

            /* regular LSB ACL segment */
            ...
            for(i = 0; i < nof_range_quals; i++) {
                ...
                /* add the shared qualifiers size to the offset */
            ...
            res = KBP_INFO.db_info.nof_range_qualifiers.get(unit, table_id, &nof_range_quals);
            SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 50, exit);
        }
