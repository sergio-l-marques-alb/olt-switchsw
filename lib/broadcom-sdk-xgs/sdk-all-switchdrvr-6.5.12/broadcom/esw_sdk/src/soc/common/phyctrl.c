...
soc_phyctrl_init(int unit, soc_port_t port)
{
    ...
    if (SOC_IS_ROBO(unit)) {
        ...
        if (SOC_IS_ROBO_ARCH_VULCAN(unit) || SOC_IS_DINO(unit)) {
            ...
            if (SOC_IS_VULCAN(unit) || SOC_IS_STARFIGHTER3(unit)){
                if (port == 5) {
...
soc_phyctrl_loopback_set(int unit, soc_port_t port, int enable, int serdes_linkup_wait)
{
    ...
    if (serdes_linkup_wait && !SOC_IS_SAND(unit) && !SOC_IS_TRIUMPH3(unit) &&
       !SOC_IS_CALADAN3(unit)) { 
    ...
    switch(event) {
        ...
        case phyEventResume:
            if (SOC_IS_CALADAN3(unit)) {
                ...
                return  (PHY_NOTIFY(int_pc->pd, unit, port, event, data));
            }
            break;
        case phyEventStop:
            if (SOC_IS_CALADAN3(unit)) {
