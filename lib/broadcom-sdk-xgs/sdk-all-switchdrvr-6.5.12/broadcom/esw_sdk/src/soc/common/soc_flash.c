...
static const soc_flash_conf_t flash_table[] = {
    /* id   page_size   sector_size   nr_sectors   size */
    { 0x20,   0x100,     0x10000,      0x1000,      0   },
};
                   ...
                   soc_flash_conf_t *flash_conf)
{
    ...
    for (i = 0; i < sizeof(flash_table) / sizeof(soc_flash_conf_t); ++i) {
        if (flash_table[i].id == id[0]) {
            ...
            flash->conf.nr_sectors = flash_table[i].nr_sectors;
            flash->conf.size = flash->conf.sector_size
                        * flash_table[i].nr_sectors;
            rv = SOC_E_NONE;
            break;
        }
    }
