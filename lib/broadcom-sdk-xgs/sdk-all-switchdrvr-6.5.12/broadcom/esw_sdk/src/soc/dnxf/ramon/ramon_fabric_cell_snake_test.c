    ...
    uint32 flags)
{
    ...
    bcm_port_t port;
    int rv;
    int nof_links, nof_active_link;
    SHR_FUNC_INIT_VARS(unit);

    sal_usleep(100*10000); /* sleep 1 s */

    nof_links = dnxf_data_port.general.nof_links_get(unit);
    SOC_PBMP_COUNT(PBMP_SFI_ALL(unit), nof_active_link);

    if (nof_links != nof_active_link)
    {
        SHR_ERR_EXIT(_SHR_E_PORT, "all ports must be activated [number of ports: %d, number of activated ports: %d]", nof_links ,nof_active_link);
    }

    SOC_PBMP_ITER(PBMP_SFI_ALL(unit), port)
    {
        DNXF_LINK_INPUT_CHECK(unit, port);
        rv = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_port_enable_set,(unit, port, 0));
        SHR_IF_ERR_EXIT(rv);
    }
    ...
    if (!(flags & SOC_DNXF_ENABLE_MAC_LOOPBACK)) 
	{
        ...
        PBMP_SFI_ITER(unit, port)
        {
                if(flags & SOC_DNXF_ENABLE_EXTERNAL_LOOPBACK) {
                    rv = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_port_loopback_set, (unit, port, portmodLoopbackCount));
                    SHR_IF_ERR_EXIT(rv);
                } else {
                    ...
                    SHR_IF_ERR_EXIT(rv);
                    /*Set loopback*/
                    rv = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_port_loopback_set, (unit, port, portmodLoopbackPhyGloopPMD));
                    SHR_IF_ERR_EXIT(rv);
                }
    ...
    *field_val = ((!(flags & SOC_DNXF_ENABLE_MAC_LOOPBACK)) ? 0x1:0x0);
    ...
    SOC_PBMP_ITER(PBMP_SFI_ALL(unit), port)
    {
        rv = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_port_enable_set,(unit, port, 1));
        SHR_IF_ERR_EXIT(rv);
    }
...
soc_ramon_cell_snake_test_sw_configuration_set(int unit)
{
    ...
    soc_reg_above_64_set(unit, RTP_REACHABILITY_MESSAGE_GENERATOR_CONFIGURATIONr, REG_PORT_ANY, 0, reg_above64_val);
    ...
    soc_reg_above_64_set(unit, BRDC_DCML_ECC_ERR_1B_MONITOR_MEM_MASKr, REG_PORT_ANY, 0, reg_above64_val);
    soc_reg_above_64_set(unit, BRDC_DCML_ECC_ERR_2B_MONITOR_MEM_MASKr, REG_PORT_ANY, 0, reg_above64_val);
    ...
    /* FMAC configuration */
    SHR_IF_ERR_EXIT(WRITE_BRDC_FMAC_ECC_ERR_1B_MONITOR_MEM_MASKr(unit, 0xFFFFFFFF));
    SHR_IF_ERR_EXIT(WRITE_BRDC_FMAC_ECC_ERR_2B_MONITOR_MEM_MASKr(unit, 0xFFFFFFFF));
