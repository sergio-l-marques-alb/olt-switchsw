...
/**
 * \brief
 *   Utility struct for pipe weight functions.
 *
 *   The struct holds information about the registers to be configured.
 */
typedef struct
{
    /* Soc block info */
...
/**
 * \brief
 *   Fill pipe weight registers info struct according to gport.
 *
 *   WFQ can be set on ingress pipe, middle pipe, and egress pipe.
 *   The procedure is the same for all pipes, but the registers are
 *   different.
 */
STATIC void
...
{
    if (soc_dnxf_cosq_pipe_type_ingress == pipe_type) /* ingress */
    {
    ...
    else if (soc_dnxf_cosq_pipe_type_rtp == pipe_type) /* rtp */
    {
    ...
    else if (soc_dnxf_cosq_pipe_type_middle == pipe_type) /* middle */
    {
    ...
    else if (soc_dnxf_cosq_pipe_type_egress == pipe_type) /* egress */
    {
        info->nof_instances = dnxf_data_device.blocks.nof_instances_lcm_get(unit);
        info->soc_block = SOC_BLK_LCM;

        info->pipes_weights_register = LCM_PIPES_WEIGHTS_REGISTERr;
        info->wfq_weight_p_0 = WFQ_WEIGHT_P_0f;
        info->wfq_weight_p_1 = WFQ_WEIGHT_P_1f;
        info->wfq_weight_p_2 = WFQ_WEIGHT_P_2f;
 ...
 *   Set pipes WFQs.
 *
 *   This function is common for fabric rx/mid/tx.
 *   Whether to configure rx/mid/tx WFQ is done according to the
 *   pipe_type param.
 */
shr_error_e
soc_ramon_cosq_pipe_weight_set(int unit, soc_dnxf_cosq_pipe_type_t pipe_type, int pipe, int port, soc_dnxf_cosq_weight_mode_t mode, int weight)
{
    ...
    /* Fill registers info according to pipe type (ingress, middle, egress) */
 ...
 *
 *   This function is common for fabric rx/mid/tx.
 *   Whether to get configuration of rx/mid/tx WFQ is done according
 *   to the pipe_type param.
 */
shr_error_e
