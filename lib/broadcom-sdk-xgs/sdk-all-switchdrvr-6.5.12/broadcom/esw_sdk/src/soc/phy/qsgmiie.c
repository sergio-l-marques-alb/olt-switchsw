...
qsgmiie_show_serdes_info(phy_ctrl_t *pc, qsgmiie_dev_info_t *pInfo, phymod_core_info_t *core_info)
{
    ...
    sal_strncpy(pInfo->name,"QSGMIIE-", strlen("QSGMIIE-"));
...
STATIC int 
qsgmiie_ref_clk_convert(int port_refclk_int, phymod_ref_clk_t *ref_clk)
{
...
qsgmiie_speed_to_interface_config_get(qsgmiie_speed_config_t* speed_config, phymod_phy_inf_config_t* interface_config)
{
    ...
    */
    ...
    SOC_IF_ERROR_RETURN(
        qsgmiie_ref_clk_convert(speed_config->port_refclk_int, &(interface_config->ref_clock)));
...
phy_qsgmiie_speed_get(int unit, soc_port_t port, int *speed)
{
    ...
    pCfg = (qsgmiie_config_t *) pc->driver_data;
    SOC_IF_ERROR_RETURN(
        qsgmiie_ref_clk_convert(pCfg->speed_config.port_refclk_int, &ref_clk));
                              ...
                              soc_port_ability_t *ability)
{
    ...
    reg_ability = 0 & (PHYMOD_AN_CAP_SYMM_PAUSE|PHYMOD_AN_CAP_ASYM_PAUSE);
...
STATIC
int
qsgmiie_sdk_poly_to_phymod_poly(uint32 sdk_poly, phymod_prbs_poly_t *phymod_poly){
    switch(sdk_poly){
 ...
 */
STATIC int
qsgmiie_scrambler_set(soc_phymod_ctrl_t *pmc, soc_port_t port, uint32 value)
{
    phymod_phy_access_t     *pm_phy;
...
phy_qsgmiie_control_set(int unit, soc_port_t port, soc_phy_control_t type, uint32 value)
{
    ...
    case SOC_PHY_CONTROL_SCRAMBLER:
        rv = qsgmiie_scrambler_set(pmc, port, value);
        break;
 ...
 */
STATIC int
qsgmiie_scrambler_get(soc_phymod_ctrl_t *pmc, soc_port_t port, uint32 *value)
{
    phymod_phy_access_t     *pm_phy;
    phymod_phy_inf_config_t config;
    phy_ctrl_t              *pc;
...
phy_qsgmiie_control_get(int unit, soc_port_t port, soc_phy_control_t type, uint32 *value)
{
    ...
    /* SCRAMBLER */
    case SOC_PHY_CONTROL_SCRAMBLER:
       rv = qsgmiie_scrambler_get(pmc, port, value);
       break;
    case SOC_PHY_CONTROL_LANE_SWAP:
                       ...
                       soc_phy_event_t event, uint32 value)
{
    ...
    case phyEventLpiBypass:
        rv = phy_qsgmiie_control_set(unit, port, SOC_PHY_CONTROL_EEE, value);
        break;
    default:
        rv = SOC_E_UNAVAIL;
        break;
