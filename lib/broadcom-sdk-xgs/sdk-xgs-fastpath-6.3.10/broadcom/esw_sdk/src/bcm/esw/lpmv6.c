...
_bcm_defip_cfg_t_dump(_bcm_defip_cfg_t *lpm_cfg)
{
    ...
    if (lpm_cfg) {
        soc_cm_debug(DK_TESTS, "flags: 0x%x  vrf: %d\n", lpm_cfg->defip_flags, 
                                               lpm_cfg->defip_vrf);
        if (lpm_cfg->defip_flags & BCM_L3_IP6) {
            soc_cm_debug(DK_TESTS, "defip_ip6_addr - ");   
            for (i = 0; i < sizeof(bcm_ip6_t); i++) {
                soc_cm_debug(DK_TESTS,"0x%x:", lpm_cfg->defip_ip6_addr[i]); 
            }
            soc_cm_debug(DK_TESTS,"\n");
        } else {
            soc_cm_debug(DK_TESTS,"defip_ip_addr: 0x%x\n", lpm_cfg->defip_ip_addr);
        }
        soc_cm_debug(DK_TESTS,"defip_sub_len: %d defip_index: %d\n", 
                      lpm_cfg->defip_sub_len, lpm_cfg->defip_index);
        for(i = 0; i < sizeof(bcm_mac_t); i++) {
            soc_cm_debug(DK_TESTS,"defip_mac_addr - ");  
            soc_cm_debug(DK_TESTS,"0x%x:", lpm_cfg->defip_mac_addr[i]);
        }

        if (lpm_cfg->defip_flags & BCM_L3_IP6) {
            soc_cm_debug(DK_TESTS,"\ndefip_nexthop_ip6 - ");   
            for (i = 0; i < sizeof(bcm_ip6_t); i++) {
                soc_cm_debug(DK_TESTS,"0x%x:", lpm_cfg->defip_nexthop_ip6[i]); 
            }
            soc_cm_debug(DK_TESTS,"\n");
        } else {
            soc_cm_debug(DK_TESTS,"defip_nexthop_ip: 0x%x\n", lpm_cfg->defip_nexthop_ip);
        }
       
        soc_cm_debug(DK_TESTS,"defip_tunnel: %d defip_prio: %d\n", 
                      lpm_cfg->defip_tunnel, lpm_cfg->defip_prio);  

       soc_cm_debug(DK_TESTS,"defip_intf: %d defip_port_tgid: %d\n", 
                    lpm_cfg->defip_intf, lpm_cfg->defip_port_tgid);

       soc_cm_debug(DK_TESTS,"defip_stack_port: %d defip_modid: %d\n", 
                    lpm_cfg->defip_stack_port, lpm_cfg->defip_modid);

       soc_cm_debug(DK_TESTS,"defip_vid: %d defip_ecmp: %d\n", lpm_cfg->defip_vid, 
                                                      lpm_cfg->defip_ecmp);   

      soc_cm_debug(DK_TESTS,"defip_ecmp_count: %d defip_ecmp_index: %d\n", 
                   lpm_cfg->defip_ecmp_count, lpm_cfg->defip_ecmp_index);

      soc_cm_debug(DK_TESTS,"defip_l3hw_index: %d defip_tunnel_option: %d\n", 
                   lpm_cfg->defip_l3hw_index, lpm_cfg->defip_tunnel_option);

      soc_cm_debug(DK_TESTS,"defip_mpls_label: %d defip_lookup_class: %d\n",
