/*
 * $Id: subport.c,v 1.52 Broadcom SDK $
...
#define SCACHE_ALLOC_SIZE_INIT(_var_)                                          \
    do {                                                                       \
       ...
       SUBPORT_VVERB(("WarmBoot: Scache offset initialized to 0\n"));          \
    } while (0);

#define SCACHE_ALLOC_SIZE_INCR(_var_, _count_)                                 \
    do {                                                                       \
        (_var_) += (_count_); (_scache_ofst_) += (_count_);                    \
        SUBPORT_VVERB(("WarmBoot: Scache offset incr by %d to %d\n",           \
...
_bcm_kt2_subport_sw_dump(int unit)
{
    ...
    if (BCM_PBMP_NOT_NULL(SOC_INFO(unit).subtag_allowed_pbm)) {
        soc_cm_print("Subtag Subport Groups: total (%d)\n",
                     _bcm_subtag_subport_group_count[unit]);
        for(idx = 0; idx < _BCM_KT2_SUBPORT_GROUP_MAX; idx++) {
            if(_BCM_KT2_SUBTAG_SUBPORT_GROUP_USED_GET(unit, idx)) {
                soc_cm_print("%d ", idx);
            }
        }
        soc_cm_print("\n");
        ...
        soc_cm_print("Subtag Subport ids and pp_port used:\n");
        for (idx = 0; idx < SOC_MAX_NUM_PP_PORTS; idx++) {
            if (_BCM_KT2_SUBTAG_SUBPORT_PORT_INFO_VALID_GET(unit, idx)) {
                soc_cm_print("group_gport = 0x%x, vlan = %d tcam_idx = %d\n", 
        ...
        soc_cm_print("Subtag Subports not enabled!!\n");
    }
    soc_cm_print("\n--**--\n");

    if (BCM_PBMP_NOT_NULL(SOC_INFO(unit).linkphy_pbm)) {
        soc_cm_print("Linkphy Subport Groups: total (%d)\n",
                     _bcm_linkphy_subport_group_count[unit]);
        for(idx = 0; idx < _BCM_KT2_SUBPORT_GROUP_MAX; idx++) {
        ...
        soc_cm_print("Linkphy Subport ids and pp_port used:\n");
        for (idx = 0; idx < SOC_MAX_NUM_PP_PORTS; idx++) {
            if (_BCM_KT2_LINKPHY_SUBPORT_PORT_INFO_VALID_GET(unit, idx)) {
                soc_cm_print("group_gport = 0x%x, streams(%d) = {%d, %d}\n", 
        ...
        soc_cm_print("Linkphy Subports not enabled!!\n");
    }
