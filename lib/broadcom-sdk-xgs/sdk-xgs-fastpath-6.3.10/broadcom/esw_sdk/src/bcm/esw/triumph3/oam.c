/*
 * $Id: oam.c,v 1.72.2.2 Broadcom SDK $
 *
                                    ...
                                    _bcm_oam_hash_data_t *h_data_p)
{
    ...
    if (BCM_FAILURE(rv)) {
        OAM_ERR(("OAM Error: RMEP pool state get failed "
                    " - %s.\n", bcm_errmsg(rv)));
        return (rv);
    }
    ...
    if(allocCount == 0)
    {
        ...
        rv = WRITE_MA_STATEm(unit, MEM_BLOCK_ALL, h_data_p->group_index,
                &ma_state_entry);
        if (BCM_FAILURE(rv)) {
            OAM_ERR(("OAM Error: Group state (GID=%d) table write"
                        " failed - %s.\n", h_data_p->group_index,
                        bcm_errmsg(rv)));
            return (rv);
        }
        ...
        if (h_data_p->is_remote) {
            ...
            */
            ...
            if (BCM_FAILURE(rv) && rv != BCM_E_NOT_FOUND) {
                OAM_ERR(("OAM(unit %d) Error: bhh_session_hw_delete failled " 
                "(EP=%d) - %s.\n", unit, ep_id, bcm_errmsg(rv)));
                return (rv);
            }
                            ...
                            bcm_oam_endpoint_info_t *ep_info_p)
{
    ...
    while (NULL != cur) {
        ...
        if (NULL == h_data_p) {
            OAM_ERR(("OAM(unit %d)Error:Hash data is empty\n",
                     unit));
            return (BCM_E_INTERNAL);
        }
                                  ...
                                  bcm_oam_endpoint_info_t *ep_info_p)
{
    ...
    if (ep_info_p->flags & BCM_OAM_ENDPOINT_CCM_COPYFIRSTTOCPU) {
        OAM_ERR(("OAM (unit %d) Error: Unsupported flag\n", unit));
        return (BCM_E_PARAM);
    }
   ...
   /*
    ...
    if(_bcm_tr3_oam_mepid_validate(unit, ep_info_p)) {
        OAM_ERR(("OAM(unit %d)Error:MEPID:%x is not unique in group %x\n",
...
_bcm_tr3_oam_fp_trunk_create(int unit, _bcm_oam_control_t *oc, 
    ...
    unit, FUNCTION_NAME(), hash_data->dglp));
    ...
    unit, FUNCTION_NAME(), local_port, modid));
...
_bcm_tr3_oam_fp_create(int unit, _bcm_oam_control_t *oc, 
    ...
    if (hash_data->vp != 0) {
        ...
        if (BCM_FAILURE(rv)) {
            OAM_ERR(("OAM(unit %d) Error: Qualifying VlanTranslationHit (rx), \
            EP=%d %s.\n", unit, hash_data->ep_id, bcm_errmsg(rv)));
            return (rv);
        }
#endif
        ...
        OAM_VERB(("OAM(unit %d): %s - Local Port %d, modid %d, trunkid %d.\n", 
                 unit, FUNCTION_NAME(), local_port, modid, tgid));
    ...
    OAM_VERB(("OAM(unit %d): %s _bcm_oam_lm_dm_search_data.match_count %d, \
        ...
        unit, FUNCTION_NAME(), _bcm_oam_lm_dm_search_data.match_count, 
 ...
 */
STATIC int
_bcm_tr3_oam_wb_group_recover(int unit, int stable_size, uint8 **oam_scache, int grpal)
{
                           ...
                           uint16 recovered_ver)
{
    ...
    if (recovered_ver >= BCM_WB_VERSION_1_2) {
        ...
        if (BCM_FAILURE(rv)) {
            rv = (rv == BCM_E_RESOURCE) ? (BCM_E_EXISTS) : rv;
            OAM_ERR(("OAM(unit %d) Error: (EP_ID=%d) Index reserve "
                    " failed  - %s.\n", unit, ep_id, bcm_errmsg(rv)));
            return (rv);
        }
...
_bcm_tr3_oam_reinit(int unit)
{
    ...
    */
        ...
        if (SOC_FAILURE(rv)) {
            OAM_ERR(("OAM(unit %d) Error: scache alloc failed  - %s.\n",       
                    unit, bcm_errmsg(rv))); 
            goto cleanup;
        }
    }
    ...
    if (soc_feature(unit, soc_feature_bhh)) {
        ...
        if (group_info->flags & BCM_OAM_GROUP_REMOTE_DEFECT_TX) {  
           ...
           while (NULL != cur) {
               ...
               if (NULL == h_data_p) {
                   OAM_ERR(("OAM(unit %d) Error: Group=%d endpoints access failed -"\
                             " %s.\n", unit, group_info->id, bcm_errmsg(BCM_E_INTERNAL)));
                   _BCM_OAM_UNLOCK(oc);
                   return (BCM_E_INTERNAL);
               }
                        ...
                        bcm_oam_endpoint_info_t *endpoint_info)
{
    ...
    if(hash_data->flags & BCM_OAM_ENDPOINT_LOSS_MEASUREMENT) {
        ...
        rv = soc_profile_mem_add(unit, &oc->ing_service_pri_map,
                (void *) &entries, BCM_OAM_INTPRI_MAX, &profile_index);
        if (BCM_FAILURE(rv)) {
            OAM_ERR(("BHH(unit %d) Error: Adding ING_SERVICE_PRI_MAP entry"\
                     " - %s.\n", unit, bcm_errmsg(rv)));
    ...
    case bcmOAMEndpointTypeBHHMPLSVccv:
        ...
        if (BCM_SUCCESS(rv)) {
            if ((soc_MPLS_ENTRYm_field32_get(unit, &mpls_entry, VALIDf) == 0x1)) 
            {
                ...
                if (BCM_FAILURE(rv)) {
                    OAM_ERR(("OAM(unit %d) Error: %s MPLS_ENTRY write failed - "
                             "%s.\n", unit, __FUNCTION__, bcm_errmsg(rv)));
                    bcm_tr_mpls_unlock(unit);
                    return (rv);
                }
            }
