...
int
_bcm_tr3_mpls_match_delete(int unit, int vp)
{
    _bcm_tr_mpls_bookkeeping_t *mpls_info = MPLS_INFO(unit);
    int rv=BCM_E_NONE;
    vlan_xlate_extd_entry_t vent;
        ...
        sal_memset(&ret_ment, 0, sizeof(mpls_entry_entry_t));
         ...
         if (rv == SOC_E_NONE) {
              ...
              if (soc_MPLS_ENTRYm_field32_get(unit, &ret_ment, 
                   ...
                   /* Use specified EXP-map to determine internal prio/color */
                   mpls_port->exp_map = 
                        soc_MPLS_ENTRYm_field32_get(unit, &ret_ment, MPLS__EXP_MAPPING_PTRf);
                   ...
                   /* Use the specified internal priority value */
                   mpls_port->int_pri = 
                        soc_MPLS_ENTRYm_field32_get(unit, &ret_ment, MPLS__NEW_PRIf);
...
_bcm_tr3_mpls_match_key_recover(int unit)
{
    ...
    for (i = index_min; i <= index_max; i++) {
        ...
        if (_bcm_vp_used_get(unit, vp, _bcmVpTypeMpls)) {
            if (soc_VLAN_XLATE_EXTDm_field32_get(unit, &vent, KEY_TYPE_0f) ==
             TR3_VLXLT_X_HASH_KEY_TYPE_OVID) {
                ...
                bcm_tr_mpls_port_match_count_adjust(unit, vp, 1);
        } else if (soc_VLAN_XLATE_EXTDm_field32_get(unit, &vent, KEY_TYPE_0f) ==
             TR3_VLXLT_X_HASH_KEY_TYPE_IVID) {
