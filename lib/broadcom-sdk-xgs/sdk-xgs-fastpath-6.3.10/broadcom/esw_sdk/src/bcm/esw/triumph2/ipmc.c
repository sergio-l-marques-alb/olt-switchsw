/*
 * $Id: ipmc.c,v 1.60.6.1 Broadcom SDK $
...
#ifdef BCM_KATANA2_SUPPORT
    if (SOC_IS_KATANA2(unit)) {
       _bcm_kt2_init_check(unit, ipmc_id);
    }
#endif
                            ...
                            bcm_port_t port, int count)
{
...
#ifdef BCM_KATANA2_SUPPORT
    if (SOC_MEM_IS_VALID(unit, repl_grp_mem[0])) {
        for (i = 0; i <= 2; i++) {
    ...
    dest_pbm_mem = SOC_IS_KATANA2(unit) ? L3_IPMC_2m : L3_IPMCm;
    ...
    flush_active = 1;
    timeout_val = soc_property_get(unit, spn_MMU_QUEUE_FLUSH_TIMEOUT, 200000);
    ...
    while (flush_active) { 
        ...
        if (soc_timeout_check(&timeout)) {
            soc_cm_print("ERROR: Queue  flush operation failed \n");
    ...
    for (i = 0; i < 2; i++) {     
        if ((queue_id = soc_MMU_EXT_MC_QUEUE_LIST0m_field32_get(unit, 
            ...
            if (SOC_MEM_FIELD_VALID(unit, MMU_EXT_MC_QUEUE_LIST0m,             
                ...
                vlan_last = soc_MMU_EXT_MC_QUEUE_LIST0m_field32_get(unit, &qlist0_entry,
                                                           l3_last[i]);
                ptr |= ((copy << 15) | (buf_type << 16) | (vlan_last << 17));
            }
                                        ...
                                        int *q_count_increased)
{
    ...
    if (q_found) {
        ...
        if (ptr_type == 0) {
            ...
            if (SOC_IS_KATANA(unit)) {
                if (if_count == 1) {
                    _bcm_kt_ipmc_egress_intf_add(unit, ipmc_id, port,
                                                 ipmc_ptr, *id,
                                                 &new_ipmc_ptr,
                                                 &last_ipmc_flag);

                } else {
                    _bcm_kt_ipmc_egress_intf_set(unit, ipmc_id, port,
                                                 ipmc_ptr, if_count,
                                                 id, &new_ipmc_ptr,
                                                 &last_ipmc_flag, FALSE);
            ...
            if (ipmc_ptr < partition) {
                if_array[0] = ipmc_ptr + BCM_XGS3_DVP_EGRESS_IDX_MIN;
            } else {
        ...
        if (SOC_IS_KATANA(unit)) {
            ...
            if (if_count == 1) {
                ...
                if (!BCM_XGS3_DVP_EGRESS_IDX_VALID(unit, *id)) {
                    ...
                    /* Next hop entry */
                    nh_index = *id - BCM_XGS3_DVP_EGRESS_IDX_MIN ;
                }
       ...
       for (i = 0; i < q_count; i++) {
            ...
            if (SOC_IS_KATANA(unit)) {
                if ((repl_ptr_array[i] >> 14) & 1) { /* if its a nh or l3 intf */
                    ...
                    {
                        /* Next hop */
                        if_array[vlan_count] =  id + BCM_XGS3_DVP_EGRESS_IDX_MIN;
                    } else {
                        /* L3 interface */
