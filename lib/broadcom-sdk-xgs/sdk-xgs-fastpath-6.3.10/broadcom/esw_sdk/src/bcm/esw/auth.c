/*
 * $Id: auth.c,v 1.52.6.2 Broadcom SDK $
    ...
    BCM_PBMP_ITER(pbmp, p) {
        ...
        case BCM_AUTH_MODE_UNAUTH:
            ...
            if (mode & BCM_AUTH_BLOCK_IN) {
                if (soc_feature(unit, soc_feature_field)) {
                    (void)_auth_field_install_all(unit, p); 
                }
...
_auth_field_install(int unit, int port, bcm_mac_t mac) 
{
    ...
    if (!fp_cntl[unit].inited) {
        ...
        BCM_FIELD_QSET_ADD(qset0, bcmFieldQualifyDstMac);
        BCM_FIELD_QSET_ADD(qset0, bcmFieldQualifyEtherType);
        BCM_IF_ERROR_RETURN(
            bcm_esw_field_group_create(unit, qset0, 14, &group0));
        BCM_IF_ERROR_RETURN(
            bcm_esw_field_entry_create(unit, group0, &entry0));
        BCM_PBMP_PORT_SET(pbm, port);
        BCM_IF_ERROR_RETURN(
            bcm_esw_field_qualify_InPorts(unit, entry0, pbm, PBMP_E_ALL(unit)));
        BCM_IF_ERROR_RETURN(
        ...
        BCM_FIELD_QSET_ADD(qset2, bcmFieldQualifyInPorts);
        BCM_IF_ERROR_RETURN(
            bcm_esw_field_group_create(unit, qset2, 13, &group2));
        BCM_IF_ERROR_RETURN(
            bcm_esw_field_entry_create(unit, group2, &entry2));
        BCM_IF_ERROR_RETURN(
            bcm_esw_field_qualify_InPorts(unit, entry2, pbm, PBMP_E_ALL(unit)));
        ...
        if (BCM_FAILURE(rv) || BCM_FAILURE(rv2)) {
            bcm_esw_field_entry_remove(unit, entry0);
            bcm_esw_field_entry_destroy(unit, entry0);
            bcm_esw_field_group_destroy(unit, group0);
            bcm_esw_field_entry_remove(unit, entry2);
            bcm_esw_field_entry_destroy(unit, entry2);
            bcm_esw_field_group_destroy(unit, group2);
