/*
 * $Id: qos.c,v 1.84.4.4 Broadcom SDK $
    ...
    case _BCM_QOS_MAP_TYPE_EGR_MPLS_MAPS:
        if (!_BCM_QOS_EGR_MPLS_USED_GET(unit, id)) {
            ...
            if ( SOC_IS_KATANAX(unit) || SOC_IS_TRIUMPH3(unit) ||
                ...
                if (BCM_FAILURE(rv)) {
                    SOC_ERROR_PRINT((DK_WARN, "Unit:%d, Warn(%s) clear mapid (%d)"
                                 " from EGR_VLAN_CONTROL_3 registers \n", unit,
                              soc_errmsg(rv),QOS_INFO(unit)->egr_mpls_hw_idx[id]));
                }
            }
...
bcm_tr2_qos_map_add(int unit, uint32 flags, bcm_qos_map_t *map, int map_id)
{
    ...
    case _BCM_QOS_MAP_TYPE_EGR_MPLS_MAPS:
        if (!_BCM_QOS_EGR_MPLS_USED_GET(unit, id)) {
            ...
            if (!((flags & BCM_QOS_MAP_L2) || (flags & BCM_QOS_MAP_L2_INNER_TAG)||
