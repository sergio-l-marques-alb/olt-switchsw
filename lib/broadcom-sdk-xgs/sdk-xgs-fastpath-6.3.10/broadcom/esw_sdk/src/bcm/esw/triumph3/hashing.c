 ...
 *
 * File:        hashing.c
 * Purpose:     TR3-Hash calcualtions for trunk and ECMP packets.
 */
 ...
 */
STATIC int
main_tr3_do_rtag7_hashing(int unit,
                       ...
                       bcm_rtag7_base_hash_t *hash_res)
{
    ...
    if ((rtag7_a_sel == RTAG7_IPV6) || (rtag7_a_sel == RTAG7_IPV4)) {
        ...
        if (((rtag7_a_sel == RTAG7_IPV6) && 
            ...
            BCM_VERB(("Unit %d - Symmetric hash swap for hash A calculation.\n", unit));
        }

        if ((rtag7_a_sel == RTAG7_IPV6) && rtag7_en_flow_label_ipv6_a) {
            BCM_VERB(("Unit %d - Hash calculation: The system is set to use ipv6 flow label" 
    ...
    if ((rtag7_b_sel == RTAG7_IPV6) || (rtag7_b_sel == RTAG7_IPV4)) {
        ...
        if (((rtag7_b_sel == RTAG7_IPV6) && 
            ...
            temp = hash[10]; hash[10] = hash[8]; hash[8] = temp;
            temp = hash[6]; hash[6] = hash[5]; hash[5] = temp;
            BCM_VERB(("Unit %d - Symmetric hash swap for hash B calculation.\n", unit));
        }
 ...
 */
STATIC int
select_tr3_hash_subfield(uint32 concat, int hash_sub_sel, uint64 *selected_subfield,
 ...
 */
STATIC int
main_tr3_compute_lbid(int unit, bcm_rtag7_base_hash_t *hash_Base)
{
    /*regular var declaration*/
    ...
    if (SOC_REG_FIELD_VALID(unit, ING_CONFIG_64r, LBID_RTAGf)) {
        ...
        if (BCM_SUCCESS(rv)) {
            ...
            BCM_VERB(("Unit %d -compute_lbid fail, lbid_rtag=0\n", unit));
            lbid_rtag =0;
        }
    } else {
        rv = (BCM_E_UNAVAIL);
    ...
    if (lbid_rtag == 7) {
        ...
        BCM_VERB(("Unit %d - Hash calculation: This function doesn't support rtag 0 6"
                  " pls change register ING_CONFIG.LBID_RTAG to value 7\n", unit));
        hash_Base->rtag7_lbid_hash = 0;
        hash_Base->lbid_hash_valid = FALSE;
    }

    BCM_VERB(("unit %d - lbid_hash_val=%d, valid=%d\n", unit,
 ...
 */
STATIC int
compute_tr3_ecmp_hash(int unit, bcm_rtag7_base_hash_t *hash_Base,
                  uint32 *hash_value)
{
    /*regular var declaration*/
    uint32 hash_sub_sel;
    ...
    if (ecmp_hash_use_rtag7 == 0) {
        BCM_VERB(("Unit %d - ECMP Hash calculation:  non rtag7 calc not supported\n", unit));
        *hash_value =  0;
        return BCM_E_NONE;
    }
    ...
    BCM_VERB(("Unit %d - ecmp hash_seb_sel=%d, hash_offset=%d, concat=%d\n",
                           unit, hash_sub_sel, hash_offset, concat));
    BCM_IF_ERROR_RETURN
        (select_tr3_hash_subfield(concat, hash_sub_sel, &hash_subfield, hash_Base));
    ...
    *hash_value &= 0xffff;  
    ...
    BCM_VERB(("Unit %d - ecmp hash val=%d\n", unit,  *hash_value));
 ...
 */
STATIC int
compute_tr3_rtag7_hash_trunk(int unit , bcm_rtag7_base_hash_t *hash_Base,
                         uint32 *hash_value)
{
    uint32 hash_sub_sel;
    uint32 hash_offset;    
    ...
    BCM_VERB(("Unit %d - Trunk hash_seb_sel=%d, hash_offset=%d, concat=%d\n",
                  unit, hash_sub_sel, hash_offset, concat));
    ...
    *hash_value &= offset_shift;  
    ...
    BCM_VERB(("Unit %d - Trunk hash_value=%d\n", unit, *hash_value));
    ...
    if (hash_Base->is_nonuc && nuc_trunk_hash_use_rtag7 == 0) {
        BCM_VERB(("Unit %d - NonUC trunk Hash calculation:  non rtag7 calc not supported\n", unit));
        *hash_value = 0;
    }
...
/*
 * Function:
 *          compute_tr3_rtag7_hash_hg_trunk
 * Description:
 ...
 */
STATIC int
compute_tr3_rtag7_hash_hg_trunk(int unit , bcm_rtag7_base_hash_t *hash_Base,
                         uint32 *hash_value)
{
    uint32 hash_sub_sel;
    uint32 hash_offset;     
    ...
    *hash_value &= offset_shift;  
    ...
    BCM_VERB(("Unit %d - HG Trunk hash_value=%d\n", unit, *hash_value));
 ...
 *  tgid - trunk port id
 *  hash_index - hash value calculated from compute_tr3_rtag7_hash_trunk
 *  hash_rh_index -  hash value calculated from compute_tr3_rtag7_hash_rh_trunk
 ...
 */
STATIC int
get_tr3_hash_trunk(int unit, int tgid, uint32 hash_index, bcm_gport_t *dst_gport)
{
    uint32 trunk_base;
 ...
 */
STATIC int
get_tr3_hash_trunk_nuc(int unit, int tgid, 
                   ...
                   uint32 hash_index, bcm_gport_t *dst_gport)
{
    ...
    BCM_VERB(("Unit %d - Nonuc-trunk table index = %d\n", 
 ...
 *  unit - StrataSwitch PCI device unit number (driver internal).
 *  ecmp_group - multipath id
 *  hash_index - hash value calculated from compute_tr3_ecmp_hash
 *  hash_rh_index -  hash value calculated from compute_tr3_ecmp_rh_hash
 ...
 */
STATIC int
get_tr3_hash_ecmp(int unit, int ecmp_group, uint32 hash_index, bcm_if_t *nh_id)
{
    ...
    BCM_VERB(("\tECMP offset 0x%08x, ptr 0x%x\n", ecmp_offset, ecmp_ptr));
...
STATIC int
get_tr3_hash_hg_trunk(int unit, int hgtid, uint32 hash_index, bcm_gport_t *dst_gport)
{
    uint32 trunk_base;
    uint32 trunk_group_size;    
    uint32 rtag;
    ...
    BCM_VERB(("\tHG Trunk HW index 0x%08x\n", trunk_index));
    BCM_VERB(("\tHG Trunk group size 0x%08x\n", trunk_group_size));
...
/*
 * Function:
 *          _bcm_tr3_switch_pkt_info_hash_get
 * Description:
 *    main tr3 hash get function  
 *
 * Parameters     
 *  unit - StrataSwitch PCI device unit number (driver internal).
 ...
 */
int
_bcm_tr3_switch_pkt_info_hash_get (int unit,
                                   bcm_switch_pkt_info_t *pkt_info,
                                   ...
                                   bcm_if_t *dst_intf)
{
    ...
    BCM_IF_ERROR_RETURN
        (main_tr3_do_rtag7_hashing(unit, pkt_info, &hash_res));
    BCM_IF_ERROR_RETURN
        (main_tr3_compute_lbid(unit, &hash_res));
    ...
    if (0 != (pkt_info->flags & BCM_SWITCH_PKT_INFO_HASH_MULTIPATH)) {
        ...
        BCM_IF_ERROR_RETURN
            (compute_tr3_ecmp_hash(unit, &hash_res, &hash_value));
        BCM_VERB(("\tECMP Hash value 0x%08x\n", hash_value));

        BCM_IF_ERROR_RETURN
            (get_tr3_hash_ecmp(unit,
    ...
    if (0 != (pkt_info->flags & BCM_SWITCH_PKT_INFO_HASH_TRUNK)) {
        ...
        if (trunk >= ti.trunk_id_min && trunk <= ti.trunk_id_max) {
            BCM_IF_ERROR_RETURN
                (compute_tr3_rtag7_hash_trunk(unit, &hash_res, &hash_value));
            BCM_VERB(("\tTrunk Hash value 0x%08x\n", hash_value));
            if (hash_res.is_nonuc) {
                BCM_IF_ERROR_RETURN
                    (get_tr3_hash_trunk_nuc(unit, trunk, pkt_info->fwd_reason, 
                                hash_value, dst_gport));
            } else {
                BCM_IF_ERROR_RETURN
                    (get_tr3_hash_trunk(unit, trunk,
                                    hash_value, dst_gport));
            }
        } else if (trunk >= ti.trunk_fabric_id_min && trunk <= ti.trunk_fabric_id_max) {
            BCM_IF_ERROR_RETURN
                (compute_tr3_rtag7_hash_hg_trunk(unit, &hash_res, &hash_value));
            BCM_VERB(("\tHG-Trunk Hash value 0x%08x\n", hash_value));
            BCM_IF_ERROR_RETURN
                (get_tr3_hash_hg_trunk(unit, trunk - ti.trunk_fabric_id_min,
                                hash_value, dst_gport));
        }
