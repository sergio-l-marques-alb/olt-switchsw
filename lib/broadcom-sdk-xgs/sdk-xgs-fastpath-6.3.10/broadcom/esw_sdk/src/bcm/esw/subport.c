...
#endif /*(defined(BCM_KATANA2_SUPPORT)||defined(BCM_TRIUMPH2_SUPPORT)||defined(BCM_TRX_SUPPORT))&& defined(INCLUDE_L3)*/
/*
 * Function:
                              ...
                              void *user_data)
{
    ...
    if (soc_feature(unit, soc_feature_linkphy_coe) ||
        soc_feature(unit, soc_feature_subtag_coe)) {
        rv = bcm_kt2_subport_port_traverse(unit, cb, user_data);
        if (!(soc_feature(unit, soc_feature_l3))) {
            return rv;
        }
    }
...
#if defined(BCM_TRIUMPH2_SUPPORT) && defined(INCLUDE_L3)
    if (soc_feature(unit, soc_feature_subport_enhanced)) {
        rv = bcm_tr2_subport_port_traverse(unit, cb, user_data);
        return rv;
    }
#endif /* BCM_TRIUMPH2_SUPPORT && INCLUDE_L3 */
#if defined(BCM_TRX_SUPPORT) && defined(INCLUDE_L3)
    if (soc_feature(unit, soc_feature_subport)) {
        rv = bcm_tr_subport_port_traverse(unit, cb, user_data);
    }
#endif /* BCM_TRX_SUPPORT && INCLUDE_L3 */
    return rv;
}
