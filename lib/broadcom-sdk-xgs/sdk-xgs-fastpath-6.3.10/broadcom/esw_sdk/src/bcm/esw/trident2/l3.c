/*
 * $Id: l3.c,v 1.84.2.10 Broadcom SDK $
...
#define _BCM_TD2_GPORT_TRUNK_BIT      (0x8000)
                                        ...
                                        int *nh_idx, void *l3x_entry)
{
    ...
    if (embedded_nh) {
        ...
        l3cfg->l3c_port_tgid = glp & _BCM_TD2_L3_PORT_MASK;
        l3cfg->l3c_modid = (glp >> _BCM_TD2_L3_MODID_SHIFT)
                           & _BCM_TD2_L3_MODID_MASK;
        if (glp & _BCM_TD2_GPORT_TRUNK_BIT) {
...
_bcm_td2_l3_defip_mem_get(int unit, uint32 flags, int plen, soc_mem_t *mem)
{
    ...
    *mem = L3_DEFIPm;
    if ((flags & BCM_L3_IP6)) {
        ...
        if (plen > 64) {
            soc_cm_debug(DK_ERR, "Cannot allow prefix length > 64 in "
                         "L3_DEFIP\n");
            return BCM_E_PARAM;
        }
    } else if (BCM_XGS3_L3_DEFIP_TBL_SIZE(unit) == 0) {
        /* no l3_defip. cannot insert ipv4 */
        soc_cm_debug(DK_ERR, "Cannot allow ipv4 routes in "
                     "L3_DEFIP_PAIR_128\n");
        return BCM_E_PARAM;
    }
                        ...
                        void *alpm_entry)
{
    ...
    if (ipv6) {
        ...
        lpm_cfg->defip_sub_len = soc_mem_field32_get(unit, alpm_mem, alpm_entry, LENGTHf);
#if 0        
        /* Get subnet mask. */
...
_bcm_td2_alpm_update_match(int unit, _bcm_l3_trvrs_data_t *trv_data)
{
    ...
    if (SOC_WARM_BOOT(unit)) {
         ...
         if (soc_mem_index_count(unit, L3_DEFIP_PAIR_128m) != 0) {
             ...
             if (rv < 0) {
                 soc_cm_print("ERROR!  ALPM Warmboot V6-128 recovery failed");
             }
         }
         return (rv);
    }
    ...
    for (idx = (idx_end - 1); idx >= idx_start; idx--) {
        ...
        if (ipv6 && (soc_mem_field32_get(unit, L3_DEFIPm, lpm_entry, MODE0f) == 0)) {
            /* Function called for IPv6 LPM/ALPM walk; LPM entry is IPv4 */
            continue;
        }
