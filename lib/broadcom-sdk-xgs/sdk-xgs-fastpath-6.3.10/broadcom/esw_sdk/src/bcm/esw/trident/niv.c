/*
 * $Id: niv.c,v 1.28.2.1 Broadcom SDK $
...
bcm_trident_niv_sw_dump(int unit)
{
    ...
    for (i = 0; i < num_vp; i++) {
        if (!(NIV_PORT_INFO(unit, i)->flags & BCM_NIV_PORT_MATCH_NONE)) {
            ...
            soc_cm_print("    Trunk member bitmap = %s\n", 
            ...
            while (NULL != curr_ptr) {
                ...
                soc_cm_print("      Match VLAN = 0x%x\n",
                        curr_ptr->match_vlan);
                soc_cm_print("      Service TPID = 0x%x\n",
                        curr_ptr->service_tpid);
                soc_cm_print("      Service VLAN = 0x%x\n",
                        curr_ptr->service_vlan);
                soc_cm_print("      Service PRI = 0x%x\n",
                        curr_ptr->service_pri);
                soc_cm_print("      Service CFI = 0x%x\n",
                        curr_ptr->service_cfi);
                soc_cm_print("      Service Qos Map ID = 0x%x\n",
                        curr_ptr->service_qos_map_id);
                soc_cm_print("      Next Hop Index = 0x%x\n",
                        curr_ptr->next_hop_index);
                soc_cm_print("      L3 Intf = 0x%x\n",
				        curr_ptr->intf);
                soc_cm_print("      Multicast_flags = 0x%x\n",
