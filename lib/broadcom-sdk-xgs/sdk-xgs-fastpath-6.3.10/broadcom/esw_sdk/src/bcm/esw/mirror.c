/*
 * $Id: mirror.c,v 1.353.4.1 Broadcom SDK $
                                   ...
                                   void **entries)
{
    ...
    niv_buffer = (uint32) mirror_dest->niv_src_vif & _BCM_TD_MIRROR_NIV_SRC_VIF_MASK;
...
_bcm_mirror_sw_dump(int unit)
{
    ...
    if (SOC_IS_TRX(unit) && !SOC_IS_HURRICANEX(unit) &&
                !soc_feature(unit, soc_feature_mirror_flexible)) {
        soc_cm_print("  Mirror Exclusive   : %s\n",
    ...
    for (idx = 0; idx < BCM_MIRROR_MTP_COUNT; idx++) {
        soc_cm_print("  MTP(%d).pbmp_used=%s\n",idx,
                     SOC_PBMP_FMT(
