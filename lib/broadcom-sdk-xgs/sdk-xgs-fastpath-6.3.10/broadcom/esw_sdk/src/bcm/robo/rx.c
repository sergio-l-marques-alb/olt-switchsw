...
#define ROBO_RX_UNIT_STARTED(unit) \
    (RX_IS_SETUP(unit) && (rx_units_started & (1 << unit)))
 ...
 */
#define ROBO_MARK_PKT_PROCESSED(unit, chan, dv, idx)                         \
    do {(void)chan;                                                          \
...
bcm_robo_rx_init(int unit)
{
    ...
    _BCM_ROBO_RX_SYSTEM_UNLOCK;
    soc_cm_debug(DK_RX, "RX: Initialized unit %d\n", unit);
...
bcm_robo_rx_start(int unit, bcm_rx_cfg_t *cfg)
{
    ...
    if (rx_units_started & (1 << unit)) {
        if (rx_units_rxsusped) {
            soc_cm_debug(DK_RX, "RxMon start\n");
            soc_eth_dma_rxenable(unit);
            rx_units_rxsusped = 0;
            return BCM_E_NONE;
        } else {
            soc_cm_debug(DK_RX, "RX start:  Already started\n");
            return BCM_E_BUSY;
        }
    }
    ...
    soc_cm_debug(DK_RX, "RX: Starting unit %d\n", unit);
    ...
    FOREACH_SETUP_CHANNEL(unit, chan) {
        soc_cm_debug(DK_RX, "RX: Starting unit %d chan %d\n", unit, chan);

        soc_eth_dma_occupancy_get(chan, &test_unit);
        if ((test_unit >= 0) && (test_unit != unit)) {
            soc_cm_debug(DK_ERR, "RX: DMA %d already be used by unit %d\n",
                         chan, test_unit);
        ...
        if (rv < 0) {
            soc_cm_debug(DK_ERR, "RX: Error on setup unit %d, chan %d\n",
                         unit, chan);
            robo_rx_cleanup(unit);
            return rv;
        }
...
bcm_robo_rx_stop(int unit, bcm_rx_cfg_t *cfg)
{
    ...
    if (robo_rx_control.thread_running) {
        ...
        if (!robo_rx_control.thread_exit_complete) {
            soc_cm_debug(DK_WARN, "RX %d: Thread %p running after signaled "
                ...
                uint8 priority, void *cookie, uint32 flags)
{
    ...
    soc_cm_debug(DK_RX, "RX: Registering %s on %d, flags 0x%x%s\n",
                 name, unit, flags, flags & BCM_RCO_F_INTR ? "(intr)" : "");

    if (!(flags & BCM_RCO_F_COS_ACCEPT_MASK) &&
            !(flags & BCM_RCO_F_ALL_COS)) {
        soc_cm_debug(DK_WARN, "RX unit %d: "
                     "Registering callback with no COS accepted.\n", unit);
        soc_cm_debug(DK_WARN, "    Callbacks will not occur to %s\n",
                     name);
    }
...
robo_rx_chain_start(int unit, int chan, eth_dv_t *dv)
{
    ...
    soc_cm_debug(DK_RX + DK_VERBOSE, "RX: Starting %d/%d/%d\n",
...
robo_rx_chain_start_or_sched(int unit, int chan, eth_dv_t *dv)
{
    ...
    soc_cm_debug(DK_RX + DK_VERBOSE, "RX: Chain. glob tok %d.\n",
                 RX_TOKENS(unit));
    ...
    if (!global_usecs) { /* Can start immediately */
        ...
        soc_cm_debug(DK_RX, "RX: Scheduling %d/%d/%d in %d us; "
                     "cur %u; sleep %u\n", unit, chan, ROBO_DV_INDEX(dv),
...
robo_rx_cleanup(int unit)
{
    ...
    if (soc_robo_dma_abort(unit) < 0) {
        soc_cm_debug(DK_WARN, "RX: Error aborting DMA\n");
    }
    */
...
robo_rx_pkt_thread(void *param)
{
    ...
    soc_cm_debug(DK_RX, "RX:  Packet thread starting\n");
    ...
    soc_cm_debug(DK_RX, "RX: Packet thread exitting\n");
...
robo_rx_done_packet(int unit, eth_dv_t *dv, eth_dcb_t *dcb)
{
    ...
    if (RX_CHAN_RUNNING(unit, chan)) {
        ...
        if (!dv->dv_flags) {
            ...
            ROBO_MARK_PKT_PROCESSED(unit, chan, dv, pkt_dcb_idx);
        }
...
robo_rx_process_packet(int unit, bcm_pkt_t *pkt)
{
    ...
    assert(pkt == ROBO_DV_PKT(dv, idx));

#ifdef  BROADCOM_DEBUG
    if (soc_cm_debug_check(DK_PACKET)) {
        /* Dump the packet info */
        soc_cm_debug(DK_RX + DK_PACKET, "robo_rx_process_packet: packet in\n");
        /*bcm_pkt_dump(unit, pkt, soc_cm_debug_check(DK_VERBOSE));*/
    ...
    for (rco = rx_ctl[pkt->rx_unit]->rc_callout; rco; rco = rco->rco_next) {
        ...
        case BCM_RX_HANDLED:
            handled = TRUE;
            ROBO_MARK_PKT_PROCESSED(unit, chan, dv, idx);
            soc_cm_debug(DK_PACKET | DK_VERBOSE, "rx: pkt handled by %s\n",
                         rco->rco_name);
            rco->rco_pkts_handled++;
            break;
        case BCM_RX_HANDLED_OWNED:
            handled = TRUE;
            ROBO_MARK_PKT_PROCESSED(unit, chan, dv, idx);
            soc_cm_debug(DK_PACKET | DK_VERBOSE, "rx: pkt owned by %s\n",
                         rco->rco_name);
            pkt->alloc_ptr = NULL;  /* Mark as stolen */
            ...
            break;
        default:
            soc_cm_debug(DK_WARN, "robo_rx_process_packet: unit=%d: "
...
robo_rx_user_cfg_check(int unit)
{
    ...
    FOREACH_SETUP_CHANNEL(unit, chan) {
        if (RX_CHAINS(unit, chan) < 0) {
            soc_cm_debug(DK_WARN, "rx_config %d %d: Warning: chains < 0.",
                         unit, chan);
            RX_CHAINS(unit, chan) = 0;
        } else {
            chan_count++;
            if (RX_CHAINS(unit, chan) > ROBO_RX_CHAINS_MAX) {
                soc_cm_debug(DK_WARN, "rx_config %d %d: Warning: "
    ...
    if (cfg->pkts_per_chain <= 0 ||
        ...
        soc_cm_debug(DK_WARN, "rx_config: Warning: bad pkts/chn %d. "
    ...
    for (i = 0; i < RX_PPC(unit); i++) {
        ...
        if (pkt->_pkt_data.data == NULL) {
            ...
            if (pkt_data == NULL) {/* Failed to allocate a pkt for this DV. */
                if (!warned) {
                    warned = 1;
                    soc_cm_debug(DK_WARN, "RX: Failed to allocate mem\n");
                }
...
bcm_robo_rx_show(int unit)
{
    ...
    if (!RX_INIT_DONE(unit)) {
        soc_cm_print("Initializing RX subsystem (%d)\n",
                     bcm_rx_init(unit));
    }
    ...
    soc_cm_print("    Pkt Size %d. Pkts/Chain %d. Pkts/sec %d. Burst %d.\n",
                 RX_PKT_SIZE(unit),
                 RX_PPC(unit), RX_PPS(unit), RX_BURST(unit));
    soc_cm_print("    Cntrs:  Pkts %d. Last start %d. Bad Hndlr %d.\n"
    ...
    soc_cm_print("  Registered callbacks:\n");
    ...
    soc_cm_print("  Channel Info\n");

    FOREACH_SETUP_CHANNEL(unit, chan) {
        soc_cm_print("    Chan %d is %s: Chains %d. COS 0x%x. DCB/pkt %d\n",
                     chan,
                     RX_CHAN_RUNNING(unit, chan) ? "running" : "setup",
                     RX_CHAINS(unit, chan), 
                     RX_CHAN_CFG(unit, chan).cos_bmp,
                     RX_CHAN_CTL(unit, chan).dcb_per_pkt);
        soc_cm_print("        rpkt %d. rbyte %d. dpkt %d. dbyte %d. "
               "mem fail %d.\n", 
        ...
        if (soc_cm_debug_check(DK_VERBOSE)) {
            for (i = 0; i < RX_CHAINS(unit, chan); i++) {
                ...
                if (dv == NULL || !soc_eth_dma_dv_valid(dv)) {
                    soc_cm_print("        DV %d (%p) is not valid\n", i, (void *)dv);
                } else {
                    soc_cm_print("        DV %d (%d, %p) %s. chan %d. "
                                 "pkt cnt %d. pkt[%d] %p\n",
    ...
    soc_cm_print("  Queue Info\n");

    /* Display queue info */
    for (i = 0; i <= RX_QUEUE_MAX(unit); i++) {
        queue = RX_QUEUE(unit, i);
        soc_cm_print("    Queue %d: PPS %d. CurPkts %d. TotPkts %d. "
                     "Disc %d.\n",
                     i, queue->pps,
                     queue->count,
                     queue->tot_pkts,
                     queue->rate_disc);
        soc_cm_debug(DK_VERBOSE, "        Tokens %d. Last_fill %u. Max %d. "
