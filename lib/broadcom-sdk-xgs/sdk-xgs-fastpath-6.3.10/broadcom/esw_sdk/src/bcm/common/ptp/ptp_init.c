    ...
    int unit)
{
    ...
    if (BCM_FAILURE(rv = _bcm_ptp_stack_destroy(unit, 0))) {
        PTP_ERROR_FUNC("_bcm_ptp_stack_destroy()");
    }
    ...
    bcm_ptp_stack_info_t *info)
{
    ...
    if (soc_feature(unit, soc_feature_ptp)) {
        ...
        if (stack_p->memstate != PTP_MEMSTATE_INITIALIZED) {
            PTP_LOG("Found no supported PTP platforms\n");
            rv = BCM_E_FAIL;
            goto done;
        }
...
int _bcm_ptp_stack_destroy(int unit, bcm_ptp_stack_id_t ptp_id)
{
    int rv;
    _bcm_ptp_info_t *ptp_info_p;
    _bcm_ptp_stack_info_t *stack_p;
    ...
    bcm_ptp_stack_id_t ptp_id)
{
    ...
    if (_bcm_common_ptp_unit_array[unit].stack_array[ptp_id].memstate == PTP_MEMSTATE_INITIALIZED) {
        ...
        sal_free(_bcm_common_ptp_unit_array[unit].stack_array[ptp_id].clock_array);
