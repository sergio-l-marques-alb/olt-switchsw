						...
						int num_bits, int num_rsps)
{
    ...
    if (tr3_tcam_access_debug) {
        soc_cm_print("initial ETU_SBUS_CMD_ERR register state:\n");
        SOC_IF_ERROR_RETURN(READ_ETU_SBUS_CMD_ERR_INFO1r(unit, &status));
        soc_cm_print("ETU_SBUS_CMD_ERR_INFO1:0x%08x\n", status);
        SOC_IF_ERROR_RETURN(READ_ETU_SBUS_CMD_ERR_INFO2r(unit, &status));
        soc_cm_print("ETU_SBUS_CMD_ERR_INFO2:0x%08x\n", status);
    }
#endif
...
#if 1
    if (tr3_tcam_access_debug) {
        soc_cm_print("after dw wr ETU_SBUS_CMD_ERR register state:\n");
        SOC_IF_ERROR_RETURN(READ_ETU_SBUS_CMD_ERR_INFO1r(unit, &status));
        soc_cm_print("ETU_SBUS_CMD_ERR_INFO1:0x%08x\n", status);
        SOC_IF_ERROR_RETURN(READ_ETU_SBUS_CMD_ERR_INFO2r(unit, &status));
        soc_cm_print("ETU_SBUS_CMD_ERR_INFO2:0x%08x\n", status);
...
#if 1
    if (tr3_tcam_access_debug) {
        soc_cm_print("after 1st cw wr ETU_SBUS_CMD_ERR register state:\n");
        SOC_IF_ERROR_RETURN(READ_ETU_SBUS_CMD_ERR_INFO1r(unit, &status));
        soc_cm_print("ETU_SBUS_CMD_ERR_INFO1:0x%08x\n", status);
        SOC_IF_ERROR_RETURN(READ_ETU_SBUS_CMD_ERR_INFO2r(unit, &status));
        soc_cm_print("ETU_SBUS_CMD_ERR_INFO2:0x%08x\n", status);
    ...
    if (SOC_SUCCESS(rv)) {
        for (retry = 2000; retry; retry--) {
            rv = READ_ETU_TX_RAW_REQ_CONTROL_WORDr(unit, &tcam_raw_control_word);
            if (tr3_tcam_access_debug) soc_cm_print("ETU_TX_RAW_REQ_CONTROL_WORD readback: 0x%08x\n", tcam_raw_control_word);
            if (SOC_FAILURE(rv)) {
                break;
            }
#if 0
        ...
        if (!retry) {
            soc_cm_print("soc_triumph3_tcam_access: Instruction timeout\n");
            if (tr3_tcam_sim) { return SOC_E_NONE; }
            rv = SOC_E_TIMEOUT;
        }
		rv = READ_ETU_GLOBAL_INTR_STSr(unit, &status);
        if (tr3_tcam_access_debug) soc_cm_print("ETU_GLOBAL_INTR_STS: 0x%08x\n", status);
        if (!(soc_reg_field_get(unit, ETU_GLOBAL_INTR_STSr, status, TX_RAW_REQ_DONEf))) rv = SOC_E_FAIL; 
            ...
            if (status) {
                ...
                if (tr3_tcam_access_debug) soc_cm_print("ETU_SBUS_CMD_ERR_INFO1:0x%08x\n", status3);
                SOC_IF_ERROR_RETURN(READ_ETU_SBUS_CMD_ERR_INFO2r(unit, &status3));
                if (tr3_tcam_access_debug) soc_cm_print("ETU_SBUS_CMD_ERR_INFO2:0x%08x\n", status3);
...
_soc_tr3_esm_init_set_esm_mode_per_port(int unit)
{
    ...
    tcam_info = SOC_CONTROL(unit)->tcam_info;
    if (tcam_info == NULL) {
        soc_cm_print("unit %d: tr3_set_esm_mode_per_port: No tcam entries\n", 
                     unit);
        return SOC_E_NONE;
    }
    partitions = tcam_info->partitions;
        ...
        for (i = 0; i < blks_per_part; i++) {
            ...
            if (partitions[part].ad_info.hit_bit_enable) {
                ...
                if ((hbit_base << 11) >= max_hit_bits) {
                    /* flag error */
                    soc_cm_print("unit %d: ESM max no. of hit bits (%d) reached.",
                                  unit, max_hit_bits);
                    soc_cm_print("Some entries may not have hit bits\n");
                    hbit_avail = 0;
                }
            }
            ...
            if (!SOC_SUCCESS(mem_rv)) {
               soc_cm_print("unit %d: No more et_pa_xlat memory. Partition = %d\n", unit, part);
                return mem_rv;
            }
            ...
            if (!SOC_SUCCESS(mem_rv)) {
               soc_cm_print("unit %d: Could not get more et_pa_xlat memory. Partition = %d\n", unit, part);
                return mem_rv;
            }
            ...
            if (!SOC_SUCCESS(rv)) {
                soc_cm_print("unit %d: Failure writing et_pa_xlat entry. Partition = %d\n", unit, part);
                return rv;
            ...
            if (db_duplicated) {
                rv = _soc_write_et_pa_xlat_entry(unit, 
                                                ...
                                                &partitions[part].ad_info);
                if (!SOC_SUCCESS(rv)) {
                    soc_cm_print("unit %d: Failure writing duplicated db et_pa_xlat entry. Partition = %d\n", unit, part);
                    return rv;
...
_soc_triumph3_esm_init_adjust_mem_size(int unit)
{
    ...
    memState[EXT_SRC_HIT_BITS_IPV6_128m].index_max = /* SRC hit bit */
        ...
        (partitions[TCAM_PARTITION_FWD_IP6].num_entries + 63)/64 - 1;
    ...
    if (partitions[TCAM_PARTITION_ACL_IP6S].num_entries) {
        ...
        if (!fwd_ipv6_present || !fwd_ipv6u_present || !fwd_l2_present) {
            ...
            SOC_TR3_BUILD_FM_ENTRY(key_map, acl_key_type, 
                                ...
                                F2_FLAG_L3_IIF_VALID, 1, 
                                KEY_GEN_RSP_TYPE_L2_DA, KEY_GEN_RSP_TYPE_ACL, 
                                KEY_GEN_RSP_TYPE_L2_SA, 0, 1, 11, 2, 
...
soc_tr3_search_ext_mem(int unit, soc_mem_t *mem, void *key, int *index_ptr)
{
    ...
    if (rv < 0) {
        soc_cm_print("Internal search request failed\n");
        return rv;
    }
    tmp = *index_ptr;
    if (!(tmp & MAKE_FIELD(TR3_WRCMP_RSP_SMF, 1))) {
        soc_cm_debug(DK_VERBOSE, "Lookup failed\n");
        return SOC_E_NOT_FOUND;
    }
...
*/
    ...
    soc_cm_print("Exitting with miss\n");
    return SOC_E_NOT_FOUND;
}
...
soc_tr3_set_vbit(int unit, soc_mem_t mem, int index, int val)
{
    ...
    /* soc_mem_read automatically figures out correct word to read */
    rv = soc_mem_read(unit, EXT_TCAM_VBITm, MEM_BLOCK_ANY, physical_index, 
                     (void *)&vbits);
    if (rv < 0) return rv;
