/*
 * $Id: alpm_trie_v6.c,v 1.1 Broadcom SDK $
        ...
        if (SOC_SUCCESS(rv)) {
            ...
            for(index=0; index < numkey && rv == SOC_E_NONE; index++) {
                ...
                if (rv != SOC_E_NONE) {
                    ...
                    if (rv != SOC_E_NONE) {
                        soc_cm_print("SEARCH: Key=0x%x 0x%x 0x%x 0x%x  0x%x len %d SEARCH idx:%d failed on both trie!!!!\n", 
...
#ifdef LOG
        soc_cm_print("+ Inserted Key=0x%x 0x%x 0x%x 0x%x 0x%x 0x%x vpn=0x%x Len=%d idx:%d\n", 
		     pyld[index].key[0], pyld[index].key[1], pyld[index].key[2], pyld[index].key[3],
		     pyld[index].key[4], vrf, pyld[index].len, index);
#endif
        rv = trie_insert(trie, &pyld[index].key[0], NULL, pyld[index].len, &pyld[index].node);
        if (rv != SOC_E_NONE) {
            soc_cm_print("FAILED to Insert Key=0x%x 0x%x 0x%x 0x%x 0x%x vpn=0x%x Len=%d idx:%d\n", 
			 pyld[index].key[0], pyld[index].key[1], pyld[index].key[2], pyld[index].key[3],
			 pyld[index].key[4], vrf, pyld[index].len, index);
        }
...
#endif
            if (rv != SOC_E_NONE) {
                soc_cm_print("SEARCH: Key=0x%x 0x%x 0x%x 0x%x 0x%x len %d SEARCH idx:%d failed!!!!\n", 
			     pyld[index].key[0], pyld[index].key[1], pyld[index].key[2], pyld[index].key[3],
			     pyld[index].key[4], pyld[index].len, index);
                break;
            } else {
...
#ifdef VERBOSE
                soc_cm_print("Lkup[%d] key/len: 0x%x 0x%x 0x%x 0x%x 0x%x/%d Found Key/len: 0x%x 0x%x 0x%x 0x%x 0x%x/%d \n",
                             index, pyld[index].key[0], pyld[index].key[1], pyld[index].key[2],
    ...
    while(num_deleted < numkey && rv == SOC_E_NONE) {
        ...
        if (pyld[index].len != 0xFFFFFFFF) {
            ...
            if (rv == SOC_E_NONE) {
                ...
                if (rv == SOC_E_NONE) {
#ifdef LOG
                    soc_cm_print("Deleted Key=0x%x 0x%x 0x%x 0x%x 0x%x Len=%d idx:%d Num-Key:%d\n", 
				 pyld[index].key[0], pyld[index].key[1], pyld[index].key[2], pyld[index].key[3],
				 pyld[index].key[4], pyld[index].len, index, num_deleted);
                    ...
                    for (index=0; index < numkey; index++) {
                        ...
                        rv = trie_search(trie, &pyld[index].key[0], pyld[index].len, &pyldptr);
                        if (rv != SOC_E_NONE) {
                            soc_cm_print("ALL SEARCH after delete: Key= 0x%x 0x%x 0x%x 0x%x 0x%x len %d SEARCH idx:%d failed!!!!\n", 
                    ...
                    soc_cm_print("Deleted Key=0x%x 0x%x 0x%x 0x%x 0x%x Len=%d idx:%d FAILED!!!\n", 
                           pyld[index].key[0], pyld[index].key[1], pyld[index].key[2], pyld[index].key[3], pyld[index].key[4], 
                           pyld[index].len, index);
                    break;
                }
...
static void ut_bpm_dump_expect_list(expect_datum_t *datum, char *str)
{
    ...
    if (datum) {
        ...
        DQ_TRAVERSE(&datum->list, elem) {
            v6_payload_t *velem = DQ_ELEMENT_GET(v6_payload_t*, elem, listnode); 
            soc_cm_print(" Pivot: 0x%x 0x%x 0x%x 0x%x 0x%x Len: %d \n", 
...
int tmu_taps_bpm_trie_v6_ut(int id, unsigned int seed)
{
    ...
    do {
        ...
        if (rv != SOC_E_NONE) {
            soc_cm_print("FAILED to Insert PIVOT Key=0x%x 0x%x 0x%x 0x%x 0x%x Len=%d idx:%d\n", 
            ...
            if (SOC_SUCCESS(rv) && ++pivot < _MAX_TEST_PIVOTS_) {
                ...
                if (SOC_SUCCESS(rv)) {
                    ...
                    soc_cm_print("BPM for split pivot: 0x%x 0x%x 0x%x 0x%x 0x%x / %d = [0x%x 0x%x 0x%x 0x%x 0x%x] \n",
