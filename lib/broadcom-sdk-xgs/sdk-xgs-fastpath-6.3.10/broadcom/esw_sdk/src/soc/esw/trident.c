/*
 * $Id: trident.c,v 1.221.2.6 Broadcom SDK $
                                ...
                                int enable)
{
    ...
    /* Return SOC_E_NONE if a table can't be found in memory protection list */
    return SOC_E_NONE;
}
                                     ...
                                     char *prefix_str)
{
    ...
    soc_cm_debug(DK_VERBOSE, "wred bitmap: 0x%08x\n", rval);
    SOC_IF_ERROR_RETURN(READ_WRED_PARITY_ERROR_INFOr(unit, &rval));
    soc_cm_debug(DK_VERBOSE, "wred info: 0x%08x\n", rval);
    log = rval;
    ...
    soc_cm_debug(DK_ERR, "%s %s port %d index %d parity error\n", prefix_str,
                                     ...
                                     char *prefix_str)
{
    ...
    if (rval) {
        soc_cm_debug(DK_VERBOSE, "thdi pipe 0 status: 0x%08x\n", rval);
    ...
    rval = COMPILER_64_LO(rval64);
    if (rval) {
        soc_cm_debug(DK_VERBOSE, "thdi pipe 1 status: 0x%08x\n", rval);
        if (soc_reg_field_get(unit, PARITY_ERROR_STATUS_0r, rval, RESET_FLOOR_HITf)) {
            addr = soc_reg_field_get(unit, PARITY_ERROR_STATUS_0r, rval, RESET_FLOOR_ADDRf);
            spci.reg = PG_RESET_FLOOR_CELLr;
        } else if (soc_reg_field_get(unit, PARITY_ERROR_STATUS_0r, rval, RESET_OFFSET_HITf)) {
                                     ...
                                     char *prefix_str)
{
    ...
    soc_cm_debug(DK_VERBOSE, "MTRO addr: %d\n", addr);
                                   ...
                                   char *prefix_str)
{
    ...
    if (!COMPILER_64_IS_ZERO(rval64)) {
        soc_cm_debug(DK_VERBOSE, "ES S2 pipe 0 status: 0x%02x%08x\n",
    ...
    if (!COMPILER_64_IS_ZERO(rval64)) {
        soc_cm_debug(DK_VERBOSE, "ES S2 pipe 1 status: 0x%02x%08x\n",
                     COMPILER_64_HI(rval64), COMPILER_64_LO(rval64));
    ...
    if (!COMPILER_64_IS_ZERO(rval64)) {
        soc_cm_debug(DK_VERBOSE, "ES S3 pipe 0 status: 0x%04x%08x\n",
                     COMPILER_64_HI(rval64), COMPILER_64_LO(rval64));
    ...
    if (!COMPILER_64_IS_ZERO(rval64)) {
        soc_cm_debug(DK_VERBOSE, "ES S3 pipe 1 status: 0x%04x%08x\n",
                     COMPILER_64_HI(rval64), COMPILER_64_LO(rval64));
...
#define _SOC_TD_MMU_IPMC_SUB_GROUP_MAX  4
    ...
    soc_event_generate(unit, SOC_SWITCH_EVENT_PARITY_ERROR, 
                       SOC_SWITCH_EVENT_DATA_ERROR_PARITY, rval, minfo);
    soc_cm_debug(DK_ERR, "%s %s index %d parity error\n", prefix_str,
...
soc_trident_parity_error(void *unit_vp, void *d1, void *d2, void *d3, void *d4)
{
    ...
    soc_cm_print("Start after: %d, Exist after: %d\n", ser_time_2, ser_time_3);
#endif
...
_soc_td_mem_nack_error_test(int unit, _soc_ser_test_t test_type)
{
            ...
            for (table = 0; info[table].type != _SOC_PARITY_TYPE_NONE; table++) {
                ...
                for (i = 0; ser_test_skipped_mems[i].mem != INVALIDm; i++) {
                    if (((ser_test_skipped_mems[i].mem == memTable) &&
                        (ser_test_skipped_mems[i].acc_type == acc_type))) {
                        soc_cm_debug(DK_VERBOSE, "SER test disabled for memory %s, skipping.\n",
    ...
    soc_cm_print("\nStandard memories tested on unit %d: %d\n",
                unit, num_par_mems);
    soc_cm_print("Tests passed: \t%d\n", tests_passed);
    soc_cm_print("Tests skipped: \t%d (Use verbose argument to see skipped memories.)\n", skip_count);
    soc_cm_print("Tests failed: \t%d\n", tests_failed);
    return rv;
}
...
soc_td_ser_mem_test(int unit, soc_mem_t mem, _soc_ser_test_t test_type, int cmd)
{
    ...
    for(i = 0; i < num_tcams; i++) {
        ...
        if(tcam_info[i].mem == mem) {
            ...
            soc_ser_create_test_data(unit, tmp_entry, field_data,
                                     ...
                                     INVALIDf, tcam_info[i].mem, INVALIDf,
    ...
    if (found_mem && !cmd) {
        ...
        soc_cm_print("Memory %s not found.\n", SOC_MEM_NAME(unit, mem));
    }
    return rv;
}
...
soc_td_ser_test(int unit, _soc_ser_test_t test_type)
{
    ...
    if (memscan_running) {
        ...
        if (rv != SOC_E_NONE) {
            soc_cm_print("Error disabling memscan.  Aborting SER test.\n");
            return rv;
        }
    }
#endif
    ...
    if (memscan_running) {
        ...
        if (rv != SOC_E_NONE) {
            soc_cm_print("Error starting memscan.\n");
            return rv;
        }
    }
#endif
...
soc_td_ser_test_overlay (int unit, _soc_ser_test_t test_type)
{
    ...
    if (rv >= 0) {
        soc_cm_print(" Overlay memories tested: \t %d\n", numOverlays);
        soc_cm_print(" Overlay memories passed: \t %d\n", numOverlays - rv);
        soc_cm_print(" Overlay memories failed: \t %d\n\n", rv);
    } else {
        soc_cm_print ("Internal Error during overlay test.\n");
    }
    return rv;
}
