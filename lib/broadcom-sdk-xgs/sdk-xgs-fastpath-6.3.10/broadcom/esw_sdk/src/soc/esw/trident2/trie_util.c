...
int taps_key_shift(uint32 max_key_size, uint32 *key, uint32 length, int32 shift)
{
    ...
    if (_taps_util_debug) {
	SOC_DEBUG_PRINT((DK_VERBOSE, "Original key before shift:\n"));
    ...
    if (_taps_util_debug) {
	SOC_DEBUG_PRINT((DK_VERBOSE, "Resulted key after shift:\n"));
...
#define _TAPS_UTIL_MAX_MATCH_KEY_SIZE (256)
    ...
    if (_taps_util_debug) {
	SOC_DEBUG_PRINT((DK_VERBOSE, "key1:\n"));
	taps_show_prefix(max_key_size, key1, length1);

	SOC_DEBUG_PRINT((DK_VERBOSE, "key2:\n"));
    ...
    if (rv != SOC_E_NONE) {
      SOC_DEBUG_PRINT((DK_VERBOSE, "taps key shift failed 0x%x\n",rv));
    }
	...
	if (tmp_key1[index] != tmp_key2[index]) {
	    ...
	    if (_taps_util_debug) {
		SOC_DEBUG_PRINT((DK_VERBOSE, "Key1 and key2 not matching:\n"));
	    }
	    return FALSE;
	}
    }
    ...
    /* all matched */
    if (_taps_util_debug) {
	SOC_DEBUG_PRINT((DK_VERBOSE, "Key1 and key2 matched:\n"));
    }
    return TRUE;
}
