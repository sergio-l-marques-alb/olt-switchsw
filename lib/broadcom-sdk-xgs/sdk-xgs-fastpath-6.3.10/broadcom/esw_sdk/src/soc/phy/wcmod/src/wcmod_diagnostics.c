/*
 * $Id: wcmod_diagnostics.c,v 1.63 Broadcom SDK $
 *
...
int wcmod_diag_autoneg(wcmod_st *ws)
{
  ...
  if (ws->verbosity > 1) sal_sprintf ((char*)(msg+strlen(msg)), "%s",an_adv0_print(data));

  /* Adv 1 reg */
  READ_WC40_AN_IEEE1BLK_AN_ADVERTISEMENT1r(ws->unit, ws, &data);
  if (ws->verbosity > 1) sal_sprintf ((char*)(msg+strlen(msg)), "%s",an_adv1_print(data));

  /* Adv 2 reg */
  READ_WC40_AN_IEEE1BLK_AN_ADVERTISEMENT2r(ws->unit, ws, &data);
  if (ws->verbosity > 1) sal_sprintf ((char*)(msg+strlen(msg)), "%s",an_adv2_print(data));
...
int wcmod_diag_topology(wcmod_st *ws)
{
  ...
  if (data & COMBO_IEEE0_MIICNTL_GLOOPBACK_MASK) {
    strncat(msg, "Gloop enabled in COMBO_IEEE0_MIICNTL_NAME\n",
            strlen("Gloop enabled in COMBO_IEEE0_MIICNTL_NAME\n"));
  } else {
    strncat(msg, "Gloop NOT enabled in COMBO_IEEE0_MIICNTL_NAME\n",
             strlen("Gloop NOT enabled in COMBO_IEEE0_MIICNTL_NAME\n"));
  }
  ws->lane_num_ignore = 1;
  ...
  for (n = 0; n < 4; ++n) {
    sal_sprintf((char*)(msg+strlen(msg)), "Lane %d: ", n);

    /* Check R-loop 1g */
    if (data & (0x1u << (XGXSBLK1_LANECTRL2_RLOOP1G_SHIFT + n))) {
      strncat(msg, "Rloop-1G     enabled, ", 
              strlen("Rloop-1G     enabled, "));
    } else {
      strncat(msg, "Rloop-1G NOT enabled, ",
              strlen("Rloop-1G NOT enabled, "));
    }

    /* Check G-loop 1g -- reads the same register as r-loop 1g */
    if (data & (0x1u << (XGXSBLK1_LANECTRL2_GLOOP1G_SHIFT + n))) {
      strncat(msg, "Gloop-1G     enabled in XGXSBLK1_LANECTRL2_NAME\n",
              strlen("Gloop-1G     enabled in XGXSBLK1_LANECTRL2_NAME\n"));
    } else {
      strncat(msg, "Gloop-1G NOT enabled in XGXSBLK1_LANECTRL2_NAME\n",
              strlen("Gloop-1G NOT enabled in XGXSBLK1_LANECTRL2_NAME\n"));
    }
  }
  ...
  strncat(msg,"TX digital-to-analog lane swaps in XGXSBLK8_TXLNSWAP1_NAME\n",
          strlen("TX digital-to-analog lane swaps in XGXSBLK8_TXLNSWAP1_NAME\n"));
  for(n = 0; n < 4; ++n) {
    shift = (XGXSBLK8_TXLNSWAP1_TX0_LNSWAP_SEL_SHIFT + 2 * n);
    laneSwapVal=(data & (XGXSBLK8_TXLNSWAP1_TX0_LNSWAP_SEL_MASK << shift)) >> shift;
    sal_sprintf((char*)(msg+strlen(msg)), "Lane %d -> Lane %d\n", n, laneSwapVal);
  }
  ...
  strncat(msg, "TX analog-to-digital lane swap in XGXSBLK8_TXLNSWAP2_NAME\n",
          strlen("TX analog-to-digital lane swap in XGXSBLK8_TXLNSWAP2_NAME\n"));
  for(n = 0; n < 4; ++n) {
  ...
  strncat(msg, "RX analog-to-digital lane swaps in XGXSBLK8_RXLNSWAP1_NAME\n",
          strlen("RX analog-to-digital lane swaps in XGXSBLK8_RXLNSWAP1_NAME\n"));
  for(n = 0; n < 4; ++n) {
  ...
  strncat(msg, "RX digital-to-analog lane swap in XGXSBLK8_RXLNSWAP2_NAME\n",
          strlen("RX digital-to-analog lane swap in XGXSBLK8_RXLNSWAP2_NAME\n"));
  for(n = 0; n < 4; ++n) {
  ...
  if (data & TX0_ANATXACONTROL0_TXPOL_FLIP_MASK) {
    strncat(msg, "Lane 0: TX polarity flipped in TX0_ANATXACONTROL0_NAME\n",
            strlen("Lane 0: TX polarity flipped in TX0_ANATXACONTROL0_NAME\n"));
  } else {
    strncat(msg, "Lane 0: TX polarity NOT flipped in TX0_ANATXACONTROL0_NAME\n",
            strlen("Lane 0: TX polarity NOT flipped in TX0_ANATXACONTROL0_NAME\n"));
  }
  ...
  if (data & RX0_ANARXCONTROLPCI_RX_POLARITY_R_MASK) {
    strncat(msg,"Lane 0: RX polarity flipped in RX0_ANARXCONTROLPCI_NAME\n",
            strlen("Lane 0: RX polarity flipped in RX0_ANARXCONTROLPCI_NAME\n"));
  } else {
    strncat(msg,"Lane 0: RX polarity NOT flipped in RX0_ANARXCONTROLPCI_NAME\n",
            strlen("Lane 0: RX polarity NOT flipped in RX0_ANARXCONTROLPCI_NAME\n"));
  }
  ...
  if (data & TX1_ANATXACONTROL0_TXPOL_FLIP_MASK) {
    strncat(msg,"Lane 1: TX polarity flipped in TX1_ANATXACONTROL0_NAME \n",
           strlen("Lane 1: TX polarity flipped in TX1_ANATXACONTROL0_NAME \n"));
  } else {
    strncat(msg,"Lane 1: TX polarity NOT flipped in TX1_ANATXACONTROL0_NAME\n",
           strlen("Lane 1: TX polarity NOT flipped in TX1_ANATXACONTROL0_NAME\n"));
  }
  ...
  if (data & RX1_ANARXCONTROLPCI_RX_POLARITY_R_MASK) {
    strncat(msg,"Lane 1: RX polarity     flipped in RX1_ANARXCONTROLPCI_NAME\n",
            strlen("Lane 1: RX polarity     flipped in RX1_ANARXCONTROLPCI_NAME\n"));
  } else {
    strncat(msg,"Lane 1: RX polarity NOT flipped in RX1_ANARXCONTROLPCI_NAME\n",
            strlen("Lane 1: RX polarity NOT flipped in RX1_ANARXCONTROLPCI_NAME\n"));
  }
  ...
  if (data & TX2_ANATXACONTROL0_TXPOL_FLIP_MASK) {
    strncat(msg,"Lane 2: TX polarity     flipped in TX2_ANATXACONTROL0_NAME \n",
            strlen("Lane 2: TX polarity     flipped in TX2_ANATXACONTROL0_NAME \n"));
  } else {
    strncat(msg,"Lane 2: TX polarity NOT flipped in TX2_ANATXACONTROL0_NAME\n",
            strlen("Lane 2: TX polarity NOT flipped in TX2_ANATXACONTROL0_NAME\n"));
  }
  ...
  if (data & RX2_ANARXCONTROLPCI_RX_POLARITY_R_MASK) {
    strncat(msg,"Lane 2: RX polarity     flipped in RX2_ANARXCONTROLPCI_NAME\n",
            strlen("Lane 2: RX polarity     flipped in RX2_ANARXCONTROLPCI_NAME\n"));
  } else {
    strncat(msg,"Lane 2: RX polarity NOT flipped in RX2_ANARXCONTROLPCI_NAME\n",
            strlen("Lane 2: RX polarity NOT flipped in RX2_ANARXCONTROLPCI_NAME\n"));
  }
  ...
  if (data & TX3_ANATXACONTROL0_TXPOL_FLIP_MASK) {
    strncat(msg,"Lane 3: TX polarity     flipped in TX3_ANATXACONTROL0_NAME \n",
            strlen("Lane 3: TX polarity     flipped in TX3_ANATXACONTROL0_NAME \n"));
  } else {
    strncat(msg,"Lane 3: TX polarity NOT flipped in TX3_ANATXACONTROL0_NAME \n",
            strlen("Lane 3: TX polarity NOT flipped in TX3_ANATXACONTROL0_NAME \n"));
  }
  ...
  if (data & RX3_ANARXCONTROLPCI_RX_POLARITY_R_MASK) {
    strncat(msg,"Lane 3: RX polarity     flipped in RX3_ANARXCONTROLPCI_NAME\n",
            strlen("Lane 3: RX polarity     flipped in RX3_ANARXCONTROLPCI_NAME\n"));
  } else {
    strncat(msg,"Lane 3: RX polarity NOT flipped in RX3_ANARXCONTROLPCI_NAME\n",
            strlen("Lane 3: RX polarity NOT flipped in RX3_ANARXCONTROLPCI_NAME\n"));
  }
  printf("%s\n", msg);
  return SOC_E_NONE;
}
...
int wcmod_diag_general(wcmod_st *ws)
{
  ...
  case 0:  strncat(msg, "A", strlen("A")); break;
  case 1:  strncat(msg, "B", 1 ); break;
  case 2:  strncat(msg, "C", 1); break;
  default: strncat(msg, "UNKNOWN", strlen("UNKNOWN")); break;
  }
    sal_sprintf((char*)(msg+strlen(msg)), "Rev Num: %d\n",
                                 (data & SERDESID_SERDESID0_REV_NUMBER_MASK) >>
                                  SERDESID_SERDESID0_REV_NUMBER_SHIFT);
  printf ("%s", msg); strcpy (msg, "");

  strncpy (msg, "Model = ", strlen("Model = "));
  switch((data & SERDESID_SERDESID0_MODEL_NUMBER_MASK)
          >> SERDESID_SERDESID0_MODEL_NUMBER_SHIFT) {
  case MODEL_SERDES_CL73_COMBO_CORE:
                             strncat(msg, "Serdes_cl73_combo_core", strlen("Serdes_cl73_combo_core")); break;
  case MODEL_XGXS_16G    :   strncat(msg, "XGXS_16G",  strlen("XGXS_16G"));               break;
  case MODEL_HYPERCORE   :   strncat(msg, "Hypercore", strlen("Hypercore"));              break;
  case MODEL_HYPERLITE   :   strncat(msg, "Hyperlite", strlen("Hyperlite"));              break;
  case MODEL_PCIE_G2_PIPE:   strncat(msg, "PCIE_G2_PIPE", strlen("PCIE_G2_PIPE"));           break;
  case MODEL_1p25GBd_SERDES: strncat(msg, "1.25GBd_Serdes", strlen("1.25GBd_Serdes"));         break;
  case MODEL_SATA2      :    strncat(msg, "Sata2", strlen("Sata2"));                  break;
  case MODEL_QSGMII     :    strncat(msg, "QSGMII", strlen("QSGMII"));                 break;
  case MODEL_XGXS10G    :    strncat(msg, "XGXS10G", strlen("XGXS10G"));                break;
  case MODEL_WARPCORE   :    strncat(msg, "Warpcore", strlen("Warpcore"));               break;
  case MODEL_XGXS_CL73  :    strncat(msg, "XGXS_CL73", strlen("XGXS_CL73"));              break;
  case MODEL_SERDES_CL73:    strncat(msg, "Serdes_cl73", strlen("Serdes_cl73"));            break;
  case MODEL_WARPLITE   :    strncat(msg, "WARPLITE", strlen("WARPLITE"));               break;
  case MODEL_QUADSGMII  :    strncat(msg, "QUADSGMII", strlen("QUADSGMII"));              break;
  case MODEL_WARPCORE_C :    strncat(msg, "WARPCORE_C", strlen("WARPCORE_C"));             break;
  default               :    strncat(msg, "UNKNOWN", strlen("UNKNOWN"));                break;
  }
  printf("%s\n", msg); strncpy (msg, "", 1);
  ...
  if (data & PLL_ANAPLLSTATUS_PLLSEQDONE_MASK) {
    strncpy (msg,"PLL startup sequence done\n",
             strlen("PLL startup sequence done\n"));
  } else {
    strncpy (msg,"PLL startup sequence not done\n",
             strlen("PLL startup sequence not done\n"));
  }

  if (data & PLL_ANAPLLSTATUS_PLLSEQPASS_MASK) {
    strncat (msg,"PLL startup sequence passed and PLL is locked\n",
             strlen("PLL startup sequence passed and PLL is locked\n"));
  } else {
    strncat (msg,"PLL startup sequence FAILED. PLL Lock status UNKNOWN\n",
             strlen("PLL startup sequence FAILED. PLL Lock status UNKNOWN\n"));
  }

  /* PLL frequence detection sequence */
  if (data & PLL_ANAPLLSTATUS_FREQDONE_MASK) {
    strncat (msg,"PLL frequency detection sequence done\n",
             strlen("PLL frequency detection sequence done\n"));
  } else {
    strncat (msg,"PLL frequency detection sequence NOT done\n",
             strlen("PLL frequency detection sequence NOT done\n"));
  }

  if (data & PLL_ANAPLLSTATUS_FREQPASS_MASK) {
    strncat (msg,"PLL frequency detection sequence passed\n",
             strlen("PLL frequency detection sequence passed\n"));
  } else {
    strncat (msg,"PLL frequency detection sequence FAILED\n",
             strlen("PLL frequency detection sequence FAILED\n"));
  }

  /* VCO range selection sequence */
  if (data & PLL_ANAPLLSTATUS_VCODONE_MASK) {
    strncat (msg,"VCO range selection sequence done\n",
             strlen("VCO range selection sequence done\n"));
  } else {
    strncat (msg,"VCO range selection sequence NOT done\n",
             strlen("VCO range selection sequence NOT done\n"));
  }

  if (data & PLL_ANAPLLSTATUS_VCOPASS_MASK) {
    strncat (msg,"VCO range selection sequence passed\n",
             strlen("VCO range selection sequence passed\n"));
  } else {
    strncat (msg,"VCO range selection sequence FAILED\n",
             strlen("VCO range selection sequence FAILED\n"));
  }

  /* PLL lock history check. Comment out. Folks just get confused.
  if (data & PLL_ANAPLLSTATUS_LOSTPLLLOCK_SM_MASK) {
    strcat (msg,"PLL NOT locked\n");
  } else {
    strcat (msg,"PLL locked\n");
  }
  */
  printf("%s", msg); strncpy (msg, "", 1);

  /* Frequency divider ratio */
  strncpy (msg,"PLL frequency divider = ", strlen("PLL frequency divider = "));
  switch((data & PLL_ANAPLLSTATUS_PLL_MODE_AFE_MASK) >> PLL_ANAPLLSTATUS_PLL_MODE_AFE_SHIFT) {
   case PLL_ANAPLLSTATUS_PLL_MODE_AFE_div32: strncat(msg,"32",2);break;
   case PLL_ANAPLLSTATUS_PLL_MODE_AFE_div36: strncat(msg,"36",2);break;
   case PLL_ANAPLLSTATUS_PLL_MODE_AFE_div40: strncat(msg,"40",2);break;
   case PLL_ANAPLLSTATUS_PLL_MODE_AFE_div42: strncat(msg,"42",2);break;
   case PLL_ANAPLLSTATUS_PLL_MODE_AFE_div48: strncat(msg,"48",2);break;
   case PLL_ANAPLLSTATUS_PLL_MODE_AFE_div50: strncat(msg,"50",2);break;
   case PLL_ANAPLLSTATUS_PLL_MODE_AFE_div52: strncat(msg,"52",2);break;
   case PLL_ANAPLLSTATUS_PLL_MODE_AFE_div54: strncat(msg,"54",2);break;
   case PLL_ANAPLLSTATUS_PLL_MODE_AFE_div60: strncat(msg,"60",2);break;
   case PLL_ANAPLLSTATUS_PLL_MODE_AFE_div64: strncat(msg,"64",2);break;
   case PLL_ANAPLLSTATUS_PLL_MODE_AFE_div66: strncat(msg,"66",2);break;
   case PLL_ANAPLLSTATUS_PLL_MODE_AFE_div68: strncat(msg,"68",2);break;
   case PLL_ANAPLLSTATUS_PLL_MODE_AFE_div70: strncat(msg,"70",2);break;
   case PLL_ANAPLLSTATUS_PLL_MODE_AFE_div80: strncat(msg,"80",2);break;
   case PLL_ANAPLLSTATUS_PLL_MODE_AFE_div92: strncat(msg,"92",2);break;
   case PLL_ANAPLLSTATUS_PLL_MODE_AFE_div100:strncat(msg,"100",3);break;
   default:                              strncat(msg,"UNKNOWN", strlen("UNKNOWN"));break;
  }
  printf("%s", msg); strncpy (msg, "", 1);
  ...
  if (data & XGXSBLK0_XGXSCONTROL_PLL_BYPASS_MASK) {
    strncpy(msg,"PLL bypassedin XGXSBLK0_XGXSCONTROL_NAME\n",
            strlen("PLL bypassedin XGXSBLK0_XGXSCONTROL_NAME\n"));
  } else {
    strncpy(msg,"PLL not bypassed in XGXSBLK0_XGXSCONTROL_NAME\n",
            strlen("PLL not bypassed in XGXSBLK0_XGXSCONTROL_NAME\n"));
  }
  printf("%s", msg); strncpy (msg, "", 1);
  ...
  if (data & GP2_REG_GP2_8_MICRO_INIT_DONE_MASK) {
    strncpy(msg,"Micro-controller init done", strlen("Micro-controller init done"));
  } else {
    strncpy(msg,"Micro-controller init NOT done", strlen("Micro-controller init NOT done"));
  }
  printf("%s", msg); strncpy (msg, "", 1);
  return SOC_E_NONE;
}
...
int wcmod_diag_link(wcmod_st *ws)
{
    ...
    if (cntl == 0x1) {
        ...
        if ((ws->port_type == WCMOD_COMBO) && (ws->model_type == WCMOD_WC_C0)) {
            return SOC_E_NONE;
        }
        ...
        strncat(msg, "Lane 0:  RX sequencer done\n",
                strlen("Lane 0:  RX sequencer done\n"));
        } else {
        strncat(msg, "Lane 0:  RX sequencer NOT done\n",
                strlen("Lane 0:  RX sequencer NOT done\n"));
        }

        if (data & RX0_ANARXSTATUS_SYNC_STATUS_SYNC_STATUS_MASK) {
        strncat(msg, "\t     Synced\n", strlen("\t     Synced\n"));
        } else {
        strncat(msg, "\t NOT Synced\n", strlen("\t NOT Synced\n"));
        }
        ...
        strncat(msg, "Lane 1:  RX sequencer done\n",
                strlen("Lane 1:  RX sequencer done\n"));
        } else {
        strncat(msg, "Lane 1:  RX sequencer NOT done\n",
                strlen("Lane 1:  RX sequencer NOT done\n"));
        }
        ...
        strncat(msg, "Lane 2:  RX sequencer done\n",
                strlen("Lane 2:  RX sequencer done\n"));
        } else {
        strncat(msg, "Lane 2:  RX sequencer NOT done\n",
                strlen("Lane 2:  RX sequencer NOT done\n"));
        }
        ...
        strncat(msg, "Lane 3:  RX sequencer done\n",
                strlen("Lane 3:  RX sequencer done\n"));
        } else {
        strncat(msg, "Lane 3:  RX sequencer NOT done\n",
                strlen("Lane 3:  RX sequencer NOT done\n"));
        }
        ...
        READ_WC40_XGXSBLK0_XGXSCONTROLr(ws->unit, ws, &data);
        ws->lane_num_ignore = 0;
        strncat(msg, "Mode = ", strlen("Mode = "));
        switch((data & XGXSBLK0_XGXSCONTROL_MODE_10G_MASK) >> XGXSBLK0_XGXSCONTROL_MODE_10G_SHIFT) {
        case xgxsBlk0_operationModes_IndLane_OS8: strncat(msg, "Indep_OS8", strlen("Indep_OS8")); break;
        case xgxsBlk0_operationModes_IndLane_OS5:  strncat(msg, "Indep_OS5", strlen("Indep_OS5")); break;
        case XGXSBLK0_XGXSCONTROL_MODE_10G_ComboCoreMode:strncat(msg, "combo", strlen("combo"));break;
        default: strncat(msg, "UNKNOWN", strlen("UNKNOWN")); break;
        }
        ...
        strncat(msg, "TX Speed = ", strlen("TX Speed = "));
        if (strlen(getSpeedString(speed)) > sizeof(msg)) {
            return SOC_E_ERROR;
        } else {
            strncat(msg, getSpeedString(speed), strlen(getSpeedString(speed)));
            printf ("%s\n", msg);
        }
        ...
        sal_sprintf(msg, "Raw line clk rate@ %.2f MHz=%.1f MHz\n", refClkFreq, speed);
        } else {
        strcat(msg, "Cant compute raw line rate. Unknown value in PLL_ANAPLLSTATUS_NAME");
        }*/
...
int wcmod_diag_internal_tfc(wcmod_st *ws)
{
  ...
  for (n = 0; n < 4; ++n) {
    ...
    if (data & (XGXSBLK1_LANEPRBS_PRBS_EN0_MASK << shift)) {
      sal_sprintf(msg, "Lane %d:  PRBS enabled\n", n);
      /* check inversion status */
      if (data & (XGXSBLK1_LANEPRBS_PRBS_INV0_MASK << shift)) {
	strncat(msg, ", inverted\n", strlen(", inverted\n"));
      } else {
	strncat(msg, ", NOT inverted\n", strlen(", NOT inverted\n"));
      }
      /* Check PRBS order */
      switch ((data & (XGXSBLK1_LANEPRBS_PRBS_ORDER0_MASK << shift)) >> (shift + XGXSBLK1_LANEPRBS_PRBS_ORDER0_SHIFT)) {

       case XGXSBLK1_LANEPRBS_PRBS_ORDER0_prbs7:  strncat(msg, "PRBS 7", strlen("PRBS 7")) ;break;
       case XGXSBLK1_LANEPRBS_PRBS_ORDER0_prbs15: strncat(msg, "PRBS 15", strlen("PRBS 15"));break;
       case XGXSBLK1_LANEPRBS_PRBS_ORDER0_prbs23: strncat(msg, "PRBS 23", strlen("PRBS 23"));break;
       case XGXSBLK1_LANEPRBS_PRBS_ORDER0_prbs31: strncat(msg, "PRBS 31", strlen("PRBS 31"));break;
       default:                      strncat(msg, "PRBS order unknown\n", strlen("PRBS order unknown\n"));break;
      }
    } else {
      sal_sprintf(msg, "Lane %d:  PRBS NOT enabled\n", n);
    }
    printf ("%s\n", msg);
  }
  ...
  if (data & XGXSBLK0_XGXSCONTROL_PGEN_EN_MASK) {
    strncat(msg, "Pattern generator enabled in XGXSBLK0_XGXSCONTROL_NAME\n",
            strlen("Pattern generator enabled in XGXSBLK0_XGXSCONTROL_NAME\n"));
  } else {
    strncat(msg, "Pattern generator NOT enabled in XGXSBLK0_XGXSCONTROL_NAME\n",
            strlen("Pattern generator NOT enabled in XGXSBLK0_XGXSCONTROL_NAME\n"));
  }

  if (data & XGXSBLK0_XGXSCONTROL_PCMP_EN_MASK) {
    strncat(msg, "Pattern comparator enabled in XGXSBLK0_XGXSCONTROL_NAME\n",
            strlen("Pattern comparator enabled in XGXSBLK0_XGXSCONTROL_NAME\n"));
  } else {
    strncat(msg,"Pattern comparator NOT enabled in XGXSBLK0_XGXSCONTROL_NAME\n",
            strlen("Pattern comparator NOT enabled in XGXSBLK0_XGXSCONTROL_NAME\n"));
  }
  ...
  if (data & DIGITAL4_MISC5_TGEN_DATA_SEL_MASK) {
    strncat(msg,"Test generator data selected in DIGITAL4_MISC5_NAME\n",
            strlen("Test generator data selected in DIGITAL4_MISC5_NAME\n"));
  } else {
    strncat(msg,"Test generator data NOT selected in DIGITAL4_MISC5_NAME\n",
            strlen("Test generator data NOT selected in DIGITAL4_MISC5_NAME\n"));
  }

  /* test link enabled */
  if (data & DIGITAL4_MISC5_TGEN_LINK_MASK) {
    strncat(msg,"Test generator link enabled in DIGITAL4_MISC5_NAME\n",
            strlen("Test generator link enabled in DIGITAL4_MISC5_NAME\n"));
  } else {
    strncat(msg,"Test generator link NOT enabled in DIGITAL4_MISC5_NAME\n",
            strlen("Test generator link NOT enabled in DIGITAL4_MISC5_NAME\n"));
  }

  /* RX CRC */
  if (data & DIGITAL4_MISC5_RCHK_EN_MASK) {
    strncat(msg,"Rx crc enabled in DIGITAL4_MISC5_NAME\n",
            strlen("Rx crc enabled in DIGITAL4_MISC5_NAME\n"));
  } else {
    strncat(msg,"Rx crc NOT enabled in DIGITAL4_MISC5_NAME\n",
            strlen("Rx crc NOT enabled in DIGITAL4_MISC5_NAME\n"));
  }
  ...
  if (data & LNRXBERT_RXBERTCTRL_CRCCHK_EN_MASK) {
    strncat(msg, "Rx crc enabled in RXBERT_RXBERTCTRL_NAME\n",
            strlen("Rx crc enabled in RXBERT_RXBERTCTRL_NAME\n"));
  } else {
    strncat(msg, "Rx crc NOT enabled in RXBERT_RXBERTCTRL_NAME\n",
            strlen("Rx crc NOT enabled in RXBERT_RXBERTCTRL_NAME\n"));
  }
  ...
  if (data & TXBERT_TXBERTCTRL_CJPAT_EN_MASK) {
    strncat(msg, "CJPat enabled in TXBERT_TXBERTCTRL_NAME\n",
            strlen("CJPat enabled in TXBERT_TXBERTCTRL_NAME\n"));
  } else {
    strncat(msg, "CJPat NOT enabled in TXBERT_TXBERTCTRL_NAME\n",
            strlen("CJPat NOT enabled in TXBERT_TXBERTCTRL_NAME\n"));
  }

  /* CRPat */
  if (data & TXBERT_TXBERTCTRL_CRPAT_EN_MASK) {
    strncat(msg, "CRPat enabled in TXBERT_TXBERTCTRL_NAME\n",
            strlen("CRPat enabled in TXBERT_TXBERTCTRL_NAME\n"));
  } else {
    strncat(msg, "CRPat NOT enabled in TXBERT_TXBERTCTRL_NAME\n",
            strlen("CRPat NOT enabled in TXBERT_TXBERTCTRL_NAME\n"));
  }
  ...
  /* Prog data */
  /* Lane 0 */
  strncat(msg, "Lane 0:  ", strlen("Lane 0:  "));
  READ_WC40_TX0_ANATXACONTROL0r(ws->unit, ws, &data);

  if (data & TX0_ANATXACONTROL0_TX_MDATA_EN_MASK) {
    strncat(msg, "Programmable data enabled\n",
            strlen("Programmable data enabled\n"));
    ...
    sal_sprintf((char*)(msg+strlen(msg)), "\t 10-bit data: 0x%x\n", data);
    ...
    sal_sprintf((char*)(msg+strlen(msg)), "\t 20-bit data: 0x%x\n", data);
  } else {
    strncat(msg, "Programmable data NOT enabled\n",
            strlen("Programmable data NOT enabled\n"));
  }

  /* Lane 1 */
  strncat(msg, "Lane 1:  ", strlen("Lane 1:  "));
  READ_WC40_TX1_ANATXACONTROL0r(ws->unit, ws, &data);
  ...
  /* Lane 2 */
  strncpy(msg, "Lane 2:  ", strlen("Lane 2:  "));
  READ_WC40_TX2_ANATXACONTROL0r(ws->unit, ws, &data);
  ...
  /* Lane 3 */
  strncat(msg, "Lane 3:  ", strlen("Lane 3:  "));
  READ_WC40_TX3_ANATXACONTROL0r(ws->unit, ws, &data);
...
void printTaps(int lane, uint16 data)
{
  ...
  sal_sprintf (msg, "Lane %d TX Taps: Main:0x%02x Post:0x%02x Pre:0x%02x\n",
                                                lane, mainTap, postTap, preTap);

  if ((preTap + mainTap + postTap) > MAX_TAP_SUM_CONSTRAINT) {
    sal_sprintf((char*)(msg+strlen(msg)),
...
int wcmod_diag_dfe(wcmod_st *ws)
{
  ...
  strncpy(msg, "IDriver = ", strlen("IDriver = "));
  switch ((data & TX0_TX_DRIVER_IDRIVER_MASK) >> TX0_TX_DRIVER_IDRIVER_SHIFT) {
    case TX0_TX_DRIVER_IDRIVER_v680mV: strncat(msg, "680 mV\n", strlen("680 mV\n")); break; 
    case TX0_TX_DRIVER_IDRIVER_v730mV: strncat(msg, "730 mV\n", strlen("730 mV\n")); break; 
    case TX0_TX_DRIVER_IDRIVER_v780mV: strncat(msg, "780 mV\n", strlen("780 mV\n")); break; 
    case TX0_TX_DRIVER_IDRIVER_v830mV: strncat(msg, "830 mV\n", strlen("830 mV\n")); break; 
    case TX0_TX_DRIVER_IDRIVER_v880mV: strncat(msg, "880 mV\n", strlen("880 mV\n")); break; 
    case TX0_TX_DRIVER_IDRIVER_v930mV: strncat(msg, "930 mV\n", strlen("930 mV\n")); break; 
    case TX0_TX_DRIVER_IDRIVER_v980mV: strncat(msg, "980 mV\n", strlen("980 mV\n")); break; 
    case TX0_TX_DRIVER_IDRIVER_v1010mV: strncat(msg, "1010 mV\n", strlen("1010 mV\n")); break; 
    case TX0_TX_DRIVER_IDRIVER_v1040mV: strncat(msg, "1040 mV\n", strlen("1040 mV\n")); break; 
    case TX0_TX_DRIVER_IDRIVER_v1060mV: strncat(msg, "1060 mV\n", strlen("1060 mV\n")); break; 
    case TX0_TX_DRIVER_IDRIVER_v1070mV: strncat(msg, "1070 mV\n", strlen("1070 mV\n")); break; 
    case TX0_TX_DRIVER_IDRIVER_v1080mV: strncat(msg, "1080 mV\n", strlen("1080 mV\n")); break; 
    case TX0_TX_DRIVER_IDRIVER_v1085mV: strncat(msg, "1085 mV\n", strlen("1085 mV\n")); break; 
    case TX0_TX_DRIVER_IDRIVER_v1090mV: strncat(msg, "1090 mV\n", strlen("1090 mV\n")); break; 
    case TX0_TX_DRIVER_IDRIVER_v1095mV: strncat(msg, "1095 mV\n", strlen("1095 mV\n")); break; 
    case TX0_TX_DRIVER_IDRIVER_v1100mV: strncat(msg, "1100 mV\n", strlen("1100 mV\n")); break; 
    default:                            strncat(msg, "UNKNOWN\n", strlen("UNKNOWN\n")); break; 
  }
  /* Taps */
  ...
  if (data & CL72_USERB0_CL72_TX_FIR_TAP_REGISTER_TX_FIR_TAP_FORCE_MASK) {
    strncat(msg, "TX fir taps forced\n", strlen("TX fir taps forced\n"));
  } else {
    strncat(msg, "TX fir taps NOT forced\n", strlen("TX fir taps NOT forced\n"));
  }
  ...
  sal_sprintf((char*)(msg+strlen(msg)),"DSC3B0_CDR_STATUS0r:%d\n",data);
  READ_WC40_DSC3B0_CDR_STATUS1r(ws->unit, ws, &data);
  sal_sprintf((char*)(msg+strlen(msg)),"DSC3B0_CDR_STATUS1r:%d\n",data);
  READ_WC40_DSC3B0_CDR_STATUS2r(ws->unit, ws, &data);
  sal_sprintf((char*)(msg+strlen(msg)),"DSC3B0_CDR_STATUS2r:%d\n",data);

  /* PI */
  READ_WC40_DSC3B0_PI_STATUS0r(ws->unit, ws, &data);
  sal_sprintf((char*)(msg+strlen(msg)),"DSC3B0_PI_STATUS0r:%d\n",data);

  /* DFE VGA */
  READ_WC40_DSC3B0_DFE_VGA_STATUS0r(ws->unit, ws, &data);
  sal_sprintf((char*)(msg+strlen(msg)),"DSC3B0_DFE_VGA_STATUS0r:%d\n",data);
  ...
  sal_sprintf((char*)(msg+strlen(msg)),"DSC3B0_ACQ_SM_STATUS0r%d\n",data);

  READ_WC40_DSC3B0_ACQ_SM_STATUS1r(ws->unit, ws, &data);
  sal_sprintf((char*)(msg+strlen(msg)),"DSC3B0_ACQ_SM_STATUS1r%d\n",data);

  /* ANA status */
  READ_WC40_DSC3B0_ANA_STATUS0r(ws->unit, ws, &data);
  sal_sprintf((char*)(msg+strlen(msg)),"DSC3B0_ANA_STATUS0r%d\n",data);

  READ_WC40_DSC3B0_ANA_STATUS1r(ws->unit, ws, &data);
  sal_sprintf((char*)(msg+strlen(msg)),"DSC3B0_ANA_STATUS1r%d\n",data);

  READ_WC40_DSC3B0_ANA_STATUS2r(ws->unit, ws, &data);
  sal_sprintf((char*)(msg+strlen(msg)),"DSC3B0_ANA_STATUS2r%d\n",data);

  READ_WC40_DSC3B0_ANA_STATUS3r(ws->unit, ws, &data);
  sal_sprintf((char*)(msg+strlen(msg)),"DSC3B0_ANA_STATUS3r%d\n",data);
...
int wcmod_diag_ieee(wcmod_st *ws)
{
  ...
  if (data & CL72_USERB1_CL72_FAULT_REGISTER_FRAME_LOCK_MASK) {
    strncpy(msg, "CL72 frame locked\n", strlen("CL72 frame locked\n"));
  } else {
    strncpy(msg, "CL72 frame NOT locked\n", strlen("CL72 frame NOT locked\n"));
  }
  ...
  if (data & GP2_REG_GP2_E_FEC_ENABLE_MASK) {
    strncat(msg, "FEC enabled\n", strlen("FEC enabled\n"));

    sal_sprintf ((char*)(msg+strlen(msg)), "FEC signal OK: 0x%x\n",
	    (data & GP2_REG_GP2_E_FEC_SIGNAL_OK_MASK) >> GP2_REG_GP2_E_FEC_SIGNAL_OK_SHIFT);
  } else {
    strncat(msg, "FEC NOT enabled\n", strlen("FEC NOT enabled\n"));
  }
  printf("%s\n", msg);
  return SOC_E_NONE;
}
...
int wcmod_diag_eee(wcmod_st *ws)
{
  ...
  if (data & CL73_USERB0_CL73_EEECTRL_CL73_EEEEN_MASK) {
    strncpy(msg, "EEE enabled\n", strlen("EEE enabled\n"));
  } else {
    strncpy(msg, "EEE NOT enabled\n", strlen("EEE NOT enabled\n"));
  }

  if (data & CL73_USERB0_CL73_EEECTRL_HOLD_EEE_LP_ABILITY_MASK) {
    strncat(msg, "Link partner EEE enabled\n", strlen("Link partner EEE enabled\n"));
  } else {
    strncat(msg, "Link partner EEE NOT enabled\n", strlen("Link partner EEE NOT enabled\n"));
  }
  ...
  if (data & XGXSBLK7_EEECONTROL_LPI_EN_RX_MASK) {
    strncat(msg, "RX LPI pass-thru enabled in XGXSBLK7_EEECONTROL_NAME\n",
            strlen( "RX LPI pass-thru enabled in XGXSBLK7_EEECONTROL_NAME\n"));
  } else {
    strncat(msg, "RX LPI pass-thru NOT enabled in XGXSBLK7_EEECONTROL_NAME\n",
            strlen("RX LPI pass-thru NOT enabled in XGXSBLK7_EEECONTROL_NAME\n"));
  }

  if (data & XGXSBLK7_EEECONTROL_LPI_EN_TX_MASK) {
    strncat(msg,"TX LPI pass-thru enabled in XGXSBLK7_EEECONTROL_NAME\n",
            strlen("TX LPI pass-thru enabled in XGXSBLK7_EEECONTROL_NAME\n"));
  } else {
    strncat(msg,"TX LPI pass-thru NOT enabled in XGXSBLK7_EEECONTROL_NAME\n",
            strlen("TX LPI pass-thru NOT enabled in XGXSBLK7_EEECONTROL_NAME\n"));
  }

  READ_WC40_EEE_USERB0_EEE_COMBO_CONTROL0r(ws->unit, ws, &data);

  if (data & EEE_USERB0_EEE_COMBO_CONTROL0_LPI_EN_FORCE_MASK) {
    strncat(msg, "LPI pass-thru forced\n", strlen("LPI pass-thru forced\n"));
  } else {
    strncat(msg, "LPI pass-thru derived from autoneg\n", strlen("LPI pass-thru derived from autoneg\n"));
  }
  printf("%s\n", msg);
  return SOC_E_NONE;
}
...
int _wcmod_avg_vga_dfe(wcmod_st* ws, int tapsel, int *avg_value) 
{
   ...
   for (loop_var = 0; loop_var < avg_cnt; loop_var++) {
      ...
      printf("WC_VEYE : val_curr = %d, val_avg = %d, tap_select = %d\n", 
...
int _wcmod_eye_margin_data_get(wcmod_st* ws, WCMOD_EYE_DIAG_INFOt *pInfo,int type) 
{
  ...
  printf("%-22s :  max_runtime_loop:%d\n",FUNCTION_NAME(), max_runtime_loop);
  ...
  printf("WC_EYE: offset_max %d DSC2B0_ctrl 0x%x u=%d p=%d\n", 
  ...
  for (loop_var = 0; loop_var < offset_max; loop_var++) {
    ...
    veye_cnt += 1;
    /* Set a offset */
    if (type == WC_UTIL_HEYE_R) { data16 =-offset[loop_var];
    ...
    clk90_p_offset &= 0x7f;
    tmp = (short)data16;
    printf("Starting BER msmt at offset: %d clk90_p_offset: 0x%x u=%d p=%d\n",
    ...
    if (rv == SOC_E_TIMEOUT) {
      printf("WC_EYE: uC waits for offset fail: u=%d p=%d\n",ws->unit,ws->port);
      return (SOC_E_TIMEOUT);
    }
    ...
    for (runtime_loop = 0; runtime_loop <= max_runtime_loop; runtime_loop++) {
      ...
      printf("Starting prbs run for %d seconds : u=%d p=%d\n", curr_runtime,
                    ws->unit, ws->port);
        ...
        if ((pInfo->total_errs[loop_var] >= HI_CONFIDENCE_ERR_CNT) ||
            (pInfo->total_elapsed_time[loop_var] >= max_total_time)) {
             printf("WC_EYE: done PRBS err count: u=%d p=%d, total_err: %d"
  ...
  if (rv == SOC_E_TIMEOUT) {
    printf("WC_VEYE : uC waits for offset=0 fail!: u=%d p=%d\n",ws->unit, ws->port);
    return (SOC_E_TIMEOUT);
  }
...
_wcmod_eye_margin_ber_cal(WCMOD_EYE_DIAG_INFOt *pInfo, int type) 
{
    ...
    float eye_unit;
    float curr_ber_log;
    float prev_ber_log = 0;
    float good_ber_level = -7.8;
    ...
    for (loop_var = 0; loop_var < pInfo->veye_cnt; loop_var++) { 
        ...
        printf("BER measurement at offset: %f\n", margins[loop_var]);
        ...
        if (pInfo->total_errs[loop_var] == 0) { 
            ...
            printf("BER @ %04f %% = 1e%04f (%d errors in %d seconds)\n",
            ...
            bers[loop_var] /= 1000;
            printf("BER @ %2.2f%% = 1e%2.2f (%d errors in %d seconds)\n",
...
_wcmod_eye_margin_diagram_cal(WCMOD_EYE_DIAG_INFOt *pInfo, int type) 
{
    ...
    printf("first_good_ber_idx: %d, first_small_errcnt_idx: %d\n",
	...
	printf("BER *worse* than 1e-%d\n", (int)outputs[0]);
	printf("Negative margin @ 1e-12, 1e-15 & 1e-18\n");
    } else {
        low_confidence = 0;
	...
	/* Below this point the code assumes statistically valid point available */
        ...
        printf("start_n: %d, stop_n: %d, veye: %d, n_mono: %d\n",
	...
	for( loop_index = start_n; loop_index < stop_n; loop_index++ ) {
        ...
        printf("\tstart=%d, stop=%d, low_confidence=%d\n",
                                     start_n, stop_n, low_confidence);
        for (loop_index=start_n; loop_index < stop_n; loop_index++){
            printf("\ttotal_errs[%d]=0x%08x\n",
	...
	printf("Exy=%f, Eyy=%f, Exx=%f, Ey=%f,Ex=%f alpha=%f, beta=%f\n",
	...
	if(beta > 9.32){
	  ...
	  printf("\t\t- The *extrapolated* minimum BER will be reported as 1E-37.\n");
	  printf("\t\t- This may occur if the channel is near ideal (e.g. test loopback)\n");
	  printf("\t\t- While not discrete, reporting an extrapolated BER < 1E-37 is numerically corect, and informative\n\n");
	}
	...
	outputs[1] = _wcmod_util_round_real(proj_margin_18,1);
        ...
        printf("\t\tlog1e-12=%f, sq=%f\n",(float)log(1e-12),(float)sqrt(-log(1e-12)));
        printf("\t\talpha=%f\n",alpha);
        printf("\t\tbeta=%f\n",beta);
        printf("\t\tproj_ber=%f\n",proj_ber);
        printf("\t\tproj_margin12=%f\n",proj_margin_12);
        printf("\t\tproj_margin12=%f\n",proj_margin_15);
        printf("\t\tproj_margin18=%f\n",proj_margin_18);
        printf("\t\toutputs[0]=%f\n",outputs[0]);
        printf("\t\toutputs[1]=%f\n",outputs[1]);
        printf("\t\toutputs[2]=%f\n",outputs[2]);
        ...
        if( low_confidence == 1 ) {
          if(beta_max){
            printf("BER(extrapolated) is *better* than 1e-37\n");
            printf("Margin @ 1e-12    is *better* than %f\n", outputs[2]);
            printf("Margin @ 1e-15    is *better* than %f\n", outputs[3]);
            printf("Margin @ 1e-18    is *better* than %f\n", outputs[1]);
          }
          else{
            printf("BER(extrapolated) is *better* than 1e-%f\n", outputs[0]);
            printf("Margin @ 1e-12    is *better* than %f\n", outputs[2]);
            printf("Margin @ 1e-15    is *better* than %f\n", outputs[3]);
            printf("Margin @ 1e-18    is *better* than %f\n", outputs[1]);
          }
        ...
        /* JPA> Extrapolated results, high confidence */
          ...
          if(beta_max){
            printf("BER(extrapolated) = 1e-37\n");
            printf("Margin @ 1e-12    is *better* than %f\n", outputs[2]);
            printf("Margin @ 1e-15    is *better* than %f\n", outputs[3]);
            printf("Margin @ 1e-18    is *better* than %f\n", outputs[1]);
          }
          else{
          printf("BER(extrapolated) = 1e-%4.2f\n", outputs[0]);
          printf("Margin @ 1e-12    = %4.2f%%\n", outputs[2]);
          printf("Margin @ 1e-15    = %4.2f%%\n", outputs[3]);
          printf("Margin @ 1e-18    = %4.2f%%\n", outputs[1]);
          }
        }
    }
    return SOC_E_NONE;
...
wcmod_printfDigital3Up1(unsigned short int data)
{
    char msg[128] = {0};
    strncpy(msg, "\t ") ;
    if(data&0x1)   strncat(msg,"2P5G,"); 
    if(data&0x2)   strncat(msg,"5G_X4,"); 
    if(data&0x4)   strncat(msg,"6G_X4,"); 
    if(data&0x8)   strncat(msg,"10G_HIGIG,"); 
    if(data&0x10)  strncat(msg,"10G_CX4,"); 
    if(data&0x20)  strncat(msg,"12G_X4,"); 
    if(data&0x40)  strncat(msg,"12P5_X4,"); 
    if(data&0x80)  strncat(msg,"13G_X4,"); 
    if(data&0x100) strncat(msg,"15G_X4,"); 
    if(data&0x200) strncat(msg,"16G_X4,"); 
    if(data&0x400) strncat(msg,"20G_X4,"); 
    printf("%s\n", msg); 
}
/* need to review */
STATIC void 
wcmod_printfDigital3Up3(uint16 data)
{
    char msg[128] = {0};
    strncpy(msg, "\t ") ;
    if(data&0x20)  strncat(msg,"40G,"); 
    if(data&0x80)  strncat(msg,"31P5G,"); 
    if(data&0x100) strncat(msg,"25P455G,"); 
    if(data&0x200) strncat(msg,"21G_X4,"); 
    if(data&0x400) strncat(msg,"LAST=1,"); 
    if(data&0x1)   strncat(msg,"Higig2=1,"); 
    if(data&0x2)   strncat(msg,"FEC=1,"); 
    if(data& DIGITAL3_UP3_CL72_MASK)   strncat(msg,"CL72=1,"); 
    printf("%s\n", msg); 
}
