/*
 * $Id: phy84328.c,v 1.126 Broadcom SDK $
                           ...
                           soc_port_ability_t *ability)
{
    ...
    SOC_DEBUG_PRINT((DK_PHY,
        "_phy_84328_ability_advert_get: u=%d p=%d speed(FD)=0x%x pause=0x%x\n",
                            ...
                            soc_port_ability_t *ability)
{
    ...
    SOC_DEBUG_PRINT((DK_PHY,
                     "_phy_84328_ability_remote_get: u=%d p=%d speed(FD)=0x%x pause=0x%x\n",
                     unit, port, ability->speed_full_duplex, ability->pause));
    return SOC_E_NONE;
}
							 ...
							 soc_port_ability_t *ability)
{
    ...
    SOC_DEBUG_PRINT((DK_PHY,
					 "_phy_84328_ability_advert_set: u=%d p=%d speed(FD)=%x pause=0x%x\n",
					 unit, port, ability->speed_full_duplex, ability->pause));
    return SOC_E_NONE;
}
    ...
    *value = 0;
    ...
    for (lane_idx = 0; lane_idx < lanes; lane_idx++) {
        if (PHY84328_SINGLE_PORT_MODE(pc)) {
            /* Select the lane and side to access */
            SOC_IF_ERROR_RETURN(_phy_84328_channel_select(unit, port, 
                                           PHY84328_INTF_SIDE_LINE, lane_idx));
        }
	...
	for (side = PHY84328_INTF_SIDE_LINE; side <= PHY84328_INTF_SIDE_SYS; side++) {
		...
		if (PHY84328_SINGLE_PORT_MODE(pc)) {
			...
			soc_cm_print(
			    ...
			    soc_cm_print(
			    "Lane:%d: %s: type=%s, speed=%d, forced cl72=%d, mode(1.%04x)=%04x\n"
				"           pmdtype(1.%04x)=%04x, lb(1.%04x)=%04x, txdis(1.%04x)=%04x/(1.%04x)=%04x\n"
				"           link(1.%04x)=%04x, txdrv(1.%04x)=%04x, pol(1.%04x)=%04x", 
				i, (side == PHY84328_INTF_SIDE_LINE) ? "Line    " : "System  ",
				phy84328_intf_names[intf->type], intf->speed, SYS_FORCED_CL72(pc),
				PHY84328_DEV1_PMD_CONTROL_REGISTER, data1,
				PHY84328_DEV1_PMD_CONTROL_2_REGISTER, data2,
				PHY84328_SYS_DEV1_ANATXACONTROL6, data3,
				...
				pol_sts[i]);
			    if (side == PHY84328_INTF_SIDE_LINE) {
				    soc_cm_print(", pol_inv(1.%04x)=%04x", PHY84328_DEV1_GP_REGISTER_RX_INV, pol_inv_sts[i]);
			    }
			    soc_cm_print("\n"
...
phy_84328_probe(int unit, phy_ctrl_t *pc)
{
    ...
    if (all_cores > PHY84328_MAX_CORES) {
        SOC_DEBUG_PRINT((DK_ERR,"u=%d p=%d: too many cores %d\n",
                         unit, port, all_cores));
        return SOC_E_NOT_FOUND;
    }
