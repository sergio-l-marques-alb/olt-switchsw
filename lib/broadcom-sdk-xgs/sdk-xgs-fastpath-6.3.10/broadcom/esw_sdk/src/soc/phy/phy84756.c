/*
 * $Id: phy84756.c,v 1.25 Broadcom SDK $
 *
...
phy_84756_init(int unit, soc_port_t port)
{
    ...
    if (PHYCTRL_INIT_STATE(pc) == PHYCTRL_INIT_STATE_DEFAULT ||
        ...
        if (mmi_mdio_addr < 0) {
            soc_cm_debug(DK_WARN, "phy_84756_init: "
        ...
        if(rv != BMACSEC_E_NONE) {
            SOC_DEBUG_PRINT((DK_WARN, "phy_84756_init: bmacsec_phy_mac_addr_set "
                             "returned error for u=%d p=%d\n", unit, port));
            return SOC_E_FAIL;
        }

        SOC_DEBUG_PRINT((DK_PHY, "phy_84756_init: u=%d p=%d\n",unit, port));
    ...
    if ((PHYCTRL_INIT_STATE(pc) == PHYCTRL_INIT_STATE_PASS2) ||
        ...
        if (MACSEC_ENABLE(pc)) {
            ...
            if (!SOC_SUCCESS(rv)) {
            soc_cm_debug(DK_ERR, "soc_macsecphy_init: MACSEC init for"
                     " u=%d p=%d FAILED ", unit, port);
            } else {
            soc_cm_debug(DK_PHY, "soc_macsecphy_init: MACSEC init for"
...
phy_84756_enable_set(int unit, soc_port_t port, int enable)
{
    ...
    SOC_DEBUG_PRINT((DK_PHY, "phy_84756_enable_set: "
...
phy_84756_probe(int unit, phy_ctrl_t *pc)
{
    ...
    if (0xff == soc_property_port_get(unit, pc->port, spn_FCMAP_ENABLE, 0)) {
        ...
        soc_cm_print("u=%d p=%d Using Standalone PHY driver for BCM84756\n", unit, pc->port);
        return SOC_E_NONE;
    }
    return SOC_E_NOT_FOUND;
