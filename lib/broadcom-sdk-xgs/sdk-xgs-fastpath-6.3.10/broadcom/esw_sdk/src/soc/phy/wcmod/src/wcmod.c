/*
 * $Id: wcmod.c,v 1.388.2.6 Broadcom SDK $ 
...
_phy_wcmod_qsgmii_core_init(int unit, soc_port_t port)
{
    ...
    phyident_core_info_t core_info[3];
    int array_max = 3;
    int array_size = 0;
    uint16 data;
    ...
    if (SOC_IS_CALADAN3(unit)) {
        ...
        if (SOC_IS_CALADAN3(unit)) {
            qsgmii_ws->phy_ad = core_info[0].mdio_addr;
        }
        SOC_IF_ERROR_RETURN (_wcmod_getRevDetails(qsgmii_ws));
	    ...
	    SOC_IF_ERROR_RETURN
	      (_wcmod_phy_parameter_copy(pc, ws));	    
	    ws->phy_ad = core_info[1].mdio_addr;
            ws->this_lane = pc->lane_num % 4;
            SOC_IF_ERROR_RETURN (_wcmod_getRevDetails(ws));
        }
...
_phy_wcmod_enable_set(int unit, soc_port_t port, int enable)
{
    ...
    } else if (SOC_IS_CALADAN3(unit) && (ws->model_type == WCMOD_QS_A0)) {
        wcmod_st *qsgmii_ws;
        qsgmii_ws   =  (wcmod_st *)( pDesc + 1);
        ws = (wcmod_st *)(qsgmii_ws + 1);
	...
	SOC_IF_ERROR_RETURN
	  (_wcmod_phy_parameter_copy(pc, ws));
        ...
        qsgmii_ws->per_lane_control = data;
        SOC_IF_ERROR_RETURN
          (wcmod_tier1_selector("POWER_CONTROL", qsgmii_ws, &rv));
    } else if ((SOC_PBMP_MEMBER(PBMP_IL_ALL(unit), pc->port)) || (SOC_INFO(unit).port_num_lanes[pc->port] == 12)) {
...
phy_wcmod_lb_set(int unit, soc_port_t port, int enable)
{
    ...
    if (IS_MULTI_CORE_PORT(pc)) {
        ...
        for (i = 0 ; i < num_core; i++) {
            ...
            temp_ws->this_lane = 0;
            SOC_IF_ERROR_RETURN(READ_WC40_XGXSBLK1_LANECTRL2r(temp_ws->unit, temp_ws, &data16));
...
_phy_wcmod_rx_DFE_tap2_ovr_control_set(int unit, phy_ctrl_t *pc, uint32 enable)
{
    ...
    if (SOC_PBMP_MEMBER(PBMP_IL_ALL(unit), pc->port) && (ws->this_lane != 0)) {
        ...
        num_core = (SOC_INFO(unit).port_num_lanes[pc->port] + 3) / 4;
        if (SOC_IS_CALADAN3(unit)) {
            /* for il50 narrowband mode, somehow we need to init all 3 cores
             * even though only 2 are being used
             */
            if (SOC_INFO(unit).port_num_lanes[pc->port] >= 6) {
                num_core = 3;
            }
        }
        for (i = 0 ; i < num_core; i++) {
...
phy_wcmod_100g_plus_config(int unit, soc_port_t port, int speed)
{
  ...
  /*  ws_slave0   =  (wcmod_st *)( ws_master + 1);
    ...
    SOC_IF_ERROR_RETURN(_init_wcmod_st(ws_slave0));
    SOC_IF_ERROR_RETURN(_init_wcmod_st(ws_slave1));
    SOC_IF_ERROR_RETURN(_init_wcmod_st(ws_mld)); */
...
#if 0
    printf("master copy phy id is 0x%x num of lane is %d \n", ws_master->phy_ad, ws_master->num_of_lane);
    printf("slave0 copy phy id is 0x%x num of lane is %d \n", ws_slave0->phy_ad, ws_slave0->num_of_lane);
    printf("slave1 copy phy id is 0x%x num of lane is %d \n", ws_slave1->phy_ad, ws_slave1->num_of_lane);
    printf("mld copy phy id is 0x%x num of lane is %d \n", ws_mld->phy_ad, ws_mld->num_of_lane);
#endif
...
*/
    ...
    if(uc_desc == NULL) {
        ...
        soc_cm_print("%2s %2s %12s %4s %9s|%8s %6s %6s %4s %4s|%4s %4s %4s %4s " 
                         "%4s|%4s %4s %7s|%4s %4s %5s %4s %4s|%4s %4s %4s %4s %4s|%6s\n",
                         "LN", "SD", "PMD(FW_MODE)", "PPM", "clk90_ofs", "clkp1_of",
                          "p1_lvl", "m1_lvl", "PF", "lowp", "VGA", "BIAS", "DFE1", "DFE2",
                           "DFE3", "DFE4", "DFE5", "TX_DRVR", "PREC", "MAIN", "POSTC", "MTRC", "PE",
                           "ZE",   "ME",  "PO",  "ZO",  "MO", "DSC_SM");
     
        for (i = 0; i < size; i++) { 
            pDesc = &uc_desc_s[i]; 
            soc_cm_print("%2d %2d %12s %4d %9d|%8d %6d %6d %4d %4d|%4d %4s %4d %4d " 
                         "%4d|%4d %4d %#7x|%4d %4d %5d %4d %4d|%4d %4d %4d %4d %4d|%#6x\n", 
