/*
 * $Id: phy84793.c,v 1.7.6.2 Broadcom SDK $
...
_phy_84793_debug_info(int unit, soc_port_t port)
{
	...
	soc_cm_print("Port %-2d: micro ver(1.%04x)=%04x,Mode Sel(1.%04x)=%04x\n"
	...
	for (side = PHY84793_SYS_SIDE; side <= PHY84793_LINE_SIDE; side++) {
        ...
        if (mode == PHY84793_MODE_10X10G_TO_4X25G || mode == PHY84793_MODE_10X10G_TO_10X10G_SYNC 
            ...
            for (att = 0; att < 7; att++) {
               if (att == 0) {
                   if (side == PHY84793_SYS_SIDE) {
                       soc_cm_print("          linksts(DC:80b0)     : ");
                   } else {
                       soc_cm_print(" linksts(VSR:9800 HC: 80b0)    : ");
                   }
                   lane_val = link_sts;
               } else if(att == 1) {
                   if (side == PHY84793_SYS_SIDE) {
                       soc_cm_print("          tx_pol(1.f516)       : ");
                   } else {
                       soc_cm_print("          tx_pol(1.f633)       : ");
                   }
                   lane_val = tx_pol_sts;
               } else if(att == 2) {
                   if (side == PHY84793_SYS_SIDE) {
                       soc_cm_print("          rx_pol(DC:80BA)      : ");
                   } else {
                       soc_cm_print("   rx_pol(HC:80BA/9811)        : ");
                   }
                   lane_val = rx_pol_sts;
               } else if(att == 3) {
                   soc_cm_print("          pre_main             : ");
                   lane_val = preemph_main;
               } else if(att == 4) {
                   soc_cm_print("          pre_post1            : ");
                   lane_val = preemph_post1;
               } else if(att == 5) {
                   soc_cm_print("          pre_post2            : ");
                   lane_val = preemph_post2;
               } else if(att == 6) {
                   if (side == PHY84793_SYS_SIDE) {
                       soc_cm_print("          drv_cur(DC:8065)     : ");
                   } else {
                       soc_cm_print("   drv_cur(HC:8065/%04x)       : ",
               ...
               for (lane_idx = start_lane; lane_idx < end_lane; lane_idx++) {
                   ...
                   if ((mode == PHY84793_MODE_10X10G_TO_4X25G ||
                       ...
                       soc_cm_print("(ln%d=-NA-)",lane_idx);
                   } else {
                       soc_cm_print("(ln%d=%04x)",lane_idx, lane_val[lane_idx]);
                   }
               }
               soc_cm_print("\n");
            }
	...
	if (int_pc) {
		...
		soc_cm_print(" Internal: type=%s, speed=%d, link=%d \n===\n", 
...
_phy_84793_dsc_diag(int32 unit, soc_port_t port, int32 intf, int32 lane)
{
    ...
    if (intf == PHY84793_LINE_SIDE) {
        soc_cm_print("\nVSR core \n");
        soc_cm_print("LaneN  PPM clk90_ofs PF SL_TRGT LA  "
    ...
    if (hc_dc == PHY84793_DECA_CORE) {
        soc_cm_print("DECA CORE\n");
    } else {
        soc_cm_print("HEXA CORE\n");
    }
    soc_cm_print("LaneN  PPM_XFR clk90_ofs PF SL_TRGT VGA DFE1 DFE2 "
    ...
    for (i = 0; i < lanes; i++) {
        ...
        soc_cm_print("  %02d    %05d    %04d    %02d   %02d    "
                "%03d %04d %04d" 
                                                     ...
                                                     int32 op_cmd, void *arg)
{
    ...
    pc = EXT_PHY_SW_STATE(unit, port);
    SOC_DEBUG_PRINT((DK_PHY, "84793 diag_ctrl: u=%d p=%d ctrl=0x%x\n", unit, 
                                                               port, op_cmd));
    lane = PHY_DIAG_INST_LN(inst);
    intf = PHY_DIAG_INST_INTF(inst);
    ...
    switch (op_cmd) {
        ...
        default:
            if (op_type == PHY_DIAG_CTRL_SET) {
                ...
                SOC_DEBUG_PRINT((DK_PHY,"84793 diag_ctrl bad operation:\
                                     u=%d p=%d ctrl=0x%x\n", unit, port, op_cmd));
                rv = SOC_E_UNAVAIL;
            }
        break;
            ...
            case PHY84793_MODE_10X10G_TO_4X25G_OTL:
                ...
                if (rddata & (1 << lane)) {
                    ...
                    if (l_lock & (1 << lane)) {
                        ...
                        SOC_IF_ERROR_RETURN
                           (_phy_84793_modify_pma_pmd_reg (unit, port, pc, 
                           RG_RX_RX_GB_TEST_MISC_CTRL, 11, 8 ,(1 << lane)));
                        ...
                        /* LOSS OF LOCK occured once, so one error at least */
                        *value = ((err_lsb | (err_msb<<16)) & 0xffffffff) | 1;
                    } else {
                        *value = 0;
                    }
                } else {
...
int32 _phy_84793_init_pass2 (int32 unit, soc_port_t port)
{
    ...
    SOC_DEBUG_PRINT((DK_PHY, "PHY84793 init pass2: u=%d p=%d\n", unit, port));
    pc = EXT_PHY_SW_STATE(unit, port);
    ...
    if (if_sys_idx >= (sizeof(phy_84793_sys_to_port_if)/sizeof(soc_port_if_t))) {
        SOC_DEBUG_PRINT((DK_ERR, "PHY84793 invalid system side\
...
_phy_84793_config_update(int32 unit, soc_port_t port)
{
    ...
    SOC_DEBUG_PRINT((DK_PHY, "PHY84793 INIT: u=%d p=%d TX Pol=0x%x RX Pol:0x%x\n", 
...
int32 phy_84793_init(int32 unit, soc_port_t port)
{
    ...
    SOC_DEBUG_PRINT((DK_PHY, "PHY84793 init: u=%d p=%d state=%d\n", 
                               ...
                               phy_ctrl_t *pc, uint8 *new_fw, uint32 datalen)
{
    ...
    if ((data16 & 0x2000) == 0x2000) {
        SOC_DEBUG_PRINT((DK_VERBOSE|DK_PHY, "FW already downloaded,"
    ...
    SOC_DEBUG_PRINT((DK_VERBOSE|DK_PHY, "Check Sum: 0x%X \n", data16));
    if (data16 == 0x600D) {
        interface = &(INTERFACE(pc));
        interface->type = SOC_PORT_IF_SR4; 
        interface->speed = PHY84793_MODE1_SPD;
    ...
    SOC_DEBUG_PRINT((DK_VERBOSE|DK_PHY, "Version: 0x%X\n", data16));
    return SOC_E_NONE;
}
...
phy_84793_probe(int32 unit, phy_ctrl_t *pc)
{
    ...
    if (devid == PHY84793_ID_84793 || devid == PHY84793_ID_84790 ||
        ...
        SOC_DEBUG_PRINT((DK_PHY, "port %d: BCM84xxx type"
    ...
    switch(pif) 
    {
        case SOC_PORT_IF_SR4:
            if (intf->speed == PHY84793_MODE2_SPD) {
                soc_cm_print("PHY84793 MODE2 Configured\n");
                SOC_IF_ERROR_RETURN
                 (_phy_84793_config_mode (unit, port, pc, 
                                          PHY84793_MODE_4X10G_TO_4X10G));
            } else {
                soc_cm_print("PHY84793 MODE1 Configured\n");
                SOC_IF_ERROR_RETURN
                 (_phy_84793_config_mode (unit, port, pc, 
        ...
        break;
        case SOC_PORT_IF_SR10:
            soc_cm_print("PHY84793 MODE3 Configured\n");
            SOC_IF_ERROR_RETURN
             (_phy_84793_config_mode (unit, port, pc, 
        ...
        break;
        case SOC_PORT_IF_SR:
            soc_cm_print("PHY84793 MODE4 Configured\n");
            SOC_IF_ERROR_RETURN
             (_phy_84793_config_mode (unit, port, pc, 
        ...
        break;
        case SOC_PORT_IF_OTL:
            soc_cm_print("PHY84793 MODE5 Configured\n");
            SOC_IF_ERROR_RETURN
             (_phy_84793_config_mode (unit, port, pc, 
                                      PHY84793_MODE_10X10G_TO_4X25G_OTL));
...
phy_84793_speed_set(int32 unit, soc_port_t port, int32 speed)
{
    ...
    pc = EXT_PHY_SW_STATE(unit, port);
    intf = &(INTERFACE(pc));
    SOC_DEBUG_PRINT((DK_PHY,"phy_84793_speed_set: u=%d p=%d speed=%d\n", 
					 unit, port, speed));
    switch (speed) {
        case PHY84793_MODE1_SPD:
           if (intf->type != SOC_PORT_IF_SR4) {
               /* By default if the speed is 100G
                * mode1 will be configured, For mode3 and mode5
                * use if=SR10 and if=OTL*/
    ...
    if (SOC_FAILURE(rv)) {
        SOC_DEBUG_PRINT((DK_ERR, "84793  %s failed: u=%d p=%d\n", 
...
int32 phy_84793_hw_reset(int32 unit, soc_port_t port, void *not_used)
{
    ...
    SOC_DEBUG_PRINT((DK_PHY,"84793 HW Reset.....\n"));
