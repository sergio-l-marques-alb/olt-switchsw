/*
 * $Id: uc_msg.c,v 1.54 Broadcom SDK $
    ...
    while (1) {
        ...
        if (rcv.s.subclass == MOS_MSG_SUBCLASS_SYSTEM_LOG) {
            ...
            if (rcv.s.len == 0) {
                ...
                soc_cm_debug(DK_VERBOSE, "SYSTEM INFO REPLY (%08x)\n", (unsigned)send.s.data);
...
/* Workaround for a compiler that is too smart, and inserting division when we avoid it */
#if defined (__KERNEL__)
volatile uint32 always_zero = 0;
#endif
    ...
    soc_cmic_uc_ts_data_t *ts_data)
{
...

    {
        ...
        /* in TOT: COMPILER_64_UDIV_64(abs_ts_minus_ts0_sec, one_billion); */
        /* use COMPILER_HAS_LONGLONG_MUL as proxy for COMPILER_HAS_LONGLONG_DIV */
#if defined COMPILER_HAS_LONGLONG_MUL && ! defined (__KERNEL__)
        abs_ts_minus_ts0_sec /= one_billion;
...
#else
        COMPILER_64_ZERO(abs_ts_minus_ts0_sec);
        while (COMPILER_64_GE(abs_ts_minus_ts0_nsec, one_billion)) {
            COMPILER_64_SUB_32(abs_ts_minus_ts0_nsec, 1000000000);
            COMPILER_64_ADD_32(abs_ts_minus_ts0_sec, 1);
#if defined (__KERNEL__)
            COMPILER_64_ADD_32(abs_ts_minus_ts0_nsec, always_zero);
            COMPILER_64_ADD_32(abs_ts_minus_ts0_sec, always_zero);
#endif /* __KERNEL__ */
        }
        COMPILER_REFERENCE(temp);
#endif
