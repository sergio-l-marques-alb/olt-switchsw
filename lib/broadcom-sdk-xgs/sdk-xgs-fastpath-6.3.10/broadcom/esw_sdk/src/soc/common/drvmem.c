/*
 * $Id: drvmem.c,v 1.69 Broadcom SDK $
    ...
    if (soc_mem_is_valid(unit, mem) &&
        ...
        if (!SOC_MEM_IS_ARRAY(unit, mem)) {
            if((address < (SOC_MEM_INFO(unit, mem).base + SOC_MEM_INFO(unit, mem).index_min)) ||
                ...
                *numels = 0;
                *index = (address - SOC_MEM_INFO(unit, mem).base)/SOC_MEM_INFO(unit, mem).bytes;
                return SOC_E_NONE;
            }
        }  else {
            /* for arrays */        
            if((address < (SOC_MEM_INFO(unit, mem).base + SOC_MEM_INFO(unit, mem).index_min)) || 
                ...
                *index  = ((address - (SOC_MEM_INFO(unit, mem).base + SOC_MEM_INFO(unit, mem).index_min)) % \
                          (SOC_MEM_INFO(unit, mem).index_max - SOC_MEM_INFO(unit, mem).index_min))/SOC_MEM_INFO(unit, mem).bytes;
                return SOC_E_NONE;
            }
        }
    }
