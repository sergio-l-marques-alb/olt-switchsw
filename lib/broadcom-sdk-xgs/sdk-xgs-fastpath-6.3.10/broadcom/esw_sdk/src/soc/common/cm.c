/*
 * $Id: cm.c,v 1.296.2.5 Broadcom SDK $
...
soc_cm_device_destroy(int dev)
{
...
#if defined(BCM_TK371X_SUPPORT)
            soc_cm_print("Call soc_ea_deattach(dev) here!\n");
    ...
    if (info->dev_type & SOC_SWITCH_DEV_TYPE) {
        ...
        if(dev_idx == soc_ndev) {
            soc_cm_debug(DK_ERR, "ERROR: Device %d does not appear in the array soc_ndev_idx2dev_map !", dev_idx);
            rv = SOC_E_NOT_FOUND;
        } else {
