/*
 * $Id: mem.c,v 1.455.2.10 Broadcom SDK $
...
soc_mem_t soc_mem_format_map(int unit, soc_mem_t mem) {
    if (!SOC_IS_ARAD(unit)){
        return INVALIDm;
    }
    switch (mem) {
    ...
    case EPNI_TRILL_FORMAT_B_0m:
        return EPNI_EEDB_BANKm;
    case IDR_MCDA_PRFCFG_FORMAT_0m:
    case IDR_MCDA_PRFCFG_FORMAT_1m:
        return IDR_MCDA_PRFCFG_0m;
...
#define SOC_MEM_FORMAT_TO_ORIG(unit,mem)         ( mem = SOC_MEM_IS_VALID(unit, soc_mem_format_map(unit,mem)) ? \
                                                  soc_mem_format_map(unit,mem) : mem )
#else
#define SOC_MEM_FORMAT_TO_ORIG(unit,mem)
#endif
...
soc_mem_cache_get(int unit, soc_mem_t mem, int copyno)
{
    ...
    SOC_MEM_FORMAT_TO_ORIG(unit,mem);
    assert(SOC_MEM_IS_VALID(unit, mem));
...
soc_mem_dmaable(int unit, soc_mem_t mem, int copyno)
{
    ...
    if(SOC_IS_ARAD(unit)) {
#ifndef PLISIM
        if ((SOC_BLOCK_TYPE(unit, copyno) == SOC_BLK_SCH)) {
            return FALSE;
        }
        return TRUE;
#else
                  ...
                  uint32 ser_flags, void *buffer)
{
    ...
    if (rv < 0) {
        if (rv != SOC_E_TIMEOUT) {
            ...
            soc_cm_debug(DK_ERR, "TableDmaTimeOut:%s, %s.%s %s timeout\n",
                         FUNCTION_NAME(), SOC_MEM_UFNAME(unit, mem),
                         SOC_BLOCK_NAME(unit, copyno),
    ...
    if (!flags || (flags & SOC_MEM_WRITE_STATUS_ONLY)) {
        ...
        if (rv < 0) {
            if (rv != SOC_E_TIMEOUT) {
                ...
                soc_cm_debug(DK_ERR, "SlamDmaTimeOut:%s, %s.%s %s timeout\n",
                             FUNCTION_NAME(), SOC_MEM_UFNAME(unit, mem),
                             SOC_BLOCK_NAME(unit, copyno),
              ...
              int copyno, int index, void *entry_data)
{
    ...
    if (!(soc_feature(unit, soc_feature_field_stage_lookup_512_half_slice) ||
          soc_feature(unit, soc_feature_field_stage_lookup_256_half_slice) ||
    ...
    if (soc_feature(unit, soc_feature_field_stage_quarter_slice)) {
        ...
        } else if (mem == FP_GLOBAL_MASK_TCAMm || mem == FP_TCAMm || mem == VFP_TCAMm) {
             if ((SOC_IS_TRIUMPH3(unit)) && (((index/128) % 2) ||
                                             ((index/64) % 2))) {
                 return SOC_E_NONE;
             }
            ...
            if((SOC_MEM_INFO(unit, mem).flags & SOC_MEM_FLAG_CAM) &&
               (!(SOC_MEM_INFO(unit, mem).flags & SOC_MEM_FLAG_EXT_CAM))) {
                ...
                if (corrupt != NULL) {
                    ...
                    soc_cm_debug(DK_VERBOSE | DK_SOCMEM, "Unit:%d Mem[%s] index[%d] "
    ...
    if (soc_cm_shared_good_range(p)) {
        if (SHARED_GOOD_START(p)  && SHARED_GOOD_END_DEBUG(p)) {
            length = soc_mem_entry_bytes(unit, mem) * 
                     (index_max - index_min + 1);
            if (length > p->size) {
                soc_cm_debug(DK_WARN, "Some problem in calling :%s::%d:"
               ...
               int index, void *entry_data)
{
    ...
    /*Legacy no-cache input handling*/
    if (index < 0) {
        no_cache = 1;
        index = -index;
    }
    ...
    if (SOC_IS_KATANA2(unit) &&
        ...
        (mem == ING_PHYSICAL_PORT_TABLEm)) {
         ...
         soc_reg_field_set(unit, ING_Q_BEGINr, &rval, 
             ING_PHYSICAL_PORT_SBUS_WITH_PKT_DISABLEf , 1);
         WRITE_ING_Q_BEGINr(unit,rval);
         soc_cm_debug(DK_VERBOSE, "WRITE PKT_DISABLE=1\n");
         rv = soc_mem_array_write(unit, mem, 0, copyno, index, entry_data);
         soc_reg_field_set(unit, ING_Q_BEGINr, &rval, 
             ING_PHYSICAL_PORT_SBUS_WITH_PKT_DISABLEf , 0);
         WRITE_ING_Q_BEGINr(unit,rval);
         soc_cm_debug(DK_VERBOSE, "WRITE PKT_DISABLE=0\n");
    } else 
#endif
    {
        rv = soc_mem_array_write(unit, mem, 0, copyno, index, entry_data);
                ...
                if (SOC_FAILURE(rc)) { /* Case where correction has not yet happened */
                    if (rc == SOC_E_NOT_FOUND) {
                        ...
                        if (!SOC_IS_TD2_TT2(unit)) {
                            ...
                            soc_cm_debug(DK_VERBOSE, "Check at index: %d\n", 
                                         index);
                            entry_base = index;
                        }
    ...
    if (soc_feature(unit, soc_feature_shared_hash_ins) && 
        ...
        for (bix = 0; bix < num_banks; bix++) {
            ...
            rv1 = soc_mem_generic_lookup(unit, mem, copyno, bid,
                                         entry, NULL, &index);
            soc_cm_debug(DK_VERBOSE+DK_SOCMEM, "Unit:%d Sh-Ins-Lukp: bank[%d] "
                         "rv[%d] index[%d]\n", unit, bid, rv1, index);
            if (SOC_FAILURE(rv1)) {
            ...
            soc_cm_debug(DK_VERBOSE+DK_SOCMEM, "Unit:%d Sh-Ins-Lukp: "
                         "replace at index:%d\n", unit, index);
            break;
        }
        if (!ok) {
                          ...
                          int recurse_depth)
{
    ...
    for (bix = 0; bix < num_banks; bix++) {
        ...
        for (i = 0; i < num_ent;) {
            ...
            soc_cm_debug(DK_VERBOSE, "  Insert successfully\n");
            /* Entry was moved successfully. */
            found = TRUE;
            /* Delete old entry from original location */ 
            soc_cm_debug(DK_VERBOSE, 
                  ...
                  int count, int flags)
{
    ...
    if (rv == SOC_E_TIMEOUT) {
        soc_cm_debug(DK_ERR, "CcmDmaTimeout: unit %d, ccm_dma timeout\n", unit);
...
soc_mem_config_save(int unit, char *config_str, char *config_value)
{
     ...
     if (soc_mem_config_set != NULL) {
         ...
         if (config_value != NULL) {
             soc_cm_print("%s=%s #Add in config.bcm\n",
