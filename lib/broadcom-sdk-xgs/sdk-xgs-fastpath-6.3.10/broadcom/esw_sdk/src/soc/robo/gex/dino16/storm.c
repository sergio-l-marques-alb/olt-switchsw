...
#include <soc/drv.h>
#include <soc/debug.h>
#include <gex/dino16/robo_dino16.h>

/*
 *  Function : _drv_dino16_storm_control_type_enable_set
 *
 *  Purpose :
 ...
 */
static int 
_drv_dino16_storm_control_type_enable_set(int unit, uint32 type, 
    uint32 enable)
{
    uint32  temp = 0;
    uint32  reg_value;
    ...
    if (type & DRV_STORM_CONTROL_BCAST) {
        if (enable) {
            temp |= DINO16_STORM_SUPPRESSION_BROADCAST_MASK;
        } else {
            temp &= ~DINO16_STORM_SUPPRESSION_BROADCAST_MASK;
        }
    }
    if (type & DRV_STORM_CONTROL_MCAST) {
        if (enable) {
            temp |= DINO16_STORM_SUPPRESSION_MULTICAST_MASK;
        } else {
            temp &= ~DINO16_STORM_SUPPRESSION_MULTICAST_MASK;
        }
    } 
    if (type & DRV_STORM_CONTROL_DLF) {
        if (enable) {
            temp |= DINO16_STORM_SUPPRESSION_DLF_MASK;
        } else {
            temp &= ~DINO16_STORM_SUPPRESSION_DLF_MASK;
        }
    }
...
/*
 *  Function : _drv_dino16_storm_control_type_enable_get
 *
 *  Purpose :
 ...
 */
static int 
_drv_dino16_storm_control_type_enable_get(int unit, uint32 *type)
{
    uint32  temp = 0;
    uint32  reg_value;
    ...
    if (temp & DINO16_STORM_SUPPRESSION_BROADCAST_MASK) {
        *type |= DRV_STORM_CONTROL_BCAST;
    }

    if (temp & DINO16_STORM_SUPPRESSION_MULTICAST_MASK) {
        *type |= DRV_STORM_CONTROL_MCAST;
    }

    if (temp & DINO16_STORM_SUPPRESSION_DLF_MASK) {
...
/*
 *  Function : drv_dino16_storm_control_enable_set
 *
 *  Purpose :
 ...
 */
int 
drv_dino16_storm_control_enable_set(int unit, uint32 port, uint8 enable)
{
    uint32  temp;
    uint32  reg_value;

    soc_cm_debug(DK_PORT, "drv_dino16_storm_control_enable_set: \
...
/*
 *  Function : drv_dino16_storm_control_enable_get
 *
 *  Purpose :
 ...
 */
int 
drv_dino16_storm_control_enable_get(int unit, uint32 port, uint8 *enable)
{
    uint32  temp = 0;
    uint32  reg_value;
    ...
    *enable = temp;
    ...
    soc_cm_debug(DK_PORT, "drv_dino16_storm_control_enable_get: \
        unit = %d, port = %d, %sable\n", unit, port, (*enable) ? "en" : "dis");
...
/*
 *  Function : drv_dino16_storm_control_set
 *
 *  Purpose :
 ...
 */
int 
drv_dino16_storm_control_set(int unit, soc_pbmp_t bmp, uint32 type, 
    uint32 limit, uint32 burst_size)
{
    ...
    soc_cm_debug(DK_PORT, "drv_dino16_storm_control_set: \
    ...
    */
    ...
    PBMP_ITER(bmp, port) {
        if (limit == 0) {
            ...
            disable_type = DINO16_STORM_CONTROL_PKT_MASK;

            /* Set storm suppression type */
            SOC_IF_ERROR_RETURN(_drv_dino16_storm_control_type_enable_set
                (unit, type, TRUE));
...
/*
 *  Function : drv_dino16_storm_control_get
 *
 *  Purpose :
 ...
 */
int 
drv_dino16_storm_control_get(int unit, uint32 port, uint32 *type, 
    uint32 *limit, uint32 *burst_size)
{
    uint32  reg_value, temp = 0;

    SOC_IF_ERROR_RETURN(_drv_dino16_storm_control_type_enable_get
        (unit, type));
    ...
    soc_cm_debug(DK_PORT, "drv_dino16_storm_control_get: \
