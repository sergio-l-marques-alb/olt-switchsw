...
#endif /* BCM_TB_SUPPORT */
    soc_cm_debug(DK_ARL, "ARL FS status init!\n");
}
    ...
    if (frozen) {
        if (SOC_ROBO_ARL_FS_IS_INIT(frozen_search_status)) {
            ...
            if (search_op == _ARL_SEARCH_VALID_OP_DONE && 
                        op_status == SOC_E_NONE){
                frozen_action_l2_index++ ;
                soc_cm_debug(DK_ARL, "%s,L2 FS SYNC count=%d\n",
...
status_change:
    ...
    soc_cm_debug(DK_ARL, "%s,L2 FS status set to %d!\n", 
    ...
    if (frozen) {
        if (SOC_ROBO_ARL_FS_IS_INIT(frozen_scan_status)) {
            ...
            if (scan_index == max_index){
                ...
                soc_cm_debug(DK_ARL, "%s,L2 FS SYNC(Scan Valid) count=%d\n",
...
status_change_tb:
    frozen_scan_status++;   
    soc_cm_debug(DK_ARL, "%s,L2 FS status(Scan Valid) set to %d!\n", 
...
cleanup_exit:
    ...
    soc_cm_debug(DK_ARL, "soc_arl_thread_tb: exiting\n");
