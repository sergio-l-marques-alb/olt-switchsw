 ...
 * Debug Macros
 */
#define BCMX_OUT(_stuff)  BCMX_DEBUG(BCMX_DBG_NORMAL, _stuff)
...
_bcmx_port_add(int unit, bcm_port_t port, bcm_gport_t lport, uint32 flags)
{
    ...
    if (_bcmx_uport_add(uport, lport) == NULL) {
        BCMX_DEBUG(BCMX_DBG_WARN, ("BCMX: Failed to add uport hash, "
                                   "u %d, p %d\n", unit, port));
    }
    ...
    if (_bcmx_devport_hash_add(unit, port, port_info) == NULL) {
        BCMX_DEBUG(BCMX_DBG_WARN, ("BCMX: Failed to add devport hash, "
                                   "u %d, p %d\n", unit, port));
    }
...
bcmx_device_attach(int bcm_unit)
{
    ...
    rv = bcm_port_config_get(bcm_unit, &config);
    if (rv < 0) {
        BCMX_DEBUG(BCMX_DBG_WARN, ("BCMX: Could not get port config %d: %s\n",
                    rv, bcm_errmsg(rv)));
        return rv;
    }
    ...
    BCM_PBMP_ITER(config.all, port) {
        /*
         * Set 'lport' value to use gport format.  When a device
         * does not have a module it, bcm_port_gport_set will fail.
         * In this case, use DEVPORT format.
         *
         * The gport 'lport' formats would be of type:
         *     MODPORT  for devices with module-id
         *     DEVPORT  for devices with no module-id
         */
        if (BCM_FAILURE(bcm_port_gport_get(bcm_unit, port, &lport[i]))) {
            BCM_GPORT_DEVPORT_SET(lport[i], bcm_unit, port);
        }
    ...
    BCM_PBMP_ITER(config.all, port) {
        ...
        if (BCM_FAILURE(rv)) {
            BCMX_DEBUG(BCMX_DBG_WARN,
                       ("BCMX: Failed to add lport 0x%x (unit %d, port %d). "
                        "%d: %s.\n",
    ...
    if (rv < 0) {
        BCMX_DEBUG(BCMX_DBG_WARN, ("BCMX: Failed to add unit to RX, "
                    "u %d.  %d: %s\n", bcm_unit, rv, bcm_errmsg(rv)));
    }

    rv = bcmx_l2_device_add(bcm_unit);
    if (rv < 0 && rv != BCM_E_UNAVAIL) {
        BCMX_DEBUG(BCMX_DBG_WARN, ("BCMX: Failed to add unit for L2 notify, "
                    "u %d.  %d: %s\n", bcm_unit, rv, bcm_errmsg(rv)));
    }

    rv = bcmx_linkscan_device_add(bcm_unit);
    if (rv < 0) {
        BCMX_DEBUG(BCMX_DBG_WARN, ("BCMX: Failed to add unit for linkscan, "
                    "u %d.  %d: %s\n", bcm_unit, rv, bcm_errmsg(rv)));
    }
...
bcmx_device_detach(int bcm_unit)
{
    ...
    BCMX_DEBUG(BCMX_DBG_VERBOSE, ("BCMX: Detaching unit %d\n", bcm_unit));
    ...
    if ((rv = bcm_rlink_device_detach(bcm_unit)) < 0) {
        BCMX_DEBUG(BCMX_DBG_WARN,
                   ("BCMX(unit %d): Rlink device detach failed: %d\n",
                    bcm_unit, rv));
    }
#endif
...
_bcmx_port_info_dump(void)
{
    ...
    for (i  = 0; i < BCMX_PORT_HASH_COUNT; i++) {
        ...
        /* Check each entry in link list */
        while (port_info != NULL) {
            BCMX_OUT((" %d: lport=0x%x unit=%d port=%d uport=%d flags=0x%x "
                      "modid=%d modport=%d\n",
                      i, port_info->lport,
