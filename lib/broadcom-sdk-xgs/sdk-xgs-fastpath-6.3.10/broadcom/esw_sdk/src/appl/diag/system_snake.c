/*
 * $Id: system_snake.c,v 1.39 Broadcom SDK $
...
ss_unit_setup(system_snake_opts_t *opts, int unit)
{
    ...
    if (opts->verbose) {
        printk("Configuring Ethernet ports on unit %d (%s)\n", 
    ...
    if (soc_property_get_str(unit, spn_SS_IGNORE_PBMP)) {
        ...
        printk("Removed ignore_pbm from valid ports \n");
    }
    DPORT_BCM_PBMP_ITER(unit, ss_pbm, dport, port) {
        if (opts->verbose) {
            printk("\tConfiguring port %s\n", BCM_PORT_NAME(unit, port));
        }
...
ss_port_interconnect_execute(system_snake_opts_t *opts)
{
    ...
    for (snake = 0; snake < snake_num; snake++) {
        if (opts->verbose) {
            printk("Executing connections for snake %d\n", snake);
        }
...
ss_packet_alloc(system_snake_opts_t *opts, int unit)
{
    ...
    if (!ssi.lw[unit].rx_pkts) {
        ...
        printk("Unable to allocate rx packet memory\n");
        return BCM_E_MEMORY;
    }
    ...
    if (opts->verbose) {
        printk("%d Tx packet match markers for unit %d\n", num_packets, unit);
    }
...
system_snake_init(system_snake_opts_t *opts)
{
    ...
    if (opts->verbose) {
        printk("Initializing system snake\n");
    }
    ...
    SS_UNIT_ITER(sysid, unit) {
        ...
        if (diag_rc_load(unit) != CMD_OK) {
            printk("Test: ERROR: RC init script "
        ...
        if ((init_soc_file = ss_init_file[sysid]) &&
            ...
            printk("Test: ERROR: %s init script "
                   "for system snake test failed\n", SS_SOC_SCRIPT);
            return (-1);
        }
#endif
        ...
        ssi.stats[unit] =
            sal_alloc(SOC_MAX_NUM_PORTS * sizeof(lb2_port_stat_t),
                      "SS statistics");
        if (!ssi.stats[unit]) {
            printk("Unable to allocate statistics memory\n");
            return BCM_E_MEMORY;
        }
     ...
     */
    if (opts->mode == SS_MODE_EXT) {
        printk("Checking external links... ");
        SS_UNIT_ITER(sysid, unit) {
...
system_snake_stats(int unit, bcm_pbmp_t bitmap, lb2_port_stat_t stats[])
{
    ...
    return lbu_snake_stats(unit, bitmap,
                           (SOC_IS_XGS(unit) || SOC_IS_SIRIUS(unit)) ? LB2_MODE_EXT : LB2_MODE_PHY,
                           stats);
}
...
system_snake_run(system_snake_opts_t *opts)
{
    ...
    for (i = 0; i < ssi.io_unit_num; i++) {
        ...
        if (opts->verbose) {
            printk("\t%d packets to unit %d\n", num_packets, io_unit);
        }

        for (ix = 0; ix < num_packets; ix++ ) {
            if ((rv = bcm_tx(io_unit,
                             ssi.lw[io_unit].tx_pkts[ix], NULL)) < 0) {
                printk("System snake TX pkt %d: ERROR: bcm_tx: %s\n", 
                       ix, bcm_errmsg(rv));
            }
        }
    }

    printk("Running (%d seconds): Speed=%s Mode=%s\n",
           opts->duration, 
    ...
    while (timer < opts->duration) {
        ...
        timer += sleep_time;
        printk("Time elapsed:  %d seconds %s\n", timer,
    ...
    for (i = 0; i < ssi.io_unit_num; i++) {
        ...
        if (opts->verbose) {
            printk("Retrieving packets from unit %d\n", io_unit);
        }
        ...
        if (opts->verbose) {
            printk("Checking packets received from unit %d\n", io_unit);
        }
...
system_snake_done(system_snake_opts_t *opts)
{
    ...
    for (ix = 0; ix < ssi.io_unit_num; ix++) {
        ...
        if (opts->verbose) {
            printk("Unregistering Rx callback on unit %d\n", unit);
        }

        if ((rv = bcm_rx_unregister(unit, lbu_rx_callback,
                                    BCM_RX_PRIO_MAX)) < 0) {
            printk("Failed to unregister RX handler.\n");
        }

        if (opts->verbose) {
            printk("Stopping Rx on unit %d\n", unit);
        }

        if ((rv = bcm_rx_stop(unit, NULL)) < 0) {
            printk("system_snake_done: could not stop packet driver: %s\n",
                   bcm_errmsg(rv));
        }
...
cmd_ss(int unit, args_t *args)
{
    ...
    if (sysid == SS_SYSID_NUM) {
        printk("This test only runs on certain platforms.\n");
        printk("One of the following config variables must be\n");
        printk("set to indicate the platform:\n");
        for (sysid = 0; sysid < SS_SYSID_NUM; sysid++) {
            printk("   %s\n", ss_sysid_str[sysid]);
        }
    ...
    if (ARG_CNT(args) > 0) {
        printk("%s: Unrecognized extra argument: %s\n",
    ...
    if (sysid ==  SS_SYSID_FIREBOLT) {
        if (opts.mode != SS_MODE_EXT) {
            printk("%s Mode not supported on %s platform. Use tr 39 instead.\n",
...
int _diag_system_snake_not_empty;
