...
mcstimestamp_cmd(int unit, args_t *a)
{
    ...
    printk(" #  Last Timestamp    Prev Timestamp    <difference>  Equivalent TS1     Equivalent Full Time\n");
    ...
    for (i = min_event_id; i <= max_event_id; ++i) {
        ...
        if (SOC_FAILURE(rv)) {
            printk("soc_cmic_uc_msg_timestamp_get failed: %s (%d)\n", soc_errmsg(rv), rv);
        } else {
            diff = ts_data.hwts;
            ...
            printk("%2d: %08x:%08x %08x:%08x <%10u>  %08x:%08x  %s.%09u\n",
                   i,
