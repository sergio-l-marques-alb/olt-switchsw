/*
 * $Id: port.c,v 1.292 Broadcom SDK $
...
_phy_sym_debug_server(void *unit)
{
    ...
    while (1) {
        ...
        if (buffer.op != MDIO_HANDSHAKE) {
            printk("Unknown connection, Handshake not "
        ...
        while (1) {
            ...
            if ((n < 0) || (n < PHY_SYM_DBG_MDIO_MSG_LEN)) {
                printk("MDIO message corrupted or garbage read"
...
brief_port_info(char *port_ref, bcm_port_info_t *info, uint32 flags)
{
    ...
    _CHECK_PRINT(flags, BCM_PORT_ATTR_SPEED_MASK,
                 "%5s ", _if_fmt_speed(sbuf, info->speed));
    ...
    if (ARG_CNT(a) == 0) {
        ...
        if (!sal_strcasecmp(_ARG_CUR(a), "detach")) {
            ...
            DPORT_BCM_PBMP_ITER(u, pbm, dport, p) {
                ...
                if (rv < 0) {
                    printk("Unit:%d Port:%d :Port control lanes set failed"
                            "with %d",u, p, rv);
                    break;
                }
            }
            ...
            if ((c = ARG_CUR(a)) != NULL) {
                ...
                DPORT_SOC_PBMP_ITER(u, pbm, dport, p) {
                    ...
                    if (flags & 0x8) {
                        if (sal_strcasecmp(stats, "get") == 0) {
                            ...
                            printk("Tx events = %d TX Duration = %d(us) "
                ...
                DPORT_BCM_PBMP_ITER(u, pbm, dport, p) {
                    ...
                    printk("%5s(%3d) %9s %14u %14u %12u %14u %12u %14u\n",
                           SOC_PORT_NAME(u, p), p,
...
#define _call_pdi(u, p, mp, s) \
    ...
    DPORT_BCM_PBMP_ITER(u, pbm, dport, p) {
        ...
        if ((rv = soc_port_ability_to_mode(&info_given.port_ability,
            ...
            printk("port parse: Error: Could not "
...
{
    char *subcmd, *c;
    int port, arg_port = -1, min_port = 0, max_port = 31;
...
_phy_diag_mfg(int unit, bcm_pbmp_t pbmp, args_t *args)
{
    ...
    switch (test) {
        ...
        case 0:
            ...
            break;
        default:
            printk("Test should be : 1 (HYB_CANC), 2 (DENC), "
                   "3 (TX_ON) or 0 (EXIT)\n");
            return CMD_FAIL;
            break;
    }
...
_phy_diag_state(int unit, bcm_pbmp_t pbmp, args_t *args)
{
    ...
    switch (test) {
        ...
        case 5:
            ...
            break;    
        default:
            printk("Test should be : 1 (STATE_TRACE1), 2 (STATE_TRACE2), "
                   "3 (WHERE_AM_I), 4 (TEMP)\n");
            return CMD_FAIL;
            break;
    }
...
_phy_diag_eyescan(int unit, bcm_pbmp_t pbmp, args_t *args)
{
    ...
    if (ports == NULL) {
        printk("ERROR, in phy_diag_eyescan: failed to allocate ports");
        sal_free(results);
        return CMD_FAIL;
    }
    local_lane_num =
    ...
    if (local_lane_num == NULL) {
        printk("ERROR, in phy_diag_eyescan: failed to allocate local_lane_num");
        sal_free(results);
        sal_free(ports);
        return CMD_FAIL;
    }
    ...
    if (lane_num == 0xff) {
        ...
        if (lane_num > 9) {
            ...
            sal_memcpy(local_lane_num, &lane_num, sizeof(int) * port_count);
        }
    }

    if (params.sample_time > params.time_upper_bound) {
        printk
            ("ERROR, in phy_diag_eyescan: sample time %d"
             " greater than upper bound time %d\n",
    ...
    for (i = 0; i < nof_ports; i++) {
        ...
        rv = soc_port_phy_eyescan_res_print(unit, speed, ports[i],
                                         ...
                                         mode)
{
    ...
    switch (mode) {
        ...
        case bcmPortPhyTimesyncEventMessageEgressModeReplaceCorrectionFieldOrigin:
            soc_cm_print
                ("Replace_Correctionfield_origin\n");
            break;
...
_print_inband_timesync_config(bcm_port_phy_timesync_config_t *conf)
{
    printk("InBand TimeStamp Sync (Y or N) - %s\n",
           conf->inband_ts_control.
           flags & SOC_PORT_PHY_TIMESYNC_INBAND_TS_SYNC_ENABLE ? "Yes" : "No");
    printk("InBand TimeStamp Delay Request (Y or N) - %s\n",
           conf->inband_ts_control.
           flags & SOC_PORT_PHY_TIMESYNC_INBAND_TS_DELAY_RQ_ENABLE ? "Yes" :
           "No");
    printk("InBand TimeStamp Pdelay Request (Y or N) - %s\n",
           conf->inband_ts_control.
           flags & SOC_PORT_PHY_TIMESYNC_INBAND_TS_PDELAY_RQ_ENABLE ? "Yes" :
           "No");
    printk("InBand TimeStamp Pdelay reSponse (Y or N) - %s\n",
           conf->inband_ts_control.
           flags & SOC_PORT_PHY_TIMESYNC_INBAND_TS_PDELAY_RESP_ENABLE ? "Yes" :
           "No");
    printk("InBand TimeStamp ID - 0x%04x\n", conf->inband_ts_control.resv0_id);
    printk("InBand TimeStamp ID CHhecK (Y or N) - %s\n",
           conf->inband_ts_control.
           flags & SOC_PORT_PHY_TIMESYNC_INBAND_TS_RESV0_ID_CHECK ? "Yes" :
           "No");
    printk("InBand TimeStamp ID UPDate (Y or N) - %s\n",
           conf->inband_ts_control.
           flags & SOC_PORT_PHY_TIMESYNC_INBAND_TS_RESV0_ID_UPDATE ? "Yes" :
           "No");
...
_print_heartbeat_ts(int unit, bcm_port_t port)
{
    ...
    if (rv != BCM_E_NONE) {
        printk("bcm_port_control_phy_timesync_get "
...
if_esw_phy(int u, args_t *a)
{
...
#endif
            printk("%5s(%3d) %5x %5x %5x %5x %23s %10d %s \n",
    ...
    if (c != NULL && sal_strcasecmp(c, "eee") == 0) {
        ...
        uint32 latency, idle =
            0, tx_events, tx_duration, rx_events, rx_duration;
        uint32 flags;
        int i;
        parse_table_t pt;
                ...
                if (flags & 0x8) {
                    if (sal_strcasecmp(stats, "get") == 0) {
                        ...
                        (void)bcm_port_phy_control_get(u, p,
                                                       ...
                                                       &rx_duration);
                        printk("Port %s Tx events = %d TX Duration = %d "
                               "RX events = %d RX Duration = %d\n", 
        ...
        if ((c = ARG_CUR(a)) != NULL) {
            ...
            uint32 enable, capture_ts, heartbeat_ts, rx_crc, as, l2, ip4, ip6, ec,      /* bool */
             itpid, otpid, tx_offset, rx_offset, original_timecode_seconds,
                ...
                flags_2, ibts_sync = 0;
            uint32 ibts_dreq = 0, ibts_pdreq = 0, ibts_pdrsp =
                0, ibts_resv_id_chk = 0, ibts_resv_id_upd = 0, ibts_resv_id = 0;
            ...
            DPORT_SOC_PBMP_ITER(u, pbm, dport, p) {
                ...
                if ((rv =
                     bcm_port_phy_timesync_config_get(u, p,
                                                      &conf)) == BCM_E_FAIL) {
                    printk("bcm_port_phy_timesync_config_get() "
                           "failed, u=%d, p=%d\n", u, p);
                    return BCM_E_FAIL;
                }

                conf.validity_mask &=
                    ~BCM_PORT_PHY_TIMESYNC_VALID_PHY_1588_INBAND_TS_CONTROL;
                ...
                if (flags & (1U << 28)) {
                    conf.validity_mask |=
                        BCM_PORT_PHY_TIMESYNC_VALID_PHY_1588_INBAND_TS_CONTROL;
                    conf.inband_ts_control.flags &=
                        ~BCM_PORT_PHY_TIMESYNC_INBAND_TS_SYNC_ENABLE;
                    conf.inband_ts_control.flags |=
                        ibts_sync ? BCM_PORT_PHY_TIMESYNC_INBAND_TS_SYNC_ENABLE
                        : 0;
                }
                if (flags & (1U << 29)) {
                    conf.validity_mask |=
                    ...
                    conf.inband_ts_control.flags |=
                        ibts_dreq ?
                        BCM_PORT_PHY_TIMESYNC_INBAND_TS_DELAY_RQ_ENABLE : 0;
                }
                if (flags & (1U << 30)) {
                    conf.validity_mask |=
                        BCM_PORT_PHY_TIMESYNC_VALID_PHY_1588_INBAND_TS_CONTROL;
                    ...
                    conf.inband_ts_control.flags |=
                        ibts_pdreq ?
                        BCM_PORT_PHY_TIMESYNC_INBAND_TS_PDELAY_RQ_ENABLE : 0;
                }
                if (flags & (1U << 31)) {
                    conf.validity_mask |=
                        BCM_PORT_PHY_TIMESYNC_VALID_PHY_1588_INBAND_TS_CONTROL;
                    ...
                    conf.inband_ts_control.flags |=
                        ibts_pdrsp ?
                        BCM_PORT_PHY_TIMESYNC_INBAND_TS_PDELAY_RESP_ENABLE : 0;
                }
                if (flags_2 & (1U << 0)) {
                    conf.validity_mask |=
                        BCM_PORT_PHY_TIMESYNC_VALID_PHY_1588_INBAND_TS_CONTROL;
                ...
                if (flags_2 & (1U << 1)) {
                    ...
                    conf.inband_ts_control.flags |=
                        ibts_resv_id_chk ?
                        BCM_PORT_PHY_TIMESYNC_INBAND_TS_RESV0_ID_CHECK : 0;
                }
                if (flags_2 & (1U << 2)) {
                    conf.validity_mask |=
                        BCM_PORT_PHY_TIMESYNC_VALID_PHY_1588_INBAND_TS_CONTROL;
                    conf.inband_ts_control.flags &=
                        ~BCM_PORT_PHY_TIMESYNC_INBAND_TS_RESV0_ID_UPDATE;
                    conf.inband_ts_control.flags |=
                        ibts_resv_id_upd ?
                        BCM_PORT_PHY_TIMESYNC_INBAND_TS_RESV0_ID_UPDATE : 0;
                }
                if (flags_2 & (1U << 3)) {
                    conf.validity_mask |=
                        BCM_PORT_PHY_TIMESYNC_VALID_PHY_1588_INBAND_TS_CONTROL;
                ...
                if ((rv =
                     bcm_port_phy_timesync_config_set(u, p,
                                                      &conf)) == BCM_E_FAIL) {
                    printk("bcm_port_phy_timesync_config_set() "
                           "failed, u=%d, p=%d\n", u, p);
                    return BCM_E_FAIL;
                }
                if (flags & (1U << 26)) {
                    ...
                    rv = bcm_port_control_phy_timesync_set(u, p,
                                                           ...
                                                           val64);
                    if (rv != BCM_E_NONE) {
                        printk("bcm_port_control_phy_timesync_set failed "
...
#define FIRMWARE_BUF_LEN   0x80000
        ...
        if ((c = ARG_CUR(a)) != NULL) {
            ...
            parse_table_init(u, &pt);
            parse_table_add(&pt, "set", PQ_DFL | PQ_STRING, 0, &filename, NULL);
        ...
        if (!no_confirm) {
            ...
            if ((NULL ==
                ...
                sal_fclose(fp);
                printk("Firmware updating aborted. No writes to the "
        ...
        if ((c = ARG_CUR(a)) != NULL) {
            ...
            DPORT_SOC_PBMP_ITER(u, pbm, dport, p) {
                ...
                if (!(flags & (1U << 7)) &&
                    ...
                    printk(" MACAddrHi/MACAddrLow cannot be used without 'MACIndex. "
    ...
    if (c != NULL && sal_strcasecmp(c, "raw") == 0) {
        ...
        if ((c = ARG_GET(a)) == NULL) { /* Read register */
            if (is_c45) {
                rv = soc_miimc45_read(u, phy_addr, phy_devad, phy_reg,
                                      &phy_data);
                if (rv < 0) {
                    printk("ERROR: MII Addr %d: soc_miim_read failed: %s\n",
                           phy_addr, soc_errmsg(rv));
    ...
    if (ARG_CNT(a) == 0) {      /*  show information for all registers */
        DPORT_SOC_PBMP_ITER(u, pbm, dport, p) {
            phy_addr = (intermediate ?
                        PORT_TO_PHY_ADDR_INT(u, p) : PORT_TO_PHY_ADDR(u, p));
            if (phy_addr == 0xff) {
        ...
        if (ARG_CNT(a) == 0) {  /* no more args; show this register */
            DPORT_SOC_PBMP_ITER(u, pbm, dport, p) {
                ...
                if ((!intermediate) && (soc_phy_is_c45_miim(u, p))) {
                    ...
                    printk("Port %s (PHY addr 0x%02x) Reg 0x%04x: ",
                    ...
                    case COMBO_CMD_WATCH_ON:
                        if (!combo_watch[u][p]) {
                            r = bcm_port_medium_status_register(u, p,
                                                                ...
                                                                [p]);
                            if (r < 0) {
                                printk("Error registerinig medium "
                    ...
                    case COMBO_CMD_WATCH_OFF:
                        if (combo_watch[u][p]) {
                            r = bcm_port_medium_status_unregister(u, p,
                                                                  ...
                                                                  [u][p]);
                            if (r < 0) {
                                printk("Error unregisterinig medium "
                                       "status change callback for %s: %s\n",
                                        BCM_PORT_NAME(u, p),
                                        soc_errmsg(r));
                                return (CMD_FAIL);
...
config_i1e1_in(int unit, bcm_port_t port)
{
    ...
    if (rv != BCM_E_NONE) {
        printk("bcm_port_phy_timesync_config_get failed "
               "with error u=%d p=%d %s\n",
                unit, port, bcm_errmsg(rv));
    }
    ...
    if (rv != BCM_E_NONE) {
        printk("bcm_port_phy_timesync_config_set failed "
               "with error  u=%d p=%d %s\n",
                unit, port, bcm_errmsg(rv));
    }
}
...
void
config_tc5p1(int unit, bcm_port_t port)
{
    int rv;
    bcm_port_phy_timesync_config_t config;
...
void
config_tc6p1(int unit, bcm_port_t port)
{
    int rv;
    bcm_port_phy_timesync_config_t config;
...
void
config_tc7p1(int unit, bcm_port_t port)
{
    int rv;
    bcm_port_phy_timesync_config_t config;
...
void
config_tc8p1(int unit, bcm_port_t port)
{
    int rv;
    bcm_port_phy_timesync_config_t config;
...
void
config_tc5p1_fs(int unit, bcm_port_t port)
{
    int rv;
    bcm_port_phy_timesync_config_t config;
...
void
config_tc6p1_fs(int unit, bcm_port_t port)
{
    int rv;
    bcm_port_phy_timesync_config_t config;
...
void
config_tc7p1_fs(int unit, bcm_port_t port)
{
    int rv;
    bcm_port_phy_timesync_config_t config;
...
void
config_tc8p1_fs(int unit, bcm_port_t port)
{
    int rv;
    bcm_port_phy_timesync_config_t config;
...
print_local_time(int unit, bcm_port_t port)
{
    ...
    if (rv != BCM_E_NONE) {
        printk("bcm_port_control_phy_timesync_get failed "
               "with error  u=%d p=%d %s\n",
                unit, port, bcm_errmsg(rv));
    }
...
void
config_port_sync_1k(int unit, bcm_port_t port)
{
    int rv;
    bcm_port_phy_timesync_config_t config;
    ...
    config.flags =
        BCM_PORT_PHY_TIMESYNC_ENABLE | BCM_PORT_PHY_TIMESYNC_HEARTBEAT_TS_ENABLE |
...
phy_oam_y1731_rx(int unit, int port)
{
    ...
    if (rv != BCM_E_NONE) {
        printk("bcm_port_control_phy_oam_set failed "
