                 ...
                 uint64 *outval, uint32 flags)
{
    ...
    switch (regtype) {
        ...
        case soc_cosreg:
            ...
            if (r < 0) {
                if (!DIAG_DEBUG_CHECK(SOC_DBG_REG)){
...
cmd_robo_reg_edit(int unit, args_t *a)
{
    ...
    for (f = 0; f < (int)reginfo->nFields; f++) {
        ...
        if ((fld->len) > 32) {
#ifdef BE_HOST
            snprintf(dfl_str, sizeof(dfl_str), "0x%x%8x", *fld_ptr, *(fld_ptr+1));
#else
            snprintf(dfl_str, sizeof(dfl_str), "0x%x%8x", *(fld_ptr+1), *fld_ptr);
#endif
        } else {
            snprintf(dfl_str, sizeof(dfl_str), "0x%x", *fld_ptr);
        }
        
        snprintf(ans, sizeof(ans),        /* Also use ans[] for prompt */
            "  %s<%d", soc_robo_fieldnames[fld->field], fld->bp + fld->len - 1);
        if (fld->len > 1) {
            snprintf(ans + strlen(ans), (sizeof(ans) - strlen(ans)), ":%d", fld->bp);
        }
