...
init_symtab()
{
     ...
     */
    soc_cm_debug(DK_SYMTAB,
                 "symtab: init %d regs, %d symbols, %d linear, %d chained,\n"
            ...
            a->addr = (DRV_SERVICES(unit)->reg_addr)(unit, reg, port, idx);
...
#endif
                {
                    if (!soc_feature(unit, soc_feature_new_sbus_format)) {
                        ...
                        int blk;
                        uint8 at;
                        a->addr = soc_reg_addr_get(unit, reg, port, idx, &blk, &at);
                    }
                }
#endif
            }    
    ...
    if ((idx1 = sal_strchr(name, '[')) != 0) {
        ...
        *idx1++ = 0;
        if ((tmpchr = sal_strchr(idx1, ']')) == 0) {
            soc_cm_debug(DK_WARN, "Could not parse index in %s.\n", name);
        } else {
            *tmpchr = 0;
        }
 ...
 *
 * Prints a warning (DK_WARN) and returns -1 if unit does not support
 * the register.
 */
...
parse_symbolic_reference(int unit, soc_regaddrlist_t *alist, char *ref)
{
    ...
    soc_cm_debug(DK_SYMTAB, "parsing: %s\n", ref);
    ...
    if (!SOC_REG_IS_VALID(unit, reg)) {
        soc_cm_debug(DK_WARN, "Register %s %d is not valid for chip %s\n",
    ...
    if (!SOC_REG_IS_ENABLED(unit, reg)) {
        if (SOC_IS_ROBO(unit)){
#ifdef BCM_ROBO_SUPPORT
            soc_cm_debug(DK_WARN, "Register %s is not enabled for this configuration\n",
    ...
    if (diag_parse_range(idx1, idx2, &minidx, &maxidx,
        ...
        soc_cm_debug(DK_WARN, "Register %s: bad index specification\n", ref);
        goto _return_error;
    }

    soc_cm_debug(DK_SYMTAB, "symtab: index %d:%d type %d regblktype [ ",
                 minidx, maxidx, reginfo->regtype);
    while (regblktype[idx] != SOC_BLOCK_TYPE_INVALID) {
        soc_cm_debug(DK_SYMTAB, "%s ",
    ...
    case soc_phy_reg:
        ...
        soc_cm_debug(DK_SYMTAB+DK_VERBOSE, "symtab: port bitmap %s\n",
    ...
    case soc_ppportreg:
        ...
        soc_cm_debug(DK_SYMTAB+DK_VERBOSE, "symtab: ppport bitmap %s\n",
                     SOC_PBMP_FMT(bm, pfmt));
        switch (regblktype[0]) {
        case SOC_BLK_IPIPE:
    ...
    if (soc_cm_debug_check(DK_SYMTAB)) {
        int i;
