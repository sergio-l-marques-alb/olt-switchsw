/*
 * $Id: l3.c,v 1.105.6.1 Broadcom SDK $
...
_l3_cmd_alpm_debug_show(int unit, args_t *a)
{
    ...
    if (c != NULL && sal_strcasecmp(c, "all") == 0) {
        ARG_NEXT(a);
    } else {
        parse_table_init(unit, &pt);
...
_l3_cmd_alpm_find(int unit, args_t *a)
{
    ...
    printk("Matching <%d, %s/%d> ==> %s\n", vrf, ip_str, ip_len, 
           bcm_errmsg(rv));
    
    if (rv >= 0) {
        if (route.l3a_flags & BCM_L3_IP6) {
            printk("%-7s %-6s %-41s %-18s %-4s %4s %5s %1s %4s %4s %s\n", "#", 
                   "VRF", "Net-addr", "Next-Hop-Mac", "INTF", "MODID", "PORT", 
                   "PRI", "CLASS", "HIT", "VLAN");
        } else {
            printk("%-7s %-6s %-19s %-18s %-4s %4s %5s %1s %4s %4s %s\n", "#", 
                   "VRF", "Net-addr", "Next-Hop-Mac", "INTF", "MODID", "PORT", 
                   "PRI", "CLASS", "HIT", "VLAN");
        }
...
_l3_cmd_alpm_sanity_check(int unit, args_t *a)
{
    ...
    if (parse_memory_name(unit, &mem, mem_name, &copyno, 0) < 0) { 
        printk("Parse memory name failed\n");
        return CMD_USAGE;
    }

    printk("Processing ALPM sanity check for memory %s"
           " index %d\n", SOC_MEM_NAME(unit, mem), index);
    switch (mem) {
    case L3_DEFIPm:
        rv = soc_alpm_bucket_sanity_check(unit, mem, index);
        if (BCM_FAILURE(rv)) {
            printk("Bucket sanity check failed, mem %s "
                   "return %d\n", SOC_MEM_NAME(unit, mem), rv);
        }
        rv = soc_alpm_pivot_sanity_check(unit, mem, index);
        if (BCM_FAILURE(rv)) {
            printk("Pivot sanity check failed, mem %s "
                   "return %d\n", SOC_MEM_NAME(unit, mem), rv);
        }
        break;
    ...
    if (BCM_SUCCESS(rv)) {
        printk("Sanity check for memory %s index %d "
               "Passed\n", SOC_MEM_NAME(unit, mem), index);
    }
