/*
 * $Id: pcktwatch.c,v 1.66 Broadcom SDK $
...
typedef struct pup_s {
    ...
    eth_dv_t   *pup_robo_dv;   /* DMA descriptor, ROBO SOC mode only */
...
#   define	PU_CHANNEL(_c)	(1 << (_c))
    ...
    VOL pup_t	*pu_ch_active[N_DMA_CHAN+1]; /* Active DMA operations */
    int		pu_ch_count[N_DMA_CHAN+1]; /* # received on channel */
...
STATIC void             pw_robo_done_intr(int unit, eth_dv_t *dv);
#endif
...
pw_init(int unit)
{
    ...
    if (!pu->pu_sync || !pu->pu_lock || !pu->pu_spinlock) {
        printk("%s ERROR:  Could not allocate sync/lock\n", pw_name[unit]);
    }
    pu->chan = 0;
    pu->rx_pri = 100;
 ...
 */
{
...
#if defined(BCM_ESW_SUPPORT) || defined(BCM_SBX_CMIC_SUPPORT) || defined(BCM_ARAD_SUPPORT)
    if (SOC_IS_ESW(unit) || SOC_IS_SIRIUS(unit) || SOC_IS_CALADAN3(unit) || SOC_IS_ARAD(unit)) {
	...
	if (sal_sem_take(pu->pu_sync, sal_sem_FOREVER)) {
	    printk("%s: Failed to wait for start\n", pw_name[unit]);
	    return -1;
	}
    }
 ...
 */
{
    ...
    while (pu->pu_pid != SAL_THREAD_ERROR) {
	  /* Handle the case of pw_thread not exiting */
         if (soc_timeout_check(&to)) {
             soc_cm_debug(DK_ERR,
                  "pw_stop: pw_thread did not exit\n");
	...
	printk("%s: Termination requested...\n", pw_name[unit]);
    }
    return CMD_OK;
}
 ...
 */
{
    ...
    switch (pu->mode) {
        case PW_MODE_RX:
            ...
            if ((rv = bcm_rx_stop(unit, NULL)) < 0) {
                printk("PW stop error: Cannot stop RX: %s.\n",
                       bcm_errmsg(rv));
            }
            if ((rv = bcm_rx_queue_unregister(unit, BCM_RX_COS_ALL, 
                                              pw_rx_callback, pu->rx_pri))
                       < 0) {
                printk("PW stop error: Cannot unregister handler: %s.\n",
                       bcm_errmsg(rv));
            }
            PW_LOCK(unit);
	...
	pu->pu_log_free = pup->pup_next;
        if (pu->mode == PW_MODE_SOC) {
            if (SOC_IS_ROBO(unit)) {
#if defined(BCM_ROBO_SUPPORT)                
                soc_eth_dma_dv_free(unit, pup->pup_robo_dv);
#endif
            } else {
#if defined(BCM_ESW_SUPPORT) || defined(BCM_SBX_CMIC_SUPPORT) || defined(BCM_ARAD_SUPPORT)
	...
	soc_cm_debug(DK_VERBOSE, "PW-Daemon[%d]: Exiting\n", unit);
    }
    ...
    PW_UNLOCK(unit);   
    if (stat < 0) {
        soc_cm_debug(DK_ERR, "AbnormalThreadExit:%s\n", thread_name);  
    }
 ...
 */
{
    ...
    for (i = 0; i < pup_count; i++) {
        ...
        if (pu->mode == PW_MODE_SOC) {
            if (SOC_IS_ROBO(unit)) {
#if defined(BCM_ROBO_SUPPORT)                 
                pup->pup_robo_dv = soc_eth_dma_dv_alloc(unit, DV_RX, 1);
                if (pup->pup_robo_dv == NULL) {
                    pw_exit(unit, -1);
                }
#endif                
            } else {
    ...
    if (pu->pu_flags & PU_F_SYNC) {
        ...
        printk("PW-daemon[%d] -- Started\n", unit);
    }

    /* Kick Start channels */
    if ((rv = _pw_start_op(unit)) < 0) {
        printk("PW rx mode: Cannot start %s: %s.\n", pw_modes[pu->mode],
               bcm_errmsg(rv));
        pw_exit(unit, -1);
    }        
    ...
    while (1) {
        ...
        if (sem_rv < 0) {
            printk("Failed sem_take, exiting\n");
            pw_exit(unit, -1);
        }
 ...
 */
{
	...
	if (pu->pu_channel & (1 << i)) {
	    ...
	    if (rv < 0) {
		printk("%s: DMA channel configuration failed: %s\n", 
	...
	printk("%s: Unable to configure TX DMA channel: %s\n", 
	       pw_name[unit],
	       tx == -1 ? "No remaining channels" : soc_errmsg(rv));

	rv = soc_dma_init(unit);
	printk("%s: Unable to re-initialize DMA: %s\n", 
	       pw_name[unit],
	       soc_errmsg(rv));
 ...
 */
{
	...
	printk("%s: Status: %s. Mode %s. Buffering up to %d packets.%s\n",
	       pw_name[unit],
	...
	if (pu->mode == PW_MODE_SOC) {
            if (pu->pu_interval != 0) {
                printk("Interval %d us", pu->pu_interval);
            }
        }
        if (pu->rate <= 0) {
            printk("Rate limiting is off.\n");
        } else {
            printk("Rate limit is %d (soc intvl %d).\n", pu->rate,
                   pu->pu_interval);
        }
            
	printk("Reporting is enabled for: ");
	parse_mask_format(80, pw_report_table, pu->pu_report);
	printk("Reporting is disabled for: ");
	parse_mask_format(80, pw_report_table, pu->pu_report ^ ~0);

        printk("Dump options are enabled for: ");
	parse_mask_format(80, pw_report_table, pu->pu_dump_options);
	printk("Dump options are disabled for: ");
	parse_mask_format(80, pw_report_table, pu->pu_dump_options ^ ~0);

        printk("RX on for channel(s): ");
	if (pu->pu_channel) {
	    parse_mask_format(50, pw_channel_table, pu->pu_channel);
	    printk("RX off for channel(s): ");
	    parse_mask_format(50, pw_channel_table, ~pu->pu_channel);
	} else {
	    printk(" -- using default --\n");
	}
	...
	if (!sal_strcasecmp(c, "start")) {
	    ...
	    if (ARG_CNT(a) == 0) {
		printk("%s: RX on for channel(s): ", pw_name[unit]);
		if (pu->pu_channel) {
		    parse_mask_format(50, pw_channel_table, pu->pu_channel);
		    printk("%s: RX off for channel(s): ", pw_name[unit]);
		    parse_mask_format(50, pw_channel_table, ~pu->pu_channel);
		} else {
		    printk(" -- using default --\n");
		}
		...
		printk("%s: Unable to configure DMA channels "
		       "while running\n", pw_name[unit]);
		return(CMD_FAIL);
	    }
	    ...
	    if (pu->pu_flags & PU_F_RUN) {
		printk("%s: Warning channel re-assignment will not take\n"
		       "%s: until restarted.\n",
	    ...
	    c = ARG_GET(a);
            if (c == NULL) {
                if (pu->rate <= 0) {
                    printk("%s: No rate limiting.\n", pw_name[unit]);
                } else {
                    printk("%s: Rate limit is currently %d pkts/sec.\n",
		...
		printk("%s: Reporting on for: ", pw_name[unit]);
		parse_mask_format(50, pw_report_table, pu->pu_report);
		printk("%s: Reporting off for: ", pw_name[unit]);
		parse_mask_format(50, pw_report_table, ~pu->pu_report);
	    }
		...
		if (!sal_strcasecmp(c, "on")) {
		    ...
		    printk("%s: Invalid count \"%s\"\n", pw_name[unit], c);
		    return(CMD_FAIL);
		}
		pu->pu_log_max = n;
	    }
	    printk("%s: Logging(%d-packets)\n",
		...
		n = pu->pu_log_cnt;
                ...
                if (!sal_strcasecmp(c, "options")) {
                    if (ARG_CNT(a)) {
                        ...
                        printk("%s: Dump options on for: ", pw_name[unit]);
                        parse_mask_format(50, pw_report_table, pu->pu_dump_options);
                        printk("%s: Dump options off for: ", pw_name[unit]);
                        parse_mask_format(50, pw_report_table, ~pu->pu_dump_options);
                    }
                    return(CMD_OK);
                } else {
                    printk("%s: Invalid log count \"%s\"\n", pw_name[unit], c);
                    return(CMD_FAIL);
                }
            } else {
		n = parse_integer(c);
	    ...
	    printk("%s: Received %d packets [(%d),%d,%d,%d,%d] (%d/sec), "
	    ...
	    n = parse_integer(c);
	    if (n < 0) {
		printk("%s: Invalid interval: %s\n", pw_name[unit], c);
		return(CMD_FAIL);
	    }
	    pu->pu_interval = n;
	} else if (!sal_strcasecmp(c, "mode")) {
	    if ((c = ARG_GET(a)) == NULL) {
		soc_cm_print("Current mode is %s\n", pw_modes[pu->mode]);
		return CMD_OK;
	    }

	    if (pw_running(unit)) {
		soc_cm_print("Can't set modes while running\n");
		return CMD_FAIL;
	    }
 ...
 */
{
    ...
    char        pfx[64];
        ...
        if (SOC_IS_ROBO(unit)) {
#if defined(BCM_ROBO_SUPPORT)          
            robo_dcb = pup->pup_robo_dv->dv_dcb;
#endif
        } else {
    ...
    if (SOC_IS_ROBO(unit)) {
#if defined(BCM_ROBO_SUPPORT)        
        pw_dump_start(unit, pfx, pup, report, (int)pup->pup_robo_dv->dv_pw_chan, -1);
#endif
    } else {
#if defined(BCM_ESW_SUPPORT) || defined(BCM_SBX_CMIC_SUPPORT) || defined(BCM_ARAD_SUPPORT)
    ...
    if (report & REPORT_COUNT) {
        ...
        if (SOC_IS_ROBO(unit)) {
#if defined(BCM_ROBO_SUPPORT)            
            SOC_DCB_REASON_DUMP(unit, robo_dcb, pfx);
#endif
        } else {
#if defined(BCM_ESW_SUPPORT) || defined(BCM_SBX_CMIC_SUPPORT) || defined(BCM_ARAD_SUPPORT)
    ...
    if (report & REPORT_DMA) {
        if (SOC_IS_ROBO(unit)) {
#if defined(BCM_ROBO_SUPPORT)            
            soc_eth_dma_dump_dv(unit, pfx, pup->pup_robo_dv);
#endif
        } else {
#if defined(BCM_ESW_SUPPORT) || defined(BCM_SBX_CMIC_SUPPORT) || defined(BCM_ARAD_SUPPORT)
    ...
    if (report & REPORT_RAW) {
        if (SOC_IS_ROBO(unit)) {
#if defined(BCM_ROBO_SUPPORT)            
            soc_eth_dma_dump_pkt(unit, pfx,
                             (uint8 *)robo_dcb->dcb_vaddr,
                             robo_dcb->len);
#endif
        } else {
#if defined(BCM_ESW_SUPPORT) || defined(BCM_SBX_CMIC_SUPPORT) || defined(BCM_ARAD_SUPPORT)
    ...
    if (report & REPORT_DECODE) {
	    ...
	    d_packet_format(pfx, DECODE_ETHER, pup->pup_pkt,
			    SOC_DCB_XFERCOUNT_GET(unit, robo_dcb), NULL);
#endif
	 } else {
#if defined(BCM_ESW_SUPPORT) || defined(BCM_SBX_CMIC_SUPPORT) || defined(BCM_ARAD_SUPPORT)
            d_packet_format(pfx, DECODE_ETHER, pup->pup_pkt,
...
pw_dump_packet_rx(int unit, pup_t *pup, uint32 report)
{
    ...
    if (report & REPORT_DMA) {
        if (pkt->_dv) {
            if (SOC_IS_ROBO(unit)) {
#if defined(BCM_ROBO_SUPPORT)                
                soc_eth_dma_dump_dv(unit, pfx, (eth_dv_t *)pkt->_dv);
#endif
            } else {
#if defined(BCM_ESW_SUPPORT) || defined(BCM_SBX_CMIC_SUPPORT) || defined(BCM_ARAD_SUPPORT)
    ...
    if (report & REPORT_RAW) {
        ...
        if (SOC_IS_ROBO(unit)) {
#if defined(BCM_ROBO_SUPPORT)            
            soc_eth_dma_ether_dump(unit, pfx, pkt->_pkt_data.data, pkt->pkt_len, 0);
#endif
        } else {
#if defined(BCM_ESW_SUPPORT) || defined(BCM_SBX_CMIC_SUPPORT) || defined(BCM_ARAD_SUPPORT)
        ...
        printk("%slength %d (%d). rx-port %d. cos %d. prio_int %d. vlan %d. "
               "reason 0x%x. %s.\n", pfx,
        ...
        if ((pkt->flags & BCM_PKT_STK_F_SRC_PORT) && 
            pkt->flags & BCM_PKT_STK_F_DST_PORT ) {
            printk("%sdest-gport %d. src-gport %d. "
                   "opcode %d. %s matched %d. classification-tag %d.\n",
                   pfx, pkt->dst_gport, pkt->src_gport, pkt->opcode, 
                   (pkt->flags & BCM_RX_TRUNCATED) ? "Truncated." : "",
                   pkt->rx_matched, pkt->rx_classification_tag);
        } else if ((pkt->flags & BCM_PKT_STK_F_SRC_PORT)) {
            printk("%sdest-port %d. dest-mod %d. src-gport %d. "
                   "opcode %d. %s matched %d. classification-tag %d.\n",
                   pfx, pkt->dest_port, pkt->dest_mod,
                   pkt->src_gport, pkt->opcode,
                   (pkt->flags & BCM_RX_TRUNCATED) ? "Truncated." : "",
                   pkt->rx_matched, pkt->rx_classification_tag);
        } else if ((pkt->flags & BCM_PKT_STK_F_DST_PORT)) {
            printk("%sdest-gport %d. %s %d. src-mod %d. "
                   "opcode %d. %s matched %d. classification-tag %d.\n",
                   pfx, pkt->dst_gport, 
        ...
        printk("%sdest-port %d. dest-mod %d. %s %d. src-mod %d. "
               "opcode %d. %s matched %d. classification-tag %d.\n",
               pfx, pkt->dest_port, pkt->dest_mod,
               (pkt->flags & BCM_PKT_F_TRUNK) ? "src-trunk" : "src-port",
               (pkt->flags & BCM_PKT_F_TRUNK) ? pkt->src_trunk : pkt->src_port, 
 ...
 */
{
	...
	PW_UNLOCK(unit);
	printk("pw_dump_log[%d]: Warning: no packets logged "
	       "or selected to dump\n", unit);
 ...
 */
{
	...
	pw_log_packet(unit, (pup_t *)pup);
        if (pu->mode == PW_MODE_SOC) {
            if (SOC_IS_ROBO(unit)) {
#if defined(BCM_ROBO_SUPPORT)                
                pw_start_channel(unit, (dma_chan_t)pup->pup_robo_dv->dv_pw_chan);
                chan = (int)pup->pup_robo_dv->dv_pw_chan + 1;
#endif                
            } else {
#if defined(BCM_ESW_SUPPORT) || defined(BCM_SBX_CMIC_SUPPORT) || defined(BCM_ARAD_SUPPORT)
 ...
 */
{
    ...
    if (SOC_IS_ROBO(unit)) {
#if defined(BCM_ROBO_SUPPORT)        
        robo_dv = pup->pup_robo_dv;
        /* soc_robo_dma_dv_reset(DV_RX, robo_dv); */
        rv = soc_eth_dma_desc_add(robo_dv, (sal_vaddr_t)pup->pup_pkt,
                                   PW_PACKET_SIZE);
        if (rv < 0) {
        	pw_pup_free(unit, pup);
        	printk("Failed to DMA descriptor\n");
        }
        robo_dv->dv_done_chain = pw_robo_done_intr;
    
        robo_dv->dv_pw_chan = (uint32)chan;
        robo_dv->dv_pw_pup  = pup;
    
        pu->pu_ch_active[chan + 1] = pup;
    
        rv = soc_eth_dma_start(unit, robo_dv);
        if (rv < 0) {
        	pu->pu_ch_active[chan + 1] = NULL;
        	pw_pup_free(unit, pup);
        	printk("Failed to start DMA: %s\n", soc_errmsg(rv));
        }
#endif
    } else {
#if defined(BCM_ESW_SUPPORT) || defined(BCM_SBX_CMIC_SUPPORT) || defined(BCM_ARAD_SUPPORT)
 ...
 */
STATIC	void
pw_robo_done_intr(int unit, eth_dv_t *dv)
{
    pwu_t	*pu  = &pw_units[unit];
    pup_t	*pup = (pup_t *)dv->dv_pw_pup;
