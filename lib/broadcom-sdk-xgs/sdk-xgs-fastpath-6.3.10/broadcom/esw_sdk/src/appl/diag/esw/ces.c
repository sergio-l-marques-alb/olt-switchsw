...
void bcm_esw_show_header(bcm_ces_packet_header_t *header, bcm_ces_packet_header_t *strict) {
    ...
    printk("---- Packet Headers ----------------------------------\n");
    printk("Field                     Header                 Strict\n");
    printk("------------------------  ---------------------- ---------------------\n");
    printk("encapsulation           : %-18s     %-18s\n", 
	   bcm_diag_esw_ces_encap_string[header->encapsulation],
	   bcm_diag_esw_ces_encap_string[strict->encapsulation]);
    printk("a_source                : %02x:%02x:%02x:%02x:%02x:%02x      %02x:%02x:%02x:%02x:%02x:%02x\n",
    ...
    printk("a_destination           : %02x:%02x:%02x:%02x:%02x:%02x      %02x:%02x:%02x:%02x:%02x:%02x\n",
	...
	for (i = 0;i < n;i++) {
	    printk("vlan%d priority          : %d                      %d\n", 
		   i,
		   header->vlan[i].priority,
		   strict->vlan[i].priority);
	    printk("vlan%d vid               : 0x%02x                   0x%02x\n", 
		   i,
		   header->vlan[i].vid,
		   strict->vlan[i].vid);
	}
	...
	for (i = 0;i < n;i++) {
	    printk("mpls%d label              : 0x%08x             0x%08x\n", 
		   i,
		   header->mpls[i].label,
		   strict->mpls[i].label);
	    printk("mpls%d experimental       : 0x%08x             0x%08x\n", 
		   i,
		   header->mpls[i].experimental,
		   strict->mpls[i].experimental);
	    printk("mpls%d ttl                : 0x%08x             0x%08x\n", 
		   i,
		   header->mpls[i].ttl,
		   strict->mpls[i].ttl);
	}
    ...
    printk("n_label                 : %08x               %08x\n",
	   header->vc_label,
	   strict->vc_label);

    printk("n_ip_version            : %u                      %u\n",
	   header->ip_version,
	   strict->ip_version);
    if (header->ip_version == bcmCesIpV4 || strict->ip_version == bcmCesIpV4) {
	printk("ipv4.n_source           : %d.%d.%d.%d              %d.%d.%d.%d\n",
	...
	printk("ipv4.n_destination      : %d.%d.%d.%d              %d.%d.%d.%d\n",
	...
	for (i = 0;i < 16;i += 2) {
	    printk("%02x%02x", strict->ipv6.source[i], strict->ipv6.source[i + 1]);      /* IPv6 source address */
	    if (i != 14)
		printk(":");
	    else
		printk("\n");
	}
	printk("ipv6.destination        : ");
	for (i = 0;i < 16;i += 2) {
	    printk("%02x%02x", header->ipv6.destination[i], header->ipv6.destination[i + 1]);      /* IPv6 destination address */
	    if (i != 14)
		printk(":");
	    else
		printk("    ");
	...
	for (i = 0;i < 16;i += 2) {
	    printk("%02x%02x", strict->ipv6.destination[i], strict->ipv6.destination[i + 1]);      /* IPv6 destination address */
	    if (i != 14)
		printk(":");
	    else
		printk("\n");
    ...
    printk("udp.n_destination       : %08x               %08x\n",
	   header->udp.destination,
	   strict->udp.destination);
    printk("udp.n_source            : %08x               %08x\n",
	   header->udp.source,
	   strict->udp.source);
    printk("udp_chksum              : %d                      %d\n",
	   header->udp_chksum,
	   strict->udp_chksum);

    if (header->rtp_exists || strict->rtp_exists) {
	printk("rtp_exists              : %d                      %d\n",
	       header->rtp_exists,
	       strict->rtp_exists);
	printk("rtp_pt                  : %d                      %d\n",
	       header->rtp_pt,
...
int bcm_diag_esw_ces_cclk_config_set(int unit, bcm_ces_cclk_config_t *config)
{
	...
	printk("Invalid common clock source selection\n");
	return BCM_E_PARAM;
    }
	...
	printk("Invalid clock 1 source selection\n");
	return BCM_E_PARAM;
    }
	...
	printk("Invalid clock 2 source selection\n");
	return BCM_E_PARAM;
    }
	...
	if (config->ref_clk_1_port < 39 || config->ref_clk_1_port > 55) {
	    printk("c1_port out of range\n");
	    return BCM_E_PARAM;
	}
	if (config->ref_clk_2_port < 39 || config->ref_clk_2_port > 55) {
	    printk("c2_port out of range\n");
	    return BCM_E_PARAM;
	}
...
void bcm_ces_tdm_init(int unit) 
{
    ...
    if (ret != BCM_E_NONE)
    {
	printk("bcm_esw_port_tdm_init() failed with code:%d\n", ret);
    }

    printk("TDM module initialized\n");
}
#endif
...
void bcm_diag_ces_tdm_show_summary(int unit) {
    ...
    printk("F - Free  C - Configured  M - Modified  E - Enabled\n");
    printk("Str - Structured  Oct - Octet Aligned  Fra - Framed\n");
    printk("Sig - Signalling  Crx - Recieve Clock  Ctx - Transmit Clock\n");
    printk("FrM - Framer master\n\n");
    printk("Port F C M E Str Oct Fra Sig Crx Ctx FrM |      |  Timeslots    |       |\n");
    printk("---- - - - - --- --- --- --- --- --- --- --------------------------------\n");
	...
	printk("%4d %c %c %c %c %3c %3c %3c %3c %3d %3d %3c %s\n",
	       port,
...
void bcm_diag_esw_ces_framer_loopback(int unit, int port, int enable, int loopback) {
    ...
    default:
	printk("Unsupported loopback type:%d\n", loopback);
	return;
    }
    ...
    ret = bcm_esw_port_tdm_framer_port_loopback_set(unit,
						...
						0);
    if (ret != BCM_E_NONE) {
	printk("Framer loopback %s failed for port:%d loopback%d\n",
...
void bcm_diag_esw_ces_framer_pm(int unit, int *port) {
    ...
    if (ret != BCM_E_NONE) {
	printk("Framer pm failed for port:%d\n",
	       *port);
    } else {
	printk("---- Framer PM for Port:%d ----\n", *port);
	printk("n_crc_error_counter       : %d\n", ndFramerPortPm.n_crc_error_counter);
	printk("n_line_code_voi_error     : %d\n", ndFramerPortPm.n_line_code_voi_error);
	printk("n_e_bit_error_count       : %d\n", ndFramerPortPm.n_e_bit_error_count);
	printk("n_transmit_slip_count     : %d\n", ndFramerPortPm.n_transmit_slip_count);
	printk("n_recive_slip_count       : %d\n", ndFramerPortPm.n_recive_slip_count);
	printk("n_frame_alim_error_count  : %d\n", ndFramerPortPm.n_frame_alim_error_count);
	printk("n_change_framer_alim_count: %d\n", ndFramerPortPm.n_change_framer_alim_count);
	printk("n_sev_frame_error_count   : %d\n", ndFramerPortPm.n_sev_frame_error_count);
	printk("-------------------------------\n");
    }
}
#endif
...
int bcm_diag_ces_tdm_set(int unit, int *port, bcm_tdm_port_config_t *config) {
	...
	printk("Port is out of range, valid range is 39 through 54\n");
	return BCM_E_PARAM;
    }

    if (config->e_clk_rx_select > 1) {
	printk("Receive clock out of range\n");
	return BCM_E_PARAM;
    }
    if (config->e_clk_tx_select > 2) {
	printk("Tramsmit clock out of range\n");
	return BCM_E_PARAM;
    }
	...
	break;
    default:
	printk("Invalid receive clock selection\n");
	return BCM_E_PARAM;
	break;
    }
	...
	break;
    default:
	printk("Invalid transmit clock selection\n");
	return BCM_E_PARAM;
	break;
    }
    ...
    if (ret != BCM_E_NONE) {
	printk("bcm_tdm_port_config_set() failed with code:%d\n", ret);
    }
...
cmd_result_t cmd_esw_ces_tdm(int unit, args_t *args) {
	...
	if (ARG_CNT(args) >= 1)
	{
	    ...
	    {
		printk("Invalid CES tdm framer subcommand: %s\n", arg_string_p);
		return CMD_FAIL;
	    }
	}
	else
	{
	    printk("Insuficient number of arguments. Expecting one of loopback, status or pm.\n");
	    return CMD_FAIL;
	}
    }
#endif
	...
	if (ARG_CNT(args) > 0) {
		...
		printk("Expecting either abcd or change\n");
	    }
	} else {
	    printk("Expecting either abcd or change\n");
	}
    ...
    {
	printk("Invalid CES tdm subcommand: %s, expecing one of init, set, framer or show\n", 
	       arg_string_p);
	return CMD_FAIL;
    }
...
void cmd_esw_ces_rpc_show_flags(int unit, uint32 flags) {
	...
	printk("\tPolicy does not enable any counts\n");
    }
}
...
cmd_result_t cmd_esw_ces_rpc(int unit, args_t *args) {
	...
	printk("Expecting either get or set\n");
	ret = CMD_FAIL;
    }
...
int cmd_esw_ces_mii_get(int unit, bcm_ces_mac_cmd_config_t *config) {
    printk("\n\n");
    printk("b_rx_error_discard_enable         : %d\n", config->b_rx_error_discard_enable);
    printk("b_no_length_check                 : %d\n", config->b_no_length_check);
    printk("b_control_frame_enable            : %d\n", config->b_control_frame_enable);
    printk("b_node_wake_up_request_indication : %d\n", config->b_node_wake_up_request_indication);
    printk("b_put_core_in_sleep_mode          : %d\n", config->b_put_core_in_sleep_mode);         
    printk("b_enable_magic_packet_detection   : %d\n", config->b_enable_magic_packet_detection);  
    printk("b_software_reset                  : %d\n", config->b_software_reset);                 
    printk("b_is_late_collision_condition     : %d\n", config->b_is_late_collision_condition);    
    printk("b_is_excessive_collision_condition: %d\n", config->b_is_excessive_collision_condition);
    printk("b_enable_half_duplex              : %d\n", config->b_enable_half_duplex);              
    printk("b_insert_mac_addr_on_transmit     : %d\n", config->b_insert_mac_addr_on_transmit);     
    printk("b_ignore_pause_frame_quanta       : %d\n", config->b_ignore_pause_frame_quanta);       
    printk("b_fwd_pause_frames                : %d\n", config->b_fwd_pause_frames);               
    printk("b_fwd_crc_field                   : %d\n", config->b_fwd_crc_field);                   
    printk("b_enable_frame_padding            : %d\n", config->b_enable_frame_padding);            
    printk("b_enable_promiscuous_mode         : %d\n", config->b_enable_promiscuous_mode);         
    printk("b_enable_mac_receive              : %d\n", config->b_enable_mac_receive);              
    printk("b_enable_mac_transmit             : %d\n", config->b_enable_mac_transmit);   
    printk("\n\n");
...
int bcm_diag_esw_ces_liu_loopback_get(int unit, bcm_diag_ces_loopback_config_t *loopback_config) {
     ...
     */
    printk("     +---- Loopback -----+\n");
    printk("Port Analog Remote Digital\n");
    printk("---- ------ ------ -------\n");
...
int bcm_diag_esw_ces_liu_loopback_set(int unit, bcm_diag_ces_loopback_config_t *loopback_config) {
	...
	default:
	    printk("Invalid loopback type:%s, expecting either a|r|d\n", loopback_config->type);
	    return BCM_E_INTERNAL;
	}
...
int bcm_diag_esw_ces_liu_reg_check(int unit, bcm_diag_ces_reg_config_t *config) {
	...
	printk("Inavlid bank value:%d, valid values either 0=Primary, 1=Secondary, 2=Individual, 3=BERT\n", config->bank);
	return 0;
    }
	...
	printk("Invalid LIU:%d, valid values are between 0 and 15\n", config->liu);
	return 0;
    }
	...
	printk("Invalid write value:0x%x, valid values are between 0x00 and 0xFF\n", config->value);
	return 0;
    }
	...
	if ((config->address >= 0x16 && config->address <= 0x1E) ||
	    ...
	    config->address > 0x3F) {
	    printk("Invalid address:0x%x for bank:%d, valid values are between 0x00 and 0x15 or 0x20 and 0x35\n",
		   config->address,
		   config->bank);
	    return 0;
	}
	...
	if ((config->address >= 0x07 && config->address <=0x1E) ||
	    ...
	    config->address > 0x3F) {
	    printk("Invalid address:0x%x for bank:%d, valid values are between 0x00 and 0x07 or 0x20 and 0x27\n",
		   config->address,
		   config->bank);
	    return 0;
	}
...
int bcm_diag_esw_ces_liu_reg_write(int unit, bcm_diag_ces_reg_config_t *config) {
	...
	printk("Error: Incorrect value read back from device\n");
    } else {
    /*
     * Display the result
     */
...
cmd_result_t cmd_esw_ces_liu(int unit, args_t *args) {
	...
	if (ARG_CNT(args) >= 1) {
		...
		printk("Invalid CES LIU loopback subcommand: %s, expecting either get|set\n", arg_string_p);
		return CMD_FAIL;
	    }
	} else {
	    printk("Missing loopback subcommand, expecting either get|set\n");
	    return CMD_FAIL;
	}
    } else if (sal_strcasecmp(arg_string_p, "status") == 0) {
        /*
	...
	if (ARG_CNT(args) >= 1) {
		...
		printk("Invalid CES LIU reg subcommand: %s, expecting either read|write\n", arg_string_p);
		return CMD_FAIL;
	    }
	} else {
	    printk("Missing CES LIU reg subcommand, expecting either read|write\n");
	    return CMD_FAIL;
	}

    } else {
	printk("Invalid CES LIU subcommand: %s, expecting either reg, loopback, status, init or reset\n", arg_string_p);
	return CMD_FAIL;
    }
...
cmd_result_t cmd_esw_ces_diag(int unit, args_t *args) {
	...
	if (ARG_CNT(args) > 0) {
	    ...
	    printk("Expecting args\n");
	    return CMD_FAIL;
	}
    } else {
	printk("Missing diag subcommand, expecting either get|set\n");
	return CMD_FAIL;
    }
...
cmd_result_t bcm_diag_esw_ces_test_system(int unit, int time, uint8 *ports, int num_ports) {
     ...
     * Init ces
     */
    printk("Initializing CES (This may take a few seconds)\n");
     ...
     */
    printk("Initializing LIUs\n");
    if (bcm_diag_esw_ces_liu_init(unit, 1)!= BCM_E_NONE) { /* T1 mode */ 
	printk("LIU device failed to initialize\n");
        sal_free(map_config);
	return CMD_FAIL;
    }

    printk("Setting GE0 line loopback\n");
	...
	if (ports[i] > 7 && !lowDisable) {
	    lowDisable = 1;
	    printk("Disabling LIU ports 0-7\n");
	    for (j = 0;j < 8;j++) {
	...
	printk("LIU Port:%d system side PRBS test", ports[i]);
     ...
     * GE0 loopback
     */
    printk("Removing GE0 line loopback\n");
    READ_COMMAND_CONFIGr(unit, 1, &rval);
...
cmd_result_t bcm_diag_esw_ces_test_line(int unit, int time, char *loopback, uint8 *ports, int num_ports) {
	...
	printk("Setting digital loopback on LIUs\n");
	loopback_config.port = -1;
	...
	printk("/***********************************************/\n");
	printk("/*                                             */\n");
	printk("/*  Make sure that LIU ports have loopback     */\n");
	printk("/*  plugs in place or all tests will fail.     */\n");
	printk("/*                                             */\n");
	printk("/***********************************************/\n");
    }
	...
	printk("LIU Port:%d line side PRBS test", ports[i]);
...
cmd_result_t cmd_esw_ces_test(int unit, args_t *args) {
	...
	if (parse_arg_eq(args, &parse_table) < 0 || ARG_CNT(args) > 0) {
		...
		printk("Invalid loopback type, valid values are d or p\n");
		ret = CMD_FAIL;
	    } else {
	...
	printk("Missing test subcommand, expecting line | system\n\n");
...
void bcm_diag_esw_ces_service_show_summary(int unit) {
    ...
    printk("\nCES Service Summary\n");
    printk("F - Free  C - Configured  M - Modified  E - Enabled\n\n");
    printk("Srv F C M E | Srv F C M E | Srv F C M E | Srv F C M E\n");
    printk("--- - - - - + --- - - - - + --- - - - - + --- - - - -\n");
	...
	printk("%3d %c %c %c %c | %3d %c %c %c %c | %3d %c %c %c %c | %3d %c %c %c %c\n", 
	       i,
...
int bcm_diag_ces_service_set_header(int unit, bcm_ces_service_config_t *config, int service, int encapsulation) {
	...
	break;
    default:
	printk("Unsupported encapsulation:%d, valid values are 0 thorugh 3\n", encapsulation);
	return CMD_FAIL;
	break;
    }
...
void bcm_diag_ces_service_modify(int unit, int *service) {
	...
	{
	    printk("CES service:%d modify failed\n", *service);
	}
	else
	    printk("CES service:%d modified\n", *service);
    } else 
	printk("Invalid service:%d\n", *service);
}
...
int bcm_diag_ces_create(int unit, int *service, bcm_ces_channel_map_t *map_config, int encapsulation, int ip_version) {
	...
	printk("Invalid IP version:%d. Valid values are 4 and 6\n", ip_version);
        sal_free(config);
        sal_free(def_config);
	return CMD_FAIL;
    }
	...
	printk("Failed to get default config\n");
        sal_free(config);
        sal_free(def_config);
	return CMD_FAIL;
    }
	...
	printk("CES service create failed\n");
    } else {
...
cmd_result_t cmd_esw_ces_service_cas(int unit, int service, args_t *args) {
	...
	if (ARG_CNT(args) > 0) {
	    ...
	    if (sal_strcasecmp(arg_string_p, "get") == 0) {
		    ...
		    ret = CMD_OK;
		    printk("CAS is %s for CES service %d\n", 
		...
		printk("Expected either get or set\n");
		ret = CMD_FAIL;
	    }
	}
	...
	printk("Expected either enable or tx\n");
	ret = CMD_FAIL;
    }
...
cmd_result_t cmd_esw_ces_service_pm(int unit, int service, args_t *args) {
	...
	printk("bad_length_packets            : %d\n", pm_counts.jbf_bad_length_packets);
	printk("missordered_dropped_packets   : %d\n", pm_counts.jbf_dropped_ooo_packets);
	printk("reordered_packets             : %d\n", pm_counts.jbf_reordered_ooo_packets);
	printk("minimum_jitter_buffer_depth   : %d\n", pm_counts.jbf_depth_min);
	printk("maximum_jitter_buffer_depth   : %d\n", pm_counts.jbf_depth_max);
	printk("jitter_buffer_missing_packets : %d\n", pm_counts.jbf_missing_packets);
	printk("jitter_buffer_underrun_packets: %d\n", pm_counts.jbf_underruns);
/*	printk("jitter_buffer_restart         : %d\n", pm_counts.jitter_buffer_restart); */
	printk("transmitted_bytes             : %d\n", pm_counts.transmitted_bytes);
	printk("transmitted_packets           : %d\n", pm_counts.transmitted_packets);
	printk("received_bytes                : %d\n", pm_counts.received_bytes);
	printk("received_packets              : %d\n", pm_counts.received_packets);
...
cmd_result_t cmd_esw_ces_service(int unit, args_t *args) {
	...
	if (sal_strcasecmp(arg_string_p, "create") == 0) {
	    ...
	    printk("Invalid CES service subcommand: %s, expecting one of create or show\n", arg_string_p);
	    return CMD_FAIL;
	}
    } else {
	...
	if (service < 0 || service >= BCM_CES_CIRCUIT_IDX_MAX) {
	    printk("Invalid service number, valid values are between 0 and 63\n");
	    return CMD_FAIL;
	}
	...
	if (sal_strcasecmp(arg_string_p, "set") == 0) {
		...
		if (sal_strcasecmp(arg_string_p, "channel") == 0) {
			...
			if (sal_strcasecmp(arg_string_p, "ingress") == 0) {
			    ...
			    printk("Unknown service set channel option:%s\n", arg_string_p);
			    ret = CMD_FAIL;
			}
		    } else {
			printk("Expected either ingress or egress\n");
			ret = CMD_FAIL;
		    }
		    ...
		    printk("Expected either channel, map or header\n");
		    ret = CMD_FAIL;
		}
	    } else {
		printk("Unknown service set option:%s\n", arg_string_p);
		ret = CMD_FAIL;
	    }
	} else if (sal_strcasecmp(arg_string_p, "show") == 0) {
		...
		{
		    printk("Invalid CES service subcommand: %s\n", arg_string_p);
		    return CMD_FAIL;
		}
	    }
	    else
	    {
		printk("Insuficient number of arguments. Expecting one of status|get|set.\n");
		return CMD_FAIL;
	    }
	}
	else
	{
	    printk("Invalid CES service subcommand: %s, expecting one of create|delete|show|rclock \n", arg_string_p);
	    return CMD_FAIL;
	}
    }
...
cmd_result_t cmd_esw_ces(int unit, args_t *args) {
	...
	{
	    printk("Insuficient number of arguments. Expecting at least get|set.\n");
	    return CMD_FAIL;
	}
    }
	...
	printk("Invalid CES subcommand: %s\n", arg_string_p);
	return CMD_FAIL;
    }
