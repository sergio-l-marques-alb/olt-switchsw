/*
 * $Id: multicast.c,v 1.11.150.1 Broadcom SDK $
                           ...
                           void *user_data)
{
    ...
    if (rv < 0) {
        printk("%s ERROR: egress port count get failed - ?%s\n",
    ...
    if (port_array == NULL) {
        printk("%s ERROR: port_array mem alloc failed\n",
    ...
    if (encap_id_array == NULL) {
        printk("%s ERROR: encap_id_array mem alloc failed\n",
               ARG_CMD((args_t *)user_data));
        sal_free(port_array);
        return BCM_E_MEMORY;        
    }
    ...
    rv = bcm_multicast_egress_get(unit, group, port_count,
                                  ...
                                  &port_count);
    if (rv < 0) {
        printk("%s ERROR: egress get failure - %s\n",
               ARG_CMD((args_t *)user_data), bcm_errmsg(rv));
        sal_free(port_array);
...
cmd_multicast(int unit, args_t *a)
{
    ...
    if (!sal_strcasecmp(subcmd, "create")) {
        ...
        if (rv < 0) {
            printk("%s ERROR: fail to create %s group - %s\n",
                   ARG_CMD(a), cmd_multicast_parse_type[type],
                   bcm_errmsg(rv));
            return CMD_FAIL;
        }

        printk("group id 0x%x\n", group);
    ...
    if (!sal_strcasecmp(subcmd, "destroy")) {
        ...
        if (rv < 0) {
            printk("%s ERROR: fail to destroy group %d - %s\n",
    ...
    if (!sal_strcasecmp(subcmd, "l3encap")) {
        ...
        if (intf == -1) {
            printk("%s: Interface not specified\n", ARG_CMD(a));
            return CMD_FAIL;
        }
        ...
        if (rv < 0) {
            printk("%s ERROR: fail to get L3 encap - %s\n",
    ...
    if (!sal_strcasecmp(subcmd, "l2encap")) {
        ...
        if (rv < 0) {
            printk("%s ERROR: fail to get L2 encap - %s\n",
                   ARG_CMD(a), bcm_errmsg(rv));
            return CMD_FAIL;
        }
        printk("Encap ID %d\n", encap_id);
    ...
    if (!sal_strcasecmp(subcmd, "vplsencap")) {
        ...
        if (group < 0) {
            printk("%s: Group not specified\n", ARG_CMD(a));
            return CMD_FAIL;
        }
        if (port == BCM_GPORT_INVALID) {
            printk("%s: Port not specified\n", ARG_CMD(a));
            return CMD_FAIL;
        }
        if (mpls_port_id == BCM_GPORT_INVALID) {
            printk("%s: MplsPortId not specified\n", ARG_CMD(a));
            return CMD_FAIL;
        }
        ...
        rv = bcm_multicast_vpls_encap_get(unit, group, port, mpls_port_id,
                                          &encap_id);
        if (rv < 0) {
            printk("%s ERROR: fail to get VPLS encap - %s\n",
                   ARG_CMD(a), bcm_errmsg(rv));
            return CMD_FAIL;
        }
        printk("Encap ID %d\n", encap_id);
    ...
    if (!sal_strcasecmp(subcmd, "subportEncap")) {
        ...
        if (subport == BCM_GPORT_INVALID) {
            printk("%s: SubPort not specified\n", ARG_CMD(a));
            return CMD_FAIL;
        }
        ...
        rv = bcm_multicast_subport_encap_get(unit, group, port, subport,
                                             &encap_id);
        if (rv < 0) {
            printk("%s ERROR: fail to get subport encap - %s\n",
                   ARG_CMD(a), bcm_errmsg(rv));
            return CMD_FAIL;
        }
        printk("Encap ID %d\n", encap_id);
    ...
    if (!sal_strcasecmp(subcmd, "mimencap")) {
        ...
        if (mim_port_id == BCM_GPORT_INVALID) {
            printk("%s: MimPortId not specified\n", ARG_CMD(a));
            return CMD_FAIL;
        }
        ...
        rv = bcm_multicast_mim_encap_get(unit, group, port, mim_port_id,
                                         &encap_id);
        if (rv < 0) {
            printk("%s ERROR: fail to get MIM encap - %s\n",
                   ARG_CMD(a), bcm_errmsg(rv));
            return CMD_FAIL;
        }
        printk("Encap ID %d\n", encap_id);
    ...
    if (!sal_strcasecmp(subcmd, "wlanencap")) {
        ...
        if (wlan_port_id == BCM_GPORT_INVALID) {
            printk("%s: WlanPortId not specified\n", ARG_CMD(a));
            return CMD_FAIL;
        }
        ...
        rv = bcm_multicast_wlan_encap_get(unit, group, port, wlan_port_id,
                                          &encap_id);
        if (rv < 0) {
            printk("%s ERROR: fail to get WLAN encap - %s\n",
                   ARG_CMD(a), bcm_errmsg(rv));
            return CMD_FAIL;
        }
        printk("Encap ID %d\n", encap_id);
    ...
    if (!sal_strcasecmp(subcmd, "add")) {
        ...
        if (rv < 0) {
            printk("%s ERROR: egress add failure - %s\n",
                   ARG_CMD(a), bcm_errmsg(rv));
            return CMD_FAIL;
        }
        return CMD_OK;
    ...
    if (!sal_strcasecmp(subcmd, "delete")) {
        ...
        if (port == BCM_GPORT_INVALID && encap_id == BCM_IF_INVALID) {
            ...
            if (rv < 0) {
                printk("%s ERROR: egress delete all failure - %s\n",
                       ARG_CMD(a), bcm_errmsg(rv));
                return CMD_FAIL;
            }
        } else {
            ...
            if (rv < 0) {
                printk("%s ERROR: egress delete failure - %s\n",
                       ARG_CMD(a), bcm_errmsg(rv));
                return CMD_FAIL;
            }
        }
