...
_report_flags(int unit, int port, uint32 flags) {
    if (port > 0) {
        printk("Stack flags for unit %d, port %d: (0x%x)\n", unit,
               port, flags);
    } else {
        printk("Stack flags for unit %d: (0x%x)\n", unit, flags);
    }
...
cmd_stk_mode(int unit, args_t *args)
{
    ...
    if ((none < 0) && (sl < 0) && (hg < 0)) { /* Get */
        ...
        if (rv < 0) {
            printk("ERROR: bcm_stk_mode_get returns %s\n", bcm_errmsg(rv));
            return CMD_FAIL;
        }
        ...
        if (rv < 0) {
            printk("ERROR: bcm_stk_mode_set returns: %s\n", bcm_errmsg(rv));
            printk("   Unit %d. Flags: 0x%x\n", stk_unit, flags);
            return CMD_FAIL;
        }
    }
...
cmd_stk_port_get(int unit, args_t *args)
{
    ...
    if (stk_port >= 0) {
        if (stk_unit < 0) {
            printk("ERROR: Can't specify port with unit < 0\n");
            return CMD_FAIL;
        }

        rv = bcm_stk_port_get(stk_unit, stk_port, &flags);
        if (rv < 0) {
            printk("ERROR: stk_port_get unit %d port %d flags 0x%x: %s\n",
                   stk_unit, stk_port, flags, bcm_errmsg(rv));
            return CMD_FAIL;
        }
    ...
    for (i = start_unit; i < end_unit; i++) {
        ...
        if (rv < 0) {
            printk("ERROR: stk_pbmp_get unit %d: %s\n", i, bcm_errmsg(rv));
            return CMD_FAIL;
        }
...
cmd_modmap(int unit, args_t *a)
{
    ...
    if (sal_strcasecmp(subcmd, "enable") == 0) {
        ...
        DPORT_BCM_PBMP_ITER(unit, pbmp, dport, port) {
            ...
            printk("port %s modid mapping enabled.\n",
    ...
    if (sal_strcasecmp(subcmd, "disable") == 0) {
        ...
        DPORT_BCM_PBMP_ITER(unit, pbmp, dport, port) {
            ...
            printk("port %s modid mapping disabled.\n",
                   BCM_PORT_NAME(unit, port));
        }
        return CMD_OK;
    }
    ...
    if (sal_strcasecmp(subcmd, "lmod") == 0) {
        ...
        if (sal_strcasecmp(subcmd, "set") == 0) {
            ...
            printk("port %s assigns fmod=%d and lmod=%d.\n",
        ...
        if (sal_strcasecmp(subcmd, "get") == 0) {
            ...
            printk("port %s has fmod=%d maps to lmod=%d.\n",
    ...
    if (sal_strcasecmp(subcmd, "smod") == 0) {
        ...
        if (sal_strcasecmp(subcmd, "set") == 0) {
            ...
            DPORT_BCM_PBMP_ITER(unit, pbmp, dport, port) {
                ...
                if (arg_nports != 0) {
                    printk("port %s: fmod=%d maps smod=%d "
			   "with %d ports (base_port=%d)\n",
                           BCM_PORT_NAME(unit, port),
			   arg_fmod, arg_smod, arg_nports, arg_sport);
                } else {
                    printk("port %s: fmod=%d maps smod=%d deleted\n",
                           BCM_PORT_NAME(unit, port),
			   arg_fmod, arg_smod);
                }
            }
    ...
    if (sal_strcasecmp(subcmd, "group") == 0) {
        ...
        if (sal_strcasecmp(subcmd, "set") == 0) {
            ...
            if (arg_group > group) {
                printk("ERROR: Modmap group %d exceeds maximum of %d\n",
                       arg_group, group);
                return CMD_FAIL;
            }
            ...
            DPORT_BCM_PBMP_ITER(unit, pbmp, dport, port) {
                ...
                printk("port %s set to modmap group %d\n",
