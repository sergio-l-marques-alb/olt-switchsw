/*
 * $Id: nh_tx.c,v 1.29 Broadcom SDK $
...
#define NH_DEBUG(stuff)  TKS_DEBUG(TKS_DBG_NHV, stuff)
#define VERY_VERBOSE(stuff) TKS_DEBUG(TKS_DBG_NHV | TKS_DBG_VVERBOSE, stuff)
#else
#define NH_DEBUG(stuff)
#define VERY_VERBOSE(stuff)
...
nh_tx_src_mod_port_set(int unit, int port, int src_mod, int src_port)
{
    ...
    for (i = 0; i < num_stk_ports; i++) {
        if (nh_stk_ports[i].unit == unit &&
            ...
            NH_DEBUG(("NH: Set unit %d, port %d, src-mod %d, src-port %d\n",
    ...
    if (rv == BCM_E_NOT_FOUND) {
        if (num_stk_ports < STK_PORTS_MAX) {
            ...
            NH_DEBUG(("NH: Add unit %d, port %d, src-mod %d, src-port %d\n",
                      unit, port, src_mod, src_port));
    ...
    BCM_FOREACH_LOCAL_UNIT(unit) {
        if (install) {
            ...
            if ((rv != BCM_E_UNAVAIL) && (rv != BCM_E_NONE)) {
                TKS_DEBUG(TKS_DBG_ERR, ("NH TX ERROR adding L2 addr to "
                           "unit %d: %s\n", unit, bcm_errmsg(rv)));
                return rv;
            }
        } else {
      ...
      void *cookie)
{
    ...
    VERY_VERBOSE(("NHTX: (%d, %d). %p, len %d, type %d. flags %x "
...
nh_tx_reset(int reset_to_defaults)
{
    ...
    if (pending_count) {
        sal_usleep(100000);
        if (pending_count) {
            NH_DEBUG(("NHTX: %d Packets still pending on reset\n",
                      pending_count));
            NH_UNLOCK;
            return BCM_E_FAIL;
        }
