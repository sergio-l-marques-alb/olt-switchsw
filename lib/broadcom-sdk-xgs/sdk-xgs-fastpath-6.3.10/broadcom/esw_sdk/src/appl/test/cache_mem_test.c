...
int cache_mem_test_cache_set( int unit,  soc_mem_t mem, void* enable);
int cache_mem_test_read_and_compare( int unit,  soc_mem_t mem, void* data);
int cache_mem_test_cache_attach(int unit, soc_reg_t mem, int block_instance, int enable);
...
void cache_mem_test_create_mask(  uint32 unit, soc_mem_t mem,  soc_reg_above_64_val_t mask);
...
int
cache_mem_test_cache_attach(int unit, soc_reg_t mem, int block_instance, int enable)
{
    int rc;
    ...
    if (soc_mem_is_valid(unit, mem) &&
       ...
       rc = soc_mem_cache_set(unit, mem, block_instance, enable);
    } else {
            rc = SOC_E_UNAVAIL;
    }
...
int cache_mem_test_skip_mem(uint32 unit, soc_mem_t mem)
{
	...
	if (!SOC_MEM_IS_VALID(unit, mem))
	{
		SOC_DEBUG_PRINT((DK_MEM, "CACHE_MEM_TEST: unit %d Skip INVALID memory %d (%s)\n", unit, mem, SOC_MEM_NAME(unit, mem)));
        return TRUE;
	}
	...
	{
		SOC_DEBUG_PRINT((DK_MEM, "CACHE_MEM_TEST: unit %d Skip %s memory %d (%s)\n",
				unit,
    ...
    if(SOC_IS_ARAD(unit))
    {
    	...
    	{
			/* skip dynamic memory */
    		SOC_DEBUG_PRINT((DK_MEM, "CACHE_MEM_TEST: unit %d Skip DYNAMIC memory %d (%s)\n", unit, mem, SOC_MEM_NAME(unit, mem)));
    		return TRUE;
    	}
    }
#endif
	/*Skip big entry_len tables */
	if ((mem == IHB_MEM_230000m) || (mem == IHB_FIFO_0_8_TO_41m) || (mem == IHB_FIFO_1_8_TO_41m) || (mem == IHB_FIFO_2_8_TO_41m))	/*causes ASSERT - memory len too big (1024) */
	{
		SOC_DEBUG_PRINT((DK_MEM, "unit %d Skip big entry memory %d (%s)\n", unit, mem, SOC_MEM_NAME(unit, mem)));
		return TRUE;
	}

    /*Skip Signal tables */
	if ((mem == CRPS_MEM_0080000m) || (mem == CRPS_MEM_0090000m) || (mem == CRPS_MEM_00A0000m) || (mem == CRPS_MEM_00B0000m) ||
			(mem == ECI_MEM_00010000m) || (mem == IPS_MEM_180000m) || (mem == IPS_MEM_200000m) || (mem == IPS_MEM_220000m) ||
			(mem == IPS_MEM_240000m) || (mem == IPS_MEM_1A0000m) || (mem == IQM_MEM_7E00000m) || (mem == IPS_MEM_220000m) ||
			(mem == ECI_MEM_00010000m) || (mem == IPS_MEM_180000m) || (mem == IPS_MEM_200000m) || (mem == PORT_WC_UCMEM_DATAm))
	{
		SOC_DEBUG_PRINT((DK_MEM, "unit %d Skip SIGNAL memory %d (%s)\n", unit, mem, SOC_MEM_NAME(unit, mem)));
		return TRUE;
	}
    ...
    if(SOC_IS_ARAD(unit))
    {
		if ((mem == IRR_MEM_300000m) || (mem == IRR_MEM_340000m) || (mem == IRR_MEM_3C0000m))
		{
			SOC_DEBUG_PRINT((DK_MEM, "unit %d Skip SIGNAL memory %d (%s)\n", unit, mem, SOC_MEM_NAME(unit, mem)));
			return TRUE;
		}
...
#endif
    /* Skip because of error: _soc_mem_array_sbusdma_read: Received sbus ack data size is not same is in rep_words fields */
    if ((mem == IHP_LIF_TABLE_TRILL_0m) || (mem == IHP_LIF_TABLE_TRILL_1m) || (mem == IHP_LIF_TABLE_TRILL_2m) || (mem == IHP_LIF_TABLE_TRILL_3m))
	{
		SOC_DEBUG_PRINT((DK_MEM, "unit %d Skip memory %d (%s)\n", unit, mem, SOC_MEM_NAME(unit, mem)));
		return TRUE;
	}
...
int cache_mem_test_cache_set( int unit,  soc_mem_t mem, void* enable)
{
	...
	rv = cache_mem_test_cache_attach(unit, mem, COPYNO_ALL, *(int *)enable);
    if (rv != SOC_E_NONE)
    	SOC_ERROR_PRINT((DK_MEM, "CACHE_MEM_TEST: unit %d cache enable failed for %d (%s) rv %d\n", unit, mem, SOC_MEM_NAME(unit, mem), rv));
		  ...
		  soc_reg_above_64_val_t mask)
{
    ...
    /* Print the Mask */
    SOC_DEBUG_PRINT((DK_MEM, "CACHE_MEM_TEST: Mask for memory %d %s, nFields %d mask 0x",
                		mem, SOC_MEM_NAME(unit, mem), meminfo->nFields));

    for ( i = entry_dw; i > 0; i--)
    {
        SOC_DEBUG_PRINT((DK_MEM, "%X", mask[i]));
    }
...
uint32 cache_mem_test_fill_values_soc_mem_write_callback(uint32 unit, soc_mem_t mem, uint32		i_array, tr_do_cache_mem_test_t *test_params)
{
	...
	soc_cm_print("CACHE_MEM_TEST: WRITE SOC: mem %d %s, num_of_entries %d, entry_dw %d\n",
     ...
     void *opaque)
{
	...
	*value = 0;
	...
	for (i = 0; i < entry_size; i++)
    {
    	...
    	soc_cm_print("CACHE_MEM_TEST: WRITE DMA: mem  %d %s i %d value 0x%X mask 0x%X \n",
...
cache_mem_test_write_iter_callback(int unit, soc_mem_t mem, void* data)
{
	...
	if (SOC_IS_ARAD(unit))
	{
	    ...
	    {
	        /* skip memory*/
	    	soc_cm_print("CACHE_MEM_TEST: WRITE: skipping memory %d %s\n", mem, SOC_MEM_NAME(unit, mem));
	    	goto done;
  		}
	}
#endif
    ...
    switch (test_params->write_type)
    {
		case cache_mem_test_write_type_dma:
		{
			...
			{
/*				soc_cm_print("CACHE_MEM_TEST: WRITE SOC: write memory %d %s \n", mem, SOC_MEM_NAME(unit, mem));*/
...
int
cache_mem_test_read_and_compare( int unit,  soc_mem_t mem, void* data)
{
	soc_error_t	rv = SOC_E_NONE;
	...
	if (SOC_IS_ARAD(unit))
	{
	    ...
	    {
	        /* skip memory*/
	    	soc_cm_print("CACHE_MEM_TEST: READ: skipping memory %d %s\n", mem, SOC_MEM_NAME(unit, mem));
	    	goto done;
	    }
	}
#endif
    ...
    soc_cm_print("CACHE_MEM_TEST: READ read memory %d %s, num_of_entries %d  \n",
    ...
    for (i_array = start_array_index; i_array <= end_array_index; i_array++)
    {
    	for (index = start_index; index <= end_index; index++ )
    	{
    		...
    		if (rv != SOC_E_NONE)
			{
    			SOC_ERROR_PRINT((DK_MEM, "CACHE_MEM_TEST:Read FAILED rv %d: read_value 0x%X cache_value 0x%X mask 0x%X mem %d %s, index %d, entry_index %d array %d\n",
    		...
    		for (entry_index = 0; entry_index < entry_dw; entry_index++)
    		{
    			...
    			{
    		    	SOC_DEBUG_PRINT((DK_MEM, "CACHE_MEM_TEST: READ: read_value 0x%X cache_value 0x%X == expected_value 0x%X, (0x%X == 0x%X), mask 0x%X, mem %d, index %d, entry_index %d array %d, len %d\n",
    			...
    			{
    				soc_cm_print("CACHE_MEM_TEST: READ FAILED: read_value 0x%X, cache_value 0x%X, expected_value 0x%X, (0x%X != 0x%X != 0x%X), mask 0x%X mem %d, index %d, entry_index %d array %d\n",
    		    			read_value[entry_index] & mem_field_mask[entry_index],
    		    			read_cache_value[entry_index] & mem_field_mask[entry_index],
							expected_value[entry_index] & mem_field_mask[entry_index],
...
void do_cache_mem_test_print_usage(void)
{
	soc_cm_print("Usage for Cache Memory Test: \n"
...
do_cache_mem_test_init(int unit, args_t *a, void **p)
{
    ...
    *p = (void *) test_params;
	...
	if (SOC_IS_ARAD(unit))
	{
		...
		if (CMD_OK == rv)
		{
			soc_cm_print("CACHE_MEM_TEST:unit %d counter processor background accesses suspended\n", unit);
		}
		else
		{
			SOC_ERROR_PRINT((DK_MEM, "CACHE_MEM_TEST: unit %d counter processor background access suspend failed: %d (%s)\n",
					unit, rv, _SHR_ERRMSG(rv)));
		}

		if ((rv |= soc_dpp_device_reset(unit, SOC_DPP_RESET_MODE_REG_ACCESS, SOC_DPP_RESET_ACTION_INOUT_RESET)) < 0)
		{
			SOC_ERROR_PRINT((DK_MEM, "CACHE_MEM_TEST: unit %d ERROR: Unable to reinit  \n", unit));
			goto done;
		}
	}
#endif
...
do_cache_mem_test_done(int unit,  void *p)
{
	...
	soc_cm_print("CACHE_MEM_TEST: unit %d done\n", unit);
    return rv;
}
...
do_cache_mem_test(int unit,  args_t *a, void* tr_do_cache_mem_test)
{
	...
	if (test_params->test_part == cache_mem_test_full || test_params->test_part == cache_mem_test_write_only)
	{
		if (soc_mem_iterate(unit, cache_mem_test_cache_set, &cache_enable) < 0)
			SOC_ERROR_PRINT((DK_MEM, "CACHE_MEM_TEST: unit %d cache enable failed\n", unit));
	}
#endif
	...
	{
		if (soc_mem_iterate(unit, cache_mem_test_cache_set, &cache_enable) < 0)
			SOC_ERROR_PRINT((DK_MEM,  "CACHE_MEM_TEST: unit %d cache disable failed\n", unit));
	}
#endif
