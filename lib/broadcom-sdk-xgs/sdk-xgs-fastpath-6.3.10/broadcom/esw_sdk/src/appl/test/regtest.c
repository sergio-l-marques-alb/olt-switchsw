/*
 * $Id: regtest.c,v 1.93 Broadcom SDK $
              ...
              uint64 mask)
{
    ...
    if (SAL_BOOT_PLISIM) {
        if (!SOC_IS_XGS(rd->unit) && SOC_REG_IS_64(rd->unit,ainfo->reg)) {
            soc_cm_debug(DK_WARN,"Skipping 64 bit %s register in sim\n",regname);
            return 0;
      }
    }
    ...
    if (SOC_IS_POLAR(rd->unit)) {
        if ((r = soc_robo_anyreg_read(rd->unit, ainfo, &rd64)) < 0) {
            soc_cm_debug(DK_ERR, "ERROR: read reg %s failed: %s\n",
    ...
    soc_cm_debug(DK_TESTS, "Write %s: value %s mask %s\n",
                 regname, wr_str, mask_str);
#ifdef BCM_POLAR_SUPPORT
    if (SOC_IS_POLAR(rd->unit)) {
        if ((r = soc_robo_anyreg_write(rd->unit, ainfo, wr64)) < 0) {
            soc_cm_debug(DK_ERR, "ERROR: write reg %s failed: %s wrote %s (mask %s)\n",
    ...
    if (SOC_IS_POLAR(rd->unit)) {
        if ((r = soc_robo_anyreg_read(rd->unit, ainfo, &rrd64)) < 0) {    	
            soc_cm_debug(DK_ERR, "ERROR: reread reg %s failed: %s after wrote %s (mask %s)\n",
                         regname, soc_errmsg(r), wr_str, mask_str);
            rd->error = r;
            return -1;
        }
    ...
    soc_cm_debug(DK_TESTS, "Read  %s: value %s expecting %s\n",
                 regname, rrd_str, pat_str);

    if (COMPILER_64_NE(rrd64, pat64)) {
        soc_cm_debug(DK_ERR, "ERROR %s: wrote %s read %s (mask %s)\n",
    ...
    if (SOC_IS_POLAR(rd->unit)) {
        if ((r = soc_robo_anyreg_write(rd->unit, ainfo, rd64)) < 0) {    	
            soc_cm_debug(DK_ERR, "ERROR: rewrite reg %s failed: %s\n",
                         regname, soc_errmsg(r));
            rd->error = r;
            return -1;        
        }
...
try_reg(int unit, soc_regaddrinfo_t *ainfo, void *data)
{
    ...
    if (SOC_IS_ENDURO(unit) || SOC_IS_HURRICANE(unit) || SOC_IS_KATANA(unit)) {
        if (ainfo->reg == OAM_SEC_NS_COUNTER_64r) {
            soc_cm_debug(DK_WARN,"Skipping OAM_SEC_NS_COUNTER_64 register\n");
...
reg_test(int unit, args_t *a, void *pa)
{
    ...
    soc_cm_debug(DK_TESTS, "Register read/write test\n");
    ...
    while ((s = ARG_GET(a)) != NULL) {
        ...
        soc_cm_debug(DK_WARN, "WARNING: unknown argument '%s' ignored\n", s);
    }
    ...
    if (SOC_IS_SIRIUS(unit)) {
        if ((r = soc_sirius_reset(unit)) < 0) {
            soc_cm_debug(DK_ERR, "ERROR: Unable to reset unit %d: %s\n",
                         unit, soc_errmsg(r));
            goto done;
        }
    }
...
#endif
    {
        ...
        if (SOC_IS_ARAD(unit))
        {
             if ((r = soc_dpp_device_reset(unit, SOC_DPP_RESET_MODE_REG_ACCESS,SOC_DPP_RESET_ACTION_INOUT_RESET)) < 0) {
                   soc_cm_debug(DK_ERR, "ERROR: Unable to reinit unit %d: %s\n", unit, soc_errmsg(r));
                goto done;
            }
        } else
#endif
    ...
    if (SOC_IS_POLAR(unit)) {
        if (soc_robo_reg_iterate(unit, try_reg_dispatch, &rd) < 0) {
            soc_cm_debug(DK_TESTS, "Continuing test.\n");
   	        rv = 0;
        } else {
            rv = rd.error;
        }
...
rval_test_proc_above_64(int unit, soc_regaddrinfo_t *ainfo, void *data)
{
    ...
    if (rval_test_skip_reg(unit, ainfo)) {
        /* soc_reg_sprint_addr(unit, buf, ainfo);
            soc_cm_debug(DK_WARN,"Skipping register %s\n", buf); */
        return 0;
    }
    ...
    if (!SOC_REG_ABOVE_64_IS_EQUAL(rrd_val, rval)) {
        soc_cm_debug(DK_ERR, "ERROR %s: default %s read %s (mask %s)\n",
...
rval_test_proc(int unit, soc_regaddrinfo_t *ainfo, void *data)
{
    ...
    if (SOC_IS_HAWKEYE(unit)) {
        if (ainfo->reg == MAC_MODEr) {
            soc_cm_debug(DK_WARN,"Skipping MAC_MODE register\n");
    ...
    if (SOC_IS_HURRICANE2(unit)) {
        if (ainfo->reg == TOP_XG_PLL0_CTRL_3r) {
            soc_cm_debug(DK_WARN,"Skipping TOP_XG_PLL0_CTRL_3 register\n"); 
            return 0;
        }
    }
#endif
    ...
    if (SOC_IS_POLAR(rd->unit)) {
        if ((r = soc_robo_anyreg_read(rd->unit, ainfo, &value)) < 0) {
            soc_cm_debug(DK_ERR, "ERROR: read reg %s (0x%x) failed: %s\n",
    ...
    soc_cm_debug(DK_TESTS, "Read %s: reset mask %s, reset value %s, read %s\n",
    ...
    if (!COMPILER_64_IS_ZERO(chk)) {
        ...
        format_uint64(val_str, value);
        soc_cm_debug(DK_ERR, "ERROR: %s: expected %s, got %s, reset mask %s\n",
...
rval_test(int unit, args_t *a, void *pa)
{
    ...
    soc_cm_debug(DK_TESTS, "Register reset value test\n");
   ...
   /* } */
...
#endif
    {
        ...
        if (SOC_IS_FE1600(unit))
        {
            ...
            sal_sleep(1);
            SOC_IF_ERROR_RETURN(WRITE_CMIC_SOFT_RESET_REGr(unit, 0x0));
            sal_usleep(20);
            SOC_IF_ERROR_RETURN(WRITE_CMIC_SOFT_RESET_REGr(unit, 0xffffffff));
            sal_usleep(20);
            COMPILER_64_SET(reg_val64, 0x7, 0xFFFFFFFF);
            SOC_IF_ERROR_RETURN(WRITE_ECI_FE_1600_SOFT_RESETr(unit, reg_val64));
            sal_sleep(1);
            COMPILER_64_SET(reg_val64, 0x0, 0x0);
            SOC_IF_ERROR_RETURN(WRITE_ECI_FE_1600_SOFT_RESETr(unit,reg_val64));
            reg_val32 = 0x1ff;
            SOC_IF_ERROR_RETURN(WRITE_ECI_SB_RSTN_AND_POWER_DOWNr(unit,reg_val32));
            sal_sleep(1);
        } else 
#endif
        {
