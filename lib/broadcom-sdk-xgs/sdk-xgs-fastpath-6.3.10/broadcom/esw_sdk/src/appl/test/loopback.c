/*
 * $Id: loopback.c,v 1.206 Broadcom SDK $
...
#ifdef BCM_CALADAN3_SUPPORT
int
lb_caladan3_cmic_credits_refresh(int unit) {

    uint32 regval = 0;

    SOC_IF_ERROR_RETURN(soc_reg32_get(unit,
        PT_XL_CMIC_ILKN_CLIENT_DEBUG_CREDIT1r, SOC_REG_ADDR_INSTANCE_MASK, 0, &regval));
    if (0 == soc_reg_field_get(unit, PT_XL_CMIC_ILKN_CLIENT_DEBUG_CREDIT1r, regval, CMIC_PORT_CREDITf)) {
        SOC_IF_ERROR_RETURN(WRITE_CMIC_RXBUF_EPINTF_RELEASE_ALL_CREDITSr(unit, 0));
        sal_usleep(100);
        SOC_IF_ERROR_RETURN(WRITE_CMIC_RXBUF_EPINTF_RELEASE_ALL_CREDITSr(unit, 1));
    }
    return SOC_E_NONE;
}

int 
lb_dev_init(int unit, loopback_testdata_t *lp) {
    int rv;
    soc_control_t *soc;
    int cleanstart = 1; 

    if (!SOC_UNIT_VALID(unit)) {
        test_error(unit, "Unit %d: not attached", unit);
        return -1;
    }
    if (test_get_last_test_status(unit) >= 0) {
        cleanstart = 0;
    }
    if (test_is_no_reinit_mode(unit)) {
        test_msg("No-Re-Init set, skipping initialization\n");
        return 0;
    }
    ...
    if ((cleanstart) || (soc && !(soc->soc_flags & SOC_F_INITED))) {
        ...
        /* Force re-init after init soc */
        cleanstart = 1;
    }

#ifdef BCM_CALADAN3_SVK
    /* C3 SVK PHYs need to be taken out of reset */
    soc_i2c_probe(unit);
    cpu_i2c_probe(cpu_i2c_defaultbus());
    sal_config_set("phy_qsgmii_an", "0");
#endif

    if ((cleanstart) || (!(BCM_UNIT_VALID(unit)))) {
        rv = bcm_init(unit);
        if (SOC_FAILURE(rv)) {
    ...
    if (SOC_FAILURE(rv)) {
        test_error(unit, "Unit %d: link scan disable failed got (%d)\n", unit, rv);
        return -1;
    }
 ...
 */
{
    ...
    if (SOC_IS_CALADAN3(unit)) {
        ...
        int status = 0;
        BCM_PBMP_ITER(link_pbm, port) {
            do {
                sal_udelay(2000);
                rv = bcm_port_link_status_get(unit, port, &status);
                if (SOC_FAILURE(rv)) {
                    test_error(unit, "Failed getting link status (%d)\n", rv);
                    return -1;
                } else if ( status == BCM_PORT_LINK_STATUS_UP) {
                    break;
                }
                timeout+=2000;
            } while (timeout < LB_LINK_TIMEOUT);
            if (status != BCM_PORT_LINK_STATUS_UP) {
                test_error(unit, "Link status down for port %d \n", port);
                return -1;
            }
        }
        return 0;
 ...
 */
{
    ...
    lw->lw_mac_dst_inc = 0;
#ifdef BCM_CALADAN3_SUPPORT
    lw->lw_c3_lrp_bypass = soc_property_get(unit, spn_LRP_BYPASS, 1);
#endif
...
#ifdef BCM_CALADAN3_SUPPORT
        if (SOC_IS_CALADAN3(unit)) continue;
#endif
        setport = !setport;
    }
    lp->lp_dst_inc      = 1;
...
lb_unused_10g_ports_enable_set(int unit, soc_port_t rx_port, int enable)
{
    ...
    if (SOC_IS_XGS3_SWITCH(unit)) {
        ...
        PBMP_HG_ITER(unit, port) {
            if (port != rx_port) {
                bcm_port_enable_set(unit, port, enable);
            }
        }
    ...
    if (SOC_IS_XGS_SWITCH(lw->lw_unit)) {
        ...
        if (SOC_IS_XGS3_SWITCH(lw->lw_unit)) {
            ...
            if (SOC_IS_TR_VL(lw->lw_unit) && !(SOC_IS_TRIUMPH2(lw->lw_unit) 
                ...
                soc_xgs3_port_to_higig_bitmap(lw->lw_unit,
                                              CMIC_PORT(lw->lw_unit),
                                              &cpu_pbm);
 ...
 */
{
    ...
    if (!SOC_DCB_DONE_GET(unit, last_dcb)) {
        printk("ERROR: Chain done when not done at dcb %p\n", last_dcb);
    ...
    if (dv_chain == lw->lw_tx_dv_chain_done) {
        ...
        printk("Warning: dv chain done on DV_CHAIN[%p] "
    ...
    if (!lw->lw_sema_woke) {
        ...
        if (sal_sem_give(lw->lw_sema)) {
            printk("Warning: Chain done give failed\n");
        }
    }
    sal_spl(level);
}
 ...
 */
{
    ...
    if (!lw->lw_sema_woke) {
        lw->lw_sema_woke = TRUE;
        if (sal_sem_give(lw->lw_sema)) {
            printk("Warning: Desc done give failed\n");
        }
    }
    sal_spl(level);
}
 ...
 */
{
...
#ifdef BCM_CALADAN3_SUPPORT
    /* Inspect HIGIG header, this is part of the received frame */
    if (SOC_IS_CALADAN3(lw->lw_unit) && 
             IS_HG_PORT(lw->lw_unit, lw->lw_lp->lp_tx_port)) {
        if (lw->lw_lp->lp_hg_mode == 2) {
#ifdef BCM_HIGIG2_SUPPORT
            soc_higig2_hdr_t *tx_hdr = (soc_higig2_hdr_t*)tx_data;
            soc_higig2_hdr_t *rx_hdr = (soc_higig2_hdr_t*)rx_data;

            if (soc_higig2_field_get(lw->lw_unit, tx_hdr, HG_start) !=
                soc_higig2_field_get(lw->lw_unit, rx_hdr, HG_start)) {
                printk("ERROR: HIGIG START not detected (tx=%x,rx=%x)\n",
                       soc_higig2_field_get(lw->lw_unit, tx_hdr, HG_start),
                       soc_higig2_field_get(lw->lw_unit, rx_hdr, HG_start));
            }
            /* Adjust headers to point to data */
            rx_data += SOC_HIGIG2_HDR_SIZE;
            tx_data += SOC_HIGIG2_HDR_SIZE;
            rx_len -= SOC_HIGIG2_HDR_SIZE;
            /* tx_len doesnt account the transmitted higig header */
#endif
        } else if (lw->lw_lp->lp_hg_mode == 1) {
            ...
            /* Adjust headers to point to frame */
    ...
    if (ENET_CMP_MACADDR(mac_dst, &rx_data[0]) ||
        ...
        printk("ERROR: MAC address miscompare:\n"
    ...
    if (tx_len != rx_len) {
        ...
        printk("ERROR: Length miscompare: TX(%d) RX(%d)\n",
               tx_len, rx_len);
        rv = -1;
    } else if ((i = packet_compare(rx_data + 12,
                                   tx_data + 12,
                                   4)) >= 0) {
        printk("ERROR: VLAN tag miscompare: offset 0x%x\n", i + 12);
        rv = -1;
    } else if ((i = packet_compare(rx_data + 16,
                                   tx_data + 16,
                                   4)) >= 0) {
        printk("ERROR: Sequence # miscompare: offset 0x%x\n"
        ...
        rv = -1;
                                   ...
                                   tx_data + 20,
                                   tx_compare_len - 20)) >= 0) {
        printk("ERROR: Payload miscompare: offset 0x%x\n", i + 20);
        rv = -1;
    }
    ...
    if (lw->lw_lp->lp_check_crc) {
        ...
        if (calc_crc != rx_crc) {
            printk("ERROR: CRC miscompare: calc=0x%08x rx=0x%08x\n",
                   calc_crc, rx_crc);
            if (! rx_crc_valid)
                printk("ERROR: Note that invalid CRC was expected "
    ...
    if (rv) {
        ...
        printk("TX packet: len=%d\n", tx_len);
        soc_dma_dump_pkt(lw->lw_unit, "  ", tx_data, tx_len, TRUE);

        printk("RX packet: len=%d%s\n", orig_rx_len,
               skip_vlan ? " - 4 (VLAN)" : "");
        soc_dma_dump_pkt(lw->lw_unit, "  ", orig_rx_data, orig_rx_len, TRUE);
 ...
 */
{
    ...
    end_dcb = SOC_DCB_IDX2PTR(unit, dv->dv_dcb, dv->dv_vcnt); /* last dcb */
    rx_dcb = SOC_DCB_IDX2PTR(unit, dv->dv_dcb, dv->dv_dsc);
    tx_dcb = SOC_DCB_IDX2PTR(unit, lw->lw_tx_dv[0]->dv_dcb, dv->dv_dsc);
    ...
    while (rx_dcb < end_dcb) {
        ...
        if (lp->lp_check_data) {
           /* Dummy DCB used on caladan3, skip port check */
            soc_port_t  sp = SOC_DCB_RX_INGPORT_GET(unit, rx_dcb);
            if (!(SOC_IS_CALADAN3(lw->lw_unit)) && sp != lp->lp_rx_port) {
                printk("RX packet with invalid port: "
...
lb_allocate(int unit, dvt_t dvt, int dv_cnt, int dv_size, dv_t ***dv_array_ptr,
            int packet_cnt, int packet_size, uint8 ***packet_array_ptr)
/*
 ...
 */
{
 ...
 fail:
    lb_deallocate(unit, dv_cnt, &dv_array, packet_cnt, &packet_array);
    return(-1);
}
int
 ...
 */
{
    ...
    while (!lw->lw_eoc_tx || !lw->lw_eoc_rx) {
        if (sal_sem_take(lw->lw_sema, lw->lw_timeout_usec)) {
            printk("Time-out waiting for completion "
                   "Tx(%s)=%s Rx(%s)=%s\n",
    ...
    if (rv == 0 && soc_cm_debug_check(DK_DMA)) {
...
#ifdef BCM_CALADAN3_SUPPORT
    if (SOC_IS_CALADAN3(lw->lw_unit)) {
        lb_setup_dcbs(lw->lw_rx_packets, dv, length + SOC_HIGIG2_HDR_SIZE,
                  chain, empty_pbm, empty_pbm, 0, cos, 0, 0, NULL);
        lb_fill_packet_rx(lw, lw->lw_rx_packets, chain, length + SOC_HIGIG2_HDR_SIZE);
    } else  {
#endif /* BCM_CALADAN3_SUPPORT */

        lb_setup_dcbs(lw->lw_rx_packets, dv, length + 4,
                  chain, empty_pbm, empty_pbm, 0, cos, 0, 0, NULL);
        lb_fill_packet_rx(lw, lw->lw_rx_packets, chain, length + 4);
 ...
 */
{
...
#ifdef BCM_CALADAN3_SUPPORT
    if (SOC_IS_CALADAN3(lw->lw_unit)) {
        /* HiGIG header is part of the frame */
        if (IS_HG_PORT(lw->lw_unit, lw->lw_lp->lp_tx_port)) {
            soc_higig_hdr_t     *xghp = (soc_higig_hdr_t *)packet;
            sal_memset(xghp, 0, SOC_HIGIG_HDR_SIZE);
            soc_higig_field_set(lw->lw_unit, xghp, HG_start, SOC_HIGIG_START);
            soc_higig_field_set(lw->lw_unit, xghp, HG_hgi, SOC_HIGIG_HGI);
             soc_higig_field_set(lw->lw_unit, xghp, HG_hdr_format, SOC_HIGIG_HDR_DEFAULT);
            if (lp->lp_hg_mode == 1) {
                eh = (enet_hdr_t *)(packet + SOC_HIGIG_HDR_SIZE);
            } else {
                eh = (enet_hdr_t *)(packet + SOC_HIGIG2_HDR_SIZE);
            }
        } else {
            eh = (enet_hdr_t *)packet;
        }
    ...
    if (SOC_IS_CALADAN3(lw->lw_unit) &&
        ...
        if (lp->lp_hg_mode == 1) {
            *tx_len += SOC_HIGIG_HDR_SIZE;
        } else if (lp->lp_hg_mode == 2) {
            *tx_len += SOC_HIGIG2_HDR_SIZE;
        }
    }
#endif
}
 ...
 */
{
    ...
    lb_setup_dcbs(lw->lw_tx_packets, dv, tx_len, chain,
                  pbm, lp->lp_ubm,
                  crc_mode_to_dmac(lp->lp_crc_mode), cos, dport, dmod, xghp);
 ...
 */
{
    ...
    soc_cm_debug(DK_TESTS,
                 "Testing %s->%s count %d, chain %d-%d += %d, "
                 "len %d-%d += %d, cos %d-%d\n",
...
#ifdef BCM_CALADAN3_SUPPORT
        if (SOC_IS_CALADAN3(lw->lw_unit) && 
            IS_IL_PORT(lw->lw_unit, lp->lp_rx_port)) {
            /* No SNMP stats on the IL */
            return (test_rv);
        }
#endif
 ...
 */
{
    ...
    for (rx_chan = 0; rx_chan < N_DMA_CHAN; rx_chan++) {
        soc_cm_debug(DK_TESTS, "Configuring RX-channel: %d\n", rx_chan);
        ...
        if(soc_feature(unit, soc_feature_cmicm)) {
            soc_cm_debug(DK_TESTS, "Assign all COS to channel: %d\n", rx_chan);
            SOC_IF_ERROR_RETURN(
                bcm_rx_queue_channel_set(unit, -1, rx_chan));
        }
#endif
        for (tx_chan = 0; tx_chan < N_DMA_CHAN; tx_chan++) {
            if (rx_chan == tx_chan) {   /* Does not make sense */
                continue;
            }
            soc_cm_debug(DK_TESTS, "Configuring TX-channel: %d\n", tx_chan);
                     ...
                     soc_port_t src_port)
{
    ...
    soc_cm_debug(DK_TESTS, "Setting up static path for port %s\n",
...
#ifdef BCM_CALADAN3_SUPPORT
    if (SOC_IS_CALADAN3(unit) && (lw->lw_c3_lrp_bypass)) {
        soc_cm_debug(DK_TESTS, "Skipping ARL setup for port %s\n",
                     SOC_PORT_NAME(unit, src_port));
        return SOC_E_NONE;
    }
#endif
    soc_cm_debug(DK_TESTS, "Setting up ARL for port %s\n",
                 SOC_PORT_NAME(unit, src_port));
 ...
 */
{
    ...
    if (lp) {
        lb_deallocate(lw->lw_unit, 
                      ...
                      lp->lp_ppc_end, &lw->lw_rx_packets);

        lb_deallocate(lw->lw_unit, 
                      lp->lp_dv_end, &lw->lw_tx_dv,
                      lp->lp_ppc_end, &lw->lw_tx_packets);
    }
	...
	if (SOC_DPP_PP_ENABLE(lw->lw_unit) && SOC_IS_ARAD(lw->lw_unit)) {
	    ...
	    if (lp) {
                soc_cm_print("QUICKTURN:: ATTN : Restoring VLAN \n");
	        rv |= lb_restore_vlan(lw);
	    }
        }
    /*
...
#ifdef BCM_CALADAN3_SUPPORT
    if (SOC_IS_CALADAN3(lw->lw_unit)) {
        lb_caladan3_cmic_credits_refresh(lw->lw_unit);
    }
#endif
 ...
 */
{
    ...
    if (lb_allocate(unit, DV_RX, lp->lp_dv_end, lp->lp_c_end, &lw->lw_rx_dv,
                    lp->lp_ppc_end, lp->lp_l_end, &lw->lw_rx_packets) ||
        lb_allocate(unit, DV_TX, lp->lp_dv_end, lp->lp_c_end, &lw->lw_tx_dv,
                    lp->lp_ppc_end, lp->lp_l_end, &lw->lw_tx_packets)) {
...
_lb_max_frame_set(loopback_test_t *lw, loopback_testdata_t *lp)
{
    ...
    if (max_adj) {
        printk("NOTICE: Increased max frame size for %d-byte packets\n",
               max_pkt);
    }

    if (jumbo_adj) {
        printk("NOTICE: Enabled JUMBO frames on GE\n");
    }
    return(0);
}
                   ...
                   loopback_testdata_t *lp)
{
     ...
     */
    for (c = 0; c < N_DMA_CHAN; c++) {
        soc_cm_debug(DK_TESTS, "lb_dma_init: Deconfiguring channel %d\n", c);
...
#ifdef BCM_CALADAN3_SUPPORT
    if (SOC_IS_CALADAN3(unit) && (lw->lw_c3_lrp_bypass)) {
        soc_sbx_caladan3_sws_cmic_port_hpp_loopback(unit, 1, -1, -1);
        return 0;
    }
#endif
...
#endif
#ifdef BCM_CALADAN3_SUPPORT
    if (SOC_IS_CALADAN3(unit) && ((&lb_work[unit])->lw_c3_lrp_bypass)) {
        soc_sbx_caladan3_sws_cmic_port_hpp_loopback(unit, -1, -1, 1);
        return 0;
    }
#endif
 ...
 */
{
    ...
    if ((SOC_DPP_PP_ENABLE(unit) && SOC_IS_ARAD(unit)) || (!SOC_IS_ARAD(unit))) {
        if (!SOC_IS_SHADOW(unit)) {
            if ((!SOC_IS_CALADAN3(unit)) || (SOC_IS_CALADAN3(unit) && !(lw->lw_c3_lrp_bypass))) {
     ...
     * it should be OK if we do this for CMICe as well */
    if(soc_feature(unit, soc_feature_cmicm)) {
        soc_cm_debug(DK_TESTS, "Assign all COS to channel: 1\n" );
        SOC_IF_ERROR_RETURN(
 ...
 */
{
    ...
    soc_cm_debug(DK_TESTS, "lb_mac_test: unit %d starting ....\n", lw->lw_unit);
    ...
    PBMP_ITER(pbm, port) {
...
#ifdef BCM_CALADAN3_SUPPORT
        if (SOC_IS_CALADAN3(unit) && lw->lw_c3_lrp_bypass) {
            soc_sbx_caladan3_sws_cmic_to_port_mac_loopback(unit, port, 0);
        } else
#endif
#ifdef BCM_SHADOW_SUPPORT
        if (SOC_IS_SHADOW(unit)) {
        ...
        if (IS_HG_PORT(unit, port)) {
            lp->lp_hg_mode = soc_property_port_get(unit, port, spn_HIGIG2_HDR_MODE, 0) ? 2 : 1;
        }
#endif
...
#elif defined(BCM_CALADAN3_SUPPORT)
             {
                sbx_do_show_counters(unit, INVALIDr, portpbm,
                               SHOW_CTR_CHANGED | SHOW_CTR_SAME | SHOW_CTR_NZ);
            }
#endif
            return -1;
...
#ifdef BCM_CALADAN3_SUPPORT
        if (SOC_IS_CALADAN3(unit) && lw->lw_c3_lrp_bypass) {
            soc_sbx_caladan3_sws_cmic_to_port_mac_loopback(unit, port, 1);
        } 
#endif
    }
...
#ifdef BCM_CALADAN3_SUPPORT
    if (SOC_IS_CALADAN3(unit)) {
        /* Caladan3 ports loop on to themselves! */
        return port;
    }
#endif
...
#ifdef BCM_CALADAN3_SUPPORT  /* C3 ports loop on to themselves */
  if (!SOC_IS_CALADAN3(unit)) {
#endif
    ...
    while (!rv && (cur_packet < ppc)) {
        ...
        while (cur_dcb <= end_dcb) {
            ...
            if (!SOC_DCB_DONE_GET(unit, cur_dcb)) {
                printk("Error: TX: Expected done bit in DCB Pckt(%d)\n",
                       cur_packet);
                rv = -1;
            }
            ...
            if (SOC_DCB_REQCOUNT_GET(unit, cur_dcb) != 
                ...
                printk("Error: TX: Unexpected count Packet %d: "
         ...
         */
        if (cur_len != length) {
            printk("Error: TX: Unexpected total length: "
                   "Expected %d Got %d\n", length, cur_len);
            rv = -1;
        }
 ...
 */
{
    ...
    while (!rv && (cur_packet < ppc)) {
        ...
        if (!SOC_DCB_RX_END_GET(unit, end_dcb)) {
            printk("Error: RX: DCB (%d) Last DCB for packet does not have "
                   "END bit set\n",
        ...
        if (!SOC_DCB_RX_START_GET(unit, cur_dcb) && 
            ...
            printk("Error: RX: DCB (%d) First for packet does not have "
                   "START set\n", SOC_DCB_PTR2IDX(unit, end_dcb, dv->dv_dcb));
            rv = -1;
        }
        ...
        while (cur_dcb <= end_dcb) {
            ...
            if ((cur_dcb != start_dcb) && 
                ...
                printk("Error: RX: DCB (%d) Unexpected START bit\n",
            ...
            if ((cur_dcb != end_dcb) &&
                ...
                printk("Error: RX: DCB (%d) Unexpected END bit\n",
                       SOC_DCB_PTR2IDX(unit, cur_dcb, dv->dv_dcb));
                rv = -1;
            }
            ...
            if (!SOC_DCB_DONE_GET(unit, cur_dcb)) {
                printk("Error: RX: DCB (%d) Expected DONE set\n",
                       SOC_DCB_PTR2IDX(unit, cur_dcb, dv->dv_dcb));
                rv = -1;
            }
            ...
            if (SOC_DCB_REQCOUNT_GET(unit, cur_dcb) != 
                SOC_DCB_XFERCOUNT_GET(unit, cur_dcb)) {
                printk("Error: RX: Unexpected count: Expected %d Got %d\n",
                       SOC_DCB_REQCOUNT_GET(unit, cur_dcb), 
                       SOC_DCB_XFERCOUNT_GET(unit, cur_dcb));
                rv = -1;
            }
        ...
        soc_cm_debug(DK_TESTS, "lb_sg_check_rx : rxlen = %d txlen = %d\n",cur_len,length);
 ...
 */
{
    ...
    if (SOC_IS_XGS12_FABRIC(lw->lw_unit)) {
        /* Update length */
        soc_cm_debug(DK_TESTS, "lb_sg_wait: length = %d\n", length);
 ...
 */
{
    ...
    if (SOC_IS_XGS12_FABRIC(lw->lw_unit)) {
        /* Update length */
        soc_cm_debug(DK_TESTS, "lb_do_sg_rx: p_len = %d\n", p_len);
 ...
 */
{
    ...
    soc_cm_debug(DK_TESTS,
           "Testing Port %s --> %s cnt(%d) chn(%d,%d += %d) "
           "l(%d,%d += %d) cos(%d,%d)\n",
           SOC_PORT_NAME(lw->lw_unit, lw->lw_lp->lp_tx_port),
           SOC_PORT_NAME(lw->lw_unit, lw->lw_lp->lp_rx_port),
           lp->lp_count,
                ...
                for (cur_ppc = lp->lp_ppc_start;
                    ...
                    for (tx_dpp = lp->lp_dpp_start;
                        ...
                        for (rx_dpp = lp->lp_dpp_start;
                            ...
                            soc_cm_debug(DK_TESTS, "Testing cnt=%d L=%d cos=%d "
 ...
 */
{
    ...
    int         rx_chan, tx_chan;       /* RX/TX channel numbers */
    int rv;
 ...
 */
{
     ...
     */
    lb_unused_10g_ports_enable_set(unit, lp->lp_rx_port, 0);
    ...
    if (!SOC_PBMP_MEMBER(lp->lp_pbm, CMIC_PORT(unit))) {
        soc_cm_debug(DK_TESTS, "Warning: CPU port not set in bitmap.\n"
 ...
 */
{
    ...
    while ((NULL !=dv) && (length_rcv < length)) {
        ...
        if (!SOC_DCB_DONE_GET(unit, dcb)) {
            printk("Expected DONE not set in dv[%d@%p] dcb[%d@%p]\n",
                   *dv_idx, (void *)dv,
        ...
        if (length_rcv == 0) {
            if (!SOC_DCB_RX_START_GET(unit, dcb) && 
                !soc_feature(unit, soc_feature_dcb_st0_bug)) {
                printk("Expected Start dv[%d@%p] dcb[%d] not set\n",
                       *dv_idx, (void *)dv,
            ...
            printk("Un-expected Start dv[%d@%p] dcb[%d] set\n",
                   *dv_idx, (void *)dv, 
                   SOC_DCB_PTR2IDX(unit, dcb, dv->dv_dcb));
            rv = -1;
            break;                      /* while */
        ...
        if (magic != LB_START_MAGIC) {
            rv = -1;
            printk("Prefix of block clobbered: "
        ...
        if (magic != LB_END_MAGIC) {
            rv = -1;
            printk("Suffix of block clobbered: "
                   "Addr: %p Exp: 0x%x Rcv: 0x%x\n",
        ...
        if (SOC_DCB_XFERCOUNT_GET(unit, dcb) > (uint32)(length - length_rcv)) {
            rv = -1;
            printk("Too many bytes: Exp: <= 0x%x Rcv: 0x%x\n",
    ...
    if (length_rcv < length) { /* Something went wrong */
        printk("End of chain and length-received=%d expected=%d\n",
               length_rcv, length);
        rv = -1;
    }
 ...
 */
{
    ...
    soc_cm_debug(DK_TESTS,
           "Random: Tx-chan(%d) Rx-chan(%d) l=%d pkt-cnt=%d "
           "chn-cnt=%d dv-cnt=%d\n",
    ...
    if (SOC_IS_XGS12_FABRIC(lw->lw_unit)) {
        /* Update length */
        soc_cm_debug(DK_TESTS, "lb_random_restore_dv: l = %d\n", l);
...
int
lb_random_exec(loopback_test_t *lw, int dry_run)
{
    loopback_testdata_t *lp = lw->lw_lp;
    dma_chan_t  tx_chan, rx_chan;
 ...
 */
{
    ...
    for (channel = 0; channel < N_DMA_CHAN; channel++) {
        soc_cm_debug(DK_TESTS, "PBMP(0) testing Tx channel %d\n", channel);
        ...
        for (channel = 0; channel < N_DMA_CHAN; channel++) {
            ...
            for (d = lp->lp_dv_start; d <= lp->lp_dv_end; d += lp->lp_dv_inc) {
                ...
                if ((soc_dma_rv = soc_dma_start(unit, channel,
                                                dv_array[0])) < 0) {
                    printk("%s Channel %d: Unexpected DMA start: %s\n",
                           tx ? "Tx" : "Rx", channel, soc_errmsg(soc_dma_rv));
                    rv = -1;
                }
                ...
                while (!rv) {
                    ...
                    if (dma_desc != reload_addr) {
                        printk("%s channel %d: Expected ADDR=%p "
                               "Received ADDR=%p\n",
                               tx ? "Tx" : "Rx", channel,
                ...
                for (i = 0; !rv && (i < d); i++) {
                    ...
                    if (!SOC_DCB_DONE_GET(unit, dcb)) {
                        rv = -1;
                        printk("DCB[%d] Done expected but not set\n", i);
                    }

                    if (SOC_DCB_RX_START_GET(unit, dcb) ||
                        SOC_DCB_RX_END_GET(unit, dcb)) {
                        printk("DCB[%d] SW/EW set but not expected\n", i);
                        rv = -1;
                    }
                }
            ...
            if ((rv |= soc_dma_chan_config(unit, channel,
                                           DV_NONE, SOC_DMA_F_INTR))) {
                printk("%s Channel %d: unable to deconfigure\n",
                       tx ? "Tx" : "Rx", channel);
            }
        }
    }
