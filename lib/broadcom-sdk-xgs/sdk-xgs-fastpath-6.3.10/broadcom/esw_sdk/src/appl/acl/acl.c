...
#define ACL_IS_INIT()                                             \
    if (_acl_control == NULL) {                                   \
        ACL_ERR(("ACL Error: ACL not initialized\n"));            \
        return BCM_E_INIT;                                        \
    }
...
bcma_acl_init(void) 
{
    ...
    if (control == NULL) {
        ACL_ERR(("ACL Error: allocation failure for ACL control.\n"));
        return BCM_E_MEMORY;
    }
    ...
    if (control->acl_lock == NULL) {
        sal_free(control);
        ACL_ERR(("ACL Error: creation failure for ACL lock.\n"));
        return BCM_E_MEMORY;
    }
...
bcma_acl_add(bcma_acl_t *list)
{
    ...
    ACL_VVERB(("ACL bcma_acl_add()\n"));
    ACL_IS_INIT();

    if (list == NULL) {
        ACL_ERR(("ACL Error: null list pointer in bcma_acl_add()\n"));
        return BCM_E_PARAM;
    }

    /* Confirm that list ID does not already exist. */
    if (_acl_find(list->list_id) != NULL) {
        ACL_ERR(("ACL Error: List ID=%d already exists\n", list->list_id));
        return BCM_E_EXISTS;
    }
    ...
    if (list_link == NULL) {
        ACL_ERR(("ACL Error: allocation failure for new list node.\n"));
        return BCM_E_MEMORY;
    }
    list_link->list = list;
...
bcma_acl_rule_add(bcma_acl_list_id_t list_id, bcma_acl_rule_t *rule)
{
    ...
    ACL_VVERB(("ACL bcma_acl_rule_add(list_id=%d, rule=%p)\n",
               list_id, (void *)rule));
    ACL_IS_INIT();
    ...
    if (list_link == NULL) {
        ACL_UNLOCK(_acl_control);
        ACL_ERR(("ACL Error: List ID=%d not found\n", list_id));
        return BCM_E_NOT_FOUND;
    }

    if (_acl_rule_find(rule->rule_id) != NULL) {
        ACL_UNLOCK(_acl_control);
        ACL_ERR(("ACL Error: Rule ID=%d already exists\n", rule->rule_id));
        return BCM_E_EXISTS;
    }
    ...
    if (rule_link == NULL) {
        ACL_UNLOCK(_acl_control);
        ACL_ERR(("ACL Error: Allocation failure in _acl_rule_link_alloc()\n"));
        return BCM_E_MEMORY;
    }
    rule_link->rule = rule;
    rule_link->next = NULL;
    ...
    if (list_link->rules == NULL) {
        ...
        ACL_UNLOCK(_acl_control);
        ACL_VVERB(("ACL bcma_acl_rule_add() END\n"));
        return BCM_E_NONE;
    }
...
#define _ACL_L4_PORT_VALIDATE(_port_)                                    \
    do {                                                                 \
        if ((_port_) < _ACL_L4_MIN || _ACL_L4_MAX < (_port_)) {          \
            ACL_ERR(("ACL Error: L4 port=%d out of range\n", (_port_))); \
            return BCM_E_PARAM;                                          \
        }                                                                \
    } while (0);
...
bcma_acl_rule_remove(bcma_acl_list_id_t list_id, bcma_acl_rule_id_t rule_id)
{
    ...
    ACL_VVERB(("ACL bcma_acl_rule_remove(list_id=%d, rule_id=%d)\n",
               list_id, rule_id));
    ACL_IS_INIT();
    ...
    ACL_UNLOCK(_acl_control);
    ACL_ERR(("ACL Error: Rule ID=%d not found in List ID=%d\n",
             rule_id, list_id));
    return BCM_E_NOT_FOUND;
}
...
bcma_acl_rule_get(bcma_acl_rule_id_t rule_id, bcma_acl_rule_t **rule)
{
    ...
    ACL_VVERB(("ACL bcma_acl_rule_get(rule_id=%d, rule=%p)\n", 
               rule_id, (void *)rule));
    ACL_IS_INIT();
    ...
    if (rule_link == NULL) {
        ACL_ERR(("ACL Error: Rule ID=%d not found\n", rule_id));
        return BCM_E_NOT_FOUND;
    }
...
bcma_acl_remove(bcma_acl_list_id_t list_id)
{
    ...
    ACL_VVERB(("ACL bcma_acl_remove(list_id=%d)\n", list_id));
    ACL_IS_INIT();
...
bcma_acl_get(bcma_acl_list_id_t list_id, bcma_acl_t *list)
{
    ...
    ACL_VVERB(("ACL bcma_acl_get(list_id=%d, list=%p)\n", list_id,
              (void *)list));
    ACL_IS_INIT();

    if (list == NULL) {
        ACL_ERR(("ACL Error: *list is NULL\n"));
        return BCM_E_PARAM;
    }
...
_acl_rule_link_alloc(void) {
    ...
    ACL_VVERB(("ACL _acl_rule_link_alloc()\n"));
    ...
    ACL_VVERB(("ACL _acl_rule_link_alloc() returning=%p\n",
              (void *)rule_link));
    return rule_link;
}
...
bcma_acl_install(void)
{
    ...
    ACL_VVERB(("ACL bcma_acl_install()\n"));
    ACL_IS_INIT();
...
bcma_acl_uninstall(void)
{
    ...
    ACL_VVERB(("ACL bcma_acl_uninstall()\n"));
    ACL_IS_INIT();
    ACL_LOCK();
...
acl_range_to_list(uint16 min, uint16 max, acl_node_t **list_p, int *count) {
    ...
    if (list_p == NULL || count == NULL) {
        ACL_ERR(("ACL Error: list or count is NULL\n"));
        return BCM_E_PARAM;
    }
    ACL_VVERB(("ACL acl_range_to_list(min=%d, max=%d)\n", min, max));
    *list_p = NULL;
...
int acl_range_destroy(acl_node_t *list, int count) {
    ...
    while (count > 0) {
        node_prev = node_cur;
        if (node_prev == NULL) {
            ACL_ERR(("ACL Error: count is wrong\n"));
            return BCM_E_PARAM;
        }
...
bcma_acl_show(void)
{
    ...
    if (BCM_FAILURE(retval)) {
        ACL_ERR(("ACL Error: Device specific acl_show() failed\n"));
        return retval;
    }
...
#define ACL_SHOW_RANGE(rule, name, flag)                                  \
    if ((rule)->flags & (flag)) {                                         \
        switch (flag) {                                                   \
            case BCMA_ACL_RULE_L4_SRC_PORT:                               \
                ACL_OUT(("%s=%d-%d, ", (name), (rule)->src_port_min,      \
                             (rule)->src_port_max));                      \
                break;                                                    \
            case BCMA_ACL_RULE_L4_DST_PORT:                               \
                ACL_OUT(("%s=%d-%d, ", (name), (rule)->dst_port_min,      \
                             (rule)->dst_port_max));                      \
                break;                                                    \
            case BCMA_ACL_RULE_VLAN:                                      \
                ACL_OUT(("%s=%d-%d, ", (name), (rule)->vlan_min,          \
                             (rule)->vlan_max));                          \
                break;                                                    \
        }                                                                 \
    }

#define ACL_SHOW_MAC(rule, name, mac_str, flag)                           \
    if ((rule)->flags & (flag)) {                                         \
        ...
        ACL_OUT(("%s=%s, ", (name), (mac_str)));                          \
    }

#define ACL_SHOW_IP(rule, name, str_p, flag)                              \
    if ((rule)->flags & flag) {                                           \
        if (flag == BCMA_ACL_RULE_SRC_IP4) {                              \
            format_ipaddr((str_p), (rule->src_ip));                       \
            ACL_OUT(("%s=%s, ", (name), (str_p)));                        \
            format_ipaddr((str_p), (rule->src_ip_mask));                  \
            ACL_OUT(("%sMask=%s, ", (name), (str_p)));                    \
        }                                                                 \
...
#define ACL_SHOW_ETHERTYPE(rule, name)                                    \
    if ((rule)->flags & (BCMA_ACL_RULE_ETHERTYPE)) {                      \
        ACL_OUT(("%s=0x%04x, ", (name), rule->ether_type));               \
    }

#define ACL_SEPARATOR(first_print)                                        \
        if ((first_print) == 1) {                                         \
            (first_print) = 0;                                            \
        } else {                                                          \
            ACL_OUT((", "));                                              \
...
bcma_acl_list_show(bcma_acl_list_id_t list_id)
{
    ...
    ACL_VVERB(("ACL bcma_acl_list_show(list_id=%d)\n", list_id));
    ACL_IS_INIT();
    ...
    ACL_OUT(("acl={id=%d, lplist={", list_link->list->list_id));
...
bcma_acl_rule_show_id(bcma_acl_rule_id_t rule_id)
{
    ...
    ACL_VVERB(("ACL bcma_acl_rule_show_id(rule_id=%d)\n", rule_id));
    ACL_IS_INIT();
...
bcma_acl_rule_show(bcma_acl_rule_t *rule)
{
    ...
    ACL_VVERB(("ACL bcma_acl_rule_show(rule=%p)\n", (void *)rule));
    ACL_IS_INIT();

    if (rule == NULL) {
        ACL_ERR(("ACL Error: null rule passed to bcma_acl_rule_show()\n"));
        return BCM_E_PARAM;
    }

    ACL_VVERB(("ACL bcma_acl_rule_show(rule_id=%d)\n", rule->rule_id));

    ACL_OUT(("rule={id=%d, ", rule->rule_id));

    ACL_VERB(("flags=0x%03x, ", rule->flags));
...
bcma_acl_action_show(bcma_acl_action_t *action)
{
    ...
    ACL_VVERB(("ACL bcma_acl_action_show(action=%p)\n", (void *)action));
    ACL_IS_INIT();

    if (action == NULL) {
        ACL_ERR(("ACL Error: null action passed to bcma_acl_action_show()\n"));
        return BCM_E_PARAM;
    }
    ...
    ACL_OUT(("action={"));
    ACL_VERB(("flags=0x%x, ", action->flags));
    ...
    if (action->flags & BCMA_ACL_ACTION_REDIR) {
        ACL_SEPARATOR(first_print);
        ACL_OUT(("Redirect={lport=%d}", action->redir_port));
    }

    if (action->flags & BCMA_ACL_ACTION_MIRROR) {
        ACL_SEPARATOR(first_print);
        ACL_OUT(("Mirror={lport=%d}", action->mirror_port));
    }
