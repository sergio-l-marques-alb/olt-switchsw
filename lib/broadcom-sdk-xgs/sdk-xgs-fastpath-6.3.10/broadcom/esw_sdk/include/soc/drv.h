/*
 * $Id: drv.h,v 1.631.6.5.2.6 Broadcom SDK $
...
#define SOC_IS_ARAD(unit)        ((SOC_INFO(unit).chip_type == SOC_INFO_CHIP_TYPE_ARAD) || (SOC_INFO(unit).chip_type == SOC_INFO_CHIP_TYPE_ARADPLUS))
#define SOC_IS_ARADPLUS(unit)     (SOC_INFO(unit).chip_type == SOC_INFO_CHIP_TYPE_ARADPLUS)
...
#define SOC_IS_ARAD_B1_AND_BELOW(unit) (SOC_IS_ARAD(unit)     && (!SOC_IS_ARADPLUS_A0(unit)))
...
#define SOC_IS_DFE_TYPE(dev_type) ((dev_type) == BCM88750_DEVICE_ID || (dev_type) == BCM88754_DEVICE_ID  || (dev_type) == BCM88755_DEVICE_ID) 
...
#define SOC_IS_DPP(unit) (SOC_IS_PETRAB(unit) || SOC_IS_ARAD(unit) || SOC_IS_ACP(unit) || SOC_IS_JERICHO_P3(unit))
#define SOC_IS_DFE(unit) (SOC_IS_FE1600(unit))
...
#define NUM_OLP_PORT(unit)              (SOC_INFO(unit).olp_port > 0 ? 1 : 0)
#define NUM_ERP_PORT(unit)              (SOC_INFO(unit).erp_port > 0 ? 1 : 0)
#define NUM_OAMP_PORT(unit)             (SOC_INFO(unit).oamp_port > 0 ? 1 : 0)
...
#define EGQ_BLOCK(unit) (SOC_INFO(unit).egq_block) 
...
#define SCH_BLOCK(unit) (SOC_INFO(unit).sch_block)            
#define EPNI_BLOCK(unit) (SOC_INFO(unit).epni_block)               
...
#define SOC_INFO_CHIP_XGS            (SOC_INFO_CHIP_XGS_FABRIC | \
    ...
    int             olp_port;               /* offload processor port */
    int             erp_port;               /* egress replocation port */
    int             oamp_port;               /* OAM processor port */
...
#define SOC_RECONFIG_TDM (SOC_CONTROL(unit)->reconfig_flag)
    pbmp_t          xe_skip_pbm;            /* XE ports that don't change during reconfiguration */
    pbmp_t          ce_skip_pbm;            /* CE ports that don't change during reconfiguration */
    pbmp_t          hg_skip_pbm;            /* HG line ports that don't change during reconfiguration */
    pbmp_t          ge_skip_pbm;            /* GE ports that don't change during reconfig - not XL */
    pbmp_t          all_skip_pbm;           /* All ports to skip during reconfiguration */
    int             port_l2p_mapping[SOC_MAX_NUM_PORTS];
    int             wc0_speed;
    int             wc1_speed;
    int             wc2_speed;
    int             cl0_speed_change;       /* Indicates whether or not CL0 and PT0 need to be
                                               reset as part of reconfiguration */
    int             xt_port_count;          /* Number of XT ports before reconfiguration */
#endif
