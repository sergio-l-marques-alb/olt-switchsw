/*
 * $Id: mpls.c, 2014/09/16 ramji Exp $
 ...
 *
 * File:    mpls.c
 * Purpose: Manages Tomahawk MPLS functions
 */

#include <soc/defs.h>
#include <sal/core/libc.h>
#include <shared/bsl.h>
#if defined(BCM_TOMAHAWK_SUPPORT) && defined(BCM_MPLS_SUPPORT) && \
    defined(INCLUDE_L3)
#include <soc/drv.h>
#include <soc/hash.h>
#include <soc/mem.h>
...
STATIC int
_bcm_th_mpls_entry_get_data(int unit, mpls_entry_entry_t *ment,
...
STATIC int
_bcm_th_mpls_entry_set_key(int unit, bcm_mpls_tunnel_switch_t *info,
 ...
 */
int
bcm_th_mpls_tunnel_switch_add(int unit, bcm_mpls_tunnel_switch_t *info)
{
    mpls_entry_entry_t ment; 
    ...
    BCM_IF_ERROR_RETURN(_bcm_th_mpls_entry_set_key(unit, info, &ment));
    ...
    if (rv == SOC_E_NONE) {
        ...
        else if ((action == 0x6) && soc_feature(unit, soc_feature_mpls_lsr_ecmp)) {
            /* SWAP_ECMP */
            old_ecmp_index = soc_MPLS_ENTRYm_field32_get(unit, &ment, ECMP_PTRf);
        }
    ...
    if (tunnel_switch_update) {
        ...
        if (action ==  0x3) { /* SWAP_NHI */
         ...
         else if (soc_feature(unit, soc_feature_mpls_lsr_ecmp) && (action == 0x6)) {
            /* SWAP_ECMP */
            rv = bcm_xgs3_ecmp_group_del(unit, old_ecmp_index);
        }
    }
...
STATIC int
_bcm_th_mpls_entry_delete(int unit, mpls_entry_entry_t *ment)
{   
    ...
    if (action == 0x3) {
        ...
        rv = bcm_xgs3_nh_del(unit, 0, nh_index);
               ...
               (soc_feature(unit, soc_feature_mpls_lsr_ecmp) && (action == 0x6))) {
        /* L3_ECMP or MPLS ECMP*/
 ...
 */
int
bcm_th_mpls_tunnel_switch_delete(int unit, bcm_mpls_tunnel_switch_t *info)
{
    int rv, index;
    mpls_entry_entry_t ment;


    rv = _bcm_th_mpls_entry_set_key(unit, info, &ment);
    BCM_IF_ERROR_RETURN(rv);
    ...
    rv = _bcm_th_mpls_entry_delete(unit, &ment);
 ...
 */
int
bcm_th_mpls_tunnel_switch_delete_all(int unit)
{
 ...
 */
int
bcm_th_mpls_tunnel_switch_get(int unit, bcm_mpls_tunnel_switch_t *info)
{
    int rv, index;
    mpls_entry_entry_t ment;
    ...
    rv = _bcm_th_mpls_entry_get_data(unit, &ment, info);
