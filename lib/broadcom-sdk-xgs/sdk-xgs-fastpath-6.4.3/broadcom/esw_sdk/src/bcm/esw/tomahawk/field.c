...
STATIC int
_field_th_tcam_policy_mem_get(int unit, _field_entry_t *f_ent,
                              soc_mem_t *tcam_mem,
...
/*
 * Function:
 *     _field_th_instance_logical_tables_deinit
 * Purpose:
 *     Deinitialize a Logical Table instance in Ingress stage.
 * Parameters:
 *     unit     - (IN) BCM device number
 ...
 */
STATIC int
_field_th_instance_logical_tables_deinit(int unit,
                                         _field_stage_t *stage_fc,
                                         int inst)
{
    ...
    /* Deallocate memory allocated for LT information. */
    for (lt_idx = 0; lt_idx < stage_fc->num_logical_tables; lt_idx++) {
        if (NULL != stage_fc->lt_info[inst][lt_idx]) {
            sal_free(stage_fc->lt_info[inst][lt_idx]);
            stage_fc->lt_info[inst][lt_idx] = NULL;
        }
    }
...
_field_th_stage_delete(int unit, _field_control_t *fc, _field_stage_t *stage_fc)
{
    ...
    for (inst = 0; inst < stage_fc->num_instances; inst++) {
        ...
        if (_BCM_FIELD_STAGE_INGRESS == stage_fc->stage_id) {
            ...
            /* Deallocate logical table resources. */
            _field_th_instance_logical_tables_deinit(unit, stage_fc, inst);
        }
    }
...
_field_th_meter_pools_init(int unit, _field_stage_t *stage_fc)
{
    ...
    for (inst = 0; inst < stage_fc->num_instances; inst++) {
        for (pool_idx = 0; pool_idx < stage_fc->num_meter_pools; pool_idx++) {
            ...
            mp->size = size;
            mp->free_meters = size;
            mp->num_meter_pairs = (_IFP_TH_NUM_METER_POOLS);
...
/*
 * Function:
 *     _field_th_instance_lt_init
 * Purpose:
 *     Initialize a Logical Table configuration instance in Ingress stage.
 * Parameters:
 *     unit     - (IN) BCM device number.
 *     stage_fc - (IN) stage control structure pointer.
 *     inst     - (IN) Field Pipe Instance for which Logical tables must be
 ...
 */
STATIC int
_field_th_instance_lt_init(int unit, _field_stage_t *stage_fc, int inst)
{
    int lt_idx;                 /* Logical Table iterator.         */
    ...
    for (lt_idx = 0; lt_idx < stage_fc->num_logical_tables; lt_idx++) {
        ...
        stage_fc->lt_info[inst][lt_idx] = lt_p;
    }
    return (BCM_E_NONE);
}
...
/*
 * Function:
 *    _field_th_stage_logical_tables_init
 * Purpose:
 *    Initialize logical table information based on stage group operational
 *    mode.
 *
 * Parameters:
 *    unit      - (IN) BCM device number.
 ...
 */
STATIC int
_field_th_stage_logical_tables_init(int unit,
                                    _field_control_t *fc,
                                    _field_stage_t *stage_fc)
{
 ...
 */
int
_field_th_ingress_actions_init(int unit, _field_stage_t *stage_fc)
{
    _FP_ACTION_DECL
    uint32 offset;             /* General variable to carry offset. */
    uint32 profile_one_offset; /* Offset of PROFILE_ONE_SET. */
    uint32 profile_two_offset; /* Offset of PROFILE_TWO_SET. */
    uint32 redirect_offset;    /* Offset of REDIRECT_SET. */
    uint32 l3swl2change_offset; /* Offset of L3SW_CHANGE_L2_SET. */
    uint32 nat_offset; /* Offset of NAT_SET. */
    uint32 mirror_offset; /* Offset of MIRROR_SET. */
    uint32 lb_offset; /* Offset of LB_CONTROL_SET. */
    uint32 nat_override_offset; /* Offset of NAT_OVERRIDE_SET. */
    uint32 copytocpu_offset; /* Offset of COPY_TO_CPU_SET. */
    uint32 cutthrough_offset; /* Offset of CUT_THRU_OVERRIDE_SET. */
    uint32 urpf_offset; /* Offset of URPF_OVERRIDE_SET. */
    uint32 ttl_offset;  /* Offset of TTL_SET. */
    uint32 cpucos_offset; /* Offset of CHANGE_CPU_COS_SET. */
    uint32 drop_offset; /* Offset of DROP_SET. */
    uint32 mirror_override_offset; /* Offset of MIRROR_OVERRIDE_SET. */
    uint32 sflow_offset; /* Offset of SFLOW_SET. */
    uint32 debug_offset; /* Offset of INSTRUMENTATION_TRIGGERS_ENABLE. */
...
_field_th_actions_init(int unit, _field_stage_t *stage_fc)
{
    ...
    switch (stage_fc->stage_id) {
        case _BCM_FIELD_STAGE_INGRESS:
            return (_field_th_ingress_actions_init(unit, stage_fc));
        default:
            ;
    }
                              ...
                              _field_action_t *fa)
{
    ...
    BCM_IF_ERROR_RETURN
         (_field_th_tcam_policy_mem_get(unit, f_ent, &tcam_mem, &mem));
...
_field_th_stage_quals_ibus_map_init(int unit, _field_stage_t *stage_fc)
{
    ...
    _FP_QUAL_TWO_SEC_INFO_CTRL_SEL_ADD(unit, stage_fc,
        bcmFieldQualifySrcModPortGport, _FieldExtCtrlSrcDestCont1Sel, 1,
        _FieldKeygenExtSelDisable, 0, _FieldKeygenExtSelDisable, 0, 16,  1, 104,
        153);
    _FP_QUAL_TWO_SEC_INFO_CTRL_SEL_ADD(unit, stage_fc,
        bcmFieldQualifySrcMimGport, _FieldExtCtrlSrcDestCont1Sel, 2,
        _FieldKeygenExtSelDisable, 0, _FieldKeygenExtSelDisable, 0, 16,  1, 104,
        153);
    _FP_QUAL_TWO_SEC_INFO_CTRL_SEL_ADD(unit, stage_fc,
        bcmFieldQualifySrcNivGport, _FieldExtCtrlSrcDestCont1Sel, 2,
        _FieldKeygenExtSelDisable, 0, _FieldKeygenExtSelDisable, 0, 16,  1, 104,
        153);
    _FP_QUAL_TWO_SEC_INFO_CTRL_SEL_ADD(unit, stage_fc,
        bcmFieldQualifySrcVxlanGport, _FieldExtCtrlSrcDestCont1Sel, 2,
        _FieldKeygenExtSelDisable, 0, _FieldKeygenExtSelDisable, 0, 16,  1, 104,
        153);
    _FP_QUAL_TWO_SEC_INFO_CTRL_SEL_ADD(unit, stage_fc,
        bcmFieldQualifySrcMplsGport, _FieldExtCtrlSrcDestCont1Sel, 2,
        _FieldKeygenExtSelDisable, 0, _FieldKeygenExtSelDisable, 0, 16,  1, 104,
        153);
    _FP_QUAL_TWO_SEC_INFO_CTRL_SEL_ADD(unit, stage_fc,
        bcmFieldQualifySrcVlanGport, _FieldExtCtrlSrcDestCont1Sel, 2,
        _FieldKeygenExtSelDisable, 0, _FieldKeygenExtSelDisable, 0, 16,  1, 104,
        153);
    _FP_QUAL_TWO_SEC_INFO_CTRL_SEL_ADD(unit, stage_fc, bcmFieldQualifyDstTrunk,
        ...
        _FieldKeygenExtSelDisable, 0, 16,  1, 104, 153);
    _FP_QUAL_TWO_SEC_INFO_CTRL_SEL_ADD(unit, stage_fc, bcmFieldQualifyDstGport,
        _FieldExtCtrlSrcDestCont1Sel, 4, _FieldKeygenExtSelDisable, 0,
        _FieldKeygenExtSelDisable, 0, 16,  1, 104, 153);
    _FP_QUAL_TWO_SEC_INFO_CTRL_SEL_ADD(unit, stage_fc,
        ...
        153);
    _FP_QUAL_TWO_SEC_INFO_CTRL_SEL_ADD(unit, stage_fc,
        bcmFieldQualifyDstMimGport, _FieldExtCtrlSrcDestCont1Sel, 6,
        _FieldKeygenExtSelDisable, 0, _FieldKeygenExtSelDisable, 0, 16,  1, 104,
        153);
    _FP_QUAL_TWO_SEC_INFO_CTRL_SEL_ADD(unit, stage_fc,
        bcmFieldQualifyDstNivGport, _FieldExtCtrlSrcDestCont1Sel, 6,
        _FieldKeygenExtSelDisable, 0, _FieldKeygenExtSelDisable, 0, 16,  1, 104,
        153);
    _FP_QUAL_TWO_SEC_INFO_CTRL_SEL_ADD(unit, stage_fc,
        bcmFieldQualifyDstVxlanGport, _FieldExtCtrlSrcDestCont1Sel, 6,
        _FieldKeygenExtSelDisable, 0, _FieldKeygenExtSelDisable, 0, 16,  1, 104,
        153);
    _FP_QUAL_TWO_SEC_INFO_CTRL_SEL_ADD(unit, stage_fc,
        bcmFieldQualifyDstVlanGport, _FieldExtCtrlSrcDestCont1Sel, 6,
        _FieldKeygenExtSelDisable, 0, _FieldKeygenExtSelDisable, 0, 16,  1, 104,
        153);
    _FP_QUAL_TWO_SEC_INFO_CTRL_SEL_ADD(unit, stage_fc,
        bcmFieldQualifyDstMplsGport, _FieldExtCtrlSrcDestCont1Sel, 6,
        _FieldKeygenExtSelDisable, 0, _FieldKeygenExtSelDisable, 0, 16,  1, 104,
        153);
    _FP_QUAL_TWO_SEC_INFO_CTRL_SEL_ADD(unit, stage_fc,
    ...
    _FP_QUAL_SEC_INFO_ADD(unit, stage_fc, bcmFieldQualifyTos,
        _FieldKeygenExtSelL1E8, 12, 8, 96);
    _FP_QUAL_SEC_INFO_ADD(unit, stage_fc, bcmFieldQualifyTtl,
    ...
    _FP_QUAL_SEC_INFO_ADD(unit, stage_fc, bcmFieldQualifyIpProtocol,
        _FieldKeygenExtSelL1E8, 16, 8, 128);
    _FP_QUAL_TWO_SEC_INFO_ADD(unit, stage_fc, bcmFieldQualifyTcpControl,
        _FieldKeygenExtSelL1E4, 2, _FieldKeygenExtSelL1E4, 3, 4, 4, 8,
        12);
    ...
    _FP_QUAL_TWO_SEC_INFO_CTRL_SEL_ADD(unit, stage_fc, bcmFieldQualifySrcTrunk,
        _FieldExtCtrlSrcContASel, 1, _FieldKeygenExtSelL1E16, 10,
        _FieldKeygenExtSelL1E2, 3, 16, 2, 160, 6);
    _FP_QUAL_SEC_INFO_ADD(unit, stage_fc, bcmFieldQualifySrcPort,
        _FieldKeygenExtSelL1E16, 10, 16, 160);
    _FP_QUAL_SEC_INFO_ADD(unit, stage_fc, bcmFieldQualifySrcPort,
        _FieldKeygenExtSelL1E16, 11, 16, 176);
    ...
    _FP_QUAL_TWO_SEC_INFO_ADD(unit, stage_fc, bcmFieldQualifyVrf,
        ...
        116);
    _FP_QUAL_SEC_INFO_ADD(unit, stage_fc, bcmFieldQualifySrcClassL2,
    ...
    _FP_QUAL_TWO_SEC_INFO_CTRL_SEL_ADD(unit, stage_fc, bcmFieldQualifySrcTrunk,
        ...
        _FieldKeygenExtSelL1E2, 3, 16,  2, 176, 6);
    _FP_QUAL_TWO_SEC_INFO_CTRL_SEL_ADD(unit, stage_fc,
        bcmFieldQualifySrcMplsGport, _FieldExtCtrlSrcContBSel, 2,
        _FieldKeygenExtSelL1E16, 11, _FieldKeygenExtSelL1E2, 3, 16,  2, 176, 6);
    _FP_QUAL_SEC_INFO_CTRL_SEL_ADD(unit, stage_fc,
        bcmFieldQualifySrcClassField, _FieldExtCtrlClassIdContASel, 5,
        _FieldKeygenExtSelL1E16, 15, 16, 240);
    _FP_QUAL_SEC_INFO_CTRL_SEL_ADD(unit, stage_fc,
    ...
    _FP_QUAL_SEC_INFO_CTRL_SEL_ADD(unit, stage_fc,
        ...
        _FieldKeygenExtSelL1E16, 13, 16, 208);
    _FP_QUAL_SEC_INFO_CTRL_SEL_ADD(unit, stage_fc,
        bcmFieldQualifyInterfaceClassL2, _FieldExtCtrlClassIdContCSel, 5,
        _FieldKeygenExtSelL1E16, 13, 16, 208);
    _FP_QUAL_SEC_INFO_CTRL_SEL_ADD(unit, stage_fc,
        bcmFieldQualifyInterfaceClassL3, _FieldExtCtrlClassIdContCSel, 6,
        _FieldKeygenExtSelL1E16, 13, 16, 208);
    ...
    _FP_QUAL_FIVE_SEC_INFO_ADD(unit, stage_fc, _bcmFieldQualifyData0,
        _FieldKeygenExtSelL1E32, 8, _FieldKeygenExtSelL1E32, 9,
        _FieldKeygenExtSelL1E32, 10, _FieldKeygenExtSelL1E16, 8,
        _FieldKeygenExtSelL1E16, 9, 32, 32, 32, 16, 16, 256, 288, 320, 128,
        144);
    _FP_QUAL_FIVE_SEC_INFO_ADD(unit, stage_fc, _bcmFieldQualifyData1,
        _FieldKeygenExtSelL1E32, 5, _FieldKeygenExtSelL1E32, 6,
        _FieldKeygenExtSelL1E32, 7, _FieldKeygenExtSelL1E16, 6,
        _FieldKeygenExtSelL1E16, 7, 32, 32, 32, 16, 16, 160, 192, 224, 96,
        112);
...
_field_th_ingress_qualifiers_init(int unit, _field_stage_t *stage_fc)
{
    ...
    _FP_QUAL_ADD(unit, stage_fc, bcmFieldQualifyOuterVlanPri,
        _bcmFieldSliceSelDisable, 0, 13, 3);
    _FP_QUAL_ADD(unit, stage_fc, bcmFieldQualifyOuterVlanId,
    ...
    _FP_QUAL_MULTI_INFO_ADD(unit, stage_fc, bcmFieldQualifyL2SrcHit,
        _bcmFieldSliceSelDisable, 0, 0, 1, 2);
    _FP_QUAL_MULTI_INFO_ADD(unit, stage_fc, bcmFieldQualifyL2SrcStatic,
        _bcmFieldSliceSelDisable, 0, 0, 1, 3);
    ...
    _FP_QUAL_MULTI_INFO_ADD(unit, stage_fc, bcmFieldQualifyDstIpLocal,
        _bcmFieldSliceSelDisable, 0, 0, 1, 1);
    ...
    _FP_QUAL_ADD(unit, stage_fc, bcmFieldQualifySrcClassField,
        _bcmFieldSliceSelDisable, 0, 0, 16);
    _FP_QUAL_ADD(unit, stage_fc, bcmFieldQualifySrcModuleGport,
        _bcmFieldSliceSelDisable, 0, 0, 17);
...
_bcm_field_th_ingress_stat_hw_free (int unit, _field_entry_t *f_ent)
{
    ...
    BCM_IF_ERROR_RETURN(_field_th_tcam_policy_mem_get(unit, f_ent, &tcam_mem, &policy_mem));
                                    ...
                                    _field_entry_t *f_ent)
{
    ...
    int    fp_stat_mode_max;  /* Max stat modes.                 */
    uint8  idx;               /* HW counter modes iterator.      */
    uint8  counter_allocated; /* To indicate counter is
                               * allocated or not.
                               */
    uint8  c_pool;            /* counter pool number.            */
    ...
    uint32 mode_id;           /* Flex stat mode Id.              */
    uint32 req_bmap;          /* Requested statistics bitmap.    */
    uint32 hw_bmap;           /* HW supported statistics bitmap. */
    ...
    _bcm_ifp_color_map_t    ifp_counter_color; /* Flex stat Color.          */
    ...
    bcm_stat_group_mode_attr_selector_t attr_sel; /* Flext stat Attribute selector structure. */
    ...
    if (_FP_INVALID_INDEX == f_st->hw_index) {
        ...
        num_selectors = 0;
        rv = _bcm_esw_stat_group_mode_id_create(unit,
                                                BCM_STAT_GROUP_MODE_INGRESS,
                                                num_hw_cntrs, num_selectors,
                                                &attr_sel, &mode_id);
        ...
        /* First try allocating counters from one of the existing
         * pools of the group.
         */
        counter_allocated = 0;
        for (c_pool = 0; c_pool < _FIELD_MAX_COUNTER_POOLS; c_pool++) {
            if (_FP_COUNTER_POOL_BMP_TEST(
                                 f_ent->group->counter_pool_bmp, c_pool)) {
                _bcm_esw_stat_get_counter_id(0, 0, 0, c_pool, 0, &flex_sid);
                num_hw_cntrs = 0;
                rv = _bcm_esw_stat_custom_group_create(unit, mode_id,
                                              bcmStatObjectIngFieldStageIngress,
                                              &flex_sid, &num_hw_cntrs);
                if (BCM_E_RESOURCE == rv) {
                    /* No free counters are avavilable in the given pool
                     * so try with next pool of the group.
                     */
                    continue;
                } else if (BCM_FAILURE(rv)) {
                    /* Error(othet than BCm E_RESOURCE) in allocating counters
                     * from a given pool fo the group so simply return error.
                     */
                    return rv;
                } else {
                    /* Counters are allocated successfully */
                    counter_allocated = 1;
                    break;
                }
            }
        ...
        /* if no free counters are avaiable in pools already allocated
         * for the group, then allocate counters from next available pool.
         */
        if (!counter_allocated) {
            /* Create Custom Flex Stat Group */
            num_hw_cntrs = 0;
            flex_sid = -1;
            rv = _bcm_esw_stat_custom_group_create(unit, mode_id,
                                                   bcmStatObjectIngFieldStageIngress,
        ...
        switch (color) {
            case _bcmFieldStatColorGreen:
                ifp_counter_color.map[0] = BCM_STAT_FLEX_COLOR_GREEN;
                ifp_counter_color.map[1] = 0;
                ifp_counter_color.map[2] = 0;
                break;
            case _bcmFieldStatColorYellow:
                ifp_counter_color.map[0] = BCM_STAT_FLEX_COLOR_YELLOW;
                ifp_counter_color.map[1] = 0;
                ifp_counter_color.map[2] = 0;
                break;
            case _bcmFieldStatColorRed:
                ifp_counter_color.map[0] = BCM_STAT_FLEX_COLOR_RED;
                ifp_counter_color.map[1] = 0;
                ifp_counter_color.map[2] = 0;
                break;
            ...
            case _bcmFieldStatColorGreenYellow:
                ifp_counter_color.map[0] = BCM_STAT_FLEX_COLOR_GREEN;
                ifp_counter_color.map[1] = BCM_STAT_FLEX_COLOR_YELLOW;
                ifp_counter_color.map[2] = 0;
                break;
            case _bcmFieldStatColorGreenRed:
                ifp_counter_color.map[0] = BCM_STAT_FLEX_COLOR_GREEN;
                ifp_counter_color.map[1] = BCM_STAT_FLEX_COLOR_RED;
                ifp_counter_color.map[2] = 0;
                break;
            ...
            case _bcmFieldStatColorYellowNotYellow:
                ifp_counter_color.map[0] = BCM_STAT_FLEX_COLOR_YELLOW;
                ifp_counter_color.map[1] = (BCM_STAT_FLEX_COLOR_GREEN |
                                            BCM_STAT_FLEX_COLOR_RED);
                ifp_counter_color.map[2] = 0;
                break;
            ...
            case _bcmFieldStatColorGreenYellowRed:
                ifp_counter_color.map[0] = BCM_STAT_FLEX_COLOR_GREEN;
                ifp_counter_color.map[1] = BCM_STAT_FLEX_COLOR_YELLOW;
                ifp_counter_color.map[2] = BCM_STAT_FLEX_COLOR_RED;
                break;
            default:
                return BCM_E_INTERNAL;
        }

        /* Set the color map of flex counters created for the stat attached
         *  to the entry.
         */
        rv = _bcm_esw_ifp_color_map_set(unit, flex_sid, &ifp_counter_color);
        BCM_IF_ERROR_RETURN(rv);
                               ...
                               _field_policer_t *f_pl)
{
    ...
    soc_mem_t policer_mem[_FP_MAX_NUM_PIPES]; /* Meter table name's
                                                    of all pipes.             */
    _field_stage_t *stage_fc;           /* Stage Field control structure.     */
...
_field_th_pipes_hw_clear(int unit, _field_stage_t *stage_fc)
{
    ...
    soc_mem_t udf_tcam[_FP_MAX_NUM_PIPES] = /* UDF_TCAM memory names. */
        {
            FP_UDF_TCAM_PIPE0m,
            FP_UDF_TCAM_PIPE1m,
            FP_UDF_TCAM_PIPE2m,
    ...
    soc_mem_t udf_offset[_FP_MAX_NUM_PIPES] =  /* UDF_OFFSET memroy names. */
        {
    ...
    switch (stage_fc->stage_id) {
        case _BCM_FIELD_STAGE_INGRESS:
            for (pipe = 0; pipe < stage_fc->num_pipes; pipe++) {
                BCM_IF_ERROR_RETURN(soc_mem_clear(unit, udf_tcam[pipe],
                    COPYNO_ALL, TRUE));
                BCM_IF_ERROR_RETURN(soc_mem_clear(unit, udf_offset[pipe],
                    COPYNO_ALL, TRUE));
...
_field_th_stage_extractors_init(int unit, _field_stage_t *stage_fc)
{
    ...
    _FP_EXT_CONFIG_ADD(unit, stage_fc, emode, part, level, gran, (ext_num + 2),
        ...
        (_FP_EXT_ATTR_NOT_IN_EIGHTY_BIT));
    _FP_EXT_CONFIG_ADD(unit, stage_fc, emode, part, level, gran, (ext_num + 3),
    ...
    for (idx = 0; idx < num_pass_thru_bits; idx++) {
        _FP_EXT_CONFIG_ADD(unit, stage_fc, emode, part, level, gran,
            ...
            (96 + idx), 0, 0, _FP_EXT_ATTR_PASS_THRU);
    }
    ...
    for (idx = 6; idx < 10; idx++) {
        _FP_EXT_CONFIG_ADD(unit, stage_fc, emode, part, level, gran,
            (ext_num + idx), _FieldKeygenExtSelL3E4, _FieldKeygenExtSelL4,
            80 + (idx * gran), 0, 0, _FP_EXT_ATTR_NOT_WITH_SRC_DST_CONT_1);
    }
    ...
    _FP_EXT_CONFIG_ADD(unit, stage_fc, emode, 0, level, gran, ext_num + 2,
        _FieldKeygenExtSelL2AE16, _FieldKeygenExtSelL3AE16, 32, 0, 0, 0);
    _FP_EXT_CONFIG_ADD(unit, stage_fc, emode, 0, level, gran, ext_num + 3,
        _FieldKeygenExtSelL2AE16, _FieldKeygenExtSelL3AE16, 48, 0, 0, 0);
    ...
    /* Overlay with _FP_POST_MUX_SRC_DST_CONT_0. */
    _FP_EXT_CONFIG_ADD(unit, stage_fc, emode, 0, level, gran, ext_num + 2,
        _FieldKeygenExtSelL3AE16, _FieldKeygenExtSelL4A, 48, 0, 0,
        _FP_EXT_ATTR_NOT_WITH_SRC_DST_CONT_0);
    _FP_EXT_CONFIG_ADD(unit, stage_fc, emode, 0, level, gran, ext_num + 3,
        _FieldKeygenExtSelL3AE16, _FieldKeygenExtSelL4A, 64, 0, 0, 0);
    ...
    _FP_EXT_CONFIG_ADD(unit, stage_fc, emode, 2, level, gran, ext_num + 8,
        _FieldKeygenExtSelL2CE16, _FieldKeygenExtSelL3CE16, 64, 0, 0, 0);
    _FP_EXT_CONFIG_ADD(unit, stage_fc, emode, 2, level, gran, ext_num + 9,
        _FieldKeygenExtSelL2CE16, _FieldKeygenExtSelL3CE16, 80, 0, 0, 0);
...
_bcm_field_th_entry_install(int unit, _field_entry_t *f_ent, int tcam_idx)
{
    ...
    rv = _field_th_tcam_policy_mem_get(unit, f_ent,
                                       &tcam_mem, &policy_mem);
    BCM_IF_ERROR_RETURN(rv);
                                          ...
                                          _field_tcam_t *tcam)
{
    ...
    BCM_IF_ERROR_RETURN
        (_field_th_tcam_policy_mem_get(unit, f_ent,
                                       &tcam_mem, &policy_mem));
...
_field_th_tcam_policy_clear(int unit, _field_entry_t *f_ent, int tcam_idx)
{
    ...
    BCM_IF_ERROR_RETURN
        (_field_th_tcam_policy_mem_get(unit, f_ent, &tcam_mem, &policy_mem));
    ...
    bcm_field_group_oper_mode_t mode)
{
   ...
   memset(set_mode, ser_mode, sizeof(set_mode));
   ...
   switch (stage_id) {
     ...
     case _BCM_FIELD_STAGE_INGRESS:
          ...
          tcam_mem[1] = SRC_COMPRESSIONm;
          tcam_mem[2] = SRC_COMPRESSION_TCAM_ONLYm;
          tcam_mem[3] = DST_COMPRESSIONm;
          tcam_mem[4] = DST_COMPRESSION_TCAM_ONLYm;
          tcam_mem[5] = IFP_LOGICAL_TABLE_SELECTm;
          tcam_mem[6] = IFP_LOGICAL_TABLE_SELECT_TCAM_ONLYm;
          tcam_mem[7] = IFP_TCAMm;
          tcam_mem[8] = IFP_TCAM_WIDEm;
          /* Set IFP_TCAM_WIDE Memory in always Unique mode */
          set_mode[8] = _SOC_SER_MEM_MODE_PIPE_UNIQUE;
          num_tcam = 9;
          break;
     case _BCM_FIELD_STAGE_EGRESS:
                          ...
                          int qual, uint32 *data, uint32 *mask)
{
    ...
    _bcm_field_qual_offset_t q_offset;
    _field_entry_t           *f_ent;
    _bcm_field_qual_data_t   out_data;
    _bcm_field_qual_data_t   out_mask;
    uint32                   multi_width = 0;
    ...
    for (part_idx = 0; part_idx < parts_cnt; ++part_idx) {
        /* Retrieve the Qualifier offsets for the particular entry part */  
        rv =
           (_bcm_field_th_qual_part_offset_get(unit, f_ent + part_idx, part_idx,
                                               qual, &q_offset));
        if (rv == BCM_E_NOT_FOUND) {
            continue;
        } else if (BCM_FAILURE(rv)) {
        ...
        _FP_QUAL_DATA_CLEAR(out_data);
        _FP_QUAL_DATA_CLEAR(out_mask);

        /* Split primary portion of the qualifier. */
        rv = _bcm_field_qual_partial_data_get(data, start_offset,
                                              multi_width, out_data);
        BCM_IF_ERROR_RETURN(rv);
        rv = _bcm_field_qual_partial_data_get(mask, start_offset,
                                              multi_width, out_mask);
        BCM_IF_ERROR_RETURN(rv);
...
_bcm_field_th_entry_phys_dump(int unit, _field_entry_t *f_ent)
{
    ...
    for (qual_idx = 0; qual_idx < bcmFieldQualifyCount; qual_idx++) {
        ...
        if (efp_slice_mode <= 5) { /* Valid values */
            ...
            for (idx = 0; idx < slice_ent_cnt; idx++) {
                ...
                for (i = 0; i < parts_count; i++) {
                    ...
                    rv = _bcm_field_tcam_part_to_entry_flags(unit, i, fg->flags,
                                                             &f_ent[i].flags);
                    if (BCM_FAILURE(rv)) {
                        sal_free(f_ent);
                        goto cleanup;
    ...
    for (slice_idx = 0; slice_idx < stage_fc->tcam_slices; slice_idx++) {
        ...
        for(part_index = parts_count - 1; part_index >= 0; part_index--) {
            ...
            if (0 == (entry_flags & _FP_ENTRY_SECOND_HALF)) {
                ...
                stage_fc->slices[_FP_DEF_INST][old_physical_slice + slice_num].next =
                    fs;
                fs->prev =
                    &stage_fc->slices[_FP_DEF_INST][old_physical_slice + slice_num];
            }
        }
