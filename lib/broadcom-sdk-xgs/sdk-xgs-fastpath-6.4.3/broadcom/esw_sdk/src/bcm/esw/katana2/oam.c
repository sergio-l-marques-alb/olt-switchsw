    ...
    if (h_data_p->oam_domain != _BCM_OAM_DOMAIN_BHH) {
        ...
        if (BCM_XGS3_L3_EGRESS_IDX_VALID(unit, h_data_p->egress_if)) {
            soc_EGR_MP_GROUPm_field32_set (unit, egr_mp_grp_key_p, NHIf, 
                            h_data_p->egress_if - BCM_XGS3_EGRESS_IDX_MIN);
        } else {
            if (BCM_XGS3_DVP_EGRESS_IDX_VALID(unit, h_data_p->egress_if)) {
                soc_EGR_MP_GROUPm_field32_set (unit, egr_mp_grp_key_p, NHIf, 
                            h_data_p->egress_if - BCM_XGS3_DVP_EGRESS_IDX_MIN);
            }
        }
    }
#endif
     ...
     */
        ...
        if (h_data_p->is_remote) {
            ...
            */
            ...
            if (!(h_data_p->flags2 & BCM_OAM_ENDPOINT2_REDIRECT_TO_CPU)) {
        ...
        if (mdl_bitmap & (1 << mdl)) {
            ...
            switch (h_data_p->oam_domain) {
                ...
                case _BCM_OAM_DOMAIN_VP:
                    ...
                    h_data_p->flags |= BCM_OAM_ENDPOINT_PBB_TE;
                    break;
                default :
                    break;
...
_bcm_kt2_oam_reinit(int unit)
{
    ...
    if (recovered_ver >= BCM_WB_VERSION_1_1) {
        rv = _bcm_kt2_oam_wb_group_recover(unit, stable_size, &oam_scache, 1);
...
bcm_kt2_oam_detach(int unit)
{
    ...
    /*Since FP init happens before OAM init, data qualifier might already have 
      been destroyed hence BCM_E_NOT_FOUND is not an error for this operation*/
    ...
    rv = bcm_esw_field_data_qualifier_destroy(unit, oc->bhh_qual_ach.qual_id);
    if (rv != BCM_E_NOT_FOUND && BCM_FAILURE(rv)) {
        if (NULL != oc->oc_lock) {
            _BCM_OAM_UNLOCK(oc);
        }
    ...
    rv = bcm_esw_field_data_qualifier_destroy(unit, oc->bhh_qual_label.qual_id);
    if (rv != BCM_E_NOT_FOUND && BCM_FAILURE(rv)) {
        if (NULL != oc->oc_lock) {
            _BCM_OAM_UNLOCK(oc);
        }
        ...
        if (group_info->flags & BCM_OAM_GROUP_REMOTE_DEFECT_TX) { 
            ...
            while (NULL != cur) {
                ...
                if (h_data_p->oam_domain == _BCM_OAM_DOMAIN_BHH) {
                    ...
                    buffer     = oc->dma_buffer;
                    buffer_ptr = 
                        shr_bhh_msg_ctrl_sess_set_pack(buffer, &msg_sess);
                    buffer_len = buffer_ptr - buffer;
                                ...
                                _bcm_oam_hash_key_t  *hash_key)
{
    ...
    /* Vlaidate EP Id if BCM_OAM_ENDPOINT_WITH_ID flag is set */
    if (endpoint_info->flags & BCM_OAM_ENDPOINT_WITH_ID) {
    ...
    if((is_remote) && (endpoint_info->local_id < 
                       _BCM_OAM_BHH_KT2_ENDPOINT_OFFSET)) {
        return (BCM_E_PARAM);
    }
    ...
    */
    ...
    if (is_remote) {
        ...
        if(!(endpoint_info->flags2 & BCM_OAM_ENDPOINT2_REDIRECT_TO_CPU)){
            /*
        ...
        if (BCM_XGS3_DVP_EGRESS_IDX_VALID(unit, endpoint_info->intf_id)) {
            egr_nh_index = endpoint_info->intf_id - BCM_XGS3_DVP_EGRESS_IDX_MIN;
        }
        else {
            egr_nh_index = endpoint_info->intf_id - BCM_XGS3_EGRESS_IDX_MIN;
        }
...
_bcm_kt2_oam_bhh_callback_thread(void *param)
{
    ...
    while (1) {
        ...
        if(!(event_mask & BHH_CCM_EVENT_SET_OR_CLEAR_MASK))
        {
            /*Handle event clear section once new flag API is approved*/
            continue;/*as of now*/
