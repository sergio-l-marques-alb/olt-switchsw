...
_bcm_td2_l3_entry_add(int unit, _bcm_l3_cfg_t *l3cfg, int nh_idx)
{
    ...
    if (embedded_nh) {
        ...
        modid   = (SOC_MODID_MAX(unit) & l3cfg->l3c_modid) <<
                   port_mask_len;
        glp     = modid | port_id;
 
        if (l3cfg->l3c_flags & BCM_L3_TGID) {
            glp |= (1 << (port_mask_len + modid_mask_len));
        }
...
#ifdef BCM_TOMAHAWK_SUPPORT
        BCM_IF_ERROR_RETURN
            (_bcm_th_l3_vp_entry_del(unit, ipv6, l3cfg, buf_entry));
#endif
        }
    }
                        ...
                        int cf_max, int *cf_count)
{
    ...
    *cf_count = 0;
    ...
    for (bidx = 9; bidx >= (6 - shared_l3_banks); bidx--) {
        ...
        index = soc_td2_l3x_entry_hash(unit, bidx, hash_offset, use_lsb, bufp);
...
_bcm_td2_l3_ip4_options_profile_idx2id(int unit, int hw_idx, int *profile_id)
{
    ...
    for (id = 0; id < _BCM_IP4_OPTIONS_LEN; id++) {
        if (_BCM_IP4_OPTIONS_USED_GET(unit, id)) {
            if (L3_INFO(unit)->ip4_profiles_hw_idx[id] * _BCM_IP_OPTION_PROFILE_CHUNK == hw_idx) {
                *profile_id = id;
                L3_UNLOCK(unit);
                return BCM_E_NONE;
            }
