                                 ...
                                 bcm_gport_t *gport)
{
    ...
    int sp_group_idx = -1, mod_port_index = 0, modid, portid, port, base_index;
    ...
    /* Check if the number of subports on this physical port
       is already reached? */
    if(*(_bcm_subport_group_subport_port_count[unit] + sp_group_idx) >= 80) {
        return BCM_E_FULL;
    }
    ...
    /* Extract the base for the mod in question */
    SOC_IF_ERROR_RETURN(READ_SOURCE_TRUNK_MAP_MODBASEm(unit,
                MEM_BLOCK_ANY, modid, &stm_modbase_entry));
    base_index = soc_SOURCE_TRUNK_MAP_MODBASEm_field32_get(unit,
                     &stm_modbase_entry, BASEf);

    /* Compute the (mod,port) index and update the egress memory */
    mod_port_index = base_index + port;
...
/*
 * Function:
 *      bcmi_xgs5_subport_group_resolve
 * Purpose:
...
int
bcmi_xgs5_subport_group_resolve(int unit,
...
/*
 * Function:
 *      bcmi_xgs5_subport_port_resolve
 * Purpose:
...
int
bcmi_xgs5_subport_port_resolve(int unit,
                               bcm_gport_t subport_port_gport,
                               bcm_module_t *modid, bcm_port_t *port,
                               bcm_trunk_t *trunk_id, int *id)
{
