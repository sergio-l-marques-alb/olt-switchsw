...
bcm_tr2_failover_ecmp_prot_nhi_set(int unit, int ecmp, int index, 
                                   ...
                                   int prot_nh_index)
{
    int i, base_ptr, count, nhi, idx_max;
    ...
    if (ecmp < 0) {
        ...
        BCM_IF_ERROR_RETURN(
            soc_mem_read(unit, INITIAL_L3_ECMP_GROUPm,
                         MEM_BLOCK_ANY, ecmp, hw_buf));
        base_ptr = soc_mem_field32_get(unit, INITIAL_L3_ECMP_GROUPm, hw_buf, 
                                       BASE_PTRf);
            
        count    = soc_mem_field32_get(unit, INITIAL_L3_ECMP_GROUPm, hw_buf, 
                                       COUNTf);
    }
    count++; /* Count is zero based. */ 
