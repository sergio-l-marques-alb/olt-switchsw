...
_bcm_xgs3_l3_intf_init(int unit)
{
    ...
    BCM_XGS3_L3_IF_TBL_SIZE(unit) = (tbl_ptr->idx_max - tbl_ptr->idx_min) + 1;
        ...
        int32 *hostmode, int32 *intfMapmode, int *ecmp_max_paths, int *ecmp_refcnt)
{
    ...
    *intfMapmode = 0;
    ...
    if (stable_size > 0) { /* Limited and Extended recovery only */
        ...
        /* each bit save per additional 1K L3_ECMP_GROUP entry valid/invalid */
        if (soc_feature(unit, soc_feature_l3) &&
            soc_feature(unit, soc_feature_l3_ecmp_2k_groups)) {
            alloc_sz += SHR_BITALLOCSIZE(BCM_L3_MAX_ECMP_GROUPS);
        }
#endif /* BCM_TOMAHAWK_SUPPORT */
...
#ifdef BCM_TOMAHAWK_SUPPORT
        /* ECMP group resorting flag for the additional 1K groups */
        if (soc_feature(unit, soc_feature_l3) &&
            soc_feature(unit, soc_feature_l3_ecmp_2k_groups)) {
            alloc_sz += SHR_BITALLOCSIZE(BCM_L3_MAX_ECMP_GROUPS);
        }
        ...
        if (BCM_SUCCESS(rv)) {
...
#ifdef BCM_TOMAHAWK_SUPPORT
                    SHR_BITDCL egbitmap2k[_SHR_BITDCLSIZE(2 * BCM_L3_MAX_ECMP_GROUPS)] = {0};
                    if (soc_feature(unit, soc_feature_l3_ecmp_2k_groups)) {
                        sal_memcpy(egbitmap2k, l3_scache_ptr,
                           SHR_BITALLOCSIZE(2 * BCM_L3_MAX_ECMP_GROUPS));
                        l3_scache_ptr += SHR_BITALLOCSIZE(2 * BCM_L3_MAX_ECMP_GROUPS);
                        for (i = 0; i < BCM_L3_ECMP_MAX_2K_GROUPS; i++) {
                            if (!SHR_BITGET(egbitmap2k, i)) {
                                ecmp_refcnt[i] = 0;
                            }
                        }
                    } else 
#endif /* BCM_TOMAHAWK_SUPPORT */
                    {
                        sal_memcpy(egbitmap, l3_scache_ptr,
                           SHR_BITALLOCSIZE(BCM_L3_MAX_ECMP_GROUPS));
...
#ifdef BCM_TOMAHAWK_SUPPORT
                if (soc_feature(unit, soc_feature_l3_ecmp_2k_groups)) {
                    /* Allocate scache space for storing addtional 1k ecmp ref counts */
                    additional_scache_sz += SHR_BITALLOCSIZE(BCM_L3_MAX_ECMP_GROUPS);
                }
#endif /* BCM_TOMAHAWK_SUPPORT */
...
#ifdef BCM_TOMAHAWK_SUPPORT
                    SHR_BITDCL egbiflgtmap2k[_SHR_BITDCLSIZE(BCM_L3_ECMP_MAX_2K_GROUPS)] = {0};
                    if (soc_feature(unit, soc_feature_l3_ecmp_2k_groups)) {
                        sal_memcpy(egbiflgtmap2k, l3_scache_ptr,
                                   SHR_BITALLOCSIZE(BCM_L3_ECMP_MAX_2K_GROUPS));
                        l3_scache_ptr += SHR_BITALLOCSIZE(BCM_L3_ECMP_MAX_2K_GROUPS);
                        for (i = 0; i < BCM_L3_ECMP_MAX_2K_GROUPS; i++) {
                            if (SHR_BITGET(egbiflgtmap2k, i)) {
...
#endif /* BCM_TOMAHAWK_SUPPORT */
                    {
                        sal_memcpy(egbiflgtmap, l3_scache_ptr,
                                   SHR_BITALLOCSIZE(BCM_L3_MAX_ECMP_GROUPS));
                        l3_scache_ptr += SHR_BITALLOCSIZE(BCM_L3_MAX_ECMP_GROUPS);
                        for (i = 0; i < BCM_L3_MAX_ECMP_GROUPS; i++) {
...
#ifdef BCM_TOMAHAWK_SUPPORT
                if (soc_feature(unit, soc_feature_l3_ecmp_2k_groups)) {
                    /* Allocate scache space for storing addtional 1k ecmp restoring flag */
                    additional_scache_sz += SHR_BITALLOCSIZE(BCM_L3_MAX_ECMP_GROUPS);
                }
#endif /* BCM_TOMAHAWK_SUPPORT */
            }
            ...
            if (soc_feature(unit, soc_feature_l3) &&
                ...
                BCM_IF_ERROR_RETURN
                    (bcm_th_l3_ecmp_rh_member_recover(unit, &l3_scache_ptr));
            }
#endif /* BCM_TOMAHAWK_SUPPORT */
...
_bcm_xgs3_l3_reinit(int unit)
{
    ...
    static int ecmp_refcnt[2 * BCM_L3_MAX_ECMP_GROUPS];
     ...
     */
    for (i = 0; i < (2 * BCM_L3_MAX_ECMP_GROUPS); i++) {
        ecmp_refcnt[i] = 1;
    }
...
_bcm_xgs3_l3_ecmp_table_init(int unit)
{
    ...
    if (SOC_IS_TOMAHAWK(unit)) {
         ...
         * Set to default of 1024 and use 16K only for RH.
         */
...
#ifdef BCM_TOMAHAWK_SUPPORT
    if (soc_feature(unit, soc_feature_ecmp_resilient_hash_optimized)) {
        BCM_IF_ERROR_RETURN(bcm_th_ecmp_rh_init(unit));
    }
#endif /* BCM_TOMAHAWK_SUPPORT */
...
_bcm_xgs3_egress_l3_intf_id_alloc(int unit, int *id)
{
    ...
    for (idx = BCM_XGS3_L3_INTF_IDX_MIN(unit);
            idx < BCM_XGS3_L3_IF_TBL_SIZE(unit); idx++) {
        /* Skip cpu interface. */
...
bcm_xgs3_l3_intf_id_create(int unit, _bcm_l3_intf_cfg_t *intf_info)
{
    ...
    if (intf_info->l3i_index < BCM_XGS3_L3_INTF_IDX_MIN(unit) ||
        ...
        if (group_size != -1) {
            if (!group_size) {
                ...
                if (soc_feature(unit, soc_feature_l3) &&
                    soc_feature(unit, soc_feature_ecmp_resilient_hash_optimized) &&
                    group_size > 1024) {
                    /* This is a dummy value and placeholder for the RH
                     * count to be programmed while setting the loadbalancing mode
                     * Capping this value since the count field is only 10 bits wide
                     */
                    group_size = 1024;
                }
#endif /* BCM_TOMAHAWK_SUPPORT */
        ...
        if (BCM_XGS3_L3_EGRESS_MODE_ISSET(unit)) {
            BCM_IF_ERROR_RETURN
              (_bcm_xgs3_ecmp_max_grp_size_get(unit, ecmp_group_id,
                                              &max_paths));
            if (soc_feature(unit, soc_feature_l3_ecmp_1k_groups)) {
                /* TBD revist this when other TH changes are done */
                if (!SOC_IS_TOMAHAWK(unit) && max_paths > 1024) {
                    max_paths = 1024;
                }
                /*For TD+*/
                if (SOC_IS_TRIDENT(unit)){
                        ...
                        int *group_index)
{
    ...
    if (soc_feature(unit, soc_feature_l3_dynamic_ecmp_group)) {
        if (BCM_XGS3_L3_EGRESS_MODE_ISSET(unit)) {
              /* Limit Max_paths */
            if (soc_feature(unit, soc_feature_l3_ecmp_1k_groups)) {
                if (!(ecmp_flags & BCM_L3_ECMP_RH_OPT) && (max_paths > 1024)) {
                    max_paths = 1024;
                }
                if (SOC_IS_TRIDENT(unit)){
                    SOC_IF_ERROR_RETURN
    ...
    if (old_tgid != -1) {
        ...
        if (is_local) {
            portid = soc_mem_field32_get(unit, ING_L3_NEXT_HOPm,
                                         &ing_nh, PORT_NUMf);
            if (old_portid != portid) {
                local_ports[num_local_ports++] = old_portid;
            }
        }
    }
                                         ...
                                         int incr)
{
    ...
    if (flags & BCM_L3_MULTIPATH) {
        ...
        tbl_ptr = BCM_XGS3_L3_TBL_PTR(unit, next_hop);
        if (BCM_XGS3_L3_EGRESS_IDX_VALID(unit, intf)) {
            nh_id = intf - BCM_XGS3_EGRESS_IDX_MIN;
        } else {
            nh_id = intf - BCM_XGS3_DVP_EGRESS_IDX_MIN;
        }
    }
                                           ...
                                           bcm_if_t *intf_array, bcm_if_t *mpintf)
{
    ...
    if (max_paths == 0) {
        ...
        if (soc_feature(unit, soc_feature_ecmp_resilient_hash_optimized) &&
            ...
            max_paths = BCM_XGS3_L3_ECMP_RH_MAX_PATHS(unit);
        } else 
#endif /* BCM_TOMAHAWK_SUPPORT */
        {
        ...
        /* TBD revisit during other TH changes */
        if (soc_feature(unit, soc_feature_ecmp_resilient_hash_optimized)) {
            if ((ecmp_flags & BCM_L3_ECMP_RH_OPT) &&  (max_paths > 16384)) {
                return (BCM_E_PARAM);
            } else if (!(ecmp_flags & BCM_L3_ECMP_RH_OPT) &&  (max_paths > 1024)) {
                return (BCM_E_PARAM);
            } 
        }
...
#ifdef BCM_TOMAHAWK_SUPPORT
         /* If this group is an RH set, then get members from the info struct */
         if (soc_feature(unit, soc_feature_ecmp_resilient_hash_optimized)) {
             bcm_l3_egress_ecmp_t_init(&ecmp);
             ecmp.ecmp_intf = idx + BCM_XGS3_MPATH_EGRESS_IDX_MIN;

             /* Get ECMP group's load balancing mode and check if RH */
             rv = bcm_th_l3_egress_ecmp_lb_get(unit, &ecmp);
             if (BCM_FAILURE(rv)) {
                 sal_free(intf_array);
                 return (rv);
             }
             ...
             if (ecmp.dynamic_mode == BCM_L3_ECMP_DYNAMIC_MODE_RESILIENT) {
                 rv = bcm_th_ecmp_rh_get_multipath(unit,
                           (ecmp.ecmp_intf - BCM_XGS3_MPATH_EGRESS_IDX_MIN),
                                    BCM_XGS3_L3_ECMP_MAX(unit),
                                    intf_array,
                                    &intf_count);
                 if (BCM_FAILURE(rv)) {
                          ...
                          void *user_data)
{
    ...
    for (; idx <= tbl_ptr->idx_maxused; idx++) {
         ...
         if (intf_count < 1) {
             /* This is to skip over first 256 reserved group entries */
             continue;
         }
    ...
    bcm_l3_egress_ecmp_t *ecmp_info)
{
    ...
    /* Get ecmp group index. */
    ecmp_group_idx = ecmp_info->ecmp_intf - BCM_XGS3_MPATH_EGRESS_IDX_MIN; 
    ...
    if (SOC_IS_TOMAHAWK(unit) &&
        soc_feature(unit, soc_feature_ecmp_resilient_hash_optimized)) {
        BCM_IF_ERROR_RETURN(
            bcm_th_l3_egress_ecmp_lb_get(unit, ecmp_info));
    }
#endif /* BCM_TOMAHAWK_SUPPORT */
...
int
_bcm_xgs3_l3_ecmp_egress_if_get(
    int unit, 
    bcm_l3_egress_ecmp_t *ecmp_info, 
    int intf_size, 
    bcm_if_t *intf_array, 
    int *intf_count)
{
    ...
    if (ecmp_info->dynamic_mode == BCM_L3_ECMP_DYNAMIC_MODE_RESILIENT &&
        soc_feature(unit, soc_feature_ecmp_resilient_hash_optimized)) {
        rv = bcm_th_ecmp_rh_get_multipath(unit, 
                        ecmp_info->ecmp_intf - BCM_XGS3_MPATH_EGRESS_IDX_MIN, 
                        intf_size, intf_array, intf_count);
    } else
#endif /* BCM_TOMAHAWK_SUPPORT */
    {
        rv = bcm_xgs3_l3_egress_multipath_get(unit, ecmp_info->ecmp_intf,
                                              intf_size,
                                              intf_array, intf_count);
    }
 ...
 * Parameters:
 *      unit         - Unit number.
 *      ecmp_intf    - Egress ECMP fowarding object.
    ...
    bcm_l3_ecmp_member_t *ecmp_member)
{
    ...
    if (soc_feature(unit, soc_feature_ecmp_failover)) {
        ...
        if (!BCM_XGS3_L3_MPATH_EGRESS_IDX_VALID(unit, ecmp_intf)) {
            return (BCM_E_PARAM);
        }
        ...
        grp_idx  = ecmp_intf - BCM_XGS3_MPATH_EGRESS_IDX_MIN;
        nh_index = ecmp_member->egress_if - BCM_XGS3_EGRESS_IDX_MIN;
        failover_id = ecmp_member->failover_id;
        failover_egress_if = ecmp_member->failover_egress_if;

        if ((_bcm_esw_failover_id_check(unit, failover_id) == BCM_E_NONE) &&
            BCM_XGS3_L3_EGRESS_IDX_VALID(unit, failover_egress_if)) {
            prot_nh_index = 
                ecmp_member->failover_egress_if - BCM_XGS3_EGRESS_IDX_MIN;
    ...
    bcm_l3_ecmp_member_t *ecmp_member)
{
    ...
    if (soc_feature(unit, soc_feature_ecmp_failover)) {
        ...
        if (BCM_SUCCESS(rv)) {
            failover_intf = prot_nh_index + BCM_XGS3_EGRESS_IDX_MIN;
    ...
    int *ecmp_member_count)
{
    ...
    /* Get ECMP group's existing interface array */
    rv = _bcm_xgs3_l3_ecmp_egress_if_get(unit, ecmp_info, ecmp_member_size, 
                                         intf_array, &intf_count);
    if (BCM_FAILURE(rv)) {
        goto clean_up;
    }
    ...
    *ecmp_member_count = intf_count;
    ...
    if (ecmp_member_size > 0) {
        for (i = 0; i < *ecmp_member_count; i++) {
            ...
            rv = _bcm_xgs3_l3_ecmp_member_failover_get(unit, ecmp_info->ecmp_intf, 
...
_bcm_xgs3_l3_ecmp_member_set(
    int unit, 
    bcm_l3_egress_ecmp_t *ecmp_info, 
    int ecmp_member_index, 
    bcm_l3_ecmp_member_t *ecmp_member)
{
    int rv = BCM_E_NONE;

    /* Configure the failover_id and failover_intf */
    rv = _bcm_xgs3_l3_ecmp_member_failover_set(unit, ecmp_info->ecmp_intf,
                                               ecmp_member_index, ecmp_member);
    return rv;
}
    ...
    bcm_if_t ecmp_group_id)
{
    ...
    if (BCM_SUCCESS(rv)) {
        if (soc_feature(unit, soc_feature_ecmp_resilient_hash_optimized)) {
            rv = bcm_th_l3_egress_ecmp_rh_destroy(unit, ecmp_group_id);
        }
    }
    if (BCM_SUCCESS(rv) && SOC_IS_TOMAHAWK(unit)) {
        rv = bcm_th_ecmp_lb_mode_reset(unit, ecmp_group_id);
    }
#endif /* BCM_TOMAHAWK_SUPPORT */
    ...
    bcm_l3_egress_ecmp_t *ecmp_info)
{
    ...
    if (soc_feature(unit, soc_feature_ecmp_resilient_hash_optimized)) {
        if (rv == BCM_E_NOT_FOUND) {
            rv = bcm_th_l3_egress_rh_ecmp_find(unit, intf_count, intf_array, 
                                               &ecmp_info->ecmp_intf);
        }
    }
#endif /* BCM_TOMAHAWK_SUPPORT */
    ...
    void *user_data)
{
    ...
    for (; idx <= tbl_ptr->idx_maxused; idx++) {
        ...
        ecmp_info.ecmp_intf = idx + BCM_XGS3_MPATH_EGRESS_IDX_MIN;
...
bcm_xgs3_l3_add(int unit, _bcm_l3_cfg_t *l3cfg)
{
    ...
    if (BCM_XGS3_L3_MCAST_ENTRY(l3cfg)) {
        ...
        if (BCM_GPORT_IS_BLACK_HOLE(l3cfg->l3c_port_tgid) ||
             ...
             nh_idx = BCM_XGS3_L3_BLACK_HOLE_NH_IDX;
        } else {
             /* Get next hop index. */
...
#ifdef BCM_TOMAHAWK_SUPPORT
        BCM_XGS3_L3_TBL(unit, ecmp_info).ecmp_rh_max_paths = max_paths;
#endif /* BCM_TOMAHAWK_SUPPORT */
                           ...
                           int *tnl_idx)
{
    ...
    data.tbl_ptr =  BCM_XGS3_L3_TBL_PTR(unit, tnl_init);
    data.width = _BCM_TUNNEL_OUTER_HEADER_IPV6(info->type) ?
        _BCM_DOUBLE_WIDE: _BCM_SINGLE_WIDE;
...
bcm_xgs3_tunnel_config_set(int unit, bcm_tunnel_config_t *tconfig)
{
    ...
    if (SOC_IS_TRIUMPH2(unit)  || SOC_IS_APOLLO(unit)   ||
        ...
        SOC_IS_TRIDENT2X(unit) || SOC_IS_TOMAHAWK(unit) ||
...
_bcm_xgs3_l3_tnl_term_add(int unit, bcm_tunnel_terminator_t *tnl_info)
{
    ...
    if(SOC_MEM_FIELD_VALID(unit, L3_TUNNELm, TUNNEL_CLASS_IDf)) {                 
        soc_L3_TUNNELm_field32_set(unit, entry_ptr, TUNNEL_CLASS_IDf, 
...
_bcm_xgs3_l3_clear_all(int unit)
{
     ...
     */
    if ((SAL_BOOT_BCMSIM && !SOC_IS_TRIDENT2PLUS(unit)) || SAL_BOOT_QUICKTURN) {
        return BCM_E_NONE;
    }
...
#ifdef BCM_TRIDENT_SUPPORT
    if ((SOC_IS_TD_TT(unit) || SOC_IS_TD2_TT2(unit)) && (!SOC_IS_TOMAHAWK(unit))) {
        defip_hit_only_y_entry_t hit_entry_y;
        defip_hit_only_x_entry_t hit_entry_x;
        uint32 hit;
        if (lpm_cfg->defip_flags & BCM_L3_IP6) {
...
_bcm_xgs3_route_tables_resize(int unit,  int arg)
{
    ...
    if (SOC_IS_TRIUMPH3(unit)) {
        ...
        rv = bcm_esw_switch_control_get(unit, bcmSwitchL3RouteCache, &cache_on);
    }
