...
#define MARK_PKT_PROCESSED_LOCAL(unit, chan, dv)                        \
    do {                                                                \
        ...
        RX_INTR_UNLOCK;                                                 \
        if ((SOC_DMA_MODE(unit, chan) != SOC_DMA_MODE_CONTINUOUS) &&    \
            (pkts_processed == RX_PPC(unit))) {                         \
...
#if defined (BCM_CMICD_SUPPORT)
STATIC INLINE int      rx_dv_concat(int unit, int chan, dv_t *dv);
#endif
                                 ...
                                 bcm_rx_chan_t chan_id)
{
    ...
    if (!soc_feature(unit, soc_feature_cos_rx_dma)) {
        ...
        if (SOC_IS_TRIDENT2PLUS(unit)) {
            /* trident2plus has 6 special queues (58...63)*/
            if (( queue_id < 64 ) && ( queue_id > 57 )) {
                error = 0;
            }
        }
        if ( error ) {
    ...
    if(soc_feature(unit, soc_feature_cmicm)) {
        ...
        if (queue_id < 0) { /* All COS Queues */
            ...
            if (queue_id < startq || queue_id >= endq) {
                int error = 1;
                /* TBD COSQ. The following should be chaneged after full solution for new queues */
                if (SOC_IS_TRIDENT2PLUS(unit) && (cmc == pci_cmc)) {
                    /* trident2plus has 6 special queues (58...63)*/
                    if (( queue_id < 64 ) && ( queue_id > 57 )) {
                        error = 0;
                        SHR_BITCLR(CPU_ARM_RSVD_QUEUE_BITMAP(unit, cmc), 58);
                    }
                }
                if ( error ) {
                    return BCM_E_PARAM;
            ...
            for (ix = start_chan_id;
                ...
                /* TBD COSQ. The following should be chaneged after full solution for new queues */
                if (SOC_IS_TRIDENT2PLUS(unit) && (cmc == pci_cmc)) {
                    /* change deafault for speacial queues */
                    reg_val |=
...
#if defined (BCM_CMICD_SUPPORT)
        if (SOC_DMA_MODE(unit, chan) == SOC_DMA_MODE_CONTINUOUS) {
            /* Concatenate dv at end of chain */
            rv = rx_dv_concat(unit, chan, dv);
        } else
#endif
        {
    ...
    while (pkt_list) {
        ...
        if (drop_all) {
            if (RX_IS_RCPU(unit)) {
                ...
                /* Unit checks are taken care before calling this function,
                 * having unit number check is redundant here
                 */
...
rx_dv_fill(int unit, int chan, dv_t *dv)
{
    ...
    if (SOC_DMA_MODE(unit, chan) == SOC_DMA_MODE_CONTINUOUS) {
        ...
        if (dv->dv_vcnt < dv->dv_cnt) {
            ...
            SOC_DCB_ADDR_SET(unit, d, (sal_vaddr_t)d);

            SOC_DCB_RELOAD_SET(unit, d, 1);             /* Reload */
            SOC_DCB_CHAIN_SET(unit, d, 1);              /* No Chain intr */

            /* Set chain bit in last pkt DCB to avoid chain done interrupt */
            if (dv->dv_vcnt > 0) {
 ...
 *      rx_dv_concat
 * Purpose:
 *      Try to concatenate the dv to the end of the chain
 * Parameters:
 *      unit
 *      chan
 *      dv to be appended at the end of the chain
 * Returns:
 *      BCM_E_XXX
 * Notes:
 *
 */
STATIC INLINE int
rx_dv_concat(int unit, int chan, dv_t *dv)
{
    int rv = BCM_E_NONE;
    ...
    /* Set the HW register for the new LAST_DESC_ADDR- new for CMICD */
    ...
    reg_val = soc_cm_l2p(unit,
                         (void*)SOC_DCB_IDX2PTR(unit, dv->dv_dcb,
                                                (dv->dv_vcnt - 1)));
    soc_pci_write(unit, reg_addr, reg_val);
