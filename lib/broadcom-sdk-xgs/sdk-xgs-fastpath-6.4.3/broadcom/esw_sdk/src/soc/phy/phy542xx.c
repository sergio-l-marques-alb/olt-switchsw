/*
 * $Id: phy542xx.c,v 1.59 Broadcom SDK $ 
...
STATIC int _phy_bcm542xx_medium_change(int unit, soc_port_t port, int force_update,
...
_phy_bcm54280_init(int unit, soc_port_t port)
{
    ...
    /* Copper Auxiliary Control Reg., RDB.0x028, Legacy Reg.0x18 shawow 0     *\
    ...
    \*     bit[14] COPPER_JUMBO_MSB, copper jumbo frames support      */
    SOC_IF_ERROR_RETURN
     ...
     *     bit[0]  COPPER_JUMBO_LSB, copper jumbo frames support *
    ...
    SOC_IF_ERROR_RETURN
        (PHY_MODIFY_BCM542XX_MII_ECRr(unit, pc,
                              ...
                              | PHY_BCM542XX_MII_ECR_EN_LEDT,
                                PHY_BCM542XX_MII_ECR_FIFO_ELAST_0
                              | PHY_BCM542XX_MII_ECR_EN_LEDT));
    /* Misc 1000-X Control 2 Reg.  , RDB.0x236, Legacy Reg.0x1C shawow 0x16 *\
    \*     bit[0] SERDES_JUMBO_MSB, SGMII/Fiber jumbo frames support        */
    SOC_IF_ERROR_RETURN
        (PHY_MODIFY_BCM542XX_1ST_SERDES_CTRLr(unit, pc,
                                PHY_BCM542XX_SERDES_CTRL_JUMBO_MSB,
                                PHY_BCM542XX_SERDES_CTRL_JUMBO_MSB));
    /* Auxiliary 1000-X Contro Reg., RDB.0x23B, Legacy Reg.0x1C shawow 0x1B *\
    \*     bit[1] SERDES_JUMBO_LSB, SGMII/Fiber jumbo frames support        */
    SOC_IF_ERROR_RETURN
...
/*
 * Function:
 *    phy_bcm54200_init    ( for 54210/220/240/285/294/295/296 )  dual media
 * Purpose:
                   ...
                   int fiber_enable   , int copper_enable )
{
    ...
    if ( fiber_enable && (automedium || fiber_preferred) ) {
        ...
        if ( PHY_IS_BCM54210_220(pc) ) {
            ...
            if ( fiber_preferred ) {
                data |= PHY_BCM542XX_EXT_SERDES_AUTO_FX;
            }
            /* External Serdes Control Reg., DIGX_SHD_1C_14, RDB_0x234 */
                         ...
                         int copper_enable)
{
    ...
    SOC_IF_ERROR_RETURN(
        PHY_WRITE_BCM542XX_MII_CTRLr(unit, pc,
                                     (ctrl | PHY_BCM542XX_MII_CTRL_RESET)));
...
phy_bcm542xx_init(int unit, soc_port_t port)
{
    ...
    /* default to legacy register access mode (none RDB mode) */
   ...
   /* Init PHYS and MACs to defaults */
    ...
    if ( SOC_WARM_BOOT(unit) ) {
        int  medium, andone0, andone1 ;

        /* retrieve the media mode (copper or fiber) */
        ...
        if ( medium == SOC_PORT_MEDIUM_COPPER ) {
            ...
            SOC_IF_ERROR_RETURN(
                phy_bcm542xx_autoneg_get(unit, port, &(pc->copper.autoneg_enable), &andone0) );
            SOC_IF_ERROR_RETURN(
            ...
            SOC_IF_ERROR_RETURN(
                phy_bcm542xx_autoneg_get(unit, port, &(pc->fiber.autoneg_enable), &andone1) );
            SOC_IF_ERROR_RETURN(
                         ...
                         soc_port_medium_t force_medium)
{
    ...
    if (medium == SOC_PORT_MEDIUM_COPPER) {
        if (!PHY_COPPER_MODE(unit, port) || force_update) { /* Was fiber */ 
            ...
            LOG_BSL_INFO(BSL_LS_SOC_PHY,
                     (BSL_META_U(unit, "_phy_bcm542xx_link_auto_detect: "
                                       "u=%d p=%d [F->C]\n"),
                      unit, port));
        }
...
phy_bcm542xx_speed_set(int unit, soc_port_t port, int speed)
{
    ...
    uint16       serdes_100fx_ctrl_mask = 0;
    ...
    if ( PHY_COPPER_MODE(unit, port) ) {
        ...
        if ( speed == 10 ) {  /* ehter 1000Base-X or 100Base-FX */
            return SOC_E_UNAVAIL;
        }
        SOC_IF_ERROR_RETURN(
...
{
    phy_ctrl_t  *pc;
    uint16       mii_ctrl, mii_stat, mii_anp, mii_gb_stat;
                                  ...
                                  uint64 value)
{
    ...
    case SOC_PORT_CONTROL_PHY_TIMESYNC_LOAD_CONTROL:
        ...
        SOC_IF_ERROR_BREAK(
            phy_bcm542xx_rdb_reg_write(unit, pc,
                    PHY_BCM542XX_TOP_1588_SHD_CTL_REG_OFFSET, temp1), rv );
        SOC_IF_ERROR_BREAK(
            phy_bcm542xx_rdb_reg_write(unit, pc,
                    PHY_BCM542XX_TOP_1588_SHD_LD_REG_OFFSET , temp2), rv );
        break;
                                  ...
                                  uint64 *value)
{
    ...
    case SOC_PORT_CONTROL_PHY_TIMESYNC_LOAD_CONTROL:
        SOC_IF_ERROR_BREAK(
            phy_bcm542xx_rdb_reg_read(unit, pc,
                  PHY_BCM542XX_TOP_1588_SHD_CTL_REG_OFFSET, &value1), rv);
        SOC_IF_ERROR_BREAK(
            phy_bcm542xx_rdb_reg_read(unit, pc,
                  PHY_BCM542XX_TOP_1588_SHD_LD_REG_OFFSET , &value2), rv);
                              ...
                              int bit_shift, uint32 enable)
{
    ...
    \*  bit[4]: disable primary  , bit[3:0] source of primary   clock     */
    data = (enable)
           ? ((dev_port & PHY_BCM542XX_SYNCE_RECOVERY_CLK_REG_REC_CLK_SEL)
               << bit_shift)
           : (PHY_BCM542XX_SYNCE_RECOVERY_CLK_REG_REC_CLK_DIS);
