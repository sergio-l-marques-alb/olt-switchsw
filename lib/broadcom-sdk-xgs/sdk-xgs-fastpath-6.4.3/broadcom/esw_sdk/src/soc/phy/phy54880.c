/*
 * $Id: phy54880.c,v 1.101 Broadcom SDK $
...
/*
 * Function:
 *      _phy_89500_A0_br_init
 * Purpose:
 *      Function to initialize the LPF for automotive EMC and improve the link time for BR mode.
 * Parameters:
 *      unit - StrataSwitch unit #.
 *      port - StrataSwitch port #.
 * Returns:
...
STATIC int
_phy_89500_A0_br_init(int unit, soc_port_t port)
{
    phy_ctrl_t        *pc;
    ...
    /* threshold setting  */
    SOC_IF_ERROR_RETURN
        (WRITE_PHY54880_EXP_BR_LDS_CTRLr(unit, pc, 0x107F));
    /* enable BroadR-Reach  */
    SOC_IF_ERROR_RETURN
        (WRITE_PHY54880_EXP_BR_LRE_MISC_CTRL1r(unit, pc, 0x0001));
    /* restart and enable LDS */
    SOC_IF_ERROR_RETURN
        (WRITE_PHY54880_LRE_CTRLr(unit, pc, 0x3000));
    /* set speed to 100 Mb/s */
    SOC_IF_ERROR_RETURN
        (WRITE_PHY54880_LRE_CTRLr(unit, pc, 0x0200));
    ...
    SOC_IF_ERROR_RETURN
        (WRITE_PHY54880_EXP_BR_LRE_MISC_CTRL1r(unit, pc, 0x0000));
    /* disable LDS */
    SOC_IF_ERROR_RETURN
        (WRITE_PHY54880_LRE_CTRLr(unit, pc, 0x0100));

    /* gain setting */
    SOC_IF_ERROR_RETURN
        (WRITE_PHY_REG(unit, pc, 0x17, 0x0001));
    SOC_IF_ERROR_RETURN
        (WRITE_PHY_REG(unit, pc, 0x15, 0x0027));
    
    /* set threshold for automotive immunity */
    SOC_IF_ERROR_RETURN
        (WRITE_PHY_REG(unit, pc, 0x17, 0x000E));
    SOC_IF_ERROR_RETURN
        (WRITE_PHY_REG(unit, pc, 0x15, 0x9B52));
    SOC_IF_ERROR_RETURN
        (WRITE_PHY_REG(unit, pc, 0x17, 0x000F));
    SOC_IF_ERROR_RETURN
        (WRITE_PHY_REG(unit, pc, 0x15, 0xA04D));

    /* enable BroadR-Reach  */
    SOC_IF_ERROR_RETURN
        (WRITE_PHY54880_EXP_BR_LRE_MISC_CTRL1r(unit, pc, 0x0001));

    /* link acquistion setting */
    SOC_IF_ERROR_RETURN
        (WRITE_PHY54880_EXP_BR_LRE_MISC_CTRL3r(unit, pc, 0x9225));

    /* change link acqusiition target */
    SOC_IF_ERROR_RETURN
        (WRITE_PHY_REG(unit, pc, 0x17, 0x000A));
    SOC_IF_ERROR_RETURN
        (WRITE_PHY_REG(unit, pc, 0x15, 0x0323));

    /* clock setting */
    SOC_IF_ERROR_RETURN
        (WRITE_PHY_REG(unit, pc, 0x17, 0x0FFD));
    SOC_IF_ERROR_RETURN
        (WRITE_PHY_REG(unit, pc, 0x15, 0x1C3F));
    SOC_IF_ERROR_RETURN
    ...
    SOC_IF_ERROR_RETURN
        (WRITE_PHY54811_EXP_BR_LDS_TIMER_CTRLr(unit, pc, 0x7180));
    
    /* enable BR force mode link check, enable timer override */
    SOC_IF_ERROR_RETURN
        (WRITE_PHY54811_EXP_BR_LDS_CHAN_STATUSr(unit, pc, 0x34C0));
    ...
    /* 
     * Below setting will be done in the initialization
     * 1.  Force to slave mode, 100Mbps, 1-pair
     */
...
/*
 * Function:
 *      _phy_89500_B0_ieee_init
 * Purpose:
 *      Function to initialize the IEEE 100TX mode.
 * Parameters:
 *      unit - StrataSwitch unit #.
 *      port - StrataSwitch port #.
 * Returns:
...
STATIC int
_phy_89500_B0_ieee_init(int unit, soc_port_t port) {
    phy_ctrl_t        *pc;

    pc = EXT_PHY_SW_STATE(unit, port);

    /* Disable AGC freeze feature */
    SOC_IF_ERROR_RETURN
        (WRITE_PHY54880_SLED_1r(unit, pc, 0xAC01));

    /* Enable SM_DSP clock */
    SOC_IF_ERROR_RETURN
        (MODIFY_PHY54880_MII_AUX_CTRLr(unit,pc,0x0c00,0x0c00));

    /* Set the startup PLL bandwidth and gamma for IEEE mode */
    SOC_IF_ERROR_RETURN
        (WRITE_PHY_REG(unit, pc, 0x17, 0x000A));
    SOC_IF_ERROR_RETURN
        (WRITE_PHY_REG(unit, pc, 0x15, 0x010B));
    /* Set MSE threshold for IEEE mode */
    SOC_IF_ERROR_RETURN
        (WRITE_PHY_REG(unit, pc, 0x17, 0x000E));
    SOC_IF_ERROR_RETURN
        (WRITE_PHY_REG(unit, pc, 0x15, 0xA352));
    SOC_IF_ERROR_RETURN
        (WRITE_PHY_REG(unit, pc, 0x17, 0x000F));
    SOC_IF_ERROR_RETURN
        (WRITE_PHY_REG(unit, pc, 0x15, 0xA04C));

    /* Disable SM_DSP clock */
    SOC_IF_ERROR_RETURN
        (MODIFY_PHY54880_MII_AUX_CTRLr(unit,pc,0x0400,0x0c00));

    /* Disable link check timer */
    SOC_IF_ERROR_RETURN
        (WRITE_PHY54811_EXP_BR_LDS_CHAN_STATUSr(unit, pc, 0x20C0));

    /* 
     * Below setting will be done in the initialization
     * 1. Disable BroadR-Reach function
     * 2. Provide default ability
     * 3. Set 100TX, autoneg on, full-duple 
     */
...
/*
 * Function:
 *      _phy_89500_B0_br_init
 * Purpose:
 *      Function to improve the link time for BR mode.
 * Parameters:
 *      unit - StrataSwitch unit #.
 *      port - StrataSwitch port #.
 * Returns:
...
STATIC int
_phy_89500_B0_br_init(int unit, soc_port_t port) {
    phy_ctrl_t        *pc;
    /* uint16            value; */
    ...
    /* gain resetting */
    SOC_IF_ERROR_RETURN
        (WRITE_PHY_REG(unit, pc, 0x1C, 0xAC00));
...
_phy_54880_reset_setup(int unit, soc_port_t port)
{
    ...
    if (PHY_IS_BCM89500_A0(pc) && br_config->enable) {
        /* Initialize the LPF for automotive EMC and improve the link time for BR mode */
        _phy_89500_A0_br_init(unit, port);
    } 

    if (PHY_IS_BCM89500_B0(pc)) {
        if (br_config->enable) {
            /* Improve the link time for BR mode */
            _phy_89500_B0_br_init(unit, port);
        } else {
            /* IEEE mode initialization */
            _phy_89500_B0_ieee_init(unit, port);
        }
    }
    ...
    } else if (PHY_IS_BCM89500(pc)) {
        SOC_IF_ERROR_RETURN(
            phy_acd_cable_diag_init(unit, port));
    }  
...
phy_54880_init(int unit, soc_port_t port)
{
    ...
    if (PHY_IS_BCM54881(pc) || PHY_IS_BCM89500(pc)) {
    ...
    br_config->bypass_reset = 
        soc_property_port_get(unit, port, spn_BCM89500_RAPID_BOOT, 0);
...
_phy_54880_br_speed_set(int unit, soc_port_t port, int speed)
{
    ...
    if (PHY_IS_BCM89500(pc)) {
        /* Reduce amplitude for 10BASE-T, otherwise short cable can't link up for 10BASE-T */
        SOC_IF_ERROR_RETURN
            (MODIFY_PHY54880_MII_MISC_TESTr
                (unit, pc, (speed == 10) ? (1U<<3):0, (1U<<3)));
    }
...
phy_54880_autoneg_set(int unit, soc_port_t port, int autoneg)
{
    ...
    if (PHY_IEEE_MODE(unit, port)) {
        ...
        if(PHY_IS_BCM89500(pc)) {
            /* Disable LDS to prevent self-loop energy detect in AN */
            SOC_IF_ERROR_RETURN
                (_phy_54880_br_autoneg_set(unit, port, FALSE));
        }
    } else {
        SOC_IF_ERROR_RETURN
            ...
            soc_port_cable_diag_t *status)
{
    ...
    if ((PHY_IS_BCM54880(pc)) && ((pc->phy_rev & 0xf) > 0x1))  {
        ...
        SOC_IF_ERROR_RETURN(
            phy_acd_cable_diag(unit, port, status));
    } else {
        SOC_IF_ERROR_RETURN(
