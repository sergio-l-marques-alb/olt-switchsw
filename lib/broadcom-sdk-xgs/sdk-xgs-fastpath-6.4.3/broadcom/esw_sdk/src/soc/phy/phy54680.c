/*
 * $Id: phy54680.c,v 1.166 Broadcom SDK $
...
STATIC int
_phy_nsp_eee_war(int unit, soc_port_t port)
{
    phy_ctrl_t *pc;
    pc = EXT_PHY_SW_STATE(unit, port);
...
_phy_54680_reset_setup(int unit, soc_port_t port)
{
    ...
    if (soc_phy_primary_and_offset_get(unit, port, &primary_port, &index) == SOC_E_NONE) {
        if (index & 0x80) {
            PHY54680_FLAGS(pc) |= PHY54680_PHYA_REV;
        } else {
            PHY54680_FLAGS(pc) &= ~PHY54680_PHYA_REV;
        }
        ...
        PHY54680_DEV_SET_BASE_ADDR(pc); /* phy address of port 0 */
...
#if defined(BCM_NORTHSTARPLUS_SUPPORT)
    if (PHY_REVISION_CHECK(pc, PHY_BCM53020_MODEL, PHY_BCM53020_OUI, 0x0)) {
        /* Apply the EEE workaround */
        SOC_IF_ERROR_RETURN(
            _phy_nsp_eee_war(unit, port));
    }
#endif /* BCM_NORTHSTARPLUS_SUPPORT */
...
#if defined(BCM_53128) || defined(BCM_NORTHSTAR_SUPPORT) || defined(BCM_NORTHSTARPLUS_SUPPORT)
    /* BCM53125 internal GPHY is EEE capable */
    if (PHY_IS_BCM53128(pc) || PHY_IS_BCM53010(pc) || PHY_IS_BCM53020(pc)) {
         PHY_FLAGS_SET(unit, port, PHY_FLAGS_EEE_CAPABLE);
    }
#endif /* BCM_53128 || BCM_NORTHSTAR_SUPPORT || BCM_NORTHSTARPLUS_SUPPORT */    
    ...
    if (!enable) {
        if ((PHY_FLAGS_TST(unit, port, PHY_FLAGS_EEE_ENABLED)) &&
           (((pc->phy_rev & 0x4) == 0x0) || /* A0,A1,B0,B1 */
            ...
            (PHY_IS_BCM53020(pc) && pc->phy_rev == 0x0))) {
...
phy_54680_ability_local_get(int unit, soc_port_t port, soc_port_ability_t *ability)
{
    ...
    if (PHY_IS_BCM53125(pc) || PHY_IS_BCM53128(pc) || PHY_IS_BCM53324(pc) ||
        PHY_IS_BCM53010(pc) || PHY_IS_BCM53020(pc)) {
        /* No SGMII interface */
...
phy_54680_lb_set(int unit, soc_port_t port, int enable)
{
    ...
    if (PHY_IS_BCM53125(pc) || PHY_IS_BCM53128(pc) || PHY_IS_BCM53324(pc) || 
        PHY_IS_BCM52681E(pc) || PHY_IS_BCM53010(pc) || PHY_IS_BCM53020(pc)) {
        /* No SGMII interface */
...
_phy_54680_eee_enable(int unit, soc_port_t port, int enable)
{
    ...
    if (enable == 1) {
        if (PHY_IS_BCM54680E_A0A1(pc) || PHY_IS_BCM54880E_A0A1(pc)) {
            ...
            PHY_IS_BCM53128(pc) || PHY_IS_BCM53010(pc) || PHY_IS_BCM53020(pc)) {
...
phy_54680_link_up(int unit, soc_port_t port)
{
    ...
    if (PHY_IS_BCM53125(pc) || PHY_IS_BCM53128(pc) || PHY_IS_BCM53324(pc) || \
        ...
        if ((PHY_IS_BCM53125(pc) && pc->phy_rev == 0x4) ||
            (PHY_IS_BCM53128(pc) && pc->phy_rev == 0x4) ||
            (PHY_IS_BCM53324(pc) && pc->phy_rev == 0x0)) {
            rev_b_cfg = 1;
        }
