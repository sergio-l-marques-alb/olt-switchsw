...
#define VIPER_IF_SR4    (1 << SOC_PORT_IF_SR4)
...
#define VIPER_IF_SGMII  (1 << SOC_PORT_IF_SGMII)
...
viper_show_serdes_info(phy_ctrl_t *pc, viper_dev_info_t *pInfo, phymod_core_info_t *core_info)
{
    ...
    sal_strncpy(pInfo->name,"VIPER-", strlen("VIPER-"));
    ...
    /* phy_mode: single port mode, port uses all four lanes 
     *           quad port mode, port uses 1 lane
     */
    if (IND_LANE_MODE(pc)) {
...
STATIC int 
viper_ref_clk_convert(int port_refclk_int, phymod_ref_clk_t *ref_clk)
{
    switch (port_refclk_int)
    {
        case 50:
            *ref_clk = phymodRefClk50Mhz;
            break;
        case 156:
...
#if 0
    SOC_IF_ERROR_RETURN
        (viper_ref_clk_convert(speed_config->port_refclk_int, &(interface_config->ref_clock)));
#endif
    return SOC_E_NONE;
}
                 ...
                 phymod_phy_init_config_t  *pm_phy_init_config)
{
    ...
    */
    ...
    /* For VIPERX family, both lane_map_rx and lane_map_tx are logic lane base */
 ...
 *      Per-logical port information is saved in (viper_cfg_t *) &pc->driver_data.
...
phy_viper_speed_get(int unit, soc_port_t port, int *speed)
{
    ...
    /*SOC_IF_ERROR_RETURN( 
    viper_ref_clk_convert(pCfg->speed_config.port_refclk_int, &ref_clock)); */
...
phy_viper_an_set(int unit, soc_port_t port, int enable)
{
    ...
    /* CHECK if speed = 10G what is an??? */
    if (pCfg->cl37an) {
        switch (pCfg->cl37an) {
            case VIPER_CL37_WO_BAM:
...
phy_viper_interface_get(int unit, soc_port_t port, soc_port_if_t *pif)
{
    ...
    case phymodInterfaceSFI:
        *pif = SOC_PORT_IF_SFI;
        break;
    /* QUESTION: should be phymodInterfaceXAUI ??? */
    ...
    case phymodInterfaceBypass:
        *pif = SOC_PORT_IF_ILKN;
        break;
    default:
        /*QUESTION: don't use XGMII, it's supposed to use SGMII, right? 
        *pif =  SOC_PORT_IF_XGMII;
        */
        *pif =  SOC_PORT_IF_SGMII;
        break;
...
STATIC int
_viper_device_create_attach(soc_phymod_core_t *core, uint32_t core_id) 
{
    return SOC_E_NONE;
}
...
phy_viper_probe(int unit, phy_ctrl_t *pc)
{
    ...
    */
    ...
    case 10:
        num_phys = 3;
        pc->phy_mode = PHYCTRL_MULTI_CORE_PORT;
        /* 100g_config to decide lane_map[0/1/2] */
        lane_map[1] = 0xf ;
        lane_map[2] = 0x3 ;
        break;
#endif
    ...
    for (idx = 0; idx < num_phys ; idx++) {
        ...
        if (rv == SOC_E_NOT_FOUND) {
            ...
            rv |= _viper_device_create_attach(phy->core, core_id) ;
        }
        if (SOC_FAILURE(rv)) {
            break;
        }        
