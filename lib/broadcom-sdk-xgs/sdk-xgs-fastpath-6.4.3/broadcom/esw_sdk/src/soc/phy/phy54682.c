...
_phy_54682_reset_setup(int unit, soc_port_t port)
{
    ...
    if (soc_phy_primary_and_offset_get(unit, port, &primary_port, &index) == SOC_E_NONE) {
        ...
        LOG_BSL_WARN(BSL_LS_SOC_PHY,
                 (BSL_META_U(unit,
                             "_phy_54682_reset_setup: Config property 'phy_port_primary_and_offset' not set for u=%d, p=%d\n"),
                             unit, port));
...
phy_54682_ability_advert_set(int unit, soc_port_t port, soc_port_ability_t *ability)
{
    ...
    if ( PHY_COPPER_MODE(unit, port) ) {
        ...
        uint16  mii_adv = MII_ANA_C37_FD;  /* Always advertise 1000X full duplex */
