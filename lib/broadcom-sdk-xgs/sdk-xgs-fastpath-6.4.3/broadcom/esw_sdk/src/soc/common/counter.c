...
#ifdef BCM_TRIDENT2PLUS_SUPPORT
    if (SAL_BOOT_QUICKTURN && SOC_IS_TRIDENT2PLUS(unit)) {
                ...
                if (pipe_ct) {
                    ...
                    if (pipe_offset) {
                        for (; i < NUM_PIPE(unit); i++) {
                            if ((non_dma->dma_mem[i] != INVALIDm) && pipe_offset) {
                                pipe_offset--;
                                continue;
                            }
                            if (!pipe_offset) {
                                    ...
                                    soc_reg_t ctr_reg)
{
    ...
    int index_min = 0, index_max = 0, pipe, entries_per_pipe = 0;
    ...
    if (soc->ctr_evict_interval) {
        ...
        efp_ctr_rval = 0;
        soc_reg32_get(unit, reg, REG_PORT_ANY, 0, &efp_ctr_rval);
        soc_reg_field_set(unit, reg, &efp_ctr_rval, CLR_ON_READf, 1);
        soc_reg32_set(unit, reg, REG_PORT_ANY, 0, efp_ctr_rval);
    }
    ...
    for (; id < non_dma_counter_end; id++) {
        ...
        if (non_dma->mem != INVALIDm) {
            ...
            do {
                ...
                if (ctr_reg == INVALIDr) {
                    if (!entries_per_pipe && (non_dma->dma_mem[0] != INVALIDm)) {
                        entries_per_pipe = soc_mem_index_count(unit, non_dma->dma_mem[0]);
                    }
                    for (i = 0; i < NUM_PIPE(unit); i++) {
                        /* Do only if the Pipe mem is VALID. */
...
soc_counter_thread(void *unit_vp)
{
    ...
    while ((interval = soc->counter_interval) != 0) {
        ...
        if (SOC_IS_TD2_TT2(unit)) {
            ...
            SOC_PBMP_ASSIGN(cnt_pbmp, soc->counter_pbmp);
            if (SOC_PBMP_NEQ(cur_pbmp, cnt_pbmp)) {
                SOC_PBMP_ASSIGN(soc->counter_pbmp, cur_pbmp);
                SOC_PBMP_XOR(cur_pbmp, cnt_pbmp);
                SOC_PBMP_CLEAR(cnt_pbmp);
                PBMP_ITER(cur_pbmp, p) {
                    if (SOC_PBMP_MEMBER(soc->counter_pbmp, p)) {
                        SOC_PBMP_PORT_ADD(cnt_pbmp, p);
                    }
                }
