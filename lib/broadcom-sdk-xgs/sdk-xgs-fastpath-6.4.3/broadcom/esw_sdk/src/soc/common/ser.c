...
#define SOC_SER_REG_CACHE_MAX 23
            ...
            if (SOC_IS_TOMAHAWK(unit)) {
                LOG_BSL_ERROR(BSL_LS_SOC_COMMON,
                          (BSL_META_U(unit,
                                      ...
                                      "blk: %s index: %d\n"),
                           restore_pipe, SOC_MEM_NAME(unit, mem),
                           mem, SOC_BLOCK_NAME(unit, copyno), index));
            } else
#endif /* BCM_TOMAHAWK_SUPPORT */
            {
                LOG_BSL_ERROR(BSL_LS_SOC_COMMON,
                          (BSL_META_U(unit,
                                      ...
                                      "blk: %s index: %d\n"),
                           pipe ? 'X':'Y', SOC_MEM_NAME(unit, mem),
                           mem, SOC_BLOCK_NAME(unit, copyno), index));
            }
            stat->ser_err_restor++;
...
#define _SOC_SER_MEM_OVERLAY_SET 6
    ...
    case FPEM_ECCm:
        SOC_IF_ERROR_RETURN
            (soc_th_field_mem_mode_get(unit, EXACT_MATCH_2m, &fpem_mem_mode));
    ...
    case EXACT_MATCH_2m:
        ...
        SOC_IF_ERROR_RETURN
            (soc_th_field_mem_mode_get(unit, reported_mem, &fpem_mem_mode));
        num_mems = 1;
        mem_list[0] = _SOC_FPEM_EM2(pipe, fpem_mem_mode); iratio[0] = 2;
    ...
    case L3_DEFIP_DATA_ONLYm:
        ...
        if (SOC_IS_TRIUMPH(unit) || SOC_IS_ENDURO(unit)) {
#ifdef BCM_ENDURO_SUPPORT
            if (SOC_IS_ENDURO(unit)) {
        ...
        if (SOC_IS_TRX(unit)) {
            ...
            if (SOC_IS_TOMAHAWK(unit)) {
                ...
                case EXACT_MATCH_2_PIPE3m:
                     ...
                     * location.
                     */
                    mem = _SOC_FPEM_EM2_ENTRY_ONLY
                        (pipe, _SOC_SER_MEM_MODE_PIPE_UNIQUE);
                    vindex = index/2;
                    break;
                case EXACT_MATCH_4m:
                    if (!reported_mem_is_fpem_lp) {
...
#define _SOC_SER_SRAM_CHK_RETURN(str) \
    ...
    void *null_entry;
    uint8 *vmap, at = 0, _l3_lock = 0, _l2_lock = 0;
#if defined(ALPM_ENABLE)
    uint8 _alpm_lock = 0;
#endif /* ALPM_ENABLE */
    ...
    if (SOC_IS_TOMAHAWK(unit)) {
        ...
        if ((mem == L3_ENTRY_LPm) || (mem == L3_ENTRY_ISS_LPm) ||
            (mem == L3_ENTRY_ONLY_ECCm)) {
            L3_LOCK(unit);
            _l3_lock = 1;
        }
