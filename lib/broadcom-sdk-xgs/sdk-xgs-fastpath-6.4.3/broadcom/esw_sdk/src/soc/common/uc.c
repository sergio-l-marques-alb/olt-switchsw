...
soc_uc_addr_to_pcie(int unit, int uC, uint32 addr)
{
    ...
    if (soc_feature(unit, soc_feature_uc_mhost)) {
        /* For mHost, the TCM mapping may be different between chips */
...
soc_uc_mhost_in_reset(int unit, int uC)
{
    ...
    if (uC == 0) {
        READ_MHOST_0_CR5_RST_CTRLr(unit, &ctrl);
        halt = soc_reg_field_get(
                    unit, MHOST_0_CR5_RST_CTRLr, ctrl, CPU_HALT_Nf);
        return (halt == 0);
    }
