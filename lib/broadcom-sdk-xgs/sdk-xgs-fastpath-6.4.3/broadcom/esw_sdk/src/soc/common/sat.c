    ...
    )
{
    ...
    if ((pkt_hdr == NULL) || (pkt_hdr_tbl_info == NULL)) {
        rv = SOC_E_PARAM;
        goto exit;
    }
    ...
    pkt_hdr_tbl_info->pkt_hdr_len = pkt_hdr->pkt_data[0].len - 1;
    ...
    )
{
    ...
    if ((pkt_edit == NULL) || (stamp_field_info == NULL)) {
        rv = SOC_E_PARAM;
        goto exit;
    }
    ...
    for (i = 0; i < pkt_edit->number_of_stamps; i++) {
        ...
        if (tmp_stamp->stamp_type == socSatStampConstant2Bit) {
            ...
            stamp_field_info->stamp_2_offset = tmp_stamp->offset/8;
            stamp_field_info->stamp_2_shift = tmp_stamp->offset%8;
            ...
            stamp_field_info->counter_8_offset = tmp_stamp->offset/8;
            stamp_field_info->counter_8_shift = tmp_stamp->offset%8;
            ...
            stamp_field_info->counter_16_offset = tmp_stamp->offset/8;
            stamp_field_info->counter_16_shift = tmp_stamp->offset%8;
    ...
    )
{
    ...
    if ((rate_mnt_val == NULL) || (rate_exp_val == NULL)) {
        return SOC_E_PARAM;
    }
    ...
    )
{
    ...
    uint32 sequence_ena = 1, sequence_offset = 0, sequence_period = 0;
    ...
    for (priority = 0; priority < SOC_SAT_GTF_NUM_OF_PRIORITIES; priority++) {
        ...
        sequence_period = config->packet_edit[priority].number_of_ctfs;
    ...
    for (priority = 0; priority < SOC_SAT_GTF_NUM_OF_PRIORITIES; priority++) {
        ...
        field_data = config->packet_edit[priority].pattern_length;
        ...
        /* SEQ_NUM_XXXf */
		soc_mem_field_set(unit, OAMP_SAT_TX_GEN_PARAMSm, tx_gen_params_data, SEQ_NUM_ENABLEf, &sequence_ena);
		soc_mem_field_set(unit, OAMP_SAT_TX_GEN_PARAMSm, tx_gen_params_data, SEQ_NUM_PERIODf, &sequence_period);
		soc_mem_field_set(unit, OAMP_SAT_TX_GEN_PARAMSm, tx_gen_params_data, SEQ_NUM_OFFSETf, &sequence_offset);
    ...
    )
{
    ...
    soc_reg_above_64_field32_set(unit, OAMP_SAT_GEN_RATE_CONTROLr, data, BURST_MODE__Nf, config->rate_pattern_mode);
    ...
    )
{
    ...
    soc_mem_field32_set(unit, OAMP_SAT_RX_FLOW_IDm, &data, FLOW_VALIDf, 1);
    ...
    )
{
    ...
    soc_mem_field32_set(unit, OAMP_SAT_RX_FLOW_IDm, &data, FLOW_IDf, 0);
    soc_mem_field32_set(unit, OAMP_SAT_RX_FLOW_IDm, &data, FLOW_VALIDf, 0);
    ...
    )
{
    ...
    soc_mem_field32_set(unit, OAMP_RX_OAM_ID_TCAMm, &data, VALIDf, 1);
    ...
    )
{
    ...
    soc_mem_field32_set(unit, OAMP_RX_OAM_ID_TCAMm, &data, KEYf, 0);
    soc_mem_field32_set(unit, OAMP_RX_OAM_ID_TCAMm, &data, MASKf, 0);
    soc_mem_field32_set(unit, OAMP_RX_OAM_ID_TCAMm, &data, DATf, 0);
    soc_mem_field32_set(unit, OAMP_RX_OAM_ID_TCAMm, &data, VALIDf, 0);
