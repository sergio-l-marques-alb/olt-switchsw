...
soc_endian_config(int unit)
{
    ...
    if (soc_feature(unit, soc_feature_cmicm)) {
        ...
        if (soc_feature(unit, soc_feature_iproc)) {
             ...
             */
            if ((soc_cm_get_bus_type(unit) & SOC_PCI_DEV_TYPE) && big_pio) {
                /*
                 ...
                 */
                big_packet = (big_packet) ? 0 : 1;
                big_other = (big_other) ? 0 : 1;
                /*
                 * Given that big_pio is set, simply clearing
                 * pio_endian is identical to:
                 *
                 *   big_pio = (big_pio) ? 0 : 1;
                 *   pio_endian = (big_pio) ? 0x01010101 : 0;
                 */
                pio_endian = 0;
            }
...
*/
STATIC soc_error_t
_soc_xgxs_reset_tsc_triplet(int unit, soc_port_t port, int even_block) {
    ...
    int         start_index;
    uint64      rval64;
    /* TD2+ specific block layout.  See comment in trident2.c
     * for function soc_trident2_port_config_init for more details.
     */
    if (even_block) {
        start_index = 0;
    } else {
...
soc_tsc_xgxs_reset(int unit, soc_port_t port, int reg_idx)
{
    ...
    if (!SOC_IS_TRIDENT2X(unit)) {
        ...
        /*
         * See comments in trident2.c for soc_trident2_port_config_init
         * to see where the calculation for block_num is derived
         * In TD2/TD2+, port blocks are arranged in an alternating configuration
         * Odd PGWs have TSCs mapped as follows:
         *  PGW_TSC0_CTRL_REGr -> TSC-4
         *  PGW_TSC1_CTRL_REGr,->|       |
         *  PGW_TSC2_CTRL_REGr,->|TSC-12 |
         *  PGW_TSC3_CTRL_REGr,->|       |
         * Even PGWs have them mapped in the following configuration.
         *  PGW_TSC0_CTRL_REGr,->|       |
        ...
        if (!SOC_IS_TRIDENT2PLUS(unit) ||
            ...
            rv = _soc_xgxs_reset_tsc_triplet(unit, port, even_block);
        }
    }
    return rv;
}
