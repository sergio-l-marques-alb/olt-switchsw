...
int pm4x25_pm_init(int unit, const portmod_pm_create_info_internal_t* pm_add_info, int wb_buffer_index, pm_info_t pm_info)
{
    ...
    pm_is_active = 0;
    rv = PM4x25_IS_ACTIVE_SET(unit, pm_is_active);
    _SOC_IF_ERR_EXIT(rv);

    bypass_enable = 0;
    rv = PM4x25_IS_BYPASSED_SET(unit, bypass_enable);
    _SOC_IF_ERR_EXIT(rv);

    three_ports_mode = 0;
    rv = PM4x25_THREE_PORTS_MODE_SET(unit, three_ports_mode);
    _SOC_IF_ERR_EXIT(rv);
...
int _pm4x25_pm_enable(int unit, int port, pm_info_t pm_info, int port_index, const portmod_port_add_info_t* add_info, int enable)
{
    ...
    if(enable) {
        ...
        /* Port configuration */
        rv = PM4x25_THREE_PORTS_MODE_GET(unit, &three_ports_mode);
        _SOC_IF_ERR_EXIT(rv);
        ...
        for(i=0 ; i<PM_4x25_INFO(pm_info)->nof_phys ; i++) {
            ...
            _SOC_IF_ERR_EXIT(portmod_interface_to_phymod_interface(unit, add_info->interface_config.interface, &core_conf.interface.interface_type));
...
int pm4x25_port_attach(int unit, int port, pm_info_t pm_info, const portmod_port_add_info_t* add_info)
{
    ...
    rv = PM4x25_IS_BYPASSED_GET(unit, &pm_is_bypassed);
    _SOC_IF_ERR_EXIT(rv);
    if(pm_is_bypassed){
    ...
    SOC_PBMP_ITER(PM_4x25_INFO(pm_info)->phys, phy){
        if(SOC_PBMP_MEMBER(port_phys_in_pm, phy)){
            rv = PM4x25_LANE2PORT_SET(unit, ii, port);
            _SOC_IF_ERR_EXIT(rv);
    ...
    rv = PM4x25_IS_ACTIVE_GET(unit, &pm_is_active);
    _SOC_IF_ERR_EXIT(rv);
    ...
    if(PM_4x25_INFO(pm_info)->nof_phys > 0) {
        ...
        /*
         * Fill initial TX[] for all cores.
         */
        for(ii=0 ; ii<PM4X25_LANES_PER_CORE ; ii++) {
          _SOC_IF_ERR_EXIT(phymod_phy_media_type_tx_get(&phy_access, phymodMediaTypeChipToChip, &init_config.tx[ii]));
        }
        init_config.an_en = add_info->autoneg_en;
        init_config.cl72_en = add_info->link_training_en;
        for(ii=0 ; ii<PM4X25_LANES_PER_CORE ; ii++) {
            rv = PM4x25_LANE2PORT_GET(unit, ii, &port_i);
            _SOC_IF_ERR_EXIT(rv);
            ...
            sal_memcpy(&init_config.polarity, &PM_4x25_INFO(pm_info)->polarity, sizeof(init_config.polarity));
...
int pm4x25_port_detach(int unit, int port, pm_info_t pm_info)
{
    ...
    for( i = 0 ; i < MAX_PORTS_PER_PM4X25; i++){
        rv = PM4x25_LANE2PORT_GET(unit, i, &tmp_port);
        _SOC_IF_ERR_EXIT(rv);
        if(tmp_port == port){
            port_index = (port_index == -1 ? i : port_index);
            rv = PM4x25_LANE2PORT_SET(unit, i, invalid_port);
            _SOC_IF_ERR_EXIT(rv);
        } else if (tmp_port != -1){
            is_last_one = FALSE;
        }
    ...
    if(is_last_one){
       ...
       rv = PM4x25_IS_ACTIVE_SET(unit, inactive);
       _SOC_IF_ERR_EXIT(rv);
    }    
exit:
    SOC_FUNC_RETURN; 
...
int pm4x25_port_enable_set(int unit, int port, pm_info_t pm_info, int flags, int enable)
{
    ...
    rv = PM4x25_IS_BYPASSED_SET(unit, is_bypassed);
    _SOC_IF_ERR_EXIT(rv);
...
int pm4x25_ext_phy_attach_to_pm(int unit, pm_info_t pm_info, const phymod_core_access_t *ext_phy_access)
{
    ...
    /* Validate parameters */
    _SOC_IF_ERR_EXIT(phymod_core_access_t_validate(ext_phy_access));
...
int pm4x25_port_update(int unit, int port, pm_info_t pm_info, const portmod_port_update_control_t* update_control)
{
    ...
    if(!link) {
        ...
        if(autoneg.enabled && autoneg.locked) {
            /* update MAC */
            _SOC_IF_ERR_EXIT(clmac_enable_set(unit, port, 0, 0));
            _SOC_IF_ERR_EXIT(clmac_speed_set(unit, port, autoneg.data_rate));
