...
static int num_shared_alpm_banks[SOC_MAX_NUM_DEVICES] = {4};
...
soc_tomahawk_port_speed_update(int unit, soc_port_t port, int speed)
{
    ...
    sal_memset(entry, 0, sizeof(egr_per_port_buffer_sft_reset_entry_t));
    soc_mem_field32_set(unit, EGR_PER_PORT_BUFFER_SFT_RESETm, entry, ENABLEf, 1);
    SOC_IF_ERROR_RETURN
        (WRITE_EGR_PER_PORT_BUFFER_SFT_RESETm(unit, MEM_BLOCK_ALL, port, entry));
    sal_memset(entry, 0, sizeof(egr_per_port_buffer_sft_reset_entry_t));
    SOC_IF_ERROR_RETURN
        (WRITE_EGR_PER_PORT_BUFFER_SFT_RESETm(unit, MEM_BLOCK_ALL, port, entry));

    /* Clear MMU port credits */
    ...
    /* 
     * Enable egress - this will internally run the h/w state machine and 
     * update the MMU credits 
     */
...
_soc_tomahawk_tdm_calculation(int unit, int initTime, _soc_tomahawk_tdm_t *tdm)
{
    ...
    PBMP_PORT_ITER(unit, port) {
        ...
        if (initTime) {
            ...
            tdm->speed[phy_port] = 25000 * si->port_num_lanes[port];
        }
    }
...
_soc_tomahawk_misc_init(int unit)
{
    ...
    if (parity_enable) {
        if ((!SAL_BOOT_SIMULATION || SAL_BOOT_QUICKTURN) &&
            !SOC_WARM_BOOT(unit)) {
            SOC_IF_ERROR_RETURN(soc_tomahawk_clear_all_port_data(unit));
            SOC_IF_ERROR_RETURN(soc_mem_clear(unit, EFP_COUNTER_TABLEm,
                                              COPYNO_ALL, 1));
    ...
    SOC_BLOCK_ITER(unit, block_info_idx, SOC_BLK_CLPORT) {
        ...
        rval = 0;
        soc_reg_field_set(unit, CLPORT_FLOW_CONTROL_CONFIGr, &rval,
                          MERGE_MODE_ENf, 1);
        SOC_IF_ERROR_RETURN
            (WRITE_CLPORT_FLOW_CONTROL_CONFIGr(unit, port, rval));
    }
 ...
 * Returns : Number of Cells reserved additionally for CT/Flex
 */
STATIC int
                               ...
                               _soc_mmu_device_info_t *devcfg, int lossless)
{
    ...
    int in_reserved = 0, asf_reserved = 0;
    ...
    for (idx = 0; idx < NUM_PIPE(unit); idx++) {
        ...
        asf_reserved += soc_th_mmu_additional_buffer_reserve(unit, idx, FALSE);
    }
    /* total_reserve is for all pipes. Each XPE operates on a pair of pipes.
     * Hence asf_reserved = asf_reserved / 2 for each XPE
     */
    asf_reserved /= 2;
                                         ...
                                         int lossless)
{
    ...
    for (idx = 0; idx < NUM_PIPE(unit); idx++) {
        total_reserved += soc_th_mmu_additional_buffer_reserve(unit, idx, TRUE);
    }
    /* total_reserve is for all pipes. Each XPE operates on a pair of pipes.
                              ...
                              _soc_mmu_device_info_t *devcfg, int lossless)
{
    ...
    for (idx = 0; idx < _TH_MMU_NUM_POOL; idx++) {
        ...
        /* In Tomahawk, each XPE carries half the ports/queues.
         * Hence queue_guarantee/2 represents the value for each XPE.
         * RQE queue limit (88) is also included in the queue_guranatee. 
         * Reducing 44, to offset RQE limit for the per XPE calculation
         */
