/*
 * $Id: pbsmh.c,v 1.25 Broadcom SDK $
                       ...
                       soc_pbsmh_field_t field)
{
    ...
    case PBSMH_lm_ctr1_index: return (mh->overlay3.ctr1_id_1 |
                                      (mh->overlay3.ctr1_id_2 << 8));
    case PBSMH_lm_ctr2_index: return (mh->overlay3.ctr2_id_1 |
                                      (mh->overlay3.ctr2_id_2 << 3));
    case PBSMH_ctr1_action:   return mh->overlay3.ctr1_action;
    case PBSMH_ctr2_action:   return mh->overlay3.ctr2_action;
...
soc_pbsmh_dump(int unit, char *pfx, soc_pbsmh_hdr_t *mh)
{
    ...
    case 32:
        ...
        LOG_CLI((BSL_META_U(unit,
                            "%s0x%02x%02x%02x%02x "
                            "<SPID_OVERRIDE=%d INPUT_PRI=%d SPAP=%d "
                            "SPID=%d COS=%d\n"),
                 pfx,
                 ((uint8 *)mh)[8],
                 ((uint8 *)mh)[9],
