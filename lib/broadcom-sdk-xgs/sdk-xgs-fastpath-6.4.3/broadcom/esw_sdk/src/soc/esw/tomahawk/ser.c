        ...
        for (i = 0; mmu_intr_info[i].int_statf != INVALIDf; i++) {
            if (soc_reg_field_get(unit, info_list->intr_status_reg, rval,
                ...
                LOG_CLI((BSL_META_U(unit,
                                    ...
                                    "= %d !!\n"),
                         rval, mmu_intr_info[i].int_statf, i));
#endif /* _SOC_SER_ENABLE_CLI_DBG */
        ...
        if (soc_mem_field32_get(unit, mem, entry, VALIDf)) {
            ...
            if (regmem == _SOC_TH_SER_MEM) {
                ...
                if (non_sbus == 0) {
                    ...
                    if (spci.mem != INVALIDm) {
                        spci.flags |= SOC_SER_REG_MEM_KNOWN; 
                        spci.index = _soc_tomahawk_mem_index_remap(unit, spci.mem, index);
...
typedef struct _soc_th_ser_port_clear_info_s {
    uint32 reg_base_addr;
    int num_regs;
    int reg_inc_addr;
    int blk;
    int reg_is_64;
} _soc_th_ser_port_clear_info_t;

static const
_soc_th_ser_port_clear_info_t port_clear_info[] = {
    { 0x28000000, 12, 0x100, 2, 1 }, /* TDBGCn */
    { 0x28000c00,  1,   0x0, 2, 1 }, /* EGR_TRILL_TX_PKTS */
    { 0x28000d00,  1,   0x0, 2, 1 }, /* EGR_TRILL_TX_ACCESS_PORT_TRILL_PKTS_DISCARDED */
    { 0x28000e00,  1,   0x0, 2, 1 }, /* EGR_TRILL_TX_NETWORK_PORT_NON_TRILL_PKTS_DISCARDED */

    { 0x20000700,  1,   0x0, 2, 1 }, /* EGR_1588_LINK_DELAY_64 */

    { 0x34000200,  1,   0x0, 1, 1 }, /* ING_TRILL_ADJACENCY */

    { 0x54000500,  1,   0x0, 1, 1 }, /* RIPD4 */
    { 0x54000600,  1,   0x0, 1, 1 }, /* RIPC4 */
    { 0x54000700,  1,   0x0, 1, 1 }, /* RIPHE4 */
    { 0x54000800,  1,   0x0, 1, 1 }, /* IMRP4 */
    { 0x54000900,  1,   0x0, 1, 1 }, /* RIPD6 */
    { 0x54000a00,  1,   0x0, 1, 1 }, /* RIPC6 */
    { 0x54000b00,  1,   0x0, 1, 1 }, /* RIPHE6 */
    { 0x54000c00,  1,   0x0, 1, 1 }, /* IMRP6 */
    { 0x54000d00,  1,   0x0, 1, 1 }, /* RDISC */
    { 0x54000e00,  1,   0x0, 1, 1 }, /* RUC */
    { 0x54000f00,  1,   0x0, 1, 1 }, /* RPORTD */
    { 0x54001000,  6, 0x100, 1, 1 }, /* RDBGC0-5 */
    { 0x54001600,  1,   0x0, 1, 1 }, /* IUNHGI */
    { 0x54001700,  1,   0x0, 1, 1 }, /* ICTRL */
    { 0x54001800,  1,   0x0, 1, 1 }, /* IBCAST_64 */
    { 0x54001900,  1,   0x0, 1, 1 }, /* ILTOMC_64 */
    { 0x54001a00,  1,   0x0, 1, 1 }, /* IIPMC_64 */
    { 0x54001b00,  1,   0x0, 1, 1 }, /* IUNKOPC */
    { 0x54001c00,  3, 0x100, 1, 1 }, /* RDBGC6-8 */
    { 0x54001f00,  1,   0x0, 1, 1 }, /* ING_TRILL_RX_PKTS */
    { 0x54002000,  1,   0x0, 1, 1 }, /* ING_TRILL_RX_ACCESS_PORT_TRILL_PKTS_DISCARDED */
    { 0x54002100,  1,   0x0, 1, 1 }, /* ING_TRILL_RX_NETWORK_PORT_NON_TRILL_PKTS_DISCARDED */
    { 0x54002200,  1,   0x0, 1, 1 }, /* ING_NIV_RX_FRAMES_ERROR_DROP */
    { 0x54002300,  1,   0x0, 1, 1 }, /* ING_NIV_RX_FRAMES_FORWARDING_DROP */
    { 0x54002400,  1,   0x0, 1, 1 }, /* ING_NIV_RX_FRAMES_VLAN_TAGGED */
    { 0x54002500,  1,   0x0, 1, 1 }, /* ING_ECN_COUNTER_64 */

    { 0x44000100,  1,   0x0, 1, 0 }, /* STORM_CONTROL_METER_CONFIG */
    { 0 }
};

int
soc_tomahawk_clear_all_port_data(int unit)
{
    int i, port, at;
    uint32 addr;
    uint64 rval64;
    COMPILER_64_ZERO(rval64);
#define _SOC_SER_NUM_PORTS 136
/* #define _SOC_SER_NUM_PORTS NUM_PORT(unit) is config dependent */
#define _SOC_SER_NUM_PIPES NUM_PIPE(unit)
    for (i = 0; (port_clear_info[i].reg_base_addr != 0); i++) {
        addr = port_clear_info[i].reg_base_addr +
               (i * port_clear_info[i].reg_inc_addr);
        for (port = 0; port < _SOC_SER_NUM_PORTS; port++) {
            for (at = 0; at < _SOC_SER_NUM_PIPES; at++) {
                if (port_clear_info[i].reg_is_64) {
                    _soc_reg64_set(unit, port_clear_info[i].blk, at, addr+port,
                                   rval64);
                } else {
                    _soc_reg32_set(unit, port_clear_info[i].blk, at, addr+port,
                                   0);
                }
            }
        }
...
const ser_th_skipped_mem_t th_skipped_mems[] = {
    ...
    { L2_BULKm, _SOC_ACC_TYPE_PIPE_ANY },
    { L2_LEARN_INSERT_FAILUREm, _SOC_ACC_TYPE_PIPE_ANY },

    { IFP_TCAMm, _SOC_ACC_TYPE_PIPE_ANY, 0 },
    { IFP_TCAM_WIDEm, _SOC_ACC_TYPE_PIPE_ANY, 0 },
    ...
    { IS_TDM_CALENDAR0m, _SOC_ACC_TYPE_PIPE_ANY, 0 },
    { IS_TDM_CALENDAR0_PIPE0m, _SOC_ACC_TYPE_PIPE_ANY, 0 },
    { IS_TDM_CALENDAR0_PIPE1m, _SOC_ACC_TYPE_PIPE_ANY, 0 },
    { IS_TDM_CALENDAR0_PIPE2m, _SOC_ACC_TYPE_PIPE_ANY, 0 },
    { IS_TDM_CALENDAR0_PIPE3m, _SOC_ACC_TYPE_PIPE_ANY, 0 },
    { IS_TDM_CALENDAR1m, _SOC_ACC_TYPE_PIPE_ANY, 0 },
    { IS_TDM_CALENDAR1_PIPE0m, _SOC_ACC_TYPE_PIPE_ANY, 0 },
    { IS_TDM_CALENDAR1_PIPE1m, _SOC_ACC_TYPE_PIPE_ANY, 0 },
    { IS_TDM_CALENDAR1_PIPE2m, _SOC_ACC_TYPE_PIPE_ANY, 0 },
    { IS_TDM_CALENDAR1_PIPE3m, _SOC_ACC_TYPE_PIPE_ANY, 0 },

    { ING_FLEX_CTR_OFFSET_TABLE_0m, _SOC_ACC_TYPE_PIPE_ANY, 0 },
    { ING_FLEX_CTR_OFFSET_TABLE_1m, _SOC_ACC_TYPE_PIPE_ANY, 0 },
    { ING_FLEX_CTR_OFFSET_TABLE_2m, _SOC_ACC_TYPE_PIPE_ANY, 0 },
    { ING_FLEX_CTR_OFFSET_TABLE_3m, _SOC_ACC_TYPE_PIPE_ANY, 0 },
    { ING_FLEX_CTR_OFFSET_TABLE_4m, _SOC_ACC_TYPE_PIPE_ANY, 0 },
    { ING_FLEX_CTR_OFFSET_TABLE_5m, _SOC_ACC_TYPE_PIPE_ANY, 0 },
    { ING_FLEX_CTR_OFFSET_TABLE_6m, _SOC_ACC_TYPE_PIPE_ANY, 0 },
    { ING_FLEX_CTR_OFFSET_TABLE_7m, _SOC_ACC_TYPE_PIPE_ANY, 0 },
    { ING_FLEX_CTR_OFFSET_TABLE_8m, _SOC_ACC_TYPE_PIPE_ANY, 0 },
    { ING_FLEX_CTR_OFFSET_TABLE_9m, _SOC_ACC_TYPE_PIPE_ANY, 0 },
    { ING_FLEX_CTR_OFFSET_TABLE_10m, _SOC_ACC_TYPE_PIPE_ANY, 0 },
    { ING_FLEX_CTR_OFFSET_TABLE_11m, _SOC_ACC_TYPE_PIPE_ANY, 0 },
    { ING_FLEX_CTR_OFFSET_TABLE_13m, _SOC_ACC_TYPE_PIPE_ANY, 0 },
    { ING_FLEX_CTR_OFFSET_TABLE_14m, _SOC_ACC_TYPE_PIPE_ANY, 0 },
    { ING_FLEX_CTR_OFFSET_TABLE_15m, _SOC_ACC_TYPE_PIPE_ANY, 0 },
    { ING_FLEX_CTR_OFFSET_TABLE_16m, _SOC_ACC_TYPE_PIPE_ANY, 0 },
    { ING_FLEX_CTR_OFFSET_TABLE_17m, _SOC_ACC_TYPE_PIPE_ANY, 0 },
    { ING_FLEX_CTR_OFFSET_TABLE_18m, _SOC_ACC_TYPE_PIPE_ANY, 0 },
    { ING_FLEX_CTR_OFFSET_TABLE_19m, _SOC_ACC_TYPE_PIPE_ANY, 0 },

    { EGR_FLEX_CTR_OFFSET_TABLE_0m, _SOC_ACC_TYPE_PIPE_ANY, 0 },
    { EGR_FLEX_CTR_OFFSET_TABLE_1m, _SOC_ACC_TYPE_PIPE_ANY, 0 },
    { EGR_FLEX_CTR_OFFSET_TABLE_2m, _SOC_ACC_TYPE_PIPE_ANY, 0 },
    { EGR_FLEX_CTR_OFFSET_TABLE_3m, _SOC_ACC_TYPE_PIPE_ANY, 0 },


     { SRC_MODID_INGRESS_BLOCKm, _SOC_ACC_TYPE_PIPE_ANY, 0 },
     { EGR_VLAN_STGm, _SOC_ACC_TYPE_PIPE_ANY, 0 },

    { EGR_IP_TUNNELm, _SOC_ACC_TYPE_PIPE_ANY, 0 },
    { EGR_IP_TUNNEL_IPV6m, _SOC_ACC_TYPE_PIPE_ANY, 0 },
    { EGR_IP_TUNNEL_MPLSm, _SOC_ACC_TYPE_PIPE_ANY, 0 },
    { L3_DEFIP_DATA_ONLYm, _SOC_ACC_TYPE_PIPE_ANY, 0 },
...
STATIC soc_error_t
_soc_tomahawk_perform_ser_test(int unit, ser_test_data_t *test_data,
                               ...
                               int *mem_skipped, int *mem_failed)
{
    ...
    for (k = 0; th_skipped_mems[k].mem != INVALIDm; k++) {
        if ((th_skipped_mems[k].mem == test_data->mem) &&
    ...
    if (!(SOC_MEM_INFO(unit, test_data->mem).flags & SOC_MEM_SER_FLAGS)) {
        ...
        LOG_CLI((BSL_META_U(unit,
                 ...
                 test_data->mem_name, SOC_MEM_INFO(unit,test_data->mem).flags));
#endif /* _SOC_SER_ENABLE_CLI_DBG */
        skip_mem = TRUE;
    }
    /* skip mems which could be of 0 depth due to config properties */
    if (!soc_mem_index_count(unit, test_data->mem))	{
#ifdef _SOC_SER_ENABLE_CLI_DBG
        LOG_CLI((BSL_META_U(unit,
    ...
    mem_acc_type = SOC_MEM_ACC_TYPE(unit, test_data->mem);
    if ((mem_acc_type == _SOC_TH_ACC_TYPE_ADDR_SPLIT_DIST) ||
        (mem_acc_type == _SOC_TH_ACC_TYPE_ADDR_SPLIT_SPLIT)) {
#ifdef _SOC_SER_ENABLE_CLI_DBG
        LOG_CLI((BSL_META_U(unit,
                            "Memory %s skipped as acc_type is %0d \n"),
                 SOC_MEM_NAME(unit,test_data->mem), mem_acc_type));
#endif /* _SOC_SER_ENABLE_CLI_DBG */
        skip_mem = TRUE;
    }
    ...
    if (!skip_mem) {
        ...
        if (SOC_FAILURE(rv)) {
            LOG_CLI((BSL_META_U(unit,
                                ...
                                " \n"),
                     SOC_MEM_NAME(unit,test_data->mem), test_data->acc_type));
        } else {
            LOG_CLI((BSL_META_U(unit,
   ...
   if (found_mem) {
       if (inject_only) {
           ...
           if (cmd) {
               ...
               rv = _soc_tomahawk_perform_ser_test(unit, &test_data, test_type,
...
soc_th_ser_tcam_test(int unit, _soc_ser_test_t test_type)
{
    ...
    for (i = 0; tcams[i].mem != INVALIDm; i++) {
        ...
        _soc_tomahawk_perform_ser_test(unit, &test_data, test_type,
...
#ifdef _SOC_SER_ENABLE_CLI_DBG
    LOG_CLI((BSL_META_U(unit, "TCAM tests skipped:\t%d \n"),
             mem_skipped));
#endif
...
soc_th_ser_hardware_test(int unit, _soc_ser_test_t test_type) {
            ...
            for (j = 0; fifo_parity[j].mem != INVALIDm; j++) {
                ...
                if (mem_acc_type > _SOC_UNIQUE_ACC_TYPE_PIPE_3) {
                    for (test_data.acc_type = _SOC_UNIQUE_ACC_TYPE_PIPE_1;
                         ...
                         test_data.acc_type++) {
#if defined(_SER_DBG_EXIT_ON_FIRST_FAILURE)
                        rv = _soc_tomahawk_perform_ser_test(unit, &test_data,
                                                            test_type,
                                                            &mem_skipped,
                                                            &mem_failed);
                        if (SOC_FAILURE(rv)) {
...
#else
                        _soc_tomahawk_perform_ser_test(unit, &test_data,
                                                       test_type,
                                                       &mem_skipped,
                                                       &mem_failed);
#endif
    ...
    LOG_CLI((BSL_META_U(unit, "H/W tests passed:\t%d\n"),
             mem_tests - mem_failed - mem_skipped));
#ifdef _SOC_SER_ENABLE_CLI_DBG
    LOG_CLI((BSL_META_U(unit, "H/W tests skipped:\t%d \n"),
             mem_skipped));
#endif
...
soc_th_ser_test(int unit, _soc_ser_test_t test_type) {
    ...
    soc_ser_test_long_sleep_time_us = 100000;
