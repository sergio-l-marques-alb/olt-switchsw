...
#define SOC_MEM_IS_ALIAS_OR_FORMAT(unit,mem)    (SOC_MEM_IS_VALID(unit, SOC_MEM_ALIAS_MAP(unit,mem)))
...
const soc_mem_t  default_tested_mems_list[] = {
      IHP_MEM_500000m,
      IHP_MEM_590000m,
      IRR_MCDBm,
      PPDB_B_LIF_TABLEm,
      ...
      IPT_FDTDATAm,
      PPDB_A_TCAM_ACTIONm,
      IPS_QUEUE_TYPE_LOOKUP_TABLEm,
      EPNI_MEM_520000m,
      IPS_FLOW_ID_LOOKUP_TABLEm,
...
static soc_mem_t  ser_mem_name_to_mem(int unit, char *name)
{
    int         copyno;
    soc_mem_t   mem;
    unsigned    array_index;
    ...
    for (i=0;i< numels;i++) {
        ...
        for (entry_data = ser_mem_read->mem_ptr,entry_index = memp->index_min; entry_index <= memp->index_max; ++entry_index) {
            ...
            if ( nof_error_bits>0 ) {
                ...
                ser_mem_read->num_of_err_entries += nof_error_bits ? 1 : 0;
            }
        }
...
STATIC int mark_tested_mems(int unit,int nof_requested_mems, char **requested_mems,uint8 *tested_mems)
{
    ...
    int len=sizeof(  default_tested_mems_list)/sizeof(soc_mem_t); 
    if (nof_requested_mems>0) {

        for (i=0;i<nof_requested_mems;i++) {
            mem = ser_mem_name_to_mem(unit,requested_mems[i]);
            if ((mem == INVALIDm)) {
                return -1;
            }
            tested_mems[mem] = 1;
    ...
    for (i=0;i<len;i++) {
        tested_mems[default_tested_mems_list[i]] = 1;
    }
    return 0;
}
...
static int diag_mems_ser(int unit, uint8 time_units, int sleep_time, int num_of_iterations, int num_of_read_iterations, int num_of_patterns, int *patterns, int nof_requested_mems, char **requested_mems ,uint32 error_constant)
{
    int i,j,k;
    ...
    /* Dma mem Allocation */
    if ((ser_mem_read.mem_ptr =sal_alloc(sizeof(uint32) * ser_mem_init.max_table_size, "")) == NULL) {
        return -1;
    }
    /*
    ...
    */
    ...
    for (i=0; i < MAX(num_of_iterations,1);i++) {
        ...
        for (j=0; j < num_of_patterns;j++) {
            ...
            for (k=0; k< MAX(num_of_read_iterations,1);k++) {
