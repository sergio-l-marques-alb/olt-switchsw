 ...
 */
{
    ...
    if (ulen > 0) {
        for (ualt = 0; ualt < soc_ndev && rv == CMD_OK; ualt++) {
            if ((umap & (1 << SOC_NDEV_IDX2DEV(ualt))) != 0) {
            args_t        *a_copy;
            ...
            sh_swap_unit_vars(SOC_NDEV_IDX2DEV(ualt));
            ...
            rv = sh_process_statement(SOC_NDEV_IDX2DEV(ualt), a_copy);
...
appl_warmboot_storage_requirements(int unit)
{
    ...
    uint32 module, storage_per_module[BCM_MODULE__COUNT+3] = {0};
    int part_count = 0;
    uint32 size = 0;
...
#if defined(BCM_DPP_SUPPORT) || defined(BCM_DFE_SUPPORT)
    "Parameters: [on | off | show | shutdown | storage | usage | test_mode <on|off|dvapi|no_wb_test|allow_wb_test>]\n"
#else
