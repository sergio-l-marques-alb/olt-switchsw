/*
 * $Id: verinet.c,v 1.22 Broadcom SDK $
...
int pli_client_attach(int devNo)
{
	...
	cli_out("pli_client_attach: can't resolve host names in vxworks\n");
    }
	...
	cli_out("pli_client_attach: could not connect unit %d to [%s.%d] "
	       "-service unavailable.\n",
    ...
    if (getsockname(v->sockfd, (struct sockaddr *)&cli_addr, &cli_addr_size) < 0) {
        perror("pli_client_attach: getsockname function failed.");
        goto error;
    }
				 ...
				 uint32 netAddr)
{
...
#if 0
    cli_out("register_client:s=%d irqPort=%d dmaPort=%d netAddr=%d.%d.%d.%d\n",
	...
	cli_out("Error: INTC connect failed to [0x%x.%d] -service unavailable.\n",
	       netAddr, intPort);
	perror("connect");
	close(intSockFD);
	free(nc);
	...
	cli_out("Error: DMAC connect failed to [0x%x.%d] -service unavailable.\n",
	       netAddr, dmaPort);
	perror("connect");
	close(intSockFD);
	close(dmaSockFD);
