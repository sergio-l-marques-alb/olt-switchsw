/* $Id: ui_ppd_api_eg_filter.c,v 1.8 Broadcom SDK $
...
{   
  ...
  SOC_PPD_EG_FILTER_PORT_INFO_clear(&prm_port_info);
  ...
  { 
    ...
    if (param_val->numeric_equivalent == SOC_PPD_EG_FILTER_PORT_ENABLE_NONE)
    {
...
{   
  ...
  SOC_PPD_EG_FILTER_PORT_INFO_print(&prm_port_info);
...
{   
  ...
  unit = soc_ppd_get_default_unit(); 
  SOC_PPD_LLP_PARSE_INFO_clear(&prm_eg_prsr_out_key);
...
{   
  ...
  SOC_PPD_EG_FILTER_PVLAN_PORT_TYPE   
    prm_pvlan_port_type;
...
{   
  ...
  cli_out("pvlan_port_type: %s\n\r",SOC_PPD_EG_FILTER_PVLAN_PORT_TYPE_to_string(prm_pvlan_port_type));
