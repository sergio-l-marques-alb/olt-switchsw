...
int enable_sch_gtimer(int unit, int core, int nof_clocks_int) {
   ...
   if (SOC_IS_JERICHO(unit)) {
       ...
       if (READ_SCH_SCHEDULER_CONFIGURATION_REGISTERr(unit, REG_PORT_ANY, &val32) != BCM_E_NONE) return BCM_E_FAIL;
       soc_reg_field_set(unit, SCH_SCHEDULER_CONFIGURATION_REGISTERr, &val32, COUNT_BY_GTIMERf, 1);   
       if (WRITE_SCH_SCHEDULER_CONFIGURATION_REGISTERr(unit, REG_PORT_ANY, val32) != BCM_E_NONE) return BCM_E_FAIL;
   }
...
int disable_sch_gtimer(int unit) {
   uint32  val32 = 0;
   int     core;
...
int enable_egq_gtimer(int unit, int nof_clocks_int) {
   /*Enable gtimer for EGQ sub-block inside the EGQ and enable the gtimer for clk of 600MHz:*/
   if (WRITE_EGQ_GTIMER_CONFIGr(unit, REG_PORT_ANY, nof_clocks_int) != BCM_E_NONE) return BCM_E_FAIL;
   if (WRITE_EGQ_GTIMER_CONFIG_CONTr(unit, REG_PORT_ANY, 1) != BCM_E_NONE) return BCM_E_FAIL; /*CLEAR_GTIMER=0x1,ACTIVATE_GTIMER=0x0 */
   if (WRITE_EGQ_GTIMER_CONFIG_CONTr(unit, REG_PORT_ANY, 3) != BCM_E_NONE) return BCM_E_FAIL; /*CLEAR_GTIMER=0x1,ACTIVATE_GTIMER=0x1 */

   /* Enable gtimer for EPNI sub-block inside the EPNI and enable the gtimer for clk of 600MHz:*/
   if (WRITE_EPNI_GTIMER_CONFIGr(unit, REG_PORT_ANY, nof_clocks_int) != BCM_E_NONE) return BCM_E_FAIL;
   if (WRITE_EPNI_GTIMER_CONFIG_CONTr(unit, REG_PORT_ANY, 1) != BCM_E_NONE) return BCM_E_FAIL;
   if (WRITE_EPNI_GTIMER_CONFIG_CONTr(unit, REG_PORT_ANY, 3) != BCM_E_NONE) return BCM_E_FAIL;
   return CMD_OK;
}

int get_sch_otm_port(int unit, int local_port_id, int *otm_port) {
   uint32  val32;
   if (READ_SCH_DSP_2_PORT_MAP_DSPPm(unit, SOC_BLOCK_ANY, local_port_id, &val32) != BCM_E_NONE) {
       return BCM_E_FAIL;
   }
...
int get_otm_port(int unit, int local_port_id, int *otm_port) {
   uint64  val64;
   COMPILER_64_ZERO(val64);
   if (READ_EGQ_DSP_PTR_MAPm(unit, SOC_BLOCK_ANY, local_port_id, &val64) != BCM_E_NONE) {
       return BCM_E_FAIL;
   }
...
int get_sch_ps_mode(int unit, int local_port_id, uint32 *ps_mode) {
   int    otm_port;
   int    ps_num;
   uint32 val32_1, val32_2;
   uint32 ps_1p_mode = 0, ps_2p_mode = 0;

   if (get_sch_otm_port(unit, local_port_id, &otm_port) != CMD_OK) return BCM_E_FAIL;
   ps_num   = get_field32(otm_port, 3, 7);

   if (READ_SCH_PS_1P_PRIORITY_MODE_REGISTERr(unit, REG_PORT_ANY, &val32_1) != BCM_E_NONE) return BCM_E_FAIL;
   if (READ_SCH_PS_2P_PRIORITY_MODE_REGISTERr(unit, REG_PORT_ANY, &val32_2) != BCM_E_NONE) return BCM_E_FAIL;
...
int get_ps_mode(int unit, int local_port_id, uint32 *ps_mode) {
   int    otm_port;
   int    ps_num;
   uint64 val64;
   ...
   ps_num   = get_field32(otm_port, 3, 7);
   if (READ_EGQ_PS_MODEr(unit, REG_PORT_ANY, &val64) != BCM_E_NONE) return BCM_E_FAIL;
...
int calc_sch_ps_rate(int unit, int local_port_id) {
   ...
   if ((local_port_id != -1) && (local_port_id < 256)) {
      if (get_sch_otm_port(unit, local_port_id, &otm_port) != BCM_E_NONE) return BCM_E_FAIL;
      ps_num   = get_field32(otm_port, 3, 7);
      if (get_sch_ps_mode(unit, local_port_id, &ps_mode) != BCM_E_NONE) return BCM_E_FAIL;
   ...
   if (soc_port_sw_db_core_get(unit, local_port_id, &core) != SOC_E_NONE) return BCM_E_FAIL;
   ...
   for (i = 0; i < max_tc; i++) {
      ...
      if (READ_SCH_CREDIT_COUNTER_CONFIGURATION_REG_2r(unit, REG_PORT_ANY, &val32) != BCM_E_NONE) return BCM_E_FAIL;
      ...
      if (WRITE_SCH_CREDIT_COUNTER_CONFIGURATION_REG_2r(unit, REG_PORT_ANY, val32) != BCM_E_NONE) return BCM_E_FAIL;

      enable_sch_gtimer(unit, core, nof_clocks_int);
      sal_usleep(100000);    /* Wait 0.1 sec */
      
      if (READ_SCH_DVS_CREDIT_COUNTERr(unit, REG_PORT_ANY, &counter_data[hr_num]) != BCM_E_NONE) return BCM_E_FAIL;
...
int calc_sch_flow_bw(int unit, int flow_id) {
   ...
   SOC_DPP_CORES_ITER(SOC_CORE_ALL, core) {
       if (READ_SCH_CREDIT_COUNTER_CONFIGURATION_REG_1r(unit, REG_PORT_ANY, &reg_val1) != BCM_E_NONE) return BCM_E_FAIL;
       soc_reg_field_set(unit, SCH_CREDIT_COUNTER_CONFIGURATION_REG_1r, &reg_val1, FILTER_FLOWf, flow_id);
       if (WRITE_SCH_CREDIT_COUNTER_CONFIGURATION_REG_1r(unit, REG_PORT_ANY, reg_val1) != BCM_E_NONE) return BCM_E_FAIL;

       if (READ_SCH_CREDIT_COUNTER_CONFIGURATION_REG_2r(unit, REG_PORT_ANY, &reg_val1) != BCM_E_NONE) return BCM_E_FAIL;
       ...
       if (WRITE_SCH_CREDIT_COUNTER_CONFIGURATION_REG_2r(unit, REG_PORT_ANY, reg_val1) != BCM_E_NONE) return BCM_E_FAIL;
...
int calc_egq_rate(int unit, int local_port_id, int tc_indx) {
   ...
   if (enable_egq_gtimer(unit, nof_clocks_int) != BCM_E_NONE) return BCM_E_FAIL;

   if ((local_port_id != -1) && (local_port_id < 256)) {
	  if (get_otm_port(unit, local_port_id, &otm_port) != CMD_OK) return BCM_E_FAIL;
	  ps_num   = get_field32(otm_port, 3, 7);
	  if (get_ps_mode(unit, local_port_id, &ps_mode) != CMD_OK) return BCM_E_FAIL;
	  ...
	  if (WRITE_EGQ_CHECK_BW_TO_OFPr(unit, REG_PORT_ANY, 0x100 + qp_num) != BCM_E_NONE) return BCM_E_FAIL;
   }
   ...
   if (READ_EGQ_EHP_UNICAST_PACKET_COUNTERr(unit, REG_PORT_ANY, &val64) != BCM_E_NONE) return BCM_E_FAIL;
   print_packet_rate(val64, "EGQ_EHP_UNICAST_PACKET_COUNTER");
   if (READ_EGQ_EHP_MULTICAST_HIGH_PACKET_COUNTERr(unit, REG_PORT_ANY, &val64) != BCM_E_NONE) return BCM_E_FAIL;
   print_packet_rate(val64, "EGQ_EHP_MULTICAST_HIGH_PACKET_COUNTER");
   if (READ_EGQ_EHP_MULTICAST_LOW_PACKET_COUNTERr(unit, REG_PORT_ANY, &val64) != BCM_E_NONE) return BCM_E_FAIL;
   print_packet_rate(val64, "EGQ_EHP_MULTICAST_LOW_PACKET_COUNTER");
   if (READ_EGQ_EHP_DISCARD_PACKET_COUNTERr(unit, REG_PORT_ANY, &val64) != BCM_E_NONE) return BCM_E_FAIL;
   print_packet_rate(val64, "EGQ_EHP_DISCARD_PACKET_COUNTER");
   if (READ_EGQ_PQP_UNICAST_PACKET_COUNTERr(unit, REG_PORT_ANY, &val64) != BCM_E_NONE) return BCM_E_FAIL;
   print_packet_rate(val64, "EGQ_PQP_UNICAST_PACKET_COUNTER");
   if (READ_EGQ_PQP_MULTICAST_PACKET_COUNTERr(unit, REG_PORT_ANY, &val64) != BCM_E_NONE) return BCM_E_FAIL;
   print_packet_rate(val64, "EGQ_PQP_MULTICAST_PACKET_COUNTER");
   if (READ_EGQ_PQP_DISCARD_UNICAST_PACKET_COUNTERr(unit, REG_PORT_ANY, &val64) != BCM_E_NONE) return BCM_E_FAIL;
   print_packet_rate(val64, "EGQ_PQP_DISCARD_UNICAST_PACKET_COUNTER");
   if (READ_EGQ_PQP_DISCARD_MULTICAST_PACKET_COUNTERr(unit, REG_PORT_ANY, &val64) != BCM_E_NONE) return BCM_E_FAIL;
   print_packet_rate(val64, "EGQ_PQP_DISCARD_MULTICAST_PACKET_COUNTER");
   if (READ_EGQ_FQP_PACKET_COUNTERr(unit, REG_PORT_ANY, &val64) != BCM_E_NONE) return BCM_E_FAIL;
   print_packet_rate(val64, "EGQ_FQP_PACKET_COUNTER");
   if (READ_EPNI_EPE_PACKET_COUNTERr(unit, REG_PORT_ANY, &val64) != BCM_E_NONE) return BCM_E_FAIL;
   print_packet_rate(val64, "EPNI_EPE_PACKET_COUNTER");
   if (READ_EPNI_EPE_DISCARDED_PACKETS_COUNTERr(unit, REG_PORT_ANY, &val64) != BCM_E_NONE) return BCM_E_FAIL;
   print_packet_rate(val64, "EPNI_EPE_DISCARDED_PACKETS_COUNTER");

   if (READ_EGQ_PQP_UNICAST_BYTES_COUNTERr(unit, REG_PORT_ANY, &val64) != BCM_E_NONE) return BCM_E_FAIL;
   print_bit_rate(val64, "EGQ_PQP_UNICAST_BYTES_COUNTER");
   if (READ_EGQ_PQP_MULTICAST_BYTES_COUNTERr(unit, REG_PORT_ANY, &val64) != BCM_E_NONE) return BCM_E_FAIL;
   print_bit_rate(val64, "EGQ_PQP_MULTICAST_BYTES_COUNTER");
   if (READ_EPNI_EPE_BYTES_COUNTERr(unit, REG_PORT_ANY, &val64) != BCM_E_NONE) return BCM_E_FAIL;
   print_bit_rate(val64, "EPNI_EPE_BYTES_COUNTER");

   if (WRITE_EGQ_GTIMER_CONFIG_CONTr(unit, REG_PORT_ANY, 0) != BCM_E_NONE) return BCM_E_FAIL;
   if (WRITE_EPNI_GTIMER_CONFIG_CONTr(unit, REG_PORT_ANY, 0) != BCM_E_NONE) return BCM_E_FAIL;
   return CMD_OK;
}
...
int calc_pqp_rate(int unit, int local_port_id, int tc_indx) {
   ...
   if (enable_egq_gtimer(unit, nof_clocks_int) != CMD_OK) return BCM_E_FAIL;
   ...
   if (WRITE_EGQ_CHECK_BW_TO_OFPr(unit, REG_PORT_ANY, 0) != BCM_E_NONE) return BCM_E_FAIL;
   if (WRITE_EGQ_GTIMER_CONFIG_CONTr(unit, REG_PORT_ANY, 0) != BCM_E_NONE) return BCM_E_FAIL;
   if (WRITE_EPNI_GTIMER_CONFIG_CONTr(unit, REG_PORT_ANY, 0) != BCM_E_NONE) return BCM_E_FAIL;
   return CMD_OK;
}
...
int calc_epni_rate(int unit, int check_bw_scheme, int check_bw_num) {
   ...
   if (enable_egq_gtimer(unit, nof_clocks_int) != CMD_OK) return BCM_E_FAIL;
   if (WRITE_EPNI_MASK_CHECK_BW_TO_PACKET_DESCRIPTORr(unit, REG_PORT_ANY, 0x1fffbff) != BCM_E_NONE) return BCM_E_FAIL;

   /*calc nif port rate*/
   if (check_bw_scheme == 1) {
	  if (WRITE_EPNI_MASK_CHECK_BW_TO_PACKET_DESCRIPTORr(unit, REG_PORT_ANY, 0x1ff03ff) != BCM_E_NONE) return BCM_E_FAIL;
	  if (WRITE_EPNI_CHECK_BW_TO_PACKET_DESCRIPTORr(unit, REG_PORT_ANY, 2048 * check_bw_num) != BCM_E_NONE) return BCM_E_FAIL;
   }
   /*calc ofp rate*/
   else if (check_bw_scheme == 2) {
	  if (WRITE_EPNI_MASK_CHECK_BW_TO_PACKET_DESCRIPTORr(unit, REG_PORT_ANY, 0x1fffb07) != BCM_E_NONE) return BCM_E_FAIL;
	  if (WRITE_EPNI_CHECK_BW_TO_PACKET_DESCRIPTORr(unit, REG_PORT_ANY, check_bw_num) != BCM_E_NONE) return BCM_E_FAIL;
   }
   /*calc qp rate*/
   else if (check_bw_scheme == 3) {
	  if (WRITE_EPNI_MASK_CHECK_BW_TO_PACKET_DESCRIPTORr(unit, REG_PORT_ANY, 0x1fffb00) != BCM_E_NONE) return BCM_E_FAIL;
	  if (WRITE_EPNI_CHECK_BW_TO_PACKET_DESCRIPTORr(unit, REG_PORT_ANY, check_bw_num) != BCM_E_NONE) return BCM_E_FAIL;
   }
   /*calc channel rate*/
   else if (check_bw_scheme == 4) {
	  if (WRITE_EPNI_MASK_CHECK_BW_TO_PACKET_DESCRIPTORr(unit, REG_PORT_ANY, 0x100fbff) != BCM_E_NONE) return BCM_E_FAIL;
	  if (WRITE_EPNI_CHECK_BW_TO_PACKET_DESCRIPTORr(unit, REG_PORT_ANY, 65536 * check_bw_num) != BCM_E_NONE) return BCM_E_FAIL;
   }
   /*clac mirrored rate*/
   else if (check_bw_scheme == 5) {
	  if (WRITE_EPNI_MASK_CHECK_BW_TO_PACKET_DESCRIPTORr(unit, REG_PORT_ANY, 0xfffbff) != BCM_E_NONE) return BCM_E_FAIL;
	  if (WRITE_EPNI_CHECK_BW_TO_PACKET_DESCRIPTORr(unit, REG_PORT_ANY, 16777216 * check_bw_num) != BCM_E_NONE) return BCM_E_FAIL;
   }
   /*calc total BW*/
   else {
	  if (WRITE_EPNI_MASK_CHECK_BW_TO_PACKET_DESCRIPTORr(unit, REG_PORT_ANY, 0x1fffbff) != BCM_E_NONE) return BCM_E_FAIL;
...
int calc_epe_port_rate(int unit, int local_port_id, int tc_indx) {
   ...
   /*qp rate calculation*/
	  ...
	  if (WRITE_EPNI_MASK_CHECK_BW_TO_PACKET_DESCRIPTORr(unit, REG_PORT_ANY, 0x1fffb00) != BCM_E_NONE) return BCM_E_FAIL;
	  if (WRITE_EPNI_CHECK_BW_TO_PACKET_DESCRIPTORr(unit, REG_PORT_ANY, qp_num) != BCM_E_NONE) return BCM_E_FAIL;
   }
   /*otm-port rate calculation*/
	  ...
	  if (WRITE_EPNI_MASK_CHECK_BW_TO_PACKET_DESCRIPTORr(unit, REG_PORT_ANY, 0x1fffbff) != BCM_E_NONE) return BCM_E_FAIL;
	  /*second step unmask the relevant parameter*/
	  if (WRITE_EPNI_MASK_CHECK_BW_TO_PACKET_DESCRIPTORr(unit, REG_PORT_ANY, 0x1fffb00 + qp_mask) != BCM_E_NONE) return BCM_E_FAIL;
	  if (WRITE_EPNI_CHECK_BW_TO_PACKET_DESCRIPTORr(unit, REG_PORT_ANY, otm_port) != BCM_E_NONE) return BCM_E_FAIL;
   }
...
/*========================================================================================*/
int calc_epe_if_rate(int unit, int if_num) {
   int     nof_clocks_int;
   uint64   val64;
   ...
   if ((if_num >= 32)) {
	  cli_out("ERROR in calc_epe_if_rate if_num=%d\n", if_num);
	  return CMD_FAIL;
   }
   /*if rate calculation*/
   else if ((if_num != -1)) {
	  cli_out("calc epe if rate: if_num=%d\n", if_num);
	  /*first step mask all parameters*/
	  if (WRITE_EPNI_MASK_CHECK_BW_TO_PACKET_DESCRIPTORr(unit, REG_PORT_ANY, 0x1fffbff) != BCM_E_NONE) return CMD_OK;
	  /*second step unmask the relevant parameter*/
	  if (WRITE_EPNI_MASK_CHECK_BW_TO_PACKET_DESCRIPTORr(unit, REG_PORT_ANY, 0x1fffb00) != BCM_E_NONE) return CMD_OK;
	  if (WRITE_EPNI_CHECK_BW_TO_PACKET_DESCRIPTORr(unit, REG_PORT_ANY, 2048 * if_num) != BCM_E_NONE) return CMD_OK;
   }
   /*total rate calculation*/
   else {
	  cli_out("calc total epni rate\n");
   ...
   /* Enable gtimer for EPNI sub-block inside the EPNI and enable the gtimer for clk of 600MHz:*/
   if (enable_egq_gtimer(unit, nof_clocks_int) != BCM_E_NONE) return CMD_OK;


   sal_usleep(100000);

   /*packet rate*/
   if (READ_EPNI_EPE_PACKET_COUNTERr(unit, REG_PORT_ANY, &val64) != BCM_E_NONE) return CMD_OK;
   print_packet_rate(val64, "EPNI_EPE_PACKET_COUNTER");

   /*bit rate*/
   if (READ_EPNI_EPE_BYTES_COUNTERr(unit, REG_PORT_ANY, &val64) != BCM_E_NONE) return CMD_OK;
   print_bit_rate(val64, "EPNI_EPE_BYTES_COUNTER");

   /*mask all parameters*/
   if (WRITE_EPNI_MASK_CHECK_BW_TO_PACKET_DESCRIPTORr(unit, REG_PORT_ANY, 0x1fffbff) != BCM_E_NONE) return CMD_OK;
   if (WRITE_EGQ_GTIMER_CONFIG_CONTr(unit, REG_PORT_ANY, 0) != BCM_E_NONE) return CMD_OK;
   if (WRITE_EPNI_GTIMER_CONFIG_CONTr(unit, REG_PORT_ANY, 0) != BCM_E_NONE) return CMD_OK;
   return CMD_OK;
}
...
int gtimer_enable(int unit, char* block_name, int period_num) {
   ...
   /* 1. check block_name */
   nof_clocks_1us = 600; /* 600,000,000 clocks --> 1 sec*/
