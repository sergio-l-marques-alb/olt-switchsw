/*
 * $Id: g2xx_cmds.c,v 1.9 Broadcom SDK $
...
cmd_sbx_g2xx_get (int unit, args_t *args)
{
    ...
    if (!SOC_IS_SBX_FE2000(unit)) {
        cli_out("This command will only work on an FE2000.\n");
        return CMD_FAIL;
    }
    ...
    if (sbx->ucodetype != SOC_SBX_UCODE_TYPE_G2XX) {
        cli_out("ERROR: not running g2xx v1.3 microcode\n");
        return CMD_FAIL;
    }
