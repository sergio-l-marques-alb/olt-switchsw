...
typedef struct ddrconfig_s {
    uint8 uValidDelay[6];
    uint8 uPhaseSelect[6];
    uint8 uQkMidRange[6];
    uint8 uQknMidRange[6];
    uint8 uXMidRange[6];
    uint16 chksum;
} ddrconfig_t;
...
brd_fe2kxt_ddrconfig_readwrite(int unit, int write, sbFe2000XtInitParamsMmDdrConfig_t *ddrconfig)
{
    ...
    uint32 length = sizeof(ddrconfig_t);
    uint32 tmp_length;
    ddrconfig_t nvdata;
    uint16 tmp_chksum;
    ...
    devno = soc_bsc_devopen(unit, BSC_AT24C32_NAME);
        if (devno  < 0) {
            cli_out("cannot open I2C device %s\n", BSC_AT24C32_NAME);
            return BOARD_E_FAIL;
        }
    if (write) {
        index = 0;
        ...
        nvdata.chksum = _shr_crc16(0, (uint8*)&nvdata, length-2);
        ...
        if (length != tmp_length) {
            cli_out("%s: read length check failed\n", BSC_AT24C32_NAME);
            return BOARD_E_FAIL;
        }
        tmp_chksum = _shr_crc16(0, (uint8*)&nvdata, length-2);
        if (tmp_chksum != nvdata.chksum) {
          cli_out("ERROR %s: chksum check failed, ddr config NOT loaded, retv = 0x%x\n", BSC_AT24C32_NAME, retv);
            return BOARD_E_FAIL;
        }
        index = 0 ;
        for (i=0; i < SB_FE2000_NUM_MM_INSTANCES; i++) {
