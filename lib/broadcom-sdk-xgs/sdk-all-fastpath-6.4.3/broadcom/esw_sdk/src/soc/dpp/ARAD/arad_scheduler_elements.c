...
/* $Id: arad_scheduler_elements.c,v 1.6 Broadcom SDK $
  ...
  )
{
  ...
  else /* FQ or HR*/
  {
    ...
    se_offset_ndx = arad_sch_flow2se_id(flow_id) - ARAD_CL_SE_ID_MIN;
  }
  ...
  )
{
  ...
  spouse_se_id = arad_sch_flow2se_id(spouse_flow_id);
  ...
  if (se->state == ARAD_SCH_SE_STATE_DISABLE)
  {
    ...
    res = arad_sch_flow_nof_subflows_get(
            unit,
            ARAD_SCH_SUB_FLOW_BASE_FLOW(se->id, TRUE),
            &fsf_nof_subflows
          );
    SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
...
{
  ARAD_SCH_SE_TYPE
    se_type_from_id = ARAD_SCH_SE_TYPE_NONE;

  SOC_SAND_INIT_ERROR_DEFINITIONS_NO_DEVID(ARAD_SCH_SE_ID_AND_TYPE_MATCH_VERIFY);

  se_type_from_id = arad_sch_se_get_type_by_id(se_id);

  if (se_type_from_id != se_type)
  {
    SOC_SAND_SET_ERROR_CODE(ARAD_SCH_SE_ID_AND_TYPE_MISMATCH_ERR, 10, exit)
  }
  ...
  )
{
  ...
  SOC_SAND_ERR_IF_ABOVE_MAX(
    cl_type_ndx, ARAD_SCH_NOF_CLASS_TYPES-1,
    ARAD_SCH_CL_ID_OUT_OF_RANGE_ERR, 10, exit
  );
  ...
  )
{
  ...
  SOC_SAND_ERR_IF_ABOVE_MAX(
    cl_type_ndx, ARAD_SCH_NOF_CLASS_TYPES - 1,
    ARAD_SCH_CL_ID_OUT_OF_RANGE_ERR, 2, exit
  );
  ...
  )
{
  ...
  ARAD_COPY(exact_class_type, class_type, ARAD_SCH_SE_CL_CLASS_INFO, 1);
  ...
  )
{
  ...
  SOC_SAND_ERR_IF_ABOVE_MAX(
    ...
    ARAD_SCH_NOF_CLASS_TYPES_OUT_OF_RANGE_ERR, 10, exit
  );
  ...
  for (idx = 0; idx < sct->nof_class_types; idx++)
  {
    ...
    SOC_SAND_ERR_IF_ABOVE_MAX(
      sct->class_types[idx].id, ARAD_SCH_NOF_CLASS_TYPES-1,
      ARAD_SCH_CL_ID_OUT_OF_RANGE_ERR, 20, exit
    );

    SOC_SAND_ERR_IF_ABOVE_MAX(
      sct->class_types[idx].mode, ARAD_SCH_CL_CLASS_MODE_LAST-1,
      ARAD_SCH_CL_CLASS_MODE_OUT_OF_RANGE_ERR, 30, exit
    );
    ...
    for (idx2 = 0; idx2 < ARAD_SCH_MAX_NOF_DISCRETE_WEIGHT_VALS; idx2++)
    {
      SOC_SAND_ERR_IF_ABOVE_MAX(
        sct->class_types[idx].weight[idx2], ARAD_SCH_DESCRETE_WEIGHT_MAX,
        ARAD_SCH_DESCRETE_WEIGHT_OUT_OF_RANGE_ERR, 40, exit
      );
    }

    SOC_SAND_ERR_IF_ABOVE_MAX(
      sct->class_types[idx].weight_mode, ARAD_SCH_CL_CLASS_WEIGHTS_MODE_LAST-1,
      ARAD_SCH_CL_CLASS_WEIGHTS_MODE_OUT_OF_RANGE_ERR, 50, exit
    );

    SOC_SAND_ERR_IF_ABOVE_MAX(
      sct->class_types[idx].enhanced_mode, ARAD_SCH_CL_ENHANCED_MODE_LAST-1,
      ARAD_SCH_CL_ENHANCED_MODE_OUT_OF_RANGE_ERR, 60, exit
    );
  ...
  )
{
  ...
  if ((se->state != ARAD_SCH_SE_STATE_ENABLE) &&
        (se->state != ARAD_SCH_SE_STATE_DISABLE))
  {
    SOC_SAND_SET_ERROR_CODE(ARAD_SCH_SE_STATE_OUT_OF_RANGE_ERR, 3, exit);
  }
  ...
  if((se->state == ARAD_SCH_SE_STATE_ENABLE))
  {
    ...
    {
    case ARAD_SCH_SE_TYPE_HR:
      if ((se->type_info.hr.mode != ARAD_SCH_HR_MODE_DUAL_WFQ) &&
          (se->type_info.hr.mode != ARAD_SCH_HR_MODE_SINGLE_WFQ) &&
          (se->type_info.hr.mode != ARAD_SCH_HR_MODE_ENHANCED_PRIO_WFQ)
         )
      {
        SOC_SAND_SET_ERROR_CODE(ARAD_SCH_HR_MODE_OUT_OF_RANGE_ERR, 30, exit);
      }
      break;
    case ARAD_SCH_SE_TYPE_CL:
      SOC_SAND_ERR_IF_ABOVE_MAX(
        se->type_info.cl.id, ARAD_SCH_NOF_CLASS_TYPES-1,
        ARAD_SCH_CL_ID_OUT_OF_RANGE_ERR, 40, exit
      );
      break;
    case ARAD_SCH_SE_TYPE_FQ:
        /* Nothing to verify */
        break;
    default:
      SOC_SAND_SET_ERROR_CODE(ARAD_SCH_SE_TYPE_UNDEFINED_ERR, 50, exit);
      break;
    }
  }

  SOC_SAND_ERR_IF_ABOVE_MAX(
    se->group, ARAD_SCH_GROUP_LAST-1,
    ARAD_SCH_GROUP_OUT_OF_RANGE_ERR, 50, exit
  );
