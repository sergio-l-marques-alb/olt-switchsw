  ...
  )
{
  ...
  *internal_weight |= SOC_SAND_SET_FLD_IN_PLACE(exp,
  ...
  {
    *internal_weight = ARAD_SCH_SUB_FLOW_COS_HR_EF1-1;
  }
  ...
  )
{
  ...
  */
  ...
  else if(config_mode == ARAD_SCH_FLOW_IPF_CONFIG_MODE_INVERSE)
  {
    ...
    ARAD_TBL_REF(unit, SCH_FLOW_DESCRIPTOR_MEMORY_STATIC__FDMSm, COSf, &cos_fld);
  ...
  )
{
  ...
  ARAD_TBL_REF(unit, SCH_SHAPER_DESCRIPTOR_MEMORY_STATIC__SHDSm, PEAK_RATE_MAN_EVENf, &peak_rate_man_fld);
  ARAD_TBL_REF(unit, SCH_SHAPER_DESCRIPTOR_MEMORY_STATIC__SHDSm, PEAK_RATE_EXP_EVENf, &peak_rate_exp_fld);
  ARAD_TBL_REF(unit, SCH_SHAPER_DESCRIPTOR_MEMORY_STATIC__SHDSm, MAX_BURST_EVENf, &max_burst_fld);
