  ...
  )
{
  ...
  if (SOC_IS_JERICHO(unit)) {
      ...
      for (bank_id = 0; bank_id < SOC_DPP_DEFS_GET_TCAM_NOF_BANKS(unit); ++bank_id)
      {
          ...
          if ((bank_id != 12) && (bank_id != 13)) {
              res = arad_sw_db_tcam_bank_valid_set(unit, bank_id, TRUE /* valid */);
              SOC_SAND_CHECK_FUNC_RESULT(res, 160, exit);
              res = arad_sw_db_tcam_bank_nof_entries_free_set(unit, bank_id, 0 /* nof-free-entries */);
