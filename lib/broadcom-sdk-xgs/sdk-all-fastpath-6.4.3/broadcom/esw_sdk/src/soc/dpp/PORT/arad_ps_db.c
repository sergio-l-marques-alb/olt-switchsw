...
#define ARAD_PS_DB_IS_PS_ERP_TYPE(ps_id) (ARAD_ERP_PORT_ID == ARAD_PS2BASE_PORT_TC(ps_id))
...
#define ARAD_PS_MAX_NIF_INTERFACE 29 /* Including CPU */
...
#define ARAD_PS_NON_CH_INTERFACE  ARAD_PS_MAX_NIF_INTERFACE + 5 /* Used for Jericho - non channalizedinterfaces can use the same PS */
#define ARAD_PS_DB_NOF_INTERFACES ARAD_PS_MAX_NIF_INTERFACE + 6
...
STATIC void
arad_ps_db_try_allocate(arad_ps_db_data_t* ps, int out_port_priority, int is_binding, int* allocated, int* internal_id) {
...
STATIC void
arad_ps_db_try_allocate_with_id(arad_ps_db_data_t* ps, int out_port_priority, int is_binding, int internal_id, int* allocated) {
    ...
    switch(interface_type) {
        ...
        default:
            ...
            if(SOC_IS_ARADPLUS_AND_BELOW(unit)) {
                ...
                rv = MBCM_DPP_SOC_DRIVER_CALL(unit, mbcm_dpp_port2egress_offset, (unit, core, tm_port, interface /*egr_if*/));
...
arad_ps_db_alloc_binding_ps_with_id(int unit, soc_port_t port, int out_port_priority, int base_q_pair) {
    ...
    if(SHR_BITGET(core_resources[unit][core].allocated_ps[interface].bitmap, ps_idx)) {
        arad_ps_db_try_allocate_with_id(&core_resources[unit][core].ps[ps_idx], out_port_priority, 1, internal_id, &allocated);
        if(!allocated) {
...
int 
arad_ps_db_find_free_binding_ps(int unit, soc_port_t port, int out_port_priority, int* base_q_pair) {
    ...
    *base_q_pair = -1;
    ...
    for(i=0 ; i<ARAD_PS_DB_NOF_PS_RESOURCES ; i++) {
        if(SHR_BITGET(core_resources[unit][core].allocated_ps[interface].bitmap, i)) {
            arad_ps_db_try_allocate(&core_resources[unit][core].ps[i], out_port_priority, 1, &allocated, &internal_id);
            if(allocated) {
...
int 
arad_ps_db_find_free_non_binding_ps(int unit, int core, int* base_q_pair) {
    ...
    *base_q_pair = -1;
    ...
    for(i=0 ; i<ARAD_PS_DB_NOF_PS_RESOURCES ; i++) {
        if(SHR_BITGET(core_resources[unit][core].free_ps[psResourceOlp].bitmap, i) ||
           ...
           arad_ps_db_try_allocate(&core_resources[unit][core].ps[i], 1, 0, &allocated, &internal_id);
            if(allocated) {
                *base_q_pair = i*ARAD_EGR_NOF_Q_PAIRS_IN_PS + internal_id;
                break;
            }
