...
soc_jer_ofp_rates_init(SOC_SAND_IN int unit)
{
    ...
    SOC_DPP_CORES_ITER(SOC_CORE_ALL, core) {
        ...
        /* By default Enable OTM, QPair, TCG shapers */
        fld_val = 0x1;
        ...
        SOCDNX_SAND_IF_ERR_EXIT(arad_sw_db_egr_ports_egq_tcg_qpair_shaper_enable_set(unit,TRUE));
    ...
    )
{
    ...
    if (is_channelized) {
        ...
        if (is_single_cal_mode) {
            ...
            SOC_PBMP_ITER(ports_bm, port_i) {
                ...
                SOCDNX_SAND_IF_ERR_EXIT(arad_sw_db_egq_port_rate_get(unit, base_q_pair_i, core_i, &rate_i));
            ...
            SOCDNX_IF_ERR_EXIT(soc_jer_ofp_rates_calendar_allocate(unit, tm_port, core, num_required_slots, old_cal, &new_cal));
            /* mark calendar as modified */
            SOCDNX_SAND_IF_ERR_EXIT(soc_sand_occ_bm_occup_status_set(modified_cals_occ, new_cal, TRUE));
        ...
        /* mark calendar as modified */
        SOCDNX_SAND_IF_ERR_EXIT(soc_sand_occ_bm_occup_status_set(modified_cals_occ, SOC_DPP_DEFS_GET(unit, non_channelized_cal_id), TRUE));
    }
    ...
    /* Setting rate to sw */
    SOCDNX_SAND_IF_ERR_EXIT(arad_sw_db_egq_port_rate_set(unit, base_q_pair, core, rate));

    /* Mark port as valid */
    SOCDNX_SAND_IF_ERR_EXIT(arad_sw_db_is_port_valid_set(unit, base_q_pair, core, TRUE));
    ...
    SOC_SAND_OUT uint32          *new_cal)
{
    ...
    *new_cal = INVALID_CALENDAR;
    ...
    if (old_cal == INVALID_CALENDAR) { /* allocate new calendar */
        if (num_required_slots != 0) {
            found = FALSE;
            if (num_required_slots <= small_cal_size) { /* allocate samll calendar */
                SOCDNX_SAND_IF_ERR_EXIT(soc_sand_occ_bm_get_next_in_range(cals_occ, num_of_big_calendars, (num_of_calendars-2), TRUE, new_cal, &found));  
            } 

            if (!found) { /* try to allocate big calendar since all small calendars are occupied */
                SOCDNX_SAND_IF_ERR_EXIT(soc_sand_occ_bm_get_next_in_range(cals_occ, 0, (num_of_big_calendars -1), TRUE, new_cal, &found));
                if (!found) {
            ...
            SOCDNX_SAND_IF_ERR_EXIT(soc_sand_occ_bm_occup_status_set(cals_occ, *new_cal, TRUE));
        }
        ...
        if (num_required_slots == 0) {
            ...
            found = FALSE;
            if (num_required_slots <= small_cal_size) { /* suits for small channelized cal */
            ...
            } else { /* num_required_slots > small_channelized_cal_size */
    ...
    SOC_SAND_IN  uint32          cal_id)
{
    ...
    SOCDNX_SAND_IF_ERR_EXIT(soc_sand_occ_bm_occup_status_set(cals_occ, cal_id, FALSE));
