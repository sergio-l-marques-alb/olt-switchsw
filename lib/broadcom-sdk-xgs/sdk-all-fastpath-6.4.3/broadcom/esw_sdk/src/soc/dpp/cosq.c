  ...
  )
{
  ...
  if (flow_and_up_info->credit_sources_nof == 0)
  {
      ...
      if (is_flow) {
         ...
         SOCDNX_IF_ERR_EXIT(soc_dpp_is_fap_id_local_and_get_core_id(unit, mapped_fap_id, &is_fap_id_local, &core));
         ...
         ret = MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_ipq_destination_id_packets_base_queue_id_get,(unit, core, dest_id, &valid, &is_sw_only, &(flow_and_up_info->base_queue)));
         ...
         ret = MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_ipq_queue_to_flow_mapping_get,(unit, SOC_CORE_ALL, queue_quartet_ndx, &(flow_and_up_info->qrtt_map_info)));
  ...
  )
{
  ...
  /*
   * For now, set core to '0'. It may need to be modified when
   * diagnostics are handled for Jericho.
   */
  core = 0 ;
