...
soc_error_t soc_jer_pp_oam_init(int unit)
{
    ...
    */
    ...
    for (outlif_profile=0 ; outlif_profile < 0x40  /*all outlif profiles*/ ;++outlif_profile) {
        ...
        rv = MBCM_PP_DRIVER_CALL(unit, mbcm_pp_profile_mgmt_if_profile_get_val_from_map, 
                                 (unit,SOC_PPC_PROFILE_MGMT_TYPE_OUTLIF, SOC_PPC_PROFILE_MGMT_OUTLIF_PROFILE_TYPE_OAM_PCP , outlif_profile,&oam_outlif_profile));
        SOCDNX_IF_ERR_EXIT(rv); 
