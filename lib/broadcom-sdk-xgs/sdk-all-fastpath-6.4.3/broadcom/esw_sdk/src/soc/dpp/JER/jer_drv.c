...
soc_jer_general_phy_pbmp_get(int unit, soc_port_t port, uint32 id, uint32 default_nof_lanes, soc_jer_ucode_port_config_t* config)
{
    ...
    SOCDNX_IF_ERR_EXIT(soc_jer_qsgmii_offsets_add(unit, &pbmp, &config->phy_pbmp));
...
soc_jer_ilkn_phy_pbmp_get(int unit, soc_port_t port, uint32 id, uint32 default_nof_lanes, soc_jer_ucode_port_config_t* config)
{
    ...
    SOCDNX_IF_ERR_EXIT(soc_jer_qsgmii_offsets_add(unit, &phy_pbmp, &config->phy_pbmp));
...
int soc_jer_specific_info_config_derived(int unit) 
{
    ...
    SOC_DPP_CONFIG(unit)->jer->pll.ref_clk_pml_in[0] = soc_property_suffix_num_get(unit, 0, spn_SERDES_NIF_CLK_FREQ, "in", soc_dcmn_init_serdes_ref_clock_125);; 
    SOC_DPP_CONFIG(unit)->jer->pll.ref_clk_pml_out[0] = soc_property_suffix_num_get(unit, 0, spn_SERDES_NIF_CLK_FREQ, "out", soc_dcmn_init_serdes_ref_clock_125);; 
    SOC_DPP_CONFIG(unit)->jer->pll.ref_clk_pml_in[1] = soc_property_suffix_num_get(unit, 1, spn_SERDES_NIF_CLK_FREQ, "in", soc_dcmn_init_serdes_ref_clock_125);; 
    SOC_DPP_CONFIG(unit)->jer->pll.ref_clk_pml_out[1] = soc_property_suffix_num_get(unit, 1, spn_SERDES_NIF_CLK_FREQ, "out", soc_dcmn_init_serdes_ref_clock_125);; 
    SOC_DPP_CONFIG(unit)->jer->pll.ref_clk_pmh_in = soc_property_suffix_num_get(unit, 2, spn_SERDES_NIF_CLK_FREQ, "in", soc_dcmn_init_serdes_ref_clock_125);; 
    SOC_DPP_CONFIG(unit)->jer->pll.ref_clk_pmh_out = soc_property_suffix_num_get(unit, 2, spn_SERDES_NIF_CLK_FREQ, "out", soc_dcmn_init_serdes_ref_clock_125);; 
    SOC_DPP_CONFIG(unit)->jer->pll.ref_clk_fabric_in[0] = soc_property_suffix_num_get(unit, 0, spn_SERDES_FABRIC_CLK_FREQ, "in", soc_dcmn_init_serdes_ref_clock_125);; 
    SOC_DPP_CONFIG(unit)->jer->pll.ref_clk_fabric_out[0] = soc_property_suffix_num_get(unit, 0, spn_SERDES_FABRIC_CLK_FREQ, "out", soc_dcmn_init_serdes_ref_clock_125);; 
    SOC_DPP_CONFIG(unit)->jer->pll.ref_clk_fabric_in[1] = soc_property_suffix_num_get(unit, 1, spn_SERDES_FABRIC_CLK_FREQ, "in", soc_dcmn_init_serdes_ref_clock_125);; 
    SOC_DPP_CONFIG(unit)->jer->pll.ref_clk_fabric_out[1] = soc_property_suffix_num_get(unit, 1, spn_SERDES_FABRIC_CLK_FREQ, "out", soc_dcmn_init_serdes_ref_clock_125);; 
    SOC_DPP_CONFIG(unit)->jer->pll.ref_clk_synce_out[0] = soc_property_suffix_num_get(unit, 0, spn_SYNC_ETH_CLK_DIVIDER, "clk", soc_dcmn_init_serdes_ref_clock_125);; 
    SOC_DPP_CONFIG(unit)->jer->pll.ref_clk_synce_out[1] = soc_property_suffix_num_get(unit, 1, spn_SYNC_ETH_CLK_DIVIDER, "clk", soc_dcmn_init_serdes_ref_clock_125);; 
            ...
            ARAD_PORT_SW_DB_VALID_PORTS_ITER(unit,port_i) {/*!!! Iteration !!!*/
                ...
                if (port_i != SOC_DPP_PORT_INTERNAL_ERP(core)) {
                    ...
                    if (header_type_in == SOC_TMC_PORT_HEADER_TYPE_TDM) {
                        SOCDNX_SAND_IF_ERR_EXIT(arad_tdm_ftmh_set(unit, core, tm_port, &ftmh_info));
                    }
                }
            }
        }
