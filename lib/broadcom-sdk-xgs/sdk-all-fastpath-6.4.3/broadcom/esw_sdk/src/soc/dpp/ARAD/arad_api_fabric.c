  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_LINK_ON_OFF_SET);
  SOC_SAND_CHECK_DRIVER_AND_DEVICE;

  SOC_SAND_CHECK_NULL_INPUT(info);

  res = arad_link_on_off_verify(
    unit,
    link_ndx,
    info
  );
  ...
  res = arad_link_on_off_set_unsafe(
    unit,
    link_ndx,
    info
  );
...
exit_semaphore:
  SOC_SAND_GIVE_DEVICE_SEMAPHORE;
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_link_on_off_set()",0,0);
}
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_LINK_ON_OFF_GET);
  SOC_SAND_CHECK_DRIVER_AND_DEVICE;
  ...
  res = arad_link_on_off_get_unsafe(
    unit,
    link_ndx,
    info
  );
...
exit_semaphore:
  SOC_SAND_GIVE_DEVICE_SEMAPHORE;
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_link_on_off_get()",0,0);
}
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_FABRIC_LINK_UP_GET);
  SOC_SAND_CHECK_DRIVER_AND_DEVICE;
  ...
  res = arad_fabric_link_up_get_verify(
    unit,
    link_ndx,
    up
  );
  ...
  res = arad_fabric_link_up_get_unsafe(
    unit,
    link_ndx,
    up
  );
...
exit_semaphore:
  SOC_SAND_GIVE_DEVICE_SEMAPHORE;
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_fabric_link_up_get()",0,0);
}
