...
STATIC uint32
  soc_jer_fabric_cell_formats_init(
    SOC_SAND_IN int             unit
  )
{
    ...
    if (field_val) {
        SOCDNX_IF_ERR_EXIT(READ_ECI_GLOBAL_GENERAL_CFG_1r(unit, &reg_val));
        soc_reg_field_set(unit, ECI_GLOBAL_GENERAL_CFG_1r, &reg_val, RESERVED_QTSf, field_val);
        SOCDNX_IF_ERR_EXIT(WRITE_ECI_GLOBAL_GENERAL_CFG_1r(unit, reg_val));
    }
exit:
    SOCDNX_FUNC_RETURN;
}
  ...
  )
{
    ...
    SOCDNX_IF_ERR_EXIT(READ_FDT_FDT_ENABLER_REGISTERr(unit, &reg_val));
    soc_reg_field_set(unit, FDT_FDT_ENABLER_REGISTERr, &reg_val, PARALLEL_DATA_PATHf, nof_pipes);
    SOCDNX_IF_ERR_EXIT(WRITE_FDT_FDT_ENABLER_REGISTERr(unit, reg_val));

    SOCDNX_IF_ERR_EXIT(READ_FDR_FDR_ENABLERS_REGISTER_2r(unit, &reg_val));
    soc_reg_field_set(unit, FDR_FDR_ENABLERS_REGISTER_2r, &reg_val, PARALLEL_DATA_PATHf, nof_pipes);
    SOCDNX_IF_ERR_EXIT(WRITE_FDR_FDR_ENABLERS_REGISTER_2r(unit, reg_val));

    SOCDNX_IF_ERR_EXIT(READ_FDA_FDA_ENABLERSr(unit, &reg_val));
    soc_reg_field_set(unit, FDA_FDA_ENABLERSr, &reg_val, PARALLEL_DATA_PATHf, nof_pipes);
    SOCDNX_IF_ERR_EXIT(WRITE_FDA_FDA_ENABLERSr(unit, reg_val));

    for ( i = 0; i < nof_instances_fmac; ++i) {
        for ( j = 0; j < nof_links_fmac; ++j) {
            SOCDNX_IF_ERR_EXIT(READ_FMAC_FMAL_GENERAL_CONFIGURATIONr(unit, i, j, &reg_val)); 
            soc_reg_field_set(unit, FMAC_FMAL_GENERAL_CONFIGURATIONr, &reg_val, FMAL_N_PARALLEL_DATA_PATHf, nof_pipes);
  ...
  )
{
    ...
    SOCDNX_IF_ERR_EXIT(soc_jer_fabric_cell_formats_init(unit));
    ...
    /* Configure repeater remote links */ 
    SOCDNX_IF_ERR_EXIT(soc_jer_fabric_repeater_config_init(unit)); /*empty*/

    /* Initiate all WB Engine buffers */
    SOCDNX_IF_ERR_EXIT(soc_dpp_wb_engine_init_buffer(unit, SOC_DPP_WB_ENGINE_BUFFER_JER_MODID_GROUP_MAP_TO_DEST)); 
...
{
    int i;
    SOC_DPP_WB_ENGINE_VAR count_var, array_var;
    SOCDNX_INIT_FUNC_DEFS;

    switch (local_dest) {
    case 0:
        count_var = SOC_DPP_WB_ENGINE_VAR_JER_GROUP_MAP_DEST_0_COUNT;
        array_var = SOC_DPP_WB_ENGINE_VAR_JER_MODID_GROUP_MAP_DEST_0;
        break;
    case 1:
        count_var = SOC_DPP_WB_ENGINE_VAR_JER_GROUP_MAP_DEST_1_COUNT;
        array_var = SOC_DPP_WB_ENGINE_VAR_JER_MODID_GROUP_MAP_DEST_1;
        break;
    case 2:
        count_var = SOC_DPP_WB_ENGINE_VAR_JER_GROUP_MAP_DEST_2_COUNT;
        array_var = SOC_DPP_WB_ENGINE_VAR_JER_MODID_GROUP_MAP_DEST_2;
        break;
    default:
        SOCDNX_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_SOC_MSG("destination id is invalid")));
        break;
    }
    SOCDNX_IF_ERR_EXIT(SOC_DPP_WB_ENGINE_SET_VAR(unit, count_var, &modid_count));

    for (i = 0; i < modid_count; ++i) {
        SOCDNX_IF_ERR_EXIT(SOC_DPP_WB_ENGINE_SET_ARR(unit, array_var, &modid_array[i], i));
    }
  ...
  )
{
    ...
    SOCDNX_IF_ERR_EXIT(SOC_DPP_WB_ENGINE_GET_VAR(unit, count_var, &id_count));
    ...
    *modid_count = id_count;
    for (i = 0; i < id_count; ++i) {
        SOCDNX_IF_ERR_EXIT(SOC_DPP_WB_ENGINE_GET_ARR(unit, array_var, &modid_array[i], i));
    }
exit:
  SOCDNX_FUNC_RETURN;
  ...
  )
{
    ...
    bit_pos = (is_master) ? SOC_JER_FABRIC_SYNCE_CFG_MASTER_SEL : SOC_JER_FABRIC_SYNCE_CFG_SLAVE_SEL;
    /*need to set pll config mux to select nif synce only if port is nif- otherwise dont touch*/
    if (SOC_IS_JERICHO(unit) && port_id < SOC_INFO(unit).fabric_logical_port_base) {
        SOCDNX_IF_ERR_EXIT(READ_ECI_MISC_PLL_7_CONFIGr(unit, reg_above_64_val));
        SHR_BITSET(reg_above_64_val, bit_pos);
        SOCDNX_IF_ERR_EXIT(READ_ECI_MISC_PLL_7_CONFIGr(unit, reg_above_64_val));
    }
  ...
  )
{
    ...
    SOCDNX_IF_ERR_EXIT(READ_ECI_MISC_PLL_7_CONFIGr(unit, reg_above_64_val));
    *is_fabric_synce = SHR_BITGET(reg_above_64_val, bit_pos);
