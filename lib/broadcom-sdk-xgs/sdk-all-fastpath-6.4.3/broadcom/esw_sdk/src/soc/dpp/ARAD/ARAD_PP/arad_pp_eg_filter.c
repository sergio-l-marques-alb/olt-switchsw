  ...
  )
{
  ...
  if (SOC_IS_JERICHO(unit)) {
      ...
      if (soc_property_get(unit, spn_SPLIT_HORIZON_FORWARDING_GROUPS_MODE, 0)) {
          reg_val = 0x8421; /* default filter traffic between same group ids */
  ...
  if (SOC_IS_ARADPLUS(unit)) {
    ...
    if (soc_property_get(unit, spn_BCM886XX_LOGICAL_INTERFACE_BRIDGE_FILTER_ENABLE, 0)) {
      ...
      res = MBCM_PP_DRIVER_CALL(unit,mbcm_pp_profile_mgmt_if_profile_val_set,(unit, SOC_PPC_PROFILE_MGMT_TYPE_INLIF, SOC_PPC_PROFILE_MGMT_INLIF_PROFILE_TYPE_SIMPLE_SAME_INTERFACE, 3/*0b11*/, &inlif_profile_same_if_mask));
  ...
  )
{
  ...
  SOC_SAND_ERR_IF_ABOVE_NOF(lif_eep_ndx, SOC_DPP_DEFS_GET(unit, nof_local_lifs), ARAD_PP_LIF_ID_OUT_OF_RANGE_ERR, 10, exit);
  ...
  )
{
  ...
  SOC_SAND_ERR_IF_ABOVE_MAX(info->in_lif_profile_disable_same_interface_filter_bitmap, SOC_PPC_PROFILE_MGMT_EG_FILTER_INLIF_PROFILE_BITMAP_MAX, ARAD_PP_EG_FILTER_ACCEPTABLE_FRAMES_PROFILE_OUT_OF_RANGE_ERR, 11, exit);
