...
)
{
    ...
    *group_size = 0;
    ...
    DPP_MC_FOREACH_CORE(cores_to_get, cores_bm, core) {
        ...
        SOCDNX_IF_ERR_EXIT(mcds->get_replications_from_entry(unit, core, cur_index,
        ...
        while (cur_index != end_index) {
            DPP_MC_ASSERT(cur_index > 0 && cur_index < MCDS_INGRESS_LINK_END(mcds));
            SOCDNX_IF_ERR_EXIT(mcds->get_replications_from_entry(unit, core, cur_index, entry_type, &entries_remaining, group_size, &cur_index));
