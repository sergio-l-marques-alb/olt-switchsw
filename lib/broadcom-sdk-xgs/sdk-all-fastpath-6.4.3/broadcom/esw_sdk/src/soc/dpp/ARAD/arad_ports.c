...
/* PCS register vals */
#define ARAD_PORT_PCS_8_9_LEGACY_FEC_VAL (0)
#define ARAD_PORT_PCS_8_10_VAL           (1) 
#define ARAD_PORT_PCS_64_66_FEC_VAL      (2)          
#define ARAD_PORT_PCS_64_66_BEC_VAL      (3)  
  ...
  )
{
  ...
  {
    if (port_i == SOC_INFO(unit).erp_port[0])
    {
        erp_exist = TRUE;
        break;
    }
  ...
  if (erp_exist) {
      ...
      res = arad_sw_db_egr_ports_erp_interface_set(unit, free_nif_interface);
  ...
  )
{
  ...
  SOC_SAND_ERR_IF_ABOVE_MAX(
    sys_phys_port_ndx, ARAD_NOF_SYS_PHYS_PORTS-1,
    ARAD_SYSTEM_PHYSICAL_PORT_OUT_OF_RANGE_ERR, 10, exit
  );
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_PORT_CONTROL_PCS_SET_UNSAFE);
  ...
  switch(pcs)
  {
    case ARAD_PORT_PCS_8_9_LEGACY_FEC:
      encoding_type = ARAD_PORT_PCS_8_9_LEGACY_FEC_VAL;
    ...
    case ARAD_PORT_PCS_8_10:
      encoding_type = ARAD_PORT_PCS_8_10_VAL;
      COMPILER_64_SET(slow_read_rate, 0, SOC_SAND_NUM2BOOL(TRUE));
      syndrom = SOC_SAND_NUM2BOOL(FALSE);
      tx_ack_mask_period = 8;
      set_high_latency=1;
    ...
    default:
      SOC_SAND_SET_ERROR_CODE(ARAD_PORT_CONTROL_PCS_OUT_OF_RANGE_ERR, 5, exit);
  }

  SOC_SAND_SOC_IF_ERROR_RETURN(res, 10, exit, READ_FMAC_FMAL_GENERAL_CONFIGURATIONr(unit, blk_id, inner_link, &general_config));
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 20, exit, READ_FMAC_ASYNC_FIFO_CONFIGURATIONr(unit, blk_id, inner_link, &async_fifo_config));
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 30, exit, READ_FMAC_FPS_RX_FEC_CONFIGURATIONr(unit, blk_id, inner_link, &rx_fec_config));
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 50, exit, READ_FMAC_FPS_TX_CONFIGURATIONr(unit, blk_id, inner_link, &tx_fec_config));
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 40, exit, READ_FMAC_BEC_CONFIGURATIONr(unit, blk_id, inner_link, &bec_config));
  ...
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 60, exit, WRITE_FMAC_FMAL_GENERAL_CONFIGURATIONr(unit, blk_id, inner_link, general_config));
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 70, exit, WRITE_FMAC_ASYNC_FIFO_CONFIGURATIONr(unit, blk_id, inner_link, async_fifo_config));
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 80, exit, WRITE_FMAC_FPS_RX_FEC_CONFIGURATIONr(unit, blk_id, inner_link, rx_fec_config));
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 90, exit, WRITE_FMAC_FPS_TX_CONFIGURATIONr(unit, blk_id, inner_link, tx_fec_config));
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 100, exit, WRITE_FMAC_BEC_CONFIGURATIONr(unit, blk_id, inner_link, bec_config));
  ...
  if ((link_ndx%SOC_DPP_DEFS_GET(unit, nof_fabric_links_in_mac)) == 0) {
      res = arad_fabric_force_signal_detect_set(unit, blk_id);
  ...
  if(SOC_DPP_IS_MESH((unit)))
  {
    ...
    SOC_SAND_CHECK_FUNC_RESULT(
       arad_fabric_port_speed_get(
          unit,
          SOC_DPP_FABRIC_LINK_TO_PORT((unit),link_ndx)
          ,&speed),
        110, exit);
    SOC_SAND_CHECK_FUNC_RESULT(arad_fabric_mesh_topology_values_config(unit, speed, pcs), 120, exit);
  }
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_ports_control_pcs_set_unsafe()",0,0);
}
     ...
     )
{
  ...
  res=arad_port_control_pcs_get_unsafe(unit,link_ndx,&pcs);
  SOC_SAND_CHECK_FUNC_RESULT(res, 1, exit);

  switch (pcs)
  {
  case ARAD_PORT_PCS_8_9_LEGACY_FEC:
      SOC_SAND_SET_ERROR_CODE(SOC_TMC_CONFIG_ERR, 4, exit);
      break;
  case ARAD_PORT_PCS_8_10:
      SOC_SAND_SET_ERROR_CODE(SOC_TMC_CONFIG_ERR, 5, exit);
      break;
  case ARAD_PORT_PCS_64_66_FEC:
      valid=1;
      break;
  case ARAD_PORT_PCS_64_66_BEC:
      ...
      break;
  case ARAD_PORT_PCS_64_66:
      SOC_SAND_SET_ERROR_CODE(SOC_TMC_CONFIG_ERR ,6, exit);
      break;
  default:
      SOC_SAND_SET_ERROR_CODE(SOC_TMC_CONFIG_ERR ,10, exit);
      break;
  }

  if (valid)
  {
     SOC_SAND_SOC_IF_ERROR_RETURN(res, 2, exit, READ_FMAC_FMAL_GENERAL_CONFIGURATIONr(unit, blk_id, inner_link, &reg_val)); 
     soc_reg_field_set(unit, FMAC_FMAL_GENERAL_CONFIGURATIONr, &reg_val, FMAL_N_LOW_LATENCY_LLFCf, value? 1:0);
     SOC_SAND_SOC_IF_ERROR_RETURN(res, 3, exit, WRITE_FMAC_FMAL_GENERAL_CONFIGURATIONr(unit, blk_id, inner_link, reg_val));

  }
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_port_control_low_latency_set()",0,0);
     ...
     )
{
 uint32 blk_id,
  ...
  {
  case ARAD_PORT_PCS_8_9_LEGACY_FEC:
      SOC_SAND_SET_ERROR_CODE(SOC_TMC_CONFIG_ERR, 2, exit);
      break;
  case ARAD_PORT_PCS_8_10:
      SOC_SAND_SET_ERROR_CODE(SOC_TMC_CONFIG_ERR, 3, exit);
      break;
  case ARAD_PORT_PCS_64_66_FEC:
      valid=1;
      break;
  ...
  if (valid)
  {
     SOC_SAND_SOC_IF_ERROR_RETURN(res, 2, exit, READ_FMAC_FPS_RX_FEC_CONFIGURATIONr(unit, blk_id, inner_link, &reg_val)); 
     soc_reg_field_set(unit, FMAC_FPS_RX_FEC_CONFIGURATIONr, &reg_val, FPS_N_RX_FEC_ERR_MARK_ALLf, value? 1:0);
     SOC_SAND_SOC_IF_ERROR_RETURN(res, 3, exit, WRITE_FMAC_FPS_RX_FEC_CONFIGURATIONr(unit, blk_id, inner_link, reg_val));
  }
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_port_control_fec_error_detect_set()",0,0);
}
     ...
     )
{
  ...
  if (! (pcs == ARAD_PORT_PCS_64_66_FEC || pcs == ARAD_PORT_PCS_64_66_BEC) )
  {
      *value=0;
      SOC_SAND_SET_ERROR_CODE(SOC_TMC_CONFIG_ERR ,10, exit);

  }
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 1, exit, READ_FMAC_FMAL_GENERAL_CONFIGURATIONr (unit, blk_id, inner_link, &reg_val));
  *value=soc_reg_field_get(unit,FMAC_FMAL_GENERAL_CONFIGURATIONr,reg_val,FMAL_N_LOW_LATENCY_LLFCf);

exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_port_control_low_latency_get()",0,0);
     ...
     )
{
     ...
     if (! (pcs == ARAD_PORT_PCS_64_66_FEC) )
     {
         *value=0;
         SOC_SAND_SET_ERROR_CODE(SOC_TMC_CONFIG_ERR ,10, exit);
     ...
     SOC_SAND_SOC_IF_ERROR_RETURN(res, 1, exit, READ_FMAC_FPS_RX_FEC_CONFIGURATIONr (unit, blk_id, inner_link, &reg_val));
     *value=soc_reg_field_get(unit,FMAC_FPS_RX_FEC_CONFIGURATIONr,reg_val,FPS_N_RX_FEC_ERR_MARK_ALLf);

exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_port_control_fec_error_detect_get()",0,0);
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_PORT_CONTROL_PCS_GET_UNSAFE);

  blk_id = SOC_SAND_DIV_ROUND_DOWN(link_ndx, 4);
  inner_link = link_ndx % 4;

  SOC_SAND_SOC_IF_ERROR_RETURN(res, 1510, exit, READ_FMAC_FMAL_GENERAL_CONFIGURATIONr(unit, blk_id, inner_link, &general_config));
  encoding_type = soc_reg_field_get(unit, FMAC_FMAL_GENERAL_CONFIGURATIONr, general_config, FMAL_N_ENCODING_TYPEf);

  SOC_SAND_SOC_IF_ERROR_RETURN(res, 1520, exit, READ_FMAC_FPS_RX_FEC_CONFIGURATIONr(unit, blk_id, inner_link,&fmac_config));
  rx_fec_en = soc_reg_field_get(unit, FMAC_FPS_RX_FEC_CONFIGURATIONr,fmac_config,FPS_N_RX_FEC_FEC_ENf  );

  
  switch(encoding_type)
  {
    case ARAD_PORT_PCS_8_9_LEGACY_FEC_VAL: 
      *pcs = ARAD_PORT_PCS_8_9_LEGACY_FEC;
      break;
    case ARAD_PORT_PCS_8_10_VAL: 
      *pcs = ARAD_PORT_PCS_8_10;
      break;
    case ARAD_PORT_PCS_64_66_FEC_VAL:
      if (rx_fec_en) *pcs = ARAD_PORT_PCS_64_66_FEC;
      else           *pcs = ARAD_PORT_PCS_64_66;
      break;
    case ARAD_PORT_PCS_64_66_BEC_VAL: 
      *pcs = ARAD_PORT_PCS_64_66_BEC;
      break;
  default:
    SOC_SAND_SET_ERROR_CODE(ARAD_PORT_CONTROL_PCS_GET_ENCODING_TYPE_OUT_OF_RANGE_ERR, 20, exit);       
  }
  
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_port_control_pcs_get_unsafe()",0,0);
}
...
{
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_PORT_CONTROL_PCS_GET_VERIFY);

  SOC_SAND_ERR_IF_ABOVE_MAX(
    link_ndx, SOC_DPP_DEFS_GET(unit, nof_fabric_links)-1,
    ARAD_PORT_CONTROL_PCS_LINK_OUT_OF_RANGE_ERR, 5, exit
  );

  SOC_SAND_CHECK_NULL_INPUT(pcs);

exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_port_control_pcs_get_verify()",0,0);
}
  ...
  )
{
  ...
  if (ARAD_NIF_IS_ARAD_ID(map_info->if_id))
  {
    ...
    */
    ...
    /* Set the NIF on for not SerDes */
    rv = soc_port_sw_db_is_initialized_get(unit, port_ndx, &is_active);
    if(SOC_FAILURE(rv)) {
        SOC_SAND_SET_ERROR_CODE(ARAD_NIF_INVALID_TYPE_ERR, 61, exit);
    }
    if(is_active) {
        rv = soc_pm_enable_set(unit, port_ndx, is_mapped);
        if(SOC_FAILURE(rv)) {
            SOC_SAND_SET_ERROR_CODE(ARAD_NIF_INVALID_TYPE_ERR, 62, exit);
        }
    }
  }
  ...
  )
{
  ...
  if (ARAD_NIF_IS_ARAD_ID(map_info->if_id))
  {
    ...
    if (nif_type == ARAD_NIF_TYPE_ILKN) {
       ...
       SOC_SAND_SOC_IF_ERROR_RETURN(res, 20, exit, soc_port_sw_db_protocol_offset_get(unit, port_ndx, &offset));
  ...
  else if (ARAD_IS_ERP_IF_ID(map_info->if_id))
  {
    /* ERP */
    txi_sel_val = ARAD_EGQ_TXI_TH_ID_RCY;
    ifc = arad_sw_db_egr_ports_erp_interface_get(unit);
    ...
    *if_id = ARAD_IF_ID_NONE;
    ...
    if (!found)
    {
        ...
        if (base_q_pair == ARAD_ERP_PORT_ID) {
            ...
            erp_interface_id = arad_sw_db_egr_ports_erp_interface_get(unit);
        ...
        switch(header_type) {
...
#ifdef BCM_88660_A0
                    if (SOC_IS_ARADPLUS(unit) && soc_dpp_pp_port_is_pon_port(unit, pp_port)) {
                        prge_prog_select = ARAD_PRGE_TM_SELECT_ETH_WITH_LLID_TAG;
                    } else
#endif
                    {
            ...
            case ARAD_PORT_HEADER_TYPE_RAW:
                ...
                if(soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "user_header_always_remove", 0) == 1) {
                    ...
                    prge_prog_select = ARAD_PRGE_TM_SELECT_REMOVE_SYSTEM_HEADER; /* TM profile */
                }
                break;
            ...
            case ARAD_PORT_HEADER_TYPE_TM:
                ...
                if(soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "user_header_always_remove", 0) == 1) {
                    ...
                    prge_prog_select = ARAD_PRGE_TM_SELECT_OTMH_BASE; /* TM profile: OTMH_BASE && <= OTMH_WITH_SRC_AND_MC_ID_EXT */
                }
                break;
            ...
            case ARAD_PORT_HEADER_TYPE_STACKING:
                ...
                if(soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "user_header_always_remove", 0) == 1) { 
                    ...
                    prge_prog_select = ARAD_PRGE_TM_SELECT_STACKING; /* TM profile */
                }
                break;
            ...
            case ARAD_PORT_HEADER_TYPE_TDM:
                ...
                if(soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "user_header_always_remove", 0) == 1) {
                    ...
                    prge_prog_select = (tdm_mode == ARAD_MGMT_TDM_MODE_TDM_OPT) ? ARAD_PRGE_TM_SELECT_REMOVE_TDM_OPT_FTMH:ARAD_PRGE_TM_SELECT_REMOVE_SYSTEM_HEADER;           
                }
                break;
            ...
            case ARAD_PORT_HEADER_TYPE_TDM_PMM:
                ...
                if(soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "user_header_always_remove", 0) == 1) { 
                    ...
                    prge_prog_select = ARAD_PRGE_TM_SELECT_TDM_PMM_HEADER; /* TM profile */
                }
                break;
            ...
            case ARAD_PORT_HEADER_TYPE_INJECTED:
                ...
                if(soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "user_header_always_remove", 0) == 1) { 
                    ...
                    prge_prog_select = ARAD_PRGE_TM_SELECT_TDM_STAMP_CUD; /* TM profile */
                }
                break;
            ...
            case ARAD_PORT_HEADER_TYPE_XGS_DiffServ:
                ...
                if(soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "user_header_always_remove", 0) == 1) { 
                    ...
                    prge_prog_select = ARAD_PRGE_TM_SELECT_XGS_PE_FROM_FRC_LITE; /* TM profile */
                }
                eg_tm_profile = ARAD_EGR_PROG_TM_PORT_PROFILE_RAW; /* ? */
                break;
            ...
            case ARAD_PORT_HEADER_TYPE_XGS_MAC_EXT:
                port_type_val = ARAD_PORT_PCT_TYPE_ETH;
                if(soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "user_header_always_remove", 0) == 1) { 
                    SOCDNX_IF_ERR_EXIT(arad_egr_prog_editor_profile_get(unit, ARAD_PRGE_TM_SELECT_XGS_PE_FROM_FTMH, &prge_prog_select));               
                } else {
                    prge_prog_select = ARAD_PRGE_TM_SELECT_XGS_PE_FROM_FTMH; /* TM profile */
                }
                eg_tm_profile = ARAD_EGR_PROG_TM_PORT_PROFILE_ETH; 
                break;
            ...
            case ARAD_PORT_HEADER_TYPE_L2_REMOTE_CPU:
                ...
                if(soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "user_header_always_remove", 0) == 1) { 
                    ...
                    prge_prog_select = ARAD_PRGE_TM_SELECT_L2_REMOTE_CPU; /* TM profile */
                }
                break;
            ...
            case ARAD_PORT_HEADER_TYPE_MPLS_RAW:
                port_type_val = ARAD_PORT_PCT_TYPE_ETH;
                eg_tm_profile = ARAD_EGR_PROG_TM_PORT_PROFILE_ETH;
                prge_prog_select = ARAD_PRGE_TM_SELECT_RAW_MPLS; 
                break;
            ...
            for (curr_q_pair = base_q_pair; curr_q_pair < base_q_pair + nof_q_pairs; curr_q_pair++) 
            {
                ...
                if(soc_property_port_suffix_num_get(unit, port, -1, spn_CUSTOM_FEATURE, "add_size_header", 0))
                {
                    if(soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "user_header_always_remove", 0) == 1) {
                        ...
                        egq_pct_data.prog_editor_profile = ARAD_PRGE_TM_SELECT_ADD_PACKET_SIZE_HEADER;
                    }
                }
  ...
  )
{
    ...
    switch(port_parser_program_pointer) {
        ...
        case ARAD_PARSER_PARSER_PROGRAM_POINTER_FTMH:
            SOCDNX_IF_ERR_EXIT(MBCM_DPP_SOC_DRIVER_CALL(unit,mbcm_dpp_tdm_ifp_get,(unit, port, &is_tdm)));
            *header_type_in = (is_tdm == TRUE) ? ARAD_PORT_HEADER_TYPE_TDM : ARAD_PORT_HEADER_TYPE_STACKING;
            break;
        default:
  ...
  )
{
    ...
    switch(port_type_val) {
        case ARAD_PORT_PCT_TYPE_CPU:
            ...
            port_profile = arad_sw_db_egr_ports_prog_editor_profile_get(unit, port);
  ...
  )
{
    ...
    {   
        ...
        SOC_SAND_IF_ERR_EXIT(arad_sw_db_egr_ports_prog_editor_profile_set(unit,port,tm_port_profile));
        ...
        for (curr_q_pair = base_q_pair; curr_q_pair < base_q_pair + nof_pairs; curr_q_pair++) {
            ...
            if (is_double_tag) {
                if(soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "user_header_always_remove", 0) == 1) {
                    ...
                    pct_tbl_data.prog_editor_profile = ARAD_PRGE_TM_SELECT_PP_RCY_PORT;
                }
                ...
                if(soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "user_header_always_remove", 0) == 1) {
                    ...
                    pct_tbl_data.prog_editor_profile = ARAD_PRGE_TM_SELECT_OTMH_WITH_CUD_EXT;
                }
            } else {
  ...
  )
{
  ...
  port_profile = arad_sw_db_egr_ports_prog_editor_profile_get(unit,port_ndx);
  ...
  )
{
  ...
  if (
      (in_enable && in_lag->nof_entries   > ARAD_PORTS_LAG_IN_MEMBERS_MAX) ||
  ...
  if (out_enable)
  {
    if ((lag_member->flags & SOC_PPC_LAG_MEMBER_INGRESS_DISABLE) == 0x0 ) {
         ...
         */
        res = arad_irr_lag_mapping_tbl_set_unsafe(
                unit,
                lag_ndx,
                out_lag->nof_entries,
                &irr_lag_mapping_tbl_data
    ...
    */
    ...
    if((lag_member->flags & SOC_PPC_LAG_MEMBER_INGRESS_DISABLE) == 0x0) {
            ...
            res = arad_ports_lag_members_ranges_set_unsafe(
                    unit,
                    lag_ndx,
                    range, 
                    lag_member->sys_port,
  ...
  )
{
  ...
  if (in_enable == TRUE)
  {
        ...
        member_rep <= 1 )
    {
      ...
      res = arad_sw_db_modport2sysport_get(
                 unit, 
                 fap_id, 
                 port_id, 
                 &org_sys_port
    ...
    if (removed_member_index != out_lag->nof_entries-1)
    {
       ...
       */
       sys_phys_port_id = irr_lag_mapping_tbl_data.destination & ~INGRESS_DESTINATION_TYPE_SYS_PHY_PORT;
        ...
        if (moved_member_fap_id == sys_fap_id_self )
        {               
          uint32 moved_member_logic_sysport;
       ...
       */        
      if((fap_id == sys_fap_id_self) && (member_rep > 1)){
  ...
  */
    ...
    if (out_enable) {
        ...
        if((info->lag_member_sys_ports[lag_info_idx].flags & SOC_PPC_LAG_MEMBER_INGRESS_DISABLE) == 0x0 ) {
          ...
          res = arad_irr_lag_mapping_tbl_set_unsafe(
                  unit,
                  lag_ndx,
                  lag_info_idx - nof_ing_disable_members,
                  &irr_lag_mapping_tbl_data
                );
  ...
  if (out_enable)
  {
     ...
     res = arad_ports_lag_members_ranges_set_unsafe(
                unit,
                lag_ndx,
                info->nof_entries - nof_ing_disable_members, 
                0,
                FALSE
              );
  ...
  )
{
  ...
  SOC_SAND_ERR_IF_ABOVE_MAX(
    lag_ndx, ARAD_NOF_LAG_GROUPS-1,
    ARAD_PORT_LAG_ID_OUT_OF_RANGE_ERR, 10, exit
  );
    ...
    SOC_SAND_IN  uint32                              bitmap)
{
   ...
   ARAD_EGQ_PCT_TBL_DATA
    ...
    res = soc_port_sw_db_tm_port_to_base_q_pair_get(unit, core_id, local_port_ndx, &base_q_pair);
    SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 5, exit);
    res = soc_port_sw_db_tm_port_to_out_port_priority_get(unit, core_id, local_port_ndx, &nof_pairs);
...
exit:
    SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_port_stacking_route_history_bitmap_set_unsafe()",local_port_ndx,0);
}
...
soc_error_t 
arad_port_prbs_tx_enable_set(int unit, soc_port_t port, int value)
{
  ...
  blk_id = (int)port/SOC_DPP_DEFS_GET(unit, nof_fabric_links_in_mac);
  inner_port = port % SOC_DPP_DEFS_GET(unit, nof_fabric_links_in_mac);

  soc_sand_rv = arad_port_control_pcs_get(unit, port, &pcs);
...
soc_error_t 
arad_port_prbs_tx_enable_get(int unit, soc_port_t port, int* value)
{
  int blk_id, inner_port;
  uint32 reg_val32;
  ARAD_PORT_PCS  pcs;
...
soc_error_t 
arad_port_prbs_rx_enable_set(int unit, soc_port_t port, int value)
{
  int blk_id, inner_port;
  uint32 reg_val32;
  ARAD_PORT_PCS  pcs;
...
soc_error_t 
arad_port_prbs_rx_enable_get(int unit, soc_port_t port, int* value)
{
  int blk_id, inner_port;
  uint32 reg_val32;
  ARAD_PORT_PCS  pcs;
...
soc_error_t 
arad_port_prbs_rx_status_get(int unit, soc_port_t port, int* value)
{
  int blk_id, inner_port;
...
uint32 alloc_reassembly_context_and_recycle_channel_unsafe(int unit, int core_id, uint32 out_port, uint32 *out_context, uint32 *out_channel)
{
  ...
  rv = MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_drv_allocate_tm_port_in_range_and_recycle_channel,(unit, ARAD_PORTS_REASSEMBLY_CONTEXT_LAST - 1, &port, out_channel));
    ...
    SOC_SAND_IN  uint32     port)
{
    ...
    res = arad_pp_egq_dsp_ptr_map_tbl_get_unsafe(unit,port,SOC_CORE_ALL,&dsp_tbl_data);
    SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 20, exit);

    res = arad_egr_dsp_pp_to_base_q_pair_get_unsafe(unit, port, &base_q_pair);
    SOC_SAND_CHECK_FUNC_RESULT(res, 25, exit);
    dsp_tbl_data.out_tm_port = base_q_pair;

    res = arad_pp_egq_dsp_ptr_map_tbl_set_unsafe(unit,SOC_CORE_ALL,port,&dsp_tbl_data);
    SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 30, exit);
...
exit:
    ...
    SOC_SAND_EXIT_AND_SEND_ERROR("soc_arad_ports_egq_enabler()", port, 0);
}
    ...
    SOC_SAND_OUT uint32     enable)
{
    ...
    if(enable) 
    {
        res = soc_arad_ports_egq_enabler(unit, tm_port);
        SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 5, exit);
        ...
        res = READ_EGQ_INVALID_OTMr(unit, REG_PORT_ANY, &reg_val);
        SOC_SAND_SOC_CHECK_FUNC_RESULT(res , 50, exit);
        ...
        res=arad_ofp_q_pair_shapers_enable(unit,tm_port,0);
        SOC_SAND_CHECK_FUNC_RESULT(res , 60, exit);
        res=arad_ofp_tcg_shapers_enable(unit,tm_port,0);
        ...
        res = arad_pp_egq_dsp_ptr_map_tbl_get_unsafe(unit,SOC_CORE_ALL,tm_port,&dsp_tbl_data);
        SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 100, enable_shapers);

        dsp_tbl_data.out_tm_port = field_val;

        res = arad_pp_egq_dsp_ptr_map_tbl_set_unsafe(unit,SOC_CORE_ALL,tm_port,&dsp_tbl_data);
...
enable_shapers:
       res = arad_ofp_q_pair_shapers_enable(unit, tm_port, TRUE); 
       SOC_SAND_CHECK_FUNC_RESULT(res , 150, exit);
       res=arad_ofp_tcg_shapers_enable(unit,tm_port,1);
       SOC_SAND_CHECK_FUNC_RESULT(res , 160, exit);
       ...
       {
           res=arad_ofp_rates_egq_interface_shaper_set(unit,core,tm_port,  SOC_TMC_OFP_SHPR_UPDATE_MODE_OVERRIDE,if_spr_reg_val);
    ...
    SOC_SAND_IN ARAD_PORTS_APPLICATION_MAPPING_INFO *info)
{
  ...
  if (info->type == ARAD_PORTS_APPLICATION_MAPPING_TYPE_XGS_MAC_EXTENDER) 
  {
    ...
    /* get the queues mapped to this port */
    res = soc_port_sw_db_tm_port_to_base_q_pair_get(unit, core, port_ndx, &base_q_pair);
    SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 45, exit);
    res = soc_port_sw_db_tm_port_to_out_port_priority_get(unit, core, port_ndx, &nof_pairs);
    SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 50, exit);
   ...
   )
{
    ...
    if (interface != SOC_PORT_IF_ERP) {
        ...
        SOCDNX_IF_ERR_EXIT(arad_port_header_type_set_unsafe(unit,core,port,ARAD_PORT_DIRECTION_INCOMING,header_type_in));
        SOCDNX_IF_ERR_EXIT(arad_port_header_type_set_unsafe(unit,core,port,ARAD_PORT_DIRECTION_OUTGOING,header_type_out));
        ...
        if (header_type_out == ARAD_PORT_HEADER_TYPE_TM) {
            ...
            SOCDNX_IF_ERR_EXIT(arad_ports_otmh_extension_set(unit,core,port,&otmh_ext_en));
        }
