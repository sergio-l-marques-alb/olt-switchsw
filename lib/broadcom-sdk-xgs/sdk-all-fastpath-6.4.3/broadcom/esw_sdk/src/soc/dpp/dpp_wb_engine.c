...
static uint8 bcm_dpp_port_tpids_count[BCM_MAX_NUM_UNITS][SOC_TMC_NOF_FAP_PORTS_MAX] = {{0}};

int
soc_dpp_wb_engine_state_init_port(int unit)
{
    sal_memset(bcm_dpp_port_tpids_count[unit], 0, sizeof(uint8) * SOC_TMC_NOF_FAP_PORTS_MAX);
    return SOC_E_NONE;
}

static uint32* bcm_dpp_fec_remote_lif[BCM_MAX_NUM_UNITS] = {0};

int
soc_dpp_wb_engine_state_init_gport_info(int unit)
{

    /* bit per FEC */
    int mem_size = ((SOC_DPP_CONFIG(unit)->l3.fec_db_size + 31) / 32) * sizeof(uint32);

    if(bcm_dpp_fec_remote_lif[unit] == NULL) {
        bcm_dpp_fec_remote_lif[unit] = sal_alloc(mem_size, "bcm_dpp_fec_remote_lif");
    }
    if (bcm_dpp_fec_remote_lif[unit] == NULL) {
        return SOC_E_MEMORY;
    }

    sal_memset(bcm_dpp_fec_remote_lif[unit], 0, mem_size);

    soc_dpp_wb_engine_init_buffer(unit,SOC_DPP_WB_ENGINE_BUFFER_ARAD_GPORT);


    return SOC_E_NONE;
}

/* Deinit the gport information */
int
soc_dpp_wb_engine_state_deinit_gport_info(int unit)
{
    if (bcm_dpp_fec_remote_lif[unit] != NULL) {
        sal_free(bcm_dpp_fec_remote_lif[unit]);
        bcm_dpp_fec_remote_lif[unit] = NULL;
    }
   ...
   uint8 is_remote)
{
	...
	rv = SOC_DPP_WB_ENGINE_GET_ARR(unit,
								   SOC_DPP_WB_ENGINE_VAR_ARAD_GPORT_FEC_LIF_REMOTE,
								   &tmp,
								   fec_id/32);
	if (rv != SOC_E_NONE) {
		return rv;
...
/* VLAN edit tables - Set the size to the maximum of all the devices */
static uint8 _bcm_dpp_vlan_edit_eg_action_mapping[BCM_MAX_NUM_UNITS][SOC_PPD_MAX_NOF_EGRESS_VLAN_EDIT_ACTION_MAPPINGS];
static _bcm_dpp_vlan_translate_action_t _bcm_dpp_vlan_edit_ing_action[BCM_MAX_NUM_UNITS][SOC_PPD_NOF_INGRESS_VLAN_EDIT_ACTION_IDS];
static _bcm_dpp_vlan_translate_action_t _bcm_dpp_vlan_edit_eg_action[BCM_MAX_NUM_UNITS][SOC_PPD_MAX_NOF_EGRESS_VLAN_EDIT_ACTION_IDS];

int
soc_dpp_wb_engine_state_init_vlan_edit(int unit)
{
    sal_memset(_bcm_dpp_vlan_edit_eg_action_mapping[unit], 0, sizeof(uint8) * SOC_PPD_MAX_NOF_EGRESS_VLAN_EDIT_ACTION_MAPPINGS);
    sal_memset(_bcm_dpp_vlan_edit_ing_action[unit], 0, sizeof(_bcm_dpp_vlan_translate_action_t) * SOC_PPD_NOF_INGRESS_VLAN_EDIT_ACTION_IDS);
    sal_memset(_bcm_dpp_vlan_edit_eg_action[unit], 0, sizeof(_bcm_dpp_vlan_translate_action_t) * SOC_PPD_MAX_NOF_EGRESS_VLAN_EDIT_ACTION_IDS);
    return SOC_E_NONE;
}
...
soc_dpp_wb_engine_init_tables(int unit, int buffer_id)
{
     ...
     *  }
     */
        SOC_DPP_WB_ENGINE_ADD_BUFF(SOC_DPP_WB_ENGINE_BUFFER_ARAD_PORT_MAP, "port_map", NULL, VERSION(1), SOC_WB_ENGINE_PRE_RELEASE)
        {
            SOC_DPP_WB_ENGINE_ADD_ARR(SOC_DPP_WB_ENGINE_VAR_ARAD_PORT_MAP_PP_PORT,
                "port_map_pp_port",
                SOC_DPP_WB_ENGINE_BUFFER_ARAD_PORT_MAP,
                sizeof(int),
                SOC_MAX_NUM_PORTS,
                VERSION(1));

            SOC_DPP_WB_ENGINE_ADD_ARR(SOC_DPP_WB_ENGINE_VAR_ARAD_PORT_MAP_PP_PORT_USE,
                "port_map_pp_port_use",
                SOC_DPP_WB_ENGINE_BUFFER_ARAD_PORT_MAP,
                sizeof(soc_dpp_port_t),
                SOC_DPP_DEFS_GET(unit, nof_pp_ports),
                VERSION(1));
        }

        SOC_DPP_WB_ENGINE_ADD_BUFF(SOC_DPP_WB_ENGINE_BUFFER_RX, "bcm rx", NULL, VERSION(1), SOC_WB_ENGINE_PRE_RELEASE)
        {
            SOC_DPP_WB_ENGINE_ADD_ARR(SOC_DPP_WB_ENGINE_VAR_RX_VIRTUAL_TRAPS,
                "_rx_virtual_traps",
                SOC_DPP_WB_ENGINE_BUFFER_RX,
                sizeof(soc_dpp_rx_virtual_traps_t),
                _SOC_NUMBER_OF_VIRTUAL_TRAPS,
                VERSION(1));
        }
       
        SOC_DPP_WB_ENGINE_ADD_BUFF(SOC_DPP_WB_ENGINE_BUFFER_DPP_L3, "bcm l3", NULL, VERSION(1), SOC_WB_ENGINE_PRE_RELEASE)
        {
            SOC_DPP_WB_ENGINE_ADD_ARR(SOC_DPP_WB_ENGINE_VAR_DPP_L3_VRRP_PROTOCOL_GROUP_MEMBERS,
                "dpp_vrrp_protocol_set_types",
                SOC_DPP_WB_ENGINE_BUFFER_DPP_L3,
                sizeof(uint32),
                SOC_PPC_L3_NOF_PROTOCOL_GROUPS(unit),
                VERSION(1));
            SOC_DPP_WB_ENGINE_ADD_ARR(SOC_DPP_WB_ENGINE_VAR_DPP_L3_VRRP_PROTOCOL_GROUP_COUNT,
                "dpp_vrrp_protocol_set_counts",
                SOC_DPP_WB_ENGINE_BUFFER_DPP_L3,
                sizeof(uint32),
                SOC_PPC_L3_NOF_PROTOCOL_GROUPS(unit),
                VERSION(1));
        ...
        SOC_DPP_WB_ENGINE_ADD_BUFF(SOC_DPP_WB_ENGINE_BUFFER_ALLOC_MNGR_MULTI_CORE, "alloc mngr multi core", NULL, VERSION(1), SOC_WB_ENGINE_PRE_RELEASE)
        {
            SOC_DPP_WB_ENGINE_ADD_ARR(SOC_DPP_WB_ENGINE_VAR_ALLOC_MNGR_MULTI_CORE_RESOURCE_TO_BASE_POOL_MAP,
                "alloc_mngr_resource_to_pool_map",
                SOC_DPP_WB_ENGINE_BUFFER_ALLOC_MNGR_MULTI_CORE,
                sizeof(int),
                SOC_DPP_CONFIG(unit)->am.nof_am_resource_ids + SOC_DPP_CONFIG(unit)->am.nof_am_lif_resource_ids + SOC_DPP_CONFIG(unit)->am.nof_am_cosq_resource_ids,
                VERSION(1));
            SOC_DPP_WB_ENGINE_ADD_ARR(SOC_DPP_WB_ENGINE_VAR_ALLOC_MNGR_MULTI_CORE_TEMPLATE_TO_BASE_POOL_MAP,
                "alloc_mngr_template_to_pool_map",
                SOC_DPP_WB_ENGINE_BUFFER_ALLOC_MNGR_MULTI_CORE,
                sizeof(int),
                SOC_DPP_CONFIG(unit)->am.nof_am_template_ids,
                VERSION(1));
            SOC_DPP_WB_ENGINE_ADD_ARR(SOC_DPP_WB_ENGINE_VAR_ALLOC_MNGR_MULTI_CORE_GLOBAL_LIF_EGRESS_BANK_TYPES,
                "alloc_mngr_global_lif_egress_bank_types",
                SOC_DPP_WB_ENGINE_BUFFER_ALLOC_MNGR_MULTI_CORE,
                sizeof(bcm_dpp_am_global_lif_bank_type_t),
                _BCM_DPP_AM_MAX_NOF_GLOBAL_LIF_BANKS,
                VERSION(1));
            SOC_DPP_WB_ENGINE_ADD_ARR(SOC_DPP_WB_ENGINE_VAR_ALLOC_MNGR_MULTI_CORE_GLOBAL_LIF_INGRESS_BANK_TYPES,
                "alloc_mngr_global_lif_ingress_bank_types",
                SOC_DPP_WB_ENGINE_BUFFER_ALLOC_MNGR_MULTI_CORE,
                sizeof(bcm_dpp_am_global_lif_bank_type_t),
                _BCM_DPP_AM_MAX_NOF_GLOBAL_LIF_BANKS,
                VERSION(1));
            SOC_DPP_WB_ENGINE_ADD_VAR(SOC_DPP_WB_ENGINE_VAR_ALLOC_MNGR_MULTI_CORE_GLOBAL_LIF_EGRESS_COUNT,
                "alloc_mngr_global_lif_egress_count",
                SOC_DPP_WB_ENGINE_BUFFER_ALLOC_MNGR_MULTI_CORE,
                sizeof(int),
                VERSION(1));
            SOC_DPP_WB_ENGINE_ADD_VAR(SOC_DPP_WB_ENGINE_VAR_ALLOC_MNGR_MULTI_CORE_GLOBAL_LIF_INGRESS_COUNT,
                "alloc_mngr_global_lif_ingress_count",
                SOC_DPP_WB_ENGINE_BUFFER_ALLOC_MNGR_MULTI_CORE,
                sizeof(int),
                VERSION(1));
        }

        SOC_DPP_WB_ENGINE_ADD_BUFF(SOC_DPP_WB_ENGINE_BUFFER_ARAD_SW_DB_LIF_RIF_PROFILE, "lif_and_rif_profiles", NULL, VERSION(1), SOC_WB_ENGINE_PRE_RELEASE)
        {
            SOC_DPP_WB_ENGINE_ADD_ARR(SOC_DPP_WB_ENGINE_VAR_ARAD_SW_DB_INLIF_PROFILE_MASK,
                "inlif_profile_mask",
                SOC_DPP_WB_ENGINE_BUFFER_ARAD_SW_DB_LIF_RIF_PROFILE,
                sizeof(uint32),
                SOC_PPC_PROFILE_MGMT_INLIF_PROFILE_NOF_BITS,
                VERSION(1));
            SOC_DPP_WB_ENGINE_ADD_ARR(SOC_DPP_WB_ENGINE_VAR_ARAD_SW_DB_OUTLIF_PROFILE_MASK,
                "outlif_profile_mask",
                SOC_DPP_WB_ENGINE_BUFFER_ARAD_SW_DB_LIF_RIF_PROFILE,
                sizeof(uint32),
                SOC_PPC_PROFILE_MGMT_OUTLIF_PROFILE_NOF_BITS(unit),
                VERSION(1));
            SOC_DPP_WB_ENGINE_ADD_ARR(SOC_DPP_WB_ENGINE_VAR_ARAD_SW_DB_RIF_PROFILE_MASK,
                "rif_profile_mask",
                SOC_DPP_WB_ENGINE_BUFFER_ARAD_SW_DB_LIF_RIF_PROFILE,
                sizeof(uint32),
                SOC_PPC_PROFILE_MGMT_RIF_PROFILE_NOF_BITS,
                VERSION(1));
        }
		
#if defined(INCLUDE_KBP) && !defined(BCM_88030)
        SOC_DPP_WB_ENGINE_ADD_BUFF(SOC_DPP_WB_ENGINE_BUFFER_ARAD_KBP, "arad KBP", NULL, VERSION(1), SOC_WB_ENGINE_PRE_RELEASE)
        {
            SOC_DPP_WB_ENGINE_ADD_ARR(SOC_DPP_WB_ENGINE_VAR_ARAD_KBP_DB_INFO,
                "Arad_kbp_db_info",
                SOC_DPP_WB_ENGINE_BUFFER_ARAD_KBP,
                sizeof(ARAD_KBP_DB_HANDLES),
                ARAD_KBP_MAX_NUM_OF_TABLES,
                VERSION(1));
            SOC_DPP_WB_ENGINE_ADD_ARR(SOC_DPP_WB_ENGINE_VAR_ARAD_KBP_LTR_INFO,
                "Arad_kbp_ltr_info",
                SOC_DPP_WB_ENGINE_BUFFER_ARAD_KBP,
                sizeof(ARAD_KBP_LTR_HANDLES),
                ARAD_KBP_MAX_NUM_OF_FRWRD_DBS,
                VERSION(1));
        }
#endif /* defined(INCLUDE_KBP) && !defined(BCM_88030) */

        SOC_DPP_WB_ENGINE_ADD_BUFF(SOC_DPP_WB_ENGINE_BUFFER_OUTLIF_DB, "outlif information", NULL, VERSION(1), SOC_WB_ENGINE_PRE_RELEASE)
        {
            SOC_DPP_WB_ENGINE_ADD_ARR(SOC_DPP_WB_ENGINE_VAR_OUTLIF_INFO,
                "outlif_info",
                SOC_DPP_WB_ENGINE_BUFFER_OUTLIF_DB,
                sizeof(_bcm_dpp_outlif_match_info_t),
                _BCM_GPORT_NOF_LIFS,
                VERSION(1));
        }
		
		SOC_DPP_WB_ENGINE_ADD_BUFF(SOC_DPP_WB_ENGINE_BUFFER_JER_MODID_GROUP_MAP_TO_DEST, "jer modid group map to dest", NULL, VERSION(1), SOC_WB_ENGINE_PRE_RELEASE)
        {
            SOC_DPP_WB_ENGINE_ADD_ARR(SOC_DPP_WB_ENGINE_VAR_JER_MODID_GROUP_MAP_DEST_0,
                "jer_modid_group_map_dest_0",
                SOC_DPP_WB_ENGINE_BUFFER_JER_MODID_GROUP_MAP_TO_DEST,
                sizeof(int),
                SOC_DPP_MAX_NOF_FAP_ID_MAPPED_TO_DEST_LIMIT,
                VERSION(1));
            SOC_DPP_WB_ENGINE_ADD_VAR(SOC_DPP_WB_ENGINE_VAR_JER_GROUP_MAP_DEST_0_COUNT,
                "jer_group_map_dest_0_count",
                SOC_DPP_WB_ENGINE_BUFFER_JER_MODID_GROUP_MAP_TO_DEST,
                sizeof(int),
                VERSION(1));
            SOC_DPP_WB_ENGINE_ADD_ARR(SOC_DPP_WB_ENGINE_VAR_JER_MODID_GROUP_MAP_DEST_1,
                "jer_modid_group_map_dest_1",
                SOC_DPP_WB_ENGINE_BUFFER_JER_MODID_GROUP_MAP_TO_DEST,
                sizeof(int),
                SOC_DPP_MAX_NOF_FAP_ID_MAPPED_TO_DEST_LIMIT,
                VERSION(1));
            SOC_DPP_WB_ENGINE_ADD_VAR(SOC_DPP_WB_ENGINE_VAR_JER_GROUP_MAP_DEST_1_COUNT,
                "jer_group_map_dest_1_count",
                SOC_DPP_WB_ENGINE_BUFFER_JER_MODID_GROUP_MAP_TO_DEST,
                sizeof(int),
                VERSION(1));
            SOC_DPP_WB_ENGINE_ADD_ARR(SOC_DPP_WB_ENGINE_VAR_JER_MODID_GROUP_MAP_DEST_2,
                "jer_modid_group_map_dest_2",
                SOC_DPP_WB_ENGINE_BUFFER_JER_MODID_GROUP_MAP_TO_DEST,
                sizeof(int),
                SOC_DPP_MAX_NOF_FAP_ID_MAPPED_TO_DEST_LIMIT,
                VERSION(1));
            SOC_DPP_WB_ENGINE_ADD_VAR(SOC_DPP_WB_ENGINE_VAR_JER_GROUP_MAP_DEST_2_COUNT,
                "jer_group_map_dest_2_count",
                SOC_DPP_WB_ENGINE_BUFFER_JER_MODID_GROUP_MAP_TO_DEST,
                sizeof(int),
                VERSION(1));
        }

        SOC_DPP_WB_ENGINE_ADD_BUFF(SOC_DPP_WB_ENGINE_BUFFER_ARAD_PP_BFD_YDISCR_RANGE_REF_COUNTER, "YourDiscriminator BFD endpoints reference counter", NULL, VERSION(1), SOC_WB_ENGINE_PRE_RELEASE)
        {
            SOC_DPP_WB_ENGINE_ADD_VAR(SOC_DPP_WB_ENGINE_VAR_ARAD_PP_BFD_YDISCR_RANGE_REF_COUNTER,
                "YourDiscriminator_bfd_ref_count",
                SOC_DPP_WB_ENGINE_BUFFER_ARAD_PP_BFD_YDISCR_RANGE_REF_COUNTER,
                sizeof(int),
                VERSION(1));
        }


        SOC_DPP_WB_ENGINE_ADD_BUFF(SOC_DPP_WB_ENGINE_BUFFER_FEC_BANK_GROUPS, "FEC bank groups", NULL, VERSION(1), SOC_WB_ENGINE_PRE_RELEASE)
        {
            SOC_DPP_WB_ENGINE_ADD_ARR(SOC_DPP_WB_ENGINE_VAR_FEC_BANK_GROUPS,
                "fec_bank_groups",
                SOC_DPP_WB_ENGINE_BUFFER_FEC_BANK_GROUPS,
                sizeof(uint8),
                SOC_DPP_DEFS_GET(unit, nof_fec_banks),
                VERSION(1));
        }

        SOC_DPP_WB_ENGINE_ADD_BUFF(SOC_DPP_WB_ENGINE_BUFFER_JER_FAILOVER_SW_DB, "Failover SW DB used by Jericho", NULL, VERSION(1), SOC_WB_ENGINE_PRE_RELEASE)
        {
            SOC_DPP_WB_ENGINE_ADD_ARR(SOC_DPP_WB_ENGINE_VAR_JER_FAILOVER_ID_SEQ_SIZE,
                "Failover ID sequence size",
                SOC_DPP_WB_ENGINE_BUFFER_JER_FAILOVER_SW_DB,
                sizeof(uint8),
                SOC_DPP_DEFS_GET(unit, nof_failover_egress_ids),
                VERSION(1));
        }

        SOC_DPP_WB_ENGINE_ADD_BUFF(SOC_DPP_WB_ENGINE_BUFFER_DPP_INTERRUPTS, "dpp interrupts", NULL, VERSION(1), SOC_WB_ENGINE_PRE_RELEASE)
        {
            SOC_DPP_WB_ENGINE_ADD_VAR(SOC_DPP_WB_ENGINE_VAR_DPP_INTERRUPTS_BCM_SWITCH_EVENT_STORM_NOMINAL,
                "dpp_interrupts_bcm_switch_event_storm_nominal",
                SOC_DPP_WB_ENGINE_BUFFER_DPP_INTERRUPTS,
                sizeof(int),
                VERSION(1));
        }

        SOC_DPP_WB_ENGINE_ADD_BUFF(SOC_DPP_WB_ENGINE_BUFFER_BFD_S_HOP, "BFD IPv4 TOS profile reference counter", NULL, VERSION(1), SOC_WB_ENGINE_PRE_RELEASE)
        {
            SOC_DPP_WB_ENGINE_ADD_VAR(SOC_DPP_WB_ENGINE_VAR_ARAD_PP_BFD_S_HOP_REF_COUNTER,
                "BFD IPv4 TOS profile reference counter",
                SOC_DPP_WB_ENGINE_BUFFER_BFD_S_HOP,
                sizeof(int),
                VERSION(1));

            SOC_DPP_WB_ENGINE_ADD_VAR(SOC_DPP_WB_ENGINE_VAR_ARAD_PP_BFD_S_HOP_TOS_VALUE,
                "BFD IPv4 TOS data",
                SOC_DPP_WB_ENGINE_BUFFER_BFD_S_HOP,
                sizeof(int),
                VERSION(1));
    ...
    uint32    *input_buffer)
{
    ...
    for (i=start_reg; i<=end_reg; i++) {
        rv = soc_wb_engine_var_get(unit, SOC_WB_ENGINE_PRIMARY, var_ndx, 0, i, (uint8 *)(&(output_buffer[i-start_reg])));
        SOCDNX_IF_ERR_EXIT(rv);
    }
    ...
    for (i=start_reg; i<=end_reg; i++) {
        rv = soc_wb_engine_var_set(unit, SOC_WB_ENGINE_PRIMARY, var_ndx, 0, i, (uint8 *)(&(output_buffer[i-start_reg])));
        SOCDNX_IF_ERR_EXIT(rv);
    }
  ...
  )
{
    ...
    for (i=start_reg; i<=end_reg; i++) {
        rv = soc_wb_engine_var_get(unit, SOC_WB_ENGINE_PRIMARY, var_ndx, 0, i, (uint8 *)(&(input_buffer[i-start_reg])));
        SOCDNX_IF_ERR_EXIT(rv);
    }
