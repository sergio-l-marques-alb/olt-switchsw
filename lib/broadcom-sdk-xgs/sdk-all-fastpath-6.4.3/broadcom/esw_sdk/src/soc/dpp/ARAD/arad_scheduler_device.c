  ...
  )
{
  ...
  res = arad_interface_id_verify(unit, if_ndx);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);

  SOC_SAND_ERR_IF_ABOVE_MAX(
    rate, ARAD_IF_MAX_RATE_KBPS,
    ARAD_OFP_RATES_SCH_RATE_OUT_OF_RANGE_ERR , 20, exit
  );
  ...
  )
{
    ...
    if (is_channelized)
    {
        SOCDNX_IF_ERR_EXIT(READ_SCH_CH_NIF_RATES_CONFIGURATION__CNRCm(unit, SCH_BLOCK(unit, core), offset, tbl_data));
        credit_rate = soc_SCH_CH_NIF_RATES_CONFIGURATION__CNRCm_field32_get(unit, tbl_data, CH_NI_FXX_MAX_CR_RATEf);
    }
    else
    {
        SOCDNX_IF_ERR_EXIT(READ_SCH_ONE_PORT_NIF_CONFIGURATION__OPNCm(unit, SCH_BLOCK(unit, core), offset, tbl_data));
        credit_rate = soc_SCH_ONE_PORT_NIF_CONFIGURATION__OPNCm_field32_get(unit, tbl_data, PORT_NIF_MAX_CR_RATEf);
    }
  ...
  )
{
  ...
  SOC_SAND_ERR_IF_ABOVE_MAX(
    ...
    ARAD_SCH_INTERFACE_WEIGHT_INDEX_OUT_OF_RANGE_ERR, 10, exit
  );

  for (weight_idx = 0; weight_idx < ARAD_SCH_NOF_IF_WEIGHTS; weight_idx++)
  {
    SOC_SAND_ERR_IF_ABOVE_MAX(
      if_weights->weight[weight_idx].id, ARAD_SCH_NOF_IF_WEIGHTS,
 ...
 * COVERITY
 *
 * ARAD_SCH_DEVICE_IF_WEIGHT_MIN may be changed to be bigger than 0.
 */
/* coverity[unsigned_compare] */
    SOC_SAND_ERR_IF_OUT_OF_RANGE(
      if_weights->weight[weight_idx].val, ARAD_SCH_DEVICE_IF_WEIGHT_MIN, ARAD_SCH_DEVICE_IF_WEIGHT_MAX,
      ARAD_SCH_DEVICE_IF_WEIGHT_OUT_OF_RANGE_ERR, 30, exit
    );
  }
  ...
  )
{
  ...
  SOC_SAND_ERR_IF_ABOVE_MAX(
      weight_index, ARAD_SCH_NOF_IF_WEIGHTS,
      ARAD_SCH_INTERFACE_WEIGHT_INDEX_OUT_OF_RANGE_ERR, 30, exit
    );
  ...
  )
{
  ...
  /* Get scheduler offset */
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 1260, exit, READ_SCH_FC_MAP_FCMm(unit,MEM_BLOCK_ANY,egress_offset,&data));
  ...
  if (is_channelized)
  {
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 1270, exit, READ_SCH_CH_NIF_CALENDAR_CONFIGURATION__CNCCm(unit, MEM_BLOCK_ANY, offset, &tbl_data));
    weight_idx_result = soc_SCH_CH_NIF_CALENDAR_CONFIGURATION__CNCCm_field32_get(unit, &tbl_data, CH_NI_FXX_WEIGHTf);
  }
  else
  {
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 1280, exit, READ_SCH_ONE_PORT_NIF_CONFIGURATION__OPNCm(unit,MEM_BLOCK_ANY,offset,&tbl_data));
    weight_idx_result = soc_SCH_ONE_PORT_NIF_CONFIGURATION__OPNCm_field32_get(unit, &tbl_data, PORT_NIF_WEIGHTf);
  } 

  *weight_index = weight_idx_result;

exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_sch_device_if_weight_idx_get_unsafe()",if_ndx,0);
}
  ...
  )
{
  ...
  /* Get scheduler offset */
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 1380, exit, READ_SCH_FC_MAP_FCMm(unit,MEM_BLOCK_ANY,egress_offset,&data));
  sch_offset = soc_SCH_FC_MAP_FCMm_field32_get(unit,&data,FC_MAP_FCMf);
  ...
  if (is_channelized)
  {
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 1390, exit, READ_SCH_CH_NIF_CALENDAR_CONFIGURATION__CNCCm(unit, MEM_BLOCK_ANY, offset, &tbl_data));
    soc_SCH_CH_NIF_CALENDAR_CONFIGURATION__CNCCm_field32_set(unit, &tbl_data, CH_NI_FXX_WEIGHTf,weight_index);
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 1400, exit, WRITE_SCH_CH_NIF_CALENDAR_CONFIGURATION__CNCCm(unit, MEM_BLOCK_ANY, offset, &tbl_data));
  }
  else
  {
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 1410, exit, READ_SCH_ONE_PORT_NIF_CONFIGURATION__OPNCm(unit,MEM_BLOCK_ANY,offset,&tbl_data));
    soc_SCH_ONE_PORT_NIF_CONFIGURATION__OPNCm_field32_set(unit, &tbl_data, PORT_NIF_WEIGHTf,weight_index);
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 1420, exit, WRITE_SCH_ONE_PORT_NIF_CONFIGURATION__OPNCm(unit,MEM_BLOCK_ANY,offset,&tbl_data));
  } 

exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_sch_device_if_weight_idx_set_unsafe()",if_ndx,0);
}
  ...
  )
{
    ...
    else
    {
        ...
        credit_rate_nof_bits = soc_mem_field_length(unit, SCH_CH_NIF_RATES_CONFIGURATION__CNRCm, CH_NI_FXX_MAX_CR_RATEf);
        SOC_SAND_LIMIT_FROM_ABOVE(credit_rate, SOC_SAND_BITS_MASK((credit_rate_nof_bits-1),0));
        SOC_SAND_LIMIT_FROM_BELOW(credit_rate, ARAD_SCH_DEV_RATE_INTERVAL_RESOLUTION);
    }

    SOCDNX_IF_ERR_EXIT(READ_SCH_CH_NIF_RATES_CONFIGURATION__CNRCm(unit,SCH_BLOCK(unit, core), sch_offset,&tbl_data));
    soc_SCH_CH_NIF_RATES_CONFIGURATION__CNRCm_field32_set(unit,&tbl_data,CH_NI_FXX_SUM_OF_PORTSf,credit_rate);
    SOCDNX_IF_ERR_EXIT(WRITE_SCH_CH_NIF_RATES_CONFIGURATION__CNRCm(unit,SCH_BLOCK(unit, core), sch_offset,&tbl_data));
  ...
  )
{
    ...
    SOCDNX_IF_ERR_EXIT(READ_SCH_CH_NIF_RATES_CONFIGURATION__CNRCm(unit, SCH_BLOCK(unit, core), sch_offset, &tbl_data));
    credit_rate = soc_SCH_CH_NIF_RATES_CONFIGURATION__CNRCm_field32_get(unit, &tbl_data,CH_NI_FXX_SUM_OF_PORTSf);
