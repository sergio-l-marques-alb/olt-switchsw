  ...
  )
{
  ...
  /* LB mode */
  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  10,  exit, ARAD_REG_ACCESS_ERR, soc_reg32_get(unit, global_f, REG_PORT_ANY, 0, &reg_val));
  ARAD_FLD_FROM_REG(global_f, FTMH_LB_KEY_EXT_ENf, ftmh_lb_key_enable, reg_val, 20, exit);
  res = arad_pp_sw_db_fwd_header_data_ftmh_lb_key_ext_en_set(unit, &ftmh_lb_key_enable);
  SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);

  /* Stacking mode */
  ARAD_FLD_FROM_REG(global_f, FTMH_STACKING_EXT_ENABLEf, ftmh_stacking_ext_mode, reg_val, 50, exit);
  res = arad_pp_sw_db_fwd_header_data_ftmh_stacking_ext_enable_set(unit, &ftmh_stacking_ext_mode);
  SOC_SAND_CHECK_FUNC_RESULT(res, 60, exit);

  /* System headers mode */
  ARAD_FLD_FROM_REG(global_f, SYSTEM_HEADERS_MODEf, system_headers_mode, reg_val, 80, exit);
  arad_pp_sw_db_fwd_header_data_system_headers_mode_set(unit, &system_headers_mode);
  ...
  )
{
  ...
  */
  ...
  /* Set sa-based prefix */
  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  160,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, IHP_LINK_LAYER_LOOKUP_CFGr, SOC_CORE_ALL, 0, SA_BASED_VID_PREFIXf,  ARAD_PP_LEM_ACCESS_KEY_TYPE_PREFIX_SA_AUTH));
