...
_soc_trident2p_ser_enable_all(int unit, const _soc_td2p_ser_block_info_t *ser_block_info, int enable)
{
    ...
    if (enable) {
        ...
        if (SOC_IS_TRIDENT2PLUS(unit)) {
            soc_reg_field_set(unit, MISCCONFIGr, &rval, PARITY_ENf, 1);
...
_soc_trident2_tcam_ser_init(int unit)
{
    ...
    if (!SOC_IS_TRIDENT2PLUS(unit)) {
        ...
        override_idx = COUNTOF(_soc_td2_tcam_ser_info_template) - 3;
...
const soc_td2_valid_fallback_lanes_t soc_td2_valid_lanes[] = {
    {SOC_LANE_CONFIG_100G_4_4_2, {TRUE, TRUE, FALSE} },
    {SOC_LANE_CONFIG_100G_3_4_3, {FALSE, TRUE, FALSE} },
    {SOC_LANE_CONFIG_100G_2_4_4, {FALSE, TRUE, TRUE} },
...
soc_trident2_port_config_init(int unit, uint16 dev_id, uint8 rev_id)
{
    ...
    for (port = 0; port < num_port; port++) {
        ...
        /*
         * portmap.<port>=<physical port number>:<bandwidth in Gb>:<100G TSC lanes>:<100G Fallback XLMAC>:i
        ...
        /* Skip ':' between Bandwidth and 100G lane config */
        sub_str = sub_str_end;
        if (*sub_str != '\0') {
            if (*sub_str != ':') {
...
_soc_trident2_tdm_init(int unit)
{
    ...
    int wt_group = 0;
#ifdef BCM_TRIDENT2PLUS_SUPPORT
	td2p_tdm_globals_t tdm_globals;
	td2p_tdm_pipes_t tdm_pipe_tables;
    int wt_group_select;
#endif
...
#ifdef BCM_TRIDENT2PLUS_SUPPORT
		for (index = 0; index < NUM_EXT_PORTS; index++) {
			tdm_globals.speed[index] = _soc_td2_tdm.speed[index];
		}
		for (index = 1; index < NUM_EXT_PORTS; index++) {
			tdm_globals.port_rates_array[index-1] = _soc_td2_tdm.port_state[index];
		}
		for (index = 0; index < _TD2P_NUM_PORT_MODULES; index++) {
			tdm_globals.pm_encap_type[index] = _soc_td2_tdm.encap_type[index];
		}
        /*TDM code supports max 760Mhz*/
		tdm_globals.clk_freq = ( si -> frequency > 760 )? 760 : si -> frequency;
        if (_soc_td2_tdm.manage_port_type > 0) {
            tdm_pipe_tables.pgw_tdm_tbl_x0[0] = 1234;
        }
		rv = TD2P_set_tdm_tbl(&tdm_globals, &tdm_pipe_tables);
#endif
    } else {
	...
	if (SOC_IS_TRIDENT2PLUS(unit)) {
#ifdef BCM_TRIDENT2PLUS_SUPPORT
        sal_memcpy(_soc_td2_tdm.pgw_tdm[0], tdm_pipe_tables.pgw_tdm_tbl_x0, sizeof(int)*MAX_PGW_TDM_LENGTH);
        sal_memcpy(_soc_td2_tdm.pgw_ovs_tdm[0], tdm_pipe_tables.ovs_tdm_tbl_x0, sizeof(int)*_PGW_TDM_OVS_SIZE);
        sal_memcpy(_soc_td2_tdm.pgw_ovs_spacing[0], tdm_pipe_tables.ovs_spacing_x0, sizeof(int)*_PGW_TDM_OVS_SIZE);

        sal_memcpy(_soc_td2_tdm.pgw_tdm[1], tdm_pipe_tables.pgw_tdm_tbl_x1, sizeof(int)*MAX_PGW_TDM_LENGTH);
        sal_memcpy(_soc_td2_tdm.pgw_ovs_tdm[1], tdm_pipe_tables.ovs_tdm_tbl_x1, sizeof(int)*_PGW_TDM_OVS_SIZE);
        sal_memcpy(_soc_td2_tdm.pgw_ovs_spacing[1], tdm_pipe_tables.ovs_spacing_x1, sizeof(int)*_PGW_TDM_OVS_SIZE);		

        sal_memcpy(_soc_td2_tdm.pgw_tdm[2], tdm_pipe_tables.pgw_tdm_tbl_y0, sizeof(int)*MAX_PGW_TDM_LENGTH);
        sal_memcpy(_soc_td2_tdm.pgw_ovs_tdm[2], tdm_pipe_tables.ovs_tdm_tbl_y0, sizeof(int)*_PGW_TDM_OVS_SIZE);
        sal_memcpy(_soc_td2_tdm.pgw_ovs_spacing[2], tdm_pipe_tables.ovs_spacing_y0, sizeof(int)*_PGW_TDM_OVS_SIZE);		

        sal_memcpy(_soc_td2_tdm.pgw_tdm[3], tdm_pipe_tables.pgw_tdm_tbl_y1, sizeof(int)*MAX_PGW_TDM_LENGTH);
        sal_memcpy(_soc_td2_tdm.pgw_ovs_tdm[3], tdm_pipe_tables.ovs_tdm_tbl_y1, sizeof(int)*_PGW_TDM_OVS_SIZE);
        sal_memcpy(_soc_td2_tdm.pgw_ovs_spacing[3], tdm_pipe_tables.ovs_spacing_y1, sizeof(int)*_PGW_TDM_OVS_SIZE);

        sal_memcpy(_soc_td2_tdm.mmu_tdm[0], tdm_pipe_tables.mmu_tdm_tbl_x, sizeof(int)*_MMU_TDM_LENGTH);
        sal_memcpy(_soc_td2_tdm.mmu_ovs_group_tdm[0][0], tdm_pipe_tables.mmu_tdm_ovs_x_0, sizeof(int)*_MMU_OVS_GROUP_TDM_LENGTH);
        sal_memcpy(_soc_td2_tdm.mmu_ovs_group_tdm[0][1], tdm_pipe_tables.mmu_tdm_ovs_x_1, sizeof(int)*_MMU_OVS_GROUP_TDM_LENGTH);
        sal_memcpy(_soc_td2_tdm.mmu_ovs_group_tdm[0][2], tdm_pipe_tables.mmu_tdm_ovs_x_2, sizeof(int)*_MMU_OVS_GROUP_TDM_LENGTH);
        sal_memcpy(_soc_td2_tdm.mmu_ovs_group_tdm[0][3], tdm_pipe_tables.mmu_tdm_ovs_x_3, sizeof(int)*_MMU_OVS_GROUP_TDM_LENGTH);
        sal_memcpy(_soc_td2_tdm.mmu_ovs_group_tdm[0][4], tdm_pipe_tables.mmu_tdm_ovs_x_4, sizeof(int)*_MMU_OVS_GROUP_TDM_LENGTH);
        sal_memcpy(_soc_td2_tdm.mmu_ovs_group_tdm[0][5], tdm_pipe_tables.mmu_tdm_ovs_x_5, sizeof(int)*_MMU_OVS_GROUP_TDM_LENGTH);
        sal_memcpy(_soc_td2_tdm.mmu_ovs_group_tdm[0][6], tdm_pipe_tables.mmu_tdm_ovs_x_6, sizeof(int)*_MMU_OVS_GROUP_TDM_LENGTH);
        sal_memcpy(_soc_td2_tdm.mmu_ovs_group_tdm[0][7], tdm_pipe_tables.mmu_tdm_ovs_x_7, sizeof(int)*_MMU_OVS_GROUP_TDM_LENGTH);

        sal_memcpy(_soc_td2_tdm.mmu_tdm[1], tdm_pipe_tables.mmu_tdm_tbl_y, sizeof(int)*_MMU_TDM_LENGTH);
        sal_memcpy(_soc_td2_tdm.mmu_ovs_group_tdm[1][0], tdm_pipe_tables.mmu_tdm_ovs_y_0, sizeof(int)*_MMU_OVS_GROUP_TDM_LENGTH);
        sal_memcpy(_soc_td2_tdm.mmu_ovs_group_tdm[1][1], tdm_pipe_tables.mmu_tdm_ovs_y_1, sizeof(int)*_MMU_OVS_GROUP_TDM_LENGTH);
        sal_memcpy(_soc_td2_tdm.mmu_ovs_group_tdm[1][2], tdm_pipe_tables.mmu_tdm_ovs_y_2, sizeof(int)*_MMU_OVS_GROUP_TDM_LENGTH);
        sal_memcpy(_soc_td2_tdm.mmu_ovs_group_tdm[1][3], tdm_pipe_tables.mmu_tdm_ovs_y_3, sizeof(int)*_MMU_OVS_GROUP_TDM_LENGTH);
        sal_memcpy(_soc_td2_tdm.mmu_ovs_group_tdm[1][4], tdm_pipe_tables.mmu_tdm_ovs_y_4, sizeof(int)*_MMU_OVS_GROUP_TDM_LENGTH);
        sal_memcpy(_soc_td2_tdm.mmu_ovs_group_tdm[1][5], tdm_pipe_tables.mmu_tdm_ovs_y_5, sizeof(int)*_MMU_OVS_GROUP_TDM_LENGTH);
        sal_memcpy(_soc_td2_tdm.mmu_ovs_group_tdm[1][6], tdm_pipe_tables.mmu_tdm_ovs_y_6, sizeof(int)*_MMU_OVS_GROUP_TDM_LENGTH);
        sal_memcpy(_soc_td2_tdm.mmu_ovs_group_tdm[1][7], tdm_pipe_tables.mmu_tdm_ovs_y_7, sizeof(int)*_MMU_OVS_GROUP_TDM_LENGTH);
#endif	
	}
    ...
    if (SOC_IS_TRIDENT2PLUS(unit)) {
#ifdef BCM_TRIDENT2PLUS_SUPPORT
        rv = TD2P_set_iarb_tdm_table (_soc_td2_tdm.tdm_bw,
                                _soc_td2_tdm.pipe_ovs_state[0],
                                _soc_td2_tdm.pipe_ovs_state[1],
...
_soc_td2_ledup_init(int unit)
{
    ...
    if ( SAL_BOOT_QUICKTURN ) {
        SOC_BLOCK_ITER(unit, blk, SOC_BLK_XLPORT) {
            ...
            SOC_IF_ERROR_RETURN(soc_reg32_set(unit, XLPORT_LED_CHAIN_CONFIGr, port, 0, 0x00000002));
        }
    } else {
...
_soc_trident2_misc_init(int unit)
{
    ...
    SOC_BLOCK_ITER(unit, blk, SOC_BLK_CPORT) {
        ...
        if (si->port_speed_max[port] >= 100000) {
            ...
            soc_reg_field_set(unit, CPORT_SOFT_RESETr, &rval, CPORT_COREf, 1);
            SOC_IF_ERROR_RETURN(WRITE_CPORT_SOFT_RESETr(unit, port, rval));

            SOC_IF_ERROR_RETURN(
                READ_EGR_PER_PORT_BUFFER_SFT_RESETm(unit, MEM_BLOCK_ALL, port, entry));
            ...
            SOC_IF_ERROR_RETURN(
                READ_EGR_PER_PORT_BUFFER_SFT_RESETm(unit, MEM_BLOCK_ALL, port, entry));
            soc_mem_field32_set(unit, EGR_PER_PORT_BUFFER_SFT_RESETm, entry, ENABLEf, 0);
            SOC_IF_ERROR_RETURN(
                WRITE_EGR_PER_PORT_BUFFER_SFT_RESETm(unit, MEM_BLOCK_ALL, port, entry));
            ...
            SOC_IF_ERROR_RETURN(READ_CPORT_SOFT_RESETr(unit, port, &rval));
            soc_reg_field_set(unit, CPORT_SOFT_RESETr, &rval, CPORT_COREf, 0);
            SOC_IF_ERROR_RETURN(WRITE_CPORT_SOFT_RESETr(unit, port, rval));
        }
    ...
    if (soc_feature(unit, soc_feature_my_station_2)) {
        /*
         * Set USE_MY_STATION1_FOR_NON_TUNNELS to 1 
         * so that MY_STATION_TCAM_1 will be used for L3.
         * If 0, MY_STATION_TCAM_2 will be used for L3.
         */
        soc_reg64_field32_set(unit, ING_CONFIG_64r, &rval64,
...
const soc_ser_overlay_test_t soc_ser_td2_overlay_mems[] = {
    {MODPORT_MAP_M0m, ISW2_SER_CONTROL_0r, MODPORT_MAP_MIRROR_PARITY_ENf, MODPORT_MAP_MIRRORm, _SOC_ACC_TYPE_PIPE_ANY},
    {MODPORT_MAP_M1m, ISW2_SER_CONTROL_0r, MODPORT_MAP_MIRROR_PARITY_ENf, MODPORT_MAP_MIRRORm, _SOC_ACC_TYPE_PIPE_ANY},
    {MODPORT_MAP_M2m, ISW2_SER_CONTROL_0r, MODPORT_MAP_MIRROR_PARITY_ENf, MODPORT_MAP_MIRRORm, _SOC_ACC_TYPE_PIPE_ANY},
    {MODPORT_MAP_M3m, ISW2_SER_CONTROL_0r, MODPORT_MAP_MIRROR_PARITY_ENf, MODPORT_MAP_MIRRORm, _SOC_ACC_TYPE_PIPE_ANY},
                            ...
                            int inject_only, int cmd, uint32 flags)
{
    ...
    if (!found_mem) {
        /*Check for overlay memories*/
        for (i = 0; soc_ser_td2_overlay_mems[i].mem != INVALIDm; i++) {
            if (soc_ser_td2_overlay_mems[i].mem == mem) {
                    soc_ser_create_test_data(unit, tmp_entry, field_data,
                                soc_ser_td2_overlay_mems[i].parity_enable_reg,
                                SOC_INVALID_TCAM_PARITY_BIT,
                                soc_ser_td2_overlay_mems[i].parity_enable_field,
                                soc_ser_td2_overlay_mems[i].mem, EVEN_PARITYf,
