...
soc_dma_start_channel(int unit, sdc_t *sc)
{
    ...
    if (soc_feature(unit, soc_feature_cmicm)) {
        ...
        if (SOC_DMA_MODE(unit, sc->sc_channel) == SOC_DMA_MODE_CONTINUOUS) {
...
/*
 * Function:
 *      _soc_dma_start_reload (internal)
 * Purpose:
 *      Main part of soc_dma_start_reload function.
 ...
 */
STATIC void
_soc_dma_start_reload(int unit, sdc_t *sc, dv_t *dv_chain)
{
    uint32        soc_dma_lock;
    int           i;
    dv_t          *dv;
    ...
    /* Create a standalone start call for Continuous mode */
    if (sc->sc_dv_active == NULL) {     /* Start DMA if channel not active */
...
/*
 * Function:
 *      soc_dma_start_reload
 * Purpose:
 *      Launch a SOC_DMA DMA operation. Continuous mode only.
 * Parameters:
 *      unit - unit number.
 *      channel - DMA channel to use (0-3) or -1 if any
 *                      channel ok.
...
int
soc_dma_start_reload(int unit, dma_chan_t channel, dv_t *dv_chain)
{
    sdc_t       *sc;                    /* Channel pointer */
    ...
    _soc_dma_start_reload(unit, sc, dv_chain);
    return(SOC_E_NONE);
}
...
#if defined (BCM_CMICD_SUPPORT)
            sc = &soc->soc_channels[dv->dv_channel];
            if (SOC_DMA_MODE(unit, sc->sc_channel) == SOC_DMA_MODE_CONTINUOUS) {
                ...
                if(SOC_DCB_RELOAD_GET(unit, dcb)) {
                    ...
                    if (rld_dcb[DCB_LAST_DWORD] & DCB_DONE_BIT) {
                        ...
                        /* Done reload returns the next or newly active DV */
                        return (soc_dma_done_reload(unit, dv->dv_channel));
                    } else {
                        dv->dv_dcnt = i;
                        return(dv);
                    }
...
#if defined (BCM_CMICD_SUPPORT)
        uint32 mode = 0;
        /* API commitee if this will be configurable */
        mode = soc_property_get(unit, "pdma_mode", 0);
        if (mode == SOC_DMA_MODE_CONTINUOUS) {
            sc->sc_mode = SOC_DMA_MODE_CONTINUOUS;
        }
        else
#endif
        {
            sc->sc_mode = SOC_DMA_MODE_DEFAULT;
        }

        /* Initial state of channel */
        sc->sc_flags = 0;                   /* Clear flags to start */

        (void)soc_cmicm_intr0_disable(unit, IRQ_CMCx_DESC_DONE(c) |
                                      IRQ_CMCx_CHAIN_DONE(c));
...
#if defined (BCM_CMICD_SUPPORT)
            if (SOC_DMA_MODE(unit,c) == SOC_DMA_MODE_CONTINUOUS) {
                (void)soc_cmicm_intr0_enable(unit, IRQ_CMCx_DESC_DONE(c));
            } else
#endif
            {
                (void)soc_cmicm_intr0_enable(unit, IRQ_CMCx_DESC_DONE(c) |
