/*
 * $Id: phyctrl.c,v 1.160 Broadcom SDK $
...
int soc_phyctrl_software_reconfig(int unit, int cmic_port)
{
    int     port;
    ...
    for(port = 0; port < cmic_port; port++) {
        ...
        if (int_phy_ctrl[unit][port] != NULL) {
            sal_free(int_phy_ctrl[unit][port]);
            int_phy_ctrl[unit][port] = NULL;
        }
        if (ext_phy_ctrl[unit][port] != NULL) {
            sal_free(ext_phy_ctrl[unit][port]);
            ext_phy_ctrl[unit][port] = NULL;
        }
        sal_memset(&phy_port_info[unit][port], 0, sizeof(soc_phy_info_t));
    }

    PBMP_ALL_ITER(unit, port) {
        if (int_phy_ctrl[unit][port] != NULL) {
            int_phy_ctrl[unit][port]->flags &= (~PHYCTRL_INIT_DONE);
            int_phy_ctrl[unit][port]->block_init_flag = 0;
        }
    }
...
int soc_phyctrl_software_port_delete(int unit, soc_port_t port)
{
...
soc_phyctrl_software_init(int unit)
{
    ...
    if (!SOC_RECONFIG_TDM) {
        ...
        soc_sbx_caladan3_cmic_port_get(unit, &cmic_port);
        if (!SOC_HOTSWAP_TDM) {
            soc_phyctrl_software_reconfig(unit, cmic_port);
        }
    } /* !SOC_RECONFIG_TDM */
#endif
    ...
    PBMP_PORT_ITER(unit, port) {
#ifdef BCM_CALADAN3_SUPPORT
        if (SOC_RECONFIG_TDM && port >= cmic_port) {
            continue;
        }
#endif
...
soc_phyctrl_probe(int unit, soc_port_t port)
{
    ...
    int_pc.unit       = unit;
    int_pc.port       = port;
    int_pc.init_speed = SOC_INFO(unit).port_init_speed[port];
    int_pc.speed_max  = SOC_INFO(unit).port_speed_max[port];
    ext_pc.unit       = unit;
    ext_pc.port       = port;
    ext_pc.init_speed = SOC_INFO(unit).port_init_speed[port];
    ...
    if (SOC_IS_ROBO(unit)) {
        ...
        if (soc_feature(unit, soc_feature_sgmii_autoneg)) {
            if (SOC_IS_TBX(unit)){
                if (IS_GE_PORT(unit, port) && !IS_GMII_PORT(unit, port)){
                    /* for those GE port bounded with internal serdes 
                       * TB: S_PORT means 2.5G combo serdes port
...
soc_phyctrl_loopback_set(int unit, soc_port_t port, int enable, int serdes_linkup_wait)
{
    ...
    if (serdes_linkup_wait && !SOC_IS_DFE(unit) && !SOC_IS_TRIUMPH3(unit) &&
       !SOC_IS_ARAD(unit) && !SOC_IS_CALADAN3(unit)) { 
...
soc_phy_primary_and_offset_get(int unit, soc_port_t port, soc_port_t *primary_port, int *offset) 
{
    ...
    *primary_port = (primary_and_offset >> 8) & 0xff;
