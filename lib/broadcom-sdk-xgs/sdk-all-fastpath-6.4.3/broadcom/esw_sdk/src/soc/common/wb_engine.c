 ...
 * $Id: wb_engine.c,v 1.20 Broadcom SDK $
...
soc_wb_engine_var_set(int unit, int engine_id, int var_ndx, uint32 outer_arr_ndx, uint32 inner_arr_ndx, CONST uint8 *var)
{
    ...
    if (variables[var_ndx].data_orig) {
        ...
        dst = buffer->scache_ptr + variables[var_ndx].offset + (outer_arr_ndx * variables[var_ndx].outer_arr_jump) + (inner_arr_ndx * variables[var_ndx].inner_arr_jump);
    }
...
soc_wb_engine_var_get(int unit, int engine_id, int var_ndx, uint32 outer_arr_ndx, uint32 inner_arr_ndx, uint8 *var)
{
    ...
    if (buffer->is_only_copy) {
        src = buffer->scache_ptr + variables[var_ndx].offset + (outer_arr_ndx * variables[var_ndx].outer_arr_jump) + (inner_arr_ndx * variables[var_ndx].inner_arr_jump);
    }
    else{
...
static int
soc_wb_engine_calculate_offsets(int unit, int engine_id, uint32 buffer_id, uint16 version)
{
    soc_wb_engine_buffer_info_t *buffer;
    soc_wb_engine_var_info_t *variables;
    int var_ndx;
    ...
    for (var_ndx = 0; var_ndx < soc_wb_engine_nof_vars[unit][engine_id]; var_ndx++) {
        if (variables[var_ndx].buffer == buffer_id && 
            variables[var_ndx].version_added <= version && 
    ...
    if (buffer->is_dynamic == 1) {
        ...
        for (var_ndx = 0; var_ndx < soc_wb_engine_nof_vars[unit][engine_id]; var_ndx++) {
            ...
            if (variables[var_ndx].buffer == buffer_id && variables[var_ndx].is_dynamic == 1
                && variables[var_ndx].version_added <= version && variables[var_ndx].version_removed > version) {
                is_dynamic_count++;
            }
        }
    ...
    for (var_ndx = 0; var_ndx < soc_wb_engine_nof_vars[unit][engine_id]; var_ndx++) {
        ...
        if (variables[var_ndx].version_removed <= version) {
        ...
        if (variables[var_ndx].is_dynamic == 1 && variables[var_ndx].is_enabled == 0) {
            /* if var is dynamic but was never enabled - ignore it*/
            continue;
        }

        variables[var_ndx].offset = offset;
        nof_entries = variables[var_ndx].outer_arr_length * variables[var_ndx].inner_arr_length;
        offset = offset + (variables[var_ndx].data_size * nof_entries);
        offset = (offset + 4) & ~(0x3); /* next offset at a word boundary */
    }
    buffer->size = offset;
...
int
soc_wb_engine_restore(int unit, int engine_id, uint32 buffer_id, uint16 recovered_ver)
{
    soc_wb_engine_buffer_info_t *buffer;
    soc_wb_engine_var_info_t *variables;
    wb_engine_vars_info_header_t *vars_info_header;
    ...
    if (buffer->is_dynamic) {
        for (var_ndx = 0; var_ndx < soc_wb_engine_nof_vars[unit][engine_id]; var_ndx++) {
            if (variables[var_ndx].buffer == buffer_id && variables[var_ndx].is_dynamic == 1) {
                variables[var_ndx].offset = dynamic_vars_header[is_dynamic_count].offset;
                variables[var_ndx].flags = dynamic_vars_header[is_dynamic_count].flags;
                variables[var_ndx].data_size = dynamic_vars_header[is_dynamic_count].data_size;
                variables[var_ndx].outer_arr_length = dynamic_vars_header[is_dynamic_count].outer_arr_length;
                variables[var_ndx].inner_arr_length = dynamic_vars_header[is_dynamic_count].inner_arr_length;
                variables[var_ndx].outer_arr_jump = dynamic_vars_header[is_dynamic_count].outer_arr_jump;
                variables[var_ndx].inner_arr_jump = dynamic_vars_header[is_dynamic_count].inner_arr_jump;
                variables[var_ndx].is_enabled = dynamic_vars_header[is_dynamic_count].is_enabled;
                is_dynamic_count++;
            }
        }
    ...
    for (var_ndx = 0; var_ndx < soc_wb_engine_nof_vars[unit][engine_id]; var_ndx++) {
        ...
        if (variables[var_ndx].version_added > recovered_ver) {
            /* init with default value */
            if (variables[var_ndx].default_value != NULL) {
                for (outer_arr_ndx = 0; outer_arr_ndx < variables[var_ndx].outer_arr_length; outer_arr_ndx++) {
                    for(inner_arr_ndx = 0; inner_arr_ndx < variables[var_ndx].inner_arr_length; inner_arr_ndx++) {
                        src = variables[var_ndx].default_value;
                        dst = variables[var_ndx].data_orig + (outer_arr_ndx * variables[var_ndx].outer_arr_jump) + (inner_arr_ndx * variables[var_ndx].inner_arr_jump);
                        sal_memcpy(dst,src,variables[var_ndx].data_size);
                    }
                }
        ...
        if (variables[var_ndx].version_removed <= buffer->version) {
            continue;
        }
         ...
         */
        /* pointer to the offset of the variable's space in his buffer */
        data = buffer->scache_ptr + variables[var_ndx].offset;
        if (!buffer->is_only_copy) {
            for (outer_arr_ndx = 0; outer_arr_ndx < variables[var_ndx].outer_arr_length; outer_arr_ndx++) {
                for(inner_arr_ndx = 0; inner_arr_ndx < variables[var_ndx].inner_arr_length; inner_arr_ndx++) {
                    src = data + (outer_arr_ndx * variables[var_ndx].inner_arr_length * variables[var_ndx].data_size) + (inner_arr_ndx * variables[var_ndx].data_size);
                    dst = variables[var_ndx].data_orig + (outer_arr_ndx * variables[var_ndx].outer_arr_jump) + (inner_arr_ndx * variables[var_ndx].inner_arr_jump);
                    sal_memcpy(dst,src,variables[var_ndx].data_size);
                }
            }
...
void
soc_wb_engine_mem_print(uint8 *mem, int size)
{
    int i;
    for (i=0; i<size; i++) {
        LOG_CLI((BSL_META("%02X"), mem[i]));
    }
}

int
soc_wb_engine_dump(int unit, int engine_id, int flags, uint32 var_id, uint32 buffer_id, int print_orig_data)
{
    int                                rc = SOC_E_NONE;
    uint32                             buff_ndx;
    uint32                             var_ndx;
    ...
    LOG_CLI((BSL_META_U(unit,
                        "\n######################################################################################\n")));
    LOG_CLI((BSL_META_U(unit,
                        "# you are dumping a view of the data stored using wb_engine module.                  #\n")));
    LOG_CLI((BSL_META_U(unit,
                        "# for each buffer, youll see some of the buffer attributes followed by details on    #\n")));
    LOG_CLI((BSL_META_U(unit,
                        "# all of the buffer's variables, including their \"real\" value and their value in     #\n")));
    LOG_CLI((BSL_META_U(unit,
                        "# the scache buffer that is written to the external storage.                         #\n")));
    LOG_CLI((BSL_META_U(unit,
                        "# the variables printed value is not aware of the variable's type and is printed as  #\n")));
    LOG_CLI((BSL_META_U(unit,
                        "# a hex buffer                                                                       #\n")));
    LOG_CLI((BSL_META_U(unit,
                        "######################################################################################\n\n")));
    ...
    for (buff_ndx = 0; buff_ndx < soc_wb_engine_nof_buffers[unit][engine_id]; buff_ndx++) {
        ...
        else
        {
            ...
            LOG_CLI((BSL_META_U(unit,
                                "buffer's name - %s,\t"), buffers[buff_ndx].buff_string));
            LOG_CLI((BSL_META_U(unit,
                                "buffer's version - %d,\t"), buffers[buff_ndx].version));
            LOG_CLI((BSL_META_U(unit,
                                "buffer's size - %d,\t"), buffers[buff_ndx].size));
            LOG_CLI((BSL_META_U(unit,
                                "is dirty - %s\n"), (buffers[buff_ndx].dirty ? "yes" : "no")));
            if (buffers[buff_ndx].is_only_copy) {
            LOG_CLI((BSL_META_U(unit,
                                "data is stored exclusively on buffer (buffer does not contai copies of global variables)\n")));
            }
            LOG_CLI((BSL_META_U(unit,
                                "----------------------------------------\n")));
        }
        ...
        for (var_ndx = 0; var_ndx < soc_wb_engine_nof_vars[unit][engine_id]; var_ndx++) {
            ...
            if (variables[var_ndx].is_dynamic && (!variables[var_ndx].is_enabled)) {
                LOG_CLI((BSL_META_U(unit,
                                    "variable #%d (%s) - is a dynamic variable that was not yet created\n"), var_ndx, variables[var_ndx].var_string));
                LOG_CLI((BSL_META_U(unit,
                                    "---\n")));
            }

            LOG_CLI((BSL_META_U(unit,
                                "variable #%d (%s) - "), var_ndx, variables[var_ndx].var_string));
            LOG_CLI((BSL_META_U(unit,
                                "%s%s%s, "), (variables[var_ndx].is_dynamic ? "dynamic" : "static"), (variables[var_ndx].outer_arr_length > 1 ? " 2D" : ""), (variables[var_ndx].inner_arr_length > 1 ? " array" : " variable")));
            LOG_CLI((BSL_META_U(unit,
                                "offset in scache buffer = %d, size of one data unit = %d, added in version %d\n"), variables[var_ndx].offset, variables[var_ndx].data_size, variables[var_ndx].version_added));
            if (variables[var_ndx].version_removed != 0xff) {
                LOG_CLI((BSL_META_U(unit,
                                    "removed in version %d\n"), variables[var_ndx].version_removed));
            }
            ...
            for (outer_arr_ndx = 0; outer_arr_ndx < variables[var_ndx].outer_arr_length; outer_arr_ndx++) {
                for(inner_arr_ndx = 0; inner_arr_ndx < variables[var_ndx].inner_arr_length; inner_arr_ndx++) {
                    ...
                    LOG_CLI((BSL_META_U(unit,
                                        "%s - var_%d"), variables[var_ndx].var_string,var_ndx));
                    if (variables[var_ndx].outer_arr_length>1) {
                        LOG_CLI((BSL_META_U(unit,
                                            "[%d]"), outer_arr_ndx));
                    }
                    ...
                    if (!buffers[buff_ndx].is_only_copy && print_orig_data) {
                        LOG_CLI((BSL_META_U(unit,
                                            "variable's warmboot data at [%d][%d] - 0x"), outer_arr_ndx, inner_arr_ndx));
                        soc_wb_engine_mem_print(dst, variables[var_ndx].data_size);
                        LOG_CLI((BSL_META_U(unit,
                                            "\n")));
                        LOG_CLI((BSL_META_U(unit,
                                            "variable's original data at [%d][%d] - 0x"), outer_arr_ndx, inner_arr_ndx));
                        soc_wb_engine_mem_print(src, variables[var_ndx].data_size);
                        LOG_CLI((BSL_META_U(unit,
                                            "\n")));
                    }
                    else{
...
#ifdef BCM_WARM_BOOT_SUPPORT
    if (SOC_WARM_BOOT(unit) && (buffer->is_added_after_relase == SOC_WB_ENGINE_PRE_RELEASE)) {
        /* warmboot */
                        ...
                        if ((var_info_arr[nof_vars].nof_items_in_array != (variables[var_ndx].inner_arr_length * variables[var_ndx].outer_arr_length)) && !buffer->is_dynamic) {
                            LOG_ERROR(BSL_LS_SOC_COMMON,
                                      (BSL_META_U(unit,
                                       ...
                                       var_info_arr[nof_vars].nof_items_in_array, (variables[var_ndx].inner_arr_length * variables[var_ndx].outer_arr_length)));
                            return SOC_E_INTERNAL;
                        }
...
#endif /*BCM_WARM_BOOT_SUPPORT*/
     {
        ...
        for (var_ndx = 0; var_ndx < soc_wb_engine_nof_vars[unit][engine_id]; var_ndx++) {
            if (variables[var_ndx].buffer == buffer_id && 
                ...
                variables[var_ndx].version_removed > version) 
            {
                var_info_arr[nof_vars].data_size = variables[var_ndx].data_size;
                var_info_arr[nof_vars].nof_items_in_array = variables[var_ndx].inner_arr_length * variables[var_ndx].outer_arr_length;
                var_str_len = sal_strlen(variables[var_ndx].var_string);
                if (var_str_len < 128) {
                    /* coverity[secure_coding] */
                    sal_strcpy(var_info_arr[nof_vars].name, variables[var_ndx].var_string);
                }
                else
                {
                    LOG_ERROR(BSL_LS_SOC_COMMON,
