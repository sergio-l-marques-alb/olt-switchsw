...
#define NUM_LANES  4
STATIC int
_phy_84741_squelch_enable(int unit,soc_port_t port, int enable);
STATIC int
...
STATIC int phy_84756_fcmap_reg_write(int unit, soc_port_t port, uint32 flags,
                   uint32 phy_reg_addr, uint32 phy_data);
STATIC int
phy_84756_fcmap_squelch_enable(int unit, soc_port_t port, int intf, int enable);
STATIC int
...
phy_84756_fcmap_lb_set(int unit, soc_port_t port, int enable)
{
    ...
    if (PHY84756_IS_8474X(pc)) {
        ...
        SOC_IF_ERROR_RETURN
            (_phy_84741_squelch_enable(unit,port, enable ? FALSE : TRUE));
    }
                     ...
                     soc_phy_control_t type, uint32 value)
{
    ...
    case SOC_PHY_CONTROL_TX_LANE_SQUELCH:
        rv = phy_84756_fcmap_squelch_enable(unit, port, intf, value);
        break;
    default:
        rv = SOC_E_UNAVAIL;
        break;
...
#if defined(INCLUDE_FCMAP) || defined(INCLUDE_MACSEC)
STATIC int
bfcmap_phy84756_enable_macsec_block(phy_ctrl_t *pc)
{
    buint16_t data;
    ...
    if (data & 0x0010) {
        ...
        SOC_IF_ERROR_RETURN
             (BFCMAP_PHY84756_REG_RD(pc, 00, 0x1, 0xc800, &data));
        data &= (1 << 7);
        SOC_IF_ERROR_RETURN
             (BFCMAP_PHY84756_REG_MOD(pc, 00, 0x1, 0xc800,
                                      data? 0:(1 << 7), (1 << 7)));
                               ...
                               int phy_ext_boot)
{
    ...
    if ((!soc_property_port_get(pc->unit, pc->port, spn_FCMAP_ENABLE, 0)) &&
        ...
        if (soc_property_port_get(pc->unit, pc->port, spn_FCMAP_ENABLE, 0)) { 
            ...
            /* Enable MACSEC */
            SOC_IF_ERROR_RETURN(bfcmap_phy84756_enable_macsec_block(pc));
...
_phy_84741_squelch_enable(int unit,soc_port_t port, int enable)
{
    ...
    if (REVERSE_MODE(pc)) {  /* reverse mode */
        if (PHY84740_SINGLE_PORT_MODE(pc)) {
        ...
        if (PHY84740_SINGLE_PORT_MODE(pc)) {
            ...
            for (ix = 0; ix < NUM_LANES; ix++) {
                ...
                SOC_IF_ERROR_RETURN(
                    MODIFY_PHY84740_XFI_PMA_PMD_REG(unit, pc,
                    0xcd18,
                    data16, mask16));
...
phy_84741_init(int unit, soc_port_t port)
{
    ...
    if ((PHYCTRL_INIT_STATE(pc) == PHYCTRL_INIT_STATE_PASS2) ||
        ...
        /* Enable Squelch */
        SOC_IF_ERROR_RETURN
            (_phy_84741_squelch_enable(unit,port, TRUE));
...
STATIC int
phy_84741_lb_set(int unit, soc_port_t port, int enable)
{
    phy_ctrl_t *pc; 
    uint16     tmp;
    uint16     lane;
...
STATIC int
phy_84741_speed_set(int unit, soc_port_t port, int speed)
{
    phy_ctrl_t  *pc;
    phy_ctrl_t  *int_pc;
    int rv = SOC_E_NONE;
