...
struct dnx_fabric_s{
    ...
    uint32 blk_id; /* Used for mem access */
};
...
{   
    ...
    fabric_data->blk_id = pm_add_info->blk_id;
    ...
    if(!SOC_WARM_BOOT(unit)){
        ...
        {
            rv = SOC_WB_ENGINE_SET_ARR(unit, SOC_WB_ENGINE_PORTMOD, pm_info->wb_vars_ids[wb_polarity_rx], &info->polarity[i].rx_polarity, i); 
            SOCDNX_IF_ERR_EXIT(rv);
            rv = SOC_WB_ENGINE_SET_ARR(unit, SOC_WB_ENGINE_PORTMOD, pm_info->wb_vars_ids[wb_polarity_tx], &info->polarity[i].tx_polarity, i);
            SOCDNX_IF_ERR_EXIT(rv);
        }
    }
exit:
...
int dnx_fabric_core_init_phase1(int unit, int port, pm_info_t pm_info, const portmod_port_add_info_t* add_info)
{
    ...
    if(!PHYMOD_CORE_INIT_F_RESUME_AFTER_FW_LOAD_GET(add_info)) {
        ...
        /* Probe */
        SOCDNX_IF_ERR_EXIT(phymod_core_probe(&fabric_data->access, &(fabric_data->core_type)));
    ...
    /* Init Core */
    SOCDNX_IF_ERR_EXIT(dnx_fabric_port_core_access_get(unit, port, pm_info, 0, 1, &core_access, &cores_num));
...
int dnx_fabric_port_attach(int unit, int port, pm_info_t pm_info, const portmod_port_add_info_t* add_info)
{
    ...
    SOCDNX_IF_ERR_EXIT(rv);
    if(!is_core_probed || PORTMOD_PORT_ADD_F_INIT_RESUME_AFTER_FW_LOAD_GET(add_info)){ 
    ...
    params.phyn = 0;
    SOCDNX_IF_ERR_EXIT(dnx_fabric_port_phy_lane_access_get(unit, port, pm_info, &params, 1, &phy_access, &phys));
    ...
    SOCDNX_IF_ERR_EXIT(dnx_fabric_port_interface_config_set(unit, port, pm_info , &add_info->interface_config));
...
int dnx_fabric_port_enable_set(int unit, int port, pm_info_t pm_info, int flags, int enable)
{
    ...
    /*get phy info in case we touch the phy*/
    SOCDNX_IF_ERR_EXIT(dnx_fabric_port_phy_lane_access_get(unit, port, pm_info, &params, 1, &phy_access, &phys));
...
int dnx_fabric_port_enable_get(int unit, int port, pm_info_t pm_info, int flags, int *enable)
{
    ...
    if(PORTMOD_PORT_ENABLE_PHY_GET(flags)){
        ...
        if(PORTMOD_PORT_ENABLE_RX_GET(flags)){
            *enable = (phy_power.rx == phymodPowerOn)? 1 : 0;
        }
        else{ /*tx*/
            *enable = (phy_power.tx == phymodPowerOn)? 1 : 0;
        }
    }
    else{/*MAC*/
        SOCDNX_IF_ERR_EXIT(dnx_fabric_port_phy_index_get(unit, port, &phy_index));
...
int dnx_fabric_port_interface_config_set(int unit, int port, pm_info_t pm_info, const portmod_port_interface_config_t* config)
{
    ...
    SOCDNX_IF_ERR_EXIT(dnx_fabric_port_phy_lane_access_get(unit, port, pm_info, &params, 1 , &phy , &phys_num));
    if(phys_num != 1){
    ...
    flags = 0;
    PORTMOD_PORT_ENABLE_PHY_SET(flags);
    SOCDNX_IF_ERR_EXIT(dnx_fabric_port_enable_get(unit, port, pm_info, flags, &phy_enabled));
    flags = 0;
    PORTMOD_PORT_ENABLE_MAC_SET(flags);
    SOCDNX_IF_ERR_EXIT(dnx_fabric_port_enable_get(unit, port, pm_info, flags, &mac_enabled));
    ...
    flags = 0;
    PORTMOD_PORT_ENABLE_MAC_SET(flags);
    SOCDNX_IF_ERR_EXIT(dnx_fabric_port_enable_set(unit, port, pm_info, flags, mac_enabled));
    flags = 0;
    PORTMOD_PORT_ENABLE_PHY_SET(flags);
    SOCDNX_IF_ERR_EXIT(dnx_fabric_port_enable_set(unit, port, pm_info, flags, phy_enabled));
...
int dnx_fabric_port_loopback_set(int unit, int port, pm_info_t pm_info, portmod_loopback_mode_t loopback_type, int enable)
{
    ...
    /*loopback set*/
    if(is_phy_loopback){
        SOCDNX_IF_ERR_EXIT(portmod_common_phy_loopback_set(unit, port, loopback_type, enable));
    } else { /*MAC looopback*/
...
int dnx_fabric_port_loopback_get(int unit, int port, pm_info_t pm_info, portmod_loopback_mode_t loopback_type, int* enable)
{
    ...
    case portmodLoopbackPhyGloopPMD:
    case portmodLoopbackPhyRloopPMD: /*slide*/
        SOCDNX_IF_ERR_EXIT(portmod_common_phy_loopback_get(unit, port, loopback_type, enable));
        break;
    /*PHY pcs modes and MAC core are not supported*/
    default:
        SOCDNX_EXIT_WITH_ERR(SOC_E_UNAVAIL, (_BSL_SOCDNX_MSG("unsupported loopback type %d"), loopback_type));
...
int dnx_fabric_port_prbs_config_set(int unit, int port, pm_info_t pm_info, int mode, int flags, const phymod_prbs_t* config)
{
    uint32 reg_val;
    uint32 phy_index;
    uint32 poly_val = 0;
    ...
    if(mode == 0){ /*phy*/
        SOCDNX_IF_ERR_EXIT(portmod_common_phy_prbs_config_set(unit, port, flags, config));
    }
    else{
        switch(config->poly){
...
int dnx_fabric_port_prbs_config_get(int unit, int port, pm_info_t pm_info, int mode, int flags, phymod_prbs_t* config)
{
    uint32 reg_val;
    uint32 tx_poly;
    uint32 poly_val = 0; 
    ...
    if(mode == 0){ /*phy*/
        SOCDNX_IF_ERR_EXIT(portmod_common_phy_prbs_config_get(unit, port, flags, config));
    }
    else{
        SOCDNX_IF_ERR_EXIT(dnx_fabric_port_phy_index_get(unit, port, &phy_index));
        if(PHYMOD_PRBS_DIRECTION_RX_GET(flags) || (flags == 0)){
...
int dnx_fabric_port_prbs_enable_set(int unit, int port, pm_info_t pm_info, int mode, int flags, int enable)
{
    uint32 reg_val;
    uint32 phy_index;
    dnx_fabric_t fabric_data = pm_info->pm_data.dnx_fabric;
    ...
    if(mode == 0){ /*phy*/
        SOCDNX_IF_ERR_EXIT(portmod_common_phy_prbs_enable_set(unit, port, flags, enable));
    }
    else{
        SOCDNX_IF_ERR_EXIT(dnx_fabric_port_phy_index_get(unit, port, &phy_index));
        if(PHYMOD_PRBS_DIRECTION_RX_GET(flags) || (flags == 0)){
...
int dnx_fabric_port_prbs_enable_get(int unit, int port, pm_info_t pm_info, int mode, int flags, int* enable)
{
    uint32 reg_val;
    int tx_enable;
    uint32 phy_index;
    ...
    if(mode == 0){ /*phy*/
        SOCDNX_IF_ERR_EXIT(portmod_common_phy_prbs_enable_get(unit, port, flags, enable));
    }
    else{
        SOCDNX_IF_ERR_EXIT(dnx_fabric_port_phy_index_get(unit, port, &phy_index));
        if(PHYMOD_PRBS_DIRECTION_RX_GET(flags) || (flags == 0)){
...
int dnx_fabric_port_prbs_status_get(int unit, int port, pm_info_t pm_info, int mode, int flags, phymod_prbs_status_t* status)
{
    uint32 reg_val;
    uint32 phy_index;  
    dnx_fabric_t fabric_data = pm_info->pm_data.dnx_fabric;
    ...
    if(mode == 0){ /*phy*/
        SOCDNX_IF_ERR_EXIT(portmod_common_phy_prbs_status_get(unit, port, flags, status));
    } else {
        SOCDNX_IF_ERR_EXIT(dnx_fabric_port_phy_index_get(unit, port, &phy_index));
...
int
dnx_fabric_port_phy_lane_access_get(int unit, int port, pm_info_t pm_info, const portmod_access_get_params_t* params, int max_phys, phymod_phy_access_t* phy_access_arr, int* nof_phys)
{
    dnx_fabric_t fabric_data = pm_info->pm_data.dnx_fabric;
    uint32 phy_index;
    SOCDNX_INIT_FUNC_DEFS;
...
int
dnx_fabric_port_core_access_get(int unit, int port, pm_info_t pm_info, int phyn, int max_cores, phymod_core_access_t* core_access_arr, int* cores_num)
{
    dnx_fabric_t fabric_data = pm_info->pm_data.dnx_fabric;
    SOCDNX_INIT_FUNC_DEFS;
