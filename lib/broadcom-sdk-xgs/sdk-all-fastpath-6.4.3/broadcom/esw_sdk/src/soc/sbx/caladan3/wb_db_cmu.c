...
int soc_sbx_caladan3_wb_cmu_sync(int unit, int arg)
{
   ...
   if (arg == 0) {
       ...
       _SOC_IF_ERR_EXIT(WRITE_CM_MANUAL_EJECT_RATEr(unit, 10000));
       ...
       for (nSegment=0; nSegment < SOC_SBX_CALADAN3_CMU_NUM_SEGMENT; nSegment++) {
           if (pCmuCfg->segments[nSegment].bEnabled) { 
               ...
               /* Sleep 20ms for thread to process */
               sal_usleep(20000);
               /* Disable segment */
       ...
       /* Disable CMIC ch0 */
       _SOC_IF_ERR_EXIT(_soc_mem_sbus_fifo_dma_stop(unit, 0));

       /* disable CMU ejection_fifo_ready interrupt */
        soc_reg_field_set(unit, CM_INTERRUPT_STATUSr, &uRegValue, EJECTION_FIFO_READYf, 0); 

        soc_cmicm_intr3_disable(unit, (1<<SOC_SBX_CALADAN3_CM_INTR_POS));

        soc_cmicm_intr0_disable(unit, IRQ_CMCx_FIFO_CH_DMA(SOC_MEM_FIFO_DMA_CHANNEL_0));
...
_soc_sbx_caladan3_wb_cmu_state_restore(int unit)
{
    ...
    case SOC_SBX_CALADAN3_WB_CMU_VERSION_1_0:
        ...
        for (nSegment=0; nSegment < SOC_SBX_CALADAN3_CMU_NUM_SEGMENT; nSegment++) {
            pSegCfg = &pCmuCfg->segments[nSegment];
            if (pSegCfg->bEnabled != SOC_SBX_API_PARAM_NO_CHANGE) {
                _SOC_IF_ERR_EXIT(READ_CM_SEGMENT_ENABLEr(unit, &uRegValue));
                if (pSegCfg->bEnabled) {
                    uRegValue |= (1 << nSegment);
                } else {
                    uRegValue &= (~(1 << nSegment));
                }
                SOC_CALADAN3_ALLOW_WARMBOOT_WRITE(WRITE_CM_SEGMENT_ENABLEr(unit, uRegValue), rv);
                _SOC_IF_ERR_EXIT(rv);
            }           
        }
        ...
        rv = soc_sbx_caladan3_cmu_ring_process_thread_start(unit, flags, pCmuCfg->ring.interval);
        if (SOC_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_SOC_STAT,
                      (BSL_META_U(unit,
                                  "Failed to start CMU ring process thread\n")));
            _SOC_IF_ERR_EXIT(rv);
        }
        ...
        rv = soc_cmicm_intr3_enable(unit, (1<<SOC_SBX_CALADAN3_CM_INTR_POS));
        if (SOC_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_SOC_STAT,
                      (BSL_META_U(unit,
                                  "%s Failed to enable CMU block interrupt\n"),
                       FUNCTION_NAME()));
            _SOC_IF_ERR_EXIT(rv);
        }
        
        rv = soc_cmicm_intr0_enable(unit, IRQ_CMCx_FIFO_CH_DMA(SOC_MEM_FIFO_DMA_CHANNEL_0));
        if (SOC_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_SOC_STAT,
                      (BSL_META_U(unit,
                                  "%s Failed to enable FIFO DMA interrupt\n"),
                       FUNCTION_NAME()));
            _SOC_IF_ERR_EXIT(rv);
        }
