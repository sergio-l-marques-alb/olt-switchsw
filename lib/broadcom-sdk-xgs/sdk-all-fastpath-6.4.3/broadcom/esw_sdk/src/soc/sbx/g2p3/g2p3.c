...
/*
 * $Id: g2p3.c,v 1.148 Broadcom SDK $
...
#include <soc/sbx/g2p3/c2ss_g2p3_mplstp_ucode.h>
#include <soc/sbx/g2p3/c1_g2p3_ucode.h>
...
_g2p3_isr(void *_unit, uint32 ireg)
{
    ...
    if ( interrupt & (SAND_HAL_FE2000_FIELD_MASK(sbx->sbhdl, PC_INTERRUPT_MASK, DMA_0_DONE_DISINT)
        ...
        LOG_VERBOSE(BSL_LS_SOC_INTR,
                    (BSL_META_U(unit,
                                "%d:g2p3_isr: calling sbFe2000DmaIsr\n"),
                     unit));
    ...
    if (SOC_IRQ_MASK(unit) & (1 << SOC_SBX_FE2000_ISR_TYPE_CMU)) {
        LOG_VERBOSE(BSL_LS_SOC_INTR,
                    (BSL_META_U(unit,
                                "%d:g2p3_isr: calling sbFe2000CmuRingProcess\n"),
                     unit));
...
_soc_sbx_g2p3_commons_init(int unit)
{
    ...
    uint32 uHppFreq;
    uint32 uIgmpProxyMode;
    int wb = 0;
    void *cls_wb_data = NULL;
    uint32 ach_type = 0;
    soc_sbx_g2p3_ach_channel_type_t ach_entry;
    int master_unit;
    soc_sbx_g2p3_state_t *master_fe;
    ...
    /* Allow custom override of MPLSTP ACH Channel types */
    if (SOC_SBX_MPLSTP_ENABLE(unit)) {
        soc_sbx_g2p3_ach_channel_type_t_init(&ach_entry);
        ach_type = soc_property_get(unit, spn_SBX_MPLSTP_MCC_TYPE, 0x1);
        if (ach_type != 0x1) {
            /* re-program */
            ach_entry.mcc = ach_type;
        }
        ach_type = soc_property_get(unit, spn_SBX_MPLSTP_SCC_TYPE, 0x2);
        if (ach_type != 0x2) {
            /* re-program */
            ach_entry.scc = ach_type;
        }
        ach_type = soc_property_get(unit, spn_SBX_MPLSTP_PSC_TYPE, 0x7ff9);
        if (ach_type != 0x7ff9) {
            /* re-program */
            ach_entry.psc = ach_type;
        }
        ach_type = soc_property_get(unit, spn_SBX_MPLSTP_LSPPING_TYPE, 0x30);
        if (ach_type != 0x30) {
            /* re-program */
            ach_entry.ping = ach_type;
        }
        ach_type = soc_property_get(unit, spn_SBX_MPLSTP_IPBFD_TYPE, 0x21);
        if (ach_type != 0x21) {
            /* re-program */
            ach_entry.ipbfd = ach_type;
        }
        ach_type = soc_property_get(unit, spn_SBX_MPLSTP_RAWBFD_TYPE, 0x7);
        if (ach_type != 0x7) {
            /* re-program */
            ach_entry.rawbfd = ach_type;
        }
        rv = soc_sbx_g2p3_ach_channel_type_set(unit, 0, &ach_entry);
        if (BCM_FAILURE(rv)) {
            LOG_CLI((BSL_META_U(unit,
                                "\nError setting ACH channel types, using defaults")));
        }
    }
...
_soc_sbx_g2p3_sdk_overrides(int unit)
{
    ...
    tm->policerbanks[SOC_SBX_G2P3_XTPOL_ID].kbps = 
        soc_property_get(unit, "policer_exception_base_rate",
                         tm->policerbanks[SOC_SBX_G2P3_XTPOL_ID].kbps);
    ...
    if (tm->policerbanks[SOC_SBX_G2P3_XTPOL_ID].kbps ==
        tm->policerbanks[SOC_SBX_G2P3_INGEGRPOL_ID].kbps) {
        LOG_CLI((BSL_META_U(unit,
                            "ERROR: Not supported: exception and application policers"
...
soc_sbx_g2p3_drv_hw_init(int unit)
{
    ...
    if (SOC_IS_SBX_FE2KXT(unit)) {
	    ...
	    } else if (sal_strcmp(ucodestr, "c2ssmplstp") == 0) {
                if (SOC_SBX_LOAD_UCODE(soc_sbx_c2ss_g2p3_mplstp_fe2kAsm2UcodeEc) != 0) {
                    return SOC_E_FAIL;
                }
            } else {
        ...
        if (ucodestr && (sal_strcmp(ucodestr, "c1_g2p3_mplstp") == 0)) {
            if (SOC_SBX_LOAD_UCODE(soc_sbx_c1_g2p3_fe2kAsm2UcodeEc) != 0) {
                return SOC_E_FAIL;
            }
        } else {
...
soc_sbx_g2p3_drv_init(int unit)
{
    ...
    if (SOC_IS_SBX_FE2KXT(unit)) {
        ...
        if (ucodestr){
            if (sal_strcmp(ucodestr, "c2_g2p3") == 0) {
                if (soc_sbx_g2p3_v6_ena(unit)) {
                    LOG_CLI((BSL_META_U(unit,
                                        "soc_sbx_fe2000xt_init() IPV6 Not Supported in ucode\n")));
                    return SOC_E_FAIL;
                }
                if (!soc_sbx_g2p3_mim_ena(unit)) {
                    LOG_CLI((BSL_META_U(unit,
                                        "soc_sbx_fe2000xt_init() MiM Not Enabled\n")));
                }
                if (SOC_SBX_LOAD_UCODE(soc_sbx_c2_g2p3_fe2kAsm2UcodeEc) != 0) {
                    return SOC_E_FAIL;
                }
                ...
                if (soc_sbx_g2p3_mim_ena(unit)) {
                    LOG_CLI((BSL_META_U(unit,
                                        "soc_sbx_fe2000xt_init() MiM Not Supported in ucode\n")));
                    return SOC_E_FAIL;
                }
                if (!soc_sbx_g2p3_v6_ena(unit)) {
                    LOG_CLI((BSL_META_U(unit,
                                        "soc_sbx_fe2000xt_init() IPv6 Not Enabled\n")));
                }
                ...
                LOG_CLI((BSL_META_U(unit,
                                    "soc_sbx_fe2000xt_init() Unsupported ucode %s detected\n"), ucodestr));
                return SOC_E_FAIL;
            }
        } else {
            if (SOC_SBX_LOAD_UCODE(soc_sbx_c2_g2p3_fe2kAsm2UcodeEc) != 0) {
	    ...
	    } else if (sal_strcmp(ucodestr, "c1_g2p3_mplstp") == 0) {
                if (SOC_SBX_LOAD_UCODE(soc_sbx_c1_g2p3_fe2kAsm2UcodeEc) != 0) {
                    return SOC_E_FAIL;
                }
            } else {
                LOG_CLI((BSL_META_U(unit,
                                    "soc_sbx_fe2000_init() Unsupported ucode detected\n")));
                return SOC_E_FAIL;
	    }
        } else {
            if (SOC_SBX_LOAD_UCODE(soc_sbx_g2p3_20g_fe2kAsm2UcodeEc) != 0) {
    ...
    if (SOC_IS_SBX_FE2KXT(unit)) {
        ...
        if (ucodestr && 
	    (sal_strcmp(ucodestr, "c2mplstp") == 0 ||
	     sal_strcmp(ucodestr, "c2ssmplstp") == 0)) {
    ...
    if (SOC_SBX_CFG(unit)->v4_ena) {
        ...
        } else if((!SOC_IS_SBX_FE2KXT(unit)) && SOC_SBX_CFG(unit)->mplstp_ena) {
            SOC_IF_ERROR_RETURN
                (soc_sbx_g2p3_oi2e_table_size_set(unit, (72 * 1024)));
            SOC_IF_ERROR_RETURN
                (soc_sbx_g2p3_ft_table_size_set(unit, (176 * 1024)));
        }
    ...
    if((!SOC_IS_SBX_FE2KXT(unit)) && SOC_SBX_CFG(unit)->mplstp_ena) {
            SOC_IF_ERROR_RETURN
                (soc_sbx_g2p3_oi2e_table_size_set(unit, (72 * 1024)));
            SOC_IF_ERROR_RETURN
                (soc_sbx_g2p3_ft_table_size_set(unit, (192 * 1024)));

            SOC_IF_ERROR_RETURN
                (soc_sbx_g2p3_mac_table_size_set(unit, (300 * 1024)));
    }
     ...
     * Must update the `efficiency' globals.  We always do this,
    ...
    if (SOC_IS_SBX_FE2KXT(unit)) {
        ...
        if (ucodestr && (sal_strcmp(ucodestr, "c1_g2p3_mplstp") == 0)) {
            SOC_IF_ERROR_RETURN
                (soc_sbx_g2p3_oamepremap_c1_table_size_get(unit, 
                                                          (uint*) &size));
            SOC_IF_ERROR_RETURN
                (soc_sbx_g2p3_oamepremap_table_size_set(unit, size));
        }
