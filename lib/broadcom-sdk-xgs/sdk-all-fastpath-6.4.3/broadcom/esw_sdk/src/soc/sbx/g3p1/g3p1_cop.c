...
static uint32 cop_ocm_memory[2] = {131712, 65536 }; /* this is calculated by ASM3 */
...
static int init_sequencer_segment_oamseq(int unit) {
  ...
  rv = soc_sbx_caladan3_cop_segment_register(unit, 0, 3,
          128, SOC_SBX_CALADAN3_COP_SEGMENT_TYPE_SN_CHECKER, &config);
  if (SOC_FAILURE(rv)) {
    LOG_CLI((BSL_META_U(unit,
  ...
  sequencer_segment_desc[COP_SEQUENCER_SEGMENT_OAMSEQ].num_entries = 128;
...
static int init_coherent_table_segment_coherent(int unit) {
  ...
  rv = soc_sbx_caladan3_cop_segment_register(unit, 0, 4,
            256, SOC_SBX_CALADAN3_COP_SEGMENT_TYPE_COHERENT, &config);
  if (SOC_FAILURE(rv)) {
    LOG_CLI((BSL_META_U(unit,
  ...
  coherent_table_segment_desc[COP_COHERENT_TABLE_SEGMENT_COHERENT].num_entries = 256;
