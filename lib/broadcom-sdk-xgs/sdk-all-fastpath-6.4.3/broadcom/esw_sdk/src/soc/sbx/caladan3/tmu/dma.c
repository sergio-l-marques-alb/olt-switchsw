...
int soc_dma_reg_field_set(int unit, soc_reg_t reg, uint32 *regval,
    ...
    soc_dma_reg_field_set(unit, reg, regval, field, value);
    return SOC_E_NONE;
}
...
int tmu_resp_dma_mgr_init(int unit, int fifoid)
{
    ...
    if (SOC_SUCCESS(status)) {
        ...
        SOC_CALADAN3_ALLOW_WARMBOOT_WRITE_NO_ERR(soc_dma_reg_field_set(unit, TMB_UPDATER_RSP_FIFO_INACTIVITY_THRESHr, 
                                                                   &regval, THRESHOLDf, 
                                                                   SOC_SBX_CALADAN3_TMU_DEF_RESP_TRAILER_THRESHOLD), 
                                                 status);
        SOC_CALADAN3_ALLOW_WARMBOOT_WRITE(WRITE_TMB_UPDATER_RSP_FIFO_INACTIVITY_THRESHr(unit, fifoid, regval), status);
        if (SOC_FAILURE(status)) {
            TMU_DMA_UNLOCK(unit);
            return status;
        }
...
#if 1
        TMU_FIFO_RESP_MGR(unit,fifoid).ring_pointer = &TMU_FIFO_RESP_MGR(unit,fifoid).dma_buffer[0];
        SOC_CALADAN3_ALLOW_WARMBOOT_WRITE(soc_mem_fifo_dma_start(unit, channels[fifoid],
