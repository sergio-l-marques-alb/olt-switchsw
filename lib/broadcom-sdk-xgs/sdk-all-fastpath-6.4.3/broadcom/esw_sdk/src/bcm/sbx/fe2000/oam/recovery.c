/*
 * $Id: recovery.c,v 1.15 Broadcom SDK $
...
_oam_wb_maidmep_scan(int unit)
{
    ...
    while (BCM_SUCCESS(rv)) {
        ...
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                                  "failed to read oammaidmep[0x%x, 0x%x]:"
...
_oam_wb_recover_ep(int unit, int group, int lsmi, int ep_idx)
{
    ...
    if (oam_ep.nextentry) {
        ...
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                                  "Failed to read next entry [0x%x]:%s\n"),
        ...
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                                  "Failed to reserve extented ep 0x%x: %s\n"),
                       oam_ep.nextentry, bcm_errmsg(rv)));
            return rv;
        }
    }
    ...
    if (oam_ep.function == _state[unit]->ep_type_ids.bfd) {
        ...
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "Extended local endpoint found as root node\n")));
        rv = BCM_E_INTERNAL;
        ...
        if (oam_ep.policerid != INVALID_POLICER_ID) {
            ...
            if (BCM_FAILURE(rv)) {
                LOG_ERROR(BSL_LS_BCM_OAM,
                          (BSL_META_U(unit,
                                      "Failed to clear timer calendar for 0x%x "
                                       ": %s (ignoring)\n"), 
...
_oam_wb_up_mep_recover(int unit)
{
    ...
    for (dmac_type=0; dmac_type < 2; dmac_type++) {
        ...
        if (fti == SBX_DROP_FTE(unit)) {
            LOG_VERBOSE(BSL_LS_BCM_OAM,
                        (BSL_META_U(unit,
                                    "No UP meps configured; skipping this "
                                     "phase of recovery")));
            return BCM_E_NONE;
        }
        ...
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                                  "Failed to reserve oi2e[0x%08x]: %s\n"),
        ...
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                                  "Failed to reserve eteencap[0x%08x]: %s\n"),
        ...
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                                  "Failed to reserve ete[0x%08x]: %s\n"),
        ...
        if (local_data->rec_idx && 
            ...
            if (local_data->flags & BCM_OAM_ENDPOINT_UP_FACING) {
                ...
                if (BCM_FAILURE(rv)) {
                    LOG_ERROR(BSL_LS_BCM_OAM,
                              (BSL_META_U(unit,
                                          "Failed to map qid 0x%x: %s\n"),
                               local_data->gport, bcm_errmsg(rv)));
                    return rv;
                }
