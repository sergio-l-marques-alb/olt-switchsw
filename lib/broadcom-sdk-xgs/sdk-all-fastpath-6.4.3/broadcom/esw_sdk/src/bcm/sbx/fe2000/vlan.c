/*
 * $Id: vlan.c,v 1.401 Broadcom SDK $
 ...
 */
unsigned int _bcm_fe2k_max_vlan_gports;
/*
...
extern int _bcm_fe2000_qos_map_validate(int unit,
                                        int qosMapId,
                                        uint32 *qosMapRaw);
int _bcm_fe2000_is_vswitch_inuse(int unit, bcm_vlan_t vsi);
int bcm_fe2000_mpls_vpn_id_get(int                    unit,
                           bcm_vpn_t              vpn,
...
void _vlan_lock_take(int unit){
  if (_vlan_state_lock[unit]) sal_mutex_take(_vlan_state_lock[unit], sal_mutex_FOREVER);
}

void _vlan_lock_release(int unit){
  if (_vlan_state_lock[unit]) sal_mutex_give(_vlan_state_lock[unit]);
}
...
_bcm_fe2000_vlan_dump_state(int unit) 
{
    ...
    LOG_INFO(BSL_LS_BCM_VLAN,
             (BSL_META_U(unit,
                         "Port nativeVid   nvflags   divergence\n")));
    LOG_INFO(BSL_LS_BCM_VLAN,
             (BSL_META_U(unit,
                         "----+---------+-----------+-----------\n")));
                         ...
                         soc_sbx_g2p3_pv2e_t *e)
{
    ...
    if (divergence & _FE2K_VLAN_DIVERGE_ALL) {
        ...
        if (SOC_SBX_CFG(unit)->mplstp_ena && e->vpws) {
            lpiSrc = e->lpi + _BCM_FE2000_VPWS_UNI_OFFSET;
        } else {
        ...
        if (SOC_SBX_CFG(unit)->mplstp_ena && pv2e.vpws ) {
            lpi    = pv2e.lpi;
            lpiDst = lpi + _BCM_FE2000_VPWS_UNI_OFFSET;
        } else {
    ...
    if (0 != _vlan_state[unit]->nvid[port]) {
        ...
        if (PBMP_MEMBER(_vlan_state[unit]->tag_drop, port)) {
            ...
            LOG_VERBOSE(BSL_LS_BCM_VLAN,
                        (BSL_META_U(unit,
                                    "port %d in drop_tag mode, force vlan to"
                                     "nativeVid:%03X\n"), port, pv2e.vlan));
    ...
    if (BCM_E_NONE == result) {
        ...
        if (SOC_IS_SBX_G2P3(unit)) {
            ...
            if (BCM_E_NONE != result) {
                LOG_ERROR(BSL_LS_BCM_VLAN,
                          (BSL_META_U(unit,
                                      "unable to write to %d:eteL2[%08X]:"
                                       " %d (%s)\n"),
                           unit,
                           eteNum,
                           result,
    ...
    if (BCM_E_NONE == result) {
        ...
        if (BCM_E_NONE == result) {
            ...
            if (BCM_E_NONE != result) {
                ...
                LOG_ERROR(BSL_LS_BCM_VLAN,
                          (BSL_META_U(unit,
                                      "unable to free old %d:eteL2[%08X]:"
                                       " %d (%s)\n"),
                           unit,
                           eteOld,
                           result,
            ...
            if (BCM_E_NONE != result) {
                ...
                LOG_ERROR(BSL_LS_BCM_VLAN,
                          (BSL_META_U(unit,
                                      "unable to free new %d:eteL2[%08X]:"
                                       " %d (%s)\n"),
                           unit,
                           eteNum,
                           result,
                          ...
                          const unsigned int untagged)
{
    ...
    if (BCM_E_NONE == result) {
        ...
        if (BCM_E_NONE != result) {
            LOG_ERROR(BSL_LS_BCM_VLAN,
                      (BSL_META_U(unit,
                                  "_bcm_fe2000_stg_vlan_port_add(%d,%04X,%d)"
                                   " returned %d (%s)\n"),
                       unit,
                       SBX_VSI_FROM_VID(vid),
                       port,
                                   ...
                                   uint32 *portQueue)
{
    ...
    if (BCM_E_NONE == result) {
        ...
        case BCM_GPORT_TYPE_TRUNK:
            trunkInfo = sal_alloc(sizeof(bcm_trunk_add_info_t), "trunkInfo");
            if (trunkInfo == NULL) {
                LOG_ERROR(BSL_LS_BCM_VLAN,
                          (BSL_META_U(unit,
                                      "unable to alloc %d bytes for"
                                       " unit %d trunkInfo\n"),
                           sizeof(bcm_trunk_add_info_t),unit));
                result = BCM_E_MEMORY;
                break;
            }
            ...
            doFtLookup = FALSE;
            result = bcm_fe2000_trunk_get_old(unit, BCM_GPORT_TRUNK_GET(gport),
                                   trunkInfo);
            if (BCM_FAILURE(result)) {
    ...
    *allocEteEncap = *allocEteL2 = FALSE;
    ...
    if ((vlan_port->e.flags & _BCM_FE2K_VLAN_GP_EXTERN_OHI) ||
        ...
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_VLAN,
                      (BSL_META_U(unit,
                                  "unable to read %d:eteencap[0x%08X]:"
                                   " %d (%s)\n"),
        ...
        *allocEteEncap = TRUE;
        ...
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_VLAN,
                      (BSL_META_U(unit,
                                  "unable to allocate ETE L2 entry on"
                                   " unit %d: %d (%s)\n"),
                       unit, rv, bcm_errmsg(rv)));
            return rv;
        }
    ...
    LOG_VERBOSE(BSL_LS_BCM_VLAN,
                (BSL_META_U(unit,
                            "%s ETEEncap 0x%08x ETE L2 0x%08x\n"),
                             ...
                             uint32 pid)
{
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_VLAN,
                  (BSL_META_U(unit,
                              "unable to write %d:etel2[0x%08X]: %d (%s)\n"),
                   unit, eteL2Index, rv, bcm_errmsg(rv)));
        return rv;
    }
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_VLAN,
                  (BSL_META_U(unit,
                              "unable to write %d:eteEncap[0x%08X]: "
                               "%d (%s)\n"),
                   unit, eteEncapIndex, rv, bcm_errmsg(rv)));
        return rv;
    }
    ...
    LOG_VERBOSE(BSL_LS_BCM_VLAN,
                (BSL_META_U(unit,
                            "Updated OI[0x%08x] ETEEncap[0x%08x] "
                             "ETE L2 [0x%08x]\n"),
                                      ...
                                      uint32 pid)
{
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_VLAN,
                  (BSL_META_U(unit,
                              "unable to read failover %d:eteEncap[0x%08X]:"
                               " %d (%s)\n"),
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_VLAN,
                  (BSL_META_U(unit,
                              "unable to write failover %d:eteEncap[0x%08X]"
                               ": %d (%s)\n"),
                   unit, eteEncapIdx, rv, bcm_errmsg(rv)));
        return rv;
    }
                                  ...
                                  uint32 logicalPort)
{
    ...
    LOG_VERBOSE(BSL_LS_BCM_VLAN,
                (BSL_META_U(unit,
                            "LP=0x%08X FTE=0x%08X OI=0x%08X ETEEC=0x%08X"
                             " ETEL2=0x%08X u=%d\n"),
                                  ...
                                  bcm_gport_t gport)
{
    ...
    if (BCM_GPORT_IS_TRUNK(vlanPortData->p.port)) {
        ...
        rv = bcm_fe2000_trunk_get_old(unit, trunkId, trunkInfo);
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_VLAN,
                      (BSL_META_U(unit,
    ...
    if (BCM_SUCCESS(rv)) {
        ...
        if (BCM_E_NONE != rv) {
            LOG_ERROR(BSL_LS_BCM_VLAN,
                      (BSL_META_U(unit,
                                  "unable to get data %d:eteencap[0x%x]:"
                                   " %d (%s)\n"),
        ...
        if (BCM_E_NONE != rv) {
            LOG_ERROR(BSL_LS_BCM_VLAN,
                      (BSL_META_U(unit,
                                  "unable to set data %d:eteencap[0x%x]:"
                                   " %d (%s)\n"),
                       unit, oi2e.eteptr, rv, _SHR_ERRMSG(rv)));
        }
    }
                            ...
                            bcm_vlan_port_t *vlan_port)
{
    ...
    gportId = _bcm_fe2k_max_vlan_gports;
     ...
     * GPORT FTE
     */
    if (BCM_SUCCESS(result) && gportId >= _bcm_fe2k_max_vlan_gports) {
        LOG_ERROR(BSL_LS_BCM_VLAN,
                  (BSL_META_U(unit,
    ...
    if (BCM_SUCCESS(result) && BCM_GPORT_IS_TRUNK(vlan_port->port)) {
        ...
        result = bcm_fe2000_trunk_get_old(unit, tid, &trunk_info);
        if (BCM_FAILURE(result)) {
            LOG_ERROR(BSL_LS_BCM_VLAN,
                      (BSL_META_U(unit,
                                  "Failed to get trunk %d info: %s\n"),
                                   ...
                                   uint32 logicalPort)
{
        ...
        !(vlan_port->e.flags & _BCM_FE2K_VLAN_GP_EXTERN_OHI))
    {
        ...
        if (BCM_E_NONE == result) {
            ...
            LOG_ERROR(BSL_LS_BCM_VLAN,
                      (BSL_META_U(unit,
                                  "unable to read %d:ETE[0x%08X]Encap:"
                                   " %d (%s)\n"),
        ...
        !(vlan_port->e.flags & _BCM_FE2K_VLAN_GP_EXTERN_OHI))
    {
        ...
        if (BCM_E_NONE != result) {
            LOG_ERROR(BSL_LS_BCM_VLAN,
                      (BSL_META_U(unit,
                                  "unable to write %d:ete[0x%08X]L2:"
                                   " %d (%s)\n"),
                       unit, eteL2Index, result, _SHR_ERRMSG(result)));
        }
    }
                                           ...
                                           bcm_port_t phyPort, bcm_vlan_t vpn)
{
    ...
    fastStpstate = sal_alloc(sizeof(uint32) * (BCM_VLAN_MAX + 1),
                        "fastStpstate");
    fastUntaggedStrip = sal_alloc(sizeof(uint32) * (BCM_VLAN_MAX + 1),
                        "fastUntaggedStrip");
    ...
    if (BCM_SUCCESS(status)) {
        ...
        if (BCM_FAILURE(status)) {
            LOG_ERROR(BSL_LS_BCM_VLAN,
                      (BSL_META_U(unit,
                                  "failed to get pv2e[%d,*] untagged strip:"
                                   " %d %s\n"),
                       phyPort, status, bcm_errmsg(status)));
        }
    }
                                      ...
                                      bcm_vlan_vector_t vlan_vec)
{
    ...
    if (BCM_GPORT_IS_TRUNK(vlan_port.port)) {
        ...
        rv = bcm_fe2000_trunk_get_old(unit, trunk_id, &trunk_info);
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_VLAN,
                      (BSL_META_U(unit,
                                  "Failed to get trunk %d info: %s\n"),
                             ...
                             const bcm_port_t port)
{
    ...
    if (BCM_E_NONE == result) {
        ...
        if (BCM_E_NONE != result) {
            LOG_ERROR(BSL_LS_BCM_VLAN,
                      (BSL_META_U(unit,
                                  "_bcm_fe2000_vlan_free_ete(%d,%08X)"
                                   " returned %d (%s)\n"),
                       unit,
                       eteOld,
                       result,
    ...
    if (BCM_E_NONE == result) {
        ...
        if ((BCM_E_NONE != result) && (BCM_E_INIT != result)) {
            LOG_ERROR(BSL_LS_BCM_VLAN,
                      (BSL_META_U(unit,
                                  "%s: _bcm_fe2000_stg_vlan_port_remove(%d,%03X,%d) "
                                   "returned %d (%s)\n"),
                       FUNCTION_NAME(),
                       unit,
                       vid,
    ...
    if (BCM_E_NONE == result) {
        ...
        if (BCM_E_NONE == result) {
            ...
            if (BCM_E_NONE != result) {
                LOG_ERROR(BSL_LS_BCM_VLAN,
                          (BSL_META_U(unit,
                                      "bcm_fe2000_stg_vlan_add(%d,%d,%04X)"
                                       " returned %d (%s)\n"),
                           unit,
                           stg,
                           BCM_VLAN_DEFAULT,
    ...
    if (BCM_E_NONE == result) {
        PBMP_ALL_ITER(unit, pIndex) {
            ...
            if (BCM_E_NONE != tmpres) {
                LOG_ERROR(BSL_LS_BCM_VLAN,
                          (BSL_META_U(unit,
                                      "_bcm_fe2000_vlan_port_remove/add"
                                       "(%d,%04X,%d[,FALSE])"
                                       " returned %d (%s)\n"),
                           unit,
                           vid,
                        ...
                        const bcm_vlan_t vid)
{
    ...
    if (bcm_fe2000_mpls_vpn_id_get(unit, vid, 
    ...
    if (_bcm_fe2000_is_vswitch_inuse(unit, vid) != BCM_E_NOT_FOUND) {
        LOG_ERROR(BSL_LS_BCM_VLAN,
                  (BSL_META_U(unit,
        ...
        PBMP_ALL_ITER(unit,pIndex) {
            ...
            if ((BCM_E_NONE == tempRes) && (curvid == vid)) {
                ...
                if (BCM_E_NONE != result) {
                    ...
                    LOG_ERROR(BSL_LS_BCM_VLAN,
                              (BSL_META_U(unit,
                                          "bcm_fe2000_port_untagged_vlan_set"
                ...
                LOG_ERROR(BSL_LS_BCM_VLAN,
                          (BSL_META_U(unit,
                                      "bcm_fe2000_port_untagged_vlan_get"
                                       "(%d,%d,*) returned %d (%s)\n"),
                           unit,
                           pIndex,
                           result,
    ...
    if (BCM_E_NONE == result) {
        ...
        if (SOC_IS_SBX_G2P3(unit)) {
            ...
            if (BCM_E_NONE == result) {
                ...
                LOG_ERROR(BSL_LS_BCM_VLAN,
                          (BSL_META_U(unit,
                                      "unable to read %d:pv2appdata[%d,%03X]:"
                                       " %d (%s)\n"),
                           unit,
                           port,
                           vid,
                                              ...
                                              int *drop)
{
    ...
    if ((!drop) || (!vlan)) {
        ...
        LOG_ERROR(BSL_LS_BCM_VLAN,
                  (BSL_META_U(unit,
                              "NULL pointer for output argument\n")));
        return BCM_E_PARAM;
    }
                                            ...
                                            const int drop)
{
    ...
    if (PBMP_MEMBER(_vlan_state[unit]->untag_drop, port)) {
        ...
        if (npVid2Etc.vpws && SOC_SBX_CFG(unit)->mplstp_ena) {
...
_bcm_fe2000_vlan_gport_targets_recover(int unit)
{
    ...
    for (lpi = SBX_MAX_PORTS; lpi <= SBX_LPORT_END; lpi++) {
        ...
        if (SBX_LPORT_TYPE(unit, lpi) == BCM_GPORT_VLAN_PORT) {
            ...
            LOG_VERBOSE(BSL_LS_BCM_VLAN,
                        (BSL_META_U(unit,
                                    "Second pass recovery of vlan gport: "
                                     "0x%08x\n"),
            ...
            if (BCM_FAILURE(rv)) {
                LOG_ERROR(BSL_LS_BCM_VLAN,
                          (BSL_META_U(unit,
                                      "Failed to convert fabric gport "
                                       "%d:%d: %s\n"),
                               ...
                               uint32 lpi, uint32 vsi, soc_sbx_g2p3_lp_t *lp)
{
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_VLAN,
                  (BSL_META_U(unit,
                              "Failed to read eteencap[0x%06x]: %s\n"),
    ...
    if (gp_ptr == NULL) {
        LOG_ERROR(BSL_LS_BCM_VLAN,
                  (BSL_META_U(unit,
                              "Failed to to allocate memory for gport\n")));
        return BCM_E_MEMORY;
    }
    ...
    LOG_VERBOSE(BSL_LS_BCM_VLAN,
                (BSL_META_U(unit,
                            "Recovering vlan gport 0x%08x:\n"), 
                 gp_ptr->p.vlan_port_id));
    LOG_VERBOSE(BSL_LS_BCM_VLAN,
                (BSL_META_U(unit,
                            "ohi=0x%04x encap=0x%08x fabNode=%d fabPort=%d\n"),
                 fte.oi, SOC_SBX_ENCAP_ID_FROM_OHI(fte.oi),
                 fab_node, fab_port));
    LOG_VERBOSE(BSL_LS_BCM_VLAN,
                (BSL_META_U(unit,
                            "vsi=0x%03x, policer=0x%04x qos_map=0x%x\n"),
...
_bcm_fe2000_vlan_recover(int unit) 
{
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_VLAN,
                  (BSL_META_U(unit,
                              "failed to recover default vid: %s\n"),
                   bcm_errmsg(rv)));
        return rv;
    }
    ...
    PBMP_ALL_ITER(unit, port) {
        ...
        if (BCM_SUCCESS(rv)) {
            ...
            LOG_VERBOSE(BSL_LS_BCM_VLAN,
                        (BSL_META_U(unit,
                                    "Found a valid pvv2e entry on [%d,0,0xfff]\n"),
                         port));
    ...
    LOG_INFO(BSL_LS_BCM_VLAN,
             (BSL_META_U(unit,
                         "Recovery Data time = %d uS (%d S, %d uS)\n"),
...
bcm_fe2000_vlan_init(int unit)
{
    ...
    if (!(_vlan_state[unit])) {
        ...
        if (_vlan_state[unit]) {
           ...
           _bcm_fe2k_max_vlan_gports = soc_property_get(unit,
                                    spn_FTE_NUM_LOCAL_GPORTS, SBX_MAX_GPORTS);
           _vlan_state[unit]->gportInfo.lpid = 
                         sal_alloc(sizeof(uint16) * _bcm_fe2k_max_vlan_gports,
                              ...
                              uint32 flags_value)
{
    ...
    if (forceflood || polbypass || unknucast || learn) {
        ...
        if (BCM_SUCCESS(rv) && polbypass) {
            rv = soc_sbx_g2p3_v2e_policerbypass_fast_set(unit,
                                                      0,
                                                      0xFFF,
                                                      fastSets,
                                                      fastPolicerBypass,
        ...
        if (BCM_SUCCESS(rv) && learn) {
            rv = soc_sbx_g2p3_v2e_dontlearn_fast_set(unit,
                                                 0,
                                                 0xFFF,
                                                 fastSets,
                                                 fastLearn,
        ...
        if (BCM_SUCCESS(rv) && unknucast) {
            rv = soc_sbx_g2p3_v2e_dropunkucast_fast_set(unit,
                                                      0,
                                                      0xFFF,
                                                      fastSets,
                                                      fastUUC,
...
int
bcm_fe2000_vlan_control_vlan_selective_set(int unit,
                                 bcm_vlan_t vid,
                                 uint32 valid_fields,
                                 bcm_vlan_control_vlan_t *control)
{
...
int
bcm_fe2000_vlan_control_vlan_selective_get(int unit,
                                 bcm_vlan_t vid,
                                 uint32 valid_fields,
                                 bcm_vlan_control_vlan_t *control)
{
                            ...
                            if(BCM_FAILURE(status)) {
                                ...
                                if(BCM_FAILURE(status)) {
                                    ...
                                    if(BCM_FAILURE(status)) {
                                        LOG_ERROR(BSL_LS_BCM_VLAN,
                                                  (BSL_META_U(unit,
                                                              "Failed to Set  counter[%d] on LP[%d]: %d %s\n"),
                            ...
                            if (BCM_FAILURE(rv)) {
                                ...
                                LOG_VERBOSE(BSL_LS_BCM_VLAN,
                                            (BSL_META_U(unit,
                                             ...
                                             port, vlan, COMPILER_64_HI(uuVal), 
                                             COMPILER_64_LO(uuVal)));
                                COMPILER_64_ADD_64(*val, uuVal);
                            }
                        }
                    }
                                 ...
                                 uint32 *ing_flags, uint32 *egr_flags)
{
    ...
    *ing_flags  = BCM_QOS_MAP_INGRESS | BCM_QOS_MAP_L2;
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_VLAN,
                  (BSL_META_U(unit,
                              "failed to read %d:eteencap[0x%08X]:"
                               " %d (%s)\n"),
                                   ...
                                   bcm_port_t port, int diverage)
{
    ...
    if (diverage) {
        ...
        if (logicalPort == port) {
            ...
            /* inherit logical port type */
            SBX_LPORT_TYPE(unit, logicalPort) = SBX_LPORT_TYPE(unit, port);
        }
