...
#if defined(BCM_TRIDENT2PLUS_SUPPORT)
    if (soc_feature(unit, soc_feature_egr_vlan_control_is_memory)) {
        old_tpid_index = soc_mem_field32_get(unit, EGR_VLAN_CONTROL_1m,
                                                &entry, OUTER_TPID_INDEXf);
    } else 
#endif
    {
...
#if defined(BCM_TRIDENT2PLUS_SUPPORT)
    if (soc_feature(unit, soc_feature_egr_vlan_control_is_memory)) {
        soc_mem_field32_set(unit, EGR_VLAN_CONTROL_1m, &entry, 
                        OUTER_TPID_INDEXf, tpid_index);
        rv = WRITE_EGR_VLAN_CONTROL_1m(unit, MEM_BLOCK_ANY, port, &entry);
    } else 
#endif
    {
...
#if defined(BCM_TRIDENT2PLUS_SUPPORT)
    if (soc_feature(unit, soc_feature_egr_vlan_control_is_memory)) {
        cfi_cng = soc_mem_field32_get(unit, EGR_VLAN_CONTROL_1m,
                                       &entry, CFI_AS_CNGf);
    } else 
#endif
    {
...
_bcm_fb2_port_tpid_delete_all(int unit, bcm_port_t port)
{
    ...
    if (soc_feature(unit, soc_feature_egr_vlan_control_is_memory)) {
        ...
        if (!BCM_FAILURE(rv)) {
            ...
            /*TBD!!! do we need to add _bcm_fb2....*/
