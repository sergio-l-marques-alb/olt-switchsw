    ...
    int unit)
{
    ...
    SOC_PPD_FRWRD_FCF_GLBL_INFO fc_info;
    int trap_id;
    int rv = BCM_E_NONE;
    ...
    SOC_PPD_FRWRD_FCF_GLBL_INFO_clear(&fc_info);
   ...
   /*
    * Trap setting for WA: fix offset of forwarding header, to keep Sof.
    */
    trap_id = SOC_PPC_TRAP_CODE_FCOE_WA_FIX_OFFSET;
    /* allocate user define trap */
    rv = bcm_petra_rx_trap_type_create(unit, BCM_RX_TRAP_REPLACE|BCM_RX_TRAP_WITH_ID, bcmRxTrapUserDefine, &trap_id);
    ...
    trap_info.bitmap_mask = SOC_PPD_TRAP_ACTION_PROFILE_OVERWRITE_FRWRD_OFFSET;
    ...
    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    soc_sand_rv = soc_ppd_trap_frwrd_profile_info_set(unit,trap_id,&trap_info);
    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
...
STATIC uint32 _bcm_petra_foce_route_key_to_ppd(
        ...
        SOC_PPD_FRWRD_FCF_ROUTE_KEY *route_key,
        uint32     *insertion_type
   ){
    uint32 db_flags;
    BCMDNX_INIT_FUNC_DEFS;

    SOC_PPD_FRWRD_FCF_ROUTE_KEY_clear(route_key);
...
STATIC uint32 _bcm_petra_foce_route_info_to_ppd(
        int                             unit, 
        bcm_fcoe_route_t                *route,
        SOC_PPD_FRWRD_FCF_ROUTE_INFO    *route_info,
        uint32                          insertion_type
   ){
    ...
    SOC_PPD_FRWRD_FCF_ROUTE_INFO_clear(route_info);
    ...
    if(_BCM_PETRA_L3_ITF_IS_RIF(route->intf)) {
    ...
    /* destination is FEC */
    if(_BCM_PETRA_L3_ITF_IS_FEC(route->intf)) {
    ...
    if(frwrd_type == 3) { /* port + outRIF */
        rif_entry_id = _BCM_PETRA_L3_ITF_VAL_GET(route->intf);
        route_info->frwrd_decision.additional_info.outlif.type = SOC_PPD_OUTLIF_ENCODE_TYPE_RAW;
    ...
    /* FEC + MAC --> forward decision = FEC + ARP (EEI)*/
        ...
        route_info->frwrd_decision.additional_info.eei.type = SOC_PPD_EEI_TYPE_OUTLIF;
        ...
        /* IP-UC type used to say move out-RIF from out-LIF field to EEI field in FEC, at last it will be writing as tunnel format */
        rv = _bcm_dpp_gport_fix_fec_type(unit, _BCM_PETRA_L3_ITF_VAL_GET(route->intf), SOC_PPD_FEC_TYPE_IP_UC);
        BCMDNX_IF_ERR_EXIT(rv);
    }
...
STATIC uint32 _bcm_petra_foce_route_info_from_ppd(
    ...
    if (route_info->frwrd_decision.additional_info.eei.type == SOC_PPD_EEI_TYPE_OUTLIF) {
        eep_val = route_info->frwrd_decision.additional_info.eei.val.outlif;
        if(eep_val != 0){
            frwrd_type = 2;
        }
    }/* EEP + port */
    else if (route_info->frwrd_decision.additional_info.outlif.type == SOC_PPD_OUTLIF_ENCODE_TYPE_RAW) {
        eep_val = route_info->frwrd_decision.additional_info.outlif.val;
        if(eep_val != 0){
            frwrd_type = 1;
        }
    }
    if((frwrd_type == 0 || frwrd_type == 2) && (route_info->frwrd_decision.type != SOC_PPD_FRWRD_DECISION_TYPE_FEC)){
    ...
    if (frwrd_type == 0 || frwrd_type == 2) {
        ...
        rv = _bcm_dpp_gport_from_fwd_decision(unit,&route->port,&(route_info->frwrd_decision));
        BCMDNX_IF_ERR_EXIT(rv);
    }
    ...
    /* get RIF*/
    if (frwrd_type == 2) { 
        _BCM_PETRA_L3_ITF_SET(route->intf,_BCM_PETRA_L3_ITF_RIF,eep_val);
    }
    /* set encap */
    if (frwrd_type == 1) { 
        _BCM_PETRA_L3_ITF_SET(route->intf,_BCM_PETRA_L3_ITF_ENCAP,eep_val);
    }
...
STATIC uint32 _bcm_petra_foce_zone_key_to_ppd(
   ...
   SOC_PPD_FRWRD_FCF_ZONING_KEY    *zone_key
)
{
    BCMDNX_INIT_FUNC_DEFS;

    SOC_PPD_FRWRD_FCF_ZONING_KEY_clear(zone_key);
...
STATIC uint32 _bcm_petra_foce_zone_info_to_ppd(
        int unit, 
        bcm_fcoe_zone_entry_t *zone,
        SOC_PPD_FRWRD_FCF_ZONING_INFO *zone_info
   )
{
    int rv = BCM_E_NONE;
    uint32 soc_sand_rv;
    ...
    SOC_PPD_FRWRD_FCF_ZONING_INFO_clear(zone_info);
   ...
   )
{
    ...
    /* if drop/allow trap */
    if (zone_info->frwrd_decision.type == SOC_PPD_FRWRD_DECISION_TYPE_TRAP) {
    ...
    rv = _bcm_dpp_gport_from_fwd_decision(unit,&zone->dest_port,&(zone_info->frwrd_decision));
    BCMDNX_IF_ERR_EXIT(rv);
...
int bcm_petra_fcoe_route_add(
    ...
    bcm_fcoe_route_t *route)
{
    SOC_PPD_FRWRD_FCF_ROUTE_KEY 
        route_key;
    SOC_PPD_FRWRD_FCF_ROUTE_INFO 
        route_info;
    uint32     
        insertion_type;
    ...
    bcm_fcoe_route_t *route)
{
    ...
    SOC_PPD_FRWRD_FCF_ROUTE_INFO 
        route_info;
    SOC_PPD_FRWRD_FCF_ROUTE_STATUS
        route_status;
    uint32     
        insertion_type;
    uint8
...
int bcm_petra_fcoe_zone_add(
    ...
    bcm_fcoe_zone_entry_t *zone)
{ 
    SOC_PPD_FRWRD_FCF_ZONING_KEY 
        zone_key;
    SOC_PPD_FRWRD_FCF_ZONING_INFO 
        zone_info;
    SOC_SAND_SUCCESS_FAILURE
        failure_indication;
    int rv = BCM_E_NONE;
    ...
    bcm_fcoe_zone_entry_t *zone)
{
    ...
    remove_flags = SOC_PPD_FRWRD_FCF_ZONE_REMOVE_BY_VFT;
    ...
    bcm_fcoe_zone_entry_t *zone)
{
    ...
    remove_flags = SOC_PPD_FRWRD_FCF_ZONE_REMOVE_BY_D_ID;
...
{ 
    ...
    SOC_PPD_FRWRD_FCF_ZONING_KEY_clear(&remove_key);
