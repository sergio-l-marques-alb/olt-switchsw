...
#define BCM_DPP_TUNNEL_INIT_CHECK \
    ...
    } else if ((!(SOC_DPP_STATE(unit)->l3_state->init))) { \
        BCMDNX_ERR_EXIT_MSG(BCM_E_INIT, (_BSL_BCM_MSG("unit %d not initialised"), unit)); \
    } 
 ...
 * MACROs
 */
#define IP_TUNNEL_STATE(_unit_)  (SOC_DPP_STATE(_unit_)->l3_state)
#define IP_TUNNEL_INFO(_unit_)   (&IP_TUNNEL_STATE(_unit_)->_bcm_dpp_tunnel_bk_info)
...
int _bcm_dpp_ipt_egress_tunnel_set(
    int unit,
    int tunnel_id,
    int lif)
{
    _bcm_dpp_tunnel_bookkeeping_t *ipt_info = IP_TUNNEL_INFO(unit);
    int rv = BCM_E_NONE;

    ipt_info->egress_tunnel_id[lif] = tunnel_id;

#ifdef BCM_WARM_BOOT_SUPPORT
    rv = _bcm_dpp_wb_l3_update_egr_tnl_id(unit, lif);
#endif

    return rv;
}

int _bcm_dpp_ipt_egress_tunnel_get(
    int unit,
    int *tunnel_id,
    int lif)
{
    ...
    *tunnel_id = ipt_info->egress_tunnel_id[lif];
...
STATIC void _bcm_petra_ipt_free_resource(
    int unit,
    _bcm_dpp_tunnel_bookkeeping_t *ipt_info)
{
    ...
    if (ipt_info->egress_tunnel_id) {
        BCM_FREE(ipt_info->egress_tunnel_id);
        ipt_info->egress_tunnel_id = NULL;
    }

    if (ipt_info->vsi_usage) {
	BCM_FREE(ipt_info->vsi_usage);
        ipt_info->vsi_usage = NULL;
    }
...
int _bcm_ip_tunnel_sw_init(int unit) 
{
    ...
    num_lifs = _BCM_GPORT_NOF_LIFS;

    /* alloc */

    /* set init values */

    BCMDNX_ALLOC(ipt_info->egress_tunnel_id, sizeof(int) * num_lifs, "egress_tunnel_id");
    if (ipt_info->egress_tunnel_id == NULL) {
        _bcm_petra_ipt_free_resource(unit, ipt_info);
        BCMDNX_ERR_EXIT_MSG(BCM_E_MEMORY, (_BSL_BCM_MSG("free memory failed")));
    }

    /* set init values */
    sal_memset(ipt_info->egress_tunnel_id, 0, sizeof(int) * num_lifs);

    /* bit per vsi */
    BCMDNX_ALLOC(ipt_info->vsi_usage, sizeof(uint8) * (((_BCM_GPORT_NOF_VSIS + 7) / 8) * 2),
                  "vsi usage");
    if (ipt_info->vsi_usage == NULL) {
        _bcm_petra_ipt_free_resource(unit, ipt_info);
        BCMDNX_ERR_EXIT_MSG(BCM_E_MEMORY, (_BSL_BCM_MSG("free memory failed")));
    }

    /* set init values */
    sal_memset(ipt_info->vsi_usage, 0,
               sizeof(uint8) * (((_BCM_GPORT_NOF_VSIS + 7) / 8) * 2));
...
/* check if the given vpn is allocated as l2 IPv4 VPN */

int _bcm_dpp_ipt_vsi_usage_get(
    int unit,
    int vsi,
    _bcm_vsi_type_e *vpn_usage)
{
    _bcm_dpp_tunnel_bookkeeping_t *ipt_info = IP_TUNNEL_INFO(unit);
    int tmp = 0;
    BCMDNX_INIT_FUNC_DEFS;
    ...
    *vpn_usage = 0;
    ...
    tmp = ipt_info->vsi_usage[vsi / 4];
    *vpn_usage = (3 << ((vsi % 4) * 2)) & tmp;
...
int _bcm_dpp_ipt_vsi_usage_set(
    int unit,
    int vsi,
    _bcm_vsi_type_e vpn_usage)
{
    ...
    ipt_info->vsi_usage[vsi / 4] = tmp;

#ifdef BCM_WARM_BOOT_SUPPORT
    BCMDNX_IF_ERR_EXIT(_bcm_dpp_wb_l3_update_vsi_usage(unit, (vsi/4)));
#endif
...
/*
 * given interface and tunnen returns eep. 
...
int _bcm_petra_tunnel_initiator_erspan_rspan_set(
   ...
   bcm_l3_intf_t *intf, 
   bcm_tunnel_initiator_t *tunnel,
   int outlif /* outlif to use for ERSPAN tunnel */
   )
{
    int rv = BCM_E_NONE;
    ...
    if (dummy_outlif_valid) {
        /* Add to SW DB */
        rv = _bcm_dpp_out_lif_tunnel_initiator_match_add(unit,tunnel,dummy_outlif);
        BCMDNX_IF_ERR_EXIT(rv);
    }
    ...
    int *tunnel_eep)
{
    ...
    int is_tunneled = 0, eep=0, flags = 0, vsi = 0, ll_eep = 0;
    int eep_alloced=0;
    int index = 0;
    uint32 sip = 0;
    ...
    if (SOC_IS_PETRAB(unit)) {
        ...
        rv = _bcm_tunnel_intf_is_tunnel(unit, intf->l3a_intf_id, &is_tunneled, &eep, &vsi, &ll_eep);
        BCMDNX_IF_ERR_EXIT(rv);
         ...
         */
        if (with_id) {
            rv = _bcm_petra_tunnel_id_eep_get(unit,intf,tunnel,&eep,&is_tunneled);
            BCMDNX_IF_ERR_EXIT(rv);
        }
    ...
    if (tunnel->type == bcmTunnelTypeErspan || tunnel->type == bcmTunnelTypeRspan) {
        rv = _bcm_petra_tunnel_initiator_erspan_rspan_set(unit,intf,tunnel,eep);
        BCMDNX_IF_ERR_EXIT(rv);
        BCM_EXIT;
    }
    ...
    if (is_tunneled) {
        ...
        rv = bcm_dpp_am_ip_tunnel_eep_is_alloced(unit,ip_tunnel_alloc_flag,eep);
        if (rv == BCM_E_PARAM) {
    ...
    if(!eep_alloced) {/* need to allocate IP tunnel */
        ...
        if (SOC_IS_ARADPLUS(unit)) {
            ...
            rv = soc_ppd_eg_encap_entry_type_get(unit, _BCM_PETRA_L3_ITF_VAL_GET(tunnel->l3_intf_id), &entry_type);
            BCM_IF_ERROR_RETURN(rv);
        ...
        rv = bcm_dpp_am_ip_tunnel_eep_alloc(unit, ip_tunnel_eep_alloc_type, flags|ip_tunnel_alloc_flag, &eep);
        if (rv != BCM_E_NONE) {
    ...
    if (SOC_IS_PETRAB(unit)) {
        ...
        /* write to HW entry */
        soc_sand_rv =
        soc_ppd_eg_encap_ipv4_entry_add(soc_sand_dev_id, eep, 
                        &ipv4_encap_info, 0);
        SOC_SAND_IF_ERR_RETURN(soc_sand_rv);

        /* update SW according to allocated tunnel ID */
        /* mark this interface as tunneled */
        rv = _bcm_tunnel_intf_eep_set(unit, intf->l3a_intf_id, eep);
        if (rv != BCM_E_NONE) {
        ...
        next_eep[0] = _BCM_PETRA_L3_ITF_VAL_GET(tunnel->l3_intf_id); 

        /* according to TYPE fill tunnel data */
        if(!is_ipv6_tunnel) {
            
            rv = _bcm_petra_tunnel_init_ipv4_to_ppd(unit,tunnel,eep,eep_alloced,&ipv4_encap_info);
            BCMDNX_IF_ERR_EXIT(rv);
            ...
            /* write to HW entry*/
            soc_sand_rv =
            soc_ppd_eg_encap_ipv6_entry_add(soc_sand_dev_id, eep, 
                            &ipv6_encap_info, next_eep[0]);
            SOC_SAND_IF_ERR_RETURN(soc_sand_rv);
        }
        
        /* update SW according to allocated tunnel ID */
        rv = _bcm_petra_tunnel_id_eep_set(unit,intf,tunnel,eep);
        BCMDNX_IF_ERR_EXIT(rv);

        /* Add to SW DB */
        rv = _bcm_dpp_out_lif_tunnel_initiator_match_add(unit,tunnel,eep);
        BCMDNX_IF_ERR_EXIT(rv);
...
bcm_petra_tunnel_initiator_clear(
    ...
    bcm_l3_intf_t *intf)
{
    int eep, is_tunneled, vsi, ll_eep;
    SOC_PPD_EG_ENCAP_ENTRY_INFO encap_entry_info[SOC_PPD_NOF_EG_ENCAP_EEP_TYPES];
    ...
    if(SOC_IS_PETRAB(unit)) {
        ...
        rv = _bcm_petra_tunnel_id_eep_get(unit,intf,0,&eep,&is_tunneled);
        BCMDNX_IF_ERR_EXIT(rv);
    }

    BCMDNX_IF_ERR_EXIT(_bcm_dpp_lif_usage_get(unit, -1, eep, NULL, &lif_usage));
    ...
    soc_sand_rv = soc_ppd_eg_encap_entry_get(soc_sand_dev_id,
                     SOC_PPD_EG_ENCAP_EEP_TYPE_TUNNEL_EEP, eep, 1,
    ...
    /* ERSPAN / RSPAN*/
        ...
        rv = bcm_petra_mirror_destination_tunnel_destroy(unit,eep);
        BCMDNX_IF_ERR_EXIT(rv);
   ...
   /* check if this tunnel is allocated */
    ...
    if(SOC_IS_PETRAB(unit)) {
        ...
        soc_sand_rv = soc_ppd_eg_encap_entry_remove(soc_sand_dev_id,SOC_PPD_EG_ENCAP_EEP_TYPE_TUNNEL_EEP,eep);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        ...
        /* free memory */
        rv = bcm_dpp_am_l3_lif_ip_tnl_dealloc(unit, eep);
        BCMDNX_IF_ERR_EXIT(rv);

        rv = bcm_dpp_am_ip_tunnel_eep_dealloc(unit, ip_tunnel_alloc_flag, eep);
        BCMDNX_IF_ERR_EXIT(rv);
    }
    else{ /* is-soc_petra */
#ifdef BCM_ARAD_SUPPORT
        ...
        if (!is_ipv6_tunnel) {
            rv = _bcm_dpp_am_template_ip_tunnel_exchange(unit,eep,0 /* exist entry */,0xFFFFFFFF,0xFFFFFFFF,0xFFFFFFFF,
                                                          &sip_template, &ttl_template, &tos_template,&is_allocated);
            BCMDNX_IF_ERR_EXIT(rv);
    ...
    bcm_tunnel_initiator_t *tunnel)
{
    ...
    if(SOC_IS_PETRAB(unit)) {
        ...
        if (usage == _bcmDppLifTypeOverlayLinkLayer) {
            _BCM_PETRA_L3_ITF_SET(tunnel->l3_intf_id, _BCM_PETRA_L3_ITF_ENCAP, next_eep[0]); 
        } 
        /* otherwise it's an outRif. */
        else {
            _BCM_PETRA_L3_ITF_SET(tunnel->l3_intf_id, _BCM_PETRA_L3_ITF_RIF, next_eep[0]);
        }
        ...
        if (encap_entry_info[0].entry_type == SOC_PPD_EG_ENCAP_ENTRY_TYPE_IPV4_ENCAP) {
            ...
            rv = bcm_dpp_am_eg_data_erspan_is_alloced(unit,eep);
            if (rv == BCM_E_NOT_FOUND) {
    ...
    bcm_tunnel_terminator_t *info) 
{
    ...
    if (with_id && BCM_GPORT_IS_TUNNEL(info->tunnel_id)) {
        lif_indx = BCM_GPORT_TUNNEL_ID_GET(info->tunnel_id);
        alloc_flags |= SHR_RES_ALLOC_WITH_ID;

        rv = bcm_dpp_am_l3_lif_is_allocated(unit, _BCM_DPP_AM_L3_LIF_IP_TERM, 0, lif_indx);
    ...
    if (update) {
        ...
        rv = bcm_dpp_am_l3_lif_alloc(unit, _BCM_DPP_AM_L3_LIF_IP_TERM, alloc_flags, &lif_indx);
        BCMDNX_IF_ERR_EXIT(rv);
    }
    ...
    rv = _bcm_dpp_in_lif_tunnel_term_add(unit, info, lif_indx);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    bcm_tunnel_terminator_t *info)
{
    ...
    rv = _bcm_dpp_in_lif_tunnel_term_get(unit, info, lif_indx);
    BCMDNX_IF_ERR_EXIT(rv);
