...
_bcm_petra_port_control_lanes_set(int unit, bcm_port_t port, int value)
{
    ...
    if (SOC_IS_ARAD(unit)) {
        ...
        BCMDNX_IF_ERR_EXIT(soc_port_sw_db_port_validate_and_add(unit, core, port, channel, flags, intf, phy_ports));
                            ...
                            bcm_port_control_t type, int value)
{
    ...
    case bcmPortControlEgressModifyDscp:
      ...
      case bcmPortControlPadToSize:
          BCMDNX_IF_ERR_EXIT(soc_pm_pad_size_set(unit, port, value));
          break;
      default:
        rv = BCM_E_UNAVAIL;
        break;
                            ...
                            bcm_port_control_t type, int *value) 
{
    ...
    case bcmPortControlExtenderEnable:
        ...
        break;
    case bcmPortControlPadToSize:
          rv = soc_pm_pad_size_get(unit, port, value);
          BCMDNX_IF_ERR_EXIT(rv);
          break;
    default:
            rv = BCM_E_UNAVAIL;
    ...
    bcm_port_mdix_t mode)
{
   ...
   BCMDNX_IF_ERR_EXIT(_bcm_dpp_gport_to_phy_port(unit, port, _BCM_DPP_GPORT_TO_PHY_OP_LOCAL_IS_MANDATORY, &gport_info)); 
   MIIM_LOCK(unit); 
   rv = soc_phyctrl_mdix_set(unit, gport_info.local_port, mode); 
   MIIM_UNLOCK(unit); 
    ...
    bcm_port_mdix_t *mode)
{
   ...
   BCMDNX_IF_ERR_EXIT(_bcm_dpp_gport_to_phy_port(unit, port, _BCM_DPP_GPORT_TO_PHY_OP_LOCAL_IS_MANDATORY, &gport_info)); 
   MIIM_LOCK(unit); 
   rv = soc_phyctrl_mdix_get(unit, gport_info.local_port, mode); 
   MIIM_UNLOCK(unit); 
    ...
    bcm_port_mdix_status_t *status)
{
   ...
   BCMDNX_IF_ERR_EXIT(_bcm_dpp_gport_to_phy_port(unit, port, _BCM_DPP_GPORT_TO_PHY_OP_LOCAL_IS_MANDATORY, &gport_info)); 
   MIIM_LOCK(unit); 
   rv = soc_phyctrl_mdix_status_get(unit, gport_info.local_port, status); 
   MIIM_UNLOCK(unit); 
...
bcm_petra_fap_port_parent_nif_get(int unit, int fap_port, int *p_nif_port)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    BCMDNX_IF_ERR_EXIT(soc_port_sw_db_local_to_tm_port_get(unit, fap_port, &tm_port, &core));
...
bcm_petra_port_tpid_class_set(int unit, bcm_port_tpid_class_t *tpid_class)
{
    ...
    if (is_egress) {    
        /* read old buffer this port used */
        rv = _bcm_dpp_am_template_port_tpid_class_egress_acceptable_frame_type_data_get(unit,soc_ppd_port_i,&tpid_class_acceptable_frame_info);
        BCMDNX_IF_ERR_EXIT(rv);
...
int bcm_petra_port_add(int unit, bcm_port_t port, uint32 flags, bcm_port_interface_info_t *interface_info, bcm_port_mapping_info_t *mapping_info)  
{
    ...
    BCMDNX_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_ports_port_to_nif_id_get, (unit, core, tm_port, &if_id)));
    ...
    if (nof_channels == 1) {
        ...
        SOCDNX_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_egr_q_nif_cal_set,(unit)));
    ...
    if (nof_channels == 1 || SOC_PBMP_MEMBER(SOC_INFO(unit).cmic_bitmap, port)) {
        ...
        if (is_channelized) {
            ...
            SOCDNX_IF_ERR_EXIT(soc_port_sw_db_is_single_cal_mode_set(unit, port, FALSE));
        } else {
            SOCDNX_IF_ERR_EXIT(soc_port_sw_db_is_single_cal_mode_set(unit, port, TRUE));
        }
    ...
    if (!is_channelized) {
        ...
        if ((kbits_sec_max_get + interface_max_rate*1000) <= SOC_TMC_IF_MAX_RATE_KBPS_ARAD) {
...
{
    uint32           is_valid, is_master, counter_flags, dummy1, dummy2, kbits_sec_max_get , cal_id;
    bcm_port_if_t    interface;
    ...
    uint32           tm_port, out_port_priority, nof_channels, interface_max_rate;
    bcm_gport_t      gport, fap_port;
    ...
    if (!is_channelized) {
        ...
        if (kbits_sec_max_get > interface_max_rate*1000) {
            BCMDNX_IF_ERR_EXIT(bcm_petra_cosq_gport_bandwidth_set(unit,fap_port,0,0,(kbits_sec_max_get - interface_max_rate*1000),0));
        } else {
            BCMDNX_IF_ERR_EXIT(bcm_petra_cosq_gport_bandwidth_set(unit,fap_port,0,0,0,0));
        }

        /* set E2E interface bandwidth */
        if (!IS_TDM_PORT(unit,port)) {
            gport_info.in_gport = port;
            BCMDNX_IF_ERR_EXIT(bcm_petra_cosq_gport_handle_get(unit,bcmCosqGportTypeE2EPort,&gport_info));
            BCMDNX_IF_ERR_EXIT(bcm_petra_fabric_port_get(unit,gport_info.out_gport,0,&fap_port));
            BCMDNX_IF_ERR_EXIT(bcm_petra_cosq_gport_bandwidth_get(unit,fap_port,0,&dummy1,&kbits_sec_max_get,&dummy2));
    ...
    if (nof_channels == 1 && is_channelized) { 
        ...
        /* high priority cal */
        SOCDNX_IF_ERR_EXIT(soc_port_sw_db_high_priority_cal_get(unit, port, &cal_id));
        ...
        /* low priority cal */
        SOCDNX_IF_ERR_EXIT(soc_port_sw_db_low_priority_cal_get(unit, port, &cal_id));
        if (cal_id != INVALID_CALENDAR) {
            SOCDNX_SAND_IF_ERR_EXIT(soc_sand_occ_bm_occup_status_set(cals_occ, cal_id, FALSE));
    ...
    BCMDNX_IF_ERR_EXIT(_bcm_dpp_am_template_cosq_egr_thresh_exchange(unit,port,&thresh_info,&old_profile,&is_last,&new_profile,&is_allocated)); 
