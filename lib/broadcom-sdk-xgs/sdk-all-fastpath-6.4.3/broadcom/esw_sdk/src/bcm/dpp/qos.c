         ...
         if (flags & BCM_QOS_MAP_L2_VLAN_PCP) {
            ...
            BCM_INT_QOS_MAP_EGRESS_PCP_VLAN_SET(*map_id, profile_ndx);  
             ...
             if (SOC_IS_JERICHO(unit)) {
                 /* Only verify  the profile is legal and set the map.*/
...
bcm_petra_qos_map_destroy(int unit, int map_id)
{
    ...
    for (idx=0; idx < _BCM_QOS_MAP_ING_VLAN_PCP_STAG_MAX; idx++) {
        ...
        if (!SHR_BITGET(SOC_DPP_STATE(unit)->qos_state->ing_pcp_vlan_stag_bitmap, _DPP_QOS_ENTRY_INDEX_ING_PCP_VLAN_STAG(profile_ndx, qos_map.int_pri, qos_map.color))) {
            continue;
        }
    ...
    for (idx=0; idx < _BCM_QOS_MAP_ING_VLAN_PCP_CTAG_MAX; idx++) {
        ...
        if (!SHR_BITGET(SOC_DPP_STATE(unit)->qos_state->ing_pcp_vlan_ctag_bitmap, _DPP_QOS_ENTRY_INDEX_ING_PCP_VLAN_CTAG(profile_ndx, qos_map.int_pri))) {
            continue;
        }
        rv = bcm_petra_qos_map_delete(unit, flags, &qos_map, map_id);
        if (rv != BCM_E_NONE) {
    ...
    for (idx=0; idx < _BCM_QOS_MAP_ING_VLAN_PCP_UTAG_MAX; idx++) {
        ...
        if (!SHR_BITGET(SOC_DPP_STATE(unit)->qos_state->ing_pcp_vlan_utag_bitmap, _DPP_QOS_ENTRY_INDEX_ING_PCP_VLAN_UTAG(profile_ndx, qos_map.int_pri, qos_map.color))) {
            continue;
        }
        rv = bcm_petra_qos_map_delete(unit, flags, &qos_map, map_id);
        if (rv != BCM_E_NONE) {
    ...
    for (idx = 0; idx < _BCM_QOS_MAP_ING_MPLS_EXP_MAX; idx++) {
        qos_map.exp = idx;
        if (!SHR_BITGET(SOC_DPP_STATE(unit)->qos_state->ing_lif_cos_mpls_bitmap, _DPP_QOS_ENTRY_INDEX_ING_LIF_COS_MPLS(profile_ndx, qos_map.exp))) {
            continue;
        }
    ...
    for (idx = 0; idx < _BCM_QOS_MAP_ING_L3_DSCP_MAX; idx++) {
        qos_map.dscp = idx;
        if (SHR_BITGET(SOC_DPP_STATE(unit)->qos_state->ing_lif_cos_ipv4_bitmap, _DPP_QOS_ENTRY_INDEX_ING_LIF_COS_L3(profile_ndx, qos_map.dscp))) {
        ...
        if (SHR_BITGET(SOC_DPP_STATE(unit)->qos_state->ing_lif_cos_ipv6_bitmap, _DPP_QOS_ENTRY_INDEX_ING_LIF_COS_L3(profile_ndx, qos_map.dscp))) {
    ...
    for (idx = 0; idx < _BCM_QOS_MAP_ING_L2_PRI_CFI_MAX; idx++) {
        ...
        if (SHR_BITGET(SOC_DPP_STATE(unit)->qos_state->ing_lif_cos_l2_outer_bitmap, _DPP_QOS_ENTRY_INDEX_ING_LIF_COS_L2_OUTER(profile_ndx, qos_map.pkt_pri, qos_map.pkt_cfi))) {
        ...
        if (SHR_BITGET(SOC_DPP_STATE(unit)->qos_state->ing_lif_cos_l2_inner_bitmap, _DPP_QOS_ENTRY_INDEX_ING_LIF_COS_L2_INNER(profile_ndx, qos_map.pkt_pri, qos_map.pkt_cfi))) {
    ...
    for (idx = 0; idx < _BCM_QOS_MAP_ING_L2_PRI_CFI_MAX; idx++) {
        ...
        if (!SHR_BITGET(SOC_DPP_STATE(unit)->qos_state->ing_lif_cos_l2_untag_bitmap, _DPP_QOS_ENTRY_INDEX_ING_LIF_COS_L2_UNTAG(profile_ndx, qos_map.pkt_pri, qos_map.pkt_cfi))) {
            continue;
        }
        rv = bcm_petra_qos_map_delete(unit, flags, &qos_map, map_id);
        if (rv != BCM_E_NONE) {
    ...
    for (int_pri=0; int_pri < _BCM_QOS_MAP_EGR_VLAN_PRI_MAX; int_pri++) {
        ...
        if (!SHR_BITGET(SOC_DPP_STATE(unit)->qos_state->egr_pcp_vlan_stag_bitmap, _DPP_QOS_ENTRY_INDEX_EGR_PCP_VLAN_STAG(profile_ndx, qos_map.int_pri, qos_map.color))) {
            continue;
        }
        rv = bcm_petra_qos_map_delete(unit, flags, &qos_map, map_id);
        if (rv != BCM_E_NONE) {
    ...
    for (int_pri=0; int_pri < _BCM_QOS_MAP_EGR_VLAN_PRI_MAX; int_pri++) {
        ...
        if (!SHR_BITGET(SOC_DPP_STATE(unit)->qos_state->egr_pcp_vlan_ctag_bitmap, _DPP_QOS_ENTRY_INDEX_EGR_PCP_VLAN_CTAG(profile_ndx, qos_map.int_pri))) {
            continue;
        }
        rv = bcm_petra_qos_map_delete(unit, flags, &qos_map, map_id);
        if (rv != BCM_E_NONE) {
    ...
    for (int_pri=0; int_pri < _BCM_QOS_MAP_EGR_VLAN_PRI_MAX; int_pri++) {
        ...
        if (!SHR_BITGET(SOC_DPP_STATE(unit)->qos_state->egr_pcp_vlan_utag_bitmap, _DPP_QOS_ENTRY_INDEX_EGR_PCP_VLAN_UTAG(profile_ndx, qos_map.int_pri, qos_map.color))) {
            continue;
        }
        rv = bcm_petra_qos_map_delete(unit, flags, &qos_map, map_id);
        if (rv != BCM_E_NONE) {
    ...
    if ( profile_ndx == SOC_DPP_CONFIG(unit)->qos.egr_pcp_vlan_dscp_exp_profile_id ) {
        ...
        if (!SHR_BITGET(SOC_DPP_STATE(unit)->qos_state->egr_pcp_vlan_ipv4_bitmap, _DPP_QOS_ENTRY_INDEX_EGR_PCP_VLAN_L3(profile_ndx, qos_map.dscp))) {
            continue;
        }
        rv = bcm_petra_qos_map_delete(unit, flags, &qos_map, map_id);
        if (rv != BCM_E_NONE) {
        ...
        if (!SHR_BITGET(SOC_DPP_STATE(unit)->qos_state->egr_pcp_vlan_ipv6_bitmap, _DPP_QOS_ENTRY_INDEX_EGR_PCP_VLAN_L3(profile_ndx, qos_map.dscp))) {
            continue;
        }
        rv = bcm_petra_qos_map_delete(unit, flags, &qos_map, map_id);
        if (rv != BCM_E_NONE) {
        ...
        if (!SHR_BITGET(SOC_DPP_STATE(unit)->qos_state->egr_pcp_vlan_mpls_bitmap, _DPP_QOS_ENTRY_INDEX_EGR_PCP_VLAN_MPLS(profile_ndx, qos_map.exp))) {
            continue;
        }
        rv = bcm_petra_qos_map_delete(unit, flags, &qos_map, map_id);
        if (rv != BCM_E_NONE) {
    ...
    if (rv != BCM_E_NONE) {
        ...
        for (int_pri=0; int_pri < _BCM_QOS_MAP_EGR_VLAN_PRI_MAX; int_pri++) {
            ...
            if (!SHR_BITGET(SOC_DPP_STATE(unit)->qos_state->egr_l2_i_tag_bitmap, _DPP_QOS_ENTRY_INDEX_EGR_L2_I_TAG(profile_ndx, qos_map.int_pri, qos_map.color))) {
                continue;
            }
            rv = bcm_petra_qos_map_delete(unit, flags, &qos_map, map_id);
            if (rv != BCM_E_NONE) {
        ...
        for (resolved_dp_idx = 0; resolved_dp_idx < _BCM_QOS_MAP_EGR_RESOLVED_DP_MAX; resolved_dp_idx++) {
            for (tc_idx = 0; tc_idx < _BCM_QOS_MAP_TC_MAX; tc_idx++) {
                for (inlif_idx = 0; inlif_idx < SOC_PPC_PROFILE_MGMT_NOF_INLIF_PROFILES_TRANSFERED_TO_EGRESS; inlif_idx++) {
                    ...
                    if (!SHR_BITGET(SOC_DPP_STATE(unit)->qos_state->egr_dscp_exp_marking_bitmap, 
        ...
        if (BCM_INT_QOS_MAP_IS_ENCAP(map_id)) {
           ...
           for (remark_int_pri = 0; remark_int_pri < _BCM_QOS_MAP_EGR_REMARK_MPLS_EXP_MAX; remark_int_pri++) { 
             ...
             if (!SHR_BITGET(SOC_DPP_STATE(unit)->qos_state->egr_remark_encap_mpls_bitmap, _DPP_QOS_ENTRY_INDEX_EGR_REMARK_ENP_MPLS(profile_ndx, qos_map.remark_int_pri))) {
                 continue;
             }
           ...
           for (remark_int_pri = 0; remark_int_pri < _BCM_QOS_MAP_EGR_REMARK_L3_DSCP_MAX; remark_int_pri++) { 
             bcm_qos_map_t_init(&qos_map);
             qos_map.remark_int_pri = remark_int_pri;
             if (!SHR_BITGET(SOC_DPP_STATE(unit)->qos_state->egr_remark_encap_ipv4_bitmap, _DPP_QOS_ENTRY_INDEX_EGR_REMARK_ENP_L3(profile_ndx, qos_map.remark_int_pri))) {
                 continue;
             }
           ...
           for (remark_int_pri = 0; remark_int_pri < _BCM_QOS_MAP_EGR_REMARK_L3_DSCP_MAX; remark_int_pri++) { 
             bcm_qos_map_t_init(&qos_map);
             qos_map.remark_int_pri = remark_int_pri;
             if (!SHR_BITGET(SOC_DPP_STATE(unit)->qos_state->egr_remark_encap_ipv6_bitmap, _DPP_QOS_ENTRY_INDEX_EGR_REMARK_ENP_L3(profile_ndx, qos_map.remark_int_pri))) {
                 continue;
             }
             rv = bcm_petra_qos_map_delete(unit, BCM_QOS_MAP_ENCAP | BCM_QOS_MAP_L3 | BCM_QOS_MAP_IPV6, &qos_map, map_id);
             if (rv != BCM_E_NONE) {
           ...
           for (color = 0; color < SOC_DPP_CONFIG(unit)->qos.dp_max; color++) {
             ...
             qos_map.color = color;
             qos_map.int_pri = int_pri;
             if (!SHR_BITGET(SOC_DPP_STATE(unit)->qos_state->egr_remark_mpls_bitmap, _DPP_QOS_ENTRY_INDEX_EGR_REMARK_MPLS(profile_ndx, qos_map.int_pri, qos_map.color))) {
                 continue;
             }
             rv = bcm_petra_qos_map_delete(unit, BCM_QOS_MAP_MPLS, &qos_map, map_id);
             if (rv != BCM_E_NONE) {
           ...
           for (color = 0; color < SOC_DPP_CONFIG(unit)->qos.dp_max; color++) {
             ...
             qos_map.color = color;
             qos_map.int_pri = int_pri;
             if (!SHR_BITGET(SOC_DPP_STATE(unit)->qos_state->egr_remark_ipv6_bitmap, _DPP_QOS_ENTRY_INDEX_EGR_REMARK_L3(profile_ndx, qos_map.int_pri, qos_map.color))) {
                 continue;
             }
             rv = bcm_petra_qos_map_delete(unit, BCM_QOS_MAP_L3|BCM_QOS_MAP_IPV6, &qos_map, map_id);
             if (rv != BCM_E_NONE) {
           ...
           for (color = 0; color < SOC_DPP_CONFIG(unit)->qos.dp_max; color++) {
             ...
             qos_map.color = color;
             qos_map.int_pri = int_pri;
             if (!SHR_BITGET(SOC_DPP_STATE(unit)->qos_state->egr_remark_ipv4_bitmap, _DPP_QOS_ENTRY_INDEX_EGR_REMARK_L3(profile_ndx, qos_map.int_pri, qos_map.color))) {
                 continue;
             }
    ...
    if (rv != BCM_E_NONE) {
        ...
        for (idx = 0; idx < _BCM_QOS_MAP_EGR_EXP_MAX; idx++) {
            qos_map.int_pri = idx;
            if (SHR_BITGET(SOC_DPP_STATE(unit)->qos_state->egr_mpls_php_ipv4_bitmap, _DPP_QOS_ENTRY_INDEX_EGR_MPLS_PHP(profile_ndx, qos_map.int_pri))) {
            ...
            if (SHR_BITGET(SOC_DPP_STATE(unit)->qos_state->egr_mpls_php_ipv6_bitmap, _DPP_QOS_ENTRY_INDEX_EGR_MPLS_PHP(profile_ndx, qos_map.int_pri))) {
...
_bcm_petra_qos_egress_gport_map_set(int unit, bcm_gport_t gport, int egr_map)
{
    ...
    if (BCM_INT_QOS_MAP_IS_REMARK(egr_map)) {
        ...
        rv = MBCM_PP_DRIVER_CALL(unit, mbcm_pp_profile_mgmt_if_profile_val_set, 
                                 (unit,SOC_PPC_PROFILE_MGMT_TYPE_OUTLIF, SOC_PPC_PROFILE_MGMT_OUTLIF_PROFILE_TYPE_OAM_PCP , BCM_QOS_MAP_PROFILE_GET(egr_map),&full_outlif_profile));
        BCMDNX_IF_ERR_EXIT(rv); 
    ...
    if (BCM_INT_QOS_MAP_IS_EGRESS_PCP_VLAN(egr_map)) {
        ...
        if (profile_ndx >= SOC_DPP_CONFIG(unit)->qos.nof_egr_pcp_vlan) {
            ...
            BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_i) {
                ...
                soc_sand_rv = soc_ppd_port_info_get(dev_id, core, soc_ppd_port, &port_info);
                BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
