...
#define _BCM_L2_EGRESS_SUPPORTED_FLAGS (BCM_L2_EGRESS_DEST_MAC_PREFIX5_REPLACE)
                                        ...
                                        )
{
    ...
    */
    ...
    if (!(flags & BCM_L2_DELETE_NO_CALLBACKS) || trav_fn != NULL || SOC_DPP_IS_EM_SIM_ENABLE(unit)) {
        hw_acc = FALSE;
    }
...
_bcm_petra_l2_egress_create_verify(int unit, bcm_l2_egress_t *egr) {
    ...
    if (SOC_IS_ARADPLUS_A0(unit) && (egr->ethertype != L2_EGRESS_ETHERTYPE_IP)) {
...
bcm_petra_l2_egress_create(int unit, bcm_l2_egress_t *egr)
{
    ...
    if (egr->flags & BCM_L2_EGRESS_WITH_ID) {
        outlif = _BCM_PETRA_L3_ITF_VAL_GET(egr->encap_id); 
    ...
    _BCM_PETRA_L3_ITF_SET(egr->encap_id,_BCM_PETRA_L3_ITF_ENCAP, outlif); 
    ...
    if (SOC_IS_JERICHO(unit)) {
        ...
        if (update) {
            ...
            /* get the current index for CfgEtherTypeIndex from eedb entry */
            rv = _bcm_petra_l2_egress_eedb_roo_ll_eth_type_get(unit, outlif, &old_eth_type_index_ndx); 
            BCMDNX_IF_ERR_EXIT(rv);
    ...
    bcm_l2_egress_t *egr)
{
    ...
    outlif = _BCM_PETRA_L3_ITF_VAL_GET(encap_id); 

    /* Check in SW DB that the LIF is LL */
    BCMDNX_IF_ERR_EXIT(_bcm_dpp_lif_usage_get(unit, -1, outlif, NULL, &usage));
    if (usage != _bcmDppLifTypeOverlayLinkLayer) {
    ...
    rv = bcm_dpp_am_l3_eep_is_alloced(unit, outlif);
    if (rv != BCM_E_EXISTS) {
       BCMDNX_IF_ERR_EXIT(rv);
    }
    ...
    if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
        ...
        rv = soc_ppd_eg_encap_entry_get(
           unit, SOC_PPC_EG_ENCAP_EEP_TYPE_DATA, outlif, 0, encap_entry_info, next_eep, &nof_entries);
        BCM_SAND_IF_ERR_EXIT(rv);
    ...
    bcm_if_t encap_id)
{
    ...
    /* free used outlif (SW) */
    rv = bcm_dpp_am_l3_eep_dealloc(unit, outlif);
    BCMDNX_IF_ERR_EXIT(rv);

    /* free used eedb entry */
    if (SOC_IS_ARADPLUS_A0(unit)) {
        rv = soc_ppd_eg_encap_entry_remove(unit, SOC_PPC_EG_ENCAP_EEP_TYPE_DATA, outlif);
        BCM_SAND_IF_ERR_EXIT(rv);
    } else {
        SOC_PPD_EG_ENCAP_ENTRY_INFO encap_entry_info[SOC_PPD_NOF_EG_ENCAP_EEP_TYPES];
        uint32 next_eep[SOC_PPD_NOF_EG_ENCAP_EEP_TYPES];
        ...
        rv = soc_ppd_eg_encap_entry_remove(unit, SOC_PPC_EG_ENCAP_EEP_TYPE_ROO_LL, outlif);
        BCM_SAND_IF_ERR_EXIT(rv);
    ...
    rv = _bcm_dpp_out_lif_match_delete(unit, outlif);
    BCMDNX_IF_ERR_EXIT(rv);
