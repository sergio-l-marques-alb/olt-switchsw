                                    ...
                                    uint8 *mirror_enable)
{
    ...
    *mirror_port_vlan = mirror_num;
    ...
    SOC_SAND_IF_ERR_RETURN(soc_ppd_eg_mirror_port_dflt_get(unit, core, pp_port,  &config));
...
{   
    ...
    if (mirror_enable) {    
        ...
        result = MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_port_to_interface_map_set,(unit, core, pp_port, SOC_TMC_PORT_DIRECTION_INCOMING, &port2if_mapping_info));
        BCMDNX_IF_ERR_EXIT(soc_sand_get_error_code_from_error_word(result));
            
        /* Step 3: port header type set */
        result = MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_port_header_type_set,(unit, core, pp_port, SOC_TMC_PORT_DIRECTION_INCOMING, SOC_TMC_PORT_HEADER_TYPE_RAW));
        BCMDNX_IF_ERR_EXIT(soc_sand_get_error_code_from_error_word(result));
        ...
        result = MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_port_pp_port_get,(unit, core, pp_port, &pp_port_info));
        BCMDNX_IF_ERR_EXIT(soc_sand_get_error_code_from_error_word(result));  
        
        pp_port_info.header_type = SOC_TMC_PORT_HEADER_TYPE_RAW;
        result = MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_port_pp_port_set,(unit, core, pp_port, &pp_port_info, &failure));
        BCMDNX_IF_ERR_EXIT(soc_sand_get_error_code_from_error_word(result));        
                               ...
                               bcm_gport_t mirror_dest)
{
    ...
    if (flags & BCM_MIRROR_PORT_EGRESS) { /* handle outbound mirroring */
        /* check that the port is not already mirrored */
        SOC_SAND_IF_ERR_RETURN(soc_ppd_eg_mirror_port_dflt_get((unit), core, pp_port, &config));
        found_profile = config.mirror_command;
        ...
        SOC_SAND_IF_ERR_RETURN(soc_ppd_eg_mirror_port_dflt_set((unit), core, pp_port,  &config));
    }
                                    ...
                                    bcm_gport_t mirror_dest)
{
    ...
    if (flags & BCM_MIRROR_PORT_EGRESS) { /* handle outbound mirroring */
        /* check that the port+vlan is not already mirrored */
        SOC_SAND_IF_ERR_RETURN(soc_ppd_eg_mirror_port_vlan_get((unit), loc_port, vlan, &config));
        if (config.mirror_command) {
...
#else
        if (config.mirror_command > DPP_MIRROR_ACTION_NDX_MAX || config.mirror_command < DPP_MIRROR_ACTION_NDX_MIN) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_INTERNAL, (_BSL_BCM_MSG("retrieved mirror destination %u is out of range"),(unsigned)config.mirror_command));
        }
        if (config.mirror_command != action_profile_id) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_NOT_FOUND, (_BSL_BCM_MSG("The outbound port-vlan combination has destination profile %u, and not the specified one"),
              (unsigned)config.mirror_command));
        }
#endif
        /* call dpp to remove the port-vlan mirroring */
    ...
    *mirror_dest_count = 0;
    ...
    if (flags & BCM_MIRROR_PORT_EGRESS) { /* handle outbound mirroring */
        ...
        if (SOC_IS_PETRAB(unit)) {
            ...
            BCMDNX_IF_ERR_EXIT(_bcm_petrab_mirror_port_outbound_get(unit, port, &mirr_profile, &mirr_port, &mirr_port_vlan, &mirr_enable)); 
            config.mirror_command = mirr_profile;
        }            
#else
        if (config.mirror_command > DPP_MIRROR_ACTION_NDX_MAX || config.mirror_command < DPP_MIRROR_ACTION_NDX_MIN) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_INTERNAL, (_BSL_BCM_MSG("retrieved mirror destination %u is out of range"),(unsigned)config.mirror_command));
...
#endif
        BCMDNX_IF_ERR_EXIT(_bcm_petra_is_mirror_dest_created(unit, config.mirror_command, &is_created));
        if (!is_created) { /* verify that the destination already exists */
            BCMDNX_ERR_EXIT_MSG(BCM_E_INTERNAL, (_BSL_BCM_MSG("retrieved mirroring destination ID %u is not created"), (unsigned)config.mirror_command));
        }
        if (*mirror_dest_count >= mirror_dest_size) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_MEMORY, (_BSL_BCM_MSG("output size is too small")));
        }
        BCM_GPORT_MIRROR_SET(*mirror_dest, (bcm_gport_t)config.mirror_command);
        ++*mirror_dest_count;
    }
