...
do { \
    ...
    if (!_dpp_policer_state[unit]->lock) { \
...
do { \
    if (sal_mutex_take(_dpp_policer_state[unit]->lock, sal_mutex_FOREVER)) { \
...
do { \
    if(1 == _lock_taken) { \
        _lock_taken = 0;  \
        if (sal_mutex_give(_dpp_policer_state[unit]->lock)) { \
...
_dpp_policer_state_t *_dpp_policer_state[BCM_MAX_NUM_UNITS];

#define _DPP_POLICER_INFO(_unit_)   ((_dpp_policer_state[_unit_]))
        ...
        int                        policer_id)
{
    ...
    if (!is_eth_policer) {
        switch (pol_cfg->mode) {
        case bcmPolicerModeSrTcm: /* eir has to be zero in single rate */
   ...
   ) 
{
    ...
    if (pol_cfg.ckbits_sec == 0) { /* zero, means disable credits, yet set to minimum cir */
        ...
        if (SOC_IS_ARAD(unit) && dpp_profile_info->is_sharing_enabled) {
            dpp_profile_info->max_cir = dpp_profile_info->cir;
        }
...
bcm_petra_policer_detach(int unit) 
{
    ...
    BCM_DPP_UNIT_CHECK(unit);
    if (!_dpp_policer_state[unit]) {
        /* not init, so detach done already */
        BCM_RETURN_VAL_EXIT(BCM_E_NONE);
    }
    /* get rid of allocated structure */
    BCM_FREE(_dpp_policer_state[unit]);

#ifdef BCM_WARM_BOOT_SUPPORT
    result = _bcm_dpp_wb_policer_state_deinit(unit);
#endif
...
{
    int rv = BCM_E_NONE;
    _dpp_policer_state_t *temp_state = NULL;
    SOC_PPD_MTR_BW_PROFILE_INFO   dpp_profile_info;
    SOC_PPD_MTR_BW_PROFILE_INFO   exact_dpp_profile_info;
    SOC_SAND_SUCCESS_FAILURE failure_indication;
    SOC_PPD_MTR_METER_ID meter_id;
    ...
    BCMDNX_ALLOC(temp_state, sizeof(_dpp_policer_state_t), "bcm_petra_policer_init");
    if (temp_state) {
        sal_memset(temp_state, 0, sizeof(_dpp_policer_state_t));
    ...
    SOC_DPP_STATE(unit)->policer_state = _dpp_policer_state[unit] = temp_state;
...
#ifdef BCM_WARM_BOOT_SUPPORT
    rv = _bcm_dpp_wb_policer_state_init(unit);
    BCMDNX_IF_ERR_EXIT(rv);
#endif
...
int bcm_petra_policer_group_create(
   ...
   int *npolicers) 
{
    _dpp_policer_state_t *policer_info = _DPP_POLICER_INFO(unit);
    int rv = BCM_E_NONE;
    BCMDNX_INIT_FUNC_DEFS;
    ...
    policer_info->policer_group[*policer_id].mode = mode;

#ifdef BCM_WARM_BOOT_SUPPORT
    BCMDNX_IF_ERR_EXIT(_bcm_dpp_wb_policer_update_group_mode_state(unit, *policer_id));
#endif
...
{
    int policer_indx;
    _dpp_policer_state_t *policer_info;
    int num_to_alloc = 0;
    bcm_policer_group_mode_t mode;
    ...
    DPP_POLICER_INIT_CHECK(unit);
    policer_info = _DPP_POLICER_INFO(unit);
    ...
    if (policer_id != 0) {
        ...
        /* policer to mode */
        mode = policer_info->policer_group[policer_indx].mode;
