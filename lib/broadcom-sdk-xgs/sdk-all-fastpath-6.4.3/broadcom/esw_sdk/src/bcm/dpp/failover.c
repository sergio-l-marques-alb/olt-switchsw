 ...
 * Returns:
 *      FALSE - The failover ID is Invalid
 *      TRUE -  The failover ID is Valid
 */
    ...
    int    unit)
{
    ...
    /* Set the Number of Faiover tables to reset according to the device.
    ...
    for (failover_type_idx = 0; failover_type_idx < nof_failover_tables; ++failover_type_idx ) {
        ...
        /* Perform any Jericho specific reset */
        if (SOC_IS_JERICHO(unit)) {
            /* Init the Jericho failover SW DB with Failover ID sequence for each Failover ID */
            if (failover_type == DPP_FAILOVER_TYPE_ENCAP) {
                rv = soc_dpp_wb_engine_init_buffer(unit, SOC_DPP_WB_ENGINE_BUFFER_JER_FAILOVER_SW_DB);
                BCMDNX_IF_ERR_EXIT(rv);
            }
        }
    }
    ...
    int    unit)
{
    ...
    /* 
     * SW: destroy shr_mres_type used for Failover-id
     */
    rv = bcm_petra_failover_reset(unit);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    bcm_failover_t *failover_id)
{
    ...
    SOC_PPD_FRWRD_FEC_ENTRY_INFO    hw_fec_entry_info;
    ...
    if ((SOC_IS_ARAD(unit)) && (failover_type == DPP_FAILOVER_TYPE_L2_LOOKUP)) {
        ...
        if (!(flags & BCM_FAILOVER_REPLACE)) {
            ...
            SOC_PPD_FRWRD_FEC_ENTRY_INFO_clear(&hw_fec_entry_info);
        ...
        if (failover_type == DPP_FAILOVER_TYPE_ENCAP) {
            rv = SOC_DPP_WB_ENGINE_SET_ARR(unit, SOC_DPP_WB_ENGINE_VAR_JER_FAILOVER_ID_SEQ_SIZE, (uint8 *)&nof_alloc_ids, tmp_failover_id);
            BCMDNX_IF_ERR_EXIT(rv);
        }
    ...
    bcm_failover_t  failover_id)
{
    ...
    if ((SOC_IS_ARAD(unit)) && (DPP_FAILOVER_IS_L2_LOOKUP(failover_id))) {
        /* Free the HW Entry */
    ...
    if (failover_type == DPP_FAILOVER_TYPE_ENCAP) {
        rv = SOC_DPP_WB_ENGINE_GET_ARR(unit, SOC_DPP_WB_ENGINE_VAR_JER_FAILOVER_ID_SEQ_SIZE, &nof_alloc_ids, failover_id_val);
        BCMDNX_IF_ERR_EXIT(rv);

        rv = SOC_DPP_WB_ENGINE_SET_ARR(unit, SOC_DPP_WB_ENGINE_VAR_JER_FAILOVER_ID_SEQ_SIZE, (uint8 *)&reset_value, failover_id_val);
        BCMDNX_IF_ERR_EXIT(rv);
    }
    ...
    int    enable)
{
    ...
    rv = _bcm_dpp_failover_state_hw_set(unit, failover_id_val, failover_type, failover_status);
    BCMDNX_IF_ERR_EXIT(rv);
