...
int mvpn_run(int unit) {
    ...
    /* AC configuration:
     * - at the 2nd pass, AC lif is resolved by port x vlan.
    ...
    /* the match criteria is port x vlan */
    vp1.criteria = BCM_VLAN_PORT_MATCH_PORT_VLAN;                                                                                                 
    vp1.port = cint_mvpn_info.recycle_port;                                                                                                       
    vp1.match_vlan = cint_mvpn_info.internal_vlan;                                                                                                               
    vp1.egress_vlan = cint_mvpn_info.internal_vlan;                                                                                                              
    vp1.flags = 0;                                                                                                                                
    vp1.vsi = cint_mvpn_info.internal_vlan;                                                                                                                      
    rv = bcm_vlan_port_create(unit,&vp1);                                                                                                         
    if (rv != BCM_E_NONE) {                                                                                                                       
        printf("Error, bcm_vlan_port_create\n");                                                                                                  
        return rv;                                                                                                                                
    }                                                                                                                                             
    printf("AC lif for the overlay at the 2nd pass configured: port(%d) x vlan(%d) -> inLif -> vsi (%d) \n ", vp1.port, vp1.match_vlan, vp1.vsi); 
