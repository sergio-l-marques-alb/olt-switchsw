...
int is_arad_a0(int unit, int *yesno)
{
  bcm_info_t info;
  ...
  *yesno = ((info.device == 0x8650) && (info.revision == 0)   ) ;
...
int oam_example(int unit) {
  ...
  */
  ...
  if (is_arad_plus==0) {
      src_mac_mep_2[5] = port_2; /* In Arad non Plus the LSB of the src mac address must equal the local port. No such restriction in Arad+.*/
  }
...
int register_events(int unit) {
  ...
  rv = bcm_oam_event_register(0, timein_event, cb_oam, (void*)5);
  if (rv != BCM_E_NONE) {
      printf("(%s) \n",bcm_errmsg(rv));
      return rv;
  }
  ...
  rv = bcm_oam_event_register(0, timein_event, cb_oam, (void*)6);
  if (rv != BCM_E_NONE) {
      printf("(%s) \n",bcm_errmsg(rv));
      return rv;
  }
