...
l3_vrrp_test(int unit, int is_arad, int max_nof_vlans, int ipv6_distinct)
{
    ...
    if (is_aradplus && count != 1 && vrids[0] != vrid0) {
        printf ("Incorrect count or incorrect vrid. \n");
        return BCM_E_FAIL;
    }
 ...
 */
int create_traffic_mapping(int unit, int in_port, int out_port, int in_vlan){
    int rv;
    int ing_intf_in; 
    int ing_intf_out; 
    int fec;      
    ...
    rv = create_l3_intf(unit, flags, open_vlan, in_port, out_vlan, 0, mac_address, &ing_intf_out); 
    if (rv != BCM_E_NONE) {
        printf("Error, create ingress interface-1, in_port=%d, \n", in_port);
    }
...
int traffic_example(int unit, int in_port, int out_port, int max_nof_vlans, int ipv6_distinct){
    ...
    if (ipv6_distinct) {
        /* add VRRP mac_lsb for in_vlan, once for IPV6 and once for IPV4
           for IPV4 only add also VRRP mac_lsb+1 for in_vlan
           for IPV6 only add also VRRP mac_lsb+2 for in_vlan */
        ...
        rv = bcm_l3_vrrp_config_add(unit, vrrp_flags, in_vlan, vrid+1);
        if (rv != BCM_E_NONE) {
            printf("Error. bcm_l3_vrrp_config_add with vlan %d and vrid %d\n", in_vlan, vrid+1);
            return BCM_E_FAIL;
        }
        ...
        rv = bcm_l3_vrrp_config_add(unit, vrrp_flags, in_vlan, vrid+2);
        if (rv != BCM_E_NONE) {
            printf("Error. bcm_l3_vrrp_config_add with vlan %d and vrid %d\n", in_vlan, vrid+2);
            return BCM_E_FAIL;
        }
    ...
    rv = create_traffic_mapping(unit, in_port, out_port, in_vlan);
    if (rv != BCM_E_NONE) {
