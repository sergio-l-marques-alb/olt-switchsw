...
struct vxlan_roo_s {
    ...
    int enable_add_entry_routing_table; /* add entry to routing table: not available for arad+ */
};
...
int vxlan_roo_run(int unit, int in_port, int out_port, int vpn_id, int add_routing_table_entry){
    ...
    ip_tunnel_term_glbl_info.skip_ethernet_flag = (!is_jericho);
   ...
   */
  printf("Create FEC for native routing. \n"); 
  rv = create_l3_egress(unit, 
                   0, /* flags */
                   out_port,  
                   out_vlan, /* out-vlan */
  ...
  if (g_vxlan_roo.enable_add_entry_routing_table) {
      ...
      rv = create_l3_egress(unit, 
                       0,  /* flags             */
                       vxlan_port_id[1], /* dest: fec-id instead of port */
                       out_vlan, /* out-vlan */
                       native_ing_intf, /* l3_intf  */
                       native_next_hop, /* next-hop */
