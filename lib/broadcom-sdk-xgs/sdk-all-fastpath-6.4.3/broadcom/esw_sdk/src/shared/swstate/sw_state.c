...
int soc_sw_state_deinit(int unit) {
    /* clean the sw_state pointer */
    /* actual space will be free during scache detach */
    sw_state[0] = NULL;
 ...
 *   soc_sw_state_free
 * FUNCTION:
 *   allocate space from the sw_state data block. space is not alloced by the operating system.
 *   a big chunk of memory was alloced during init in order to store all the future sw state.
...
soc_sw_state_free(int unit, uint8 *ptr) {
    ...
    /* iterate and remove all occurences of ptr in the active ptr array */
...
int soc_sw_state_init(int unit, int flags, soc_sw_state_init_mode_t init_mode, uint32 size) {
    ...
    switch (init_mode) {
        ...
        case socSwStateDataBlockRestoreAndOveride:
            ...
            /* fix ptr value of next free slot (since ptr values have chnged due to the restore)*/
            soc_sw_state_data_block_header[unit]->next_free_data_slot += (soc_sw_state_data_block_header[unit]->data_ptr - restored_data_ptr);

            

            /* caculate and change restored (invalid) ptrs to represent ptrs in the new data block */       
            for (indx = 0; indx < soc_sw_state_data_block_header[unit]->next_free_offset_list_idx; indx++) {
                ...
                /* from the recovered pointer stored at offset substract the restored base ptr and add the new base ptr */
