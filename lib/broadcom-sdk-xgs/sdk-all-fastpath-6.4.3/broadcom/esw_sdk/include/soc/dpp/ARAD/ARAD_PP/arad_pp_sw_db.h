...
typedef struct
{
  ARAD_PP_SW_DB_OAM_MY_MAC_LSB_REF_COUNT ref_count;
...
typedef struct
{
  uint8 llp_mirror_port_vlan[ARAD_PORT_NOF_PP_PORTS]; /* bitmap storing whether port x vlan pairs were added to be mirrored */
  SOC_SAND_MULTI_SET_INFO mirror_profile_multi_set;
} ARAD_PP_SW_DB_LLP_MIRROR;

typedef struct
{
  uint32 vid_sa_based_enable[ARAD_PP_SW_DB_PP_PORTS_NOF_U32];
...
typedef struct
{
  uint8 eg_mirror_port_vlan[ARAD_PORT_NOF_PP_PORTS]; /* bitmap storing whether port x vlan pairs were added to be mirrored */
  SOC_SAND_MULTI_SET_INFO mirror_profile_multi_set;
...
typedef struct
{
  ARAD_PP_FRWRD_FEC_GLBL_INFO glbl_info;
  uint8 fec_entry_type[SOC_DPP_DEFS_MAX(NOF_FECS)];
...
typedef struct
{
  ARAD_PP_SW_DB_OAM_MY_MAC_LSB *oam_my_mac_lsb;

  ARAD_PP_MGMT_OPERATION_MODE *oper_mode;
  ARAD_PP_SW_DB_LLP_FILTER *llp_filter;

  ARAD_PP_SW_DB_LLP_TRAP *llp_trap;
  ARAD_PP_SW_DB_LLP_MIRROR *llp_mirror;
  ARAD_PP_SW_DB_LLP_VID_ASSIGN *llp_vid_assign;
  ARAD_PP_SW_DB_PON_DOUBLE_LOOKUP *pon_double_lookup;
  ...
  ARAD_PP_SW_DB_ILM_INFO *ilm_info;

  ARAD_PP_SW_DB_FWD_MACT *fwd_mact;

  ARAD_PP_SW_DB_LIF_COS *lif_cos;

  ARAD_PP_SW_DB_LIF_TABLE *lif_table;

  ARAD_PP_SW_DB_FEC *fec;

  ARAD_PP_SW_DB_DIAG *diag;

  ARAD_PP_SW_DB_ISEM *isem;

  ARAD_PP_SW_DB_LAG *lag;

  ARAD_PP_SW_DB_L2_LIF *l2_lif;

  ARAD_PP_SW_DB_L2_LIF_AC *l2_lif_ac;

  ARAD_PP_SW_DB_VRRP_INFO *vrrp_info;

  ARAD_PP_SW_DB_MAC_LIMIT_PER_TUNNEL_INFO *mac_limit_info;

  ARAD_PP_SW_DB_HEADER_DATA *header_data;

/* ARAD+ data*/
  ARAD_PP_SW_DB_RIF_TO_LIF_GROUP_MAP *rif_to_lif_group_map;

  ARAD_PP_SW_DB_SLB_CONFIG *slb_config;

  ARAD_PP_SW_DB_ECMP *ecmp_info;
...
uint32
  arad_pp_sw_db_lag_lb_key_mask_set(
    SOC_SAND_IN int unit,
    SOC_SAND_IN ARAD_PP_HASH_MASKS masks
  );
