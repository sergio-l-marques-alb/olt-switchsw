...
typedef enum
{
   ...
   */
  SOC_PPC_NOF_DIAG_LIF_LKUP_TYPES = 9
...
typedef struct
{
   ...
   *  work. Bitmap as decoded by SOC_PPD_DIAG_FLAVOR
   */
  uint32 flavor;
...
typedef struct
{
   ...
   *  Packet information
   */
  SOC_PPC_TRAP_PACKET_INFO pkt_info;

} SOC_PPC_DIAG_TRAP_PACKET_INFO;
...
typedef struct
{
   ...
   *  if the SOC_PPD_TRAP_CODE y condition was fulfilled. Note:
...
typedef struct
{
  SOC_SAND_MAGIC_NUM_VAR
  /*
   *  Relevant when diag_flavor = SOC_PPD_DIAG_FLAVOR_RAW
   */
  uint32 raw[6];
  /*
...
typedef struct
{
  ...
  SOC_PPC_FRWRD_EXTEND_P2P_FRWRD_INFO extend_p2p;
...
typedef struct
{
  ...
  SOC_TMC_PARSER_PKT_HDR_STK_TYPE hdr_type_arad;
...
typedef struct
{
  SOC_SAND_MAGIC_NUM_VAR
  /*
   *  Type of termination, Range/lookup.
   */
  SOC_PPC_DIAG_MPLS_TERM_TYPE term_type;
  /*
   *  MPLS label.
   */
  uint32 label;
  /*
   *  Returns in which Termination range (set by
   *  soc_ppd_mpls_term_label_range_set()) there was match. Note:
   *  [0]- for outer most label in the packet and so on. Note
   *  label that match a range not necessarily terminated. For
   *  such label to be terminated: the following should be
   *  occur:- all the previous labels were terminated- either
   *  soc_ppd_mpls_term_range_terminated_label_set() is set for
   *  the label in the range, or terminated using
   *  soc_ppd_rif_mpls_label_map_add()
   */
  SOC_PPC_DIAG_TERM_MPLS_LABEL_INFO range_match;
  /*
   *  Which bit in termination was invoked
   */
  uint32 range_bit;
  /*
   *  Is this label bottom of stack
   */
  uint8 is_bos;

} SOC_PPC_DIAG_TERMINATED_MPLS_LABEL_INFO;

typedef struct
{
  SOC_SAND_MAGIC_NUM_VAR
  /*
   *  For each MPLS label in the packet, returns in which
   *  ranges it in match.
   */
  SOC_PPC_DIAG_TERMINATED_MPLS_LABEL_INFO lbls_info[3];

} SOC_PPC_DIAG_TERM_MPLS_INFO;
...
typedef struct
{
  ...
  /*
   *  MPLS termination info.
   */
  SOC_PPC_DIAG_TERM_MPLS_INFO mpls;
  /*
...
typedef struct
{
  ...
  SOC_PPC_FRWRD_DECISION_INFO frwrd_decision;
  /*
   *  Is forwarding decision valid, for example in PWE
   *  Multipoint or simple bridging, LIF forwarding decision
   *  is not relevant.
   */
  uint8 is_valid;
...
typedef struct
{
   ...
   *  interpreted as described in SOC_PPD_DIAG_PKT_TM_FIELD
   */
  uint32 valid_fields;
  /*
...
typedef struct
{
   ...
   *  Packet stack type
   */
  SOC_PPC_PKT_HDR_STK_TYPE type;
  /*
   *  The local TM port from which to emulate the packet was
   *  received from.
   */
  uint32 in_tm_port;
  /*
   ...
   */
  SOC_PPC_DIAG_ETH_PACKET_INFO ether_info;

} SOC_PPC_DIAG_PACKET_INFO;
...
typedef struct
{
   ...
   */ 
   SOC_SAND_PP_VLAN_TAG ac_tag2;
} SOC_PPC_DIAG_EGRESS_VLAN_EDIT_RES;
/* } */
/*************
 * GLOBALS   *
 *************/
