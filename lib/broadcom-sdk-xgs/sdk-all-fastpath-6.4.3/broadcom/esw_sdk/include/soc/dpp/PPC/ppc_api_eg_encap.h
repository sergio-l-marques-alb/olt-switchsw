...
#define  SOC_PPC_EG_ENCAP_DATA_INFO_MAX_SIZE                        (SOC_DPP_DEFS_MAX(NOF_EEBD_PAYLOADS) / SOC_SAND_NOF_BITS_IN_UINT32 + 1)
...
#define SOC_PPC_EG_ENCAP_NOF_BANKS                                 (16) 
#define SOC_PPC_EG_ENCAP_BANK_NDX_MAX                              (SOC_PPC_EG_ENCAP_NOF_BANKS-1) 

/* next eep pointer is invalid */
#define SOC_PPC_EG_ENCAP_NEXT_EEP_INVALID                        (0xffffffff)

/* update next-out-LIF */
#define SOC_PPC_EG_ENCAP_ENTRY_UPDATE_NEXT_LIF     (1)

/* update drop  */
#define SOC_PPC_EG_ENCAP_ENTRY_UPDATE_DROP     (2)

#define SOC_PPC_PRGE_DATA_ENTRY_LSBS_COUPLING_SWAP (1)
...
#define SOC_PPC_EG_ENCAP_EEDB_ERSPAN_FORMAT_SET(__unit, __prio, __trunc, __span_id, __data) \
            (__data)[1] = 0;                                                    \
            (__data)[0] = (((__prio << 21)) | ((__trunc) << 18) | ((__span_id) << 8) | SOC_PPC_PRGE_DATA_ENTRY_LSBS_ERSPAN);
  /* __span_id is 10b shifted 8b */
#define SOC_PPC_EG_ENCAP_EEDB_ERSPAN_FORMAT_SPAN_ID_GET(__span_id, __data) \
    (__span_id) = ((((__data)[0]) >> 8) & 0x3ff); 
...
typedef enum
{
   ...
   */
  SOC_PPC_NOF_EG_ENCAP_EEP_TYPES_ARAD = 7
...
typedef enum
{
   ...
   *  Number of types in SOC_PPC_EG_ENCAP_ENTRY_TYPE
   */
  SOC_PPC_NOF_EG_ENCAP_ENTRY_TYPES_ARAD = 14 
...
typedef enum
{
  ...
  /*
   *  Number of types in SOC_PPC_EG_ENCAP_ACCESS_PHASE
   */
  SOC_PPC_NOF_EG_ENCAP_ACCESS_PHASE_TYPES = 4
...
typedef struct
{
   ...
   */ 
  SOC_PPC_EG_ENCAP_ACCESS_PHASE bank_access_phase[SOC_PPC_EG_ENCAP_NOF_BANKS];
...
typedef struct
{
   ...
   *  SOC_PPD_MPLS_COMMAND_TYPE_POP_INTO_ETHERNET. Needed for the
...
typedef struct
{
   ...
   *  SOC_PPD_MPLS_COMMAND_TYPE_POP_INTO_ETHERNET. Needed to
...
typedef struct
{
   ...
   *  (SOC_PPD_EG_ENCAP_EXP_MARK_MODE_FROM_PUSH_PROFILE)Range: 0 -
