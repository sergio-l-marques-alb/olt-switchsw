 ...
 *
 * File:        wb_db_stg.h
 * Purpose:     WarmBoot - Level 2 support (STG Module)
 */

#ifndef _BCM_DPP_WB_DB_STG_H_
#define _BCM_DPP_WB_DB_STG_H_
 ...
 *
 * Currently there is a single handle for STG module.
...
#define BCM_DPP_WB_STG_VERSION_1_0            SOC_SCACHE_VERSION(1,0)
#define BCM_DPP_WB_STG_CURRENT_VERSION        BCM_DPP_WB_STG_VERSION_1_0


#define BCM_DPP_WB_STG_INFO(unit)             (_dpp_wb_stg_info_p[unit])
#define BCM_DPP_WB_STG_INFO_INIT(unit)        ( (_dpp_wb_stg_info_p[unit] != NULL) &&          \
                                                 (_dpp_wb_stg_info_p[unit]->init_done == TRUE) )
...
#define BCM_DPP_WB_DEV_STG_DIRTY_BIT_SET(unit)                                       \
                        SOC_CONTROL_LOCK(unit);                                       \
                        SOC_CONTROL(unit)->scache_dirty = 1;                          \
                        BCM_DPP_WB_STG_INFO(unit)->is_dirty = 1;                     \
                        SOC_CONTROL_UNLOCK(unit);

#define BCM_DPP_WB_STG_DIRTY_BIT_CLEAR(unit)                                         \
                        SOC_CONTROL_LOCK(unit);                                       \
                        BCM_DPP_WB_STG_INFO(unit)->is_dirty = 0;                     \
                        SOC_CONTROL_UNLOCK(unit);

#define BCM_DPP_WB_STG_IS_DIRTY(unit)                                                \
                        (BCM_DPP_WB_STG_INFO(unit)->is_dirty == 1)
 ...
 * runtime information
 */
typedef struct bcm_dpp_wb_stg_info_s {
    int            init_done;
    int            is_dirty;
    ...
    int            vlan_f_off;
    int            vlan_n_off;
} bcm_dpp_wb_stg_info_t;

extern bcm_dpp_wb_stg_info_t   *_dpp_wb_stg_info_p[BCM_MAX_NUM_UNITS];
