...
/********************************************************************************/
static err_code_t _merlin16_pcieg3_reg_print_no_buf(srds_access_t *sa__, uint8_t reg_section)
{
    uint16_t addr, rddata = 0;

    for (addr = merlin16_pcieg3_reg_dump_arr[reg_section][0]; addr < merlin16_pcieg3_reg_dump_arr[reg_section][1]; addr++) 
    {
        if (!(addr & 0x000F))  
        {
            EFUN_PRINTF(("\n%04x ",addr));
...
/********************************************************************************/
static err_code_t _merlin16_pcieg3_reg_print_with_buf(srds_access_t *sa__, uint8_t reg_section, char reg_buffer[][250], uint8_t *buf_index)
{
    uint16_t addr, rddata = 0;
	uint8_t first = 0;
    char temp_buf[16];
...
/********************************************************************************/
static err_code_t _merlin16_pcieg3_reg_print(srds_access_t *sa__, uint8_t reg_section, char reg_buffer[][250], uint8_t *buf_index)
{
    if(reg_buffer == NULL)
    {
         EFUN(_merlin16_pcieg3_reg_print_no_buf(sa__, reg_section));
    }
    else
    {
        EFUN(_merlin16_pcieg3_reg_print_with_buf(sa__, reg_section, reg_buffer, buf_index));
    }
    return (ERR_CODE_NONE);
}
...
{    
    ...
    while(reg_section < MERLIN16_PCIEG3_REG_DUMP_SECTIONS) 
    {
        if(merlin16_pcieg3_reg_dump_arr[reg_section][0] == MERLIN16_PCIEG3_REG_PLL_START)
        {
                while( reg_section < MERLIN16_PCIEG3_REG_DUMP_SECTIONS && merlin16_pcieg3_reg_dump_arr[reg_section][0] != MERLIN16_PCIEG3_REG_UC_START)
                {
                    EFUN(_merlin16_pcieg3_reg_print(sa__, reg_section, reg_buffer, &buf_index));
                    reg_section++;
                }
        }
        else if (merlin16_pcieg3_reg_dump_arr[reg_section][0] == MERLIN16_PCIEG3_REG_UC_START)
        {
            ...
            ESTM(micro_orig = merlin16_pcieg3_get_micro_idx(sa__));
            for(micro_idx = 0; micro_idx < num_micros; micro_idx++)
            {
                reg_section = uc_start_section;
                EFUN(merlin16_pcieg3_set_micro_idx(sa__,micro_idx));
#endif
                while(reg_section < MERLIN16_PCIEG3_REG_DUMP_SECTIONS)
                {
                    EFUN(_merlin16_pcieg3_reg_print(sa__, reg_section, reg_buffer, &buf_index));
            ...
            EFUN(merlin16_pcieg3_set_micro_idx(sa__,micro_orig));
#endif
        }
        else
        {
...
err_code_t merlin16_pcieg3_uc_core_var_dump(srds_access_t *sa__, uint8_t core, char ram_buffer[][250]) {
  ...
  if (ram_buffer == NULL) {
      ...
      for (addr = 0x0; addr < core_var_ram_size; addr++) {
          ...
          ESTM(rddata = merlin16_pcieg3_rdbc_uc_var(sa__, __ERR, addr));
...
err_code_t merlin16_pcieg3_uc_lane_var_dump(srds_access_t *sa__, uint8_t core, uint8_t lane, char ram_buffer[][250]) {
  ...
  if (ram_buffer == NULL) {
      ...
      for (addr = 0x0; addr < lane_var_ram_size; addr++) {
         ...
         ESTM(rddata = merlin16_pcieg3_rdbl_uc_var(sa__, __ERR, addr));
         USR_SPRINTF((tempBuf,"%02x ", rddata));
...
/*  Note: No EFUNs or ESTMs should be used in this function as this print routine is called by _error() handler. */
    merlin16_pcieg3_triage_info info;
    err_code_t error;
    ...
    UNUSED(error);
    USR_MEMSET(&info, 0, sizeof(merlin16_pcieg3_triage_info));
    info.error = err_code;
    info.line = line;
    if (print_header) {
    ...
    error = merlin16_pcieg3_version(sa__, &info.api_ver);
    if (error) {
    ...
    if(merlin16_pcieg3_INTERNAL_sigdet_status(sa__, &info.sig_det, &info.sig_det_chg)) {
        error_seen = 1;
    }
    if (merlin16_pcieg3_INTERNAL_pmd_lock_status(sa__, &info.pmd_lock, &info.pmd_lock_chg)) {
        error_seen = 1;
    }
    ...
    info.core = merlin16_pcieg3_get_core(sa__);
    info.lane = merlin16_pcieg3_get_lane(sa__);
    if (print_data) {
        if ((err_code == ERR_CODE_UC_CMD_POLLING_TIMEOUT) || (err_code == ERR_CODE_UC_NOT_STOPPED)) {
            USR_PRINTF(("%4d, %4d,  %X_%X,    %X_%X, %17d, %7d,%d       , %11d,   0x%04x, %8d, %6d,    0x%x,0x%x   , %s\n",
                        ...
                        info.dsc_one_hot[0],
                        info.dsc_one_hot[1],
                        merlin16_pcieg3_INTERNAL_e2s_err_code(info.error) ));
        } else {
