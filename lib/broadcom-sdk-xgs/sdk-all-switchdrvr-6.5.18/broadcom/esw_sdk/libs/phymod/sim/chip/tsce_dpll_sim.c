...
#ifndef EXCLUDE_PHYMOD_TSCE_DPLL_SIM 
...
#define TSCE_DPLL_BF_SET(_val, _mask, _shift) _val |= ((_mask) << (_shift))
#define TSCE_DPLL_BF_GET(_val, _mask, _shift) (((_val) >> (_shift)) & (_mask))

/*
 * Raw 32-bit address consists of AER value in upper 16 bits and
 * clause 45 address in lower 16 bits.
 */
#define TSCE_DPLL_DEVAD_SHIFT       27
#define TSCE_DPLL_DEVAD_MASK        0x1f
#define TSCE_DPLL_DEVAD_GET(_addr) \
    TSCE_DPLL_BF_GET(_addr, TSCE_DPLL_DEVAD_MASK, TSCE_DPLL_DEVAD_SHIFT)
#define TSCE_DPLL_LANE_SHIFT        16
#define TSCE_DPLL_LANE_MASK         0x7
#define TSCE_DPLL_LANE_GET(_addr) \
    TSCE_DPLL_BF_GET(_addr, TSCE_DPLL_LANE_MASK, TSCE_DPLL_LANE_SHIFT)
#define TSCE_DPLL_REG_SHIFT         0
#define TSCE_DPLL_REG_MASK          0xffff
#define TSCE_DPLL_REG_GET(_addr) \
    TSCE_DPLL_BF_GET(_addr, TSCE_DPLL_REG_MASK, TSCE_DPLL_REG_SHIFT)

#define TSCE_DPLL_ADDR(_devad, _lane, _reg) \
    (((_devad) << TSCE_DPLL_DEVAD_SHIFT) +  \
     ((_lane) << TSCE_DPLL_LANE_SHIFT) +    \
     ((_reg) << TSCE_DPLL_REG_SHIFT))

#define TSCE_DPLL_AER               TSCE_DPLL_ADDR(0, 0, 0xffde)
#define TSCE_DPLL_BLK               TSCE_DPLL_ADDR(0, 0, 0x001f)

/*
 * The CL45 indicator is used to determine whether the upper 16 bits
 * of the address is an AER value or a clause 45 DEVAD.
 */
#define TSCE_DPLL_CL45              (0x20 << 16)
#define TSCE_DPLL_CL45_MASK         (0xe0 << 16)

#define TSCE_DPLL_ID0               0x600d
#define TSCE_DPLL_ID1               0x8770
#define TSCE_DPLL_SERDES_ID         0x02e3

#define TSCE_DPLL_SIM_FW_LOAD_DONE_REG_ADDR (0x0800d205)
#define TSCE_DPLL_SIM_DSC_UC_CTRL_REG_ADDR (0x0800d00d)

/* Forward declarations */
STATIC int
tsce_dpll_sim_write(phymod_sim_data_t *pms_data, uint32_t addr, uint32_t data);
STATIC int
tsce_dpll_sim_read(phymod_sim_data_t *pms_data, uint32_t addr, uint32_t *data);

STATIC uint32_t
tsce_dpll_sim_default_data_get(uint32_t addr)
{
    uint32_t reg;

    reg = TSCE_DPLL_REG_GET(addr);

    /*force FW load success */
    if(addr == TSCE_DPLL_SIM_FW_LOAD_DONE_REG_ADDR) {
        return (1 << 15);
    }

    switch (reg) {
        case 0x0002:
            return TSCE_DPLL_ID0;
        case 0x0003:
            return TSCE_DPLL_ID1;
        case 0x900e:
            return TSCE_DPLL_SERDES_ID;
        case 0x9003:
            return 0xe4e4;
        case 0x925a:
            return 0x0002;
...
STATIC uint32_t
tsce_dpll_sim_reg_copies_get(uint32_t addr)
{
    uint32_t devad, reg;

    devad = TSCE_DPLL_DEVAD_GET(addr);
    reg = TSCE_DPLL_REG_GET(addr);

    if (reg == TSCE_DPLL_AER || reg == TSCE_DPLL_BLK) {
        return 1;
    }
...
STATIC int
_tsce_dpll_sim_write(phymod_sim_data_t *pms_data, uint32_t addr, uint32_t data)
{
    int idx;
    uint32_t mask;
    ...
    for (idx = 0; idx < pms_data->entries_used; idx++) {
        ...
        if (pse->addr == addr) {
            pse->data &= ~mask;
            pse->data |= data;
            DBG_VERB(("_tsce_dpll_sim_write 0x%08"PRIx32" = 0x%04"PRIx32"\n",
                      addr, pse->data));
    ...
    if (data == tsce_dpll_sim_default_data_get(addr)) {
        return PHYMOD_E_NONE;
    }
    ...
    DBG_VERB(("_tsce_dpll_sim_write 0x%08"PRIx32" = 0x%04"PRIx32" (new)\n",
              addr, pse->data));
...
STATIC uint32_t
tsce_dpll_sim_write_adjust(phymod_sim_data_t *pms_data, uint32_t addr, uint32_t data)
{
    ...
    if (devad == 0) {
        ...
        case 0xd127:
            ...
            addr = TSCE_DPLL_ADDR(devad, 0, reg);
            (void)_tsce_dpll_sim_write(pms_data, addr, data);

            addr = TSCE_DPLL_ADDR(devad, 1, reg);
            (void)_tsce_dpll_sim_write(pms_data, addr, data);

            addr = TSCE_DPLL_ADDR(devad, 2, reg);
            (void)_tsce_dpll_sim_write(pms_data, addr, data);

            addr = TSCE_DPLL_ADDR(devad, 3, reg);
            (void)_tsce_dpll_sim_write(pms_data, addr, data);
            break;
        case 0xd203:
             ...
             */
            _tsce_dpll_sim_write(pms_data, addr + 1, data);
            break;
        case 0xd0c1:
             ...
             * 0xF6D = 0xd0c1 - 0xc154*/
            tsce_dpll_sim_read(pms_data, (addr - 0xF6D) & 0x7ffff, &val);
            val = (val & 0xfffd) | (((data&0x80)==0x80)?0:0x2);
            tsce_dpll_sim_write(pms_data, (addr - 0xF6D) & 0x7ffff, val);
            break;
        default:
            break;
        }
...
STATIC int
tsce_dpll_sim_init(phymod_sim_data_t *pms_data,
...
STATIC int
tsce_dpll_sim_reset(phymod_sim_data_t *pms_data)
{
    uint32_t sim_size;
...
tsce_dpll_sim_read(phymod_sim_data_t *pms_data, uint32_t addr, uint32_t *data)
{
    ...
    if (addr < TSCE_DPLL_BLK) {
        /* Assume clause 22 access */
        (void)tsce_dpll_sim_read(pms_data, TSCE_DPLL_BLK, &blk);
        /* IEEE bit */
        if (addr & 0x10) {
            blk |= 0x8000;
        } else {
        ...
        if (addr != TSCE_DPLL_AER && addr != TSCE_DPLL_BLK) {
            (void)tsce_dpll_sim_read(pms_data, TSCE_DPLL_AER, &aer);
            addr |= (aer << 16);
        }
    } else {
        /* Extract devad if clause 45 address format */
        if ((addr & TSCE_DPLL_CL45_MASK) == TSCE_DPLL_CL45) {
            devad = (addr >> 16) & 0x1f;
            addr &= 0xffff;
        }
    }
    ...
    if (addr != TSCE_DPLL_AER && addr != TSCE_DPLL_BLK) {
        ...
        if (lane > 3) {
            ...
            addr = TSCE_DPLL_ADDR(TSCE_DPLL_DEVAD_GET(addr), 0, TSCE_DPLL_REG_GET(addr));
        }
    }
    ...
    devad = TSCE_DPLL_DEVAD_GET(addr);
    reg = TSCE_DPLL_REG_GET(addr);
    copies = tsce_dpll_sim_reg_copies_get(addr);
    if (copies == 1) {
        lane = 0;
    } else if (copies == 2) {
        lane &= ~0x1;
    }
    addr = TSCE_DPLL_ADDR(devad, lane, reg);

    /* for DSC_UC_CTRL always succeed */
    if(addr == TSCE_DPLL_SIM_DSC_UC_CTRL_REG_ADDR) {
        *data = 0x80;
        DBG_VERB(("tsce_dpll_sim_read 0x%08"PRIx32" = 0x%04"PRIx32"\n",
                      addr, *data));
        return PHYMOD_E_NONE;
    }
    ...
    *data = tsce_dpll_sim_default_data_get(addr);
...
tsce_dpll_sim_write(phymod_sim_data_t *pms_data, uint32_t addr, uint32_t data)
{
    ...
    if (addr != TSCE_DPLL_AER && addr != TSCE_DPLL_BLK) {
        ...
        if (lane > 3) {
            ...
            if (lane == 4 || lane == 6) {
                /* Write lanes 0 and 1 */
                addr = TSCE_DPLL_ADDR(devad, 8, reg);
                (void)tsce_dpll_sim_write(pms_data, addr, data);
                addr = TSCE_DPLL_ADDR(devad, 1, reg);
                (void)tsce_dpll_sim_write(pms_data, addr, data);
            }
            if (lane == 5 || lane == 6) {
    ...
    data = tsce_dpll_sim_write_adjust(pms_data, addr, data);
    ...
    return _tsce_dpll_sim_write(pms_data, addr, data);
}

STATIC int
tsce_dpll_sim_event(phymod_sim_data_t *pms_data, phymod_sim_event_t event)
{
    if (pms_data == NULL || pms_data->entries == NULL) {
        return PHYMOD_E_INIT;
    }
...
phymod_sim_drv_t tsce_dpll_sim_drv = {
    tsce_dpll_sim_init,
    tsce_dpll_sim_reset,
    tsce_dpll_sim_read,
    tsce_dpll_sim_write,
...
#endif /* EXCLUDE_PHYMOD_TSCE_DPLL_SIM */ 
