...
err_code_t blackhawk7_v1l8p1_display_eye_scan(srds_access_t *sa__) {
    ...
    ESTM(core=blackhawk7_v1l8p1_get_core(sa__));
    ESTM(lane=blackhawk7_v1l8p1_get_lane(sa__));
    ...
    {   err_code_t err_code = blackhawk7_v1l8p1_meas_eye_scan_start(sa__, EYE_SCAN_HORIZ);
        if (err_code) {
            if (err_code == ERR_CODE_DIAG_SCAN_NO_PMD_LOCK) {
                return(ERR_CODE_NONE);
            }
            else {
                EFUN((blackhawk7_v1l8p1_meas_eye_scan_done(sa__), err_code));
            }
        }
    }
    ...
    for (y = y_max;y>=-y_max;y=y-y_step)
    {
        ...
        EFUN(blackhawk7_v1l8p1_display_eye_scan_stripe(sa__, y,&stripe[0]));
        EFUN_PRINTF(("\n"));
    }
    /* stop acquisition */
...
err_code_t blackhawk7_v1l8p1_read_eye_scan_stripe(srds_access_t *sa__, uint32_t *buffer, uint16_t *status) {
    ...
    const uint8_t lane = blackhawk7_v1l8p1_get_lane(sa__);
    ...
    EFUN(blackhawk7_v1l8p1_INTERNAL_verify_blackhawk7_v1l8p1_info(sa__, blackhawk7_v1l8p1_info_ptr));
        ...
        lane_diag_base = blackhawk7_v1l8p1_info_ptr->diag_mem_ram_base + ((lane%blackhawk7_v1l8p1_info_ptr->lane_count) * blackhawk7_v1l8p1_info_ptr->diag_mem_ram_size) +
            blackhawk7_v1l8p1_info_ptr->grp_ram_size*blackhawk7_v1l8p1_INTERNAL_grp_idx_from_lane(blackhawk7_v1l8p1_get_physical_lane(sa__));

    EFUN(blackhawk7_v1l8p1_INTERNAL_poll_diag_data(sa__, blackhawk7_v1l8p1_info_ptr, status, &diag_rd_ptr, stripe_size, 400));
    ...
    diag_rd_ptr = ((uint32_t)diag_rd_ptr + stripe_size) % blackhawk7_v1l8p1_info_ptr->diag_mem_ram_size;
...
err_code_t blackhawk7_v1l8p1_display_eye_scan_header(srds_access_t *sa__, int8_t i) {
    ...
    if(blackhawk7_v1l8p1_info_ptr->info_table_version > 0x37) {
        uint16_t diag_sts;
        ESTM(diag_sts = rdv_usr_diag_status());
        if((diag_sts&DIAG_EYE_SCAN_RANGE_MASK) == DIAG_EYE_SCAN_HALF) {
            stepsize_var = 128;
        }
    }
    ...
    for(x=1;x<=i;x++) {
        EFUN_PRINTF(("  UI/%3d : -30  -25  -20  -15  -10  -5    0    5    10   15   20   25   30", stepsize_var));
    }
    EFUN_PRINTF(("\n"));
    for(x=1;x<=i;x++) {
        EFUN_PRINTF(("         : -|----|----|----|----|----|----|----|----|----|----|----|----|-"));
...
err_code_t blackhawk7_v1l8p1_eye_margin_proj(srds_access_t *sa__, USR_DOUBLE rate, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    ...
    ESTM(offset_start = rd_uc_dsc_data());
    {
        int16_t nlmv_val = 0;
        afe_override_slicer_sel_t slicer = INVALID_SLICER;
        EFUN(blackhawk7_v1l8p1_INTERNAL_ladder_setting_to_mV(sa__, (int16_t)offset_start, 0, slicer, &nlmv_val));
    ...
    EFUN(blackhawk7_v1l8p1_INTERNAL_poll_diag_done(sa__, &sts,timer_control*2000));
...
static err_code_t _display_ber_scan_data(srds_access_t *sa__, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    ...
    if(ber_scan_mode & DIAG_BER_HORZ) {
        ...
        EFUN(blackhawk7_v1l8p1_INTERNAL_ladder_setting_to_mV(sa__, (int16_t)offset_start, range250, slicer, &nlmv_val));
...
err_code_t blackhawk7_v1l8p1_display_diag_data(srds_access_t *sa__, uint16_t  diag_level) {
    ...
    if(diag_level & SRDS_DIAG_LANE) {
        ...
        EFUN(blackhawk7_v1l8p1_log_full_pmd_state(sa__, &lane_st));

        EFUN(blackhawk7_v1l8p1_disp_full_pmd_state(sa__, &lane_st, 1));
    }
    ...
    core = blackhawk7_v1l8p1_get_core(sa__);
    lane = blackhawk7_v1l8p1_get_lane(sa__);
    ...
    if(diag_level & SRDS_DIAG_UC_CORE) {
        ...
        {
        uint8_t orig_lane, num_micros, tmp_lane;

        ESTM(num_micros = rdc_micro_num_uc_cores());
        orig_lane = blackhawk7_v1l8p1_get_physical_lane(sa__);
        for (tmp_lane = 0; tmp_lane < num_micros*2; tmp_lane += 2) {
        EFUN(blackhawk7_v1l8p1_set_physical_lane(sa__, tmp_lane));

            EFUN(blackhawk7_v1l8p1_uc_micro_var_dump(sa__, core, tmp_lane, NULL));
        }
        EFUN(blackhawk7_v1l8p1_set_physical_lane(sa__, orig_lane));
        }
    }
    ...
    if((diag_level & SRDS_DIAG_PRBS_PROJ)
        ...
        EFUN_PRINTF(("\n********************************************************\n"));
        EFUN_PRINTF(("Running CORE %d LANE %d PRBS Error Analyzer for %d seconds\n", blackhawk7_v1l8p1_get_core(sa__), blackhawk7_v1l8p1_get_lane(sa__), timeout_s));
        EFUN_PRINTF(("********************************************************\n"));
        /* Display PRBS Error Analyzer for 60s */
        EFUN(blackhawk7_v1l8p1_diag_prbs_error_analyzer_proj(sa__, 5440, timeout_s));
    }
...
/* Required Diagnostic Functions */
err_code_t blackhawk7_v1l8p1_diag_access(srds_access_t *sa__, enum srds_diag_access_enum type, uint16_t addr, uint16_t data, uint16_t param) {
    ...
    case SRDS_REG_READ:      {
                             ...
                             if(data > 1) {
                                 ...
                                 for (i = 0; i < data; i++) {
                                     ...
                                     EFUN(blackhawk7_v1l8p1_pmd_rdt_reg(sa__, i+addr,&tmp));
                                     ESTM_PRINTF(("%04x ", tmp));
                                 }
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 EFUN(blackhawk7_v1l8p1_pmd_rdt_reg(sa__, addr,&tmp));
    ...
    case SRDS_REG_RMW:       {
                             ...
                             EFUN(blackhawk7_v1l8p1_pmd_wr_reg(sa__, addr,tmp2));
    ...
    case SRDS_LANE_RAM_READ_BYTE: {
                             if(data > 1) {
                                 ...
                                 EFUN(blackhawk7_v1l8p1_INTERNAL_rdblk_uc_generic_ram(sa__, (addr + blackhawk7_v1l8p1_info_ptr->lane_var_ram_base + (blackhawk7_v1l8p1_get_physical_lane(sa__) * blackhawk7_v1l8p1_info_ptr->lane_var_ram_size)),
                                                                           data,
                                                                           0,
                                                                           data,
                                                                           &state,
                                 ...
                                 ESTM_PRINTF(("Lane RAM Read byte: x%04x = x%02x\n",addr,blackhawk7_v1l8p1_rdbl_uc_var(sa__, __ERR,addr)));
                             }
                             } break;
    ...
    case SRDS_LANE_RAM_READ_WORD: {
                             if(data > 1) {
                                 ...
                                 ESTM_PRINTF(("Lane RAM Read word: x%04x = x%04x\n",addr,blackhawk7_v1l8p1_rdwl_uc_var(sa__, __ERR, addr)));
                             }
                             } break;
    ...
    case SRDS_LANE_RAM_RMW_BYTE:  {
                             uint8_t tmp, tmp2;
                             ESTM(tmp = blackhawk7_v1l8p1_rdbl_uc_var(sa__, __ERR,addr));
                             tmp2 = (tmp & (uint8_t)~param) | ((uint8_t)param & data);
                             EFUN(blackhawk7_v1l8p1_wrbl_uc_var(sa__, addr,tmp2));
    ...
    case SRDS_LANE_RAM_RMW_WORD:  {
                             uint16_t tmp, tmp2;
                             ESTM(tmp = blackhawk7_v1l8p1_rdwl_uc_var(sa__, __ERR,addr));
                             tmp2 = (tmp & ~param) | (param & data);
                             EFUN(blackhawk7_v1l8p1_wrwl_uc_var(sa__, addr,tmp2));
    ...
    case SRDS_GLOB_RAM_RMW_BYTE:  {
                             uint8_t tmp, tmp2;
                             ESTM(tmp = blackhawk7_v1l8p1_INTERNAL_rdb_uc_var(sa__, __ERR,addr));
                             tmp2 = (tmp & (uint8_t)~param) | ((uint8_t)param & data);
                             EFUN(blackhawk7_v1l8p1_INTERNAL_wrb_uc_var(sa__, addr,tmp2));
    ...
    case SRDS_GLOB_RAM_RMW_WORD:  {
                             uint16_t tmp, tmp2;
                             ESTM(tmp = blackhawk7_v1l8p1_INTERNAL_rdw_uc_var(sa__, __ERR,addr));
                             tmp2 = (tmp & ~param) | (param & data);
                             EFUN(blackhawk7_v1l8p1_INTERNAL_wrw_uc_var(sa__, addr,tmp2));
                             EFUN_PRINTF(("Glob RAM RMW word: x%04x = x%04x -> x%04x\n",addr,tmp,tmp2));
                             } break;
    case SRDS_UC_CMD:        {
                             uint16_t tmp;
                             EFUN(blackhawk7_v1l8p1_pmd_uc_cmd_with_data(sa__, (enum srds_pmd_uc_cmd_enum)addr, (uint8_t)param, data, GRACEFUL_STOP_TIME));
