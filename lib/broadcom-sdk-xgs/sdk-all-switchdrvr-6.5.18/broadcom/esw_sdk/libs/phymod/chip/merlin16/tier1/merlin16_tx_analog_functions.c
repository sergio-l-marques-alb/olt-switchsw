/*******************************************************************************************
 *******************************************************************************************
 *  File Name     :  merlin16_tx_analog_functions.c                                        *
...
/** @file merlin16_tx_analog_functions.c
...
err_code_t _txfir_tap_mapping(srds_access_t *sa__, int8_t pre, int8_t main, int8_t post1, int8_t post2) {
  ...
  dc_level = main - pre - calc_post1 - calc_post2;
...
err_code_t merlin16_apply_txfir_cfg(srds_access_t *sa__, int8_t pre, int8_t main, int8_t post1, int8_t post2) {
  ...
  uint8_t osr_mode;  
  err_code_t failcode = merlin16_validate_txfir_cfg(pre, main, post1, post2);
...
err_code_t merlin16_read_tx_afe(srds_access_t *sa__, enum merlin16_tx_afe_settings_enum param, int8_t *val) {
  if(!val) {
  ...
  switch(param) {
    case TX_AFE_PRE:
      EFUN(merlin16_INTERNAL_get_tx_pre(sa__, val));
      break;
    case TX_AFE_MAIN:
      EFUN(merlin16_INTERNAL_get_tx_main(sa__, val));
      break;
    case TX_AFE_POST1:
      EFUN(merlin16_INTERNAL_get_tx_post1(sa__, val));
      break;
    case TX_AFE_POST2:
      EFUN(merlin16_INTERNAL_get_tx_post2(sa__, val));
      break;
    default:
      return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
      break;
...
err_code_t merlin16_config_tx_hpf(srds_access_t *sa__, uint8_t val) {  /* KD */
...
err_code_t merlin16_rd_tx_hpf_config(srds_access_t *sa__, uint8_t *pval) {  /* KD */
...
static err_code_t _update_p2sr_reg_word(struct p2sr_reg_st *st) {
  uint16_t in = 0;
  in <<= 8; in |= 0/*st->field.reserved0*/      & BFMASK(8);
  in <<= 1; in |= st->field.dc_level_post2_2x_1 & BFMASK(1);
  in <<= 1; in |= st->field.post2_2x_1          & BFMASK(1);
  in <<= 1; in |= st->field.dc_level_post2_2x_0 & BFMASK(1);
  in <<= 1; in |= st->field.post2_2x_0          & BFMASK(1);
  in <<= 1; in |= st->field.dc_level_post2_1x_1 & BFMASK(1);
  in <<= 1; in |= st->field.post2_1x_1          & BFMASK(1);
  in <<= 1; in |= st->field.dc_level_post2_1x_0 & BFMASK(1);
  in <<= 1; in |= st->field.post2_1x_0          & BFMASK(1);
  st->word = in;
  return ERR_CODE_NONE;
}
...
static err_code_t _update_p1sr_reg_word(struct p1sr_reg_st *st) {
  ...
  in <<= 1; in |= st->field.dc_level_post1pre_1x_0 & BFMASK(1);
  in <<= 1; in |= st->field.post1_post1pre_1x_0    & BFMASK(1);
  in <<= 1; in |= st->field.pre_post1pre_1x_0      & BFMASK(1);
  in <<= 2; in |= 0/*st->field.reserved0*/         & BFMASK(2);
  in <<= 1; in |= st->field.dc_level_post1_2x_2    & BFMASK(1);
  in <<= 1; in |= st->field.post1_2x_2             & BFMASK(1);
  in <<= 1; in |= st->field.dc_level_post1_2x_1    & BFMASK(1);
  in <<= 1; in |= st->field.post1_2x_1             & BFMASK(1);
  in <<= 1; in |= st->field.dc_level_post1_2x_0    & BFMASK(1);
  in <<= 1; in |= st->field.post1_2x_0             & BFMASK(1);
  in <<= 1; in |= st->field.dc_level_post1_1x_1    & BFMASK(1);
  in <<= 1; in |= st->field.post1_1x_1             & BFMASK(1);
  in <<= 1; in |= st->field.dc_level_post1_1x_0    & BFMASK(1);
  in <<= 1; in |= st->field.post1_1x_0             & BFMASK(1);
  st->word = in;
  return ERR_CODE_NONE;
}
...
static err_code_t _update_presr_reg_word(struct presr_reg_st *st) {
  uint16_t in = 0;
  in <<= 1; in |= 0/*st->field.reserved0*/         & BFMASK(1);
  in <<= 1; in |= st->field.dc_level_post1pre_2x_3 & BFMASK(1);
  in <<= 1; in |= st->field.post1_post1pre_2x_3    & BFMASK(1);
  in <<= 1; in |= st->field.pre_post1pre_2x_3      & BFMASK(1);
  in <<= 1; in |= st->field.dc_level_post1pre_2x_2 & BFMASK(1);
  in <<= 1; in |= st->field.post1_post1pre_2x_2    & BFMASK(1);
  in <<= 1; in |= st->field.pre_post1pre_2x_2      & BFMASK(1);
  in <<= 1; in |= st->field.dc_level_post1pre_2x_1 & BFMASK(1);
  in <<= 1; in |= st->field.post1_post1pre_2x_1    & BFMASK(1);
  in <<= 1; in |= st->field.pre_post1pre_2x_1      & BFMASK(1);
  in <<= 1; in |= st->field.dc_level_post1pre_2x_0 & BFMASK(1);
  in <<= 1; in |= st->field.post1_post1pre_2x_0    & BFMASK(1);
  in <<= 1; in |= st->field.pre_post1pre_2x_0      & BFMASK(1);
  in <<= 1; in |= st->field.dc_level_post1pre_1x_1 & BFMASK(1);
  in <<= 1; in |= st->field.post1_post1pre_1x_1    & BFMASK(1);
  in <<= 1; in |= st->field.pre_post1pre_1x_1      & BFMASK(1);
  st->word = in;
  return ERR_CODE_NONE;
}
...
static err_code_t _update_mssr_reg_word(struct mssr_reg_st *st) {
  uint16_t in = 0;
  in <<=12; in |= 0/*st->field.reserved0*/ & BFMASK(12);
  in <<= 1; in |= st->field.dc_level_main_2x_1 & BFMASK(1);
  in <<= 1; in |= st->field.dc_level_main_2x_0 & BFMASK(1);
  in <<= 1; in |= st->field.dc_level_main_1x   & BFMASK(1);
  in <<= 1; in |= st->field.dc_level_0p5x      & BFMASK(1);
  st->word = in;
  return ERR_CODE_NONE;
}
