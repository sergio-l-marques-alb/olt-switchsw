...
err_code_t blackhawk7_v1l8p1_get_physical_tx_addr(srds_access_t *sa__, uint8_t lane, uint8_t *physical_tx_lane) {
  ...
  if (lane == 0) 
    ...
    ESTM(*physical_tx_lane = rdc_tx_lane_addr_4());
  else if (lane == 5) 
    ESTM(*physical_tx_lane = rdc_tx_lane_addr_5());
  else if (lane == 6) 
    ESTM(*physical_tx_lane = rdc_tx_lane_addr_6());
  else if (lane == 7) 
    ESTM(*physical_tx_lane = rdc_tx_lane_addr_7());
  return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p1_get_physical_rx_addr(srds_access_t *sa__, uint8_t lane, uint8_t *physical_rx_lane) {
  ...
  if (lane == 0) 
    ...
    ESTM(*physical_rx_lane = rdc_rx_lane_addr_4());
  else if (lane == 5) 
    ESTM(*physical_rx_lane = rdc_rx_lane_addr_5());
  else if (lane == 6) 
    ESTM(*physical_rx_lane = rdc_rx_lane_addr_6());
  else if (lane == 7) 
    ESTM(*physical_rx_lane = rdc_rx_lane_addr_7());
  return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p1_ucode_pram_load(srds_access_t *sa__, char const * ucode_image, uint32_t ucode_len) {
    ...
    blackhawk7_v1l8p1_ucode_load_init(sa__, 1);
    ...
    while (ucode_len > 0) {
        EFUN(blackhawk7_v1l8p1_pmd_wr_pram(sa__, *ucode_image));
        --ucode_len;
        ++ucode_image;
    }
    ...
    while (ucode_len_remainder > 0) {
        EFUN(blackhawk7_v1l8p1_pmd_wr_pram(sa__, 0));
        --ucode_len_remainder;
    }
    ...
    blackhawk7_v1l8p1_ucode_load_close(sa__, 1);
    return(ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p1_map_lanes(srds_access_t *sa__, uint8_t num_lanes, uint8_t const *tx_lane_map, uint8_t const *rx_lane_map) {
...

    {
        ...
        ESTM(micro_orig = blackhawk7_v1l8p1_get_micro_idx(sa__));
        ESTM(num_micros = rdc_micro_num_uc_cores());
        for (micro_idx = 0; micro_idx < num_micros; micro_idx++) {
            EFUN(blackhawk7_v1l8p1_set_micro_idx(sa__, micro_idx));
            ESTM(rd_val |= rdc_micro_core_rstb());
        }
        EFUN(blackhawk7_v1l8p1_set_micro_idx(sa__, micro_orig));
    }
    if (rd_val != 0) {
        return (ERR_CODE_UC_NOT_RESET);
    }
...
err_code_t blackhawk7_v1l8p1_write_rx_afe(srds_access_t *sa__, enum srds_rx_afe_settings_enum param, int8_t val) {
  /* Assumes the micro is not actively tuning */
    ...
    switch(param) {
    case RX_AFE_PF:
        return(blackhawk7_v1l8p1_INTERNAL_set_rx_pf_main(sa__, val));

    case RX_AFE_PF2:
        return(blackhawk7_v1l8p1_INTERNAL_set_rx_pf2(sa__, val));

    case RX_AFE_VGA:
      return(blackhawk7_v1l8p1_INTERNAL_set_rx_vga(sa__, val));
    ...
    case RX_AFE_PF3:
        return(blackhawk7_v1l8p1_INTERNAL_set_rx_pf3(sa__, val));
    default:
        return(blackhawk7_v1l8p1_error(sa__, ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
    }
}
...
err_code_t blackhawk7_v1l8p1_uc_reset(srds_access_t *sa__, uint8_t enable, ucode_info_t ucode_info) {
  if (enable) {
...

    {
      ...
      EFUN(blackhawk7_v1l8p1_pmd_wr_reg(sa__, 0xD22E, (0x8000 | (stack_size)<<2)));
    }
  }
  else {
 ...
 
    {
        ...
        for (micro_idx = num_micros-1; micro_idx >= 0; micro_idx--) {
            EFUN(blackhawk7_v1l8p1_set_micro_idx(sa__, micro_idx));
...
err_code_t blackhawk7_v1l8p1_init_blackhawk7_v1l8p1_info(srds_access_t *sa__) {
    ...
    if (blackhawk7_v1l8p1_info_ptr == 0) {
    ...
    if (blackhawk7_v1l8p1_info_ptr->signature == SRDS_INFO_SIGNATURE) {
        ...
        err_code = blackhawk7_v1l8p1_INTERNAL_match_ucode_from_info(sa__);
...
err_code_t blackhawk7_v1l8p1_tx_clock_align_enable(srds_access_t *sa__, uint8_t enable, uint8_t lane_mask) {
    ...
    ESTM(current_lane = blackhawk7_v1l8p1_get_lane(sa__));
    ...
    for(lane = 0;lane<num_lanes;lane++) {
        if ((lane_mask >> lane) & 0x1) {
            EFUN(blackhawk7_v1l8p1_set_lane(sa__,lane));
    ...
    if (enable)
    {
        ...
        EFUN(blackhawk7_v1l8p1_set_lane(sa__,current_lane)); /* return lane to previous state */
