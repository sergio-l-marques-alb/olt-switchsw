...
int tscbh_gen2_port_enable_set(const phymod_phy_access_t* phy, uint32_t enable)
{
    ...
    if ((port_an_enable) || (port_enable && !pcs_enable)) {
        ...
        if (enable == 1) {
            ...
            PHYMOD_IF_ERR_RETURN(blackhawk7_v1l8p2_tx_disable(&phy_copy.access, 0));
            PHYMOD_IF_ERR_RETURN(blackhawk7_v1l8p2_pmd_force_signal_detect(&phy_copy.access, 0, 0));
            phy_copy.access.lane_mask = 1 << start_lane;
        }
        PHYMOD_IF_ERR_RETURN
        ...
        if (!enable) {
            ...
            PHYMOD_IF_ERR_RETURN(blackhawk7_v1l8p2_tx_disable(&phy_copy.access, 1));
            PHYMOD_IF_ERR_RETURN(blackhawk7_v1l8p2_pmd_force_signal_detect(&phy_copy.access, 1, 0));
        }
    ...
    *new_pll_state = *old_pll_state;
    ...
    if (!PHYMOD_DEVICE_OP_MODE_PCS_BYPASS_GET(phy->device_op_mode)) {
        ...
        if (osr_2p5_is_required) {
            PHYMOD_IF_ERR_RETURN
                (blackhawk7_v1l8p2_osr_mode_set(&pm_phy_copy.access, PMD_OSR_2P5));
        } else {
            PHYMOD_IF_ERR_RETURN
...
int _tscbh_gen2_core_init_pass1(const phymod_core_access_t* core, const phymod_core_init_config_t* init_config, const phymod_core_status_t* core_status)
{
    ...
    ucode.crc_value = blackhawk7_v1l8p2_ucode_crc;
    blackhawk7_ucode_len = blackhawk7_v1l8p2_ucode_len;
    blackhawk7_ucode = blackhawk7_v1l8p2_ucode;
    ...
    if (init_config->firmware_load_method != phymodFirmwareLoadMethodNone) {
        ...
        if (PHYMOD_CORE_INIT_F_FIRMWARE_LOAD_VERIFY_GET(init_config)) {
            rv = blackhawk7_v1l8p2_ucode_load_verify(&core_copy.access, (uint8_t *) blackhawk7_ucode, blackhawk7_ucode_len);
            if (rv != PHYMOD_E_NONE) {
...
int _tscbh_gen2_core_init_pass2(const phymod_core_access_t* core, const phymod_core_init_config_t* init_config, const phymod_core_status_t* core_status)
{
    ...
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_v1l8p2_set_usr_ctrl_lane_event_log_level(&core_copy.access, 2));
    ...
    *new_pll_adv_state = *old_pll_adv_state;
    ...
    for (i = requested_pll_num; i >= 0; i--) {
        if (request_pll_div[i] == ovco_pll_div) {
            ...
            cnt = 0;
            while (cnt < 1000) {
                PHYMOD_IF_ERR_RETURN(blackhawk7_v1l8p2_pll_lock_get(&phy_copy.access, &pll_lock));
                cnt = cnt + 1;
                if (pll_lock) {
                    break;
                } else {
...
int tscbh_gen2_timesync_adjust_set(const phymod_phy_access_t* phy, uint32_t flags, uint32_t ts_am_norm_mode)
{
    ...
    /* 4. Program UI */
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_v1l8p2_clk4sync_enable_get(&phy_copy.access, &clk4sync_en, &clk4sync_div));
    /*
                                 ...
                                 phymod_synce_clk_ctrl_t cfg)
{
    ...
    if ((cfg.stg0_mode == 0x2) && (cfg.stg1_mode == 0x0)) {
        ...
        PHYMOD_IF_ERR_RETURN
            (blackhawk7_v1l8p2_lane_pll_selection_get(&phy_copy.access, &current_pll_index));
        ...
        phy_copy.access.lane_mask = 0x1;
        PHYMOD_IF_ERR_RETURN
            (blackhawk7_v1l8p2_INTERNAL_read_pll_div(&phy_copy.access, (uint32_t *)&pll_div));
