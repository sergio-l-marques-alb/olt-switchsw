...
int tscbh_gen3_phy_tx_override_set(const phymod_phy_access_t* phy, const phymod_tx_override_t* tx_override)
{
    phymod_phy_access_t phy_copy;
    int start_lane, num_lane, i;
...
int tscbh_gen3_phy_tx_override_get(const phymod_phy_access_t* phy, phymod_tx_override_t* tx_override)
{
    phymod_phy_access_t phy_copy;
    int16_t temp_value;
    PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));
    ...
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_v1l8p1_tx_pi_control_get(&phy_copy.access, &temp_value));
                                   ...
                                   phymod_phy_pll_state_t* new_pll_state)
{
    ...
    uint32_t /*is_pam4, osr_mode, */ osr_2p5_is_required = 0;
    int port_enable;
...
#if 0
STATIC
int _tscbh_gen3_speed_id_to_speed_config_get(const phymod_phy_access_t* phy,
                                        int speed_id,
                                        int num_lane,
                                        phymod_phy_speed_config_t* speed_config)
{

    uint32_t /*pll_div, */ refclk_in_hz = 156250000, data_rate_lane;
    int osr_mode = 0;
    tbhmod_gen3_refclk_t ref_clk;
    uint32_t vco_rate = 20625000;
    phymod_phy_access_t phy_copy;
...
int _tscbh_gen3_core_init_pass2(const phymod_core_access_t* core, const phymod_core_init_config_t* init_config, const phymod_core_status_t* core_status)
{
    ...
    /*once the microcode is ready, this needs to be updated */
    ...
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_v1l8p1_set_usr_ctrl_lane_event_log_level(&core_copy.access, 2));
#endif
...
int tscbh_gen3_phy_init(const phymod_phy_access_t* phy, const phymod_phy_init_config_t* init_config)
{
    ...
    for (i = 0; i < num_lane; i++) {
        ...
        /* set tx parameters */
        PHYMOD_IF_ERR_RETURN
            (tscbh_gen3_phy_tx_set(&pm_phy_copy, &init_config->tx[i]));
    }
    ...
    if (!PHYMOD_PHY_INIT_F_SW_WAR_100G_2LANE_IS_DONE_GET(init_config)) {
