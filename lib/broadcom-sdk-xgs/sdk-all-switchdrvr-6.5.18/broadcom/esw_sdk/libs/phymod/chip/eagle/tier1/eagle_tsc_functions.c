...
*  Revision      :  $Id: eagle_tsc_functions.c 1140 2015-09-18 21:05:29Z kirand $ *
*                                                                                 *
...
err_code_t eagle_tsc_display_eye_scan( const phymod_access_t *pa ){
	...
	EFUN(eagle_tsc_display_eye_scan_header( pa, 1));

	/* start horizontal acquisition */
	{   err_code_t err_code = eagle_tsc_meas_eye_scan_start(pa, EYE_SCAN_HORIZ);
	    if (err_code)
		    EFUN((eagle_tsc_meas_eye_scan_done( pa ), err_code)); }

	/* display stripes */
	for (y = 31; y >= -31; y = y - 1) {
		{   err_code_t err_code = eagle_tsc_read_eye_scan_stripe( pa, &stripe[0], &status);
		    if (err_code)
			    EFUN((eagle_tsc_meas_eye_scan_done( pa ), err_code)); }
		EFUN(eagle_tsc_display_eye_scan_stripe( pa, y, &stripe[0]));
		EFUN_PRINTF(("\n"));
	}
	/* stop acquisition */
	EFUN(eagle_tsc_meas_eye_scan_done( pa ));
	EFUN(eagle_tsc_display_eye_scan_footer( pa, 1));
	...
	for (y = eyescan_options.vert_max; y >= eyescan_options.vert_min; y = y - eyescan_options.vstep) {
		for (x = eyescan_options.horz_min; x <= eyescan_options.horz_max; x = x + eyescan_options.hstep) {
...
#endif
			{
				...
				ENULL_MEMSET(&osr_mode, 0, sizeof(eagle_tsc_osr_mode_st));
				EFUN(_eagle_tsc_get_osr_mode( pa, &osr_mode));
				if (osr_mode.tx_rx == EAGLE_TSC_OSX1)
					EFUN(eagle_tsc_prbs_err_count_ll( pa, &errors));
				else if (osr_mode.tx_rx == EAGLE_TSC_OSX2)
...
err_code_t eagle_tsc_meas_eye_scan_start( const phymod_access_t *pa, uint8_t direction){
	...
	if (direction == EYE_SCAN_VERTICAL)
		EFUN(eagle_tsc_pmd_uc_diag_cmd( pa, CMD_UC_DIAG_START_VSCAN_EYE, 200));
	else
		EFUN(eagle_tsc_pmd_uc_diag_cmd( pa, CMD_UC_DIAG_START_HSCAN_EYE, 200));
	return ERR_CODE_NONE;
}
...
err_code_t eagle_tsc_read_eye_scan_stripe( const phymod_access_t *pa, uint32_t *buffer, uint16_t *status){
	...
	*status = 0;
	for (i = 0; i < 32; i++) {
		{   err_code_t err_code = eagle_tsc_poll_diag_eye_data(pa,&val[0], &sts, 200);
		    *status |= sts & 0xF000;
		    if (err_code)
			    return err_code; }
		buffer[i * 2]     = val[0];
...
err_code_t eagle_tsc_meas_eye_density_data( const phymod_access_t *pa, const struct eagle_tsc_eyescan_options_st eyescan_options, int32_t *buffer, uint16_t *buffer_size){
	...
	EFUN(eagle_tsc_pmd_uc_diag_cmd( pa, CMD_UC_DIAG_DENSITY, 2000));
	i = 0; hzcnt = 0;
	...
	for (y = eyescan_options.vert_max; y >= eyescan_options.vert_min; y = y - eyescan_options.vstep) {
		...
		for (x = eyescan_options.horz_min; x <= eyescan_options.horz_max; x = x + eyescan_options.hstep) {
			...
			EFUN(eagle_tsc_poll_dsc_state_equals_uc_tune( pa, 2000));
...
err_code_t eagle_tsc_display_eye_density_data( const phymod_access_t *pa, const struct eagle_tsc_eyescan_options_st eyescan_options, int32_t *buffer, uint16_t buffer_size){
	...
	if (maxval == (((int32_t)0)    )) {
		scalen =  0; scaled =      1;
	}else if (maxval >= (((int32_t)1) << 27)) {
		scalen =  1; scaled = maxval / 16;
	...
	for (y = eyescan_options.vert_max - 1; y >= eyescan_options.vert_min; y = y - eyescan_options.vstep) {
		...
		for (x = eyescan_options.horz_min; x <= eyescan_options.horz_max; x = x + eyescan_options.hstep) {
			...
			if (val > 15)
				val = 15;
			for (z = 1; z <= eyescan_options.hstep; z++) {
				if (z != 1) EFUN_PRINTF((" ")); else if (val) EFUN_PRINTF(("%X", (uint32_t)val)); else if ((x % 5) == 0 && ((y + 3) % 5) == 0) EFUN_PRINTF(("+")); else if ((x % 5) != 0 && ((y + 3) % 5) == 0) EFUN_PRINTF(("-")); else if ((x % 5) == 0 && ((y + 3) % 5) != 0) EFUN_PRINTF((":")); else EFUN_PRINTF((" "));
			}
			j++;
		}
		EFUN_PRINTF(("\n"));
...
err_code_t eagle_tsc_meas_eye_scan_done( const phymod_access_t *pa ){
	EFUN(eagle_tsc_pmd_uc_diag_cmd( pa, CMD_UC_DIAG_DISABLE, 200));
	return ERR_CODE_NONE;
}
...
err_code_t eagle_tsc_read_ber_scan_data( const phymod_access_t *pa, uint32_t *errors, uint32_t *timer_values, uint8_t *cnt, uint32_t timeout){
	...
	*cnt = (sts & 0x00FF) / 3;
	for (i = 0; i < *cnt; i++) {
		...
		EFUN(eagle_tsc_pmd_uc_cmd( pa,  CMD_READ_DIAG_DATA_WORD, 0, timeout));
		...
		EFUN(eagle_tsc_pmd_uc_cmd( pa,  CMD_READ_DIAG_DATA_BYTE, 0, timeout));
...
err_code_t eagle_tsc_eye_margin_proj( const phymod_access_t *pa, USR_DOUBLE rate, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control){
	...
	EFUN(eagle_tsc_pmd_uc_cmd( pa, CMD_CAPTURE_BER_END, 0x00, 50));
	...
	EFUN(eagle_tsc_display_ber_scan_data(pa, rate, ber_scan_mode, &errs[0], &time[0], (uint8_t)_abs(offset_start)));
...
err_code_t eagle_tsc_display_lane_state( const phymod_access_t *pa ){
	err_code_t err_code = _eagle_tsc_display_lane_state_no_newline( pa );
...
err_code_t eagle_tsc_display_core_state_line( const phymod_access_t *pa ){
	err_code_t err_code = _eagle_tsc_display_core_state_no_newline( pa );
...
err_code_t eagle_tsc_display_core_config( const phymod_access_t *pa ){
    uint16_t vco_mhz;
	EFUN_PRINTF(("\n\n*****************************************\n"  ));
	EFUN_PRINTF((    "**** SERDES CORE 0x%03x CONFIGURATION ****\n", pa->addr  ));
	EFUN_PRINTF((    "*****************************************\n\n"));
	{
		struct   eagle_tsc_uc_core_config_st core_cfg;
		ENULL_MEMSET(&core_cfg, 0, sizeof(core_cfg));
		EFUN(eagle_tsc_get_uc_core_config( pa, &core_cfg));
		{
...
err_code_t eagle_tsc_display_lane_config( const phymod_access_t *pa ){
	...
	EFUN_PRINTF((    "*************************************\n\n"                                        ));
	EFUN(                                                         eagle_tsc_get_uc_lane_cfg( pa, &lane_cfg)      );
...
err_code_t eagle_tsc_display_core_state( const phymod_access_t *pa ){
	EFUN(eagle_tsc_display_core_state_hdr( pa ));
	EFUN(eagle_tsc_display_core_state_line( pa ));
	EFUN(eagle_tsc_display_core_state_legend( pa ));
	return ERR_CODE_NONE;
}
...
err_code_t eagle_tsc_display_lane_debug_status( const phymod_access_t *pa ){
	...
	EFUN(eagle_tsc_get_usr_ctrl_disable_startup( pa, &ds));
	...
	EFUN(                                                            eagle_tsc_get_usr_ctrl_disable_startup_dfe( pa, &dsd) );
	...
	EFUN(                                                           eagle_tsc_get_usr_ctrl_disable_steady_state( pa, &dss) );
	...
	EFUN(                                                      eagle_tsc_get_usr_ctrl_disable_steady_state_dfe( pa, &dssd) );
...
err_code_t eagle_tsc_stop_uc_lane( const phymod_access_t *pa, uint8_t enable){
	...
	if (enable)
		return eagle_tsc_pmd_uc_control( pa, CMD_UC_CTRL_STOP_GRACEFULLY, 100);
	else
		return eagle_tsc_pmd_uc_control( pa, CMD_UC_CTRL_RESUME, 50);
}
...
err_code_t eagle_tsc_request_stop_rx_adaptation( const phymod_access_t *pa ){
	...
	return eagle_tsc_pmd_uc_control_return_immediate( pa, CMD_UC_CTRL_STOP_GRACEFULLY);
}
...
err_code_t eagle_tsc_ucode_crc_verify( const phymod_access_t *pa, uint16_t ucode_len, uint16_t expected_crc_value){
	...
	EFUN(eagle_tsc_pmd_uc_cmd_with_data( pa, CMD_CALC_CRC, 0, ucode_len, 200));
...
err_code_t eagle_tsc_start_ucode_crc_calc( const phymod_access_t *pa, uint16_t ucode_len){
	...
	EFUN(eagle_tsc_pmd_uc_cmd_with_data_return_immediate( pa, CMD_CALC_CRC, 0, ucode_len)); /* Invoke Calculate CRC command and return control immediately */
	return ERR_CODE_NONE;
}
...
err_code_t eagle_tsc_pmd_uc_cmd_return_immediate( const phymod_access_t *pa, enum srds_pmd_uc_cmd_enum cmd, uint8_t supp_info){
	...
	err_code = eagle_tsc_poll_uc_dsc_ready_for_cmd_equals_1( pa, 1); /* Poll for uc_dsc_ready_for_cmd = 1 to indicate eagle_tsc ready for command */
	if (err_code) {
...
err_code_t eagle_tsc_pmd_uc_cmd( const phymod_access_t *pa, enum srds_pmd_uc_cmd_enum cmd, uint8_t supp_info, uint32_t timeout_ms){
	...
	EFUN(eagle_tsc_pmd_uc_cmd_return_immediate( pa, cmd, supp_info));    /* Invoke eagle_tsc_uc_cmd and return control immediately */
...
err_code_t eagle_tsc_pmd_uc_cmd_with_data( const phymod_access_t *pa, enum srds_pmd_uc_cmd_enum cmd, uint8_t supp_info, uint16_t data, uint32_t timeout_ms){
	...
	EFUN(eagle_tsc_pmd_uc_cmd_with_data_return_immediate( pa, cmd, supp_info, data));    /* Invoke eagle_tsc_uc_cmd and return control immediately */
...
err_code_t eagle_tsc_pmd_uc_control_return_immediate( const phymod_access_t *pa, enum srds_pmd_uc_ctrl_cmd_enum control){
	return eagle_tsc_pmd_uc_cmd_return_immediate( pa, CMD_UC_CTRL, (uint8_t)control);
}
...
err_code_t eagle_tsc_set_uc_core_config( const phymod_access_t *pa, struct eagle_tsc_uc_core_config_st struct_val){
	{
		...
		if (reset_state < 7) {
			EFUN_PRINTF(("ERROR: eagle_tsc_set_uc_core_config( pa, ..) called without core_dp_s_rstb=0 Lane=%d reset_state=%d\n", eagle_tsc_get_lane(pa), reset_state));
...
err_code_t eagle_tsc_set_uc_lane_cfg( const phymod_access_t *pa, struct eagle_tsc_uc_lane_config_st struct_val){
	...
	if (reset_state < 7) {
		EFUN_PRINTF(("ERROR: eagle_tsc_set_uc_lane_cfg( pa, ..) called without ln_dp_s_rstb=0 Lane=%d reset_state=%d\n", eagle_tsc_get_lane(pa), reset_state));
...
#else
	{   uint16_t rddata;
	    EFUN_PRINTF(("// ATE_LOG eagle_tsc_poll_uc_dsc_ready_for_cmd_equals_1 begin\n"));
	    if (timeout_ms < 100)
		    EFUN(eagle_tsc_delay_us(1000 * timeout_ms));
	    else
		    EFUN(eagle_tsc_delay_ms(timeout_ms));
	    EFUN(eagle_tsc_pmd_rdt_reg(pa,DSC_A_DSC_UC_CTRL, &rddata));
	    if (rddata & 0x0080) {              /* bit 7 is uc_dsc_ready_for_cmd */
		    if (rddata & 0x0040) {      /* bit 6 is uc_dsc_error_found   */
			    ESTM_PRINTF(("ERROR : DSC command returned error (after cmd) cmd = 0x%x, supp_info = 0x%02x !\n", rd_uc_dsc_gp_uc_req(), rd_uc_dsc_supp_info()));
		    ...
		    EFUN_PRINTF(("// ATE_LOG eagle_tsc_poll_uc_dsc_ready_for_cmd_equals_1 end\n"));
		    return ERR_CODE_NONE;
	    }
	    EFUN_PRINTF(("// ATE_LOG eagle_tsc_poll_uc_dsc_ready_for_cmd_equals_1 timeout\n")); }
#endif
...
err_code_t eagle_tsc_init_pram_for_uc_load( const phymod_access_t *pa, uint16_t ucode_len){
	...
	/* Wait 8 pram clocks */
	/* call eagle_tsc_finish_pram_load */
	return ERR_CODE_NONE;
}
...
err_code_t eagle_tsc_uc_core_var_dump( const phymod_access_t *pa ){
	...
	for (addr = 0x0; addr < 0xFF; addr++) {
		if (!(addr % 26))
			EFUN_PRINTF(("\n%04x ", addr));
		ESTM_PRINTF(("%02x ", eagle_tsc_rdbc_uc_var( pa, __ERR, addr)));
	}
	return ERR_CODE_NONE;
}
...
err_code_t eagle_tsc_uc_lane_var_dump( const phymod_access_t *pa ){
	...
	if (rx_lock == 1) {
		...
		EFUN(eagle_tsc_pmd_uc_control( pa, CMD_UC_CTRL_STOP_IMMEDIATE, 200));

	for (addr = 0x0; addr < LANE_VAR_RAM_SIZE; addr++) {
		if (!(addr % 26))
			EFUN_PRINTF(("\n%04x ", addr));
		ESTM_PRINTF(("%02x ", eagle_tsc_rdbl_uc_var( pa, __ERR, addr)));
	}
...
err_code_t eagle_tsc_display_diag_data( const phymod_access_t *pa, int32_t diag_level){
	...
	EFUN(eagle_tsc_version( pa, &api_version));
	...
	EFUN(eagle_tsc_display_lane_state_hdr( pa ));
	EFUN(eagle_tsc_display_lane_state( pa ));
	...
	if (diag_level & SRDS_DIAG_BER_VERT) {
		...
		EFUN(_eagle_tsc_display_ber_scan_data( pa, ber_mode, timer_control,  err_threshold));
...
err_code_t eagle_tsc_diag_access( const phymod_access_t *pa, enum srds_diag_access_enum type, uint16_t addr, uint16_t data, uint16_t param){
	...
	case SRDS_REG_READ:      {
		...
		if (data > 1) {
			...
			for (i = 0; i < data; i++) {
				if (!(i % 16))
					ESTM_PRINTF(("\n%04x ", i + addr));
				EFUN(eagle_tsc_pmd_rdt_reg(pa, i + addr, &tmp));
				ESTM_PRINTF(("%04x ", tmp));
			}
			EFUN_PRINTF(("\n"));
		} else {
			EFUN(eagle_tsc_pmd_rdt_reg(pa, addr, &tmp));
	...
	case SRDS_REG_RMW:       {
		...
		EFUN(eagle_tsc_pmd_wr_reg(pa, addr, tmp2));
	...
	case SRDS_CORE_RAM_READ_BYTE: {
		if (data > 1) {
			...
			for (i = 0; i < (uint8_t)data; i++) {
				if (!(i % 26))
					ESTM_PRINTF(("\n%04x ", i + (uint8_t)addr));
				ESTM_PRINTF(("%02x ", eagle_tsc_rdbc_uc_var( pa, __ERR, i + (uint8_t)addr)));
			}
			EFUN_PRINTF(("\n"));
		} else
	...
	case SRDS_LANE_RAM_READ_BYTE: {
		if (data > 1) {
			...
			for (i = 0; i < data; i++) {
				if (!(i % 26))
					ESTM_PRINTF(("\n%04x ", i + addr));
				ESTM_PRINTF(("%02x ", eagle_tsc_rdbl_uc_var( pa, __ERR, i + addr)));
			}
			EFUN_PRINTF(("\n"));
		} else
	...
	case SRDS_CORE_RAM_READ_WORD: {
		if (data > 1) {
			...
			for (i = 0; i < (uint8_t)data; i += 2) {
				if (!(i % 16))
					ESTM_PRINTF(("\n%04x ", i + (uint8_t)addr));
				ESTM_PRINTF(("%04x ", eagle_tsc_rdwc_uc_var( pa, __ERR, i + (uint8_t)addr)));
			}
			EFUN_PRINTF(("\n"));
		} else
	...
	case SRDS_LANE_RAM_READ_WORD: {
		if (data > 1) {
			...
			for (i = 0; i < data; i += 2) {
				if (!(i % 16))
					ESTM_PRINTF(("\n%04x ", i = i + addr));
				ESTM_PRINTF(("%04x ", eagle_tsc_rdwl_uc_var( pa, __ERR, i + addr)));
			}
			EFUN_PRINTF(("\n"));
		} else
	...
	case SRDS_CORE_RAM_RMW_BYTE:  {
		uint8_t tmp, tmp2;
		ESTM(tmp = eagle_tsc_rdbc_uc_var( pa, __ERR, (uint8_t)addr));
		tmp2 = (tmp & (uint8_t) ~param) | ((uint8_t)param & data);
		EFUN(eagle_tsc_wrbc_uc_var( pa, (uint8_t)addr, tmp2));
		EFUN_PRINTF(("Core RAM RMW byte: x%04x = x%02x -> x%02x\n", addr, tmp, tmp2));
	} break;
	case SRDS_LANE_RAM_RMW_BYTE:  {
		uint8_t tmp, tmp2;
		ESTM(tmp = eagle_tsc_rdbl_uc_var( pa, __ERR, addr));
		tmp2 = (tmp & (uint8_t) ~param) | ((uint8_t)param & data);
		EFUN(eagle_tsc_wrbl_uc_var( pa, addr, tmp2));
		EFUN_PRINTF(("Lane RAM RMW byte: x%04x = x%02x -> x%02x\n", addr, tmp, tmp2));
	} break;
	case SRDS_CORE_RAM_RMW_WORD:  {
		uint16_t tmp, tmp2;
		ESTM(tmp = eagle_tsc_rdwc_uc_var( pa, __ERR, (uint8_t)addr));
		tmp2 = (tmp & ~param) | (param & data);
		EFUN(eagle_tsc_wrwc_uc_var( pa, (uint8_t)addr, tmp2));
		EFUN_PRINTF(("Core RAM RMW word: x%04x = x%04x -> x%04x\n", addr, tmp, tmp2));
	} break;
	case SRDS_LANE_RAM_RMW_WORD:  {
		uint16_t tmp, tmp2;
		ESTM(tmp = eagle_tsc_rdwl_uc_var( pa, __ERR, addr));
		tmp2 = (tmp & ~param) | (param & data);
		EFUN(eagle_tsc_wrwl_uc_var( pa, addr, tmp2));
	...
	case SRDS_GLOB_RAM_READ_BYTE: {
		if (data > 1) {
			...
			for (i = 0; i < data; i++) {
				if (!(i % 16))
					ESTM_PRINTF(("\n%04x ", i + addr));
				ESTM_PRINTF(("%02x ", _eagle_tsc_rdb_uc_var( pa, __ERR, i + addr)));
			}
			EFUN_PRINTF(("\n"));
		} else
	...
	case SRDS_GLOB_RAM_RMW_BYTE:  {
		uint8_t tmp, tmp2;
		ESTM(tmp = _eagle_tsc_rdb_uc_var( pa, __ERR, addr));
		tmp2 = (tmp & (uint8_t) ~param) | ((uint8_t)param & data);
		EFUN(_eagle_tsc_wrb_uc_var( pa, addr, tmp2));
	...
	case SRDS_GLOB_RAM_READ_WORD: {
		if (data > 1) {
			...
			for (i = 0; i < data; i += 2) {
				if (!(i % 16))
					ESTM_PRINTF(("\n%04x ", i + addr));
				ESTM_PRINTF(("%04x ", _eagle_tsc_rdw_uc_var( pa, __ERR, i + addr)));
			}
			EFUN_PRINTF(("\n"));
		} else
	...
	case SRDS_GLOB_RAM_RMW_WORD:  {
		uint16_t tmp, tmp2;
		ESTM(tmp = _eagle_tsc_rdw_uc_var( pa, __ERR, addr));
		tmp2 = (tmp & ~param) | (param & data);
		EFUN(_eagle_tsc_wrw_uc_var( pa, addr, tmp2));
		EFUN_PRINTF(("Glob RAM RMW word: x%04x = x%04x -> x%04x\n", addr, tmp, tmp2));
	} break;
	case SRDS_UC_CMD:        {
		uint16_t tmp;
		EFUN(eagle_tsc_pmd_uc_cmd_with_data( pa, (enum srds_pmd_uc_cmd_enum)addr, (uint8_t)param, data, 100));
		ESTM(tmp = rd_uc_dsc_data());
		EFUN_PRINTF(("uC Command: cmd=x%02x supp=x%02x data=x%04x returned=x%04x\n", addr, param, data, tmp));
	} break;
	case SRDS_BER_PROJ_DATA: {
		/* display ber projections for all channels */
		EFUN(_eagle_tsc_display_ber_scan_data( pa, (uint8_t)addr, (uint8_t)data,  (uint8_t)(param >> 4)));
	} break;
	case SRDS_EN_BREAKPOINT: 
	case SRDS_GOTO_BREAKPOINT:
	case SRDS_RD_BREAKPOINT:
	case SRDS_DIS_BREAKPOINT:
	default: EFUN_PRINTF(("Invalid request type eagle_tsc_diag_access\n"));
	}
	...
	if (display_mode == EVENT_LOG_DECODED || display_mode == EVENT_LOG_HEX_AND_DECODED) {
		...
		while (addr < trace_mem_size - 8) {                                                     /* last 8-byte reserved for trace memory wraparound */
			...
			if (*(trace_mem + addr) == 0xff) {                                           /* timestamp wraparound event handling */
				...
				EFUN(eagle_tsc_uc_lane_idx_to_system_id(uc_lane_id_str, cc));
...
err_code_t eagle_tsc_display_state( const phymod_access_t *pa ){
	EFUN(eagle_tsc_display_core_state( pa ));
	EFUN(eagle_tsc_display_lane_state_hdr( pa ));
	EFUN(eagle_tsc_display_lane_state( pa ));
	EFUN(eagle_tsc_display_lane_state_legend( pa ));
	return ERR_CODE_NONE;
}

err_code_t eagle_tsc_display_config( const phymod_access_t *pa ){
	EFUN(eagle_tsc_display_core_config( pa ));
	EFUN(eagle_tsc_display_lane_config( pa ));
	return ERR_CODE_NONE;
}
...
err_code_t eagle_tsc_read_die_temperature( const phymod_access_t *pa, int16_t *die_temp){
	...
	EFUN(eagle_tsc_pmd_uc_cmd( pa, CMD_UC_DBG, CMD_UC_DBG_DIE_TEMP, 50));
...
err_code_t eagle_tsc_read_die_temperature_double(const phymod_access_t *pa, USR_DOUBLE *die_temp){
	...
	*die_temp = 410.04 - 0.48705 * ((USR_DOUBLE)die_temp_sensor_reading);
	...
	return ERR_CODE_NONE;
#else
	EFUN_PRINTF((" Function 'eagle_tsc_read_die_temperature_double' needs SERDES_API_FLOATING_POINT defined to operate \n"));
	return _error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT);
#endif
}
