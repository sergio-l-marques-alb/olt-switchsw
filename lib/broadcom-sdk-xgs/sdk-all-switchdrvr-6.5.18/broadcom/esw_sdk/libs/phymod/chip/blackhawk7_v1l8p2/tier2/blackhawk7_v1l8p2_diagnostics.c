...
#include "blackhawk7_v1l8p2/tier1/common/srds_api_enum.h"
...
#include "blackhawk7_v1l8p2/tier1/blackhawk7_v1l8p2_types.h"
#include "blackhawk7_v1l8p2/tier1/blackhawk7_v1l8p2_cfg_seq.h"

#ifdef PHYMOD_LINKCAT_BLACKHAWK7_V1L8P2_SUPPORT
...
int blackhawk7_v1l8p2_diagnostics_eye_margin_proj( const phymod_phy_access_t* phy, uint32_t flags,
                                        const phymod_phy_eyescan_options_t* eyescan_options);

/*phymod, internal enum mappings*/
STATIC
int _blackhawk7_v1l8p2_prbs_poly_phymod_to_blackhawk(phymod_prbs_poly_t phymod_poly, enum srds_prbs_polynomial_enum *blackhawk_poly)
{
    switch(phymod_poly){
...
STATIC
int _blackhawk7_v1l8p2_prbs_poly_blackhawk_to_phymod(enum srds_prbs_polynomial_enum  *blackhawk_poly, phymod_prbs_poly_t *phymod_poly)
{
...
int blackhawk7_v1l8p2_phy_prbs_config_set(const phymod_phy_access_t* phy, uint32_t flags , const phymod_prbs_t* prbs)
{
    ...
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_v1l8p2_phy_firmware_lane_config_get(phy, &fw_config));

    PHYMOD_IF_ERR_RETURN(_blackhawk7_v1l8p2_prbs_poly_phymod_to_blackhawk(prbs->poly, &blackhawk_poly));
    ...
    for (i = 0; i < num_lane; i++) {
        ...
        if (PHYMOD_PRBS_DIRECTION_RX_GET(flags)) {
            ...
            if (fw_config.ForcePAM4Mode) {
                PHYMOD_IF_ERR_RETURN
                    (blackhawk7_v1l8p2_config_rx_prbs(&phy_copy.access, blackhawk_poly,PRBS_INITIAL_SEED_NO_HYSTERESIS,  prbs->invert));
            } else {
                PHYMOD_IF_ERR_RETURN
                    (blackhawk7_v1l8p2_config_rx_prbs(&phy_copy.access, blackhawk_poly,PRBS_INITIAL_SEED_HYSTERESIS,  prbs->invert));
            }
        } else if (PHYMOD_PRBS_DIRECTION_TX_GET(flags)) {
            PHYMOD_IF_ERR_RETURN
                (blackhawk7_v1l8p2_config_tx_prbs(&phy_copy.access, blackhawk_poly, prbs->invert));
        } else {
            /* if PAM4 speed, use mode 2 */
            if (fw_config.ForcePAM4Mode) {
                PHYMOD_IF_ERR_RETURN
...
int blackhawk7_v1l8p2_phy_prbs_config_get(const phymod_phy_access_t* phy, uint32_t flags , phymod_prbs_t* prbs)
{
    ...
    if (PHYMOD_PRBS_DIRECTION_TX_GET(flags)) {
        PHYMOD_IF_ERR_RETURN(blackhawk7_v1l8p2_get_tx_prbs_config(&phy_copy.access, &blackhawk_poly, &invert));
        config_tmp.invert = invert;
        PHYMOD_IF_ERR_RETURN(_blackhawk7_v1l8p2_prbs_poly_blackhawk_to_phymod(&blackhawk_poly, &config_tmp.poly));
        prbs->invert = config_tmp.invert;
        prbs->poly = config_tmp.poly;
    } else if (PHYMOD_PRBS_DIRECTION_RX_GET(flags)) {
        PHYMOD_IF_ERR_RETURN(blackhawk7_v1l8p2_get_rx_prbs_config(&phy_copy.access,
                                                                &blackhawk_poly,
                                                                &prbs_checker_mode,
                                                                &invert));
        config_tmp.invert = invert;
...
int blackhawk7_v1l8p2_phy_prbs_enable_set(const phymod_phy_access_t* phy, uint32_t flags , uint32_t enable)
{
    phymod_phy_access_t phy_copy;
    int start_lane, num_lane, i;
    ...
    for (i = 0; i < num_lane; i++) {
        ...
        phy_copy.access.lane_mask = 1 << (start_lane + i);
        if (PHYMOD_PRBS_DIRECTION_TX_GET(flags)) {
            PHYMOD_IF_ERR_RETURN(blackhawk7_v1l8p2_tx_prbs_en(&phy_copy.access, enable));
        } else if (PHYMOD_PRBS_DIRECTION_RX_GET(flags)) {
            PHYMOD_IF_ERR_RETURN(blackhawk7_v1l8p2_rx_prbs_en(&phy_copy.access, enable));
        } else {
            PHYMOD_IF_ERR_RETURN(blackhawk7_v1l8p2_tx_prbs_en(&phy_copy.access, enable));
            PHYMOD_IF_ERR_RETURN(blackhawk7_v1l8p2_rx_prbs_en(&phy_copy.access, enable));
        }
...
int blackhawk7_v1l8p2_phy_prbs_enable_get(const phymod_phy_access_t* phy, uint32_t flags , uint32_t* enable)
{
    uint8_t enable_tmp;
    phymod_phy_access_t phy_copy;
    ...
    if (PHYMOD_PRBS_DIRECTION_TX_GET(flags)) {
        PHYMOD_IF_ERR_RETURN(blackhawk7_v1l8p2_get_tx_prbs_en(&phy_copy.access, &enable_tmp));
        *enable = enable_tmp;
    } else if (PHYMOD_PRBS_DIRECTION_RX_GET(flags)) {
        PHYMOD_IF_ERR_RETURN(blackhawk7_v1l8p2_get_rx_prbs_en(&phy_copy.access, &enable_tmp));
        *enable = enable_tmp;
    } else {
        PHYMOD_IF_ERR_RETURN(blackhawk7_v1l8p2_get_tx_prbs_en(&phy_copy.access, &enable_tmp));
        *enable = enable_tmp;
...
int blackhawk7_v1l8p2_phy_prbs_status_get(const phymod_phy_access_t* phy, uint32_t flags, phymod_prbs_status_t* prbs_status)
{
    ...
    for (i = 0; i < num_lane; i++) {
        ...
        if (prbs_checker_mode == PRBS_INITIAL_SEED_HYSTERESIS) {
            PHYMOD_IF_ERR_RETURN(blackhawk7_v1l8p2_prbs_chk_lock_state(&phy_copy.access, &status));
            if (status) {
                /*next check the lost of lock and error count */
                status = 0;
                PHYMOD_IF_ERR_RETURN
                    (blackhawk7_v1l8p2_prbs_err_count_state(&phy_copy.access, &prbs_err_count, &status));
             ...
             */
            PHYMOD_IF_ERR_RETURN(blackhawk7_v1l8p2_get_rx_prbs_en(&phy_copy.access, &chk_en));
            if (!chk_en) {
                PHYMOD_DEBUG_VERBOSE((" Lane :: %d PRBS not locked\n", i ));
                prbs_status->prbs_lock = 0;
            } else {
                 ...
                 */
                PHYMOD_IF_ERR_RETURN
                    (blackhawk7_v1l8p2_prbs_err_count_state(&phy_copy.access,
                                                            &prbs_err_count,
                                                            &status_ori));
                /* 2. Toggle prbs_chk_en bit. Read the PRBS counter to clear
                 * the counter and lock_loss status. Then check PRBS lock status.
                 */
                PHYMOD_IF_ERR_RETURN(blackhawk7_v1l8p2_rx_prbs_en(&phy_copy.access, 0));
                PHYMOD_IF_ERR_RETURN(blackhawk7_v1l8p2_rx_prbs_en(&phy_copy.access, 1));
                PHYMOD_IF_ERR_RETURN
                    (blackhawk7_v1l8p2_prbs_err_count_state(&phy_copy.access,
                                                            &prbs_err_count_temp,
                                                            &status));
                PHYMOD_IF_ERR_RETURN
                    (blackhawk7_v1l8p2_prbs_chk_lock_state(&phy_copy.access,
                                                           &status));
...
int blackhawk7_v1l8p2_phy_pattern_config_set(const phymod_phy_access_t* phy, const phymod_pattern_t* pattern)
{
    int bit;
    uint32_t i,j = 0;
    ...
    PHYMOD_IF_ERR_RETURN
            (blackhawk7_v1l8p2_config_shared_tx_pattern (&phy_copy.access,
				(uint8_t) pattern->pattern_len, (const char *) patt));
    return PHYMOD_E_NONE;

}

int blackhawk7_v1l8p2_phy_pattern_config_get(const phymod_phy_access_t* phy, phymod_pattern_t* pattern)
{
    phymod_phy_access_t phy_copy;

    PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_v1l8p2_config_shared_tx_pattern_idx_get(&phy_copy.access,
                                  &pattern->pattern_len,
                                  pattern->pattern));
...
int blackhawk7_v1l8p2_phy_pattern_enable_set(const phymod_phy_access_t* phy, uint32_t enable, const phymod_pattern_t* pattern)
{
    phymod_phy_access_t phy_copy;

    PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_v1l8p2_tx_shared_patt_gen_en(&phy_copy.access, (uint8_t) enable, (uint8_t)pattern->pattern_len));
    return PHYMOD_E_NONE;
}

int blackhawk7_v1l8p2_phy_pattern_enable_get(const phymod_phy_access_t* phy, uint32_t* enable)
{
    phymod_phy_access_t phy_copy;
    uint8_t enable_8;
    ...
    PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));
    PHYMOD_IF_ERR_RETURN
       (blackhawk7_v1l8p2_tx_shared_patt_gen_en_get(&phy_copy.access, &enable_8));
...
int blackhawk7_v1l8p2_phy_pmd_info_dump(const phymod_phy_access_t* phy, const char *type)
{
    ...
    tsc_info_ptr = blackhawk7_v1l8p2_INTERNAL_get_blackhawk7_v1l8p2_info_ptr(&phy_copy.access);
    ...
    if (cmd_type == phymod_diag_DSC) {
       for (i = start_lane; i < start_lane + num_lane; i++) {
            ...
            PHYMOD_IF_ERR_RETURN
                (blackhawk7_v1l8p2_display_diag_data(&phy_copy.access, SRDS_DIAG_CORE));
       }
            ...
            PHYMOD_DIAG_OUT(("    +--------------------------------------------------------------------+\n"));
             ...
             for (j = 0; j < 8; j++) {
               ...
               PHYMOD_IF_ERR_RETURN
                  (blackhawk7_v1l8p2_display_diag_data(&phy_copy.access, SRDS_DIAG_CORE | SRDS_DIAG_LANE | SRDS_DIAG_EVENT));
             }
            ...
            for (j = 0; j < 8; j++) {
                 phy_copy.access.lane_mask = 0x1 << j;
                 PHYMOD_IF_ERR_RETURN
                    (blackhawk7_v1l8p2_display_diag_data(&phy_copy.access, SRDS_DIAG_CORE | SRDS_DIAG_LANE | SRDS_DIAG_EVENT | SRDS_DIAG_REG_LANE | SRDS_DIAG_REG_CORE | SRDS_DIAG_UC_LANE | SRDS_DIAG_UC_CORE | SRDS_DIAG_EYE));
             }
    } else {
        for (i = 0; i < num_lane; i++) {
            if (!PHYMOD_LANEPBMP_MEMBER(phy->access.lane_mask, start_lane + i)) {
            ...
            case phymod_diag_CFG:
                ...
                if(i==0) {
                    ...
                    PHYMOD_IF_ERR_RETURN
                        (blackhawk7_v1l8p2_display_core_config(&phy_copy.access));
                    phy_copy.access.lane_mask = tmp_lane_mask;
                }
                PHYMOD_IF_ERR_RETURN
                    (blackhawk7_v1l8p2_display_lane_config(&phy_copy.access));
                break;
            ...
            casephymod_diag_CL72:
                ...
                PHYMOD_IF_ERR_RETURN
                    (blackhawk7_v1l8p2_display_cl93n72_status(&phy_copy.access));
                break;
#endif
            ...
            case phymod_diag_DEBUG:
                ...
                PHYMOD_IF_ERR_RETURN
                    (blackhawk7_v1l8p2_display_lane_debug_status(&phy_copy.access));
                break;
...
static void _blackhawk7_v1l8p2_diag_uc_reg_dump(phymod_access_t *pa)
{
    err_code_t errc;
...
STATIC int blackhawk7_v1l8p2_diagnostics_eyescan_run_uc(const phymod_phy_access_t* phy, uint32_t flags)
{
    ...
    if(PHYMOD_EYESCAN_F_PROCESS_GET(flags)) {
        for(j=0; j< PHYMOD_CONFIG_MAX_LANES_PER_CORE; j++) { /* Loop for all lanes. */
            ...
            rc  = blackhawk7_v1l8p2_display_eye_scan(&(phy_copy.access));
            if(rc != PHYMOD_E_NONE) {
                _blackhawk7_v1l8p2_diag_uc_reg_dump(&(phy_copy.access));
                                        ...
                                        const phymod_phy_eyescan_options_t* eyescan_options)
{
  ...
  struct   blackhawk7_v1l8p2_uc_core_config_st core_cfg;
  if(PHYMOD_EYESCAN_F_PROCESS_GET(flags)) {
     PHYMOD_IF_ERR_RETURN
        (phymod_util_lane_config_get(&phy->access, &start_lane, &num_lane));
     PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));
     for (i = 0; i < num_lane; i++) {
         ...
         if (found == 0) {
             PHYMOD_IF_ERR_RETURN(blackhawk7_v1l8p2_get_uc_core_config(&(phy_copy.access), &core_cfg));
             PHYMOD_IF_ERR_RETURN(blackhawk7_v1l8p2_osr_mode_get(&phy_copy.access, &osr_mode));
             PHYMOD_IF_ERR_RETURN(phymod_phy_speed_config_t_init(&speed_config));
             PHYMOD_IF_ERR_RETURN(phymod_phy_speed_config_get(phy, &speed_config));
         ...
         PHYMOD_IF_ERR_RETURN(blackhawk7_v1l8p2_eye_margin_proj(&phy_copy.access, data_rate, eyescan_options->ber_proj_scan_mode,
...
int blackhawk7_v1l8p2_phy_eyescan_run(const phymod_phy_access_t* phy,
                           uint32_t flags,
                           ...
                           const phymod_phy_eyescan_options_t* eyescan_options)
{
    ...
    PHYMOD_IF_ERR_RETURN
      (blackhawk7_v1l8p2_pmd_lock_status(&phy_copy.access, &pmd_rx_lock));
    ...
    switch(mode) {
        case phymodEyescanModeFast:
            return blackhawk7_v1l8p2_diagnostics_eyescan_run_uc(phy, flags);
        case phymodEyescanModeBERProj:
            return blackhawk7_v1l8p2_diagnostics_eye_margin_proj(phy, flags, eyescan_options);
        default:
...
int blackhawk7_v1l8p2_phy_PAM4_tx_pattern_enable_set(const phymod_phy_access_t* phy, phymod_PAM4_tx_pattern_t pattern_type, uint32_t enable)
{
    phymod_phy_access_t phy_copy;
    ...
    case phymod_PAM4TxPattern_JP03B:
        PHYMOD_IF_ERR_RETURN
            (blackhawk7_v1l8p2_config_tx_jp03b_pattern(&phy_copy.access, (uint8_t) enable));
        break;
    case phymod_PAM4TxPattern_Linear:
        PHYMOD_IF_ERR_RETURN
            (blackhawk7_v1l8p2_config_tx_linearity_pattern(&phy_copy.access, (uint8_t) enable));
        break;
    default:
...
int blackhawk7_v1l8p2_phy_PAM4_tx_pattern_enable_get(const phymod_phy_access_t* phy, phymod_PAM4_tx_pattern_t pattern_type, uint32_t* enable)
{
    phymod_phy_access_t phy_copy;
    ...
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_v1l8p2_pam4_tx_pattern_enable_get(&phy_copy.access, pattern_type, enable));
    return PHYMOD_E_NONE;
}

static
int _blackhawk7_v1l8p2_phy_post_fec_ber_proj(const phymod_phy_access_t* phy, const phymod_phy_ber_proj_options_t* options)
{
    ...
    switch (options->ber_proj_phase) {
        ...
        case PHYMOD_BER_PROJ_PHASE_F_CONFIG:
            ...
            for (i = 0; i < num_lane; i++) {
                ...
                phy_copy.access.lane_mask = 1 << (start_lane + i);
                PHYMOD_IF_ERR_RETURN
                    (blackhawk7_v1l8p2_prbs_error_analyzer_reset(&phy_copy.access));
                ...
                PHYMOD_IF_ERR_RETURN
                    (blackhawk7_v1l8p2_prbs_error_analyzer_config(&phy_copy.access, &err_analyzer));
                PHYMOD_IF_ERR_RETURN
                    (blackhawk7_v1l8p2_display_prbs_error_analyzer_config(&phy_copy.access, &err_analyzer, options->ber_proj_timeout_s));
            }
            break;
        case PHYMOD_BER_PROJ_PHASE_F_START:
            for (i = 0; i < num_lane; i++) {
                phy_copy.access.lane_mask = 1 << (start_lane + i);
                PHYMOD_IF_ERR_RETURN(blackhawk7_v1l8p2_prbs_error_analyzer_start(&phy_copy.access));
            }
            break;
        case PHYMOD_BER_PROJ_PHASE_F_COLLECT:
            for (i = 0; i < num_lane; i++) {
                phy_copy.access.lane_mask = 1 << (start_lane + i);
                PHYMOD_MEMSET(&err_analyzer, 0, sizeof(blackhawk7_v1l8p2_prbs_err_analyzer_config_status_st));
                ...
                PHYMOD_IF_ERR_RETURN(
                    blackhawk7_v1l8p2_get_prbs_error_analyzer_err_count(&phy_copy.access, &err_analyzer));
        ...
        case PHYMOD_BER_PROJ_PHASE_F_CAL:
            for (i = 0; i < num_lane; i++) {
                 ...
                 * Alternative: use err_analyzer->pp_field2 = blackhawk7_v1l8p2_get_lane(&phy_copy.access).
                 */
                PHYMOD_IF_ERR_RETURN(blackhawk7_v1l8p2_get_prbs_error_analyzer_err_count(&phy_copy.access, &err_analyzer));
                ...
                PHYMOD_IF_ERR_RETURN(blackhawk7_v1l8p2_display_prbs_error_analyzer_err_count(&phy_copy.access, &err_analyzer));
                PHYMOD_IF_ERR_RETURN(
                    blackhawk7_v1l8p2_prbs_error_analyzer_compute_proj(&phy_copy.access, err_analyzer, options->ber_proj_timeout_s));
            }
            break;
        default:
...
/* BER Projection*/
int blackhawk7_v1l8p2_phy_ber_proj(const phymod_phy_access_t* phy, phymod_ber_proj_mode_t mode, const phymod_phy_ber_proj_options_t* options)
{
    switch (mode) {
        case phymodBERProjModePostFEC:
            PHYMOD_IF_ERR_RETURN(_blackhawk7_v1l8p2_phy_post_fec_ber_proj(phy, options));
            break;
        default:
...
int blackhawk7_v1l8p2_phy_fast_ber_proj_get(const phymod_phy_access_t* phy, uint32_t* ber_proj_data)
{
    ...
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_v1l8p2_INTERNAL_get_BER_data(&phy_copy.access, 100, &ber_data_local, USE_SW_TIMERS));
...
int blackhawk7_v1l8p2_phy_pmd_lane_diag_debug_level_set(const phymod_phy_access_t* phy, uint32_t level)
{
    phymod_phy_access_t phy_copy;
    PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_v1l8p2_set_usr_ctrl_lane_event_log_level(&phy_copy.access, level));

    return PHYMOD_E_NONE;
}

int blackhawk7_v1l8p2_phy_pmd_lane_diag_debug_level_get(const phymod_phy_access_t* phy, uint32_t *level)
{
    phymod_phy_access_t phy_copy;
    PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_v1l8p2_get_usr_ctrl_lane_event_log_level(&phy_copy.access, (uint8_t *) level));

    return PHYMOD_E_NONE;
}


int blackhawk7_v1l8p2_phy_linkcat(const phymod_phy_access_t* phy, phymod_linkCAT_op_mode_t mode) {
    ...
    return(blackhawk7_v1l8p2_LinkCat(&phy->access, &linkcat_config, &linkcat_status));
#else
    return PHYMOD_E_UNAVAIL;
#endif /* PHYMOD_LINKCAT_BLACKHAWK7_V1L8P2_SUPPORT */ 
