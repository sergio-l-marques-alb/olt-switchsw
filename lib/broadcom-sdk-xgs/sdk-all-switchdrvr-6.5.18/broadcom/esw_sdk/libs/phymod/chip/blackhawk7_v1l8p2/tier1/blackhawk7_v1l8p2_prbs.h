 ...
 * Note: The patt_length_bits input to the function should be the value sent to the blackhawk7_v1l8p2_config_shared_tx_pattern() function
 * @param sa__ is an opaque state vector passed through to device access functions.
 * @param enable Enable shared fixed pattern generator (1 = Enable; 0 = Disable)
 * @param patt_length_bits length of the pattern in bits used in blackhawk7_v1l8p2_config_shared_tx_pattern()
 ...
 *       To get the BER data, user needs to call blackhawk7_v1l8p2_get_BER_data_HW_mode() for ber result.
 *       The time between this function call and call to the blackhawk7_v1l8p2_get_BER_data_HW_mode() can be time_ms long.
 * @param sa__ is an opaque state vector passed through to device access functions.
 ...
 * Note: This function call may take up to time_ms delay to complete after calling the blackhawk7_v1l8p2_config_BER_HW_mode().
 * @param sa__ is an opaque state vector passed through to device access functions.
 * @param ber_data is a struct that contains number of errors and number of bits as elements
...
extern const char* blackhawk7_v1l8p2_e2s_prbs_mode_enum[22];
...
* @param sa__ is an opaque state vector passed through to device access functions.
* @param prbs_err_fec_size Defines the size of the FEC frames in bits 
* @param prbs_err_aggregate_mode Aggregation Mode - SRDS_IEEE_50GE/SRDS_SINAI_50GE/SRDS_IEEE_100GE_2_LANE/SRDS_IEEE_200GE_4_LANE/SRDS_IEEE_400GE_8_LANE..../SRDS_CUSTOM
* @param lane_mask_aggregate Lane mask used for aggregation. Example: binary 01100011 uses lanes 6,5,1,0
* @param timeout_s Time (in seconds) for which the PRBS errors are accumulated
 ...
 * @param sa__ is an opaque state vector passed through to device access functions.
 * @param *status returns with the RX tuning status (1 = BER is optimal; 0 = BER is not optimal)
