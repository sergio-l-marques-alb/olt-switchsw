...
extern err_code_t blackhawk7_v1l8p2_config_BER_HW_mode(srds_access_t *sa__, uint16_t time_ms, struct ber_data_st *ber_data, struct prbs_chk_hw_timer_ctrl_st *prbs_chk_hw_timer_ctrl);
extern err_code_t blackhawk7_v1l8p2_get_BER_data_HW_mode(srds_access_t *sa__, struct ber_data_st *ber_data, struct prbs_chk_hw_timer_ctrl_st *prbs_chk_hw_timer_ctrl); 
...
static srds_info_t blackhawk7_v1l8p2_info;
#endif

srds_info_t *blackhawk7_v1l8p2_INTERNAL_get_blackhawk7_v1l8p2_info_ptr(srds_access_t *sa__) {
#if defined(SERDES_EXTERNAL_INFO_TABLE_EN)
    return blackhawk7_v1l8p2_get_info_table_address(sa__);
#else
    UNUSED(sa__);
...
srds_info_t *blackhawk7_v1l8p2_INTERNAL_get_blackhawk7_v1l8p2_info_ptr_with_check(srds_access_t *sa__) {
    ...
    if (blackhawk7_v1l8p2_info_ptr->signature != SRDS_INFO_SIGNATURE) {
        err_code = blackhawk7_v1l8p2_init_blackhawk7_v1l8p2_info(sa__);
    } 
    if (err_code != ERR_CODE_NONE) {
        EFUN_PRINTF(("ERROR: Serdes Info pointer not initialized correctly\n"));
    }
    return blackhawk7_v1l8p2_info_ptr;
}
...
err_code_t blackhawk7_v1l8p2_INTERNAL_verify_blackhawk7_v1l8p2_info(srds_access_t *sa__, srds_info_t const *blackhawk7_v1l8p2_info_ptr)
{
    ...
    if (blackhawk7_v1l8p2_info_ptr->signature != SRDS_INFO_SIGNATURE) {
        EFUN_PRINTF(("ERROR:  Mismatch in blackhawk7_v1l8p2_info signature.  Expected 0x%08X, but received 0x%08X.\n",
                    SRDS_INFO_SIGNATURE, blackhawk7_v1l8p2_info_ptr->signature));
        return (blackhawk7_v1l8p2_error(sa__, ERR_CODE_INFO_TABLE_ERROR));
    }
...
static uint32_t blackhawk7_v1l8p2_osr_mode_enum_to_int_x1000(uint8_t);
static uint32_t blackhawk7_v1l8p2_osr_mode_enum_to_int_x1000(uint8_t osr_mode) {
    switch(osr_mode) {
    ...
    case BLACKHAWK7_V1L8P2_OSX2P5:    return 2500;
...
err_code_t blackhawk7_v1l8p2_INTERNAL_get_num_bits_per_ms(srds_access_t *sa__, uint8_t select_rx, uint32_t *num_bits_per_ms) {
    ...
    {
        blackhawk7_v1l8p2_osr_mode_st osr_mode_st;
        ENULL_MEMSET(&osr_mode_st, 0, sizeof(blackhawk7_v1l8p2_osr_mode_st));
        EFUN(blackhawk7_v1l8p2_INTERNAL_get_osr_mode(sa__, &osr_mode_st));
        if (select_rx) {
    ...
    EFUN(blackhawk7_v1l8p2_set_pll_idx(sa__,pll_select));
        ...
        *num_bits_per_ms = (uint32_t)((((uint32_t)core_config.vco_rate_in_Mhz * 100000UL) / blackhawk7_v1l8p2_osr_mode_enum_to_int_x1000(osr_mode))*10);
...
err_code_t blackhawk7_v1l8p2_INTERNAL_get_BER_string(srds_access_t *sa__, uint16_t time_ms, char *string) {
    ...
    EFUN(blackhawk7_v1l8p2_INTERNAL_get_BER_data(sa__, time_ms, &ber_data_local, USE_HW_TIMERS));
...
err_code_t blackhawk7_v1l8p2_INTERNAL_get_BER_data(srds_access_t *sa__, uint16_t time_ms, struct ber_data_st *ber_data, enum blackhawk7_v1l8p2_prbs_chk_timer_selection_enum timer_sel) {
    ...
    if(timer_sel == USE_HW_TIMERS) {
        ...
        EFUN(blackhawk7_v1l8p2_config_BER_HW_mode(sa__, time_ms, ber_data, &prbs_chk_hw_timer_ctrl_bak));
        EFUN(blackhawk7_v1l8p2_get_BER_data_HW_mode(sa__, ber_data, &prbs_chk_hw_timer_ctrl_bak));
    } else
    {
        UNUSED(timer_sel);
...
err_code_t blackhawk7_v1l8p2_INTERNAL_pam4_to_bin(srds_access_t *sa__, char var, char bin[]) {
  ...
  switch (var) {
    ...
    default :  ENULL_STRCPY(bin,"");
               ...
               return (blackhawk7_v1l8p2_error(sa__, ERR_CODE_CFG_PATT_INVALID_PAM4));
  }
  return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p2_INTERNAL_compute_bin(srds_access_t *sa__, char var, char bin[]) {
  ...
  switch (var) {
    ...
    default :  ENULL_STRCPY(bin,"");
               ...
               return (blackhawk7_v1l8p2_error(sa__, ERR_CODE_CFG_PATT_INVALID_HEX));
  }
  return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p2_INTERNAL_compute_hex(srds_access_t *sa__, char bin[],uint8_t *hex) {
  ...
  if (!USR_STRCMP(bin,"0000")) {
    ...
    *hex = 0x0;
    return (blackhawk7_v1l8p2_error(sa__, ERR_CODE_CFG_PATT_INVALID_BIN2HEX));
  }
  return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p2_INTERNAL_wrw_uc_var(srds_access_t *sa__, uint32_t addr, uint16_t wr_val) {
    if (addr%2 != 0) {                                       /* Validate even address */
      USR_PRINTF(("Error incorrect addr x%04x\n",addr));
        return (blackhawk7_v1l8p2_error(sa__, ERR_CODE_INVALID_RAM_ADDR));
    }
...
err_code_t blackhawk7_v1l8p2_INTERNAL_read_event_log_with_callback(srds_access_t *sa__,
    ...
    EFUN(blackhawk7_v1l8p2_set_physical_lane(sa__,(uint8_t)(micro_num<<1)));
    EFUN_PRINTF(("\n\n********************************************\n"));
    ...
    if (bypass_micro) {
        ...
        if (blackhawk7_v1l8p2_info_ptr->trace_memory_descending_writes) {
            ...
            if (rd_idx == 0) {
                rd_idx = (uint16_t)blackhawk7_v1l8p2_info_ptr->trace_mem_ram_size;
            }
            --rd_idx;
        }
    } else {
    ...
    if (blackhawk7_v1l8p2_info_ptr->trace_memory_descending_writes) {
        ...
        EFUN(blackhawk7_v1l8p2_INTERNAL_rdblk_uc_generic_ram(sa__,
                                                  blackhawk7_v1l8p2_info_ptr->trace_mem_ram_base + (uint32_t)(blackhawk7_v1l8p2_info_ptr->grp_ram_size*micro_num),
                                                  (uint16_t)blackhawk7_v1l8p2_info_ptr->trace_mem_ram_size,
                                                  rd_idx,
                                                  (uint16_t)blackhawk7_v1l8p2_info_ptr->trace_mem_ram_size,
                                                  arg,
                                                  callback));
...
err_code_t blackhawk7_v1l8p2_INTERNAL_set_rx_pf_main(srds_access_t *sa__, uint8_t val) {
    if (val > PF_MAX_VALUE) {
       return (blackhawk7_v1l8p2_error(sa__, ERR_CODE_PF_INVALID));
    }
...
err_code_t blackhawk7_v1l8p2_INTERNAL_set_rx_vga(srds_access_t *sa__, uint8_t val) {
    ...
    if (val > RX_VGA_CTRL_VAL_MAX) {
      return (blackhawk7_v1l8p2_error(sa__, ERR_CODE_VGA_INVALID));
    }
...
err_code_t blackhawk7_v1l8p2_INTERNAL_set_rx_dfe2(srds_access_t *sa__, int8_t val) {
    if ((val > 31) || (val < -31))
    {
        ...
        return (blackhawk7_v1l8p2_error(sa__, ERR_CODE_DFE_TAP));
    }
...
err_code_t blackhawk7_v1l8p2_INTERNAL_get_rx_dfe2(srds_access_t *sa__, int8_t *val) {
    ESTM(*val = (int8_t)((rd_rxa_dfe_tap2_status() + rd_rxb_dfe_tap2_status())>>1));
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p2_INTERNAL_get_rx_dfe3(srds_access_t *sa__, int8_t *val) {
    ESTM(*val = (int8_t)((rd_rxa_dfe_tap3_status() + rd_rxb_dfe_tap3_status())>>1));
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p2_INTERNAL_get_rx_dfe4(srds_access_t *sa__, int8_t *val) {
    ESTM(*val = (int8_t)((rd_rxa_dfe_tap4_status() + rd_rxb_dfe_tap4_status())>>1));
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p2_INTERNAL_get_rx_dfe5(srds_access_t *sa__, int8_t *val) {
    ESTM(*val = (int8_t)((DFE_TAP_SGB2SG(rd_rxa_dfe_tap5()) + DFE_TAP_SGB2SG(rd_rxb_dfe_tap5()))>>1));
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p2_INTERNAL_get_rx_dfe6(srds_access_t *sa__, int8_t *val) {
    ESTM(*val = (int8_t)((DFE_TAP_SGB2SG(rd_rxa_dfe_tap6()) + DFE_TAP_SGB2SG(rd_rxb_dfe_tap6()))>>1));
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p2_INTERNAL_get_rx_dfe7(srds_access_t *sa__, int8_t *val) {
    ESTM(*val = (int8_t)((DFE_TAP_SGB2SG(rd_rxa_dfe_tap7()) + DFE_TAP_SGB2SG(rd_rxb_dfe_tap7()))>>1));
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p2_INTERNAL_get_rx_dfe8(srds_access_t *sa__, int8_t *val) {
    ESTM(*val = (int8_t)((DFE_TAP_SGB2SG(rd_rxa_dfe_tap8()) + DFE_TAP_SGB2SG(rd_rxb_dfe_tap8()))>>1));
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p2_INTERNAL_get_rx_dfe9(srds_access_t *sa__, int8_t *val) {
    ESTM(*val = (int8_t)((DFE_TAP_SGB2SG(rd_rxa_dfe_tap9()) + DFE_TAP_SGB2SG(rd_rxb_dfe_tap9()))>>1));
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p2_INTERNAL_get_rx_dfe10(srds_access_t *sa__, int8_t *val) {
    ESTM(*val = (int8_t)((DFE_TAP_SGB2SG(rd_rxa_dfe_tap10()) + DFE_TAP_SGB2SG(rd_rxb_dfe_tap10()))>>1));
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p2_INTERNAL_get_rx_dfe11(srds_access_t *sa__, int8_t *val) {
    ESTM(*val = (int8_t)((DFE_TAP_SGB2SG(rd_rxa_dfe_tap11()) + DFE_TAP_SGB2SG(rd_rxb_dfe_tap11()))>>1));
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p2_INTERNAL_get_rx_dfe12(srds_access_t *sa__, int8_t *val) {
    ESTM(*val = (int8_t)((DFE_TAP_SGB2SG(rd_rxa_dfe_tap12()) + DFE_TAP_SGB2SG(rd_rxb_dfe_tap12()))>>1));
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p2_INTERNAL_get_rx_dfe13(srds_access_t *sa__, int8_t *val) {
    ESTM(*val = (int8_t)((DFE_TAP_SGB2SG(rd_rxa_dfe_tap13()) + DFE_TAP_SGB2SG(rd_rxb_dfe_tap13()))>>1));
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p2_INTERNAL_get_rx_dfe14(srds_access_t *sa__, int8_t *val) {
    ESTM(*val = (int8_t)((DFE_TAP_SGB2SG(rd_rxa_dfe_tap14()) + DFE_TAP_SGB2SG(rd_rxb_dfe_tap14()))>>1));
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p2_INTERNAL_get_bwr(srds_access_t *sa__, uint8_t *vga1_bwr, uint8_t *vga2_bwr, uint8_t *eq1_bwr, uint8_t *eq2_bwr) {
  ...
  *eq1_bwr  = blackhawk7_v1l8p2_INTERNAL_gray_to_uint8(*eq1_bwr);
  *eq2_bwr  = blackhawk7_v1l8p2_INTERNAL_gray_to_uint8(*eq2_bwr);
  *vga1_bwr = blackhawk7_v1l8p2_INTERNAL_gray_to_uint8(*vga1_bwr);
  *vga2_bwr = blackhawk7_v1l8p2_INTERNAL_gray_to_uint8(*vga2_bwr);
  return(ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p2_INTERNAL_set_bwr(srds_access_t *sa__, uint8_t vga1_bwr, uint8_t vga2_bwr, uint8_t eq1_bwr, uint8_t eq2_bwr, uint8_t vga_dis_ind) {
  eq1_bwr  = blackhawk7_v1l8p2_INTERNAL_uint8_to_gray(eq1_bwr);
  eq2_bwr  = blackhawk7_v1l8p2_INTERNAL_uint8_to_gray(eq2_bwr);
  vga1_bwr = blackhawk7_v1l8p2_INTERNAL_uint8_to_gray(vga1_bwr);
  vga2_bwr = blackhawk7_v1l8p2_INTERNAL_uint8_to_gray(vga2_bwr);
...
err_code_t blackhawk7_v1l8p2_INTERNAL_validate_full_txfir_cfg(srds_access_t *sa__, enum blackhawk7_v1l8p2_txfir_tap_enable_enum enable_taps, blackhawk7_v1l8p2_txfir_st *txfir) {
    ...
    in_nrz_range = ((enable_taps == BLACKHAWK7_V1L8P2_NRZ_LP_3TAP) || (enable_taps == BLACKHAWK7_V1L8P2_NRZ_6TAP)) ? 1 : 0;
    ...
    return (blackhawk7_v1l8p2_error(sa__, err_code));
}
...
err_code_t blackhawk7_v1l8p2_INTERNAL_apply_full_txfir_cfg(srds_access_t *sa__, enum blackhawk7_v1l8p2_txfir_tap_enable_enum enable_taps, blackhawk7_v1l8p2_txfir_st *txfir) {
    ...
    EFUN(wr_txfir_nrz_tap_range_sel(((enable_taps == BLACKHAWK7_V1L8P2_NRZ_LP_3TAP) || (enable_taps == BLACKHAWK7_V1L8P2_NRZ_6TAP)) ? 1 : 0));
    for (tap_num=0; tap_num<6; tap_num++) {
        EFUN(blackhawk7_v1l8p2_INTERNAL_set_tx_tap(sa__, tap_num, txfir->tap[tap_num]));
    }
    if ((enable_taps == BLACKHAWK7_V1L8P2_NRZ_6TAP) || (enable_taps == BLACKHAWK7_V1L8P2_PAM4_6TAP)) {
...
err_code_t blackhawk7_v1l8p2_INTERNAL_get_pam_eye_margin_est(srds_access_t *sa__, eye_margin_t *eye_margin) {
  ...
  for(i=0; i<NUM_PAM_EYES; i++) {
      ESTM(eye_margin->left_eye_mUI [i] = blackhawk7_v1l8p2_INTERNAL_eye_to_mUI(sa__,  rdv_usr_sts_pam_heye_left(i)));
      ESTM(eye_margin->right_eye_mUI[i] = blackhawk7_v1l8p2_INTERNAL_eye_to_mUI(sa__, rdv_usr_sts_pam_heye_right(i)));
      ESTM(eye_margin->upper_eye_mV [i] = blackhawk7_v1l8p2_INTERNAL_eye_to_mV (sa__, rdv_usr_sts_pam_veye_upper(i), ladder_range));
      ESTM(eye_margin->lower_eye_mV [i] = blackhawk7_v1l8p2_INTERNAL_eye_to_mV (sa__, rdv_usr_sts_pam_veye_lower(i), ladder_range));
  }
  return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p2_display_pam_eye_margin_est(srds_access_t *sa__) {
  ...
  ESTM(blackhawk7_v1l8p2_INTERNAL_get_pam_eye_margin_est(sa__, &eye_margin));
...
uint16_t blackhawk7_v1l8p2_INTERNAL_eye_to_mV(srds_access_t *sa__, uint8_t var, uint8_t ladder_range)
{
    ...
    EFUN(blackhawk7_v1l8p2_INTERNAL_ladder_setting_to_mV(sa__, var, ladder_range, slicer, &nlmv_val));
...
#else
#error "Unknown core for blackhawk7_v1l8p2_INTERNAL_pmd_lock_status()"    
#endif
    return(ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p2_INTERNAL_read_lane_state(srds_access_t *sa__, blackhawk7_v1l8p2_lane_state_st *istate) {
  ...
  EFUN(blackhawk7_v1l8p2_read_rx_afe(sa__, RX_AFE_PF, &state.pf_main));
  EFUN(blackhawk7_v1l8p2_read_rx_afe(sa__, RX_AFE_PF2, &state.pf2_ctrl));
  ESTM(state.bwr_ctrl = rdv_afe_bw_adjust());
  EFUN(blackhawk7_v1l8p2_read_rx_afe(sa__, RX_AFE_VGA, &state.vga));

  ESTM(state.dc_offset = rd_dc_offset_bin());
  EFUN(blackhawk7_v1l8p2_read_rx_afe(sa__, RX_AFE_PF3, &state.pf3_ctrl));
  ESTM(state.p1_lvl = rdv_usr_main_tap_est()/32);
  EFUN(blackhawk7_v1l8p2_INTERNAL_ladder_setting_to_mV(sa__, state.p1_lvl, 0, INVALID_SLICER, &state.p1_lvl));
  if (state.rx_pam_mode == NRZ) {
      ESTM(state.m1_lvl = rd_rx_phase02_status());
      EFUN(blackhawk7_v1l8p2_INTERNAL_ladder_setting_to_mV(sa__, (int8_t)state.m1_lvl, 0, INVALID_SLICER, &state.m1_lvl));
  ...
  ESTM(state.link_time = (uint16_t)((((uint32_t)rdv_usr_sts_link_time())*8)/10));     /* convert from 80us to 0.1 ms units */
  {
      ...
      if (state.rx_lock != rx_lock_at_end) {
          USR_PRINTF(("WARNING: Lane %d rx_lock status changed while reading lane state (rx_lock_start=%d, rx_lock_chg_start=%d, rx_lock_at_end=%d, rx_lock_chg_at_end=%d) \n",
                      blackhawk7_v1l8p2_get_lane(sa__), state.rx_lock, state.rx_lock_chg, rx_lock_at_end, rx_lock_chg_at_end));
      }
  ...
  if (!state.stop_state || (state.stop_state == 0xFF)) {
      /* manually check error code instead of EFUN*/
      {
          ...
          resume_status = blackhawk7_v1l8p2_stop_rx_adaptation(sa__, 0);
          if (resume_status) {
...
err_code_t blackhawk7_v1l8p2_INTERNAL_display_lane_state_no_newline(srds_access_t *sa__) {
  ...
  const char* e2s_rx_pam_mode_enum_srds[3] = {
    "NRZ",
    "P4N",
    "P4E"
  };
  ...
  if(state.osr_mode.tx_rx != 255) {
      ...
      r = (char *)e2s_tx_osr_mode_enum[state.osr_mode.rx];
      {
          char *t;
          uint8_t tx, rx;
          t = (char *)e2s_rx_pam_mode_enum_srds[state.rx_pam_mode];
  ...
  EFUN_PRINTF((" %1d%s", state.sig_det, state.sig_det_chg ? "*" : " "));
  {
      ...
      EFUN(blackhawk7_v1l8p2_get_rx_tuning_status(sa__, &rx_tuning_done));
  ...
  }
  {
      ...
      if ((state.rx_pam_mode != NRZ) && (dss.field.pam_eye_margin_estimation)) {
          EFUN_PRINTF((" ( x , x , x , x ) "));
      }
      else {
  ...
  if(state.ucv_status != 0) {
    EFUN_PRINTF(("\n"));
    EFUN(blackhawk7_v1l8p2_decode_uc_sts(sa__, state.ucv_status));
  }
  if(state.ucv_status_ext != 0) {
    EFUN(blackhawk7_v1l8p2_decode_uc_sts_ext(sa__, state.ucv_status_ext));
  }

  return (ERR_CODE_NONE);
}  /* blackhawk7_v1l8p2_INTERNAL_display_lane_state_no_newline(srds_access_t *sa__) */
...
err_code_t blackhawk7_v1l8p2_INTERNAL_read_core_state(srds_access_t *sa__, blackhawk7_v1l8p2_core_state_st *istate) {
  ...
  EFUN(blackhawk7_v1l8p2_version(sa__, &state.api_version));
...
err_code_t blackhawk7_v1l8p2_INTERNAL_update_usr_ctrl_disable_functions_byte(struct blackhawk7_v1l8p2_usr_ctrl_disable_functions_st *st) {
  ...
  uint16_t in = 0;
  in = (uint16_t)(in << 3); in = (uint16_t)(in | (st->field.reserved & BFMASK(3)));
...
err_code_t blackhawk7_v1l8p2_INTERNAL_check_uc_lane_stopped(srds_access_t *sa__) {
  ...
  if (!is_micro_stopped) {
      return(blackhawk7_v1l8p2_error(sa__, ERR_CODE_UC_NOT_STOPPED));
  } else {
      return(ERR_CODE_NONE);
  }
}
...
err_code_t blackhawk7_v1l8p2_INTERNAL_calc_patt_gen_mode_sel(srds_access_t *sa__, uint8_t *mode_sel, uint8_t *zero_pad_len, uint8_t patt_length) {
  ...
  if (!(140 % patt_length)) {
    ...
    *zero_pad_len = 0;
    ...
    return (blackhawk7_v1l8p2_error(sa__, ERR_CODE_CFG_PATT_INVALID_PATT_LENGTH));
  }
  return(ERR_CODE_NONE);
}
#endif /*SMALL_FOOTPRINT */
...
err_code_t blackhawk7_v1l8p2_INTERNAL_get_refclk_in_hz(srds_access_t *sa__, enum blackhawk7_v1l8p2_pll_refclk_enum refclk, uint32_t *refclk_in_hz) {
    switch (refclk) {
        ...
        case BLACKHAWK7_V1L8P2_PLL_REFCLK_245P76MHZ:      *refclk_in_hz = 245760000; break;
        case BLACKHAWK7_V1L8P2_PLL_REFCLK_250MHZ:         *refclk_in_hz = 250000000; break;
        ...
        case BLACKHAWK7_V1L8P2_PLL_REFCLK_166P015625MHZ:  *refclk_in_hz = 166015625; break;
        ...
        case BLACKHAWK7_V1L8P2_PLL_REFCLK_311P04MHZ:      *refclk_in_hz = 311040000; break;
        ...
        default:
            ...
            *refclk_in_hz = 0xFFFFFFFF;
            return (blackhawk7_v1l8p2_error(sa__, ERR_CODE_REFCLK_FREQUENCY_INVALID));
    }
    return (ERR_CODE_NONE);
}
 ...
 */
static err_code_t _blackhawk7_v1l8p2_get_divisor_ratio(srds_access_t *sa__, enum blackhawk7_v1l8p2_pll_div_enum srds_div, uint16_t *numerator, uint16_t *denominator)
{
    switch (srds_div) {
        ...
        case BLACKHAWK7_V1L8P2_PLL_DIV_74P25:      *denominator = 297;  *numerator = 4000; break;
        ...
        case BLACKHAWK7_V1L8P2_PLL_DIV_90P75:      *denominator = 363;  *numerator = 4000; break;
        ...
        case BLACKHAWK7_V1L8P2_PLL_DIV_54:         *denominator =  27;  *numerator =  500; break;
        case BLACKHAWK7_V1L8P2_PLL_DIV_108:        *denominator =  27;  *numerator =  250; break;
...
static err_code_t _blackhawk7_v1l8p2_get_refclk_from_div_vco(srds_access_t *sa__, uint32_t *refclk_freq_hz, enum blackhawk7_v1l8p2_pll_div_enum srds_div, uint32_t vco_freq_khz, enum blackhawk7_v1l8p2_pll_option_enum pll_option) {
    uint16_t numerator, denominator;
    EFUN(_blackhawk7_v1l8p2_get_divisor_ratio(sa__, srds_div, &numerator, &denominator));
...
static const enum blackhawk7_v1l8p2_pll_div_enum _blackhawk7_v1l8p2_div_candidates[] = {
    ...
    BLACKHAWK7_V1L8P2_PLL_DIV_74P25,
    ...
    BLACKHAWK7_V1L8P2_PLL_DIV_90P75,
    ...
    BLACKHAWK7_V1L8P2_PLL_DIV_54,
    BLACKHAWK7_V1L8P2_PLL_DIV_108,
...
static err_code_t _blackhawk7_v1l8p2_check_div(srds_access_t *sa__, enum blackhawk7_v1l8p2_pll_div_enum srds_div) {
    uint8_t i, found = 0;
    for (i=0; i<_blackhawk7_v1l8p2_div_candidates_count; i++) {
        found = (uint8_t)(found | (srds_div == _blackhawk7_v1l8p2_div_candidates[i]));
    }
    if (!found) {
...
static err_code_t _blackhawk7_v1l8p2_check_vco_freq_khz(srds_access_t *sa__, uint32_t vco_freq_khz) {
    if (vco_freq_khz < SERDES_VCO_FREQ_KHZ_MIN - SERDES_VCO_FREQ_KHZ_TOLERANCE) {
        ...
        return (blackhawk7_v1l8p2_error(sa__, ERR_CODE_VCO_FREQUENCY_INVALID));
    }
...
static err_code_t _blackhawk7_v1l8p2_get_div(srds_access_t *sa__, uint32_t refclk_freq_hz, uint32_t vco_freq_khz, enum blackhawk7_v1l8p2_pll_div_enum *srds_div, enum blackhawk7_v1l8p2_pll_option_enum pll_option) {
    ...
    for (i=0; i<_blackhawk7_v1l8p2_div_candidates_count; i++) {
        ...
        if (vco_freq_khz_min_error > vco_freq_khz_error) {
            ...
            *srds_div = _blackhawk7_v1l8p2_div_candidates[i];
        }
    }
...
err_code_t blackhawk7_v1l8p2_INTERNAL_resolve_pll_parameters(srds_access_t *sa__,
    ...
    const uint8_t given_param_count = (uint8_t)(((refclk == BLACKHAWK7_V1L8P2_PLL_REFCLK_UNKNOWN) ? 0 : 1)
                                       + ((*srds_div == BLACKHAWK7_V1L8P2_PLL_DIV_UNKNOWN) ? 0 : 1)
                                       + ((*vco_freq_khz == 0) ? 0 : 1));
    ...
    if (given_param_count < 2) {
        return (blackhawk7_v1l8p2_error(sa__, ERR_CODE_INSUFFICIENT_PARAMETERS));
    }

    /* Skip verification if option is selected. Error if all parameters not given. */
    if ((pll_option & BLACKHAWK7_V1L8P2_PLL_OPTION_DISABLE_VERIFY) == BLACKHAWK7_V1L8P2_PLL_OPTION_DISABLE_VERIFY) {
    ...
    pll_option = (enum blackhawk7_v1l8p2_pll_option_enum)(pll_option & BLACKHAWK7_V1L8P2_PLL_OPTION_REFCLK_MASK);

    /* The refclk value is checked in various functions below. */

    /* Verify that the requested div value is allowed. */
    if (*srds_div != BLACKHAWK7_V1L8P2_PLL_DIV_UNKNOWN) {
        EFUN(_blackhawk7_v1l8p2_check_div(sa__, *srds_div));
    }
    ...
    if (refclk == BLACKHAWK7_V1L8P2_PLL_REFCLK_UNKNOWN) {
        ...
        EFUN(_blackhawk7_v1l8p2_get_refclk_from_div_vco(sa__, refclk_freq_hz, *srds_div, *vco_freq_khz, pll_option));
    } else {
        EFUN(blackhawk7_v1l8p2_INTERNAL_get_refclk_in_hz(sa__, refclk, refclk_freq_hz));
    }
...
err_code_t blackhawk7_v1l8p2_INTERNAL_poll_uc_dsc_ready_for_cmd_equals_1(srds_access_t *sa__, uint32_t timeout_ms, enum srds_pmd_uc_cmd_enum cmd) {
    ...
    for (loop = 0; loop < 100; loop++) {
        ...
        if (rddata & 0x0080) {    /* bit 7 is uc_dsc_ready_for_cmd */
            if (rddata & 0x0040) {  /* bit 6 is uc_dsc_error_found   */
                ...
                return(blackhawk7_v1l8p2_error(sa__, ERR_CODE_UC_CMD_RETURN_ERROR));
            }
            return (ERR_CODE_NONE);
        }
        if(loop>10) {
    ...
    }
    {
        ...
        blackhawk7_v1l8p2_INTERNAL_print_triage_info(sa__, ERR_CODE_UC_CMD_POLLING_TIMEOUT, 1, 1, (uint16_t)__LINE__);
...
err_code_t blackhawk7_v1l8p2_INTERNAL_poll_dsc_state_equals_uc_tune(srds_access_t *sa__, uint32_t timeout_ms) {
  ...
  return (blackhawk7_v1l8p2_error(sa__, ERR_CODE_POLLING_TIMEOUT));          /* Error Code for polling timeout */
}
...
err_code_t blackhawk7_v1l8p2_INTERNAL_poll_dbgfb_idx_lock(srds_access_t *sa__, uint8_t timeout_ms) {
  ...
  if (idx_lock) {
      ...
      return (blackhawk7_v1l8p2_error(sa__, ERR_CODE_POLLING_TIMEOUT));
  }
}
...
err_code_t blackhawk7_v1l8p2_get_dbgfb_summary(srds_access_t *sa__, dbgfb_cfg_st *dbgfb_cfg) {
    ...
    EFUN(blackhawk7_v1l8p2_INTERNAL_get_rx_pam_mode(sa__, &pam_mode));
    if (pam_mode == NRZ) {
        EFUN_PRINTF(("!!!!ERROR!!!! rx_pam_mode is set to NRZ , blackhawk7_v1l8p2_get_dbgfb_summary cannot continue\n"));
        return(ERR_CODE_INVALID_RX_PAM_MODE);
    }
    ...
    if (dbgfb_cfg->x) {
        ...
        EFUN(blackhawk7_v1l8p2_INTERNAL_poll_dbgfb_idx_lock(sa__, 100));
...
err_code_t blackhawk7_v1l8p2_collect_dbgfb_stats(srds_access_t *sa__, uint32_t time_in_us, dbgfb_stats_st *dbgfb_stats) {
    ...
    EFUN(blackhawk7_v1l8p2_INTERNAL_get_rx_pam_mode(sa__, &pam_mode));
    if (pam_mode == NRZ) {
        EFUN_PRINTF(("!!!!ERROR!!!! rx_pam_mode is set to NRZ , blackhawk7_v1l8p2_collect_dbgfb_stats cannot continue\n"));
        return(ERR_CODE_INVALID_RX_PAM_MODE);
    }
    ...
    for (y=0; y < max_y; y++){
        for (x = 0; x < _X_; x++){
            ...
            dbgfb_cfg.data2 = 0;
            dbgfb_cfg.data3 = 0;
            EFUN(blackhawk7_v1l8p2_get_dbgfb_summary(sa__, &dbgfb_cfg));
            dbgfb_stats->time_in_us = time_in_us;
...
err_code_t blackhawk7_v1l8p2_INTERNAL_display_info_table (srds_access_t *sa__) {
    ...
    EFUN_PRINTF(("signature                 = 0x%X\n", blackhawk7_v1l8p2_info_ptr->signature));
    EFUN_PRINTF(("diag_mem_ram_base         = 0x%X\n", blackhawk7_v1l8p2_info_ptr->diag_mem_ram_base));
    EFUN_PRINTF(("diag_mem_ram_size         = 0x%X\n", blackhawk7_v1l8p2_info_ptr->diag_mem_ram_size));
    EFUN_PRINTF(("core_var_ram_base         = 0x%X\n", blackhawk7_v1l8p2_info_ptr->core_var_ram_base));
    EFUN_PRINTF(("core_var_ram_size         = 0x%X\n", blackhawk7_v1l8p2_info_ptr->core_var_ram_size));
    EFUN_PRINTF(("lane_var_ram_base         = 0x%X\n", blackhawk7_v1l8p2_info_ptr->lane_var_ram_base));
    EFUN_PRINTF(("lane_var_ram_size         = 0x%X\n", blackhawk7_v1l8p2_info_ptr->lane_var_ram_size));
    EFUN_PRINTF(("trace_mem_ram_base        = 0x%X\n", blackhawk7_v1l8p2_info_ptr->trace_mem_ram_base));
    EFUN_PRINTF(("trace_mem_ram_size        = 0x%X\n", blackhawk7_v1l8p2_info_ptr->trace_mem_ram_size));
    EFUN_PRINTF(("micro_var_ram_base        = 0x%X\n", blackhawk7_v1l8p2_info_ptr->micro_var_ram_base));
    EFUN_PRINTF(("lane_count                = 0x%X\n", blackhawk7_v1l8p2_info_ptr->lane_count));
    EFUN_PRINTF(("trace_memory_descending_writes = 0x%X\n", blackhawk7_v1l8p2_info_ptr->trace_memory_descending_writes));
    EFUN_PRINTF(("micro_count               = 0x%X\n", blackhawk7_v1l8p2_info_ptr->micro_count));
    EFUN_PRINTF(("micro_var_ram_size        = 0x%X\n", blackhawk7_v1l8p2_info_ptr->micro_var_ram_size));
    EFUN_PRINTF(("grp_ram_size              = 0x%X\n", blackhawk7_v1l8p2_info_ptr->grp_ram_size));
    EFUN_PRINTF(("ucode_version             = 0x%X\n", blackhawk7_v1l8p2_info_ptr->ucode_version));
    EFUN_PRINTF(("info_table_version        = 0x%X\n", blackhawk7_v1l8p2_info_ptr->info_table_version));
