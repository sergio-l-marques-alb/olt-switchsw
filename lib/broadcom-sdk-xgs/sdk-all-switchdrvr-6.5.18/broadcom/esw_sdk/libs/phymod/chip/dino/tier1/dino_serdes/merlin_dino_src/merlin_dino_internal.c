/***********************************************************************************
 ***********************************************************************************
 *  File Name     :  merlin_dino_internal.c                                         *
 *  Created On    :  13/02/2014                                                    *
 *  Created By    :  Justin Gaither                                                *
 *  Description   :  APIs for Serdes IPs                                           *
 *  Revision      :  $Id: merlin_dino_internal.c 1346 2016-02-06 00:09:35Z kirand $ *
 *                                                                                 *
...
/** @file merlin_dino_internal.c
 * Implementation of API functions
 */

#include <phymod/phymod_system.h>
#include "merlin_dino_internal.h"
...
SDK_STATIC err_code_t _get_afe_hw_version( const phymod_access_t *pa, uint8_t *afe_hw_version) {
...
SDK_STATIC err_code_t _compute_bin( const phymod_access_t *pa, char var, char bin[]) {
    if(!bin) {
        return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
    }
...
SDK_STATIC err_code_t _compute_hex( const phymod_access_t *pa, char bin[],uint8_t *hex) {
    if(!hex) {
        return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
    }
...
SDK_STATIC err_code_t _ull_tx_mode_setup( const phymod_access_t *pa, uint8_t enable) {
    ...
    EFUN(merlin_dino_delay_us(1024));                          /* Wait for phase detector path to settle */
    ...
    EFUN(merlin_dino_poll_st_afe_tx_fifo_resetb_equals_1( pa, 1));  /* Check if FIFO reset is reported to be released within 1ms time interval */
...
SDK_STATIC err_code_t _ull_rx_mode_setup( const phymod_access_t *pa, uint8_t enable) {
...
SDK_STATIC uint8_t _merlin_dino_rdb_uc_var( const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr) {
    uint8_t rddata;
    ...
    EPSTM(rddata = merlin_dino_rdb_uc_ram( pa, err_code_p, addr)); /* Use Micro register interface for reading RAM */

    return (rddata);
}

/* Micro Global RAM Word Read */
SDK_STATIC uint16_t _merlin_dino_rdw_uc_var( const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr) {
  uint16_t rddata;
...
#endif
  EPSTM(rddata = merlin_dino_rdw_uc_ram( pa, err_code_p, (addr))); /* Use Micro register interface for reading RAM */

  return (rddata);
}

/* Micro Global RAM Byte Write  */
SDK_STATIC err_code_t _merlin_dino_wrb_uc_var( const phymod_access_t *pa, uint16_t addr, uint8_t wr_val) {

    return (merlin_dino_wrb_uc_ram( pa, addr, wr_val));                                /* Use Micro register interface for writing RAM */
}
/* Micro Global RAM Word Write  */
SDK_STATIC err_code_t _merlin_dino_wrw_uc_var( const phymod_access_t *pa, uint16_t addr, uint16_t wr_val) {
    if (addr%2 != 0) {                                                                /* Validate even address */
        return (_error(ERR_CODE_INVALID_RAM_ADDR));
    }
    return (merlin_dino_wrw_uc_ram( pa, addr, wr_val));                                 /* Use Micro register interface for writing RAM */
}
...
SDK_STATIC char* _error_val_2_str( const phymod_access_t *pa, uint8_t val) {
        switch (val) {
...
SDK_STATIC uint32_t _float8_to_int32(float8_t input) {
        uint32_t x;
        if(input == 0) return(0);
        x = (input>>5) + 8;
        if((input & 0x1F) < 3) {
...
SDK_STATIC uint32_t _float12_to_uint32( const phymod_access_t *pa, uint8_t byte, uint8_t multi) {
...
SDK_STATIC uint8_t _uint32_to_float12( const phymod_access_t *pa, uint32_t input, uint8_t *multi) {
        int8_t cnt;
        uint8_t output;
        if(!multi) {
                return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
...
SDK_STATIC err_code_t _set_rx_pf_main( const phymod_access_t *pa, uint8_t val) {
        if (val > 15) {
...
SDK_STATIC err_code_t _get_rx_pf_main( const phymod_access_t *pa, int8_t *val) {
                ESTM(*val = (int8_t)rd_pf_ctrl());
 return (ERR_CODE_NONE);
}

SDK_STATIC err_code_t _set_rx_pf2( const phymod_access_t *pa, uint8_t val) {
        if (val > 7) {
                return (_error(ERR_CODE_PF_INVALID));
        }
...
SDK_STATIC err_code_t _get_rx_pf2( const phymod_access_t *pa, int8_t *val) {
                ESTM(*val = (int8_t)rd_pf2_lowp_ctrl());
 return (ERR_CODE_NONE);
}

SDK_STATIC err_code_t _set_rx_vga( const phymod_access_t *pa, uint8_t val) {
...
SDK_STATIC err_code_t _get_rx_vga( const phymod_access_t *pa, int8_t *val) {
...
SDK_STATIC err_code_t _set_rx_dfe3( const phymod_access_t *pa, int8_t val) {
    if ((val > 31) || (val < -31)) {
...
SDK_STATIC err_code_t _get_rx_dfe3( const phymod_access_t *pa, int8_t *val) {
    ESTM(*val = rd_dfe_3_cmn());
 return (ERR_CODE_NONE);
}

SDK_STATIC err_code_t _set_rx_dfe4( const phymod_access_t *pa, int8_t val) {
    if ((val > 15) || (val < -15)) {
...
SDK_STATIC err_code_t _get_rx_dfe4( const phymod_access_t *pa, int8_t *val) {
    ESTM(*val = rd_dfe_4_cmn());
    return (ERR_CODE_NONE);
}

SDK_STATIC err_code_t _set_rx_dfe5( const phymod_access_t *pa, int8_t val) {
    if ((val > 15) || (val < -15)) {
...
SDK_STATIC err_code_t _get_rx_dfe5( const phymod_access_t *pa, int8_t *val) {
...
SDK_STATIC err_code_t _get_tx_pre( const phymod_access_t *pa, int8_t *val) {
    ESTM(*val = rd_txfir_pre_after_ovr());
    return (ERR_CODE_NONE);
}


SDK_STATIC err_code_t _get_tx_main( const phymod_access_t *pa, int8_t *val) {
    ESTM(*val = rd_txfir_main_after_ovr());
    return (ERR_CODE_NONE);
}


SDK_STATIC err_code_t _get_tx_post1( const phymod_access_t *pa, int8_t *val) {
    ESTM(*val = rd_txfir_post_after_ovr());
    return (ERR_CODE_NONE);
}


SDK_STATIC err_code_t _get_tx_post2( const phymod_access_t *pa, int8_t *val) {
...
SDK_STATIC err_code_t _merlin_dino_core_clkgate( const phymod_access_t *pa, uint8_t enable) {
...
SDK_STATIC err_code_t _merlin_dino_lane_clkgate( const phymod_access_t *pa, uint8_t enable) {
...
SDK_STATIC err_code_t _merlin_dino_get_osr_mode( const phymod_access_t *pa, merlin_dino_osr_mode_st *imode) {
    merlin_dino_osr_mode_st mode;

     ENULL_MEMSET(&mode, 0, sizeof(merlin_dino_osr_mode_st));
...
/*SDK_STATIC*/ err_code_t _merlin_dino_read_lane_state( const phymod_access_t *pa, merlin_dino_lane_state_st *istate) {

  merlin_dino_lane_state_st state;
  uint8_t ladder_range = 0;

  ENULL_MEMSET(&state, 0, sizeof(merlin_dino_lane_state_st));

  if(!istate)
          return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));

  EFUN(_merlin_dino_pmd_lock_status( pa, &state.rx_lock, &state.rx_lock_chg));
  ...
  EFUN(_merlin_dino_sigdet_status( pa, &state.sig_det, &state.sig_det_chg));
  ESTM(state.rx_ppm = rd_cdr_integ_reg()/84);
  EFUN(merlin_dino_get_clk90_offset( pa, &state.clk90));
...
SDK_STATIC err_code_t _merlin_dino_display_lane_state_no_newline( const phymod_access_t *pa ) {
  uint16_t lane_idx;
  merlin_dino_lane_state_st state;
  ...
  EFUN(_merlin_dino_read_lane_state( pa, &state));

  lane_idx = merlin_dino_get_lane(pa);
...
SDK_STATIC err_code_t _merlin_dino_read_core_state( const phymod_access_t *pa, merlin_dino_core_state_st *istate) {
  merlin_dino_core_state_st state;
  struct merlin_dino_uc_core_config_st core_cfg;

  ENULL_MEMSET(&state, 0, sizeof(merlin_dino_core_state_st));
  ENULL_MEMSET(&core_cfg, 0, sizeof(struct merlin_dino_uc_core_config_st));
  ...
  {  int16_t                           die_temp = 0;
     EFUN(merlin_dino_read_die_temperature( pa, &die_temp));
  ...
  EFUN(_merlin_dino_pll_lock_status( pa, &state.pll_lock, &state.pll_lock_chg));
...
SDK_STATIC err_code_t _merlin_dino_display_core_state_no_newline( const phymod_access_t *pa ) {
    merlin_dino_core_state_st state;
    ENULL_MEMSET(&state     , 0, sizeof(state     ));
        EFUN(_merlin_dino_read_core_state( pa, &state));
    EFUN_PRINTF(("%02d  "              ,  merlin_dino_get_core(pa)));
    ...
    EFUN(_merlin_dino_display_pll_to_divider( pa,   state.pll_div));
...
SDK_STATIC err_code_t _merlin_dino_pll_lock_status( const phymod_access_t *pa, uint8_t *pll_lock, uint8_t *pll_lock_chg) {

    uint16_t rddata;
    EFUN(merlin_dino_pmd_rdt_reg(pa,PLL_STATUS_ADDR, &rddata));
...
SDK_STATIC err_code_t _merlin_dino_pmd_lock_status( const phymod_access_t *pa, uint8_t *pmd_lock, uint8_t *pmd_lock_chg) {

    uint16_t rddata;
    EFUN(merlin_dino_pmd_rdt_reg(pa,PMD_LOCK_STATUS_ADDR, &rddata));
...
SDK_STATIC err_code_t _merlin_dino_sigdet_status( const phymod_access_t *pa, uint8_t *sig_det, uint8_t *sig_det_chg) {
    uint16_t rddata;
    EFUN(merlin_dino_pmd_rdt_reg(pa,SIGDET_STATUS_ADDR, &rddata));
...
SDK_STATIC err_code_t _update_uc_lane_config_st( const phymod_access_t *pa, struct  merlin_dino_uc_lane_config_st *st) {
  uint16_t in = st->word;
...
SDK_STATIC err_code_t _update_usr_ctrl_disable_functions_st( const phymod_access_t *pa, struct merlin_dino_usr_ctrl_disable_functions_st *st) {
  uint8_t in = st->byte;
...
SDK_STATIC err_code_t _update_usr_ctrl_disable_dfe_functions_st( const phymod_access_t *pa, struct merlin_dino_usr_ctrl_disable_dfe_functions_st *st) {
  uint8_t in = st->byte;
...
SDK_STATIC err_code_t _update_uc_core_config_st( const phymod_access_t *pa, struct merlin_dino_uc_core_config_st *st) {
  uint16_t in = st->word;
...
SDK_STATIC err_code_t _update_uc_core_config_word( const phymod_access_t *pa, struct merlin_dino_uc_core_config_st *st) {
  uint16_t in = 0;
...
SDK_STATIC err_code_t _update_uc_lane_config_word( const phymod_access_t *pa, struct merlin_dino_uc_lane_config_st *st) {
  uint16_t in = 0;
...
SDK_STATIC err_code_t _update_usr_ctrl_disable_functions_byte( const phymod_access_t *pa, struct merlin_dino_usr_ctrl_disable_functions_st *st) {
  uint8_t in = 0;
...
SDK_STATIC err_code_t _update_usr_ctrl_disable_dfe_functions_byte( const phymod_access_t *pa, struct  merlin_dino_usr_ctrl_disable_dfe_functions_st *st) {
  uint8_t in = 0;
...
SDK_STATIC err_code_t _merlin_dino_display_pll_to_divider( const phymod_access_t *pa, uint8_t val) {
...
SDK_STATIC err_code_t _merlin_dino_display_ber_scan_data( const phymod_access_t *pa, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    ...
    uint8_t range250;
    merlin_dino_osr_mode_st osr_mode;
    struct merlin_dino_uc_core_config_st core_config;

    ENULL_MEMSET(&core_config,0,sizeof(core_config));
    ENULL_MEMSET(&osr_mode,0,sizeof(osr_mode));

    EFUN(merlin_dino_get_uc_core_config( pa, &core_config));
    EFUN(_merlin_dino_get_osr_mode( pa, &osr_mode));
    ...
    for(i=0;i < cnt;i++) {
        ...
        EFUN(merlin_dino_pmd_uc_cmd( pa,  CMD_READ_DIAG_DATA_WORD, 0, 100));
        ...
        EFUN(merlin_dino_pmd_uc_cmd( pa,  CMD_READ_DIAG_DATA_BYTE, 0, 100));
    ...
    EFUN_PRINTF(("\n"));
    EFUN(merlin_dino_pmd_uc_cmd( pa, CMD_CAPTURE_BER_END,0x00,2000));
