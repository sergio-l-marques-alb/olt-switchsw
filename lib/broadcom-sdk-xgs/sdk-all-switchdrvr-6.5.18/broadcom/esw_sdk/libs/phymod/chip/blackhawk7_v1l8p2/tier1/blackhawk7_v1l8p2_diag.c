...
err_code_t blackhawk7_v1l8p2_display_eye_scan(srds_access_t *sa__) {
    ...
    ESTM(core=blackhawk7_v1l8p2_get_core(sa__));
    ...
    for (y = y_max;y>=-y_max;y=(int16_t)(y-y_step))
    {
        ...
        EFUN(blackhawk7_v1l8p2_display_eye_scan_stripe(sa__, y,&stripe[buffer_idx]));
        EFUN_PRINTF(("\n"));
    }
...
err_code_t blackhawk7_v1l8p2_INTERNAL_poll_diag_data(srds_access_t *sa__, const srds_info_t *blackhawk7_v1l8p2_info_ptr, uint16_t *status, uint8_t *diag_rd_ptr, uint8_t byte_count, uint32_t timeout_ms) {
    ...
    *diag_rd_ptr = 0;
    ...
    if (byte_count > (lane_diag_size / 2)) {
        ...
        return (blackhawk7_v1l8p2_error(sa__, ERR_CODE_DIAG_TIMEOUT));
    }
...
static err_code_t blackhawk7_v1l8p2_read_eye_scan_stripe_callback(srds_access_t *sa__, void *arg, uint8_t byte_count, uint16_t data) {
    ...
    UNUSED(sa__);
    *(cast_arg->buffer_ptr++) = blackhawk7_v1l8p2_INTERNAL_float8_to_uint32((float8_t)(data & 0xFF));
    if (byte_count > 1) {
        *(cast_arg->buffer_ptr++) = blackhawk7_v1l8p2_INTERNAL_float8_to_uint32((float8_t)(data >> 8));
    }
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p2_read_eye_scan_stripe(srds_access_t *sa__, uint32_t *buffer, uint16_t *status) {
    ...
    EFUN(blackhawk7_v1l8p2_INTERNAL_verify_blackhawk7_v1l8p2_info(sa__, blackhawk7_v1l8p2_info_ptr));
        ...
        lane_diag_base = blackhawk7_v1l8p2_info_ptr->diag_mem_ram_base + ((lane%blackhawk7_v1l8p2_info_ptr->lane_count) * blackhawk7_v1l8p2_info_ptr->diag_mem_ram_size) +
            (uint32_t)(blackhawk7_v1l8p2_info_ptr->grp_ram_size*blackhawk7_v1l8p2_INTERNAL_grp_idx_from_lane(blackhawk7_v1l8p2_get_physical_lane(sa__)));

    EFUN(blackhawk7_v1l8p2_INTERNAL_poll_diag_data(sa__, blackhawk7_v1l8p2_info_ptr, status, &diag_rd_ptr, stripe_size, timeout_ms));

    arg.buffer_ptr = buffer;
    EFUN(blackhawk7_v1l8p2_INTERNAL_rdblk_uc_generic_ram(sa__,
                                              lane_diag_base,
                                              (uint16_t)blackhawk7_v1l8p2_info_ptr->diag_mem_ram_size,
                                              diag_rd_ptr,
                                              stripe_size,
                                              &arg,
                                              blackhawk7_v1l8p2_read_eye_scan_stripe_callback));

    diag_rd_ptr = (uint8_t)(((uint32_t)diag_rd_ptr + stripe_size) % blackhawk7_v1l8p2_info_ptr->diag_mem_ram_size);
    EFUN(wrv_usr_diag_rd_ptr(diag_rd_ptr));
    ESTM(*status = rdv_usr_diag_status() & 0xFF);
    return(ERR_CODE_NONE);
}

err_code_t blackhawk7_v1l8p2_display_eye_scan_stripe(srds_access_t *sa__, int16_t y,uint32_t *buffer) {
    ...
    EFUN(blackhawk7_v1l8p2_INTERNAL_ladder_setting_to_mV(sa__, slicer_range, data_thresh, slicer, &slicer_range_mV));
...
err_code_t blackhawk7_v1l8p2_display_eye_scan_header(srds_access_t *sa__, int8_t i) {
    ...
    uint8_t lane_bak = 0;
    srds_info_t * const blackhawk7_v1l8p2_info_ptr = blackhawk7_v1l8p2_INTERNAL_get_blackhawk7_v1l8p2_info_ptr(sa__);
    ...
    if(i!=1) {ESTM(lane_bak = blackhawk7_v1l8p2_get_lane(sa__));}
    for(lane=0;lane<i;lane++) {
        if(i!=1) {EFUN(blackhawk7_v1l8p2_set_lane(sa__, lane));}
        ESTM(diag_sts[lane] = rdv_usr_diag_status());
        if(blackhawk7_v1l8p2_info_ptr->info_table_version > 0x37) {
            if((diag_sts[lane]&DIAG_EYE_SCAN_RANGE_MASK) == DIAG_EYE_SCAN_HALF) {
                stepsize_var[lane] = 128;
            }
        }
    }
    if(i!=1) {EFUN(blackhawk7_v1l8p2_set_lane(sa__, lane_bak));}
...
err_code_t blackhawk7_v1l8p2_display_eye_scan_footer(srds_access_t *sa__, int8_t i) {
    ...
    if(i!=1) {lane_bak = blackhawk7_v1l8p2_get_lane(sa__);}
    for(lane=0;lane<i;lane++) {
        if(i!=1) {EFUN(blackhawk7_v1l8p2_set_lane(sa__, lane));}
        ESTM(diag_sts[lane] = rdv_usr_diag_status());
        if(blackhawk7_v1l8p2_info_ptr->info_table_version > 0x37) {
...
err_code_t blackhawk7_v1l8p2_read_ber_scan_data(srds_access_t *sa__, uint32_t *errors, uint32_t *timer_values, uint8_t *cnt, uint32_t timeout) {
 ...
 ESTM(sts = rdv_usr_diag_status());
    if((sts & 0x8000) == 0) {
        return(blackhawk7_v1l8p2_error(sa__, ERR_CODE_DATA_NOTAVAIL));
    }
...
err_code_t blackhawk7_v1l8p2_eye_margin_proj_with_data(srds_access_t *sa__, USR_DOUBLE rate, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    ...
    ESTM(offset_start = (int16_t)rd_uc_dsc_data());
    {
        int16_t nlmv_val = 0;
        afe_override_slicer_sel_t slicer = INVALID_SLICER;
        EFUN(blackhawk7_v1l8p2_INTERNAL_ladder_setting_to_mV(sa__, offset_start, 0, slicer, &nlmv_val));
    ...
    EFUN(blackhawk7_v1l8p2_INTERNAL_poll_diag_done(sa__, &sts,(uint32_t)(timer_control*2000)));
...
err_code_t blackhawk7_v1l8p2_eye_margin_proj(srds_access_t *sa__, USR_DOUBLE rate, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
	err_code_t error;
	error = blackhawk7_v1l8p2_eye_margin_proj_with_data(sa__, rate, ber_scan_mode, timer_control, max_error_control);
	return(error);
}
...
#ifndef STANDALONE_EVENT
            if(intrusive) {
                EFUN(blackhawk7_v1l8p2_INTERNAL_ladder_setting_to_mV(sa__, (int16_t)offset[i], 0, slicer, &nlmv_val));
                margins[i] = direction * nlmv_val;
            } else {
                EFUN(blackhawk7_v1l8p2_INTERNAL_ladder_setting_to_mV(sa__, (int16_t)offset[i], range250, slicer, &nlmv_val));
                margins[i] = direction * nlmv_val;
            }
#else
            margins[i] = info_out->margins[i];
...
static err_code_t _display_ber_scan_data(srds_access_t *sa__, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    ...
    if(osr_mode.rx > BLACKHAWK7_V1L8P2_OSX2P5) {
    ...
    if(ber_scan_mode & DIAG_BER_HORZ) {
        ...
        EFUN(blackhawk7_v1l8p2_INTERNAL_ladder_setting_to_mV(sa__, offset_start, range250, slicer, &nlmv_val));
    ...
    for(i=0;i < cnt;i++) {
        ...
        if(DIAG_VERBOSE < 5) {
            ...
            if(ber_scan_mode & DIAG_BER_HORZ) {
                ...
                EFUN(blackhawk7_v1l8p2_INTERNAL_ladder_setting_to_mV(sa__, (int16_t)(SRDS_ABS(offset_start)-i), range250, slicer, &nlmv_val));
...
err_code_t blackhawk7_v1l8p2_display_diag_data(srds_access_t *sa__, uint16_t  diag_level) {
    ...
    core = blackhawk7_v1l8p2_get_core(sa__);
    lane = blackhawk7_v1l8p2_get_lane(sa__);

    if((diag_level & SRDS_DIAG_REG_CORE)
       || (diag_level & SRDS_DIAG_REG_LANE)) {
        EFUN(blackhawk7_v1l8p2_reg_dump(sa__, core, lane, NULL));
    }
    if(diag_level & SRDS_DIAG_UC_CORE) {
        EFUN(blackhawk7_v1l8p2_uc_core_var_dump(sa__, core, NULL));

        {
        uint8_t orig_lane, num_micros, srds_lane;

        ESTM(num_micros = rdc_micro_num_uc_cores());
        orig_lane = blackhawk7_v1l8p2_get_physical_lane(sa__);
        for (srds_lane = 0; srds_lane < num_micros*2; srds_lane =(uint8_t)(srds_lane + 2)) {
            EFUN(blackhawk7_v1l8p2_set_physical_lane(sa__, srds_lane));

            EFUN(blackhawk7_v1l8p2_uc_micro_var_dump(sa__, core, srds_lane, NULL));
        }
        EFUN(blackhawk7_v1l8p2_set_physical_lane(sa__, orig_lane));
        }
    }
    if(diag_level & SRDS_DIAG_UC_LANE) {

        EFUN(blackhawk7_v1l8p2_uc_lane_var_dump(sa__, core, lane, NULL));
    }
    ...
    if((diag_level & SRDS_DIAG_PRBS_PROJ)
        ...
        EFUN_PRINTF(("\n********************************************************\n"));
        EFUN_PRINTF(("Running CORE %d LANE %d PRBS Error Analyzer for %d seconds\n", blackhawk7_v1l8p2_get_core(sa__), blackhawk7_v1l8p2_get_lane(sa__), timeout_s));
        EFUN_PRINTF(("********************************************************\n"));
        /* Display PRBS Error Analyzer for 60s */
        lane_mask_aggregate      = (uint8_t)(0x1 << (blackhawk7_v1l8p2_get_lane(sa__)));
        ...
        blackhawk7_v1l8p2_display_aggregate_prbs_error_analyzer_proj(sa__, 5440, SRDS_IEEE_50GE, lane_mask_aggregate, timeout_s, diag_mode);
    }

    if(diag_level & SRDS_DIAG_INFO_TABLE) {
        EFUN(blackhawk7_v1l8p2_INTERNAL_display_info_table(sa__));
    }
    /* re enable micro */
    if (!micro_stop) {
...
/* Required Diagnostic Functions */
err_code_t blackhawk7_v1l8p2_diag_access(srds_access_t *sa__, enum srds_diag_access_enum type, uint32_t addr, uint32_t data, uint16_t param) {
    srds_info_t * const blackhawk7_v1l8p2_info_ptr = blackhawk7_v1l8p2_INTERNAL_get_blackhawk7_v1l8p2_info_ptr_with_check(sa__);
    ...
    case SRDS_REG_READ:      {
                             ...
                             if(data > 1) {
                                 ...
                                 for (i = 0; i < data; i++) {
                                     ...
                                     EFUN(blackhawk7_v1l8p2_pmd_rdt_reg(sa__, (uint16_t)(i+addr),&tmp));
                                     ESTM_PRINTF(("%04x ", tmp));
                                 }
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 EFUN(blackhawk7_v1l8p2_pmd_rdt_reg(sa__, (uint16_t)addr,&tmp));
    ...
    case SRDS_REG_RMW:       {
                             ...
                             EFUN(blackhawk7_v1l8p2_pmd_wr_reg(sa__, (uint16_t)addr,tmp2));
    ...
    case SRDS_LANE_RAM_READ_BYTE: {
                             if(data > 1) {
                                 ...
                                 ESTM_PRINTF(("Lane RAM Read byte: x%04x = x%02x\n",addr,blackhawk7_v1l8p2_rdbl_uc_var(sa__, __ERR, (uint16_t)addr)));
                             }
                             } break;
    ...
    case SRDS_LANE_RAM_READ_WORD: {
                             if(data > 1) {
                                 ...
                                 ESTM_PRINTF(("Lane RAM Read word: x%04x = x%04x\n",addr,blackhawk7_v1l8p2_rdwl_uc_var(sa__, __ERR, (uint16_t)addr)));
                             }
                             } break;
    ...
    case SRDS_LANE_RAM_RMW_BYTE:  {
                             uint8_t tmp, tmp2;
                             ESTM(tmp = blackhawk7_v1l8p2_rdbl_uc_var(sa__, __ERR, (uint16_t)addr));
                             tmp2 = (uint8_t)((tmp & (uint8_t)~param) | ((uint8_t)param & data));
                             EFUN(blackhawk7_v1l8p2_wrbl_uc_var(sa__, (uint16_t)addr,tmp2));
    ...
    case SRDS_LANE_RAM_RMW_WORD:  {
                             uint16_t tmp, tmp2;
                             ESTM(tmp = blackhawk7_v1l8p2_rdwl_uc_var(sa__, __ERR,(uint16_t)addr));
                             tmp2 = (uint16_t)((tmp & ~param) | (param & (uint16_t)data));
                             EFUN(blackhawk7_v1l8p2_wrwl_uc_var(sa__, (uint16_t)addr,tmp2));
    ...
    case SRDS_GLOB_RAM_READ_BYTE: {
                             if(data > 1) {
                                 ...
                                 ESTM_PRINTF(("Glob RAM Read byte: x%04x = x%02x\n",addr,blackhawk7_v1l8p2_INTERNAL_rdb_uc_var(sa__, __ERR, DATA_RAM_BASE | addr)));
                             }
                             } break;
    case SRDS_GLOB_RAM_RMW_BYTE:  {
                             uint8_t tmp, tmp2;
                             ESTM(tmp = blackhawk7_v1l8p2_INTERNAL_rdb_uc_var(sa__, __ERR, DATA_RAM_BASE | addr));
                             tmp2 = (uint8_t)((tmp & (uint8_t)~param) | ((uint8_t)param & data));
                             EFUN(blackhawk7_v1l8p2_INTERNAL_wrb_uc_var(sa__, DATA_RAM_BASE | addr, tmp2));
    ...
    case SRDS_GLOB_RAM_READ_WORD: {
                             if(data > 1) {
                                 ...
                                 ESTM_PRINTF(("Glob RAM Read word: x%04x = x%04x\n",addr,blackhawk7_v1l8p2_INTERNAL_rdw_uc_var(sa__, __ERR, DATA_RAM_BASE | addr)));
                             }
                             } break;
    case SRDS_GLOB_RAM_RMW_WORD:  {
                             uint16_t tmp, tmp2;
                             ESTM(tmp = blackhawk7_v1l8p2_INTERNAL_rdw_uc_var(sa__, __ERR, DATA_RAM_BASE | addr));
                             tmp2 = (uint16_t)((tmp & ~param) | (param & (uint16_t)data));
                             EFUN(blackhawk7_v1l8p2_INTERNAL_wrw_uc_var(sa__, DATA_RAM_BASE | addr, tmp2));
                             EFUN_PRINTF(("Glob RAM RMW word: x%04x = x%04x -> x%04x\n",addr,tmp,tmp2));
                             } break;
    case SRDS_UC_CMD:        {
                             uint16_t tmp;
                             EFUN(blackhawk7_v1l8p2_pmd_uc_cmd_with_data(sa__, (enum srds_pmd_uc_cmd_enum)addr, (uint8_t)param, (uint16_t)data, GRACEFUL_STOP_TIME));
    ...
    case SRDS_EN_BREAKPOINT: {
        EFUN(blackhawk7_v1l8p2_INTERNAL_en_breakpoint(sa__, (uint8_t)addr));
                             } break;
    case SRDS_GOTO_BREAKPOINT: {
        EFUN(blackhawk7_v1l8p2_INTERNAL_goto_breakpoint(sa__, (uint8_t)addr));
                             } break;
    case SRDS_RD_BREAKPOINT: {
        EFUN(blackhawk7_v1l8p2_INTERNAL_rd_breakpoint(sa__));
                             } break;
    case SRDS_DIS_BREAKPOINT: {
        EFUN(blackhawk7_v1l8p2_INTERNAL_dis_breakpoint(sa__));
                             } break;
