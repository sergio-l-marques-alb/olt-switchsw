...
static long my_am_ts_calc_value_cnt[4][20] = {{0}};
static long my_am_ts_calc_value_avg[4][20] = {{0}};

static long my_am_ts_calc_value_low[4][20]  = {{0}};
static long my_am_ts_calc_value_high[4][20] = {{0}};
static long my_am_ts_calc_value_diff[4][20] = {{0}};
...
int tefmod16_gen3_tx_loopback_control(PHYMOD_ST* pc, int pcs_gloop_en, int starting_lane, int num_lanes)
{
    ...
    tefmod16_gen3_trigger_speed_change(&pa_copy);
...
int tefmod16_gen3_fec_byp_ind_set(PHYMOD_ST *pc, int enable)
{
    RX_X1_CL91_CFGr_t RX_X1_CL91_CFGr_reg;

    RX_X1_CL91_CFGr_CLR(RX_X1_CL91_CFGr_reg);
    /* Bypass error indication function */
    RX_X1_CL91_CFGr_FEC_BYP_IND_ENf_SET(RX_X1_CL91_CFGr_reg, enable);
...
*/
int _tefmod16_gen3_calculate_am_ts(PHYMOD_ST* pc, uint16_t speed,long my_base_ts,uint16_t bit_pos_adj,
                                            uint16_t base_ts_fclk_adj,uint16_t am_block_offset ,uint16_t  cl74_fec_block_num,
                                            uint16_t num_of_bitmuxed_lane, uint16_t bit_muxed_lane, int blk_size, int cl74_en, long *my_am_ts)

{
  ...
  long my_ui_value = 0;
  long my_bit_pos_adj = 0;
  long my_base_ts_fclk_adj = 0;
  long my_am_block_offset = 0;
  long my_cl74_fec_block_num = 0;
  long my_bit_muxed_lane = 0;
  ...
  my_ui_value |= ui_lsb;
  TEFMOD16_GEN3_DBG_IN_FUNC_VAR_INFO(pc, ("%-22s : addr=0x%x my_ui_value           %016lx = ui_msb %08x << 9 | ui_lsb %08x \n", __func__, pc->addr, my_ui_value , ui_msb, ui_lsb));

  TEFMOD16_GEN3_DBG_IN_FUNC_VAR_INFO(pc, ("%-22s : addr=0x%x my_base_ts            %016lx = my_base_ts %03lx_%01lx\n", __func__, pc->addr, my_base_ts , my_base_ts >> 4,  (my_base_ts & 0xf) ));

  /* Sign Extend if the bit offset is negative */
  my_bit_pos_adj = (bit_pos_adj & 0x100) ? 0xfffffe00 | (int) bit_pos_adj : (int) bit_pos_adj ;
  TEFMOD16_GEN3_DBG_IN_FUNC_VAR_INFO(pc, ("%-22s : addr=0x%x my_bit_pos_adj        %016lx = %0ld : Converted bit_pos_adj %03x to Long\n", __func__, pc->addr, my_bit_pos_adj, my_bit_pos_adj, bit_pos_adj)) ;

  my_bit_pos_adj = my_bit_pos_adj - (long) bit_muxed_lane ;
  TEFMOD16_GEN3_DBG_IN_FUNC_VAR_INFO(pc, ("%-22s : addr=0x%x my_bit_pos_adj        %016lx = %0ld : Subtracted (long) bit_muxed_lane %01x\n", __func__, pc->addr, my_bit_pos_adj, my_bit_pos_adj, bit_muxed_lane));

  my_bit_pos_adj = my_bit_pos_adj * (long) num_of_bitmuxed_lane * my_ui_value ;
  TEFMOD16_GEN3_DBG_IN_FUNC_VAR_INFO(pc, ("%-22s : addr=0x%x my_bit_pos_adj        %016lx = my_bit_pos_adj * (long) num_of_bitmuxed_lane %01x * my_ui_value %016lx\n", __func__, pc->addr, my_bit_pos_adj, num_of_bitmuxed_lane, my_ui_value));

  /* Calculate bit muxed lanes */
  my_bit_muxed_lane = (long) bit_muxed_lane * my_ui_value;
  TEFMOD16_GEN3_DBG_IN_FUNC_VAR_INFO(pc, ("%-22s : addr=0x%x my_bit_muxed_lane     %016lx = (long) bit_muxed_lane %01x * my_ui_value %016lx\n", __func__, pc->addr, my_bit_muxed_lane, bit_muxed_lane, my_ui_value));

  /* Calculate my_base_ts_fclk_adj */
  tefmod16_gen3_mapped_frac_ns_get(speed, &frac_ns);

  my_base_ts_fclk_adj = (long) base_ts_fclk_adj * (long) frac_ns;
  TEFMOD16_GEN3_DBG_IN_FUNC_VAR_INFO(pc, ("%-22s : addr=0x%x my_base_ts_fclk_adj   %016lx = (long) base_ts_fclk_adj %02x * (long) frac_ns %04x\n", __func__, pc->addr, my_base_ts_fclk_adj, base_ts_fclk_adj, frac_ns));

  /* Calculate my_am_block_offset */
  my_am_block_offset = (long) am_block_offset * (long) blk_size * (long) num_of_bitmuxed_lane * my_ui_value ;
  TEFMOD16_GEN3_DBG_IN_FUNC_VAR_INFO(pc, ("%-22s : addr=0x%x my_am_block_offset    %016lx = (long) am_block_offset %02x * (long) blk_size %08x * (long) num_of_bitmuxed_lane %01x * my_ui_value %016lx\n", __func__, pc->addr, my_am_block_offset , am_block_offset, blk_size, num_of_bitmuxed_lane, my_ui_value ));
  ...
  if(cl74_en ==1 ) {
    ...
    TEFMOD16_GEN3_DBG_IN_FUNC_VAR_INFO(pc, ("%-22s : addr=0x%x my_cl74_fec_block_num %016lx = (((long) cl74_fec_block_num %02x + (long) am_block_offset %02x) & (long) 0x1f) * (long) num_of_bitmuxed_lane %04x * my_ui_value %016lx : cl74_en = %01x\n", __func__, pc->addr, my_cl74_fec_block_num, cl74_fec_block_num, am_block_offset, num_of_bitmuxed_lane, my_ui_value, cl74_en));
  } else {
    TEFMOD16_GEN3_DBG_IN_FUNC_VAR_INFO(pc, ("%-22s : addr=0x%x my_cl74_fec_block_num %016lx : cl74_en = %01x\n\n", __func__, pc->addr, my_cl74_fec_block_num, cl74_en));
  }
  ...
  *my_am_ts = (my_base_ts << 12) + (my_bit_pos_adj / 512) + (my_bit_muxed_lane >> 9) + my_base_ts_fclk_adj + (my_am_block_offset >> 9) - (my_cl74_fec_block_num >> 9);
  ...
  TEFMOD16_GEN3_DBG_IN_FUNC_VAR_INFO(pc, ("%-22s : addr=0x%x  +  my_base_ts            %012lx_%04lx\n", __func__, pc->addr, ((my_base_ts            <<  12) >> 16), ((my_base_ts            <<  12) & 0xffff)));
  TEFMOD16_GEN3_DBG_IN_FUNC_VAR_INFO(pc, ("%-22s : addr=0x%x +/- my_bit_pos_adj        %012lx_%04lx\n", __func__, pc->addr, ((my_bit_pos_adj        /  512) >> 16), ((my_bit_pos_adj        /  512) & 0xffff)));
  TEFMOD16_GEN3_DBG_IN_FUNC_VAR_INFO(pc, ("%-22s : addr=0x%x  +  my_bit_muxed_lane     %012lx_%04lx\n", __func__, pc->addr, ((my_bit_muxed_lane     >>   9) >> 16), ((my_bit_muxed_lane     >>   9) & 0xffff)));
  TEFMOD16_GEN3_DBG_IN_FUNC_VAR_INFO(pc, ("%-22s : addr=0x%x  +  my_base_ts_fclk_adj   %012lx_%04lx\n", __func__, pc->addr,  (my_base_ts_fclk_adj           >> 16),  (my_base_ts_fclk_adj           & 0xffff)));
  TEFMOD16_GEN3_DBG_IN_FUNC_VAR_INFO(pc, ("%-22s : addr=0x%x  +  my_am_block_offset    %012lx_%04lx\n", __func__, pc->addr, ((my_am_block_offset    >>   9) >> 16), ((my_am_block_offset    >>   9) & 0xffff)));
  TEFMOD16_GEN3_DBG_IN_FUNC_VAR_INFO(pc, ("%-22s : addr=0x%x  -  my_cl74_fec_block_num %012lx_%04lx\n", __func__, pc->addr, ((my_cl74_fec_block_num >>   9) >> 16), ((my_cl74_fec_block_num >>   9) & 0xffff)));

  TEFMOD16_GEN3_DBG_IN_FUNC_VAR_INFO(pc, ("%-22s : addr=0x%x     my_am_ts              %012lx_%04lx\n\n", __func__, pc->addr, (*my_am_ts                    >> 16), (*my_am_ts                      & 0xffff)));
...
int tefmod16_gen3_1588_rx_deskew_set(PHYMOD_ST* pc, int type, phymod_timesync_compensation_mode_t timesync_am_norm_mode)
{
    ...
    long my_base_ts[20] = {0};
    long my_base_ts_max = 0;
    long my_am_ts_calc_value[20] = {0};
    long my_am_ts_calc_value_norm[20] = {0};
    long my_am_ts_calc_value_prev_avg = 0 ;
    long my_am_ts_calc_value_min = {0};
    long my_ref_value = 0;
    long my_am_ts = 0;
    long my_ui_value = 0;
    long my_deskew_val = 0;
    long my_deskew_val_int = 0;
    long my_deskew_val_frac = 0;
    ...
    TEFMOD16_GEN3_DBG_IN_FUNC_VAR_INFO(pc, ("%-22s : addr=0x%x lane_mask=0x%x: my_ui_value=%016lx = ui_msb %08x << 9 | ui_lsb %08x \n",
    ...
    TEFMOD16_GEN3_DBG_IN_FUNC_VAR_INFO(pc, ("%-22s : addr=0x%x lane_mask=0x%x: idx=%0d am_bits=0x%lx fec_en=%0d cl91_en=%d\n",
    ...
    if((type & TEFMOD_RX_DESKEW_INIT) && ((pc->lane_mask == 0xc) || (pc->lane_mask == 0x3) || (pc->lane_mask == 0xf))){
        for(j=0; j<20; j++) {
            ...
            my_am_ts_calc_value_avg[calc_array_idx][j] = 0 ;

            my_am_ts_calc_value_low[calc_array_idx][j]  = 0 ;
            my_am_ts_calc_value_high[calc_array_idx][j] = 0 ;
            my_am_ts_calc_value_diff[calc_array_idx][j] = 0 ;
        }
    }
    ...
    if(type & TEFMOD_RX_DESKEW_CALC) {
        ...
        */
        ...
        for(j = 0 ; j < no_of_vl ; j++) { my_base_ts[j] = base_ts[j] ; }

        /* Find Max my_base_ts */
        my_base_ts_max = my_base_ts[0] ;
        for(j = 1 ; j < no_of_vl ; j++) { if(my_base_ts_max < my_base_ts[j]) my_base_ts_max = my_base_ts[j] ; }

        /* Rollover Check and Adjust */
        for(j = 0 ; j < no_of_vl ; j++) { if((my_base_ts_max - my_base_ts[j]) > 0x08000) my_base_ts[j] |= 0x10000 ; }
        ...
        for(k = 0 ; k < no_of_vl ; k++) {
          TEFMOD16_GEN3_DBG_IN_FUNC_VAR_INFO(pc, ("%-22s : After  am_calc adjust addr=0x%x lane_mask=0x%x : array_num %2d : my_am_ts_calc_value %012lx_%04lx : normalized %012lx_%04lx %9.4f :\n",
                                            __func__, pc_copy.addr, pc_copy.lane_mask, k, my_am_ts_calc_value[k] >> 16, my_am_ts_calc_value[k] & 0xffff,
                                           (my_am_ts_calc_value[k] - my_am_ts_calc_value_min) >> 16, (my_am_ts_calc_value[k] - my_am_ts_calc_value_min) & 0xffff,
                                           (float) (my_am_ts_calc_value[k] - my_am_ts_calc_value_min) / 65536));
        }
        ...
        for(j=0; j<no_of_vl; j++) {
          ...
          TEFMOD16_GEN3_DBG_IN_FUNC_VAR_INFO(pc, ("%-22s : Calc Normalized value addr=0x%x lane_mask=0x%x : array_num %2d : my_am_ts_calc_value=%016lx my_am_ts_calc_value[ref_lane]=%016lx my_am_ts_calc_value_norm=%016lx \n",
                                             __func__, pc_copy.addr, pc_copy.lane_mask, j, my_am_ts_calc_value[j], my_am_ts_calc_value[ref_lane], my_am_ts_calc_value_norm[j]));
        }
        ...
        for(j=0; j<no_of_vl; j++) {
          ...
          if( (my_am_ts_calc_value_high[calc_array_idx][j] == 0) || (my_am_ts_calc_value_high[calc_array_idx][j] < my_am_ts_calc_value_avg[calc_array_idx][j]) ) {
            my_am_ts_calc_value_high[calc_array_idx][j] = my_am_ts_calc_value_avg[calc_array_idx][j] ;
          }

          if( (my_am_ts_calc_value_low[calc_array_idx][j] == 0) || (my_am_ts_calc_value_low[calc_array_idx][j] > my_am_ts_calc_value_avg[calc_array_idx][j]) ) {
            my_am_ts_calc_value_low[calc_array_idx][j] = my_am_ts_calc_value_avg[calc_array_idx][j] ;
          }
          ...
          TEFMOD16_GEN3_DBG_IN_FUNC_VAR_INFO(pc, ("%-22s : Calc new re-avg value : addr=0x%x lane_mask=0x%x :array_num %2d : my_am_ts_calc_value_prev_avg %016lx : my_am_ts_calc_value_cnt %04ld : my_am_ts_calc_value_norm %016lx : re-average my_am_ts_calc_value_avg %016lx : Prev_Avg %9.4f : New_Val %9.4f : Re_Avg %9.4f : Mid %9.4f : Low %9.4f : High %9.4f : Diff %9.4f : Fclk %9.4f + S&H 0.2000 = %9.4f : Fclk+S&H%% %7.2f %% :\n",
                                             __func__, pc_copy.addr, pc_copy.lane_mask, j, my_am_ts_calc_value_prev_avg, my_am_ts_calc_value_cnt[calc_array_idx][j], my_am_ts_calc_value_norm[j],
                                             my_am_ts_calc_value_avg[calc_array_idx][j], (float) (my_am_ts_calc_value_prev_avg) / 65536, (float) (my_am_ts_calc_value_norm[j]) / 65536,
                                             (float) (my_am_ts_calc_value_avg[calc_array_idx][j]) / 65536, (float) ((my_am_ts_calc_value_low[calc_array_idx][j] + my_am_ts_calc_value_high[calc_array_idx][j]) / 2) / 65536,
                                             (float) (my_am_ts_calc_value_low[calc_array_idx][j]) / 65536, (float) (my_am_ts_calc_value_high[calc_array_idx][j]) / 65536,
                                             (float) (my_am_ts_calc_value_diff[calc_array_idx][j]) / 65536, (float) (frac_ns) / 65536, ((float) (frac_ns) / 65536) + 0.2, 
                                             (float) (my_am_ts_calc_value_diff[calc_array_idx][j]) / ((float) (frac_ns) + 0.2) * 100));
    ...
    if(type & TEFMOD_RX_DESKEW_ADJUST) {
        ...
        if((pc->lane_mask == 0x3) || pc->lane_mask == 0xc) {
          for(j=0; j<2; j++) {
              ...
              pc_copy.lane_mask = 0x1 << reg_copy;
              my_deskew_val = my_am_ts_calc_value_avg[calc_array_idx][no_of_lanes_pp];
              my_deskew_val_int  = (my_deskew_val >> 16) & 0xfff; /* deskew value (23-bit)={deskew_int(12-bits),deskew_frac(10-bits)} */
              ...
              TEFMOD16_GEN3_DBG_IN_FUNC_VAR_INFO(pc, ("%-22s : addr=0x%x lane_mask=0x%x : array_num=%02d my_deskew_val=%016lx my_deskew_val_int=%04lx my_deskew_val_frac=%03lx\n",
                                                 __func__, pc_copy.addr, pc_copy.lane_mask, no_of_lanes_pp, my_deskew_val, my_deskew_val_int, my_deskew_val_frac)) ;
         ...
         if(no_of_vl == 4) { /* 40G MLD over 2 lanes has 4 VLs */
          ...
          TEFMOD16_GEN3_DBG_IN_FUNC_VAR_INFO(pc, ("%-22s : addr=0x%x lane_mask=0x%x : my_deskew_val=%016lx my_deskew_val_int=%04lx my_deskew_val_frac=%03lx\n",
                                            __func__, pc_copy.addr, pc_copy.lane_mask, my_deskew_val, my_deskew_val_int, my_deskew_val_frac));
        }
    } /* if(pc->deskew_cnt == 1) */
