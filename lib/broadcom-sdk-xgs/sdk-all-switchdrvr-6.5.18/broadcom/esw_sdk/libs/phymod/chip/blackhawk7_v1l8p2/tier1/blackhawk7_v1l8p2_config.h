 ...
 * Note: blackhawk7_v1l8p2_ucode_load_init() must be called before the first call to blackhawk7_v1l8p2_ucode_load_write(),
 * and blackhawk7_v1l8p2_ucode_load_close() must be called after the last call to blackhawk7_v1l8p2_ucode_load_write().
 * @param sa__ is an opaque state vector passed through to device access functions.
...
/** Ends Microcode loading state. Call after the last blackhawk7_v1l8p2_ucode_load_write().
 * @param sa__ is an opaque state vector passed through to device access functions.
 * @param pram indicates if the microcode is loaded through the PRAM interface (1 = yes; 0 = no )
 * @return Error Code generated by API (returns ERR_CODE_NONE if no errors)
 */
 ...
 * Dummy function to maintain compatibility with BHK7 APIs [calls blackhawk7_v1l8p2_uc_reset(..) internally]
 ...
 * Issues blackhawk7_v1l8p2_uc_reset followed by blackhawk7_v1l8p2 core reset (core_s_rstb) after selecting com_clk as micro_clk
