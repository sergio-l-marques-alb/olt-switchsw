...
err_code_t merlin7_tsc_display_diag_data(srds_access_t *sa__, uint16_t  diag_level) {
    ...
    if(diag_level & SRDS_DIAG_LANE) {
        ...
        EFUN(merlin7_tsc_log_full_pmd_state(sa__, &lane_st));

        EFUN(merlin7_tsc_disp_full_pmd_state(sa__, &lane_st, 1));
    }
...
err_code_t merlin7_tsc_diag_access(srds_access_t *sa__, enum srds_diag_access_enum type, uint16_t addr, uint16_t data, uint16_t param) {
    ...
    case SRDS_REG_READ:      {
                             ...
                             if(data > 1) {
                                 ...
                                 EFUN(merlin7_tsc_pmd_rdt_reg(sa__, addr,&tmp));
    ...
    case SRDS_REG_RMW:       {
                             ...
                             EFUN(merlin7_tsc_pmd_wr_reg(sa__, addr,tmp2));
    ...
    case SRDS_LANE_RAM_READ_BYTE: {
                             if(data > 1) {
                                 ...
                                 ESTM_PRINTF(("Lane RAM Read byte: x%04x = x%02x\n",addr,merlin7_tsc_rdbl_uc_var(sa__, __ERR,addr)));
                             }
                             } break;
    ...
    case SRDS_LANE_RAM_READ_WORD: {
                             if(data > 1) {
                                 ...
                                 ESTM_PRINTF(("Lane RAM Read word: x%04x = x%04x\n",addr,merlin7_tsc_rdwl_uc_var(sa__, __ERR, addr)));
                             }
                             } break;
    ...
    case SRDS_LANE_RAM_RMW_BYTE:  {
                             uint8_t tmp, tmp2;
                             ESTM(tmp = merlin7_tsc_rdbl_uc_var(sa__, __ERR,addr));
                             tmp2 = (uint8_t)((tmp & (uint8_t)~param) | ((uint8_t)param & data));
                             EFUN(merlin7_tsc_wrbl_uc_var(sa__, addr,tmp2));
    ...
    case SRDS_LANE_RAM_RMW_WORD:  {
                             uint16_t tmp, tmp2;
                             ESTM(tmp = merlin7_tsc_rdwl_uc_var(sa__, __ERR,addr));
                             tmp2 = (uint16_t)((tmp & ~param) | (param & data));
                             EFUN(merlin7_tsc_wrwl_uc_var(sa__, addr,tmp2));
    ...
    case SRDS_GLOB_RAM_RMW_BYTE:  {
                             uint8_t tmp, tmp2;
                             ESTM(tmp = merlin7_tsc_INTERNAL_rdb_uc_var(sa__, __ERR,addr));
                             tmp2 = (uint8_t)((tmp & (uint8_t)~param) | ((uint8_t)param & data));
                             EFUN(merlin7_tsc_INTERNAL_wrb_uc_var(sa__, addr,tmp2));
    ...
    case SRDS_GLOB_RAM_RMW_WORD:  {
                             uint16_t tmp, tmp2;
                             ESTM(tmp = merlin7_tsc_INTERNAL_rdw_uc_var(sa__, __ERR,addr));
                             tmp2 = (uint16_t)((tmp & ~param) | (param & data));
                             EFUN(merlin7_tsc_INTERNAL_wrw_uc_var(sa__, addr,tmp2));
                             EFUN_PRINTF(("Glob RAM RMW word: x%04x = x%04x -> x%04x\n",addr,tmp,tmp2));
                             } break;
    case SRDS_UC_CMD:        {
                             uint16_t tmp;
                             EFUN(merlin7_tsc_pmd_uc_cmd_with_data(sa__, (enum srds_pmd_uc_cmd_enum)addr, (uint8_t)param, data, GRACEFUL_STOP_TIME));
