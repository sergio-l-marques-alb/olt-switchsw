       ...
       for (i = start_lane; i < start_lane + num_lane; i++) {
        ...
        for (i = 0; i < num_lane; i++) {
            ...
            casephymod_diag_CL72:
                ...
                PHYMOD_IF_ERR_RETURN
                    (blackhawk7_v1l8p1_display_cl93n72_status(&phy_copy.access));
                break;
#endif
                                        ...
                                        const phymod_phy_eyescan_options_t* eyescan_options)
{
  ...
  struct   blackhawk7_v1l8p1_uc_core_config_st core_cfg;
  if(PHYMOD_EYESCAN_F_PROCESS_GET(flags)) {
     PHYMOD_IF_ERR_RETURN
        (phymod_util_lane_config_get(&phy->access, &start_lane, &num_lane));
     PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));
     for (i = 0; i < num_lane; i++) {
         ...
         if (found == 0) {
             PHYMOD_IF_ERR_RETURN(blackhawk7_v1l8p1_get_uc_core_config(&(phy_copy.access), &core_cfg));
             PHYMOD_IF_ERR_RETURN(blackhawk7_v1l8p1_osr_mode_get(&phy_copy.access, &osr_mode));
        ...
        case PHYMOD_BER_PROJ_PHASE_F_CONFIG:
            ...
            for (i = 0; i < num_lane; i++) {
                ...
                if ((options->ber_proj_hist_errcnt_thresh == 0)) {
                    ...
                    PHYMOD_IF_ERR_RETURN
                        (blackhawk7_v1l8p1_optimize_hist_errcnt_thresh(&phy_copy.access,
                                                                   &err_analyzer,
                                                                   timeout_s,
                ...
                PHYMOD_IF_ERR_RETURN
                    (blackhawk7_v1l8p1_prbs_error_analyzer_config(&phy_copy.access,
                                                              options->ber_proj_fec_size,
                                                              prbs_errcnt_thresh,
                                                              hist_errcnt_thresh));
                PHYMOD_IF_ERR_RETURN
                    (blackhawk7_v1l8p1_display_prbs_error_analyzer_config(&phy_copy.access, prbs_errcnt_thresh, hist_errcnt_thresh, options->ber_proj_timeout_s));
            }
            break;
        case PHYMOD_BER_PROJ_PHASE_F_START:
            for (i = 0; i < num_lane; i++) {
                phy_copy.access.lane_mask = 1 << (start_lane + i);
                PHYMOD_IF_ERR_RETURN(blackhawk7_v1l8p1_error_analyzer_status_clear(&phy_copy.access));
            }
            break;
        case PHYMOD_BER_PROJ_PHASE_F_COLLECT:
            for (i = 0; i < num_lane; i++) {
                phy_copy.access.lane_mask = 1 << (start_lane + i);
                PHYMOD_MEMSET(&err_analyzer, 0, sizeof(blackhawk7_v1l8p1_prbs_err_analyzer_errcnt_st));
                PHYMOD_MEMCPY(err_analyzer.prbs_errcnt, options->ber_proj_prbs_errcnt[i].prbs_errcnt, sizeof(err_analyzer.prbs_errcnt));
                PHYMOD_IF_ERR_RETURN(
                    blackhawk7_v1l8p1_prbs_error_analyzer_accumulate_err_count(&phy_copy.access, &err_analyzer));
