...
err_code_t blackhawk7_v1l8p2_config_shared_tx_pattern(srds_access_t *sa__, uint8_t patt_length_bits, const char pattern[]) {
  ...
  EFUN(blackhawk7_v1l8p2_INTERNAL_calc_patt_gen_mode_sel(sa__, &mode_sel,&zero_pad_len,patt_length_bits));
  ...
  if ((str_len > 2) && ((USR_STRNCMP(pattern, "0x", 2)) == 0)) {
    ...
    for (i=2; i < str_len; i++) {
      if (pattern[i] != '_') {
        ...
        if (actual_patt_len > 240) {
          ...
          return (blackhawk7_v1l8p2_error(sa__, ERR_CODE_CFG_PATT_PATTERN_BIGGER_THAN_MAXLEN));
        }
      }
    }
    ...
    if ((offset_len > 3)  || (actual_patt_len < patt_length_bits)) {
      ...
      return (blackhawk7_v1l8p2_error(sa__, ERR_CODE_CFG_PATT_LEN_MISMATCH));
    }
    else if (offset_len) {
    ...
    for (i=0; i < str_len; i++) {
      if ((pattern[i] == '0') || (pattern[i] == '1')) {
        ...
        return (blackhawk7_v1l8p2_error(sa__, ERR_CODE_CFG_PATT_INVALID_PATTERN));
      }
    }
  ...
  for (i=0; i < 15; i++) {
    ...
    switch (i) {
      ...
      default: EFUN_PRINTF(("ERROR: Invalid write to patt_gen_seq register\n"));
               return (blackhawk7_v1l8p2_error(sa__, ERR_CODE_CFG_PATT_INVALID_SEQ_WRITE));
    }
  }
...
err_code_t blackhawk7_v1l8p2_tx_shared_patt_gen_en(srds_access_t *sa__, uint8_t enable, uint8_t patt_length_bits) {
  uint8_t zero_pad_len = 0; /* suppress warnings, changed by blackhawk7_v1l8p2_INTERNAL_calc_patt_gen_mode_sel() */
  uint8_t mode_sel     = 0; /* suppress warnings, changed by blackhawk7_v1l8p2_INTERNAL_calc_patt_gen_mode_sel() */
  ...
  if (enable) {
    if ((mode_sel < 1) || (mode_sel > 6)) {
      return (blackhawk7_v1l8p2_error(sa__, ERR_CODE_PATT_GEN_INVALID_MODE_SEL));
    }
...
err_code_t blackhawk7_v1l8p2_config_prbs_chk_hw_timer(srds_access_t *sa__, uint16_t time_ms, uint16_t *time_ms_adjusted) {
    ...
    EFUN(blackhawk7_v1l8p2_INTERNAL_get_prbs_timeout_count_from_time(time_ms, time_ms_adjusted, &prbs_chk_hw_timer_ctrl));
...
const char* blackhawk7_v1l8p2_e2s_prbs_mode_enum[22] = {
    " PRBS_7",
    " PRBS_9",
    "PRBS_11",
    "PRBS_15",
...
err_code_t blackhawk7_v1l8p2_prbs_error_analyzer_config(srds_access_t *sa__, blackhawk7_v1l8p2_prbs_err_analyzer_config_status_st *err_analyzer) {
    ...
    if ((err_analyzer->prbs_err_fec_size < BLACKHAWK7_V1L8P2_PRBS_ERR_ANALYZER_FEC_SIZE_MIN) || (err_analyzer->prbs_err_fec_size > BLACKHAWK7_V1L8P2_PRBS_ERR_ANALYZER_FEC_SIZE_MAX)) { 
        return (blackhawk7_v1l8p2_error(sa__, ERR_CODE_INVALID_PRBS_ERR_ANALYZER_FEC_SIZE));
    }

    fec_size = (uint8_t)((err_analyzer->prbs_err_fec_size + BLACKHAWK7_V1L8P2_PRBS_FEC_FRAME_SIZE_ROUND_UP) / BLACKHAWK7_V1L8P2_PRBS_FEC_FRAME_SIZE_MULTIPLIER);

    fec_size_bits = (uint16_t)(fec_size * BLACKHAWK7_V1L8P2_PRBS_FEC_FRAME_SIZE_MULTIPLIER);
    ...
    switch (err_analyzer->prbs_err_aggregate_mode) {
        case SRDS_IEEE_50GE: /* 50G w/ Bit Muxing 2:1 and No Code Word Interleaving 50GE/100GE 56G BHK */
           ...
           err_analyzer->fec_size_frac = BLACKHAWK7_V1L8P2_PRBS_FEC_SIZE_FRAC_1_8;
        ...
        case SRDS_BHK_200GE:              /* 200G w/ Bit Muxing 2:1 and 2 Code Word Interleaving 200GE/400GE 56G BHK */
           ...
           err_analyzer->fec_size_frac = BLACKHAWK7_V1L8P2_PRBS_FEC_SIZE_FRAC_1_4;
           EFUN(wr_tlb_err_symbol_msb_lsb_group(0x1));
           EFUN(wr_tlb_err_fec_msb_en(0x1));
...
err_code_t blackhawk7_v1l8p2_get_prbs_error_analyzer_config(srds_access_t *sa__, blackhawk7_v1l8p2_prbs_err_analyzer_config_status_st *err_analyzer) {
    ...
    err_analyzer->prbs_err_fec_size = (uint16_t)(prbs_err_fec_size * BLACKHAWK7_V1L8P2_PRBS_FEC_FRAME_SIZE_MULTIPLIER);
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p2_get_prbs_error_analyzer_lane_err_count(srds_access_t *sa__, blackhawk7_v1l8p2_prbs_err_analyzer_config_status_st *err_analyzer, uint8_t lane) {
    ...
    if (lane > num_lanes) {
        ...
        EFUN(blackhawk7_v1l8p2_read_dac_pattern_memory(sa__,dacmem,(uint8_t)(4*lane),4));
        ...
        EFUN(blackhawk7_v1l8p2_read_dac_pattern_memory(sa__,dacmem2,(uint8_t)(4*(num_lanes+1)),2));
        ...
        if (PRBS_VERBOSE > 2) {
            EFUN_PRINTF(("\n\n\t=====> DEBUG INFO (start) [Core = %d, Lane = %d]\n", blackhawk7_v1l8p2_get_core(sa__), lane));
...
err_code_t blackhawk7_v1l8p2_get_prbs_error_analyzer_err_count(srds_access_t *sa__, blackhawk7_v1l8p2_prbs_err_analyzer_config_status_st *err_analyzer) {
    ...
    err_analyzer->pp_field2 = blackhawk7_v1l8p2_get_lane(sa__); 
    EFUN(blackhawk7_v1l8p2_INTERNAL_get_num_bits_per_ms(sa__, 1, &err_analyzer->pp_field1));
    ...
    EFUN(blackhawk7_v1l8p2_get_prbs_error_analyzer_lane_err_count(sa__, err_analyzer, lane));

    return (ERR_CODE_NONE);
}

/* Get PRBS Error Analyzer Aggregate Error Count */
err_code_t blackhawk7_v1l8p2_get_prbs_error_analyzer_aggregate_err_count(srds_access_t *sa__, blackhawk7_v1l8p2_prbs_err_analyzer_config_status_st *err_analyzer) {
    ...
    EFUN(blackhawk7_v1l8p2_get_prbs_error_analyzer_lane_err_count(sa__, err_analyzer, num_lanes)); /* For aggregate counters, just pass the num_lanes to point to the correct memory location in the DAC */ 
...
err_code_t blackhawk7_v1l8p2_display_prbs_error_analyzer_err_count(srds_access_t *sa__, blackhawk7_v1l8p2_prbs_err_analyzer_config_status_st *err_analyzer) {
    ...
    if (err_analyzer->diag_mode)
    {
        ...
        EFUN_PRINTF(("%08x\n", err_analyzer->prbs_errcnt[BLACKHAWK7_V1L8P2_PRBS_NUM_OF_ERROR_ANALYZER_COUNTERS-1]));
        EFUN_PRINTF(("%08x\n", err_analyzer->prbs_frames_all));
    }
    else
    {
...
err_code_t blackhawk7_v1l8p2_display_prbs_error_analyzer_aggregate_err_count(srds_access_t *sa__, blackhawk7_v1l8p2_prbs_err_analyzer_config_status_st *err_analyzer) {
    uint8_t i;
...
err_code_t blackhawk7_v1l8p2_display_prbs_error_analyzer_struct(srds_access_t *sa__, blackhawk7_v1l8p2_prbs_err_analyzer_config_status_st *err_analyzer, uint32_t timeout_s) {
    /* Start of data dump */
...
err_code_t blackhawk7_v1l8p2_prbs_error_analyzer_aggregate_config(srds_access_t *sa__, blackhawk7_v1l8p2_prbs_err_analyzer_config_status_st *err_analyzer) {
    uint8_t num_lanes, lane;
    /* uint8_t aggr_mode; */
...
err_code_t blackhawk7_v1l8p2_display_prbs_error_analyzer_proj(srds_access_t *sa__, uint16_t prbs_err_fec_size, uint32_t timeout_s) {
    ...
    blackhawk7_v1l8p2_display_aggregate_prbs_error_analyzer_proj(sa__, prbs_err_fec_size, SRDS_IEEE_50GE, lane_mask_aggregate, timeout_s, 0x0);
    return (ERR_CODE_NONE);
}

/* Display Aggregate PRBS Error Analyzer Projection */
err_code_t blackhawk7_v1l8p2_display_aggregate_prbs_error_analyzer_proj(srds_access_t *sa__, uint16_t prbs_err_fec_size, enum srds_prbs_error_analyzer_aggregate_mode_enum prbs_err_aggregate_mode, uint8_t lane_mask_aggregate, uint32_t timeout_s, uint8_t diag_mode) {
    ...
    ENULL_MEMSET(&err_analyzer, 0, sizeof(blackhawk7_v1l8p2_prbs_err_analyzer_config_status_st));
    ...
    EFUN(blackhawk7_v1l8p2_prbs_error_analyzer_aggregate_config(sa__, &err_analyzer));  /* Configure PRBS Error Analyzer Aggregation Mode*/
    ...
    { 
       ...
       err_analyzer.prbs_frames_all = 0;
       EFUN(blackhawk7_v1l8p2_get_prbs_error_analyzer_aggregate_err_count(sa__, &err_analyzer));
       EFUN(blackhawk7_v1l8p2_display_prbs_error_analyzer_aggregate_err_count(sa__, &err_analyzer));
       EFUN(blackhawk7_v1l8p2_prbs_error_analyzer_compute_proj(sa__, err_analyzer, timeout_s));
    }
...
err_code_t blackhawk7_v1l8p2_prbs_error_analyzer_report_proj(srds_access_t *sa__, blackhawk7_v1l8p2_prbs_err_analyzer_config_status_st err_analyzer, uint32_t timeout_s, blackhawk7_v1l8p2_prbs_err_analyzer_report_st *err_analyzer_report) {
    ...
    ENULL_MEMSET(err_analyzer_report, 0, sizeof(blackhawk7_v1l8p2_prbs_err_analyzer_report_st));
    EFUN(blackhawk7_v1l8p2_INTERNAL_get_num_bits_per_ms(sa__, 1, &num_bits_per_ms));

    if (err_analyzer.fec_size_frac == BLACKHAWK7_V1L8P2_PRBS_FEC_SIZE_FRAC_1_8)
        overlap_factor = 8;
    else if (err_analyzer.fec_size_frac == BLACKHAWK7_V1L8P2_PRBS_FEC_SIZE_FRAC_1_4)
        overlap_factor = 4;
    else if (err_analyzer.fec_size_frac == BLACKHAWK7_V1L8P2_PRBS_FEC_SIZE_FRAC_1_2)
        overlap_factor = 2;
    else
        overlap_factor = 1;
    ...
    for (i=BLACKHAWK7_V1L8P2_PRBS_NUM_OF_ERROR_ANALYZER_COUNTERS; i > 0; i--) {
        if (err_analyzer.prbs_errcnt[i - 1] != 0xFFFFFFFF) {
...
err_code_t blackhawk7_v1l8p2_get_rx_tuning_status(srds_access_t *sa__, uint8_t *status) {
    ESTM(*status = ((rdv_opt_err_pam4_prbsfb_byte_byte() & 0x40) == 0x40));
    return(ERR_CODE_NONE);
}


err_code_t blackhawk7_v1l8p2_config_BER_HW_mode(srds_access_t *sa__, uint16_t time_ms, struct ber_data_st *ber_data, struct prbs_chk_hw_timer_ctrl_st *prbs_chk_hw_timer_ctrl) {
    uint8_t lcklost = 0;
    uint32_t err_cnt= 0;
    uint8_t clk_gate = 0;
    ...
    if((ber_data->prbs_chk_en == 0) || (clk_gate == 1)) {
        ...
        EFUN(blackhawk7_v1l8p2_INTERNAL_prbs_chk_cdr_lock_lost(sa__, &ber_data->cdrlcklost));

        /* save prbs hardware timer config registers and prbs mode */
        EFUN(blackhawk7_v1l8p2_get_prbs_chk_hw_timer_ctrl(sa__, prbs_chk_hw_timer_ctrl));

        /* Configure hardware timer to count prbs errors */
        EFUN(blackhawk7_v1l8p2_config_prbs_chk_hw_timer(sa__, time_ms, &prbs_chk_hw_timer_ctrl->time_ms_adjusted_bak));
        ...
        EFUN(blackhawk7_v1l8p2_INTERNAL_hw_timer_sync_delay(sa__));

    return(ERR_CODE_NONE);
}

err_code_t blackhawk7_v1l8p2_get_BER_data_HW_mode(srds_access_t *sa__, struct ber_data_st *ber_data, struct prbs_chk_hw_timer_ctrl_st *prbs_chk_hw_timer_ctrl) {
    uint8_t lcklost = 0;
    uint32_t err_cnt= 0;
...
*/
    ...
    /* restore prbs hardware timer config registers */
    EFUN(blackhawk7_v1l8p2_set_prbs_chk_hw_timer_ctrl(sa__, prbs_chk_hw_timer_ctrl));
