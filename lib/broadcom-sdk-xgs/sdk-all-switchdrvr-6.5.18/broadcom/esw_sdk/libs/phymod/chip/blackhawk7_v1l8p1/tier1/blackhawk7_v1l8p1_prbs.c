...
err_code_t blackhawk7_v1l8p1_config_shared_tx_pattern(srds_access_t *sa__, uint8_t patt_length, const char pattern[]) {
  ...
  EFUN(blackhawk7_v1l8p1_INTERNAL_calc_patt_gen_mode_sel(sa__, &mode_sel,&zero_pad_len,patt_length));
...
err_code_t blackhawk7_v1l8p1_get_tx_prbs_config(srds_access_t *sa__, enum srds_prbs_polynomial_enum *prbs_poly_mode, uint8_t *prbs_inv) {
  ...
  ESTM(val += (rd_prbs_gen_mode_sel_msb())?PRBS_10:0);  /* Add the MSB for mode_sel. PRBS10 is the offset to the 2nd group of PRBS pattern */
...
err_code_t blackhawk7_v1l8p1_get_rx_prbs_config(srds_access_t *sa__, enum srds_prbs_polynomial_enum *prbs_poly_mode, enum srds_prbs_checker_mode_enum *prbs_checker_mode, uint8_t *prbs_inv) {
  ...
  if (auto_det_en) {
    ...
    if (auto_det_lock) {
      ...
      ESTM(val += (rd_prbs_chk_mode_sel_msb_auto_detect())? PRBS_10:0);  /* Add the MSB for Auto-Detect mode_sel. PRBS10 is the offset to the 2nd group of PRBS pattern */
      *prbs_poly_mode = (enum srds_prbs_polynomial_enum)val;
    ...
    ESTM(val += (rd_prbs_chk_mode_sel_msb())? PRBS_10:0);  /* Add the MSB for mode_sel. PRBS10 is the offset to the 2nd group of PRBS pattern */
    *prbs_poly_mode = (enum srds_prbs_polynomial_enum)val;
...
err_code_t blackhawk7_v1l8p1_display_detailed_prbs_state(srds_access_t *sa__) {
    ...
    ESTM_PRINTF(("  %d ",blackhawk7_v1l8p1_get_lane(sa__)));
...
err_code_t blackhawk7_v1l8p1_prbs_error_analyzer_config(srds_access_t *sa__, blackhawk7_v1l8p1_prbs_err_analyzer_config_status_st *err_analyzer) {
    ...
    fec_size = ((err_analyzer->prbs_err_fec_size + BLACKHAWK7_V1L8P1_PRBS_FEC_FRAME_SIZE_ROUND_UP) / BLACKHAWK7_V1L8P1_PRBS_FEC_FRAME_SIZE_MULTIPLIER);

    fec_size_bits = (fec_size * BLACKHAWK7_V1L8P1_PRBS_FEC_FRAME_SIZE_MULTIPLIER);
    ...
    switch (err_analyzer->prbs_err_aggregate_mode) {
        case SRDS_IEEE_50GE:
           EFUN(wr_tlb_err_fec_size_frac(BLACKHAWK7_V1L8P1_PRBS_FEC_SIZE_FRAC_1_8));    /* Set fraction to 1/8 for single lane*/
#ifdef OSPREY           
           EFUN(wr_tlb_err_fec_4_1_bit_mux_en(0x0));
#endif
...
err_code_t blackhawk7_v1l8p1_get_prbs_error_analyzer_config(srds_access_t *sa__, blackhawk7_v1l8p1_prbs_err_analyzer_config_status_st *err_analyzer) {
    ...
    err_analyzer->prbs_err_fec_size       = prbs_err_fec_size * BLACKHAWK7_V1L8P1_PRBS_FEC_FRAME_SIZE_MULTIPLIER;
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p1_get_prbs_error_analyzer_lane_err_count(srds_access_t *sa__, blackhawk7_v1l8p1_prbs_err_analyzer_config_status_st *err_analyzer, uint8_t lane) {
    ...
    if (lane > num_lanes) {
        ...
        EFUN(blackhawk7_v1l8p1_read_dac_pattern_memory(sa__,dacmem,4*lane,4));
        ...
        if ((dacmem[2]&0x7)>0)     err_analyzer->prbs_frames_all = 0xFFFFFFFF;
        ...
        EFUN(blackhawk7_v1l8p1_read_dac_pattern_memory(sa__,dacmem2,36,2));
        ...
        if (PRBS_VERBOSE > 2) {
            EFUN_PRINTF(("\n\n\t=====> DEBUG INFO (start) [Core = %d, Lane = %d]\n", blackhawk7_v1l8p1_get_core(sa__), lane));
...
err_code_t blackhawk7_v1l8p1_get_prbs_error_analyzer_err_count(srds_access_t *sa__, blackhawk7_v1l8p1_prbs_err_analyzer_config_status_st *err_analyzer) {
    ...
    err_analyzer->pp_field2 = blackhawk7_v1l8p1_get_lane(sa__); 
    EFUN(blackhawk7_v1l8p1_INTERNAL_get_num_bits_per_ms(sa__, &err_analyzer->pp_field1));
...
err_code_t blackhawk7_v1l8p1_display_prbs_error_analyzer_proj(srds_access_t *sa__, uint16_t prbs_err_fec_size, uint32_t timeout_s) {
    ...
    if(!prbs_lock) {
        ...
        err_analyzer.lanes_active      = 0x1 << (blackhawk7_v1l8p1_get_lane(sa__));
        err_analyzer.diag_mode         = 0x0;
...
err_code_t blackhawk7_v1l8p1_diag_prbs_error_analyzer_proj(srds_access_t *sa__, uint16_t prbs_err_fec_size, uint32_t timeout_s) {
    ...
    if(!prbs_lock) {
        ...
        /* Display PRBS Error Analyzer Config and Error Counts*/
...
err_code_t blackhawk7_v1l8p1_prbs_error_analyzer_report_proj(srds_access_t *sa__, blackhawk7_v1l8p1_prbs_err_analyzer_config_status_st err_analyzer, uint32_t timeout_s, blackhawk7_v1l8p1_prbs_err_analyzer_report_st *err_analyzer_report) {
    ...
    EFUN(blackhawk7_v1l8p1_INTERNAL_get_num_bits_per_ms(sa__, &num_bits_per_ms));
    ...
    for(i=0; i < BLACKHAWK7_V1L8P1_PRBS_NUM_OF_ERROR_ANALYZER_COUNTERS; i++) {
        if ((timeout_s !=0) && (num_bits_per_ms !=0)) {
            meas_fec_ber[i] = ((((USR_DOUBLE)(err_analyzer.prbs_errcnt[i]) / (USR_DOUBLE) BLACKHAWK7_V1L8P1_PRBS_ERR_ANALYZER_FRAME_OVERLAP_FACTOR) / (USR_DOUBLE)(timeout_s * 1000)) / (USR_DOUBLE) num_bits_per_ms);
        } 
        else {
        ...
        if (PRBS_VERBOSE > 2) {
            EFUN_PRINTF(("\t Measured Equivalent BER at 't=%2d' (((frameErr/overlap_factor)/time_ms)/rate) = (((%u/%d)/%d)/%d): %0.3e \n",i, err_analyzer.prbs_errcnt[i], 
                                                                                BLACKHAWK7_V1L8P1_PRBS_ERR_ANALYZER_FRAME_OVERLAP_FACTOR, timeout_s*1000, num_bits_per_ms, meas_fec_ber[i]));
        }
    }
    ...
    for (i=BLACKHAWK7_V1L8P1_PRBS_NUM_OF_ERROR_ANALYZER_COUNTERS-1; i >= 0; i--) {
        ...
        if (err_analyzer.prbs_errcnt[i] < 10*BLACKHAWK7_V1L8P1_PRBS_ERR_ANALYZER_FRAME_OVERLAP_FACTOR) {  /* "10*BLACKHAWK7_V1L8P1_PRBS_ERR_ANALYZER_FRAME_OVERLAP_FACTOR"  means  "10 error events" */
            stop_idx = i;
        }
    }
...
err_code_t blackhawk7_v1l8p1_config_BER_HW_mode(srds_access_t *sa__, uint16_t time_ms, struct ber_data_st *ber_data, struct prbs_chk_hw_timer_ctrl_st *prbs_chk_hw_timer_ctrl) {
    ...
    if(prbs_chk_hw_timer_ctrl == NULL) {
    ...
    if((ber_data->prbs_chk_en == 0) || (clk_gate == 1)) {
        ...
        /* Clear prbs error counters and lock lost status */
...
err_code_t blackhawk7_v1l8p1_get_BER_data_HW_mode(srds_access_t *sa__, struct ber_data_st *ber_data, struct prbs_chk_hw_timer_ctrl_st *prbs_chk_hw_timer_ctrl) {
    uint8_t lcklost = 0;
    uint32_t err_cnt= 0;
    uint8_t clk_gate = 0, prbs_chk_en = 0;
    ...
    ESTM(prbs_chk_en = rd_prbs_chk_en());
    ESTM(clk_gate = rd_ln_rx_s_clkgate_frc_on());
    if((prbs_chk_en == 0) || (clk_gate == 1)) {
        return(ERR_CODE_NONE);
    }
    ...
    if (prbs_chk_lock_state == PRBS_CHECKER_NOT_ENABLED) {
        ...
        switch (ber_data->prbs_lck_state) {
            ...
            case PRBS_CHECKER_LOCKED:
                ...
                COMPILER_64_UMUL_32(ber_data->num_bits,
                                    prbs_chk_hw_timer_ctrl->time_ms_adjusted_bak);
