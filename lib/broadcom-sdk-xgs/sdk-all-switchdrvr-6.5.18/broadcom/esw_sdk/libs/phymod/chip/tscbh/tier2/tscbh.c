    ...
    if (intr_type == phymodIntrEccRx1588400g) {
        ...
        if (intr_status.is_1b_err || intr_status.is_2b_err) {
            if ((intr_status.err_addr < mem_offset) ||
                (intr_status.err_addr >= (mem_offset + TBHMOD_TS_TABLE_SIZE))) {
        ...
        if (intr_status.is_1b_err || intr_status.is_2b_err) {
            if (((intr_status.err_addr * 2) < mem_offset) ||
                ((intr_status.err_addr * 2) >= (mem_offset + TBHMOD_TS_TABLE_SIZE))) {
                if (intr_status.is_2b_err) {
        ...
        if (intr_status.is_1b_err || intr_status.is_2b_err) {
            if (((intr_status.err_addr * 2 + 1) < mem_offset) ||
                ((intr_status.err_addr * 2 + 1) >= (mem_offset + TBHMOD_TS_TABLE_SIZE))) {
                if (intr_status.is_2b_err) {
...
int tscbh_phy_ecc_intr_enable_set(const phymod_phy_access_t* phy, phymod_interrupt_type_t intr_type, uint32_t enable)
{
    phymod_phy_access_t phy_copy;

    PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));
    PHYMOD_IF_ERR_RETURN(tbhmod_ecc_error_intr_enable_set(&phy_copy.access, intr_type,  enable));
    return PHYMOD_E_NONE;
}



int tscbh_phy_ecc_intr_enable_get(const phymod_phy_access_t* phy, phymod_interrupt_type_t intr_type, uint32_t* enable)
{
    phymod_phy_access_t phy_copy;

    PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));
    PHYMOD_IF_ERR_RETURN(tbhmod_ecc_error_intr_enable_get(&phy_copy.access, intr_type, enable));
    return PHYMOD_E_NONE;
}
