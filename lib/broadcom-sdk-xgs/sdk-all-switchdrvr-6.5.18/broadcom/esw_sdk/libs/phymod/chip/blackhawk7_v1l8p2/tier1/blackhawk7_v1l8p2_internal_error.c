...
/***********************************************************************************
 ***********************************************************************************
 *  File Name     :  blackhawk7_v1l8p2_internal_error.c                                   *
...
/** @file blackhawk7_v1l8p2_internal_error.c
...
err_code_t blackhawk7_v1l8p2_INTERNAL_print_err_msg(srds_access_t *sa__, uint16_t err_code, const char *filename, const char *func_name, uint16_t line_num)
{
    if (err_code != ERR_CODE_NONE) {
        USR_PRINTF(("ERROR: SerDes err_code = %s : %d filename = %s, function = %s, linenum=%d\n",blackhawk7_v1l8p2_INTERNAL_e2s_err_code(err_code),err_code, filename, func_name, line_num));
    }
    return err_code;
}
err_code_t blackhawk7_v1l8p2_INTERNAL_print_err_msg_and_triage_info(srds_access_t *sa__, uint16_t err_code, const char *filename, const char *func_name, uint16_t line_num)
{
    if (err_code != ERR_CODE_NONE) {
        USR_PRINTF(("ERROR:%s->%s() SerDes err_code = %s : %d %d\n", filename, func_name, blackhawk7_v1l8p2_INTERNAL_e2s_err_code(err_code),err_code, line_num));
        blackhawk7_v1l8p2_INTERNAL_print_triage_info(sa__, err_code, 1, 1, line_num);
    }
    return err_code;
}
