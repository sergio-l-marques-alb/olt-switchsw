...
err_code_t blackhawk7_v1l8p1_core_pwrdn(srds_access_t *sa__, enum srds_core_pwrdn_mode_enum mode) {
    ...
    case PWR_ON:
      ...
      /*  EFUN(wrc_ams_pll_pwrdn_bias4reflad(0x0)); */
     /*   EFUN(wrc_ams_pll_pwrdn_constant_gm(0x0)); */
        EFUN(wrc_ams_pll_pwrdn_buf(0x0));
        EFUN(wrc_ams_pll_pwrdn_buf_2(0x0));
        EFUN(wrc_ams_pll_bias_en(0x1));
    ...
    case PWRDN_DEEP:
        ...
        EFUN(USR_DELAY_NS(500)); /* wait >50 comclk cycles  */
        EFUN(wrc_ams_pll_pwrdn(0x1));
        EFUN(wrc_ams_pll_bias_en(0x0));
        EFUN(wrc_ams_pll_pwrdn_buf_2(0x1));
        EFUN(wrc_ams_pll_pwrdn_buf(0x1));
       /* EFUN(wrc_ams_pll_pwrdn_constant_gm(0x1)); */
       /* EFUN(wrc_ams_pll_pwrdn_bias4reflad(0x1)); */
...
err_code_t blackhawk7_v1l8p1_lane_pwrdn(srds_access_t *sa__, enum srds_core_pwrdn_mode_enum mode) {
    ...
    case PWR_ON:
        ...
        /* afe power on rx */
        EFUN(wr_ams_rx_sigdet_pwrdn(0x0));
        EFUN(wr_ams_rx_ibias(0x4));
        EFUN(wr_ams_rx_ibias_pibuf(0x4));
        EFUN(wr_ams_rx_clkmux_ibias(0x4));
        EFUN(wr_ams_rx_phase_interpolators_ibias(0x4));
        EFUN(wr_afe_rx_pwrdn_frc_val(0x0));
        EFUN(wr_afe_rx_pwrdn_frc(0x0));
        EFUN(wr_afe_rx_reset_frc_val(0x0));
        EFUN(wr_afe_rx_reset_frc(0x0));
        EFUN(wr_ln_rx_s_pwrdn(0x0));
        /* afe power on tx */
        EFUN(wr_ams_rx_clkmux_tx_ibias(0x4));
        EFUN(wr_afe_tx_pwrdn_frc_val(0x0));
        EFUN(wr_afe_tx_pwrdn_frc(0x0));
    ...
    case PWRDN_TX:
        ...
        EFUN(wr_afe_tx_pwrdn_frc(0x1));
        EFUN(wr_afe_tx_pwrdn_frc_val(0x1));
        EFUN(wr_ams_rx_clkmux_tx_ibias(0x0));
        break;
    case PWRDN_RX:
        EFUN(wr_ln_rx_s_pwrdn(0x1));
        EFUN(wr_afe_rx_reset_frc(0x1));
        EFUN(wr_afe_rx_reset_frc_val(0x1));
        EFUN(USR_DELAY_NS(500));
        EFUN(wr_afe_rx_pwrdn_frc(0x1));
        EFUN(wr_afe_rx_pwrdn_frc_val(0x1));
        EFUN(wr_ams_rx_phase_interpolators_ibias(0x0));
        EFUN(wr_ams_rx_clkmux_ibias(0x0));
        EFUN(wr_ams_rx_ibias_pibuf(0x0));
        EFUN(wr_ams_rx_ibias(6));
        EFUN(wr_ams_rx_sigdet_pwrdn(0x1));
        break;
    case PWROFF_DEEP:
        EFUN(blackhawk7_v1l8p1_lane_pwrdn(sa__, PWRDN_TX));
...
err_code_t blackhawk7_v1l8p1_pwrdn_all(srds_access_t *sa__, enum srds_core_pwrdn_mode_enum mode) {
    ...
    {
        uint8_t lane_orig = blackhawk7_v1l8p1_get_lane(sa__);
        uint8_t lane;
