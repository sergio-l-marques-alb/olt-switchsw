...
typedef struct blackhawk7_v1l8p2_triage_info_st {
   uint32_t     api_ver;
   uint32_t     ucode_ver;
   uint8_t      core;
   uint8_t      lane;
...
/* Change the size if blackhawk7_v1l8p2_detailed_lane_status_st size changes */
...
#if !defined(EXCLUDE_FOR_SWIG)
typedef struct { 
    uint8_t extended_lane_st_tmp[sizeof(struct blackhawk7_v1l8p2_detailed_lane_status_st)==SIZE_OF_EXTENDED_LANE_STATE ? 1: -1]; 
} blackhawk7_v1l8p2_detailed_lane_status_st_size_check;
