...
static err_code_t _merlin7_tsc_reg_print_with_buf(srds_access_t *sa__, uint8_t reg_section, char reg_buffer[][250], uint8_t *buf_index);
static err_code_t _merlin7_tsc_reg_print(srds_access_t *sa__, uint8_t reg_section, char reg_buffer[][250], uint8_t *buf_index);
...
err_code_t merlin7_tsc_reg_dump(srds_access_t *sa__, uint8_t core, uint8_t lane, char reg_buffer[][250])
{    
...
err_code_t merlin7_tsc_uc_core_var_dump(srds_access_t *sa__, uint8_t core, char ram_buffer[][250]) {
  uint8_t rddata;
...
err_code_t merlin7_tsc_uc_lane_var_dump(srds_access_t *sa__, uint8_t core, uint8_t lane, char ram_buffer[][250]) {
...
err_code_t merlin7_tsc_log_full_pmd_state_noPRBS (srds_access_t *sa__, struct merlin7_tsc_detailed_lane_status_st *lane_st) {
  ...
  ESTM(lane_st->dfe1_dcd = (int8_t)(rd_dfe_1_e()-rd_dfe_1_o()));
  ESTM(lane_st->dfe2_dcd = (int8_t)((rd_dfe_2_se() ? -rd_dfe_2_e() : rd_dfe_2_e()) -(rd_dfe_2_so() ? -rd_dfe_2_o() : rd_dfe_2_o())));
...
err_code_t merlin7_tsc_log_full_pmd_state (srds_access_t *sa__, struct merlin7_tsc_detailed_lane_status_st *lane_st) {
    ...
    EFUN(merlin7_tsc_INTERNAL_get_BER_string(sa__,100,&lane_st->ber_string[0]));
