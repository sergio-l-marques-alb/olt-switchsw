...
err_code_t blackhawk_tsc_display_eye_scan(srds_access_t *sa__) {
    ...
    uint16_t   status = 0;
    int8_t     y,y_max,y_step,core,lane;
    ...
    ESTM(core=blackhawk_tsc_get_core(sa__));
    ESTM(lane=blackhawk_tsc_get_lane(sa__));
...
err_code_t blackhawk_tsc_display_lane_state_legend(void) {
  ...
  EFUN_PRINTF(("PF(M,L,H,R): Peaking Filter Main (0..15), Low Freq (0..7), High Freq (0..7), Peaking/BW Reduction settings\n"));
  ...
  EFUN_PRINTF(("         2 - AFE BW, For NR mode (0 to 5 - for shortest to longest), For ER mode (0 to 3 - for shortest to longest)\n"));
...
err_code_t blackhawk_tsc_display_diag_data(srds_access_t *sa__, uint16_t  diag_level) {
    ...
    if(diag_level & (SRDS_DIAG_EVENT|SRDS_DIAG_EVENT_SORT)) {
    ...
    if(diag_level & SRDS_DIAG_INFO_TABLE) {
        EFUN(blackhawk_tsc_INTERNAL_display_info_table(sa__));
    }
