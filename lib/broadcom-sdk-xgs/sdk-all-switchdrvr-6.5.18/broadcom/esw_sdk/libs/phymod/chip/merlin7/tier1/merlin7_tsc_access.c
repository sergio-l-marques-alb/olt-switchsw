...
static uint16_t merlin7_tsc_INTERNAL_get_lane_addr(srds_access_t *, uint16_t, uint8_t);
static uint16_t merlin7_tsc_INTERNAL_get_core_address(srds_access_t *, uint16_t);
...
static uint16_t merlin7_tsc_INTERNAL_get_lane_addr(srds_access_t *sa__, uint16_t addr, uint8_t lane) {
    ...
    if (merlin7_tsc_info_ptr != NULL) {
        lane_var_ram_base = (uint16_t)merlin7_tsc_info_ptr->lane_var_ram_base;
        ...
        lane_addr_offset = (uint16_t)(lane_var_ram_base+addr+((lane%lane_count)*lane_var_ram_size) + (grp_ram_size*merlin7_tsc_INTERNAL_grp_idx_from_lane(lane)));
    }
...
uint8_t merlin7_tsc_rdbl_uc_var(srds_access_t *sa__, err_code_t *err_code_p, uint16_t addr) {
...

    {
        ...
        lane_addr_offset = merlin7_tsc_INTERNAL_get_lane_addr(sa__, addr,lane);
...
static uint16_t merlin7_tsc_INTERNAL_get_core_address(srds_access_t *sa__, uint16_t addr) {
    uint16_t core_var_ram_base=0;
    uint16_t core_addr_offset = 0;
    srds_info_t const * const merlin7_tsc_info_ptr = merlin7_tsc_INTERNAL_get_merlin7_tsc_info_ptr_with_check(sa__);

    if (merlin7_tsc_info_ptr != NULL) {
        core_var_ram_base = (uint16_t)merlin7_tsc_info_ptr->core_var_ram_base;
...
uint8_t merlin7_tsc_rdbc_uc_var(srds_access_t *sa__, err_code_t *err_code_p, uint8_t addr) {
    ...
    {
        core_addr_offset = merlin7_tsc_INTERNAL_get_core_address(sa__, addr);
...
uint32_t merlin7_tsc_rd_long_uc_prog_ram(srds_access_t *sa__, err_code_t *err_code_p, uint16_t addr) {
   uint32_t rddata;
   if(!err_code_p) {
        return(0);
   }
...
err_code_t merlin7_tsc_rdblk_uc_prog_ram(srds_access_t *sa__, uint8_t *mem, uint16_t addr, uint16_t cnt) {
...
err_code_t merlin7_tsc_wr_long_uc_ram(srds_access_t *sa__, uint16_t offset, uint32_t value) {
    EFUN(merlin7_tsc_wrw_uc_ram(sa__, (uint16_t)(offset+2), (uint16_t)(value >> 16)));
    EFUN(merlin7_tsc_wrw_uc_ram(sa__, offset, value & 0xFFFF));
    return (ERR_CODE_NONE);
}
/* Micro RAM Word Write */
err_code_t merlin7_tsc_wrw_uc_ram(srds_access_t *sa__, uint16_t addr, uint16_t wr_val) {
...
err_code_t merlin7_tsc_wrb_uc_ram(srds_access_t *sa__, uint16_t addr, uint8_t wr_val) {
    EFUN(wrc_micro_autoinc_wraddr_en(0));
...
/* Micro RAM Long Read */
uint32_t merlin7_tsc_rd_long_uc_ram(srds_access_t *sa__, err_code_t *err, uint16_t offset) {
    uint32_t result;
    ESTM(result = (uint32_t)(merlin7_tsc_rdw_uc_ram(sa__, err, (uint16_t)(offset+2)) << 16));
    ESTM(result |= merlin7_tsc_rdw_uc_ram(sa__, err, offset) & 0xFFFF);
    return result;
}
/* Micro RAM Word Read */
uint16_t merlin7_tsc_rdw_uc_ram(srds_access_t *sa__, err_code_t *err_code_p, uint16_t addr) {
   uint16_t rddata;
   if(!err_code_p) {
        return(0);
   }
...
/* Micro RAM Byte Read */
uint8_t merlin7_tsc_rdb_uc_ram(srds_access_t *sa__, err_code_t *err_code_p, uint16_t addr) {
    uint8_t rddata;
    if(!err_code_p) {
        return(0);
    }
...
err_code_t merlin7_tsc_wrws_uc_ram(srds_access_t *sa__, uint16_t addr, int16_t wr_val) {
    return (merlin7_tsc_wrw_uc_ram(sa__, addr, (uint16_t)wr_val));
}

/* Micro RAM Byte Signed Write */
err_code_t merlin7_tsc_wrbs_uc_ram(srds_access_t *sa__, uint16_t addr, int8_t wr_val) {
    return (merlin7_tsc_wrb_uc_ram(sa__, addr, (uint8_t)wr_val));
}

/* Micro RAM Word Signed Read */
int16_t merlin7_tsc_rdws_uc_ram(srds_access_t *sa__, err_code_t *err_code_p, uint16_t addr) {
    return ((int16_t)merlin7_tsc_rdw_uc_ram(sa__, err_code_p, addr));
}

/* Micro RAM Byte Signed Read */
int8_t merlin7_tsc_rdbs_uc_ram(srds_access_t *sa__, err_code_t *err_code_p, uint16_t addr) {
    return ((int8_t)merlin7_tsc_rdb_uc_ram(sa__, err_code_p, addr));
}

/* Micro RAM Block Read */
err_code_t merlin7_tsc_rdblk_uc_ram(srds_access_t *sa__, uint8_t *mem, uint16_t addr, uint16_t cnt) {
    merlin7_tsc_INTERNAL_rdblk_callback_arg_t arg;
    ...
    arg.mem_ptr = mem;
    return merlin7_tsc_INTERNAL_rdblk_uc_generic_ram(sa__, (uint32_t)addr | 0x20000000, cnt, 0, cnt, &arg, merlin7_tsc_INTERNAL_rdblk_callback);
}
