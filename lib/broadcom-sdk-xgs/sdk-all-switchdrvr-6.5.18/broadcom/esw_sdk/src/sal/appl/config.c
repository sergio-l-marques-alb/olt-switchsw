...
sal_config_get_rvalue(char *str, sc_t *sc)
{
    ...
    if (SAL_BOOT_PLISIM)
    {
        if (sal_strncmp(sc->sc_name, "stable_location", 15) == 0)
        {
            sal_strncpy(sc->sc_value, "3", 2);
            sc->sc_value[2] = '\0';
        }

        if (sal_strncmp(sc->sc_name, "stable_filename", 15) == 0)
        {
            sal_strncpy(sc->sc_value, "warmboot_data_0", 16);
            sc->sc_value[16] = '\0';
        }
    }
#endif /* BCM_DNX_SUPPORT */
...
#else
        return sal_config_refresh_fileless();
#endif /* IN_CODE_SUBSTITUTE */
    }
...
#ifdef IN_CODE_SUBSTITUTE
int
sal_config_refresh_fileless(void)
{
    int rv = 0;
    ...
    rv = pre_compiled_bridge_router_additional_soc_file();
    ...
    return pre_compiled_bridge_router_soc_properties_autogenerated(&sal_config_compiled_values, &sal_config_imported_values);

}

/*
 * Function:
 *  sal_config_set_full_info
 * Purpose:
 *  Adds a entry in the sal_config list with provided all information.
 * Parameters:
 *  name - Name of the soc property
 *  value - Value
 *  flag - Relevant flags (SC_COMPILE_VALUE, SC_FLUSHED and SC_HIT)
 *  line - Specify line of the soc property
 *  sc_hash - Hash that was created for a specific soc property name.
 * Returns:
 *  0 - success
 *  -1 - failed.
 */
int
sal_config_set_full_info(char *name, char *value, uint16 flag, uint16 line, sc_hash_t sc_hash)
{
    sc_t  *sc;
    sc_t  *found_sc;

    sc = sal_config_list[sc_hash];

    /* Check for pre-existing default or duplicates in the file */        
    found_sc = sal_config_list[sc_hash];
    while (found_sc != NULL) {
        if (sal_strcmp(found_sc->sc_name, name) == 0) {
            break;
        }
        found_sc = found_sc->sc_next;
    }
    ...
    /*
     * If we already have a entry, just exit with no error.
     */
    if (found_sc != NULL)
    {
        return 0;
    ...
    sc->sc_next = sal_config_list[sc_hash];
    sal_config_list[sc_hash] = sc;
