...
static void
_diag_flexe_reg_mem_access_mode(
    int is_set,
    int *mode)
{
    static int flexe_access_mode = 0;

    if (is_set)
    {
        flexe_access_mode = *mode;
    }
    else
    {
        *mode = flexe_access_mode;
    }
}

int
diag_flexe_reg_mem_access_mode_get(
    )
{
    int rc = 0;

    _diag_flexe_reg_mem_access_mode(0, &rc);
    return rc;
}

void
diag_flexe_reg_mem_access_mode_set(
    int mode)
{
    _diag_flexe_reg_mem_access_mode(1, &mode);
    return;
}
    ...
    uint32 *value)
{
    ...
    acc_mode = diag_flexe_reg_mem_access_mode_get();
    if (acc_mode == 0)
    {
        SHR_IF_ERR_EXIT(diag_regp_read(unit, address, value));
    }
    else
    {
    ...
    uint32 *value)
{
    ...
    if (acc_mode == 0)
    {
        SHR_IF_ERR_EXIT(diag_ram_read(unit, address, index, width, value));
    }
    else
    {
    ...
    uint32 value)
{
    ...
    if (acc_mode == 0)
    {
        SHR_IF_ERR_EXIT(diag_regp_write(unit, address, value));
    }
    else
    {
    ...
    uint32 *value)
{
    ...
    if (acc_mode == 0)
    {
        SHR_IF_ERR_EXIT(diag_ram_write(unit, address, index, width, value));
    }
    else
    {
    ...
    bcm_port_t * dst_port)
{
    ...
    if (traffic_type == DBAL_ENUM_FVAL_FEU_RMC_TRAFFIC_TYPE_ESB)
    {
        ...
        {
            *dst_port = SOC_PORT_INVALID;
            SHR_EXIT_WITH_LOG(_SHR_E_NONE, "The traffic_type ESB port_or_channel is 0.%s%s%s\n", EMPTY, EMPTY, EMPTY);
        }
    }
             ...
             (traffic_type == DBAL_ENUM_FVAL_FEU_RMC_TRAFFIC_TYPE_ILU_HP))
    {
        ...
        {
            *dst_port = SOC_PORT_INVALID;
            SHR_EXIT_WITH_LOG(_SHR_E_NONE, "The traffic_type ILU port_or_channel is 0.%s%s%s\n", EMPTY, EMPTY, EMPTY);
        }
#endif
    }
    else
    {
        *dst_port = SOC_PORT_INVALID;
        SHR_EXIT_WITH_LOG(_SHR_E_NONE, "The traffic_type is %d.%s%s\n", traffic_type, EMPTY, EMPTY);
    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    {
        oh_status = 0;
        SHR_IF_ERR_EXIT(diag_flexe_core_oh_instance_alm_get(unit, instance_id, &oh_status));
        if (oh_status)
        {
            PRT_ROW_ADD(PRT_ROW_SEP_NONE);
