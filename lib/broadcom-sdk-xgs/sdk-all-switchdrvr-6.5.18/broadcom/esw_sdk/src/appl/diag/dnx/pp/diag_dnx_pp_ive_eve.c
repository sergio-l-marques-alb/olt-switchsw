    ...
    sh_sand_control_t * sand_control)
{
    ...
    sal_snprintf(sig_name, RHNAME_MAX_SIZE - 1, "ETPS.ETPS_%d.ENCAP_DESTINATION", ac_etps_id);
    SHR_IF_ERR_EXIT(dpp_dsig_read(unit, core_id, "ETPP", "Term", "FWD", sig_name, &lif_outer_vid, 1));

    /** ENCAP_DESTINATION may include 1/2/3 VIDs depending on its size*/
    lif_inner_vid = BCM_VLAN_NONE;
    if (lif_outer_vid > BCM_VLAN_MAX)
    {
        lif_inner_vid = lif_outer_vid >> 12;
        lif_outer_vid = lif_outer_vid & BCM_VPN_ID_MASK;
    }

    if (lif_inner_vid > BCM_VLAN_MAX)
    {
        lif_inner_vid = lif_inner_vid & BCM_VPN_ID_MASK;
    }
    ...
    {
        sal_snprintf(sig_name, RHNAME_MAX_SIZE - 1, "ETPS.ETPS_%d.FORWARD_DOMAIN", etps_index);
    ...
    /** since we're dealing with DBAL ENUMs, which return the logical value, we first transfer to hw value */
    dbal_fields_enum_hw_value_get(unit, DBAL_FIELD_EVE_OUTER_VID_SRC, vid_src_out, &vid_src_out);
    dnx_dbal_fields_string_form_hw_value_get(unit, (char *) dbal_field_to_string(unit, DBAL_FIELD_EVE_OUTER_VID_SRC),
                                             vid_src_out, vid_parse_out);

    dbal_fields_enum_hw_value_get(unit, DBAL_FIELD_EVE_OUTER_PCP_DEI_SRC, pcp_dei_src_out, &pcp_dei_src_out);
    dnx_dbal_fields_string_form_hw_value_get(unit,
                                             (char *) dbal_field_to_string(unit, DBAL_FIELD_EVE_OUTER_PCP_DEI_SRC),
                                             pcp_dei_src_out, pcp_dei_parse_out);

    dbal_fields_enum_hw_value_get(unit, DBAL_FIELD_EVE_INNER_VID_SRC, vid_src_in, &vid_src_in);
    dnx_dbal_fields_string_form_hw_value_get(unit, (char *) dbal_field_to_string(unit, DBAL_FIELD_EVE_INNER_VID_SRC),
                                             vid_src_in, vid_parse_in);

    dbal_fields_enum_hw_value_get(unit, DBAL_FIELD_EVE_INNER_PCP_DEI_SRC, pcp_dei_src_in, &pcp_dei_src_in);
    dnx_dbal_fields_string_form_hw_value_get(unit,
                                             (char *) dbal_field_to_string(unit, DBAL_FIELD_EVE_INNER_PCP_DEI_SRC),
                                             pcp_dei_src_in, pcp_dei_parse_in);
    ...
    if (!is_encap_stage)
    {
        __PRT_TABLE_ENTRY_HEX(0, "LLVP_Profile", llvp_profile, PRT_ROW_SEP_NONE);
    }

    __PRT_TABLE_ENTRY_HEX(1, "Cmd_Index", (is_encap_stage ? vlan_edit_profile : eve_vlan_edit_cmd),
                          PRT_ROW_SEP_UNDERSCORE_BEFORE);
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SH_DNX_CORES_ITER(unit, core_in, core_id)
    {
        ...
        for (etps_index = 1; etps_index < 9; etps_index++)
        {
            ...
            sal_snprintf(sig_name, RHNAME_MAX_SIZE - 1, "ETPS.ETPS_%d.EGRESS_VLAN_EDIT_PROFILE", etps_index);
