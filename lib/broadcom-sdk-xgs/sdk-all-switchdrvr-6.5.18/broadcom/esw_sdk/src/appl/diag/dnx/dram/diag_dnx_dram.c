...
typedef enum dnx_dram_hbm_bist_data_mode_e
{
    HBM_BIST_DATA_MODE_PRBS = 0,
    HBM_BIST_DATA_MODE_PATTERN = 1,
} dnx_dram_hbm_bist_data_mode_t;
    ...
    sh_sand_control_t * sand_control)
{
    ...
    {
        LOG_CLI((BSL_META("Cannot run DRAM PHY RESET on simulated system(pcid, cmodel etc') or no hbm-supported\n")));
    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    switch (type)
    {
        case PREDEFINED_BIST_IO_STRESS:
        {
            int seed = 0;
            uint32 dram_index;
            bist_info.data_mode = HBM_BIST_DATA_MODE_PRBS;
        ...
        case PREDEFINED_BIST_ALL_ADDRESS:
        {
            ...
            for (dram_index = dram_start; dram_index <= dram_end; ++dram_index)
            {
                ...
                {
                    int pattern_index = 0;
                    bist_info.data_mode = HBM_BIST_DATA_MODE_PATTERN;
    ...
    sh_sand_control_t * sand_control)
{
    ...
    {
        int rv;
        int temp_read_is_valid = 0;
        bcm_switch_thermo_sensor_t sensor_data;
        PRT_ROW_ADD(PRT_ROW_SEP_NONE);

        /** Read Temperature */
        rv = bcm_dnx_switch_thermo_sensor_read(unit, bcmSwitchThermoSensorDram, dram_index, &sensor_data);
        if (rv == _SHR_E_NONE)
        {
            temp_read_is_valid = 1;
        }

        /** Print info */
        PRT_CELL_SET("0x%x", dram_index);
        if (temp_read_is_valid)
        {
...
static sh_sand_enum_t sand_diag_hbm_data_mode_enum[] = {
    /**String    Value                      Description */
    {"prbs",     HBM_BIST_DATA_MODE_PRBS,       "PRBS data mode, the data is generated according to the PRBS seed"},
    {"pattern",  HBM_BIST_DATA_MODE_PATTERN,    "User defined pattern, should be inserted in relevant fields"},
    {NULL}
};
...
static sh_sand_option_t dnx_bist_start_options_list_hbm[] = {
    ...
    {"mode",                SAL_FIELD_TYPE_ENUM,    "data mode, possible values are prbs and pattern",                                  "prbs",                 (void*)sand_diag_hbm_data_mode_enum},
...
#if 1
    .examples = "dram=0 channel=4 count=0x1000 mode=prbs prbs_data_seed=0x3451231231234353245432523412345432598098230495893427589798798732290754123421232545789135548903af4554bd48574747578487438412345678 \n" \
...
static sh_sand_man_t sh_dnx_dram_phy_reg_read_man = {
    ...
    .examples = "dram=1 channel=3 address=0x16 \n" \
...
static sh_sand_man_t sh_dnx_dram_phy_reg_write_man = {
    ...
    .full = "access to the dram phy registers is done through this command.\n"
            "it is required to provide the address of the phy register that is wanted and value is attempting to write it"
            "access is possible to the different channels and to midstack as well, when accessing midstack, channel is not relevant\n",
    .examples = "dram=1 channel=3 address=0x16 value=0x3 \n" \
...
static sh_sand_cmd_t sh_dnx_dram_bist_cmds[] = {
    ...
    {"start",  sh_dnx_dram_hbm_bist_start_cmd,    NULL,  dnx_bist_start_options_list_hbm,   &sh_dnx_dram_bist_start_man, NULL, NULL, SH_CMD_CONDITIONAL, sh_cmd_is_hbm_available},
    {"start",  sh_dnx_dram_gddr6_bist_start_cmd,  NULL,  dnx_bist_start_options_list_gddr6, &sh_dnx_dram_bist_start_man, NULL, NULL, SH_CMD_CONDITIONAL, sh_cmd_is_gddr6_available},
...
static sh_sand_cmd_t sh_dnx_dram_phy_register_cmds[] = {
    /* Name     | Leaf Action          | Junction Array Pointer   | Options for Leaf                   | Usage */
    {"read",    sh_dnx_dram_phy_reg_read_cmd,   NULL,            dnx_phy_reg_read_options_list,        &sh_dnx_dram_phy_reg_read_man,        NULL, NULL, SH_CMD_CONDITIONAL, sh_cmd_is_hbm_available},
    ...
    {"write",   sh_dnx_dram_phy_reg_write_cmd,  NULL,            dnx_phy_reg_write_options_list,       &sh_dnx_dram_phy_reg_write_man,       NULL, NULL, SH_CMD_CONDITIONAL, sh_cmd_is_hbm_available},
...
static sh_sand_cmd_t sh_dnx_dram_phy_cmds[] = {
    /* Name           | Leaf Action               | Junction Array Pointer       | Options for Leaf          | Usage */
    {"tune",      sh_dnx_dram_phy_tune,   NULL,                          dnx_phy_tune_options_list_gddr6, &sh_dnx_dram_phy_tune_man, NULL, NULL, SH_CMD_CONDITIONAL, sh_cmd_is_gddr6_available},
    {"tune",      sh_dnx_dram_phy_tune,   NULL,                          dnx_phy_tune_options_list_hbm,   &sh_dnx_dram_phy_tune_man, NULL, NULL, SH_CMD_CONDITIONAL, sh_cmd_is_hbm_available},
    {"register",  NULL,                   sh_dnx_dram_phy_register_cmds, NULL,                            &sh_dnx_dram_phy_register_man},
    {"reset",     sh_dnx_dram_phy_reset,  NULL,                          dnx_phy_reset_options_list,      &sh_dnx_dram_phy_reset_man, NULL, NULL, SH_CMD_CONDITIONAL, sh_cmd_is_hbm_available},
    {NULL}
};
...
static sh_sand_cmd_t sh_dnx_dram_debug_cmds[] = {
    ...
    {"EyeScan",             sh_dnx_dram_debug_eye_scan_cmd,             NULL,               dnx_debug_eye_scan_option_list,         &sh_dnx_dram_debug_eye_scan_man,        NULL, NULL, SH_CMD_CONDITIONAL,                         sh_cmd_is_hbm_available},
    {"EyeScan",             sh_dnx_dram_debug_eye_scan_cmd,             NULL,               dnx_debug_eye_scan_option_list_gddr6,   &sh_dnx_dram_debug_eye_scan_man,        NULL, NULL, SH_CMD_CONDITIONAL,                         sh_cmd_is_gddr6_available},
    ...
    {"bist",                sh_dnx_dram_debug_hbm_bist_cmd,             NULL,               dnx_debug_bist_option_list,             &sh_dnx_dram_debug_bist_man,            NULL, NULL, SH_CMD_CONDITIONAL,                         sh_cmd_is_hbm_available},
    {"bist",                sh_dnx_dram_debug_gddr6_bist_cmd,           NULL,               dnx_debug_bist_option_list,             &sh_dnx_dram_debug_bist_man,            NULL, NULL, SH_CMD_CONDITIONAL,                         sh_cmd_is_gddr6_available},
    {"Temperature",         sh_dnx_dram_debug_temp_cmd,                 NULL,               dnx_debug_temp_option_list,             &sh_dnx_dram_debug_temp_man,            NULL, NULL, SH_CMD_NO_XML_VERIFY|SH_CMD_CONDITIONAL},
