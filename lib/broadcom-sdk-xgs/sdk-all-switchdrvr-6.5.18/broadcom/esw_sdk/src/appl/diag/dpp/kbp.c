...
extern int flexible_mode_ref_app(int unit, bcm_switch_concat_header_t concat_header, uint8 verbosity_level);
...
cmd_dpp_flexible_mode_ref_app(int unit, args_t *a)
{
    ...
    if (ARG_CNT(a) > 0) {
        ...
        if (parse_arg_eq(a, &pt) < 0) {
            ...
            cli_out(
                ...
                "\n\t\t[verbosity]        - Decided to concat extra packet header to master key or not "
    ...
    soc_sand_rv = flexible_mode_ref_app(unit, concat_header, verbosity_level);
