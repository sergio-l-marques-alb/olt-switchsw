            ...
            if (!skip_print)
            {
                for (ctx_id_index = ctx_id_lower; ctx_id_index <= ctx_id_upper; ctx_id_index++)
                {
                    ...
                    SHR_IF_ERR_EXIT(dnx_algo_field_context_id_is_allocated
                                    (unit, field_stage, ctx_id_index, &is_allocated));
                    if (!is_allocated)
                    {
                        continue;
                    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SHR_IF_ERR_EXIT(diag_dnx_field_utils_groups_per_context_get
                    (unit, context_id, field_stage, &nof_fgs_per_ctx, fgs_per_context));
             ...
             DNX_FIELD_QUAL_TYPE_INVALID; qual_iter++)
        {
                ...
                DNX_FIELD_FFC_ID_INVALID)
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "For opcode_id %d, fg_id %d, qualifier 0x%x, no FFC found.\n",
                             opcode_id, fgs_per_context[fg_id_index],
                             attach_full_info->group_ffc_info.key.qualifier_ffc_info[qual_iter].qual_type);
            }
            ...
            SHR_IF_ERR_EXIT(dnx_field_key_fwd2_to_acl_key_id_convert
                            (unit,
                             attach_full_info->group_ffc_info.key.qualifier_ffc_info[qual_iter].ffc_info[0].ffc.key_id,
                             &kbp_kbr_id));
            offset_in_kbp_kbr =
