/*
 * $Id: diag_alloc.c,v 1.8 Broadcom SDK $
...
cmd_result_t cmd_diag_alloc_eedb_banks(int unit)
{
    ...
    for (bank_id_iter = 0 ; bank_id_iter < _BCM_DPP_AM_EGRESS_LIF_NOF_EEDB_HALF_BANKS(unit) ; bank_id_iter++) 
    {
        ...
        cli_out(" Phase= %d , is extended = %d, ext_type=%d Type=%d  \n\r", eedb_bank_info.phase, eedb_bank_info.is_extended , eedb_bank_info.ext_type, eedb_bank_info.type  );
...
cmd_dpp_diag_tmplt(int unit, args_t* a)
{
    ...
    if (function) {
        if(!sal_strncasecmp(function, "all", strlen(function))) {
            val = -1;
        } else {
        if (!sal_strncasecmp(function, "type", strlen(function))) {
            function = ARG_GET(a);
            if (function) {
