 ...
 *   \param [out] layer_type - layer type.
 *   \param [out] layer_offset - layer offset inpacket header.
 *   \param [out] nof_vlan_tags - vlan tags number.
 * \par DIRECT OUTPUT:
 *   shr_error_e
 */
static shr_error_e
sh_dnx_pp_cos_ingress_layer_info_get(
    int unit,
    int core,
    uint32 layer_index,
    char *layer_protocol,
    ...
    uint32 *qos_var_flags)
{
    ...
    if (qos_layer_index != layer_index)
    {
        /**layer protocol is current qos profile layer, others used for qos key which comes from next layer*/
        SHR_IF_ERR_EXIT(sh_dnx_pp_cos_ingress_layer_info_get
                        (unit, core, layer_index, layer_protocol, NULL, NULL, NULL));
        SHR_IF_ERR_EXIT(sh_dnx_pp_cos_ingress_layer_info_get
                        (unit, core, qos_layer_index, NULL, &layer_header_type, &layer_offset, &nof_vlan_tags));
    }
    else
    {
        SHR_IF_ERR_EXIT(sh_dnx_pp_cos_ingress_layer_info_get
                        (unit, core, layer_index, layer_protocol, &layer_header_type, &layer_offset, &nof_vlan_tags));
    }
    ...
    uint32 *final_qos_value)
{
     ...
     */
    SHR_IF_ERR_EXIT(dpp_dsig_read(unit, core, "IRPP", "VTT5", "FWD1", "Fwd_Layer_Index", fwd_layer_index, 1));

    /** translate the layer index to layer protocol*/
    SHR_IF_ERR_EXIT(sh_dnx_pp_cos_ingress_layer_info_get
                    (unit, core, *fwd_layer_index, fwd_layer_protocol, NULL, NULL, NULL));

    /*
     * 2. get the qos_layer_index and its parsing
     */
    SHR_IF_ERR_EXIT(dpp_dsig_read(unit, core, "IRPP", "VTT5", "FWD1", s_qos_layer_index, qos_layer_index, 1));
    SHR_IF_ERR_EXIT(dpp_dsig_read(unit, core, "IRPP", "VTT5", "FWD1", s_qos_layer_strength, &qos_strength, 1));
    if (qos_strength == 0)
    {
     ...
     */
    SHR_IF_ERR_EXIT(dpp_dsig_read(unit, core, "IRPP", "VTT5", "FWD1", s_qos_mapping_profile, qos_profile, 1));
     ...
     */
    SHR_IF_ERR_EXIT(sh_dnx_pp_cos_ingress_src_qos_info_get(unit, core, *qos_layer_index, *cs_profile,
     ...
     */
    SHR_IF_ERR_EXIT(bcm_qos_map_id_get_by_profile
                    (unit, (flags | BCM_QOS_MAP_INGRESS), (int) *qos_profile, &encoded_profile));

    if (!(((qos_var_flags & BCM_QOS_MAP_L2) && (flags & BCM_QOS_MAP_L2_UNTAGGED)) ||
          (qos_var_flags & BCM_QOS_MAP_PORT)))
    {
        flags |= BCM_QOS_MAP_OPCODE;
        flags |= qos_var_flags;
        bcm_qos_map_t_init(&qos_map_array);
        array_count = 0;
        SHR_IF_ERR_EXIT(bcm_qos_map_multi_get(unit, flags, encoded_profile, 1, &qos_map_array, &array_count));
        if (array_count == 0)
        {
            SHR_CLI_EXIT(_SHR_E_INTERNAL, "get opcode failed per profile(0x%08X) flags(0x%08X)\n",
                         encoded_profile, flags);
        }
        *opcode = DNX_QOS_MAP_PROFILE_GET(qos_map_array.opcode);
    }

    /*
     * 8. get mapped qos value and its parsing.
     */
    SHR_IF_ERR_EXIT(dpp_dsig_read(unit, core, "IRPP", "FWD1", "FWD2", s_mapped_qos_value_1, &(mapped_qos_value[0]), 1));

    if (!ISEMPTY(s_mapped_qos_value_2))
    {
        SHR_IF_ERR_EXIT(dpp_dsig_read
                        (unit, core, "IRPP", "FWD1", "FWD2", s_mapped_qos_value_2, &(mapped_qos_value[1]), 1));
    }

    /*
     * 9. get the qos value to ITM and its parsing.
     */
    SHR_IF_ERR_EXIT(dpp_dsig_read(unit, core, "IRPP", "IPMF3", "LBP", s_mapped_qos_value_1, &(mapped_qos_value[0]), 1));

    if (!ISEMPTY(s_mapped_qos_value_2))
    {
        SHR_IF_ERR_EXIT(dpp_dsig_read
                        (unit, core, "IRPP", "IPMF3", "LBP", s_mapped_qos_value_2, &(mapped_qos_value[1]), 1));
    }
    ...
    uint32 *comp_strength)
{
     ...
     */
    SHR_IF_ERR_EXIT(sh_dnx_pp_cos_ingress_layer_info_get
                    (unit, core, *qos_layer_index, qos_layer_protocol, NULL, NULL, NULL));
    ...
    if (sal_strcasecmp(stage_name, "LLR"))
    {
        ...
        *propag_profile = 0;
        ...
        else
        {
            ...
            {
                local_in_lif_core = local_lif_info.core_id;
    ...
    uint32 *updated_qos_dp)
{
    ...
    uint32 in_dp, in_nwk_qos, nwk_qos_idx, ecn_enable = 0;
    uint8 use_etps = 0, nwk_qos_preserve = 0;
    uint8 fwd_code_remark_or_keep, fwd_code_ecn_is_eligible, fwd_code_ecn_keep, remark_profile_remark_or_keep;
    ...
    if ((stage == EKLEAP_STAGE_PRP2) || (stage == EKLEAP_STAGE_TERM))
    {
        /*
         * skip 1,2,3,4,5,6 
         * 7. get nwk_qos signal and its parsing 
         */
        SHR_IF_ERR_EXIT(dpp_dsig_read(unit, core, "ETPP", cur_stage_name, next_stage_name, "Nwk_QOS", nwk_qos, 1));
        ...
        {
            SHR_IF_ERR_EXIT(dpp_dsig_read(unit, core, "ETPP", NULL, "PRP2", "ftmh_dp", &in_dp, 1));
        }
        else
        {
            SHR_IF_ERR_EXIT(dpp_dsig_read(unit, core, "ETPP", NULL, "term", "ftmh_dp", &in_dp, 1));
        }

        *updated_qos_dp = (in_dp << DNX_QOS_DP_OFFSET) | *nwk_qos;
    }
    else if (shared_data->shifted_etps < shared_data->etps_data.entries_nof)
    {
        ...
        if (stage == EKLEAP_STAGE_FWD)
        {
             ...
             */
            /** get ftmh_dp and its parsing*/
            SHR_IF_ERR_EXIT(dpp_dsig_read(unit, core, "ETPP", pre_stage_name, cur_stage_name, "ftmh_dp", &in_dp, 1));

            /** get network_qos and its parsing*/
            SHR_IF_ERR_EXIT(dpp_dsig_read
                            (unit, core, "ETPP", pre_stage_name, cur_stage_name, "nwk_qos", &in_nwk_qos, 1));

            *in_qos_dp = (in_dp << DNX_QOS_DP_OFFSET) | in_nwk_qos;

            /*
             * 2. get qos_var_type
             */
            /** get forward_code and its parsing*/
            SHR_IF_ERR_EXIT(dpp_dsig_read(unit, core, "ETPP", "PRP2", "TERM", "fwd_code", &fwd_code, 1));

            /** get ftmh_ecn_enable and its parsing*/
            SHR_IF_ERR_EXIT(dpp_dsig_read(unit, core, "ETPP", "ETParser", "PRP2", "ftmh_ecn_enable", &ecn_enable, 1));
            ...
            SHR_IF_ERR_EXIT(dpp_dsig_read(unit, core, "ETPP", pre_stage_name, cur_stage_name, "context", &context, 1));
            ...
            dbal_entry_key_field32_set(unit, entry_handle_id, stage_info.ctx_field_id, context);
            SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_GET_ALL_FIELDS));
            SHR_IF_ERR_EXIT(dbal_entry_handle_value_field8_get
            ...
            if (use_etps)
            {
                ...
                list_element = utilex_rhlist_entry_get_by_name(etps_1_list_element->field_list, "REMARK_PROFILE");
            }
            ...
            {
                _GET_ETPS_ENTRY_BY_ID(core, shared_data, shared_data->shifted_etps, etps_1_list_element);
                list_element = utilex_rhlist_entry_get_by_name(etps_1_list_element->field_list, "FORWARD_QOS_MODEL");
            }
            if (list_element != NULL)
            {
                *qos_model = list_element->value[0];
            }
            else
            {
                *qos_model = 0;
            ...
            {
                _GET_ETPS_ENTRY_BY_ID(core, shared_data, shared_data->shifted_etps, etps_1_list_element);
                list_element = utilex_rhlist_entry_get_by_name(etps_1_list_element->field_list, "NWK_QOS_IDX");
            }

            if (list_element != NULL)
            {
                nwk_qos_idx = list_element->value[0];
            }
            else
            {
                nwk_qos_idx = 0;
        ...
        else
        {
            ...
            /** get network_qos and its parsing*/
            SHR_IF_ERR_EXIT(dpp_dsig_read
                            (unit, core, "ETPP", pre_stage_name, cur_stage_name, "Updated_QOS_and_DP", in_qos_dp, 1));
            ...
            /** get qos_var_type*/
            SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                            (unit, entry_handle_id, DBAL_FIELD_QOS_VAR_TYPE, INST_SINGLE, qos_var_type));
            SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
             ...
             */
            *qos_profile = 0;
            _GET_ETPS_ENTRY_BY_ID(core, shared_data, shared_data->shifted_etps + remark_profile_index, list_element);
            list_element = utilex_rhlist_entry_get_by_name(list_element->field_list, "REMARK_PROFILE");
            if (list_element != NULL)
            {
                *qos_profile = list_element->value[0];
            }
            ...
            *qos_model = 0;
            _GET_ETPS_ENTRY_BY_ID(core, shared_data, shared_data->shifted_etps, etps_1_list_element);
            list_element = utilex_rhlist_entry_get_by_name(etps_1_list_element->field_list, "ENCAP_QOS_MODEL");
            ...
            if (list_element == NULL)
            {
                ...
                {
                    list_element = utilex_rhlist_entry_get_by_name(etps_2_list_element->field_list, "ENCAP_QOS_MODEL");
                }
            }
            if (list_element != NULL)
            {
            ...
            if (list_element == NULL)
            {
                ...
                if (etps_2_list_element != NULL)
                {
                    list_element = utilex_rhlist_entry_get_by_name(etps_2_list_element->field_list, "NWK_QOS_IDX");
                }
            }
            if (list_element != NULL)
            {
        ...
        *remark_or_keep = 1;
        if (stage == EKLEAP_STAGE_FWD)
        {
            if (dnx_data_qos.hw_bug.feature_get(unit, dnx_data_qos_hw_bug_egress_remark_or_keep_enabler))
            {
        ...
        {
            in_dp = (*in_qos_dp >> DNX_QOS_DP_OFFSET) & DNX_QOS_DP_MASK;
            ...
            dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_DP, in_dp);
        ...
        {
            SHR_IF_ERR_EXIT(dpp_dsig_read(unit, core, "ETPP", cur_stage_name, next_stage_name,
                                          "Updated_QOS_and_DP", updated_qos_dp, 1));
        }
