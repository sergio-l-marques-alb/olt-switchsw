 ...
 */
#define KBP_MASTER_KEY_SEGMENT_SET(__unit, __byte_offset, __segment_bytes, __field_type, __sand_arg)             \
do                                                                                                              \
{                                                                                                               \
    if (field_type == DBAL_FIELD_TYPE_DEF_L2_MAC)                                                               \
    {                                                                                                           \
        sal_memcpy((void*)&master_key[byte_offset],(uint32 *)&SH_SAND_ARG_MAC_DATA(sand_arg),segment_bytes);    \
    }                                                                                                           \
    else if ((field_type == DBAL_FIELD_TYPE_DEF_IPV6) |                                                         \
             (field_type == DBAL_FIELD_TYPE_DEF_IPV6_GROUP))                                                    \
    {                                                                                                           \
        sal_memcpy((void*)&master_key[byte_offset],(uint8 *)&SH_SAND_ARG_IP6_DATA(sand_arg),segment_bytes);     \
    }                                                                                                           \
    else                                                                                                        \
    {                                                                                                           \
        SHR_IF_ERR_EXIT(dnx_kbp_buffer_dbal_to_kbp(unit, (segment_bytes<<3),SH_SAND_ARG_ARRAY_DATA(sand_arg),   \
                        (uint8 *)&master_key[byte_offset]));                                                    \
    }                                                                                                           \
}while(0)
...
#define KBP_COMPATIBILITY_MESSAGE "Command is available only when KBP support is enabled"
    ...
    sh_sand_control_t * sand_control)
{
    ...
    DNX_KBP_TRY(kbp_device_dump(Kbp_app_data[unit]->device_p, info, kbp_file));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    DNX_KBP_TRY(kbp_device_read_last_result(Kbp_app_data[unit]->device_p, info, kbp_file));
        ...
        while (!context_is_end)
        {
            ...
            if (opcode_id == mapped_opcode_id)
            {
                ...
                SHR_IF_ERR_EXIT(dbal_entry_handle_key_field_arr32_get
                                (unit, ctx_entry_handle_id, DBAL_FIELD_FWD2_CONTEXT_ID, &context_id));
                SHR_IF_ERR_EXIT(dbal_fields_enum_hw_value_get
                                (unit, DBAL_FIELD_FWD2_CONTEXT_ID, context_id, &context_id));
                SHR_IF_ERR_EXIT(dnx_dbal_fields_string_form_hw_value_get
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SH_SAND_GET_ITERATOR(sand_arg)
    {
        ...
        for (index = 0; index < nof_master_key_segments; index++)
        {
            ...
            {
                KBP_MASTER_KEY_SEGMENT_SET(unit, byte_offset, segment_bytes, field_type, sand_arg);
            }
            else
            {
