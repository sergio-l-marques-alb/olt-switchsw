...
static sh_sand_man_t sh_dnx_tm_queue_mapping_man = {
    .brief = "Queue Mapping",
    .full = "Dump mapping between system-port, base queue and modport",
    .synopsis = NULL,
    .examples = NULL
};
    ...
    sh_sand_control_t * sand_control)
{
    ...
    mapping_table =
        sal_alloc(sizeof(sh_dnx_tm_queue_mapping_t) * dnx_data_device.general.max_nof_system_ports_get(unit),
                  "sh_dnx_tm_queue_mapping_cmd.sh_dnx_tm_queue_mapping_t");
    if (mapping_table == NULL)
    {
        SHR_CLI_EXIT(_SHR_E_INTERNAL, "Memory allocation failure");
    }
    ...
    for (sysport = 0; sysport < dnx_data_device.general.max_nof_system_ports_get(unit); ++sysport)
    {
         ...
         */
        SHR_IF_ERR_CONT(dnx_cosq_ipq_dbal_system_port_to_voq_base_get(unit, sysport, &base_queue, &is_valid));
        /*
        ...
        SHR_IF_ERR_EXIT(dnx_cosq_ipq_dbal_queue_quartet_to_sys_port_map_get(unit, base_queue_quartet, &master_sysport));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SHR_IF_ERR_EXIT(dnx_ingress_congestion_dbal_voq_rate_class_get(unit, queue, &rate_class));
    SHR_IF_ERR_EXIT(dnx_ingress_congestion_dbal_voq_traffic_class_get(unit, queue, &traffic_class));
    SHR_IF_ERR_EXIT(dnx_ingress_congestion_dbal_voq_connection_class_get(unit, queue, &connection_class));

    PRT_TITLE_SET("Queue %d Mapping info", queue);
    PRT_COLUMN_ADD("Type");
    PRT_COLUMN_ADD("ID");
    ...
    sh_sand_control_t * sand_control)
{
    ...
    int base_queue, queue, core;

    PRT_INIT_VARS;
    SHR_FUNC_INIT_VARS(unit);

    /** Get queue id (For printing purposes only) */
    DIAG_DNX_QUEUE_ID_GET(voq_gport, cos, queue);
    /**
    ...
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core)
    {
        ...
        /**
         * Remap the core (BCM_CORE_ALL by default)in order to get information for specific queue
         */
        if (BCM_GPORT_IS_MCAST_QUEUE_GROUP(voq_gport))
        {
            base_queue = BCM_GPORT_MCAST_QUEUE_GROUP_QUEUE_GET(voq_gport);
            BCM_GPORT_MCAST_QUEUE_GROUP_CORE_QUEUE_SET(voq_gport, core, base_queue);
        }
        else
        {
            base_queue = BCM_GPORT_UNICAST_QUEUE_GROUP_QID_GET(voq_gport);
            BCM_GPORT_UNICAST_QUEUE_GROUP_CORE_QUEUE_SET(voq_gport, core, base_queue);
        }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_ingress_congestion_voq_occupancy_info_get(unit, core, queue, &queue_info));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    /** print profile */
    PRT_TITLE_SET("Queue %d --> credit request profile %d", queue + cos, credit_request_profile_id);
    PRT_COLUMN_ADD("Name");
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SHR_IF_ERR_EXIT(dnx_cosq_ingress_compensation_voq_compesation_profile_hw_get(unit, base_voq + cos, &comp_profile));

    /** print profile */
    PRT_TITLE_SET("Queue %d --> compensation profile %d", base_voq + cos, comp_profile);
    PRT_COLUMN_ADD("Name");
    PRT_COLUMN_ADD("Value");
    ...
    *num_cos = -1;
    ...
    for (index = 0; index < DIAG_DNX_QUEUE_NOF_VALID_BUNDLE_SIZES; index++)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_ipq_db.base_queues.
                        num_cos.get(unit, base_queue_temp / dnx_data_ipq.queues.min_bundle_size_get(unit),
                                    &sw_state_num_cos));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    /** Get Base queue*/
    SHR_IF_ERR_EXIT(sh_dnx_tm_queue_base_queue_get(unit, queue, &base_queue, &num_cos));
    ...
    {
        /** Multicast */
        BCM_GPORT_MCAST_QUEUE_GROUP_CORE_QUEUE_SET(voq_gport, BCM_CORE_ALL, base_queue);
    }
    else if (queue >= 0 && queue < dnx_data_ipq.queues.nof_queues_get(unit))
    {
        /** Unicast */
        BCM_GPORT_UNICAST_QUEUE_GROUP_CORE_QUEUE_SET(voq_gport, BCM_CORE_ALL, base_queue);
    }
    else
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Invalid queue %d provided", queue);
    ...
    SHR_IF_ERR_EXIT(sh_dnx_tm_queue_mapping_info_cmd(unit, queue, sand_control));
    ...
    SHR_IF_ERR_EXIT(sh_dnx_tm_queue_compensation_cmd(unit, base_queue, cos, sand_control));
    ...
    SHR_IF_ERR_EXIT(sh_dnx_tm_queue_state_info_cmd(unit, voq_gport, cos, sand_control));

    /** print queue size information */
    SHR_IF_ERR_EXIT(sh_dnx_tm_queue_size_info_cmd(unit, queue, sand_control));
