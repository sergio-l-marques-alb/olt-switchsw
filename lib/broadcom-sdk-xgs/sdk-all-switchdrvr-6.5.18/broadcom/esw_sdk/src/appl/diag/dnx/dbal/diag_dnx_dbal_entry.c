...
#define DBAL_ENTRY_COMMIT_VALUE_FIELD(unit, entry_handle_id, field_id, res_type, sand_arg)    \
do                                                                                  \
{                                                                                   \
    dbal_field_types_defs_e field_type;                                             \
    SHR_IF_ERR_EXIT(dbal_fields_field_type_get(unit, field_id, &field_type));       \
    ...
    char *cmd)
{
    ...
    SH_SAND_GET_ITERATOR(sand_arg)
    {
        ...
        else
        {
            ...
            DBAL_ENTRY_COMMIT_VALUE_FIELD(unit, entry_handle_id, field_id, res_type, sand_arg);
            value_field_index++;
        }
    }
    ...
    uint32 entry_handle_id)
{
    ...
    if ((table->access_method == DBAL_ACCESS_METHOD_MDB) &&
        (!dnx_data_l2.hw_bug.feature_get(unit, dnx_data_l2_hw_bug_age_state_not_updated)) &&
    ...
    sh_sand_control_t * sand_control)
{
    ...
    else
    {
        ...
        if (table->access_method == DBAL_ACCESS_METHOD_MDB)
        {
            ...
            if (!dnx_data_l2.hw_bug.feature_get(unit, dnx_data_l2_hw_bug_age_state_not_updated))
            {
