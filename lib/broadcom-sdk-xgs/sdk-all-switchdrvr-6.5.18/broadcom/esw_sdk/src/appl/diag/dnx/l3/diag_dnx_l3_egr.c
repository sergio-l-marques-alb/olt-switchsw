    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (; start_id < end_id; start_id++)
    {
        ...
        nof_macro_a_type = 0;
        nof_macro_b_type = 0;
        SHR_IF_ERR_EXIT(algo_l3_fec_mdb_table_resources_get(unit, start_id, &nof_clusters, clusters));
        for (idx = 0; idx < nof_clusters; idx += 2)
        {
             ...
             */
            for (cl_id = 0; cl_id < DIAG_EGR_FEC_CLUSTERS_WIDTH; cl_id++)
            {
                uint32 temp_hierarchy;
                bank_id =
                    DNX_ALGO_L3_PHYSICAL_FEC_GET(unit,
                                                 clusters[idx +
                                                          cl_id].logical_start_address * DIAG_EGR_FEC_CLUSTERS_WIDTH) /
                    dnx_data_l3.fec.nof_physical_fecs_per_bank_get(unit) + cl_id;

                SHR_IF_ERR_EXIT(mdb_db_infos.fec_hierarchy_info.fec_hierarchy_map.get(unit, bank_id, &temp_hierarchy));

                /** If the sub resource is not taken by this hierarchy it means that the FEC cluster is free */
                if (temp_hierarchy == start_id)
                {
                    addr_id = 1;
                }
            }
        ...
        if (advanced_mode == TRUE)
        {
            PRT_CELL_SET("%d", hier_id[start_id]);
