    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (ctx_iter = 0; ctx_iter < DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_CONTEXTS; ctx_iter++)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_algo_field_context_id_is_allocated(unit, field_stage, ctx_iter, &is_cs_allocated));
        if (!is_cs_allocated)
        {
            continue;
        }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    else
    {
        if (field_group != DNX_FIELD_GROUP_INVALID)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_algo_field_context_id_is_allocated(unit, fg_stage, field_context, &is_ctx_allocated));
            if (!is_ctx_allocated)
            {
    ...
    for (; fg_iter <= max_nof_fgs; fg_iter++)
    {
        ...
        {
            SHR_IF_ERR_EXIT(dnx_algo_field_context_id_is_allocated(unit, fg_stage, ctx_iter, &is_ctx_allocated));
            if (!is_ctx_allocated)
            {
                continue;
            }
                 ...
                 DNX_FIELD_EFES_ID_INVALID; efes_action_iter++)
            {
                ...
                if (efes_action_info.is_condition_valid == TRUE)
                {
                    ...
                    if (is_all_fes2msb_actions_same)
                    {
                        ...
                        PRT_CELL_SET("%d", efes_action_info.efes_condition_conf[0].action_or_mask);
                    }
                    else
                    {
                        for (fes2msb_index = 0; fes2msb_index < DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_PROG_PER_FES;
                             fes2msb_index++)
                        {
                            ...
                            else
                            {
                                ...
                                PRT_CELL_SET("%d",
                                             efes_action_info.efes_condition_conf[fes2msb_index].valid_bit_polarity);
                                PRT_CELL_SET("%d", efes_action_info.efes_condition_conf[fes2msb_index].action_or_mask);
                            }
                        }
                    }
