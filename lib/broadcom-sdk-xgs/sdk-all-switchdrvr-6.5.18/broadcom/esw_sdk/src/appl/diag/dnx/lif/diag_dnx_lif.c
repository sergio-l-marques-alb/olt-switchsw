    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (outlif_bank = 0; outlif_bank < nof_outlif_banks; outlif_bank++)
    {
        ...
        nof_eedb_banks_per_outlif_bank =
            DNX_ALGO_LOCAL_OUTLIF_BANK_SIZE(unit) / (dnx_data_mdb.dh.nof_rows_in_eedb_bank_get(unit) *
...
dnx_outlif_allocation_phase_cnt_status_cmd(
    ...
    sh_sand_control_t * sand_control)
{
    uint32 entry_handle_id, result_type, logical_phase, phase_idx, table_idx;
    dbal_tables_e table_id;
    uint32 max_nof_result_types = DIAG_LIF_MAX_NUM_RESULT_TYPE;
    int phase_iter, table_iter, res_type_iter, is_end, phase_id;
    char *table_name;
    ...
    while (!is_end)
    {
        dbal_entry_handle_value_field32_get(unit, entry_handle_id, DBAL_FIELD_DBAL_APP_DB_NAME, INST_SINGLE, &table_id);
        dbal_entry_handle_value_field32_get(unit, entry_handle_id, DBAL_FIELD_DBAL_RESULT_TYPE, INST_SINGLE,
                                            &result_type);
        dbal_entry_handle_value_field32_get(unit, entry_handle_id, DBAL_FIELD_LOGICAL_PHASE, INST_SINGLE,
                                            &logical_phase);
        if ((table_id > DBAL_NOF_TABLES) || (result_type > max_nof_result_types)
            || (logical_phase > DNX_ALGO_LOCAL_OUTLIF_LOGICAL_PHASE_COUNT))
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "Invalid value received");
        }

        counter_array[logical_phase][table_id][result_type]++;
        SHR_IF_ERR_EXIT(dbal_iterator_get_next(unit, entry_handle_id, &is_end));
    }
