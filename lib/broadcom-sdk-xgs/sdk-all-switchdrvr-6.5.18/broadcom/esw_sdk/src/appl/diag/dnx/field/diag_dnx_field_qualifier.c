...
static sh_sand_man_t field_map_qual_bcm_man = {
    ...
    .synopsis = "[class=<meta|layer|header>]\n",
    ...
    sh_sand_control_t * sand_control)
{
    char *match_n;
    dnx_field_stage_e stage;
    dnx_field_qual_class_e class, in_class;
    char *qual_desc;
    ...
    if (stage == DNX_FIELD_STAGE_ACE)
    {
        LOG_CLI_EX("\r\n" "Qualifier diagnostic doesn't support ACE stage!!%s%s%s%s\r\n\n", EMPTY, EMPTY, EMPTY, EMPTY);
        SHR_EXIT();
    }
    ...
    for (bcm_qual = 0; bcm_qual < bcmFieldQualifyCount; bcm_qual++)
    {
        ...
        if (SHR_FAILURE(dnx_field_map_qual_bcm_to_dnx_int(unit, stage, bcm_qual, &dnx_qual, &meta_qual_map)))
        {
            continue;
        }
        ...
        else
        {
            ...
            {
                if (SHR_FAILURE
                    (dnx_field_map_qual_bcm_to_dnx_int(unit, stage, bcm_qual_raw, &dnx_qual_raw, &meta_qual_map)))
                {
                    continue;
                }
