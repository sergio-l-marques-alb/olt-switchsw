    ...
    int *any_key_was_displayed_p)
{
         ...
         key_info_p->key_template.key_qual_map[qual_index].qual_type != DNX_FIELD_QUAL_TYPE_INVALID; qual_index++)
    {
         ...
         */
        for (temp_qual_size = key_info_p->key_template.key_qual_map[qual_index].size;
             temp_qual_size > 0; temp_qual_size -= max_num_bits_in_ffc, number_of_used_ffc_per_qual++);

        /** Iterate over maximum number of FFCs per qualifier.*/
        for (ffc_id_index = 0; ffc_id_index < number_of_used_ffc_per_qual; ffc_id_index++)
        {
