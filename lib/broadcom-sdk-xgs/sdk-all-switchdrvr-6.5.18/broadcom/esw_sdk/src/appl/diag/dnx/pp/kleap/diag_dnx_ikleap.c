...
/* *INDENT-ON* */
    ...
    for (ii = MAX_SUPPORTED_STRENGTH - 1; ii >= 0; ii--)
    {
        ...
        if ((sub_stage == 1)
            || (!dnx_data_debug.hw_bug.feature_get(unit, dnx_data_debug_hw_bug_no_hit_bit_on_mdb_access)))
        {
    ...
    uint32 *pd_mask)
{
    ...
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_DIRECT_EXTRACTION_PD_RESULT_TYPE, 0);
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (dnx_data_debug.hw_bug.feature_get(unit, dnx_data_debug_hw_bug_no_hit_bit_on_mdb_access))
    {
