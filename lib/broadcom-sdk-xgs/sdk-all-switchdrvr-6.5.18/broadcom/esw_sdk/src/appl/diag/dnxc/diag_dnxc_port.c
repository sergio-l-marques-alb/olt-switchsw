    ...
    int enable)
{
    ...
    /**
     * Due to inconsistency on simulation the resource_set call might fail so skip
     *  Due to JR2 B0, J2C and Q2A cmodel issue change is not supported on cmodel.
     */
    if (!SAL_BOOT_PLISIM)
    {
        /*
    ...
    sh_sand_control_t * sand_control)
{
    bcm_pbmp_t pbmp;
    bcm_port_t port;
    int is_state = 0, is_config = 0, is_state_eye = 0;
    char *option_str;
    int interval = 0;
    ...
    SH_SAND_GET_BOOL("state_eye", is_state_eye);
    ...
    sh_sand_control_t * sand_control)
{
    ...
    /**
     * Due to inconsistency on simulation the resource_set call inside of port_loopback_set sequence might fail so skip
     * Due to JR2 B0, J2C and Q2A cmodel issue change is not supported on cmodel.
     */
    if (!SAL_BOOT_PLISIM)
    {
...
static sh_sand_man_t dnxc_phy_raw_sbus_man = {
    ...
    .examples = "pm_id=0 devad=1 regad=0x96 lane=0x1 pindx=0x1 \n"
                ...
                "pm_id=1 devad=1 regad=0x96 lane=0x1 pindx=0x1 \n"
...
static sh_sand_option_t dnxc_phy_dsc_options[] = {
    ...
    {"state_eye", SAL_FIELD_TYPE_BOOL,  "Detailed DSC dump, including lane info, core info and lane event info and EYE Scan", "false", NULL},
