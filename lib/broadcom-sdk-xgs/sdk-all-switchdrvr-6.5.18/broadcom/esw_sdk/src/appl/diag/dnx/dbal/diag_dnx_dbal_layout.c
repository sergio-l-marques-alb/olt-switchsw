    ...
    char *str)
{
    ...
    if (encode_info->formula != NULL)
    {
         ...
         *  cFormula.  */
        if (encode_info->formula->formula_offset_cb != NULL)
        {
            sal_snprintf(str, CMD_MAX_STRING_LENGTH, "%s", "cFormula");
        }
        else
        {
            sal_snprintf(str, CMD_MAX_STRING_LENGTH, "%s", encode_info->formula->as_string);
        }
    }
}
        ...
        for (ii = 0; ii < DBAL_NOF_HL_ACCESS_TYPES; ii++)
        {
            ...
            for (jj = 0; jj < l2p_hl_info[ii].num_of_access_fields; jj++)
            {
                ...
                if ((l2p_hl_info[ii].l2p_fields_info[jj].data_offset_info.formula) &&
                    (l2p_hl_info[ii].l2p_fields_info[jj].data_offset_info.formula->nof_vars == 1) &&
                    (l2p_hl_info[ii].l2p_fields_info[jj].data_offset_info.formula->field_id == DBAL_FIELD_EMPTY))
                {
    ...
    soc_mem_t memory)
{
    ...
    if (dnx_data_ingress_cs.
        hw_workarounds.feature_get(unit, dnx_data_ingress_cs_hw_workarounds_disable_ecc_fix_en_before_read_write))
    {
    ...
    sh_sand_control_t * sand_control)
{
    ...
    PRT_COLUMN_ADD("Has result type");
    PRT_COLUMN_ADD("Rresult type mapped to sw");
    PRT_COLUMN_ADD("Range set");
    PRT_COLUMN_ADD("Priority attribute");
    PRT_COLUMN_ADD("Learning");
    PRT_COLUMN_ADD("Commit mode");
    PRT_COLUMN_ADD("Image specific");
    PRT_COLUMN_ADD("Table is Dirty");
    PRT_COLUMN_ADD("Iterator Optimized");
    ...
    PRT_CELL_SET("%s", (SHR_IS_BITSET(table->indications_bm, DBAL_TABLE_IND_HAS_RESULT_TYPE) ? "YES" : "NO"));
    ...
    PRT_CELL_SET("%s",
                 (SHR_IS_BITSET(table->indications_bm, DBAL_TABLE_IND_IS_NONE_DIRECT_OPTIMIZED) ? "OPT" : "STANDARD"));
    ...
    PRT_CELL_SET("%s", (SHR_IS_BITSET(table->indications_bm, DBAL_TABLE_IND_IS_ITERATOR_OPTIMIZED) ? "YES" : "NO"));
    PRT_COMMITX;
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (table->max_capacity != 0)
    {
        ...
        {
            LOG_CLI((BSL_META("\tEntries Status: Max Capacity: HW depended (see mapping), Committed %d \n"),
                     nof_entries));
        }
    }
    ...
    LOG_CLI((BSL_META("\tMaturity_level: %s \n"),
             (table->maturity_level != 2) ? "Not running in regression" : "Passed regression"));
    ...
    if (print_access)
    {
        ...
        else if (table->access_method == DBAL_ACCESS_METHOD_MDB)
        {
            ...
            {
                LOG_CLI((BSL_META("\tMDB Entry Type (ONE/HALF/QURTER/EIGHTH) per result type:\n")));
                for (ii = 0; ii < table->nof_result_types; ii++)
                {
        ...
        else if (table->access_method == DBAL_ACCESS_METHOD_KBP)
        {
            ...
            if (kbp_mngr_db_id_by_name_get(unit, table_string.table_name, &db_id) != _SHR_E_NONE)
            {
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (field_type_info->nof_enum_values > 0)
    {
        ...
        {
            PRT_ROW_ADD(PRT_ROW_SEP_NONE);
            PRT_CELL_SET("%s", &(field_type_info->enum_val_info[ii].name[str_len]));
    ...
    else if (field_type_info->nof_child_fields > 0)
    {
        ...
        if (field_type_info->refernce_field_id != DBAL_FIELD_TYPE_DEF_EMPTY)
        {
            PRT_INFO_ADD("Reference Field name: %s",
                         dbal_field_types_def_to_string(unit, field_type_info->refernce_field_id));
        }
