    ...
    char out_qual_value[DIAG_DNX_FIELD_UTILS_STR_SIZE])
{
    ...
    for (qual_index = 0; qual_index < bcmFieldQualifyCount && user_qual_size; qual_index++)
    {
        ...
        if (SHR_FAILURE(dnx_field_map_dnx_qual_size(unit, field_stage, pbus_dnx_qual, &qual_size)))
        {
    ...
    uint32 payload_ids[DNX_DATA_MAX_FIELD_GROUP_NOF_FGS])
{
    ...
    SHR_IF_ERR_EXIT(diag_dnx_field_utils_groups_per_context_get
                    (unit, context_id, DNX_FIELD_STAGE_EXTERNAL, &nof_fgs_per_ctx, fgs_per_context));
