...
rval_test_skip_reg(int unit, soc_regaddrinfo_t *ainfo, reg_data_t *rd)
{
    ...
    if(SOC_IS_DNX(unit)) {
        ...
        if (sal_strstr(SOC_REG_NAME(unit, ainfo->reg), "HBC_") != NULL) {
            return 1; /* Skip HBC registers (sw only registers)*/
        }
        ...
        switch(ainfo->reg) {
            ...
            case CGM_MAX_QB_VSQ_QSIZE_TRACK_SETTINGSr:
            case CGM_MC_TC_PRIORITY_MAPr:
            case CGM_VSQA_FC_STATUS_MASKr:
            case ECI_FAB_PLL_CONFIGURATIONSr:
            case ECI_TOP_LEVEL_SAMPLING_CFGr:
            case IPS_DEBUG_FILTERr:
            case IPS_QPRIORITY_IS_HIGHr:
