    ...
    sh_sand_control_t * sand_control)
{
    ...
    uint32 max_occupancy;
    uint32 fifo_level;
    int nof_priority_groups;
    int nof_entries, group_count;
    dnx_algo_port_info_s port_info;
    bcm_port_t logical_port, i;
    ...
    BCM_PBMP_ITER(nif_ports, logical_port)
    {
        ...
        if (!DNX_ALGO_PORT_TYPE_IS_NIF_ETH(unit, port_info, FALSE, FALSE))
        {
            continue;
        }
        ...
        for (i = 0; i < nof_priority_groups; ++i)
        {
             ...
             */
            SHR_IF_ERR_EXIT(dnx_algo_port_nif_logical_fifo_get(unit, logical_port, 0, i, &rmc_info));
            ...
            /*
             * Calculate nof entries
             */
            nof_entries = rmc_info.last_entry - rmc_info.first_entry + 1;

            /*
             * Format the PRD Priority string
             */
            sal_memset(prd_prio_str, 0, sizeof(prd_prio_str));
            if (rmc_info.prd_priority & BCM_PORT_F_PRIORITY_0)
            {
                utilex_str_append(prd_prio_str, "0");
            }
            if (rmc_info.prd_priority & BCM_PORT_F_PRIORITY_1)
            {
                utilex_str_append(prd_prio_str, "1");
            }
            if (rmc_info.prd_priority & BCM_PORT_F_PRIORITY_2)
            {
                utilex_str_append(prd_prio_str, "2");
            }
            if (rmc_info.prd_priority & BCM_PORT_F_PRIORITY_3)
            {
            ...
            SHR_IF_ERR_EXIT(imb_port_rx_fifo_status_get
                            (unit, logical_port, rmc_info.sch_priority, &max_occupancy, &fifo_level));
            ...
            PRT_CELL_SET("%s", sch_prio_str[rmc_info.sch_priority]);
