    ...
    sh_sand_control_t * sand_control)
{
    ...
    /** Determine if the user passed a correct core value. Amend start and end variables for iteration */
    if (core == 0 || core == 1)
    {
        nof_cores = (uint32) core + 1;
        core_id = (uint32) core;
    }
    else
    {
    ...
    if (!ISEMPTY(stage_name))
    {
        ...
        {
            int iter;
            SHR_IF_ERR_EXIT(dbal_tables_table_get(unit, table_names[stage_start], &table));
            ...
            {
                field_name = dbal_field_to_string(unit, table->keys_info[iter].field_id);
                dbal_entry_key_field32_set(unit, entry_handle_id, table->keys_info[iter].field_id, context_val);
                PRT_CELL_SET("%s = %u", field_name, context_val);
            }
            ...
            SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_GET_ALL_FIELDS));
                    /** Iterate over the values of the context enablers table for the given stage and print them in a table */
            for (iter = 0; iter < table->multi_res_info[0].nof_result_fields; iter++)
            {
                field_id = table->multi_res_info[0].results_info[iter].field_id;
                field_name = dbal_field_to_string(unit, field_id);
                PRT_ROW_ADD(PRT_ROW_SEP_NONE);

                PRT_CELL_SET("%s", field_name);
                SHR_IF_ERR_EXIT(dbal_entry_handle_value_field_arr32_get
                                (unit, entry_handle_id, field_id, INST_SINGLE, &field_value));
                if (field_value)
                {
                    PRT_CELL_SET("TRUE");
                }
