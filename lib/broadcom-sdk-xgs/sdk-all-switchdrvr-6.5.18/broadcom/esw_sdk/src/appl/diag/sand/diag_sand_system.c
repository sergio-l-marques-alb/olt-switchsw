    ...
    char *arguments_string)
{
    ...
    for (option_index = 0; option_index < option_count; option_index++)
    {
        ...
        if (per_keyword_tokens[1] != NULL)
        {
            ...
            /*
             * If the value contains .. split it to start and and end.
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if ((flag == TRUE) && (sh_sys_cmds_a != NULL))
    {
        ...
        SH_SAND_CMD_ITERATOR_COND(sh_sand_cmd, sh_sys_cmds_a)
        {
            PRT_ROW_ADD(PRT_ROW_SEP_NONE);
            if (!ISEMPTY(sh_sand_cmd->full_key))
            {
    ...
    int flags)
{
    ...
    if (flags & SH_CMD_OUTPUT_XML)
    {
        ...
        else
        {
            char *folder = "xml";
            /*
             * Currently put all dumps under xml - we may reconsider it later - probably use "folder" argument instead
             */
    ...
    uint32 token_count)
{
    ...
    SH_SAND_CMD_ITERATOR_ALL(sh_sand_cmd, sh_sand_cmd_a)
    {
        ...
        {
            SHR_CLI_EXIT_IF_ERR(sh_sand_manual_leaf(unit, sh_sand_cmd, cur_node, depth + 1, label_tokens, token_count),
                                "");
        }
        else
        {
    ...
    sh_sand_control_t * sand_control)
{
     ...
     */
    {
        ...
        sh_sand_manual_leaf(unit, &system_arguments_cmd, intro_node, 1, label_tokens, realtokens);
    }
    /*
    ...
    {
        SHR_CLI_EXIT_IF_ERR(sh_sand_manual_leaf(unit, sand_control->sh_sand_cmd, top_node, 0, label_tokens, realtokens),
                            "");
    }
...
sh_sand_option_t sys_manual_arguments[] = {
    { "label",  SAL_FIELD_TYPE_STR,   "Specific label to sort the comaands with.",  ""},
    {NULL}
};
