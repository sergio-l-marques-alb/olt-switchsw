...
{   
  ...
  else if ((params_num > 0) && (! sal_strncasecmp(flavor, "clear_on_get", strlen(flavor)))) {
    ...
    cmd_ppd_api_diag_pp_mc_info_print(int unit, int core, uint32 mc_id)
{
    ...
    if(SOC_IS_QAX(unit)){
        ...
        qax_mcdb_entry_t *entry;

        SOCDNX_IF_ERR_EXIT(READ_TAR_MCDBm(unit, MEM_BLOCK_ANY, entry_index, data));
        entry = (qax_mcdb_entry_t *)data;
        if (DPP_MCDS_TYPE_IS_INGRESS(QAX_MCDS_ENTRY_GET_TYPE(entry))) { /* ingress multicast */
            uint32 format;
            uint32 destination;
            uint32 cud;
