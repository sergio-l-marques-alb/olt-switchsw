...
#define __PP_LAST_GET_FWD_LAYER_LOOKUP_TERMS(__fwd_layer)                                                                                                                           \
    ...
    {                                                                                                                                                                       \
        ...
        if (field_header_sig)                                                                                                                                               \
        {                                                                                                                                                                   \
            sal_strncpy_s(current_fwd_layer_lookup_terms.lookup_term_signals[i_sig_fwd_lookup_term], field_header_sig->print_value, RHNAME_MAX_SIZE-1);                   \
        }                                                                                                                                                                   \
        else                                                                                                                                                                \
        {                                                                                                                                                                   \
            sal_strncpy_s(current_fwd_layer_lookup_terms.lookup_term_signals[i_sig_fwd_lookup_term], "Not Found", RHNAME_MAX_SIZE-1);                                     \
        }                                                                                                                                                                   \
    }                                                                                                                                                                       \
                                                                                                                                                                            \
    ...
    {                                                                                                                                                                       \
        ...
        {                                                                                                                                                                   \
            sal_strncpy_s(current_fwd_layer_lookup_terms.extra_lookup_term_str[i_extra_fwd_lookup_term], fodo_str, RHNAME_MAX_SIZE-1);                                    \
        }                                                                                                                                                                   \
        else if (pp_last_fwd_layers_lookup_terms[__fwd_layer].extra_lookup_term[i_extra_fwd_lookup_term] == PP_LAST_GLOBAL_LIF)                                                             \
        {                                                                                                                                                                   \
            sal_strncpy_s(current_fwd_layer_lookup_terms.extra_lookup_term_str[i_extra_fwd_lookup_term], "Global LIF = ", RHNAME_MAX_SIZE-1);                             \
            sal_strncat(current_fwd_layer_lookup_terms.extra_lookup_term_str[i_extra_fwd_lookup_term], global_lif_str, RHNAME_MAX_SIZE-1);                                \
        }                                                                                                                                                                   \
    }
...
{
    char lookup_term_signals[PP_LAST_MAX_NOF_LAYER_LOOKUP_TERMS][RHNAME_MAX_SIZE];
    int nof_lookup_terms_signals;
    char extra_lookup_term_str[PP_LAST_MAX_NOF_LAYER_LOOKUP_TERMS][RHNAME_MAX_SIZE];
    int nof_lookup_terms_extra;
    ...
    int core_id)
{
    ...
    char val_string[RHNAME_MAX_SIZE] = "Not Found";
    char fwd_layer_string[RHNAME_MAX_SIZE] = "";

    int pparse_layer_offset = 0;

    /** global lif and PP_LAST_FODO used as lookup up terms of fwd layer in FWD stage print */
    char global_lif_str[RHNAME_MAX_SIZE] = "Global LIF N/A";
    char fodo_str[RHNAME_MAX_SIZE] = "FODO N/A";
     ...
     * - for special cases where instead of from "NIF", used another block name as "InNetworkAdaptor" *
     **************************************************************************************************/
    if (sand_signal_output_find(unit, core_id, 0, "IRPP", "NIF", "", "header", &sig_list_element) != _SHR_E_NONE)
    {
    ...
    {
        uint8 skip_print = 0;
        sal_strncpy_s(val_string, "Not Found", RHNAME_MAX_SIZE - 1);
        ...
        if (signal_result[i_sig].status == BCM_E_NONE)
        {
            ...
            if (i_sig == 0)
            {
                ...
                sal_strncat(val_string, signal_result[i_sig].print_value, RHNAME_MAX_SIZE - 1);

                /** store the Global LIF for FWD stage lookup terms  */
                sal_strncpy_s(global_lif_str, val_string, RHNAME_MAX_SIZE - 1);
            }
            /** for PP_LAST_FODO construct a string of PP_LAST_FODO Type + Value, and store for FWD stage lookup terms */
            else if (i_sig == 1)
            {
                sal_strncpy_s(val_string, signal_result[1].print_value, RHNAME_MAX_SIZE - 1);
                sal_strncat(val_string, " = 0x", RHNAME_MAX_SIZE - 1);
                ...
                sal_strncpy_s(fodo_str, val_string, RHNAME_MAX_SIZE - 1);
    ...
    if (sand_signal_output_find(unit, core_id, 0, "IRPP", "NIF", "", "pkt_header", &pkt_header_sig) != _SHR_E_NONE)
    {
    ...
    i_sig = 0;
    sal_strncpy_s(val_string, "idx(", RHNAME_MAX_SIZE - 1);
    sal_strncat(val_string, signal_result[3].print_value, RHNAME_MAX_SIZE - 1);
    sal_strncat(val_string, ") - ", RHNAME_MAX_SIZE - 1);
    sal_strncpy_s(fwd_layer_string, protocols_sig->print_value, RHNAME_MAX_SIZE - 1);
    ...
    sal_strncat(val_string, fwd_layer_string, RHNAME_MAX_SIZE - 1);
    PRT_CELL_SET_PL(PP_LAST_VTT_SUMMARY_TABLE_ID, "%s", val_string);
...
/**
 * \brief - simple function that sends TCPoIPv4oETH1oPTCH packet with default fields.
 *     Packet is sent from port 201.
 * \param [in] unit - unit id
 * \return
 ...
 */
shr_error_e
dnx_pp_last_packet_send_simple(
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(diag_sand_packet_proto_add(unit, packet_h, "ETH2"));
...
sh_sand_man_t dnx_pp_last_man = {
    ...
    .init_cb = dnx_pp_last_packet_send_simple,
    .examples = "core=0\n" "core=1\n" "\n",
