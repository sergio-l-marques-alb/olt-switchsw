...
#ifdef BCM_LTSW_SUPPORT
        /*
         * In HSDK system, cm_dev may not be consistent with bde handle,
         * so keep the bde handle info in CMDEV.
         */
        soc_cm_dev_num_set(cm_dev, u);
#else
        assert(cm_dev == u);
#endif
        sysconf_probe_done++;
...
sysconf_attach(int unit)
{
    ...
    assert(unit >= 0 && unit < soc_cm_device_count);
