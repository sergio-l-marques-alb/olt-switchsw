 ...
 */
const dnx_l3_ecmp_consistent_type_t ecmp_consistent_types_attributes[L3_ECMP_CONSISTENT_TABLE_NOF_TYPES] = {
    {
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(diag_sand_packet_send(unit, in_port, packet_h, SAND_PACKET_RX));
    ...
    char *values[DIAG_DNX_PP_VIS_FER_NOF_COLUMNS])
{
    ...
    SHR_IF_ERR_EXIT(bcm_l3_egress_ecmp_get(unit, &ecmp_group, L3_ECMP_MAX_NOF_ECMP_MEMBERS, intf_array, &intf_count));
    ...
    SHR_IF_ERR_EXIT(dbal_table_field_printable_string_get
                    (unit, DBAL_TABLE_ECMP_GROUP_PROFILE_TABLE, DBAL_FIELD_MAX_GROUP_SIZE_MINUS_ONE,
                     (uint32 *) &ecmp_profile.group_size, NULL, 0, FALSE, values[2]));
    SHR_IF_ERR_EXIT(dbal_table_field_printable_string_get
    ...
    {
        fec_offset = (ecmp_lb_keys[hierarchy_id] % ecmp_profile.group_size) * (ecmp_profile.protection_flag + 1);
        used_member = BCM_L3_ITF_VAL_GET(intf_array[0]) + fec_offset;
    }
    else
    {
        uint32 full_fec_offset;
        uint8 fec_offset_cons;
        if (ecmp_profile.ecmp_mode == DBAL_ENUM_FVAL_ECMP_MODE_CONSISTENT_SMALL_TABLE)
        {
            full_fec_offset =
                (ecmp_lb_keys[hierarchy_id] % L3_ECMP_CONSISTENT_TABLE_SMALL_NOF_ENTRIES) *
                (ecmp_profile.protection_flag + 1);
        }
        else if (ecmp_profile.ecmp_mode == DBAL_ENUM_FVAL_ECMP_MODE_CONSISTENT_MEDIUM_TABLE)
        {
            full_fec_offset =
                (ecmp_lb_keys[hierarchy_id] % L3_ECMP_CONSISTENT_TABLE_MEDIUM_NOF_ENTRIES) *
                (ecmp_profile.protection_flag + 1);
        }
        else
        {
            full_fec_offset =
                (ecmp_lb_keys[hierarchy_id] %
                 ((L3_ECMP_CONSISTENT_TABLE_LARGE_NOF_ENTRIES / ecmp_profile.group_size) *
                  ecmp_profile.group_size)) * (ecmp_profile.protection_flag + 1);
        }
        SHR_IF_ERR_EXIT(dnx_l3_ecmp_group_entry_result_get(unit, ecmp_group_id, &ecmp_atr));
        unique_id = L3_ECMP_CHM_KEY_GET(unit, ecmp_atr.profile_index, hierarchy);
        SHR_IF_ERR_EXIT(ecmp_db_info.ecmp_fec_members_table.get(unit, unique_id, full_fec_offset, &fec_offset_cons));
    ...
    SHR_IF_ERR_EXIT(bcm_switch_control_get(unit, ecmp_hier_enum[hierarchy_id], &hashing_func));
    sal_snprintf(values[10], DBAL_MAX_PRINTABLE_BUFFER_SIZE, "%s", crc_functions[hashing_func]);
    control_key.type = bcmSwitchHashSeed;
    control_key.index = hashing_func;
    ...
    char *values[DIAG_DNX_PP_VIS_FER_NOF_COLUMNS])
{
    ...
    SHR_IF_ERR_EXIT(dbal_table_field_printable_string_get(unit, DBAL_TABLE_SUPER_FEC_1ST_HIERARCHY,
                                                          DBAL_FIELD_SUPER_FEC_ID, fec_id, NULL, 0, TRUE, values[1]));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    bcm_instru_vis_signal_key_t signal_key[3];
    bcm_instru_vis_signal_result_t signal_result[3];
    ...
    for (hier_id = 0; hier_id < nof_hierarchies; hier_id++)
    {
        ...
        {
            ecmp_values[hier_id][idx] = sal_alloc(sizeof(char) * DBAL_MAX_PRINTABLE_BUFFER_SIZE, "ECMP values print");
    ...
    sal_strncpy(signal_key[0].from, dnx_pp_stage_name(unit, DNX_PP_STAGE_IFWD2),
                BCM_INSTRU_VIS_STAGE_NAME_MAX_SIZE - 1);
    sal_strncpy(signal_key[0].signal, "Fwd_Action_Dst.type", BCM_INSTRU_VIS_SIGNAL_NAME_MAX_SIZE - 1);
    ...
    sal_strncpy(signal_key[1].from, dnx_pp_stage_name(unit, DNX_PP_STAGE_IFWD2),
                BCM_INSTRU_VIS_STAGE_NAME_MAX_SIZE - 1);
    sal_strncpy(signal_key[1].signal, "Fwd_Action_Dst.value", BCM_INSTRU_VIS_SIGNAL_NAME_MAX_SIZE - 1);
    ...
    sal_strncpy(signal_key[2].from, dnx_pp_stage_name(unit, DNX_PP_STAGE_IFWD2),
                BCM_INSTRU_VIS_STAGE_NAME_MAX_SIZE - 1);
    sal_strncpy(signal_key[2].signal, "Fwd_Action_Dst", BCM_INSTRU_VIS_SIGNAL_NAME_MAX_SIZE - 1);
    ...
    {
        fwd_dest_type = signal_result[0].value[0];
        fwd_destination = signal_result[1].value[0];
    }
    else
    {
        LOG_CLI(("Could not retrieve signal data for core = %d\n", core_id));
     ...
     */
    if (fwd_dest_type != fwd_type)
    {
        uint32 fwd_dest_w_encoding = signal_result[2].value[0];
    ...
    for (hier_id = 0; hier_id < nof_hierarchies; hier_id++)
    {
        ...
        if (is_allocated == 1)
        {
            SHR_IF_ERR_EXIT(algo_l3_fec_hierarchy_stage_map_get(unit, BCM_L3_ITF_VAL_GET(fwd_destination), &hierarchy));
        ...
        else
        {
            ...
            dpp_dsig_read(unit, core_id, dnx_pp_stage_block_name(unit, DNX_PP_STAGE_FER),
                          dnx_pp_stage_name(unit, DNX_PP_STAGE_FER), NULL, "Fwd_action_dst.value", &fwd_dest_verify, 1);
            if (fwd_dest_verify != (fwd_destination & 0xFFFFFF))
            {
                LOG_CLI(("Core_Id=%d: last FEC destination: %d, is not equal to the FER destination: %d!\n", core_id,
                         (fwd_destination & 0xFFFFFF), fwd_dest_verify));
            }
            break;
        }
    }
...
exit:
    for (hier_id = nof_hierarchies - 1; hier_id >= 0; hier_id--)
    {
        ...
        {
            if (ecmp_values[hier_id][idx] != NULL)
            {
                sal_free(ecmp_values[hier_id][idx]);
            }
            if (fec_values[hier_id][idx] != NULL)
            {
                sal_free(fec_values[hier_id][idx]);
            }
        }
    }
    PRT_FREE;
...
sh_sand_option_t dnx_pp_vis_fer_options[] = {
    {"core", SAL_FIELD_TYPE_INT32, "Core ID", "0"}
    ,
