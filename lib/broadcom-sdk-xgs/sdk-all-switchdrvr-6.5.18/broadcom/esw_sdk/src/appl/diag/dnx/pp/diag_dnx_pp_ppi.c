...
#define MAX_PORT_SIGNALS        5
        ...
        if (dnx_data_debug.hw_bug.feature_get(unit, dnx_data_debug_hw_bug_iparser_signals_valid))
        {
            i_sig++;
            sal_memset(&signal_key[i_sig], 0, sizeof(bcm_instru_vis_signal_key_t));
            sal_strncpy(signal_key[i_sig].block, "IRPP", BCM_INSTRU_VIS_BLOCK_NAME_MAX_SIZE - 1);
                ...
                if (!next_sub_layer_flag)
                {
                    ...
                    if (!system_header_flag)
                    {
                        ...
                        if (sal_strcasecmp(top_layer_name, "ETH") == 0)
                        {
                             ...
                             */
                            SHR_CLI_EXIT_IF_ERR(dpp_dsig_read(unit, core_id, "ERPP", "PRP", "",
                                                              "eth_tag_format.outer_tag_is_priority",
                                                              &tpid_prio_list_element, 1),
...
exit:
    ...
    sand_signal_output_free(field_list_element);
