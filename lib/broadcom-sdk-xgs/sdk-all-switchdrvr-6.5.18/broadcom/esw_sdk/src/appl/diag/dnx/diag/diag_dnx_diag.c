    ...
    else
    {
        ...
        for (reg_idx = 0; reg_idx < nof_regs; reg_idx++)
        {
            ...
            if (SOC_BLOCK_IN_LIST
                (SOC_REG_INFO(unit, blk_group[*blk_idx].counter_table[*cnt_idx].reg_arr[reg_idx]).block, SOC_BLK_CDU))
            {
                nof_instance = dnx_data_nif.eth.cdu_nof_get(unit) - dnx_data_nif.eth.cdum_nof_get(unit);
            }
            else if (SOC_BLOCK_IN_LIST
                     (SOC_REG_INFO(unit, blk_group[*blk_idx].counter_table[*cnt_idx].reg_arr[reg_idx]).block,
                      SOC_BLK_CDUM))
            {
            ...
            for (instance_id = 0; instance_id < nof_instance; instance_id++)
            {
                ...
                if (instance_id < (nof_instance / dnx_data_device.general.nof_cores_get(unit)))
                {
                    ...
                    SHR_IF_ERR_EXIT(sh_dnxc_get_cnt_and_ovf_value
                                    (unit, 1, blk_group[*blk_idx].counter_table[*cnt_idx].reg_arr[reg_idx],
                                     reg_value, blk_group[*blk_idx].counter_table[*cnt_idx].cnt_field[reg_idx],
                                     field_len, temp_fld_value, &overflow[COUNTER_GRAPHIC_CORE_1ST]));
                ...
                else
                {
                    ...
                    SHR_IF_ERR_EXIT(sh_dnxc_get_cnt_and_ovf_value
                                    (unit, 1, blk_group[*blk_idx].counter_table[*cnt_idx].reg_arr[reg_idx],
                                     reg_value, blk_group[*blk_idx].counter_table[*cnt_idx].cnt_field[reg_idx],
                                     field_len, temp_fld_value, &overflow[COUNTER_GRAPHIC_CORE_2ND]));
                    COMPILER_64_ZERO(temp_fld_value_u64);
                    COMPILER_64_SET(temp_fld_value_u64, temp_fld_value[1], temp_fld_value[0]);
    ...
    sh_dnx_counter_graphical_block_t * working_block)
{
    ...
    for (entry = 0; entry < dnx_data_graphical.diag.num_of_block_entries_get(unit); entry++)
    {
        ...
        table->overflow_field = dnx_data_graphical.diag.counters_get(unit, blk, entry)->overflow_field;
    ...
    dnx_diag_graphical_block * block)
{
    ...
    for (row = 0; row < dnx_data_graphical.diag.num_of_rows_get(unit); row++)
    {
        for (index = 0; index < dnx_data_graphical.diag.num_of_index_get(unit); index++)
        {
            /** check left */
            if ((dnx_data_graphical.diag.blocks_left_get(unit, row, index)->valid) &&
                (!sal_strncasecmp(dnx_data_graphical.diag.blocks_left_get(unit, row, index)->name, name, strlen(name))))
            {
                blk = dnx_data_graphical.diag.blocks_left_get(unit, row, index)->block;
                break;
            }
            ...
            if ((dnx_data_graphical.diag.blocks_right_get(unit, row, index)->valid) &&
                (!sal_strncasecmp
                 (dnx_data_graphical.diag.blocks_right_get(unit, row, index)->name, name, strlen(name))))
            {
                blk = dnx_data_graphical.diag.blocks_right_get(unit, row, index)->block;
                break;
            }
    ...
    uint64 *counter)
{
    ...
    for (reg_idx = 0; reg_idx < nof_regs; reg_idx++)
    {
        ...
        /** if CDU, need to sum all instance values and devide them to core 0/1 according to the instance id */
        if (SOC_BLOCK_IN_LIST(SOC_REG_INFO(unit, reg_entry->reg_arr[reg_idx]).block, SOC_BLK_CDU))
        {
            nof_instance = dnx_data_nif.eth.cdu_nof_get(unit) - dnx_data_nif.eth.cdum_nof_get(unit);
        }
        else if (SOC_BLOCK_IN_LIST(SOC_REG_INFO(unit, reg_entry->reg_arr[reg_idx]).block, SOC_BLK_CDUM))
        {
            nof_instance = dnx_data_nif.eth.cdum_nof_get(unit);
        }
        else if (SOC_BLOCK_IN_LIST(SOC_REG_INFO(unit, reg_entry->reg_arr[reg_idx]).block, SOC_BLK_CLU))
        {
            nof_instance = dnx_data_nif.eth.clu_nof_get(unit);
        }
        else if (SOC_BLOCK_IN_LIST(SOC_REG_INFO(unit, reg_entry->reg_arr[reg_idx]).block, SOC_BLK_ILU))
        {
            nof_instance = dnx_data_nif.ilkn.ilkn_unit_nof_get(unit);
        }
        else
        ...
        for (instance_id = 0; instance_id < nof_instance; instance_id++)
        {
            ...
            if ((SOC_BLOCK_IN_LIST(SOC_REG_INFO(unit, reg_entry->reg_arr[reg_idx]).block, SOC_BLK_FDT)) ||
                (SOC_BLOCK_IN_LIST(SOC_REG_INFO(unit, reg_entry->reg_arr[reg_idx]).block, SOC_BLK_FDA)))
            {
    ...
    sh_sand_control_t * sand_control)
{
    ...
    PRT_COLUMN_ADDX(PRT_XML_ATTRIBUTE, object_col_id, NULL, "Description");

    if (!block_is_present)
    {
        /** no block present */
        SHR_IF_ERR_EXIT_WITH_LOG(_SHR_E_NOT_FOUND, "No block specified for block register display%s%s%s\n", EMPTY,
                                 EMPTY, EMPTY);
    }
    else
    {
