    ...
    ekleap_shared_data_t * data)
{
    ...
    for (etps_i = 0; (valid >> etps_i) > 0; ++etps_i)
    {
         ...
         * Get ETPS entry
         */
        sal_snprintf(signal_name, RHNAME_MAX_SIZE, "ETPS.ETPS_%d", etps_i + 1);
        SHR_IF_ERR_EXIT_NO_MSG(sand_signal_output_find(unit, core, 0, "ETPP", "Term", NULL, signal_name, &etps_sig));
    ...
    prt_control_t * prt_ctr)
{
    ...
    dbal_entry_key_field32_set(unit, entry_handle_id, ctx_table->keys_info[0].field_id, ctx_val);
    ...
    for (field_i = 0; field_i < ctx_table->multi_res_info[0].nof_result_fields; ++field_i)
    {
        ...
        else if (prt_context_details)
        {
            ...
            else
            {
                ...
                SHR_IF_ERR_EXIT_NO_MSG(dbal_table_field_printable_string_get
                                       (unit, stage_info->ctx_table_id, field_info->field_id, &field_dbal_value, NULL,
    ...
    prt_control_t * prt_ctr)
{
    ...
    for (esem_acc = 0; esem_acc < EKLEAP_TERM_ESEM_ACC_NOF; ++esem_acc)
    {
        ...
        else
        {
            ...
            PRT_CELL_SET("%s", (signal_output->value[0] ? "Valid" : "Disabled"));
    ...
    char **desc)
{
    ...
    switch (ctx)
    {
        case DBAL_ENUM_FVAL_TRAP_CONTEXT_ID_GENERAL___RCH_ENC:
    ...
    prt_control_t * prt_ctr)
{
    ...
    if (bytes_added == 0)
    {
        SHR_IF_ERR_EXIT_NO_MSG(btc_gen_header_empty_add(unit, prt_ctr));
    }
    else
    {
