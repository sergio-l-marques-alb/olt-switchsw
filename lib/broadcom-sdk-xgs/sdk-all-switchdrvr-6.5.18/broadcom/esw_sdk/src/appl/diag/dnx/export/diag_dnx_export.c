    ...
    xml_node * top)
{
     ...
     */
    if (NULL ==
        (in_top = dbx_file_get_xml_top(unit, filename, "top", CONF_OPEN_CURRENT_LOC | CONF_OPEN_NO_ERROR_REPORT)))
    {
        char relative_filename[DNX_DIAG_EXPORT_CMD_LEN_MAX];
        /*
         * Local file open failed.
         * Try the global scrips directory.
         */
        sal_snprintf(relative_filename, DNX_DIAG_EXPORT_CMD_LEN_MAX, "%s%s",
                     DNX_DIAG_EXPORT_DEFAULT_INPUT_SCRIPTS_REL_PATH, filename);
        if ((in_top = dbx_file_get_xml_top(unit, relative_filename, "top", CONF_OPEN_PER_DEVICE)) == NULL)
        {
