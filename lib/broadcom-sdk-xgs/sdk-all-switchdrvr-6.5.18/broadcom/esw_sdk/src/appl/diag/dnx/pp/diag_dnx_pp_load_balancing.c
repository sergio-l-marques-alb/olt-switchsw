    ...
    sh_sand_control_t * sand_control)
{
    ...
    SH_DNX_CORES_ITER(unit, core_in, core_id)
    {
        ...
        for (idx = 0; idx < max_layer_id; idx++)
        {
             ...
             */
            if ((idx >= forwarding_layer || layer_type == DBAL_ENUM_FVAL_LAYER_TYPES_MPLS
                                        || layer_type == DBAL_ENUM_FVAL_LAYER_TYPES_MPLS_UA) && is_fragmented == FALSE)
            {
                PRT_CELL_SET("TRUE");
            }
            else
            ...
            if (layer_type == DBAL_ENUM_FVAL_LAYER_TYPES_IPV4 || layer_type == DBAL_ENUM_FVAL_LAYER_TYPES_IPV6)
            {
