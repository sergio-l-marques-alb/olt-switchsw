    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (is_stif || is_crps)
    {
        if (is_database_present || is_stif)
        {
            SHR_IF_ERR_EXIT(diag_dnx_ingress_compensation_header_truncate_get
                            (unit, core_id, database_id, &is_hdr_truncate_enabled));
    ...
    if (is_qid_present)
    {
        ...
        SHR_IF_ERR_EXIT(sh_dnx_tm_queue_base_queue_get(unit, qid, &base_qid, &num_cos));
        BCM_GPORT_UNICAST_QUEUE_GROUP_CORE_QUEUE_SET(gport, BCM_CORE_ALL, base_qid);
