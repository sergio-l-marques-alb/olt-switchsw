    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (ISEMPTY(table_name))
    {
        ...
        if (clusters_print == TRUE)
        {
             ...
             */
            PRT_TITLE_SET("Cluster allocation");
            ...
            for (macro_idx = 0; macro_idx < dnx_data_mdb.dh.total_nof_macroes_get(unit); macro_idx++)
            {
                PRT_ROW_ADD(PRT_ROW_SEP_NONE);
                PRT_CELL_SET("Macro %d", macro_idx);
                ...
                for (cluster_idx = 0; cluster_idx < dnx_data_mdb.dh.nof_macro_clusters_get(unit); cluster_idx++)
                {
                    ...
                    if (macro_idx < dnx_data_mdb.dh.nof_macroes_type_A_get(unit))
                    {
                        macro_type = MDB_MACRO_A;
                        serial_cluster_index =
                            MDB_DH_SERIAL_CLUSTER_NUMBER_FOR_MACRO_TYPE(unit, macro_idx, cluster_idx);
                    }
                    else
                    {
                        macro_type = MDB_MACRO_B;
                        serial_cluster_index =
                            MDB_DH_SERIAL_CLUSTER_NUMBER_FOR_MACRO_TYPE(unit,
                                                                        macro_idx -
                    ...
                    SHR_IF_ERR_EXIT(mdb_db_infos.mdb_cluster_infos.macro_cluster_assoc.
                                    pdb.get(unit, macro_type, serial_cluster_index, &mdb_physical_table_id));
...
mdb_diag_verify_kbpsdk(
    int unit)
{
    const char *kbp_sdk_ver_expected = "KBP SDK 1.5.11_EA2";
    ...
    else
    {
        LOG_INFO(BSL_LOG_MODULE,
                 (BSL_META_U(unit, "KBPSDK version test passed, version: %s\n"), kbp_device_get_sdk_version()));
    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    uint8 hit_bit_enabled =
        (((dbal_physical_table_id == DBAL_PHYSICAL_TABLE_FEC_1) || (dbal_physical_table_id == DBAL_PHYSICAL_TABLE_FEC_2)
          || (dbal_physical_table_id == DBAL_PHYSICAL_TABLE_FEC_3))
         && (dnx_data_l3.hw_bug.feature_get(unit, dnx_data_l3_hw_bug_fec_hit_bit))) ? FALSE : TRUE;
    PRT_INIT_VARS;
        ...
        else if (get_cmd != 0)
        {
            ...
            has_two_hit_bits =
                (((dbal_physical_table_id == DBAL_PHYSICAL_TABLE_FEC_1)
                  ...
                  || (dbal_physical_table_id == DBAL_PHYSICAL_TABLE_FEC_3))
                 && (!dnx_data_l3.hw_bug.feature_get(unit, dnx_data_l3_hw_bug_fec_hit_bit)));
            if (hit_cmd == TRUE)
            {
                entry.hitbit |= (DBAL_PHYSICAL_KEY_HITBIT_GET | DBAL_PHYSICAL_KEY_HITBIT_PRIMARY);
                if (has_two_hit_bits)
...
static sh_sand_man_t dnx_mdb_em_man = {
    ...
    .examples =
        ...
        "get hw table=LEM key=0x123456789abc key_size=49 payload_size=48 app_id=0\n"
#ifndef ADAPTER_SERVER_MODE
        "get table=LEM key=0x123456789abc key_size=49 payload_size=48 hit app_id=0\n"
        "get table=LEM key=0x123456789abc key_size=49 payload_size=48 hit_clr app_id=0\n"
#endif
    "delete table=LEM key=0x123456789abc key_size=49 payload_size=48 app_id=0\n" "clear table=LEM app_id=0xFFFFFFFF"
};
