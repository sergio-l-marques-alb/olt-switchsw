    ...
    prt_control_t * prt_ctr)
{
         ...
         key_info_p->key_id.id[key_index] != DNX_FIELD_KEY_ID_INVALID; key_index++)
    {
             ...
             key_info_p->key_template.key_qual_map[qual_index].qual_type != DNX_FIELD_QUAL_TYPE_INVALID; qual_index++)
        {
            ...
            {
                PRT_CELL_SET("Could not retrieve value");
            }
    ...
    *is_entry_found_p = FALSE;
    ...
    for (entry_id = 0; entry_id < max_nof_tcam_entries; entry_id++)
    {
        ...
        if (found)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_field_group_tcam_entry_fg_id(unit, entry_id_from_location, &handle_fg_id));
            if (handle_fg_id == fg_id)
            {
                SHR_IF_ERR_EXIT(dnx_field_entry_tcam_hit_per_core_get
    ...
    sh_sand_control_t * sand_control)
{
    ...
    uint32 entry_handle_id;
    dbal_entry_handle_t *entry_handle_internal = NULL;
    ...
    *is_entry_found_p = FALSE;
    ...
    SHR_IF_ERR_EXIT(dnx_field_group_kbp_segment_indices(unit, fg_id, opcode_id_uint8, seg_index_on_master_by_qual));
    ...
    SHR_IF_ERR_EXIT(dbal_entry_handle_get_internal(unit, entry_handle_id, &entry_handle_internal));
         ...
         qual_iter++)
    {
        ...
        /** Iterate over all segments and see, which is matching on the current qualifier. */
        for (seg_iter = 0; seg_iter < nof_segments; seg_iter++)
        {
            qual_size = BYTES2BITS(ms_key_segments[seg_iter].nof_bytes);
            ...
            {
                master_key_seg_offset += qual_size;
            }
        }

        /** Calculating the location for the current qualifier value in the kbp_search_key, which will be used for kbp_search. */
        kbp_search_start_offset -= BITS2BYTES(qual_size);
        sal_memcpy(&kbp_search_key[kbp_search_start_offset], &rop_packet.payload[BITS2BYTES(master_key_seg_offset)],
                   BITS2BYTES(qual_size));
        ...
        SHR_IF_ERR_EXIT(dnx_kbp_buffer_kbp_to_dbal
                        (unit, qual_size, &rop_packet.payload[BITS2BYTES(master_key_seg_offset)], qual_val));
        SHR_IF_ERR_EXIT(utilex_bitstream_set_any_field
                        (qual_val, last_key_value_array_offset, qual_size,
                         last_diag_fg_info_p[fg_id_index].kbp_info.kbp_acl_key_value));
        /** Store the offset on master key for every qualifier. */
        last_diag_fg_info_p[fg_id_index].kbp_info.quals_offset_on_master_key[qual_iter] = master_key_seg_offset;

        last_key_value_array_offset += qual_size;
    ...
    DNX_KBP_TRY(kbp_db_search((((kbp_db_handles_t *) (entry_handle_internal->table->kbp_handles))->db_p),
                              &kbp_search_key[kbp_search_start_offset], &db_entry, &access_id, &prio_len));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SHR_IF_ERR_EXIT(diag_dnx_field_utils_groups_per_context_get
                    (unit, context_id, field_stage, &nof_fgs_per_context, fgs_per_context));
    ...
    for (fg_id_index = 0; fg_id_index < nof_fgs_per_context; fg_id_index++)
    {
        ...
        sal_memset(last_diag_fg_info_p[fg_id_index].key_values, 0, sizeof(last_diag_fg_info_p[fg_id_index].key_values));
        sal_memset(last_diag_fg_info_p[fg_id_index].result_values, 0,
                   sizeof(last_diag_fg_info_p[fg_id_index].result_values));
        ...
        switch (fg_type)
        {
            case DNX_FIELD_GROUP_TYPE_TCAM:
            {
                     ...
                     attach_full_info->key_id.id[key_id_iter] != DNX_FIELD_KEY_ID_INVALID; key_id_iter++)
                {
                    ...
                    switch (Field_key_enum_table[attach_full_info->key_id.id[key_id_iter]].string[0])
                    {
                        case 'A':
                        {
                            if (DIAG_DNX_FIELD_LAST_IS_SIM)
                            {
                                if (field_stage == DNX_FIELD_STAGE_IPMF1)
                                {
                                    SHR_IF_ERR_EXIT(diag_dnx_field_utils_last_key_table_info_get
                                                    (unit, DBAL_TABLE_FIELD_IPMF1_LAST_KEY_INFO, core_id,
                            ...
                            else
                            {
                                ...
                                SHR_IF_ERR_EXIT(dpp_dsig_read
                                                (unit, core_id, block_p, to_p, from_p, "Result_A",
                                                 field_result_value, 2));
                            }
                            break;
                        }
                        case 'B':
                        {
                            ...
                            else
                            {
                                ...
                                SHR_IF_ERR_EXIT(dpp_dsig_read
                                                (unit, core_id, block_p, to_p, from_p, "Result_B",
                                                 field_result_value, 2));
                            }
                            break;
                        }
                        case 'C':
                        {
                            if (DIAG_DNX_FIELD_LAST_IS_SIM)
                            {
                                if (field_stage == DNX_FIELD_STAGE_IPMF1)
                                {
                                    SHR_IF_ERR_EXIT(diag_dnx_field_utils_last_key_table_info_get
                                                    (unit, DBAL_TABLE_FIELD_IPMF1_LAST_KEY_INFO, core_id,
                                                     DBAL_ENUM_FVAL_FIELD_KEY_C, field_key_value));
                                }
                                else if (field_stage == DNX_FIELD_STAGE_IPMF3)
                                {
                                    SHR_IF_ERR_EXIT(diag_dnx_field_utils_last_key_table_info_get
                            ...
                            else
                            {
                                ...
                                SHR_IF_ERR_EXIT(dpp_dsig_read
                                                (unit, core_id, block_p, to_p, from_p, "Result_C", field_result_value,
                                                 2));
                            }
                            break;
                        }
                        case 'D':
                        {
                            if (DIAG_DNX_FIELD_LAST_IS_SIM)
                            {
                                if (field_stage == DNX_FIELD_STAGE_IPMF1)
                                {
                                    SHR_IF_ERR_EXIT(diag_dnx_field_utils_last_key_table_info_get
                                                    (unit, DBAL_TABLE_FIELD_IPMF1_LAST_KEY_INFO, core_id,
                                                     DBAL_ENUM_FVAL_FIELD_KEY_D, field_key_value));
                                }
                            }
                            else
                            {
                                ...
                                SHR_IF_ERR_EXIT(dpp_dsig_read
                                                (unit, core_id, block_p, to_p, from_p, "Result_D", field_result_value,
                                                 2));
                            }
                            break;
                        }
                                ...
                                if (field_stage == DNX_FIELD_STAGE_IPMF2)
                                {
                                    SHR_IF_ERR_EXIT(diag_dnx_field_utils_last_key_table_info_get
                                                    (unit, DBAL_TABLE_FIELD_IPMF2_LAST_KEY_INFO, core_id,
                                                     DBAL_ENUM_FVAL_FIELD_KEY_F, field_key_value));
                                }
                            }
                            else
                            {
                                ...
                                SHR_IF_ERR_EXIT(dpp_dsig_read
                                                (unit, core_id, block_p, to_p, from_p, "Result_F", field_result_value,
                                                 2));
                            }
                            break;
                        }
                        case 'G':
                        {
                            if (DIAG_DNX_FIELD_LAST_IS_SIM)
                            {
                                if (field_stage == DNX_FIELD_STAGE_IPMF2)
                                {
                                    SHR_IF_ERR_EXIT(diag_dnx_field_utils_last_key_table_info_get
                                                    (unit, DBAL_TABLE_FIELD_IPMF2_LAST_KEY_INFO, core_id,
                                                     DBAL_ENUM_FVAL_FIELD_KEY_G, field_key_value));
                                }
                            }
                            else
                            {
                                ...
                                SHR_IF_ERR_EXIT(dpp_dsig_read
                                                (unit, core_id, block_p, to_p, from_p, "Result_G", field_result_value,
                                                 2));
                            }
                            break;
                        }
                        case 'H':
                        {
                            if (DIAG_DNX_FIELD_LAST_IS_SIM)
                            {
                                if (field_stage == DNX_FIELD_STAGE_IPMF2)
                                {
                                    SHR_IF_ERR_EXIT(diag_dnx_field_utils_last_key_table_info_get
                                                    (unit, DBAL_TABLE_FIELD_IPMF2_LAST_KEY_INFO, core_id,
                                                     DBAL_ENUM_FVAL_FIELD_KEY_H, field_key_value));
                                }
                            }
                            else
                            {
                                ...
                                SHR_IF_ERR_EXIT(dpp_dsig_read
                                                (unit, core_id, block_p, to_p, from_p, "Result_H", field_result_value,
                                                 2));
                            }
                            break;
                        }
                        case 'I':
                        {
                            ...
                            else
                            {
                                ...
                                SHR_IF_ERR_EXIT(dpp_dsig_read
                                                (unit, core_id, block_p, to_p, from_p, "Result_I",
                                                 field_result_value, 2));
                            }
                            break;
                        }
                    ...
                    SHR_IF_ERR_EXIT(utilex_bitstream_set_any_field
                                    (field_result_value, result_value_start_offset,
                    ...
                    if (add_new_fg_id)
                    {
                        if (DIAG_DNX_FIELD_LAST_IS_SIM)
                        {
                            SHR_IF_ERR_EXIT(diag_dnx_field_last_entry_info_get
                                            (unit, core_id, fg_id_index, fgs_per_context[fg_id_index],
                                             last_diag_fg_info_p, &is_entry_found_p, sand_control));

                            /**
                             * In case of Cmodel we rely on returned hit indication 'is_entry_found_p' by
            ...
            {
                action_size_single = dnx_data_field.exem.small_max_result_size_get(unit);
                PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE_BEFORE);
                /** Take the name for current FG from the SW state. */
                SHR_IF_ERR_EXIT(dnx_field_group_sw.fg_info.name.get(unit, fgs_per_context[fg_id_index], &group_name));
                /**
                     ...
                     attach_full_info->key_id.id[key_id_iter] != DNX_FIELD_KEY_ID_INVALID; key_id_iter++)
                {
                    ...
                    switch (Field_key_enum_table[attach_full_info->key_id.id[key_id_iter]].string[0])
                    {
                        case 'A':
                        {
                            ...
                            {
                                SHR_IF_ERR_EXIT(dpp_dsig_read
                                                (unit, core_id, block_p, from_p, "LEXEM1", "Key_A", field_key_value,
                                                 5));
                                SHR_IF_ERR_EXIT(dpp_dsig_read
                                                (unit, core_id, block_p, "LEXEM1", from_p, "Hit_A", &field_key_hit, 1));
                                SHR_IF_ERR_EXIT(dpp_dsig_read
                                                (unit, core_id, block_p, "LEXEM1", from_p, "Result_A",
                                                 field_result_value, 2));
                            }
                            break;
                        }
                        case 'C':
                        {
                            ...
                            {
                                SHR_IF_ERR_EXIT(dpp_dsig_read
                                                (unit, core_id, block_p, from_p, "SEXEM3", "Key_C", field_key_value,
                                                 5));
                                SHR_IF_ERR_EXIT(dpp_dsig_read
                                                (unit, core_id, block_p, "SEXEM3", from_p, "Hit_C", &field_key_hit, 1));
                                SHR_IF_ERR_EXIT(dpp_dsig_read
                                                (unit, core_id, block_p, "SEXEM3", from_p, "Result", field_result_value,
                                                 2));
                            }
                            break;
                        }
                        case 'E':
                        {
                            if (DIAG_DNX_FIELD_LAST_IS_SIM)
                            {
                                if (field_stage == DNX_FIELD_STAGE_IPMF1)
                                {
                                    SHR_IF_ERR_EXIT(diag_dnx_field_utils_last_key_table_info_get
                                                    (unit, DBAL_TABLE_FIELD_IPMF1_LAST_KEY_INFO, core_id,
                                                     DBAL_ENUM_FVAL_FIELD_KEY_E, field_key_value));
                                }
                                field_key_hit = TRUE;
                            }
                            else
                            {
                                SHR_IF_ERR_EXIT(dpp_dsig_read
                                                (unit, core_id, block_p, from_p, "LEXEM1", "Key_E", field_key_value,
                                                 5));
                                SHR_IF_ERR_EXIT(dpp_dsig_read
                                                (unit, core_id, block_p, "LEXEM1", from_p, "Hit", &field_key_hit, 1));
                                SHR_IF_ERR_EXIT(dpp_dsig_read
                                                (unit, core_id, block_p, "LEXEM1", from_p, "Result", field_result_value,
                                                 2));
                            }
                            break;
                        }
            ...
            {
                action_size_single = dnx_data_field.map.action_size_large_get(unit);
                PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE_BEFORE);
                /** Take the name for current FG from the SW state. */
                SHR_IF_ERR_EXIT(dnx_field_group_sw.fg_info.name.get(unit, fgs_per_context[fg_id_index], &group_name));
                /**
                    ...
                    switch (Field_key_enum_table[attach_full_info->key_id.id[key_id_iter]].string[0])
                    {
                        case 'C':
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    SHR_IF_ERR_EXIT(dpp_dsig_read
                                                    (unit, core_id, block_p, from_p, "TCAMorDT", "Key_C",
                                                     field_key_value, 5));
                                }
                                SHR_IF_ERR_EXIT(dpp_dsig_read
                                                (unit, core_id, block_p, "DT", from_p, "Hit", &field_key_hit, 1));
                                SHR_IF_ERR_EXIT(dpp_dsig_read
                                                (unit, core_id, block_p, "DT", from_p, "Result", field_result_value,
                                                 4));
                            }
                            break;
                        }
                        case 'H':
                        {
                            ...
                            {
                                SHR_IF_ERR_EXIT(dpp_dsig_read
                                                (unit, core_id, block_p, from_p, "TCAMorDT", "Key_H", field_key_value,
                                                 5));
                                SHR_IF_ERR_EXIT(dpp_dsig_read
                                                (unit, core_id, block_p, "DT", from_p, "Hit", &field_key_hit, 1));
                                SHR_IF_ERR_EXIT(dpp_dsig_read
            ...
            {
                action_size_single = dnx_data_field.tcam.action_size_half_get(unit);
                PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE_BEFORE);
                /** Take the name for current FG from the SW state. */
                SHR_IF_ERR_EXIT(dnx_field_group_sw.fg_info.name.get(unit, fgs_per_context[fg_id_index], &group_name));
                /**
                    ...
                    switch (Field_key_enum_table[attach_full_info->key_id.id[key_id_iter]].string[0])
                    {
                        case 'A':
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    SHR_IF_ERR_EXIT(dpp_dsig_read
                                                    (unit, core_id, block_p, from_p, "TCAMorDE", "Key_A",
                                                     field_key_value, 5));
                                }
                                else
                                {
                                    SHR_IF_ERR_EXIT(dpp_dsig_read
                                                    (unit, core_id, block_p, from_p, "TCAM", "Key_A", field_key_value,
                                                     5));
                                }
                                SHR_IF_ERR_EXIT(dpp_dsig_read
                                                (unit, core_id, block_p, "TCAM", from_p, "Hit_A", &field_key_hit, 1));
                                SHR_IF_ERR_EXIT(dpp_dsig_read
                                                (unit, core_id, block_p, "TCAM", from_p, "Result_A", field_result_value,
                                                 2));
                            }
                            break;
                        }
                        case 'B':
                        {
                            ...
                            else
                            {
                                ...
                                SHR_IF_ERR_EXIT(dpp_dsig_read
                                                (unit, core_id, block_p, "TCAM", from_p, "Result_B", field_result_value,
                                                 2));
                            }
                            break;
                        }
                        case 'F':
                        {
                            ...
                            else
                            {
                                ...
                                SHR_IF_ERR_EXIT(dpp_dsig_read
                                                (unit, core_id, block_p, "TCAM", from_p, "Result_F", field_result_value,
                                                 2));
                            }
                            break;
                        }
            ...
            case DNX_FIELD_GROUP_TYPE_KBP:
            {
                ...
                sal_memset(last_diag_fg_info_p[fg_id_index].kbp_info.kbp_acl_key_value, 0,
                           sizeof(last_diag_fg_info_p[fg_id_index].kbp_info.kbp_acl_key_value));
                sal_memset(last_diag_fg_info_p[fg_id_index].kbp_info.quals_offset_on_master_key, BCM_FIELD_INVALID,
                           sizeof(last_diag_fg_info_p[fg_id_index].kbp_info.quals_offset_on_master_key));
                ...
                SHR_IF_ERR_EXIT(dpp_dsig_read
                                (unit, core_id, block_p, from_p, to_p, "Elk_Lkp_Payload.HitBit", &hit_bit_buff, 1));
                ...
                SHR_IF_ERR_EXIT(diag_dnx_field_last_external_entry_info_get
                                (unit, core_id, fg_id_index, fgs_per_context[fg_id_index],
                                 attach_full_info, last_diag_fg_info_p, &is_entry_found_p, sand_control));
 ...
 * \param [in] mismatch_error_print_p - Indicates if there is any mismatch found during
    ...
    prt_control_t * prt_ctr)
{
    ...
    if (action_valid)
    {
        for (act_val_iter = DNX_DATA_MAX_FIELD_ENTRY_NOF_ACTION_PARAMS_PER_ENTRY - 1; act_val_iter >= 0; act_val_iter--)
        {
            ...
            sal_strncat_s(action_val_exp_string, act_val_buff, sizeof(action_val_exp_string));
        ...
        else
        {
            if (is_action_user_defined)
            {
                SHR_IF_ERR_EXIT(dnx_field_map_dnx_action_signals(unit, field_stage, base_dnx_action, action_signals));
            }
            else
            {
        ...
        else
        {
            ...
            else
            {
                ...
                uint32 entry_payload_action_hw_value = 0;
                ...
                /** Get the current entry action value from the action_result_buff_p. */
                SHR_IF_ERR_EXIT(utilex_bitstream_get_any_field
                ...
                if (is_hit_found)
                {
                    ...
                    else
                    {
                        ...
                        SHR_IF_ERR_EXIT(dnx_field_map_dnx_to_dbal_action(unit, field_stage, dnx_action, &dbal_field));
                        rv = dbal_fields_enum_hw_value_get(unit, dbal_field, entry_payload_action_value,
                                                           &entry_payload_action_hw_value);
                        if (rv != _SHR_E_PARAM)
                        {
                            entry_payload_action_value = entry_payload_action_hw_value;
                        }
                        ...
                        {
                            *mismatch_error_print_p = TRUE;
                            LOG_CLI_EX
                                ("Mismatch between %s action values, result 0x%X, entry payload 0x%X entry hw val payload 0x%X \r\n",
                ...
                for (sig_iter = 0; sig_iter < DNX_DATA_MAX_FIELD_DIAG_NOF_SIGNALS_PER_ACTION; sig_iter++)
                {
                    ...
                    if (rv == _SHR_E_NOT_FOUND)
                    {
                        DIAG_DNX_FIELD_UTILS_STR_INPUT_VERIFY(NULL, "%s: %s\n",
                                                              ...
                                                              [dnx_action_id].signal_name[sig_iter],
                                                              "Could not retrieve value");
                        sal_snprintf(act_val_buff, sizeof(act_val_buff), "%s: %s\n",
                                     dnx_field_map_stage_info[fg_stage].
                                     static_action_info[dnx_action_id].signal_name[sig_iter],
                                     "Could not retrieve value");
                        DIAG_DNX_FIELD_UTILS_STR_INPUT_VERIFY(action_val_string, "%s", act_val_buff);
                        sal_strncat_s(action_val_string, act_val_buff, sizeof(action_val_string));
                    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (qual_action_iter = 0; qual_action_iter < max_num_qual_actions_fems; qual_action_iter++)
    {
        ...
        if (action_left)
        {
            ...
            SHR_IF_ERR_EXIT(diag_dnx_field_last_action_print
                            (unit, is_hit_found, core_id, qual_action_iter, fg_id, context_id, block_p, from_p, to_p,
                             ...
                             action_result_buff, mismatch_error_print_p, prt_ctr));
        }
        else if (fem_left)
        {
            /** Print the FEM actions info. */
            SHR_IF_ERR_EXIT(diag_dnx_field_last_fem_actions_print(unit, is_hit_found, core_id, context_id, fg_id,
                                                                  block_p, from_p, to_p, key_values[key_index],
        ...
        {
            SHR_IF_ERR_EXIT(diag_dnx_field_last_added_efes_action_print(unit, is_hit_found, core_id, fg_id, context_id,
                                                                        block_p, from_p, to_p,
         ...
         (qual_iter < max_num_qual_actions_fems) && (action_iter < max_num_qual_actions_fems);)
    {
        ...
        else if (!qual_left && !action_left && fem_left)
        {
            ...
            SHR_IF_ERR_EXIT(diag_dnx_field_last_fem_actions_print
                            (unit, FALSE, core_id, context_id, fg_id, block_p, from_p, to_p,
        ...
        else if (!qual_left && !action_left && !fem_left && added_efes_left)
        {
            ...
            SHR_IF_ERR_EXIT(diag_dnx_field_last_added_efes_action_print(unit, TRUE, core_id, fg_id, context_id,
                                                                        block_p, from_p, to_p,
                                                                        fg_info_p->group_basic_info.field_stage,
                                                                        attach_full_info_p, key_values[key_index],
                                                                        result_values_p, added_efes_id, prt_ctr));
 ...
 *  presented in the table.
 * \param [in] mismatch_error_print_p - Indicates if there is any mismatch found durring
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (qual_action_iter = 0; qual_action_iter < max_num_qual_actions; qual_action_iter++)
    {
        ...
        if (qual_left)
        {
            ...
            SHR_IF_ERR_EXIT(diag_dnx_field_last_external_qual_print
                            (unit, is_hit_found, kbp_info_p->quals_offset_on_master_key[qual_action_iter],
                             current_key_offset, qual_size, qual_action_iter, kbp_info_p->kbp_acl_key_value, fg_info_p,
                             entry_info_p, prt_ctr));

            current_key_offset += BYTES2BITS(BITS2BYTES(qual_size));
        }
        else
        {
            /**
        ...
        if (action_left)
        {
            ...
            SHR_IF_ERR_EXIT(diag_dnx_field_last_action_print
                            (unit, is_hit_found, core_id, qual_action_iter, fg_id, context_id, block_p, from_p, to_p,
                             NULL, fg_info_p, NULL, entry_info_p, &start_bit, action_result_buff,
                             mismatch_error_print_p, prt_ctr));
        }
        else
        {
            /**
    ...
    sh_sand_control_t * sand_control)
{
        ...
        switch (fg_type)
        {
            ...
            case DNX_FIELD_GROUP_TYPE_DIRECT_TABLE_TCAM:
            {
                ...
                SHR_IF_ERR_EXIT(diag_dnx_field_last_common_fg_key_payload_print
                                (unit, is_hit_found, none_fem_occupied, nof_allocated_fems, nof_allocated_added_efes,
                                 core_id, context_id, fg_id_p[fg_iter], fg_info, attach_full_info, &entry_info, block_p,
                                 from_p, to_p, last_diag_fg_info_p[fg_iter].key_values,
                                 last_diag_fg_info_p[fg_iter].result_values, &do_display_any_fg_info,
                                 &mismatch_error_print, sand_control));
                break;
            }
            case DNX_FIELD_GROUP_TYPE_DIRECT_EXTRACTION:
            {
                SHR_IF_ERR_EXIT(diag_dnx_field_last_de_fg_key_payload_print
                                (unit, none_fem_occupied, nof_allocated_fems, nof_allocated_added_efes, core_id,
                                 ...
                                 last_diag_fg_info_p[fg_iter].key_values, last_diag_fg_info_p[fg_iter].result_values,
            ...
            case DNX_FIELD_GROUP_TYPE_KBP:
            {
                ...
                SHR_IF_ERR_EXIT(diag_dnx_field_last_external_fg_key_payload_print
                                (unit, is_hit_found, core_id, context_id,
                                 ...
                                 &last_diag_fg_info_p[fg_iter].kbp_info, &do_display_any_fg_info, &mismatch_error_print,
                                 sand_control));
#else /* defined(INCLUDE_KBP) */
                LOG_CLI_EX("\r\n"
                           "Field Group Type External should not exit, as KBP is not compiled, and yet it does!! %s%s%s%s\r\n\n",
    ...
    if (mismatch_error_print)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL,
                     "Diag failed on action values mismatch check! Look above for 'Mismatch' prints.\r\n");
    }
        ...
        case DNX_FIELD_STAGE_IPMF3:
        {
            ...
            SHR_IF_ERR_EXIT(diag_dnx_field_last_fg_print
                            (unit, core_id, field_stage, context_id, "IRPP", "IPMF3", "TCAM", fg_performed_lookup,
                             &num_fgs, last_diag_fg_info, sand_control));
            if (DIAG_DNX_FIELD_LAST_IS_SIM == FALSE)
            {
                SHR_IF_ERR_EXIT(diag_dnx_field_last_fg_key_payload_print
...
#if defined(INCLUDE_KBP)
            SHR_IF_ERR_EXIT(diag_dnx_field_last_cs_print
                            (unit, core_id, field_stage, "IRPP", "FWD2", "IPMF1", &context_id, sand_control));
            SHR_IF_ERR_EXIT(diag_dnx_field_last_fg_print
                            (unit, core_id, field_stage, context_id, "IRPP", "FWD2", "IPMF1", fg_performed_lookup,
                             &num_fgs, last_diag_fg_info, sand_control));
            SHR_IF_ERR_EXIT(diag_dnx_field_last_fg_key_payload_print
                            (unit, core_id, context_id, fg_performed_lookup, num_fgs, "IRPP", "FWD2", "IPMF1",
                             last_diag_fg_info, sand_control));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (DIAG_DNX_FIELD_LAST_IS_SIM)
    {
        ...
        {
            LOG_CLI_EX("\r\n" "Invalid CORE was given!!! CORE values for Cmodel, can be '0' or 'all'!%s%s%s%s\r\n\n",
                       EMPTY, EMPTY, EMPTY, EMPTY);
            SHR_EXIT();
        }
    }
    ...
    for (; core_index <= num_of_cores; core_index++)
    {
        ...
        for (stage_index = stage_lower; stage_index <= stage_upper; stage_index++)
        {
            ...
            if (DIAG_DNX_FIELD_LAST_IS_SIM == TRUE &&
                (field_stage == DNX_FIELD_STAGE_EPMF || field_stage == DNX_FIELD_STAGE_EXTERNAL))
            {
                LOG_CLI_EX("\r\n" "Last Info diag on Cmodel is not supported for %s stage!!!%s%s%s\r\n\n",
                           dnx_field_stage_text(unit, field_stage), EMPTY, EMPTY, EMPTY);
                continue;
            }

            SHR_IF_ERR_EXIT(diag_dnx_field_last_info_print(unit, field_stage, core_index, sand_control));
