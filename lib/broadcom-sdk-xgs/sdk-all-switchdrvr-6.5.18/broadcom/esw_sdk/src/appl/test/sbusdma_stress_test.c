...
cmicx_continuous_dma_write(void *void_param)
{
    ...
    soc_reg_field_set(params->unit, CMIC_CMC1_SBUSDMA_CH1_DESC_START_ADDRESS_LOr, &rval,
                      ADDRESSf, HOST_ADDR_LO_CMICX(soc_cm_l2p(params->unit, dma_desc)));
    ...
    soc_reg_field_set(params->unit, CMIC_CMC1_SBUSDMA_CH1_DESC_START_ADDRESS_HIr, &rval,
                      ADDRESSf, HOST_ADDR_HI_CMICX(soc_cm_l2p(params->unit, dma_desc)));
    if (soc_cm_get_bus_type(params->unit) & SOC_PCI_DEV_TYPE) {
...
cmicx_continuous_dma_read(void *void_param)
{
    ...
    rval = soc_pci_read(params->unit, CMIC_CMCx_SBUSDMA_CHy_DESCADDR_LO(cmc,ch));
    soc_reg_field_set(params->unit, CMIC_CMC1_SBUSDMA_CH0_DESC_START_ADDRESS_LOr, &rval,
                      ADDRESSf, HOST_ADDR_LO_CMICX(soc_cm_l2p(params->unit, dma_desc)));
    soc_pci_write(params->unit, CMIC_CMCx_SBUSDMA_CHy_DESCADDR_LO(cmc,ch), rval);

    rval = soc_pci_read(params->unit, CMIC_CMCx_SBUSDMA_CHy_DESCADDR_HI(cmc,ch));
    soc_reg_field_set(params->unit, CMIC_CMC1_SBUSDMA_CH0_DESC_START_ADDRESS_HIr, &rval,
                      ADDRESSf, HOST_ADDR_HI_CMICX(soc_cm_l2p(params->unit, dma_desc)));
    if (soc_cm_get_bus_type(params->unit) & SOC_PCI_DEV_TYPE) {
        rval |= CMIC_PCIE_SO_OFFSET;
    }
