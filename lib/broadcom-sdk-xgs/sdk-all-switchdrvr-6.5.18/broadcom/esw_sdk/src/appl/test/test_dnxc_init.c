...
test_dnxc_init_test(int unit, args_t *a, void *p)
{
        ...
        if (test_params->appl_init_params.warmboot) {
            if(dnxc_sw_state_is_warmboot_supported_get(unit) == FALSE) {
                ...
                if (!test_params->appl_init_params.no_dump) {
                    ...
                    sal_strncat(pre_reboot_file_name, "/sw_state_pre_reboot.txt", sizeof(pre_reboot_file_name) - strlen(pre_reboot_file_name) - 1);
                    sal_strncat(post_reboot_file_name, "/sw_state_post_reboot.txt", sizeof(post_reboot_file_name) - strlen(post_reboot_file_name) - 1);

                    cli_out("%d: Dumping SW State Data Block to file - sw_state_pre_reboot.txt.\n", unit);
    ...
    test_params->appl_init_params.loop_id = i;
        ...
        if (test_params->appl_init_params.warmboot && !test_params->appl_init_params.no_dump) {
            cli_out("%d: Dumping SW State Data Block to file - sw_state_post_reboot.txt.\n", unit);
