...
/** see appl_ref_field_wa_init.h*/
shr_error_e
appl_dnx_field_wa_learn_and_limit_cb(
    int unit,
    int *dynamic_flags)
{
    ...
    if (((dnx_data_field.init.wa_j1_learning_get(unit)) &&
         (dnx_data_headers.system_headers.system_headers_mode_get(unit) ==
          dnx_data_headers.system_headers.system_headers_mode_jericho_get(unit)))
        || (dnx_data_field.init.wa_learn_limit_get(unit)))
    {
        *dynamic_flags = 0;
    }
    else
...
/** see appl_ref_field_wa_init.h*/
shr_error_e
appl_dnx_field_wa_j1_php_cb(
    int unit,
    int *dynamic_flags)
{

    if (((dnx_data_field.init.wa_j1_php_get(unit)) &&
         (dnx_data_headers.system_headers.system_headers_mode_get(unit) ==
          dnx_data_headers.system_headers.system_headers_mode_jericho_get(unit))))
    {
        *dynamic_flags = 0;
...
/** see appl_ref_field_wa_init.h*/
shr_error_e
appl_dnx_field_wa_ipmc_inlif_cb(
    int unit,
    int *dynamic_flags)
{

    if ((dnx_data_field.init.wa_jr1_ipmc_inlif_get(unit)) &&
        (dnx_data_headers.system_headers.system_headers_mode_get(unit) ==
         dnx_data_headers.system_headers.system_headers_mode_jericho_get(unit)))
    {
        *dynamic_flags = 0;
    ...
    bcm_gport_t trap_do_nothing)
{
    ...
    bcm_rx_trap_t *field_wa_trap_tcp_udp_types_p = NULL;

    SHR_FUNC_INIT_VARS(unit);
    SHR_ALLOC(field_wa_trap_tcp_udp_types_p, sizeof(*field_wa_trap_tcp_udp_types_p) * tcp_udp_nof_traps,
              "field_wa_trap_tcp_udp_types_p", "%s%s%s", EMPTY, EMPTY, EMPTY);

    field_wa_trap_tcp_udp_types_p[0] = bcmRxTrapTcpSnFlagsZero;
    field_wa_trap_tcp_udp_types_p[1] = bcmRxTrapTcpSnZeroFlagsSet;
    field_wa_trap_tcp_udp_types_p[2] = bcmRxTrapTcpSynFin;
    field_wa_trap_tcp_udp_types_p[3] = bcmRxTrapTcpEqualPorts;
    field_wa_trap_tcp_udp_types_p[4] = bcmRxTrapTcpFragmentIncompleteHeader;
    field_wa_trap_tcp_udp_types_p[5] = bcmRxTrapTcpFragmentOffsetLt8;
    field_wa_trap_tcp_udp_types_p[6] = bcmRxTrapUdpEqualPorts;
    ...
    for (ii = 0; ii < tcp_udp_nof_traps; ii++)
    {
        ...
        SHR_IF_ERR_EXIT(bcm_rx_trap_type_get(unit, 0, field_wa_trap_tcp_udp_types_p[ii], &trap_code));
        ...
        if (field_wa_trap_tcp_udp_types_p[ii] == bcmRxTrapUdpEqualPorts)
        {
...
exit:
    SHR_FREE(field_wa_trap_tcp_udp_types_p);
    SHR_FUNC_EXIT;
    ...
    int unit)
{
     ...
     * */
    p_id.presel_id = dnx_data_field.preselector.default_j1_same_port_wa_presel_id_epmf_get(unit);
    ...
    int unit)
{
...

    {
        ...
        SHR_IF_ERR_EXIT(appl_dnx_field_wa_fec_dest_on_context(unit, context_id_w_o_roo_with_rif, FALSE));
        SHR_IF_ERR_EXIT(appl_dnx_field_wa_fec_dest_on_context(unit, context_id_w_o_roo_no_rif, FALSE));
    }
...
shr_error_e
appl_dnx_field_wa_ipmc_inlif_init(
    int unit)
{
...
shr_error_e
appl_dnx_field_wa_jr1_php_init(
    int unit)
{
...
static shr_error_e
appl_dnx_field_j1_learn_wa_ingress(
    int unit)
{
    ...
    uint32 recycle_port = LEARN_WA_RCY_PORT;
     ...
     */
    SHR_IF_ERR_EXIT(bcm_port_control_set(unit, recycle_port, bcmPortControlDistributeLearnMessageMode, TRUE));
     ...
     * */
    p_id.presel_id = dnx_data_field.preselector.default_j1_learn_wa_presel_id_1st_pass_ipmf1_get(unit);;
    ...
    */
    ...
    SHR_IF_ERR_EXIT(dbal_fields_parent_field32_value_set
                    (unit, DBAL_FIELD_DESTINATION, DBAL_FIELD_PORT_ID, &recycle_port, &recycle_port_destination));
    ...
    /*
     * For EEI+FEC, it just go with 1 PASS
     * destination is from ITMH, etc MCID.
     */
    SHR_IF_ERR_EXIT(dbal_fields_parent_field32_value_set
    ...
    for (ii = 0; ii < BCM_FIELD_NUMBER_OF_MAPPING_BITS_PER_FEM; ii++)
    {
        ...
        fem_action_info.fem_extraction[1].output_bit[ii].forced_value = ((olp_mc_destination >> ii) & 0x1);
    }
     ...
     * create the context selection based on OLP port profile
     * */
    p_id.presel_id = dnx_data_field.preselector.default_j1_learn_wa_presel_id_2nd_pass_ipmf1_get(unit);;
    p_id.stage = bcmFieldStageIngressPMF1;
    p_data.entry_valid = TRUE;
    ...
    bcm_field_context_t context_id)
{
     ...
     * They are planned to handle all possible combinations of J1 compatible learning WA limit WA.
     *
     ...
     * OLP_0     XXX              XXX               XXX        OLP_1       OLP_1       OLP_1
     */
    uint32 learn_bug_rule_action[] = { OLP_PORT_1, OLP_PORT_1, OLP_PORT_0, OLP_PORT_0, OLP_PORT_1 };
    uint32 limit_bug_rule_action[] = { OLP_PORT_0, OLP_PORT_1, OLP_PORT_0, OLP_PORT_1, OLP_PORT_1 };
    uint32 limit_and_learn_bug_rule_action[] = { OLP_PORT_3, OLP_PORT_1, OLP_PORT_0, OLP_PORT_2, OLP_PORT_1 };
     ...
     */
    if ((dnx_data_field.init.wa_j1_learning_get(unit)) &&
        (is_j1_mode) && (dnx_data_field.init.wa_learn_limit_get(unit)))
    {
        rule_action = limit_and_learn_bug_rule_action;
        cmd_type_insert = 1;
    }
    else if ((dnx_data_field.init.wa_j1_learning_get(unit)) && (is_j1_mode))
    {
        rule_action = learn_bug_rule_action;
        cmd_type_insert = 1;
    }
    else if (dnx_data_field.init.wa_learn_limit_get(unit))
    {
    ...
    entry_info.entry_qual[3].mask[0] = 0x7;
    entry_info.entry_action[0].type = fg_info.action_types[0];
...
*/
static shr_error_e
appl_dnx_field_j1_same_intf_wa_egress(
    int unit,
    bcm_field_context_t context_id)
{
    bcm_field_ace_format_info_t ace_format_info;
...
*/
static shr_error_e
appl_dnx_field_j1_learn_and_limit_wa_egress(
    int unit)
{
    void *dest_char;
    bcm_field_context_t context_id;
    ...
    p_id.presel_id = dnx_data_field.preselector.default_learn_limit_wa_presel_id_epmf_get(unit);;
    p_id.stage = bcmFieldStageEgress;
    p_data.entry_valid = TRUE;
    p_data.context_id = context_id;
    p_data.nof_qualifiers = 1;
    ...
    if ((dnx_data_field.init.wa_j1_learning_get(unit)) &&
        (dnx_data_headers.system_headers.system_headers_mode_get(unit) ==
         dnx_data_headers.system_headers.system_headers_mode_jericho_get(unit)))
    {
        SHR_IF_ERR_EXIT(appl_dnx_field_j1_same_intf_wa_egress(unit, context_id));
    }
...
/** see appl_ref_field_wa_init.h*/
shr_error_e
appl_dnx_field_learn_and_limit_wa_init(
    int unit)
{
    ...
    if (num_of_ports == 0)
    {
        SHR_EXIT_WITH_LOG(_SHR_E_NONE, "OLP port should be configured in the device for the field WA%s%s%s.\n", EMPTY,
                          EMPTY, EMPTY);
    }
    ...
    if ((dnx_data_field.init.wa_j1_learning_get(unit)) &&
        (dnx_data_headers.system_headers.system_headers_mode_get(unit) ==
         dnx_data_headers.system_headers.system_headers_mode_jericho_get(unit)))
    {
        if (!BCM_PBMP_MEMBER(port_config.rcy, LEARN_WA_RCY_PORT))
        {
            SHR_EXIT_WITH_LOG(_SHR_E_NONE,
                              "RCY port should be configured in the device for the field WA in J1 mode%s%s%s.\n", EMPTY,
                              EMPTY, EMPTY);
        }
        SHR_IF_ERR_EXIT(appl_dnx_field_j1_learn_wa_ingress(unit));
    }
    SHR_IF_ERR_EXIT(appl_dnx_field_j1_learn_and_limit_wa_egress(unit));
