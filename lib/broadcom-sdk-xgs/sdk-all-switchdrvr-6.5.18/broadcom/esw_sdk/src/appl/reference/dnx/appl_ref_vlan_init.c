    ...
    int unit)
{
    ...
    BCM_PBMP_ITER(pbmp_pp_eth_candidates, logical_port)
    {
        ...
        if (flags &
            (BCM_PORT_ADD_KBP_PORT | BCM_PORT_ADD_STIF_PORT | BCM_PORT_ADD_CROSS_CONNECT | BCM_PORT_ADD_FLEXE_PHY))
        {
            continue;
        }
        if ((port_interface_info.interface == BCM_PORT_IF_ERP) || (port_interface_info.interface == BCM_PORT_IF_SCH)
