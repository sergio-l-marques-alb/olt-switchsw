...
#define TCAM_BANK_FOR_OAM_IDENTIFICATION    12
    ...
    bcm_gport_t cpu_port)
{
    ...
    SHR_IF_ERR_EXIT(appl_dnx_oam_single_punt_config(unit, bcmRxTrapOampPuntNextGoodPacket, DEBUG_TRAP, cpu_port));
    ...
    oam_traps_t * oam_traps)
{
    ...
    if (dnx_data_oam.hw_bug.feature_get(unit, dnx_data_oam_hw_bug_oam_offset_supported))
    {
         ...
         * 1. In Jr2-A0 device due to HW BUG this is done through the PMF (not trap)
    ...
    int unit)
{
    ...
    if (indexed_mode_enable == IN_LIF_PROFILE_PWE_INDEXED_MODE_NOF_VALUES)
    {
        ...
        bcm_field_tcam_bank_info_t_init(&add_info);
        add_info.fg_id = BCM_FIELD_ID_INVALID;
        add_info.pp_app = bcmFieldAppDbOamIdentification;
        add_info.nof_banks = 1;
        add_info.tcam_bank_ids[0] = TCAM_BANK_FOR_OAM_IDENTIFICATION;
        rv = bcm_field_tcam_bank_add(unit, 0, &add_info);
        if (BCM_FAILURE(rv))
        {
            LOG_ERROR(BSL_LS_APPL_COMMON,
                      (BSL_META_U
                       (unit, "bcm_field_tcam_bank_add for oam identification failed. Error:%d (%s)\n"), rv,
                       bcm_errmsg(rv)));
            return rv;
        }
