 ...
 */
int
dnxf_interrupt_appl_log_defaults_set(
    int unit)
{
    int rv, i;
    bcm_switch_event_control_t event;
    const dnxc_data_table_info_t *table_info;
    const dnxf_data_intr_general_disable_print_on_init_t *intr_disable_print;
    ...
    table_info = dnxf_data_intr.general.disable_print_on_init_info_get(unit);
    ...
    {
        intr_disable_print = dnxf_data_intr.general.disable_print_on_init_get(unit, i);
 ...
 */
int
dnxf_interrupt_appl_mask_defaults_set(
    int unit)
{
    int rv, i;
    bcm_switch_event_control_t event;
    const dnxc_data_table_info_t *table_info;
    const dnxf_data_intr_general_active_on_init_t *intr_active;
    const dnxf_data_intr_general_disable_on_init_t *intr_disable;
    uint32 nof_interrupts;
     ...
     */
    table_info = dnxf_data_intr.general.disable_on_init_info_get(unit);
    if (table_info->key_size[0] > nof_interrupts)
    {
    ...
    for (i = 0; i < table_info->key_size[0]; i++)
    {
        intr_disable = dnxf_data_intr.general.disable_on_init_get(unit, i);
     ...
     */
    table_info = dnxf_data_intr.general.active_on_init_info_get(unit);
    if (table_info->key_size[0] > nof_interrupts)
    {
    ...
    for (i = 0; i < table_info->key_size[0]; i++)
    {
        intr_active = dnxf_data_intr.general.active_on_init_get(unit, i);
