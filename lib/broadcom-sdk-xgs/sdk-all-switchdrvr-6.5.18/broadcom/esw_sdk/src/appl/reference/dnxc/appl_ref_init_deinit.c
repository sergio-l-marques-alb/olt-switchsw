...
int sal_alloc_debug_nof_allocs_get(
    void);
void sal_alloc_debug_last_allocs_get(
    alloc_info * array,
    int nof_last_allocs);
    ...
    appl_dnxc_init_param_t * params)
{
    ...
    LOG_INFO_EX(BSL_LOG_MODULE, "%d: APPL Init pre list get\n%s%s%s", unit, EMPTY, EMPTY, EMPTY);
    /** get the list to run */
    SHR_IF_ERR_EXIT(appl_dnxc_step_list_get(unit, &appl_steps));

    LOG_INFO_EX(BSL_LOG_MODULE, "%d: APPL Init pre params set\n%s%s%s", unit, EMPTY, EMPTY, EMPTY);
    /** Get init params. */
    if (params != NULL)
    {
     ...
     * Init if needed
     */
    LOG_INFO_EX(BSL_LOG_MODULE, "%d: APPL Init pre seq run\n%s%s%s", unit, EMPTY, EMPTY, EMPTY);
