    ...
    *mc_group_id = OLP_MC_ID;
    ...
    for (device_idx = 0; device_idx < sys_params->nof_devices; device_idx++)
    {
        ...
        SHR_IF_ERR_EXIT(appl_sand_device_to_sysport_convert(unit, device_idx,
                                                            ...
                                                            olp_port, &olp_sys_port));
        BCM_GPORT_SYSTEM_PORT_ID_SET(gport, olp_sys_port);

        rep_array[port_index++].port = gport;
    }

    SHR_IF_ERR_EXIT(bcm_multicast_add
                    (unit, *mc_group_id, BCM_MULTICAST_WITH_ID | BCM_MULTICAST_INGRESS_GROUP, sys_params->nof_devices,
                     rep_array));
