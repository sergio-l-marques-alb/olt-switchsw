...
#include "appl_ref_eedb_access_wa_init.h"
...
static shr_error_e
j2b0_j2c_q2a_j2p_skip(
    int unit,
    int *dynamic_flags)
{
    if (SOC_IS_J2C(unit) || SOC_IS_JERICHO2_B(unit) || SOC_IS_Q2A(unit) || SOC_IS_J2P(unit))
    {
        *dynamic_flags = UTILEX_SEQ_STEP_F_SKIP;
    }
    else
...
static const appl_dnxc_init_step_t appl_dnx_ref_apps_steps[] = {
    ...
    {APPL_DNX_STEP_SYS_PARAMS,             "System Params",              "sys_params",       appl_dnx_sys_params_parse,                      NULL,                             NULL,                           STANDALONE,                        NULL},
...
/** APPL_DNX_STEP_EEDB_ACCESS must be before APPL_DNX_STEP_SYS_PORTS as it adds ports which need to be handled by subsequent steps */
    {APPL_DNX_STEP_EEDB_ACCESS,            "EEDB Access WA",             "eedb_access_wa",   appl_dnx_eedb_access_wa_init,                   NULL,                             appl_dnx_eedb_access_wa_cb,     DEF_PROP_EN | WB_SKIP,             NULL},
    ...
    {APPL_DNX_STEP_KBP,                    "KBP",                        "kbp",              appl_dnx_kbp_init,                              appl_dnx_kbp_deinit,               NULL,                          DEF_PROP_EN,                       NULL},
#endif
    ...
    {APPL_DNX_STEP_COMPRESSED_LAYER_TYPE,  "Compressed layer type",      "clt",              appl_dnx_mtu_compressed_layer_type_init,        NULL,                              NULL,                          DEF_PROP_EN | WB_SKIP,             NULL},
    ...
    {APPL_DNX_STEP_ITMH,                   "Field ITMH",                 "field_itmh",       appl_dnx_field_itmh_init,                       NULL,                              NULL,                          DEF_PROP_EN | WB_SKIP,             NULL},
    {APPL_DNX_STEP_ITMH_PPH,               "Field ITMH_PPH",             "field_itmh_pph",   appl_dnx_field_itmh_pph_init,                   NULL,                              NULL,                          DEF_PROP_EN | WB_SKIP | TIME_PER_DEV, NULL},
    ...
    {APPL_DNX_STEP_STACKING,               "Field Stacking",             "field_stacking",   appl_dnx_field_stacking_init,                   NULL,                              j2b0_j2c_q2a_j2p_skip,         DEF_PROP_DIS | WB_SKIP,            NULL},
    ...
    {APPL_DNX_STEP_INTERRUPT,              "Interrupt Init",             "intr_init",        appl_dnxc_interrupt_init,                       appl_dnxc_interrupt_deinit,        j2p_skip,                      DEF_PROP_EN,                       NULL},
    {APPL_DNX_STEP_DRAM,                   "Dram cb registration",       "dram_cb_reg",      appl_dnx_dram_cb_register,                      appl_dnx_dram_cb_unregister,       j2p_skip,                      DEF_PROP_EN,                       NULL}, /* this step should run after WB */
...
static const appl_dnxc_init_step_t appl_dnx_ref_field_wa_apps_steps[] = {
    /*STEP_ID                              NAME                         SUFFIX                 INIT_FUNC                                DEINIT_FUNC    FLAG_CB                                  STEP_FLAGS                           SUB_LIST*/
    {APPL_DNX_STEP_FP_WA_FEC_DEST,         "Field FEC Dest WA",         "fec_dest_wa_init",    appl_dnx_field_wa_fec_dest_init,         NULL,          appl_dnx_field_wa_fec_dest_cb,           DEF_PROP_EN | WB_SKIP,               NULL},
    {APPL_DNX_STEP_FP_WA_OAM_INDEX,        "Field OAM FWD Index WA",    "oam_fwd_idx_wa_init", appl_dnx_field_wa_oam_layer_index_init,  NULL,          appl_dnx_field_wa_oam_layer_index_cb,    DEF_PROP_EN | WB_SKIP | TIME_PER_DEV,   NULL},
    {APPL_DNX_STEP_FP_WA_TRAP_L4,          "Field L4 Trap WA",          "L4_trap_wa_init",     appl_dnx_field_wa_trap_l4_init,          NULL,          appl_dnx_field_wa_trap_l4_cb,            DEF_PROP_DIS| WB_SKIP,               NULL},
    {APPL_DNX_STEP_FP_WA_OAM_STAT,         "Field OAM Stat WA",         "oam_stat_wa_init",    appl_dnx_field_wa_oam_stat_init,         NULL,          appl_dnx_field_wa_oam_stat_cb,           DEF_PROP_EN | WB_SKIP,               NULL},
    {APPL_DNX_STEP_FP_WA_FLOW_ID,          "Field Flow Id WA",          "flow_id_wa_init",     appl_dnx_field_wa_flow_id_init,          NULL,          appl_dnx_field_wa_flow_id_cb,            DEF_PROP_EN | WB_SKIP,               NULL},
    {APPL_DNX_STEP_FP_WA_ROO,              "Field ROO WA",              "roo_wa_init",         appl_dnx_field_wa_roo_init,              NULL,          appl_dnx_field_wa_roo_cb,                DEF_PROP_EN | WB_SKIP,               NULL},
    {APPL_DNX_STEP_FP_WA_IPMC_INLIF,       "Field IPMC INLIF WA",       "ipmc_inlif_wa_init",  appl_dnx_field_wa_ipmc_inlif_init,       NULL,          appl_dnx_field_wa_ipmc_inlif_cb,         DEF_PROP_EN | WB_SKIP,               NULL},
    {APPL_DNX_STEP_FP_WA_J1_SAME_PORT,     "Field J1 Same port WA",     "j1_same_p_init",      appl_dnx_field_wa_j1_same_port_init,     NULL,          appl_dnx_field_wa_j1_same_port_cb,       DEF_PROP_EN | WB_SKIP,               NULL},
    {APPL_DNX_STEP_FP_WA_LEARN_AND_LIMIT,  "Field Learn and Limit WA",  "learn_and_limit_init",appl_dnx_field_learn_and_limit_wa_init,  NULL,          appl_dnx_field_wa_learn_and_limit_cb,    DEF_PROP_EN | WB_SKIP,               NULL},
    {APPL_DNX_STEP_FP_WA_J1_PHP,           "Field J1 PHP WA",           "j1 php_init",         appl_dnx_field_wa_jr1_php_init,          NULL,          appl_dnx_field_wa_j1_php_cb,             DEF_PROP_EN | WB_SKIP,               NULL},
    {APPL_DNX_STEP_LAST_STEP,              NULL,                        NULL,                  NULL,                                    NULL,          NULL,                                    0,                                   NULL} /* Last step must be all NULLs*/
...
static const appl_dnxc_init_step_t appl_dnx_init_steps[] = {
    ...
    {APPL_DNX_STEP_FP_WA_APPS,  "Field WA Applications", "field_wa",     NULL,                    NULL,                       bare_metal_skip,  DEF_PROP_EN | TIME_PER_DEV,           appl_dnx_ref_field_wa_apps_steps},
    {APPL_DNX_STEP_INIT_DONE,   "Init Done",             "init_done",    appl_dnx_init_done_init, appl_dnx_init_done_deinit,  NULL,             DEF_PROP_EN,                       NULL}, /* this step must be last */
    {APPL_DNX_STEP_LAST_STEP,    NULL,                    NULL,          NULL,                    NULL,                       NULL,             0,                                 NULL}  /* Last step must be all NULLs*/
};
