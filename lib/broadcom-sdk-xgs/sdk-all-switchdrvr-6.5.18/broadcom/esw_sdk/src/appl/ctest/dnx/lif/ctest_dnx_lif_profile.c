    ...
    sh_sand_control_t * sand_control)
{
    ...
    SHR_IF_ERR_EXIT(mdb_db_infos.capacity.get(unit, DBAL_PHYSICAL_TABLE_INLIF_1, &max_ref_count));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    /** It is used one dummy reference in etpp outlif smart template manager for the vxlan range */
    if (ref_count != dnx_data_lif.out_lif.nof_local_out_lifs_get(unit) + 1)
    {
    ...
    for (profiles_counter = DNX_OUT_LIF_PROFILE_DEFAULT; profiles_counter < NOF_OUT_LIF_PROFILES; profiles_counter++)
    {
        ...
        /** Using one dummy reference for default IP_TUNNEL profile */
        if (profiles_counter == DNX_OUT_LIF_PROFILE_DEFAULT
            && ref_count != dnx_data_lif.out_lif.nof_local_out_lifs_get(unit) - 4 + 1)
        {
            SHR_ERR_EXIT(_SHR_E_FAIL, "wrong ref_count for out_lif_profile = %d,ref_count = %d,should be %d\n",
                         profiles_counter, ref_count, dnx_data_lif.out_lif.nof_local_out_lifs_get(unit) - 4 + 1);
        }
    ...
    for (profiles_counter = DNX_OUT_LIF_PROFILE_DEFAULT; profiles_counter < NOF_OUT_LIF_PROFILES; profiles_counter++)
    {
        ...
        /** It is used one dummy reference in etpp outlif smart template manager for the vxlan range */
        if (profiles_counter == DNX_OUT_LIF_PROFILE_DEFAULT
            && ref_count != dnx_data_lif.out_lif.nof_local_out_lifs_get(unit) + 1)
        {
            SHR_ERR_EXIT(_SHR_E_FAIL, "wrong ref_count for out_lif_profile = %d,ref_count = %d,should be %d\n",
                         profiles_counter, ref_count, dnx_data_lif.out_lif.nof_local_out_lifs_get(unit) + 1);
        }
