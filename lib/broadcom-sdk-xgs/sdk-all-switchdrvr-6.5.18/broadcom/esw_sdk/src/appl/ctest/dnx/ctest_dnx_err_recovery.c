    ...
    int unit)
{
    return ((0 == dnx_err_recovery_manager[unit].is_suppressed_counter)
            ...
            (dnx_err_recovery_common_flag_is_on
             (unit, DNXC_ROLLBACK_JOURNAL_TYPE_ROLLBACK, DNX_ERR_RECOVERY_COMMON_IS_ON_NOT_INVALIDATED))
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (!dnx_err_recovery_common_flag_is_on
        (unit, DNXC_ROLLBACK_JOURNAL_TYPE_ROLLBACK, DNX_ERR_RECOVERY_COMMON_IS_ON_INITIALIZED))
    {
