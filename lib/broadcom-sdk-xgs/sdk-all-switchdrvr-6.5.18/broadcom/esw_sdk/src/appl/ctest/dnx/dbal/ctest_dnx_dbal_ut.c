...
static shr_error_e
dnx_dbal_ut_hl_validations_permission(
    int unit,
    ...
    multi_res_info_t * result_type_set)
{
    ...
    rv = dbal_db_init_field_hw_entity_validate(unit, table_id, curr_l2p_field, is_reg, FALSE);
    ...
    dbal_tables_e table_id)
{
    ...
    PRT_TITLE_SET("Failed Tables with invalid field");
            ...
            for (access_type = DBAL_HL_ACCESS_MEMORY; access_type <= DBAL_HL_ACCESS_REGISTER; access_type++)
            {
                if (hl_mapping->l2p_hl_info[access_type].num_of_access_fields > 0)
                {
                    ...
                    for (hl_iter = 0; hl_iter < l2p_hl_info->num_of_access_fields; hl_iter++)
                    {
                        ...
                        rv = dnx_dbal_ut_hl_validations_permission(unit,
                                                                   table, curr_l2p_field, is_reg, result_type_set);

                        if (rv != _SHR_E_NONE)
                        {
                            PRT_ROW_ADD(PRT_ROW_SEP_NONE);
                            PRT_CELL_SET("%s", table->table_name);
                            PRT_CELL_SET("%s", dbal_field_to_string(unit, curr_l2p_field->field_id));
                            fail_counter++;
                            break;
                        }
                    }
...
shr_error_e
dnx_dbal_ut_hl_validations_hw_entity(
    int unit,
    dbal_tables_e table_id)
{
    CONST dbal_logical_table_t *table;
            ...
            for (access_type = DBAL_HL_ACCESS_MEMORY; access_type <= DBAL_HL_ACCESS_REGISTER; access_type++)
            {
                if (hl_mapping->l2p_hl_info[access_type].num_of_access_fields > 0)
                {
                    ...
                    for (hl_iter = 0; hl_iter < l2p_hl_info->num_of_access_fields; hl_iter++)
                    {
                        ...
                        /** validation field hw entity for the mem/reg */
                        rv = dbal_db_init_field_hw_entity_validate(unit, table_iter, curr_l2p_field, is_reg, FALSE);
                        if (rv != _SHR_E_NONE)
                        {
                            PRT_ROW_ADD(PRT_ROW_SEP_NONE);
                            PRT_CELL_SET("%s", table->table_name);
    ...
    dbal_tables_e table_id)
{
    ...
    for (ii = 0; ii < table->nof_key_fields; ii++)
    {
        ...
        {
            dbal_entry_key_field32_set(unit, entry_handle_id, table->keys_info[ii].field_id, 0);
        }
        else
        {
    ...
    dbal_tables_e table_id)
{
    ...
    if (table_id == DBAL_TABLE_INGRESS_PTC_PORT)
    {
        ...
        value_field_id_1 = DBAL_FIELD_VTT2_PTC_CS_PROFILE;
        value_field_id_2 = DBAL_FIELD_VTT3_PTC_CS_PROFILE;
    ...
    dbal_tables_e table_id)
{
     ...
     */
    SHR_IF_ERR_EXIT(dnx_dbal_ut_struct_arr_prefix_field(unit, DBAL_FIELD_FEC_DESTINATION_MPLS_POP, DBAL_FIELD_EEI));
    ...
    dbal_tables_e table_id)
{
    ...
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "only none direct tables is supportd fopr this test \n");
    }
