...
#define CTEST_SW_STATE_ER_CLEANUP_START(_unit)                                     \
do {                                                                               \
    if (dnx_rollback_journal_start(_unit) != _SHR_E_NONE)                          \
    {                                                                              \
        SHR_ERR_EXIT(_SHR_E_FAIL, "Could not start err recovery transaction \r\n");\
    }                                                                              \
} while(0)

#define CTEST_SW_STATE_ER_CLEANUP_END(_unit)                                           \
do {                                                                                   \
    if (dnx_rollback_journal_end(_unit, TRUE) != _SHR_E_NONE)                          \
    {                                                                                  \
...
#else
#define CTEST_SW_STATE_ER_CLEANUP_START(unit)
#define CTEST_SW_STATE_ER_CLEANUP_END(unit)
#endif
...
#define CTEST_SW_STATE_FUNC_INIT_VARS(unit)                       \
    ...
    CTEST_SW_STATE_ER_CLEANUP_START(unit)

#define CTEST_SW_STATE_FUNC_EXIT                                  \
    CTEST_SW_STATE_ER_CLEANUP_END(unit);                          \
    ...
    sh_sand_control_t * sand_control)
{
    ...
    LOG_CLI((BSL_META("Check if mutex has already been created\n")));
    SHR_IF_ERR_EXIT(example.mutex_test.is_created(unit, &is_created));
    if (is_created)
    {
        LOG_CLI((BSL_META("Unit: %d, Mutex is already created!\n"), unit));
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Test FAILED\n");
    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    LOG_CLI((BSL_META("Check if semaphore has already been created\n")));
    SHR_IF_ERR_EXIT(example.sem_test.is_created(unit, &is_created));
    if (is_created)
    {
        LOG_CLI((BSL_META("Unit: %d, Semaphore is already created!\n"), unit));
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Test FAILED\n");
    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    LOG_CLI((BSL_META("Checking what will happen if you try to free it.\n")));
    SHR_IF_ERR_EXIT(example.dynamic_array_zero_size.free(unit));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SHR_IF_ERR_EXIT(example.htb.delete_all(unit));
...
sh_sand_cmd_t dnx_swstate_test_cmds[] = {
    ...
    {"semaphore", sw_state_sem_test_cmd, NULL, NULL, &sw_state_sem_test_man, NULL, NULL, CTEST_PRECOMMIT},
