    ...
    int expected_value)
{
    ...
    /** The c-model doesn't reproduce events */
    ...
    int unit)
{
    ...
    BCM_OAM_EVENT_TYPE_CLEAR_ALL(event);
    BCM_OAM_EVENT_TYPE_SET(event, bcmBFDEventEndpointTimeout);
    ...
    BCM_OAM_EVENT_TYPE_CLEAR_ALL(event);
    BCM_OAM_EVENT_TYPE_SET(event, bcmBFDEventEndpointTimein);
    SHR_CLI_EXIT_IF_ERR(bcm_bfd_event_register(unit, event, diag_dnx_oamp_test_bfd_cb, (void *) 1),
                        "Test failed! Register of timein event failed.");
    BCM_OAM_EVENT_TYPE_CLEAR_ALL(event);
    BCM_OAM_EVENT_TYPE_SET(event, bcmBFDEventStateChange);
    SHR_CLI_EXIT_IF_ERR(bcm_bfd_event_register(unit, event, diag_dnx_oamp_test_bfd_cb, (void *) 1),
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (sematic_scale_validation == TEST_TYPE_SEMANTIC)
    {
        LOG_CLI((BSL_META("\nC-model is limited for scale testing, aborting the run..\n")));
        SHR_FUNC_EXIT;
    }
#endif
    ...
    */
    for (endpoint_id = 0; endpoint_id < global_parameters.num_entries - nof_short_entries; endpoint_id++)
    {
