    ...
    rhlist_t * test_list)
{
    ...
    {
        SHR_CLI_EXIT(_SHR_E_NONE, "Error recovery is not supported for J2P.\n");
    }
    ...
    while (is_found == UTILEX_SEQ_NEXT_STEP_FOUND)
    {
        ...
        sal_snprintf(test_name, SH_DNXC_INIT_STEP_NAME_SIZE, "step_%s_id_%d", step->step_name, step->step_id);
...
/** Callback function for dnx/dnxf time thresholds per step*/
static shr_error_e
dnxc_time_thresh_cb(
    int unit,
    int step_id,
    ...
    sh_sand_control_t * sand_control)
{
    ...
    /** run the test */
    SHR_CLI_EXIT_IF_ERR(utilex_seq_time_test_run(unit, &seq, first_step_id, &test_info),
    ...
    if (test_info.fail_count > 0)
    {
        ...
        PRT_COLUMN_ADD("Time step took [microseconds]");
        ...
        for (ii = 0; ii < test_info.fail_count; ii++)
        {
            ...
            PRT_CELL_SET("%s", test_info.fail_step_name[ii]);
            PRT_CELL_SET("%d", test_info.fail_step_id[ii]);
            PRT_CELL_SET("%d", test_info.fail_step_thresh[ii]);
            PRT_CELL_SET("%d", test_info.fail_time[ii]);
        }
...
sh_sand_cmd_t dnxc_infra_init_test_cmds[] = {
    ...
    {"TIME", diag_dnxc_infra_init_time_test_cmd, NULL,
     ...
     NULL, NULL, SH_CMD_SKIP_EXEC, diag_dnxc_infra_init_time_tests_creation},
#endif
