    ...
    sh_sand_control_t * sand_control)
{
     ...
     */
    {
        ...
        sal_memset(&action, 0, sizeof(action));
        action.flags = BCM_MPLS_RANGE_ACTION_EVPN_IML;
        for (index = 0; index < num_ranges; range_index++, index++)
        {
            if (range_index >= num_ranges)
            {
                range_index = 0;
            }
            current_label = first_label + (range_index * num_labels_in_range);
            label_low = current_label;
            current_label += (num_labels_in_range - 1);
            label_high = current_label;
            current_label++;
     ...
     */
    {
         ...
         * Create the ranges
         */
        for (range_index = 0; range_index < (num_ranges / 2); range_index++)
        {
            label_low = current_label;
            current_label += (num_labels_in_range - 1);
            label_high = current_label;
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (test_i = 0; test_i < nof_tests; test_i++)
    {
        ...
        BCM_L3_ITF_SET(eg_label_w_id.tunnel_id, BCM_L3_ITF_TYPE_LIF, 0x1100);
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (test_i = 0; test_i < nof_tests; test_i++)
    {
        ...
        for (vpn = 0; vpn <= 33; vpn += 33)
        {
            ...
            tunnel_w_id.vpn = vpn;
            BCM_GPORT_TUNNEL_ID_SET(tunnel_w_id.tunnel_id, 0x1100);
