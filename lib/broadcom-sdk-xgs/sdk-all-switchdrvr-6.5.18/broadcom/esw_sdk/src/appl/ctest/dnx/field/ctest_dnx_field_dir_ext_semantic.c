...
sh_sand_option_t Sh_dnx_field_dir_ext_sem_options[] = {
    ...
    {CTEST_DNX_DIR_EXT_SEMANTIC_OPTION_CLEAN,      SAL_FIELD_TYPE_BOOL,    "Will test perform HW and SW-state clean-up or not",     "Yes"},
...
sh_sand_man_t Sh_dnx_dir_ext_sem_man = {
    ...
    "ctest field dir_ext_sem type=<BCM | DNX> stage=<IPMF2 | IPMF3> count=<integer> clean=<yes | no> count=<integer>",
    "type=DNX de_type=old"
};
/**
...
{ DNX_FIELD_IPMF1_QUAL_CONTEXT_KEY_GEN_VAR,    DNX_FIELD_IPMF1_QUAL_GLOB_IN_LIF_0,   DNX_FIELD_IPMF1_QUAL_FWD_DOMAIN,     DNX_FIELD_IPMF1_QUAL_IN_TTL}, /**IPMF2*/
{ DNX_FIELD_IPMF3_QUAL_CONTEXT_KEY_GEN_VAR,    DNX_FIELD_IPMF3_QUAL_GLOB_IN_LIF_0,   DNX_FIELD_IPMF3_QUAL_FWD_DOMAIN,     DNX_FIELD_IPMF3_QUAL_IN_TTL}, /**IPMF3*/
};
/**
    ...
    dnx_field_group_t * fg_id_p)
{
    ...
    if (test_flag == CTEST_DNX_DIR_EXT_POSITIVE_TEST)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_field_group_context_attach
                        (unit, 0, *fg_id_p, DNX_FIELD_CONTEXT_ID_DEFAULT(unit), attach_info_p));
    }
    /**
    ...
    dnx_field_group_t * fg_id_p)
{
    ...
    for (fg_key_size = CTEST_DNX_DIR_EXT_FG_FIRST; fg_key_size < CTEST_DNX_DIR_EXT_NOF_FG; fg_key_size++)
    {
        ...
        LOG_INFO_EX(BSL_LOG_MODULE, "%s(), line %d, Stage: %s Create FG %d with DE key!\r\n",
                    __FUNCTION__, __LINE__, dnx_field_stage_e_get_name(field_stage), fg_key_size);
         ...
         * Delete the FG
         */
        CTEST_DNX_FIELD_UTIL_ERR(dnx_field_group_context_detach(unit, *fg_id_p, DNX_FIELD_CONTEXT_ID_DEFAULT(unit)));
    ...
    dnx_field_group_t * fg_id_p)
{
         ...
         negative_case_iterator < CTEST_DNX_DIR_EXT_NOF_TESTS; negative_case_iterator++)
    {
        ...
        LOG_INFO_EX(BSL_LOG_MODULE, "%s(), line %d, Stage: %s NEGATIVE TESTING! CASE: %d \r\n",
                    __FUNCTION__, __LINE__, dnx_field_stage_e_get_name(field_stage), negative_case_iterator);
        nof_iterations = 0;
        ...
        switch (negative_case_iterator)
        {
            ...
            case CTEST_DNX_DIR_EXT_NEGATIVE_DIFFERENT_STAGE_QUAL:
            {
                negative_test_quals[0] =
                    DNX_QUAL(DNX_FIELD_QUAL_CLASS_META, DNX_FIELD_STAGE_EPMF, DNX_FIELD_EPMF_QUAL_LB_KEY);
                break;
            }
            /**
        ...
        */
        ...
        rv = ctest_dnx_dir_ext_sem_create_fg_and_attach(unit,
                                                        ...
                                                        fg_info_p, attach_info_p, fg_id_p);
       /**
...
{
    unsigned int dir_ext_semantic_test_type, dir_ext_semantic_test_stage, dir_ext_semantic_test_clean;
    ...
    */
    ...
    SHR_FUNC_INIT_VARS(unit);
    /**
     * Give the default value to dir_ext_semantic_test_clean
     */
    dir_ext_semantic_test_clean = 0;
    /**
     ...
     */
    SH_SAND_GET_BOOL(CTEST_DNX_DIR_EXT_SEMANTIC_OPTION_CLEAN, dir_ext_semantic_test_clean);
    ...
    if (dir_ext_semantic_test_type == 0)
    {
        LOG_DEBUG_EX(BSL_LOG_MODULE,
                     ...
                     __FUNCTION__, __LINE__, "BCM", "dnx_field_dir_ext_semantic");
    }
...
exit:
    if (dir_ext_semantic_test_clean == TRUE)
    {
