...
/**     test name                       test enum                    test flags              test callback         */
    ...
    {"hl_validations",              HL_VALIDATIONS,               CTEST_PRECOMMIT, dnx_dbal_ut_hl_validations,               UT_ALL_DEVICES       , {DBAL_UT_NO_TABLE}},
    {"hl_validations_hw_entity",    HL_VALIDATIONS_HW_ENT,        CTEST_PRECOMMIT, dnx_dbal_ut_hl_validations_hw_entity,     UT_ALL_DEVICES       , {DBAL_UT_NO_TABLE}},
    ...
    {"dpc_core_any",                DPC_CORE_ANY,                 CTEST_PRECOMMIT, dnx_dbal_ut_core_any_dpc,                 UT_DUAL_CORE_DEVICES , {DBAL_TABLE_INGRESS_PTC_PORT, DBAL_TABLE_EXAMPLE_SW_DIRECT, DBAL_UT_FILL_TABLE_FOR_2}},
    ...
    {"iterator_hit_bit_rule",       ITERATOR_HIT_BIT_RULE,        CTEST_PRECOMMIT, dnx_dbal_ut_iterator_hit_bit_rule,        UT_ALL_DEVICES       , {DBAL_TABLE_ING_VSI_INFO_DB, DBAL_UT_FILL_TABLE_FOR_1}},
    ...
    {"commit_forced",               COMMIT_FORCED,                CTEST_PRECOMMIT, dnx_dbal_ut_commit_forced,                UT_ALL_DEVICES       , {DBAL_TABLE_LOCAL_DPC_INLIF_INFO_SW, DBAL_TABLE_INGRESS_OAM_ACC_MEP_DB, DBAL_TABLE_IPV6_MULTICAST_PRIVATE_LPM_FORWARD, DBAL_UT_FILL_TABLE_FOR_3}},
    ...
    {"err_code_non_direct",         ERR_CODE_NON_DIRECT,          CTEST_PRECOMMIT, dnx_dbal_ut_non_direct_error_codes,       UT_ALL_DEVICES       , {DBAL_TABLE_BIER_BFER_TERMINATION, DBAL_TABLE_BRIDGE_IPV4_MULTICAST_SOURCE_SPECIFIC_IVL, DBAL_TABLE_BFD_ENDPOINT_SW_INFO, DBAL_UT_FILL_TABLE_FOR_3}},
    ...
    {"field_allocator",             FIELD_ALLOCATOR,              CTEST_POSTCOMMIT, dnx_dbal_ut_field_allocator,              UT_ALL_DEVICES       , {DBAL_TABLE_QOS_INGRESS_PHB_PROFILE_INFO, DBAL_TABLE_QOS_INGRESS_PHB_PROFILE_TO_INTERNAL_MAP_ID, DBAL_TABLE_EXAMPLE_SW_ALLOC, DBAL_UT_FILL_TABLE_FOR_3}},
    ...
    {"mact_iterator",               MACT_ITERATOR,                CTEST_PRECOMMIT, dnx_dbal_ut_mact_iterator,                UT_ALL_DEVICES       , {DBAL_TABLE_FWD_MACT, DBAL_UT_FILL_TABLE_FOR_1}},
...
static dbal_tables_e dbal_performance_test_list_sw[] = {
    ...
    DBAL_TABLE_LOCAL_OUT_LIF_INFO_SW,
    ...
    dbal_ltt_flags_e test_specific_flags)
{
    ...
    for (ii = iter_start_pos; ii < iter_end_pos; ii++)
    {
        ...
        if (filtered_tables)
        {
            /** check if t he table should be skipped */
                 ...
                 || _SHR_IS_FLAG_SET(ltt_input.flags, LTT_FLAGS_SNAPSHOT_NEGATIVE))
        {
            ...
            curr_res =
                (_SHR_E_NONE == err_rec_test_ltt_res) ? diag_dbal_test_logical_table(unit, ii,
                                                                                     ltt_input.flags) : _SHR_E_NONE;
        }
        else
        {
            curr_res = diag_dbal_test_logical_table(unit, ii, ltt_input.flags);
        }
...
cmd_dnx_dbal_ctest_ltt_full(
    ...
    sh_sand_control_t * sand_control)
{
    dbal_ltt_flags_e flags = LTT_FLAGS_FULL_ITERATOR_TABLE_CLEAR_TEST | LTT_FLAGS_MEASUREMERNTS;
    ...
    int *entries_per_sec)
{
    ...
    if (dbal_physical_table_is_eedb(table->physical_db_id[0]) || (table_id == DBAL_TABLE_ETM_PP_DESCRIPTOR_EXPANSION))
    {
    ...
    for (entry_iter = 0; entry_iter < nof_entries; entry_iter++)
    {
        ...
        for (field_iter = 0; field_iter < table->multi_res_info[0].nof_result_fields; field_iter++)
        {
            if (table->multi_res_info[0].results_info[field_iter].permission != DBAL_PERMISSION_READONLY)
            {
                ...
                {
                    utilex_ll_timer_set("result field set", timers_group, timer_idx[RESULT_FIELD_SET]);
                    ...
                    utilex_ll_timer_stop(timers_group, timer_idx[RESULT_FIELD_SET]);
                }
                else
                {
                    if (SHR_BITGET
...

    {
        ...
        LOG_CLI((BSL_META
                 ("\n### Tests completed, Commit Results: total time - %d, nof entries - %d, per entry - %.2f[us] Rate - %.0f[E/s] ###\n\n"),
...
static sh_sand_option_t dnx_dbal_perofrmance_options[] = {
    {"TaBLe", SAL_FIELD_TYPE_STR, "DBAL logical table name", ""},
    {"EntryNum", SAL_FIELD_TYPE_UINT32, "Number of entries", "16384"},
    {"ACCess", SAL_FIELD_TYPE_STR, "DBAL logical table access", ""},
    {"update", SAL_FIELD_TYPE_BOOL, "if update = true, will perofrm commit_update (for non-direct tables only)", "No"},
    {NULL}
};
...
sh_sand_cmd_t dnx_dbal_test_cmds[] = {
    ...
    {"Performance",      cmd_dnx_dbal_performance,    NULL, dnx_dbal_perofrmance_options, &dnx_dbal_performance_man, NULL, NULL, CTEST_POSTCOMMIT, NULL},
