    ...
    dnx_kbp_combo_field_indication_e field_indication)
{
    ...
    RHDATA_ITERATOR(iter_node, fields_node, "Field")
    {
        ...
        /** Set the field size */
        RHDATA_GET_INT_STOP(iter_node, "BitSize", tmp_xml_int_reader);
    ...
    dnx_kbp_combo_group_info_t * group_info)
{
    ...
    DNX_KBP_COMBO_API_EXECUTE_NO_ERR_EXIT(dbal_tables_table_create
                                          (unit, DBAL_ACCESS_METHOD_KBP, DBAL_TABLE_TYPE_TCAM_BY_ID,
    ...
    dnx_kbp_combo_env_info_t * env_info)
{
    ...
    for (index = 0; index < env_info->nof_groups; index++)
    {
        if (env_info->group_info[index].status == DNX_KBP_COMBO_GROUP_STATUS_VALID)
        {
            ...
            SHR_IF_ERR_EXIT(kbp_mngr_db_id_by_name_get(unit, env_info->group_info[index].name, &db_id));
    ...
    dnx_kbp_combo_opcode_info_t * opcode_info)
{
    ...
    SHR_IF_ERR_EXIT(kbp_mngr_opcode_master_key_segments_add(unit, opcode_id, nof_overlay_fields, overlay_field));
    ...
    int nof_lookups)
{
    ...
    SHR_IF_ERR_EXIT(kbp_mngr_opcode_master_key_segments_add(unit, opcode_id, nof_new_mk_segments, new_mk_segment));
    ...
    dnx_kbp_combo_entry_get_expectation_e * expectation)
{
    ...
    char expectation_strings[3][16] = { "NOT_FOUND", "FOUND", "VALIDATE" };
    dnx_kbp_combo_execute_action_e expectations[3] = {
    ...
    RHDATA_GET_STR_STOP(step_node, "Expectation", expectation_string_get);
    ...
    void *entries_root)
{
    ...
    DNX_KBP_COMBO_LOG(DNX_KBP_COMBO_LOG_ENTRY, "Entry add ID %d ", id);
    ...
    if (action == DNX_KBP_COMBO_TEST_ACTION_EXECUTE)
    {
        ...
        /** Set entry access ID and priority if the table type is TCAM */
        if (table->table_type == DBAL_TABLE_TYPE_TCAM || table->table_type == DBAL_TABLE_TYPE_TCAM_BY_ID)
        {
            int priority;
    ...
    void *entries_root)
{
    ...
    char expect_str[DNX_KBP_COMBO_ENTRY_GET_EXPECTATION_NOF][16] = { "INVALID", "NOT_FOUND", "FOUND", "VALIDATE" };
    ...
    DNX_KBP_COMBO_LOG(DNX_KBP_COMBO_LOG_ENTRY, "Expect %s\n", expect_str[expectation]);
    ...
    if (action == DNX_KBP_COMBO_TEST_ACTION_EXECUTE)
    {
        ...
        /** Set entry access ID if the table type is TCAM */
        if (table->table_type == DBAL_TABLE_TYPE_TCAM || table->table_type == DBAL_TABLE_TYPE_TCAM_BY_ID)
        {
            SHR_IF_ERR_EXIT(dnx_kbp_combo_entry_access_id_parse_action(unit, entry_node, entry_handle_id));
        }
        ...
        {
            SHR_IF_ERR_EXIT(dnx_kbp_combo_entry_get_validate(unit, entry_node, entry_handle_id, dbal_table_id));
        }
        else
        {
    ...
    dnx_kbp_combo_execute_action_e * action)
{
    ...
    char action_strings[4][8] = { "ADD", "GET", "DELETE", "SEARCH" };
    dnx_kbp_combo_execute_action_e actions[4] = {
    ...
    switch (execute_action)
    {
        case DNX_KBP_COMBO_EXECUTE_ACTION_ENTRY_ADD:
        {
            DNX_KBP_COMBO_API_EXECUTE_NO_ERR_EXIT(dnx_kbp_combo_entry_execute_add_action
                                                  (unit, action, step_node, root_nodes_info->entries_root), test_info,
                                                  "failed entry add",
        ...
        case DNX_KBP_COMBO_EXECUTE_ACTION_SEARCH:
        {
            DNX_KBP_COMBO_API_EXECUTE_NO_ERR_EXIT(dnx_kbp_combo_search_execute_action
                                                  (unit, action, step_node, root_nodes_info->searches_root), test_info,
                                                  "failed search", DNX_KBP_COMBO_TEST_STATUS_FAILED_SEARCH);
            break;
        }
        default:
        {
