    ...
    ctest_soc_property_t * ctest_soc_property_p)
{
    ...
    static char appl_enable_field_wa_str[RHNAME_MAX_SIZE];
     ...
     * applied.
     */
    sal_snprintf(appl_enable_field_wa_str, RHNAME_MAX_SIZE, "%s.%d", "appl_enable_field_wa", unit);
    ctest_soc_property_p[ii].property = appl_enable_field_wa_str;
    ...
    sh_sand_control_t * sand_control)
{
    ...
    }
    {
        ...
        SHR_SET_CURRENT_ERR(sand_signal_verify
                            (unit, core_id_dst, "ETPP", NULL, "ETparser", "FTMH_Base", (uint32 *) expected_value, size,
                             NULL, (char *) result, 0));
        SHR_SET_CURRENT_ERR(_SHR_E_NONE);
    ...
    sh_sand_control_t * sand_control)
{
     ...
     */
    SHR_IF_ERR_EXIT(dnx_tdm_load_soc_properties
                    (unit, NULL, tdm_mode_str, flexe_mode_str, ucode_port_name_all_src, ucode_port_name_src,
                     ucode_port_value_src,
                     ucode_port_name_dst, ucode_port_value_dst, ucode_port_name_all_dst, COUNTOF(ucode_port_name_mcast),
                     ((ucode_port_name_mcast[0] == 0) ? NULL : ucode_port_name_mcast), ucode_port_value_mcast,
                     ucode_port_name_all_mcast, stream_multicast_prefix, COUNTOF(ctest_soc_property),
                     &ctest_soc_property[0]));
    SHR_IF_ERR_EXIT(ctest_dnxc_set_soc_properties(unit, ctest_soc_property, &ctest_soc_set_h));
    if (dst_present == FALSE)
    {
        /*
        ...
        if (use_optimized_ftmh)
        {
            if (dst_present != FALSE)
            {
                 ...
                 */
                {
                    ...
                    SHR_IF_ERR_EXIT(utilex_bitstream_u8_ms_byte_first_get_field
                                    (unit, ftmh_image, ms_ftmh_stamped_offset, nof_bits_ftmh_stamped_value, &pp_dsp));
                    nof_bits_ftmh_stamped_value =
                        dnx_data_headers.optimized_ftmh.user_defined_offset_get(unit) -
                        dnx_data_headers.optimized_ftmh.fap_id_offset_get(unit);
                    ms_ftmh_stamped_offset =
                        nof_bits_ftmh_container - (ls_offset_ftmh_header +
                                                   ...
                                                   nof_bits_ftmh_stamped_value);
                    SHR_IF_ERR_EXIT(utilex_bitstream_u8_ms_byte_first_get_field
                                    (unit, ftmh_image, ms_ftmh_stamped_offset, nof_bits_ftmh_stamped_value, &fap_id));
                    LOG_CLI_EX("\r\n" "FAP ID,PP DSP\r\n" "Value: %d %d %s %s\r\n", fap_id, pp_dsp, EMPTY, EMPTY);
                    if (pp_dsp != dst_port)
                    {
                        SHR_ERR_EXIT(_SHR_E_FAIL,
 ...
 *   Currently, this test is carried out for device only (not on cmodel)
 * \param [in] unit             - unit id
    ...
    rhlist_t * invoke_list)
{
    ...
    {
        /*
         * Reach here if this is 'cmodel'. In that case, return
    ...
    sh_sand_control_t * sand_control)
{
    ...
    /** send traffic */
    {
        ...
        /** send packets and verify result*/
        {
            ...
            uint32 expected_value;
            uint8 result_str[(DNX_DATA_MAX_HEADERS_FTMH_BASE_HEADER_SIZE * (SAL_UINT8_NOF_BITS / SAL_NIBBLE_NOF_BITS)) +
                             sizeof(uint32) + 1];
