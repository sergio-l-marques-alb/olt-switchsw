    ...
    sh_sand_control_t * sand_control)
{
    ...
    else if (test_type == 1)
    {
        ...
        SHR_IF_ERR_EXIT(bcm_field_context_param_set(unit, 0, bcmFieldStageIngressPMF1,
                                                    DNX_FIELD_CONTEXT_ID_DEFAULT(unit), &context_params));
        ...
        fg_info_in.dnx_actions[1] =
            DNX_ACTION(DNX_FIELD_ACTION_CLASS_STATIC, DNX_FIELD_STAGE_IPMF1, DBAL_ENUM_FVAL_IPMF1_ACTION_USER_HEADER_1);
        ...
        SHR_IF_ERR_EXIT(dnx_field_group_context_attach
                        (unit, 0, grp_id, DNX_FIELD_CONTEXT_ID_DEFAULT(unit), &attach_info));
...
exit:
    if (test_type && udh_test_clean)
    {
        bcm_field_context_param_info_t context_params;
        CTEST_DNX_FIELD_UTIL_ERR(dnx_field_group_context_detach(unit, grp_id, DNX_FIELD_CONTEXT_ID_DEFAULT(unit)));
        ...
        /** Reset default context system header to the default system header */
        context_params.param_type = bcmFieldContextParamTypeSystemHeaderProfile;
        context_params.param_val = bcmFieldSystemHeaderProfileFtmhTshPphUdh;
        SHR_IF_ERR_EXIT(bcm_field_context_param_set(unit, 0, bcmFieldStageIngressPMF1,
                                                    DNX_FIELD_CONTEXT_ID_DEFAULT(unit), &context_params));
