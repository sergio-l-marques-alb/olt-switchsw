    ...
    rhlist_t * test_list)
{
    ...
    SH_SAND_CMD_ITERATOR_COND(sh_sand_cmd, sh_sand_device_a)
    {
    ...
    int flags)
{
     ...
     * Add test only if gold result exists, Check existence and leave peacefully if there is no gold
     */
    if ((sh_sand_cmd->flags & SH_CMD_NO_XML_VERIFY) || sh_sand_gold_exists(unit, sh_sand_cmd->cmd_only) != _SHR_E_NONE)
    {
        SHR_EXIT();
    }
    /*
    ...
    rhlist_t * test_list)
{
    ...
    SHR_CLI_EXIT_IF_ERR(sh_sand_cmd_traverse(unit, NULL, sh_sand_device_a, shell_verify_test_add,
                                             (void *) test_list, 0), "");
