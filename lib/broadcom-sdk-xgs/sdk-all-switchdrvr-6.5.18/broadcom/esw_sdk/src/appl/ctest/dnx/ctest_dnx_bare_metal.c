    ...
    char *format_name)
{
    ...
    uint32 local_lif = 0;
    int global_out_lif_int = 0;
    lif_mngr_local_outlif_str_info_t outlif_info;

    /** Allocate Local lif*/
    global_out_lif_int = *global_out_lif;
    outlif_info.outlif_phase_name = "EXPLICIT_2";
    outlif_info.table_name = table_name;
    outlif_info.result_type_name = format_name;
    rv = dnx_cint_lif_lib_allocate(unit, with_id, &global_out_lif_int, NULL, &outlif_info);
    if (rv)
    {
        printf("Error! In \"dnx_cint_lif_lib_allocate\". with_id=%d, global_out_lif=%d, table=%s, format=%s\n",
               with_id, *global_out_lif, table_name, format_name);
        return 0xFFFFFFFF;
    }
    *global_out_lif = global_out_lif_int;
    ...
    int unit)
{
    ...
    /** EEDB ETH Encapsulation - for Host*/
    {
        ...
        if (rv == 0xFFFFFFFF)
        {
            printf("Error! In \"bare_metal_add_eedb_entry\"\n");
            return rv;
        }
        else
        {
...
{
    SHR_FUNC_INIT_VARS(unit);
    if (sal_strcmp("BRIDGE_ROUTER_IPV4_UNICAST_ROUTE_TABLE", table_name) == 0)
    {
        ...
        table_def->key_fields[0] = "BRIDGE_ROUTER_VRF";
        table_def->key_fields[1] = "BRIDGE_ROUTER_IPV4_DIP";
        ...
        table_def->result_fields[0] = "BRIDGE_ROUTER_FEC";
        table_def->result_type = "BRIDGE_ROUTER_FEC_FORMAT";
        table_def->table_type = "LPM";
    }
    else if (sal_strcmp("BRIDGE_ROUTER_IPV4_UNICAST_HOST_TABLE", table_name) == 0)
    {
    ...
    else if (sal_strcmp("BRIDGE_ROUTER_ETH_BRIDGE_TABLE", table_name) == 0)
    {
        ...
        table_def->key_fields[0] = "BRIDGE_ROUTER_VSI";
        table_def->key_fields[1] = "BRIDGE_ROUTER_L2_MACDA";
        table_def->key_is_default[0] = 1;
        ...
        table_def->result_fields[0] = "BRIDGE_ROUTER_DESTINATION";
        table_def->result_fields[1] = "BRIDGE_ROUTER_OUTLIF";
        table_def->result_type = "BRIDGE_ROUTER_DESTINATION_OUTLIF_FORMAT";
        table_def->table_type = "LEM";
    }
    else if (sal_strcmp("BRIDGE_ROUTER_PORT_VLAN_CLASSIFICATION_TABLE", table_name) == 0)
    {
        ...
        table_def->key_fields[0] = "BRIDGE_ROUTER_IN_PORT";
        table_def->key_fields[1] = "BRIDGE_ROUTER_VID";
        table_def->nof_result_fields = 1;
        table_def->result_fields = sal_alloc(sizeof(char) * 18, "Port vlan table result");
        table_def->result_fields[0] = "BRIDGE_ROUTER_VSI";
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (is_image == 0)
    {
        printf("Expected image for performance test is Bridge_Router\n");
        SHR_EXIT();
    }
    ...
    for (idx = 0; idx < nof_entries; idx++)
    {
        ...
        dnx_dbal_entry_handle_take(unit, table_name, &entry_handle_id);
        /** Fill in table keys. */
        if (sal_strcmp(table_def.table_type, "ISEM") != 0)
        {
            for (field_idx = 0; field_idx < table_def.nof_key_fields; field_idx++)
            {
                dnx_dbal_entry_key_field_arr32_set(unit, entry_handle_id, table_def.key_fields[field_idx],
                                                   table_def.key_is_default ? &default_val : &field_value);
            }
        }
        else
        {
