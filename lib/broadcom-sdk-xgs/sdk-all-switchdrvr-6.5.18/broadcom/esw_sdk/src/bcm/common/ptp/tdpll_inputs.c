    ...
    bcm_tdpll_input_clock_l1mux_t *l1mux)
{
    ...
    if (SOC_IS_MONTEREY(unit)) {
        ...
        rv = _bcm_esw_synce_phy_lane_get_port(0, INPUT_CLOCK(clock_index).l1mux.port, 1, &lport);
        LOG_DEBUG(BSL_LS_BCM_PTP,
                (BSL_META_U(unit, "bcm_common_tdpll_input_clock_l1mux_get: laneadjusted[%d] logical_port[%d]\n"), INPUT_CLOCK(clock_index).l1mux.port, lport));
        l1mux->port = lport;
        return BCM_E_NONE;
    }
#endif
    ...
    bcm_tdpll_input_clock_l1mux_t *l1mux)
{
    ...
    if (SOC_IS_MONTEREY(unit)) {
        ...
        int port = l1mux->port;
        rv = _bcm_esw_synce_phy_port_get_lane(0, port, 1, NULL, &port);
        LOG_DEBUG(BSL_LS_BCM_PTP,
                    (BSL_META_U(unit, "bcm_tdpll_input_clock_l1mux_set: logicalport[%d] laneadjusted[%d] \n"), l1mux->port, port));
        l1mux->port = port;
    }
#endif
