    ...
    int ts0_ts1_combined_mode_en, int uC)
{
    ...
    if (soc_feature(unit, soc_feature_cmicx)) {
        ...
        start_msg64.s.subclass = MOS_MSG_SUBCLASS_TS0_TS1_COMBINED_MODE_EN;
        start_msg64.s.len = 0;
        start_msg64.s.data = bcm_htonl(ts0_ts1_combined_mode_en);

        rc = soc_cmic_uc_msg64_send(unit, uC, &start_msg64, timeout_usec);
    } else
#endif
    {
    ...
    int ptp_cf_update_mode, int uC)
{
    ...
    if (soc_feature(unit, soc_feature_cmicx)) {
        start_msg64.s.mclass = MOS_MSG_CLASS_1588;
        start_msg64.s.subclass = MOS_MSG_SUBCLASS_PTP_CF_SW_UPDATE_EN;
        start_msg64.s.len = 0;
        start_msg64.s.data = bcm_htonl(ptp_cf_update_mode);
