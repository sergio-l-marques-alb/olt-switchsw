...
#if defined(BCM_MONTEREY_SUPPORT) && defined(INCLUDE_GDPLL)
void _bcm_common_tdpll_output_clock_portattach(int unit, int clock_index, bcm_port_t port)
{
    int synceOutClock = 0;
    int attached = 0;
    int tdpllout_synce_index = clock_index - BCM_TDPLL_OUTPUT_CLOCK_NUM_BROADSYNC;

    /* Check if this port is attached to any of tdpll synce out clocks
     * logical ports are from 1 to N
     */
    for (synceOutClock=0; synceOutClock < BCM_TDPLL_OUTPUT_CLOCK_NUM_SYNCE; synceOutClock++) {
        /* Skip the check on this synce clock index */
        if (synceOutClock == tdpllout_synce_index)
            continue;

        if (tdpllout_gdpll_state.tdpll_output_synceport[synceOutClock][port-1] == 1) {
            attached = 1;
            break;
        }
    }
    ...
    if (!attached) {
        tdpllout_gdpll_state.tdpll_output_synceport[tdpllout_synce_index][port-1] = 1;
    }
}
#endif
    ...
    if (clock_index < BCM_TDPLL_OUTPUT_CLOCK_NUM_BROADSYNC) {
        ...
        BCM_PBMP_ITER(pbmp, port) {
            ...
            if(!IS_CPRI_PORT(unit, port)) {
                _bcm_common_tdpll_output_clock_portattach(unit, clock_index, port);
            }
            else {
            }
        }
        ...
        /* Traverse through all the ports */
        for (port = 1; port <= BCM_GDPLL_NUM_PORTS; port++) {
            /* Check if this port is part of this clock_id */
            if (tdpllout_gdpll_state.tdpll_output_synceport[clock_index - BCM_TDPLL_OUTPUT_CLOCK_NUM_BROADSYNC][port-1] == 1) {
                ...
                cli_out(" tdpll_output_clock: Output clock is SyncE: clock_index:%d, port:%d TXPIClock:%d divisor:%d\n",
                   clock_index, port, frequency, frequency/(OUTPUT_CLOCK(clock_index).frequency.tsevent * 2));

                if (frequency == 0) {
                    continue;
                }

                /* Create the channel on the port if valid frequency and event rate */
                if (frequency && OUTPUT_CLOCK(clock_index).frequency.tsevent) {
                    /* Feedback config */
                    gdpll_chan.event_fb.port            = port;
                    gdpll_chan.event_fb.event_divisor   = frequency/(OUTPUT_CLOCK(clock_index).frequency.tsevent * 2);
                    ...
                    result = bcm_gdpll_chan_create (unit, flags, &gdpll_chan, &gdpllChan);
                    if (BCM_FAILURE(result)) {
                        cli_out(" bcm_common_tdpll_output_clock_create: Error!! gdpll chan create failed\n");
                    } else {
                        rv = _bcm_esw_gdpll_tdpllinstance_bind(unit, gdpllChan, clock_index);
