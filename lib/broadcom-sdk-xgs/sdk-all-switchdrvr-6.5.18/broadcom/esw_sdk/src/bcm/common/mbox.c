...
_bcm_mbox_bs_wb_prep(int unit)
{
    ...
    mos_msg64_data_t wb_msg64, rcv64;
#endif
    ...
    if (soc_feature(unit, soc_feature_cmicx)) {
        wb_msg64.s.mclass     = MOS_MSG_CLASS_BS;
        wb_msg64.s.subclass   = MOS_MSG_SUBCLASS_MBOX_WB_PREP;
        rv = soc_cmic_uc_msg64_send(unit, c, &wb_msg64, timeout_usec);
    } else
#endif
    {
...
#if defined(BCM_CMICX_SUPPORT)
        if (soc_feature(unit, soc_feature_cmicx)) {
            rv = soc_cmic_uc_msg64_receive(unit, c, MOS_MSG_CLASS_BS,
                                         &rcv64, timeout_usec);
            rlen = rcv64.s.len;
        } else
#endif
...
#if defined(BCM_CMICX_SUPPORT)
   if (soc_feature(unit, soc_feature_cmicx)) {
        msg64.s.mclass = MOS_MSG_CLASS_BS;
        msg64.s.subclass = MOS_MSG_SUBCLASS_PTP_COSQ_PORT;
        msg64.s.len = 0;
        msg64.s.data = bcm_htonl(cosq_port);
        rv = soc_cmic_uc_msg64_send(unit, c, &msg64, timeout_usec);
    } else
#endif
    {
...
#if defined(BCM_CMICX_SUPPORT)
            if (soc_feature(unit, soc_feature_cmicx)) {
                start_msg64.s.mclass = MOS_MSG_CLASS_BS;
                start_msg64.s.subclass = MOS_MSG_SUBCLASS_MBOX_CONFIG;
                _shr_uint16_write((uint8*)(&(start_msg64.s.len)), (uint16) appl_type);
                start_msg64.s.data = soc_htonl(PTR_TO_INT(buf_paddr));
                rv = soc_cmic_uc_msg64_send(unit, c, &start_msg64, timeout_usec);
            } else
#endif
            {
...
#if defined(BCM_CMICX_SUPPORT)
        if (soc_feature(unit, soc_feature_cmicx)) {
            rv = soc_cmic_uc_msg64_send(unit, mbox_info.unit_state[unit].core_num,
                                        &uc_msg64, 1000000);
        } else
#endif
        {
...
#if defined(BCM_CMICX_SUPPORT)
    if (soc_feature(unit, soc_feature_cmicx)) {
        sal_memset(&uc_msg64, 0, sizeof(uc_msg64));
        uc_msg64.s.mclass = MOS_MSG_CLASS_BS;
        uc_msg64.s.subclass = MOS_MSG_SUBCLASS_MBOX_CMDRESP;
        uc_msg64.s.len = message_len;
        uc_msg64.s.data = 0;
        rv = soc_cmic_uc_msg64_send(unit, mbox_info.unit_state[unit].core_num,
                                    &uc_msg64, 1000000);
    } else
