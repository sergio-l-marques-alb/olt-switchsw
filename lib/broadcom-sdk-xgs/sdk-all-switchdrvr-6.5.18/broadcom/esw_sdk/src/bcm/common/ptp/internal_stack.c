    ...
    for (c = (SOC_INFO(unit).num_ucs - 1); c >= 0; c--) {
        ...
        if (SOC_E_NONE == result){
            ...
            if (soc_feature(unit, soc_feature_cmicx)) {
                start_msg64.s.mclass = MOS_MSG_CLASS_1588;
                start_msg64.s.subclass = MOS_MSG_SUBCLASS_MBOX_CONFIG;
                start_msg64.s.len = 0;
                start_msg64.s.data = bcm_htonl(PTR_TO_INT(buf_paddr));

                if (BCM_FAILURE(rv = soc_cmic_uc_msg64_send(unit, c, &start_msg64, timeout_usec))) {
                    SOC_CMIC_UCDBG_LOG_ADD((unit, _bcmCmicUcDbgLogHostPtp,
                        "Error in Initializing PTP application\n"));
                    PTP_ERROR_FUNC("soc_cmic_uc_msg64_send()");
                }
            } else
#endif
            {
