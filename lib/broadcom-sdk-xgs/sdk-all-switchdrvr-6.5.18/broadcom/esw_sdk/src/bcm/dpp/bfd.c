...
/*
 * In case Rx Special TTL for BFD over MPLS is enabled.
 * 1. Packet walk:
 *    For rx BFDoMpls with TTL 1 --- the behavior is same as before
 *      1) go through existing hard logic and identification is based on your-discr.
 *      2) it won't go through tcam identification
 *      3) search oem table with your-discr
 *    For rx BFDoMpls with TTL 255
 *      1) it only go through tcam identification, and based on in lif
 *      2) search oem table with in lif
 * 2. HW changes:
 *    1) A tcam entry is added for bfd identification for rx TTL 255
 *    2) One more oem entry is added with key=in_lif for rx TTL 255
 * 3. API changes:
 *    1) Added a customer SOC to enable rx TTL 255 for BFD over MPLS.
 *    2) both your discr and gport, are used.
 *       For oem key, your discr is used in case of rx ttl 1. gport is used as oem key in case of rx ttl 255.
 *    3) your discr is also used for oam id && discr range check in existing identificaon logic --- this is same as before
 */
                                      ...
                                      uint8 *is_endpoint_acc_to_oamp)
{
    ...
    if (endpoint_info_lcl->flags & BCM_BFD_ENDPOINT_UPDATE) {
        ...
        if (endpoint_info_lcl->faults & BCM_BFD_ENDPOINT_REMOTE_LOC) {
             BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
                              (_BSL_BCM_MSG("Error: BCM_BFD_ENDPOINT_REMOTE_LOC is allowed to be set only on creation.\n")));
        }
        if (is_default) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
   ...
   bcm_bfd_endpoint_info_t *endpoint_info)
{
    ...
    if (endpoint_info->type == bcmBFDTunnelTypeMpls || endpoint_info->type == bcmBFDTunnelTypeMplsPhp) {
        if(endpoint_info->flags & BCM_BFD_ENDPOINT_MULTIHOP){
