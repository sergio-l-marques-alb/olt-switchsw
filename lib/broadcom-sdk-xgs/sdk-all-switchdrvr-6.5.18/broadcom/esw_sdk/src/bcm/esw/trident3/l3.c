        ...
        bcm_port_t port, int vc_swap_index)
{
    ...
    if (process_vntag_p) {
        vntag_p = (module >> BCMI_TD3_L3_NH_IFA_DST_PORT_SHIFT) & 0x1;
        soc_mem_field32_set(unit, view_id, &vc_entry, VNTAG_Pf, vntag_p);
    }

    if (process_vntag_dst_vif) {
        vntag_dst_vif = (port & BCMI_TD3_L3_NH_IFA_DST_PORT_MASK) |
            ((module & BCMI_TD3_L3_NH_IFA_DST_PORT_MASK) <<
            BCMI_TD3_L3_NH_IFA_DST_PORT_SHIFT);
...
*/
    ...
    if (process_next_ptr && (l3_egress->flags2 & BCM_L3_FLAGS2_IFA_DST_PORT)) {
        int vc_swap_index = 0;
        if (l3_egress->port > (1 << BCMI_TD3_L3_NH_IFA_DST_PORT_SHIFT)) {
            return BCM_E_PARAM;
        }
        BCM_IF_ERROR_RETURN(
