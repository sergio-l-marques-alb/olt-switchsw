                               ...
                               sal_usecs_t timeout)
{
    ...
    if (soc_feature(unit, soc_feature_cmicx)) {
        ...
        send64.s.mclass = MOS_MSG_CLASS_IFA;
        send64.s.subclass = s_subclass;
        send64.s.len = bcm_htons(s_len);

        if (MOS_MSG_DMA_MSG(s_subclass) ||
                MOS_MSG_DMA_MSG(r_subclass)) {
            send64.s.data = bcm_htonl(PTR_TO_INT(dma_paddr));
        } else {
            send64.s.data = bcm_htonl(s_data);
        }

        rv = soc_cmic_uc_msg64_send_receive(unit, ifa_info->uc_num,
                                            &send64, &reply64,
                                            timeout);
        if (rv != SOC_E_NONE){
        ...
        *r_len = bcm_ntohs(reply64.s.len);
        ...
        if((rv == SOC_E_NONE) && ((reply64.s.mclass != MOS_MSG_CLASS_IFA) ||
...
#endif
{
        ...
        if (MOS_MSG_DMA_MSG(s_subclass) ||
                MOS_MSG_DMA_MSG(r_subclass)) {
            send.s.data = bcm_htonl(dma_paddr);
        } else {
