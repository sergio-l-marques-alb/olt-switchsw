 ...
 *
 * File:        hashing.c
 * Purpose:     GH2-Hash calcualtions for trunk and ECMP packets.
 */
...
/* Valid RTAG7 hash value mask for unicast and non-unicast trunk resolution */
#define GH2_RTAG7_HASH_TRUNK_UC_MASK  0xff
#define GH2_RTAG7_HASH_TRUNK_NUC_MASK 0xf

/* fields from TRUNK_GROUPm */
STATIC soc_field_t _gh2_portf[8] = {
    PORT0f, PORT1f, PORT2f, PORT3f,
    PORT4f, PORT5f, PORT6f, PORT7f
};

STATIC soc_field_t _gh2_modulef[8] = {
    MODULE0f, MODULE1f, MODULE2f, MODULE3f,
    MODULE4f, MODULE5f, MODULE6f, MODULE7f
};

STATIC soc_field_t _gh2_hg_portf[8] = {
 ...
 */
STATIC int
main_gh2_do_rtag7_hashing(int unit,
                       ...
                       bcm_rtag7_base_hash_t *hash_res)
{
    ...
    if (SOC_REG_FIELD_VALID(unit, RTAG7_HASH_CONTROLr,
                            ENABLE_FLOW_LABEL_IPV6_Af)) {
        rtag7_en_flow_label_ipv6_a =
            soc_reg64_field32_get(unit, RTAG7_HASH_CONTROLr,
                                  rtag7_hash_control,
                                  ENABLE_FLOW_LABEL_IPV6_Af);
    } else {
        ...
        if (rtag7_a_sel == RTAG7_IPV4) {  /* Use IPv4 BITMAPs */
            ...
            if (_BCM_SWITCH_PKT_INFO_FLAG_TEST(pkt_info, PROTOCOL) &&
                ...
                if (_BCM_SWITCH_PKT_INFO_FLAG_TEST(pkt_info, SRC_L4_PORT) &&
                    ...
                    sc_field = IPV6_TCP_UDP_SCTP_SRC_EQ_DST_FIELD_BITMAP_Af;
                    LOG_VERBOSE(BSL_LS_BCM_COMMON,
                                (BSL_META_U(unit,
                                            "Hash calculation: Bitmap is block A IPv6 TCP=UDP\n")));
                } else {
                    sc_reg = RTAG7_IPV6_TCP_UDP_HASH_FIELD_BMAP_2r;
                    sc_field = IPV6_TCP_UDP_SCTP_FIELD_BITMAP_Af;
                    LOG_VERBOSE(BSL_LS_BCM_COMMON,
                                (BSL_META_U(unit,
        ...
        if (rtag7_b_sel == RTAG7_IPV4) {  /* Use IPv4 BITMAPs */
            ...
            if (_BCM_SWITCH_PKT_INFO_FLAG_TEST(pkt_info, PROTOCOL) &&
                ...
                if (_BCM_SWITCH_PKT_INFO_FLAG_TEST(pkt_info, SRC_L4_PORT) &&
                    ...
                    (pkt_info->src_l4_port == pkt_info->dst_l4_port)) {
                    sc_reg = RTAG7_IPV6_TCP_UDP_HASH_FIELD_BMAP_1r;
                    sc_field = IPV6_TCP_UDP_SCTP_SRC_EQ_DST_FIELD_BITMAP_Bf;
                    LOG_VERBOSE(BSL_LS_BCM_COMMON,
                                (BSL_META_U(unit,
                                            "Hash calculation: Bitmap is block B IPv6 TCP=UDP\n")));
                } else {
                    sc_reg = RTAG7_IPV6_TCP_UDP_HASH_FIELD_BMAP_2r;
                    sc_field = IPV6_TCP_UDP_SCTP_FIELD_BITMAP_Bf;
                    LOG_VERBOSE(BSL_LS_BCM_COMMON,
                                (BSL_META_U(unit,
                                            "Hash calculation: Bitmap is block B IPv6 TCP=UDP\n")));
                }
 ...
 */
STATIC int
select_gh2_hash_subfield(uint32 concat, int hash_sub_sel, uint64 *selected_subfield,
...
/*
 * Function:
 *          main_gh2_compute_lbid
 * Description:
 *     get lbid value that is used in hashing calculation
 *     if flow hash is used, get SUB_SEL_LBID_OR_ENTROPYf and 
 *        OFFSET_LBID_OR_ENTROPYf from RTAG7_FLOW_BASED_HASHm 
 ...
 */
STATIC int
main_gh2_compute_lbid(int unit, bcm_rtag7_base_hash_t *hash_Base)
{
    /*regular var declaration*/
    ...
    if (lbid_rtag == 7) {
        ...
        if ((loadbalance_use_flow_hash_nonuc&& (hash_Base->is_nonuc == 0)) || 
            ...
            /* in RTAG7_FLOW_BASED_HASHm, using same selection for UC and NONUC */
            lbid_hash_sub_sel = soc_RTAG7_FLOW_BASED_HASHm_field32_get(unit,
                                           &flow_based_hash_entry, SUB_SEL_LBIDf);
            lbid_hash_offset  = soc_RTAG7_FLOW_BASED_HASHm_field32_get(unit,
                                           &flow_based_hash_entry, OFFSET_LBIDf);
            ...
            if (hash_Base->dev_src_port < 0) {
                ...
                if (soc_feature(unit, soc_feature_rtag7_port_profile)) {
                    BCM_IF_ERROR_RETURN
                        (_bcm_esw_port_tab_get(unit, hash_Base->dev_src_port,
                                               RTAG7_PORT_PROFILE_INDEXf, &hash_index));
                    port_based_hash_index = (uint32) hash_index;
                } else {
 ...
 */
STATIC int
compute_gh2_ecmp_hash(int unit, bcm_rtag7_base_hash_t *hash_Base,
                  uint32 *hash_value)
{
    /*regular var declaration*/
    uint32 hash_sub_sel;
    ...
    BCM_IF_ERROR_RETURN
        (select_gh2_hash_subfield(concat, hash_sub_sel, &hash_subfield, hash_Base));
 ...
 */
STATIC int
compute_gh2_rtag7_hash_trunk(int unit , bcm_rtag7_base_hash_t *hash_Base,
                         uint32 *hash_value)
{
    uint32 hash_sub_sel;
    uint32 hash_offset;    
...
/*
 * Function:
 *          compute_gh2_rtag7_hash_hg_trunk
 * Description:
 *     get HGT hash value from RTAG7_PORT_BASED_HASHm 
 *     if unicast, SUB_SEL_HG_TRUNK_UCf and OFFSET_HG_TRUNK_UCf are used 
 *     if not unicast, SUB_SEL_TRUNK_NONUCf and OFFSET_HG_TRUNK_NONUCf
 *     if flow hash is used, get subsel and offset from RTAG7_FLOW_BASED_HASH 
 * Parameters:
 *          unit - StrataSwitch PCI device unit number (driver internal).
 *          hash_Base - internal data where trunk hash value is storead
 ...
 */
STATIC int
compute_gh2_rtag7_hash_hg_trunk(int unit , bcm_rtag7_base_hash_t *hash_Base,
                         uint32 *hash_value)
{
    uint32 hash_sub_sel;
    uint32 hash_offset;     
 ...
 *     get_gh2_hash_trunk
 * Description:
 *     Compute unicast destination trunk member port.
 * Parameters:
 *     unit       - (IN) Device unit number.
 *     tgid       - (IN) Trunk port id.
 *     hash_index - (IN) Hash value calculated from
 *                       compute_gh2_rtag7_hash_trunk.
 *     dst_gport  - (OUT) Where egress member port id is stored.
 *
 * Returns:
 *     BCM_E_xxxx
 */
    ...
    bcm_gport_t *dst_gport)
{
    ...
    if (rtag != 7) {
        LOG_VERBOSE(BSL_LS_BCM_COMMON,
                    (BSL_META_U(unit,
                                "Hash calculation: uport only RTAG7 calc "
                                "no support for rtag %d\n"),
                     rtag));
    }

    trunk_index = ((hash_index & GH2_RTAG7_HASH_TRUNK_UC_MASK) %
                   (trunk_group_size + 1));
 ...
 */
STATIC int
get_gh2_hash_trunk_nuc(int unit, int tgid, 
                   ...
                   uint32 hash_index, bcm_gport_t *dst_gport)
{
    ...
    nuc_tbm_index = (nuc_type << 4) |
                    (hash_index & GH2_RTAG7_HASH_TRUNK_NUC_MASK);
 ...
 *  hash_index - hash value calculated from compute_gh2_ecmp_hash
 *  hash_rh_index -  hash value calculated from compute_gh2_ecmp_rh_hash
 ...
 */
STATIC int
get_gh2_hash_ecmp(int unit, int ecmp_group, uint32 hash_index, bcm_if_t *nh_id)
{
...
/*
 * Function:
 *    _bcm_gh2_switch_pkt_info_hash_get
 * Description:
 *    Main gh2 hash get function  
 *
 * Parameters     
 *  unit - StrataSwitch PCI device unit number (driver internal).
 ...
 */
int
_bcm_gh2_switch_pkt_info_hash_get (int unit,
                                   bcm_switch_pkt_info_t *pkt_info,
                                   bcm_gport_t *dst_gport,
                                   bcm_if_t *dst_intf)
{
    ...
    BCM_IF_ERROR_RETURN
        (main_gh2_do_rtag7_hashing(unit, pkt_info, &hash_res));
    BCM_IF_ERROR_RETURN
        (main_gh2_compute_lbid(unit, &hash_res));
    ...
    if (0 != (pkt_info->flags & BCM_SWITCH_PKT_INFO_HASH_MULTIPATH)) {
        ...
        BCM_IF_ERROR_RETURN
            (compute_gh2_ecmp_hash(unit, &hash_res, &hash_value));
        LOG_VERBOSE(BSL_LS_BCM_COMMON,
                    (BSL_META_U(unit,
    ...
    if (0 != (pkt_info->flags & BCM_SWITCH_PKT_INFO_HASH_TRUNK)) {
        ...
        if (trunk >= ti.trunk_id_min && trunk <= ti.trunk_id_max) {
            BCM_IF_ERROR_RETURN
                (compute_gh2_rtag7_hash_trunk(unit, &hash_res, &hash_value));
            LOG_VERBOSE(BSL_LS_BCM_COMMON,
                        (BSL_META_U(unit,
            ...
            if (hash_res.is_nonuc) {
                BCM_IF_ERROR_RETURN
                    (get_gh2_hash_trunk_nuc(unit, trunk, pkt_info->fwd_reason, 
                                hash_value, dst_gport));
            } else {
                BCM_IF_ERROR_RETURN
                    (get_gh2_hash_trunk(unit, trunk,
                                    hash_value, dst_gport));
            }
        } else if (trunk >= ti.trunk_fabric_id_min && trunk <= ti.trunk_fabric_id_max) {
            BCM_IF_ERROR_RETURN
                (compute_gh2_rtag7_hash_hg_trunk(unit, &hash_res, &hash_value));
            LOG_VERBOSE(BSL_LS_BCM_COMMON,
                        (BSL_META_U(unit,
                                    "\tHG-Trunk Hash value 0x%08x\n"),
                         hash_value));
            BCM_IF_ERROR_RETURN
                (get_gh2_hash_hg_trunk(unit, trunk - ti.trunk_fabric_id_min,
                                hash_value, dst_gport));
        }
