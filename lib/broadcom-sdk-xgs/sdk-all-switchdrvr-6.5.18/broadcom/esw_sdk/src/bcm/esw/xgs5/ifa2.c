                                ...
                                sal_usecs_t timeout)
{
    ...
    if (soc_feature(unit, soc_feature_cmicx)) {
        ...
        send64.s.mclass = MOS_MSG_CLASS_IFA2;
        ...
        *r_len = bcm_ntohs(reply64.s.len);
        ...
        if((rv == SOC_E_NONE) && ((reply64.s.mclass != MOS_MSG_CLASS_IFA2) ||
                                             ...
                                             shr_ifa2_msg_ctrl_collector_create_t *msg)
{
    ...
    /* Set protobuf fields */
    msg->component_id = collector->protobuf.component_id;
    msg->system_id_length = collector->protobuf.system_id_length;
    memcpy(&(msg->system_id), &(collector->protobuf.system_id),
           collector->protobuf.system_id_length);
                                    ...
                                    int export_profile_id)
{
    ...
    if (_bcm_esw_collector_check_user_is_other(unit,
                                               collector_id,
                                               _BCM_COLLECTOR_EXPORT_APP_IFA2_PROTOBUF)) {
        return (BCM_E_CONFIG);
    }
    ...
    if (collector.protobuf.system_id_length == 0) {
        return BCM_E_PARAM;
    }
    ...
    if (export_profile.wire_format != bcmCollectorWireFormatProtoBuf) {
        return BCM_E_PARAM;
    }
    if (export_profile.flags & BCM_COLLECTOR_EXPORT_PROFILE_FLAGS_USE_NUM_RECORDS) {
        if (export_profile.num_records > SHR_IFA2_EXPORT_NUM_RECORDS_MAX) {
            return BCM_E_PARAM;
        }
    } else {
...
int _bcm_xgs5_ifa2_init(int unit, uint8 *success)
{
    ...
    *success = 0;
    ...
    if (ifa2_info->max_rx_pkt_length > _BCM_XGS5_IFA2_MAX_RX_PKT_LENGTH) {
        BCM_IF_ERROR_RETURN(_bcm_xgs5_ifa2_detach(unit));
        return BCM_E_PARAM;
    }
    ...
    if (ifa2_info->max_export_pkt_length > _BCM_XGS5_IFA2_MAX_EXPORT_PKT_LENGTH) {
        BCM_IF_ERROR_RETURN(_bcm_xgs5_ifa2_detach(unit));
        return BCM_E_PARAM;
    }
