                             ...
                             int coll_enable)
{
    ...
    soc_mem_field32_set(unit, collector_mem, &config, FLUSHf, 0x0);
                    ...
                    bcmi_fte_template_record_info_t *record)
{
    ...
    while(ftg_template) {
        ...
        LOG_DEBUG(BSL_LS_BCM_FLOWTRACKER, (BSL_META_U(unit,
                        "[TemplateValidate]Element Record_Idx: %2d | Id: %2d (%20s) |"
                        " Size: %02d (0x%x)\n\r"), idx, info_elem, info_name, data_bits,
                        check_id));
    ...
    for (ix = 0; ix < num_in_export_elements; ix++) {
        ...
        switch(in_export_elements[ix].element) {
            ...
            case bcmFlowtrackerExportElementTypeFlowIdentifier:
                ...
                /* FLOW_INDEX_GROUP_ID */
                field_valid = soc_format_field_valid(unit,
                            format, FLOW_INDEX_GROUP_IDf);
                if (!field_valid) {
                    info_elem =
                    ...
                    LOG_DEBUG(BSL_LS_BCM_FLOWTRACKER, (BSL_META_U(unit,
                                    "Export element <%s> not supported\n\r"),
                                info_name));
                    return BCM_E_CONFIG;
                }
                field_info = NULL;
                field_info = soc_format_fieldinfo_get(unit, format,
                        FLOW_INDEX_GROUP_IDf);
                if (field_info != NULL) {
                    offset = field_info->bp;
                    length = field_info->len;
                    info_elem =
