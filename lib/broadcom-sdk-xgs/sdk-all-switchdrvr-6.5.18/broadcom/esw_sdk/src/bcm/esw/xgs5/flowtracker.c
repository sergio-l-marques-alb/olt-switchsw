                               ...
                               sal_usecs_t timeout)
{
    ...
    if (soc_feature(unit, soc_feature_cmicx)) {
        ...
        sal_memset(&reply64, 0, sizeof(reply64));
        send64.s.mclass = MOS_MSG_CLASS_FT;
        send64.s.subclass = s_subclass;
        send64.s.len = bcm_htons(s_len);
        ...
        if (MOS_MSG_DMA_MSG(s_subclass) ||
            ...
            send64.s.data = bcm_htonl(soc_cm_l2p(unit, dma_buffer));
        } else {
            send64.s.data = bcm_htonl(s_data);
        }

        rv = soc_cmic_uc_msg_send_receive(unit, ft_info->uc_num,
                                          (mos_msg_data_t *)&send64,
                                          (mos_msg_data_t *)&reply64,
                                          timeout);
        if (rv != SOC_E_NONE){
        ...
        uc_rv  = bcm_ntohl(reply64.s.data);
        rv     = _bcm_xgs5_ft_convert_uc_error(uc_rv);

        *r_len = bcm_ntohs(reply64.s.len);

        /*Check reply class and subclass*/
        if((rv == SOC_E_NONE) && ((reply64.s.mclass != MOS_MSG_CLASS_FT) ||
            (reply64.s.subclass != r_subclass)))
        {
            return BCM_E_INTERNAL;
        }
    } else
        ...
        bcm_cosq_burst_monitor_flow_view_info_t *flow_view)
{
    ...
    if (options == BCM_QCM_CONFIG_UPDATE) {
        ...
        if (flow_view->scan_interval_usecs < SHR_QCM_MIN_SCAN_INTERVAL) {
            LOG_ERROR(BSL_LS_BCM_FLOWTRACKER, (BSL_META_U(unit,
        ...
        if (flow_view->num_flows != ft_info->max_flows_total) {
            LOG_ERROR(BSL_LS_BCM_FLOWTRACKER, (BSL_META_U(unit,
                      "num_flows : %d is mismatch with max_flows_total: %d\n"),
                      flow_view->num_flows, ft_info->max_flows_total));
            return BCM_E_PARAM;
        }
        flow_cfg.drop_weight       = flow_view->export_param.drop_weight;
        flow_cfg.rx_weight         = flow_view->export_param.rx_weight;
        flow_cfg.export_threshold  = flow_view->export_param.export_threshold;
        ...
        bcm_cosq_burst_monitor_flow_view_info_t *flow_view)
{
    ...
    flow_view->export_param.drop_weight        = flow_cfg.drop_weight;
    flow_view->export_param.rx_weight          = flow_cfg.rx_weight;
    flow_view->export_param.export_threshold   = flow_cfg.export_threshold;
        ...
        uint32 usr_mem_size, uint8 *usr_mem_addr)
{
    ...
    for (idx = 0; idx < host_buf_msg.num_flow_views; ++idx) {
        ...
        host_ptr += ((ft_info->max_flows_total - flow_idx) *
                     (SHR_QCM_TUPLE_SIZE +
                      (SHR_QCM_MAX_NUM_SAMPLE * SHR_QCM_NUM_BYTE_COUNT) +
                      (SHR_QCM_MAX_NUM_SAMPLE * ((SHR_QCM_NUM_BYTE_COUNT * 2) + SHR_QCM_NUM_SHARE_COUNT)) +
