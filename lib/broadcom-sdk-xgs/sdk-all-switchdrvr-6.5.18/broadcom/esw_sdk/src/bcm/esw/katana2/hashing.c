/*
 * $Id: hashing.c,v 1.14 Broadcom SDK $
 * $Copyright: (c) 2019 Broadcom.
 * Broadcom Proprietary and Confidential. All rights reserved.$
 *
 * File:        hashing.c
 * Purpose:     KT2-Hash calcualtions for trunk and ECMP packets.
 */
 ...
 */
STATIC int
main_kt2_do_rtag7_hashing(int unit,
                       ...
                       bcm_rtag7_base_hash_t *hash_res)
{
    ...
    SOC_IF_ERROR_RETURN
        (READ_RTAG7_HASH_CONTROL_64r(unit, &RTAG7_HASH_CONTROL_64));
    SOC_IF_ERROR_RETURN
    ...
    rtag7_disable_hash_ipv4_a =
        soc_reg64_field32_get(unit, RTAG7_HASH_CONTROL_64r, RTAG7_HASH_CONTROL_64,
    ...
    if (SOC_REG_FIELD_VALID(unit, RTAG7_HASH_CONTROL_2_64r,
                            ENABLE_FLOW_LABEL_IPV6_Af)) {
        rtag7_en_flow_label_ipv6_a =
            soc_reg64_field32_get(unit, RTAG7_HASH_CONTROL_2_64r,
                                  RTAG7_HASH_CONTROL_2_64,
                                  ENABLE_FLOW_LABEL_IPV6_Af);
    } else {
    ...
    if ((_BCM_SWITCH_PKT_INFO_FLAG_TEST(pkt_info, ETHERTYPE) &&
        ...
        if (_BCM_SWITCH_PKT_INFO_FLAG_TEST(pkt_info, DST_IP)) {
            ...
            dip_a = 0;
                ...
                (pkt_info->ethertype == ETHERTYPE_MPLS))) {

        rtag7_a_sel = RTAG7_MPLS;
    ...
    } else if (rtag7_a_sel == RTAG7_MPLS) {

        if (sip_valid) {
            hash[6] = (sip_a >> 16) & 0xffff;
            hash[5] =  sip_a & 0xffff;
        } else {
            hash_element_valid_bits &= ~0xc00;
        }
        if (dip_valid) {
            hash[8]  = (dip_a >> 16) & 0xffff;
            hash[7]  =  dip_a & 0xffff;
        } else {
            hash_element_valid_bits &= ~0x1e0;
        }

        if (_BCM_SWITCH_PKT_INFO_FLAG_TEST(pkt_info, TOP_LABEL)) {
            hash[11]  = pkt_info->top_label & 0xffff;
        } else {
            hash_element_valid_bits &= ~0x800;
        }

        if (_BCM_SWITCH_PKT_INFO_FLAG_TEST(pkt_info, 2ND_LABEL)) {
            hash[10]  = pkt_info->second_label & 0xffff;
        } else {
            hash_element_valid_bits &= ~0x400;
        }

        if (_BCM_SWITCH_PKT_INFO_FLAG_TEST(pkt_info, 3RD_LABEL)) {
            hash[4]  = pkt_info->third_label & 0xffff;
        } else {
            hash_element_valid_bits &= ~0x10;
        }

        if (_BCM_SWITCH_PKT_INFO_FLAG_TEST(pkt_info, LABELS_4MSB)) {
            hash[9] = 0;
            hash[9] = (hash[11] >> 16) | (hash[10] >> 16) | (hash[4] >> 16);
        } else {
            hash_element_valid_bits &= ~0x200;
        }

        sc_reg = RTAG7_MPLS_OUTER_HASH_FIELD_BMAPr;
        sc_field = MPLS_OUTER_BITMAP_Af;
        LOG_VERBOSE(BSL_LS_BCM_COMMON,
                    (BSL_META_U(unit,
                                "Hash calculation: Bitmap is block A MPLS\n")));
    ...
    if ((rtag7_b_sel == RTAG7_IPV6) || (rtag7_b_sel == RTAG7_IPV4)) {
    ...
    } else if (rtag7_b_sel == RTAG7_MPLS) {
        ...
        sc_reg = RTAG7_MPLS_OUTER_HASH_FIELD_BMAPr;
        sc_field = MPLS_OUTER_BITMAP_Bf;
        LOG_VERBOSE(BSL_LS_BCM_COMMON,
                    (BSL_META_U(unit,
                                "Hash calculation: Bitmap is block B MPLS\n")));
 ...
 */
STATIC int
select_kt2_hash_subfield(uint32 concat, int hash_sub_sel, uint64 *selected_subfield,
 ...
 */
STATIC int
main_kt2_compute_lbid(int unit, bcm_rtag7_base_hash_t *hash_Base)
{
    /*regular var declaration*/
 ...
 */
STATIC int
compute_kt2_ecmp_hash(int unit, bcm_rtag7_base_hash_t *hash_Base,
                  uint32 *hash_value)
{
    /*regular var declaration*/
    uint32 hash_sub_sel;
    ...
    BCM_IF_ERROR_RETURN
        (select_kt2_hash_subfield(concat, hash_sub_sel, &hash_subfield, hash_Base));
 ...
 */
STATIC int
compute_kt2_rtag7_hash_trunk(int unit , bcm_rtag7_base_hash_t *hash_Base,
                         uint32 *hash_value)
{
    uint32 hash_sub_sel;
    uint32 hash_offset;
...
/*
 * Function:
 *          compute_kt2_rtag7_hash_hg_trunk
 * Description:
 ...
 */
STATIC int
compute_kt2_rtag7_hash_hg_trunk(int unit , bcm_rtag7_base_hash_t *hash_Base,
                         uint32 *hash_value)
{
    uint32 hash_sub_sel;
    uint32 hash_offset;
 ...
 *  tgid - trunk port id
 *  hash_index - hash value calculated from compute_kt2_rtag7_hash_trunk
 *  hash_rh_index -  hash value calculated from compute_kt2_rtag7_hash_rh_trunk
 *  dst_tgid - where egress member port id is stored
 *  lag_rh_hash - flag indicating if rh is enable for this hashing calculation
 ...
 */
STATIC int
get_kt2_hash_trunk(int unit, int tgid, uint32 hash_index, bcm_gport_t *dst_gport)
{
    uint32 trunk_base;
 ...
 */
STATIC int
get_kt2_hash_trunk_nuc(int unit, int tgid,
                   ...
                   uint32 hash_index, bcm_gport_t *dst_gport)
{
    ...
    nuc_tbm_index = (nuc_type << 4) | (hash_index & 0xf);
 ...
 *  unit - StrataSwitch PCI device unit number (driver internal).
 *  ecmp_group - multipath id
 *  hash_index - hash value calculated from compute_kt2_ecmp_hash
 *  hash_rh_index -  hash value calculated from compute_kt2_ecmp_rh_hash
 ...
 */
STATIC int
get_kt2_hash_ecmp(int unit, int ecmp_group, uint32 hash_index, bcm_if_t *nh_id)
{
    ...
    BCM_IF_ERROR_RETURN
        (READ_INITIAL_L3_ECMP_GROUPm(unit, MEM_BLOCK_ANY, ecmp_group, &ecmpc_entry));

    ecmp_ptr = soc_INITIAL_L3_ECMP_GROUPm_field32_get(unit, &ecmpc_entry,
                                              BASE_PTR_0f);
    ecmp_count = soc_INITIAL_L3_ECMP_GROUPm_field32_get(unit, &ecmpc_entry,
                                              COUNT_0f);
...
STATIC int
get_kt2_hash_hg_trunk(int unit, int hgtid, uint32 hash_index, bcm_gport_t *dst_gport)
{
    uint32 trunk_base;
    uint32 trunk_group_size;
    uint32 rtag;
...
/*
 * Function:
 *          _bcm_kt2_switch_pkt_info_hash_get
 * Description:
 *    main kt2 hash get function
 *
 * Parameters
 *  unit - StrataSwitch PCI device unit number (driver internal).
 ...
 */
int
_bcm_kt2_switch_pkt_info_hash_get (int unit,
                                   bcm_switch_pkt_info_t *pkt_info,
                                   ...
                                   bcm_if_t *dst_intf)
{
    ...
    BCM_IF_ERROR_RETURN
        (main_kt2_do_rtag7_hashing(unit, pkt_info, &hash_res));
    BCM_IF_ERROR_RETURN
        (main_kt2_compute_lbid(unit, &hash_res));
    ...
    if (0 != (pkt_info->flags & BCM_SWITCH_PKT_INFO_HASH_MULTIPATH)) {
        ...
        BCM_IF_ERROR_RETURN
            (compute_kt2_ecmp_hash(unit, &hash_res, &hash_value));
        LOG_VERBOSE(BSL_LS_BCM_COMMON,
                    (BSL_META_U(unit,
    ...
    if (0 != (pkt_info->flags & BCM_SWITCH_PKT_INFO_HASH_TRUNK)) {
        ...
        if (trunk >= ti.trunk_id_min && trunk <= ti.trunk_id_max) {
            BCM_IF_ERROR_RETURN
                (compute_kt2_rtag7_hash_trunk(unit, &hash_res, &hash_value));
            LOG_VERBOSE(BSL_LS_BCM_COMMON,
                        (BSL_META_U(unit,
            ...
            if (hash_res.is_nonuc) {
                BCM_IF_ERROR_RETURN
                    (get_kt2_hash_trunk_nuc(unit, trunk, pkt_info->fwd_reason,
                                hash_value, dst_gport));
            } else {
                BCM_IF_ERROR_RETURN
                    (get_kt2_hash_trunk(unit, trunk,
                                    hash_value, dst_gport));
            }
        } else if (trunk >= ti.trunk_fabric_id_min && trunk <= ti.trunk_fabric_id_max) {
            BCM_IF_ERROR_RETURN
                (compute_kt2_rtag7_hash_hg_trunk(unit, &hash_res, &hash_value));
            LOG_VERBOSE(BSL_LS_BCM_COMMON,
                        (BSL_META_U(unit,
                                    "\tHG-Trunk Hash value 0x%08x\n"),
                         hash_value));
            BCM_IF_ERROR_RETURN
                (get_kt2_hash_hg_trunk(unit, trunk - ti.trunk_fabric_id_min,
                                hash_value, dst_gport));
        }
