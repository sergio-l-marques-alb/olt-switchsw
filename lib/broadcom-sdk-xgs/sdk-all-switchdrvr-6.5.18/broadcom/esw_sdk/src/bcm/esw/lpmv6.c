                              ...
                              int nh_ecmp_idx) 
{
    ...
    /* Write buffer to hw. */
#if defined(BCM_FIREBOLT6_SUPPORT)
if (soc_feature_update_defip_pair_data_only &&
    (mem == L3_DEFIP_PAIR_128m) &&
    (lpm_cfg->defip_flags & BCM_L3_REPLACE)) {
    /* Zero buffers. */
    ...
    BCM_IF_ERROR_RETURN(BCM_XGS3_MEM_WRITE(unit, L3_DEFIP_PAIR_128_DATA_ONLYm, hw_index, entry_data));
    soc_mem_field_get(unit, L3_DEFIP_PAIR_128m, hw_entry, HITf, &entry_hit);
    BCM_IF_ERROR_RETURN(BCM_XGS3_MEM_WRITE(unit, L3_DEFIP_PAIR_128_HIT_ONLYm, hw_index, &entry_hit));
} else
#endif
   {
       rv = BCM_XGS3_MEM_WRITE(unit, mem, hw_index, hw_entry);
    ...
    if (SOC_URPF_STATUS_GET(unit) &&
        ...
        if (soc_feature_update_defip_pair_data_only &&
            ...
            sal_memset(entry_data, 0, WORDS2BYTES(SOC_MAX_MEM_FIELD_WORDS));
            SOC_MEM_OPT_FLD_GET(unit, L3_DEFIP_PAIR_128m, hw_entry, DATA_RANGEf, entry_data);
            BCM_IF_ERROR_RETURN(BCM_XGS3_MEM_WRITE(unit, L3_DEFIP_PAIR_128_DATA_ONLYm,
               (hw_index + BCM_DEFIP_PAIR128_URPF_OFFSET(unit)), entry_data));
            soc_mem_field_get(unit, L3_DEFIP_PAIR_128m, hw_entry, HITf, &entry_hit);
            BCM_IF_ERROR_RETURN(BCM_XGS3_MEM_WRITE(unit, L3_DEFIP_PAIR_128_HIT_ONLYm,
