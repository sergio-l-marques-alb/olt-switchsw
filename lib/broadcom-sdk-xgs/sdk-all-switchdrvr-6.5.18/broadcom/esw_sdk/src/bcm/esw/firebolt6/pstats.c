...
#if defined(INCLUDE_PSTATS) && defined(BCM_FIREBOLT6_SUPPORT)
#include <soc/drv.h>
#include <soc/mem.h>
...
#define FB6_MAX_XPEPIPE             _FB6_PIPES_PER_DEV * _FB6_XPES_PER_DEV

#define FB6_MAX_POOL_COUNT          4
#define FB6_MAX_UCQ_COUNT           3680

/* Max Elements Count = UCQ + ING POOL + EGR POOL */
#define FB6_MAX_ELEMENT_COUNT       FB6_MAX_UCQ_COUNT + FB6_MAX_POOL_COUNT * 2
 ...
 * |   1    |  0  |  1   |
 *  */
#define FB6_XPE_PIPE_INDEX(unit, xpe, pipe)                     \
    (((xpe) == -1) ? (pipe) :                                   \
     ((pipe) == -1) ? (xpe) : (xpe) * NUM_PIPE(unit) + (pipe))

#define FB6_XPE_VALID(unit, xpe, valid)                        \
    if (xpe >  NUM_XPE(unit)) {                                \
        valid = FALSE;                                         \
    }                                                          \
...
typedef struct pstats_session_s {
    ...
    bcm_pstats_session_element_t    element_array[FB6_MAX_ELEMENT_COUNT];
...
typedef struct pstats_control_s {
    ...
    pstats_buffer_t                 ucq_buffer[FB6_MAX_XPEPIPE];
    pstats_buffer_t                 ing_pool_buffer[FB6_MAX_POOL_COUNT];
    pstats_buffer_t                 egr_pool_buffer[FB6_MAX_POOL_COUNT];
...
STATIC int
bcm_fb6_pstats_session_create(int unit, int options, int array_count,
                              bcm_pstats_session_element_t  *element_array,
                              bcm_pstats_session_id_t *session_id);
STATIC int
bcm_fb6_pstats_session_destroy(int unit, bcm_pstats_session_id_t session_id);
STATIC int
bcm_fb6_pstats_session_get(int unit, bcm_pstats_session_id_t session_id,
                           int array_max,
                           bcm_pstats_session_element_t *element_array,
                           int *array_count);
STATIC int bcm_fb6_pstats_data_sync(int unit);
STATIC int
bcm_fb6_pstats_session_data_get(int unit, bcm_pstats_session_id_t session_id,
                                int sync,
                                bcm_pstats_timestamp_t *timestamp,
                                int array_max,
                                ...
                                int *array_count);
STATIC int
bcm_fb6_pstats_session_data_clear(int unit, bcm_pstats_session_id_t session_id);

STATIC int
bcm_fb6_pstats_session_traverse(int unit, bcm_pstats_session_traverse_cb cb,
                                void *user_data);

#ifdef BCM_WARM_BOOT_SUPPORT_SW_DUMP
/*
 * Function:
 *     bcm_fb6_pstats_sw_dump
 * Purpose:
 ...
 */
STATIC void
bcm_fb6_pstats_sw_dump(int unit)
{
...
STATIC void
_bcm_fb6_pstats_session_cleanup(int unit, pstats_session_t *session)
{
...
_bcm_fb6_pstats_session_get(int unit, bcm_pstats_session_id_t session_id)
{
    int                 found = 0;
    pstats_session_t   *cur_session;
...
STATIC void
_bcm_fb6_pstats_session_next_id_set(int unit)
{
    pstats_session_t            *cur_session;
...
STATIC int
_bcm_fb6_pstats_session_delete(int unit, bcm_pstats_session_id_t session_id)
{
    pstats_session_t            *cur_session;

    cur_session = _bcm_fb6_pstats_session_get(unit, session_id);
    if (NULL == cur_session) {
    ...
    _bcm_fb6_pstats_session_cleanup(unit, cur_session);
    if (NULL != cur_session) {
        sal_free(cur_session);
    }
    PSTATS_CTRL(unit).session_count--;

    _bcm_fb6_pstats_session_next_id_set(unit);

    return BCM_E_NONE;
}

STATIC void
_bcm_fb6_pstats_session_insert(int unit, pstats_session_t *session)
{
    pstats_session_t            *cur_session;
...
STATIC void
_bcm_fb6_pstats_buffer_deinit(int unit, pstats_buffer_t *pbuf)
{
    if (NULL == pbuf) {
        return;
    }
...
STATIC int
_bcm_fb6_pstats_buffer_init(int unit, soc_mem_t mem, soc_mem_t clear_mem,
...
/*
 * Function:
 *      bcm_fb6_pstats_deinit
 * Purpose:
 *      Deinitialize PSTATS driver functions
 *      for FIREBOLT6 (BCM56870).
 * Parameters:
 ...
 */
void
bcm_fb6_pstats_deinit(int unit)
{
...
#ifdef BCM_PSTATS_MEASURE
    _bcm_fb6_pstats_buffer_deinit(unit, &PSTATS_CTRL(unit).local_time);
    _bcm_fb6_pstats_buffer_deinit(unit, &PSTATS_CTRL(unit).local_time_end);
#endif
    _bcm_fb6_pstats_buffer_deinit(unit, &PSTATS_CTRL(unit).utc_time);
    for (i = 0; i < FB6_MAX_XPEPIPE; i++) {
        _bcm_fb6_pstats_buffer_deinit(unit, &PSTATS_CTRL(unit).ucq_buffer[i]);
    }
    for (i = 0; i < FB6_MAX_POOL_COUNT; i++) {
        _bcm_fb6_pstats_buffer_deinit(unit, &PSTATS_CTRL(unit).ing_pool_buffer[i]);
        _bcm_fb6_pstats_buffer_deinit(unit, &PSTATS_CTRL(unit).egr_pool_buffer[i]);
    }
...
bcm_fb6_pstats_init(int unit)
{
    ...
    pstats_driver->session_create = bcm_fb6_pstats_session_create;
    pstats_driver->session_destroy = bcm_fb6_pstats_session_destroy;
    pstats_driver->session_get = bcm_fb6_pstats_session_get;
    pstats_driver->data_sync = bcm_fb6_pstats_data_sync;
    pstats_driver->session_data_get = bcm_fb6_pstats_session_data_get;
    pstats_driver->session_data_clear = bcm_fb6_pstats_session_data_clear;
    pstats_driver->session_traverse = bcm_fb6_pstats_session_traverse;
#ifdef BCM_WARM_BOOT_SUPPORT_SW_DUMP
    pstats_driver->pstats_sw_dump = bcm_fb6_pstats_sw_dump;
#endif
    ...
    rv = _bcm_fb6_pstats_buffer_init(unit, mem, mem, pbuf);
    if (BCM_FAILURE(rv)) {
        goto cleanup;
    }
    mem = MMU_INTFO_TIMESTAMPm;
    ...
    for (pipe = 0; pipe < NUM_PIPE(unit); pipe++) {
        for (xpe = 0; xpe < NUM_XPE(unit); xpe++) {
            ...
            index = FB6_XPE_PIPE_INDEX(unit, xpe, pipe);
            if (index >= FB6_MAX_XPEPIPE) {
                rv = BCM_E_INTERNAL;
                goto cleanup;
            }
            pbuf = &PSTATS_CTRL(unit).ucq_buffer[index];

            rv = _bcm_fb6_pstats_buffer_init(unit, mem, clear_mem, pbuf);
            if (BCM_FAILURE(rv)) {
                goto cleanup;
            }
        }
    ...
    for (xpe = 0; xpe < NUM_XPE(unit); xpe++) {
        ...
        index = FB6_XPE_PIPE_INDEX(unit, xpe, -1);
        if (index >= FB6_MAX_POOL_COUNT) {
            rv = BCM_E_INTERNAL;
            goto cleanup;
        }
    ...
    if (SOC_WARM_BOOT(unit)) {
        ...
        rv = soc_helix5_mmu_pstats_mode_get(unit, &flag);
        if (BCM_FAILURE(rv)) {
            goto cleanup;
        }

        /* HW config is different with config property */
        if (!(flag & _FB6_MMU_PSTATS_ENABLE) ||
            !(flag & _FB6_MMU_PSTATS_PKT_MOD)) {
        ...
        if (flag & _FB6_MMU_PSTATS_HWM_MOD) {
            PSTATS_CTRL(unit).stat_mode = 1;
            if (flag & _FB6_MMU_PSTATS_RESET_ON_READ) {
        ...
        flag |= _FB6_MMU_PSTATS_SYNC | _FB6_MMU_PSTATS_MOR_EN;

        /* Reset pstats mode in warmboot */
        rv = soc_helix5_mmu_pstats_mode_set(unit, flag);
        if (BCM_FAILURE(rv)) {
            goto cleanup;
        }
    } else
#endif
    {
        ...
        /* call soc function */
        flag = _FB6_MMU_PSTATS_ENABLE |
               _FB6_MMU_PSTATS_SYNC |
               _FB6_MMU_PSTATS_MOR_EN;


        if (PSTATS_CTRL(unit).stat_mode) {
            flag |= _FB6_MMU_PSTATS_HWM_MOD;
            if (PSTATS_CTRL(unit).hw_cor_in_max_use_count == 1) {
                flag |= _FB6_MMU_PSTATS_RESET_ON_READ;
            }
        }
...
cleanup:
    soc_pstats_deinit(unit);
    (void )bcm_fb6_pstats_deinit(unit);

    return rv;
}

STATIC int
_bcm_fb6_pstats_session_element_check(int unit,
                                       bcm_pstats_session_element_t element)
{
    ...
    LOG_INFO(BSL_LS_BCM_PSTATS,
        (BSL_META_U(unit, " type = %d gport = %d cosq = %d \n"), unit,  element.gport, element.cosq));
    ...
    if (element.type == bcmPStatsSessionUnicastQueue) {
        ...
        if ((element.pool < 0) || (element.pool >= FB6_MAX_POOL_COUNT)) {
            LOG_INFO(BSL_LS_BCM_PSTATS,
                (BSL_META_U(unit, "pstats_session_element_check: "
                              ...
                              bcm_pstats_session_id_t *session_id)
{
    ...
    /* element check */
    if ((array_count < 0) ||
        (array_count > FB6_MAX_ELEMENT_COUNT)) {
    ...
    for (i = 0; i < array_count; i++) {
        BCM_IF_ERROR_RETURN(
                _bcm_fb6_pstats_session_element_check(unit, element_array[i]));
    }

    if (options & BCM_PSTATS_OPTIONS_REPLACE) {
        /* delete original session first */
        id = *session_id;
        BCM_IF_ERROR_RETURN(_bcm_fb6_pstats_session_delete(unit, id));
        ...
        cur_session = _bcm_fb6_pstats_session_get(unit, id);
        if (NULL != cur_session) {
            LOG_INFO(BSL_LS_BCM_PSTATS,
    ...
    _bcm_fb6_pstats_session_insert(unit, cur_session);
    _bcm_fb6_pstats_session_next_id_set(unit);
...
/*
 * Function:
 *      bcm_fb6_pstats_session_destroy
 * Purpose:
 ...
 */
STATIC int
bcm_fb6_pstats_session_destroy(int unit,
                               bcm_pstats_session_id_t session_id)
{
    return _bcm_fb6_pstats_session_delete(unit, session_id);
}

/*
 * Function:
 *      bcm_fb6_pstats_session_get
 * Purpose:
 ...
 */
STATIC int
bcm_fb6_pstats_session_get(int unit,
                           bcm_pstats_session_id_t session_id,
                           int array_max,
                           bcm_pstats_session_element_t *element_array,
                           int *array_count)
...
STATIC int
_bcm_fb6_pstats_session_data_update(int unit,
                                    bcm_pstats_session_element_t element,
                                    bcm_pstats_data_t *data)
{
    bcm_gport_t                 gport;
...
STATIC int
_bcm_fb6_pstats_session_update(int unit, pstats_session_t *session)
{
    bcm_pstats_session_element_t    element;
    bcm_pstats_data_t               *data;
    ...
    for (i = 0; i < cur_session->element_array_count; i++) {
        ...
        BCM_IF_ERROR_RETURN(
            _bcm_fb6_pstats_session_data_update(unit, element, data));
    }
 ...
 */
STATIC int
_bcm_fb6_pstats_hw_sync(int unit)
{
    pstats_buffer_t             *pbuf;
    int                         index, pipe, xpe;
    soc_info_t                  *si = &SOC_INFO(unit);
...
/*
 * Function:
 *      _bcm_fb6_pstats_session_data_sync
 * Purpose:
 ...
 */
STATIC int
_bcm_fb6_pstats_session_data_sync(int unit, pstats_session_t *session)
{
    BCM_IF_ERROR_RETURN(_bcm_fb6_pstats_hw_sync(unit));
    BCM_IF_ERROR_RETURN(_bcm_fb6_pstats_session_update(unit, session));
    return BCM_E_NONE;
}

/*
 * Function:
 *      bcm_fb6_pstats_data_sync
 * Purpose:
 *      Start a pstats sync operation.
 * Parameters:
 *      unit - (IN) StrataSwitch unit number
 ...
 */
STATIC int
bcm_fb6_pstats_data_sync(int unit)
{
    pstats_session_t    *cur_session = NULL;
    ...
    while (NULL != cur_session) {
        ...
        BCM_IF_ERROR_RETURN(
                _bcm_fb6_pstats_session_update(unit, cur_session));
        cur_session = cur_session->next_session;
    }
    return BCM_E_NONE;
}
...
/*
 * Function:
 *      bcm_fb6_pstats_session_data_get
 * Purpose:
                                ...
                                int *array_count)
{
    ...
    if (TRUE == sync) {
        if (mode) {
            BCM_IF_ERROR_RETURN(bcm_fb6_pstats_data_sync(unit));
        } else {
            BCM_IF_ERROR_RETURN(
                    _bcm_fb6_pstats_session_data_sync(unit, cur_session));
        }
    }
...
STATIC int
_bcm_fb6_pstats_session_data_hw_clear(int unit, bcm_pstats_session_element_t element)
{
    bcm_gport_t                 gport;
    bcm_port_t                  local_port;
    bcm_cos_queue_t             cosq;
...
bcm_fb6_pstats_session_data_clear(int unit, bcm_pstats_session_id_t session_id)
{
    ...
    if (need_hw_clear == 1) {
        for (i = 0; i < cur_session->element_array_count; i++) {
            element = cur_session->element_array[i];
            BCM_IF_ERROR_RETURN(
                _bcm_fb6_pstats_session_data_hw_clear(unit, element));
        }
    }
...
/*
 * Function:
 *      bcm_fb6_pstats_session_traverse
 * Purpose:
                                ...
                                void *user_data)
{
    ...
    element_array = (bcm_pstats_session_element_t *)
                     sal_alloc(FB6_MAX_ELEMENT_COUNT * sizeof(bcm_pstats_session_element_t),
                     "pstats traverse"); 
    cur_session = PSTATS_CTRL(unit).first_session;
    while (NULL != cur_session) {
...
#else /* INCLUDE_PSTATS && BCM_FIREBOLT6_SUPPORT */
int bcm_esw_firebolt6_pstats_not_empty;
#endif /* INCLUDE_PSTATS && BCM_FIREBOLT6_SUPPORT */
