...
Subport Coe/LinkPhy Ports\n Disable CoE and run the API again.\n")));
    ...
    for (loop = 0; loop < SB2_MAX_PORTS_PER_BLOCK; loop++) {
         block_port=(*sb2_block_ports[unit])[block][loop];
         if (SOC_PBMP_MEMBER((PBMP_ALL(unit)), block_port)) {
             SOC_IF_ERROR_RETURN(_soc_saber2_mmu_reconfigure(unit, block_port));
         }
    }
    /* ReConfigure H/W */
...
#if defined(BCM_METROLITE_SUPPORT)
        if (SOC_IS_METROLITE(unit)){
            SOC_IF_ERROR_RETURN(_soc_ml_mmu_reconfigure(unit, port));
	}else
#endif
	{
            SOC_IF_ERROR_RETURN(_soc_saber2_mmu_reconfigure(unit, port));
	}
...
int _bcm_saber2_mmu_dest_port_cfg_init(int unit, int port)
{
    ...
    if (IS_EXT_MEM_PORT(unit, port)) {
        ...
        SOC_IF_ERROR_RETURN(READ_RQE_PP_PORT_CONFIGr(unit, port, &rval3));
        soc_reg_field_set(unit, RQE_PP_PORT_CONFIGr, &rval3, BUFF_TYPEf, 1);
        SOC_IF_ERROR_RETURN(WRITE_RQE_PP_PORT_CONFIGr(unit, port, rval3));
        ...
        if (si->port_num_subport[port] > 0) {
            ...
            for (pp_port = start_pp_port; pp_port < end_pp_port; pp_port++) {
                ...
                SOC_IF_ERROR_RETURN(READ_RQE_PP_PORT_CONFIGr(unit, pp_port, &rval3));
                soc_reg_field_set(unit, RQE_PP_PORT_CONFIGr, &rval3, BUFF_TYPEf, 1);
                SOC_IF_ERROR_RETURN(WRITE_RQE_PP_PORT_CONFIGr(unit, pp_port, rval3));
        ...
        SOC_IF_ERROR_RETURN(READ_RQE_PP_PORT_CONFIGr(unit, port, &rval3));
        soc_reg_field_set(unit, RQE_PP_PORT_CONFIGr, &rval3, BUFF_TYPEf, 0);
        SOC_IF_ERROR_RETURN(WRITE_RQE_PP_PORT_CONFIGr(unit, port, rval3));
...
int _bcm_saber2_mmu_credit_reset_ext_mem(int unit, int port)
{
    ...
    for (loop = 0;loop < SB2_MAX_PORTS_PER_BLOCK; loop++) {
        ...
        /* For block 6, we only update current port and not all
           ports in the block */
        if(block == 6) {
            if (block_port != port) {
                continue;
            ...
            BCM_IF_ERROR_RETURN(READ_DEQ_EFIFO_CFGr(unit,block_port,&rval));
            block_max_nxtaddr = soc_reg_field_get(unit, DEQ_EFIFO_CFGr,
                                            rval, EGRESS_FIFO_START_ADDRESSf);
        }
...
STATIC
int _bcm_saber2_reset_toq(int unit, int queue)
{
    mmu_toq_state_mem0_entry_t   mmu_toq_state_mem0_entry = {{0}};
    SOC_IF_ERROR_RETURN(READ_MMU_TOQ_STATE_MEM0m(unit,
                         MEM_BLOCK_ANY, queue,
                         &mmu_toq_state_mem0_entry));
    if (soc_mem_field32_get(unit, MMU_TOQ_STATE_MEM0m,
                           &mmu_toq_state_mem0_entry,
                           EMPTYf) &&
        soc_mem_field32_get(unit, MMU_TOQ_STATE_MEM0m,
                           &mmu_toq_state_mem0_entry,
                           QBLOCK_NEXT_VLDf)) {
        soc_mem_field32_set(unit, MMU_TOQ_STATE_MEM0m,
                            &mmu_toq_state_mem0_entry, WS_VLDf, 0);
        soc_mem_field32_set(unit, MMU_TOQ_STATE_MEM0m,
                           &mmu_toq_state_mem0_entry, WHITESPACEf, 0);
        soc_mem_field32_set(unit, MMU_TOQ_STATE_MEM0m,
                            &mmu_toq_state_mem0_entry, TAIL_OFFSETf, 0);
        soc_mem_field32_set(unit, MMU_TOQ_STATE_MEM0m,
                            &mmu_toq_state_mem0_entry, TAIL_BUFFERf, 0);
        soc_mem_field32_set(unit, MMU_TOQ_STATE_MEM0m,
                            &mmu_toq_state_mem0_entry, Q_WRPTRf, 0);
        soc_mem_field32_set(unit, MMU_TOQ_STATE_MEM0m,
                            &mmu_toq_state_mem0_entry, Q_RDPTRf, 0);
        soc_mem_field32_set(unit, MMU_TOQ_STATE_MEM0m,
                            &mmu_toq_state_mem0_entry, QBLOCK_NEXT_VLDf, 0);
        soc_mem_field32_set(unit, MMU_TOQ_STATE_MEM0m,
                            &mmu_toq_state_mem0_entry, QBLOCK_NEXTf, 0);
        soc_mem_field32_set(unit, MMU_TOQ_STATE_MEM0m,
                            &mmu_toq_state_mem0_entry, PQ_DISABLEDf, 0);
        soc_mem_field32_set(unit, MMU_TOQ_STATE_MEM0m,
                            &mmu_toq_state_mem0_entry, HEAD_OFFSETf, 0);
        soc_mem_field32_set(unit, MMU_TOQ_STATE_MEM0m,
                            &mmu_toq_state_mem0_entry, HEAD_BUFFERf, 0);
        SOC_IF_ERROR_RETURN(WRITE_MMU_TOQ_STATE_MEM0m(unit,
                            MEM_BLOCK_ANY, queue,
                            &mmu_toq_state_mem0_entry));
    }
    return SOC_E_NONE;
                ...
                for (word = 0; word < word_count; word++) {
                    ...
                    for (bit = 0; bit < SHR_BITWID; bit++) {
                        ...
                        SOC_IF_ERROR_RETURN(WRITE_MMU_THDO_QOFFSET_CELLm(unit,
                                            MEM_BLOCK_ALL,queue,
                                            &mmu_thdo_qoffset_cell_entry));
                        SOC_IF_ERROR_RETURN(_bcm_saber2_reset_toq(unit, queue));
                    }
                }
            }
        }

        SOC_IF_ERROR_RETURN(_bcm_saber2_mmu_dest_port_cfg_init(unit, port));
    } else {
        /* 2.2 External Buffer */
        if (SOC_PBMP_MEMBER(PBMP_EXT_MEM (unit), port)) {
            return SOC_E_NONE;
