...
const char *alpm_prj_fail_name[] = {
    "error returned",
    "multi-dimension routes in shared resource",
    "actual max usage too small",
    "multi-dimension routes in shared resource and actual max usage too small",
};

#define ALPM_PRJ_BASED_ROUTES   0
#define ALPM_PRJ_BASED_TEST_DB  1
#define ALPM_PRJ_BASED_CONFIG   2

const char *alpm_prj_base_name[] = {
    "based on current route trend projection,",
    "based on test DB result interpolation,  ",
    "based on current configuration setting, ",
};
...
int
alpm_route_projection(int u)
{
    int rv = BCM_E_NONE;
    int grp, prj_route = 0, max_lvl = 0, max_usage = 0, reason = 0;

    for (grp = 0; grp < bcmL3RouteGroupCounter; grp++) {
        rv = bcm_esw_alpm_route_grp_projection(u, grp, &prj_route,
                                               &max_lvl, &max_usage, &reason);

        if (BCM_FAILURE(rv)) { /* default unavailable */
            if (ALPM_MODE_CHK(u, BCM_ALPM_MODE_COMBINED)) {
                ...
                cli_out("%s route projection unavailable due to: %s\n",
                        alpm_route_comb_name[grp], alpm_prj_fail_name[reason]);
            } else {
                cli_out("%s route projection unavailable due to: %s\n",
                        alpm_route_grp_name[grp], alpm_prj_fail_name[reason]);
            }
            continue;
        }
        ...
        if (ALPM_MODE_CHK(u, BCM_ALPM_MODE_COMBINED)) {
            ...
            cli_out("%s Max Routes:%7d (%s max L%d_usage:%3d%%)\n",
                    alpm_route_comb_name[grp], prj_route, alpm_prj_base_name[reason],
                    max_lvl + 1, max_usage);
        } else {
            cli_out("%s Max Routes:%7d (%s max L%d_usage:%3d%%)\n",
                    alpm_route_grp_name[grp], prj_route, alpm_prj_base_name[reason],
                    max_lvl + 1, max_usage);
        }
    }
...
int
bcm_esw_alpm_route_grp_projection(int u, bcm_l3_route_group_t grp, int *prj_route,
                                  int *def_lvl, int *def_usage, int *reason)
{
    int i, route_cnt, ipt_route_cnt, ghi_route_cnt;
    int grp_comb; /* same ipt, opposite combined grp (private vs global) */
    int grp_ipt, grp_ipt_comb; /* opposite ipt grp, opposite opt & combined grp */
    int grp_ghi; /* same ipt, global high grp */
    int max_lvl, max_usage, max_usage100, lv_usage;
    int multi_v4v6_share, multi_ghi_share;
    ...
    *reason = 0;
    ...
    ipt_route_cnt = info->res[grp_ipt].route_cnt; /* opposite ipt grp route_cnt */
    ...
    if (ALPM_MODE_CHK(u, BCM_ALPM_MODE_COMBINED)) {
        ...
        ipt_route_cnt += info->res[grp_ipt_comb].route_cnt;
    }
    ...
    /* Multi-dinmensional routes in V4V6 shared resource */
    multi_v4v6_share = ((ALPM_128B(u) && (ALPM_BKT_RSVD(u) || (max_lvl == 0)))  ? 0 : 1) &&
                       (ipt_route_cnt > 1);

    /* Multi-dimensional routes shared with GHI routes when max_lvl=0 */
    multi_ghi_share = (max_lvl == 0) && (ghi_route_cnt > 1);

    /* Use default test DB interpolation for multi-dimension routes in
       shared resource or max_usage too small */
    if (multi_v4v6_share || multi_ghi_share || max_usage < ALPM_PRJ_MIN_USAGE) {
        int cur_cnt_total[] = {res->lvl_usage[0].cnt_total,
                               res->lvl_usage[1].cnt_total,
                               res->lvl_usage[2].cnt_total};

        rv = alpm_default_usage_get(u, grp, prj_route, def_lvl, def_usage, cur_cnt_total);
        if (BCM_FAILURE(rv)) {
            *prj_route = route_cnt;
            *def_lvl = max_lvl;
            *def_usage = max_usage;

            /* Coded reason: bit0 - multi-dimension routes in shared resource
               and bit1 - max_usage too small */
            *reason = (multi_v4v6_share || multi_ghi_share) |
                      ((max_usage < ALPM_PRJ_MIN_USAGE) << 1);
            return rv;
        }
        *reason = ALPM_PRJ_BASED_TEST_DB;
    } else {
        /* Otherwise use projection formula: prj_route = route_cnt*limit_usage/max_usage */
