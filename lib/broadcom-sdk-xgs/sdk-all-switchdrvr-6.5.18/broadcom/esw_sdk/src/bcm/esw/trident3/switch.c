...
/*
 * Function:
 *      _bcm_td3_ioam_e2e_type_set
 * Purpose:
 *      Set ioam e2e type value.
 * Parameters:
 *      unit - SOC unit number.
 *      ioam_e2e_type - IOAM e2E type value
 * Returns:
 *      BCM_E_XXX.
 */
int
_bcm_td3_ioam_e2e_type_set(int unit, int ioam_e2e_type)
{
    if ((ioam_e2e_type < 0) ||
        (ioam_e2e_type > 0xFFFF)) {
        return BCM_E_PARAM;
    }

    if (!SOC_REG_IS_VALID(unit, EGR_GPE_CONTROL_1r)) {
        return BCM_E_UNAVAIL;
    }

    if (SOC_REG_FIELD_VALID(unit, EGR_GPE_CONTROL_1r, NEXT_PROTOCOL_IOAM_E2Ef)){
        SOC_IF_ERROR_RETURN(
                soc_reg_field32_modify(unit, EGR_GPE_CONTROL_1r,
                    REG_PORT_ANY, NEXT_PROTOCOL_IOAM_E2Ef, ioam_e2e_type));
    }
...
/*
 * Function:
 *      _bcm_td3_ioam_e2e_type_get
 * Purpose:
 *      Get ioam e2e type value.
 * Parameters:
 *      unit - SOC unit number.
 *      ioam_e2e_type - Get IOAM e2E type value
 * Returns:
 *      BCM_E_XXX.
 */
int
_bcm_td3_ioam_e2e_type_get(int unit, int *ioam_e2e_type)
{
    int rv = BCM_E_NONE;
    uint32 reg32 = 0;
    ...
    SOC_IF_ERROR_RETURN(READ_EGR_GPE_CONTROL_1r(unit, &reg32));
    *ioam_e2e_type = soc_reg_field_get(unit, EGR_GPE_CONTROL_1r,
            reg32, NEXT_PROTOCOL_IOAM_E2Ef);
    return rv;
}
