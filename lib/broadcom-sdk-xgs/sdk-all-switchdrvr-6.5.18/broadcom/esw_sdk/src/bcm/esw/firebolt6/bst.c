 ...
 * Purpose: API to set and manage various BST resources for FB6
 *
 */
...
#define _FB6_MMU_PORTS_PER_PIPE     72
...
int soc_fb6_set_hw_intr_cb(int unit)
{
...
int _bst_fb6_thd_reg_index_get(int unit, bcm_bst_stat_id_t bid, int pipe,
                                    int hw_index, int *inst, int *index)
{
    if (bid == bcmBstStatIdEgrPortPoolSharedMcast) {
        ...
        mmu_port_base = _FB6_MMU_PORTS_PER_PIPE * pipe;
        mmu_port = hw_index / _FB6_MMU_NUM_POOL;
        phy_port = si->port_m2p_mapping[mmu_port + mmu_port_base];
        *inst = si->port_p2l_mapping[phy_port];
        *index = hw_index % _FB6_MMU_NUM_POOL;
    }

    return BCM_E_NONE;
}

int _bst_fb6_cmn_io_op(int unit, int op, int pipe, int hw_index,
                      ...
                      soc_field_t field, uint32 *resval)
{
    ...
    if ((resInfo->flags & _BCM_BST_CMN_RES_F_PORTED) &&
        (op & BST_OP_THD)) {
        BCM_IF_ERROR_RETURN(
            _bst_fb6_thd_reg_index_get(unit, bid, pipe, hw_index,
                                      &port, &reg_index));
    }
...
bcm_bst_fb6_init(int unit)
{
    ...
    bst_info->hw_intr_cb = soc_fb6_set_hw_intr_cb;
    bst_info->port_to_mmu_inst_map = _bst_td3_port_to_mmu_inst_map_get;
    bst_info->bst_cmn_io_op = _bst_fb6_cmn_io_op;
    ...
    /* Get the mem attributes(Max idx) from the first child,
     * which could be used for other instances.
     * Note: In FB6, XPE 0/Pipe 0 instance valid for all memories(Ing/Egr).
     */
    child_mem = SOC_MEM_UNIQUE_ACC_XPE_PIPE(unit, stat_mem, 0, 0);
    idx_count = soc_mem_index_max(unit, child_mem) + 1;
