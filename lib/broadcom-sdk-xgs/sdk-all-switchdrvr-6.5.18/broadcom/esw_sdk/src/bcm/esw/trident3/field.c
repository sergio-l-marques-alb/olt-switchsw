...
_bcm_field_td3_flex_qual_ceh_db_init(int unit)
{
    ...
    _FP_QUAL_FLEX_DB_ADD(unit, bcmFieldQualifyVlanFormat,               0, 1, 1, 0, 3, 1, "Z1_MATCH_ID", "\0",          "OTAG_ITAG",      "\0");
    ...
    switch (f_ent->group->stage_id) {
        ...
        case _BCM_FIELD_STAGE_EGRESS:
           switch (fa->action) {
              ...
              case bcmFieldActionSubportSvtagSignature:
              case bcmFieldActionSubportSvtagPayloadOffset:
              case bcmFieldActionSubportSvtagSubportNum:
                  if (fa->param[0] >> 8) {
                      return BCM_E_PARAM;
                  }
                  return BCM_E_NONE;
              case bcmFieldActionSubportSvtagPrio:
                  if (fa->param[0] >> 1) {
                      return BCM_E_PARAM;
                  }
                  return BCM_E_NONE;
              case bcmFieldActionSubportSvtagDeviceId:
                  if (fa->param[0] >> 2) {
                      return BCM_E_PARAM;
                  }
                  return BCM_E_NONE;
              case bcmFieldActionSubportSvtagPktType:
                switch (fa->param[0]) {
                    case bcmFieldMacSecPktTypeMacSecEncrypted:
                    case bcmFieldMacSecPktTypeControlPkt:
    ...
    switch (f_ent->group->stage_id) {
        ...
        case _BCM_FIELD_STAGE_EGRESS:
             switch (action) {
                  ...
                  case bcmFieldActionSubportSvtagPktType:
                  case bcmFieldActionSubportSvtagSubportNum:
                  case bcmFieldActionSubportSvtagEnable:
    ...
    switch (fa->action) {
        ...
        case bcmFieldActionSubportSvtagSignature:
            for (fa_temp = f_ent->actions;
                (NULL != fa_temp) && (bcmFieldActionSubportSvtagPayloadOffset != fa_temp->action);
        ...
        case bcmFieldActionSubportSvtagPayloadOffset:
            for (fa_temp = f_ent->actions;
                (NULL != fa_temp) && (bcmFieldActionSubportSvtagSignature != fa_temp->action);
                fa_temp = fa_temp->next);
            value = 0;
            if (NULL != fa_temp) {
        ...
        case bcmFieldActionSubportSvtagSubportNum:
            ...
            /*
             * bcmFieldActionSubportSvtagSubportNum and bcmFieldActionSubportSvtagPktType are not
             * conflicts as both can be present and bcmFieldActionSubportSvtagSubportNum programs
             * whats needed for bcmFieldActionSubportSvtagPktType as well.
             */
            /* Index to EFP_FLEX_DATA_MASKm */
...
_field_td3_stage_quals_ibus_map_init(int unit, _field_stage_t *stage_fc)
{
    ...
    if (SOC_IS_FIREBOLT6(unit)) {
        BCMI_KEYGEN_QUAL_CFG_OFFSET_ADD(1, BCMI_KEYGEN_EXT_SECTION_L1E2, 28, 56, 1);
    }
    BCMI_KEYGEN_QUAL_CFG_INSERT(unit, db, bcmFieldQualifyVrf, qual_flags);

    BCMI_KEYGEN_QUAL_CFG_INIT(unit);
    /* Get sufficient Bitmap width needed for the depth of VRF table */
    BCMI_KEYGEN_IF_ERR_EXIT(_bcm_field_mem_width_bmp_get(unit, VRF_ATTRS_2m, &width));
    BCMI_KEYGEN_QUAL_CFG_OFFSET_ADD(0, BCMI_KEYGEN_EXT_SECTION_L1E16, 17, 272, width);
    BCMI_KEYGEN_QUAL_CFG_INSERT(unit, db, bcmFieldQualifyVrf, qual_flags);
    ...
    if (no_pbm_access == 0) {
       ...
       for (inst = 0; inst < num_pipe; inst++) {
          BCM_PBMP_ITER(_f_pbmp[inst].mask, port) {
              ...
              if (port < _FP_TD3_TCAM_IPBMP_SIZE) {
                 BCM_PBMP_PORT_ADD(ipbm_mask[inst], port);

                 if (set_data == 1) {
                    BCM_PBMP_PORT_ADD(ipbm_data[inst], port);
                 }
                 ipbm_hit = 1;
              } else if (port < (_FP_TD3_TCAM_IPBMP_SIZE + _FP_FLEX_PBMP_SIZE)) {
                 temp_p = port - _FP_TD3_TCAM_IPBMP_SIZE;
                 ...
                 opq1_hit = 1;
              } else if (port < (_FP_TD3_TCAM_IPBMP_SIZE + (_FP_FLEX_PBMP_SIZE * 2))) {
                 temp_p = port - (_FP_TD3_TCAM_IPBMP_SIZE + _FP_FLEX_PBMP_SIZE);
                               ...
                               bcm_pbmp_t mask)
{
    ...
    if (bcmFieldGroupOperModePipeLocal == stage_fc->oper_mode) {
       ...
       BCM_PBMP_ITER(mask, port) {
          ipbm_hit = opq1_hit = opq2_hit = 0;
          temp_p = port - (_BCM_TD3_NUM_PIPE_PORTS * fg->instance);
          ...
          if (temp_p < _FP_TD3_TCAM_IPBMP_SIZE) {
             BCM_PBMP_PORT_ADD(ipbm_mask, temp_p);

             if (set_data == 1) {
                BCM_PBMP_PORT_ADD(ipbm_data, temp_p);
             }
             ipbm_hit = 1;
          } else if (temp_p < (_FP_TD3_TCAM_IPBMP_SIZE + _FP_FLEX_PBMP_SIZE)) {
             temp_p = temp_p - _FP_TD3_TCAM_IPBMP_SIZE;

             if (set_data == 1) {
                pbm_34_49 |= 1 << temp_p;
             } 

             pbm_34_49_mask |= 1 << temp_p;

             opq1_hit = 1;   
          } else if (temp_p < (_FP_TD3_TCAM_IPBMP_SIZE + (_FP_FLEX_PBMP_SIZE * 2))) {
             temp_p = temp_p - (_FP_TD3_TCAM_IPBMP_SIZE + _FP_FLEX_PBMP_SIZE);

             if (set_data == 1) {
                pbm_50_65 |= 1 << temp_p;
             }

             pbm_50_65_mask |= 1 << temp_p;
             opq2_hit = 1;
          } else {
             LOG_ERROR(BSL_LS_BCM_FP, (BSL_META(
          ...
          LOG_DEBUG(BSL_LS_BCM_FP, (BSL_META(
                 ...
                 pbm_50_65_mask, ((ipbm_hit == 1) ? temp_p : (1 << temp_p))));
