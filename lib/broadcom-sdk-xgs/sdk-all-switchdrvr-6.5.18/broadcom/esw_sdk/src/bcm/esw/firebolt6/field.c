    ...
    uint32 *e_buf)
{
    ...
    if (aft_group_id != -1) {
        ...
        if (SOC_MEM_FIELD_VALID(unit, policy_mem, ft_idx_valid_fld)) {
            value = (action_aft_index_set == TRUE) ? 1: 0;
            soc_mem_field_set(unit, policy_mem, (void *) e_buf,
    ...
    bcm_gport_t queue_gport)
{
    ...
    if (soc_feature(unit, soc_feature_field_flowtracker_v3_support)) {
        if (BCM_GPORT_IS_UCAST_QUEUE_GROUP(queue_gport) ||
                (BCM_GPORT_IS_MCAST_QUEUE_GROUP(queue_gport))) {
            BCM_IF_ERROR_RETURN(
                    _bcm_esw_gport_resolve(unit, queue_gport,
                        NULL, NULL, NULL, &id));
        } else {
            return BCM_E_PARAM;
        }

        rv = _field_qualify32(unit, entry, bcmFieldQualifyCosQueueId,
                (uint32) id, -1);
    }
#endif
    ...
    bcm_gport_t *queue_gport)
{
    ...
    if (soc_feature(unit, soc_feature_field_flowtracker_v3_support)) {
        ...
        mc_queue_base = SOC_FB6_NUM_UC_QUEUES_PER_PIPE;

        /* Construct gport */
        if (data >= mc_queue_base) {
            BCM_GPORT_UCAST_QUEUE_GROUP_SET(*queue_gport, data);
        } else {
            data -= mc_queue_base;
            BCM_GPORT_MCAST_QUEUE_GROUP_SET(*queue_gport, data);
        }
    }
#endif
                                              ...
                                              _bcm_field_basic_info_key_t *key_info)
{
    ...
    while (cid_list) {
        if (0 == sal_memcmp(cid_list->cid.w, key_info->cid.w, sizeof(_bcm_field_cid_t))) {
            found = 1;
            break;
        }
    ...
    if (found) {
        ...
        cid_list->num_entries += 1;
        sal_memcpy(cid_list->cid.w, key_info->cid.w, sizeof(_bcm_field_cid_t));
        if (cid_list_prev) {
                                     ...
                                     void *user_data)
{
            ...
            while (cid_list) {
                if (0 == sal_memcmp(cid_list->cid.w, list_iter->old_cid.w, sizeof(_bcm_field_cid_t))) {
                    for (i = 0; i < cid_list->num_entries; i++) {
                        ...
                        if (SHR_BITEQ_RANGE(f_ent->key_info[*trie_type].key.w, list_iter->key.w,
                                            _bcm_field_fb6_trie_info[unit][*trie_type].key_width - list_iter->prefix,
                                            list_iter->prefix) &&
                            list_iter->prefix == f_ent->key_info[*trie_type].prefix) {
    ...
    while (cid_list) {
        ...
        while (list_iter) {
            if (0 == sal_memcmp(cid_list->cid.w, list_iter->old_cid.w, sizeof(_bcm_field_cid_t))) {
                /* Found the cid */
                sal_memcpy(cid_list->cid.w, list_iter->new_cid.w, sizeof(_bcm_field_cid_t));
                for (i = 0; i < cid_list->num_entries; i++) {
                    /* Update all entries using that cid */
                    BCM_IF_ERROR_RETURN
                        (_field_entry_get(unit, cid_list->entries[i], _FP_ENTRY_PRIMARY, &f_ent));

                    sal_memcpy(f_ent->key_info[*trie_type].cid.w, list_iter->new_cid.w, sizeof(_bcm_field_cid_t));
                    sal_memcpy(f_ent->key_info[*trie_type].cid_mask.w, list_iter->new_cid_mask.w, sizeof(_bcm_field_cid_t));

                    if (!(f_ent->key_info[*trie_type].flags & _BCM_FIELD_TRIE_NODE_LIST_COMPRESSED)) {
                        BCM_IF_ERROR_RETURN
                            (_bcm_field_th_qualify_set(unit, f_ent->eid,
                                                       _bcm_field_fb6_trie_info[unit][*trie_type].iqual,
                                                       list_iter->new_cid.w, list_iter->new_cid_mask.w,
                                                       _FP_QUALIFIER_ADD));
                                                ...
                                                _field_entry_t **f_ent_lc)
{
    ...
    for (idx = 0; idx < fg->group_status.entry_count; idx++) {
        ...
        action_dirty = 0;
        if (BCM_FAILURE(_field_entry_action_dirty(unit, f_ent, &action_dirty))) {
            continue;
        }
        dirty = 0;
        if (BCM_FAILURE(_field_entry_dirty(unit, f_ent, &dirty))) {
            continue;
        }
        ...
        for (trie_type = 0; trie_type < _BCM_FIELD_COMPRESS_TYPE_MAX; trie_type++) {
            if ((f_ent->key_info[trie_type].ref_count > 0 &&
                 f_ent_iter->key_info[trie_type].ref_count == 0) ||
                (f_ent->key_info[trie_type].ref_count == 0 &&
                 f_ent_iter->key_info[trie_type].ref_count > 0)) {
                continue;
            }
        }
...
_bcm_field_fb6_compress_entry_install(int unit,  _field_entry_t *f_ent)
{
    ...
    int trie_type, parts_count = 0, i;

    BCM_IF_ERROR_RETURN(_field_stage_control_get(unit, f_ent->group->stage_id,
                                                 &stage_fc));

    if (f_ent->flags & _FP_ENTRY_INSTALLED) {
        /* If entry already installed, remove existing key
         * This is needed so the cid can be freed and re-used
         */
    ...
    for (trie_type = 0; trie_type < _BCM_FIELD_COMPRESS_TYPE_MAX; trie_type++) {
        if (f_ent->key_info[trie_type].ref_count) {
            ...
            BCM_IF_ERROR_RETURN
                (_bcm_field_compress_key_insert(unit, &f_ent->key_info[trie_type],
            ...
            if (f_ent->key_info[trie_type].flags & _BCM_FIELD_TRIE_NODE_LIST_COMPRESSED) {
                ...
                BCM_IF_ERROR_RETURN
                    (_bcm_field_th_qualify_set(unit, f_ent->eid,
                                               _bcm_field_fb6_trie_info[unit][trie_type].iqual,
                                               f_ent->key_info[trie_type].cid.w,
                                               f_ent->key_info[trie_type].cid_mask.w,
                                               _FP_QUALIFIER_ADD));
            }
        }
    }
...
_bcm_field_fb6_compress_entry_remove(int unit, _field_entry_t *f_ent)
{
    ...
    for (trie_type = 0; trie_type < _BCM_FIELD_COMPRESS_TYPE_MAX; trie_type++) {
        if (f_ent->key_info[trie_type].flags & _BCM_FIELD_TRIE_NODE_LIST_COMPRESSED) {
            /* List compressed entries are not actually installed in H/W */
...
_bcm_field_fb6_compress_entry_destroy(int unit, _field_entry_t *f_ent)
{
    ...
    for (trie_type = 0; trie_type < _BCM_FIELD_COMPRESS_TYPE_MAX; trie_type++) {
        if (f_ent->key_info[trie_type].ref_count) {
            rv = _bcm_field_fb6_compress_basic_info_key_delete(unit, f_ent->eid, trie_type,
                                                               &f_ent->key_info[trie_type]);
            if (rv == BCM_E_NOT_FOUND) {
                /* Key not inserted in trie */
                return BCM_E_NONE;
            } else {
                return rv;
            }
            ...
            if (f_ent->key_info[trie_type].flags & _BCM_FIELD_TRIE_NODE_LIST_COMPRESSED) {
                ...
                BCM_IF_ERROR_RETURN
                    (_bcm_field_entry_tcam_parts_count(unit, f_ent->group->stage_id,
                                                       f_ent->group->flags, &parts_count));
                for (i = 0; i < parts_count; i++) {
                    /* free_count was already incremented for list compressed entry
                     * Decrement the free_count here to keep it in sync
                     */
                    f_ent[i]->fs[i].free_count--;
                }
            }
                                    ...
                                    uint32 *data, uint32 *mask)
{
    ...
    for (i = 0; i <  _bcm_field_fb6_trie_info[unit][trie_type].num_qual; i++) {
        if (qual == _bcm_field_fb6_trie_info[unit][trie_type].qual[i]) {
            ...
            SHR_BITCOPY_RANGE(f_ent->key_info[trie_type].key.w,
                              _bcm_field_fb6_trie_info[unit][trie_type].start_bit[i],
                              data, 0,
                              _bcm_field_fb6_trie_info[unit][trie_type].width[i]);
            SHR_BITCOPY_RANGE(f_ent->key_info[trie_type].key_mask.w,
                              _bcm_field_fb6_trie_info[unit][trie_type].start_bit[i],
                              mask, 0,
                              _bcm_field_fb6_trie_info[unit][trie_type].width[i]);
            SHR_BITCOUNT_RANGE(f_ent->key_info[trie_type].key_mask.w,
                               prefix, 0,
                               _bcm_field_fb6_trie_info[unit][trie_type].key_width);
            f_ent->key_info[trie_type].prefix = (uint8)prefix;

            /* Keep track of which compression keys are valid for this entry */
            f_ent->key_info[trie_type].ref_count = 1;
        }
    }
                                                ...
                                                _bcm_field_qual_data_t mask)
{
    ...
    for (i = 0; i <  _bcm_field_fb6_trie_info[unit][trie_type].num_qual; i++) {
        if (qual == _bcm_field_fb6_trie_info[unit][trie_type].qual[i]) {
            SHR_BITCOPY_RANGE(data, 0, f_ent->key_info[trie_type].key.w,
                              _bcm_field_fb6_trie_info[unit][trie_type].start_bit[i],
                              _bcm_field_fb6_trie_info[unit][trie_type].width[i]);

            SHR_BITCOPY_RANGE(mask, 0, f_ent->key_info[trie_type].key_mask.w,
                              _bcm_field_fb6_trie_info[unit][trie_type].start_bit[i],
                              _bcm_field_fb6_trie_info[unit][trie_type].width[i]);
        }
    }
...
_field_fb6_compress_init(int unit, _field_stage_t *stage_fc)
{
    ...
    for (trie_type = 0; trie_type < _BCM_FIELD_COMPRESS_TYPE_MAX; trie_type++) {
        ...
        if (trie_type == _BCM_FIELD_COMPRESS_TYPE_SRC_IPV4) {
            ...
            _bcm_field_fb6_trie_info[unit][trie_type].width[0] = 32;
            _bcm_field_fb6_trie_info[unit][trie_type].start_bit[0] = 0;
            _bcm_field_fb6_trie_info[unit][trie_type].key_width = 32;
            ...
            _bcm_field_fb6_trie_info[unit][trie_type].width[0] = 128;
            _bcm_field_fb6_trie_info[unit][trie_type].start_bit[0] = 0;
            _bcm_field_fb6_trie_info[unit][trie_type].key_width = 128;
