                               ...
                               sal_usecs_t timeout)
{
    ...
    if (soc_feature(unit, soc_feature_cmicx)) {
        ...
        send64.s.mclass = MOS_MSG_CLASS_BFD;
        ...
        if (MOS_MSG_DMA_MSG(s_subclass) ||
            MOS_MSG_DMA_MSG(r_subclass)) {
            send64.s.data = bcm_htonl(PTR_TO_INT(buf_paddr));
        } else {
            send64.s.data = bcm_htonl(s_data);
        }
        rv = soc_cmic_uc_msg64_send_receive(unit, bfd_info->uc_num,
                                            &send64, &reply64, timeout);
        if (rv != SOC_E_NONE){
        ...
        *r_len = bcm_ntohs(reply64.s.len);
        ...
        if((rv == SOC_E_NONE) && ((reply64.s.mclass != MOS_MSG_CLASS_BFD) ||
...
#ifdef BCM_CMICX_SUPPORT
        if (soc_feature(unit, soc_feature_cmicx)) {
            rv = soc_cmic_uc_msg64_receive(bfd_info->unit, bfd_info->uc_num,
                                         MOS_MSG_CLASS_BFD_EVENT, &event_msg64,
                                         sal_sem_FOREVER);
            ...
            sess_id    = bcm_ntohs(event_msg64.s.len);
            event_mask = bcm_ntohl(event_msg64.s.data);
        } else
#endif
        {
