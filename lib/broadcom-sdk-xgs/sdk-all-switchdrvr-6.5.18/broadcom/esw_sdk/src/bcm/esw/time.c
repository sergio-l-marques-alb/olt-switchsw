...
STATIC
soc_reg_t  _bcm_time_gpio_regs[TS_GPIO_COUNT] = { NS_TIMESYNC_GPIO_0_CTRLr,
...
STATIC
soc_reg_t  _bcm_time_bs_pll_regs[2] = { NS_BROADSYNC0_CLK_EVENT_CTRLr,
...
_bcm_esw_time_hw_clear(int unit, bcm_time_if_t intf_id)
{
    ...
    if (SOC_HAS_TIME_V3_OR_IPROC_MANAGED_BROADSYNC(unit)) {
        if (!soc_feature(unit, soc_feature_time_v3_no_bs)) {
            if(SOC_IS_TOMAHAWK3(unit) || SOC_IS_MONTEREY(unit) ||  SOC_IS_HURRICANE4(unit)) {
...
_bcm_esw_time_interface_input_validate(int unit, bcm_time_interface_t *intf)
{
    ...
    if (soc_feature(unit, soc_feature_time_v3) &&
        ...
        !SOC_IS_HELIX5(unit) && !(intf->flags & BCM_TIME_SYNC_STAMPER)) {
        /*
...
bcm_esw_time_interface_add (int unit, bcm_time_interface_t *intf)
{
    ...
    if (intf->flags & BCM_TIME_SYNC_STAMPER) {
        ...
        } else if (SOC_HAS_TIME_V3_OR_IPROC_MANAGED_BROADSYNC(unit)) {
            uint32 regval;
            /* in case of broadsync, enable common control for both counters */
...
#define BCM_TIME_HX4_PORT_VALID(x) (((x) >= 1 && (x) <= 48 ) || ((x)>=65 && (x) <= 66))
...
bcmi_esw_time_hr4_synce_L1_rcvd_clk_port_get(int phy_port)
{
    ...
    else if (phy_port >= 25 && phy_port <= 28) {
        /* PM4x10Q_0           :  8 SyncE Clocks */
        return (phy_port - 1);
    }
    else if (phy_port >= 41 && phy_port <= 44) {
        /* PM4x10Q_1           :  4 SyncE Clocks */
        return (phy_port - 12 - 1);
    }
    else if (phy_port >= 57 && phy_port <= 64) {
        /* PM4x25_0 + PM4x10_0 :  4 SyncE Clocks */
...
/* Check if lport is in gmii mode or not. */
STATIC int
    ...
    int lport)
{
    ...
    if (SOC_IS_HELIX5(unit)) {
        /* port_gmii_mode_1/_17/_33 not supported */
        if (phy_port <= 48) {
...
#if defined(BCM_HURRICANE4_SUPPORT)
    if (SOC_IS_HURRICANE4(unit)) {
        /* port_gmii_mode_25/_41 not supported */
        if (25 <= phy_port && phy_port <= 56) {
            /* PM4x10Q_0: physical port 25-40 */
            if (soc_property_phys_port_get(unit, 25, spn_PORT_GMII_MODE, 0)) {
                return TRUE;
            }
            /* PM4x10Q_1: physical port 41-56 */
            if (soc_property_phys_port_get(unit, 41, spn_PORT_GMII_MODE, 0)) {
                return TRUE;
            }
        }
    }
                              ...
                              bcm_time_synce_divisor_setting_t *divisor)
{
    ...
    if (val < BCMI_TIME_L1_RCVD_CLK_PORT_SEL_MAX) {
        ...
        divisor->input_src = bcmTimeSynceInputSourceTypePLL;
        divisor->index = val - BCMI_TIME_L1_RCVD_CLK_PORT_SEL_MAX;
    }
                               ...
                               bcm_time_synce_divisor_setting_t *divisor)
{
    ...
    SOC_IF_ERROR_RETURN
        (soc_reg_field32_modify(unit, synce_ctrl, REG_PORT_ANY,
                                    l1_rcvd_sw_ovwr_valid, 1));
    SOC_IF_ERROR_RETURN
        (soc_reg_field32_modify(unit, synce_ctrl, REG_PORT_ANY,
                                     l1_rcvd_sw_ovwr_enable, 1 ));
    /* TOP_MISC_CONTROL_2 */
    SOC_IF_ERROR_RETURN
        (soc_reg_field32_modify(unit, synce_ctrl, REG_PORT_ANY,
                                l1_rcvd_clk_rstn, 1));
...
/*
 * Function:
 *      _bcm_esw_time_synce_clock_source_control_get
 * Purpose:
...

{
    ...
    if((clk_src_config->input_src == bcmTimeSynceInputSourceTypePort) &&
       (bcmi_esw_time_port_is_gmii_mode(unit, clk_src_config->port) == TRUE)) {
        return BCM_E_CONFIG;
    }
    if (SOC_IS_HELIX5(unit) || SOC_IS_HURRICANE4(unit)) {
        /* HELIX5 or HR4 */
...
#if defined(BCM_TOMAHAWK_SUPPORT) || defined(BCM_TRIDENT3_SUPPORT) || defined(BCM_APACHE_SUPPORT) || defined(BCM_MAVERICK2_SUPPORT) || defined(BCM_HELIX5_SUPPORT) || \
...

{
    ...
    if (SOC_IS_HELIX4(unit)) {  /* HELIX4 SyncE Freq Divisor*/
        if(!BCM_TIME_HX4_L1_FREQ_SEL_MODE(frequency)) {
             return BCM_E_PARAM;
         }
...
#ifdef PHYMOD_VIPER_SUPPORT
                    if (type == phymodDispatchTypeViper) {
                        break;
                    }
#endif
                default:
...
#endif /* TH TD3 AP MV2 HX5 support */
...
/*
 * Function:
 *      bcmi_time_tod_iproc_ptp_set
 * Purpose:
 *      Internal function to set PTP tod values to iProc.
 * Parameters:
 *      unit    - (IN)  Unit number.
    ...
    bcm_time_tod_t *tod)
{
    ...
    SOC_IF_ERROR_RETURN(
        soc_reg_field32_modify(unit, NS_PTP_TOD_A_VALUE_2r,
                               REG_PORT_ANY, VALf, data_sec[1]));
    SOC_IF_ERROR_RETURN(
        soc_reg_field32_modify(unit, NS_PTP_TOD_A_VALUE_1r,
                               REG_PORT_ANY, VALf, data_sec[0]));

    /* PTP.TOD[31:0] Value. */
    SOC_IF_ERROR_RETURN(
        soc_reg_field32_modify(unit, NS_PTP_TOD_A_VALUE_0r,
                               REG_PORT_ANY, VALf, tod->ts.nanoseconds));
...
/*
 * Function:
 *      bcmi_time_tod_iproc_ntp_set
 * Purpose:
 *      Internal function to set NTP tod values to iProc.
 * Parameters:
 *      unit    - (IN)  Unit number.
 *      tod     - (IN)  Time of the day values.
 * Returns:
    ...
    bcm_time_tod_t *tod)
{
    ...
    /* NTP.TOD[63:32] Value. */
    COMPILER_64_TO_32_LO(data_sec, tod->ts.seconds);
    SOC_IF_ERROR_RETURN(
        soc_reg_field32_modify(unit, NS_NTP_TOD_VALUE_1r,
                               REG_PORT_ANY, VALf, data_sec));

    /* NTP.TOD[31:0] Value. */
    SOC_IF_ERROR_RETURN(
        soc_reg_field32_modify(unit, NS_NTP_TOD_VALUE_0r,
                               REG_PORT_ANY, VALf, tod->ts.nanoseconds));
        ...
        bcm_time_tod_t *tod)
{
    ...
    bcmi_time_tod_stage_t stage;
          ...
          BCM_TOMAHAWK2_SUPPORT || BCM_MAVERICK2_SUPPORT */
    ...
    if (stages & BCM_TIME_STAGE_ALL) {
        for (stage = bcmi_time_tod_stage_ingress_ft;
            ...
            if (set) {
                BCM_IF_ERROR_RETURN
                    (bcmi_time_tod_set(unit, tod, stage));
            }
        }
        rv = BCM_E_NONE;
...
#if defined(BCM_TIME_NANOSYNC_SUPPORT)
        if (SOC_IS_MONTEREY(unit) || SOC_IS_TOMAHAWK3(unit)) {
            BCM_TIME_WRITE_NS_REGr(unit, NS_TS_INT_ENABLEr, 0, INTR_NS_TIME_CAPTURE);
        }
#ifdef BCM_CMICX_SUPPORT
        ...
        bcm_time_capture_cb_type_t cb_type)
{
    ...
    if (cb_overflow.cb == NULL && cb_fill.cb == NULL && cb_all.cb == NULL){
        ...
        if (soc_feature(unit, soc_feature_timesync_nanosync)) {
            ...
            BCM_TIME_WRITE_NS_REGr(unit, NS_TS_INT_ENABLEr, 0, 0);
            } else {
       ...
       /* TS0_CNT_OFFSET_UPDATED interrupt */
        ...
        if (tsfifo_flag && soc_reg_field_get(unit, NS_TS_INT_STATUSr, g_int_status, TS_FIFO_NOT_EMPTYf)) {
            ...
            while (fifo_depth != 0) {
                ...
                upper = ((upper & 0xF) << 12) | ((middle >> 20) & 0xFFF);
...
bcm_esw_time_capture_ts_init(int unit)
{
    ...
    if (soc_feature(unit, soc_feature_cmicx)) {
        ...
        handle.num = TS_INTR;
#if defined(BCM_TIME_NANOSYNC_SUPPORT)
        if (soc_feature(unit, soc_feature_timesync_nanosync) || (SOC_IS_FIRELIGHT(unit))) {
...
{
    int rv = BCM_E_UNAVAIL;
#if defined (BCM_MONTEREY_SUPPORT) && defined(COMPILER_HAS_LONGLONG)
...
int _bcm_bs_dpll_chan_create(int unit, bcm_time_interface_t *intf) 
{
    ...
    if(master) {
        ...
        bs_slave_thread = sal_thread_create("BS Slave", SAL_THREAD_STKSZ, 100, _bs_slave_thread, intf);
