    ...
    hbmc_shmoo_bist_info_t * info)
{
    ...
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_DATA_MODE, INST_SINGLE, &info->data_mode);
...
/* see header file */
shr_error_e
dnx_hbmc_dbal_access_phy_control_set(
    int unit,
    int hbm_index,
    uint32 iddq)
{
    ...
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_HBMC_INDEX, hbm_index);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_HBM_PHY_IDDQ, INST_SINGLE, iddq ? 1 : 0);
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
...
/* see header file */
shr_error_e
dnx_hbmc_dbal_access_hcc_configuration_set(
    int unit)
{
    uint32 entry_handle_id;
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_HBM_TWR, INST_SINGLE,
                                 dnx_data_dram.general_info.timing_params_get(unit)->twr +
                                 dnx_data_dram.general_info.write_latency_get(unit) +
    ...
    SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_HCC_TSM_CONFIG, entry_handle_id));
    dbal_entry_key_field32_range_set(unit, entry_handle_id, DBAL_FIELD_HBMC_INDEX, DBAL_RANGE_ALL, DBAL_RANGE_ALL);
    dbal_entry_key_field32_range_set(unit, entry_handle_id, DBAL_FIELD_CHANNEL, DBAL_RANGE_ALL, DBAL_RANGE_ALL);
    ...
    SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_HCC_DATA_SOURCE_CONFIG, entry_handle_id));
    dbal_entry_key_field32_range_set(unit, entry_handle_id, DBAL_FIELD_HBMC_INDEX, DBAL_RANGE_ALL, DBAL_RANGE_ALL);
    dbal_entry_key_field32_range_set(unit, entry_handle_id, DBAL_FIELD_CHANNEL, DBAL_RANGE_ALL, DBAL_RANGE_ALL);
    ...
    uint32 *data_source_id)
{
    ...
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_DATA_SOURCE_ID, core, data_source_id);
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));
    ...
    int unit)
{
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LEAKY_BUCKET_FULL_SIZE, INST_SINGLE, 44);
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_TDU_CONFIG, &entry_handle_id));
    dram_bitmap = dnx_data_dram.general_info.dram_info_get(unit)->dram_bitmap;
    if (dram_bitmap == 0x2)
    {
        /** if dram bitmap has only second HBM, configure data source id in a different way */
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_DATA_SOURCE_ID, 1, 0);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_DATA_SOURCE_ID, 0, 1);
    }
    else
    {
        /** in any other occurrence configure in the regular way */
        for (int data_source = 0; data_source < dnx_data_device.general.nof_cores_get(unit); ++data_source)
        {
