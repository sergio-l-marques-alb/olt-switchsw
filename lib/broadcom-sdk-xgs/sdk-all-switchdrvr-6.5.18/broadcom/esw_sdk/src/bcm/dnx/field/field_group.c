    ...
    int *buffer_shift_p)
{
    ...
    if (dnx_data_field.features.exem_vmv_removable_from_payload_get(unit))
    {
        ...
        if (fg_type == DNX_FIELD_GROUP_TYPE_EXEM)
        {
            ...
            if ((*buffer_shift_p) != 0)
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "Buffer shift (%d) was done for EXEM other than for VMV shift.\n",
                             *buffer_shift_p);
            }
    ...
    dnx_field_group_info_t * fg_info_in_p)
{
         ...
         && fg_info_in_p->dnx_quals[qual_ndx] != DNX_FIELD_QUAL_TYPE_INVALID; qual_ndx++)
    {
        for (qual_ndx_2 = 0; qual_ndx_2 < qual_ndx; qual_ndx_2++)
        {
            ...
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "Qual 0x%x appears twice (in %d and %d places).\n",
         ...
         && fg_info_in_p->dnx_actions[action_ndx] != DNX_FIELD_ACTION_INVALID; action_ndx++)
    {
        for (action_ndx_2 = 0; action_ndx_2 < action_ndx; action_ndx_2++)
        {
            ...
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "Action 0x%x appears twice (in %d and %d places).\n",
    ...
    uint32 *found_segment_idx_p)
{
    ...
    *found_segment_idx_p = DNX_KBP_INVALID_SEGMENT_ID;
    ...
    /*
     * Ranged and non-ranged qualifiers cannot be shared at the moment.
     */
    SHR_IF_ERR_EXIT(dnx_field_group_dbal_table_id_get(unit, fg_id, &dbal_table_id));
    ...
    {
        if (already_shared_segments[seg_idx])
        {
            continue;
        }
        /*
         ...
         * Treat different user defined qualifiers as identical if they have the same size, unless we 
         * force qualifiers to be the same.
         */
        ...
        SHR_IF_ERR_EXIT(dnx_field_key_kbp_acl_compare_qualifiers_for_sharing
                        (unit, contender_qual_type, &qual_attach_info_arr[qual_in_key_template_idx], qual_type,
                         qual_info_p, &shareable));
        if (shareable == FALSE)
        {
            continue;
        }
        ...
        SHR_IF_ERR_EXIT(dnx_field_map_dnx_to_dbal_qual
                        (unit, DNX_FIELD_STAGE_EXTERNAL, contender_qual_type, &contender_qual_field_id));
        ...
        if ((is_qual_ranged && (is_contender_qual_ranged == FALSE))
            || (is_contender_qual_ranged && (is_qual_ranged == FALSE)))
        {
            continue;
        }

        /*
         * If we reached here, the qualifiers are shareable.
         */
        *found_segment_idx_p = seg_idx;
        LOG_DEBUG_EX(BSL_LOG_MODULE,
                     "Found ACL match with fg_id %d opcod_id %d qual %s seg_idx %d\n",
                     contender_fg_id, opcode_id, dnx_field_dnx_qual_text(unit, qual_type), seg_idx);
    ...
    uint32 *found_segment_idx_p)
{
    ...
    *found_segment_idx_p = DNX_KBP_INVALID_SEGMENT_ID;
    ...
    for (seg_idx = 0; (seg_idx < nof_fwd_segments) && (*found_segment_idx_p == DNX_KBP_INVALID_SEGMENT_ID); seg_idx++)
    {
        if (already_shared_segments[seg_idx])
        {
            continue;
        ...
        else if (!sal_strncmp(ms_key_segments[seg_idx].name, "GLOBAL_IN_LIF", sizeof(ms_key_segments[seg_idx].name)))
        {
            associated_dnx_qual =
        ...
        if (associated_dnx_qual != DNX_FIELD_QUAL_TYPE_INVALID)
        {
            ...
            if (BITS2BYTES(qual_size) != ms_key_segments[seg_idx].nof_bytes)
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "Segment \"%.*s\" is %d bytes, while qualifier %s is %d bytes.\r\n",
                             ...
                             ms_key_segments[seg_idx].nof_bytes, dnx_field_dnx_qual_text(unit, associated_dnx_qual),
             ...
             */
            SHR_IF_ERR_EXIT(dnx_field_key_kbp_acl_compare_qualifiers_for_sharing
                            (unit, associated_dnx_qual, &associated_attach_info, qual_type, qual_info_p, &shareable));
            if (shareable == FALSE)
            {
                continue;
            }
    ...
    uint32 *found_segment_idx_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_group_kbp_acl_segment_exist
                    (unit, opcode_id, fg_id, force_same_qual, qual_type, qual_info_p, already_shared_segments,
                     found_segment_idx_p));
    /*
     * If not found in the ACL part, Check if the qualifier (future segment) exists in the FWD part of the master key 
     */
    if ((*found_segment_idx_p == DNX_KBP_INVALID_SEGMENT_ID) && (force_same_qual == FALSE))
    {
        SHR_IF_ERR_EXIT(dnx_field_group_kbp_fwd_segment_exist
                        (unit, opcode_id, fg_id, qual_type, qual_info_p, already_shared_segments, found_segment_idx_p));
    }
    ...
    dnx_field_attach_context_info_t * context_info_p)
{
     ...
     */
    SHR_IF_ERR_EXIT(kbp_mngr_opcode_master_key_get(unit, opcode_id, &absolute_key_segs_idx, ms_key_segments_existing));
    LOG_DEBUG_EX(BSL_LOG_MODULE,
    ...
    for (sharing_iter = 0; sharing_iter < 2; sharing_iter++)
    {
        for (qual_idx = 0, lookup_seg_idx = nof_quals - 1; qual_idx < nof_quals; qual_idx++, lookup_seg_idx--)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_field_group_kbp_segment_exist(unit,
                                                              ...
                                                              key_template.key_qual_map[qual_idx].qual_type,
            ...
            if (segment_found != DNX_KBP_INVALID_SEGMENT_ID)
            {
                ...
                found_in_ms_key_seg_idx[lookup_seg_idx] = segment_found;
     ...
     */
    {
        ...
        {
            SHR_IF_ERR_EXIT(dnx_field_map_dnx_qual_size
                            (unit, field_stage, key_template.key_qual_map[qual_idx].qual_type, &qual_sizes[qual_idx]));
         ...
         */
        for (zero_padding_idx = 0; zero_padding_idx < nof_zero_padding_segments; zero_padding_idx++)
        {
            ...
            for (lookup_seg_idx = 0, qual_idx = nof_quals - 1; qual_idx >= 0; qual_idx--, lookup_seg_idx++)
            {
                if (found_in_ms_key_seg_idx[lookup_seg_idx] == DNX_KBP_INVALID_SEGMENT_ID &&
                    in_ms_key_utilizes_zero_padding_seg_idx[lookup_seg_idx] == DNX_KBP_INVALID_SEGMENT_ID &&
                    zero_padding_segment_size[smallest_zero_padding_idx] >= qual_sizes[qual_idx] &&
                    is_one_ffc_qual[qual_idx] &&
                    ((qual_sizes[qual_idx] > biggest_qualifier_size) ||
                     (biggest_qualifier_idx == DNX_KBP_INVALID_SEGMENT_ID)))
                {
                    biggest_qualifier_size = qual_sizes[qual_idx];
            ...
            sal_memset(&fg_segment_info, 0x0, sizeof(fg_segment_info));
            fg_segment_info.fg_id = fg_id;
            fg_segment_info.qual_idx = biggest_qualifier_idx;
            SHR_IF_ERR_EXIT(dnx_field_group_sw.kbp_info.master_key_info.
                            segment_info.set(unit, opcode_id, smallest_zero_padding_idx, &fg_segment_info));
     ...
     */
    for (key_seg_idx = 0, lookup_seg_idx = 0, qual_idx = nof_quals - 1; qual_idx >= 0; qual_idx--, lookup_seg_idx++)
    {
        int is_shared;
        ...
        is_shared = (found_in_ms_key_seg_idx[lookup_seg_idx] != DNX_KBP_INVALID_SEGMENT_ID);
         ...
         */
        if (is_shared && is_zero_padding_filling)
        {
            /** Sanity check*/
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "qualifier %d of fg %d in opcode %d is both shared (with segment %d) and "
                         ...
                         found_in_ms_key_seg_idx[lookup_seg_idx],
        ...
        else if (is_shared)
        {
            ...
            lookup_info.key_segment_index[lookup_seg_idx] = found_in_ms_key_seg_idx[lookup_seg_idx];
            LOG_DEBUG_EX(BSL_LOG_MODULE,
                         "Re-use qual %s fg_id %d opcode_id %d key_seg_idx %d \n",
                         ms_key_segments_existing[lookup_info.key_segment_index[lookup_seg_idx]].name,
        ...
        else if (is_zero_padding_filling)
        {
             ...
             */
            lookup_info.nof_segments++;
            lookup_info.key_segment_index[lookup_seg_idx] = in_ms_key_utilizes_zero_padding_seg_idx[lookup_seg_idx];
            LOG_DEBUG_EX(BSL_LOG_MODULE,
                         "Filling zero padding qual %s fg_id %d opcode_id %d key_seg_idx %d \n",
                         ms_key_segments_existing[lookup_info.key_segment_index[lookup_seg_idx]].name,
                         fg_id, opcode_id, found_in_ms_key_seg_idx[lookup_seg_idx]);
        }
        else
        {
            ...
            ms_key_segments_new[key_seg_idx].nof_bytes = BITS2BYTES(qual_size);
     ...
     */
    SHR_IF_ERR_EXIT(kbp_mngr_opcode_master_key_segments_add(unit, opcode_id, key_seg_idx, ms_key_segments_new));
    /*
    ...
    dnx_field_attach_context_info_t * context_info_p)
{
    ...
    /**Get app db id*/
    SHR_IF_ERR_EXIT(dbal_tables_app_db_id_get(unit, dbal_table_id, &app_db_id_uint32, &app_db_size));
    key_in_info.app_db_id = app_db_id_uint32;
            ...
            for (fwd_context_idx = 0; fwd_context_idx < fwd_nof_contexts; fwd_context_idx++)
            {
                     ...
                     acl_context_idx++)
                {
                    ...
                    SHR_IF_ERR_EXIT(dnx_field_key_kbp_ffc_info_get
                                    (unit, fg_id, opcode_id, fwd_acl_ctx_mapping[fwd_context_idx].fwd_context,
                                     acl_context, group_ffc_info_p));
                }
            }
        }
        else
    ...
    dnx_field_group_fg_info_for_sw_t * fg_info_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_group_dbal_table_create
                    (unit, fg_id, fg_info_p->field_stage,
                     ...
                     DBAL_TABLE_TYPE_TCAM_BY_ID, DBAL_ACCESS_METHOD_MDB, 0, use_hitbit, &fg_info_p->dbal_table_id));
    /**
    ...
    dnx_field_group_fg_info_for_sw_t * fg_info_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_group_dbal_table_create
                    (unit, fg_id, fg_info_p->field_stage,
                     fg_info_in_p->dnx_quals, fg_info_in_p->qual_is_ranged, actions_info_arr, fg_info_in_p->name,
                     DBAL_TABLE_TYPE_DIRECT, DBAL_ACCESS_METHOD_MDB, 0, FALSE, &fg_info_p->dbal_table_id));
    ...
    dnx_field_group_fg_info_for_sw_t * fg_info_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_group_dbal_table_create
                    (unit, fg_id, fg_info_p->field_stage,
                     fg_info_in_p->dnx_quals, fg_info_in_p->qual_is_ranged, actions_info_arr, fg_info_in_p->name,
                     DBAL_TABLE_TYPE_EM, DBAL_ACCESS_METHOD_MDB, 0, FALSE, &fg_info_p->dbal_table_id));
    ...
    dnx_field_group_fg_info_for_sw_t * fg_info_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_group_dbal_table_create
                    (unit, fg_id, fg_info_p->field_stage,
                     fg_info_in_p->dnx_quals, fg_info_in_p->qual_is_ranged, actions_info_arr, fg_info_in_p->name,
                     DBAL_TABLE_TYPE_TCAM_BY_ID, DBAL_ACCESS_METHOD_KBP, SAL_UINT8_NOF_BITS, FALSE,
                     &(fg_info_p->dbal_table_id)));
    ...
    dnx_field_group_fg_info_for_sw_t * fg_info_p)
{
    ...
    if (fg_info_p->key_template.key_size_in_bits > dnx_data_field.tcam.key_size_single_get(unit))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Given DE key size (%d) is larger than maximum allowed (%d)",
                     fg_info_p->key_template.key_size_in_bits, dnx_data_field.tcam.key_size_single_get(unit));
    }
    ...
    dnx_field_group_t * fg_id_p)
{
    ...
    switch (fg_info_p->fg_type)
    {
        ...
        case DNX_FIELD_GROUP_TYPE_KBP:
        {
            SHR_IF_ERR_EXIT(dnx_field_group_kbp_add(unit, *fg_id_p, fg_info_in_p, fg_info_p));
            break;
        }
        case DNX_FIELD_GROUP_TYPE_DIRECT_TABLE_MDB:
        {
    ...
    switch (fg_info_p->group_basic_info.fg_type)
    {
        ...
        case DNX_FIELD_GROUP_TYPE_EXEM:
        {
            ...
            SHR_IF_ERR_EXIT(dbal_tables_app_db_id_get
                            (unit, fg_info_p->group_full_info.dbal_table_id, &(fg_info_p->app_db_id), &app_db_size));
            break;
        }
        case DNX_FIELD_GROUP_TYPE_DIRECT_EXTRACTION:
        case DNX_FIELD_GROUP_TYPE_CONST:
    ...
    uint8 seg_index_on_master_by_qual[DNX_DATA_MAX_FIELD_GROUP_NOF_QUALS_PER_FG])
{
    ...
    /*
     * Sanity check: count the number of qualifiers on the field group and verify that they are indntical to the 
    ...
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "fg_id %d, opcode_id %d, mismatch between number of qualifiers %d and "
    ...
    int unit)
{
    ...
    {
        DNX_FIELD_KBP_OPCODE_ITERATOR_IN_USE_CONTINUE(predef_opcode);
        ...
        for (seg_idx = 0; seg_idx < nof_fwd_segments; seg_idx++)
        {
            ...
            for (opcode_idx = 0; opcode_idx < nof_user_defiend_opcodes_used + 1; opcode_idx++)
            {
                ...
                SHR_IF_ERR_EXIT(kbp_mngr_opcode_to_contexts_get
                                (unit, opcode_id_curr, &fwd_nof_contexts, fwd_acl_ctx_mapping));
                for (context_idx = 0; context_idx < fwd_nof_contexts; context_idx++)
                {
                    int acl_contexts_idx;
                    if (fwd_acl_ctx_mapping[context_idx].nof_acl_contexts != 1)
                    {
                        SHR_ERR_EXIT(_SHR_E_INTERNAL,
                                     "opcode %d fwd_context %d has %d nof_acl_contexts, must be 1.\n",
                                     opcode_id_curr, fwd_acl_ctx_mapping[context_idx].fwd_context,
                                     fwd_acl_ctx_mapping[context_idx].nof_acl_contexts);
                    }
                    for (acl_contexts_idx = 0; acl_contexts_idx < fwd_acl_ctx_mapping[context_idx].nof_acl_contexts;
                         acl_contexts_idx++)
                    {
                        ...
                        if ((nof_context_to_share_ffc > 0) && (context_idx > 0) &&
                            (fwd_acl_ctx_mapping[context_idx].acl_contexts[acl_contexts_idx] ==
                             contexts_to_share_ffc[nof_context_to_share_ffc - 1]))
                        {
                            continue;
                        }
    ...
    int unit)
{
    ...
    for (opcode_id = 0; opcode_id < DNX_KBP_MAX_NOF_OPCODES; opcode_id++)
    {
        ...
        for (fwd_context_idx = 0; fwd_context_idx < fwd_nof_contexts; fwd_context_idx++)
        {
            ...
            acl_context = fwd_acl_ctx_mapping[fwd_context_idx].acl_contexts[0];
            if (context_used_for_opcode[acl_context])
            {
                SHR_IF_ERR_EXIT(kbp_mngr_context_acl_is_dynamic(unit, acl_context, &acl_context_is_user_defined));
            ...
            SHR_IF_ERR_EXIT(dnx_field_key_kbp_set(unit, opcode_id,
                                                  fwd_acl_ctx_mapping[fwd_context_idx].fwd_context, acl_context,
                                                  &kbp_opcode_info));
