    ...
    *base_q_pair = -1;
    ...
    for (ps = 0; ps < DNX_EGR_NOF_PS; ps++)
    {
        ...
        if (allocation_bmp[0] && prio_mode == out_port_priority && port_interface == ps_interface)
        {
             ...
             */
            SHR_IF_ERR_EXIT(dnx_sch_allocated_hrs_get(unit, core, ps, allocated_hrs, &used_sch_priority));
            if (used_sch_priority == 0 || used_sch_priority == out_port_priority)
            {
