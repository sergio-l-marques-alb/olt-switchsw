 ...
 * for all ETH layer types the QUALIFIER MASK is 0x44 (bit2 & bit 6) 
 */
    ...
    switch (map_index)
    {
        ...
        case DBAL_ENUM_FVAL_INGRESS_QOS_MAP_INDEX_RCH:
            if (flags & BCM_QOS_MAP_PHB)
            {
                *map_key =
                    ((map_entry->
                      pkt_pri & DNX_QOS_TC_MASK) << DNX_QOS_TC_OFFSET) | (map_entry->pkt_cfi & DNX_QOS_DP_MASK);
            }
    ...
    uint32 qos_init_type)
{
    ...
    for (ffc_index = 0; ffc_index < QOS_MAX_NUM_FFC; ffc_index++)
    {
        ...
        if ((qos_init_type == DBAL_ENUM_FVAL_QOS_INGRESS_APP_TYPE_PHB
            ...
            && (dnx_data_qos.hw_bug.feature_get(unit, dnx_data_qos_hw_bug_phb_rm_ffc_extend)))
        {
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(dnx_qos_tables_ingress_type_init
                    (unit, entry_handle_id, DBAL_ENUM_FVAL_QOS_INGRESS_APP_TYPE_PHB, &qos_phb_cs_table_init[0]));

    SHR_IF_ERR_EXIT(dnx_qos_tables_ingress_type_init
                    (unit, entry_handle_id, DBAL_ENUM_FVAL_QOS_INGRESS_APP_TYPE_REMARK, &qos_remark_cs_table_init[0]));
...
{
    uint32 entry_handle_id;
    uint32 pcp_profile, profile_mask;
    int idx = 0;
    bcm_qos_map_t *map;
    uint32 in_pcp;
    uint32 in_dei;
    ...
    profile_mask = DNX_QOS_MAP_PROFILE_MASK;
    ...
    int *array_count)
{
    uint32 entry_handle_id;
    int idx = 0, is_end;
    uint32 dp_profile, profile_mask;
    bcm_qos_map_t *map;
    uint32 in_pcp;
    uint32 in_dei;
    uint32 ingress_dp;
    ...
    int *array_count)
{
    ...
    opcode_mask = DNX_QOS_MAP_PROFILE_MASK;
    ...
    uint32 flags)
{
    ...
    *ing_map = BCM_DNX_QOS_MAP_INVALID_ID;
    ...
    if (is_physical_port)
    {
        ...
        dbal_entry_handle_value_field32_get(unit, entry_handle_id, DBAL_FIELD_PHB_QOS_PROFILE, INST_SINGLE,
                                            &profile_ndx);
    }
    else
    {
        ...
        if (flags & BCM_QOS_MAP_L2_VLAN_PCP)
        {
            ...
            dbal_fields_struct_field_decode(unit, DBAL_FIELD_VLAN_EDIT_PCP_DEI_PROFILE,
                                            DBAL_FIELD_PCP_DEI_QOS_PROFILE, &profile_ndx, &extend_pcp_dei_profile);
        }
        else
        {
            SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
 ...
 *           type PIPE/UNIFORM/SHORTPIPE/STUCK
...
/**
 * \brief 
 *  Set egress remark profile for ingress MPLS swap case.
 * \see
 *   qos_internal.h
 */
shr_error_e
dnx_qos_ingress_control_mpls_swap_remark_profile_set(
    int unit,
    int egr_map)
{
    uint32 entry_handle_id;
    uint32 profile_index;
    uint32 mpls_swap_cmd;
    int system_headers_mode;

    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);

    profile_index = DNX_QOS_MAP_PROFILE_GET(egr_map);

    /** set egr_map to HW */
    system_headers_mode = dnx_data_headers.system_headers.system_headers_mode_get(unit);
    if ((dnx_data_headers.hw_bug.feature_get(unit, dnx_data_headers_hw_bug_fhei_mpls_cmd_from_eei)) &&
        (system_headers_mode != DBAL_ENUM_FVAL_SYSTEM_HEADERS_MODE_JERICHO_MODE))
    {
        SHR_IF_ERR_EXIT(dbal_fields_enum_hw_value_get
                        (unit, DBAL_FIELD_EEI_IDENTIFIER, DBAL_ENUM_FVAL_EEI_IDENTIFIER_SWAP, &mpls_swap_cmd));
    }
    else
    {
        mpls_swap_cmd = DBAL_ENUM_FVAL_FHEI_TYPE_MPLS_SWAP;
    }

    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_EGRESS_MPLS_FHEI_TO_EES_RESOLUTION, &entry_handle_id));
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_FHEI_SWAP_REMARK, mpls_swap_cmd, profile_index);
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
...
/**
 * \brief 
 *  Get egress remark profile for ingress MPLS swap case.
 *
 * \see
 *   qos_internal.h
 */
shr_error_e
dnx_qos_ingress_control_mpls_swap_remark_profile_get(
    int unit,
    int *egr_map)
{
    uint32 entry_handle_id;
    ...
    SHR_NULL_CHECK(egr_map, _SHR_E_PARAM, "egr_map");
    ...
    *egr_map = BCM_DNX_QOS_MAP_INVALID_ID;
    ...
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_EGRESS_MPLS_FHEI_TO_EES_RESOLUTION, &entry_handle_id));
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_FHEI_SWAP_REMARK, mpls_swap_cmd, &profile_index);
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));
    ...
    int arg)
{
    ...
    if (_SHR_IS_FLAG_SET(flags, BCM_QOS_MAP_IPV4))
    {
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_MPLS_SPECIAL_LABEL_PROFILE,
                                   DBAL_ENUM_FVAL_VTT_MPLS_SPECIAL_LABEL_PROFILE_IPV4_EXPLICIT_NULL);
    }
    else if (_SHR_IS_FLAG_SET(flags, BCM_QOS_MAP_IPV6))
    {
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_MPLS_SPECIAL_LABEL_PROFILE,
    ...
    switch (type)
    {
        ...
        {
            SHR_IF_ERR_EXIT(dnx_qos_ingress_qos_model_to_strength(unit, qos_model, &comp_strength, &prop_strength));
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_RM_COMPARE_STRENGTH, INST_SINGLE,
                                         comp_strength);
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_RM_PROPAGATION_STRENGTH, INST_SINGLE,
                                         prop_strength);
            break;
        }
    ...
    int *arg)
{
    ...
    switch (type)
    {
        ...
        case bcmQosControlMplsExplicitNullIngressRemarkModel:
        {
            dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_RM_COMPARE_STRENGTH, INST_SINGLE,
                                       &comp_strength);
            dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_RM_PROPAGATION_STRENGTH, INST_SINGLE,
                                       &prop_strength);
            SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));
            SHR_IF_ERR_EXIT(dnx_qos_ingress_strength_to_qos_model(unit, comp_strength, prop_strength, &qos_model));
            *arg = qos_model;
