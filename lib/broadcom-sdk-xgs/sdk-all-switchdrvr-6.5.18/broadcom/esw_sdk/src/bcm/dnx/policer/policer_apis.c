    ...
    int database_handle)
{
    ...
    if (!(pol_cfg->flags & BCM_POLICER_REPLACE_SHARED) && (pol_cfg->entropy_id != 0))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "entropy_id != 0 is allowed only if flag BCM_POLICER_REPLACE_SHARED set\n");
    }
    ...
    bcm_policer_database_config_t * config)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_policer_mgmt_nof_expansion_groups_get(unit,
                                                                  _SHR_POLICER_DATABASE_HANDLE_CORE_ID_GET
                                                                  (policer_database_handle), &nof_exp_groups));
        if (nof_exp_groups == 0)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "no expansion groups configured. please configure expansion groups before create a database with expansion enabled\n");
        }
    }
