    ...
    if (dnx_data_oam.general.oam_lm_read_index_field_exists_get(unit))
    {
        ...
        for (core_id = 0; core_id < nof_cores; core_id++)
        {
            for (counter_if = COUNTER_IF_0; counter_if <= NUM_COUNTER_IF; counter_if++)
            {
                ...
                /*
                 * Mappind Counter_if (0-2) to lm_read_index (1-3)
                 * This is the configuration for the workaround.
                 * B0 - PmfGeneral.StatObjLmReadIndex = 'b00111001  (57)  
                 * This is configuration so it matches A0
                 * A0 - PmfGeneral.StatObjLmReadIndex = 'b11100100  (228)
                 */
