    ...
    int unit)
{
     ...
     */
    default_mc_queues = (dnx_data_fabric.general.feature_get(unit, dnx_data_fabric_general_is_supported) ?
    ...
    dnx_ipq_tc_map_t * tc_mapping)
{
    ...
    for (sys_tc = 0; sys_tc < DNX_COSQ_NOF_TC; ++sys_tc)
    {
        for (dp = 0; dp < nof_dps; dp++)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_cosq_ipq_dbal_tc_to_voq_offset_map_set(unit, sys_tc, dp, profile, is_flow, is_fmq,
                                                                       voq_offset));
        }
    }
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(dnx_cosq_ipq_queue_allocate
                    (unit, DNX_ALGO_RES_ALLOCATE_WITH_ID, DNX_ALGO_IPQ_QUEUE_TYPE_UNICAST,
    ...
    dnx_ipq_tc_map_t * tc_mapping)
{
    ...
    if (is_flow)
    {
        SHR_IF_ERR_EXIT(dnx_ipq_alloc_mngr_db.
                        tc_voq_flow_map.profile_data_get(unit, profile_ndx, &ref_count, tc_mapping));
    }
    else
    {
        SHR_IF_ERR_EXIT(dnx_ipq_alloc_mngr_db.
    ...
    int *is_allocated)
{
    ...
    if (is_flow)
    {
        SHR_IF_ERR_EXIT(dnx_ipq_alloc_mngr_db.tc_voq_flow_map.exchange(unit, flags, tc_mapping, *old_map_profile, NULL,
    ...
    bcm_port_t port)
{
    ...
    int system_port = 0, core, queue_id, force_core_all = TRUE;
    ...
    else
    {
        ...
        SHR_RANGE_VERIFY(system_port, 0, max_system_ports, _SHR_E_PARAM, "System port out of bounds\n");
    }
    ...
    bcm_cosq_queue_offset_t offset_array[])
{
    ...
    for (i = 0; i < count; i++)
    {
        ...
        for (dp = start_dp; dp < end_dp; dp++)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_cosq_ipq_dbal_tc_to_voq_offset_map_set(unit, tc, dp, profile, is_flow, is_fmq,
                                                                       voq_offset));
        }
    }
    ...
    bcm_cosq_queue_offset_t offset_array[])
{
    ...
    for (i = 0; i < count; i++)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_cosq_ipq_dbal_tc_to_voq_offset_map_get(unit, tc, dp, profile, is_flow, is_fmq,
    ...
    bcm_cosq_gport_connection_t * gport_connect)
{
    ...
    /** voq to sysport **/
    SHR_IF_ERR_EXIT(dnx_ipq_queue_sysport_map_get(unit, base_qid, &sys_port_id));
    ...
    int is_composite)
{
    ...
    SHR_IF_ERR_EXIT(dnx_cosq_ipq_dbal_queue_quartet_to_sys_port_map_range_set
                    (unit, BCM_CORE_ALL, base_voq_qrtt, num_quartets, sysport));
    ...
    int is_mc)
{
    ...
    if (!is_mc)
    {
         ...
         */
        SHR_IF_ERR_EXIT(dnx_cosq_ipq_dbal_system_port_to_voq_base_get(unit, sysport, &sysport_base_voq, &is_map_valid));
    ...
    uint32 alloc_flags)
{
    ...
    extra_create_info.grain_size = region_nof_quartets;
    /*
...
dnx_cosq_ipq_invalid_destination_queue_set(
    ...
    int enable)
{
    int core, queue_id, force_core_all = TRUE;
    SHR_FUNC_INIT_VARS(unit);
    ...
    SHR_IF_ERR_EXIT(dnx_cosq_ipq_dbal_invalid_destination_queue_set(unit, queue_id));
    ...
    int *enable)
{
    ...
    SHR_IF_ERR_EXIT(dnx_cosq_ipq_dbal_invalid_destination_queue_get(unit, &invalid_dest_queue_id));

    *enable = (queue_id == invalid_dest_queue_id);
exit:
    SHR_FUNC_EXIT;
}
