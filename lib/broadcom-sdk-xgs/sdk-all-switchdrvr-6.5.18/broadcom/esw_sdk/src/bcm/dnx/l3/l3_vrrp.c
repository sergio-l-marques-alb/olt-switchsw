...
/**
 * \brief - Add handle for VSI based VRRP
 */
static shr_error_e
    ...
    int *index)
{
    ...
    for (pp_port_index = 0; pp_port_index < gport_info.internal_port_pp_info.nof_pp_ports; pp_port_index++)
    {
        ...
        /** Set HW table (order is important - if entry exists it will fail) */
        {
            ...
            /** set key - protocol */
            if (protocol & BCM_L2_STATION_MPLS)
            {
            ...
            else if (protocol & BCM_L2_STATION_IPV4)
            {
                dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_LAYER_TYPE_CURRENT_LAYER_PLUS_1,
                                           DBAL_ENUM_FVAL_LAYER_TYPES_IPV4);
            }
            else if (protocol & BCM_L2_STATION_IPV6)
            {
                dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_LAYER_TYPE_CURRENT_LAYER_PLUS_1,
            ...
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "protocol based l2 station: unsupported protocol!!");
            }
            /** set value - my mac indication*/
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_MY_MAC_TYPE, INST_SINGLE, 1);
    ...
    bcm_vlan_t vlan)
{
    ...
    /** iterate over all EXEM_VRID_MY_MAC entries with a given vlan, and delete them */
    {
        ...
        DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_EXEM_VRID_MY_MAC_WITH_PROTOCOL, &entry_handle_id);
    ...
    uint32 *count)
{
    ...
    *count = 0;
    ...
    {
        uint32 entry_handle_id, key_rule_value, key_rule_mask, vrid, protocol_rule;
        int is_end;
        ...
        /** Add KEY rule */
        key_rule_value = vlan;
        key_rule_mask = DNX_L3_VRRP_L3_VLAN_FIELD_FULL_MASK;
        SHR_IF_ERR_EXIT(dbal_iterator_key_field_arr32_rule_add(unit, entry_handle_id, DBAL_FIELD_VSI,
