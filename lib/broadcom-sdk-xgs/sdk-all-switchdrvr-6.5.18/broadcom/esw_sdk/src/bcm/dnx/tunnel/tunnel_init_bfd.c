...
/* Indicate whether tunnel type bfd sip work around */
void
dnx_tunnel_initiator_is_bfd_sip_wa(
    int unit,
    bcm_tunnel_type_t tunnel_type,
    bcm_ip_t dip,
    bcm_ip_t sip,
    bcm_if_t arp_out_lif,
    uint32 *is_bfd_sip_wa)
{
    ...
    uint32 *local_outlif)
{
    ...
    lif_table_mngr_outlif_info.table_specific_flags = DNX_ALGO_LIF_TABLE_SPECIFIC_BFD_SIP_WA;
...
shr_error_e
dnx_tunnel_initiator_bfd_sip_wa_table_get(
    int unit,
    dbal_tables_e dbal_table_id,
    ...
    uint32 arp_out_lif)
{
    ...
    else
    {
        ...
        if ((gport_hw_resources.outlif_dbal_table_id != DBAL_TABLE_EEDB_IPV4_TUNNEL) ||
            (gport_hw_resources.outlif_dbal_result_type != DBAL_RESULT_TYPE_EEDB_IPV4_TUNNEL_ETPS_BFD_IPV4_SIP_WA))
        {
            SHR_ERR_EXIT(_SHR_E_NOT_FOUND, "Illegal tunnel, not related to BFD Source IP");
        }

        /** Get entry information from IPv4 Tunnel BFD SIP WA table */
        SHR_IF_ERR_EXIT(dnx_tunnel_initiator_bfd_sip_wa_table_get
...
shr_error_e
dnx_tunnel_initiator_bfd_sip_wa_clear(
    int unit,
    int global_outlif,
    int local_outlif)
{
