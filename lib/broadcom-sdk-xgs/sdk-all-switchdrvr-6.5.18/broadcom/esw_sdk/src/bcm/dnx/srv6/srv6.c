...
/** following next_protocol used when ETH is over SRv6 */
#define IANA_IPV6_NO_NEXT_HEADER        (59)
    ...
    bcm_srv6_sid_initiator_info_t * info)
{
    ...
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "encap_access (%d) must be one of bcmEncapAccessTunnel[1..4]", info->encap_access);
    }
    ...
    bcm_srv6_srh_base_initiator_info_t * info)
{
    ...
    /** check that nof_sids can be 1-4 */
    if ((info->nof_sids < 1) || (info->nof_sids > 4))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Number of SIDs (%d) needs to be 1 to 4", info->nof_sids);
    }
    ...
    bcm_srv6_sid_initiator_info_t * info)
{
    ...
    SHR_IF_ERR_EXIT(dnx_srv6_sid_initiator_lif_allocate(unit, info, &global_lif, &local_lif));
    ...
    bcm_srv6_sid_initiator_info_t * info)
{
    ...
    info->encap_access =
        bcmEncapAccessTunnel1 + tunnel_gport_hw_resources.outlif_phase - LIF_MNGR_OUTLIF_PHASE_IP_TUNNEL_1;
    ...
    uint32 *nwk_qos_profile)
{
    ...
    SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get(unit, entry_handle_id, DBAL_FIELD_SRH_SEGMENT_LEFT,
    ...
    int local_lif)
{
     ...
     * Also it's used for ETPS context selection to let eaach Encap stage know how many SIDs to
     * construct in each
     */
    nof_sids_on_out_packet = info->nof_sids;
    if (is_reduced)
    {
        nof_sids_on_out_packet -= 1;
    }
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_CS_VAR, INST_SINGLE, nof_sids_on_out_packet * 2);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_SRH_LAST_ENTRY, INST_SINGLE,
                                 nof_sids_on_out_packet - 1);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_SRH_SEGMENT_LEFT, INST_SINGLE, info->nof_sids - 1);
...
/**
 * \brief - General verification for next_protocol add/get/delete APIs
 *
 *   \param [in] unit
 *   \param [in] info - the structure which includes the RCH Port and RCH Encap_id
 *          [in] info.flags - BCM_SRV6_TERMINATOR_NEXT_PROTOCOL_XXX flags
 *          [in] info.next_protocol - protocol above SRv6 according to IANA RFC
 *
 * \return
 *   \retval shr_error_e - Negative in case of an error, zero in case all ok.
 */
static shr_error_e
dnx_srv6_terminator_next_protocol_general_verify(
    int unit,
    bcm_srv6_terminator_next_protocol_mapping_t * info)
{

    /** Next protocols per IANA RFC that are supported */
    int srv6_iana_recognized_next_protocols_arr[] =
        { IANA_IPV4_NEXT_PROTOCOL, IANA_IPV6_NEXT_PROTOCOL, IANA_IPV6_NO_NEXT_HEADER };
    int array_nof_elements =
        sizeof(srv6_iana_recognized_next_protocols_arr) / sizeof(srv6_iana_recognized_next_protocols_arr[0]);

    int next_protocol_recognized_flag = 0;
    int ii = 0;
    ...
    /*
     *  scan the array of supported next_protocols and exit if no match to current next_protocol
     */
    for (ii = 0; ii < array_nof_elements; ii++)
    {
        if (srv6_iana_recognized_next_protocols_arr[ii] == info->next_protocol)
        {
            next_protocol_recognized_flag = 1;
            break;
        }
    }

    if (!next_protocol_recognized_flag)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "SRv6 next_protocol:%d is not supported", info->next_protocol);
    }
...
/**
 * \brief - General verification for terminator_next_protocol traverse API
 */
static shr_error_e
dnx_srv6_terminator_next_protocol_traverse_verify(
    int unit,
 ...
 */
static shr_error_e
dnx_srv6_terminator_next_protocol_in_lif_allocate(
    int unit,
    int *local_inlif)
{
 ...
 */
static shr_error_e
dnx_srv6_terminator_next_protocol_in_lif_free(
    int unit,
    uint32 local_inlif)
{
...
/**
 * \brief - Maps the next_protocol to local IN_LIF
 *
 *
 *   \param [in] unit
 *   \param [in] next_protocol
 ...
 */
static shr_error_e
dnx_srv6_terminator_next_protocol_lookup_add(
    int unit,
    int next_protocol,
    int local_inlif)
{
    ...
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_IP_PROTOCOL, next_protocol);
...
/**
 * \brief - Deletes the mapping of next_protocol to a local IN_LIF
 *
 *
 *   \param [in] unit
 *   \param [in] next_protocol
 ...
 */
static shr_error_e
dnx_srv6_terminator_next_protocol_lookup_delete(
    int unit,
    int next_protocol)
{
...
/**
 * \brief - Gets  local IN_LIF which was mapped by next_protocol
 *
 *
 *   \param [in] unit
 *   \param [in] next_protocol
 ...
 */
static shr_error_e
dnx_srv6_terminator_next_protocol_lookup_get(
    int unit,
    int next_protocol,
    uint32 *local_inlif)
{
    ...
    /** get the in_lif mapped from next_protocol */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_SRV6_TERMINATED_LIF, &entry_handle_id));
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_IP_PROTOCOL, next_protocol);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, _SHR_CORE_ALL);
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_GET_ALL_FIELDS));
 ...
 *
 *   \param [in] unit
 *   \param [in] info - next_protocol mapping to RCH port and encapsulation structure
 ...
 */
static shr_error_e
dnx_srv6_terminator_next_protocol_in_lif_table_set(
    int unit,
    bcm_srv6_terminator_next_protocol_mapping_t * info,
    int local_inlif)
{
    ...
    /*
     * set the P2P LIF Propagation Profile so that it would be Uniform - for TTL and also PHB, Remark, ECN
     * -in order not to influence previous LIFs TTL profile (namely IPv6 termination)
     * -for this using the Propagation Profile allocation API and setting this value to the LIF field
     */
    ...
    /** setting the Propagation Profile to represent UNIFORM TTL */
...
/**
 * \brief - Delete in_lif from lif table, that was mapped by SRv6 next_protocol
 ...
 */
static shr_error_e
dnx_srv6_terminator_next_protocol_in_lif_table_delete(
    int unit,
    uint32 local_inlif)
{
    ...
    /** Delete propagation profile template and hw (if it's not used anywhere else) */
 ...
 */
static shr_error_e
dnx_srv6_terminator_next_protocol_in_lif_param_get(
    int unit,
    uint32 local_inlif,
    bcm_srv6_terminator_next_protocol_mapping_t * info)
{
 ...
 *          [in] info.flags - BCM_SRV6_TERMINATOR_NEXT_PROTOCOL_XXX flags
 *                            BCM_SRV6_SRH_NEXT_PROTOCOL_TERMINATOR_REPLACE - Replace functionality,
    ...
    bcm_srv6_terminator_next_protocol_mapping_t * info)
{
     ...
     * Verification of supported features.
     */
    SHR_INVOKE_VERIFY_DNX(dnx_srv6_terminator_next_protocol_general_verify(unit, info));

    if (!(_SHR_IS_FLAG_SET(info->flags, BCM_SRV6_TERMINATOR_NEXT_PROTOCOL_REPLACE)))
    {
        /*
         * Allocate the SRv6 tunnel terminator
         */
        SHR_IF_ERR_EXIT(dnx_srv6_terminator_next_protocol_in_lif_allocate(unit, &local_inlif));
    }
    else
    {
         ...
         */
        SHR_IF_ERR_EXIT(dnx_srv6_terminator_next_protocol_lookup_get(unit, info->next_protocol, &local_inlif_u32));
     ...
     */
    SHR_IF_ERR_EXIT(dnx_srv6_terminator_next_protocol_in_lif_table_set(unit, info, local_inlif));

    if (!(_SHR_IS_FLAG_SET(info->flags, BCM_SRV6_TERMINATOR_NEXT_PROTOCOL_REPLACE)))
    {
        /*
         * Received a local_inlif from allocation manager, map the next_protocol to local IN_LIF
         */
        SHR_IF_ERR_EXIT(dnx_srv6_terminator_next_protocol_lookup_add(unit, info->next_protocol, local_inlif));
    }
...
/**
 * \brief - Function deletes the port and encap_id that were mapped by the next_protocol (above SRv6 layer)
 *
 *   \param [in] unit - device id
    ...
    bcm_srv6_terminator_next_protocol_mapping_t * info)
{
     ...
     */
    SHR_IF_ERR_EXIT(dnx_srv6_terminator_next_protocol_lookup_get(unit, info->next_protocol, &local_inlif));

    /*
     * delete next_protocol to in_lif lookup
     */
    SHR_IF_ERR_EXIT(dnx_srv6_terminator_next_protocol_lookup_delete(unit, info->next_protocol));

    /*
     * delete the in_lif from its table
     */
    SHR_IF_ERR_EXIT(dnx_srv6_terminator_next_protocol_in_lif_table_delete(unit, local_inlif));

    /*
     * free the in_lif allocation
     */
    SHR_IF_ERR_EXIT(dnx_srv6_terminator_next_protocol_in_lif_free(unit, local_inlif));
...
/**
 * \brief - Function returns the port and encap_id, that were mapped by the next_protocol (above SRv6 layer)
 *
 *   \param [in] unit - device id
 *   \param [in,out] info - structure which describes the SRv6 next protocol mapping to port and encapsulation
 *          [in] info.next_protocol -protocol above SRv6 according to IANA RFC
    ...
    bcm_srv6_terminator_next_protocol_mapping_t * info)
{
     ...
     */
    SHR_IF_ERR_EXIT(dnx_srv6_terminator_next_protocol_in_lif_param_get(unit, local_inlif, info));
...
/**
 * \brief - Function traverses all next_protocols mappings and runs a callback function provided by the user on this data, along
 *          with user_data.
 *
 *   \param [in] unit - device id
 *   \param [in] additional_info - structure for additional control over cb traverse
 *          [in] additional_info.flags - BCM_SRV6_TERMINATOR_NEXT_PROTOCOL_XXX flags
 *          [in] additional_info.traverse_flags - BCM_SRV6_TERMINATOR_NEXT_PROTOCOL_TRAVERSE_XXX flags
 *
 *   \param [in] user_data - user data to be used along with the call back function
 *   \param [in] cb - user call back function of the following structure:
 *
    ...
    void *user_data)
{
     ...
     * Verify
     */
    SHR_INVOKE_VERIFY_DNX(dnx_srv6_terminator_next_protocol_traverse_verify(unit, additional_info, cb));
    ...
    while (!is_end)
    {
        uint32 next_protocol[1];
        bcm_srv6_terminator_next_protocol_mapping_t entry_info;

        bcm_srv6_terminator_next_protocol_mapping_t_init(&entry_info);

        SHR_IF_ERR_EXIT(dbal_entry_handle_key_field_arr32_get
                        (unit, entry_handle_id, DBAL_FIELD_IP_PROTOCOL, next_protocol));

        entry_info.flags = 0;
        entry_info.next_protocol = next_protocol[0];
        SHR_IF_ERR_EXIT(bcm_dnx_srv6_terminator_next_protocol_get(unit, &entry_info));
        SHR_IF_ERR_EXIT(cb(unit, &entry_info, user_data));
        SHR_IF_ERR_EXIT(dbal_iterator_get_next(unit, entry_handle_id, &is_end));
    }
