...
#define MDB_DH_INIT_BPU_PDB_ON_CLUSTER_MASK(unit)          ((0x1<<dnx_data_mdb.dh.nof_macro_clusters_get(unit)) | 0x1)
    ...
    dh_em_bank_info * em_bank_info)
{
    ...
    if (dnx_data_mdb.pdbs.pdbs_info_get(unit, mdb_physical_table)->db_type == DBAL_ENUM_FVAL_MDB_DB_TYPE_EM)
    {
            ...
            if (em_bank_info[bank_iter].bank_size_used_macro_b > 0)
            {
                ...
                if (MDB_EM_TABLE_IS_EMP(mdb_physical_table))
                {
                    ...
                    uint32 nof_entries_in_bank =
                        em_bank_info[bank_iter].bank_size_used_macro_b << dnx_data_mdb.
                        dh.nof_bits_address_in_macro_b_cluster_get(unit);;
        ...
        if (MDB_EM_TABLE_IS_EMP(mdb_physical_table))
        {
            ...
            SHR_IF_ERR_EXIT(mdb_dh_em_emp_get_nof_aging_bits(unit, mdb_physical_table, nof_entries, &nof_aging_bits));
...
mdb_dh_set_bpu_setting_macro(
    ...
    int macro_idx)
{
    dbal_enum_value_field_mdb_physical_table_e mdb_physical_table_id_array[DNX_DATA_MAX_MDB_DH_NOF_MACRO_CLUSTERS];
    int cluster_idx;
    ...
    for (cluster_idx = 0; cluster_idx < dnx_data_mdb.dh.nof_macro_clusters_get(unit); cluster_idx++)
    {
        ...
        int cluster_glob_index = MDB_DH_SERIAL_CLUSTER_NUMBER_FOR_MACRO_TYPE(unit, macro_idx, cluster_idx);
    ...
    dbal_physical_tables_e dbal_physical_table_id)
{
    ...
    SHR_IF_ERR_EXIT(mdb_em_calc_vmv_regs
                    (unit, encoding_request, encoding_request, dbal_physical_table_id, vmv_encoding_data, NULL));
        ...
        for (cluster_idx = 0; cluster_idx < nof_clusters; cluster_idx++)
        {
            ...
            if (db_type == DBAL_ENUM_FVAL_MDB_DB_TYPE_EEDB)
            {
                ...
                else
                {
                    ...
                    uint8 shift_bits =
                        (macro_type ==
                         MDB_MACRO_A) ? utilex_log2_round_up(dnx_data_mdb.dh.nof_rows_in_macro_a_cluster_get(unit)) :
            ...
            else if (MDB_INIT_IS_FEC_TABLE(mdb_physical_table_id))
            {
                ...
                uint32 fec_address_mapping_bits_offset = dnx_data_mdb.dh.nof_bits_address_in_macro_a_cluster_get(unit);
            ...
            if (macro_type == MDB_MACRO_A)
            {
                macro_cluster_counter =
                    (start_address_remapped / dnx_data_mdb.dh.nof_rows_in_macro_a_cluster_get(unit)) %
            ...
            {
                macro_cluster_counter =
                    (start_address_remapped / dnx_data_mdb.dh.nof_rows_in_macro_b_cluster_get(unit)) %
                    dnx_data_mdb.dh.nof_macro_clusters_get(unit);
            ...
            abk_cluster_counter = ((macro_cluster_counter * (nof_cluster_per_entry << is_flex)) +
                                   MDB_DH_EFFECTIVE_CLUSTER_POS(is_flex, cluster_info)) %
                dnx_data_mdb.dh.nof_macro_clusters_get(unit);
            ...
            {
                if (dnx_data_mdb.dh.macro_interface_mapping_get(unit, macro_index)->interfaces[cluster_interface_id] ==
                    mdb_physical_table_id)
                {
                    break;
                }
            ...
            SHR_IF_ERR_EXIT(utilex_bitstream_set_bit
                            (&macros[macro_index].intf[if_instance_used].alloc_bit_map, cluster_info.cluster_index));
            ...
            if (_SHR_IS_FLAG_SET(cluster_flags, MDB_INIT_CLUSTER_FLAG_FIRST_INTF_CLUSTER))
            {
                ...
                macros[macro_index].intf[if_instance_used].flags |= (cluster_flags & MDB_INIT_CLUSTER_FLAG_FLEX);
            }
            ...
            if ((cluster_info.start_address != MDB_DIRECT_INVALID_START_ADDRESS) &&
                (((cluster_info.macro_type == MDB_MACRO_A)
                  && (cluster_info.start_address % dnx_data_mdb.dh.nof_rows_in_macro_a_cluster_get(unit) != 0))
                 || ((cluster_info.macro_type == MDB_MACRO_B)
                     && (cluster_info.start_address % dnx_data_mdb.dh.nof_rows_in_macro_b_cluster_get(unit) != 0))
                 || ((cluster_info.macro_type == MDB_EEDB_BANK)
                     && (cluster_info.start_address % dnx_data_mdb.dh.nof_rows_in_eedb_bank_get(unit) != 0))))
            {
