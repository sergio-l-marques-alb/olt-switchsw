    ...
    int clear_hw)
{
    ...
    /** ECN_Profile(2) =  {FTMH.ECN-En, FTMH.Reserved}
     *   00  - {ECN disabled, classic}
     *   01   - {ECN disabled, L4S}
     *   10  -  {ECN enable, classic}
     *   11 -  {ECN enabled, L4S}  */
    ...
    int arg)
{
    ...
    for (mantissa_input = 0; mantissa_input <= max_mantissa; mantissa_input++)
    {
        ...
        {
             /**calculate the CQ, F(CQ) and find the closest CQ to F(CQ)
                - return new mantissa/exponent
              
                 CQ probability is calculated from input mantissa and input exponent
                 x is the new calculated mantissa - output mantissa
                 y is the new calculated exponent - output exponent

               calculation explanation:
               HW idea is to holds the 4 MSB bits (which not zero) of the prob, 
               and use the MSB bit as EXP and 3 other bits as MANT
               CQ = 2^E | (M << (E-mantissa_size)) / 2^32
               nomilize32_CQ = 2^E | (M << (E-mantissa_size))
               F(CQ) = a * CQ^b.

                the actual calc is:
                a * (nomilize32_CQ / 2^32) ^ b = (x *2^y) / 2^32
                a * (nomilize32_CQ ^ b) / (2^(32*b-32)) = (x *2^y)
                since we cannot cross 64 bits, we will do it in iterations.
               Next, need to find x,y that gives closest Fnew(nomilize32_CQ)
             */
    ...
    int unit)
{
    ...
    /** get default values for exponent from sw state - call the
     *  calculation function which will also get accordingly the
     *  default value of the coefficient from the sw state and
     *  update all possible (mantissa,exponent) entries
     *  
     *  Default setting is to reduce by half the CQ probability
     *  if possible */
    SHR_IF_ERR_EXIT(dnx_cosq_latency_db.probability_convert.exponent_b.get(unit, &exponent_b));
    ...
    bcm_gport_t profile_gport)
{
    ...
    /** Remove the valid bit for the latency profile per flow id if
     *  valid */
    ...
    int unit)
{
    ...
    {
        /** create end to end AQM latency res mngr */
        SHR_IF_ERR_EXIT(dbal_tables_field_info_get(unit, DBAL_TABLE_LATENCY_EGRESS_PROFILE, DBAL_FIELD_PROFILE_ID,
                                                   TRUE, 0, 0, &field_info));
        sal_memset(&data, 0, sizeof(dnx_algo_res_create_data_t));
        data.first_element = 0;
        ...
        /** create AQM flow id latency res mngr */
        SHR_IF_ERR_EXIT(dbal_tables_field_info_get
                        (unit, DBAL_TABLE_LATENCY_EGRESS_L4Q_MAP_AQM_FLOW_ID_TO_PROFILE, DBAL_FIELD_L4Q_FLOW_ID, TRUE,
                         0, 0, &field_info));
        sal_memset(&data, 0, sizeof(dnx_algo_res_create_data_t));
        data.first_element = 0;
        data.nof_elements = field_info.max_value + 1;
    ...
    int unit)
{
    ...
    if (dnx_data_latency.features.feature_get(unit, dnx_data_latency_features_valid_end_to_end_aqm_profile))
    {
        ...
        /** configurations for average latency maintainance */
        SHR_IF_ERR_EXIT(dnx_cosq_latency_end_to_end_aqm_cfg_maintain_average_latency(unit, 0));
        /** set default mantissa and exponend for probability average
         *  cni */
    ...
    *actual_count = 0;
    ...
    for (index = 0; index <= field_info.max_value; index++)
    {
        ...
        if ((*actual_count) < max_count)
        {
            ...
            max_latency_pkts[(*actual_count)].latency_flow = flow_id;
            max_latency_pkts[(*actual_count)].cosq = (int) tc;
            /** translate core + tm port to modid + ftmh pp dsp */
...
*  probabilistic latency based marking cni/drop
*  mechanism - calculation according to the formula
*  (max_latency - min_latency) << shift  <= 2^32
*  
* \param [in] unit -unit id
* \param [in] delta_value - (max_latency - min_latency) value
    ...
    uint32 *shift_value)
{
    ...
    /** formula is (max_latency - min_latency) << shift  <= 2^32
     *  in order to find shift: (max-min) * 2^shift = 2^32
     *  2^shift=2^32 / (max-min)
     *  
     *  uint64 needed for 2^32 */
    COMPILER_64_ZERO(work_reg_64);
    ...
    int arg)
{
    ...
    switch (type)
    {
        ...
        case bcmCosqControlLatencyEgressAqmAverageWeight:
            /** temp_avg_latency = (current_latency >> AverageWeight) +
    ...
    int arg)
{
    ...
    switch (type)
    {
        case bcmCosqControlLatencyEgressAqmFlowId:
            ...
            /*
             * User needs to provide logical port as argument - Get tm Port + Core 
             */
