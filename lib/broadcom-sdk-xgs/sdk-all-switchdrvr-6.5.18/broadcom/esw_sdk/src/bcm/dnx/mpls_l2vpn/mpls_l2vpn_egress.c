...
* \param [in] esem_cmd_range_flags - esem access command range flags.
    ...
    uint8 *esem_cmd_profile_last_reference)
{
    ...
    if (_SHR_IS_FLAG_SET(mpls_port_flags, BCM_MPLS_PORT_REPLACE))
    {
        SHR_IF_ERR_EXIT(dnx_algo_gpm_gport_to_hw_resources(unit, mpls_port_id,
                                                           ...
                                                           &gport_hw_resources));
        if (gport_hw_resources.outlif_dbal_result_type == DBAL_RESULT_TYPE_EEDB_EVPN_ETPS_MPLS_1_IML)
        {
    ...
    if (!_SHR_IS_FLAG_SET(l2vpn_flags, DNX_MPLS_L2VPN_FLAG_EGRESS_IML))
    {
        ...
        else
        {
            ...
            SHR_IF_ERR_EXIT(dnx_port_esem_command_add
                            (unit, native_ac_esem_app_db, ESEM_ACCESS_TYPE_ETH_NATIVE_AC,
                             esem_default_result_profile, &esem_accesses));

            /** Do the exchanging.*/
            SHR_IF_ERR_EXIT(dnx_port_esem_cmd_exchange(unit, mpls_port_id,
                                                       DNX_PORT_ESEM_CMD_ALLOCATE | esem_cmd_range_flags, 0,
                                                       &esem_accesses, old_esem_cmd, new_esem_cmd,
    ...
    else
    {
        ...
        else
        {
            SHR_IF_ERR_EXIT(dnx_port_esem_command_add
                            (unit, native_ac_esem_app_db, ESEM_ACCESS_TYPE_ETH_NATIVE_AC,
                             dnx_data_esem.default_result_profile.default_native_get(unit), &esem_accesses));

            SHR_IF_ERR_EXIT(dnx_port_esem_cmd_exchange(unit, mpls_port_id,
                                                       DNX_PORT_ESEM_CMD_ALLOCATE |
                                                       esem_cmd_range_flags, 0,
                                                       &esem_accesses, old_esem_cmd, new_esem_cmd,
                                                       esem_cmd_data, esem_cmd_profile_first_reference,
                                                       esem_cmd_profile_last_reference));
        }
        ...
        SHR_IF_ERR_EXIT(dnx_port_esem_command_add
                        (unit, DBAL_ENUM_FVAL_ESEM_APP_DB_ID_SSP_NAMESPACE, ESEM_ACCESS_TYPE_DUAL_HOME,
                         dnx_data_esem.default_result_profile.default_dual_homing_get(unit), &esem_accesses));
    ...
    dnx_mpls_l2vpn_info_t * l2vpn_info)
{
    ...
    {
        old_esem_cmd = dnx_data_esem.access_cmd.default_native_get(unit);
    }
    ...
    if (_SHR_IS_FLAG_SET(l2vpn_info->flags, DNX_MPLS_L2VPN_FLAG_EGRESS_IML))
    {
        ...
        esem_range_flags = DNX_PORT_ESEM_CMD_RANGE_0_15;
    }
    else if (encap_info.protection_pointer > 0)
    {
        /**ESEM_CMD Field size is 3*/
        esem_range_flags = DNX_PORT_ESEM_CMD_RANGE_0_7;
    }
    else
    {
        /** Suppose the format is ETPS_MPLS_1.*/
        esem_range_flags = DNX_PORT_ESEM_CMD_RANGE_0_3;
    }
