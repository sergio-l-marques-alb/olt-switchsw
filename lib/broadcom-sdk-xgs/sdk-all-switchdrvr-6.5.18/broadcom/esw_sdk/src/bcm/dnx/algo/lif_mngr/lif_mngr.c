    ...
    uint32 fixed_entry_size)
{
    ...
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Given dbal table id is not a lif table.");
    }
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_FIXED_ENTRY_SIZE, INST_SINGLE, fixed_entry_size);
    ...
    uint32 *fixed_entry_size)
{
    ...
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Given phy table id is not a lif table. %d", physical_table_id);
    }
    ...
    if (fixed_entry_size)
    {
        dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_FIXED_ENTRY_SIZE, INST_SINGLE, fixed_entry_size);
    }
    ...
    uint32 fixed_entry_size)
{
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LOGICAL_PHASE, INST_SINGLE, outlif_logical_phase);
            ...
            && (table_id != DBAL_TABLE_EEDB_DATA_ENTRY))
        {
            ...
            if (!found)
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL,
                             ...
                             outlif_logical_phase + 1, dbal_logical_table_to_string(unit, table_id),
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LL_EXISTS, INST_SINGLE, ll_exists);
...
{
    uint32 entry_handle_id;
    dbal_tables_e tmp_dbal_table_id;
    ...
    if (fixed_entry_size)
    {
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                        (unit, entry_handle_id, DBAL_FIELD_FIXED_ENTRY_SIZE, INST_SINGLE, fixed_entry_size));
    }

    SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                    (unit, entry_handle_id, DBAL_FIELD_DBAL_APP_DB_NAME, INST_SINGLE, &tmp_dbal_table_id));

    if (dbal_table_id)
    {
        *dbal_table_id = tmp_dbal_table_id;
    }

    if (outlif_phase)
    {
        SHR_IF_ERR_EXIT(dnx_algo_local_outlif_to_eedb_logical_phase
                        (unit, local_out_lif, tmp_dbal_table_id, outlif_phase));
    }

    if (result_type)
    {
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                        (unit, entry_handle_id, DBAL_FIELD_DBAL_RESULT_TYPE, INST_SINGLE, result_type));
    }

    if (has_ll)
    {
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field8_get
                        (unit, entry_handle_id, DBAL_FIELD_LL_EXISTS, INST_SINGLE, has_ll));
    }
    ...
    if (is_egress)
    {
        SHR_IF_ERR_EXIT(dnx_lif_mngr_outlif_sw_info_get
                        (unit, outlif_info->local_outlif, &outlif_info->dbal_table_id,
                         &outlif_info->dbal_result_type, &outlif_info->outlif_phase, NULL, NULL));
    ...
    int *nof_glem_lines)
{
    ...
    {
        SHR_IF_ERR_EXIT(mdb_db_infos.capacity.get(unit, DBAL_PHYSICAL_TABLE_GLEM_1, nof_glem_lines));
    }
    else
    {
    ...
    int *nof_dpc_inlifs)
{
    ...
    if (nof_sbc_inlifs)
    {
        ...
        {
            *nof_sbc_inlifs = DNX_ALGO_LIF_MNGR_REDUCED_MODE_NOF_LIFS;
        }
    }
    ...
    if (nof_dpc_inlifs)
    {
        ...
        {
            *nof_dpc_inlifs = DNX_ALGO_LIF_MNGR_REDUCED_MODE_NOF_LIFS;
        }
    }
exit:
    SHR_FUNC_EXIT;
