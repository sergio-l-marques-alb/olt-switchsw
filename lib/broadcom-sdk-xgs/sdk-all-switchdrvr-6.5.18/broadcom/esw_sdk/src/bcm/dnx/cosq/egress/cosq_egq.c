...
/**
 * \brief -
 *   Given 'gport' (mod:port), Get the corresponding 'TM port' (FAP port)
 * \param [in] unit -
 *   Unit HW id
 * \param [in] gport -
 *   bcm_gport_t. Gport to convert
 * \param [out] tm_port_p -
 *   Pointer to uint32. This procedure loads pointed memory by 'TM port'
 *   corresponding to input 'gport'.
 * \param [out] core_p -
 *   Pointer to int. This procedure loads pointed memory by 'core'
 *   corresponding to input 'gport'.
 * \return
    ...
    int *core_p)
{
    ...
    SHR_NULL_CHECK(tm_port_p, _SHR_E_PARAM, "tm_port_p");
    SHR_NULL_CHECK(core_p, _SHR_E_PARAM, "core_p");
    SHR_IF_ERR_EXIT(algo_gpm_tm_gport_to_port_get(unit, 0, gport, &logical_port));
    SHR_IF_ERR_EXIT(dnx_algo_port_tm_port_get(unit, logical_port, core_p, tm_port_p));
    LOG_DEBUG(BSL_LS_BCM_COSQ, (BSL_META_U(unit, "unit(%d) port(%d) fap_port(%d)\n"), unit, logical_port, *tm_port_p));
exit:
    SHR_FUNC_EXIT;
}
/**
    ...
    int weight)
{
    ...
    SHR_FUNC_INIT_VARS(unit);
    SHR_IF_ERR_EXIT(dnx_cosq_egq_fap_port_get(unit, gport, &tm_port, &core));
    ...
    int weight)
{
     ...
     */
    SHR_IF_ERR_EXIT(dnx_ofp_rates_wfq_get(unit, logical_port, core, queue_id, &sch_wfq));
    /*
     ...
     */
    SHR_IF_ERR_EXIT(dnx_ofp_rates_wfq_set(unit, logical_port, core, queue_id, &sch_wfq));
exit:
    SHR_FUNC_EXIT;
}
static shr_error_e
    ...
    int arg)
{
    ...
    switch (type)
    {
        case bcmCosqControlBandwidthBurstMax:
        {
             ...
             * Set conifguration 
             */
            SHR_IF_ERR_EXIT(dnx_ofp_rates_egq_tcg_max_burst_set(unit, core, logical_port, tcg, arg));
            break;
        }
        default:
        {
    ...
    int *arg)
{
    ...
    switch (type)
    {
        case bcmCosqControlBandwidthBurstMax:
        {
             ...
             * Get existing configuration 
             */
            SHR_IF_ERR_EXIT(dnx_ofp_rates_egq_tcg_max_burst_get(unit, core, logical_port, tcg, (uint32 *) arg));
            break;
        }
        default:
        {
 ...
 */
static shr_error_e
bcm_dnx_cosq_gport_egress_multicast_config_set_validate(
    int unit,
    bcm_gport_t gport,
    bcm_cos_t ingress_pri,
    bcm_color_t ingress_dp,
    ...
    bcm_cosq_egress_multicast_config_t * config_p)
{
             ...
             (flags & BCM_COSQ_MULTICAST_SCHEDULED))
    {
        ...
        /** Validate input params */
        SHR_INVOKE_VERIFY_DNX(bcm_dnx_cosq_gport_egress_multicast_config_set_validate
 ...
 */
static shr_error_e
bcm_dnx_cosq_gport_egress_map_get_validate(
    int unit,
    bcm_gport_t gport,
    bcm_cos_t ingress_pri,
    bcm_color_t ingress_dp,
    ...
    bcm_cos_queue_t * offset_p)
{
     ...
     * Validate input params 
     */
    SHR_INVOKE_VERIFY_DNX(bcm_dnx_cosq_gport_egress_map_get_validate(unit, gport, ingress_pri, ingress_dp, offset_p));
    if (BCM_COSQ_GPORT_IS_UCAST_EGRESS_QUEUE(gport) || BCM_COSQ_GPORT_IS_MCAST_EGRESS_QUEUE(gport) ||
        BCM_GPORT_IS_TRUNK(gport))
    {
