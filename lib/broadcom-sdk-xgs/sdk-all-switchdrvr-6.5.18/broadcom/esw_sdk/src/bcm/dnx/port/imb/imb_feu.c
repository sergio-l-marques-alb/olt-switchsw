    ...
    imb_specific_create_info_t * imb_specific_info)
{
    ...
    if (!sw_state_is_warm_boot(unit))
    {
        ...
        {
            burst_size = dnx_data_nif.flexe.ilkn_burst_size_in_feu_get(unit);
            SHR_IF_ERR_EXIT(imb_feu_internal_ilkn_burst_config_set(unit, burst_size));
        }
    }
    ...
    bcm_port_t port)
{
    ...
    if ((client_channel == dnx_data_nif.flexe.oam_client_channel_get(unit)) ||
        (client_channel == dnx_data_nif.flexe.ptp_client_channel_get(unit)))
    {
    ...
    bcm_port_t port)
{
    ...
    SHR_IF_ERR_EXIT(imb_feu_port_priority_config_enable_set(unit, port, rmc_arr, nof_priority_groups, 0));
 ...
 */
static int
imb_feu_port_mac_start_tx_thr_set(
    int unit,
    bcm_port_t port,
    int speed)
{
     ...
     */
    for (idx = 0; idx < dnx_data_nif.flexe.mac_tx_threshold_table_info_get(unit)->key_size[0]; idx++)
    {
        if (speed <= dnx_data_nif.flexe.mac_tx_threshold_table_get(unit, idx)->speed)
        {
            tx_thr = dnx_data_nif.flexe.mac_tx_threshold_table_get(unit, idx)->tx_thr;
            break;
        }
    }
...
/**
 * \brief - Configure TX threshold in TMC
 *
 * \param [in] unit - chip unit id
 * \param [in] port - logical port
 * \param [in] speed - port interface rate
 ...
 */
static int
imb_feu_port_tmc_start_tx_thr_set(
    int unit,
    bcm_port_t port,
    int speed)
{
    ...
    int speed)
{
    ...
    SHR_IF_ERR_EXIT(imb_feu_port_tmc_start_tx_thr_set(unit, port, speed));
    SHR_IF_ERR_EXIT(imb_feu_port_mac_start_tx_thr_set(unit, port, speed));
    ...
    int unit)
{
    ...
    _SHR_PBMP_ITER(logical_ports, logical_port)
    {
        ...
        if (client_channel == dnx_data_nif.flexe.ptp_client_channel_get(unit))
        {
            /*
             * Force the slot to "1"
             */
    ...
    int unit)
{
    ...
    _SHR_PBMP_ITER(logical_ports, logical_port)
    {
        ...
        else if (client_channel == dnx_data_nif.flexe.ptp_client_channel_get(unit))
        {
    ...
    const bcm_port_prio_config_t * priority_config)
{
    ...
    int total_nof_mem_entries = 0, has_speed;
    bcm_pbmp_t fifo_entries;
    ...
    if (has_speed)
    {
        ...
        if ((orig_nof_groups != priority_config->nof_priority_groups) || (new_nof_entries != orig_nof_entries))
        {
            ...
            if (orig_nof_groups != 0)
            {
                SHR_IF_ERR_EXIT(imb_feu_rmc_resource_free(unit, port));
            }
        ...
        SHR_IF_ERR_EXIT(imb_feu_port_priority_config_enable_set
                        (unit, port, rmc, priority_config->nof_priority_groups, 1));
    }
    else
    {
        /*
    ...
    int enable)
{
    ...
    for (i = 0; i < nof_rmcs; ++i)
    {
        ...
        /*
         * Map RMC to client, only relevant for non-L1 switching port AND
         * device mode is centralized. For other cases, the flow set
         * API will decide the RMC mapping.
         */
        SHR_IF_ERR_EXIT(dnx_algo_port_info_get(unit, port, &port_info));
        if (((dnx_data_nif.flexe.flexe_mode_get(unit) == DNX_FLEXE_MODE_CENTRALIZED)
             || (client_channel == dnx_data_nif.flexe.ptp_client_channel_get(unit))
             || (client_channel == dnx_data_nif.flexe.oam_client_channel_get(unit)))
            && !DNX_ALGO_PORT_TYPE_IS_FLEXE_MAC_L1(unit, port_info))
        {
            SHR_IF_ERR_EXIT(imb_feu_internal_rmc_to_client_map_set(unit, client_channel, &rmc_arr[i], enable));
        }
        /*
         * Map client to RMC
         */
    ...
    uint32 flags)
{
    ...
    if (IMB_FLEXE_DIRECTION_RX_ONLY_GET(flags))
    {
        ...
        oam_channel = dnx_data_nif.flexe.oam_client_channel_get(unit);
        if ((nof_ports > 1) && (client_channel == oam_channel))
        {
            /*
