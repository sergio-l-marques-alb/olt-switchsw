    ...
    *valid_event_exist = 0;
    ...
    else
    {
        ...
        SHR_IF_ERR_EXIT(dbal_entry_handle_key_payload_update
                        (unit, entry_handle_id, NULL, _SHR_CORE_ALL, dma_register_size_in_bits, host_entry));
    ...
    dnx_oam_event_and_rmep_info * eventt)
{
    ...
    SHR_IF_ERR_EXIT(shr_htb_insert(htbl, (shr_htb_key_t) eventt, (shr_htb_data_t) 0));
    /*
    ...
    if (event_info->is_oam)
    {
        ...
        else
        {
            ...
            if (_g_oam_performance_event_cb[unit][event_info->event_number])
            {
                ...
                {       /* DM */
                    COMPILER_64_SET(report_data.last_delay, (event_info->event_data[1]) << 2,
                                    (event_info->event_data[0]) << 2);
                    COMPILER_64_SET(report_data.last_delay_near, (event_info->event_data[3]) << 2,
    ...
    void *host_entry)
{
    ...
    while (valid_event_exist)
    {
        for (event_index = 0; event_index < DNX_OAM_OAMP_MAX_JR2_FORMAT_EVENTS; event_index++)
        {
            ...
            {
                rv = dnx_oam_oamp_event_update_sw(unit, 0 /** event interrupt*/ , event_htbl, &event_list, &event_data);
                if (rv != _SHR_E_NONE)
                {
 ...
 * \param [in] entry - pointer to OAMP interrupt message.
 * \param [in] entry_size - OAMP interrupt message size.
 *   
 * \return
 *   void
 */
    ...
    int entry_size)
{
    ...
    SHR_IF_ERR_EXIT(dnx_oam_fifo_interrupt_handler(unit, entry));
 ...
 * \brief - This is the call back function used in sal_dpc call.
 *
 * \param [in] unit - hardware unit used.
 *   
 * \return
 *   void
 */
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(dnx_oam_fifo_interrupt_handler(unit, NULL));
...
/**
 * \brief - Init the event interrupt DMA channel of the OAMP
 *
...
/**
 * \brief - Stop and free the dma channel of the oamp
 *
 * \param [in] unit - unit id
 *  
 * \return
...
/**
 * \brief - Init the event interrupt report DMA channel of the OAMP statistics
 *
 * \param [in] unit - unit id
 *
 * \return
...
/**
 * \brief - Stop and free the dma report channel of the oamp statistics
 *
 * \param [in] unit - unit id
 *
 * \return
