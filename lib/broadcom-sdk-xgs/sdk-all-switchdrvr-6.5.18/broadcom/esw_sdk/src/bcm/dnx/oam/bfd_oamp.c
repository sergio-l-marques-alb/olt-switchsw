    ...
    uint32 entry_id)
{
    ...
    dbal_entry_handle_value_field32_get(unit, entry_handle_id, DBAL_FIELD_EXT_DATA_LENGTH, INST_SINGLE, &data_length);
    ...
    bfd_temp_oamp_mep_db_data_t * mep_hw_write_data)
{
    ...
    if (rv == _SHR_E_RESOURCE)
    {
        ...
        if (endpoint_info->type != bcmBFDTunnelTypeUdp)
        {
            SHR_IF_ERR_EXIT(algo_bfd_db.bfd_oamp_mpls_sport.profile_data_get(unit, DISCR_PROF, &dummy, &actual_port));
        }
        else
        {
            SHR_IF_ERR_EXIT(algo_bfd_db.bfd_oamp_udp_sport.profile_data_get(unit, DISCR_PROF, &dummy, &actual_port));
        }
    ...
    bfd_temp_oamp_mep_db_data_t * mep_hw_write_data)
{
    ...
    full_entry_threshold = dnx_data_oam.oamp.oamp_mep_full_entry_threshold_get(unit);
    SHR_IF_ERR_EXIT(dnx_oam_oamp_scan_count_calc(unit, endpoint_info->id, 0, (endpoint_info->id < full_entry_threshold), DBAL_ENUM_FVAL_CCM_INTERVAL_3MS, 0     /* don't
                                                                                                                                                                 * * care
                                                                                                                                                                 * * for * 
                                                                                                                                                                 * * * * * 
                                                                                                                                                                 * * * *
    ...
    dnx_bfd_oamp_endpoint_t * entry_values)
{
    ...
    if (force_discr_msb_1)
    {
        SHR_IF_ERR_EXIT(dnx_oamp_pe_mep_pe_profile_sw_get
                        (unit, DBAL_ENUM_FVAL_MEP_PE_PROFILE_SW_BFD_DISCR_MSB_1, &mep_pe_profile));
    ...
    if (endpoint_info->type == bcmBFDTunnelTypeUdp || endpoint_info->type == bcmBFDTunnelTypeMpls)
    {
        if (_SHR_IS_FLAG_SET(endpoint_info->flags, BCM_BFD_ENDPOINT_IPV6))
        {
            ...
            {
                SHR_IF_ERR_EXIT(dnx_oamp_pe_mep_pe_profile_sw_get
                                (unit, DBAL_ENUM_FVAL_MEP_PE_PROFILE_SW_BFD_IPV6_MPLS, &mep_pe_profile));
                entry_values->mep_pe_profile = (uint8) mep_pe_profile;
    ...
    if (mep_hw_write_data->mep_profile_write == BFD_CLUSTER_TX_PERIOD_UPDATE)
    {
        ...
        for (index = 0; index < 4; index++)
        {
            ...
            if (cluster_mep_profile_valid)
            {
                ...
                SHR_IF_ERR_EXIT(dnx_oam_oamp_mep_profile_set(unit, cluster_mep_profile, &mep_profile_data));
            }
        }
