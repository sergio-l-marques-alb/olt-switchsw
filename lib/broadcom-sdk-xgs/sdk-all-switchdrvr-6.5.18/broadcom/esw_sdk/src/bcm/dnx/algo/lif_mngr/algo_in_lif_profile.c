    ...
    int unit)
{
    ...
    int nof_sbc_inlifs, nof_dpc_inlifs, nof_cores, nof_references;
    ...
    SHR_IF_ERR_EXIT(mdb_db_infos.capacity.get(unit, DBAL_PHYSICAL_TABLE_INLIF_1, &nof_sbc_inlifs));
    SHR_IF_ERR_EXIT(mdb_db_infos.capacity.get(unit, DBAL_PHYSICAL_TABLE_INLIF_2, &nof_dpc_inlifs));
    nof_cores = dnx_data_device.general.nof_cores_get(unit);
    nof_references = nof_sbc_inlifs + nof_dpc_inlifs * nof_cores + dnx_data_l3.rif.nof_rifs_get(unit);
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(mdb_db_infos.capacity.get(unit, DBAL_PHYSICAL_TABLE_INLIF_1, &data.max_references));
