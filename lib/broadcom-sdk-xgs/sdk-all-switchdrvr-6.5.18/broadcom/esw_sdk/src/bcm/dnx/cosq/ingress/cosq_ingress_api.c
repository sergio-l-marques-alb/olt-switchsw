    ...
    bcm_cos_queue_t cosq)
{
    ...
    if (BCM_COSQ_GPORT_IS_VSQ_CTCC(vsq))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_ingress_congestion_dbal_voq_connection_class_set(unit, queue_id, vsq_connection_cls));
    }
    if (BCM_COSQ_GPORT_IS_VSQ_CTTC(vsq))
    {
        vsq_traffic_cls = BCM_COSQ_GPORT_VSQ_TC_GET(vsq);
        SHR_IF_ERR_EXIT(dnx_ingress_congestion_dbal_voq_traffic_class_set(unit, queue_id, vsq_traffic_cls));
    }
    ...
    bcm_cos_queue_t cosq)
{
    ...
    if (BCM_COSQ_GPORT_IS_VSQ_CTCC(vsq))
    {
        SHR_IF_ERR_EXIT(dnx_ingress_congestion_dbal_voq_connection_class_set(unit, queue_id, 0));
    }
    if (BCM_COSQ_GPORT_IS_VSQ_CTTC(vsq))
    {
        SHR_IF_ERR_EXIT(dnx_ingress_congestion_dbal_voq_traffic_class_set(unit, queue_id, 0));
    }
    ...
    bcm_gport_t * pg_base_vsq_gport)
{
    ...
    if (!DNX_ALGO_PORT_TYPE_IS_NIF(unit, port_info, FALSE, FALSE, FALSE, FALSE) && config->numq != 0 &&
