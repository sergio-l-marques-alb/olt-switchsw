...
/**
* \brief
*  Initiation procedure for dnx_field_key_ffc_sharing_info_t
* \param [in] unit            - Device Id
* \param [in] info            - array of dnx_field_key_ffc_sharing_info_t to be initiated
* \return
...
* \param [in] allocate_with_id - Indicates whether to allocate a the FFC ID in ffc_id or any FFC ID.
* \param [in,out] ffc_id       - hold the id of FFC that was allocated per qualifier
* \return
*   shr_error_e - Error Type
* \remark
*   * None
    ...
    dnx_field_key_initial_ffc_info_t * ffc_initial_p)
{
     ...
     * */
    rv = dnx_algo_field_key_ipmf1_initial_key_f_occupation_bmp_allocate(unit, DNX_ALGO_RES_ALLOCATE_SIMULATION,
    ...
    else        /* (rv != _SHR_E_NONE) */
    {
         ...
         * */
        rv = dnx_algo_field_key_ipmf1_initial_key_g_occupation_bmp_allocate(unit, DNX_ALGO_RES_ALLOCATE_SIMULATION,
                                                                            context_id, ffc_size, &dest_offset);
        if (rv == _SHR_E_NONE)
        {
            /*
...
* \param [in] second_part_payload - For TCAM double key field groups only, indicating if we are using the first or
*                                   second half of the double payload.
    ...
    int app_db_id)
{
    ...
    if (key_id_p->id[1] != DNX_FIELD_KEY_ID_INVALID)
    {
         ...
         * each key in a double key pair. In JR2, we have TCAM double keys mode only, therefore we use
        ...
        case DNX_FIELD_INPUT_TYPE_META_DATA:
        {
                ...
                && (dnx_qual_class != DNX_FIELD_QUAL_CLASS_VW))
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             ...
                             "qualifiers of virtual wire qualifers (qualifier %s).\r\n",
                             dnx_field_dnx_qual_text(unit, qual_type));
            }
            break;
        }
...
/**
* \brief
*  This function returns the base qualifier in ipmf2 for the action buffer of ipmf1
*  that we want to cascade from.
* \param [in] unit                 - Device Id
* \param [in] context_id           - Context ID
* \param [in] cascaded_from_fg_id  - The field group we cascade from.
* \param [in] relative_qual_offset - The relative offset of the qualifier inside the action buffer.
    ...
    dnx_field_qual_t * ipmf2_base_qual_p)
{
    ...
    /*
     * Get the field_io for the field_group we have cascaded from.
     */
    ...
    /*
     * Indicating if the offset starts in the first or second half of the payload, relevant only for double key TCAM.
     */
...
*  We do this by getting the key_id of the cascacede_from field group, then
*  calculating the offset of the action buffer for the relevant key.
* \param [in]  unit                 - Device Id
* \param [in]  field_stage          - The field stage where the qualifier is used.
* \param [in]  context_id           - Context ID to configure FFC's for.
* \param [in]  cascaded_from_fg_id  - The field group we cascade from.
* \param [in]  relative_qual_offset - The relative offset of the qualifier inside the action buffer.
* \param [out] absolute_qual_offset - Absolute offset on the PBUS for the cascading qualifier.
    ...
    uint8 *is_up_to_one_ffc_p)
{
    ...
    if (dnx_qual_info.input_type == DNX_FIELD_INPUT_TYPE_CONST)
    {
        ...
        /** Create fake mapping on the key.*/
        key_qual_map.lsb = 0;
...
/**
* \brief
*  Configure a ipmf2 qualifier info. Example of usage: internal qualifiers for initial keys in ipmf1-ipmf2
* \param [in] unit          - Device Id
* \param [in] context_id    - Context ID to configure FFC's for
* \param [in] key_id        - key id
* \param [in] internal_ipmf2_qual_info_p - Qualifier Info
* * \return
*   shr_error_e             - Error Type
* \remark
*   * None
    ...
    dnx_field_key_ffc_instruction_info_t * ffc_instruction_info_p)
{
    ...
    {
        /*
         * Make Valgrind happy - always initialize the field_index member 
         */
...
/**
* \brief
*  This procedure fills the information of allowed initial keys and ranges
*  according to the ipmf1 context properties
* \param [in] unit            - Device Id
* \param [in] field_stage     - Field stage
* \param [in] context_id      - Context ID in IMF1 that the context_ffc_sharing_info is filled about
* \param [out] context_ffc_sharing_info   - array of dnx_field_key_ffc_sharing_info_t to be filled
* \return
*   shr_error_e - Error Type
* \remark
*   * None
...
/**
* \brief
*  This function determines if the given intial_ffc_p is shared,
*  meaning if there is an IPMF2 qualifier that relies on the given ffc.
*  In case this FFC is 'shared' - we can't deallocate it.
*  Important Note: This function is called in the middle of delete procedure.
*  Valid result is achieved by cleaning the ipmf2 kbr of this
*  field group BEFORE calling to this function. So if there are any other IPMF2 FFC's
*  that 'read' data from the given FFC - means that it is shared and should not be deleted.
* \param [in] unit          - Device ID
* \param [in] field_stage   - For which Field stage to Configure HW
* \param [in] context_id    - Context ID
* \param [in] group_ffc_info_p - group information. Used ONLY for determining shared ffc's in the same group
* \param [in] intial_ffc_p  - given initial ffc information.
* \param [out] shared_p     - TRUE if intial_ffc_p is used by another qualifier
*                           - FALSE if intial_ffc_p is not used
*                             in FALSE case there is no problem to delete such ffc as
    ...
    int *shared_p)
{
    ...
    *shared_p = FALSE;
    ...
    /*
     * Currently supports only IPMF2 FG. The sharing is for the initial ffc's in IPMF1
     */
    if (field_stage != DNX_FIELD_STAGE_IPMF2)
    {
    ...
    /*
     * Skip the sharing verification for this intial_ffc_p if it is in a not-valid key (or range) for the given
     * cascaded_from_context_id. we can't share / handle here the ffc's used by context properties 
     */
    ...
    /*
     * Go through the ffc's in this FG. Validates that there are no similar qualifiers in the same fg that might use
     * same initial ffc 
     */
         ...
         (group_ffc_info_p->key.qualifier_ffc_info[qual_index].qual_type != DNX_FIELD_QUAL_TYPE_INVALID); qual_index++)
    {
        for (ffc_index = 0; ffc_index < DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_FFC_IN_QUAL; ffc_index++)
        {
            ...
            {
                /*
                 * Go through all the (i~DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_FFC_IN_QUAL) quals, to validate that the 
                 * same ffc id is not used more than once. In case it is used more then once - we will remove it's
                 * duplications and leave only one instance 
                 */
                ...
                /*
                 * In some cases, (aka constant qualifiers), we may have a usecase where the same qualifier uses an ffc 
                 * more then one time. In case we would like to delete such qualifier - we need to delete the ffc only
                 * once. Thus, replace all it's instances in group_ffc_info with INVALID 
                 */
                for (internal_ffc_idx = 0; internal_ffc_idx < DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_FFC_IN_QUAL;
                     internal_ffc_idx++)
                {
    ...
    for (key_idx = DBAL_ENUM_FVAL_FIELD_KEY_F; key_idx < DBAL_NOF_ENUM_FIELD_KEY_VALUES; key_idx++)
    {
        ...
        for (ffc_index = 0; ffc_index < DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_FFC; ffc_index++)
        {
            if (ffc_id_list[ffc_index] != DNX_FIELD_FFC_ID_INVALID)
            {
                ...
                /*
                 * Compare the instruction with the instruction of new ffc
                 */
...
/**
* \brief
*  This function determines if the initial FFCs for the given IPMF2 qualifer
*  already exist in the given context in the given stage. In such case we can
*  re-use the existing FFCs instead of allocating new ones.
*  Important Note: This function is called only in dnx_field_key_ffc_set() sequence.
*  Valid result would be achieved in case this function is called BEFORE
*  allocating new initial FFCs
* \param [in] unit          - Device ID
* \param [in] field_stage   - For which Field stage to Configure HW
* \param [in] context_id    - Context ID
...
* \param [in] dnx_qual_info_p       - Additional information for header qualifiers,
*                                     to know from which layer the qualifier should be taken
* \param [out] qualifier_ffc_info_p - output ffc information that will be used to configure the
*                                     ipmf2 qualifier.
* \param [out] shared_p     - TRUE if initial ffc that required for the given qualifier already
*                             present in the systen and can be shared
*                           - FALSE if not (in such case new initial ffc's should be
*                             allocated and configuered for this qualifier)
* \return
*   shr_error_e - Error Type
* \remark
*   * None
    ...
    int *shared_p)
{
    ...
    dbal_fields_e dbal_ffc_type;
    /*
     * found_match is initialised to TRUE in order to enter the first loop. After that will be set to TRUE * Only if a
     * match was found. In case one of the ffc's for the given qualifier doesn't * match - we decide that there is no
     * match for this qualifier. 
     */
    ...
    *shared_p = FALSE;
    ...
    {
        /*
         * found_match is initialised to TRUE in order to enter the first loop. After that it will be set to TRUE Only
         * if a match was found. In case one of the FFCs for the given qualifier doesn't match - we decide that there
         * is no match for this qualifier.
         */
        found_match = FALSE;
        /*
         * Calculate the ffc instructions, considering initial ffc
        ...
        /*
         * Go through all (initial) keys in the system
         */
        for (key_idx = DBAL_ENUM_FVAL_FIELD_KEY_F; key_idx < DBAL_NOF_ENUM_FIELD_KEY_VALUES; key_idx++)
        {
            ...
            {
                /*
                 * Key can't be used for sharing. Skip 
                 */
                continue;
            }
            /*
            ...
            for (ffc_in_key_idx = 0; ffc_in_key_idx < DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_FFC; ffc_in_key_idx++)
            {
                if (ffc_id[ffc_in_key_idx] != DNX_FIELD_FFC_ID_INVALID)
                {
                    ...
                    {
                        /*
                         * Verify that the insturction is within the valid range for this context The reason for this
                         * is that in case the cascaded_from_context_id is a special context (aka hash or compare) we
                         * don't want the ffc sharing to be applied on the reserved area in the initial key
                         */
                        ...
                        {
                            /*
                             * Found matching ffc - meaning the new ffc could be shared
                             */
    ...
    int forced_ffc_id)
{
    ...
    if ((dnx_qual_info_p->native_pbus == FALSE) && (field_stage == DNX_FIELD_STAGE_IPMF2))
    {
        ...
        /** FFC Sharing: check if the qualifier already exists in the initial data.*/
        ...
        /*
         * FFC Sharing: check if the qualifier already exists in the initial data. If so, skip the allocation &
         * configuration of the initial qual configuration, update the configuration of the ipmf2
         */
        if (!is_shared)
        {
    ...
    dnx_field_key_id_t *key_id_p)
{
     ...
     * types then, allocate by bit-range.
     */
    ...
    SHR_IF_ERR_EXIT(dnx_algo_field_key_id_bit_range_allocate(unit, field_stage, fg_type, context_id,
                                                             ...
                                                             &align_info, key_id_p, &bit_range_offset_within_key));
 ...
 * \brief
 *  Attach context ID per KEY (I.e., allocate key per context
 *  and configure FFC and KBR accordingly) for MDB DT FG
 * \param [in] unit             -
 *   HW device ID
 * \param [in] context_id       -
 *   Context ID ID to perform the key attach for
 ...
 * \brief
 *  Attach context ID per KEY (I.e., allocate key per context
 *  and configure FFC and KBR accordingly) for EXEM FG
 * \param [in] unit             -
 *   HW device ID
 * \param [in] context_id       -
 *   Context ID ID to perform the key attach for
    ...
    dnx_field_app_db_id_t * app_db_id_p)
{
         ...
         (key_template_p->key_qual_map[qual_idx].qual_type != DNX_FIELD_QUAL_TYPE_INVALID); qual_idx++)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_field_key_qual_ffc_get
                        (unit, field_stage, context_id, fg_type, key_id_p,
...
* \param [in] field_stage   - For which Field stage to Configure HW
* \param [in] context_id    - Context ID
* \param [in] key_in_info_p         - key attach information
    ...
    for (qual_idx = 0; (qual_idx < DNX_DATA_MAX_FIELD_GROUP_NOF_QUALS_PER_FG); qual_idx++)
    {
              ...
              DNX_FIELD_FFC_ID_INVALID); ffc_idx++)
        {
                ...
                DNX_FIELD_FFC_ID_INVALID)
            {
                ...
                /*
                 * Clean the initial ffc's kbr ONLY if not used by other fg
                 */
                if (!is_shared)
                {
                    SHR_IF_ERR_EXIT(dnx_field_key_kbr_single_ffc_hw_remove
    ...
    if (field_stage == DNX_FIELD_STAGE_IPMF2)
    {
        ...
        for (qual_idx = 0; (qual_idx < DNX_DATA_MAX_FIELD_GROUP_NOF_QUALS_PER_FG); qual_idx++)
        {
                  ...
                  DNX_FIELD_FFC_ID_INVALID); ffc_idx++)
            {
                ...
                /*
                 * Clean the initial ffc's ONLY if not used by other fg
                 */
                if (!is_shared)
                {
                    /*
...
/**
 * \brief
 *  Detach all context ID key information for MDB DT FG, i.e. deallocate all resources that were allocated for context ID to key at input
 * All relevant information about the Key can be read from the HW
 * \param [in] unit        - Device Id
 * \param [in] context_id  - Context ID to detach
 * \param [in] key_in_info_p   - Key info
    ...
    dnx_field_key_id_t *key_id_p)
{
    ...
    if (key_id_p->id[0] != DNX_FIELD_KEY_ID_INVALID)
    {
        CONSTRUCT_KEY_ALLOCATION_ID(key_id_temp_handle, DNX_FIELD_GROUP_TYPE_DIRECT_TABLE_MDB,
                                    key_in_info_p->field_stage, context_id, key_id_p->id[0], key_id_p->id[1],
                                    key_id_p->key_part);
        SHR_IF_ERR_EXIT(dnx_algo_field_key_id_free(unit, key_id_temp_handle));
    }
    ...
    switch (key_id)
    {
        case DBAL_ENUM_FVAL_FIELD_KEY_F:
            SHR_IF_ERR_EXIT(dnx_algo_field_key_ipmf1_initial_key_f_occupation_bmp_allocate
                            (unit, DNX_ALGO_RES_ALLOCATE_WITH_ID, context_id, size, initial_offset_in_key_p));
            break;
        case DBAL_ENUM_FVAL_FIELD_KEY_G:
    ...
    dnx_field_key_id_t *tcam_key_id_p)
{
         ...
         (initial_key_id_p->id[key_idx] != DNX_FIELD_KEY_ID_INVALID); key_idx++)
    {
        SHR_IF_ERR_EXIT(dnx_algo_field_key_ipmf1_initial_key_occupation_bmp_with_key_id_allocate
                        (unit, DNX_ALGO_RES_ALLOCATE_WITH_ID, context_id, initial_key_id_p->id[key_idx],
    ...
    dnx_field_key_id_t *initial_key_id_2_p)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_algo_field_key_ipmf1_initial_key_occupation_bmp_with_key_id_allocate
                        (unit, DNX_ALGO_RES_ALLOCATE_WITH_ID, context_id, initial_key_id_1_p->id[key_idx],
                         dnx_data_field.stage.stage_info_get(unit, DNX_FIELD_STAGE_IPMF1)->compare_key_size,
                         &compare_key_starting_offset));
    }
         ...
         (initial_key_id_2_p->id[key_idx] != DNX_FIELD_KEY_ID_INVALID); key_idx++)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_algo_field_key_ipmf1_initial_key_occupation_bmp_with_key_id_allocate
                        (unit, DNX_ALGO_RES_ALLOCATE_WITH_ID, context_id, initial_key_id_2_p->id[key_idx],
                         dnx_data_field.stage.stage_info_get(unit, DNX_FIELD_STAGE_IPMF1)->compare_key_size,
                         &compare_key_starting_offset));
...
{
    uint32 key_idx;
    int state_table_key_starting_offset = DNX_FIELD_KEY_STATE_TABLE_STARTING_OFFSET(unit);
...
dnx_field_key_kbp_acl_compare_qualifiers_for_sharing(
    int unit,
    dnx_field_qual_t dnx_qual_1,
    dnx_field_qual_attach_info_t * qual_attach_info_1_p,
    dnx_field_qual_t dnx_qual_2,
    dnx_field_qual_attach_info_t * qual_attach_info_2_p,
    int *shareable_p)
{
    ...
    *shareable_p = FALSE;
    ...
    /*
     * Qaulifiers must have the same input type.
     */
    if (qual_attach_info_1_p->input_type == qual_attach_info_2_p->input_type)
    {
        /*
         * At the moment, only certain input types are supported.
         */
        if (qual_attach_info_1_p->input_type == DNX_FIELD_INPUT_TYPE_LAYER_FWD ||
            qual_attach_info_1_p->input_type == DNX_FIELD_INPUT_TYPE_LAYER_ABSOLUTE ||
            qual_attach_info_1_p->input_type == DNX_FIELD_INPUT_TYPE_LAYER_RECORDS_FWD ||
            qual_attach_info_1_p->input_type == DNX_FIELD_INPUT_TYPE_LAYER_RECORDS_ABSOLUTE ||
            qual_attach_info_1_p->input_type == DNX_FIELD_INPUT_TYPE_META_DATA)
        {
            /*
             * Qualifier must have the same size, and the same FFC field offset.
             */
            SHR_IF_ERR_EXIT(dnx_field_key_qual_info_get
                            (unit, DNX_FIELD_STAGE_EXTERNAL, DNX_FIELD_CONTEXT_ID_INVALID, dnx_qual_1,
                             qual_attach_info_1_p, &qual_info_1));
            SHR_IF_ERR_EXIT(dnx_field_key_qual_info_get
                            (unit, DNX_FIELD_STAGE_EXTERNAL, DNX_FIELD_CONTEXT_ID_INVALID, dnx_qual_2,
                             qual_attach_info_2_p, &qual_info_2));
             /** Sanity check, verify the mapped size is the same.*/
            {
                uint32 qual_size_1, qual_size_2;
                SHR_IF_ERR_EXIT(dnx_field_map_dnx_qual_size(unit, DNX_FIELD_STAGE_EXTERNAL, dnx_qual_1, &qual_size_1));
                SHR_IF_ERR_EXIT(dnx_field_map_dnx_qual_size(unit, DNX_FIELD_STAGE_EXTERNAL, dnx_qual_2, &qual_size_2));
            ...
            /** Sanity check.*/
            if ((*shareable_p) &&
                ((qual_info_1.input_type != qual_info_2.input_type) || (qual_info_1.ffc_type != qual_info_2.ffc_type)))
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "Mismatch for shareable qualifiers, input type is %d, "
                             ...
                             qual_attach_info_1_p->input_type, dnx_qual_1, dnx_qual_2,
    ...
    int *size_on_key_p)
{
    ...
    /** Get the bitmap of the keys used by the KBP for the device, and count the number of keys of the device. */
    ...
    int *size_on_key_p)
{
    ...
    if (fwd_nof_contexts > DNX_DATA_MAX_FIELD_KBP_MAX_ACL_CONTEXT_NUM)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "opcode %d has %d fwd contexts, maximum is %d.\n",
                     opcode_id, fwd_nof_contexts, DNX_DATA_MAX_FIELD_KBP_MAX_ACL_CONTEXT_NUM);
    }
    for (fwd_ctx_index = 0; fwd_ctx_index < fwd_nof_contexts; fwd_ctx_index++)
    {
        if (fwd_acl_ctx_mapping[fwd_ctx_index].nof_acl_contexts <= 0)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "opcode %d fwd_context %d (index %d) has %d acl contexts.\n",
                         opcode_id, fwd_acl_ctx_mapping[fwd_ctx_index].fwd_context, fwd_ctx_index,
                         fwd_acl_ctx_mapping[fwd_ctx_index].nof_acl_contexts);
        }
        SHR_IF_ERR_EXIT(dnx_field_key_kbp_segment_position(unit, opcode_id,
                                                           fwd_acl_ctx_mapping[fwd_ctx_index].fwd_context,
                                                           fwd_acl_ctx_mapping[fwd_ctx_index].acl_contexts[0],
    ...
    int ffc_id)
{
    ...
    if (segment_info_p->qual_idx >= DNX_DATA_MAX_FIELD_GROUP_NOF_QUALS_PER_FG ||
        (unsigned int) segment_info_p->qual_idx >= DNX_DATA_MAX_FIELD_GROUP_NOF_QUALS_PER_FG)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "segment_info_p->qual_idx %d exceeds maximum %d.\n",
                     segment_info_p->qual_idx, DNX_DATA_MAX_FIELD_GROUP_NOF_QUALS_PER_FG);
    }
     ...
     */
    qual_type = key_template.key_qual_map[segment_info_p->qual_idx].qual_type;
    ...
    SHR_IF_ERR_EXIT(dnx_field_key_qual_info_get
                    (unit, DNX_FIELD_STAGE_EXTERNAL, DNX_FIELD_CONTEXT_ID_INVALID, qual_type,
                     &qual_attach_info_arr[segment_info_p->qual_idx], &dnx_qual_info));

    SHR_IF_ERR_EXIT(kbp_mngr_opcode_to_contexts_get(unit, opcode_id, &fwd_nof_contexts, fwd_acl_ctx_mapping));
    for (context_index = 0; context_index < fwd_nof_contexts; context_index++)
    {
        fwd_context = fwd_acl_ctx_mapping[context_index].fwd_context;
        acl_context = fwd_acl_ctx_mapping[context_index].acl_contexts[0];
        /** Sanity check.*/
        if (fwd_acl_ctx_mapping[context_index].nof_acl_contexts != 1)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "Opcode %d fwd context %d has %d acl contexts, must be 1.\n",
                         opcode_id, fwd_context, fwd_acl_ctx_mapping[context_index].nof_acl_contexts);
        }
        /** Sanity check.*/
        SHR_IF_ERR_EXIT(kbp_mngr_context_acl_is_dynamic(unit, acl_context, &ctx_is_dynamic));
        ...
        if ((!ctx_is_dynamic) != (!opcode_is_dynamic))
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL,
                         ...
                         opcode_id, acl_context, ctx_is_dynamic, opcode_is_dynamic);
        }
        if ((opcode_is_dynamic == FALSE) && (fwd_context != acl_context))
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL,
                         "Opcode %d is predefined, but fwd context %d doesn't equal acl context %d.\n",
                         opcode_id, fwd_context, acl_context);
        }
        if (opcode_is_dynamic && (context_index > 0) &&
            (acl_context != fwd_acl_ctx_mapping[context_index - 1].acl_contexts[0]))
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL,
                         "Opcode %d is usr defined, it has more than one ACL contexts (%d and %d).\n",
                         opcode_id, acl_context, fwd_acl_ctx_mapping[context_index - 1].acl_contexts[0]);
        }
        ...
        SHR_IF_ERR_EXIT(dnx_field_key_kbp_segment_position
                        (unit, opcode_id, fwd_context, acl_context, segment_id,
        ...
        SHR_IF_ERR_EXIT(dnx_field_key_ffc_set
                        (unit, DNX_FIELD_STAGE_EXTERNAL, acl_context, &dummy_qual_map, &dnx_qual_info,
    ...
    dnx_field_group_kbp_info_t * kbp_opcode_info_p)
{
    ...
    for (seg_idx = (DNX_KBP_MAX_NOF_SEGMENTS_PER_MASTER_KEY - 1); seg_idx >= nof_fwd_seg; seg_idx--)
    {
         ...
         */
        qual_in_key_template_idx = kbp_opcode_info_p->master_key_info.segment_info[seg_idx].qual_idx;

        /*
         * Qualifier type is pointed by the qual_in_key_template_idx, extracted from the fg_info
         */
        qual_type = key_template.key_qual_map[qual_in_key_template_idx].qual_type;
    ...
    for (seg_idx = (DNX_KBP_MAX_NOF_SEGMENTS_PER_MASTER_KEY - 1); seg_idx >= 0; seg_idx--)
    {
         ...
         * Qualifier type is pointed by the qual_in_key_template_idx, extracted from the fg_info
         */
        qual_type = key_template.key_qual_map[qual_in_key_template_idx_arr[seg_idx]].qual_type;
        ...
        SHR_IF_ERR_EXIT(dnx_field_key_qual_info_get
                        (unit, DNX_FIELD_STAGE_EXTERNAL, DNX_FIELD_CONTEXT_ID_INVALID, qual_type,
                         &qual_attach_info_arr[qual_in_key_template_idx_arr[seg_idx]], &dnx_qual_info));
        ...
        aligned_location_on_key =
            ...
            qual_lsb_unaligned[seg_idx] - qual_lsb_aligning_size[seg_idx];
        SHR_IF_ERR_EXIT(dnx_field_key_ffc_set
                        (unit, DNX_FIELD_STAGE_EXTERNAL, acl_context_id, &dummy_qual_map, &dnx_qual_info,
                         key_ids[qual_key_index[seg_idx]], aligned_location_on_key, FALSE, DNX_FIELD_FFC_ID_INVALID));
    ...
    dnx_field_key_group_ffc_info_t * group_ffc_info_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_group_kbp_segment_indices(unit, fg_id, opcode_id, seg_index_on_master_by_qual));
            ...
            if (is_segment_found)
            {
                ...
                SHR_IF_ERR_EXIT(dnx_field_key_qual_ffc_get_internal
                                (unit, DNX_FIELD_STAGE_EXTERNAL, context_for_ffc, fwd_kbr_id,
                                 &dummy_qual_map, starting_key_offset, &qualifier_ffc_info, &output_ffc_index));
...
/**
 * \brief
 *  This function manages the FFCs available for ACL context by
    ...
    int unit)
{
    ...
    {
        DNX_FIELD_KBP_OPCODE_ITERATOR_IN_USE_CONTINUE(opcode_id);
        ...
        if (fwd_nof_contexts > DNX_DATA_MAX_FIELD_KBP_MAX_ACL_CONTEXT_NUM)
        {
            SHR_ERR_EXIT(_SHR_E_CONFIG, "opcode %d has %d fwd contexts, maximum is %d.\n",
                         opcode_id, fwd_nof_contexts, DNX_DATA_MAX_FIELD_KBP_MAX_ACL_CONTEXT_NUM);
        }
        for (fwd_ctx_index = 0; fwd_ctx_index < fwd_nof_contexts; fwd_ctx_index++)
        {
            uint8 fwd_context = fwd_acl_ctx_mapping[fwd_ctx_index].fwd_context;

            if (fwd_context >= DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_CONTEXTS || fwd_context >= nof_ifwd2_contexts)
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "fwd_context %d is out of bounds.\n", fwd_context);
            }
            if (ctx_used_by_opcode[fwd_context] != DBAL_ENUM_FVAL_KBP_FWD_OPCODE_INVALID)
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "fwd_context %d is used by both opcode_id %d and %d.\n",
                             fwd_context, ctx_used_by_opcode[fwd_context], opcode_id);
            }

            ctx_used_by_opcode[fwd_context] = opcode_id;
        }
    }

    /*
     * Technically there are only DBAL_NOF_ENUM_FWD2_CONTEXT_ID_VALUES Forwarding contexts. So theoretically we were
     * supposed to update the ffc's that belong only the NOF_ENUM_FWD2_CONTEXT first contexts according to Pemla's
     * resolution. All the rest are completely ACL. In fact Pemla marks all contexts, so we can rely on it and run the
    ...
    int unit)
{
    ...
    uint32 mpls_context_enum_val[1] = { DBAL_ENUM_FVAL_FWD2_CONTEXT_ID_MPLS___FWD };
    uint32 mpls_context_encoded_val[1];

    SHR_FUNC_INIT_VARS(unit);

    /** Encode DBAL_ENUM_FVAL_FWD2_CONTEXT_ID_MPLS___FWD */
    SHR_IF_ERR_EXIT(dbal_fields_field_encode(unit, DBAL_FIELD_FWD2_CONTEXT_ID, DBAL_FIELD_EMPTY, 0,
                                             mpls_context_enum_val, mpls_context_encoded_val));
    ...
    for (context_id = 0; context_id < nof_ifwd2_contexts; context_id++)
    {
         ...
         * LOG_ERROR_EX(BSL_LOG_MODULE, "MPLS context has only %d FFCs for ACL purposes. Expected number is at least %d 
         * %s %s\n", nof_ffc, min_nof_acl_ffc * 2, EMPTY, EMPTY); return _SHR_E_RESOURCE; } 
         */
    }
