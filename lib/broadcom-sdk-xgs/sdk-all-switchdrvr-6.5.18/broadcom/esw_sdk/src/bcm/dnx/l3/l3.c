    ...
    int unit)
{
    ...
    int prefix_length_index;
    /** Static KAPS prefix to strength mapping. Format {low limit, high limit, strength} */
    int prefix_length_ranges[5][3] = { {0, 51, 1}, {52, 63, 2}, {64, 64, 3}, {65, 96, 4}, {97, 160, 5} };
    ...
    /*
     * Set the KAPS prefix to entry strength mapping table
     */
    ...
    {
        /*
         * Set all prefixes entry strength to '1' for all interfaces
         */
        dbal_entry_key_field32_range_set(unit, entry_handle_id, DBAL_FIELD_KAPS_INTERFACE, 0, 3);
        dbal_entry_key_field32_range_set(unit, entry_handle_id, DBAL_FIELD_PREFIX_LENGTH, 0, 160);

        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_STRENGTH_NON_DEFAULT, INST_SINGLE, 1);

        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));

        /** set interface 1 strengths (KBR 10) */
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_KAPS_INTERFACE,
                                   DBAL_ENUM_FVAL_KAPS_INTERFACE_INTERFACE_1);

        for (prefix_length_index = 0; prefix_length_index < 5; prefix_length_index++)
        {
            dbal_entry_key_field32_range_set(unit, entry_handle_id, DBAL_FIELD_PREFIX_LENGTH,
                                             prefix_length_ranges[prefix_length_index][0],
                                             prefix_length_ranges[prefix_length_index][1]);

            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_STRENGTH_NON_DEFAULT, INST_SINGLE,
                                         prefix_length_ranges[prefix_length_index][2]);
