 ...
 * {
 */
#define IPMC_SUPPORTED_BCM_IPMC_FLAGS (BCM_IPMC_REPLACE | BCM_IPMC_IP6 | BCM_IPMC_HIT_CLEAR | BCM_IPMC_TCAM | BCM_IPMC_RAW_ENTRY | BCM_IPMC_L2 | BCM_IPMC_HIT)
/*
 ...
 */
#define IPMC_IS_IPV4_HOST_ENTRY(data) (!_SHR_IS_FLAG_SET(data->flags, BCM_IPMC_TCAM) && (data->vid > 0 || data->ing_intf > 0)\
 ...
 */
#define IPMC_IS_IPV6_HOST_ENTRY(data) (!_SHR_IS_FLAG_SET(data->flags, BCM_IPMC_TCAM) && IPMC_IS_IPV6_NO_MASKING(data->mc_ip6_mask)\
    ...
    bcm_ipmc_addr_t * data)
{
    ...
    if (_SHR_IS_FLAG_SET(data->flags, BCM_IPMC_IP6))
    {
        ...
        if (_SHR_IS_FLAG_SET(data->flags, BCM_IPMC_L2)
            || (!IPMC_IS_IPV6_HOST_ENTRY(data) && !_SHR_IS_FLAG_SET(data->flags, BCM_IPMC_TCAM)))
        {
    ...
    uint32 *entry_handle_id)
{
    ...
    if (_SHR_IS_FLAG_SET(data->flags, BCM_IPMC_IP6))
    {
        ...
        {
            dbal_entry_key_field32_masked_set(unit, *entry_handle_id, DBAL_FIELD_GLOBAL_IN_LIF_15_00,
                                              DNX_IPMC_GLOB_IN_LIF_LSB_GET(data->vid), IPMC_GET_IN_LIF_MASK(data->vid));
    ...
    uint32 *dbal_table)
{
    ...
    if (_SHR_IS_FLAG_SET(data->flags, BCM_IPMC_IP6))
    {
        if (_SHR_IS_FLAG_SET(data->flags, BCM_IPMC_TCAM))
        {
            ...
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "Cannot configure IPV6 TCAM table. Table is not supported\n");
                /*
        ...
        else if (_SHR_IS_FLAG_SET(data->flags, BCM_IPMC_L2))
        {
            ...
            if (!IPMC_IS_IPV6_FULL_MASKING(data->s_ip6_mask) && data->mod_id > 0)
            {
                SHR_ERR_EXIT(_SHR_E_UNAVAIL, "LPM IVL table for Bridge IPV6 is not yet supported. \n");
            }
            else if (!IPMC_IS_IPV6_FULL_MASKING(data->s_ip6_mask) && data->mod_id == 0)
            {
                *dbal_table = DBAL_TABLE_BRIDGE_IPV6_MULTICAST_SOURCE_SPECIFIC_SVL;
            }
            else if (data->mod_id > 0)
            {
                SHR_ERR_EXIT(_SHR_E_UNAVAIL, "LEM IVL table for Bridge IPV6 is not yet supported. \n");
            }
            else
            {
                SHR_ERR_EXIT(_SHR_E_UNAVAIL, "LEM SVL Table for Bridge IPV6 is not yet supported. \n");
            }
        }
    ...
    bcm_ipmc_addr_t * dbal_entry)
{
    ...
    else
    {
        ...
        if (IPMC_TABLE_IS_KBP(dbal_table))
        {
            ...
            field_value[0] = IPMC_KBP_IN_LIF_SET(((field_value[0] << IPMC_IPV6_GLOB_IN_LIF_NOF_LSB) | field_value_lsb));
        }
        else
        {
            SHR_IF_ERR_EXIT(dbal_entry_handle_key_field_arr32_get
