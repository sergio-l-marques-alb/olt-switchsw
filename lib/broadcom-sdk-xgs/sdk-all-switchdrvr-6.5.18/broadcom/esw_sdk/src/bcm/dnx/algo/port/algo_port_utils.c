    ...
    bcm_pbmp_t * pbmp_pp_arr)
{
    ...
    for (pp_port = 0; pp_port < dnx_data_port.general.nof_pp_ports_get(unit); pp_port++)
    {
        ...
        {
            SHR_IF_ERR_EXIT(dnx_algo_port_db.pp.valid.get(unit, pp_port_core, pp_port, &valid));
            if (valid)
            {
    ...
    dnx_algo_port_diag_info_all_t * diag_info_all)
{
    ...
    if (DNX_ALGO_PORT_TYPE_IS_NIF(unit, logical_info->port_info, TRUE, TRUE, TRUE, TRUE))
    {
        ...
        if (DNX_ALGO_PORT_TYPE_IS_NIF_ILKN(unit, logical_info->port_info, 1 /* elk */ , 1 /* L1 */ ))
        {
            ...
            /** get is interleaved  */
            SHR_IF_ERR_EXIT(dnx_algo_port_is_ingress_interleave_get(unit, logical_port, &ilkn_info->is_interleaved));
        }
    }
    ...
    char port_type_str[DNX_ALGO_PORT_TYPE_STR_LENGTH])
{
    ...
    switch (port_type)
    {
        ...
        case DNX_ALGO_PORT_TYPE_FLEXE_CLIENT:
            sal_snprintf(port_type_str, DNX_ALGO_PORT_TYPE_STR_LENGTH, "%s", "FLEXE_CLEINT");
            break;
    ...
    int *internal_offset)
{
    ...
    else
    {
        ...
        granularity = dnx_data_nif.global.nif_interface_id_to_unit_id_granularity_get(
    unit);
        granularity_adjusted_nif_id = nif_interface_id - nif_interface_id % granularity;

        *nif_unit_id = dnx_data_nif.global.nif_interface_id_to_unit_id_get(
    unit,
    granularity_adjusted_nif_id)->unit_id;

        *internal_offset = dnx_data_nif.global.nif_interface_id_to_unit_id_get(
    unit,
    granularity_adjusted_nif_id)->internal_offset + nif_interface_id % granularity;
    }
