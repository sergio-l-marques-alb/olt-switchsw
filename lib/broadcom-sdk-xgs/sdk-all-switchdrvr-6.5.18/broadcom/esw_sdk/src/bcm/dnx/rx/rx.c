...
{
    LOG_VERBOSE(BSL_LS_BCM_PKT, (BSL_META("%s %s %u\n\r"), "lb_extension:", "lb_key:", lb_extension->lb_key));
}
    ...
    bcm_pkt_dnx_ftmh_dest_extension_t * dest_extension)
{
    ...
    LOG_VERBOSE(BSL_LS_BCM_PKT, (BSL_META("%s %s %u\n\r"),
                                 "dest_extension:", "dst_sysport:", dest_extension->dst_sysport));
}
...
{
    LOG_VERBOSE(BSL_LS_BCM_PKT, (BSL_META("%s %u %s 0x%x %s 0x%x\n\r"),
                                 "forward_domain:", internal->forward_domain,
    ...
    bcm_pkt_dnx_otsh_t * otsh)
{
    LOG_VERBOSE(BSL_LS_BCM_PKT, (BSL_META("%s %s %s %s %s %u %s %u %s %u %s %u %s %u %s %u %u \n\r"),
                                 "otsh_type:", dnx_rx_dump_otsh_type_to_string(otsh->otsh_type),
                                 "oam_sub_type:", dnx_rx_dump_otsh_oam_subtype_to_string(otsh->oam_sub_type),
...
/**
 * \brief - Map fwd_additional_info to Unknown-address.
 * \param [in] unit - Device number
 * \param [in] fwd_additional_info - Forwarding-Layer-Additional-Info in INTERNAL
 * \param [out] unknown_address - Indicate unknown address
 * \return
 *   None
 * \remark
 *   None
 ...
 */
static shr_error_e
dnx_rx_convert_fai_to_unknow_addr(
    int unit,
    uint32 fwd_additional_info,
    uint8 *unknown_address)
{
    uint32 entry_handle_id;
    ...
    dbal_value_field8_request(unit, entry_handle_id, DBAL_FIELD_BUM_TRAFFIC_ENABLE, INST_SINGLE, unknown_address);
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));
    ...
    bcm_gport_t src_sys_port)
{
    ...
    if ((start_type == DNX_RX_PARSE_HEADER_INTERNAL_VALUE_1) && (layer_index == DNX_RX_PARSE_HEADER_INTERNAL_VALUE_0))
    {
        ...
        /** MAP FAI to unknown_addr */
        SHR_IF_ERR_EXIT(dnx_rx_convert_fai_to_unknow_addr(unit, fld_val, &packet_info->unknown_addr));
    }
    ...
    dnx_rx_parse_headers_t * packet_info)
{
    ...
    /** UDH size in JR mode is compensated per INTERNAL.forward-header-offset */
...
/**
* \brief
*  Configure all that relevant for RX initiation
* \par DIRECT INPUT:
*   \param [in] unit  -  Unit ID
* \par INDIRECT INPUT:
*   * None
...
*   * None
* \remark
*   * 1. SW initialization for CPU RX
*   * 2. Configure all trap related that is relevant for device initiation
* \see
*   * None
*/
shr_error_e
...
/**
* \brief
*   De-allocate all that relevant for RX deinit
* \par DIRECT INPUT:
*   \param [in] unit  -  Unit ID
* \par INDIRECT INPUT:
*   * None
...
*   * None
* \remark
*   * 1. Stop RX for the given unit, save current configuration
*   * 2. free RX pool
*   * 3. Shutdown RX threads
* \see
*   * None
*/
shr_error_e
