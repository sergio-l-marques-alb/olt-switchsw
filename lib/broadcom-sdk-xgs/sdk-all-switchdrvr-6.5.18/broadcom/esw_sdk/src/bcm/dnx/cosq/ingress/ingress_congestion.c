    ...
    int *enable)
{
    ...
    SHR_IF_ERR_EXIT(dnx_ingress_congestion_dbal_port_to_src_vsq_enable_get(unit, core_id, cgm_input_port[0], enable));
    ...
    int vsq_e_id)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_port_info_get(unit, src_port, &port_info));
    if (!DNX_ALGO_PORT_TYPE_IS_NIF(unit, port_info, FALSE, FALSE, FALSE, FALSE)) /** for NIF port, we didn't allocate, so no need to deallocate */
    {
    ...
    bcm_port_t port)
{
    ...
    if (DNX_ALGO_PORT_TYPE_IS_ING_TM(unit, port_info))
    {
        ...
        if (tdm_mode != DNX_ALGO_PORT_TDM_MODE_BYPASS || DNX_ALGO_PORT_TYPE_IS_NIF_ETH(unit, port_info, 0, 0))
        {
    ...
    switch (thresh_info->resource_type)
    {
        ...
        case bcmResourceOcbPacketDescriptors:
            ...
            if (threshold != NULL)
            {
                ...
                SHR_IF_ERR_EXIT(dnx_ingress_congestion_guaranteed_or_global_drop_change_verify(unit, resource_type, 0,
                                                                                               global_drop_pool_dp));
            }
            break;
        default:
    ...
    bcm_cosq_static_threshold_t * threshold)
{
    ...
    SHR_INVOKE_VERIFY_DNX(dnx_ingress_congestion_vsq_shared_free_drop_threshold_arg_verify(unit,
                                                                                           pool_id, dp, thresh_info,
                                                                                           threshold));
    ...
    SHR_IF_ERR_EXIT(dnx_ingress_congestion_shared_reject_thr_calc_and_set(unit, resource_type, 0, global_drop_pool_dp));
    ...
    bcm_cosq_static_threshold_t * threshold)
{
    ...
    {
        case DNX_COSQ_THRESHOLD_INDEX_POOL_DP:
            SHR_IF_ERR_EXIT(dnx_ingress_congestion_vsq_shared_free_drop_threshold_get(unit,
                                                                                      /** indexes */
                                                                                      index_info->pool_id,
                                                                                      index_info->dp,
                                                                                      thresh_info, threshold));
    ...
    int guaranteed_change)
{
    ...
    SHR_IF_ERR_EXIT(dnx_ingress_congestion_guaranteed_or_global_drop_change_verify(unit, res_type, guaranteed_change,
                                                                                   global_drop_pool_dp));
    ...
    uint32 global_drop_pool_dp[][DNX_COSQ_NOF_DP])
{
     ...
     */
    SHR_IF_ERR_EXIT(dnx_ingress_congestion_db.guaranteed.voq_guaranteed.get(unit, res_type, &voq_total_guaranteed));
    ...
    uint32 clear_thr)
{
    ...
    SHR_IF_ERR_EXIT(dnx_ingress_congestion_dbal_voq_shared_res_threshold_set
                    (unit, dbal_table_id, dp, hw_set_thr, hw_clear_thr));
    ...
    int guaranteed_change)
{
    ...
    SHR_IF_ERR_EXIT(dnx_ingress_congestion_global_drop_pool_dp_all_get(unit, res_type, global_drop_pool_dp));
    SHR_IF_ERR_EXIT(dnx_ingress_congestion_shared_reject_thr_calc_and_set(unit, res_type, guaranteed_change,
                                                                          global_drop_pool_dp));
    ...
    uint32 global_drop_pool_dp[][DNX_COSQ_NOF_DP])
{
    ...
    for (dp = 0; dp < DNX_COSQ_NOF_DP; ++dp)
    {
        ...
        if (resources_left)
        {
            drop_percentage = dnx_data_ingr_congestion.info.dp_free_res_presentage_drop_get(unit, dp)->drop_precentage;
        ...
        SHR_IF_ERR_EXIT(dnx_ingress_congestion_shared_res_reject_status_thr_hw_set
                        (unit, res_type, dp, reject_st_thr, reject_st_thr));
    }
    ...
    SHR_IF_ERR_EXIT(dnx_ingress_congestion_db.guaranteed.
                    voq_guaranteed.set(unit, res_type, voq_total_guaranteed + guaranteed_change));
exit:
    SHR_FUNC_EXIT;
}
    ...
    int guaranteed_change[DNX_INGRESS_CONGESTION_RESOURCE_NOF])
{
    ...
    for (res_type = 0; res_type < DNX_INGRESS_CONGESTION_RESOURCE_NOF; res_type++)
    {
        SHR_IF_ERR_EXIT(dnx_ingress_congestion_shared_res_reject_status_thr_set
                        (unit, res_type, guaranteed_change[res_type]));
    }
exit:
    SHR_FUNC_EXIT;
}
    ...
    uint32 nof_remapped_voqs)
{
    ...
    for (res_type = 0; res_type < DNX_INGRESS_CONGESTION_RESOURCE_NOF; res_type++)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_ingress_congestion_res_type_guaranteed_verify(unit, res_type, guaranteed_change));
    }
exit:
    SHR_FUNC_EXIT;
}
    ...
    bcm_cosq_ingress_queue_bundle_gport_config_t * config)
{
    ...
    for (rate_class_index = 0; rate_class_index < nof_rate_class; rate_class_index++)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_ingress_congestion_rate_class_total_guaranteed_verify
                        (unit, old_rate_class, rate_class, rate_class_queues_to_add));
    }
exit:
    SHR_FUNC_EXIT;
}
    ...
    int rate_class)
{
     ...
     */
    SHR_IF_ERR_EXIT(dnx_ingress_congestion_dbal_voq_rate_class_get(unit, qid, &old_rate_class));

    /*
     * Validate that re-mapping the queue is legal for quaranteed thresholds.
     */
    SHR_IF_ERR_EXIT(dnx_ingress_congestion_rate_class_total_guaranteed_verify(unit, old_rate_class, rate_class, 1));
    ...
    int rate_class)
{
    ...
    SHR_INVOKE_VERIFY_DNX(dnx_ingress_congestion_voq_to_rate_class_profile_map_set_verify(unit, voq, rate_class));

    SHR_IF_ERR_EXIT(dnx_ingress_congestion_dbal_voq_rate_class_get(unit, voq, &old_rate_class));
    ...
    if (old_rate_class != rate_class)
    {
         ...
         */
        SHR_IF_ERR_EXIT(dnx_ingress_congestion_dbal_voq_rate_class_set(unit, voq, rate_class));
        ...
        SHR_IF_ERR_EXIT(dnx_ingress_congestion_shared_res_reject_status_thr_all_res_types_set(unit, guaranteed_change));

        /*
         * update ref count for the rate classes
         */
        SHR_IF_ERR_EXIT(dnx_ingress_congestion_db.voq_rate_class.ref_count.dec(unit, old_rate_class, 1));
    ...
    int *rate_class)
{
    ...
    SHR_IF_ERR_EXIT(dnx_ingress_congestion_dbal_voq_rate_class_get(unit, voq, &rate_class_profile));
    ...
    bcm_cosq_inst_congestion_info_data_t * data)
{
    ...
    if (key->type == bcmCosqInstCongestionInfoTypeVoq)
    {
        /** get data */
