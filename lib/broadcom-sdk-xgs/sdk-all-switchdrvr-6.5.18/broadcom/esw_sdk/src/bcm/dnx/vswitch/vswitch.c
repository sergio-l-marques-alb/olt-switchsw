    ...
    int *is_p2p)
{
    ...
    *is_p2p = FALSE;
    ...
    if ((is_field_on_handle == TRUE) && (service_type == DBAL_ENUM_FVAL_VTT_LIF_SERVICE_TYPE_SERVICE_TYPE_P2P))
    {
        *is_p2p = TRUE;
    }
    ...
    bcm_gport_t gport2)
{
    ...
    else
    {
         ...
         */
        if (BCM_GPORT_SUB_TYPE_IS_VIRTUAL_INGRESS_NATIVE(gport1))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
    ...
    dnx_algo_gpm_forward_info_t * forward_info)
{
    ...
    SHR_VAL_VERIFY(!(temp_destination | temp_outlif | temp_eei), 0, _SHR_E_NOT_FOUND, "Port is not cross-connected\n");
    ...
    dnx_algo_gpm_forward_info_t * forward_info)
{
    ...
    SHR_IF_ERR_EXIT(dnx_vswitch_cross_connect_get_forward_info(unit, entry_handle_id, forward_info));
    ...
    dnx_algo_gpm_forward_info_t * forward_info)
{
    ...
    SHR_IF_ERR_EXIT(dnx_vswitch_cross_connect_get_forward_info(unit, entry_handle_id, &current_forward_info));
    ...
    dnx_algo_gpm_forward_info_t * forward_info)
{
    ...
    else if (forward_info->fwd_info_result_type == DBAL_RESULT_TYPE_L2_GPORT_TO_FORWARDING_SW_INFO_DEST_ONLY)
    {
        ...
        if (gport_hw_resources->inlif_dbal_table_id == DBAL_TABLE_IN_LIF_FORMAT_PWE &&
            gport_hw_resources->inlif_dbal_result_type == DBAL_RESULT_TYPE_IN_LIF_FORMAT_PWE_IN_LIF_PWE2EEI_NO_LEARNING)
        {
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_EEI, INST_SINGLE, 0);
        }
    }
