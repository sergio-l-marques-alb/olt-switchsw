     ...
     */ if (LOG_CHECK(BSL_VERBOSE | BSL_LS_BCMDNX_TEMPLATEMNGR))
    {
        LOG_VERBOSE(BSL_LS_BCMDNX_TEMPLATEMNGR,
                    (BSL_META
                     ...
                     unit, algo_temp->name, flags, profile_data, extra_arguments));
        if (_SHR_IS_FLAG_SET(flags, DNX_ALGO_TEMPLATE_ALLOCATE_WITH_ID))
        {
...
exit:
    if (LOG_CHECK(BSL_VERBOSE | BSL_LS_BCMDNX_TEMPLATEMNGR))
    {
        ...
        if (SHR_GET_CURRENT_ERR() == _SHR_E_NONE)
        {
            ...
            LOG_VERBOSE(BSL_LS_BCMDNX_TEMPLATEMNGR, (BSL_META(", profile_data:%p"), profile_data));
    ...
    if (LOG_CHECK(BSL_VERBOSE | BSL_LS_BCMDNX_TEMPLATEMNGR))
    {
        LOG_VERBOSE(BSL_LS_BCMDNX_TEMPLATEMNGR,
                    (BSL_META
                     ...
                     unit, algo_temp->name, profile, new_profile_data));
    }
    ...
    if (LOG_CHECK(BSL_VERBOSE | BSL_LS_BCMDNX_TEMPLATEMNGR))
    {
        LOG_VERBOSE(BSL_LS_BCMDNX_TEMPLATEMNGR,
                    (BSL_META("\n dnx_algo_template_profile_get(unit:%d, name:%s, profile_data:%p)\n"),
                     unit, algo_temp->name, profile_data));
    }
    ...
    uint8 *last_reference)
{
    ...
    if (ref_count_old != 0)
    {
         ...
         */
        SHR_IF_ERR_EXIT(Template_callbacks_map_sw[algo_temp->advanced_algorithm].free_cb(unit, module_id, algo_temp->multi_set_template, old_profile, 1 /* nof_refs 
                                                                                                                                                         */ ,
                                                                                         last_reference));
        /*
    ...
    /*
     *  In this if-else block we perform the allocation of the ref.
     */
    if (_SHR_IS_FLAG_SET(flags, DNX_ALGO_TEMPLATE_ALLOCATE_WITH_ID) && (new_profile_old_ref_count > 0))
    {

        /*
         *  If data already exists, take the existing data.
         */
        SHR_IF_ERR_EXIT(Template_callbacks_map_sw[algo_temp->advanced_algorithm].allocate_cb(unit, module_id, algo_temp->multi_set_template, flags, 1   /* nof_refs 
    ...
    else        /* either not WITH_ID or WITH_ID but data did not exist */
    {
        /*
         *  Add new data.
         */
    ...
    if (_SHR_IS_FLAG_SET(flags, DNX_ALGO_TEMPLATE_EXCHANGE_TEST))
    {
         ...
         */
        SHR_IF_ERR_EXIT(Template_callbacks_map_sw[algo_temp->advanced_algorithm].free_cb(unit, module_id, algo_temp->multi_set_template, *new_profile, 1        /* nof_refs 
                                                                                                                                                                 */ ,
                                                                                         &test_last_reference));
...
exit:
    if (SHR_FUNC_ERR() || !add_success || _SHR_IS_FLAG_SET(flags, DNX_ALGO_TEMPLATE_EXCHANGE_TEST))
    {
        ...
        if (old_profile_freed)
        {
            ...
            rv = Template_callbacks_map_sw[algo_temp->advanced_algorithm].allocate_cb(unit, module_id, algo_temp->multi_set_template, DNX_ALGO_TEMPLATE_ALLOCATE_WITH_ID, 1     /* nof_refs 
                                                                                                                                                                                 */ ,
                                                                                      old_profile_data,
                                                                                      (void *) extra_arguments,
                                                                                      &old_profile,
    ...
    if (LOG_CHECK(BSL_VERBOSE | BSL_LS_BCMDNX_TEMPLATEMNGR))
    {
        LOG_VERBOSE(BSL_LS_BCMDNX_TEMPLATEMNGR,
                    (BSL_META
                     ...
                     unit, algo_temp->name, flags, profile_data, old_profile));
        if (_SHR_IS_FLAG_SET(flags, DNX_ALGO_TEMPLATE_ALLOCATE_WITH_ID))
        {
