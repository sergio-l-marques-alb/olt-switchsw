    ...
    int *flags)
{
    ...
    if (time_analyze_mode_str != NULL && (!sal_strncasecmp("PER_STEP", time_analyze_mode_str, strlen("PER_STEP"))))
    {
    ...
    int forward)
{
     ...
     */
    SHR_IF_ERR_EXIT(dnx_init_seq_convert(unit, flags, dnx_init_info, dnx_init_deinit_seq, seq_ptr_arr[unit], forward));
...
int
bcm_dnx_init_only_enough_for_access(
    int unit,
    int action)
{
    SHR_FUNC_INIT_VARS(unit);
