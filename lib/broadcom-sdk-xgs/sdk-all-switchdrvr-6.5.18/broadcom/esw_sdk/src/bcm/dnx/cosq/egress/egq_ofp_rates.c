        ...
        BCM_PBMP_ITER(port_bm, logical_port)
        {
            ...
            if (cal_rate)
            {
                ...
                SHR_IF_ERR_EXIT(dnx_ofp_rate_db.otm_cal.nof_calcal_instances.get
                                (unit, core, calendar_id, &calcal_instances));
                /*
    ...
    uint32 *if_shaper_rate_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_ofp_rates_calculate_rate_from_credits
                    (unit, calcal_length, calcal_instances, 1, DNX_EGQ_UNITS_VAL_IN_BITS_INTF_RECYCLE, &internal_rate));
    ...
    dnx_egq_calendar_tbl_data_t * egq_calendar_tbl_data_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_dbal_gen_set(unit, egq_credit_table_name, 3, 3,
                                     ...
                                     (uint32) (egq_calendar_tbl_data_p->port_max_credit), GEN_DBAL_FIELD_LAST_MARK));
    /*
    ...
    uint32 *port_rate_p)
{
    ...
    if (port_rate_p[0] != 0)
    {
         ...
         * 
         *                 numInstances [slot/calCycle] * total_port_credits [credits/calCycle] * core_frequency [kilo-cycle/sec]
         *   -----------------------------------------------------------------------------------------------------------------------------------
        ...
        /*
         * If remainder is non-zero then add '1' to calculated rate (calc)
         */
 ...
 *   to each port, the corresponding rate (in kilo bits per sec).
 *   Note that there are two modes: In one, all ports are loaded while,
 *   in the other, only one specified port is loaded (saves execution time.
 * \param [in] unit -
 *   Identifier of HW platform.
 * \param [in] core -
 *   Core to use to get internal input parameters (calCal lenght, no. of instances
 ...
 * \param [in] calendar_p -
 *   Pointer to dnx_ofp_rates_cal_egq_t which contains an array of type
 ...
 *   uint32. Number of elements in 'slots[]'. See 'calendar_p' above.
 * \param [in] nof_ports -
 *   uint32. 
 *   If 'specific_port' below is negative, this is the number of elements (ports) in array
 *     'ports_rates' below. This is the number of the ports for which 'rate' is to be
 *     calculated.
 *   Else
 *     This is just the maximal number of ports and is only used to verify 'specific_port'
 *     is within range.
 * \param [in] specific_port -
 *   int32. 
 *   If 'specific_port' is negative, then calculate 'rates' for all ports (as per 'nof_ports'
 *     and load into array ports_rates[]
 *   Else
 *     Calculate 'rate' for 'specific_port' ONLY. In that case, 'specific_port' must be a
 *     legitimate 'port index' in the range 0 - nof_ports. In that case, 'ports_rates' is
 *     assumed to be an array of ONE ELEMENT only and this one element is loaded by the
 *     assigned 'rate' of 'specific_port'.
 * \param [out] ports_rates -
 *   Pointer tp uint32. 
 *   If 'specific_port' is negative, this is an array whose index is 'port id'. Each value
 *     is the rate, in kbps, assigned to this 'port id'.
 *     Caller is reponsible to assign a large enough array.
 *   Else, this is an array of one element which is loaded by the rate, in kbps, assigned
 *     to 'specific_port'.
 *
...
dnx_ofp_rates_from_generic_calendar_to_ports_egq_rate(
    ...
    uint32 *ports_rates)
{
    uint32 port_idx, slot_id, egq_resolution;
    ...
    if (cal_info_p->cal_type == DNX_OFP_RATES_EGQ_CAL_CHAN_ARB)
    {
        SHR_IF_ERR_EXIT(dnx_ofp_rate_db.calcal_len.get(unit, core, &calcal_length));
        SHR_IF_ERR_EXIT(dnx_ofp_rate_db.otm_cal.nof_calcal_instances.get
                        (unit, core, cal_info_p->chan_arb_id, &calcal_instances));
        /*
         * If callender type is channelized arbiter EGQ resolution is 1/256 
         */
    ...
    else
    {
        ...
        calcal_instances = 1;
        /*
         * If callender type is QPAIR or TCG, EGQ resolution is 1/256
         */
        egq_resolution = DNX_EGQ_UNITS_VAL_IN_BITS_QPAIR_TCG;
    }
    if ((int) specific_port >= 0)
    {
        /*
         * We are in the mode where 'specific_port' is the single port for which 'rate' is required.
         * Verify that it is a legitimate port number.
         */
        if (specific_port >= nof_ports)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "Illegal 'specific_port' (%d). Must be smaller than %d", specific_port,
                         nof_ports);
        }
        ports_rates[0] = 0;
        for (slot_id = 0; slot_id < calendar_len; ++slot_id)
        {
            ...
            {
                /*
                 * Increase the total sum of credits of this 'specific _port'
                 * by the val of this slot
                 */
         ...
         */
        SHR_IF_ERR_EXIT(dnx_ofp_rates_calculate_rate_from_credits
                        (unit, calcal_length, calcal_instances, calendar_len, egq_resolution, &ports_rates[0]));
    }
    else
    {
        /*
         * We are in the mode where 'rate' is required for ALL ports.
         * Verify that it is a legitimate port number.
         */
        ...
        /*
         *  Calculate shaper rate for each port_idx:
         */
        for (port_idx = 0; port_idx < nof_ports; ++port_idx)
        {
            SHR_IF_ERR_EXIT(dnx_ofp_rates_calculate_rate_from_credits
                            (unit, calcal_length, calcal_instances, calendar_len, egq_resolution,
                             &ports_rates[port_idx]));
        }
    }
exit:
    SHR_FUNC_EXIT;
    ...
    uint32 *rate)
{
    ...
    else
    {
        ...
        SHR_IF_ERR_EXIT(dnx_ofp_rates_from_generic_calendar_to_ports_egq_rate(unit,
                                                                              ...
                                                                              &egq_cal,
                                                                              egq_cal_len,
                                                                              DNX_EGR_NOF_BASE_Q_PAIRS, base_q_pair,
                                                                              egq_rates));
        /*
    ...
    int core)
{
    ...
    for (chan_arb_i = 0; chan_arb_i < dnx_data_egr_queuing.params.nof_calendars_get(unit); ++chan_arb_i)
    {
        SHR_IF_ERR_EXIT(dnx_ofp_rate_db.otm_cal.nof_calcal_instances.set
                        (unit, core, chan_arb_i, egq_calcal_instances[chan_arb_i]));
    }
exit:
    ...
    bcm_core_t core)
{
    ...
    SHR_FUNC_INIT_VARS(unit);
    SHR_ALLOC(logical_ports_p, sizeof(*logical_ports_p) * SOC_PBMP_PORT_MAX, "logical_ports_p",
              "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
    if (dnx_data_egr_queuing.params.feature_get(unit, dnx_data_egr_queuing_params_conditional_crdt_table_access))
    {
        /*
        ...
        if (nof_instances > 0)
        {
            ...
            if (cal_slots)
            {
                 ...
                 */
                {
                    ...
                    SHR_IF_ERR_EXIT(dnx_ofp_rate_db.calcal_len.get(unit, core, &calcal_length));
                    SHR_IF_ERR_EXIT(dnx_ofp_rate_db.otm_cal.nof_calcal_instances.get
                                    (unit, core, chan_arb_i, &calcal_instances));
                    SHR_IF_ERR_EXIT(dnx_ofp_rates_set_select_get(unit, core, cal_info.cal_type, &fld_val));
                    /*
                     * Select the non-active set.
                     */
                    egq_to_set =
                        (fld_val == DNX_OFP_RATES_CAL_SET_A) ? DNX_OFP_RATES_CAL_SET_B : DNX_OFP_RATES_CAL_SET_A;
                    /*
                ...
                for (cal_slots = 0; cal_slots < egq_calendar_len; cal_slots++)
                {
                    ...
                    {
                        /*
                         * From the port get the associated shaper mode (DATA or PACKET) and number of priorities
                         * The base Q-pair is known.
                         */
                        SHR_IF_ERR_EXIT(dnx_egr_queuing_packet_mode_get(unit, logical_port_i, &packet_mode));
                    }
                    if (packet_mode == 0)
                    {
                        /*
    ...
    uint32 *tcg_rate)
{
    ...
    sal_memset(&cal_info, 0x0, sizeof(cal_info));
    {
        ...
        SHR_IF_ERR_EXIT(dnx_ofp_rates_from_generic_calendar_to_ports_egq_rate(unit,
                                                                              ...
                                                                              &egq_cal,
                                                                              egq_cal_len,
                                                                              DNX_EGR_NOF_Q_PAIRS, tcg_id, egq_rates));
        /*
         * Get the rate of the requested port
         */
    ...
    uint32 flags)
{
    ...
    if (DNX_ALGO_PORT_TYPE_IS_EGR_TM(unit, port_info))
    {
        ...
        if (!channelized)
        {
            ...
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "total_rate(=%d) <  prev_kbits_sec_max(=%d)",
                             total_rate, prev_kbits_sec_max);
            }
