    ...
    imb_specific_create_info_t * imb_specific_info)
{
    ...
    /*
     * Specifically in Jr2, the ilkn id correlates with the ILKN unit id. otherwise, we need to iterate over all ILKN
     * IDs in the ILKN unit.
     */
    ilkn_id = imb_info->inst_id * dnx_data_nif.ilkn.ilkn_unit_if_nof_get(unit);
    /*
    ...
    uint32 flags)
{
    ...
    if (is_master_port)
    {
        ...
        /*
         * Set ETH units (CDU/CLU) ilkn Data MUX
         */
...
imb_ile_port_detach(
    ...
    bcm_port_t port)
{
    int is_over_fabric, nof_channels, ilkn_nof_ports, is_core_reset, master_port, is_master_port, phy;
    ...
    /*
     * Remove the PM mapping if port is master
     */
    SHR_IF_ERR_EXIT(dnx_algo_port_to_ilkn_master_get(unit, port, &master_port));
    is_master_port = (port == master_port);
    ...
    if (nof_channels == 1)
    {
        ...
        {
            /*
             *  3. Disable fabric SerDes for ILKN over fabric
             */
    ...
    {
        /*
         * 4. Set CLU ilkn Data MUX
         */
        if ((ilkn_info.pm_imb_type == imbDispatchTypeImb_clu)
            && (dnx_data_nif.ilkn.feature_get(unit, dnx_data_nif_ilkn_clu_ilkn_selectors)))
        {
            SHR_IF_ERR_EXIT(imb_ile_port_mux_selectors_reset(unit, port, ilkn_nof_ports));
        }
    }
    ...
    int speed)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_port_ilkn_access_info_get(unit, port, &ilkn_info));
    SHR_IF_ERR_EXIT(dnx_algo_port_imb_type_to_pm_type_get(unit, ilkn_info.pm_imb_type, (int *) &pm_type));
    is_valid = FALSE;
    /**
    ...
    int is_set)
{
    ...
    switch (type)
    {
        ...
        case BCM_PORT_PHY_CONTROL_TX_POLARITY:
        {
            if (is_set)
            {
                SHR_ERR_EXIT(_SHR_E_UNAVAIL, "PHY control type %d set is not supported on Blackhawk/Falcon PHY.\r\n",
                             type);
            }
            break;
        }
