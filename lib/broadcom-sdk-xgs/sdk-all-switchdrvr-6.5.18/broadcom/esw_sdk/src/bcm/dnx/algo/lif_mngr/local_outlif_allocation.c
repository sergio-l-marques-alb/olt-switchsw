    ...
    if (_SHR_IS_FLAG_SET(outlif_hw_info->flags, DNX_ALGO_LIF_MNGR_LOCAL_OUTLIF_UPDATE_DATA_BANK))
    {
        ...
        if (tmp == DNX_ALGO_OUTLIF_BANK_ASSIGNED_TO_EEDB)
        {
            ...
            nof_outlifs_per_eedb_bank =
                EEDB_MAX_DATA_ENTRY_SIZE / address_granularity * dnx_data_mdb.dh.nof_rows_in_eedb_bank_get(unit);
    ...
    dnx_algo_local_outlif_hw_info_t * outlif_hw_info)
{
    ...
    else
    {
         ...
         * If the bank traffic lock bug is in effect, always use the banks with linked list because some phases will have a problem to
         *
         */
        ...
        /*
         * If we have the eedb traffic lock bug, and there's no available outlif bank in the mdb cluster, then it's likely
        ...
        if (outlif_bank == DNX_ALGO_LOCAL_OUTLIF_INVALID_BANK
            ...
            && dnx_data_mdb.hw_bug.feature_get(unit, dnx_data_mdb_hw_bug_eedb_bank_traffic_lock))
        {
            SHR_IF_ERR_EXIT(dnx_algo_local_outlif_allocation_find_existing_bank_to_allocate(unit,
                                                                                            outlif_info->local_lif_flags,
                                                                                            logical_phase, TRUE,
    ...
    int *ll_match)
{
    ...
    SHR_IF_ERR_EXIT(dnx_lif_mngr_outlif_sw_info_get
                    (unit, outlif_info->local_outlif, NULL, NULL, NULL, NULL, &alternative_ll_required));
    ...
    dnx_local_outlif_ll_indication_e * ll_indication)
{
    ...
    if (sal_strncasecmp(ll_indicaiton_name_str, "Mandatory", DBAL_MAX_STRING_LENGTH) == 0)
    {
        *ll_indication = DNX_LOCAL_OUTLIF_LL_ALWAYS;
    }
    else if (sal_strncasecmp(ll_indicaiton_name_str, "Optional", DBAL_MAX_STRING_LENGTH) == 0)
    {
        *ll_indication = DNX_LOCAL_OUTLIF_LL_OPTIONAL;
    }
    else if (sal_strncasecmp(ll_indicaiton_name_str, "Never", DBAL_MAX_STRING_LENGTH) == 0)
    {
        *ll_indication = DNX_LOCAL_OUTLIF_LL_NEVER;
    }
    else if (sal_strncasecmp(ll_indicaiton_name_str, "NA", DBAL_MAX_STRING_LENGTH) == 0)
    {
        *ll_indication = DNX_LOCAL_OUTLIF_LL_OPTIONAL;
    }
    else
...
shr_error_e
dnx_algo_local_outlif_eedb_ovf_set(
    int unit,
    uint8 enable_data_ovf,
    uint8 enable_ll_ovf)
{
    ...
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_EEDB_OVF_CONFIGURATION, &entry_handle_id));

    dbal_entry_key_field32_range_set(unit, entry_handle_id, DBAL_FIELD_EEDB_ENCAP_STAGE_INDEX, DBAL_RANGE_ALL,
                                     DBAL_RANGE_ALL);

    dbal_entry_value_field8_set(unit, entry_handle_id, DBAL_FIELD_ENABLE_DATA_OVF, INST_SINGLE, enable_data_ovf);
    dbal_entry_value_field8_set(unit, entry_handle_id, DBAL_FIELD_ENABLE_LL_OVF, INST_SINGLE, enable_ll_ovf);
