    ...
    int priority)
{
    ...
    /** verify prio */
    if (prio >= dnx_data_nif.prd.nof_priorities_get(unit))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "unit %d, priority %d is out of range\n", unit, prio);
    }
    /** verify is_tdm */
    if (is_tdm && !dnx_data_tdm.params.feature_get(unit, dnx_data_tdm_params_is_supported))
    {
