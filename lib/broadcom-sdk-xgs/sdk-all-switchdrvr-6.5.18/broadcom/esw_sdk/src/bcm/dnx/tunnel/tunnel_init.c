    ...
    bcm_tunnel_type_t tunnel_type)
{
    ...
    {
        case bcmTunnelTypeGreAnyIn4:
            return dnx_data_tunnel.general.esem_cmd_arr_prefix_gre_any_ipv4_get(unit);
        default:
            return 0;
    }
}
    ...
    uint32 *lif_additional_header_profile)
{
    ...
    switch (bcm_tunnel_type)
    {
        ...
        case bcmTunnelTypeIpAnyIn6:
        {
            ...
            {
                *lif_additional_header_profile = DBAL_ENUM_FVAL_LIF_AHP_GENERAL_LIF_AHP_NONE;
            }
            break;
        }
        case bcmTunnelTypeUdp:
        ...
        case bcmTunnelTypeRspanAdvanced:
        {
            /** no lif additional headewr profile for those tunnel type */
            break;
        }
        default:
        {
    ...
    bcm_tunnel_type_t * tunnel_type)
{
    ...
    if (_SHR_IS_FLAG_SET(table_specific_flags, DNX_ALGO_LIF_TABLE_SPECIFIC_GRE_ANY_IN_4))
    {
        *tunnel_type = bcmTunnelTypeGreAnyIn4;
             ...
             _SHR_IS_FLAG_SET(table_specific_flags, DNX_ALGO_LIF_TABLE_SPECIFIC_BFD_SIP_WA))
    {
...
{
    uint32 esem_cmd_range_flags = DNX_PORT_ESEM_CMD_RANGE_0_3;
    ...
    if (_SHR_IS_FLAG_SET(tunnel_esem_access_flags, TUNNEL_INIT_ESEM_ACCESS_TYPE_VXLAN_VNI))
    {
        SHR_IF_ERR_EXIT(dnx_port_esem_command_add
                        (unit, DBAL_ENUM_FVAL_ESEM_APP_DB_ID_FODO_NAMESPACE_CTAG, ESEM_ACCESS_TYPE_VXLAN_VNI,
    ...
    {
        SHR_IF_ERR_EXIT(dnx_port_esem_command_add
                        (unit, DBAL_ENUM_FVAL_ESEM_APP_DB_ID_OUT_LIF, ESEM_ACCESS_TYPE_SVTAG,
                         DNX_PORT_ESEM_DEFAULT_RESULT_PROFILE_INVALID, &esem_accesses));
    }

    SHR_IF_ERR_EXIT(dnx_port_esem_cmd_exchange(unit, tunnel_id,
                                               DNX_PORT_ESEM_CMD_ALLOCATE | esem_cmd_range_flags, esem_cmd_prefix,
                                               &esem_accesses,
    ...
    bcm_tunnel_initiator_t * tunnel)
{
    ...
    dnx_tunnel_initiator_is_bfd_sip_wa(unit, tunnel->type, tunnel->dip, tunnel->sip, tunnel->l3_intf_id,
                                       &tunnel_is_bfd_sip_wa);
            ...
            else
            {
                ...
                SHR_IF_ERR_EXIT(dnx_tunnel_init_ipv4_ipv6_eedb_entry_get
                                (unit, gport_hw_resources.outlif_dbal_table_id, gport_hw_resources.local_out_lif,
                                 ...
                                 &old_ttl_pipe_profile, &is_ttl_pipe_mode, &qos_model_dummy_var, &nwk_qos_profile,
                ...
                SHR_IF_ERR_EXIT(dnx_tunnel_initiator_sip_ttl_qos_table_delete
                                (unit, local_outlif, old_sip_profile, old_ttl_pipe_profile,
                                 nwk_qos_profile, remove_old_sip_profile, remove_old_ttl_profile,
    ...
    bcm_tunnel_initiator_t * tunnel)
{
     ...
     * UDP src/dst ports.
     */
    if ((tunnel->type == bcmTunnelTypeUdp) && lif_additional_header_profile != 0)
    {
        udp_ports_t udp_ports;
    ...
    if (esem_cmd_profile != dnx_data_esem.access_cmd.no_action_get(unit))
    {
        ...
        tunnel->flags |= (svtag_present * BCM_MPLS_EGRESS_LABEL_FLAGS2_SVTAG_ENABLE);
    }
    ...
    bcm_l3_intf_t * intf)
{
    ...
    if ((gport_hw_resources.outlif_dbal_table_id == DBAL_TABLE_EEDB_IPV4_TUNNEL) &&
        (gport_hw_resources.outlif_dbal_result_type == DBAL_RESULT_TYPE_EEDB_IPV4_TUNNEL_ETPS_BFD_IPV4_SIP_WA))
    {
        SHR_IF_ERR_EXIT(dnx_tunnel_initiator_bfd_sip_wa_clear
                        (unit, gport_hw_resources.global_out_lif, gport_hw_resources.local_out_lif));
        SHR_EXIT();
    }
     ...
     */
    if (tunnel_is_ipv4 && (tunnel_type == bcmTunnelTypeUdp) && (lif_additional_header_profile != 0))
    {
        old_udp_ports_profile = lif_additional_header_profile;
