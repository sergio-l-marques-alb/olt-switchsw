...
/**
 * \brief - Provide mapping between source system port and gport PTC
 *
 * \param [in] unit - Unit id
 * \param [in] core_id - In core id
    ...
    int preserve_dscp_mode)
{
    ...
    if (BCM_GPORT_IS_TUNNEL(gport) && BCM_GPORT_TUNNEL_ID_GET(gport) < dnx_data_l3.rif.nof_rifs_get(unit))
    {
        /** It's a RIF*/
        ...
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, gport_hw_resources.inlif_dbal_table_id, &entry_handle_id));
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_VSI, gport_hw_resources.local_in_lif);
        SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_GET_ALL_FIELDS));
        ...
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get(unit, entry_handle_id, DBAL_FIELD_VSI_PROFILE, INST_SINGLE,
                                                            (uint32 *) &old_in_lif_profile));
        old_in_lif_profile = CALCULATE_VSI_PROFILE_FROM_ETH_RIF_PROFILE(VSI_MSB_PREFIX, old_in_lif_profile);
    ...
    if (old_in_lif_profile != new_in_lif_profile)
    {
        ...
        else
        {
            ...
            uint32 new_vsi_profile;
            new_vsi_profile = CALCULATE_VSI_PROFILE_FROM_ETH_RIF_PROFILE(VSI_MSB_PREFIX, new_in_lif_profile);

            dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_VSI, gport_hw_resources.local_in_lif);
            dbal_entry_value_field16_set(unit, entry_handle_id, DBAL_FIELD_VSI_PROFILE, INST_SINGLE, new_vsi_profile);
        }
    ...
    int *preserve_dscp_mode)
{
    ...
    if (BCM_GPORT_IS_TUNNEL(gport) && BCM_GPORT_TUNNEL_ID_GET(gport) < dnx_data_l3.rif.nof_rifs_get(unit))
    {
        ...
        in_lif_profile = CALCULATE_VSI_PROFILE_FROM_ETH_RIF_PROFILE(VSI_MSB_PREFIX, in_lif_profile);
    ...
    dbal_enum_value_field_ptc_profile_vtt4_e vtt4_cs_profile)
{
    ...
    BCM_PBMP_ITER(gport_info.local_port_bmp, local_port)
    {
        ...
        dbal_entry_key_field16_set(unit, entry_handle_id, DBAL_FIELD_PTC, ptc);
        /** Value fields */
        dbal_entry_value_field8_set(unit, entry_handle_id, DBAL_FIELD_VTT1_PTC_CS_PROFILE, INST_SINGLE,
                                    vtt1_cs_profile);
        dbal_entry_value_field8_set(unit, entry_handle_id, DBAL_FIELD_VTT2_PTC_CS_PROFILE, INST_SINGLE,
                                    vtt2_cs_profile);
        dbal_entry_value_field8_set(unit, entry_handle_id, DBAL_FIELD_VTT3_PTC_CS_PROFILE, INST_SINGLE,
                                    vtt3_cs_profile);
        dbal_entry_value_field8_set(unit, entry_handle_id, DBAL_FIELD_VTT4_PTC_CS_PROFILE, INST_SINGLE,
                                    vtt4_cs_profile);
    ...
    dbal_enum_value_field_ptc_profile_vtt4_e * vtt4_cs_profile)
{
    ...
    dbal_value_field8_request(unit, entry_handle_id, DBAL_FIELD_VTT1_PTC_CS_PROFILE, INST_SINGLE, &profile1);
    dbal_value_field8_request(unit, entry_handle_id, DBAL_FIELD_VTT2_PTC_CS_PROFILE, INST_SINGLE, &profile2);

    dbal_value_field8_request(unit, entry_handle_id, DBAL_FIELD_VTT3_PTC_CS_PROFILE, INST_SINGLE, &profile3);
    dbal_value_field8_request(unit, entry_handle_id, DBAL_FIELD_VTT4_PTC_CS_PROFILE, INST_SINGLE, &profile4);
    ...
    uint32 enable)
{
    ...
    BCM_PBMP_ITER(gport_info.local_port_bmp, local_port)
    {
        ...
        dbal_entry_value_field8_set(unit, entry_handle_id, DBAL_FIELD_PTC_ROUTING_ENABLE_PROFILE_VTT1, INST_SINGLE,
                                    enable ? DBAL_ENUM_FVAL_PTC_ROUTING_ENABLE_PROFILE_VTT1_ALWAYSTERMINATE :
                                    DBAL_ENUM_FVAL_PTC_ROUTING_ENABLE_PROFILE_VTT1_DEFAULT);
        dbal_entry_value_field8_set(unit, entry_handle_id, DBAL_FIELD_VTT2_PTC_CS_PROFILE, INST_SINGLE,
                                    enable ? DBAL_ENUM_FVAL_PTC_PROFILE_VTT2_2ND_PASS_DROP_AND_CONTINUE :
                                    DBAL_ENUM_FVAL_PTC_PROFILE_VTT2_DEFAULT);
        dbal_entry_value_field8_set(unit, entry_handle_id, DBAL_FIELD_VTT3_PTC_CS_PROFILE, INST_SINGLE,
                                    enable ? DBAL_ENUM_FVAL_PTC_PROFILE_VTT3_2ND_PASS_DROP_AND_CONTINUE :
    ...
    uint32 *enable)
{
    ...
    dbal_value_field8_request(unit, entry_handle_id, DBAL_FIELD_VTT2_PTC_CS_PROFILE, INST_SINGLE, &field_val);
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));

    *enable = (field_val == DBAL_ENUM_FVAL_PTC_PROFILE_VTT2_2ND_PASS_DROP_AND_CONTINUE) ? 1 : 0;
    ...
    int switch_header_type)
{
    ...
    if ((count_olp > 1) && (!BCM_GPORT_IS_TRUNK(port)))
    {
        ...
        /** INDENT-OFF */
        /*
         * Following is the OLP ports port_profile configurations to support limit WA and J1 mode learning WA:
         *
         *                   | olp_0                           |olp_1 |olp_2                            |olp_3                   |
         * ----------------------------------------------------|------|---------------------------------|------------------------|-----------
         * term port profile | JR1_DSP_SWITCH_DEST_ENC_JR1_ARAD| 0    | -                               | -                      |  A0 - j1
         * fwd port profile  | ETHERNET_TYPICAL                | 0    | -                               | -                      |
         * ----------------------------------------------------------------------------------------------------------------------------------
         * term port profile | JR2_DSP_CMD_TO_REFRESH          | 0    | -                               | -                      |  B - j2
         * fwd port profile  | 0                               | 0    | -                               | -                      |
         * ----------------------------------------------------------------------------------------------------------------------------------
         * term port profile | JR1_DSP_SWITCH_DEST_ENC_JR1_ARAD| 0    | JR1_DSP_SWITCH_DEST_ENC_JR1_ARAD| 0                      |  B - j1
         * fwd port profile  | JR1_DSP_CMD_TO_REFRESH          | 0    | JR1_TYPE_A1                     | JR1_DSP_CMD_TO_REFRESH |
         */
        ...
        if (gport_info.local_port == gport_info_olp.local_port)
        {
            ...
            {
                /*
                 * JR1 headers mode: DSP change from Arad to Jericho WA
                 */
                ...
                {
                    /*
                     * JR1 headers mode: MACT limit WA
                     */
            ...
            else if (dnx_data_l2.hw_bug.feature_get(unit, dnx_data_l2_hw_bug_learn_limit))
            {
                /*
                 * MACT limit WA
                 */
        ...
        {
            /*
             * JR1 headers mode: DSP change from Arad to Jericho WA, no MACT limit WA
             */
            egress_port_info.term_port_profile =
                DBAL_ENUM_FVAL_ETPP_TERMINATION_PORT_CS_VAR_MACT_WA_JR1_DSP_SWITCH_DEST_ENC_JR1_ARAD;
            egress_port_info.forward_port_profile = DBAL_ENUM_FVAL_ETPP_FORWARD_PORT_CS_VAR_ETHERNET_TYPICAL;
        ...
        {
            /*
             * JR1 headers mode: MACT limit WA + DSP change from Arad to Jericho WA
             */
            egress_port_info.forward_port_profile =
                DBAL_ENUM_FVAL_ETPP_FORWARD_PORT_CS_VAR_MACT_WA_JR1_DSP_CMD_TO_REFRESH;
        }
    ...
    uint32 header_type)
{
    ...
    BCM_PBMP_ITER(gport_info->local_port_bmp, local_port)
    {
        ...
        if (header_type == BCM_SWITCH_PORT_HEADER_TYPE_MPLS_RAW)
        {
             ...
             */
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_VTT1_PTC_CS_PROFILE,
                                         INST_SINGLE, DBAL_ENUM_FVAL_PTC_PROFILE_VTT1_RAW_MPLS);
            /*
        ...
        else
        {
             ...
             */
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_VTT1_PTC_CS_PROFILE,
                                         INST_SINGLE, DBAL_ENUM_FVAL_PTC_PROFILE_VTT1_DEFAULT);
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_PTC_ROUTING_ENABLE_PROFILE_VTT1, INST_SINGLE,
                                         DBAL_ENUM_FVAL_PTC_ROUTING_ENABLE_PROFILE_VTT1_DEFAULT);
        }
        for (pp_port_index = 0; pp_port_index < gport_info->internal_port_pp_info.nof_pp_ports; pp_port_index++)
    ...
    uint32 *data)
{
    ...
    if (!SHR_FAILURE(rv))
    {
        ...
        data_mask = 0;
        SHR_BITSET_RANGE(&data_mask, 0, field_info_0.field_nof_bits);
        dbal_entry_handle_value_field32_get(unit, entry_handle_id, DBAL_FIELD_LIF_GENERIC_DATA_0, INST_SINGLE,
                                            &data_tmp);
        *data = data_tmp & data_mask;

        data_mask = 0;
        SHR_BITSET_RANGE(&data_mask, field_info_0.field_nof_bits, field_info_1.field_nof_bits);
        dbal_entry_handle_value_field32_get(unit, entry_handle_id, DBAL_FIELD_LIF_GENERIC_DATA_1, INST_SINGLE,
                                            &data_tmp);
    ...
    bcm_port_flood_group_t * flood_groups)
{
     ...
     */
    bc_same_as_unknown_mc = dnx_data_l2.hw_bug.feature_get(unit, dnx_data_l2_hw_bug_bc_same_as_unknown_mc);
 ...
 */
shr_error_e
dnx_port_lif_layer_protocol_dbal_key_value_set(
    int unit,
    uint32 entry_handle_id,
    ...
    uint32 flag)
{
    ...
    if ((!_SHR_IS_FLAG_SET(flag, BCM_HASH_LAYER_ETHERNET_DISABLE))
        && (!_SHR_IS_FLAG_SET(flag, BCM_HASH_LAYER_IPV4_DISABLE))
        && (!_SHR_IS_FLAG_SET(flag, BCM_HASH_LAYER_IPV6_DISABLE))
        && (!_SHR_IS_FLAG_SET(flag, BCM_HASH_LAYER_MPLS_DISABLE))
        && (!_SHR_IS_FLAG_SET(flag, BCM_HASH_LAYER_TCP_DISABLE))
        && (!_SHR_IS_FLAG_SET(flag, BCM_HASH_LAYER_UDP_DISABLE)))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "The flag %d that is provided is not supported!", flag);
    }

    if (_SHR_IS_FLAG_SET(flag, BCM_HASH_LAYER_ETHERNET_DISABLE))
    {
        dbal_entry_key_field32_set(unit, entry_handle_id, field_id, DBAL_ENUM_FVAL_LAYER_TYPES_ETHERNET);
    }
    if (_SHR_IS_FLAG_SET(flag, BCM_HASH_LAYER_IPV4_DISABLE))
    {
        dbal_entry_key_field32_set(unit, entry_handle_id, field_id, DBAL_ENUM_FVAL_LAYER_TYPES_IPV4);
    }
    if (_SHR_IS_FLAG_SET(flag, BCM_HASH_LAYER_IPV6_DISABLE))
    {
        dbal_entry_key_field32_set(unit, entry_handle_id, field_id, DBAL_ENUM_FVAL_LAYER_TYPES_IPV6);
    }
    if (_SHR_IS_FLAG_SET(flag, BCM_HASH_LAYER_MPLS_DISABLE))
    {
        dbal_entry_key_field32_set(unit, entry_handle_id, field_id, DBAL_ENUM_FVAL_LAYER_TYPES_MPLS_UNTERM);
    }
    if (_SHR_IS_FLAG_SET(flag, BCM_HASH_LAYER_TCP_DISABLE))
    {
        dbal_entry_key_field32_set(unit, entry_handle_id, field_id, DBAL_ENUM_FVAL_LAYER_TYPES_TCP);
    }
    if (_SHR_IS_FLAG_SET(flag, BCM_HASH_LAYER_UDP_DISABLE))
    {
        dbal_entry_key_field32_set(unit, entry_handle_id, field_id, DBAL_ENUM_FVAL_LAYER_TYPES_UDP);
    }
    ...
    uint32 flags)
{
    ...
    /** Exchange the new_port_profile, based on the new flags*/
    SHR_IF_ERR_EXIT(dnx_pp_profile_exchange(unit, old_port_profile, flags, &new_port_profile, &is_first, &is_last));

    /** Update INGRESS_PP_PORT table with the new_port_profile value*/
    if (is_first)
    {
        /** Set key fields*/
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_PP_PORT,
        ...
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID,
                                   gport_info.internal_port_pp_info.core_id[0]);
        /** Update the new port lb profile*/
        ...
        dnx_port_lif_layer_protocol_dbal_key_value_set(unit, entry_handle_id, DBAL_FIELD_LAYER_PROTOCOL, flags);
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_PORT_LB_PROFILE, new_port_profile);
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_IN_LIF_LB_PROFILE, 0);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_USE_LAYER_RECORD, INST_SINGLE, 0);
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    }
    ...
    if (is_last)
    {
        ...
        dbal_entry_key_field32_range_set(unit, entry_handle_id, DBAL_FIELD_LAYER_PROTOCOL, DBAL_RANGE_ALL,
                                         DBAL_RANGE_ALL);
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_PORT_LB_PROFILE, old_port_profile);
