    ...
    dnx_oam_mep_profile_t * mep_profile_data)
{
    ...
    DNX_OAM_SET_LMM_OFFSET(ccm_mep_db_entry->mep_type,
                           ...
                           mep_profile_data->piggy_back_lm, mep_profile_data->lmm_offset);
    DNX_OAM_SET_LMR_OFFSET(ccm_mep_db_entry->mep_type,
                           ccm_mep_db_entry->nof_vlan_tags,
                           mep_profile_data->piggy_back_lm, mep_profile_data->lmr_offset);
    ...
    dnx_oam_mep_profile_t * mep_profile_data)
{
     ...
     */
    DNX_OAM_SET_LMM_OFFSET(ccm_mep_db_entry->mep_type,
                           ccm_mep_db_entry->nof_vlan_tags, 0, mep_profile_data->lmm_offset);
    DNX_OAM_SET_LMR_OFFSET(ccm_mep_db_entry->mep_type,
                           ccm_mep_db_entry->nof_vlan_tags, 0, mep_profile_data->lmr_offset);
    /*
     * Setting phase count for LMM
     */
    ...
    uint32 lm_stat_entry_idx)
{
    ...
    {
        if (dm_exists && (MEP_DB_ENTRY_TO_OAM_ID(ccm_mep_db_entry->flex_lm_dm_entry) != dm_entry_idx))
        {
            /*
 ...
 * \param [in] group_info - Pointer to the group info,
    ...
    bcm_oam_loss_t * loss_ptr)
{
    ...
    uint8 dm_exists = 0, lm_exists = 0, lm_stat_exists = 0, flexibe_da_enabled = 0;
    SHR_FUNC_INIT_VARS(unit);
    ...
    if (!DNX_OAM_MEP_IS_BETWEEN_4_7_BANK(loss_ptr->id) &&
        (dnx_data_oam.hw_bug.feature_get(unit, dnx_data_oam_hw_bug_oamp_txm_bank_limitation)) &&
    ...
    if ((loss_ptr->pkt_pri_bitmap != 0xff) && (loss_ptr->pkt_pri_bitmap != 0))
    {
         ...
         */
        DNX_OAM_GET_PRIORITY_FROM_BITMAP(loss_ptr->pkt_pri_bitmap, priority);
        SHR_IF_ERR_EXIT(dnx_oam_oamp_lm_dm_session_get(unit, loss_ptr->id, priority, &loss_ptr_id, &dm_exists));
        if (dm_exists)
        {
     ...
     */
    SHR_IF_ERR_EXIT(dnx_oam_loss_add_verify(unit, loss_ptr, ccm_mep_db_entry, dm_exists, dm_entry_idx, &priority));
    ...
    if ((loss_ptr->pkt_pri_bitmap != 0xff) && (loss_ptr->pkt_pri_bitmap != 0) && !(loss_ptr->flags & BCM_OAM_LOSS_SLM))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_oam_add_loss_delay_pcp
                        (unit, curr_mep_profile_data, hw_write_info, &endpoint_info, &group_info, ccm_mep_db_entry,
    ...
    SHR_IF_ERR_EXIT(dnx_oam_loss_mep_id_alloc(unit, loss_ptr, ccm_mep_db_entry, dm_exists, dm_entry_idx,
                                              lm_mep_db_entry->lm_db_entry_idx, lm_stat_exists,
                                              lm_mep_db_entry->lm_stat_entry_idx));
    ...
    bcm_oam_loss_t * loss_ptr)
{
    ...
    /** Set key */
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_OAMP_ENTRY_ID, loss_ptr->loss_id);
    ...
    bcm_oam_loss_t * loss_ptr)
{
    ...
    if (loss_ptr->flags & BCM_OAM_LOSS_UPDATE_NEXT_RECEIVED_SLR)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_oam_loss_measure_next_slr_set(unit, loss_ptr));
    }
    else
    {
    ...
    int *measurement_period)
{
    ...
    /** Read format   */
    dbal_entry_handle_value_field32_get(unit, entry_handle_id,
                                        DBAL_FIELD_TXM_SLM_MEASUREMENT_PERIOD_TIMER_INIT_VALUE__N, INST_SINGLE,
    ...
    else
    {
        ...
        for (priority = 0; priority < 8; priority++)
        {
            ...
            if (is_exists)
            {
                 ...
                 * Delete loss with priority
                 */
                loss_with_priority_exists |= is_exists;
                loss_ptr->id = session_oam_id;
    ...
    uint8 *priority)
{
    ...
    DNX_OAM_GET_PRIORITY_FROM_BITMAP(delay_ptr->pkt_pri_bitmap, *priority)
         ...
         */
        SHR_IF_ERR_EXIT(dnx_oam_lm_dm_add_mep_db_entry_verify
                        (unit, delay_ptr, ccm_mep_db_entry, 1 /* DM */ , lm_exists, lm_db_entry_ptr));
    ...
    uint32 lm_stat_entry_idx)
{
    ...
    {
        if (lm_exists && (MEP_DB_ENTRY_TO_OAM_ID(ccm_mep_db_entry->flex_lm_dm_entry) != lm_db_entry_idx))
        {
            /*
             * DM entry is first.
    ...
    bcm_oam_delay_t * delay_ptr)
{
    ...
    SHR_IF_ERR_EXIT(dnx_oam_delay_add_verify
                    (unit, delay_ptr, ccm_mep_db_entry, lm_db_exists, lm_db_entry_ptr, &priority));
    ...
    if (delay_ptr->tlvs[0].tlv_type == bcmOamTlvTypeData)
    {
        ...
        hw_write_info->is_48b_group = (_SHR_IS_FLAG_SET(group_info.flags, BCM_OAM_GROUP_FLEXIBLE_MAID_48_BYTE));
        sw_endpoint_info.sw_state_flags |= DBAL_DEFINE_OAM_ENDPOINT_SW_STATE_FLAGS_JUMBO_DM_TLV;
