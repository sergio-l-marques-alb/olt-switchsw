    ...
    bcm_cosq_delay_tolerance_t * delay_tolerance)
{
    ...
    if (!(preset_attr->flags & BCM_COSQ_DELAY_TOLERANCE_PRESET_ATTR_F_PUSH_QUEUE))
    {
        ...
        /** IF((CreditResolution=0),128-32,IF((CreditResolution =1),256-64,IF((CreditResolution =2),512-96))) */
        /** credit_resolution variable hold 1, 2 or 4 */
        /*
         * (credit_resolution - credit_resolution / 4) introduces the following mapping
         * 1 -> 1
         * 2 -> 2
         * 4 -> 3
         */
        delay_tolerance->credit_request_satisfied_backoff_enter_thresh = (128 * credit_resolution - 32 * (credit_resolution - credit_resolution / 4)) * 1024;    /** bytes */
        delay_tolerance->credit_request_satisfied_backoff_exit_thresh = (128 * credit_resolution - 32 * (credit_resolution - credit_resolution / 4)) * 1024;     /** bytes */
        delay_tolerance->credit_request_satisfied_backlog_enter_thresh = (128 * credit_resolution - 32 * (credit_resolution - credit_resolution / 4)) * 1024;    /** bytes */
        delay_tolerance->credit_request_satisfied_backlog_exit_thresh = (128 * credit_resolution - 32 * (credit_resolution - credit_resolution / 4)) * 1024;     /** bytes */
        ...
        /** IF(Slow Enable,2*CreditSize, MIN(500*1000, 2*MaxInflight[Credits]*CreditSize)) */
        delay_tolerance->credit_request_satisfied_empty_queue_max_balance_thresh = slow_enable ? 2 * preset_attr->credit_size : 2 * max_inflight_bytes;   /** bytes */
        ...
        /** if(SlowEnable, min(500,ceiling.math(2*(MaxInflight[Credits]+1)* CreditSize/1024)),-(ceiling.math(3* MaxInflight[Credits]* CreditSize /1024))) */
        delay_tolerance->credit_request_hungry_off_to_normal_thresh =
            slow_enable ? (2 * (max_inflight_credits + 1) * preset_attr->credit_size) : (-1 * 3 * max_inflight_bytes);
                                                                                                                                 /** bytes */
        ...
        if (slow_enable)
        {
            ...
            for (slow_level_index = 0; slow_level_index < BCM_COSQ_DELAY_TOLERANCE_SLOW_LEVELS; slow_level_index++)
            {
                ...
                else
                {
                    ...
                    {
                        default_slow_rates[slow_level_index] =
                            UTILEX_DIV_ROUND_UP(default_slow_rates[slow_level_index - 1] * factor, 100);
                    }
    ...
    int unit)
{
    ...
    if (dnx_data_iqs.dqcq.feature_get(unit, dnx_data_iqs_dqcq_8_priorities))
    {
         ...
         */
        SHR_IF_ERR_EXIT(bcm_dnx_cosq_control_set(unit, 0, -1, bcmCosqControlIngressMaxLowPriority,
