    ...
    bcm_port_resource_t * resource)
{
    ...
    if (is_flexe_enabled)
    {
        ...
        if (flexe_mode_distributed)
        {
             ...
             * All the traffic to ILKN are from BusC or BusB directly.
             */
    ...
    if (is_flexe_in_use)
    {
        ...
        {
            /** When flexe in centralized mode, total NIF ports speed cannot exceed 800G(exclude 80,81) */
    ...
    dnx_port_resource_port_info_t * ports_info)
{
    ...
    for (resource_index = 0; resource_index < nport; ++resource_index)
    {
        ...
        if (DNX_ALGO_PORT_TYPE_IS_TM(unit, ports_info[resource_index].port_info))
        {
        ...
        SHR_IF_ERR_EXIT(dnx_algo_port_soc_info_port_bitmaps_set
                        (unit, resource[resource_index].port, port_info, tdm_mode));
    }
    if (dnx_data_nif.flexe.feature_get(unit, dnx_data_nif_flexe_is_supported))
    {
    ...
    for (resource_index = 0; resource_index < nport; ++resource_index)
    {
            ...
            || DNX_ALGO_PORT_TYPE_IS_L1(unit, ports_info[resource_index].port_info))
        {
            ...
            if (dnx_data_esb.general.feature_get(unit, dnx_data_esb_general_esb_support))
            {
                SHR_IF_ERR_EXIT(dnx_esb_nif_calendar_set(unit));
                break;
            }
        }
    }
    ...
    dnx_port_resource_port_info_t * ports_info)
{
    ...
    for (resource_index = 0; resource_index < nport; ++resource_index)
    {
        if (DNX_ALGO_PORT_TYPE_IS_NIF(unit, ports_info[resource_index].port_info, FALSE, FALSE, TRUE, FALSE))
        {
    ...
    bcm_port_resource_t * resource)
{
    ...
    for (resource_index = 0; resource_index < nport; ++resource_index)
    {
        if (DNX_ALGO_PORT_TYPE_IS_IMB(unit, ports_info[resource_index].port_info, TRUE))
        {
            ...
            SHR_IF_ERR_EXIT(imb_port_enable_set(unit, resource[resource_index].port, 0, 0));
        }
    }
    ...
    for (resource_index = 0; resource_index < nport; ++resource_index)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_algo_port_speed_set
                        (unit, resource[resource_index].port, DNX_ALGO_PORT_SPEED_F_TX,
                         resource[resource_index].speed));
        /*
        ...
        rx_speed =
            (resource[resource_index].
             flags & BCM_PORT_RESOURCE_ASYMMETRICAL) ? resource[resource_index].rx_speed : resource[resource_index].
            speed;
        SHR_IF_ERR_EXIT(dnx_algo_port_speed_set
                        (unit, resource[resource_index].port, DNX_ALGO_PORT_SPEED_F_RX, rx_speed));
    }
    ...
    for (resource_index = 0; resource_index < nport; ++resource_index)
    {
        if (DNX_ALGO_PORT_TYPE_IS_IMB(unit, ports_info[resource_index].port_info, TRUE))
        {
            ...
            {
                /** enable port */
                SHR_IF_ERR_EXIT(imb_port_enable_set(unit, resource[resource_index].port, 0, 1));
            }
            /*
