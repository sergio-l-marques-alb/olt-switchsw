    ...
    bcm_vlan_control_vlan_t * control)
{
    ...
    /**ERRATA #136 - ETPP ENC push VSI statistics entry regardless of use_vsd context enabler*/
    ...
    if (control.ingress_stat_pp_profile != STAT_PP_PROFILE_INVALID)
    {
        if (result_type != DBAL_RESULT_TYPE_ING_VSI_INFO_DB_VSI_ENTRY_WITH_STATS)
        {
            SHR_IF_ERR_EXIT(dbal_entry_result_type_update
                            (unit, entry_handle_id, DBAL_RESULT_TYPE_ING_VSI_INFO_DB_VSI_ENTRY_WITH_STATS,
                             DBAL_RESULT_TYPE_UPDATE_MODE_LOOSE));
        }

        if (STAT_PP_PROFILE_ID_GET(control.ingress_stat_pp_profile) > 1)
        {
