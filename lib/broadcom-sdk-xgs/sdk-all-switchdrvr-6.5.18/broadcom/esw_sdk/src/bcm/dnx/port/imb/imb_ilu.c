    ...
    imb_specific_create_info_t * imb_specific_info)
{
    ...
    {
        ilkn_id = (imb_info->inst_id) * dnx_data_nif.ilkn.ilkn_unit_if_nof_get(unit) + ilkn_iter;
        /*
         * ELK is defined as static configuration - it should be defined using SOC properties. so we initalize the ILKN on
         * top of NIF or Fabric according to SOC properties
         * If there is at least one ILKN over fabric port, mark ILKN over fabric
         * is enabled on the ILKN core.
         */
    ...
    bcm_port_t port)
{
         ...
         ++ilkn_hrf_type)
    {
        ...
        else
        {
             ...
             */
            SHR_IF_ERR_EXIT(imb_ilu_port_scheduler_config_set(unit, port, 0));

            /*
             * Enable flush contexts in NMG
             */
            SHR_IF_ERR_EXIT(imb_ilu_internal_port_rx_nmg_flush_context_enable_hw_set
                            (unit, ilkn_access_info.ilkn_core, ilkn_hrf_type, TRUE));
             ...
             */
            SHR_IF_ERR_EXIT(imb_ilu_internal_port_rx_nmg_flush_context_enable_hw_set
                            (unit, ilkn_access_info.ilkn_core, ilkn_hrf_type, FALSE));

            /*
             * Restore the priority in the NIF scheduler round-robin
             */
            SHR_IF_ERR_EXIT(imb_ilu_port_scheduler_config_set(unit, port, 1));
        }
...
exit:
    ...
    {
        SHR_IF_ERR_RETURN(imb_ilu_port_scheduler_config_set(unit, port, 1));
    }
    SHR_FUNC_EXIT;
}
    ...
    int enable)
{
    ...
    if (enable)
    {
         ...
         * Enable ILE
         */
        SHR_IF_ERR_EXIT(imb_ile_port_enable_set(unit, port, 0, enable));
    ...
    else
    {
        ...
        {
            /*
             * Flush and reset the Rx HRF controller
             */
...
imb_ilu_prd_threshold_set(
    ...
    uint32 threshold)
{
    int nof_segments, is_extended_memory_used, hrf_size;
    SHR_FUNC_INIT_VARS(unit);
