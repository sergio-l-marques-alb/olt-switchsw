    ...
    int trap_id)
{
    ...
    if (is_oam_punt_or_protection == FALSE)
    {
        ...
        if (predefined_trap_info.trap_class != DNX_RX_TRAP_CLASS_USER_DEFINED)
        {
            ...
            if (predefined_trap_info.trap_class != DNX_RX_TRAP_CLASS_OAM)
            {
                SHR_IF_ERR_EXIT(dbal_fields_enum_hw_value_get(unit, DBAL_FIELD_INGRESS_TRAP_ID, trap_id, &hw_value));
                if ((hw_value == 0) && (type != bcmRxTrapLinkLayerHeaderSizeErr))
                {
                    SHR_ERR_EXIT(_SHR_E_UNAVAIL, "Trap bcmRxTrap%s is not supported on the current device \n",
                                 predefined_trap_info.trap_name);
    ...
    uint8 enable)
{
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_CFG_GLOBAL_MODE_VISIBILITY_TRAP_EN,
                                 INST_SINGLE, enable);
    ...
    int *trap_id_p)
{
    ...
    is_punt_or_protection = (((type >= bcmRxTrapOampTrapErr) && (type <= bcmRxTrapOampProtection)) ||
                             ...
                             (type == bcmRxTrapOampPuntNextGoodPacket));
    ...
    switch (trap_block)
    {
        ...
        case DNX_RX_TRAP_BLOCK_OAMP:
            ...
            /**
             * We do not want to encode Trap Class to OAMP Trap error traps, so we leave it as 0,
             * else we will change the supplied trap_ID. They need to be in 0x400 - 0x4FF range.
             */
    ...
    *is_strengthless = TRUE;
    ...
    if (ingress_ud_info.valid == FALSE)
    {
        for (trap_type_idx = 0; trap_type_idx < bcmRxTrapCount; trap_type_idx++)
        {
            ...
            if (predefined_trap_info.trap_id == trap_type)
            {
    ...
    bcm_rx_trap_config_t * config_p)
{
    ...
    if ((dnx_data_trap.ingress.feature_get(unit, dnx_data_trap_ingress_stat_sampling_code_clear_action) == FALSE) &&
        (config_p->flags2 & BCM_RX_TRAP_FLAGS2_STAT_SAMPLING_CODE_CLEAR))
    {
        SHR_ERR_EXIT(_SHR_E_UNAVAIL, "Statistical sampling code clear action does not exist on this device! \n");
    }
    ...
    bcm_rx_mtu_profile_value_t * mtu_data_p)
{
     ...
     * Values
     */
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_MTU, INST_SINGLE, mtu_data_p->mtu_val);
    trap_id = BCM_GPORT_TRAP_GET_ID(mtu_data_p->trap_gport);
    ...
    bcm_rx_mtu_profile_value_t * mtu_data_p)
{
     ...
     * Values
     */
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_MTU, INST_SINGLE, &(mtu_data_p->mtu_val));
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_TRAP_ACTION_PROFILE, INST_SINGLE,
                               (uint32 *) &fwd_act_prof);
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_TRAP_FWD_STRENGTH, INST_SINGLE,
    ...
    dnx_rx_trap_etpp_config_t * etpp_config_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_rx_trap_etpp_set_verify(unit, trap_class, etpp_config_p));
    ...
    int unit)
{
    ...
    dbal_fields_e fwd1_fields[] = { DBAL_FIELD_FWD1___MPLS___FWD, DBAL_FIELD_FWD1___MPLS___FWD_COUPLED,
...
* \par DIRECT INPUT: 
*   \param [in] unit       -  unit Id
*   \param [in] word_index -  IPv6 32b chunk index for NDPMyIP
...
* \par DIRECT INPUT:
*   \param [in] unit      -  unit Id
*   \param [in] config_p   -   My-ARP protocol traps additional configuration
* \par INDIRECT INPUT:
*   * None
* \par DIRECT OUTPUT:
*   shr_error_e - Error type
...
/**
* \brief
*  Get additional configuration of My-ARP protocol trap from HW.
* \par DIRECT INPUT:
*   \param [in] unit      -  unit Id
*   \param [in] config_p  -   My-ARP protocol traps additional configuration
* \par INDIRECT INPUT:
    ...
    bcm_gport_t lif_gport)
{
    ...
    if ((lif_hw_resources_p->outlif_dbal_table_id == DBAL_TABLE_EEDB_OUT_AC) ||
        (lif_hw_resources_p->outlif_dbal_table_id == DBAL_TABLE_EEDB_PWE) ||
        (lif_hw_resources_p->outlif_dbal_table_id == DBAL_TABLE_EEDB_IPV4_TUNNEL) ||
        (lif_hw_resources_p->outlif_dbal_table_id == DBAL_TABLE_EEDB_SRV6) ||
...
static shr_error_e
bcm_dnx_rx_trap_svtag_verify_common(
    int unit,
    ...
    bcm_gport_t action_gport)
{
    ...
    if (svtag_trap_key->pkt_type > bcmbcmSvtagPktTypeKayMgmt)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "The Packet type is out of range! Maximum value is bcmbcmSvtagPktTypeKayMgmt");
    }
...
*/
static shr_error_e
bcm_dnx_rx_trap_svtag_set_verify(
    int unit,
    bcm_rx_trap_svtag_key_t * svtag_trap_key,
    bcm_gport_t action_gport)
{
    ...
    SHR_IF_ERR_EXIT(bcm_dnx_rx_trap_svtag_verify_common(unit, svtag_trap_key, action_gport));
    ...
    bcm_gport_t action_gport)
{
    ...
    SHR_IF_ERR_EXIT(bcm_dnx_rx_trap_svtag_set_verify(unit, svtag_trap_key, action_gport));
