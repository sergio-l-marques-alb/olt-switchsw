 ...
 * \param [in] unit                 - Device ID
 * \param [in] field_stage          - The stage for whcih we check the context selection.
    ...
    if (entry_id_p->stage == DNX_FIELD_STAGE_EXTERNAL && entry_data_intern_pruned.entry_valid)
    {
        ...
        if (fwd_nof_contexts <= 0)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "For Apptype %d, opcode_id %d, the number of FWD contexts is %d. "
                         "There must be at least one fwd_contest per apptype.\n", apptype, opcode_id, fwd_nof_contexts);
        }
        ...
        else
        {
            ...
            SHR_IF_ERR_EXIT(dnx_field_map_apptype_to_opcode_bcm_to_dnx(unit, apptype, &opcode_id));
            SHR_IF_ERR_EXIT(kbp_mngr_opcode_to_contexts_get(unit, opcode_id, &fwd_nof_contexts, fwd_acl_ctx_mapping));
            /** Sanity check: Verify that there is only one acl_context.*/
            for (fwd_ctx_idx = 0; fwd_ctx_idx < fwd_nof_contexts; fwd_ctx_idx++)
            {
                if (fwd_acl_ctx_mapping[fwd_ctx_idx].nof_acl_contexts != 1)
                {
                    SHR_ERR_EXIT(_SHR_E_INTERNAL, "For Apptype %d, opcode_id %d, fwd_context %d, the number of "
                                 "acl_context is %d, must be 1.\n",
                                 apptype, opcode_id, fwd_acl_ctx_mapping[fwd_ctx_idx].fwd_context,
                                 fwd_acl_ctx_mapping[fwd_ctx_idx].nof_acl_contexts);
                }
                if (fwd_acl_ctx_mapping[0].acl_contexts[0] != fwd_acl_ctx_mapping[fwd_ctx_idx].acl_contexts[0])
                {
                    SHR_ERR_EXIT(_SHR_E_INTERNAL, "For user defined Apptype %d, opcode_id %d "
                                 "there is more than one acl_context (%d and %d at indices %d and %d). "
                                 "Only one acl_context per user defined Apptype is supported.\n",
                                 apptype, opcode_id,
                                 fwd_acl_ctx_mapping[0].acl_contexts[0],
                                 fwd_acl_ctx_mapping[fwd_ctx_idx].acl_contexts[0], 0, fwd_ctx_idx);
                }
            }
    ...
    dnx_field_presel_entry_data_t * entry_data_p)
{
    ...
    if (entry_data_p->entry_valid && (entry_id_p->stage == DNX_FIELD_STAGE_EXTERNAL))
    {
        ...
        else if (enable_context == DNX_FIELD_PRESEL_CTX_ID_VALID_SET)
        {
             ...
             */
            SHR_IF_ERR_EXIT(dnx_field_map_apptype_to_opcode_bcm_to_dnx(unit, base_apptype, &base_opcode_id));
            SHR_IF_ERR_EXIT(kbp_mngr_opcode_to_contexts_get
                            (unit, base_opcode_id, &fwd_nof_contexts, fwd_acl_ctx_mapping));
            /** Sanity check.*/
            if (fwd_nof_contexts <= 0)
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL,
                             "Opcode %d (AppType %d) has no fwd_contexts (fwd_nof_contexts %d).\n",
                             base_opcode_id, base_apptype, fwd_nof_contexts);
            }
            SHR_IF_ERR_EXIT(kbp_mngr_context_to_opcode_get
                            (unit, fwd_acl_ctx_mapping[0].fwd_context, entry_data_p->context_id, &opcode_id, NULL));
    ...
    dnx_field_presel_entry_data_t * dnx_entry_data_p)
{
    ...
    for (qual_idx = 0; qual_idx < bcm_entry_data_p->nof_qualifiers; qual_idx++)
    {
         ...
         */
        SHR_IF_ERR_EXIT(dnx_field_presel_cs_qual_supported(unit, entry_id_p->stage,
        ...
        if (qual_is_supported == FALSE)
        {
            SHR_ERR_EXIT(_SHR_E_NOT_FOUND, "BCM Qualifier %s (%d) with argument %d is not supported for stage:%s "
                         ...
                         bcm_entry_data_p->qual_data[qual_idx].qual_arg, dnx_field_stage_text(unit, entry_id_p->stage));
        }
    }
