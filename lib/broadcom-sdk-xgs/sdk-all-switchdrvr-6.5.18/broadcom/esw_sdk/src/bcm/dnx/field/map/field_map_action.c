    ...
    const dnx_field_action_map_t ** action_map_entry_p)
{
    ...
    else if ((bcm_action >= 0) && (bcm_action < bcmFieldActionCount))
    {
        ...
        if (dnx_global_action_map[bcm_action].dnx_action != 0)
        {
            ...
            dnx_action |= (stage << DNX_ACTION_STAGE_SHIFT);
        }
        else
        {
            uint8 is_std_1;
            ...
            if (!is_std_1 && (dnx_field_map_stage_info[stage].static_action_id_map[bcm_action].conversion_cb != NULL))
            {
                SHR_SET_CURRENT_ERR(_SHR_E_NOT_FOUND);
                SHR_EXIT();
            }
             ...
             (bcm_action < dnx_data_field.action.vw_1st_get(unit) + dnx_data_field.action.vw_nof_get(unit)))
    {
        ...
        if (dnx_per_vw_action_info[dnx_vw_action_info[action_id].vw_index].nof_actions_per_stage[stage] <=
            dnx_vw_action_info[action_id].action_index_within_vw)
        {
            SHR_SET_CURRENT_ERR(_SHR_E_NOT_FOUND);
            SHR_EXIT();
        ...
        dnx_action = DNX_ACTION(DNX_FIELD_ACTION_CLASS_VW, stage, action_id);
    }
    else
    {
        /*
    ...
    dnx_field_action_t * dnx_action_p)
{
    ...
    rv = dnx_field_map_action_bcm_to_dnx_int(unit, stage, bcm_action, dnx_action_p, &action_map_entry);
    if (rv == _SHR_E_NOT_FOUND)
    {
        uint8 is_std_1;
        SHR_IF_ERR_EXIT(dnx_pp_prgm_default_image_check(unit, &is_std_1));
        if (!is_std_1 && DNX_FIELD_MAP_ACTION_MAPPING_HAS_VALUE_CONVERSION(action_map_entry))
        {
            SHR_IF_ERR_EXIT_WITH_LOG(rv,
                                     "BCM Action %s(%d) is not supported in BareMetal mode! Only RAW actions are supported!%s",
    ...
    int unit)
{
    ...
    for (i_bcm_act = 0; i_bcm_act < bcmFieldActionCount; i_bcm_act++)
    {
        ...
        else
        {
            ...
            DNX_FIELD_STAGE_ACTION_ITERATOR(dnx_stage)
            {
                ...
                {
                    SHR_ERR_EXIT(_SHR_E_INTERNAL, "BCM Action %s(%d) not supported for stage:%s\n",
    ...
    for (i_bcm_act = 0; i_bcm_act < bcmFieldActionCount; i_bcm_act++)
    {
        ...
        if ((ISEMPTY(bcm_action_description[i_bcm_act])) == FALSE)
        {
            ...
            DNX_FIELD_STAGE_ACTION_ITERATOR(dnx_stage)
            {
                rv = dnx_field_map_action_bcm_to_dnx_int(unit, dnx_stage, i_bcm_act, &dnx_action, &action_map_entry_p);
