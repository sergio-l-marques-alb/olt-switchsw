    ...
    bcm_pbmp_t * okay_pbmp)
{
    ...
    if (!is_init_sequence)
    {
        ...
        else
        {
            ...
            BCM_PBMP_ITER(*okay_pbmp, logical_port)
            {
                BCM_PBMP_PORT_ADD(*counter_pbmp_p, logical_port);
            }
        }
        /*
    ...
    int unit)
{
    ...
    int fmac_sbus_chain[DNX_DATA_MAX_FABRIC_BLOCKS_NOF_INSTANCES_FMAC];
    int enable = 0;
    ...
    for (index = nof_fsrds - 1; index >= 0; --index)
    {
    ...
    {
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_FSRD, fsrd_last_in_chain);
    ...
    for (fmac = 0; fmac < nof_fmacs; fmac++)
    {
        ...
        if (enable)
        {
            ...
            fmac_sbus_chain[dnx_data_fabric.blocks.fmac_sbus_chain_get(unit, fmac)->index_in_chain] = fmac;
        }
    }
    for (index = nof_fmacs - 1; index >= 0; --index)
    {
        if (fmac_sbus_chain[index] != -1)
        {
            fmac_last_in_chain = fmac_sbus_chain[index];
            break;
        }
    }
    if (fmac_last_in_chain != -1)
    {
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_MAC, fmac_last_in_chain);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_IS_LAST_IN_CHAIN, INST_SINGLE, 1);
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    }
