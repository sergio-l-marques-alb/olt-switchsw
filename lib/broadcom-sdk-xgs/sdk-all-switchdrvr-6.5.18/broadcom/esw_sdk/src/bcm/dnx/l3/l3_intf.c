...
#define L3_INTF_INGRESS_SUPPORTED_BCM_L3_FLAGS (BCM_L3_INGRESS_WITH_ID | BCM_L3_INGRESS_REPLACE | BCM_L3_INGRESS_L3_MCAST_L2 | BCM_L3_INGRESS_MPLS_INTF_NAMESPACE | BCM_L3_INGRESS_GLOBAL_ROUTE | BCM_L3_INGRESS_IPMC_BRIDGE_FALLBACK | \
                                                BCM_L3_INGRESS_ROUTE_DISABLE_IP4_UCAST | BCM_L3_INGRESS_ROUTE_DISABLE_IP4_MCAST | BCM_L3_INGRESS_ROUTE_DISABLE_IP6_UCAST | \
                                                BCM_L3_INGRESS_ROUTE_DISABLE_IP6_MCAST | BCM_L3_INGRESS_ROUTE_DISABLE_MPLS)
    ...
    bcm_l3_intf_t * intf)
{
    ...
    {
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_STAT_OBJECT_ID, INST_SINGLE, intf->stat_id);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_STAT_OBJECT_CMD, INST_SINGLE,
                                     STAT_PP_PROFILE_ID_GET(intf->stat_pp_profile));
    }
    ...
    bcm_if_t * intf_id)
{
    ...
    {
        if (dnx_data_l3.hw_bug.feature_get(unit, dnx_data_l3_hw_bug_mc_bridge_fallback))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "BCM_L3_INGRESS_IPMC_BRIDGE_FALLBACK flag is not supported in this device.\n");
        }
        else if (!BCM_L3_ITF_TYPE_IS_RIF(*intf_id))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
...
*  Get VSI information.
*   \param [in] unit     - The Unit number.
*   \param [in] l3a_vid         - VSI id to be looked up
*   \param [in] in_lif_profile         -  LIF profile used for 'profile_to_write' calculation
*   \param [in] rif_allocated         -  indicated if RIF was already allocated
*   \param [out] table_result_type         -  VSI result type
*   \param [out] profile_to_write         -  Valid pointer to mac address prefix provided by the algorithm.
* \return
*   shr_error_e
*/
static shr_error_e
dnx_l3_intf_ingress_rif_vsi_info_get(
    int unit,
    bcm_vlan_t l3a_vid,
    int in_lif_profile,
    int rif_allocated,
    ...
    uint32 *profile_to_write)
{
    ...
    SHR_IF_ERR_EXIT(vlan_db.vsi.is_allocated(unit, l3a_vid, &is_vsi_allocated));

    /** if VSI already exists, result type should be preserved and vsi profile updated */
    if (is_vsi_allocated || rif_allocated)
    {
        /** get the current VSI_profile from VSI table */
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_ING_VSI_INFO_DB, &entry_handle_id));
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_VSI, l3a_vid);
        SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_GET_ALL_FIELDS));
    ...
    *profile_to_write = CALCULATE_VSI_PROFILE_FROM_ETH_RIF_PROFILE(VSI_MSB_PREFIX, in_lif_profile);
...
dnx_l3_intf_eth_rif_create_ingress(
    ...
    bcm_l3_intf_t * intf)
{
    uint32 profile_to_write, table_result_type, entry_handle_id, old_vsi_profile;
    ...
    if (is_replace || is_vsi_allocated)
    {
        ...
        if (is_replace)
        {
            old_in_lif_profile = old_vsi_profile;
            SHR_IF_ERR_EXIT(dnx_in_lif_profile_get_data(unit, old_in_lif_profile, &in_lif_profile_info, ETH_RIF));
        }
    }
    else
    {
    ...
    /** get VSI related info, if exists */
    SHR_IF_ERR_EXIT(dnx_l3_intf_ingress_rif_vsi_info_get
                    (unit, intf->l3a_vid, new_in_lif_profile, _SHR_IS_FLAG_SET(intf->l3a_flags, BCM_L3_REPLACE),
                     &table_result_type, &profile_to_write));
    ...
    SHR_IF_ERR_EXIT(dnx_l3_intf_ingress_vsi_hw_set
                    (unit, intf, table_result_type, profile_to_write, propag_profile, old_mymac_prefix_profile,
                     ecn_mapping_profile));
    ...
    bcm_if_t * intf_id)
{
    ...
    if (BCM_L3_ITF_TYPE_IS_RIF(*intf_id) && _SHR_IS_FLAG_SET(ing_intf->flags, BCM_L3_INGRESS_L3_MCAST_L2))
    {
        /** Update the forwarding mode in in_lif_profile_info to IP */
        in_lif_profile_info.ingress_fields.l2_v4_mc_fwd_type = DBAL_ENUM_FVAL_VSI_PROFILE_L2_V4_MC_FWD_TYPE_IPV4MC;
        in_lif_profile_info.ingress_fields.l2_v6_mc_fwd_type = DBAL_ENUM_FVAL_VSI_PROFILE_L2_V6_MC_FWD_TYPE_IPV6MC;
    }
    else if (BCM_L3_ITF_TYPE_IS_RIF(*intf_id) && !_SHR_IS_FLAG_SET(ing_intf->flags, BCM_L3_INGRESS_L3_MCAST_L2))
    {
        /** Update the forwarding mode in in_lif_profile_info to Bridge */
        in_lif_profile_info.ingress_fields.l2_v4_mc_fwd_type = DBAL_ENUM_FVAL_VSI_PROFILE_L2_V4_MC_FWD_TYPE_V4BRIDGE;
        in_lif_profile_info.ingress_fields.l2_v6_mc_fwd_type = DBAL_ENUM_FVAL_VSI_PROFILE_L2_V6_MC_FWD_TYPE_V6BRIDGE;
    }

    if (BCM_L3_ITF_TYPE_IS_RIF(*intf_id) && (ing_intf->hash_layers_disable != 0)
        && dnx_data_switch.load_balancing.hashing_selection_per_layer_get(unit))
    {
        /** Check the flags */
