    ...
    int imb_id)
{
    ...
    if (!is_active)
    {
        ...
        /*
         * Enable two mechanisms for Rx valid in case of local fault
         */
        if (dnx_data_nif.flexe.feature_get(unit, dnx_data_nif_flexe_rx_valid_in_lf))
        {
            ...
            SHR_IF_ERR_EXIT(imb_feu_internal_use_alter_port_for_rx_valid_enable_set(unit, 1));
        }
        /*
    ...
    portmod_port_add_info_t * add_info)
{
    ...
    {
        PORTMOD_PORT_ADD_F_FLEXE_CONFIG_CLR(add_info);
    }
    ...
    uint32 flags)
{
    ...
    if (!is_flexe_clock_src_pm_initialized && (flexe_clock_src_pm != pm_id))
    {
        ...
        SHR_IF_ERR_EXIT(portmod_core_add(unit, flexe_clock_src_pm, &add_info));
    }
...
/**
 * \brief - Configure FlexE Northbound wrapper and
 *    configure speed in portmod
 *
 * \param [in] unit - chip unit id
 * \param [in] port - Logical port ID
 * \param [in] speed_config - speed configuration
 *
 * \return
 *   int - see _SHR_E*
 *
 ...
 */
int
imb_feu_phy_port_speed_config_set(
    int unit,
    bcm_port_t port,
    const portmod_speed_config_t * speed_config)
{
    int flexe_core_port, has_speed, orig_speed = 0;
    dnx_algo_port_ethu_access_info_t ethu_info;
    ...
    if (flexe_core_port == DNX_ALGO_PORT_INVALID)
    {
         ...
         */
        SHR_IF_ERR_EXIT(dnx_algo_flexe_core_port_alloc(unit, speed_config->speed, &flexe_core_port));
        /*
         * Store the flexe core port in DB.
         */
        SHR_IF_ERR_EXIT(dnx_algo_port_flexe_core_port_set(unit, port, flexe_core_port));
    }
    /*
     ...
     * Configure Speed in portmod
     */
    SHR_IF_ERR_EXIT(portmod_port_speed_config_set(unit, port, speed_config));
    /*
     * Configure FlexE Default instatnce configuration if there is speed change
     */
    ...
    if (orig_speed != speed_config->speed)
    {
        SHR_IF_ERR_EXIT(flexe_core_phy_default_config_set(unit, flexe_core_port, speed_config->speed));
    }
exit:
    SHR_FUNC_EXIT;
}
...
/**
 * \brief - Get speed configuration in portmod
 *
 * \param [in] unit - chip unit id
 * \param [in] port - Logical port ID
 * \param [out] speed_config - speed configuration
 *
 * \return
 *   int - see _SHR_E*
 *
 ...
 */
int
imb_feu_phy_port_speed_config_get(
    int unit,
    bcm_port_t port,
