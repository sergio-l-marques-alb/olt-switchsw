    ...
    bcm_gport_t * gport)
{
    ...
    /*
     * Verify local core id - must be CORE_ALL. Asymmetrical mode is not supported.
     */
    if (config->local_core_id != BCM_CORE_ALL)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Core must be set to BCM_CORE _ALL instead of %d\n", config->local_core_id);
    }
 ...
 */
static shr_error_e
bcm_dnx_cosq_sysport_ingress_queue_map_set_verify(
    int unit,
    uint32 flags,
    bcm_gport_t sysport,
    bcm_gport_t ingress_queue)
{
    ...
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Base VOQ 0x%x core %d is out of range. Core can be only BCM_CORE_ALL",
                     ingress_queue, core);
    }

    /** Get the current output VOQ for the system port */
    SHR_IF_ERR_EXIT(dnx_cosq_ipq_dbal_system_port_to_voq_base_get
                    (unit, sysport_id, &curr_base_voq, &curr_voq_is_valid));
     ...
     */
    SHR_IF_ERR_EXIT(dnx_cosq_ipq_queue_is_allocated(unit, new_base_voq, &is_allocated));
    if (!is_allocated)
    {
    ...
    SHR_IF_ERR_EXIT(dnx_ipq_queue_sysport_map_get(unit, new_base_voq, &master_sysport_id));
    ...
    bcm_gport_t ingress_queue)
{
    ...
    SHR_INVOKE_VERIFY_DNX(bcm_dnx_cosq_sysport_ingress_queue_map_set_verify(unit, flags, sysport, ingress_queue));

    new_base_voq = BCM_GPORT_UNICAST_QUEUE_GROUP_QID_GET(ingress_queue);
    sysport_id = BCM_GPORT_SYSTEM_PORT_ID_GET(sysport);

    /** Get the current output VOQ for the systemport */
    SHR_IF_ERR_EXIT(dnx_cosq_ipq_dbal_system_port_to_voq_base_get(unit, sysport_id, &curr_base_voq, &voq_is_valid));
    ...
    if (is_unmap)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_ipq_db.base_queues.sys_port_ref_counter.dec(unit, queue_quartet, 1));
    }
    else
    {
        if (voq_is_valid)
        ...
        SHR_IF_ERR_EXIT(dnx_ipq_db.base_queues.sys_port_ref_counter.inc(unit, new_queue_quartet, 1));
    }

    /** Unmap/Map new SLAVE_SYSPORT to VOQ */
    SHR_IF_ERR_EXIT(dnx_cosq_ipq_dbal_system_port_to_voq_base_set(unit, sysport_id, new_base_voq, voq_is_valid));
 ...
 */
static shr_error_e
bcm_dnx_cosq_sysport_ingress_queue_map_get_verify(
    int unit,
    uint32 flags,
    bcm_gport_t sysport,
    bcm_gport_t * ingress_queue)
    ...
    bcm_gport_t * ingress_queue)
{
    ...
    SHR_INVOKE_VERIFY_DNX(bcm_dnx_cosq_sysport_ingress_queue_map_get_verify(unit, flags, sysport, ingress_queue));

    sysport_id = BCM_GPORT_SYSTEM_PORT_ID_GET(sysport);

    /** Get the current output VOQ for the systemport */
    SHR_IF_ERR_EXIT(dnx_cosq_ipq_dbal_system_port_to_voq_base_get(unit, sysport_id, &base_voq, &voq_is_valid));
    ...
    BCM_GPORT_UNICAST_QUEUE_GROUP_CORE_QUEUE_SET(*ingress_queue, BCM_CORE_ALL, base_voq);
...
static shr_error_e
bcm_dnx_cosq_mapping_default_profile_mapping_set(
    int unit,
    int is_flow,
    bcm_cos_t sys_tc,
    bcm_cos_queue_t voq_offset)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_cosq_ipq_dbal_tc_to_voq_offset_map_set(unit, sys_tc, dp, tc_profile, is_flow, is_fmq,
                                                                   voq_offset));
    }
    /*
    ...
    if (is_flow)
    {
        SHR_IF_ERR_EXIT(dnx_ipq_alloc_mngr_db.
                        tc_voq_flow_map.profile_data_get(unit, tc_profile, &ref_count, &tc_mapping));
        for (dp = 0; dp < nof_dps; dp++)
        {
            tc_mapping.offset[sys_tc][dp] = voq_offset;
        }
        SHR_IF_ERR_EXIT(dnx_ipq_alloc_mngr_db.tc_voq_flow_map.replace_data(unit, tc_profile, &tc_mapping));
    }
    else
    {
        SHR_IF_ERR_EXIT(dnx_ipq_alloc_mngr_db.
    ...
    bcm_cos_queue_t cosq)
{
     ...
     */
    is_flow = TRUE;
    SHR_IF_ERR_EXIT(bcm_dnx_cosq_mapping_default_profile_mapping_set(unit, is_flow, sys_tc, voq_offset));
    is_flow = FALSE;
    SHR_IF_ERR_EXIT(bcm_dnx_cosq_mapping_default_profile_mapping_set(unit, is_flow, sys_tc, voq_offset));
    ...
    bcm_cos_queue_t * cosq)
{
    ...
    SHR_IF_ERR_EXIT(dnx_cosq_ipq_dbal_tc_to_voq_offset_map_get(unit, sys_tc, dp, tc_profile, is_flow, is_fmq,
                                                               &voq_offset));
