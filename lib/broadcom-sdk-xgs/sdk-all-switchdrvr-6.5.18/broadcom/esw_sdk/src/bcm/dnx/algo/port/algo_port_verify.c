    ...
    bcm_pbmp_t phys)
{
    ...
    BCM_PBMP_ITER(phys, phy_id)
    {
        ...
        if (dnx_data_nif.eth.pm_properties_get(unit, pm_index)->dispatch_type == portmodDispatchTypePm4x25)
        {
    ...
    dnx_algo_port_if_add_t * info)
{
    ...
    { /** i.e. master port */
        ...
        if (!dnx_data_dev_init.general.access_only_get(unit) && !soc_is(unit, J2P_DEVICE))
        {
