    ...
    int unit)
{
    ...
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core)
    {
        ...
        if (SHR_FUNC_ERR())
        {
            ...
            {
                for (iter = 0; iter < DNX_KBP_ROP_RETRANSMISSION(unit); iter++)
                {
                    ...
                    {
                        if (iter < DNX_KBP_ROP_RETRANSMISSION(unit) - 1)
                        {
                            LOG_INFO(BSL_LOG_MODULE,
                                     (BSL_META_U(unit, "KBP recovery iteration %d/%d failed, trying again.\n"),
                                      iter + 1, DNX_KBP_ROP_RETRANSMISSION(unit)));
                        }
                        else
                        {
    ...
    dnx_kbp_lut_data_t * lut_data)
{
    ...
    SHR_IF_ERR_EXIT(dnx_kbp_callback_mdio_write
                    (&Kbp_app_data[unit]->user_data, 0, DNX_KBP_LUT_PROG_REG_DEVAD_PORT0,
                     DNX_KBP_LUT_PROG0_REG_ADDR_PORT0, first));
    SHR_IF_ERR_EXIT(dnx_kbp_callback_mdio_write
                    (&Kbp_app_data[unit]->user_data, 0, DNX_KBP_LUT_PROG_REG_DEVAD_PORT0,
                     DNX_KBP_LUT_PROG1_REG_ADDR_PORT0, second));
    SHR_IF_ERR_EXIT(dnx_kbp_callback_mdio_write
                    (&Kbp_app_data[unit]->user_data, 0, DNX_KBP_LUT_PROG_REG_DEVAD_PORT1,
    ...
    SHR_IF_ERR_EXIT(dnx_kbp_callback_mdio_read
                    (&Kbp_app_data[unit]->user_data, 0, 1, DNX_KBP_LUT_PROG0_REG_ADDR_PORT0, &dummy_val));
exit:
    SHR_FUNC_EXIT;
}
/**
    ...
    dnx_kbp_lut_data_t * lut_data)
{
    ...
    SHR_IF_ERR_EXIT(dnx_kbp_callback_mdio_write
                    (&Kbp_app_data[unit]->user_data, 0, 1, DNX_KBP_LUT_RDADDR_REG_ADDR_PORT0, addr));

    SHR_IF_ERR_EXIT(dnx_kbp_callback_mdio_read
                    (&Kbp_app_data[unit]->user_data, 0, 1, DNX_KBP_LUT_RDATA0_REG_ADDR_PORT0, &first));
    SHR_IF_ERR_EXIT(dnx_kbp_callback_mdio_read
                    (&Kbp_app_data[unit]->user_data, 0, 1, DNX_KBP_LUT_RDATA1_REG_ADDR_PORT0, &second));
    ...
    dnx_kbp_rop_packet_t * pkt)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_kbp_callback_mdio_read
                        (&Kbp_app_data[unit]->user_data, 0, DNX_KBP_OP_DEV0_LAST_REQUEST_REG_DEVAD, reg_addr,
                         &reg_data[i]));
        reg_addr++;
        /*
    ...
    dnx_kbp_rop_packet_t * pkt)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_kbp_callback_mdio_read
                        (&Kbp_app_data[unit]->user_data, 0, DNX_KBP_OP_LAST_RESPONSE_REG_DEVAD, reg_addr,
                         &reg_data[i]));
        reg_addr++;
