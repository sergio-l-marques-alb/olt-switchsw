    ...
    phymod_lane_map_t * phy_lane_map)
{
    ...
    int is_over_fabric, ilkn_id, lane_id, last_phy;
    dnx_algo_lane_map_type_e type;
    ...
    int fec_bypass_enable)
{
    ...
    int iter_phy, first_lane_in_cdu, ethu_id, ethu_id_in_core, core, phy_in_core, is_over_fabric, pm_index,
        first_phy_in_pm, tx_serdes_source;
    ...
    soc_dnxc_lane_map_db_map_t lane2serdes[DNX_DATA_MAX_NIF_PHYS_NOF_PHYS];
    bcm_pbmp_t ilkn_phys, tx_phys, pm_phys, tmp_bmp;
    dnx_algo_port_ilkn_access_info_t ilkn_access_info;
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);
    ...
    SHR_IF_ERR_EXIT(dnx_algo_lane_map_lane_to_serdes_map_get(unit,
                                                             ...
                                                             (is_over_fabric) ? dnx_data_fabric.
                                                             links.nof_links_get(unit) : dnx_data_nif.phys.
                                                             nof_phys_get(unit), lane2serdes));
    ...
    _SHR_PBMP_ITER(tx_phys, iter_phy)
    {
        ...
        if (fec_bypass_valid)
        {
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_FEC_DECODER_CONTROL, INST_SINGLE,
                                         (fec_bypass_enable ? DBAL_ENUM_FVAL_FEC_DECODER_CONTROL_MODE_BYPASS :
    ...
    bcm_port_t port)
{
    ...
    int iter_phy, lane, ethu_id_in_core, ethu_id, core, imb_type_id, is_over_fabric, pm_index, facing_core_id,
        number_of_non_facing_lanes, pm_id_in_clu, facing_clu, is_facing = 1, rx_direction = 0;
    dnx_algo_port_ilkn_access_info_t ilkn_access_info;
    soc_dnxc_lane_map_db_map_t lane2serdes[DNX_DATA_MAX_NIF_PHYS_NOF_PHYS];
    imb_dispatch_type_t imb_type;
    bcm_pbmp_t ilkn_phys;
    ...
    int ilkn_nof_ports)
{
    ...
    int iter_phy, lane, ethu_id_in_core, ethu_id, core, is_over_fabric, pm_index, facing_core_id,
        number_of_non_facing_lanes, facing_clu, is_facing = 1, second_ilkn_port = -1, ilkn_port_i;
    ...
    if (disable_tx)
    {
        ...
        _SHR_PBMP_ITER(tx_phys, iter_phy)
        {
            ...
            if (!is_facing)
            {
                facing_clu = dnx_data_nif.ilkn.ilkn_cores_clup_facing_get(unit, ilkn_access_info.ilkn_core)->clu_unit;
