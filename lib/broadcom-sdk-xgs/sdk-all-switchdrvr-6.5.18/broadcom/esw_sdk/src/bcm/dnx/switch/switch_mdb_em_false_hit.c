...
#ifndef DNXC_ROLLBACK_JOURNAL_IS_ENABLED
#error "DNXC_ROLLBACK_JOURNAL_IS_ENABLED must be defined for EM false hit WA"
#endif
...
static shr_error_e
dnx_switch_control_mdb_exact_match_false_hit_wa_mdb(
    int unit)
{
    ...
    for (macro_idx = 0; macro_idx < DNX_DATA_MAX_MDB_DH_TOTAL_NOF_MACROES; macro_idx++)
    {
        int cluster_idx;
        mdb_macro_types_e macro_type;
        int macro_local_idx;
        dbal_enum_value_field_mdb_physical_table_e mdb_physical_table_id =
            dnx_data_mdb.pdbs.pdb_info_get(unit, dh_overwrite_dbal_table_ids[macro_idx])->logical_to_physical;

        macro_type = (macro_idx < dnx_data_mdb.dh.nof_macroes_type_A_get(unit)) ? MDB_MACRO_A : MDB_MACRO_B;
        macro_local_idx =
            (macro_idx <
             dnx_data_mdb.dh.nof_macroes_type_A_get(unit)) ? macro_idx : macro_idx -
            dnx_data_mdb.dh.nof_macroes_type_A_get(unit);

        for (cluster_idx = 0; cluster_idx < DNX_DATA_MAX_MDB_DH_NOF_MACRO_CLUSTERS; cluster_idx++)
        {
            int cluster_glob_index = (macro_local_idx * DNX_DATA_MAX_MDB_DH_NOF_MACRO_CLUSTERS) + cluster_idx;
...
static shr_error_e
dnx_switch_control_mdb_exact_match_false_hit_wa_le_and_pipe(
    int unit)
{
    uint32 entry_handle_id;
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_KBR_IS_ACL, INST_SINGLE, 0);
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    dbal_entry_key_field32_range_set(unit, entry_handle_id, DBAL_FIELD_FWD12_KBR_ID, 6, 6);
    dbal_entry_value_field_arr32_set(unit, entry_handle_id, DBAL_FIELD_FFC_BITMAP, INST_SINGLE, ffc_bitmap_stage_a);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_KBR_IS_ACL, INST_SINGLE, 0);
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    dbal_entry_key_field32_range_set(unit, entry_handle_id, DBAL_FIELD_FWD12_KBR_ID, 11, 11);
    dbal_entry_value_field_arr32_set(unit, entry_handle_id, DBAL_FIELD_FFC_BITMAP, INST_SINGLE, ffc_bitmap_stage_a);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_KBR_IS_ACL, INST_SINGLE, 0);
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
...
static shr_error_e
dnx_mdb_false_hit_wa_build_packet(
    int unit,
    uint32 *in_pp_ports,
    uint32 *packet_0,
    uint32 *packet_1)
...
static shr_error_e
dnx_switch_control_mdb_exact_match_false_hit_wa_send_packets(
    int unit,
    int verify)
{
    uint32 entry_handle_id;
                ...
                for (cluster_iter = 0; cluster_iter < dnx_data_mdb.dh.nof_macro_clusters_get(unit); cluster_iter++)
                {
                    ...
                    if (cnt == 0)
                    {
                        if ((dnx_data_mdb.dh.feature_get(unit, dnx_data_mdb_dh_macro_A_half_nof_clusters_enable))
                            && (dh_iter < dnx_data_mdb.dh.nof_macroes_type_A_get(unit))
                        ...
                        LOG_INFO(BSL_LOG_MODULE,
                                 (BSL_META_U
                                  (unit, "MDB EM WA counter not set, cnt_mode:0x%02x, DH: %2d, cluster: %d\n"),
...
static shr_error_e
dnx_switch_control_mdb_exact_match_false_hit_wa_config_send_packets(
    int unit,
    int verify)
{
    ...
    SHR_IF_ERR_EXIT(dnx_mdb_false_hit_wa_build_packet
    ...
    SHR_IF_ERR_EXIT(dnx_switch_control_mdb_exact_match_false_hit_wa_send_packets(unit, verify));
...
shr_error_e
dnx_switch_control_mdb_exact_match_false_hit_wa_set(
    int unit,
    int verify)
{
    ...
    SHR_ERR_EXIT(_SHR_E_INIT,
                 "Error. MDB EM init requires journaling, compilation flag DNXC_ROLLBACK_JOURNAL_IS_ENABLED must be enabled.\n");
...
#endif
    SHR_IF_ERR_EXIT(dnx_switch_control_mdb_exact_match_false_hit_wa_mdb(unit));

    SHR_IF_ERR_EXIT(dnx_switch_control_mdb_exact_match_false_hit_wa_le_and_pipe(unit));

    SHR_IF_ERR_EXIT(dnx_switch_control_mdb_exact_match_false_hit_wa_config_send_packets(unit, verify));
