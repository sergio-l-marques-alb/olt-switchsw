...
flexe_dbg_init(
    int unit)
{
    int phy_map_offset, phy_index;
    uint32 calendar_slots[32] = { 0 };
    uint32 zero = 0;
    ...
    for (phy_map_offset = 0; phy_map_offset < DNX_FLEXE_MAP_PHY_MAP_OCTETS; ++phy_map_offset)
    {
        for (phy_index = 0; phy_index < DNX_FLEXE_MAX_NOF_PHYS; ++phy_index)
        {
            DNX_FLEXE_IF_ERR_EXIT(oh_tx_phymap_cca_ccb_ins_cfg_set
                                  (unit, phy_index, 0, phy_map_offset, (UINT_32 *) & zero));
            DNX_FLEXE_IF_ERR_EXIT(oh_rx_phymap_rx_exp_set(unit, phy_index, phy_map_offset, (UINT_32 *) & zero));
        }
    }
    for (phy_index = 0; phy_index < DNX_FLEXE_MAX_NOF_PHYS; ++phy_index)
    {
        ...
        /*
         * Clear CCA and CCB ram
         */
        DNX_FLEXE_IF_ERR_EXIT(flexe_oh_tx_ccab_set(unit, 0, phy_index, (UINT_32 *) & calendar_slots[0]));
        DNX_FLEXE_IF_ERR_EXIT(flexe_oh_exp_ccab_set(unit, 0, phy_index, (UINT_32 *) & calendar_slots[0]));
        DNX_FLEXE_IF_ERR_EXIT(flexe_oh_tx_ccab_set(unit, 1, phy_index, (UINT_32 *) & calendar_slots[0]));
        DNX_FLEXE_IF_ERR_EXIT(flexe_oh_exp_ccab_set(unit, 1, phy_index, (UINT_32 *) & calendar_slots[0]));
        /*
         * Configure default PHY ID
         */
        DNX_FLEXE_IF_ERR_EXIT(flexe_oh_tx_pid_set(unit, phy_index, 1));
        DNX_FLEXE_IF_ERR_EXIT(flexe_oh_exp_pid_set(unit, phy_index, 1));
        /*
         * Configure default Group ID
         */
        DNX_FLEXE_IF_ERR_EXIT(flexe_oh_tx_gid_set(unit, phy_index, 1));
        DNX_FLEXE_IF_ERR_EXIT(flexe_oh_exp_gid_set(unit, phy_index, 1));
    }
    ...
    int ts_num)
{
    ...
    if (flags & BCM_PORT_FLEXE_RX)
    {
        DNX_FLEXE_IF_ERR_EXIT(sar_rx_ts_cfg_asic(unit, 1, client_channel, 1, ts_num));
    }

    if (flags & BCM_PORT_FLEXE_TX)
    {
        DNX_FLEXE_IF_ERR_EXIT(sar_tx_ts_cfg_asic(unit, 1, client_channel, 1, ts_num));
    }
exit:
    SHR_FUNC_EXIT;
}
    ...
    int ts_num)
{
    ...
    if (flags & BCM_PORT_FLEXE_RX)
    {
        DNX_FLEXE_IF_ERR_EXIT(sar_rx_ts_cfg_asic(unit, 0, client_channel, 0, ts_num));
    }

    if (flags & BCM_PORT_FLEXE_TX)
    {
        DNX_FLEXE_IF_ERR_EXIT(sar_tx_ts_cfg_asic(unit, 0, client_channel, 0, ts_num));
    }
