 ...
 * \param [in] qual_map_entry_p - Poinmter to qualifier mapping of type dnx_field_qual_map_t.
 * \return
...
{
    uint8 is_vw = FALSE;
    char vw_name[RHSTRING_MAX_SIZE];
    void *dest_char;
    SHR_FUNC_INIT_VARS(unit);
    ...
    switch (qual_dbal_field_id)
    {
        ...
        {
            dest_char = &(vw_name[0]);
            sal_strncpy_s(dest_char, "vwAcLifWide___AC_IN_LIF_WIDE_DATA", sizeof(vw_name));
            is_vw = TRUE;
            break;
        }
        ...
        {
            dest_char = &(vw_name[0]);
            sal_strncpy_s(dest_char, "vwNativeAcLifWide___NATIVE_AC_IN_LIF_WIDE_DATA", sizeof(vw_name));
            is_vw = TRUE;
            break;
        }
        default:
    ...
    if (is_vw == TRUE)
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Stage %d do not support Virtual Wire\n", stage);
        }

        SHR_IF_ERR_EXIT(pemladrv_vw_wire_get(unit, vw_name, sdk_stage, &vwInfo));
        if (vwInfo->nof_mappings > 1)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Virtual Wire \"%.*s\" has more then one mapping, this is not supported. \n",
                         RHSTRING_MAX_SIZE, vw_name);
        }
        if (vwInfo->nof_mappings <= 0)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "No mapping found for virtual wire \"%.*s\". \n", RHSTRING_MAX_SIZE, vw_name);
        }
        (*offset_p) += vwInfo->vw_mappings_arr[0].physical_wire_lsb;
    }
    ...
    const dnx_field_qual_map_t ** qual_map_entry_p)
{
    ...
    else if ((bcm_qual >= 0) && (bcm_qual < bcmFieldQualifyCount))
    {
        ...
        if ((dnx_qual == 0) || (dnx_qual == DNX_FIELD_QUAL_TYPE_INVALID))
        {
            ...
            if (dnx_field_map_stage_info[dnx_stage].meta_qual_map)
            {
                ...
                else
                {
                    ...
                    SHR_IF_ERR_EXIT(dnx_pp_prgm_default_image_check(unit, &is_std_1));
                    if (!is_std_1 && (dnx_field_map_stage_info[dnx_stage].meta_qual_map[bcm_qual].conversion_cb != 0))
                    {
                        SHR_SET_CURRENT_ERR(_SHR_E_NOT_FOUND);
                        SHR_EXIT();
                    }
        ...
        SHR_IF_ERR_EXIT(dnx_pp_prgm_default_image_check(unit, &is_std_1));
        if (!is_std_1 && (dnx_global_qual_map[bcm_qual].conversion_cb != 0))
        {
            SHR_SET_CURRENT_ERR(_SHR_E_NOT_FOUND);
            SHR_EXIT();
        }
    ...
    dnx_field_qual_t * dnx_qual_p)
{
    ...
    rv = dnx_field_map_qual_bcm_to_dnx_int(unit, dnx_stage, bcm_qual, dnx_qual_p, &qual_map_entry);
    if (rv == _SHR_E_NOT_FOUND)
    {
        uint8 is_std_1;
        SHR_IF_ERR_EXIT(dnx_pp_prgm_default_image_check(unit, &is_std_1));
        if (!is_std_1 && DNX_FIELD_MAP_QUAL_MAPPING_HAS_VALUE_CONVERSION(qual_map_entry))
        {
            SHR_IF_ERR_EXIT_WITH_LOG(rv,
                                     "BCM Qualifier %s(%d) is not supported in BareMetal mode! Only RAW qualifiers are supported!%s",
    ...
    int *has_conversion_p)
{
    ...
    SHR_IF_ERR_EXIT_WITH_LOG(dnx_field_map_qual_bcm_to_dnx_int(unit, dnx_stage, bcm_qual, &dnx_qual, &qual_map_entry),
                             "BCM Qualifier %s(%d) is not supported for stage:%s",
    ...
    int unit)
{
    ...
    /*
     * Verify that all qualifiers that have description are mapped.
     */
    for (i_bcm_qual = 0; i_bcm_qual < bcmFieldQualifyCount; i_bcm_qual++)
    {
        dnx_field_stage_e dnx_stage;
        ...
        int is_mapped;
        int rv;
        if ((ISEMPTY(bcm_qual_description[i_bcm_qual])) == FALSE)
        {
            is_mapped = FALSE;
            DNX_FIELD_STAGE_QUAL_ITERATOR(dnx_stage)
            {
                rv = dnx_field_map_qual_bcm_to_dnx_int(unit, dnx_stage, i_bcm_qual, &dnx_qual, &qual_map_entry_p);
            ...
            {
                /*
                 * Look for the qualifier in the CS qualifiers.
                 * Note: We may assume dnx_field_map_cs_qual_init() is run before dnx_field_map_qual_init,
                 *       although under current implementation we do not.
                 */
                DNX_FIELD_STAGE_CS_QUAL_ITERATOR(dnx_stage)
                {
                    SHR_IF_ERR_EXIT(dnx_field_map_cs_qual_bcm_mapping_exists(unit, dnx_stage, i_bcm_qual, &is_mapped));
                    if (is_mapped)
                    {
                        break;
                    }
            ...
            if (is_mapped == FALSE)
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "BCM Qualifier:\"%s\" (%d) has a description but isn't mapped.\n",
                             dnx_field_bcm_qual_text(unit, i_bcm_qual), i_bcm_qual);
            }
        }
    }
