    ...
    uint32 pll_post_reset)
{
    ...
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_DRAM_INDEX, dram_index);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_DRAM_PLL_POST_RESETB, INST_SINGLE, !!pll_post_reset);
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    ...
    int enable)
{
    ...
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_DRAM_INDEX, dram_index);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ENABLE_DYNAMIC_MEMORY_ACCESS, INST_SINGLE, !!enable);
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
...
/* see header file */
shr_error_e
dnx_gddr6_dbal_access_training_cadt_config_set(
    int unit,
    uint32 dram_index,
    dnx_gddr6_training_cadt_bist_configuration * cadt_bist_conf)
{
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_TRAINING_CADT_NUM_OF_COMMANDS, INST_SINGLE,
                                 cadt_bist_conf->commands);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_CADT_MODE, INST_SINGLE, cadt_bist_conf->cadt_mode);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_CADT_TO_CADT_PRD, INST_SINGLE,
                                 cadt_bist_conf->cadt_to_cadt_prd);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_CADT_TO_RDEN_PRD, INST_SINGLE,
                                 cadt_bist_conf->cadt_to_rden_prd);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_CADT_PRBS_MODE, INST_SINGLE,
                                 cadt_bist_conf->cadt_prbs_mode);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_CADT_SEED, INST_SINGLE, cadt_bist_conf->cadt_seed);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_CADT_INVERT_CA_UI_2, INST_SINGLE,
                                 cadt_bist_conf->cadt_invert_caui2);
    ...
    for (pattern_index = 0; pattern_index < SHMOO_G6PHY16_BIST_NOF_PATTERNS; ++pattern_index)
    {
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_CADT_PATTERN_N, pattern_index,
                                     cadt_bist_conf->cadt_pattern[pattern_index]);
    }
    ...
    int set_init_position)
{
    ...
    for (int byte = 0; byte < dnx_data_dram.gddr6.bytes_per_channel_get(unit); ++byte)
    {
        ...
        if (set_init_position)
        {
            ...
            normalized_starting_vdl_position = (step_size.size1000UI / 1000) * ui_shift + vdl_position;
