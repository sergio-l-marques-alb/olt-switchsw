    ...
    uint32 location_end)
{
    ...
    while (location_to_copy_from != location_start)
    {
         ...
         */
        SHR_IF_ERR_EXIT(dnx_field_tcam_location_sw.location_priority_arr.get(unit, core, location_to_copy_from, &prio));
        SHR_IF_ERR_EXIT(dnx_field_tcam_location_sw.
                        location_priority_arr.set(unit, core, location_to_copy_from,
                                                  DNX_FIELD_TCAM_ENTRY_PRIORITY_INVALID));
        SHR_IF_ERR_EXIT(dnx_field_tcam_location_sw.location_priority_arr.set(unit, core, location_to_copy_to, prio));
