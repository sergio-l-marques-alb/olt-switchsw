...
dnx_algo_flexe_channel_verify(
    ...
    int client_channel)
{
    int ptp_channel, oam_channel, is_valid;

    SHR_FUNC_INIT_VARS(unit);

    ptp_channel = dnx_data_nif.flexe.ptp_client_channel_get(unit);
    oam_channel = dnx_data_nif.flexe.oam_client_channel_get(unit);

    if (DNX_ALGO_PORT_TYPE_IS_FLEXE_MAC(unit, port_info, TRUE)
        && ((client_channel == ptp_channel) || (client_channel == oam_channel)))
    {
        /*
...
{
    int client_channel, oam_channel, ptp_channel, max_speed;
    ...
    if ((client_channel == ptp_channel) || (client_channel == oam_channel))
    {
        max_speed =
            (client_channel ==
             ptp_channel) ? dnx_data_nif.flexe.ptp_channel_max_bandwidth_get(unit) : dnx_data_nif.
            flexe.oam_channel_max_bandwidth_get(unit);
