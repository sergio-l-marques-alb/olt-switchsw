    ...
    uint8 *is_cdu)
{
    ...
    {
        /** ETHU type is CDU */
        ethu_id = block_instance + dnx_data_nif.eth.cdum_nof_get(unit);
    }
    else if ((*is_cdu == FALSE) && (dnx_data_nif.eth.cdum_nof_get(unit) != 0))
    {
        /** ETHU type is CDUM */
    ...
    /** From the ETHU ID deterine the core  and the ETHU ID in core*/
    ...
    /** Get the first Phy of the ETHU and the PFC Deadock debug info.
     * The debug info will show which port internal for the ETHU is in deadlock.
     * Using the first Phy as offset, find the number of the Phy that si in deadlock.
     */
    ...
    /** Set the userdata for the periodic event callback, then start and stop periodic event.
    ...
    int arg)
{
    ...
    switch (type)
    {
        ...
        case bcmCosqPFCDeadlockDetectionTimer:
            ...
            if (COMPILER_64_GT(time.time, config.period))
            {
              ...
              * Find the smallest power of two which is larger than or equal to the input interval_in_clocks.
              */
            interval_in_clocks = (1 << utilex_log2_round_up(interval_in_clocks));
        ...
        case bcmCosqPFCDeadlockRecoveryTimer:
            ...
            time.time_units = DNXCMN_TIME_UNIT_MILISEC;
            SHR_IF_ERR_EXIT(dnxcmn_time_units_convert(unit, &time, DNXCMN_TIME_UNIT_USEC, &time_usec));
            SHR_IF_ERR_EXIT(flow_control_db.pfc_deadlock_recovery.
                            timer.set(unit, phy, pfc_priority, COMPILER_64_LO(time_usec.time)));
            break;
