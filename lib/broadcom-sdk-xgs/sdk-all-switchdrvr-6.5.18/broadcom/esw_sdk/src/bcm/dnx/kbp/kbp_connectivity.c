...
#define DNX_KBP_VALID_KBP_PORTS_NUM    (DNX_KBP_OP_IS_DUAL_MODE(unit) ? 4 : 2)
    ...
    dnx_kbp_user_data_t * user_data)
{
    ...
    user_data->device = unit;
    user_data->kbp_mdio_id = dnx_data_elk.connectivity.mdio_id_get(unit);
    ...
    struct kbp_device_config *kbp_config)
{
    ...
    kbp_config->handle = &Kbp_app_data[unit]->user_data;
    ...
    if (dnx_kbp_interface_enabled(unit))
    {
        ...
        BCM_PBMP_ITER(port_pbmp, ilkn_port)
        {
            ...
            kbp_config->reverse_lanes |=
                (dnx_data_elk.connectivity.ilkn_reverse_get(unit, ilkn_id)->reverse << kbp_port_id);
        }

        SHR_IF_ERR_EXIT(dnx_kbp_ilkn_interface_param_get
                        (unit, core, &ilkn_port, &ilkn_num_lanes, &ilkn_rate, &ilkn_metaframe));
        ...
        for (i = 0; i < DNX_KBP_VALID_KBP_PORTS_NUM; i++)
        {
            kbp_config->port_map[i].start_lane = dnx_data_elk.connectivity.topology_get(unit)->start_lane[i];
        ...
        if (Kbp_app_data[unit]->device_type == KBP_DEVICE_OP2)
        {
            ...
            else
            {
                ...
                kbp_config->settings.tap_settings.enable_taps =
                    dnx_data_elk.connectivity.kbp_serdes_tx_taps_get(unit)->txfir_tap_enable;
                kbp_config->settings.tap_settings.pre2 = dnx_data_elk.connectivity.kbp_serdes_tx_taps_get(unit)->pre2;
                kbp_config->settings.tap_settings.pre1 = dnx_data_elk.connectivity.kbp_serdes_tx_taps_get(unit)->pre;
                kbp_config->settings.tap_settings.mainx = dnx_data_elk.connectivity.kbp_serdes_tx_taps_get(unit)->main;
                kbp_config->settings.tap_settings.post1 = dnx_data_elk.connectivity.kbp_serdes_tx_taps_get(unit)->post;
                kbp_config->settings.tap_settings.post2 = dnx_data_elk.connectivity.kbp_serdes_tx_taps_get(unit)->post2;
                kbp_config->settings.tap_settings.post3 = dnx_data_elk.connectivity.kbp_serdes_tx_taps_get(unit)->post3;
            }
    ...
    bcm_core_t core)
{
    ...
    lookup_time_delay = SOC_IS_Q2A(unit) ? 0x7D0 : 0xA28;
    ...
    bcm_core_t core)
{
     ...
     */
    trans_timer_delay = SOC_IS_Q2A(unit) ? 0x10 : 0x13;
    ...
    bcm_core_t core)
{
    ...
    spr_dly_mega = core_clk_ticks /
        ((ilkn_total_burst_rate * (meta_frame_len - 4) * 64 * EGW_TO_ILKN_DATA_WIDTH) /
         (meta_frame_len * 67 * (EGW_TO_ILKN_DATA_WIDTH + 8)));
    ...
    int unit)
{
    ...
    {
        sal_msleep(1);
        DNX_KBP_TRY(kbp_pcie_reset(KBP_DEVICE_OP2, Kbp_app_data[unit]->xpt_p, KBP_PCIE_PM0_SW_SRST));
        DNX_KBP_TRY(kbp_pcie_reset(KBP_DEVICE_OP2, Kbp_app_data[unit]->xpt_p, KBP_PCIE_PM1_SW_SRST));
        ...
        if (link_training == TRUE)
        {
            DNX_KBP_TRY(kbp_device_pm_interface_init
                        (unit, Kbp_app_data[unit]->xpt_p, (final_speed / 1000), port_bmp, is_nrz, link_training, NULL));
        }
        else
        {
                ...
                DNXC_PORT_TX_FIR_INVALID_MAIN_TAP)
            {
                ...
                DNX_KBP_TRY(kbp_device_pm_interface_init
                            (unit, Kbp_app_data[unit]->xpt_p, (final_speed / 1000), port_bmp, is_nrz, link_training,
                             &txfir_vals));
            }
            else
            {
            ...
            for (int lane_idx = 0; lane_idx < 32; lane_idx++)
            {
                    ...
                    DNXC_PORT_TX_FIR_INVALID_MAIN_TAP)
                {
                    ...
                    txfir_vals.pre2 = dnx_data_port.static_add.ext_stat_lane_serdes_tx_taps_get(unit, lane_idx)->pre2;
                    txfir_vals.pre1 = dnx_data_port.static_add.ext_stat_lane_serdes_tx_taps_get(unit, lane_idx)->pre;
                    txfir_vals.mainx = dnx_data_port.static_add.ext_stat_lane_serdes_tx_taps_get(unit, lane_idx)->main;
                    txfir_vals.post1 = dnx_data_port.static_add.ext_stat_lane_serdes_tx_taps_get(unit, lane_idx)->post;
                    txfir_vals.post2 = dnx_data_port.static_add.ext_stat_lane_serdes_tx_taps_get(unit, lane_idx)->post2;
                    txfir_vals.post3 = dnx_data_port.static_add.ext_stat_lane_serdes_tx_taps_get(unit, lane_idx)->post3;
                    DNX_KBP_TRY(kbp_device_pm_interface_serdes_tap_settings
                                (Kbp_app_data[unit]->xpt_p, (1 << lane_idx), &txfir_vals));
                }
            }
        }
    }
    ...
    int unit)
{
    ...
    struct kbp_serdes_tap_settings txfir_vals;
    struct kbp_device_config *kbp_config = &Kbp_app_data[unit]->device_config;
    ...
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_kbp_init_kbp_interface(unit, core));
    }
    ...
    {
        /** Configure TXFIR per lane, if exist. API gets uint32 bitmap lanes. Each call we will configure different lane */
        for (int lane_idx = 0; lane_idx < max_lane_id; lane_idx++)
        {
            /** INVALID_MAIN_TAP means that there is no Soc property configuration for this lane */
                ...
                DNXC_PORT_TX_FIR_INVALID_MAIN_TAP)
            {
                ...
                DNX_KBP_TRY(kbp_device_interface_serdes_tap_settings
                            (KBP_DEVICE_OP2, kbp_config, (1 << lane_idx), &txfir_vals));
            }
        }
    }
    ...
    int unit)
{
    ...
    if (!SAL_BOOT_PLISIM)
    {
        DNX_KBP_TRY(kbp_device_interface_reset(Kbp_app_data[unit]->device_type, &Kbp_app_data[unit]->device_config));
    }
exit:
    SHR_FUNC_EXIT;
}
