    ...
    uint8 to_delete)
{
    ...
    else
    {
        ...
        switch (fec_entry_info->fec_entry_type)
        {
            ...
            {
                uint32 eei_mpls_pop = 0;
                /** add SWAP prefix to mpls label value provided in eei */
                SHR_IF_ERR_EXIT(dbal_fields_parent_field32_value_set
                                (unit, DBAL_FIELD_EEI, DBAL_FIELD_EEI_MPLS_POP_COMMAND, &fec_entry_info->eei,
                                 &eei_mpls_pop));
                SHR_IF_ERR_EXIT(dbal_fields_struct_field_encode
                                (unit, fec_entry_info->fec_entry_type, DBAL_FIELD_EEI, &eei_mpls_pop, fec_entry));
                break;
            }
            case DBAL_FIELD_FEC_DESTINATION_MPLS_SWAP:
            {
                uint32 eei_mpls_label = 0;
                /** add SWAP prefix to mpls label value provided in eei */
                SHR_IF_ERR_EXIT(dbal_fields_parent_field32_value_set
                                (unit, DBAL_FIELD_EEI, DBAL_FIELD_EEI_MPLS_SWAP_COMMAND, &fec_entry_info->eei,
                                 &eei_mpls_label));
                SHR_IF_ERR_EXIT(dbal_fields_struct_field_encode
                                (unit, fec_entry_info->fec_entry_type, DBAL_FIELD_EEI, &eei_mpls_label, fec_entry));
                break;
            }
    ...
    l3_fec_entry_info_t * fec_entry_info)
{
    ...
    switch (fec_entry_type)
    {
        ...
        case DBAL_FIELD_FEC_DESTINATION_MPLS_POP:
            ...
            /** remove SWAP prefix to mpls label value provided from eei */
            dbal_fields_struct_field_decode(unit, DBAL_FIELD_EEI, DBAL_FIELD_EEI_MPLS_POP_COMMAND,
                                            &fec_entry_info->eei, &fec_entry_info->eei);
            break;
        case DBAL_FIELD_FEC_DESTINATION_MPLS_SWAP:
            dbal_fields_struct_field_decode(unit, fec_entry_type, DBAL_FIELD_EEI, &fec_entry_info->eei,
                                            decoded_fec_entry_data);
            /** remove SWAP prefix to mpls label value provided from eei */
            dbal_fields_struct_field_decode(unit, DBAL_FIELD_EEI_MPLS_SWAP_COMMAND, DBAL_FIELD_MPLS_LABEL,
                                            &fec_entry_info->eei, &fec_entry_info->eei);
            break;
            /** DBAL_FIELD_FEC_JR1_DESTINATION_EEI is relevant only in Jericho mode */
            /*
        ...
        if (egr->mpls_action == BCM_MPLS_EGRESS_ACTION_PUSH)
        {
            ...
            if (_SHR_IS_FLAG_SET(flags, BCM_L3_REPLACE) && old_use_push)
            {
                ...
                SHR_IF_ERR_EXIT(dnx_mpls_tunnel_create_push_entry_from_eei
                                (unit, &label_entry, FALSE, FALSE, TRUE, &push_cmd));
            }
            else
            {
                SHR_IF_ERR_EXIT(dnx_mpls_tunnel_create_push_entry_from_eei
                                (unit, &label_entry, FALSE, FALSE, FALSE, &push_cmd));
            }
