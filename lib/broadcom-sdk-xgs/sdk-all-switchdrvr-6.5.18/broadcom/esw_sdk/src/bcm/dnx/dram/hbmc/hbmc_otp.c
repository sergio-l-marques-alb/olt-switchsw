...
/**
 * \brief - config cmic sbus ring related to HBM OTP
 *
 ...
 */
static shr_error_e
dnx_hbmc_otp_cmic_sbus_set(
    int unit,
    int enable)
{
    uint32 reg_val;
    SHR_FUNC_INIT_VARS(unit);
    SHR_IF_ERR_EXIT(READ_CMIC_TOP_SBUS_RING_MAP_56_63r(unit, &reg_val));
    soc_reg_field_set(unit, CMIC_TOP_SBUS_RING_MAP_56_63r, &reg_val, RING_NUM_SBUS_ID_56f,
                      ((enable == TRUE) ? 0x6 : 0x0));
    soc_reg_field_set(unit, CMIC_TOP_SBUS_RING_MAP_56_63r, &reg_val, RING_NUM_SBUS_ID_57f,
                      ((enable == TRUE) ? 0x6 : 0x0));
    SHR_IF_ERR_EXIT(WRITE_CMIC_TOP_SBUS_RING_MAP_56_63r(unit, reg_val));
    ...
    uint32 *concatenate_data)
{
    ...
    SHR_FUNC_INIT_VARS(unit);
     ...
     /** enable CMIC SBUS */
    SHR_IF_ERR_EXIT(dnx_hbmc_otp_cmic_sbus_set(unit, TRUE));
    /** enable crup access */
...
exit:
     ...
     * disable CMIC SBUS 
     */
    SHR_IF_ERR_EXIT(dnx_hbmc_otp_cmic_sbus_set(unit, FALSE));
    SHR_FUNC_EXIT;
}
    ...
    for (int channel_ndx = 0; channel_ndx < dnx_data_dram.general_info.nof_channels_get(unit); channel_ndx++)
    {
        for (int metadata_ndx = 0; metadata_ndx < HBMC_SHMOO_CHANNEL_METADATA_SIZE; metadata_ndx++)
        {
            ...
            if ((channel_metadata[metadata_ndx].level & otp_restore_version) != 0)
            {
                ...
                bit_idx += field_size;
                /** MAX_VDL_DQS_N is not stored in otp. WA - copy from MAX_VDL_DQS_P to MAX_VDL_DQS_N */
                /** metadata table order is MAX_VDL_DQS_P before MAX_VDL_DQS_N */
            ...
            else
            {
                ...
                {
                    /** MAX_VDL_DQS_N is not stored in otp. WA for version 1 - copy from MAX_VDL_DQS_P to MAX_VDL_DQS_N */
                    /** metadata table order is MAX_VDL_DQS_P before MAX_VDL_DQS_N */
