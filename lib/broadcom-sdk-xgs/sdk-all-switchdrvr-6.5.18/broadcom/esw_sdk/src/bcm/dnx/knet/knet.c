    ...
    int *incoming_header_type)
{
    ...
    switch (prt_profile)
    {
        ...
        case DBAL_ENUM_FVAL_PORT_TERMINATION_PTC_PROFILE_PTCH2:
        {
            ...
            BCM_PBMP_CLEAR(pbmp);
            BCM_PBMP_OR(pbmp, port_config.e);
...
/**
 * \brief - Get source system port per gport PTC
 *
 ...
 */
int
dnx_knet_ptc_to_src_system_port_get(
    int unit,
    bcm_core_t core_id,
    uint32 tm_port,
    ...
    bcm_knet_netif_t * netif)
{
    ...
    switch (netif->type)
    {
        ...
        case BCM_KNET_NETIF_T_TX_META_DATA:
            SHR_ERR_EXIT(BCM_E_PARAM, "KNET: Unsupported netif type BCM_KNET_NETIF_T_TX_META_DATA\n");
        default:
    ...
    {
        SHR_ERR_EXIT(BCM_E_PARAM, "KNET: Unsupported RCPU encapsulation\n");
    }
exit:
    SHR_FUNC_EXIT;
}
    ...
    bcm_knet_netif_t * netif)
{
    ...
    {
        SHR_IF_ERR_EXIT(bcm_port_get(unit, injected_port, &flags, &interface_info, &mapping_info));
        ptch_ssp = mapping_info.pp_port;
        tm_port = mapping_info.tm_port;
    }
    ...
    SHR_IF_ERR_EXIT(dnx_knet_switch_header_type_in_get(unit, injected_port, &header_type_incoming));
    ...
    {
        /** Module Header */
        SHR_IF_ERR_EXIT(dnx_algo_port_channel_get(unit, tm_port, &cpu_channel));
    ...
    bcm_knet_filter_t * filter)
{
            ...
            && (filter->match_flags & BCM_KNET_FILTER_M_SRC_MODPORT)))
    {
        ...
        else
        {
            ...
            {
                dnx_knet_ptc_to_src_system_port_get(spa_unit, mapping_info.core, mapping_info.tm_port, &spa);
                meta_data->spa = spa;
                meta_mask->spa = spa_mask;
            }
            else
    ...
    /**  Information to parse dune headers */
    ...
    bcm_knet_filter_t * filter)
{
    ...
    if (filter_get.filter.oob_data_size)
    {
        ...
        if (meta_mask->spa)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_trunk_system_port_is_spa(spa_unit, meta_data->spa, &is_spa));
