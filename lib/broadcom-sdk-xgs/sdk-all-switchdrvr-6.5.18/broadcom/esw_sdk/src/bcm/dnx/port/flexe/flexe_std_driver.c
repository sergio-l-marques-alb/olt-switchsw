    ...
    const flexe_core_port_info_t * port_info)
{
    ...
    {
        /*
         * bypass will be handled by creating bypass client
         */
        _SHR_PBMP_ITER(flexe_core_port_bmp, flexe_core_port)
        {
    ...
    int nof_slots)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_flexe_core_db.mac_client.timeslot_bitmap.bit_range_write
                    (unit, client_channel, 0, 0, DNX_DATA_MAX_NIF_FLEXE_NOF_MAC_TIMESLOTS, tsmap));
        ...
        if (loop == 0)
        {
            ...
            if (enable && (logical_phy_id_2_clr != logical_phy_id))
            {
                ...
                if (found)
                {
                    ...
                    SHR_IF_ERR_EXIT(dnx_algo_flexe_core_db.group.flexe_ports.previous_node(unit, group, node, &prev));
                    SHR_IF_ERR_EXIT(dnx_algo_flexe_core_db.group.flexe_ports.remove_node(unit, group, node));

                    if (prev == SW_STATE_LL_INVALID)
                    {
                        SHR_IF_ERR_EXIT(dnx_algo_flexe_core_db.group.
                                        flexe_ports.add_first(unit, group, (uint32 *) &logical_phy_id,
                                                              (uint32 *) &port));
                    }
                    else
                    {
                        SHR_IF_ERR_EXIT(dnx_algo_flexe_core_db.group.
                                        flexe_ports.add_after(unit, group, prev, (uint32 *) &logical_phy_id,
                                                              (uint32 *) &port));
                    }
                }
                else
                {
                    /*
                     * If not found then just add a new node to sort linked list.
                     */
                    SHR_IF_ERR_EXIT(dnx_algo_flexe_core_db.group.
    ...
    int cal_id)
{
    ...
    while (DNX_SW_STATE_LL_IS_NODE_VALID(node))
    {
        SHR_IF_ERR_EXIT(dnx_algo_flexe_core_db.group.flexe_ports.node_value(unit, group, node, (uint32 *) &port));
        SHR_IF_ERR_EXIT(dnx_algo_flexe_core_db.port.instance.get(unit, port, &instance));
        SHR_IF_ERR_EXIT(dnx_algo_flexe_core_db.port.nof_instances.get(unit, port, &nof_instances));

        for (ii = 0; ii < nof_instances; ++ii)
        {
            SHR_BITSET(instance_bitmap, instance + ii);
        }

        SHR_IF_ERR_EXIT(dnx_algo_flexe_core_db.group.flexe_ports.next_node(unit, group, node, &node));
    }

    SHR_IF_ERR_EXIT(soc_flexe_std_active_calendar_set(unit, instance_bitmap, cal_id));
    ...
    int *cal_id)
{
    ...
    SHR_IF_ERR_EXIT(soc_flexe_std_active_calendar_get(unit, instance, flags, cal_id));
