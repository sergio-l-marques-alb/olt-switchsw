 ...
 */
/*
 * Not all result types has an ESEM command support, this macro return true in case the result type has an ESEM_COMMAND
 * field or false otherwise
 */
#define MPLS_TUNNEL_ENCAP_DBAL_RESULT_TYPE_SUPPORTS_ESEM_CMD(result_type) ((result_type == DBAL_RESULT_TYPE_EEDB_MPLS_TUNNEL_ETPS_MPLS_1) ||\
                                                                           (result_type == DBAL_RESULT_TYPE_EEDB_MPLS_TUNNEL_ETPS_MPLS_1) ||\
                                                                           (result_type == DBAL_RESULT_TYPE_EEDB_MPLS_TUNNEL_ETPS_MPLS_1))
/*
 * }
 */
    ...
    bcm_mpls_egress_label_t * label_array)
{
    ...
    /** If this is a new allocation the old ESEM CMD is empty */
    ...
    if (_SHR_IS_FLAG_SET(label_array[0].flags, BCM_MPLS_EGRESS_LABEL_REPLACE))
    {
        ...
        if (MPLS_TUNNEL_ENCAP_DBAL_RESULT_TYPE_SUPPORTS_ESEM_CMD(gport_hw_resources.outlif_dbal_result_type))
        {
            /** Get the previous ESEM command in case of "replace" or use the default one otherwise */
            SHR_IF_ERR_EXIT(dnx_port_match_esem_cmd_port_get(unit, gport, &old_esem_cmd_profile));
        }
    }
    ...
    if (_SHR_IS_FLAG_SET(label_array[0].flags2, BCM_MPLS_EGRESS_LABEL_FLAGS2_SVTAG_ENABLE))
    {
        ...
        {
            BCM_GPORT_SUB_TYPE_VIRTUAL_EGRESS_POINTED_SET(gport, egr_pointed_id);
        }
        else
        {
            BCM_L3_ITF_SET(gport, BCM_L3_ITF_TYPE_LIF, out_global_lif);
        }
        ...
        SHR_IF_ERR_EXIT(dnx_port_esem_cmd_exchange(unit, gport,
                                                   DNX_PORT_ESEM_CMD_ALLOCATE | DNX_PORT_ESEM_CMD_RANGE_0_3, 0,
                                                   &esem_accesses,
    ...
    dnx_jr_mode_mpls_push_profile_t * push_profile_info)
{
    ...
    push_profile_info->esem_command_profile = (is_pwe) ? dnx_data_esem.access_cmd.default_native_get(unit) : 0;
    ...
    int *push_cmd)
{
    ...
    SHR_IF_ERR_EXIT(dnx_mpls_label_info_to_push_profile(unit, label_entry, is_pwe, has_cw, &push_profile_info));
