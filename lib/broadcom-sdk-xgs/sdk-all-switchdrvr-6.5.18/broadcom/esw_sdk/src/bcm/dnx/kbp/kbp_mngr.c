    ...
    int *algo_mode)
{
    ...
    dbal_entry_key_field8_set(unit, entry_handle_id, DBAL_FIELD_DB_ID, kbp_db_id);

    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_GET_ALL_FIELDS));

    dbal_entry_handle_value_field32_get(unit, entry_handle_id, DBAL_FIELD_ALGO_MODE, INST_SINGLE, &algo_mode_uint32);
    ...
    dbal_entry_handle_value_field32_get(unit, entry_handle_id, DBAL_FIELD_TABLE_SIZE, INST_SINGLE, initial_capacity);

    dbal_entry_handle_value_field8_get(unit, entry_handle_id, DBAL_FIELD_ENTRY_COUNTERS_EN, INST_SINGLE,
                                       counters_enable);
    ...
    uint8 *db_id)
{
    ...
    SHR_SET_CURRENT_ERR(kbp_mngr_db_id_by_name_get_internal(unit, name, db_id));
    ...
    uint32 *nof_ffcs)
{
    ...
    if (opcode_id == DBAL_ENUM_FVAL_KBP_FWD_OPCODE_NOP ||
        opcode_id == DBAL_ENUM_FVAL_KBP_FWD_OPCODE_MPLS_PER_PLATFORM ||
        opcode_id == DBAL_ENUM_FVAL_KBP_FWD_OPCODE_BRIDGE_SVL)
    {
        (*nof_ffcs) = 12;
    }
    else
...
/** 
* \brief
* This function disables the KBP lookups in case the soc properties of the KBP are disabled. 
    ...
    uint32 context_index)
{
    ...
    dbal_entry_key_field32_set(unit, aligner_entry_handle_id, DBAL_FIELD_KBP_KBR_IDX, DBAL_ENUM_FVAL_KBP_KBR_IDX_KEY_2);
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, aligner_entry_handle_id, DBAL_COMMIT));
    ...
    int device_type)
{
    ...
    prefixed_file_name_len = dnx_drv_soc_property_name_max() + 256;
