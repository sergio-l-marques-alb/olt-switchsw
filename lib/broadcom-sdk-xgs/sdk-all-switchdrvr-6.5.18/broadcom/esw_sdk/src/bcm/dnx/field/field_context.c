    ...
    dnx_field_context_state_table_info_t * state_table_info_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_key_id_t_init(unit, &state_table_info_p->key_id));
exit:
    SHR_FUNC_EXIT;
}
...
/**
* \brief
*  Set context compare info of a Field context
* \param [in] unit       - Device ID
* \param [in] context_id     - Field Context ID
* \param [in] context_compare_info_p  - Field Context compare info_field_context_types.h)
* \return
 ...
 */
static shr_error_e
dnx_field_context_compare_info_set(
    int unit,
    ...
    dnx_field_context_compare_info_t * context_compare_info_p)
{
    ...
    SHR_IF_ERR_EXIT(DNX_FIELD_CONTEXT_IPMF1_INFO.compare_info.set(unit, context_id, context_compare_info_p));
    ...
    dnx_field_context_ipmf1_sw_info_t * context_ipmf1_info_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_context_state_table_info_t_init(unit, &(context_ipmf1_info_p->state_table_info)));
    ...
    dnx_field_context_mode_t * context_mode_p)
{
    ...
    context_mode_p->context_ipmf1_mode.state_table_mode = DNX_FIELD_CONTEXT_STATE_TABLE_MODE_DISABLED;
    ...
    dnx_field_context_t * field_context_id_p)
{
    ...
    if (stage == DNX_FIELD_STAGE_IPMF1)
    {
         ...
         */
        if ((context_mode_p->context_ipmf1_mode.state_table_mode == DNX_FIELD_CONTEXT_STATE_TABLE_MODE_ENABLED) &&
            (context_mode_p->context_ipmf1_mode.hash_mode == DNX_FIELD_CONTEXT_HASH_MODE_ENABLED))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Context can't have hash mode enabled along with state table enabled.\r\n");
        }
    }
    else if (stage == DNX_FIELD_STAGE_IPMF2)
    {
         ...
         */
        SHR_IF_ERR_EXIT(dnx_algo_field_context_id_is_allocated(unit, DNX_FIELD_STAGE_IPMF1, cascaded_from, &is_alloc));
        if (!is_alloc)
        {
    ...
    dnx_field_context_t context_id)
{
    ...
    if (stage == DNX_FIELD_STAGE_IPMF1)
    {
        ...
        SHR_IF_ERR_EXIT(DNX_FIELD_CONTEXT_IPMF1_INFO.state_table_info.get(unit, context_id, &state_table_info));
        if (state_table_info.mode == DNX_FIELD_CONTEXT_STATE_TABLE_MODE_ENABLED)
        {
            SHR_IF_ERR_EXIT(dnx_field_key_state_table_resources_reserve(unit, context_id, &state_table_info.key_id));
        }
    }
    ...
    dnx_field_context_t context_id)
{
    ...
    if (stage == DNX_FIELD_STAGE_IPMF1)
    {
        ...
        if (state_table_info.mode == DNX_FIELD_CONTEXT_STATE_TABLE_MODE_ENABLED)
        {
            SHR_IF_ERR_EXIT(dnx_field_key_state_table_resources_free(unit, context_id, &state_table_info.key_id));
        }
    }
exit:
    SHR_FUNC_EXIT;
    ...
    dnx_field_context_mode_t * context_mode_p)
{
    ...
    if (stage == DNX_FIELD_STAGE_IPMF1)
    {
         ...
         */
        SHR_IF_ERR_EXIT(dnx_field_context_compare_info_t_init(unit, &compare_info));

        compare_info.pair_1.mode = context_mode_p->context_ipmf1_mode.compare_mode_1;
        compare_info.pair_2.mode = context_mode_p->context_ipmf1_mode.compare_mode_2;
        ...
        {
            SHR_IF_ERR_EXIT(dnx_algo_field_compare_key_id_get(unit,
        ...
        LOG_DEBUG_EX(BSL_LOG_MODULE,
                     ...
                     compare_info.pair_1.key_info[DNX_FIELD_CONTEXT_COMPARE_FIRST_KEY].key_id.id[0],
                     compare_info.pair_1.key_info[DNX_FIELD_CONTEXT_COMPARE_SECOND_KEY].key_id.id[0],
                     compare_info.pair_2.key_info[DNX_FIELD_CONTEXT_COMPARE_FIRST_KEY].key_id.id[0],
                     compare_info.pair_2.key_info[DNX_FIELD_CONTEXT_COMPARE_SECOND_KEY].key_id.id[0]);

        SHR_IF_ERR_EXIT(dnx_field_context_compare_info_set(unit, field_context_id, &compare_info));

        /*
         * Configure hashing mode.
         */
        SHR_IF_ERR_EXIT(dnx_field_context_hashing_info_t_init(unit, &hashing_info));
        hashing_info.mode = context_mode_p->context_ipmf1_mode.hash_mode;
        ...
        {
            SHR_IF_ERR_EXIT(dnx_algo_field_hash_key_id_get(unit, &(hashing_info.key_info.key_id)));
        }
        SHR_IF_ERR_EXIT(dnx_field_context_hashing_info_set(unit, field_context_id, &hashing_info));

        /*
         * Configure state table mode.
         */
        SHR_IF_ERR_EXIT(dnx_field_context_state_table_info_t_init(unit, &state_table_info));
        state_table_info.mode = context_mode_p->context_ipmf1_mode.state_table_mode;
        /** Set the hash to be unset, even though dnx_field_context_hashing_info_t_init() should take care of it.*/
        state_table_info.is_set = FALSE;
        if (context_mode_p->context_ipmf1_mode.state_table_mode == DNX_FIELD_CONTEXT_STATE_TABLE_MODE_ENABLED)
        {
            SHR_IF_ERR_EXIT(dnx_algo_field_state_table_key_id_get(unit, &(state_table_info.key_id)));
        }
         ...
         */
        SHR_IF_ERR_EXIT(DNX_FIELD_CONTEXT_IPMF2_INFO.cascaded_from.set(unit, field_context_id, field_context_id));
    }
    else if (stage == DNX_FIELD_STAGE_IPMF2)
    {
        SHR_IF_ERR_EXIT(DNX_FIELD_CONTEXT_IPMF2_INFO.
                        cascaded_from.set(unit, field_context_id, context_mode_p->context_ipmf2_mode.cascaded_from));
    }
    /*
    ...
    bcm_field_AppType_t * apptype_p)
{
    ...
    if (sal_strncmp(apptype_info_p->name, "", sizeof(apptype_info_p->name)))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_field_apptype_sw.info.name.stringncpy(unit, apptype_index, apptype_info_p->name));
    }

    /** Set the rest of the information to SW state.*/
    SHR_IF_ERR_EXIT(dnx_field_apptype_sw.info.flags.set(unit, apptype_index, flags));
    SHR_IF_ERR_EXIT(dnx_field_apptype_sw.info.
                    base_apptype.set(unit, apptype_index, apptype_info_p->base_static_apptype));
    SHR_IF_ERR_EXIT(dnx_field_apptype_sw.info.acl_context.set(unit, apptype_index, acl_context));
    SHR_IF_ERR_EXIT(dnx_field_apptype_sw.info.opcode_id.set(unit, apptype_index, opcode_id));
    SHR_IF_ERR_EXIT(dnx_field_apptype_sw.info.profile_id.set(unit, apptype_index, cs_profile_id));
    ...
    bcm_field_AppType_t * base_static_apptype_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_apptype_sw.info.base_apptype.get(unit, apptype_index, &base_apptype_uint8));
    ...
    dnx_field_context_apptype_info_t * apptype_info_p)
{
    ...
    if (apptype > bcmFieldAppTypeCount)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_field_apptype_sw.info.name.stringget(unit, apptype_index, apptype_info_p->name));
    ...
    bcm_field_AppType_t user_app_types[DNX_DATA_MAX_FIELD_KBP_APPTYPE_USER_NOF])
{
     ...
     */
    if (predef_app_type < 0
        && predef_app_type >= (sizeof(dnx_field_static_apptype_map) / sizeof(dnx_field_static_apptype_map[0])))
    {
    ...
    bcm_field_AppType_t * predef_app_type_p)
{
     ...
     */
    if (user_app_type < (sizeof(dnx_field_static_apptype_map) / sizeof(dnx_field_static_apptype_map[0])))
    {
...
/** See field_context.h*/
shr_error_e
dnx_field_context_hashing_info_set(
    int unit,
    dnx_field_context_t context_id,
    dnx_field_context_hashing_info_t * context_hashing_info_p)
{
    ...
    SHR_IF_ERR_EXIT(DNX_FIELD_CONTEXT_IPMF1_INFO.hashing_info.set(unit, context_id, context_hashing_info_p));
    ...
    dnx_field_context_state_table_info_t * state_table_info_p)
{
    ...
    SHR_IF_ERR_EXIT(DNX_FIELD_CONTEXT_IPMF1_INFO.state_table_info.set(unit, context_id, state_table_info_p));
    ...
    dnx_field_context_state_table_info_t * state_table_info_p)
{
    ...
    SHR_IF_ERR_EXIT(DNX_FIELD_CONTEXT_IPMF1_INFO.state_table_info.get(unit, context_id, state_table_info_p));
    ...
    dnx_field_context_t field_context_id)
{
    ...
    else if (stage == DNX_FIELD_STAGE_IPMF2)
    {
         ...
         */
        SHR_IF_ERR_EXIT(dnx_algo_field_context_id_is_allocated
                        (unit, DNX_FIELD_STAGE_IPMF2, field_context_id, &is_alloc));
        if (is_alloc)
        {
    ...
    dnx_field_context_mode_t * context_mode_p)
{
    ...
    if (stage == DNX_FIELD_STAGE_IPMF1)
    {
        ...
        SHR_IF_ERR_EXIT(DNX_FIELD_CONTEXT_IPMF1_INFO.state_table_info.
                        mode.get(unit, context_id, &context_mode_p->context_ipmf1_mode.state_table_mode));
    }
    else if (stage == DNX_FIELD_STAGE_IPMF2)
    {
        SHR_IF_ERR_EXIT(DNX_FIELD_CONTEXT_IPMF2_INFO.
    ...
    dnx_field_context_hash_info_t * hash_info_p)
{
    ...
    hash_info_p->hash_config.action_key = DBAL_NOF_ENUM_FIELD_HASH_LB_KEY_VALUES;
    hash_info_p->hash_config.crc_select = DNX_FIELD_CRC_SELECT_INVALID;
    hash_info_p->hash_config.hash_action = DNX_FIELD_CONTEXT_HASH_ACTION_NOF;
    ...
    dnx_field_context_hash_info_t * hash_info_p)
{
    ...
    {
        SHR_ERR_EXIT(_SHR_E_EXISTS, "'Hash' was alreasy set. Please call hash destroy first.\r\n");
    }
    ...
    dnx_field_context_hash_info_t * hash_info_p)
{
    ...
    /**
     * We are checking if any on the input params are set in order to be written to the DBAL.
     * Unset/disabled values should not be written as that is the default behavior.
     */
    if (hash_info_p->compression == TRUE || hash_info_p->order == TRUE
        || hash_info_p->hash_function != DBAL_NOF_ENUM_CONTEXT_HASH_FUNCTION_VALUES)
    {
        ...
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_COMPRESSION, INST_SINGLE,
                                     hash_info_p->compression);

        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_SWITCH, INST_SINGLE, hash_info_p->order);

        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_HASH_FUNC, INST_SINGLE,
                                     hash_info_p->hash_function);
    ...
    /**
     * We are checking if action_key set in order to be written to the DBAL.
     * Unset/disabled values should not be written as that is the default behavior.
     */
    if (hash_info_p->hash_config.action_key != DBAL_NOF_ENUM_FIELD_HASH_LB_KEY_VALUES)
    {
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC
                        (unit, DBAL_TABLE_FIELD_CONTEXT_HASH_CONFIGURATION_WITH_KEY, &entry_handle_id));
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_FIELD_HASH_LB_KEY,
                                   hash_info_p->hash_config.action_key);
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_FIELD_PMF_CTX_ID, context_id);

        if (hash_info_p->hash_config.crc_select != DNX_FIELD_CRC_SELECT_INVALID)
        {
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_CRC_SELECT, INST_SINGLE,
                                         hash_info_p->hash_config.crc_select);
        }
        if (hash_info_p->hash_config.hash_action != DNX_FIELD_CONTEXT_HASH_ACTION_NOF)
        {
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_HASH_ACTION, INST_SINGLE,
                                         hash_info_p->hash_config.hash_action);
        }
    ...
    SHR_IF_ERR_EXIT(dnx_field_context_hashing_info_set(unit, context_id, &hashing_info));
    ...
    dnx_field_context_hash_info_t * hash_info_p)
{
    ...
    if (hash_info_p->hash_config.action_key != DBAL_NOF_ENUM_FIELD_HASH_LB_KEY_VALUES)
    {
        ...
        dbal_value_field8_request(unit, entry_handle_id, DBAL_FIELD_CRC_SELECT, INST_SINGLE, &crc_select);

        dbal_value_field8_request(unit, entry_handle_id, DBAL_FIELD_HASH_ACTION, INST_SINGLE, &hash_action);

        SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));

        hash_info_p->hash_config.crc_select = crc_select;
        hash_info_p->hash_config.hash_action = hash_action;
    }
    ...
    dnx_field_key_compare_pair_info_t * cmp_pair_p)
{
     ...
     * */
    SHR_IF_ERR_EXIT(dnx_field_context_compare_info_set(unit, context_id, &compare_info));
    ...
    if (found == FALSE)
    {
        for (apptype_ndx = user_apptype_first; apptype_ndx <= user_apptype_last; apptype_ndx++)
        {
            ...
            if (is_alloc)
            {
                ...
                sal_memset(name_ndx, 0x0, sizeof(name_ndx));
                SHR_IF_ERR_EXIT(dnx_field_apptype_sw.info.name.stringget(unit, apptype_ndx, name_ndx));
                if (0 == sal_strncmp(name, name_ndx, DBAL_MAX_STRING_LENGTH))
                {
                    found = TRUE;
                    break;
    ...
    int unit)
{
    ...
    DNX_FIELD_KBP_OPCODE_ITERATOR(opcode_id)
    {
        ...
        {
            uint8 fwd_context = fwd_acl_ctx_mapping[fwd_context_idx].fwd_context;

            /** Sanity check*/
            if (fwd_acl_ctx_mapping[fwd_context_idx].nof_acl_contexts != 1)
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "Opcode %d, fwd_context %d has %d acl_contexts. only one supported.\n",
                             opcode_id, fwd_context, fwd_acl_ctx_mapping[fwd_context_idx].fwd_context);
            }

            for (acl_context_idx = 0; acl_context_idx < fwd_acl_ctx_mapping[fwd_context_idx].nof_acl_contexts;
                 acl_context_idx++)
            {
                uint8 acl_context = fwd_acl_ctx_mapping[fwd_context_idx].acl_contexts[acl_context_idx];

                /** Sanity check*/
                if (acl_context != fwd_context)
                {
                    SHR_ERR_EXIT(_SHR_E_INTERNAL, "Opcode %d, fwd_context %d acl_context %d. For predefined opcodes, "
                                 "we expect the fwd_context and acl_context to be equal.\n",
                                 opcode_id, fwd_context, acl_context);
                }

                if (acl_context_marked[acl_context])
                {
                    /** If we already set a profile for the ACL context, check that we do no have a profile mismatch.*/
                    SHR_IF_ERR_EXIT(dnx_field_context_apptype_acl_cs_profile_get(unit, acl_context, &profile_id_get));
                    if (profile_id_get != profile_id)
                    {
                        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Trying to rewrite CS profile %d as %d for ACL context %d.\n",
                                     profile_id_get, profile_id, acl_context);
                    }
                }
                else
                {
                    /** Write the profile*/
                    SHR_IF_ERR_EXIT(dnx_field_context_apptype_acl_cs_profile_set(unit, acl_context, profile_id));
                    /** Only at init, we also write the FWD context profile for the predefined apptypes*/
                }
            }
            /** Only at init, we also write the FWD context profile for the predefined apptypes*/
            SHR_IF_ERR_EXIT(dnx_field_context_apptype_fwd_cs_profile_set(unit, fwd_context, profile_id));
            SHR_IF_ERR_EXIT(dnx_field_context_apptype_fwd_cs_profile_ifwd2_set(unit, fwd_context, profile_id));
        }
    }
    ...
    int unit)
{
    ...
    if (dnx_kbp_device_enabled(unit) && is_image_standard_1)
    {
        /*
