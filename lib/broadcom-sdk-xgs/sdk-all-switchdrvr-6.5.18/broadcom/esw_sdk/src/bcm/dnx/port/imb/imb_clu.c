    ...
    portmod_pm_create_info_t * pm_info)
{
    ...
    ref_clk = dnx_data_pll.pll3.nif_pll_get(unit, lcpll)->out_freq;
    if (ref_clk == DNX_SERDES_REF_CLOCK_BYPASS)
    {
        ref_clk = dnx_data_pll.pll3.nif_pll_get(unit, lcpll)->in_freq;
    }
    ...
    imb_specific_create_info_t * imb_specific_info)
{
    ...
    if (!dnx_drv_soc_warm_boot(unit))
    {
        ...
        if (dnx_data_nif.eth.feature_get(unit, dnx_data_nif_eth_rmc_flush_support))
        {
             ...
             */
            SHR_IF_ERR_EXIT(imb_ethu_sch_cnt_dec_thres_hw_set(unit, ethu_id, 0));
        }
        /*
...
imb_clu_port_mode_update(
    ...
    bcm_port_t port)
{
    int nof_port_lanes, first_phy, first_lane_local = 0, current_mode = portmodPortModeCount, new_port_mode =
    ...
    first_lane_local = first_phy % dnx_data_nif.eth.nof_lanes_in_clu_get(unit);
    ...
    uint32 flags)
{
     ...
     *
     */
    SHR_IF_ERR_EXIT(imb_clu_portmod_add_info_config(unit, port, &add_info));
 ...
 */
int
imb_clu_port_scheduler_config_set(
    int unit,
    bcm_port_t port,
    int enable)
{
    ...
    int is_set)
{
    ...
    switch (type)
    {
        ...
        case BCM_PORT_PHY_CONTROL_TX_POLARITY:
        {
            if (is_set)
            {
                SHR_ERR_EXIT(_SHR_E_UNAVAIL, "PHY control type %d set is not supported on Falcon PHY.\r\n", type);
            }
            break;
        }
                    ...
                    if (abilities[i].speed == dnx_data_nif.eth.supported_an_abilities_get(unit, ii)->speed)
                    {
                            ...
                            dnx_data_nif.eth.supported_an_abilities_get(unit, ii)->nof_lanes)
                        {
                                ...
                                dnx_data_nif.eth.supported_an_abilities_get(unit, ii)->fec_type)
                            {
                                ...
                                {
                                    if ((abilities[i].channel ==
                                         dnx_data_nif.eth.supported_an_abilities_get(unit, ii)->channel)
                                        || (bcmPortPhyChannelAll ==
                                            dnx_data_nif.eth.supported_an_abilities_get(unit, ii)->channel))
                                    {
                                        is_ability_valid = 1;
                                        break;
