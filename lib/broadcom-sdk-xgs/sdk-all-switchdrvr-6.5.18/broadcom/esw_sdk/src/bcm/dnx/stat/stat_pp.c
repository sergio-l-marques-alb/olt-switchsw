...
/**
 *  \brief Dispatch algo callbacks according to engine
 *  In case of reversed limitation on ETPP - dispatch according to counting, meter profile IDs
 *  ERRATA #145 - ETPP FWD,ENC blocks generate stat commands reversed
 */
    ...
    bcm_stat_pp_profile_info_t * stat_pp_profile_info)
{
    ...
    switch (engine_source)
    {
        ...
        case bcmStatCounterInterfaceEgressTransmitPp:
            ...
            /**
             * In case HW issue #2541 exists (Statistics Entries in ETPP Blocks built differently) -
             * differentiate between counting, metering profiles
             * ERRATA #145 - ETPP FWD,ENC blocks generate stat commands reversed
             */
...
/**
 * \brief - set egress lif dbal table id, lif dbal table key based on virtual, non-virtual lif
 *
 * \param [in] unit - relevant unit
 * \param [in] gport - physical or logical port
 ...
 */
static shr_error_e
dnx_stat_pp_set_lif_table_id_and_key(
    int unit,
    bcm_gport_t gport,
    int local_out_lif,
    uint32 *outlif_entry_handle_id)
{
    uint8 is_virtual = FALSE;
    ...
    if (BCM_GPORT_SUB_TYPE_IS_VIRTUAL_EGRESS_MATCH(gport) || BCM_GPORT_SUB_TYPE_IS_VIRTUAL_EGRESS_DEFAULT(gport))
    {
        ...
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC
                        (unit, DBAL_TABLE_EGRESS_VIRTUAL_GPORT_TO_MATCH_INFO_SW, &get_entry_handle_id));
    ...
    {
        lif_table_key = local_out_lif;
        lif_table_key_field = DBAL_FIELD_LOCAL_OUT_LIF;
    }
    ...
    bcm_stat_pp_info_t stat_info)
{
    ...
    else
    {
                    ...
                    DBAL_RESULT_TYPE_EEDB_MPLS_TUNNEL_ETPS_MPLS_PHP_STAT)))
        {
            ...
            /**Due to reverse stat_cmd, stat_id bug, can't use the WA for stat ids smaller than 16 bits.
             * For stat_id larger than 15 bits - check and verify the maximum possible value*/
            ...
            {
                SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_LOCAL_OUT_LIF_INFO_SW, &outlif_entry_handle_id));
            }
            SHR_IF_ERR_EXIT(dnx_stat_pp_set_lif_table_id_and_key
    ...
    bcm_stat_pp_info_t * stat_info)
{
         ...
         (gport_hw_resources.outlif_dbal_result_type != DBAL_RESULT_TYPE_EEDB_MPLS_TUNNEL_ETPS_MPLS_PHP_STAT)))
    {
        ...
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get(unit,
                                                            ...
                                                            DBAL_FIELD_STAT_PP_ETPP_IS_COUNTING_PROFILE, INST_SINGLE,
                                                            &is_counting_profile));
