    ...
    int lane_id)
{
    ...
    if ((DNX_ALGO_PORT_TYPE_IS_NIF_ILKN(unit, port_info, 1, 1))
        && (dnx_data_fabric.general.feature_get(unit, dnx_data_fabric_general_is_supported))
    ...
    soc_dnxc_lane_map_db_map_t * lane2serdes)
{
    ...
    for (lane = 0; lane < map_size; ++lane)
    {
        ...
        if ((serdes_rx != pre_lane2serdes.rx_id) || (serdes_tx != pre_lane2serdes.tx_id))
        {
            ...
            if (DNX_ALGO_LANE_MAP_IS_NIF_SIDE(unit, type))
            {
                int phy_map_granularity = dnx_data_nif.eth.phy_map_granularity_get(unit);
                int pm_index = dnx_data_nif.eth.phy_map_get(unit, lane / phy_map_granularity)->pm_index;
                _SHR_PBMP_OR(phys_to_check, dnx_data_nif.eth.pm_properties_get(unit, pm_index)->phys);
            }
            else
            {
                /** fabric side */
