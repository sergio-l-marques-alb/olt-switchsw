...
{
    uint32 entry_handle_id;
    uint32 ffc_instruction_0 = 0, ffc_instruction_1 = 0, ffc_instruction_2 = 0;
    uint32 layer_offset;
    uint32 ffc_0_input_offset, ffc_1_input_offset, ffc_2_input_offset;
    ...
    layer_offset = prt_configuration->layer_offset + first_header_size_to_skip;
     ...
     /** For PTCH1 and PTCH2 ports should be updated not only the layer offset, but also FFC0, FFC1 and FFC2 values */
    if ((prt_configuration->port_termination_ptc_profile == DBAL_ENUM_FVAL_PORT_TERMINATION_PTC_PROFILE_PTCH2_UD1)
        || (prt_configuration->port_termination_ptc_profile == DBAL_ENUM_FVAL_PORT_TERMINATION_PTC_PROFILE_PTCH2_UD2)
        || (prt_configuration->port_termination_ptc_profile == DBAL_ENUM_FVAL_PORT_TERMINATION_PTC_PROFILE_PTCH1_UD1))
    {
    ...
    uint32 first_header_size_to_skip)
{
    ...
    for (index = 0; index < prt_config_dnx_data_table_info->key_size[0]; index++)
    {
        ...
        if (prt_configuration->port_termination_ptc_profile == ptc_profile)
        {
            /** Write to INGRESS_PRT_PROFILE table */
            SHR_IF_ERR_EXIT(dnx_port_ingress_prt_user_defined_profile_configuration
                            (unit, prt_configuration, first_header_size_to_skip));
        }
    }
    ...
    int *first_header_size_value)
{
    ...
    if ((ptc_profile == DBAL_ENUM_FVAL_PORT_TERMINATION_PTC_PROFILE_ETH_UD1)
        || (ptc_profile == DBAL_ENUM_FVAL_PORT_TERMINATION_PTC_PROFILE_ETH_UD2)
        || (ptc_profile == DBAL_ENUM_FVAL_PORT_TERMINATION_PTC_PROFILE_PTCH1_UD1)
        || (ptc_profile == DBAL_ENUM_FVAL_PORT_TERMINATION_PTC_PROFILE_PTCH2_UD1)
        || (ptc_profile == DBAL_ENUM_FVAL_PORT_TERMINATION_PTC_PROFILE_PTCH2_UD2))
    {
        /** Get layer_offset */
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_INGRESS_PRT_INFO, &entry_handle_id));
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_PRT_RECYCLE_PROFILE,
        ...
        else if (ptc_profile == DBAL_ENUM_FVAL_PORT_TERMINATION_PTC_PROFILE_PTCH1_UD1)
        {
    ...
    int *first_header_size_value)
{
    ...
    *first_header_size_value = 0;
    /** Get the ptc profile */
    SHR_IF_ERR_EXIT(dnx_port_prt_ptc_profile_get(unit, port, &ptc_profile));
    if (ptc_profile == DBAL_ENUM_FVAL_PORT_TERMINATION_PTC_PROFILE_PTCH1_UD1_AFTER_INJECTED)
    {
        /** Get layer_offset */
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_INGRESS_PRT_INFO, &entry_handle_id));
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_PRT_RECYCLE_PROFILE,
    ...
    uint32 first_header_size_value)
{
    ...
    dbal_enum_value_field_port_termination_ptc_profile_e ptc_profile = 0;
    dbal_enum_value_field_port_termination_ptc_profile_e new_ptc_profile;
    int old_profile_index = 0;
    uint32 default_first_header_size = 0;
    ...
    SHR_IF_ERR_EXIT(dnx_switch_header_type_in_get(unit, port, &ptc_profile, &switch_header_type));
    SHR_IF_ERR_EXIT(dnx_port_first_header_size_to_skip_get(unit, port, (int *) &default_first_header_size));

    if (switch_header_type == BCM_SWITCH_PORT_HEADER_TYPE_ETH)
    {
        SHR_IF_ERR_EXIT(dnx_algo_port_db.
                        eth_ports_first_header_size_profile_data.profile_get(unit, &default_first_header_size,
                                                                             &default_profile));
    }
    else if (switch_header_type == BCM_SWITCH_PORT_HEADER_TYPE_INJECTED)
    {
        SHR_IF_ERR_EXIT(dnx_algo_port_db.
                        ptch1_ports_first_header_size_profile_data.profile_get(unit, &default_first_header_size,
                                                                               &default_profile));
    }
    ...
    {
        SHR_IF_ERR_EXIT(dnx_algo_port_db.
                        ptch2_ports_first_header_size_profile_data.profile_get(unit, &default_first_header_size,
                                                                               &default_profile));
    }

    old_profile_index = default_profile;

    /** Add new profile to the template manager */
    if (switch_header_type == BCM_SWITCH_PORT_HEADER_TYPE_ETH)
    {
        SHR_IF_ERR_EXIT(dnx_algo_port_db.
                        eth_ports_first_header_size_profile_data.exchange(unit, 0, &first_header_size_value,
                                                                          old_profile_index, NULL, &new_profile_index,
    ...
    {
        SHR_IF_ERR_EXIT(dnx_algo_port_db.
                        ptch2_ports_first_header_size_profile_data.exchange(unit, 0, &first_header_size_value,
                                                                            old_profile_index, NULL, &new_profile_index,
                                                                            &first_reference, &last_reference));
    }
    ...
    {
        SHR_IF_ERR_EXIT(dnx_algo_port_db.
                        ptch1_ports_first_header_size_profile_data.exchange(unit, 0, &first_header_size_value,
                                                                            old_profile_index, NULL, &new_profile_index,
                                                                            &first_reference, &last_reference));
    }
    ...
    if (first_reference)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_port_prt_profiles_update(unit, new_profile_index, first_header_size_value));
    }

    if ((switch_header_type == BCM_SWITCH_PORT_HEADER_TYPE_ETH)
        && (new_profile_index == DBAL_ENUM_FVAL_PORT_TERMINATION_PTC_PROFILE_ETH_UD2 + 1))
    {
        new_ptc_profile = DBAL_ENUM_FVAL_PORT_TERMINATION_PTC_PROFILE_ETHERNET;
    }
    else if ((switch_header_type == BCM_SWITCH_PORT_HEADER_TYPE_INJECTED)
             && (new_profile_index == DBAL_ENUM_FVAL_PORT_TERMINATION_PTC_PROFILE_PTCH1_UD1 + 1))
    {
        new_ptc_profile = DBAL_ENUM_FVAL_PORT_TERMINATION_PTC_PROFILE_PTCH1;
              ...
              || (switch_header_type == BCM_SWITCH_PORT_HEADER_TYPE_INJECTED_2_PP))
             && (new_profile_index == DBAL_ENUM_FVAL_PORT_TERMINATION_PTC_PROFILE_PTCH2_UD2 + 1))
    {
    ...
    if (last_reference)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_port_prt_profiles_update(unit, old_profile_index, 0));
    }
    ...
    uint32 first_header_size_value)
{
    ...
    if (switch_header_type == BCM_SWITCH_PORT_HEADER_TYPE_INJECTED)
    {
        SHR_IF_ERR_EXIT(dnx_port_first_header_size_to_skip_after_injected_get
                        (unit, port, (int *) &default_first_header_size));
        SHR_IF_ERR_EXIT(dnx_algo_port_db.ptch1_ports_first_header_size_after_injected_profile_data.profile_get(unit,
                                                                                                               &default_first_header_size,
                                                                                                               &old_profile));
        SHR_IF_ERR_EXIT(dnx_algo_port_db.ptch1_ports_first_header_size_after_injected_profile_data.exchange(unit, 0,
    ...
    if (new_profile_index == DBAL_ENUM_FVAL_PORT_TERMINATION_PTC_PROFILE_PTCH1_UD1_AFTER_INJECTED + 1)
    {
        new_ptc_profile = DBAL_ENUM_FVAL_PORT_TERMINATION_PTC_PROFILE_PTCH1;
    }
    else
    ...
    {
        /** Set user defined PRT configuration to its default values */
        SHR_IF_ERR_EXIT(dnx_port_prt_profiles_update(unit, old_profile, 0));
    }
    ...
    int value)
{
    ...
    switch (type)
    {
        case bcmPortControlMplsFRREnable:
        {
                ...
                (llr_cs_profile != DBAL_ENUM_FVAL_LLR_CS_PORT_CS_PROFILE_802_1_BR_WITH_FRR_OPTIMIZATION))
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             ...
                             "==> It is %d. This is illegal. Quit.", __FUNCTION__, llr_cs_profile);
            }
            /*
        ...
        case bcmPortControlOverlayRecycle:
        {
            if (value == 1)
            {
                SHR_IF_ERR_EXIT(dnx_port_pp_overlay_recycle_set(unit, port, 1));
                /*
            ...
            {
                SHR_IF_ERR_EXIT(dnx_port_pp_overlay_recycle_set(unit, port, 0));
            }
        ...
        case bcmPortControlDistributeLearnMessageMode:
        {
            ...
            if (value == 1)
            {
                /** Configure termination and forward context port profile. No need for MACT limit WA */
                if (dnx_data_l2.hw_bug.feature_get(unit, dnx_data_l2_hw_bug_learn_limit))
                {
                    forward_context_port_profile =
                        DBAL_ENUM_FVAL_ETPP_FORWARD_PORT_CS_VAR_MACT_WA_JR1_DSP_CMD_TO_REFRESH;
                }
                SHR_IF_ERR_EXIT(dnx_port_pp_term_and_forward_context_port_profile_set
                                (unit, port,
                                 DBAL_ENUM_FVAL_ETPP_TERMINATION_PORT_CS_VAR_MACT_WA_JR1_DSP_SWITCH_DEST_ENC_JR1_ARAD,
                                 forward_context_port_profile));
            }
            else if (value == 0)
            {
        ...
        {
            SHR_IF_ERR_EXIT(imb_port_eee_enable_set(unit, port, value));
            break;
        }
        case bcmPortControlEEETransmitIdleTime:
        {
            ...
            SHR_IF_ERR_EXIT(imb_port_eee_get(unit, port, &eee));
            eee.tx_idle_time = value;
            SHR_IF_ERR_EXIT(imb_port_eee_set(unit, port, &eee));
            break;
        }
    ...
    int *value_p)
{
    ...
    switch (type)
    {
        ...
        case bcmPortControlEEEEnable:
        {
            SHR_IF_ERR_EXIT(imb_port_eee_enable_get(unit, port, value_p));
            break;
        }
        case bcmPortControlEEETransmitIdleTime:
        {
...
/**
 * \brief - Configure PRT_VIRTUAL_PORT_TCAM per SYSTEM_PORT_AGGREGATE
 *
 * \param [in] unit - Relevant unit
 ...
 */
static shr_error_e
dnx_port_prt_virtual_port_mapping_set(
    int unit,
    ...
    uint32 pp_port)
{
    ...
    SHR_IF_ERR_EXIT(dbal_fields_struct_field_encode
                    (unit, DBAL_FIELD_PRTTCAM_KEY_VP_SPA, DBAL_FIELD_SYSTEM_PORT_AGGREGATE, &system_port_aggregate,
                     &complete_field_val));
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_PRTTCAM_KEY_VP_SPA, complete_field_val);
    ...
    {
        SHR_ERR_EXIT(_SHR_E_FULL, "Cannot insert the entry to PRT_VIRTUAL_PORT_TCAM, it's full, capacity is: %d!\n",
                     max_capacity);
    }

    SHR_IF_ERR_EXIT(dbal_entry_handle_access_id_set(unit, entry_handle_id, entry_access_id));

    /** Use flag DBAL_COMMIT_FORCE */
    ...
    SHR_IF_ERR_EXIT(dnx_port_sit_virtual_port_tcam_sw_set
                    (unit, entry_access_id, 1, 0, pp_port, system_port_aggregate, complete_field_val));
 ...
 */
shr_error_e
dnx_port_prt_virtual_port_mapping_clear(
    int unit,
    bcm_port_t logical_port)
{
    ...
    rv = dnx_port_sit_virtual_port_access_id_get_by_port(unit, logical_port, &access_id);
    ...
    uint32 is_lag)
{
    ...
    SHR_IF_ERR_EXIT(dnx_port_prt_virtual_port_mapping_set(unit, system_port, out_pp_port));
    ...
    int unit)
{
    ...
    /** Clear PRT_VIRTUAL_PORT_TCAM */
    SHR_IF_ERR_EXIT(dnx_port_prt_virtual_port_mapping_clear(unit, logical_port));
