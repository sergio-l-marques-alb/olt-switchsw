    ...
    uint32 *default_outlif_profile)
{
     ...
     * which is a subject to ARR Prefix, which is used for VXLAN WA and should be supported only in the IP namespace scope
     */
    ...
    uint32 *resultType)
{
    ...
    if ((BCM_GPORT_IS_TRUNK(vlan_port->port)) &&
        dnx_data_esem.hw_bug.feature_get(unit, dnx_data_esem_hw_bug_key_with_sspa_contains_member_id))
    {
            ...
            else if (retVal == _SHR_E_NONE)
            {
                ...
                if (hw_resultType != DBAL_RESULT_TYPE_ESEM_FORWARD_DOMAIN_MAPPING_DB_ETPS_L2_FODO)
                {
                    SHR_ERR_EXIT(_SHR_E_INTERNAL,
                                 ...
                                 0, hw_resultType, DBAL_RESULT_TYPE_ESEM_FORWARD_DOMAIN_MAPPING_DB_ETPS_L2_FODO);
                }
    ...
    dnx_ingress_ac_table_info_t * ingress_ac_table_info)
{
    ...
    ingress_ac_table_info->large_learn_field = FALSE;
    ...
    if (!(_SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_NATIVE)))
    {
        ...
        {
            ingress_ac_table_info->large_learn_field = TRUE;
        }
        if (_SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_INGRESS_WIDE))
        {
            /*
        ...
        if (is_native == FALSE)
        {
            switch (ingress_ac_table_info->learn_context)
            {
                ...
                case DBAL_ENUM_FVAL_LEARN_PAYLOAD_CONTEXT_NO_LIF_OPTIMIZED:
                    ...
                    if (ingress_ac_table_info->large_learn_field)
                    {
                        /** generic data with 28bits, use BASIC_SYM_LIF */
                        if (_SHR_IS_FLAG_SET(vlan_port->flags, BCM_VLAN_PORT_INGRESS_WIDE) && !has_protection)
                        {
                            dbal_entry_value_field32_set(unit, entry_handle_id,
                                                         DBAL_FIELD_LEARN_INFO_NO_LIF_BASIC_SYM_LIF_BASIC, INST_SINGLE,
    ...
    bcm_vlan_port_t * vlan_port)
{
    ...
    else
    {
        /*
         * Handle REPLACE - 
         * LIF table manager also handling the match-info reallocation, thus nothing to do here!
    ...
    bcm_vlan_port_t * vlan_port)
{
    ...
    is_native_ac =
        ...
        TRUE : FALSE;
    is_pon_dtc_ac = (gport_hw_resources->inlif_dbal_table_id == DBAL_TABLE_PON_DTC_CLASSIFICATION) ? TRUE : FALSE;
 ...
 * \param [in] is_replace - is replace
 * \param [in] gport_hw_resources - pointer to the gport HW resource (only valid for REPLACE case)
    ...
    bcm_vlan_port_t * vlan_port)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_ingress_inlif_profile_alloc
                        (unit, vlan_port->ingress_network_group_id,
     ...
     */
    SHR_IF_ERR_EXIT(dnx_vlan_port_learn_info_fill(unit, ingress_ac_table_info, &forward_info, ll_app_type, is_egress));
    ...
    bcm_vlan_port_t * vlan_port)
{
    ...
    SHR_IF_ERR_EXIT(dnx_vlan_port_create_ingress_lif_match_info_set
                    (unit, local_inlif, ingress_ac_table_info->old_local_inlif, is_replace, vlan_port));
    ...
    if (!is_replace)
    {
        ...
        switch (vlan_port->criteria)
        {
                ...
                if (vlan_port->match_ethertype)
                {
                    SHR_IF_ERR_EXIT(dnx_vlan_port_create_ingress_match_port_vlan_vlan_pcp_ethertype
                                    (unit, vlan_port->port, BCM_VLAN_INVALID, BCM_VLAN_INVALID,
                                     vlan_port->match_ethertype, BCM_DNX_PCP_INVALID, local_inlif));
                }
                else
                {
                    SHR_IF_ERR_EXIT(dnx_vlan_port_create_ingress_match_port_default
                                    (unit, vlan_port->port, local_inlif));
                }
                break;
            }
            case BCM_VLAN_PORT_MATCH_PORT_VLAN:
                ...
                if (is_native_ac == FALSE)
                {
                    ...
                    else
                    {
                        SHR_IF_ERR_EXIT(dnx_vlan_port_create_ingress_match_port_vlan
                                        (unit, vlan_port->port, vlan_port->match_vlan, local_inlif));
                    }
                }
                else
                {
            ...
            case BCM_VLAN_PORT_MATCH_PORT_PCP_VLAN:
            {
                ...
                SHR_IF_ERR_EXIT(dnx_vlan_port_create_ingress_match_port_vlan_vlan_pcp_ethertype
                                (unit, vlan_port->port, vlan_port->match_vlan, BCM_VLAN_INVALID,
                                 vlan_port->match_ethertype, vlan_port->match_pcp, local_inlif));
                break;
            }
            case BCM_VLAN_PORT_MATCH_PORT_PCP_VLAN_STACKED:
            {
            ...
            case BCM_VLAN_PORT_MATCH_PORT_TUNNEL:
            {
                SHR_IF_ERR_EXIT(dnx_vlan_port_create_ingress_match_port_tunnel_unmatched
                                (unit, vlan_port->port, vlan_port->match_tunnel_value, local_inlif));
                break;
            }
            case BCM_VLAN_PORT_MATCH_PORT_TUNNEL_VLAN:
            {
            ...
            case BCM_VLAN_PORT_MATCH_PORT_TUNNEL_VLAN_STACKED:
            {
                SHR_IF_ERR_EXIT(dnx_vlan_port_create_ingress_match_port_tunnel_svlan_cvlan
                                (unit, vlan_port->port, vlan_port->match_tunnel_value,
                                 vlan_port->match_vlan, vlan_port->match_inner_vlan, local_inlif));
                break;
            }
            case BCM_VLAN_PORT_MATCH_PORT_TUNNEL_CVLAN_STACKED:
            {
    ...
    bcm_gport_t gport)
{
    ...
    /*
     * 2. Set MP relevant fields:
     *      - LEARN_ENABLE to FALSE.
    ...
    if (!is_virtual_ac)
    {
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LEARN_ENABLE, INST_SINGLE, FALSE);
    }
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_FODO_ASSIGNMENT_MODE, INST_SINGLE,
    ...
    dnx_ingress_ac_fields_table_info_get_t * ingress_ac_fields_info)
{
    ...
    if (is_ingress == TRUE)
    {
         ...
         */
        SHR_IF_ERR_EXIT(dnx_vlan_port_set_ll_app_type(unit, vlan_port, TRUE, &gport_hw_resources, &ll_app_type));
        ...
        SHR_IF_ERR_EXIT(dnx_vlan_port_create_ingress
                        (unit, gport_hw_resources.global_in_lif, &ingress_ac_table_info,
                         ingress_ac_fields_info, TRUE, vlan_port));
    }
    ...
    bcm_vlan_port_t * vlan_port)
{
    ...
    else
    {
         ...
         */
        SHR_IF_ERR_EXIT(dnx_vlan_port_set_ll_app_type(unit, vlan_port, FALSE, NULL, &ll_app_type));
        ...
        {
            SHR_IF_ERR_EXIT(dnx_vlan_port_create_ingress
                            (unit, global_lif, &ingress_ac_table_info, NULL, FALSE, vlan_port));
        }
        if (is_egress == TRUE)
        {
            SHR_IF_ERR_EXIT(dnx_vlan_port_create_egress
    ...
    int *created_local_lif)
{
    ...
    dnx_qos_propagation_type_e ecn_propagation_type = DNX_QOS_PROPAGATION_TYPE_UNIFORM;
    ...
    int unit)
{
    ...
    if (inlif_info.local_inlif & 0x2)
    {
        ...
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_RESULT_TYPE, INST_SINGLE,
                                     DBAL_RESULT_TYPE_IN_LIF_FORMAT_DUMMY_MPLS_IN_LIF_MPLS_DUMMY);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_TERMINATION_TYPE, INST_SINGLE, 0);
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    }
    else
    {
    ...
    void *user_data)
{
    uint32 entry_handle_id;
    int is_end;
    uint32 esem_handle_non_native, esem_handle_native, esem_handle_dual_homing;
    uint32 dual_homing_resultType;
     ...
     * and result-type is not "DUAL-HOMING"
     */
    esem_handle_native = dnx_data_esem.default_result_profile.default_native_get(unit);
    SHR_IF_ERR_EXIT(dbal_iterator_key_field_arr32_rule_add
                    (unit, entry_handle_id, DBAL_FIELD_ESEM_DEFAULT_RESULT_PROFILE, DBAL_CONDITION_NOT_EQUAL_TO,
                     &esem_handle_native, NULL));

    esem_handle_non_native = dnx_data_esem.default_result_profile.default_ac_get(unit);
    SHR_IF_ERR_EXIT(dbal_iterator_key_field_arr32_rule_add
                    (unit, entry_handle_id, DBAL_FIELD_ESEM_DEFAULT_RESULT_PROFILE, DBAL_CONDITION_NOT_EQUAL_TO,
                     &esem_handle_non_native, NULL));

    esem_handle_dual_homing = dnx_data_esem.default_result_profile.default_dual_homing_get(unit);
    SHR_IF_ERR_EXIT(dbal_iterator_key_field_arr32_rule_add
                    (unit, entry_handle_id, DBAL_FIELD_ESEM_DEFAULT_RESULT_PROFILE, DBAL_CONDITION_NOT_EQUAL_TO,
                     &esem_handle_dual_homing, NULL));
...
/**
 * \brief -
 *  Set native index in in_lif_profile.
 *
 * \param [in] unit - the relevant unit.
 * \param [in] gport - LIF-ID in gport format.
 * \param [in] native_indexed_mode - native indexed mode or unindexed mode.
 *
 * \return
 *  shr_error_e
 *
 ...
 */
static shr_error_e
dnx_vlan_port_lif_native_indexed_update(
    int unit,
    bcm_gport_t gport,
    int native_indexed_mode)
{
    ...
    in_lif_profile_info.native_indexed_mode = 1;
 ...
 */
shr_error_e
dnx_vlan_port_pwe_indexed_mode_native_indexed_update(
    int unit,
    bcm_gport_t gport,
    bcm_port_match_info_t * match_info)
{
    int is_virtual_ac = 0, is_native_ac = 0;
    dnx_ingress_ac_fields_table_info_get_t ingress_ac_fields_info;
    bcm_vlan_port_t vlan_port;
    bcm_mpls_port_t mpls_port;
    int native_indexed_mode = TRUE;

    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);

    sal_memset(&ingress_ac_fields_info, 0, sizeof(dnx_ingress_ac_fields_table_info_get_t));
    bcm_vlan_port_t_init(&vlan_port);
    vlan_port.vlan_port_id = gport;

    SHR_IF_ERR_EXIT(dnx_vlan_port_find_by_gport(unit, &vlan_port, &ingress_ac_fields_info));
    is_native_ac = (vlan_port.flags & BCM_VLAN_PORT_NATIVE) ? 1 : 0;
    is_virtual_ac = (vlan_port.flags & BCM_VLAN_PORT_VLAN_TRANSLATION) ? 1 : 0;

    /** Only update for pwe indexed tagged mode: ac should be native, non virtual */
    if (!is_native_ac || is_virtual_ac)
    {
        SHR_EXIT();
    }
    if (!BCM_GPORT_IS_MPLS_PORT(match_info->port))
    {
        SHR_EXIT();
    }

    bcm_mpls_port_t_init(&mpls_port);
    mpls_port.mpls_port_id = match_info->port;
    SHR_IF_ERR_EXIT(bcm_mpls_port_get(unit, vlan_port.vsi, &mpls_port));
    /** Only update for pwe indexed tagged mode: pwe lif should be tagged mode  */
    if (!(mpls_port.flags & BCM_MPLS_PORT_SERVICE_TAGGED))
    {
        SHR_EXIT();
    }

    /** Update in_lif_proflie for native ac */
    SHR_IF_ERR_EXIT(dnx_vlan_port_lif_native_indexed_update(unit, gport, native_indexed_mode));
