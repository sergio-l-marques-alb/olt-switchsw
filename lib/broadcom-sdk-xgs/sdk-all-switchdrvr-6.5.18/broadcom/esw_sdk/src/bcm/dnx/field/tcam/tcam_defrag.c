...
/**
 * \brief
 *  Checks the banks status on the given core  for the given handler, and returns whether the
 *  banks associated with the given handler are full or not.
 *
 * \param [in] unit       - Device ID
 * \param [in] core       - Core ID
 * \param [in] handler_id - The handler id to check the banks status for
 * \param [out] full      - TRUE in case the banks associated with the given handler are full, FALSE
 *                          otherwise
 *
 * \return
    ...
    uint8 *full)
{
    ...
    *full = TRUE;
    ...
    for (bank_i = 0; bank_i < bank_count; bank_i++)
    {
        ...
        {
            /*
             * It's sufficent that one bank is not full, to declare the status as not full
             */
            break;
        }
    }
    ...
    dnx_field_tcam_location_t * location)
{
    ...
    /*
     * Check whether a new bank needs to be added
     */
    ...
    /*
     * What we want right now is to search for an empty place in the bank
     * and move the entries up/down in order to make room in the given location.
     * For example:
     * 
     * We want to make room in the given location, so, first we search for an
     * empty location:
     * y - other handler allocated location
     * x - our handler allocated location
     * o - empty location
     * +-----+
     * |  x1  |
     * |  o   | <- Empty location
     ...
     * +=====+
     *
     * Then we move the Entries from the given location to the empty location
     * while keeping the entries order, in order making space in the given
     * location:
     *
     * +-----+
     * |  x1  |
     * |  X2  | <- PREV Empty location
     * |  X3  |
     * |  y1  |
     * |  O   | <- Given location
     * |  y2  |
    ...
    if (found && (absolute_location != empty_place))
    {
        if (absolute_location > empty_place)
        {
            /*
             * - -
             *   In case we're shuffling entries backwards, we need to start shuffling from
             *   the previous handler entry to absolute_location. Since the absolute_location
             *   is an indication of where the entry-to-be-added should be placed (reshuffle
             *   start point) for a forward shuffle (Thus, pointing on the first entry whose
             *   prio is larger than the entry-to-be-added prio).
             *   Example:
             *   Assume entry-to-be-added prio is P2 (P1 <= P2 < P3).
             *   +---+
             *   | -- |
             *   | P1 |
             *   | P3 | <- absolute_location
             *   | -- |
             *   +===+
             *
             *   If we're doing a forward shuffle, we will start from P3 entry(absolute_location),
             *   but if we're doing a backward shuffle, we need to rather start from P1 entry
             *                                                                        - -
             */
     ...
     *   else
     *   {
     *     In case no empty location was found, it's either we failed defragging,
     *     or 'hopefully' the bank addition will do the trick.
     *     So, we exit silently and let the second location_find() invocation that
     *     will happen after this function finishes, determine whether it can find
     *     a suitable place for the new entry or not.
     *   }
     *                                                                        - -
     */
    ...
    int bank_id)
{
    ...
    if (nof_free_backward > 0)
    {
        ...
        /**
         *  -  -
         *   In order to move entries backward, we first find the first empty space that suits the handler,
         *   afterwards we get the entry for the handler in the bank that its index equals the number of
         *   free spaces available. If number of free space available is bigger than number of handler's
         *   entries inside the bank, the last entry in the bank is returned. Afterwards, the move operation
         *   is started.
         *                                                                                  -  -
         */
        SHR_IF_ERR_EXIT(dnx_field_tcam_location_algo_empty_location_find
    ...
    int bank_ids[])
{
    ...
    for (bank_idx = 0; bank_idx < nof_banks; bank_idx++)
    {
        /** Evacuate all entries on current bank from all cores */
        ...
        /** Complete removal of bank (no entry on any core left) */
