    ...
    bcm_port_extender_mapping_info_t * mapping_info)
{
    ...
    if (type == bcmPortExtenderMappingTypePonTunnel)
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "PON_SUBPORT_ID of PON out range[0~3]!");
        }
    }
    else if (type == bcmPortExtenderMappingTypePortVlan)
    {
        ...
        max_value = 0;
        SHR_BITSET_RANGE(&max_value, 0, 6);
        if (mapping_info->phy_port > max_value)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "PTC our of range[0~%d]!", max_value);
        }
    }
    else
    {
    ...
    bcm_extender_port_t * extender_port)
{
    ...
    {
        dbal_entry_handle_value_field16_get(unit, entry_handle_id, DBAL_FIELD_ECID_OR_PON_TAG, INST_SINGLE,
    ...
    bcm_extender_port_t * extender_port)
{
    ...
    if (ingress)
    {
        SHR_IF_ERR_EXIT(bcm_dnx_extender_port_ingress_add(unit, global_lif, local_in_lif, 0, extender_port));
    }
