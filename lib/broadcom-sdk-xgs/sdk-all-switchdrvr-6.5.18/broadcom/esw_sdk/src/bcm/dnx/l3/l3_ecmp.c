...
#define L3_ECMP_GET_OFFSET_FROM_MEMBERS_TBL_ADDR(members_address) (members_address / ecmp_consistent_types_attr[L3_ECMP_CONSISTENT_TABLE_SMALL].nof_rows_in_members_table)
/*
    ...
    uint32 *nof_valid_entries_in_table)
{
    ...
    uint32 nof_entries = (L3_ECMP_CONSISTENT_TABLE_LARGE_NOF_ENTRIES / ecmp_group_size) * ecmp_group_size;
    /*
    ...
    uint32 *nof_entries_used)
{
    ...
    if (consistent_table_type == L3_ECMP_CONSISTENT_TABLE_LARGE)
    {
        dnx_l3_ecmp_consistent_large_table_nof_valid_entries_get(unit, ecmp_group_size, nof_entries_used);
    }
    else
    {
        *nof_entries_used = ecmp_consistent_types_attr[consistent_table_type].nof_entries;
    }
    ...
    int *member_count)
{
    ...
    offset = L3_ECMP_GET_OFFSET_FROM_MEMBERS_TBL_ADDR(members_address);
    /*
    ...
    nof_memb_in_row = ecmp_consistent_types_attr[consistent_type].nof_members_in_table_row;
    /*
    ...
    dnx_l3_ecmp_profile_t * ecmp_profile)
{
    ...
    else
    {
         ...
         */
        address =
            (address + tp_table_index) * ecmp_consistent_types_attr[consistent_table_type].nof_rows_in_members_table;
    ...
    bcm_if_t intf)
{
    ...
    if (profile_data->ecmp_mode == DBAL_ENUM_FVAL_ECMP_MODE_MULTIPLY_AND_DIVIDE)
    {
        if (profile_data->group_size == L3_ECMP_MAX_NOF_ECMP_MEMBERS)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "The ECMP already contains the maximal number of members %d\n",
                         L3_ECMP_MAX_NOF_ECMP_MEMBERS);
        }
    ...
    uint32 *table_address)
{
     ...
     */
    *table_address = chm_resource_offset / ecmp_consistent_types_attr[consistent_table_type].nof_resources;
    ...
    consistent_hashing_calendar_t * calendar)
{
    ...
    nof_memb_in_row = ecmp_consistent_types_attr[calendar->profile_type].nof_members_in_table_row;
    /*
    ...
    bcm_l3_ecmp_tunnel_priority_map_info_t * map_info)
{
    ...
    if (_SHR_IS_FLAG_SET(map_info->l3_flags, ~(BCM_L3_WITH_ID | BCM_L3_2ND_HIERARCHY | BCM_L3_3RD_HIERARCHY)))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "One or more of the used flags 0x%x aren't supported (supported flags are 0x%x).",
                     map_info->l3_flags, L3_ECMP_SUPPORTED_BCM_L3_FLAGS);
    }
    ...
    if (map_profile_alloc_expected || _SHR_IS_FLAG_SET(map_info->l3_flags, BCM_L3_WITH_ID))
    {
        ...
        if (is_allocated && _SHR_IS_FLAG_SET(map_info->l3_flags, BCM_L3_WITH_ID))
        {
