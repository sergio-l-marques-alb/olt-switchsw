    ...
    int unit)
{
    ...
    wrong_learn_command_bug = dnx_data_l2.hw_bug.feature_get(unit, dnx_data_l2_hw_bug_learn_events_wrong_command);
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_JR_CMD_TYPE_INSERT, INST_SINGLE,
                                 wrong_learn_command_bug ? DBAL_ENUM_FVAL_OLP_COMMAND_LEARN :
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_JR_CMD_TYPE_LEARN, INST_SINGLE,
                                 wrong_learn_command_bug ? DBAL_ENUM_FVAL_OLP_COMMAND_LEARN_UNUSED :
    ...
    dnx_vsi_learning_profile_info_t * vsi_learning_profile_info)
{
    ...
    uint8 lel_use_insert_cmd;
    uint8 distribution_mc;
    ...
    if (dnx_data_l2.general.feature_get(unit, dnx_data_l2_general_learning_use_insert_cmd))
    {
        ...
        if ((vsi_learning_profile_info->limit == -1) || (vsi_learning_profile_info->limit == predef_max_limit))
        {
            lel_use_insert_cmd = 0;
            distribution_mc = 0;
        }
        else
        {
            lel_use_insert_cmd = 1;
            distribution_mc = 1;
        }

        SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_OLP_GENERAL_CONFIGURATION, entry_handle_id));
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LEL_USE_INSERT_CMD, INST_SINGLE,
                                     lel_use_insert_cmd);
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));

        SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_OLP_DSP_EVENT_ROUTE, entry_handle_id));
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_EVENT_FORWARDING_PROFILE,
                                   vsi_learning_profile_info->event_forwarding_profile);
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_OLP_COMMAND, DBAL_ENUM_FVAL_OLP_COMMAND_INSERT);
        dbal_entry_key_field32_range_set(unit, entry_handle_id, DBAL_FIELD_MACT_APP_DB_INDEX, 0, 1);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_OLP_DISTRIBUTION_MC, INST_SINGLE,
                                     distribution_mc);
    ...
    dnx_event_forwarding_profile_info_t * event_forwarding_profile_info)
{
    ...
    SHR_IF_ERR_EXIT(algo_l2_db.event_forwarding_profile.profile_data_get
                    (unit, 0, &ref_count, (void *) &old_event_forwarding_profile_info));
    ...
    int unit)
{
    ...
    learn_limit_bug = dnx_data_l2.hw_bug.feature_get(unit, dnx_data_l2_hw_bug_learn_limit);

    if ((learn_limit_bug == 1) ||
        ((dnx_data_field.init.wa_j1_learning_get(unit)) &&
    ...
    {
        /** WA for these bugs can work only with one command in DSP  */
        max_cmd_in_dsp = 1;
    }
    ...
    int lpkgv_no_outlif)
{
    ...
    /** Learn data[4:0] to destination first part */
    currect_offset -= 5;
    learn_ffc_instruction = BUILD_LEARN_PAYLOAD_FFC(currect_offset, (5 - 1), LEARN_PAYLOAD_SOURCE_LEARN_DATA_OFFSET);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LEARN_FFC_INSTRUCTION, 4, learn_ffc_instruction);
    ...
    int lpkgv_one_outlif)
{
    ...
    /** Learn data[44:47] = 0. Skip this copy */
    currect_offset -= 4;
    ...
    int unit)
{
     ...
     */
    SHR_IF_ERR_EXIT(dnx_l2_init_symmetric_lif_optimized(unit, lpkgv_hw_value_single_outlif));

    /*
     * MACT Payload Context for Optimized Asymmetric Global In-LIF
     */
    SHR_IF_ERR_EXIT(dnx_l2_init_asymmetric_lif_optimized(unit, lpkgv_hw_value_single_outlif));
