    ...
    imb_diag_synce_config_t * synce)
{
    ...
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    /*
     * Config SyncE Per CDU Control - ref clock lane
     */
    /*
     * alloc DBAL table handle
     */
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_CLOCK_CHAIN_SEL, INST_SINGLE, synce->clk_chain_sel);
    ...
    imb_diag_synce_config_t * synce)
{
    ...
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_CLOCK_CHAIN_SEL, INST_SINGLE, &synce->clk_chain_sel);
    /*
     * commit request
     */
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));
    ...
    uint32 *ch0_out_remainder)
{
    ...
    if (dnx_data_pll.pll3.nof_pll3_get(unit) > 0)
    {
        ...
        ref_clk = dnx_data_pll.pll3.nif_pll_get(unit, pll_index)->in_freq;
    ...
    else
    {
        SHR_IF_ERR_EXIT(imb_diag_ref_clk_int_reminder_get
                        (unit, dnx_data_pll.pll3.nif_pll_get(unit, 0)->out_freq, (int *) ch0_out_int,
    ...
    uint32 *ch0_out_remainder)
{
    ...
    if (dnx_data_pll.pll3.nof_pll3_get(unit) > 0)
    {
        ...
        ref_clk = dnx_data_pll.pll3.fabric_pll_get(unit, pll_index)->in_freq;
    ...
    {
        SHR_IF_ERR_EXIT(imb_diag_ref_clk_int_reminder_get
                        (unit, dnx_data_pll.pll3.fabric_pll_get(unit, 0)->out_freq, (int *) ch0_out_int,
                         (int *) ch0_out_remainder));
    }
...
/**
 * \brief - provide the nif PHY measurment
 * 
 * \param [in] unit - chip unit id
    ...
    soc_dnxc_port_phy_measure_t * phy_measure)
{
     ...
     */
    SHR_IF_ERR_EXIT(soc_dnx_nif_synce_enable_get(unit, ethu_info.core, &synce_enable_default));
    synce_state_read = 1;
    /*
     * Enable syncE
     */
    SHR_IF_ERR_EXIT(soc_dnx_nif_synce_enable_set(unit, ethu_info.core, 1));
    /*
     * Save syncE configuration to restore at end of function 
     */
...
exit:
    if (unit < SOC_MAX_NUM_DEVICES)
    {
        ...
        if (synce_state_read)
        {
            if ((_func_rv = soc_dnx_nif_synce_enable_set(unit, ethu_info.core, synce_enable_default)) < 0)
            {
