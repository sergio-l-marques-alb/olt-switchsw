    ...
    uint32 is_oam)
{
    ...
    /** Delete entry again, in case an event was generated */
    SHR_IF_ERR_EXIT(dbal_entry_clear(unit, entry_handle_id, DBAL_COMMIT));
    ...
    uint16 *nof_hw_scans)
{
    ...
    /*
     * Calculate RMEP DB scan interval in nanoseconds
     */
    ...
    int unit)
{
    ...
    scanner_limitation = dnx_data_oam.hw_bug.feature_get(unit, dnx_data_oam_hw_bug_oamp_scanner_limitation);
    ...
    dnx_oamp_ccm_endpoint_extra_data_t * extra_data)
{
    ...
    if (_SHR_IS_FLAG_SET(extra_data->flags, DNX_OAMP_CCM_ENDPOINT_EXTRA_DATA_DMAC))
    {
        ...
        if (_SHR_IS_FLAG_SET(extra_data->flags, DNX_OAMP_CCM_ENDPOINT_EXTRA_DATA_MAID48))
        {
            sal_memcpy(extra_header_data + DNX_LMDM_FLEXIBLE_DA_LENGTH, extra_data->maid48, BCM_OAM_GROUP_NAME_LENGTH);
            nof_bits += DNX_OAM_OAMP_NOF_MAID_BITS;
        }
    ...
    uint16 oam_id)
{
    dnx_oam_oamp_lm_dm_search_t search_params;
    dnx_oam_oamp_mep_db_args_t *oamp_params = NULL;
    uint32 lm_db_entry, lm_stat_entry, flex_lm_dm_entry, second_mep_db_entry, next_bank_offset;
    SHR_FUNC_INIT_VARS(unit);
    ...
    oam_oamp_profile_hw_data_t * mep_hw_profiles_delete_data)
{
   ...
   /** Map bcm ep type to mep db type */
    ...
    if (_SHR_IS_FLAG_SET(endpoint_info->flags, BCM_OAM_ENDPOINT_REPLACE))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_oam_oamp_ccm_interval_to_ccm_period(unit, mep_db_entry->ccm_interval, &prev_ccm_period));
     ...
     */
    DNX_OAM_SET_LMM_OFFSET(mep_db_entry->mep_type, mep_db_entry->nof_vlan_tags, is_dual_ended_lm,
                           mep_profile.lmm_offset);
    DNX_OAM_SET_LMR_OFFSET(mep_db_entry->mep_type, mep_db_entry->nof_vlan_tags, is_dual_ended_lm,
    ...
    uint16 *crc_2)
{
    ...
    if (crc_1 != NULL)
    {
        dbal_entry_handle_value_field32_get(unit, entry_handle_id, DBAL_FIELD_CHECK_CRC_VALUE_1, INST_SINGLE,
                                            &check_crc_value_1);
        *crc_1 = (uint16) check_crc_value_1;
    }
    if (crc_2 != NULL)
    {
        dbal_entry_handle_value_field32_get(unit, entry_handle_id, DBAL_FIELD_CHECK_CRC_VALUE_2, INST_SINGLE,
                                            &check_crc_value_2);
    ...
    uint16 calculated_crc)
{
    ...
    SHR_IF_ERR_EXIT(dnx_oamp_mep_db_extra_data_set
                    (unit, OAM_ID_TO_MEP_DB_ENTRY(group_info->group_name_index),
                     DNX_OAM_OAMP_NOF_MAID_BITS,
                     extra_header_data, DBAL_DEFINE_OAM_OPCODE_CCM, calculated_crc, 0,
                     _SHR_IS_FLAG_SET(group_info->flags, BCM_OAM_GROUP_REPLACE)));
exit:
    SHR_FREE(extra_header_data);
    SHR_FUNC_EXIT;
}
