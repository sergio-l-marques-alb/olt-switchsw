 ...
 */
#define STIF_MGMT_INSTANCE_INVALID (-1)
    ...
    int unit)
{
    ...
    for (stif_instance_id = 0; stif_instance_id < dnx_data_stif.port.max_nof_instances_get(unit); stif_instance_id++)
    {
        ...
        {
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_STIF_INSTANCE_OUTPUT_INT_QSIZE_RPRT_PERIOD,
                                         INST_SINGLE, dnx_data_stif.report.qsize_idle_report_period_invalid_get(unit));
            if (dnx_data_device.general.nof_cores_get(unit) > 1)
            {
                dbal_entry_value_field32_set(unit, entry_handle_id,
                                             ...
                                             dnx_data_stif.report.qsize_idle_report_period_invalid_get(unit));
            }
        }
        else
        {
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_STIF_INSTANCE_OUTPUT_INT_QSIZE_RPRT_PERIOD,
                                         INST_SINGLE, dnx_data_stif.report.qsize_idle_report_period_get(unit));
            if (dnx_data_device.general.nof_cores_get(unit) > 1)
            {
                dbal_entry_value_field32_set(unit, entry_handle_id,
                                             DBAL_FIELD_STIF_INSTANCE_OUTPUT_EXT_QSIZE_RPRT_PERIOD, INST_SINGLE,
                                             dnx_data_stif.report.qsize_idle_report_period_get(unit));
            }
        }
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    }
...
{
    uint8 is_init;
    int core_id, nof_cores, nof_instances, instance_id;
    SHR_FUNC_INIT_VARS(unit);
    ...
    if (is_init == FALSE)
    {
        ...
        for (core_id = 0; core_id < nof_cores; core_id++)
        {
            ...
            {
                SHR_IF_ERR_EXIT(dnx_stif_db.instance.logical_port.set(unit, core_id, instance_id, -1));
            }
        }
    }
exit:
    ...
    uint32 init)
{
    ...
    /** HW Bug - the reset of the flow-control flip-flops is the
     *  init of output port - SW WA is to never init output port
     *  0 (instance 0) */
...
static shr_error_e
dnx_stif_init_verify(
    int unit)
{
    SHR_FUNC_INIT_VARS(unit);
    if ((dnx_data_stif.config.feature_get(unit, dnx_data_stif_config_stif_enable) == TRUE) &&
        (dnx_data_stif.config.stif_report_mode_get(unit) == dnx_stat_stif_mgmt_report_mode_billing) &&
        (dnx_data_stif.config.stif_report_size_egress_get(unit) == DBAL_ENUM_FVAL_STIF_BILLING_REPORT_SIZE_96_BITS))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "billing mode, 96 bits egress record size is not allowed \n");
    }
exit:
    SHR_FUNC_EXIT;
}
    ...
    int unit)
{
    ...
    /** FIRST: Reset all STIF related FIFOs */
     /** enter SIF in reset */
    SHR_INVOKE_VERIFY_DNX(dnx_stif_init_verify(unit));
    ...
    int *instance_id)
{
    ...
    *instance_core = port_core;
    ...
    for (instance = 0; instance < dnx_data_stif.port.max_nof_instances_get(unit); instance++)
    {
        ...
        {
            /** if there is an instance that is not connected - save it */
     ...
     *  to the last found free instance to the same port core */
...
dnx_stif_mgmt_source_mapping_set(
    ...
    bcm_port_t port)
{
    int report_mode, instance_id = STIF_MGMT_INSTANCE_INVALID;
    ...
    if (port == BCM_PORT_INVALID)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_stif_mgmt_source_mapping_get(unit, 0, source, &port, &instance_id, &port_core));

        /** if instance invalid, it means that that source is not connect at all, so no need to disconnect it */
        if (instance_id != STIF_MGMT_INSTANCE_INVALID)
        {
            SHR_IF_ERR_EXIT(dnx_stif_mgmt_source_instance_id_disconnect(unit, port, source, port_core, instance_id));
        }
    }
    else
    {
    ...
    int *instance_core)
{
    ...
    /** go over the instances - check if the given source is
     *  connected to one of it - if it is - return the port, if
     *  it is not - return port invalid */
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, instance_core_idx)
    {
        for (instance_idx = 0; instance_idx < dnx_data_stif.port.max_nof_instances_get(unit); instance_idx++)
        {
            ...
            if (is_connected)
            {
                SHR_IF_ERR_EXIT(dnx_stif_db.instance.logical_port.get(unit, instance_core_idx, instance_idx, port));
                if (instance_id != NULL)
                {
    ...
    if (is_connected == FALSE)
    {
        *port = BCM_PORT_INVALID;
        if (instance_id != NULL)
        {
            *instance_id = STIF_MGMT_INSTANCE_INVALID;
        }
    }
exit:
    SHR_FUNC_EXIT;
        ...
        if (mask_get != 0)
        {
                 ...
                 DBAL_ENUM_FVAL_STIF_BILLING_REPORT_SIZE_96_BITS))
            {
                elements_array_get[elements_count].shift =
                    shift_get + dnx_data_stif.report.billing_enq_ing_size_wa_shift_get(unit);
            }
            else
            {
    ...
    bcm_stat_stif_record_format_element_t * record_format_elements)
{
    ...
    SHR_IF_ERR_EXIT(dbal_table_clear(unit, DBAL_TABLE_STIF_CGM_BILLING_STAMP));
    ...
    for (element = 0; element < nof_elements - 1; element++)
    {
        ...
        else
        {
            ...
            if ((dnx_data_stif.config.stif_report_size_ingress_get(unit) ==
                 DBAL_ENUM_FVAL_STIF_BILLING_REPORT_SIZE_96_BITS))
            {
                if (dnx_data_stif.report.billing_enq_ing_size_wa_shift_get(unit) > current_shift_place)
                {
                ...
                {
                    final_shift_place =
                        current_shift_place - dnx_data_stif.report.billing_enq_ing_size_wa_shift_get(unit);
                }
            }
            else
            {
