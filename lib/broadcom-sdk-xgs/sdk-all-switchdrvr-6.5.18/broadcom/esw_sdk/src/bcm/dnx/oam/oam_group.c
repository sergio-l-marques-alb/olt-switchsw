    ...
    bcm_oam_group_info_t * group_info)
{
    ...
    if (_SHR_IS_FLAG_SET(group_info->flags, BCM_OAM_GROUP_REPLACE))
    {
        if ((_SHR_IS_FLAG_SET(group_info->flags, BCM_OAM_GROUP_WITH_ID) == 0) ||
            (_SHR_IS_FLAG_SET(group_info->flags, BCM_OAM_GROUP_FLEXIBLE_MAID_48_BYTE) == 0))
        {
    ...
    bcm_oam_group_info_t * group_info)
{
    ...
    SHR_IF_ERR_EXIT(dnx_oam_group_handle_sw_icc_data(unit, group_info->id, DNX_OAMP_OAM_GROUP_CREATE, hw_write_data));
