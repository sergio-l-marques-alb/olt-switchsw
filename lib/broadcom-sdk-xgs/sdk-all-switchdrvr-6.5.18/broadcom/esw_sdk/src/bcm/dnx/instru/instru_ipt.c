...
/**
 * \brief - Define all possible metadata flags for Tail-Edit
 */
#define DNX_INSTRU_IPT_TAIL_EDIT_METADATA_FLAGS_ALL (BCM_INSTRU_IPT_METADATA_FLAG_INGRESS_TOD |      \
...
/**
 * \brief - Define all possible metadata flags for INT
 */
#define DNX_INSTRU_IPT_INT_METADATA_FLAGS_ALL (BCM_INSTRU_IPT_METADATA_FLAG_INGRESS_TOD |              \
        BCM_INSTRU_IPT_METADATA_FLAG_INGRESS_SYS_PORT | BCM_INSTRU_IPT_METADATA_FLAG_INGRESS_DEV_ID |  \
        BCM_INSTRU_IPT_METADATA_FLAG_INGRESS_RESERVED | BCM_INSTRU_IPT_METADATA_FLAG_EGRESS_TOD |      \
        BCM_INSTRU_IPT_METADATA_FLAG_EGRESS_SYS_PORT | BCM_INSTRU_IPT_METADATA_FLAG_EGRESS_DEV_ID |    \
        BCM_INSTRU_IPT_METADATA_FLAG_SWITCH_ID | BCM_INSTRU_IPT_METADATA_FLAG_EGRESS_RESERVED |        \
    ...
    bcm_instru_ipt_t * config)
{
    ...
    /** verify flags */
    supported_flags = BCM_INSTRU_IPT_METADATA_FLAG_INGRESS_TOD | BCM_INSTRU_IPT_METADATA_FLAG_INGRESS_SYS_PORT |
        BCM_INSTRU_IPT_METADATA_FLAG_INGRESS_DEV_ID | BCM_INSTRU_IPT_METADATA_FLAG_INGRESS_RESERVED |
        BCM_INSTRU_IPT_METADATA_FLAG_EGRESS_TOD | BCM_INSTRU_IPT_METADATA_FLAG_EGRESS_SYS_PORT |
        BCM_INSTRU_IPT_METADATA_FLAG_EGRESS_DEV_ID | BCM_INSTRU_IPT_METADATA_FLAG_SWITCH_ID |
        BCM_INSTRU_IPT_METADATA_FLAG_EGRESS_RESERVED | BCM_INSTRU_IPT_METADATA_FLAG_QUEUE_INFO;
    ...
    bcm_instru_ipt_t * config)
{
    ...
    /** verify flags */
    supported_flags = DNX_INSTRU_IPT_TAIL_EDIT_METADATA_FLAGS_ALL;
    ...
    if (_SHR_IS_FLAG_SET(config->metadata_flags, BCM_INSTRU_IPT_METADATA_FLAG_QUEUE_INFO))
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "When queue info is required in Tal-Edit profile, then the following flags must "
    ...
    bcm_instru_ipt_t * config)
{
    ...
    /** verify flags */
    supported_flags = DNX_INSTRU_IPT_INT_METADATA_FLAGS_ALL;
    SHR_MASK_VERIFY(flags, supported_flags, _SHR_E_PARAM, "non supported flags specified.\n");
    ...
    bcm_instru_ipt_t * config)
{
    ...
    /** commit to HW */
    {
        ...
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_METADATA_BITMAP, INST_SINGLE, metadata_bitmap);

        /** set value - metadata size */
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_METADATA_SIZE, INST_SINGLE, metadata_size);
    ...
    int *switch_id)
{
    ...
    /** null check */
    SHR_NULL_CHECK(switch_id, _SHR_E_PARAM, "switch_id");

    /** create handle */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_INSTRU_IPT_GENERAL_CONFIG, &entry_handle_id));

    /** request Switch ID */
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_SWITCH_ID, INST_SINGLE, &field32);
    ...
    int switch_id)
{
    ...
    /** set value - switch ID */
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_SWITCH_ID, INST_SINGLE, switch_id);
    ...
    int unit)
{
    ...
    /** By default, set all IPT profiles to include all possible metadata */
    {
        ...
        for (ipt_profile = 0; ipt_profile < profile_table_info->key_size[0]; ipt_profile++)
        {
            ...
            if (profile_info->type == bcmInstruIptNodeTypeTail)
            {
                config.metadata_flags = DNX_INSTRU_IPT_TAIL_EDIT_METADATA_FLAGS_ALL;
            }
            /** INT */
            else if (profile_info->type == bcmInstruIptNodeTypeIntOverVxlanFirst ||
                     profile_info->type == bcmInstruIptNodeTypeIntOverVxlanIntermediate)
            {
                config.metadata_flags = DNX_INSTRU_IPT_INT_METADATA_FLAGS_ALL;
            }
            else
            {
                continue;
