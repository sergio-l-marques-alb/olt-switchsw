...
#define DNX_OAM_CCM_ENDPOINT_EXTRA_DATA_LENGHT_CALC(is_maid48) (is_maid48 ? 3 : 1)
...
/** Indicate setting MAID48 in extra data   */
...
typedef struct
{
    ...
    /** Highest one-way delay measurement (1DM) - read only */
    uint8 max_delay_one_way[ONE_WAY_DELAY_SIZE];

    /** Lowest one-way delay measurement (1DM) - read only */
    uint8 min_delay_one_way[ONE_WAY_DELAY_SIZE];

    /** Last value of one-way delay measurement (1DM) - read only */
