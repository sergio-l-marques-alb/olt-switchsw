    ...
    int enable)
{
     ...
     */
    rmc_id = enable ? ((rmc->prd_priority & BCM_PORT_F_PRIORITY_0) ? rmc->rmc_id : 0) : 0;
    ...
    rmc_id = enable ? ((rmc->prd_priority & BCM_PORT_F_PRIORITY_1) ? rmc->rmc_id : 0) : 0;
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_PRD_PRIORITY, 1);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_RMC, INST_SINGLE, rmc_id);
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));

    rmc_id = enable ? ((rmc->prd_priority & BCM_PORT_F_PRIORITY_2) ? rmc->rmc_id : 0) : 0;
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_PRD_PRIORITY, 2);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_RMC, INST_SINGLE, rmc_id);
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));

    rmc_id = enable ? ((rmc->prd_priority & BCM_PORT_F_PRIORITY_3) ? rmc->rmc_id : 0) : 0;
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_PRD_PRIORITY, 3);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_RMC, INST_SINGLE, rmc_id);
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));

    rmc_id = enable ? ((rmc->prd_priority & BCM_PORT_F_PRIORITY_TDM) ? rmc->rmc_id : 0) : 0;
...
/**
 * \brief - get number of ports from a FEU on the specified
 *        scheduler
 *
 * \param [in] unit - chip unit id
 ...
 */
int
imb_feu_internal_ilkn_burst_config_set(
    int unit,
    int burst_size)
{
    uint32 entry_handle_id;
    ...
    uint64 *val)
{
    ...
    result =
        dbal_tables_field_info_get_no_err(unit, DBAL_TABLE_TINY_MAC_MIB_COUNTERS_RX, field_id, FALSE, 0, INST_SINGLE,
                                          &field_info);
    if (result == _SHR_E_NONE)
    {
        table_id = DBAL_TABLE_TINY_MAC_MIB_COUNTERS_RX;
    }
    else
    {
        table_id = DBAL_TABLE_TINY_MAC_MIB_COUNTERS_TX;
    }
    /*
     * alloc DBAL table handle
     */
