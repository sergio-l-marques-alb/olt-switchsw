    ...
    int mode)
{
    ...
    {
        /** means FMQ handle was taken by the user using allocation API, so user is required to deallocate */
    ...
    int mode)
{
    ...
    if (is_enhanced && mode == DNX_COSQ_FMQ_SCH_MODE_BASIC)
    {
        ...
        /*
         * When switching back to simple scheduling mode,
         * need to make sure that the first queue quartet is saved for FMQ.
         */
    ...
    else if (is_enhanced == FALSE && mode == DNX_COSQ_FMQ_SCH_MODE_ENHANCED)
    {
         ...
         */
        SHR_IF_ERR_EXIT(dnx_cosq_ipq_queue_deallocate(unit, base_queue));
    }
