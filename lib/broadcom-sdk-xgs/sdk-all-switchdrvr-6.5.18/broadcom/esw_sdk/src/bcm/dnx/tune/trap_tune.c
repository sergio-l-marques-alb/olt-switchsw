    ...
    int unit)
{
    ...
    uint8 ip_and_l4_traps_en = FALSE;
    int system_headers_mode;
    SHR_FUNC_INIT_VARS(unit);

    unknown_da_enabled = dnx_data_trap.erpp.feature_get(unit, dnx_data_trap_erpp_unknown_da_trap_en);
    ip_and_l4_traps_en = dnx_data_trap.erpp.feature_get(unit, dnx_data_trap_erpp_ip_4_6_l4_traps_enabled);
    ...
    if (ip_and_l4_traps_en == TRUE)
    {
        ...
        SHR_IF_ERR_EXIT(bcm_rx_trap_action_profile_set(unit, 0, bcmRxTrapEgIpv6MulticastDestination, trap_gport_drop));
