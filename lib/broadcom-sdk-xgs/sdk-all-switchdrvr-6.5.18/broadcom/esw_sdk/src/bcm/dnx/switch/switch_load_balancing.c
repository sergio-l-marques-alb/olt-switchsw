    ...
    int unit)
{
    ...
    }
    {
        ...
        mpls_identification.mpls_identification |= SAL_BIT(DBAL_ENUM_FVAL_LAYER_TYPES_MPLS_UNTERM);
    ...
    dnx_switch_hash_header_attr_t * header_info)
{
    ...
    switch (type)
    {
        ...
        case bcmSwitchHashL4InnerField:
        {
            ...
            is_outer = (type == bcmSwitchHashL4OuterField);
            header_info->field_flags[field_counter].flag_fields[header_info->field_flags[field_counter].nof_fields++] =
                is_outer ? DBAL_FIELD_L4TCPOUTERPORTSENABLE : DBAL_FIELD_L4TCPINNERPORTSENABLE;
            header_info->field_flags[field_counter].flag_fields[header_info->field_flags[field_counter].nof_fields++] =
                is_outer ? DBAL_FIELD_L4UDPOUTERPORTSENABLE : DBAL_FIELD_L4UDPINNERPORTSENABLE;
            if (is_outer)
            {
                /** SCTP has only an outer option */
                header_info->field_flags[field_counter].flag_fields[header_info->
                                                                    field_flags[field_counter].nof_fields++] =
                    DBAL_FIELD_L4SCTPOUTERPORTSENABLE;
            }
            header_info->field_flags[field_counter++].flag = BCM_HASH_NON_SYMMETRICAL;
            header_info->field_flags[field_counter].flag_fields[header_info->field_flags[field_counter].nof_fields++] =
                is_outer ? DBAL_FIELD_L4OUTERSYMMETRIC : DBAL_FIELD_L4INNERSYMMETRIC;
...
/**
 * \brief - Set the bits of a header that can be part of the construction of an LB key according to the user masking option.
 *
 * \param [in] unit - Relevant unit.
    ...
    uint32 mask)
{
    ...
    for (flag_iter = 0; flag_iter < header_info.nof_field_flags; flag_iter++)
    {
         ...
         * They should now only be set to 1 as setting one of them to 1 and the other to zero will create an
         * affect as they share HW bits configuration.
         */
...
/**
 * \brief - return the mask option that determine the bits of the header that can be part of the construction of an LB key.
 *
 * \param [in] unit - Relevant unit.
 * \param [in] type - The type of header to be masked.
    ...
    uint32 *mask)
{
    ...
    {
        value = 0;
        /** As a single flag affect NOF DBAL fields, only the first field can be use to get the value as the rest are configured the same. **/
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                        (unit, entry_handle_id, header_info.field_flags[flag_iter].flag_fields[0], INST_SINGLE,
                         &value));
        flags_mask |= (value != 0) ? header_info.field_flags[flag_iter].flag : 0;
    }

    /*
     * If the non-symmetrical field is on the symmetrical field should be added as they are
     * set together on the "set" API.
     */
    if (_SHR_IS_FLAG_SET(flags_mask, BCM_HASH_NON_SYMMETRICAL))
    {
        flags_mask |= header_info.symmetrical_flag;
    }
