    ...
    imb_specific_create_info_t * imb_specific_info)
{
    ...
    if (!sw_state_is_warm_boot(unit))
    {
        ...
        if (dnx_data_nif.eth.feature_get(unit, dnx_data_nif_eth_rmc_flush_support))
        {
             ...
             */
            SHR_IF_ERR_EXIT(imb_ethu_sch_cnt_dec_thres_hw_set(unit, cdu_id, 0));
        }
        /*
        ...
        {
            /*
             * There is a bug in the Q2A CDU threshold register where the max value of the threshold for the CDU unit
             * can be 0x3fff (14b) while the physical register size is 15b` and default value is 0x4000. We need to
             * re-stamp this register at init with the maximum allowed value. 
             */
    ...
    int is_set)
{
    ...
    switch (type)
    {
        ...
        case BCM_PORT_PHY_CONTROL_TX_POLARITY:
        {
            if (is_set)
            {
                SHR_ERR_EXIT(_SHR_E_UNAVAIL, "PHY control type %d set is not supported on Blackhawk PHY.\r\n", type);
            }
            break;
        }
 ...
 *
 * \remarks
 *   * needed only because of bug described in
 * dnx_data_nif.hw_workarounds.feature_get(unit, dnx_data_nif_hw_workarounds_pm_mode_0122_invalid
 * \see
 *   * None
 */
int
    ...
    bcm_pbmp_t phys)
{
    ...
    if (dnx_data_nif.hw_workarounds.feature_get(unit, dnx_data_nif_hw_workarounds_pm_mode_0122_invalid))
    {
