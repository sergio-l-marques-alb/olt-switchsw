    ...
    uint32 *optimized_result_size)
{
    ...
    {
        /** Optimized result is not enabled or table is not used for forwarding */
        *regular_result_size = table->max_payload_size;
        *optimized_result_size = 0;
    }
    ...
    uint8 opcode_id)
{
    ...
    LOG_VERBOSE(BSL_LOG_MODULE, (BSL_META_U(unit, "Updating opcode %s (%d) with contexts FWD %d and ACL %d\n"),
    ...
    if (opcode_id != configured_opcode_id)
    {
        ...
        if (lookup_bmp && is_standard_image)
        {
            ...
            SHR_ERR_EXIT(_SHR_E_INTERNAL,
                         ...
                         dbal_enum_id_type_to_string(unit, DBAL_FIELD_TYPE_DEF_KBP_FWD_OPCODE, opcode_id), opcode_id);
        }
    ...
    uint8 is_standard_image)
{
    ...
    void *iter;
    uint32 entry_handle_id;
    uint32 fwd_ctx_id[DNX_DATA_MAX_FIELD_KBP_MAX_FWD_CONTEXT_NUM_FOR_ONE_APPTYPE] = { 0 };
    uint32 fwd_ctx_enum_id;
    ...
    RHDATA_ITERATOR(iter, iter_top, "Context")
    {
        ...
        SHR_IF_ERR_EXIT(dbal_enum_type_string_to_id
                        (unit, DBAL_FIELD_TYPE_DEF_FWD2_CONTEXT_ID, ctx_name, &fwd_ctx_enum_id));

        SHR_IF_ERR_EXIT(dbal_fields_enum_hw_value_get
                        (unit, DBAL_FIELD_FWD2_CONTEXT_ID, fwd_ctx_enum_id, &fwd_ctx_id[ctx_index]));

        /*
         * For static/predefines contexts, the FWD context and the ACL context are identical.
         */
        acl_ctx_id[ctx_index] = fwd_ctx_id[ctx_index];

        SHR_IF_ERR_EXIT(kbp_mngr_fwd_context_to_opcode_hw_verify_update
                        (unit, fwd_ctx_id[ctx_index], acl_ctx_id[ctx_index], opcode_id, is_standard_image));

        LOG_VERBOSE(BSL_LOG_MODULE, (BSL_META_U(unit, "opcode %s (%d) related to contexts %s (%d) ACL context %d\n"),
                                     dbal_enum_id_type_to_string(unit, DBAL_FIELD_TYPE_DEF_KBP_FWD_OPCODE, opcode_id),
                                     opcode_id, ctx_name, fwd_ctx_id[ctx_index], acl_ctx_id[ctx_index]));
    ...
    uint8 is_standard_image)
{
    ...
    /** For Ipv4/IPv6 incase the elk application is disabled do not load the lookup db's */
    ...
    /** Simulation case all opcodes are valid always, in real device incase that the customer close the feature for IPVx it
     *  should not load the lookup db's*/
    if ((!SAL_BOOT_PLISIM) || (dnx_data_elk.general.device_type_get(unit)))
    {
        /** Perform FWD applications validation only for standard_1 */
        if (is_standard_image)
        {
                ...
                (opcode_id == DBAL_ENUM_FVAL_KBP_FWD_OPCODE_IPV4_MULTICAST_PRIVATE_W_UC_RPF))
            {
                if (!dnx_data_elk.application.feature_get(unit, dnx_data_elk_application_ipv4))
                {
                    LOG_VERBOSE(BSL_LOG_MODULE,
                                (BSL_META_U
                                 (unit, "Opcode %s (%d) is disabled since IPv4 external fwd is not enabled\n"),
                                 opcode_name, opcode_id));
            /** Ipv4 fwd is disabled in KBP do not load FWD DB's */
                    disable_fwd_db = TRUE;
                }
            }
                ...
                (opcode_id == DBAL_ENUM_FVAL_KBP_FWD_OPCODE_IPV6_MULTICAST_PRIVATE_W_UC_RPF))
            {
                if (!dnx_data_elk.application.feature_get(unit, dnx_data_elk_application_ipv6))
                {
                    LOG_VERBOSE(BSL_LOG_MODULE,
                                (BSL_META_U
                                 (unit, "Opcode %s (%d) is disabled since IPv6 external fwd is not enabled\n"),
                                 opcode_name, opcode_id));

                    /** Ipv6 fwd is disabled in KBP do not load FWD DB's */
                    disable_fwd_db = TRUE;
                }
            }
    ...
    if (!disable_fwd_db)
    {
        ...
        RHDATA_ITERATOR(iter, opcode_node, "AppDB")
        {
            ...
            if (is_standard_image)
            {
                 ...
                 * Lookup 0 (FWD) - size 64b; offset 0b; type FWD;
                 * Lookup 1 (RPF) - size 32b; offset 64b; type RPF;
                 * Lookup 2:7 - will not be set
                 */
    ...
    SHR_IF_ERR_EXIT(kbp_mngr_opcode_to_context_read(unit, opcode_node, opcode_id, is_standard_image));
    ...
    uint8 acl_context)
{
    ...
    kbp_mngr_key_segment_t key_segments = {.nof_bytes = result_size_bytes,.name = "ZERO" };
    ...
    SHR_IF_ERR_EXIT(kbp_mngr_opcode_master_key_segments_add_internal(unit, opcode_id, 1, &key_segments));
    ...
    uint32 smt_id)
{
    ...
    while (!is_end)
    {
        ...
        else
        {
            ...
            if (!dnx_data_elk.application.feature_get(unit, dnx_data_elk_application_ipv4))
            {
                ...
                {
                    SHR_IF_ERR_EXIT(kbp_mngr_pd_order_disable(unit, opcode_fwd_context_id));
                }
            }
            ...
            {
                if ((opcode_id == DBAL_ENUM_FVAL_KBP_FWD_OPCODE_IPV6_MULTICAST_PRIVATE_W_UC_RPF) ||
                    (opcode_id == DBAL_ENUM_FVAL_KBP_FWD_OPCODE_IPV6_UNICAST_PRIVATE_W_UC_RPF))
                {
                    SHR_IF_ERR_EXIT(kbp_mngr_pd_order_disable(unit, opcode_fwd_context_id));
                }
            }
                ...
                if (utilex_bitstream_test_bit(&lookup_bmp, index))
                {
...
#if 0
                    /*
                     * Currently this workaround from KBP SDK for non-split RPF is not required.
                     * In non-split RPF the RPF result is optimized and the payload does not need to be shifted.
                     */
                    if (!kbp_mngr_split_rpf_in_use(unit))
                    {
                        /*
                         * FWD result is 64 bits and RPF is 32 bits.
                         * When FWD and RPF are split, FWD and RPF AD widths are aligned to the result sizes.
                         * In case the FWD and RPF are not split, the FWD and RPF tables are physically the same and
                         * have the same AD width (64 bits). The RPF result needs to be configured with offset in order
                         * to take the correct part of the 32 bits payload.
                         */
                    ...
                    LOG_VERBOSE(BSL_LOG_MODULE,
                                (BSL_META_U(unit, "Setting lookup DB (%d) %s search index %d result location %d\n"),
                                 db_id, db_name, index, result_index));
                    /** Read DB handles from SW state */
                    SHR_IF_ERR_EXIT(KBP_ACCESS.db_handles_info.get(unit, db_id, &db_handle));
                    DNX_KBP_TRY_PRINT(kbp_instruction_add_db(inst_p, db_handle.db_p, result_index), "DB %d", db_id);
                }
            }
    ...
    /** Disable the KBP lookups in case the soc properties of the KBP are disabled. It is done be zeroing the
    ...
    int algo_mode)
{
    ...
    /** DB properties */
    SHR_IF_ERR_EXIT(dnx_kbp_init_device_type_get(unit, &device_type));
    xl_db = dnx_data_elk.application.db_property_get(unit, table_id)->xl_db;
    ...
    for (key_index = 0; key_index < table->nof_key_fields; key_index++)
    {
        ...
        uint32 segment_type = (SHR_BITGET(table->keys_info[key_index].field_indication_bm, DBAL_FIELD_IND_IS_RANGED)) ?
    ...
    int unit)
{
    ...
    while (!is_end)
    {
        ...
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field_arr8_get
                        (unit, entry_handle_id, DBAL_FIELD_DB_NAME, INST_SINGLE, (uint8 *) db_name));
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field_arr32_get
        ...
        if (!sw_state_is_warm_boot(unit))
        {
            ...
            if (clone_db == 0)
            {
                ...
                if (counters_enable)
                {
                    DNX_KBP_TRY_PRINT(kbp_db_set_property(db_handles.db_p, KBP_PROP_ENABLE_COUNTERS, 1), "DB %d",
                                      db_id);
                }
                ...
                if (result_width)
                {
                    DNX_KBP_TRY_PRINT(kbp_ad_db_init
                                      (Kbp_app_data[unit]->device_p, ad_db_id, regular_capacity, result_width,
                                       &db_handles.ad_db_p[DNX_KBP_AD_DB_INDEX_REGULAR]), "DB %d AD_DB %d", db_id,
                                      ad_db_id);
                ...
                if (DNX_KBP_USE_OPTIMIZED_RESULT && opt_result_width)
                {
                    ...
                    DNX_KBP_TRY_PRINT(kbp_ad_db_init
                                      (Kbp_app_data[unit]->device_p, ad_db_id, optimized_capacity, opt_result_width,
                ...
                {
                    DNX_KBP_TRY_PRINT(kbp_ad_db_init
                                      (Kbp_app_data[unit]->device_p, ad_db_id, 32 * 1024, 0,
                                       &db_handles.ad_db_zero_size_p), "DB %d AD DB %d", db_id, ad_db_id);
            ...
            DNX_KBP_TRY_PRINT(kbp_key_init(Kbp_app_data[unit]->device_p, &key), "DB %d", db_id);
            ...
            /** When large multicast is enabled, set the following db properties to KBP */
                 ...
                 dbal_table_id == DBAL_TABLE_KBP_IPV6_MULTICAST_TCAM_FORWARD))
            {
                ...
                DNX_KBP_TRY_PRINT(kbp_db_set_property(db_handles.db_p, KBP_PROP_MC_DB, KBP_DB_MC_TYPE1), "DB %d",
                                  db_id);
            }
        ...
        {
            /** Warmboot: refresh DB handles */

            if (db_id != DNX_KBP_DUMMY_HOLE_DB_ID)
            {
                LOG_VERBOSE(BSL_LOG_MODULE, (BSL_META_U(unit, "\nRefreshing DB ID (%d) %s\n"), db_id, db_name));
                SHR_IF_ERR_EXIT(KBP_ACCESS.db_handles_info.get(unit, db_id, &db_handles));
                DNX_KBP_TRY_PRINT(kbp_db_refresh_handle
                                  (Kbp_app_data[unit]->device_p, db_handles.db_p, &db_handles.db_p), "DB %d", db_id);

                if (result_width)
                {
                    DNX_KBP_TRY_PRINT(kbp_ad_db_refresh_handle
                                      (Kbp_app_data[unit]->device_p, db_handles.ad_db_p[DNX_KBP_AD_DB_INDEX_REGULAR],
                                       &db_handles.ad_db_p[DNX_KBP_AD_DB_INDEX_REGULAR]), "DB %d AD_DB %d", db_id,
                                      ad_db_id);
                }
                if (DNX_KBP_USE_OPTIMIZED_RESULT && opt_result_width)
                {
                    db_handles.opt_result_size = opt_result_width;
                    DNX_KBP_TRY_PRINT(kbp_ad_db_refresh_handle
                                      (Kbp_app_data[unit]->device_p, db_handles.ad_db_p[DNX_KBP_AD_DB_INDEX_OPTIMIZED],
                                       &db_handles.ad_db_p[DNX_KBP_AD_DB_INDEX_OPTIMIZED]), "DB %d AD_DB %d", db_id,
                                      ad_db_id);
                }
                ...
                {
                    DNX_KBP_TRY_PRINT(kbp_ad_db_refresh_handle
                                      (Kbp_app_data[unit]->device_p, db_handles.ad_db_zero_size_p,
                                       &db_handles.ad_db_zero_size_p), "DB %d AD_DB %d", db_id, ad_db_id);
                }
            }
        }
    ...
    int unit)
{
    ...
    if (!sw_state_is_warm_boot(unit))
    {
        ...
        else if (kbp_mngr_status == DBAL_ENUM_FVAL_KBP_DEVICE_STATUS_CONFIG_UPDATED)
        {
            ...
            LOG_WARN(BSL_LOG_MODULE,
                     (BSL_META_U(unit, "KBP is being reconfigured. All KBP entries will be deleted.\n")));
    ...
    if (!sw_state_is_warm_boot(unit) && kbp_mngr_split_rpf_in_use(unit))
    {
