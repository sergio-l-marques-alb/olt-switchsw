 ...
 *   The decision considers Mesh mode and number of active DTQs.
 */
static shr_error_e
    ...
    uint32 *fabric_2_fc_enable)
{
    ...
    else
    {
        ...
        {
                /*
                 * In case of 3 DTQs, the FMC HP PDQ traffic
            ...
            case 3:
            {
                *fabric_1_fc_enable = TRUE;
                ...
                /*
                 * In case of 2 DTQs, the FMC PDQ traffic
                 * flows towards Fabric1 DTQ
                 */
            case 2:
            {
                *fabric_1_fc_enable = TRUE;
                ...
                /*
                 * In case of single DTQ, the FMC PDQ traffic
    ...
    uint32 *fabric_2_fc_enable)
{
    ...
    else
    {
        ...
        {
                /*
                 * In case of 3 DTQs, the FMC LP PDQ traffic
                 * flows towards Fabric2 DTQ
                 */
            case 3:
            {
 ...
 */
static shr_error_e
dnx_fabric_transmit_pdqs_to_dqcq_fc_th_select(
    int unit)
{
    uint32 entry_handle_id;
    ...
    for (context = 0; context < DBAL_NOF_ENUM_FABRIC_PDQ_CONTEXT_VALUES; ++context)
    {
        ...
        {
            fc_th_select = dnx_data_fabric.transmit.pdq_to_dqcq_fc_th_select_get(unit, pdq_priority, dqcq_priority);
 ...
 */
static shr_error_e
dnx_fabric_transmit_pdqs_to_dqcq_sram_to_dram_fc_th_select(
    int unit)
{
    uint32 entry_handle_id;
    uint32 pdq_priority = 0;
    ...
    uint32 s2d_pdq_size)
{
     ...
     * Select PDQs to DQCQs FC thresholds index
     */
    SHR_IF_ERR_EXIT(dnx_fabric_transmit_pdqs_to_dqcq_fc_th_select(unit));
     ...
     * Select PDQs to DQCQs SRAM to DRAM FC thresholds index
     */
    SHR_IF_ERR_EXIT(dnx_fabric_transmit_pdqs_to_dqcq_sram_to_dram_fc_th_select(unit));
    ...
    int unit)
{
    ...
    /*
     * The leaky bucket works as follows:
     * There are 4 deltas which denotes how many tokens to add/subtract
     * from the leaky bucket.
     * Each 2 bits in the 'select' field represent a fifo level, and mapping
     * between this fifo level and a delta, meaning that for this fifo
     * level need to add the corresponding tokens.
     *
     * The standard configuration is as follows:
     * Delta0 - add +10 to the bucket (2's complement)
     * Delta1 - add +1 to the bucket (2's complement)
     * Delta2 - subtruct -1 from the bucket (2's complement)
     * Delta3 - subtruct -10 from the bucket (2's complement)
     * Delta Select:
     *   AF - Almost Full
     *   AE - Almost Empty
     *     Indication                      | Select | Meaning
     *     -------------------------------------------------------
     *     fifo is not AF/AE (in between)  | 2      | subtruct -1
     *     don't care                      | x      | 
     *     fifo is partially AF            | 1      | add +1
     *     fifo is AF                      | 0      | add +10
     *     don't care                      | x      | 
     *     don't care                      | x      | 
     *     fifo was once AE                | 2      | subtract -1
     *     don't care                      | x      | 
     *     fifo is not stable... AE and AF | 2      | subtruct -1
     *     don't care                      | x      | 
     *     fifo is always AE               | 3      | subtract -10
     *     don't care                      | x      | 
     *     don't care                      | x      | 
     *     don't care                      | x      | 
     */
    ...
    int unit)
{
    ...
    dbal_enum_value_field_fabric_pdq_context_e context;
    /** Get full rate */
    uint32 shaper_rate_per_link_mbps =
        (dnx_data_device.general.core_clock_khz_get(unit) / 1000) * dnx_data_device.general.bus_size_in_bits_get(unit) *
        105 / 100;
     ...
     *   below the threshold.
     ***
     * for (index = 0; index < DNX_FABRIC_DTQ_SHAPERS_FC_NOF; ++index)
     * {
...
/*
 * \brief
 *   Initialize Sub-DTQs contexts:
 *     1. Set which Sub-DTQs are active.
 *     2. Set the size of each Sub-DTQ.
 *   There are 6 Sub-DTQs - MIXED+OCB for each of the 3 Fabric DTQs.
    ...
    int unit)
{
    ...
    /*
     * Each DTQ is composed of 2 sub-DTQs: Mixed sub-DTQ and OCB-Only sub-DTQ.
    ...
    /*
     * The size of the Sub-DTQ is defined until the start of the next Sub-DTQ
     */
    ...
    /*
     * The last Sub-DTQ needs a specific register to tell where it ends
     */
