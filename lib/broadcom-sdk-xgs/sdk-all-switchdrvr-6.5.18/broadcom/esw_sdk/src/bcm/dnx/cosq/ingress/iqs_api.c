    ...
    int priority)
{
     ...
     */
    SHR_IF_ERR_EXIT(dnx_iqs_queue_to_priority_map_set(unit, voq, priority));
    ...
    int *priority)
{
     ...
     */
    SHR_IF_ERR_EXIT(dnx_iqs_queue_to_priority_map_get(unit, voq, &hw_priority));
...
dnx_iqs_queue_flush_set(
    ...
    int timeout)
{
    int voq, core, force_core_all = TRUE;
    uint32 flush_trigger, is_del_dqcq;
    SHR_FUNC_INIT_VARS(unit);
     ...
     */
    SHR_IF_ERR_EXIT(dnx_iqs_queue_flush_queue_id_set(unit, voq));
    ...
    is_del_dqcq = TRUE;
    flush_trigger = TRUE;
    SHR_IF_ERR_EXIT(dnx_iqs_queue_flush_attributes_set(unit, is_del_dqcq, flush_trigger));
    ...
    if (timeout)
    {
         ...
         * Check flush status
         */
        SHR_IF_ERR_EXIT(dnx_iqs_queue_flush_attributes_get(unit, &is_del_dqcq, &flush_trigger));
        if (flush_trigger)
        {
            SHR_ERR_EXIT(_SHR_E_TIMEOUT, "Queue Flush is not done in %d usecs\n", timeout);
        }
    }
