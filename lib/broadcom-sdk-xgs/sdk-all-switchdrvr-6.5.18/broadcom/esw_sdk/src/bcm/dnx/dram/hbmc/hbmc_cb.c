/** \file src/bcm/dnx/dram/hbmc/hbmc_cb.c
 *
 *
 *  This file contains implimentation of call back functions for HBM PHY tuning
 *
 */
    ...
    uint32 mask)
{
    ...
    /** verify data and mask are fit mode register */
...
/**
 * \brief - verify that hbm index and channel are valid, except channel == MIDSTACK as well
 *
...
/**
 * \brief - verify that hbm index and channel are valid
 *
 * \param [in] unit - unit number
 * \param [in] hbm_ndx - hbm index
 * \param [in] channel - channel
    ...
    const hbmc_shmoo_bist_info_t * info)
{
    ...
    {
        /** need to force bist configuration for this specific run */
        /** however need to restore previous configuration for next run */
        /** so save current configuration and restore it later */
    ...
    if (in_loopback_mode)
    {
        ...
        else
        {
            /** if write LFSR mode */
            /**
             * DWORD MISR control - LFSR Compare mode (write direction)
             */
    ...
    /** Start BIST - if not infinite should end by its own after num of actions are done */
    SHR_IF_ERR_EXIT(dnx_hbmc_bist_start(unit, hbm_ndx, channel));

    /** there are 2 ways to run Dram BIST. the first is according to number of actions to perform,
     * and the other is according to how long should the Dram BIST last. for the first we check that
     * the number of actions performed is according to expectations, for the second we sleep and
     * manually stop the Dram BIST when the timer expired */
    if (info->bist_timer_us)
    {
        /** sleep till timer expires */
        sal_usleep(info->bist_timer_us);

        /** stop BIST run */
        SHR_IF_ERR_EXIT(dnx_hbmc_dbal_access_bist_run(unit, hbm_ndx, channel, 0));
    }
    else
    {
        COMPILER_UINT64 orig_nof_bist_actions_for_arethmetics;

        /** check if infinite BIST run */
        COMPILER_64_SET(orig_nof_bist_actions_for_arethmetics, info->bist_num_actions[1], info->bist_num_actions[0]);
        if (COMPILER_64_IS_ZERO(orig_nof_bist_actions_for_arethmetics))
        {
            /** this Bist run is infinite, waiting for action to be done is not required,
             * Bist needs to be terminated externally */
            SHR_EXIT();
        }

        /** check that BIST run was done, do this by polling for bist done. in the future might need to make
         * the timeout period dependent on the number of actions in the run */
    ...
    if (in_loopback_mode)
    {
        ...
        {
            /** re-configure BIST to read the Write LFSR results */
            ...
            /**
             * DWORD Read Mux control - Return LFSR_COMPARE_STICKY,
             * DWORD MISR control - Register mode (read and write directions) DWORD writes are
             *                      captured directly into the MISR registers without compression.
             *                      The MISR registers will contain the most recent write data .
             */
            SHR_IF_ERR_EXIT(dnx_hbmc_dbal_access_mr_set(unit, hbm_ndx, channel, 7, 0x17));

            /** start bist run, no need to poll for done sense only 1 command is issued, let poll anyways to be sure */
            SHR_IF_ERR_EXIT(dnx_hbmc_bist_start(unit, hbm_ndx, channel));
            SHR_IF_ERR_EXIT(dnx_hbmc_bist_run_done_poll(unit, hbm_ndx, channel));
        }
    ...
    /** restore bist info of previous run */
    ...
    hbmc_shmoo_bist_err_cnt_t * error_counters)
{
    ...
    if (flags & HBMC_SHMOO_CFG_FLAG_LOOPBACK_MODE_WRITE_LFSR)
    {
        /** when running with write LFSR loopback mode, there's a different method to get the bist error counters. */
        SHR_IF_ERR_EXIT(dnx_hbmc_dbal_access_bist_write_lfsr_error_counters_get
                        (unit, hbm_ndx, channel, error_counters));
    }
    else
    {
        /** use normal function to get error counters from DBAL */
    ...
    int *is_aligned)
{
    ...
    /** check if alignment check is required for this architecture */
    if (!dnx_data_dram.hbm.feature_get(unit, dnx_data_dram_hbm_dword_alignment_check))
    {
        /** alignment check is not required, mark as aligned and get out */
        *is_aligned = 1;
        SHR_EXIT();
    }

    /** configure bist with consecutive_write=0 consecutive_read=100 num_of_commands=100 */
    biPtr.write_weight = 0;
    biPtr.read_weight = 100;
    biPtr.bist_num_actions[0] = 100;
    /** the rest is the shmoo defaults */
    ...
    /** read status - this is done to clear any old runs that were done */
    ...
    /** read bist error array just to clear the errors */
    ...
    /** if read_data_counter value was the amount of commands that were sent, dwords are aligned */
