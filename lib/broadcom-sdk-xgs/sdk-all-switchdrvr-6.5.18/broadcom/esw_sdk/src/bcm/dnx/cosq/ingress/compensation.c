    ...
    int delta)
{
    ...
    /** get old profile */
    SHR_IF_ERR_EXIT(dnx_cosq_ingress_compensation_voq_compesation_profile_hw_get(unit, voq, &old_comp_profile));

    /** exchange to a profile with the requested delta */
    SHR_IF_ERR_EXIT(dnx_ingress_compensation_db.
                    compensation.exchange(unit, 0, &delta, old_comp_profile, NULL, &new_comp_profile, &is_first,
                                          &is_last));

    if (is_first)
    {
        /** configure compensation in HW */
        SHR_IF_ERR_EXIT(dnx_cosq_ingress_compensation_voq_delta_hw_set(unit, new_comp_profile, delta));
    }
    if (is_last && new_comp_profile != old_comp_profile)
    {
        /** clear old profile compensation */
        SHR_IF_ERR_EXIT(dnx_cosq_ingress_compensation_voq_delta_hw_set(unit, old_comp_profile, 0));
    }

    /** Set mapping to new profile */
    SHR_IF_ERR_EXIT(dnx_cosq_ingress_compensation_voq_compesation_profile_hw_set(unit, voq, new_comp_profile));
    ...
    int *delta)
{
    ...
    SHR_IF_ERR_EXIT(dnx_cosq_ingress_compensation_voq_compesation_profile_hw_get(unit, voq, &compensation_profile));

    /** get compensation from HW */
    SHR_IF_ERR_EXIT(dnx_cosq_ingress_compensation_voq_delta_hw_get(unit, compensation_profile, delta));
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(dnx_ingress_compensation_db.compensation.create(unit, &create_data, NULL));

    /** configure default compensation in HW */
    SHR_IF_ERR_EXIT(dnx_cosq_ingress_compensation_voq_delta_hw_set(unit, default_profile, default_compensation));
exit:
    SHR_FUNC_EXIT;
}
        ...
        case bcmCosqPktSizeAdjustSourceGlobal:
                ...
                || BCM_GPORT_IS_MCAST_QUEUE_GROUP(adjust_info->gport))
            {
                ...
                SHR_IF_ERR_EXIT(dnx_cosq_gport_to_voq_get
                                (unit, adjust_info->gport, adjust_info->cosq, TRUE, &core, &voq));
                /** configure compensation delta */
                SHR_IF_ERR_EXIT(dnx_cosq_ingress_compensation_voq_delta_set(unit, voq, delta));
    ...
    switch (adjust_info->source_info.source_type)
    {
        case bcmCosqPktSizeAdjustSourceGlobal:
                ...
                || BCM_GPORT_IS_MCAST_QUEUE_GROUP(adjust_info->gport))
            {
                ...
                SHR_IF_ERR_EXIT(dnx_cosq_ingress_compensation_voq_delta_get(unit, voq, delta));
            }
            else if (BCM_GPORT_IS_PROFILE(adjust_info->gport))
            {
                /** append size ptr compensation (per OutLif profile)*/
