...
#define DNX_PORT_ADD_SUPPORTED_FLAGS (BCM_PORT_ADD_BASE_Q_PAIR_WITH_ID | BCM_PORT_ADD_CGM_AUTO_ADJUST_DISABLE | \
                                      ...
                                      BCM_PORT_ADD_SCH_PRIORITY_PROPAGATION_EN)
    ...
    bcm_port_mapping_info_t * mapping_info)
{
        ...
        && !DNX_ALGO_PORT_TYPE_IS_FLEXE(unit, port_info, TRUE, TRUE))
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "Only NIF port (excluding ELK / STIF), mirror-recycle and FlexE client can be dynamically added. port=%d\n",
                         port);
        }
    }
    ...
    if (DNX_ALGO_PORT_TYPE_IS_IMB(unit, port_info, FALSE))
    {
        ...
        if (interface_info->interface == BCM_PORT_IF_ILKN)
        {
            ...
            int nof_ilkn_ids = dnx_data_nif.ilkn.ilkn_unit_nof_get(unit) * dnx_data_nif.ilkn.ilkn_unit_if_nof_get(unit);
            ...
            if (dnx_data_nif.ilkn.max_nof_elk_ifs_get(unit) != -1
    ...
    uint32 flags)
{
    ...
    if (((!DNX_ALGO_PORT_TYPE_IS_NIF(unit, port_info, FALSE /* elk */ , FALSE /* stif */ , TRUE /* L1 */ , TRUE /* FLEXE 
         ...
         (!DNX_ALGO_PORT_TYPE_IS_FLEXE(unit, port_info, TRUE, TRUE))) || is_ilkn_over_fabric)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "Only NIF port (excluding ELK / STIF/ ILKNoFabric), mirror-recycle and FlexE client can be removed. port=%d\n",
                     port);
    }
    ...
    bcm_port_mapping_info_t * mapping_info)
{
    ...
    if (DNX_ALGO_PORT_TYPE_IS_TM(unit, port_info))
    {
         ...
         */
        if (DNX_ALGO_PORT_TYPE_IS_EGR_TM(unit, port_info) && (!dnx_data_dev_init.general.access_only_get(unit)))
        {
     ...
     * E2E SCH ports only
     */
    if (DNX_ALGO_PORT_TYPE_IS_E2E_SCH(unit, port_info) && (!dnx_data_dev_init.general.access_only_get(unit)))
    {
        int with_id = 0;
    ...
    *flags = 0;
    ...
    if (DNX_ALGO_PORT_TYPE_IS_TM(unit, port_info))
    {
        ...
        if (DNX_ALGO_PORT_TYPE_IS_E2E_SCH(unit, port_info))
        {
            ...
            SHR_IF_ERR_EXIT(dnx_algo_port_sch_priorities_nof_get(unit, port,
                                                                 (int *) &(mapping_info->num_sch_priorities)));
...
static const dnx_port_add_remove_step_t port_add_remove_sequence[] = {
    ...
    {"ImbPath",           dnx_port_imb_path_port_add_handle,        dnx_port_imb_path_port_remove_handle,        NULL,    j2p_skip},
...
/**
 * \brief - Configure average IPG for NIF ports.
 *
 * \param [in] unit - chip unit id
 * \param [in] port - logical port
 *
 ...
 */
int
dnx_port_init_average_ipg_config(
    int unit,
    bcm_port_t port)
{
    int average_ipg;
    ...
    if (DNX_ALGO_PORT_TYPE_IS_NIF_ETH(unit, port_info, FALSE, TRUE))
    {
        average_ipg = dnx_data_nif.eth.average_ipg_in_bits_get(unit);
        SHR_IF_ERR_EXIT(bcm_dnx_port_ifg_set(unit, port, 0, BCM_PORT_DUPLEX_FULL, average_ipg));
    }
    ...
    int unit)
{
    ...
    BCM_PBMP_ITER(nif_master_ports_bitmap, nif_port)
    {
        ...
        /*
         * Configure average IPG
         */
        SHR_IF_ERR_EXIT(dnx_port_init_average_ipg_config(unit, nif_port));
    }
    ...
    int unit)
{
    ...
    BCM_PBMP_ITER(logical_ports, port)
    {
        ...
        if (header_type_out == BCM_SWITCH_PORT_HEADER_TYPE_INJECTED)
        {
    ...
    if (soc_is(unit, J2P_DEVICE))
    {
        ...
        BCM_PBMP_ITER(nif_ports_bitmap, nif_port)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_algo_port_speed_set
                            (unit, nif_port, DNX_ALGO_PORT_SPEED_F_RX | DNX_ALGO_PORT_SPEED_F_TX, port_speed->val));
        }
    }
    else
    {
