    ...
    bcm_port_t logical_port)
{
    ...
    */
    ...
    while (current_queue != ESB_QUEUE_ID_INVALID)
    {
        ...
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_ESP_Q_NUM, current_queue);
    ...
    int is_add)
{
    ...
    if (DNX_ALGO_PORT_TYPE_IS_NIF(unit, port_info, 0, 0, 0, 0) || DNX_ALGO_PORT_TYPE_IS_FLEXE_MAC(unit, port_info, 0))
    {
        ...
        {
            txi_irdy_th =
                (UTILEX_DIV_ROUND_DOWN(9, (min_gap + 1)) + 1) * dnx_data_esb.general.txi_irdy_th_factor_get(unit) - 1;
        }
        else
        {
            txi_irdy_th = 0;
    ...
    int unit)
{
    ...
    int max_flexe_l1_dest_queue_rate = 0, nof_flexe_l1_dest = 0;
    ...
    /**
     * Get all Master logical ports for all cores. Will be used to iterate and calculate the
     * per port weights.
     */
    SHR_IF_ERR_EXIT(dnx_algo_port_logicals_get(unit, _SHR_CORE_ALL, DNX_ALGO_PORT_LOGICALS_TYPE_VALID,
    ...
    BCM_PBMP_ITER(port_bm, port)
    {
        ...
        if (DNX_ALGO_PORT_TYPE_IS_ETH_L1(unit, interface_type))
        {
            /**
             ...
             */
            max_flexe_l1_dest_queue_rate = UTILEX_MAX(max_flexe_l1_dest_queue_rate, if_rate_mbps);
            nof_flexe_l1_dest++;
        }
        ...
        else
        {
             ...
             */
            SHR_IF_ERR_EXIT(dnx_egr_queuing_nif_rate_to_weight(unit, if_rate_mbps, &weights[esb_queue_id]));
        }
    }
    ...
    if (nof_flexe_l1_dest != 0)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_egr_queuing_nif_rate_to_weight
                        (unit, (max_flexe_l1_dest_queue_rate * nof_flexe_l1_dest * 2),
                         &weights[dnx_data_esb.general.flexe_l1_dest_dedicated_queue_get(unit)]));
    }
    ...
    if (nof_slow_ports != 0)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_egr_queuing_nif_rate_to_weight
                        (unit, (max_slow_port_rate * nof_slow_ports),
                         &weights[dnx_data_esb.general.slow_port_dedicated_queue_get(unit)]));
    }
    ...
    SHR_IF_ERR_EXIT(dnx_egr_queuing_cal_params_calc
                    (unit, weights, dnx_data_esb.general.total_nof_esb_queues_get(unit), &cal_len_optimal,
    ...
    SHR_IF_ERR_EXIT(dnx_algo_cal_simple_fixed_len_cal_build(unit, weights, nof_cal_objects,
                                                            cal_len_optimal, dnx_data_esb.general.mux_cal_len_get(unit),
                                                            calendar->slots));
    ...
    for (ii = 0; ii < dnx_data_esb.general.mux_cal_len_get(unit); ii++)
    {
         ...
         */
        SHR_IF_ERR_EXIT(dnx_esb_cal_entry_set(unit, ii, calendar->slots[ii]));
    }

    /**
     * Enable hardware to load new calendar settings to internal usage
     */
    SHR_IF_ERR_EXIT(dnx_esb_cal_load(unit));
...
{
    int if_speed_mbps;
    uint32 entry_handle_id1, entry_handle_id2;
    uint32 nif_port_id;
    int credits_size;
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);
    ...
    /** set the credit size */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_ESB_CREDIT_SIZE, &entry_handle_id1));
    dbal_entry_value_field32_set(unit, entry_handle_id1, DBAL_FIELD_CREDIT_SIZE, INST_SINGLE, credits_size);
    ...
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_ESB_PER_NIF_IF_CREDITS_CONFIG, &entry_handle_id2));
    dbal_entry_key_field32_set(unit, entry_handle_id2, DBAL_FIELD_NIF_IF, nif_port_id);
    dbal_entry_value_field32_set(unit, entry_handle_id2, DBAL_FIELD_CREDIT_INIT, INST_SINGLE, 1);
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id2, DBAL_COMMIT));
    dbal_entry_value_field32_set(unit, entry_handle_id2, DBAL_FIELD_CREDIT_INIT, INST_SINGLE, 0);
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id2, DBAL_COMMIT));
