    ...
    int unit)
{
    ...
    sal_strncpy(data.name, DNX_ALGO_FIELD_IPMF1_INITIAL_KEY_F_OCC_BMP, DNX_ALGO_RES_MNGR_MAX_NAME_LENGTH - 1);

    /** Need to allocate FFC per PMF program hence creating indexed pool */
    SHR_IF_ERR_EXIT(algo_field_info_sw.ipmf1_initial_key_f_occ_bmp.alloc(unit,
                                                                         dnx_data_field.stage.stage_info_get(unit,
    ...
    int unit)
{
    ...
    sal_strncpy(data.name, DNX_ALGO_FIELD_IPMF1_INITIAL_KEY_G_OCC_BMP, DNX_ALGO_RES_MNGR_MAX_NAME_LENGTH - 1);

    /** Need to allocate FFC per PMF program hence creating indexed pool */
    SHR_IF_ERR_EXIT(algo_field_info_sw.ipmf1_initial_key_g_occ_bmp.alloc(unit,
                                                                         dnx_data_field.stage.stage_info_get(unit,
                                                                                                             DNX_FIELD_STAGE_IPMF1)->
                                                                         nof_contexts));
    for (sub_resource_id = 0;
    ...
    int unit)
{
    ...
    sal_strncpy(data.name, DNX_ALGO_FIELD_IPMF1_INITIAL_KEY_H_OCC_BMP, DNX_ALGO_RES_MNGR_MAX_NAME_LENGTH - 1);

    /** Need to allocate FFC per PMF program hence creating indexed pool */
    SHR_IF_ERR_EXIT(algo_field_info_sw.ipmf1_initial_key_h_occ_bmp.alloc(unit,
                                                                         dnx_data_field.stage.stage_info_get(unit,
                                                                                                             DNX_FIELD_STAGE_IPMF1)->
                                                                         nof_contexts));
    for (sub_resource_id = 0;
    ...
    int unit)
{
    ...
    sal_strncpy(data.name, DNX_ALGO_FIELD_IPMF1_INITIAL_KEY_I_OCC_BMP, DNX_ALGO_RES_MNGR_MAX_NAME_LENGTH - 1);

    /** Need to allocate FFC per PMF program hence creating indexed pool */
    SHR_IF_ERR_EXIT(algo_field_info_sw.ipmf1_initial_key_i_occ_bmp.alloc(unit,
                                                                         dnx_data_field.stage.stage_info_get(unit,
                                                                                                             DNX_FIELD_STAGE_IPMF1)->
                                                                         nof_contexts));
    for (sub_resource_id = 0;
    ...
    int unit)
{
    ...
    sal_strncpy(data.name, DNX_ALGO_FIELD_IPMF1_INITIAL_KEY_J_OCC_BMP, DNX_ALGO_RES_MNGR_MAX_NAME_LENGTH - 1);

    /** Need to allocate FFC per PMF program hence creating indexed pool */
    SHR_IF_ERR_EXIT(algo_field_info_sw.ipmf1_initial_key_j_occ_bmp.alloc(unit,
                                                                         dnx_data_field.stage.stage_info_get(unit,
                                                                                                             DNX_FIELD_STAGE_IPMF1)->
                                                                         nof_contexts));
    for (sub_resource_id = 0;
    ...
    uint32 *bit_range_offset_within_key_p)
{
    ...
    *failed_to_find_p = DNX_FIELD_KEY_FAILED_NO_PLACE;
    ...
    SHR_INVOKE_VERIFY_DNX(dnx_algo_field_key_bit_range_alloc_generic_verify
                          (unit, fg_type, field_stage, context_id, key_length, align_info_p, key_id_p,
    ...
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Failed to find a placement that fulfills alignment options, ocuupied or not.\r\n");
    }
exit:
    SHR_FUNC_EXIT;
}
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_keys_per_stage_allocation_sw.init(unit));
    {
         ...
         */
        {
             ...
             * availability_odd_only          : TRUE
             * Number of keys for MDB_DT      : DNX_FIELD_KEY_NOF_FOR_IPMF1_FOR_MDB_DT
             * First key on fg type           : DBAL_ENUM_FVAL_FIELD_KEY_C
             * Last key                       : DBAL_ENUM_FVAL_FIELD_KEY_C
             * availability_odd_only          : TRUE
             */
            ...
            SHR_IF_ERR_EXIT(KEYS_AVAILABILITY_GROUP_INFO.nof_available_keys.set(unit, stage_index, group_type_index,
                                                                                DNX_FIELD_KEY_NOF_FOR_IPMF1_FOR_MDB_DT
                                                                                (unit)));
            SHR_IF_ERR_EXIT(KEYS_AVAILABILITY_GROUP_INFO.first_available_key.set(unit, stage_index, group_type_index,
                                                                                 DBAL_ENUM_FVAL_FIELD_KEY_A +
                                                                                 dnx_data_field.map.
        ...
        }
        {
             ...
             * Number of keys for MDB_DT : DNX_FIELD_KEY_NOF_FOR_IPMF2_FOR_MDB_DT
             * First key on fg type      : DBAL_ENUM_FVAL_FIELD_KEY_H
             * Last key                  : DBAL_ENUM_FVAL_FIELD_KEY_H
             * availability_odd_only     : TRUE
             */
            supported_stages[num_supported_stages++] = stage_index;
            SHR_IF_ERR_EXIT(KEYS_AVAILABILITY_STAGE_INFO.
            ...
            group_type_index = DNX_FIELD_GROUP_TYPE_DIRECT_TABLE_MDB;
            SHR_IF_ERR_EXIT(KEYS_AVAILABILITY_GROUP_INFO.nof_available_keys.set(unit, stage_index, group_type_index,
                                                                                DNX_FIELD_KEY_NOF_FOR_IPMF2_FOR_MDB_DT
                                                                                (unit)));
            SHR_IF_ERR_EXIT(KEYS_AVAILABILITY_GROUP_INFO.first_available_key.set(unit, stage_index, group_type_index,
                                                                                 DBAL_ENUM_FVAL_FIELD_KEY_F +
                                                                                 dnx_data_field.map.
        ...
        }
        {
            ...
            group_type_index = DNX_FIELD_GROUP_TYPE_DIRECT_TABLE_MDB;
            SHR_IF_ERR_EXIT(KEYS_AVAILABILITY_GROUP_INFO.nof_available_keys.set(unit, stage_index, group_type_index,
                                                                                DNX_FIELD_KEY_NOF_FOR_IPMF3_FOR_MDB_DT
                                                                                (unit)));
            SHR_IF_ERR_EXIT(KEYS_AVAILABILITY_GROUP_INFO.first_available_key.set(unit, stage_index, group_type_index,
                                                                                 DBAL_ENUM_FVAL_FIELD_KEY_A +
                                                                                 dnx_data_field.map.
            ...
            for (stage_index = 0; stage_index < num_supported_stages; stage_index++)
            {
                ...
                for (fg_type_index = DNX_FIELD_GROUP_TYPE_FIRST; fg_type_index < fg_type_max; fg_type_index++)
                {
                    ...
                    if (supports_bitmap_allocation)
                    {
                        ...
                        /*
                         * Ccreating an indexed pool of similar resorces, one per key (on odd half keys)
    ...
    switch (field_stage)
    {
        ...
        case DNX_FIELD_STAGE_EPMF:
        {
            SHR_IF_ERR_EXIT(dnx_algo_field_key_bit_range_alloc_generic
                            (unit, fg_type, field_stage, context_id, key_length, align_info_p, &failed_to_find,
