...
static int imb_ethu_master_ports_get(
    int unit,
    bcm_port_t port,
...
/**
 * \brief - Flush packets in ETHU RX
 */
static int
    ...
    int enable)
{
    ...
    if (enable)
    {
        ...
        if (!DNX_ALGO_PORT_TYPE_IS_NIF_ETH_STIF(unit, port_info))
        {
            ...
            {
                SHR_IF_ERR_EXIT(imb_ethu_port_rx_nmg_reset_hw_set
                                (unit, port, IMB_COSQ_ALL_RMCS, IMB_COMMON_OUT_OF_RESET));
            }
            else
            {
        ...
        if (!DNX_ALGO_PORT_TYPE_IS_NIF_ETH_STIF(unit, port_info) && DNX_ALGO_PORT_TYPE_IS_EGR_TM(unit, port_info))
        {
            /*
             * Enable Qpairs
             */
    ...
    else
    {
        if (!DNX_ALGO_PORT_TYPE_IS_NIF_ETH_STIF(unit, port_info))
        {
            ...
            {
                /*
                 * Flush and reset the port in ETHU (RMC)
                 */
            ...
            {
                /*
                 * reset the port in ETHU (RMC)
                 */
                SHR_IF_ERR_EXIT(imb_ethu_port_rx_reset_hw_set(unit, port, IMB_COSQ_ALL_RMCS, IMB_COMMON_IN_RESET));
            }
 ...
 */
static int
imb_ethu_port_priority_config_rmc_empty_wait(
    int unit,
    bcm_port_t port,
    uint32 rmc_id)
{
    ...
    SHR_IF_ERR_EXIT(imb_ethu_internal_rmc_level_get(unit, port, rmc_id, &fifo_level));
    ...
    int enable)
{
    ...
    for (ii = 0; ii < nof_rmcs; ++ii)
    {
         ...
         */
        SHR_IF_ERR_EXIT(imb_ethu_port_priority_config_rmc_empty_wait(unit, port, rmc_arr[ii].rmc_id));
        ...
        else
        {
             ...
             * reset the port in ETHU (RMC)
             */
            SHR_IF_ERR_EXIT(imb_ethu_port_rx_reset_hw_set(unit, port, rmc_arr[ii].rmc_id, IMB_COMMON_IN_RESET));
        }

        /*
         * Reset the RMCs in the ETHU and NMG
         */
        if (dnx_data_nif.eth.feature_get(unit, dnx_data_nif_eth_nif_scheduler_context_per_rmc_support))
        {
            SHR_IF_ERR_EXIT(imb_ethu_port_rx_nmg_reset_hw_set(unit, port, rmc_arr[ii].rmc_id, IMB_COMMON_IN_RESET));
        }
        else
        {
            SHR_IF_ERR_EXIT(imb_ethu_port_rx_nmg_reset_sch_prio_set
                            (unit, port, rmc_arr[ii].sch_priority, IMB_COMMON_IN_RESET));
        }
    ...
    if (enable)
    {
        ...
        for (ii = 0; ii < nof_rmcs; ++ii)
        {
            ...
            /*
             * Take RMCs out of reset in the ETHU and NMG
             */
            if (dnx_data_nif.eth.feature_get(unit, dnx_data_nif_eth_nif_scheduler_context_per_rmc_support))
            {
                SHR_IF_ERR_EXIT(imb_ethu_port_rx_nmg_reset_hw_set
                                (unit, port, rmc_arr[ii].rmc_id, IMB_COMMON_OUT_OF_RESET));
            }
            else
            {
                SHR_IF_ERR_EXIT(imb_ethu_port_rx_nmg_reset_sch_prio_set
                                (unit, port, rmc_arr[ii].sch_priority, IMB_COMMON_OUT_OF_RESET));
            }
            SHR_IF_ERR_EXIT(imb_ethu_port_rx_reset_hw_set(unit, port, rmc_arr[ii].rmc_id, IMB_COMMON_OUT_OF_RESET));
        }
    }
exit:
    SHR_FUNC_EXIT;
...
/**
 * \brief - Set priority groups configuration according to default values
 *
 * \param [in] unit - chip unit id
 * \param [in] port - logical port
 */
static int
imb_ethu_port_priority_config_default_config_set(
    int unit,
    bcm_port_t port)
{
    dnx_algo_port_rmc_info_t default_rmc_arr[DNX_DATA_MAX_NIF_ETH_PRIORITY_GROUPS_NOF];
    int nof_default_rmc = 0;
    SHR_FUNC_INIT_VARS(unit);

    /*
     * Get default priority configuration
     */
    SHR_IF_ERR_EXIT(imb_ethu_port_priority_config_default_get(unit, port, default_rmc_arr, &nof_default_rmc));
    /*
     * Set default priority configuration
     */
    SHR_IF_ERR_EXIT(imb_ethu_port_priority_config_enable_set(unit, port, default_rmc_arr, nof_default_rmc, 1));
...
/**
 * \brief - Set priority groups configuration according to data stored
 *          in sw state.
 *
 * \param [in] unit - chip unit id
 * \param [in] port - logical port
 */
static int
imb_ethu_port_priority_config_user_config_set(
    int unit,
    bcm_port_t port)
{
    int ii;
    ...
    /*
     * Set user configuration that was retrieved from sw state
     */
    SHR_IF_ERR_EXIT(imb_ethu_port_priority_config_enable_set(unit, port, rmc_arr, nof_prio_groups, 1));
    ...
    const bcm_port_prio_config_t * priority_config)
{
    ...
    {
        /** Set default priority configuration */
        SHR_IF_ERR_EXIT(imb_ethu_port_priority_config_default_config_set(unit, port));
    }
    else
    {
        /** Set user priority configuration which is stored in sw state db */
        SHR_IF_ERR_EXIT(imb_ethu_port_priority_config_user_config_set(unit, port));
    }
