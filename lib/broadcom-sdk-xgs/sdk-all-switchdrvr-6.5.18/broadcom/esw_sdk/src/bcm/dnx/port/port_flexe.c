    ...
    uint32 flags)
{
    ...
    int rx_speed = 0, tx_speed = 0, ptp_oam_channel;
    ...
    ptp_oam_channel = ((client_channel == dnx_data_nif.flexe.oam_client_channel_get(unit))
                       || (client_channel == dnx_data_nif.flexe.ptp_client_channel_get(unit))) ? 1 : 0;
    ...
    if (DNX_ALGO_PORT_FLEXE_RX_GET(flags))
    {
        ...
        if (has_rx_speed)
        {
            ...
            nof_rx_slots = (ptp_oam_channel) ? 1 : (rx_speed / dnx_data_nif.flexe.client_speed_granularity_get(unit));
        }
    ...
    if (DNX_ALGO_PORT_FLEXE_TX_GET(flags))
    {
        ...
        if (has_tx_speed)
        {
            ...
            nof_tx_slots = (ptp_oam_channel) ? 1 : (tx_speed / dnx_data_nif.flexe.client_speed_granularity_get(unit));
        }
    ...
    int is_set)
{
    ...
    if ((type == bcmPortFlexeOhGroupID) || (type == bcmPortFlexeOhCalInUse) ||
        (type == bcmPortFlexeOhCalRequest) || (type == bcmPortFlexeOhCalAck))
    {
        /** Verify port type */
        SHR_IF_ERR_EXIT(dnx_algo_flexe_group_port_verify(unit, gport));
        group_index = BCM_FLEXE_GROUP_GPORT_GROUP_INDEX_GET(gport);
    ...
    int *val)
{
    ...
    if ((type == bcmPortFlexeOhGroupID) || (type == bcmPortFlexeOhCalInUse) ||
        (type == bcmPortFlexeOhCalAck) || (type == bcmPortFlexeOhCalRequest))
    {
        group_index = BCM_FLEXE_GROUP_GPORT_GROUP_INDEX_GET(gport);
        SHR_IF_ERR_EXIT(dnx_algo_flexe_core_ports_get(unit, group_index, &flexe_core_ports));
    ...
    int is_add)
{
    ...
    /*
     * Restore the value to IRE LP when deleting the flow
     */
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_TRAFFIC_TYPE, INST_SINGLE,
                                 (is_add ? DBAL_ENUM_FVAL_FEU_RMC_TRAFFIC_TYPE_ESB :
    ...
    int is_add)
{
    ...
    SHR_IF_ERR_EXIT(flexe_core_sar_channel_map_set(unit, SOC_PORT_FLEXE_TX, sar_channel, ilu_channel, is_add));
    ...
    int is_add)
{
    ...
    SHR_IF_ERR_EXIT(flexe_core_sar_channel_map_set(unit, SOC_PORT_FLEXE_RX, sar_channel, ilu_channel, is_add));
    ...
    int is_add)
{
    ...
    switch (flow_type)
    {
        ...
        case DNX_ALGO_FLEXE_FLOW_MAC2ETH:
        {
            ...
            {
                SHR_IF_ERR_EXIT(dnx_port_flexe_flow_mismatch_rate_config_set(unit, src_port, dst_port, is_add));
            }
            SHR_IF_ERR_EXIT(dnx_port_flexe_flow_tmac2eth_set(unit, src_channel, &rmc, ethu_info.lane_in_core,
                                                             ethu_info.ethu_id, ethu_info.first_lane_in_port, is_add));
            break;
        }
        case DNX_ALGO_FLEXE_FLOW_ETH2MAC:
        {
             ...
             */
            SHR_IF_ERR_EXIT(dnx_algo_port_nif_first_phy_get(unit, src_port, 0, &src_channel));
            /*
             * get FLEXE MAC channel 
             */
            SHR_IF_ERR_EXIT(dnx_algo_port_channel_get(unit, dst_port, &dst_channel));
        ...
        case DNX_ALGO_FLEXE_FLOW_SAR2ILKN:
        {
            ...
            SHR_IF_ERR_EXIT(dnx_port_flexe_flow_sar2ilkn_set(unit, src_channel, dst_channel, is_add));
            break;
        }
        case DNX_ALGO_FLEXE_FLOW_ILKN2SAR:
        {
            ...
            SHR_IF_ERR_EXIT(dnx_port_flexe_flow_ilkn2sar_set(unit, src_channel, dst_channel, is_add));
            break;
        }
        case DNX_ALGO_FLEXE_FLOW_MAC2ILKN:
        {
            ...
            for (i = 0; i < nof_priority_groups; ++i)
            {
                 ...
                 * get FlexE RMC info 
                 */
                SHR_IF_ERR_EXIT(dnx_algo_port_nif_logical_fifo_get(unit, src_port, 0, i, &rmc));

                if (rmc.rmc_id != DNX_ALGO_PORT_INVALID)
                {
                    SHR_IF_ERR_EXIT(dnx_port_flexe_flow_tmac2ilkn_set
                                    (unit, src_channel, &rmc, ilkn_info.port_in_core, dst_channel, is_add));
                }
            }
            break;
        }
    ...
    int is_set)
{
    ...
    /** Verify period value */
    if (is_set && (type == bcmPortFlexeOamControlRxBasePeriod || type == bcmPortFlexeOamControlTxBasePeriod))
    {
