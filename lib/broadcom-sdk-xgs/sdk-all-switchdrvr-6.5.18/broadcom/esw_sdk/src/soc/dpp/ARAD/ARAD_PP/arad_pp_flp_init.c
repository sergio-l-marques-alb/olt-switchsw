...
/*P2P service after terminated lsp or pwe, irrespective of forward-code*/
...
/*
* This program use to match packet "OAM over Ethernet over CW over PWE over Ethernet"
*  and make it to skip OAM TCAM lookup.
*/
uint32
   ...
   )
{
  ...
  /** FLP program selection for IPv4 UC */
   ...
   )
{
  ...
  /** FLP program selection for IPv6 UC*/
   ...
   )
{
  ...
  int32 cam_sel_id;
  DPP_PFC_E pfc_type;
  DPP_PLC_E parser_leaf_context = DPP_PLC_NOF /* initialize to invalid parameter*/;
  SOC_SAND_INIT_ERROR_DEFINITIONS(0);
  ...
  if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "non_bfd_forwarding", 0)) {
      /* When using this soc property program selection is based on FLP PTC profile.
         Packet should be forwarded to a dedicated recycle port in the first pass and in the second pass
         port is assigned a special FLP-PTC profile. */
    ...
    )
{
    ...
    if (SOC_IS_JERICHO(unit)) {
        ...
        if(ARAD_KBP_PROG_IS_L2_SHARING(opcode)) {
            ...
            /*
             * In case of DIP SIP sharing for IPv6:
             * For UC no RPF- need to allocate CE to the SIP + configure the DIP to be sent to  KBP
             * For UC RPF or MC: both SIP and DIP already allocated in FLP - just need to configure them to be sent to KBP
             * Each data that already configured by FLP, configured to be optimized in KAPS, therefore it contain 2 bytes of padding in KBP
             */
                int bytes_lsb = 10;
                int bytes_msb = 8;
   ...
   )
{
  ...
  trap_code_ndx = SOC_PPC_TRAP_CODE_OAM_FTMH_MIRROR_TRAP_CODES_MIP_EGRESS_SNOOP/* snoop WA trap */;
   ...
   )
{
  ...
  flp_lookups_tbl.lem_2nd_lkp_or_value   = ARAD_PP_FLP_ETH_KEY_OR_MASK(unit);
  if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "gpon_application_enable", 0)) {
      /* learning is allowed for GPON16 application */
      flp_lookups_tbl.learn_key_select      = ARAD_PP_FLP_LKP_KEY_SELECT_FID_FWD_MAC_KEY_HW_VAL;
  } else {
      flp_lookups_tbl.learn_key_select   = 0;
  }
   ...
   )
{
  ...
  if (SOC_DPP_CONFIG(unit)->pp.test2) {
      ...
      flp_key_construction_tbl.key_c_inst_0_to_5_valid    = 0x19 /*6'b011001*/;
  } else {
      ...
      flp_key_construction_tbl.key_c_inst_0_to_5_valid    = 0x1B /*6'b011011*/;
  }
   ...
   )
{
  ...
  if (SOC_DPP_CONFIG(unit)->pp.test2) {
      ...
      if (SOC_IS_JERICHO(unit)) {
          ...
          flp_key_construction_tbl.key_d_inst_0_to_7_valid    = 0xA4 /*8'b10100100*/;
      }
  }
   ...
   )
{
  ...
  if(soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "multi_hop_bfd_extra_dip_support", 0)) {
    flp_key_construction_tbl.key_a_inst_0_to_5_valid    = 0x8;  /*Do not use TTL 6'b001000*/
   } else {
   ...
   )
{
  ...
  if(ARAD_KBP_ENABLE_IPV4_UC || ARAD_KBP_ENABLE_IPV4_MPLS_EXTENDED){
      ...
      /* KBP default route lookup in KAPS */
     ...
     uint32 prog_id)
{
  ...
  /* For ingress learning, get learn key from key D */
...
/*
 * This function is referring to IPv4 UC public private RPF program.
 */
uint32
...
/*
 * This function is referring to IPv4 UC public private program
 * or to a custom IP routing program.
 */
uint32
   ...
   )
{
    ...
    /*ARAD_PP_FLP_INIT_PRINT_ADVANCE("arad_pp_flp_process_mpls_exp_null_ttl_reject");*/
...
/*
* mode variable is input to the following function:
* mode==1: PROG_FLP_IPV4MC_BRIDGE_USER_MODE is installed dynamically.
* mode==2: PROG_FLP_IPV4MC_BRIDGE_USER_MODE_UNTAGGED is installed dynamically.
* else   : PROG_FLP_IPV4MC_BRIDGE is installed statically.
*/
uint32
   ...
   )
{
  ...
  if (mode == 1 && SOC_DPP_CONFIG(unit)->pp.ipmc_ivl) { /*for user mode both tagged/untagged allocate the program dynamically */
  ...
  if (mode == 1 && SOC_DPP_CONFIG(unit)->pp.ipmc_ivl) { /* tagged packet */
      prog_selection_cam_tbl.lsb_rif_profile_bit = 0x1; /* user mode */
      prog_selection_cam_tbl.lsb_rif_profile_bit_mask = 0x0;
  }

  if (mode == 2 && SOC_DPP_CONFIG(unit)->pp.ipmc_ivl) { /* untagged packet */
      prog_selection_cam_tbl.lsb_rif_profile_bit = 0x1; /* user mode */
      prog_selection_cam_tbl.lsb_rif_profile_bit_mask = 0x0;
      /* removing mask on TPIDS, when both are NONE, this is an untagged packet */
   ...
   )
{
  ...
  if((SOC_IS_JERICHO(unit)) && (SOC_DPP_CONFIG(unit)->pp.ipmc_l2_ssm_mode == BCM_IPMC_SSM_ELK_LPM)) {
      flp_process_tbl.include_elk_fwd_in_result_a = 2; /* higher priority than LEM */
   ...
   )
{
  ...
  flp_process_tbl.sa_lkp_result_select         = 0;
  flp_process_tbl.sa_lkp_process_enable        = 0;
  flp_process_tbl.procedure_ethernet_default  = 3; /* enable ethernet default procedure is lookupA not found */
  flp_process_tbl.enable_hair_pin_filter       = 1;
...
/*  IPv4 compatible multicast with MIM learning */
/*  ------------------------------------------- */
   ...
   )
{
  ...
  prog_selection_cam_tbl.port_profile_mask            = 3     /* ARAD_PP_FLP_PORT_PROFILE_MASK_CONSIDER_NONE */;
  prog_selection_cam_tbl.ptc_profile_mask             = 0x03;
  prog_selection_cam_tbl.forwarding_code_mask         = 0x00;
  prog_selection_cam_tbl.forwarding_offset_index_mask = 0;
  prog_selection_cam_tbl.in_rif_uc_rpf_enable_mask    = 0x1;
   ...
   )
{
  ...
  flp_key_construction_tbl.key_a_inst_0_to_5_valid = 0x1e; /*hex 6'b011110*/  /*  in-port, B-VLAN, B-SA */
  flp_key_construction_tbl.key_b_inst_0_to_5_valid = 0x20; /*hex 6'b100000*/  /*  DIP[27:0] */
  flp_key_construction_tbl.key_c_inst_0_to_5_valid = 0x0;
   ...
   )
{
  ...
  SOC_SAND_CHECK_FUNC_RESULT(res, 42, exit);
  {
      ...
      /* LEM2: {In-PP-Port, BVID, BSA} - key-A */
      flp_lookups_tbl.lem_2nd_lkp_valid     = 1;
      flp_lookups_tbl.lem_2nd_lkp_key_select = 0;
      flp_lookups_tbl.lem_2nd_lkp_and_value  = 0x0;
   ...
   )
{
    ...
    SOC_SAND_CHECK_FUNC_RESULT(res, 41, exit);
    tmp = 2; /* 0: Apply Ethernet traps on forwarding-header 1: Apply IPv4 traps on forwarding-header and L4 Traps on the following header 2: Apply IPv6 traps on forwarding-header and L4 Traps on the following header 3: Apply MPLS traps on forwarding-header 4: Apply FC traps on forwarding-header Else: Don't apply any protocol traps */
    SOC_REG_ABOVE_64_CLEAR(reg_val);
    res = READ_IHP_PROTOCOL_TRAPS_PROGRAM_SELECTr(unit, 0, reg_val);
    SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 40, exit);
    SHR_BITCOPY_RANGE(reg_val,3*PROG_FLP_IPV6UC,&tmp,0,3);
   ...
   arad_pp_flp_process_gpon_l2_ip4mc(int unit, int prog_id)
{
  ...
  flp_process_tbl.include_lem_1st_in_result_a = 1;     /* L2_IPMC */
  flp_process_tbl.include_lem_2nd_in_result_a = 2;     /* L2_IPMC_SSM */
  flp_process_tbl.select_default_result_a     = 2;     /* Default result = not found trap */
  flp_process_tbl.result_a_format             = 0;     /* Format1 */
  flp_process_tbl.apply_fwd_result_a           = 1;    /* Apply result A always */
   ...
   * 1: Apply IPv4 traps on forwarding-header and L4 Traps on the following header
   * 2: Apply IPv6 traps on forwarding-header and L4 Traps on the following header
   * 3: Apply MPLS traps on forwarding-header 4: Apply FC traps on forwarding-header Else: Don't apply any protocol traps
   */
  SOC_REG_ABOVE_64_CLEAR(reg_val);
  res = READ_IHP_PROTOCOL_TRAPS_PROGRAM_SELECTr(unit, 0, reg_val);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 40, exit);
   ...
   )
{
  ...
  /*ARAD_PP_FLP_INIT_PRINT_ADVANCE("arad_pp_flp_process_gpon_l2_ip4mc");*/
  ...
  flp_process_tbl.include_lem_1st_in_result_a = 1;     /* EUI-64 SA */
  flp_process_tbl.select_default_result_a     = 2;     /* Default result = not found trap */
  flp_process_tbl.result_a_format             = 0;     /* Format1 */
  flp_process_tbl.apply_fwd_result_a           = 1;    /* Apply result A always */
  flp_process_tbl.procedure_ethernet_default   = 1;
     ...
     int unit)
{
  ...
  /* Ethernet_ing_learn for the packets from IBC port */
...
/*
 * Program selection for GPON IPMC
*/
uint32
   ...
   )
{
    ...
    /*
     * Packet-Format-Code = *oIPv4oE
     * Forwarding-Code = Ethernet
     * Is-Compatible-Multicast is set
     * CoS-Profile = {1'b1, 5'bXXXXX}
     */
    ...
    prog_selection_cam_tbl.packet_is_compatible_mc        = 1;
    prog_selection_cam_tbl.packet_is_compatible_mc_mask   = 0x0; /* compatible MC only*/
    prog_selection_cam_tbl.ptc_profile                           = SOC_TMC_PORTS_FLP_PROFILE_NONE; /* Not PON port */
    prog_selection_cam_tbl.ptc_profile_mask              = 0;
...
/*
 * the arad_pp_flp_init_bfd_program initialize the BFD program selection and the BFD program
 * from the arad_pp_flp_process_init.
 */
uint32
...
STATIC uint32 arad_pp_flp_program_get_namespace(int unit, uint8 app_id, int *name_space_idx)
{
    ...
    /*
     * process for multiplex FLP program first
     */
    /* PPPoE */
    ...
    switch (app_id) {
        ...
        case PROG_FLP_BIDIR:    /*IPMC BIDIR*/
...
STATIC uint32 arad_pp_flp_program_assigne_namespace(int unit, uint8 app_id, int hw_prog)
{
    ...
    /* map app_id to name_space_idx */
    res = arad_pp_flp_program_get_namespace(unit, app_id, &name_space_idx);
    SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);

    /* update counter and program array in this namespace */
...
/* update PEM_context with prog_id */
uint32 arad_pp_flp_update_programs_pem_contex(int unit)
{
    ...
    /* 
     * prog_data = {1+5+5}: valid(1)+prog_idt(5)+5
     * Process only for QUX
     */
    if (SOC_IS_QUX(unit)) {
...
STATIC uint32 arad_pp_flp_set_programs_name_space(int unit)
{
    ...
    for (prog_id = 0; prog_id < nof_flp_hw_programs; prog_id++) {
        ...
        /*Get sw program from hw program*/
        res = sw_state_access[unit].dpp.soc.arad.pp.fec.flp_progs_mapping.get(unit, prog_id, &app_id);
        SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 40, exit);
   ...
   )
{
  ...
  /*
   * In the advance parser mode and in the "none BFD forwarding" mode it is important to set the BFD program before
   * the routing programs to avoid BFD packets to hit other programs.
   * The early_bfd_init indicate that the BFD programs will be initializes earlier in the FLP program init
   * sequence then they would without any special configuration
   */
  if((SOC_DPP_CONFIG(unit)->pp.parser_mode == 1) ||
  ...
  /*
   * BFD program selection priority is hither than The myMAC and IP disabled trap program init should be
   */
  ...
  /* -------------------------------------------------------*/
  /* ----------- Workaround for MPLS Exp Null TTL traps ----*/
  /* -------------------------------------------------------*/
  ...
  if (SOC_DPP_PPPOE_IS_ENABLE(unit)) {
      if (SOC_DPP_L3_SRC_BIND_IPV4_ENABLE(unit)) {
          /* PPPoE(LEM) & IPv4(KAPS) anti-spoofing + IPv4 DIP lookup(LEM+KAPS)
           * 1st LEM lookup: PPPoE anti-spoofing
           * 1st KAPS lookup: SIPv4 anti-spoofing
           * 2nd LEM lookup: IPv4 DIP lookup(host)
           * 2nd KAPS lookup IPv4 DIP lookup(subnet)
           */
      ...
      if (SOC_DPP_L3_SRC_BIND_IPV6_ENABLE(unit)) {
          /* PPPoE(LEM) & IPv6(KAPS) anti-spoofing + IPv6 DIP lookup(KAPS)
           * 1st LEM lookup: PPPoE anti-spoofing
           * 1st KAPS lookup: SIPv6 anti-spoofing
           * 2nd KAPS lookup IPv6 DIP lookup(host & subnet)
           */
  ...
  /* A different selection to FLP P2P prog.
   * The same flp_key_const, flp_lookups and flp_process are used.
   * So, no additional configurations are needed except for the selector.
   */
  res = arad_pp_flp_prog_sel_cam_mpls_p2p(unit);
  SOC_SAND_CHECK_FUNC_RESULT(res, 126, exit);

  /* To drop the MAC lookup unmatched ces packet by select to program: PROG_FLP_TM.
   */
  ...
  if(is_routing_enabled)
  {
...
#else
      {
          ...
          if(JER_KAPS_ENABLE(unit) && SOC_DPP_CONFIG(unit)->pp.custom_ip_route){
              int prog_id = 0;
              /*FLP prgram init for IPv4 customer routing.*/
              ...
              /*FLP prgram init for IPv6 customer routing.*/
...
#endif
     {
      ...
      /* ---------------------------------------*/
      if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "mim_spb_enable", 0)){
           /* In case SPBM is used, an extra line will be added to the TCAM which will select the IPV4UC program for packets coming from NNI */
    ...
    if (SOC_IS_ARADPLUS_A0(unit) && SOC_DPP_CONFIG(unit)->pp.oam_statistics) {
      ...
      if (!ARAD_PP_FLP_PON_PROG_LOAD() && !soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "ipmc_l2_ssm_mode_lem", 0)) {
          ...
          arad_pp_flp_prog_sel_cam_ipv4mc_bridge(unit,0x0/*regular mode*/,NULL);
      ...
      if (SOC_DPP_CONFIG(unit)->pp.ipmc_ivl) {
          ...
          /* notice: the order between initiating the tagged/untagged modes is important in the selection cam! */
          arad_pp_flp_prog_sel_cam_ipv4mc_bridge(unit,user_mode_untagged, &prog_id_untagged);  /* allocate PROG_FLP_IPV4MC_BRIDGE_USER_MODE_UNTAGGED dynamically */

          arad_pp_flp_prog_sel_cam_ipv4mc_bridge(unit,user_mode, &prog_id);  /* allocate PROG_FLP_IPV4MC_BRIDGE_USER_MODE dynamically */

          arad_pp_flp_dbal_ipv4mc_bridge_program_tables_init_ivl(unit,prog_id_untagged,prog_id); /* arad_pp_flp_dbal_ipv4mc_bridge_program_tables_init */

          arad_pp_flp_process_ipv4mc_bridge_ivl(unit,prog_id_untagged); /* arad_pp_flp_process_ipv4mc_bridge */
  ...
  if (!(SOC_DPP_CONFIG(unit)->l3.ipmc_vpn_lookup_enable)) {
      if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "ipmc_l2_ssm_mode_lem", 0)) {
         ...
         /* IPv4 MC SSM using SIP lookup in the LEM */
         ...
         /* IPv6 MC SSM using EUI lookup in the LEM */
  ...
  if (ARAD_PP_FLP_PON_PROG_LOAD()) {
      ...
      /* Inter-board communication port */
      if (ARAD_PP_FLP_IBC_PORT_ENABLE(unit)) {
  ...
  /*
   * In the basic parser mode it is important to set the BFD program at the end of the initialization to avoid
   * FCOE and PON program selection conflicts.
   */
  if(early_bfd_init == 0) {
      arad_pp_flp_init_bfd_program(unit);
  }
   ...
   )
{
  ...
  /* SDK assign dynamic program ID(17) for PON default downstream and SA learning is disabled
   * in PON default downstream program.Unfortunatly SA learning is enabled with PROG_FLP_IPV4MC_BRIDGE(fixed value).
   */
  if (!ARAD_PP_FLP_PON_PROG_LOAD()) {
   ...
   )
{
    ...
    for (index = 0; index < SOC_DPP_DEFS_GET(unit, nof_flp_programs); index++) {
        ...
        /* If current program has a dedicated not_found_trap_code, doesn't change it */
   ...
   )
{
    ...
    /* PROG_FLP_TM always is installed and no dedicated flp not found trap ID */
    res = arad_pp_ihb_flp_process_tbl_get_unsafe(unit, PROG_FLP_TM, &flp_process_tbl);
    SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
