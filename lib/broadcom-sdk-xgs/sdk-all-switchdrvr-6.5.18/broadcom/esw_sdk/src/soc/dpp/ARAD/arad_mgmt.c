  ...
  )
{
  ...
  if (SOC_IS_JERICHO(unit)) {
      ...
      conf[conf_idx].reg = (flags & ARAD_MGMT_ALL_CTRL_CELLS_FLAGS_SOFT_RESET) == 0 ? MESH_TOPOLOGY_REG_0117r : INVALIDr; /*Do not config as part of soft reset sequence*/ 
      conf[conf_idx].field = (flags & ARAD_MGMT_ALL_CTRL_CELLS_FLAGS_SOFT_RESET) == 0 ? FIELD_9_9f  : INVALIDf; /*Do not config as part of soft reset sequence*/
  ...
  conf[conf_idx].reg = (flags & ARAD_MGMT_ALL_CTRL_CELLS_FLAGS_SOFT_RESET) == 0 ? MESH_TOPOLOGY_MESH_TOPOLOGYr : INVALIDr; /*Do not config as part of soft reset sequence*/
  conf[conf_idx].field = (flags & ARAD_MGMT_ALL_CTRL_CELLS_FLAGS_SOFT_RESET) == 0 ? CONFIG_6f : INVALIDf; /*Do not config as part of soft reset sequence*/
  conf[conf_idx].val = (enable == TRUE ? 0x1 : 0x0);
  conf[conf_idx].instance_id = REG_PORT_ANY;
  conf[conf_idx].polling_func = NULL;
  conf[conf_idx].failure_func = NULL;
