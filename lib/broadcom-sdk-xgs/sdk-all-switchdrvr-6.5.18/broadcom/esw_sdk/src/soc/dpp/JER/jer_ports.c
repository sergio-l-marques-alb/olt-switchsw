...
/* 
   returns TRUE iff input port has its own separate context (TDM or dedicated TDM port)
*/
...
/* 
   Find another port (reference) sharing the same context as an input ILKN port.
   Return pointer to reference port if found.
*/
                                                  ...
                                                  soc_port_t* ref_port)
{
    ...
    *ref_port = SOC_PORT_INVALID;
    ...
    /* this function is called for interface type == SOC_PORT_IF_ILKN only */
    ...
    if (interface == SOC_PORT_IF_ILKN) {
        ...
        if(!soc_jer_port_ilkn_has_separate_context(unit, port, is_dedicated_tdm_context, &tdm_queuing_pbmp)) { 
            ...
            /*
              Iterate over all ports and find first port which is not TDM
            */
            SOC_PBMP_ITER(ports_pbmp, temp_port) {
                if (!soc_jer_port_ilkn_has_separate_context(unit, temp_port, is_dedicated_tdm_context, &tdm_queuing_pbmp)) {
                    if (temp_port != port) {
                        /* first non TDM port is the reference port */
                        *ref_port = temp_port;
                        break;
                    }
                    if (!is_dynamic_port) {
                        break; /* first non TDM port is the master - do not search further */
                    }
                }
            }
        }
...
/* 
   Calculate size of group of ILKN channels (ports) sharing the same context with provided input port.
   Given input port is not counted
*/
    ...
    *size = 0;
    ...
    if (interface == SOC_PORT_IF_ILKN) {
        ...
        if(!soc_jer_port_ilkn_has_separate_context(unit, port, is_dedicated_tdm_context, &tdm_queuing_pbmp)) { 
            ...
            SOCDNX_IF_ERR_EXIT(soc_port_sw_db_ports_to_same_interface_get(unit, port, &ports_pbmp));
            /*
              Iterate over all ports and find all ports which are not TDM
            */
            SOC_PBMP_ITER(ports_pbmp, temp_port) {
...
/* 
   this function computes if a new context should be allocated for the provided port.
   the result is returned in allocate_new parameter.
   if allocate_new is FALSE, a port which can used to obtain reassembly context is returned in ref_port parameter
*/
                                         ...
                                         int* allocate_new,  soc_port_t* ref_port)
{
    ...
    *allocate_new = 0;
    ...
    if (interface == SOC_PORT_IF_ILKN) {
        ...
        /* the only interleaved interfaces are : ILKN and RCY */
        if (is_master_port ||
    ...
    if (is_last_port) {
        *need_dealloc = 1;
        ...
        if ((ilkn_config->interleaved && !is_dedicated_tdm_context) || (is_dedicated_tdm_context && SOC_PBMP_MEMBER(tdm_queuing_pbmp, port))) {
            ...
            *need_dealloc = (grp_size == 0); /* the port to be removed is not counted */
        }
    }
   ...
   )
{
    ...
    {   
        ...
        if (IS_TDM_PORT(unit, port) && ((interface == SOC_PORT_IF_ILKN) || is_nif)) {
            /* In Jericho: TDM context map is written statically to port# (512 context, each port has 1 context) regardless of the port status (TDM/non TDM)
             * In QAX TDM port: using mapping from logical port to TM port */
            data = 0;
   ...
   )
{
    ...
    if(interface == SOC_PORT_IF_ERP || SOC_PORT_IS_LB_MODEM(flags)) {
        /* Modem port must be the last port on the physical interface. */
   ...
   )
{
    ...
    if(interface != SOC_PORT_IF_ERP) {
        ...
        if ((interface == SOC_PORT_IF_LBG) && (if_rate_mbps >= 20000)) {
            /* when running 20G and above rate, LBG have to configue same as 40G */
            if_rate_mbps = 40000;
        }
        ...
        if(interface == SOC_PORT_IF_CPU || interface == SOC_PORT_IF_OAMP) {
            ...
            /* for qax and later devices, recalculate cancel_en according to core clock */
            ...
            if (SOC_IS_QAX(unit) && if_rate_mbps <= 50000) {
                ...
                if (if_rate_mbps <= 30000) {
                    /** cancel_en 2 and 3 should be set for ports below 40G only */
        ...
        if ((is_skip_egq_for_lb != 1)) {
            ...
            /* enable port queues, must be done after ppct_tbl update */
           ...
           (is_skip_nif_for_lb != 1)) {
            if(interface == SOC_PORT_IF_ILKN) {
                ...
                if (ilkn_tdm_dedicated_queuing == ARAD_MGMT_ILKN_TDM_DEDICATED_QUEUING_MODE_ON) {
                        SOC_SAND_SET_BIT(egr_if, 0, 0); /* write the high priority(tdm) egr_if to NBIH */
                }
                /* protocol offset */
    ...
    SOC_SAND_IN uint32 profile)
{
    ...
    if ((interface == SOC_PORT_IF_ILKN) && (ilkn_tdm_dedicated_queuing == ARAD_MGMT_ILKN_TDM_DEDICATED_QUEUING_MODE_ON)) {
        SOC_SAND_SET_BIT(egr_if, 0, 0); /*we should config the high priority(tdm) egr_if first*/
    }
    SOCDNX_IF_ERR_EXIT(READ_EGQ_PER_IFC_CFGm(unit, EGQ_BLOCK(unit ,core), egr_if, data));
    th_profile = profile;
...
/*********************************************************************
*     Initialize OAM-TS header for timestamp and ssp application.
*********************************************************************/
...
/*********************************************************************
*     Initialize timestamp and ssp application.
*********************************************************************/
