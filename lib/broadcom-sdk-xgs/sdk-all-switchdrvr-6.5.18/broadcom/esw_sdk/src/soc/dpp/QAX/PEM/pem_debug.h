...
#define PEM_DEBUG_ARRAY_MAX_SIZE                8       /*that is header offsets array and qualifiers array*/
#define PEM_DEBUG_DBUS_SIZE_IN_BITS             768
#define PEM_DEBUG_MAX_FIFO_ENTRY_SIZE_IN_BITS   2824    /*that is the the largest packet size for all stages (load or update)*/
...
typedef struct pem_load_dbus_desc_s{
  ...
  int                field_msb_on_dbus;   /*what is the msb of the field on dbus - not all bits appear on dbus if not needed*/
...
/* 
* debug data for load and update dbus per context
*/
...
/* info concerning registers and memories to read from design
*/
...
/* structure per stage
*  holds the fifo desc for stage and the dbus (load+update) desc per context
*/
...
/*
* this is structure which holds the debug database for all stages
*/
...
/*first call 'read_fifo_and_dbuses' to read from design, then the 3 following functions to get the relevant data*/
