    ...
    if ( flags & ARAD_PMF_SEL_ALLOC_ADVANCED_MODE ) {
       ...
       /* get current level 1 info */
        ...
        if (!(flags & ARAD_PMF_SEL_ALLOC_CHECK_ONLY)) {
            ...
            /*if preselector was created with program ID we do not want to overwrite the prog_id with the default prog id (7)*/
            ...
            /* here the prog_id is overwrite with the default ether prog id (7)*/
            ...
            if (is_with_prog_id) {
                ...
                /** Added requested program ID to the allocated program resource bitmap */
...
)
{
    ...
    if (ethernet_or_tm_only == 1) {
        ...
        if (stage == SOC_PPC_FP_DATABASE_STAGE_INGRESS_PMF) {
            ...
            /* WA on JR+/QAX for raw port to create pmf group by user */
  ...
  )
{
  ...
  if ((stage == SOC_PPC_FP_DATABASE_STAGE_INGRESS_PMF) && (soc_property_get(unit, spn_FIELD_PRESEL_MGMT_ADVANCED_MODE, FALSE)) && (!is_for_tm) ) 
  {
    ...
    *success = (nof_pmf_pgms_needed <= nof_pmf_pgms_free)? TRUE: FALSE;
    ...
    /*in case field group was created with program ID skip this check*/
...
/*
 * Iterates through all the pre-selectors of the database, and
 * returns the program of those pre-selectors who has a matching 
 * qualifier.
 * Works in advanced preselector management mode.
 */
uint32
  ...
  )
{
    ...
    /* find pre-selectors which has matching qualifier */
    pfg_info_current.stage = stage;
    for (pfg_ndx = 0; pfg_ndx < SOC_PPC_FP_NOF_PFGS_ARAD; pfg_ndx++) {
            ...
            if (pfg_info_current.is_array_qualifier) {
                for (qual_type_ndx=0; qual_type_ndx<SOC_PPC_FP_NOF_QUALS_PER_DB_MAX; ++qual_type_ndx) {
                    if (pfg_info_current.qual_vals[qual_type_ndx].type == qual->type) {
                        /* consider it as a match if pfg_info_current has all significant bits set, and
                        * pfg_info_current.qual_vals.val & qual->is_valid == qual->val & qual->is_valid
                        */
    ...
    /* find programs bitmap for MPLS pre-selectors */
    pmf_pgm_match_bmp = 0;
    /* traverse over all levels */
    for(level_indx = 0; level_indx < ARAD_PMF_NOF_LEVELS; ++level_indx) 
    {
