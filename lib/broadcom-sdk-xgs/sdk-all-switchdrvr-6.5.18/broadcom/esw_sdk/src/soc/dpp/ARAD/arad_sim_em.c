                    ...
                    )
{
    ...
    if (is_cr_supported == 0) {
        /* don't allocate LEM crash recovery transaction shadow */
        table_count--;
    }
    ...
    SOC_SAND_IN       uint32     prefix_size)
{
    ...
    uint32                      data_out[ARAD_PP_LEM_ACCESS_PAYLOAD_IN_UINT32S * SIM_EM_REMOVE_ALL_NOF_ENTRIES_FOR_TABLE_BLOCK_GET * 10 /*safety 10*/] = {0};
    ...
    while(!SOC_SAND_TBL_ITER_IS_END(&(block_range.iter))){
        ...
        for (i = 0; i < nof_entries; i++) {
            /* if prefix supplied - read the entry's prefix */
            if (prefix != -1) {
            ...
            /* if prefix supplied, remove only if entry's prefix match the supplied prefix */
