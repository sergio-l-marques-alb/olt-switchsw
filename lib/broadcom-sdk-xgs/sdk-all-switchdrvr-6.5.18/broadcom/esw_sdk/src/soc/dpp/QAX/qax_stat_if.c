...
uint32 soc_qax_stat_if_init_set(int unit, ARAD_INIT_STAT_IF *stat_if)
{
    ...
    /* enabling SIF */
    SOCDNX_IF_ERR_EXIT(READ_CGM_INITr(unit, &reg_val));
    soc_reg_field_set(unit,CGM_INITr, &reg_val, STAT_IF_INITf, 0);
    SOCDNX_IF_ERR_EXIT(WRITE_CGM_INITr(unit, reg_val));
...
uint32 soc_qax_stat_if_report_info_set(int unit, int nof_sif_ports, ARAD_INIT_STAT_IF *stat_if)
{
    ...
    switch (stat_if->stat_if_info[0].mode)
    {
        ...
        case SOC_TMC_STAT_IF_REPORT_MODE_BILLING_INGRESS:
            ...
            /* etpp mode counters cnfigurations */
        ...
        case SOC_TMC_STAT_IF_REPORT_QSIZE:
            ...
            /* set scrubber thresholds */
            /* register contain 16 thresholds, but the last one is not in used. only 15 thresholds allowed */
            for(threshold_index = 0; threshold_index < ARAD_STAT_IF_REPORT_THRESHOLD_LINES; threshold_index++) 
            {
                ...
                /* set sram packet descriptor buffer threshold */
                ...
                /* set dram buffer descriptor buffer */
    ...
    /* set number of reports per packet */
    switch (stat_if->stat_if_info[0].if_nof_reports_per_packet)
    {
        ...
        case SOC_TMC_STAT_IF_NOF_REPORTS_128:
            ...
            break;
        default:
            /* SOC_TMC_STAT_IF_NOF_REPORTS_8 not applicable in QAX */
    ...
    /* if working in billing mode in one of the ports, need to enable SIF per port */
    if (enable_per_port)
    {
    ...
    if (is_billing_sif_0)
    {    
        SOCDNX_IF_ERR_EXIT(READ_CGM_ENABLE_DYNAMIC_MEMORY_ACCESSr_REG32(unit, &reg_val));
        soc_reg_field_set(unit, CGM_ENABLE_DYNAMIC_MEMORY_ACCESSr, &reg_val, ENABLE_DYNAMIC_MEMORY_ACCESSf, 1);
        SOCDNX_IF_ERR_EXIT(WRITE_CGM_ENABLE_DYNAMIC_MEMORY_ACCESSr_REG32(unit, reg_val));
        /* Header delta profile is selected according to port and VOQ. driver will select.*/
        /* Here we select only profile 0. We also set only profile 0 with the right value*/
        for (index = 0; index < 256; index++)
        {
        ...
        /* set the HDR DELTA only to profile 0 */
        /* the selection is made according to: 3 Msb bits of CGM_VOQ_HCP'STAT_PROFILE  and 3 bits of CGM_IPP_HCP' STAT_PROFILE */
        /* it is set by deafult to 0, so only profile 0 will be selected */
        /* the delta is 4 in order to compensate the CRC that was removed by the device */        
    ...
    /* determine alway combine Q-size (SRAM + DRAM) */
