...
int arad_kbp_recover_txpcs_soft_reset_toggle(int unit, int core, int mdio_id)
{
    ...
    /* dummy read to avoid mdio lock up on OP */
    SOCDNX_IF_ERR_EXIT(arad_port_phy_reg_get(unit, mdio_id, flags, 0x02F00F, &data));
    sal_usleep(200000);
...
/* Note that only kbp_mdio_id[0] will be used because for OP, kbp_mdio_id[1] may be invalid */
int arad_kbp_op_link_stability_check(int unit, uint32 core, uint32 kbp_mdio_id, uint32 retries)
{
    ...
    for (i = 0; i < retries; ++i) {
        /* reset eye margin averages and toggle_txpcs indicator for each retry */
        hz_l_avg = 0;
        hz_r_avg = 0;
        vt_u_avg = 0;
        vt_d_avg = 0;
        ...
        if (i < retries - 1) {
            /* If any lane is a "dead_lane" i.e. any eye margin parameter was zero, then toggle OP PHMFIFO then toggle txpcs*/
            if (dead_lane) {
            ...
            if (toggle_txpcs) {
                ...
                /* Else the eye looks good.  Check JR+ link status. If link is not up, toggle TxPCS again */
                for (j = 0; j < 100; j++) {
                    sal_usleep(10000);
