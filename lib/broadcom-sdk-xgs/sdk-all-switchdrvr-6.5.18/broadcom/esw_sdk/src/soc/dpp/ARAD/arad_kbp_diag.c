/* $Id: arad_kbp_diag.c,v 1.50 Broadcom SDK $
    ...
    )
{
    ...
    if(print_ltr < 0) { /* Specific LTR is not requested; print tables */
        ...
        for ( idx = 0; 
              ...
              idx++) 
        {
            if(print_mode <= 0 && /* don't show dummy tables when using basic print */
        ...
        if(print_mode < 0) { /* print list of LTRs */
            ...
            return; /* nothing else requested to print */
        }
          ...
          idx++) 
    {
        ...
        if (ltr_config_info.valid) 
        {
            ...
            LOG_CLI((BSL_META_U(unit,
                                "\n")));
        } else if (print_ltr >= 0) { /* specific LTR was requested; show it's invalid */
            LOG_CLI((BSL_META_U(unit,
                                "\n\nInvalid LTR Index\n")));
        }
    }
...
/* Wrapper for kbp_device_dump diagnostic. Outputs DB and LTR configurations, keys and entries in XML format */
void
  ...
  )
{
    ...
    if(print_mode > 0) { /* show each result only when using advanced or debug print */
  ...
  )
{
    ...
    if (info->valid) 
    {
        ...
        for (index = 0, search_id = 0; search_id < num_of_kbp_searches; index++, search_id++)
        {
            ...
            if(print_mode <= 0 && /* don't show searches in dummy tables when using bacis print */
            ...
            /* Public LTRs do not have the private DBs configured inside them.
             * Check if the search index corresponds to the hadcoded private search and
             * print the info of the private table.
             */
            if(search_id == 0) {
            ...
            } else if(search_id == 3) { /* Public UC+RPF expects private RPF search to on index 3 */
            ...
            if(print_private_table != -1) {
                ...
                /* Increment index to print the next search which returns the same result */
                search_id++;
            }
    ...
    SOC_SAND_IN int                     entry_index)
{
    ...
    if(acl_indication == TRUE){
        ...
        for(i=0;i<data_len_bytes;i++) {
            /* calculate mask and print masked data because of issue with IPv6 UC DIP */
    ...
    SOC_SAND_IN  uint8                  *result)
{
    ...
    if(SOC_IS_ARADPLUS_AND_BELOW(unit)) {
        max_nof_device_searches = 4; /* MAX number of searches for Arad and Arad Plus */
    } else if(ARAD_KBP_IS_OP_OR_OP2) {
        max_nof_device_searches = 8; /* MAX number of searches for Optimus Prime and Optimus prime 2 */
    } else {
        max_nof_device_searches = 6; /* MAX number of searches for Jericho and above with NL12K */
    }
