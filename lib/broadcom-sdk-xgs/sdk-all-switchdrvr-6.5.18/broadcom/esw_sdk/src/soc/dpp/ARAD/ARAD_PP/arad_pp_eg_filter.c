...
/*
 * Same-interface filtering according to logical-interface ID while incoming and outgoing port-ID will be ignored
 *  (ports are equal) and (both interfaces are ports) or 
 *  (Ports are equal) and (both interfaces are not ports) and (interfaces are equal) or
 *  (Ports are not equal) and (both interfaces are not ports) and (interfaces are equal)
 */
#define ARAD_PP_INLIF_PROFILE_LOGICAL_INTF_SAME_FILTER  0xC202

/*
 * Same-interface filtering with 2 scope:
 * Device scope:
 *  (ports are equal) and (both interfaces are ports) or 
 *  (Ports are equal) and (both interfaces are not ports) and (interfaces are equal)
 * System scope:
 *  (Ports are not equal) and (both interfaces are not ports) and (interfaces are equal)
 */
  ...
  if (SOC_IS_JERICHO_PLUS(unit)) {
    ...
    if (SOC_IS_ARADPLUS(unit))
    {
        ...
        if (soc_property_get(unit, spn_BCM886XX_LOGICAL_INTERFACE_SAME_FILTER_ENABLE, 0))
        {
            ...
            res = MBCM_PP_DRIVER_CALL(unit, mbcm_pp_occ_mgmt_app_set,(unit, SOC_OCC_MGMT_TYPE_INLIF, SOC_OCC_MGMT_INLIF_APP_LOGICAL_INTF_SAME_FILTER, 1/*0b1*/, &inlif_profile_same_if_mask));
  ...
  )
{
    ...
    /*
     * Basic legacy mode 1,none of soc enabled
     * or Only spn_BCM886XX_LOGICAL_INTERFACE_BRIDGE_FILTER_ENABLE=1
     */
    if (!soc_property_get(unit, spn_BCM886XX_LOGICAL_INTERFACE_SAME_FILTER_ENABLE, 0)) {
    ...
    /*
     * Basic legacy mode 2:
     * Only spn_BCM886XX_LOGICAL_INTERFACE_SAME_FILTER_ENABLE=1
     */
    else if (!soc_property_get(unit, spn_BCM886XX_LOGICAL_INTERFACE_BRIDGE_FILTER_ENABLE, 0))
    {
        for (inlif_profile_idx = 0; inlif_profile_idx < SOC_OCC_MGMT_NOF_INLIF_PROFILES_TRANSFERED_TO_EGRESS; inlif_profile_idx++)
        {
            ...
            {
                /* If enable logical interface same filter for this inlif profile, set the filter as
                 *  (ports are equal) and (both interfaces are ports) or 
                 *  (Ports are equal) and (both interfaces are not ports) and (interfaces are equal) or
                 *  (Ports are not equal) and (both interfaces are not ports) and (interfaces are equal)
                 */
    ...
    /*
     * Advanced mode:
     * spn_BCM886XX_LOGICAL_INTERFACE_BRIDGE_FILTER_ENABLE=1
     * spn_BCM886XX_LOGICAL_INTERFACE_SAME_FILTER_ENABLE=1
     * In-LIF support device scope filter and system scope filter
     * 
     */
    else 
    {
