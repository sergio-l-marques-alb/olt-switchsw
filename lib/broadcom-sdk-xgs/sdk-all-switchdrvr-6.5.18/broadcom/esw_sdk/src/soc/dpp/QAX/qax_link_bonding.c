  ...
  )
{
    ...
    /* 2.1 get FIFO size and credit size, according to speed */
    if (speed <= 1000) {/* 1G */
        *fifo_size = 10;
        *credit_size = 20;
    } else if (speed <= 10000) {/* 10G */
        *fifo_size = 29;
        *credit_size = 58;
    } else if (speed <= 20000) {/* 20G */
        *fifo_size = 58;
        *credit_size = 116;
    } else if (speed <= 30000) {/* 30G */
        *fifo_size = 88;
        *credit_size = 174;
    }
  ...
  )
{
    ...
    /* set clock frequency for the timeout counters */
  ...
  )
{
    ...
    case 0:   /* Fall-through on purpose */
    case 2:   /* Fall-through on purpose */
    case 4:   /* Fall-through on purpose */
    case 6:   /* Fall-through on purpose */
    case 8:   /* Fall-through on purpose */
    case 10:  /* Fall-through on purpose */
    case 12:  /* Fall-through on purpose */
    case 14:  /* Fall-through on purpose */
    case 16:  /* Fall-through on purpose */
  ...
  )
{
    ...
    /* 2.1 get optimial value of credit and fifo size */
  ...
  )
{
    ...
    /* for QUX, segment mode is per lbg; for QAX, segment mode is global configuration */
    if (SOC_IS_QUX(unit)) {
        SOC_TMC_LB_VALUE_MAX_CHECK(lbg_id, SOC_TMC_LB_LBG_MAX, "lbg_id");
    }
    else {
  ...
  )
{
    ...
    LOG_VERBOSE(BSL_LS_SOC_LB, (BSL_META_U(unit, "%s: is_check_by_port(%d) port(%d) lbg(%d) speed(%d)\n"), 
        __FUNCTION__, is_check_by_port, port, lbg_id, speed)); 
...
/* Purpose: according to speed, update SW DB, FIFO size, Credit size */
soc_error_t
  ...
  )
{
    ...
    case 2:  /* Fall-through on purpose */
    case 3:  /* Fall-through on purpose */
    case 4:  /* Fall-through on purpose */
    case 5:  /* Fall-through on purpose */
    case 6:  /* Fall-through on purpose */
    case 7:  /* Fall-through on purpose */
    case 8:  /* Fall-through on purpose */
    ...
    if (SOC_IS_QUX(unit)) {
        /* get number of total shared buffer */
  ...
  )
{
    ...
    if (SOC_IS_QUX(unit)) {
        /* get number of guaranteed buffer */
  ...
  )
{
    ...
    /* get seq_num_width */
    if (SOC_IS_QUX(unit)) {
        /* return sequence number width of first modem, which belong to the lbg */
