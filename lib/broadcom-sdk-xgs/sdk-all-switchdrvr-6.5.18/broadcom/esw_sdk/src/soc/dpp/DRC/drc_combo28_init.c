...
/*
 * $Id: drc_combo28_init.c,v 1.1.2.12 Broadcom SDK $
...
int soc_dpp_drc_combo28_util_ps2ck(int unit, int mr, int t_ck, int val, uint32 *clk)
{
    ...
    if (val == 0x0) {
        /* If val is 0x0 number of CLK is also 0x0 */
        *clk = 0x0;
...
{
    /* values taken from dram vendors specs */
...
int soc_dpp_drc_combo28_init_dprc_init(int unit, int drc_ndx, soc_dpp_drc_combo28_info_t *drc_info)
{
    ...
    /* combo28_shmoo: Configure PHY PLL and wait for lock */
    ...
    /* put all dprc components out of reset. including aligner PRC and DDR-PHY (enabling write FIFO) */
...
int soc_dpp_drc_combo28_drc_clam_shell_cfg(int unit, int drc_ndx, soc_dpp_drc_combo28_info_t *drc_info) 
{
    ...
    /* 
     * Configure Shadow MRS for clam shell
     */
    for (mr_ndx = 0; mr_ndx < SOC_DPP_DRC_COMBO28_MRS_NUM_MAX; mr_ndx++) {
        ...
        rv = soc_dpp_drc_combo28_util_calm_shell_bits_swap(unit, drc_info->mr[mr_ndx], &field32_val);
        SOCDNX_IF_ERR_EXIT(rv);
        reg32_val = 0;
    ...
    /* AUX's Mux */
    reg32_val = 0x0;
    field32_val = 12; /* Making its output to be address bit 9 */
    soc_reg_field_set(unit, DRCA_AUXS_MUXr, &reg32_val, AUX_0_MUXf, field32_val);
    field32_val = 13; /* Making its output to be address bit 2 */
...
/*
 * Function:
 *       soc_dpp_drc_combo28_drc_addr_bank_swap
 * Purpose:
 *      Configure the registers according to the recived Swap data.
 * Parameters:
 *      unit          - Device Number
 *      drc_ndx       - Index of DRC to configure its registers
 *      drc_info      - all the relevant info regarding this DRC, including the wanted swap configuration
 * Returns:
 *      SOC_E_XXX
 */
int soc_dpp_drc_combo28_drc_addr_bank_swap(int unit, int drc_ndx, soc_dpp_drc_combo28_info_t *drc_info)
{
    ...
    /* Get relevant Reg according to drc_ndx */
    ...
    for (bit = 0; bit < SOC_DPP_DRC_COMBO28_NOF_ADDR_BANK_BITS_PER_INTERFACE ; ++bit) {
        /* Set values of swapping, if NO SWAP keep default value, count swaps */
    ...
    /* if any swapping had occured */
    if (swap_counter > 0) {
        /* Set activation bit of the swapping */
        soc_reg_above_64_field32_set(unit, DRCA_ADDR_BANK_MAPr, reg_above64_val, ADDR_MAP_ENf, 0x1); 

        /* Set relevant Reg according to drc_ndx */
...
int soc_dpp_drc_combo28_drc_dq_swap(int unit, int drc_ndx, soc_dpp_drc_combo28_info_t *drc_info) 
{
    ...
    uint32      reg32_val;
    uint32      field32_val;
    /* values for addr training taken from GDDR5 specs */
    ...
    /* this is to handle regular addr/bank bits */
    ...
    { 
        /* Calculate bit's new position after taking into account byte swapping */
    ...
    /* check if bytes 0-1 were swapped iwth byte 2-3, this is to handle DBI bits*/
...
int soc_dpp_drc_combo28_sw_state_init(int unit)
{
    ...
    /* init Dram RD CRC interrupt status array */
    ...
    /* init Dram WR CRC interrupt status array */
...
int soc_dpp_drc_combo28_info_prepare(int unit, soc_dpp_drc_combo28_info_t *drc_info)
{
    ...
    /* dqs_prem_1_ck = DqsWrPreamble1ck, DqsRdPreamble1ck */
    ...
    case SHMOO_COMBO28_DRAM_TYPE_DDR4:
        ...
        break;
    case SHMOO_COMBO28_DRAM_TYPE_GDDR5:
         SOCDNX_IF_ERR_EXIT(soc_dpp_drc_combo28_util_ps2ck(unit, 0, t_ck, 3900 /* 1.7 + 2.2 ns*/, &(drc_info->dram_int_param.c_pcb)));
        break;
    default:
...
int soc_dpp_drc_combo28_drc_general_register_set(int unit, int drc_ndx, soc_dpp_drc_combo28_info_t *drc_info, uint32 write_crc) 
{
    ...
    /* DRAM_COMPLIANCE_CONFIGURATION_REGISTER */
    ...
    /* 
     * BANK_GROUP_TOPO:
     *  0 : 2 groups X 4 banks. 8 Bank Total. DDR4 (Default)
     *  1 : 4 groups X 2 banks. 8 Bank Total. DDR4
     *  2 : 4 groups X 4 banks. 16 Bank Total. GDDR5
     *  3 : Reserved
     */
    ...
    /* Should be aligned to STOP_MMU_PIPE_INSIDE_DPRC in MMU (MMU_GENERAL_CONFIGURATION_REGISTER). Both set to 0x0, in GDDR5 with dynamic calibration both need to be set to 1 */
    field32_val = (drc_info->dram_type == SHMOO_COMBO28_DRAM_TYPE_GDDR5) ? 1 : 0;
    ...
    /* EVENT_TRIGGER_ON_PARITY_PAD_CONFIG */
...
int soc_dpp_drc_combo28_drc_ac_op_register_set(int unit, int drc_ndx, soc_dpp_drc_combo28_info_t *drc_info, uint32 write_crc) 
{
    ...
    soc_reg_field_set(unit, DRCA_AC_OPERATING_CONDITIONS_5r, &reg32_val, DD_RT_ZQCSf, field32_val);
     if ( drc_info->dram_type == SHMOO_COMBO28_DRAM_TYPE_DDR4 ) {
         ...
         field32_val = 9 /* drc2phy_data_latency */ + ((drc_info->dram_int_param.bl + write_crc + tmp_val) / 2) + 3 /* phy2drc_alert_latency */;
     } else {
         field32_val = 0; /* Irrelevant for gddr5 */
     }
...
int soc_dpp_drc_combo28_mr_register_info_prepare(int unit, soc_dpp_drc_combo28_info_t *drc_info)
{
    ...
    case SHMOO_COMBO28_DRAM_TYPE_DDR4:
        ...
        /* MR0 */
        drc_info->mr[0]  = 0x0;
        drc_info->mr[0] |= SOC_SAND_SET_BITS_RANGE(SOC_SAND_GET_BITS_RANGE(cas_latency_calc,4,4), 12, 12);  /* CAS Latency */
        drc_info->mr[0] |= SOC_SAND_SET_BITS_RANGE(drc_info->dram_int_param.wr_recovery_op, 11, 9);         /* Write recovery */
        drc_info->mr[0] |= SOC_SAND_SET_BITS_RANGE(0x1, 8, 8);                                              /* Reset DLL. 0 = NO 1 = Yes */
        drc_info->mr[0] |= SOC_SAND_SET_BITS_RANGE(0x0, 7, 7);                                              /* MR0_test_mode. 0 = Normal 1 = Test */
        drc_info->mr[0] |= SOC_SAND_SET_BITS_RANGE(SOC_SAND_GET_BITS_RANGE(cas_latency_calc,3,1), 6, 4);    /* CAS Latency */
        drc_info->mr[0] |= SOC_SAND_SET_BITS_RANGE(0x0, 3, 3);                                              /* Read burst type. 0 = Sequential 1 = Interleave */
        drc_info->mr[0] |= SOC_SAND_SET_BITS_RANGE(SOC_SAND_GET_BITS_RANGE(cas_latency_calc,0,0), 2, 2);    /* CAS Latency */
        drc_info->mr[0] |= SOC_SAND_SET_BITS_RANGE(mrs_brst_lng, 1, 0);                                     /* Burst length. 00=8(Fixed), 01=BC4 or 8(on the fly), 10=BC4(Fixed), 11=Reserved */

        /* MR1 */
        drc_info->mr[1]  = 0x0;
        drc_info->mr[1] |= SOC_SAND_SET_BITS_RANGE(0x0, 12, 12);                         /* Qoff, Output buffer. 0 = enabled, 1 = disabled */
        drc_info->mr[1] |= SOC_SAND_SET_BITS_RANGE(0x0, 11, 11);                         /* TDQS. 0 = Disable 1 = Enable */
        drc_info->mr[1] |= SOC_SAND_SET_BITS_RANGE(0x0, 10, 8);                          /* RTT_NOM. 0x0 - Off*/
        drc_info->mr[1] |= SOC_SAND_SET_BITS_RANGE(0x0, 7, 7);                           /* Write Leveling. 0 = Disable 1 = Enable */
        drc_info->mr[1] |= SOC_SAND_SET_BITS_RANGE(drc_info->dram_int_param.c_al, 4, 3); /* Additive Latency. 00=0(AL disabled), 10=CL-2, 01=CL-1, 11=Resrved */
        drc_info->mr[1] |= SOC_SAND_SET_BITS_RANGE(ods, 2, 1);                           /* Output Driver Impedance Control */
        drc_info->mr[1] |= SOC_SAND_SET_BITS_RANGE(0x1, 0, 0);                           /* DLL. 0 = Disable 1 = Enable. */

        /* MR2 */
        drc_info->mr[2]  = 0x0;
        drc_info->mr[2] |= SOC_SAND_SET_BITS_RANGE(write_crc, 12, 12); /* Write CRC. 0 = Disable 1 = Enable */
        drc_info->mr[2] |= SOC_SAND_SET_BITS_RANGE(0x0, 10, 9);                                 /* RTT_WR. 0 - disabled, 1 - RZQ/2 = 120Ohm, 2 - RZQ/4 =60Ohm */
        drc_info->mr[2] |= SOC_SAND_SET_BITS_RANGE(0x0, 7, 6);                                  /* Low Power Array Self Refresh (LP ASR). 00=Manual Mode (Normal Operaing Temperature Range) */
        drc_info->mr[2] |= SOC_SAND_SET_BITS_RANGE(wr_latency_calc, 5, 3);                      /* CAS Write Latency(CWL) */

        /* MR3 */
        drc_info->mr[3]  = 0x0; 
        drc_info->mr[3] |= SOC_SAND_SET_BITS_RANGE(0x1, 12, 12);               
        drc_info->mr[3] |= SOC_SAND_SET_BITS_RANGE(0x0, 8, 6);                      /* Fine Granularity Refresh Mode. 0 - Normal */
        drc_info->mr[3] |= SOC_SAND_SET_BITS_RANGE(0x1, 5, 5); 
        drc_info->mr[3] |= SOC_SAND_SET_BITS_RANGE(drc_info->gear_down_mode, 3, 3); /* Geardown Mode. 0 = 1/2 Rate 1 = 1/4 Rate */

        /* MR4 */
        drc_info->mr[4]  = 0x0;
        drc_info->mr[4] |= SOC_SAND_SET_BITS_RANGE(SOC_SAND_NUM2BOOL_INVERSE(drc_info->dram_int_param.dqs_prem_1_ck), 12, 12); /* Write Preamble. 0 = 1 nCK 1 = 2 nCK */
        drc_info->mr[4] |= SOC_SAND_SET_BITS_RANGE(SOC_SAND_NUM2BOOL_INVERSE(drc_info->dram_int_param.dqs_prem_1_ck), 11, 11); /* Read  Preamble. 0 = 1 nCK 1 = 2 nCK */

        /* MR5 */
        drc_info->mr[5]  = 0x0;
        drc_info->mr[5] |= SOC_SAND_SET_BITS_RANGE(drc_info->read_dbi, 12, 12);  /* Read DBI. 0 = Disable 1 = Enable */
        drc_info->mr[5] |= SOC_SAND_SET_BITS_RANGE(drc_info->write_dbi, 11, 11); /* Write DBI. 0 = Disable 1 = Enable */
        drc_info->mr[5] |= SOC_SAND_SET_BITS_RANGE(0x1, 8, 6);                   /* RTT_Park. ZQ/4 */
        drc_info->mr[5] |= SOC_SAND_SET_BITS_RANGE(cmd_par_latency_calc, 2, 0);  /* C/A Parity Latency Mode */

        /* MR6 */
        drc_info->mr[6]  = 0x0;
        drc_info->mr[6] |= SOC_SAND_SET_BITS_RANGE(ccd_l_calc, 12, 10); /* tCCD_L */
        drc_info->mr[6] |= SOC_SAND_SET_BITS_RANGE(0x10, 5, 0);

        /* The Following MRS are GDDR5 only */
        drc_info->mr[7]  = 0x0;
        drc_info->mr[8]  = 0x0;
        drc_info->mr[12]  = 0x0;
        drc_info->mr[14]  = 0x0;
    ...
    case SHMOO_COMBO28_DRAM_TYPE_GDDR5:
        ...
        /* MR0 */
        drc_info->mr[0]  = 0x0;
        drc_info->mr[0] |= SOC_SAND_SET_BITS_RANGE(drc_info->dram_int_param.wr_recovery_op, 11, 8); /* Write recovery */
        drc_info->mr[0] |= SOC_SAND_SET_BITS_RANGE(cas_latency_calc, 6, 3);                         /* CAS Latency */
        drc_info->mr[0] |= SOC_SAND_SET_BITS_RANGE(wr_latency_calc, 2, 0);                          /* CAS Write Latency(CWL) */

        /* MR1 */
        drc_info->mr[1]  = 0x0;
        drc_info->mr[1] |= SOC_SAND_SET_BITS_RANGE(0, 11, 11);                                            /* PLL Reset */
        drc_info->mr[1] |= SOC_SAND_SET_BITS_RANGE(SOC_SAND_NUM2BOOL_INVERSE(drc_info->abi), 10, 10);     /* Write Preamble. 0 = 1 nCK 1 = 2 nCK */
        drc_info->mr[1] |= SOC_SAND_SET_BITS_RANGE(SOC_SAND_NUM2BOOL_INVERSE(drc_info->write_dbi), 9, 9); /* Write DBI. */
        drc_info->mr[1] |= SOC_SAND_SET_BITS_RANGE(SOC_SAND_NUM2BOOL_INVERSE(drc_info->read_dbi), 8, 8);  /* Read DBI. */
        drc_info->mr[1] |= SOC_SAND_SET_BITS_RANGE(0x0, 7, 7);                                            /* PLL Enable */
        drc_info->mr[1] |= SOC_SAND_SET_BITS_RANGE(SOC_SAND_NUM2BOOL(drc_info->dram_auto_calibration_update_disable), 6, 6);  /* calibration update. 1 - disable 0 - enabe */
        drc_info->mr[1] |= SOC_SAND_SET_BITS_RANGE(0x1, 5, 4);                                              /* ADDR/CMD termination. 0 CKE on reset, 1 ZQ , 2 ZQ/2 , Vendor */
        drc_info->mr[1] |= SOC_SAND_SET_BITS_RANGE(0x1, 3, 2);                                              /* DATA termination. 0 disable, 1 ZQ/2 , 2 ZQ */
        drc_info->mr[1] |= SOC_SAND_SET_BITS_RANGE(0x0, 1, 0);                                              /* output drive strength 0 auto calib. 1 vendor, 2 nominal */
        ...
        /* MR3 */
        drc_info->mr[3]  = 0x0;
        drc_info->mr[3] |= SOC_SAND_SET_BITS_RANGE(SOC_SAND_GET_BITS_RANGE(ccd_l_calc, 1, 0), 11, 10); /* tCCD_L */
        drc_info->mr[3] |= SOC_SAND_SET_BITS_RANGE(0x1, 9, 8);                                         /* WCK Termination. 0 disable, 1 ZQ/2, 2 ZQ */
        drc_info->mr[3] |= SOC_SAND_SET_BITS_RANGE(0, 5, 5);                                           /* Read DQS Mode. 0 disable */
        drc_info->mr[3] |= SOC_SAND_SET_BITS_RANGE(0, 4, 4);                                           /* WCK2CK Training. 0 disable */

        /* MR4 */
        drc_info->mr[4]  = 0x0;
        drc_info->mr[4] |= SOC_SAND_SET_BITS_RANGE(0, 11, 11);                                                             /* EDC Hold Pattern Invert for EDC1 + EDC3. 0 - do not invert one byte of edc */
        drc_info->mr[4] |= SOC_SAND_SET_BITS_RANGE(SOC_SAND_NUM2BOOL_INVERSE(write_crc), 10, 10); /* Write DBI. */
        drc_info->mr[4] |= SOC_SAND_SET_BITS_RANGE(SOC_SAND_NUM2BOOL_INVERSE(drc_info->read_crc), 9, 9);    /* Read DBI. */
        drc_info->mr[4] |= SOC_SAND_SET_BITS_RANGE(crc_rd_latency_calc, 8, 7);                                             /* CRC Read Latency (CRCRL) */
        drc_info->mr[4] |= SOC_SAND_SET_BITS_RANGE(crc_wr_latency_calc, 6, 4);                                             /* CRC Write Latency (CRCWL) */
        drc_info->mr[4] |= SOC_SAND_SET_BITS_RANGE(0xf, 3, 0);                                                             /* EDC Hold Pattern */

        /* MR5 */
        drc_info->mr[5]  = 0x0;
        if(drc_info->t_ras_enable){
            drc_info->mr[5] |= SOC_SAND_SET_BITS_RANGE(ras_calc, 11, 6);  /* when the ext_ram_t_ras_enable is 1 then the driver will use ext_ram_t_ras value other it will behave as before. */
        }
        drc_info->mr[5] |= SOC_SAND_SET_BITS_RANGE(0, 2, 2);          /* When bit A2 (LP3) is set, RDTR, WRTR and LDFF commands are not allowed while a REF command is being executed */
        drc_info->mr[5] |= SOC_SAND_SET_BITS_RANGE(0, 1, 1);          /* When bit A1 (LP2) is set, the WCK receivers may be turned off during power-down */
        drc_info->mr[5] |= SOC_SAND_SET_BITS_RANGE(0, 0, 0);          /* When bit A0 (LP1) is set, several GDDR5M SGRAM core parameters are relaxed for lower power consumption of the device. LP1 is optional. */

        /* MR6 */
        drc_info->mr[6]  = 0x0;
        drc_info->mr[6] |= SOC_SAND_SET_BITS_RANGE(5, 11, 8); /* vref offset A11:A8, VREFD OffsetBytes in rows A-F */
        drc_info->mr[6] |= SOC_SAND_SET_BITS_RANGE(5, 7, 4);  /* vref offset A7:A4,  VREFD OffsetBytes in rows M-U */
        drc_info->mr[6] |= SOC_SAND_SET_BITS_RANGE(1, 3, 3);  /* vref offset A7:A4,  VREFD OffsetBytes in rows M-U */
        drc_info->mr[6] |= SOC_SAND_SET_BITS_RANGE(0, 0, 0);  /* A0WCK2CK Alignment Pt. 0 - PD inside DRAM, 1 - PD at pins */

        /* MR7 */
        drc_info->mr[7]  = 0x0;
        drc_info->mr[7] |= SOC_SAND_SET_BITS_RANGE(0, 11, 10); /* Duty Cycle Correction (DCC) */
        drc_info->mr[7] |= SOC_SAND_SET_BITS_RANGE(0, 9, 8);   /* VDD Range */
        drc_info->mr[7] |= SOC_SAND_SET_BITS_RANGE(0, 7, 7);   /* Half VFRED */
        drc_info->mr[7] |= SOC_SAND_SET_BITS_RANGE(0, 6, 6);   /* Temperature Sensor */
        drc_info->mr[7] |= SOC_SAND_SET_BITS_RANGE(1, 5, 5);   /* DQ Preamble */
        drc_info->mr[7] |= SOC_SAND_SET_BITS_RANGE(0, 4, 4);   /* WCK2CK Auto Sync */
        drc_info->mr[7] |= SOC_SAND_SET_BITS_RANGE(0, 3, 3);   /* Low Frequency Mode */

        /* MR8 */
        drc_info->mr[8]  = 0x0;
        drc_info->mr[8] |= SOC_SAND_SET_BITS_RANGE(SOC_SAND_GET_BITS_RANGE(drc_info->dram_int_param.wr_recovery_op ,4,4), 1, 1); /* WRITE Recovery MSB - Extra High Frequency */
        drc_info->mr[8] |= SOC_SAND_SET_BITS_RANGE(SOC_SAND_GET_BITS_RANGE(cas_latency_calc,4,4), 0, 0);                         /* CAS Latency MSB - Extra High Frequency */
        ...
        /* MR15 */
        drc_info->mr[15]  = 0x0;
        drc_info->mr[15] |= SOC_SAND_SET_BITS_RANGE(0, 10, 10); /* Address Training (ADT). 0 - Off, 1 - On */
        drc_info->mr[15] |= SOC_SAND_SET_BITS_RANGE(0, 9, 9);   /* MR0-14 Enable MF=1. 0 - Enabled, 1 - Disabled */
        drc_info->mr[15] |= SOC_SAND_SET_BITS_RANGE(0, 8, 8);   /* MR0-14 Enable MF=0. 0 - Enabled, 1 - Disabled */
...
/*
 * Function:
 *      soc_dpp_drc_combo16_init_dynamic_calibration
 * Purpose:
 *      configure initial configuration for dynamic calibration.
 * Parameters:
 *      unit      - Device Number
 *      drc_ndx   - DRC index
 * Returns:
...
int soc_dpp_drc_combo16_init_dynamic_calibration(int unit, int drc_ndx)
{
    ...
    /* Corresponding field in the MMU (MMU_GENERAL_CONFIGURATION_REGISTERr) should be also set accordingly */
...
/*
 * Function:
 *      soc_dpp_drc_combo28_init_dynamic_calibration
 * Purpose:
 *      configure initial configuration for dynamic calibration.
 * Parameters:
 *      unit      - Device Number
...
int soc_dpp_drc_combo28_init_drc_init(int unit, int drc_ndx, soc_dpp_drc_combo28_info_t *drc_info)
{
    ...
    /* combo28_shmoo: configure PHY registers */
#if defined(PLISIM)
    if (!SAL_BOOT_PLISIM)
#endif
    {
    ...
    /* Perfom DQ Swap according to user configurtion. */
    rv = soc_dpp_drc_combo28_drc_dq_swap(unit, drc_ndx, drc_info); 
    SOCDNX_IF_ERR_EXIT(rv);
    
    /* Perfom Addr Bank Swap according to user configurtion. */
    rv = soc_dpp_drc_combo28_drc_addr_bank_swap(unit, drc_ndx, drc_info); 
    SOCDNX_IF_ERR_EXIT(rv);
            
    /* Configure Dynamic calibration for GDDR5*/
    if (drc_info->dram_type == SHMOO_COMBO28_DRAM_TYPE_GDDR5) {
        if (SOC_IS_SINAI(unit))
        {
...
/*
 * Function:
 *       soc_dpp_drc_combo28_init_check_bist_result
 * Purpose:
 *      Checks the result of a performed shmoo by checking the combo28_bist_err_cnt_t that was produced.
 * Parameters:
 *      unit                  - Device Number
 *      dram_ndx              - Index of dram checked
 *      combo28_bist_err_cnt  - ptr to struct containing the error counter that were produced by the bist
 *      drc_info              - ptr to DRC info struct
 * Returns:
 *      if bist was successful - SOC_E_NONE
 *      if bist failed         - SOC_E_FAIL
 */
...
/*
 * Function:
 *       soc_dpp_drc_combo28_init_run_bist_unsafe
 * Purpose:
 *      Run BIST without checking input parameters, this bist run is not configurable and runs with hard coded run info
 * Parameters:
 *      unit      - Device Number
 *      drc_ndx   - Dram index to run shmoo on
 *      drc_info  - ptr to DRC info struct
 * Returns:
 *      SOC_E_XXX
 */
int soc_dpp_drc_combo28_init_run_bist_unsafe(int unit, int drc_ndx, soc_dpp_drc_combo28_info_t *drc_info)
{
    ...
    /* Check Result, If an error occured - bist failed */
...
/*
 * Function:
 *      soc_dpp_drc_combo28_init_gddr5_cdr_mechanism
 * Purpose:
 *      Initiate the CDR mechanism, perform initial run and configure monitoring params
 * Parameters:
 *      unit                        - Device Number
 *      drc_ndx                     - DRC index
