...
arad_link_port_fault_get(int unit, bcm_port_t link_id, uint32* flags)
{
  ...
  (*flags) = 0;
  SOCDNX_IF_ERR_EXIT(READ_RTP_LOCALLY_GENERATED_ACLr_REG64(unit, &reg_64val)); /* RTP_LINK_STATE_VECTORr in FE1600 */
  ...
  SOCDNX_IF_ERR_EXIT(READ_RTP_ACL_RECEIVEDr_REG64(unit, &reg_64val)); /* RTP_ACL_VECTORr in FE1600 */
  arr_64bits[1] = COMPILER_64_HI(reg_64val);
  arr_64bits[0] = COMPILER_64_LO(reg_64val);
  if(!SHR_BITGET(arr_64bits,link_id)) {
