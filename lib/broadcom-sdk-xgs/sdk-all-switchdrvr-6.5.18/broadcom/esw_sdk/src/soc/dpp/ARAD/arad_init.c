    ...
    SOC_SAND_IN int                         unit)
{
    ...
    soc_reg_above_64_val_t
        ...
        field_above_64;
    uint32 reg32_val;
    uint32 Ndiv_int = 120; /* AR+ legacy Ndiv value */
    uint32 Ch1_Mdiv = 150; /* AR+ legacy Mdiv value */
    ...
    if(soc_property_get(unit, spn_ARAD_20MHZ_BS, 0x0)) { /* AR+: to configure FW with 10MHz/4KHz bitclk/HB generation */
        Ndiv_int = 140; /* legacy Ndvi/Mdiv = 0.8 is also proper,but to have FW side identification for 20MHz BS PLL changes*/
        Ch1_Mdiv = 175;
    }
    ...
    SOC_SAND_IN  int                                unit)
{
    ...
    /* There are 2 types network interface(NNI & IBC) when enable IBC port function */
        ...
        SOC_PBMP_ITER(valid_port_bm, port){ 
            ...
            if (propval) {      
                if (sal_strcmp(propval, "TRUE") == 0) {
                    /* Set FLP PTC profile for IBC port */
                    arad_ARAD_PORTS_PROGRAMS_INFO_clear(&port_prog_info);
                    res = arad_ports_programs_info_get_unsafe(unit, port, &port_prog_info);
                    SOC_SAND_CHECK_FUNC_RESULT(res, 50, exit);
  ...
  )
{
...

  {
      ...
      /* set the total of the guaranteed VOQ resource */
    ...
    if (soc_property_suffix_num_get(unit, -1, spn_LOW_POWER, "fabric_mac", 1)) {
        ...
        for (quad = 0; quad < ARAD_NOF_ARAD_FABRIC_QUADS(unit); ++quad) {
            ...
            if (quad_disabled) {
                ...
                /* set the block to invalid since the block is in reset status */
            ...
            if (quad % 3 == 2) { /* If this is the last quad in a three quad FMAC */
                if (nof_disabled_quad == nof_disabled_quad_before_this_3 + 3) {
                    ...
                    /* set the block to invalid since the block is powered down*/
