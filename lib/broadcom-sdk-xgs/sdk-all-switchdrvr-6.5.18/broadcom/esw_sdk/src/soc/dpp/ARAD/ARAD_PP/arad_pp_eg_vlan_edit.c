  ...
  )
{
  ...
  if (SOC_IS_JERICHO_PLUS(unit)) {
      ...
      /* native EVEC and EVEC have the same fields, except for "packet_is_tagged_after_eve" (aka membership check type).*/
  ...
  /* We only support ESEM lookup: lif x VSI in native link layer.
     Therefore, we init the mapping table outlif profile -> esem lookup type
     with lif x VSI ESEM lookup */
    if (SOC_IS_JERICHO_PLUS(unit)) {
        ...
        /*Configuration for MEF-L2CP transparent profile mapping*/
        /* get is L2CP Profile bit allocated by occ manager */
        ...
        if (nof_bits != 0) {
         ...
         /* set outlif profiles containing the masked bit.
            Ex: mask: 001100 
            For xx11xx outlif profile values, we need to set L2CP Profiles */
  ...
  )
{
  ...
  /* PCP_DEI_PROFILE and NATIVE_PCP_DEI_PROFILE  are duplicated */
  if (SOC_IS_JERICHO_PLUS(unit)) {
