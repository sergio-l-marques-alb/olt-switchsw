    ...
    )
{
  ...
  /* Encode Key into buffer according to the following format:
   *
   * {FID address in 79:64, DIP-address in 63:32, SIP-address in 31:0}
   *
   */
    ...
    )
{
    ...
    /* Physical DB type of SOC_DPP_DBAL_SW_TABLE_ID_IPV4MC_KAPS is never LEM. */
  ...
  SOC_PPC_FP_QUAL_VAL
    ...
    if (SOC_IS_JERICHO(unit)) {
        ...
        if (!ARAD_KBP_IPV4_LOOKUP_MODEL_1_IS_IN_USE(unit)) {
            /* For KBP with ipmc_lookup_model=1 In-RIF is valid even if Source is masked */
            if ((route_key->inrif_valid == 1)&&(route_key->source.prefix_len != 32)) {
                SOC_SAND_SET_ERROR_CODE(ARAD_PP_FRWRD_IPV4_MC_ILLEGAL_DEST_TYPE_ERR,10,exit);
            }
        }
    ...
    )
{
  ...
  uint32 kaps_payload;
        ...
        if(route_key->prefix_len == 0) {
            ...
            if (is_scale && ARAD_KBP_ENABLE_IPV4_DC) {
                ...
                if ((vrf_ndx == 0) && (ARAD_KBP_ENABLE_IPV4_UC_PUBLIC)) {
                    ...
                    frwrd_table_id = ARAD_KBP_FRWRD_TBL_ID_IPV4_UC_RPF_0;
                    table_id = SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_FWD_KBP;
                    /* keep is_host information for kbp payload buffer decode */
    ...
    if (*found) {
        ...
        } else if(!ARAD_KBP_ENABLE_IPV4_MPLS_EXTENDED && SOC_IS_JERICHO(unit) && (flags & _BCM_L3_FLAGS2_RAW_ENTRY)) {
            /* Do not decode if raw entry */
