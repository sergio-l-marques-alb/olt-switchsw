...
/* $Id: qax_pp_api_eg_encap.c,v 1.22 Broadcom SDK $
...
#define SOC_QAX_EG_ENCAP_PER_PKT_HDR_COMP_NOF_VALUE_BITS    (PPC_API_EG_ENCAP_PER_PKT_HDR_COMP_NOF_VALUE_LSBS + 1 /* Sign indication */) 
...
/* Set Reserved label profile table in QAX and above.
   This function sets one entry in this table, pointed by a another entry in the push profile table.
   Each entry in this table holds information relevant for add a reserved label to the MPLS stack .*/
...
/* Get Reserved label profile table in QAX and above.
   This function gets one entry in this table, pointed by a another entry in the push profile table.
   Each entry in this table holds information relevant for add a reserved label to the MPLS stack .*/
...
soc_qax_eg_encap_header_compensation_per_cud_set(int unit, int cud_msb, int value){
    ...
    /* 
     *  Translate the input value into the field value, represented by a positive number in the LSBs, and a
     *      bit indication in the MSB.
     */
...
soc_qax_eg_encap_header_compensation_per_cud_get(int unit, int cud_msb, int *value){
    ...
    /* Translate the field value into an integer. */
    *value = field_val & SOC_QAX_EG_ENCAP_PER_PKT_HDR_COMP_VALUE_LSBS_MASK; /* The lsbs. */
