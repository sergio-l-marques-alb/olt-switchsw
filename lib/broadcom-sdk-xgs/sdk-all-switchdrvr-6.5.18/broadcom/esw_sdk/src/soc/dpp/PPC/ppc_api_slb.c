...
/* Print callback for an object. */
/* This callback should only handle the current type, not the base types. */
/* untyped_obj - The object to print. */
/* info - The info of the object type. */
/* Both parameters can be assumed to be non-NULL. */
...
struct _soc_ppc_slb_object_info_s {
  ...
  /* The type of the base class of the object. */
  SOC_PPC_SLB_OBJECT_TYPE base_class;

  /* Is object abstract. */
  uint8 is_abstract;

  /* If defined, then this method is used to print the object. */
  _soc_ppc_slb_object_print_callback print_cb;

};

/* Make sure the same number of objects is in the object list and in object types */
/* and that all types in the object list have an object type. { */
  enum {
...
/* Declare a print function for each object SOC_PPC_SLB_suffix { */
...
static _soc_ppc_slb_object_info _soc_ppc_slb_object_infos[] = { 
  ...
  #define OBJECT_INFO_GENERAL(suffix, parent_suffix, is_abstract)             \
    /* Initialize by index can't be used because of ISO C90. */               \
    /* Will be verified dynamically instead. */                               \
    /* [ SOC_PPC_SLB_OBJECT_TYPE_ ## suffix ] = */ {                          \
      SOC_PPC_SLB_OBJECT_TYPE_ ## suffix,         /* type */                  \
      sizeof(SOC_PPC_SLB_ ## suffix),             /* size */                  \
      "SOC_PPC_SLB_OBJECT_" #suffix,              /* name */                  \
      SOC_PPC_SLB_OBJECT_TYPE_ ## parent_suffix,  /* base_class */            \
      is_abstract,                                /* is_abstract */           \
      PRINT_CB(suffix),                           /* print_cb */              \
      }
  ...
  /* These must be ordered according to the order of the TYPE enum. */
...
/* Print functions. */
/* These must be here because they are used in object infos definition. */
/* In these functions it is assumed that the type is valid. */
/* These functions should only print contents of the object, not the name of the type. The base classes should not be printed. */
/* If a function is NULL then it is not used. */
...
/* Traverse the type tree executing a callback for each type this object is (includes the final SOC_PPC_SLB_OBJECT object). */
/* If return is an error then stop traverse. */
/* depth - The position in the tree. 0 - the object. n - base of class n - 1. */
  ...
  )
{
  ...
  /* Pass the final SOC_PPC_SLB_OBJECT info. */
...
STATIC uint32 _soc_ppc_slb_trv_cb_print(SOC_SAND_IN SOC_PPC_SLB_OBJECT *object, SOC_SAND_IN _soc_ppc_slb_object_info *info, int depth, void *opaque)
{
  ...
  /* If depth != 0 then this is a base class. */
  if (depth) {
...
uint32 soc_ppc_slb_init(void)
{
  ...
  /* Because of ISO C90 not allowing init by index, we verify array[type_num].type == type_num dynamically. */
  for (object_type_idx = 0; object_type_idx < SOC_PPC_SLB_NOF_OBJECT_TYPES; object_type_idx++) {
    /* If this fails, then some type was initialized out of order (i.e. array[type].type != type. */
