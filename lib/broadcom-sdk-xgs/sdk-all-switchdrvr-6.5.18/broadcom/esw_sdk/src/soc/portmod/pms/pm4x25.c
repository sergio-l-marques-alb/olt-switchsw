...
#define PM4x25_WB_BUFFER_VERSION        (14)
...
typedef enum pm4x25_fabric_wb_vars{
    ...
    pll_vco_rate /*VCO value of the PM*/
}pm4x25_wb_vars_t;
                                   ...
                                   uint32* rxcdr_clk)
{
    ...
    *rxcdr_clk *= 10;
    ...
    /*first calculate rx cdr clock, which is VCO/(40 * osr) */
    if (osr_mode == phymodOversampleMode1) {
        *txpi_clk = *rxcdr_clk;
    } else if (osr_mode == phymodOversampleMode2) {
        *txpi_clk = *rxcdr_clk / 2;
        *rxcdr_clk = *txpi_clk;
    }
