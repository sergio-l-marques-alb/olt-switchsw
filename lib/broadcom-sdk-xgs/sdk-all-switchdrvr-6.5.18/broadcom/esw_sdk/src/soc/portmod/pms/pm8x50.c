...
typedef enum pm8x50_wb_vars{
    ...
    coreisFlexeConfigured,   /* used for Pm8x50FlexE if the core is configured for FlexE port */
...
{
    pm_info_t pm_info;
    int port_is_pcs_bypased, core_is_flexe_configured;
...
static int pm8x50_periodic_callback(int unit, int port)
{
   ...
   if (rlm_enabled == 1) {
    ...
    if (rlm_done && an_done && ber_war_off) {
                       ...
                       const portmod_port_add_info_t* add_info)
{
    ...
    int core_is_flexe_configured = 0;
    ...
    core_is_flexe_configured = PORTMOD_PORT_ADD_F_FLEXE_CONFIG_GET(add_info) ? 1 : 0;
    rv = PM8x50_IS_CORE_FLEXE_CONFIGURED_SET(unit, pm_info, core_is_flexe_configured);
    _SOC_IF_ERR_EXIT(rv);
...
int pm8x50_port_detach(int unit, int port, pm_info_t pm_info)
{
    ...
    int is_core_flexe_configured = 0;
    ...
    _SOC_IF_ERR_EXIT(PM8x50_IS_CORE_FLEXE_CONFIGURED_GET(unit, pm_info, &is_core_flexe_configured));
    if (!is_core_flexe_configured){
                                  ...
                                  portmod_pm_vco_setting_t* vco_setting)
{
    ...
    _SOC_IF_ERR_EXIT(
            PM8x50_IS_CORE_FLEXE_CONFIGURED_GET(unit, pm_info, &core_is_flexe_configured));
    ...
    /* next check if Flexe configured is set */
    if (core_is_flexe_configured) {
    ...
    /** SW workaround for Jericho 2  HW issue */
...
int pm8x50_port_speed_config_set(int unit, int port, pm_info_t pm_info, const portmod_speed_config_t* speed_config)
{
     ...
     * workaround for DNX device only
     */
    if (PM_8x50_INFO(pm_info)->clock_buffer_disable_required) {
       tvco_pll_index = PM8X50_TVCO_PLL_INDEX_GET(pm_info);
       ovco_pll_index = (tvco_pll_index == 1)? 0: 1;
