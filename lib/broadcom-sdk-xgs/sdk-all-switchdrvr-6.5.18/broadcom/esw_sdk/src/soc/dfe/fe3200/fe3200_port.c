/*
 * $Id: fe3200_port.c,v 1.37.20.1 Broadcom SDK $
 *
...
soc_fe3200_port_soc_init(int unit)
{
    ...
    for (port_i = 0; port_i < SOC_FE3200_NOF_LINKS ; ++port_i) {
        ...
        if(!SHR_BITGET(&puc,SOC_FE3200_PORT_ECI_POWER_UP_CONFIG_STATIC_PLL_BIT) || SAL_BOOT_PLISIM) {
            /*if ECI_POWERUP_CONFIG[SOC_FE3200_PORT_ECI_POWER_UP_CONFIG_STATIC_PLL_BIT]==0 then LC PLL config is taken from CMIC regs*/
            switch(lcpll) {
            case 0:
            ...
            if(SOC_FE3200_PORT_PLL_CONFIG_OUT_125_MHZ_WORD_0 == pll_control) { /*control word for 125MHz*/
                SOC_INFO(unit).port_refclk_int[port_i] = 125;
            } else if(SOC_FE3200_PORT_PLL_CONFIG_OUT_156_25_MHZ_WORD_0 == pll_control){ /*control word for 156.25MHz*/
                SOC_INFO(unit).port_refclk_int[port_i] = 156;
            } else {
                SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG("Unrecognized LC-PLL value")));
            }
        } else {
            /*if ECI_POWERUP_CONFIG[SOC_FE3200_PORT_ECI_POWER_UP_CONFIG_STATIC_PLL_BIT]==1 then LC PLL config is taken from the PUC*/
            switch(lcpll) {
            case 0:
        ...
        if (SOC_DFE_IS_FE13_ASYMMETRIC(unit) || SOC_DFE_IS_FE2(unit)) /* if fe13 is in asymmetric mode || FE2 , need to change the clk frequence of changed quads to the values configured*/
        {
...
int soc_fe3200_core_address_get(int unit, int core_index, uint16 *phy_addr){
    ...
    /* 
      Bus 1 cores: 0-2, 9-11 
      Bus 2 cores  3-5, 12-14
      Bus 3 cores  6-8, 15-17
      Bus 4 cores  18-20, 27-29
      Bus 5 cores  21-23, 30-32
      Bus 6 cores  24-26, 33-35
     */
    bus_id = (core_index / 18) * 3 + 1 ; 
    bus_id += ((core_index % 9) / 3);
    /*port 0,4,8 for the low cores  in the bus, and 12-20 for high cores in the bus*/
...
soc_fe3200_port_first_link_in_fsrd_get(int unit, soc_port_t port, int *is_first_link, int enable)
{
    ...
    for (i=fsrd_block*SOC_DFE_DEFS_GET(unit, nof_links_in_fsrd) ; i<fsrd_block*SOC_DFE_DEFS_GET(unit, nof_links_in_fsrd) + SOC_DFE_DEFS_GET(unit, nof_links_in_fsrd); i++) /* create a mask for all the links in the fsrd*/
    {
...
/*
 * Function:
 *      soc_fe3200_dynamic_port_restore
 * Purpose:
...
soc_fe3200_port_burst_control_set(int unit, soc_port_t port, soc_dcmn_loopback_mode_t loopback)
{
    ...
    if (loopback == soc_dcmn_loopback_mode_mac_outer || loopback == soc_dcmn_loopback_mode_mac_async_fifo)
    {
        ...
        SOCDNX_IF_ERR_EXIT(MBCM_DFE_DRIVER_CALL(unit,mbcm_dfe_fabric_link_device_mode_get,(unit, port, 0/*tx*/, &link_mode)));
    ...
    if (SOC_DFE_IS_FE13_ASYMMETRIC(unit) || SOC_DFE_IS_FE2(unit)) 
    {
        if (is_first_link)
        {
            for (blk = fmac_first_block; blk < fmac_first_block +  SOC_DFE_DEFS_GET(unit, nof_quads_in_fsrd) ; blk++)
            {
                ...
                {
                    SOCDNX_IF_ERR_EXIT(READ_FSRD_SRD_QUAD_CTRLr(unit, 8, 2, &reg32_val)); /* 8*3 + 2 */
                ...
                {
                    SOCDNX_IF_ERR_EXIT(READ_FSRD_SRD_QUAD_CTRLr(unit, 11, 2, &reg32_val)); /* 11*3 + 2 */
                    soc_reg_field_set(unit, FSRD_SRD_QUAD_CTRLr, &reg32_val, SRD_QUAD_N_LCREF_ENf, 0);
