    ...
    uint32 flags)
{
    ...
    if (module == MULTITHREAD_ANALYZER_MODULE_ID) {
        /* 
         * don't enforce on multithread analyzer as we do want to log also during init deinit etc.
         */ 
        SHR_EXIT();
    }

    if (!dnxc_ha_is_access_disabled(unit,UTILEX_SEQ_ALLOW_SW_STATE)) {
        /* if tmp_allow flag is on, don't enforce verification rules */
        SHR_EXIT();
    }

    /* 
     * mark usage for multithread analyzer
     */
    ...
    /*
     * Skip verifications if DNXC_SW_STATE_ALLOW_WRITES_DURING_ALL flag is set.
     */
    ...
    uint32 flags)
{
     ...
     * In general, big allocations are only allowed if size of 
     * allocation is derived from a dnx data value. this is verified
     * by sw state autogenerated interface. We allow big allocations
     * for non-autogenerated internal sw state allocations and if it 
     * came for sw state journal as a reverse of a 'free' 
     */
    ...
    char *dbg_string)
{
    ...
    /*
     * check if we are attempting to allocate bigger size than permitted. Only if the variable is array.
     */
    if (nof_elements > 1)
    {
