    ...
    char *stamp)
{
    ...
    {
        DNXC_ROLLBACK_JOURNAL_IF_ERR_ASSERT_WITH_LOG(_SHR_E_INTERNAL, "sw state comparison journal ERROR: invalid entry data detected.\n%s%s%s", EMPTY,
                                 EMPTY, EMPTY);
    }
    ...
    char *stamp)
{
    ...
    {
        DNXC_ROLLBACK_JOURNAL_IF_ERR_ASSERT_WITH_LOG(_SHR_E_INTERNAL, "sw state journal ERROR: invalid entry data detected.\n%s%s%s", EMPTY,
                                 EMPTY, EMPTY);
    }
#endif /* DNX_ERR_RECOVERY_VALIDATION */
    ...
    char *stamp)
{
...
#ifdef DNX_ERR_RECOVERY_VALIDATION
    if (NULL == entry->data.mutex_take.ptr_mtx)
    {
        DNXC_ROLLBACK_JOURNAL_IF_ERR_ASSERT_WITH_LOG(_SHR_E_INTERNAL, "sw state journal ERROR: invalid entry data detected.\n%s%s%s", EMPTY,
                                 EMPTY, EMPTY);
    }
    ...
    SHR_IF_ERR_EXIT(dnxc_sw_state_dispatcher[unit][module_id].mutex_give
                    (unit, module_id, entry->data.mutex_take.ptr_mtx, DNXC_SW_STATE_JOURNAL_ROLLING_BACK));
    ...
    char *stamp)
{
...
#ifdef DNX_ERR_RECOVERY_VALIDATION
    if (NULL == entry->data.mutex_give.ptr_mtx)
    {
        DNXC_ROLLBACK_JOURNAL_IF_ERR_ASSERT_WITH_LOG(_SHR_E_INTERNAL, "sw state journal ERROR: invalid entry data detected.\n%s%s%s", EMPTY,
                                 EMPTY, EMPTY);
    }
    ...
    SHR_IF_ERR_EXIT(dnxc_sw_state_dispatcher[unit][module_id].mutex_take
                    (unit, module_id, entry->data.mutex_give.ptr_mtx, usec, DNXC_SW_STATE_JOURNAL_ROLLING_BACK));
    ...
    SW_STATE_HASH_TABLE_KEY * const key)
{
    ...
    if(NULL == hash_table)
    {
        DNXC_ROLLBACK_JOURNAL_IF_ERR_ASSERT_WITH_LOG(_SHR_E_INTERNAL, "sw state journal ERROR: Invalid hash table specified for journal entry.\n%s%s%s", EMPTY, EMPTY, EMPTY);

    }

    if(NULL == key)
    {
        DNXC_ROLLBACK_JOURNAL_IF_ERR_ASSERT_WITH_LOG(_SHR_E_INTERNAL, "sw state journal ERROR: Invalid hash table key specified for journal entry.\n%s%s%s", EMPTY, EMPTY, EMPTY);
    }
#endif /* DNX_ERR_RECOVERY_VALIDATION */
    ...
    uint8 is_cmp)
{
    ...
    entry.element_size = 0;
    entry.module_id = module_id;
    entry.data.mutex_take.ptr_mtx = ptr_mtx;
    ...
    uint8 is_cmp)
{
    ...
    entry.element_size = 0;
    entry.module_id = module_id;
    entry.data.mutex_give.ptr_mtx = ptr_mtx;
    ...
    uint8 is_disable)
{
    ...
    SHR_IF_ERR_EXIT(dnxc_rollback_journal_disabled_counter_change(unit, is_disable, DNXC_SW_STATE_ROLLBACK_JOURNAL_HANDLE, TRUE));
    SHR_IF_ERR_EXIT(dnxc_rollback_journal_disabled_counter_change(unit, is_disable, DNXC_SW_STATE_COMPARISON_JOURNAL_HANDLE, TRUE));
    ...
    uint8 is_disable)
{
    ...
    SHR_IF_ERR_EXIT(dnxc_rollback_journal_disabled_counter_change(unit, is_disable, DNXC_SW_STATE_COMPARISON_JOURNAL_HANDLE, FALSE));
    ...
    uint8 is_comparison)
{
    ...
    if(!is_comparison)
    {
        SHR_IF_ERR_EXIT(dnxc_rollback_journal_logger_state_change(unit, DNXC_SW_STATE_ROLLBACK_JOURNAL_HANDLE));
    }
    else
    {
        SHR_IF_ERR_EXIT(dnxc_rollback_journal_logger_state_change(unit, DNXC_SW_STATE_COMPARISON_JOURNAL_HANDLE));
    }
