/**
 * \file src/soc/dnxc/drv_dnxc_utils.c
 *
 * This file contains procedures used for 'hiding'
 * drv.h
 *
 */
    ...
    int type)
{
    ...
    ret = SOC_BLOCK_IN_LIST(blk, type);
    return (ret);
}
/*
 * See drv_dnxc_utils.h
    ...
    int reg)
{
    void *ret;
    /*
     * Return pointer to structure of type 'soc_reg_info_t'
     * (Note that input 'reg' is 'int' and we cast to 'soc_reg_t' here
     * This is so as to save the caller the knowledge of 'soc_reg_t')
     */
...
/* 
 * \brief -
 *   Procedure implemented to access SOC_INFO (SOC_CONTROL(unit)->info) on the various 
 *   'soc_ptype_t' elements such as 'fe', 'ge', 'ce', 'cde', ...
 * \param [in] unit -
 *   HW Identifier of the device to access.
 * \param [in] ptype -
 *   soc_ptype_e. Identifier of the specific element to get address of (on 'soc_info_t',
 *   within  soc_control[DEVICE ID].info)
 * \param [out] ptype_p_p -
 *   soc_ptype_t **. This procedure loads pointed memory by a pointer to specified element
 *   (within  soc_control[DEVICE ID].info) which is of type 'soc_ptype_t'.
 * \return
...
/** 
 * \brief - Procedure used to add port to soc info port bitmaps 
...
/** 
 * \brief - Procedure used to remove port to soc info port bitmaps 
 *          ptype - See SOC_INFO list of port types  
 */
shr_error_e
    ...
    int unit)
{
    ...
    ret = soc_dnx_soft_init(unit, 1);
    return (ret);
}
shr_error_e
dnx_drv_soc_read_jr2_hbm_temp(
    const int unit,
    ...
    uint32 *out_temp)
{
    ...
    ret = soc_read_jr2_hbm_temp(unit, hbm_number, out_temp);
    return (ret);
}
/* } */
#endif
