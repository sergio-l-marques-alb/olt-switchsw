/*
 * $Id: dnxc_intr_corr_act_func.c, v1 Broadcom SDK $
    ...
    dnxc_xor_mem_info * xor_mem_info)
{
    ...
    if (SOC_IS_DNX(unit))
    {
        ...
        /*
         * Not get xor memory, return error 
         */
...
/* See dnxc_mem_decide_corrective_action
    DCMN_ECC_PARITY_DC
    DCMN_ECC_ECC1B_DC,
    DCMN_ECC_ECC2B_DC
*/
    ...
    int isEcc1b)
{
    ...
    static char *str_ecc_action[] = {
        ...
        "EM_SOFT_RECOVERY",     /* 3 */
        "SOFT_RESET",   /* 4 */
        "SHADOW if Cached, Else ECC_1B_FIX",    /* 5 -- not used now */
        "SHADOW_AND_SOFT_RESET if Cached, Else HARD_RESET",     /* 6 */
        "SHADOW if Cached, Else HARD_RESET",    /* 7 */
        "HARD_RESET",   /* 8 */
        "XOR_FIX if Cached, Else No Action",    /* 9 */
        "MDB 1b Correct",       /* 10 */
        "MDB 2b Correct"        /* 11 */
    };
    ...
    mem_is_accessible = !(soc_mem_is_readonly(unit, mem) || soc_mem_is_writeonly(unit, mem));
    {
        ...
        {
            rc_ecc1b = 0;       /* DCMN_INT_CORR_ACT_NONE */
            if (mem_is_accessible && SOC_IS_DNXF(unit))
            {
                rc_ecc1b = 1;   /* DNXC_INT_CORR_ACT_ECC_1B_FIX */
            }
        ...
        else if (mem_is_accessible)
        {
            ...
            if (SOC_MEM_TYPE(unit, mem) == SOC_MEM_TYPE_XOR)
            {
                rc_ecc1b = 2;   /* DNXC_INT_CORR_ACT_XOR_ECC_1BIT_FIX */
            }
...
*      type                     - (IN)  SER error type
*      mem                    - (IN)  memory id
*      copyno                 -(OUT)  block
*      action_type          - (OUT)  corrective action
*      special_msg        - (OUT)  out message
*
* Returns:
*      _SHR_E_XXX
*/
...
*      special_msg           -(OUT)  out message info
*      p_corrective_action          - (OUT)  corrective action
*      shadow_correct_info        - (OUT)  memory information
*
* Returns:
*      _SHR_E_XXX
*/
...
* Purpose:
*     recover memory from shadow memory
* Parameters:
*      unit                   - (IN)  Device number
*      block_instance         - (IN)  block number
*      interrupt_id           - (IN)  interrupt number
*      shadow_correct_info_p -(IN)  correction information
*      msg                 - (IN)  output message
*
* Returns:
*      _SHR_E_XXX
*/
...
* Purpose:
*     correct SER error for XOR memory
* Parameters:
*      unit                   - (IN)  Device number
*      block_instance         - (IN)  block number
*      interrupt_id           - (IN)  interrupt number
*      xor_correct_info -(IN)  correct information
*      msg                 - (IN)  output message
*
* Returns:
*      _SHR_E_XXX
    ...
    char *msg)
{
    ...
    for (current_index = xor_correct_info->min_index; current_index <= xor_correct_info->max_index; current_index++)
    {
        ...
        for (sp_index = 0; sp_index < sp_banks_num; sp_index++)
        {
            ...
            {
                /*
                 * for ecc2bit or parity error, must recover data from cache 
                 */
...
* Purpose:
*     correct SER error for ECC 1bit error
* Parameters:
*      unit                   - (IN)  Device number
*      block_instance         - (IN)  block number
*      interrupt_id           - (IN)  interrupt number
*      ecc_1b_correct_info_p -(IN)  correct information
*      msg                 - (IN)  output message
*
* Returns:
*      _SHR_E_XXX
    ...
    char *msg)
{
    ...
    if (SOC_IS_DNX(unit))
    {
        SHR_IF_ERR_EXIT(dnx_redo_soft_reset_soft_init(unit, 0, 0));
    }
    else
#endif
    {
...
*      block_instance         - (IN)  block number
*      interrupt_id           - (IN)  interrupt number
*      correct_info           - (IN)  correct information
*      msg                    - (IN)  output message
*
* Returns:
*      _SHR_E_XXX
...
* Purpose:
*     correct SER error for ECC 2bit error
* Parameters:
*      unit                   - (IN)  Device number
*      block_instance         - (IN)  block number
*      interrupt_id           - (IN)  interrupt number
    ...
    char *msg)
{
    SHR_FUNC_INIT_VARS(unit);
    /*
     * Do nothing, should be handled by BdbFifoNotEmpty interrupt 
     */
    SHR_FUNC_EXIT;
}
...
* Purpose:
*     recover memory for RTP SLSCT memory
* Parameters:
*      unit                   - (IN)  Device number
*      block_instance         - (IN)  block number
*      interrupt_id           - (IN)  interrupt number
    ...
    char *special_msg)
{
    ...
    /*
     * Corrective action will be performed only if BCM_AND_USR_CB flag set or corrective action override flag not
     * overriden 
     */
...
* Purpose:
*      Output correction action decission
* Parameters:
*      unit                    - (IN)  Device number
*      block_instance   - (IN)  block number
*      interrupt_id        - (IN)  interrupt number
*      msg_print          - (IN)  message can be output
*      msg                   - (IN)  unused
*
* Returns:
*      _SHR_E_XXX
...
int
dnxc_decide_interrupt_action_tcam_query_failure(
    int unit,
    int block_instance,
    uint32 en_interrupt,
    dnxc_int_corr_act_type * p_corrective_action,
    char *special_msg)
{
    ...
    /*
     * Get query failure in which an error occured
     */
    ...
    for (ii = 0; ii < 8; ii++)
    {
        ...
        sal_strncat(special_msg, str, 20);
    }
    for (ii = 0; ii < 2; ii++)
    {
...
int
dnxc_decide_interrupt_action_crps_CounterOvfError(
    int unit,
    int block_instance,
    uint32 en_interrupt,
    dnxc_int_corr_act_type * p_corrective_action,
    char *special_msg)
{
    ...
    sal_snprintf(special_msg, DNXC_INTERRUPT_SPECIAL_MSG_SIZE - 1, "CRPS Counter overflow, block %d, address %d",
...
*      block_instance   - (IN)  block number
*      interrupt_id        - (IN)  interrupt number
*      msg                   - (IN)  message can be output
*      corr_act             - (IN)  corrective action
*      param1              - (IN)  used for caller input
*      param2              - (IN)  unused
*
* Returns:
*      _SHR_E_XXX
