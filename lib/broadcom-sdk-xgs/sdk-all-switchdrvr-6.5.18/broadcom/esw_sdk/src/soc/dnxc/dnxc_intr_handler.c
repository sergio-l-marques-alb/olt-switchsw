    ...
    void *userdata)
{
    ...
    else
    {
        ...
        if (dnxc_intr_interrupt_recurring_detect(unit, block_instance, interrupt_id))
        {
            ...
            rc = SOC_CONTROL(unit)->interrupts_info->interrupt_db_info[interrupt_id].func_arr_recurring_action(unit,
                                                                                                               block_instance,
                                                                                                               interrupt_id,
                                                                                                               info);
            SHR_IF_ERR_EXIT(rc);
        ...
        else
        {
            ...
            {
                char *info = SOC_CONTROL(unit)->interrupts_info->interrupt_db_info[interrupt_id].msg;
                rc = SOC_CONTROL(unit)->interrupts_info->interrupt_db_info[interrupt_id].func_arr(unit, block_instance,
                                                                                                  interrupt_id, info);
                SHR_IF_ERR_EXIT(rc);
            }
            else
