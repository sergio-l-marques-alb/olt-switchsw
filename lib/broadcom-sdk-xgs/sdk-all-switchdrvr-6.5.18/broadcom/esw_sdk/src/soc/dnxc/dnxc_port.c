    ...
    uint32_t * val)
{
    ...
    rv = soc_cmicx_miim_operation(cl45_user_data->unit, 0 /* is_write */ , 45, core_addr, reg_addr, &val16);
    (*val) = val16;
    ...
    soc_pbmp_t * pbmp)
{
    ...
    /** Get phy_access of a fabric link */
    ...
    int *single_pll_restriction_disable)
{
    ...
    *single_pll_restriction_disable =
    ...
    if (!*single_pll_restriction_disable)
    {
        ...
        *single_pll_restriction_disable = (SHR_BITGET(data, otp_bit) ? TRUE : FALSE);
    }
exit:
    SHR_FUNC_EXIT;
}
 ...
 * \param [in] bypass_lanes_bitmap -
 *   Bitmap of phys in bypass mode
 * \param [in] use_mutex -
    ...
    dnxc_core_address_get_f address_get_func)
{
    ...
    SHR_IF_ERR_EXIT(soc_dnxc_mdio_phy_access_get
                    (unit, &g_port_user_access[unit][core], addr, &pm_info.pm_specific_info.pm8x50_fabric.access,
                     &is_sim));
    ...
    if (SOC_IS_DNX(unit))
    {
        ...
        int pll_index = -1;
        int nof_fabric_plls = dnx_data_pll.pll3.fabric_pll_info_get(unit)->key_size[0];

        pll_index =
            (core * DNXC_PORT_FABRIC_LANES_PER_CORE) / (dnx_data_fabric.links.nof_links_get(unit) / nof_fabric_plls);
        ref_clk_dnx = dnx_data_pll.pll3.fabric_pll_get(unit, pll_index)->out_freq;
        if (ref_clk_dnx == DNX_SERDES_REF_CLOCK_BYPASS)
        {
            ref_clk_dnx = dnx_data_pll.pll3.fabric_pll_get(unit, pll_index)->in_freq;
        }
    ...
    for (core_port_index = 0; core_port_index < DNXC_PORT_FABRIC_LANES_PER_CORE; core_port_index++)
    {
        if (!SOC_WARM_BOOT(unit))
        {
            if (SOC_IS_DNX(unit))
            {
                ...
                /*
                 * Polarity for Fabric links can be configured in soc properties
                 * per link, so we converted the phy to link.
                 */
    ...
    dnxc_port_fabric_init_config_t * port_config)
{
    ...
    if (!SOC_WARM_BOOT(unit))
    {
        ...
        /*
         * pass to pm an array where all lanes are mapped (even lanes defined as unmapped by user) - to configure FMAC
         * and phymod without holes 
         */
 ...
 * Purpose:
 *      Get the lane (SerDes) rate from port resource
 * Parameters:
 *      unit       - (IN)   Unit number.
 *      resource   - (IN)   Port resource
 *      lane_rate  - (OUT)  Lane rate (MHz)
 * Returns:
 *      _SHR_E_xxx
 * Notes:
 */
    ...
    int *lane_rate)
{
    ...
    /*
     * Added to support compilations which exclude BCM_DNX_SUPPORT 
     */
    SOC_EXIT;
    ...
    soc_phy_control_t type)
{
    ...
    switch (type)
    {
        ...
        case SOC_PHY_CONTROL_PMD_DIAG_DEBUG_LANE_EVENT_LOG_LEVEL:
            break;
        /** case SOC_PHY_CONTROL_AN_GOOD_TRAP: */
        ...
        case SOC_PHY_CONTROL_PRBS_RX_STATUS:
            /** These PHY controls are needed when the port type is per lane Gport */
            if (IS_SFI_PORT(unit, port))
            {
        ...
        case SOC_PHY_CONTROL_EEE:
            ...
            break;
            /*
             * EYE margins, only supported for NRZ speed 
             */
    ...
    uint32 phy_data)
{
    ...
    else
    {
        if (flags & BCM_PORT_PHY_NOMAP)
        {
            ...
            rv = soc_cmicx_miim_operation(unit, 1 /* is_write */ , 45, phy_id, phy_reg,
                                          &phy_wr_data);
            SHR_IF_ERR_EXIT(rv);
        }
        else
...
shr_error_e
soc_dnxc_port_disable_and_save_enable_state(
    int unit,
    soc_port_t port,
    int *orig_enabled)
{
...
shr_error_e
soc_dnxc_port_restore_enable_state(
    int unit,
    soc_port_t port,
    int orig_enabled)
{
...
/* enable/disable fec bypass on control cells.
 * enabling fec bypass on control cells means that error correction (FEC) is not done on flow status and credit control cells.
 * disabling FEC bypass means to do error correction (FEC) on those cells.
 * this configuration is relevant only if the FEC is one of the RS FEC types */
shr_error_e
...
/* get if fec bypass on control cells is enabled or disabled.
 * enabling fec bypass on control cells means that error correction (FEC) is not done on flow status and credit control cells.
 * disabling FEC bypass means to do error correction (FEC) on those cells.
 * this configuration is relevant only if the FEC is one of the RS FEC types */
shr_error_e
