            ...
            switch(debug_signal->block_id)
            {
                ...
                case DBAL_ENUM_FVAL_PP_ASIC_BLOCK_FROM_ENGINE:
                {
                    ...
                    for(range_idx = 0; range_idx < debug_signal->range_num; range_idx++)
                    {
                        ...
                        do
                        {
                            SHR_SET_CURRENT_ERR(dnx_debug_pema_read(unit, core_id, direction, debug_signal->address[0].low,
    ...
    char to_n[RHNAME_MAX_SIZE])
{
     ...
     */
    SHR_CLI_EXIT_IF_ERR(sand_signal_output_find(unit, core, 0, "", stage_str, "", "Nasid",
...
/**
 * \brief Check if debig_signal matches filter criteria after passing block and stage filet in sand_signal_get
    ...
    debug_signal_t ** signal_p)
{
    ...
    match.to = to;
    match.name = name;
    match.flags = extra_flags | SIGNALS_MATCH_EXACT | SIGNALS_MATCH_NOCOND | SIGNALS_MATCH_ONCE | SIGNALS_MATCH_NOVALUE;
