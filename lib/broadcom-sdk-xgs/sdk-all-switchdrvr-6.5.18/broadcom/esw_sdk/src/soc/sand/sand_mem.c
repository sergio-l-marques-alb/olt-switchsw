  ...
  )
{
    ...
    if (soc_sand_is_emulation_system(unit))
    {
        ...
        sand_fill_table_with_entry(unit, mem, copyno, entry_data); 
    }
    else
#endif /* DNX_EMULATION_1_CORE */
    {
#ifdef DNX_MEM_BRINGUP_DONE
        SHR_IF_ERR_EXIT(sand_fill_table_with_entry(unit, mem, copyno, entry_data));
#else
        /* coverity[check_return:FALSE] */  
        sand_fill_table_with_entry(unit, mem, copyno, entry_data); 
#endif
...
#ifdef BCM_DNX_SUPPORT
        if ((mem == MDB_EEDB_ENTRY_BANKm) && (dnx_data_mdb.hw_bug.feature_get(unit, dnx_data_mdb_hw_bug_eedb_bank_traffic_lock))) {
            continue;
        }
#endif
...
{
    soc_reg_above_64_val_t regval;
    int reg_cmd, red_addr, reg_data, reg_wide;
    int i;
    soc_reg_access_info_t access_info;
    int cmic_blk;
    uint32 address;
    ...
    address = soc_mem_addr_get(unit, mem, array_index, blk_id, entry_index, &access_type);
    ...
    /** Verify last indirect operation over block is done */
    SHR_IF_ERR_EXIT(sand_mem_indirect_poll_done(unit, blk_id, 10000));
    ...
    SHR_IF_ERR_EXIT(soc_direct_reg_set(unit, cmic_blk, red_addr, 1, &address));
    ...
    _soc_mem_write_cache_update_range(unit, mem, array_index, array_index, blk_id, 
                                      entry_index, entry_index+nof_entries-1, value);
exit:
    SHR_FUNC_EXIT;
}
