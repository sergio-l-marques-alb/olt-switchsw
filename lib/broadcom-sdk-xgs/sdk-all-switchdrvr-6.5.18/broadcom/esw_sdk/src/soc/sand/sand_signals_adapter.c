/** \File: sand_signals_adapter.c
 * Misc Signal Related routines for adapter environment only
 */
/*
 * $Id: Exp $
        ...
        int flags)
{
    ...
    /** Has substitute means that signals are fetched through the substitute stage */
    ...
    while(retry_count)
    {
        ...
        /*
         * For request with retry make 2 attempts to have non-zero number of signals per stage
         * Use it only for requests with specific "from" in match
         */
        if((adapter_info.nof == 0) && (flags & SIGNALS_MATCH_RETRY))
        {   /** Free the info area and retry */
        ...
        {   /** Either no retry defined or success, leave the retry while loop */
            break;
        }
    }
    ...
    {
        /* No signals obtained from adapter for block */
        SHR_EXIT();
    }

    /*
     * Now traverse the buffer and extract stage names and ms_id
     */
    ...
    /** Is substitute means we need to search signals on peer ASIC stage to find match */
    if(cur_pp_stage->status & SIGNAL_STAGE_IS_SUBSTITUTE)
    {
        ...
        /** allocate space for potentially all signals, we have no idea how much there will be actually */
    ...
    for(i_sig = 0; i_sig < adapter_info.nof; i_sig++)
    {
        ...
        /*
         * Fetch signal size and if it is bigger than MAX skip the signal
         */
        ...
        /*
         * Make string null terminated because above macro assigns memory only
         */
        signal_name[name_length] = 0;
        /*
         * Now look for ASIC name paired with this adapter one
         */
        ...
        /*
         * Fill HW name with original adapter name
         */
        ...
        {
            /*
             * Not found - stick with original adapter one
             */
        ...
        device_t * device)
{
    ...
    for(i_ms = 0; (i_ms < stage_info_m.nof) && (stage_info_m.length > sizeof(uint32) * 2); i_ms++)
    {
        ...
        {
            /** No need to continue searching - stage found on previous block */
            if(stage_found == TRUE)
                break;
            ...
            for (i_st = 0; i_st < pp_block->stage_num; i_st++)
            {
                ...
                if(!sal_strcasecmp(pp_stage->adapter_name, adapter_name))
                {
                    ...
                    /*
                     * If stage is the last its to remains NULL
                     */
                    ...
                    /*
                     * Update to stage for peer one, that may be not initialized
                     */
