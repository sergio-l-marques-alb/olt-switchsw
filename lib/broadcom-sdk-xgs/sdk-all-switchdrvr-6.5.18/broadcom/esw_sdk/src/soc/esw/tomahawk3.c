...
#define BCM_TH3_MAX_LOGICAL_PORT_NUM    (160)
    ...
    for (port = 0; port < BCM_TH3_MAX_LOGICAL_PORT_NUM; port++) {
        if (si->port_speed_max[port] == 400000) {
            ...
            si->port_num_lanes[port] = 4;
                   ...
                   si->port_num_lanes[port] == -1) {
            /* if it's 100G and the user didn't specify the number of lanes,
             * choose 2 lanes (2 x 50G) */
            si->port_num_lanes[port] = 2;
        } else if (si->port_speed_max[port] == 50000 &&
                   si->port_num_lanes[port] == -1) {
            /* if it's 50G and the user didn't specify the number of lanes,
             * choose 1 lane (1 x 50G) */
            si->port_num_lanes[port] = 1;
    ...
    for (port = 0; port < BCM_TH3_MAX_LOGICAL_PORT_NUM; port++) {
        ...
        /* Verify speed for front panel port is valid */
...
{
    uint32 rval=0;
    int lane, lport, port, phy_port_base, block_info_idx;
    ...
    SOC_BLOCK_ITER(unit, block_info_idx, SOC_BLK_CDPORT) {
        ...
        for (lane = 0; lane < _TH3_PORTS_PER_PBLK; lane++) {
            ...
            if (lport != -1) {
                SOC_IF_ERROR_RETURN(WRITE_CDMAC_MIB_COUNTER_CTRLr(unit, lport, rval));
            }
        }
    }
    sal_usleep(1);
    SOC_BLOCK_ITER(unit, block_info_idx, SOC_BLK_CDPORT) {
        ...
        for (lane = 0; lane < _TH3_PORTS_PER_PBLK; lane++) {
            lport = si->port_p2l_mapping[phy_port_base + lane];
            if (lport != -1) {
                SOC_IF_ERROR_RETURN(WRITE_CDMAC_MIB_COUNTER_CTRLr(unit, lport, 0));
            }
        }
    }
                             ...
                             void *d2, void *d3, void *d4)
{
    ...
    if (rv != SOC_E_NONE) {
        LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META_U(unit, "soc_th3_process_pm_intr sw PM id for "
                                    ...
                                    (cmic_bit_num - 1)));
        /* Disable the specific PM interrupt */
        (void) soc_ser_top_intr_reg_enable(unit, cmic_reg_type,
                                           (cmic_bit_num - 1), 0);
        return;
    }
    ...
    /* Enable the specific PM interrupt */
    (void) soc_ser_top_intr_reg_enable(unit, cmic_reg_type,
                                       (cmic_bit_num - 1), 1);
...
soc_th3_pm_intr_handler(int unit, uint32 cmic_val, int reg_num)
{
    ...
    /* Disable the specific PM interrupt */
    (void) soc_ser_top_intr_reg_enable(unit, reg_num,
                                       (cmic_bit_num - 1), 0);

    sal_dpc(soc_th3_process_pm_intr, INT_TO_PTR(unit),
            INT_TO_PTR(cmic_bit_num), INT_TO_PTR(reg_num), 0, 0);
