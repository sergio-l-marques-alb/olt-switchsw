 ...
 *
 *  File:        hurricane4_mmu_port_up_sequence.c
 *  Purpose:
 *  Requires:
 */
...
/*! @fn int soc_hurricane4_mmu_port_up_sequence(int unit,
        ...
        int unit, soc_port_schedule_state_t *port_schedule_state_t)
{
    ...
    soc_hurricane4_mmu_get_bst_mode(unit, &bst_mode);
    soc_hurricane4_mmu_get_pktstat_mode(unit, &pktstat_mode);
    ...
    for (port=0; port<port_schedule_state_t->nport; port++) {
        if (port_schedule_state_t->resource[port].physical_port != -1) {
            ...
            soc_hurricane4_mmu_set_mmu_to_phy_port_mapping(
                    unit, &port_schedule_state_t->resource[port], port_schedule_state_t);

            /*  Reinitialize CT setting */
            /* CT not supported in HR4
             * soc_hurricane4_mmu_reinit_ct_setting(
            ...
            soc_hurricane4_mmu_clear_mtro_bucket_mems(
                    unit, &port_schedule_state_t->resource[port]);
            ...
            soc_hurricane4_mmu_wred_clr(unit,
                    &port_schedule_state_t->resource[port]);
            soc_hurricane4_mmu_thdi_setup(unit,
                    &port_schedule_state_t->resource[port],
                    lossy);
            ...
            soc_hurricane4_mmu_ctr_clr(unit,
                    &port_schedule_state_t->resource[port]);
            if (bst_mode == HR4_MMU_BST_WMARK_MODE) {
                /* Clear THDI BST in watermark mode..*/
                soc_hurricane4_mmu_thdi_bst_clr(
                        unit, &port_schedule_state_t->resource[port]);
                /* Clear THDU BST counters in watermark mode*/
                soc_hurricane4_mmu_thdu_bst_clr(
                        unit, &port_schedule_state_t->resource[port]);
                /* Clear THDM BST counters in watermark mode*/
                soc_hurricane4_mmu_thdm_bst_clr(
                        unit, &port_schedule_state_t->resource[port]);
            }
            if (pktstat_mode == HR4_PKTSTATS_ON) {
                /* Clear PktStat counters in THDU for Queues*/
                soc_hurricane4_mmu_thdu_pktstat_clr(
                        unit, &port_schedule_state_t->resource[port]);
            }
            ...
            if (port_schedule_state_t->resource[port].oversub == 1) {
                ...
                /*soc_hurricane4_mmu_reinit_dip_fsaf_setting(
                  unit, &port_schedule_state_t->resource[port]);*/
            }
        } else {
            soc_hurricane4_mmu_set_mmu_to_phy_port_mapping(
...
int
soc_hurricane4_flex_mmu_port_up(int                        unit,
        soc_port_schedule_state_t *port_schedule_state_t)
{
    int port;
    int pipe;
