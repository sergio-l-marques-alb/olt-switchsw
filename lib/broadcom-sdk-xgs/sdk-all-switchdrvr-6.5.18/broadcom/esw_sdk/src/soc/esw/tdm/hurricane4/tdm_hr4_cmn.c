...
/**
@name: tdm_hr4_cmn_get_port_pm
@param:

Return Port Macro number of the given port
**/
int
tdm_hr4_cmn_get_port_pm(int port_token, tdm_mod_t *_tdm)
{
    int port_pm;
...
**/
int
tdm_hr4_cmn_get_port_pm_chk(int port_token, tdm_mod_t *_tdm)
{
    int port_pm=0;
    ...
    if (_tdm->_chip_data.soc_pkg.pmap_num_lanes > 0 &&
        port_token >=_tdm->_chip_data.soc_pkg.soc_vars.fp_port_lo &&
        port_token <=_tdm->_chip_data.soc_pkg.soc_vars.fp_port_hi) {
	if(port_token > 56 ){ /* pm 4x10q sis port spacing is across 16 ports */ 
        port_pm = (port_token - 1) / _tdm->_chip_data.soc_pkg.pmap_num_lanes;
	}
	else if(port_token > 0 && port_token <9){
        port_pm = (port_token - 1) / 8 ;
	}else if(port_token > 8 && port_token <17){
        port_pm = ((port_token - 1) / 8) + 1 ;
	}else if(port_token > 16 && port_token <25){
        port_pm = ((port_token - 1) / 8) + 2 ;
  } else if(port_token >24 && port_token <41) {
        port_pm = ((port_token - 1) / 16) + 4;
	} else if(port_token >40 && port_token <57) {
        port_pm = ((port_token - 1) / 16) +5;
  }
...
/**
@name: tdm_hr4_cmn_get_port_speed
@param:

Return port speed of the given port
**/
int
tdm_hr4_cmn_get_port_speed(int port_token, tdm_mod_t *_tdm)
{
...
/**
@name: tdm_hr4_cmn_get_port_state
@param:

Return port state of the given port
**/
int
tdm_hr4_cmn_get_port_state(int port_token, tdm_mod_t *_tdm)
{
    if (port_token >= _tdm->_chip_data.soc_pkg.soc_vars.fp_port_lo &&
        port_token <= _tdm->_chip_data.soc_pkg.soc_vars.fp_port_hi) {
...
/**
@name: tdm_hr4_cmn_get_speed_lanes
@param:
...
**/
int
tdm_hr4_cmn_get_speed_lanes(int port_speed)
{
    int lanes = 0;
...
/**
@name: tdm_hr4_cmn_get_speed_slots
@param:
...
**/
int 
tdm_hr4_cmn_get_speed_slots(int port_speed)
{
    int slot_num=0;
...
/**
@name: tdm_hr4_cmn_get_pipe_port_range
@param:

Return physical port number range of the given pipe
**/
void 
tdm_hr4_cmn_get_pipe_port_range(int cal_id, int *phy_lo, int *phy_hi)
{
    int prt_lo = 0, prt_hi = 0;

    switch(cal_id){
        case 0: prt_lo = HR4_CAL_0_PORT_LO; prt_hi = HR4_CAL_0_PORT_HI; break;
        case 1: prt_lo = HR4_CAL_1_PORT_LO; prt_hi = HR4_CAL_1_PORT_HI; break;
        case 4: prt_lo = HR4_CAL_4_PORT_LO; prt_hi = HR4_CAL_4_PORT_HI; break;
        case 5: prt_lo = HR4_CAL_5_PORT_LO; prt_hi = HR4_CAL_5_PORT_HI; break;
        default: break;
    }
...
/**
@name: tdm_hr4_cmn_get_pipe_ethernet
@param:
...
**/
int 
tdm_hr4_cmn_get_pipe_ethernet(int cal_id, tdm_mod_t *_tdm) 
{
...
/**
@name: tdm_hr4_cmn_get_pipe_higig
@param:
...
**/
int 
tdm_hr4_cmn_get_pipe_higig(int cal_id, tdm_mod_t *_tdm)
{
    int result = BOOL_FALSE;

    /* check regular ports */
    if (tdm_hr4_cmn_get_pipe_ethernet(cal_id, _tdm) == BOOL_FALSE) {
        result = BOOL_TRUE;
    }
    /* check management ports 
    if (cal_id == HR4_IDB_PIPE_1_CAL_ID &&
        _tdm->_chip_data.soc_pkg.soc_vars.hr4.mgmt_pm_hg == BOOL_TRUE) {
        result = BOOL_TRUE;
    }*/
...
/**
@name: tdm_hr4_cmn_get_pipe_cal_len_max
@param:

Return the size of selected calendar (regardless of frequency/higig)
**/
int
tdm_hr4_cmn_get_pipe_cal_len_max(int cal_id, tdm_mod_t *_tdm)
{
    int cal_len = 0;
...
/**
@name: tdm_hr4_cmn_get_pipe_cal_len
@param:
...
**/
int
tdm_hr4_cmn_get_pipe_cal_len(int cal_id, tdm_mod_t *_tdm)
{
    int cal_len = 0;
    int param_freq;
    ...
    /* get calendar length */
    /*if (tdm_hr4_cmn_get_pipe_higig(cal_id, _tdm) == BOOL_FALSE) {
        switch (param_freq) {
            case HR4_CLK_496MHZ:  cal_len = HR4_CAL_1_LEN; break;
            case HR4_CLK_415MHZ:  cal_len = HR4_CAL_2_LEN; break;
            case HR4_CLK_495MHZ:  cal_len = HR4_CAL_3_LEN; break;
            case HR4_CLK_135MHZ:  cal_len = HR4_CAL_4_LEN; break;
            case HR4_CLK_445MHZ:  cal_len = HR4_CAL_5_LEN; break;
            case HR4_CLK_265MHZ:  cal_len = HR4_CAL_6_LEN; break;
            case HR4_CLK_200MHZ:  cal_len = HR4_CAL_7_LEN; break;
            case HR4_CLK_100MHZ:  cal_len = HR4_CAL_8_LEN; break;
            case HR4_CLK_325MHZ:  cal_len = HR4_CAL_9_LEN; break;
            case HR4_CLK_266MHZ:  cal_len = HR4_CAL_10_LEN; break;
            default:  break;
        }
    } else {*/
        switch (param_freq) {
    ...
    } else if (cal_len > tdm_hr4_cmn_get_pipe_cal_len_max(cal_id, _tdm)) {
        cal_len = 0;
...
/**
@name: tdm_hr4_cmn_get_same_port_dist
@param:

**/
int
tdm_hr4_cmn_get_same_port_dist(int slot, int dir, int *cal_main, int cal_len)
{
    int n, k, dist = 0;
...
/**
@name: tdm_hr4_cmn_get_speed_jitter
@param:

**/
void
tdm_hr4_cmn_get_speed_jitter(int port_speed, int cal_len, int *space_min, int *space_max)
{
