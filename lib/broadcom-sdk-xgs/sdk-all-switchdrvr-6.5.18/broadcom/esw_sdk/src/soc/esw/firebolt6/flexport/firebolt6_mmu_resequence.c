        ...
        int unit,soc_port_resource_t *port_resource_t, soc_port_schedule_state_t *port_schedule_state)
{
    ...
    if (port_resource_t->physical_port == -1) {
        ...
        mmu_port =
            port_schedule_state->in_port_map.port_l2i_mapping[port_resource_t->logical_port];
    }
    else {
    ...
    COMPILER_64_ZERO(rval);
    soc_reg64_field32_set(unit, reg, &rval, DEVICE_PORTf, port_resource_t->logical_port);
    /* BT:IPORT, AT: SINGLE, Block ID: MMU_GLOBAL */
    SOC_IF_ERROR_RETURN(soc_reg_rawport_set(unit, reg, inst, 0, rval));
        ...
        int pipe, int up_dwn)
{
    ...
    if(up_dwn)
      COMPILER_64_SET(cmp64, 0, FB6_MMU_FLUSH_OFF);
    else
      COMPILER_64_SET(cmp64, 0, FB6_MMU_FLUSH_ON);
    ...
    /*     if (port_schedule_state_t->resource[port].physical_port == -1) { */
            if (port_schedule_state_t->resource[port].pipe == pipe) {
                ...
                if (lcl_mmu_port < 64) {
                    ...
                    if (COMPILER_64_EQ(cmp64,temp64)) {
