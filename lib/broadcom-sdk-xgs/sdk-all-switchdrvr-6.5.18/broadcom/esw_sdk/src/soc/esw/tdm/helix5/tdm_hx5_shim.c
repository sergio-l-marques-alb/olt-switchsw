...
/**
@name: tdm_hx5_shim_get_port_pm
@param:

Return PM number to which the input port belongs
**/
int
tdm_hx5_shim_get_port_pm(tdm_mod_t *_tdm)
{
    return (tdm_hx5_cmn_get_port_pm(_tdm->_core_data.vars_pkg.port, _tdm));
}


/**
@name: tdm_hx5_shim_get_pipe_ethernet
@param:
...
**/
int
tdm_hx5_shim_get_pipe_ethernet(tdm_mod_t *_tdm)
{
    int result = BOOL_TRUE;

    if (_tdm->_core_data.vars_pkg.port<=64) {
        result = tdm_hx5_cmn_get_pipe_ethernet(0, _tdm);
    } else if (_tdm->_core_data.vars_pkg.port<=128){
        result = tdm_hx5_cmn_get_pipe_ethernet(1, _tdm);
    }
...
**/
int
tdm_hx5_shim_core_vbs_scheduler_ovs(tdm_mod_t *_tdm)
{
    if (_tdm->_chip_data.soc_pkg.flex_port_en) {
        /* Used for FlexPort */
        /* return tdm_hx5_vbs_scheduler_ovs_flex_port(_tdm); */
        /* return tdm_hx5_vbs_scheduler_ovs_flex_port_delta(_tdm) */;
