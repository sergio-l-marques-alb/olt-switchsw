...
*
* $File:  maverick2_mmu_port_resequence_up.c
*/
...
/*! @fn int soc_maverick2_mmu_reinit_ct_setting(int unit,
 ...
 */
int
soc_maverick2_mmu_reinit_ct_setting(int                   unit,
                                    ...
                                    int                   oversub_ratio)
{
    ...
    ct_decode_speed = soc_maverick2_get_ct_class(exact_speed);
    ...
    soc_maverick2_get_min_max_src_ct_speed(unit, ct_decode_speed, ct_mode,
                                           ct_profile, &min_speed,
                                           &max_speed);
    soc_maverick2_get_asf_xmit_start_count(unit, ct_decode_speed, ct_mode,
                                           &asf_count); /* mmu config apis*/
    /* CT Fifo Threshold & Depth from michael's e2e ct analysis slides */
    soc_maverick2_get_ct_fifo_threshold_depth(unit, ct_decode_speed, ct_mode,
    ...
    soc_maverick2_mmu_get_ct_ep_credit_low_high_threshold(
...
/*! @fn int soc_maverick2_mmu_ctr_clr(int unit,
 *              soc_port_resource_t *port_resource_t)
 *  @param unit Device number
 *  @param *port_resource_t Port Resource Struct
 ...
 */
int
soc_maverick2_mmu_ctr_clr(int unit, soc_port_resource_t *port_resource_t)
{
    ...
    local_mmu_port = (port_resource_t->mmu_port) % MV2_MMU_PORT_PIPE_OFFSET;
    ...
    /*ING DROP*/
        soc_maverick2_mmu_get_valid_ipipes_for_xpe(unit, xpe, ipipes);
    ...
    /*UC DROP*/
        soc_maverick2_mmu_get_valid_epipes_for_xpe(unit, xpe, epipes);
        if (epipes[pipe_number] == 1) {
            mmu_ctr_uc_drop = SOC_MEM_UNIQUE_ACC_PIPE(unit,
                                  MMU_CTR_UC_DROP_MEMm, pipe_number);
            cosq_base = local_mmu_port * MV2_MMU_NUM_UCQ_PER_HSP_PORT;
            for(q = 0; q < MV2_MMU_NUM_UCQ_PER_HSP_PORT; q++) {
                index = cosq_base + q;
    ...
    /*COLOR DROP*/
        ...
        if (epipes[pipe_number] == 1) {
            ...
            for(q = 0; q < MV2_MMU_NUM_PORT_COLOR_DROP_ENTRIES; q++) {
                index = port_base + (q * MV2_MMU_MAX_PORTS_PER_PIPE);
...
/*! @fn int soc_maverick2_mmu_clear_mtro_bucket_mems(int unit,
 *              soc_port_resource_t *port_resource_t)
 *  @param unit Device number
 *  @param *port_resource_t Port Resource Struct
 ...
 */
int
soc_maverick2_mmu_clear_mtro_bucket_mems(int                  unit,
                                         soc_port_resource_t *port_resource_t)
{
    int mmu_port;
    int pipe_number;
    ...
    lcl_port = mmu_port % MV2_MMU_PORT_PIPE_OFFSET;
    soc_maverick2_mmu_get_num_l0_nodes_per_port(unit, lcl_port, pipe_number,
                                                &num_l0_entries);
    soc_maverick2_mmu_get_l0_base_offset_for_port(unit, lcl_port, &l0_offset);

    soc_maverick2_mmu_get_num_l1_uc_nodes_per_port(unit, lcl_port, pipe_number,
                                                   &num_l1_uc_entries);
    soc_maverick2_mmu_get_num_l1_mc_nodes_per_port(unit, lcl_port, pipe_number,
                                                   &num_l1_mc_entries);

    soc_maverick2_mmu_get_l1_base_offset_for_port(unit, lcl_port, 1,
                                                  &l1_offset_uc);
    soc_maverick2_mmu_get_l1_base_offset_for_port(unit, lcl_port, 0,
                                                  &l1_offset_mc);
...
/*! @fn int soc_maverick2_mmu_clear_prev_ep_credits(int unit,
 *              soc_port_resource_t *port_resource_t)
 *  @param unit Device number
 *  @param *port_resource_t Port Resource Struct
 ...
 */
int
soc_maverick2_mmu_clear_prev_ep_credits(int                  unit,
                                        soc_port_resource_t *port_resource_t)
{
    uint64 rval, fldval64;
    uint64 rd_val;
...
/*! @fn int soc_maverick2_mmu_reinit_dip_fsaf_setting(int unit,
 *              soc_port_resource_t *port_resource_t)
 *  @param unit Device number
 *  @param *port_resource_t Port Resource Struct
 ...
 */
int
soc_maverick2_mmu_reinit_dip_fsaf_setting(int                  unit,
                                          soc_port_resource_t *port_resource_t)
{
    uint64 rval;
    int pipe_num;
    ...
    soc_maverick2_mmu_return_dip_delay_amount(
...
/*! @fn int soc_maverick2_mmu_thdi_bst_clr(int unit,
 *              soc_port_resource_t *port_resource_t)
 *  @param unit Device number
 *  @param *port_resource_t Port Resource Struct
 ...
 */
int
soc_maverick2_mmu_thdi_bst_clr(int unit, soc_port_resource_t *port_resource_t)
{
...
/*! @fn int soc_maverick2_mmu_thdi_setup(int unit,
 ...
 */
int
soc_maverick2_mmu_thdi_setup(int unit, soc_port_resource_t *port_resource_t,
                             int lossy)
{
    ...
    for(pg = 0; pg < MV2_MMU_NUM_PGS; pg++) {
       ...
       if (pg != MV2_MMU_PAUSE_PG) {
               pg_min_setting  = MV2_MMU_THDI_PG_MIN_LIMIT_LOSSY;
               pg_hdrm_setting = MV2_MMU_THDI_PG_HDRM_LIMIT_LOSSY;
        }
        else {

		         pg_min_setting =
                (!lossy) ? MV2_MMU_THDI_PG_MIN_LIMIT_LOSSLESS :
                 MV2_MMU_THDI_PG_MIN_LIMIT_LOSSY;
              if (!lossy) {
                 SOC_IF_ERROR_RETURN(soc_maverick2_mmu_thdi_get_pg_hdrm_setting(unit,
...
/*! @fn int soc_maverick2_mmu_thdm_bst_clr_db_queue(int unit, int pipe,
 *              int mmu_port)
 ...
 */
int
soc_maverick2_mmu_thdm_bst_clr_db_queue(int unit, int pipe, int mmu_port)
{
    ...
    num_queues =
        MV2_MMU_IS_CPU_PORT(pipe,
                            mmu_port) ? MV2_MMU_NUM_MCQ_FOR_CPU_PORT :
        MV2_MMU_NUM_MCQ_PER_HSP_PORT;

    soc_maverick2_mmu_get_valid_epipes_for_xpe(unit, 0, epipes);
    if (epipes[pipe] == 1) {
        thdm_db_queue_bst_mem= SOC_MEM_UNIQUE_ACC_PIPE(unit,
                                   MMU_THDM_DB_QUEUE_BSTm, pipe);
        mem_idx =
            MV2_MMU_IS_LPBK_PORT(mmu_port) ? (MV2_MMU_NUM_HSP_PORTS_PER_PIPE *
                                              MV2_MMU_NUM_MCQ_PER_HSP_PORT +
                                              MV2_MMU_NUM_MCQ_FOR_CPU_PORT) : (
                mmu_port * MV2_MMU_NUM_MCQ_PER_HSP_PORT);
...
/*! @fn int soc_maverick2_mmu_thdm_bst_clr_qe_queue(int unit,
 ...
 */
int
soc_maverick2_mmu_thdm_bst_clr_qe_queue(int unit, int pipe, int mmu_port)
{
...
/*! @fn int soc_maverick2_mmu_thdm_bst_clr_db_port(int unit,
 *                                   int pipe, int mmu_port)
 *  @param int unit Chip unit number
 *  @param int pipe Egress pipe in which port exists
 *  @param int mmu_port local mmu port
 ...
 */
int
soc_maverick2_mmu_thdm_bst_clr_db_port(int unit, int pipe, int mmu_port)
{
    ...
    if (epipes[pipe] == 1) {
        ...
        mem_idx = mmu_port * MV2_MMU_NUM_SPS;
        for (curr_idx = 0; curr_idx < MV2_MMU_NUM_SPS; curr_idx++)
        {
            SOC_IF_ERROR_RETURN(soc_mem_write(unit,
...
/*! @fn int soc_maverick2_mmu_thdm_bst_clr_qe_port(int unit,
 *                                   int pipe, int mmu_port)
 *  @param int unit Chip unit number
 *  @param int pipe Egress pipe in which port exists
 *  @param int mmu_port local mmu port
 ...
 */
int
soc_maverick2_mmu_thdm_bst_clr_qe_port(int unit, int pipe, int mmu_port)
{
...
/*! @fn int soc_maverick2_mmu_thdm_bst_clr(int unit,
 *              soc_port_resource_t *port_resource_t)
 *  @param int unit Chip unit number
 ...
 */
int
soc_maverick2_mmu_thdm_bst_clr(int unit, soc_port_resource_t *port_resource_t)
{
    int local_mmu_port;
    int pipe;
    ...
    /* Clear db queue */
    SOC_IF_ERROR_RETURN(soc_maverick2_mmu_thdm_bst_clr_db_queue(unit, pipe,
                                                          local_mmu_port));
    /* Clear qe queue */
    SOC_IF_ERROR_RETURN(soc_maverick2_mmu_thdm_bst_clr_qe_queue(unit, pipe,
                                                           local_mmu_port));
    /* Clear db port */
    SOC_IF_ERROR_RETURN(soc_maverick2_mmu_thdm_bst_clr_db_port(unit, pipe,
                                                           local_mmu_port));
    /* Clear qe port */
    SOC_IF_ERROR_RETURN(soc_maverick2_mmu_thdm_bst_clr_qe_port(unit, pipe,
                                                           local_mmu_port));

    return SOC_E_NONE;
}

/*! @fn int soc_maverick2_mmu_thdu_bst_clr_queue(int unit,
 *                                 int pipe, int mmu_port)
 *  @param int unit Chip unit number
 ...
 */
int
soc_maverick2_mmu_thdu_bst_clr_queue(int unit, int pipe, int mmu_port)
{
    ...
    if (epipes[pipe] == 1) {
       thdu_bst_queue_mem = SOC_MEM_UNIQUE_ACC_PIPE(unit,
        ...
        mem_idx = mmu_port * MV2_MMU_THDU_UCQ_PER_PORT;
        for (curr_idx = 0; curr_idx < MV2_MMU_THDU_UCQ_PER_PORT; curr_idx++)
        {
...
/*! @fn int soc_maverick2_mmu_thdu_bst_clr_qgrp(int unit,
 *                                int pipe, int mmu_port)
 *  @param int unit Chip unit number
 *  @param int pipe Egress pipe
 *  @param int mmu_port Local mmu port
 ...
 */
int
soc_maverick2_mmu_thdu_bst_clr_qgrp(int unit, int pipe, int mmu_port)
{
...
/*! @fn int soc_maverick2_mmu_thdu_bst_clr_port(int unit,
 *                                int pipe, int mmu_port)
 *  @param int unit Chip unit number
 *  @param int pipe Egress pipe
 *  @param int mmu_port Local mmu port
 ...
 */
int
soc_maverick2_mmu_thdu_bst_clr_port(int unit, int pipe, int mmu_port)
{
...
/*! @fn int soc_maverick2_mmu_thdu_bst_clr(int unit,
 *              soc_port_resource_t *port_resource_t)
 *  @param int unit Chip unit number
 *  @param soc_port_resource_t *port_resource_t Port resource struct containing
 *         epipe and port info
 ...
 */
int
soc_maverick2_mmu_thdu_bst_clr(int unit, soc_port_resource_t *port_resource_t)
{
    int local_mmu_port;
    int pipe;
    ...
    /* Clear queue */
    SOC_IF_ERROR_RETURN(soc_maverick2_mmu_thdu_bst_clr_queue(unit, pipe,
                                                        local_mmu_port));
    /* Clear qGrp */
    SOC_IF_ERROR_RETURN(soc_maverick2_mmu_thdu_bst_clr_qgrp(unit, pipe,
                                                        local_mmu_port));
    /* Clear port */
    SOC_IF_ERROR_RETURN(soc_maverick2_mmu_thdu_bst_clr_port(unit, pipe,
                                                        local_mmu_port));
    return SOC_E_NONE;
}

/*! @fn int soc_maverick2_mmu_thdu_pktstat_clr_queue(int unit, int pipe,
 *              int mmu_port)
 *  @param int unit Chip unit number
 *  @param int pipe Egress pipe
 *  @param int mmu_port Local mmu port
 ...
 */
int
soc_maverick2_mmu_thdu_pktstat_clr_queue(int unit, int pipe, int mmu_port)
{
...
/*! @fn int soc_maverick2_mmu_thdu_pktstat_clr(int unit,
 *              soc_port_resource_t *port_resource_t)
 *  @param int unit Chip unit number
 *  @param soc_port_resource_t *port_resource_t Port resource struct containing
 *         epipe and port info
 ...
 */
int
soc_maverick2_mmu_thdu_pktstat_clr(int                  unit,
                                   soc_port_resource_t *port_resource_t)
{
    int local_mmu_port;
    int pipe;
    ...
    /* Clear queue */
    SOC_IF_ERROR_RETURN(soc_maverick2_mmu_thdu_pktstat_clr_queue(unit, pipe,
                                                           local_mmu_port));
    return SOC_E_NONE;
}

/*! @fn int soc_maverick2_mmu_thdu_qgrp_min_limit_config(int unit,
 *              soc_port_resource_t *port_resource_t, int lossy)
 *  @param int unit Chip unit number
 *  @param soc_port_resource_t *port_resource_t Port resource struct containing
 *             epipe and port info
    ...
    int unit, soc_port_resource_t *port_resource_t, int lossy)
{
    ...
    if (lossy) {
        qgrp_min_limit = MV2_MMU_THDU_QGRP_MIN_LIMIT_LOSSY;
    }
    else {
        qgrp_min_limit = MV2_MMU_THDU_QGRP_MIN_LIMIT_LOSELESS;
    }
    sal_memset(data, 0, sizeof(data));
...
/*! @fn int soc_maverick2_mmu_clear_vbs_credit_memories(int unit,
 *              soc_port_resource_t *port_resource_t)
 *  @param unit Device number
 *  @param *port_resource_t Port Resource Struct
    ...
    int unit,soc_port_resource_t *port_resource_t)
{
    ...
    /*vbs_l2_credit_mem = SOC_MEM_UNIQUE_ACC_PIPE(unit,
                            Q_SCHED_L2_CREDIT_MEMm, pipe_number); No L2 in MV2 */
    ...
    /*vbs_l2_accum_comp_mem = SOC_MEM_UNIQUE_ACC_PIPE(unit,
                                Q_SCHED_L2_ACCUM_COMP_MEMm, pipe_number); No L2 in MV2 */
    ...
    /* Since in MV2 uc entries==mc_entries only looping once over UC entries.*/
    for (j=0; j<num_l1_uc_entries; j++) {
        mem_indx=  l1_offset_uc+j;
        mem_indx1 =  l1_offset_mc+j;

        /*mem_indx_toq_b_uc = mem_indx + MV2_MMU_PIPE1_QUEUE_START;
        mem_indx_toq_b_mc = mem_indx1 + MV2_MMU_PIPE1_QUEUE_START;*/
...
/*! @fn int soc_maverick2_mmu_wred_clr(int unit,
 *              soc_port_resource_t *port_resource_t)
 *  @param unit Device number
 *  @param *port_resource_t Port Resource Struct
 ...
 */
int
soc_maverick2_mmu_wred_clr(int unit, soc_port_resource_t *port_resource_t)
{
    /*Variables*/
    ...
    local_mmu_port = (port_resource_t->mmu_port) % MV2_MMU_PORT_PIPE_OFFSET;
    cosq_base = local_mmu_port * MV2_MMU_NUM_UCQ_PER_HSP_PORT;
    portsp_base = MV2_MMU_NUM_UCQ + (local_mmu_port * MV2_MMU_NUM_SPS);
