...
_soc_utt_ifp_mem_config(int unit)
{
    ...
    if (soc_feature(unit, soc_feature_utt_ver1_5)) {
        ...
        sop->memState[IFP_POLICY_UTT_ADDR_XLATEm].index_max = num_tcams - 1;
    }
    return SOC_E_NONE;
}
...
soc_utt_ifp_bank_config(int unit)
{
    ...
    if (soc_feature(unit, soc_feature_utt_ver1_5)) {
            int sw_id = 0;
            int sv=0, sj=0, lookup_sel=0;

        /* Deduce the hard coded values from config. */
        for (sv=0; sv<12; sv++) {
            for (sj=0; sj<6; sj++) {
...
_soc_utt_lpm_mem_config(int unit)
{
    ...
    sop->memState[L3_DEFIP_PAIR_128_DATA_UTT_ADDR_XLATEm].index_max =
    sop->memState[L3_DEFIP_PAIR_128_HIT_UTT_ADDR_XLATEm].index_max =
        SOC_CONTROL(unit)->l3_defip_utt_paired_tcams * 2 - 1;
    ...
    sop->memState[L3_DEFIP_AUX_TABLEm].index_max =
        sop->memState[L3_DEFIPm].index_max;
    sop->memState[L3_DEFIP_DATA_UTT_ADDR_XLATEm].index_max =
    sop->memState[L3_DEFIP_HIT_UTT_ADDR_XLATEm].index_max =
    sop->memState[L3_DEFIP_AUX_UTT_ADDR_XLATEm].index_max =
...
soc_utt_l3_defip_alpm_urpf_index_map(int unit, int wide, int index)
{
    ...
    if (wide == 0) {
        new_index = index + soc_defip_index_remap / 4;
...
soc_utt_l3_defip_alpm_urpf_index_remap(int unit, int wide, int index)
{
    ...
    if (wide == 0) {
        ...
        if (is_next_half_half_part) {
            new_index += soc_defip_index_remap / 4;
        }
                                            ...
                                            int v6_fent, int *fent_for_move)
{
    ...
    *fent_for_move = 0;
    ...
    next_index = l_tcam_start_index + (offset << 1);
