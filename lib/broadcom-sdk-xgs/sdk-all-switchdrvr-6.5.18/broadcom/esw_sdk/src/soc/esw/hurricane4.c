...
#define PORT_IS_MACSEC(unit, phy_port) \
 ...
 **********************************************************************************/
#define  SOC_HR4_NUM_SC_QM_CONTROL_QUEUE_PER_PORT    2
#define  SOC_HR4_NUM_HIGH_QUEUE_PORT                 16
#define  SOC_HR4_NUM_STACKING_PORT                   4
#define  SOC_HR4_NUM_UPLINK_PORT                     4
int
soc_hurricane4_num_cosq_init_port(int unit, int port)
{
    ...
    if (IS_CPU_PORT(unit, port)) {
        ...
        if (mmu_port < SOC_HR4_NUM_STACKING_PORT) {
            /* Stacking port */
            si->port_num_cosq[port] = SOC_HR4_NUM_MCAST_QUEUE_PER_ST_PORT;
            si->port_num_uc_cosq[port] = SOC_HR4_NUM_UCAST_QUEUE_PER_PORT;
        } else if (mmu_port < (SOC_HR4_NUM_STACKING_PORT + SOC_HR4_NUM_UPLINK_PORT)) {
            /* Uplink port */
            si->port_num_cosq[port] = SOC_HR4_NUM_MCAST_QUEUE_PER_ST_PORT -
                                      SOC_HR4_NUM_SC_QM_CONTROL_QUEUE_PER_PORT;
            si->port_num_uc_cosq[port] = SOC_HR4_NUM_UCAST_QUEUE_PER_PORT  -
                                         SOC_HR4_NUM_SC_QM_CONTROL_QUEUE_PER_PORT;
        } else {
...
_soc_hr4_port_lanes_set(int unit, int phy_port, int *lanes, int port_bandwidth)
{
    if (PORT_IS_FALCON(unit, phy_port)) {
        *lanes =   (port_bandwidth > 53000) ? 4
                 ...
                 : 1;
    } else if (PORT_IS_MACSEC(unit, phy_port)) {
...
soc_hurricane4_port_config_init(int unit, uint16 dev_id)
{
    ...
    for (port = 1; port < num_port; port++) {
        ...
        if ((PORT_IS_MACSEC(unit, phy_port) ||
             PORT_IS_MACSEC(unit, port)) && phy_port != port) {
            LOG_CLI((BSL_META_U(unit,
...
#define _SOC_HR4_DEF_TEMP_MAX   110
    ...
    _soc_hr4_temp_mon_mask[unit] = temp_mask;
...
soc_hr4_mmu_config_init_port(int unit, int port)
{
    ...
    rv = _soc_hr4_mmu_config_buf_set_hw_port(unit, port, buf, &devcfg,
                                             lossless, &swcfg);
...
soc_hr4_support_speeds(int unit, int port, uint32 *speed_mask)
{
    ...
    case 1:
        ...
        if (PORT_IS_MACSEC(unit, phy_port)) {
            speed_valid |= SOC_PA_SPEED_20GB | SOC_PA_SPEED_25GB;
        } else if (PORT_IS_FALCON(unit, phy_port)) {
...
soc_hurricane4_show_voltage(int unit)
{
    ...
    /* AVS Voltage calculation */
    reg = TOP_PVTMON_RESULT_6r;
    ...
    LOG_CLI((BSL_META_U(unit,
                        "AVS Voltage monitor: voltage = %d.%03dV\n"),
             (fval / 1000), (fval % 1000)));
    return SOC_E_NONE;
}
