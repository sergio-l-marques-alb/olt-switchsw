...
soc_monterey_chip_reset(int unit)
{
    ...
    if (power_reset && soc_property_get(unit, spn_LC_PLL_EXT_CLOCK, 0)) {
        rval = 0;
        SOC_IF_ERROR_RETURN(READ_TOP_SOFT_RESET_REG_2r(unit, &rval));
    ...
    if (!SAL_BOOT_SIMULATION) {
        ...
        for (index = 0; index < 4; index++) {
           if ((index == 0 || index == 3)  && SOC_PBMP_IS_NULL(cpri_pbmp)) {
               continue;
           } 
                                 ...
                                 void *d2, void *d3, void *d4)
{
    ...
    for (rbi = 0; ; rbi++) {
        ...
        soc_reg_get(unit, rb->status_reg, REG_PORT_ANY, 0, &rb_rval64);
        if (COMPILER_64_IS_ZERO(rb_rval64)) {
            continue;
        }
                             ...
                             void *d2, void *d3, void *d4)
{
    ...
    for (rbi = 0; ; rbi++) {
        ...
        if (port != -1) {
            ...
            (void)portmod_cpri_port_interrupt_process(unit, port);

            memset(&ecc_info, 0, sizeof(ecc_info));

            /* Process ECC interrupts */
            (void)portmod_cpri_port_ecc_interrupt_status_get(unit, port,
                                                             &ecc_info);

            /* Report ECC errors */
            if (ecc_info.ecc_num_bits_err != PORTMOD_PM_CPRI_ECC_ERR_NONE) {
                pm_ecc_info.type = _SOC_PARITY_TYPE_ECC;
                pm_ecc_info.ecc_err_type =
