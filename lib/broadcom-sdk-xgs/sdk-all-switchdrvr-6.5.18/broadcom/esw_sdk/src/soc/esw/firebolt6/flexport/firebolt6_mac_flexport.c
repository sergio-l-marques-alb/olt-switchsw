...
/*! @file firebolt6_mac_flexport.c
 *  @brief MAC flex for Trident3.
 *  Details are shown below.
 */

int
soc_firebolt6_flex_mac_rx_port_down(
...
int
soc_firebolt6_flex_mac_tx_port_down(
    int unit, soc_port_schedule_state_t *port_schedule_state)
{
    int i;
    uint64 rval64;
...
int
soc_firebolt6_flex_mac_port_up(int                        unit,
                               soc_port_schedule_state_t *port_schedule_state)
{
	...
	int speed_mode;
	int hdr_mode;
    static const int clport_mode_values[SOC_FB6_PORT_RATIO_COUNT] = {
        4, 3, 3, 3, 2, 2, 1, 1, 0
    };


	LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit, "START soc_firebolt6_flex_mac_port_up\n")));
    /*Disable TSC lanes: */
    ...
    for (i = 0; i < port_schedule_state->nport; i++) {
        if (port_schedule_state->resource[i].physical_port != -1) {
            ...
            subp = soc_firebolt6_get_subp_from_phy_pnum(phy_port);
           /* clport = ((phy_port-1)>>2); */
            clport = soc_firebolt6_get_pm_from_phy_pnum(phy_port);
            soc_fb6_tdm_get_port_ratio(unit, port_schedule_state, clport,
                                         &mode,
                                         0);
	...
	LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit, "END soc_firebolt6_flex_mac_port_up\n")));
    return SOC_E_NONE;
}
