...
soc_esw_portctrl_add(int unit, soc_port_t port, int init_flag, void *port_add_info)
{
    ...
    if ((init_flag == PORTMOD_PORT_ADD_F_INIT_PASS2)  || (!init_flag)){
        ...
        if (SOC_IS_FIREBOLT6(unit)) {
            ...
            if ((si->frequency == 1013) &&
                ((add_info->interface_config.speed == 50000) ||
                (add_info->interface_config.speed == 100000))) {

                /* 1013 indicates BCM56472 P/N 1 in 1012Mhz core frequency. Set TX_THRESHOLD to 2 to avoid MAC underrun.
                 */
