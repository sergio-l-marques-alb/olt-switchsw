    ...
    int is_port_down)
{
    ...
    if (1 == is_port_down) {
        for (class = 0; class < TH3_EP_MAX_CT_CLASSES; class++) {
            xmit_start_cnt[class] = 0;
        }
    } else {
    ...
    for (class = 0; class < TH3_EP_NUM_CT_CLASSES; class++) {
        sal_memset(entry, 0, sizeof(uint32) *
                   soc_mem_entry_words(unit, EDB_XMIT_START_COUNT_PIPE0m));
        mem_index = (logical_port % _TH3_DEV_PORTS_PER_PIPE) * TH3_EP_MAX_CT_CLASSES + class;
