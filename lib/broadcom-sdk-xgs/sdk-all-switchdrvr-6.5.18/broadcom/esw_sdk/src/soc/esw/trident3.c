/*
 * $Id: trident3.c,v 1.1.10.25 Broadcom SDK $
...
soc_trident3_port_speed_update(int unit, soc_port_t port, int speed)
{
    ...
    if (IS_CL_PORT(unit, port)) {
        ...
        if (soc_reg_field_get(unit, ASF_EPORT_CFGr, rval, ENABLEf)) {
            _soc_trident3_speed_to_ct_class_mapping(unit, speed, &class);
...
STATIC int
_soc_td3_mmu_config_shared_update_check(int val1, int val2, int flags)
{
    int rv = 0;
    if (flags == 1) {
        if (val1 > val2) {
...
#define TD3_MMU_SHARED_LIMIT_CHK(val1, val2, flags) \
    _soc_td3_mmu_config_shared_update_check((val1), (val2), (flags))


int
soc_td3_mmu_config_shared_buf_recalc(int unit, int res, int thdi_shd,
                                    ...
                                    int post_update)
{
    ...
    if (res & 0x1) { /* if (res & THDI) { */
        ...
        for (idx = 0; idx < _TD3_MMU_NUM_POOL; idx++) {
            ...
            if ((cur_limit != 0) &&
                TD3_MMU_SHARED_LIMIT_CHK(cur_limit, thdi_shd/granularity,
                                        post_update)) {
    ...
    if (res & 0x2) { /* if (res & THDO) { */
        ...
        for (idx = 0; idx < _TD3_MMU_NUM_POOL; idx++) {
            ...
            cur_limit = soc_reg_field_get(unit, reg, rval, field);
            if ((cur_limit != 0) &&
                TD3_MMU_SHARED_LIMIT_CHK(cur_limit, thdo_db_shd/granularity,
                                        post_update)) {
            ...
            cur_limit = soc_reg_field_get(unit, reg, rval, field);
            if ((cur_limit != 0) &&
                TD3_MMU_SHARED_LIMIT_CHK(cur_limit, sal_ceil_func(thdo_db_shd, granularity),
                                        post_update)) {
            ...
            cur_limit = soc_reg_field_get(unit, reg, rval, field);
            if ((cur_limit != 0) &&
                TD3_MMU_SHARED_LIMIT_CHK(cur_limit, thdo_qe_shd/granularity,
                                        post_update)) {
        ...
        for (pipe = 0; pipe < NUM_PIPE(unit); pipe++) {
            ...
            for (idx = 0; idx < SOC_TD3_MMU_CFG_QGROUP_MAX; idx++) {
                ...
                if (!soc_mem_field32_get(unit, mem, &cfg_qgrp, Q_LIMIT_DYNAMIC_CELLf)) {
                    ...
                    if (TD3_MMU_SHARED_LIMIT_CHK(cur_limit, thdo_db_shd/granularity,
                                                post_update)) {
        ...
        PBMP_PORT_ITER(unit, port) {
            ...
            for (idx = 0; idx < numq; idx++) {
                ...
                if (!soc_mem_field32_get(unit, mem, entry0, Q_COLOR_LIMIT_DYNAMIC_CELLf)) {
                    field = LIMIT_YELLOW_CELLf;
                    cur_limit = soc_mem_field32_get(unit, mem, entry0, field);
                    if (TD3_MMU_SHARED_LIMIT_CHK(cur_limit, sal_ceil_func(thdo_db_shd, granularity),
                                                post_update)) {
                        soc_mem_field32_set(unit, mem, entry0,
                    ...
                    field = LIMIT_RED_CELLf;
                    cur_limit = soc_mem_field32_get(unit, mem, entry0, field);
                    if (TD3_MMU_SHARED_LIMIT_CHK(cur_limit,
