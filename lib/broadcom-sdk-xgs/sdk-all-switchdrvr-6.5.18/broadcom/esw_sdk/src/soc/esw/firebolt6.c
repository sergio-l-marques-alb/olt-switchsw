...
static int num_shared_alpm_banks[SOC_MAX_NUM_DEVICES] = {6};
int prio_low = 0;
int prio_high = 1;
...
/* Temporary SKU numbers. It is only used for BCMSIM */

#define FB6_CLK_4730_1118 1118
#define FB6_CLK_4731_1012 1011
#define FB6_CLK_4720_1012 1012
#define FB6_CLK_4721_1012 1013
...
#define FB6_PORT_BLOCK_BASE_PORT(port)                              \
...
typedef const struct port_map_s {
    int phy_port;
    int half_pipe;
    int idb_port;
    int mmu_port;
...
typedef struct _soc_hx5_mmu_traffic_ctrl_s {
    uint8   thdo_drop_bmp[FB6_NUM_PORT];
} _soc_fb6_mmu_traffic_ctrl_t;

typedef const struct _obm_setting_s {
    int discard_limit;
    int lossless_xoff;
...
static    _soc_fb6_mmu_traffic_ctrl_t
              *_soc_hx5_mmu_traffic_ctrl[SOC_MAX_NUM_DEVICES];
 ...
 */
int
soc_fb6_mmu_bmp_reg_pos_get(int unit, soc_port_t port,
                            int *pipe, int *split, int *pos)
{
    return soc_td3_mmu_bmp_reg_pos_get(unit, port, pipe, split, pos);
}
 ...
 */
STATIC void
_soc_fb6_sku_params_get(int unit,
                                 int type,
                                 int *val)
{
    soc_info_t *si;
    ...
    switch (soc_property_get(unit, spn_CORE_CLOCK_FREQUENCY, si->frequency)) {
...
_soc_fb6_mmu_idb_ports_assign(int unit)
{
    ...
    _soc_fb6_sku_params_get(unit,
        BCMI_FB6_SKU_ELEM_TYPE_PORTMAP, &portmap);
...
#if 0
        if (port_mapping[portmap][phy_port - 1].is_uplink) {
            SOC_PBMP_PORT_ADD(si->uplink_pbm, port);
        }

        if (port_mapping[portmap][phy_port - 1].is_stacking) {
...
soc_firebolt6_mem_config(int unit)
{
     ...
     * bank 2-9 are shared banks (32k L2/L3 or 8k FPEM entries
    ...
    sop->memState[EXACT_MATCH_LPm].index_max = ((shared_fpem_banks * 7 * 1024));
    sop->memState[EXACT_MATCH_LP_PIPE0m].index_max = ((shared_fpem_banks * 7 * 1024));
...
uint32 _soc_fb6_mmu_port(int unit, int port)
{
...
{
    int rv;
    rv = soc_property_port_get(unit, phy_port, spn_PORT_STACKING, 0);
    return rv;
}
...
{
    int rv;
    rv = soc_property_port_get(unit, phy_port, spn_PORT_UPLINK, 0);
    return rv;
}

int
_soc_fb6_mmu_port_l0_mapping_set(int unit)
{
    int port;
    uint32 rval;
...
soc_firebolt6_num_cosq_init(int unit)
{
    ...
    SOC_IF_ERROR_RETURN(
        _soc_fb6_mmu_port_l0_mapping_set(unit));
                               ...
                               int skew_id, int *frequency)
{
    ...
    case BCM56470_DEVICE_ID:
        *frequency = soc_property_get(unit, spn_CORE_CLOCK_FREQUENCY, 1012);
        break;
    default:
...
void
soc_firebolt6_pipe_map_get(int unit, uint32 *pipe_map)
{
 ...
 */
void
soc_firebolt6_port_ratio_get(int unit, int port, int *mode)
{
    soc_info_t *si;
    int phy_port_base, lane;
    int num_lanes[FIREBOLT6_PORTS_PER_PBLK];
    int speed_max[FIREBOLT6_PORTS_PER_PBLK];

    si = &SOC_INFO(unit);
    phy_port_base = FB6_PORT_BLOCK_BASE_PORT(port);

    for (lane = 0; lane < FIREBOLT6_PORTS_PER_PBLK; lane++) {
...
/* Function to get the number of ports present in a given Port Macro */
STATIC int
_soc_fb6_ports_per_pm_get(int unit, int pm_id)
{
    soc_info_t *si;
    int phy_port, num_ports = 0, i;
    ...
    phy_port = 1 + (pm_id * _FB6_PORTS_PER_PBLK);

    for (i = 0; i < _FB6_PORTS_PER_PBLK; ) {
        if (si->port_p2l_mapping[phy_port + i] != -1) {
            num_ports ++;
            i += si->port_num_lanes[si->port_p2l_mapping[phy_port + i]];
        } else {
...
_soc_fb6_port_flex_init(int unit, int is_any_cap)
{
   ...
   soc_info_t *si;
    ...
    for (blk_idx = 0; blk_idx < SOC_MAX_NUM_BLKS; blk_idx++) {
        ...
        pm = SOC_BLOCK_NUMBER(unit, blk_idx);
        if (blk_type == SOC_BLK_CLPORT) {
            static_ports = _soc_fb6_ports_per_pm_get(unit, pm);
...
_soc_fb6_mmu_idb_ports_assign(int unit)
{
     ...
     * MMU/IDB Ports: 0-3
     */
    for (phy_port = 1; phy_port <= 68; phy_port++, mmu_port_os++) {
...
soc_fb6_port_config_init(int unit, uint16 dev_id)
{
    ...
    _soc_fb6_mmu_idb_ports_assign(unit);
 ...
 */
STATIC int
_soc_fb6_alpm_addr_xlate_set(int unit, soc_mem_t mem,
...
_soc_firebolt6_port_mapping_init(int unit)
{
    ...
    mem = ING_IDB_TO_DEVICE_PORT_NUMBER_MAPPING_TABLEm;
    /* Mapping for Front Panel ports handled in DV flexport code */
    for (port = 0; port < FB6_NUM_PORT; port++) {
    ...
    for (phy_port=1; phy_port <= FB6_NUM_PHY_PORT; phy_port++) {
        ...
        /* Phy_port needs to be started from index 0 in this table.*/
        SOC_IF_ERROR_RETURN
...
*/
    ...
    _soc_fb6_sku_params_get(unit,
        BCMI_FB6_SKU_ELEM_TYPE_DPR , &latency);
...
_soc_fb6_set_idb_dpp_ctrl(int unit)
{
    ...
    _soc_fb6_sku_params_get(unit,
        BCMI_FB6_SKU_ELEM_TYPE_CREDITS , &credits);


    soc_firebolt6_pipe_map_get(unit, &pipe_map);
...
#if 1
STATIC int
_soc_firebolt6_hash_init(int unit)
{
...
#if 0
    Fb6FIXME:  defined(BCM_WARM_BOOT_SUPPORT)
    if (SOC_WARM_BOOT(unit)) {
        SOC_IF_ERROR_RETURN(
            soc_hx5_tdm_scache_recovery(unit));
        return SOC_E_NONE;
    } else {
        SOC_IF_ERROR_RETURN(
            soc_hx5_tdm_scache_allocate(unit));
    }
#endif
                               ...
                               int *obm_id, int *lane)
{
    ...
    si = &SOC_INFO(unit);
    phy_port = si->port_l2p_mapping[port];
    port_block_base = FB6_PORT_BLOCK_BASE_PORT(port);
    if (lane != NULL) {
...
STATIC int
_soc_fb6_tscx_firmware_set(int unit, int port, uint8 *array, int datalen)
{
...
STATIC int
_soc_firebolt6_init_idb_memory(int unit)
{
    return soc_trident3_init_idb_memory(unit);
}
...
#define _SOC_FB6_DEF_TEMP_MAX 110 
    ...
    SOC_IF_ERROR_RETURN(_soc_firebolt6_init_idb_memory(unit));
...
_soc_firebolt6_misc_init(int unit)
{
    ...
    SOC_IF_ERROR_RETURN(_soc_firebolt6_hash_init(unit));
    ...
    _phy_tsce_firmware_set_helper[unit] = _soc_fb6_tscx_firmware_set;
    _phy_tscf_firmware_set_helper[unit] = _soc_fb6_tscx_firmware_set;
 ...
 * Defined as Macro: SOC_FB6_MMU_PIPED_MEM_INDEX
 * Note: If no special indexig required, return the same index.
 */
uint32 _soc_fb6_piped_mem_index(int unit, soc_port_t port,
                               soc_mem_t mem, int arr_off)
{
    int     mmu_port, index;

    mmu_port = SOC_FB6_MMU_PORT(unit, port);

    switch (mem) {
        case MMU_THDM_DB_PORTSP_CONFIGm:
        case MMU_THDM_MCQE_PORTSP_CONFIGm:
            index = (_FB6_PORTS_PER_PIPE * arr_off) + (mmu_port &
                     SOC_FB6_MMU_PORT_STRIDE);
            break;
...
#if 0
#if !defined(BCM_HX5_ASF_EXCLUDE)
/*
 * Funtion to caltulate global buffer reserve for ASF.
 */
STATIC int
_soc_hx5_mmu_config_buf_asf(int unit, int lossless, int *cnt)
{
    soc_info_t *si;
    ...
    if (asf_profile[unit] == -1) {
        asf_profile[unit] = soc_property_get(unit, spn_ASF_MEM_PROFILE,
            _SOC_HX5_ASF_MEM_PROFILE_SIMILAR);
        if ((asf_profile[unit] > _SOC_HX5_ASF_MEM_PROFILE_EXTREME)
            || (asf_profile[unit] < _SOC_HX5_ASF_MEM_PROFILE_NONE)) {
            asf_profile[unit] = _SOC_HX5_ASF_MEM_PROFILE_NONE;
        }
    }
                                ...
                                int lossless)
{
    ...
    (void)_soc_fb6_mmu_config_buf_asf(unit, lossless, &asf_rsvd);
#endif
                               ...
                               _soc_fb6_mmu_sw_config_info_t *swcfg)
{
    ...
    if (mem0 != INVALIDm) {
        for (idx = 0; idx < _FB6_MMU_NUM_POOL; idx++) {
            buf_port_pool = &buf_port->pools[idx];
            midx = SOC_FB6_MMU_PIPED_MEM_INDEX(unit, port, mmu_thdi_port_mem[0],
                                               idx);
    ...
    if (mem1 != INVALIDm) {
        for (idx = 0; idx < _FB6_MMU_NUM_PG; idx++) {
            ...
            midx = SOC_FB6_MMU_PIPED_MEM_INDEX(unit, port, mmu_thdi_port_mem[1],
                                               idx);
    ...
    for (idx = 0; idx < _FB6_MMU_NUM_POOL; idx++) {
        ...
        index1 = SOC_FB6_MMU_PIPED_MEM_INDEX(unit, port,
...
STATIC int
_soc_fb6_age_timer_get(int unit, int *age_seconds, int *enabled)
{
    soc_control_t *soc = SOC_CONTROL(unit);
...
STATIC int
_soc_fb6_age_timer_max_get(int unit, int *max_seconds)
{
...
STATIC int
_soc_fb6_age_timer_set(int unit, int age_seconds, int enable)
{
    soc_control_t *soc = SOC_CONTROL(unit);
...
int soc_fb6_mmu_traffic_ctrl_size(int unit, int *req_scache_size)
{
    *req_scache_size += sizeof(_soc_fb6_mmu_traffic_ctrl_t);

    return SOC_E_NONE;
}

int soc_fb6_mmu_traffic_ctrl_wb_sync(int unit, uint8 **scache_ptr)
{
    _soc_fb6_mmu_traffic_ctrl_t *mmu_traffic_ctrl_scache_p;
    _soc_fb6_mmu_traffic_ctrl_t *mmu_traffic_ctrl;

    mmu_traffic_ctrl_scache_p =
        (_soc_fb6_mmu_traffic_ctrl_t *)(*scache_ptr);

    mmu_traffic_ctrl = _soc_hx5_mmu_traffic_ctrl[unit];
    if (mmu_traffic_ctrl == NULL || mmu_traffic_ctrl_scache_p == NULL) {
        return SOC_E_NONE;
    }
    sal_memcpy(mmu_traffic_ctrl_scache_p, mmu_traffic_ctrl,
               sizeof(_soc_fb6_mmu_traffic_ctrl_t));
    mmu_traffic_ctrl_scache_p++;
...
int soc_fb6_mmu_traffic_ctrl_wb_restore(int unit, uint8 **scache_ptr)
{
    _soc_fb6_mmu_traffic_ctrl_t *mmu_traffic_ctrl_scache_p;
    _soc_fb6_mmu_traffic_ctrl_t *mmu_traffic_ctrl;
...
STATIC int
_soc_fb6_thdo_hw_get(int unit, soc_port_t port, int *enable)
{
    uint64 rval64;
    int pipe, i;
    int split, pos;
    ...
    SOC_IF_ERROR_RETURN(
        soc_fb6_mmu_bmp_reg_pos_get(unit, port, &pipe, &split, &pos));
...
STATIC int
_soc_fb6_thdo_hw_set(int unit, soc_port_t port, int enable)
{
...
/*
 * Function:
 *      _soc_fb6_port_thdo_rx_enable_set
 * Purpose:
 ...
 */
STATIC int
_soc_fb6_port_thdo_rx_enable_set(int unit, soc_port_t port,
                                 int enable, uint8 flag)
{
    ...
    if (!enable) {
        if (*thdo_drop_bmp & flag) {
            ...
            if (*thdo_drop_bmp == 0) {
                rv = _soc_fb6_thdo_hw_set(unit, port, enable);
                if (SOC_FAILURE(rv)) {
                    LOG_ERROR(BSL_LS_SOC_COMMON,
                              (BSL_META_U(unit,
...
/*
 * Function:
 *     _soc_hx5_port_thdo_rx_enable_get
 * Purpose:
 ...
 */
STATIC int
_soc_fb6_port_thdo_rx_enable_get(int unit, soc_port_t port,
                                 int *enable)
{
    int rv = SOC_E_NONE;

    rv = _soc_fb6_thdo_hw_get(unit, port, enable);
    LOG_INFO(BSL_LS_SOC_MMU,
             (BSL_META_UP(unit, port,
                          "[GET]MMU Tx drop bmp: 0x%02x, enable status: %d.\n"),
                          _soc_hx5_mmu_traffic_ctrl[unit]->thdo_drop_bmp[port],
                          *enable));
...
/*
 * Function:
 *     soc_hx5_port_mmu_rx_enable_set
 * Purpose:
 ...
 */
int
soc_fb6_port_mmu_rx_enable_set(int unit, soc_port_t port,
                               int enable)
{
    return soc_td3_port_mmu_rx_enable_set(unit, port, enable);
}

/*
 * Function:
 *     soc_hx5_port_mmu_rx_enable_get
 * Purpose:
 ...
 */
int
soc_fb6_port_mmu_rx_enable_get(int unit, soc_port_t port,
                               int *enable)
{
    uint32 rval;
...
int
soc_fb6_port_traffic_egr_enable_set(int unit, soc_port_t port,
                               int enable)
{
    return _soc_fb6_port_thdo_rx_enable_set(unit, port, enable,
                                            PORT_TX_EN_CTRL);
}

int
soc_fb6_port_mmu_tx_enable_set(int unit, soc_port_t port,
                               int enable)
{
    return _soc_fb6_port_thdo_rx_enable_set(unit, port, enable,
                                            MMU_TRAFFIC_EN_CTRL);
...
int
soc_fb6_port_mmu_tx_enable_get(int unit, soc_port_t port,
                               int *enable)
{
    return _soc_fb6_port_thdo_rx_enable_get(unit, port, enable);
}
                                    ...
                                    int post_update)
{
    ...
    if (res & 0x1) { /* if (res & THDI) { */
        ...
        PBMP_ALL_ITER(unit, port) {
            ...
            for (idx = 0; idx < _FB6_MMU_NUM_PG; idx++) {
                midx = SOC_FB6_MMU_PIPED_MEM_INDEX(unit, port, mem, idx);
                sal_memset(&pg_config_mem, 0, sizeof(pg_config_mem));
                SOC_IF_ERROR_RETURN
    ...
    if (res & 0x2) { /* if (res & THDO) { */
        ...
        PBMP_PORT_ITER(unit, port) {
            ...
            mem = SOC_MEM_UNIQUE_ACC(unit, base_mem)[pipe];
            for (idx = 0; idx < _FB6_MMU_NUM_POOL; idx++) {
                index1 = SOC_FB6_MMU_PIPED_MEM_INDEX(unit, port, mem, idx);
...
_soc_fb6_pstats_tbl_desc_all = {SOC_BLK_NONE, INVALIDm, FALSE, FALSE};
/* pstats support */
soc_pstats_tbl_desc_t _soc_fb6_pstats_tbl_desc[] = {
...
void
soc_fb6_mmu_pstats_tbl_config_all(int unit)
{
    soc_control_t *soc = SOC_CONTROL(unit);
    soc_pstats_mem_desc_t *desc, *desc_all;
    int ti, mi, di = 0;

    desc_all = _soc_fb6_pstats_tbl_desc_all.desc;
    ...
    soc->pstats_tbl_desc = &_soc_fb6_pstats_tbl_desc_all;
    soc->pstats_tbl_desc_count = 1;
}

void
soc_fb6_mmu_pstats_tbl_config(int unit)
{
    soc_control_t *soc = SOC_CONTROL(unit);
    ...
    soc->pstats_tbl_desc = _soc_fb6_pstats_tbl_desc;
    soc->pstats_tbl_desc_count = COUNTOF(_soc_fb6_pstats_tbl_desc);
    ...
    if (soc_property_get(unit, "pstats_desc_all", 1)) {
        ...
        soc_fb6_mmu_pstats_tbl_config_all(unit);
    }
}

int
soc_fb6_mmu_pstats_mode_set(int unit, uint32 flags)
{
    soc_control_t *soc = SOC_CONTROL(unit);
    int pf = 0;
    uint32 rval = 0;
    ...
    if (flags & _FB6_MMU_PSTATS_HWM_MOD) {
        soc_reg_field_set(unit, MMU_GCFG_PKTSTAT_OOBSTATr, &rval,
                          HWM_MODEf, 1);
        pf |= _FB6_MMU_PSTATS_HWM_MOD;
        if (flags & _FB6_MMU_PSTATS_RESET_ON_READ) {
            soc_reg_field_set(unit, MMU_GCFG_PKTSTAT_OOBSTATr, &rval,
                              HWM_RESET_ON_READf, 1);
            pf |= _FB6_MMU_PSTATS_RESET_ON_READ;
        }
    }

    if (soc_feature(unit, soc_feature_mor_dma) &&
            (flags & _FB6_MMU_PSTATS_MOR_EN)) {
        soc_reg_field_set(unit, MMU_GCFG_PKTSTAT_OOBSTATr, &rval,
                MOR_ENf, 1);
    }
    if (flags & _FB6_MMU_PSTATS_MOR_EN) {
        pf |= _FB6_MMU_PSTATS_MOR_EN;
    }

    if (flags & _FB6_MMU_PSTATS_ENABLE) {
        soc_reg_field_set(unit, MMU_GCFG_PKTSTAT_OOBSTATr, &rval,
                ENABLEf, 1);
        pf |= _FB6_MMU_PSTATS_ENABLE;
        if ((flags & _FB6_MMU_PSTATS_SYNC) ||
                (flags & _FB6_MMU_PSTATS_HWM_MOD &&
                 flags & _FB6_MMU_PSTATS_RESET_ON_READ)) {
            soc->pstats_mode = PSTATS_MODE_PKT_SYNC;
            pf |= _FB6_MMU_PSTATS_SYNC;
        } else {
...
int
soc_fb6_mmu_pstats_mode_get(int unit, uint32 *flags)
{
    int pf = 0;
    uint32 rval = 0;
    int rv;
...
int
soc_fb6_mmu_pstats_mor_enable(int unit)
{
    soc_control_t *soc = SOC_CONTROL(unit);
    uint32 rval;


    if (soc->pstats_flags & _FB6_MMU_PSTATS_MOR_EN) {
...
int
soc_fb6_mmu_pstats_mor_disable(int unit)
{
    soc_control_t *soc = SOC_CONTROL(unit);
    uint32 rval;

    if (!soc_feature(unit, soc_feature_mor_dma) &&
        (soc->pstats_flags & _FB6_MMU_PSTATS_MOR_EN)) {
        SOC_IF_ERROR_RETURN(READ_MMU_GCFG_PKTSTAT_OOBSTATr(unit, &rval));
...
soc_functions_t soc_firebolt6_drv_funs = {
    ...
    _soc_fb6_age_timer_get,
    _soc_fb6_age_timer_max_get,
    _soc_fb6_age_timer_set,
    _soc_fb6_tscx_firmware_set,
