  ...
  if(flag == 0){
    ...
    for (i = 0; i < cal_len; i++) {
      if(cal_main[i] == *start_port) {
        ...
        /*if(*num_active_lanes == 4 && *num_1g_tokens ==0 && *num_2g_tokens == 8){
          j = j+1;
...
/**
@name: tdm_hr4_main_transcription_16x1G
@param:
...
**/
static int
tdm_hr4_main_transcription_16x1G(tdm_mod_t *_tdm, int flag, int start_port, int end_port)
{
    int n=0, port=0, port_phy=0, is_super_pm=0, num_1g_tokens=0, num_2g_tokens=0,
         num_active_lanes=0;
    int j=0,num_1g_ports,num_2g_ports=0,
        cal_len=0, cal_id=0, *cal_main = NULL;
    int *param_1g_en = NULL;
    ...
    param_1g_en  = _tdm->_chip_data.soc_pkg.soc_vars.hr4.port_1g_en;
    port_num_1g_ports  = _tdm->_chip_data.soc_pkg.soc_vars.hr4.port_num_1g_ports;
    port_num_2g_ports  = _tdm->_chip_data.soc_pkg.soc_vars.hr4.port_num_2g_ports;
    param_speeds = _tdm->_chip_data.soc_pkg.speed;
    param_states = _tdm->_chip_data.soc_pkg.state;


    if (flag == 0) { /* encode super PM from 16x1G to 1x40G */
        for (port_phy = start_port;
             port_phy <= end_port;
             port_phy += ( (end_port - start_port) + 1) ) {
          if (((param_speeds[port_phy] == SPEED_1G) || (param_speeds[port_phy] == SPEED_2p5G) ) &&
            ...
            is_super_pm = BOOL_TRUE;
	          num_1g_ports = 0;
	          num_2g_ports = 0;
            for (n = 0; n < ( (end_port - start_port) + 1); n++) {
              port = port_phy + n;
            ...
            TDM_PRINT2("%d, is_super_pm %d num_1g_ports \n", is_super_pm,num_1g_ports);
            TDM_PRINT2("%d, is_super_pm %d num_2g_ports \n", is_super_pm,num_2g_ports);
            if (num_1g_ports > 16 ){
            ...
            if (is_super_pm == BOOL_TRUE) {
              for (n = 0; n < ( (end_port-start_port) + 1 ) ; n++) { /* recursive loop for setting tokens on first pm,
	              each quad speed is -> first pm 4 lanes */    	
                port = port_phy + n;
	              if( param_speeds[port] == SPEED_1G )
                  param_1g_en[port - 1] = HR4_1G_PORT_STATE_16x1G;
	              else if( param_speeds[port] == SPEED_2p5G )
                  param_1g_en[port - 1] = HR4_1G_PORT_STATE_16x2G;
              }
	            num_1g_tokens      = 0;
	            num_2g_tokens      = 0;
              num_active_lanes   = 0;
              cal_len            = 0; /* This is not use in below call  */
              cal_main           = 0; /* This is not use in below call  */
              tdm_hr4_find_active_lanes(_tdm, &start_port, &end_port, &num_1g_tokens, &num_2g_tokens, &num_active_lanes, cal_main, cal_len, 0);
              if((end_port - start_port == (HR4_NUM_GPORT_SUBPORTS -1))){
                param_speeds[port_phy]     = SPEED_10G;
                param_states[(port_phy-1)] = PORT_STATE__LINERATE;
                for(j= (port_phy+1); j <= end_port; j++) {
                  param_speeds[j]     = SPEED_0;
                  param_states[(j-1)] = PORT_STATE__DISABLED;
                }
              } else if((end_port - start_port) == (HR4_NUM_PMQ_SUBPORTS -1)) {
                if(num_1g_tokens + num_2g_tokens <= 8) {
                  param_speeds[port_phy]     = SPEED_10G;
                } else if(num_1g_tokens + num_2g_tokens <= 16) {
                  param_speeds[port_phy]     = SPEED_20G;
                } else{
              ...
              /*if( num_active_lanes == 4 && (total_1g_2g_tokens <=8 && total_1g_2g_tokens >4) ){
                param_speeds[port_phy]     = SPEED_10G;
                param_states[(port_phy-1)] = PORT_STATE__LINERATE;
                for(j= (port_phy+1); j <= end_port; j++) {
                  param_speeds[j]     = SPEED_0;
              ...
              if( num_active_lanes <= 8 && num_1g_tokens ==8 && num_2g_tokens == 0 ){
                ...
                param_speeds[port_phy]     = SPEED_15G;
               for(j= (port_phy+1); j <= end_port; j++) {
                 param_speeds[j]     = SPEED_0;
                 param_states[(j-1)] = PORT_STATE__DISABLED;
                }
   ...
   /* checking the updates done */ 
      ...
      num_1g_ports = 0 ;
      num_2g_ports = 0 ;
      for (port_phy = start_port; port_phy <= end_port; port_phy++) {
             if (num_1g_ports == 0)   
             num_1g_ports = (port_num_1g_ports[port_phy - 1]); 
             if (num_2g_ports == 0)   
             num_2g_ports = (port_num_2g_ports[port_phy - 1]); 
                  TDM_PRINT2("%d, is_super_pm %d num_1g_ports \n", param_1g_en[port_phy - 1],num_1g_ports);
                  TDM_PRINT2("%d, is_super_pm %d num_2g_ports \n", param_1g_en[port_phy - 1],num_2g_ports);
        if (param_1g_en[port_phy - 1] == HR4_1G_PORT_STATE_16x1G) {
          if(num_1g_ports > 0){
        ...
        if (param_1g_en[port_phy - 1] == HR4_1G_PORT_STATE_16x2G) {
          if(num_2g_ports > 0){
            param_speeds[port_phy] = SPEED_2p5G;
            param_states[port_phy - 1] = PORT_STATE__LINERATE;
            num_2g_ports = num_2g_ports -1 ;
    	...
    	/* token allocation */
      for (port_phy = start_port; port_phy <= end_port; port_phy += ( (end_port - start_port) + 1 )) {
        if (param_1g_en[port_phy - 1] >= HR4_1G_PORT_STATE_16x1G) {
          for (cal_id = (port_phy - 1) / HR4_NUM_PHY_PORTS_PER_PIPE; cal_id < 8; cal_id += 4) {
            ...
            if (cal_main != NULL && cal_len > 0) {
              tdm_hr4_find_active_lanes(_tdm, &start_port, &end_port, &num_1g_tokens, &num_2g_tokens, &num_active_lanes, cal_main, cal_len, 1);
            }
          }
        }
      }
