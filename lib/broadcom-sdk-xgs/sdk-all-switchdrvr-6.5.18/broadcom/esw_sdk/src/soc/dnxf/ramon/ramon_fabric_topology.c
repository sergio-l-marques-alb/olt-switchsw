...
soc_ramon_fabric_topology_isolate_set(int unit, soc_dnxc_isolation_status_t val)
{
    ...
    if(soc_dnxc_isolation_status_isolated == val) {
        ...
        /* wait for peer FE device (a device connected to the one we are currently isolating) to exit GPD mode.
         * if an FE device enters GPD mode by receiving a GPD message,
         * after this time period it will get out of GPD mode (if no additional GPD messages were received during it).
        ...
        sleep_time_msec = (gpd_threshold / core_clock_speed) * 10; /* multiply by 10 to make sure we wait enough time */
...
soc_ramon_fabric_topology_lb_set(int unit, soc_module_t destination_local_id, int links_count, soc_port_t *links_array)
{
    ...
    /* If links_count>nof_links/2 only normal mode is allowed (limitation is caused by size of MULTI_TB, which is not used for normal mode) */
 ...
 * Purpose:
 *      Enabling/disabling the control cell generation for FE1 local route.
 *      Errata  HWFE6400FE-195 <internal ID #22>
 * Parameters:
 *      unit        - (IN)  Unit number.
 *      enable      - (IN)  Control cells enabled/disabled
 * Returns:
 *      _SHR_E_xxx
 */
shr_error_e
