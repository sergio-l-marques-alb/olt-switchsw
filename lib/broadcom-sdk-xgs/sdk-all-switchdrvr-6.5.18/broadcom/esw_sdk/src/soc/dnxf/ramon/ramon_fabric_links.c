...
#define SOC_RAMON_FABRIC_LINKS_VSC256_MAX_CELL_SIZE        (256 /*data*/ + 16 /*header*/ + 1/*CRC*/)
 ...
 * Purpose:
 *      Validates that link is within the valid range
 *      and also that the link is ENABLED
 * Parameters:
 *      unit  - (IN) Unit number.
 *      link  - (IN) Link 
 * Returns:
...
soc_ramon_fabric_link_repeater_enable_set(int unit, soc_port_t port, int enable)
{
    ...
    /*Shaper should include empty cells iff at least single link (in the fmac) is connected to a repeater*/
    if (enable)
    {
...
soc_ramon_fabric_links_pcp_enable_set(int unit, soc_port_t port, int enable)
{
    ...
    SHR_IF_ERR_EXIT(soc_dnxc_port_disable_and_save_enable_state(unit, port, &orig_enabled));
    ...
    /* enable simple unpacking if PCP disabled*/
    ...
    SHR_IF_ERR_EXIT(soc_dnxc_port_restore_enable_state(unit, port, orig_enabled));
...
#define _SOC_RAMON_REMOTE_PIPE_NOF_BLOCKS      (3) /*nof blocks to configure*/
shr_error_e 
soc_ramon_fabric_links_pipe_map_set(int unit, soc_port_t port, soc_dnxf_fabric_link_remote_pipe_mapping_t pipe_map)
{
    ...
    /* Convert port into block instance and link inside the block */
    ...
    /*
     * Link's pipes configuration for DCH, DCML, LCM blocks.
     */
    for (block_i = 0; block_i < _SOC_RAMON_REMOTE_PIPE_NOF_BLOCKS; ++block_i)
    {
        /*
         * link's 2 pipes configuration
         */
        reg32_val = 0;
    ...
    {
        /*
         * remote primary pipe mapping to local pipe for blocks DCH, DCML, LCM
         */
        for (block_i = 0; block_i < _SOC_RAMON_REMOTE_PIPE_NOF_BLOCKS; ++block_i)
        {
    ...
    {
        /*
         * remote seconday pipe mapping to local pipe for blocks DCH, DCML, LCM
         */
        for (block_i = 0; block_i < _SOC_RAMON_REMOTE_PIPE_NOF_BLOCKS; ++block_i)
        {
            SOC_REG_ABOVE_64_CLEAR(reg_above_64_val);
    ...
    /* 
     * Enable/Disable pipe mapping for blocks DCH, DCML, LCM
     */
    for (block_i = 0; block_i < _SOC_RAMON_REMOTE_PIPE_NOF_BLOCKS; ++block_i)
    {
        reg32_val = 0;
...
/**
 * \brief
 *   Force link integrity to be down.
 * \param [in] unit -
 *   The unit number.
 * \param [in] port -
 *   The link number.
 * \param [in] force_down -
 *   1 - force link integrity to be down.
 *   0 - link integrity will remain according to all parameters.
 * \return
 *   See \ref shr_error_e.
 * \remark
 *   None.
...
soc_ramon_fabric_links_isolate_set(int unit, soc_port_t link, soc_dnxc_isolation_status_t val) {
    ...
    if (soc_dnxc_isolation_status_active == val) {
        ...
        /* Return unicast table update */
        SHR_IF_ERR_EXIT(WRITE_RTP_RESERVED_10r(unit, reg32_val));

        /*
         * The unicast tables are updated from the RTP maintenance table.
         * The table is devided to lines, and each line has a 'dirty bit'
         * that tells if this line has been changed.
         *
         * Since we disabled the update for 10ms, the maintenance table might have
        ...
        /*
         * Update multicast allowed links.
         */
        SHR_IF_ERR_EXIT(READ_QRH_MULICAST_ALLOWED_LINKS_REGISTERr(unit, 0, multicast_allowed_bm));
...
soc_ramon_fabric_links_llf_control_source_set(int unit, soc_port_t link, soc_dnxc_fabric_pipe_t val) {
    ...
    * Put FMAC into reset before access if needed
    */
   SHR_IF_ERR_EXIT(soc_dnxc_port_disable_and_save_enable_state(unit, link, &link_original_state));
   ...
   *field_val_tx = soc_reg_field_get(unit, FMAC_LINK_LEVEL_FLOW_CONTROL_ENABLE_REGISTERr, reg_val, LNK_LVL_FC_TX_ENf);
    ...
    * Restore port original state
    */
   SHR_IF_ERR_EXIT(soc_dnxc_port_restore_enable_state(unit, link, link_original_state));
exit:
   SHR_FUNC_EXIT;
}
...
soc_ramon_fabric_links_aldwp_config(int unit, soc_port_t port)
{
    ...
    /*Retrieve the relevant parameters: link rate and cell_format*/
    ...
    SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_link_device_mode_get,(unit,port, 1/*rx*/, &device_mode)));
    ...
    SOC_PBMP_ITER(pbmp, port_index)
    {
        ...
        aldwp = (3* max_cell_size * dnxf_data_device.general.core_clock_khz_get(unit)) / (speed / 8 /*bits to bytes*/); /*mili ticks*/
    ...
    else if (highest_aldwp == 0) /*Check retrieved values*/
    {
...
soc_ramon_fabric_links_aldwp_init(int unit)
{
    ...
    {
        rv = soc_ramon_fabric_links_aldwp_config(unit, dch_instance *  nof_links_in_dcq/*First link in dch*/);
        SHR_IF_ERR_EXIT(rv);
    }
 ...
 * Purpose:
 *      swap SerDes TX between 2 lanes
 * Parameters:
 *      unit   - (IN)  The unit number
 *      link_a - (IN)  First Link
 *      link_b - (IN)  Second Link
 *      enable - (IN)  1 - swap TX, 0 - restore original TX
 * Returns:
 *      _SHR_E_xxx
 */
static shr_error_e
soc_ramon_fabric_links_tx_swap_set(int unit, soc_port_t link_a, soc_port_t link_b, int enable)
{
    ...
    /* convert link-ID [0-191] to core index [0-7] */
 ...
 * Purpose:
 *      return peer link connected in retimer loopback.
 * Parameters:
 *      unit   - (IN)  The unit number
 *      link_a - (IN)  Input link
 *      link_b - (OUT) Peer link
 * Returns:
 *      _SHR_E_xxx
 */
static shr_error_e
soc_ramon_fabric_links_retimer_loopback_peer_get(int unit, soc_port_t link_a, soc_port_t *link_b)
{
    ...
    *link_b = -1;
    ...
    /*
     * get link connected in retimer loopback
     * ======================================
     * assuming link_a is connected in loopback retimer:
     *   a. it is connected to one links in quad.
     *   b. if is not connected to itself (connected to a different link in quad) it's TX serdes was swapped with the connected link.
     *
     * get current TX serdes of lane from HW.
     * search in lane map database (containing the original TX mapping) for a lane with the current HW TX.
     * this lane is connected to link_a in retimer loopback
     */
    for (lane_index = 0 ; lane_index < dnxf_data_device.blocks.nof_links_in_phy_core_get(unit) ; lane_index++)
    {
        ...
        {
            /* convert core index [0-7] to link-ID [0-191] */
 ...
 * Purpose:
 *      poll on cdr (Rx) lock in 2 steps:
 *      1. consequent polls with no sleeping between them
 *      2. polls with exponential backoff sleep periods between them
 * Parameters:
 *      unit            - (IN) The unit number
 *      link            - (IN) Link id
 *      min_nof_polls   - (IN) Nof consequent polls with no sleeping between them (step 1)
 *      usec_timeout    - (IN) The maximum total sleep time between the polls (step 2)
 *      rx_locked       - (OUT) Is cdr (Rx) locked
 * Returns:
 *      _SHR_E_xxx
 */
static shr_error_e
 ...
 * Purpose:
 *      Retimer links calibration
 * Parameters:
 *      unit   - (IN)  The unit number
 *      link_a - (IN)  One of the links to connect
 *      link_b - (IN)  The other link to connect. If link_b is '-1' it means disconnecting the links, so no need for calibration
 * Returns:
 *      _SHR_E_xxx
 */
shr_error_e
soc_ramon_fabric_link_retimer_calibrate(int unit,  soc_port_t link_a, soc_port_t link_b)
{
    ...
    /* ====================== */
    /* Poll for CDR (Rx) Lock */
    /* ====================== */
    min_nof_polls = 1;
    /* total sleep of 30ms between polls */
    usec_timeout = 30000;
    ...
    /* retimer calibrate */
    /* ================= */
    /* to start calibration it is required to wait 100msec after CDR is locked */
    sal_usleep(100000);
 ...
 *      unit   - (IN)  The unit number
 *      link_a - (IN)  One of the links to connect.
 *      link_b - (IN)  The other link to connect. If link_b is '-1' then disconnect the links.
 * Returns:
 *      _SHR_E_xxx
 */
shr_error_e
soc_ramon_fabric_link_retimer_connect_set(int unit, soc_port_t link_a, soc_port_t link_b)
{
    ...
    if (link_b != -1)
    {
        /* validate links are not in retimer mode */
        ...
        /* validate that both links have the same FEC configuration */
    ...
    /* =============================================== */
    /* Restore original enable state of the FMAC lanes */
    /* =============================================== */
 ...
 * Parameters:
 *      unit   - (IN)  The unit number
 *      link_a - (IN)  The retimer link.
 *      link_b - (OUT) The retimer connected link. link_b == -1 means that the links are disconnected.
 * Returns:
 *      _SHR_E_xxx
 */
shr_error_e
soc_ramon_fabric_link_retimer_connect_get(int unit, soc_port_t link_a, soc_port_t *link_b)
{
    ...
    /* Loopback Retimer or Remote Loopback */
    else
    {
