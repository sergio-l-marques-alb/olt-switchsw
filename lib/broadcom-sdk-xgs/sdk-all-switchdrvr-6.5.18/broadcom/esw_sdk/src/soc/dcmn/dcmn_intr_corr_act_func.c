    ...
    char* msg)
{
    ...
    /* Don't care cachable or not */
    SOCDNX_NULL_CHECK(ecc_1b_correct_info_p);
    /* handle write-only table, can not display its data */
    ...
    /* dump data from memory without cache */
...
get_mem_severity(int unit, soc_mem_t mem)
{
    if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
        switch (mem) {
            /* add these memory have ecc filed, but SW not release */
...
/* See dcmn_mem_decide_corrective_action
    DCMN_ECC_PARITY_DC
    DCMN_ECC_ECC1B_DC,
    DCMN_ECC_ECC2B_DC
*/
char* dcmn_mem_ser_ecc_action_info_get(int unit, soc_mem_t mem, int isEcc1b)
{
    ...
    static char*    str_ecc_action[] = 
        {
            ...
            "EM_SOFT_RECOVERY",     /* 3 */
            "SOFT_RESET",           /* 4 */
            "SHADOW if Cached, Else ECC_1B_FIX",                /* 5 */
    ...
    if (SOC_MEM_TYPE(unit,mem) == SOC_MEM_TYPE_XOR)
    {
        ...
        {
            rc_ecc1b = 1;   /* DCMN_INT_CORR_ACT_EM_ECC_1B_FIX */
        }
        else if(dynamic_mem)
        {
        ...
        {
            rc_ecc1b = 2;   /* DCMN_INT_CORR_ACT_XOR_FIX */
        }
        rc_ecc2b  = 9;
    }
    else if (dynamic_mem)
    ...
    *p_corrective_action = DCMN_INT_CORR_ACT_NONE;
    ...
    if (addr_validf != 0) {
     ...
     if (soc_property_suffix_num_get(unit,-1, spn_CUSTOM_FEATURE, "ser_stop_traffic", 0)) {
        /* once SER error, disable packet traffic */
        if (SOC_IS_JERICHO(unit)) {
    ...
    for (current_index = xor_correct_info->min_index; current_index <= xor_correct_info->max_index; current_index++) {
        ...
        for (sp_index = 0; sp_index < sp_banks_num; sp_index++) {
            ...
            if( clear) {
                ...
                /* for ecc2bit or parity error, must get data from cache */
    ...
    dcmn_int_corr_act_type* corrective_action)
{
    ...
    /* using tcam entry shadow value to for tcam parity error correction. There are no needs to care entry size */
