...
/**
 * \brief 
 * Update dbal pemla tables mappings according to the current
 * ucode mappings for table ID's, fieldID's and field sizes, a 
 * table which is not found in Application DB (Pemla ucode db), 
 * cannot be accessed using the pemla driver, this table is set 
 * to ML=0, and an Error status is set. 
 */
shr_error_e
    ...
    int unit)
{
    ...
    for (table_id = 0; table_id < DBAL_NOF_TABLES; table_id++)
    {
        ...
        if (table->access_method == DBAL_ACCESS_METHOD_PEMLA)
        {
            ...
            {
                /*
                 * Table info not found in Pemla driver ucode DB so we cannot access it using pemla driver - disable table.
                 */
                table->maturity_level = DBAL_MATURITY_LOW;
            ...
            /*
             * update table fields mapping and size
             */
            for (res_type_index = 0; res_type_index < table->nof_result_types; res_type_index++)
            {
                ...
                for (field_index = 0; field_index < multi_res_info->nof_result_fields; field_index++)
                {
                    ...
                    /*
                     * update field info in dbal table
                     */
                    ...
                    {
                        /*
                         * size mismatch - set to lower boundary.
                         */
...
/**
 * This function initialize a pool of buffers for accessing the pemla driver.
 */
shr_error_e
dbal_pemla_init(
    int unit)
...
/**
 * This function deallocate the a pool of buffers for accessing
 * the pemla driver.
 */
shr_error_e
dbal_pemla_deinit(
...
/** ****************************************************
* \brief
* The function is used to add entry to a dbal pemla table of
* type pemla_reg it build the pemla driver data base according
* to the commit request and call the driber API
*
*  \par DIRECT INPUT:
*    \param [in] unit
...
* \brief
* The function is used to add entry to a DBAL PEMLA table of
* type pemla_memory it build the PEMLA driver data base according
* to the commit request and call the driver API
*
*  \par DIRECT INPUT:
*    \param [in] unit
*    \param [in] entry_handle - \n
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    /**SHR_IF_ERR_EXIT(pemladrv_direct_write(unit, entry_handle->table->pemla_mapping.reg_mem_id, pemla_drv_mem));*/
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    switch (entry_handle->table->table_type)
    {
        ...
        case DBAL_TABLE_TYPE_DIRECT:
            /** need to add indication for register/memory  */
...
/** ****************************************************
* \brief
* The function is used to get entry from dbal pemla table of
* type pemla_reg the function assume it reads all fields from hw
* using pemla driver and return the requested fields.
*
*  \par DIRECT INPUT:
*    \param [in] unit
*    \param [in] entry_handle - \n
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    /** There is an assumption that read one field in
     *  register of all fields will have the same performance. */
    for (field_index = 0; field_index < nof_results_field; field_index++)
    {
        table_field_info = &(DBAL_RES_INFO.results_info[field_index]);
        ...
        if (entry_handle->table->pemla_mapping.result_fields_mapping[field_index] == DBAL_PEMLA_FIELD_MAPPING_INVALID)
        {
            /** Field is disabled due to mismatch between ARCH to ucode */
            continue;
        }
    ...
    /** check if we need to specify field id, can I assume order
     *  is fixed */
...
/** ****************************************************
* \brief
* The function is the generic API to get an entry from dbal
* PEMLA tables.
*  \par DIRECT INPUT:
*    \param [in] unit
*    \param [in] entry_handle - \n
...
/** ****************************************************
* \brief
* The function is used to clear entry of a dbal pemla table of
* type pemla_reg
*
*  \par DIRECT INPUT:
*    \param [in] unit
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    /** create entry payload buffer, keep size aligned to words */
...
/** ****************************************************
* \brief
* The function is the generic API to clear an entry in dbal
* PEMLA tables.
*  \par DIRECT INPUT:
*    \param [in] unit
*    \param [in] entry_handle - \n
...
/** ****************************************************
* \brief
* The function is used to clear a dbal pemla table of
* type pemla_reg
*
*  \par DIRECT INPUT:
*    \param [in] unit
...
/**
 * \brief - pemla table iterator get_next.
 *
 * \par DIRECT_INPUT:
 *   \param [in] unit -
 *   \param [in] entry_handle -
