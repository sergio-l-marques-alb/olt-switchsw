    ...
    int unit)
{
    ...
    alloc_size =
        ...
        dnx_data_mdb.dh.nof_rows_in_eedb_bank_get(unit);
    ...
    uint32 cluster_index_val)
{
    ...
    alloc_range_size = dnx_data_mdb.dh.nof_rows_in_eedb_bank_get(unit);
         ...
         address_iter++)
    {
        ...
        else if (eedb_mem_type == MDB_EEDB_MEM_TYPE_PHY)
        {
            ...
            addr_map_val &=
                ~((EEDB_ADDR_MAP_MASK << EEDB_ADDR_MAP_DATA_TABLE_OFFSET) |
                  (EEDB_ADDR_MAP_MASK << EEDB_ADDR_MAP_DATA_CLUSTER_OFFSET));
    ...
    int *cluster_rows)
{
    ...
    {
        *cluster_rows = dnx_data_mdb.dh.nof_rows_in_macro_a_cluster_get(unit);
    }
    else if (macro_type == MDB_MACRO_B)
    {
        *cluster_rows = dnx_data_mdb.dh.nof_rows_in_macro_b_cluster_get(unit);
    }
    else if (macro_type == MDB_EEDB_BANK)
    {
        *cluster_rows = dnx_data_mdb.dh.nof_rows_in_eedb_bank_get(unit);
    }
    else
    {
    ...
    mdb_eedb_entry_info_t * eedb_entry_info)
{
    ...
    if (mdb_physical_table_id_ll != EEDB_ADDR_MAP_INVALID)
    {
    ...
    if (mdb_physical_table_id_data != EEDB_ADDR_MAP_INVALID)
    {
        ...
        if ((mdb_physical_table_id_data == DBAL_ENUM_FVAL_MDB_PHYSICAL_TABLE_EEDB_1_DATA) ||
            (mdb_physical_table_id_data == DBAL_ENUM_FVAL_MDB_PHYSICAL_TABLE_EEDB_2_DATA) ||
            (mdb_physical_table_id_data == DBAL_ENUM_FVAL_MDB_PHYSICAL_TABLE_EEDB_3_DATA) ||
            (mdb_physical_table_id_data == DBAL_ENUM_FVAL_MDB_PHYSICAL_TABLE_EEDB_4_DATA) ||
            (mdb_physical_table_id_data == DBAL_ENUM_FVAL_MDB_PHYSICAL_TABLE_EEDB_5_DATA) ||
            (mdb_physical_table_id_data == DBAL_ENUM_FVAL_MDB_PHYSICAL_TABLE_EEDB_6_DATA) ||
            (mdb_physical_table_id_data == DBAL_ENUM_FVAL_MDB_PHYSICAL_TABLE_EEDB_7_DATA) ||
            (mdb_physical_table_id_data == DBAL_ENUM_FVAL_MDB_PHYSICAL_TABLE_EEDB_8_DATA))
        {
            SHR_IF_ERR_EXIT(mdb_db_infos.db.
    ...
    uint32 flags)
{
    ...
    {
        entry_max_size =
            MDB_PAYLOAD_SIZE_TYPE_TO_PAYLOAD_SIZE(DBAL_ENUM_FVAL_MDB_DIRECT_PAYLOAD_TYPE_30B) +
    ...
    if (eedb_entry_info.ll_valid == TRUE)
    {
        ...
        {
            MDB_EEDB_ACCESS_WA(unit, soc_mem_array_read
        ...
        {
            MDB_EEDB_ACCESS_WA(unit, soc_mem_array_write
                               (unit, dnx_data_mdb.eedb.entry_bank_get(unit), eedb_entry_info.ll_bank_idx,
                    ...
                    else
                    {
                        MDB_EEDB_ACCESS_WA(unit, soc_mem_array_read
                                           (unit, dnx_data_mdb.eedb.entry_bank_get(unit), eedb_entry_info.data_bank_idx,
                                            MEM_BLOCK_ANY, eedb_entry_info.data_bank_row, row_data));
                    }
                }
    ...
    uint32 flags)
{
    ...
    if (eedb_entry_info.ll_valid == TRUE)
    {
        ...
        SHR_BITCOPY_RANGE(entry->payload, 0, row_data,
                          (MDB_NOF_CLUSTER_ROW_BITS - eedb_entry_info.ll_row_offset) - ll_entry_size, ll_entry_size);
    ...
    int full_time)
{
    ...
    for (op_iter = 0; op_iter < 4; op_iter++)
    {
        ...
        for (table_iter = 0; table_iter < 3; table_iter++)
        {
            ...
            if (table_iter == 0)
            {
                ...
                entry_payload_type = DBAL_ENUM_FVAL_MDB_DIRECT_PAYLOAD_TYPE_30B;
            }
                ...
                while (entry_index < end_address * entries_per_row)
                {
                    ...
                    if (op_iter == 0)
                    {
                        ...
                        SHR_IF_ERR_EXIT(mdb_eedb_table_entry_add
                                        (unit, physical_tables, 0 /* app_id */ , &entry, entry_flags[0]));
                        if (full_time == FALSE)
                        {
                    ...
                    if (op_iter == 1)
                    {
                        ...
                        SHR_IF_ERR_EXIT(mdb_eedb_table_entry_get
                                        (unit, dbal_physical_table_id, 0 /* app_id */ , &entry, entry_flags[0]));
                        if (full_time == FALSE)
                        {
                        ...
                        if (utilex_bitstream_have_one_in_range
                            (entry.payload, 0 /* start_place */ , entry.payload_size - 1))
                        {
    ...
    LOG_INFO(BSL_LOG_MODULE,
             (BSL_META_U
              (unit,
               "In total %d entries were added, full time measurement = %d,\navg usecs per entry add: %d\navg usecs per entry HW get: %d\navg usecs per entry delete: %d\navg usecs per entry SW get after delete: %d\n"),
              total_entry_count == 1 ? 0 : total_entry_count, full_time, add_total_time_usecs / total_entry_count,
              get_total_time_usecs / total_entry_count, del_total_time_usecs / total_entry_count,
              get_post_total_time_usecs / total_entry_count));
    ...
    dbal_enum_value_field_mdb_physical_table_e mdb_physical_table_id)
{
    ...
    {
        case DBAL_ENUM_FVAL_MDB_PHYSICAL_TABLE_EEDB_1_DATA:
        case DBAL_ENUM_FVAL_MDB_PHYSICAL_TABLE_EEDB_1_LL:
        case DBAL_ENUM_FVAL_MDB_PHYSICAL_TABLE_EEDB_1_2_DATA:
            phase_index = DBAL_ENUM_FVAL_MDB_EEDB_PHASE_PHASE_1;
            break;
        case DBAL_ENUM_FVAL_MDB_PHYSICAL_TABLE_EEDB_2_DATA:
        case DBAL_ENUM_FVAL_MDB_PHYSICAL_TABLE_EEDB_2_LL:
            phase_index = DBAL_ENUM_FVAL_MDB_EEDB_PHASE_PHASE_2;
            break;
        case DBAL_ENUM_FVAL_MDB_PHYSICAL_TABLE_EEDB_3_DATA:
        case DBAL_ENUM_FVAL_MDB_PHYSICAL_TABLE_EEDB_3_LL:
        case DBAL_ENUM_FVAL_MDB_PHYSICAL_TABLE_EEDB_3_4_DATA:
            phase_index = DBAL_ENUM_FVAL_MDB_EEDB_PHASE_PHASE_3;
            break;
        case DBAL_ENUM_FVAL_MDB_PHYSICAL_TABLE_EEDB_4_DATA:
        case DBAL_ENUM_FVAL_MDB_PHYSICAL_TABLE_EEDB_4_LL:
            phase_index = DBAL_ENUM_FVAL_MDB_EEDB_PHASE_PHASE_4;
            break;
        case DBAL_ENUM_FVAL_MDB_PHYSICAL_TABLE_EEDB_5_DATA:
        case DBAL_ENUM_FVAL_MDB_PHYSICAL_TABLE_EEDB_5_LL:
        case DBAL_ENUM_FVAL_MDB_PHYSICAL_TABLE_EEDB_5_6_DATA:
            phase_index = DBAL_ENUM_FVAL_MDB_EEDB_PHASE_PHASE_5;
            break;
        case DBAL_ENUM_FVAL_MDB_PHYSICAL_TABLE_EEDB_6_DATA:
        case DBAL_ENUM_FVAL_MDB_PHYSICAL_TABLE_EEDB_6_LL:
            phase_index = DBAL_ENUM_FVAL_MDB_EEDB_PHASE_PHASE_6;
            break;
        case DBAL_ENUM_FVAL_MDB_PHYSICAL_TABLE_EEDB_7_DATA:
        case DBAL_ENUM_FVAL_MDB_PHYSICAL_TABLE_EEDB_7_LL:
        case DBAL_ENUM_FVAL_MDB_PHYSICAL_TABLE_EEDB_7_8_DATA:
            phase_index = DBAL_ENUM_FVAL_MDB_EEDB_PHASE_PHASE_7;
            break;
        case DBAL_ENUM_FVAL_MDB_PHYSICAL_TABLE_EEDB_8_DATA:
        case DBAL_ENUM_FVAL_MDB_PHYSICAL_TABLE_EEDB_8_LL:
            phase_index = DBAL_ENUM_FVAL_MDB_EEDB_PHASE_PHASE_8;
            break;
        default:
    ...
    dbal_enum_value_field_mdb_physical_table_e mdb_physical_table_id)
{
    ...
    switch (mdb_physical_table_id)
    {
        ...
        case DBAL_ENUM_FVAL_MDB_PHYSICAL_TABLE_EEDB_5_6_DATA:
        case DBAL_ENUM_FVAL_MDB_PHYSICAL_TABLE_EEDB_7_8_DATA:
            eedb_mem_type = MDB_EEDB_MEM_TYPE_PHY;
            break;
        case DBAL_ENUM_FVAL_MDB_PHYSICAL_TABLE_EEDB_1_LL:
        case DBAL_ENUM_FVAL_MDB_PHYSICAL_TABLE_EEDB_2_LL:
        case DBAL_ENUM_FVAL_MDB_PHYSICAL_TABLE_EEDB_3_LL:
