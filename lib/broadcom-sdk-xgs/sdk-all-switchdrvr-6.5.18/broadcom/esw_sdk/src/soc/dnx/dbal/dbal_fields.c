    ...
    dbal_field_types_defs_e field_type)
{
    ...
    /** First updating all the sizes (In case the length is 0 it means that the size should be taken from the type */
    ...
    /** now updating offsets starting from 1 since the first one is "0" */
    ...
    /** now update arr prefix size and arr prefix value if exist */
    for (ii = 0; ii < struct_info->nof_struct_fields; ii++)
    {
        if (struct_info->struct_field_info[ii].arr_prefix_table_entry_width)
        {
            ...
            SHR_IF_ERR_EXIT(dbal_field_arr_prefix_calculate(unit,
                                                            ...
                                                            struct_info->
                                                            struct_field_info[ii].arr_prefix_table_entry_width,
                                                            struct_info->
                                                            struct_field_info[ii].arr_prefix_table_entry_value,
                                                            int_field_type_info->max_size,
...
/**
 * \brief
 * validating the field type max size according to the print type.
 */
static shr_error_e
...
/** run over all field types, Validate the sizes. updating the struct fields sizes, this is done only after all field
 *  types where set since the sizes of the struct fields are defined in other field types. */
static shr_error_e
    ...
    int unit)
{
    ...
    for (field_type = DBAL_FIELD_TYPE_DEF_EMPTY + 1; field_type < DBAL_NOF_FIELD_TYPES_DEF; field_type++)
    {
        ...
        /** updating the struct fields sizes */
        if (field_types_basic_info[unit][field_type].print_type == DBAL_FIELD_PRINT_TYPE_STRUCT)
        {
            SHR_IF_ERR_EXIT(dbal_field_types_update_structure_info(unit, field_type));
        }

        /** callback assign for illeagal values  */
        SHR_IF_ERR_EXIT(dbal_field_types_init_illegal_value_cb_assign(unit,
                                                                      field_type,
                                                                      &(field_types_basic_info[unit][field_type])));
    }
    ...
    xml_node top_node)
{
    ...
    /** indication at xml file level.
       See DBAL_FIELD_IND_IS_COMMON and DBAL_FIELD_IND_IS_DEFAULT for more information */
    uint8 is_common = 0;
    uint8 is_default = 0;
    ...
    {
        /** the field type dynamically loaded is expected to be in a
           specific image. isCommon attribute is for field type shared 
           between all images isDefault attribute is for field type 
           which can be overriden. image indicate that the field type 
           is to be loaded for a specific image */
    ...
    /**Iterate on all the field types in the xml*/
    RHDATA_ITERATOR(field_type_node, top_node, "FieldType")
    {
        ...
        /**Get the relative field types array entry*/
        field_type_info = &field_types_basic_info[unit][field_types_def_id];

        /** the overriden field type is expected to have default */
        ...
        /** setting invalid enum values, all the enum values that not exists on the new enum*/
        ...
        /**Re-alloc the field type enum's structs*/
        ...
        sal_memcpy(field_type_info->enum_val_info, tmp_enum_val_info,
                   field_type_info->nof_enum_values * sizeof(dbal_enum_decoding_info_t));
        /**Set all original enums as invalid. Later on we'll traverse the new enums and set all of the new values.
         * The enums which only existed in the original list will remain invalid*/
        ...
        {
            /**Make sure that the new enum was added in EnumValues and in EnumValueMapping at the same index*/
            ...
            sal_snprintf(name_str, DBAL_MAX_LONG_STRING_LENGTH, "%s_%s", field_type_name, str);
            rv = dbal_enum_type_string_to_id_no_err(unit, field_types_def_id, str, &enum_idx);
            if (rv == _SHR_E_NOT_FOUND)
            {
                /**Add the new enum*/
                enum_idx = next_free_idx++;
                sal_strncpy(field_type_info->enum_val_info[enum_idx].name, name_str,
                            DBAL_MAX_EXTRA_LONG_STRING_LENGTH - 1);
            }
    ...
    int unit)
{
    ...
    /**Dynamic loading in case of a non-standard image*/
    if (DBAL_DYNAMIC_XML_TABLES_ENABLED)
    {
        ...
        SHR_IF_ERR_EXIT(dbal_dynamic_updates_pre_compiled(unit, dnx_dynamic_xml_field_types_update));
...
/*
 *  This function is used to verify the struct field arr_prefix ,
 *  It verify min val and max val for the field including the arr prefix
 */
static shr_error_e
    ...
    uint32 *full_field_mask)
{
    ...
    {
        /*
         * Verify the arr prefix for the field check min and max values
         */
    ...
    shr_error_e ret_val)
{
    ...
    {
     /** in this case the value is not valid for this device.
      *  during init ignoring this error, if we are not in std1
      *  after init printing an error
      *  for key fields also need to set the handle status to ignore the action TBD. */
        ...
        {
            SHR_SET_CURRENT_ERR(_SHR_E_UNAVAIL);
            /** Key not mapped error, key fields with error _SHR_E_UNAVAIL required that the complete operation will be skipped,
             *  the hadnle error is updated  to _SHR_E_UNAVAIL and the field Id to the not mapped key field id. (this happen outside
...
{
    dbal_field_types_defs_e field_type;
    uint32 parent_field_val_arr[DBAL_FIELD_ARRAY_MAX_SIZE_IN_WORDS] = { 0 }; /** for coverity*/
