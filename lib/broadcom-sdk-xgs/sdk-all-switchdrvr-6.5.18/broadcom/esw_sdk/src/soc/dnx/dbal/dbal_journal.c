            ...
            entries_match =
                (sal_memcmp
                 (ptr_payload->phy_entry.payload, get_entry_handle->phy_entry.payload,
                  BITS2BYTES(UTILEX_MAX(get_entry_handle->phy_entry.payload_size, ptr_payload->phy_entry.payload_size)))
                 == 0);
        }
    ...
    uint8 is_dbal_error)
{
    ...
    if (is_er
        ...
        && (!is_dbal_error || (DNXC_ROLLBACK_JOURNAL_ENTRY_STATE_ALWAYS_VALID == entry_state)))
    {
    ...
    uint8 is_comparison)
{
    ...
    if (is_comparison)
    {
        dnxc_rollback_journal_disabled_counter_get(unit, DNX_DBAL_COMPARISON_JOURNAL_HANDLE, &counter);
    }
    else
    {
        dnxc_rollback_journal_disabled_counter_get(unit, DNX_DBAL_ROLLBACK_JOURNAL_HANDLE, &counter);
    }
    ...
    uint8 is_comparison)
{
    ...
    if (!is_comparison)
    {
        SHR_IF_ERR_EXIT(dnxc_rollback_journal_logger_state_change(unit, DNX_DBAL_ROLLBACK_JOURNAL_HANDLE));
    }
    else
    {
        SHR_IF_ERR_EXIT(dnxc_rollback_journal_logger_state_change(unit, DNX_DBAL_COMPARISON_JOURNAL_HANDLE));
    }
