 ...
 * The hooked fields are mapped to ETPPA_PER_PORT_TABLE, ERPP_PER_PORT_TABLE, ERPP_CFG_ENABLE_FILTER_PER_PORT_TABLE
 * or ETPPC_EDITING_PER_PORT_TABLE
    ...
    uint32 *has_hook_fields)
{
    ...
    if ((!
        ...
        && (*entry_handle->table->hl_mapping_multi_res[entry_handle->cur_res_type].l2p_hl_info[DBAL_HL_ACCESS_MEMORY].
            l2p_fields_info[iter].memory != ERPP_CFG_ENABLE_FILTER_PER_PORT_TABLEm) && (!(SOC_IS_JERICHO2_A0(unit)
                                                                                          && (*entry_handle->table->
                                                                                              ...
                                                                                              ETPPC_EDITING_PER_PORT_TABLEm))))
    {
        ...
        /*
         * Remove the field from entry_handle_hook only if the field is not mapped also to hook memory
         * ETPPA_PER_PORT_TABLE, ERPP_PER_PORT_TABLE, ERPP_CFG_ENABLE_FILTER_PER_PORT_TABLE or ETPPC_EDITING_PER_PORT_TABLE)
         */
        if (((entry_handle_hook->value_field_ids[field_pos] != DBAL_FIELD_EMPTY) && (iter == 0))
            || ((entry_handle_hook->value_field_ids[field_pos] != DBAL_FIELD_EMPTY) && (field_pos > 0) && (iter > 0)
                ...
                ((!(SOC_IS_JERICHO2_ONLY(unit)
                 ...
                 &&
                 (!(SOC_IS_JERICHO2_A0(unit)
                    && (*entry_handle->table->
                        hl_mapping_multi_res[entry_handle->cur_res_type].l2p_hl_info[DBAL_HL_ACCESS_MEMORY].
                        l2p_fields_info[iter - 1].memory != ETPPC_EDITING_PER_PORT_TABLEm))))) || ((field_pos > 0)
                                                                                                   && (iter > 0)
                                                                                                   &&
                                                                                                   (entry_handle->table->
                                                                                                    hl_mapping_multi_res
                                                                                                    [entry_handle->cur_res_type].l2p_hl_info
                                                                                                    [DBAL_HL_ACCESS_MEMORY].
                                                                                                    l2p_fields_info
                                                                                                    [iter].field_id !=
                                                                                                    entry_handle->
                                                                                                    table->hl_mapping_multi_res
                                                                                                    [entry_handle->
                                                                                                     cur_res_type].l2p_hl_info
                                                                                                    [DBAL_HL_ACCESS_MEMORY].
                                                                                                    l2p_fields_info[iter
            ...
            || ((field_pos > 0) && (iter > 0)
                ...
                ((!(SOC_IS_JERICHO2_ONLY(unit)
                 ...
                 && *entry_handle->table->hl_mapping_multi_res[entry_handle->
                                                               cur_res_type].l2p_hl_info
                 [DBAL_HL_ACCESS_MEMORY].l2p_fields_info[iter + 1].memory != ERPP_CFG_ENABLE_FILTER_PER_PORT_TABLEm
                 &&
                 (!(SOC_IS_JERICHO2_A0(unit)
                    && *entry_handle->table->hl_mapping_multi_res[entry_handle->
                                                                  cur_res_type].l2p_hl_info
                    [DBAL_HL_ACCESS_MEMORY].l2p_fields_info[iter + 1].memory != ETPPC_EDITING_PER_PORT_TABLEm)))
                && (entry_handle->table->
                    hl_mapping_multi_res[entry_handle->
                                         cur_res_type].l2p_hl_info[DBAL_HL_ACCESS_MEMORY].l2p_fields_info[iter].
                    field_id ==
                    entry_handle->table->hl_mapping_multi_res[entry_handle->
                                                              cur_res_type].l2p_hl_info
    ...
    else
    {
        ...
        /*
         * In case that the access is to memory ETPPA_PER_PORT_TABLE, ERPP_PER_PORT_TABLE, ERPP_CFG_ENABLE_FILTER_PER_PORT_TABLE
         * or ETPPC_EDITING_PER_PORT_TABLE the field is deleted from
        ...
        if (((entry_handle_regular_fields->value_field_ids[field_pos] != DBAL_FIELD_EMPTY) && (iter == 0))
            || ((entry_handle_regular_fields->value_field_ids[field_pos] != DBAL_FIELD_EMPTY) && (field_pos > 0)
                ...
                && (entry_handle->table->
                    ...
                    entry_handle->table->hl_mapping_multi_res[entry_handle->
                                                              cur_res_type].l2p_hl_info[DBAL_HL_ACCESS_MEMORY].
                    l2p_fields_info[iter - 1].field_id) && ((SOC_IS_JERICHO2_ONLY(unit)
                                                             && *entry_handle->table->
                                                             hl_mapping_multi_res[entry_handle->
                                                                                  cur_res_type].l2p_hl_info
                                                            ...
                                                            ((SOC_IS_JERICHO2_A0(unit)
                                                              ...
                                                              hl_mapping_multi_res[entry_handle->
                                                                                   cur_res_type].l2p_hl_info
                                                              [DBAL_HL_ACCESS_MEMORY].l2p_fields_info[iter -
                                                                                                      1].memory ==
                                                              ETPPC_EDITING_PER_PORT_TABLEm)))) || ((field_pos > 0)
                                                                                                    && (iter > 0)
                                                                                                    &&
                                                                                                    (entry_handle->table->
                                                                                                     hl_mapping_multi_res
            ...
            || ((field_pos > 0) && (iter > 0)
                && (iter <
                    entry_handle->
                    table->hl_mapping_multi_res[0].l2p_hl_info[DBAL_HL_ACCESS_MEMORY].num_of_access_fields - 1)
                &&
                ((SOC_IS_JERICHO2_ONLY(unit)
                  && *entry_handle->table->hl_mapping_multi_res[entry_handle->
                                                                cur_res_type].l2p_hl_info
                ...
                && (entry_handle->table->
                    ...
                    l2p_fields_info[iter + 1].field_id ==
                    entry_handle->
                    table->hl_mapping_multi_res[entry_handle->cur_res_type].l2p_hl_info
                    [DBAL_HL_ACCESS_MEMORY].l2p_fields_info[iter].field_id)))
        {
            entry_handle_regular_fields->value_field_ids[field_pos] = DBAL_FIELD_EMPTY;
            entry_handle_regular_fields->nof_result_fields = entry_handle_regular_fields->nof_result_fields - 1;
...
/**
 * \brief
 * Get entry from a memory which is caucing ECC error issues.
 * To avoid them the following procedure is performed:
 * 1. Mask the relevant bit from the ECC indications.
 * 2. Read the memory. Check the data if there was an ECC error (1 or 2 bits).
 * 3. If yes - repeat "2" again until there isn't an error. After 500 iterations, if fails return error.
 * 4. If it does not fail - this is the correct data.
 * 5. Enable the relevant bit from the ECC indications back.
 */
static shr_error_e
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    /** Get the current value and disable ECC error check for the relevant HW */
    index = 0;
    ...
    for (iter = 0; iter < l2p_hl_info->num_of_access_fields; iter++)
    {
        ...
        /** Calculated the parameters needed for the access */
        ...
        if (is_condition_pass)
        {
            ...
            {   /** need to read new memory from HW */
                ...
                /** Read the memory until it is sure that the correct data is returned */
                while (correct_data_is_read != TRUE)
                {
                    ...
                    /** Check if the memory has ECC field. If yes, do manual ECC check */
                    meminfo = &SOC_MEM_INFO(unit, memory_to_read);
                    SOC_FIND_FIELD(ECCf, meminfo->fields, meminfo->nFields, fieldinfo);
                    if (fieldinfo != NULL)
                    {
                        /** From the data get the ECC field fo the memory */
                        soc_mem_field_get(unit, memory_to_read, data, ECCf, &ecc);

                        /** Get the bit width of the data without the ECC field and calculate what the ECC should be according to it */
                        mem_row_bit_width =
                        ...
                        /**
                         * If the actual ECC that was read from the memory does not match the ECC
                         *  calculated by SW we will read again. If it still does not match after 500 reads, return an error.
                         */
                        ...
                        /**
                         * If the actual ECC that was read from the memory matches the ECC
                         *  calculated by SW we have the correct data.
                         */
                        if (ecc == ecc_expected)
                        {
    ...
    /** Enable ECC error check for the relevant HW */
    index = 0;
    while (dnx_data_device.general.ecc_err_masking_get(unit, block_type)->ecc_1bit_err_field[index] != -1)
    {
        ecc_mask_reg = dnx_data_device.general.ecc_err_masking_get(unit, block_type)->ecc_1bit_err_mask_reg;
...
/**
 * \brief
 * Hook to get entry from DBAL tables that have
 * HW memories which are causing ECC issues when it is read.
 */
static shr_error_e
    ...
    for (access_type = 0; access_type < DBAL_NOF_HL_ACCESS_TYPES; access_type++)
    {
        if (table->hl_mapping_multi_res[entry_handle->cur_res_type].l2p_hl_info[access_type].num_of_access_fields > 0)
        {
            ...
            if ((access_type == DBAL_HL_ACCESS_MEMORY) && (save_only_in_sw_state == 0))
            {
                ...
                {
                    /*
                     * In case that DBAL_CORE_ALL is used and there are fields that are mapped to ETPPA_PER_PORT_TABLE, ERPP_PER_PORT_TABLE,
                     * ERPP_CFG_ENABLE_FILTER_PER_PORT_TABLE or ETPPC_EDITING_PER_PORT_TABLE
