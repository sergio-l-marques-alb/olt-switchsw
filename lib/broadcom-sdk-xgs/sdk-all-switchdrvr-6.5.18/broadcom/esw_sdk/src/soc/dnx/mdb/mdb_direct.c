...
#define MDB_DIRECT_ADDR_OFFSET_OFFSET(unit,macro_type)  ((macro_type == MDB_MACRO_A)\
                                                  ? dnx_data_mdb.dh.nof_bits_address_in_macro_a_cluster_get(unit) : dnx_data_mdb.dh.nof_bits_address_in_macro_b_cluster_get(unit))
    ...
    int unit)
{
    ...
    alloc_size =
        ...
        dnx_data_mdb.dh.nof_rows_in_macro_b_cluster_get(unit);
    ...
    uint32 cluster_index_val)
{
    ...
    alloc_range_size = dnx_data_mdb.dh.nof_rows_in_macro_b_cluster_get(unit);
    ...
    int cluster_index)
{
    ...
    {
        alloc_size = dnx_data_mdb.dh.nof_rows_in_macro_a_cluster_get(unit) + 1;
        SHR_IF_ERR_EXIT(mdb_db_infos.mdb_cluster_infos.macro_a_cluster_cache.alloc(unit, global_cluster_idx, alloc_size,
                                                                                   BITS2WORDS
                                                                                   (DNX_DATA_MAX_MDB_DH_CLUSTER_ROW_WIDTH_BITS)));
    }
    else if (macro_type == MDB_MACRO_B)
    {
        alloc_size = dnx_data_mdb.dh.nof_rows_in_macro_b_cluster_get(unit) + 1;
        SHR_IF_ERR_EXIT(mdb_db_infos.mdb_cluster_infos.macro_b_cluster_cache.alloc(unit, global_cluster_idx, alloc_size,
                                                                                   BITS2WORDS
                                                                                   (DNX_DATA_MAX_MDB_DH_CLUSTER_ROW_WIDTH_BITS)));
    }
    else if (macro_type == MDB_EEDB_BANK)
    {
        alloc_size = dnx_data_mdb.dh.nof_rows_in_eedb_bank_get(unit);
        SHR_IF_ERR_EXIT(mdb_db_infos.mdb_cluster_infos.eedb_banks_cache.alloc(unit, cluster_index, alloc_size,
                                                                              BITS2WORDS
                                                                              (DNX_DATA_MAX_MDB_DH_CLUSTER_ROW_WIDTH_BITS)));
    }
    else
    ...
    uint32 flags)
{
    ...
    if (payload_size_rounded_up > row_width)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     ...
                     dbal_physical_table_to_string(unit, dbal_physical_table_id), app_id, entry->payload_size,
                     payload_size_rounded_up, row_width);
    }
    ...
    else
    {
        ...
        SHR_IF_ERR_EXIT(mdb_direct_table_read_entry_from_cluster
                        (unit, mem, macro_type, macro_index, bucket_index, cluster_index_in_bucket, blk, &cluster_info,
                         entry->mdb_action_apply, row_in_cluster, row_data, cache_enabled));
             ...
             hit_bit_index++)
        {
                    ...
                    && _SHR_IS_FLAG_SET(entry->hitbit, DBAL_PHYSICAL_KEY_HITBIT_SECONDARY)))
            {
                ...
                row_in_cluster_modified =
                    (row_in_cluster + (hit_bit_entry_remainder_size / row_width)) & MDB_DIRECT_ADDR_OFFSET_MASK(unit,
                                                                                                                macro_type);
                ...
                for (bank_iter = MDB_ABK_BANK_A; bank_iter < MDB_NOF_ABK_BANKS; bank_iter++)
                {
                    ...
                    SHR_BITCOPY_RANGE(&hitbit, 0, hitbit_row,
                                      MDB_HITBIT_DATA_OFFSET + MDB_HITBIT_DATA_SIZE * relevant_cluster + hitbit_offset,
                                      1);
                    if (hitbit != 0)
                    {
    ...
    uint32 flags)
{
    ...
    SHR_IF_ERR_EXIT(mdb_direct_table_entry_add(unit, physical_tables, app_id, &temp_entry, 0));
    ...
    int full_time)
{
    ...
    for (op_iter = 0; op_iter < 4; op_iter++)
    {
        ...
        while (cluster_index < nof_clusters)
        {
            ...
            while (entry_index < UTILEX_DIV_ROUND_UP(end_address * row_width,
                                                     ((UTILEX_DIV_ROUND_UP
                                                       (entry_index_size,
                                                        MDB_DIRECT_BASIC_ENTRY_SIZE)) * MDB_DIRECT_BASIC_ENTRY_SIZE)))
            {
                ...
                if (op_iter == 0)
                {
                    ...
                    SHR_IF_ERR_EXIT(mdb_direct_table_entry_add(unit, physical_tables, 0 /* app_id */ , &entry, 0));
                    if (full_time == FALSE)
                    {
                ...
                if (op_iter == 1)
                {
                    ...
                    SHR_IF_ERR_EXIT(mdb_direct_table_entry_get
                                    (unit, dbal_physical_table_id, 0 /* app_id */ , &entry, 0));
                    if (full_time == FALSE)
                    {
                ...
                if (op_iter == 2)
                {
                    ...
                    SHR_IF_ERR_EXIT(mdb_direct_table_entry_delete(unit, physical_tables, 0 /* app_id */ , &entry, 0));
                    if (full_time == FALSE)
                    {
