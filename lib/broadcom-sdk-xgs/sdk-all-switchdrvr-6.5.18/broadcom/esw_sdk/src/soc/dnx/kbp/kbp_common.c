    ...
    uint8 *add_entry)
{
    ...
    {
        /*
         * The AD entry is in different AD DB. Need to delete the old one and add the new one.
         * In case the old AD entry is a 0b, the delete API will delete only the DB entry and not the AD entry.
         */
        SHR_IF_ERR_EXIT(dnx_kbp_entry_lpm_delete(unit, db_p, entry_info.ad_db, NULL, NULL, 0, 0, access_bmp_indication,
                                                 kbp_handles, db_entry));
        *add_entry = TRUE;
    }
    ...
    int unit)
{
     ...
     * As a general solution, KBP will not be initialized over cmodel for bare-metal.
