...
/** ACTION NAME        | PRINTS                  | VALIDATIONS                                        | LEGAL FLAGS  */
    ...
    {"COMMIT",         PRE_ACCESS_PRINTS_ENABLED, ALL_KEY_FIELDS_MUST_EXIST | RESULT_FIELD_MUST_EXIST | VALIDATE_ALLOCATOR_FIELD,  COMMON_LEGAL_FLAGS | DBAL_COMMIT_UPDATE | DBAL_COMMIT_FORCE | DBAL_COMMIT_OVERRIDE_DEFAULT | DBAL_COMMIT_OVERRUN},

    /** DBAL_ACTION_ENTRY_COMMIT_UPDATE */
    {"COMMIT UPDATE",  PRE_ACCESS_PRINTS_ENABLED, ALL_KEY_FIELDS_MUST_EXIST | RESULT_FIELD_MUST_EXIST | VALIDATE_ALLOCATOR_FIELD,  COMMON_LEGAL_FLAGS},

    /** DBAL_ACTION_ENTRY_COMMIT_FORCE */
    {"COMMIT FORCE",   PRE_ACCESS_PRINTS_ENABLED, ALL_KEY_FIELDS_MUST_EXIST | RESULT_FIELD_MUST_EXIST | VALIDATE_ALLOCATOR_FIELD,  COMMON_LEGAL_FLAGS},

    /** DBAL_ACTION_ENTRY_COMMIT_RANGE */
    {"COMMIT RANGE",   PRE_ACCESS_PRINTS_ENABLED, ALL_KEY_FIELDS_MUST_EXIST | RESULT_FIELD_MUST_EXIST | VALIDATE_ALLOCATOR_FIELD,  COMMON_LEGAL_FLAGS},

    /** DBAL_ACTION_ENTRY_GET */
    {"GET",            POST_ACCESS_PRINTS_ENABLED, ALL_KEY_FIELDS_MUST_EXIST | RESULT_FIELD_MUST_EXIST | VALIDATE_ALLOCATOR_FIELD, COMMON_LEGAL_FLAGS | DBAL_GET_ALL_FIELDS | DBAL_COMMIT_VALIDATE_OTHER_CORE},

    /** DBAL_ACTION_ENTRY_GET_ALL */
    {"GET ALL FIELDS", POST_ACCESS_PRINTS_ENABLED, ALL_KEY_FIELDS_MUST_EXIST| VALIDATE_ALLOCATOR_FIELD,          COMMON_LEGAL_FLAGS},
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    else
    {
             ...
             dbal_tables_is_merge_entries_supported(unit, entry_handle->table_id)) || entry_handle->er_flags)
        {
            ...
            /** if needed merging the entry with the exsiting entry in the HW  */
            if (DBAL_HANDLE_IS_MERGE_NEEDED(entry_handle))
            {
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    else
    {
        ...
        {
            /** W.A for mact we perform get from SW  */
    ...
    dbal_actions_e * action)
{
    ...
    /** key not mapped after init dumping error to consule during init ignoring */
            ...
            if (entry_handle->core_id == DBAL_CORE_ALL)
            {
                for (core_id = 0; core_id < DBAL_MAX_NUM_OF_CORES; core_id++)
                {
                    if (bit_set)
                    {
                        ...
                        DBAL_SW_STATE_TBL_PROP.iterator_optimized.bitmap.bit_set(unit, entry_handle->table_id, core_id,
                                                                                 entry_key);
                    }
                    else
                    {
    ...
    int is_sw_get)
{
    ...
    if (get_entry_handle != NULL)
    {
        ...
        /** Performance improvements can use: dbal_actions_access_entry_prepare_for_get */
        ...
        if (SHR_IS_BITSET(table->indications_bm, DBAL_TABLE_IND_HAS_RESULT_TYPE))
        {
            ...
            if (table->access_method != DBAL_ACCESS_METHOD_MDB
                || (SHR_IS_BITSET(table->indications_bm, DBAL_TABLE_IND_RESULT_TYPE_MAPPED_TO_SW)
                    && !(dbal_logical_table_is_out_lif_or_rif_allocator_eedb(table))
    ...
    if (SHR_IS_BITSET(table->indications_bm, DBAL_TABLE_IND_IS_ITERATOR_OPTIMIZED))
    {
        ...
        SHR_IF_ERR_EXIT(dbal_actions_direct_is_exists(unit, get_entry_handle, &is_exists));

        if (!is_exists)
        {
            if (!is_get_on_new_entry)
            {
                /** since entry not exists need to set to zero the payload. for  is_get_on_new_entry payload is already zero. */
                sal_memset(&get_entry_handle->phy_entry.payload, 0, DBAL_TABLE_BUFFER_IN_BYTES(table));
                sal_memset(&get_entry_handle->phy_entry.p_mask, 0, DBAL_TABLE_BUFFER_IN_BYTES(table));
            }

            /** if for one of the result types the default entry is_default_non_standard we need to update the payload to the no
             *  standard values. currenlty we are doing it by just performing entry get from HW future improvement is to do
             *  result type resolution if needed and dbal_hl_non_standard_default_entry_build() */
                ...
                && (table->table_type == DBAL_TABLE_TYPE_DIRECT))
            {
                ...
                {
                    if (table->hl_mapping_multi_res[res_type_iter].is_default_non_standard)
                    {
                        is_exists = TRUE;
                        break;
                    }
        ...
        if (!is_exists)
        {
            ...
            /** TBD - check if it better to change it _SHR_E_NONE since for direct tables there is no such thing as not_found.. */
            SHR_EXIT();
        }
    }
    ...
    dbal_entry_handle_t * entry_handle)
{
        ...
        || ((table->nof_result_types > 1) && (entry_handle->cur_res_type == DBAL_RESULT_TYPE_NOT_INITIALIZED)))
    {
        ...
        else
        {
            ...
            /** get_entry_handle is the entry that we received from HW incase that the entry is default no need to delete the entry */
            SHR_IF_ERR_EXIT(dbal_entry_direct_is_default(unit, get_entry_handle, &is_default));
            if (is_default)
            {
                SHR_EXIT();
    ...
    if (DBAL_TABLE_IS_NONE_DIRECT(entry_handle->table))
    {
        /** WA for limit per LIF counters. Payload should be added to the delete command of MACT entries */
        if (dnx_data_l2.hw_bug.feature_get(unit, dnx_data_l2_hw_bug_limit_per_lif_counters) &&
            dbal_logical_table_is_mact(unit, entry_handle->table_id))
        {
    ...
    uint8 *continue_iterating)
{
    ...
    {
        DNX_ERR_RECOVERY_EXPR_SUPPRESS_IF_ERR_EXIT(DBAL_SW_STATE_TBL_PROP.
    ...
    if (iterator_attrib_info->hit_bit_rule_valid)
    {
        ...
        if (!dbal_logical_table_is_mact(unit, entry_handle->table_id))
        {
            SHR_IF_ERR_EXIT(dbal_entry_attribute_request
                            (unit, entry_handle_id, iterator_attrib_info->hit_bit_rule_type, NULL));
        }
        else
        {
    ...
    if (iterator_attrib_info->age_rule_valid)
    {
        ...
        SHR_IF_ERR_EXIT(dbal_entry_attribute_request
                        (unit, entry_handle_id, DBAL_ENTRY_ATTR_AGE, &iterator_attrib_info->age_rule_entry_value));
    }
    ...
    {
        SHR_IF_ERR_EXIT(dbal_entry_attribute_request
                        (unit, entry_handle_id, iterator_attrib_info->hit_bit_action_get_type, NULL));
    }
    ...
    {
        SHR_IF_ERR_EXIT(dbal_entry_attribute_request
                        (unit, entry_handle_id, iterator_attrib_info->hit_bit_action_clear_type, NULL));
    }
