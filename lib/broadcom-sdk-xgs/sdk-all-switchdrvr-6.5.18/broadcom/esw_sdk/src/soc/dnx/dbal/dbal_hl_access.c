...
#include <soc/drv.h>    /* SOC_BLOCK_ITER_ALL */
    ...
    int *num_of_blocks)
{
    ...
    else
    {
        ...
        {
            /** this check can be added once entry_get will be modified to call this function with specific core once the core is
             *  all (since get is only relevant for one core...) */
            ...
            /** the following code was the original related to the above issue */
#if 0
                (*num_of_blocks) = 1;
                (*block) = REG_PORT_ANY;
                                     /**just making sure that this is the value*/
#endif
            }
            /*
             * else { SHR_ERR_EXIT(_SHR_E_INTERNAL, "issue with core mode for DPC table (%s) core_id = %d\n",
             * entry_handle->table->table_name, core_id); }
             */
        }
    }
    ...
    int *num_of_blocks)
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "issue with core mode for DPC table (%s) core_id = %d\n",
                             entry_handle->table->table_name, core_id);
            }*/
        }
    }
    ...
    uint32 *dst)
{
    ...
    if (curr_l2p_info->hw_field != INVALIDf)
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "table %s, field %s,  hw_field %s is invalid for device\n",
                         dbal_logical_table_to_string(unit, entry_handle->table_id), dbal_field_to_string(unit,
                                                                                                          curr_l2p_info->field_id),
    ...
    int num_of_entries)
{
    ...
    {
        /** incase of DPC table need to peform action per core */
        core_id = 0;
        start_iteration = 0;
    ...
    {
        /** for SBC tables configuration is identical for both cores, for DPC only one core is affected  */
        start_iteration = 0;
        end_iteration = 1;
        core_id = entry_handle->core_id;
    }
        ...
        for (iter = 0; iter < l2p_hl_info->num_of_access_fields; iter++)
        {
            ...
            if (is_condition_pass)
            {
                    ...
                    (last_mem_offset_used != mem_entry_offset) || (last_mem_array_offset_used != mem_array_offset))
                {
                    ...
                    /** during init not performing get */
    ...
    int group_offset)
{
    ...
    else
    {
        ...
        if (l2p_info->hw_entity_group_id != DBAL_HW_ENTITY_GROUP_EMPTY)
        {
                        /** incase group offset is -1 and we have a group need to update all registers in group   */
            all_reg_in_group = 1;
        }
    }
    ...
    for (block_indx = 0; block_indx < nof_blocks; block_indx++)
    {
        ...
        /** updating the remaining registers in group if needed */
    if (all_reg_in_group)
    {
...
/**
 *  \brief
 *  this function build a default entry that uses to clear the entry setting the requested key fields and updating the
 *  result fields with the default values of the entry. used also by tcam_cs
 */
shr_error_e
    ...
    dbal_entry_handle_t * default_handle)
{
    ...
    if (entry_handle->table->table_type == DBAL_TABLE_TYPE_DIRECT)
    {
        ...
        {
            /** entry merged set to one, it means that we don't expect that all the fields was set
             *  (this is improtant for the packed mode only since the packed mode expect that all the fields where set) */
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    if (!rv)
    {
        /** in-case that all the result fields are readonly no need to write to HW...*/
