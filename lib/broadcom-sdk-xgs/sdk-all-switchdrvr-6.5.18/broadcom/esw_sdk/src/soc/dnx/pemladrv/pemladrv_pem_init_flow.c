...
/*
 * \brief - Read all the registers/memories that are going to be changed.
 */
static shr_error_e
...
/*
 * \brief - Write all the registers/memories that were changed,
 *          with the prior original values
 */
static shr_error_e
...
/*
 * \brief - Configure pipe and pem, and send packet.
 *          This should fix a VTC PEM bug
 */
static shr_error_e
    ...
    int unit)
{
    ...
    /*
     * The following configuration should be set before configuring the PEM,
     * to overcome a bug in VTC PEM
     */
    ...
    for (ii = 0; ii < 8; ii++)
    {
        /** PEM_XXX_CONFIG have 4 elements */
        for (jj = 0; jj < 4; jj++)
        {
