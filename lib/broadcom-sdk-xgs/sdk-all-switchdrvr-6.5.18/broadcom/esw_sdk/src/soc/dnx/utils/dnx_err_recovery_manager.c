...
do {                                                                                                                                       \
    if(!dnx_err_recovery_common_flag_is_on(unit, DNXC_ROLLBACK_JOURNAL_TYPE_ROLLBACK, DNX_ERR_RECOVERY_COMMON_IS_ON_NOT_INVALIDATED)){\
        SHR_EXIT();                                                                                                                        \
    }                                                                                                                                      \
}                                                                                                                                          \
while(0)
    ...
    int unit)
{
    ...
    dnx_err_recovery_common_flag_clear(unit, DNXC_ROLLBACK_JOURNAL_TYPE_ROLLBACK,
                                       DNX_ERR_RECOVERY_COMMON_IS_ON_IN_TRANSACTION);

    dnx_err_recovery_manager[unit].is_suppressed_counter = 0;
    dnx_err_recovery_common_flag_set(unit, DNXC_ROLLBACK_JOURNAL_TYPE_ROLLBACK,
    ...
    uint8 is_on)
{
    ...
    SHR_IF_ERR_EXIT(dnxc_rollback_journal_is_init_check_bypass(unit, is_on));
    ...
    int unit)
{
    return (SOC_UNIT_VALID(unit)
            && dnxc_rollback_journal_is_done_init(unit) && !SOC_IS_DETACHING(unit)
            && dnx_err_recovery_common_flag_is_on(unit, DNXC_ROLLBACK_JOURNAL_TYPE_ROLLBACK,
                                                  DNX_ERR_RECOVERY_COMMON_IS_ON_INITIALIZED));
}
...
dnx_err_recovery_is_on(
    int unit)
{
    return dnx_err_recovery_common_is_on(unit, DNXC_ROLLBACK_JOURNAL_TYPE_ROLLBACK)
    ...
    int unit)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnxc_generic_state_journal_all_tmp_suppress_unsafe(unit, TRUE));
    ...
    int unit)
{
    ...
    if (dnx_err_recovery_manager[unit].transaction_counter != 0 && dnx_er_threading_is_journaling_thread(unit))
    {
        SHR_IF_ERR_EXIT(dnxc_generic_state_journal_all_tmp_suppress_unsafe(unit, FALSE));
...
{
#ifdef DNXC_ROLLBACK_JOURNAL_IS_ENABLED
    return dnx_err_recovery_common_flag_is_on(unit, DNXC_ROLLBACK_JOURNAL_TYPE_ROLLBACK,
                                              DNX_ERR_RECOVERY_COMMON_IS_ON_INITIALIZED);
#else
    return 0;
#endif /* DNXC_ROLLBACK_JOURNAL_IS_ENABLED */
