    ...
    if (dnx_data_dram.hbm.feature_get(unit, dnx_data_dram_hbm_is_supported))
    {
        ...
        SHR_BIT_ITER(dram_bitmap, nof_hbms, hbm_index)
        {
            for(int channel = 0; channel < nof_channels_in_hbm; ++channel)
            {
                ...
                SHR_IF_ERR_EXIT(WRITE_HBC_SBUS_LAST_IN_CHAINr(unit, channel + hbm_index * nof_channels_in_hbm, 0));
            }
            /** set last channel of hbm to be last in chain */
            hbc_last_in_chain =  ((hbm_index + 1) * nof_channels_in_hbm) - 1;
            /** count active HBMs */
            ++active_hbms;
        }
        /** if any hbms are enabled, set last in chain */
        if (hbc_last_in_chain != -1)
        {
            if(nof_hbms == active_hbms)
            {
                /** if all HBMs are active, take last one from dnx data. this is required because the HBMs indexes and order on the bus are not aligned */
                SHR_IF_ERR_EXIT(WRITE_HBC_SBUS_LAST_IN_CHAINr(unit, dnx_data_dram.hbm.hbc_physical_last_in_chain_get(unit), 1));
            }
            else
            {
                /** else take the value found */
                SHR_IF_ERR_EXIT(WRITE_HBC_SBUS_LAST_IN_CHAINr(unit, hbc_last_in_chain, 1));
            }
        }
    }
exit:
 ...
 */
int dnx_redo_soft_reset_soft_init(int unit, int perform_soft_reset, int without_fabric)
{
    ...
    SHR_IF_ERR_EXIT(soc_dnx_soft_init(unit, without_fabric));

    /*
     * After soft reset all credits counters init to zero
     * Therfore we should reconfigued the credits for the valid interfaces
     * Provide credits to FQP manually.
     */
    if (SOC_IS_Q2A(unit) || SOC_IS_J2C(unit))
    {
        soc_port_t port;
        soc_pbmp_t nif_ports, flexe_mac_ports;
...
extern int
bcm_dnx_init_only_enough_for_access(int unit, int action);
...
soc_dnx_device_reset(int unit, int mode, int action)
{
    ...
    case SOC_DNX_RESET_MODE_BLOCKS_SOFT_RESET:
        SHR_IF_ERR_EXIT(dnx_redo_soft_reset_soft_init(unit, 0, 1));
        break;
    ...
    case SOC_DNX_RESET_MODE_REG_ACCESS:
        SHR_IF_ERR_EXIT(bcm_dnx_init_only_enough_for_access(unit, action));
        break;
    default:
