    ...
    dbal_pointer_type_e pointer_type)
{
    ...
    if (DBAL_RUNTIME_VERIFICATIONS_ENABLED(unit) && (table_field->field_nof_bits <= 32) &&
    ...
    if (field_mask)
    {
        ...
        DBAL_FIELD_ERR_HANDLE(dbal_fields_max_size_get(unit, field_id, &field_max_size),
                              "Cannot get the field max size");
        ...
        if (encode_type == DBAL_VALUE_FIELD_ENCODE_PREFIX)
        {
            ...
            {
                utilex_bitstream_bit_flip_range(encoded_field_mask, 0, nof_bits - 1);
            }
        ...
        else
        {
                ...
                || (entry_handle->table_id == DBAL_TABLE_ECOL_HEADER_SIZE_CAM))
            {
                ...
                utilex_bitstream_bit_flip_range(field_mask_copy, 0, nof_bits - 1);
                ret_val =
                    utilex_bitstream_set_any_field(field_mask_copy, offset, nof_bits,
                                                   &(entry_handle->phy_entry.k_mask[0]));
            }
            else
            {
    ...
    {
        if ((entry_handle->error_info.error_exists == _SHR_E_UNAVAIL) && (entry_handle->error_info.field_id == field_id)
        ...
        case DBAL_FIELD_PRINT_TYPE_STRUCT:
        {
            ...
            for (ii = nof_struct_fields - 1; ii >= 0; ii--)
            {
                SHR_IF_ERR_EXIT(dbal_fields_struct_field_decode_masked
                                (unit, field_id, field_info->struct_field_info[ii].struct_field_id,
    ...
    uint32 *result)
{
        ...
        if (formula->nof_vars == 1)
        {
            ...
            else if (formula->field_id != DBAL_FIELD_EMPTY)
            {
                ...
                else
                {
                    ...
                    /** at this stage field must have an index and known if it is a key or result! */
    ...
    for (ii = 0; ii < formula->nof_vars; ii++)
    {
        /** the 1st var can be used in 3 cases:
         *  1) if only one var and no action: calculate formula is
         *  actually the var itself
         *  2) if only one var and action is a function: we calculate
         *  a function with 1st var as parameter of the function
         *  3) if action is not a function: 1st var is used as left
         *  operand of binary operations: +, -, *, /  */
        if (ii == 0)
        {
        ...
        /**
         * calculate: operand operator operand operator operand
         * ex: vars[0] + vars[1] - vars[2]
         * orig_entry_handle  vars[0] * vars[1] / vars[2]
         */
        else
        {
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    /** appID is part of the key, but i doesn't being set by the user, for LPM need it to calculate the prefix length */
...
shr_error_e
dbal_dynamic_updates_pre_compiled(
    int unit,
    dbal_dynamic_xml_add_f dbal_dynamic_xml_add)
{
    xml_node top_node;
    ...
    top_node = dbx_xml_top_get("Npl_Bridge_Router_Basic_mdb_definition.xml", "AppDbCatalog", 0);
    SHR_IF_ERR_EXIT(dbal_dynamic_xml_add(unit, top_node));

    top_node = dbx_xml_top_get("Npl_Bridge_Router_Basic_arr_formats_enum_encoding.xml", "FieldTypesDbCatalog", 0);
    SHR_IF_ERR_EXIT(dbal_dynamic_xml_add(unit, top_node));

    top_node =
        dbx_xml_top_get("Npl_Bridge_Router_Basic_egress_ctx_id_field_types_definition.xml", "FieldTypesDbCatalog", 0);
    SHR_IF_ERR_EXIT(dbal_dynamic_xml_add(unit, top_node));

    top_node =
        dbx_xml_top_get("Npl_Bridge_Router_Basic_ingress_ctx_id_field_types_definition.xml", "FieldTypesDbCatalog", 0);
    SHR_IF_ERR_EXIT(dbal_dynamic_xml_add(unit, top_node));

    top_node = dbx_xml_top_get("Npl_Bridge_Router_Basic_profile_field_types.xml", "FieldTypesDbCatalog", 0);
    SHR_IF_ERR_EXIT(dbal_dynamic_xml_add(unit, top_node));
...
shr_error_e
dbal_field_arr_prefix_calculate(
    int unit,
    uint8 *arr_prefix_size,
    uint32 *arr_prefix_value,
    int arr_prefix_table_entry_width,
    int arr_prefix_table_entry_value,
    uint32 max_field_size,
    int field_size_in_table)
{
    SHR_FUNC_INIT_VARS(unit);

    /** arr prefix size the difference between the field type size and the field in table */
    *arr_prefix_size = (max_field_size - field_size_in_table);

    /** arr prefix value is the msbs of the arr prefix table entries.
     *  nof msbs to take is: arr_prefix_size */
    *arr_prefix_value = (arr_prefix_table_entry_value >> (arr_prefix_table_entry_width - *arr_prefix_size));
