...
#define MDB_LPM_ADS_DEPTH_IN_MACRO_A_UNITS(unit,macro_a_units)   (dnx_data_mdb.dh.nof_rows_in_macro_a_cluster_get(unit)*macro_a_units)
    ...
    int *macro_index)
{
    ...
    if (global_macro_index == DNX_DATA_MAX_MDB_DH_NOF_MACROES_TYPE_A + DNX_DATA_MAX_MDB_DH_NOF_MACROES_TYPE_B)
    {
        LOG_ERROR(BSL_LOG_MODULE,
    ...
    if (rv == KBP_OK)
    {
        if (global_macro_index < dnx_data_mdb.dh.nof_macroes_type_A_get(unit))
        {
            *macro_type = MDB_MACRO_A;
            *macro_index = global_macro_index;
        }
        else
        {
            *macro_type = MDB_MACRO_B;
            *macro_index = global_macro_index - dnx_data_mdb.dh.nof_macroes_type_A_get(unit);
        }
    ...
    soc_mem_t * mem_hitbit_b)
{
    ...
    if (rv == KBP_OK)
    {
        ...
        dbal_enum_value_field_mdb_physical_table_e assoc_mdb_physical_table_id_low, assoc_mdb_physical_table_id_high;
    ...
    uint8 *bytes)
{
    ...
    if ((blk_id == MDB_LPM_SECOND_ADS_FIRST_BLK_ID) || (blk_id == MDB_LPM_SECOND_ADS_LAST_BLK_ID))
    {
        ...
        rv = mdb_direct_table_entry_add(unit, physical_tables, 0 /* app_id */ , &data_entry, 0);
        if (rv != _SHR_E_NONE)
        {
            LOG_ERROR(BSL_LOG_MODULE,
    ...
    else if ((blk_id >= MDB_LPM_SECOND_BB_FIRST_BLK_ID) && (blk_id <= MDB_LPM_SECOND_BB_LAST_BLK_ID))
    {
        ...
        {
            rv = dnx_sbusdma_desc_add_mem(unit, mem, bucket_index, blk, row_in_cluster, row_data);
        }
        else
        {
    ...
    uint8 *bytes)
{
    ...
    if (mem != INVALIDm)
    {
        ...
        if (mem == KAPS_RPB_TCAM_CPU_COMMANDm)
        {
            ...
            soc_mem_field_set(unit, KAPS_RPB_TCAM_CPU_COMMANDm, &(mem_array[MDB_LPM_RPB_MAX_UINT32_WIDTH]),
                              RPB_TCAM_CPU_COMMAND_INST_Af, &mem_field);
            soc_mem_field_set(unit, KAPS_RPB_TCAM_CPU_COMMANDm, &(mem_array[MDB_LPM_RPB_MAX_UINT32_WIDTH]),
                              RPB_TCAM_CPU_COMMAND_INST_Bf, &mem_field);
        ...
        for (core = 0; core < dnx_data_device.general.nof_cores_get(unit); core++)
        {
            for (row_iter = nof_rows - 1; row_iter >= 0; row_iter--)
            {
                if (dnx_sbusdma_desc_is_enabled(unit, SBUSDMA_DESC_MODULE_ENABLE_MDB_LPM) == TRUE)
                {
                    rv = dnx_sbusdma_desc_add_mem(unit, mem, array_index, KAPS_BLOCK(unit, core), offset + row_iter,
                                                  &(mem_array[row_iter * MDB_LPM_RPB_MAX_UINT32_WIDTH]));
                }
                else
                {
                    rv = soc_mem_array_write(unit, mem, array_index, KAPS_BLOCK(unit, core), offset + row_iter,
                                             &(mem_array[row_iter * MDB_LPM_RPB_MAX_UINT32_WIDTH]));
                }
                if (rv != _SHR_E_NONE)
                {
    ...
    uint8_t * bytes)
{
    ...
    if ((nbytes != MDB_LPM_BB_NBYTES) && (nbytes != MDB_LPM_RPB_NBYTES) && (nbytes != 2 * MDB_LPM_RPB_NBYTES)
        && (nbytes != MDB_LPM_ADS_NBYTES) && (nbytes != MDB_LPM_REG_NBYTES))
    {
    ...
    int small_kaps)
{
    ...
    LOG_INFO(BSL_LOG_MODULE, (BSL_META_U(unit, " LBUCKET:0x%x"), field));
    ...
    dbal_enum_value_field_mdb_physical_table_e big_kaps_bb)
{
    ...
    for (cluster_idx = 0; cluster_idx < nof_clusters; cluster_idx++)
    {
        ...
        for (offset = cluster_info.start_address; offset < cluster_info.end_address; offset++)
        {
            ...
            SHR_IF_ERR_EXIT(mdb_direct_table_entry_get(unit, dbal_physical_table_id, 0 /* app_id */ , &data_entry, 0));
            if (utilex_bitstream_have_one_in_range
