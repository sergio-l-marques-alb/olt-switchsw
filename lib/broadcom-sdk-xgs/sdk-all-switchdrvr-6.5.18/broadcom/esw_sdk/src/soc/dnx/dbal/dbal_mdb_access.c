...
/** for the old implentation of the dbal MDB interface this define should be set to 1 when MDB will support the new
 *  interface (updating multiple phy DBs), this define should be changed (actually the old implementation should be
 *  deleted) */
#define DBAL_MDB_OLD_IMPLEMENTATION                     1
/**
 * \brief
    ...
    uint8 *is_eedb_ll)
{
    ...
    SHR_IF_ERR_EXIT(dnx_lif_mngr_outlif_sw_info_get
                    (unit, phy_entry->key[0], NULL, NULL, &outlif_phase, NULL, is_eedb_ll));
    SHR_IF_ERR_EXIT(dnx_algo_local_outlif_phase_enum_to_logical_phase_num(unit, outlif_phase, &logical_phase));
    *physical_table = logical_phase + DBAL_PHYSICAL_TABLE_EEDB_1;
    ...
    if (phy_db_id < DBAL_NOF_PHYSICAL_TABLES)
    {
        if (app_db_id < DBAL_NOF_APP_IDS_PER_PHYSICAL_TABLE)
        {
            for (ii = 0; ii < num_of_tables; ii++)
            {
                ...
                SHR_IF_ERR_EXIT(dbal_table_is_compatible_image(unit, ii, &is_compatible_image));
...
uint8
dbal_logical_table_is_out_lif_or_rif_allocator_eedb(
    CONST dbal_logical_table_t * table)
{
    if (dbal_logical_table_is_out_lif_allocator_eedb(table) || dbal_logical_table_is_out_rif_allocator(table))
    {
        return TRUE;
    }
    else
    ...
    uint32 *is_mact)
{
    ...
    if ((dbal_physical_table == DBAL_PHYSICAL_TABLE_LEM) && ((app_id == mact_app_id) || (app_id == mact_ivl_app_id)))
    {
        *is_mact = TRUE;
    }
    else
    ...
    uint8 resolve_from_sw)
{
    ...
    if ((dbal_logical_table_is_out_lif_or_rif_allocator_eedb(entry_handle->table))
        || (dbal_physical_table_is_in_lif(entry_handle->table->physical_db_id[0])))
    {
    ...
    uint8 is_eedb_ll)
{
     ...
     */
    {
        ...
        /** getting the entry form the MDB, the entry is received aligned to the max_payload_size. (this way it is
    ...
    dbal_entry_handle_t * entry_handle)
{
...

    {
        DBAL_ITERATE_OVER_CORES(entry_handle, core_id)
        {
            ...
            physical_table = (dbal_logical_table_is_out_rif_allocator(table)
                              || dbal_logical_table_is_out_lif_allocator_eedb(table)) ? eedb_physical_table :
                table->physical_db_id[core_id];
 ...
 * physical_db_ids, only first element relevant for now see DBAL_MDB_OLD_IMPLEMENTATION
 */
static shr_error_e
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    if (DBAL_MDB_OLD_IMPLEMENTATION)
    {
        int core_id;
        DBAL_ITERATE_OVER_CORES(entry_handle, core_id)
        {
            ...
            physical_tables[0] = (dbal_logical_table_is_out_rif_allocator(table)
                                  || dbal_logical_table_is_out_lif_allocator_eedb(table)) ? eedb_physical_table :
                table->physical_db_id[core_id];
            SHR_IF_ERR_EXIT(dbal_physical_entry_print
                            (unit, entry_handle, physical_tables[0], phy_entry, app_id, TRUE, "delete from"));
    ...
    else
    {
        ...
        DBAL_ITERATE_OVER_CORES(entry_handle, core_id)
        {
            ...
            physical_tables[core_id] = (dbal_logical_table_is_out_rif_allocator(table)
                                        || dbal_logical_table_is_out_lif_allocator_eedb(table)) ? eedb_physical_table :
                table->physical_db_id[core_id];
    ...
    uint8 is_eedb_ll)
{
    ...
    SHR_IF_ERR_EXIT(mdb_db_infos.mdb_em_init_done.get(unit, &mdb_em_init_done));
    if (!mdb_em_init_done)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "It is illegal to add MDB entries at this stage, as MDB EM Init. is not done\n");
    }
    ...
    SHR_IF_ERR_EXIT(PhysicalTable->table_actions->entry_add(unit, physical_db_ids, app_id, phy_entry, entry_flags[0]));
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    if (DBAL_MDB_OLD_IMPLEMENTATION)
    {
        ...
        DBAL_ITERATE_OVER_CORES(entry_handle, core_id)
        {
            ...
            physical_tables[0] = (dbal_logical_table_is_out_rif_allocator(table)
                                  || dbal_physical_table_is_eedb(table->physical_db_id[0])) ? eedb_physical_table :
                table->physical_db_id[core_id];
            SHR_IF_ERR_EXIT(dbal_physical_entry_print
                            (unit, entry_handle, physical_tables[0], phy_entry, app_id, FALSE, "add to"));
