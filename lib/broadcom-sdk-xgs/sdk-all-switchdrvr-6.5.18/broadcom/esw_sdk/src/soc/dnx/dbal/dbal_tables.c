    ...
    int unit)
{
    ...
    {
#ifndef IN_CODE_SUBSTITUTE
        char file_path[RHNAME_MAX_SIZE] = { 0 };
        char *db_dir;
        uint8 dir_override = TRUE;

        /**Get db path from soc property or env variable. If doesn't exists - take default*/
        ...
        /**Load dynamic tables from xml on non-standard images*/
        SHR_IF_ERR_EXIT(dbal_dynamic_updates_dispatcher
                        (unit, file_path, dnx_dynamic_xml_tables_update, "AppDbCatalog"));
#else /* IN_CODE_SUBSTITUTE */
        SHR_IF_ERR_EXIT(dbal_dynamic_updates_pre_compiled(unit, dnx_dynamic_xml_tables_update));
...
/**
 * \brief
 * validate that groups has been initialized and is not empty
 */
static shr_error_e
    ...
    void *access_info)
{
    ...
    /*
     * Use state rollback journal to attempt to undo the changes to the table.
     * Note that no change to SW state should appear before this journal log.
     * Only relevant to dynamic tables.
     * Assumes all changes to static tables happen during init or device lock.
     */
    if (table_id >= DBAL_NOF_TABLES)
    {
        SHR_IF_ERR_EXIT(dnxc_generic_state_journal_log_dbal_table_info_change(unit, table_id));
    }
    ...
    switch (access_method)
    {
        ...
        case DBAL_ACCESS_METHOD_KBP:
            ...
            {
                sw_state_table_index = table_id - DBAL_NOF_TABLES;
                SHR_IF_ERR_EXIT(DBAL_DYNAMIC_TBL_IN_SW_STATE.access_info.memwrite(unit, sw_state_table_index,
                                                                                  (uint8 *) (table->kbp_handles), 0,
                                                                                  sizeof(kbp_db_handles_t)));
    ...
    int res_type_hw_value)
{
     ...
     * (which is not necessarily a problematic scenario).
     */
...
#if defined(INCLUDE_KBP)
            SHR_IF_ERR_EXIT(DBAL_DYNAMIC_TBL_IN_SW_STATE.
                            access_info.memread(unit, sw_state_table_index, table->kbp_handles, 0,
                                                sizeof(kbp_db_handles_t)));
#endif
            break;
...
/*
 * Used to validate and set dbal table key fields
 */
static shr_error_e
...
/*
 * Used to validate and set dbal new result type fields
 */
static shr_error_e
...
/*
 * Used to validate and set dbal table result fields
 */
static shr_error_e
...
/*
 * Create a new dbal table (internal)
 */
static shr_error_e
    ...
    dbal_tables_e * table_id)
{
    ...
    /**In case of warm boot - use already set table id*/
    if (sw_state_is_warm_boot(unit))
    {
...
/*
 * Add dynamic table mdb properties
 */
static shr_error_e
    ...
    xml_node appdb_to_phy_mapping_node)
{
    ...
    /** update that the table is a specific table for this image */
    ...
    if (table_entry->access_method == DBAL_ACCESS_METHOD_MDB)
    {
        ...
        if (table_entry->table_type == DBAL_TABLE_TYPE_TCAM)
        {
            dbx_xml_child_get_content_str(appdb_to_phy_mapping_node, "Stage", str, RHNAME_MAX_SIZE);
            SHR_IF_ERR_EXIT(dbal_stage_string_to_id(unit, str, &table_entry->dbal_stage));
        }
...
/*
 * Add tables straight from the xml file
 */
shr_error_e
    ...
    xml_node top_node)
{
    ...
    /**Top node serves as the xml's top node, must exist*/
    ...
    /**Iterate on all the tables in the xml*/
    RHDATA_ITERATOR(appdb_node, top_node, "AppDB")
    {
        ...
        {
            /**Invalid table, either Result or MultipleResults nodes must be set, along with Key node*/
            SHR_ERR_EXIT(_SHR_E_RESOURCE, "Couldn't find fields and/or keys nodes");
        }

        nof_keys = 0;
        /** Iterate over the keys and gather the keys info */
        RHDATA_ITERATOR(cur_node, keys_node, "Field")
        {
            ...
            {
                /**Field not found - Use field name as type*/
            ...
            {   /**Field type not found - use UINT field type as default if nothing else was found*/
            ...
            if (nof_bits == 0)
            {
                /**Size field not found in node, take the default size from the field type*/
        ...
        /**Add the parsed table to the tables db*/
    ...
    SHR_IF_ERR_EXIT(dbal_tables_table_create_internal
                    (unit, access_method, table_type, core_mode, nof_key_fields, keys_info, &nof_result_fields,
                     &internal_results_info[0][0], 1,
                                                              /**nof_result_types*/
                     NULL,                                 /**result_types_names*/
                     table_name, FALSE,
                                                           /**is_xml_dynamic_loading*/
                     table_id));
    ...
    int *result_type_index)
{
    ...
    /*
     * Set new result type fields in table
     */
    SHR_IF_ERR_EXIT(dbal_tables_table_create_result_type_set_internal
...
#ifdef DNXC_ROLLBACK_JOURNAL_IS_ENABLED
    /*
     * use state rollback journal to attempt delete added result type
     */
    SHR_IF_ERR_EXIT(dnxc_generic_state_journal_log_result_type_add(unit, table_id, new_res_type_idx));
#endif /* DNXC_ROLLBACK_JOURNAL_IS_ENABLED */
    ...
    dbal_tables_e table_id)
{
    ...
    /*
     * assume that the ETM_PP_DESCRIPTOR_EXPANSION table has one static result type
     */
...
#ifdef DNXC_ROLLBACK_JOURNAL_IS_ENABLED
    /*
     * use state rollback journal to attempt to re-create deleted result type
     */
    SHR_IF_ERR_EXIT(dnxc_generic_state_journal_log_result_type_delete(unit, table_id));
#endif /* DNXC_ROLLBACK_JOURNAL_IS_ENABLED */
    ...
    uint32 mode)
{
    ...
    if (!sw_state_is_warm_boot(unit))
    {
        if (mode)
        {
            SHR_BITSET(table->indications_bm, DBAL_TABLE_IND_IS_TABLE_DIRTY);
        }
        else
        {
            SHR_BITCLR(table->indications_bm, DBAL_TABLE_IND_IS_TABLE_DIRTY);
        }
        SHR_IF_ERR_EXIT(DBAL_SW_STATE_TBL_PROP.indications_bm.set(unit, table_id, table->indications_bm[0]));
    }
    ...
    uint8 is_add_operation)
{
    ...
    /** for direct tables, no need to count the number of entries. for KBP TCAM direct we still want to do it since this
     *  is only indication that the table is ACL table and not realy direct table. */
    ...
    if (table->access_method == DBAL_ACCESS_METHOD_MDB)
    {
        ...
        for (jj = 0; jj < table->nof_physical_tables; jj++)
        {
            ...
            {
                SHR_IF_ERR_EXIT(DBAL_SW_STATE_MDB_PHY_TBL.nof_entries.set(unit, table->physical_db_id[jj], 0));
            }
...
/** see also macro DBAL_TABLE_IS_NONE_DIRECT for internal use */
uint8
...
/** merge entries supported means that the access support setting of partial of the entry. */
uint8
    ...
    uint8 *is_ranged)
{
    ...
    /** Note that the values given to result_type_idx doesn't matter since is_key=TRUE*/
...
shr_error_e
dbal_table_is_compatible_image(
    int unit,
    dbal_tables_e table_id,
    uint8 *is_compatible_image)
{
    ...
    SHR_IF_ERR_EXIT(DBAL_SW_STATE_TBL_PROP.table_status.get(unit, table_id, &table_status));
    *is_compatible_image = (table_status != DBAL_TABLE_INCOMPATIBLE_IMAGE);
