    ...
    dbal_flush_shadow_info_t * rules_info)
{
    ...
    int flush_wait_count = 8;

    SHR_FUNC_INIT_VARS(unit);

    SHR_IF_ERR_EXIT(dbal_flush_in_work(unit, &flush_in_work));

    /** Waiting 8 milisecond to make sure the flush is not in work */
    ...
    int unit)
{
    ...
    while (age_machine_status != 0)
    {
        ...
        age_machine_status = soc_reg64_field32_get(unit, lem_em_status_reg, data, ITEM_36_37f);
