...
#define SOC_DNX_STAGE0_SDM_FRAC_DIV_MODE    2
    ...
    soc_port_t port)
{
    ...
    SHR_IF_ERR_EXIT(soc_dnx_synce_pll_set(unit, synce_index, dummy, dummy, dummy, dummy, TRUE, FALSE));
    ...
    int *divider)
{
    ...
    int port, fsrd_block, fabric_link;
...
static int
soc_dnx_fabric_synce_div_set_verfiy(
    int unit,
    int synce_index,
    int divider)
{

    int synce_enabled, port, fsrd_block, synce_enable_to_check;
    ...
    if (synce_enabled)
    {
        SHR_IF_ERR_EXIT(soc_dnx_fabric_synce_link_get(unit, synce_enable_to_check, &port_to_check));
        SHR_IF_ERR_EXIT(dnx_algo_port_fabric_link_get(unit, port_to_check, &fabric_link));
        fsrd_block_to_check = fabric_link / SOC_DNX_NOF_LINKS_IN_FSRD;
        SHR_IF_ERR_EXIT(soc_dnx_fabric_synce_div_get(unit, synce_enable_to_check, &divider_to_check));
    ...
    int divider)
{
    ...
    int port, fabric_link, fsrd_block;
    ...
    soc_port_t port)
{
     ...
     */
    clk_blk_id = first_phy / dnx_data_nif.eth.nof_lanes_in_cdu_get(unit);
    phy_lane = first_phy % dnx_data_nif.eth.nof_lanes_in_cdu_get(unit);
    swapped_lane = lane2serdes[first_phy].rx_id % dnx_data_nif.eth.nof_lanes_in_cdu_get(unit);
    ...
    {
        chain_id = dnx_data_synce.general.nmg_chain_map_get(unit, clk_blk_id)->chain_id;
    ...
    soc_dnxc_init_serdes_ref_clock_t ref_clock_out)
{
     ...
     */
    SHR_IF_ERR_EXIT(soc_dnx_synce_pll_set(unit, synce_index, ndiv, mdiv, pdiv, fref, FALSE, TRUE));
    ...
    soc_port_t * port)
{
    uint32 entry_handle_id;
    int is_end;
    uint32 synce_clk_blk_id, clock_chain = 0, synce_idx_tmp;
    uint32 clock_lane = 0, inner_clock_lane = 0;
    int phy_lane;
    int lanes_per_pm = dnx_data_nif.eth.nof_lanes_in_cdu_get(unit);
 ...
 */
static int
soc_dnx_synce_tscbh_sdm_config_get(
    int unit,
    soc_port_t port,
    uint32 divider,
 ...
 */
static int
soc_dnx_synce_tscbh_fabric_sdm_config_get(
    int unit,
    soc_port_t port,
    uint32 divider,
    portmod_port_synce_clk_ctrl_t * config)
 ...
 */
static int
soc_dnx_synce_tscf_sdm_config_get(
    int unit,
    soc_port_t port,
    uint32 divider,
    portmod_port_synce_clk_ctrl_t * config)
{
    ...
    ref_clk = dnx_data_pll.pll3.nif_pll_get(unit, phy_access.access.pll_idx)->out_freq;
    if (ref_clk == DNX_SERDES_REF_CLOCK_BYPASS)
    {
        ref_clk = dnx_data_pll.pll3.nif_pll_get(unit, phy_access.access.pll_idx)->in_freq;
    }
    ...
    portmod_port_synce_clk_ctrl_t * config)
{
    ...
    switch (pm_type)
    {
        case portmodDispatchTypePm8x50:
            SHR_IF_ERR_EXIT(soc_dnx_synce_tscbh_sdm_config_get(unit, port, divider, config));
            break;

        case portmodDispatchTypePm8x50_fabric:
            SHR_IF_ERR_EXIT(soc_dnx_synce_tscbh_fabric_sdm_config_get(unit, port, divider, config));
            break;

        case portmodDispatchTypePm4x25:
            SHR_IF_ERR_EXIT(soc_dnx_synce_tscf_sdm_config_get(unit, port, divider, config));
            break;
    ...
    config->stg0_mode = SOC_DNX_STAGE0_SDM_FRAC_DIV_MODE;
 ...
 */
static int
soc_dnx_nif_ethu_synce_ref_clk_sel_set(
    int unit,
    uint32 synce_index,
    soc_port_t port)
{
     ...
     */
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_CLOCK_LANE_SEL, INST_SINGLE, swapped_lane);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_VALID_LANE_SEL, INST_SINGLE, phy_lane);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LINK_STATUS_LANE_SEL, INST_SINGLE, swapped_lane);
...
/**
 * \brief
 *   Set the NIF SyncE reference clock source link in SDM mode
 * \param [in] unit -
 *   The unit number.
 * \param [in] synce_index -
 *   SyncE index.
 * \param [in] port -
 *   Source link for NIF SyncE
 * \return
 *   See \ref shr_error_e
 * \remark
 *   None.
    ...
    soc_port_t port)
{
    ...
    /*
     * Specify NIF ethu reference clock
     */
    SHR_IF_ERR_EXIT(soc_dnx_nif_ethu_synce_ref_clk_sel_set(unit, synce_index, port));
    /*
     * Specify NIF reference clock divider 
     */
    SHR_IF_ERR_EXIT(soc_dnx_nif_synce_ref_clk_nif_div_set(unit, core_id, synce_index, port));
    ...
    if (dnx_data_pll.synce_pll.feature_get(unit, dnx_data_pll_synce_pll_present))
    {
        SHR_IF_ERR_EXIT(soc_dnx_synce_pll_set(unit, synce_index, dummy, dummy, dummy, dummy, TRUE, TRUE));
    }
    ...
    soc_port_t * port)
{
    ...
    soc_dnxc_lane_map_db_map_t serdes2lane[DNX_DATA_MAX_NIF_PHYS_NOF_PHYS];
    int map_size = dnx_data_nif.phys.nof_phys_get(unit);
    int lanes_per_cdu = dnx_data_nif.eth.nof_lanes_in_cdu_get(unit);
    int lanes_per_clu = dnx_data_nif.eth.nof_lanes_in_clu_get(unit);
    int cdu_nof = dnx_data_nif.eth.cdu_nof_get(unit);
    ...
    while (!is_end)
    {
        ...
        else
        {
            ...
            if (synce_idx_tmp == synce_index)
            {
                ...
                first_lane_in_ethu = synce_clk_blk_id < cdu_nof ? synce_clk_blk_id * lanes_per_cdu :
                    cdu_nof * lanes_per_cdu + (synce_clk_blk_id - cdu_nof) * lanes_per_clu;
                clock_lane = first_lane_in_ethu + inner_clock_lane;
                phy_lane = serdes2lane[clock_lane].rx_id;
    ...
    uint32 is_nif_synce)
{
    ...
    if (is_nif_synce)
    {
        ...
        SHR_IF_ERR_EXIT(soc_dnx_nif_synce_link_get(unit, synce_index, &port));
    }
    else
    {
        SHR_RANGE_VERIFY(divider, dnx_data_synce.general.fabric_div_min_get(unit),
    ...
    SHR_IF_ERR_EXIT(soc_dnx_synce_sdm_config_get(unit, port, divider, &synce_clk_config));
    ...
    soc_port_t port)
{
             ...
             DNX_ALGO_PORT_TYPE_IS_FLEXE_PHY(unit, port_info))
    {
        ...
        {
            SHR_IF_ERR_EXIT(soc_dnx_nif_synce_link_set(unit, synce_index, port));
        }
    }
    else
    {
    ...
    int divider)
{
    ...
    if (dnx_data_synce.general.feature_get(unit, dnx_data_synce_general_synce_no_sdm_mode))
    {
        if (is_nif_synce)
        {
            SHR_RANGE_VERIFY(divider, dnx_data_synce.general.nif_div_min_get(unit),
                             dnx_data_synce.general.nif_div_max_get(unit), _SHR_E_PARAM,
                             "SyncE divider is out of bound!\n");
            SHR_IF_ERR_EXIT(soc_dnx_nif_synce_clk_div_set
                            (unit, synce_index, (soc_dnxc_init_serdes_ref_clock_t) divider));
        }
        else
        {
            SHR_IF_ERR_EXIT(soc_dnx_fabric_synce_div_set_verfiy(unit, synce_index, divider));
    ...
    {
        SHR_IF_ERR_EXIT(soc_dnx_synce_clk_div_set(unit, synce_index, divider, is_nif_synce));
    }
exit:
    SHR_FUNC_EXIT;
}
    ...
    int *divider)
{
    ...
    if (dnx_data_synce.general.feature_get(unit, dnx_data_synce_general_synce_no_sdm_mode))
    {
        if (is_nif_synce)
        {
            SHR_IF_ERR_EXIT(soc_dnx_nif_synce_clk_div_get
                            (unit, synce_index, (soc_dnxc_init_serdes_ref_clock_t *) divider));
        }
        else
        {
