...
_cmicx_sbusdma_ch_get(int unit, int *cmc, int *ch)
{
    ...
    uint32 ifa_leap_num_sbusdma = soc_property_get(unit,
                                                spn_IFA_LEAP_NUM_SBUS_DMA, 2);
    if ((SOC_IS_TRIDENT3(unit)) && (ifa_leap_num_sbusdma > 2)) {
        /* CMC_0 2 SBUS DMA Channels are used by IFA LEAP Eapp on M0 */
