...
/* PAXB_0_PAXB_HOTSWAP_STATr FSM_STATEf values */
#define HOTSWAP_FSM_STATE_IDLE 0
#define HOTSWAP_FSM_STATE_ACTIVE 1
#define HOTSWAP_FSM_STATE_WAIT_IDLE 2
#define HOTSWAP_FSM_STATE_DUMMYRESP 3
 ...
 * Parameters:
 *      unit - SOC unit #
 *      flags - CMICX_PCIE_HOTSWAP_HANDLING_FLAG_* flags
 * Returns:
 *      SOC_E_NONE - success
 *      SOC_E_XXXX - error code
 * Notes:
 ...
 */
int
cmicx_pcie_hotswap_handling(int unit, uint32 flags)
{
    int rv = SOC_E_NONE;
    ...
    if (!soc_feature(unit, soc_feature_cmicx) ||
        (soc_cm_get_bus_type(unit) & SOC_PCI_DEV_TYPE) == 0 ||
    ...
    if (hot_swap_cause != 0 || fsm_state == HOTSWAP_FSM_STATE_DUMMYRESP) {
        if ((flags & CMICX_PCIE_HOTSWAP_HANDLING_FLAG_DONOT_DISABLE_INTERRUPTS) == 0) {
        ...
        while (fsm_state != HOTSWAP_FSM_STATE_IDLE && fsm_state != HOTSWAP_FSM_STATE_DUMMYRESP && !soc_timeout_check(&to)) {
            SOC_IF_ERROR_RETURN(READ_PAXB_0_PAXB_HOTSWAP_STATr(unit, &rval));
        ...
        if ((flags & CMICX_PCIE_HOTSWAP_HANDLING_FLAG_DONOT_ABORT) == 0) {
...
soc_cmicx_handle_hotswap_and_dma_abort(int unit)
{
    ...
    if (!SOC_WARM_BOOT(unit)) {
        /* always abort current DMA operations on all CMCs */
        if ((rv = soc_cmicx_dma_abort(unit, 0)) != SOC_E_NONE) {
            LOG_ERROR(BSL_LS_SOC_DMA, (BSL_META_U(unit, "Failed aborting DMA operations\n")));
        }
        rv2 = cmicx_pcie_hotswap_handling(unit, CMICX_PCIE_HOTSWAP_HANDLING_FLAG_DONOT_ABORT |
          CMICX_PCIE_HOTSWAP_HANDLING_FLAG_DONOT_DISABLE_INTERRUPTS);
        if (rv2 != SOC_E_NONE && rv == SOC_E_NONE) {
            rv = rv2;
        }
    } else {
        rv = cmicx_pcie_hotswap_handling(unit, CMICX_PCIE_HOTSWAP_HANDLING_FLAG_DONOT_DISABLE_INTERRUPTS);
    }
    return rv;
}
