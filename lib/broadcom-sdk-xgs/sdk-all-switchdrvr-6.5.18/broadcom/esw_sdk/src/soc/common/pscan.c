                                 ...
                                 uint16 *r_len, uint32 *r_data)
{
    ...
    if (soc_feature(unit, soc_feature_cmicx)) {
        ...
        send64.s.mclass = MOS_MSG_CLASS_PSCAN;
        send64.s.subclass = s_subclass;
        send64.s.len = soc_htons(s_len);

        if (MOS_MSG_DMA_MSG(s_subclass) || MOS_MSG_DMA_MSG(r_subclass)) {
            send64.s.data = soc_htonl(PTR_TO_INT(paddr));
        } else {
            send64.s.data = soc_htonl(s_data);
        }

        rv = soc_cmic_uc_msg64_send_receive(unit, pscan_info->uc_num,
                                          &send64, &reply64,
                                          pscan_uc_msg_timeout_usecs);

        /* Check reply class, subclass */
        if ((rv != SOC_E_NONE) ||
            (reply64.s.mclass != MOS_MSG_CLASS_PSCAN) ||
            (reply64.s.subclass != r_subclass)) {
            return SOC_E_INTERNAL;
        }

        if (r_data != NULL) {
            *r_data = soc_ntohl(reply64.s.data);
        }
        if (r_len != NULL) {
            *r_len = soc_ntohs(reply64.s.len);
        }
    } else
#endif
    {
...
#if defined(BCM_CMICX_SUPPORT)
        if (soc_feature(unit, soc_feature_cmicx)) {
            rv = soc_cmic_uc_msg64_receive(pscan_info->unit, pscan_info->uc_num,
                                           MOS_MSG_CLASS_PSCAN_EVENT, &event_msg64,
                                           sal_sem_FOREVER);
            ...
            port = soc_ntohs(event_msg64.s.len);
            code = soc_ntohl(event_msg64.s.data);
        } else
#endif
        {
