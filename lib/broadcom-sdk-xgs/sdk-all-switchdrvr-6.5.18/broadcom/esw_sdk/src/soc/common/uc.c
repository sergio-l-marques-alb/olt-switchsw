...
soc_uc_mhost_reset(int unit, int uC)
{
    ...
    defined(BCM_MAVERICK2_SUPPORT) || defined(BCM_DNX_SUPPORT)
    uint32 strap_status;
#endif
    ...
    soc_reg_field_set(unit, MHOST_0_CR5_RST_CTRLr, &rst_control,
                      PRESET_DBG_Nf, 1);
#if defined(BCM_TRIDENT3_SUPPORT) || defined(BCM_TOMAHAWK3_SUPPORT) || defined(BCM_MAVERICK2_SUPPORT) || defined(BCM_DNX_SUPPORT)
    if ((uC == 0) &&
        (SOC_IS_TRIDENT3(unit) || SOC_IS_TOMAHAWK3(unit) || SOC_IS_MAVERICK2(unit) || SOC_IS_DNX(unit))) {
...
#if defined(BCM_TOMAHAWK_SUPPORT) || defined(BCM_TOMAHAWK2_SUPPORT) || defined(BCM_TOMAHAWK3_SUPPORT) || \
    defined(BCM_TRIDENT3_SUPPORT) || defined(BCM_MAVERICK2_SUPPORT)
...
int soc_uc_ping(int unit, int uC, sal_usecs_t timeout)
{
    ...
    if (soc_feature(unit, soc_feature_cmicx)) {
        send64.s.mclass = MOS_MSG_CLASS_SYSTEM;
        send64.s.subclass = MOS_MSG_SUBCLASS_SYSTEM_PING;
        send64.s.len = 0;
        send64.s.data = 0;
        return soc_cmic_uc_msg64_send(unit, uC, &send64, timeout);
    } else
#endif
    {
 ...
 */
{
...
#if defined(BCM_CMICX_SUPPORT)
    mos_msg64_data_t send64, reply64;
#endif
    int len = 256;
    char *version_buffer;
    uint16 dev_id;
    ...
    if (version_buffer) {
        ...
        if (soc_feature(unit, soc_feature_cmicx)) {
            send64.s.mclass = MOS_MSG_CLASS_SYSTEM;
            send64.s.subclass = MOS_MSG_SUBCLASS_SYSTEM_VERSION;
            send64.s.len = soc_htons(len);
            send64.s.data = soc_htonl(PTR_TO_INT(buf_paddr));

            rc = soc_cmic_uc_msg64_send(unit, uC, &send64, 5 * 1000 * 1000);
            if (rc != SOC_E_NONE) {
                soc_cm_sfree(unit, version_buffer);
                return NULL;
            }
            rc = soc_cmic_uc_msg64_receive(unit, uC, MOS_MSG_CLASS_VERSION,
                                         &reply64, 5 * 1000 * 1000);
        } else
#endif
        {
            ...
            send.s.data = soc_htonl(buf_paddr);
...
char *soc_uc_firmware_thread_info(int unit, int uC)
{
    ...
    if (thread_info_buffer) {
        ...
        if (soc_feature(unit, soc_feature_cmicx)) {
            send64.s.mclass = MOS_MSG_CLASS_SYSTEM;
            send64.s.subclass = MOS_MSG_SUBCLASS_SYSTEM_THREAD_INFO;
            send64.s.len = soc_htons(len);
            send64.s.data = soc_htonl(PTR_TO_INT(buf_paddr));

            rc = soc_cmic_uc_msg64_send(unit, uC, &send64, _SYSTEM_UC_MSG_TIMEOUT);
            if (rc != SOC_E_NONE) {
                soc_cm_sfree(unit, thread_info_buffer);
                return NULL;
            }
            rc = soc_cmic_uc_msg64_receive(unit, uC, MOS_MSG_CLASS_THREAD_INFO,
                                         &reply64, _SYSTEM_UC_MSG_TIMEOUT);
        } else
#endif
        {
            send.s.mclass = MOS_MSG_CLASS_SYSTEM;
...
#if defined(BCM_CMICX_SUPPORT)
    if (soc_feature(unit, soc_feature_cmicx)) {
        sal_memset(&send64, 0, sizeof(send64));
        send64.s.mclass = MOS_MSG_CLASS_SYSTEM;
        send64.s.subclass = MOS_MSG_SUBCLASS_SYSTEM_CMD_CONSOLE_LOG;
        send64.s.len = soc_htons(on);
        rc = soc_cmic_uc_msg64_send(unit, uC, &send64, _SYSTEM_UC_MSG_TIMEOUT);
    } else
#endif
    {
...
int soc_uc_stats_reset(int unit, int uC)
{
    ...
    if (soc_feature(unit, soc_feature_cmicx)) {
        sal_memset(&send64, 0, sizeof(send64));
        send64.s.mclass = MOS_MSG_CLASS_SYSTEM;
        send64.s.subclass = MOS_MSG_SUBCLASS_SYSTEM_CMD_STATS_RESET;
        rc = soc_cmic_uc_msg64_send(unit, uC, &send64, _SYSTEM_UC_MSG_TIMEOUT);
    } else
#endif
    {
