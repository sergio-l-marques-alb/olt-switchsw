/*
 * $Id: shmoo_g6phy16.c,v 1.1.2.21 Broadcom SDK $
...
_g6phy16_run_bist(int unit, int drc_ndx, g6phy16_shmoo_container_t *scPtr, g6phy16_bist_info_t *biPtr, g6phy16_training_bist_info_t *gbiPtr, g6phy16_shmoo_error_array_t *seaPtr)
{
    ...
    switch(scPtr->shmooType)
    {
        ...
        case SHMOO_G6PHY16_EDC_START_FISH:
        {
            ...
            if(gbe.bist_global_sanity_error_occurred)
            {
                /** if a sanity error happened, override results with all errors. */
                (*seaPtr)[0] = 0xf;
                LOG_VERBOSE(BSL_LS_SOC_DDR, (BSL_META_U(unit, "sanity error in this spot\n")));
            }
            else
            {
                /** else take results from array */
                (*seaPtr)[0] = gbe.bist_edc_err_bitmap[0];
            }
            break;
        }
        case SHMOO_G6PHY16_RD_DESKEW:
...
_shmoo_g6phy16_rd_extended(int unit, int drc_ndx, g6phy16_shmoo_container_t *scPtr)
{
    ...
    {
        case 1:
            scPtr->sizeX = scPtr->endUI[3] + 1; /* + ((data >> 4) & 0x1FF) + 20; */
            scPtr->calibStart = 0;
            break;
        case 2:
            scPtr->sizeX = scPtr->endUI[3] + 1; /* + ((data >> 4) & 0x1FF) + 20; */
...
/*A10*/ LOG_INFO(BSL_LS_SOC_DDR, (BSL_META_U(unit, "A10. ZQ calibration\n")));
        ...
        _SOC_IF_ERR_EXIT( soc_phy_g6ddrc16_reg_field_set(unit, SOC_PHY_GDDR6_REG_DQ_BYTE0_DATA_IO_CONFIGr, &data, SOC_PHY_GDDR6_DEMf, 0x2));
