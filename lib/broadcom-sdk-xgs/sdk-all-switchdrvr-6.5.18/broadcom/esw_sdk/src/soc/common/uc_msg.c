...
static int soc_cmic_uc_msg32_send_internal(int unit, int uC, mos_msg_data_t *msg,
               sal_usecs_t timeout);

static int soc_cmic_uc_msg32_receive_internal(int unit, int uC, uint8 mclass,
               mos_msg_data_t *msg, int timeout, int is_system);

#ifdef BCM_CMICX_SUPPORT
static int soc_cmic_uc_msg64_receive_internal(int unit, int uC, uint8 mclass,
               mos_msg64_data_t *msg64, int timeout, int is_system);

static int soc_cmic_uc_msg64_send_internal(int unit, int uC, mos_msg64_data_t *msg64,
               sal_usecs_t timeout);
#endif
...
int _soc_cmic_uc_msg32_process_status(int unit, int uC) {
...
#ifdef BCM_CMICX_SUPPORT
int _soc_cmic_uc_msg64_process_status(int unit, int uC) {
    soc_control_t       *soc = SOC_CONTROL(unit);

    uint32 msg_base = _soc_cmic_uc_msg_base(unit);
    uint32 area_in = MSG64_AREA_ARM_TO_HOST(msg_base, uC);
    uint32 area_out = MSG64_AREA_HOST_TO_ARM(msg_base, uC);
    ...
    int         out_status = 0;        /* Set when new out status */
    int         i;
    mos_msg64_ll_node_t *msg64_node = NULL;
    ...
    for (i = MOS_MSG_STATUS_SENT_INDEX(soc->uc_msg_prev_status_in[uC]);
        ...
        msg64.words[0] = soc_cmic_mcs_read(unit,CMICX_DATA_WORD0_BASE(area_in) +
                            MOS_MSG64_DATA_SIZE * i);
        msg64.words[1] = soc_cmic_mcs_read(unit,CMICX_DATA_WORD1_BASE(area_in) +
                            MOS_MSG64_DATA_SIZE * i);

        LOG_VERBOSE(BSL_LS_SOC_COMMON,
                    (BSL_META_U(unit,
                                "UC%d: msg recv 0x%08x 0x%08x\n"),
                     uC, msg64.words[0], msg64.words[1]));

        /* swap to host endian */
        msg64.words[0] = soc_ntohl(msg64.words[0]);
        msg64.words[1] = soc_ntohl(msg64.words[1]);

        LOG_VERBOSE(BSL_LS_SOC_COMMON,
                    (BSL_META_U(unit,
                                "UC%d: msg recv mclass 0x%02x sclass 0x%02x len 0x%04x data 0x%08x\n"),
                     uC, msg64.s.mclass, msg64.s.subclass,
                     msg64.s.len, msg64.s.data));
        if (msg64.s.mclass > MAX_MOS_MSG_CLASS) {
            MOS_MSG_ACK_BIT_CLEAR(soc->uc_msg_prev_status_out[uC], i);
        } else if ((msg64.s.mclass == MOS_MSG_CLASS_SYSTEM) &&
                   (msg64.s.subclass == MOS_MSG_SUBCLASS_SYSTEM_PING)) {
            /* Special case - PING just gets an ACK */
            MOS_MSG_ACK_BIT_SET(soc->uc_msg_prev_status_out[uC], i);
        } else {
            if (((soc->uc_msg_rcvd_ll[uC][msg64.s.mclass]).ll_count) < MOS_MSG_EVENT_QUEUE_SIZE) {
                /* Enqueue the message in respective LL */
                msg64_node = (mos_msg64_ll_node_t *)sal_alloc(sizeof(mos_msg64_ll_node_t), "UC Msg Node");
                if (msg64_node != NULL) {
                    MOS_MSG_ACK_BIT_SET(soc->uc_msg_prev_status_out[uC], i);
                    msg64_node->msg_data.words[0] = msg64.words[0];
                    msg64_node->msg_data.words[1] = msg64.words[1];
                    ll_insert_tail(&(soc->uc_msg_rcvd_ll[uC][msg64.s.mclass]), (ll_element_t *)msg64_node);

                    /* Kick the waiter */
                    sal_sem_give(soc->uc_msg_rcv_sems[uC][msg64.s.mclass]);
        ...
        LOG_VERBOSE(BSL_LS_SOC_COMMON,
                    (BSL_META_U(unit,
                                "UC%d: Ack slot %d 0x%08x 0x%08x\n"),
                     uC, i, msg64.words[0], msg64.words[1]));
        out_status = 1;
    }
    ...
    while (1) {
        while (1) {
            ...
            if (soc_feature(unit, soc_feature_cmicx)) {
                rv = soc_cmic_uc_msg64_receive_internal(unit, uC, MOS_MSG_CLASS_SYSTEM,
                        &rcv64, sal_sem_FOREVER, 1);
                rsubclass = rcv64.s.subclass;
                rlen = rcv64.s.len;
                rdata = rcv64.s.data;
            } else
#endif
            {
                rv = soc_cmic_uc_msg32_receive_internal(unit, uC, MOS_MSG_CLASS_SYSTEM,
        ...
        if (rsubclass == MOS_MSG_SUBCLASS_SYSTEM_LOG) {
            ...
            if (rlen == 0) {
                ...
                if (soc_feature(unit, soc_feature_cmicx)) {
                    ...
                    send64.s.subclass = MOS_MSG_SUBCLASS_SYSTEM_DMA_ENDIAN;
                    send64.s.len = 0;
                    send64.s.data = soc_htonl(PTR_TO_INT(endian_buf_paddr));
                    send_rv = soc_cmic_uc_msg64_send_internal(unit, uC, &send64,
                                                   soc->uc_msg_send_timeout);
                } else
#endif
                {
                    ...
                    send.s.len = 0;
                    send.s.data = soc_htonl(endian_buf_paddr);
                    send_rv = soc_cmic_uc_msg32_send_internal(unit, uC, &send,
                                                   soc->uc_msg_send_timeout);
                }
            }
...
#ifdef BCM_CMICX_SUPPORT
    mos_msg64_ll_node_t *msg64_node;
#endif
...
#ifdef BCM_CMICX_SUPPORT
    if (soc_feature(unit, soc_feature_cmicx)) {
        area_in = MSG64_AREA_ARM_TO_HOST(msg_base, uC);
        area_out = MSG64_AREA_HOST_TO_ARM(msg_base, uC);
    } else
#endif
    {
...
#ifdef BCM_CMICX_SUPPORT
            if (soc_feature(unit, soc_feature_cmicx)) {
                if (_soc_cmic_uc_msg64_process_status(unit, uC)) {
...
#endif
            {
                if (_soc_cmic_uc_msg32_process_status(unit, uC)) {
                    break;              /* Received reset */
                }
                if (sal_sem_take(soc->swIntr[uC_intr], sal_sem_FOREVER)) {
                    goto thread_stop;
...
#ifdef BCM_CMICX_SUPPORT
                if (soc_feature(unit, soc_feature_cmicx)) {
                    msg64_node = (mos_msg64_ll_node_t *)ll_remove_head(&(soc->uc_msg_rcvd_ll[uC][i]));
                    if (msg64_node != NULL){
                        sal_free(msg64_node);
                    }
                } else
...
static int soc_cmic_uc_msg64_send_internal(int unit, int uC, mos_msg64_data_t *msg64,
    ...
    SOC_CMIC_UCDBG_LOG_ADD((unit, _bcmCmicUcDbgLogHostUcMsg,
        "uC: %d msg->s.mclass: %d msg->s.subclass: %d msg->s.len :%d msg->s.addr: 0x%x\n",
        uC, msg64->s.mclass, msg64->s.subclass, msg64->s.len, msg64->s.data));
        ...
        if (sal_mutex_take(soc->uc_msg_control, soc->uc_msg_ctl_timeout) == 0) {
            ...
            LOG_VERBOSE(BSL_LS_SOC_COMMON,
                        (BSL_META_U(unit,
                         ...
                         uC, msg64->s.mclass, msg64->s.subclass,
                         msg64->s.len, msg64->s.data));
            omsg64.words[0] = soc_htonl(msg64->words[0]);
            omsg64.words[1] = soc_htonl(msg64->words[1]);

            /* Update the outbound area that contains the new message */
            LOG_VERBOSE(BSL_LS_SOC_COMMON,
                        (BSL_META_U(unit,
                                    "UC%d: send slot %d 0x%08x 0x%08x\n"),
                         uC, index, omsg64.words[0], omsg64.words[1]));
            area_out = MSG64_AREA_HOST_TO_ARM(msg_base, uC);
            soc_cmic_mcs_write(unit, CMICX_DATA_WORD0_BASE(area_out) +
                               MOS_MSG64_DATA_SIZE * index, 
                               omsg64.words[0]);
            soc_cmic_mcs_write(unit, CMICX_DATA_WORD1_BASE(area_out) +
                               MOS_MSG64_DATA_SIZE *index,
                               omsg64.words[1]);
...
int soc_cmic_uc_msg64_send(int unit, int uC, mos_msg64_data_t *msg,
                         sal_usecs_t timeout) {
    return soc_cmic_uc_msg64_send_internal(unit, uC, msg, timeout);
}
#endif
...
#ifdef BCM_CMICX_SUPPORT
    if (soc_feature(unit, soc_feature_cmicx)) {
        return soc_cmic_uc_msg64_send_internal(unit, uC, (mos_msg64_data_t *)msg, timeout);
    } else
#endif
    {
        return soc_cmic_uc_msg32_send_internal(unit, uC, msg, timeout);
    }
}
                            ...
                            mos_msg64_data_t *msg64, int timeout, int is_system)
{
    ...
    /* Remove the message from Receive LL head*/
    msg64_node = (mos_msg64_ll_node_t *)ll_remove_head(&(soc->uc_msg_rcvd_ll[uC][mclass]));
    if (msg64_node != NULL){
        msg64->words[0] = msg64_node->msg_data.words[0];
        msg64->words[1] = msg64_node->msg_data.words[1];
        sal_free(msg64_node);
    } else {
        LOG_VERBOSE(BSL_LS_SOC_COMMON,
                    (BSL_META_U(unit,
    ...
    if ((rc == SOC_E_NONE) && (msg64->s.mclass != mclass)) {
        LOG_VERBOSE(BSL_LS_SOC_COMMON,
                    (BSL_META_U(unit,
                                "UC%d Reply from wrong mclass\n"), uC));
        SOC_CMIC_UCDBG_LOG_ADD((unit, _bcmCmicUcDbgLogHostUcMsg,
...
int soc_cmic_uc_msg64_receive(int unit, int uC, uint8 mclass,
                            mos_msg64_data_t *msg, int timeout)
{
    return soc_cmic_uc_msg64_receive_internal(unit, uC, mclass, msg, timeout, 0);
}
#endif
...
#ifdef BCM_CMICX_SUPPORT
    if (soc_feature(unit, soc_feature_cmicx)) {
        return soc_cmic_uc_msg64_receive_internal(unit, uC, mclass,
                                    (mos_msg64_data_t *)msg, timeout, 0);
    } else
#endif
    {
        return soc_cmic_uc_msg32_receive_internal(unit, uC, mclass, msg, timeout, 0);
    }
}
...
#ifdef BCM_CMICX_SUPPORT
        if (soc_feature(unit, soc_feature_cmicx)) {
            msg64_node = (mos_msg64_ll_node_t *)ll_remove_head(&(soc->uc_msg_rcvd_ll[uC][msg_class]));
            if (msg64_node != NULL) {
                sal_free(msg64_node);
            }
        } else
 ...
 *
 */
int soc_cmic_uc_msg32_send_receive(int unit, int uC, mos_msg_data_t *send,
    ...
    rc = soc_cmic_uc_msg32_send_internal(unit, uC, send, timeout);
    ...
    if (timeout <= time_elapsed) {
        ...
        rc = soc_cmic_uc_msg32_receive_internal(unit, uC, send->s.mclass,
                                                reply, timeout, 0);
    }
...
#ifdef BCM_CMICX_SUPPORT
int soc_cmic_uc_msg64_send_receive(int unit, int uC, mos_msg64_data_t *send64,
                                 mos_msg64_data_t *reply64, sal_usecs_t timeout) {
    int               rc;

    sal_usecs_t start = sal_time_usecs();
    sal_usecs_t now, time_elapsed;
    
    rc = soc_cmic_uc_msg64_send_internal(unit, uC, send64, timeout);
    if (rc != SOC_E_NONE) {               /* send failed */
        return rc;
    }
    ...
    if (timeout <= time_elapsed) {
        ...
        timeout -= time_elapsed;
        rc = soc_cmic_uc_msg64_receive_internal(unit, uC, send64->s.mclass,
                                                reply64, timeout, 0);
    }
...
#ifdef BCM_CMICX_SUPPORT
    if (soc_feature(unit, soc_feature_cmicx)) {
        return soc_cmic_uc_msg64_send_receive(unit, uC,
                                (mos_msg64_data_t *)send,
                                (mos_msg64_data_t *)reply, timeout);
     } else
#endif
    {
        return soc_cmic_uc_msg32_send_receive(unit, uC, send, reply, timeout);
    }
}
                          ...
                          soc_cmic_uc_appl_cb_t *shutdown_cb, void *cb_data)
{
    ...
    if (soc_feature(unit, soc_feature_cmicx)) {
        send64.s.mclass = MOS_MSG_CLASS_SYSTEM;
        send64.s.subclass = MOS_MSG_SUBCLASS_SYSTEM_APPL_INIT;
        send64.s.len = soc_htons(msg_class);
        send64.s.data = soc_htonl(version_info);
        rc = soc_cmic_uc_msg64_send_internal(unit, uC, &send64, timeout);
        if (rc != SOC_E_NONE) {
            LOG_VERBOSE(BSL_LS_SOC_COMMON,
        ...
        rc = soc_cmic_uc_msg64_receive_internal(unit, uC, msg_class, &rcv64, timeout, 0);
        rlen = rcv64.s.len;
        rdata = rcv64.s.data;
    } else
#endif
    {
        ...
        rc = soc_cmic_uc_msg32_receive_internal(unit, uC, msg_class, &rcv, timeout, 0);
        rlen = rcv.s.len;
        rdata = rcv.s.data;
    }
...
#ifdef BCM_CMICX_SUPPORT
    if (soc_feature(unit, soc_feature_cmicx))
        msg_end = MSG64_AREA_END(_soc_cmic_uc_msg_base(unit));
    else
#endif
