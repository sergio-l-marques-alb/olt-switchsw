...
#define XGP_POINT_TO_XFI(unit, pc)\
    do {\
            SOC_IF_ERROR_RETURN\
                (WRITE_PHY8481_TOPLVL1_REG(unit, pc, 0x8003, 1));\
            SOC_IF_ERROR_RETURN\
...
#define RESTORE_XGP(unit, pc)\
    do {\
            SOC_IF_ERROR_RETURN\
                (WRITE_PHY8481_TOPLVL1_REG(unit, pc, 0x4110, 0x1));\
            SOC_IF_ERROR_RETURN\
                (WRITE_PHY8481_TOPLVL1_REG(unit, pc,0x8003, 0));\
    } while(0)

#define WRITE_ORCA_PMAD_REG(unit, pc, addr, val)\
    do {\
            SOC_IF_ERROR_RETURN\
                (WRITE_PHY8481_TOPLVL1_REG(unit, pc, 0x4110, 0x2004));\
            SOC_IF_ERROR_RETURN\
...
err_code_t eagle_phy_1_pmd_mwr_reg(uint16_t addr, uint16_t mask, uint8_t lsb, uint16_t val) {
    ...
    if (otmp != tmp) {
          WRITE_ORCA_PMAD_REG(pc_int->unit, pc_int, addr, tmp);
    }
...
#endif
err_code_t eagle_phy_1_pmd_rdt_reg(uint16_t address, uint16_t *data){
    if(pc_int) {
        uint16_t val=0;
        XGP_POINT_TO_XFI(pc_int->unit, pc_int);
        SOC_IF_ERROR_RETURN
...
err_code_t eagle_phy_1_pmd_wr_reg(uint16_t address, uint16_t val){
    ...
    if(pc_int) {
        WRITE_ORCA_PMAD_REG(pc_int->unit, pc_int, address, val);    
    }
 
    return(0);
}

err_code_t eagle_phy_1_pmd_rdt_reg_addr32(uint32_t address, uint16_t *data){
