...
control_handler_rsfec_symb_err_counter_get(int unit, phymod_phy_access_t *phy, uint32 param, uint32 *value)
{
    ...
    for (i = 0; i < PM_MAX_LANES; i++) {
        if (phy->access.lane_mask & (1 << i)) {
            if (i < symbol_error_count.actual_count) {
                count += error_count[i];
            } else {
                return SOC_E_PARAM;
            }
...
STATIC int
control_handler_base_r_fec_intr_enable_set(int unit, phymod_phy_access_t *phy, uint32 value, uint32 param2)
{
    SOC_IF_ERROR_RETURN(phymod_phy_ecc_intr_enable_set(phy, phymodIntrEccBaseRFEC, value));
    return SOC_E_NONE;
}

STATIC int
control_handler_base_r_fec_intr_enable_get(int unit, phymod_phy_access_t *phy,  uint32 param2, uint32 *value)
{
    SOC_IF_ERROR_RETURN(phymod_phy_ecc_intr_enable_get(phy, phymodIntrEccBaseRFEC, value));
    return SOC_E_NONE;
}
