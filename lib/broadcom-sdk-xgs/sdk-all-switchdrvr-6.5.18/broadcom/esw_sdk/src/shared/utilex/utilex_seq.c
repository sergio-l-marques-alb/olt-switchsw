    ...
    int *is_last_done)
{
    ...
    for (step_index = 0; step_index < nof_steps_in_list; ++step_index)
    {
        ...
        if (seq->multithread_en && (total_flags & UTILEX_SEQ_STEP_F_JOIN_ALL_THREADS))
        {
            ...
            for (thread_id = 0; thread_id < seq->active_threads_nof; thread_id++)
            {
                sal_thread_join(seq->active_threads[thread_id], NULL);
                seq->active_threads[thread_id] = 0;
            }
            seq->active_threads_nof = 0;
        }
        ...
        else
        {
            ...
            if (is_enabled == 1)
            {
                ...
                if (seq->multithread_en &&
                    ...
                    current_step->forward != NULL && current_step->step_sub_array != NULL)
                {
                ...
                if (current_step->forward != NULL)
                {
                    ...
                    if (seq->multithread_en && total_flags & UTILEX_SEQ_STEP_F_RUN_ON_THREAD)
                    {
                        utilex_seq_parallel_execute_input_t thread_info;
                        thread_info.unit = unit;
                        thread_info.step_name = current_step->step_name;
                        thread_info.forward = current_step->forward;
                        ...
                        seq->active_threads[seq->active_threads_nof] =
                            sal_thread_create("utilex step", SAL_THREAD_STKSZ, 1,
                                              utilex_seq_run_execute_forward_in_parallel, &thread_info);
                ...
                if (current_step->step_sub_array != NULL)
                {
                    ...
                    if (seq->multithread_en && total_flags & UTILEX_SEQ_STEP_F_RUN_ON_THREAD)
                    {
                        utilex_seq_parallel_run_input_t thread_info;

                        thread_info.unit = unit;
                        thread_info.step_name = current_step->step_name;
                        thread_info.seq = seq;
                        thread_info.depth = depth + 1;
                        thread_info.step_list = current_step->step_sub_array;
                        thread_info.time_overall = time_overall;
                        thread_info.swstate_overall = swstate_overall;
                        thread_info.memory_overall = memory_overall;
                        thread_info.is_first_done = is_first_done;
                        thread_info.is_last_done = is_last_done;

                        /*
                         * all sub steps on parallel thread should be silent 
                         */
                        utilex_seq_silent_all_sub_steps(unit, thread_info.step_list);

                        seq->active_threads[seq->active_threads_nof] =
                            sal_thread_create("utilex step", SAL_THREAD_STKSZ, 1,
                                              utilex_seq_run_step_list_forward_in_parallel, &thread_info);
    ...
    utilex_seq_time_test_t * test_info)
{
    ...
    uint32 total_flags = 0;
    int step_index, nof_steps_in_list;
    int is_found, is_skip, dynamic_flags = 0;
    int first_run = 1; /** required in order to run first step in the list if it was provided explicitly */

    SHR_FUNC_INIT_VARS(unit);

    sal_memset(test_info, 0, sizeof(utilex_seq_time_test_t));
    ...
    while ((previous_step_id != original_last_step_id) || first_run)
    {
        ...
        if (is_skip == FALSE)
        {
            ...
            /** Check if the time for the current step is exceeding the user defined threshold */
            if ((time_stamp_end - time_stamp_start) > time_thresh)
            {
                BSL_LOG(BSL_LOG_MODULE, ("#############################################\n"));
                BSL_LOG(BSL_LOG_MODULE, ("Error!!! Step %s Failed!\n", (current_step->step_name)));
                BSL_LOG(BSL_LOG_MODULE, ("#############################################\n"));
                ...
                {
                    test_info->fail_step_name[test_info->fail_count] = current_step->step_name;
                    test_info->fail_time[test_info->fail_count] = (time_stamp_end - time_stamp_start);
                    test_info->fail_step_id[test_info->fail_count] = current_step->step_id;
                    test_info->fail_step_thresh[test_info->fail_count] = time_thresh;
                    (test_info->fail_count)++;
                }
            }
            else
