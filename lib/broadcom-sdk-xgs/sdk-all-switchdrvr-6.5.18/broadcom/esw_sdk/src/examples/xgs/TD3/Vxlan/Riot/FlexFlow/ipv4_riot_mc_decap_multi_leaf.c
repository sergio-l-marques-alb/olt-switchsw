 ...
 *
 * Usage    : BCM.0> cint ipv4_riot_mc_decap_multi_leaf.c
 *
 * config   : config_td3_flex.bcm
 *
 * Log file : ipv4_riot_mc_decap_multi_leaf_log.txt
 *
 * Test Topology :
 *
 *                                  +--------------------+
 ...
 *   SMAC 00::01:01                 |                    |
 *            +---+          VLAN-10|                    |
 *  Host1(acc)|   +<----------------+                    |   VLAN-11    Underlay-NextHop
 *            +---+  (L3)egress_port|                    | ingress_port       +---+
 *                                  |     Trident-3      +<-------------------+   |
 *            +---+          VLAN-12|                    | 00::bb:bb          +---+
 *  Host2(acc)|   +<----------------+                    |
 *            +---+  (L2)egress_port|                    |
...
2. Access packet L2 copy :
0100 5e00 1001 0000 0000 0202 8100 000c
...
bcm_port_t             egress_port[num_egr_ports];
bcm_gport_t            ingress_phy_gport;
bcm_gport_t            egress_phy_gport[num_egr_ports];
...
test_setup(int unit)
{
    ...
    for(iter=0; iter<num_egr_ports; iter++){
...
config_vxlan_access(int unit, vxlan_cfg_t *vxlan_cfg,
                    vxlan_acc_cfg_t *vxlan_acc, int l3,
                    bcm_multicast_t acc_mc_group, bcm_gport_t *acc_flow_port)
{
    ...
    if(l3){
        ...
        rv = bcm_multicast_l2_encap_get(unit, acc_mc_group,
                                        vxlan_acc->phy_Gport,
                                        vxlan_acc->acc_vlan,
                                        &encap_id);
        if (BCM_FAILURE(rv)) {
...
config_vxlan_ipv4_vtep(int unit)
{
    ...
    vxlan_acc_cfg_t vxlan_acc_1 = {
        egress_port[0],         /*access physical port*/
    ...
    rv = config_ipmc_add(unit, vxlan_acc_1, acc_mc_group, vxlan_cfg.vrf);
    if (BCM_FAILURE(rv)) {
