...
/*
 * Feature  : VPWS Tunnel (L2 MPLS) Initiation and Termination scenarios with Double tag
 *
 * Usage    : BCM.0> cint vpws_double_tag.c
 *
 * config   : mpls_config.bcm
 *
 * Log file : vpws_double_tag_log.txt
 *
 * Test Topology :
 *
 *                  +------------------------------+
...
verify(int unit)
{
    ...
    printf("\na) Sending a customer packet with Double VLAN to access_port:%d\n", access_port);
    snprintf(str, 512, "tx 1 pbm=%d data=0x000000445566000000AABBCC9100000B8100000C08004500002A0000000040FF79D60000000000000000000102030405060708090A0B0C0D0E0F1011121314157B9BCEDD; sleep quiet 1", access_port);
    bshell(unit, str);
    bshell(unit, "show c");
    bshell(unit, "sleep 1");
    ...
    printf("\nb) Sending a MPLS packet with TWO Labels with Double tagged inner payload to network_port:%d\n", network_port);
    snprintf(str, 512, "tx 1 pbm=%d data=0x0000000000BB0000000000AA8100001688470022204000333140000000AABBCC0000004455669100000B8100000C08004500002E0000000040FF79D20000000000000000910000038902A0010446000000000000D36D61696434386279747B9BCEDD; sleep quiet 1", network_port);
    bshell(unit, str);
    bshell(unit, "show c");
    bshell(unit, "sleep 1");
...
config_vpws_double_tag(int unit)
{
    ...
    /* To match double tag on access port */
    BCM_IF_ERROR_RETURN(bcm_vlan_control_port_set(unit, customer_port, bcmVlanPortTranslateKeyFirst,
            bcmVlanTranslateKeyPortDouble));
    BCM_IF_ERROR_RETURN(bcm_port_tpid_set(unit, customer_port, 0x9100));
    BCM_IF_ERROR_RETURN(bcm_port_inner_tpid_set(unit, customer_port, 0x8100));
    ...
    /* VLAN Configuration */
    BCM_IF_ERROR_RETURN(bcm_vlan_create(unit, customer_ovid));
    BCM_PBMP_CLEAR(ubmp);
    BCM_PBMP_CLEAR(pbmp);
    BCM_PBMP_PORT_ADD(pbmp, customer_port);
    BCM_IF_ERROR_RETURN(bcm_vlan_port_add(unit, customer_ovid, pbmp, ubmp));
    ...
    mpls_c_port.criteria = BCM_MPLS_PORT_MATCH_PORT_VLAN_STACKED;
    mpls_c_port.match_vlan = customer_ovid;
    mpls_c_port.match_inner_vlan = customer_ivid;
 ...
 *  a) test setup
 *  b) actual configuration (Done in config_vpws_double_tag())
...
execute(void)
{
    ...
    print "~~~ #2) config_vpws_double_tag(): ** start **";
    if (BCM_FAILURE((rv = config_vpws_double_tag(unit)))) {
        printf("config_vpws_double_tag() failed.\n");
        return -1;
    }
    print "~~~ #2) config_vpws_double_tag(): ** end **";
