...
/*
 * Feature  : VPWS Tunnel (L2 MPLS) Initiation and Termination scenarios
 *
 * Usage    : BCM.0> cint vpws.c
 *
 * config   : mpls_config.bcm
 *
 * Log file : vpws_log.txt
 *
 * Test Topology :
 *
 *                  +------------------------------+
 ...
 *  a) test setup
 *  b) actual configuration (Done in config_vpws())
...
execute(void)
{
    ...
    print "~~~ #2) config_vpws(): ** start **";
    if (BCM_FAILURE((rv = config_vpws(unit)))) {
        printf("config_vpws() failed.\n");
        return -1;
    }
    print "~~~ #2) config_vpws(): ** end **";
