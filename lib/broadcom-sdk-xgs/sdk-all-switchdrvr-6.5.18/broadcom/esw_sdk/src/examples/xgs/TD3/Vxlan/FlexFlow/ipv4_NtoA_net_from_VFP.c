 ...
 *            with network SVP assignment from VFP rule
 *
 * Usage    : BCM.0> cint ipv4_NtoA_net_from_VFP.c
 *
 * config   : config_td3_flex.bcm
 *
 * Log file : ipv4_NtoA_net_from_VFP_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
...
verify(int unit)
{
    ...
    printf("\nSending known unicast packet with vnid 54545 to ingress_port network side:%d\n", ingress_port_2);
    snprintf(str, 1184, "tx 1 pbm=%d data=
...
vxlan_network_port_config(int unit, bcm_gport_t gport_net)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_port_control_set(unit, gport_net, bcmPortControlVxlanTunnelbasedVnId, 1));
    return BCM_E_NONE;
}
...
config_vxlan_network(int unit, vxlan_cfg_t *vxlan_cfg, vxlan_net_cfg_t *vxlan_net, bcm_gport_t *net_flow_port, bcm_flow_handle_t flow_handle)
{
    ...
    bcm_flow_match_config_t_init(&match_info);
    match_info.flow_handle    = flow_handle;
    match_info.criteria       = BCM_FLOW_MATCH_CRITERIA_SIP_VNID;
...
/*assign SVP based on VFP rule*/
bcm_error_t
config_vfp_entry(int unit, vxlan_cfg_t *vxlan_cfg, bcm_gport_t net_flow_port)
{
    bcm_field_qset_t qset;
    bcm_field_group_t group;
    bcm_field_entry_t entry;
    bcm_mac_t mask_mac = "ff:ff:ff:ff:ff:ff";
    bcm_ip_t  mask_ip = 0xffffffff;
    bcm_l4_port_t l4_port_mask = 0xFFFF;
    bcm_mac_t dstmac = "00:00:00:00:22:33";
    bcm_ip_t  src_ip = 0xC0A80303;
    bcm_ip_t  dst_ip = 0x14141401;
    bcm_l4_port_t l4_port = 0x2118;
    ...
    BCM_IF_ERROR_RETURN(bcm_field_qualify_DstMac(unit, entry, dstmac, mask_mac));
    BCM_IF_ERROR_RETURN(bcm_field_qualify_SrcIp(unit, entry, src_ip, mask_ip));
    BCM_IF_ERROR_RETURN(bcm_field_qualify_DstIp(unit, entry, dst_ip, mask_ip));
    BCM_IF_ERROR_RETURN(bcm_field_qualify_L4DstPort(unit, entry, l4_port, l4_port_mask));

    BCM_IF_ERROR_RETURN(bcm_field_action_add(unit, entry, bcmFieldActionIngressGportSet, net_flow_port, 0));
    BCM_IF_ERROR_RETURN(bcm_field_action_add(unit, entry, bcmFieldActionVpnSet, vxlan_cfg->vxlan_vpn, 0));
...
config_vxlan_ipv4_vtep(int unit)
{
    ...
    vxlan_net_cfg_t vxlan_net_2 = {
        ...
        0xC0A80202,  /*192.168.1.1 remote IP*/
        0x12345,
    ...
    rv = config_vxlan_network(unit, &vxlan_cfg_1, &vxlan_net_1, &net_flow_port_1, flow_handle);
    if (BCM_FAILURE(rv)) {
        printf("\nError in network config: %s.\n", bcm_errmsg(rv));
        return rv;
    }
    ...
    rv = config_vxlan_network(unit, &vxlan_cfg_2, &vxlan_net_2, &net_flow_port_2, flow_handle);
    if (BCM_FAILURE(rv)) {
        printf("\nError in network config: %s.\n", bcm_errmsg(rv));
        return rv;
    }
    ...
    /*create vfp rule to set the network SVP*/
    rv = config_vfp_entry(unit, &vxlan_cfg_2, net_flow_port_2);
    if (BCM_FAILURE(rv)) {
        printf("\nError in vfp setup: %s.\n", bcm_errmsg(rv));
        return rv;
    }
