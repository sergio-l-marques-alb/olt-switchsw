...
/*
 * Feature  : L3 MPLS LSR label Swap & Push label
 *
 * Usage    : BCM.0> cint lsr_push.c
 *
 * config   : mpls_config.bcm
 *
 * Log file : lsr_push_log.txt
 *
 * Test Topology :
 *
 *                  +------------------------------+
 ...
 *
 *   2) Step2 - Configuration (Done in config_lsr_push()):
 *   =========================================================
...
bcm_error_t
config_lsr_push(int unit)
{
    bcm_error_t rv;
    bcm_port_t  in_port = ingress_port, eg_port = egress_port;
    bcm_gport_t in_gport, eg_gport;
    bcm_vlan_t  in_vlan = 11, eg_vlan = 22;
    bcm_l3_intf_t       l3_intf;
    bcm_if_t    intf_id = 10;
    bcm_vrf_t   vrf = 0;
    ...
    uint8       ttl = 16;
    uint32      in_tunnel_label = 0x111;
    uint32      out_tunnel_label = 0x222;
    uint32      out_vc_label     = 0x444;
    ...
    /* mpls tunnel l2 add mac=$l2_mac vlan=$in_vlan */
    BCM_IF_ERROR_RETURN(bcm_l2_tunnel_add(unit, l2_mac, in_vlan));

    /* l3 intf add INtf=$intf_id mac=$local_mac vlan=$eg_vlan */
    ...
    l3_egress.port     =  eg_port;
    l3_egress.mpls_label = out_vc_label;  /* Swap label */
    BCM_IF_ERROR_RETURN(bcm_l3_egress_create(unit, 0, &l3_egress, &egr_obj));

    /* Label lookp and swapping */
 ...
 *  a) test setup
 *  b) actual configuration (Done in config_lsr_push())
...
execute(void)
{
    ...
    print "~~~ #2) config_lsr_push(): ** start **";
    if (BCM_FAILURE((rv = config_lsr_push(unit)))) {
        printf("config_lsr_push() failed.\n");
        return -1;
    }
    print "~~~ #2) config_lsr_push(): ** end **";
