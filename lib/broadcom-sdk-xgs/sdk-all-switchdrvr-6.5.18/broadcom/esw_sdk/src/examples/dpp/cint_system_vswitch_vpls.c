...
int system_mpls_port_mp_example(int unit1, int port1, int second_port1, int unit2, int port2){
    ...
    bcm_if_t tunnel_intf = is_device_or_above(unit1, JERICHO2) ? 0x16000 : 32768,
        ...
        unit2_egress_intf1 = (is_device_or_above(unit1, JERICHO2)) ? 0xCCCE: (is_device_or_above(unit1, JERICHO)) ? 0x4CCE : 1024,
    ...
    rv = fec_id_alignment_field_wa_add(unit1, second_port1, BCM_L3_ITF_VAL_GET(unit1_egress_intf2));
    if (rv != BCM_E_NONE)
    {
...
int system_protected_mpls_port_example(int unit1, int port1, int unit2, int port2,int port3,int is_mp){
    ...
    bcm_gport_t gport1_lif   = is_device_or_above(unit1, JERICHO2) ? 0x1002 : 17000;
