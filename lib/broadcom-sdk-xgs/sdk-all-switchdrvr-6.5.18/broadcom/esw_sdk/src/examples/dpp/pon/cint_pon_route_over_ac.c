/* $Id: cint_pon_route_over_ac.c,v 1.10 Broadcom SDK $
...
route_over_ac_s g_route_over_ac = {   /*               host MAC                 | host ipv4 address |  gport  |   tunnel ID | vlan_o  |  edit profile  |  fec |outLiF | encap_id  */  
                                        { { {0x00, 0x00, 0x11, 0x11, 0x11, 0x11},    0x11111111     ,    -1   ,     10   ,  100    ,        10      ,   0,   -1    ,    0},                                                                                                                                                                                                                                                                                                                    
                                          { {0x00, 0x00, 0x33, 0x33, 0x33, 0x33},    0x33333333     ,    -1   ,     -1   ,  30     ,        -1      ,   0,   -1    ,    0}},
...
int route_over_ac__start_run(int unit, route_over_ac_s *param)
{
    ...
    rv = is_arad_plus_only(unit, &yesno);
    if (rv != BCM_E_NONE) {
        printf("Error, is_arad_plus_only.");
        return rv;
    }

    if (yesno == 0) {
        printf("Error, supported on arad_plus device only.");
        return BCM_E_PARAM;
    }
