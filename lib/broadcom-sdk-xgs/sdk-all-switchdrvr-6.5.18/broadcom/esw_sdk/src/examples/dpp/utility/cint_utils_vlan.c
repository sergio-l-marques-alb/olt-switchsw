 ...
 *      BCM_E_*
 */
int vlan__vlan_port_vsi_create(int unit, int vsi, bcm_port_t port, bcm_vlan_t outer_vid, bcm_vlan_t inner_vid){
    bcm_vlan_port_t vlan_port;
...
vlan__open_vlan_per_ports(int unit, uint16 vid, bcm_gport_t *ports, uint8 *tagged, int nof_ports) {
    ...
    for(port_id = 0; port_id < nof_ports; ++port_id) {
        rv = bcm_vlan_gport_add(unit, vid, ports[port_id], tagged[port_id]);
        if (rv != BCM_E_NONE) {
...
/*****************************************************************************
* Function:  vlan__port_vsi_egress_and_igress__create
* Purpose:   Create port which is both ingress and egress and return the created GPORT
* Params:
* unit   (IN)   - Device Number
* vsi    (IN)   - to which VSI the port is connected
* port    (IN)  - system port
* gport_lif (OUT) - GPORT will hold GLOBAL LIF type o AC
* Return:    (int)
*******************************************************************************/
int vlan__port_vsi_egress_and_igress__create(int unit, bcm_vlan_t vsi,  bcm_port_t port,bcm_gport_t *gport_lif)
{
    int rv = BCM_E_NONE;
    bcm_vlan_port_t vlan_port;

    int vport_flags = 0; /*No flags */
    bcm_vlan_port_t_init(&vlan_port);
    
    vlan_port.criteria =  BCM_VLAN_PORT_MATCH_PORT;
    vlan_port.flags =  vport_flags;
    vlan_port.port = port;
    vlan_port.vsi = vsi;    
    ...
    if(gport_lif)
    {
        *gport_lif = vlan_port.vlan_port_id;
    }
...
int vlan__native_default_out_ac_allocate_and_set_dflt_action(int unit, int outer_vlan, int inner_vlan) {
    ...
    rv = vlan__native_default_out_ac_allocate_and_set(unit, outer_vlan, inner_vlan, &default_action); 
    if (rv != BCM_E_NONE) {
