/*
 * $Id: cint_oam_performance_measurement.c, Exp $
 $Copyright: (c) 2019 Broadcom.
 Broadcom Proprietary and Confidential. All rights reserved.$ File: cint_oam_performance_measurement.c Purpose: basic examples for OAM loss and delay measurement.
 */

/* Short group indication */
int is_short_maid = 1;
int is_jumbo_dm = 0;
bcm_oam_endpoint_t up_mep_id = 4096;
bcm_oam_endpoint_t down_mep_id = 4100;
    ...
    bcm_port_t port2)
{
    ...
    if (rv != BCM_E_NONE)
    {
        printf("Error no %d, in oam__my_cfm_mac_set.\n", rv);
        return rv;
    }
     ...
     */
    rv = set_counter_resource(unit, port2, counter_if_0, &lm_counter_base_id_0);
    if (rv != BCM_E_NONE)
    {
    ...
    rv = set_counter_resource(unit, port2, counter_if_1, &lm_counter_base_id_1);
    if (rv != BCM_E_NONE)
    {
        printf("Error no %d, in set_counter_resource.\n", rv);
        return rv;
    ...
    for (index = 0; index < 1024; index ++ )
    {
        ...
        if (rv != BCM_E_NONE)
        {
            printf("Error no %d, in dnx_oam_cross_connected_vlan_ports_create.\n", rv);
            return rv;
        }
         ...
         */
        rv = dnx_oam_accelerated_endpoint_create(unit, 0 + mepid_steps, gport2, /*group_info_short.id*/(index >511) ? group_info_short.id : group_info_long.id,
                                                 counter_if_1, lm_counter_base_id_1, acc_ingress_profile, port2, 0);

        if (rv != BCM_E_NONE)
        {
            printf("Error no %d, in dnx_oam_accelerated_endpoint_create.\n", rv);
            return rv;
        }
    }
    ...
    for (index = 0; index < 1024; index ++ )
    {
        ...
        if (rv != BCM_E_NONE)
        {
            printf("Error no %d, in bcm_oam_endpoint_destroy.\n", rv);
            return rv;
        }
    }
    ...
    for (index = 0; index < 256; index ++ )
    {
        ...
        if (rv != BCM_E_NONE)
        {
            printf("Error no %d, in dnx_oam_loss_add.\n", rv);
            return rv;
        }
    }
    ...
    bcm_port_t port2)
{
     ...
     */
    rv = dnx_oam_accelerated_endpoint_create(unit, up_mep_id, gport2, group_info.id,
     ...
     * Create accelerated Down-MEP
     */
    rv = dnx_oam_accelerated_endpoint_create(unit, down_mep_id, gport2, group_info.id,
                                             counter_if_1, lm_counter_base_id_1, acc_ingress_profile, port2, 0);
    if (rv != BCM_E_NONE)
    {
        printf("Error no %d, in dnx_oam_accelerated_endpoint_create.\n", rv);
    ...
    if(is_jumbo_dm)
    {
         ...
         * Add delay on Up-MEP
         */
        rv = dnx_oam_delay_add(unit, up_mep_id, 65552, BCM_OAM_ENDPOINT_CCM_PERIOD_100MS);
        if (rv != BCM_E_NONE)
        {
            printf("Error no %d, in dnx_oam_delay_add.\n", rv);
            return rv;
        }
        /*
         * Add delay on Down-MEP
         */
        rv = dnx_oam_delay_add(unit, down_mep_id, 65556, BCM_OAM_ENDPOINT_CCM_PERIOD_100MS);
        if (rv != BCM_E_NONE)
        {
            printf("Error no %d, in dnx_oam_loss_add.\n", rv);
            return rv;
    ...
    else 
    {
         ...
         * Add loss on Up-MEP
         */
        rv = dnx_oam_loss_add(unit, up_mep_id, 65552, BCM_OAM_ENDPOINT_CCM_PERIOD_10MS, 0, 1);
        if (rv != BCM_E_NONE)
        {
            printf("Error no %d, in dnx_oam_loss_add.\n", rv);
            return rv;
         ...
         * Add delay on Up-MEP
         */
        rv = dnx_oam_delay_add(unit, up_mep_id, 0, BCM_OAM_ENDPOINT_CCM_PERIOD_10MS);
        if (rv != BCM_E_NONE)
        {
            printf("Error no %d, in dnx_oam_loss_add.\n", rv);
            return rv;
         ...
         */
        rv = dnx_oam_loss_add(unit, down_mep_id, 65556, BCM_OAM_ENDPOINT_CCM_PERIOD_10MS, 1, 1);
        if (rv != BCM_E_NONE)
        {
            printf("Error no %d, in dnx_oam_loss_add.\n", rv);
            return rv;
    ...
    uint8 is_up)
{
    ...
    if (is_up)
    {
        up_mep_id = acc_endpoint.id;
    }
    else
    {
        down_mep_id = acc_endpoint.id;
    }
    ...
    uint8 use_extended_statistics)
{
    ...
    if (rv != BCM_E_NONE)
    {
        printf("Error no %d, in bcm_oam_loss_add.\n", rv);
        return rv;
    }
    ...
    int period)
{
    ...
    if (rv != BCM_E_NONE)
    {
        printf("Error no %d, in bcm_oam_delay_add.\n", rv);
        return rv;
    }
