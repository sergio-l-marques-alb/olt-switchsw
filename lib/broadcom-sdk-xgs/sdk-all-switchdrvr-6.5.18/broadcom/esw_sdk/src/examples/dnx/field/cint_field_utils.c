 ...
 * \param [in]  unit                - The unit.
 * \param [in]  flags               - The flags (with or without ID).
 * \param [in]  nof_quals           - Nof of qualifiers that we want to assign.
 * \param [in]  qual_types_p        - The qualifier types that we want to assign.
 * \param [in,out]  fg_id_p         - Pointer to FG ID.
 *                            as in - in case flag WITH_ID is set,
 *                                    will hold the FG ID
 *                           as out - in case flag WITH_ID is not set,
 *                                    will return the created FG ID.
 *
 * \return
 *   shr_error_e - Type of Error
 * \remark
 ...
 *   * None
 */
int cint_field_utils_ST_cfg(
    int unit,
    uint32 flags,
    int nof_quals,
    bcm_field_qualify_t * qual_types_p,
    bcm_field_group_t * fg_id_p)
{
    ...
    /**
     * Iterate over all qualifiers that we need to assign into the ST FG.
     */
    for (qual_iter = 0; qual_iter < nof_quals; qual_iter++)
    {
        fg_info.qual_types[qual_iter] = qual_types_p[qual_iter];
    }
    /**
     * Create the FG
     */
    rv = bcm_field_group_add(unit, flags, &fg_info, fg_id_p);
    if (rv != BCM_E_NONE)
    {
        printf("Error (%d), in bcm_field_group_add\n", rv);
        return rv;
    ...
    printf("State Table FG %d was created \n", *fg_id_p);

    return rv;
}

/*********************************************************************************************
 *                              <--  END of State Table utils                                *
 *********************************************************************************************/
