                                                ...
                                                bcm_field_entry_t *entry_handle)
{
    ...
    printf("TCAM Entry Id (0x%x) Fwd_layer (%d) fwd_plus_one (%d) is_in_range (%d) with Stat_Code (0x%x) \n",
                            *entry_handle, fwd_l,fwd_l_plus,is_in_range,gport_stat_samp);
...
int cint_sflow_field_main(int unit, bcm_field_context_t context_id)
{
    ...
    */
    ...
    rv = bcm_field_group_add(unit, BCM_FIELD_FLAG_MSB_RESULT_ALIGN, &fg_info, &cint_sflow_field_fg_id);
    if (rv != BCM_E_NONE)
    {
        printf("Error (%d), in bcm_field_group_add\n", rv);
        return rv;
    }
    printf(" FG Id (%d) was add to iPMF1 Stage\n",cint_sflow_field_fg_id);
    ...
    attach_info.key_info.qual_info[1].offset = 24;
    ...
    printf(" FG Id (%d) in iPMF1 attached to context (%d) \n", cint_sflow_field_fg_id, context_id);
