...
cint_sflow_utils_eventor_set(int unit, int eventor_id, int buffer_size, int nof_samples, int eventor_port, int sflow_out_port)
{
    ...
    /* in IPTCH2, In_PP_Port is 10 bits [9:0]*/
    ...
    */
    ...
    header_data[3] = 0x18 | sflow_port_high ; /* See destiantion encoding*/
    header_data[4] = sflow_port_mid;
    header_data[5] = sflow_port_low;

    /* UDP port for SFLOW = 6343 = 0x18C7 */
    header_data[48] = cint_sflow_basic_info.udp_src_port >> 8;
    header_data[49] = cint_sflow_basic_info.udp_src_port & 0xFF;
    header_data[50] = cint_sflow_basic_info.udp_dst_port >> 8;
    header_data[51] = cint_sflow_basic_info.udp_dst_port & 0xFF;
    /* 
      UDP length =
      UDP header (8 bytes) + payload data
      payload data = SFLOW Datagram header (7 words) + aggregated data
       */
    header_data[52] = (8 + (7 + buffer_size_thr)*4) >> 8;
    header_data[53] = (8 + (7 + buffer_size_thr)*4) & 0xFF;

    printf("cint_sflow_utils_eventor_set: UDP LENGTH = 0x%04X, udp_src_port = 0x%04X, udp_dst_port = 0x%04X \n", (8 + (7 + buffer_size_thr)*4), cint_sflow_basic_info.udp_src_port, cint_sflow_basic_info.udp_dst_port);
