...
int cint_field_IFA_datapath_intermediate_ingress_main(int unit)
{
    ...
    attach_info.key_info.qual_info[0].offset = 160;
...
*
*  Operation principle:
*      The traffic is filtered (IPv4 packets in this example) for IFA, and requested packets are snooped.
...
*
*  Packet Walk:
*      First cycle: Packets are filtered and requested packets are snooped (in iPMF1). Original copy is forwarded in
*      regular path. Tail edit profile is set to 3 (ePMF) for correct egress context selection. In egress IFA1 header
*      is encapsulated in the packet.
*      Then PTCH1 header is encapsulated on the packet carrying the original source port. The packet is then recycled
*      using a recycle port on the same core as the in_port.
*      Second cycle:
*      The packet is filtered using the probe header detection (in iPMF1). Source port from PTCH1 is used for forwarding.
*      INT is set (iPMF3) for correct egress context selection. In Egress IFA1 metadata is encapsulated on the packet
*      and the packet is forwarded to the same destination as the original packet
*
*  Configuration:
...
*             - on IPv4 use actions 1 and attach to FirstIFA contexts in egress
...
int cint_field_group_const_example_IFA_gen(int unit,int Outlif,int recycle_port)
{
    ...
    ent_info.entry_qual[1].mask[0] = 0;
