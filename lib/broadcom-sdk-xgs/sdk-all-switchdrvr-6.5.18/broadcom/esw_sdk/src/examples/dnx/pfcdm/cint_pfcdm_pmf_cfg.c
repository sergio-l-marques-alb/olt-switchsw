...
pfcdm_field_compare_stat_metata_set(int unit)
{
     ...
     */
    rv = bcm_field_entry_add(unit, 0, pfcdm_compare_fg_id, &entry_info, &entry_handle);
    if (rv != BCM_E_NONE)
    {
...
int pfcdef_field_ipmf3_pp_drop_entry_install(int unit, uint16_t hw_code, uint8_t sw_code )
{
    bcm_field_entry_t entry_handle;
    bcm_field_entry_info_t ent_info;
...
*/
    ...
    /* Entry #2, 3: PP Drop filter */
    pfcdef_field_ipmf3_pp_drop_entry_install(unit, bcmRxTrapForwardingIpv4Ttl0_hw_code, bcmRxTrapForwardingIpv4Ttl0_sw_drop_reason);

    pfcdef_field_ipmf3_pp_drop_entry_install(unit, bcmRxTrapUcLooseRpfFail_hw_code, bcmRxTrapUcLooseRpfFail_sw_drop_reason);

    pfcdef_field_ipmf3_pp_drop_entry_install(unit, bcmRxTrapUnknowDest_hw_code, bcmRxTrapUnknowDest_sw_drop_reason);
...
int pfcdm_ipv4_flow_cleanup(int unit)
{
    ...
    rv = bcm_field_entry_delete_all(unit);
    if (rv != BCM_E_NONE)
    {
        printf("Error (%d), in bcm_dnx_field_entry_delete_all\n", rv);
        return rv;
    }
