    ...
    uint32 flags)
{
     ...
     *
     *  */
     port_is_nif_dynamic = is_port_nif_dynamic (unit, port, interface_info.interface, flags);
     if (!port_is_nif_dynamic)
     {
         printf("Error, cint_dyn_port_add_port_full_example supports only NIF ports (excluding ELK / STIF). port=%d\n", port);
         return BCM_E_PORT;
     }
    ...
    if (!(flags & BCM_PORT_ADD_FLEXE_PHY))
    {
        ...
        if (rv != BCM_E_NONE)
        {
            printf("Error, cint_dyn_port_dnx_port_dyanamic_add_packet_processing_properties_set: unit=%d, port=%d\n", unit, port);
            return rv;
        }
    }
    /*
