    ...
    uint32 *ace_entry_handle)
{
    ...
    ace_entry_info.entry_action[2].value[0] = value << 4;
    ...
    for (qual_index = 0; qual_index < fg_info->nof_quals; qual_index++)
    {
        ...
        if (fg_info->qual_types[qual_index] == bcmFieldQualifySrcIp)
        {
            attach_info.key_info.qual_info[qual_index].input_type = bcmFieldInputTypeLayerAbsolute;
            attach_info.key_info.qual_info[qual_index].input_arg = 1;
    ...
    int is_udp_swap)
{
    ...
    if (rv != BCM_E_NONE)
    {
        printf("bcm_l2_egress_create failed\n");
        return rv;
    }
    rcy_encap_id = recycle_entry.encap_id;
    ...
    rv = cint_reflector_l3_internal_fp_entry_add(unit, fg_id, &fg_info, sip, tm_rcy_port, ace_entry_handle);
    if (rv != BCM_E_NONE)
    {
