 ...
 *  9) Forwarding destination is resolved from MPLS->ECMP lookup in LEM.
 *  10) ECMP is resolved to FEC.
 *  11) FEC is resolved as Outlif and Destination port.
 *  11) The IP tunnel header with two additional headers - UDP and VXLAN-GPE are encapsulated at Encap-1..
 *  12) The LL ETH is encapsulated in Encap-2 from the ARP entry.
 *
