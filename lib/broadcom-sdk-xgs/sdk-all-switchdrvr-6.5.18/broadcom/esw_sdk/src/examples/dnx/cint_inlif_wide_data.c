...
int inlif_wide_data_general_pmf_configure(int unit, int vtt_stage, int dest_port, uint32 wide_data)
{
     ...
     *  8b for VTT1, 8b for VTT2, 8b for VTT3, 8b for In-LIF[1] and 8b for In-LIF[0].
     */
    attach_info.key_info.qual_info[0].offset = qual_info_get.offset+(vtt_stage-1)*8;
...
int inlif_wide_data_mpls_service(int unit, int port1, int port2, int nof_labels)
{
     ...
     * Configure fec entry 
     */
    rv = vpls_create_fec_entry(unit, ac_port.encap_fec_id, mpls_encap_tunnel[0].tunnel_id, pwe_port.port);
    if (rv != BCM_E_NONE)
    {
...
int inlif_large_wide_data_l3_fwd_config(int unit, int vsi, int out_port)
{
    ...
    /** IPv6 */
    uint8 ip6_subnet[16] = {0xfe, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01};
    uint8 ip6_mask[16] = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x00};
