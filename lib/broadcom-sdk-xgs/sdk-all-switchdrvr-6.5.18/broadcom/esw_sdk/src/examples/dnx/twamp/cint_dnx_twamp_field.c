 ...
 * \brief
 *
 *  This function sets all required HW configuration for LM read index WA and attach it to given as param context for Q2A/J2C.
 *  mapping FLP counter_if(3) to LM read index(0) for TWAMP reflector SEQ.
 *
 * \param [in] unit - The unit number.
 *
 * \return
 ...
 *
 */
int cint_field_twamp_reflector_wa_lm_read_index_set_ipmf3(int unit) {
    int i, rv = BCM_E_NONE;
    void *dest_char;
...
int cint_field_twamp_reflector_rule(int unit, int is_ipv6, bcm_gport_t gport_trap1, 
    ...
    /* Add lm_read_index wa for TWAMP */
    if (*(dnxc_data_get(unit,  "field", "init", "wa_oam_stat", NULL)) == 0) {		
	    rv = cint_field_twamp_reflector_wa_lm_read_index_set_ipmf3(unit);
	    if(rv != BCM_E_NONE)
	    {
	        printf("cint_field_twamp_reflector_wa_lm_read_index_set_ipmf3 error %d\n", rv);
	        return rv;
	    }
	}
