    ...
    int nof_forbidden_ports)
{
    ...
    while (nof_forbidden_ports--)
    {
        ...
        rv = cint_field_evpn_egress_esi_port_entry_add(unit, egress_context_id, egress_fg_id, &esi_port_handle,
                                                       esi_label, forbidden_port);
        if (rv != BCM_E_NONE)
        {
    ...
    rv = cint_field_evpn_egress_esi_entry_add(unit, egress_context_id, egress_fg_id, &esi_label_handle,
                                              esi_label);
    if (rv != BCM_E_NONE)
    {
