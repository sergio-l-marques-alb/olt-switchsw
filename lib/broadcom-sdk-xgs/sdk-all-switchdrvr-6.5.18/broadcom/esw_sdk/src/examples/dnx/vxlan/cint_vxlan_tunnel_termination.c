 ...
 *  3) Get the ETH-RIF, equal to VSI. (eth_rif_intf_core)
 ...
 *  8) VTT4 lookup: {Next-layer-Network-Domain, VNI} -> VSI
 *  9) From the tunnel termination VSI perform forwarding lookup: MACT: {DA, VSI} -> destination port
 */
 ...
 *  5) Tunnel's ETH is terminated.
 *  6) Get the VRF from ETH-RIF.
 *  7) Get the tunnel termination lif from VTT3 lookup with key: {VRF,IPv6.DIP[127:0], current_layer.first_additional_header, current_layer.second_additional_header}.
 *  8) Tunnel VXLANoUDPoIPv6 is terminated.
 *  9) VTT5 lookup: {Next-layer-Network-Domain, VNI} -> VRF
 *  10) forwarding lookup: {DIP, VRF} -> FEC
 * 
 * 
 * 
 ...
 *  
 * The default vsi (17b) is: 2b defined in tunnel in lif. 
    ...
    int network_domain) 
{
    ...
    config.vsi = 0;          
    config.flags = BCM_VXLAN_NETWORK_DOMAIN_CONFIG_L2_MAPPING | BCM_VXLAN_NETWORK_DOMAIN_CONFIG_INGRESS_ONLY;                  
    rv = bcm_vxlan_network_domain_config_add(0, config); 
    if (rv != BCM_E_NONE)
    {
    ...
    int out_port2)
{
     ...
     */
    config.flags            = BCM_VXLAN_NETWORK_DOMAIN_CONFIG_INGRESS_ONLY | BCM_VXLAN_NETWORK_DOMAIN_CONFIG_L2_MAPPING | BCM_VXLAN_NETWORK_DOMAIN_CONFIG_REPLACE;
    ...
    int unit)
{
    ...
    int flags=BCM_VXLAN_VPN_ELAN | BCM_VXLAN_VPN_WITH_VPNID | BCM_VXLAN_VPN_WITH_ID;
    ...
    rv = bcm_vxlan_network_domain_config_add(0, &vxlan_vpn_config);
    ...
    rv = bcm_vxlan_network_domain_config_get (0, &vxlan_vpn_config_get);
    ...
    printf("Step 4: Try to create the same mapping with bcm_vxlan_network_domain_config_add API\n");
    rv = bcm_vxlan_network_domain_config_add(0, &vxlan_vpn_config);
    if (rv != BCM_E_EXISTS)
    {
        printf("Error in bcm_vxlan_network_domain_config_add, entry should exist already");
        return BCM_E_EXISTS;
    }
    ...
    rv = bcm_vxlan_network_domain_config_remove(0, &vxlan_vpn_config);
