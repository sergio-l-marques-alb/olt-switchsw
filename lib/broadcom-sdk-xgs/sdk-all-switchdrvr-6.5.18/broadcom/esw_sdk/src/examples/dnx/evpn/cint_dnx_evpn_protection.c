/*
 * $Id: cint_dnx_evpn_protection.c, Exp $
    ...
    int unit)
{
    ...
    p_vport->egress_vlan = evpn_protection_info.vpn;
    ...
    int unit)
{
    ...
    egress_mc = 0;
    rv = multicast__open_mc_group(unit, &evpn_protection_info.vpn, 0);
    if (rv != BCM_E_NONE)
    {
    ...
    int add_to_flooding)
{
    ...
    if (add_to_flooding)
    {
        ...
        rv = multicast__add_multicast_entry(unit, vsi, &gport, &vport->encap_id, 1, 0);
        if (rv != BCM_E_NONE)
        {
