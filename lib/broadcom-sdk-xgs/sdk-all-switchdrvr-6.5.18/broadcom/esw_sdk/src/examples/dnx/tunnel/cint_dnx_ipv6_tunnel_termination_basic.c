...
/*
 * $Id: cint_dnx_ipv6_tunnel_termination.c,v 1.00 Broadcom SDK $
 $Copyright: (c) 2019 Broadcom.
 Broadcom Proprietary and Confidential. All rights reserved.$ File: cint_dnx_ipv6_tunnel_termination.c Purpose: Various examples for IPTunnels.
 */
 ...
 *  7) Do IPv6 SIP compression in the VTT. Get the SIPv6-MSB-ID with lookup key : {IPv6.SIP[127:48]} (80 SIP MSBs) - VTT2 (VTB)
 *  8) Get the DIP-Index-Type and DIP-Index-Data from VTT lookup with key: {VRF,IPv6.DIP[127:0], current_layer.first_additional_header, current_layer.second_additional_header} - VTT3(VTC)
 *  9) Get the tunnel terminated lif from the VTT lookup with key: {IPv6-DIP-IDX-Data,SIP-IDX-Data,SIP[47:0]} - VTT4(VTD)
 *  10) Tunnel Gre4oIpv6 is terminated.
 *  11) From the tunnel termination LIF, we get the VRF
 *  12) Packet is routed according to VRF, DIP
 */
