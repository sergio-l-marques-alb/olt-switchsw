    ...
    int is_bridge_fallback)
{
    ...
    if (is_jericho2)
    {
        ipv6_mc_da_is_uc = *(dnxc_data_get(unit, "l3", "hw_bug", "ipv6_mc_compatible_dmac", NULL));
    }
    ...
    /* JR2_B0 WA: broadcast_group and unknown_multicast_group must be the same*/
    if (*dnxc_data_get(unit, "l2", "hw_bug", "bc_same_as_unknown_mc", NULL)) {
        flood_groups.broadcast_group = flood_groups.unknown_multicast_group;
    }

    /*
     * Due to a HW bug, in JR2_a0 the IPv6 MC packet ETH header is parsed as a UC one.
