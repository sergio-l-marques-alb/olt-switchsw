/*
 * $Id: cint_ip_tunnel_to_tunnel_basic.c,v 1.15 2017/7/20 12:39:34 sk019289 Exp $
 $Copyright: (c) 2019 Broadcom.
 Broadcom Proprietary and Confidential. All rights reserved.$ File: cint_ip_tunnel_to_tunnel_basic.c Purpose: Example of basic IP tunnel to tunnel configuration.
 */
 ...
 *
 *  Packet walk:
 *  1) Packet arrives at the incoming port (in_provider_port).
 ...
 *  9) From the tunnel termination LIF, we get the tunnel resolved VRF.
 *  10) Get the forwarding destination from a {DIP, tunnel resolved VRF}->FEC lookup in LPM.
 *  11) Forwarding resolution is a FEC which points to the Out-Lif.
 *  12) The IP tunnel header with two additional headers - UDP and VXLAN-GPE are encapsulated at Encap-1.
 *  13) The LL ETH is encapsulated in Encap-2 from the ARP entry.
 */
