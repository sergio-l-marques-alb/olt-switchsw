 ...
 * exit;
 *
 * dbal entry coMmit table=TRAP_FWD_RCY_CMD_MAP RECYCLE_COMMAND_ID=15 CORE_ID=0 TRAP_FWD_STRENGTH=0
                            ...
                            int out_port_egress)     /** Outgoing port of SRV6 Egress */
{
     ...
     */
    {
        ...
        rv = bcm_port_control_set(unit, cint_srv6_tunnel_info.recycle_port, bcmPortControlOverlayRecycle, 1);
        if (rv != BCM_E_NONE)
        {
        ...
        rv = create_recycle_entry(unit, srh_nxt_hdr_to_eth_type(cint_srv6_tunnel_info.srh_next_protocol_ipv4), &recycle_entry_encap_id);
        if (rv != BCM_E_NONE)
        {
        ...
        next_protocol_info.next_protocol = cint_srv6_tunnel_info.srh_next_protocol_ipv4;
        /** Convert Port number to GPORT */
        BCM_GPORT_LOCAL_SET(next_protocol_info.port, cint_srv6_tunnel_info.recycle_port);
        ...
        /*
         * repeat RCY configuration for IPv6 as next layer above RCY - to same RCY port, just encapsulation for IPv6
         */
        rv = create_recycle_entry(unit, srh_nxt_hdr_to_eth_type(cint_srv6_tunnel_info.srh_next_protocol_ipv6), &recycle_entry_encap_id);
        if (rv != BCM_E_NONE)
        {
            printf("Error, create_recycle_entry \n");
            return rv;
        ...
        next_protocol_info.flags = 0;
        next_protocol_info.next_protocol = cint_srv6_tunnel_info.srh_next_protocol_ipv6;
        /** Convert Port number to GPORT */
        BCM_GPORT_LOCAL_SET(next_protocol_info.port, cint_srv6_tunnel_info.recycle_port);
        /** Convert from L3_ITF to GPORT */
        BCM_L3_ITF_LIF_TO_GPORT_TUNNEL(next_protocol_info.encap_id , recycle_entry_encap_id);
