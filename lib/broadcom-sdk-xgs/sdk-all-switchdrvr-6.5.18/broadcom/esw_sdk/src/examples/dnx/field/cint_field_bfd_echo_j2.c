...
/*
 * Valid on JR1 header mode: create conditioned Direct extraction Field group
 * the condition is hit when ipmf1 FG had a hit, sets on the valid bit of ipmf1 action
...
int cint_field_echo_bfd_de_for_oam_offset_jr1_header_mode(
  int unit,
    bcm_field_context_t context_id_of_ipmf1_tcam_fg,
    bcm_field_group_t fg_id_of_ipmf1_tcam_fg,
    int offset_of_valid_bit_of_ipmf1_tcam_action)
...
int cint_field_bfd_echo_main(int unit,bcm_field_context_t context_id_bfd_echo, int is_ipv6)
{
    ...
    /*the offset should be reduced by 2 due to the PTCH bug in Jer1 mode, J2C doesn't have this issue*/
    if ( !is_device_or_above(unit, JERICHO2C) && (system_headers_mode == 0) ) {
        rv = cint_field_echo_bfd_de_for_oam_offset_jr1_header_mode(unit,context_id_bfd_echo,cint_field_echo_bfd_fg_id,offset-1);
    }
    else{
