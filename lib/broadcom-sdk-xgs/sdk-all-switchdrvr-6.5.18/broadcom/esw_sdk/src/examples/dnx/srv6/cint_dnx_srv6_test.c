    ...
    int out_port)
{
    ...
    bcm_mac_t dmac = {0x00, 0x00, 0x00, 0x00, 0x00, 0x30}; /* dmac, for L2 forward */
    bcm_gport_t gport;
    ...
    /** Create recycle entry for IPv4*/
    rv = create_recycle_entry(unit, 0x0800, &recycle_entry_encap_id);
    if (rv != BCM_E_NONE)
    {
    ...
    /** Create termination for next protocol IPv4 to RCH port and encap_id */
    bcm_srv6_terminator_next_protocol_mapping_t next_protocol_info;

    next_protocol_info.flags = 0;
    next_protocol_info.next_protocol = 4;
    ...
    /** Create recycle entry for IPv6*/
    rv = create_recycle_entry(unit, 0x86dd, &recycle_entry_encap_id);
    if (rv != BCM_E_NONE)
    {
        printf("Error, create_recycle_entry \n");
        return rv;
    ...
    /** Create termination for next protocol IPv6 to RCH port and encap_id */
    next_protocol_info.next_protocol = 41;
    ...
    /** Create recycle entry for ETH*/
    rv = create_recycle_entry(unit, 0, &recycle_entry_encap_id);
    if (rv != BCM_E_NONE)
    {
        printf("Error, create_recycle_entry \n");
        return rv;
    ...
    /** Create termination for next protocol no_next_protocol to RCH port and encap_id */
    next_protocol_info.next_protocol = 59;
