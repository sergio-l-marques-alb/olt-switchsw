                                         ...
                                         bcm_field_entry_t *entry_handle)
{
    ...
    ent_info.nof_entry_quals = cint_sflow_field_group_info.nof_quals;
    ...
    printf("TCAM Entry Id=(0x%x), layer_type=(%d), is_ip_short_packet=(%d) with Stat_Code (0x%x) \n",
                            *entry_handle, u32, is_ip_short_packet, gport_stat_samp);
...
int cint_sflow_field_advanced_group_create(int unit)
{
    ...
    */
    ...
    if (rv != BCM_E_NONE)
    {
       printf("Error (%d), in bcm_field_range_set cint_sflow_field_range_id_0\n", rv);
       return rv;
    }
    ...
    if (rv != BCM_E_NONE)
    {
       printf("Error (%d), in bcm_field_range_set cint_sflow_field_range_id_1\n", rv);
       return rv;
    }
    ...
    /* Set quals */
    cint_sflow_field_group_info.nof_quals = 2;
    ...
    rv = bcm_field_group_add(unit, BCM_FIELD_FLAG_MSB_RESULT_ALIGN, &cint_sflow_field_group_info, &cint_sflow_field_fg_id);
    if (rv != BCM_E_NONE)
    {
...
int cint_sflow_field_advanced_group_context_presel_set(int unit, bcm_field_context_t context_id,  bcm_field_presel_t presel_id, bcm_field_AppType_t app_type)
{
    ...
    */
    printf("0: context_id= %d, presel_id=%d, app_type=%d \n", context_id, presel_id, app_type+0);

    if ( app_type == bcmFieldAppTypeL2 ) {
        layer_type = bcmFieldLayerTypeEth;

        printf("1: layer_type=%d \n", layer_type);
    }
    else {
        layer_type = bcmFieldLayerTypeIp4;

        printf("2: layer_type=%d \n", layer_type);
    }
