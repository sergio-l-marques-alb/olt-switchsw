...
int egress_injection_ep_create(int unit, int maid_48b, bcm_oam_endpoint_memory_type_t endpoint_memory_type ) {
    ...
    if (is_device_or_above(unit, JERICHO2))
    {
        rv= set_counter_resource(unit, g_egress_injection.sys_port[0], 0, &g_egress_injection.endpoint[1].lm_counter);
    }
    else
    {
    ...
    if (is_device_or_above(unit, JERICHO2))
    {
        rv= set_counter_resource(unit, g_egress_injection.sys_port[1], 0, &g_egress_injection.endpoint[0].lm_counter);
    }
    else
    {
...
int inject_oam_packets(int unit) {
    ...
    rv = egress_injection_with_ports__start_run(0,port1,port2, 0, 0);
    if (rv != BCM_E_NONE) {
