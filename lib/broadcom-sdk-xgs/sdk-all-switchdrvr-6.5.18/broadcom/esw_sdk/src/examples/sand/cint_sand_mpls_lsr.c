...
/* $Id: cint_sand_mpls_lsr.c,v 1.0 2018/05/22 08:05:38 Gaopeng Guo $
 ...
 */
  ...
  struct cint_mpls_lsr_info_s
 {
     ...
     int is_mpls_raw;                  /* mpls raw service */
 };
    ...
    int out_sys_port)
{
    ...
    if (cint_mpls_lsr_info.is_mpls_raw)
    {
    ...
    uint32 force_flags)
{
    ...
    if (is_device_or_above(unit, JERICHO2) &&
        ...
        /*under IOP mode, PHP from EEDB should use dedicated LIF range.*/
        BCM_L3_ITF_SET(cint_mpls_lsr_info.egr_mpls_tunnel_encap_id,BCM_L3_ITF_TYPE_LIF, 0x3EF80);
    }
