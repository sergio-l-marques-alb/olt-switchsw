...
int field_jr2_comp_evpn_esi_db(int unit, int presel_id) {
    ...
    if(rv != BCM_E_NONE)
    {
        printf("field_jr2_comp_evpn_esi_db: Error in bcm_port_class_set port_class %d\n", bcmPortClassFieldIngressPMF1PacketProcessingPortCs);
        return rv;
    }
    ...
    if(rv != BCM_E_NONE)
    {
        printf("field_jr2_comp_evpn_esi_db: Error in bcm_field_context_create context %d\n", context_id);
        return rv;
    }
    ...
    if(rv != BCM_E_NONE)
    {
        printf("field_jr2_comp_evpn_esi_db Error in bcm_field_presel_set\n");
        return rv;
    }
    ...
    if (mpls_term_till_bos) {
        /** In case the featrure is supported, need to check the ESI label based on IML.BOS*/
        fg_info.nof_quals ++;
        fg_info.qual_types[1] = bcmFieldQualifyMplsLabelBos;  /** BOS in prior label*/
    }

    fg_info.nof_actions = 2;
    fg_info.action_types[0] = bcmFieldActionLearnRaw1;
    ...
    printf("Successfully attached context %d with presel_id %d to feild group %d\n", context_id, presel_id, ingress_fg_id);
    ...
    if (mpls_term_till_bos) {
        entry_in_info.entry_qual[1].type = bcmFieldQualifyMplsLabelId;
        entry_in_info.entry_qual[1].value[0] = 1;
        entry_in_info.entry_qual[1].mask[0] = 1;
    }
    ...
    /** destination(21b) = {5'b01100, sys_port(16b)} = learn_info_data1[22:2].*/
    dst_sys_port = BCM_GPORT_SYSTEM_PORT_ID_GET(forbidden_out_sys_pot) | (0xC << 16);
    uint32 learn_data = (800328 & 0xff800003) | (dst_sys_port << 2);
    entry_in_info.entry_action[0].type = bcmFieldActionLearnRaw1;
    entry_in_info.entry_action[0].value[0] = learn_data;
    ...
    dst_sys_port = BCM_GPORT_SYSTEM_PORT_ID_GET(forbidden_out_sys_pot) | (0xC << 16);
    learn_data = (800328 & 0xff800003) | (dst_sys_port << 2);
    entry_in_info.entry_action[0].type = bcmFieldActionLearnRaw1;
    entry_in_info.entry_action[0].value[0] = learn_data;
    ...
    if (mpls_term_till_bos) {
        entry_in_info.entry_qual[1].type = bcmFieldQualifyMplsLabelId;
        entry_in_info.entry_qual[1].value[0] = 0;
        entry_in_info.entry_qual[1].mask[0] = 0;
    }

    entry_in_info.entry_action[0].type = bcmFieldActionRedirect;
    entry_in_info.entry_action[0].value[0] = BCM_GPORT_BLACK_HOLE;
...
int field_jr2_comp_evpn_egress_filter_db(int unit, int presel_id) {
    ...
    /************ Init the port **************/
    uint32 egr_port_pmf_cs_profile = 2;  /** Do not use value 1 which is used for learn WA in ePMF*/
    rv = bcm_port_class_set(unit, cint_field_evpn_info.ports[2], bcmPortClassFieldEgressPacketProcessingPortCs, egr_port_pmf_cs_profile);
    if(rv != BCM_E_NONE)
    {
        printf("field_jr2_comp_evpn_esi_db: Error in bcm_port_class_set port_class %d\n", bcmPortClassFieldIngressPMF1PacketProcessingPortCs);
        return rv;
    ...
    if(rv != BCM_E_NONE)
    {
        printf("field_jr2_comp_evpn_egress_filter_db: Error in bcm_field_context_create context %d\n", context_id);
        return rv;
    }
    ...
    if(rv != BCM_E_NONE)
    {
        printf("Error in bcm_field_group_context_attach fg_id %d context_id %d\n", egress_fg_id, context_id);
        return rv;
    }
    printf("Successfully attached context %d with presel_id %d to feild group %d\n", context_id, presel_id, egress_fg_id);
    ...
    /*
     * Here we add 3 entries for 3 forbidden-out-port.
     */
    for (port_idx = 0; port_idx < 3; port_idx++) {
        bcm_field_entry_info_t_init(&entry_in_info);

        entry_in_info.entry_qual[0].type = bcmFieldQualifyDstPort;
        entry_in_info.entry_qual[0].value[0] = cint_field_evpn_info.ports[port_idx];
        entry_in_info.entry_qual[0].mask[0] = 0xFFFF;

        entry_in_info.entry_qual[1].type = FieldQualifyLrnSrcPort; 
        entry_in_info.entry_qual[1].value[0] = cint_field_evpn_info.ports[port_idx];
