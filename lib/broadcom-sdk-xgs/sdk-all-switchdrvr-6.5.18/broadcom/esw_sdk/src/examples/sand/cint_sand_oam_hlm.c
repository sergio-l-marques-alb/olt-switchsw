/*
 * $Id: cint_sand_oam_hlm.c, Exp $
 $Copyright: (c) 2019 Broadcom.
 Broadcom Proprietary and Confidential. All rights reserved.$ File: cint_sand_oam_hlm.c Purpose: Example of using OAM Hierarchical loss measurement.
 */
/*
 *
 * Usage:
    ...
    int port2)
{
    ...
    {
        rv = set_counter_resource(unit, port1, lm_counter_if[0], &lm_counter_base_ids[0]);
        if (rv != BCM_E_NONE)
        {
            printf("Error no %d, in set_counter_resource.\n", rv);
            return rv;
        }
        rv = set_counter_resource(unit, port1, lm_counter_if[1], &lm_counter_base_ids[2]);
        if (rv != BCM_E_NONE)
        {
            printf("Error no %d, in set_counter_resource.\n", rv);
            return rv;
        }
        rv = set_counter_resource(unit, port2, lm_counter_if[0], &lm_counter_base_ids[1]);
        if (rv != BCM_E_NONE)
        {
            printf("Error no %d, in set_counter_resource.\n", rv);
            return rv;
        }
        rv = set_counter_resource(unit, port2, lm_counter_if[1], &lm_counter_base_ids[3]);
        if (rv != BCM_E_NONE)
        {
            printf("Error no %d, in set_counter_resource.\n", rv);
            return rv;
    ...
    int out_sysport)
{
    ...
    {
        rv = set_counter_resource(unit, in_sysport, lm_counter_if[0], &lm_counter_base_ids[0]);
        if (rv != BCM_E_NONE)
        {
            printf("Error no %d, in set_counter_resource.\n", rv);
            return rv;
        }
        rv = set_counter_resource(unit, in_sysport, lm_counter_if[1], &lm_counter_base_ids[1]);
        if (rv != BCM_E_NONE)
        {
            printf("Error no %d, in set_counter_resource.\n", rv);
            return rv;
        }
        rv = set_counter_resource(unit, in_sysport, lm_counter_if[2], &lm_counter_base_ids[2]);
        if (rv != BCM_E_NONE)
        {
            printf("Error no %d, in set_counter_resource.\n", rv);
            return rv;
