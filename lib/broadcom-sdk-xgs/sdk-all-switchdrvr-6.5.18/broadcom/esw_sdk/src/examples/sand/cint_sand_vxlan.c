 ...
 * 
 * cint ../../src/examples/sand/dpp/cint_utils_global.c  
    ...
    int is_ecmp)
{
    ...
    /* Note: JR2PPMDL-416: IPv6 MC mac "3333" is parsed as is_da_mc=0( ETH_Qualifier) in JR2 A0
     * Due to this JR2_A0 bug, PPH will not have the PPH_Lif_In_lif header that in_lif_profile_1 
     * will not use the right in_lif_profile, caused the split horizon not work for VXLAN ROO IPv6 MC .
     */
...
/*
 * Function that creates the fec WA for core side(from access to core into overlay).
 * Inputs: unit - relevant unit;
 * Inputs: tunnel_idx - tunnel index;
 */
int vxlan_encap_fec_learn_wa(
    int unit, 
    int tunnel_idx) 
{
    int rv = 0;
    ...
    {
        /*
        * JR2 IOP: use following WA to update FEC-ID to 18bit.
    ...
    int vdc_enable)
{
    ...
    */
    ...
    /*
     * configure fec WA for learning purpose 
     */
    rv = vxlan_encap_fec_learn_wa(unit, tunnel_idx);
    if (rv != BCM_E_NONE)
    {
        printf("Error(%d), in vxlan_encap_fec_learn_wa\n", rv);
        return rv;
    }
