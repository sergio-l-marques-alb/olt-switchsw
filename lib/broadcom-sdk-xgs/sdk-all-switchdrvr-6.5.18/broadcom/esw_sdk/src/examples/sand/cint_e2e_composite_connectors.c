...
int credit_worth;
int is_dnx;
bcm_device_member_get(c5_unit, 0, bcmDeviceMemberDNX, &is_dnx);
if (is_dnx) {
     ...
     */
    credit_worth = *(dnxc_data_get(c5_unit, "iqs", "credit", "worth", NULL));
...
*/
    ...
    rv = bcm_cosq_gport_add(c5_unit, sys_gport[idx], C5_NUM_COS, 
...
int e2e_composite_connectors_verify(int kbits_sec_max_port, int port_idx)
{
    ...
    rc = bcm_port_get(0, port, &dummy_flags, &interface_info, &mapping_info);
    if (rc != BCM_E_NONE) {
