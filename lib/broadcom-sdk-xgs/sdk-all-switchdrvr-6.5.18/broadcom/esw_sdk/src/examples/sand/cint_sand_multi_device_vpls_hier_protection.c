...
/**
     ...
     int core_unit)
 {
    ...
    int dest_fap_id[2] = {access_unit,core_unit};
    int is_fabric_supported = 1;
    
    rv = bcm_multicast_create(0, BCM_MULTICAST_WITH_ID, &mc_id);
    if (rv != BCM_E_NONE) {
    printf("Error, bcm_multicast_create \n");
    return rv;
    }

    if (is_device_or_above(0, JERICHO2)) {
        is_fabric_supported = *(dnxc_data_get(0, "fabric", "general", "is_supported", NULL));
    }
	
    if (is_fabric_supported) {
        rv = bcm_fabric_multicast_set(0, mc_id, 0, 2, dest_fap_id);
        if (rv != BCM_E_NONE) {
      ...
      int is_access_side)
{
 ...
 multi_dev_vpls_hier_protection_add_access_port(int unit){
    ...
    vlan_port_1.egress_vlan = cint_hvpls_info.ac_outer_vlan;
    vlan_port_1.egress_inner_vlan = cint_hvpls_info.ac_inner_vlan;
    ...
    int pwe_sys_port)
{
    ...
    rv = multi_dev_vpls_hier_protection_fabric_mc_set(access_unit,core_unit);
    if (rv != BCM_E_NONE)
    {
