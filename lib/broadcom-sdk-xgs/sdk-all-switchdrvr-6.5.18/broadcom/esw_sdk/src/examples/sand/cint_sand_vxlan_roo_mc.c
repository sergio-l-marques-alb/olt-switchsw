    ...
    int unit)
{
    ...
    if (!is_device_or_above(unit, JERICHO2)) {
        ...
        recycle_entry.ethertype = (ip_tunnel_basic_info.tunnel_type[0] == bcmTunnelTypeVxlan6) ? 0x86DD: 0x0800;
        ...
        g_vxlan_roo_mc.recycle_entry_encap_id = recycle_entry.encap_id;
    }
...
int vxlan_roo_mc_add_native_mc(int unit){
    ...
    for (idx = 2; idx < 4; idx++) 
    {
        ...
        rv = bcm_multicast_vxlan_encap_get(unit, -1 /* not used */, -1 /* not used */, 
                                           cint_vxlan_basic_info.vxlan_port_id, &(encap_lif[idx]));
        if (rv != BCM_E_NONE) {
