    ...
    int pwe_port_2)
{
    ...
    {
        /*
         * JR2 IOP: use following WA to translate the 15bit global fec_id to 18bit local one.
         */
        int is_jericho2_mode = *(dnxc_data_get(0,  "headers", "system_headers", "system_headers_mode", NULL));
        if (!is_jericho2_mode) {
    ...
    bcm_vpn_t vpn_id)
{
    ...
    if (egress_mc && is_device_or_above(unit, JERICHO2) && (system_headers_mode == 0)
        && (*dnxc_data_get(unit, "fabric", "general", "is_supported", NULL)))
    {
        ...
        rv = bcm_fabric_multicast_set(0, mc_id, 0, nof_cores, mreps);
        if (rv != BCM_E_NONE) {
