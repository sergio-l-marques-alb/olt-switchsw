...
l3_vrrp_test(int unit, int is_arad, int max_nof_vlans, int ipv6_distinct)
{
    ...
    int _is_jr2_dev=is_device_or_above(unit, JERICHO2);
    if (_is_jr2_dev)
    {
        too_high_vlan = 4096;  /*JR2 support vrrp for all vlan*/
        print bcm_vlan_create(0,vlan);  /*JR2 need create vsi firstly*/
        too_high_vrid =256; /*JR2 support vrid form 0 to 255*/
    }

    rv = bcm_l3_vrrp_config_add(unit, vrrp_flags, too_high_vlan, 0);
    if (_is_jr2_dev) {
        /*JR2 return -7, because it will check if vis exist firstly*/
    ...
    /*JR2 does not support configure/get VRID for all VSI by vid=0, skip configure with vrid0*/
    if (!_is_jr2_dev) {
    ...
    /*On JR2, set array_size 1, get one vrid */
    if (!_is_jr2_dev && rv != BCM_E_PARAM) {
    ...
    /*JR2 does not configure entry with vrid0, then expected count is 2*/
    if (_is_jr2_dev) {
        expected_count = 2;
    }
    ...
    /*JR2 only configure vrid1 and vrid2*/
    if (_is_jr2_dev) {
        if ((vrids[0] != vrid1) || (vrids[1] != vrid2)) {
            printf("Incorrect vrids returned\n");
            return BCM_E_FAIL;
        }
    } else if ((vrids[0] != vrid1) || (vrids[1] != vrid2) || (is_aradplus && vrids[2] != vrid0)) {
      printf("Incorrect vrids returned\n");
      return BCM_E_FAIL;
    }

    /*JR2 only configure vrid1 and vrid2, dose not cofigure vrid0*/
    if (! _is_jr2_dev) {
