...
int l2_pp_lag_property_set (int unit, int trunk_gport) {
    ...
    rv = bcm_port_untagged_vlan_set(unit, trunk_gport, BCM_VLAN_DEFAULT);
    if (rv != BCM_E_NONE) {
    ...
    int Port_4)
{
    ...
    param.ports[IN_PORT_INDEX] = Port_1;
    param.ports[OUT_PORT_INDEX[0]] = Port_2;
    param.ports[OUT_PORT_INDEX[1]] = Port_3;
    param.ports[OUT_PORT_INDEX[2]] = Port_4;
    ...
    l2_basic_bridge_trunk_s * param)
{
    ...
    BCM_PBMP_PORT_ADD(untag_pbmp, g_l2_basic_bridge_trunk.ports[IN_PORT_INDEX]);
     ...
     */
    rv = bcm_port_untagged_vlan_set(unit, g_l2_basic_bridge_trunk.ports[IN_PORT_INDEX], g_l2_basic_bridge_trunk.vsi);
    if (rv != BCM_E_NONE)
    {
        printf("Error, in bcm_port_untagged_vlan_set\n");
        return rv;
     ...
     */
    BCM_GPORT_SUB_TYPE_LIF_SET(g_l2_basic_bridge_trunk.inLifList[IN_PORT_INDEX], BCM_GPORT_SUB_TYPE_LIF_EXC_INGRESS_ONLY, 5000);
    BCM_GPORT_VLAN_PORT_ID_SET(g_l2_basic_bridge_trunk.inLifList[IN_PORT_INDEX], g_l2_basic_bridge_trunk.inLifList[IN_PORT_INDEX]);
    bcm_vlan_port_t vlan_port;
    bcm_vlan_port_t_init(&vlan_port);
    vlan_port.criteria = BCM_VLAN_PORT_MATCH_PORT;
    vlan_port.vlan_port_id = g_l2_basic_bridge_trunk.inLifList[IN_PORT_INDEX];
    vlan_port.port = g_l2_basic_bridge_trunk.ports[IN_PORT_INDEX];
    ...
    for (i = 0; i < NUM_OF_OUT_PORTS; i++)
    {
        ...
        BCM_GPORT_SYSTEM_PORT_ID_SET(gport, g_l2_basic_bridge_trunk.ports[OUT_PORT_INDEX[i]]);
        member.gport = gport;
        ...
        if (i != 0)
        {
            ...
            if (rv != BCM_E_NONE)
            {
                printf("Error, in bcm_trunk_member_add( %d)\n", g_l2_basic_bridge_trunk.ports[OUT_PORT_INDEX[i]]);
                return rv;
            }
        }
        else
