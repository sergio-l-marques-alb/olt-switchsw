...
int is_dnx;
bcm_device_member_get(c4_unit, 0, bcmDeviceMemberDNX, &is_dnx);
...
if (is_dnx) {
     ...
     */
    int credit_worth = *(dnxc_data_get(c4_unit, "iqs", "credit", "worth", NULL));
...
*/
    ...
    rv = bcm_port_get(0, port, &dummy_flags, &interface_info, &mapping_info);
    if (rv != BCM_E_NONE) {
    ...
    rv = bcm_cosq_gport_add(c4_unit, sys_gport[idx], num_cos, 
...
int e2e_composite_se_teardown(int port_idx)
{
    ...
    rc = bcm_port_get(0, port_idx, &dummy_flags, &interface_info, &mapping_info);
    if (rc != BCM_E_NONE) {
