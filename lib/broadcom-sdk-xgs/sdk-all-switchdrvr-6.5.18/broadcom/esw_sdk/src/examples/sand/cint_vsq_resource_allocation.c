...
cint_src_vsqs_resource_allocate_qax(int unit, int numq, uint8 pool_id, bcm_gport_t src_vsq_gport, bcm_gport_t pg_base_vsq_gport)
{
    ...
    amounts.max_shared_pool = (pool_id == 0) ? 4096: 5120 * factor;
    ...
    for (offset = 0; offset < numq; offset++) {
        ...
        switch (offset) {
            ...
            case 6:
                reserved = 512 * factor;
                nominal_headroom = 256;
                max_headroom = nominal_headroom + 1;
                max_shared_pool = 2000 * factor;
            break;
            case 7:
                /*
                 * PG 7 is NON Lossless
                 */
                reserved = 256;
                nominal_headroom = 0;
                max_headroom = 0;
...
cint_src_vsqs_pfc_setup_qax(int unit, int numq, bcm_gport_t src_vsq_gport, bcm_gport_t pg_base_vsq_gport)
{
     ...
     */
    sal_memset(&config, 0, sizeof(config));
    config.xoff_threshold = 102400 * factor;
    config.xon_threshold = 102400 * factor;
    ...
    /* Thresholds for PG PFC */
    sal_memset(&config, 0, sizeof(config));
    config.xoff_fadt_threshold.thresh_min = 1024 * factor;
    config.xoff_fadt_threshold.thresh_max = 1024 * factor;
