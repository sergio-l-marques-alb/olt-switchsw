    ...
    int ttl_threshold)
{
        ...
        if (i == 4)
        {
            ...
            if (rc != BCM_E_NONE)
            {
                if (lpm_raw_data_enabled) {
                    ...
                    rc = add_ipv6_ipmc(unit, route4 /* MC DIP */, route0 /* UC SIP */, in_vlan, mc_id[0], vrf, 0, 0);
                }
                if (rc != BCM_E_NONE) {
            ...
            if (second_unit>=0)
            {
                ...
                if (rc != BCM_E_NONE) {
                    if (lpm_raw_data_enabled) {
                        ...
                        rc = add_ipv6_ipmc(second_unit, route4 /* MC DIP */, route0 /* UC SIP */, in_vlan, mc_id[1], vrf, 0, 0);
                    }
                    if (rc != BCM_E_NONE) {
                        printf("Error. add_ip6mc_entry failed: $rc\n", rc);
                        return BCM_E_FAIL;
        ...
        else if (i == 5)
        {
            ...
            if (rc != BCM_E_NONE)
            {
                if (lpm_raw_data_enabled) {
                    ...
                    rc = add_ipv6_ipmc(unit, route5 /* MC DIP */, route0 /* UC SIP */, in_vlan, mc_id[0], vrf, 0, 0);
                }
                if (rc != BCM_E_NONE) {
                    printf("Error. add_ip6mc_entry failed: $rc\n", rc);
                    return BCM_E_FAIL;
            ...
            if (second_unit>=0)
            {
                ...
                if (rc != BCM_E_NONE) {
                    if (lpm_raw_data_enabled) {
                        ...
                        rc = add_ipv6_ipmc(second_unit, route5 /* MC DIP */, route0 /* UC SIP */, in_vlan, mc_id[1], vrf, 0, 0);
                    }
                    if (rc != BCM_E_NONE) {
                        printf("Error. add_ip6mc_entry failed: $rc\n", rc);
                        return BCM_E_FAIL;
    ...
    if (fall2bridge_ive_enable) {
        ...
        if (rc != BCM_E_NONE)
        {
            printf("Error, bcm_l3_ip6_fall2bridge_ive\n");
            return rc;
        }
    }
