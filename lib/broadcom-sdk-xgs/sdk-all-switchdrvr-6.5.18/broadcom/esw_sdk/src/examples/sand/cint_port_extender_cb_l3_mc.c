...
int port_extender_cb_l3_mc__start_run(int unit, port_extender_cb_l3_mc_s *param){
    ...
    if (!is_device_or_above(unit, JERICHO2)) {
        ...
        rv = bcm_multicast_create(unit, BCM_MULTICAST_INGRESS_GROUP | BCM_MULTICAST_WITH_ID, &g_port_extender_cb_l3_mc.ipmc_index);
    }
    if (rv != BCM_E_NONE) {
    ...
    if (!is_device_or_above(unit, JERICHO2)) {
        ...
        rv = bcm_multicast_set(unit, g_port_extender_cb_l3_mc.ipmc_index,BCM_MULTICAST_INGRESS_GROUP, NOF_EXTENDER_PORTS, reps);
    }
    if (rv != BCM_E_NONE) {
