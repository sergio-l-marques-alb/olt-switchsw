 ...
 *
 *  Packet walk:
 *  1) Packet arrives at the incoming port (core_port)
 *  2) Get the default's VSI per port
 *  3) Get the ETH-RIF, equal to VSI. (core_eth_rif)
 *  4) ETH-RIF is enabled for routing.
 *  5) Tunnel's ETH is terminated.
 *  6) Get the VRF from ETH-RIF.
 *  7) Get the tunnel termination lif from VTT lookup
 *  8) Tunnel Ipv6 is terminated.
