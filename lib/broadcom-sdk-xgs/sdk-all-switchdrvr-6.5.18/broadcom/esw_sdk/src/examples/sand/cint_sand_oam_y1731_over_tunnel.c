...
/** Use non-protected VPLS configuration for OAM over PWE*/
int oam_pwe_use_non_protected_vpls = 1;
  ...
  if (type==bcmOAMEndpointTypeBHHMPLS) {
      ...
      if(is_device_or_above(unit, JERICHO2))
      {
          if ((type==bcmOAMEndpointTypeBHHPwe ||
               type==bcmOAMEndpointTypeBHHPweGAL) &&
               oam_pwe_use_non_protected_vpls)
          {
              /**
          ...
          else
          {
              ...
              if(type==bcmOAMEndpointTypeBhhSection) {
                  ...
                  /** Same port is used for Rx and Tx */
                  out_gport = cint_vpls_basic_info.mpls_port_id_egress;
              }
          }
      }
      else
  ...
  */
  ...
  if(is_device_or_above(unit, JERICHO2))
  {
     ...
     else
     {
      ...
      {
          /* Set channel type */
    	  rv = bcm_oam_mpls_tp_channel_type_tx_set(0,bcmOamMplsTpChannelPweonoam,0x8902);
    	     if (rv != BCM_E_NONE)
    	     {
  ...
  if(is_mpls_pwe_jumbo_dm){
    mep_acc_info.id = 32704;
...
int oam_o_gach_o_gal_o_pwe_o_lsp_example(int unit, int port1, int port2) {
    ...
    if(is_device_or_above(unit, JERICHO2))
    {
        ...
        /* inlif = cint_vpls_basic_info.mpls_port_id_ingress; */
        /* inlif = mpls_lsr_info_1.ingress_intf; */
    }
    else
    {
