...
cint_bfd_mesh_config(int fe_unit,int server_unit,int client_unit){
   int rv = BCM_E_NONE;
   int server_unit_j2c, client_unit_j2c;
   ...
   rv = is_device_j2c(server_unit, &server_unit_j2c);
   if (rv != BCM_E_NONE) {
     printf ("Failed to check if server device is J2C: %d \n", rv);
     return rv;

   }
   rv = is_device_j2c(client_unit, &client_unit_j2c);
   if (rv != BCM_E_NONE) {
     printf ("Failed to check if client device is J2C: %d \n", rv);
     return rv;
   }

   if(server_unit_j2c || client_unit_j2c)
   {
       cint_dnxf_fe2_to_loopback_retimer_set(fe_unit,server_unit,client_unit,48,fe_to_fap0_min_links_array,fe_to_fap1_min_links_array);

       fabric_mesh_config_example (server_unit,0,3,fap_id_min_array_1,48,fe_fap_min_links_1);
       fabric_mesh_config_example (client_unit,0,3,fap_id_min_array_2,48,fe_fap_min_links_2);
   }
   else
   {
       cint_dnxf_fe2_to_loopback_retimer_set($fe_unit,server_unit,client_unit,96,fe_to_fap0_links_array,fe_to_fap1_links_array);

       fabric_mesh_config_example (server_unit,0,2,fap_id_array_1,96,fe_fap_links_1);
       fabric_mesh_config_example (client_unit,0,2,fap_id_array_2,96,fe_fap_links_2);
   }
...
int bfd_server_example(int server_unit, int client_unit) {
    ...
    oam_offset_supported = *(dnxc_data_get(client_unit, "oam", "hw_bug", "oam_offset_supported", NULL));
    /** Step 0: Create LAG.  */
    ...
    if (is_dnx) 
    {
        ...
        else
        {
            if(is_ipv6)
            {
                ...
                rv = basic_example_ipv6(server_unit,bfd_endpoint_over_lag_1.eg_port, bfd_endpoint_over_lag_1.trunk_gport,0,0,0);
            ...
            else
            {
                 ...
                 */
                rv = l3_interface_init(server_unit, bfd_endpoint_over_lag_1.trunk_gport, bfd_endpoint_over_lag_1.eg_port, &next_hop_mac, &tunnel_id, 0, 0, 0);
                if (rv != BCM_E_NONE) {
                    printf("(%s) \n", bcm_errmsg(rv));
                    return rv;
                }
    ...
    if (is_dnx)
    {
      ...
      if(is_ipv6 || is_mpls)
      {
           rv = bcm_port_internal_get(server_unit, BCM_PORT_INTERNAL_OAMP, 2, oamp_gport, &count);
           if (rv != BCM_E_NONE) {
              printf("(%s) \n", bcm_errmsg(rv));
              return rv;
           }
           remote_oamp_sysport = oamp_gport[0];
      }
      else
      {
          if(is_dnx)
