...
/* $Id: cint_sand_multi_device_ipmc.c,v 1.0 2018/05/22 08:05:38 Jingli Guo $
    ...
    */
    if (mc_mode == 1) {
        ...
        if (mc_mode == 3) {
            ...
            int dest_fap_id[2] = {ingress_unit,egress_unit};
            uint32 max_nof_cores = 2;
            int is_fabric_supported = 1;
            if (is_device_or_above(egress_unit, JERICHO2)) {
                max_nof_cores = *(dnxc_data_get(egress_unit, "device", "general", "nof_cores", NULL));
                is_fabric_supported = *(dnxc_data_get(egress_unit, "fabric", "general", "is_supported", NULL));
            } 
            if (ingress_unit == egress_unit) {
                bcm_stk_modid_config_t modid_config;
                bcm_stk_modid_config_t_init(&modid_config);
                bcm_stk_modid_config_get(ingress_unit, &modid_config);
                dest_fap_id[0] = modid_config.modid;
                dest_fap_id[1] = modid_config.modid + 1;
            }
            bcm_multicast_create(fe_unit, BCM_MULTICAST_WITH_ID, &fe_mc_id);
            if (is_fabric_supported) {
                bcm_fabric_multicast_set(fe_unit, fe_mc_id, 0, max_nof_cores, dest_fap_id);
            }
        }
