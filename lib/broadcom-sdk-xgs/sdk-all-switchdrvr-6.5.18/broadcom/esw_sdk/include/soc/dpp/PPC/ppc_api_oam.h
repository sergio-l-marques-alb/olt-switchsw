...
#define SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY_FLAG_BFD_MPLS_PHP                0x8000000 /* set for bcmBFDTunnelTypeMplsPhp endpoint */
#define SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY_FLAG_MPLS_LM_DM_ILM              0x10000000 /* Inferred-LM indication for RFC-6374 endpoint */
...
typedef struct {
    ...
    uint8 is_ais_entry; /*indicate for AIS entry */
    /*RFC 6374*/
    uint32 session_identifier_id; /* uniquely identifies a measurement operation.*/
    uint32 timestamp_format; /* 0- Use NTP timestamp format; 1- use 1588 timestamp format.*/
    uint8  is_ilm;           /* 0- Use direct LM; 1- use infered LM.*/
    /*bfd, MPLS, PWE*/
    uint32 dst_ip_add;
...
typedef struct {
    ...
    uint8 is_jumbo_dm; /* enable jumbo data TLV*/
...
typedef struct {
    ...
    uint8 is_rfc_6374_entry; /** Check if the MEP is with type RFC6374 */
