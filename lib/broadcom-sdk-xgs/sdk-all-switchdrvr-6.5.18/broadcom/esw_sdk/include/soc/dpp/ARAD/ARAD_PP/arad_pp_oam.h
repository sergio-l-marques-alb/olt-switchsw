...
/*
 * TCAM index is taken backwards from MAX ENTRY ID because,
 * the TCAM index space is managed by getting the last entry used
 * and adding +1 to it. But this will not work for these dynamic entries.
 * Instead of creating holes in the middle of the TCAM, we are using the last
 * few entries since currently we do not have that many number of entries.
 *
 * NOTE: The number 16 below assumes 16 SRC IP profiles 
 * for acc endpoints in JERICHO and above. 
 * say if max_entry_id is 127, 
 *     then 
 *        112 to 127 entry IDs are for accelerated.
 *        and 
 *        96  to 111 entry IDs are for non-accelerated.
 */
...
typedef struct {
    ...
    uint8                           is_up_mep;
    uint32                          sys_port;             /* system_port of endpoint. */
    uint32                          local_port;            /* local_port  */
    /* Statistics */
...
typedef struct ARAD_PP_OAM_OAMP_MEP_DB_LM_DM_ENTRIES_INFO_S {
    ...
    uint32 is_maid_48; /*Flexible maid 48*/
    uint32 out_lif;
...
/* OAMA table configuration for RFC6374 endpoints */
...
/* OAMB table configuration for RFC6374 endpoints */
...
/* Insert to OAM2 - RFC-6374 accelerated entries */
...
/*********************************************************************
 * NAME:
 *   arad_pp_oam_diag_hw_tcam
 * TYPE:
 *   PROC
 * FUNCTION:
 *   Print oam tcam entries in HW TCAM.
 * INPUT:
 *   SOC_SAND_IN  int                   unit
 * REMARKS:
 *   Used by oam tcam diagnostics.
 ...
 * Set the TOD configurations at the ECI. Used for setting NTP, 1588 timestamping.
 *
 * @author vipin sharma (10/29/2018)
 *
 * @param unit
 * @param ts_format - NTP or IEEE1588
 * @param leap_info - values to be set:  NTP or IEEE1588 LEAP_SEC_CONTROL
 * @return uint32
 */
