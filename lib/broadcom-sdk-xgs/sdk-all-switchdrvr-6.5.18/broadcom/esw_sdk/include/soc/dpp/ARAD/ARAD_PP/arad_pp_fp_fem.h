...
#define ARAD_PP_FP_FEM_ALLOC_FES_POLARITY                   (0x40) /* Flip FES valid bit interpretation */
#define ARAD_PP_FP_FEM_ALLOC_FES_COMBINED                   (0x80) /* flag for FES that in combined action*/

#define ARAD_PP_FP_FES_COMBINE_MOVE_CONDITION_L             (0)     /* combined FES move when FES priority low */
#define ARAD_PP_FP_FES_COMBINE_MOVE_CONDITION_H             (1)     /* combined FES move when FES priority high */
 ...
 *     programs that will bypass this FEM (set as NOP). This allows 
 *     different program in the same DB to have different action results.
