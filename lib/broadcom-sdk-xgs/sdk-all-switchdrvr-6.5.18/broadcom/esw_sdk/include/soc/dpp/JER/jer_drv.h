...
typedef struct soc_dpp_config_jer_pp_s {
  ...
  uint32  kaps_large_db_bits;                       /* length of large direct look up key */
  ...
  uint32  res_sizes[ARAD_PP_FLP_KBP_MAX_NUMBER_OF_RESULTS]; /*external tcam result size segment */
  uint8   kaps_mgmt_advanced_mode;                  /* Enable/disable field large direct lookup advanced mode */
