...
/* CE for Hash key is fixed to 4~6, limit the place */
...
#define ARAD_PP_FP_KEY_ALLOC_ELK_RANGE         (0x200) /* for range of ELK*/
...
typedef struct
{
    ...
    uint32 msb_padding_bits; /* for KBP - number of bits for padding at CE MSB, those bits will be masked in TCAM */
...
/**
 * Given a qualifier (including UDFs) check if search_qual is equal it or
 * if it is the base of any of the UDFs.
 *
 * @param unit
 * @param qual_type - The qualifier.
 * @param search_qual - The qualifier to search for.
 * @param found - 1 if found, 0 otherwise.
 */
soc_error_t
