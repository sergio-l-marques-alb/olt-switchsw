...
typedef struct sand_cpu2tap_dynamic_s {
    uint32      nof_errors;
    uint8       skip_errors;
    uint8       log_level; /* If non zero, should be BSL_<LEVEL> = BSL_SEVERITY_ENC(bslSeverity*) */
...
/* This macro also does not stop the script in case of errors, but returns an error */
