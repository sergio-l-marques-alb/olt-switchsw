/*
 * $Id: nlmcmrbtree_def.h,v 1.2.8.2 2012/12/20 10:43:07 kirankum Exp $
...
/* This implementation was ported from C++ (done by JSI). You can find the C++
   source for this in /vobs/fitter/c39k/ut39k directory. This port to C was
   done by HDM. The following text was edited to reflect some of the changes
   that had to be done for the C implementation.
  
   Thank you Joseph (JSI) for writing your code in a way that made it so easy
   to port it to C :-) -- Haneef (HDM)
  
   A Basic Red Black Balanced Binary Search Tree
  
   Cf. Algorithms by Cormen, Leiserson, and Rivest, chapters 13 and 14.
  
   NOTE:
   Our tree nodes are RBNode objects; therefore, an RBNode can be in a
   single RBTree -- we use the pointers defined as part of the RBNode as the
   links in the tree.
  
   FYI: compile this optimized for better performance.  rbverify, quiet mode,
   sweeping through a set of 1000 entries, on a Sun Ultra 5:
   <PRE>
    debug:  20 seconds
    plain:  16 seconds
    -O2:     4 seconds 90% utilization
    -O3:     4 seconds 95% utilization
    -O4:     4 seconds 95% utilization
   </PRE>

   These times refer to the C++ implementation. The C implentation should be
   significantly faster in debug mode and a little faster in optimized mode.
   This is not a dig against C++ 'cause if we had used templates for our C++
   implementation, it could have been just as fast as C and a lot easier to
   read/implement/debug.
  
   Given a pointer to a key in the tree, we wanted to be able to locate the
   (root of) the tree; therefore, we had to have the linkage pointer as part of
   the layout, so the key must be embedded.  Note that this implies one of two
   options: (1) that we use a sentinel node for the root pointer, and climb to
   the top of the tree to the sentinel root pointer to access the tree itself;
   or (2), the Nil sentinel node can provide the tree.  We opt for (2).  Note
   that since Nil is tree specific, we need to see something special to know
   when we get to the Nil sentinel.  We opt to have the left pointer be 0.
   Embedding the pointers also allows us to point at the object itself, rather
   than the tree node, which simplifies the interface.  Lastly, clients depend
   on being able to delete an element to which an iterator is *not* positioned -- fortunately,
   since we splice elements (vice update key fields), that works, and works easily.

   NB: we support Remove(); therefore, we use sentinel Nil() instead of 0; cf.
   CLR, page 273.  Note that we did not determine and apply (all) the implied
   optimizations now that we're using a Nil sentinel; in particular, we don't
   know the interplay between RemoveFixup and Left/RightRotate; cf. CLR, page
   277, Exercise 14.4-4.
 */
