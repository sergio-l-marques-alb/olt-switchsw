...
/*@@NlmCmPrefix Module
   ...
   Summary
   This module includes the functions to create, compare, manipulate, and
   destroy prefixes in LPM tables. It provides the means by which routing
   prefixes are communicated to/from Nlmnapse.

   Description
   Prefixes are stored in LPM tables from MSB to LSB.

   The m_avail member indicates the number of bits of space that have 
   been allocated for storage.  The m_inuse member indicates the length of
   the data stored in the m_data array.  The m_data array is an over indexed
   array of all of the bits in the string.

   Bit locations start at 0 with the MSB being the most significant bit of the
   first byte. The LSB is the least significant bit in the last byte. 

   <CODE>
   Byte 0       Byte 1      Byte n (where n = ceil(N/8) and N is number of bits)
   76543210     76543210    76543210
   --------     --------    --------
   M                               L
   </CODE>

   M is the most significant bit (index = 0). L is the least significant
   bit (index = n-1).

   In the event of a bounds violation in any NlmCmPrefix method, an assert will
   be triggered. All data lengths specified in the formal parameters are
   in bits.

   The data stored in the m_data is guaranteed to contain zeros in bits beyond
   the inuse bits.

   The m_inuse member variable is a private variable that can only legally be
   changed with calls to NlmCmPrefix__TruncateInUse, or other functions that
   indirectly effect it such as NlmCmPrefix__SetString.
