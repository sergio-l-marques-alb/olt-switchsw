/*
 * $Id: nlmcmrbtree_impl.h,v 1.2.8.2 2012/12/20 10:43:07 kirankum Exp $
    ...
    NlmCmRBNodeType *x)
{
    ...
    if (x == NlmCmRBNode__GetParent(x)->m_left_p) {
        ...
        /* <<< done fixup from the left >>> */
    }
    else {
        /* >>> start fixup from the right <<< */
    ...
    NlmCmRBNodeType * z)
{
    ...
    */
    ...
    /* Jskud: since using sentinel, must ensure that x points to y's original parent 
       <B>after</B> splicing in y, since splice will make y parent of Nil if any child of y
       is Nil, yet x, which could be Nil, must point to y's parent when calling RemoveFixup().
    */
