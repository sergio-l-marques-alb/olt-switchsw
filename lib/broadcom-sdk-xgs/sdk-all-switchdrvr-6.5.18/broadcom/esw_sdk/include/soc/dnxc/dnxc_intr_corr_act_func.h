...
/* XOR memory flags */
/*
    For XOR memory IPS_QTYPE/IPS_QSPM/CGM_VOQ_VSQS_PRMS, write XOR bank will cause traffic broken.
    Set this flag as WA that doesn't recover XOR bank at SER corrective handler.
    It is just used as WA before ASIC provided final solution
*/
#define DNXC_XOR_MEMORY_NOT_RECOVER_XOR_BANK_FLAGS  0x1
/*
    For XOR memory, CGM_IPP_MAP it should be high severity but be marked as low.
    This flag is used as WA before ASIC change pl defination and do MCM release
*/
...
int dnxc_decide_interrupt_action_fmac_tuning_serdes(
    ...
    dnxc_int_corr_act_type * p_corrective_action,
    char *special_msg);
int dnxc_decide_interrupt_action_tcam_query_failure(
    int unit,
    int block_instance,
    uint32 en_interrupt,
    dnxc_int_corr_act_type * p_corrective_action,
    char *special_msg);
int dnxc_decide_interrupt_action_crps_CounterOvfError(
    int unit,
    int block_instance,
    uint32 en_interrupt,
    dnxc_int_corr_act_type * p_corrective_action,
