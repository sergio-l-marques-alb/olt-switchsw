...
typedef struct {
    mos_msg_host_status_t status;
    mos_msg64_data_t data[NUM_MOS_MSG_SLOTS];
} mos_msg64_area_t;
...
#define MSG64_AREA_HOST_TO_ARM(base, arm) ((base) + (sizeof(mos_msg64_area_t) * (arm)))
...
#define MSG64_AREA_ARM_TO_HOST(base, arm) ((base) + (sizeof(mos_msg64_area_t) * (NUM_UCS + (NUM_UCS * (arm)) + (arm))))
#define MSG_AREA_ARM_TO_ARM(base, FROM, TO) ((base) + (sizeof(mos_msg_area_t) * (NUM_UCS  + (NUM_UCS * (FROM)) + (TO))))
#define MSG64_AREA_ARM_TO_ARM(base, FROM, TO) ((base) + (sizeof(mos_msg64_area_t) * (NUM_UCS  + (NUM_UCS * (FROM)) + (TO))))

#define MSG_AREA_END(base) ((base) + sizeof(mos_msg_area_t) * 2 * (NUM_UCS+1) * NUM_UCS)
#define MSG64_AREA_END(base) ((base) + sizeof(mos_msg64_area_t) * 2 * (NUM_UCS+1) * NUM_UCS)
