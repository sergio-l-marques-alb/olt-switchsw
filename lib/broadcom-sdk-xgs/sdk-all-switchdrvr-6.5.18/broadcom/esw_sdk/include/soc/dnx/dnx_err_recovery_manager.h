...
#define DNXC_ROLLBACK_JOURNAL_DIAGNOSTICS_TRANSACTION_END(_unit)\
    dnxc_rollback_journal_diag_end(_unit)
...
/**
 * \brief
 *  Check if currently in top level API
 *  Used by regression functions only
 * \param [in] unit    - Device Id
 * \return
 *   uint8 - boolean result if access is err recovery
 * \remark
 ...
 *   * None
 */
uint8 dnx_er_validate_is_top_level_api(
    int unit);

/**
 * \brief
 *  Check if currently in top level transaction in top level api
 *  Used by regression functions only
 * \param [in] unit    - Device Id
 * \return
 *   uint8 - boolean result if access is err recovery
 ...
 *   * None
 */
uint8 dnx_er_validate_is_transaction_in_top_level_api(
    int unit);

/**
 * \brief
 *  Check if currently outside of trnasaction
 *  Used by regression functions only
 * \param [in] unit    - Device Id
 * \return
 *   uint8 - boolean result if access is err recovery
 ...
 *   * None
 */
uint8 dnx_er_validate_is_outside_of_transaction(
    int unit);
#endif /* DNX_ERR_RECOVERY_REGRESSION_TESTING */
