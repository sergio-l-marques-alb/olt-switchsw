/** \file dnx_data_internal_ingress_cs.h
 * 
 * MODULE DEVICE DATA - DNX_DATA
 * 
 * Device Data
 * SW component that maintains per device data
 * For additional details about Device Data Component goto 'dnxc_data_mgmt.h'
 *        
 *     
 * 
 * AUTO-GENERATED BY AUTOCODER!
 * DO NOT EDIT THIS FILE!
 */
/* *INDENT-OFF* */
/*
 * $Copyright: (c) 2019 Broadcom.
 * Broadcom Proprietary and Confidential. All rights reserved.$
 */
#ifndef _DNX_DATA_INTERNAL_INGRESS_CS_H_
/*{*/
#define _DNX_DATA_INTERNAL_INGRESS_CS_H_
/*
 * INCLUDE FILES:
 * {
 */
#include <soc/dnx/dnx_data/auto_generated/dnx_data_internal.h>
#include <soc/dnx/dnx_data/auto_generated/dnx_data_ingress_cs.h>
/*
 * }
 */

/*!
* \brief This file is only used by DNX (JR2 family). Including it by
* software that is not specific to DNX is an error.
*/
#ifndef BCM_DNX_SUPPORT
#error "This file is for use by DNX (JR2) family only!"
#endif

/*
 * TYPEDEFS:
 * {
 */
/**
 * \brief 
 * Submodule enum
 */
typedef enum
{
    dnx_data_ingress_cs_submodule_properties,
    dnx_data_ingress_cs_submodule_hw_workarounds,
    dnx_data_ingress_cs_submodule_parser,

    /**
     * Must be last one!
     */
    _dnx_data_ingress_cs_submodule_nof
} dnx_data_ingress_cs_submodule_e;

/*
 * }
 */

/*
 * SUBMODULE PROPERTIES:
 * {
 */
/*
 * Features
 */
/* Feature enum - placed in if header (should be exposed to component user */
/* Get Data */
/**
 * \brief get submodule feature - disabled/enabled
 * 
 * \param [in] unit - Unit #
 * \param [in] feature - feature to get (see enum definition for more details
 * 
 * \return
 *     is_supported - returns 1 if and only if supported and 0 otherwise
 * \remark
 *   * Auto-generated using device data autocoder
 * \see
 *   * None
 */
int dnx_data_ingress_cs_properties_feature_get(
    int unit,
    dnx_data_ingress_cs_properties_feature_e feature);

/*
 * Defines
 */
/**
 * \brief 
 * Define enum
 */
typedef enum
{
    dnx_data_ingress_cs_properties_define_cs_buffer_nof_bits_max,
    dnx_data_ingress_cs_properties_define_nof_bits_entry_size_prefix,

    /**
     * Must be last one!
     */
    _dnx_data_ingress_cs_properties_define_nof
} dnx_data_ingress_cs_properties_define_e;

/* Get Data */
/**
 * \brief returns define data of cs_buffer_nof_bits_max
 * Module - 'ingress_cs', Submodule - 'properties', data - 'cs_buffer_nof_bits_max'
 * Length in bits of the Context Selection TCAM buffer for qualifiers or masks, the maximum of all ingress CS TCAMs.
 * 
 * \param [in] unit - unit #
 * 
 * \return
 *     cs_buffer_nof_bits_max - the required data value
 * \remark
 *   * Auto-generated using device data autocoder
 * \see
 *   * None
 */
uint32 dnx_data_ingress_cs_properties_cs_buffer_nof_bits_max_get(
    int unit);

/**
 * \brief returns define data of nof_bits_entry_size_prefix
 * Module - 'ingress_cs', Submodule - 'properties', data - 'nof_bits_entry_size_prefix'
 * Number of bits used in the prefix indicating half key or full key. Expected to be used for each half key at the lsb for tables that support half entries.
 * 
 * \param [in] unit - unit #
 * 
 * \return
 *     nof_bits_entry_size_prefix - the required data value
 * \remark
 *   * Auto-generated using device data autocoder
 * \see
 *   * None
 */
uint32 dnx_data_ingress_cs_properties_nof_bits_entry_size_prefix_get(
    int unit);

/*
 * Tables
 */
/**
 * \brief 
 * Table enum
 */
typedef enum
{
    dnx_data_ingress_cs_properties_table_per_stage,

    /**
     * Must be last one!
     */
    _dnx_data_ingress_cs_properties_table_nof
} dnx_data_ingress_cs_properties_table_e;

/* Get Data */
/**
 * \brief get table per_stage entry 
 * Per stage (CS DB) information, including whether the CS_TCAM supports half entries.
 * 
 * \param [in] unit - unit #
 * \param [in] stage - CS DB, which CS TCAM to provide information for.
 * 
 * \return
 *     per_stage - returns the relevant entry values grouped in struct - see dnx_data_ingress_cs_properties_per_stage_t
 * \remark
 *   * Auto-generated using device data autocoder
 * \see
 *   * None
 */
const dnx_data_ingress_cs_properties_per_stage_t * dnx_data_ingress_cs_properties_per_stage_get(
    int unit,
    int stage);

/* Get Value Str */
/**
 * \brief Get table value as string
 * Module - 'ingress_cs', Submodule - 'properties', table - 'per_stage'
 * Per stage (CS DB) information, including whether the CS_TCAM supports half entries.
 * 
 * \param [in] unit - Unit #
 * \param [out] buffer - buffer for the value
 * \param [in] key0 - key #0 index - 0 if not used
 * \param [in] key1 - key #1 index - 0 if not used
 * \param [in] value_index - value index to get - the index is alligned with values array in table structure
 * 
 * \return
 *     err - See shr_error_e
 * \remark
 *   * Auto-generated using device data autocoder
 * \see
 *   * None
 */
shr_error_e dnx_data_ingress_cs_properties_per_stage_entry_str_get(
    int unit,
    char *buffer,
    int key0,
    int key1,
    int value_index);

/* Get Table info */
/**
 * \brief get general info table about table (for example key size)per_stage info
 * Per stage (CS DB) information, including whether the CS_TCAM supports half entries.
 * 
 * \param [in] unit - unit #
 * 
 * \return
 *     per_stage - returns table info
 * \remark
 *   * Auto-generated using device data autocoder
 * \see
 *   * None
 */
const dnxc_data_table_info_t * dnx_data_ingress_cs_properties_per_stage_info_get(
    int unit);

/*
 * SUBMODULE HW_WORKAROUNDS:
 * {
 */
/*
 * Features
 */
/* Feature enum - placed in if header (should be exposed to component user */
/* Get Data */
/**
 * \brief get submodule feature - disabled/enabled
 * 
 * \param [in] unit - Unit #
 * \param [in] feature - feature to get (see enum definition for more details
 * 
 * \return
 *     is_supported - returns 1 if and only if supported and 0 otherwise
 * \remark
 *   * Auto-generated using device data autocoder
 * \see
 *   * None
 */
int dnx_data_ingress_cs_hw_workarounds_feature_get(
    int unit,
    dnx_data_ingress_cs_hw_workarounds_feature_e feature);

/*
 * Defines
 */
/**
 * \brief 
 * Define enum
 */
typedef enum
{

    /**
     * Must be last one!
     */
    _dnx_data_ingress_cs_hw_workarounds_define_nof
} dnx_data_ingress_cs_hw_workarounds_define_e;

/* Get Data */
/*
 * Tables
 */
/**
 * \brief 
 * Table enum
 */
typedef enum
{

    /**
     * Must be last one!
     */
    _dnx_data_ingress_cs_hw_workarounds_table_nof
} dnx_data_ingress_cs_hw_workarounds_table_e;

/* Get Data */
/* Get Value Str */
/* Get Table info */
/*
 * SUBMODULE PARSER:
 * {
 */
/*
 * Features
 */
/* Feature enum - placed in if header (should be exposed to component user */
/* Get Data */
/**
 * \brief get submodule feature - disabled/enabled
 * 
 * \param [in] unit - Unit #
 * \param [in] feature - feature to get (see enum definition for more details
 * 
 * \return
 *     is_supported - returns 1 if and only if supported and 0 otherwise
 * \remark
 *   * Auto-generated using device data autocoder
 * \see
 *   * None
 */
int dnx_data_ingress_cs_parser_feature_get(
    int unit,
    dnx_data_ingress_cs_parser_feature_e feature);

/*
 * Defines
 */
/**
 * \brief 
 * Define enum
 */
typedef enum
{

    /**
     * Must be last one!
     */
    _dnx_data_ingress_cs_parser_define_nof
} dnx_data_ingress_cs_parser_define_e;

/* Get Data */
/*
 * Tables
 */
/**
 * \brief 
 * Table enum
 */
typedef enum
{

    /**
     * Must be last one!
     */
    _dnx_data_ingress_cs_parser_table_nof
} dnx_data_ingress_cs_parser_table_e;

/* Get Data */
/* Get Value Str */
/* Get Table info */
/*
 * FUNCTIONS:
 * {
 */
/**
 * \brief Init module
 * 
 * \param [in] unit - Unit #
 * \param [out] module_data - pointer to module data
 * 
 * \return
 *     rv - see 'shr_error_e
 * \remark
 *   * Auto-generated using device data autocoder
 * \see
 *   * None
 */
shr_error_e dnx_data_ingress_cs_init(
    int unit,
    dnxc_data_module_t *module_data);

/*
 * }
 */
/*}*/
#endif /*_DNX_DATA_INTERNAL_INGRESS_CS_H_*/
/* *INDENT-ON* */
