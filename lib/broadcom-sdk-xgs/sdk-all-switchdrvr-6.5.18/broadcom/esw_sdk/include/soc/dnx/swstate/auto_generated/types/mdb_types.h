...
/**
 * Information required for the eedb bank traffic lock bug workaround.
 */
typedef struct {
    /**
     * Holds the local lif for which a workaround is required when accesing  the eedb. First index is instance, second is phase.
     */
    int* local_lifs[2];
    /**
     * Use these global lifs as glboal lif destination when trying to      workaround the eedb access issue
     */
    int global_lif_destination[2];
} eedb_bank_traffic_lock_workaround_t;
...
typedef struct {
    ...
    /**
     * EM entry capacity estimate based on table and encoding.
     */
    ...
    eedb_bank_traffic_lock_workaround_t eedb_bank_traffic_lock_workaround;
    /**
