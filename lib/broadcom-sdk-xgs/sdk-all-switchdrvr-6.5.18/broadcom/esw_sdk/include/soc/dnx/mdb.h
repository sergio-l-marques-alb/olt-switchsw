...
#define MDB_OUTLIF_AND_GRANULARITY_TO_PREFIX(unit, outlif, granularity) \
        ((outlif >> utilex_log2_round_up(dnx_data_mdb.dh.nof_rows_in_eedb_bank_get(unit) * MDB_NOF_CLUSTER_ROW_BITS / granularity)) \
...
#define MDB_DH_SERIAL_CLUSTER_NUMBER_FOR_MACRO_TYPE(unit, macro_idx, cluster_idx) (((macro_idx)*dnx_data_mdb.dh.nof_macro_clusters_get(unit))+cluster_idx)
...
/**
 * \brief
 * Set the age refresh hit bit mode in the mdb.
 *  \param [in] unit - The unit number.
 *  \param [in] mdb_table - The mdb_table.
 *  \param [in] mode - the hit bit mode selected by the user.
...
/**
 * \brief
 * Get the age refresh hit bit mode in the mdb.
 *  \param [in] unit - The unit number.
 *  \param [out] mode - the return hit bit mode selected by the user.
 *  \return
 *   Error indication according to shr_error_e enum
 *  \remark
 *   None.
