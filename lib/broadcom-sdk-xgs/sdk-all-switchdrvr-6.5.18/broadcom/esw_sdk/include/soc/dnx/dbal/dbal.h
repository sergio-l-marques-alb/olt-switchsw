...
{                                                                                     \
  ...
  *   \param [in] _unit -
  *     Value of 'unit' input parameter at entry.
  *   \param [in] _src_handle_id - value of current entry_id, created and used befor calling this function
  *   \param [in] _dst_handle_id - value of copied entry_id, allocated internally
  * \par INDIRECT INPUT:
  * \par INDIRECT OUTPUT:
  *    \param [in] _dst_handle_id - value of copied entry_id
  */
...
/**
 * \brief Verifyes for a key field in a table, whether the key field has the "is_ranged_attribute."
 *
 *   \param [in] unit
 *   \param [in] table_id - the table to look in
...
/**
 * \brief Indicate if a table has a compatible image. Compatible image is indicated by sw state table status 
 * Note: API checks here only compatible image. A table with hw_error will still be compatible. 
 *   \param [in] unit
 *   \param [in] table_id - table id \n
 *   \param [out] is_compatible_image - compatible image
 *          indication \n
 * \return
 *   Non-zero(shr_error_e) in case of an error
 */
...
/**
 * \brief
 * perform tables init update dbal DB according to the XML ouput from DBAL autocoder.
 * \par DIRECT INPUT:
 *   \param [in] unit
 * \par DIRECT OUTPUT:
 *   Non-zero(shr_error_e) in case of an error
