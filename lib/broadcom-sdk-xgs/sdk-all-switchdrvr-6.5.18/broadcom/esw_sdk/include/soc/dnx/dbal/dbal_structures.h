...
#  define DBAL_FIELD_MAX_NUM_OF_STRUCT_FIELDS                   150
...
/** \brief the size in bits of the key buffer in the entry handle */
#  define DBAL_PHYSICAL_RES_SIZE_IN_BITS                        2800
...
/** This macro returns the buffer size of the table to perform operations. (it is rounded to words for endiannes) */
#define DBAL_TABLE_BUFFER_IN_BYTES(table)               WORDS2BYTES(BITS2WORDS(table->max_payload_size))

/** checks id the table is not dorect, corresponds to the function: dbal_tables_is_non_direct */
...
#define DBAL_FIELD_IND_IS_FIELD_ADDRESS                 5 /** incase a field is L2 address or v6 address size validations can not be done */
 ...
 *  defines that relevant to dbal_table_info_t. table indications_bm (the value means which bit is relevant to
 *  the indication)
 */
typedef enum {
    ...
    DBAL_TABLE_IND_IS_TABLE_DIRTY,                      /** indicates that an entry was added to this table. this indication can be changed dynamicly 
...
typedef enum
{
    ...
    /** Allow information loss, all result fields which only exist in the existing result type will be ignored . In case of existing field with different range - take it's new default value */
...
typedef struct
{
    ...
    /**
     * Hard logic (or HL+SW) access mapping. access mapping per mem,reg,sw For disabled result types, 
     * the hl_mapping_multi_res is all 0.
     **/
    dbal_hl_access_info_t *hl_mapping_multi_res;

    /** Tcam CS access info (alsp used dbal_stage)  */
...
typedef struct
{
    ...
    int is_std_1;
    dbal_status_e pemla_status; /** used for indicate that the pemla access is applicable (after loading the pemla driver)*/
