...
#define DNX_FIELD_CONTEXT_IS_ALLOCATED(_stage, _context_id, _no_print) \
    ...
    { \
        ...
        SHR_IF_ERR_EXIT(dnx_algo_field_context_id_is_allocated(unit, _stage, _context_id, &_is_allocated)); \
        if (!_is_allocated) \
        { \
            if (_no_print != 0) \
            { \
 ...
 * and we expect to enable this constant (or scrap it altogether.)
 */
