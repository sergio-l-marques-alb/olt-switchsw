...
/**
 * \brief - Given a unit, returns the number of inlifs (dpc, sbc or both) in the device.
 *
 * \param [in] unit - Identifier of the device to access.
 * \param [out] nof_sbc_inlifs - if not null, filled with the number of sbc inlifs in the device.
 * \param [out] nof_dpc_inlifs - if not null, filled with the number of dpc inlifs in the device.
 * \return
 *   shr_error_e
 *
 * \remark
...
/**
 * \brief
 *      Set overflow enable for data, ll.
 *      Setting ovf to 0 --> don't assert for higher values than the mag can handle (relevant for 20 bits outlif pointer, in which the mag outputs 1 extra bit for the data banks which should be ignored)
 *
 * \param [in] unit - Unit-ID
 * \param [in] enable_data_ovf - set data ovf.
 * \param [in] enable_ll_ovf - set ll ovf.
 *
 * \return
 *   shr_error_e
 */
shr_error_e dnx_algo_local_outlif_eedb_ovf_set(
    int unit,
