/***********************************************************************************
 ***********************************************************************************
 *  File Name     :  merlin_dino_functions.c                                        *
 *  Created On    :  29/04/2013                                                    *
 *  Created By    :  Kiran Divakar                                                 *
 *  Description   :  APIs for Serdes IPs                                           *
 *  Revision      :  $Id: merlin_dino_functions.c 1192 2015-10-20 22:08:45Z jgaither $ *
 *                                                                                 *
...
#include "merlin_dino_functions.h"
#include "merlin_dino_dependencies.h"

#include "merlin_dino_field_access.c"
#include "merlin_dino_internal.c"
#include "merlin_dino_dv_functions.c"
...
err_code_t merlin_dino_display_eye_scan( const phymod_access_t *pa ) {
    uint32_t   stripe[64];
    uint16_t   status = 0;
    int8_t     y;

    EFUN(merlin_dino_display_eye_scan_header( pa, 1));
    /* start horizontal acquisition */
    {   err_code_t err_code = merlin_dino_meas_eye_scan_start( pa, EYE_SCAN_HORIZ);
        if (err_code) {
            EFUN((merlin_dino_meas_eye_scan_done( pa ), err_code));
        }
    }
    ...
    for (y = 31;y>=-31;y=y-1)
    {
        {   err_code_t err_code = merlin_dino_read_eye_scan_stripe( pa, &stripe[0], &status);
            if (err_code) {
                EFUN((merlin_dino_meas_eye_scan_done( pa ), err_code));
            }
        }
        EFUN(merlin_dino_display_eye_scan_stripe( pa, y,&stripe[0]));
        EFUN_PRINTF(("\n"));
    }
    /* stop acquisition */
    EFUN(merlin_dino_meas_eye_scan_done( pa ));
    EFUN(merlin_dino_display_eye_scan_footer( pa, 1));

    return(ERR_CODE_NONE);
}

/* This function is for Falcon and is configured for passive mode */
err_code_t merlin_dino_meas_lowber_eye( const phymod_access_t *pa, const struct merlin_dino_eyescan_options_st eyescan_options, uint32_t *buffer) {
  int8_t y,x;
  int16_t i;
  uint16_t status;
  uint32_t errors = 0;
  ...
  for (y = eyescan_options.vert_max;y>=eyescan_options.vert_min;y=y-eyescan_options.vstep) {
    for (x=eyescan_options.horz_min;x<=eyescan_options.horz_max;x=x+eyescan_options.hstep) {
      /* acquire sample */
        EFUN(merlin_dino_pmd_uc_cmd_with_data( pa, CMD_DIAG_EN,CMD_UC_DIAG_GET_EYE_SAMPLE,((uint16_t)x)<<8 | (uint8_t)y,200));
        /* wait for sample complete */
#if 0
        do {
            EFUN(merlin_dino_delay_us(1000));
            ESTM(status=rdv_usr_diag_status());
            EFUN_PRINTF(("status=%04x\n",status));
        } while((status & 0x8000) == 0);
#else
        EFUN(merlin_dino_poll_diag_done( pa, &status, (((uint32_t)timeout)<<7)*10 + 20000));
#endif
        {
            merlin_dino_osr_mode_st osr_mode;
            ENULL_MEMSET(&osr_mode, 0, sizeof(merlin_dino_osr_mode_st));
            EFUN(_merlin_dino_get_osr_mode( pa, &osr_mode));
            if(osr_mode.tx_rx == MERLIN_DINO_OSX1) {
                EFUN(merlin_dino_prbs_err_count_ll( pa, &errors));
            } else if(osr_mode.tx_rx == MERLIN_DINO_OSX2) {
...
err_code_t merlin_dino_display_lowber_eye( const phymod_access_t *pa, const struct merlin_dino_eyescan_options_st eyescan_options, uint32_t *buffer) {
    int8_t x,y,i,z;
...
err_code_t merlin_dino_meas_eye_scan_start( const phymod_access_t *pa, uint8_t direction) {
    uint8_t lock;
    ...
    if(direction == EYE_SCAN_VERTICAL) {
        EFUN(merlin_dino_pmd_uc_diag_cmd( pa, CMD_UC_DIAG_START_VSCAN_EYE,200));
    } else {
        EFUN(merlin_dino_pmd_uc_diag_cmd( pa, CMD_UC_DIAG_START_HSCAN_EYE,200));
    }
    return(ERR_CODE_NONE);
}

err_code_t merlin_dino_read_eye_scan_stripe( const phymod_access_t *pa, uint32_t *buffer,uint16_t *status){
    uint32_t val[2] = {0,0};
    uint16_t sts = 0;
    int8_t   i;
    ...
    *status = 0;
    for(i=0;i<32;i++) {
        {   err_code_t err_code = merlin_dino_poll_diag_eye_data(pa,&val[0], &sts, 200);
            *status |= sts & 0xF000;
            if (err_code) {
                return (err_code);
            }
...
err_code_t merlin_dino_display_eye_scan_stripe( const phymod_access_t *pa, int8_t y,uint32_t *buffer) {
...
err_code_t merlin_dino_meas_eye_density_data( const phymod_access_t *pa, const struct merlin_dino_eyescan_options_st eyescan_options, int32_t *buffer,uint16_t *buffer_size) {
  ...
  EFUN(merlin_dino_pmd_uc_diag_cmd( pa, CMD_UC_DIAG_DENSITY,2000));
  i = 0;hzcnt=0;
  ...
  for (y = eyescan_options.vert_max;y>=eyescan_options.vert_min;y=y-eyescan_options.vstep) {
    ...
    for (x=eyescan_options.horz_min;x<=eyescan_options.horz_max;x=x+eyescan_options.hstep) {
         ...
         EFUN(merlin_dino_poll_dsc_state_equals_uc_tune( pa, 2000));
...
err_code_t merlin_dino_display_eye_scan_header( const phymod_access_t *pa, int8_t i) {
int8_t x;
    EFUN_PRINTF(("\n"));
...
err_code_t merlin_dino_display_eye_scan_footer( const phymod_access_t *pa, int8_t i) {
int8_t x;
    for(x=1;x<=i;x++) {
        EFUN_PRINTF(("         : -|----|----|----|----|----|----|----|----|----|----|----|----|-"));
    }
...
/*eye_scan_status_t merlin_dino_read_eye_scan_status() */
err_code_t merlin_dino_read_eye_scan_status( const phymod_access_t *pa, uint16_t *status) {
...
err_code_t merlin_dino_meas_eye_scan_done( const phymod_access_t *pa ) {
  EFUN(merlin_dino_pmd_uc_diag_cmd( pa, CMD_UC_DIAG_DISABLE,200));
  return(ERR_CODE_NONE);
}


err_code_t merlin_dino_start_ber_scan_test( const phymod_access_t *pa, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    uint8_t lock,sts;
    ESTM(lock = rd_pmd_rx_lock());
    if(lock == 0) {
    ...
    EFUN(merlin_dino_pmd_uc_cmd( pa, CMD_CAPTURE_BER_START, ber_scan_mode,500));
    return(ERR_CODE_NONE);
}

err_code_t merlin_dino_read_ber_scan_data( const phymod_access_t *pa, uint32_t *errors, uint32_t *timer_values, uint8_t *cnt, uint32_t timeout) {
 ...
 ESTM(sts = rdv_usr_diag_status());
    ...
    *cnt = (sts & 0x00FF)/3;
    for(i=0;i < *cnt;i++) {
        ...
        EFUN(merlin_dino_pmd_uc_cmd( pa,  CMD_READ_DIAG_DATA_WORD, 0, timeout));
        ...
        EFUN(merlin_dino_pmd_uc_cmd( pa,  CMD_READ_DIAG_DATA_BYTE, 0, timeout));
...
err_code_t merlin_dino_eye_margin_proj(const phymod_access_t *pa, USR_DOUBLE rate, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    ...
    EFUN(merlin_dino_start_ber_scan_test( pa, ber_scan_mode, timer_control, max_error_control));
    ...
    if(verbose > 5) {
        do {
            EFUN(merlin_dino_delay_us(2000000));
            ESTM(sts = rdv_usr_diag_status());
            EFUN_PRINTF(("sts=%04x\n",sts));
        ...
        EFUN(merlin_dino_poll_diag_done( pa, &sts,timer_control*2000));
    }
    if(verbose > 2) EFUN_PRINTF(("delay done\n"));

    EFUN(merlin_dino_read_ber_scan_data( pa,  &errs[0], &time[0], &cnt, 2000));

    if(verbose > 2) EFUN_PRINTF(("read done cnt=%d\n",cnt));

    EFUN(merlin_dino_pmd_uc_cmd( pa, CMD_CAPTURE_BER_END,0x00,50));
 ...
 /* }                                                                  */
    ...
    EFUN(merlin_dino_display_ber_scan_data(pa, rate, ber_scan_mode, &errs[0], &time[0],(uint8_t)_abs(offset_start)));
...
err_code_t merlin_dino_display_ber_scan_data (const phymod_access_t *pa, USR_DOUBLE rate, uint8_t ber_scan_mode, uint32_t *total_errs, uint32_t *total_time, uint8_t max_offset) {
...
err_code_t merlin_dino_display_lane_state_hdr( const phymod_access_t *pa ) {
...
err_code_t merlin_dino_display_lane_state_legend( const phymod_access_t *pa ) {
  EFUN_PRINTF(("\n"));
  EFUN_PRINTF(("**********************************************************************************************\n"));
...
err_code_t merlin_dino_display_lane_state( const phymod_access_t *pa ) {
  err_code_t err_code = _merlin_dino_display_lane_state_no_newline( pa );
  EFUN_PRINTF(("\n"));
  return (err_code);
}

err_code_t merlin_dino_get_eye_margin_est( const phymod_access_t *pa, int *left_eye_mUI, int *right_eye_mUI, int *upper_eye_mV, int *lower_eye_mV) {
...
err_code_t merlin_dino_display_core_state_hdr( const phymod_access_t *pa ) {
    char core_type[20] = "merlin_dino";
...
err_code_t merlin_dino_display_core_state_line( const phymod_access_t *pa ) {
  err_code_t err_code = _merlin_dino_display_core_state_no_newline( pa );
  EFUN_PRINTF(("\n"));
  return (err_code);
}

err_code_t merlin_dino_display_core_state_legend( const phymod_access_t *pa ) {
  EFUN_PRINTF(("\n"));
  EFUN_PRINTF(("**************************************************************************************************************\n"));
...
err_code_t merlin_dino_display_core_config( const phymod_access_t *pa ) {
    EFUN_PRINTF(("\n\n***********************************\n"  ));
    EFUN_PRINTF((    "**** SERDES CORE CONFIGURATION ****\n"  ));
    EFUN_PRINTF((    "***********************************\n\n"));
    {
        struct   merlin_dino_uc_core_config_st core_cfg;
        ENULL_MEMSET(&core_cfg, 0, sizeof(core_cfg));
            EFUN(merlin_dino_get_uc_core_config( pa, &core_cfg));
        {
...
err_code_t merlin_dino_display_lane_config( const phymod_access_t *pa ) {
    struct merlin_dino_uc_lane_config_st lane_cfg;

    ENULL_MEMSET(&lane_cfg, 0, sizeof(lane_cfg));

    EFUN_PRINTF(("\n\n*************************************\n"                                          ));
    ESTM_PRINTF((   "**** SERDES LANE %d CONFIGURATION ****\n"  , merlin_dino_get_lane(pa)                     ));
    EFUN_PRINTF((    "*************************************\n\n"                                        ));
    EFUN(                                                         merlin_dino_get_uc_lane_cfg( pa, &lane_cfg)      );
...
err_code_t merlin_dino_display_core_state( const phymod_access_t *pa ) {
    EFUN(merlin_dino_display_core_state_hdr( pa ));
    EFUN(merlin_dino_display_core_state_line( pa ));
    EFUN(merlin_dino_display_core_state_legend( pa ));
    return ERR_CODE_NONE;
}



err_code_t merlin_dino_display_lane_debug_status( const phymod_access_t *pa ) {
    /* startup */
    struct merlin_dino_usr_ctrl_disable_functions_st     ds;
    struct merlin_dino_usr_ctrl_disable_dfe_functions_st dsd;
    /* steady state */
    struct merlin_dino_usr_ctrl_disable_functions_st     dss;
    struct merlin_dino_usr_ctrl_disable_dfe_functions_st dssd;
    ...
    EFUN(merlin_dino_get_usr_ctrl_disable_startup( pa, &ds));
    ...
    EFUN(                                                            merlin_dino_get_usr_ctrl_disable_startup_dfe( pa, &dsd) );
    ...
    EFUN(                                                           merlin_dino_get_usr_ctrl_disable_steady_state( pa, &dss) );
    ...
    EFUN(                                                      merlin_dino_get_usr_ctrl_disable_steady_state_dfe( pa, &dssd) );
...
err_code_t merlin_dino_stop_uc_lane( const phymod_access_t *pa, uint8_t enable) {
  ...
  if (enable) {
    return(merlin_dino_pmd_uc_control( pa, CMD_UC_CTRL_STOP_GRACEFULLY,100));
  }
  else {
    return(merlin_dino_pmd_uc_control( pa, CMD_UC_CTRL_RESUME,50));
  }
}
...
err_code_t merlin_dino_stop_rx_adaptation( const phymod_access_t *pa, uint8_t enable) {
...
err_code_t merlin_dino_request_stop_rx_adaptation( const phymod_access_t *pa ) {

  return(merlin_dino_pmd_uc_control_return_immediate( pa, CMD_UC_CTRL_STOP_GRACEFULLY));
}
...
err_code_t merlin_dino_ucode_crc_verify( const phymod_access_t *pa, uint16_t ucode_len,uint16_t expected_crc_value) {
    uint16_t calc_crc;

    EFUN(merlin_dino_pmd_uc_cmd_with_data( pa, CMD_CALC_CRC,0,ucode_len,200));
...
err_code_t merlin_dino_start_ucode_crc_calc( const phymod_access_t *pa, uint16_t ucode_len) {

    EFUN(merlin_dino_pmd_uc_cmd_with_data_return_immediate( pa, CMD_CALC_CRC, 0, ucode_len)); /* Invoke Calculate CRC command and return control immediately */
    return(ERR_CODE_NONE);
}

err_code_t merlin_dino_check_ucode_crc( const phymod_access_t *pa, uint16_t expected_crc_value, uint32_t timeout_ms) {
    uint16_t calc_crc;
    err_code_t err_code;

    err_code = merlin_dino_poll_uc_dsc_ready_for_cmd_equals_1( pa, timeout_ms);     /* Poll for uc_dsc_ready_for_cmd = 1 to indicate merlin_dino ready for command */
    if (err_code) {
...
err_code_t merlin_dino_pmd_uc_cmd_return_immediate( const phymod_access_t *pa, enum srds_pmd_uc_cmd_enum cmd, uint8_t supp_info) {
  ...
  err_code = merlin_dino_poll_uc_dsc_ready_for_cmd_equals_1( pa, 1); /* Poll for uc_dsc_ready_for_cmd = 1 to indicate merlin_dino ready for command */
  if (err_code) {
...
/*EFUN(wr_uc_dsc_ready_for_cmd(0x0)  ); */                     /* Issue command, by clearing "ready for command" field */
  ...
  EFUN(merlin_dino_pmd_wr_reg(pa,DSC_A_DSC_UC_CTRL, cmddata));         /* This address is same for Eagle, and all Merlin */
...
err_code_t merlin_dino_pmd_uc_cmd( const phymod_access_t *pa, enum srds_pmd_uc_cmd_enum cmd, uint8_t supp_info, uint32_t timeout_ms) {
  ...
  EFUN(merlin_dino_pmd_uc_cmd_return_immediate( pa, cmd, supp_info));    /* Invoke merlin_dino_uc_cmd and return control immediately */

  EFUN(merlin_dino_poll_uc_dsc_ready_for_cmd_equals_1( pa, timeout_ms)); /* Poll for uc_dsc_ready_for_cmd = 1 to indicate merlin_dino ready for command */
...
err_code_t merlin_dino_pmd_uc_cmd_with_data( const phymod_access_t *pa, enum srds_pmd_uc_cmd_enum cmd, uint8_t supp_info, uint16_t data, uint32_t timeout_ms) {
    ...
    EFUN(merlin_dino_pmd_uc_cmd_with_data_return_immediate( pa, cmd, supp_info, data)); /* Invoke merlin_dino_uc_cmd and return control immediately */
...
err_code_t merlin_dino_pmd_uc_control_return_immediate( const phymod_access_t *pa, enum srds_pmd_uc_ctrl_cmd_enum control) {
  return(merlin_dino_pmd_uc_cmd_return_immediate( pa, CMD_UC_CTRL, (uint8_t) control));
}
...
err_code_t merlin_dino_pmd_uc_control( const phymod_access_t *pa, enum srds_pmd_uc_ctrl_cmd_enum control, uint32_t timeout_ms) {
  return(merlin_dino_pmd_uc_cmd( pa, CMD_UC_CTRL, (uint8_t) control, timeout_ms));
}
...
err_code_t merlin_dino_pmd_uc_diag_cmd( const phymod_access_t *pa, enum srds_pmd_uc_diag_cmd_enum control, uint32_t timeout_ms) {
  return(merlin_dino_pmd_uc_cmd( pa, CMD_DIAG_EN, (uint8_t) control, timeout_ms));
}
...
uint8_t merlin_dino_rdbl_uc_var( const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr) {
    uint8_t rddata;
    ...
    EPSTM(rddata = merlin_dino_rdb_uc_ram( pa, err_code_p, (LANE_VAR_RAM_BASE+addr+(merlin_dino_get_lane(pa)*LANE_VAR_RAM_SIZE)))); /* Use Micro register interface for reading RAM */

    return (rddata);
}

/* Micro RAM Lane Byte Signed Read */
int8_t merlin_dino_rdbls_uc_var( const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr) {
  return ((int8_t) merlin_dino_rdbl_uc_var( pa, err_code_p, addr));
}

/* Micro RAM Lane Word Read */
uint16_t merlin_dino_rdwl_uc_var( const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr) {
  uint16_t rddata;
  ...
  EPSTM(rddata = merlin_dino_rdw_uc_ram( pa, err_code_p, (LANE_VAR_RAM_BASE+addr+(merlin_dino_get_lane(pa)*LANE_VAR_RAM_SIZE)))); /* Use Micro register interface for reading RAM */
...
int16_t merlin_dino_rdwls_uc_var( const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr) {
  return ((int16_t) merlin_dino_rdwl_uc_var( pa, err_code_p, addr));
}
...
err_code_t merlin_dino_wrbl_uc_var( const phymod_access_t *pa, uint16_t addr, uint8_t wr_val) {
    ...
    if(addr < LANE_VAR_RAM_SIZE) {
        return (merlin_dino_wrb_uc_ram( pa, (LANE_VAR_RAM_BASE+addr+(merlin_dino_get_lane(pa)*LANE_VAR_RAM_SIZE)), wr_val));    /* Use Micro register interface for writing RAM */
    } else {
...
err_code_t merlin_dino_wrbls_uc_var( const phymod_access_t *pa, uint16_t addr, int8_t wr_val) {
  return (merlin_dino_wrbl_uc_var( pa, addr, wr_val));
}
...
err_code_t merlin_dino_wrwl_uc_var( const phymod_access_t *pa, uint16_t addr, uint16_t wr_val) {
    ...
    if(addr < LANE_VAR_RAM_SIZE) {
        return (merlin_dino_wrw_uc_ram( pa, (LANE_VAR_RAM_BASE+addr+(merlin_dino_get_lane(pa)*LANE_VAR_RAM_SIZE)), wr_val));    /* Use Micro register interface for writing RAM */
    } else {
        return(_error(ERR_CODE_INVALID_RAM_ADDR));
    }
}
...
err_code_t merlin_dino_wrwls_uc_var( const phymod_access_t *pa, uint16_t addr, int16_t wr_val) {
  return (merlin_dino_wrwl_uc_var( pa, addr,wr_val));
}
...
uint8_t merlin_dino_rdbc_uc_var( const phymod_access_t *pa, err_code_t *err_code_p, uint8_t addr) {
  ...
  EPSTM(rddata = merlin_dino_rdb_uc_ram( pa, err_code_p, (CORE_VAR_RAM_BASE+addr)));                      /* Use Micro register interface for reading RAM */

  return (rddata);
}

/* Micro RAM Core Byte Signed Read */
int8_t merlin_dino_rdbcs_uc_var( const phymod_access_t *pa, err_code_t *err_code_p, uint8_t addr) {
  return ((int8_t) merlin_dino_rdbc_uc_var( pa, err_code_p, addr));
}

/* Micro RAM Core Word Read */
uint16_t merlin_dino_rdwc_uc_var( const phymod_access_t *pa, err_code_t *err_code_p, uint8_t addr) {
  ...
  EPSTM(rddata = merlin_dino_rdw_uc_ram( pa, err_code_p, (CORE_VAR_RAM_BASE+addr)));                  /* Use Micro register interface for reading RAM */

  return (rddata);
}

/* Micro RAM Core Word Signed Read */
int16_t merlin_dino_rdwcs_uc_var( const phymod_access_t *pa, err_code_t *err_code_p, uint8_t addr) {
  return ((int16_t) merlin_dino_rdwc_uc_var( pa, err_code_p, addr));
}
...
err_code_t merlin_dino_wrbc_uc_var( const phymod_access_t *pa, uint8_t addr, uint8_t wr_val) {
    ...
    return (merlin_dino_wrb_uc_ram( pa, (CORE_VAR_RAM_BASE+addr), wr_val));                                /* Use Micro register interface for writing RAM */
    } else {
        return(_error(ERR_CODE_INVALID_RAM_ADDR));
    }
}
...
err_code_t merlin_dino_wrbcs_uc_var( const phymod_access_t *pa, uint8_t addr, int8_t wr_val) {
  return (merlin_dino_wrbc_uc_var( pa, addr, wr_val));
}
...
err_code_t merlin_dino_wrwc_uc_var( const phymod_access_t *pa, uint8_t addr, uint16_t wr_val) {
    ...
    if(addr < CORE_VAR_RAM_SIZE) {
        return (merlin_dino_wrw_uc_ram( pa, (CORE_VAR_RAM_BASE+addr), wr_val));                                 /* Use Micro register interface for writing RAM */
    } else {
        return(_error(ERR_CODE_INVALID_RAM_ADDR));
    }
}
...
err_code_t merlin_dino_wrwcs_uc_var( const phymod_access_t *pa, uint8_t addr, int16_t wr_val) {
  return(merlin_dino_wrwc_uc_var( pa, addr,wr_val));
}
...
err_code_t merlin_dino_set_uc_core_config( const phymod_access_t *pa, struct merlin_dino_uc_core_config_st struct_val) {
    {   
#ifndef ATE_LOG
        uint8_t reset_state;
        ESTM(reset_state = rdc_core_dp_reset_state());
        if(reset_state < 7) {
            EFUN_PRINTF(("ERROR: merlin_dino_set_uc_core_config( pa, ..) called without core_dp_s_rstb=0 Lane=%d reset_state=%d\n",merlin_dino_get_lane(pa),reset_state));
...
err_code_t merlin_dino_set_uc_lane_cfg( const phymod_access_t *pa, struct merlin_dino_uc_lane_config_st struct_val) {
#ifndef ATE_LOG
    uint8_t reset_state;

    ESTM(reset_state = rd_lane_dp_reset_state());
    if(reset_state < 7) {
        EFUN_PRINTF(("ERROR: merlin_dino_set_uc_lane_cfg( pa, ..) called without ln_dp_s_rstb=0 Lane=%d reset_state=%d\n",merlin_dino_get_lane(pa),reset_state));
...
err_code_t merlin_dino_get_uc_core_config( const phymod_access_t *pa, struct merlin_dino_uc_core_config_st *get_val)
{
...
err_code_t merlin_dino_get_uc_lane_cfg( const phymod_access_t *pa, struct merlin_dino_uc_lane_config_st *get_val) {
...
err_code_t merlin_dino_poll_diag_done( const phymod_access_t *pa, uint16_t *status, uint32_t timeout_ms) {
 uint8_t loop;
 ...
 for(loop=0;loop < 100; loop++) {
     ...
     if(loop>10) {
         EFUN(merlin_dino_delay_us(10*timeout_ms));
     }
 }
...
/* poll for microcontroller to populate the dsc_data register */
err_code_t merlin_dino_poll_diag_eye_data(const phymod_access_t *pa,uint32_t *data,uint16_t *status, uint32_t timeout_ms) {
 uint8_t loop;
 uint16_t dscdata;
 if(!data || !status) {
     return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
 ...
 for(loop=0;loop < 100; loop++) {
     ...
     if(((*status & 0x00FF) > 2) || ((*status & 0x8000) > 0)) {
        EFUN(merlin_dino_pmd_uc_cmd( pa,  CMD_READ_DIAG_DATA_WORD, 0, 200));
     ...
     if(loop>10) {
         EFUN(merlin_dino_delay_us(25*timeout_ms));
     }
 }
 return(_error(ERR_CODE_DIAG_TIMEOUT));
}
...
err_code_t merlin_dino_poll_uc_dsc_ready_for_cmd_equals_1( const phymod_access_t *pa, uint32_t timeout_ms) {
#ifndef ATE_LOG
  uint16_t loop;
  /* read quickly for 4 tries */
  for (loop = 0; loop < 100; loop++) {
    uint16_t rddata;
    EFUN(merlin_dino_pmd_rdt_reg(pa,DSC_A_DSC_UC_CTRL, &rddata));
...
#else
  {   uint16_t rddata;
      EFUN_PRINTF(("// ATE_LOG merlin_dino_poll_uc_dsc_ready_for_cmd_equals_1 begin\n"));
      if(timeout_ms < 100) {
          EFUN(merlin_dino_delay_us(1000 * timeout_ms));
      } else {
          EFUN(merlin_dino_delay_ms(timeout_ms));
      }
      EFUN(merlin_dino_pmd_rdt_reg(pa,DSC_A_DSC_UC_CTRL, &rddata));
      if (rddata & 0x0080) {    /* bit 7 is uc_dsc_ready_for_cmd */
          if (rddata & 0x0040) {  /* bit 6 is uc_dsc_error_found   */
          ...
          EFUN_PRINTF(("// ATE_LOG merlin_dino_poll_uc_dsc_ready_for_cmd_equals_1 end\n"));
          return (ERR_CODE_NONE);
      }
      EFUN_PRINTF(("// ATE_LOG merlin_dino_poll_uc_dsc_ready_for_cmd_equals_1 timeout\n"));
  }
#endif
...
err_code_t merlin_dino_poll_dsc_state_equals_uc_tune( const phymod_access_t *pa, uint32_t timeout_ms) {
  uint16_t loop;
  uint16_t dsc_state;
...
err_code_t merlin_dino_poll_st_afe_tx_fifo_resetb_equals_1( const phymod_access_t *pa, uint32_t timeout_ms) {
  uint16_t loop;
...
err_code_t merlin_dino_poll_micro_ra_initdone( const phymod_access_t *pa, uint32_t timeout_ms) {
  uint16_t loop;
...
err_code_t merlin_dino_tx_ull_config( const phymod_access_t *pa, uint8_t enable) {
    EFUN(merlin_dino_tx_rptr_mode_timing( pa, enable)); /* Enable jitter filter enable */
    EFUN(_ull_tx_mode_setup( pa, enable));
    return (ERR_CODE_NONE);
}

/* Configure ULL mode for RX */
err_code_t merlin_dino_rx_ull_config( const phymod_access_t *pa, uint8_t enable) {
    EFUN(merlin_dino_rx_rptr_mode_timing(enable));
    EFUN(_ull_rx_mode_setup( pa, enable));
    return (ERR_CODE_NONE);
}

/* TX PI setup for Repeater Mode */
err_code_t merlin_dino_tx_rptr_mode_timing( const phymod_access_t *pa, uint8_t enable) {
        ...
        EFUN(merlin_dino_delay_us(25));                            /* Wait for TX_PI to settle */
    return (ERR_CODE_NONE);
}

/* RX setup for Repeater Mode timing */
err_code_t merlin_dino_rx_rptr_mode_timing(uint8_t enable) {
    (void)enable;
...
err_code_t merlin_dino_init_pram_for_uc_load( const phymod_access_t *pa, uint16_t ucode_len) {
    ...
    EFUN(merlin_dino_poll_micro_ra_initdone( pa, 250));               /* Poll for micro_ra_initdone = 1 to indicate initialization done */
    ...
    /* Wait 8 pram clocks */
    /* call merlin_dino_finish_pram_load */
    USR_UNUSED_VAR(ucode_len);
    return(ERR_CODE_NONE);
}

err_code_t merlin_dino_finish_pram_load( const phymod_access_t *pa ) {
...
err_code_t merlin_dino_reg_dump( const phymod_access_t *pa ) {
  ...
  for (addr = 0x0; addr < 0x10; addr++) {
    ...
    EFUN(merlin_dino_pmd_rdt_reg(pa,addr,&rddata));
...
err_code_t merlin_dino_uc_core_var_dump( const phymod_access_t *pa ) {
  ...
  for (addr = 0x0; addr < 0xFF; addr++) {
    ...
    ESTM_PRINTF(("%02x ", merlin_dino_rdbc_uc_var( pa, __ERR, addr)));
  }
  return (ERR_CODE_NONE);
}
...
err_code_t merlin_dino_uc_lane_var_dump( const phymod_access_t *pa ) {
  ...
  EFUN_PRINTF(("\n**** SERDES UC LANE %d RAM VARIABLE DUMP ****",merlin_dino_get_lane(pa)));
  ...
  if (rx_lock == 1) {
      ...
      if (!uc_stopped) {
          EFUN(merlin_dino_stop_rx_adaptation( pa, 1));
      }
  } else {
      EFUN(merlin_dino_pmd_uc_control( pa, CMD_UC_CTRL_STOP_IMMEDIATE,200));
  }
    ...
    for (addr = 0x0; addr < LANE_VAR_RAM_SIZE; addr++) {
      ...
      ESTM_PRINTF(("%02x ", merlin_dino_rdbl_uc_var( pa, __ERR, addr)));
    }

  if (rx_lock == 1) {
      if (!uc_stopped) {
          EFUN(merlin_dino_stop_rx_adaptation( pa, 0));
      }
  } else {
      EFUN(merlin_dino_stop_rx_adaptation( pa, 0));
  }
...
err_code_t merlin_dino_display_diag_data( const phymod_access_t *pa, uint16_t  diag_level) {
    ...
    ESTM_PRINTF(("Core = %d; LANE = %d\n",merlin_dino_get_core(pa),merlin_dino_get_lane(pa)));
    EFUN(merlin_dino_version( pa, &api_version));
    ...
    if (rx_lock == 1) {
        ...
        EFUN(merlin_dino_display_lane_state_hdr( pa ));
        EFUN(merlin_dino_display_lane_state( pa ));
    ...
    if(diag_level & SRDS_DIAG_EVENT) {
        ...
        EFUN(merlin_dino_read_event_log( pa, trace_mem,EVENT_LOG_HEX));
    }
    if(diag_level & SRDS_DIAG_EYE) {
        EFUN(merlin_dino_display_eye_scan( pa ));
    }
    if(diag_level & SRDS_DIAG_REG_CORE) {
        EFUN(merlin_dino_reg_dump( pa ));
    }
    ...
    if(diag_level & SRDS_DIAG_UC_CORE) {
        EFUN(merlin_dino_uc_core_var_dump( pa ));
    }
    if(diag_level & SRDS_DIAG_UC_LANE) {
        EFUN(merlin_dino_uc_lane_var_dump( pa ));
    }
    if(diag_level & SRDS_DIAG_LANE_DEBUG) {
        EFUN(merlin_dino_display_lane_debug_status( pa ));
    }
    if(diag_level & SRDS_DIAG_BER_VERT) {
        ...
        EFUN(_merlin_dino_display_ber_scan_data( pa, ber_mode, timer_control,  err_threshold));
...
/* Required Diagnostic Functions */
err_code_t merlin_dino_diag_access( const phymod_access_t *pa, enum srds_diag_access_enum type, uint16_t addr, uint16_t data, uint16_t param) {
    ...
    case SRDS_REG_READ:      {
                             ...
                             if(data > 1) {
                                 ...
                                 for (i = 0; i < data; i++) {
                                     ...
                                     EFUN(merlin_dino_pmd_rdt_reg(pa,i+addr,&tmp));
                                     ESTM_PRINTF(("%04x ", tmp));
                                 }
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 EFUN(merlin_dino_pmd_rdt_reg(pa,addr,&tmp));
    ...
    case SRDS_REG_RMW:       {
                             ...
                             EFUN(merlin_dino_pmd_wr_reg(pa,addr,tmp2));
    ...
    case SRDS_CORE_RAM_READ_BYTE: {
                             if(data > 1) {
                                 ...
                                 for (i = 0; i < (uint8_t)data; i++) {
                                     ...
                                     ESTM_PRINTF(("%02x ", merlin_dino_rdbc_uc_var( pa, __ERR, i+(uint8_t)addr)));
                                 }
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 ESTM_PRINTF(("Core RAM Read byte: x%04x = x%02x\n",(uint8_t)addr,merlin_dino_rdbc_uc_var( pa, __ERR, (uint8_t)addr)));
                             }
                             } break;
    case SRDS_LANE_RAM_READ_BYTE: {
                             if(data > 1) {
                                 ...
                                 for (i = 0; i < data; i++) {
                                     ...
                                     ESTM_PRINTF(("%02x ", merlin_dino_rdbl_uc_var( pa, __ERR,i+addr)));
                                 }
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 ESTM_PRINTF(("Lane RAM Read byte: x%04x = x%02x\n",addr,merlin_dino_rdbl_uc_var( pa, __ERR,addr)));
                             }
                             } break;
    case SRDS_CORE_RAM_READ_WORD: {
                             if(data > 1) {
                                 ...
                                 for (i = 0; i < (uint8_t)data; i+=2) {
                                     ...
                                     ESTM_PRINTF(("%04x ", merlin_dino_rdwc_uc_var( pa, __ERR, i+(uint8_t)addr)));
                                 }
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 ESTM_PRINTF(("Core RAM Read word: x%04x = x%04x\n",addr,merlin_dino_rdwc_uc_var( pa, __ERR, (uint8_t)addr)));
                             }
                             } break;
    case SRDS_LANE_RAM_READ_WORD: {
                             if(data > 1) {
                                 ...
                                 for (i = 0; i < data; i+=2) {
                                     ...
                                     ESTM_PRINTF(("%04x ", merlin_dino_rdwl_uc_var( pa, __ERR,i+addr)));
                                 }
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 ESTM_PRINTF(("Lane RAM Read word: x%04x = x%04x\n",addr,merlin_dino_rdwl_uc_var( pa, __ERR, addr)));
                             }
                             } break;
    case SRDS_CORE_RAM_RMW_BYTE:  {
                             uint8_t tmp, tmp2;
                             ESTM(tmp = merlin_dino_rdbc_uc_var( pa, __ERR, (uint8_t)addr));
                             tmp2 = (tmp & (uint8_t)~param) | ((uint8_t)param & data);
                             EFUN(merlin_dino_wrbc_uc_var( pa, (uint8_t)addr,tmp2));
                             EFUN_PRINTF(("Core RAM RMW byte: x%04x = x%02x -> x%02x\n",addr,tmp,tmp2));
                             } break;
    case SRDS_LANE_RAM_RMW_BYTE:  {
                             uint8_t tmp, tmp2;
                             ESTM(tmp = merlin_dino_rdbl_uc_var( pa, __ERR,addr));
                             tmp2 = (tmp & (uint8_t)~param) | ((uint8_t)param & data);
                             EFUN(merlin_dino_wrbl_uc_var( pa, addr,tmp2));
                             EFUN_PRINTF(("Lane RAM RMW byte: x%04x = x%02x -> x%02x\n",addr,tmp,tmp2));
                             } break;
    case SRDS_CORE_RAM_RMW_WORD:  {
                             uint16_t tmp, tmp2;
                             ESTM(tmp = merlin_dino_rdwc_uc_var( pa, __ERR, (uint8_t)addr));
                             tmp2 = (tmp & ~param) | (param & data);
                             EFUN(merlin_dino_wrwc_uc_var( pa, (uint8_t)addr,tmp2));
                             EFUN_PRINTF(("Core RAM RMW word: x%04x = x%04x -> x%04x\n",addr,tmp,tmp2));
                             } break;
    case SRDS_LANE_RAM_RMW_WORD:  {
                             uint16_t tmp, tmp2;
                             ESTM(tmp = merlin_dino_rdwl_uc_var( pa, __ERR,addr));
                             tmp2 = (tmp & ~param) | (param & data);
                             EFUN(merlin_dino_wrwl_uc_var( pa, addr,tmp2));
    ...
    case SRDS_GLOB_RAM_READ_BYTE: {
                             if(data > 1) {
                                 ...
                                 for (i = 0; i < data; i++) {
                                     ...
                                     ESTM_PRINTF(("%02x ", _merlin_dino_rdb_uc_var( pa, __ERR,i+addr)));
                                 }
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 ESTM_PRINTF(("Glob RAM Read byte: x%04x = x%02x\n",addr,_merlin_dino_rdb_uc_var( pa, __ERR,addr)));
                             }
                             } break;
    case SRDS_GLOB_RAM_RMW_BYTE:  {
                             uint8_t tmp, tmp2;
                             ESTM(tmp = _merlin_dino_rdb_uc_var( pa, __ERR,addr));
                             tmp2 = (tmp & (uint8_t)~param) | ((uint8_t)param & data);
                             EFUN(_merlin_dino_wrb_uc_var( pa, addr,tmp2));
    ...
    case SRDS_GLOB_RAM_READ_WORD: {
                             if(data > 1) {
                                 ...
                                 for (i = 0; i < data; i+=2) {
                                     ...
                                     ESTM_PRINTF(("%04x ", _merlin_dino_rdw_uc_var( pa, __ERR,i+addr)));
                                 }
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 ESTM_PRINTF(("Glob RAM Read word: x%04x = x%04x\n",addr,_merlin_dino_rdw_uc_var( pa, __ERR,addr)));
                             }
                             } break;
    case SRDS_GLOB_RAM_RMW_WORD:  {
                             uint16_t tmp, tmp2;
                             ESTM(tmp = _merlin_dino_rdw_uc_var( pa, __ERR,addr));
                             tmp2 = (tmp & ~param) | (param & data);
                             EFUN(_merlin_dino_wrw_uc_var( pa, addr,tmp2));
                             EFUN_PRINTF(("Glob RAM RMW word: x%04x = x%04x -> x%04x\n",addr,tmp,tmp2));
                             } break;
    case SRDS_UC_CMD:        {
                             uint16_t tmp;
                             EFUN(merlin_dino_pmd_uc_cmd_with_data( pa, (enum srds_pmd_uc_cmd_enum)addr, (uint8_t)param, data, 100));
                             ESTM(tmp = rd_uc_dsc_data());
                             EFUN_PRINTF(("uC Command: cmd=x%02x supp=x%02x data=x%04x returned=x%04x\n",addr,param,data,tmp));
                             } break;
    case SRDS_BER_PROJ_DATA: {
                             /* display ber projections for all channels */
                             EFUN(_merlin_dino_display_ber_scan_data( pa, (uint8_t)addr, (uint8_t)data,  (uint8_t)(param>>4)));
                             } break;
    case SRDS_EN_BREAKPOINT: 
    case SRDS_GOTO_BREAKPOINT:
    case SRDS_RD_BREAKPOINT:
    case SRDS_DIS_BREAKPOINT:
    default: EFUN_PRINTF(("Invalid request type merlin_dino_diag_access\n"));
    }
...
err_code_t merlin_dino_version( const phymod_access_t *pa, uint32_t *api_version) {

    if(!api_version) {
        return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
    }
    *api_version = 0xA1020B;
    return (ERR_CODE_NONE);
}
...
err_code_t merlin_dino_read_event_log( const phymod_access_t *pa, uint8_t *trace_mem,enum srds_event_log_display_mode_enum display_mode) {
    ...
    EFUN(merlin_dino_event_log_stop( pa ));

    EFUN(merlin_dino_event_log_readmem( pa, trace_mem));

    EFUN(merlin_dino_event_log_display( pa, trace_mem, display_mode));
...
err_code_t merlin_dino_event_log_stop( const phymod_access_t *pa ) {
    ...
    EFUN(merlin_dino_pmd_uc_cmd( pa, CMD_EVENT_LOG_READ, CMD_EVENT_LOG_READ_START, 10));
...
err_code_t merlin_dino_event_log_readmem( const phymod_access_t *pa, uint8_t *trace_mem) {
    ...
    do {
        /* Read Next */
        EFUN(merlin_dino_pmd_uc_cmd( pa, CMD_EVENT_LOG_READ, CMD_EVENT_LOG_READ_NEXT, 10));
    ...
    EFUN(merlin_dino_pmd_uc_cmd( pa, CMD_EVENT_LOG_READ, CMD_EVENT_LOG_READ_DONE, 10));
    ...
    if (display_mode == EVENT_LOG_DECODED || display_mode == EVENT_LOG_HEX_AND_DECODED) {
        ...
        while (addr < trace_mem_size-8) {                                                            /* last 8-byte reserved for trace memory wraparound */
            ...
            if (*(trace_mem+addr) == 0xff) {                                                         /* timestamp wraparound event handling */
                ...
                EFUN(merlin_dino_uc_lane_idx_to_system_id(uc_lane_id_str, cc));
...
err_code_t merlin_dino_display_state ( const phymod_access_t *pa ) {
    EFUN(merlin_dino_display_core_state( pa ));
    EFUN(merlin_dino_display_lane_state_hdr( pa ));
    EFUN(merlin_dino_display_lane_state( pa ));
    EFUN(merlin_dino_display_lane_state_legend( pa ));
    return(ERR_CODE_NONE);
}

err_code_t merlin_dino_display_config ( const phymod_access_t *pa ) {
    EFUN(merlin_dino_display_core_config( pa ));
    EFUN(merlin_dino_display_lane_config( pa ));
    return(ERR_CODE_NONE);
}
...
err_code_t merlin_dino_force_die_temperature ( const phymod_access_t *pa, int16_t die_temp) {
...
err_code_t merlin_dino_read_die_temperature ( const phymod_access_t *pa, int16_t *die_temp) {

    uint16_t die_temp_sensor_reading;

    EFUN(merlin_dino_pmd_uc_cmd( pa, CMD_UC_DBG,CMD_UC_DBG_DIE_TEMP,100));
...
err_code_t merlin_dino_read_die_temperature_double (const phymod_access_t* pa, USR_DOUBLE *die_temp) {
#ifdef SERDES_API_FLOATING_POINT
    /* Formula for PVTMON: T = 410.04-0.48705*reg10bit (from PVTMON Analog Module Specification v5.0 section 6.2) */
    uint16_t die_temp_sensor_reading;
    ...
    *die_temp = 410.04-0.48705* ((USR_DOUBLE)die_temp_sensor_reading);
    ...
    return(ERR_CODE_NONE);
#else
    EFUN_PRINTF((" Function 'merlin_dino_read_die_temperature_double' needs SERDES_API_FLOATING_POINT defined to operate \n"));
    return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
#endif
}
