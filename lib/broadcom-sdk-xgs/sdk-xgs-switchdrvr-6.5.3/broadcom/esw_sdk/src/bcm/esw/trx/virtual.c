/*
 * $Id: virtual.c,v 1.69 Broadcom SDK $
        ...
        *base_vp = i;
        for (j = 0; j < count; j++) {
            ...
            if (vp_info.flags & _BCM_VP_INFO_NETWORK_PORT) {
                _BCM_NETWORK_VP_USED_SET(unit, i+j);
            }
            if (vp_info.flags & _BCM_VP_INFO_SHARED_PORT) {
                ...
                _BCM_SHARED_VP_USED_SET(unit, i+j);
            }
        }
        VIRTUAL_UNLOCK(unit);
        return BCM_E_NONE;
                                ...
                                int vp_type, bcm_if_t intf, int network_port)
{
    ...
    if (BCM_XGS3_L3_EGRESS_IDX_VALID(unit, intf) || 
        ...
        if (BCM_XGS3_L3_EGRESS_IDX_VALID(unit, intf)) {
            nh_ecmp_index = intf - BCM_XGS3_EGRESS_IDX_MIN;
        } else {
            nh_ecmp_index = intf - BCM_XGS3_DVP_EGRESS_IDX_MIN;
        }
        ecmp = 0;
    } else if (BCM_XGS3_L3_MPATH_EGRESS_IDX_VALID(unit, intf)) {
        nh_ecmp_index = intf - BCM_XGS3_MPATH_EGRESS_IDX_MIN;
        ecmp = 1;        
...
int
_bcm_vp_ing_dvp_to_ing_dvp2(int unit, ing_dvp_table_entry_t *dvp, int vp)
{
    ...
    if (soc_feature(unit, soc_feature_multiple_split_horizon_group)) {
        ...
        nw_port = soc_ING_DVP_TABLEm_field32_get(unit, dvp, 
                NETWORK_PORTf);
