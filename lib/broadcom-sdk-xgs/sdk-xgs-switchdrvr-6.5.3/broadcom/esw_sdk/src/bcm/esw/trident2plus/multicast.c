...
#define _AP_PORTS_PIPE 75
#endif 
typedef struct _bcm_td2p_aggid_info_s {
    uint8 free_aggid[SOC_MAX_NUM_PORTS];
...
bcm_td2p_aggregation_id_list_init(int unit)
{
    ...
    if (SOC_IS_APACHE(unit)) { 
        num_pipes = _AP_NUM_PIPES; 
    }
    else 
#endif 
    { 
        num_pipes = _TD2P_NUM_PIPES; 
    ...
    td2p_free_aggid[unit] = sal_alloc((num_pipes * 
                                 sizeof(_bcm_td2p_aggid_info_t)), "free_aggid");
    if (td2p_free_aggid[unit] == NULL) {
        return BCM_E_MEMORY;
    }
    sal_memset(td2p_free_aggid[unit], 0xFF, num_pipes * 
...
bcm_td2p_aggid_trunk_map_init(int unit)
{
    ...
    td2p_trunk_aggid_info[unit] = sal_alloc((num_pipes * 
    ...
    sal_memset(td2p_trunk_aggid_info[unit], 0xFF, num_pipes * 
               sizeof (_bcm_td2p_trunk_aggid_map_t) *
               sizeof(_bcm_td2p_trunk_aggid_info_t));
...
#if defined BCM_APACHE_SUPPORT
    if (SOC_IS_APACHE(unit)) { 
        max_ports = _AP_PORTS_PIPE;
    }
    else 
#endif 
    { 
        max_ports = _TD2P_PORTS_PIPE; 
    ...
    for (ltp_idx = 0; ltp_idx < local_member_count; ltp_idx++) {
        ...
        if (pipe != prev_pipe) {
            ...
            */
            ...
            BCM_IF_ERROR_RETURN
                (set_free_aggregation_id(unit, pipe, (mmu_port % max_ports)));
        }
