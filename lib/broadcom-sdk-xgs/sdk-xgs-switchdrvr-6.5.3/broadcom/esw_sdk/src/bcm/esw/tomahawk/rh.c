 ...
 *
 * File:    rh.c
 * Purpose: Tomahawk Resilient Hashing function implementations
 */
...
typedef struct _th_ecmp_rh_info_s {
    ...
    _th_ecmp_rh_group_t *rhg;
} _th_ecmp_rh_info_t;

STATIC _th_ecmp_rh_info_t *_th_ecmp_rh_info[BCM_MAX_NUM_UNITS];

/* RH member */
typedef struct _th_ecmp_rh_member_s {
    ...
    int next_replica_id; /* Index of the next replica to be assigned to
                            ...
                            replica. */
} _th_ecmp_rh_member_t;

/*----- STATIC FUNCS ----- */
STATIC int
_bcm_th_rh_ecmp_grp_hash_calc(int unit, void *buf, uint16 *hash)
{
 ...
 */
int
bcm_th_ecmp_rh_deinit(int unit)
{
    int i;
    bcm_if_t **rh_intf_arr_ptr;
    if (_th_ecmp_rh_info[unit]) {
        if (_th_ecmp_rh_info[unit]->rhg) {
            for (i = 0; i < BCM_XGS3_L3_ECMP_MAX_GROUPS(unit); i++) {
                /* Free ECMP member array */
                rh_intf_arr_ptr = &(_th_ecmp_rh_info[unit]->rhg[i].rh_intf_arr);
                if (*rh_intf_arr_ptr) {
                    sal_free(*rh_intf_arr_ptr);
                    *rh_intf_arr_ptr = NULL;
                }
            }
            sal_free(_th_ecmp_rh_info[unit]->rhg);
        }
        sal_free(_th_ecmp_rh_info[unit]);
        _th_ecmp_rh_info[unit] = NULL;
    }
    return BCM_E_NONE;
}
 ...
 */
int
bcm_th_ecmp_rh_init(int unit)
{
    int rv = BCM_E_NONE;

    bcm_th_ecmp_rh_deinit(unit);

    _th_ecmp_rh_info[unit] = sal_alloc(sizeof(_th_ecmp_rh_info_t),
            "_th_ecmp_rh_info");
    if (_th_ecmp_rh_info[unit] == NULL) {
        return BCM_E_MEMORY;
    }
    sal_memset(_th_ecmp_rh_info[unit], 0, sizeof(_th_ecmp_rh_info_t));

    _th_ecmp_rh_info[unit]->rhg = sal_alloc(
            sizeof(_th_ecmp_rh_group_t) * BCM_XGS3_L3_ECMP_MAX_GROUPS(unit),
            "_th_ecmp_rh_info");
    if (_th_ecmp_rh_info[unit]->rhg == NULL) {
        return BCM_E_MEMORY;
    }
    sal_memset(_th_ecmp_rh_info[unit]->rhg, 0,
              sizeof(_th_ecmp_rh_group_t) * BCM_XGS3_L3_ECMP_MAX_GROUPS(unit));

    /* Set the seed for the pseudo-random number generator */
    _th_ecmp_rh_info[unit]->ecmp_rh_rand_seed = sal_time_usecs();
 ...
 */
int
bcm_th_ecmp_rh_set_intf_arr(int unit,
                            int intf_count,
                            bcm_if_t *intf_array,
                            int ecmp_group_idx,
                            int group_size,
                            int max_paths)
{
    ...
    if (BCM_XGS3_L3_MPATH_EGRESS_IDX_VALID(unit, ecmp_group_idx +
        ...
        rh_intf_arr_ptr = &(_th_ecmp_rh_info[unit]->rhg[ecmp_group_idx].rh_intf_arr);
        if (*rh_intf_arr_ptr) {
            sal_free(*rh_intf_arr_ptr);
            *rh_intf_arr_ptr = NULL;
        }
        *rh_intf_arr_ptr = sal_alloc(sizeof(bcm_if_t) * intf_count,
        ...
        _th_ecmp_rh_info[unit]->rhg[ecmp_group_idx].rh_intf_count = intf_count;
        _th_ecmp_rh_info[unit]->rhg[ecmp_group_idx].max_paths = max_paths;
        ...
        _shr_sort(hash_intf_array, intf_count, sizeof(int), _th_rh_cmp_int);
        /* Copy sorted ECMP member array */
        sal_memcpy(*rh_intf_arr_ptr, hash_intf_array, 
                            (sizeof(bcm_if_t) * intf_count));     
        /* Compute member hash */
        _bcm_th_rh_ecmp_grp_hash_calc(unit, hash_intf_array, &hash);
        _th_ecmp_rh_info[unit]->rhg[ecmp_group_idx].data_hash = hash;
...
bcm_th_ecmp_group_rh_set(int unit, int ecmp_group_idx, int enable)
{
    ...
    _th_ecmp_rh_info[unit]->rhg[ecmp_group_idx].enable = enable ? 1 : 0;
    return;
}

/*
 * Function:
 *     bcm_th_ecmp_group_is_rh
 * Purpose:
 ...
 */
int
bcm_th_ecmp_group_is_rh(int unit, int ecmp_group_idx)
{
    ...
    /* coverity[overrun-local : FALSE] */
    return _th_ecmp_rh_info[unit]->rhg[ecmp_group_idx].enable ? TRUE : FALSE;
}

/*
 * Function:
 *     bcm_th_ecmp_rh_multipath_get
 * Purpose:
 ...
 */
int
bcm_th_ecmp_rh_multipath_get(int unit, int ecmp_group_idx, int intf_size,
                             bcm_if_t *intf_arr, int *intf_count, int *max_paths)
{
    ...
    intf_arr_ptr = _th_ecmp_rh_info[unit]->rhg[ecmp_group_idx].rh_intf_arr;
    if (intf_arr_ptr) {
        if (intf_count) {
            *intf_count = _th_ecmp_rh_info[unit]->rhg[ecmp_group_idx].rh_intf_count;
        }
        if (max_paths) {
            *max_paths = _th_ecmp_rh_info[unit]->rhg[ecmp_group_idx].max_paths;
        }
        if (intf_size == 0) {
            return (BCM_E_NONE);
 ...
 */
STATIC int
_bcm_th_ecmp_rh_free_resource(int unit, int ecmp_group_idx)
{
    ...
    if (BCM_ECMP_LB_MODE_RH != soc_L3_ECMP_COUNTm_field32_get(unit,
    ...
    soc_L3_ECMP_COUNTm_field32_set(unit, &ecmp_count_entry,
            LB_MODEf, 0);
    BCM_IF_ERROR_RETURN(WRITE_L3_ECMP_COUNTm(unit, MEM_BLOCK_ALL,
                ecmp_group_idx, &ecmp_count_entry));
    ...
    _th_ecmp_rh_info[unit]->rhg[ecmp_group_idx].data_hash = 0;
    _th_ecmp_rh_info[unit]->rhg[ecmp_group_idx].rh_intf_count = 0;
 ...
 */
STATIC int
_bcm_th_ecmp_rh_rand_get(int unit, int rand_max, int *rand_num)
{
    int modulus;
    int rand_seed_shift;
    ...
    _th_ecmp_rh_info[unit]->ecmp_rh_rand_seed =
        _th_ecmp_rh_info[unit]->ecmp_rh_rand_seed * 1103515245 + 12345;

    *rand_num = (_th_ecmp_rh_info[unit]->ecmp_rh_rand_seed >> rand_seed_shift) %
                modulus;
 ...
 */
int
bcm_th_l3_egress_rh_ecmp_find(int unit, int intf_count,
                              bcm_if_t *intf_array, bcm_if_t *mpintf)
{
    ...
    if (_th_ecmp_rh_info[unit]) {
        for (i = 0; i < BCM_XGS3_L3_ECMP_MAX_GROUPS(unit); i++) {
            if (_th_ecmp_rh_info[unit]->rhg[i].rh_intf_arr) {
                if ((hash == _th_ecmp_rh_info[unit]->rhg[i].data_hash) &&
                     (intf_count == _th_ecmp_rh_info[unit]->rhg[i].rh_intf_count)) {
                    /* Compare ecmp groups. */
                    if (sal_memcmp(_th_ecmp_rh_info[unit]->rhg[i].rh_intf_arr,
                                   hash_intf_array, (intf_count * sizeof(int))) == 0) {
                        *mpintf = i + BCM_XGS3_MPATH_EGRESS_IDX_MIN;
                        break;
                    }
                }
            }
 ...
 */
STATIC int
_bcm_th_ecmp_rh_member_choose(int unit, int num_members, int *entry_count_arr,
                              int *max_entry_count, int *chosen_index)
{
    ...
    *chosen_index = 0;
    ...
    BCM_IF_ERROR_RETURN(_bcm_th_ecmp_rh_rand_get(unit, num_members - 1,
                &member_index));
        ...
        if (next_index == member_index) {
            ...
            if (entry_count_arr[member_index] < *max_entry_count) {
                ...
                *chosen_index = member_index;
                 ...
                 * _bcm_th_ecmp_rh_populate_empty_entries.
                 */
                next_index = (member_index + 1) % num_members;
                while (next_index != member_index) {
                    if (entry_count_arr[next_index] < *max_entry_count) {
...
/*
 * Function:
 *      _bcm_th_ecmp_rh_member_replica_find
 * Purpose:
 ...
 */
STATIC int
_bcm_th_ecmp_rh_member_replica_find(int unit, int num_members,
                                    _th_ecmp_rh_member_t *member_array)
{
    int i, k;
 ...
 */
STATIC int
_bcm_th_ecmp_rh_add_rebalance(int unit,
                              int num_entries, bcm_if_t *rh_intf_array,
                              int *member_id_buf, int num_members,
                              _th_ecmp_rh_member_t *member_array,
                              int *entry_count_array,
                              _th_ecmp_rh_member_t *new_member,
                              int *entry_count)
{
    ...
    *entry_count = 0;
    while (*entry_count < lower_bound) {
        ...
        BCM_IF_ERROR_RETURN
            (_bcm_th_ecmp_rh_rand_get(unit, num_entries - 1, &entry_index));
        ...
        if (!is_new_member && (entry_count_array[member_index] > threshold)) {
            rh_intf_array[entry_index] = new_member->nh_index + BCM_XGS3_EGRESS_IDX_MIN;
...
/*
 * Function:
 *      _bcm_th_ecmp_rh_member_id_buf_assign
 * Purpose:
 ...
 */
STATIC int
_bcm_th_ecmp_rh_member_id_buf_assign(int unit, int num_members,
                                     _th_ecmp_rh_member_t *member_array,
                                     int num_entries,
                    ...
                    bcm_if_t *new_intf_array)
{
    ...
    for (i = 0; i < ecmp->dynamic_size; i++) {
        ...
        rv = _bcm_th_ecmp_rh_member_choose(unit, intf_count,
                    ...
                    bcm_if_t *rh_intf_array)
{
    ...
    int alloc_size;
    int num_entries;
    _th_ecmp_rh_member_t *existing_member_arr = NULL;
    int i;
    ...
    if (intf_count == 1) {
        ...
        return _bcm_th_ecmp_rh_set(unit, ecmp, intf_count, intf_array,
                                   rh_intf_array);
    }
    num_entries = ecmp->dynamic_size; 
    /* Construct an array of existing members */
    alloc_size = num_existing_members * sizeof(_th_ecmp_rh_member_t);
     ...
     */
    rv = _bcm_th_ecmp_rh_member_replica_find(unit, num_existing_members,
            existing_member_arr);
    if (BCM_FAILURE(rv)) {
        goto cleanup;
    }
    ...
    rv = _bcm_th_ecmp_rh_member_id_buf_assign(unit, num_existing_members,
    ...
    /* Re-balance rh set entries from existing members to the new member */
    sal_memset(&new_member, 0, sizeof(_th_ecmp_rh_member_t));
    new_member.nh_index = new_next_hop_index;
    new_member.member_id = num_existing_members;
    rv = _bcm_th_ecmp_rh_add_rebalance(unit, num_entries, rh_intf_array,
 ...
 */
STATIC int
_bcm_th_ecmp_rh_delete(int unit,
                       bcm_l3_egress_ecmp_t *ecmp,
                       int intf_count,
                       bcm_if_t *intf_array,
                       ...
                       bcm_if_t *rh_intf_array)
{
    ...
    if (intf_count == 0) {
        ...
        BCM_IF_ERROR_RETURN(_bcm_th_ecmp_rh_free_resource(unit, ecmp_group));
    ...
    for (i = 0; i < num_entries; i++) {
        ...
        rv = _bcm_th_ecmp_rh_member_choose(unit, num_remaining_members,
...
/*
 * Function:
 *      _bcm_th_ecmp_rh_clear_by_member_id_th
 * Purpose:
 ...
 */
STATIC int
_bcm_th_ecmp_rh_clear_by_member_id_th(int unit,
 ...
 */
STATIC int
_bcm_th_ecmp_rh_populate_empty_entries(int unit,
                                       int num_entries,
                                       bcm_if_t *rh_intf_array,
                                       int *member_id_buf,
                                       int num_members,
    ...
    for (i = 0; i < num_entries; i++) {
       if (member_id_buf[i] != -1) {
        ...
        if (k == num_shared_members) {
             ...
             */
            BCM_IF_ERROR_RETURN(_bcm_th_ecmp_rh_member_choose(unit, num_members,
                        entry_count_array, &max_entry_count, &chosen_index));
        }
        /* Set rh set entry */
        member_id_buf[i] = member_array[chosen_index].member_id;
        rh_intf_array[i] = member_array[chosen_index].nh_index + BCM_XGS3_EGRESS_IDX_MIN;
    }
...
/*
 * Function:
 *      _bcm_th_ecmp_rh_balance_with_min_shared_mod
 * Purpose:
 ...
 */
STATIC int
_bcm_th_ecmp_rh_balance_with_min_shared_mod(int unit,
                                            int num_entries,
                                            bcm_if_t *rh_intf_array,
                                            ...
                                            bcm_if_t *ex_new_intf_array)
{
    ...
    int num_members;
    int alloc_size;
    _th_ecmp_rh_member_t *member_array = NULL;
    ...
    alloc_size = num_members * sizeof(_th_ecmp_rh_member_t);
     ...
     */
    rv = _bcm_th_ecmp_rh_member_replica_find(unit, old_intf_count,
            member_array);
    if (BCM_FAILURE(rv)) {
        goto cleanup;
    }
    ...
    sal_memset(member_id_buf, 0, alloc_size);
    rv = _bcm_th_ecmp_rh_member_id_buf_assign(unit, old_intf_count,
     ...
     */
    rv = _bcm_th_ecmp_rh_clear_by_member_id_th(unit, num_entries, rh_intf_array,
     ...
     */
    for (i = shared_intf_count; i < old_intf_count; i++) {
        sal_memset(&member_array[i], 0, sizeof(_th_ecmp_rh_member_t));
    }
    for (i = 0; i < shared_intf_count; i++) {
        member_array[i].num_replica = 1;
        member_array[i].replica_id = 0;
     ...
     * update the replica information in member_array.
     */
    rv = _bcm_th_ecmp_rh_member_replica_find(unit, new_intf_count,
            member_array);
    if (BCM_FAILURE(rv)) {
        goto cleanup;
    }
    ...
    rv = _bcm_th_ecmp_rh_populate_empty_entries(unit, num_entries, rh_intf_array,
...
/*
 * Function:
 *      _bcm_th_ecmp_rh_arrange_with_no_shared_entries
 * Purpose:
 ...
 */
STATIC int
_bcm_th_ecmp_rh_arrange_with_no_shared_entries(int unit,
                                               int num_entries,
                                               bcm_if_t *rh_intf_array,
                                               int old_intf_count,
                                               ...
                                               bcm_if_t *ex_new_intf_array)
{
    ...
    sal_memset(member_id_buf, 0, alloc_size);
    rv = _bcm_th_ecmp_rh_member_id_buf_assign(unit, num_members,
            member_array, num_entries, rh_intf_array, member_id_buf);
    if (BCM_FAILURE(rv)) {
        goto cleanup;
    }
    ...
    for (i = 0; i < num_entries; i++) {
        ...
        rh_intf_array[i] = new_intf_array[member_id] + BCM_XGS3_EGRESS_IDX_MIN;
    }
 ...
 */
STATIC int
_bcm_th_ecmp_rh_replace(int unit,
                        bcm_l3_egress_ecmp_t *ecmp,
                        int intf_count,
                        bcm_if_t *intf_array,
                        int old_intf_count,
                        ...
                        bcm_if_t *rh_intf_array)
{
    ...
    if (old_intf_count == 0) {
        ...
        return _bcm_th_ecmp_rh_set(unit, ecmp, intf_count, intf_array, rh_intf_array);
    }
    ...
    if (shared_intf_count == 0) {
         ...
         */
        rv = _bcm_th_ecmp_rh_arrange_with_no_shared_entries(unit,
     ...
     * shared members to a minimum.
     */
    rv = _bcm_th_ecmp_rh_balance_with_min_shared_mod(unit,
            num_entries, rh_intf_array,
            shared_intf_count, shared_intf_array,
            ex_old_intf_count, ex_old_intf_array,
            ex_new_intf_count, ex_new_intf_array);
...
/*
 * Function:
 *      bcm_th_l3_egress_ecmp_rh_create 
 * Purpose:
 ...
 */
int 
bcm_th_l3_egress_ecmp_rh_create(int unit, bcm_l3_egress_ecmp_t *ecmp,
                                int intf_count,
                                ...
                                bcm_if_t *rh_intf_array)
{
    ...
    if (op == BCM_L3_ECMP_MEMBER_OP_SET) {
        ...
        if (rh_enable) {
            ...
            BCM_IF_ERROR_RETURN
                (_bcm_th_ecmp_rh_set(unit, ecmp, intf_count, intf_array, rh_intf_array));
        }
    } else if (op == BCM_L3_ECMP_MEMBER_OP_ADD) {
        if (rh_enable) {
            /* Add new resilient hashing member to ecmp group */
            BCM_IF_ERROR_RETURN
                (_bcm_th_ecmp_rh_add(unit, ecmp, intf_count, intf_array, *intf,
                         rh_intf_array));
        }
        ...
        if (rh_enable) {
            ...
            BCM_IF_ERROR_RETURN
                (_bcm_th_ecmp_rh_delete(unit, ecmp, intf_count, intf_array, 
                             *intf, rh_intf_array));
        }
        ...
        if (rh_enable) {
            ...
            BCM_IF_ERROR_RETURN
                (_bcm_th_ecmp_rh_replace(unit, ecmp, intf_count, intf_array, 
...
/*
 * Function:
 *      bcm_th_l3_egress_ecmp_rh_destroy
 * Purpose:
 ...
 */
int 
bcm_th_l3_egress_ecmp_rh_destroy(int unit, bcm_if_t mpintf) 
{
    int ecmp_group;
 ...
 */
int
bcm_th_l3_egress_ecmp_lb_get(int unit, bcm_l3_egress_ecmp_t *ecmp)
{
    int ecmp_group;
    int lb_mode;
    ecmp_count_entry_t ecmp_count_entry;
    ...
    if (lb_mode == BCM_ECMP_LB_MODE_RH) {
        ecmp->dynamic_mode = BCM_L3_ECMP_DYNAMIC_MODE_RESILIENT;
        count = soc_L3_ECMP_COUNTm_field32_get(unit,  
    ...
    } else if (lb_mode == BCM_ECMP_LB_MODE_RANDOM) {
        ecmp->dynamic_mode = BCM_L3_ECMP_DYNAMIC_MODE_RANDOM;
    } else if (lb_mode == BCM_ECMP_LB_MODE_RR) {
 ...
 */
int
bcm_th_ecmp_lb_mode_set(int unit,
                        int ecmp_group_idx,
                        uint8 lb_mode)
{
    ecmp_count_entry_t ecmp_count_entry;
    ...
    BCM_IF_ERROR_RETURN(READ_L3_ECMP_COUNTm(unit, MEM_BLOCK_ANY, ecmp_group_idx,
                &ecmp_count_entry));
    soc_L3_ECMP_COUNTm_field32_set(unit, &ecmp_count_entry,
            LB_MODEf, lb_mode);
    BCM_IF_ERROR_RETURN(WRITE_L3_ECMP_COUNTm(unit, MEM_BLOCK_ALL, ecmp_group_idx,
                &ecmp_count_entry));
    /* Reset round robin counter for the group */
    if (lb_mode == BCM_ECMP_LB_MODE_RR) {
 ...
 */
int
bcm_th_ecmp_lb_mode_reset(int unit,
                        bcm_if_t mpintf)
{
    ecmp_count_entry_t ecmp_count_entry;
    int ecmp_group_idx;
...
/*
 * Function:
 *      bcm_th_l3_egress_ecmp_rh_shared_copy
 * Purpose:
 ...
 */
int
bcm_th_l3_egress_ecmp_rh_shared_copy(int unit, int old_ecmp_group,
        int new_ecmp_group)
{
    int rv = BCM_E_NONE;
    ...
    rv = bcm_xgs3_l3_egress_multipath_get(unit,
                                          old_ecmp_group + BCM_XGS3_MPATH_EGRESS_IDX_MIN,
    ...
    replace_ecmp.ecmp_intf = new_ecmp_group + BCM_XGS3_MPATH_EGRESS_IDX_MIN;
    ...
    BCM_XGS3_L3_ECMP_GROUP_FLAGS_RESET(unit,
                                    (replace_ecmp.ecmp_intf - BCM_XGS3_MPATH_EGRESS_IDX_MIN),
                                    BCM_L3_ECMP_PATH_NO_SORTING);

    /* set back lb mode. no need to set intf array, since they are same */
    rv = bcm_th_ecmp_lb_mode_set(unit, new_ecmp_group, 
                                 BCM_ECMP_LB_MODE_RH);
 ...
 */
int
bcm_th_l3_ecmp_rh_member_sync(int unit, uint8 **scache_ptr)
{
    int16 i;
    int j;
    bcm_if_t *rh_intf_arr_ptr = NULL;
    ...
    for (i = 0; i < BCM_XGS3_L3_ECMP_MAX_GROUPS(unit); i++) {
        ...
        if (rh_intf_arr_ptr) {
            sal_memcpy((*scache_ptr),                     
                       &(_th_ecmp_rh_info[unit]->rhg[i].max_paths),
                       sizeof(uint16));
        }
...
/*
 * Function:
 *      bcm_th_l3_ecmp_rh_member_recover
 * Purpose:
 ...
 */
int
bcm_th_l3_ecmp_rh_member_recover(int unit, uint8 **scache_ptr, int ecmp_max_paths)
{
    bcm_if_t **rh_intf_arr_ptr;
    int stable_size = 0;
    int intf_count;
    ...
    if (SOC_WARM_BOOT(unit)) {
        ...
        if (stable_size == 0) { /* level 1 */
            ...
            for (j = 0; j < BCM_XGS3_L3_ECMP_MAX_GROUPS(unit); j++) {
                sal_memcpy(&(_th_ecmp_rh_info[unit]->rhg[j].max_paths),
                           (*scache_ptr),
                           sizeof(uint16));
                (*scache_ptr) += sizeof(uint16);
            }
            ...
            while ((*scache_ptr) < end_scache_ptr) {
                ...
                if (ecmp_group_index != 0) {
                    ...
                    ecmp_group_index--;
                    _th_ecmp_rh_info[unit]->rhg[ecmp_group_index].rh_intf_count =
                               intf_count; 
                    rh_intf_arr_ptr =
                         &(_th_ecmp_rh_info[unit]->rhg[ecmp_group_index].rh_intf_arr);
                    *rh_intf_arr_ptr = sal_alloc(sizeof(bcm_if_t) * intf_count,
                                              "ECMP RH entry count array");
                    if (NULL == *rh_intf_arr_ptr) {
                        return(BCM_E_MEMORY);
                    ...
                    if (!(BCM_XGS3_L3_EGRESS_IDX_VALID(unit, *(*rh_intf_arr_ptr)))
                        ...
                        *(*rh_intf_arr_ptr)))) {
                        _th_ecmp_rh_info[unit]->rhg[ecmp_group_index].
                                rh_intf_count = 0;
                        intf_count = 0;
                    }
                    ...
                    /* Compute member hash */
                    _bcm_th_rh_ecmp_grp_hash_calc(unit, hash_intf_array, &hash);
                    _th_ecmp_rh_info[unit]->rhg[ecmp_group_index].data_hash = hash;
                    sal_free(hash_intf_array);
                    hash_intf_array = NULL;

                    BCM_IF_ERROR_RETURN(READ_L3_ECMP_COUNTm(unit, MEM_BLOCK_ANY,
                        ecmp_group_index, &entry));
                    if (BCM_ECMP_LB_MODE_RH ==
 ...
 *     None
 */
void bcm_th_ecmp_rh_sw_dump(int unit)
{
    int i, j;
    bcm_if_t *rh_intf_arr_ptr;
        ...
        for (i = 0; i < BCM_XGS3_L3_ECMP_MAX_GROUPS(unit); i++) {
             ...
             if (rh_intf_arr_ptr) {
                 LOG_CLI((BSL_META_U(unit,
                                     ...
                                     i, _th_ecmp_rh_info[unit]->rhg[i].rh_intf_count,
                                     _th_ecmp_rh_info[unit]->rhg[i].max_paths));
                 for (j = 0; j < _th_ecmp_rh_info[unit]->rhg[i].rh_intf_count; j++)
                     {
                         LOG_CLI((BSL_META_U(unit,
