...
bcm_td_port_niv_type_set(int unit, bcm_port_t port, int value)
{
    ...
    BCM_IF_ERROR_RETURN(
        _bcm_esw_egr_port_tab_set(unit, port, VNTAG_ACTIONS_IF_PRESENTf,
                                  VNTAG_NOOP));
    BCM_IF_ERROR_RETURN(
        _bcm_esw_egr_port_tab_set(unit, port, NIV_PRUNE_ENABLEf, 0));
    BCM_IF_ERROR_RETURN(
        _bcm_esw_egr_port_tab_set(unit, port, NIV_UPLINK_PORTf, 0));
    ...
    switch (value) {
        ...
        case BCM_PORT_NIV_TYPE_DOWNLINK_ACCESS:
            ...
            BCM_IF_ERROR_RETURN(
                ...
                unit, port, VNTAG_ACTIONS_IF_PRESENTf, VNTAG_DELETE));
            BCM_IF_ERROR_RETURN(
                _bcm_esw_egr_port_tab_set(unit, port, NIV_PRUNE_ENABLEf, 1));
            break;
...
bcm_td_port_niv_type_get(int unit, bcm_port_t port, int *value)
{
    ...
    BCM_IF_ERROR_RETURN(_bcm_esw_egr_port_tab_get(unit, port,NIV_PRUNE_ENABLEf,
                                                  &prune_enable));
                                 ...
                                 int *count)
{
    ...
    for (index = index_min; index <= index_max; index++) {
        vent = soc_mem_table_idx_to_pointer(unit, VLAN_XLATEm,
                                            vlan_xlate_entry_t *,
                                            vt_buf, index);
                ...
                case VLXLT_HASH_KEY_TYPE_VIF_VLAN:
                    if (match_array->match_vlan == BCM_VLAN_INVALID) {
                        match_array->match_vlan =
                            soc_VLAN_XLATEm_field32_get(unit, vent, VIF__VLANf);
                    }
