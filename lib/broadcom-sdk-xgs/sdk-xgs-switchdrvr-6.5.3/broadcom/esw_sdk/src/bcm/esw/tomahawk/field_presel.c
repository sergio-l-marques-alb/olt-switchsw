...
_field_th_presel_qualifiers_init(int unit, _field_stage_t *stage_fc)
{
    ...
    _FP_PRESEL_QUAL_ADD(unit, stage_fc, bcmFieldQualifyMplsLabel2Hit,
        0, _bcmFieldSliceSelDisable, 0, _bcmFieldQualifierTypePresel,
        59+17 - key_base_off - em_base_shift, 1);
    ...
    _FP_PRESEL_QUAL_ADD(unit, stage_fc, bcmFieldQualifyL4Ports,
        0, _bcmFieldSliceSelDisable, 0, _bcmFieldQualifierTypePresel,
        85 - key_base_off - em_base_shift, 1);
    /* L3_TYPE - 5 */
    _FP_PRESEL_QUAL_ADD(unit, stage_fc, bcmFieldQualifyIpType,
        0, _bcmFieldSliceSelDisable, 0, _bcmFieldQualifierTypePresel,
        86 - key_base_off - em_base_shift, 5);
...
_field_th_lt_config_init(int unit, _field_control_t *fc)
{
    ...
    _field_lt_config_t *lt_p;   /* Field LT configuration pointer. */
    ...
    for (inst = 0; inst < _FP_MAX_NUM_PIPES; inst++) {
        for (lt_idx = 0; lt_idx < _FP_MAX_NUM_LT; lt_idx++) {
            ...
            /* Initialize allocated LT memory. */
            sal_memset(lt_p, 0, sizeof(_field_lt_config_t));
                                  ...
                                  int inst)
{
    ...
    for (slice = 0; slice < stage_fc->tcam_slices; slice++) {
        ...
        lt_fs[slice].slice_flags = _BCM_FIELD_SLICE_SW_ATOMICITY_SUPPORT;
    }
                              ...
                              int qid, _bcm_field_qual_offset_t *q_offset)
{
    ...
    if ((NULL != stage_fc->f_presel_qual_arr[qid]) && 
        (NULL != stage_fc->f_presel_qual_arr[qid]->conf_arr)) {
        sal_memcpy(q_offset,
                   &stage_fc->f_presel_qual_arr[qid]->conf_arr->offset,
...
_bcm_field_presel_qualifiers_dump(int unit, bcm_field_presel_t presel_id)
{
    ...
    for (qual_id = 0; qual_id < _bcmFieldQualifyCount; qual_id++) {
        ...
        LOG_CLI((BSL_META_U(unit, "\tOffset: %-2d Width: %-2d\n\r"),
                            q_offset.offset[0], q_offset.width[0]));
                                   ...
                                   _field_presel_entry_t *f_presel)
{
    ...
    BCM_IF_ERROR_RETURN
         (_bcm_field_th_entry_tcam_parts_count(unit, fg->flags, &parts));
 ...
 *     BCM_E_XXX
 *
 * Note: TCAM Index has to be validated by the caller function.
 */
int
                               ...
                               _field_lt_slice_t *lt_fs)
{
    ...
    valid = (f_presel->group->flags & _FP_GROUP_LOOKUP_ENABLED) ? 1 : 0;
    ...
    bcm_field_presel_set_t *presel)
{
    ...
    /* Verify if any entry is associated with the group */
...
/*
 * Function:
 *     _field_th_ingress_lt_action_prio_install
 * Purpose:
 ...
 */
STATIC int
_field_th_ingress_lt_action_prio_install(int unit, _field_stage_t *stage_fc,
                                         _field_group_t *fg)
{
    ...
    static soc_mem_t lt_action_mem[_FP_MAX_NUM_PIPES] = {
    ...
    if (stage_fc->oper_mode == bcmFieldGroupOperModeGlobal) {
        for (inst = 0 ; inst < _FP_MAX_NUM_PIPES; inst++) {
            ...
            for (lt_idx = 0; lt_idx < _FP_MAX_NUM_LT; lt_idx++) {
                ...
                soc_mem_field32_set(unit, lt_action_mem[inst], &entry,
                        field, value);
            }

            /* Write LT action priority entry in hardware. */
            BCM_IF_ERROR_RETURN(soc_mem_write(unit, lt_action_mem[inst],
                        MEM_BLOCK_ANY, 0, &entry));
        }
    } else {
        /* Read logical table action priority entry from hardware. */
        BCM_IF_ERROR_RETURN(soc_mem_read(unit, lt_action_mem[fg->instance],
                    MEM_BLOCK_ANY, 0, &entry));
        ...
        for (lt_idx = 0; lt_idx < _FP_MAX_NUM_LT; lt_idx++) {
            ...
            /* Set the value in entry buffer that's to be programmed in HW. */
            soc_mem_field32_set(unit, lt_action_mem[fg->instance], &entry,
                    field, value);
        }

        /* Write LT action priority entry in hardware. */
        BCM_IF_ERROR_RETURN(soc_mem_write(unit, lt_action_mem[fg->instance],
                    MEM_BLOCK_ANY, 0, &entry));
    }
                                              ...
                                              int slice)
{
    ...
    static soc_reg_t lt_config_reg[_FP_MAX_NUM_PIPES] = {
        IFP_LOGICAL_TABLE_CONFIG_PIPE0r,
        IFP_LOGICAL_TABLE_CONFIG_PIPE1r,
        IFP_LOGICAL_TABLE_CONFIG_PIPE2r,
        IFP_LOGICAL_TABLE_CONFIG_PIPE3r,
    };
    ...
    if (stage_fc->oper_mode == bcmFieldGroupOperModeGlobal) {
        for (inst = 0 ; inst < _FP_MAX_NUM_PIPES; inst++) {
            rv = soc_reg_get(unit, lt_config_reg[inst], REG_PORT_ANY,
                    lt_id, &lt_config_entry);
            BCM_IF_ERROR_RETURN(rv);

            /* Get logical table info for the default FP instance. */
            lt_info = fc->lt_info[inst][lt_id];
            for (slice_num = 0; slice_num < stage_fc->tcam_slices; slice_num++) {
                ...
                soc_reg64_field32_set(unit, lt_config_reg[inst],
                        &lt_config_entry, field, value);
            }
            soc_reg64_field32_set(unit, lt_config_reg[inst], &lt_config_entry,
                    LOGICAL_PARTITION_MAPf, lt_info->lt_part_map);

            /* Write new LT configuration in hardware. */
            BCM_IF_ERROR_RETURN(soc_reg_set(unit, lt_config_reg[inst],
                        REG_PORT_ANY, lt_id, lt_config_entry));
        }
    } else {
        rv = soc_reg_get(unit, lt_config_reg[fg->instance], REG_PORT_ANY,
                lt_id, &lt_config_entry);
        BCM_IF_ERROR_RETURN(rv);
        ...
        for (slice_num = 0; slice_num < stage_fc->tcam_slices; slice_num++) {
            ...
            field = lt_part_pri[slice_num];
            value = lt_info->lt_part_pri[slice_num];
            soc_reg64_field32_set(unit, lt_config_reg[fg->instance],
                    &lt_config_entry, field, value);
        }
        soc_reg64_field32_set(unit, lt_config_reg[fg->instance], &lt_config_entry,
                LOGICAL_PARTITION_MAPf, lt_info->lt_part_map);

        /* Write new LT configuration in hardware. */
        BCM_IF_ERROR_RETURN(soc_reg_set(unit, lt_config_reg[fg->instance],
                    REG_PORT_ANY, lt_id, lt_config_entry));
    }
    return BCM_E_NONE;
}
                                              ...
                                              _field_group_t *fg)
{
    ...
    rv = _field_th_ingress_lt_action_prio_install(unit, stage_fc, fg);
    if (BCM_FAILURE(rv)) {
       LOG_ERROR(BSL_LS_BCM_FP, (BSL_META_U(unit,
...
/*
 * Function:
 *     _bcm_field_th_ingress_lt_part_prio_reset
 * Purpose:
 *     Reset the LT Partition Priority for the given slice.
 * Parameters:
 *     unit     - (IN) BCM device number.
 *     stage_fc - (IN) Field stage control structure.
 *     fg       - (IN) Field group Structure.
    ...
    */
    for (fs_ptr = fg->slices; fs_ptr != NULL; fs_ptr = fs_ptr->next) {
        ...
        if (found == 1) {
            if (stage_fc->oper_mode == bcmFieldGroupOperModeGlobal) {
                for (inst = 0 ; inst < _FP_MAX_NUM_PIPES; inst++) {
                    fc->lt_info[inst][fg->lt_id]->lt_part_pri[fs_ptr->slice_number] += 1;
                }
            } else {
                                         ...
                                         int *lt_part_prio)
{
    ...
    for (group_fs = fg->slices; group_fs != NULL; group_fs = group_fs->next) {
        if (stage_fc->oper_mode == bcmFieldGroupOperModeGlobal) {
            part_prio = fc->lt_info[0][fg->lt_id]->lt_part_pri[group_fs->slice_number];
        } else {
                                    ...
                                    bcm_field_src_class_mode_t mode)
{
    ...
    if (group_mode == bcmFieldGroupOperModeGlobal) {
        ...
        if (!BCM_PBMP_EQ(port_config.all, pbmp)) {
            return BCM_E_PARAM;
        }
        ...
        for (pipe_idx = 0; pipe_idx < BCM_PIPES_MAX ; pipe_idx++) {
            ...
            BCM_PBMP_ASSIGN(pbmp_temp, port_config.per_pipe[pipe_idx]);
    ...
    if(mode != stage_fc->field_src_class_mode[pipe_idx]) {
        ...
        /* Write to HW */
        if (SOC_REG_FIELD_VALID(unit, lt_select_config,
                    SOURCE_CLASS_MODEf)) {
            BCM_IF_ERROR_RETURN(soc_reg_field32_modify(unit, lt_select_config,
                        REG_PORT_ANY,
                        SOURCE_CLASS_MODEf,
                        mode));
        } else {
                                      ...
                                      bcm_field_src_class_t mask)
{
     ...
     * for Ingress and extact match stages */
