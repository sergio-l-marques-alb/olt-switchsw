...
#define PORT_IS_PM4x25_PORT(unit, port) \
            (((SOC_INFO(unit).port_l2p_mapping[port] <= 36)  &&  \
              (SOC_INFO(unit).port_l2p_mapping[port] >= 29)) || \
             ((SOC_INFO(unit).port_l2p_mapping[port] <= 72) &&  \
              (SOC_INFO(unit).port_l2p_mapping[port] >= 65)))
...
STATIC int
_bcm_apache_port_mixed_sister_encap_check(int unit, bcm_port_t port, int mode);
...
static bcm_esw_port_drv_t bcm_apache_port_drv = {
    ...
    /* mixed_sister_encap_check   */ _bcm_apache_port_mixed_sister_encap_check,
...
bcmi_apache_port_fn_drv_init(int unit)
{
    ...
    BCM_IF_ERROR_RETURN
        (bcmi_xgs5_port_fn_drv_init(unit, &bcm_apache_port_drv,
...
_bcm_apache_port_attach(int unit, bcm_port_t port)
{
    ...
    BCM_IF_ERROR_RETURN(
    bcm_ap_cosq_flex_port_update( unit , port , TRUE));
                                    ...
                                    bcm_apache_port_resource_t *resource)
{
    ...
    for (i = 0; i < nport; i++) {
        ...
        /* Disable AN */
        BCM_IF_ERROR_RETURN(bcmi_esw_portctrl_autoneg_set(unit, resource[i].port, 0));
...
/*
 * Function:
 *      _bcm_apache_port_mixed_sister_encap_check
 * Purpose:
...
{
    BCM_IF_ERROR_RETURN
        (_bcm_apache_port_drain_cells(unit, port));
#if 0
    BCM_IF_ERROR_RETURN
        (_bcm_apache_cosq_port_queue_state_check(unit, port));
#endif
    return BCM_E_NONE;
}
