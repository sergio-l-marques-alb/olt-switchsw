...
_bcm_td2_vxlan_tunnel_initiator_reinit(int unit)
{
    ...
    for (i = idx_min; i <= idx_max; i++) {
        ...
        if (_BCM_VXLAN_TUNNEL_TYPE ==
            soc_EGR_IP_TUNNELm_field32_get(unit,
            &egr_tnl_entry, TUNNEL_TYPEf)) {

            bcm_tunnel_initiator_t_init(&tnl_init_info);
            tnl_init_info.type = bcmTunnelTypeVxlan;
            /* Get source ip */
            tnl_init_info.sip = soc_mem_field32_get(unit, EGR_IP_TUNNELm,
                &egr_tnl_entry, SIPf);            
            tnl_init_info.udp_dst_port = soc_mem_field32_get(unit,
                EGR_IP_TUNNELm, &egr_tnl_entry, L4_DEST_PORTf);
            BCM_IF_ERROR_RETURN(
                _bcm_td2_vxlan_tunnel_initiator_hash_calc(unit, &tnl_init_info,
...
bcm_td2_vxlan_vpn_create(int unit, bcm_vxlan_vpn_config_t *info)
{
    ...
    if (info->flags & BCM_VXLAN_VPN_WITH_VPNID) {
        ...
        if (soc_feature(unit, soc_feature_vrf_aware_vxlan_termination)) {
            ...
            soc_MPLS_ENTRYm_field32_set(unit, &ment, VXLAN_VN_ID__OVIDf, info->vlan);
        }
#endif
        ...
        if (info->flags & BCM_VXLAN_VPN_REPLACE) {
            ...
            sal_memset(&old_vxlate_entry, 0, sizeof(egr_vlan_xlate_entry_t));
...
#if defined(BCM_TRIDENT2PLUS_SUPPORT)
                if (soc_feature(unit, soc_feature_vrf_aware_vxlan_termination)) {
                    soc_MPLS_ENTRYm_field32_set(unit, &ment, VXLAN_VN_ID__OVIDf,
                             ...
                             void *user_data)
{
    ...
    default_vp = soc_reg_field_get(unit, VXLAN_DEFAULT_NETWORK_SVPr,
                                    reg_val, SVPf);
        ...
        if (_bcm_vp_used_get(unit, idx, _bcmVpTypeVxlan)) {
            ...
            if ((_bcm_vp_used_get(unit, idx, _bcmVpTypeNiv) ||
                ...
                if (BCM_GPORT_IS_NIV_PORT(trunk_member.gport) ||
                    ...
                    BCM_IF_ERROR_RETURN(
                        _bcm_td2_vxlan_port_get(unit, BCM_VXLAN_VPN_INVALID,
                                                idx, &info));
                }
            } else {
                ...
                if (idx == default_vp) {
                    ...
                    BCM_IF_ERROR_RETURN(_bcm_td2_vxlan_port_get(unit,
                                          BCM_VXLAN_VPN_INVALID, idx, &info));
                }
            }
            BCM_IF_ERROR_RETURN(cb(unit, &info, user_data));
        }
                                         ...
                                         vlan_xlate_entry_t   *tr_ent, uint8 out_mode, int clean_flag)
{
    ...
    if (soc_feature(unit, soc_feature_vrf_aware_vxlan_termination)) {
        soc_VLAN_XLATEm_field32_set(unit, tr_ent,
                                    VXLAN_DIP__OVIDf, tnl_info->vlan);
    }
#endif
                                         ...
                                         int multicast_flag)
{
    ...
    if (rv == SOC_E_NONE) {
        ...
        if (net_receiver != soc_VLAN_XLATEm_field32_get(
            unit, &vxlate_entry, VXLAN_DIP__NETWORK_RECEIVERS_PRESENTf)) {
            soc_VLAN_XLATEm_field32_set(unit, &vxlate_entry,  
                VXLAN_DIP__NETWORK_RECEIVERS_PRESENTf, net_receiver);
            if (net_receiver) {
                _bcm_td2_vxlan_bud_loopback_enable(unit);
            }
            BCM_IF_ERROR_RETURN(
                soc_mem_write(unit, VLAN_XLATEm, MEM_BLOCK_ALL,
                              index, &vxlate_entry));
        }
    }
...
bcm_td2_vxlan_tunnel_terminator_create(int unit, bcm_tunnel_terminator_t *tnl_info)
{
    ...
    (void) _bcm_td2_vxlan_tunnel_terminate_entry_key_set(unit, tnl_info,
        &vxlate_entry, 0, 1);
    rv = soc_mem_search(unit, VLAN_XLATEm, MEM_BLOCK_ANY, &index,
        ...
        if (vxlan_info->vxlan_tunnel_term[tunnel_idx].dip != 0) {
            ...
            if (vxlan_info->vxlan_tunnel_term[tunnel_idx].activate_flag &
                ...
                soc_VLAN_XLATEm_field32_set(
                    unit, &vxlate_entry, VXLAN_DIP__USE_OUTER_HEADER_PHBf, 0x1);
            } else if (vxlan_info->vxlan_tunnel_term[tunnel_idx].activate_flag &
                _BCM_VXLAN_TUNNEL_TERM_USE_OUTER_PCP) {
                soc_VLAN_XLATEm_field32_set(
                    unit, &vxlate_entry, VXLAN_DIP__USE_OUTER_HEADER_PHBf, 0x2);
            }
              ...
              _bcm_td2_vxlan_nh_info_t  *egr_nh_info, int new_entry_type)
{
    ...
    if ((new_entry_type == _BCM_VXLAN_EGR_NEXT_HOP_SDTAG_VIEW) &&
        (old_entry_type == _BCM_VXLAN_EGR_NEXT_HOP_L3_VIEW)) {
            ...
            soc_mem_field32_set(unit, EGR_L3_NEXT_HOPm, &egr_nh, 
                    SD_TAG__HG_HDR_SELf, hg_ppd_type);
            ...
            if (soc_mem_field_valid(unit, EGR_L3_NEXT_HOPm,
                ...
                if (!soc_mem_field32_get(unit, ING_L3_NEXT_HOPm, &ing_nh, Tf)) {
                    ...
                    if (_bcm_xgs5_subport_coe_mod_port_local(unit, modid, port)) {
                        soc_mem_field32_set(unit, EGR_L3_NEXT_HOPm, &egr_nh,
                                            SD_TAG__HG_MC_DST_PORT_NUMf, port);
                        soc_mem_field32_set(unit, EGR_L3_NEXT_HOPm, &egr_nh,
        ...
        (old_entry_type == _BCM_VXLAN_EGR_NEXT_HOP_SDTAG_VIEW)) {
            ...
            if (soc_mem_field_valid(unit, EGR_L3_NEXT_HOPm,
                                    SD_TAG__HG_MC_DST_PORT_NUMf)) {
                soc_mem_field32_set(unit, EGR_L3_NEXT_HOPm, &egr_nh,
                                    SD_TAG__HG_MC_DST_PORT_NUMf, 0);
                soc_mem_field32_set(unit, EGR_L3_NEXT_HOPm, &egr_nh,
                                    SD_TAG__HG_MC_DST_MODIDf, 0);
            }
#endif /* BCM_TRIDENT2PLUS_SUPPORT */
        ...
        if (soc_feature(unit, soc_feature_egr_l3_next_hop_next_ptr)) {
            soc_mem_field32_set(unit, EGR_L3_NEXT_HOPm, &egr_nh,
                                    L3MC__NEXT_PTR_TYPEf, 0x0);
        } else
#endif
        {
            soc_mem_field32_set(unit, EGR_L3_NEXT_HOPm, &egr_nh,
                                    L3MC__DVP_VALIDf, 0x0);
        }
...
bcm_td2_vxlan_next_hop_set(int unit, int nh_index, uint32 flags, int vp, int drop)
{
    ...
    if (flags & BCM_L3_IPMC) {
        ...
        if (vp_info.flags & _BCM_VP_INFO_SHARED_PORT &&
                vp_info.flags & _BCM_VP_INFO_NETWORK_PORT) { /* Shared network VP */
        ...
        if (soc_mem_field32_get(unit, hw_mem, &ing_nh, Tf)) {
            ...
            port = soc_mem_field32_get(unit, hw_mem, &ing_nh, PORT_NUMf);
#if defined(BCM_TRIDENT2PLUS_SUPPORT)
            if(soc_feature(unit, soc_feature_egr_modport_to_nhi)) {
                          ...
                          bcm_vxlan_port_t *vxlan_port)
{
    ...
    if (entry_type == _BCM_VXLAN_EGR_NEXT_HOP_SDTAG_VIEW) {
        ...
        if (!soc_mem_field32_get(unit, hw_mem, egr_nh, SD_TAG__HG_HDR_SELf)) {
...
int
bcm_td2_vxlan_ecmp_port_egress_nexthop_reset(int unit, int ecmp_index, int vp_type, int vp, bcm_vpn_t vpn)
{
    ...
    while(i < ecmp_member_count) {
        if (BCM_XGS3_L3_EGRESS_IDX_VALID(unit,
                    ecmp_member_array[i])) {
            index = (ecmp_member_array[i] - BCM_XGS3_EGRESS_IDX_MIN);
        } else {
            rv = BCM_E_PARAM;
            goto clean_up;
        }

        rv = bcm_td2_vxlan_port_egress_nexthop_reset(unit, index, vp_type,
                                     vp, vpn);
        if (BCM_FAILURE(rv)) {
            goto clean_up;
        }
...
_bcm_td2_vxlan_higig_ppd_select(int unit, int nh_index, uint32 flags)
{
    ...
    } else if (entry_type == _BCM_VXLAN_EGR_NEXT_HOP_SDTAG_VIEW) {
        if (soc_feature(unit, soc_feature_egr_modport_to_nhi) &&
            (flags & BCM_VXLAN_PORT_ENCAP_LOCAL)) {
            soc_mem_field32_set(unit, EGR_L3_NEXT_HOPm, &egr_nh,
    ...
    if (ecmp) {
        ...
        if (ref_count == 1) {
              if (nh_ecmp_index &&
                  ...
                  BCM_IF_ERROR_RETURN(
                   bcm_td2_vxlan_ecmp_port_egress_nexthop_reset
                                (unit, (nh_ecmp_index + BCM_XGS3_MPATH_EGRESS_IDX_MIN),
                                                                      vp_type, vp, vpn));
              }
        }
    } else {
...
#if defined(BCM_TRIDENT2PLUS_SUPPORT)
    if (soc_feature(unit, soc_feature_vrf_aware_vxlan_termination)) {
        soc_MPLS_ENTRYm_field32_set(unit, &ment, VXLAN_VN_ID__OVIDf, ovid);
    }
#endif
...
STATIC int
_bcm_td2_vxlan_match_vxlate_entry_set(int unit, bcm_vpn_t vpn, bcm_vxlan_port_t *vxlan_port, vlan_xlate_entry_t *vent)
{
    int rv = BCM_E_NONE;
    int index = -1;
    ...
    if (soc_feature(unit, soc_feature_vlan_vfi_membership)) {
        ...
        soc_VLAN_XLATEm_field32_set(unit, vent, XLATE__DISABLE_VLAN_CHECKSf, 0);
...
bcm_td2_vxlan_match_trunk_delete(int unit, bcm_trunk_t tgid, int vp)
{
    ...
    for (port_idx = 0; port_idx < local_member_count; port_idx++) {
        ...
        rv = _bcm_td2_vxlan_port_untagged_profile_reset(unit, local_port_out[port_idx]);
        if (BCM_FAILURE(rv)) {
            goto trunk_cleanup;
        }
    }
...
_bcm_td2_vxlan_match_add(int unit, bcm_vxlan_port_t *vxlan_port, int vp, bcm_vpn_t vpn)
{
    ...
    if (vxlan_port->criteria == BCM_VXLAN_PORT_MATCH_PORT_VLAN ) {
        ...
        soc_VLAN_XLATEm_field32_set(unit, &vent, XLATE__OVIDf, vxlan_port->match_vlan);
        ...
        soc_VLAN_XLATEm_field32_set(unit, &vent, XLATE__IVIDf, vxlan_port->match_inner_vlan);
        ...
        soc_VLAN_XLATEm_field32_set(unit, &vent, OTAGf, vxlan_port->match_vlan);
        ...
        if (BCM_GPORT_IS_TRUNK(vxlan_port->match_port)) {
            BCM_IF_ERROR_RETURN(bcm_td2_vxlan_match_trunk_add(unit, trunk_id, vp));
            if (vxlan_info->match_key[vp].flags !=
                     _BCM_VXLAN_PORT_MATCH_TYPE_SHARED) {
                vxlan_info->match_key[vp].flags = _BCM_VXLAN_PORT_MATCH_TYPE_TRUNK;
                vxlan_info->match_key[vp].trunk_id = trunk_id;
        ...
        if (soc_feature(unit, soc_feature_vrf_aware_vxlan_termination)) {
            ...
            vxlan_info->match_key[vp].match_vlan = vxlan_port->match_vlan;
            soc_MPLS_ENTRYm_field32_set(unit, &ment, VXLAN_SIP__OVIDf,
...
int
_bcm_td2_vxlan_match_delete(int unit,  int vp , bcm_vxlan_port_t vxlan_port)
{
    _bcm_td2_vxlan_bookkeeping_t *vxlan_info = VXLAN_INFO(unit);
    int rv=BCM_E_NONE;
    vlan_xlate_entry_t vent;
    ...
    if  (vxlan_info->match_key[vp].flags == _BCM_VXLAN_PORT_MATCH_TYPE_VLAN) {
        ...
        BCM_IF_ERROR_RETURN(_bcm_td2_vxlan_match_tunnel_entry_reset(unit, &ment));
...
STATIC int
_bcm_td2_vxlan_port_vlan_get(int unit, int vp,
                             bcm_vxlan_port_t *vxlan_port)
{
    _bcm_td2_vxlan_bookkeeping_t *vxlan_info = VXLAN_INFO(unit);
    int rv = BCM_E_NONE, index = 0;
    vlan_xlate_entry_t vent, return_vent;
    ...
    if (soc_VLAN_XLATEm_field32_get(unit, &vent, VALIDf)) {
        if (vxlan_info->match_key[vp].modid != -1) {
            soc_VLAN_XLATEm_field32_set(unit, &vent, XLATE__MODULE_IDf,
                                        vxlan_info->match_key[vp].modid);
            soc_VLAN_XLATEm_field32_set(unit, &vent, XLATE__PORT_NUMf,
...
_bcm_td2_vxlan_port_get(int unit, bcm_vpn_t vpn, int vp, bcm_vxlan_port_t  *vxlan_port)
{
    ...
    rv = _bcm_td2_vxlan_port_vlan_get(unit, vp, vxlan_port);
    BCM_IF_ERROR_RETURN(rv);
...
bcm_td2_vxlan_port_add(int unit, bcm_vpn_t vpn, bcm_vxlan_port_t  *vxlan_port)
{
    ...
    if (vxlan_port->flags & BCM_VXLAN_PORT_DEFAULT) {
        return _bcm_td2_vxlan_default_port_add(unit, vxlan_port);
    }
...
bcm_td2_vxlan_port_delete(int unit, bcm_vpn_t vpn, bcm_gport_t vxlan_port_id)
{
    ...
    if (vp == soc_reg_field_get(unit,
        ...
        return _bcm_td2_vxlan_default_port_delete(unit, vpn, vp);
    }
...
_bcm_td2_vxlan_match_vp_replace(int unit, int vp, int new_vp, int *old_vp)
{
    ...
    mpls_entry_entry_t ment, old_ment;
...
#define _BCM_VLXLT_KTYPE_ASSIGN(_ktype, _k) \
    ...
    switch(mkey->flags) {
        ...
        case _BCM_VXLAN_PORT_MATCH_TYPE_VLAN_PRI:
            ...
            *old_vp = soc_VLAN_XLATEm_field32_get(unit, &old_vent, SOURCE_VPf);
            soc_VLAN_XLATEm_field32_set(unit, &old_vent, SOURCE_VPf, new_vp);
        ...
        case _BCM_VXLAN_PORT_MATCH_TYPE_VNID:
            ...
            sal_memset(&ment, 0, sizeof(mpls_entry_entry_t));
            sal_memset(&old_ment, 0, sizeof(mpls_entry_entry_t));
            ...
            /* Replace entry's VP value */
            *old_vp = soc_MPLS_ENTRYm_field32_get(unit, &old_ment, 
                                                  VXLAN_SIP__SVPf);
            soc_MPLS_ENTRYm_field32_set(unit, &old_ment, 
                                        VXLAN_SIP__SVPf, new_vp);
            /* Insert new entry */
...
#define _BCM_VLXLT_KTYPE_ASSIGN(_ktype, _k) \
    ...
    switch(mkey->flags) {
        ...
        case _BCM_VXLAN_PORT_MATCH_TYPE_VLAN_PRI:
            ...
            *vp_lag_vp = soc_VLAN_XLATEm_field32_get(unit, &old_vent, 
                                                     SOURCE_VPf);
            break;
        case _BCM_VXLAN_PORT_MATCH_TYPE_PORT:
            mem = SOURCE_TRUNK_MAP_TABLEm;
        ...
        case _BCM_VXLAN_PORT_MATCH_TYPE_VNID:
...
#endif
            soc_MPLS_ENTRYm_field32_set(unit, &ment, KEY_TYPEf,
                                        _BCM_VXLAN_KEY_TYPE_TUNNEL);
            soc_mem_lock(unit, mem);
            rv = soc_mem_search(unit, mem, MEM_BLOCK_ANY, 
                                &entry_index, &ment, &old_ment, 0);
            soc_mem_unlock(unit, mem);
            ...
            /* Get VP value representing VP LAG */
            *vp_lag_vp = soc_MPLS_ENTRYm_field32_get(unit, &old_ment, 
                                                     VXLAN_SIP__SVPf);
            break;
        default:
            return BCM_E_PARAM;
