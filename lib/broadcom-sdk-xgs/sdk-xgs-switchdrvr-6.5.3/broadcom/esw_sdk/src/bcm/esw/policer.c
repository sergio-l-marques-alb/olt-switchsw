...
/* If MACRO Meter is present in given pool - applicable to MEF10.3+ devices */
static int  pool_has_macro_meter[BCM_MAX_NUM_UNITS]\
                           [BCM_POLICER_GLOBAL_METER_MAX_POOL] = {{0}};
...

{
    ...
    int size_pool = 0, num_pools = 0;
    int macro_bank_offset = 0, bank_size = 0;
    size_pool =  SOC_INFO(unit).global_meter_max_size_of_pool;
    num_pools =  SOC_INFO(unit).global_meter_pools;
    ...
    if (direction == GLOBAL_METER_ALLOC_VERTICAL) {
        for ( ;no_of_pools_checked < num_pools; pool++) {
            ...
            if (soc_feature(unit, \
                            soc_feature_global_meter_macro_micro_same_pool)) {
                /* Macro and Micro meters must be in same pool 
                 * but in different bank.
                 * 
                 * It is fair to assume that one MACRO meter shall have 
                 * at least two MICRO meters. With bank restriction, we can
                 * have max of pool_size/2 MICRO meters (bank_size) in a 
                 * given pool. This translates to max of bank_size/2 MACRO
                 * meters per pool.
                 */
                if (num_pools == skip_pool) {
                    if (*numbers == 1) {
                        /* This is MACRO meter. Is there a better check? */
                        pool_has_macro_meter[unit][pool]++;
                        bank_size = size_pool / 2;
                        macro_bank_offset = bank_size + (bank_size / 2);
                    } else {
                        /* Not part of envelope meter */
                    }
                } else if (pool != skip_pool) {
                    /* not a MACRO meter */
                    no_of_pools_checked++;
                    continue;
                }
            ...
            if (BCM_SUCCESS(rv)) {
                if (soc_feature(unit,                                   \
                               soc_feature_global_meter_macro_micro_same_pool)) {
                    if (pool_has_macro_meter[unit][pool]) {
                        if (num_pools != skip_pool) {
                            /* MICRO meter - check for bank restriction */ 
                            if ((*pid) > (size_pool / 2)) {
                                return BCM_E_FULL;
                            }
                        } else if ((*pid)  > (size_pool * 3 / 4)) {
                            /* check if reserved MACRO meter range is being used 
                               by non-envelope meters */
                            return BCM_E_FULL;
                        }

                        if (((*pid) + macro_bank_offset) >= size_pool) {
                            return BCM_E_FULL;
                        } else {
                            /* Policer ID Allocation successful */
                            (*pid) += macro_bank_offset;
                        }
                    }
                }
                                                  ...
                                                  bcm_policer_config_t *pol_cfg)
{
        ...
        switch (mode) {
            ...
            case GLOBAL_METER_MODE_CASCADE:
                ...
                if (pair_e_valid == 1) {
                    ...
                    /* Check if GCF committed policer */
                    pool = _bcm_global_meter_get_pool_from_policerId(unit, policer_id);
                    if (pair_e == (pool + 4)) {
                        pol_cfg->mode = bcmPolicerModeCoupledCascade;
                    }
                    /* For groups which are not in couple cascade mode,
                       look at hw COMMITTED_EXCESS to determine policer mode */
                    soc_SVM_METER_TABLEm_field_get(unit, &svm_entry,
                                                   COMMITTED_EXCESSf,
                                                   &c_e);
                    if (c_e == 1) {
    ...
    pol_cfg->npoflow_policer_id = nxt_e;
    ...
    pol_cfg->ncoflow_policer_id = nxt_c;

    /* Extract more info from excess pair in case of GCF mode */
    if (pol_cfg->mode == bcmPolicerModeCoupledCascade) {
        rv = _bcm_global_meter_policer_get(unit, policer_id, &policer_control);
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_POLICER,
    ...
    if (policer_control->direction == GLOBAL_METER_ALLOC_VERTICAL) {
         ...
         if (soc_feature(unit, \
             ...
             if (pool_has_macro_meter[unit][pool]) {
                 bank_size = size_pool / 2;
                 macro_bank_offset = bank_size + (bank_size / 2);
                 if (pid > (size_pool * 3 / 4)) {
                     /* Adjust pid if it is MACRO meter */
                     pid -= macro_bank_offset;
                     pool_has_macro_meter[unit][pool]--;
                 }
             }
    ...
    if (num_offsets) {
        if (!soc_feature(0,soc_feature_global_meter_mef_10dot3)) {
            ...
            for (j = 0; j < num_offsets; j++) {
                /* pool priority is 0[high]-to-7[low] */
                offset_map[j].pool = j;
                offset_map[j].offset= value;
                offset_map[j].meter_enable = 1;
            }
                                   ...
                                   int *npolicers)
{
    ...
    *policer_id |= (offset_mode + 1) << BCM_POLICER_GLOBAL_METER_MODE_SHIFT;
    ...
    /* Fill in the rest of policer_IDs on devices that support
     * clean implementation of returning policer ID list.
     *   - policer_id[0] should have lowest priority pool policer ID
     *   - policer_id[in_policers-1] should have highest pool policer ID
     */
    if ((soc_feature(unit, soc_feature_global_meter_policer_list_support)) &&
        ((direction == GLOBAL_METER_ALLOC_HORIZONTAL) && (in_policers > 1))) {
        if (SOC_IS_APACHE(unit)) {
            /* Fill the policer list as per pool priority
             * pool 0 is highest and pool 7 is lowest */
            for (index = 0; index < in_policers; index++) {
                policer_id[(in_policers - 1) - index] = policer_id[0] + 
                    (SOC_INFO(unit).global_meter_max_size_of_pool * index);
            }
        } else {
            /* Fill the policer list as per pool priority
             * pool 7 is highest and pool 0 is lowest
             *
             * NOTE: This should be used by KT KT2 TR3 etc SOCs due to hardware
             * difference in pool priorities.
             */
            for (index = 1; index < in_policers; index++) {
                policer_id[index] = policer_id[0] +
                    (SOC_INFO(unit).global_meter_max_size_of_pool * index);
            }
        }
    }
...
{    
    ...
    if ((attr_selectors[0].offset < 0) || 
        (attr_selectors[0].offset >= SVC_METER_UDF1_MAX_BIT_POSITION)) {
        LOG_VERBOSE(BSL_LS_BCM_POLICER, (BSL_META_U(unit,
