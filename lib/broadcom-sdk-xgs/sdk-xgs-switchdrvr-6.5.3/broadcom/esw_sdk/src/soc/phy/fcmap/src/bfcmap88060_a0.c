                                      ...
                                      bfcmap_port_config_t *pCfg)
{
    ...
    if (mask2 & BFCMAP_ATTR2_USE_TTS_PCS_32G_MASK) {
        x_bfcmap_port_cfg.action_mask2 |= XMOD_BFCMAP_ATTR2_USE_TTS_PCS_32G_MASK;
#if __BYTE_ORDER == __BIG_ENDIAN
        x_bfcmap_port_cfg.use_tts_pcs_32G = BCMSWAP32(pCfg->use_tts_pcs_32G);
#else
        x_bfcmap_port_cfg.use_tts_pcs_32G = pCfg->use_tts_pcs_32G;
#endif
    }
...
#if __BYTE_ORDER == __BIG_ENDIAN
        pCfg->mapper_len = BCMSWAP32(x_bfcmap_port_cfg.mapper_len);
#else
        pCfg->mapper_len = x_bfcmap_port_cfg.mapper_len;
#endif
    }
...
#if __BYTE_ORDER == __BIG_ENDIAN
        pCfg->use_tts_pcs_32G = BCMSWAP32(x_bfcmap_port_cfg.use_tts_pcs_32G);
#else
        pCfg->use_tts_pcs_32G = x_bfcmap_port_cfg.use_tts_pcs_32G;
#endif
    }
    ...
    while (tlv_count) {
        ...
        switch(pl_ptr->tlv.pld_type) {
            case FC_INB_PLD_TYPE_FC_MIBS:
                ...
                bfcmap88060_fc_mibs_update(mdc->mibs, &pl_ptr->word[0]);
                break;
...
int bfcmap88060_xmod_debug_cmd(int unit, int port, int cmd)
{
    ...
    /* call xmod */
    rv = phy_8806x_xmod_command(unit, port, XMOD_CMD_MODE_CORE(XMOD_DEV_DEBUG_CMD), xmodtxbuff, xmodtxlen, NULL, 0);
...
#if __BYTE_ORDER == __BIG_ENDIAN
    bfcmap_port_mibs->fc_c2_rxgoodframes = BCMSWAP64(*cur_fc_mibs);
    cur_fc_mibs++;
    bfcmap_port_mibs->fc_c2_rxinvalidcrc = BCMSWAP64(*cur_fc_mibs);
    cur_fc_mibs++;
    bfcmap_port_mibs->fc_c2_rxframetoolong = BCMSWAP64(*cur_fc_mibs);
    cur_fc_mibs++;
    bfcmap_port_mibs->fc_c3_rxgoodframes = BCMSWAP64(*cur_fc_mibs);
    cur_fc_mibs++;
    bfcmap_port_mibs->fc_c3_rxinvalidcrc = BCMSWAP64(*cur_fc_mibs);
    cur_fc_mibs++;
    bfcmap_port_mibs->fc_c3_rxframetoolong = BCMSWAP64(*cur_fc_mibs);
    cur_fc_mibs++;
    bfcmap_port_mibs->fc_cf_rxgoodframes = BCMSWAP64(*cur_fc_mibs);
    cur_fc_mibs++;
    bfcmap_port_mibs->fc_cf_rxinvalidcrc = BCMSWAP64(*cur_fc_mibs);
    cur_fc_mibs++;
    bfcmap_port_mibs->fc_cf_rxframetoolong = BCMSWAP64(*cur_fc_mibs);
    cur_fc_mibs++;
    bfcmap_port_mibs->fc_rxdelimitererr = BCMSWAP64(*cur_fc_mibs);
    cur_fc_mibs++;
    bfcmap_port_mibs->fc_rxdelimitererr = BCMSWAP64(*cur_fc_mibs);
    cur_fc_mibs++;
    bfcmap_port_mibs->fc_rxbbcredit0 = BCMSWAP64(*cur_fc_mibs);
    cur_fc_mibs++;
    bfcmap_port_mibs->fc_rxlinkfail = BCMSWAP64(*cur_fc_mibs);
    cur_fc_mibs++;
    bfcmap_port_mibs->fc_rxsyncfail = BCMSWAP64(*cur_fc_mibs);
    cur_fc_mibs++;
    bfcmap_port_mibs->fc_rxlosssig = BCMSWAP64(*cur_fc_mibs);
    cur_fc_mibs++;
    bfcmap_port_mibs->fc_rxprimseqerr = BCMSWAP64(*cur_fc_mibs);
    cur_fc_mibs++;
    bfcmap_port_mibs->fc_rxinvalidset = BCMSWAP64(*cur_fc_mibs);
    cur_fc_mibs++;
    bfcmap_port_mibs->fc_rxlpicount= BCMSWAP64(*cur_fc_mibs);
    cur_fc_mibs++;
    bfcmap_port_mibs->fc_rxencodedisparity = BCMSWAP64(*cur_fc_mibs);
    cur_fc_mibs++;
    bfcmap_port_mibs->fc_rxbadxword = BCMSWAP64(*cur_fc_mibs);
    cur_fc_mibs++;
    bfcmap_port_mibs->fc_rx_c2_runtframes = BCMSWAP64(*cur_fc_mibs);
    cur_fc_mibs++;
    bfcmap_port_mibs->fc_rx_c3_runtframes = BCMSWAP64(*cur_fc_mibs);
    cur_fc_mibs++;
    bfcmap_port_mibs->fc_rx_cf_runtframes = BCMSWAP64(*cur_fc_mibs);
    cur_fc_mibs++;
    bfcmap_port_mibs->fc_c2_rxbyt = BCMSWAP64(*cur_fc_mibs);
    cur_fc_mibs++;
    bfcmap_port_mibs->fc_c3_rxbyt = BCMSWAP64(*cur_fc_mibs);
    cur_fc_mibs++;
    bfcmap_port_mibs->fc_cf_rxbyt = BCMSWAP64(*cur_fc_mibs);
    cur_fc_mibs++;
    bfcmap_port_mibs->fc_tx_c2_frames = BCMSWAP64(*cur_fc_mibs);
    cur_fc_mibs++;
    bfcmap_port_mibs->fc_tx_c3_frames = BCMSWAP64(*cur_fc_mibs);
    cur_fc_mibs++;
    bfcmap_port_mibs->fc_tx_cf_frames = BCMSWAP64(*cur_fc_mibs);
    cur_fc_mibs++;
    bfcmap_port_mibs->fc_tx_c2_oversized_frames = BCMSWAP64(*cur_fc_mibs);
    cur_fc_mibs++;
    bfcmap_port_mibs->fc_tx_c3_oversized_frames = BCMSWAP64(*cur_fc_mibs);
    cur_fc_mibs++;
    bfcmap_port_mibs->fc_tx_cf_oversized_frames = BCMSWAP64(*cur_fc_mibs);
    cur_fc_mibs++;
    bfcmap_port_mibs->fc_txbbcredit0 = BCMSWAP64(*cur_fc_mibs);
    cur_fc_mibs++;
    bfcmap_port_mibs->fc_c2_txbyt = BCMSWAP64(*cur_fc_mibs);
    cur_fc_mibs++;
    bfcmap_port_mibs->fc_c3_txbyt = BCMSWAP64(*cur_fc_mibs);
    cur_fc_mibs++;
    bfcmap_port_mibs->fc_cf_txbyt = BCMSWAP64(*cur_fc_mibs);
#else
    bfcmap_port_mibs->fc_c2_rxgoodframes = *cur_fc_mibs++;
    bfcmap_port_mibs->fc_c2_rxinvalidcrc = *cur_fc_mibs++;
    bfcmap_port_mibs->fc_c2_rxframetoolong = *cur_fc_mibs++;
    bfcmap_port_mibs->fc_c3_rxgoodframes = *cur_fc_mibs++;
    bfcmap_port_mibs->fc_c3_rxinvalidcrc = *cur_fc_mibs++;
    bfcmap_port_mibs->fc_c3_rxframetoolong = *cur_fc_mibs++;
    bfcmap_port_mibs->fc_cf_rxgoodframes = *cur_fc_mibs++;
    bfcmap_port_mibs->fc_cf_rxinvalidcrc = *cur_fc_mibs++;
    bfcmap_port_mibs->fc_cf_rxframetoolong = *cur_fc_mibs++;
    bfcmap_port_mibs->fc_rxdelimitererr = *cur_fc_mibs++;
    bfcmap_port_mibs->fc_rxdelimitererr = *cur_fc_mibs++;
    bfcmap_port_mibs->fc_rxbbcredit0 = *cur_fc_mibs++;
    bfcmap_port_mibs->fc_rxlinkfail = *cur_fc_mibs++;
    bfcmap_port_mibs->fc_rxsyncfail = *cur_fc_mibs++;
    bfcmap_port_mibs->fc_rxlosssig = *cur_fc_mibs++;
    bfcmap_port_mibs->fc_rxprimseqerr = *cur_fc_mibs++;
    bfcmap_port_mibs->fc_rxinvalidset = *cur_fc_mibs++;
    bfcmap_port_mibs->fc_rxlpicount = *cur_fc_mibs++;
    bfcmap_port_mibs->fc_rxencodedisparity = *cur_fc_mibs++;
    bfcmap_port_mibs->fc_rxbadxword = *cur_fc_mibs++;
    bfcmap_port_mibs->fc_rx_c2_runtframes = *cur_fc_mibs++;
    bfcmap_port_mibs->fc_rx_c3_runtframes = *cur_fc_mibs++;
    bfcmap_port_mibs->fc_rx_cf_runtframes = *cur_fc_mibs++;
    bfcmap_port_mibs->fc_c2_rxbyt = *cur_fc_mibs++;
    bfcmap_port_mibs->fc_c3_rxbyt = *cur_fc_mibs++;
    bfcmap_port_mibs->fc_cf_rxbyt = *cur_fc_mibs++;
    bfcmap_port_mibs->fc_tx_c2_frames = *cur_fc_mibs++;
    bfcmap_port_mibs->fc_tx_c3_frames = *cur_fc_mibs++;
    bfcmap_port_mibs->fc_tx_cf_frames = *cur_fc_mibs++;
    bfcmap_port_mibs->fc_tx_c2_oversized_frames = *cur_fc_mibs++;
    bfcmap_port_mibs->fc_tx_c3_oversized_frames = *cur_fc_mibs++;
    bfcmap_port_mibs->fc_tx_cf_oversized_frames = *cur_fc_mibs++;
    bfcmap_port_mibs->fc_txbbcredit0 = *cur_fc_mibs++;
    bfcmap_port_mibs->fc_c2_txbyt = *cur_fc_mibs++;
    bfcmap_port_mibs->fc_c3_txbyt = *cur_fc_mibs++;
    bfcmap_port_mibs->fc_cf_txbyt = *cur_fc_mibs++;
#endif
