...
#define SOC_SER_REG_CACHE_MAX 28
...
soc_generic_sram_mem_scan_start(int unit)
{
    ...
    if (soc_property_get(unit, spn_SRAM_SCAN_ENABLE, SAL_BOOT_SIMULATION ? 0 : 
                          (SOC_IS_TOMAHAWK(unit) || SOC_IS_TD2P_TT2P(unit)
                           || SOC_IS_APACHE(unit)))) {
        sal_usecs_t interval = 0;
        int rate = 0;
    ...
    if ((SOC_IS_TD2P_TT2P(unit) ||
        ...
        case L3_DEFIP_DATA_ONLYm:
            ...
            if (mem != L3_DEFIP_ALPM_RAWm) {
                 ...
                 * when correcting the view will be changed to L3_DEFIP_DATA_ONLY. For
                 * L3_DEFIP_PAIR_128 view the physical index is equal to logical index.
                 */
                if (is_physical_index &&
                    SOC_IS_TD2_TT2(unit) &&
                    !SOC_IS_APACHE(unit) &&
                    ...
                    report_index = soc_trident2_l3_defip_mem_index_get(unit,
                                                                       index,
                                                                       &report_mem);
            ...
            if (SOC_IS_TOMAHAWK(unit)) {
                LOG_ERROR(BSL_LS_SOC_SER,
                          (BSL_META_U(unit,
                                      ...
                                      "blk: %s index: %d\n"),
                           pipe, restore_pipe, SOC_MEM_NAME(unit, mem),
...
*/
    ...
    case L3_ENTRY_IPV6_MULTICASTm:
        ...
        if ((SOC_IS_TRIDENT(unit) || SOC_IS_KATANA2(unit)) && hw_cpu) {
            num_mems = 1;
            mem_list[0] = reported_mem;
        } else
#endif
...
soc_mem_alpm_aux_table_correction(int unit, int pipe, int index)
{
    ...
    if (SOC_IS_TD2_TT2(unit)) {
        ...
        LOG_ERROR(BSL_LS_SOC_SER,
              (BSL_META_U(unit,
                          "Inline correction: mem:L3_DEFIP_AUX_TABLE pipe:%d index:%d\n"),
                pipe, index));
        ...
        if (SOC_IS_TD2_TT2(unit)) {
            ...
            if (mem == L3_DEFIPm || mem == L3_DEFIP_PAIR_128m) {
                if ((NUM_PIPE(unit) != 1) ||
                     ...
                     * these mems can be restored from cache when ALPM
                     * is not enabled.
                     */
                    is_l3_defip = TRUE;
                }
        ...
        case SOC_MEM_FLAG_SER_WRITE_CACHE_RESTORE:
            ...
            if (cache == NULL) {
                ...
                if (SOC_IS_TD2_TT2(unit) &&
                    ...
                    rv = _soc_ser_recovery_hw_cache(unit, si->pipe_num, mem, copyno, index, si, TRUE);
                } else
#endif
                {
                    rv = _soc_ser_recovery_hw_cache(unit, si->pipe_num, mem, copyno, index, si, FALSE);
                }
                if (rv == SOC_E_NONE || rv != SOC_E_UNAVAIL) {
