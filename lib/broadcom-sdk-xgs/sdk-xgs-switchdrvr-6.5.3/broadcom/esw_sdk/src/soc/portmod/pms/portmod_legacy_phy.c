...
/* Be aware that this function only retrieves the PAUSE information currently */
int portmod_port_legacy_ability_remote_get(int unit, int port, phymod_autoneg_ability_t* an_ability) {
    int rv = SOC_E_NONE;
    soc_port_ability_t ability;

    rv = soc_phyctrl_ability_remote_get(unit, port, &ability);

    if(ability.pause == 1){
        an_ability->capabilities |= PHYMOD_AN_CAP_ASYM_PAUSE;
    } else if(ability.pause == 2){
        an_ability->capabilities |= PHYMOD_AN_CAP_ASYM_PAUSE;
        an_ability->capabilities |= PHYMOD_AN_CAP_SYMM_PAUSE;
    } else if(ability.pause == 3){
        an_ability->capabilities |= PHYMOD_AN_CAP_SYMM_PAUSE;
    } else{
    }
