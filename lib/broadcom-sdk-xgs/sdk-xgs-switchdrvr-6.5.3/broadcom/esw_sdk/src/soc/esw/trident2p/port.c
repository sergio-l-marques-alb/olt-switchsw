...
{
    int i;
    int lport, phy_port, sister_port;
    ...
    for (phy_port = 1; phy_port < TD2P_INFO(unit)->phy_ports_max - 1;
         ...
         for (i = 1; i < _TD2_PORTS_PER_XLP; i++) {
             ...
             sister_speed = soc_property_port_get(unit, lport, spn_PORT_INIT_SPEED,
                                                  si->port_speed_max[lport]);
             if (speed != sister_speed) {
                 LOG_ERROR(BSL_LS_SOC_PORT, (BSL_META_U(unit,
...
/*
 * Function:
 *      soc_td2p_port_mixed_sister_encap_validate
 * Purpose:
 *      Check all sister ports in each PortMacro have same encap mode in device.
 * Parameters:
 *      unit     - (IN) Unit number.
 * Returns:
 *      SOC_E_XXX
 ...
 *      Assumes that this function is called at early stage of initialization
 *      (For example, soc_trident2_port_config_init).
 *      Access pbmp_xport_xe instead of using IS_HG_PORT which is not ready to use yet.
 */
int
soc_td2p_port_mixed_sister_encap_validate(int unit)
{
    int i;
    int lport, phy_port, sister_phy_port;
    int encap, sister_encap;
    ...
    for (phy_port = 1; phy_port < TD2P_INFO(unit)->phy_ports_max - 1;
        ...
        my_pbmp_xport_xe = soc_property_get_pbmp_default(unit, spn_PBMP_XPORT_XE,
                                                         my_pbmp_xport_xe);
        encap = SOC_PBMP_MEMBER(my_pbmp_xport_xe, lport) ?
                SOC_ENCAP_IEEE : SOC_ENCAP_HIGIG2;

        for (i = 1; i < _TD2_PORTS_PER_XLP; i++) {
            sister_phy_port = TD2P_PHY_PORT(unit, phy_port).sisters_ports[i];
            lport = si->port_p2l_mapping[sister_phy_port];
            if (lport < 0) {
                continue;
            }
            ...
            sister_encap = SOC_PBMP_MEMBER(my_pbmp_xport_xe, lport) ?
                           SOC_ENCAP_IEEE : SOC_ENCAP_HIGIG2;
            if (encap != sister_encap) {
                LOG_ERROR(BSL_LS_SOC_PORT, (BSL_META_U(unit,
                          "Mixed port encap is not acceptable "
                          "on physical ports %d %d %d %d\n"),
                           TD2P_PHY_PORT(unit, phy_port).sisters_ports[0],
                           TD2P_PHY_PORT(unit, phy_port).sisters_ports[1],
                           TD2P_PHY_PORT(unit, phy_port).sisters_ports[2],
...
/*
 * Function:
 *      soc_td2p_port_mixed_sister_encap_check
 * Purpose:
 *      Check if changing the encap mode of given port can be acceptable or not.
 * Parameters:
 *      unit     - (IN) Unit number.
 *      port     - (IN) Logical port number
 *      mode     - (IN) encap mode
 ...
 */
int
soc_td2p_port_mixed_sister_encap_check(int unit, soc_port_t in_lport, int in_mode)
{
    int i;
    int lport;
    int phy_port, sister_phy_port, sister_encap;
    soc_info_t *si = &SOC_INFO(unit);

    phy_port = si->port_l2p_mapping[in_lport];

    for (i = 0; i < _TD2_PORTS_PER_XLP; i++) {
        sister_phy_port = TD2P_PHY_PORT(unit, phy_port).sisters_ports[i];
        lport = si->port_p2l_mapping[sister_phy_port];
        if ((lport < 0) || (lport == in_lport)) {
            continue;
        }
