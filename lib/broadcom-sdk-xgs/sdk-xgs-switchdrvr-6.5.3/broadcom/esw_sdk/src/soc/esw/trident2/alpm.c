...
#define SOC_ALPM_TABLE_MEM(_u_, _v6_, _mem_)             \
...
static int
_soc_alpm_lpm_ent_init(int unit, void *alpm_entry, soc_mem_t mem, int ipv6, int vrf,
                       int bucket, int index, void *lpm_entry);
static int
_soc_key_lpm_ent_init(int unit, uint32 *key, int len, int vrf, int ipv6,
                      defip_entry_t *lpm_entry, int nh_ecmp_idx, int init);
...
static int
_soc_alpm_mask_len_get(int u, const void *entry, int *pfx)
{
    int     rv;
    uint32  ipv4a;
    int     ipv6;
                               ...
                               int convert, void *shadow)
{
    ...
    int rv = SOC_E_NONE, pfx, pfx2, vrf, vrf2;
    ...
    if (!v6) {
        ...
        rv = soc_alpm_lpm_vrf_get(u, (void *)&tmp_ipv4, &vrf, &pfx);
        SOC_IF_ERROR_RETURN(rv);

        /* vrf for ipv4 entry 1 */
        SOC_IF_ERROR_RETURN(soc_alpm_lpm_ip4entry1_to_0(u, &tmp_ipv4, 
                            &tmp_ipv4, PRESERVE_HIT));
        rv = soc_alpm_lpm_vrf_get(u, (void *)&tmp_ipv4, &vrf2, &pfx2);
        SOC_IF_ERROR_RETURN(rv);
    } else {
        rv = soc_alpm_lpm_vrf_get(u, entry, &vrf, &pfx);
    }
    if (SOC_URPF_STATUS_GET(u)) {
    ...
    if (vrf == SOC_L3_VRF_OVERRIDE ||
        (SOC_MEM_FIELD_VALID(u, mem, MULTICAST_ROUTE0f) &&
    ...
    if (!v6) {
        soc_alpm_db_ent_type_encoding(u, pfx2, &db_type, &ent_type);
        if (vrf2 == SOC_L3_VRF_OVERRIDE ||
            (SOC_MEM_FIELD_VALID(u, mem, MULTICAST_ROUTE1f) &&
                      ...
                      alpm_mem_prefix_array_t *prefix_array, int *new_index)
{
    ...
    defip_alpm_ipv4_entry_t alpmv4_entry, alpmv4_sip_entry; 
    defip_alpm_ipv6_64_entry_t alpmv6_entry, alpmv6_sip_entry;     
                 ...
                 int tcam_index)  /* return key location */
{
    ...
    if (split) {
        ...
        if (tmp_pyld->bkt_ptr) {
            ...
            if (tmp_pyld->bkt_ptr == payload) {
                ...
                sal_memcpy(bufp, alpm_data, v6 ? 
                           sizeof(defip_alpm_ipv6_64_entry_t) :
                           sizeof(defip_alpm_ipv4_entry_t));
                if (SOC_URPF_STATUS_GET(u)) {
        ...
        (void) _soc_key_lpm_ent_init(u, pivot, length, vrf, v6,
                              &lpm_entry, 0, 0);
...
rollback_bkt_move:
    ...
    for (i = 0; i < prefix_array.count; i ++) {
        ...
        if (new_index[i] != -1) {
            if (v6) {
                sal_memset(bufp, 0, sizeof(defip_alpm_ipv6_64_entry_t));
            } else {
                sal_memset(bufp, 0, sizeof(defip_alpm_ipv4_entry_t));
            }
...
_soc_alpm_bkt_count(int u, soc_mem_t mem, int bkt_idx, int v6, void *bufp) 
{
    ...
    for (bank = 0; bank < 4; bank++) {
      for (entry = 0; entry < (v6 ? 4 : 6); entry++) {
        ...
        SOC_IF_ERROR_RETURN(rv);
        if (sal_memcmp(bufp, soc_mem_entry_null(u, v6 ? L3_DEFIP_ALPM_IPV6_64m :
                   L3_DEFIP_ALPM_IPV4m), v6 ? 
            sizeof(defip_alpm_ipv6_64_entry_t) : 
                        sizeof(defip_alpm_ipv4_entry_t))) {
            return 1;
        }
                 ...
                 int tcam_index, int key_index, int propagate)  /* TCAM entry data */
{
    ...
    defip_alpm_ipv4_entry_t alpmv4_entry, alpmv4_sip_entry, alpmv42;
    defip_alpm_ipv6_64_entry_t alpmv6_entry, alpmv6_sip_entry, alpmv62;
    defip_aux_scratch_entry_t aux_entry;
    uint32      e[SOC_MAX_MEM_FIELD_WORDS];
    soc_mem_t mem;
    ...
    if (vrf_id != SOC_L3_VRF_OVERRIDE) {
        ...
        if (!default_route) {
            ...
            if (propagate) {
                ...
                (void) _soc_key_lpm_ent_init(u, prop_pfx, bpm_pfx_len, vrf, ipv6,
                                        &lpm_entry, 0, 1);
                ...
                if (SOC_SUCCESS(rv)) {
                    ...
                    (void) _soc_key_lpm_ent_init(u, prop_pfx, length, vrf, ipv6,
                                           &lpm_entry, 0, 0);
        ...
        if ((pivot_pyld->len != 0) && (trie->trie == NULL)) { /* Empty bucket */
            ...
            /* Initialize an lpm entry to perform a match */
            (void) _soc_key_lpm_ent_init(u, hw_key, pivot_pyld->len,
                              vrf, ipv6, &lpm_entry, 0, 1);
...
soc_alpm_vrf_add(int u, int vrf, int v6)
{
    ...
    /* Initialize an lpm entry to perform a match */
    (void) _soc_key_lpm_ent_init(u, key, 0, vrf, v6, &tmp_lpm, 0, 1);
 ...
 */
int
soc_alpm_insert(int u, void *entry_data, uint32 src_flags, int bkt_idx,
                int pivot_info)
{
    /* Search Result buffer */
    defip_alpm_ipv4_entry_t alpmv4_entry, alpmv4_sip_entry ; 
    ...
    ipv6 = soc_mem_field32_get(u, L3_DEFIPm, entry_data, MODE0f);

    /* Get table memory. */
    SOC_ALPM_TABLE_MEM(unit, ipv6, mem);
    ...
    SOC_IF_ERROR_RETURN(
        _soc_alpm_mem_ent_init(u, entry_data, bufp, bufp2, mem, src_flags, 
                               &default_route));
    ...
    if (soc_feature(u, soc_feature_ipmc_defip)) {
        ipmc_route = soc_mem_field32_get(u, L3_DEFIPm, entry_data, MULTICAST_ROUTE0f);
    }
    ...
    if ((vrf_id == SOC_L3_VRF_OVERRIDE) || (ipmc_route)) {
        ...
        rv = soc_alpm_lpm_insert(u, entry_data, &index, 0,
    ...
    if (pivot_info & SOC_ALPM_LOOKUP_HIT) {
        rv = _soc_alpm_find_and_update(u, entry_data, bufp, bufp2, mem, 
                                       bkt_idx);
    } else {
        ...
        rv = _soc_alpm_insert(u, entry_data, mem, bufp, bufp2, &index, 
...
soc_alpm_lookup(int u, 
                ...
                int *cookie)
{
    defip_alpm_ipv4_entry_t alpmv4_entry; /* Search Result buffer */
    defip_alpm_ipv6_64_entry_t alpmv6_entry; /* Search Result buffer */
    soc_mem_t mem;
    int       bucket_index;
    int       tcam_index = -1;
    void      *bufp;
                          ...
                          int do_urpf)
{
    ...
    gbl_old = soc_mem_field32_get(u, lpm_mem, key_data, GLOBAL_ROUTE0f);
    vrf_old = soc_mem_field32_get(u, lpm_mem, key_data, VRF_ID_0f);
    ...
    if (vrf == SOC_L3_VRF_GLOBAL) {
        soc_mem_field32_set(u, lpm_mem, key_data, GLOBAL_ROUTE0f, gbl_old);
        soc_mem_field32_set(u, lpm_mem, key_data, VRF_ID_0f, vrf_old);
    }
    ...
    SOC_ALPM_TABLE_MEM(u, v6, mem);
    rv = _soc_alpm_mask_len_get(u, key_data, &sw_len);
    if (SOC_FAILURE(rv)) {
        return rv;
    }
    ...
    /* Found */
    if (alpm_len != -1) {
        rv = _soc_alpm_lpm_ent_init(u, alpm_bufp, mem, v6, vrf, *bucket_index, 
                                    alpm_index, e);
        if (SOC_SUCCESS(rv)) {
                         ...
                         int do_urpf)
{
    ...
    for (i = index_min; i < index_min + index_cnt; i++) {
        ...
        for (ent = 0; ent < (v6 ? 1 : 2); ent++) {
            ...
            hmsk[0] = soc_mem_field32_get(u, lpm_mem, &lpm_ent, IP_ADDR_MASK0f);
            hmsk[1] = soc_mem_field32_get(u, lpm_mem, &lpm_ent, IP_ADDR_MASK1f);
            hkey[0] = soc_mem_field32_get(u, lpm_mem, &lpm_ent, IP_ADDR0f);
            hkey[1] = soc_mem_field32_get(u, lpm_mem, &lpm_ent, IP_ADDR1f);
            smsk[0] = soc_mem_field32_get(u, lpm_mem, key_data, IP_ADDR_MASK0f);
            smsk[1] = soc_mem_field32_get(u, lpm_mem, key_data, IP_ADDR_MASK1f);
            skey[0] = soc_mem_field32_get(u, lpm_mem, key_data, IP_ADDR0f);
            skey[1] = soc_mem_field32_get(u, lpm_mem, key_data, IP_ADDR1f);

            LOG_VERBOSE(BSL_LS_SOC_ALPM,
                        (BSL_META_U(u, "\thmsk %08x %08x\n\thkey %08x %08x\n"
                        "\tsmsk %08x %08x\n\tskey %08x %08x\n"), 
                        hmsk[1], hmsk[0], hkey[1], hkey[0], 
                        smsk[1], smsk[0], skey[1], skey[0]));

            /* check if length in memory is shorter than softare */
            if (v6 && (((hmsk[1] & smsk[1]) != hmsk[1]) || 
                       ((hmsk[0] & smsk[0]) != hmsk[0]))) {
                 continue;
            }
            if (!v6 && ((hmsk[ent] & smsk[0]) != hmsk[ent])) {
                 continue;
            }

            /* check if key matched */
            if (v6 && ((skey[0] & hmsk[0]) == (hkey[0] & hmsk[0])) &&
                ((skey[1] & hmsk[1]) == (hkey[1] & hmsk[1]))) {
                match = TRUE;
                break;
            } 

            if (!v6 && ((skey[0] & hmsk[ent]) == (hkey[ent] & hmsk[ent]))) {
                match = TRUE;
                break;
            }
        }
    ...
    /* Fill in AUX Scratch and perform PREFIX Operation */
    SOC_IF_ERROR_RETURN(soc_alpm_lpm_vrf_get(u, key_data, &vrf_id, &vrf));
    rv = _soc_alpm_find_best_match(u, key_data, e, vrf, &tcam_index, 
                            ...
                            uint16                      *key_index)
{
    ...
    while(index != TD2_ALPM_HASH_INDEX_NULL) {
        uint32  e[SOC_MAX_MEM_FIELD_WORDS];
                      ...
                      uint32 op, int src_default_or_convert, int src_discard)
{
     ...
     */
    SOC_IF_ERROR_RETURN
        (_soc_alpm_write_pivot_aux(u, src_index, index, entry, convert));

    SOC_IF_ERROR_RETURN
        (_lpm_fb_urpf_entry_replicate(u, src_index_uprf, index, entry,
 ...
 */
int
soc_alpm_lpm_vrf_get(int unit, void *lpm_entry, int *vrf, int *mem_vrf)
{
    int vrf_id;
...
_soc_alpm_lpm_prefix_length_get(int u, void *entry, int *pfx_len) 
{
    ...
    SOC_IF_ERROR_RETURN(soc_alpm_lpm_vrf_get(u, entry, &vrf_id, &rv));
...
int
_soc_alpm_aux_error_index(int u, int pipe, int *index)
{
    char *dmabuf = NULL;
    ...
    int num_entries = soc_mem_index_count(u, mem);
    int mem_wsz = sizeof(uint32) * soc_mem_entry_words(u, mem);

    min = soc_mem_index_min(u, mem);
    max = soc_mem_index_max(u, mem);

    dmabuf = soc_cm_salloc(u, num_entries * mem_wsz, "auxtbl_dmabuf");
    if (dmabuf == NULL) {
        return SOC_E_MEMORY;
    }
    ...
    while (min <= max) {
        ...
        if (soc_mem_ser_read_range(u, mem, MEM_BLOCK_ALL, min, mid, ser_flags,
                                   dmabuf) < 0) {
            if (max == min) {
                *index = min;
                break;
    ...
    if (min > max) {
        /* Not found */
        LOG_WARN(BSL_LS_SOC_ALPM,
            (BSL_META_U(u, "No corrupted index in AUX Table Pipe %d\n"), pipe));
        return SOC_E_NOT_FOUND;
    } else {
        LOG_WARN(BSL_LS_SOC_ALPM,
            (BSL_META_U(u, "Found corrupted Index %d in AUX Table Pipe %d\n"),
            *index, pipe));
        return SOC_E_NONE;
    }
}
        ...
        for (pipe = 0; pipe < _TD2_PIPES_PER_DEV; pipe++) {
            if (err[pipe]) {
                ...
                if (_soc_alpm_aux_op_retry < _SOC_ALPM_AUX_OP_RETRY_COUNT) {
                    int err_index;
                    if (_soc_alpm_aux_error_index(u, pipe, &err_index) >= 0) {
                        /* Call inline correction with err_index and pipe */
                        rv = soc_mem_alpm_aux_table_correction(u, pipe, err_index);
                        if (rv < 0) {
                            LOG_ERROR(BSL_LS_SOC_ALPM,
                       ...
                       int vrf, int bucket, int index, void *lpm_entry)
{
    ...
    if (vrf == SOC_L3_VRF_OVERRIDE) {
        ...
        soc_mem_field32_set(unit, L3_DEFIPm, lpm_entry, VRF_ID_0f, vrf);
                          ...
                          int alpm_idx )    /* ALPM entry index       */
{
    ...
    /* Get VRF */
    SOC_IF_ERROR_RETURN(soc_alpm_lpm_vrf_get(unit, lpm_entry, &vrf_int, &vrf));

    /* Get ALPM table memory. */
    SOC_ALPM_TABLE_MEM(unit, ipv6, mem);

    temp_lpm_entry = sal_alloc(sizeof(defip_entry_t), "Temp lpm_entr");
                          ...
                          int bkt_ptr)
{
    ...
    /* Initialize an lpm entry to perform a match */
    (void) _soc_key_lpm_ent_init(unit, key, 0, vrf, v6, lpm_entry, 0, 1);
                                ...
                                int alpm_idx)
{
    ...
    rv = _soc_alpm_warmboot_insert(unit, ipv6, lpm_entry, alpm_entry, mem,
                             ...
                             void *lpm_entry)
{
    ...
    if (soc_L3_DEFIPm_field32_get(unit, lpm_entry, VALID0f)) {
        ...
        if (soc_L3_DEFIPm_field32_get(unit, lpm_entry, VALID1f)) {
            ...
            /* Allocate temp entry */ 
            lpm_entry_hi = sal_alloc(sizeof(defip_entry_t), "lpm_entry_hi");
           
            /* Copy upper half of lpm_entry into temp lpm_entry */
    ...
    for (lpm_idx = min; lpm_idx <= max; lpm_idx++) {
        ...
        for (i = 0; i <= (ipv6 ? 0 : 1); i++) {
            ...
            if (SOC_MEM_OPT_F32_GET(u, L3_DEFIPm, lpm_entry, GLOBAL_HIGH0f) ||
                (SOC_MEM_FIELD_VALID(u, L3_DEFIPm, MULTICAST_ROUTE0f) &&
 ...
 */
int
soc_alpm_route_capacity_get(int unit, int route_type,
                            int *max_entries, int *min_entries)
{
    int rv = SOC_E_NONE;
    int max_ent, min_ent, num_ent;
    uint32 v6_64b_ent;
    uint32 v4_ent;
    uint32 v6_128b_ent;
    ...
    v4_ent = soc_mem_index_count(unit, L3_DEFIPm) * 2;
    v6_128b_ent = soc_mem_index_count(unit, L3_DEFIP_PAIR_128m);
    if (v6_128b_ent) {
        v6_64b_ent = v6_128b_ent;
    } else {
        v6_64b_ent = v4_ent >> 1;
    }

    if (route_type == L3_DEFIP_MODE_V4) {
        num_ent = v4_ent;
        if (SOC_URPF_STATUS_GET(unit)) {
            if (soc_alpm_mode_get(unit) == SOC_ALPM_MODE_COMBINED) {
                max_ent = num_ent / 2 * ALPM_IPV4_BKT_COUNT;
                min_ent = num_ent / 2 * (ALPM_IPV4_BKT_COUNT / 3);
            } else {
                max_ent = num_ent / 2 * (ALPM_IPV4_BKT_COUNT / 2);
                min_ent = num_ent / 2 * ((ALPM_IPV4_BKT_COUNT / 2) / 3);
            }
        } else {
            max_ent = num_ent * ALPM_IPV4_BKT_COUNT;
            min_ent = num_ent * (ALPM_IPV4_BKT_COUNT / 3);
        }
    } else if (route_type == L3_DEFIP_MODE_64) {
        num_ent = v6_64b_ent;
        if (SOC_URPF_STATUS_GET(unit)) {
            if (soc_alpm_mode_get(unit) == SOC_ALPM_MODE_COMBINED) {
                max_ent = num_ent / 2 * ALPM_IPV6_64_BKT_COUNT;
                min_ent = num_ent / 2 * (ALPM_IPV6_64_BKT_COUNT / 3);
            } else {
                max_ent = num_ent / 2 * (ALPM_IPV6_64_BKT_COUNT / 2);
                min_ent = num_ent / 2 * (ALPM_IPV6_64_BKT_COUNT / 2 / 3);
            }
        } else {
            if (soc_alpm_mode_get(unit) == SOC_ALPM_MODE_COMBINED) {
                max_ent = num_ent * ALPM_IPV6_64_BKT_COUNT * 2;
                min_ent = num_ent * (ALPM_IPV6_64_BKT_COUNT * 2 / 3);
            } else {
                max_ent = num_ent * ALPM_IPV6_64_BKT_COUNT;
                min_ent = num_ent * (ALPM_IPV6_64_BKT_COUNT / 3);
            }
        }
    } else if (route_type == L3_DEFIP_MODE_128) {
        num_ent = v6_128b_ent;
        if (SOC_URPF_STATUS_GET(unit)) {
            if (soc_alpm_mode_get(unit) == SOC_ALPM_MODE_COMBINED) {
                max_ent = num_ent / 2 * ALPM_IPV6_128_BKT_COUNT;
                min_ent = num_ent / 2 * (ALPM_IPV6_128_BKT_COUNT / 3);
            } else {
                max_ent = num_ent / 2 * (ALPM_IPV6_128_BKT_COUNT / 2);
                min_ent = num_ent / 2 * (ALPM_IPV6_128_BKT_COUNT / 2 / 3);
            }
        } else {
            if (soc_alpm_mode_get(unit) == SOC_ALPM_MODE_COMBINED) {
                max_ent = num_ent * ALPM_IPV6_128_BKT_COUNT * 2;
                min_ent = num_ent * (ALPM_IPV6_128_BKT_COUNT * 2 / 3);
            } else {
                max_ent = num_ent * ALPM_IPV6_128_BKT_COUNT;
                min_ent = num_ent * (ALPM_IPV6_128_BKT_COUNT / 3);
            }
        }
    } else {
        return SOC_E_UNAVAIL;
