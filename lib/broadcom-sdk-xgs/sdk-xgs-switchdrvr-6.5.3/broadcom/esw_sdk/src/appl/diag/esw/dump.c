...
#define IS_IFP_TCAM(unit, base_mem) ((SOC_IS_TOMAHAWK(unit))? \
    ...
    for (k = index; k < index + count; k++) {
...
#endif
        {
            ...
            if (flags & DUMP_TABLE_PIPE_X) {
                ...
                i = soc_mem_read_no_cache(unit, read_flags, mem, copyno, k, entry);
            } else {
                if (soc_feature(unit, soc_feature_dump_socmem_uses_dma) &&
                    do_dma) {
                    sal_memcpy(entry,
...
{ 
    ...
    || defined(BCM_VALKYRIE2_SUPPORT) || defined(BCM_TRIDENT2PLUS_SUPPORT)
