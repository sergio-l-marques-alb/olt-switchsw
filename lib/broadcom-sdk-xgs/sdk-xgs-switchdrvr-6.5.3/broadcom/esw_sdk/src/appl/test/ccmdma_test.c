...
do_ccmdma(int unit, unsigned int cmc, unsigned int* host0_mem_start_addr,
          ...
          unsigned char host0_is_internal, unsigned char host1_is_internal,
          volatile unsigned int* transfer_count)
{
    unsigned int host0_mem_pcie_addr;
    unsigned int host1_mem_pcie_addr;
    unsigned int rdata;

    host0_mem_pcie_addr = (host0_is_internal) ?
                          (*host0_mem_start_addr | 0xf0000000) :
                          soc_cm_l2p(unit, host0_mem_start_addr);
    host1_mem_pcie_addr = (host1_is_internal) ?
                          (*host1_mem_start_addr | 0xf0000000) :
                          soc_cm_l2p(unit, host1_mem_start_addr);

    do {
        if (cd_test_data.use_api) {
            /* The soc_host_ccm_copy function always call soc_cm_l2p.
               That works for external memories but not for internal memories.
               In case of internal memories, call soc_cm_p2l here before
               soc_host_ccm_copy call soc_cm_l2p. */
            if (host0_is_internal) {
                host0_mem_start_addr = soc_cm_p2l(unit, host0_mem_pcie_addr);
            }
            if (host1_is_internal) {
                host1_mem_start_addr = soc_cm_p2l(unit, host1_mem_pcie_addr);
            }
            if (soc_host_ccm_copy_multi_cmc(unit, host0_mem_start_addr,
                                            host1_mem_start_addr,
                                            entry_count, 0, cmc) < 0) {
                cli_out("\nWARNING: soc_host_ccm_copy_multi_cmc returned with error");
            }
        } else {
            soc_pci_write(unit, CMIC_CMCx_CCM_DMA_CFG_OFFSET(cmc), 0x0);
            soc_pci_write(unit, CMIC_CMCx_CCM_DMA_STAT_OFFSET(cmc), 0x0);
            soc_pci_write(unit, CMIC_CMCx_CCM_DMA_HOST0_MEM_START_ADDR_OFFSET(cmc),
                          host0_mem_pcie_addr);
            soc_pci_write(unit, CMIC_CMCx_CCM_DMA_HOST1_MEM_START_ADDR_OFFSET(cmc),
                          host1_mem_pcie_addr);
            soc_pci_write(unit, CMIC_CMCx_CCM_DMA_ENTRY_COUNT_OFFSET(cmc),
                          entry_count);
            soc_pci_write(unit, CMIC_CMCx_CCM_DMA_CFG_OFFSET(cmc), 0x1);

            while(1) {
                rdata = soc_pci_read(unit, CMIC_CMCx_CCM_DMA_STAT_OFFSET(cmc));
                if ((rdata & 0x00000001) == 0x00000001)  {
                    if ((rdata >> 1) % 2 != 0)  {
                        cd_test_data.test_fail = 1;
                        test_error(unit, "\nERROR bit set in CCMDMA status register");
                    }
                    break;
                }
            }
...
initialize_memory(int unit, unsigned int cmc_bitmap, unsigned int entry_count)
{
    ...
    unsigned int i;
    unsigned int xfer_type;
    unsigned int* int_mem_base = &cd_test_data.sram_base;
        ...
        if ((cmc_bitmap >> cmc) % 2 != 0) {
            if ((xfer_type == EXT_TO_EXT) || (xfer_type == EXT_TO_INT)) {
                cd_test_data.host0_base[cmc] =
                        (unsigned int*) sal_dma_alloc(cd_test_data.entry_count *
                ...
                clear_internal_memory(unit, *cd_test_data.host0_base[cmc],
                                      entry_count);
            }
            ...
            if ((xfer_type == EXT_TO_EXT) || (xfer_type == INT_TO_EXT)) {
                ...
                int_mem_base += entry_count;
                clear_internal_memory(unit, *cd_test_data.host1_base[cmc],
                                      entry_count);
            }
            ...
            for (i = 0; i < entry_count; i++)  {
                if ((xfer_type == EXT_TO_EXT) || (xfer_type == EXT_TO_INT)) {
                    ...
                    soc_pci_mcs_write(unit, *cd_test_data.host0_base[cmc] +
