...
{
    test_generic_hash_testdata_t *htd = p;
    uint32 entry_tmp[16][SOC_MAX_MEM_WORDS] = {{0}}, entry[SOC_MAX_MEM_WORDS] = {0};
    uint32 bucket = 0, result[SOC_MAX_MEM_WORDS] = {0}, hash, hash1;
    int ix, jx, r, idx, rv = 0;
    int ivid = 0, port = 0, key_type = 0, vrf_id = 0, vfi = 0, vp = 0;
    int iter = htd->opt_count;
    int bucket_size, num_entries;
    int bank_count, bank_num = 0;
    ...
    switch (htd->mem) {
        case L2Xm:
            if (READ_L2_TABLE_HASH_CONTROLr(unit, &hash) < 0) {
                test_error(unit, "Hash select read failed\n");
                goto done;
            }
            if (READ_HASH_CONTROLr(unit, &hash1) < 0) {
                test_error(unit, "Hash select read failed\n");
                goto done;
            }
            soc_reg_field_set(unit, L2_TABLE_HASH_CONTROLr, &hash,
                              HASH_ZERO_OR_LSBf, 1);
            if (WRITE_L2_TABLE_HASH_CONTROLr(unit, hash) < 0) {
                test_error(unit, "Hash select setting failed\n");
                rv = -1;
                goto done;
            }
            soc_reg_field_set(unit, HASH_CONTROLr, &hash1,
                              L2_AND_VLAN_MAC_HASH_SELECTf, FB_HASH_LSB);
            if (WRITE_HASH_CONTROLr(unit, hash1) < 0) {
                test_error(unit, "Hash select setting failed\n");
                rv = -1;
                goto done;
            }
        ...
        case L3_ENTRY_IPV6_UNICASTm:
        case L3_ENTRY_IPV6_MULTICASTm:
            if (READ_L3_TABLE_HASH_CONTROLr(unit, &hash) < 0) {
                test_error(unit, "Hash select read failed\n");
                goto done;
            }
            if (READ_HASH_CONTROLr(unit, &hash1) < 0) {
            ...
            soc_reg_field_set(unit, L3_TABLE_HASH_CONTROLr, &hash,
                              HASH_ZERO_OR_LSBf, 1);
            if (WRITE_L3_TABLE_HASH_CONTROLr(unit, hash) < 0) {
                test_error(unit, "Hash select setting failed\n");
                rv = -1;
                goto done;
            }
    ...
    while (iter--) {
        for (ix = 0; ix < num_entries; ix++) {
            ...
            if (SOC_MEM_FIELD_VALID(unit, htd->mem, VALIDf)) {
                soc_mem_field32_set(unit, htd->mem, (entry_tmp[ix]), VALIDf, 1);
            } else {
                soc_mem_field32_set(unit, htd->mem, (entry_tmp[ix]), VALID_0f, 1);
                soc_mem_field32_set(unit, htd->mem, (entry_tmp[ix]), VALID_1f, 1);
                    if (SOC_MEM_FIELD_VALID(unit, htd->mem, VALID_2f)) {
                        soc_mem_field32_set(unit, htd->mem, (entry_tmp[ix]), VALID_2f, 1);
                        soc_mem_field32_set(unit, htd->mem, (entry_tmp[ix]), VALID_3f, 1);
                    }
            }
            switch (htd->mem) {
                case L2Xm:
            ...
            if (SOC_MEM_FIELD_VALID(unit, htd->mem, KEY_TYPEf)) {
                soc_mem_field32_set(unit, htd->mem, (entry_tmp[ix]), KEY_TYPEf, key_type);
            } else {
                if (SOC_MEM_FIELD_VALID(unit, htd->mem, KEY_TYPE_0f)) {
                    soc_mem_field32_set(unit, htd->mem, (entry_tmp[ix]), KEY_TYPE_0f, key_type);
                }
                if (SOC_MEM_FIELD_VALID(unit, htd->mem, KEY_TYPE_1f)) {
                    soc_mem_field32_set(unit, htd->mem, (entry_tmp[ix]), KEY_TYPE_1f, key_type);
                }
                if (SOC_MEM_FIELD_VALID(unit, htd->mem, KEY_TYPE_2f)) {
                    soc_mem_field32_set(unit, htd->mem, (entry_tmp[ix]), KEY_TYPE_2f, key_type);
                }
                if (SOC_MEM_FIELD_VALID(unit, htd->mem, KEY_TYPE_3f)) {
                    soc_mem_field32_set(unit, htd->mem, (entry_tmp[ix]), KEY_TYPE_3f, key_type);
                }
            }
            switch (htd->mem) {
                case L2Xm:
                    soc_mem_field32_set(unit, htd->mem, (entry_tmp[ix]), VFIf, vfi);
                    break;
                case L3_ENTRY_IPV4_UNICASTm:
                case L3_ENTRY_IPV4_MULTICASTm:
                case L3_ENTRY_IPV6_UNICASTm:
                case L3_ENTRY_IPV6_MULTICASTm:
                    soc_mem_field32_set(unit, htd->mem, (entry_tmp[ix]), VRF_IDf, vrf_id);
                    break;
                case ING_VP_VLAN_MEMBERSHIPm:
                case EGR_VP_VLAN_MEMBERSHIPm:
                    soc_mem_field32_set(unit, htd->mem, (entry_tmp[ix]), VPf, vp);
                    break;
                case ING_DNAT_ADDRESS_TYPEm:
                    soc_mem_field32_set(unit, htd->mem, (entry_tmp[ix]), VRFf, vrf_id);
                    break;
                case L2_ENDPOINT_IDm:
                    soc_mem_field32_set(unit, htd->mem, (entry_tmp[ix]), VFIf, vfi);
                    break;
                case ENDPOINT_QUEUE_MAPm:
                    soc_mem_field32_set(unit, htd->mem, (entry_tmp[ix]), DEST_PORTf, port);
                    break;
                case VLAN_XLATEm:
                case EGR_VLAN_XLATEm:
                    soc_mem_field32_set(unit, htd->mem, (entry_tmp[ix]), IVIDf, ivid);
                    break;
                case MPLS_ENTRYm:
                    soc_mem_field32_set(unit, htd->mem, (entry_tmp[ix]), PORT_NUMf, port);
            }

            if (ix == 0) {
#ifdef BCM_APACHE_SUPPORT
                if (SOC_IS_APACHE(unit)) {
                    soc_ap_hash_bucket_get(unit, htd->mem, 0, entry_tmp[ix], &bucket);
                    idx = soc_ap_hash_index_get(unit, htd->mem, 0, bucket);
                } else
#endif
                {
                    soc_hash_bucket_get(unit, htd->mem, 0, entry_tmp[ix], &bucket);
            ...
            if ((r = soc_mem_insert(unit, htd->mem, MEM_BLOCK_ALL, (entry_tmp[ix]))) < 0) {
                if (r == SOC_E_FULL) {
        ...
        for (jx = 0; jx < ix; jx++) {
            if (soc_mem_delete(unit, htd->mem, MEM_BLOCK_ALL, (entry_tmp[jx])) < 0) {
                test_error(unit, "Delete failed at bucket %d\n", bucket);
                rv = -1;
                goto done;
            }
