...
/* soc mem read-extended flags definition: */
#define SOC_MEMOP_SCHAN_ERR_RETURN 0x1

/* soc_mem_array_read_flags, flags definition: */
 ...
 */
extern int soc_mem_pipe_select_write(int unit,
                                     soc_mem_t mem,
                                     int copyno, /* COPYNO_ALL allowed */
                                     int acc_type,
                                     int index,
...
typedef struct soc_ser_test_functions_s {
    soc_error_t (*inject_error_f)(int, soc_mem_t, int, int, int);
...
extern soc_error_t soc_ser_inject_error(int unit, soc_mem_t mem, int pipe,
                            int blk, int index);
...
extern int soc_l3_defip_indexes_init(int unit);
