/*
 * $Id: a47d613cfaba446191e1f8e23c5c45e8b955b3ea $
...
#define PHY82328_SINGLE_PORT_MODE(_pc)           ((_pc)->speed_max >= PHY82328_SPD_40G)
...
static int eye_consol_val[64][64];
                                ...
                                uint8 *new_fw, uint32 datalen)
{
    ...
    SOC_IF_ERROR_RETURN
            (READ_PHY82328_MMF_PMA_PMD_REG(unit, pc, 
                     MGT_TOP_SPA_CONTROL_STATUS_REGISTER, &data16));
    if ((data16 & 0x2000) == 0x2000) {
        /* If FW already downloaded return success */
        return SOC_E_NONE;
    }
...
_phy_82328_init_pass2(int32 unit, soc_port_t port)
{
    ...
    line_intf->speed = PHY82328_SINGLE_PORT_MODE(pc) ? PHY82328_SPD_40G : PHY82328_SPD_10G;
    if (sys_intf->type == 0) {
		/* use defaults based on current line side */
        sys_intf->speed = line_intf->speed;
        line_intf->type = (line_intf->speed == PHY82328_SPD_40G) ? SOC_PORT_IF_SR4 : SOC_PORT_IF_SR;
        sys_intf->type =  (line_intf->speed == PHY82328_SPD_40G) ? SOC_PORT_IF_XLAUI : SOC_PORT_IF_XFI;
...
_phy_82328_debug_info(int32 unit, soc_port_t port)
{
    ...
    for (side = PHY82328_INTF_SIDE_LINE; side <= PHY82328_INTF_SIDE_SYS; side++) {
        ...
        if (PHY82328_SINGLE_PORT_MODE(pc)) {
            ...
            SOC_IF_ERROR_RETURN
                  (_phy_82328_control_preemphasis_get(unit, port, 
                    SOC_PHY_CONTROL_PREEMPHASIS, side, &preemphasis[0]));
        ...
        if (PHY82328_SINGLE_PORT_MODE(pc)) {
            soc_cm_print(
				...
				"           Preemphasis       : (ln0=%08x, ln1=%08x, ln2=%08x, ln3=%08x)\n",
                ...
                preemphasis[0], preemphasis[1], preemphasis[2], preemphasis[3]);
        } else {
            soc_cm_print(
				" %s: type=%s, speed=%d, forced cl72=%d, mode(1.%04x)=%04x\n"
				"           pmdtype(1.%04x)=%04x, lb(1.%04x)=%04x, txdis(1.%04x)=%04x\n"
				"           link(1.%04x)=%04x, pol(1.%04x)=%04x, Preemphasis:%08x",
				(side == PHY82328_INTF_SIDE_LINE) ? "Line    " : "System  ",
                phy82328_intf_names[intf->type], intf->speed, SYS_FORCED_CL72(pc),
                IEEE0_BLK_PMA_PMD_PMD_CONTROL_REGISTER, data1,
				IEEE0_BLK_PMA_PMD_PMD_CONTROL_2_REGISTER, data2,
...
_phy_82328_diag_dsc(int32 unit, soc_port_t port, int32 intf, int32 lane)
{
	...
	lanes = PHY82328_SINGLE_PORT_MODE(pc) ? PHY82328_NUM_LANES : 1;
                            ...
                            soc_port_ability_t *ability)
{
    ...
    if (_phy_82328_intf_line_forced(unit, port, line_intf->type)) {
        return SOC_E_NONE;
    }
