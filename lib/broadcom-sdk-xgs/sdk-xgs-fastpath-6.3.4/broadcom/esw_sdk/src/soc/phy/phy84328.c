/*
 * $Id: phy84328.c 1.126 Broadcom SDK $
...
static unsigned int phy84328_maj_rev = 1130;
...
STATIC int phy_84328_control_set(int unit, soc_port_t port, soc_phy_control_t type, uint32 value);
STATIC int phy_84328_control_get(int unit, soc_port_t port, soc_phy_control_t type, uint32 *value);
    ...
    if (chip_id_msb == 0x8) {
        if (chip_id_lsb == 0x4328) {
            ...
            *chip_id = PHY84328_ID_84024;
			...
			SOC_DEBUG_PRINT((DK_ERR, "PHY84328  bad chip id: u=%d p=%d chipid %x%x\n",
							 ...
							 phy84328_intf_side_t side, uint16 *reg_data, uint16 *reg_mask)
{
	...
	*reg_mask = 0;
    ...
    case SOC_PORT_IF_LR4:
        ...
        break;
    default:
        SOC_DEBUG_PRINT((DK_ERR, "84328 Unsupported interface: u=%d p=%d\n", unit, port));
    ...
    *reg_data |= (data | PHY84328_DEV1_GP_REGISTER_1_FINISH_CHANGE);
    ...
    SOC_DEBUG_PRINT((DK_PHY, "84328 intf type register: u=%d, p=%d, reg=%04x/%04x, intf=%d\n",
...
_phy_84328_micro_pause(int unit, soc_port_t port, const char *loc)
{
	...
	if (data & mask) {
		...
		SOC_DEBUG_PRINT((DK_PHY, "84328 microcode did not pause in %s: "
...
_phy_84328_micro_resume(int unit, soc_port_t port)
{
	...
	if (micro_ctrl->count <= 0) {
		...
		if (micro_ctrl->count < 0) {
			SOC_DEBUG_PRINT((DK_VERBOSE, "84328 unmatched micro resume\n"));
			micro_ctrl->count = 0;;
		}
	}
...
_phy_84328_config_update(int unit, soc_port_t port)
{
    ...
    if (rv != SOC_E_NONE) {
        SOC_DEBUG_PRINT((DK_ERR, "84328  failed updating config: u=%d port=%d\n", unit, port));
    }
...
{
	uint16		ucode_csr_bef, ucode_csr, drv_csr;
	phy84328_intf_side_t saved_side;
	soc_timeout_t to;
	int			rv = SOC_E_NONE;
	uint32		checks;
	...
	if (SOC_FAILURE(rv)) {
		SOC_DEBUG_PRINT((DK_ERR, "84328 failed reading ucode csr: u=%d p=%d err=%d\n",
						 unit, port, rv));
	...
	if ((ucode_csr_bef & PHY84328_DEV1_GP_REGISTER_3_FINISH_CHANGE_MASK) ==
		...
		if (SOC_FAILURE(rv)) {
			SOC_DEBUG_PRINT((DK_ERR, "84328 failed clearing ack: u=%d p=%d err=%d\n",
							 unit, port, rv));
			goto fail;
		}
    ...
    SOC_DEBUG_PRINT((DK_PHY, "84328 intf update register: u=%d, p=%d, 1.%04x=%04x/%04x ucode_csr=%04x\n",
	...
	rv = MODIFY_PHY84328_MMF_PMA_PMD_REG(unit, pc, PHY84328_DEV1_GP_REGISTER_1,
										 reg_data, reg_mask);
	if (SOC_FAILURE(rv)) {
		SOC_DEBUG_PRINT((DK_ERR, "84328 failed sending command to ucode: u=%d p=%d err=%d\n",
						 unit, port, rv));
		goto fail;
	}
	...
	if ((ucode_csr & PHY84328_DEV1_GP_REGISTER_3_FINISH_CHANGE_MASK) != 
		...
		SOC_DEBUG_PRINT((DK_ERR, "84328: microcode failed to ack: u=%d p=%d 1.%04x=%04x 1.%04x=%04x "
						 "1.%04x=%04x checks=%d\n", 
						 unit, port,
						 PHY84328_DEV1_GP_REGISTER_1, drv_csr,
 ...
 *  line side interface speed and sytem side interface type
 *
 *  Assumption:
...
_phy_84328_intf_line_sys_params_get(int unit, soc_port_t port)
{
	...
	if (line_intf->speed == 40000) {
		...
		if (!(_phy_84328_intf_is_single_port(sys_intf->type))) {
			...
			SOC_DEBUG_PRINT((DK_ERR, "PHY84328 incompatible 40G system side interface, "
		...
		if (_phy_84328_intf_is_quad_port(sys_intf->type)) {
			...
			/* system side interface is not compatible so overwrite with default */
			SOC_DEBUG_PRINT((DK_ERR, "PHY84328 incompatible 10G/1G system side interface, "
							 "using default: u=%d p=%d\n", unit, port));
...
_phy_84328_intf_speed_reg_get(int unit, soc_port_t port, int speed, uint16 *reg_data, uint16 *reg_mask)
{
	...
	*reg_mask = 0;
	...
	default :
		SOC_DEBUG_PRINT((DK_ERR, "84328 invalid line speed %d: u=%d p=%d\n",
						 speed, unit, port));
		return SOC_E_CONFIG;
	}
	...
	*reg_mask |= (PHY84328_DEV1_GP_REGISTER_1_SPEED_1G_MASK |
	...
	SOC_DEBUG_PRINT((DK_PHY, "84328 speed set register: u=%d, p=%d, reg=%04x/%04x), speed=%d\n",
								 ...
								 uint16 *reg_data, uint16 *reg_mask)
{
	...
	*reg_mask = 0;
	...
	default :
		SOC_DEBUG_PRINT((DK_ERR, "84328 invalid datapath: u=%d p=%d datapath=%d\n",
						 unit, port, datapath));
		return SOC_E_CONFIG;
	}
	*reg_data |= PHY84328_DEV1_GP_REGISTER_1_FINISH_CHANGE;	
	*reg_mask = (PHY84328_DEV1_GP_REGISTER_1_DATAPATH_MASK |
	...
	SOC_DEBUG_PRINT((DK_PHY, "84328 datapath set register: u=%d, p=%d, reg=%04x/%04x, datapath=%d\n",
...
_phy_84328_polarity_flip_tx(int unit, soc_port_t port, uint16 cfg_pol)
{
    ...
    if ((PHY84328_SINGLE_PORT_MODE(pc))) {
        ...
        for (lane = 0; lane < PHY84328_NUM_LANES; lane++) {
			...
			SOC_DEBUG_PRINT((DK_PHY, "84328 tx polarity flip=%d: u=%d p=%d lane=%d\n", flip, unit, port, lane));
		...
		SOC_IF_ERROR_RETURN(_phy_84328_polarity_flip_tx_set(unit, port, flip));
		SOC_DEBUG_PRINT((DK_PHY, "84328  polarity flip: u=%d p=%d\n", unit, port));
	}
...
_phy_84328_polarity_flip_rx_20bit(int unit, soc_port_t port, int flip)
{
	...
	return MODIFY_PHY84328_MMF_PMA_PMD_REG(unit, pc, PHY84328_DEV1_GP_REGISTER_RX_INV, data, mask);
}
...
_phy_84328_polarity_flip_rx_4bit(int unit, soc_port_t port, int flip)
{
	...
	data = flip ? 0 : PHY84328_DEV1_ANATXACONTROL4_XFI_LOWLATENCY_POLARITY_FLIP_MASK;
	mask = PHY84328_DEV1_ANATXACONTROL4_XFI_LOWLATENCY_POLARITY_FLIP_MASK;
	return MODIFY_PHY84328_MMF_PMA_PMD_REG(unit, pc, PHY84328_DEV1_ANATXACONTROL4, data, mask);
...
_phy_84328_polarity_flip_rx(int unit, soc_port_t port, uint16 cfg_pol)
{
    ...
    if ((PHY84328_SINGLE_PORT_MODE(pc))) {
        ...
        for (lane = 0; lane < PHY84328_NUM_LANES; lane++) {
			...
			SOC_DEBUG_PRINT((DK_PHY, "84328 rx polarity flip: u=%d p=%d lane=%d\n", unit, port, lane));
		}
		/* Restore to default single port register access */
		SOC_IF_ERROR_RETURN(_phy_84328_channel_select(unit, port, PHY84328_INTF_SIDE_LINE, PHY84328_ALL_LANES));
		_phy_84328_intf_side_regs_select(unit, port, PHY84328_INTF_SIDE_LINE);
		...
		SOC_DEBUG_PRINT((DK_PHY, "84328 rx polarity flip: u=%d p=%d\n", unit, port));
	}
...
{
	SOC_DEBUG_PRINT((DK_PHY, "84328 polarity flip: u=%d p%d TX=%x RX=%x\n", unit, port, 
...
_phy_84328_tx_mode_set(int unit, soc_port_t port, phy84328_intf_side_t side, uint16 tx_mode)
{
    ...
    if ((tx_mode != 0) && (tx_mode != 1) && (tx_mode != 4)) {
        SOC_DEBUG_PRINT((DK_ERR, "PHY84328 invalid tx mode configuration - must be 0, 1, or 4: u%d p%d\n",
                         unit, port));
        return SOC_E_NONE;
    }
...
_phy_84328_init_pass2(int unit, soc_port_t port)
{
	...
	SOC_DEBUG_PRINT((DK_PHY, "PHY84328 init pass2: u=%d p=%d\n", unit, port));
	pc = EXT_PHY_SW_STATE(unit, port);
	...
	if (if_sys_idx >= (sizeof(phy_sys_to_port_if) / sizeof(soc_port_if_t))) {
		SOC_DEBUG_PRINT((DK_ERR, "PHY84328 invalid system side interface: u=%d p=%d intf=%d\n"
						 "Using default interface\n",
						 unit, port, if_sys_idx));
		if_sys_idx = 0;
	}
	sys_intf->type = phy_sys_to_port_if[if_sys_idx];
	line_intf->speed = PHY84328_SINGLE_PORT_MODE(pc) ? 40000 : 10000;
	if (sys_intf->type == 0) {
...
_phy_84328_init_pass3(int unit, soc_port_t port)
{
    ...
    SOC_DEBUG_PRINT((DK_PHY, "PHY84328 init pass3: u=%d p=%d\n", unit, port));
    pc = EXT_PHY_SW_STATE(unit, port);
    int_pc = INT_PHY_SW_STATE(unit, port);
    ...
    SOC_DEBUG_PRINT((DK_PHY, "PHY84328 init pass3 polarity verified: u=%d p=%d\n", unit, port));
	...
	if (PHY84328_SINGLE_PORT_MODE(pc)) {
		...
		if (logical_lane0 > PHY84328_NUM_LANES) {
			SOC_DEBUG_PRINT((DK_ERR, "PHY84328 bad auto-negotiation lane %d: u=%d p=%d"
	...
	if (MOD_AUTO_DETECT(pc)) {
		...
		SOC_DEBUG_PRINT((DK_ERR, "PHY84328 module auto detect enabled: u=%d p=%d\n",
						 unit, port));
		...
		if (SOC_FAILURE(rv)) {
			...
			SOC_DEBUG_PRINT((DK_ERR, "PHY84328 setting module auto detect failed: u=%d p%d\n",
							 unit, port));
		} 
	}
    ...
    if (soc_property_port_get(unit, port, "phy_84328_port_4_5_control", 0)) {
        rv = MODIFY_PHY84328_MMF_PMA_PMD_REG(unit, pc, PHY84328_DEV1_GP_REG_0, 
                                             ...
                                             PHY84328_DEV1_GP_REG_0_P4_5_CTRL);
		if (SOC_FAILURE(rv)) {
			SOC_DEBUG_PRINT((DK_ERR, "PHY84328 setting p4_5 control failed: u=%d p%d\n",
							 unit, port));
		} 
    }
...
_phy_84328_core_init_done(int unit, struct phy_driver_s *pd, int primary_port)
{
	...
	PBMP_ALL_ITER(unit, port) {
		...
		rv = phy_84328_control_get(unit, port, SOC_PHY_CONTROL_PORT_PRIMARY,
								   &core_primary_port);
		if (rv != SOC_E_NONE) {
			continue;
		}
...
_phy_84328_init_pass1(int unit, soc_port_t port)
{
	...
	SOC_DEBUG_PRINT((DK_PHY, "PHY84328 init pass1: u=%d p=%d\n", unit, port));
	...
	SOC_IF_ERROR_RETURN(phy_84328_control_set(unit, port, SOC_PHY_CONTROL_PORT_PRIMARY,
											  primary_port));
	
	SOC_IF_ERROR_RETURN(phy_84328_control_set(unit, port,SOC_PHY_CONTROL_PORT_OFFSET,
											  offset));
		 ...
		 (!_phy_84328_core_init_done(unit, pc->pd, primary_port)))) {
            ...
            if ((csr_c848 & (1 << 13)) &&
               !(soc_property_port_get(unit, port,
                ...
                /*
                soc_cm_print("84328 u=%d p=%d skip fw download\n", unit, port);
                */
            } else {
                pc->flags |= PHYCTRL_MDIO_BCST;
            }
	...
	SOC_DEBUG_PRINT((DK_PHY, "phy_84328_init: u=%d p=%d setting primary=%d offset=%d\n",
...
phy_84328_init(int unit, soc_port_t port)
{
    ...
    SOC_DEBUG_PRINT((DK_PHY, "PHY84328 init: u=%d p=%d state=%d\n", 
    ...
    if ((PHYCTRL_INIT_STATE(pc) == PHYCTRL_INIT_STATE_PASS2) ||
        ...
        SOC_DEBUG_PRINT((DK_PHY, "PHY84328 init pass2 completed: u=%d p=%d\n", unit, port));
    ...
    if ((PHYCTRL_INIT_STATE(pc) == PHYCTRL_INIT_STATE_PASS3) ||
        ...
        SOC_DEBUG_PRINT((DK_PHY, "PHY84328 init pass3 completed: u=%d p=%d\n", unit, port));
...
phy_84328_an_get(int unit, soc_port_t port, int *an, int *an_done)
{
    ...
    SOC_DEBUG_PRINT((DK_PHY, "phy_84328_an_get: u=%d p=%d an=%d\n", unit, port, *an));
    return SOC_E_NONE;
}
...
phy_84328_an_set(int unit, soc_port_t port, int an)
{
    ...
    SOC_DEBUG_PRINT((DK_PHY, "phy_84328_an_set: u=%d p=%d an=%s\n",
                     unit, port, an ? "enable" : "disable"));


	/*
	 * Determining line side AN status 
	 *  SR/LR/SR4/LR4/CR: no AN 
	 *  KR/KR4: line side: cl73/cl72, system side as forced cl72
    ...
    if (line_intf->speed > 1000) {
        ...
        if (dev_an != AN_EN(pc)) {
            SOC_DEBUG_PRINT((DK_ERR, "84328 device autonegotiation mismatch: u=%d p=%d an=%s\n",
                           ...
                           soc_port_ability_t *ability)
{
    ...
    ability->speed_full_duplex = PHY84328_SINGLE_PORT_MODE(pc) ? SOC_PA_SPEED_40GB : SOC_PA_SPEED_10GB;
...
_phy_84328_mod_auto_detect_speed_check(int unit, soc_port_t port, uint16 csr)
{
	...
	if (line_intf->speed != speed_tbl[csr_speed]) {
		SOC_DEBUG_PRINT((DK_ERR,
						 "84328 module auto detection unexpected: u=%d p=%d speed=0x%x\n",
...
_phy_84328_mod_auto_detect_process(int unit, soc_port_t port, uint16 csr) 
{
	...
	SOC_DEBUG_PRINT((DK_ERR, "PHY84328 module detected: u=%d p=%d csr=%x\n", 
					 unit, port, csr));
...
_phy_84328_mod_auto_detect_update(int unit, soc_port_t port) 
{
	...
	if (ucode_csr & PHY84328_CSR_REG_MODULE_AUTO_DETECT) {
		...
		if (drv_csr & PHY84328_CSR_REG_MODULE_AUTO_DETECT) {
			...
			SOC_DEBUG_PRINT((DK_ERR, "PHY84328 module removed u=%d p=%d\n", unit, port));
		}
	}
...
_phy_84328_intf_link_get(int unit, soc_port_t port, uint16 *link)
{
	...
	*link = 0;
	max_lane = PHY84328_SINGLE_PORT_MODE(pc) ? PHY84328_NUM_LANES : 1;
...
phy_84328_link_get(int unit, soc_port_t port, int *link)
{
	...
	if (sw_rx_los->cur_enable) {
		...
		if (_phy_84328_sw_rx_los_check(unit, port, *link, &new_link) != SOC_E_NONE) {
			SOC_DEBUG_PRINT((DK_ERR, "84328  rx los check failed: u=%d port=%d\n", 
							 unit, port));
		}
		*link = new_link;
	}
...
_phy_84328_micro_tx_squelch_enable(int unit, soc_port_t port, int enable)
{
	...
	data = enable ? 0 : 0x80;
	return MODIFY_PHY84328_MMF_PMA_PMD_REG(unit, pc, PHY84328_DEV1_GP_REG_0, data, 0x80);
}
		...
		SOC_IF_ERROR_RETURN(
			MODIFY_PHY84328_MMF_PMA_PMD_REG(unit, pc, 
										PHY84328_DEV1_ANATXACONTROL6, 
											data,  
											PHY84328_DEV1_ANATXACONTROL6_DISABLE_TRANSMIT_MASK));
	} else {
...
phy_84328_lb_set(int unit, soc_port_t port, int enable)
{
	...
	if (CFG_DATAPATH(pc) != PHY84328_DATAPATH_20) {
		CUR_DATAPATH(pc) = enable ? PHY84328_DATAPATH_20 : CFG_DATAPATH(pc);
	 ...
	 *    enable tx line
	 */
	SOC_IF_ERROR_RETURN(_phy_84328_tx_enable(unit, port, PHY84328_INTF_SIDE_LINE, !enable));
	...
	if (SOC_FAILURE(rv)) {
		SOC_DEBUG_PRINT((DK_ERR, "84328  failed setting loopback: u=%d port=%d\n", unit, port));
	} 
	_phy_84328_intf_side_regs_select(unit, port, PHY84328_INTF_SIDE_LINE);
								 ...
								 phy84328_intf_side_t side, uint32 *value)
{
	...
	if (SOC_FAILURE(rv)) {
		SOC_DEBUG_PRINT((DK_ERR, "84328  tx driver get failed: u=%d p=%d\n", unit, port));
		*value = 0xff;
	} else {
...
_phy_84328_control_prbs_tx_invert_data_set(int unit, soc_port_t port, int invert)
{
	...
	return 
		MODIFY_PHY84328_MMF_PMA_PMD_REG(unit, pc, PHY84328_DEV1_ANATXACONTROL7,	data,
...
_phy_84328_control_prbs_polynomial_set(int unit, soc_port_t port, int poly_ctrl, int tx) 
{
	...
	SOC_DEBUG_PRINT((DK_PHY, "84328  prbs polynomial set: u=%d p=%d polynomial=%d\n",
				  unit, port, poly_ctrl));

	if (poly_ctrl < 0 || poly_ctrl > 3) {
		SOC_DEBUG_PRINT((DK_ERR, "84328  prbs invalid polynomial: u=%d p=%d polynomial=%d\n",
						 unit, port, poly_ctrl));
		return SOC_E_PARAM;
	}
...
STATIC int
_phy_84328_control_prbs_rx_status_get(int unit, soc_port_t port, uint32 *value)
{
	uint16		data;
	phy_ctrl_t  *pc = EXT_PHY_SW_STATE(unit, port);
	...
	*value = 0;
	...
	SOC_IF_ERROR_RETURN(
		MODIFY_PHY84328_MMF_PMA_PMD_REG(unit, pc, PHY84328_SYS_DEV1_ANARXCONTROL,
										7, PHY84328_DEV1_ANARXCONTROL_STATUS_SEL_MASK));
...
{    
	 ...
	 */
	SOC_IF_ERROR_RETURN(_phy_84328_tx_enable(unit, port, PHY84328_INTF_SIDE_SYS, !enable));
...
_phy_84328_debug_info(int unit, soc_port_t port)
{
	...
	SOC_IF_ERROR_RETURN(phy_84328_control_get(unit, port, SOC_PHY_CONTROL_PORT_PRIMARY, 
											  &primary_port));

	SOC_IF_ERROR_RETURN(phy_84328_control_get(unit, port, SOC_PHY_CONTROL_PORT_OFFSET,
											  &offset));
	...
	soc_cm_print("Port %-2d: rev=%d:%d, chiprev=%04x, micro ver(1.%04x)=%04x, micro(1.%04x)=%04x, die temp=%d(C)%s\n"
				 "     pri:offs=%d:%d, mdio=0x%x, datapath=%d, rxLOS=%s, mod auto=%s, sync=%s, link mon=%s\n",
				 port, phy84328_maj_rev, phy84328_min_rev,
				 DEVREV(pc),
				 PHY84328_DEV1_VERSION, data0,
				 PHY84328_DEV1_GP_REG_0, data1,
				 die_temp, data2 & PHY84328_DEV1_TEMPERATURE_FORCE_TEMPERATURE_MASK ? 
				 "(forced)" : "",
	...
	soc_cm_print("     PMD(1.%04x)=%04x, bcctrl(1.%04x)=%04x, pll(1.%04x)=%04x, an(7.%04x/1.%04x)=%04x/%04x\n"
	...
	counters = &(COUNTERS(pc));
	soc_cm_print(" Counters: link down=%d, intf updates=%d, swRxLOS restarts=%d, no cdr=%d, micro nopause=%d\n"
				 "           retry internal=%d, intf chk(speed=%d, type=%d, sel=%d)\n",
				 counters->link_down, counters->intf_updates, sw_rx_los->restarts, 
				 counters->no_cdr, counters->micro_nopause,
				 counters->retry_serdes_link,
				 counters->speed_chk_err, counters->intf_chk_err, counters->side_sel_err);
	...
	for (side = PHY84328_INTF_SIDE_LINE; side <= PHY84328_INTF_SIDE_SYS; side++) {
		...
		if (PHY84328_SINGLE_PORT_MODE(pc)) {
			soc_cm_print(
				...
				(side == PHY84328_INTF_SIDE_LINE) ? "Line    " : "System  ",
				phy84328_intf_names[intf->type], intf->speed, SYS_FORCED_CL72(pc),
				PHY84328_DEV1_PMD_CONTROL_REGISTER, data1,
				PHY84328_DEV1_PMD_CONTROL_2_REGISTER, data2,
				PHY84328_SYS_DEV1_ANATXACONTROL6, data3,
				PHY84328_DEV1_ANATXACONTROL2, data4,
				PHY84328_DEV1_ANATXACONTROL6, data5,
				PHY84328_DEV1_ANARXSTATUS, link_sts[0], link_sts[1], link_sts[2], link_sts[3],
				CUR_DATAPATH(pc) == PHY84328_DATAPATH_20 ? PHY84328_DEV1_ANATXACONTROL : PHY84328_DEV1_ANATXACONTROL4,
				pol_sts[0], pol_sts[1], pol_sts[2], pol_sts[3]);

			if (side == PHY84328_INTF_SIDE_LINE) {
				soc_cm_print(
				"           pol_inv(1.%04x)   : (ln0=%04x, ln1=%04x, ln2=%04x, ln3=%04x)\n",
				PHY84328_DEV1_GP_REGISTER_RX_INV, pol_inv_sts[0], pol_inv_sts[1], pol_inv_sts[2], pol_inv_sts[3]);
			}
			soc_cm_print(
				"           txdrv(1.%04x)     : (ln0=%04x, ln1=%04x, ln2=%04x, ln3=%04x)\n"
				...
				PHY84328_DEV1_ANATXACONTROL1, tx_drv[0], tx_drv[1], tx_drv[2], tx_drv[3],
				PHY84328_SYS_DEV1_XFI_AFE_CTL2, preemph_manual[0], preemph_manual[1], preemph_manual[2], preemph_manual[3],
				PHY84328_DEV1_ANATXACONTROL2, preemph_main[0], preemph_main[1], preemph_main[2], preemph_main[3],
				PHY84328_DEV1_ANATXACONTROL3, preemph_pre_post[0], preemph_pre_post[1], preemph_pre_post[2], preemph_pre_post[3]);
		} else {
			soc_cm_print(
				" %s: type=%s, speed=%d, forced cl72=%d, mode(1.%04x)=%04x\n"
				"           pmdtype(1.%04x)=%04x, lb(1.%04x)=%04x, txdis(1.%04x)=%04x/(1.%04x)=%04x\n"
				"           link(1.%04x)=%04x, txdrv(1.%04x)=%04x, pol(1.%04x)=%04x", 
				(side == PHY84328_INTF_SIDE_LINE) ? "Line    " : "System  ",
				phy84328_intf_names[intf->type], intf->speed, SYS_FORCED_CL72(pc),
				PHY84328_DEV1_PMD_CONTROL_REGISTER, data1,
				PHY84328_DEV1_PMD_CONTROL_2_REGISTER, data2,
				...
				pol_sts[0]);
			if (side == PHY84328_INTF_SIDE_LINE) {
				soc_cm_print(", pol_inv(1.%04x)=%04x", PHY84328_DEV1_GP_REGISTER_RX_INV, pol_inv_sts[0]);
			}
			soc_cm_print("\n"
	...
	if (int_pc) {
        ...
        SOC_IF_ERROR_RETURN(phy_84328_lb_get(unit, port, &lb));
        if (lb) {
                     ...
                     soc_phy_control_t type, uint32 *value)
{
    ...
    case SOC_PHY_CONTROL_PRBS_RX_STATUS:
        rv = _phy_84328_control_prbs_rx_status_get(unit, port, value);
        break;
    case SOC_PHY_CONTROL_CLOCK_FREQUENCY:
        break;
    case SOC_PHY_CONTROL_PORT_PRIMARY:
...
_phy_84328_diag_dsc(int unit, soc_port_t port, int intf, int lane)
{
	...
	lanes = PHY84328_SINGLE_PORT_MODE(pc) ? PHY84328_NUM_LANES : 1;
    ...
    for (i = 0; i < lanes; i++) {
        ...
        soc_cm_print("%02d %04d %07d %09d %04d %04d %04d %4s %04d %04d %04d %04d "
           ...
           i,dsc_cbp->integ_reg,dsc_cbp->integ_reg_xfer,dsc_cbp->clk90_offset,
           dsc_cbp->pf_ctrl,dsc_cbp->slicer_target,dsc_cbp->vga_sum,dsc_cbp->vga_bias_reduced,
           dsc_cbp->dfe1_bin,dsc_cbp->dfe2_bin,dsc_cbp->dfe3_bin,dsc_cbp->dfe4_bin,
           dsc_cbp->dfe5_bin,dsc_cbp->tx_pre_cursor,dsc_cbp->tx_main,dsc_cbp->tx_post_cursor,
           dsc_cbp->postc_metric,dsc_cbp->offset_pe,dsc_cbp->offset_ze,dsc_cbp->offset_me,
...
_phy_84328_diag_eyescan_uc_ready(int unit, soc_port_t port)
{
	...
	if (DBG_FLAGS(pc) & PHY84328_DBG_F_EYE) {
		soc_cm_print("    _phy_84328_diag_eyescan_uc_ready\n");
	}
	...
	if ((rv == SOC_E_NONE) &&
		...
		SOC_DEBUG_PRINT((DK_ERR, "84328 EYE: uController not ready pass 1!: u=%d, p=%d "
						 "uc_ctrl(1.%04x)=%04x\n", unit, port, 
...
_phy_84328_diag_eyescan_offset(int unit, soc_port_t port, int16 supp_data, int req)
{
	...
	if (DBG_FLAGS(pc) & PHY84328_DBG_F_EYE) {
		soc_cm_print("   _phy_84328_diag_eyescan_offset\n");
	}
...
_phy_84328_diag_eyescan_livelink_en(int unit, soc_port_t port, int en)
{
	...
	if (DBG_FLAGS(pc) & PHY84328_DBG_F_EYE) {
		soc_cm_print("_phy_84328_diag_eyescan_livelink_en: en=%d\n", en);
	}
	...
	SOC_IF_ERROR_RETURN(MODIFY_PHY84328_MMF_PMA_PMD_REG(unit, pc, 
														...
														data, mask));
	if (DBG_FLAGS(pc) & PHY84328_DBG_F_EYE) {
		soc_cm_print("_phy_84328_diag_eyescan_livelink_en: en=%d [%04x]\n", 
...
_phy_84328_diag_eyescan_livelink_clear(int unit, soc_port_t port, int unused)
{
	...
	if (DBG_FLAGS(pc) & PHY84328_DBG_F_EYE) {
		soc_cm_print("_phy_84328_diag_eyescan_livelink_clear: [%04x]\n", 
...
_phy_84328_diag_eyescan_livelink_start(int unit, soc_port_t port, int unused)
{
	...
	if (DBG_FLAGS(pc) & PHY84328_DBG_F_EYE) {
		soc_cm_print("_phy_84328_diag_eyescan_livelink_start: [%04x]\n", 
					 _phy_84328_diag_eyescan_diag_ctrl_get(unit, port));
	}
    return SOC_E_NONE;
}
...
_phy_84328_diag_eyescan_livelink_stop(int unit, soc_port_t port, int unused)
{
	...
	if (DBG_FLAGS(pc) & PHY84328_DBG_F_EYE) {
		soc_cm_print("_phy_84328_diag_eyescan_livelink_stop: [%04x]\n", 
					 _phy_84328_diag_eyescan_diag_ctrl_get(unit, port));
	} 
    return SOC_E_NONE;
}
...
_phy_84328_diag_eyescan_livelink_read(int unit, soc_port_t port, int *err_counter)
{
	...
	counter = 0;
	if (DBG_FLAGS(pc) & PHY84328_DBG_F_EYE) {
		soc_cm_print("_phy_84328_diag_eyescan_livelink_read: ");
	}
	for (i = 0; i < 4; i++) {
...
_phy_84328_diag_eyescan_init_voffset_get(int unit, soc_port_t port, int *voffset)
{
    ...
    *voffset = (data & PHY84328_DEV1_DSC_ANA_CTRL3_RSVD_FOR_ECO_MASK) >>
	...
	if (DBG_FLAGS(pc) & PHY84328_DBG_F_EYE) {
		soc_cm_print("_phy_84328_diag_eyescan_init_voffset_get: voffset=%x [%04x]\n", 
...
_phy_84328_diag_eyescan_hoffset_set(int unit, soc_port_t port, int *hoffset)
{
	...
	if (DBG_FLAGS(pc) & PHY84328_DBG_F_EYE) {
		soc_cm_print("_phy_84328_diag_eyescan_hoffset_set: hoffset=%x [%04x]\n", 
...
_phy_84328_diag_eyescan_voffset_set(int unit, soc_port_t port, int *voffset)
{
	...
	if (DBG_FLAGS(pc) & PHY84328_DBG_F_EYE) {
		soc_cm_print("_phy_84328_diag_eyescan_voffset_set: voffset=%x [%04x]\n", 
					 *voffset, _phy_84328_diag_eyescan_diag_ctrl_get(unit, port));
	}
    return SOC_E_NONE;
}
...
_phy_84328_diag_eyescan_max_left_hoffset_get(int unit, soc_port_t port, int *left_hoffset)
{
	...
	*left_hoffset = -data;
	...
	if (DBG_FLAGS(pc) & PHY84328_DBG_F_EYE) {
		soc_cm_print("_phy_84328_diag_eyescan_max_left_hoffset_get: max_left_hoffset=%d [%04x]\n", 
					 *left_hoffset, _phy_84328_diag_eyescan_diag_ctrl_get(unit, port));
	}
...
_phy_84328_diag_eyescan_max_right_hoffset_get(int unit, soc_port_t port, int *right_hoffset)
{
	...
	*right_hoffset = data;
	if (DBG_FLAGS(pc) & PHY84328_DBG_F_EYE) {
		soc_cm_print("_phy_84328_diag_eyescan_max_right_hoffset_get: max right hoffset=%d [%04x]\n", 
					 *right_hoffset, _phy_84328_diag_eyescan_diag_ctrl_get(unit, port));
	}
	return SOC_E_NONE;
}
...
_phy_84328_diag_eyescan_min_voffset_get(int unit, soc_port_t port, int *min_voffset)
{
	...
	*min_voffset = data - 256;
	...
	if (DBG_FLAGS(pc) & PHY84328_DBG_F_EYE) {
		soc_cm_print("_phy_84328_diag_eyescan_min_voffset_get: min_voffset=%d [%04x]\n", 
					 *min_voffset, _phy_84328_diag_eyescan_diag_ctrl_get(unit, port));
	}
	return SOC_E_NONE;
}
...
_phy_84328_diag_eyescan_max_voffset_get(int unit, soc_port_t port, int *max_voffset)
{
	...
	*max_voffset = data;
	if (DBG_FLAGS(pc) & PHY84328_DBG_F_EYE) {
		soc_cm_print("_phy_84328_diag_eyescan_max_voffset_get: max_voffset=%d [%04x]\n", 
					 *max_voffset, _phy_84328_diag_eyescan_diag_ctrl_get(unit, port));
	}
	return SOC_E_NONE;
}
...
STATIC int
phy_84328_diag_ctrl(int unit, soc_port_t port, uint32 inst, int op_type,  int op_cmd,  
					void *arg)   
{
    int lane;
    int intf;
    ...
    SOC_DEBUG_PRINT((DK_PHY, "84328 diag_ctrl: u=%d p=%d ctrl=0x%x\n", unit, port, op_cmd));
	...
	default:
		if (op_type == PHY_DIAG_CTRL_SET) {
			...
			SOC_DEBUG_PRINT((DK_PHY, "84328 diag_ctrl bad operation: u=%d p=%d ctrl=0x%x\n", 
							 unit, port, op_cmd));
			rv = SOC_E_UNAVAIL;
		}
		break;
		...
		if ((sys_intf->type == SOC_PORT_IF_GMII) || (sys_intf->type == SOC_PORT_IF_SGMII)) {
            ...
            if ((PHYCTRL_INIT_STATE(pc) == PHYCTRL_INIT_STATE_PASS2)) {
                /* Skip int_pc speed set during phy init */
                        ...
                        != SOC_E_NONE) {
                /*
                soc_cm_print("84328 u=%d p=%d speed get error\n", unit, port);
                */
                return rv;
            ...
            if (sys_intf->type != SOC_PORT_IF_CR4) {
                /* Skip int_pc fw mode set */
                        ...
                        != SOC_E_NONE) {
                /*
                soc_cm_print("84328 u=%d p=%d fwmode get error\n", unit, port);
                */
                return rv;
...
_phy_84328_intf_line_sys_init(int unit, soc_port_t port)
{
	...
	return _phy_84328_speed_set(unit, port, line_intf->speed);
}
...
phy_84328_speed_set(int unit, soc_port_t port, int speed)
{
    ...
    SOC_DEBUG_PRINT((DK_PHY,"phy_84328_speed_set: u=%d p=%d speed=%d\n", 
					 unit, port,speed));
    if (PHY84328_SINGLE_PORT_MODE(pc)) {
		...
		if (_phy_84328_intf_is_single_port(line_intf->type)) {
			SOC_DEBUG_PRINT((DK_ERR, "84328  invalid intf in quad port: u=%d p=%d intf=%d\n",
		...
		case 40000:
		default:
			SOC_DEBUG_PRINT((DK_ERR, "84328  invalid speed: u=%d p=%d speed=%d\n",
							 unit, port, speed));
			return SOC_E_PARAM;
		}
	}
	rv =  _phy_84328_speed_set(unit, port, speed);
	if (SOC_FAILURE(rv)) {
		SOC_DEBUG_PRINT((DK_ERR, "84328  %s failed: u=%d p=%d\n", FUNCTION_NAME(), unit, port));
	}
	return rv;
}
...
_phy_84328_intf_type_set(int unit, soc_port_t port, soc_port_if_t pif, int must_update)
{
    ...
    if (_phy_84328_intf_is_single_port(pif)) {
        ...
        if (!PHY84328_SINGLE_PORT_MODE(pc)) {
            SOC_DEBUG_PRINT((DK_ERR, "84328 invalid interface for quad port: u=%d p=%d\n",
                             unit, port));
            return SOC_E_CONFIG;
        }
		/* 
        ...
        if (PHY84328_SINGLE_PORT_MODE(pc)) {
            SOC_DEBUG_PRINT((DK_ERR, "84328 invalid interface for single port: u=%d p=%d\n",
                             unit, port));
            return SOC_E_CONFIG;
        }
        ...
        SOC_DEBUG_PRINT((DK_ERR, "84328 invalid interface for port: u=%d p=%d intf=%d\n",
                         unit, port, pif));
        return SOC_E_CONFIG;
    }
...
phy_84328_interface_set(int unit, soc_port_t port, soc_port_if_t pif)
{
	...
	SOC_DEBUG_PRINT((DK_PHY, "84328 interface set: u=%d p=%d pif=%s\n", 
	...
	if (SOC_FAILURE(rv)) {
		SOC_DEBUG_PRINT((DK_PHY, "84328  interface set check failed: u=%d p=%d\n",
						 unit, port));
	}

	if (_phy_84328_intf_line_forced(unit, port, pif)) {
		SOC_IF_ERROR_RETURN(phy_84328_an_set(unit, port, 0));
	}
...
_phy84328_init_ucode_bcst_load(int unit, int port)
{
	...
	if (pc->dev_name == dev_name_84328_a0) {
		...
		/* invalid device name */
		SOC_DEBUG_PRINT((DK_WARN | DK_VERBOSE, 
...
_phy84328_init_ucode_bcst(int unit, int port, int cmd)
{
    ...
    if (cmd == PHYCTRL_UCODE_BCST_SETUP) {
        ...
        if (data16 == 0x600D) {
			...
			SOC_DEBUG_PRINT((dbg_flag, "PHY84328 Firmware revID=0x%x: u=%d p=%d \n",
							 data16, unit, port));
        }
...
phy_84328_precondition_before_probe(int unit, phy_ctrl_t *pc)
{
    ...
    if (PHY84328_SINGLE_PORT_MODE(pc)) {
        SOC_DEBUG_PRINT((DK_PHY, "PMD must not be called when in single PMD\n"));
        return rv;
    }
    ...
    for (ids = 0; ids < 4; ids++) {
        ...
        SOC_DEBUG_PRINT((DK_PHY, "Try device with address %x\n", pc->phy_id));
                    ...
                    int addr, int count, void *data_array,uint32 ram_start)
{
    ...
    soc_cm_debug(DK_PHY,"phy_84328_bsc_read: u=%d p=%d addr=%04x\n",
                 unit, port, addr);
...
_phy_84328_dbg_read(int unit, phy_ctrl_t *pc, uint32 addr, uint16 *data)
{
	...
	if (DBG_FLAGS(pc) & PHY84328_DBG_F_REG) {
		soc_cm_print("p%d(%d): READ addr=%04x data=%04x\n", pc->port, pc->phy_id, addr, *data);
	}
	return rv;
...
_phy_84328_dbg_write(int unit, phy_ctrl_t *pc, uint32 addr, uint16 data)
{
	int rv;
	if (DBG_FLAGS(pc) & PHY84328_DBG_F_REG) {
		soc_cm_print("p%d(%d): WRITE addr=%04x data=%04x\n", pc->port, pc->phy_id, addr, data);
	}
...
_phy_84328_dbg_modify(int unit, phy_ctrl_t *pc, uint32 addr, uint16 data, uint16 mask)
{
	int rv;
	if (DBG_FLAGS(pc) & PHY84328_DBG_F_REG) {
		soc_cm_print("p%d(%d): MODIFY addr=%04x data=%04x mask=%04x\n", pc->port, pc->phy_id, 
					 addr, data, mask);
	}
