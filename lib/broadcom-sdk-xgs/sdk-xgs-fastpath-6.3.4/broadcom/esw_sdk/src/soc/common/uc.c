...
soc_uc_iproc_reset(int unit, int uC)
{
    ...
    if (SOC_IS_HELIX4(unit) || SOC_IS_KATANA2(unit) || SOC_IS_HURRICANE2(unit)) {
...
soc_uc_iproc_l2cache_purge(int unit, uint32 start, uint32 len)
{
    ...
    soc_cm_debug(DK_VERBOSE, "iproc_reset L2C_CACHE_ID 0x%08x\n", regval);

    READ_IHOST_L2C_CONTROLr(unit, &regval);
    if (regval & 0x01) {
        soc_cm_debug(DK_VERBOSE, "L2 cache enabled, clean %d bytes at 0x%08x\n",
             len, start);
    ...
    /* L2 cache disabled, skip */
        soc_cm_debug(DK_VERBOSE, "L2 cache disabled 0x%08x\n", regval);
    }
...
static
int
soc_uc_iproc_start(int unit, int uC, uint32 iproc_addr)
{
    uint32      i;
    uint32      regval;
