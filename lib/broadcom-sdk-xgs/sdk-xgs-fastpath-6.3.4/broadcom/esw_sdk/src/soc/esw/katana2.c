/*
 * $Id: katana2.c 1.162 Broadcom SDK $
...
static bcm56450_tdm_info_t kt2_scache_tdm_info={0};
static uint32 kt2_scache_tdm[KT2_MAX_TDM_SLOTS]={0};
static uint32 kt2_scache_speed[KT2_MAX_PHYSICAL_PORTS]={0};
static soc_port_details_t kt2_scache_port_details[KT2_MAX_PHYSICAL_PORTS+1]={{0}};
static uint8  kt2_scache_portgroup[KT2_MAX_MXQPORTS_PER_BLOCK * 
            ...
            SOC_BLOCK_ITER(unit, block_idx, route_block->blocktype) {
                if (SOC_BLOCK_INFO(unit, block_idx).number == route_block->id) {
                    ...
                    if (route_block->blocktype == SOC_BLK_MMU) {
                        SOC_IF_ERROR_RETURN(_soc_katana2_parity_enable_mmu(
                                            unit, route_block_en_reg, enable));
                    } else {
                                   ...
                                   soc_block_t block)
{
    ...
    if (multiple) {
        soc_cm_debug(DK_ERR, "%s %s has multiple parity errors\n",
    ...
    for (idx = 0; reg_ptr[idx].reg != INVALIDr; idx++) {
        ...
        if (soc_reg_field_valid(unit, reg, PARITY_ERRf)) {
            if (soc_reg_field_get(unit, reg, rval, PARITY_ERRf)) {
                ...
                _soc_katana2_parity_ser_correction(unit, info, prefix_str, 
                                                   mem_str_ptr, entry_idx, 
                                                   multiple, block);
        ...
        if (_soc_katana2_parity_route_blocks[group].blocktype == SOC_BLK_MMU) {
            ...
            SOC_BLOCK_ITER(unit, blk,_soc_katana2_parity_route_blocks[group].blocktype) {
                ...
                for (table = 0; info[table].type != _SOC_PARITY_TYPE_NONE; table++) {
                    ...
                    rv = ser_test_mem_pipe(unit,info[table].enable_reg, -1, info[table].enable_field,
    ...
    for (i = 0; _soc_kt2_tcam_ser_info[unit][i].mem != INVALIDm; i++) {
        if (_soc_kt2_tcam_ser_info[unit][i].mem == mem) {
            rv = ser_test_mem_pipe(unit, SER_RANGE_ENABLEr, i, -1,
                            _soc_kt2_tcam_ser_info[unit][i].mem, INVALIDf, test_type, MEM_BLOCK_ANY,
...
#define BCM56455_DEVICE_ID_MAX_CFG 29

/* Same for 56450(First 16(0..15) ) Total 18(0..17) <== 30+0=30..47 Configs  */
#define BCM56248_DEVICE_ID_OFFSET_CFG 30
#define BCM56248_DEVICE_ID_DEFAULT_CFG 34
#define BCM56248_DEVICE_ID_MAX_CFG 47
...
KT2_OLP_PORT,  25, KT2_CMIC_PORT, 27,            28, 26, 13, 9,        27,
28,            25, KT2_IDLE1,     KT2_IDLE1,     27, 26, 28, 13,       9,
27,            25, 28,            3,             13, 26, 27, 28,  KT2_LPBK_PORT,
9,             25, 27,            28,            5,  26, 13, 27,       28,
KT2_OLP_PORT, 25, 9,             27,            28, 26, 7,  13,       27,
28,            25, 1,             9,             27, 26, 28, KT2_IDLE, 13,
27,            25, 28,            KT2_CMIC_PORT, 9,  26, 27, 28,  KT2_LPBK_PORT,
13,            25, 27,            28,            6,  26, 9,  27,        28,
KT2_OLP_PORT, 25, 13,            27,            28, 26, 8,  9,         27,
28,            25, 2,             13,            27, 26, 28, KT2_IDLE,  9,
27,            25, 28,            4,             13, 26, 27, 28,  KT2_LPBK_PORT
};
const static uint32 kt2_tdm_56450A2_ref[108]= {
KT2_IDLE1,     25, 9,            28, 21,            26, 17,            27,   13,
KT2_CMIC_PORT, 25, 1,            28, 9,             26, 21,            27,   17,
KT2_OLP_PORT,  25, 13,           28, KT2_LPBK_PORT, 26, 9,             27,   21,
2,             25, 17,           28, 13,            26, KT2_CMIC_PORT, 27,   9,
3,             25, 21,           28, 17,            26, 13,            27,   7,
9,             25, 4,            28, 21,            26, 17,            27,   13,
KT2_OLP_PORT,  25, 9,            28, KT2_LPBK_PORT, 26, 21,            27,   17,
13,            25, 5,            28, 9,             26, KT2_CMIC_PORT, 27,   21,
17,            25, 13,           28, 6,             26, 9,             27,   KT2_IDLE1,
21,            25, 17,           28, 13,            26, KT2_LPBK_PORT, 27,   9,
KT2_OLP_PORT,  25, 21,           28, 17,            26, 13,            27,   8,
9,             25, 21,           28, 17,            26, 13,            27,   KT2_IDLE1
};
...
const static uint32 kt2_tdm_56452B_ref[90]= {
KT2_IDLE,      1, 27,            28,            5,  KT2_LPBK_PORT, 25,       26, 28,
27,	       1, KT2_OLP_PORT,  28,            5,  25,            KT2_IDLE, 26, 27,
28,	       1, KT2_LPBK_PORT, 25,            5,  28,            27,	     26, KT2_CMIC_PORT,
28,	       1, 25,	         KT2_OLP_PORT,  5,  27,	           28,	     26, KT2_IDLE,
25,	       1, 28,	         27,            5,  KT2_IDLE,	   28,	     26, 25,
KT2_CMIC_PORT, 1, 27,	         28,	        5,  KT2_LPBK_PORT, 25,	     26, 28,
27,	       1, KT2_IDLE1,	 KT2_IDLE1,     28, 5,	           25,	     26, 27,
28,	       1, KT2_OLP_PORT,	 25,	        5,  28,	           27,	     26, KT2_IDLE,
28,	       1, 25,	         KT2_LPBK_PORT,	5,  27,	           28,	     26, KT2_CMIC_PORT,
25,	       1, 28,	         27,	        5,  KT2_IDLE,	   28,	     26, 25
};
...
const static uint32 kt2_tdm_56248A3_ref[88] = {
...
static uint32 kt2_tdm_A1_56450_8[108]={0};
...
static uint32 kt2_tdm_A1_56450_9[108]={0};
...
static uint32 kt2_tdm_A2_56450_10[108]={0};
...
static uint32 kt2_tdm_A2_56450_11[108]={0};
...
static uint32 kt2_tdm_A2_56450_12[108]={0};
...
static uint32 kt2_tdm_A2_56450_13[108]={0};
...
static uint32 kt2_tdm_A2_56450_14[108]={0};
...
static uint32 kt2_tdm_A2_56450_15[108]={0};
...
/* 2x XE + HG[13] XE HG[13] HG[21]  + OLP */
...
/* 2xF.XAUI + 2xHG13 */
static uint32 kt2_tdm_D1_56454_1[20]={0};
...
/* 2xF.XAUI + 2xHG13 */
static uint32 kt2_tdm_D1_56454_2[20]={0};
...
const static kt2_speed_t kt2_speed_56248_16= {
       ...
       10000,0000,0000,0000,     /* 17 - 20  */
       10000,0000,0000,0000,     /* 21 - 24  */
       10000,0000,10000,13000,   /* 25 - 28  *//*WRONG.See below clarification*/
       0000,0000,0000,0000,     /* 29 - 32  */
       0000,0000,0000,0000,     /* 33 - 36  */
       0000,0000,0000,2500      /* 37 - 40  */
};
const static soc_port_details_t kt2_port_details_56248_16[]={
    ...
    {28,28,1,HG_PORT|STK_PORT,21000}, /* WRONG but doing for tdm verification*/
    {40,40,1,OLP_PORT,2500},/* OLP */
    {0,0,0,0} /* End */
};
...
static uint32 *bcm56450_tdm[]={
               ...
               kt2_tdm_A1_56450_9,               /*56450 Cfg=9:IntCfg=9 */
               kt2_tdm_A2_56450_10,              /*56450 Cfg=10:IntCfg=10 */
               kt2_tdm_A2_56450_11,              /*56450 Cfg=11:IntCfg=11 */
               kt2_tdm_A2_56450_12,              /*56450 Cfg=12:IntCfg=12 */
               kt2_tdm_A2_56450_13,              /*56450 Cfg=13:IntCfg=13 */
               kt2_tdm_A2_56450_14,              /*56450 Cfg=14:IntCfg=14 */
               kt2_tdm_A2_56450_15,              /*56450 Cfg=15:IntCfg=15 */
               ...
               kt2_tdm_56450config_deprecated0,  /*56248 Cfg=0:IntCfg=30 */
               kt2_tdm_A_56450_1,                /*56248 Cfg=1:IntCfg=31 */ 
               kt2_tdm_A_56450_2,                /*56248 Cfg=2:IntCfg=32 */
               kt2_tdm_A_56450_3,                /*56248 Cfg=3:IntCfg=33 */
               kt2_tdm_A_56450_4,                /*56248 Cfg=4:IntCfg=34 */
               kt2_tdm_A_56450_5,                /*56248 Cfg=5:IntCfg=35 */
               kt2_tdm_A_56450_6,                /*56248 Cfg=6:IntCfg=36 */
               kt2_tdm_A_56450_7,                /*56248 Cfg=7:IntCfg=37 */
               kt2_tdm_A1_56450_8,               /*56248 Cfg=8:IntCfg=38 */
               kt2_tdm_A1_56450_9,               /*56248 Cfg=9:IntCfg=39 */
               kt2_tdm_A2_56450_10,              /*56248 Cfg=10:IntCfg=40 */
               kt2_tdm_A2_56450_11,              /*56248 Cfg=11:IntCfg=41 */
               kt2_tdm_A2_56450_12,              /*56248 Cfg=12:IntCfg=42 */
               kt2_tdm_A2_56450_13,              /*56248 Cfg=13:IntCfg=43 */
               kt2_tdm_A2_56450_14,              /*56248 Cfg=14:IntCfg=44 */
               kt2_tdm_A2_56450_15,              /*56248 Cfg=15:IntCfg=45 */
               kt2_tdm_A3_56248_16,              /*56248 Cfg=16:IntCfg=46 */
               kt2_tdm_D2_56248_17               /*56248 Cfg=17:IntCfg=47 */
#ifdef BCM_WARM_BOOT_SUPPORT
               ,kt2_scache_tdm                   /*Dummy TDM reserved for WB */
#endif
};
const static uint32 *bcm56450_speed_s[]={
               ...
               kt2_tdm_56450speed_deprecated0,    /*56248 Cfg=0:IntCfg=30 */
               kt2_speed_56450_1,                 /*56248 Cfg=1:IntCfg=31 */ 
               kt2_speed_56450_2,                 /*56248 Cfg=2:IntCfg=32 */
               kt2_speed_56450_3,                 /*56248 Cfg=3:IntCfg=33 */
               kt2_speed_56450_4,                 /*56248 Cfg=4:IntCfg=34 */
               kt2_speed_56450_5,                 /*56248 Cfg=5:IntCfg=35 */
               kt2_speed_56450_6,                 /*56248 Cfg=6:IntCfg=36 */
               kt2_speed_56450_7,                 /*56248 Cfg=7:IntCfg=37 */
               kt2_speed_56450_8,                 /*56248 Cfg=8:IntCfg=38 */
               kt2_speed_56450_9,                 /*56248 Cfg=9:IntCfg=39 */
               kt2_speed_56450_10,                /*56248 Cfg=10:IntCfg=40 */
               kt2_speed_56450_11,                /*56248 Cfg=11:IntCfg=41 */
               kt2_speed_56450_12,                /*56248 Cfg=12:IntCfg=42 */
               kt2_speed_56450_13,                /*56248 Cfg=13:IntCfg=43 */
               kt2_speed_56450_14,                /*56248 Cfg=14:IntCfg=44 */
               kt2_speed_56450_15,                /*56248 Cfg=15:IntCfg=45 */
               kt2_speed_56248_16,                /*56248 Cfg=16:IntCfg=46 */
               kt2_speed_56248_17                 /*56248 Cfg=17:IntCfg=47 */
#ifdef BCM_WARM_BOOT_SUPPORT
               ,kt2_scache_speed                /* Dummy speed reserved for WB*/
#endif
};
...
const static soc_port_details_t  *kt2_port_details[]={
               ...
               kt2_port_details_deprecated0,      /*56248 Cfg=0:IntCfg=30 */
               kt2_port_details_56450_1,          /*56248 Cfg=1:IntCfg=31 */ 
               kt2_port_details_56450_2,          /*56248 Cfg=2:IntCfg=32 */
               kt2_port_details_56450_3,          /*56248 Cfg=3:IntCfg=33 */
               kt2_port_details_56450_4,          /*56248 Cfg=4:IntCfg=34 */
               kt2_port_details_56450_5,          /*56248 Cfg=5:IntCfg=35 */
               kt2_port_details_56450_6,          /*56248 Cfg=6:IntCfg=36 */
               kt2_port_details_56450_7,          /*56248 Cfg=7:IntCfg=37 */
               kt2_port_details_56450_8,          /*56248 Cfg=8:IntCfg=38 */
               kt2_port_details_56450_9,          /*56248 Cfg=9:IntCfg=39 */
               kt2_port_details_56450_10,         /*56248 Cfg=10:IntCfg=40 */
               kt2_port_details_56450_11,         /*56248 Cfg=11:IntCfg=41 */
               kt2_port_details_56450_12,         /*56248 Cfg=12:IntCfg=42 */
               kt2_port_details_56450_13,         /*56248 Cfg=13:IntCfg=43 */
               kt2_port_details_56450_14,         /*56248 Cfg=14:IntCfg=44 */
               kt2_port_details_56450_15,         /*56248 Cfg=15:IntCfg=45 */
               kt2_port_details_56248_16,         /*56248 Cfg=16:IntCfg=46 */
               kt2_port_details_56248_17          /*56248 Cfg=17:IntCfg=47 */
#ifdef BCM_WARM_BOOT_SUPPORT
               ,kt2_scache_port_details           /* Dummy port details
                                                     reserved for WB*/
#endif
};
static bcm56450_tdm_info_t bcm56450_tdm_info[]={
	       ...
	       {205,88,8,11},                     /*56450 Cfg=4:IntCfg=4 */
               ...
               {205,108,12,9},                    /*56450 Cfg=8:IntCfg=8 */
               {205,108,12,9},                    /*56450 Cfg=9:IntCfg=9 */
               {205,108,12,9},                    /*56450 Cfg=10:IntCfg=10 */
               {205,108,12,9},                    /*56450 Cfg=11:IntCfg=11 */
               {205,108,12,9},                    /*56450 Cfg=12:IntCfg=12 */
               {205,108,12,9},                    /*56450 Cfg=13:IntCfg=13 */
               {205,108,12,9},                    /*56450 Cfg=14:IntCfg=14 */
               {205,108,12,9},                    /*56450 Cfg=15:IntCfg=15 */
               ...
               {205,88,8,11},                     /*56248 Cfg=0:IntCfg=30 */
               {205,88,8,11},                     /*56248 Cfg=1:IntCfg=31 */ 
               {205,88,8,11},                     /*56248 Cfg=2:IntCfg=32 */
               {205,88,8,11},                     /*56248 Cfg=3:IntCfg=33 */
	       {205,88,8,11},                     /*56248 Cfg=4:IntCfg=34 */
               {205,88,8,11},                     /*56248 Cfg=5:IntCfg=35 */
               {205,88,8,11},                     /*56248 Cfg=6:IntCfg=36 */
               {205,88,8,11},                     /*56248 Cfg=7:IntCfg=37 */
               {205,108,12,9},                    /*56248 Cfg=8:IntCfg=38 */
               {205,108,12,9},                    /*56248 Cfg=9:IntCfg=39 */
               {205,108,12,9},                    /*56248 Cfg=10:IntCfg=40 */
               {205,108,12,9},                    /*56248 Cfg=11:IntCfg=41 */
               {205,108,12,9},                    /*56248 Cfg=12:IntCfg=42 */
               {205,108,12,9},                    /*56248 Cfg=13:IntCfg=43 */
               {205,108,12,9},                    /*56248 Cfg=14:IntCfg=44 */
               {205,108,12,9},                    /*56248 Cfg=15:IntCfg=45 */
               {205,88,8,11},                     /*56248 Cfg=16:IntCfg=46 */
               {123,40,8,5}                       /*56248 Cfg=16:IntCfg=46 */
#ifdef BCM_WARM_BOOT_SUPPORT
...
/*    } */
    ...
    SOC_PBMP_ITER (valid_port_cpu_pbmp, port) {
        if (SOC_PBMP_MEMBER(pbmp_subport, port)) {
            num_subports += ceil_func(soc_property_port_get(unit, port, 
                                      spn_NUM_SUBPORTS, 0),8);
        }
        if (!SOC_PBMP_MEMBER(PBMP_EXT_MEM (unit), port) &&
            (port >= KT2_CMIC_PORT) && (port <= KT2_LPBK_PORT)) {
            op_node = ( si->port_cosq_base[port] / 8);
            if (port != KT2_LPBK_PORT) {
                op_node -=  num_subports;
            } 
            for (op_node_offset = 0; 
                 op_node_offset < ceil_func(si->port_num_uc_cosq[port],8); 
                 op_node_offset++) {
    ...
    /* ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
    for (sub_port = KT2_MIN_SUBPORT_INDEX;
         sub_port < KT2_MAX_SUBPORTS;
         sub_port ++) {
    ...
    /* (Assuming lossless-internal memory operation)               */
    /* ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
    for (queue = si->port_uc_cosq_base[KT2_MAX_PHYSICAL_PORTS] +
                           ...
                           uint32 *offender_port)
{
   ...
   if (kt2_tdm_port_slots_info[KT2_LPBK_PORT].position == -1) {
       soc_cm_print("Loopback port  missing in configuration \n");
       return SOC_E_FAIL;
   }
   if (kt2_tdm_port_slots_info[KT2_CMIC_PORT].position == -1) {
       soc_cm_print("CMIC port  missing in configuration \n");
       return SOC_E_FAIL;
   }
   if (olp_port_flag) {
       if (kt2_tdm_port_slots_info[KT2_OLP_PORT].position == -1) {
           soc_cm_print("OLP port  missing in configuration \n");
           return SOC_E_FAIL;
       }
   }
           ...
           do {
              for(loop=0;loop<KT2_MAX_MXQPORTS_PER_BLOCK; loop++) {
                  ...
                  do {
                       ...
                       if (spacing < 3) {
                           soc_cm_debug(DK_ERR,
                               "Outer Port:%d Inner Port:%d MXQSpacing Issue"
...
_soc_katana2_mmu_tdm_init(int unit)
{
    ...
    case BCM56458_DEVICE_ID:
         ...
         case BCM56450_DEVICE_ID_OFFSET_CFG + 0:
                ...
                soc_cm_print("kt2_tdm_56450speed_deprecated0\n");
                sal_memcpy(arr,
         ...
         case BCM56450_DEVICE_ID_OFFSET_CFG + 1:/* kt2_tdm_A_56450_1 */
                soc_cm_print("kt2_tdm_A_56450_1 \n");
                sal_memcpy(arr,
         ...
         case BCM56450_DEVICE_ID_OFFSET_CFG + 2:/* kt2_tdm_A_56450_2 */
                soc_cm_print("kt2_tdm_A_56450_2 \n");
                sal_memcpy(arr,
                           &kt2_tdm_56450A_ref[0], sizeof(kt2_tdm_56450A_ref));
         ...
         case BCM56450_DEVICE_ID_OFFSET_CFG + 3:/* kt2_tdm_A_56450_3 */
                soc_cm_print("kt2_tdm_A_56450_3 \n");
                sal_memcpy(arr,
                           &kt2_tdm_56450A_ref[0],sizeof(kt2_tdm_56450A_ref));
                kt2_tdm_replace(arr,88,33,25,0);
                olp_port_flag=1;
         ...
         case BCM56450_DEVICE_ID_OFFSET_CFG + 4:/* kt2_tdm_A_56450_4 */
                soc_cm_print("kt2_tdm_A_56450_4 \n");
                sal_memcpy(arr,
                           &kt2_tdm_56450A_ref[0], sizeof(kt2_tdm_56450A_ref));
                olp_port_flag=1;
                break;
         ...
         case BCM56450_DEVICE_ID_OFFSET_CFG + 5:/* kt2_tdm_A_56450_5 */
                soc_cm_print("kt2_tdm_A_56450_5 \n");
                sal_memcpy(arr,
                           &kt2_tdm_56450A_ref[0], sizeof(kt2_tdm_56450A_ref));
         ...
         case BCM56450_DEVICE_ID_OFFSET_CFG + 6:/*  kt2_tdm_A_56450_6 */
                soc_cm_print("kt2_tdm_A_56450_6 \n");
                sal_memcpy(arr,
                           &kt2_tdm_56450A_ref[0], sizeof(kt2_tdm_56450A_ref));
         ...
         case BCM56450_DEVICE_ID_OFFSET_CFG + 7:/* kt2_tdm_A_56450_7 */
                soc_cm_print("kt2_tdm_A_56450_7 \n");
                sal_memcpy(arr,
                           &kt2_tdm_56450A_ref[0], sizeof(kt2_tdm_56450A_ref));
                kt2_tdm_swap(arr,88,9,17);
                kt2_tdm_replace(arr,88,17,26,0);
         ...
         case BCM56450_DEVICE_ID_OFFSET_CFG + 8:/* kt2_tdm_A1_56450_8 */
                soc_cm_print("kt2_tdm_A1_56450_8 \n");
                sal_memcpy(arr,
         ...
         case BCM56450_DEVICE_ID_OFFSET_CFG + 9:/* kt2_tdm_A1_56450_9 */
                soc_cm_print("kt2_tdm_A1_56450_9 \n");
                sal_memcpy(arr,
                           &kt2_tdm_56450A1_ref[0],
                           sizeof(kt2_tdm_56450A1_ref));
         ...
         case BCM56450_DEVICE_ID_OFFSET_CFG + 12: /* kt2_tdm_A2_56450_12 */
                  soc_cm_print("kt2_tdm_A2_56450_12 \n");
                  sal_memcpy(arr,
                             &kt2_tdm_56450A2_ref[0],
                             sizeof(kt2_tdm_56450A2_ref));
                  kt2_tdm_swap(arr,108,26,27);
                  kt2_tdm_replace(arr,108,26,28,0);
                  olp_port_flag=1;
                  break;
         case BCM56248_DEVICE_ID_OFFSET_CFG + 13:
         case BCM56450_DEVICE_ID_OFFSET_CFG + 13: /* kt2_tdm_A2_56450_13 */
                  soc_cm_print("kt2_tdm_A2_56450_13 \n");
                  sal_memcpy(arr,
                             &kt2_tdm_56450A2_ref[0],
                             sizeof(kt2_tdm_56450A2_ref));
                  kt2_tdm_swap(arr,108,26,27);
                  kt2_tdm_replace(arr,108,26,28,0);
                  kt2_tdm_replace(arr,108,25,27,0);
                  olp_port_flag=1;
                  break;
         case BCM56248_DEVICE_ID_OFFSET_CFG + 14:
         case BCM56450_DEVICE_ID_OFFSET_CFG + 14: /* kt2_tdm_A2_56450_14 */
                  soc_cm_print("kt2_tdm_A2_56450_14 \n");
                  sal_memcpy(arr,
                             &kt2_tdm_56450A2_ref[0],
                             sizeof(kt2_tdm_56450A2_ref));
                  kt2_tdm_replace(arr,108,21,KT2_IDLE,0);
                  kt2_tdm_replace(arr,108,17,KT2_IDLE,0);
                  olp_port_flag=1;
                  break;
         case BCM56248_DEVICE_ID_OFFSET_CFG + 15:
         case BCM56450_DEVICE_ID_OFFSET_CFG + 15: /* kt2_tdm_A2_56450_15 */
                  soc_cm_print("kt2_tdm_A2_56450_15 \n");
                  memcpy(arr,
                         &kt2_tdm_56450A2_ref[0],
                         sizeof(kt2_tdm_56450A2_ref));
                  kt2_tdm_replace(arr,108,21,KT2_IDLE,0);
                  kt2_tdm_replace(arr,108,17,KT2_IDLE,0);
                  kt2_tdm_replace(arr,108,9,KT2_IDLE,0);
                  kt2_tdm_replace(arr,108,13,17,0);
                  for(port=1;port<=8;port++) {
         ...
         case BCM56452_DEVICE_ID_OFFSET_CFG + 7:
                soc_cm_print("kt2_tdm_B_56452_7 \n");
                sal_memcpy(arr,
                          &kt2_tdm_56452B_ref[0], sizeof(kt2_tdm_56452B_ref));
                kt2_tdm_replace(arr,90,1,60,0);
                kt2_tdm_replace(arr,90,5,60,0);
         ...
         case BCM56455_DEVICE_ID_OFFSET_CFG + 1:
                soc_cm_print("kt2_tdm_C_56455_1 \n");
                sal_memcpy(arr,
                          &kt2_tdm_56455C_ref[0], sizeof(kt2_tdm_56455C_ref));
                olp_port_flag=0;
                break;
         case BCM56248_DEVICE_ID_OFFSET_CFG + 16:
                soc_cm_print("kt2_tdm_A3_56248_16 \n");
                sal_memcpy(arr,
                          &kt2_tdm_56248A3_ref[0], sizeof(kt2_tdm_56248A3_ref));
                olp_port_flag=1;
                break;
         case BCM56248_DEVICE_ID_OFFSET_CFG + 17:
                soc_cm_print("kt2_tdm_D2_56248_17 \n");
                sal_memcpy(arr,
         ...
         case COUNTOF(bcm56450_tdm_info) - 1:
                if (SOC_WARM_BOOT(unit)) {
                    sal_memcpy(arr,
                               &kt2_scache_tdm[0], sizeof(kt2_scache_tdm));
                    break;
                }
#endif
         default: olp_port_flag=0;
                soc_cm_print("Not Supported Cfg Value%d \n",cfg_num);
                return SOC_E_FAIL;
         }
         kt2_tdm_display(unit,arr, tdm_size,  row,  col);
         /* speed = bcm56450_speed[cfg_num]; */
         retVal=kt2_tdm_verify(unit,
                               (uint32 *)bcm56450_tdm[cfg_num],
                               tdm_size,tdm_freq,
...
soc_katana2_port_enable_set(int unit, soc_port_t port, int enable)
{
   ...
   if ((port == KT2_OLP_PORT) && (si->olp_port == 1) ) {
        /*
        soc_cm_print("QUICKTURN:ATTN:Olp Trick Continue"
...
soc_katana2_pbmp_init(int unit, kt2_pbmp_t kt2_pbmp)
{
    ...
    if (mxqspeeds[unit] == NULL) {
        soc_cm_print("Resource issue %s:%d..Couldn't allocate memory=%d \n",
    ...
    if (kt2_selected_port_details == NULL) {
        soc_cm_print("INTERNAL: Not Supported CFG VALUE =%d\n", 
        kt2_port_details_index);
        return ;
    }
    loop_index=0;
               ...
               if (kt2_selected_port_details[loop_index].port_type & HG_PORT) {
                   if (SOC_PBMP_MEMBER(my_pbmp_xport_xe, port)) {
                       ...
                       if (bcm56450_speed[unit][kt2_port_details_index][port-1] >=
                           10000) {
                           soc_cm_debug(DK_VERBOSE,"Original: %d \n",
                               bcm56450_speed[unit][kt2_port_details_index][port-1]);
                           bcm56450_speed[unit][kt2_port_details_index][port-1]=10000;
                           soc_cm_debug(DK_VERBOSE,"Updated: %d \n",
                               bcm56450_speed[unit][kt2_port_details_index][port-1]);
                       } else {
                           soc_cm_print("Wrong Original Setting: %d\n",
                               bcm56450_speed[unit][kt2_port_details_index][port-1]);
                       }
                   } else {
                   switch(kt2_selected_port_details[loop_index].port_speed) {
...
int _soc_katana2_misc_init(int unit)
{
    ...
    *  2 instances one each for evlan and ehcpm stages of epipe */
    ...
    if (bcm56450_tdm[cfg_num] == NULL ) {
        soc_cm_print("Internal: Not implemented Cfg => %d \n",cfg_num);
        return SOC_E_FAIL;
    }
    ...
    SOC_BLOCK_ITER(unit, blk, SOC_BLK_MXQPORT) {
        ...
        for (loop=0;loop<KT2_MAX_MXQPORTS_PER_BLOCK; loop++) {
             if (SOC_PBMP_MEMBER((PBMP_ALL(unit)), 
                 ...
                 if (((*kt2_mxqblock_ports[unit])[mxqblock][loop] == KT2_OLP_PORT) &&
                     (si->olp_port == 1) ) {
                     /*
                     soc_cm_print("QUICKTURN:ATTN:Olp Trick"
                                  "Need RegfileChange MXQ:%d:\n",mxqblock);
                      */
        ...
        SOC_IF_ERROR_RETURN(soc_katana2_get_phy_port_mode(
                            unit, any_phy_port,
                            bcm56450_speed[unit][cfg_num][any_phy_port-1], 
                            &phy_mode));
    ...
    if (SOC_PBMP_MEMBER((PBMP_ALL(unit)), port)) {
        ...
        *nxtaddr += depth;
        if (*nxtaddr > max_depth_limit) {
            soc_cm_print("FATAL: Internal Error: Limit=%d crossed "
...
_soc_katana2_mmu_init1(int unit) 
{
    ...
    for (mxqblock=0;mxqblock<KT2_MAX_MXQBLOCKS ; mxqblock++) {
     ...
     /* cpu port */
     SOC_IF_ERROR_RETURN(_soc_katana2_mmu_init1_port(unit,0,&nxtaddr));
     /* lb port */
    SOC_IF_ERROR_RETURN(_soc_katana2_mmu_init1_port(unit,41,&nxtaddr));
    for (mxqblock=0;mxqblock<KT2_MAX_MXQBLOCKS ; mxqblock++) {
         for (subport=0;subport<KT2_MAX_MXQPORTS_PER_BLOCK; subport++) {
              port = (*kt2_mxqblock_ports[unit])[mxqblock][subport];
              SOC_IF_ERROR_RETURN(_soc_katana2_mmu_init1_port(
                                  unit,port,&nxtaddr));
         }
    }
    for (mxqblock=0;mxqblock<KT2_MAX_MXQBLOCKS ; mxqblock++) {
         ...
         if (((port > KT2_CMIC_PORT) && (port < KT2_LPBK_PORT) &&
             ...
             if (soc_reg_field_valid(unit, MMU_ENQ_IP_PRI_TO_PG_PROFILE_0r, 
                 ...
                 if (!((port == KT2_OLP_PORT) && 
                       (SOC_INFO(unit).olp_port == 1))) {
                        soc_cm_print("WARN: Suspicious Config:%s:%d Port=%d\n",
...
int _soc_katana2_get_cfg_num(int unit, int *new_cfg_num)
{
    ...
    if (SOC_WARM_BOOT(unit)) {
       ...
       if (bcm56450_tdm_info[cfg_num].tdm_size != 0) {
           *new_cfg_num = cfg_num;
           return SOC_E_NONE;       
       }
       /* FAll through */
    ...
    if (cfg_num == 0xFF) {
        *new_cfg_num = def_cfg_num;
        ...
        if (cfg_num == 0) {
            if (offset_cfg_num != 0) {
                soc_cm_print("Config=0 is not allowed \n");
                return BCM_E_PARAM; 
            }
        } 
    ...
    if (*new_cfg_num > max_cfg_num) {
        soc_cm_print("Cfg_num:%d Internal cfg_num:%d \n",cfg_num,*new_cfg_num);
        soc_cm_print("Wrong %s ==> cfg_num:%d DEFAULT:%d START:%d MAX:%d \n",
                      spn_BCM5645X_CONFIG, cfg_num, 
                      def_cfg_num-offset_cfg_num,1,max_cfg_num-offset_cfg_num);
        return BCM_E_PARAM; 
    }
    soc_cm_print("Info Cfg_num:%d Internal cfg_num:%d \n",cfg_num,*new_cfg_num);
    return BCM_E_NONE;
}
...
/* #ifdef BROADCOM_SVK  */
   ...
   if (auto_polarity_flip) {
       ...
       for (mxqblock = 0; mxqblock < 8; mxqblock++) {
            for (loop=0; loop < KT2_MAX_MXQPORTS_PER_BLOCK; loop++) {       
                 if ((*kt2_mxqblock_ports[unit])[mxqblock][loop] == 0xFF) {
                     continue;
                 }
                 if ((*kt2_mxqblock_ports[unit])[mxqblock][loop] == 40) {
                     if (SOC_INFO(unit).olp_port) {
                         continue;
                     }
                 }
       ...
       sal_sprintf(config_str,"phy_xaui_rx_polarity_flip_%d",
                               (*kt2_mxqblock_ports[unit])[9][0]);
       _soc_kt2_mem_config_set(unit,config_str,config_value);
   }

   if (auto_portgroup) {
       if (SOC_WARM_BOOT(unit)) {
           soc_cm_print("Broadcom SVK..Skipping required port groups \n");
           return;
       }
       soc_cm_print("Broadcom SVK..Adding required port groups \n");
       _soc_kt2_mem_config_set(unit,"portgroup_27",NULL);
       _soc_kt2_mem_config_set(unit,"portgroup_28",NULL);
       _soc_kt2_mem_config_set(unit,"portgroup_29",NULL);
       _soc_kt2_mem_config_set(unit,"portgroup_30",NULL);
       _soc_kt2_mem_config_set(unit,"portgroup_31",NULL);
       _soc_kt2_mem_config_set(unit,"portgroup_32",NULL);
       _soc_kt2_mem_config_set(unit,"portgroup_33",NULL);
       _soc_kt2_mem_config_set(unit,"portgroup_34",NULL);
       switch(cfg_num) {
       case BCM56248_DEVICE_ID_OFFSET_CFG + 0:
       case BCM56450_DEVICE_ID_OFFSET_CFG + 0:
       ...
       case BCM56452_DEVICE_ID_OFFSET_CFG + 3:
       case BCM56452_DEVICE_ID_OFFSET_CFG + 4:
              _soc_kt2_mem_config_set(unit,"portgroup_25","4");
              _soc_kt2_mem_config_set(unit,"portgroup_26","4");
              _soc_kt2_mem_config_set(unit,"portgroup_27","4");
              _soc_kt2_mem_config_set(unit,"portgroup_28","4");
              break; 
       case BCM56452_DEVICE_ID_OFFSET_CFG + 5:
...
int _soc_katana2_flexio_scache_allocate(int unit)
{
    ...
    uint32 alloc_size =  sizeof(scache_begin_magic_number) +
                         sizeof(kt2_scache_tdm_info) + 
                         sizeof(kt2_scache_tdm) +
                         sizeof(kt2_scache_speed) +
                         sizeof(kt2_scache_port_details) +
                         sizeof(kt2_scache_portgroup) +
    ...
    soc_cm_debug(DK_VERBOSE, "%s()[LINE:%d] DONE \n",__FUNCTION__,  __LINE__);
    return SOC_E_NONE;
}
int _soc_katana2_flexio_scache_retrieve(int unit)
{
    ...
    if(sal_memcmp(&flexio_scache_ptr[scache_offset],
       ...
       soc_cm_print("Corrupt scache for flexio operation \n");
       return SOC_E_INTERNAL;
    }

    scache_offset += sizeof(scache_begin_magic_number);
    sal_memcpy(&kt2_scache_tdm_info,
               &flexio_scache_ptr[scache_offset], sizeof(kt2_scache_tdm_info));

    bcm56450_tdm_info[COUNTOF(bcm56450_tdm_info)-1] = kt2_scache_tdm_info;

    scache_offset += sizeof(kt2_scache_tdm_info);
    sal_memcpy(&kt2_scache_tdm,
               &flexio_scache_ptr[scache_offset], sizeof(kt2_scache_tdm));

    scache_offset += sizeof(kt2_scache_tdm);
    sal_memcpy(&kt2_scache_speed,
               &flexio_scache_ptr[scache_offset], sizeof(kt2_scache_speed));

    scache_offset += sizeof(kt2_scache_speed);
    sal_memcpy(&kt2_scache_port_details,
               &flexio_scache_ptr[scache_offset], 
               sizeof(kt2_scache_port_details));

    scache_offset += sizeof(kt2_scache_port_details);
    for (offset=0; offset < sizeof(kt2_scache_portgroup);offset++) {
         kt2_scache_portgroup[offset] = flexio_scache_ptr[scache_offset];
         sal_sprintf(config_value,"%d",kt2_scache_portgroup[offset]);
         sal_sprintf(config_str,"portgroup_%d",offset + 27);
         if (kt2_scache_portgroup[offset]) {
             _soc_kt2_mem_config_set(unit,config_str,config_value);
         }
         scache_offset += 1;
    }
    ...
    soc_cm_debug(DK_VERBOSE, "%s()[LINE:%d] \n",__FUNCTION__,  __LINE__);
...
int _soc_katana2_flexio_scache_sync(int unit)
{
    ...
    for (port=1 ; port <= KT2_MAX_PHYSICAL_PORTS ; port++) {
         /* mxqblock = (*kt2_port_to_mxqblock[unit])[port-1]; */
         kt2_scache_speed[port] = 0;
         if (IS_GE_PORT(unit,port)) {
             if ((bcm56450_speed[unit][cfg_num][port-1] == 0) || /*Converted */
                 (bcm56450_speed[unit][cfg_num][port-1] >= 2500)) {
                  kt2_scache_speed[port - 1] = 2500;
             } else {
                 kt2_scache_speed[port - 1] = 1000;
             }
         } 
         if (IS_HL_PORT(unit,port)) {
             kt2_scache_speed[port - 1] = 2500;
         } 
         if ((port == KT2_OLP_PORT) && (SOC_INFO(unit).olp_port)) {
             kt2_scache_speed[port - 1] = 2500;
         } 
         if (IS_XE_PORT(unit,port)) {
             kt2_scache_speed[port - 1] = 10000;
         } 
         if (IS_HG_PORT(unit,port)) {
             if (bcm56450_speed[unit][cfg_num][port-1] > 13000) {
                 kt2_scache_speed[port - 1] = 21000;
             } else {
                 kt2_scache_speed[port - 1] = 13000;
             }
         } 
    ...
    sal_memcpy(&flexio_scache_ptr[scache_offset], 
               &kt2_scache_speed[0], sizeof(kt2_scache_speed));

    scache_offset += sizeof(kt2_scache_speed);
    for (port=1 ; port <= KT2_MAX_PHYSICAL_PORTS ; port++) {
         /* mxqblock = (*kt2_port_to_mxqblock[unit])[port-1]; */
         kt2_scache_port_details[port-1].start_port_no = port;
         kt2_scache_port_details[port-1].end_port_no = port;
         kt2_scache_port_details[port-1].port_incr = 1;
         kt2_scache_port_details[port-1].port_type = 0;
         kt2_scache_port_details[port-1].port_speed = 0xFFFF;
         if (IS_GE_PORT(unit,port)) {
             if ((bcm56450_speed[unit][cfg_num][port-1] == 0) || /*Converted */
                 (bcm56450_speed[unit][cfg_num][port-1] >= 2500)) {
                  kt2_scache_port_details[port-1].port_speed = 2500;
             } else {
                  kt2_scache_port_details[port-1].port_speed = 1000;
             }
             kt2_scache_port_details[port-1].port_type = GE_PORT;
         } 
         if (IS_HL_PORT(unit,port)) {
             kt2_scache_port_details[port-1].port_speed = 2500;
             kt2_scache_port_details[port-1].port_type |= HGL_PORT;
             kt2_scache_port_details[port-1].port_type |= STK_PORT;
         } 
         if ((port == KT2_OLP_PORT) && (SOC_INFO(unit).olp_port)) {
             kt2_scache_port_details[port-1].port_speed = 2500;
             kt2_scache_port_details[port-1].port_type |= OLP_PORT;
         } 
         if (IS_XE_PORT(unit,port)) {
             kt2_scache_port_details[port-1].port_speed = 10000;
             kt2_scache_port_details[port - 1].port_type |= XE_PORT;
         } 
         if (IS_HG_PORT(unit,port)) {
             if (bcm56450_speed[unit][cfg_num][port-1] > 13000) {
                 kt2_scache_port_details[port-1].port_speed = 21000;
             } else {
                 kt2_scache_port_details[port-1].port_speed = 13000;
             }
             kt2_scache_port_details[port - 1].port_type |= HG_PORT;
             kt2_scache_port_details[port - 1].port_type |= STK_PORT;
         } 
         if (IS_HG2_ENABLED_PORT(unit,port)) {
             kt2_scache_port_details[port-1].port_speed = 21000;
