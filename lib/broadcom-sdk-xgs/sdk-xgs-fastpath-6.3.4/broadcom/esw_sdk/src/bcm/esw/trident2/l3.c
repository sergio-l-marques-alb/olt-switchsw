        ...
        bcm_if_t *old_intf_array)
{
     ...
     */
    max_replaced = (old_intf_count > intf_count) ? intf_count :
        old_intf_count;
    ...
    for (i = 0; i < max_replaced; i++) {
        ...
        new_next_hop_index = intf_array[i] - offset;
        ...
        old_next_hop_index = old_intf_array[i] - offset;

        if (new_next_hop_index == old_next_hop_index) {
            continue;
        }

        /* Replace the changed members */
        for (j = 0; j < num_entries; j++) {
            flowset_entry = soc_mem_table_idx_to_pointer(unit,
                    RH_ECMP_FLOWSETm, rh_ecmp_flowset_entry_t *, flowset_buf, j);
            next_hop_index = soc_mem_field32_get(unit, RH_ECMP_FLOWSETm, flowset_entry,
                        NEXT_HOP_INDEXf);
            if (!soc_mem_field32_get(unit, RH_ECMP_FLOWSETm, flowset_entry,
                        VALIDf)) {
                rv = BCM_E_INTERNAL;
            ...
            next_hop_index = soc_mem_field32_get(unit, RH_ECMP_FLOWSETm, flowset_entry,
                        NEXT_HOP_INDEXf);
            if (old_next_hop_index == next_hop_index) {
                soc_mem_field32_set(unit, RH_ECMP_FLOWSETm, flowset_entry,
                        NEXT_HOP_INDEXf, new_next_hop_index);
            }
        }

        old_intf_array[i] = intf_array[i]; /* For delete logic */
    }
    ...
    if (intf_count > old_intf_count) {
        for (i = max_replaced; i < intf_count; i++) {
            rv = _bcm_td2_ecmp_rh_add(unit, ecmp, 
                                    i + 1, intf_array, intf_array[i]);
            if (SOC_FAILURE(rv)) {
                goto cleanup;
            }
        }
    } else if (intf_count < old_intf_count) {
        for (i = old_intf_count - 1; i >= max_replaced; i--) {
            rv = _bcm_td2_ecmp_rh_delete(unit, ecmp, 
                                    i, old_intf_array, old_intf_array[i]);
            if (SOC_FAILURE(rv)) {
                goto cleanup;
            }
        }
