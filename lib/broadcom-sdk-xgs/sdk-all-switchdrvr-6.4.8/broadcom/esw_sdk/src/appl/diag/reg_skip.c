...
reg_mask_subset_apache(int unit, soc_regaddrinfo_t *ainfo, uint64 *mask)
{
    ...
    if (SOC_REG_BLOCK_IN_LIST(unit, ainfo->reg, SOC_BLK_MMU)) {
        port = SOC_INFO(unit).port_l2p_mapping[ainfo->port];
...
#endif
    case MMU_TDM_DEBUGr:
        /* Skip bit 0 (HW_CURR_CAL) */
        COMPILER_64_SET(temp_mask, 0, 0x1fe);
        COMPILER_64_AND(*mask, temp_mask);
        break;
    case PGW_OBM_MONITOR_CONFIGr:
        /* Skip bit 52 (CONFIG_ERROR) */
        COMPILER_64_SET(temp_mask, 0xffefffff, 0xffffffff);
        COMPILER_64_AND(*mask, temp_mask);
        break;
