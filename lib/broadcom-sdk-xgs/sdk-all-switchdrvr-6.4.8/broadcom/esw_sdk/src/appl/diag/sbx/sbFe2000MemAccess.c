/*
 * $Id: sbFe2000MemAccess.c,v 1.21 Broadcom SDK $
...
memConfigRec memConfigTable[] =
{
        ...
        {"QmFreePageFifo", FE2K_MEM_QMFREEPAGEFIFO, 0xe3f},
...
sbFe2000ShowMemNames(void)
{
   ...
   for(i = 0; i < FE2K_MEM_MAX_INDEX; i++) {
       cli_out("%-32s",memConfigTable[i].memname);
       if ((i+1)%3 == 0)
	 cli_out("\n");
   }
   cli_out("\n");
...
/* MCM was missing DequeueState0 code */
 ...
 /* MCM no PmCounter zframe */
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,2);
          sbZfCaPmCounterMemoryEntry_Unpack(&PmCounterRecord,(uint8 *)dbuf,sizeof(dbuf));
...
#else 
int sbFe2000MemAccess_not_empty;
