...
cmd_dpp_kaps_search(int unit, args_t *a)
{
    ...
    if (ARG_CNT(a) > 0) {
        ...
        if (mc_group_v4) { /* The search uses the mc_group or the dip as the DIP qual*/
            DBAL_QUAL_VAL_ENCODE_IPV4_DIP(&qual_vals[3], mc_group_v4, SOC_SAND_NOF_BITS_IN_UINT32);
        } else {
...
cmd_dpp_kaps_arm_thread(int unit, args_t *a)
{
    ...
    soc_sand_rv = jer_pp_xpt_dma_state(unit, print_status, enable_dma_thread);
    if(BCM_FAILURE(handle_sand_result(soc_sand_rv))) {
...
cmd_dpp_kaps_arm_image_load(int unit, args_t *a)
{
    ...
    if (load_file != 0x0) {
        ...
        if (init_tcm) {
            ...
            if (rv != SOC_SAND_OK) {
                LOG_ERROR(BSL_LS_APPL_SHELL, (BSL_META_U(unit, "%s: Error: Failed to jer_pp_xpt_arm_init()\n"), ARG_CMD(a)));
                return CMD_FAIL;
            }
        }
...
int dpp_kbp_init_kbp_interface(int unit, uint32 core, uint32 kbp_mdio_id, uint32 kbp_ilkn_rev) {
    ...
    uint32 soc_sand_rv, flags, offset;
    int rv = BCM_E_NONE;
    ...
    if (elk->kbp_recover_enable) {
        ...
        if (SOC_IS_JERICHO(unit)) {
            rv = soc_port_sw_db_protocol_offset_get(unit, kbp_port, 0, &offset);
            if(BCM_FAILURE(rv)){
                cli_out("Error: unit %d, soc_port_sw_db_protocol_offset_get failed. kbp_port: %u\n", unit, kbp_port);
                return rv;
            }
        } else {
    ...
    if (elk->kbp_recover_enable){
        ...
        if (SOC_IS_JERICHO(unit)) {
            rv = portmod_port_enable_set(0,kbp_port,0x8,0);
            rv = portmod_port_enable_set(0,kbp_port,0x8,1);
        } else {
...
char cmd_dpp_kbp_usage[] =
    ...
    "\tkbp kaps_show <DBAL_TBL_ID> - Prints the table contents in 0xData/valid_num_of_bits/max_num_of_bits format. \n"
