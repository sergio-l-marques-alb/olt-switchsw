...
int mirror_ports_arad_test_1(int unit, int device_type) {
    ...
    if ((rv = bcm_mirror_port_vlan_dest_get(unit, port1, vlan1, BCM_MIRROR_PORT_INGRESS, 1, &out_gport, &u_out_count)) != BCM_E_PARAM) {
        printf("Getting inbound port1+vlan1 mirror destination should have returned BCM_E_PARAM, but return value %d\n", rv);
        return BCM_E_FAIL;
    }
    if ((rv = bcm_mirror_port_vlan_dest_get(unit, port2, vlan2, BCM_MIRROR_PORT_INGRESS, 1, &out_gport, &u_out_count)) != BCM_E_PARAM) {
        printf("Getting inbound port2+vlan2 mirror destination should have returned BCM_E_PARAM, but return value %d\n", rv);
        return BCM_E_FAIL;
    }
    if ((rv = bcm_mirror_port_vlan_dest_get(unit, port2, vlan2, BCM_MIRROR_PORT_EGRESS, 1, &out_gport, &u_out_count)) != BCM_E_PARAM) {
        printf("Getting outbound port2+vlan2 mirror destination should have returned BCM_E_PARAM, but return value %d\n", rv);
        return BCM_E_FAIL;
    }
    if ((rv = bcm_mirror_port_vlan_dest_get(unit, port3, vlan3, BCM_MIRROR_PORT_EGRESS, 1, &out_gport, &u_out_count)) != BCM_E_PARAM) {
...
int mirror_destinations_arad_test_rspan(int unit, int seed) {
    ...
    /* build interface with new-vlan (RSPAN-vlan) */
	 rv = vlan__open_vlan_per_mc(unit, new_vlan, 0x1);  
     if (rv != BCM_E_NONE) {
         printf("Error, open_vlan=%d, in unit %d \n", new_vlan, unit);
     }
     rv = bcm_vlan_gport_add(unit, new_vlan, found_ports[3], 0);
     if (rv != BCM_E_NONE && rv != BCM_E_EXISTS) {
         printf("fail add port(0x%08x) to vlan(%d)\n", found_ports[3], new_vlan);
       return rv;
     }

     create_l3_intf_s intf_create;
     intf_create.vsi = new_vlan;
     intf_create.my_global_mac = dummy_mac_address;
     intf_create.my_lsb_mac = dummy_mac_address;

     rv = l3__intf_rif__create(unit, &intf_create);
     interf = intf_create.rif;
     if (rv != BCM_E_NONE) {
