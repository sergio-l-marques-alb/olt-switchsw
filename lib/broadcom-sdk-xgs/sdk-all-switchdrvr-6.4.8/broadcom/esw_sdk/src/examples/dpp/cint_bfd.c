...
int l3_interface_init(int unit, int in_sysport, int out_sysport, int * _next_hop_mac_encap_id, int * _tunnel_id, int create_mpls_tunnel){
    ...
    rv = create_l3_egress(unit,0,out_sysport,out_vlan,l3_eg_int,next_hop_mac, &fec[0], &encap_id[0]); 
    if (rv != BCM_E_NONE) {
    ...
    rv = create_l3_egress(unit,0,out_sysport,out_vlan,l3_eg_int,next_hop_mac2, &fec[1], &encap_id[1]); 
...
int bfd_ipv4_example(int unit,  bcm_gport_t remote_gport, uint32 flags) {
  ...
  if ((is_server || use_trap_with_snoop) && device_type_bfd<device_type_bfd_arad_plus) {
...
int mpls_init(int unit, bcm_mpls_tunnel_switch_t *tunnel_switch, int *tunnel_id, int is_BFDCC) {
  ...
  if (is_BFDCC && (device_type_bfd <= device_type_bfd_arad_plus)) {
 ...
 * @return int 
 */
int bfd_ipv4_run_with_defaults(int unit, int port1, int port2,  int remote_port,  int is_extended_sip, uint32 flags) {
  bcm_error_t rv;
  ...
  if ( ((flags & BCM_BFD_ENDPOINT_MULTIHOP)==0  || (flags & BCM_BFD_ENDPOINT_MICRO_BFD )!=0) 
       ...
       && device_type_bfd>=device_type_bfd_arad_plus) {
  ...
  if (remote_port == -1) {
      ...
      if (is_server || use_trap_with_snoop) {
          BCM_GPORT_TRAP_SET(remote_gport, remote_port, 7, 3);
      }
      else {
          printf("Setting remote_gport to port 0x%08x\n",remote_port);
          BCM_GPORT_LOCAL_SET(remote_gport, remote_port);
      }
  }
