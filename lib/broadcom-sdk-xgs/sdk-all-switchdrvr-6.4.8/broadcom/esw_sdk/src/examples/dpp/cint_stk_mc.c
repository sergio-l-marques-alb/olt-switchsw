 ...
 * cint
 * In TMD 0:
 *     stacking_mc_appl(0, 0);
 * In TMD 1:
 *     stacking_mc_appl(0, 1);
 *
 *
 * Flooding example:
 ...
 *    
 *    Usage:
 *        In TMD 0: stacking_flooding_appl(0, 0);
 *        In TMD 1: stacking_flooding_appl(0, 1);
 *
 *
 * Remarks:
...
int get_device_type(int unit, device_type_t *device_type)
{
    ...
    *device_type = ((info.device == 0x8675) || (info.device == 0x8375))? device_type_jericho: device_type_arad;
    return rv;
}
...
int stacking_flooding_appl(int unit, int tmd)
{
    ...
    if (tmd == 0x0) {
        ...
        mc_port_members[0] = 15; /* Stacking Ports */
        mc_port_members[1] = 16; /* Stacking Ports */
        mc_port_members[2] = erp_sysport[0];
        if (device_type == device_type_jericho) {
            mc_port_members[3] = erp_sysport[1];
            ++mc_member_count;
        }
