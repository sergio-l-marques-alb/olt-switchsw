...
int oam_lb_tst_sat_start(int unit, int gtf_id)
  ...
  * @return int 
  */
 int oam_lb_tst_sat_run(int unit, int port1, int port2, int port3, int lb_tst_flag)
{
    bcm_error_t rv;
    bcm_oam_endpoint_info_t acc_endpoint; 
    bcm_oam_endpoint_info_t remote_endpoint;
...
int oamp_server_example(int server_unit, int client_unit, int port1, int port2, int port3,int recycle_port) {
    ...
    rv = get_device_type(server_unit, &device_type);
    if (rv < 0) {
    ...
    rv = set_counter_source_and_engines(client_unit, &counter_base_id_up_mep,port2);
    BCM_IF_ERROR_RETURN(rv);
    rv = set_counter_source_and_engines(client_unit, &counter_base_id_down_mep,port1);
    BCM_IF_ERROR_RETURN(rv);
...
/* Source port from which CCMs will be injected*/
    rv =  port_to_system_port(client_unit, port2, &ep_server_up.tx_gport);
    if (rv != BCM_E_NONE) {
        printf("(%s) \n", bcm_errmsg(rv));
        return rv;
    }
   ...
   rv = bcm_oam_endpoint_create(server_unit, &rmep);
    ...
    ep_client_up.gport = gport2; /* represents the LIF on which the MEP resides*/
...
/* When using PCP the 3 LSBs of the counter must be 0. The following cints return an appropriate counter base.*/
    ...
    oam_pcp_qos_map_id = 0; /* Use OAM=outlif-profile 0 (Only available value in Jericho.)*/
