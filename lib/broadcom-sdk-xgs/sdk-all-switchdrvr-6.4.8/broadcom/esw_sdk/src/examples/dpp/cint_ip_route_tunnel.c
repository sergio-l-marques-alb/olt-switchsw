...
int example_ip_to_tunnel(int *units_ids, int nof_units, int in_sysport, int out_sysport){
    ...
    for (i = 0 ; i < nof_units ; i++){
        ...
        rv = create_l3_egress(unit,flags,out_sysport,out_vlan,tunnel_id,next_hop_mac, &fec[0], &encap_id[0]); 
        if (rv != BCM_E_NONE) {
    ...
    for (i = 0 ; i < nof_units ; i++){
        unit = units_ids[i];
        rv = create_l3_egress(unit,flags,out_sysport,out_vlan,tunnel_id,next_hop_mac2, &fec[1], &encap_id[1]); 
        if (rv != BCM_E_NONE) {
            printf("Error, create egress object, out_sysport=%d, in unit %d\n", out_sysport, unit);
        }
        if(verbose >= 1) {
