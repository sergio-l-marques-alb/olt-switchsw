...
int bfd_ipv6_intf_init(int unit, int in_vlan, int out_vlan, int out_vsi, bcm_mac_t mac_address, bcm_mac_t next_hop_mac, bcm_ip6_t src_ipv6, bcm_ip6_t dst_ipv6){
    ...
    rv = create_l3_egress(unit, flags, bfd_out_port, out_vsi, l3_eg_int, next_hop_mac, &fec, &encap_id); 
    if (rv != BCM_E_NONE) {
...
int bfd_ipv6_ep_create_example(int unit, int port1, int port2, int is_init, int is_single_hop, int is_acc, int is_passive, int is_demand, 
    ...
    if (is_init == 1) {
        /* BFD in/out Port Configuration */    
        rv=bfd_ipv6_service_init(0,port1,port2);
        if (rv != BCM_E_NONE) {
            printf("Error in bfd_ipv6_service_init\n");
            print rv;
            return rv;
                            ...
                            bcm_bfd_auth_type_t auth_type, uint32 local_min_tx, uint32 local_min_rx, uint32 udp_src_port, bcm_cos_t priority, int is_inject, int is_init)
{
    ...
    if (is_init == 1) {
        ...
        /* BFD in/out Port Configuration */    
        rv=bfd_ipv6_service_init(unit,port1,port2);
        if (rv != BCM_E_NONE) {
            printf("Error in bfd_ipv6_service_init\n");
            print rv;
            return rv;
