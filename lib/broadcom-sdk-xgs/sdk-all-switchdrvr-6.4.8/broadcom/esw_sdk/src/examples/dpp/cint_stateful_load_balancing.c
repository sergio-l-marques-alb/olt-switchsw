 ...
 *    run:
      ...
      cint cint_stateful_load_balancing.c
      cint
      print slb_create_stateful_ecmp(0, 13, 13, 4);
 * 
 ...
 *    run:
      ...
      cint cint_stateful_load_balancing.c
      cint
      slb_create_stateful_trunk(0, 200, 201, 3);
 * 
 ...
 *    run:
      cint cint_stateful_load_balancing.c
      cint
      print slb_graceful_member_removal_ecmp(0, ecmp, 1, 2000);
 *    
 ...
 *    run:
      cint cint_stateful_load_balancing.c
      cint
      print slb_disgraceful_member_removal_lag(0, tid, 1);
...
int slb_field_group_hash_two_keys(const int unit, bcm_field_qualify_t *hash_dq_qualifier, uint32 *hash_dq_length, bcm_field_data_qualifier_t *hash_dq_data_qual, uint8 hash_dq_nof, bcm_field_header_format_t preselector_header_format){
	int result, qual_id;
	...
	/*Build Key (post hashing), 24 msb are key0, 23 lsb are key1*/
 ...
 */
int slb_field_group_hash_two_keys_sip(const int unit) 
{
	int result;
	uint8 hash_dq_nof = 1;
	...
	bcm_field_header_format_t preselector_header_format;

	hash_dq_qualifier[0] = bcmFieldQualifySrcIp;
	hash_dq_length[0] = 32;
	preselector_header_format = bcmFieldHeaderFormatIp4AnyL2L3;

	result = slb_field_group_hash_two_keys(unit, hash_dq_qualifier, hash_dq_length, hash_dq_data_qual, hash_dq_nof, preselector_header_format);
	if (BCM_E_NONE != result) {
		printf("Error in slb_field_group_hash_two_keys\n");
		return result;
	}
...
int slb_create_stateful_trunk(const int unit, const int in_port, const int base_out_port, const int out_port_num)
{
  ...
  rv = l2_addr_config(unit, tid_example, 0, 0); 
  if(rv != BCM_E_NONE){
