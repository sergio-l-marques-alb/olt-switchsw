	      ...
	      uint16 addr, uint8* data, uint32 *len)
{
    ...
    if (*len > 256) {
        LOG_CLI((BSL_META_U(unit,
                            "\nCannot handle more than 256 bytes")));
        return SOC_E_PARAM;
    }
    rv = soc_i2c_write_byte(unit, saddr, addr);
    if (SOC_SUCCESS(rv)) {
        for (count=0; count < *len; count++) {
            rv = soc_i2c_read_byte(unit, saddr, &data[count]);
            if (((count+1) % 16)==0) sal_udelay(100);
...
_cxp_write(int unit, uint8 saddr , uint16 addr, uint8* data, uint32 len)
{
    ...
    rv = soc_i2c_write_byte(unit, saddr, addr);
    for (count=0; SOC_SUCCESS(rv) && (count < len); count++) {
        rv = soc_i2c_write_byte(unit, saddr, data[count]);
        if (SOC_SUCCESS(rv)) {
