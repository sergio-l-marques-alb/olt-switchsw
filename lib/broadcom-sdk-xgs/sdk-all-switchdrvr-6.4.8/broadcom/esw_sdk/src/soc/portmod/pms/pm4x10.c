                   ...
                   int wb_buffer_index, pm_info_t pm_info)
{
   ...
   /* initialize num of phys for each lane */
    ...
    if(!SOC_WARM_BOOT(unit)){
        ...
        pm_is_initialized = 0;
        rv = SOC_WB_ENGINE_SET_VAR (unit, SOC_WB_ENGINE_PORTMOD, 
                                pm_info->wb_vars_ids[isActive], &pm_is_initialized);
        _SOC_IF_ERR_EXIT(rv);
                                      ...
                                      phymod_phy_init_config_t *phy_init_config)
{
    ...
    rv = pm4x10_port_phy_lane_access_get(unit, port, pm_info,
                                    &params, 1, &phy_access_lane, &nof_phys, NULL);
    if (SOC_FAILURE(rv)) return (rv);

    is_legacy_present = ((portmod_default_user_access_t *)(phy_access_lane.access.user_acc))->is_legacy_phy_present;

    if (!is_legacy_present) {
        rv = phymod_phy_media_type_tx_get(&phy_access_lane, phymodMediaTypeChipToChip, &phymod_tx);
        if (SOC_FAILURE(rv)) return (rv);
    }
                        ...
                        const portmod_port_interface_config_t* config)
{
    ...
    if (nof_phys && phy_access_lane.access.lane_mask) {
        if (!is_legacy_phy)
        ...
        if(SOC_PBMP_MEMBER(port_phys_in_pm, phy)) {
            if( PM_4x10_INFO(pm_info)->nof_phys[temp] >= 2 ){
                ...
                for(i=0 ; i<nof_ext_phys ; i++) {
                    ...
                    if( (xphy_id != PORTMOD_XPHY_ID_INVALID) && ( xphy_id < PORTMOD_MAX_NUM_XPHY_SUPPORTED)){
                        _SOC_IF_ERR_EXIT(portmod_xphy_core_info_get(unit, xphy_id, &xphy_core_info));
                        _SOC_IF_ERR_EXIT(_pm4x10_core_probe(unit,port, pm_info, &xphy_core_info.core_access, &probe, 1));
...
int _pm4x10_pm_core_init (int unit, pm_info_t pm_info, const portmod_port_add_info_t* add_info)
{
    ...
    uint32 core_is_initialized, is_initialized;
    int lane;
    SOC_INIT_FUNC_DEFS;
            ...
            if( PM_4x10_INFO(pm_info)->nof_phys[temp] >= 2 ){
                ...
                for(i=0 ; i<nof_ext_phys ; i++) {
                    ...
                    if( (xphy_id != PORTMOD_XPHY_ID_INVALID) && ( xphy_id < PORTMOD_MAX_NUM_XPHY_SUPPORTED)){
                        ...
                        core_conf.firmware_loader = NULL;
                        if (xphy_core_info.force_fw_load) {
                        ...
                        if (xphy_id == xphy_core_info.primary_core_num) {
                             ...
                             if (PORTMOD_PORT_ADD_F_INIT_UNTIL_FW_LOAD_GET(add_info)) {
                                 if (PM_4x10_INFO(pm_info)->first_phy == phy) {
                                     is_initialized = 0;
                                     xphy_core_info.is_initialized = 0;
                        ...
                        const portmod_port_add_info_t* add_info)
{
    ...
    int my_i, first_phy = -1, phy_acc, phy_port;
    phymod_phy_init_config_t    *phy_init_config;
    phymod_phy_inf_config_t     *phy_interface_config;
    phymod_phy_access_t phy_access[1+MAX_PHYN];
    phymod_phy_access_t phy_access_lane;
    ...
    if (PORTMOD_PORT_ADD_F_INIT_UNTIL_FW_LOAD_GET(add_info) ||
        (!PORTMOD_PORT_ADD_F_INIT_UNTIL_FW_LOAD_GET(add_info) && !PORTMOD_PORT_ADD_F_INIT_RESUME_AFTER_FW_LOAD_GET(add_info))) {
                                                  ...
                                                  phymod_phy_inf_config_t *phy_interface_config)
{
    ...
    switch (interface_config->speed) {
        ...
        case 100:
            ...
            break;
        case 1000:
            if (interface_config->interface == SOC_PORT_IF_GMII) {
                phy_interface_config->interface_type = phymodInterface1000X;
            } else if (interface_config->interface == SOC_PORT_IF_SGMII) {
                phy_interface_config->interface_type = phymodInterfaceSGMII;
            } else if (fiber_pref) {
                phy_interface_config->interface_type = phymodInterface1000X;
            } else if(interface_config->interface==SOC_PORT_IF_KX) {
    ...
    switch (interface_config->speed) {
        case 10000:
            if (port_num_lanes == 4) {
                ...
                } else if(interface_config->interface==SOC_PORT_IF_ZR) {
...
{
    return ((pfc_cfg->classes != 8)? SOC_E_PARAM: xlmac_pfc_config_set(unit, port, pfc_cfg));
}
...
int pm4x10_port_phy_intr_enable_set(int unit, int port, pm_info_t pm_info, 
                                    phymod_intr_source_t intr_src, uint32 intr_enable)
{
    phymod_phy_access_t phy_access[1+MAX_PHYN];
    int nof_phys;
    SOC_INIT_FUNC_DEFS;
...
int pm4x10_port_phy_intr_enable_get(int unit, int port, pm_info_t pm_info, 
                                    phymod_intr_source_t intr_src, uint32* intr_enable)
{
    phymod_phy_access_t phy_access[1+MAX_PHYN];
    int nof_phys;
...
int pm4x10_port_phy_intr_status_get(int unit, int port, pm_info_t pm_info,
                                    phymod_intr_source_t intr_src, uint32* intr_enable)
{
    phymod_phy_access_t phy_access[1+MAX_PHYN];
    int nof_phys;
...
int pm4x10_port_polarity_update_set(int unit, int port, pm_info_t pm_info, int lane)
{
    phymod_polarity_t swapped_pol;
    int swap_lane, nof_phys = 0;
    phymod_phy_access_t phy_access[1+MAX_PHYN];
    SOC_INIT_FUNC_DEFS;

    _SOC_IF_ERR_EXIT(phymod_polarity_t_init(&swapped_pol));

    if (PM_4x10_INFO(pm_info)->lane_map.lane_map_rx[lane] != lane) {
        swap_lane = PM_4x10_INFO(pm_info)->lane_map.lane_map_rx[lane];
        if (SHR_BITGET(&PM_4x10_INFO(pm_info)->polarity.tx_polarity, swap_lane)) {
            SHR_BITSET(&swapped_pol.tx_polarity, 0);
        }
        if (SHR_BITGET(&PM_4x10_INFO(pm_info)->polarity.rx_polarity, swap_lane)) {
            SHR_BITSET(&swapped_pol.rx_polarity, 0);
        }
        _SOC_IF_ERR_EXIT(portmod_port_chain_phy_access_get(unit, port, pm_info, phy_access ,(1+MAX_PHYN), &nof_phys));

        _SOC_IF_ERR_EXIT(phymod_phy_polarity_set(phy_access, &swapped_pol));
    }
...
{   
    ...
    PM_4x10_INFO(pm_info)->nof_phys[lane_index] += 1;
