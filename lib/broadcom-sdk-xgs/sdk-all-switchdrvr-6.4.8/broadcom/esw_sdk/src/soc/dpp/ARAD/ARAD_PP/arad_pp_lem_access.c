...
#define ARAD_PP_LEM_ACCESS_KEY_TYPE_NOF_PARAMS_ILM              (4)
#define ARAD_PP_LEM_ACCESS_KEY_TYPE_NOF_PARAMS_COUPLING_ILM     (2)
...
#define ARAD_PP_FRWRD_MACT_ENTRY_KEY_AGE_STATUS_MAX              (6)
                                                         ...
                                                         SOC_SAND_IN  uint32 app_id)
{
  ...
  case ARAD_PP_LEM_ACCESS_KEY_TYPE_PREFIX_SA_AUTH:
    str = "SA_AUTH";
  break;
  case ARAD_PP_LEM_ACCESS_KEY_TYPE_PREFIX_COUPLING_ILM:
    str = "COUPLING_ILM";
  break;
  ...
  )
{
  ...
  prefix = app_id = ARAD_PP_LEM_ACCESS_KEY_TYPE_PREFIX_COUPLING_ILM;
  res = arad_pp_lem_access_prefix_alloc(unit,flags,app_id,ARAD_PP_LEM_ACCESS_PREFIX_NUM_1,&prefix);
  ...
  )
{
  ...
  if (SOC_DPP_CONFIG(unit)->pp.oam_statistics == 1) { 
      app_id = ARAD_PP_LEM_ACCESS_KEY_TYPE_OAM_STATISTICS;
      res = arad_pp_lem_access_prefix_alloc(unit,flags, app_id,ARAD_PP_LEM_ACCESS_PREFIX_NUM_1,&prefix);
      SOC_SAND_CHECK_FUNC_RESULT(res, 17, exit);

      app_id = ARAD_PP_LEM_ACCESS_KEY_TYPE_OAM_DOWN_UNTAGGED_STATISTICS;
      res = arad_pp_lem_access_prefix_alloc(unit,flags, app_id,ARAD_PP_LEM_ACCESS_PREFIX_NUM_1,&prefix);
      SOC_SAND_CHECK_FUNC_RESULT(res, 17, exit);

      app_id = ARAD_PP_LEM_ACCESS_KEY_TYPE_OAM_SINGLE_TAG_STATISTICS;
      res = arad_pp_lem_access_prefix_alloc(unit,flags, app_id,ARAD_PP_LEM_ACCESS_PREFIX_NUM_1,&prefix);
      SOC_SAND_CHECK_FUNC_RESULT(res, 17, exit);

      app_id = ARAD_PP_LEM_ACCESS_KEY_TYPE_OAM_DOUBLE_TAG_STATISTICS;
      res = arad_pp_lem_access_prefix_alloc(unit,flags, app_id,ARAD_PP_LEM_ACCESS_PREFIX_NUM_1,&prefix);
      SOC_SAND_CHECK_FUNC_RESULT(res, 17, exit); 
...
uint32
  arad_pp_lem_access_entry_type_get_arad_format(
      SOC_SAND_IN   int                       unit, 
      SOC_SAND_IN   uint32                    payload_data[ARAD_PP_LEM_ACCESS_PAYLOAD_NOF_UINT32S],
      SOC_SAND_IN  uint8                     is_learn_data,
      SOC_SAND_OUT  uint32                    *entry_type
    )
{
    ...
    res = soc_sand_bitstream_get_any_field(
        payload_data,
        /* In case of learn data the format stays as in Arad */
        SOC_DPP_LEM_ENTRY_TYPE_IS_FEC_EEI_LSB_ARAD,
        2,
        &(tmp)
      );
    SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_pp_lem_access_entry_type_get_arad_foramt()", payload_data[0], payload_data[1]);
}
...
arad_pp_lem_access_sa_drop_get(
   ...
   SOC_SAND_OUT  uint8                       *sa_drop
    ...
    if ((entry_type != ARAD_PP_LEM_ENTRY_TYPE_FORMAT_3B) && (entry_type != ARAD_PP_FWD_DECISION_PARSE_RAW_DATA)) {
        uint32 res;
        uint32 sa_drop_32 = 0;
        res = soc_sand_bitstream_get_any_field(
                payload_data,
...
uint32
  arad_pp_lem_access_payload_parse(
      SOC_SAND_IN   int                   unit,
      SOC_SAND_IN   uint32                   entry_type_input,
      SOC_SAND_IN   uint32                    payload_data[ARAD_PP_LEM_ACCESS_PAYLOAD_NOF_UINT32S],
    ...
    )
{
  ...
  uint32
    ...
    entry_type;
  ARAD_PP_FEC_TYPE
      fec_type = ARAD_PP_NOF_FEC_TYPES;
  uint8 is_destination_fec;
...
#ifdef BCM_88660_A0
  if (SOC_IS_ARADPLUS(unit) && (entry_type_input == ARAD_PP_LEM_ENTRY_TYPE_FORMAT_SLB)) {
  ...
  if (entry_type_input == ARAD_PP_LEM_ENTRY_TYPE_UNKNOWN)
  {
  ...
  /* for host format, first check if it's format 3b. */
  else if (entry_type_input == ARAD_PP_LEM_ENTRY_TYPE_HOST_FORMAT) 
  {
      ...
      /* the entry type for host format wasn't format3b.
         get the entry type from the payload data */
  ...
  else if (entry_type_input == ARAD_PP_LEM_ENTRY_TYPE_RAW_DATA) {
      entry_type = ARAD_PP_LEM_ACCESS_ASD_RAW_DATA;
  }else
  {
      entry_type = entry_type_input;
  }
  ...
  if(entry_type == ARAD_PP_LEM_ENTRY_TYPE_OUTLIF)
  {
      ...
      if(tmp || fec_type == ARAD_PP_FEC_TYPE_FORWARD) 
      {   /* outlif value */
          if (SOC_IS_JERICHO(unit) && payload->is_learn_data) {
          ...
          if (fec_type == ARAD_PP_FEC_TYPE_FORWARD) {
              payload->flags = ARAD_PP_LEM_ACCESS_ASD_OUTLIF_INVALID;
          } else {
          ...
          SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
          /*payload->flags = ARAD_PP_LEM_ACCESS_ASD_FEC;*/
      }
   ...
   */
      ...
      if (SOC_IS_JERICHO(unit)) {
          ...
          payload->asd = (ARAD_PP_EEI_IDENTIFIER_ENCAP_VAL_2_MSBS << ARAD_PP_ASD_BIT_POSITION_IDENTIFIER_OUTLIF) 
                            |  (arp_msbs <<ARAD_PP_ASD_BIT_POSITION_IDENTIFIER) 
                            | payload->asd;
      } 
      /* for arad+, eei identifier is 1111 */
      else {
          payload->asd = (ARAD_PP_EEI_IDENTIFIER_ENCAP_VAL << ARAD_PP_ASD_BIT_POSITION_IDENTIFIER) | payload->asd;
      }
  ...
  }else if (entry_type == ARAD_PP_LEM_ACCESS_ASD_RAW_DATA) {

      /* update flag */
      payload->flags |= ARAD_PP_LEM_ACCESS_ASD_RAW_DATA;
...
uint32
  arad_pp_lem_access_payload_parse_arad_format(
      SOC_SAND_IN   int                   unit,
      SOC_SAND_IN   uint32                   entry_type_input,
      SOC_SAND_IN   uint32                    payload_data[ARAD_PP_LEM_ACCESS_PAYLOAD_NOF_UINT32S],
      SOC_SAND_OUT  ARAD_PP_LEM_ACCESS_PAYLOAD   *payload
    )
{
  ...
  if (entry_type_input == ARAD_PP_LEM_ENTRY_TYPE_UNKNOWN)
  {
      res = arad_pp_lem_access_entry_type_get_arad_format(
              unit, 
              payload_data,
              payload->is_learn_data,
              &entry_type
  ...
  } else if (is_system_port == 2) /* src system port) { */
  {
      fwd_decision.dest_id = 0;

      /* Check if we have LAG or UC port */
      if (SOC_SAND_GET_BIT(*payload_data,15)==1) /* bit 15 of the destination is the is_lag field */
      {
          fwd_decision.type = SOC_PPC_FRWRD_DECISION_TYPE_UC_LAG;

          /* Delete the is lag bit (bit 15) and reserved bit (bit 14) from the payload_data since the verifiy doesn't like them. Leave the lag id only (first 14 bits) */
          res = soc_sand_bitstream_get_any_field(
                      payload_data,
                      0,
                      13,
  ...
  } else { /* Flow or FEC of prefix */
      res = arad_pp_fwd_decision_from_learn_buffer_arad_format(
            unit,
            tmp,
            ARAD_PP_LIF_ENTRY_TYPE_AC, /* the value doesn't matter */
            &fwd_decision
            );
      SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  }
  ...
  arad_pp_fwd_decision_in_buffer_build(
        unit,
        ARAD_PP_FRWRD_DECISION_APPLICATION_TYPE_DFLT, /* the value doesn't matter */
        &fwd_decision,
        &payload->dest,
        &payload->asd
  );
  ...
  {  
      ...
      res = soc_sand_bitstream_get_any_field(
              payload_data,
              35, /*ARAD_PP_LEM_LIF_VALID_LSB(unit),*/
              ARAD_PP_LEM_LIF_VALID_NOF_BITS,
              &(is_valid)
            );
      SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);      
      ...
      if(is_valid || fec_type == ARAD_PP_FEC_TYPE_FORWARD) 
      {   /* outlif value */
          
          if (fec_type == ARAD_PP_FEC_TYPE_FORWARD) {
              payload->flags = ARAD_PP_LEM_ACCESS_ASD_OUTLIF_INVALID;
          } else if (payload->asd != 0) {
              payload->flags = ARAD_PP_LEM_ACCESS_ASD_OUTLIF;
          }
  ...
  {
      SOC_SAND_SET_ERROR_CODE(ARAD_PP_LEM_ENTRY_INDEX_OUT_OF_RANGE_ERR, 10, exit);
  }
  payload->flags |= ARAD_PP_FWD_DECISION_PARSE_DEST;
  
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_pp_lem_access_payload_parse_arad_format()", 0, 0);
}
  ...
  )
{
  ...
  if(payload->flags & ARAD_PP_LEM_ACCESS_ASD_OUTLIF || payload->flags == ARAD_PP_LEM_ACCESS_ASD_NONE || payload->flags & ARAD_PP_LEM_ACCESS_ASD_OUTLIF_INVALID)
  {
      entry_type = ARAD_PP_LEM_ENTRY_TYPE_OUTLIF;
  }
  /* entry is of EEI type */
  else if(payload->flags & ARAD_PP_LEM_ACCESS_ASD_EEI) {
  ...
  else if(payload->flags & ARAD_PP_LEM_ACCESS_ASD_FORMAT_3)
  {
      entry_type = ARAD_PP_LEM_ENTRY_TYPE_FORMAT_3;
  } 
  /* entry is of outLif, HI, native-VSI type */
  else if (payload->flags & ARAD_PP_LEM_ACCESS_ASD_FORMAT_3B)
  {
      entry_type = ARAD_PP_LEM_ENTRY_TYPE_FORMAT_3B;
  }
  else if (payload->flags & ARAD_PP_LEM_ACCESS_ASD_RAW_DATA)
  {
      entry_type = ARAD_PP_LEM_ACCESS_ASD_RAW_DATA;
  }
  else
  ...
  if(entry_type == ARAD_PP_LEM_ENTRY_TYPE_OUTLIF)
  {
      ...
      SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
      /* check if outlif is valid */
      tmp = (payload->flags & ARAD_PP_LEM_ACCESS_ASD_OUTLIF) ? 1 : 0;
      res = soc_sand_bitstream_set_any_field(
              &(tmp),
              ARAD_PP_LEM_LIF_VALID_LSB(unit) - ((SOC_IS_JERICHO(unit) && (payload->is_learn_data)) ? 2 : 0),
              ARAD_PP_LEM_LIF_VALID_NOF_BITS,
      ...
      if(tmp || payload->flags & ARAD_PP_LEM_ACCESS_ASD_OUTLIF_INVALID) {
          /* outlif value */
          if (SOC_IS_JERICHO(unit) && (payload->is_learn_data)) {
...
/*  uint8
   ...
   *    If the prefix is not SA auth, retrieve waht's encoded in the four msb of the buffer.
   */
  key_type_msb = 0;
  res = soc_sand_bitstream_get_any_field(
...
if (key_type_msb != ARAD_PP_LEM_ACCESS_KEY_PREFIX_FOR_SA_AUTH)
  ...
  case ARAD_PP_LEM_ACCESS_KEY_TYPE_PREFIX_ILM:
    ...
    key->param[2].nof_bits = ARAD_PP_LEM_ACCESS_KEY_PARAM2_IN_BITS_FOR_ILM;
  ...
  case ARAD_PP_LEM_ACCESS_KEY_TYPE_PREFIX_COUPLING_ILM:
    ...
    key->param[0].nof_bits = ARAD_PP_LEM_ACCESS_KEY_PARAM0_IN_BITS_FOR_COUPLING_ILM;
    key->param[1].nof_bits = ARAD_PP_LEM_ACCESS_KEY_PARAM1_IN_BITS_FOR_COUPLING_ILM;
    key->param[2].nof_bits = 0;
    key->param[3].nof_bits = 0;
    key->param[4].nof_bits = 0;
    break;
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_pp_lem_key_encoded_parse()", 0, 0);
}
    ...
    )
{
  ...
  if (request_all->request.key.type==ARAD_PP_LEM_ACCESS_KEY_TYPE_BFD_SINGLE_HOP || request_all->request.key.type==ARAD_PP_LEM_ACCESS_KEY_TYPE_BFD_ECHO) {
      /* Format will be as follows:
           {0 (2b) ,remote_detect_mult[0:3] (4b) ,OAM-ID (13/14b) ,remote_detect_mult[4:7] (4b) , 0 (1b) ,destination (19b)}*/
      /* lem_data_size depends on weather or not we add the expected remote detect multiplier. */
      int lem_data_size= (request_all->payload.asd >>SOC_DPP_DEFS_GET(unit,oam_2_id_nof_bits)  ) ? SOC_DPP_DEFS_GET(unit,oam_2_id_nof_bits) + 4 :  SOC_DPP_DEFS_GET(unit,oam_2_id_nof_bits); 
      fld_value[0]= 0;
      fld_value[1]= 0;
      ...
      SHR_BITCOPY_RANGE(fld_value,8*3,&request_all->payload.asd, 0,lem_data_size ); 

      /* Finally, the second nibble of the remote detect multiplier. Put it in bits 20-23 (nibble allaigned)*/
      SHR_BITCOPY_RANGE(fld_value,20,&request_all->payload.asd,lem_data_size,4  ); 
  }
    ...
    )
{
  ...
  /* read and parse payload buffer into lem-payload struct*/
#ifdef ARAD_FAST_REGISTERS_AND_FIELDS_ACCESS
  ...
  entry_type = (request_all->request.key.type == ARAD_PP_LEM_ACCESS_KEY_TYPE_SA_AUTH) ?
    ARAD_PP_LEM_ENTRY_TYPE_FORMAT_3 :
    ARAD_PP_LEM_ENTRY_TYPE_UNKNOWN;

#ifdef BCM_88660_A0
  if (SOC_IS_ARADPLUS(unit) && (request_all->request.key.type == ARAD_PP_LEM_ACCESS_KEY_TYPE_SLB)) {
    entry_type = ARAD_PP_LEM_ENTRY_TYPE_FORMAT_SLB;
  }
#endif /* BCM_88660_A0 */

  request_all->payload.is_learn_data = 0;
  res = arad_pp_lem_access_payload_parse(unit,entry_type,pld_val,&request_all->payload);
 ...
 */
uint32
  arad_pp_lem_access_parse_only_arad_format(
      SOC_SAND_IN  int                            unit,
      SOC_SAND_IN uint32                            flags,
      SOC_SAND_IN  uint32                           *event_buff,
      SOC_SAND_IN  uint32                            event_buff_len,
    ...
    )
{
  ...
  ARAD_FAST_FIELD_GET(ARAD_FAST_FIELD_IHP_MACT_CPU_REQUEST_REQUEST_MACT_REQ_STAMP, reg_above_64_val, fld_value);  
#else
  ARAD_FLD_FROM_REG_ABOVE_64(IHP_MACT_CPU_REQUEST_REQUESTr, MACT_REQ_STAMPf, fld_value, reg_above_64_val, 20, exit);
#endif    

  if (dma_supported) 
  {
      SHR_BITCOPY_RANGE((uint32*)&(request_all->stamp), 0, reg_above_64_val, 78, 8);
  } 
  else if (SOC_IS_JERICHO(unit)) {
      SHR_BITCOPY_RANGE((uint32*)&(request_all->stamp), 0, reg_above_64_val, 79, 8);
  }
  else {
      request_all->stamp = fld_value[0]; 
  }
  ...
  ARAD_FAST_FIELD_GET(ARAD_FAST_FIELD_IHP_MACT_CPU_REQUEST_REQUEST_MACT_REQ_COMMAND, reg_above_64_val, fld_value);  
#else
  ARAD_FLD_FROM_REG_ABOVE_64(IHP_MACT_CPU_REQUEST_REQUESTr, MACT_REQ_COMMANDf, fld_value, reg_above_64_val, 85, exit);
#endif
  ...
  if (SOC_IS_JERICHO(unit) && !dma_supported) {
  ...
  if (SOC_IS_ARADPLUS(unit) && (request_all->request.key.type == ARAD_PP_LEM_ACCESS_KEY_TYPE_SLB)) {
    ...
    request_all->payload.is_learn_data = 1;
    res = arad_pp_lem_access_payload_parse(unit,entry_type,pld_val,&request_all->payload);
    SOC_SAND_CHECK_FUNC_RESULT(res, 107, exit);
...
#ifdef ARAD_FAST_REGISTERS_AND_FIELDS_ACCESS
  ARAD_FAST_REGISER_GET(ARAD_FAST_REG_IHP_MACT_REPLY, reg_above_64_val);
#else
...
*/
    ...
    res = chip_sim_exact_match_entry_get_unsafe(unit, ARAD_CHIP_SIM_LEM_BASE,
    ...
    *is_found = success;
    ...
    res = arad_pp_lem_access_payload_parse(unit, entry_type, data_out, payload);
    SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
  ...
  )
{
   ...
   * Thus we will read the Cache, assuming it was updated on the "request Send"*/
    if (SOC_IS_DONE_INIT(unit))
    {
        if (BCM_UNIT_DO_HW_READ_WRITE(unit))
        {
            res = arad_pp_lem_access_sw_entry_by_key_get_unsafe(unit, key, payload, is_found);
            SOC_SAND_CHECK_FUNC_RESULT(res, 200, exit);

            if (! *is_found)     /*Look for the entry in the "dynamic" section - read directly from HW*/
            {
                int                                           hw_access;
                ...
                /* Now verify that the received entry is dynamic. if it is static - drop it.*/
                if (*is_found)
                {
                    if (payload->is_dynamic == FALSE)   /* FALSE Means that this static entry was deleted from SW DB but the 'delete command' wasn't executed yet - it is still stored in the HW journal */
                        *is_found = FALSE;               /* Report this entry as 'Not Found' */
                }
				SOC_SAND_CHECK_FUNC_RESULT(res, 210, exit);
            }
  ...
  *is_found = SOC_SAND_NUM2BOOL(COMPILER_64_LO(fld_val64));
  ...
  res = arad_pp_lem_access_payload_parse(unit,entry_type,pld_val,payload);
  SOC_SAND_CHECK_FUNC_RESULT(res, 107, exit);
...
*/
  ...
  if(COMPILER_64_LO(fld_val64)){
      payload->flags |= ARAD_PP_LEM_ACCESS_ENTRY_ACCESSED;
  }
  }
#ifdef PLISIM
  if (SAL_BOOT_PLISIM) {
...
uint32
  arad_pp_lem_access_sa_based_asd_build(
    SOC_SAND_IN  int                       unit,
    SOC_SAND_IN  ARAD_PP_LLP_SA_AUTH_MAC_INFO      *auth_info,
    SOC_SAND_IN  ARAD_PP_LLP_VID_ASSIGN_MAC_INFO   *vid_assign_info,
  ...
  )
{
  ...
  {
    if (auth_info->expect_tag_vid != ARAD_PP_LLP_SA_AUTH_ACCEPT_ALL_VIDS &&
  ...
  *asd = 0;
  ...
  if (auth_info)
  {
    ...
    if (auth_info->expect_system_port.sys_id == ARAD_PP_LLP_SA_AUTH_ACCEPT_ALL_PORTS || SOC_IS_ARAD_A0(unit))
    {
      /* permit all PORTs */
      tmp = 1;
      res = soc_sand_bitstream_set_any_field(
  ...
  if (vid_assign_info)
  {
    ...
    if (vid_assign_info->vid != ARAD_PP_LLP_SA_AUTH_ACCEPT_ALL_VIDS)
    {
...
uint32
  arad_pp_lem_access_trill_sa_based_asd_build(
    SOC_SAND_IN  int                         unit,
    SOC_SAND_IN  ARAD_PP_TRILL_ADJ_INFO              *auth_info,
    SOC_SAND_OUT uint32                          *asd
  )
{
  uint32
...
uint32
  arad_pp_lem_access_sa_based_asd_parse(
    ...
    SOC_SAND_OUT ARAD_PP_LLP_VID_ASSIGN_MAC_INFO   *vid_assign_info
  )
{
  uint32
    tmp;
  ...
  if(auth_info)
  {
    ...
    if (!tmp)
    {
      auth_info->expect_tag_vid = ARAD_PP_LLP_SA_AUTH_ACCEPT_ALL_VIDS;
    }
    ...
    {
      auth_info->expect_system_port.sys_id = ARAD_PP_LLP_SA_AUTH_ACCEPT_ALL_PORTS;
    }
    else
    {
...
uint32
  arad_pp_lem_access_trill_sa_based_asd_parse(
    SOC_SAND_IN  int                       unit,
    SOC_SAND_IN  uint32                        asd,
    SOC_SAND_OUT ARAD_PP_TRILL_ADJ_INFO            *auth_info
  )
{
  uint32
    tmp;
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_pp_lem_ilm_key_build_set()", 0, 0);
}
  ...
  )
{
  ...
  info->asd = 0;
  info->is_dynamic = 0;
    info->flags = ARAD_PP_LEM_ACCESS_ASD_NONE;
    info->has_cw = 0;
    info->is_dynamic = 0;
    info->sa_drop = 0;
    info->tpid_profile = 0;
  ...
  )
{
  ...
  case ARAD_PP_LEM_ACCESS_KEY_TYPE_IPV4_MC:
    if (params_ndx == 0)
    {
      SOC_SAND_ERR_IF_OUT_OF_RANGE(info->nof_bits, ARAD_PP_LEM_ACCESS_KEY_PARAM0_IN_BITS_FOR_IPV4_MC, ARAD_PP_LEM_ACCESS_KEY_PARAM0_IN_BITS_FOR_IPV4_MC, ARAD_PP_LEM_ACCESS_KEY_PARAM_NOF_BITS_OUT_OF_RANGE_ERR, 21, exit);
      SOC_SAND_ERR_IF_ABOVE_MAX(info->value[0], (1 << ARAD_PP_LEM_ACCESS_KEY_PARAM0_IN_BITS_FOR_IPV4_MC)-1, ARAD_PP_LEM_ACCESS_KEY_PARAM_VALUE_OUT_OF_RANGE_ERR, 22, exit);
      SOC_SAND_ERR_IF_ABOVE_MAX(info->value[1], 0, ARAD_PP_LEM_ACCESS_KEY_PARAM_VALUE_OUT_OF_RANGE_ERR, 23, exit);
    }
    else  /* params_ndx == 1 */
    {
      SOC_SAND_ERR_IF_OUT_OF_RANGE(info->nof_bits, ARAD_PP_LEM_ACCESS_KEY_PARAM1_IN_BITS_FOR_IPV4_MC, ARAD_PP_LEM_ACCESS_KEY_PARAM1_IN_BITS_FOR_IPV4_MC, ARAD_PP_LEM_ACCESS_KEY_PARAM_NOF_BITS_OUT_OF_RANGE_ERR, 24, exit);
      SOC_SAND_ERR_IF_ABOVE_MAX(info->value[0], (1 << ARAD_PP_LEM_ACCESS_KEY_PARAM1_IN_BITS_FOR_IPV4_MC)-1, ARAD_PP_LEM_ACCESS_KEY_PARAM_VALUE_OUT_OF_RANGE_ERR, 25, exit);
   ...
   case ARAD_PP_LEM_ACCESS_KEY_TYPE_ILM:
     if (params_ndx == 0)
     {
       SOC_SAND_ERR_IF_OUT_OF_RANGE(info->nof_bits, ARAD_PP_LEM_ACCESS_KEY_PARAM0_IN_BITS_FOR_ILM, ARAD_PP_LEM_ACCESS_KEY_PARAM0_IN_BITS_FOR_ILM, ARAD_PP_LEM_ACCESS_KEY_PARAM_NOF_BITS_OUT_OF_RANGE_ERR, 37, exit);
       
     }
     else  if (params_ndx == 1 )
     {
       SOC_SAND_ERR_IF_OUT_OF_RANGE(info->nof_bits, ARAD_PP_LEM_ACCESS_KEY_PARAM1_IN_BITS_FOR_ILM, ARAD_PP_LEM_ACCESS_KEY_PARAM1_IN_BITS_FOR_ILM, ARAD_PP_LEM_ACCESS_KEY_PARAM_NOF_BITS_OUT_OF_RANGE_ERR, 41, exit);
       SOC_SAND_ERR_IF_ABOVE_MAX(info->value[0], (1 << ARAD_PP_LEM_ACCESS_KEY_PARAM1_IN_BITS_FOR_ILM)-1, ARAD_PP_LEM_ACCESS_KEY_PARAM_VALUE_OUT_OF_RANGE_ERR, 42, exit);
       SOC_SAND_ERR_IF_ABOVE_MAX(info->value[1], 0, ARAD_PP_LEM_ACCESS_KEY_PARAM_VALUE_OUT_OF_RANGE_ERR, 47, exit);
     }
     else if ( params_ndx == 2 )
     {
       SOC_SAND_ERR_IF_OUT_OF_RANGE(info->nof_bits, ARAD_PP_LEM_ACCESS_KEY_PARAM2_IN_BITS_FOR_ILM, ARAD_PP_LEM_ACCESS_KEY_PARAM2_IN_BITS_FOR_ILM, ARAD_PP_LEM_ACCESS_KEY_PARAM_NOF_BITS_OUT_OF_RANGE_ERR, 41, exit);
       SOC_SAND_ERR_IF_ABOVE_MAX(info->value[0], (1 << ARAD_PP_LEM_ACCESS_KEY_PARAM2_IN_BITS_FOR_ILM)-1, ARAD_PP_LEM_ACCESS_KEY_PARAM_VALUE_OUT_OF_RANGE_ERR, 42, exit);
       SOC_SAND_ERR_IF_ABOVE_MAX(info->value[1], 0, ARAD_PP_LEM_ACCESS_KEY_PARAM_VALUE_OUT_OF_RANGE_ERR, 47, exit);
     }
     else  if ( params_ndx == 3 )
     {
       SOC_SAND_ERR_IF_OUT_OF_RANGE(info->nof_bits, ARAD_PP_LEM_ACCESS_KEY_PARAM3_IN_BITS_FOR_ILM, ARAD_PP_LEM_ACCESS_KEY_PARAM3_IN_BITS_FOR_ILM, ARAD_PP_LEM_ACCESS_KEY_PARAM_NOF_BITS_OUT_OF_RANGE_ERR, 41, exit);
       SOC_SAND_ERR_IF_ABOVE_MAX(info->value[0], (1 << ARAD_PP_LEM_ACCESS_KEY_PARAM3_IN_BITS_FOR_ILM)-1, ARAD_PP_LEM_ACCESS_KEY_PARAM_VALUE_OUT_OF_RANGE_ERR, 42, exit);
       SOC_SAND_ERR_IF_ABOVE_MAX(info->value[1], 0, ARAD_PP_LEM_ACCESS_KEY_PARAM_VALUE_OUT_OF_RANGE_ERR, 47, exit);
     }
     break;

   case ARAD_PP_LEM_ACCESS_KEY_TYPE_COUPLING_ILM:
     if (params_ndx == 0)
     {
       SOC_SAND_ERR_IF_OUT_OF_RANGE(info->nof_bits, ARAD_PP_LEM_ACCESS_KEY_PARAM0_IN_BITS_FOR_COUPLING_ILM, ARAD_PP_LEM_ACCESS_KEY_PARAM0_IN_BITS_FOR_COUPLING_ILM, ARAD_PP_LEM_ACCESS_KEY_PARAM_NOF_BITS_OUT_OF_RANGE_ERR, 42, exit);
     }
     else  if (params_ndx == 1 )
     {
       SOC_SAND_ERR_IF_OUT_OF_RANGE(info->nof_bits, ARAD_PP_LEM_ACCESS_KEY_PARAM1_IN_BITS_FOR_COUPLING_ILM, ARAD_PP_LEM_ACCESS_KEY_PARAM1_IN_BITS_FOR_COUPLING_ILM, ARAD_PP_LEM_ACCESS_KEY_PARAM_NOF_BITS_OUT_OF_RANGE_ERR, 42, exit);
       SOC_SAND_ERR_IF_ABOVE_MAX(info->value[0], (1 << ARAD_PP_LEM_ACCESS_KEY_PARAM1_IN_BITS_FOR_COUPLING_ILM)-1, ARAD_PP_LEM_ACCESS_KEY_PARAM_VALUE_OUT_OF_RANGE_ERR, 42, exit);
       SOC_SAND_ERR_IF_ABOVE_MAX(info->value[1], 0, ARAD_PP_LEM_ACCESS_KEY_PARAM_VALUE_OUT_OF_RANGE_ERR, 47, exit);
     }     
     break;
  ...
  case ARAD_PP_LEM_ACCESS_KEY_TYPE_OAM_STATISTICS:
  case ARAD_PP_LEM_ACCESS_KEY_TYPE_OAM_DOWN_UNTAGGED_STATISTICS:
  case ARAD_PP_LEM_ACCESS_KEY_TYPE_OAM_SINGLE_TAG_STATISTICS:
  case ARAD_PP_LEM_ACCESS_KEY_TYPE_OAM_DOUBLE_TAG_STATISTICS:
  ...
  )
{
  ...
  case ARAD_PP_LEM_ACCESS_KEY_TYPE_ILM:
    ...
    break;
  case ARAD_PP_LEM_ACCESS_KEY_TYPE_COUPLING_ILM:
    SOC_SAND_ERR_IF_OUT_OF_RANGE(info->nof_bits, ARAD_PP_LEM_ACCESS_KEY_PREFIX_SIZE_IN_BITS_FOR_COUPLING_ILM, ARAD_PP_LEM_ACCESS_KEY_PREFIX_SIZE_IN_BITS_FOR_COUPLING_ILM, ARAD_PP_LEM_ACCESS_KEY_PREFIX_NOF_BITS_OUT_OF_RANGE_ERR, 18, exit);
    SOC_SAND_ERR_IF_OUT_OF_RANGE(info->value, ARAD_PP_LEM_ACCESS_KEY_PREFIX_FOR_COUPLING_ILM, ARAD_PP_LEM_ACCESS_KEY_PREFIX_FOR_COUPLING_ILM, ARAD_PP_LEM_ACCESS_KEY_PREFIX_PREFIX_OUT_OF_RANGE_ERR, 18, exit);
    break;
  ...
  )
{
  ...
  case ARAD_PP_LEM_ACCESS_KEY_TYPE_ILM:
    ...
    SOC_SAND_ERR_IF_OUT_OF_RANGE(info->nof_params, ARAD_PP_LEM_ACCESS_KEY_TYPE_NOF_PARAMS_ILM, ARAD_PP_LEM_ACCESS_KEY_TYPE_NOF_PARAMS_ILM, ARAD_PP_LEM_ACCESS_KEY_NOF_PARAMS_OUT_OF_RANGE_ERR, 11, exit);
    break;
  case ARAD_PP_LEM_ACCESS_KEY_TYPE_COUPLING_ILM:
    SOC_SAND_ERR_IF_OUT_OF_RANGE(info->nof_params, ARAD_PP_LEM_ACCESS_KEY_TYPE_NOF_PARAMS_COUPLING_ILM, ARAD_PP_LEM_ACCESS_KEY_TYPE_NOF_PARAMS_COUPLING_ILM, ARAD_PP_LEM_ACCESS_KEY_NOF_PARAMS_OUT_OF_RANGE_ERR, 11, exit);
    break;
  case ARAD_PP_LEM_ACCESS_KEY_TYPE_EXTENDED:
  ...
  break;
  case ARAD_PP_LEM_ACCESS_KEY_TYPE_OAM_STATISTICS:
	  SOC_SAND_ERR_IF_OUT_OF_RANGE(info->nof_params, ARAD_PP_LEM_ACCESS_KEY_NOF_PARAMS_FOR_FC_ZONING, ARAD_PP_LEM_ACCESS_KEY_NOF_PARAMS_FOR_FC_ZONING, ARAD_PP_LEM_ACCESS_KEY_NOF_PARAMS_OUT_OF_RANGE_ERR, 22, exit);
  break;
 case ARAD_PP_LEM_ACCESS_KEY_TYPE_IP6_SPOOF_STATIC:
  ...
  )
{
  ...
  if (info->flags != ARAD_PP_FWD_DECISION_PARSE_RAW_DATA) {
      ...
      SOC_SAND_ERR_IF_ABOVE_MAX(info->age, ARAD_PP_FRWRD_MACT_ENTRY_KEY_AGE_STATUS_MAX+1, ARAD_PP_LEM_AGE_OUT_OF_RANGE_ERR, 30, exit);
  }  
    ...
    if (!trap_code) {
        if (lem_entry->is_ipv6) {
            if (lem_entry->is_accelerated) {
                res = sw_state_access[unit].dpp.bcm.bfd.trap_info.trap_ids.get(unit, SOC_PPD_BFD_TRAP_ID_UC_IPV6, &trap_code); 
            ...
            if (lem_entry->is_accelerated) {
                res = sw_state_access[unit].dpp.bcm.bfd.trap_info.trap_ids.get(unit, SOC_PPD_BFD_TRAP_ID_OAMP_IPV4, &trap_code); 
                SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 214, exit_semaphore);
            } else {
                trap_code = SOC_PPC_TRAP_CODE_OAM_CPU;
            }
        }
        fwd_strngth = soc_property_get(unit, spn_DEFAULT_TRAP_STRENGTH, 6);
        snp_strength = 0;
    }
