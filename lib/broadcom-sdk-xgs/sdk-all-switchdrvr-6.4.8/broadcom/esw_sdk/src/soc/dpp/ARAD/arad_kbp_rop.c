...
/* writing opcode if needed*/
    if(SOC_DPP_CONFIG(unit)->arad->init.elk.last_opcode_set != opcode)
    {
        ...
        SOC_DPP_CONFIG(unit)->arad->init.elk.last_opcode_set = opcode;
    }    

/* before writing data, check if need to set msb trigger instead of lsb trigger */
    if (lsb_enable == 0) {
        if (SOC_DPP_CONFIG(unit)->arad->init.elk.msb_trigger_on == 0) {
            ...
            SOC_DPP_CONFIG(unit)->arad->init.elk.msb_trigger_on = 1;
        }
    } else {
        if (SOC_DPP_CONFIG(unit)->arad->init.elk.msb_trigger_on == 1) {
            ...
            SOC_DPP_CONFIG(unit)->arad->init.elk.msb_trigger_on = 0;
        }
    }
...
{
    uint8 
        msb_trigger_is_on = SOC_DPP_CONFIG(unit)->arad->init.elk.msb_trigger_on;
...
/*  when ARAD_KBP_ROP_OPTIMIZATION we are transmitting packets without reading the counter so we need to zero the counter before reading */
    ...
    if ((rop_error == 0x1) || (elk_error == 0x1)) {
            ...
            LOG_ERROR(BSL_LS_SOC_TCAM,
                      (BSL_META_U(unit,
                                  "Error in %s(): register IHB_LOOKUP_REPLY_DATA: LOOKUP_REPLY_ROP_ERROR=%d, LOOKUP_REPLY_ELK_ERROR=%d (excpected 0 on both) for core %u\n"), 
 ...
 *  ROP functions
 */
STATIC void arad_kbp_buffer_endians_fix(int unit, soc_reg_above_64_val_t buffer, uint8 is_big_endian_buffer)
{
    int big_pio, big_packet, big_other;
    uint32 i;

    soc_cm_get_endian(unit, &big_pio, &big_packet, &big_other);
    if((big_pio && is_big_endian_buffer) || (!big_pio && !is_big_endian_buffer)){
        /*nothing to do*/
#ifdef ARAD_KBP_ROP_DEBUG_PRINTS    
         LOG_VERBOSE(BSL_LS_SOC_COMMON,
                     (BSL_META_U(unit,
                                 "%s(): big_pio=%d, big_packet=%d, big_other=%d, is_big_endian_buffer=%x, No Swap !!!"),
                      FUNCTION_NAME(), big_pio, big_packet, big_other, is_big_endian_buffer));
#endif 
    } else {
#ifdef ARAD_KBP_ROP_DEBUG_PRINTS    
         LOG_VERBOSE(BSL_LS_SOC_COMMON,
                     (BSL_META_U(unit,
                                 "%s(): big_pio=%d, big_packet=%d, big_other=%d, is_big_endian_buffer=%x, Swap !!!"),
                      FUNCTION_NAME(), big_pio, big_packet, big_other, is_big_endian_buffer));
#endif         
...
uint32 
    arad_kbp_lut_write(int unit, uint8 addr, arad_kbp_lut_data_t* lut_data, soc_reg_above_64_val_t lut_data_raw)
{
    uint32 local_addr,
           res;            
    soc_reg_above_64_val_t 
    ...
    if (SOC_IS_ARADPLUS(unit)) {
        res = aradplus_kbp_cpu_record_send(unit, 0, ARAD_KBP_CPU_WR_LUT_OPCODE, msb_data, lsb_data, 0, NULL);
    } else
#endif 
    {
...
uint32 
    arad_kbp_lut_read(int unit, uint8 addr,arad_kbp_lut_data_t* lut_data, soc_reg_above_64_val_t lut_row_data)
{
    uint32 
        res;
    soc_reg_above_64_val_t 
   ...
   LOG_VERBOSE(BSL_LS_SOC_TCAM,
    ...
    /*not reversed buffer*/
    arad_kbp_buffer_endians_fix(unit, msb_data, TRUE);
        ...
        arad_kbp_rop_write_t *wr_data)
{
    ...
    soc_reg_above_64_val_t 
        msb_data,
        lsb_data;
    uint8 rev_data[ARAD_KBP_CPU_DATA_REGISTER_BYTE_LEN];

    SOC_SAND_INIT_ERROR_DEFINITIONS(0);

    SOC_REG_ABOVE_64_CLEAR(msb_data);
    SOC_REG_ABOVE_64_CLEAR(lsb_data);
    sal_memset(rev_data, 0, ARAD_KBP_CPU_DATA_REGISTER_BYTE_LEN);

#ifdef ARAD_KBP_ROP_DEBUG_PRINTS
    LOG_VERBOSE(BSL_LS_SOC_TCAM,
                (BSL_META_U(unit,
                            "%s(): wr_data->addr= "), FUNCTION_NAME()));
    ...
    /*change the order for IHB*/
    ARAD_KBP_ROP_REVERSE_DATA(rev_data, ((uint8 *)msb_data), ARAD_KBP_CPU_DATA_REGISTER_BYTE_LEN);

#ifdef ARAD_KBP_ROP_DEBUG_PRINTS
    LOG_VERBOSE(BSL_LS_SOC_TCAM,
                (BSL_META_U(unit,
                            "%s(): before msb_data= "), FUNCTION_NAME()));
    arad_kbp_array_print_debug(((uint8 *)msb_data), sizeof(soc_reg_above_64_val_t));
#endif
    ...
    if (SOC_IS_JERICHO(unit)) {
        ...
        arad_kbp_buffer_endians_fix(unit, msb_data, FALSE); 
    }

#ifdef ARAD_KBP_ROP_DEBUG_PRINTS
    LOG_VERBOSE(BSL_LS_SOC_TCAM,
                (BSL_META_U(unit,
                            "%s(): after  msb_data= "), FUNCTION_NAME()));
    arad_kbp_array_print_debug(((uint8 *)msb_data), sizeof(soc_reg_above_64_val_t));
#endif
        ...
        )
{
    uint32 
        res,
        opcode;
    uint8 receive_data[ARAD_KBP_CPU_DATA_REGISTER_BYTE_LEN];
    soc_reg_above_64_val_t 
        msb_data,
        lsb_data,
        above_64_rd_data;
    ...
    SOC_REG_ABOVE_64_CLEAR(msb_data);
    SOC_REG_ABOVE_64_CLEAR(lsb_data);
    sal_memset(receive_data , 0, ARAD_KBP_CPU_DATA_REGISTER_BYTE_LEN);

    LOG_VERBOSE(BSL_LS_SOC_TCAM,
                (BSL_META_U(unit,
                            "%s(): rd_data->dataType=%d, rd_data->addr= "), FUNCTION_NAME(), rd_data->dataType));
    ...
    LOG_VERBOSE(BSL_LS_SOC_TCAM,
                (BSL_META_U(unit,
                            "%s(): returned data= "), FUNCTION_NAME()));
    arad_kbp_array_print_debug((uint8 *)above_64_rd_data, ARAD_KBP_CPU_LOOKUP_REPLY_DATA_REGISTER_BYTE_LEN);    

    /*reversed buffer*/
    if (SOC_IS_JERICHO(unit)) {
        arad_kbp_buffer_endians_fix(unit, above_64_rd_data, TRUE); 
    } else {
        arad_kbp_buffer_endians_fix(unit, above_64_rd_data, FALSE); 
    }

    LOG_VERBOSE(BSL_LS_SOC_TCAM,
                (BSL_META_U(unit,
                            "%s(): returned data after endians fix= "), FUNCTION_NAME()));
    arad_kbp_array_print_debug((uint8 *)above_64_rd_data, ARAD_KBP_CPU_LOOKUP_REPLY_DATA_REGISTER_BYTE_LEN);    
        ...
        )
{
    ...
    if ( ARAD_KBP_CPU_DATA_REGISTER_BYTE_LEN >= cbw_cmp_data->cbw_data.data_len) {
        ...
        arad_kbp_buffer_endians_fix(unit, lsb_data, FALSE);
        lsb_enable = 1;
    }
    /* reversed buffer (outside of the function) */
    arad_kbp_buffer_endians_fix(unit, msb_data, FALSE);
    ...
    if ((cbw_cmp_data->type == NLM_ARAD_CB_INST_CMP1) || 
        ...
        /* reversed buffer*/
        arad_kbp_buffer_endians_fix(unit, above_64_cmp_data, FALSE);
