  ...
  )
{
  ...
  if (scan_time_in_micro || 
    ...
    res = arad_ticks_to_time(unit, tmp, TRUE, ARAD_ITM_WD_INFO_MIN_SCAN_CYCLE_PERIOD_MICRO_GRANULARITY, &tmp); /* calculate exact value from field */
    SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
...
#ifdef BCM_88660_A0
  if (SOC_IS_ARADPLUS(unit) && color_resolution_mode != ARAD_PP_MTR_COLOR_RESOLUTION_MODE_DEFAULT) {
...
#ifdef BCM_88660_A0
          if (SOC_IS_ARADPLUS(unit) && color_resolution_mode == ARAD_PP_MTR_COLOR_RESOLUTION_MODE_RED_DIFF){
...
*	OK or ERROR indication.
*********************************************************************/
uint32 arad_itm_congestion_statistics_get(
  SOC_SAND_IN int unit,
  SOC_SAND_IN int core,
