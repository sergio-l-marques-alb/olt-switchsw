    ...
    ARAD_FLP_CUSTOM_LPM_LOOKUP_INFO
...
#endif
        {
         ...
         },
         {
          ...
          {
            ARAD_PP_FLP_KEY_PROGRAM_IPV4UC_WITH_DEFAULT,
            /* lookup_num, {, {}, ... } */
            2, 0,
              {
                  { 2, 0, 2, /*  key C, key OR val, number of instructions */
          ...
          {
            ARAD_PP_FLP_KEY_PROGRAM_IPV4MC_WITH_RPF,
            /* lookup_num, lookup_to_use, {, {}, ... } */
            2, 0,
              {
                 { 2, 0, 2, /*  key C, key OR val, number of instructions */
   ...
   )
{
    ...
    res = arad_pp_flp_get_next_avaialable(unit, ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_ING_IVL_LEARN, &cam_sel, prog_id);
    ...
    prog_selection_cam_tbl.program = *prog_id ; /*ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_ING_IVL_LEARN;*/
    prog_selection_cam_tbl.valid = 1;
    res = arad_pp_ihb_flp_program_selection_cam_tbl_set_unsafe(unit, cam_sel, &prog_selection_cam_tbl);
    SOC_SAND_CHECK_FUNC_RESULT(res, 200, exit);
...
/*  res = arad_pp_flp_get_next_avaialable(unit, ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_ING_IVL_INNER_LEARN, &cam_sel_1, prog_id);*/
/*  SOC_SAND_CHECK_FUNC_RESULT(res, 200, exit);                                                                                 */
    ...
    for (idx=0;idx<2;idx++) {
        ...
        */
        ...
        if (idx == 0) {
            res = arad_pp_flp_get_next_avaialable(unit, ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_ING_IVL_INNER_LEARN, &cam_sel, prog_id);
            SOC_SAND_CHECK_FUNC_RESULT(res, 200, exit);
        }
        else {
            res = arad_pp_flp_cam_table_next_avaialable_get(unit, &cam_sel);
        ...
        prog_selection_cam_tbl.program = *prog_id ; /*ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_ING_IVL_INNER_LEARN;*/
        prog_selection_cam_tbl.valid = 1;
   ...
   )
{
  ...
  res = arad_pp_flp_get_next_avaialable(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV6UC_WITH_RPF, NULL, prog_id);
...
#endif
#if defined(BCM_88675_A0) && defined(INCLUDE_KBP) && !defined(BCM_88030)
  /*rpf should not be ignored in jericho*/
    ...
    )
{
    ...
    if (SOC_IS_JERICHO(unit)) {
        if((flp_lookups_tbl.elk_key_c_valid_bytes + key_size) > 10 || (flp_lookups_tbl.elk_key_c_msb_valid_bytes > 0)){
            flp_lookups_tbl.elk_key_c_msb_valid_bytes += key_size;
        }else{
   ...
   )
{
  ...
  if (!SOC_DPP_CONFIG(unit)->pp.compression_spoof_ip6_enable) {
    ...
    if (SOC_IS_JERICHO(unit)){
       ...
       flp_key_construction_tbl.instruction_1_16b        = ARAD_PP_FLP_16B_INST_P6_IN_LIF_D; /* In-AC (SOC_IS_JERICHO(unit)? 0xfaac : 0xf688)*/
...
{
  uint32
    res, fid_ce_inst = 0, esadi_ce_inst,
   ...
   )
{
  ...
  flp_process_tbl.not_found_trap_strength      = 0;
  flp_process_tbl.unknown_address              = 3;
  if ((SOC_IS_ARADPLUS(unit)) && (SOC_DPP_CONFIG(unit)->pp.ipmc_l2_ssm_mode)) {
   ...
   )
{
  ...
  {      
      ...
      if((JER_KAPS_ENABLE(unit)) && (!ARAD_KBP_ENABLE_IPV4_UC)){
          arad_pp_flp_dbal_ipv4uc_with_l3vpn_program_tables_init(unit);
      }
      else 
#endif
      {
...
#if defined(BCM_88675_A0) && defined(INCLUDE_KBP) && !defined(BCM_88030)
      if((JER_KAPS_ENABLE(unit)) && (!ARAD_KBP_ENABLE_IPV4_UC)) {
          arad_pp_flp_dbal_ipv4uc_with_rpf_program_tables_init(unit);
      }
      else 
#endif
      {
      ...
      else if((JER_KAPS_ENABLE(unit)) && (!ARAD_KBP_ENABLE_IPV6_UC)) {
  ...
  if ((SOC_DPP_CONFIG(unit)->pp.ivl_inlif_profile == 1)) {
      /* Load IVL L2 Inner Most VID
         MUST be first, higher priority than regular IVL L2 */
      int32 ivl_prog_id;
      ...
      /* Define program process for L2 UC and MC IVL Inner-most VID KEY */
      ...
      /* Define program process for L2 UC and MC IVL IVEC VID KEY */
      arad_pp_flp_process_ethernet_ing_all_ivl_learn(unit, TRUE, sa_auth_enabled, slb_enabled, ivl_prog_id);
  }
   ...
   )
{
    ...
    if (SOC_DPP_CONFIG(unit)->pp.bfd_echo_with_lem == 1){
        if (lpm_lookup->app_id == ARAD_PP_FLP_KEY_PROGRAM_IPV4UC) {
...
)
{
...
#else
    case ARAD_PP_FLP_KEY_PROGRAM_PUNT:
      return "ARAD_PP_FLP_KEY_PROGRAM_PUNT";
#endif
    ...
    case ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_ING_IVL_LEARN:
        return "ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_ING_IVL_LEARN";
    case ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_ING_IVL_INNER_LEARN:
        return "ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_ING_IVL_INNER_LEARN";
