...
#define ARAD_PMF_KEY_SELECT_TCAM_JERICHO_USE_KAPS(lookup_cycle_id,key_lsb) \
    (ARAD_PP_FP_NOF_CYCLES*ARAD_PMF_FEM_NOF_TCAM_RESULTS*ARAD_PMF_FEM_NOF_TCAM_LSB_OPTIONS + ((lookup_cycle_id * ARAD_PP_FP_NOF_CYCLES) + key_lsb))
...
static 
   uint32  /* action name, hw-id, size*/
       Jericho_pmf_fem_action_type_encoding[][3] = {
           ...
           {SOC_PPC_FP_ACTION_TYPE_VLAN_EDIT_VID_2          ,29,  12       },
           {SOC_PPC_FP_ACTION_TYPE_VLAN_EDIT_PCP_DEI        ,30,  4        },
           {SOC_PPC_FP_ACTION_TYPE_IN_RIF                   ,31,  13       },
           {SOC_PPC_FP_ACTION_TYPE_VRF                      ,32,  13       },
           {SOC_PPC_FP_ACTION_TYPE_IN_TTL                   ,33,  8        },
           {SOC_PPC_FP_ACTION_TYPE_IN_DSCP_EXP              ,34,  8        },
           {SOC_PPC_FP_ACTION_TYPE_RPF_DESTINATION_VALID    ,35,  1        },
           {SOC_PPC_FP_ACTION_TYPE_RPF_DESTINATION          ,36,  19       },
  ...
  )
{
  ...
  if (info->src_arad.is_nop == TRUE) {
      /* Set the NOP action as all ones (5:0), since in Jericho the acceptable NOP values are 60-63 */
      fld_val |= SOC_SAND_SET_BITS_RANGE(ARAD_PMF_FEM_INSTR_SRC_TYPE_NOP, ARAD_PMF_FEM_INSTR_SRC_TYPE_INDICATION_MSB, 0 /* ARAD_PMF_FEM_INSTR_SRC_TYPE_INDICATION_LSB */);
  }
...
{
    int i;
    int total_size = ARAD_PP_FLP_KBP_MAX_ROP_PAYLOAD_SIZE_IN_BITS;
  ...
  *action_lsb_egress = 0;
  ...
  if (found 
      ...
      *action_size = (action_type == SOC_PPC_FP_ACTION_TYPE_USER_HEADER_1)? 
         ( (soc_property_get(unit, spn_ITMH_PROGRAMMABLE_MODE_ENABLE, FALSE) ) ? SOC_SAND_MAX(18,user_header_0_size): user_header_0_size ) : 
         ( (soc_property_get(unit, spn_ITMH_PROGRAMMABLE_MODE_ENABLE, FALSE) ) ? SOC_SAND_MAX(18,user_header_1_size): user_header_1_size ) ; 
  }
  else if(found
