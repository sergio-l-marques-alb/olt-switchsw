    ...
    )
{
  ...
  for (cycle = 0; cycle < SOC_PB_TCAM_NOF_CYCLES; ++cycle)
  {
    ...
    for (fem_id = 0; fem_id < SOC_PB_PMF_LOW_LEVEL_NOF_FEMS; ++fem_id)
    {
      ...
      if ((fem_entry.action_type[0] != SOC_PB_PP_NOF_FP_ACTION_TYPES) && (fem_entry.action_type[0] != BCM_FIELD_ENTRY_INVALID))
      {
    ...
    )
{
  ...
  {
    if (qual_types[idx] != BCM_FIELD_ENTRY_INVALID)
    {
      found = FALSE;
      for (data_idx = 0; !found && data_idx < Soc_pb_pp_fp_predefined_key_info_size; ++data_idx)
      {
    ...
    )
{
  ...
  {
    if (qual_types[ind] == BCM_FIELD_ENTRY_INVALID)
    {
      break;
    }
    ...
    )
{
  ...
  {
    if (qual_vals[qual_type_ndx].type != BCM_FIELD_ENTRY_INVALID)
    {
      is_type_found = FALSE;
  ...
  {
    if ((action_types[action_type_ndx] != SOC_PB_PP_NOF_FP_ACTION_TYPES) && (action_types[action_type_ndx] != BCM_FIELD_ENTRY_INVALID))
    {
      is_type_found = FALSE;
  ...
  )
{
  ...
  if (!info->is_array_qualifier) {
      ...
      {
        if ((info->qual_vals[qual_ndx].type == SOC_PB_PP_NOF_FP_QUAL_TYPES) || (info->qual_vals[qual_ndx].type == BCM_FIELD_ENTRY_INVALID))
        {
          continue;
        }
  ...
  )
{
  ...
  if (total_length == 0)
  {
    ...
    {
        if ((info->qual_types[qual_ndx] == SOC_PB_PP_NOF_FP_QUAL_TYPES) || (info->qual_types[qual_ndx] == BCM_FIELD_ENTRY_INVALID))
      {
        continue;
      }
  ...
  )
{
  ...
  if (info->db_type != SOC_PB_PP_FP_DB_TYPE_EGRESS)
  {
    ...
    for (action_type_ndx = 0; action_type_ndx < SOC_PB_PP_FP_NOF_ACTIONS_PER_DB_MAX; ++action_type_ndx)
    {
      if ((info->action_types[action_type_ndx] == SOC_PB_PP_NOF_FP_ACTION_TYPES) || (info->action_types[action_type_ndx] == BCM_FIELD_ENTRY_INVALID))
      {
        continue;
      }
  ...
  )
{
  ...
  for (action_type_ndx = 0; action_type_ndx < SOC_PB_PP_FP_NOF_ACTIONS_PER_DB_MAX; ++action_type_ndx)
  {
    if ((fp_database_info.action_types[action_type_ndx] == SOC_PB_PP_NOF_FP_ACTION_TYPES) ||
        (fp_database_info.action_types[action_type_ndx] == BCM_FIELD_ENTRY_INVALID))
    {
      continue;
    }
  ...
  )
{
  ...
  else if (db_info.db_type == SOC_PB_PP_FP_DB_TYPE_DIRECT_TABLE)
  {
    ...
    for (action_type_ndx = 0; action_type_ndx < SOC_PB_PP_FP_NOF_ACTIONS_PER_DB_MAX; ++action_type_ndx)
    {
      if ((info->actions[action_type_ndx].type == SOC_PB_PP_NOF_FP_ACTION_TYPES) || (info->actions[action_type_ndx].type == BCM_FIELD_ENTRY_INVALID))
      {
        continue;
      }
  ...
  )
{
  ...
  else if (fp_database_info.db_type == SOC_PB_PP_FP_DB_TYPE_DIRECT_TABLE)
  {
    ...
    if ((info->actions[0].type == SOC_PB_PP_NOF_FP_ACTION_TYPES) || (info->actions[0].type == BCM_FIELD_ENTRY_INVALID))
    {
      *is_found = FALSE;
    }
    else
  ...
  *success = SOC_SAND_SUCCESS;
  ...
  case SOC_PB_PP_FP_CONTROL_TYPE_HDR_USER_DEF:
	...
	if (control_ndx->db_id == 0) {
		...
		if (qual_type != BCM_FIELD_ENTRY_INVALID) {
    ...
    for (bank_ndx = 0; bank_ndx < SOC_PB_TCAM_NOF_BANKS; ++bank_ndx)
    {
      ...
      if (val[0] != 0)
      {
        ...
        for (action_type_ndx = 0; action_type_ndx < SOC_PB_PP_FP_NOF_ACTIONS_PER_DB_MAX; ++action_type_ndx)
        {
          ...
          if ((fp_database_info.action_types[action_type_ndx] == SOC_PB_PP_NOF_FP_ACTION_TYPES) || (fp_database_info.action_types[action_type_ndx] == BCM_FIELD_ENTRY_INVALID))
          {
            continue;
          }
  ...
  )
{
  ...
  for (ind = 0; ind < SOC_PB_PP_FP_NOF_QUALS_PER_DB_MAX; ++ind)
  {
    ...
    else /* end_of_list == TRUE */
    {
      ...
      SOC_SAND_ERR_IF_OUT_OF_RANGE(info->qual_types[ind], BCM_FIELD_ENTRY_INVALID, BCM_FIELD_ENTRY_INVALID, SOC_PB_PP_FP_QUAL_TYPES_END_OF_LIST_ERR, 111, exit);
    }
  ...
  )
{
  ...
  {
    if ((info->fld_ext[ind].type != SOC_PB_PP_NOF_FP_QUAL_TYPES) && (info->fld_ext[ind].type != BCM_FIELD_ENTRY_INVALID))
    {
  ...
  )
{
  ...
  for (ind = 0; ind < SOC_PB_PP_FP_NOF_QUALS_PER_DB_MAX; ++ind)
  {
    if ((info->qual_vals[ind].type != SOC_PB_PP_NOF_FP_QUAL_TYPES) && (info->qual_vals[ind].type != BCM_FIELD_ENTRY_INVALID))
    {
      SOC_PB_PP_STRUCT_VERIFY(SOC_PB_PP_FP_QUAL_VAL, &(info->qual_vals[ind]), 10, exit);
    }
  }
