    ...
    )
{
  ...
  if (SOC_IS_JERICHO(unit)) {
     rv =  _bcm_dpp_global_lif_mapping_local_to_global_get(unit, _BCM_DPP_GLOBAL_LIF_MAPPING_INGRESS,lif_id,&global_lif);
    if(BCM_E_NONE != rv){
           res = SOC_SAND_ERR;
  ...
  key->param[index_lif].value[0] = (SOC_IS_JERICHO(unit))?global_lif:lif_id; 
  ...
  if (SOC_DPP_L3_SRC_BIND_IPV4_SUBNET_OR_ARP_ENABLE(unit) && lif_id)
  {
    key->param[1].value[0] = vsi_id;
     ...
     if (SOC_IS_JERICHO(unit)) {
        ...
        key->param[1].value[0] = global_lif;
     }
     else{
