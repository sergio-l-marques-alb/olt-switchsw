...
/*
 dip sip sharing is used for sharing CEs between ACLs and FWD. 
*/

ARAD_PP_FP_KEY_DIP_SIP_SHARING dip_sip_sharing[TOTAL_NOF_PROGRAMS_IN_QUALIFIER_SHARING] = 

  {{ARAD_PP_FLP_KEY_PROGRAM_IPV4UC, 
        {{"DIP", 4, SOC_PPC_FP_QUAL_HDR_IPV4_DIP},
        {"SIP", 4, SOC_PPC_FP_QUAL_HDR_IPV4_SIP},
        {"NA", 0, BCM_FIELD_ENTRY_INVALID},
        {"NA", 0, BCM_FIELD_ENTRY_INVALID}},
        2},
  ...
  {ARAD_PP_FLP_KEY_PROGRAM_IPV6UC, 
        {{"DIP", 8, SOC_PPC_FP_QUAL_HDR_IPV6_DIP_HIGH},
        {"DIP", 8, SOC_PPC_FP_QUAL_HDR_IPV6_DIP_LOW},
        {"SIP", 8, SOC_PPC_FP_QUAL_HDR_IPV6_SIP_HIGH},
        {"SIP", 8, SOC_PPC_FP_QUAL_HDR_IPV6_SIP_LOW}},
        4},
  {ARAD_PP_FLP_KEY_PROGRAM_IPV6UC_WITH_RPF, 
        {{"DIP", 8, SOC_PPC_FP_QUAL_HDR_IPV6_DIP_HIGH},
        {"DIP", 8, SOC_PPC_FP_QUAL_HDR_IPV6_DIP_LOW},
        {"SIP", 8, SOC_PPC_FP_QUAL_HDR_IPV6_SIP_HIGH},
        {"SIP", 8, SOC_PPC_FP_QUAL_HDR_IPV6_SIP_LOW}},
...
void
  dip_sip_sharing_prog_pos_get(int unit, int prog_id, int* pos){

  int i;
  uint8 prog_index;


  arad_pp_flp_app_to_prog_index_get((unit), ARAD_PP_FLP_KEY_PROGRAM_IPV6UC_WITH_RPF, &prog_index);

  if (prog_index == prog_id) {
    prog_id = ARAD_PP_FLP_KEY_PROGRAM_IPV6UC_WITH_RPF;
  }


  (*pos) = -1;

  for (i = 0; i < TOTAL_NOF_PROGRAMS_IN_QUALIFIER_SHARING; i++) {
    if (dip_sip_sharing[i].prog_id == prog_id) {
      (*pos) = i;
      break;
    }
  }
...
void
  dip_sip_sharing_sharder_qualifiers_order(int unit, int prog_id, ARAD_PP_FP_SHARED_QUAL_INFO* shard_qualifiers_info)
{
  ...
  dip_sip_sharing_prog_pos_get(unit, prog_id, &pos);

  for (i = 0; i < dip_sip_sharing[pos].nof_qualifiers; i++) {
    for (j = 0; j < MAX_NOF_SHARED_QUALIFIERS_PER_PROGRAM; j++) {

      if (dip_sip_sharing[pos].shared_quals[i].qual_type == shard_qualifiers_info_lcl[j].qual_type) {
...
void
  ...
  for (i = 0; i < dip_sip_sharing[pos].nof_qualifiers; i++) {
      ...
      if (dip_sip_sharing[pos].shared_quals[i].qual_type == qual_type) {
        (*is_exists) = 1;
        if (shared_qual_info) {
          sal_memcpy(shared_qual_info->name, dip_sip_sharing[pos].shared_quals[i].name, 20);
          shared_qual_info->qual_type = dip_sip_sharing[pos].shared_quals[i].qual_type;
          shared_qual_info->size = dip_sip_sharing[pos].shared_quals[i].size;
        }        
        break;
      }
  }
...
int
  ...
  /* for IPV6 RPF porg Id is allocated dynamiclly */
  arad_pp_flp_app_to_prog_index_get((unit), ARAD_PP_FLP_KEY_PROGRAM_IPV6UC_WITH_RPF, &prog_index);

  if (prog_index == prog_id) {
    prog_index = ARAD_PP_FLP_KEY_PROGRAM_IPV6UC_WITH_RPF;
  }else{
    prog_index = prog_id;
  }
  ...
  )
{
  ...
  if(!soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "dip_sip_sharing_enable", 0)){    
    for (i = 0; i < TOTAL_NOF_PROGRAMS_IN_QUALIFIER_SHARING; i++) {
      dip_sip_sharing[i].prog_id = -1;
    }
  }else{
    LOG_CLI((BSL_META("\n DIP SIP sharing enabled \n")));
  }
  ...
  )
{
    ...
    {
        pmf_db_info.kaps_db_id = 0; /*need to check how to allocate*/
    }
      ...
      */
      ...
      for (config_phase = 0; config_phase < 2; config_phase++) 
      {
        ...
        for (pmf_pgm_ndx = pmf_pgm_ndx_min; pmf_pgm_ndx < pmf_pgm_ndx_max; ++pmf_pgm_ndx)
        {
            ...
            if (dip_sip_sharing_is_sharing_enabled_for_program(unit, pmf_pgm_ndx)) {
            ...
            dip_sip_sharing_sharder_qualifiers_order(unit, pmf_pgm_ndx, shard_qualifiers_info);
     ...
     uint8*   elk_mask) 
{
  ...
  ARAD_PP_FP_DATABASE_INFO  fp_database_info;
  uint32 max_prog_indx, program_id, pgm_bmp_used;
  uint8  psl_alloc_success;
  uint8   data[SOC_DPP_TCAM_ACTION_ELK_KBP_MAX_LEN_BYTES] = {0};
  uint8   mask[SOC_DPP_TCAM_ACTION_ELK_KBP_MAX_LEN_BYTES] = {0};
  ...
  int i,j, offset = 0, num_of_shared_quals = 0, byte_ndx, prog_pos, handle_64_bit_data = 0;
  int found = 0;
  int qual_pos = -1;
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(0);
/*
  printf("\n************************************\narad_pp_frwrd_ip_tcam_kbp_route_add:: key = \n");
  for (i =0; i < SOC_DPP_TCAM_ACTION_ELK_KBP_MAX_LEN_BYTES; i++) {
      printf("%x:", elk_data[i]);
  }
  printf("\nmask = \n");
  for (i =0; i < SOC_DPP_TCAM_ACTION_ELK_KBP_MAX_LEN_BYTES; i++) {
...
*/
  ...
  SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);*/
    ...
    for (program_id = 0; program_id < max_prog_indx; ++program_id){
     if (SOC_SAND_GET_BIT(pgm_bmp_used, program_id) != 0x1){
        
        continue;
     }
    
     if (dip_sip_sharing_is_sharing_enabled_for_program(unit, program_id)) {
      found = 1; 
      break;
     }
    }
   ...
   dip_sip_sharing_prog_pos_get(unit, program_id, &prog_pos);
   for (i = 0; i < dip_sip_sharing[prog_pos].nof_qualifiers; i++) {     
     for (j = 0; j < SOC_PPC_FP_NOF_QUALS_PER_DB_MAX; j++) {       
       if (fp_database_info.qual_types[j] == dip_sip_sharing[prog_pos].shared_quals[i].qual_type) {
         shard_qualifiers_in_qset[num_of_shared_quals].qual_type = dip_sip_sharing[prog_pos].shared_quals[i].qual_type;
         shard_qualifiers_in_qset[num_of_shared_quals].size = dip_sip_sharing[prog_pos].shared_quals[i].size;
         num_of_shared_quals++;
       }     
     }
   }
   ...
   for (i= 0; i < (SOC_DPP_TCAM_ACTION_ELK_KBP_MAX_LEN_BYTES - (offset)); i++) {
  ...
  )
{
  ...
  for (ind = 0; ind < ARAD_PP_FP_NOF_QUALS_PER_DB_MAX; ++ind)
  {
    ...
    {
      SOC_SAND_ERR_IF_OUT_OF_RANGE(info->qual_types[ind], BCM_FIELD_ENTRY_INVALID, BCM_FIELD_ENTRY_INVALID, ARAD_PP_FP_QUAL_TYPES_END_OF_LIST_ERR, 111, exit);
    }
  }
