    ...
    )
{
    ...
    payload = route_info->dest_id + (1 << 17); /* FEC encoding in PP destination */
    ...
    )
{
    ...
    if (route_info->dest_id.dest_type == SOC_SAND_PP_DEST_MULTICAST) {
        payload = dest_val + (1 << 18); /* MC encoding in PP destination */
    }
    else if (route_info->dest_id.dest_type == SOC_SAND_PP_DEST_FEC) {
        payload = dest_val + (1 << 17); /* FEC encoding in PP destination */
    }
