/*
 * $Id: sbZfCaPtQueueArbParamEntry.c,v 1.2 Broadcom SDK $
...
sbZfCaPtQueueArbParamEntry_Unpack(sbZfCaPtQueueArbParamEntry_t *pToStruct,
  ...
  (pToStruct)->m_uPpeEnqueue =  (uint32)  ((pFromData)[7] >> 7) & 0x01;

  /* Unpack Member: m_uSpare0 */
  (pToStruct)->m_uSpare0 =  (uint32)  ((pFromData)[6] >> 7) & 0x01;
  (pToStruct)->m_uSpare0 |=  (uint32)  ((pFromData)[7] & 0x7f) << 1;

  /* Unpack Member: m_uWeight */
  (pToStruct)->m_uWeight =  (uint32)  ((pFromData)[6] ) & 0x7f;

  /* Unpack Member: m_uRxPort */
  (pToStruct)->m_uRxPort =  (uint32)  (pFromData)[5] ;

  /* Unpack Member: m_uQueue */
  (pToStruct)->m_uQueue =  (uint32)  (pFromData)[4] ;

  /* Unpack Member: m_uPbSelect */
  (pToStruct)->m_uPbSelect =  (uint32)  ((pFromData)[3] >> 7) & 0x01;

  /* Unpack Member: m_uSpare1 */
  (pToStruct)->m_uSpare1 =  (uint32)  ((pFromData)[2] >> 4) & 0x0f;
  (pToStruct)->m_uSpare1 |=  (uint32)  ((pFromData)[3] & 0x7f) << 4;

  /* Unpack Member: m_uDstInterface */
  (pToStruct)->m_uDstInterface =  (uint32)  ((pFromData)[2] ) & 0x0f;

  /* Unpack Member: m_uSpace2 */
  (pToStruct)->m_uSpace2 =  (uint32)  ((pFromData)[1] >> 6) & 0x03;

  /* Unpack Member: m_uDstPort */
  (pToStruct)->m_uDstPort =  (uint32)  ((pFromData)[1] ) & 0x3f;

  /* Unpack Member: m_uDstQueue */
  (pToStruct)->m_uDstQueue =  (uint32)  (pFromData)[0] ;
