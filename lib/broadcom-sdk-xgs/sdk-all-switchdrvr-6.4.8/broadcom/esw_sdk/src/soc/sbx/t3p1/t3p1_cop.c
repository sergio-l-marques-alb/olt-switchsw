...
static uint32 cop_ocm_memory[2] = {2496, 256 }; /* this is calculated by ASM3 */
...
static soc_sbx_t3p1_cop_timer_event_callback_f timer_cb;
...
static int init_policer_segment_ingpol(int unit) {
    ...
    policer_segment_desc[COP_POLICER_SEGMENT_INGPOL].num_entries = 256;
...
static int init_timer_segment_oamtimer(int unit) {
  ...
  rv = soc_sbx_caladan3_cop_segment_register(unit, 0, 0,        
            256,  SOC_SBX_CALADAN3_COP_SEGMENT_TYPE_TIMER, &config);
  if (SOC_FAILURE(rv)) {
    LOG_CLI((BSL_META_U(unit,
  ...
  timer_segment_desc[COP_TIMER_SEGMENT_OAMTIMER].segment = 0;
  timer_segment_desc[COP_TIMER_SEGMENT_OAMTIMER].num_entries = 256;
...
static int init_sequencer_segment_oamseq(int unit) {
  ...
  rv = soc_sbx_caladan3_cop_segment_register(unit, 0, 1,
          64, SOC_SBX_CALADAN3_COP_SEGMENT_TYPE_SN_CHECKER, &config);
  if (SOC_FAILURE(rv)) {
    LOG_CLI((BSL_META_U(unit,
  ...
  sequencer_segment_desc[COP_SEQUENCER_SEGMENT_OAMSEQ].segment = 1;
  sequencer_segment_desc[COP_SEQUENCER_SEGMENT_OAMSEQ].num_entries = 64;
...
static int init_coherent_table_segment_coherent(int unit) {
  ...
  rv = soc_sbx_caladan3_cop_segment_register(unit, 0, 2,
            128, SOC_SBX_CALADAN3_COP_SEGMENT_TYPE_COHERENT, &config);
  if (SOC_FAILURE(rv)) {
    LOG_CLI((BSL_META_U(unit,
  ...
  coherent_table_segment_desc[COP_COHERENT_TABLE_SEGMENT_COHERENT].segment = 2;
  coherent_table_segment_desc[COP_COHERENT_TABLE_SEGMENT_COHERENT].num_entries = 128;
  coherent_table_segment_desc[COP_COHERENT_TABLE_SEGMENT_COHERENT].last_entry_num = 0;

  return SOC_E_NONE;
}

static int init_coherent_table_segment_mac_limit(int unit) {
  int rv;
  soc_sbx_caladan3_cop_segment_type_specific_config_t config;
  ...
  rv = soc_sbx_caladan3_cop_segment_register(unit, 0, 3,
            2048, SOC_SBX_CALADAN3_COP_SEGMENT_TYPE_COHERENT, &config);
  if (SOC_FAILURE(rv)) {
    LOG_CLI((BSL_META_U(unit,
  ...
  coherent_table_segment_desc[COP_COHERENT_TABLE_SEGMENT_MAC_LIMIT].cop_instance = 0;
  coherent_table_segment_desc[COP_COHERENT_TABLE_SEGMENT_MAC_LIMIT].segment = 3;
  coherent_table_segment_desc[COP_COHERENT_TABLE_SEGMENT_MAC_LIMIT].num_entries = 2048;
  coherent_table_segment_desc[COP_COHERENT_TABLE_SEGMENT_MAC_LIMIT].last_entry_num = 0;

  return SOC_E_NONE;
}


int soc_sbx_t3p1_cop_init(int unit) {
  ...
  rv = init_coherent_table_segment_mac_limit(unit);
  if (SOC_FAILURE(rv)) {
      return rv;
  }
  timer_cb = NULL;
...
int soc_sbx_t3p1_cop_uninit(int unit) {
  int i, rv;
  sbx_caladan3_ocm_port_alloc_t ocm_desc;
...
void soc_sbx_t3p1_policer_config_init(soc_sbx_t3p1_policer_config_t *config) {
  sal_memset(config, 0, sizeof(soc_sbx_t3p1_policer_config_t));
...
int soc_sbx_t3p1_policer_create(int unit, soc_sbx_t3p1_policer_segment_t policer_segment, soc_sbx_t3p1_policer_config_t *config, uint32 *policer_handle) {
  int rv;
...
int soc_sbx_t3p1_policer_delete(int unit, soc_sbx_t3p1_policer_segment_t policer_segment, uint32 policer_id) {
  int rv;
  cop_segment_desc_t  *seg_desc;
...
int soc_sbx_t3p1_policer_delete_all(int unit, soc_sbx_t3p1_policer_segment_t policer_segment){
    int rv, num_entries;
...
int soc_sbx_t3p1_policer_read(int unit, soc_sbx_t3p1_policer_segment_t policer_segment, uint32 policer_id, soc_sbx_t3p1_policer_config_t *config) {
  int rv;
  cop_segment_desc_t  *seg_desc;
  uint32 policer_handle;
  soc_sbx_caladan3_cop_policer_config_t policer;
...
int soc_sbx_t3p1_timer_create(int unit, soc_sbx_t3p1_cop_timer_segment_t timer_segment, uint32 timeout_ticks, uint32* timer_handle, uint32 id, int doInterrupt, int start) {
  int rv;
  cop_segment_desc_t *seg_desc;
...
int soc_sbx_t3p1_timer_delete(int unit, uint32 timer_handle) {
  int rv;
...
int soc_sbx_t3p1_timer_delete_all(int unit, soc_sbx_t3p1_cop_timer_segment_t timer_segment){
    int rv, num_entries;
    soc_sbx_caladan3_cop_segment_type_e_t seg_type;
    soc_sbx_caladan3_cop_segment_type_specific_config_t config;
...
int soc_sbx_t3p1_timer_poll(int unit, soc_sbx_t3p1_cop_timer_event_t *event) {
  int rv, i, j;
...
int soc_sbx_t3p1_sequencer_create(int unit, soc_sbx_t3p1_cop_sequencer_segment_t seq_segment, uint32 init_value, uint32 *handle) {
  int rv;
  cop_segment_desc_t *seg_desc;
...
int soc_sbx_t3p1_sequencer_delete(int unit, uint32 seq_handle) {
  int rv;
...
int soc_sbx_t3p1_sequencer_delete_all(int unit, soc_sbx_t3p1_cop_sequencer_segment_t seq_segment){
    int rv, num_entries;
    soc_sbx_caladan3_cop_segment_type_e_t seg_type;
    soc_sbx_caladan3_cop_segment_type_specific_config_t config;
...
int soc_sbx_t3p1_coherent_table_create(int unit, soc_sbx_t3p1_cop_coherent_table_segment_t segment,
...
int soc_sbx_t3p1_coherent_table_get(int unit,
...
int soc_sbx_t3p1_coherent_table_set(int unit,
        soc_sbx_t3p1_cop_coherent_table_segment_t segment, uint32 entry,
        uint32 bits31_0, uint32 bits63_32) {
...
int soc_sbx_t3p1_coherent_table_delete(int unit, uint32 handle) {
  int rv;
...
int soc_sbx_t3p1_coherent_table_delete_all(int unit, soc_sbx_t3p1_cop_coherent_table_segment_t coherent_segment){
    int rv, num_entries;
    soc_sbx_caladan3_cop_segment_type_e_t seg_type;
    soc_sbx_caladan3_cop_segment_type_specific_config_t config;
