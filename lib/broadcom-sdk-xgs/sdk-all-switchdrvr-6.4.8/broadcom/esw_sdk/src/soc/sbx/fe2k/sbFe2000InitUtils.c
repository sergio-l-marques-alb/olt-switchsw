 ...
 * $Id: sbFe2000InitUtils.c,v 1.9 Broadcom SDK $
 *
...
void
CaP2qShadowWrite(int unit, uint8 uPte, uint8 uAddress, uint8 uQueue)
{
    p2q[unit][uPte][uAddress] = uQueue;
}

void
CaP2qShadowRead(int unit, uint8 uPte, uint8 uAddress, uint8 *pQueue)
{
...
uint8 CaPrPortToQueueRead( sbhandle userDeviceHandle,uint32 uPre, uint32 uAddress, uint32 *uQueue) {
  uint32 uData[1];
  uint8 uData_bytes[4];
  ...
  switch( uPre ) {
  case 0:
      status = sbFe2000ImfDriver_PrPortToQueueSr0Read((uint32)userDeviceHandle, uAddress, uData);
      break;
  case 1:
      status = sbFe2000ImfDriver_PrPortToQueueSr1Read((uint32)userDeviceHandle, uAddress, uData);
      break;
   case 2:
      status = sbFe2000ImfDriver_PrPortToQueueAg0Read((uint32)userDeviceHandle, uAddress, uData);
      break;
  case 3:
      status = sbFe2000ImfDriver_PrPortToQueueAg1Read((uint32)userDeviceHandle, uAddress, uData);
      break;
  case 4:
      status = sbFe2000ImfDriver_PrPortToQueueXg0Read((uint32)userDeviceHandle, uAddress, uData);
      break;
  case 5:
      status = sbFe2000ImfDriver_PrPortToQueueXg1Read((uint32)userDeviceHandle, uAddress, uData);
      break;
  case 6:
      status = sbFe2000ImfDriver_PrPortToQueuePciRead((uint32)userDeviceHandle, uAddress, uData);
      break;
  default:
    LOG_ERROR(BSL_LS_SOC_COMMON,
...
uint8 CaPrPortToQueueWrite( sbhandle userDeviceHandle,uint32 uPre, uint32 uAddress, const uint32 uQueue) {
  uint8 bSuccess = FALSE;
  switch( uPre ) {
  case 0:
    {
...
uint8 CaQmQueueState0Read( sbhandle userDeviceHandle,uint32 uAddress, sbZfFe2000QmQueueState0Entry_t *pZFrame ) {
  ...
  sbFe2000ImfDriverStatus_t status;
  status = sbFe2000ImfDriver_QmQueueState0Read((uint32)userDeviceHandle, uAddress, &uData[0]);
  if ( status == SAND_DRV_CA_STATUS_OK ) {
    /* good */
  } else {
    LOG_ERROR(BSL_LS_SOC_COMMON,
...
uint8 CaQmQueueState0Write( sbhandle userDeviceHandle,uint32 uAddress, sbZfFe2000QmQueueState0Entry_t *pZFrame ) {
  ...
  status = sbFe2000ImfDriver_QmQueueState0Write((uint32)userDeviceHandle, uAddress, uData[0]);
  if ( status == SAND_DRV_CA_STATUS_OK ) {
    /* good */
  } else {
    LOG_ERROR(BSL_LS_SOC_COMMON,
...
uint8 CaQmQueueState1Read( sbhandle userDeviceHandle,uint32 uAddress, sbZfFe2000QmQueueState1Entry_t *pZFrame ) {
  ...
  uint8 uData_bytes[4]={0};
  sbFe2000ImfDriverStatus_t status;
  status = sbFe2000ImfDriver_QmQueueState1Read((uint32)userDeviceHandle, uAddress, &uData[0]);
  if ( status == SAND_DRV_CA_STATUS_OK ) {
    /* good */
  } else {
    LOG_ERROR(BSL_LS_SOC_COMMON,
...
uint8 CaPtQueueToPortRead( sbhandle userDeviceHandle,uint32 uAddress, sbZfFe2000PtQueueToPortEntry_t *pZFrame ) {
  ...
  uint8 uData_bytes[4]={0};
  sbFe2000ImfDriverStatus_t status;
  status = sbFe2000ImfDriver_PtQueueToPortRead((uint32)userDeviceHandle, uAddress, &uData[0]);
  if ( status == SAND_DRV_CA_STATUS_OK ) {
    /* good */
  } else {
    LOG_ERROR(BSL_LS_SOC_COMMON,
...
uint8 CaPtQueueToPortWrite( sbhandle userDeviceHandle,uint32 uAddress,  sbZfFe2000PtQueueToPortEntry_t *pZFrame ) {
  ...
  CaSetDataWord(&uData[0],&uData_bytes[0]);
  status = sbFe2000ImfDriver_PtQueueToPortWrite((uint32)userDeviceHandle, uAddress, uData[0]);
  if ( status == SAND_DRV_CA_STATUS_OK ) {
    /* good */
  } else {
    LOG_ERROR(BSL_LS_SOC_COMMON,
...
uint8 CaQmQueueConfigRead( sbhandle userDeviceHandle,uint32 uAddress, sbZfFe2000QmQueueConfigEntry_t *pZFrame) {
  ...
  sbFe2000ImfDriverStatus_t status;
  status = sbFe2000ImfDriver_QmQueueConfigRead((uint32)userDeviceHandle, uAddress, &uData[0], &uData[1], &uData[2]);
  if ( status == SAND_DRV_CA_STATUS_OK ) {
    /* good */
  } else {
    LOG_ERROR(BSL_LS_SOC_COMMON,
...
uint8 CaQmQueueConfigWrite( sbhandle userDeviceHandle,uint32 uAddress, sbZfFe2000QmQueueConfigEntry_t *pZFrame) {
  ...
  status = sbFe2000ImfDriver_QmQueueConfigWrite((uint32)userDeviceHandle, uAddress, uData[0], uData[1], uData[2]);
  if ( status == SAND_DRV_CA_STATUS_OK ) {
    /* good */
  } else {
    LOG_ERROR(BSL_LS_SOC_COMMON,
...
uint8 CaPtPortToQueueSt0Write( sbhandle userDeviceHandle, uint32 uAddress, sbZfFe2000PtPortToQueueEntry_t *pZFrame) {
  ...
  CaSetDataWord(&uData[0],&uData_bytes[0]);
  status = sbFe2000ImfDriver_PtPortToQueueSt0Write((uint32)userDeviceHandle, uAddress, uData[0]);
  if ( status == SAND_DRV_CA_STATUS_OK ) {
    /* good */
  } else {
    LOG_ERROR(BSL_LS_SOC_COMMON,
...
uint8 CaPtPortToQueueSt1Write( sbhandle userDeviceHandle, uint32 uAddress, sbZfFe2000PtPortToQueueEntry_t *pZFrame) {
  ...
  sbZfFe2000PtPortToQueueEntry_Pack(pZFrame,&uData_bytes[0],1);
  CaSetDataWord(&uData[0],&uData_bytes[0]);
  status = sbFe2000ImfDriver_PtPortToQueueSt1Write((uint32)userDeviceHandle, uAddress, uData[0]);
  if ( status == SAND_DRV_CA_STATUS_OK ) {
    /* good */
  } else {
    LOG_ERROR(BSL_LS_SOC_COMMON,
...
uint8 CaPrPortToQueueSr0Write( sbhandle userDeviceHandle, uint32 uAddress, sbZfFe2000PrPortToQueueSr0Entry_t *pZFrame) {
  ...
  CaSetDataWord(&uData[0],&uData_bytes[0]);
  status = sbFe2000ImfDriver_PrPortToQueueSr0Write((uint32)userDeviceHandle, uAddress, uData[0]);
  if ( status == SAND_DRV_CA_STATUS_OK ) {
    /* good */
  } else {
    LOG_ERROR(BSL_LS_SOC_COMMON,
...
uint8 CaPrPortToQueueSr1Write( sbhandle userDeviceHandle,uint32 uAddress,  sbZfFe2000PrPortToQueueSr1Entry_t *pZFrame ) {
  ...
  CaSetDataWord(&uData[0],&uData_bytes[0]);
  status = sbFe2000ImfDriver_PrPortToQueueSr1Write((uint32)userDeviceHandle, uAddress, uData[0]);
  if ( status == SAND_DRV_CA_STATUS_OK ) {
    /* good */
  } else {
    LOG_ERROR(BSL_LS_SOC_COMMON,
...
uint8 CaPrPortToQueueAg0Write( sbhandle userDeviceHandle, uint32 uAddress, sbZfFe2000PrPortToQueueAg0Entry_t *pZFrame) {
  ...
  CaSetDataWord(&uData[0],&uData_bytes[0]);
  status = sbFe2000ImfDriver_PrPortToQueueAg0Write((uint32)userDeviceHandle, uAddress, uData[0]);
  if ( status == SAND_DRV_CA_STATUS_OK ) {
    /* good */
  } else {
    LOG_ERROR(BSL_LS_SOC_COMMON,
...
uint8 CaPrPortToQueueAg1Write( sbhandle userDeviceHandle, uint32 uAddress, sbZfFe2000PrPortToQueueAg1Entry_t *pZFrame ) {
  ...
  CaSetDataWord(&uData[0],&uData_bytes[0]);
  status = sbFe2000ImfDriver_PrPortToQueueAg1Write((uint32)userDeviceHandle, uAddress, uData[0]);
  if ( status == SAND_DRV_CA_STATUS_OK ) {
    /* good */
  } else {
    LOG_ERROR(BSL_LS_SOC_COMMON,
...
uint8 CaPtPortToQueueAg0Write( sbhandle userDeviceHandle,uint32 uAddress, sbZfFe2000PtPortToQueueEntry_t *pZFrame) {
  ...
  sbZfFe2000PtPortToQueueEntry_Pack(pZFrame,&uData_bytes[0],1);
  CaSetDataWord(&uData[0],&uData_bytes[0]);
  status = sbFe2000ImfDriver_PtPortToQueueAg0Write((uint32)userDeviceHandle, uAddress, uData[0]);
  if ( status == SAND_DRV_CA_STATUS_OK ) {
    /* good */
  } else {
    LOG_ERROR(BSL_LS_SOC_COMMON,
...
uint8 CaPtPortToQueueAg1Write( sbhandle userDeviceHandle, uint32 uAddress, sbZfFe2000PtPortToQueueEntry_t *pZFrame) {
  ...
  sbZfFe2000PtPortToQueueEntry_Pack(pZFrame,&uData_bytes[0],1);
  CaSetDataWord(&uData[0],&uData_bytes[0]);
  status = sbFe2000ImfDriver_PtPortToQueueAg1Write((uint32)userDeviceHandle, uAddress, uData[0]);
  if ( status == SAND_DRV_CA_STATUS_OK ) {
    /* good */
  } else {
    LOG_ERROR(BSL_LS_SOC_COMMON,
...
uint8 CaPtPortToQueueXg0Write( sbhandle userDeviceHandle, uint32 uAddress, sbZfFe2000PtPortToQueueEntry_t *pZFrame ) {
  ...
  sbZfFe2000PtPortToQueueEntry_Pack(pZFrame,&uData_bytes[0],1);
  CaSetDataWord(&uData[0],&uData_bytes[0]);
  status = sbFe2000ImfDriver_PtPortToQueueXg0Write((uint32)userDeviceHandle, uAddress, uData[0]);
  if ( status == SAND_DRV_CA_STATUS_OK ) {
    /* good */
  } else {
    LOG_ERROR(BSL_LS_SOC_COMMON,
...
uint8 CaPtPortToQueueXg1Write( sbhandle userDeviceHandle, uint32 uAddress, sbZfFe2000PtPortToQueueEntry_t *pZFrame ) {
  ...
  sbZfFe2000PtPortToQueueEntry_Pack(pZFrame,&uData_bytes[0],1);
  CaSetDataWord(&uData[0],&uData_bytes[0]);
  status = sbFe2000ImfDriver_PtPortToQueueXg1Write((uint32)userDeviceHandle, uAddress, uData[0]);
  if ( status == SAND_DRV_CA_STATUS_OK ) {
    /* good */
  } else {
    LOG_ERROR(BSL_LS_SOC_COMMON,
...
uint8 CaPrPortToQueueXg0Write( sbhandle userDeviceHandle, uint32 uAddress, sbZfFe2000PrPortToQueueXg0Entry_t *pZFrame) {
  ...
  CaSetDataWord(&uData[0],&uData_bytes[0]);
  status = sbFe2000ImfDriver_PrPortToQueueXg0Write((uint32)userDeviceHandle, uAddress, uData[0]);
  if ( status == SAND_DRV_CA_STATUS_OK ) {
    /* good */
  } else {
    LOG_ERROR(BSL_LS_SOC_COMMON,
...
uint8 CaPrPortToQueueXg1Write( sbhandle userDeviceHandle, uint32 uAddress, sbZfFe2000PrPortToQueueXg1Entry_t *pZFrame) {
  ...
  CaSetDataWord(&uData[0],&uData_bytes[0]);
  status = sbFe2000ImfDriver_PrPortToQueueXg1Write((uint32)userDeviceHandle, uAddress, uData[0]);
  if ( status == SAND_DRV_CA_STATUS_OK ) {
  } else {
    LOG_ERROR(BSL_LS_SOC_COMMON,
              (BSL_META("%s:Got bad status 0x%08x"), FUNCTION_NAME(),status));
...
uint8 CaPtPortToQueuePciWrite( sbhandle userDeviceHandle, uint32 uAddress, sbZfFe2000PtPortToQueueEntry_t *pZFrame ) {
  ...
  sbZfFe2000PtPortToQueueEntry_Pack(pZFrame,&uData_bytes[0],1);
  CaSetDataWord(&uData[0],&uData_bytes[0]);
  status = sbFe2000ImfDriver_PtPortToQueuePciWrite((uint32)userDeviceHandle, uAddress, uData[0]);
  if ( status == SAND_DRV_CA_STATUS_OK ) {
    /* good */
  } else {
    LOG_ERROR(BSL_LS_SOC_COMMON,
...
uint8 CaPrPortToQueuePciWrite( sbhandle userDeviceHandle, uint32 uAddress, sbZfFe2000PrPortToQueuePciEntry_t *pZFrame) {
  ...
  CaSetDataWord(&uData[0],&uData_bytes[0]);
  status = sbFe2000ImfDriver_PrPortToQueuePciWrite((uint32)userDeviceHandle,uAddress, uData[0]);
  if ( status == SAND_DRV_CA_STATUS_OK ) {
    /* good */
  } else {
    LOG_ERROR(BSL_LS_SOC_COMMON,
...
uint8 CaQmQueueState0EnWrite( sbhandle userDeviceHandle, uint32 uAddress, sbZfFe2000QmQueueState0EnEntry_t *pZFrame ) {
  ...
  CaSetDataWord(&uData[0],&uData_bytes[0]);
  status = sbFe2000ImfDriver_QmQueueState0EnWrite((uint32)userDeviceHandle, uAddress, uData[0]);
  if ( status == SAND_DRV_CA_STATUS_OK ) {
    /* good */
  } else {
    LOG_ERROR(BSL_LS_SOC_COMMON,
...
uint8 CaPpInitialQueueStateRead( sbhandle userDeviceHandle, uint32 uAddress, sbZfFe2000PpInitialQueueStateEntry_t *pZFrame) {
  ...
  uint8 uData_bytes[12]={0};
  sbFe2000ImfDriverStatus_t status;
  status = sbFe2000ImfDriver_PpInitialQueueStateRead((uint32)userDeviceHandle, uAddress, &uData[0], &uData[1], &uData[2]);
  if ( status == SAND_DRV_CA_STATUS_OK ) {
    /* good */
  } else {
    LOG_ERROR(BSL_LS_SOC_COMMON,
...
uint8 CaPpInitialQueueStateWrite( sbhandle userDeviceHandle, uint32 uAddress, sbZfFe2000PpInitialQueueStateEntry_t *pZFrame) {
  ...
  CaSetDataWord(&uData[1],&uData_bytes[4]);
  CaSetDataWord(&uData[2],&uData_bytes[8]);
  status = sbFe2000ImfDriver_PpInitialQueueStateWrite((uint32)userDeviceHandle, uAddress, uData[0], uData[1], uData[2]);
  if ( status == SAND_DRV_CA_STATUS_OK ) {
      /* good */
  } else {
    LOG_ERROR(BSL_LS_SOC_COMMON,
...
uint8 CaPpHeaderRecordSizeWrite( sbhandle userDeviceHandle, uint32 uAddress, sbZfFe2000PpHeaderRecordSizeEntry_t *pZFrame ) {
  ...
  CaSetDataWord(&uData[0],&uData_bytes[0]);
  status = sbFe2000ImfDriver_PpHeaderRecordSizeWrite((uint32)userDeviceHandle, uAddress, uData[0]);
  if ( status == SAND_DRV_CA_STATUS_OK ) {
      /* good */
  } else {
      LOG_ERROR(BSL_LS_SOC_COMMON,
...
uint8 CaPpQueuePriorityGroupWrite( sbhandle userDeviceHandle, uint32 uAddress, sbZfFe2000PpQueuePriorityGroupEntry_t *pZFrame ) {
  ...
  CaSetDataWord(&uData[0],&uData_bytes[0]);
  status = sbFe2000ImfDriver_PpQueuePriorityGroupWrite((uint32)userDeviceHandle, uAddress, uData[0]);
  if ( status == SAND_DRV_CA_STATUS_OK ) {
      /* good */
  } else {
      LOG_ERROR(BSL_LS_SOC_COMMON,
...
uint8 CaQmQueueConfigFillPattern(sbhandle userDeviceHandle, sbZfFe2000QmQueueConfigEntry_t *pZFrame) {
  ...
  CaSetDataWord(&uData[1],&uData_bytes[4]);
  CaSetDataWord(&uData[2],&uData_bytes[8]);
  status = sbFe2000ImfDriver_QmQueueConfigFillPattern((uint32)userDeviceHandle,uData[0],uData[1],uData[2]);
  if ( status == SAND_DRV_CA_STATUS_OK ) {
      /* good */
  } else {
      LOG_ERROR(BSL_LS_SOC_COMMON,
...
uint8 CaPtPortToQueueAg0FillPattern(sbhandle userDeviceHandle, sbZfFe2000PtPortToQueueEntry_t *pZFrame) {
  ...
  sbZfFe2000PtPortToQueueEntry_Pack(pZFrame,&uData_bytes[0],1);
  CaSetDataWord(&uData[0],&uData_bytes[0]);
  status = sbFe2000ImfDriver_PtPortToQueueAg0FillPattern((uint32)userDeviceHandle,uData[0]);
  if ( status == SAND_DRV_CA_STATUS_OK ) {
      /* good */
  } else {
      LOG_ERROR(BSL_LS_SOC_COMMON,
...
uint8 CaPtPortToQueueAg1FillPattern(sbhandle userDeviceHandle, sbZfFe2000PtPortToQueueEntry_t *pZFrame) {
  ...
  sbZfFe2000PtPortToQueueEntry_Pack(pZFrame,&uData_bytes[0],1);
  CaSetDataWord(&uData[0],&uData_bytes[0]);
  status = sbFe2000ImfDriver_PtPortToQueueAg1FillPattern((uint32)userDeviceHandle,uData[0]);
  if ( status == SAND_DRV_CA_STATUS_OK ) {
      /* good */
  } else {
      LOG_ERROR(BSL_LS_SOC_COMMON,
...
uint8 CaQmQueueState0FillPattern(sbhandle userDeviceHandle, sbZfFe2000QmQueueState0Entry_t *pZFrame) {
  ...
  sbZfFe2000QmQueueState0Entry_Pack(pZFrame,&uData_bytes[0],1);
  CaSetDataWord(&uData[0],&uData_bytes[0]);
  status = sbFe2000ImfDriver_QmQueueState0FillPattern((uint32)userDeviceHandle,uData[0]);
  if ( status == SAND_DRV_CA_STATUS_OK ) {
      /* good */
  } else {
      LOG_ERROR(BSL_LS_SOC_COMMON,
...
uint8 CaQmQueueState1FillPattern(sbhandle userDeviceHandle, sbZfFe2000QmQueueState1Entry_t *pZFrame) {
  ...
  CaSetDataWord(&uData[0],&uData_bytes[0]);
  status = sbFe2000ImfDriver_QmQueueState1FillPattern((uint32)userDeviceHandle,uData[0]);
  if ( status == SAND_DRV_CA_STATUS_OK ) {
      /* good */
  } else {
      LOG_ERROR(BSL_LS_SOC_COMMON,
...
uint8 CaMm0Internal1MemoryWrite(sbhandle userDeviceHandle, uint32 uAddress, sbZfFe2000MmInternal1MemoryEntry_t *pZFrame) {
  ...
  CaSetDataWord(&uData[0],&uData_bytes[0]);
  CaSetDataWord(&uData[1],&uData_bytes[4]);
  status = sbFe2000ImfDriver_Mm0Internal1MemoryWrite((uint32)userDeviceHandle,uAddress,uData[0],uData[1]);
  if ( status == SAND_DRV_CA_STATUS_OK ) {
      /* good */
  } else {
      LOG_ERROR(BSL_LS_SOC_COMMON,
...
uint8 CaMm1Internal0MemoryWrite(sbhandle userDeviceHandle, uint32 uAddress, sbZfFe2000MmInternal0MemoryEntry_t *pZFrame) {
  ...
  CaSetDataWord(&uData[0],&uData_bytes[0]);
  CaSetDataWord(&uData[1],&uData_bytes[4]);
  status = sbFe2000ImfDriver_Mm1Internal0MemoryWrite((uint32)userDeviceHandle,uAddress,uData[0],uData[1]);
  if ( status == SAND_DRV_CA_STATUS_OK ) {
      /* good */
  } else {
      LOG_ERROR(BSL_LS_SOC_COMMON,
...
uint8 CaMm1Internal1MemoryWrite(sbhandle userDeviceHandle, uint32 uAddress, sbZfFe2000MmInternal1MemoryEntry_t *pZFrame) {
  ...
  CaSetDataWord(&uData[0],&uData_bytes[0]);
  CaSetDataWord(&uData[1],&uData_bytes[4]);
  status = sbFe2000ImfDriver_Mm1Internal1MemoryWrite((uint32)userDeviceHandle,uAddress,uData[0],uData[1]);
  if ( status == SAND_DRV_CA_STATUS_OK ) {
      /* good */
  } else {
      LOG_ERROR(BSL_LS_SOC_COMMON,
...
uint8 CaMm0Internal0MemoryRead(sbhandle userDeviceHandle, uint32 uAddress, sbZfFe2000MmInternal0MemoryEntry_t *pZFrame) {
  ...
  uint8 uData_bytes[8] = {0};
  sbFe2000ImfDriverStatus_t status;
  status = sbFe2000ImfDriver_Mm0Internal0MemoryRead((uint32)userDeviceHandle,uAddress,&uData[0],&uData[1]);
  if ( status == SAND_DRV_CA_STATUS_OK ) {
    /* good */
  } else {
    LOG_ERROR(BSL_LS_SOC_COMMON,
...
uint8 CaMm0Internal1MemoryRead(sbhandle userDeviceHandle, uint32 uAddress, sbZfFe2000MmInternal1MemoryEntry_t *pZFrame) {
  ...
  uint8 uData_bytes[8] = {0};
  sbFe2000ImfDriverStatus_t status;
  status = sbFe2000ImfDriver_Mm0Internal1MemoryRead((uint32)userDeviceHandle,uAddress,&uData[0],&uData[1]);
  if ( status == SAND_DRV_CA_STATUS_OK ) {
    /* good */
  } else {
    LOG_ERROR(BSL_LS_SOC_COMMON,
...
uint8 CaMm1Internal0MemoryRead(sbhandle userDeviceHandle, uint32 uAddress, sbZfFe2000MmInternal0MemoryEntry_t *pZFrame) {
  ...
  uint8 uData_bytes[8] = {0};
  sbFe2000ImfDriverStatus_t status;
  status = sbFe2000ImfDriver_Mm1Internal0MemoryRead((uint32)userDeviceHandle,uAddress,&uData[0],&uData[1]);
  if ( status == SAND_DRV_CA_STATUS_OK ) {
    /* good */
  } else {
    LOG_ERROR(BSL_LS_SOC_COMMON,
...
uint8 CaMm1Internal1MemoryRead(sbhandle userDeviceHandle, uint32 uAddress, sbZfFe2000MmInternal1MemoryEntry_t *pZFrame) {
  ...
  uint8 uData_bytes[8] = {0};
  sbFe2000ImfDriverStatus_t status;
  status = sbFe2000ImfDriver_Mm1Internal1MemoryRead((uint32)userDeviceHandle,uAddress,&uData[0],&uData[1]);
  if ( status == SAND_DRV_CA_STATUS_OK ) {
    /* good */
  } else {
    LOG_ERROR(BSL_LS_SOC_COMMON,
...
uint8 CaMm0Internal0MemoryWrite(sbhandle userDeviceHandle, uint32 uAddress, sbZfFe2000MmInternal0MemoryEntry_t *pZFrame) {
  ...
  CaSetDataWord(&uData[0],&uData_bytes[0]);
  CaSetDataWord(&uData[1],&uData_bytes[4]);
  status = sbFe2000ImfDriver_Mm0Internal0MemoryWrite((uint32)userDeviceHandle,uAddress,uData[0],uData[1]);
  if ( status == SAND_DRV_CA_STATUS_OK ) {
      /* good */
  } else {
      LOG_ERROR(BSL_LS_SOC_COMMON,
