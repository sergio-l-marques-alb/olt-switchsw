/*
 * $Id: sbZfFe2000PedDescriptor.c,v 1.4 Broadcom SDK $
...
#include "sbZfFe2000PedDescriptor.hx"
...
uint32
sbZfFe2000PedDescriptor_Pack(sbZfFe2000PedDescriptor_t *pFrom,
                             uint8 *pToData,
                             uint32 nMaxToDataIndex) {
  int i;
  int size = SB_ZF_FE2000PEDDESCRIPTOR_SIZE_IN_BYTES;
  ...
  (pToData)[171] |= ((pFrom)->m_uContinueByte) & 0xFF;

  /* Pack Member: m_uHeaderLength */
  (pToData)[170] |= ((pFrom)->m_uHeaderLength) & 0xFF;

  /* Pack Member: m_uResv0 */
  (pToData)[168] |= ((pFrom)->m_uResv0) & 0xFF;
  (pToData)[169] |= ((pFrom)->m_uResv0 >> 8) &0xFF;

  /* Pack Member: m_uStreamSelector */
  (pToData)[167] |= ((pFrom)->m_uStreamSelector & 0x07) <<5;

  /* Pack Member: m_uBuffer */
  (pToData)[166] |= ((pFrom)->m_uBuffer) & 0xFF;
  (pToData)[167] |= ((pFrom)->m_uBuffer >> 8) & 0x1f;
  ...
  (pToData)[163] |= ((pFrom)->m_uResv1 & 0x0f) <<4;

  /* Pack Member: m_uMirrorIndex */
  (pToData)[163] |= ((pFrom)->m_uMirrorIndex & 0x0f);

  /* Pack Member: m_uCopyCount */
  (pToData)[161] |= ((pFrom)->m_uCopyCount & 0x0f) <<4;
  (pToData)[162] |= ((pFrom)->m_uCopyCount >> 4) &0xFF;

  /* Pack Member: m_uCrcMode */
  (pToData)[161] |= ((pFrom)->m_uCrcMode & 0x03) <<2;

  /* Pack Member: m_uDropBit */
  (pToData)[161] |= ((pFrom)->m_uDropBit & 0x01) <<1;

  /* Pack Member: m_uEnqueue */
  (pToData)[161] |= ((pFrom)->m_uEnqueue & 0x01);

  /* Pack Member: m_uDestinationQueue */
  (pToData)[160] |= ((pFrom)->m_uDestinationQueue) & 0xFF;

  /* Pack Member: m_uXferLength */
  (pToData)[158] |= ((pFrom)->m_uXferLength & 0x3f) <<2;
  (pToData)[159] |= ((pFrom)->m_uXferLength >> 6) &0xFF;

  /* Pack Member: m_uPacketMarking */
  (pToData)[158] |= ((pFrom)->m_uPacketMarking & 0x03);

  /* Pack Member: m_uResv3 */
  (pToData)[157] |= ((pFrom)->m_uResv3 & 0x03) <<6;

  /* Pack Member: m_uSourceFrameLength */
  (pToData)[155] |= ((pFrom)->m_uSourceFrameLength & 0x01) <<7;
  (pToData)[156] |= ((pFrom)->m_uSourceFrameLength >> 1) &0xFF;
  (pToData)[157] |= ((pFrom)->m_uSourceFrameLength >> 9) & 0x3f;
  ...
  return SB_ZF_FE2000PEDDESCRIPTOR_SIZE_IN_BYTES;
}
...
void
sbZfFe2000PedDescriptor_Unpack(sbZfFe2000PedDescriptor_t *pToStruct,
                               uint8 *pFromData,
                               uint32 nMaxToDataIndex) {
  COMPILER_UINT64 tmp;
  ...
  (pToStruct)->m_uContinueByte =  (uint32)  (pFromData)[171] ;

  /* Unpack Member: m_uHeaderLength */
  (pToStruct)->m_uHeaderLength =  (uint32)  (pFromData)[170] ;

  /* Unpack Member: m_uResv0 */
  (pToStruct)->m_uResv0 =  (uint32)  (pFromData)[168] ;
  (pToStruct)->m_uResv0 |=  (uint32)  (pFromData)[169] << 8;

  /* Unpack Member: m_uStreamSelector */
  (pToStruct)->m_uStreamSelector =  (uint32)  ((pFromData)[167] >> 5) & 0x07;

  /* Unpack Member: m_uBuffer */
  (pToStruct)->m_uBuffer =  (uint32)  (pFromData)[166] ;
  (pToStruct)->m_uBuffer |=  (uint32)  ((pFromData)[167] & 0x1f) << 8;

  /* Unpack Member: m_uExceptionIndex */
  (pToStruct)->m_uExceptionIndex =  (uint32)  (pFromData)[165] ;

  /* Unpack Member: m_uSourceQueue */
  (pToStruct)->m_uSourceQueue =  (uint32)  (pFromData)[164] ;

  /* Unpack Member: m_uResv1 */
  (pToStruct)->m_uResv1 =  (uint32)  ((pFromData)[163] >> 4) & 0x0f;

  /* Unpack Member: m_uMirrorIndex */
  (pToStruct)->m_uMirrorIndex =  (uint32)  ((pFromData)[163] ) & 0x0f;

  /* Unpack Member: m_uCopyCount */
  (pToStruct)->m_uCopyCount =  (uint32)  ((pFromData)[161] >> 4) & 0x0f;
  (pToStruct)->m_uCopyCount |=  (uint32)  (pFromData)[162] << 4;

  /* Unpack Member: m_uCrcMode */
  (pToStruct)->m_uCrcMode =  (uint32)  ((pFromData)[161] >> 2) & 0x03;

  /* Unpack Member: m_uDropBit */
  (pToStruct)->m_uDropBit =  (uint32)  ((pFromData)[161] >> 1) & 0x01;

  /* Unpack Member: m_uEnqueue */
  (pToStruct)->m_uEnqueue =  (uint32)  ((pFromData)[161] ) & 0x01;

  /* Unpack Member: m_uDestinationQueue */
  (pToStruct)->m_uDestinationQueue =  (uint32)  (pFromData)[160] ;

  /* Unpack Member: m_uXferLength */
  (pToStruct)->m_uXferLength =  (uint32)  ((pFromData)[158] >> 2) & 0x3f;
  (pToStruct)->m_uXferLength |=  (uint32)  (pFromData)[159] << 6;

  /* Unpack Member: m_uPacketMarking */
  (pToStruct)->m_uPacketMarking =  (uint32)  ((pFromData)[158] ) & 0x03;

  /* Unpack Member: m_uResv3 */
  (pToStruct)->m_uResv3 =  (uint32)  ((pFromData)[157] >> 6) & 0x03;

  /* Unpack Member: m_uSourceFrameLength */
  (pToStruct)->m_uSourceFrameLength =  (uint32)  ((pFromData)[155] >> 7) & 0x01;
  (pToStruct)->m_uSourceFrameLength |=  (uint32)  (pFromData)[156] << 1;
  (pToStruct)->m_uSourceFrameLength |=  (uint32)  ((pFromData)[157] & 0x3f) << 9;
...
void
sbZfFe2000PedDescriptor_InitInstance(sbZfFe2000PedDescriptor_t *pFrame) {
  ...
  pFrame->m_uSourceFrameLength =  (unsigned int)  0;
