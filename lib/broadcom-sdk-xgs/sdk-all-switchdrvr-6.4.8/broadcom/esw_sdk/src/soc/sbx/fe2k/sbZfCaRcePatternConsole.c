/*
 * $Id: sbZfCaRcePatternConsole.c,v 1.3 Broadcom SDK $
...
sbZfCaRcePattern_Print(sbZfCaRcePattern_t *pFromStruct) {
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("CaRcePattern:: patd=0x%08x"), (unsigned int)  pFromStruct->m_uPatternD));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" patc=0x%08x"), (unsigned int)  pFromStruct->m_uPatternC));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" patb=0x%08x"), (unsigned int)  pFromStruct->m_uPatternB));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("\n")));

  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("CaRcePattern:: pata=0x%08x"), (unsigned int)  pFromStruct->m_uPatternA));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("\n")));
...
char *
sbZfCaRcePattern_SPrint(sbZfCaRcePattern_t *pFromStruct, char *pcToString, uint32 lStrSize) {
  uint32 WrCnt = 0x0;
