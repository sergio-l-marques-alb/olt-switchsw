...
_bcm_dpp_in_lif_mpls_term_match_add(int unit, bcm_mpls_tunnel_switch_t *mpls_term, int lif)
{
    ...
    BCMDNX_IF_ERR_EXIT(_bcm_dpp_lif_to_sw_resources(unit, lif, -1, _BCM_DPP_GPORT_SW_RESOURCES_INGRESS, &gport_sw_resources));
...
_bcm_dpp_out_lif_mpls_tunnel_match_add(int unit, int lif)
{
    ...
    BCMDNX_IF_ERR_EXIT(_bcm_dpp_lif_to_sw_resources(unit, -1, lif, _BCM_DPP_GPORT_SW_RESOURCES_EGRESS, &gport_sw_resources));

    gport_sw_resources.out_lif_sw_resources.lif_type = _bcmDppLifTypeMplsTunnel;

    BCMDNX_IF_ERR_EXIT(_bcm_dpp_lif_sw_resources_set(unit, -1, lif, _BCM_DPP_GPORT_SW_RESOURCES_EGRESS, &gport_sw_resources));
    ...
    int unit)
{
    ...
    if ( soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "prge_ptagged_load", 0)) {
        rv = arad_egr_prog_editor_prog_sel_outlif_profile_set(unit, SOC_OCC_MGMT_OUTLIF_APP_CUSTOM_P_TAGGED_TYPE,
                                                              ARAD_EGR_PROG_EDITOR_PROG_USER_MODE_PTAGGED, 1 /*value*/,
                                                              1/*outlif-profile1*/);
        rv = arad_egr_prog_editor_prog_sel_outlif_profile_set(unit, SOC_OCC_MGMT_OUTLIF_APP_CUSTOM_P_TAGGED_TYPE,
                                                              ARAD_EGR_PROG_EDITOR_PROG_SERVICE_MODE_PTAGGED, 2 /*value*/,
                                                              1/*outlif-profile1*/);
        rv = arad_egr_prog_editor_prog_sel_outlif_profile_set(unit, SOC_OCC_MGMT_OUTLIF_APP_CUSTOM_P_TAGGED_TYPE,
                                                              ARAD_EGR_PROG_EDITOR_PROG_USER_MODE_NOT_PTAGGED, 3 /*value*/,
                                                              1/*outlif-profile1*/);
        BCMDNX_IF_ERR_EXIT(rv);
    }
    ...
    uint8 * is_allocated)
{
    ...
    /* Making sure this profile is allocated by trying to add it in test mode
...
int _bcm_mpls_port_delete(
   ...
   bcm_gport_t          port_id
    ...
    _bcm_dpp_gport_hw_resources gport_hw_resources;
    uint8 remove_fec;
    SOC_PPD_L2_LIF_PWE_ADDITIONAL_INFO  pwe_additional_info;
    ...
    rv = _bcm_dpp_gport_l2_intf_gport_has_remote_lif(unit, port_id, &remote_lif);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    if ((BCM_GPORT_SUB_TYPE_IS_PROTECTION(port_id) || BCM_GPORT_SUB_TYPE_IS_FORWARD_GROUP(port_id)) && (gport_hw_resources.fec_id != -1)) {
        rv = bcm_dpp_am_fec_get_usage(unit, _BCM_PPD_GPORT_FEC_TO_WORK_FEC(gport_hw_resources.fec_id), &usage);
        BCMDNX_IF_ERR_EXIT(rv);

        status = bcm_dpp_am_fec_is_alloced(unit, 0 /*flags*/, usage, 1, gport_hw_resources.fec_id);
    ...
    if (gport_hw_resources.fec_id != -1) {
        ...
        if (!is_ingress && !is_egress) { /* Forwarding group */
            rv = bcm_dpp_am_fec_dealloc(unit, 0, usage, 1, gport_hw_resources.fec_id);
        } else if (is_ingress && is_egress && _BCM_PPD_GPORT_IS_WORKING_PORT_ID(gport_hw_resources.fec_id)) {
            rv = bcm_dpp_am_fec_dealloc(unit, 0, usage, 2, gport_hw_resources.fec_id);
        } else {
    ...
    if (!remote_lif) {
        ...
        if (mpls_port.egress_label.label != BCM_MPLS_LABEL_INVALID) {
            ...
            if (is_ingress && !is_egress) {
                ...
                /* for egress lif can be taken from HW */
                rv = bcm_petra_mpls_tunnel_free_push_profile(unit, gport_hw_resources.global_out_lif, 1 /*is_pwe*/);
                BCMDNX_IF_ERR_EXIT(rv);
            }
        }
        ...
        if (is_egress) {
            ...
            rv = soc_ppd_eg_encap_entry_remove(unit, SOC_PPD_EG_ENCAP_EEP_TYPE_LIF_EEP, gport_hw_resources.local_out_lif);
    ...
    bcm_gport_t mpls_port_id)
{
    ...
    /* Remote LIF existance should be checked prior to the FEC removal */
    rv = _bcm_dpp_gport_l2_intf_gport_has_remote_lif(unit, mpls_port_id, &remote_lif);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    bcm_mpls_port_t * mpls_port)
{
    ...
    rv = _bcm_dpp_gport_l2_intf_gport_has_remote_lif(unit, mpls_port->mpls_port_id, &remote_lif);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    if (is_ingress) {
        /* Ensure InLIF is PWE */
        BCMDNX_IF_ERR_EXIT( _bcm_dpp_lif_sw_resources_lif_usage_get(soc_sand_dev_id, gport_hw_resources.local_in_lif, -1, &lif_usage, NULL) );
    ...
    if (gport_type == _bcmDppGportResolveTypeFec) {
        ...
        uint32 fec_id = _BCM_PPD_GPORT_FEC_TO_WORK_FEC(dest_id.dest_val);
        _bcm_dpp_am_fec_alloc_usage usage;

        if (dest_id.dest_val < SOC_DPP_CONFIG(unit)->l3.fec_ecmp_reserved) {
            /* Temporary solution, remove when SDK-83476 fixed */
    ...
    if (is_egress) {
        rv = _bcm_dpp_lif_sw_resources_lif_usage_get(soc_sand_dev_id, -1, gport_hw_resources.local_out_lif, NULL, &lif_usage);
        BCMDNX_IF_ERR_EXIT(rv);
        if (lif_usage != _bcmDppLifTypeMplsPort) {
    ...
    if ((_BCM_PPD_IS_VALID_FAILOVER_ID(mpls_port->failover_id) && _BCM_MULTICAST_IS_SET(mpls_port->failover_mc_group)) ||
        (!_BCM_PPD_IS_VALID_FAILOVER_ID(mpls_port->failover_id) && ((mpls_port->failover_port_id != BCM_GPORT_TYPE_NONE) && is_ingress && !is_egress)) ||
        (!_BCM_PPD_IS_VALID_FAILOVER_ID(mpls_port->failover_id) && ((mpls_port->failover_port_id == BCM_GPORT_TYPE_NONE) && is_ingress && !BCM_ENCAP_REMOTE_GET(mpls_port->encap_id)))) {
...
#ifdef BCM_88660_A0
    term_profile_info.check_bos = _BCM_DPP_MPLS_LIF_TERM_CHECK_BOS; /*for pwe this flag should not be set*/
    if (SOC_IS_JERICHO(unit)) {
    ...
    uint8 is_pwe)
{
    ...
    SOC_PPD_EG_ENCAP_EEP_TYPE eep_type;
    uint32 push_profile;
    ...
    eep_type = is_pwe ? SOC_PPD_EG_ENCAP_EEP_TYPE_LIF_EEP : SOC_PPD_EG_ENCAP_EEP_TYPE_TUNNEL_EEP;

    soc_sand_rv =
        soc_ppd_eg_encap_entry_get(soc_sand_dev_id,
                               eep_type, local_lif_id, 0,
                               encap_entry_info, next_eep, &nof_entries);
    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);

    if (is_pwe) {
        /* For PWE only first tunnel is relevant */
        nof_entries = 1;
    }

    entry_type = is_pwe ? SOC_PPD_EG_ENCAP_ENTRY_TYPE_PWE : SOC_PPD_EG_ENCAP_ENTRY_TYPE_MPLS_ENCAP;

    for (indx = 0; indx < nof_entries; ++indx) {
        if (encap_entry_info[indx].entry_type == entry_type) {
            cur_count = is_pwe ? 1 : encap_entry_info[indx].entry_val.mpls_encap_info.nof_tunnels;
            for (cur_indx = 0; cur_indx < cur_count; ++cur_indx) {
                if (is_pwe) {
                    push_profile = encap_entry_info[indx].entry_val.pwe_info.push_profile;
                }
                else {
                    push_profile = encap_entry_info[indx].entry_val.mpls_encap_info.tunnels[cur_indx].push_profile;
                }
    ...
    ) /* needed only in case intf is of type EEP*/
{
    ...
    for (indx = 0; indx < num_labels; ++indx) {
        ...
        if (label_array[indx].flags & BCM_MPLS_EGRESS_LABEL_REPLACE){
            ...
            BCMDNX_IF_ERR_EXIT( _bcm_dpp_lif_sw_resources_lif_usage_get(unit, -1, local_lif_id, NULL, &usage) );
    ...
    for (start_indx = num_labels; start_indx > 0; start_indx = start_indx - 1 - skip_entries) {
        ...
        if ((label_array[indx].flags & BCM_MPLS_EGRESS_LABEL_WITH_ID) && (label_array[indx].flags & BCM_MPLS_EGRESS_LABEL_REPLACE)) {
            ...
            rv = bcm_petra_mpls_tunnel_free_push_profile(unit, eep, 0/* is pwe */);
            BCMDNX_IF_ERR_EXIT(rv);
        }
        ...
        if(!eep_alloced) {
            ...
            if (SOC_IS_PETRAB(unit)) {
                  ...
                  if(_Bcm_dpp_sync_lif_eep) {
                      lif_id = eep;
                      rv = bcm_dpp_am_l2_vpn_pwe_alloc(unit, _BCM_DPP_AM_L2_PWE_TERM_DEFAULT, flags, 2, &lif_id);
                      BCMDNX_IF_ERR_EXIT(rv);
            ...
            if (SOC_IS_JERICHO(unit))
            {
                ...
                SOC_PPD_EG_ENCAP_ENTRY_TYPE ll_entry_type; 

                /* get local LL outlif from global LL outlif.
                the interface object l3_intf_id of type encap contains global LL outlif */
                rv = _bcm_dpp_global_lif_mapping_global_to_local_get(unit, _BCM_DPP_GLOBAL_LIF_MAPPING_EGRESS, BCM_L3_ITF_VAL_GET(label_array[indx].l3_intf_id), &local_ll_outlif);
                BCMDNX_IF_ERR_EXIT(rv);
                ...
                if (ll_entry_type == SOC_PPD_EG_ENCAP_ENTRY_TYPE_ROO_LL_ENCAP \
                    && SOC_IS_ROO_ENABLE(unit) && soc_property_get(unit, spn_EVPN_ENABLE, 0)) {
                ...
                if (counting_profile_is_allocated) {
                    if (label_array->outlif_counting_profile != BCM_STAT_LIF_COUNTING_PROFILE_NONE && BCM_DPP_COUNTER_LIF_PROFILE_IS_INGRESS(label_array->outlif_counting_profile)) {
                    ...
                    local_out_lif_info.counting_profile_id = label_array->outlif_counting_profile;
                } else {
    ...
    bcm_if_t intf)
{
    ...
    /* Remove entry from SW DB */
    rv = _bcm_dpp_lif_sw_resources_delete(unit, -1, local_lif_id, _BCM_DPP_GPORT_SW_RESOURCES_EGRESS);
    BCMDNX_IF_ERR_EXIT(rv); 
    ...
    int unit)
{
    ...
    for (indx = 0; indx < SOC_DPP_DEFS_GET(unit, nof_out_lifs) ; indx+=2 /* MPLS always even */) {
        /* Get LIF usage */
        BCMDNX_IF_ERR_EXIT( _bcm_dpp_lif_sw_resources_lif_usage_get(unit, -1, indx, NULL, &lif_usage));
    ...
    int *label_count)
{
    ...
    BCMDNX_IF_ERR_EXIT(_bcm_dpp_lif_sw_resources_lif_usage_get(soc_sand_dev_id, -1, local_lif_id, NULL, &lif_usage));
    if ((lif_usage != _bcmDppLifTypeMplsTunnel) 
    ...
    SOC_PPD_MPLS_LABEL_RIF_KEY *rif_key)
{
    ...
    if (SOC_IS_JERICHO(unit) && soc_property_get(unit, spn_EVPN_ENABLE, 0)) {
        ...
        rif_key->flags |= (is_iml_bos_indication) ? SOC_PPD_RIF_MPLS_LABEL_EXPECT_BOS : 0;
        rif_key->flags |= ((info->flags & BCM_MPLS_SWITCH_EVPN_IML) | is_iml_bos_indication) ? SOC_PPD_RIF_MPLS_LABEL_EVPN_IML : 0;
    }
exit:
    BCMDNX_FUNC_RETURN;
}
    ...
    if (rv == BCM_E_NOT_FOUND) {
        ...
        if (update) {/* update check LIF already allocated */
            ...
            if (SOC_IS_JERICHO(unit)) {
                ...
                BCMDNX_IF_ERR_EXIT(rv);
                if (counting_profile_is_allocated) {
                    if (info->inlif_counting_profile != BCM_STAT_LIF_COUNTING_PROFILE_NONE && !BCM_DPP_COUNTER_LIF_PROFILE_IS_INGRESS(info->inlif_counting_profile)) {
                        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("InLIF-counting-profile %d is out of range."), info->inlif_counting_profile));
                    }
                    local_inlif_info.counting_profile_id = info->inlif_counting_profile;
                } else {
    ...
    bcm_mpls_tunnel_switch_t * info)
{
    ...
    if (rv == BCM_E_NOT_FOUND) {
        ...
        if (rif_key.flags & SOC_PPD_RIF_MPLS_LABEL_LOOKUP_NONE) {
           ...
           BCMDNX_IF_ERR_EXIT( _bcm_dpp_lif_sw_resources_lif_usage_get(unit, local_in_lif_id, -1, &lif_usage, NULL) );
        ...
        rv = _bcm_dpp_lif_sw_resources_delete(unit, local_in_lif_id, -1, _BCM_DPP_GPORT_SW_RESOURCES_INGRESS);
        BCMDNX_IF_ERR_EXIT(rv); 
    ...
    int unit)
{
    ...
    for (cur_lif = 0; cur_lif < _BCM_GPORT_NOF_LIFS; ++cur_lif) {
        ...
        if (usage == _bcmDppLifTypeMplsTunnel) {
            ...
            /* remove status */
            rv = _bcm_dpp_lif_sw_resources_delete(unit, cur_lif, -1, _BCM_DPP_GPORT_SW_RESOURCES_INGRESS);
            BCMDNX_IF_ERR_EXIT(rv); 
        }
    }
    ...
    uint8 is_one_plus_one_protection)
{
    ...
    if (intf_global_lif != 0) {
        if (SOC_IS_JERICHO(unit) && !is_one_plus_one_protection && ((BCM_L3_ITF_TYPE_IS_FEC(mpls_port->egress_tunnel_if) && _BCM_PPD_IS_VALID_FAILOVER_ID(mpls_port->failover_id)) \
...
int _bcm_dpp_mpls_port_pwe_set(
   ...
   SOC_PPC_EG_ENCAP_PWE_INFO *old_pwe_info) {
    ...
    SOC_PPD_FEC_ID fec_id = -1, protect_fec_id;
    int update = 0;
    ...
    */
    ...
    if (with_id) {
        ...
        if ((is_forward_group || is_1_to_1_protected) && SOC_IS_JERICHO(unit) && update == 1) {
            ...
            if (!update) {
                ...
                if (SOC_IS_JERICHO(unit)) {
                    ...
                    if (is_ingress) {
                        ...
                        if (counting_profile_is_allocated) {
                            if (mpls_port->inlif_counting_profile != BCM_STAT_LIF_COUNTING_PROFILE_NONE && !BCM_DPP_COUNTER_LIF_PROFILE_IS_INGRESS(mpls_port->inlif_counting_profile)) {
                                BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("OutLIF-counting-profile %d is out of range."), mpls_port->inlif_counting_profile));
                            }
                            local_inlif_info.counting_profile_id = mpls_port->inlif_counting_profile;
                        } else {
                            local_inlif_info.counting_profile_id = BCM_STAT_LIF_COUNTING_PROFILE_NONE;
                        }
                    ...
                    if (allocate_egress) {
                        ...
                        if (counting_profile_is_allocated) {
                            if (mpls_port->egress_label.outlif_counting_profile != BCM_STAT_LIF_COUNTING_PROFILE_NONE && 
                                BCM_DPP_COUNTER_LIF_PROFILE_IS_INGRESS(mpls_port->egress_label.outlif_counting_profile)) {
                            ...
                            local_out_lif_info.counting_profile_id = mpls_port->egress_label.outlif_counting_profile;
                        } else {
                            local_out_lif_info.counting_profile_id = BCM_STAT_LIF_COUNTING_PROFILE_NONE;
                        }
                    ...
                    if (allocate_egress) {
                        if (is_ingress) {
                            rv = bcm_dpp_am_l2_vpn_pwe_alloc(unit,pwe_lif_types,lif_flags,1,(SOC_PPD_LIF_ID*)&global_lif_id);
                            BCMDNX_IF_ERR_EXIT(rv);
    ...
    if (!remote_lif && is_ingress) {
        ...
        if (SOC_IS_JERICHO(unit) && soc_property_get(unit, spn_SPLIT_HORIZON_FORWARDING_GROUPS_MODE, 1)) {
            ...
            if (update) {
                SOC_PPD_L2_LIF_PWE_INFO  pwe_info_old;
                uint8 found;
        ...
        if (_BCM_DDP_IS_MP_VPN(vpn) && !is_gal) {
            ...
            if (_BCM_PPD_IS_VALID_FAILOVER_ID(mpls_port->failover_id)) {
                ...
                rv = _bcm_dpp_lif_sw_resources_learn_gport_set(unit, local_in_lif, mpls_port->mpls_port_id);
                BCMDNX_IF_ERR_EXIT(rv);
            }
            /* protected case, where need to configure only LIF,
               in this case failover-id is invalid, but failover_port_id includes the FEC id to learn */
                ...
                rv = _bcm_dpp_lif_sw_resources_learn_gport_set(unit, local_in_lif, mpls_port->failover_port_id);
                BCMDNX_IF_ERR_EXIT(rv);
          }
          /* protected-tunnel: learn FEC + VC */
              ...
              if (BCM_L3_ITF_TYPE_IS_LIF(mpls_port->egress_tunnel_if)) {
                ...
                if ((is_ingress && is_egress && !(mpls_port->flags2 & BCM_MPLS_PORT2_LEARN_ENCAP)) || is_forward_group) {
                    rv = _bcm_dpp_gport_fix_fec_type(unit, BCM_L3_ITF_VAL_GET(mpls_port->egress_tunnel_if), SOC_PPD_FEC_TYPE_TUNNELING);
                    BCMDNX_IF_ERR_EXIT(rv);
                }
              }
              rv = _bcm_dpp_lif_sw_resources_learn_gport_set(unit, local_in_lif, mpls_port->mpls_port_id);
          ...
          /* store vsi */
          rv = _bcm_dpp_lif_sw_resources_vsi_set(unit, local_in_lif, vpn);
          BCMDNX_IF_ERR_EXIT(rv);
      }
      else /* p2p */ 
      {
    ...
    if(update && !remote_lif && is_ingress) {
        ...
        if(mpls_port->match_label != mpls_port_o.match_label){
            ...
            if(local_in_lif_o != local_in_lif) {
                rv = _bcm_dpp_lif_sw_resources_delete(unit, local_in_lif_o, -1, _BCM_DPP_GPORT_SW_RESOURCES_INGRESS);
                BCMDNX_IF_ERR_EXIT(rv); 
            }
        }
    }
    ...
    if (update) {
        if (!with_id) {
            ...
            if (BCM_GPORT_SUB_TYPE_IS_FORWARD_GROUP(mpls_port->mpls_port_id) || BCM_GPORT_SUB_TYPE_IS_PROTECTION(mpls_port->mpls_port_id)) {
                ...
                /* Check if fec is allocated */
                if (gport_hw_resources.fec_id != -1) {
                    rv = bcm_dpp_am_fec_get_usage(unit, _BCM_PPD_GPORT_FEC_TO_WORK_FEC(gport_hw_resources.fec_id), &fec_usage);
                    BCMDNX_IF_ERR_EXIT(rv);

                    status = bcm_dpp_am_fec_is_alloced(unit, 0 /*flags*/, fec_usage, 1, gport_hw_resources.fec_id);
                    if (status == BCM_E_NOT_FOUND) {
                        BCMDNX_ERR_EXIT_MSG(BCM_E_NOT_FOUND, (_BSL_BCM_MSG("In case of BCM_MPLS_PORT_REPLACE, the mpls_port_id must exist")));
                    }
                    else if (status != BCM_E_EXISTS) {
