    ...
    int unit,SOC_PPD_TRAP_FRWRD_ACTION_PROFILE_INFO* prm_profile_info,bcm_rx_trap_config_t *config)
{
    ...
    if (prm_profile_info->bitmap_mask & SOC_PPD_TRAP_ACTION_PROFILE_OVERWRITE_DEST) {        
        if (prm_profile_info->dest_info.frwrd_dest.type == SOC_PPD_FRWRD_DECISION_TYPE_MC) {
            ...
            rv = _bcm_dpp_gport_and_encap_from_fwd_decision(unit, &config->dest_port, NULL, &(prm_profile_info->dest_info.frwrd_dest), -1/*encap_usage*/);
            ...
            {
                BCM_FORWARD_ENCAP_ID_VAL_SET(config->encap_id, BCM_FORWARD_ENCAP_ID_TYPE_OUTLIF , BCM_FORWARD_ENCAP_ID_OUTLIF_USAGE_GENERAL ,  prm_profile_info->dest_info.frwrd_dest.additional_info.outlif.val);
...
{ 
    ...
    if (_BCM_TRAP_ID_IS_ETPP(trap_id_sw)) 
    {
        ...
        {
            if (prm_egr_profile_info.out_tm_port == SOC_PPD_TRAP_ACTION_PKT_DISCARD_ID) 
            {
