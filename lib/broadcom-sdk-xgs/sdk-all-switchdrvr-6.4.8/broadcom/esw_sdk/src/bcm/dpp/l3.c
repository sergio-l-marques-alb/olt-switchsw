  ...
  )
{
  ...
  if (is_host) {
      /* EEP is valid this host points to: FEC + EEP */
      if (BCM_L3_ITF_TYPE_IS_FEC(r_info->l3a_intf) && encap_id != 0) {
...
)
{
    ...
    /* FEC. For roo application in arad+, always a vxlan gport */
       ...
       rv = _bcm_dpp_gport_and_encap_from_fwd_decision(unit,&(info->l3a_port_tgid),NULL,&frwrd_decision_local, -1/*encap_usage*/);
       BCMDNX_IF_ERR_EXIT(rv);
    }
  ...
  )
{
  ...
  if (route_info.additional_info.eei.type == SOC_PPD_EEI_TYPE_OUTLIF) {
      /* Retrieve the encap-id for Host table */
      BCM_L3_ITF_SET(info->encap_id, BCM_L3_ITF_TYPE_LIF, route_info.additional_info.eei.val.outlif); 
  }
    ...
    bcm_if_t *if_id)
{
    ...
    if (is_egress) {    
        ...
        if ((encap_with_id) && (flags & BCM_L3_REPLACE)) { /* second call */
			...
			status = (SOC_IS_JERICHO(unit)) ? bcm_dpp_am_global_lif_is_alloced(unit, BCM_DPP_AM_FLAG_ALLOC_EGRESS, tunnel_global_lif) : bcm_dpp_am_l3_eep_is_alloced(unit, ll_global_lif);
            if (status != BCM_E_EXISTS) {
