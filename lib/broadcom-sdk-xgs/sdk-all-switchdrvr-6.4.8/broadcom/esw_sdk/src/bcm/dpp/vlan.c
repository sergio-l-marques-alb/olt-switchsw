...
_bcm_dpp_in_lif_ac_match_add(int unit, bcm_vlan_port_t *vlan_port, int in_lif)
{
    ...
    sal_memset(&gport_sw_resources, 0x0, sizeof(_bcm_dpp_gport_sw_resources));
    ...
    BCMDNX_IF_ERR_EXIT(_bcm_dpp_lif_sw_resources_set(unit, in_lif, -1, _BCM_DPP_GPORT_SW_RESOURCES_INGRESS, &gport_sw_resources));
    ...
    int unit)
{
    ...
    if(SOC_IS_ARADPLUS_AND_BELOW(unit) && soc_property_suffix_num_get(unit,-1,spn_CUSTOM_FEATURE,"pon_wihout_rif_support",0)) {
        /*resever lif 0 for customer application*/
        rv = dpp_am_res_alloc(unit, 0, dpp_am_res_obs_inlif, BCM_DPP_AM_FLAG_ALLOC_WITH_ID, 1, &object);
        BCMDNX_IF_ERR_EXIT(rv);
    }
    else{
        /* 
    ...
    bcm_vlan_action_set_t *action)
{
    ...
    if (SOC_IS_PETRAB(unit)) {        
        ...
        if (ac_key_map_type == _bcm_petra_ac_key_map_type_normal) {
            ...
            rv = bcm_petra_tpid_profile_info_set(unit, port_info.port_profile, port_info.port_profile_eg,profile_type, accept_mode, dtag_mode, new_ac_key_map_type, port_info.port_type == SOC_SAND_PP_PORT_L2_TYPE_CEP, FALSE /* Petra-B no EVB */);
            BCMDNX_IF_ERR_EXIT(rv);
        }
    }
    ...
    bcm_vlan_t inner_vlan_high)
{
    ...
    if (SOC_IS_PETRAB(unit) && (is_port_comp_vlan_existed == FALSE)) {
        ...
        if (ac_key_map_type == _bcm_petra_ac_key_map_type_compressional) {
            ...
            rv = bcm_petra_tpid_profile_info_set(unit, port_info.port_profile,port_info.port_profile_eg, profile_type, accept_mode, dtag_mode, new_ac_key_map_type, port_info.port_type == SOC_SAND_PP_PORT_L2_TYPE_CEP, FALSE /* PetraB no EVB */);
            BCMDNX_IF_ERR_EXIT(rv);
        }        
    }
    ...
    if (is_port_flooding || (old_template != new_template)) {
        ...
        if (is_lif_flooding) {        
          ...
          if ((lif_entry_info->type != SOC_PPD_LIF_ENTRY_TYPE_AC) && (lif_entry_info->type != SOC_PPD_LIF_ENTRY_TYPE_FIRST_AC_IN_GROUP) &&
              (lif_entry_info->type != SOC_PPD_LIF_ENTRY_TYPE_PWE) && (lif_entry_info->type != SOC_PPD_LIF_ENTRY_TYPE_EXTENDER)) {
    ...
    bcm_gport_t gport)
{
    ...
    int local_pon_out_ac_id= 0;
    int active_local_out_lif_id = 0;
    _bcm_dpp_gport_info_t gport_info;
    _bcm_dpp_gport_parse_info_t gport_parse_info;
    ...
    */
    ...
    if (is_egress) {
        BCMDNX_IF_ERR_EXIT(_bcm_dpp_lif_sw_resources_lif_usage_get(unit, -1, gport_hw_resources.local_out_lif, NULL, &lif_usage));
        global_lif = gport_hw_resources.global_out_lif;
    }
    else if (is_ingress) {
        BCMDNX_IF_ERR_EXIT(_bcm_dpp_lif_sw_resources_lif_usage_get(unit, gport_hw_resources.local_in_lif, -1, &lif_usage, NULL));        
    }
    ...
    if ((is_ingress || is_egress) && (lif_usage != _bcmDppLifTypeVlan)) { /* VLAN */
        if ( (SOC_DPP_CONFIG(unit)->pp.pon_application_enable)&&SOC_IS_ARAD_B1_AND_BELOW(unit)) {
            ...
            rv = _bcm_dpp_pon_encap_data_enty_get(unit, (uint32)gport_hw_resources.local_out_lif, &tpid, &pcp, &tunnel_id, &eedb_id_3_tags);
            BCMDNX_IF_ERR_EXIT(rv);
            local_pon_out_ac_id = eedb_id_3_tags;
            BCMDNX_IF_ERR_EXIT(_bcm_dpp_lif_sw_resources_lif_usage_get(unit, -1, local_pon_out_ac_id, NULL, &lif_usage));
    ...
    rv = _bcm_dpp_gport_l2_intf_gport_has_remote_lif(unit,gport,&remote_lif);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    if (is_local) {
        ...
        if (is_egress && (SOC_DPP_CONFIG(unit)->pp.pon_application_enable) && (BCM_E_EXISTS == bcm_dpp_am_out_ac_is_alloced(unit, gport_hw_resources.local_out_lif))) { 
            ...
            rv = _bcm_dpp_pon_lif_is_3_tags_data(unit,gport_hw_resources.local_in_lif, &is_3_tags_data);
            BCMDNX_IF_ERR_EXIT(rv);
        }
    }
    ...
    if (!remote_lif) {
        ...
        if (SOC_DPP_IS_VLAN_TRANSLATE_MODE_NORMAL(unit)) {
            ...
            if (is_egress) {
                ...
                if (!(SOC_DPP_CONFIG(unit)->pp.egress_vlan_filtering && (vlan_port.flags & BCM_VLAN_PORT_EXTENDED))) {
                    if(is_3_tags_data){
                        active_local_out_lif_id = local_pon_out_ac_id;
                    }
                    else{
                        active_local_out_lif_id = gport_hw_resources.local_out_lif;
                    }
        ...
        if (SOC_IS_JERICHO(unit)) {
            ...
            if (is_local) {
                ...
                if (is_3_tags_data) {
                    ...
                    outlif_deallocation_type = _BCM_DPP_AM_OUT_AC_TYPE_PON_3_TAGS_DATA;
            ...
            if (is_ingress) {
                ...
                if (bcm_dpp_am_l2_ac_is_alloced(unit, gport_hw_resources.local_in_lif) == BCM_E_EXISTS) {
                       if((outlif_deallocation_type == _BCM_DPP_AM_OUT_AC_TYPE_PON_3_TAGS_DATA) 
                        ||(outlif_deallocation_type == _BCM_DPP_AM_OUT_AC_TYPE_BIG_OUT_AC)){
                           rv = bcm_dpp_am_l2_vpn_ac_dealloc(unit, outlif_deallocation_type, 0,gport_hw_resources.local_in_lif);
                           BCMDNX_IF_ERR_EXIT(rv);
                       }
                       else{
            ...
            if (is_3_tags_data) {
                ...
                if (BCM_E_EXISTS == bcm_dpp_am_out_ac_is_alloced(unit, eedb_id_3_tags)) {
                    ...
                    rv = bcm_dpp_am_out_ac_dealloc(unit, _BCM_DPP_AM_OUT_AC_TYPE_PON_3_TAGS_OUT_AC, 0, eedb_id_3_tags);
                    BCMDNX_IF_ERR_EXIT(rv);
                }
               /* 3 tags port can't be remote, so we don't worry about modifying HW here. */
               if (!(SOC_IS_ARAD_A0(unit)&&(eedb_id_3_tags==0))) {
        ...
        /* Remove in lif match */
        if (is_ingress) {          
            rv = _bcm_dpp_lif_sw_resources_delete(unit, gport_hw_resources.local_in_lif, -1, _BCM_DPP_GPORT_SW_RESOURCES_INGRESS);
            BCMDNX_IF_ERR_EXIT(rv); 
        }

        /* Remove out lif match. */
        if (is_egress) {
            rv = _bcm_dpp_lif_sw_resources_delete(unit, -1, gport_hw_resources.local_out_lif, _BCM_DPP_GPORT_SW_RESOURCES_EGRESS);
            BCMDNX_IF_ERR_EXIT(rv); 
    ...
    bcm_vlan_port_t *vlan_port)
{
    ...
    if (BCM_GPORT_IS_VLAN_PORT(vlan_port->vlan_port_id) ) {
        ...
        rv = _bcm_dpp_gport_l2_intf_gport_has_remote_lif(unit,vlan_port->vlan_port_id,&remote_lif);
        BCMDNX_IF_ERR_EXIT(rv);
        ...
        if(!remote_lif) {
            ...
            if ((is_egress) && (is_local)) {
                ...
                if (SOC_DPP_CONFIG(unit)->pp.egress_vlan_filtering) {
                ...
                if (SOC_IS_ARAD(unit) && SOC_DPP_CONFIG(unit)->pp.pon_application_enable) {
                    ...
                    /* get egress info */
                    if (!(SOC_DPP_CONFIG(unit)->pp.egress_vlan_filtering && (vlan_port->flags & BCM_VLAN_PORT_EXTENDED))) {/* not necessary in case this is a data-entry*/
    ...
    if (!vlan_port->failover_id && !vlan_port->ingress_failover_id  /* These three fields indicate that protection info was */
        ...
        rv = _bcm_dpp_lif_sw_resources_learn_gport_get(unit, in_lif_id, &gport_val);
        BCMDNX_IF_ERR_EXIT(rv);
...
int _bcm_dpp_vlan_port_create(int unit, bcm_vlan_port_t *vlan_port)
{
    ...
    if (encap_with_id) {
        if(!BCM_ENCAP_REMOTE_GET(vlan_port->encap_id)) {
            ...
            if (rv != BCM_E_NOT_FOUND) {
                BCMDNX_IF_ERR_EXIT(rv);
                if (is_ingress) {
                    rv = _bcm_dpp_lif_sw_resources_lif_usage_get(unit, local_lif_id, _BCM_GPORT_ENCAP_ID_LIF_INVALID, &used_type, NULL);
                } else {
                    rv = _bcm_dpp_lif_sw_resources_lif_usage_get(unit, _BCM_GPORT_ENCAP_ID_LIF_INVALID, local_lif_id, NULL, &used_type);
                }
            ...
            if (SOC_IS_JERICHO(unit)) {
                ...
                if (is_ingress) {
                    ...
                    if (counting_profile_is_allocated) {
                        if (vlan_port->inlif_counting_profile != BCM_STAT_LIF_COUNTING_PROFILE_NONE && !BCM_DPP_COUNTER_LIF_PROFILE_IS_INGRESS(vlan_port->inlif_counting_profile)) {
                            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("InLIF-counting-profile %d is out of range."), vlan_port->inlif_counting_profile));
                        }
                        local_inlif_info.counting_profile_id = vlan_port->inlif_counting_profile;
                    } else {
                 ...
                 */  
                if (counting_profile_is_allocated) {
                    if (vlan_port->outlif_counting_profile != BCM_STAT_LIF_COUNTING_PROFILE_NONE && BCM_DPP_COUNTER_LIF_PROFILE_IS_INGRESS(vlan_port->outlif_counting_profile)) {
                        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("OutLIF-counting-profile %d is out of range."), vlan_port->outlif_counting_profile));
                    }
                    local_out_lif_info.counting_profile_id = vlan_port->outlif_counting_profile;
                } else {
    ...
    if (is_egress) {
        ...
        BCMDNX_IF_ERR_EXIT(_bcm_dpp_lif_sw_resources_lif_usage_set(unit, 0, active_local_out_lif_id, 0, _bcmDppLifTypeVlan));
    }
    ...
    if (_BCM_DPP_GPORT_INFO_IS_LOCAL_PORT(gport_info) && !remote_lif) {
            ...
            if(update) {
                ...
                if(is_tunnel_matched){/*PON:don't need to update edit_profile when flag is update*/
                    bsl_printf("CREATE:save orginal edit info\n");
            ...
            if ((vlan_port->flags & BCM_VLAN_PORT_EGRESS_PON_TUNNEL_NONE) && (SOC_DPP_CONFIG(unit)->pp.test2)) {
                in_ac_key.key_type = SOC_PPD_L2_LIF_AC_MAP_KEY_TYPE_TEST2;
            }
            ...
            if(vlan_port->vsi == BCM_VLAN_NONE || vlan_port->flags & BCM_VLAN_PORT_VSI_BASE_VID) {
                ...
                if(!update) {
                    ...
                    if (is_ingress) {
                        ...
                        _bcm_dpp_lif_sw_resources_learn_gport_set(unit, local_in_lif_id, learn_gport);
                    }
                }                
                if (vlan_port->flags & BCM_VLAN_PORT_VSI_BASE_VID) {                  
            ...
            if (is_ingress) {
                ...
                if(update) {
                    ...
                    if(in_lif_id_o != local_in_lif_id) {
                        rv = _bcm_dpp_lif_sw_resources_delete(unit, in_lif_id_o, -1, _BCM_DPP_GPORT_SW_RESOURCES_INGRESS);
                        BCMDNX_IF_ERR_EXIT(rv);
                    }
                }
            ...
            if (is_egress && !skip_egress_alloc) {
                ...
                if (SOC_DPP_IS_VLAN_TRANSLATE_MODE_NORMAL(unit)) {
                    ...
                    if ((!(vlan_port->flags & BCM_VLAN_PORT_REPLACE))&&is_tunnel_matched && (!is_3_tags_manipulated)) { 
                        uint16 pon_tunnel_tpid, outer_tpid;
                        bcm_vlan_action_set_t action;
                        bcm_vlan_action_set_t_init(&action);
