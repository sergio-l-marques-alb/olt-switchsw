...
_bcm_dpp_in_lif_tunnel_term_add(int unit, bcm_tunnel_terminator_t *tunnel_term, int lif)
{
    ...
    rv = _bcm_dpp_lif_sw_resources_set(unit, lif, -1, _BCM_DPP_GPORT_SW_RESOURCES_INGRESS, &gport_sw_resources);
    BCMDNX_IF_ERR_EXIT(rv); 
...
_bcm_dpp_in_lif_tunnel_term_get(int unit, bcm_tunnel_terminator_t *tunnel_term, int lif)
{
    ...
    rv = _bcm_dpp_lif_to_sw_resources(unit, lif, -1, _BCM_DPP_GPORT_SW_RESOURCES_INGRESS, &gport_sw_resources);
    BCMDNX_IF_ERR_EXIT(rv);
   ...
   )
{
    ...
    if (replace) {
        ...
        BCMDNX_IF_ERR_EXIT( _bcm_dpp_lif_sw_resources_lif_usage_get(unit, -1, local_lif, NULL, &usage) );
    ...
    bcm_tunnel_initiator_t *tunnel)
{
    ...
    if (SOC_IS_JERICHO(unit)) {
            ...
            rv = _bcm_dpp_gport_alloc_global_and_local_lif(unit, flags, &global_outlif, NULL, &local_out_lif_info, TRUE);
    ...
    int *tunnel_eep)
{
    ...
    if (SOC_IS_PETRAB(unit)) {
        ...
        if(BCM_L3_ITF_TYPE_IS_LIF(intf->l3a_intf_id)) {
            vsi = vsi_param;
            ll_eep = ll_eep_param;
        }
    ...
    if (SOC_IS_ARAD(unit)) {
        if (update) {
            ...
            BCMDNX_IF_ERR_EXIT( _bcm_dpp_lif_sw_resources_lif_usage_get(unit, -1, local_out_lif, NULL, &usage) );
            if (usage != _bcmDppLifTypeIpTunnel) {
                BCMDNX_ERR_EXIT_MSG(BCM_E_NOT_FOUND, (_BSL_BCM_MSG("Can't replace OutLIF to IP Tunnel, because it's a different type (%d)"),usage));
            }
    ...
    if(!eep_alloced) {/* need to allocate IP tunnel */
        ...
        if (SOC_IS_JERICHO(unit)) {
            ...
            rv = _bcm_dpp_gport_alloc_global_and_local_lif(unit, flags, &global_lif_id, NULL, &outlif_info, TRUE);
            BCM_IF_ERROR_RETURN(rv);
    ...
    if (SOC_IS_PETRAB(unit)) {
        ...
        if(BCM_L3_ITF_TYPE_IS_FEC(intf->l3a_intf_id) && !is_tunneled) {
    ...
    bcm_l3_intf_t *intf)
{
    ...
    BCMDNX_IF_ERR_EXIT(_bcm_dpp_lif_sw_resources_lif_usage_get(unit, -1, local_out_lif, NULL, &lif_usage));
    ...
    if(lif_usage == _bcmDppLifTypeCpuL2Encap) {
        ...
        rv = _bcm_dpp_lif_sw_resources_delete(unit, -1, local_out_lif, _BCM_DPP_GPORT_SW_RESOURCES_EGRESS);
        BCMDNX_IF_ERR_EXIT(rv); 
    ...
    bcm_tunnel_initiator_t *tunnel)
{
    ...
    if(SOC_IS_PETRAB(unit)) {
        ...
        if (nof_entries > 1) {
            ...
            BCMDNX_IF_ERR_EXIT(_bcm_dpp_lif_sw_resources_lif_usage_get(unit, -1,  next_eep[0], NULL, &usage));
 ...
 ) 
{
    ...
    /* vrf is used as termination key only in DIP+SIP+VRF mode */
    if (SOC_IS_JERICHO(unit) 
    ...
    bcm_tunnel_terminator_t *info) 
{
    ...
    if (!update) {
        int temp_local_lif; 
        SOC_PPD_RIF_IP_TERM_INFO temp_term_info;
        SOC_PPD_RIF_INFO temp_rif_info;
        uint8 found; 

        SOC_PPD_RIF_IP_TERM_INFO_clear(&temp_term_info); 
        SOC_PPD_RIF_INFO_clear(&temp_rif_info); 

        soc_sand_rv = soc_ppd_rif_ip_tunnel_term_get(soc_sand_dev_id, &term_key, (SOC_PPD_LIF_ID*) &temp_local_lif,
    ...
    if (update) {
        ...
        if (SOC_IS_JERICHO(unit)) {
            ...
            /* Allocate the global and local lif */
            rv = _bcm_dpp_gport_alloc_global_and_local_lif(unit, alloc_flags, &global_lif, &local_inlif_info, NULL, FALSE);
            BCM_IF_ERROR_RETURN(rv);
    ...
    bcm_tunnel_terminator_t *info)
{
    ...
    if (!(term_key.flags & SOC_PPD_RIF_IP_TERM_FLAG_KEY_ONLY)) {
        ...
        rv = _bcm_dpp_lif_sw_resources_delete(unit, local_inlif, -1, _BCM_DPP_GPORT_SW_RESOURCES_INGRESS);
        BCMDNX_IF_ERR_EXIT(rv); 
    }
