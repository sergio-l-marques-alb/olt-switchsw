                            ...
                            bcm_port_control_t type, int *value) 
{
    ...
    case bcmPortControlReserved280:
        if (SOC_DPP_CONFIG(unit)->pp.egress_full_mymac_1) {
            ...
            soc_sand_rv = (MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_ports_tm_port_var_get, (unit, soc_ppd_port, value)));
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        } else {
            BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("Invalid control")));
        }
          ...
          pbmp_t *detached)
{
    ...
    pbmp_t counter_pbmp; 
    int counter_paused, linkscan_paused;
    ...
    if (counter_paused)
    {
        soc_counter_start(unit, counter_flags, counter_interval, PBMP_SFI_ALL(unit));
    }
exit:
    BCMDNX_FUNC_RETURN;
}
...
int bcm_petra_port_add(int unit, bcm_port_t port, uint32 flags, bcm_port_interface_info_t *interface_info, bcm_port_mapping_info_t *mapping_info)  
{
    ...
    if (SOC_IS_JERICHO(unit)) {
        SOCDNX_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit,
                mbcm_dpp_qsgmii_offsets_add, (
                    unit,
                    interface_info->phy_port,
                    &(interface_info->phy_port))));
    }
    ...
    int *enabled)
{
    ...
    BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_i) {
        ...
        *enabled = 1;
        ...
        /* map egr_port to forwardig decision */
        rv = _bcm_dpp_gport_and_encap_from_fwd_decision(unit,egr_port,NULL,&(trap.dest_info.frwrd_dest), -1/*encap_usage*/);
        BCMDNX_IF_ERR_EXIT(rv);
...
{
    BCMDNX_INIT_FUNC_DEFS;
    BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("bcm_petra_port_nif_priority_set not supported")));
exit:
    BCMDNX_FUNC_RETURN;
}
...
{
    BCMDNX_INIT_FUNC_DEFS;
    BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("bcm_petra_port_nif_priority_get not supported")));
exit:
    BCMDNX_FUNC_RETURN;
}
