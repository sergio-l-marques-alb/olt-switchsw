        ...
        if(BCM_FAILURE(result)) {
            ...
            if(BCM_FAILURE(result)) {
                ...
                if(BCM_FAILURE(result)) {
                    LOG_VERBOSE(BSL_LS_BCM_MIM,
                                (BSL_META_U(unit,
                                            "WARNING %s ETE L2  allocation Failed !!!!(%s,%d)\n"),
                                 FUNCTION_NAME(),__FILE__,__LINE__));
                    status = BCM_E_FULL;
                } else {
    ...
    if(!portcb) {
        ...
        if(portcb->hwinfo.eteencap){
            ...
            if(BCM_FAILURE(status)) {
                LOG_ERROR(BSL_LS_BCM_MIM,
                          (BSL_META_U(unit,
                                      "error(%s) could not free Encap ETE[%d]\n"),
                                          ...
                                          bcm_fe2k_mim_port_control_t *portcb)
{
    ...
    if(portcb && _MIM_PORT_TYPE_VALID_CHECK(portcb->type)) {
        ...
        if(BCM_GPORT_IS_TRUNK(portcb->port)) {
            ...
            status = bcm_fe2000_trunk_get_old(unit, 
                                     ...
                                     bcm_fe2k_mim_port_control_t **ppPortCtl)
{
    ...
    if (portcb->hwinfo.lport >= SBX_LPORT_BASE) {
...
int _bcm_fe2000_dump_portcb(bcm_fe2k_mim_port_control_t *portcb)
{
    if(portcb) {
        ...
        LOG_VERBOSE(BSL_LS_BCM_MIM,
                    (BSL_META("FTIDX:[0x%x] OI:[0x%x] ETE-Encap[0x%x] ETE-L2[0x%x] \n"),
                              ...
                              void *travdata)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_MIM,
              (BSL_META("User callback returned: %d\n"), rv));
                ...
                if(BCM_SUCCESS(status)) {
                    ...
                    if (portcb) {
                        ...
                        if(BCM_SUCCESS(status)) {
                            ...
                            if (portid >= 0 && portid < SBX_MAX_GPORTS) {
...
{
    bcm_fe2k_mim_vpn_control_t *vpncb;
    bcm_fe2k_mim_port_control_t *tmpportcb = NULL;
    dq_p_t port_elem;
    int status = BCM_E_NONE;
    ...
    if(BCM_GPORT_MIM_PORT != SBX_LPORT_TYPE(unit,lport)) {
        ...
        if (portcb->type == _BCM_FE2K_MIM_DEF_BACKBONE_PORT) {
            ...
            if (isid2e.lpi == 0) {
                LOG_ERROR(BSL_LS_BCM_MIM,
                          (BSL_META_U(unit,
                                      "unexpected physical port for logicial port on isid 0x%06x\n"),
                           portcb->isid));
                status = BCM_E_INTERNAL;
            }
