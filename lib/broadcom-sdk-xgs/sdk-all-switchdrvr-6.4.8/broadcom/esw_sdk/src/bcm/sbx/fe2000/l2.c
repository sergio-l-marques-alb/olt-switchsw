...
if (SOC_IS_SBX_G2P3((u))) {        \
   LOG_WARN(BSL_LS_BCM_COMMON, \
            (BSL_META_U(u, \
                        "WARNING %s has not been ported for G2P3 (%s:%d)\n"), \
...
bcm_fe2000_l2_detach(int unit)
{
    ...
    if (BCM_FAILURE(rv = _bcm_fe2000_l2_egress_detach(unit))) {
        LOG_ERROR(BSL_LS_BCM_L2,
                  (BSL_META_U(unit,
                              "_bcm_fe2000_l2_egress_detach failed rv=%d(%s)"),
                   rv, bcm_errmsg(rv)));
    }
    ...
    if (BCM_FAILURE(rv = bcm_fe2000_l2_cache_detach(unit))) {
        LOG_ERROR(BSL_LS_BCM_L2,
                  (BSL_META_U(unit,
                              "bcm_fe2000_l2_cache_detach failed rv=%d(%s)"),
                   rv,
                   bcm_errmsg(rv)));
    }
#endif
...
bcm_fe2000_l2_init(int unit)
{
    ...
    if (BCM_FAILURE(rv = _bcm_fe2000_l2_egress_init(unit))) {
        LOG_ERROR(BSL_LS_BCM_L2,
                  (BSL_META_U(unit,
                              "_bcm_fe2000_l2_egress_init failed rv=%d(%s)"),
                   rv, bcm_errmsg(rv)));
    }
    ...
    if (BCM_FAILURE(rv = bcm_fe2000_l2_cache_init(unit))) {
        LOG_ERROR(BSL_LS_BCM_L2,
                  (BSL_META_U(unit,
                              "bcm_fe2000_l2_cache_init failed rv=%d(%s)"),
                   rv, bcm_errmsg(rv)));
    }
...
_bcm_fe2000_l2_age_timer_thread(int unit)
{
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_L2,
                  (BSL_META_U(unit,
                              "L2 age timer thread, unable to get device "
                               "handler: %d %s\n"), rv, bcm_errmsg(rv)));
        return;
    }
    ...
    sal_thread_name(_l2_age[unit].thread_id, thread_name, sizeof (thread_name));
    /* Main thread loop */
...
_bcm_fe2000_l2_age_timer_enable(int unit, int age_seconds)
{
    ...
    if (COMPILER_64_HI(uuSec) == 0) {
        ...
        if (soc_sbx_div64(uuSec, cycles, &res) == -1) {
            return BCM_E_INTERNAL;
        }
    }
    COMPILER_64_SET(_l2_age[unit].cycle_usec, 0, res);
                             ...
                             bcm_l2_stat_t stat, uint32 val)
{
    ...
    return bcm_fe2000_l2_stat_set(unit, l2_addr, stat, uuVal);
}
...
int
_bcm_fe2000_l2_egress_init(int unit)
{
    int             rv = BCM_E_NONE;
    int             l2_egr_num;
#ifdef BCM_WARM_BOOT_SUPPORT
    ...
    if ( (_l2_egress[unit].init_done) && (!SOC_WARM_BOOT(unit)) ){
        ...
        if (size > 0) {
            ...
            LOG_DEBUG(BSL_LS_BCM_L2,
                      (BSL_META("L2: allocating 0x%x (%d) bytes of scache:"),
    ...
    if (BCM_SUCCESS(rv)) {
        ...
        if (!BCM_FAILURE(rv)) {
            if (_l2_egress[unit].wb_cache) {
                ...
                LOG_VERBOSE(BSL_LS_BCM_L2,
                            (BSL_META("Obtained scache pointer=0x%08x, %d bytes, "
                             "version=%d.%d\n"),
                ...
                if (_l2_egress[unit].wb_cache->version > L2_WB_CURRENT_VERSION) {
                    LOG_ERROR(BSL_LS_BCM_L2,
                              (BSL_META("Upgrade scenario not supported.  "
                    ...
                    LOG_ERROR(BSL_LS_BCM_L2,
                              (BSL_META("Downgrade scenario not supported.  "
                               "Current version=%d.%d  found %d.%d\n"),
                               SOC_SCACHE_VERSION_MAJOR(L2_WB_CURRENT_VERSION),
                               SOC_SCACHE_VERSION_MINOR(L2_WB_CURRENT_VERSION),
                               SOC_SCACHE_VERSION_MAJOR(_l2_egress[unit].wb_cache->version),
