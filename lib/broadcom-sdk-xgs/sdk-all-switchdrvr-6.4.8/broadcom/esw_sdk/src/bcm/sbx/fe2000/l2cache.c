/*
 * $Id: l2cache.c,v 1.57 Broadcom SDK $
...
#define L2CACHE_WB_HDL(unit) _l2_cache[unit].wb_hdl
#define L2CACHE_SCACHE_SIZE(unit) _l2_cache[unit].scache_size
#else
#define L2CACHE_WB_HDL(unit) 0
#define L2CACHE_SCACHE_SIZE(unit) 0
#endif

typedef struct fe2k_l2cache_mac_entry_s {
    uint16 vlan;
    bcm_mac_t mac;
}fe2k_l2cache_mac_entry_t;
...
typedef struct fe2k_l2cache_wb_mem_layout_s {
    uint16 flags[L2CACHE_MAX_IDX_DEFAULT];
    fe2k_l2cache_mac_entry_t mac_entry[L2CACHE_WB_MAX_MAC_ENTRIES];
}fe2k_l2cache_wb_mem_layout_t;
...
#define L2CACHE_WB_L2_CONFIGURED(unit) (L2CACHE_SCACHE_SIZE(unit) != 0)
...
#ifdef BCM_WARM_BOOT_SUPPORT
int
_bcm_l2cache_wb_layout_get(int unit, fe2k_l2cache_wb_mem_layout_t **layout)
{
    int rv;
    uint32 size;
    soc_wb_cache_t *wbc;
    ...
    *layout = NULL;
    rv = soc_scache_ptr_get(unit, L2CACHE_WB_HDL(unit), 
                            (uint8**)&wbc, &size);
    if (BCM_FAILURE(rv)) {
        return rv; 
    }
    *layout = (fe2k_l2cache_wb_mem_layout_t *)wbc->cache;
    return rv;
}
#endif /* BCM_WARM_BOOT_SUPPORT */
                               ...
                               int *index_used, int verify)
{
    ...
    for (l2cpmac_idx=0; l2cpmac_idx < max_l2cpmac_idx; l2cpmac_idx++) {
        ...
        rv = soc_sbx_g2p3_l2cpmac_set(unit, l2cpmac_idx, &l2cpmac);
        if (rv != SOC_E_NONE) {
            return rv;
        }
        entry = L2CACHE_L2CPMAC_TABLE_ENTRY(unit, l2cpmac_idx);
...
int
_bcm_fe2000_g2p3_l2cpmac_free(int unit, bcm_mac_t mac)
{
    int                     rv = BCM_E_NONE, l2cpmac_idx;
    int                     max_l2cpmac_idx = 0;
    soc_sbx_g2p3_l2cpmac_t  l2cpmac;
...
#define _G2P3_L2CACHE_L2CP_FTE  1
#define _G2P3_L2CACHE_GPORT_FTE 2

int
_bcm_fe2000_g2p3_fte_allocate(int unit, bcm_l2_cache_addr_t *addr,
                              uint32 *pfte, int fte_type)
{
    ...
    if (fte_type == _G2P3_L2CACHE_L2CP_FTE) {
        rv = _sbx_gu2_resource_alloc(unit, SBX_GU2K_USR_RES_FTE_UNICAST, 
                                     1, &fte_idx, 0);
#if defined(BROADCOM_DEBUG)
        fteTypeStr = "UNICAST";
#endif
    } else if (fte_type == _G2P3_L2CACHE_GPORT_FTE) {
    ...
    */
    ...
    rv = soc_sbx_g2p3_ft_set(unit, fte_idx, &fte);
    ...
    if (rv != BCM_E_NONE) {
        ...
        if (fte_type == _G2P3_L2CACHE_L2CP_FTE) {
            _sbx_gu2_resource_free(unit, SBX_GU2K_USR_RES_FTE_UNICAST, 1, 
                                   &fte_idx, 0);
        } else if (fte_type == _G2P3_L2CACHE_GPORT_FTE) {
            _sbx_gu2_resource_free(unit, SBX_GU2K_USR_RES_FTE_GLOBAL_GPORT, 1,
                                   &fte_idx, 0);
        } 
    } else {
        *pfte = fte_idx;
...
int
_bcm_fe2000_g2p3_fte_free(int unit, uint32 fte_idx, int fte_type)
{
    int                 rv = BCM_E_NONE;
    soc_sbx_g2p3_ft_t   fte;
    
    soc_sbx_g2p3_ft_t_init(&fte);
    BCM_IF_ERROR_RETURN(soc_sbx_g2p3_ft_set(unit, fte_idx, &fte));
    if (fte_type == _G2P3_L2CACHE_L2CP_FTE) {
        rv = _sbx_gu2_resource_free(unit, SBX_GU2K_USR_RES_FTE_UNICAST, 
                                    1, &fte_idx, 0);
    } else if (fte_type == _G2P3_L2CACHE_GPORT_FTE) {
        rv = _sbx_gu2_resource_free(unit, SBX_GU2K_USR_RES_FTE_GLOBAL_GPORT, 1,
                                   &fte_idx, 0);
    }
    return rv;
}
...
#ifdef BCM_WARM_BOOT_SUPPORT
int
_bcm_fe2000_l2cache_entry_store(int unit, int index, int subtype) {
    _l2_cache_entry              *entry;
    fe2k_l2cache_wb_mem_layout_t *layout;
    int                          rv = BCM_E_NONE;
    ...
    soc_scache_handle_lock(unit, L2CACHE_WB_HDL(unit));
    ...
    soc_scache_handle_unlock(unit, L2CACHE_WB_HDL(unit));
...
int
_bcm_fe2000_l2cache_entry_recover(int unit, int index,
                                  fe2k_l2cache_wb_mem_layout_t *layout)
{
    _l2_cache_entry              *entry;
    int                         rv = BCM_E_NONE;
    soc_sbx_g2p3_l2cpslow_t     l2cpslow;
    soc_sbx_g2p3_l2cp_t         l2cp;
    soc_sbx_g2p3_ft_t           fte;
    int                         port;
    int                         subtype=0;
    int                         dmac;
    ...
    if (!(entry->flags & L2CACHE_ENTRY_DMAC) ){
        if (entry->flags & L2CACHE_ENTRY_SUBTYPE) {
            ...
            rv = soc_sbx_g2p3_l2cpslow_get(unit, subtype, port, &l2cpslow);
            if (rv != BCM_E_NONE) {
                LOG_ERROR(BSL_LS_BCM_L2,
                          (BSL_META_U(unit,
            ...
            subtype=0xff;
            rv = soc_sbx_g2p3_l2cp_get(unit, dmac, port, &l2cp);
            if (rv != BCM_E_NONE) {
                LOG_ERROR(BSL_LS_BCM_L2,
                          (BSL_META_U(unit,
        ...
        if (entry->flags & L2CACHE_ENTRY_DMAC) {
            ...
            if ((entry->flags & L2CACHE_ENTRY_PREFIX5_LOOKUP) & !(rsvd_mac)) {
                ...
                rv = _bcm_fe2000_g2p3_l2cpmac_alloc(unit, layout->mac_entry[mac_index].mac, &l2cpmac_idx, 1);
                if (rv != BCM_E_NONE){
                    LOG_ERROR(BSL_LS_BCM_L2,
                              (BSL_META_U(unit,
            ...
            rv = soc_sbx_g2p3_cmac_get(unit, layout->mac_entry[mac_index].mac, (pL2Key->ulVlan & 0xfff),
                                      &sbx_mac_data);
            if (rv != BCM_E_NONE) {
                LOG_ERROR(BSL_LS_BCM_L2,
                          (BSL_META_U(unit,
            ...
            entry->fte_idx = sbx_mac_data.pid;
        }else{
            return BCM_E_INTERNAL;
        }
    ...
    if ( entry->flags & L2CACHE_ENTRY_FTE_VALID) {
        soc_sbx_g2p3_ft_t_init(&fte);
        rv = soc_sbx_g2p3_ft_get(unit, entry->fte_idx, &fte);
        if (rv != SOC_E_NONE) {
            LOG_ERROR(BSL_LS_BCM_L2,
                      (BSL_META_U(unit,
        ...
        if ( !(entry->flags & L2CACHE_ENTRY_FTE_MPLS_GPORT) ) {
            ...
            if ( (entry->fte_idx >= SBX_GLOBAL_GPORT_FTE_BASE(unit)) &&
                ...
                rv = _sbx_gu2_resource_alloc(unit,
                                             SBX_GU2K_USR_RES_FTE_GLOBAL_GPORT,
                                             1, &entry->fte_idx, _SBX_GU2_RES_FLAGS_RESERVE);
                if (rv != BCM_E_NONE) {
                    LOG_ERROR(BSL_LS_BCM_L2,
                              (BSL_META_U(unit,
...
int
_bcm_fe2000_g2p3_l2_cache_init_recover(int unit, uint32 *pass_thru_ohi, 
                                       ...
                                       fe2k_l2cache_wb_mem_layout_t *layout)
{
    ...
    uint32                      encap_ete_idx;
    uint32                      ete_idx;
    int                         flags = _SBX_GU2_RES_FLAGS_RESERVE;
    int                         idx = 0;
    ...
    rv = soc_sbx_g2p3_max_l2cp_types_get(unit, &max_l2cp_types);
    if (BCM_FAILURE(rv)) {
        return rv;
    }

    rv = soc_sbx_g2p3_max_l2cp_subtypes_get(unit, &max_l2cp_subtypes);
    if (BCM_FAILURE(rv)) {
        return rv;
    }

    rv = soc_sbx_g2p3_l2cpmac_table_size_get(unit, &max_idx);
    if (rv == BCM_E_NONE) {
    ...
    soc_sbx_g2p3_l2cpslow_t_init(&l2cpslow);
    rv = soc_sbx_g2p3_l2cpslow_get(unit, L2CACHE_WB_MAX_VALID_L2CP_SUBTYPE, 0, &l2cpslow);
    if (rv != SOC_E_NONE) {
        LOG_ERROR(BSL_LS_BCM_L2,
                  (BSL_META_U(unit,
    ...
    *cpu_fte = l2cpslow.ftidx;
    ...
    rv = soc_sbx_g2p3_l2cpslow_get(unit, L2CACHE_WB_MAX_VALID_L2CP_SUBTYPE + 1, 0, &l2cpslow);
    if (rv != SOC_E_NONE) {
        LOG_ERROR(BSL_LS_BCM_L2,
                  (BSL_META_U(unit,
                              "%s: Error getting L2CP SLOW entry (port:%d "
    ...
    *pass_thru_ohi = l2cpslow.ftidx;
    rv = _sbx_gu2_resource_alloc(unit, SBX_GU2K_USR_RES_OHI, 
    ...
    rv = soc_sbx_g2p3_oi2e_get(unit, *pass_thru_ohi - SBX_RAW_OHI_BASE, &sbx_ohi);
    if (rv != SOC_E_NONE) {
        LOG_ERROR(BSL_LS_BCM_L2,
                  (BSL_META_U(unit,
    ...
    rv = _sbx_gu2_resource_alloc(unit, SBX_GU2K_USR_RES_ETE_ENCAP, 
                                 1, &encap_ete_idx, flags);
    if (rv != BCM_E_NONE) {
        LOG_ERROR(BSL_LS_BCM_L2,
                  (BSL_META_U(unit,
                              "%s: Error re-allocating ete encap 0x%x\n"),
                   FUNCTION_NAME(), encap_ete_idx));
        return rv;
    }
    ...
    if (rv != SOC_E_NONE) {
        LOG_ERROR(BSL_LS_BCM_L2,
                  (BSL_META_U(unit,
                              "%s: Error getting eteencap entry:%d)\n"),
                   FUNCTION_NAME(), encap_ete_idx));
        return rv;
    }
    ...
    if (rv != BCM_E_NONE) {
        LOG_ERROR(BSL_LS_BCM_L2,
                  (BSL_META_U(unit,
                              "%s: Error re-allocating ete l2 0x%x\n"),
    ...
    for (idx = 0; idx < L2CACHE_MAX_IDX; idx++) {
        if (layout->flags[idx] & L2CACHE_ENTRY_VALID) {
            _bcm_fe2000_l2cache_entry_recover(unit, idx, layout);
        }else{
            continue;
        }
    }
...
#endif
int
_bcm_fe2000_g2p3_l2cp_table_set(int unit, uint32 fte_idx, uint8 dmac_lsb, 
                                int subtype_in, int idx)
{
    ...
    for (port = port_min; port < port_max; port++) {
        for (dmac = dmac_min; dmac < dmac_max; dmac++) {
            ...
            rv = soc_sbx_g2p3_l2cp_set(unit, dmac, port, &l2cp);
            if (rv != SOC_E_NONE) {
                LOG_ERROR(BSL_LS_BCM_L2,
                          (BSL_META_U(unit,
        ...
        for (subtype = st_min; subtype < st_max; subtype++) {
            rv = soc_sbx_g2p3_l2cpslow_set(unit, subtype, port, &l2cpslow);
            if (rv != SOC_E_NONE) {
                LOG_ERROR(BSL_LS_BCM_L2,
                          (BSL_META_U(unit,
...
int
_bcm_fe2000_g2p3_l2_cache_hw_entry_delete(int unit, int idx)
{
    int                         rv = BCM_E_NONE;
    ...
    if (flags & L2CACHE_ENTRY_FTE_VALID) {
        ...
        if (flags & L2CACHE_ENTRY_FTE_MPLS_GPORT) {
            ...
            if (flags & L2CACHE_ENTRY_L2CP) {
                rv = _bcm_fe2000_g2p3_fte_free(unit, entry->fte_idx, 
                                               _G2P3_L2CACHE_L2CP_FTE);
            } else {
                rv = _bcm_fe2000_g2p3_fte_free(unit, entry->fte_idx,
                                               _G2P3_L2CACHE_GPORT_FTE);
            }
            if (rv != BCM_E_NONE) {
                return rv;
            }
    ...
    if ((entry->src_port == BCM_L2_SRCPORT_MASK_ALL) || 
        ...
        rv = _bcm_fe2000_g2p3_l2cp_table_set(unit, 0, dmac_lsb, subtype, idx);
        if (rv == BCM_E_NONE) {
            sal_memset(entry, 0x0, sizeof(_l2_cache_entry));
        }
#ifdef BCM_WARM_BOOT_SUPPORT
        /* update cache */
        rv = _bcm_fe2000_l2cache_entry_store(unit, idx, 0);
#endif
    ...
    if (flags & L2CACHE_ENTRY_L2CP) {
        ...
        if (subtype < 0) {
            ...
            BCM_IF_ERROR_RETURN(soc_sbx_g2p3_l2cp_set(unit, dmac, port, &l2cp));
        } else {
            soc_sbx_g2p3_l2cpslow_t_init(&l2cpslow);
            l2cpslow.forward = 1;
            l2cpslow.ftidx = L2CACHE_CPU_FTE(unit);
            l2cpslow.passstp = 1; /* slow protocol - ingore stp */
            BCM_IF_ERROR_RETURN(soc_sbx_g2p3_l2cpslow_set(unit, subtype, port, 
                                                          &l2cpslow));
        }
        if (rv == BCM_E_NONE) {
            sal_memset(entry, 0x0, sizeof(_l2_cache_entry));
    ...
    if (flags & L2CACHE_ENTRY_DMAC) {
        ...
        if ((flags & L2CACHE_ENTRY_PREFIX5_LOOKUP) && !rsvd_mac) {
            ...
            rv = _bcm_fe2000_g2p3_l2cpmac_free(unit, mac);
            if (rv != BCM_E_NONE) {
                return rv;
            }
        }
        rv = soc_sbx_g2p3_cmac_remove(unit, mac, pL2Key->ulVlan);
        if (rv == BCM_E_NONE) {
...
int
_bcm_fe2000_g2p3_l2_cache_hw_entry_config_get(int unit, int idx,
                                              bcm_l2_cache_addr_t *addr)
{
    ...
    if (flags & L2CACHE_ENTRY_FTE_VALID) {
        ...
        if (flags & L2CACHE_ENTRY_FTE_MPLS_GPORT) {
            ...
            soc_sbx_g2p3_ft_t_init(&fte);
            BCM_IF_ERROR_RETURN(soc_sbx_g2p3_ft_get(unit, fte_idx, &fte));
            addr->group = fte.oi;
        } else {
            soc_sbx_g2p3_ft_t_init(&fte);
            BCM_IF_ERROR_RETURN(soc_sbx_g2p3_ft_get(unit, fte_idx, &fte));
...
int
_bcm_fe2000_g2p3_l2_cache_hw_entry_set(int unit, bcm_l2_cache_addr_t *addr, 
                                       int idx)
{
    int                         rv = BCM_E_NONE;
    ...
    if (rsvd_dmac && !(entry->flags & L2CACHE_ENTRY_DMAC)) {
        if (entry->flags & L2CACHE_ENTRY_FTE_VALID) {
            if (entry->flags & L2CACHE_ENTRY_FTE_MPLS_GPORT) {
                ...
                rv = _bcm_fe2000_g2p3_fte_allocate(unit, addr, &fte_idx,
                                                   _G2P3_L2CACHE_L2CP_FTE);
                if (rv != BCM_E_NONE) {
                    return rv;
                }
        ...
        rv = _bcm_fe2000_g2p3_l2cp_table_set(unit, m_ftidx, addr->mac[5], 
                                             subtype, idx);
        if (rv != BCM_E_NONE) {
            if (fte_idx) {
                _bcm_fe2000_g2p3_fte_free(unit, fte_idx, 
                                          _G2P3_L2CACHE_GPORT_FTE);
            }
        }else{
#ifdef BCM_WARM_BOOT_SUPPORT
            rv = _bcm_fe2000_l2cache_entry_store(unit, idx, subtype);
            if (rv != BCM_E_NONE) {
                LOG_ERROR(BSL_LS_BCM_L2,
                          (BSL_META_U(unit,
        ...
        if (entry->flags & L2CACHE_ENTRY_PREFIX5_LOOKUP) {
            ...
            if (rsvd_dmac) {
                ...
                rv = _bcm_fe2000_g2p3_l2cpmac_alloc(unit, addr->mac, &l2cpmac_idx, 0);
                if (rv != BCM_E_NONE) {
                    sal_free(pL2Key);
                    return rv;
                }
        ...
        if (entry->flags & L2CACHE_ENTRY_FTE_VALID) {
            if (entry->flags & L2CACHE_ENTRY_FTE_MPLS_GPORT) {
                /* for MPLS gport, fte_idx is gport instead of fte idx */
                sbx_mac_data.pid = BCM_GPORT_MPLS_PORT_ID_GET(entry->fte_idx);
            } else {
                rv = _bcm_fe2000_g2p3_fte_allocate(unit, addr, &fte_idx, 
                                                   _G2P3_L2CACHE_GPORT_FTE);
                if (rv != BCM_E_NONE) {
		  if (!rsvd_dmac && l2cpmac_idx) {
                    _bcm_fe2000_g2p3_l2cpmac_free(unit, temp_mac);
		  }
                  sal_free(pL2Key);
		  return rv;
                }
                sbx_mac_data.pid = fte_idx;
            }
        }
        if (entry->flags & L2CACHE_ENTRY_DROP) {
            sbx_mac_data.pid = SBX_DROP_FTE(unit);
            if (fte_idx) {
                _bcm_fe2000_g2p3_fte_free(unit, fte_idx,
                                          _G2P3_L2CACHE_GPORT_FTE);
                fte_idx = 0;
        ...
        rv = soc_sbx_g2p3_cmac_set(unit, temp_mac, (addr->vlan & 0xfff),
                                  &sbx_mac_data);
        if (rv == BCM_E_NONE) {
            rv = soc_sbx_g2p3_cmac_commit(unit, SB_COMMIT_COMPLETE);
        }
        ...
        if (rv == BCM_E_NONE) {
            ...
            if (fte_idx) {
                _bcm_fe2000_g2p3_fte_free(unit, fte_idx, _G2P3_L2CACHE_GPORT_FTE);
            }
            if (!rsvd_dmac && l2cpmac_idx) {
            _bcm_fe2000_g2p3_l2cpmac_free(unit, temp_mac);
            }
...
int
_bcm_fe2000_g2p3_l2_cache_entries_init(int unit, uint32 fte_idx)
{
    int                         rv = BCM_E_NONE;
    soc_sbx_g2p3_l2cp_t         l2cp;
    soc_sbx_g2p3_l2cpslow_t     l2cpslow;
...
int 
_bcm_fe2000_g2p3_l2_cache_init(int unit, uint32 *pass_thru_ohi, 
                               uint32 *cpu_fte, uint32 *l2cpmac_table)
{
    int                         rv = BCM_E_NONE;
    uint32                      ohi_idx = ~0;
    uint32                      encap_ete_idx = ~0;
    uint32                      fte_idx = ~0;
    soc_sbx_g2p3_oi2e_t         sbx_ohi;
    soc_sbx_g2p3_eteencap_t     sbx_ete_encap;
...
#ifdef WARM_BOOT_TODO
    SOC_SCACHE_HANDLE_SET(L2CACHE_WB_HDL(unit), unit, 
    ...
    if (!SOC_WARM_BOOT(unit) && (size > 0)) {
        LOG_DEBUG(BSL_LS_BCM_L2,
                  (BSL_META_U(unit,
                   ...
                   (sizeof(fe2k_l2cache_wb_mem_layout_t) + 
                   SOC_WB_SCACHE_CONTROL_SIZE),  
                   (sizeof(fe2k_l2cache_wb_mem_layout_t) + 
                   SOC_WB_SCACHE_CONTROL_SIZE)));
        rv = soc_scache_alloc(unit, L2CACHE_WB_HDL(unit), 
                              (sizeof(fe2k_l2cache_wb_mem_layout_t) + 
                               SOC_WB_SCACHE_CONTROL_SIZE));
        if (BCM_FAILURE(rv)) {
            return rv;
    ...
    wbc = NULL;
#ifdef WARM_BOOT_TODO
    L2CACHE_SCACHE_SIZE(unit) = 0;
    if (size > 0) {
        rv = soc_scache_ptr_get(unit, L2CACHE_WB_HDL(unit), (uint8**)&wbc,
                                &L2CACHE_SCACHE_SIZE(unit));
        if (BCM_FAILURE(rv)) {
            return rv; 
        }

        /* adjust the scache size for the control word */
        L2CACHE_SCACHE_SIZE(unit) = SOC_WB_SCACHE_SIZE(L2CACHE_SCACHE_SIZE(unit));
    }
#endif
    ...
    if (wbc) {
        ...
        LOG_VERBOSE(BSL_LS_BCM_L2,
                    (BSL_META_U(unit,
                                ...
                                "version=%d.%d\n"),
                     (int)wbc->cache, L2CACHE_SCACHE_SIZE(unit),
        ...
        if (SOC_WARM_BOOT(unit)) {
            ...
            BCM_IF_ERROR_RETURN(
                _bcm_l2cache_wb_layout_get(unit, &layout));
            soc_scache_handle_lock(unit, L2CACHE_WB_HDL(unit));
            rv = _bcm_fe2000_g2p3_l2_cache_init_recover(unit, pass_thru_ohi, 
                                                        cpu_fte, 
...
#endif
#ifdef BCM_QE2000_SUPPORT
    BCM_IF_ERROR_RETURN(soc_sbx_g2p3_exc_l2cp_copy_idx_get(unit, &exc_idx));
    ...
    ft.excidx = exc_idx;
#endif
    rv = soc_sbx_g2p3_ft_set(unit, fte_idx, &ft);
    if (rv != BCM_E_NONE) {
        LOG_ERROR(BSL_LS_BCM_L2,
                  (BSL_META_U(unit,
    ...
    rv = _bcm_fe2000_g2p3_l2_cache_entries_init(unit, fte_idx);
    ...
    rv = _sbx_gu2_resource_alloc(unit, SBX_GU2K_USR_RES_OHI, 1, &ohi_idx, 0);
    if (rv != BCM_E_NONE) {
        _sbx_gu2_resource_free(unit, SBX_GU2K_USR_RES_FTE_UNICAST, 1, 
                       &fte_idx, 0);
        LOG_ERROR(BSL_LS_BCM_L2,
    ...
    if (rv != BCM_E_NONE) {
        _sbx_gu2_resource_free(unit, SBX_GU2K_USR_RES_FTE_UNICAST, 1, 
                       &fte_idx, 0);
        _sbx_gu2_resource_free(unit, SBX_GU2K_USR_RES_OHI, 1, &ohi_idx, 0);
        LOG_ERROR(BSL_LS_BCM_L2,
                  (BSL_META_U(unit,
                              "%s: allocate ENCAP ETE failed.\n"),
                   FUNCTION_NAME()));
        return rv;
    }
    ...
    if (rv != BCM_E_NONE) {
        ...
        *cpu_fte = fte_idx;
         ...
         */
            ...
            rv = soc_sbx_g2p3_l2cpslow_set(unit, L2CACHE_WB_MAX_VALID_L2CP_SUBTYPE, 0, &l2cpslow);
            if (rv != SOC_E_NONE) {
                LOG_ERROR(BSL_LS_BCM_L2,
                          (BSL_META_U(unit,
                                      "%s: Error setting L2CP SLOW entry (port:%d "
            ...
            rv = soc_sbx_g2p3_l2cpslow_set(unit, L2CACHE_WB_MAX_VALID_L2CP_SUBTYPE + 1, 0, &l2cpslow);
            if (rv != SOC_E_NONE) {
                LOG_ERROR(BSL_LS_BCM_L2,
                          (BSL_META_U(unit,
                                      "%s: Error setting L2CP SLOW entry (port:%d "
...
/*
 * Function:
 *      _bcm_fe2000_l2_cache_entry_delete
 * Purpose:
 ...
 */
int
_bcm_fe2000_l2_cache_entry_delete(int unit, int idx)
{
    int                         rv = BCM_E_NONE;
    _l2_cache_entry             *entry;
    uint32                      flags;
    ...
    case SOC_SBX_UCODE_TYPE_G2P3:
        rv = _bcm_fe2000_g2p3_l2_cache_hw_entry_delete(unit, idx);
        break;
#endif
    default:
...
/*
 * Function:
 *      _bcm_fe2000_l2_cache_entry_config_get
 * Purpose:
 ...
 */
int
_bcm_fe2000_l2_cache_entry_config_get(int unit, 
                                      uint32 idx,
                                      bcm_l2_cache_addr_t *addr)
{
    int                 rv = BCM_E_NONE;
...
#ifdef BCM_FE2000_P3_SUPPORT
    case SOC_SBX_UCODE_TYPE_G2P3:
        rv = _bcm_fe2000_g2p3_l2_cache_hw_entry_config_get(unit, idx, addr);
        break;
#endif
    default:
        SBX_UNKNOWN_UCODE_WARN(unit);
...
/*
 * Function:
 *      _bcm_fe2000_l2_cache_entry_config_set
 * Purpose:
 ...
 */
int
_bcm_fe2000_l2_cache_entry_config_set(int unit, int idx, 
                                      bcm_l2_cache_addr_t *addr)
{
    int                         rv = BCM_E_NONE;
    uint32                      fte_idx = ~0;
    ...
    if (addr->flags & BCM_L2_CACHE_TUNNEL) {
        ...
        if (BCM_GPORT_IS_MPLS_PORT(addr->dest_port)) {
            ...
            if (!_FE2K_MPLS_PORT_FTE_VALID(unit, fte_idx)) {
                LOG_ERROR(BSL_LS_BCM_L2,
                          (BSL_META_U(unit,
...
/*
 * Function:
 *      _bcm_fe2000_l2_cache_index_get
 * Purpose:
 ...
 */
int
_bcm_fe2000_l2_cache_index_get(int unit, bcm_l2_cache_addr_t *addr, int *real_idx)
{
    int rv = BCM_E_NONE;
    int port, subtype = 0;
...
/*
 * Function:
 *      _bcm_fe2000_l2_cache_entry_set
 * Purpose:
 ...
 */
int
_bcm_fe2000_l2_cache_entry_set(int unit, int index, bcm_l2_cache_addr_t *addr, 
                               int *index_used)
{
    int                     real_idx = 0;
    int                     rv = BCM_E_NONE;
    ...
    if (index < 0) {
        /* calculate index */
        rv = _bcm_fe2000_l2_cache_index_get(unit, addr, &real_idx);
        if (rv != BCM_E_NONE) {
            return rv;
        }
        ...
        if (entry->flags & L2CACHE_ENTRY_VALID) {
            rv = _bcm_fe2000_l2_cache_entry_delete(unit, real_idx);
            COMPILER_REFERENCE(rv);
        }
    }

    /* _config_set API sets up the contents of entry */
    rv = _bcm_fe2000_l2_cache_entry_config_set(unit, real_idx, addr);
    if (rv != BCM_E_NONE) {
        if (index < 0) {
...
#ifdef BCM_FE2000_P3_SUPPORT
    case SOC_SBX_UCODE_TYPE_G2P3:
        rv = _bcm_fe2000_g2p3_l2_cache_hw_entry_set(unit, addr, real_idx);
        break;
#endif
    default:
        SBX_UNKNOWN_UCODE_WARN(unit);
 ...
 */
int
bcm_fe2000_l2_cache_delete(int unit, int index)
{
    int                         rv = BCM_E_NONE;
    ...
    rv = _bcm_fe2000_l2_cache_entry_delete(unit, index);
...
/*
 * Function:
 *      bcm_fe2000_l2_cache_delete_all
 * Purpose:
 ...
 */
int
bcm_fe2000_l2_cache_delete_all(int unit)
{
    int                 rv = BCM_E_NONE;
    int                 index = 0;
    _l2_cache_entry     *entry;
...
int
bcm_fe2000_l2_cache_detach(int unit)
{
    int rv = BCM_E_NONE;
    ...
    if (!SOC_WARM_BOOT(unit)) {
#endif
    rv = bcm_fe2000_l2_cache_delete_all(unit);
    if (rv != BCM_E_NONE) {
        LOG_ERROR(BSL_LS_BCM_L2,
                  (BSL_META_U(unit,
 ...
 */
int 
bcm_fe2000_l2_cache_init(int unit)
{
    int                         rv = BCM_E_NONE;
    _l2_cache_entry             *entry;
    ...
    if (_l2_cache[unit].lock) {
        rv = bcm_fe2000_l2_cache_detach(unit);
        if (BCM_FAILURE(rv)) {
            return rv;
        }
    }
    ...
    if (_l2_cache[unit].lock) {
        ...
        _l2_cache[unit].max_idx = SOC_SBX_CFG_FE2000(unit)->l2_cache_max_idx;
...
#ifdef BCM_FE2000_P3_SUPPORT
        case SOC_SBX_UCODE_TYPE_G2P3:
            rv = _bcm_fe2000_g2p3_l2_cache_init(unit, 
 ...
 */
int 
bcm_fe2000_l2_cache_size_get(int unit, int *size)
{
    if (!size) {
        return BCM_E_PARAM;
    }
...
int 
bcm_fe2000_l2_cache_set(int unit, int index, bcm_l2_cache_addr_t *addr,
                        int *index_used)
{
    int                     rv = BCM_E_NONE;
    int                     rsvd_mac = 0;
    ...
    L2CACHE_UNIT_LOCK(unit);
    rv = _bcm_fe2000_l2_cache_entry_set(unit, index, addr, index_used);
    L2CACHE_UNIT_UNLOCK(unit);
 ...
 */
int
bcm_fe2000_l2_cache_get(int unit, int index, bcm_l2_cache_addr_t *addr)
{
    int                         rv = BCM_E_NONE;
    ...
    L2CACHE_UNIT_LOCK(unit);
    rv = _bcm_fe2000_l2_cache_entry_config_get(unit, index, addr);
    L2CACHE_UNIT_UNLOCK(unit);
