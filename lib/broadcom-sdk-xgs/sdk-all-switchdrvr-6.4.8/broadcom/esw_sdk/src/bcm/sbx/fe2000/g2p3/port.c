/*
 * $Id: port.c,v 1.119 Broadcom SDK $
...
_bcm_fe2000_g2p3_port_ilib_common_init(int unit)
{
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_PORT,
                  (BSL_META_U(unit,
                              "bcm_port_init: unit=%d rv=%d soc_sbx_g2p3_max_qos_profile_index_get\n"),
                   unit, rv));
        return rv;
    } 
                                      ...
                                      bcm_vlan_data_t *vd)
{
    ...
    if (BCM_FAILURE(temp_rv)) {
        ...
        LOG_ERROR(BSL_LS_BCM_PORT,
                  (BSL_META_U(unit,
                              "bcm_port_init: unit=%d rv=%d port=%d soc_sbx_g2p3_lp_set\n"),
                   unit, rv, port));
    }
...
_bcm_fe2000_g2p3_port_ilib_egr_init(int unit, bcm_port_t port)
{
    ...
    if (BCM_FAILURE(temp_rv)) {
        ...
        LOG_ERROR(BSL_LS_BCM_PORT,
                  (BSL_META_U(unit,
                              "bcm_port_init: unit=%d port=%d soc_sbx_g2p3_oi2e_set\n"),
                   unit, port));
    }
    ...
    LOG_VERBOSE(BSL_LS_BCM_PORT,
                (BSL_META_U(unit,
                            "write tag %d:eteL2[%08X]\n"),
                 unit, etel2));
    if (BCM_FAILURE(temp_rv)) {
        rv = temp_rv;
        LOG_ERROR(BSL_LS_BCM_PORT,
                  (BSL_META_U(unit,
                              "bcm_port_init: unit=%d port=%d soc_sbx_g2p3_etel2_set\n"),
                   unit, port));
    }
...
_bcm_fe2000_g2p3_port_ilib_tpid_init(int unit, bcm_port_t port)
{
    ...
    if (SOC_WARM_BOOT(unit)) {
        ...
        LOG_VERBOSE(BSL_LS_BCM_PORT,
                    (BSL_META_U(unit,
                                "Port %d p2e.provider=%d twin=%d stpid1=%d\n"),
                      ...
                      uint32               *etei)
{
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_PORT,
                  (BSL_META_U(unit,
                              "Failed to read ETE Encap[0x%x] FT[0x%x] "
                               "oi2e[0x%x]: %d (%s)\n"),
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_PORT,
                  (BSL_META_U(unit,
                              "Failed to read ETE L2[0x%x] FT[0x%x] "
                                        ...
                                        int set)
{
    ...
    sal_memset(&ete, 0, sizeof(soc_sbx_g2p3_etel2_t));
    sal_memset(&ft, 0, sizeof(soc_sbx_g2p3_ft_t));
    sal_memset(&outHeader, 0, sizeof(soc_sbx_g2p3_oi2e_t));
...
_bcm_fe2000_g2p3_port_ilib_well_known_egr_init(int unit)
{
    int                     rv = BCM_E_NONE;
    soc_sbx_g2p3_etel2_t    sbx_etel2;
    soc_sbx_g2p3_eteencap_t sbx_eteencap;
    soc_sbx_g2p3_oi2e_t     sbx_oi2e;
    uint32                  ete_l2, ete_encap_pm=0, ete_l2_pm=0;
    ...
    if ((!_sbx_port_handler[unit]->ete_l2_valid) ||
        ...
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_PORT,
                      (BSL_META_U(unit,
                                  "bcm_port_init: unit=%d Failed to alloc eteencap for oam pm\n"),
                       unit));
            return rv;
        }
        rv = _sbx_gu2_resource_alloc(unit,
        ...
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_PORT,
                      (BSL_META_U(unit,
                                  "bcm_port_init: unit=%d Failed to alloc etel2 for oam pm\n"),
                       unit));
            return rv;
        }
    ...
    soc_sbx_g2p3_eteencap_t_init(&sbx_eteencap);
    soc_sbx_g2p3_etel2_t_init(&sbx_etel2);
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_PORT,
                  (BSL_META_U(unit,
                              "bcm_port_init: unit=%d Failed to set raw oi2e\n"),
                   unit));
        return rv;
    }
    ...
    sbx_etel2.usevid = 0;
    sbx_etel2.usetag = 1;
    sbx_etel2.stpcheck = 1;

    rv = soc_sbx_g2p3_etel2_set(unit, ete_l2_pm, &sbx_etel2);
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_PORT,
                  (BSL_META_U(unit,
                              "bcm_port_init: unit=%d Failed to set etel2 for oam pm\n"),
                   unit));
        return rv;
    }

    sbx_eteencap.l2ete = ete_l2_pm;
    sbx_eteencap.ttlcheck = 0;
    sbx_eteencap.ipttldec = 0;
    sbx_eteencap.dmacset = 0;
    sbx_eteencap.dmacsetlsb = 0;
    sbx_eteencap.smacset = 0;
    rv = soc_sbx_g2p3_eteencap_set(unit, ete_encap_pm, &sbx_eteencap);

    if (BCM_SUCCESS(rv)) {
        soc_sbx_g2p3_oi2e_t_init(&sbx_oi2e);
        sbx_oi2e.eteptr = ete_encap_pm;
        rv = soc_sbx_g2p3_oi2e_set(unit, (SBX_RAW_OHI_BASE - SBX_RAW_OHI_BASE) + 1, &sbx_oi2e);
    }
...
_bcm_fe2000_g2p3_port_ctpid_set(int unit, bcm_port_t port, uint16 tpid)
{
    ...
    if (TPID_COUNT(unit).ctpid > 1) {
        LOG_ERROR(BSL_LS_BCM_PORT,
                  (BSL_META_U(unit,
                              "%s: unit=%d CTPID in use by %d ports, cannot change.\n"),
 ...
 */
int
_bcm_fe2000_g2p3_port_profile_shareable(int unit, uint32 curProfileIdx,
                                     ...
                                     int pkt_dscp)
{
    ...
    LOG_VERBOSE(BSL_LS_BCM_PORT,
                (BSL_META_U(unit,
                            "Read tag %d:eteL2[%08X]\n"),
                 unit, etel2));
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_PORT,
                  (BSL_META_U(unit,
                              "port_dscp_unmap_set: unit=%d port=%d soc_sbx_g2p3_etel2_get\n"),
                   unit, port));
        return rv;
    }
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_PORT,
                  (BSL_META_U(unit,
                              "port_dscp_unmap_set: unit=%d port=%d soc_sbx_g2p3_etel2_set\n"),
                   unit, port));
        return rv;
    }
                                     ...
                                     int *pkt_dscp)
{
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_PORT,
                  (BSL_META_U(unit,
                              "port_dscp_unmap_get: unit=%d port=%d soc_sbx_g2p3_etel2_get\n"),
                   unit, port));
        return rv;
    }
    ...
    if (ing_idx >= 0) {
        ...
        if (ing_idx == 0 && lpi != port) {
                ...
                (sal_memcmp(&lp, &default_lp, sizeof(lp)) == 0))
            {
                ...
                LOG_VERBOSE(BSL_LS_BCM_PORT,
                            (BSL_META_U(unit,
                                        "Logical Port 0x%0x is non-default, keeping.\n"), 
                             lpi));
            }
