...
_bcm_td2_port_basic_init(int unit, bcm_port_t port)
{
    ...
    /**********
     * Egress Link Delay
     * Set port link delay compensation as a function of port speed
     */
    rv = bcm_esw_port_speed_get(unit, port, &speed);
    if (BCM_SUCCESS(rv)) {
        ln_delay = speed == 1000 ?
            soc_property_port_get(unit, port, spn_LINK_DELAY_1GBE_NS,
                                  BCM_TD2_LINK_DELAY_1GBE_NS) :
            (speed == 10000 ?
             soc_property_port_get(unit, port, spn_LINK_DELAY_10GBE_NS,
                                   BCM_TD2_LINK_DELAY_10GBE_NS) :
             soc_property_port_get(unit, port, spn_LINK_DELAY_NS,
                                   BCM_TD2_LINK_DELAY_NS));

        SOC_IF_ERROR_RETURN
            (soc_reg_field32_modify(unit, EGR_1588_LINK_DELAY_64r, port,
                                    LINK_DELAYf, ln_delay));
    }
 ...
 *
 *      Detach 'delete' ports (physical port is -1) that are
                                       ...
                                       bcm_td2p_port_resource_t *resource)
{
    ...
    for (i = 0, pr = &resource[0]; i < nport; i++, pr++) {
        ...
        /*
         * Check that ports to be 'destroyed' or 'remapped' do not
            ...
            if (bcm_esw_linkscan_enable_port_get(unit, pr->port) !=
                ...
                LOG_ERROR(BSL_LS_BCM_PORT,
                          (BSL_META_U(unit,
                                      ...
                                      "disabled on ports to be destroyed or "
                                      "remapped, port=%d\n"),
                           pr->port));
                return BCM_E_BUSY;
            }
 ...
 *      as a result of the FlexPort operation.
 *
                               ...
                               int *op)
{
    ...
    *op = BCM_TD2P_PORT_RESOURCE_OP_NONE;
    ...
    for (i = 0, pr = &resource[0]; i < nport; i++, pr++) {
        ...
        /*
         * NOTE:
         * When a port is first configure, the given speed is the MAX
         * allowed even though the number of lanes assigned to a port
         * allows for more.
         */
 ...
 * Port Mapping
 * ------------
 * A logical to physical port mapping is set as part of the port resource
                                ...
                                int nport, bcm_td2p_port_resource_t *resource)
{
     ...
     *     + ONLY for destroyed or remapped ports.
     ...
     *     + ONLY for new or remapped ports.
     *     + New or remapped ports are initialized to a default state
    ...
    for (i = 0; i < nport; i++) {
        if (resource[i].physical_port == -1) {
            rv = bcmi_esw_portctrl_loopback_get(unit,
                                                resource[i].port, &loopback);
            if (BCM_FAILURE(rv)) {
                sal_free(soc_resource);
                return rv;
            }
                                  ...
                                  bcm_td2p_port_resource_t *resource)
{
     ...
     * Speed
     *
     * Execute speed changes only if FlexPort sequence has not
     * been performed (FlexPort sequence includes speed changes).
     */
    if ((op & BCM_TD2P_PORT_RESOURCE_OP_SPEED) && !flexed) {
        LOG_VERBOSE(BSL_LS_BCM_PORT,
                    (BSL_META_U(unit,
        ...
        for (i = 0; i < nport; i++) {
            ...
            BCM_IF_ERROR_RETURN
                (bcmi_esw_portctrl_speed_set(unit, resource[i].port,
                                             resource[i].speed));
        }
    }
