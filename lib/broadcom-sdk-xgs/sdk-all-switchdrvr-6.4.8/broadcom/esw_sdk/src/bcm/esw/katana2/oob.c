                              ...
                              int status, int dir)
{
    ...
    if (SOC_IS_SABER2(unit)) {
        if ((0 <= localport) && (localport < BCM_SB2_MAX_RX_MEM_ENTRY)) {
            BCM_IF_ERROR_RETURN
        ...
        if ((0 <= localport) && (localport < BCM_KT2_MAX_RX_MEM_ENTRY)) { 
            BCM_IF_ERROR_RETURN
                            ...
                            bcm_oob_fc_rx_intf_id_t intf_id)
{
    ...
    char *sysport_buf, *tc2pri_buf;
    uint32 old_sysport_list[BCM_KT2_MAX_RX_MEM_ENTRY];
    uint64 old_tc2pri_list[BCM_KT2_MAX_RX_MEM_ENTRY];
    uint32 new_sysport_list[BCM_KT2_MAX_RX_MEM_ENTRY];
    uint64 new_tc2pri_list[BCM_KT2_MAX_RX_MEM_ENTRY];
    int rv = BCM_E_INTERNAL;
    int mem_wsz = 0;
    int i = 0, j = 0, k = 0;
    void *pentry;
