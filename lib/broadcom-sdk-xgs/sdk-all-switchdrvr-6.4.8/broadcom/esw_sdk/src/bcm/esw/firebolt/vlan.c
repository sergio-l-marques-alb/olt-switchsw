...
#if defined (BCM_TRX_SUPPORT)
    protocol_pkt_def = &entry.protocol_pkt;
...
#if defined(BCM_TRIDENT2PLUS_SUPPORT) && defined(INCLUDE_L3)
     if (soc_feature(unit, soc_feature_vlan_vfi_membership)) {
         return bcm_td2p_vp_group_vlan_vfi_stg_get(
                    unit, vid, BCMI_VP_GROUP_INGRESS, stg_ptr);
     }
#endif /* BCM_TRIDENT2PLUS_SUPPORT */
                       ...
                       pbmp_t ing_pbmp)
{
    ...
    if (soc_feature(unit, soc_feature_vlan_vfi_membership)) {
        BCM_IF_ERROR_RETURN
            (bcm_td2p_ing_vlan_vfi_membership_add(unit, vid, ing_pbmp));
        BCM_IF_ERROR_RETURN
                          ...
                          pbmp_t ing_pbmp)
{
    ...
    if (soc_feature(unit, soc_feature_vlan_vfi_membership)) {
        BCM_IF_ERROR_RETURN
            (bcm_td2p_ing_vlan_vfi_membership_delete(unit, vid, ing_pbmp));
        BCM_IF_ERROR_RETURN
...
#if defined(BCM_TRIDENT2PLUS_SUPPORT) && defined(INCLUDE_L3)
    if (soc_feature(unit, soc_feature_vlan_vfi_membership)) {
        BCM_IF_ERROR_RETURN(bcm_td2p_vp_group_vlan_vfi_stg_set(
            unit, vid, BCMI_VP_GROUP_EGRESS, stg));

        return bcm_td2p_vp_group_vlan_vfi_stg_set(
                unit, vid, BCMI_VP_GROUP_INGRESS, stg);
    }
#endif /* BCM_TRIDENT2PLUS_SUPPORT */
