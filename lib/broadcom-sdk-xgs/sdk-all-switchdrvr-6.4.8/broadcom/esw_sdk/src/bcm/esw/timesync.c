...
defined(BCM_TRIDENT2PLUS_SUPPORT) || defined(BCM_HELIX4_SUPPORT)
    ...
    if (soc_feature(unit, soc_feature_timesync_v3) ||
        SOC_IS_KATANA2(unit) || SOC_IS_TRIUMPH3(unit) || SOC_IS_SABER2(unit) ||
        SOC_IS_TRIDENT2PLUS(unit) || SOC_IS_HELIX4(unit)) {
        if (SOC_IS_KATANA2(unit)) {
     ...
     * ports (XLPORT..), enable 48bit timestamping mode in xlport block,
    ...
    if (SOC_IS_SABER2(unit) && IS_XL_PORT(unit, port)) {
        ...
        soc_reg_field32_modify(
            unit, XLPORT_MODE_REGr, port, EGR_1588_TIMESTAMPING_CMIC_48_ENf, 0);
    }
#endif
    ...
    if (SOC_IS_HURRICANE2(unit) || SOC_IS_GREYHOUND(unit) ||
        ...
        for(index=0;
            ...
            if (val == SOC_BLK_XLPORT) {
                if (SOC_IS_HURRICANE2(unit) || SOC_IS_TRIDENT2PLUS(unit)) {
                    SOC_IF_ERROR_RETURN(
                        soc_reg_field32_modify(
