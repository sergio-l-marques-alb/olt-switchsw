...
#include <soc/defs.h>
#if defined(BCM_TRIDENT2PLUS_SUPPORT) && defined(BCM_FIELD_SUPPORT)
...
/* TOS_FN_LOW */
    ...
    _FP_QUAL_PRIMARY_ADD(unit, stage_fc, bcmFieldQualifyTos,
    ...
    if (f_st->hw_ref_count > 0) {
        ...
        if (f_st->flex_mode != 0 && f_st->hw_ref_count > 0) {
            ...
            if (stage_id == _BCM_FIELD_STAGE_INGRESS) {
                ...
                rv = _bcm_esw_stat_flex_detach_egress_table_counters (unit,
                        policy_mem, entry_idx);
            }
                                     ...
                                     _field_entry_wb_t *f_ent_wb)
{
    ...
    if ((stage_fc == NULL) || (f_ent_wb == NULL) || (f_ent == NULL)) {
        return BCM_E_PARAM;
    }
    ...
    if (oam_stat_count != 0) {
        ...
        f_ent_wb->is_present_bmp |= _FP_OAM_STAT_ACTION_WB_SYNC;


        /* Alloc Structure for oam_stat */
         oam_stat_size = sizeof (_field_oam_stat_action_wb_t)
                             * oam_stat_count;
        _FP_XGS3_ALLOC(f_ent_wb->oam_stat_action_wb,
                oam_stat_size, "Oam Stat Action WB");
        if (f_ent_wb->oam_stat_action_wb == NULL) {
            return BCM_E_MEMORY;
        }
        ...
        while (oam_stat_node != NULL) {
            if ((oam_stat_node->flags & _FP_OAM_STAT_ACTION_VALID) &&
                ...
                oam_action = oam_stat_node->oam_stat_action;
                if (oam_action != NULL) {
                    f_ent_wb->oam_stat_action_wb[idx].stat_object =
                        oam_action->stat_object;
                    f_ent_wb->oam_stat_action_wb[idx].stat_mode_id =
                        oam_action->stat_mode_id;
                }
                idx++;
            }
        ...
        /* Add oam stat count */
        f_ent_wb->oam_stat_action_count = idx;
    }
                                           ...
                                           _field_entry_wb_t *f_ent_wb)
{
    ...
    if ((f_ent == NULL) || (f_ent_wb == NULL) || (stage_fc == NULL)) {
        return BCM_E_PARAM;
    }
    ...
    if ((f_ent_wb->is_present_bmp & _FP_OAM_STAT_ACTION_WB_SYNC) == 0) {
        return BCM_E_NONE;
    }

    if (f_ent_wb->oam_stat_action_count == 0) {
        return BCM_E_NONE;
    }

    for (idx = 0 ; idx < f_ent_wb->oam_stat_action_count; idx++) {
        sal_memset (&oam_stat_action, 0, sizeof(bcm_field_oam_stat_action_t));
        oam_stat_action.stat_object  = f_ent_wb->oam_stat_action_wb[idx].stat_object;
        oam_stat_action.stat_mode_id = f_ent_wb->oam_stat_action_wb[idx].stat_mode_id;
        ...
        _field_entry_t   *f_ent,  _field_entry_wb_t *f_ent_wb)
{
    ...
    if ((f_ent_st->flags & _FP_ENTRY_EXTENDED_STAT_VALID)
        ...
        f_ent_wb->is_present_bmp |= _FP_EXTENDED_STATS_WB_SYNC;

        _FP_XGS3_ALLOC(f_ent_wb->extended_stat_wb,
                (sizeof (_field_extended_stat_wb_t)),
                "Extended Stat WB");
        if (f_ent_wb->extended_stat_wb == NULL) {
            return BCM_E_MEMORY;
        }
        extended_stat_wb = f_ent_wb->extended_stat_wb;
        ...
        rv = _bcm_field_stat_get(unit, f_ent_st->extended_sid , &f_st);
        if (BCM_FAILURE(rv)) {
            sal_free (f_ent_wb->extended_stat_wb);
            f_ent_wb->extended_stat_wb = NULL;
            return (rv);
        }
                               ...
                               bcm_field_stat_action_t action)
{
    ...
    extended_stats = f_ent_wb->extended_stat_wb;
