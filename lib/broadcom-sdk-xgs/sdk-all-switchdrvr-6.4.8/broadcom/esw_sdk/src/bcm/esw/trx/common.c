...
#ifdef BCM_TRIDENT2PLUS_SUPPORT
    if (SOC_IS_TRIDENT2PLUS(unit) && !soc_feature(unit, soc_feature_mpls)) {
        ;/* If TD2+ and MPLS feature is disabled skip MPLS memory init. */
    } else
#endif
    if (SOC_MEM_IS_VALID(unit, mem)) {
...
#ifdef BCM_KATANA2_SUPPORT
    if (soc_mem_index_max(unit, VLAN_PROTOCOL_DATAm) > 0) {
