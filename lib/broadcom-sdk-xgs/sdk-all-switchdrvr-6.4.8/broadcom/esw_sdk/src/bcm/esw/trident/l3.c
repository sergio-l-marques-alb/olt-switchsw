...
_bcm_td_l3_ecmp_grp_add (int unit, int ecmp_grp, void *buf, int max_paths)
{
        ...
        if (rv == BCM_E_FULL) {
            ...
            if (soc_feature(unit, soc_feature_hierarchical_ecmp) &&
                ...
                if (data.tbl_ptr->idx_min > 0) {
                    /* 
                     * Do not defragment when grp index is > 1K, and the 
                     * current group getting inserted is not underlay/overlay
                     */
                    if (grp_tbl_ptr->idx_min == 0) {
                        return rv;
                    }
    ...
    for (idx = 0, nh_cycle_idx = 0; idx < max_grp_size; idx++, nh_cycle_idx++) {
         ...
         sal_memset (initial_l3_ecmp, 0, SOC_MAX_MEM_FIELD_WORDS * sizeof(uint32));
        ...
        if (soc_feature(unit, soc_feature_hierarchical_ecmp)) {
            if (BCM_XGS3_L3_MPATH_EGRESS_IDX_VALID(unit,
                ...
                soc_mem_field32_set(unit, L3_ECMPm, l3_ecmp,
                                  ECMP_FLAGf, 1);
                soc_mem_field32_set(unit, INITIAL_L3_ECMPm, initial_l3_ecmp,
                                  ECMP_FLAGf, 1);
            }
        }
#endif /* BCM_TOMAHAWK_SUPPORT */
