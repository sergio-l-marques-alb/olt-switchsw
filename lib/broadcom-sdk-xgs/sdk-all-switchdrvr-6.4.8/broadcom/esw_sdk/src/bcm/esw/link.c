...
_bcm_esw_link_fault_get(int unit, int port, int *fault)
{
    ...
    if (IS_HG_PORT(unit, port) || IS_XE_PORT(unit, port) ||
        ...
        clr_reg = INVALIDr;
        if (SOC_IS_TOMAHAWK(unit) && IS_CL_PORT(unit, port)) {
    ...
    defined(BCM_VALKYRIE2_SUPPORT)
        ...
        if (new_link) {
            ...
            */
             ...
             * This can happen in the case where a Disable followed by an 
             * Enable event is missed by Linkscan i.e.
             * Disable -> Enable -> LS -> Disable -> Enable -> LS
             * In this case Linkscan and Admin Enable will result in MAC not
             * enabled.  The MAC and PHY will be out of sync unless an update
             * is performed
             */
    ...
    if (!lc->lc_warm_boot) {
        ...
        PBMP_ITER(PBMP_PORT_ALL(unit), port) {
            ...
            if ((!BCM_FAILURE(rv)) && !link) {
                /* In legacy devices, PHY status register may show wrong status
                 * on first read and we need to confirm by reading second time
                 * if PHY status is shown as down.
                 */
                /*
                 * COVERITY
                 * The iterator above PBMP_ITER will protect the port variable
