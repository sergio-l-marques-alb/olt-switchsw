...
STATIC int bcmi_xgs4_udf_offset_install(int unit,
    int e, uint32 hw_bmap, int base, int offset, int pipe_num, int class_id);
...
#if defined (BCM_TOMAHAWK_SUPPORT)
STATIC bcm_udf_oper_mode_t bcmi_xgs4_udf_oper_mode = bcmUdfOperModeGlobal;
#endif
...
#if defined (BCM_TOMAHAWK_SUPPORT)
    if (soc_feature(unit, soc_feature_udf_multi_pipe_support) &&
       (bcmi_xgs4_udf_oper_mode == bcmUdfOperModePipeLocal)) {
...
bcmi_xgs4_udf_wb_scache_size_get_1_0(int unit, int *req_scache_size)
{
    ...
    if (SOC_IS_TOMAHAWK(unit)) {
        /* operation mode */
        alloc_size += sizeof(bcmi_xgs4_udf_oper_mode);
    }
#endif
                                        ...
                                        uint8 **scache_ptr)
{
    ...
    while (num_offset_nodes > 0) {
        ...
        if ((soc_feature(unit, soc_feature_udf_multi_pipe_support)) && 
           (recovered_ver >= BCM_WB_VERSION_1_2)) {
            SOC_PBMP_ASSIGN(offset_info->associated_ports, scache_offset_p->associated_ports); 
        }
#endif
        ...
        for (i = 0; i < max_chunks; i++) {
            ...
            if (val) {
                ...
                offset_info->grp_id = UDF_CTRL(unit)->offset_entry_array[i].grp_id;
            }
        }
...
/* clear invalid udf_offset cache recovery */
   ...
   if (soc_feature(unit, soc_feature_udf_multi_pipe_support)) {
       ...
       for(i=0;i<offset_num;i++){
           if (bcmi_xgs4_udf_oper_mode == bcmUdfOperModePipeLocal) {
...
bcmi_xgs4_udf_wb_sync_1_0(int unit, uint8 **scache_ptr)
{
    ...
    *u32_scache_p = UDF_CTRL(unit)->num_pkt_formats;
    ...
    if (SOC_IS_TOMAHAWK(unit)) {
        ...
        *u32_scache_p = bcmi_xgs4_udf_oper_mode ;
        u32_scache_p++;
    }
#endif
 ...
 */
STATIC int
bcmi_xgs4_udf_offset_reserve(int unit, int max_offsets, int offset[])
{
    int j;
    int first = 0;
 ...
 */
STATIC int
bcmi_xgs4_udf_offset_unreserve(int unit, int max_offsets, int offset[])
{
    int j; 
    int gran = BCMI_XGS4_UDF_CTRL_OFFSET_GRAN(unit);
    bcmi_xgs4_udf_offset_entry_t *offset_entry;
                                    ...
                                    uint32 *hw_buf, uint16 *flags)
{
    ...
    /* If the field is not supported, reuturn ANY */
...
#if defined (BCM_TOMAHAWK_SUPPORT)
        if (soc_feature(unit, soc_feature_udf_multi_pipe_support)) {
            bcmi_xgs4_udf_oper_mode = bcmUdfOperModeGlobal;
        }
#endif
    ...
    if ((hints != NULL) && (hints->flags & BCM_UDF_CREATE_O_SHARED_HWID)) {
        ...
        for (i = 0; i < max_chunks; i ++) {
            if (SHR_BITGET(&(offset_info->hw_bmap), i)) {
                UDF_CTRL(unit)->offset_entry_array[i].num_udfs += 1;
                UDF_CTRL(unit)->offset_entry_array[i].flags |=
    ...
    bcm_udf_pkt_format_id_t pkt_format_id)
{
    ...
    /* code to prevent using same pkt_format with differnt udf offsets without proper flag */
#if defined (BCM_TOMAHAWK_SUPPORT)
    if (soc_feature(unit, soc_feature_udf_multi_pipe_support) &&
       (bcmi_xgs4_udf_oper_mode == bcmUdfOperModePipeLocal)) {
    ...
    bcm_udf_pkt_format_id_t pkt_format_id)
{
    ...
    if (soc_feature(unit, soc_feature_udf_multi_pipe_support) &&
       (bcmi_xgs4_udf_oper_mode == bcmUdfOperModePipeLocal)) {
        pipe_num = tcam_info->associated_pipe;
        ...
        tcam_mem = SOC_MEM_UNIQUE_ACC(unit, UDF_CTRL(unit)->tcam_mem)[pipe_num];
        tcam_entry = &(UDF_CTRL(unit)->tcam_entry_array_per_pipe[pipe_num][(tcam_info->hw_idx)]);
    } else 
#endif
    {
        tcam_mem   = UDF_CTRL(unit)->tcam_mem;
    ...
    rv = bcmi_xgs4_udf_offset_uninstall(unit, tcam_info->hw_idx,
                                        offset_info->hw_bmap, 
    ...
    bcm_udf_oper_mode_t mode)
{
    ...
    if (bcmi_xgs4_udf_oper_mode == mode) {
        return BCM_E_NONE;
    }
    ...
    bcm_udf_oper_mode_t mode)
{
    ...
    rv = soc_generic_ser_mem_update(unit, FP_UDF_TCAMm, ser_mode);
    if (SOC_FAILURE(rv)) {
        LOG_CLI((BSL_META("Udf SER memory update fail \n")));
        return rv;
    }
