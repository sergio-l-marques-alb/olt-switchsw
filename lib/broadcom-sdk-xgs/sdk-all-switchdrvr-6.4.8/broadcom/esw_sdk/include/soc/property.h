 ...
 * 1: Oversubscribe mode with port bitmap specified via pbmp_oversubscribe.
 * 2: Mixed mode. Check device specification for applicability.
 */
...
/* Link delay for other-than-1/10 GbE port (ns) */
...
/*
 * Following properties are used to store and28_shmoo_config_param_t Values.
 * Restores these values only if Auto-Tune is Off
 */
...
/*
 * If set, trunk MC destinations can be defined using ingress-MC only (egress MC is not allowed).
 * For ingress-MC, trunk-id should be specified as destination.
 * If unset, trunk MC destinations can be defined using both ingress and egress-MC.
 * The destinations are specified by listing all trunk member-ports as MC destinations.
 * In this case, egress pruning is applied by the HW to ensure that a single replica is egressed for a trunk destination.
 * This feature is available for BCM88660 and later devices.
 * Default value is 0 (unset)
 */
