...
STATIC int eagle_diagnostics_eyescan_run_uc(const phymod_phy_access_t* phy, uint32_t flags)   
{
    ...
    for(j=0; j<4; j++) { /* Loop for all lanes. */
...
int eagle_diagnostics_eye_margin_proj( const phymod_phy_access_t* phy, uint32_t flags,
  ...
  if(PHYMOD_EYESCAN_F_PROCESS_GET(flags)) {
    PHYMOD_IF_ERR_RETURN(eagle_tsc_get_uc_core_config (&(phy->access), &core_cfg));
    PHYMOD_IF_ERR_RETURN(eagle_osr_mode_get(&phy->access, &osr_mode));
    vco_rate_in_Mhz=(1000 * (22 + core_cfg.field.vco_rate)) >> 2;
    data_rate = (vco_rate_in_Mhz * 1000.0 * 1000.0) / (1<<osr_mode);
    PHYMOD_DIAG_OUT((" data rate       = %f\n", data_rate ));

    eagle_tsc_eye_margin_proj(&phy->access, data_rate, eyescan_options->ber_proj_scan_mode,
