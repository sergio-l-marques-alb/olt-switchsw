...
/**
 * \brief - corrective action for interrupt handler when BDB has 1b/2b ECC error.
 * This function will calculate the logical buffer which has the last ECC error and move it to quarantine (if needed).
 *
 * \param [in] unit - unit number
 * \param [in] hbc_block - HBC block where the ECC error occurred.
 *
 * \return
 *   shr_error_e
 *
