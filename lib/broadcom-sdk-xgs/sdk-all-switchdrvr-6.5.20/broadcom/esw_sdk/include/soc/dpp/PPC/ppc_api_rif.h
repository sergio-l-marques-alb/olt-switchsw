...
/* Indicate VTT termination with Port as context */
...
/* SOC_PPC_RIF_PROFILE_TO_ROUTING_ENABLERS_VECTOR_BEFORE_INIT represents the enablers that are not updated by the SOC properties
   ...
   Disable routing for arp. enalber unknown protocol routing for UNKNOWN_L3 trap */
#define  SOC_PPC_RIF_PROFILE_TO_ROUTING_ENABLERS_VECTOR_BEFORE_INIT         (SOC_IS_JERICHO_PLUS(unit) ? 0x30FFFFD : 0x30FFFFC);/* b 0000 0011 0000 1111 1111 1111 1111 1100 */
...
{ 
   ...
   */
  /* 
   * Outermost label when performing 4 label pop 
   */ 
...
typedef struct
{
  ...
  /*
   *  VLAN Domain
   *  Used in case of termination based Port.
   */
  uint32 vlan_domain;
...
typedef struct
{
  ...
  uint8 ip_gre_tunnel; 
  /*
   * Qualifier.next-protocol: The next protocol at the Parser Encoding.
   */
  uint16 qualifier_next_protocol; 
  /*
   * Qualifier Next protocol mask.
   */
