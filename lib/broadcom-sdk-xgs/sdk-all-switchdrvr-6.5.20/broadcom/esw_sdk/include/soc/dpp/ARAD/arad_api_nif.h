...
/** Define KBP LUT entry write mode, ROP or MDIO, only relevant for OP and beyond */
typedef enum
{
...
typedef struct
{
  ...
  /*
   * In Dual Host Quad Port mode, the even unit is the master device, which
   * is responsible for the OP init.
   * In the other mode, is_master is always TRUE.
   */
  uint8  is_master;
  ...
  /*
   * Use to store the kbp port to Jer core mapping.
   */
  uint8 kbp_port_core_mapping[4];
  /*
   * Use to store KBP LUT entry write mode, ROP or MDIO,
   * only relevant for BCM52311 and beyond.
   */
  ARAD_NIF_ELK_TCAM_LUT_WRITE_MODE kbp_lut_write_mode;
  /*
   * Use to enable KBP serdes PCIe init,
   * only relevant for BCM52311 and beyond.
   */
