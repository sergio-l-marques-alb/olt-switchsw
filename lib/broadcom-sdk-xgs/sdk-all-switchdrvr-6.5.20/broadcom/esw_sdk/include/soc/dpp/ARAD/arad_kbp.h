...
/* Different macros used for IPv4 and IPv6 MC in case a customer requests splitting them */
/* Macro for checking ipmc_lookup_model SOC property. Used for updating the KBP IPv4 MC DB and lookup key. */
#define ARAD_KBP_IPV4_LOOKUP_MODEL_1_IS_IN_USE(_unit_) (SOC_DPP_CONFIG(_unit_)->pp.ipmc_lookup_model == 1)
/* Macro for checking ipmc_lookup_model SOC property. Used for updating the KBP IPv6 MC DB and lookup key. */
...
#define ARAD_KBP_IS_CR_MODE(unit) (0) /*(SOC_UNIT_VALID(unit) && SOC_IS_JERICHO(unit) && (DCMN_CR_JOURNALING_MODE_DISABLED != soc_dcmn_cr_journaling_mode_get(unit)))*/
...
typedef enum
{
    ...
    ARAD_KBP_ACL_IN_MASTER_KEY_LSB_MSB       = 2, /* ACL will be split into two parts*/
    ARAD_KBP_ACL_IN_MASTER_KEY_NOT_REQUIRED  = 3  /* ACL is not required - ACL is comprised of only shared and/or ranged segments */
...
typedef enum
{
    ...
    /* IPV4 */
    ARAD_KBP_FRWRD_LTR_IPV4_UC                       = ARAD_KBP_LTR_ID_FIRST, /*0*/
...
typedef enum
{
    ARAD_KBP_FRWRD_TABLE_OPCODE_NONE          = PROG_FLP_PPPOE_IPV4UC, /* kpb can not parse opcode 0, this FLP program is not
                                                                          used with KBP and we can freely use it as invalid */

    ARAD_KBP_FRWRD_TABLE_OPCODE_IPV4_UC_RPF  = PROG_FLP_IPV4UC_RPF,         /*  3 */
    ...
    ARAD_KBP_FRWRD_TABLE_OPCODE_IPV4_MC_BRIDGE = PROG_FLP_IPV4MC_BRIDGE,    /* 17 */
    ...
    ARAD_KBP_FRWRD_TABLE_OPCODE_IPV4_UC_PUBLIC     = PROG_FLP_IPV4UC_PUBLIC, /* 2 */
    ARAD_KBP_FRWRD_TABLE_OPCODE_IPV4_UC_PUBLIC_RPF = PROG_FLP_IPV4UC_PUBLIC_RPF, /* 59, program is dynamically allocated */

    ARAD_KBP_FRWRD_TABLE_OPCODE_IPV6_UC_PUBLIC     = PROG_FLP_IPV6UC_PUBLIC, /* 66, program is dynamically allocated */
    ARAD_KBP_FRWRD_TABLE_OPCODE_IPV6_UC_PUBLIC_RPF = PROG_FLP_IPV6UC_PUBLIC_RPF, /* 67, program is dynamically allocated */
...
/* Used for Dual Host Quad Port mode */
typedef struct
{
...
#endif /* defined(INCLUDE_KBP) && !defined(__ARAD_KBP_INCLUDED__) */
