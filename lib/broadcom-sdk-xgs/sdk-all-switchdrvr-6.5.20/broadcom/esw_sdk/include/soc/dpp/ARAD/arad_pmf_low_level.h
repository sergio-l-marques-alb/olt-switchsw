...
typedef struct
{
 ...
 /*
  * Padding bits at MSB of the CE.
  * Used in ELK lookup to pad the key so that it is byte-aligned.
  */
  uint8 msb_padding;
  ...
  uint8 qual_lsb;
  /*
  * indicates if this CE is shared
   */
  SOC_PPC_FP_QUAL_TYPE                qual_type;
  /*
  * indicates if this CE is shared between different data bases
  */
  uint8 is_ce_shared;
  /*
  * Imformation about the shared CEs
  */
...
typedef struct
{
  ...
  */
  ...
  /*
   * FES is used by combined action
   */
  uint8 is_combined;
