...
/*
 * Take 35 * 4 bits from the lsb of the private key - this allows searching the public tables with inrif = 0.
 */
#define SOC_DPP_DBAL_ZERO_INRIF_IN_KEY        (35)

/*
 * hit_bit indication is passed on a uint8 variable, use the higher bits for flags
 */
#define SOC_DPP_DBAL_HITBIT_FLAG_CLEAR        (1 << 7) /*Clear on read*/
#define SOC_DPP_DBAL_HITBIT_FLAG_HIT          (1 << 0) /*Indicate there was a hit*/
...
typedef struct
{
    ...
    int                             nof_qualifiers;
    /* table extra information that is needed. for LEM/SEM/TCAM tables it is used for key type, for KAPS indicates hitbit enabled. */
