...
typedef enum
{
  ...
  */
  ...
  /*
   *  The AC is mapped according to Port x Vlan x Ethtype  for section oam.
   */
  SOC_PPC_L2_LIF_AC_MAP_KEY_TYPE_PORT_VLAN_ETH = 23,

  /*
   *  The AC is mapped according to Port x Vlan x Vlan x Ethtype   for section oam.
   */
...
typedef struct
{
  ...
  uint32 isid_domain;
  /*
   *  B-SA nickname. Enables mapping to VSID according to
   *  I-SID * B-SA. The nickname is compared against
   *  {B-SA[47:44],B-SA[39:24]}.
   */
  uint32 bsa_nickname;

  /*
   * For SPBM, Indicates whether B-SA nickname should be part of the key or not.
   * If valid, entry is written to BSA+ISID table in SEM-B.
   */
