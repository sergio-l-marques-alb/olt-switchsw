/*******************************************************************************
 *
 * Copyright 2011-2020 Broadcom Corporation
 *
...
/**
 * @brief large multicast acl db type
 */
...
enum kbp_db_properties {
    ...
    KBP_PROP_SEND_PRIORITY_OVER_AD,   /**< Send the priority as part of AD */
    KBP_PROP_ENABLE_COUNTERS,         /**< To enable search-dependent-counters on a database */
    KBP_PROP_COUNTER_CYCLE,           /**< Whether to use 1 cycle counter or two cycle */
    KBP_PROP_COUNTER_TYPE,            /**< Counter type, 32b Compressed OR 64b Full Width */
    KBP_PROP_XL_DB,                   /**< specifies whether the DB is extra large */
    KBP_PROP_LOCALITY,                /**< specifies which locality DB belongs to */
    KBP_PROP_REPLICATE_DB,            /**< specifies whether to replicate a DB for multi thread access */
    KBP_PROP_MC_DB,                   /**< specifies multicast database */
    KBP_PROP_ENABLE_DYNAMIC_ALLOCATION,/**< enable dynamic allocation of DBA and UDA */
    KBP_PROP_UTILISE_UDA_HOLES,        /**< Allocate UDA space for LSN, utilizing existing holes in UDC */
    KBP_PROP_SET_INDIRECTION_AD,      /**< Set the AD type to indirection based AD */
...
enum kbp_entry_properties {
    KBP_ENTRY_PROP_META_PRIORITY, /**< For LPM databases, sets the entry's meta priority. */
 ...
 * free space. If the database reduces in size, the extra free resources
 * borrowed may be returned back to the free pool for other databases
...
* parameters of an already created DB 'orig_db'. Then marks this newly
 ...
 * reserved by this database. All entries in the database are
 * invalidated.
 *
...
/**
 * Assigns a specific hardware resource to the database.
 *
 * @param db Valid database handle.
 * @param resource Hardware resource defined by ::kbp_hw_resource.
 * @param ... Variable arguments defined by the hardware resource type.
 *
 * @return KBP_OK on success or an error code otherwise.
 */
 ...
 * @param db Valid database handle.
 * @param resource Hardware resource defined by ::kbp_hw_resource.
 * @param ... Variable arguments defined by the hardware resource type are returned in these arguments.
 *
 * @return KBP_OK on success or an error code otherwise.
 */
...
/**
 * Adds a key that defines the layout of entries in the database. This
 ...
 * kbp_key_add_field(). This information
...
/**
 * Sets properties for the database. Available properties are defined by the enumeration ::kbp_db_properties. The properties
...
/**
 * Adds a Hit Bit (HB) database to the database. This
 * allows the control plane to associate the two databases,
 * partition the hardware resources, and program the chip.
 *
 * @param db Valid database handle.
 ...
 * database handle. A key must be added to this table to be
...
/**
 * This is same as the kbp_db_add_ace API, but it takes entry index as input
 * which, user can treat as opaque entry handle upon return from API.
 * The index value should be between 1 and 27b max.
 *
 * @param db Initialized KBP database
 * @param data Data bits represented as array of bytes.
 * @param mask Don't-care mask for the bits (1 == don't care).
 * @param priority Priority of the entry (only 22 bits of 32-bit value are valid).
 * @param index Entry index, on success user can treat this as opaque entry handle.
 *
 * @return KBP_OK on success or an error code otherwise.
 */
...
/**
 * This API overwrites an existing TCAM ACL entry and it is not coherent.
 *
 * @param db Initialized KBP database
 ...
 * in a relative position to an existing entry. The
 * database must be initialized as ACL type. The bits that
 * represent ranges should be set to don't cares, and the API
 * kbp_entry_add_range() should be used to specify the actual
 ...
 * both the APIs shall not be used to insert entries into the same DB, i.e.
 ...
 * @param entry_position Whether to insert before or after the relative entry.
 * @param priority Priority of the entry (only 22 bits of 32-bit value are valid).
 * @param entry Opaque entry handle, initialized and returned to user on success.
 *
 * @return KBP_OK on success or an error code otherwise.
...
/**
 * This is same as the kbp_db_add_prefix API, but it takes entry index as input
 * which, user can treat as opaque entry handle upon return from API.
 * The index value should be between 1 and 27b max.
 *
 * @param db Initialized KBP database.
...
/**
 * This is same as the kbp_db_add_em API, but it takes entry index as input
 * which, user can treat as opaque entry handle upon return from API.
 * The index value should be between 1 and 27b max.
 *
 * @param db Initialized KBP database.
 ...
 * memory associated with the entry is recovered.
...
/**
 * Sets properties for the entry. Available properties are defined by the enumeration ::kbp_entry_properties. The properties
 * should be followed by values within the ranges given below.
 *
 * |PROPERTY|VALUES|DEFAULT|
 * |:---:|:---:|:---:|
 * |KBP_ENTRY_PROP_META_PRIORITY| items in enum kbp_db_entry_meta_priority | Meta priority set for the db. KBP_ENTRY_META_PRIORITY_3 (lowest priority) if none set for db |
 * This API is applicable only for entries in LPM databases.
 *
 * @param db Initialized KBP database.
 ...
 * database key starting from the MSB side. Ranges are counted
 * starting from zero.
 *
 * @param db Initialized KBP database.
 ...
 *
 * @param db Initialized KBP database.
 * @param entry Valid handle returned using the call kbp_db_add_prefix(), kbp_db_add_ace() or their with_index versions
 * @param fp The file to print to.
 *
 * @return KBP_OK on success or an error code otherwise.
 */
...
/**
 * Compiles and installs any pending added/deleted entries in the KBP database onto the device.
 *
...
/**
 * Deletes all the pending (not yet installed onto the device) entries in the database/table.
 *
 * @param db Valid KBP database or table.
 *
 * @return KBP_OK on success or an error code otherwise.
...
/**
 * Returns statistics for the KBP database.
 *
 * @param db Initialized KBP database.
...
/**
 * Prototype for index change callback function. When database entries are moved
...
/**
 * Looks up a prefix handle by providing the prefix and its length. This
 ...
 * validate SW structures of the database by comparing the returned
 ...
 * @param prio_len Returns priority of the entry for ACLs, prefix match length for LPM searches, or zero for exact-match searches.
 *
 * @return KBP_OK on success or an error code otherwise.
 */
...
/**
 * Application should call this API to initiate search-dependent-counter reads from the device.
 * This is a non-blocking call and function returns immediately after launching a counter read
 ...
 * refresh Counter Master copy in the host memory. This API must be invoked
 * in a loop until is_complete becomes 1 which means all counters are read back
...
/**
 * Returns priority of an ACL entry, or prefix length of an LPM entry. For
 ...
 * @param db Valid database handle.
 * @param entry Valid KBP-entry handle.
 * @param hb hit bit handle, returned on success.
 *
 * @return KBP_OK on success or an error code otherwise.
 */
...
/**
 * Returns 64b search-dependent-counter value given the entry handle
 *
 * @param db Valid database handle
 * @param entry Valid KBP-entry handle
 * @param value 64b counter value
 *
 * @return KBP_OK on success or an error code otherwise
 */
 ...
 * ACL entries depending on range expansion, it is possible that the
 * ACL entry expands to multiple locations in the KBP. An array of
 * indices is returned to the user. The index array returned
 * must be freed using the API kbp_entry_free_index_array().
 *
 * @param db Valid database handle.
 * @param entry Valid KBP entry handle.
 * @param nindices Set by API to return the number of valid indexes.
 * @param indices Array of indexes of length nindices which holds the indexes for this entry.
 *
 * @return KBP_OK on success or an error code otherwise.
 */
...
struct kbp_entry_info
{
    ...
    struct kbp_ad_db *ad_db; /**< Used to return the AD DB pointer if present or, if not, is set to NULL. */
    struct kbp_entry_rinfo /** Low and high value of range(s) for the entry */ {
