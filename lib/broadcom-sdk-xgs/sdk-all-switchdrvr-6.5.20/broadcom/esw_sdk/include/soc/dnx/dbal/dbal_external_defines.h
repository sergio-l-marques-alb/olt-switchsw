...
/** Note: Incase DBAL_TABLE_MAX_NUM_OF_RESULT_FIELDS_PER_RES_TYPE changed must update maxOccurs in
 *  dbal_types_definition.xsd to the same value Max value is 255 since we are using uint8 variables in the entry
 *  handle. if need to increase it, should handle those variables */
#define DBAL_TABLE_MAX_NUM_OF_RESULT_FIELDS_PER_RES_TYPE      255

/** \brief Default value for max nof instances for a field with nof instances.
 *  This value can be updated per field */
...
/**
 * \brief trigger type enum, to be used by user in order to signal that collection of entries was completed.
 */
typedef enum
{
    /** when using this value the API will  return once all entries were written to HW */
    DBAL_COLLECTION_TRIGGER_PROCESS_AND_WAIT,

    /** when using this value the API will immediately return, it is only used to signal DBAL that no more entries will be
     *  collected and the pool can be processed without waiting that the queue will be full. */
...
typedef struct
{
    ...
    /** Indication that the field is packed with the field before it.
     *  This info is relevant only for KBP tables and is valid only for key fields */
    uint8 is_packed;
