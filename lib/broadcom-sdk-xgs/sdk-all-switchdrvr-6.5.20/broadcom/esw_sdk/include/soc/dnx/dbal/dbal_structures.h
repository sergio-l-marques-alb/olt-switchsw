...
/** \brief the size in bits of the key buffer in the entry handle has to be multiply of 32! */
...
/** key mask is allocated dynamiclly in the entry handle, the key mask is needed only for specific tables */
...
/** collect mode defines */

/** pool size, the number of entries that will be used for collection in each pool */
#define DBAL_COLLECTION_POOL_SIZE                   dnx_data_dbal.db_init.dbal_collection_pool_size_get(unit)

/** number of pools, currently only validated for two. */
#define DBAL_NOF_COLLECTION_POOLS                   2

/** total entry handles added to dbal. */
...
typedef enum {
    ...
    DBAL_TABLE_IND_THREAD_PROTECTION_ENABLED,           /** If this indication enabled, the table will use mutex when an handle for the table is take. 
                                                          * this indication allow to the user to support multhithreaded operations on the specific table. 
    ...
    DBAL_TABLE_IND_COLLECT_MODE,                        /** indicate that the table is working with collection mode */
    DBAL_NOF_TABLE_IND
...
typedef enum
{
    ...
    /** handle was collected and waiting in the pool */
...
typedef struct
{
    ...
    mdb_kaps_ad_db_t_p mdb_lpm_ad_db_p; /* KAPS AD DB pointer */
...
/** struct that centrelize protection info for table related to: DBAL_TABLE_IND_THREAD_PROTECTION_ENABLED */
typedef struct
{
...
typedef struct
{
    /** dbal handles pool, the first DBAL_SW_NOF_ENTRY_HANDLES handles are used for regular dbal operations other handles
     *  are used for the collect mode. DBAL_SW_NOF_ENTRY_HANDLES_FOR_COLLECTION */
    ...
    /** The last entry ID allocated from the collection pool */
    int collection_counter;

    /** mutexes that locking the collection pool */
    sal_mutex_t collection_pool_mutex_array[DBAL_NOF_COLLECTION_POOLS];

    /** mutexes to sync between 2 worker threads - this is done since we don't want that MDB will access from two threads
     *  simultaneously  */
    sal_mutex_t collection_worker_thread_sync;

    /** used to signal the worker thread to start processing */
    sal_sem_t   collection_semaphore_array[DBAL_NOF_COLLECTION_POOLS];

    /** data structure that can be used to propagate task type from main thread to worker threads see for
     *  example:DBAL_COLLECTION_SIGNAL_DESTROY */
