/*
 * $Id: dfe_diag.h,v 1.11 Broadcom SDK $
 *
...
typedef struct soc_dch_counters_info_s { 
    soc_dfe_counter_t     dch_total_in; /* first cell is sum, second is overflow*/
...
typedef struct soc_dcl_counters_info_s { 
    soc_dfe_counter_t   dcl_tot_in;  /* first cell is sum, second- overflow*/
...
typedef struct soc_dfe_queues_dch_info_s {    /* save: A.link,value B.link,value*/
    uint32 nof_link_a;
    uint32 value_a;
    uint32 nof_link_b;
    uint32 value_b;
...
typedef struct soc_dfe_queues_dch_s{         
    soc_dfe_queues_dch_info_t    nof_dch[SOC_DFE_NOF_DCH];     /*for each DCH, save the above info*/     
} soc_dfe_queues_dch_t;

typedef struct soc_dfe_queues_dcm_info_s{                   /* info per fifo for dcm*/
