    ...
    struct kaps_pct *rit)
{
    ...
    ads2_num = 1;
    ads2_depth = device->hw_res->ads2_depth_b;
    if (db->is_type_a)
    {
        ads2_num = 0;
        ads2_depth = device->hw_res->ads2_depth_a;
    }
    ...
    if (rptNode_p->m_iitLmpsofarPfx_p)
    {
        ...
        for (i = 0; i < KAPS_AD_WIDTH_1 / KAPS_BITS_IN_BYTE; ++i)
        {
         ...
         * stored. 
         */
        virtual_it_addr = KAPS_ADS2_RPT_LMPSOFAR_VIRTUAL_IX_START + rptNode_p->m_rpt_prefix_p->m_nIndex;
    ...
    if (rit->u.kaps.bkt_row == KAPS_NUM_ROWS_IN_SMALL_BB - 1)
    {
    ...
    uint32_t row_nr)
{
    ...
    while (node != head)
    {
        ...
        {
            found = 1;
            rpm_p->m_is_extra_byte_added = fibTbl->m_db->common_info->is_extra_byte_added;
            break;
        }
    ...
    uint32_t length)
{
...

    {
        ...
        errNum = kaps_rpm_write_entry_to_hw(self_p, pfxBundle_p, self_p->m_curDbId, self_p->m_is_extra_byte_added);
        if (errNum != NLMERR_OK)
...
NlmErrNum_t
kaps_rpm_pvt_fill_no_match_entries(
    kaps_rpm * rpm_p)
{
    ...
    uint32_t pos = 0;
    NlmErrNum_t errNum;
    kaps_fib_tbl_mgr *fibTblMgr = rpm_p->m_fib_tbl_mgr;

    kaps_memset(rptEntry.m_data, 0xFF, sizeof(rptEntry.m_data));
    kaps_memset(rptEntry.m_mask, 0xFF, sizeof(rptEntry.m_mask));

    for (pos = 0; pos < rpm_p->kaps_simple_dba.pool_size; ++pos)
    {
        errNum = kaps_fib_rpt_write(fibTblMgr, 0, &rptEntry, 0,
                                    (uint16_t) (rpm_p->kaps_simple_dba.pool_start_loc + pos), &reason);
            ...
            if (rptMap & (1ULL << pcm_num))
            {
                if (device->hw_res->pcm_dba_mgr[pcm_num] == NULL)
                    device->hw_res->pcm_dba_mgr[pcm_num]
                        = kaps_simple_dba_init(device, 0, device->map->pinfo[pcm_num].rpt_depth, pcm_num, 0);
                if (!result)
    ...
    NlmReasonCode * o_reason)
{
    ...
    rpm_p->m_is_extra_byte_added = is_extra_byte_added;
    ...
    uint8_t is_extra_byte_added)
{
    ...
    uint32_t numBits = pfxBundle_p->m_nPfxSize % 8;
    ...
    kaps_memset(rptEntry.m_mask, 0, sizeof(rptEntry.m_mask));

    rptStartPos = 0;

    rpm_p->m_is_extra_byte_added = is_extra_byte_added;
    if (rpm_p->m_is_extra_byte_added)
    {

        /*
         * We have added an extra byte to the MS portion of the user entry. The KCR will not be forcing this byte to
         * zero in the key and the key will have garbage values. So we are now always masking off the first byte to get 
         * a hit no matter what is present in the key. 
         */
        rptEntry.m_data[rptStartPos] = 0;
        rptEntry.m_mask[rptStartPos] = 0;
    ...
    /*
     * Directly copy the number of required bits in X-Y format after MSB 160b onwards 
     */
    if (numBytes)
    ...
    {
        rptEntry.m_mask[rptStartPos + i] = ~rptEntry.m_data[rptStartPos + i];
    }

    if (numBits > 0)
    {
        lastByteMask = (uint8_t) (0xff << (8 - numBits));
        /*
         * coverity[overrun-local] 
         */
        rptEntry.m_data[rptStartPos + numBytes] = pfxData[numBytes] & lastByteMask;
        rptEntry.m_mask[rptStartPos + numBytes] = (~pfxData[numBytes]) & lastByteMask;
    }
