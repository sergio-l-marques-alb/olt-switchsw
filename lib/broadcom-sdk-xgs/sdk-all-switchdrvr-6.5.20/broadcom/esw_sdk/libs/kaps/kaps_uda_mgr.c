...
int32_t
kaps_uda_mgr_get_udm_sel_table_new_index(
    struct kaps_uda_mgr *mgr,
    struct kaps_device *device)
{
    int32_t i;

    for (i = 0; i < MAX_UDA_REGIONS; i++)
    {
        if (!mgr->db->is_bc_required)
        {
            if (!(device->udm_sel_table_indices_in_use & (1ULL << i)))
            {
                device->udm_sel_table_indices_in_use |= (1ULL << i);
                return i;
            }
        }
        else
        {
            ...
            uint32_t bc_bmp;
            int32_t flag_useful = 1;

            bc_bmp = mgr->db->bc_bitmap;
            main_dev = kaps_get_main_bc_device(mgr->device);

            for (tmp_dev = main_dev; tmp_dev; tmp_dev = tmp_dev->next_bc_device)
            {
                if (bc_bmp & (1 << tmp_dev->bc_id))
                {
                    if (tmp_dev->udm_sel_table_indices_in_use & (1ULL << i))
                    {
                        flag_useful = 0;
                    }
                }
            ...
            if (flag_useful)
            {
                for (tmp_dev = main_dev; tmp_dev; tmp_dev = tmp_dev->next_bc_device)
                {
                    if (bc_bmp & (1 << tmp_dev->bc_id))
                    {
                        tmp_dev->udm_sel_table_indices_in_use |= (1ULL << i);
                    }
                }
                return i;
            }
    ...
    return MAX_UDA_REGIONS;
}

kaps_status
kaps_uda_mgr_is_udm_sel_table_new_index_available(
    struct kaps_uda_mgr * mgr,
    struct kaps_device * device)
{
    int64_t i;
...
kaps_status
kaps_uda_mgr_release_udm_sel_table_index(
    struct kaps_device * device,
    uint32_t device_udm_sel_table_index)
{
    kaps_sassert(device->udm_sel_table_indices_in_use & (1ULL << device_udm_sel_table_index));
    device->udm_sel_table_indices_in_use &= ~(1ULL << device_udm_sel_table_index);
    ...
    uint8_t alloc_udm[][KAPS_UDM_PER_UDC][KAPS_ALLOC_UDM_MAX_COLS])
{
    ...
    rows_in_udm = device->hw_res->rows_in_lpu;
    ...
    if (db->num_algo_levels_in_db == 3)
    {
        if (db->is_type_a)
        {
            rows_in_udm = device->hw_res->ads2_depth_a / 2;
        }
        else
        {
            rows_in_udm = device->hw_res->ads2_depth_b / 2;
        }
        ...
        if (device->silicon_sub_type == KAPS_JERICHO_2_SUB_TYPE_J2P_THREE_LEVEL)
        {
            ...
            for (i = 0; i < device->hw_res->total_num_bb ; ++i)
            {
                if (alloc_udm[0][0][i])
                {
                    if (device->map->bb[i].bb_num_rows < least_num_rows)
                    {
                        least_num_rows = device->map->bb[i].bb_num_rows;
                    }
                }
            }
            ...
            for (i = 0; i < device->hw_res->total_num_bb ; ++i)
            {
                if (alloc_udm[0][0][i])
                {
                    if (device->map->bb[i].bb_num_rows > least_num_rows)
                    {
                        num_units = device->map->bb[i].bb_num_rows / least_num_rows;

                        kaps_sassert(num_units <= KAPS_UDM_PER_UDC);

                        device->map->bb[i].num_units_in_bb = num_units;
    ...
    int8_t unit_test_mode)
{
    ...
    /*
     * Disable the compaction for kaps2. There are search mismatches if compaction is enabled for JR2. Debug and once
     * resolved, enable it
     */
    if (db->device->id == KAPS_JERICHO_2_DEVICE_ID)
        mgr->enable_compaction = 0;
...
struct uda_mem_chunk *
kaps_uda_mgr_get_chunk_from_uda_addr(
    struct kaps_uda_mgr *mgr,
    int32_t udc_no,
    int32_t udm_no,
    int32_t brick_no)
{
    struct uda_mem_chunk *chunk;
    uint32_t abs_row_no;
    uint32_t region_id = 100, chunk_offset = 0;
    kaps_status status;

    abs_row_no = udm_no * mgr->device->hw_res->rows_in_lpu + brick_no;

    status = kaps_uda_mgr_get_relative_params_from_abs(mgr, udc_no, abs_row_no, &region_id, &chunk_offset);
    kaps_sassert(status == KAPS_OK);

    for (chunk = mgr->neighbor_list; chunk; chunk = chunk->next_neighbor)
    {
        if (chunk->region_id != region_id)
            continue;

        if ((chunk->offset <= chunk_offset) && (chunk_offset < chunk->offset + chunk->size))
        {
            return chunk;
        }
    }
...
struct kaps_db *
kaps_uda_mgr_get_db_from_uda_addr(
    struct kaps_uda_mgr *mgr,
    int32_t udc_no,
    int32_t udm_no,
    int32_t brick_no)
{
    ...
    if (!mgr->num_other_shared_dbs)
        return mgr->db;

    chunk = kaps_uda_mgr_get_chunk_from_uda_addr(mgr, udc_no, udm_no, brick_no);
    kaps_sassert(chunk);
    ...
    int32_t brick_no)
{
    ...
    udm_start_row =
        (mgr->region_info[uda_chunk->region_id].udms[lpu_index] % KAPS_UDM_PER_UDC) * mgr->device->hw_res->rows_in_lpu;
    ...
    uint32_t * p_offset)
{
    ...
    udm_no = lpu_no * KAPS_UDM_PER_UDC + (row_no / mgr->device->hw_res->rows_in_lpu);
    relative_row = row_no % mgr->device->hw_res->rows_in_lpu;
