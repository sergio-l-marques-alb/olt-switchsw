...
{
    /*
     * Currently we can have only max one AD database
     */
...
static void
kaps_kaps_assign_ab_info_default(
    ...
    uint32_t key_width_1)
{
    if (key_width_1 <= 80)
    {
        ...
        ab->num_slots = ab->device->num_80b_entries_ab;

    }
    else if (key_width_1 <= 160)
    {
...
static void
kaps_kaps_assign_ab_type(
    struct kaps_db *db,
    struct kaps_ab_info *ab,
    struct memory_map *map,
    uint32_t key_width_1)
{
    kaps_kaps_assign_ab_info_default(ab, map, key_width_1);
 ...
 * @{
 *
 * Bookkeep and assign the specific ABs to the database
 *
 * @param map pointer to initialized memory_map
 * @param device valid device handle
 * @param db valid database handle
 * @param sb_num pointer to sb
 * @param num_ab number of ABs of this SB to assign
 * @param key_width_1 maximum width of the keys among all the tables in this DB
 * @param ab_list ptr to the list of ABs available for this database
 *
 * @}
 */
static void
kaps_kaps_assign_ab_single_device(
    struct memory_map *map,
    struct kaps_device *device,
    struct kaps_db *db,
    ...
    int8_t is_dup_assignment)
{
    struct kaps_device *main_dev;

    main_dev = device->main_dev ? device->main_dev : device;
    map->ab_memory[ab_no].db = db;
    sb->owners[ab_no % device->num_ab_per_sb] = db;
    sb->is_dup_owner[ab_no % device->num_ab_per_sb] = is_dup_assignment;
    kaps_kaps_assign_ab_type(db, &map->ab_memory[ab_no], map, key_width_1);

    map->ab_memory[ab_no].is_dup_ab = is_dup_assignment;
    db->common_info->num_ab_currently_allocated++;
    sb->free_ab--;
    kaps_sassert(sb->free_ab >= 0);

    main_dev->num_normal_ab_allocated++;


    if (!db->is_bc_required || db->is_main_bc_db)
        if (ab_list)
            kaps_c_list_add_tail(ab_list, &map->ab_memory[ab_no].ab_node);
}
    ...
    int8_t is_dup_assignment)
{
    ...
    {
        kaps_kaps_assign_ab_single_device(map, device, db, sb, ab_no, key_width_1, ab_list, is_dup_assignment);
        return;
    }
    ...
    for (tmp_dev = main_dev; tmp_dev; tmp_dev = tmp_dev->next_bc_device)
    {
        if (bc_bmp & (1 << tmp_dev->bc_id))
        {
            ...
            kaps_sassert(tmp_db);
            if (tmp_dev == device)
                kaps_kaps_assign_ab_single_device(tmp_dev->map, tmp_dev, tmp_db,
                                        &tmp_dev->map->sb_memory[sb->sb_no], ab_no, key_width_1,
                                        ab_list, is_dup_assignment);
            else
                kaps_kaps_assign_ab_single_device(tmp_dev->map, tmp_dev, tmp_db,
                                        &tmp_dev->map->sb_memory[sb->sb_no], ab_no, key_width_1,
                                        NULL, is_dup_assignment);
        }
    }
    ...
    struct kaps_device * device)
{
    ...
    if (device->issu_in_progress)
    {
        for (i = 0; i < device->num_ab; i++)
        {
            if (device->map->ab_memory[i].db != NULL)
            {
                ...
                if (cur_db->type == KAPS_DB_ACL)
                {
                    if (kaps_c_list_count(kaps_resource_get_ab_list(cur_db)) == 0)
                    {
    ...
    for (i = 0; i < device->max_num_searches; i++)
    {
        ...
        if (cur_db)
        {
            ...
            device->map->sb_memory[i].owners[0] = cur_db;
