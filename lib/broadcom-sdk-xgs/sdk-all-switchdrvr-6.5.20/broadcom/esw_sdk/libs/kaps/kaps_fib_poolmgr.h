...
/**
 * This is a ratio of num user entries present / max entries that can be stored in the pool. If this ratio increases beyond
 * the threshold, then a new Array Block is requested
 */
#define KAPS_NEW_BLOCK_DEFAULT_ALLOC_THRESHOLD    (0.7)
...
typedef struct kaps_pool_mgr
{
    ...
    uint32_t m_numAlwaysEmptyIptSlotsPerPool;       /**< Number of Slots that are always empty in an IPT Pool*/
    uint32_t m_numAlwaysEmptyAptSlotsPerPool;       /**< Number of Slots that are always empty in an APT Pool*/
    ...
    double m_new_block_alloc_threshold;             /**< 0 to 1. If the AB usage crosses this threshold, we will acquire new AB*/
} kaps_pool_mgr;
...
/**
 * This is a parsing function for the pool information stored in the warmboot dump file.
 * Pool information is parsed out and dumped into a human readable text
 *
 * @param bin_fp File pointer to original input warmboot dump file
 * @param txt_fp File pointer to the output text file
 * @param num_pools number of active pools including IPT and APT
 * @param bin_offset The offset in the input warmboot dump file. Pool info starts from this offset
 *
 * @return NLMERR_OK on success or error code
 */
NlmErrNum_t kaps_pool_mgr_wb_dump_pools(
    FILE * bin_fp,
    FILE * txt_fp,
    uint32_t num_pools,
    uint32_t * bin_offset);
