...
typedef struct kaps_lsn_mc_settings
{
    ...
    struct kaps_ix_chunk *m_oldIxChunkInfoPtr;
    ...
    uint16_t m_minNumPfxInAnyBrick;
...
typedef struct kaps_lsn_mc
{
    ...
    struct kaps_ix_chunk *m_ixInfo_specialEntry;        /* Special Entry entry ix_info */
...
/**
 * Parsing function to decode LSN prefixes starting at bin_offset into a human readable form. Number of prefixes to
 * parse is part of the input dump file.
 *
 * @param bin_fp File pointer for input warmboot dump file
 * @param txt_fp File pointer for output file in human readable format
 * @param lopoff_1 (RPT + IPT) lopoff in bits
 * @param bin_offset Starting offset in the input file from where prefix information begins
 *
 * @return KAPS_OK on success or an error code otherwise
 */
NlmErrNum_t kaps_lsn_mc_wb_dump_prefixes(
    FILE * bin_fp,
    FILE * txt_fp,
    uint32_t lopoff_1,
    uint32_t * bin_offset);
