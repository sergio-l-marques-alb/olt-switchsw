 ...
 */
    ...
    struct kaps_lpm_cr_entry_info
    {
        ...
        uint32_t ad_uda_address;  /**< Info for AD */
 ...
 */
    struct NlmFibStats
    {
        ...
        uint32_t numAllocatedIx;
                             /**< Number of allocated indexes (prefixes + holes)*/
        uint32_t numFreeIx;
...
#define WBPFXLIST_TO_ENTRY(ptr) CONTAINER_OF((ptr), struct kaps_wb_prefix_data, kaps_list_node);
    ...
    struct kaps_wb_apt_user_prefix
    {
        struct kaps_pfx_bundle *pfx_bundle;
        uintptr_t user_handle;
        uint32_t ad_user_handle;
...
#define WB_APT_PFXLIST_TO_ENTRY(ptr) CONTAINER_OF((ptr), struct kaps_wb_apt_user_prefix, kaps_list_node);

/**
 * API to read data from a ISSU dump File.
 *
 * @param fp File pointer from where the data is to be read
 * @param buffer Valid buffer memory to read the data to
 * @param size The number of bytes to read.
 * @param offset The offset in the file to read
 *
 * @retval 0 on success
 * @retval Nonzero on failure reading data.
 */
    int32_t kaps_lpm_wb_file_read(
    FILE * fp,
    uint8_t * buffer,
    uint32_t size,
    uint32_t offset);
...
/**
 *  API to return the location of the entry if it resides in the UDA
 *
 * @param in_db  Database Handle
 * @param in_e  user entry handle
 * @param abs_udc the UDC number is returned here
 * @param abs_row the row number in the UDC is returned here
 *
 * @return KAPS_OK on success or an error code
*/

    kaps_status kaps_lpm_db_get_entry_location(
