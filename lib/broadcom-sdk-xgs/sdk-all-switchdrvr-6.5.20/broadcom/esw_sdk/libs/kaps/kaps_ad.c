    ...
    uint32_t is_internal)
{
    ...
    {
        ad_db->byte_offset = 0;
        ad_db->device_bytes = 3;
    }
    else
    {
        ad_db->device_bytes = ad_db->db_info.width.ad_width_1 / KAPS_BITS_IN_BYTE;
...
{
    struct kaps_db *db = ad_db->db_info.common_info->ad_info.db;
    enum kaps_ad_type ad_type = kaps_resource_get_ad_type(&ad_db->db_info);
    struct kaps_entry *e = NULL;

    *device = ad_db->db_info.device;
    if (ad_type == KAPS_AD_TYPE_INDIRECTION)
    {
        *uda_address = ad->uda_address;
        return KAPS_OK;
    }

    if (ad->ref_count == 1)
    {
        e = (struct kaps_entry *) ad->entries;
    }
    ...
    KAPS_STRY(db->fn_table->get_ad(db, e, ad, uda_address, device));
    return KAPS_OK;
}
    ...
    uint8_t * value)
{
    ...
    else
    {
        ...
        if (ad_db->db_info.device->type == KAPS_DEVICE_KAPS)
            return KAPS_UNSUPPORTED;

        KAPS_WB_HANDLE_READ_LOC((&ad_db->db_info), (&ad_handle), (uintptr_t) usr_ad_handle);

        if (ad_handle->ref_count == 0)
        {
            /*
             * It is dangling AD 
             */
        ...
        status = kaps_ad_get_info(ad_db, ad_handle, &uda_addr, &device);
        KAPS_TX_ERROR_CHECK(ad_db->db_info.device, status);
        if (status != KAPS_OK)
            return status;

        {
            if (ad_db->byte_offset == 0)
            {
                value[0] = (uda_addr >> 16) & 0xFF;
                value[1] = (uda_addr >> 8) & 0xFF;
                value[2] = (uda_addr) & 0xFF;
            }
            else if (ad_db->byte_offset == 1)
            {
                value[0] = (uda_addr >> 8) & 0xFF;
                value[1] = (uda_addr) & 0xFF;
            }
            else if (ad_db->byte_offset == 2)
            {
                value[0] = (uda_addr) & 0xFF;
            }
            else
            {
                kaps_sassert(0);
    ...
    uint32_t new_width)
{
    ...
    ad_db->device_bytes = new_width / KAPS_BITS_IN_BYTE;
    ad_db->byte_offset = ad_db->device_bytes - ad_db->user_bytes;
    return KAPS_OK;
}
