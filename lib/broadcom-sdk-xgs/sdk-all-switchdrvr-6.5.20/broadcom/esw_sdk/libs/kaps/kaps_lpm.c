    ...
    struct kaps_db *in_db)
{
    ...
    if (db->db_info.hw_res.db_res->index_mgr)
    {
        kaps_sassert(db->db_info.hw_res.db_res->index_mgr->ref_count);

        db->db_info.hw_res.db_res->index_mgr->ref_count--;

        if (!db->db_info.hw_res.db_res->index_mgr->ref_count)
        {
            kaps_ix_mgr_destroy(db->db_info.hw_res.db_res->index_mgr);
        }
        else
        {
            db->db_info.common_ad_owner_db = NULL;
        }
    }

    if (db->db_info.hw_res.db_res->cascaded_index_mgr && db->db_info.common_info->ad_info.ad)
    {
        kaps_sassert(db->db_info.hw_res.db_res->cascaded_index_mgr->ref_count);

        db->db_info.hw_res.db_res->cascaded_index_mgr->ref_count--;

        if (!db->db_info.hw_res.db_res->cascaded_index_mgr->ref_count)
        {
            kaps_ix_mgr_destroy(db->db_info.hw_res.db_res->cascaded_index_mgr);
        }
    }
    ...
    struct kaps_device *this_device)
{
    ...
    kaps_status status;
    uint32_t index_range_low, index_range_high;
    ...
    while ((el = kaps_c_list_iter_next(&it)) != NULL)
    {
        ...
        index_range_low = parent->common_info->index_range_min;
        index_range_high = parent->common_info->index_range_max;

        if (parent->common_info->ad_info.ad)
        {
            index_range_low = 0;
            index_range_high = 0;
        }

        if (index_range_low == 0 && index_range_high == 0)
            index_range_high = KAPS_FIB_MAX_INDEX_RANGE;
        ...
        status = kaps_ix_mgr_init(main_device, index_range_low, index_range_high, 1024,
                                  &parent->hw_res.db_res->index_mgr,
        ...
        if (parent->common_info->is_cascaded)
        {
            if (parent->common_info->ad_info.ad)
            {
                ...
                status = kaps_ix_mgr_init(main_device->other_core, index_range_low, index_range_high, 1024,
                                          &parent->hw_res.db_res->cascaded_index_mgr,
                                          (struct kaps_ad_db *) parent->common_info->ad_info.ad,
                                          main_device->issu_status == KAPS_ISSU_INIT ? 0 : 1);
            ...
            {
                parent->hw_res.db_res->cascaded_index_mgr = parent->hw_res.db_res->index_mgr;
            }
        }
        ...
        if (status != KAPS_OK)
        {
            ...
            for (; tmp_db; tmp_db = (struct kaps_lpm_db *) tmp_db->db_info.next_tab)
            {
                ...
                while ((el = kaps_c_list_iter_next(&it)) != NULL)
                {
                     ...
                     */
                    entry->pfx_bundle->m_isAptLmpsofarPfx = 0;
    ...
    uint32_t * nv_offset)
{
    ...
    *nv_offset += len_8;
    ...
    {
        struct kaps_ad_db *ad_db;
        uint32_t uda_address = entry->ad_handle->uda_address, ad_user_hdl = entry->ad_handle->user_handle;
        ...
        *nv_offset += sizeof(ad_user_hdl);
        ...
        if (0 != write_fn(handle, (uint8_t *) & (uda_address), sizeof(uint32_t), *nv_offset))
            return NLMERR_FAIL;
    ...
    while ((e1 = kaps_c_list_iter_next(&it)) != NULL)
    {
        ...
        if (ad_db)
        {
            ...
            if (!ad_p)
            {
                ...
                {
                    uint32_t base_address, usb_num, ad_location;

                    usb_num = (pfx->ad_uda_address)
                        / (((KAPS_HW_MAX_UDA_SB_SIZE_MB * 1024 * 1024) / KAPS_BITS_IN_BYTE) / KAPS_HW_MIN_UDA_WIDTH_8);
                    base_address = usb_num
                        * (((KAPS_HW_MAX_UDA_SB_SIZE_MB * 1024 * 1024) / KAPS_BITS_IN_BYTE) / KAPS_HW_MIN_UDA_WIDTH_8);
                    ad_location = ((pfx->ad_uda_address) - base_address)
                        / (ad_db->device_bytes / KAPS_HW_MIN_UDA_WIDTH_8);
                    kaps_sassert(kaps_get_bit(ad_db->fbmap[usb_num], ad_location) == 1);
                    kaps_reset_bit(ad_db->fbmap[usb_num], ad_location);
                }

                KAPS_STRY(kaps_ad_db_add_entry_internal(ad_db, pfx->ad, &ad_p, pfx->ad_user_handle));
                ad_p->uda_address = pfx->ad_uda_address;
            }
    ...
    *nv_offset += len_8;
    ...
    if (db->common_info->ad_info.ad)
    {
        ...
        *nv_offset += sizeof(prefix->ad_user_handle);
        ...
        if (0 != read_fn(handle, (uint8_t *) & (prefix->ad_uda_address), sizeof(prefix->ad_uda_address), *nv_offset))
            return NLMERR_FAIL;
        *nv_offset += sizeof(prefix->ad_uda_address);
    }
    ...
    struct kaps_wb_cb_functions *cb_fun)
{
    ...
    *cb_fun->nv_offset += sizeof(*defer_deletes);
    ...
    KAPS_STRY(kaps_ix_mgr_wb_finalize(db->hw_res.db_res->index_mgr));
    ...
    struct kaps_device * device)
{
    ...
    while ((e1 = kaps_c_list_iter_next(&it)) != NULL)
    {
        ...
        KAPS_STRY(kaps_ix_mgr_wb_finalize(db->hw_res.db_res->index_mgr));
        if (db->hw_res.db_res->cascaded_index_mgr)
            KAPS_STRY(kaps_ix_mgr_wb_finalize(db->hw_res.db_res->cascaded_index_mgr));
    }
    ...
    uint32_t * nv_offset)
{
    uintptr_t handle;
    uint32_t index;
    uint16_t len_1, len_8, iter;
    uint8_t prefix[KAPS_LPM_KEY_MAX_WIDTH_8];

    kaps_lpm_wb_file_read(bin_fp, (uint8_t *) & len_1, sizeof(len_1), *nv_offset);
    *nv_offset += sizeof(len_1);

    kaps_lpm_wb_file_read(bin_fp, (uint8_t *) & index, sizeof(index), *nv_offset);
    *nv_offset += sizeof(index);

    kaps_lpm_wb_file_read(bin_fp, (uint8_t *) & handle, sizeof(handle), *nv_offset);
    *nv_offset += sizeof(handle);

    len_8 = (len_1 + 7) / 8;
    kaps_lpm_wb_file_read(bin_fp, prefix, len_8, *nv_offset);
    *nv_offset += len_8;
    ...
    for (iter = 0; iter < len_8; iter++)
    {
        ...
        kaps_fprintf(txt_fp, "%d", prefix[iter]);
    }
...
kaps_status
kaps_lpm_wb_parse_wb_file(
    FILE * bin_fp,
    FILE * txt_fp,
    uint32_t * nv_offset)
{
    uint32_t addr, pending, num_dbs, num_tables;
    uint8_t is_b_silicon = 0;

    kaps_trie_wb_dump_pools(bin_fp, txt_fp, nv_offset);

    kaps_lpm_wb_file_read(bin_fp, &is_b_silicon, sizeof(is_b_silicon), *nv_offset);
    *nv_offset += sizeof(is_b_silicon);

    if (!is_b_silicon)
    {
        kaps_lpm_wb_file_read(bin_fp, (uint8_t *) & addr, sizeof(addr), *nv_offset);
        *nv_offset += sizeof(addr);

        kaps_fprintf(txt_fp, "\t Rev-A Default RPT is stored at addr = %d\n", addr);
    }

    kaps_lpm_wb_file_read(bin_fp, (uint8_t *) & num_dbs, sizeof(num_dbs), *nv_offset);
    *nv_offset += sizeof(num_dbs);

    /*
     * Go over each database, print out the info 
     */
    while (num_dbs--)
    {
        KAPS_STRY(kaps_trie_wb_dump_trie(bin_fp, txt_fp, is_b_silicon, nv_offset));

        /*
         * Print the pending prefixes if exist here 
         */
        kaps_lpm_wb_file_read(bin_fp, (uint8_t *) & pending, sizeof(pending), *nv_offset);
        *nv_offset += sizeof(pending);

        while (pending--)
        {
            kaps_lpm_wb_dump_pfx_bundle(bin_fp, txt_fp, nv_offset);
        }

        /*
         * Print table info if any here 
         */
        kaps_lpm_wb_file_read(bin_fp, (uint8_t *) & num_tables, sizeof(num_tables), *nv_offset);
        *nv_offset += sizeof(num_tables);
    ...
    struct kaps_db_stats *user_db_stats)
{
    ...
    else
    {
        ...
        {
            kaps_printf(" RPT = %d, PCT = %d,", fib_stats_ptr->numRPTWrites, fib_stats_ptr->numRITWrites);
            kaps_printf(" IPT = %d, IIT = %d,", fib_stats_ptr->numIPTWrites, fib_stats_ptr->numIITWrites);
            kaps_printf(" APT = %d, AIT = %d\n", fib_stats_ptr->numAPTWrites, fib_stats_ptr->numAITWrites);
            kaps_printf("LSN_WRITES   : RMW = %d, SBC = %d, LSNCLR = %d, Metadata = %d\n", fib_stats_ptr->numLSNRmw,
                        fib_stats_ptr->numLSNSbc, fib_stats_ptr->numLSNClr, fib_stats_ptr->numMetaDataWrite);

            kaps_printf("LPU STATS    : ");
            for (lpu_iter = 0; lpu_iter < KAPS_HW_MAX_LPUS_PER_LSN; lpu_iter++)
                kaps_printf(" %7d#", lpu_iter);

            kaps_printf("\nLSNSBC LPUs  : ");
            for (lpu_iter = 0; lpu_iter < KAPS_HW_MAX_LPUS_PER_LSN; lpu_iter++)
                kaps_printf("%8d ", fib_stats_ptr->sbc_lpus[lpu_iter]);

            kaps_printf("\nLSNCLR LPUs  : ");
            for (lpu_iter = 0; lpu_iter < KAPS_HW_MAX_LPUS_PER_LSN; lpu_iter++)
                kaps_printf("%8d ", fib_stats_ptr->clr_lpus[lpu_iter]);

            kaps_printf("\nLSNRMW LPUs  : ");
            for (lpu_iter = 0; lpu_iter < (KAPS_HW_MAX_LPUS_PER_LSN * 8); lpu_iter++)
            {
                if (lpu_iter && ((lpu_iter % 8) == 0))
                    kaps_printf("\n             : ");
                kaps_printf("%8d ", fib_stats_ptr->rmw_lpus[lpu_iter]);
            }

            kaps_printf("\n");

            kaps_printf("IX STATS     : Num IX Compactions = %d, IX Allocated = %d, IX Free = %d \n",
                        fib_stats_ptr->numIXCompactionCalls, fib_stats_ptr->numAllocatedIx, fib_stats_ptr->numFreeIx);
        }

    }

    kaps_printf("\nAD_STATS     : Num AD Writes = %d \n", fib_stats_ptr->numADWrites);
    while (ad_db)
    {
        struct kaps_ix_mgr *kaps_ix_mgr = ad_db->mgr[ad_db->db_info.device->core_id];
        kaps_printf
            ("             : AD ID = %d, Width = %d, Entries = %d, Writes = %d IX Allocated = %d, IX Free = %d \n",
             ad_db->db_info.tid, ad_db->user_width_1, ad_db->ad_list.count, ad_db->num_writes,
             kaps_ix_mgr->num_allocated_ix, kaps_ix_mgr->size - kaps_ix_mgr->num_allocated_ix);
        ad_db = ad_db->next;
    }
    kaps_printf("\n");
    ...
    switch (property)
    {
        ...
        case KAPS_PROP_NUM_BBS:
            ...
            if (in_db->device->type != KAPS_DEVICE_KAPS)
                return KAPS_INVALID_ARGUMENT;
            if (value > (in_db->device->hw_res->total_num_bb / in_db->device->hw_res->num_chained_bb))
                return KAPS_INVALID_ARGUMENT;
            if (value < 0)
                return KAPS_INVALID_ARGUMENT;
...
static kaps_status
kaps_db_wb_parse_state(
    FILE * bin_fp,
    FILE * txt_fp,
    uint32_t * nv_offset)
{
    KAPS_STRY(kaps_lpm_wb_parse_wb_file(bin_fp, txt_fp, nv_offset));

    return KAPS_OK;
}

static kaps_status
kaps_db_wb_parse_info(
    struct kaps_db_wb_info *wb_info_p,
    FILE * bin_fp,
    FILE * txt_fp,
    uint32_t * nv_offset)
{
    struct kaps_db_wb_info wb_info;
    struct kaps_db_wb_info *wb_info_tables = NULL, *tmp;
    int16_t num_ab;
    int32_t uda_mb;
    uint32_t ix_max, ix_min;
    uint32_t tbl_no, clone_no, num_tables = 0;
    ...
    if (0 != kaps_lpm_wb_file_read(bin_fp, (uint8_t *) tmp, sizeof(struct kaps_db_wb_info), *nv_offset))
        return KAPS_NV_READ_WRITE_FAILED;
    *nv_offset += sizeof(struct kaps_db_wb_info);

#if 0
    if (tmp->desc)
    {
        tmp->description = (uint8_t *) kaps_sysmalloc(tmp->desc * sizeof(char));
        if (0 != kaps_lpm_wb_file_read(bin_fp, (uint8_t *) tmp->description, tmp->desc, *nv_offset))
            return KAPS_NV_READ_WRITE_FAILED;
        *nv_offset += tmp->desc;
    }
#endif

    KAPS_STRY(kaps_key_wb_read(&tmp->key, bin_fp, nv_offset));

    if (tmp->is_table || tmp->is_clone)
        return KAPS_OK;

    if (tmp->user_specified)
    {
        if (0 != kaps_lpm_wb_file_read(bin_fp, (uint8_t *) & num_ab, sizeof(num_ab), *nv_offset))
            return KAPS_NV_READ_WRITE_FAILED;
        *nv_offset += sizeof(num_ab);

        if (0 != kaps_lpm_wb_file_read(bin_fp, (uint8_t *) & uda_mb, sizeof(uda_mb), *nv_offset))
            return KAPS_NV_READ_WRITE_FAILED;
        *nv_offset += sizeof(uda_mb);
    }

    if (tmp->custom_ix_range)
    {
        if (0 != kaps_lpm_wb_file_read(bin_fp, (uint8_t *) & ix_min, sizeof(ix_min), *nv_offset))
            return KAPS_NV_READ_WRITE_FAILED;
        *nv_offset += sizeof(ix_min);

        if (0 != kaps_lpm_wb_file_read(bin_fp, (uint8_t *) & ix_max, sizeof(ix_max), *nv_offset))
            return KAPS_NV_READ_WRITE_FAILED;
        *nv_offset += sizeof(ix_max);
    }

    if (tmp->callback_data)
    {
        *nv_offset += sizeof(void *);
    ...
    if (tmp->has_tables || tmp->has_clones)
    {
        ...
        if (0 != kaps_lpm_wb_file_read(bin_fp, (uint8_t *) & num_tables, sizeof(num_tables), *nv_offset))
            return KAPS_NV_READ_WRITE_FAILED;
        *nv_offset += sizeof(num_tables);

        wb_info_tables = (struct kaps_db_wb_info *) kaps_sysmalloc(sizeof(struct kaps_db_wb_info) * num_tables);

        for (tbl_no = 0; tbl_no < num_tables; tbl_no++)
        {
            KAPS_STRY(kaps_db_wb_parse_info(&wb_info_tables[tbl_no], bin_fp, txt_fp, nv_offset));
        }
    }

    if (tmp->type == KAPS_DB_LPM)
        kaps_fprintf(txt_fp, "\nLPM \n");
    else if (tmp->type == KAPS_DB_ACL)
        kaps_fprintf(txt_fp, "\nACL \n");
    kaps_fprintf(txt_fp, "DB Id: %d ptr %u\n", tmp->tid, tmp->stale_ptr);
    kaps_fprintf(txt_fp, "Capacity: %d\n", tmp->capacity);

    if (tmp->user_specified)
    {
        kaps_fprintf(txt_fp, "DBA: %d blocks\n", num_ab);
        kaps_fprintf(txt_fp, "UDA: %d Mb\n", uda_mb);
    }

    if (tmp->custom_ix_range)
    {
        kaps_fprintf(txt_fp, "IX Range : %d - %d\n", ix_min, ix_max);
    }

    kaps_key_wb_print(tmp->key, txt_fp);
    kaps_key_wb_free(tmp->key);

    if (tmp->has_tables || tmp->has_clones)
    {
        for (tbl_no = 0; tbl_no < num_tables; tbl_no++)
        {
            if (wb_info_tables[tbl_no].is_table)
            {
                kaps_fprintf(txt_fp, "\tTable Id: %d ptr %u\n", wb_info_tables[tbl_no].tid,
                             wb_info_tables[tbl_no].stale_ptr);
                kaps_key_wb_print(wb_info_tables[tbl_no].key, txt_fp);
                kaps_key_wb_free(wb_info_tables[tbl_no].key);
                for (clone_no = 0; clone_no < num_tables; clone_no++)
                {
                    if (wb_info_tables[clone_no].is_clone
                        && wb_info_tables[clone_no].clone_of == wb_info_tables[tbl_no].stale_ptr)
                    {
                        kaps_fprintf(txt_fp, "\t\tClone Id: %d ptr %u\n", wb_info_tables[clone_no].tid,
                                     wb_info_tables[clone_no].stale_ptr);
                        kaps_key_wb_print(wb_info_tables[clone_no].key, txt_fp);
                        kaps_key_wb_free(wb_info_tables[clone_no].key);
                    }
                }
            }
        }
        ...
        for (clone_no = 0; clone_no < num_tables; clone_no++)
        {
            if (wb_info_tables[clone_no].is_clone && wb_info_tables[clone_no].clone_of == tmp->stale_ptr)
            {
                kaps_fprintf(txt_fp, "\tClone Id: %d ptr %u\n", wb_info_tables[clone_no].tid,
                             wb_info_tables[clone_no].stale_ptr);
                kaps_key_wb_print(wb_info_tables[clone_no].key, txt_fp);
    ...
    if (wb_info_tables)
        kaps_sysfree(wb_info_tables);

    return KAPS_OK;
}

static kaps_status
kaps_wb_parse_device_info(
    FILE * bin_fp,
    FILE * txt_fp,
    uint32_t * nv_offset)
{
    uint32_t db_no, num_db = 0;
    uint32_t inst_no, num_inst = 0;

    if (0 != kaps_device_wb_file_read(bin_fp, (uint8_t *) & num_db, sizeof(num_db), *nv_offset))
        return KAPS_NV_READ_WRITE_FAILED;
    *nv_offset += sizeof(num_db);

    kaps_fprintf(txt_fp, " \t --==Databases==-- \n\n");
    for (db_no = 0; db_no < num_db; db_no++)
    {
        KAPS_STRY(kaps_db_wb_parse_info(NULL, bin_fp, txt_fp, nv_offset));
    }
    kaps_fprintf(txt_fp, "\n \t --== ==-- \n");

    if (0 != kaps_device_wb_file_read(bin_fp, (uint8_t *) & num_inst, sizeof(num_inst), *nv_offset))
        return KAPS_NV_READ_WRITE_FAILED;
    *nv_offset += sizeof(num_inst);

    kaps_fprintf(txt_fp, " \t --==Instructions==-- \n\n");
    for (inst_no = 0; inst_no < num_inst; inst_no++)
    {
        KAPS_STRY(kaps_instruction_wb_print(bin_fp, txt_fp, nv_offset));
    }
    kaps_fprintf(txt_fp, "\n \t --== ==-- \n");
...
kaps_status
kaps_device_parse_state(
    FILE * bin_fp,
    FILE * txt_fp)
{
    uint32_t nv_offset = 0;
    uint32_t nv_size = 0, nv_size_crc = 0, nv_data_crc = 0, check_crc = 0;
    uint32_t byte_num;
    uint8_t nv_data;
    ...
    if (0 != kaps_device_wb_file_read(bin_fp, (uint8_t *) & nv_size, sizeof(nv_size), nv_offset))
        return KAPS_NV_READ_WRITE_FAILED;
     ...
     */
    if (0 != kaps_device_wb_file_read(bin_fp, (uint8_t *) & nv_size_crc, sizeof(nv_size_crc), nv_offset))
        return KAPS_NV_READ_WRITE_FAILED;
    ...
    for (byte_num = sizeof(nv_size) + sizeof(nv_size_crc); byte_num < nv_size; byte_num++)
    {
        if (0 != kaps_device_wb_file_read(bin_fp, (uint8_t *) & nv_data, sizeof(nv_data), byte_num))
            return KAPS_NV_READ_WRITE_FAILED;
        check_crc = kaps_crc32(check_crc, (uint8_t *) & nv_data, sizeof(nv_data));
    }

    if (0 != kaps_device_wb_file_read(bin_fp, (uint8_t *) & nv_data_crc, sizeof(nv_data_crc), nv_size))
        return KAPS_NV_READ_WRITE_FAILED;

    if (check_crc != nv_data_crc)
        return KAPS_NV_DATA_CORRUPT;

    KAPS_STRY(kaps_wb_parse_device_info(bin_fp, txt_fp, &nv_offset));

    KAPS_STRY(kaps_db_wb_parse_state(bin_fp, txt_fp, &nv_offset));
...
kaps_status
kaps_lpm_db_get_entry_location(
    struct kaps_db * in_db,
    struct kaps_entry * in_e,
    uint32_t * location)
{
    ...
    struct kaps_lpm_entry *lpm_entry = NULL;
    uint32_t abs_udc = 256, abs_row = 0xEFFFFFFF;       /* initialized to invalid value*/

    if (in_db == NULL || in_e == NULL)
        return KAPS_INVALID_ARGUMENT;

    KAPS_WB_HANDLE_READ_LOC(KAPS_GET_DB_PARENT(in_db), (&entry_p), (uintptr_t) in_e);

    lpm_entry = (struct kaps_lpm_entry *) entry_p;

    err_num = kaps_ftm_get_prefix_location(lpm_db->fib_tbl, lpm_entry, &abs_udc, &abs_row, &reason_code);
    if (err_num != NLMERR_OK)
        return kaps_map_reason_code_to_kaps_status(reason_code);

    *location = abs_row * 16 + abs_udc;
    return KAPS_OK;
}
    ...
    FILE * fp)
{
    ...
    while ((el = kaps_c_list_iter_next(&it)) != NULL)
    {
        ...
        if (tmp->type == KAPS_DB_LPM)
        {
            ...
            if (db->db_info.hw_res.db_res->index_mgr)
            {
                struct kaps_ix_mgr *index_mgr = db->db_info.hw_res.db_res->index_mgr;
                POOL_STATS(kaps_ix_chunk, &index_mgr->ix_chunk_pool, fp);
            }
        }
    }
    ...
    struct kaps_db ** dbp)
{
    ...
    if (db->db_info.type == KAPS_DB_ACL)
    {
        /*
         * The database type got switched below us,
         * do not do anything else below
         */
    ...
    fn_impl->db_fix_uda_location = kaps_lpm_db_fix_uda_location;
    ...
    struct large_kaps2_stats *stats)
{
     ...
     * AD2 
     */
    if (fibTbl->m_db->is_type_a)
    {
        stats->total_ad2_rows = device->hw_res->ads2_depth_a;
        stats->num_ad2_rows_used =
            device->hw_res->ads2_depth_a - device->kaps_shadow->ads2_overlay[0].num_free_it_slots;
    }
    else
    {
        stats->total_ad2_rows = device->hw_res->ads2_depth_b;
        stats->num_ad2_rows_used =
            device->hw_res->ads2_depth_b - device->kaps_shadow->ads2_overlay[1].num_free_it_slots;
    }
