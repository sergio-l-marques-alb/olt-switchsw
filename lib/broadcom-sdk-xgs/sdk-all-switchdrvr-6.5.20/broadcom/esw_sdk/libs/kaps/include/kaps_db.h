 ...
 *
 * Properties can be read with  call to kaps_entry_get_info.
 */


enum kaps_db_entry_meta_priority {
    KAPS_ENTRY_META_PRIORITY_0 = 0, /* Highest Priority */
    KAPS_ENTRY_META_PRIORITY_1 = 1,
    KAPS_ENTRY_META_PRIORITY_2 = 2,
    KAPS_ENTRY_META_PRIORITY_3 = 3  /* Lowest Priority */
};
 ...
 * KAPS_PROP_DESCRIPTION can be set for AD databases) :
 *
 *- kaps_db_set_property()
...
enum kaps_db_properties {
    KAPS_PROP_USE_MCOR,                /**< For ACL databases, use MCOR for the range at the specified offset. */
    KAPS_PROP_MCOR_VALUE,              /**< For ACL databases, add a most commonly occurring range. */
    KAPS_PROP_ALGORITHMIC,             /**< Indicates whether the database should be algorithmic or non-algorithmic. */
    KAPS_PROP_INDEX_RANGE,             /**< Index range for user-side AD management. */
    KAPS_PROP_INDEX_CALLBACK,          /**< Register callback function to notify users when entries are moved. */
    KAPS_PROP_MIN_PRIORITY,            /**< Hint on minimum priority used in the database. (Helps reduce shuffles.) */
    KAPS_PROP_DESCRIPTION,             /**< String description for device/databases. */
    KAPS_PROP_CASCADE_DEVICE,          /**< Request this database be placed on specific cascaded KBP device. Valid only when cascade is detected */
    KAPS_PROP_REDUCED_INDEX_CALLBACKS, /**< If set, reduces Index callbacks. */
    KAPS_PROP_SAVE_IX_SPACE,           /**< Saves the index space for LPM with external AD */
    KAPS_PROP_DMA_TAG,                 /**< DMA Tag for the database, KAPS specific. */
    KAPS_PROP_ENTRY_META_PRIORITY,     /**< Default Meta priority for the entries added to the DB. */
    KAPS_PROP_ENABLE_DB_COMPACTION,    /**< enable db compaction */
    KAPS_PROP_MAX_CAPACITY,            /**< sets the maximum allowed capacity of a DB */
    KAPS_PROP_AGE_COUNT,               /**< The count after which the entry is considered for aging */
    KAPS_PROP_PAIR_WITH_DB,            /**< This db is paired with other db sharing same DBA resource */
    KAPS_PROP_DEFER_DELETES,           /**< Defer deletes to install */
    KAPS_PROP_SCALE_UP_CAPACITY,       /**< Scale up the DB capacity by vertically partitioning the DBA blocks */
    KAPS_PROP_LOG,                     /**< Log the entry add, delete operations into a file */
    KAPS_PROP_MAX_PRIORITY,            /**< Hint on maximum priority used in the database. (Helps reduce shuffles.) */
    KAPS_PROP_SEND_PRIORITY_OVER_AD,   /**< Send the priority as part of AD */
    KAPS_PROP_ENABLE_COUNTERS,         /**< To enable search-dependent-counters on a database */
    KAPS_PROP_COUNTER_CYCLE,           /**< Whether to use 1 cycle counter or two cycle */
    KAPS_PROP_COUNTER_TYPE,            /**< Counter type, 32b Compressed OR 64b Full Width */
    KAPS_PROP_XL_DB,                   /**< specifies whether the DB is extra large */
    KAPS_PROP_LOCALITY,                /**< specifies which locality DB belongs to */
    KAPS_PROP_REPLICATE_DB,            /**< specifies whether to replicate a DB for multi thread access */
    KAPS_PROP_MC_DB,                   /**< specifies multicast database */
    KAPS_PROP_ENABLE_DYNAMIC_ALLOCATION,/**< enable dynamic allocation of DBA and UDA */
    KAPS_PROP_UTILISE_UDA_HOLES,        /**< Allocate UDA space for LSN, utilizing existing holes in UDC */
    KAPS_PROP_NUM_LEVELS_IN_DB,         /**< Number of levels present in this DB. Read only property. Should be queried after device lock*/
    KAPS_PROP_INVALID                  /**< This should be the last entry */
};
...
enum kaps_entry_properties {
    KAPS_ENTRY_PROP_META_PRIORITY, /**< For LPM databases, sets the entry's meta priority. */
    KAPS_ENTRY_PROP_INVALID        /**< This should be the last entry */
};
 ...
 * databases or tables using the API call kaps_db_add_table(). A key
 ...
 * @param type The database type. Can be ACL, LPM, or EM. ::kaps_db_type.
...
* using the above API kaps_db_init with the
 ...
 * @param resource Hardware resource defined by ::kaps_hw_resource.
 ...
 * kaps_key_add_field(). This information
...
/**
 * Sets properties for the database. Available properties are defined by the enumeration ::kaps_db_properties. The properties
 * should be followed by values within the ranges given below.
 *
 * |PROPERTY|VALUES|DEFAULT|
 * |:---:|:---:|:---:|
 * |KAPS_PROP_MCOR_VALUE| 16-bit range value, that frequently occurs for ports | None (future use) |
 * |KAPS_PROP_USE_MCOR| The range number starting from MSB of key specified by database. Count starts from zero | None (future use) |
 * |KAPS_PROP_INDEX_CALLBACK| The callback function | None. No callback is performed |
 * |KAPS_PROP_DESCRIPTION|String description|None|
 * |KAPS_PROP_ALGORITHMIC|1, 0|Algorithmic. Passing zero disables algorithmic processing|
 * |KAPS_PROP_REDUCED_INDEX_CALLBACKS|1, 0|Reduced callbacks. Passing one enables reduced callbacks mode|
 *
 * @param db Valid initialized KBP database.
 * @param property ::kaps_db_properties.
 ...
 * the key tuples must be of the type KAPS_KEY_FIELD_TABLE_ID as described
 * in ::kaps_key_field_type. This tuple must occur in all the keys at the
 * same offset.
 *
 * @param db Initialized KBP database.
...
/**
 * This is same as the kaps_db_add_prefix API, but it takes entry index as input
...
/**
 * This is same as the kaps_db_add_em API, but it takes entry index as input
 * which, user can treat as opaque entry handle upon return from API.
 * The index value should be between 1 and 27b max.
 *
 * @param db Initialized KBP database.
 ...
 *
 * @return KAPS_OK on successful deletion of entry or an error code otherwise.
 */

kaps_status kaps_db_delete_entry(struct kaps_db *db, struct kaps_entry *entry);

/**
 * Sets properties for the entry. Available properties are defined by the enumeration ::kaps_entry_properties. The properties
 * should be followed by values within the ranges given below.
 *
 * |PROPERTY|VALUES|DEFAULT|
 * |:---:|:---:|:---:|
 * |KAPS_ENTRY_PROP_META_PRIORITY| items in enum kaps_db_entry_meta_priority | Meta priority set for the db. KAPS_ENTRY_META_PRIORITY_3 (lowest priority) if none set for db |
 * This API is applicable only for entries in LPM databases.
 *
 * @param db Initialized KBP database.
 * @param entry Valid LPM entry.
 * @param property ::kaps_entry_properties.
 * @param ... The required number of arguments for the property.
 *
 * @return KAPS_OK on success or an error code otherwise.
 */
 ...
 *
 * @param db Initialized KBP database.
 * @param entry Valid handle returned using the call kaps_db_add_ace().
 ...
 *
 * @param db Initialized KBP database.
 * @param entry Valid handle returned using the call kaps_db_add_ace(), kaps_db_add_prefix() or kaps_db_add_em().
 ...
 *
 * @param db Initialized KBP database.
 * @param entry Valid handle returned using the call kaps_db_add_prefix(), kaps_db_add_ace() or their with_index versions
 * @param fp The file to print to.
 *
 * @return KAPS_OK on success or an error code otherwise.
 */
 ...
 *
 * kaps_db_set_property(db, KAPS_PROP_INDEX_CALLBACK, <function_pointer>, handle);
 *
 ...
 *
 * @retval KAPS_PREFIX_NOT_FOUND when entry cannot be found
 * @retval KAPS_OK on success, and entry pointer is valid
 */
 ...
 * @param entry Returned on a successful match. Should be referenced only when KAPS_OK is returned. NULL entry means no hit.
 * @param index The hit index for the entry on a match. Should be referenced only when KAPS_OK is returned. -1 means no match.
 ...
 * must be freed using the API kaps_entry_free_index_array().
 *
 * @param db Valid database handle.
 * @param entry Valid KBP entry handle.
...
/**
 * Frees the index array returned by the API call kaps_entry_get_index().
 *
 * @param db Valid database handle.
 * @param indices Array of indexes returned by API kaps_entry_get_index().
 *
 * @return KAPS_OK on success or an error code otherwise.
 */
 ...
 * by the API kaps_entry_get_info().
 */
...
struct kaps_entry_info
{
    ...
    uint8_t data[KAPS_HW_MAX_DBA_WIDTH_8]; /**< The data portion of the entry, which is width_8 bytes wide. */
    uint8_t mask[KAPS_HW_MAX_DBA_WIDTH_8]; /**< The mask portion of the entry, which is width_8 bytes wide. */
    struct kaps_ad *ad_handle; /**< Used to return the AD handle if present or, if not, is set to NULL. */
    struct kaps_ad_db *ad_db; /**< Used to return the AD DB pointer if present or, if not, is set to NULL. */
