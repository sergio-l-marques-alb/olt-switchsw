...
/*@@kaps_nlm_allocator Module
   ...
   accept a pointer to a kaps_nlm_allocator which can be configured for a
   particular system. The default implementation uses the malloc/free
   type allocation strategy.

   Using the kaps_nlm_allocator, it is also possible to change memory allocation
   strategies for portions of the API or easily replace the standard version
   of the allocator with a debug version of the allocator. The kaps_nlm_allocator
   provides the mechanism to accomplish such replacements.

   The data structure for kaps_nlm_allocator also provides a member variable
...
typedef struct kaps_nlm_allocator_vtbl {
    ...
    kaps_nlm_allocator_alloc_t m_malloc;   /* Pointer to malloc function */
    kaps_nlm_allocator_calloc_t m_calloc;  /* Pointer to calloc function */
    kaps_nlm_allocator_resize_t m_resize;  /* Pointer to resize function */
    kaps_nlm_allocator_free_t m_free;      /* Pointer to free function */
    kaps_nlm_allocator_dtor_t m_dtor;      /* Pointer to destructor function */
    const char *className1;     /* Name of this allocator */
    kaps_nlm_allocator_sys_malloc_t m_sysMalloc;    /* Pointer to the system Malloc function */
    kaps_nlm_allocator_sys_free_t m_sysFree;        /* Pointer to the system Free function */
} kaps_nlm_allocator_vtbl;

/* Type definition for a kaps_nlm_allocator.
...
struct kaps_nlm_allocator {
    kaps_nlm_allocator_vtbl m_vtbl;        /* Function table for basic allocation
                                           Note: m_vtbl is the contents, not the pointer */
    kaps_nlm_allocator_vtbl *m_vtbl_p;     /* Pointer to above to make some macros work transparently */
...
/* Summary
   Initialize a kaps_nlm_allocator.
...
/* Summary
   ...
   Description
   Uses kaps_nlm_allocator_malloc() to obtain a memory chunk of size newSize.
