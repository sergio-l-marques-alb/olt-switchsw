...
/**
 * Initializes the memory map for the OP Algo device
 *
 * @param device valid device handle whose memory map should be initialized
 */
    kaps_status initialize_op_mem_map(
    struct kaps_device *device);

/*op2 specific for ad index translation*/
    kaps_status uda_mgr_release_ad_trans_table(
    struct kaps_device *device,
    uint32_t usb_index);
/*op2 specific for ad index translation*/
    kaps_status resource_populate_ad_trans_table(
    struct kaps_device *device,
    uint32_t base_index,
    uint32_t usb_index,
    uint32_t ad_size);
...
/**
 * Assign and fit resources for 12K
 *
 * @param device valid device handle
 */

    kaps_status process_12k(
    struct kaps_device *device);

/**
 * @ingroup KAPS_RM
 * @{
 * Assign and fit resources for Optimus Prime.
 * @param device valid device handle
 * @}
 */
    kaps_status process_op(
    struct kaps_device *device);

/**
 * Dynamically assigns and fit AD resources for 12K
 *
 * @param ad valid ad db handle
 * @param sb_bitmap pointer to AD SB bitmap
 * @num_sb_needed number of UDA super blocks needed
 */
    kaps_status kaps_resource_dynamic_ad_alloc_12k(
    struct kaps_db *ad,
    uint8_t * sb_bitmap,
    int32_t num_sb_needed);

/**
 * Frees the indicated SB from DB to resource manager for 12k.
 *
 * @param ad valid ad db handle
 * @param sb_bitmap pointer to AD SB bitmap
 * @uda_sb_no index of the super block to be freed
 */
    kaps_status kaps_resource_dynamic_ad_free_12k(
    struct kaps_db *ad,
    uint8_t * sb_bitmap,
    int32_t uda_sb_no);

/**
 * Dynamically assigns and fit AD resources for OP
 *
 * @param ad valid ad db handle
 * @param sb_bitmap pointer to AD SB bitmap
 * @num_sb_needed number of UDA super blocks needed
 */
    kaps_status kaps_resource_dynamic_ad_alloc_op(
    struct kaps_db *ad,
    int32_t num_sb_needed);

/**
 * Frees the indicated SB from DB to resource manager for OP.
 *
 * @param ad valid ad db handle
 * @param sb_bitmap pointer to AD SB bitmap
 * @uda_sb_no index of the super block to be freed
 */
    kaps_status kaps_resource_dynamic_ad_free_op(
    struct kaps_db *ad,
    uint8_t * sb_bitmap,
    int32_t uda_sb_no);
 ...
 * @param start_lpu start LPU no of the dynamically allocated region will be returned here
 ...
 */
    ...
    kaps_status resource_op_dynamic_uda_alloc_n_lpu(
    struct kaps_device *device,
    struct kaps_db *db,
    int8_t dt_index,
 ...
 */
    ...
    kaps_status kaps_resource_find_ab_kaps(
    struct kaps_device *device,
    int32_t ab_no,
    struct kaps_ab_info **ab);

/**
 * Prints the assigned power budget of all the DBs
 * Required only for debugging and unit testing
 *
 * @param device valid device handle
 *
 */
    void resource_kaps_print_dba_power_budget(
    struct kaps_device *device);
