    ...
    int32_t core_id)
{
    ...
    switch (type)
    {
        ...
        case KAPS_DEVICE_KAPS:
            ...
            device->num_80b_entries_ab = 4096;
            ...
            if (device->id == KAPS_QUX_DEVICE_ID)
            {
                ...
                device->num_dba_sb = 2;
                device->max_num_searches = 2;
                device->num_80b_entries_ab = 1024;
            ...
            if (device->id == KAPS_JERICHO_2_DEVICE_ID)
            {
                ...
                device->num_80b_entries_ab = 2 * 1024;
            ...
            device->num_active_ab_per_sb = device->num_ab_per_sb;
            device->dba_offset = 1;
            break;
    ...
    struct kaps_device * cascade_device)
{
    ...
    while (cascade_device)
    {
        ...
        while (bc_device)
        {
            ...
            if (device->next_op2_device)
            {
                device = device->next_op2_device;
    ...
    ...)
{
    ...
    switch (property)
    {
        ...
        case KAPS_DEVICE_PROP_CLR_DEV_ERR_BITS:
        {
            val = va_arg(a_list, int32_t);
            KAPS_TRACE_PRINT("%d", val);
            if (val < 0 || val > 1)
            ...
            for (; dev; dev = dev->next_dev)
            {
                dev->clear_ESR_err_bits = val;
                if (dev->smt)
                    dev->smt->clear_ESR_err_bits = val;
            }
        ...
        case KAPS_DEVICE_PROP_CLOCK_RATE:
        {
            val = va_arg(a_list, int32_t);
            KAPS_TRACE_PRINT("%d", val);
            if (val < 0 || val > 1)
            ...
            dev->op2_clock_rate = val;
            if (dev->other_core)
                device->other_core->op2_clock_rate = val;
        ...
        case KAPS_DEVICE_PROP_COUNTER_COMPRESSION:
        {
            ...
            device->is_counter_scrub_thread = 1;
            break;
        }
    ...
    uint8_t *bytes)
{
    ...
    if (KAPS_RPB_BLOCK_START <= blk_nr && blk_nr <= KAPS2_RPB_BLOCK_END && func_num == KAPS_FUNC1)
    {
        ...
        kaps_assert(row_nr < device->num_80b_entries_ab, "Incorrect RPB Row Nr \n");
    ...
    else if (KAPS_RPB_BLOCK_START <= blk_nr && blk_nr <= KAPS2_RPB_BLOCK_END && func_num == KAPS_FUNC4)
    {
        ...
        kaps_assert(row_nr < device->num_80b_entries_ab / 2, "Incorrect ADS1 Row Nr \n");
    ...
    else if (KAPS2_SMALL_BB_START <= blk_nr && blk_nr <= KAPS2_SMALL_BB_END && func_num == KAPS_FUNC2)
    {
        ...
        kaps_assert(row_nr < KAPS_NUM_ROWS_IN_SMALL_BB, "Incorrect Small BB Row Nr \n");
        ...
        (func_num == KAPS_FUNC2 || func_num == KAPS_FUNC10))
    {
        ...
        if (device->map->bb[sw_bb_num].bb_config == KAPS_LARGE_BB_WIDTH_A000_B000) 
        {
            /*Do nothing since the Large BB is not configured for use*/
        } 
        else 
        {
            kaps_assert(row_nr < device->map->bb[sw_bb_num].bb_num_rows, "Incorrect BB row number\n");
    ...
    struct kaps_db *db)
{
    ...
    if (device->id >= KAPS_JERICHO_2_DEVICE_ID)
    {
        if (db->num_algo_levels_in_db == 2)
        {
            num_bbs_in_final_level = device->hw_res->total_small_bb / 2;
        }
        else
        {
