 ...
 */
    struct kaps_ix_compaction_chunk
    {
        ...
        int32_t sb_no;          /**< UDA SB No of the destination */
        int32_t uda_address;    /**< UDA address of the destination */
 ...
 */
    ...
    struct kaps_ix_mgr
    {
        ...
        uint32_t uda_translate[KAPS_HW_MAX_UDA_SB];
                                                  /**< UDA SB address AD entry translation */
        int32_t ix_base_to_sb[KAPS_HW_MAX_UDA_SB];

        uint32_t ix_segments[KAPS_HW_MAX_UDA_SB];
        ...
        struct kaps_ad_db *alloc_ad_db;
            POOL_DEF(
    kaps_ix_chunk) ix_chunk_pool;          /**< Pool for ix_chunks */
 ...
 * @param start_ix the start index of the index space.
 * @param end_ix the end index of the index space.
 * @param max_alloc_chunk_size Maximum size of an allocated index chunk that is possible
 * @param mgr_pp contains the pointer to the initialized Index Manager
 * @param is_warmboot flag to identify is device is in Warmboot mode.
 * @param core_id which core this kaps_ix_mgr has to be intialized
 *
 ...
 */
    ...
    kaps_status kaps_ix_mgr_wb_save_uda_translate(
    struct kaps_ix_mgr *mgr,
 ...
 */
    ...
    kaps_status kaps_ix_mgr_wb_restore_uda_translate(
    struct kaps_ix_mgr *mgr,
    kaps_device_issu_write_fn read_fn,
    void *handle,
    uint32_t * nv_offset);

    kaps_status kaps_kaps_ix_mgr_alloc_ad_dynamically(
    struct kaps_ix_mgr *mgr,
    struct kaps_ad_db *ad_db,
