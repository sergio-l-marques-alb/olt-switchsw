    ...
    struct kaps_device *device)
{
    ...
    if (should_release_mem_map)
    {
        ...
        for (j = 0; j < ndevices; j++)
        {
            ...
            for (i = 0; i < map->num_udc; i++)
            {
                device->alloc->xfree(device->alloc->cookie, map->udc[i].udm);
            }
            for (i = 0; i < map->num_bb * map->num_chained_bb; i++)
            {
                device->alloc->xfree(device->alloc->cookie, map->bb[i].row);
            }
            if (map->bb)
                device->alloc->xfree(device->alloc->cookie, map->bb);

            if (map->udc)
                device->alloc->xfree(device->alloc->cookie, map->udc);
            ...
            if (map->sb_memory)
                device->alloc->xfree(device->alloc->cookie, map->sb_memory);
        }
...
void
kaps_resource_reserve_dba_and_uda(
    struct kaps_device *device)
{
    struct memory_map *mem_map = resource_kaps_get_memory_map(device);

    if (device->num_ab_available != -1 && device->num_ab != device->num_ab_available)
    {
        int32_t start_idx = device->num_ab_available;

        device->num_normal_ab_allocated = device->num_ab - device->num_ab_available;
        for (; start_idx < device->num_ab; start_idx++)
        {
            mem_map->ab_memory[start_idx].db = MAGIC_DB;
            mem_map->ab_memory[start_idx].blk_cleared = 1;
            mem_map->sb_memory[start_idx / device->num_ab_per_sb].free_ab--;
        }
    }

    if (device->num_uda_available != -1)
    {
        int32_t num_uda_mb =
            device->hw_res->total_lpus * mem_map->num_udm_per_udc * device->num_uda_per_sb * KAPS_HW_MAX_UDA_SB_SIZE_MB;

        if (num_uda_mb != device->num_uda_available)
        {
            int32_t start_idx = device->num_uda_available / 8;
            int32_t iter;

            for (; start_idx < device->hw_res->total_lpus * mem_map->num_udm_per_udc; start_idx++)
            {
                struct udm_info *udm =
                    &mem_map->udc[start_idx / mem_map->num_udm_per_udc].udm[start_idx % mem_map->num_udm_per_udc];

                udm->occupied_bmp = 0xF;
                udm->is_reserved = 1;
                for (iter = 0; iter < KAPS_HW_NUM_SB_PER_LPU; iter++)
                {
                    udm->db[iter] = MAGIC_DB;
                }
            }
        }
    }
    ...
    struct kaps_device *device)
{
    ...
    if (device->num_ab_available != -1 || device->num_uda_available != -1)
    {

        kaps_resource_reserve_dba_and_uda(device);
    }
    ...
    if (device->type == KAPS_DEVICE_KAPS)
        status = kaps_process_kaps(device);
    else
...
kaps_status
kaps_resource_dynamic_ad_free(
    struct kaps_db * ad,
    uint8_t * sb_bitmap,
    int32_t uda_sb_no)
{
    if (!ad->common_info->ad_info.db->common_info->enable_dynamic_allocation)
    {
        return KAPS_OK;
    }
    return KAPS_INTERNAL_ERROR;
...
kaps_status
kaps_resource_find_ab_kaps(
    struct kaps_device * device,
    int32_t ab_no,
    struct kaps_ab_info ** ab)
{
    struct memory_map *mem_map = kaps_get_memory_map(device);

    if (ab_no >= device->num_ab)
        return KAPS_INVALID_AB_NUMBER;

    *ab = &mem_map->ab_memory[ab_no];

    return KAPS_OK;
}

kaps_status
kaps_resource_find_ab(
    struct kaps_device * device,
    int32_t ab_no,
    struct kaps_ab_info ** ab)
{
    if (device->type == KAPS_DEVICE_KAPS)
        return kaps_resource_find_ab_kaps(device, ab_no, ab);

    return KAPS_AB_NOT_FOUND;
}
...
kaps_status
kaps_resource_check_ab_write_permission(
    struct kaps_ab_info * ab,
...
void
resource_kaps_print_dba_power_budget(
    struct kaps_device *device)
{
    struct kaps_c_list_iter it;
    struct kaps_list_node *el;

    kaps_printf("power budget: ");
    kaps_c_list_iter_init(&device->db_list, &it);
    while ((el = kaps_c_list_iter_next(&it)) != NULL)
    {
        struct kaps_db *db = KAPS_SSDBLIST_TO_ENTRY(el);
        ...
        if (db->type == KAPS_DB_LPM || db->type == KAPS_DB_ACL)
            kaps_printf("(db %d: %d, %d) ", db->tid, db->hw_res.db_res->dba_budget, db->common_info->num_ab);
    }
    kaps_printf("\n");
}
    ...
    int32_t udm_no)
{
    ...
    mem_map->udc[udc_no].udm[udm_no].occupied_bmp = 0;
    mem_map->udc[udc_no].udm[udm_no].dt_index = 0;
    mem_map->udc[udc_no].udm[udm_no].is_dynamic = 0;
    mem_map->udc[udc_no].udm[udm_no].is_lsn = 0;
    ...
    for (k = 0; k < KAPS_HW_NUM_SB_PER_LPU; k++)
    {
        mem_map->udc[udc_no].udm[udm_no].db[k] = NULL;
    }
    ...
    int32_t udc_no)
{
    ...
    for (row = 0; row < device->map->num_udm_per_udc; row++)
    {
        KAPS_STRY(kaps_resource_release_udm(device, mem_map, db, dt_index, udc_no, row));
    }
    mem_map->udc[udc_no].is_xor = 0;
