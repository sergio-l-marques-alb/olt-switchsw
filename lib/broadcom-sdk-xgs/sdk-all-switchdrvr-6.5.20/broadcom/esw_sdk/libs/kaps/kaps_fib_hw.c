    ...
    NlmReasonCode * o_reason)
{
    ...
    for (rptNr = 0; rptNr < HW_MAX_PCM_BLOCKS; ++rptNr)
    {
        if (rptMap & (1ULL << rptNr))
        {
            ...
            start_bit_pos = (pfxIndexInBkt * KAPS_AD_WIDTH_1);
            KapsWriteBitsInArray(bktData, KAPS_BKT_WIDTH_8, start_bit_pos + KAPS_AD_WIDTH_1 - 1, start_bit_pos, ad_value);

            kaps_memcpy(bkt_rqt.data, bktData, KAPS_BKT_WIDTH_8);
            ...
            if (issue_write)
            {
                status = kaps_dm_kaps_bb_write(device, db, small_bb_nr, row_nr, 0, sizeof(bkt_rqt), (uint8_t *) & bkt_rqt);
    ...
    NlmReasonCode * o_reason)
{
    ...
    for (rptNr = 0; rptNr < HW_MAX_PCM_BLOCKS; ++rptNr)
    {
        if (rptMap & (1ULL << rptNr))
        {
            ...
            start_bit_pos = (pfxIndexInBkt * KAPS_AD_WIDTH_1);
            KapsWriteBitsInArray(bktData, KAPS_BKT_WIDTH_8, start_bit_pos + KAPS_AD_WIDTH_1 - 1, start_bit_pos, 0);
    ...
    NlmReasonCode * o_reason)
{
    ...
    while (cur_dup_ab)
    {
        ...
        status = kaps_dm_kaps_rpb_write(device, cur_dup_ab->ab_num, logicalLoc, ab_data, ab_mask, 3, 0);
    ...
    NlmReasonCode * o_reason_p)
{
    ...
    for (rptNr = 0; rptNr < HW_MAX_PCM_BLOCKS; ++rptNr)
    {
        if (rptMap & (1ULL << rptNr))
        {
            status = kaps_algo_hw_write_rpt_data(device, rptNr, entry->m_data, entry->m_mask, addr, 1 /* is_xy */ );
