...
/**
 * Parsing function for pools stored in the warmboot dump file.
 *
 * @param bin_fp File pointer for input warmboot dump file
 * @param txt_fp File pointer for output file in human readable format
 * @param bin_offset Starting offset in the input file from where pool information begins
 *
 * @return KAPS_OK on success or an error code otherwise
 */
kaps_status kaps_trie_wb_dump_pools(
    FILE * bin_fp,
    FILE * txt_fp,
    uint32_t * bin_offset);

/**
 * Parsing function for the trie stored in the input warmboot dump file. This function should be
 * called per database by the upper layer. This function takes care of parsing RPT entries, IPT
 * and APT entries under each RPT entry and prefixes within each LSN
 *
 * @param bin_fp File pointer for input warmboot dump file
 * @param txt_fp File pointer for output file in human readable format
 * @param is_b0_silicon Caller should pass 1 for Rev-B else 0 for Rev-A
 * @param bin_offset Starting offset in the input file from where trie information begins
 *
 * @return KAPS_OK on success or an error code otherwise
 */
kaps_status kaps_trie_wb_dump_trie(
    FILE * bin_fp,
    FILE * txt_fp,
