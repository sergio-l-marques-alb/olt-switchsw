    ...
    uint32_t brick_gran_ix)
{
    ...
    else if (device->id == KAPS_JERICHO_2_DEVICE_ID && db->num_algo_levels_in_db == 3)
    {
        /*
         * Make cur_lpu_nr even
         */
        ...
        format_ix = kaps_jr2_large_convert_lpu_nr_to_format_ix(device, cur_lpu_nr, db->is_type_a);

        if (db->is_type_a)
        {
            if (device->map->bb[cur_lpu_nr].bb_config != KAPS_LARGE_BB_WIDTH_A480_B000
                && device->map->bb[cur_lpu_nr].bb_config != KAPS_LARGE_BB_WIDTH_A960_B000
                && device->map->bb[cur_lpu_nr].bb_config != KAPS_LARGE_BB_WIDTH_A480_B480)
        ...
        else
        {
            ...
            if (device->map->bb[cur_lpu_nr].bb_config != KAPS_LARGE_BB_WIDTH_A000_B480
                && device->map->bb[cur_lpu_nr].bb_config != KAPS_LARGE_BB_WIDTH_A000_B960
                && device->map->bb[cur_lpu_nr].bb_config != KAPS_LARGE_BB_WIDTH_A480_B480)
                kaps_assert(0, "Mismatching configuration while computing the format ix \n");
        }
    }
    ...
    NlmReasonCode * o_reason)
{
    ...
    if (index != KAPS_LSN_NEW_INDEX)
    {
        ...
        {
            kaps_it.row_offset = kaps_jr2_large_convert_lpu_nr_to_format_ix(device, cur_lpu, db->is_type_a);
        }
        ...
        if (db->num_algo_levels_in_db == 3)
        {
            ...
            if (db->is_type_a)
            {
                max_rows_in_udm = device->hw_res->ads2_depth_a / 2;
            }
            else
            {
                max_rows_in_udm = device->hw_res->ads2_depth_b / 2;
            }
        ...
        while (curLpuBrick)
        {
            ...
            kaps_kaps_fill_format_map(self, &kaps_it, cur_lpu_nr, curLpuBrick->m_granIx);
            brick_no++;
            curLpuBrick = curLpuBrick->m_next_p;
        }
        ...
        if (device->type == KAPS_DEVICE_KAPS && device->id == KAPS_JERICHO_PLUS_DEVICE_ID)
        {
            ...
            {
                kaps_kaps_fill_format_map(self, &kaps_it, 0, self->m_lpuList->m_granIx);
            }
        }
    ...
    NlmReasonCode * o_reason)
{
    ...
    while (curBrick)
    {
        kaps_uda_mgr_compute_abs_brick_udc_and_row(*(settings->m_pMlpMemMgr), lsn_p->m_mlpMemInfo, brick_idx,
                                                   &cur_hb_blk, &cur_row);
        cur_hb_blk = cur_hb_blk * device->hw_res->num_chained_bb;

        actual_num_entries_per_hb_blk = curBrick->m_maxCapacity / device->hw_res->num_chained_bb;
        pos_in_hb_blk = 0;
        flush_write = 1;
        ...
        if (db->num_algo_levels_in_db == 3)
        {
            ...
            if (device->map->bb[main_hb_blk].bb_config == KAPS_LARGE_BB_WIDTH_A480_B480)
            {
    ...
    NlmReasonCode * o_reason)
{
    ...
    while (numLpusProcessed < numLpusToProcess)
    {
        ...
        curBktNr = curLpuNr * device->hw_res->num_chained_bb;
         ...
         */
        for (level_nr = 0; level_nr < device->hw_res->num_chained_bb; ++level_nr)
        {
...
void
kaps_lsn_mc_prepare_mlp_dataGeneric(
    uint8_t * pfxData,
    uint16_t pfxLen,
    uint16_t lopoffLen,
    uint16_t gran,
    ...
    NlmReasonCode * o_reason)
{
    ...
    numPfxPerBkt = curBrick->m_maxCapacity / device->hw_res->num_chained_bb;
    pfxIndexInLpu = pfxIndexInLsn - indexSoFar;
    pfxIndexInBkt = pfxIndexInLpu % numPfxPerBkt;
    bkt_nr = (lpu_nr * device->hw_res->num_chained_bb) + (pfxIndexInLpu / numPfxPerBkt);
    ...
    errNum = kaps_kaps_bb_write_wrapper(self, device, bkt_nr, row_nr, 1, sizeof(bkt_rqt), (uint8_t *) & bkt_rqt, o_reason);
    ...
    uint32_t oldPfxIndex)
{
    ...
    while (curBrick)
    {
        ...
        pfx_hb_index = (rowNr * num_active_hb_blocks * KAPS_HB_ROW_WIDTH_1)
            + (curLpuNr * device->hw_res->num_chained_bb * KAPS_HB_ROW_WIDTH_1);
    ...
    num_entries_per_bb = maxCapacity / device->hw_res->num_chained_bb;
    ...
    NlmReasonCode * o_reason)
{
    ...
    if (!is_ad_update)
    {
        ...
        {
            kaps_lsn_mc_prepare_mlp_dataGeneric(pfx->m_data, pfx->m_nPfxSize, self->m_nLopoff, gran, bktData,
                                                KAPS_BKT_WIDTH_8,
                                                KAPS_BKT_WIDTH_1 - 1 - (pfxIndexInBkt * curBrick->m_gran));
        }
    }

    ad_value = KapsReadBitsInArrray(&ad_handle->value[0], KAPS_AD_WIDTH_8, KAPS_AD_ARRAY_END_BIT_POS,
    ...
    int32_t to_offset)
{
    ...
    for (brick_idx = 0; brick_idx < uda_chunk->size; brick_idx++)
    {
        ...
        srcBktNr = src_udc_num * device->hw_res->num_chained_bb;
        destBktNr = dest_udc_num * device->hw_res->num_chained_bb;
        iterInLpu = 0;
        ...
        for (level_nr = 0; level_nr < device->hw_res->num_chained_bb; ++level_nr)
        {
            ...
            errNum =
                kaps_kaps_bb_write_wrapper(curLsn, device, destBktNr, dest_row_num, 1, sizeof(bkt_rqt),
                                      (uint8_t *) & bkt_rqt, &o_reason);
            if (errNum != NLMERR_OK)
            {
                return errNum;
            }
            ...
            {
                uint32_t maxNumPfxInBkt = curBrick->m_maxCapacity / device->hw_res->num_chained_bb;
                uint32_t src_hb_index, dest_hb_index;

                src_hb_index =
                    (src_row_num * device->hw_res->total_num_bb * KAPS_HB_ROW_WIDTH_1) +
                    (srcBktNr * KAPS_HB_ROW_WIDTH_1);
                dest_hb_index =
                    (dest_row_num * device->hw_res->total_num_bb * KAPS_HB_ROW_WIDTH_1) +
    ...
    NlmReasonCode * o_reason)
{
    ...
    while (curLpuBrick)
    {
        ...
        iterInLpu = 0;
        maxNumPfxInBkt = curLpuBrick->m_maxCapacity / device->hw_res->num_chained_bb;
        curBktNr = curLpuNr * device->hw_res->num_chained_bb;
        srcBktNum = srcLpuNum * device->hw_res->num_chained_bb;
        ...
        for (level_nr = 0; level_nr < device->hw_res->num_chained_bb; ++level_nr)
        {
            ...
            for (iterInBkt = 0; iterInBkt < maxNumPfxInBkt; ++iterInBkt)
            {
                ...
                if (b)
                {
                    ...
                    {
                        kaps_lsn_mc_prepare_mlp_dataGeneric(b->m_data, b->m_nPfxSize, self->m_nLopoff,
                    ...
                    ad_value = KapsReadBitsInArrray(&ad_handle->value[0], KAPS_AD_WIDTH_8, KAPS_AD_ARRAY_END_BIT_POS,
                                                KAPS_AD_ARRAY_START_BIT_POS);
                    start_bit_pos = iterInBkt * KAPS_AD_WIDTH_1;
                    KapsWriteBitsInArray(bktData, KAPS_BKT_WIDTH_8, start_bit_pos + KAPS_AD_WIDTH_1 - 1, start_bit_pos,
                                     ad_value);
            ...
            errNum =
                kaps_kaps_bb_write_wrapper(self, device, curBktNr, rowNr, 1, sizeof(bkt_rqt), (uint8_t *) & bkt_rqt,
                                      o_reason);
            if (errNum != NLMERR_OK)
            {
                return errNum;
    ...
    NlmReasonCode * o_reason)
{
    ...
    for (loopCnt = 0; loopCnt < numLPUsToClear; ++loopCnt)
    {
        ...
        destBktNr = destLPU * device->hw_res->num_chained_bb;
        for (i = 0; i < device->hw_res->num_chained_bb; ++i)
        {
            ...
            errNum =
                kaps_kaps_bb_write_wrapper(self, device, destBktNr, destRow, 1, sizeof(bkt_rqt), (uint8_t *) & bkt_rqt,
                                      o_reason);
