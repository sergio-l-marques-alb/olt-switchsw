...
#include "falcon_madura_field_access.c"
#include "falcon_madura_internal.c"
#include "falcon_madura_dv_functions.c"
#include "falcon_madura_pll_config.c"
...
err_code_t falcon_furia_madura_display_eye_scan(const phymod_access_t *pa ) {
  int8_t y;
  err_code_t err_code;
  uint16_t status = 0;
  uint32_t stripe[64];
  ...
  err_code = falcon_furia_madura_meas_eye_scan_start( pa, EYE_SCAN_HORIZ);
  if (err_code) {
    falcon_furia_madura_meas_eye_scan_done( pa );
    return (err_code);
  }

  falcon_furia_madura_display_eye_scan_header( pa, 1);
  /* display stripe */
  for (y = 62;y>=-62;y=y-2) { 
 	err_code = falcon_furia_madura_read_eye_scan_stripe( pa, &stripe[0], &status);
	if (err_code) {
		falcon_furia_madura_meas_eye_scan_done( pa );
		return (err_code);
	}

    falcon_furia_madura_display_eye_scan_stripe( pa, y,&stripe[0]);
	USR_PRINTF(("\n"));  
  }  
  /* stop acquisition */
  err_code = falcon_furia_madura_meas_eye_scan_done( pa );
  if (err_code) {
	return (err_code);
  }

  falcon_furia_madura_display_eye_scan_footer( pa, 1);

  return(ERR_CODE_NONE);
}

/* This function is for Falcon and is configured for passive mode */
err_code_t falcon_furia_madura_meas_lowber_eye(const phymod_access_t *pa, const struct falcon_furia_madura_eyescan_options_st eyescan_options, uint32_t *buffer) {
  int8_t y,x;
  int16_t i;
  uint16_t status;
  uint32_t errors = 0;
  ...
  i = 0;
  ESTM(stopped_state = rdv_falcon_furia_madura_usr_sts_micro_stopped());

  timeout = eyescan_options.timeout_in_milliseconds;
  wrcv_falcon_furia_madura_diag_max_time_control((uint8_t)timeout);
 
  wrv_falcon_furia_madura_usr_diag_mode(eyescan_options.mode);
  ...
  for (y = eyescan_options.vert_max;y>=eyescan_options.vert_min;y=y-eyescan_options.vstep) {   
	for (x=eyescan_options.horz_min;x<=eyescan_options.horz_max;x=x+eyescan_options.hstep) {
	  /* acquire sample */
        EFUN(falcon_furia_madura_pmd_uc_cmd_with_data( pa, CMD_DIAG_EN,CMD_UC_DIAG_GET_EYE_SAMPLE,((uint16_t)x)<<8 | (uint8_t)y,200));
        /* wait for sample complete */
#ifdef MADURA_UNTESTED_CODE
        do {
            falcon_furia_madura_delay_us(1000);
            ESTM(status=rdv_falcon_furia_madura_usr_diag_status());
            USR_PRINTF(("status=%04x\n",status));
        } while((status & 0x8000) == 0); 
#else
        EFUN(falcon_furia_madura_poll_diag_done( pa, &status, (((uint32_t)timeout)<<7)*10 + 20000));
#endif
        EFUN(falcon_furia_madura_prbs_err_count_ll( pa, &errors));
  ...
  USR_PRINTF(("\n"));
  EFUN(falcon_furia_madura_meas_eye_scan_done( pa ));
  EFUN(wrv_falcon_furia_madura_usr_sts_micro_stopped(stopped_state));
  return(ERR_CODE_NONE);
}


/* Display the LOW BER EyeScan */
err_code_t falcon_furia_madura_display_lowber_eye(const phymod_access_t *pa, const struct falcon_furia_madura_eyescan_options_st eyescan_options, uint32_t *buffer) {
    int8_t x,y,i,z;
...
err_code_t falcon_furia_madura_meas_eye_scan_start(const phymod_access_t *pa, uint8_t direction) {
  err_code_t err_code;
  uint8_t lock;
  ESTM(lock = rd_falcon_furia_madura_pmd_rx_lock());
  if(lock == 0) {
  ...
  if(direction == EYE_SCAN_VERTICAL) {
	err_code = falcon_furia_madura_pmd_uc_diag_cmd( pa, CMD_UC_DIAG_START_VSCAN_EYE,200);
  } else {
	  err_code = falcon_furia_madura_pmd_uc_diag_cmd( pa, CMD_UC_DIAG_START_HSCAN_EYE,200);
  }
  	if (err_code) {
		return (err_code);
	}
...
err_code_t falcon_furia_madura_read_eye_scan_stripe(const phymod_access_t *pa, uint32_t *buffer,uint16_t *status){
	int8_t i;
	uint32_t val[2] = {0,0};
    err_code_t err_code;
	uint16_t sts = 0;
	...
	*status = 0;
	for(i=0;i<32;i++) {
		err_code = falcon_furia_madura_poll_diag_eye_data(pa,&val[0], &sts, 200);
 		*status |= sts & 0xF000;
		if (err_code) {
			return (err_code);
		}
...
err_code_t falcon_furia_madura_display_eye_scan_stripe(const phymod_access_t *pa, int8_t y,uint32_t *buffer) {
  ...
  /* ESTM(data_thresh = rd_falcon_furia_madura_rx_data_thresh_sel()); */
...
err_code_t falcon_furia_madura_display_eye_scan_header(const phymod_access_t *pa, int8_t i) {
int8_t x;
	USR_PRINTF(("\n"));
	for(x=1;x<=i;x++) {
...
err_code_t falcon_furia_madura_display_eye_scan_footer(const phymod_access_t *pa, int8_t i) {
int8_t x;
	for(x=1;x<=i;x++) {
		USR_PRINTF(("         : -|----|----|----|----|----|----|----|----|----|----|----|----|-"));
	}
...
/*eye_scan_status_t falcon_furia_madura_read_eye_scan_status() */
err_code_t falcon_furia_madura_read_eye_scan_status(const phymod_access_t *pa, uint16_t *status) {
...
err_code_t falcon_furia_madura_meas_eye_scan_done(const phymod_access_t *pa ) {
  err_code_t err_code;
  err_code = falcon_furia_madura_pmd_uc_diag_cmd( pa, CMD_UC_DIAG_DISABLE,200);
  if (err_code) return(err_code);
  return(ERR_CODE_NONE);
}


err_code_t falcon_furia_madura_start_ber_scan_test(const phymod_access_t *pa, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    uint8_t lock,sts;
    ESTM(lock = rd_falcon_furia_madura_pmd_rx_lock());
    if(lock == 0) {
    ...
    ESTM(sts =rdv_falcon_furia_madura_usr_sts_micro_stopped());
    if(sts > 1) {
       USR_PRINTF(("Error: Lane is busy (%d) requesting BER scan\n",sts));
        return(ERR_CODE_DIAG_SCAN_NOT_COMPLETE);
    }

    wrcv_falcon_furia_madura_diag_max_time_control(timer_control);
    wrcv_falcon_furia_madura_diag_max_err_control(max_error_control);
    EFUN(falcon_furia_madura_pmd_uc_cmd( pa, CMD_CAPTURE_BER_START, ber_scan_mode,500));
    return(ERR_CODE_NONE);
}

err_code_t falcon_furia_madura_read_ber_scan_data(const phymod_access_t *pa, uint32_t *errors, uint32_t *timer_values, uint8_t *cnt, uint32_t timeout) {
    err_code_t err_code;
 ...
 ESTM(sts = rdv_falcon_furia_madura_usr_diag_status());
	if((sts & 0x8000) == 0) {
	...
	*cnt = (sts & 0x00FF)/3;
	for(i=0;i < *cnt;i++) {
		...
		err_code = falcon_furia_madura_pmd_uc_cmd( pa,  CMD_READ_DIAG_DATA_WORD, 0, timeout);
		if(err_code) return(err_code);
		ESTM(dataword = rd_falcon_furia_madura_uc_dsc_data());           /* LSB contains 2 -4bit nibbles */
		...
		err_code = falcon_furia_madura_pmd_uc_cmd( pa,  CMD_READ_DIAG_DATA_BYTE, 0, timeout);
		if(err_code) return(err_code);
		ESTM(prbs_byte = (uint8_t)rd_falcon_furia_madura_uc_dsc_data());
...
err_code_t falcon_furia_madura_eye_margin_proj(const phymod_access_t *pa, USR_DOUBLE rate, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
	...
	EFUN(falcon_furia_madura_start_ber_scan_test( pa, ber_scan_mode, timer_control, max_error_control));
    ESTM(offset_start = rd_falcon_furia_madura_uc_dsc_data());
	...
	if(verbose > 5) {
		do {
			EFUN(falcon_furia_madura_delay_us(2000000));
            ESTM(sts = rdv_falcon_furia_madura_usr_diag_status());
			USR_PRINTF(("sts=%04x\n",sts));
    ...
    /* coverity[dead_error_begin] */
        ...
        EFUN(falcon_furia_madura_poll_diag_done( pa, &sts,timer_control*2000));
	}
	if(verbose > 2) USR_PRINTF(("delay done\n"));

	EFUN(falcon_furia_madura_read_ber_scan_data( pa,  &errs[0], &time[0], &cnt, 2000));
	
	if(verbose > 2) USR_PRINTF(("read done cnt=%d\n",cnt));

	EFUN(falcon_furia_madura_pmd_uc_cmd( pa, CMD_CAPTURE_BER_END,0x00,50));
	...
	/*} */
   	...
   	EFUN(falcon_furia_madura_display_ber_scan_data(pa, rate, ber_scan_mode, &errs[0], &time[0],(uint8_t)_abs(offset_start)));
...
err_code_t falcon_furia_madura_display_ber_scan_data (const phymod_access_t *pa, USR_DOUBLE rate, uint8_t ber_scan_mode, uint32_t *total_errs, uint32_t *total_time, uint8_t max_offset) {
...
err_code_t falcon_furia_madura_display_lane_state_hdr(const phymod_access_t *pa ) {
...
err_code_t falcon_furia_madura_display_lane_state_legend(const phymod_access_t *pa ) {
  USR_PRINTF(("\n"));
  USR_PRINTF(("**********************************************************************************************\n")); 
...
err_code_t falcon_furia_madura_display_lane_state(const phymod_access_t *pa ) {
  err_code_t err_code;
  err_code = _falcon_furia_madura_display_lane_state_no_newline( pa );
  USR_PRINTF(("\n"));
  return (err_code);
}

err_code_t falcon_furia_madura_display_core_state_hdr(const phymod_access_t *pa ) {
...
err_code_t falcon_furia_madura_display_core_state_line(const phymod_access_t *pa ) {
  err_code_t err_code;
  err_code = _falcon_furia_madura_display_core_state_no_newline( pa ); 
  USR_PRINTF(("\n"));
  return (err_code);
}

err_code_t falcon_furia_madura_display_core_state_legend(void) {
  USR_PRINTF(("\n"));
  USR_PRINTF(("**************************************************************************************************************\n")); 
...
err_code_t falcon_furia_madura_display_core_config(const phymod_access_t *pa ) {
  ...
  ESTM(USR_PRINTF(("Lane Addr 0          = %d\n", rdc_falcon_furia_madura_lane_addr_0())));
  ESTM(USR_PRINTF(("Lane Addr 1          = %d\n", rdc_falcon_furia_madura_lane_addr_1())));
  ESTM(USR_PRINTF(("Lane Addr 2          = %d\n", rdc_falcon_furia_madura_lane_addr_2())));
  ESTM(USR_PRINTF(("Lane Addr 3          = %d\n", rdc_falcon_furia_madura_lane_addr_3())));

  return(ERR_CODE_NONE); 
}


err_code_t falcon_furia_madura_display_lane_config(const phymod_access_t *pa ) {

  err_code_t  err_code;
  struct falcon_furia_madura_uc_lane_config_st lane_cfg;

  USR_MEMSET(&lane_cfg, 0, sizeof(struct falcon_furia_madura_uc_lane_config_st));

  USR_PRINTF(("\n\n*************************************\n")); 
  USR_PRINTF(("**** SERDES LANE %d CONFIGURATION ****\n",falcon_furia_madura_get_lane(pa))); 
  USR_PRINTF(("*************************************\n\n")); 

  err_code = falcon_furia_madura_get_uc_lane_cfg( pa, &lane_cfg);
  if (err_code) {
    return (err_code);
  }
  ...
  ESTM(USR_PRINTF(("CL93/72 Training Enable  = %d\n", rd_falcon_furia_madura_cl93n72_ieee_training_enable())));
  ...
  ESTM(USR_PRINTF(("EEE Mode Enable       = %d\n", rd_falcon_furia_madura_eee_mode_en())));
  ESTM(USR_PRINTF(("OSR Mode Force        = %d\n", rd_falcon_furia_madura_osr_mode_frc())));
  ESTM(USR_PRINTF(("OSR Mode Force Val    = %d\n", rd_falcon_furia_madura_osr_mode_frc_val())));
  ESTM(USR_PRINTF(("TX Polarity Invert    = %d\n", rd_falcon_furia_madura_tx_pmd_dp_invert())));
  ESTM(USR_PRINTF(("RX Polarity Invert    = %d\n\n", rd_falcon_furia_madura_rx_pmd_dp_invert())));

  ESTM(USR_PRINTF(("TXFIR Post2           = %d\n", rd_falcon_furia_madura_txfir_post2())));
  ESTM(USR_PRINTF(("TXFIR Post3           = %d\n", rd_falcon_furia_madura_txfir_post3())));
  ESTM(USR_PRINTF(("TXFIR Main            = %d\n", rd_falcon_furia_madura_cl93n72_txfir_main())));
  ESTM(USR_PRINTF(("TXFIR Pre             = %d\n", rd_falcon_furia_madura_cl93n72_txfir_pre())));
  ESTM(USR_PRINTF(("TXFIR Post            = %d\n", rd_falcon_furia_madura_cl93n72_txfir_post())));

  return(ERR_CODE_NONE); 
}


err_code_t falcon_furia_madura_display_core_state(const phymod_access_t *pa ) {
  ...
  ESTM(bin = rdcv_falcon_furia_madura_avg_tmon_reg13bit());temp = _bin_to_degC((bin>>3));
  USR_PRINTF(("Average Die TMON_reg13bit = %d [ %3dC ]\n", bin,temp));
  }


  ESTM(USR_PRINTF(("Core Event Log Level      = %d\n\n", rdcv_falcon_furia_madura_usr_ctrl_core_event_log_level())));

  ESTM(USR_PRINTF(("Core DP Reset State       = %d\n\n", rdc_falcon_furia_madura_core_dp_reset_state())));

  ESTM(USR_PRINTF(("Common Ucode Version       = 0x%x\n", rdcv_falcon_furia_madura_common_ucode_version())));
  ESTM(USR_PRINTF(("Common Ucode Minor Version = 0x%x\n", rdcv_falcon_furia_madura_common_ucode_minor_version())));
  ESTM(USR_PRINTF(("AFE Hardware Version       = 0x%x\n\n", rdcv_falcon_furia_madura_afe_hardware_version())));
  ...
  USR_PRINTF(("VCO Rate                   = %d (~%d.%dGHz)\n",core_cfg.field.vco_rate,vco_rate_mhz/1000,vco_rate_mhz % 1000)); 
    ESTM(USR_PRINTF(("Analog VCO Range           = %d\n", rdc_falcon_furia_madura_ams_pll_range())));
    ESTM(USR_PRINTF(("PLL Divider                = %d\n\n", rdc_falcon_furia_madura_ams_pll_ndiv())));
...
err_code_t falcon_furia_madura_display_lane_debug_status(const phymod_access_t *pa ) {

  err_code_t  err_code;

  /* startup */
  struct falcon_furia_madura_usr_ctrl_disable_functions_st ds; 
  struct falcon_furia_madura_usr_ctrl_disable_dfe_functions_st dsd;

  /* steady state */
  struct falcon_furia_madura_usr_ctrl_disable_functions_st dss;
  struct falcon_furia_madura_usr_ctrl_disable_dfe_functions_st dssd;

  USR_MEMSET(&ds, 0, sizeof(struct falcon_furia_madura_usr_ctrl_disable_functions_st));
  USR_MEMSET(&dsd, 0, sizeof(struct falcon_furia_madura_usr_ctrl_disable_dfe_functions_st));
  USR_MEMSET(&dss, 0, sizeof(struct falcon_furia_madura_usr_ctrl_disable_functions_st));
  USR_MEMSET(&dssd, 0, sizeof(struct falcon_furia_madura_usr_ctrl_disable_dfe_functions_st));

  USR_PRINTF(("\n\n************************************\n")); 
  USR_PRINTF(("**** SERDES LANE %d DEBUG STATUS ****\n",falcon_furia_madura_get_lane(pa))); 
  USR_PRINTF(("************************************\n\n")); 

  ESTM(USR_PRINTF(("Restart Count       = %d\n", rdv_falcon_furia_madura_usr_sts_restart_counter())));
  ESTM(USR_PRINTF(("Reset Count         = %d\n", rdv_falcon_furia_madura_usr_sts_reset_counter())));
  ESTM(USR_PRINTF(("PMD Lock Count      = %d\n\n", rdv_falcon_furia_madura_usr_sts_pmd_lock_counter())));
 

  err_code = falcon_furia_madura_get_usr_ctrl_disable_startup( pa, &ds);
  if (err_code) {
    return (err_code);
  }
  ...
  err_code = falcon_furia_madura_get_usr_ctrl_disable_startup_dfe( pa, &dsd);
  if (err_code) {
    return (err_code);
  }
  ...
  err_code = falcon_furia_madura_get_usr_ctrl_disable_steady_state( pa, &dss);
  if (err_code) {
    return (err_code);
  }
  ...
  err_code = falcon_furia_madura_get_usr_ctrl_disable_steady_state_dfe( pa, &dssd);
  if (err_code) {
    return (err_code);
  }
  ...
  ESTM(USR_PRINTF(("Retune after Reset    = %d\n", rdv_falcon_furia_madura_usr_ctrl_retune_after_restart())));               
  ESTM(USR_PRINTF(("Clk90 offset Adjust   = %d\n", rdv_falcon_furia_madura_usr_ctrl_clk90_offset_adjust())));                
  ESTM(USR_PRINTF(("Clk90 offset Override = %d\n", rdv_falcon_furia_madura_usr_ctrl_clk90_offset_override())));              
  ESTM(USR_PRINTF(("Lane Event Log Level  = %d\n", rdv_falcon_furia_madura_usr_ctrl_lane_event_log_level())));
...
err_code_t falcon_furia_madura_stop_uc_lane(const phymod_access_t *pa, uint8_t enable) {
  ...
  if (enable) {
    return(falcon_furia_madura_pmd_uc_control( pa, CMD_UC_CTRL_STOP_GRACEFULLY,100));
  }
  else {
    return(falcon_furia_madura_pmd_uc_control( pa, CMD_UC_CTRL_RESUME,50));
  }
}
...
err_code_t falcon_furia_madura_stop_uc_lane_status(const phymod_access_t *pa, uint8_t *uc_lane_stopped) {
  ...
  ESTM(*uc_lane_stopped = rdv_falcon_furia_madura_usr_sts_micro_stopped());
...
err_code_t falcon_furia_madura_stop_rx_adaptation(const phymod_access_t *pa, uint8_t enable) {
...
err_code_t falcon_furia_madura_request_stop_rx_adaptation(const phymod_access_t *pa ) {

  return(falcon_furia_madura_pmd_uc_control_return_immediate( pa, CMD_UC_CTRL_STOP_GRACEFULLY));
}
...
err_code_t falcon_furia_madura_pmd_uc_cmd_return_immediate(const phymod_access_t *pa, enum srds_pmd_uc_cmd_enum cmd, uint8_t supp_info) {
  ...
  err_code = falcon_furia_madura_poll_uc_dsc_ready_for_cmd_equals_1( pa, 1); /* Poll for uc_dsc_ready_for_cmd = 1 to indicate falcon_furia ready for command */
  if (err_code) {
    ESTM(USR_PRINTF(("ERROR : DSC ready for command timed out (before cmd) cmd = %d, supp_info = x%02x err=%d !\n", cmd, supp_info, err_code)));      
    return (err_code);
  }
  /*wr_falcon_furia_madura_uc_dsc_supp_info(supp_info);*/                          /* Supplement info field */
  /*wr_falcon_furia_madura_uc_dsc_error_found(0x0);    */                          /* Clear error found field */
  /*wr_falcon_furia_madura_uc_dsc_gp_uc_req(cmd);      */                          /* Set command code */
  /*wr_falcon_furia_madura_uc_dsc_ready_for_cmd(0x0);  */                          /* Issue command, by clearing "ready for command" field */
  cmddata = (((uint16_t)supp_info)<<8) | (uint16_t)cmd;        /* Combine writes to single write instead of 4 RMW */

  EFUN(falcon_furia_madura_pmd_wr_reg(pa,FALCON_FURIA_DSC_A_DSC_UC_CTRL, cmddata));         /* This address is same for Eagle, and all Merlin */
...
err_code_t falcon_furia_madura_pmd_uc_cmd(const phymod_access_t *pa, enum srds_pmd_uc_cmd_enum cmd, uint8_t supp_info, uint32_t timeout_ms) {
  ...
  EFUN(falcon_furia_madura_pmd_uc_cmd_return_immediate( pa, cmd, supp_info));    /* Invoke falcon_furia_uc_cmd and return control immediately */

  EFUN(falcon_furia_madura_poll_uc_dsc_ready_for_cmd_equals_1( pa, timeout_ms)); /* Poll for uc_dsc_ready_for_cmd = 1 to indicate falcon_furia ready for command */
  ESTM(uc_dsc_error_found = rd_falcon_furia_madura_uc_dsc_error_found());
  if(uc_dsc_error_found) {
    ESTM(USR_PRINTF(("ERROR : DSC ready for command return error ( after cmd) cmd = %d, supp_info = x%02x !\n", cmd, rd_falcon_furia_madura_uc_dsc_supp_info())));      
...
err_code_t falcon_furia_madura_pmd_uc_cmd_with_data_return_immediate(const phymod_access_t *pa, enum srds_pmd_uc_cmd_enum cmd, uint8_t supp_info, uint16_t data) {
  ...
  EFUN(wr_falcon_furia_madura_uc_dsc_data(data));                                  /* Write value written to uc_dsc_data field */
  /*wr_falcon_furia_madura_uc_dsc_supp_info(supp_info);  */                        /* Supplement info field */
  /*wr_falcon_furia_madura_uc_dsc_error_found(0x0);      */                        /* Clear error found field */
  /*wr_falcon_furia_madura_uc_dsc_gp_uc_req(cmd);        */                        /* Set command code */
  /*wr_falcon_furia_madura_uc_dsc_ready_for_cmd(0x0);    */                        /* Issue command, by clearing "ready for command" field */
...
err_code_t falcon_furia_madura_pmd_uc_cmd_with_data(const phymod_access_t *pa, enum srds_pmd_uc_cmd_enum cmd, uint8_t supp_info, uint16_t data, uint32_t timeout_ms) {
  ...
  EFUN(falcon_furia_madura_pmd_uc_cmd_with_data_return_immediate( pa, cmd, supp_info, data)); /* Invoke falcon_furia_uc_cmd and return control immediately */
...
err_code_t falcon_furia_madura_pmd_uc_control_return_immediate(const phymod_access_t *pa, enum srds_pmd_uc_ctrl_cmd_enum control) {
  return(falcon_furia_madura_pmd_uc_cmd_return_immediate( pa, CMD_UC_CTRL, (uint8_t) control));
}
...
err_code_t falcon_furia_madura_pmd_uc_control(const phymod_access_t *pa, enum srds_pmd_uc_ctrl_cmd_enum control, uint32_t timeout_ms) {
  return(falcon_furia_madura_pmd_uc_cmd( pa, CMD_UC_CTRL, (uint8_t) control, timeout_ms));
}
...
err_code_t falcon_furia_madura_pmd_uc_diag_cmd(const phymod_access_t *pa, enum srds_pmd_uc_diag_cmd_enum control, uint32_t timeout_ms) {
  return(falcon_furia_madura_pmd_uc_cmd( pa, CMD_DIAG_EN, (uint8_t) control, timeout_ms));
}
...
uint8_t falcon_furia_madura_rdbl_uc_var(const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr) {
	uint8_t rddata;
	...
	{
    falcon_furia_madura_uc_lane_info_st uc_lane_info;

    USR_MEMSET(&uc_lane_info, 0, sizeof(falcon_furia_madura_uc_lane_info_st));
    EFUN(falcon_furia_madura_get_uc_ln_info(pa, &uc_lane_info));
	if (uc_lane_info.is_direct_ram_access_avail) {
		EPFUN(falcon_furia_madura_rdb_ram(pa, &rddata, uc_lane_info.lane_var_base_address+addr, 1));    /* Direct RAM access */
	}
	else {
		...
		EPFUN(falcon_furia_madura_pmd_uc_cmd( pa, CMD_READ_UC_LANE_BYTE, (uint8_t)addr, 50));          /* Use Serdes FW DSC Command Interface for reading RAM */

		EPSTM(rddata = (uint8_t)rd_falcon_furia_madura_uc_dsc_data());                                    /* Read_Value read from uc_dsc_data field */
	}
    }
...
int8_t falcon_furia_madura_rdbls_uc_var(const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr) {
  return ((int8_t) falcon_furia_madura_rdbl_uc_var( pa, err_code_p, addr));
} 

/* Micro RAM Lane Word Read */
uint16_t falcon_furia_madura_rdwl_uc_var(const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr) {
  uint16_t rddata;
  ...
  EFUN(falcon_furia_madura_get_uc_ln_info(pa, &uc_lane_info));
  if (uc_lane_info.is_direct_ram_access_avail) {
	  EFUN(falcon_furia_madura_rdw_ram(pa, &rddata, uc_lane_info.lane_var_base_address+addr, 1));                    /* Direct RAM access */
  }
  else {
	  if (addr >= uc_lane_info.lane_ram_size || addr > 255) {                       /* uC command only support 8-bit addr */                 
		  *err_code_p = ERR_CODE_INVALID_RAM_ADDR;                                  /* Error Code indicating Out of Range access */
	  ...
	  EPFUN(falcon_furia_madura_pmd_uc_cmd( pa, CMD_READ_UC_LANE_WORD, (uint8_t)addr, 50));                   /* Use Serdes FW DSC Command Interface for reading RAM */

    EPSTM(rddata = rd_falcon_furia_madura_uc_dsc_data());                                                    /* Read_Value read from uc_dsc_data field */
  }
  }
...
int16_t falcon_furia_madura_rdwls_uc_var(const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr) {
  return ((int16_t) falcon_furia_madura_rdwl_uc_var( pa, err_code_p, addr));
}
...
err_code_t falcon_furia_madura_wrbl_uc_var(const phymod_access_t *pa, uint16_t addr, uint8_t wr_val) {
    ...
    EFUN(falcon_furia_madura_get_uc_ln_info(pa, &uc_lane_info));
	if (uc_lane_info.is_direct_ram_access_avail) {
		EFUN(falcon_furia_madura_wrb_ram(pa, uc_lane_info.lane_var_base_address+addr, 1, &wr_val));
		return (ERR_CODE_NONE);
	}
	else {
		if (addr >= uc_lane_info.lane_ram_size || addr > 255) {                      /* uC command only support 8-bit addr */                             
		...
		return (falcon_furia_madura_pmd_uc_cmd_with_data( pa, CMD_WRITE_UC_LANE_BYTE, (uint8_t)addr, (uint16_t)wr_val, 10)); /* Use Serdes FW DSC Command Interface for writing RAM */
	}
    }
}
...
err_code_t falcon_furia_madura_wrbls_uc_var(const phymod_access_t *pa, uint16_t addr, int8_t wr_val) {
  return (falcon_furia_madura_wrbl_uc_var( pa, addr, wr_val));
}
...
err_code_t falcon_furia_madura_wrwl_uc_var(const phymod_access_t *pa, uint16_t addr, uint16_t wr_val) {
    ...
    EFUN(falcon_furia_madura_get_uc_ln_info(pa, &uc_lane_info));
	if (uc_lane_info.is_direct_ram_access_avail) {
		EFUN(falcon_furia_madura_wrw_ram(pa, uc_lane_info.lane_var_base_address+addr, 1, &wr_val));
		return (ERR_CODE_NONE);
	}
	else {
		if (addr >= uc_lane_info.lane_ram_size || addr > 255 ) {                   /* uC command only support 8-bit addr */                  
		...
		return (falcon_furia_madura_pmd_uc_cmd_with_data( pa, CMD_WRITE_UC_LANE_WORD, (uint8_t)addr, (uint16_t)wr_val, 10)); /* Use Serdes FW DSC Command Interface for writing RAM */
	}
    }
}
...
err_code_t falcon_furia_madura_wrwls_uc_var(const phymod_access_t *pa, uint16_t addr, int16_t wr_val) {
  return (falcon_furia_madura_wrwl_uc_var( pa, addr,wr_val));
}
...
uint8_t falcon_furia_madura_rdbc_uc_var(const phymod_access_t *pa, err_code_t *err_code_p, uint8_t addr) {
  ...
  EFUN(falcon_furia_madura_get_uc_ln_info(pa, &uc_lane_info));
  if (uc_lane_info.is_direct_ram_access_avail) {
	  EPFUN(falcon_furia_madura_rdb_ram(pa, &rddata, uc_lane_info.core_var_base_address+(uint16_t)addr, 1));                    /* Direct RAM access */
  }
  else {
	  ...
	  EPFUN(falcon_furia_madura_pmd_uc_cmd( pa, CMD_READ_UC_CORE_BYTE, addr, 10));                   /* Use Serdes FW DSC Command Interface for reading RAM */
	  EPSTM(rddata = (uint8_t)rd_falcon_furia_madura_uc_dsc_data());                                            /* Read_Value read from uc_dsc_data field */
  }
  }
...
int8_t falcon_furia_madura_rdbcs_uc_var(const phymod_access_t *pa, err_code_t *err_code_p, uint8_t addr) {
  return ((int8_t) falcon_furia_madura_rdbc_uc_var( pa, err_code_p, addr));
}

/* Micro RAM Core Word Read */
uint16_t falcon_furia_madura_rdwc_uc_var(const phymod_access_t *pa, err_code_t *err_code_p, uint8_t addr) {
  ...
  EFUN(falcon_furia_madura_get_uc_ln_info(pa, &uc_lane_info));
  if (uc_lane_info.is_direct_ram_access_avail) {
	  EFUN(falcon_furia_madura_rdw_ram(pa, &rddata, uc_lane_info.core_var_base_address+(uint16_t)addr, 1));                    /* Direct RAM access */
  }
  else {
	  if (addr >= uc_lane_info.lane_ram_size) {         
		  *err_code_p = ERR_CODE_INVALID_RAM_ADDR;                                  /* Error Code indicating Out of Range access */
	  ...
	  EPFUN(falcon_furia_madura_pmd_uc_cmd( pa, CMD_READ_UC_CORE_WORD, addr, 10));                   /* Use Serdes FW DSC Command Interface for reading RAM */
...
int16_t falcon_furia_madura_rdwcs_uc_var(const phymod_access_t *pa, err_code_t *err_code_p, uint8_t addr) {
  return ((int16_t) falcon_furia_madura_rdwc_uc_var( pa, err_code_p, addr));
}
...
err_code_t falcon_furia_madura_wrbc_uc_var(const phymod_access_t *pa, uint8_t addr, uint8_t wr_val) {
    ...
    EFUN(falcon_furia_madura_get_uc_ln_info(pa, &uc_lane_info));
	if (uc_lane_info.is_direct_ram_access_avail) {
		EFUN(falcon_furia_madura_wrb_ram(pa, uc_lane_info.core_var_base_address+(uint16_t)addr, 1, &wr_val));
		return (ERR_CODE_NONE);
	}
	else {
		if (addr >= uc_lane_info.lane_ram_size) {                      
		...
		return (falcon_furia_madura_pmd_uc_cmd_with_data( pa, CMD_WRITE_UC_CORE_BYTE, addr, (uint16_t)wr_val, 10)); /* Use Serdes FW DSC Command Interface for writing RAM */
	}
    }
} 
...
err_code_t falcon_furia_madura_wrbcs_uc_var(const phymod_access_t *pa, uint8_t addr, int8_t wr_val) {
  return (falcon_furia_madura_wrbc_uc_var( pa, addr, wr_val));
}
...
err_code_t falcon_furia_madura_wrwc_uc_var(const phymod_access_t *pa, uint8_t addr, uint16_t wr_val) {
    ...
    EFUN(falcon_furia_madura_get_uc_ln_info(pa, &uc_lane_info));
	if (uc_lane_info.is_direct_ram_access_avail) {
		EFUN(falcon_furia_madura_wrw_ram(pa, uc_lane_info.core_var_base_address+addr, 1, &wr_val));
		return (ERR_CODE_NONE);
	}
	else {
		if (addr >= uc_lane_info.lane_ram_size) {                                     
		...
		return (falcon_furia_madura_pmd_uc_cmd_with_data( pa, CMD_WRITE_UC_CORE_WORD, addr, wr_val, 10)); /* Use Serdes FW DSC Command Interface for writing RAM */
	}
    }
}
...
err_code_t falcon_furia_madura_wrwcs_uc_var(const phymod_access_t *pa, uint8_t addr, int16_t wr_val) {
  return(falcon_furia_madura_wrwc_uc_var( pa, addr,wr_val));
}
...
err_code_t falcon_furia_madura_set_uc_core_config(const phymod_access_t *pa, struct falcon_furia_madura_uc_core_config_st struct_val) {
    uint8_t reset_state;
    ESTM(reset_state = rdc_falcon_furia_madura_core_dp_reset_state());
    if(reset_state < 7) {
        USR_PRINTF(("ERROR: falcon_furia_madura_set_uc_core_config( pa, ..) called without core_dp_s_rstb=0\n"));
        return (_error(ERR_CODE_CORE_DP_NOT_RESET));
    }

  _falcon_furia_madura_update_uc_core_config_word( pa, &struct_val);
  return(wrcv_falcon_furia_madura_config_word(struct_val.word));
}

err_code_t falcon_furia_madura_set_usr_ctrl_core_event_log_level(const phymod_access_t *pa, uint8_t core_event_log_level) {
  return(wrcv_falcon_furia_madura_usr_ctrl_core_event_log_level(core_event_log_level));
}

err_code_t falcon_furia_madura_set_uc_lane_cfg(const phymod_access_t *pa, struct falcon_furia_madura_uc_lane_config_st struct_val) {
    uint8_t reset_state;
    ESTM(reset_state = rd_falcon_furia_madura_lane_dp_reset_state());
    if(reset_state < 7) {
        USR_PRINTF(("ERROR: falcon_furia_madura_set_uc_lane_cfg( pa, ..) called without ln_dp_s_rstb=0\n"));
        return (_error(ERR_CODE_LANE_DP_NOT_RESET));
    }
  _falcon_furia_madura_update_uc_lane_config_word( pa, &struct_val);
  return(wrv_falcon_furia_madura_config_word(struct_val.word));
}

err_code_t falcon_furia_madura_set_usr_ctrl_lane_event_log_level(const phymod_access_t *pa, uint8_t lane_event_log_level) {
  return(wrv_falcon_furia_madura_usr_ctrl_lane_event_log_level(lane_event_log_level));
}
...
err_code_t falcon_furia_madura_set_usr_ctrl_disable_startup(const phymod_access_t *pa, struct falcon_furia_madura_usr_ctrl_disable_functions_st set_val) {
  _falcon_furia_madura_update_usr_ctrl_disable_functions_byte( pa, &set_val);
  return(wrv_falcon_furia_madura_usr_ctrl_disable_startup_functions_word(set_val.word));
}
...
err_code_t falcon_furia_madura_set_usr_ctrl_disable_startup_dfe(const phymod_access_t *pa, struct falcon_furia_madura_usr_ctrl_disable_dfe_functions_st set_val) {
  _falcon_furia_madura_update_usr_ctrl_disable_dfe_functions_byte( pa, &set_val);
  return(wrv_falcon_furia_madura_usr_ctrl_disable_startup_dfe_functions_byte(set_val.byte));
}
...
err_code_t falcon_furia_madura_set_usr_ctrl_disable_steady_state(const phymod_access_t *pa, struct falcon_furia_madura_usr_ctrl_disable_functions_st set_val) {
  _falcon_furia_madura_update_usr_ctrl_disable_functions_byte( pa, &set_val);
  return(wrv_falcon_furia_madura_usr_ctrl_disable_steady_state_functions_word(set_val.word));
}
...
err_code_t falcon_furia_madura_set_usr_ctrl_disable_steady_state_dfe(const phymod_access_t *pa, struct falcon_furia_madura_usr_ctrl_disable_dfe_functions_st set_val) {
  _falcon_furia_madura_update_usr_ctrl_disable_dfe_functions_byte( pa, &set_val);
  return(wrv_falcon_furia_madura_usr_ctrl_disable_steady_state_dfe_functions_byte(set_val.byte));
}
...
err_code_t falcon_furia_madura_get_uc_core_config(const phymod_access_t *pa, struct falcon_furia_madura_uc_core_config_st *get_val) {

  if(!get_val) {
	 return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
  }

  ESTM(get_val->word = rdcv_falcon_furia_madura_config_word());
  _falcon_furia_madura_update_uc_core_config_st( pa, get_val);
...
err_code_t falcon_furia_madura_get_usr_ctrl_core_event_log_level(const phymod_access_t *pa, uint8_t *core_event_log_level) {
  ...
  ESTM(*core_event_log_level = rdcv_falcon_furia_madura_usr_ctrl_core_event_log_level());

  return (ERR_CODE_NONE);
}

err_code_t falcon_furia_madura_get_uc_lane_cfg(const phymod_access_t *pa, struct falcon_furia_madura_uc_lane_config_st *get_val) { 

  if(!get_val) {
	 return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
  }

  ESTM(get_val->word = rdv_falcon_furia_madura_config_word());
  _falcon_furia_madura_update_uc_lane_config_st( pa, get_val);
  return (ERR_CODE_NONE);
}
...
err_code_t falcon_furia_madura_get_usr_ctrl_lane_event_log_level(const phymod_access_t *pa, uint8_t *lane_event_log_level) {
  ...
  ESTM(*lane_event_log_level = rdv_falcon_furia_madura_usr_ctrl_lane_event_log_level());
  return (ERR_CODE_NONE);
}
...
err_code_t falcon_furia_madura_get_usr_ctrl_disable_startup(const phymod_access_t *pa, struct falcon_furia_madura_usr_ctrl_disable_functions_st *get_val) {
  ...
  ESTM(get_val->word = rdv_falcon_furia_madura_usr_ctrl_disable_startup_functions_word());
  _falcon_furia_madura_update_usr_ctrl_disable_functions_st( pa, get_val);
  return (ERR_CODE_NONE);
}
...
err_code_t falcon_furia_madura_get_usr_ctrl_disable_startup_dfe(const phymod_access_t *pa, struct falcon_furia_madura_usr_ctrl_disable_dfe_functions_st *get_val) {
  ...
  ESTM(get_val->byte = rdv_falcon_furia_madura_usr_ctrl_disable_startup_dfe_functions_byte());
  _falcon_furia_madura_update_usr_ctrl_disable_dfe_functions_st( pa, get_val);
  return (ERR_CODE_NONE);
}
...
err_code_t falcon_furia_madura_get_usr_ctrl_disable_steady_state(const phymod_access_t *pa, struct falcon_furia_madura_usr_ctrl_disable_functions_st *get_val) {
  ...
  ESTM(get_val->word = rdv_falcon_furia_madura_usr_ctrl_disable_steady_state_functions_word());
  _falcon_furia_madura_update_usr_ctrl_disable_functions_st( pa, get_val);
  return (ERR_CODE_NONE);
}
...
err_code_t falcon_furia_madura_get_usr_ctrl_disable_steady_state_dfe(const phymod_access_t *pa, struct falcon_furia_madura_usr_ctrl_disable_dfe_functions_st *get_val) {
  ...
  ESTM(get_val->byte = rdv_falcon_furia_madura_usr_ctrl_disable_steady_state_dfe_functions_byte());
  _falcon_furia_madura_update_usr_ctrl_disable_dfe_functions_st( pa, get_val);
  return (ERR_CODE_NONE);
}
...
err_code_t falcon_furia_madura_poll_diag_done(const phymod_access_t *pa, uint16_t *status, uint32_t timeout_ms) {
 ...
 for(loop=0;loop < 100; loop++) {
	 ...
	 if(loop>10) {
		 EFUN(falcon_furia_madura_delay_us(10*timeout_ms));
	 }
 }
...
err_code_t falcon_furia_madura_poll_diag_eye_data(const phymod_access_t *pa,uint32_t *data,uint16_t *status, uint32_t timeout_ms) {
 ...
 for(loop=0;loop < 100; loop++) {
	 ...
	 if(((*status & 0x00FF) > 2) || ((*status & 0x8000) > 0)) {
		EFUN(falcon_furia_madura_pmd_uc_cmd( pa,  CMD_READ_DIAG_DATA_WORD, 0, 200));
        ESTM(dscdata = rd_falcon_furia_madura_uc_dsc_data());
...
err_code_t falcon_furia_madura_poll_uc_dsc_ready_for_cmd_equals_1(const phymod_access_t *pa, uint32_t timeout_ms) {
  ...
  for (loop = 0; loop < 100; loop++) {
    uint16_t rddata;
    EFUN(falcon_furia_madura_pmd_rdt_reg(pa,FALCON_FURIA_DSC_A_DSC_UC_CTRL, &rddata));
    if (rddata & 0x0080) {    /* bit 7 is uc_dsc_ready_for_cmd */
      if (rddata & 0x0040) {  /* bit 6 is uc_dsc_error_found   */
		ESTM(USR_PRINTF(("ERROR : DSC command returned error (after cmd) cmd = 0x%x, supp_info = 0x%02x !\n", rd_falcon_furia_madura_uc_dsc_gp_uc_req(), rd_falcon_furia_madura_uc_dsc_supp_info()))); 
		return(_error(ERR_CODE_UC_CMD_RETURN_ERROR));
      } 
      return (ERR_CODE_NONE);
    }     
	if(loop>10) {
		err_code = falcon_furia_madura_delay_us(10*timeout_ms);
		if(err_code) return(err_code);
	}
  }
...
#else
  { uint16_t rddata;
      USR_PRINTF(("// ATE_LOG falcon_furia_madura_poll_uc_dsc_ready_for_cmd_equals_1 begin\n"));
      if(timeout_ms < 100) {
          EFUN(falcon_furia_madura_delay_us(timeout_ms));
      } else {
          EFUN(falcon_furia_madura_delay_ms(timeout_ms));
      }
      EFUN(falcon_furia_madura_pmd_rdt_reg(pa,FALCON_FURIA_DSC_A_DSC_UC_CTRL, &rddata));
      if (rddata & 0x0080) {    /* bit 7 is uc_dsc_ready_for_cmd */
          if (rddata & 0x0040) {  /* bit 6 is uc_dsc_error_found   */
          ...
          USR_PRINTF(("// ATE_LOG falcon_furia_madura_poll_uc_dsc_ready_for_cmd_equals_1 end\n"));
          return (ERR_CODE_NONE);
      }     
      USR_PRINTF(("// ATE_LOG falcon_furia_madura_poll_uc_dsc_ready_for_cmd_equals_1 timeout\n"));
  }
#endif
  USR_PRINTF(("ERROR : DSC ready for command is not working, applying workaround and getting debug info !\n"));
  ESTM(DISP(rd_falcon_furia_madura_uc_dsc_supp_info()));
  ESTM(DISP(rd_falcon_furia_madura_uc_dsc_gp_uc_req()));
  ESTM(DISP(rd_falcon_furia_madura_dsc_state()));
  {  char status_byte;
     ESTM(status_byte = rdcv_falcon_furia_madura_status_byte());
     USR_PRINTF(("Uc Core Status Byte = %x\n",status_byte));
  }
  /* artifically terminate the command to re-enable the command interface */
  EFUN(wr_falcon_furia_madura_uc_dsc_ready_for_cmd(0x1));
...
err_code_t falcon_furia_madura_poll_dsc_state_equals_uc_tune(const phymod_access_t *pa, uint32_t timeout_ms) {
  ...
  for (loop = 0; loop < 100; loop++) {
    uint16_t dsc_state;
    ESTM(dsc_state = rd_falcon_furia_madura_dsc_state());
  ...
  ESTM(USR_PRINTF(("DSC_STATE = %d\n", rd_falcon_furia_madura_dsc_state())));
  return (_error(ERR_CODE_POLLING_TIMEOUT));          /* Error Code for polling timeout */
}    
...
err_code_t falcon_furia_madura_poll_st_afe_tx_fifo_resetb_equals_1(const phymod_access_t *pa, uint32_t timeout_ms) {
  ...
  for (loop = 0; loop <= 100; loop++) {
    ...
    ESTM(st_afe_tx_fifo_resetb = rd_falcon_furia_madura_st_afe_tx_fifo_resetb());
...
err_code_t falcon_furia_madura_reg_dump(const phymod_access_t *pa ) {
  ...
  for (addr = 0x0; addr < 0xF; addr++) {
    EFUN(falcon_furia_madura_pmd_rdt_reg(pa,addr,&rddata));
...
err_code_t falcon_furia_madura_uc_core_var_dump(const phymod_access_t *pa ) {
  ...
  for (addr = 0x0; addr < 0xFF; addr++) {  
    rddata = falcon_furia_madura_rdbc_uc_var( pa, &err_code, addr);
    if (err_code) {
      return (err_code);
    }
...
err_code_t falcon_furia_madura_uc_lane_var_dump(const phymod_access_t *pa ) {
  ...
  uint16_t    addr;
  err_code_t  err_code = ERR_CODE_NONE;
  falcon_furia_madura_uc_lane_info_st lane_info;
  ...
  USR_PRINTF(("********************************************\n")); 
    ...
    err_code = falcon_furia_madura_get_uc_ln_info(pa, &lane_info);
    if (err_code) {
      return (err_code);
    }

    for (addr = 0x0; addr < lane_info.lane_ram_size; addr++) {  
      rddata = falcon_furia_madura_rdbl_uc_var( pa, &err_code, addr);
      if (err_code) {
        return (err_code);
      }
...
err_code_t falcon_furia_madura_version(const phymod_access_t *pa, uint32_t *api_version) {
...
err_code_t falcon_furia_madura_read_event_log(const phymod_access_t *pa, uint8_t *trace_mem,enum srds_event_log_display_mode_enum display_mode) {
	...
	EFUN(falcon_furia_madura_event_log_stop( pa ));

	EFUN(falcon_furia_madura_event_log_readmem( pa, trace_mem));

	EFUN(falcon_furia_madura_event_log_display( pa, trace_mem, display_mode));
...
err_code_t falcon_furia_madura_event_log_stop(const phymod_access_t *pa ) {
	...
	EFUN(falcon_furia_madura_pmd_uc_cmd( pa, CMD_EVENT_LOG_READ, CMD_EVENT_LOG_READ_START, 10));
...
err_code_t falcon_furia_madura_event_log_readmem(const phymod_access_t *pa, uint8_t *trace_mem) {
	...
	ESTM(read_idx = rdcv_falcon_furia_madura_trace_mem_rd_idx());
	...
	do {
		/* Read Next */
		EFUN(falcon_furia_madura_pmd_uc_cmd( pa, CMD_EVENT_LOG_READ, CMD_EVENT_LOG_READ_NEXT, 10));
		...
		ESTM(*(trace_mem++) = (uint8_t) rd_falcon_furia_madura_uc_dsc_data());
		ESTM(uc_dsc_supp_info = rd_falcon_furia_madura_uc_dsc_supp_info());
	} while (uc_dsc_supp_info != 1);

	/* Read Done to resume logging  */
	EFUN(falcon_furia_madura_pmd_uc_cmd( pa, CMD_EVENT_LOG_READ, CMD_EVENT_LOG_READ_DONE, 10));
	...
	if (display_mode == EVENT_LOG_DECODED || display_mode == EVENT_LOG_HEX_AND_DECODED) {
		...
		while (addr < trace_mem_size-8) {                                                            /* last 8-byte reserved for trace memory wraparound */
			...
			if (*(trace_mem+addr) == 0xff) {                                                         /* timestamp wraparound event handling */
                ...
                falcon_furia_madura_uc_lane_idx_to_system_id(pa, uc_lane_id_str, cc);
...
err_code_t falcon_furia_madura_display_state (const phymod_access_t *pa ) {

  err_code_t err_code;
  err_code = ERR_CODE_NONE;

  if (!err_code) err_code = falcon_furia_madura_display_core_state( pa );
  if (!err_code) err_code = falcon_furia_madura_display_lane_state_hdr( pa );
  if (!err_code) err_code = falcon_furia_madura_display_lane_state( pa );

  return (err_code);
}

err_code_t falcon_furia_madura_display_config (const phymod_access_t *pa ) {

  err_code_t err_code;
  err_code = ERR_CODE_NONE;

  if (!err_code) err_code = falcon_furia_madura_display_core_config( pa );
  if (!err_code) err_code = falcon_furia_madura_display_lane_config( pa );
...
err_code_t falcon_furia_madura_force_die_temperature (const phymod_access_t *pa, int16_t die_temp) {
    ...
    EFUN(wrcv_falcon_furia_madura_temp_frc_val((uint16_t)((431045 - ((die_temp_int<<10) + (die_temp_int<<4) + (die_temp_int<<3) + (die_temp_int<<1) + die_temp_int))>>9)));
    
    return(err_code);    
}

err_code_t falcon_furia_madura_read_die_temperature (const phymod_access_t *pa, int16_t *die_temp) {
    
    uint16_t die_temp_sensor_reading;
    
    EFUN(falcon_furia_madura_pmd_uc_cmd( pa, CMD_UC_DBG,CMD_UC_DBG_DIE_TEMP,50));
    ESTM(die_temp_sensor_reading=rd_falcon_furia_madura_uc_dsc_data());
...
err_code_t falcon_furia_madura_read_die_temperature_double (const phymod_access_t *pa, USR_DOUBLE *die_temp) {
#ifdef SERDES_API_FLOATING_POINT
    /* Formula for PVTMON: T = 410.04-0.48705*reg10bit (from PVTMON Analog Module Specification v5.0 section 6.2) */
    uint16_t die_temp_sensor_reading;
    ...
    *die_temp = 410.04-0.48705* ((USR_DOUBLE)die_temp_sensor_reading); 
    ...
    return(ERR_CODE_NONE);        
#else
    USR_PRINTF((" Function 'falcon_furia_madura_read_die_temperature_double' needs SERDES_API_FLOATING_POINT defined to operate \n"));
    return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
#endif
}

err_code_t _falcon_furia_madura_read_lane_state_define(const phymod_access_t *pa, falcon_furia_madura_lane_state_define_st *istate) {

  uint8_t is_micro_stopped;
  falcon_furia_madura_lane_state_define_st state;
  ...
  state.osr_mode = *(falcon_furia_madura_osr_mode_define_st*)&osr_mode;
  }
