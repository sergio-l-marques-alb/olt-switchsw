...
#ifndef   _QMOD16_PHYREG_H_
#define   _QMOD16_PHYREG_H_
...
extern int  qmod16_reg_aer_read  (uint32_t u,qmod16_st* w,uint32_t a,uint16_t *d);
extern int  qmod16_reg_aer_write (uint32_t u,qmod16_st* w,uint32_t a,uint16_t  d);
extern int  qmod16_reg_aer_modify(uint32_t u,qmod16_st* w,uint32_t a,uint16_t d,uint16_t m);
extern int  qmod16_tsc_iblk_read(qmod16_st *pc, uint32_t addr, uint32_t *data);
extern int  qmod16_tsc_iblk_write(qmod16_st *pc, uint32_t addr, uint32_t data); 

extern void qmod16_timeout_init(soc_timeout_t *to, uint32_t usec, int min_polls);  
extern int  qmod16_timeout_check(qmod16_st* ws,soc_timeout_t *to);  
extern int  qmod16_regbit_set_wait_check(qmod16_st* w,int r,int b,int bt,int to); 
#endif   /*  _QMOD16_PHYREG_H_ */
