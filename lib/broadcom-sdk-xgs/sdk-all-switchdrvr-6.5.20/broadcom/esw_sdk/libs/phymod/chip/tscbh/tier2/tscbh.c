...
STATIC
int _tscbh_phy_rx_pmd_lock_status_get (const phymod_phy_access_t* phy, 
                                       uint32_t* pmd_rx_locked,
                                       uint32_t* pmd_lock_changed)
{
    int start_lane, num_lane, i;
    phymod_phy_access_t pm_phy_copy;
    uint32_t tmp_lock, tmp_lock_changed;
    ...
    for (i = 0; i < num_lane; i++) {
        ...
        PHYMOD_IF_ERR_RETURN
            (blackhawk_rx_pmd_lock_status_get(&pm_phy_copy.access, 
                                              &tmp_lock, 
                                              &tmp_lock_changed));
        *pmd_rx_locked &= tmp_lock;
        *pmd_lock_changed |= tmp_lock_changed;
    }
    return PHYMOD_E_NONE;
                                    ...
                                    phymod_phy_local_fault_info_t *local_fault_info)
{
    ...
    PHYMOD_IF_ERR_RETURN
        (_tscbh_phy_rx_pmd_lock_status_get(phy, 
