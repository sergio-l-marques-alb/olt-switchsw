...
#include "../../falcon_dpll/tier1/falcon2_monterey_common.h" 
...
STATIC
int _tscf_gen3_prbs_poly_phymod_to_falcon(phymod_prbs_poly_t phymod_poly, enum srds_prbs_polynomial_enum *falcon_poly)
{
    switch(phymod_poly){
...
STATIC
int _tscf_gen3_prbs_poly_tscf_to_phymod(falcon2_monterey_prbs_polynomial_type_t tscf_poly, phymod_prbs_poly_t *phymod_poly)
{
    switch(tscf_poly){
...
STATIC
int _tscf_gen3_os_mode_phymod_to_tscf(phymod_osr_mode_t phymod_os_mode, tefmod_gen3_os_mode_type *tscf_os_mode)
{
...
STATIC
int _tscf_gen3_os_mode_tscf_to_phymod(tefmod_gen3_os_mode_type tscf_os_mode, phymod_osr_mode_t *phymod_os_mode)
{
    switch(tscf_os_mode){
...
int tscf_gen3_phy_rx_slicer_position_set(const phymod_phy_access_t* phy, uint32_t flags, const phymod_slicer_position_t* position)
{
...
int tscf_gen3_phy_rx_slicer_position_get(const phymod_phy_access_t* phy, uint32_t flags, phymod_slicer_position_t* position)
{
...
int tscf_gen3_phy_rx_slicer_position_max_get(const phymod_phy_access_t* phy, uint32_t flags, const phymod_slicer_position_t* position_min, const phymod_slicer_position_t* position_max)
{
...
int tscf_gen3_phy_prbs_config_set(const phymod_phy_access_t* phy, uint32_t flags , const phymod_prbs_t* prbs)
{

    enum srds_prbs_polynomial_enum falcon_poly;
    int i, start_lane, num_lane;
    phymod_phy_access_t phy_copy;
    PHYMOD_IF_ERR_RETURN(_tscf_gen3_prbs_poly_phymod_to_falcon(prbs->poly, &falcon_poly));
    ...
    if (PHYMOD_PRBS_DIRECTION_RX_GET(flags)) {
        for (i = 0; i < num_lane; i++) {
            ...
            PHYMOD_IF_ERR_RETURN
                    (falcon2_monterey_config_rx_prbs(&phy_copy.access, falcon_poly, PRBS_INITIAL_SEED_HYSTERESIS,  prbs->invert));
        }
        ...
        for (i = 0; i < num_lane; i++) {
            ...
            PHYMOD_IF_ERR_RETURN
                    (falcon2_monterey_config_tx_prbs(&phy_copy.access, falcon_poly, prbs->invert));
        }
    } else {
        for (i = 0; i < num_lane; i++) {
            phy_copy.access.lane_mask = 0x1 << (i + start_lane);
...
int tscf_gen3_phy_prbs_config_get(const phymod_phy_access_t* phy, uint32_t flags , phymod_prbs_t* prbs)
{

    phymod_prbs_t config_tmp;
    falcon2_monterey_prbs_polynomial_type_t tscf_poly;

    if (PHYMOD_PRBS_DIRECTION_TX_GET(flags)) {
        PHYMOD_IF_ERR_RETURN(falcon2_monterey_prbs_tx_inv_data_get(&phy->access, &config_tmp.invert));
        PHYMOD_IF_ERR_RETURN(falcon2_monterey_prbs_tx_poly_get(&phy->access, &tscf_poly));
        PHYMOD_IF_ERR_RETURN(_tscf_gen3_prbs_poly_tscf_to_phymod(tscf_poly, &config_tmp.poly));
        prbs->invert = config_tmp.invert;
        prbs->poly = config_tmp.poly;
    } else if (PHYMOD_PRBS_DIRECTION_RX_GET(flags)) {
        PHYMOD_IF_ERR_RETURN(falcon2_monterey_prbs_rx_inv_data_get(&phy->access, &config_tmp.invert));
        PHYMOD_IF_ERR_RETURN(falcon2_monterey_prbs_rx_poly_get(&phy->access, &tscf_poly));
        PHYMOD_IF_ERR_RETURN(_tscf_gen3_prbs_poly_tscf_to_phymod(tscf_poly, &config_tmp.poly));
        prbs->invert = config_tmp.invert;
        prbs->poly = config_tmp.poly;
    } else {
...
int tscf_gen3_phy_prbs_enable_set(const phymod_phy_access_t* phy, uint32_t flags , uint32_t enable)
{
    ...
    if (PHYMOD_PRBS_DIRECTION_TX_GET(flags)) {
        for (i = 0; i < num_lane; i++) {
            ...
            phy_copy.access.lane_mask = 0x1 << (i + start_lane);
            PHYMOD_IF_ERR_RETURN(falcon2_monterey_tx_prbs_en(&phy_copy.access, enable));
        }
    } else if (PHYMOD_PRBS_DIRECTION_RX_GET(flags)) {
        for (i = 0; i < num_lane; i++) {
            if (!PHYMOD_LANEPBMP_MEMBER(phy->access.lane_mask, start_lane + i)) {
            ...
            phy_copy.access.lane_mask = 0x1 << (i + start_lane);
            PHYMOD_IF_ERR_RETURN(falcon2_monterey_rx_prbs_en(&phy_copy.access, enable));
        }
    } else {
        for (i = 0; i < num_lane; i++) {
            if (!PHYMOD_LANEPBMP_MEMBER(phy->access.lane_mask, start_lane + i)) {
...
int tscf_gen3_phy_prbs_enable_get(const phymod_phy_access_t* phy, uint32_t flags , uint32_t* enable)
{

    uint32_t enable_tmp;
    if (PHYMOD_PRBS_DIRECTION_TX_GET(flags)) {
        PHYMOD_IF_ERR_RETURN(falcon2_monterey_prbs_tx_enable_get(&phy->access, &enable_tmp));
        *enable = enable_tmp;
    } else if (PHYMOD_PRBS_DIRECTION_RX_GET(flags)) {
        PHYMOD_IF_ERR_RETURN(falcon2_monterey_prbs_rx_enable_get(&phy->access, &enable_tmp));
        *enable = enable_tmp;
    } else {
        PHYMOD_IF_ERR_RETURN(falcon2_monterey_prbs_tx_enable_get(&phy->access, &enable_tmp));
        *enable = enable_tmp;
...
int tscf_gen3_phy_prbs_status_get(const phymod_phy_access_t* phy, uint32_t flags, phymod_prbs_status_t* prbs_status)
{
    ...
    for (i = 0; i < num_lane; i++) {
        ...
        phy_copy.access.lane_mask = 0x1 << (i + start_lane);
        PHYMOD_IF_ERR_RETURN(falcon2_monterey_prbs_chk_lock_state(&phy_copy.access, &status));
    if (status) {
        /*next check the lost of lock and error count */
        status = 0;
        PHYMOD_IF_ERR_RETURN
                (falcon2_monterey_prbs_err_count_state(&phy_copy.access, &prbs_err_count, &status));
...
int tscf_gen3_phy_pattern_config_set(const phymod_phy_access_t* phy, const phymod_pattern_t* pattern)
{
    ...
    PHYMOD_IF_ERR_RETURN
            (falcon2_monterey_tsc_config_shared_tx_pattern_idx_set(&phy->access,
                                                                   &pattern->pattern_len));
    if (pattern->pattern_len) {
        PHYMOD_IF_ERR_RETURN
            (falcon2_monterey_config_shared_tx_pattern (&phy->access,
...
int tscf_gen3_phy_pattern_config_get(const phymod_phy_access_t* phy, phymod_pattern_t* pattern)
{

    PHYMOD_IF_ERR_RETURN
        (falcon2_monterey_tsc_config_shared_tx_pattern_idx_get(&phy->access,
                                  &pattern->pattern_len, 
                                  pattern->pattern));
...
int tscf_gen3_phy_pattern_enable_set(const phymod_phy_access_t* phy, uint32_t enable, const phymod_pattern_t* pattern)
{

    PHYMOD_IF_ERR_RETURN
        (falcon2_monterey_tx_shared_patt_gen_en(&phy->access, (uint8_t) enable, (uint8_t)pattern->pattern_len));

    return PHYMOD_E_NONE;

}

int tscf_gen3_phy_pattern_enable_get(const phymod_phy_access_t* phy, uint32_t* enable)
{

    PHYMOD_IF_ERR_RETURN
       (falcon2_monterey_tsc_tx_shared_patt_gen_en_get(&phy->access, (uint8_t *) enable));
...
int tscf_gen3_core_diagnostics_get(const phymod_core_access_t* core, phymod_core_diagnostics_t* diag)
{

    int16_t temperature;
    falcon2_monterey_read_die_temperature(&core->access,&temperature); 
    diag->temperature = temperature ;
    falcon2_monterey_tsc_read_pll_range(&core->access, &diag->pll_range);
...
int tscf_gen3_phy_pmd_info_dump(const phymod_phy_access_t* phy, const char* type)
{

    int start_lane, num_lane;
    int tmp_lane_mask;
    phymod_phy_access_t phy_copy;
    struct falcon2_monterey_detailed_lane_status_st lane_st[4] = {{0}};
    int i = 0;
    int j = 0;
    int tmp_ln_msk;
    uint32_t cmd_type;
    ...
    if (cmd_type == TEFMOD_DIAG_DSC) {
       PHYMOD_IF_ERR_RETURN
           (falcon2_monterey_display_core_state(&phy_copy.access));
       PHYMOD_IF_ERR_RETURN
           (falcon2_monterey_display_lane_state_hdr(&phy_copy.access)); 
       for (i = 0; i < 4; i++) {
            phy_copy.access.lane_mask = 0x1 << i ;
            PHYMOD_IF_ERR_RETURN
                (falcon2_monterey_lane_soft_reset_read(&phy_copy.access, &reset));
            if (!reset) {
                PHYMOD_DIAG_OUT(("    - skip lane=%0d\n", i));
                continue;
            }
            PHYMOD_IF_ERR_RETURN
                (falcon2_monterey_display_lane_state(&phy_copy.access));
       }
        ...
        for (j = 0; j < 4; j++) {
            ...
            PHYMOD_IF_ERR_RETURN
                (falcon2_monterey_log_full_pmd_state(&phy_copy.access, &lane_st[j]));
        }
        phy_copy.access.lane_mask = tmp_ln_msk;
        PHYMOD_IF_ERR_RETURN
            (falcon2_monterey_disp_full_pmd_state(&phy_copy.access, lane_st, 4));
        PHYMOD_IF_ERR_RETURN
            (falcon2_monterey_read_event_log((&phy_copy.access), trace_mem, 2));
    } else {
        for (i = 0; i < num_lane; i++) {
            phy_copy.access.lane_mask = 0x1 << (i + start_lane);
            ...
            case TEFMOD_DIAG_CFG:
                ...
                if (i == 0) {
                    ...
                    PHYMOD_IF_ERR_RETURN
                        (falcon2_monterey_display_core_config(&phy_copy.access));
                    phy_copy.access.lane_mask = tmp_lane_mask;
                }
                PHYMOD_IF_ERR_RETURN
                    (falcon2_monterey_display_lane_config(&phy_copy.access));
                break;

            case TEFMOD_DIAG_CL72:
                PHYMOD_DEBUG_ERROR((" %s:%d type = CL72\n", __func__, __LINE__));
                PHYMOD_IF_ERR_RETURN
                    (falcon2_monterey_display_cl93n72_status(&phy_copy.access));
                break;

            case TEFMOD_DIAG_DEBUG:
                PHYMOD_DEBUG_ERROR((" %s:%d type = DBG\n", __func__, __LINE__));
                PHYMOD_IF_ERR_RETURN
                    (falcon2_monterey_display_lane_debug_status(&phy_copy.access));
                break;
            ...
            default:
                PHYMOD_DEBUG_ERROR((" %s:%d type = DEF\n", __func__, __LINE__));
                 PHYMOD_IF_ERR_RETURN
                    (falcon2_monterey_display_core_state_hdr(&phy_copy.access));
                 PHYMOD_IF_ERR_RETURN
                    (falcon2_monterey_display_core_state_line(&phy_copy.access));
                PHYMOD_IF_ERR_RETURN
                    (falcon2_monterey_display_core_state(&phy_copy.access));
                PHYMOD_IF_ERR_RETURN
                    (falcon2_monterey_display_lane_state_hdr(&phy_copy.access));
...
int tscf_gen3_phy_pcs_info_dump(const phymod_phy_access_t* phy, const char* type)
{
    ...
    PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));
    PHYMOD_IF_ERR_RETURN (tefmod_gen3_diag_disp(&phy_copy.access, type));
...
int tscf_gen3_phy_fec_correctable_counter_get(const phymod_phy_access_t* phy, uint32_t* count)
{
    ...
    for (i = 0; i < num_lane; i++) {
        ...
        phy_copy.access.lane_mask = 0x1 << (i + start_lane);
        PHYMOD_IF_ERR_RETURN(tefmod_gen3_fec_correctable_counter_get(&phy_copy.access, &lane_count));
        /* Check overflow */
...
int tscf_gen3_phy_fec_uncorrectable_counter_get(const phymod_phy_access_t* phy, uint32_t* count)
{
    ...
    for (i = 0; i < num_lane; i++) {
        ...
        phy_copy.access.lane_mask = 0x1 << (i + start_lane);
        PHYMOD_IF_ERR_RETURN(tefmod_gen3_fec_uncorrectable_counter_get(&phy_copy.access, &lane_count));
        /* Check overflow */
        if (lane_count > 0xffffffff - sum) {
            sum = 0xffffffff;
            break;
...
int tscf_gen3_phy_stat_get(const phymod_phy_access_t* phy, phymod_phy_stat_t* stat)
{

    phymod_phy_access_t phy_copy;
    tefmod_gen3_encode_mode encode_mode;
    int i = 0;
    ...
    for (i = 0; i < TSCF_NOF_LANES_IN_CORE; i++) {
        ...
        PHYMOD_IF_ERR_RETURN(tefmod_gen3_encode_mode_get(&phy_copy.access, &encode_mode));
        if (encode_mode == TEFMOD_ENCODE_MODE_CL49) {
            /* CL49 has no BIP count */
            stat->pcs_bip_err_count[i] = 0;
            PHYMOD_IF_ERR_RETURN(tefmod_gen3_cl49_ber_counter_get(&phy_copy.access, &stat->pcs_ber_count[i]));
        }
        else if (encode_mode == TEFMOD_ENCODE_MODE_CL82) {
            PHYMOD_IF_ERR_RETURN(tefmod_gen3_bip_error_counter_get(&phy_copy.access, &stat->pcs_bip_err_count[i]));
            PHYMOD_IF_ERR_RETURN(tefmod_gen3_cl82_ber_counter_get(&phy_copy.access, &stat->pcs_ber_count[i]));
        }
        else {
            stat->pcs_bip_err_count[i] = 0;
...
int tscf_gen3_phy_fec_cl91_correctable_counter_get(const phymod_phy_access_t* phy, uint32_t* count)
{
    ...
    PHYMOD_IF_ERR_RETURN(tefmod_gen3_fec_cl91_correctable_counter_get(&phy_copy.access, count));
...
int tscf_gen3_phy_fec_cl91_uncorrectable_counter_get(const phymod_phy_access_t* phy, uint32_t* count)
{
    ...
    PHYMOD_IF_ERR_RETURN(tefmod_gen3_fec_cl91_uncorrectable_counter_get(&phy_copy.access, count));
...
int tscf_gen3_phy_rsfec_symbol_error_counter_get(const phymod_phy_access_t* phy,
                                                 int max_count,
                                                 int* actual_count,
                                                 uint32_t* error_count)
{
    PHYMOD_IF_ERR_RETURN
        (tefmod_gen3_rsfec_symbol_error_counter_get(&phy->access, max_count, actual_count, error_count));

    return PHYMOD_E_NONE;
}

int tscf_gen3_phy_fec_error_inject_set(const phymod_phy_access_t* phy,
                                       ...
                                       phymod_fec_error_mask_t bit_error_mask)
{
    ...
    tefmod_gen3_fec_error_mask_t fec_error_mask;
    ...
    PHYMOD_IF_ERR_RETURN
        (phymod_util_lane_config_get(&phy->access, &start_lane, &num_lane));
    PHYMOD_IF_ERR_RETURN
        (tefmod_gen3_FEC_get(&phy->access, TEFMOD_CL74, &cl74_en));
    ...
    PHYMOD_IF_ERR_RETURN(tefmod_gen3_fec_error_inject_set(&phy_copy.access, error_control_map, fec_error_mask));

    return PHYMOD_E_NONE;
}

int tscf_gen3_phy_fec_error_inject_get(const phymod_phy_access_t* phy,
                                       uint16_t* error_control_map,
                                       phymod_fec_error_mask_t* bit_error_mask)
{
    tefmod_gen3_fec_error_mask_t fec_error_mask;
    ...
    PHYMOD_IF_ERR_RETURN(tefmod_gen3_fec_error_inject_get(&phy->access, &control_map, &fec_error_mask));
