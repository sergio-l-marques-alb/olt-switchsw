...
/***********************************************************************************
 ***********************************************************************************
 *  File Name     :  blackhawk7_l8p2_prbs.c                                             *
...
err_code_t blackhawk7_l8p2_config_shared_tx_pattern(srds_access_t *sa__, uint8_t patt_length_bits, const char pattern[]) {
  ...
  uint8_t    zero_pad_len    = 0; /* suppress warnings, changed by blackhawk7_l8p2_INTERNAL_calc_patt_gen_mode_sel() */
  uint16_t   patt_gen_wr_val = 0;
  uint8_t    mode_sel        = 0; /* suppress warnings, changed by blackhawk7_l8p2_INTERNAL_calc_patt_gen_mode_sel() */

  EFUN(blackhawk7_l8p2_INTERNAL_calc_patt_gen_mode_sel(sa__, &mode_sel,&zero_pad_len,patt_length_bits));
  ...
  if ((str_len > 2) && ((USR_STRNCMP(pattern, "0x", 2)) == 0)) {
    ...
    for (i=2; i < str_len; i++) {
      if (pattern[i] != '_') {
        EFUN(blackhawk7_l8p2_INTERNAL_compute_bin(sa__, pattern[i],bin));
        ...
        if (actual_patt_len > 240) {
          ...
          return (blackhawk7_l8p2_error(sa__, ERR_CODE_CFG_PATT_PATTERN_BIGGER_THAN_MAXLEN));
        }
      }
    }
    ...
    if ((offset_len > 3)  || (actual_patt_len < patt_length_bits)) {
      ...
      return (blackhawk7_l8p2_error(sa__, ERR_CODE_CFG_PATT_LEN_MISMATCH));
    }
    else if (offset_len) {
    ...
    for (i=1; i < str_len; i++) {
      if (pattern[i] != '_') {
        EFUN(blackhawk7_l8p2_INTERNAL_pam4_to_bin(sa__, pattern[i],bin));
        ENULL_STRNCAT(patt_mod, bin, SPC_LEFT(patt_mod) - 1);
    ...
    for (i=0; i < str_len; i++) {
      if ((pattern[i] == '0') || (pattern[i] == '1')) {
        ...
        return (blackhawk7_l8p2_error(sa__, ERR_CODE_CFG_PATT_INVALID_PATTERN));
      }
    }
  ...
  for (i=0; i < 15; i++) {
    ...
    for (j=0; j < 4; j++) {
      ...
      bin[4] = '\0';
      EFUN(blackhawk7_l8p2_INTERNAL_compute_hex(sa__,bin, &hex));
    ...
    switch (i) {
      ...
      default: EFUN_PRINTF(("ERROR: Invalid write to patt_gen_seq register\n"));
               return (blackhawk7_l8p2_error(sa__, ERR_CODE_CFG_PATT_INVALID_SEQ_WRITE));
    }
  }
...
err_code_t blackhawk7_l8p2_tx_shared_patt_gen_en(srds_access_t *sa__, uint8_t enable, uint8_t patt_length_bits) {
  ...
  if (enable) {
    if ((mode_sel < 1) || (mode_sel > 6)) {
      return (blackhawk7_l8p2_error(sa__, ERR_CODE_PATT_GEN_INVALID_MODE_SEL));
    }
...
err_code_t blackhawk7_l8p2_prbs_err_count_state(srds_access_t *sa__, uint32_t *prbs_err_cnt, uint8_t *lock_lost) {
  ...
  if(!prbs_err_cnt || !lock_lost) {
    ...
    EFUN(blackhawk7_l8p2_prbs_err_count_ll(sa__, prbs_err_cnt));
...
err_code_t blackhawk7_l8p2_config_prbs_chk_hw_timer(srds_access_t *sa__, uint16_t time_ms, uint16_t *time_ms_adjusted) {
    ...
    EFUN(blackhawk7_l8p2_INTERNAL_get_prbs_timeout_count_from_time(time_ms, time_ms_adjusted, &prbs_chk_hw_timer_ctrl));
...
const char* blackhawk7_l8p2_e2s_prbs_mode_enum[22] = {
    " PRBS_7",
    " PRBS_9",
    "PRBS_11",
    "PRBS_15",
...
err_code_t blackhawk7_l8p2_display_detailed_prbs_state(srds_access_t *sa__) {
    ...
    ESTM_PRINTF(("  %d ",blackhawk7_l8p2_acc_get_lane(sa__)));
    ...
    if(enabled) {
        ...
        EFUN(blackhawk7_l8p2_get_tx_prbs_config(sa__, &prbs_poly_mode, &prbs_inv));
        if(prbs_poly_mode == PRBS_UNKNOWN) {
            prbs_string = "UNKNOWN";
        }
        else {
            prbs_string =  (char *)blackhawk7_l8p2_e2s_prbs_mode_enum[prbs_poly_mode];
        }
    ...
    if(enabled) {
        ...
        uint8_t prbs_inv = 0;
        const char *prbs_string;
        EFUN(blackhawk7_l8p2_get_rx_prbs_config(sa__, &prbs_poly_mode, &prbs_checker_mode, &prbs_inv));
        if(prbs_poly_mode == PRBS_UNKNOWN) {
            prbs_string = "UNKNOWN";
        }
        else {
    ...
    EFUN(blackhawk7_l8p2_prbs_err_count_state(sa__,&err_cnt,&lock_lost));
...
err_code_t blackhawk7_l8p2_prbs_error_analyzer_reset(srds_access_t *sa__) {
    ...
    EFUN(blackhawk7_l8p2_clear_dac_pattern_memory(sa__));  /* Clear DAC pattern memory */
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_l8p2_prbs_error_analyzer_config(srds_access_t *sa__, blackhawk7_l8p2_prbs_err_analyzer_config_status_st *err_analyzer) {
    ...
    if ((err_analyzer->prbs_err_fec_size < BLACKHAWK7_L8P2_PRBS_ERR_ANALYZER_FEC_SIZE_MIN) || (err_analyzer->prbs_err_fec_size > BLACKHAWK7_L8P2_PRBS_ERR_ANALYZER_FEC_SIZE_MAX)) { 
        return (blackhawk7_l8p2_error(sa__, ERR_CODE_INVALID_PRBS_ERR_ANALYZER_FEC_SIZE));
    }

    fec_size = (uint8_t)((err_analyzer->prbs_err_fec_size + BLACKHAWK7_L8P2_PRBS_FEC_FRAME_SIZE_ROUND_UP) / BLACKHAWK7_L8P2_PRBS_FEC_FRAME_SIZE_MULTIPLIER);

    fec_size_bits = (uint16_t)(fec_size * BLACKHAWK7_L8P2_PRBS_FEC_FRAME_SIZE_MULTIPLIER);
    ...
    switch (err_analyzer->prbs_err_aggregate_mode) {
        case SRDS_IEEE_50GE: /* 50G w/ Bit Muxing 2:1 and No Code Word Interleaving 50GE/100GE 56G BHK */
           ...
           err_analyzer->fec_size_frac = BLACKHAWK7_L8P2_PRBS_FEC_SIZE_FRAC_1_8;
        ...
        case SRDS_BHK_200GE:              /* 200G w/ Bit Muxing 2:1 and 2 Code Word Interleaving 200GE/400GE 56G BHK */
           ...
           err_analyzer->fec_size_frac = BLACKHAWK7_L8P2_PRBS_FEC_SIZE_FRAC_1_4;
           EFUN(wr_tlb_err_symbol_msb_lsb_group(0x1));
           EFUN(wr_tlb_err_fec_msb_en(0x1));
...
err_code_t blackhawk7_l8p2_get_prbs_error_analyzer_config(srds_access_t *sa__, blackhawk7_l8p2_prbs_err_analyzer_config_status_st *err_analyzer) {
    ...
    err_analyzer->prbs_err_fec_size = (uint16_t)(prbs_err_fec_size * BLACKHAWK7_L8P2_PRBS_FEC_FRAME_SIZE_MULTIPLIER);
...
err_code_t blackhawk7_l8p2_display_prbs_error_analyzer_config(srds_access_t *sa__, blackhawk7_l8p2_prbs_err_analyzer_config_status_st *err_analyzer, uint32_t timeout_s) {
    ...
    {
        /* EFUN_PRINTF(("%u %hu %hhu %u %hhu %hhu %hhu %08x %08x\n",   timeout_s, err_analyzer->prbs_err_fec_size, err_analyzer->fec_size_frac, err_analyzer->prbs_err_aggregate_mode, err_analyzer->lanes_active, err_analyzer->diag_mode, blackhawk7_l8p2_get_lane(sa__), err_analyzer->pp_field2, err_analyzer->pp_field1));*/
        EFUN_PRINTF(("%04d\n", timeout_s));
        EFUN_PRINTF(("%04d\n", err_analyzer->prbs_err_fec_size));
        EFUN_PRINTF(("%04d\n", err_analyzer->fec_size_frac));
        EFUN_PRINTF(("%x\n", err_analyzer->prbs_err_aggregate_mode));
        EFUN_PRINTF(("%02x\n", err_analyzer->lanes_active));
        EFUN_PRINTF(("%x\n", err_analyzer->diag_mode));
        EFUN_PRINTF(("%x\n", blackhawk7_l8p2_get_lane(sa__)));
        EFUN_PRINTF(("%x\n", err_analyzer->pp_field2));
        EFUN_PRINTF(("%x\n", err_analyzer->pp_field1));
}
    else
    {
        EFUN_PRINTF(("\n ****************************************************************************** \n"));
...
err_code_t blackhawk7_l8p2_clear_dac_pattern_memory(srds_access_t *sa__) {
    ...
    EFUN(blackhawk7_l8p2_INTERNAL_clk4sync_div2_sequence(sa__, 0));
    ...
    EFUN(blackhawk7_l8p2_INTERNAL_clk4sync_div2_sequence(sa__, 1));
    BLACKHAWK7_L8P2_UAPI_NOT_AMS_A0_ERR   /* if not BLACKHAWK7_V1L8P2, print error */
...
err_code_t blackhawk7_l8p2_get_prbs_error_analyzer_lane_err_count(srds_access_t *sa__, blackhawk7_l8p2_prbs_err_analyzer_config_status_st *err_analyzer, uint8_t lane) {
    ...
    EFUN(blackhawk7_l8p2_read_dac_pattern_memory(sa__,dacmem,(uint8_t)(4*lane),4));
    ...
    blackhawk7_l8p2_read_dac_pattern_memory(sa__,dacmem2,(uint8_t)(4*(num_lanes+1)),2);
    ...
    if (PRBS_VERBOSE > 2) {
        EFUN_PRINTF(("\n\n\t=====> DEBUG INFO (start) [Core = %d, Lane = %d]\n", blackhawk7_l8p2_acc_get_core(sa__), lane));
        EFUN_PRINTF(("\n\t << Frame Errors accumulated in the DAC Pattern Memory >>\n"));
        EFUN_PRINTF(("\t FEC frames with    0 Errors = %u\n",err_analyzer->prbs_frames_all));
        for (i=0; i<BLACKHAWK7_L8P2_PRBS_NUM_OF_ERROR_ANALYZER_COUNTERS; i++) {
            if (i < (BLACKHAWK7_L8P2_PRBS_NUM_OF_ERROR_ANALYZER_COUNTERS-1)) {
                EFUN_PRINTF(("\t FEC frames with   %2d Errors = %u\n",i+1,err_analyzer->prbs_errcnt[i]));
            }
            else {
                EFUN_PRINTF(("\t FEC frames with > %2d Errors = %u\n",(BLACKHAWK7_L8P2_PRBS_NUM_OF_ERROR_ANALYZER_COUNTERS-1),err_analyzer->prbs_errcnt[i]));
            }
        }
    }

    /* Calculating cumulative error count for respective values of "t" */ 
    for (i=(BLACKHAWK7_L8P2_PRBS_NUM_OF_ERROR_ANALYZER_COUNTERS-1); i >= 0; i--) {
        if (i!=(BLACKHAWK7_L8P2_PRBS_NUM_OF_ERROR_ANALYZER_COUNTERS-1)) {
...
err_code_t blackhawk7_l8p2_get_prbs_error_analyzer_err_count(srds_access_t *sa__, blackhawk7_l8p2_prbs_err_analyzer_config_status_st *err_analyzer) {
    ...
    ESTM(err_analyzer->pp_field2 = blackhawk7_l8p2_acc_get_lane(sa__)); 
    EFUN(blackhawk7_l8p2_INTERNAL_get_num_bits_per_ms(sa__, 1, &err_analyzer->pp_field1));
    ...
    EFUN(blackhawk7_l8p2_get_prbs_error_analyzer_lane_err_count(sa__, err_analyzer, lane));
...
err_code_t blackhawk7_l8p2_get_prbs_error_analyzer_aggregate_err_count(srds_access_t *sa__, blackhawk7_l8p2_prbs_err_analyzer_config_status_st *err_analyzer) {
    ...
    EFUN(blackhawk7_l8p2_get_prbs_error_analyzer_lane_err_count(sa__, err_analyzer, num_lanes)); /* For aggregate counters, just pass the num_lanes to point to the correct memory location in the DAC */ 

    return (ERR_CODE_NONE);
}

static err_code_t _blackhawk7_l8p2_display_fec_frame_error(srds_access_t *sa__, uint8_t errs_per_frame, uint32_t num_frame_errs) {
...
err_code_t blackhawk7_l8p2_display_prbs_error_analyzer_err_count(srds_access_t *sa__, blackhawk7_l8p2_prbs_err_analyzer_config_status_st *err_analyzer) {
    ...
    if (err_analyzer->diag_mode)
    {
        for (i=0; i<(BLACKHAWK7_L8P2_PRBS_NUM_OF_ERROR_ANALYZER_COUNTERS-1); i++) {
            EFUN_PRINTF(("%08x,", err_analyzer->prbs_errcnt[i]));
        }
        EFUN_PRINTF(("%08x\n", err_analyzer->prbs_errcnt[BLACKHAWK7_L8P2_PRBS_NUM_OF_ERROR_ANALYZER_COUNTERS-1]));
        EFUN_PRINTF(("%08x\n", err_analyzer->prbs_frames_all));
    }
    else
    {
        ...
        for (i=0; i<BLACKHAWK7_L8P2_PRBS_NUM_OF_ERROR_ANALYZER_COUNTERS; i++) {
            EFUN(_blackhawk7_l8p2_display_fec_frame_error(sa__, i, err_analyzer->prbs_errcnt[i]));
        }
        EFUN_PRINTF((" -------------------------------------------------------------\n"));
    } 
...
err_code_t blackhawk7_l8p2_display_prbs_error_analyzer_struct(srds_access_t *sa__, blackhawk7_l8p2_prbs_err_analyzer_config_status_st *err_analyzer, uint32_t timeout_s) {
    ...
    /* Display PRBS Error Analyzer Config */
    EFUN(blackhawk7_l8p2_display_prbs_error_analyzer_config(sa__, err_analyzer, timeout_s));   
    /* Display PRBS Error Analyzer Err_Counts */
    EFUN(blackhawk7_l8p2_display_prbs_error_analyzer_err_count(sa__, err_analyzer));
    if (err_analyzer->diag_mode)
    {
        /* End of data dump */
...
err_code_t blackhawk7_l8p2_prbs_error_analyzer_aggregate_config(srds_access_t *sa__, blackhawk7_l8p2_prbs_err_analyzer_config_status_st *err_analyzer) {
    ...
    for(lane = 0;lane<num_lanes;lane++) {
        if ((err_analyzer->lane_mask_aggregate >> lane) & 0x1) {
            EFUN(blackhawk7_l8p2_acc_set_lane(sa__,lane));
            EFUN(blackhawk7_l8p2_prbs_error_analyzer_config(sa__, err_analyzer));  /* Configure PRBS Error Analyzer */
...
err_code_t blackhawk7_l8p2_display_prbs_error_analyzer_proj(srds_access_t *sa__, uint16_t prbs_err_fec_size, uint32_t timeout_s) {
    ...
    ESTM(lane_mask_aggregate = (uint8_t)(0x1 << (blackhawk7_l8p2_acc_get_lane(sa__))));

    /* Call the aggregate function to display error analyzer projections */
    EFUN(blackhawk7_l8p2_display_aggregate_prbs_error_analyzer_proj(sa__, prbs_err_fec_size, SRDS_IEEE_50GE, lane_mask_aggregate, timeout_s, 0x0));
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_l8p2_display_aggregate_prbs_error_analyzer_proj(srds_access_t *sa__, uint16_t prbs_err_fec_size, enum srds_prbs_error_analyzer_aggregate_mode_enum prbs_err_aggregate_mode, uint8_t lane_mask_aggregate, uint32_t timeout_s, uint8_t diag_mode) {
    ...
    uint8_t  prbs_lock;
    int8_t i;
    blackhawk7_l8p2_prbs_err_analyzer_config_status_st err_analyzer;
    ...
    ENULL_MEMSET(&err_analyzer, 0, sizeof(blackhawk7_l8p2_prbs_err_analyzer_config_status_st));
    ...
    EFUN(blackhawk7_l8p2_prbs_error_analyzer_reset(sa__));  /* Reset PRBS Error Analyzer and clear DAC */
    EFUN(blackhawk7_l8p2_prbs_error_analyzer_aggregate_config(sa__, &err_analyzer));  /* Configure PRBS Error Analyzer Aggregation Mode*/
    ...
    for(lane = 0;lane<num_lanes;lane++) {
        if ((err_analyzer.lane_mask_aggregate >> lane) & 0x1) {
            EFUN(blackhawk7_l8p2_acc_set_lane(sa__,lane));
            EFUN(blackhawk7_l8p2_prbs_error_analyzer_start(sa__));  /* Start PRBS Error Analyzer */
        }
    }

    EFUN(blackhawk7_l8p2_INTERNAL_seconds_to_displayformat(timeout_s, &hrs, &mins, &secs));
    ...
    for(lane = 0;lane<num_lanes;lane++) {
        if ((err_analyzer.lane_mask_aggregate >> lane) & 0x1) {
            ...
            EFUN(blackhawk7_l8p2_get_prbs_error_analyzer_err_count(sa__, &err_analyzer));
            /* Display PRBS Error Analyzer Config */
            EFUN(blackhawk7_l8p2_display_prbs_error_analyzer_struct(sa__, &err_analyzer, timeout_s));
            EFUN(blackhawk7_l8p2_prbs_error_analyzer_compute_proj(sa__, err_analyzer, timeout_s));
        }
    }
    ...
    { 
       ...
       err_analyzer.prbs_frames_all = 0;
       EFUN(blackhawk7_l8p2_get_prbs_error_analyzer_aggregate_err_count(sa__, &err_analyzer));
       EFUN(blackhawk7_l8p2_display_prbs_error_analyzer_aggregate_err_count(sa__, &err_analyzer));
       EFUN(blackhawk7_l8p2_prbs_error_analyzer_compute_proj(sa__, err_analyzer, timeout_s));
    }
...
err_code_t blackhawk7_l8p2_prbs_error_analyzer_report_proj(srds_access_t *sa__, blackhawk7_l8p2_prbs_err_analyzer_config_status_st err_analyzer, uint32_t timeout_s, blackhawk7_l8p2_prbs_err_analyzer_report_st *err_analyzer_report) {
    ...
    USR_DOUBLE meas_fec_ber[BLACKHAWK7_L8P2_PRBS_NUM_OF_ERROR_ANALYZER_COUNTERS];
    ...
    ENULL_MEMSET(err_analyzer_report, 0, sizeof(blackhawk7_l8p2_prbs_err_analyzer_report_st));
    EFUN(blackhawk7_l8p2_INTERNAL_get_num_bits_per_ms(sa__, 1, &num_bits_per_ms));

    if (err_analyzer.fec_size_frac == BLACKHAWK7_L8P2_PRBS_FEC_SIZE_FRAC_1_8)
        overlap_factor = 8;
    else if (err_analyzer.fec_size_frac == BLACKHAWK7_L8P2_PRBS_FEC_SIZE_FRAC_1_4)
        overlap_factor = 4;
    else if (err_analyzer.fec_size_frac == BLACKHAWK7_L8P2_PRBS_FEC_SIZE_FRAC_1_2)
        overlap_factor = 2;
    else
        overlap_factor = 1;
    ...
    start_idx = 0;
    stop_idx  = 0;
    if (err_analyzer.prbs_errcnt[BLACKHAWK7_L8P2_PRBS_NUM_OF_ERROR_ANALYZER_COUNTERS-1] != 0xFFFFFFFF) {
        stop_idx  = BLACKHAWK7_L8P2_PRBS_NUM_OF_ERROR_ANALYZER_COUNTERS-1;
    }
    for (i=BLACKHAWK7_L8P2_PRBS_NUM_OF_ERROR_ANALYZER_COUNTERS; i > 0; i--) {
        if (err_analyzer.prbs_errcnt[i - 1] != 0xFFFFFFFF) {
    ...
    if (delta_n >= 2) {                                                              /* There are at least 2 points to trace a line for extrapolation */
        for(i = start_idx; i < stop_idx; i++) {
            x    = (USR_DOUBLE) (i);
            y    = pow((-log10(meas_fec_ber[i])),BLACKHAWK7_L8P2_PRBS_ERR_ANALYZER_LINEARITY_ADJUST);
    ...
    if (delta_n >= 2) {
        ...
        proj_ber[15] = pow(proj_ber[15],1.0/BLACKHAWK7_L8P2_PRBS_ERR_ANALYZER_LINEARITY_ADJUST);
...
#ifdef SERDES_API_FLOATING_POINT
    INIT_SRDS_ERR_CODE
    blackhawk7_l8p2_prbs_err_analyzer_report_st err_analyzer_rep;
...
err_code_t blackhawk7_l8p2_config_BER_HW_mode(srds_access_t *sa__, uint16_t time_ms, struct ber_data_st *ber_data, struct prbs_chk_hw_timer_ctrl_st *prbs_chk_hw_timer_ctrl) {
    ...
    EFUN(blackhawk7_l8p2_INTERNAL_prbs_chk_cdr_lock_lost(sa__, &ber_data->cdrlcklost));

    /* save prbs hardware timer config registers and prbs mode */
    EFUN(blackhawk7_l8p2_get_prbs_chk_hw_timer_ctrl(sa__, prbs_chk_hw_timer_ctrl));

    /* Configure hardware timer to count prbs errors */
    EFUN(blackhawk7_l8p2_config_prbs_chk_hw_timer(sa__, time_ms, &prbs_chk_hw_timer_ctrl->time_ms_adjusted_bak));
    ...
    EFUN(blackhawk7_l8p2_INTERNAL_hw_timer_sync_delay(sa__));
...
err_code_t blackhawk7_l8p2_get_BER_data_HW_mode(srds_access_t *sa__, struct ber_data_st *ber_data, struct prbs_chk_hw_timer_ctrl_st *prbs_chk_hw_timer_ctrl) {
    ...
    if (prbs_chk_lock_state == PRBS_CHECKER_NOT_ENABLED) {
        ...
        do {
            ESTM(prbs_chk_timeout = rd_prbs_chk_done_lh());
            if (prbs_chk_timeout) {
                break;
            }
            EFUN(USR_DELAY_MS(1));
...
*/  
    ...
    /* restore prbs hardware timer config registers */
    EFUN(blackhawk7_l8p2_set_prbs_chk_hw_timer_ctrl(sa__, prbs_chk_hw_timer_ctrl));
