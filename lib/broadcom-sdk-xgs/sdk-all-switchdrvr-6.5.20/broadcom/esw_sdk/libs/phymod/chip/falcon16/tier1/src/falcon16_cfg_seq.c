...
err_code_t falcon16_tsc_get_eye_margin_est (srds_access_t *sa__, int *left_eye_mUI, int *right_eye_mUI, int *upper_eye_mV, int *lower_eye_mV){
    ...
    *left_eye_mUI = falcon16_tsc_INTERNAL_eye_to_mUI(sa__, rdv_usr_sts_heye_left());
    if(__err) return(__err);
    *right_eye_mUI = falcon16_tsc_INTERNAL_eye_to_mUI(sa__, rdv_usr_sts_heye_right());
    if(__err) return(__err);
    *upper_eye_mV = falcon16_tsc_INTERNAL_eye_to_mV(sa__, rdv_usr_sts_veye_upper(), ladder_range);
    if(__err) return(__err);
    *lower_eye_mV = falcon16_tsc_INTERNAL_eye_to_mV(sa__, rdv_usr_sts_veye_lower(), ladder_range);
    if(__err) return(__err);
...
err_code_t falcon16_clk4pcs_3bit_40t_config(srds_access_t *sa__)
{
    ...
    __err = wrc_ams_pll_set_clk4pcs(0x3);
    if(__err) return(__err);
...
err_code_t falcon16_tsc_txfir_post_uc_set(srds_access_t *sa__, uint8_t tx_post)
{
  ...
  __err = falcon16_tsc_wrbc_uc_var(sa__, FALCON16_TXFIR_POST_UCRAM_ADDR, tx_post);
  if(__err) return(__err);
...
err_code_t falcon16_tsc_txfir_post_uc_get(srds_access_t *sa__, uint8_t* tx_post)
{
    ...
    *tx_post = falcon16_tsc_rdbcs_uc_var(sa__, &__err, FALCON16_TXFIR_POST_UCRAM_ADDR);
    if(__err) return(__err);
