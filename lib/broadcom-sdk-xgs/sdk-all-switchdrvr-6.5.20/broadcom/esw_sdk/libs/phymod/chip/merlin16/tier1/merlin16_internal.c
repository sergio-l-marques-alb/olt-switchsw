...
static merlin16_info_t merlin16_info = {0};

merlin16_info_t *merlin16_INTERNAL_get_merlin16_info_ptr(void) {
    return &merlin16_info;
}

err_code_t merlin16_INTERNAL_match_ucode_from_info(srds_access_t *sa__) {
    ...
    if (ucode_version == merlin16_info_ptr->ucode_version) {
        return(ERR_CODE_NONE);
    } else {
        EFUN_PRINTF(("ERROR:  ucode version of the current thread not matching with stored merlin16_info->ucode_version, Expected 0x%08X, but received 0x%08X.\n",
                    merlin16_info_ptr->ucode_version, ucode_version));
        return(ERR_CODE_UCODE_VERIFY_FAIL);
    }
}

err_code_t merlin16_INTERNAL_verify_merlin16_info(merlin16_info_t const *test_info, srds_access_t *sa__)
{
    err_code_t err_code = ERR_CODE_NONE;
    
    if (test_info->signature != SRDS_INFO_SIGNATURE) {
        EFUN_PRINTF(("ERROR:  Mismatch in merlin16_info signature.  Expected 0x%08X, but received 0x%08X.\n",
...
int merlin16_osr_mode_enum_to_int_x1000(uint8_t osr_mode) {
    switch(osr_mode) {
    case MERLIN16_OSX1:      return 1000;
    case MERLIN16_OSX2:      return 2000;
    case MERLIN16_OSX4:      return 4000;
    case MERLIN16_OSX3:      return 3000;
    case MERLIN16_OSX3P3:    return 3300;
    case MERLIN16_OSX5:      return 5000;
    case MERLIN16_OSX7P5:    return 7500;
    case MERLIN16_OSX8P25:   return 8250;
    case MERLIN16_OSX10:     return 10000;
    case MERLIN16_OSX8:      return 8000;
    default:                    return 1000;
    }
}

err_code_t merlin16_INTERNAL_get_num_bits_per_ms(srds_access_t *sa__, uint32_t *num_bits_per_ms) {
    uint8_t osr_mode = 0;
#if defined(rd_rx_pam4_mode)
    uint8_t pam4_mode = 0;
#endif
    struct merlin16_uc_core_config_st core_config = {{0}};

    { 
       merlin16_osr_mode_st osr_mode_st;
       ENULL_MEMSET(&osr_mode_st, 0, sizeof(merlin16_osr_mode_st));
       EFUN(merlin16_INTERNAL_get_osr_mode(sa__, &osr_mode_st));
    ...
    ESTM(pam4_mode = rd_rx_pam4_mode());
        ...
        *num_bits_per_ms = (((uint32_t)core_config.vco_rate_in_Mhz * 100000UL) / merlin16_osr_mode_enum_to_int_x1000(osr_mode))*10;
...
err_code_t merlin16_INTERNAL_display_BER(srds_access_t *sa__, uint16_t time_ms) {
    char string[10];
    EFUN(merlin16_INTERNAL_get_BER_string(sa__,time_ms,string));
    USR_PRINTF(("%s",string));
    return(ERR_CODE_NONE);
}

err_code_t merlin16_INTERNAL_get_BER_string(srds_access_t *sa__, uint16_t time_ms, char *string) {
    char string2[3];
    ...
    EFUN(merlin16_INTERNAL_get_BER_data(sa__, time_ms, &ber_data_local));
    ...
    if(ber_data_local.lcklost == 1) {    /* lcklost = 1 */
        ...
        if(COMPILER_64_GE(ber_data_local.num_errs, ber_data_local.num_bits)) {
            ...
            while(1) {
                ...
                div = (uint16_t) COMPILER_64_LO( temp_dividend );
...
err_code_t merlin16_INTERNAL_get_BER_data(srds_access_t *sa__, uint16_t time_ms, struct ber_data_st *ber_data) {
    uint8_t lcklost = 0;
    uint32_t err_cnt= 0;
    ...
    EFUN(merlin16_prbs_err_count_state(sa__, &err_cnt,&lcklost)); /* clear error counters */

        EFUN(USR_DELAY_MS(time_ms));
        EFUN(merlin16_prbs_err_count_state(sa__, &err_cnt,&lcklost));
        ber_data->lcklost = lcklost;
        if(ber_data->lcklost == 0) {
            uint32_t num_bits_per_ms=0;
            EFUN(merlin16_INTERNAL_get_num_bits_per_ms(sa__,&num_bits_per_ms));
...
uint8_t merlin16_INTERNAL_stop_micro(srds_access_t *sa__, uint8_t graceful, err_code_t *err_code_p) {
   ...
   if (!(stop_state & 0x7F)) {
       if (graceful) {
           EPFUN2((merlin16_stop_rx_adaptation(sa__, 1)),0xFF);
       }
       else {
           EPFUN2((merlin16_pmd_uc_control(sa__, CMD_UC_CTRL_STOP_IMMEDIATE,GRACEFUL_STOP_TIME)),0xFF);
       }
   }
...
err_code_t merlin16_INTERNAL_read_event_log_with_callback(srds_access_t *sa__,
    ...
    if (bypass_micro) {
        ...
        EFUN(merlin16_pmd_uc_cmd(sa__, CMD_EVENT_LOG_READ, CMD_EVENT_LOG_READ_START, GRACEFUL_STOP_TIME));
    ...
    if (merlin16_info_ptr->trace_memory_descending_writes) {
        ...
        EFUN(merlin16_INTERNAL_rdblk_uc_generic_ram(sa__,
                                                  merlin16_info_ptr->trace_mem_ram_base + merlin16_info_ptr->grp_ram_size*micro_num,
                                                  merlin16_info_ptr->trace_mem_ram_size,
                                                  rd_idx,
                                                  merlin16_info_ptr->trace_mem_ram_size,
                                                  arg,
    ...
    if (!bypass_micro) {
        ...
        EFUN(merlin16_pmd_uc_cmd(sa__, CMD_EVENT_LOG_READ, CMD_EVENT_LOG_READ_DONE, 50));
    }
    EFUN(merlin16_set_lane(sa__,current_lane));
    return(ERR_CODE_NONE);
}
...
/*-----------------------------------------------*/
err_code_t merlin16_INTERNAL_get_eye_margin_est(srds_access_t *sa__, uint16_t *left_eye_mUI, uint16_t *right_eye_mUI, uint16_t  *upper_eye_mV, uint16_t *lower_eye_mV) {
  uint8_t ladder_range = 0;
  ESTM(ladder_range = rd_p1_thresh_sel());
  ESTM(*left_eye_mUI  = merlin16_INTERNAL_eye_to_mUI(sa__, rdv_usr_sts_heye_left()));
  ESTM(*right_eye_mUI = merlin16_INTERNAL_eye_to_mUI(sa__, rdv_usr_sts_heye_right()));
  ESTM(*upper_eye_mV = merlin16_INTERNAL_eye_to_mV(sa__, rdv_usr_sts_veye_upper(), ladder_range));
  ESTM(*lower_eye_mV = merlin16_INTERNAL_eye_to_mV(sa__, rdv_usr_sts_veye_lower(), ladder_range));
...
err_code_t merlin16_INTERNAL_get_osr_mode(srds_access_t *sa__, merlin16_osr_mode_st *imode) {
...
#ifdef wr_ams_rx_rxclk_div2p5    /* Analog 2p5 OSR mode */
  {
      ...
      ESTM(osr_2p5_pwrup = rd_ams_tx_spare_58());
      if ((mode.tx == MERLIN16_OSX1) && (osr_2p5 == 1)  && (osr_2p5_pwrup == 1)) {
          mode.tx = MERLIN16_OSX2P5;
      }
      ESTM(osr_2p5       = rd_ams_rx_rxclk_div2p5());
      ESTM(osr_2p5_pwrup = rd_ams_rx_spare_134());
      if ((mode.rx == MERLIN16_OSX1) && (osr_2p5 == 1)  && (osr_2p5_pwrup == 1)) {
          mode.rx = MERLIN16_OSX2P5;
      }
  }
#endif
  *imode = mode;
...
#else
#error "Unknown core for merlin16_INTERNAL_pmd_lock_status()"    
#endif
    return(ERR_CODE_NONE);
}
...
err_code_t merlin16_INTERNAL_read_lane_state(srds_access_t *sa__, merlin16_lane_state_st *istate) {
  ...
  EFUN(merlin16_INTERNAL_pmd_lock_status(sa__,&state.rx_lock, &state.rx_lock_chg));
  {
      ...
      state.stop_state = merlin16_INTERNAL_stop_micro(sa__,state.rx_lock,&err_code);
  ...
  EFUN(merlin16_INTERNAL_get_eye_margin_est(sa__, &state.heye_left, &state.heye_right, &state.veye_upper, &state.veye_lower));
  ...
  ESTM(state.link_time = (((uint32_t)rdv_usr_sts_link_time())*8)/10);     /* convert from 80us to 0.1 ms units */
  {
      ...
      EFUN(merlin16_INTERNAL_pmd_lock_status(sa__,&rx_lock_at_end, &rx_lock_chg_at_end));
...
err_code_t merlin16_INTERNAL_display_lane_state_no_newline(srds_access_t *sa__) {     
  ...
  EFUN(merlin16_INTERNAL_display_BER(sa__,100));
...
err_code_t merlin16_INTERNAL_read_core_state(srds_access_t *sa__, merlin16_core_state_st *istate) {  
  ...
  ESTM(merlin16_version(&state.api_version));
...
err_code_t _merlin16_get_refclk_in_hz(enum merlin16_pll_refclk_enum refclk, uint32_t *refclk_in_hz) {
    switch (refclk) {
...
static err_code_t _merlin16_get_refclk_from_div_vco(uint32_t *refclk_freq_hz, enum merlin16_pll_div_enum div, uint32_t vco_freq_khz, enum merlin16_pll_option_enum pll_option) {
...
err_code_t merlin16_INTERNAL_get_vco_from_refclk_div(uint32_t refclk_freq_hz, enum merlin16_pll_div_enum div, uint32_t *vco_freq_khz, enum merlin16_pll_option_enum pll_option) {
    uint16_t numerator, denominator;
    EFUN(_merlin16_get_divisor_ratio(div, &numerator, &denominator));
    if (pll_option == MERLIN16_PLL_OPTION_REFCLK_DOUBLER_EN) refclk_freq_hz *= 2;
    if (pll_option == MERLIN16_PLL_OPTION_REFCLK_DIV2_EN)    refclk_freq_hz /= 2;
...
static err_code_t _merlin16_get_div(uint32_t refclk_freq_hz, uint32_t vco_freq_khz, enum merlin16_pll_div_enum *div, enum merlin16_pll_option_enum pll_option) {
    int32_t vco_freq_khz_min_error = 0x7FFFFFFF;
    uint8_t i;
    for (i=0; i<_merlin16_div_candidates_count; i++) {
        uint32_t actual_vco_freq_khz = 0;
        int32_t  vco_freq_khz_error;
        EFUN(merlin16_INTERNAL_get_vco_from_refclk_div(refclk_freq_hz, _merlin16_div_candidates[i], &actual_vco_freq_khz, pll_option));
...
err_code_t merlin16_INTERNAL_resolve_pll_parameters(enum merlin16_pll_refclk_enum refclk,
    ...
    enum merlin16_pll_div_enum auto_div = MERLIN16_PLL_DIV_UNKNOWN;
    ...
    if (pll_option == MERLIN16_PLL_OPTION_DISABLE_VERIFY) {
    ...
    if (refclk == MERLIN16_PLL_REFCLK_UNKNOWN) {
        ...
        EFUN(_merlin16_get_refclk_from_div_vco(refclk_freq_hz, *div, *vco_freq_khz, pll_option));
    } else {
        EFUN(_merlin16_get_refclk_in_hz(refclk, refclk_freq_hz));
    }
    ...
    if (*vco_freq_khz == 0) {
        ...
        EFUN(merlin16_INTERNAL_get_vco_from_refclk_div(*refclk_freq_hz, *div, vco_freq_khz, pll_option));
    }
     ...
     */
    EFUN(_merlin16_get_div(*refclk_freq_hz, *vco_freq_khz, &auto_div, pll_option));
    if (*div == MERLIN16_PLL_DIV_UNKNOWN) {
...
int merlin16_INTERNAL_get_endian_offset(uint8_t *addr) {
...
err_code_t merlin16_INTERNAL_rdblk_callback(void *arg, uint8_t byte_count, uint16_t data) {
    ...
    int endian_offset = 0;
    ESTM(endian_offset = merlin16_INTERNAL_get_endian_offset(cast_arg->mem_ptr));
...
uint8_t merlin16_INTERNAL_grp_idx_from_lane(uint8_t lane) {
    return(0);
}
