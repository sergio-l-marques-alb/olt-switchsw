/*----------------------------------------------------------------------
 * $Id: tbhmod_cfg_seq.c,v 1.8 Broadcom SDK $
...
static
float _tbhmod_pcs_vco_to_clk_period(uint32_t vco, int os_mode)
{
...
int tbhmod_ts_offset_rx_set(PHYMOD_ST* pc, uint32_t vco, int os_mode, int tbl_ln, uint32_t *table)
{
    PMD_X1_PM_TMR_OFFSr_t PMD_X1_PM_TMR_OFFSr_reg;
...
#ifdef SERDES_API_FLOATING_POINT
    rx_clk_period = _tbhmod_pcs_vco_to_clk_period(vco, os_mode);
#endif
...
int tbhmod_init_pcs_ilkn(PHYMOD_ST* pc)              /* INIT_PCS_ILKN */
{
  ...
  /* Refer to tefmod16 implementation */
  ILKN_CTL0r_CLR(ILKN_CONTROL0r_reg);
...
int tbhmod_get_pcs_latched_status_local_fault_reasons (PHYMOD_ST* pc, 
                                                       uint32_t *pcs_link_status_live,
                                                       uint32_t *local_fault_latch_status,
                                                       uint32_t *deskew_latch_low)
{
    RX_X4_PCS_LATCH_STS1r_t pcs_latch_status;

    *pcs_link_status_live = 0;
    *local_fault_latch_status = 0;
    *deskew_latch_low = 0;
    RX_X4_PCS_LATCH_STS1r_CLR(pcs_latch_status) ;

    PHYMOD_IF_ERR_RETURN(
            READ_RX_X4_PCS_LATCH_STS1r(pc, &pcs_latch_status));

    if (RX_X4_PCS_LATCH_STS1r_PCS_LINK_STATUS_LIVEf_GET(pcs_latch_status)) {
        *pcs_link_status_live = 1;
    }

    if (RX_X4_PCS_LATCH_STS1r_LOCAL_FAULT_LHf_GET(pcs_latch_status)) {
        *local_fault_latch_status = 1;
    }

    if (RX_X4_PCS_LATCH_STS1r_DESKEW_STATUS_LLf_GET(pcs_latch_status)) {
        *deskew_latch_low = 1;
    }
    return PHYMOD_E_NONE;
}
...
/**
 * @brief   tbhmod_am_lock_latched_low_get
 * @param   pc handle to current TSCBH context (#tbhmod_st)
 * @returns The value PHYMOD_E_NONE upon successful completion
 * @details Returns whether am_lock latched to low for any Pseudo logical lane under given physical lane
 *          pc handle should contain a single lane mask
 * */
int tbhmod_am_lock_latched_low_get (PHYMOD_ST* pc, int bit_mux_mode, uint32_t *state)
{
    int virtual_lanes, num_psll_per_phyl;
    RX_X4_AM_LOCK_LATCH_STSr_t am_lock_status_reg;

    TBHMOD_DBG_IN_FUNC_INFO(pc);
    *state = 0;

    PHYMOD_IF_ERR_RETURN
        (_tbhmod_virtual_lane_count_get(bit_mux_mode, 1, &virtual_lanes, &num_psll_per_phyl));

    PHYMOD_IF_ERR_RETURN
        (READ_RX_X4_AM_LOCK_LATCH_STSr(pc, &am_lock_status_reg));

    if (num_psll_per_phyl > 0) {
        *state |= RX_X4_AM_LOCK_LATCH_STSr_AM_LOCK_LL_0f_GET(am_lock_status_reg);
    }

    if (num_psll_per_phyl > 1) {
        *state |= RX_X4_AM_LOCK_LATCH_STSr_AM_LOCK_LL_1f_GET(am_lock_status_reg);
    }

    if (num_psll_per_phyl > 2) {
        *state |= RX_X4_AM_LOCK_LATCH_STSr_AM_LOCK_LL_2f_GET(am_lock_status_reg);
    }

    if (num_psll_per_phyl > 3) {
        *state |= RX_X4_AM_LOCK_LATCH_STSr_AM_LOCK_LL_3f_GET(am_lock_status_reg);
    }

    if (num_psll_per_phyl > 4) {
        *state |= RX_X4_AM_LOCK_LATCH_STSr_AM_LOCK_LL_4f_GET(am_lock_status_reg);
    }
...
/**
 * @brief   tbhmod_phy_fec_align_latched_low_get
 * @param   pc handle to current TSCBH context (#tbhmod_st)
 * @returns The value PHYMOD_E_NONE upon successful completion
 * @details Returns whether FEC align latched low
 * */
int tbhmod_phy_fec_align_latched_low_get (PHYMOD_ST* pc, uint32_t *fec_align_latched_low)
{
    RX_X4_RS_FEC_RXP_STSr_t fec_rxp_status_reg;

    TBHMOD_DBG_IN_FUNC_INFO(pc);

    RX_X4_RS_FEC_RXP_STSr_CLR(fec_rxp_status_reg);
    *fec_align_latched_low = 0;

    PHYMOD_IF_ERR_RETURN
        (READ_RX_X4_RS_FEC_RXP_STSr(pc, &fec_rxp_status_reg));

    if (RX_X4_RS_FEC_RXP_STSr_FEC_ALIGN_STATUS_LLf_GET(fec_rxp_status_reg)) {
        *fec_align_latched_low = 1;
    }

    return PHYMOD_E_NONE;
}

int tbhmod_fec_error_inject_set(PHYMOD_ST *pc,
                                uint16_t error_control_map,
                                tbhmod_fec_error_mask_t bit_error_mask)
{
    ...
    TX_X4_ERR_CTLr_t TX_X4_ERR_CTLr_reg;
    ...
    TX_X4_ERR_CTLr_CLR(TX_X4_ERR_CTLr_reg);
    ...
    PHYMOD_IF_ERR_RETURN(WRITE_TX_X4_ERR_CTLr(&pa_copy, TX_X4_ERR_CTLr_reg));
    if (TBHMOD_CL74_EN_FLAG_GET(error_control_map)) {
        /* error_control_map to lane_map for single MPP*/
        error_gen = (error_control_map & 0xf) << (start_lane % 0x4);
        TX_X4_ERR_CTLr_PCS_LANE_ERROR_INJECTf_SET(TX_X4_ERR_CTLr_reg, error_gen & 0xf);
    } else {
        TX_X4_ERR_CTLr_RS_FEC_LANE_ERROR_INJECTf_SET(TX_X4_ERR_CTLr_reg, error_control_map & 0xff);
    }
    PHYMOD_IF_ERR_RETURN(WRITE_TX_X4_ERR_CTLr(&pa_copy, TX_X4_ERR_CTLr_reg));
...
int tbhmod_fec_error_inject_get(PHYMOD_ST *pc,
                                uint16_t* error_control_map,
                                tbhmod_fec_error_mask_t* bit_error_mask)
{
    int start_lane = 0, num_lane = 0;
    ...
    PHYMOD_IF_ERR_RETURN(READ_TX_X4_ERR_CTLr(&pa_copy, &TX_X4_ERR_CTLr_reg));
    if (TBHMOD_CL74_EN_FLAG_GET(*error_control_map)) {
        error_gen = TX_X4_ERR_CTLr_PCS_LANE_ERROR_INJECTf_GET(TX_X4_ERR_CTLr_reg);
        *error_control_map = (error_gen & 0xf) >> (start_lane % 4);
    } else {
        error_gen = TX_X4_ERR_CTLr_RS_FEC_LANE_ERROR_INJECTf_GET(TX_X4_ERR_CTLr_reg);
        *error_control_map = error_gen;
    }
