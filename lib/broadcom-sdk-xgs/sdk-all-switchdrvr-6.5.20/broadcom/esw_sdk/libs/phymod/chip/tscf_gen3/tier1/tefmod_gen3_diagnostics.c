/*----------------------------------------------------------------------
 * $Id: tefmod_gen3_diagnostics.c,v 1.2 Broadcom SDK $
 ...
 *---------------------------------------------------------------------
 * File       : tefmod_gen3_diagnostics.c
...
#endif
#ifdef _DV_TB_
  #define PHYMOD_ST  tefmod_gen3_st
  #define _TRG_OPN(a) _TRGO_(a->prt_ad, a->this_lane)
  #define _TRG_CLS(a) _TRGC_(a->prt_ad, a->this_lane)
#endif

extern int tefmod_gen3_diag_st(PHYMOD_ST *pc, int resolved_speed);
...
*/
int tefmod_gen3_diag_field_or(PHYMOD_ST *pc)
{
  ...
  PHYMOD_DEBUG_ERROR(("| NUM LANES: 0x%x | FEC ENA  : 0x%X | %-43s|\n",
    ...
    e2s_tefmod_gen3_scr_mode[TX_X4_MISCr_SCR_MODEf_GET(tx_misc_or)]+6));
  
  PHYMOD_DEBUG_ERROR(("|                | %-15s| %-43s|\n",
    e2s_tefmod_gen3_os_mode_type[TX_X4_MISCr_OS_MODEf_GET(tx_misc_or)]+6,
    e2s_tefmod_gen3_encode_mode[TX_X4_ENC0r_T_ENC_MODEf_GET(encode_or)]+6));
  ...
  PHYMOD_DEBUG_ERROR(("| TXLN RSTB: 0x%X | TX LN ENA: 0x%X | %-43s|\n",
    ...
    e2s_tefmod_gen3_descrambler_mode[RX_X4_PCS_CTL0r_DESCR_MODEf_GET(rx_x4_pcs_ctrl_or)]+6));
  
  PHYMOD_DEBUG_ERROR(("| CLKCNT0  : 0x%x | CLKCNT1  : 0x%X | %-43s|\n",
    TX_X4_CRED0r_CLOCKCNT0f_GET(crd_or),
    TX_X4_CRED1r_CLOCKCNT1f_GET(crd1_or),
    e2s_tefmod_gen3_dec_tl_mode[RX_X4_PCS_CTL0r_DEC_TL_MODEf_GET(rx_x4_pcs_ctrl_or)]+6));
  ...
  PHYMOD_DEBUG_ERROR(("| LP CNT0  : 0x%x | LP CNT1  : 0x%X | %-43s|\n",
    ...
    e2s_tefmod_gen3_deskew_mode[RX_X4_PCS_CTL0r_DESKEW_MODEf_GET(rx_x4_pcs_ctrl_or)]+6));
  ...
  PHYMOD_DEBUG_ERROR(("| MACCRDGEN: 0x%x | CL74FECEN: %d   | %-43s|\n",
    ...
    e2s_tefmod_gen3_dec_fsm_mode[RX_X4_PCS_CTL0r_DEC_FSM_MODEf_GET(rx_x4_pcs_ctrl_or)]+6));
...
*/
int tefmod_gen3_diag_pmd_pcs_lane_cfg(PHYMOD_ST *pc, int lane)
{
  _TRG_OPN(pc);
  _TRG_CLS(pc);
  return PHYMOD_E_NONE;
} /* tefmod_gen3_diag_pmd_pcs_lane_cfg */
...
*/
int tefmod_gen3_diag_speed(PHYMOD_ST *pc)
{
  ...
  tefmod_gen3_diag_st(pc, SC_X4_RSLVD_SPDr_SPEEDf_GET(sc_final_resolved_speed));
  tefmod_gen3_diag_field_or(pc);
  ...
  PHYMOD_DEBUG_ERROR(("| SPD CHG VLD  %d    |     OS MODE %-6s | DESCR MODE %-6s|                  |\n",
     ...
     e2s_tefmod_gen3_os_mode_type[SC_X4_RSLVD0r_OS_MODEf_GET(sc_final_resolved_0)]+18,
     e2s_tefmod_gen3_descrambler_mode[SC_X4_RSLVD1r_DESCR_MODEf_GET(sc_final_resolved_1)]+20));
  ...
  PHYMOD_DEBUG_ERROR(("| SPD CHG DONE %d    |SCR MODE %-6s|DECODE MODE %-6s|  CLK CNT0 : %04d |\n",
     ...
     e2s_tefmod_gen3_scr_mode[SC_X4_RSLVD0r_SCR_MODEf_GET(sc_final_resolved_0)]+15,
     e2s_tefmod_gen3_dec_tl_mode[SC_X4_RSLVD1r_DEC_TL_MODEf_GET(sc_final_resolved_1)]+19,
  ...
  PHYMOD_DEBUG_ERROR(("| SPD RESOLVED %04d |    ENC MODE %-6s |DESKEW MODE %-6s|  CLK CNT1 : %04d |\n",
     ...
     e2s_tefmod_gen3_encode_mode[SC_X4_RSLVD0r_T_ENC_MODEf_GET(sc_final_resolved_0)]+18,
     e2s_tefmod_gen3_deskew_mode[SC_X4_RSLVD1r_DESKEW_MODEf_GET(sc_final_resolved_1)]+20,
  ...
  PHYMOD_DEBUG_ERROR(("| #LN RESOLVED %04d |BS SM SYNC MODE %d   |DECFSM MODE %-6s| LP CNT0 : %04d |\n",
     ...
     e2s_tefmod_gen3_dec_fsm_mode[SC_X4_RSLVD1r_DEC_FSM_MODEf_GET(sc_final_resolved_1)]+17,
...
*/
int tefmod_gen3_diag_autoneg(PHYMOD_ST *pc)
{
...
*/
int tefmod_gen3_diag_topology(PHYMOD_ST *pc)
{
...
*/
int tefmod_gen3_diag_link(PHYMOD_ST *pc)
{
...
*/
int tefmod_gen3_diag_general(PHYMOD_ST *pc)
{
  tefmod_gen3_diag_topology(pc);
  tefmod_gen3_diag_link(pc);
  tefmod_gen3_diag_autoneg(pc);
  tefmod_gen3_diag_speed(pc);
...
@details Print the Internal Tfc types.
 TBD.
*/
int tefmod_gen3_diag_internal_tfc(PHYMOD_ST *pc)
{
/*
...
{                                                                                    \
  ...
  PHYMOD_DEBUG_ERROR(("| ST [%d]              | %-28s| NUM LANES : %d            |\n",\
       st_num, e2s_tefmod_gen3_spd_intfc_type_t[x1_speed]+6,                                \
  ...
  PHYMOD_DEBUG_ERROR(("| %-19s |                                                |\n",\
        e2s_tefmod_gen3_os_mode_type[SC_X1_SPD_##ovr##_0r_OS_MODEf_GET(sc_st##_0)]+6));   \
                                                                                     \
  PHYMOD_DEBUG_ERROR(("| %-19s |                                                |\n",\
        e2s_tefmod_gen3_scr_mode[SC_X1_SPD_##ovr##_0r_SCR_MODEf_GET(sc_st##_0)]+6));      \
                                                                                     \
  PHYMOD_DEBUG_ERROR(("+---------------------+-----------------------------+-----------+--------------+\n"));\
  }                                                                                  \
}
...
@details Soft Table diags
 ...
 if the resolved_speed is not set, then all the table enteries are displayed
*/
int tefmod_gen3_diag_st(PHYMOD_ST *pc, int resolved_speed)
{
...
*/
int tefmod_gen3_diag_an_timers(PHYMOD_ST* pc)
{
...
*/
int tefmod_gen3_diag_state(PHYMOD_ST* pc)
{
...
@details  Misc debug
*/
int tefmod_gen3_diag_debug(PHYMOD_ST* pc)
{
  _TRG_OPN(pc);
  _TRG_CLS(pc);
  return PHYMOD_E_NONE;
...
@details  Misc debug
*/
int tefmod_gen3_diag_ieee(PHYMOD_ST *pc)
{
  _TRG_OPN(pc);
  _TRG_CLS(pc);
  return PHYMOD_E_NONE;
...
@details  Misc debug
*/
int tefmod_gen3_diag(PHYMOD_ST *pc, tefmod_gen3_diag_type_t diag_type)
{
  int rv;
  rv = PHYMOD_E_NONE;
  ...
  if(diag_type & TEFMOD_DIAG_GENERAL)    rv |= tefmod_gen3_diag_general(pc);
  if(diag_type & TEFMOD_DIAG_TOPOLOGY)   rv |= tefmod_gen3_diag_topology(pc);
  if(diag_type & TEFMOD_DIAG_LINK)       rv |= tefmod_gen3_diag_link(pc);
  if(diag_type & TEFMOD_DIAG_SPEED)      rv |= tefmod_gen3_diag_speed(pc);
  if(diag_type & TEFMOD_DIAG_ANEG)       rv |= tefmod_gen3_diag_autoneg(pc);
  if(diag_type & TEFMOD_DIAG_TFC)        rv |= tefmod_gen3_diag_internal_tfc(pc);
  if(diag_type & TEFMOD_DIAG_AN_TIMERS)  rv |= tefmod_gen3_diag_an_timers(pc);
  if(diag_type & TEFMOD_DIAG_STATE)      rv |= tefmod_gen3_diag_state(pc);
  if(diag_type & TEFMOD_DIAG_DEBUG)      rv |= tefmod_gen3_diag_debug(pc);
  _TRG_CLS(pc);
  return rv;
}
...
/*!
*/
int tefmod_gen3_diag_disp(PHYMOD_ST *pc, const char* cmd_str)
{
    tefmod_gen3_diag_type_t type;
    ...
    if (!cmd_str)
     ...
     return (tefmod_gen3_diag(pc, type));
}
  
int tfmod_gen3_diag_set_poly_tx(PHYMOD_ST *pc, uint32_t lane, uint32_t poly) {
...
int tfmod_gen3_diag_set_poly_rx(PHYMOD_ST *pc, uint32_t lane, uint32_t poly) {
    phymod_access_t copy;
...
int tfmod_gen3_diag_inject_error(PHYMOD_ST *pc, uint32_t lane) {
    phymod_access_t copy;
    TLB_TX_PRBS_GEN_CFGr_t gen_cfg;
...
int tfmod_gen3_diag_gen_set(PHYMOD_ST *pc, uint32_t lane, uint32_t action) {
    phymod_access_t copy;
    TLB_TX_PRBS_GEN_CFGr_t gen_cfg;
...
int tfmod_gen3_diag_chk_set(PHYMOD_ST *pc, uint32_t lane, uint32_t action) {
    phymod_access_t copy;
    TLB_RX_PRBS_CHK_CFGr_t chk_cfg;
...
int tfmod_gen3_diag_get_lock(PHYMOD_ST *pc, uint32_t lane, uint32_t *lock_status) {
    phymod_access_t copy;
...
int tfmod_gen3_diag_clear_count(PHYMOD_ST *pc, uint32_t lane) {
    phymod_access_t copy;
...
int tfmod_gen3_diag_get_count(PHYMOD_ST *pc, uint32_t lane, uint32_t *count) {
    phymod_access_t copy;
    BCMI_TSCF_XGXS_TLB_RX_PRBS_CHK_ERR_CNT_MSB_STSr_t msb;
    BCMI_TSCF_XGXS_TLB_RX_PRBS_CHK_ERR_CNT_LSB_STSr_t lsb;
    uint32_t val;
