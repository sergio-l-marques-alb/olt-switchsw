 ...
 *
 * File:        bcm956275k.c
 * Purpose:     CMICx LED bit pattern composer for BCM956275K.
 * Requires:
 */
...
customer_led_handler(soc_led_custom_handler_ctrl_t *ctrl, uint32 cnt)
{
    ...
    for (led_uc_port = 0; led_uc_port < 24; led_uc_port ++) {
        ...
        LED_HW_RAM_WRITE16(ctrl->pat_ram_base, (23 - led_uc_port), pattern);
    }
    ...
    for (idx = 0; idx < LED_HW_INTF_MAX_NUM; idx++) {
        ...
        case 0:
            ...
            /* Send the pattern over LED interface 0 for ports 0 - 23 */
            lic->valid = 1;
            lic->start_row = 0;
            lic->end_row = 23;
            lic->pat_width = 2;
