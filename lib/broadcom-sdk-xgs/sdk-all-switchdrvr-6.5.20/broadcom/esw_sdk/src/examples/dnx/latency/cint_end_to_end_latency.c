    ...
    int qual_data)
{
    ...
    rv = bcm_field_group_add(unit, BCM_FIELD_FLAG_MSB_RESULT_ALIGN, &fg_info, &e2e_lat_fg_id);
    if (rv != BCM_E_NONE)
    {
...
/**
* \brief - function get from HW the max latency of specific flow
*/
    ...
    int flow_id)
{
    ...
    e2e_flow_latency_max = 0xFFFFFFFF;
    ...
    printf("------- MAX LATENCY TABLE ENTRY PRINT: (size=%d, core=%d, flow_id=%d) --------\n", 
        memory_actual_count, core, flow_id);  
    for(i=0;i<memory_actual_count; i++)
    {
        printf("max_latency_pkt[%d]:\n",i);
        print("\tlatency=\n");
