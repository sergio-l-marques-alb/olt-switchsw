...
bcm_field_qualify_t cinf_cflow_zero_qual = BCM_FIELD_ID_INVALID;
...
bcm_field_action_priority_t cint_sflow_exem_dip_invalidate_next_priority =  BCM_FIELD_ACTION_POSITION(0, 2);
bcm_field_action_priority_t cint_sflow_exem_sip_invalidate_next_priority =  BCM_FIELD_ACTION_POSITION(0, 4);
bcm_field_action_priority_t cint_sflow_trap_dip_priority =  BCM_FIELD_ACTION_POSITION(0, 3);
bcm_field_action_priority_t cint_sflow_trap_sip_priority =  BCM_FIELD_ACTION_POSITION(0, 5);
...
int cint_sflow_field_extended_gateway_first_pass_group_create_and_attach(int unit, int nof_cores, bcm_gport_t gport_stat_samp_core_0, bcm_gport_t gport_stat_samp_core_1, bcm_field_context_t context_id)
{
   ...
   /* create user defined qualifier just because TCAM must have a qualifier */    
   bcm_field_qualifier_info_create_t_init(&qual_info);
   qual_info.size = 1;
   dest_char = &(qual_info.name[0]);
   sal_strncpy_s(dest_char, "cinf_cflow_zero_qual", sizeof(qual_info.name));
   rv = bcm_field_qualifier_create(unit, 0, &qual_info, &cinf_cflow_zero_qual);
   if (rv != BCM_E_NONE)
   {
   ...
   field_group_info.qual_types[0] = cinf_cflow_zero_qual;
   ...
   rv = bcm_field_group_add(unit, BCM_FIELD_FLAG_MSB_RESULT_ALIGN, &field_group_info, &cint_sflow_snoop_field_fg_id);
   if (rv != BCM_E_NONE)
   {
...
int sflow_field_extended_gateway_second_pass_exem_dip_create_and_attach(int unit, bcm_field_context_t context_id)
{
   ...
   rv = bcm_field_group_add(unit, BCM_FIELD_FLAG_MSB_RESULT_ALIGN, &field_group_info, &cint_sflow_extended_dip_field_fg_id);
   if (rv != BCM_E_NONE)
   {
      printf("%s: Error (%d), in bcm_field_group_add\n", proc_name, rv);
      return rv;
...
int sflow_field_extended_gateway_second_pass_exem_sip_create_and_attach(int unit, bcm_field_context_t context_id)
{
   ...
   rv = bcm_field_group_add(unit, BCM_FIELD_FLAG_MSB_RESULT_ALIGN, &field_group_info, &cint_sflow_extended_sip_field_fg_id);
   if (rv != BCM_E_NONE)
   {
      printf("%s: Error (%d), in bcm_field_group_add\n", proc_name, rv);
      return rv;
...
int cint_sflow_field_extended_gateway_group_context_presel_set(int unit, bcm_field_context_t context_id, int is_first_pass)
{
   ...
   if (is_first_pass == 1) {
      ...
      presel_data.qual_data[0].qual_value = sflow_recycle_port_profile;
...
int cint_sflow_field_extended_gateway_destroy(int unit,bcm_field_context_t context_id, int nof_cores)
{
   ...
   rv = bcm_field_qualifier_destroy(unit, cinf_cflow_zero_qual);
   if (rv != BCM_E_NONE)
   {
