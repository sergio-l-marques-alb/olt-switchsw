/*
 * $Id: cint_dnx_srv6_evpn_basic.c, Exp $
 ...
 * -------------------------------------
 *
 * 1. IPv6 EVPN packet without SRH, ETHoIPv6oETH is sent through an SRv6 Egress USP Node that performs
 *    - Termination of SRv6 and IPv6 Header
 *    - ETH Forwarding
 *    - Copying ESI label from IPv6 DIP to UDH by IPMF, for EPMF packet filtering for Port/ESI combination
 *
 *
 * 2. SRv6 packet with SRH, ETHoSRV6oIPv6oETH is sent through an SRv6 Egress USP Node that performs
 *    on 1st Pass:
 *    - Termination of SRv6 and IPv6 Header
 *    - Sending packet to RCH port
 *    - Copying ESI label from IPv6 DIP to UDH by IPMF
 *    - Writing ESI label to RCH header to Type Specific Container field
 *
 *    on 2nd Pass:
 *    - RCH termination
 *    - In-lif reclassification from 1st Pass IPv6 tunnel termination
 *    - ETH Forwarding
 *    - Copying ESI label from IPv6 DIP to UDH by IPMF, for EPMF packet filtering for Port/ESI combination
 *
 *
