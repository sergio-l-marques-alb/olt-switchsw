    ...
    bcm_field_context_t pmf_context)
{
    ...
    for(profile_idx = 0; profile_idx < quartet; profile_idx++)
    {
        ...
        BCM_IF_ERROR_RETURN(bcm_cosq_control_set(unit,gport, 0, bcmCosqControlLatencyDropThreshold, 10000));
    ...
    rv = ingress_latency_flow_acl_rule_set(unit, pmf_context, 0, 0, bcmFieldQualifyInPort, 0x1FF, gport_qual);
    ...
    bcm_gport_t gport_q)
{
    ...
    bcm_gport_t gport;
    int base_profile = 16;
    int feature_probibalistic_mechanism_support = *dnxc_data_get(unit, "latency", "features", "probabilistic_mechanism_support", NULL);

    /** first verify the probabilistic feature is supported */
    if (!feature_probibalistic_mechanism_support)
    {
        printf("Error in ingress_latency_per_queue_example configuration - probabilistic mechanism is not supported\n", rv);
        return BCM_E_FAIL;
    }
    for(profile_idx = 0; profile_idx < quartet; profile_idx++)
    {
    ...
    bcm_field_context_t pmf_context)
{
    ...
    int LatencyDisabledWhenVOQBelowThisInBytes = 1000, rv=0, bin_id=0;
    int feature_voq_latency_based_admission_all_profiles_support = *dnxc_data_get(unit, "ingr_congestion", "info", "latency_based_admission_cgm_extended_profiles_enable", NULL);
    ...
    bcm_cosq_profile_mapping_get(unit, base_voq_gport, 0, 0, &voq_profile_mapping);
    ...
    for (profile_id=0; profile_id < 8; profile_id++) {
        if (feature_voq_latency_based_admission_all_profiles_support)
        {
             ...
             *  16 and above */
            bcm_cosq_latency_profile_create(unit, BCM_COSQ_LATENCY_PROFILE_ID_EXTENSION, &latency_profile_gport[profile_id]);
        }
        else
        {
            ...
            bcm_cosq_latency_profile_create(unit, BCM_COSQ_LATENCY_PROFILE_WITH_ID, &latency_profile_gport[profile_id]);
        }
        /** enable latency measurements */
        bcm_cosq_control_set(unit, latency_profile_gport[profile_id], 0, bcmCosqControlLatencyCounterEnable, TRUE);
        bcm_cosq_control_set(unit, latency_profile_gport[profile_id], 0, bcmCosqControlLatencyTrack, TRUE);
        bcm_cosq_control_set(unit, latency_profile_gport[profile_id], 0, bcmCosqControlLatencyTrackDropPacket, TRUE);
        bcm_cosq_control_set(unit, latency_profile_gport[profile_id], 0, bcmCosqControlLatencyDropEnable, TRUE);

        for (bin_id=0; bin_id < 7; bin_id++) {
            bcm_cosq_control_set(unit, latency_profile_gport[profile_id], bin_id, bcmCosqControlLatencyVoqRejectBinThreshold, Latencies[bin_id]);
        }
    }
    ...
    bcm_cosq_profile_mapping_set(unit, base_voq_gport, 0, 0, &voq_profile_mapping);

    voq_profile_mapping.mapped_profile = latency_profile_gport[4];
    voq_profile_mapping.profile_type = bcmCosqIngressQueuesToLatencyProfile;
    bcm_cosq_profile_mapping_set(unit, base_voq_gport, 4, 0, &voq_profile_mapping);


    for (dp_index=0; dp_index < 4; dp_index++) {
        key = BCM_COSQ_GENERIC_KEY_COLOR_SET(key, dp_index);
        bcm_cosq_generic_control_set(unit, 0, base_voq_rate_class_gport, key, bcmCosqGenericControlLatencyVoqRejectEnable, TRUE);
    }

    for (dp_index=0; dp_index < 4; dp_index++) {
        for (bin_id=0; bin_id < 8; bin_id++) {
            key = BCM_COSQ_GENERIC_KEY_LAT_BIN_COLOR_SET(key, bin_id, dp_index);
            bcm_cosq_generic_control_set(unit, 0, base_voq_rate_class_gport, key, bcmCosqGenericControlLatencyVoqRejectProb, Probabilities[dp_index][bin_id]);
        }
    }

    for (dp_index=0; dp_index < 4; dp_index++) {
        key = BCM_COSQ_GENERIC_KEY_COLOR_SET(key, dp_index);
        bcm_cosq_generic_control_set(unit, 0, base_voq_rate_class_gport, key, bcmCosqGenericControlLatencyVoqRejectMinThreshold, LatencyDisabledWhenVOQBelowThisInBytes);
    }
