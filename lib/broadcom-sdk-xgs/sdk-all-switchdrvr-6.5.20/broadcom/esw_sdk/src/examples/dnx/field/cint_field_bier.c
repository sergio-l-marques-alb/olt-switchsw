 ...
 * cint_reset();
 * exit;
 * cint ../../../src/examples/dnx/field/cint_field_bier.c
 * int unit = 0;
...
bcm_field_context_t cint_attach_context_id_mpls;
...
int field_attach_main(int unit)
{
    ...
    /* Set quals */
    fg_info.nof_quals = 1;
    fg_info.qual_types[0] = bcmFieldQualifyMplsLabelBos;
    /*    fg_info.qual_types[0] = bcmFieldQualifySrcIp; */
    /* Set actions */
    fg_info.nof_actions = 5;
    fg_info.action_types[0] = bcmFieldActionPrioIntNew;

    fg_info.action_types[1] = bcmFieldActionBierStringOffset;
    fg_info.action_types[2] = bcmFieldActionBierStringSize;
    fg_info.action_types[3] = bcmFieldActionPacketIsBier;
    fg_info.action_types[4] = bcmFieldActionRedirectMcast;

    dest_char = &(fg_info.name[0]);
    sal_strncpy_s(dest_char, "MPLS_Lookup", sizeof(fg_info.name));
    ...
    for(i_act = 0; i_act < attach_info.payload_info.nof_actions; i_act++)
    {
        attach_info.payload_info.action_types[i_act] = fg_info.action_types[i_act];
    }
    ...
    dest_char = &(context_info.name[0]);
    sal_strncpy_s(dest_char, "MPLS_Bier", sizeof(context_info.name));
    rv = bcm_field_context_create(unit, 0, bcmFieldStageIngressPMF1, &context_info, &cint_attach_context_id_mpls);
    if (rv != BCM_E_NONE)
    {
        printf("Error (%d), in bcm_field_context_create\n", rv);
        return rv;
    ...
    rv = bcm_field_group_context_attach(unit, 0, cint_attach_fg_id, cint_attach_context_id_mpls, &attach_info);
    if (rv != BCM_E_NONE)
    {
        printf("Error (%d), in bcm_field_group_context_attach\n", rv);
        return rv;
    ...
    printf("Attached  FG (%d) to context (%d)\n",cint_attach_fg_id,cint_attach_context_id_mpls);
    ...
    p_id.stage = bcmFieldStageIngressPMF1;
    p_data.entry_valid = TRUE;
    p_data.context_id = cint_attach_context_id_mpls;
    p_data.nof_qualifiers = 1;
    ...
    printf("Presel (%d) was configured for stage(iPMF1) context(%d) fwd_layer(Ethernet) \n",
            cint_attach_presel_ipv4,cint_attach_context_id_mpls);
...
int field_attach_destroy(int unit)
{
    ...
    rv = bcm_field_group_context_detach(unit, cint_attach_fg_id, cint_attach_context_id_mpls);
    if (rv != BCM_E_NONE)
    {
    ...
    rv = bcm_field_context_destroy(unit,bcmFieldStageIngressPMF1,cint_attach_context_id_mpls);
    if (rv != BCM_E_NONE)
    {
