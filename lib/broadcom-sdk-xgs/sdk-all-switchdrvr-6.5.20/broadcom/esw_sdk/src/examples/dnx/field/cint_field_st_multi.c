 ...
 * 1. Create 2 ST FGs.
 * 2. Create 2 different contexts with ST support.
 * 3. Attach the 2 ST FGs to the 2 IPMF1 contexts.
 * 4. Create 1 IPMF2 FG.
 * 5. Attach the IPMF2 FG to both contexts.
 ...
 * Nof state table FGs.
 */
int Cint_field_st_multi_nof_st_fg = 2;
 ...
 */
int Cint_field_st_multi_nof_quals = 5;
/**
 * Nof presels.
 */
int Cint_field_st_multi_nof_presels = 2;
/**
 * Array in which we are collecting all Qual IDs so we can delete them afterwards. Zero padding qualifiers.
 */
bcm_field_qualify_t Cint_field_st_multi_qual_id_zero_padding[Cint_field_st_multi_nof_st_fg];
/**
 * Array in which we are collecting all Qual IDs so we can delete them afterwards. Prefix qualifiers.
 */
bcm_field_qualify_t Cint_field_st_multi_qual_id_prefix[Cint_field_st_multi_nof_st_fg];
/**
 * Array in which we are collecting all Qual IDs so we can delete them afterwards. WR bit / opcode qualifiers.
 */
bcm_field_qualify_t Cint_field_st_multi_qual_id_opcode[Cint_field_st_multi_nof_st_fg];
/**
...
/**
 * Action IDs used to fill the payload for devices J2P (J2CP) or above. Collected in an array to be deleted later.
 * Since in this example they are all the same, we can use only one udq for all field groups.
 */
bcm_field_action_t Cint_field_st_multi_uda_id[Cint_field_st_multi_nof_st_fg];
 ...
 * \param [in]  is_j2p_or_above    - If the device is J2P (J2CP) or above.
 * \return
 *   shr_error_e - Type of Error
 * \remark
 *   * None
    ...
    int is_j2p_or_above)
{
    ...
    if (is_j2p_or_above)
    {
        ...
        action_info.size = 8;
        rv = bcm_field_action_create(unit, 0, &action_info, &Cint_field_st_multi_uda_id[address_prefix]);
        if (rv != BCM_E_NONE)
        {
    ...
    rv = bcm_field_qualifier_create(unit, 0, &qual_info, &Cint_field_st_multi_qual_id_zero_padding[address_prefix]);
    if (rv != BCM_E_NONE)
    {
        printf("Error (%d), in bcm_field_qualifier_create for zero padding qualifier.\n", rv);
        return rv;
    }

    bcm_field_qualifier_info_create_t_init(&qual_info);
    qual_info.size = 1;
    rv = bcm_field_qualifier_create(unit, 0, &qual_info, &Cint_field_st_multi_qual_id_prefix[address_prefix]);
    if (rv != BCM_E_NONE)
    {
        printf("Error (%d), in bcm_field_qualifier_create for prefix qualifier.\n", rv);
        return rv;
    }
    ...
    rv = bcm_field_qualifier_create(unit, 0, &qual_info, &Cint_field_st_multi_qual_id_opcode[address_prefix]);
    if (rv != BCM_E_NONE)
    {
        printf("Error (%d), in bcm_field_qualifier_create for prefix qualifier.\n", rv);
        return rv;
    ...
    {
        qual_index_data = 0;
        qual_index_address = 1;
    }
    else
    {
        qual_index_address = 0;
        qual_index_data = 3;
    }
    ...
    /**
     * Address IPv4 protocol (8bits) + constant qual for padding (1 or 4 bits) + constant qual for prefix (1bit)
     * The value of the Constant qual is the input parameter of the function.
     */
    fg_info.qual_types[qual_index_address+0] = bcmFieldQualifyIp4Protocol;
    fg_info.qual_types[qual_index_address+1] = Cint_field_st_multi_qual_id_zero_padding[address_prefix];
    fg_info.qual_types[qual_index_address+2] = Cint_field_st_multi_qual_id_prefix[address_prefix];
    /**
     * Data (8 bits)
     */
    fg_info.qual_types[qual_index_data] = bcmFieldQualifyIp4Ttl;
    /**
     * do-write (always set to 1) or for J2P or above opcode 12 (also performs write)
     */
    fg_info.qual_types[4] = Cint_field_st_multi_qual_id_opcode[address_prefix];

    /**
     * Action does nothing in this example.
     */
    if (is_j2p_or_above)
    {
        fg_info.nof_actions = 1;
        fg_info.action_types[0] = Cint_field_st_multi_uda_id[address_prefix];
     ...
     */
    attach_info.key_info.qual_types[qual_index_address+0] = fg_info.qual_types[qual_index_address+0];
    attach_info.key_info.qual_info[qual_index_address+0].input_type = bcmFieldInputTypeLayerAbsolute;
    attach_info.key_info.qual_info[qual_index_address+0].input_arg = 1;
    attach_info.key_info.qual_info[qual_index_address+0].offset = 0;

    attach_info.key_info.qual_types[qual_index_address+1] = fg_info.qual_types[qual_index_address+1];
    attach_info.key_info.qual_info[qual_index_address+1].input_type = bcmFieldInputTypeConst;
    attach_info.key_info.qual_info[qual_index_address+1].input_arg = 0;

    attach_info.key_info.qual_types[qual_index_address+2] = fg_info.qual_types[qual_index_address+2];
    attach_info.key_info.qual_info[qual_index_address+2].input_type = bcmFieldInputTypeConst;
    /**
     * First ST FG will be with prefix 0.
     * Second ST FG will be with prefix 1.
     */
    attach_info.key_info.qual_info[qual_index_address+2].input_arg = address_prefix;

    attach_info.key_info.qual_types[qual_index_data] = fg_info.qual_types[qual_index_data];
    attach_info.key_info.qual_info[qual_index_data].input_type = bcmFieldInputTypeLayerAbsolute;
    attach_info.key_info.qual_info[qual_index_data].input_arg = 1;
    attach_info.key_info.qual_info[qual_index_data].offset = 0;

    attach_info.key_info.qual_types[4] = fg_info.qual_types[4];
    attach_info.key_info.qual_info[4].input_type = bcmFieldInputTypeConst;
    if (is_j2p_or_above)
    {
        attach_info.key_info.qual_info[4].input_arg = 12;
    }
    else
    {
    ...
    int is_j2p_or_above)
{
    int rv = BCM_E_NONE;
    /**
     * Create first ST with action_prefix 0.
     */
    rv = cint_field_st_multi_cfg(unit, fwd_layer, 0, is_j2p_or_above);
    if (rv != BCM_E_NONE)
    {
    ...
    /**
     * Create second ST with action_prefix 1.
     */
    rv = cint_field_st_multi_cfg(unit, fwd_layer, 1, is_j2p_or_above);
    if (rv != BCM_E_NONE)
    {
        printf("Error (%d), in cint_field_st_multi_cfg %d\n", rv);
        return rv;
    ...
    if (rv != BCM_E_NONE)
    {
        printf("Error (%d), in cint_field_st_multi_ipmf2_fg_cfg %d\n", rv);
        return rv;
    }
    return rv;
}
...
int cint_field_st_multi_destroy(
    ...
    int is_j2p_or_above)
{
    int context_iter, entry_iterator, fg_iter, presel_iter;
    ...
    for (fg_iter = 0; fg_iter < Cint_field_st_multi_nof_st_fg; fg_iter++)
    {
        rv = bcm_field_qualifier_destroy(unit, Cint_field_st_multi_qual_id_zero_padding[fg_iter]);
        if(rv != BCM_E_NONE)
        {
            printf("Error (%d), in bcm_field_qualifier_destroy, zero padding\n", rv);
            return rv;
        }
        rv = bcm_field_qualifier_destroy(unit, Cint_field_st_multi_qual_id_prefix[fg_iter]);
        if(rv != BCM_E_NONE)
        {
            printf("Error (%d), in bcm_field_qualifier_destroy, prefix\n", rv);
            return rv;
        }
        rv = bcm_field_qualifier_destroy(unit, Cint_field_st_multi_qual_id_opcode[fg_iter]);
        if(rv != BCM_E_NONE)
        {
            printf("Error (%d), in bcm_field_qualifier_destroy, opcode\n", rv);
            return rv;
        }
    }

    printf("Destroying the user defined actions...\r\n");
    if (is_j2p_or_above)
        {
        for (fg_iter = 0; fg_iter < Cint_field_st_multi_nof_st_fg; fg_iter++)
        {
            rv = bcm_field_action_destroy(unit, Cint_field_st_multi_uda_id[fg_iter]);
            if(rv != BCM_E_NONE)
            {
