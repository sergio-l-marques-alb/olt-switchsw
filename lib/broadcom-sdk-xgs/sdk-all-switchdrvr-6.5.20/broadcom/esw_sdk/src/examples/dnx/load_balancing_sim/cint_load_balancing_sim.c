 ...
 * cint cint_load_balancing_fields_ag.c
 * cint cint_load_balancing_sim.c
 * cint cint_load_balancing_get_hw_cfg.c
 ...
 * pkt_headers_info.header_offsets[2].header_type = LB_SIM_HEADER_IPV4;
 *
 ...
 *
 * uint32 lb_keys[5];
 * load_balancing_sim_main(packet_header,packet_size, &pkt_headers_info, &load_blancing_sim_hw_cfg, lb_keys);
 *
 */
...
int DEVICE_TYPE_QUMRAN2_B0 = 3;
int DEVICE_TYPE_JERICHO2C = 4;
int DEVICE_TYPE_JERICHO2P = 5;
/*
...
struct header_offsets_t
{
    ...
    int flags;
    /*
     * Indicate which header fields are used in hashing.
     */
    int field_enablers;
};
...
{
    if (device == DEVICE_TYPE_QUMRAN2_A0 || device == DEVICE_TYPE_QUMRAN2_B0 || device == DEVICE_TYPE_JERICHO2C || device == DEVICE_TYPE_JERICHO2P)
    {
        HASH_WIDTH_WITH_PADDING = 160;
        if (device == DEVICE_TYPE_QUMRAN2_A0 || device == DEVICE_TYPE_QUMRAN2_B0 || device == DEVICE_TYPE_JERICHO2P)
        {
    ...
    uint32* lb_keys)
{
    ...
    while((header_offset_iter < headers_info->nof_header_offsets) && (layer_record_counter < NOF_LAYERS))
    {
        ...
        {
            if (headers_info->header_offsets[header_offset_iter].field_enablers != LB_SIM_HEADER_ALL_ENABLED)
            {
        ...
        for(field_iter = 0; field_iter < MAX_NOF_FIELDS_OFFSETS; field_iter++)
        {
            ...
            if(field_offset.offset == -1)
                break;
            if ((headers_info->header_offsets[header_offset_iter].field_enablers & (1 << field_iter)) != 0)
            {
                continue;
            }
            ...
            {
                /** If the symmetrical hashing is turned off for this header, then the originally symmetrical fields are copied one after the other. */
                if (headers_info->header_offsets[header_offset_iter].field_enablers & LB_SIM_HEADER_NON_SYMMETRICAL_HASHING)
                {
