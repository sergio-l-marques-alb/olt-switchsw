 ...
 * the keys will be comprised of (note the keys values are LSB to MSB):
 *
 *                          SIP                      Src_Port   Dst_Port
 * Key I:     0x1100EE00FF007700BB003300DD005500   |   08E4   |   0A00
 *
 *                          DIP                     Next Header
 * Key J:     0x001100EE006600AA002200CC00440088   |   6000   |   0000
 *
 *
 * main_func:
        ...
        uint32* action_key_value)
{
    ...
    rv = cint_field_crc_hash_sim_hashing(key_a, key_b, action_key_value);
    if (rv != BCM_E_NONE)
    {
       printf("Error (%d), in cint_field_crc_hash_sim_hashing \n", rv);
       return rv;
    }
    return rv;
}
        ...
        uint32 *action_key)
{
    ...
    rv = cint_field_crc_hash_compress_and_switch(1, 0, key_a, key_b, &compressed_key_a, &compressed_key_b);
    if (rv != 0)
    {
       printf("Error (%d), in cint_field_crc_hash_compress_and_switch \n", rv);
       return rv;
    }
    rv = cint_field_crc_hash_stamp_logic(1, 0, key_b, key_a, compressed_key_a, compressed_key_b, &crc_input);
    if (rv != BCM_E_NONE)
    {
       printf("Error (%d), in cint_field_crc_hash_stamp_logic \n", rv);
       return rv;
    }
    rv = cint_field_crc_hash_perform_crc(crc_input, &crc_output);
    if (rv != BCM_E_NONE)
    {
       printf("Error (%d), in cint_field_crc_hash_perform_crc \n", rv);
       return rv;
    }
    rv = cint_field_crc_hash_update_lb_keys(crc_output, key, &additional_lb_key);
    if (rv != BCM_E_NONE)
    {
       printf("Error (%d), in cint_field_crc_hash_update_lb_keys \n", rv);
       return rv;
    }

    *action_key = additional_lb_key.data[0];
    return rv;
}
        ...
        uint32 *compressed_key_b)
{
    ...
    if(compress_enable)
    {
        rv = cint_field_crc_hash_collapse(key_a, compressed_key_a);
        if (rv != BCM_E_NONE)
        {
           printf("Error (%d), in cint_field_crc_hash_collapse \n", rv);
           return rv;
        }
        rv = cint_field_crc_hash_collapse(key_b, compressed_key_b);
        if (rv != BCM_E_NONE)
        {
           printf("Error (%d), in cint_field_crc_hash_collapse \n", rv);
           return rv;
    ...
    {
        compressed_key_a = cint_field_crc_hash_lsb_n_bits(key_a, 0, 32);
        compressed_key_b = cint_field_crc_hash_lsb_n_bits(key_a, 32, 32);
    }
    if (switch_enable)
    {
        cint_field_crc_hash_sim_key_t temp;
        cint_field_crc_hash_copy_key(key_a, &temp);
        cint_field_crc_hash_copy_key(key_b, &key_a);
        cint_field_crc_hash_copy_key(temp, &key_b);
    }
    return rv;
}
...
{
    *compressed_key = cint_field_crc_hash_lsb_n_bits(key, 96, 32) ^ cint_field_crc_hash_lsb_n_bits(key, 64, 32) ^ cint_field_crc_hash_lsb_n_bits(key, 32, 32) ^ cint_field_crc_hash_lsb_n_bits(key, 0, 32);
    return BCM_E_NONE;
}
...
int cint_field_crc_hash_stamp_logic(int compress_enable, int switch_enable, cint_field_crc_hash_sim_key_t key_a, cint_field_crc_hash_sim_key_t key_b, uint32 compressed_key_a, uint32 compressed_key_b, cint_field_crc_hash_sim_key_t *output)
{
    if(compress_enable)
    {
        output->data[3] = cint_field_crc_hash_lsb_n_bits(key_a, key_a.size - 32, 32);
        output->data[4] = cint_field_crc_hash_lsb_n_bits(key_b, key_b.size - 32, 32);
    ...
    else
    {
        if (switch_enable)
        {
            output->data[3] = cint_field_crc_hash_lsb_n_bits(key_b, key_b.size - 32, 32);
            output->data[4] = cint_field_crc_hash_lsb_n_bits(key_b, key_b.size - 64, 32);
            output->data[5] = compressed_key_a;
            output->data[6] = compressed_key_b;
        }
        else
        ...
        cint_field_crc_hash_sim_key_t *output)
{
    ...
    for (i = 0; i < input.size; i++) {
        ...
        if (cint_field_crc_hash_msb(temp))
        {
        ...
        rv = cint_field_crc_hash_set_lsb_n_bits(&crc_result, 0, 1, cint_field_crc_hash_lsb_bit(input, i) ^ cint_field_crc_hash_msb(temp));
        if (rv != BCM_E_NONE)
        {
           printf("Error (%d), in cint_field_crc_hash_set_lsb_bit \n", rv);
           return rv;
        }
    }
    output->data[0] = cint_field_crc_hash_lsb_n_bits(crc_result, 0, size);
    return rv;
}
...
int cint_field_crc_hash_xor(int size, cint_field_crc_hash_sim_key_t input, cint_field_crc_hash_sim_key_t *result)
{
    ...
    for (i = 0; i < size; i++)
    {
        for (j = 0; (i + j * size) < input.size; j++)
        {
            ...
            rv = cint_field_crc_hash_set_lsb_n_bits(result, i, 1, temp);
            if (rv != BCM_E_NONE)
            {
               printf("Error (%d), in cint_field_crc_hash_set_lsb_bit \n", rv);
               return rv;
...
{
    int rv = BCM_E_NONE;
    output->data[0] = cint_field_crc_hash_reverse_bits(cint_field_crc_hash_sim_augmented_lb_key_value, 16);

    switch(cint_field_crc_hash_sim_hash_action)
    {
    case bcmFieldContextHashActionValueReplaceCrc:
        output->data[0] = input_crc.data[0];
        break;
    case bcmFieldContextHashActionValueAugmentCrc:
        rv = cint_field_crc_hash_augment(input_crc, output);
        break;
    case bcmFieldContextHashActionValueAugmentKey:
        rv = cint_field_crc_hash_augment(key, output);
        break;
    }

    output->data[0] = cint_field_crc_hash_reverse_bits(output->data[0], output->size);
    return rv;
}
        ...
        cint_field_crc_hash_sim_key_t *output)
{
    ...
    if (cint_field_crc_hash_sim_lsb_to_msb)
    {
        ...
        cint_field_crc_hash_shift_to_lsb(&hash_initial_value, 80 - augmented_value.size);
    }
    else
    {
        seed = 0;
        ...
        cint_field_crc_hash_shift_to_lsb(&hash_initial_value, 64 - augmented_value.size);
    }
    rv = cint_field_crc_hash_calc_crc(16, seed, hash_initial_value, polynomial, output);
    if (rv != BCM_E_NONE)
    {
       printf("Error (%d), in cint_field_crc_hash_calc_crc \n", rv);
       return rv;
    }
    return rv;
}
        ...
        cint_field_crc_hash_sim_key_t *crc)
{
    ...
    for (i = 0; i < data.size; i++)
    {
        ...
        for (j = 0; j < size; j++)
        {
            hash_bit = (hash_output >> j) & 1;
            taps_bit = (polynomial >> j) & 1;
            new_bit = hash_bit ^ (msb_bit & taps_bit);
            hash_output ^= ((0 - new_bit) ^ hash_output) & (1 << j);
        }
    }
    msb_bit = (hash_output >> (size - 1)) & 1;
    crc->data[0] = cint_field_crc_hash_reverse_bits(hash_output, 16);
    rv = cint_field_crc_hash_set_msb_n_bits(crc, 0, 1, msb_bit);
    if (rv != BCM_E_NONE)
    {
       printf("Error (%d), in cint_field_crc_hash_set_msb_n_bits \n", rv);
       return rv;
    }
    return rv;
}

uint32 cint_field_crc_hash_msb(cint_field_crc_hash_sim_key_t input)
{
    return input.data[0] & 1;
}

uint32 cint_field_crc_hash_lsb_n_bits(cint_field_crc_hash_sim_key_t key, int lsb, int count)
{
...
{
    return cint_field_crc_hash_lsb_n_bits(key, lsb, 1);
}
...
{
    if(count > CRC_HASH_SIM_UINT32_NOF_BITS) {
        return BCM_E_PARAM;
    }

    uint32 mask, mask1, mask2, data1, data2;
    lsb = key->size - lsb;
    int lsb_shift = lsb % CRC_HASH_SIM_UINT32_NOF_BITS;

    mask1 = (1 << ((lsb - count) % CRC_HASH_SIM_UINT32_NOF_BITS)) - 1;
    mask2 = 0 - (1 << ((lsb_shift) % CRC_HASH_SIM_UINT32_NOF_BITS));

    data1 = data << ((lsb - count) % CRC_HASH_SIM_UINT32_NOF_BITS);
    data2 = data >> (CRC_HASH_SIM_UINT32_NOF_BITS - ((lsb - count) % CRC_HASH_SIM_UINT32_NOF_BITS));

    if((lsb / CRC_HASH_SIM_UINT32_NOF_BITS) != ((lsb - count) / CRC_HASH_SIM_UINT32_NOF_BITS))
    {
        key->data[(lsb - count) / CRC_HASH_SIM_UINT32_NOF_BITS] = (key->data[(lsb - count) / CRC_HASH_SIM_UINT32_NOF_BITS] & (mask1)) | (~mask1 & data1);
        key->data[lsb / CRC_HASH_SIM_UINT32_NOF_BITS] = (key->data[lsb / CRC_HASH_SIM_UINT32_NOF_BITS] & (mask2)) | (~mask2 & data2);
    }
    else {
        mask = mask1 | mask2;
        key->data[lsb / CRC_HASH_SIM_UINT32_NOF_BITS] = (key->data[lsb / CRC_HASH_SIM_UINT32_NOF_BITS] & (mask)) | (~mask & data1);
    }
    return BCM_E_NONE;
}
