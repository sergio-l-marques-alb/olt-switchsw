...
* \brief
*  Configures:
*  1) Hash - 5 tupple of IPv4, assuemd that packet type is ETH | IPV4 | TCP/UDP
...
int cint_field_ele_flow_FEM_add(int unit)
{
    ...
    int input_offset=0;
    int ii = 0;
    int dnx_data_exem_vmv_removable_from_payload = *(dnxc_data_get(unit, "field", "features", "exem_vmv_removable_from_payload", NULL));
    int vmv_shift;
    int rv = BCM_E_NONE;

    /**In case device does not return vmv to PMF addtional 4 bits need to be shifted at input*/
    if(dnx_data_exem_vmv_removable_from_payload)
    {
        input_offset = EXEM_32MSB_SHIFT-4;
        vmv_shift = 0;
    }
    else
    {
        input_offset = EXEM_32MSB_SHIFT;
        vmv_shift = 4;
    }
    ...
    /*Any action priority (FEM_ID) can be chosen here*/
    action_priority = BCM_FIELD_ACTION_POSITION(3, 6);
    rv = bcm_field_fem_action_add(unit, 0, cint_field_ele_flow_exem_fg_id, action_priority, &fem_action_info);
    if(rv != BCM_E_NONE)
    {
...
int cint_field_ele_flow_exem_fg_set(int unit)
{
    ...
    rv = bcm_field_group_add(unit, BCM_FIELD_FLAG_MSB_RESULT_ALIGN, &exem_fg_info, &cint_field_ele_flow_exem_fg_id);
    if (rv != BCM_E_NONE)
    {
...
/**
* \brief
*  Configures Elephant flow, for IPv4 packet
* \param [in] unit        -  Device id
* \param [in] fwd_layer        -  Assumed that packet is Eth | IPv4 in this cint, so fwd_layer can be either ETH or IPv4
* \return
*   int - Error Type
* \remark
*   * None
...
int cint_field_ele_flow_main(int unit,  bcm_field_layer_type_t fwd_layer, int snoop_cmd, int snoop_str, int age_profile)
{
    ...
    /*1. Packet assumed to be ETH | IPv4 | TCP//UDP*/
    if(fwd_layer != bcmFieldLayerTypeEth && fwd_layer != bcmFieldLayerTypeIp4)
    {
        printf("Error Forwarding Layer type can be Ethernet or IPv4\n");
        return BCM_E_PARAM;
    }
