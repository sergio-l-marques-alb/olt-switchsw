 ...
 *  cint
 *  cint_elephant_flow_init(unit, inPort);
 ...
 */
int
cint_elephant_flow_init(int unit, int in_port)
{
    int rv;
    ...
    printf("cint_elephant_flow_init: Set port = %d Trace probability to %d\n", in_port, trace_prob);
...
cint_elephant_flow_main(int unit, int in_port, int route_out_port, int cpu_port)
{
    ...
    flush_profile_info.high_threshold_value = 20;
    flush_profile_info.low_threshold_value = 5;
    flush_profile_info.increment_value = 2;
    flush_profile_info.decrement_value = 2;
    flush_profile_info.maximal_value = 50;
    flush_profile_info.out_value = 0;
    flush_profile_info.init_value = 10;
     ...
     */
    snoop_str = 7;
    rv = cint_field_ele_flow_main(unit, bcmFieldLayerTypeIp4, snoop_cmd, snoop_str, cint_elephant_flow_info.flush_profile_id);
    if (rv != BCM_E_NONE)
    {
