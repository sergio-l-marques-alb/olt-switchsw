 ...
 * \brief
 *      cint example of configrung STIF for Billing mode.
 *      use as example for J2P - OP2 connection:
 *      OP2 works in 1Ghz and J2P works on 1.5Ghz, 
 *      meaning OP2 cannot hold the records rate of J2P
 *      Solution: use 4 STIF ports, 2 ports per core
 *      records are split between both ports - 50% records for each port
 *      this way for 1500Mrps per core - each port will get max Ingress 750Mrps and Egress 750Mrps
 *  
 *      The sequence is:
 *      a. configure STIF soc properties 
 *      b. configure source mapping according to the selected
