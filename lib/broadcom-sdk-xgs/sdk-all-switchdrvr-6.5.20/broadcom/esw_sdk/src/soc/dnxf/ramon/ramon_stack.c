        ...
        if (valid_field == 1) /* The register is holding a fap id which is ignored */
        {
            if (id_field == module) 
            {
                if (arg == 0) /* need to clear this register */ 
                {
                    ...
                    SHR_EXIT();  /* the way we implemented, each fap id will be ignored only once, so we can exit safely without worrying that an additional register will ignore this id*/
                }
