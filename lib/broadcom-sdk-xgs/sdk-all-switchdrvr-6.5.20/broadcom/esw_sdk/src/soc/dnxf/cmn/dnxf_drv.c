    ...
    int unit)
{
    ...
    for (mem = 0; mem < NUM_SOC_MEM; mem++)
    {
        ...
        /* BRDC memory should not be cacheable */
    ...
    int unit)
{
    ...
    if (!SOC_WARM_BOOT(unit) && !SOC_IS_RELOADING(unit))
    {
        /*
         * Stop all interrupts!
         * During the isolation process it is expected for some interrupts to start pop up
         * This may trigger interrupt thread error recovery mechanism which may be even hard reset.
         * This will lead to hard reset during soft reset which will result in unknown device state.
         */
...
/* This function should not be used for future devices. */
int
    ...
    int unit)
{
    ...
    /** Handle iproc hot swap events, abort previous DMA operations and disable DMA channels */
 ...
 * This function main role is to iter all memory. Deallocate table cache memory, if caching enabled 
 */
