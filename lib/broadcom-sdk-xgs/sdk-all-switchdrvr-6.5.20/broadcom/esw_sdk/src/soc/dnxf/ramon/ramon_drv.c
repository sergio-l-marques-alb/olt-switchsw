...
_soc_ramon_drv_fabric_local_routing_set(int unit, int local_routing_enable_uc, int local_routing_enable_mc)
{
    ...
    for (i = 0; i < nof_qrh/2; i++) {
        ...
        /* If FE2 gets disconnected, use local route to send the MC traffic (best effort), even if MC local route wasn't enabled */
        soc_reg_field_set(unit, QRH_LOCAL_ROUTE_CONFIGURATIONSr, &reg_val32, MC_FORCE_LOCAL_ENf, 0x1);
        /* If it is not possible anymore to reach all dest FAPs via local route (for example a cable between the FE13 and one of the FAPs got disconnected),
         * don't send the MC via local route, but send to FE2, because there is a good chance it will reach all dest FAPs that way.
         * There is no partial local route. */
...
soc_ramon_set_dcml_config(int unit)
{
    ...
    /* Disable all links initially from  sending reachaboility cells
     * After dnxf_port_init the enabled by soc_properties ports
     * will be enabled and will start sending reachability cells 
     * The same registers are modified in soc_ramon_port_enable_set 
     * when enabeling or diabeling a port 
     */
