...
/*
 * Function to return a pointer to the table to parse each field of the captured CELL in DCML_DFL_CPU_CELLm.
 * Note! Inband cells are not implemented
 */
shr_error_e
soc_ramon_fabric_cell_parse_table_get(int unit, soc_dnxc_fabric_cell_entry_t entry, soc_dnxc_fabric_cell_type_t *cell_type, soc_dnxc_fabric_cell_parse_table_t *parse_table)
{
    ...
    /**
     * Cell size field is saving 1 bit and we should compensate
     * (this is a trick often used to save space in the registry field)
     */
    cell_size_bytes +=1;
    ...
    /**
     * Copy relevant table based on cell_type
     */
    if (*cell_type == soc_dnxc_fabric_cell_type_sr_cell)
    {
        ...
        /**
         * Adjust payload field based on CELL specifics
         */
        ...
        if (cell_size_bytes <= SOC_RAMON_FABRIC_CAPTURED_CELL_SMALL_CELL_MAX_SIZE)
        {
            ...
            {
                /**
                 * If data is in LSB (1023:0) start bit is still 0 and than only adjust the length
                 */
            }
            /**
             * Inside the valid range itself we have an offset based on following formula
             */
        ...
        {
            /*
             * Big cells are always next to the header (bits [2047:0])
             */
        }
    ...
    else
    {
        ...
        if (cell_size_bytes <= SOC_RAMON_FABRIC_CAPTURED_CELL_SMALL_CELL_MAX_SIZE)
        {
            ...
            {
                /**
                * If data is in NOT MSB than it should be in (1024:2047) not in (0:2047)
                */
                parse_table[line].src_start_bit += SOC_RAMON_FABRIC_CAPTURED_CELL_DATA_IN_MSB_OFFSET;
            }
            else
            {
                /**
                * If data is in NOT MSB than it should be in (0:1023) not in (0:2047)
                */
