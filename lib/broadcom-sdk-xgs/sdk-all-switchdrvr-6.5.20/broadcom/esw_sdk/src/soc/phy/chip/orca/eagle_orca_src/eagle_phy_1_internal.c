/***********************************************************************************
 ***********************************************************************************
 *  File Name     :  eagle_phy_1_internal.c                                         *
 *  Created On    :  13/02/2014                                                    *
 *  Created By    :  Justin Gaither                                                *
 *  Description   :  APIs for Serdes IPs                                           *
 *  Revision      :  $Id: eagle_phy_1_internal.c 1553 2017-05-12 21:01:08Z cvazquez $ *
 *                                                                                 *
...
static uint8_t _eagle_phy_1_rdb_uc_var(err_code_t *err_code_p, uint16_t addr) {
    uint8_t rddata;
    ...
    EPSTM(rddata = eagle_phy_1_rdb_uc_ram(err_code_p, addr)); /* Use Micro register interface for reading RAM */

    return (rddata);
}

/* Micro Global RAM Word Read */
static uint16_t _eagle_phy_1_rdw_uc_var(err_code_t *err_code_p, uint16_t addr) {
  uint16_t rddata;
...
#endif
  EPSTM(rddata = eagle_phy_1_rdw_uc_ram(err_code_p, (addr))); /* Use Micro register interface for reading RAM */

  return (rddata);
}

/* Micro Global RAM Byte Write  */
static err_code_t _eagle_phy_1_wrb_uc_var(uint16_t addr, uint8_t wr_val) {

    return (eagle_phy_1_wrb_uc_ram(addr, wr_val));                                /* Use Micro register interface for writing RAM */
}
/* Micro Global RAM Word Write  */
static err_code_t _eagle_phy_1_wrw_uc_var(uint16_t addr, uint16_t wr_val) {
    if (addr%2 != 0) {                                                                /* Validate even address */
        return (_error(ERR_CODE_INVALID_RAM_ADDR));
    }
    return (eagle_phy_1_wrw_uc_ram(addr, wr_val));                                 /* Use Micro register interface for writing RAM */
}
...
static char* _error_val_2_str(uint8_t val) {
        ...
        case INVALID_OTP_CONFIG_FAIL_PLL:
                return ("INVALID OTP CONFIGURATION FAIL PLL");
        case INVALID_OTP_CONFIG_FAIL_DFE:
                return ("INVALID OTP CONFIGURATION FAIL DFE");
...
static err_code_t _set_rx_pf_main(uint8_t val) {
        if (val > 15) {
...
static err_code_t _set_rx_pf2(uint8_t val) {
        if (val > 7) {
                return (_error(ERR_CODE_PF_INVALID));
        }
...
static err_code_t _set_rx_vga(uint8_t val) {
...
static err_code_t _get_tx_driver_mode(int8_t *val) {
    ESTM(*val = rd_ams_tx_drivermode());
    return(ERR_CODE_NONE);
}


static err_code_t _eagle_phy_1_core_clkgate(uint8_t enable) {
...
static err_code_t _eagle_phy_1_lane_clkgate(uint8_t enable) {
...
SDK_STATIC err_code_t _eagle_phy_1_get_osr_mode(eagle_phy_1_osr_mode_st *imode) {
    eagle_phy_1_osr_mode_st mode;

     ENULL_MEMSET(&mode, 0, sizeof(eagle_phy_1_osr_mode_st));
...
SDK_STATIC err_code_t _eagle_phy_1_read_lane_state(eagle_phy_1_lane_state_st *istate) {

  eagle_phy_1_lane_state_st state;
  uint8_t ladder_range = 0;

  ENULL_MEMSET(&state, 0, sizeof(eagle_phy_1_lane_state_st));

  if(!istate)
          return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));

  EFUN(_eagle_phy_1_pmd_lock_status(&state.rx_lock, &state.rx_lock_chg));
  ...
  EFUN(_eagle_phy_1_sigdet_status(&state.sig_det, &state.sig_det_chg));
  ESTM(state.rx_ppm = rd_cdr_integ_reg()/84);
  EFUN(eagle_phy_1_get_clk90_offset(&state.clk90));
  ...
  ESTM(state.tx_ppm = (int16_t)(((int32_t)(rdc_tx_pi_integ2_reg()))*3125/32768));
...
static err_code_t _eagle_phy_1_display_lane_state_no_newline(void) {
  uint16_t lane_idx;
  eagle_phy_1_lane_state_st state;
  ...
  EFUN(_eagle_phy_1_read_lane_state(&state));

  lane_idx = eagle_phy_1_get_lane();
...
SDK_STATIC err_code_t _eagle_phy_1_read_core_state(eagle_phy_1_core_state_st *istate) {
  eagle_phy_1_core_state_st state;
  struct eagle_phy_1_uc_core_config_st core_cfg;

  ENULL_MEMSET(&state, 0, sizeof(eagle_phy_1_core_state_st));
  ENULL_MEMSET(&core_cfg, 0, sizeof(struct eagle_phy_1_uc_core_config_st));
  ...
  {  int16_t                           die_temp = 0;
     EFUN(eagle_phy_1_read_die_temperature(&die_temp));
  ...
  EFUN(_eagle_phy_1_pll_lock_status(&state.pll_lock, &state.pll_lock_chg));
...
static err_code_t _eagle_phy_1_display_core_state_no_newline(void) {
    eagle_phy_1_core_state_st state;
    ENULL_MEMSET(&state     , 0, sizeof(state     ));
        EFUN(_eagle_phy_1_read_core_state(&state));
    EFUN_PRINTF((" %02d "              ,  eagle_phy_1_get_core()));
    ...
    EFUN(_eagle_phy_1_display_pll_to_divider(  state.pll_div));
...
SDK_STATIC err_code_t _eagle_phy_1_pll_lock_status(uint8_t *pll_lock, uint8_t *pll_lock_chg) {

    uint16_t rddata;
    EFUN(eagle_phy_1_pmd_rdt_reg(PLL_STATUS_ADDR, &rddata));
...
SDK_STATIC err_code_t _eagle_phy_1_pmd_lock_status(uint8_t *pmd_lock, uint8_t *pmd_lock_chg) {

    uint16_t rddata;
    EFUN(eagle_phy_1_pmd_rdt_reg(PMD_LOCK_STATUS_ADDR, &rddata));
    *pmd_lock     = (rddata & 0x1);
...
SDK_STATIC err_code_t _eagle_phy_1_sigdet_status(uint8_t *sig_det, uint8_t *sig_det_chg) {
    uint16_t rddata;
    EFUN(eagle_phy_1_pmd_rdt_reg(SIGDET_STATUS_ADDR, &rddata));
...
static err_code_t _update_uc_lane_config_st(struct  eagle_phy_1_uc_lane_config_st *st) {
  uint16_t in = st->word;
...
static err_code_t _update_usr_ctrl_disable_functions_st(struct eagle_phy_1_usr_ctrl_disable_functions_st *st) {
  uint8_t in = st->byte;
...
static err_code_t _update_usr_ctrl_disable_dfe_functions_st(struct eagle_phy_1_usr_ctrl_disable_dfe_functions_st *st) {
  uint8_t in = st->byte;
...
static err_code_t _update_uc_core_config_st(struct eagle_phy_1_uc_core_config_st *st) {
  uint16_t in = st->word;
...
static err_code_t _update_uc_core_config_word(struct eagle_phy_1_uc_core_config_st *st) {
  uint16_t in = 0;
...
static err_code_t _update_uc_lane_config_word(struct eagle_phy_1_uc_lane_config_st *st) {
  uint16_t in = 0;
...
static err_code_t _update_usr_ctrl_disable_functions_byte(struct eagle_phy_1_usr_ctrl_disable_functions_st *st) {
  uint8_t in = 0;
...
static err_code_t _update_usr_ctrl_disable_dfe_functions_byte(struct  eagle_phy_1_usr_ctrl_disable_dfe_functions_st *st) {
  uint8_t in = 0;
...
static err_code_t _eagle_phy_1_display_pll_to_divider(uint8_t val) {
...
static err_code_t _eagle_phy_1_display_ber_scan_data(uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    ...
    uint8_t range250;
    eagle_phy_1_osr_mode_st osr_mode;
    struct eagle_phy_1_uc_core_config_st core_config;

    ENULL_MEMSET(&core_config,0,sizeof(core_config));
    ENULL_MEMSET(&osr_mode,0,sizeof(osr_mode));

    EFUN(eagle_phy_1_get_uc_core_config(&core_config));
    EFUN(_eagle_phy_1_get_osr_mode(&osr_mode));
    ...
    if(BER_PROJ_VERBOSE > 2) EFUN_PRINTF(("start begin\n"));
    ...
    if(BER_PROJ_VERBOSE > 2) EFUN_PRINTF(("delay done\n"));
    ...
    for(i=0;i < cnt;i++) {
        ...
        EFUN(eagle_phy_1_pmd_uc_cmd( CMD_READ_DIAG_DATA_WORD, 0, 100));
        ...
        EFUN(eagle_phy_1_pmd_uc_cmd( CMD_READ_DIAG_DATA_BYTE, 0, 100));
    ...
    EFUN_PRINTF(("\n"));
    EFUN(eagle_phy_1_pmd_uc_cmd(CMD_CAPTURE_BER_END,0x00,2000));

  return(ERR_CODE_NONE);
}

void get_dsc_param_orca(void) {
    eagle_phy_1_display_lane_state_hdr();
    eagle_phy_1_display_lane_state();
}
