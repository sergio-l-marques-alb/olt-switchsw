...
_phy56275_ieee_compliance_setup(int unit, phy_ctrl_t *pc)
{
    ...
    SOC_IF_ERROR_RETURN
        (phy_egphy16_rdb_reg_write(unit, pc, 0x033, 0x0006));
    /* CORE_EXP03: Release reset */
    SOC_IF_ERROR_RETURN
        (phy_egphy16_rdb_reg_write(unit, pc, 0x033, 0x0000));
    /* PLL_PLL_RESET_CONTROL : Reset PLL*/
    SOC_IF_ERROR_RETURN
        (phy_egphy16_rdb_reg_write(unit, pc, 0x1c1, 0x0000));
    /* PLL_NDIV_2 : ndiv_integer */
    SOC_IF_ERROR_RETURN
        (phy_egphy16_rdb_reg_write(unit, pc, 0x1c4, 0x00A0));
    /* PLL_MANUAL_CONFIG : pdiv=1 : default Kp,Ki */
    SOC_IF_ERROR_RETURN
        (phy_egphy16_rdb_reg_write(unit, pc, 0x1ce, 0x0001));
    /* PLL_NDIV: Ndiv_fraction*/
    SOC_IF_ERROR_RETURN
        (phy_egphy16_rdb_reg_write(unit, pc, 0x1c5, 0x0000));
    /* PLL_NUDGE: frequency nudge factor*/
    SOC_IF_ERROR_RETURN
        (phy_egphy16_rdb_reg_write(unit, pc, 0x1c6, 0x0000));
    /* PLLREF_REFERENCE: reference frequency: 25MHz, mode 0*/
    SOC_IF_ERROR_RETURN
        (phy_egphy16_rdb_reg_write(unit, pc, 0x1c3, 0x0019));
    /* PLL_CONTROLS_2: Default init_bypass mode*/
    SOC_IF_ERROR_RETURN
        (phy_egphy16_rdb_reg_write(unit, pc, 0x1cb, 0x0000));
    /* PLL_CONTROLS: Bypass code*/
    SOC_IF_ERROR_RETURN
        (phy_egphy16_rdb_reg_write(unit, pc, 0x1cc, 0x0002));
    /* PLL_LDO_CONTROL: LDOs at default settings*/
    SOC_IF_ERROR_RETURN
        (phy_egphy16_rdb_reg_write(unit, pc, 0x1c2, 0x01c0));
    /*
     * PLL_CONFIG_OVERRIDE (PLL_MANUAL_CONFIG_2):
     * config override, disable
     */
    SOC_IF_ERROR_RETURN
        (phy_egphy16_rdb_reg_write(unit, pc, 0x1cd, 0x0000));
    /* CORE_EXP03: Release PLL reset*/
    SOC_IF_ERROR_RETURN
        (phy_egphy16_rdb_reg_write(unit, pc, 0x0033, 0x0001));
    sal_usleep(10000);
    /* CORE_EXP03: Release reset*/
    SOC_IF_ERROR_RETURN
        (phy_egphy16_rdb_reg_write(unit, pc, 0x0033, 0x0003));
    /* AFE_BG_CONFIG: Bandgap curvature correction to correct default */
    SOC_IF_ERROR_RETURN
        (phy_egphy16_rdb_reg_write(unit, pc, 0x1e0, 0x0010));
    /* AFE_CAL_CONFIG_2: enable max averaging*/
    SOC_IF_ERROR_RETURN
        (phy_egphy16_rdb_reg_write(unit, pc, 0x1e7, 0x0038));
    sal_usleep(10000);
    /* AFE_CAL_CONFIG_2: no reset and analog powerup */
    SOC_IF_ERROR_RETURN
        (phy_egphy16_rdb_reg_write(unit, pc, 0x1e7, 0x003b));
    sal_usleep(10000);
    /* AFE_CAL_CONFIG_2: start calibration */
    SOC_IF_ERROR_RETURN
        (phy_egphy16_rdb_reg_write(unit, pc, 0x1e7, 0x003f));
    /* AFE_CAL_CONFIG_0: Vref=1000, Target=10, averaging enabled*/
    SOC_IF_ERROR_RETURN
        (phy_egphy16_rdb_reg_write(unit, pc, 0x1e5, 0x1c82));
    sal_usleep(10000);
    /* AFE_CAL_CONFIG_0: no reset and analog powerup*/
    SOC_IF_ERROR_RETURN
        (phy_egphy16_rdb_reg_write(unit, pc, 0x1e5, 0x9e82));
    sal_usleep(10000);
    /* AFE_CAL_CONFIG_0: start calibration */
    SOC_IF_ERROR_RETURN
        (phy_egphy16_rdb_reg_write(unit, pc, 0x1e5, 0x9f82));
    /* AFE_CAL_CONFIG_2: select Rext*/
    SOC_IF_ERROR_RETURN
        (phy_egphy16_rdb_reg_write(unit, pc, 0x1e7, 0x007f));
    /* AFE_BIAS_CONFIG_0: Adjust 10BT amplitude additional +7% and 100BT +2%*/
    SOC_IF_ERROR_RETURN
        (phy_egphy16_rdb_reg_write(unit, pc, 0x1e1, 0xe7ea));
    /* AFE_BIAS_CONFIG_1: Adjust 1G mode amplitude and 1G testmode1*/
    SOC_IF_ERROR_RETURN
        (phy_egphy16_rdb_reg_write(unit, pc, 0x1e2, 0xede0));
    /* CORE_EXPA9: Read data */
    SOC_IF_ERROR_RETURN
        (phy_egphy16_rdb_reg_read(unit, pc, 0x0a9, &rd_data));
    ...
    SOC_IF_ERROR_RETURN
        (phy_egphy16_rdb_reg_write(unit, pc, 0x1e7, wr_data));
    /* AFE_BIAS_CONFIG_0: Adjust 10BT amplitude additional +7% andi 100BT +2% */
    SOC_IF_ERROR_RETURN
        (phy_egphy16_rdb_reg_write(unit, pc, 0x1e1, 0xe7dd));
    /*
     * AFE_RXCONFIG_3:
     * invert adc clock output and 'adc refp ldo current To correct default
     */
    SOC_IF_ERROR_RETURN
        (phy_egphy16_rdb_reg_write(unit, pc, 0x1ec, 0x8002));
    /*
     * AFE_TX_CONFIG_1:
     * 100BT stair case, high BW, 1G stair case, alternate encode
     */
    SOC_IF_ERROR_RETURN
        (phy_egphy16_rdb_reg_write(unit, pc, 0x1f3, 0xf882));
    /*
     * AFE_TX_CONFIG_2:
     * 1000BT DAC transition method per Erol, bits[32], DAC Shuffle sequence 1
     */
    SOC_IF_ERROR_RETURN
        (phy_egphy16_rdb_reg_write(unit, pc, 0x1f4, 0x0201));
    /* AFE_RXCONFIG_1: some non-overlap fix*/
    SOC_IF_ERROR_RETURN
        (phy_egphy16_rdb_reg_write(unit, pc, 0x1ea, 0x0c00));
    /* AFE_RXCONFIG_0 */
    SOC_IF_ERROR_RETURN
        (phy_egphy16_rdb_reg_write(unit, pc, 0x1e9, 0x0002));
    /* AFE_RXCONFIG_CH0 */
    SOC_IF_ERROR_RETURN
        (phy_egphy16_rdb_reg_write(unit, pc, 0x1ee, 0x0000));
    /* AFE_RXCONFIG_CH1 */
    SOC_IF_ERROR_RETURN
        (phy_egphy16_rdb_reg_write(unit, pc, 0x1ef, 0x0000));
    /* AFE_RXCONFIG_CH2 */
    SOC_IF_ERROR_RETURN
        (phy_egphy16_rdb_reg_write(unit, pc, 0x1f0, 0x0000));
    /* AFE_RXCONFIG_CH3 */
    SOC_IF_ERROR_RETURN
        (phy_egphy16_rdb_reg_write(unit, pc, 0x1f1, 0x0000));
    /* AFE_RXCONFIG_2 */
    SOC_IF_ERROR_RETURN
        (phy_egphy16_rdb_reg_write(unit, pc, 0x1eb, 0x0800));
    sal_usleep(10000);
    sal_usleep(10000);
    /* AFE_RXCONFIG_0 */
    SOC_IF_ERROR_RETURN
        (phy_egphy16_rdb_reg_write(unit, pc, 0x1e9, 0x0000));
