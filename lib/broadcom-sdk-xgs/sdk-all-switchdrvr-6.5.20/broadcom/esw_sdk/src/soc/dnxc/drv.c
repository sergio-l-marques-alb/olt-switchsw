...
/** value used to mark that a system reset of the device is needed */
    ...
    int needs_system_reset)
{
    ...
    /** mark that the device needs a system reset */
    ...
    int unit)
{
        ...
        SOC_CONTROL(unit)->system_reset_cb(unit, SOC_SYSTEM_RESET_FLAG_IS_SUPPORTED) == SOC_E_NONE)
    {
        ...
        {   /** check that the system reset was successful */
            ...
            if (results == SOC_E_NONE)
            { /** If the system reset was performed successfully, mark that a system reset is not needed any more */
                soc_dnxc_set_need_system_reset(unit, 0);
                /*
                 * LOG_ERROR(BSL_LOG_MODULE, (BSL_META_U(unit, "Please re-run the SDK\n"))); exit(0);
                 */
            }
        }
