    ...
    char *msg)
{
    ...
    /*
     * corrective action, log move ecc failure
     */
    LOG_ERROR(BSL_LS_SOC_INTR, (BSL_META_U(unit, "%s\n"), msg));
    ...
    sand_intr_tcam_location * location)
{
    ...
    /*
     * Get the kaps scan index an error occurred(scan machine pause at this index for ecc_2bits)
     */
    if (is_ecc_2bits)
    {
        ...
        /*
         * Read RPB tcam scan pause register to resume scan machine cycle
         */
        if (is_rpb_a)
        {
    ...
    char *msg)
{
    ...
    /*
     * corrective action, log query failure reason
     */
    LOG_ERROR(BSL_LS_SOC_INTR, (BSL_META_U(unit, "%s\n"), msg));
