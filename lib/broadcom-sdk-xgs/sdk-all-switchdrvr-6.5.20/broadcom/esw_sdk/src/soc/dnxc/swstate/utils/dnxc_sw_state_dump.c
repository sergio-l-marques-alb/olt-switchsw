...
/* return the number of allocated elements pointed by ptr */
uint32
    ...
    void *ptr)
{
    ...
    /* ptr expected to be only swstate pointer */
...
/* documentation in h file (dnx_sw_state_dump.h) */
    ...
    char *first_string_to_print)
{
    ...
    if (dnx_sw_state_stride_dump_output_file[unit] != NULL)
        return;
    /* attach that comes from swstate config comes with first_string_to_print == ""
       this is the only case when we want to print to file if dnx_sw_state_stride_dump_directory_name[unit] == "" */
...
dnx_sw_state_dump_string(
    ...
    char *str)
{
    /* sometimes the string given by DNX_SW_STATE_PRINT_FILE should be empty
     * by the compiler doesn't eat it, so we pass " " instead of "" */
    if (sal_strcmp(str, " ") == 0) {
        return;
    }
...
#else
        /*
         * In case we do not have regex, save the pointer to the string.
         */
    ...
    char *string)
{
    ...
    int result = FALSE;
    /*
     * Variable to store the comparison handle.
     */
    void *compare_handle = NULL;

    /*
     * Init the  comparison handle.
     */
...
#else /* !defined(__KERNEL__) && !defined(NO_REGEX) */
        /*
         * Searches for the first occurrence of the substring needle in the string haystack.
         * It will be NULL, if the string was not found.
         */
        if (sal_strcasestr(compare_handle, string) != NULL)
        {
            result = TRUE;
        }
#endif /* !defined(__KERNEL__) && !defined(NO_REGEX) */
    }
