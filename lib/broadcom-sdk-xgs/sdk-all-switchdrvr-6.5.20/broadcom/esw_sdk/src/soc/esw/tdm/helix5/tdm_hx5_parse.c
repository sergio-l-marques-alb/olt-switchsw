...
/**
@name: tdm_hx5_print_pipe_config
@param:

Parses pipe config from class structs
**/
void
tdm_hx5_print_pipe_config(tdm_mod_t *_tdm)
{
    int port_phy;
    ...
    if (param_port_start >= param_phy_lo && param_port_end <= param_phy_hi){
        ...
        for (port_phy=param_port_start; port_phy<=param_port_end; port_phy++) {
            if (param_speeds[port_phy]>0) {
                ...
                TDM_PRINT1("%8d", tdm_hx5_cmn_get_port_pm(port_phy, _tdm));
...
/**
@name: tdm_hx5_print_lr_cal
@param:

Print TDM calendar content of current pipe
**/
int
tdm_hx5_print_lr_cal(tdm_mod_t *_tdm, int cal_id)
{
    int j, token;
    int mgmt_2nd_en = 0;
    ...
    param_cal_id    = cal_id;
    param_cal_len   = tdm_hx5_cmn_get_pipe_cal_len(param_cal_id, _tdm);
    TDM_SEL_CAL(param_cal_id, param_cal_main);

    COMPILER_REFERENCE(mgmt_2nd_en);
    if (param_mgmt_mode == HX5_MGMT_MODE_2_PORT_EN ||
        param_mgmt_mode == HX5_MGMT_MODE_2_PORT_HG) {
        mgmt_2nd_en = 1;
    }
    ...
    if (param_cal_main != NULL && param_cal_len > 0) {
        for (j=0; j<param_cal_len; j++) {
            ...
            if (((token != HX5_OVSB_TOKEN) && 
                 !(token >= param_phy_lo && token <= param_phy_hi)) 
                ) {
                TDM_PRINT0("contains ACCESSORY TOKEN - ");
                switch (token) {
                    case HX5_CMIC_TOKEN: TDM_PRINT0("CMIC/CPU\n"); break;
                    case HX5_FAE_TOKEN: TDM_PRINT0("FAE-1\n"); break;
                    case HX5_LPB0_TOKEN: TDM_PRINT0("LOOPBACK-0\n"); break;
                    case HX5_IDL1_TOKEN: TDM_PRINT0("OPPORTUNISTIC-1\n"); break;
                    case HX5_NULL_TOKEN: TDM_PRINT0("NULL\n"); break;
                    case HX5_IDL2_TOKEN: TDM_PRINT0("IDLE\n"); break;
...
/**
@name: tdm_hx5_print_pipe_cal
@param:

Print TDM calendar content of current pipe
**/
void
tdm_hx5_print_pipe_cal(tdm_mod_t *_tdm)
{
    int i, j, n;
    ...
    if (param_cal != NULL) {
        ...
        if (param_grp_num > 0 && param_grp_len > 0 && param_cal_grp != NULL) { 
            ...
            for (j=0; j<HX5_SHAPING_GRP_LEN; j++) {
                ...
                TDM_PRINT1("\t%d", param_cal_grp[HX5_SHAPING_GRP_IDX_0][j]);
            }
            TDM_PRINT0("\n\n");
            TDM_PRINT1("\nTDM: PIPE %d, PKT SCHEDULER HALF PIPE 1\n",param_pipe_id);
            for (j=0; j<HX5_SHAPING_GRP_LEN; j++) {
                if (j%16==0) { TDM_PRINT2("\n %3d : %3d\t", j, j+15);}
                TDM_PRINT1("\t%d",param_cal_grp[HX5_SHAPING_GRP_IDX_1][j]);
            }
            TDM_PRINT0("\n");
            TDM_SML_BAR
        }
        ...
        for (j=(16*param_cal_id); j<(16*(param_cal_id+1)) && j<param_num_phy_pm; j++) {
            TDM_PRINT2("\t %2d:%2d",j, _tdm->_chip_data.soc_pkg.soc_vars.hx5.pm_num_to_pblk[j]);
        }
        TDM_PRINT0("\n");
        TDM_SML_BAR
    }
...
/*
int
tdm_hx5_parse_mgmt(tdm_mod_t *_tdm, int cal_id)
{
        ...
        while (cnt < mgmt_slot_num && found == BOOL_TRUE) {
            ...
            if (found == BOOL_TRUE) {
                switch (param_cal_id) {
                    case 0: 
                    case 4: 
                        param_cal_main[idx] = HX5_CMIC_TOKEN;
                        break;
                    case 1: 
                    case 5: 
                        if (mgmt_2nd_en) {
                            ...
                            param_cal_main[idx] = HX5_MGM1_TOKEN;
                        }
                        break;
                    default :
                        break;
...
*/
/**
@name: tdm_hx5_parse_acc_idb
@param:

Parse ACCESSORY slots (LOOPBACK, OPP1, OPP2, NULL, IDLE)
**/
int
tdm_hx5_parse_acc_idb(tdm_mod_t *_tdm, int cal_id)
{
    int j, cnt = 0;
    int param_token_lpbk, param_token_ancl, param_cal_id, param_cal_len,param_token_fae;
    int *param_cal_main=NULL;
    ...
    switch (param_cal_id) {
        case 0:
               param_token_lpbk = HX5_LPB0_TOKEN; 
               param_token_fae =  HX5_FAE_TOKEN; 
               break;
    /*    case 1: param_token_lpbk = HX5_LPB1_TOKEN; break; */
    ...
    if (param_cal_main != NULL && param_cal_len > 0) {
        for (j=0; j<param_cal_len; j++) {
            if (param_cal_main[j] == param_token_ancl) {
                cnt++;
                switch (cnt) {
                    case 1: case 7: case 13:  case 16 : case 18 : case 20 : case 22 : case 24 :
                        param_cal_main[j] = param_token_lpbk;
                        break;
                    case 2: case 8: 
                        param_cal_main[j] = HX5_IDL1_TOKEN;
                        break;
                    case 3: case 9:
                        param_cal_main[j] = HX5_NULL_TOKEN;
                        break;
                    case 4: case 10: case 21 : case 15 :
                        param_cal_main[j] = HX5_IDL2_TOKEN;
                        break;
                    case 5: case 11: case 14: case 17:
                        param_cal_main[j] = param_token_fae;
                        break;
                    case 6 : case 12:  case 19: case 23 :
                        param_cal_main[j] = HX5_CMIC_TOKEN;
                        break;
                    default:
                        break;
                }
...
/**
@name: tdm_hx5_parse_acc_mmu
@param:

Parse ACCESSORY slots (LOOPBACK, OPP1, OPP2, NULL, IDLE)
**/
int
tdm_hx5_parse_acc_mmu(tdm_mod_t *_tdm, int cal_id)
{
    int cnt = 0, j;
    ...
    switch (param_cal_id) {
        case 4: param_token_lpbk = HX5_LPB0_TOKEN; break;
/*        case 5: param_token_lpbk = HX5_LPB1_TOKEN; break; */
        default: return FAIL; break;
    }
    ...
    if (param_cal_main != NULL && param_cal_len > 0) {
        for (j=0; j<param_cal_len; j++) {
            if (param_cal_main[j] == param_token_ancl) {
                cnt++;
                switch (cnt) {
                    case 1: case 4: case 8:  case 12: case 16 : case 18 : case 20 : case 24:
                        param_cal_main[j] = param_token_lpbk;
                        break;
                    case 3: case 9: case 21: case 15 :
                        param_cal_main[j] = HX5_CMIC_TOKEN;
                        break;
                    case 2: case 6:  case 10 : case 14 : case 11 : case 22:
                        param_cal_main[j] = HX5_NULL_TOKEN;
                        break;
                    case 5: case 7 : case 13 : case 17 : case 23: case 19 :
                        param_cal_main[j] = HX5_IDL2_TOKEN;
                        break;
                    default:
                        break;
...
/**
@name: tdm_hx5_parse_ancl
@param:

Parse ANCILLARY slots (MGMT/CMIC, LOOPBACK, OPP1, OPP2, NULL, IDLE)
**/
int
tdm_hx5_parse_ancl(tdm_mod_t *_tdm, int cal_id)
{
    ...
    /* MGMT/CMIC */
 /*   if (tdm_hx5_parse_mgmt(_tdm, cal_id) != PASS) { */
 ...
 /*   } */
    ...
    if (idb_en == 1) {
        if (tdm_hx5_parse_acc_idb(_tdm, cal_id) != PASS) {
            TDM_ERROR1("Failed in ANCL allocation, cal_id %0d\n", cal_id);
            result = FAIL;
        }
    } else if (mmu_en == 1) {
        if (tdm_hx5_parse_acc_mmu(_tdm, cal_id) != PASS) {
            TDM_ERROR1("Failed in ANCL allocation, cal_id %0d\n", cal_id);
            result = FAIL;
        }
    }
...
/**
@name: tdm_hx5_parse_tdm_tbl
@param:

Post-process TDM calendar tokenized entries
**/
int
tdm_hx5_parse_tdm_tbl(tdm_mod_t *_tdm)
{
    int j, result = PASS;
    ...
    if (tdm_hx5_parse_ancl(_tdm, param_cal_id_idb) != PASS) {
        result = FAIL;
    }
                TDM_PRINT1("done ancy %d \n",result);
    tdm_hx5_print_lr_cal(_tdm, param_cal_id_idb);

    /* parse ancillary slots in MMU TDM calendar */
    if (tdm_hx5_parse_ancl(_tdm, param_cal_id_mmu) != PASS) {
        result = FAIL;
    }
                TDM_PRINT1("done ancy %d \n",result);
    tdm_hx5_print_lr_cal(_tdm, param_cal_id_mmu);
                TDM_PRINT0("done print lr \n");

    /* construct Oversub Half Pipes */
    if (tdm_hx5_vbs_scheduler_ovs(_tdm) != PASS) {
        result = FAIL;
    }

                TDM_PRINT1("done half pipe %d \n",result);
    /* print: OVSB_HALF_PIPE, Packet_Scheduler, PORT_BLOCK_Map */
    tdm_hx5_print_pipe_cal(_tdm);
