...
#if defined(BCM_UTT_SUPPORT) && defined(BCM_TOMAHAWK_SUPPORT)
...
uint32 soc_td3_bsk_hs_fifo_bmask[SOC_MAX_NUM_DEVICES][2]={{0}};
...
STATIC int _soc_td3_ser_single_bit_for_bus_buf_disable(int unit);
STATIC int _soc_td3_dlb_correction_type_remap(int unit, soc_mem_t mem, int index);
...
int soc_trident3_ser_enable_special_table(int unit) {
    ...
    for (i = 0; i < COUNTOF(special_mem)-1; i++) {
        if (SOC_MEM_IS_VALID(unit, special_mem[i])) {
            SOC_IF_ERROR_RETURN(soc_td3_mem_parity_control(unit, special_mem[i], 0,
                        1, 0));
        }
    }
    return SOC_E_NONE;
...
soc_trident3_ser_enable_all(int unit, int enable)
{
    ...
    /* Change the default HW reset value */
    (void)_soc_td3_ser_single_bit_for_bus_buf_disable(unit);
 ...
 */
    switch (mem) {
    /* 
     * These two tables have HW update fields, but the response is ECC_CORRECTABLE
     * not WRITE_CACHE_RESTORE.
     */
    case SFLOW_ING_FLEX_DATA_SOURCEm:
    case SFLOW_ING_DATA_SOURCEm:
    case L2_ENTRY_ECCm:
        rv = 1;
        break;
    default:
        ...
        if (final_disable_parity) {
            ...
            if ( SOC_IS_HURRICANE4(unit) &&
                ...
                SOC_IF_ERROR_RETURN(soc_td3_parity_bit_enable(unit, parity_enable_reg,
                            parity_enable_mem,
                            parity_enable_field,
                            parity_enable_field_position, (block_type != SOC_BLK_MACSEC) ? 0 : 1));
        ...
        if (final_disable_parity) {
            ...
            if ( SOC_IS_HURRICANE4(unit) &&
                ...
                SOC_IF_ERROR_RETURN(soc_td3_parity_bit_enable(unit, parity_enable_reg,
                            parity_enable_mem,
                            parity_enable_field,
                            parity_enable_field_position, (block_type != SOC_BLK_MACSEC) ? 1 : 0));
            } else
#endif
            {
                SOC_IF_ERROR_RETURN(soc_td3_parity_bit_enable(unit, parity_enable_reg,
...
STATIC int
_soc_td3_ser_reg_get(int unit, soc_reg_t reg, int port, int index,
                     uint64 *data, int mmu_base_index)
{
    ...
    switch (blocktype) {
    case SOC_BLK_MMU_XPE:
       return soc_trident3_xpe_reg_get(unit, reg, block_num,
                                       base_type_index, index, data);
    case SOC_BLK_MMU_SC:
       return soc_trident3_sc_reg_get(unit, reg, block_num,
                                      base_type_index, index, data);
   case SOC_BLK_MMU_SED:
      return soc_trident3_sed_reg_get(unit, reg, block_num,
                                      base_type_index, index, data);
    default:
...
STATIC int
_soc_td3_ser_reg_set(int unit, soc_reg_t reg, int port, int index,
                     uint64 data, int mmu_base_index)
{
    int block_num = mmu_base_index;
    int base_type_index = mmu_base_index;
    ...
    switch (blocktype) {
    case SOC_BLK_MMU_XPE:
       return soc_trident3_xpe_reg_set(unit, reg, block_num,
                                       base_type_index, index, data);
    case SOC_BLK_MMU_SC:
       return soc_trident3_sc_reg_set(unit, reg, block_num,
                                      base_type_index, index, data);
   case SOC_BLK_MMU_SED:
      return soc_trident3_sed_reg_set(unit, reg, block_num,
                                      base_type_index, index, data);
    default:
                               ...
                               int index, int mmu_base_index)
{
    ...
    if (SOC_REG_IS_64(unit, reg)) {
        SOC_IF_ERROR_RETURN
            (_soc_td3_ser_reg_get(unit, reg, port, index, &rval64, mmu_base_index));
        soc_reg64_field32_set(unit, reg, &rval64, field, value);
        SOC_IF_ERROR_RETURN
            (_soc_td3_ser_reg_set(unit, reg, port, index, rval64, mmu_base_index));
    } else {
        SOC_IF_ERROR_RETURN
...
STATIC void
_soc_td3_check_flex_counter(soc_mem_t mem, int index, int *disable)
{
    switch(mem) {
        case ING_FLEX_CTR_COUNTER_TABLE_0m:
        ...
        case EGR_FLEX_CTR_COUNTER_TABLE_0m:
...
STATIC void
_soc_utt_reverse_xlate(int unit, soc_mem_t mem, soc_field_t field, int *index)
{
    uint32 entry[SOC_MAX_MEM_WORDS] = {0}, addr_msb, addr_lsb;
    int i = 0;
    for (i = 0; i <= soc_mem_index_max(unit, mem); i++) {
        soc_mem_read(unit, mem, MEM_BLOCK_ANY, i, entry);
        addr_msb = *index >> (18 - soc_mem_field_length(unit, mem, field));
        addr_lsb = *index & ~(~0 << (18 - soc_mem_field_length(unit, mem, field)));
        if (addr_msb == soc_mem_field32_get(unit, mem, entry, field)) {
            addr_msb = i << (18 - soc_mem_field_length(unit, mem, field));
            *index = addr_msb | addr_lsb;
            break;
        }
    }
}
...
STATIC int
_soc_trident3_tcam_only_remap(int unit, soc_mem_t *mem, int *index, uint32 type)
{
    if (!SOC_MEM_IS_VALID(unit, *mem)) {
        return SOC_E_PARAM;
    }
        ...
        switch (*mem) {
            ...
            case L3_DEFIP_DATA_ONLYm:
                ...
                if (SOC_IS_FIREBOLT6(unit) &&
                    *mem == L3_DEFIP_DATA_ONLYm && type != 3) {
                    _soc_utt_reverse_xlate(unit, L3_DEFIP_DATA_UTT_ADDR_XLATEm,
                                           XLATE_ADDRf, index);
                }
#endif
...
#ifdef BCM_FIREBOLT6_SUPPORT
                if (SOC_IS_FIREBOLT6(unit) &&
                    *mem == L3_DEFIP_PAIR_128_DATA_ONLYm && type != 3) {
                    _soc_utt_reverse_xlate(unit, L3_DEFIP_PAIR_128_DATA_UTT_ADDR_XLATEm,
                                           XLATE_ADDRf, index);
                }
#endif
            ...
            case IFP_POLICY_TABLE_WIDEm:
                if (SOC_IS_FIREBOLT6(unit)) {
                    *index *= 2;
                    _soc_utt_reverse_xlate(unit, IFP_POLICY_UTT_ADDR_XLATEm,
                                           SBUS_TABLE_INDEX_MSBf, index);
                    *index /= 2;
                }
                break;
            case IFP_POLICY_TABLEm:
                              ...
                              char *prefix_str, int l2_mgmt_ser_fifo)
{
    ...
    _soc_ser_correct_info_t spci_nxt_idx;
#endif
    ...
    case SOC_BLK_EPIPE:
        if (l2_mgmt_ser_fifo) {
            ...
            mask = soc_td3_bsk_hs_fifo_bmask[unit][pipe];
            l2_mgmt_ser_fifo = 0;
        } else {
            mem = epsf[pipe];
    ...
    do {
        ...
        if (valid) {
            ...
            if (SOC_IS_HURRICANE4(unit) && (type == 3) &&
                (EGR_SEQUENCE_NUMBER_TABLEm == soc_addr_to_mem_extended(unit, sblk,
                                                                        -1, address))) {
                LOG_WARN(BSL_LS_SOC_SER,
                          (BSL_META_U(unit,
                                      "SBUS transaction caused ECC error in EGR_SEQUENCE_NUMBER_TABLE."
                                      "Ignoring SER recovery.\n")));
                return SOC_E_NONE;
            }
#endif
            switch (ecc_parity) {
                ...
                if (non_sbus == 0) {
                    ...
                    if (l2_mgmt_ser_fifo) {
                        ...
                        spci.mem = soc_addr_to_mem_extended(unit, spci.sblk,
                                                            spci.acc_type,
                                                            spci.addr);
                        rv = _soc_trident3_tcam_only_remap(unit, &spci.mem, &spci.index, type);
                        if (rv < 0) {
                            LOG_ERROR(BSL_LS_SOC_SER,
                                (BSL_META_U(unit,
                                            "mem %s is invalid !!\n"),
                    ...
                    if (!SOC_IS_HURRICANE4(unit)) {
                        _soc_td3_dlb_correction_type_remap(unit, spci.mem, spci.index);
                    }
                    if (spci.mem != INVALIDm) {
                        spci.flags |= SOC_SER_REG_MEM_KNOWN;
                        ...
                        if (!spci.double_bit) {
                            ...
                            _soc_td3_check_flex_counter(spci.mem, spci.index, &disable_ser_correction);
                        }
 ...
 */
                    ...
                    if (!disable_ser_correction && !l2_mgmt_ser_fifo) {
                        ...
                        if ((soc_feature(unit,
                           ...
                           ((spci.mem == BSK_FTFP_TCAMm) ||
                            (spci.mem == BSK_FTFP_COMBO_TCAM_POLICYm)) &&
                           ((spci.index % 2) == 0)) {
                            sal_memcpy(&spci_nxt_idx, &spci, sizeof(spci_nxt_idx));
                            rv = soc_ser_correction(unit, &spci);
                            if (SOC_SUCCESS(rv)) {
                                spci_nxt_idx.index += 1;
                                rv = soc_ser_correction(unit, &spci_nxt_idx);
                            }
                        } else
...
#ifdef BCM_FIREBOLT6_SUPPORT
static _soc_td3_tcam_wrapper_info_t _soc_fb6_tcam_wrapper_table[] = {
    { L3_TUNNELm, IVXLT1_SER_CONTROLr, L3_TUNNEL_ONLY_PARITY_ENf, TCAM_PARITY_MASKf},
    { ING_SNATm, RSEL2_CAM_DBGCTRLr, ING_SNAT_ONLY_PARITY_ENf, TCAM_PARITY_MASKf},
    { VLAN_SUBNETm, IVXLT2_SER_CONTROLr, VLAN_SUBNET_ONLY_PARITY_ENf, TCAM_PARITY_MASKf},
    { MY_STATION_TCAMm, IVP_MY_STATION_CAM_CONTROLr, PARITY_ENf, TCAM_PARITY_MASKf},
    ...
    { IP_MULTICAST_TCAMm, IFWD1_SER_CONTROLr, IP_MULTICAST_TCAM_PARITY_ENf, TCAM_PARITY_MASKf},
    { CPU_COS_MAPm, CPU_COS_CAM_DBGCTRLr, PARITY_ENf, TCAM_PARITY_MASKf},
    { L3_DEFIPm, UTT_TCAM_CAM_CONTROLr, PARITY_ENf, TCAM_PARITY_MASKf},
    { L3_DEFIP_PAIR_128m, UTT_TCAM_CAM_CONTROLr, PARITY_ENf, TCAM_PARITY_MASK_UPRf},
    { IFP_TCAMm, UTT_TCAM_CAM_CONTROLr, PARITY_ENf, TCAM_PARITY_MASKf},
    { IFP_TCAM_WIDEm, UTT_TCAM_CAM_CONTROLr, PARITY_ENf, TCAM_PARITY_MASKf},
    { PHB_SELECT_TCAMm, IRSEL1_SER_CONTROLr, PHB_SELECT_TCAM_PARITY_ENf, TCAM_PARITY_MASKf},
    { FLEX_RTAG7_HASH_TCAMm, IFWD2_SER_CONTROLr, FLEX_RTAG7_HASH_TCAM_PARITY_ENf, TCAM_PARITY_MASKf},
    { PKT_FLOW_SELECT_TCAM_0m, ICFG_SER_CONTROLr, PKT_FLOW_SELECT_TCAM_0_PARITY_ENf, TCAM_PARITY_MASKf},
    { PKT_FLOW_SELECT_TCAM_1m, IPARS1_SER_CONTROLr, PKT_FLOW_SELECT_TCAM_1_PARITY_ENf, TCAM_PARITY_MASKf},
    ...
    { IP_PARSER2_HME_STAGE_TCAM_ONLY_1m, IP_PARSER2_HME_STAGE_TCAM_SER_CONTROL_1r, SER_ENf, TCAM_PARITY_MASK0f},
    { IP_PARSER2_HME_STAGE_TCAM_NARROW_ONLY_2m, IP_PARSER2_HME_STAGE_TCAM_SER_CONTROL_2r, SER_ENf, TCAM_PARITY_MASKf},
    { IP_PARSER2_HME_STAGE_TCAM_ONLY_3m, IP_PARSER2_HME_STAGE_TCAM_SER_CONTROL_3r, SER_ENf, TCAM_PARITY_MASK0f},
    { IP_PARSER2_HME_STAGE_TCAM_ONLY_4m, IP_PARSER2_HME_STAGE_TCAM_SER_CONTROL_4r, SER_ENf, TCAM_PARITY_MASK0f},
    { IP_PARSER2_MICE_TCAM_0m, IP_PARSER2_MICE_TCAM_SER_CONTROL_0r, SER_ENf, TCAM_PARITY_MASKf},
    { IP_PARSER2_MICE_TCAM_1m, IP_PARSER2_MICE_TCAM_SER_CONTROL_1r, SER_ENf, TCAM_PARITY_MASKf},
    { EGR_PKT_FLOW_SELECT_TCAMm, EGR_EPFS_TCAM_SER_CONTROLr, EGR_PKT_FLOW_SELECT_TCAM_PARITY_ENf, TCAM_PARITY_MASKf},
    ...
    { BSK_FTFP_TCAMm, BSK_FTFP_CAM_CONTROLr, PARITY_ENf, TCAM_PARITY_MASKf},
    { BSK_FTFP_LTS_LOGICAL_TBL_SEL_TCAMm, BSK_FTFP_LTS_A_KEY_GEN_SER_CONTROLr, LOGICAL_TBL_SEL_TCAM_PARITY_ENf, TCAM_PARITY_MASKf},
    { BSK_AEFP_TCAMm, BSK_AEFP_CAM_CONTROLr, PARITY_ENf, TCAM_PARITY_MASKf},
    { BSK_AMFP_TCAMm, BSK_AMFP_CAM_CONTROLr, PARITY_ENf, TCAM_PARITY_MASKf},
    { BSK_FTFP2_TCAMm, BSK_FTFP2_CAM_CONTROLr, PARITY_ENf, TCAM_PARITY_MASKf},
    { BSK_FTFP2_LTS_LOGICAL_TBL_SEL_TCAMm, BSK_FTFP2_LTS_A_KEY_GEN_SER_CONTROLr, LOGICAL_TBL_SEL_TCAM_PARITY_ENf, TCAM_PARITY_MASKf},
    { INVALIDm},
};
#endif

#ifdef BCM_HURRICANE4_SUPPORT
static _soc_td3_tcam_wrapper_info_t _soc_hr4_tcam_wrapper_table[] = {
    { L3_TUNNELm, IVXLT1_SER_CONTROLr, L3_TUNNEL_ONLY_PARITY_ENf, TCAM_PARITY_MASKf},
    { VLAN_SUBNETm, IVXLT2_SER_CONTROLr, VLAN_SUBNET_ONLY_PARITY_ENf, TCAM_PARITY_MASKf},
    { MY_STATION_TCAMm, IVP_MY_STATION_CAM_CONTROLr, PARITY_ENf, TCAM_PARITY_MASKf},
    { MY_STATION_TCAM_2m, IFWD1_SER_CONTROLr, MY_STATION_TCAM_2_PARITY_ENf, TCAM_PARITY_MASKf},
...
soc_trident3_l3_memwr_parity_check(int unit, soc_mem_t mem, int disable)
{
    ...
    if (!SOC_MEM_FIELD_VALID(unit, disable_mem, disable_field)) {
        return SOC_E_NONE;
    }
...
{
    int slice, slice_skip = 0, slice_size, slice_count;
    ...
    case IFP_TCAM_PIPE0m:
    case IFP_TCAM_PIPE1m:
        slice_size = SOC_TD3_FP_SLICE_ENTRY_NARROW_VIEW(unit);
        break;
    case IFP_TCAM_WIDEm:
    case IFP_TCAM_WIDE_PIPE0m:
    case IFP_TCAM_WIDE_PIPE1m:
        slice_size = SOC_TD3_FP_SLICE_ENTRY_WIDE_VIEW(unit);
        break;
    default:
        return SOC_E_PARAM;
    }
    ...
    if (soc_feature(unit, soc_feature_utt)) {
        slice_count = _FP_MAX_LT_PARTS(unit);
        if (soc_feature(unit, soc_feature_utt_ver1_5)) {
            _FP_XGS3_ALLOC(slice_depth, slice_count * sizeof(int), "array of slice depth.");
            if (NULL == slice_depth) {
                LOG_VERBOSE(BSL_LS_SOC_SER, (BSL_META(
                                "slice_depth memory resource error\n")));
                return (BCM_E_MEMORY);
            }
            BCM_IF_ERROR_CLEANUP(
                soc_utt_ifp_lookup_depth_get(unit, slice_count, slice_depth));
        }
    } else
#endif
    {
    ...
    LOG_VERBOSE(BSL_LS_SOC_SER,
            (BSL_META_U(unit,
                        "slice_size %d slice_count %d mem_size %d for mem %s\n"),
             slice_size, slice_count, soc_mem_index_count(unit, mem),
             SOC_MEM_NAME(unit, mem)));
    ...
    for (slice = 0; slice < slice_count; slice++) {
        ...
        if (soc_feature(unit, soc_feature_utt_ver1_5) &&
            ...
            switch (mem) {
                ...
                case IFP_TCAM_PIPE0m:
                case IFP_TCAM_PIPE1m:
                    slice_size = 2 * slice_depth[slice];
                    break;
                case IFP_TCAM_WIDEm:
                case IFP_TCAM_WIDE_PIPE0m:
                case IFP_TCAM_WIDE_PIPE1m:
                    slice_size = slice_depth[slice];
                    break;
            }
        }
#endif
...
soc_td3_check_scrub_skip(int unit, soc_mem_t mem, int check_hw_global_mode)
{
    ...
    */
    ...
    /* specials which need not be scrubbed */
        ...
        /* Skip narrow view since wide view is used for these mems
         * making narrow view not accessible
         */
    ...
    /* global views which already have _PIPE0-PIPE3 views */
        ...
        case IP_PARSER2_HME_STAGE_TCAM_NARROW_ONLY_3m:
            return SOC_IS_FIREBOLT6(unit) ? 1 : 0;
#endif
        ...
        if (SOC_REG_IS_64(unit, reg)) {
            ...
            if (20 == at) { /* ACC_TYPE_SINGLE - used only in MMU blocks */
                ...
                for (pipe = 0; pipe < max_pipe; pipe++) {
                    ...
                    LOG_VERBOSE(BSL_LS_SOC_SER, (BSL_META_U(unit,
                        "HF detected: setting reg.field[%s.%s]=%d, mmu_base_type %s, pipe %d, using acc_type %0d\n"),
                        SOC_REG_NAME(unit, reg), SOC_FIELD_NAME(unit, field), enable,
                ...
                SOC_IF_ERROR_RETURN
                    (soc_reg32_set(unit, reg, port, 0, rval));
                LOG_VERBOSE(BSL_LS_SOC_SER, (BSL_META_U(unit,
                    "HF detected: setting reg.field[%s.%s]=%d, using acc_type %0d (default)\n"),
                    SOC_REG_NAME(unit, reg), SOC_FIELD_NAME(unit, field), enable,
                    at));
            }
        } /* reg32 */
                              ...
                              int en, int en_1b)
{
    ...
    if (SOC_IS_HURRICANE4(unit) && soc_feature(unit, soc_feature_xflow_macsec)) {
        rv = soc_td3_mem_parity_set(unit, mem, copyno,
                                    SOC_MACSEC_MEM_SER_INFO(unit),
                                    en ? 0 : 1,
                                    en_1b ? 0 : 1);
        if (rv == SOC_E_NONE) {
            return rv;
...
int _soc_td3_ser_single_bit_for_bus_buf_disable(int unit)
{
    soc_reg_t reg = INVALIDr;
    uint32 rval = 0;
    uint64 rval64;
    /* We disable the single bit error report for bus buffer by default,
       but for some registers the HW reset value is 1, so need to disable
       the report manually.
     */

    reg = IDB_SER_CONTROLr;
    SOC_IF_ERROR_RETURN
        (soc_reg32_get(unit, reg, REG_PORT_ANY, 0, &rval));
    soc_reg_field_set(unit, reg, &rval, IS_TDM_ERR1_RPT_ENf, 0);
    SOC_IF_ERROR_RETURN
        (soc_reg32_set(unit, reg, REG_PORT_ANY, 0, rval));

    reg = EGR_EVLAN_EN_COR_ERR_RPTr;
    SOC_IF_ERROR_RETURN
        (soc_reg_get(unit, reg, REG_PORT_ANY, 0, &rval64));
    soc_reg64_field32_set(unit, reg, &rval64, PT_HWYf, 0);
    soc_reg64_field32_set(unit, reg, &rval64, HWY2f, 0);
    soc_reg64_field32_set(unit, reg, &rval64, HWY1f, 0);
    SOC_IF_ERROR_RETURN
        (soc_reg_set(unit, reg, REG_PORT_ANY, 0, rval64));

    reg = EGR_EL3_EN_COR_ERR_RPTr;
    SOC_IF_ERROR_RETURN
        (soc_reg_get(unit, reg, REG_PORT_ANY, 0, &rval64));
    soc_reg64_field32_set(unit, reg, &rval64, HWY2f, 0);
    soc_reg64_field32_set(unit, reg, &rval64, HWY1f, 0);
...
int soc_td3_ser_single_bit_report_for_bus_buf_set(int unit, int enable)
{
    int i, value, k;
    soc_reg_t ecc_reg;
    soc_field_t ecc_fld;
    ...
    _soc_buffer_ser_en_info_t *buf_ser_info_all[BUS_BUF_SER_INFO_COUNT] = {NULL};

    value = enable ? 1:0;

    bus_ser_info_all[0] = SOC_IP_BUS_SER_INFO(unit);
    bus_ser_info_all[1] = SOC_EP_BUS_SER_INFO(unit);
    buf_ser_info_all[0] = SOC_IP_BUF_SER_INFO(unit);
    buf_ser_info_all[1] = SOC_EP_BUF_SER_INFO(unit);

    if ((NULL == bus_ser_info_all[0]) ||
        (NULL == bus_ser_info_all[1]) ||
        (NULL == buf_ser_info_all[0]) ||
        (NULL == buf_ser_info_all[1])) {
...
#ifdef BCM_FIREBOLT6_SUPPORT
            if (_soc_fb6_reg_skip(unit, ecc_reg)) {
                continue;
            }
#endif
            LOG_VERBOSE(BSL_LS_SOC_SER,
...
STATIC int
_soc_td3_dlb_correction_type_remap(int unit, soc_mem_t mem, int index)
{
    int idx_min = 0, idx_max = 0, i = 0;
    int dlb_mode, entry_base, entry_size;
    int flowset_idx_min, flowset_idx_max;
    dlb_ecmp_group_control_entry_t dlb_ecmp_group_control;
    int entries[9] = {0, 256, 512, 1024, 2048, 4096, 8192, 16384, 32768};

    if (mem != DLB_ECMP_FLOWSET_MEMBERm) {
        return SOC_E_NONE;
    }

    idx_min = soc_mem_index_min(unit, DLB_ECMP_GROUP_CONTROLm);
    idx_max = soc_mem_index_max(unit, DLB_ECMP_GROUP_CONTROLm);

    sal_memset(&dlb_ecmp_group_control, 0, sizeof(dlb_ecmp_group_control_entry_t));

    for (i = idx_min; i <= idx_max; i++) {
        SOC_IF_ERROR_RETURN(READ_DLB_ECMP_GROUP_CONTROLm(unit, MEM_BLOCK_ANY,
                            i, &dlb_ecmp_group_control));
        entry_base = soc_DLB_ECMP_GROUP_CONTROLm_field32_get(unit,
                                                    &dlb_ecmp_group_control,
                                                    FLOW_SET_BASEf);
        entry_size = soc_DLB_ECMP_GROUP_CONTROLm_field32_get(unit,
                                                    &dlb_ecmp_group_control,
                                                    FLOW_SET_SIZEf);
        flowset_idx_min = entry_base;
        flowset_idx_max = entry_base + entries[entry_size] - 1;
        if ((index >= flowset_idx_min) && (index <= flowset_idx_max)) {
            break;
        }
    }

    if (i > idx_max) {
        /* Not find the DLB group, use the default correction type*/
        return SOC_E_NONE;
    }
    ...
    /* For DLB_ECMP_FLOWSET_MEMBER, if the DLB mode is TIME_ELIGIBILITY and
           PACKET_SPRAY, this table is updated by HW, the correction type is entry_clear,
           if the DLB mode is FIXED_ASSIGNMENT, this table is updated by SW, the correction
           type is cache_restore */
    if (dlb_mode == 0 || dlb_mode == 2) {
...
#ifdef BCM_FIREBOLT6_SUPPORT
    /* NOTEST memory */
    { MMU_REPL_GROUP_INFO_TBLX_PIPE0m, _SOC_ACC_TYPE_PIPE_ANY, 1 },
#endif
#if defined(INCLUDE_XFLOW_MACSEC) && defined(BCM_HURRICANE4_SUPPORT)
    /* Inject-only memory */
    { ISEC_SP_TCAMm , _SOC_ACC_TYPE_PIPE_ANY, 1 },
    { ISEC_SC_TCAMm , _SOC_ACC_TYPE_PIPE_ANY, 1 },
#endif
...
#if defined(BCM_FIREBOLT6_SUPPORT)
const ser_td3_skipped_mem_t fb6_skipped_mems[] = {
    { EFP_METER_TABLEm, _SOC_ACC_TYPE_PIPE_ANY, 0 },
    { EFP_METER_TABLE_PIPE0m, _SOC_ACC_TYPE_PIPE_ANY, 0 },
...
#ifdef BCM_HURRICANE4_SUPPORT
    if (SOC_IS_HURRICANE4(unit)) {
        tcams = _soc_hr4_tcam_wrapper_table;
    }
#endif /* BCM_HURRICANE4_SUPPORT */
#ifdef BCM_FIREBOLT6_SUPPORT
    if (SOC_IS_FIREBOLT6(unit)) {
        tcams = _soc_fb6_tcam_wrapper_table;

        /* Check if memory is listed in skipped mems structure */
        for (i = 0; fb6_skipped_mems[i].mem != INVALIDm; i++) {
            if ((fb6_skipped_mems[i].mem == mem) &&
                ((_SOC_ACC_TYPE_PIPE_ANY == fb6_skipped_mems[i].acc_type) ||
                (fb6_skipped_mems[i].acc_type == soc_acc_type_target))) {
                if (fb6_skipped_mems[i].allow_error_inj) {
                    return SOC_E_NONE;
                } else {
                    return rv;
                }
...
#if defined(INCLUDE_XFLOW_MACSEC) && defined(BCM_HURRICANE4_SUPPORT)
   if (SOC_IS_HURRICANE4(unit) && soc_feature(unit, soc_feature_xflow_macsec)) {
       rv = _ser_td3_ser_support_mem_found(unit, mem, SOC_MACSEC_MEM_SER_INFO(unit));
       if(rv == SOC_E_NONE) {
           return rv;
       }
   }
                               ...
                               int *mem_skipped, int *mem_failed)
{
    ...
    if (SOC_IS_FIREBOLT6(unit)) {
        /* Check if memory is listed in skipped mems structure */
        for (k = 0; fb6_skipped_mems[k].mem != INVALIDm; k++) {
            if ((fb6_skipped_mems[k].mem == test_data->mem_fv) &&
                ((_SOC_ACC_TYPE_PIPE_ANY == fb6_skipped_mems[k].acc_type) ||
                (fb6_skipped_mems[k].acc_type == test_data->acc_type))) {
...
#if defined (BCM_HURRICANE4_SUPPORT) && (INCLUDE_XFLOW_MACSEC)
                              ...
                              int inject_only, int cmd, uint32 flags)
{
    ...
    if (SOC_IS_HURRICANE4(unit)) {
        tcams = _soc_hr4_tcam_wrapper_table;
        if (_soc_hr4_mem_skip(unit, mem)) {
            LOG_ERROR(BSL_LS_SOC_SER,
                      (BSL_META_U(unit,
                                  "unit %d, mem %s is not valid "
                                  "or parity is disabled for this mem !!\n"),
                       unit, SOC_MEM_NAME(unit,mem)));
            return SOC_E_UNAVAIL;
        }
    ...
    if (SOC_IS_FIREBOLT6(unit)) {
        ...
        for (i = 0; fb6_skipped_mems[i].mem != INVALIDm; i++) {
            if ((fb6_skipped_mems[i].mem == mem) &&
                ...
                if (inject_only && fb6_skipped_mems[i].allow_error_inj) {
                    break;
                }
            ...
            if (rv == SOC_E_NONE) {
                ...
                if (rv == SOC_E_NONE) {
                    ...
                    if (SOC_IS_HURRICANE4(unit) &&
                        (soc_feature(unit, soc_feature_xflow_macsec))) {
                        rv = soc_td3_test_mem_create(unit, mem, block, index, &flags, _SOC_ACC_TYPE_PIPE_ANY,
                                tmp_entry, field_data, &test_data, SOC_MACSEC_MEM_SER_INFO(unit));
                        if (rv == SOC_E_NONE) {
                            found_mem = TRUE;
                        }
                        if ((ISEC_SP_TCAMm == mem) ||
                            (ISEC_SC_TCAMm == mem)) {
                            soc_ser_create_test_data_with_new_format(unit,
                                    tmp_entry, field_data,
                                    INVALIDr,
                                    INVALIDm,
                                    SOC_INVALID_TCAM_PARITY_BIT,
                                    INVALIDf,
...
soc_td3_ser_tcam_test(int unit, _soc_ser_test_t test_type)
{
    ...
    for (i = 0; tcams[i].mem != INVALIDm; i++) {
        if(!SOC_REG_FIELD_VALID(unit, tcams[i].enable_reg,
                                tcams[i].enable_field)) {
            continue;
        }
#ifdef BCM_HURRICANE4_SUPPORT
        if (SOC_IS_HURRICANE4(unit) &&
            _soc_hr4_mem_skip(unit, tcams[i].mem)) {
            continue;
        }
#endif /* BCM_HURRICANE4_SUPPORT */
#ifdef BCM_FIREBOLT6_SUPPORT
        if (SOC_IS_FIREBOLT6(unit) &&
            _soc_fb6_mem_skip(unit, tcams[i].mem)) {
            continue;
        }
#endif /* BCM_FIREBOLT6_SUPPORT */
...
#ifdef BCM_HURRICANE4_SUPPORT
        if (SOC_IS_HURRICANE4(unit) &&
            ((ser_info_list[i].mem == EGR_SEQUENCE_NUMBER_TABLEm) ||
             (ser_info_list[i].mem == EGR_SEQUENCE_NUMBER_TABLE_PIPE0m) ||
             _soc_hr4_mem_skip(unit, ser_info_list[i].mem))) {
            continue;
        }
#endif /* BCM_HURRICANE4_SUPPORT */
#ifdef BCM_FIREBOLT6_SUPPORT
        if (SOC_IS_FIREBOLT6(unit) &&
                _soc_fb6_mem_skip(unit, ser_info_list[i].mem)) {
            continue;
        }
#endif /* BCM_FIREBOLT6_SUPPRT */
...
soc_td3_ser_hardware_test(int unit, _soc_ser_test_t test_type) {
   ...
   if (SOC_IS_HURRICANE4(unit) && soc_feature(unit, soc_feature_xflow_macsec)) {
        ...
        {
            rv = soc_td3_ser_block_test(unit, test_type, &mem_failed, &mem_tests, &mem_skipped,
                                        SOC_MACSEC_MEM_SER_INFO(unit), SOC_BLK_MACSEC);
        }
    }
#endif
                                  ...
                                  soc_field_t *ser_enable_field)
{
    ...
    for (i = 0; tcams[i].mem != INVALIDm; i++) {
        ...
        if (tcams[i].mem == mem ) {
            *ser_control_reg = tcams[i].enable_reg;
            *ser_enable_field = tcams[i].enable_field;
            return SOC_E_NONE;  /* found */
        }
    }
    return SOC_E_UNAVAIL;
...
soc_td3_tcam_engine_enable(int unit, int enable)
{
    ...
    if (enable) {
        if (!SOC_IS_FIREBOLT6(unit)) {
            ...
            soc_reg64_field32_set(unit, IDB_SER_SCAN_CONFIGr, &config, MAX_IDXf, 0x3fff);
        ...
        soc_reg64_field32_set(unit, EGR_ARB_SER_SCAN_CONFIGr, &config, MAX_IDXf, 0x3fff);
        ...
        if (SOC_REG_IS_VALID(unit, BSK_HS_SER_SCAN_CONFIGr)) {
            /* config BSK_HS HW ser scan engine */
            COMPILER_64_ZERO(config);
            soc_reg64_field32_set(unit, BSK_HS_SER_SCAN_CONFIGr, &config, START_IDXf, 0);
            soc_reg64_field32_set(unit, BSK_HS_SER_SCAN_CONFIGr, &config, MAX_IDXf, 0x3fff);
            soc_reg64_field32_set(unit, BSK_HS_SER_SCAN_CONFIGr, &config, OP_BLOCKf, 0);
            soc_reg64_field32_set(unit, BSK_HS_SER_SCAN_CONFIGr, &config, INTERVALf, 64);
            soc_reg64_field32_set(unit, BSK_HS_SER_SCAN_CONFIGr, &config, ENABLEf, 1);
            SOC_IF_ERROR_RETURN(soc_reg64_set
                (unit, BSK_HS_SER_SCAN_CONFIGr, REG_PORT_ANY, 0, config));
        }
    } else {
        if (!SOC_IS_FIREBOLT6(unit)) {
            soc_reg64_field32_set(unit, IDB_SER_SCAN_CONFIGr, &config, START_IDXf, 0);
        ...
        if (SOC_REG_IS_VALID(unit, BSK_HS_SER_SCAN_CONFIGr)) {
            ...
            soc_reg64_field32_set(unit, BSK_HS_SER_SCAN_CONFIGr, &config, OP_BLOCKf, 0);
            soc_reg64_field32_set(unit, BSK_HS_SER_SCAN_CONFIGr, &config, INTERVALf, 64);
            soc_reg64_field32_set(unit, BSK_HS_SER_SCAN_CONFIGr, &config, ENABLEf, 0);
            SOC_IF_ERROR_RETURN(soc_reg64_set
                (unit, BSK_HS_SER_SCAN_CONFIGr, REG_PORT_ANY, 0, config));
        }
    }
    ...
    for (i = 0; tcams[i].mem != INVALIDm; i++) {
        ...
        if(!SOC_REG_FIELD_VALID(unit, tcams[i].enable_reg,
            ...
            LOG_DEBUG(BSL_LS_SOC_SER,
                      (BSL_META_U(unit,
                                  "unit %d, mem %s enable reg %s "
                                  "enable Field %s is INVALID or not valid !!\n"),
                       unit, SOC_MEM_NAME(unit, tcams[i].mem),
                       SOC_REG_NAME(unit, tcams[i].enable_reg),
                       SOC_FIELD_NAME(unit, tcams[i].enable_field)));
            continue;
        }
