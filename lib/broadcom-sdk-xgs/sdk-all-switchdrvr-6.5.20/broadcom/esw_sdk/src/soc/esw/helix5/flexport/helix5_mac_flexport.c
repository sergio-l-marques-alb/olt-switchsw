...
int
soc_helix5_flex_mac_rx_port_down(
    ...
    for (i = 0; i < port_schedule_state->nport; i++) {
	   ...
	   }else
	   {
	    ...
	    if((phy_port < 49) && (qmode)){
            LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit, "Bringing GPORT rx port %0d down\n"), phy_port));
            SOC_IF_ERROR_RETURN(soc_reg_rawport_get(unit, COMMAND_CONFIGr, phy_port,
                                                    0, &rval64));
            soc_reg64_field32_set(unit, COMMAND_CONFIGr, &rval64, RX_ENAf, 0);
...
int
soc_helix5_flex_mac_tx_port_down(
    int unit, soc_port_schedule_state_t *port_schedule_state)
{
    int i;
    uint64 rval64;
...
int
soc_helix5_flex_mac_port_up(int                        unit,
                               soc_port_schedule_state_t *port_schedule_state)
{
	...
	LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit, "START soc_helix5_flex_mac_port_up\n")));
    /*Disable TSC lanes: */
    ...
    for (i = 0; i < port_schedule_state->nport; i++) {
        if (port_schedule_state->resource[i].physical_port != -1) {
            ...
            subp = soc_helix5_get_subp_from_phy_pnum(phy_port);
           /* clport = ((phy_port-1)>>2); */
            clport = soc_helix5_get_pm_from_phy_pnum(phy_port);
            soc_hx5_tdm_get_port_ratio(unit, port_schedule_state, clport,
                                         &mode,
                                         0);
	...
	LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit, "END soc_helix5_flex_mac_port_up\n")));
    return SOC_E_NONE;
}
