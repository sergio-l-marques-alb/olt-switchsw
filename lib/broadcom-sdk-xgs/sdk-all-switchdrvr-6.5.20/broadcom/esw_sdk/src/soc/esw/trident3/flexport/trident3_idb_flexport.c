...
/* Bubble MOP */
typedef struct soc_trident3_bmop_s {
...
static const struct soc_trident3_bmop_cfg_s {
    int speed;
    soc_trident3_bmop_t bmop_enable;                  /* Bubble MOP enable */
} soc_trident3_bmop_cfg_tbl[] = {
    {    -1, {{0, 0, 0}, {0, 0, 0}}},       /* SAF     */
...
static const struct soc_trident3_obm_setting_s {
    int discard_limit;
...
static const struct soc_trident3_fsaf_dur_timer {
    int frequency;
	/* ForceSAF detection duration timer. It is 1.5 * core clk in MHz */
    int duration_timer;             
} soc_trident3_fsaf_dur_timer_tbl[] = {
    { 1525,  2288 },
    { 1425,  2138 },
    { 1325,  1988 },
    { 1012,  2550 },
    {  850,  1275 },      /*TRIDENT3_FORCE_SAF_TIMER_ENTRY_CNT=5*/
    {   -1,  12000}
};


static const soc_reg_t soc_trident3_obm_ca_ctrl_regs[TRIDENT3_PIPES_PER_DEV][
    TRIDENT3_PBLKS_PER_PIPE] = {
...
static const soc_reg_t obm_ctrl_regs[TRIDENT3_PIPES_PER_DEV][
	TRIDENT3_PBLKS_PER_PIPE] = {
...
/*! @fn int soc_trident3_get_pipe_from_phy_pnum(int pnum)
 *  @param pnum Physical por number.
 *  @brief Helper function to calculate pipe number from physical port number.
 *  @returns pipe_number
 */
int
soc_trident3_get_pipe_from_phy_pnum(int pnum)
{
    return((pnum==TRIDENT3_PHY_PORT_LPBK0)                        ? 0 :
           (pnum==TRIDENT3_PHY_PORT_LPBK1)                        ? 1 :
           (pnum==TRIDENT3_PHY_PORT_CPU)                          ? 0 :
           (pnum==TRIDENT3_PHY_PORT_MNG0)                         ? 1 :
           (pnum>(TRIDENT3_PBLKS_PER_PIPE*TRIDENT3_PORTS_PER_PBLK*1)) ? 1 : 0);
}

/*! @fn int soc_trident3_get_pm_from_phy_pnum(int pnum)
 *  @param pnum Physical por number.
 *  @brief Helper function to calculate PM number from physical port number.
 *  @returns PM number
 */
int
soc_trident3_get_pm_from_phy_pnum(int pnum)
{
	  /* Return 0 - 15 for straight order for both pipes. */
    return (((pnum-1)&0x3f)>>2);
}

/*! @fn int soc_trident3_get_subp_from_phy_pnum(int pnum)
 *  @param pnum Physical por number.
 *  @brief Helper function to calculate subport number from physical port number.
 *  @returns subport number
 */
int
soc_trident3_get_subp_from_phy_pnum(int pnum)
{
    return ((pnum-1)&0x3);
}

/*! @fn int soc_trident3_idb_clear_stats_new_ports(int unit,
 ...
 */
int
soc_trident3_idb_clear_stats_new_ports(
    int unit, soc_port_schedule_state_t *port_schedule_state)
{
    ...
    const soc_reg_t obm_usage_regs[TRIDENT3_PIPES_PER_DEV][
	TRIDENT3_PBLKS_PER_PIPE] = {
        ...
        {IDB_OBM0_MAX_USAGE_PIPE1r,
         IDB_OBM1_MAX_USAGE_PIPE1r,
         IDB_OBM2_MAX_USAGE_PIPE1r,
         IDB_OBM3_MAX_USAGE_PIPE1r,
         IDB_OBM4_MAX_USAGE_PIPE1r,
         IDB_OBM5_MAX_USAGE_PIPE1r,
         IDB_OBM6_MAX_USAGE_PIPE1r,
         IDB_OBM7_MAX_USAGE_PIPE1r,
         IDB_OBM8_MAX_USAGE_PIPE1r,
         IDB_OBM9_MAX_USAGE_PIPE1r,
         IDB_OBM10_MAX_USAGE_PIPE1r,
         IDB_OBM11_MAX_USAGE_PIPE1r,
         IDB_OBM12_MAX_USAGE_PIPE1r,
         IDB_OBM13_MAX_USAGE_PIPE1r,
         IDB_OBM14_MAX_USAGE_PIPE1r,
         IDB_OBM15_MAX_USAGE_PIPE1r}
        };
        ...
        if (port_schedule_state->resource[prt].physical_port != -1) {
            pnum =
                port_schedule_state->out_port_map.port_l2p_mapping[
                    ...
                    ->resource[
                        prt].logical_port];
            pipe_num = soc_trident3_get_pipe_from_phy_pnum(pnum);
            pm_num = soc_trident3_get_pm_from_phy_pnum(pnum);
            subp = soc_trident3_get_subp_from_phy_pnum(pnum);
...
/*! @fn int soc_trident3_idb_reset_buffer(int unit, int pipe_num, int pm_num,
 ...
 */
int
soc_trident3_idb_obm_reset_buffer(int unit, int pipe_num, int pm_num, int subp,
                                   int reset_buffer)
{
    soc_reg_t reg;
    uint32 rval;
...
/*! @fn int soc_trident3_idb_ca_reset_buffer(int unit, int pipe_num, int pm_num,
 *              int subp, int reset_buffer)
 *  @param unit Chip unit number.
 *  @param pipe_num Trident3 pipe number.
 *  @param pm_num PM number within the pipe: Ranges from 0 to 15.
 ...
 */
int
soc_trident3_idb_ca_reset_buffer(int unit, int pipe_num, int pm_num, int subp,
                                  int reset_buffer)
{
    soc_reg_t reg;
    uint32 rval;

    reg = soc_trident3_obm_ca_ctrl_regs[pipe_num][pm_num];
...
/*! @fn int soc_trident3_idb_lpbk_ca_reset_buffer(int unit, int pipe_num, 
                      int reset_buffer)
 *  @param unit Chip unit number.
 *  @param pipe_num Trident3 pipe number.
 *  @param reset_buffer IDB Cell Assembly reset buffer: 1 - Apply reset.
 ...
 */
int
soc_trident3_idb_lpbk_ca_reset_buffer(int unit, int pipe_num, int reset_buffer)
{
    soc_reg_t reg;
    uint32 rval;
    const soc_reg_t idb_ca_lpbk_ctrl[TRIDENT3_PIPES_PER_DEV]= {
...
/*! @fn int soc_trident3_idb_lpbk_ca_in_reset(int unit, int pipe_num)
 *  @param unit Chip unit number.
 *  @param pipe_num Trident3 pipe number.
 ...
 */
int
soc_trident3_idb_lpbk_ca_in_reset(int unit, int pipe_num)
{
    soc_reg_t reg;
    uint32 rval;
    int buffer_in_reset;
...
/*! @fn int soc_trident3_idb_mgmt_ca_in_reset(int unit, int pipe_num)
 *  @param unit Chip unit number.
 *  @param pipe_num Trident3 pipe number.
 *  @brief Helper function to check if the IDB MGMT CA is in reset.
 *  @returns Reset status;
 */
int
soc_trident3_idb_mgmt_ca_in_reset(int unit, int pipe_num)
{
    soc_reg_t reg;
    ...
    if(pipe_num == 0) {
        LOG_CLI((BSL_META_U(unit,
                "___ERROR invalid pipe number 0 used in "
                "soc_trident3_idb_mgmt_ca_in_reset")));
        return SOC_E_FAIL;
    }
...
/*! @fn int soc_trident3_idb_cpu_ca_reset_buffer(int unit, int pipe_num, 
                      int reset_buffer)
 *  @param unit Chip unit number.
 *  @param pipe_num Trident3 pipe number.
 *  @param reset_buffer IDB Cell Assembly reset buffer: 1 - Apply reset.
 ...
 */
int
soc_trident3_idb_cpu_ca_reset_buffer(int unit, int pipe_num, int reset_buffer)
{
    soc_reg_t reg;
    uint32 rval;
    const soc_reg_t idb_ca_lpbk_ctrl[TRIDENT3_PIPES_PER_DEV]= {
...
/*! @fn int soc_trident3_idb_cpu_ca_in_reset(int unit, int pipe_num)
 *  @param unit Chip unit number.
 *  @param pipe_num Trident3 pipe number.
 ...
 */
int
soc_trident3_idb_cpu_ca_in_reset(int unit, int pipe_num)
{
    soc_reg_t reg;
    uint32 rval;
    int buffer_in_reset;
    static const soc_reg_t idb_ca_cpu_ctrl[TRIDENT3_PIPES_PER_DEV]= {
         IDB_CA_CPU_CONTROL_PIPE0r,
         IDB_CA_CPU_CONTROL_PIPE1r};
...
/*! @fn int soc_trident3_idb_mgmt_ca_reset_buffer(int unit, int pipe_num, 
                      int reset_buffer)
 *  @param unit Chip unit number.
 *  @param pipe_num Trident3 pipe number.
 *  @param reset_buffer IDB Cell Assembly reset buffer: 1 - Apply reset.
 ...
 */
int
soc_trident3_idb_mgmt_ca_reset_buffer(int unit, int pipe_num, int reset_buffer)
{
    soc_reg_t reg;
    uint32 rval;
    const soc_reg_t idb_ca_lpbk_ctrl[TRIDENT3_PIPES_PER_DEV]= {
    ...
    if(pipe_num == 0) {
        LOG_CLI((BSL_META_U(unit, 
                "___ERROR invalid pipe number 0 used in "
                "soc_trident3_idb_mgmt_ca_reset_buffer")));
        return SOC_E_FAIL;
    }
...
/*! @fn int soc_trident3_idb_port_mode_set(int unit,
 *              soc_port_schedule_state_t *port_schedule_state)
 *  @param unit Chip unit number.
 *  @param port_schedule_state Pointer to a soc_port_schedule_state_t struct
 *         variable
 ...
 */
int
soc_trident3_idb_port_mode_set(int                        unit,
                                soc_port_schedule_state_t *port_schedule_state)
{
    soc_reg_t reg;
    int i;
    ...
    int mode;
    int clport;
    static const int soc_trident3_hw_mode_values[SOC_TD3_PORT_RATIO_COUNT] = {
        0, 1, 1, 1, 3, 5, 4, 6, 2
    };
    ...
    for (i = 0; i < port_schedule_state->nport; i++) {
        if (port_schedule_state->resource[i].physical_port != -1) {
            ...
            soc_reg_field_set(unit, reg, &rval, PORT_MODEf,
                              soc_trident3_hw_mode_values[mode]);
            SOC_IF_ERROR_RETURN(soc_reg32_rawport_set(unit, reg, REG_PORT_ANY,
                                                      0, rval));
        }
    }
...
/*! @fn int soc_trident3_idb_wr_obm_shared_config(int unit, int pipe_num,
 ...
 */
int
soc_trident3_idb_wr_obm_shared_config(int unit, int pipe_num, int pm_num,
                                       int subp,
                                       int num_lanes, int lossless)
{
    ...
    const soc_reg_t obm_shared_cfg[TRIDENT3_PIPES_PER_DEV][
		TRIDENT3_PBLKS_PER_PIPE] = {
    ...
    if (lossless) {
        sop_discard_mode =
            soc_trident3_obm_settings[TRIDENT3_OBM_PARAMS_LOSSLESS_INDEX]
            [num_lanes].sop_discard_mode;
    } else {
        sop_discard_mode =
            soc_trident3_obm_settings[TRIDENT3_OBM_PARAMS_LOSSY_INDEX]
            [num_lanes].sop_discard_mode;
    }
...
/*! @fn int soc_trident3_idb_wr_obm_flow_ctrl_cfg(int unit, int pipe_num,
 *              int pm_num, int subp, int num_lanes)
 *  @param unit Chip unit number.
 *  @param pipe_num Pipe number.
 *  @param pm_num Port Macro number.
 ...
 */
int
soc_trident3_idb_wr_obm_flow_ctrl_cfg(int unit, int pipe_num, int pm_num,
                                       int subp, int lossless)
{
    soc_reg_t reg;
    uint64 rval64;

    const soc_reg_t obm_flow_ctrl_cfg[TRIDENT3_PIPES_PER_DEV][
			TRIDENT3_PBLKS_PER_PIPE] = {
...
/*! @fn int soc_trident3_idb_wr_obm_port_ca_sop(int unit, int pipe_num, int pm_num,
 *              int subp)
 *  @param unit Chip unit number.
 *  @param pipe_num Pipe number.
 *  @param pm_num Port Macro number.
 ...
 */
int
soc_trident3_idb_wr_obm_port_ca_sop(int unit, int pipe_num, int pm_num, 
                                    int subp, int speed)
{
    soc_reg_t reg;
    uint32 rval, ca_sop;
...
/*! @fn int soc_trident3_idb_wr_obm_ovs_en(int unit, int pipe_num, int pm_num,
 *              int subp)
 *  @param unit Chip unit number.
 *  @param pipe_num Pipe number.
 *  @param pm_num Port Macro number.
 ...
 */
int
soc_trident3_idb_wr_obm_ovs_en(int unit, int pipe_num, int pm_num, int subp)
{
    soc_reg_t reg;
    uint32 rval;
...
/*! @fn int soc_trident3_idb_wr_obm_buffer_limit(int unit, int pipe_num, int pm_num,
 ...
 */
int
soc_trident3_idb_wr_obm_buffer_limit(int unit, int pipe_num, int pm_num, int subp,
                                int num_lanes)
{
    soc_reg_t reg;
    uint64 rval64;
    ...
    const soc_reg_t obm_buffer_config[TRIDENT3_PIPES_PER_DEV][
		TRIDENT3_PBLKS_PER_PIPE] = {
        ...
        {IDB_OBM0_BUFFER_CONFIG_PIPE1r,
		 IDB_OBM1_BUFFER_CONFIG_PIPE1r,
		 IDB_OBM2_BUFFER_CONFIG_PIPE1r,
		 IDB_OBM3_BUFFER_CONFIG_PIPE1r,
		 IDB_OBM4_BUFFER_CONFIG_PIPE1r,
		 IDB_OBM5_BUFFER_CONFIG_PIPE1r,
		 IDB_OBM6_BUFFER_CONFIG_PIPE1r,
		 IDB_OBM7_BUFFER_CONFIG_PIPE1r,
		 IDB_OBM8_BUFFER_CONFIG_PIPE1r,
		 IDB_OBM9_BUFFER_CONFIG_PIPE1r,
		 IDB_OBM10_BUFFER_CONFIG_PIPE1r,
		 IDB_OBM11_BUFFER_CONFIG_PIPE1r,
		 IDB_OBM12_BUFFER_CONFIG_PIPE1r,
		 IDB_OBM13_BUFFER_CONFIG_PIPE1r,
		 IDB_OBM14_BUFFER_CONFIG_PIPE1r,
         IDB_OBM15_BUFFER_CONFIG_PIPE1r} 
	};


	buffer_start 	= subp * TRIDENT3_OBM_BUFFER_PER_LANE;

	if(num_lanes == 0) 	buffer_end = buffer_start;
	else 				buffer_end = buffer_start + 
							(num_lanes * TRIDENT3_OBM_BUFFER_PER_LANE) - 1;
    LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit,
            "soc_trident3_idb_wr_obm_buffer_limit: pipe: %1d, pm_num: %1d,"
...
/*! @fn int soc_trident3_idb_wr_obm_thresh(int unit, int pipe_num, int pm_num,
 *              int subp)
 *  @param unit Chip unit number.
 *  @param pipe_num Pipe number.
 *  @param pm_num Port Macro number.
 ...
 */
int
soc_trident3_idb_wr_obm_thresh(int unit, int pipe_num, int pm_num, int subp,
                                int num_lanes, int lossless)
{
    soc_reg_t reg;
    int i;
    ...
    const soc_reg_t obm_thresh_pp[TRIDENT3_PIPES_PER_DEV][
		TRIDENT3_PBLKS_PER_PIPE][2] = {
...
/*! @fn int soc_trident3_idb_wr_obm_fc_threshold(int unit, int pipe_num, int pm_num, *              int subp, int num_lanes)
 *  @param unit Chip unit number.
 *  @param pipe_num Pipe number.
 *  @param pm_num Port Macro number.
 *  @param subp Sub-port number.
 ...
 */
int
soc_trident3_idb_wr_obm_fc_threshold(int unit, int pipe_num, int pm_num,
                                      int subp,
                                      int num_lanes, int lossless)
{
    soc_reg_t reg;
    ...
    uint32 lossless0_xon, lossless0_xoff, lossless1_xon, lossless1_xoff,
           port_xon, port_xoff;
    uint64 rval64;
    const soc_reg_t obm_fc_thresh_pp[TRIDENT3_PIPES_PER_DEV][
		TRIDENT3_PBLKS_PER_PIPE][2] = {
...
/*! @fn int soc_trident3_idb_obm_poll_buffer_empty(int unit, int pipe_num,
 *              int pm_num, int subp)
 *  @param unit Chip unit number.
 *  @param pipe_num Trident3 pipe number.
 *  @param pm_num PM number within the pipe: Ranges from 0 to 15.
 ...
 */
int
soc_trident3_idb_obm_poll_buffer_empty(int unit, int pipe_num, int pm_num,
                                        int subp)
{
    soc_reg_t reg;
    uint64 rval64;
...
/*! @fn int soc_trident3_idb_ca_poll_buffer_empty(int unit, int pipe_num,
 *              int pm_num, int subp)
 *  @param unit Chip unit number.
 *  @param pipe_num Trident3 pipe number.
 *  @param pm_num PM number within the pipe: Ranges from 0 to 15.
 ...
 */
int
soc_trident3_idb_ca_poll_buffer_empty(int unit, int pipe_num, int pm_num,
                                       int subp)
{
    soc_reg_t reg;
    uint64 rval64;
    ...
    static const soc_reg_t ca_status_regs[TRIDENT3_PIPES_PER_DEV][
        TRIDENT3_PBLKS_PER_PIPE] = {
...
/*! @fn int soc_trident3_idb_ca_mgmt_poll_buffer_empty(int unit, int pipe_num)
 *  @param unit Chip unit number.
 *  @param pipe_num Trident3 pipe number.
 ...
 */
int
soc_trident3_idb_ca_mgmt_poll_buffer_empty(int unit, int pipe_num)
{
    soc_reg_t reg;
    uint32 rval32;
    uint32 ca_fifo_empty;
    const soc_reg_t idb_ca_mgmt_hw_status[TRIDENT3_PIPES_PER_DEV]= {
    ...
    if(pipe_num == 0) {
        LOG_CLI((BSL_META_U(unit, 
                "___ERROR invalid pipe number 0 used in "
                "soc_trident3_idb_ca_mgmt_poll_buffer_empty")));
        return SOC_E_FAIL;
    }
...
/*! @fn int soc_trident3_idb_ca_cpu_poll_buffer_empty(int unit, int pipe_num)
 *  @param unit Chip unit number.
 *  @param pipe_num Trident3 pipe number.
 ...
 */
int
soc_trident3_idb_ca_cpu_poll_buffer_empty(int unit, int pipe_num)
{
    soc_reg_t reg;
    uint32 rval32;
    uint32 ca_fifo_empty;
    const soc_reg_t idb_ca_cpu_hw_status[TRIDENT3_PIPES_PER_DEV]= {
...
/*! @fn int soc_trident3_idb_ca_lpbk_poll_buffer_empty(int unit, int pipe_num)
 *  @param unit Chip unit number.
 *  @param pipe_num Trident3 pipe number.
 ...
 */
int
soc_trident3_idb_ca_lpbk_poll_buffer_empty(int unit, int pipe_num)
{
    soc_reg_t reg;
    uint32 rval32;
    uint32 ca_fifo_empty;
    const soc_reg_t idb_ca_lpbk_hw_status[TRIDENT3_PIPES_PER_DEV]= {
...
/*! @fn int soc_trident3_speed_to_bmop_class_map(int speed)
 ...
 */
int
soc_trident3_speed_to_bmop_class_map(int speed)
{
    int bmop_class;
    int i;
    ...
    bmop_class=0;
    for (i = 0; i <= 12; i++) {
        if (soc_trident3_bmop_cfg_tbl[i].speed == speed) {
            bmop_class = i;
            break;
        }
    }
...
/*! @fn int soc_trident3_idb_obm_bubble_mop_set(int unit,
 *              soc_port_schedule_state_t *port_schedule_state)
 *  @param unit Chip unit number.
 *  @param port_schedule_state Pointer to a soc_port_schedule_state_t struct
 *         variable
 ...
 */
int
soc_trident3_idb_obm_bubble_mop_set(
    int unit, soc_port_schedule_state_t *port_schedule_state)
{
    soc_reg_t reg;
    uint32 rval32;
    ...
    for (i = 0; i < port_schedule_state->nport; i++) {
        if (port_schedule_state->resource[i].physical_port != -1) {
            ...
            pipe_num = soc_trident3_get_pipe_from_phy_pnum(phy_port);
            pm_num = soc_trident3_get_pm_from_phy_pnum(phy_port);
            ...
            speed_encoding =
                (port_schedule_state->cutthru_prop.asf_modes[port]==
                 _SOC_ASF_MODE_SAF) ? 0 :
                soc_trident3_speed_to_bmop_class_map(
                    port_schedule_state->resource[i].speed);
            bubble_mop_disable =
                oversub ? !(soc_trident3_bmop_cfg_tbl[speed_encoding].
                            bmop_enable.
                            oversub[latency]) :
                !(soc_trident3_bmop_cfg_tbl[speed_encoding].
...
/*! @fn int soc_trident3_idb_obm_port_config_set(int unit,
 *              soc_port_schedule_state_t *port_schedule_state)
 *  @param unit Chip unit number.
 ...
 */
int
soc_trident3_idb_obm_port_config_set(
    int unit, soc_port_schedule_state_t *port_schedule_state)
{
    soc_reg_t reg;
    uint32 rval32;
    ...
    int port;
    int i;
    static const soc_reg_t obm_port_config_regs[TRIDENT3_PIPES_PER_DEV][
        TRIDENT3_PBLKS_PER_PIPE] = {
...
/*! @fn int soc_trident3_idb_obm_dscp_map_set(int unit,
 *              soc_port_schedule_state_t *port_schedule_state)
 *  @param unit Chip unit number.
 *  @param port_schedule_state Pointer to a soc_port_schedule_state_t struct variable
 ...
 */
int
soc_trident3_idb_obm_dscp_map_set(
    int unit, soc_port_schedule_state_t *port_schedule_state)
{
    soc_mem_t mem;
    ...
    int lossless;
    uint32 priority;
    static const soc_mem_t obm_pri_map_mem[][TRIDENT3_PIPES_PER_DEV][
		TRIDENT3_PORTS_PER_PBLK] = {
    ...
    priority = lossless ? TRIDENT3_OBM_PRIORITY_LOSSLESS0 : 
							TRIDENT3_OBM_PRIORITY_LOSSY_LO;
...
/*! @fn int soc_trident3_idb_obm_force_saf_set(int unit,
 *              soc_port_schedule_state_t *port_schedule_state)
 *  @param unit Chip unit number.
 *  @param port_schedule_state Pointer to a soc_port_schedule_state_t struct variable
 ...
 */
int
soc_trident3_idb_obm_force_saf_set(
    int unit, soc_port_schedule_state_t *port_schedule_state)
{
    soc_reg_t reg;
    uint32 rval32;
    ...
    int duration_timer;
    uint32 ovs_prt_up;
    static const soc_reg_t obm_force_saf_config_regs[TRIDENT3_PIPES_PER_DEV][
        TRIDENT3_PBLKS_PER_PIPE] = {
    ...
    static const soc_reg_t idb_force_saf_config_regs[TRIDENT3_PIPES_PER_DEV] =
    ...
    LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit,
              "soc_trident3_idb_obm_force_saf_set ovs_prt_up:: %x \n"),
              ovs_prt_up));
    duration_timer=
        soc_trident3_fsaf_dur_timer_tbl[TRIDENT3_FORCE_SAF_TIMER_ENTRY_CNT].
        duration_timer;
    for (i=0; i<TRIDENT3_FORCE_SAF_TIMER_ENTRY_CNT; i++) {
        if (port_schedule_state->frequency ==
            soc_trident3_fsaf_dur_timer_tbl[i].frequency) {
            duration_timer=soc_trident3_fsaf_dur_timer_tbl[i].duration_timer;
        }
    }
    for (i = 0; i < TRIDENT3_PIPES_PER_DEV; i++) {
        pipe_num=i;
        LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit,
                  "soc_trident3_idb_obm_force_saf_set ovs_prt_up:: %x "
                  "pipe_num %d\n"), ovs_prt_up,pipe_num));
        if (ovs_prt_up>>pipe_num) {
            ...
            rval32=0;
            LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit,
                      "soc_trident3_idb_obm_force_saf_set Write to "
...
/*! @fn int soc_trident3_flex_dis_forwarding_traffic(int unit,
 *              soc_port_schedule_state_t *port_schedule_state)
 *  @param unit Chip unit number.
 *  @param port_schedule_state Pointer to a soc_port_schedule_state_t struct variable
 ...
 */
int
soc_trident3_flex_dis_forwarding_traffic(
    int unit, soc_port_schedule_state_t *port_schedule_state)
{
    int i;
    uint32 entry[SOC_MAX_MEM_WORDS];
...
/*! @fn int soc_trident3_flex_en_forwarding_traffic(int unit,
 *              soc_port_schedule_state_t *port_schedule_state)
 *  @param unit Chip unit number.
 *  @param port_schedule_state Pointer to a soc_port_schedule_state_t struct variable
 *  @brief Helper function to Write EPC_LINK_BMAP table to enable forwarding traffic
 *         to port. Follows the sequence specified in document "TD3_uA_flexport.pdf",
 *         Section 6.6.2, bullet number 5.
 *  @returns SOC_E_NONE
 */
int
soc_trident3_flex_en_forwarding_traffic(
    int unit, soc_port_schedule_state_t *port_schedule_state)
{
    int i;
    uint32 entry[SOC_MAX_MEM_WORDS];
...
/*! @fn int soc_trident3_idb_port_down(int unit,
 *              soc_port_schedule_state_t *port_schedule_state)
 *  @param unit Chip unit number.
 *  @param port_schedule_state Pointer to a soc_port_schedule_state_t struct
 *         variable
 ...
 */
int
soc_trident3_flex_idb_port_down(int                        unit,
                                 soc_port_schedule_state_t *port_schedule_state)
{
    int i;
    int pipe_num;
    ...
    for (i = 0; i < port_schedule_state->nport; i++) {
        if (port_schedule_state->resource[i].physical_port == -1) {
            ...
            pm_num = soc_trident3_get_pm_from_phy_pnum(phy_port);
            subp = soc_trident3_get_subp_from_phy_pnum(phy_port);
            if(soc_trident3_is_2nd_mgmt_port(&port_schedule_state->in_port_map, phy_port)) {
               if (!soc_trident3_idb_mgmt_ca_in_reset(unit, pipe_num)){
                   SOC_IF_ERROR_RETURN(soc_trident3_idb_ca_mgmt_poll_buffer_empty(
                                                           unit, pipe_num));
               }
            } else if((phy_port == TRIDENT3_PHY_PORT_CPU) ||
                      (phy_port == TRIDENT3_PHY_PORT_MNG0)) {
                if (!soc_trident3_idb_cpu_ca_in_reset(unit,pipe_num)){
                    SOC_IF_ERROR_RETURN(soc_trident3_idb_ca_cpu_poll_buffer_empty(
                                                         unit, pipe_num));
                }
            } else if((phy_port == TRIDENT3_PHY_PORT_LPBK0)||
                      (phy_port == TRIDENT3_PHY_PORT_LPBK1)) {
                if (!soc_trident3_idb_lpbk_ca_in_reset(unit,pipe_num)){
                    SOC_IF_ERROR_RETURN(soc_trident3_idb_ca_lpbk_poll_buffer_empty(
                                                             unit, pipe_num));
                }
            } else {
                SOC_IF_ERROR_RETURN(soc_trident3_idb_obm_poll_buffer_empty(unit,
                                                         pipe_num,
                                                         pm_num,
                                                         subp));
                SOC_IF_ERROR_RETURN(soc_trident3_idb_ca_poll_buffer_empty(unit,
                                                         pipe_num,
                                                         pm_num,
                                                         subp));
            }
    ...
    for (i = 0; i < port_schedule_state->nport; i++) {
        if (port_schedule_state->resource[i].physical_port == -1) {
            ...
            reset_buffer = 1;
            if(soc_trident3_is_2nd_mgmt_port(&port_schedule_state->in_port_map, phy_port)) {
                SOC_IF_ERROR_RETURN(soc_trident3_idb_mgmt_ca_reset_buffer(
                                       unit, pipe_num, reset_buffer));
            } else if((phy_port == TRIDENT3_PHY_PORT_CPU) ||
                      (phy_port == TRIDENT3_PHY_PORT_MNG0)) {
                SOC_IF_ERROR_RETURN(soc_trident3_idb_cpu_ca_reset_buffer(
                                       unit, pipe_num, reset_buffer));
            } else if((phy_port == TRIDENT3_PHY_PORT_LPBK0)||
                      (phy_port == TRIDENT3_PHY_PORT_LPBK1)) {
                SOC_IF_ERROR_RETURN(soc_trident3_idb_lpbk_ca_reset_buffer(
                                       unit, pipe_num, reset_buffer));
            } else {
                SOC_IF_ERROR_RETURN(soc_trident3_idb_obm_reset_buffer(unit,
                                                              pipe_num,
                                                              pm_num, subp,
                                                              reset_buffer));
                SOC_IF_ERROR_RETURN(soc_trident3_idb_ca_reset_buffer(unit,
                                                              pipe_num,
                                                              pm_num, subp,
                                                              reset_buffer));
            }
...
/*! @fn int soc_trident3_flex_idb_reconfigure(int unit,
 *              soc_port_schedule_state_t *port_schedule_state)
 *  @param unit Chip unit number.
 *  @param port_schedule_state Pointer to a soc_port_schedule_state_t struct variable
 ...
 */
int
soc_trident3_flex_idb_reconfigure(
    int unit, soc_port_schedule_state_t *port_schedule_state)
{
    int i;
    int pipe_num;
    ...
    uint32 memfld;
    soc_mem_t mem;
    static soc_mem_t idb_to_dev_port_map_mem[2] = 
          {ING_IDB_TO_DEVICE_PORT_NUMBER_MAPPING_TABLE_PIPE0m,
           ING_IDB_TO_DEVICE_PORT_NUMBER_MAPPING_TABLE_PIPE1m};

    SOC_IF_ERROR_RETURN(soc_trident3_tdm_flexport_idb(unit,
                                                       port_schedule_state));
    SOC_IF_ERROR_RETURN(_soc_td3_tdm_set_idb_hsp(unit,
                                                       port_schedule_state));
    SOC_IF_ERROR_RETURN(_soc_td3_tdm_set_idb_calendar(unit,
                                                            port_schedule_state));
    ...
    for(i = 0; i < port_schedule_state->nport; i++) {
        ...
        memfld = (physical_port == -1) ? 0xff : logical_port;
        pipe_num = logical_port / TRIDENT3_DEV_PORTS_PER_PIPE;
        mem = idb_to_dev_port_map_mem[pipe_num];	
    ...
    for (i = 0; i < port_schedule_state->nport; i++) {
        if (port_schedule_state->resource[i].physical_port != -1) {
            ...
            SOC_IF_ERROR_RETURN(soc_trident3_idb_wr_obm_ovs_en(unit, pipe_num,
                                                                pm_num,subp));

            SOC_IF_ERROR_RETURN(soc_trident3_idb_wr_obm_port_ca_sop(unit, pipe_num,
                                                                pm_num,subp,speed));

            SOC_IF_ERROR_RETURN(soc_trident3_idb_wr_obm_buffer_limit(unit, pipe_num,
                                                                pm_num,subp,
                                                                num_lanes));

            SOC_IF_ERROR_RETURN(soc_trident3_idb_wr_obm_thresh(unit, pipe_num,
                                                                pm_num,subp,
                                                                num_lanes,
                                                                lossless));
            LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit,
            ...
            SOC_IF_ERROR_RETURN(soc_trident3_idb_wr_obm_shared_config(unit,
                                                                      pipe_num,
                                                                      pm_num,
                                                                      subp,
                                                                      num_lanes,
            ...
            LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit,
                      ...
                      "subp=%1d num_lanes=%1d\n"),
                      i, pipe_num, pm_num, subp, num_lanes));
            SOC_IF_ERROR_RETURN(soc_trident3_idb_wr_obm_flow_ctrl_cfg(unit,
                                                                      pipe_num,
                                                                      pm_num,
                                                                      subp,
                                                                      lossless));
            LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit,
                      ...
                      "num_lanes=%1d\n"),
                      i, pipe_num, pm_num, subp, num_lanes));
            SOC_IF_ERROR_RETURN(soc_trident3_idb_wr_obm_fc_threshold(unit,
                                                                     pipe_num,
                                                                     pm_num,
                                                                     subp,
                                                                     num_lanes,
    ...
    SOC_IF_ERROR_RETURN(soc_trident3_idb_obm_bubble_mop_set(unit,
                                                     port_schedule_state));
    LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit,
              "Call soc_trident3_idb_port_mode_set \n")));
    SOC_IF_ERROR_RETURN(soc_trident3_idb_port_mode_set(unit,
                                                     port_schedule_state));
    LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit,
              "Call soc_trident3_idb_obm_force_saf_set \n")));
    SOC_IF_ERROR_RETURN(soc_trident3_idb_obm_force_saf_set(unit,
                                                     port_schedule_state));
    LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit,
              "Call soc_trident3_idb_obm_port_config_set \n")));
    SOC_IF_ERROR_RETURN(soc_trident3_idb_obm_port_config_set(unit,
                                                     port_schedule_state));
    LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit,
              "Call soc_trident3_idb_obm_dscp_map_set \n")));
    SOC_IF_ERROR_RETURN(soc_trident3_idb_obm_dscp_map_set(unit,
                                                     port_schedule_state));
    LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit,
              "Call soc_trident3_idb_clear_stats_new_ports \n")));
    SOC_IF_ERROR_RETURN(soc_trident3_idb_clear_stats_new_ports(unit,
                                                     port_schedule_state));

    return SOC_E_NONE;
}

/*! @fn int soc_trident3_flex_idb_port_up(int unit,
 *              soc_port_schedule_state_t *port_schedule_state)
 *  @param unit Chip unit number.
 *  @param port_schedule_state Pointer to a soc_port_schedule_state_t struct variable
 ...
 */
int
soc_trident3_flex_idb_port_up(int                        unit,
                               soc_port_schedule_state_t *port_schedule_state)
{
    int i;
    int pipe_num;
    ...
    for (i = 0; i < port_schedule_state->nport; i++) {
        if (port_schedule_state->resource[i].physical_port != -1) {
            ...
            subp = soc_trident3_get_subp_from_phy_pnum(phy_port);
            reset_buffer = 1;
            if(soc_trident3_is_2nd_mgmt_port(&port_schedule_state->out_port_map, phy_port)) {
                SOC_IF_ERROR_RETURN(soc_trident3_idb_mgmt_ca_reset_buffer(
                                       unit, pipe_num, reset_buffer));
            } else if((phy_port == TRIDENT3_PHY_PORT_CPU) ||
                      (phy_port == TRIDENT3_PHY_PORT_MNG0)) {
...
int soc_trident3_is_2nd_mgmt_port(soc_port_map_type_t *port_map, int phy_port) {
	int mgmt_port_cnt;

	SOC_PBMP_COUNT(port_map->management_pbm, mgmt_port_cnt);
	if((mgmt_port_cnt > 1) && (phy_port == TRIDENT3_PHY_PORT_MNG1)) {
		return 1;
	} else {
		return 0;
	}
...
int
soc_trident3_idb_flexport(int                        unit,
                           soc_port_schedule_state_t *port_schedule_state)
{

    SOC_IF_ERROR_RETURN(soc_trident3_flex_start(unit, port_schedule_state));

    /* IDB Ports down. */
    SOC_IF_ERROR_RETURN(soc_trident3_flex_idb_port_down(unit,
                                                    port_schedule_state));

    /* startTDM flex related codes: Florin's routines.*/
    SOC_IF_ERROR_RETURN(soc_trident3_tdm_calculation_flexport(unit,
                                                    port_schedule_state));

    /* Reconfigure IDB. */
    SOC_IF_ERROR_RETURN(soc_trident3_flex_idb_reconfigure(unit,
                                                    port_schedule_state));

    /* IDB Ports up*/
    SOC_IF_ERROR_RETURN(soc_trident3_flex_idb_port_up(unit,
                                                    port_schedule_state));

    SOC_IF_ERROR_RETURN(soc_trident3_flex_end(unit, port_schedule_state));
