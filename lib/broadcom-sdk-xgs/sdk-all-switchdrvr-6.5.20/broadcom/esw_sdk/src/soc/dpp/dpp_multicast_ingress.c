...
)
{
  ...
  /*
   * there are 3 options for group status: closed, open_full & open_mmc
   * when this function called to change open to close/close to open the action should take place because it's called from destroy/open context
   * When this function called to change full to mini nothing should be done
   * When this function called to change mini to full error should be returned because this means that user configured more replicatons than mini multicast allows
   */
  ...
  {
      if (value_internal != value) { /* If setting a different value, update hardware */
