...
/*
 * Function:
 *      soc_jer_shared_mem_reject_config_get
 * Purpose:
 *      Get the VOQ shared memory reject percentage per DP.
 *      Reduce percentage fractions to get maximal precision.
 * Parameters:
 *      unit    - Device Number
 * Returns:
 *      SOC_E_XXX
...
{
    int dp;
    int shared_mem_reject_percentage[SOC_TMC_NOF_DROP_PRECEDENCE] = { 100, /* 100% for GREEN  */
                                                                      85,  /* 85%  for YELLOW */
                                                                      75,  /* 75%  for RED    */
                                                                      0    /* 0%   for BLACK  */
    };
    ...
    int factors[] = {5, 5, 2, 2}; /* 100 primary factors */
...
int soc_jer_specific_info_config_direct(int unit)
{
    ...
    /* Enable/Disable truncate (ITM editing) for counter processor */
    ...
    if (!SOC_WARM_BOOT(unit)) 
    {
        ...
        if (SOC_IS_JERICHO_PLUS(unit)) {
            ...
            for (ii = SOC_REG_INFO(unit, KAPS_RPB_TCAM_SCAN_ERROR_MASKr).first_array_index; ii < SOC_REG_INFO(unit, KAPS_RPB_TCAM_SCAN_ERROR_MASKr).first_array_index + SOC_REG_NUMELS(unit, KAPS_RPB_TCAM_SCAN_ERROR_MASKr); ii++) {
                 ...
                 * frequency is in khz
                 * The window is between each tcam line, divide by the number of TCAM rows
                 */
