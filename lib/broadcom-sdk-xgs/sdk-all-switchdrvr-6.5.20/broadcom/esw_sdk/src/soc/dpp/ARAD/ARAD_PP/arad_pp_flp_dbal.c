...
/* This function allocates prefixes to the tables in KAPS
   These prefixes are assigned dynamically in order to maximize the IPV4 UC and IPV4 MC capacity
...
arad_pp_flp_dbal_kaps_table_prefix_get(int unit, SOC_DPP_DBAL_SW_TABLE_IDS table_id, uint32 *table_prefix, uint32 *table_prefix_len)
{
    ...
    uint32 dynamic_tables_additional_bits = 0;  /* All dynamic table additional bits */
    uint32 dynamic_table_prefix = 0; /*unique dynamic table prefix*/
    uint32 dynamic_tables_additional_bits_max = JER_KAPS_DYNAMIC_TABLE_PREFIX_MAX_BIT + 1; /* additional one bit for IPV4 MC */
    ...
    if (SOC_DPP_L3_IPV6_COMPRESSED_SIP_ENABLE(unit)) {
        num_of_dynamic_tables++;
        num_of_dynamic_tables++; /* There are two tables for IPv6 MC SSM */
    ...
    */
    ...
    /* count all dynamic table additional bits */
    ...
    if (dynamic_tables_additional_bits <= JER_KAPS_DYNAMIC_TABLE_PREFIX_MAX_BIT) {
        ...
        /* count mc dynamic table additional bits */
    ...
    case SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_RPF_KAPS: /* Always dynamic_table_prefix = 0 */
        ...
        *table_prefix_len =  JER_KAPS_TABLE_PREFIX_LENGTH + ipv4uc_dynamic_tables_additional_bits;
        break;
    case SOC_DPP_DBAL_SW_TABLE_ID_IPV4MC_KAPS:     /* Always dynamic_table_prefix = 0 */
    ...
    */
    if (dynamic_table_prefix != 0) {
        if (dynamic_table_prefix <= (JER_KAPS_DYNAMIC_TABLE_PREFIX_MAX_NUM - 1)) { /* Allocated in IPV4 UC dynamic prefix */
           *table_prefix = (JER_KAPS_IPV4_UC_AND_NON_IP_TABLE_PREFIX << ipv4uc_dynamic_tables_additional_bits) + dynamic_table_prefix;
            *table_prefix_len =  JER_KAPS_TABLE_PREFIX_LENGTH + ipv4uc_dynamic_tables_additional_bits;
        } else { /* Allocated in IPV4 MC dynamic prefix */
    ...
    arad_pp_flp_dbal_pon_ipv4_sav_lem_table_create(int unit, uint8 is_not_arp, uint8 is_arp_up, SOC_DPP_DBAL_SW_TABLE_IDS table_id)
{
    ...
    if (!is_table_initiated) {
        /* ARP packets share SIP+FID LPM/LEM DB */
        DBAL_QUAL_INFO_CLEAR(&qual_info, SOC_PPC_FP_NOF_QUALS_PER_DB_MAX);
    ...
    arad_pp_flp_dbal_ipv6uc_kaps_route_scale_short_table_create(int unit)
{
    ...
    if (!is_table_initiated) {
        ...
        DBAL_QUAL_INFO_CLEAR(&qual_info, SOC_PPC_FP_NOF_QUALS_PER_DB_MAX);
        /* Use 60 bits of IPv6 prefix, 6(TID) + 14(VRF) + 60( 32 + 16 + 12 DIP) = 80 */
        qual_info[qualifiers_counter].qual_type = SOC_PPC_FP_QUAL_HDR_FWD_IPV6_DIP_HIGH;
        qual_info[qualifiers_counter].qual_offset = 48;
        qual_info[qualifiers_counter++].qual_nof_bits = 12;
        qual_info[qualifiers_counter].qual_type = SOC_PPC_FP_QUAL_HDR_FWD_IPV6_DIP_HIGH;
    ...
    arad_pp_flp_dbal_ipv4mc_bridge_kaps_table_create(int unit)
{
    ...
    if (!is_table_initiated) {
        ...
        if (table_prefix_len < 5) {
            qual_info[qualifiers_counter].qual_type = SOC_PPC_FP_QUAL_IRPP_ALL_ZEROES;
            qual_info[qualifiers_counter++].qual_nof_bits = 5 - table_prefix_len; /*Align to 80bits*/
        }
    ...
    arad_pp_flp_dbal_ipv4mc_bridge_kaps_table_create_ivl(int unit)
{
    ...
    /* both tables are initiated with same prefix, SOC_DPP_DBAL_SW_TABLE_ID_IPV4MC_BRIDGE_VLAN_FID, */
    ...
    /* MSB */
    qual_info[3].qual_type = SOC_PPC_FP_QUAL_IRPP_ALL_ZEROES;
    qual_info[3].qual_nof_bits = 25 - table_prefix_len; /*Align to 80bits*/
    ...
    arad_pp_flp_dbal_ipv4uc_anti_spoofing_kaps_table_create(int unit)
{
    ...
    if (!is_table_initiated) {
        ...
        DBAL_QUAL_INFO_CLEAR(&qual_info, SOC_PPC_FP_NOF_QUALS_PER_DB_MAX);
        qual_info[qualifiers_counter].qual_type = SOC_PPC_FP_QUAL_IRPP_ALL_ZEROES;
        qual_info[qualifiers_counter++].qual_nof_bits = table_prefix_len == JER_KAPS_TABLE_PREFIX_LENGTH ? 28 : 24; /* Align to 80bits according to the dynamic prefix len*/
...
arad_pp_flp_dbal_ipv6uc_kbp_sip_sharing_table_create(int unit)
{
    ...
    /* this table is used for entry management and lookup indication */
    ...
    arad_pp_flp_dbal_kbp_default_route_kaps_table_create(int unit, SOC_DPP_DBAL_SW_TABLE_IDS route_table)
{
    ...
    if(!is_table_initiated) {
        ...
        case SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_KBP_DEFAULT_ROUTE_KAPS:
            ...
            break;
        default: /* for SOC_DPP_DBAL_SW_TABLE_ID_IPV6MC_KBP_DEFAULT_ROUTE_KAPS */
    ...
    arad_pp_flp_dbal_ipv4mc_bridge_kbp_table_create(int unit)
{
    ...
    if (!is_table_initiated) {
        ...
        qual_info[num_tables++].qual_nof_bits = 4; /* byte alligned */
        qual_info[num_tables].qual_type = SOC_PPC_FP_QUAL_FID;
        qual_info[num_tables++].qual_nof_bits = 15;
        qual_info[num_tables].qual_type = SOC_PPC_FP_QUAL_IRPP_ALL_ZEROES;
        qual_info[num_tables++].qual_nof_bits = 1; /* byte alligned */
        ...
        if (SOC_DPP_CONFIG(unit)->pp.l3_mc_use_tcam != ARAD_PP_FLP_L3_MC_USE_TCAM_DISABLE) {
            ...
            if(JER_KAPS_ENABLE(unit)){
                ...
                if (SOC_DPP_CONFIG(unit)->pp.ipmc_lookup_model == 1) {
                    ...
                    qual_info[qualifiers_counter].qual_type = SOC_PPC_FP_QUAL_IRPP_ALL_ZEROES;
                    if (table_prefix_len != JER_KAPS_TABLE_PREFIX_LENGTH) { /* dynamic prefix, ipv4mc dynamic additional bits max is 3 */
                    ...
                    if (table_prefix_len == JER_KAPS_TABLE_PREFIX_LENGTH) { /* static prefix */
                        /* LSB */
                        qual_info[qualifiers_counter].qual_type = SOC_PPC_FP_QUAL_IRPP_ALL_ZEROES;
    ...
    arad_pp_flp_dbal_ipv6mc_ssm_compressed_sip_table_create(int unit)
{
     ...
     if (!is_table_initiated) {
          ...
          qual_info[qualifiers_counter++].qual_nof_bits = 1;
          qual_info[qualifiers_counter++].qual_type = SOC_PPC_FP_QUAL_IRPP_IN_RIF;
          qual_info[qualifiers_counter].qual_type = SOC_PPC_FP_QUAL_HDR_FWD_IPV6_SIP_LOW;  /* SIP 31:0 */
          qual_info[qualifiers_counter].qual_offset = 32;
          qual_info[qualifiers_counter++].qual_nof_bits = 32;
          qual_info[qualifiers_counter].qual_type = SOC_PPC_FP_QUAL_HDR_FWD_IPV6_SIP_LOW;  /* SIP 63:32 */
          qual_info[qualifiers_counter].qual_offset = 0;
          qual_info[qualifiers_counter++].qual_nof_bits = 32;
          /* MSB */
          qual_info[qualifiers_counter].qual_type = SOC_PPC_FP_QUAL_HDR_FWD_IPV6_SIP_HIGH;  /* SIP 95:64 */
          qual_info[qualifiers_counter].qual_offset = 32;
          qual_info[qualifiers_counter++].qual_nof_bits = 32;
          qual_info[qualifiers_counter].qual_type = SOC_PPC_FP_QUAL_HDR_FWD_IPV6_SIP_HIGH;  /* SIP 127:96 */
          qual_info[qualifiers_counter].qual_offset = 0;
          qual_info[qualifiers_counter++].qual_nof_bits = 32;
          qual_info[qualifiers_counter].qual_type = SOC_PPC_FP_QUAL_IRPP_ALL_ZEROES;
    ...
    arad_pp_flp_dbal_ipv6mc_ssm_table_create(int unit)
{
     ...
     if (!is_table_initiated) {
          ...
          DBAL_QUAL_INFO_CLEAR(&qual_info, SOC_PPC_FP_NOF_QUALS_PER_DB_MAX);
          /* LSB */
          qual_info[qualifiers_counter].qual_type = SOC_PPC_FP_QUAL_IPV6_COMPRESSED_SIP; /* RCH.iPMF-Opaque[0:15] as IPv6 compressed SIP(16b) */
          qual_info[qualifiers_counter].qual_nof_bits = 16;
          qual_info[qualifiers_counter].qual_offset = 0;
...
#endif /*#if defined(BCM_88675_A0) && defined(INCLUDE_KBP) */
    ...
    arad_pp_flp_dbal_oam_outlif_l2g_lem_table_create(int unit)
{
    ...
    if(!is_table_initiated) {
        ...
        qual_info[0].qual_type = SOC_PPC_FP_QUAL_HDR_DSP_EXTENSION_AFTER_FTMH;    /* local outlif [15:0] in DSP extension */
        qual_info[0].qual_offset = 0;
        qual_info[0].qual_nof_bits = 16;
        qual_info[0].qual_is_in_hdr = 1;

        qual_info[1].qual_type = SOC_PPC_FP_QUAL_HDR_FTMH;                        /* local outlif_msb [18:16] in low 3 bits of FTMH */
    ...
    arad_pp_flp_dbal_two_pass_mc_encap_to_dest_lem_table_create(int unit)
{
    ...
    if(!is_table_initiated) {
        ...
        qual_info[1].qual_type = SOC_PPC_FP_QUAL_HDR_FTMH;   /* Multicast-ID-or-OutLIF high 3 bits in [20:22] FTMH */
        qual_info[1].qual_offset = 49;
        qual_info[1].qual_nof_bits = 3;
        qual_info[1].qual_is_in_hdr = 1;

        qual_info[0].qual_type = SOC_PPC_FP_QUAL_HDR_FTMH;   /* Multicast-ID-or-OutLIF low 16 bits in [4:19] FTMH */
   ...
   )
{
    ...
    if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "mim_spb_enable", 0)) {
        /* for SPB, add a LEM lookup for L2 RPF */
   ...
   )
{
    ...
    if(SOC_DPP_CONFIG(unit)->pp.ipmc_l2_ssm_mode == BCM_IPMC_SSM_TCAM_LPM)
    {
        ...
        if(SOC_IS_JERICHO(unit) && ARAD_KBP_IS_IN_USE) {
            /* IPMC SSM uses key D if ELK lookup is enabled */
    ...
    if (SOC_DPP_CONFIG(unit)->pp.ipmc_l2_ssm_mode != BCM_IPMC_SSM_DISABLE) {
        ...
        if (SOC_DPP_CONFIG(unit)->pp.ipmc_l2_ssm_mode == BCM_IPMC_SSM_ELK_LPM) {
            keys_to_table_id[2].lookup_number = 0; /* lookup 0 used as FWD in KBP */
        } else {
            keys_to_table_id[2].lookup_number = 2;
        }
        nof_valid_keys = 3;
    ...
    /* update LEM 1st lookup type (LEM_1ST_LKP_KEY_TYPE) for IPV4MC Bridge Program */
   ...
   )
{
    ...
    if(ARAD_KBP_ENABLE_IPV4_MC){
        ...
        /* Set the number of qualifiers of IPV4MC_KBP_DEFAULT_ROUTE_KAPS temporarily to 0
         * in order not to assign new CEs, but instead to reuse the CEs in KEY_D_MSB,
         * which will be created for the identical table IPV4UC_KBP_DEFAULT_ROUTE_KAPS
         */
...
#if defined(BCM_88675_A0) && defined(INCLUDE_KBP)
    if (ARAD_KBP_ENABLE_IPV4_MC) {
        /* Set the number of qualifiers of IPV4MC_KBP_DEFAULT_ROUTE_KAPS to its original value */
        SOC_SAND_IF_ERR_EXIT(sw_state_access[unit].dpp.soc.arad.pp.dbal_info.dbal_tables.get(unit, SOC_DPP_DBAL_SW_TABLE_ID_IPV4MC_KBP_DEFAULT_ROUTE_KAPS, &table));
   ...
   )
{
    ...
    if(ARAD_KBP_ENABLE_IPV4_RPF_PUBLIC && SOC_IS_JERICHO(unit)){
        /*
         * At this stage DBAL will configure the program LTR with the public tables only.
         * Public and private tables are going to share the same result.
         * Current KBP search-to-result mapping is 1-to-1 and public and private tables cannot be both configured at this stage.
         * The private tables will be added to the program LTR at later stage in KBP init appl.
         */
   ...
   )
{
    ...
    /* custom route: DA LEM lookup table*/
...
/*
 * This program is a copy of the ipv4uc public program with the following changes:
 * private lookup to LEM(with vrf!=0) is replaced with the {port,vlan,mac} lookup,
 * and both the public and private lookups to KAPS are done with VRF == 0, 
 * Making sure hit is possible only in the public data base. (Let's check aborting private lookup).
 * Overall the following lookups are done: 
 * 1) Public lookup to KAPS.
 * 2) Public lookup to LEM (1st lookup).
 * 3) {port,vlan,mac} to LEM(1st lookup).
 */
uint32
   ...
   )
{
    ...
    if (ARAD_KBP_ENABLE_IPV6_RPF) {
        ...
        keys_to_table_id[nof_tables].sw_table_id = SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_MASTER_KBP;
        keys_to_table_id[nof_tables].lookup_number = 0;
        /* Align the CE allocation in IPv6 UC/IPv6 MC/IPv6 UC with RPF programs to save CEs for ACL, this will move CE24/25->CE28/29 */
        sal_memset(use_32_bit_ce, 0, sizeof(use_32_bit_ce));
        use_32_bit_ce[nof_tables][3] = 1; /* 16 bits SOC_PPC_FP_QUAL_HDR_FWD_IPV6_DIP_HIGH in MSB uses CE32 */
        use_32_bit_ce[nof_tables][9] = 1; /* 16 bits SOC_PPC_FP_QUAL_HDR_FWD_IPV6_SIP_HIGH in MSB uses CE32 */
   ...
   )
{
    ...
    if(prog_id == PROG_FLP_IPV6UC){
        /* IPV6 UC private only - The DIP is on KEY_A */
        keys_to_table_id[nof_tables].key_id = SOC_DPP_DBAL_PROGRAM_KEY_B;
    } else {
        /* IPV6 UC private + public program - The DIP is on KEY_B*/
        keys_to_table_id[nof_tables].key_id = SOC_DPP_DBAL_PROGRAM_KEY_A;
    }
   ...
   )
{
    ...
    if (SOC_IS_ARADPLUS_AND_BELOW(unit) || (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "l3_ipv6_uc_use_tcam", 0))) {
        ...
        if (ARAD_KBP_ENABLE_IPV6_UC) {
            ...
            /* Align the CE allocation in IPv6 UC/IPv6 MC/IPv6 UC with RPF programs to save CEs for ACL, this will move CE24/25->CE28/29 */
            sal_memset(use_32_bit_ce, 0, sizeof(use_32_bit_ce));
            use_32_bit_ce[num_of_tables][3] = 1; /* 16 bits SOC_PPC_FP_QUAL_HDR_FWD_IPV6_DIP_HIGH in MSB uses CE32 */
            use_32_bit_ce[num_of_tables][9] = 1; /* 16 bits SOC_PPC_FP_QUAL_HDR_FWD_IPV6_SIP_HIGH in MSB uses CE32 */
            use_32_bit_ce_ptr = use_32_bit_ce;
            keys_to_table_id[num_of_tables++].public_lpm_lookup_size = 0;
   ...
   )
{
...
#if defined(BCM_88675_A0) && defined(INCLUDE_KBP)
    {
        ...
        if (ARAD_KBP_ENABLE_IPV6_MC) {
            ...
            keys_to_table_id[nof_tables].lookup_number = 0;
            /* Align the CE allocation in IPv6 UC/IPv6 MC/IPv6 UC with RPF programs to save CEs for ACL, this will move CE25->CE23 */
            sal_memset(use_32_bit_ce, 0, sizeof(use_32_bit_ce));
            use_32_bit_ce[nof_tables][6] = 1; /* 16 bits first_qual_low in MSB uses CE32 */
            use_32_bit_ce_ptr = use_32_bit_ce;
            keys_to_table_id[nof_tables++].public_lpm_lookup_size = 0;
            ...
            /* Set the number of qualifiers of IPV6MC_KBP_DEFAULT_ROUTE_KAPS temporarily to 0
             * in order not to assign new CEs, but instead to reuse the CEs in KEY_D_MSB,
             * which will be created for the identical table IPV6UC_KBP_DEFAULT_ROUTE_KAPS
             */
        ...
        if (ARAD_KBP_ENABLE_IPV6_MC) {
            /* Set the number of qualifiers of IPV6MC_KBP_DEFAULT_ROUTE_KAPS to its original value */
            SOC_SAND_IF_ERR_EXIT(sw_state_access[unit].dpp.soc.arad.pp.dbal_info.dbal_tables.get(unit, SOC_DPP_DBAL_SW_TABLE_ID_IPV6MC_KBP_DEFAULT_ROUTE_KAPS, &table));
            table.nof_qualifiers = nof_qualifiers;
            SOC_SAND_IF_ERR_EXIT(sw_state_access[unit].dpp.soc.arad.pp.dbal_info.dbal_tables.set(unit, SOC_DPP_DBAL_SW_TABLE_ID_IPV6MC_KBP_DEFAULT_ROUTE_KAPS, &table));
        }
    ...
    arad_pp_flp_dbal_gpon_l2_ip4mc_tables_init(int unit, int prog_id)
{
    ...
    if (!is_table_initiated) {
        /* Create IPMC Source Specific Lookup Table key B = {FID,DIP[23:0],SIP} */
        nof_qual=0;
        DBAL_QUAL_INFO_CLEAR(&qual_info, SOC_PPC_FP_NOF_QUALS_PER_DB_MAX);
    ...
    arad_pp_flp_dbal_gpon_l2_ip6mc_tables_init(int unit, int prog_id)
{
    ...
    /* Create IPMC Source Specific Lookup Table key B = {FID,SIP}, where SIP is represented by MAC SA for EUI-64 encoded SIP) */
    DBAL_QUAL_INFO_CLEAR(&qual_info, SOC_PPC_FP_NOF_QUALS_PER_DB_MAX);
