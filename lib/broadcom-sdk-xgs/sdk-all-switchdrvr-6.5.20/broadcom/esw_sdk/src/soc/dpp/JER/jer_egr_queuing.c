    ...
    if((master_port == port) || SOC_PORT_IS_LB_MODEM(flags)) { /* First port in interface */
         ...
         switch(interface_type) {
            ...
            case SOC_PORT_IF_ILKN:
                ...
                {
                    /* ILKN oversubscribe feature is enabled only for 0 & 16 ILKN interfaces. Here we are making sure that first 2 ILKN will get these numbers */
...
{   
    ...
    SOC_PBMP_ITER(pbmp, port_i) {
        ...
        SOCDNX_IF_ERR_EXIT(soc_jer_port_to_interface_map_set(unit, port_i, 0 /* unmap */, 0 /* is_dynamic_port */));
    ...
    /* ILKN over subscribe feature is to enable ILKN transmit skip while the ILKN at almost full status */
  ...
  )
{
    ...
    */
    ...
    /**
     * For the dummy entries in the FQP MUX there are two options - 
     * to fill them with invalid values, or to give them to the Special interfaces (CPU, OLP, OAMP, Delete FIFO). 
     * It is up to user to decide, both configurations might have implications in some corner cases.
     */
    if(soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "enhanced_egress_calendar_config", 0) == TRUE) 
    {
        /** Fill the dummy entries with invalid values */
        fqp_padding_value = SOC_JER_FQP_CAL_IF_INVALID;
    }
    else
    {
        /** Fill the dummy entries with special IFs */
        fqp_padding_value = SOC_JER_FQP_CAL_IF_OTHERS_GIVE_BW_IF_NEEDED;
    }

    /** Fill the dummy entries with the appropriate value */
