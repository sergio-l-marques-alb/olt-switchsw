/* $Id: arad_pp_llp_parse.c,v 1.27 Broadcom SDK $
...
{ 
  ...
  /* OAMP tpid map set - syncronized with ARAD tpid values */
...
{ 
  ...
  if (tpid_profile_ndx <= ARAD_PP_LLP_PARSE_TPID_PROFILE_ID_MAX)
  {
    ...
    if (mac_in_mac_enabled && (mim_tpid == 0)) {
        /* check if this is the Mac-in-Mac TPID profile (if points at B-tag TPID) */
        ...
        if (tpid_vals.tpid_vals[tpid_profile_info->tpid1.index] == ARAD_PP_LLP_PARSE_MIM_BTAG_TPID) { 
            /* points at B-tag tpid - write down Mac-in-Mac TPID */
  ...
  /*else
  {
    lsb = (tpid_profile_ndx - ARAD_PP_LLP_PARSE_TPID_PROFILE_ID_MAX-1)*2;
    msb = (tpid_profile_ndx - ARAD_PP_LLP_PARSE_TPID_PROFILE_ID_MAX-1)*2 + 1;*/

    /* else tpid-profile > 3 and this profile for egress parsing only */
    /* TPID-0*/
    /*SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  50,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_read(unit, ECI_GLOBAL_PP_4r, REG_PORT_ANY, 0, TPID_PROFILE_OUTER_INDEXf, &tmp));
    res = soc_sand_set_field(&tmp, msb, lsb, tpid_profile_info->tpid1.index);
    SOC_SAND_CHECK_FUNC_RESULT(res, 22, exit);
    SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  65,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, ECI_TPID_PROFILE_CONFIGURATION_REGr, REG_PORT_ANY, 0, TPID_PROFILE_OUTER_INDEX1f,  tmp));*/
    /* TPID-1*/
    /*SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  70,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_read(unit, ECI_TPID_PROFILE_CONFIGURATION_REGr, REG_PORT_ANY, 0, TPID_PROFILE_INNER_INDEX1f, &tmp));
    res = soc_sand_set_field(&tmp, msb, lsb, tpid_profile_info->tpid2.index);
    SOC_SAND_CHECK_FUNC_RESULT(res, 27, exit);
    SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  85,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, ECI_TPID_PROFILE_CONFIGURATION_REGr, REG_PORT_ANY, 0, TPID_PROFILE_INNER_INDEX1f,  tmp));
  }*/
  ...
  )
{
  ...
  /*else
  {
    ...
    /* else tpid-profile > 3 and this profile for egress parsing only */
    /*SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  50,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_read(unit, ECI_TPID_PROFILE_CONFIGURATION_REGr, REG_PORT_ANY, 0, TPID_PROFILE_OUTER_INDEX1f, &tmp));
    tpid_profile_info->tpid1.index = (uint8)SOC_SAND_GET_BITS_RANGE(tmp, msb, lsb);
    SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  70,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_read(unit, ECI_TPID_PROFILE_CONFIGURATION_REGr, REG_PORT_ANY, 0, TPID_PROFILE_INNER_INDEX1f, &tmp));
    tpid_profile_info->tpid2.index = (uint8)SOC_SAND_GET_BITS_RANGE(tmp, msb, lsb);
  }*/
  ...
  )
{
  ...
  /* 
   * Special case: if packet tag format is S-tag and initial_inner_tag is set 
   * then incoming_vid does not exist.
   * Its needed to invoke Initial-VID on the C-tag instead of on the S-tag
   */
  ...
  )
{
  ...
  /* map parsing key into index in the EPNI LLVP table */
  ...
  /* Reset the LLVP fields and set only those that require setting */
  ...
  )
{
  ...
  /* in advance mode priority_tag_type is not in use */
  if(!ARAD_PP_LLP_PARSE_ADVANCE_MODE(unit)){
  ...
  )
{
  ...
  /*
   * Data is the same for all PLC, so we just just take one
   */
  res = arad_pp_ihp_vtt_llvp_tbl_get_unsafe(
          unit,
  ...
  /*
   * Update flags with SOC_PPC_LLP_PARSE_FLAGS_OUTER_C. 
   * See arad_pp_llp_parse_packet_format_info_set_unsafe
   */
  ...
  )
{
  ...
  /* Retrieve the flags - ARAD_PP_LLP_PARSE_FLAGS_XXX.
     Unfortunately, the information resides only at the Egress. */
  format_info->flags = 0;
  if (ARAD_PP_LLP_PARSE_ADVANCE_MODE(unit)) {
    ...
    /* The flags are available only for C-TAG. The C-TAG offset determines
    if it's an Outer or Inner C-TAG. */
