...
soc_jer_core_clock_verification(int unit)
{
     ...
     /*initialize with the values for jer and jer+*/
     uint32 ndiv_offset=0, ndiv_num_of_bits=8, mdiv_offset=8, mdiv_num_of_bits=4;
     SOCDNX_INIT_FUNC_DEFS;
     /*
      * mdiv is zero in emulation boot
      */
     if (! (SAL_BOOT_PLISIM || soc_sand_is_emulation_system(unit)))
     {
        ...
        /* read the register - all bits! is cpu_override?*/
        SOCDNX_IF_ERR_EXIT(READ_ECI_OGER_1000r(unit, reg_val));
        /* in case there is field: return only the field from the var reg_val_long which holds the whole register:*/
        cpu_override=soc_reg_field_get(unit, ECI_OGER_1000r, reg_val[0], FIELD_0_0f);

        if (!cpu_override) /* cpu override mode is not supported */
        {
            /* read the register - all bits!*/
