...
int soc_dpp_drc_combo28_mr_register_info_prepare(int unit, soc_dpp_drc_combo28_info_t *drc_info)
{
    ...
    SHR_BIT_ITER(drc_info->dram_bitmap, max_nof_dram_interfaces, drc_ndx) {
        ...
        case SHMOO_COMBO28_DRAM_TYPE_DDR4:
            ...
            drc_info->mr[drc_ndx][0] |= SOC_SAND_SET_BITS_RANGE(SOC_SAND_GET_BITS_RANGE(cas_latency_calc,4,4), 12, 12);  /* CAS Latency */
            drc_info->mr[drc_ndx][0] |= SOC_SAND_SET_BITS_RANGE(drc_info->dram_int_param.wr_recovery_op, 11, 9);         /* Write recovery */
            drc_info->mr[drc_ndx][0] |= SOC_SAND_SET_BITS_RANGE(0x1, 8, 8);                                              /* Reset DLL. 0 = NO 1 = Yes */
            drc_info->mr[drc_ndx][0] |= SOC_SAND_SET_BITS_RANGE(0x0, 7, 7);                                              /* MR0_test_mode. 0 = Normal 1 = Test */
            drc_info->mr[drc_ndx][0] |= SOC_SAND_SET_BITS_RANGE(SOC_SAND_GET_BITS_RANGE(cas_latency_calc,3,1), 6, 4);    /* CAS Latency */
            drc_info->mr[drc_ndx][0] |= SOC_SAND_SET_BITS_RANGE(0x0, 3, 3);                                              /* Read burst type. 0 = Sequential 1 = Interleave */
            drc_info->mr[drc_ndx][0] |= SOC_SAND_SET_BITS_RANGE(SOC_SAND_GET_BITS_RANGE(cas_latency_calc,0,0), 2, 2);    /* CAS Latency */
            drc_info->mr[drc_ndx][0] |= SOC_SAND_SET_BITS_RANGE(mrs_brst_lng, 1, 0);                                     /* Burst length. 00=8(Fixed), 01=BC4 or 8(on the fly), 10=BC4(Fixed), 11=Reserved */

            /* MR1 */
            drc_info->mr[drc_ndx][1]  = 0x0;
            drc_info->mr[drc_ndx][1] |= SOC_SAND_SET_BITS_RANGE(0x0, 12, 12);                         /* Qoff, Output buffer. 0 = enabled, 1 = disabled */
            drc_info->mr[drc_ndx][1] |= SOC_SAND_SET_BITS_RANGE(0x0, 11, 11);                         /* TDQS. 0 = Disable 1 = Enable */
            drc_info->mr[drc_ndx][1] |= SOC_SAND_SET_BITS_RANGE(0x0, 10, 8);                          /* RTT_NOM. 0x0 - Off*/
            drc_info->mr[drc_ndx][1] |= SOC_SAND_SET_BITS_RANGE(0x0, 7, 7);                           /* Write Leveling. 0 = Disable 1 = Enable */
            drc_info->mr[drc_ndx][1] |= SOC_SAND_SET_BITS_RANGE(drc_info->dram_int_param.c_al, 4, 3); /* Additive Latency. 00=0(AL disabled), 10=CL-2, 01=CL-1, 11=Resrved */
            drc_info->mr[drc_ndx][1] |= SOC_SAND_SET_BITS_RANGE(ods, 2, 1);                           /* Output Driver Impedance Control */
            drc_info->mr[drc_ndx][1] |= SOC_SAND_SET_BITS_RANGE(0x1, 0, 0);                           /* DLL. 0 = Disable 1 = Enable. */

            /* MR2 */
            drc_info->mr[drc_ndx][2]  = 0x0;
            drc_info->mr[drc_ndx][2] |= SOC_SAND_SET_BITS_RANGE(write_crc, 12, 12); /* Write CRC. 0 = Disable 1 = Enable */
            drc_info->mr[drc_ndx][2] |= SOC_SAND_SET_BITS_RANGE(0x0, 10, 9);                                 /* RTT_WR. 0 - disabled, 1 - RZQ/2 = 120Ohm, 2 - RZQ/4 =60Ohm */
            drc_info->mr[drc_ndx][2] |= SOC_SAND_SET_BITS_RANGE(0x0, 7, 6);                                  /* Low Power Array Self Refresh (LP ASR). 00=Manual Mode (Normal Operaing Temperature Range) */
            drc_info->mr[drc_ndx][2] |= SOC_SAND_SET_BITS_RANGE(wr_latency_calc, 5, 3);                      /* CAS Write Latency(CWL) */

            /* MR3 */
            drc_info->mr[drc_ndx][3]  = 0x0; 
            drc_info->mr[drc_ndx][3] |= SOC_SAND_SET_BITS_RANGE(0x1, 12, 12);               
            drc_info->mr[drc_ndx][3] |= SOC_SAND_SET_BITS_RANGE(0x0, 8, 6);                      /* Fine Granularity Refresh Mode. 0 - Normal */
            drc_info->mr[drc_ndx][3] |= SOC_SAND_SET_BITS_RANGE(0x1, 5, 5); 
            drc_info->mr[drc_ndx][3] |= SOC_SAND_SET_BITS_RANGE(drc_info->gear_down_mode, 3, 3); /* Geardown Mode. 0 = 1/2 Rate 1 = 1/4 Rate */

            /* MR4 */
            drc_info->mr[drc_ndx][4]  = 0x0;
            drc_info->mr[drc_ndx][4] |= SOC_SAND_SET_BITS_RANGE(SOC_SAND_NUM2BOOL_INVERSE(drc_info->dram_int_param.dqs_prem_1_ck), 12, 12); /* Write Preamble. 0 = 1 nCK 1 = 2 nCK */
            drc_info->mr[drc_ndx][4] |= SOC_SAND_SET_BITS_RANGE(SOC_SAND_NUM2BOOL_INVERSE(drc_info->dram_int_param.dqs_prem_1_ck), 11, 11); /* Read  Preamble. 0 = 1 nCK 1 = 2 nCK */

            /* MR5 */
            drc_info->mr[drc_ndx][5]  = 0x0;
            drc_info->mr[drc_ndx][5] |= SOC_SAND_SET_BITS_RANGE(drc_info->read_dbi, 12, 12);  /* Read DBI. 0 = Disable 1 = Enable */
            drc_info->mr[drc_ndx][5] |= SOC_SAND_SET_BITS_RANGE(drc_info->write_dbi, 11, 11); /* Write DBI. 0 = Disable 1 = Enable */
            drc_info->mr[drc_ndx][5] |= SOC_SAND_SET_BITS_RANGE(0x1, 8, 6);                   /* RTT_Park. ZQ/4 */
            drc_info->mr[drc_ndx][5] |= SOC_SAND_SET_BITS_RANGE(cmd_par_latency_calc, 2, 0);  /* C/A Parity Latency Mode */

            /* MR6 */
            drc_info->mr[drc_ndx][6]  = 0x0;
            drc_info->mr[drc_ndx][6] |= SOC_SAND_SET_BITS_RANGE(ccd_l_calc, 12, 10); /* tCCD_L */
        ...
        case SHMOO_COMBO28_DRAM_TYPE_GDDR5:
            ...
            /* MR0 */
            drc_info->mr[drc_ndx][0]  = 0x0;
            drc_info->mr[drc_ndx][0] |= SOC_SAND_SET_BITS_RANGE(drc_info->dram_int_param.wr_recovery_op, 11, 8); /* Write recovery */
            drc_info->mr[drc_ndx][0] |= SOC_SAND_SET_BITS_RANGE(cas_latency_calc, 6, 3);                         /* CAS Latency */
            drc_info->mr[drc_ndx][0] |= SOC_SAND_SET_BITS_RANGE(wr_latency_calc, 2, 0);                          /* CAS Write Latency(CWL) */

            /* MR1 */
            drc_info->mr[drc_ndx][1]  = 0x0;
            drc_info->mr[drc_ndx][1] |= SOC_SAND_SET_BITS_RANGE(0, 11, 11);                                            /* PLL Reset */
            drc_info->mr[drc_ndx][1] |= SOC_SAND_SET_BITS_RANGE(SOC_SAND_NUM2BOOL_INVERSE(drc_info->abi), 10, 10);     /* Write Preamble. 0 = 1 nCK 1 = 2 nCK */
            drc_info->mr[drc_ndx][1] |= SOC_SAND_SET_BITS_RANGE(SOC_SAND_NUM2BOOL_INVERSE(drc_info->write_dbi), 9, 9); /* Write DBI. */
            drc_info->mr[drc_ndx][1] |= SOC_SAND_SET_BITS_RANGE(SOC_SAND_NUM2BOOL_INVERSE(drc_info->read_dbi), 8, 8);  /* Read DBI. */
            drc_info->mr[drc_ndx][1] |= SOC_SAND_SET_BITS_RANGE(0x0, 7, 7);                                            /* PLL Enable */
            drc_info->mr[drc_ndx][1] |= SOC_SAND_SET_BITS_RANGE(SOC_SAND_NUM2BOOL(drc_info->dram_auto_calibration_update_disable), 6, 6);  /* calibration update. 1 - disable 0 - enabe */
            drc_info->mr[drc_ndx][1] |= SOC_SAND_SET_BITS_RANGE(0x1, 5, 4);                                              /* ADDR/CMD termination. 0 CKE on reset, 1 ZQ , 2 ZQ/2 , Vendor */
            drc_info->mr[drc_ndx][1] |= SOC_SAND_SET_BITS_RANGE(0x1, 3, 2);                                              /* DATA termination. 0 disable, 1 ZQ/2 , 2 ZQ */
            drc_info->mr[drc_ndx][1] |= SOC_SAND_SET_BITS_RANGE(0x0, 1, 0);                                              /* output drive strength 0 auto calib. 1 vendor, 2 nominal */
            ...
            /* MR3 */
            drc_info->mr[drc_ndx][3]  = 0x0;
            drc_info->mr[drc_ndx][3] |= SOC_SAND_SET_BITS_RANGE(SOC_SAND_GET_BITS_RANGE(ccd_l_calc, 1, 0), 11, 10); /* tCCD_L */
            drc_info->mr[drc_ndx][3] |= SOC_SAND_SET_BITS_RANGE(0x1, 9, 8);                                         /* WCK Termination. 0 disable, 1 ZQ/2, 2 ZQ */
            drc_info->mr[drc_ndx][3] |= SOC_SAND_SET_BITS_RANGE(0, 5, 5);                                           /* Read DQS Mode. 0 disable */
            drc_info->mr[drc_ndx][3] |= SOC_SAND_SET_BITS_RANGE(0, 4, 4);                                           /* WCK2CK Training. 0 disable */

            /* MR4 */
            drc_info->mr[drc_ndx][4]  = 0x0;
            drc_info->mr[drc_ndx][4] |= SOC_SAND_SET_BITS_RANGE(0, 11, 11);                                                             /* EDC Hold Pattern Invert for EDC1 + EDC3. 0 - do not invert one byte of edc */
            drc_info->mr[drc_ndx][4] |= SOC_SAND_SET_BITS_RANGE(SOC_SAND_NUM2BOOL_INVERSE(write_crc), 10, 10); /* Write DBI. */
            drc_info->mr[drc_ndx][4] |= SOC_SAND_SET_BITS_RANGE(SOC_SAND_NUM2BOOL_INVERSE(drc_info->read_crc), 9, 9);    /* Read DBI. */
            drc_info->mr[drc_ndx][4] |= SOC_SAND_SET_BITS_RANGE(crc_rd_latency_calc, 8, 7);                                             /* CRC Read Latency (CRCRL) */
            drc_info->mr[drc_ndx][4] |= SOC_SAND_SET_BITS_RANGE(crc_wr_latency_calc, 6, 4);                                             /* CRC Write Latency (CRCWL) */
            drc_info->mr[drc_ndx][4] |= SOC_SAND_SET_BITS_RANGE(0xf, 3, 0);                                                             /* EDC Hold Pattern */

            /* MR5 */
            drc_info->mr[drc_ndx][5]  = 0x0;
            if(drc_info->t_ras_enable){
                drc_info->mr[drc_ndx][5] |= SOC_SAND_SET_BITS_RANGE(ras_calc, 11, 6);  /* when the ext_ram_t_ras_enable is 1 then the driver will use ext_ram_t_ras value other it will behave as before. */
            }
            drc_info->mr[drc_ndx][5] |= SOC_SAND_SET_BITS_RANGE(0, 2, 2);          /* When bit A2 (LP3) is set, RDTR, WRTR and LDFF commands are not allowed while a REF command is being executed */
            drc_info->mr[drc_ndx][5] |= SOC_SAND_SET_BITS_RANGE(0, 1, 1);          /* When bit A1 (LP2) is set, the WCK receivers may be turned off during power-down */
            drc_info->mr[drc_ndx][5] |= SOC_SAND_SET_BITS_RANGE(0, 0, 0);          /* When bit A0 (LP1) is set, several GDDR5M SGRAM core parameters are relaxed for lower power consumption of the device. LP1 is optional. */

            /* MR6 */
            drc_info->mr[drc_ndx][6]  = 0x0;
            drc_info->mr[drc_ndx][6] |= SOC_SAND_SET_BITS_RANGE(5, 11, 8); /* vref offset A11:A8, VREFD OffsetBytes in rows A-F */
            drc_info->mr[drc_ndx][6] |= SOC_SAND_SET_BITS_RANGE(5, 7, 4);  /* vref offset A7:A4,  VREFD OffsetBytes in rows M-U */
            drc_info->mr[drc_ndx][6] |= SOC_SAND_SET_BITS_RANGE(1, 3, 3);  /* vref offset A7:A4,  VREFD OffsetBytes in rows M-U */
            drc_info->mr[drc_ndx][6] |= SOC_SAND_SET_BITS_RANGE(0, 0, 0);  /* A0WCK2CK Alignment Pt. 0 - PD inside DRAM, 1 - PD at pins */

            /* MR7 */
            drc_info->mr[drc_ndx][7]  = 0x0;
            drc_info->mr[drc_ndx][7] |= SOC_SAND_SET_BITS_RANGE(0, 11, 10); /* Duty Cycle Correction (DCC) */
            drc_info->mr[drc_ndx][7] |= SOC_SAND_SET_BITS_RANGE(0, 9, 8);   /* VDD Range */
            drc_info->mr[drc_ndx][7] |= SOC_SAND_SET_BITS_RANGE(0, 7, 7);   /* Half VFRED */
            drc_info->mr[drc_ndx][7] |= SOC_SAND_SET_BITS_RANGE(0, 6, 6);   /* Temperature Sensor */
            drc_info->mr[drc_ndx][7] |= SOC_SAND_SET_BITS_RANGE(1, 5, 5);   /* DQ Preamble */
            drc_info->mr[drc_ndx][7] |= SOC_SAND_SET_BITS_RANGE(0, 4, 4);   /* WCK2CK Auto Sync */
            drc_info->mr[drc_ndx][7] |= SOC_SAND_SET_BITS_RANGE(0, 3, 3);   /* Low Frequency Mode */

            /* MR8 */
            drc_info->mr[drc_ndx][8]  = 0x0;
            drc_info->mr[drc_ndx][8] |= SOC_SAND_SET_BITS_RANGE(SOC_SAND_GET_BITS_RANGE(drc_info->dram_int_param.wr_recovery_op ,4,4), 1, 1); /* WRITE Recovery MSB - Extra High Frequency */
            drc_info->mr[drc_ndx][8] |= SOC_SAND_SET_BITS_RANGE(SOC_SAND_GET_BITS_RANGE(cas_latency_calc,4,4), 0, 0);                         /* CAS Latency MSB - Extra High Frequency */
            ...
            /* MR15 */
            drc_info->mr[drc_ndx][15]  = 0x0;
            drc_info->mr[drc_ndx][15] |= SOC_SAND_SET_BITS_RANGE(0, 10, 10); /* Address Training (ADT). 0 - Off, 1 - On */
            drc_info->mr[drc_ndx][15] |= SOC_SAND_SET_BITS_RANGE(0, 9, 9);   /* MR0-14 Enable MF=1. 0 - Enabled, 1 - Disabled */
            drc_info->mr[drc_ndx][15] |= SOC_SAND_SET_BITS_RANGE(0, 8, 8);   /* MR0-14 Enable MF=0. 0 - Enabled, 1 - Disabled */
