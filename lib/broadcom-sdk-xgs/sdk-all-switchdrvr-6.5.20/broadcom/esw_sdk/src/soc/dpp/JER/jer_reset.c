...
/* when stopping traffic in soft reset process - we configure this low drop threshold in FDA fifos
 * to avoid them from filling-up and sending flow control to FDR */
...
int soc_jer_reset_blocks_poll_init_finish(int unit)
{
    ...
    if (SOC_IS_FLAIR(unit)) {
        /* Flair don't need reset traffic Igress/Egress */
    }
        ...
        /* Per-core status polling, same as JER */
        ...
        /* When we reach here it is not JR+ */
...
STATIC int jer_sch_reset(int unit)
{
    ...
    for (i = 0; i < sch_nof_instances; i++) {
        /* decided not to stop DVS */

        /*
         * Note: DISABLE_FABRIC_MSGSf, FORCE_PAUSEf, DLM_ENABLEf taken care in jer_mgmt_enable_traffic_set_unsafe()
         *       jer_mgmt_enable_traffic_set_unsafe() is called before and after jer_reset()
         */
    ...
    SOC_SAND_IN  ARAD_DBG_RST_DOMAIN    rst_domain)
{
    ...
    /*
     * TDM reassemble contexts should be disabled prior to traffic disable
     */
    if (!SOC_IS_QUX(unit))
    {
    ...
    /**************************************************************************/
    /**  QAX: Avoid QMLF flow control - flow control isn't                    */
    /**  restored well after soft reset action                                */
    /**************************************************************************/
    if (SOC_IS_QUX(unit)) {
    ...
    if (is_ingr) 
    {
        ...
        /* Put order dependent blocks into reset first */
    ...
    if (is_fabric) {
        ...
        if (!SOC_IS_QAX(unit)) { /*Jericho, QMX, Jericho plus. For QMX- set 9 FMACs but uses only 4 of them*/
    ...
    if (is_ingr)
    {
        ...
        if (is_fabric) 
        {
            ...
            soc_reg_above_64_field_set(unit, ECI_BLOCKS_SOFT_INITr, soft_init_reg_val, BLOCKS_SOFT_INIT_33f, fld_above64_val);/* FDA Block ID   */
        }
        SOCDNX_IF_ERR_EXIT(WRITE_ECI_BLOCKS_SOFT_INITr(unit, soft_init_reg_val));
    ...
    if (is_egr) 
    {
        ...
        /* Bring order dependent blocks out of reset first */
    ...
    /* Wait between in-reset and out-of-reset */
    sal_usleep(500);
    ...
    if (is_ingr) 
    {
        /* Run order dependent blocks init first */
        for (field_index=0; field_index<ingress_order_dependent_field_list_size; field_index++)
        {
            soc_reg_above_64_field_set(unit, ECI_BLOCKS_SOFT_INITr, soft_init_reg_val, ingress_order_dependent_field_list[field_index], fld_above64_val);
            SOCDNX_IF_ERR_EXIT(WRITE_ECI_BLOCKS_SOFT_INITr(unit, soft_init_reg_val));
    ...
    if(is_egr) 
    {
        /* Run order dependent reset first */
        for (field_index=0; field_index<egress_order_dependent_field_list_size; field_index++) {
        ...
        if (SOC_IS_QUX(unit) && SOC_DPP_CONFIG(unit)->qax->link_bonding_enable) {
            /* Reset Link Bonding TXI credits */
    ...
    /*
     * Set TDM reassemble context to original value
     */
    if (!SOC_IS_QUX(unit))
    {
