  ...
  if (is_for_update && insertion_order_respected) {
      ...
      do
      {
        ...
        if (!is_indices_enabled)
        {
           ...
           * criterion depends on 'insertion_order_respected')
           */
          while (!priority_found && (current != SOC_SAND_SORTED_LIST_ITER_BEGIN(unit,list))) 
          {
             ...
             *  If 'insertion_order_respected' then
        ...
        else
        {
          ...
          {
            /*
             * Highest element on list is lower (or equal if 'insertion_order_respected')
          ...
          while (!priority_found)
          {
            ...
            if (index_on_indices == 0)
            {
              ...
              {
                /*
                 * The higher of the two elements on list is lower (or equal if 'insertion_order_respected')
                 * than input priority. Search has ended.
                 */
                priority_found = TRUE ;
                break ;
              ...
              {
                /*
                 * The lower of the two elements on list is lower (or equal if 'insertion_order_respected')
                 * than input priority. Search has ended.
                 */
                priority_found = TRUE ;
                break ;
            ...
            {
              /*
               * Found element which has equal priority and 'insertion_order_respected' is NOT set.
            ...
            /*
             * Current element either does NOT have equal priority or 'insertion_order_respected'
             * is set.
             */
...
arad_pp_ihb_tcam_action_tbl_cache_after_move_update(
    ...
    */
    ...
    /*
    * Be consistent with hardware move -
    *     Hardware moves two action tables or four action tables, depends on move_couple.
    */
...
uint32
  arad_tcam_db_entry_remove_unsafe(
    ...
    SOC_SAND_IN  uint32               tcam_db_id,
    SOC_SAND_IN  uint32               entry_id,
    SOC_SAND_IN  uint8               is_keep_location   /* If TRUE, harware location is kept */
  )
{
  uint32
      bank_nof_entries,
  ...
  if (found)
  {
    ...
    if (!is_keep_location) {    /* entry in the bank is kept in keep_location */
        bank_nof_entries --;
    }
