...
/**
 * This function returns the bank owner of the given bank_ndx as defined
 * by the soc_property "tcam_bank_block_owner".
 */
STATIC
  uint32
    ...
    )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(0);
    ...
    *success = FALSE;
    ...
    {
      /* We're only looking for reserved banks */
      ARAD_DO_NOTHING_AND_EXIT;
    }
    if (block_owner_found && bank_block_owner != bank_owner)
    {
      /* Exit with success=false since this bank is reserved for another block */
      ARAD_DO_NOTHING_AND_EXIT;
    }
    ...
    )
{
  ...
  {
    /* In the first iteration we try to allocate reserved banks only */
  ...
  )
{
  ...
  if (bank_found)
  {
      /* When single bank mode, key A is allocated which in turn works only with banks 0-7
       * (HW Limitation for <ARAD+)
       */
...
/*********************************************************************
*     Gets an entry from TCAM database, and allocates more and parse 
...
/*********************************************************************
*     Delete an entry from a TCAM database for keep location mode.
 *     Details: in the H file. (search for prototype)
*********************************************************************/
uint32
  ...
  )
{
  ...
  res = arad_tcam_db_entry_remove_unsafe(
          unit,
          FALSE /* do not free tcam bank in keep_location mode */,
          tcam_db_id,
          entry_id,
          TRUE
        );
