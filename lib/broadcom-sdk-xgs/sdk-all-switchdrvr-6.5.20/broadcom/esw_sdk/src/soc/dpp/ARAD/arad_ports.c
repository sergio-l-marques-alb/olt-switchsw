  ...
  )
{
  ...
  SOC_DPP_CORES_ITER(SOC_CORE_ALL, core) {
      for (pp_port = 0; pp_port < MAX_PORTS_IN_CORE ; ++pp_port)
      {
          ...
          if (PBMP_MEMBER(pon_port_bm,port)) {
              /* GPON port msb are configured in first 2 bytes after start of header */
  ...
  if (SOC_DPP_CONFIG(unit)->pp.pon_application_enable || is_pon_dpp_support) {
    if (if_pon_pp_port_map_bypass == 0) {
        ...
        if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "gpon_application_enable", 0)) {
            for (fem_bit_ndx = 0; fem_bit_ndx < SOC_DPP_DEFS_GET(unit, virtual_port_nof_bits); ++fem_bit_ndx) {
               /*
                * GPON Action is defined for Incoming PON PTC as follows:
                * 32 fem bits: 16bits msb are from header that include Tunnel-ID and 16bits lsb that include PTC
                * Action is 14bits.
                * Action should be:
		*  Arad+:
                *   Action[2:0] from PTC[2:0]
                *   Action[4:3] from Tunnel-ID from header
                *   Action[13:5] set to zero
		* Jericho:
                *   Action[3:0] from PTC[3:0]
                *   Action[5:4] from Tunnel-ID from header
                *   Action[13:6] set to zero
                */
                virtual_port_fem_field_select_map.virtual_port_fem_field_select[fem_bit_ndx] =
            ...
            case ARAD_PORT_HEADER_TYPE_ETH:
...
#endif
                {
                    if(soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "user_header_always_remove", 0) == 1) {
						...
						/* ARAD_PRGE_TM_SELECT_ETH_REMOVE_SYSTEM_HEADERS TM profile should be ORed with other TM profiles */
    ...
    )
{
    ...
    /* resolve irr */
    /* if removed member has disable flag it means that it was already removed from irr (TM) related tables,
    ...
    if( ! removed_member_has_disable_flag )
    {
        ...
        /* those member indexes are in referrence to the IRR HW and not to the trunk_sw_db */
        ...
        {
            /** short sleep to allow pipeline to clear any packets which are on the way before changing the mapping to invalid */
            sal_usleep(10);
            /* empty invalid entry */
    ...
    /* If need to full remove and NOT to EGRESS_DISABLE */
    if (need_to_disable_not_to_full_remove == 0)
    {
        /* if only 1 replication - pp part of removed port need to be removed from HW tables */
        if (nof_replications == 1) 
        {
        ...
        /* if a member is removed - it is possible that another member was moved, and it is possible that
           its sspa and dspa need to be realigned */
        /* Re-arrange values for moved members */
        /* check that moved and removed member are not same member */
        /* those member indexes are in referrence to the trunk_sw_db and not to the IRR HW */
        /* the sspa and dspa are aligned to indexes in the trunk_sw_db, the reason is that they are relevant also for disabled members */
    ...
    )
{
    ...
    /* clear the all members of group in LAG_MAPPING table first */
...
uint32 alloc_reassembly_context_and_recycle_channel_unsafe(int unit, int core_id, uint32 out_pp_port, uint32 *channel, uint32 *reassembly_context)
{
  ...
  *channel = 0;
  ...
  /* Replace header type to RAW, in order to avoid wrong header handling */
            ...
            if ((info->value.pp_port_ext.index>>(ARAD_PORTS_FEM_PP_PORT_VENDOR2_FIELD_1_MSB+1)) < ARAD_PORTS_FEM_PP_PORT_VENDOR2_FIELD_2_UC_CTRL_TYPE) {
               addtional_remove = 1;
                 ...
                 && ((((info->value.pp_port_ext.index>>(ARAD_PORTS_FEM_PP_PORT_VENDOR2_FIELD_0_MSB+1))&
                    ((0x1L<<ARAD_PORTS_FEM_PP_PORT_VENDOR2_FIELD_1_SIZE)-1)) >= ARAD_PORTS_FEM_PP_PORT_VENDOR2_FIELD_1_SUB_TYPE))) {/*L3 VPN MC*/
   ...
   )
{
    ...
    /* Only support dynamic switching for CoE port and 802.1br and vn tag port termination */
    ...
    /* Check for extender_map_lc_exists == 2 
     * vlaue = 0: COE port disable
     * value = 1: COE port enable + TC/DP disable
     * value = 2: invalid parametre
     * vlaue = 3: COE port enable + TC/DP enable
     */
   ...
   )
{
    ...
    *value = 0;
    ...
    if ((SOC_DPP_CONFIG(unit))->pp.port_extender_map_lc_exists) {
        ...
        /* for 802.1br and vn tag port termination swap is enabled */
   ...
   )
{
    ...
    soc_pbmp_t pon_port_bm; /* pon port < 16 */
    /*uint32 tbl_data_64b[2] = {0};*/
    int core;
    uint32 count;
    uint32 res;
    ...
    /* Only support Jericho and above */
    ...
    if (value) {
        /* Can't co-exist with following feature */
        ...
        /* PON global configuration */
        if (pon_app_status_is_change) {
            SOC_DPP_CONFIG(unit)->pp.pon_application_enable = 1;            
        }
        PORT_SW_DB_PORT_ADD(pon, port);

        /* PON per port setting */
        /*SOCDNX_IF_ERR_EXIT(soc_port_sw_db_port_validate_and_add(unit, core, port, channel, SOC_PORT_FLAGS_PON_INTERFACE, port_interface, phy_ports));*/
        /*SOCDNX_IF_ERR_EXIT(soc_port_sw_db_port_add(unit, core, port, channel, SOC_PORT_FLAGS_PON_INTERFACE, port_interface, phy_ports));*/
    ...
    */
    ...
    /* setting byte to remove for every pp port that will mapped */
    ...
    /* setting profile for VT and FLP program */
   ...
   )
{
    ...
    /*
    if (SOC_DPP_CONFIG(unit)->pp.pon_application_enable == 0) {
        SOCDNX_EXIT_WITH_ERR(SOC_E_FAIL, (_BSL_SOCDNX_MSG("PON application is not enable")));
    }
    */
...
/* get recycle channel from mirror port*/
int
   ...
   )
{
    ...
    /*get queue pair from logical port */
