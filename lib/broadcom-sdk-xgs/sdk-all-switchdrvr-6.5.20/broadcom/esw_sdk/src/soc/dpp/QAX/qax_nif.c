...
int soc_qax_ilkn_pml_override_clk_data_mux(int unit, pbmp_t *pbmp)
{
    ...
    /*
         * If there are QSGMII ports on one PM, the data path clock should be from core clock.
         * Otherwise the data patch clock is from Pll under 325MHz.
         */
...
STATIC int soc_qax_nif_priority_ilkn_high_low_set(int unit, int core, int is_high, uint32 ilkn_id) {
    ...
    /* The upper function for QAX will never pass value bigger than 3 for ilkn_id*/
...
STATIC int soc_qax_nif_priority_quad_high_low_set(int unit, int core, int is_high, uint32 quad) {
    ...
    if (is_high) {
        /* QUX LOW nif priority register is 64 bits 
           and it repeats itself every 16 bits*/
        if (SOC_IS_QUX(unit)) {
...
soc_qax_nif_ilkn_pbmp_from_num_lanes_get(int unit ,uint32 num_lanes, uint32 first_phy, uint32 ilkn_id, soc_pbmp_t* pbmp)
{
    ...
    /* If first_phy is given as a flag SOC_QAX_FIRST_ILKN_PHY(-1) we start from the first relevant phy*/
    if (first_phy == SOC_QAX_FIRST_ILKN_PHY) {
        ilkn_wrap_pm = ((ilkn_id & 1) == 0) ? 0 : SOC_QAX_MAX_PMS_PER_ILKN_PORT/2; /* First ILKN PM */
    } else {
        /* If function is called with specific first phy offset we shift the ilkn_wrap_pm*/
        ...
        /*First quad (first_phy_port might not start at first lane of the quad)*/
        ...
        /*Check if number of lanes to be configured are less than the number of lanes possible in the quad*/
...
/*
 * Function:
 *      soc_qax_port_ilkn_bypass_interface_rx_enable_get
 * Purpose:
...
soc_qax_port_ilkn_bypass_interface_rx_enable_get(int unit, int port, uint32* rx_enable) {
    ...
    /* rx_enable is true, only if rx_stataus on all nbi are true */
    if (nof_lanes > 0) {
...
/*
* Function to take into consideration the lane swaps for SKU restrictions
*/
static int
soc_qax_nif_sku_restricted_lane_find(int unit, uint32 phy_lane, soc_pbmp_t phy_pbmp, uint8 *restricted_lane_configured){
    ...
    *restricted_lane_configured=FALSE;
    ...
    /** Calculate the QUAD of the lane */
    quad = (phy_lane-1)/NUM_OF_LANES_IN_PM;
    /** Calculate the position of the PHY lane in the QUAD */
    lane_in_quad = (phy_lane-1)%NUM_OF_LANES_IN_PM;
    /** Get the lane map */
    SOCDNX_IF_ERR_EXIT(soc_jer_lane_map_get(unit, quad, &lane_map));
    /** According to the mapping calculate that actual lane that needs to be restricted for RX */
    lanes_to_restrict[SOC_QAX_DIRECTION_RX] = (quad * NUM_OF_LANES_IN_PM) + lane_map.lane_map_rx[lane_in_quad] + 1;
    /** According to the mapping calculate that actual lane that needs to be restricted for TX */
    ...
    {
        /* Get the NIF port from the lane */
        SOCDNX_IF_ERR_EXIT(soc_qax_qsgmii_offsets_add(unit, lanes_to_restrict[direction], &phy_port));

        /* Since the lane can have more than one port and we want to check all of them, get the port of the next lane */
        ...
        /* Check all the ports of the lane */
        for (port=phy_port; port < next_lane_port; port++)
        {
            if(SOC_PBMP_MEMBER(phy_pbmp, port)) {
                /* If any of the ports of the restricted lanes have been configured return indication TRUE */
...
soc_qax_nif_sku_restrictions(int unit, soc_pbmp_t phy_pbmp, soc_port_if_t interface, uint32 protocol_offset, uint32 is_kbp){
        ...
        /* The port bitmap phy_pbmp is 1 based, so all lanes that are checked for restrictions in this function are checked using (lane+1) */
        ...
        if (dcmn_device_block_for_feature(unit,DCMN_QAX_NIF_10_32_FEATURE)) {
           ...
           /* For lane 10 take lane mapping into account, both RX and TX */
           ...
           /* For lane 11 take lane mapping into account, both RX and TX */
           ...
           /* For QUAD 3, since the lane swapping is per QUAD, no need to take lane mapping into account */
       ...
       /* QUAD 3, lanes 18 and 19 of QUAD 4, QUAD 7 and lanes 44 and 47 of QUAD 11 are invalid for QAX SKUs*/
       if (dcmn_device_block_for_feature(unit,DCMN_QAX_NIF_12_24_FEATURE)) {

           /* For lane 18 take lane mapping into account, both RX and TX */
           ...
           /* For lane 19 take lane mapping into account, both RX and TX */
           ...
           /* For lane 44 take lane mapping into account, both RX and TX */
           ...
           /* For lane 47 take lane mapping into account, both RX and TX */
          ...
          /* For QUAD 3 and QUAD 7 since the lane swapping is per QUAD, no need to take lane mapping into account */
          if(SOC_PBMP_MEMBER(phy_pbmp, 13) ||
             SOC_PBMP_MEMBER(phy_pbmp, 14) ||
             SOC_PBMP_MEMBER(phy_pbmp, 15) ||
             SOC_PBMP_MEMBER(phy_pbmp, 16) ||
       ...
       if (dcmn_device_block_for_feature(unit,DCMN_QAX_NIF_16_24_FEATURE)) {
          ...
          /* For QUAD 7 and QUAD 11 since the lane swapping is per QUAD, no need to take lane mapping into account */
       ...
       if (dcmn_device_block_for_feature(unit,DCMN_QUX_NIF_24_6_FEATURE)) {
           ...
           /* For lane 6 take lane mapping into account, both RX and TX */
           ...
           /* For lane 7 take lane mapping into account, both RX and TX */
          ...
          /* For QUAD 8 since the lane swapping is per QUAD, no need to take lane mapping into account */
       ...
       if (dcmn_device_block_for_feature(unit,DCMN_QUX_NIF_12_4_FEATURE)) {
           ...
           /* For QUAD 1, QUAD 5, QUAD 6, QUAD 7 and QUAD 8 since the lane swapping is per QUAD, no need to take lane mapping into account */
       ...
       /* QUAD 0 lane3&4 QUAD 1, QUAD 5, QUAD 6, QUAD 7 and QUAD 8 are invalid for QUX SKU 88274 */
...
int soc_qax_led_pmlq_bypass_config_adjust(int unit, soc_port_t port, int add) {
    ...
    if ( add ) {
        ...
        /*check if there are other QSGMII ports in the same quad*/
