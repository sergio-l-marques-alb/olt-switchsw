...
int qax_parse_meminfo_definition_file(const char *file_name) {
  ...
  while((fgets(line, sizeof(line), fp) != NULL) && (ret_val >= 0)) { 
    ...
    /*This data on string "line" is purposely tainted.
      Yuval wrote his code this way so he can rewrite 
      the fifo memories names, that are not available in ucode.*/
...
} /* end of qax_parse_meminfo_definition_file() */
...
/*Insert one chunk of DIRECT database into qax_api_info.db_direct_container.db_direct_info_arr*/
...
/*Insert one key chunk of TCAM database into qax_api_info.db_tcam_container.db_tcam_info_arr*/
...
/*Insert one result chunk of TCAM database into qax_api_info.db_tcam_container.db_tcam_info_arr*/
...
/*Insert one key chunk of EM database into qax_api_info.db_em_container.db_em_info_arr*/
...
/*Insert one result chunk of EM database into qax_api_info.db_em_container.db_em_info_arr*/
...
/*Insert one key chunk of LPM database into qax_api_info.db_lpm_container.db_lpm_info_arr*/
...
/*Insert one result chunk of LPM database into qax_api_info.db_lpm_container.db_lpm_info_arr*/
