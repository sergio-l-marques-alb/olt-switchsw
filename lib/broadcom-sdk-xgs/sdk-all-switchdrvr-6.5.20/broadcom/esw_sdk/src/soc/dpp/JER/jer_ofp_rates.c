  ...
  )
{
    ...
    /** if shaper for none channelized interface, round up the internal rate, as WA to scenario of many add/remove ports for none channelized ports. */
    /** Each time adding/removing port, new rate is configured, and the rate accuracy is demage accumulative */
    /** for example, the rate after many add/remove, will be much lower than the first time  untill getting negative value and error occur */
    /** the WA is to round up. now the accuracy still wrong, but after many times of add/remove, it will be bigger than the first time until getting to max value */
    ...
    if (ilkn_tdm_dedicated_queuing == ARAD_MGMT_ILKN_TDM_DEDICATED_QUEUING_MODE_ON && interface_type == SOC_PORT_IF_ILKN) {
        /* ILKN tdm interleave egr interface */
        ...
        /*ILKN not tdm interleave egr interface*/
        egr_if_id +=1; 
    }
    SOCDNX_IF_ERR_EXIT(soc_jer_ofp_rates_interface_internal_rate_set(unit, core, egr_if_id, internal_rate, FALSE /*is_cal_shaper*/));
