...
/* this struct used for DIP SIP sharing. there are three sharing clusters: IPv4, IPv6 and L2
...
void
   ...
   * when the KBP is not used for IPv6 forwarding or public programs are enabled.
   */
  ...
  )
{
  ...
  *success = SOC_SAND_SUCCESS;
      ...
      if (info->is_for_hw_commit || info->is_psl_with_program) {
          ...
          if (info->is_array_qualifier || info->is_psl_with_program) {
              ...
              if (info->is_psl_with_program) {
                  ...
                  curr_level->lines[pfg_ndx].line_id = pfg_ndx; /* preselector ID */
                  ...
                  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 27, exit);
                  /* finish updating in the SW state so exit*/
                  ARAD_DO_NOTHING_AND_EXIT;
              } 
  ...
  /* in case preselector was created with program ID we will get to this point by calling "bcm_field_qualify..." */
...
/*
 * Check for direct extraction DB, when SOC_PPC_FP_DATABASE_INFO_FLAGS_ALLOCATE_FES is not
 * set, if there is any action that the size is larger than the maximum size FEM can support.
 * If TRUE - then the key of the database is created when FP entry is installed. This is for 
 * backward compatibility.
 * Note that creating DE DB with large action without setting SOC_PPC_FP_DATABASE_INFO_FLAGS_ALLOCATE_FES
 * flag is deprecated.
 */
STATIC uint32
...
static uint32 arad_pp_fp_key_with_hw_id_alloc_in_prog(
        ...
        SOC_SAND_IN  uint32                       start_place,/* key size in halves */
        SOC_SAND_IN  SOC_PPC_FP_DATABASE_INFO     *info,
       /* out */
        ...
        SOC_SAND_OUT  uint8                       *key_alloced)
{
    ...
    /* This function is called only when the "WITH_CE_ID" called, which is coupled with "WITH_CYCLE" flag */
    ...
    res = arad_pp_fp_key_alloc_in_prog(
              ...
              start_place,
              ce_cons,
              alloc_info->nof_ce,/* for each Qual which CE is allocated */
              alloc_info,
              key_alloced
           );
  ...
  )
{
  ...
  *success = SOC_SAND_FAILURE_OUT_OF_RESOURCES;
  ...
  /* 
   * Special treatment when we want to allocate FES for a large direct extraction
   * DB created without BCM_FIELD_GROUP_CREATE_LARGE flag.
   */
  ...
  /* if field group was created with program ID overwrite pmf_pgm_bmp_used
  pmf_pgm_bmp_used = bit map of the program IDs which are associated with this field group*/
/*  if (info->nof_programs) {
      pmf_pgm_bmp_used = 0;
      for (i = 0; i < info->nof_programs; i++) {
          pmf_pgm_bmp_used |= (1 << (info->pgm_bmp));
      }
  }
*/
  if (info->pgm_bmp) {
 ...
 /* Check if there is a key-change action or qualifier */
   ...
   */
    ...
    /* Skip the constraints calculations when the WITH_CE_ID is on. This will be done inside arad_pp_fp_key_with_hw_id_alloc_in_prog()*/
      ...
      for (config_phase = 0; config_phase < 2; config_phase++) 
      {
        ...
        if (info->flags & SOC_PPC_FP_DATABASE_INFO_FLAGS_ELK_RANGE) {
            for(k = 0, i = 0; k < total_ce_indx; ++k) {
                ...
                total_qual_size += ce_const[k].nof_bits;   /*tmp value*/
            }
        }
        ...
        if (j >= 0) {
            ...
            /* use this to store the bits instead of the bytes and convert them to bytes later */
            key_size_in_bytes = 32;
            key_size_in_bytes_msb = 0;
            for (k = 2; k < total_ce_indx; k++) {
                /*now only (80-32) bits space in the low zone*/
        ...
        /* check custom_feature_ext_flexible_mode soc is enabled
           if enabled, arad_kbp_add_acl is not executed. need to set pmf_pgm_bmp_new_sw to equal pmf_pgm_bmp_new */
  ...
  else 
  {
    ...
    for (round = 0; round < 2 ; round++)
    {
        ...
        for (pmf_pgm_ndx = pmf_pgm_ndx_min; pmf_pgm_ndx < pmf_pgm_ndx_max; ++pmf_pgm_ndx)
        {
            ...
            else if (!(info->pgm_bmp)) /*when field group is created without program ID a default program Id is assigned (id=7) then after checking resource
                                            allocation fisibility we allocate a new PMF program, this is in order to support field group creation under traffic*/
            {
            ...
            else { /* in case that field group was created with program ID we dont have a duplicated program since we do not suuport configuration under traffic
                   so pmf_pgm_ndx_duplicate = pmf_pgm_ndx*/
                pmf_pgm_ndx_duplicate = pmf_pgm_ndx;
                pmf_pgm_alloced = 1;
            }
            ...
            if(round != 0)  /* Skip hw configuration and program duplication in alloc check */
            {
                ...
                /*set KeyGenVar for the program if KeyGenVar is used to build key*/
                if (SOC_IS_QUX(unit)) {
            ...
            {
                /* Allocate the Key */
                /*Since Large DE might have several action on same key, we would like to allocate key also here
                            * This way there will be only one key 
                            */
                if ((info->flags & SOC_PPC_FP_DATABASE_INFO_FLAGS_WITH_CE_ID))
                {
            ...
            {                
                ...
                /* in case field group was created with program ID we didnt use the default Ethernet program so nothing to handle*/
                if (!info->pgm_bmp) {
    ...
    /* Special case if custom SOC is enabled, for large direct table:
    * IF 
    *     (qualifier inclues forward decision destination)
    *     AND (action includes EEI) 
    *     AND (pre-selector has a fwd_code which equals to MPLS)
    * THEN
    *     Allocate one additional FEM to extract EEI.
    * The logic is:
    *     EEI = action table[n+23:n] (default, extracted by FES)
    *     If for MPLS, EEI = { 4'bxxxx, action_table[n+19, n] } (extracted by FEM)
    */
    if ((stage == SOC_PPC_FP_DATABASE_STAGE_INGRESS_PMF) 
        && (info->db_type == SOC_PPC_FP_DB_TYPE_DIRECT_TABLE)
        && (info->flags & SOC_PPC_FP_DATABASE_INFO_FLAGS_USE_KAPS)
        ...
        do {
            ...
            fem_info->actions[0].fld_ext[0].type = SOC_PPC_FP_QUAL_IRPP_KAPS_PASS1_PAYLOAD; /* In addition with the cycle of the program */
            fem_info->actions[0].fld_ext[0].cst_val = 0;
            fem_info->actions[0].fld_ext[0].fld_lsb = action_lsbs[action_ndx] + 1 /* Valid bit */;
  ...
  )
{
  ...
  if (is_for_tm) {
    ...
    */
    ...
    } /* else: key removed during entry removal */
  ...
  )
{
  ...
  if (!is_tcam_dt && SOC_IS_QUX(unit)) {
      /*
      * QUX KAPS large direct lookup specifically:
      *     key bits [13:12] are not used by hardware.
      * This is to move key bits [12+:] to key bits [14+:]
      */
              ...
              if (phase_ndx == 0) {
                ...
                else if(SOC_IS_JERICHO(unit))
                {
                    ...
                    if (SOC_IS_QUX(unit) && (entry_ndx_dt & QUX_KAPS_DIRECT_LOOKUP_UNUSED_BITS_MASK)) {
                        /*
                        * For QUX and unused bits in direct lookup key is not 0, skip it.
                        * Only set it once (when unused bits are all zeros).
                        */
                        continue;
                    }
    ...
    if ((entry_ndx_dt & is_dt_bit_meaningful) == (entry_dt_first & is_dt_bit_meaningful)) {
        ...
        else if(SOC_IS_JERICHO(unit))
        {
                    ...
                    if (SOC_IS_QUX(unit) && (entry_ndx_dt & QUX_KAPS_DIRECT_LOOKUP_UNUSED_BITS_MASK)) {
                        /*
                        * For QUX and unused bits in direct lookup key is not 0, skip it.
                        * Only remove it once (when unused bits are all zeros).
                        */
                        continue;
                    }
...
uint32
  arad_pp_fp_entry_remove_unsafe(
    ...
    SOC_SAND_IN  uint32 db_id_ndx,
    SOC_SAND_IN  uint32 entry_id_ndx,
    SOC_SAND_IN  uint32 is_sw_remove_only   /*bit1 of 2LSB is used to indicate if keep mode for TCAM entry*/
  )
{
  uint32
     res = SOC_SAND_OK,
  ...
  if (tcam_key_type) {
    if (ARAD_IP_TCAM_ENTRY_TYPE_IS_KBP(tcam_key_type)) {
        if (is_sw_remove_only & ARAD_TCAM_REMOVE_IS_SW_ONLY) { /* bit0 indicate if sw remove only*/
  ...
  )
{
  ...
  *success = SOC_SAND_SUCCESS;
  ...
  /* Direct extraction is allocated in odd cycle */
  ...
  /* In case of large DE but LARGE flag is not set */    
  if (large_de_without_flag) {
      ...
      for (pmf_pgm_ndx = pmf_pgm_ndx_min; pmf_pgm_ndx < pmf_pgm_ndx_max; ++pmf_pgm_ndx)
      {
          ...
          /* alloc_info will be cleared in arad_pp_fp_key_alloc_in_prog */
          if (alloc_info == NULL) {
              ARAD_ALLOC(alloc_info, ARAD_PP_FP_KEY_ALLOC_INFO, 1, "arad_pp_fp_database_create_unsafe.alloc_info");
          }
  ...
  if ((info->qual_vals[0].type != SOC_PPC_NOF_FP_QUAL_TYPES) && (info->qual_vals[0].type != BCM_FIELD_ENTRY_INVALID)) {
    ...
    if (soc_sand_u64_is_zero(&info->qual_vals[0].is_valid)) {
        ...
        if (total_valid_bits > 1) {
            /* The entry has more than one bit to match, use FEM */
            use_fem = TRUE;
        }
        else {
            /* total valid bits = 1, check that it is the LSB of fld_ext[0] */
            for (action_ndx = 0; action_ndx < SOC_PPC_FP_NOF_ACTIONS_PER_DB_MAX; ++action_ndx) {
  ...
  *fem_info = *info;
  ...
  /* Decide per each action whether we can allocate a FES for it */
  for (action_ndx = 0; action_ndx < SOC_PPC_FP_NOF_ACTIONS_PER_DB_MAX; ++action_ndx) {
    if (info->actions[action_ndx].type == SOC_PPC_FP_ACTION_TYPE_INVALID) {
        /* done */
        break;
    ...
    /* FES does not support adding a base to the action */
    use_fes = (!use_fem) && (info->actions[action_ndx].base_val == 0);

    /*candidate for FES? */
    /*Loop on all extractions and check there are no "holes" or lost-bits*/
    total_ext_bits = 0;
    for (extr_ndx = 0; (extr_ndx < info->actions[action_ndx].nof_fields) && !use_fem && use_fes; ++extr_ndx)
    {
        ...
        /* fld_ext 0 in LSB, fld_ext n in MSB */
        if (extr_ndx == 0 )
        {
            tmp_lsb = key_lsb;
    ...
    if (!use_fes && (total_ext_bits > SOC_DPP_DEFS_GET(unit, fem_max_action_size_nof_bits))) {
        /* If must allocate FES but failed to do so */
        LOG_ERROR(BSL_LS_SOC_FP,
           (BSL_META_U(unit,
  ...
  if (fes_action_type[0] != SOC_PPC_FP_ACTION_TYPE_INVALID) {
    ...
    /* Check if the number of FESes can be allocated */
    ...
    for (pmf_pgm_ndx = pmf_pgm_ndx_min; pmf_pgm_ndx < pmf_pgm_ndx_max; ++pmf_pgm_ndx)
    {
        ...
        for (action_ndx = 0; action_ndx < SOC_PPC_FP_NOF_ACTIONS_PER_DB_MAX; ++action_ndx) {
            ...
            action_const.tcam_res_id[1] = (db_info.alloc_place == ARAD_PP_FP_KEY_CE_HIGH)? ARAD_PP_FP_TCAM_HALF_KEY_SIZE: 0;  /*Which LSB bit*/ 
            action_const.tcam_res_id[1] += action_lsbs[action_ndx];  /*CE contstraint lost bits*/
            action_const.cycle = selected_cycle[pmf_pgm_ndx];
            alloc_flags |= ARAD_PP_FP_FEM_ALLOC_FES_FROM_KEY;
            alloc_flags |= fes_alloc_flags;

            /* Install FES actions one by one */
...
exit_install_fem:
  ...
  /*
   * Remove FEM fro HW for this entry
   */
  SOC_PPC_FP_FEM_ENTRY_clear(&fem_entry);
  fem_entry.is_for_entry = TRUE;
  fem_entry.db_id = db_id_ndx;
...
exit_install_key:
  /* De-allocate key for large DE DBs */ 
  if (large_de_without_flag) {
  ...
  )
{
  ...
  *is_found = FALSE;
  ...
  /* Find all FEMs belong to this DB in the same pgm. Extraction is the same for different pgm.
  */
            ...
            for (fld_ndx = 0; fld_ndx < SOC_PPC_FP_DIR_EXTR_MAX_NOF_FIELDS; ++fld_ndx)  {
              ...
              if (!fes_input_info.is_action_always_valid
                && (info->qual_vals[0].type == BCM_FIELD_ENTRY_INVALID)) {
                  /* Conditional FES - set qualifier but clear valid bits */
              ...
              if (fes_qual != BCM_FIELD_ENTRY_INVALID) {
                  ...
                  qual_nof_bits -= (pos - key_lsb); /* should be 0 if fld_ndx != 0 */
              ...
              /* else: special case when extracting from key, but the key is built in update key stage instead of built by CE, 
              * e.g., SOC_PPC_FP_QUAL_KEY_AFTER_HASHING */
  ...
  )
{
  ...
  if (!is_found) {
      /* Remove a DE entry that does not exist? */
      ARAD_DO_NOTHING_AND_EXIT;
  }
...
exit_egress:
  ...
  for (qual_index = 0; qual_index < ARAD_PP_FP_EGRESS_NOF_CAM_PARAMS; qual_index++) {
      ...
      case 6:
          ...
          break;
      case 7:
          /* IPMC-fall-to-bridge, for QAX B0 and above */
      ...
      for (action_ndx = 0; action_ndx < SOC_DPP_DEFS_GET(unit, nof_egress_pmf_actions); action_ndx++) {
          ...
          case SOC_PPC_FP_ACTION_TYPE_EGR_OFP:
              if(SOC_IS_JERICHO(unit)) {
                  ...
                  action_val = val[0] & 0x1FF; /* 9 LSBits */
              } else {
                  /* ARAD */
  ...
  )
{
  ...
  for(cycle_idx = 0; cycle_idx < ARAD_PMF_LOW_LEVEL_NOF_CYCLES; cycle_idx++)
  {
    ...
    {
       /*Scan all FEM programs, includeing TM*/
  ...
  for (ind = 0; ind < SOC_PPC_FP_NOF_QUALS_PER_DB_MAX; ++ind)
  {
        ...
        && (info->qual_vals[ind].type != BCM_FIELD_ENTRY_INVALID))
    {
        if (ind == 0) {
            ...
            if (total_valid_bits > 1) {
                /* The entry has a qualifier to match, use FEM */
                allocate_fem = TRUE;
            }
            ...
            /* Direct extraction can have one qualifier */
  ...
  for (ind = 0; ind < SOC_PPC_FP_NOF_ACTIONS_PER_DB_MAX; ++ind)
  {
    ...
    if (info->actions[ind].type != SOC_PPC_FP_ACTION_TYPE_INVALID)
    {
        ...
        if (allocate_fem) {
          ...
          /* If !allocate_fem and total_bits > MAX FEM action size */
          if (total_ext_bits > SOC_DPP_DEFS_GET(unit, fem_max_action_size_nof_bits)) {
              /* Must allocate FES, check qualifier valid bit is the LSB in fld_ext */
  ...
  )
{
  ...
  /* Get Output Action type and Value */
