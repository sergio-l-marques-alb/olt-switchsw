...
/*
 * Used to set the DESCRIPTORS_REMAINING field in the CONTROL word
 * SOC_SBUSDMA_CFG_PREFETCH_ENB must be set
 */
...
/*
 * The descriptor control,
 * the number of descriptors in a chain and HW descriptor pointer are updated for each commit
 */
...
/* Holds the descriptor handle, 0 if not allocated */
    ...
    uint8 safe)
{
    ...
    if (dnx_sbusdma_desc_counter[unit] != 0)
    {
        ...
        /*
         * Iterate on the last descriptors and update their prefetch DESCRIPTORS_REMAINING
         */
...
/*
 * Custom version of _cmicx_sbusdma_desc_prepare with limited options for faster implementation
 */
void
    ...
    soc_sbusdma_desc_ctrl_t * ctrl)
{
    ...
    /*
     * max DESCRIPTORS_REMAINING for DESC_PREFETCH_ENABLE
     */
    ...
    {
        /*
         * soc_reg_field_set(unit, CMIC_CMC0_SBUSDMA_CH0_REQUESTr, &(desc->req), REQ_WORDSf, cfg->width);
         */
        desc->req |= cfg->width << 5;
        /*
         * soc_reg_field_set(unit, CMIC_CMC0_SBUSDMA_CH0_REQUESTr, &(desc->req), REP_WORDSf, 0);
         */
        if (cfg->mem_clear)
        {
            /*
             * soc_reg_field_set(unit, CMIC_CMC0_SBUSDMA_CH0_REQUESTr, &(desc->req), REQ_SINGLEf, 1);
             */
            desc->req |= 1 << 30;
        }
        else
        {
            /*
             * soc_reg_field_set(unit, CMIC_CMC0_SBUSDMA_CH0_REQUESTr, &(desc->req), REQ_SINGLEf, 0);
             */
        }
    }
    else
    {
        /*
         * soc_reg_field_set(unit, CMIC_CMC0_SBUSDMA_CH0_REQUESTr, &(desc->req), REQ_WORDSf, 0);
         */
        /*
         * soc_reg_field_set(unit, CMIC_CMC0_SBUSDMA_CH0_REQUESTr, &(desc->req), REP_WORDSf, cfg->width);
         */
        desc->req |= cfg->width << 0;
    }
    /*
     * soc_reg_field_set(unit, CMIC_CMC0_SBUSDMA_CH0_REQUESTr, &(desc->req), INCR_SHIFTf, cfg->addr_shift);
     */
...

    {
        ...
        {
            /*
             * soc_reg_field_set(unit, CMIC_CMC0_SBUSDMA_CH0_REQUESTr, &(desc->req), HOSTMEMWR_ENDIANESSf, 1);
             */
            desc->req |= 1 << 11;
            /*
             * soc_reg_field_set(unit, CMIC_CMC0_SBUSDMA_CH0_REQUESTr, &(desc->req), HOSTMEMRD_ENDIANESSf, 1);
             */
            desc->req |= 1 << 10;
        }
    }
    ...
    uint32 timeout_usec)
{
    ...
    {
        /*
         * desc_cfg is not utilized with SOC_SBUSDMA_CFG_USE_SUPPLIED_DESC
         * Temporarily allocate and free to use common infrastructure
         */
...
#if !defined(ADAPTER_SERVER_MODE) && !defined(PLISIM)
    /*
     * Both descriptor DMA size properties must be non-zero
     * timeout may be zero for a main thread only commit (no timeout thread)
     */
    if ((buff_size_kb != 0) && (chain_length_max != 0))
    {
