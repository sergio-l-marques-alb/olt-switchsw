...
#define DNX_KAPS_TRY(A)                                                                                              \
    ...
    {                                                                                                               \
        ...
        {                                                                                                           \
            SHR_IF_ERR_EXIT_WITH_LOG(dnx_kaps_error_translation(__tmp_status),                                       \
                                       "\n"#A" failed: %s%s%s\n",                                                   \
    ...
    uint8 *kaps_hitbit)
{
    ...
    SHR_IF_ERR_EXIT(dnx_kaps_buffer_dbal_to_kaps(unit, key_size_in_bits, key, data));
    ...
    DNX_KAPS_TRY(kaps_ad_db_get(ad_db_p, ad_entry, asso_data));

    SHR_IF_ERR_EXIT(dnx_kaps_buffer_kaps_to_dbal(unit, payload_size_in_bits, asso_data, payload));
    ...
    uint8 kaps_hitbit)
{
    ...
    SHR_IF_ERR_EXIT(dnx_kaps_buffer_dbal_to_kaps(unit, key_size_in_bits, key, data));
    SHR_IF_ERR_EXIT(dnx_kaps_buffer_dbal_to_kaps(unit, payload_size_in_bits, payload, asso_data));

    kaps_db_get_prefix_handle(db_p, data, prefix_length, &db_entry);

    DNX_KAPS_INDIRECT_COMMIT_MODE_CHECK_AND_UPDATE(db_entry, *indirrect_commit_mode);
    ...
    if (add_entry)
    {
        ...
        if (DNX_KAPS_IS_ERROR(res))
        {
            ...
            if (dnx_kaps_error_translation(res) == _SHR_E_FULL)
            {
    ...
    uint32 prefix_length)
{
    ...
    SHR_IF_ERR_EXIT(dnx_kaps_buffer_dbal_to_kaps(unit, key_size_in_bits, key, data));
    DNX_KAPS_TRY(kaps_db_get_prefix_handle(db_p, data, prefix_length, &db_entry));
    ...
    int device_type)
{
    ...
    Kaps_warmboot_data[unit].kaps_file_read = &dnx_kaps_file_read_func;
    Kaps_warmboot_data[unit].kaps_file_write = &dnx_kaps_file_write_func;
    ...
    uint32 db_id)
{
    ...
    if (!SOC_WARM_BOOT(unit))
    {
        ...
        if (db_handles_info.is_valid == 1)
        {
            ...
            DNX_KAPS_TRY(kaps_db_set_ad(db_handles_info.db_p, db_handles_info.ad_db_p));

            DNX_KAPS_TRY(kaps_hb_db_init(Lpm_app_data[unit]->kaps_device_p, db_id, db_size, &db_handles_info.hb_db_p));
    ...
    int unit)
{
    ...
    if (!SOC_WARM_BOOT(unit))
    {
        ...
        DNX_KAPS_TRY(kaps_instruction_init(Lpm_app_data[unit]->kaps_device_p, 0,
                                           0, &(instruction_handles_info.inst_p)));
    ...
    int *ipv6_capacity)
{
    ...
    SHR_IF_ERR_EXIT(mdb_lpm_big_kaps_enabled(unit, db_idx, &big_kaps_enabled));

    SHR_IF_ERR_EXIT(MDB_KAPS_ACCESS.db_info.get(unit, db_idx, &db_handles_info));
    if (db_handles_info.is_valid)
    {
        DNX_KAPS_TRY(kaps_db_get_property(db_handles_info.db_p, KAPS_PROP_NUM_LEVELS_IN_DB, &kaps_hier));
        if (!(((kaps_hier == 3) && (big_kaps_enabled == TRUE)) || ((kaps_hier == 2) && (big_kaps_enabled == FALSE))))
        {
    ...
    if (big_kaps_enabled == FALSE)
    {
        ...
        SHR_IF_ERR_EXIT(mdb_lpm_get_nof_small_bbs(unit, dbal_physical_table_id, &nof_small_bbs));

        total_nof_bb_rows = nof_small_bbs * dnx_data_mdb.kaps.nof_rows_in_small_bb_get(unit);
    ...
    uint32 flags)
{
    ...
    SHR_IF_ERR_EXIT(dnx_kaps_entry_lpm_add(unit,
    ...
    uint32 flags)
{
...
#endif
    {
        ...
        res = dnx_kaps_entry_lpm_get(unit,
                                     db_handles_info.db_p,
                                     db_handles_info.ad_db_p,
                                     db_handles_info.hb_db_p,
                                     entry->key,
    ...
    uint32 flags)
{
    ...
    SHR_IF_ERR_EXIT(dnx_kaps_entry_lpm_delete(unit,
                                              db_handles_info.db_p,
                                              db_handles_info.ad_db_p,
                                              db_handles_info.hb_db_p,
    ...
    else
    {
        ...
        if (kpb_e != NULL)
        {
            if (kpb_e_info.ad_handle != NULL)
            {
                ...
                DNX_KAPS_TRY(kaps_ad_db_get(physical_entry_iterator->mdb_lpm_ad_db_p, kpb_e_info.ad_handle, ad_8));
    ...
    while (ext_iterator < max_ext_iterations)
    {
        ...
        do
        {
            ...
            if (utilex_bitstream_have_one_in_range
                (entry_duplicate.payload, 0, dnx_data_mdb.kaps.ad_width_in_bits_get(unit) - 1))
            {
                SHR_IF_ERR_EXIT(utilex_bitstream_xor
                                (entry_duplicate.payload, entry.payload,
                                 (dnx_data_mdb.kaps.ad_width_in_bits_get(unit) + SAL_UINT32_NOF_BITS -
    ...
    dbal_physical_tables_e dbal_physical_table_id)
{
    int core;
    mdb_kaps_ip_db_id_e db_idx = MDB_KAPS_IP_PRIVATE_DB_ID;
    uint32 nof_tcam_rows = dnx_data_mdb.kaps.nof_rows_in_rpb_get(unit);
    int tcam_row;
    int ecc_nof_bits = 16;
    ...
    for (core = 0; core < dnx_data_device.general.nof_cores_get(unit); core++)
    {
        for (tcam_row = 0; tcam_row < nof_tcam_rows; tcam_row++)
        {
            ...
            {
                SHR_ERR_EXIT(_SHR_E_FAIL,
                             "TCAM scan test failed, db_idx = %d, core = %d, tcam_row = %d, ecc_data_orig = 0x%x, ecc_data_mod = 0x%x, ecc_data_read = 0x%x.\n",
