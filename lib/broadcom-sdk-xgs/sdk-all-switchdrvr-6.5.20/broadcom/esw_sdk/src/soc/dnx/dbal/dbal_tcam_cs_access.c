...
/*
 * The value of the prefix on the key indicating that the entry is half.
 */
#define TCAM_CS_KEY_SIZE_PREFIX_BITS_HALF 0
/*
 * The value of the prefix on the key indicating that the entry is full.
 */
#define TCAM_CS_KEY_SIZE_PREFIX_BITS_FULL 1
/*
 * The value of the mask for the prefix on the key indicating whether the entry is half or full.
 */
#define TCAM_CS_KEY_SIZE_PREFIX_BITS_MASK 1
/*
 * Width of data in CS_TCAM_BANK_TBL in uint32.
 */
...
/*
 * When writing a full key the two valid bits in *_TCAM_BANK_COMMANDm, must be asserted
 */
#define TCAM_CS_VALID_BITS_FULL 0x3
/*
 * When writing a half key for even entry, the first bit in *_TCAM_BANK_COMMANDm, must be asserted
 */
#define TCAM_CS_VALID_BITS_HALF_FIRST 0x1
/*
 * When writing a half key for off entry, the second bit in *_TCAM_BANK_COMMANDm, must be asserted
 */
#define TCAM_CS_VALID_BITS_HALF_SECOND 0x2
/*
 * When clearing a TCAM line the two valid bits should be 0
 */
#define TCAM_CS_VALID_BITS_NONE 0x0

/**
 * Thevalue written to disable the ECC_FIX_EN
 * See CS_TCAM_BANK_STAGE_ECC_FIX_EN_WRITE, CS_TCAM_BANK_STAGE_ECC_FIX_EN_READ, DBAL_TCAM_CS_ECC_DISABLE, DBAL_TCAM_CS_ECC_REENABLE
 */
#define CS_TCAM_BANK_STAGE_ECC_FIX_EN_DISABLE_VALUE (0)

/**
 * \brief TCAM CS ECC disabling before read/write.
 *
 * This macro should appear before accessing the CS TCAM after
 ...
 *
 * internal variables:
 *    _tcam_cs_ecc_value_acquired - Whether we got the older value of the ECC_FIX_EN register.
 *    _tcam_cs_ecc_disable_needed - Whether the feature is relevant for the unit.
 *    _tcam_cs_ecc_reenable_needed - Whether ECC_FIX_EN was disabled to begin with.
 *    _tcam_cs_ecc_value - The old value of the ECC_FIX_EN register.
 *    _tcam_cs_unit - The device ID.
 *    _tcam_cs_ecc_fix_en_reg - The register of the ECC_FIX_EN for the CS TCAM.
 *
 * \par DIRECT INPUT:
 *   \param [in] _in_tcam_cs_unit -
 *     Value of 'unit' input parameter at entry.
 *   \param [in] _in_tcam_cs_ecc_fix_en_reg -
 *     The register of the ECC_FIX_EN for the CS TCAM. (soc_reg_t)
 * \par INDIRECT INPUT:
 * \par INDIRECT OUTPUT:
 *
 *  Example:
 *
 * \code{.c}
 * int my_func(int unit)
 * {
 *   int local_idx;
 *
 *   SHR_FUNC_INIT_VARS(unit);
 *   DBAL_TCAM_CS_ECC_DISABLE(unit);
 *
 *   Code accessing (reading or writing)
 *
 * exit:
 *   DBAL_TCAM_CS_ECC_REENABLE(unit);
 *   SHR_FUNC_EXIT;
 * }
 * \endcode
 */
...
{ \
  /** Note that only the third variable is needed. */ \
  ...
  { \
    ...
    /* \
     * Note that we read only from the first core, so if any core has a different value from FIX_EN it will get \
     * the value of core 0, and this value will be written to the other cores.\
     * Only the scenario where all cores are identical is supported. \
     */ \
...
/**
 * \brief TCAM CS ECC disabling after read/write.
 *
 * This macro is used to return the ECC fix to its earlier state after read/write.
 * This MACRO has 1 extra } at the end for compilation
 * reason as variable defined in the DBAL_FUNC_INIT_VARS after
 * code. the corresponding { is located at the beginning of
 * DBAL_TCAM_CS_ECC_DISABLE macro.
 *
 * \par DIRECT INPUT:
 * \par INDIRECT INPUT:
 *    \param [in] _tcam_cs_ecc_value_acquired - Whether we got the older value of the ECC_FIX_EN register
 *    \param [in] _tcam_cs_ecc_disable_needed - Whether the feature is relevant for the unit
 *    \param [in] _tcam_cs_ecc_reenable_needed - Whether ECC_FIX_EN was disabled to begin with.
 *    \param [in] _tcam_cs_ecc_value - The old value of the ECC_FIX_EN register
 *    \param [in] _tcam_cs_unit - The device ID.
 *    \param [in] _tcam_cs_ecc_fix_en_reg - The register of the ECC_FIX_EN for the CS TCAM.
 *
 * \par INDIRECT OUTPUT:
 *    None
 *
 ...
 */
/**
 * \brief - returns CS TCAM HW info per stage.
 *
 * \param [in] unit - Identifier of HW platform.
 * \param [in] dbal_stage - The physical CS_TCAM
 * \param [out] stage_info -  The information of the CS TCAM mapping and size.
 * \return
 *   shr_error_e
 *
 * \remark
    ...
    dbal_tcam_cs_stage_info_t * stage_info)
{
    ...
    /*
     * Get the size of the key from the field used to read it.
     */
    ...
    /*
     * If we support half key, remove the key size prefixes
     */
    ...
    char *action)
{
    ...
    if (bsl_severity >= bslSeverityInfo)
    {
        ...
        /** Print key and key_mask */
            ...
            if (!print_only_key)
            {
            /** Print key and key_mask if it is TCAM   */
                if (is_tcam)
                {
            ...
            /** payload is not relevant in the TCAM mode */
                if (!is_tcam)
                {
...
/**
 * \brief - Read a full line from CS TCAM. Function intended to be called by dbal_tcam_cs_key_write()
 *          or dbal_tcam_cs_key_read()
 *
 * \param [in] unit - Identifier of HW platform.
 * \param [in] stage_info - Information about the CS TCAM, provided by dbal_tcam_cs_stage_info_get.
 * \param [in] line_index - Line index in the memory block (0-127 for most).
 * \param [out] valid_bits -  The line's valid bits. Updated even if invalid.
 * \param [out] key -  The key read. Remains unchanged if the entire line is invalid.
 * \param [out] k_mask -  The mask read. Remains unchanged if the entire line is invalid.
 * \param [in] entry_handle - DBAL entry handle - holds the entry information
 *
 * A TCAM data consists of: value(200b), mask(200b), valid(1b), entry_handle_id
 *
 * \return
 *   shr_error_e
 *
 * \remark
 *   * Note that this function does not disable the ECC, and expects to be called from dbal_tcam_cs_key_write()
 *     or dbal_tcam_cs_key_read().
 * \see
 *   * None
 */
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    /*
     * Perform HW read for the mask
     * We first read the odd entry in the TCAM bank table (with the mask),
     * because we can only read the valid bits from odd entries.
     * If the entry is valid, we read the key (even entry in TCAM bank).
     */
    SOC_REG_ABOVE_64_CLEAR(mem_val);
    /*
     * Set *_TCAM_CPU_CMD_RDf of *_TCAM_BANK_COMMANDm to perform read action
     */
    ...
    {
        /*
         * if core mode is DPC && core_id defined, set appropriate block
         */
    ...
    /*
     * Read the mask, which include the valid bits.
     * Take the DATA_OUT from *_TCAM_BANK_REPLYm at odd line (line_index*2+1), to retrieve the MASK VALUE
     */
    ...
    *valid_bits = soc_mem_field32_get(unit, stage_info->reply_memory, mem_val, stage_info->reply_valid_field);
    ...
    /*
     * If the entry is invalid, don't bother reading the key.
     */
    ...
    /*
     * Sanity check: Checking that not half entry valid bits found if half entries are not supported in the TCAM.
     */
    if (stage_info->supports_half_entries == FALSE &&
        *valid_bits != TCAM_CS_VALID_BITS_FULL && *valid_bits != TCAM_CS_VALID_BITS_NONE)
    {
        /*
         * Since in PCID the zeroing of the CS TCAM tables is expected not to work, we don't send an error
         * if we are in PCID.
         */
#ifdef PLISIM
    ...
    /*
     * Take the DATA_OUT from *_TCAM_BANK_REPLYm at even line (line_index*2), to retrieve the KEY VALUE
     */
    SOC_REG_ABOVE_64_CLEAR(data_out);
    soc_mem_field_get(unit, stage_info->reply_memory, mem_val, stage_info->reply_data_out_field, data_out);
...
/**
 * \brief - Write a full line into CS TCAM. Function intended to be called by dbal_tcam_cs_key_write().
 *
 * \param [in] unit - Identifier of HW platform.
 * \param [in] stage_info - Information about the CS TCAM, provided by dbal_tcam_cs_stage_info_get.
 * \param [in] line_index - Line index in the memory block (0-127)
 * \param [in] valid_bits - What to write to the valid bits. Changes depending on whether we write a half entry or a
 *                          full entry, and on whether we write or delete an entry.
 * \param [in] key -  The key to write
 * \param [in] k_mask -  The key mask to write
 * \param [in] entry_handle - DBAL entry handle - holds the entry information
 * A TCAM value contains value(200b), mask(200b), valid(2b) and entry_handle_id which have to be written as follows:
 * value and mask will be combined in a variable of 400b and written in one atomic operation on line_index*2.
 * 2 bits of valid must be asserted when writing a key
 *
 * \return
 *   shr_error_e
 *
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    dbal_offset_encode_info_t block_index_info = { 0 };
    /*
     * - in_data_mask/value contain VALUE(200b) and MASK(200b) given by user.
     */
    ...
    SOC_REG_ABOVE_64_CLEAR(mem_val);
    /*
     * Set *_TCAM_CPU_CMD_WRf of *_TCAM_BANK_COMMANDm to perform write action
     */
    ...
    /*
     * if core mode is DPC && core_id defined, write the data to the relevant blocks
     */
    else
    {
        SHR_IF_ERR_EXIT(dbal_hl_mem_block_calc(unit, entry_handle, entry_handle->core_id, stage_info->cmd_memory,
...
/**
 * \brief - Read a single entry from a CS TCAM that supports using half keys.
 *
 * \param [in] unit - Identifier of HW platform.
 * \param [in] stage_info - Information about the CS TCAM, provided by dbal_tcam_cs_stage_info_get.
 * \param [in] entry_num - The index of the entry.
 * \param [in] entry_handle -  DBAL entry handle - holds the entry information
 *
 * A TCAM data consists of: value(200b), mask(200b), valid(1b), entry_handle_id
 *
 ...
 * \remark
 * \remark
 *   * Note that this function does not disable the ECC, and expects to be called from dbal_tcam_cs_key_read().
 * \see
 *   * None
 */
static shr_error_e
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    /*
     * Copy the key and mask to the entry handle. Remove TCAM mode prefix.
     */
    ...
    /*
     * Check if we write to the first or second half of the line.
     */
    if (entry_num % 2 == 0)
    {
        /**Copy the first half of the line to the first half of the key.*/
    ...
    {
        /**Copy the second half of the line to the first half of the key.*/
...
/**
 * \brief - Read a single entry from CS TCAM.
 *
 * \param [in] unit - Identifier of HW platform.
 * \param [in] stage_info - Information about the CS TCAM, provided by dbal_tcam_cs_stage_info_get.
 * \param [in] entry_num - The index of the entry.
...
/**
 * \brief - Write a single entry into a CS TCAM that supports using half keys.
 *
 * \param [in] unit - Identifier of HW platform.
 * \param [in] stage_info - Information about the CS TCAM, provided by dbal_tcam_cs_stage_info_get.
 * \param [in] entry_num - The index of the entry.
 * \param [in] entry_handle -  DBAL entry handle - holds the entry information
 * \param [in] dbal_action -  DBAL action
 * Supported dbal actions for TCAM write: DBAL_ACTION_ENTRY_COMMIT - for adding an entry
 *                                        DBAL_ACTION_ENTRY_CLEAR - for clearing an entry
 * \return
 *   shr_error_e
 *
 * \remark
    ...
    dbal_actions_e dbal_action)
{
    ...
    /*
     * If we have a half entry, we must first read the full line.
     * If we write a full line, we treat the current line as if it is empty.
     * Note that we rely on the fact the if both valid bits are zero the valid bits are updated but not key and mask
     * in dbal_tcam_cs_key_read_full_line.
     */
    ...
    /*
     * If we delete the entry we invert the valid bits (to write zeros).
     */
    ...
    /*
     * Copy the key and mask to the part of the line to be written.
     * Also write the TCAM mode if the TCAM supports half entries.
     * Note we do not support combining half and full entries, and we always write the mask of the TCAM mode to be 1's.
     */
    key_size_prefix_size = dnx_data_ingress_cs.properties.nof_bits_entry_size_prefix_get(unit);
    half_entry_size = stage_info->line_length_key_only / 2;
    ...
    if (entry_num % 2 == 0)
    {
        /**Copy the first half of the key to the first half of the line.*/
    ...
    {
        /**Copy the first half of the key to the second half of the line.*/
...
/**
 * \brief - Write a single entry into CS TCAM.
 *
 * \param [in] unit - Identifier of HW platform.
 * \param [in] stage_info - Information about the CS TCAM, provided by dbal_tcam_cs_stage_info_get.
 * \param [in] entry_num - The index of the entry.
...
/**
 * \brief
 *    this API get the result part of the entry to the HW. the result part is mapped to a single memory in the HW as HL
 *    field.
 */
shr_error_e
...
/**
 * \brief
 *    this API sets the result part of the entry to the HW. the result part is mapped to a single memory in the HW as HL
 *    field.
 */
shr_error_e
...
/**
 * \brief - Add or clear TCAM entry according to dbal_action
 *
...
/**
 * \brief - Entry set in TCAM CS tables
 *
 * \param [in] unit - Relevant unit
 * \param [in] entry_handle -  DBAL entry handle - holds the entry information
 *
...
/**
 * \brief - Get entry from TCAM CS table
 *
 * \param [in] unit - Relevant unit
 * \param [in] entry_handle - DBAL entry handle - holds the entry information
 *
...
/**
 * \brief - Clear entry from TCAM CS table
 *
 * \param [in] unit - Relevant unit
 * \param [in] entry_handle - DBAL entry handle - holds the entry information
 *
...
/**
 * \brief - Table clear of TCAM CS tables.
 *
 * \param [in] unit - Relevant unit
 * \param [in] entry_handle - DBAL entry handle - holds the entry information
 *
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    {
        /*
         * Make sure that in DPC mode, we clear each entry for all the cores
         */
...
/**
 * \brief - Iterator init. TCAM CS tables (this function does nothing for TCAM CS)
 *
 * \param [in] unit - Relevant unit
 * \param [in] entry_handle - DBAL entry handle - holds the entry information
 *
...
/**
 * \brief - Iterator get next in TCAM CS tables
 *
 * \param [in] unit - Relevant unit
 * \param [in] entry_handle - DBAL entry handle - holds the entry information
 *
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    while ((!entry_found) && (!iterator_info->is_end))
    {
        if (iterator_info->used_first_key)
        {
            ...
            /*
             * If we are in SBC mode or in DPC mode, if core is 0, entry number is incremented
             */
            if ((entry_handle->table->core_mode == DBAL_CORE_MODE_SBC) || (entry_handle->core_id == 0))
            {
                ...
                /** Check whether all entries were iterated */
...
/**
 * \brief - Iterator deinit. TCAM CS tables (this function does nothing for TCAM CS)
 *
 * \param [in] unit - Relevant unit
 * \param [in] entry_handle - DBAL entry handle - holds the entry information
 *
