...
/**
 * \brief - dbal function definitions
 */
#define DNX_DBAL_JOURNAL_DEFS\
    SHR_FUNC_INIT_VARS(unit);\

/**
 * \brief - dbal function returns
 */
...
/*
 * \brief - notify the managers when entryl is attempted to be inserted in journal.
 */
...
/**
 * \brief check if dbak etnry should be excluded
 */
static uint8
    ...
    char *stamp)
{
    ...
    /*
     * Sanitizeer flags if handle was journaled with any flags being set
     */
...
/**
 * \brief - masks "trigger" value field data in dbal handle payload.
 */
static shr_error_e
    ...
    uint8 is_dbal_error)
{
    ...
    else
    {
        ...
        /*
         * LOG_CLI((BSL_META("entry add to journal table %s, key %d, restype %d payload %x %x %x
         * %x\n"),get_handle->table->table_name, get_handle->phy_entry.key[0], get_handle->cur_res_type,
         * get_handle->phy_entry.payload[0], get_handle->phy_entry.payload[1], get_handle->phy_entry.payload[2],
         * get_handle->phy_entry.payload[3]));
         */
    }
