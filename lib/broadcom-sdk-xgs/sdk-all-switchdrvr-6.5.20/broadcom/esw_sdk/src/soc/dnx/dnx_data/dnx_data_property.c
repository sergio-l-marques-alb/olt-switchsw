    ...
    uint32 *links)
{
    ...
    if (propval == CLEAR_VAL)
    {
        ...
        if (propval == CLEAR_VAL)
        {
            /** default value as defines in propdata.pl */
            *links = 0;
        }
        else
        {
    ...
    dnx_data_port_static_add_ucode_port_t * ucode_port)
{
    ...
    if (propval)
    {
        ...
        /*
         * Parse extra attributes (is_stif, is_kbp, flexe, etc...)
         */
        while (str_pointer && (*str_pointer == ':'))
        {
            ++str_pointer;

            /** Parse stat data interface (stif port that can receive Ethernet traffic) */
            prefix = "stat_data";
            prefix_len = sal_strlen(prefix);
            if (!sal_strncasecmp(str_pointer, prefix, prefix_len))
            {
    ...
    dnx_data_elk_connectivity_topology_t * topology)
{
    ...
    for (kbp_port = 0; kbp_port < dnx_data_elk.general.max_ports_get(unit); ++kbp_port)
    {
        ...
        {
            /*
             * below soc applied for JR2, it should be supported.
             * ext_tcam_start_lane_port0=0
             * ext_tcam_start_lane_port2=32
             * then, specify port_core_mapping[3] is
             * different from port_core_mapping[1]
             */
            if (kbp_port % 2 == 1)
            {
    ...
    uint32 *is_over_fabric)
{
    ...
    /** search for latest soc property name */
    ...
    if (propval == DEFAULT_VAL)
    {
        /*
         * preserve backward compatibility
         * If not found, search for deprecated soc property name
         */
        propval =
        ...
        if (propval == DEFAULT_VAL)
        {
            /** both soc properties weren't found */
            *is_over_fabric = 0;
        }
        else
        {
    ...
    uint32 *nof_rifs)
{
    ...
    /*
     * The number of RIFs must be divisible by the size of an outlif physical bank.
     * As the name of the SOC property suggest the MAX RIF ID and not the MAX NOF of RIFs as it should we allow it to
     * be 1 less than expected and round it up.
     */
    ...
    /*
     * The RIF template manager works in a grain size of two which force the max NOF RIFs to be even.
     * The outlif_physical_bank_size is most likely an even number so the following verification will probably pass in any case.
     */
    if ((rif_id_max & 0x1) != 0)
    {
    ...
    uint32 *dram_bitmap)
{
    ...
    *dram_bitmap = dnx_drv_soc_property_get(unit, spn_EXT_RAM_ENABLED_BITMAP, *dram_bitmap);
    ...
    {
        /*
         * Each dram has a master phy which should be active inorder for it to work.
         * Need to make sure for each dram that its master phy is in the bitmap,
         * otherwise this dram will fail to load.
         */
    ...
    uint32 *sa_per_sc_nof)
{
    ...
    *sa_per_sc_nof =
    ...
    /*
     * verify validity of the values.
     * Might be replaced by a more generic mechanism later on.
     */
