    ...
    int entry_index)
{
    ...
    else
    {
        ...
        /** no need access since it was already done inside the statment */
        access_needed = FALSE;
    }
    ...
    int *resolved_res_type)
{
    ...
    if (entry_handle->table->access_method == DBAL_ACCESS_METHOD_SW_STATE)
    {
        /**in-case the table is only SW table the buffer can be kept directly in the handle */
 ...
 *    \param [in] entry_index - index in the direct table to clear
 *  \par DIRECT OUTPUT:
 *    shr_error_e - \n
 *    Error code
 */
    ...
    dbal_entry_handle_t * entry_handle)
{
                                                   ...
                                                   key_val));
    {
        DBAL_ITERATE_OVER_CORES(entry_handle, core_id)
        {
            ...
            {
                /** this is done for tables that are MDB with result type is SW */
                break;
            }
        }
    }
...
/**
 * \brief - calculate the number of SW entries to allocate according to the table type.
 */
shr_error_e
    ...
    uint32 *num_of_entries)
{
    ...
    if (table->table_type == DBAL_TABLE_TYPE_DIRECT)
    {
        ...
        /** possible optimization is to use the num_of entries from dbal_tables_max_key_value_get, this may reduce the number
         *  of entries allocated in specific cases (if the field size and max value are very different) */
