...
#define MDB_ALIGNMENT_DONT_FIT_POSITION(position, alignment) ((MDB_IS_ODD(position) && (alignment == MDB_FEC_BANK_ALIGNMENT_EVEN)) || (!MDB_IS_ODD(position) && (alignment == MDB_FEC_BANK_ALIGNMENT_ODD)))
    ...
    int unit)
{
    ...
    mdb_parse_xml_fec_banks_alignment_types_e odd_even_align_iter;
    ...
    {
        int dhb_iter, allocated_clusters;
        int odd_count = 0;
        int macro_b_count = 0;
        uint32 fec_cluster_alloc_granularity = dnx_data_mdb.dh.dh_info_get(unit,
                                                                           mdb_fec_hierarchy_index_to_fec_table_map
        ...
        for (dhb_iter = dnx_data_mdb.dh.macro_type_info_get(unit, MDB_MACRO_B)->global_start_index;
             dhb_iter < dnx_data_mdb.dh.total_nof_macroes_get(unit); dhb_iter++)
        {
            odd_count += MDB_IS_ODD(hier_info[fec_hier_iter].macro_info[dhb_iter].count);
            macro_b_count += hier_info[fec_hier_iter].macro_info[dhb_iter].count;
        }
        ...
        high_granularity_required_for_one_or_more_hier |= (!hier_info[fec_hier_iter].low_granularity_suppport);
    ...
    for (odd_even_align_iter = 0; odd_even_align_iter < MDB_FEC_BANK_ALIGNMENT_BOTH; odd_even_align_iter++)
    {
        ...
        {
            if ((hier_info[fec_hier_iter].low_granularity_suppport) && (hier_info[fec_hier_iter].valid_alloc == FALSE)
                && ((hier_info[fec_hier_iter].alignments[MDB_FEC_BANK_GRANULARITY_LOW] == odd_even_align_iter)
                    || (hier_info[fec_hier_iter].alignments[MDB_FEC_BANK_GRANULARITY_LOW] ==
                        MDB_FEC_BANK_ALIGNMENT_BOTH)))
            {
                bank_top_pointer -= hier_info[fec_hier_iter].nof_banks;

                if (MDB_ALIGNMENT_DONT_FIT_POSITION
                    ((bank_top_pointer + 1), hier_info[fec_hier_iter].alignments[MDB_FEC_BANK_GRANULARITY_LOW]))
                {
                    bank_top_pointer--;
                }
    ...
    {
        bank_top_pointer = UTILEX_MIN(bank_top_pointer, high_granularity_max_bank) + 1;
    }
    ...
    while (bank_bottom_pointer >= 0)
    {
        ...
        uint32 start_offset;
        uint32 increment_was_done = FALSE;
        uint8 tmp_valid[DNX_DATA_MAX_MDB_DIRECT_NOF_FEC_ECMP_HIERARCHIES] =
            { FALSE, hier_info[DBAL_ENUM_FVAL_HIERARCHY_LEVEL_HIERARCHY_LEVEL_2].valid_alloc,
            hier_info[DBAL_ENUM_FVAL_HIERARCHY_LEVEL_HIERARCHY_LEVEL_3].valid_alloc
        };
        ...
        for (start_offset = 0; start_offset < 2; start_offset++)
        {
            ...
            if (bank_bottom_pointer_tmp + hier_info[DBAL_ENUM_FVAL_HIERARCHY_LEVEL_HIERARCHY_LEVEL_1].nof_banks >
                high_granularity_max_bank)
            {
                if (!hier_info[DBAL_ENUM_FVAL_HIERARCHY_LEVEL_HIERARCHY_LEVEL_1].low_granularity_suppport)
                {
                    break;
                }
                granularity = MDB_FEC_BANK_GRANULARITY_LOW;
            }
            else
            {
                granularity = MDB_FEC_BANK_GRANULARITY_HIGH;
            }

            if (MDB_ALIGNMENT_DONT_FIT_POSITION
                (bank_bottom_pointer_tmp,
                 hier_info[DBAL_ENUM_FVAL_HIERARCHY_LEVEL_HIERARCHY_LEVEL_1].alignments[granularity]))
            {
                bank_bottom_pointer_tmp++;
            }
            else
        ...
        hier_info[DBAL_ENUM_FVAL_HIERARCHY_LEVEL_HIERARCHY_LEVEL_1].first_bank = bank_bottom_pointer_tmp;
        ...
        bank_bottom_pointer_tmp =
            MDB_GET_NEXT_BANK_POINTER((hier_info[DBAL_ENUM_FVAL_HIERARCHY_LEVEL_HIERARCHY_LEVEL_1].first_bank +
                                      ...
                                      dnx_data_l3.feature.feature_get(unit,
                                                                      dnx_data_l3_feature_fer_fec_granularity_double_size));

        while ((!tmp_valid[DBAL_ENUM_FVAL_HIERARCHY_LEVEL_HIERARCHY_LEVEL_2]
                || !tmp_valid[DBAL_ENUM_FVAL_HIERARCHY_LEVEL_HIERARCHY_LEVEL_3]))
        {
            found = FALSE;
            for (fec_hier_iter = DBAL_ENUM_FVAL_HIERARCHY_LEVEL_HIERARCHY_LEVEL_2;
                 fec_hier_iter < dnx_data_mdb.direct.nof_fec_ecmp_hierarchies_get(unit); fec_hier_iter++)
            {
                if ((!tmp_valid[fec_hier_iter])
                    && !MDB_ALIGNMENT_DONT_FIT_POSITION(bank_bottom_pointer_tmp,
                                                        hier_info[fec_hier_iter].alignments
                                                        [MDB_FEC_BANK_GRANULARITY_HIGH]))
                {
                    tmp_valid[fec_hier_iter] = TRUE;
        ...
        if ((bank_bottom_pointer_tmp > bank_top_pointer)
            || (!tmp_valid[DBAL_ENUM_FVAL_HIERARCHY_LEVEL_HIERARCHY_LEVEL_2])
            || (!tmp_valid[DBAL_ENUM_FVAL_HIERARCHY_LEVEL_HIERARCHY_LEVEL_3]))
        {
            bank_bottom_pointer--;
        }
    ...
    for (fec_hier_iter = 0; fec_hier_iter < dnx_data_mdb.direct.nof_fec_ecmp_hierarchies_get(unit); fec_hier_iter++)
    {
        ...
        uint32 bank_pointer;
        uint32 step_count;
        uint32 range_start, range_size, cluster_count = 0;
        int position = 0;
        int start_address;
        int end_address;
        ...
        {
            if (hier_info[fec_hier_iter].first_bank + hier_info[fec_hier_iter].nof_banks > high_granularity_max_bank)
            {
        ...
        macro_b_nof_rows = dnx_data_mdb.dh.macro_type_info_get(unit, MDB_MACRO_B)->nof_rows;
        for (macro_global_index = 0; macro_global_index < dnx_data_mdb.dh.total_nof_macroes_get(unit);
             macro_global_index++)
        {
            step_count =
                (macro_global_index < dnx_data_mdb.dh.macro_type_info_get(unit, MDB_MACRO_A)->nof_macros) ? 2 : 1;

            for (cluster_iter = 0; cluster_iter < hier_info[fec_hier_iter].macro_info[macro_global_index].count;
                 cluster_iter++)
            {
                bank_pointer =
                    hier_info[fec_hier_iter].macro_info[macro_global_index].position[cluster_iter] + offset +
                    hier_info[fec_hier_iter].first_bank;
                start_address = bank_pointer * macro_b_nof_rows;
                end_address = (bank_pointer + step_count) * macro_b_nof_rows;
                cluster_count += step_count;
                ...
                for (fec_cluster_iter = 0; fec_cluster_iter < fec_cluster_alloc_granularity; fec_cluster_iter++)
                {
                    SHR_IF_ERR_EXIT(mdb_db_infos.db.clusters_info.
                                    start_address.set(unit, mdb_fec_hierarchy_index_to_fec_table_map[fec_hier_iter],
                                                      hier_info[fec_hier_iter].
                                                      macro_info[macro_global_index].sw_state_matching_cluster_id
                                                      [cluster_iter] + fec_cluster_iter, start_address));
                    SHR_IF_ERR_EXIT(mdb_db_infos.db.clusters_info.
                                    end_address.set(unit, mdb_fec_hierarchy_index_to_fec_table_map[fec_hier_iter],
                                                    hier_info[fec_hier_iter].
                                                    macro_info[macro_global_index].sw_state_matching_cluster_id
                                                    [cluster_iter] + fec_cluster_iter, end_address));
                }

            }
        }

        range_start = MDB_CALC_FEC_ID_START_ADDR_OF_BANK_ID(unit, hier_info[fec_hier_iter].first_bank);

        range_size =
            MDB_CALC_FEC_ID_START_ADDR_OF_BANK_ID(unit,
                                                  (hier_info[fec_hier_iter].first_bank + cluster_count)) - range_start;

        SHR_IF_ERR_EXIT(mdb_db_infos.fec_hierarchy_info.fec_id_range_start.set(unit, fec_hier_iter, range_start));

        SHR_IF_ERR_EXIT(mdb_db_infos.fec_hierarchy_info.fec_id_range_size.set(unit, fec_hier_iter, range_size));
    }
    ...
    int *cluster_count)
{
    ...
    if (is_dha == 1)
    {
        macro_idx_max = dnx_data_mdb.dh.macro_type_info_get(unit, MDB_MACRO_A)->nof_macros;
        sal_strcpy(dh_name_compare, "dha");
    }
    else
    {
        macro_idx_max = dnx_data_mdb.dh.macro_type_info_get(unit, MDB_MACRO_B)->nof_macros;
    ...
    for (phydb_name_idx = 0; phydb_name_idx < DBAL_NOF_ENUM_MDB_PHYSICAL_TABLE_VALUES; phydb_name_idx++)
    {
        if (phydb_names[phydb_name_idx] != NULL)
        {
                ...
                || (current_mdb_table == DBAL_ENUM_FVAL_MDB_PHYSICAL_TABLE_BIG_KAPS_BB_2))
            {
                ...
                {
                    mdb_kaps_ip_db_id_e db_idx = (requested_kaps_cfg / MDB_LPM_KAPS_CFG_SMALL_KAPS_ENABLE) - 1;
                    dbal_enum_value_field_mdb_physical_table_e small_kaps_mdb_table;
                    void *small_kaps_phydb;
                    int small_kaps_name_idx;

                    if ((db_idx >= MDB_KAPS_IP_NOF_DB)
                        || ((requested_kaps_cfg % MDB_LPM_KAPS_CFG_SMALL_KAPS_ENABLE) != 0))
                    {
                        SHR_ERR_EXIT(_SHR_E_CONFIG,
                                     ...
                                     MDB_KAPS_IP_NOF_DB * MDB_LPM_KAPS_CFG_SMALL_KAPS_ENABLE,
                                     MDB_LPM_KAPS_CFG_SMALL_KAPS_ENABLE);
                    }

                    small_kaps_mdb_table = DBAL_ENUM_FVAL_MDB_PHYSICAL_TABLE_BIG_KAPS_BB_1 + db_idx;
                    small_kaps_name_idx = phydb_name_idx + (small_kaps_mdb_table - current_mdb_table);

                    if (small_kaps_name_idx >= DBAL_NOF_ENUM_MDB_PHYSICAL_TABLE_VALUES)
                    {
                        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Internal error, small_kaps_name_idx (%d) calculation failed.\n",
                                     small_kaps_name_idx);
                    }

                    small_kaps_phydb = dbx_xml_child_get_first(curTop, phydb_names[small_kaps_name_idx]);
            ...
            if (current_mdb_table == DBAL_ENUM_FVAL_MDB_PHYSICAL_TABLE_EEDB_5_6_DATA
                || current_mdb_table == DBAL_ENUM_FVAL_MDB_PHYSICAL_TABLE_EEDB_7_8_DATA)
            {
                nof_cluster_to_allocate += max_nof_macro_clusters;
            }
            else if (current_mdb_table == DBAL_ENUM_FVAL_MDB_PHYSICAL_TABLE_FEC_1
    ...
    void *curTop)
{
    ...
    for (phase_iter = DBAL_ENUM_FVAL_MDB_EEDB_PHASE_PHASE_1; phase_iter < DBAL_NOF_ENUM_MDB_EEDB_PHASE_VALUES;
         phase_iter++)
    {
        mdb_cluster_info_t clusters[DNX_DATA_MAX_MDB_DH_MAX_NOF_CLUSTERS * DNX_DATA_MAX_MDB_PDBS_MAX_NOF_INTERFACE_DHS];
    ...
    RHDATA_ITERATOR(current_eedb_phydb, eedb_phydb, "lldb_mem_block")
    {
        ...
        cluster.end_address = offset + dnx_data_mdb.dh.macro_type_info_get(unit, MDB_EEDB_BANK)->nof_rows;
    ...
    dbal_enum_value_field_mdb_physical_table_e mdb_physical_table_id)
{
    ...
    if (nof_ways > 0)
    {
        ...
        uint32 row_width = dnx_data_mdb.dh.dh_info_get(unit, mdb_physical_table_id)->row_width;
        ...
        for (encoding_type_iter = 0; encoding_type_iter < DNX_DATA_MAX_MDB_EM_NOF_ENCODING_TYPES; encoding_type_iter++)
        {
            ...
            capacity_estimate = MDB_EM_ADAPTER_SIZE;
            capacity_estimate_avg = MDB_EM_ADAPTER_SIZE;
#endif
            ...
            SHR_IF_ERR_EXIT(mdb_db_infos.
                            em_entry_capacity_estimate_average.set(unit, mdb_direct_mdb_to_dbal[mdb_physical_table_id],
                                                                   encoding_type_iter, capacity_estimate_avg));
        }
    }
                ...
                if (macros[macro_global_index].intf[interface_iter].nof_cluster_allocated > 0)
                {
                    ...
                    for (clusters_bundle_iter = 0; clusters_bundle_iter < nof_bundles; clusters_bundle_iter++)
                    {
                             ...
                             cluster_idx++)
                        {
                            ...
                            {
                                int start_address = cluster_accumulation * macro_b_nof_rows;
                                end_address = (cluster_accumulation + nof_macro_b) * macro_b_nof_rows;
                                SHR_IF_ERR_EXIT(mdb_db_infos.db.clusters_info.
