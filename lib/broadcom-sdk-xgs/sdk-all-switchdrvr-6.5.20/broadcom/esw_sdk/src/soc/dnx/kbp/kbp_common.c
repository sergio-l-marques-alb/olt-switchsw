...
/*
 * Utility macro for checking and updating the indirect commit mode.
 * Its only purpose is to minimize the code footprint and make it more readable.
 */
#define DNX_KBP_INDIRECT_COMMIT_MODE_CHECK_AND_UPDATE(_db_entry_, _mode_) \
do { \
    /*
     * In case is_for_update == DBAL_INDIRECT_COMMIT_MODE_FORCE:
     * Add the entry if it doesn't exist and update it if it does exist.
...
/*
 * Utility function to handle entry update, taking into consideration the existence of multiple AD DBs
 * Possible cases:
 * 1. New entry payload is in the same AD DB as the old entry
 *      In case of 0b AD DB, do nothing
 *      In case of regular or optimized AD DB, update the AD entry
 *      Indicate that the new entry doesn't need to be added
 * 2. New entry payload is in different AD DB than the than the old entry
 *      Delete the old entry and indicate that the new entry needs to be added
 */
static shr_error_e
    ...
    uint8 *add_entry)
{
    ...
    /*
     * KBP can have 3 AD DBs: regular, optimized and 0b size.
    ...
    {
        /** The AD entry is in the same AD DB. Update it only if it's not 0b. */
    ...
    uint32 *op2_payload_size)
{
    ...
    /** Check if the entry is installed or is pending. Exit with NOT_FOUND if it's pending (not installed) */
    res = kbp_entry_get_info(db_p, db_entry, &entry_info);
    if (res == KBP_PENDING_ENTRY)
    {
        /** Entry is pending. Exit without error print. */
        SHR_SET_CURRENT_ERR(_SHR_E_NOT_FOUND);
        SHR_EXIT();
    }
    ...
    {
        /** If hitbit is not configured for this entry, db_entry will be NULL and the hitbit won't be retrieved  */
        DNX_KBP_TRY(kbp_entry_get_hb(db_p, db_entry, &hb_entry));
    }

    sal_memset(asso_data, 0x0, sizeof(asso_data[0]) * payload_size_in_bytes);

    /** If ad_db_p is not provided, use the ad_db_p from the entry info */
    if (ad_db_p == NULL)
    {
    ...
    kbp_db_handles_t * op2_kbp_handles)
{
    ...
    /** Get the db_entry to perform indirect commit mode validation */
    kbp_db_get_prefix_handle(db_p, data, prefix_length, &db_entry);

    /** Validate and update the commit action */
    ...
    if ((*indirrect_commit_mode) != DBAL_INDIRECT_COMMIT_MODE_NORMAL)
    {
        ...
        {
            /** KAPS has no second AD DB. Update the AD entry. */
            DNX_KBP_TRY(kbp_ad_db_update_entry(ad_db_p, ad_entry, asso_data));
            add_entry = FALSE;
        }
        else
        {
            /** KBP can have multiple AD DBs. This API is handling the update. */
    ...
    if (add_entry)
    {
...
#if 0
        /*
         * Below code is needed when public tables are supported.
         * It is needed to correctly set the entry priority according to this schema:
         * Highest priority > PUBLIC -> PRIVATE -> PUBLIC DEFAULT -> PRIVATE DEFAULT < Lowest priority
         */
        ...
        /** Do not create an AD entry when payload size is zero. The 0b entry created during init will be used. */
        if (op2_kbp_handles && (ad_db_p == op2_kbp_handles->ad_db_zero_size_p))
        {
            /** Do not roll back the ad entry for zero size AD database (As we created only one entry) */
    ...
    struct kbp_entry *op2_db_entry_in)
{
    ...
    /** When db_entry_in is provided, there is no need to retrieve it again */
    if (op2_db_entry_in == NULL)
    {
        /** db_entry_in was not provided, retrieve it */
        SHR_IF_ERR_EXIT(dnx_kbp_buffer_dbal_to_kbp(unit, key_size_in_bits, key, data));
        DNX_KBP_TRY(kbp_db_get_prefix_handle(db_p, data, prefix_length, &db_entry));
    }
    else
    {
        /** Use the provided db_entry_in */
    ...
    int unit)
{
    ...
    /*
     * In bare-metal, not all images support KBP - means they do not define the KBP DBs
     * As a general solution, KBP will not be initialized over adapter for bare-metal.
     * Bare-metal images that should run KBP will run it over device
     */
