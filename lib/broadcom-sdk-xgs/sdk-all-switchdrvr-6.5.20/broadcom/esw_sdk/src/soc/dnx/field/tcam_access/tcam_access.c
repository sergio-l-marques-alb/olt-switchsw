    ...
    uint8 *entry_hit_p)
{
    ...
    *entry_hit_p = 0;
    ...
    /** Set the SW state hit indication in case hitbit_volatile is set for current device. */
    ...
    uint32 entry_handle)
{
    ...
    /** On adapter we perform direct write, on device indirect. */
#ifdef ADAPTER_SERVER_MODE
    /** On adapter only core 0 is available, no need to do change the value for other cores. */
    if (core_id == 0)
    {
        SHR_IF_ERR_EXIT(soc_mem_array_read
                        (unit, mem, bank_sram_id, TCAM_BLOCK(unit, core_id), bank_line_id, &mem_entry_data));
...
/**
 * \brief
 *  Writes a single-sized entry payload to TCAM at the given bank_id/bank_offset.
...
/**
 * \brief
 *  Writes a single-sized entry key to TCAM at the given bank_id/bank_offset.
 *  The write operation is performed on TCAM HW at the given core.
 *
 * \param [in] unit        - Device ID
 * \param [in] core        - Core ID
    ...
    uint8 is_update)
{
    ...
    if (db_info.key_size == dnx_data_field.tcam.key_size_double_get(unit))
    {
        ...
        {
            /*
             * In case of Double-sized key update, invalidating first bank entry ensures traffic safeness 
             */
