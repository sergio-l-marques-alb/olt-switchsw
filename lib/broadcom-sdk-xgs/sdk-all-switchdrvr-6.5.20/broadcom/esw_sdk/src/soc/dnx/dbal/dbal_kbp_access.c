    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    {
        /*
         * The result is optimized. Check if it's also 0b size.
         * access_id_set indicates that the user requested the entry to be added with 0b size.
         */
        ...
        if (kbp_handles->ad_db_zero_size_p)
        {
            ...
            /** converting to bytes align */
            payload_size = (BITS2BYTES(payload_size));
            if (!sal_memcmp(all_zeros, asso_data, payload_size))
            {
                /** Payload data is all zeros. Use 0b result. Update the payload size. */
    ...
    uint32 *payload_size)
{
    ...
    {
        /** Entry is pending (not installed) */
        SHR_SET_CURRENT_ERR(_SHR_E_NOT_FOUND);
        SHR_EXIT();
    }
...
/**
 * \brief
 *   Iterates over all KBP DBs swstate and perform caching if the DB complies with the caching requirements:
 *   caching for this DB is enabled and the DB corresponds to the input FWD or ACL indication.
 *   \param [in] unit - Relevant unit.
    ...
    int is_acl)
{
    ...
    for (db_id = 0; db_id < DNX_KBP_MAX_NOF_DBS; db_id++)
    {
        ...
        {
            /** Perform install for all pending entries of this DB */
            ...
            /*
             * Delete the AD entries, which are no longer attached to a DB entry due to deletion of the DB entry.
             * Zero size AD DB entries should not be touched.
             */
            if (op2_kbp_handles.ad_db_p[DNX_KBP_AD_DB_INDEX_REGULAR] != NULL)
            {
