...
#define MDB_EM_MACT_REFRESH_POLL_ATTEMPTS 50000
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(READ_MACT_INDIRECT_COMMANDr(unit, 0, &reg_val));
    soc_reg_field_set(unit, MACT_INDIRECT_COMMANDr, &reg_val, INDIRECT_COMMAND_TIMEOUTf, timeout_val);
    SHR_IF_ERR_EXIT(WRITE_MACT_INDIRECT_COMMANDr(unit, 0, reg_val));
    ...
    SHR_IF_ERR_EXIT(READ_DDHA_INDIRECT_COMMANDr(unit, REG_PORT_ANY, &reg_val));
    ...
    {
        SHR_IF_ERR_EXIT(WRITE_DDHA_INDIRECT_COMMANDr(unit, block_iter, reg_val));
    }

    SHR_IF_ERR_EXIT(READ_DDHB_INDIRECT_COMMANDr(unit, REG_PORT_ANY, &reg_val));
    ...
    {
        SHR_IF_ERR_EXIT(WRITE_DDHB_INDIRECT_COMMANDr(unit, block_iter, reg_val));
    }
    ...
    if (dnx_data_mdb.dh.nof_dhc_blocks_get(unit) != 0)
    {
        ...
        {
            SHR_IF_ERR_EXIT(WRITE_DHC_INDIRECT_COMMANDr(unit, block_iter, reg_val));
        }
    }
    ...
    dbal_physical_entry_t * entry)
{
    ...
    if (MDB_EM_TABLE_IS_EMP(mdb_physical_table_id))
    {
        ...
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC
                        (unit,
                         dnx_data_mdb.em.mdb_emp_tables_info_get(unit, mdb_physical_table_id)->mdb_22_table,
        ...
        if (scan_machine_auto_en != 0)
        {
            ...
            soc_field_t emp_scan_status_field =
                dnx_data_mdb.em.em_info_get(unit, dbal_physical_table_id)->emp_scan_status_field;
    ...
    int *retry)
{
    ...
    *retry = FALSE;
    ...
    {
        SHR_IF_ERR_EXIT(READ_MACT_LARGE_EM_REPLYr(unit, 0, data));
        ...
        if ((command == MDB_EM_MACT_CMD_REFRESH) && (attempt_counter >= MDB_EM_MACT_REFRESH_POLL_ATTEMPTS)
            && (dnx_data_mdb.feature.feature_get(unit, dnx_data_mdb_feature_em_mact_use_refresh_on_insert)))
        {
            *retry = TRUE;
            SHR_EXIT();
        }
            ...
            && (dnx_data_mdb.feature.feature_get(unit, dnx_data_mdb_feature_em_mact_delete_transplant_no_reply)))
        {
            ...
            SHR_IF_ERR_EXIT(READ_MACT_LARGE_EM_ERROR_DELETE_NON_EXIST_COUNTERr(unit, 0, delete_counter_data));
    ...
    uint32 flags)
{
    ...
    if (is_update)
    {
        ...
        get_entry.hitbit = 0;
        get_entry.age = 0;
        rv_get = mdb_em_entry_get(unit, dbal_physical_table_id_arr[0], app_id, &get_entry, 0);
            ...
            || (entry->mdb_action_apply == DBAL_MDB_ACTION_APPLY_HW_ONLY))
        {
            ...
            if (is_mact)
            {
                SHR_IF_ERR_EXIT(mdb_em_mact_get_physical_memory
                                (unit, &mdb_physical_memory_arr[table_iter],
            ...
            do
            {
                if (dnx_data_mdb.feature.feature_get(unit, dnx_data_mdb_feature_em_sbus_interface_shutdown))
                {
                    ...
                    if (is_mact)
                    {
                        ...
                        if ((retry == TRUE) && (command == MDB_EM_MACT_CMD_REFRESH)
                            && (dnx_data_mdb.
                                feature.feature_get(unit, dnx_data_mdb_feature_em_mact_use_refresh_on_insert)))
                        {
                            uint32 insert_command = MDB_EM_MACT_CMD_INSERT;

                            soc_mem_field_set(unit, mdb_physical_memory_arr[table_iter], data, COMMANDf,
                                              &insert_command);
                        }
                    }
                    else
                ...
                else
                {
                    ...
                    if ((!is_mact)
                        ...
                        (dnx_sbusdma_desc_is_enabled
                         (unit, mdb_em_dbal_table_to_dma_enum(dbal_physical_table_id_arr[table_iter])) == TRUE))
                    {
    ...
    {
        if ((dbal_physical_table_id_arr[0] == DBAL_PHYSICAL_TABLE_GLEM_1) ||
            (dbal_physical_table_id_arr[0] == DBAL_PHYSICAL_TABLE_GLEM_2))
        {
            SHR_IF_ERR_EXIT(adapter_mdb_access_em_write(unit, dbal_physical_table_id_arr[0], app_id, entry));
        }
        else
        {
            for (table_iter = 0; table_iter < tables_num; table_iter++)
            {
                SHR_IF_ERR_EXIT(adapter_mdb_access_em_write
                                (unit, dbal_physical_table_id_arr[table_iter], app_id, entry));
            }
        }
    }
    ...
    dbal_enum_value_field_mdb_emp_table_e * mdb_emp_table_id)
{
    ...
    {
        case DBAL_PHYSICAL_TABLE_LEM:
            *mdb_emp_table_id = DBAL_ENUM_FVAL_MDB_EMP_TABLE_LEM;
            break;
        case DBAL_PHYSICAL_TABLE_SEXEM_3:
            *mdb_emp_table_id = DBAL_ENUM_FVAL_MDB_EMP_TABLE_EXEM_3;
            break;
        case DBAL_PHYSICAL_TABLE_LEXEM:
            *mdb_emp_table_id = DBAL_ENUM_FVAL_MDB_EMP_TABLE_EXEM_4;
            break;
        default:
            SHR_ERR_EXIT(_SHR_E_PARAM, "%s is not an MDB EMP table.\n",
                         dbal_physical_table_to_string(unit, dbal_physical_table_id));
    }
        ...
        if (found && retrieve_hitbit_or_age)
        {
            ...
            do
            {
                ...
                if (entry->age & DBAL_PHYSICAL_KEY_AGE_GET)
                {
                    ...
                    SHR_IF_ERR_EXIT(mdb_em_dbal_table_to_emp(unit, dbal_physical_table_id, &mdb_emp_table_id));
                    max_nof_age_entry_bits =
                        dnx_data_mdb.em.em_aging_info_get(unit, mdb_emp_table_id)->max_nof_age_entry_bits;
    ...
    uint32 flags)
{
    ...
    for (table_iter = 0; table_iter < tables_num; table_iter++)
    {
        ...
        if ((entry->mdb_action_apply == DBAL_MDB_ACTION_APPLY_ALL)
            || (entry->mdb_action_apply == DBAL_MDB_ACTION_APPLY_SW_SHADOW))
        {
            SHR_IF_ERR_EXIT(mdb_em_shadow_remove_key(unit, dbal_physical_table_id_arr[table_iter], app_id, entry));
            ...
            if (entry_encoding == MDB_EM_NOF_ENTRY_ENCODINGS)
            {
                ...
                SHR_IF_ERR_EXIT(mdb_em_db.vmv_info.
                                app_id_size.get(unit, dbal_physical_table_id_arr[table_iter], app_id, &app_id_size));
    ...
    uint32 app_id)
{
    ...
    for (table_iter = 0; table_iter < tables_num; table_iter++)
    {
        SHR_IF_ERR_EXIT(MDB_EM_SW_ACCESS.shadow_em_db.delete_all(unit, physical_tables[table_iter]));
    }
exit:
    SHR_FUNC_EXIT;
}
    ...
    int full_time)
{
    ...
    for (dbal_logical_table_id = 0; dbal_logical_table_id < DBAL_NOF_TABLES; dbal_logical_table_id++)
    {
             ...
             physical_table_iter++)
        {
            if (dbal_logical_table->physical_db_id[physical_table_iter] == dbal_physical_table_id)
            {
                ...
                SHR_IF_ERR_EXIT(mdb_em_db.vmv_info.
                                max_payload_size.get(unit, dbal_physical_table_id, dbal_logical_table->app_id,
                                                     &payload_size_int));
    ...
    else
    {
        ...
        {
            SHR_IF_ERR_EXIT(mdb_em_test_iterator(unit, dbal_logical_table, physical_table_iter));
        }
        else
        {
            SHR_IF_ERR_EXIT(mdb_em_test_table
                            (unit, dbal_logical_table, physical_table_iter, mode, delete_cmd, full_time));
