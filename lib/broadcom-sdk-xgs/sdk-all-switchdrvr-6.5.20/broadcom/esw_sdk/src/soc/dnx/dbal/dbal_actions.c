    ...
    dbal_entry_handle_t * entry_handle)
{
         ...
         dbal_tables_is_merge_entries_supported(unit, entry_handle->table_id)) || entry_handle->er_flags)
    {
        ...
        /** if entry exists, and we are doing error recovery we need to update get_entry_copy for later to add to jurnal
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    if (DBAL_TABLE_IS_NONE_DIRECT(entry_handle->table))
    {
                ...
                || entry_handle->er_flags))
        {
            ...
            {
                /*
                 * If the table is with disabled learning, it means all the entries are static, and can be read from SW 
                 * only. If not, get operation needs to be done from HW 
                 */
                ...
                /*
                 * This handle will be used for the recovery journal, and thus needs to be HW+SW 
                 */
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    /** in-case that non direct mode is optimized no need to perfrom get - we trust the input.. */
          ...
          !(DBAL_HANDLE_IS_MERGE_NEEDED(entry_handle))) || entry_handle->er_flags)
    {
        ...
        /*
         * If the table is with disabled learning, it means all the entries are static, and can be read from SW only.
         * If not, get operation needs to be done from HW 
         */
        if (SHR_IS_BITSET(entry_handle->table->indications_bm, DBAL_TABLE_IND_IS_LEARNING_EN))
        {
            is_sw_get = 0;
    ...
    if ((action != DBAL_ACTION_TABLE_CLEAR) && (action != DBAL_ACTION_ENTRY_GET_ACCESS_ID))
    {
        if (DBAL_TABLE_IS_TCAM(table) && !DBAL_ACTIONS_IS_TCAM_BY_KEY(table))
        {
            if (entry_handle->access_id_set == 0)
            {
                ...
                {
                    /** for KBP tables it is not supported adding entry with ID, the ID is retreived from the KBP SDK */
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    {
        /**
         * Already set, no need to resolve it This flow can happen when setting the result type that we expect to get and still
         * call 'get_all_fields'
         */
        SHR_EXIT();
    }
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    if (SHR_IS_BITSET(entry_handle->table->indications_bm, DBAL_TABLE_IND_IS_ITERATOR_OPTIMIZED))
    {
        ...
        {
            /** currently for EM tables only global rif is supported for interator optimized */
            if (entry_handle->table_id == DBAL_TABLE_GLOBAL_RIF_EM)
            {
    ...
    dbal_iterator_info_t * iterator_info)
{
    ...
    {
        /*
         * FWD_MACT is running through flush machine, so setting the get hit rule only when flush
         * machine is ended
         */
