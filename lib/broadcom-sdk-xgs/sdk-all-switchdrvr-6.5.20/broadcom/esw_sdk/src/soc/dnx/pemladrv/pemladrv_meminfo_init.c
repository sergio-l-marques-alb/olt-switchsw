/** \file src/soc/dnx/pemladrv/pemladrv_meminfo_init.c
 *
...
#if (_MSC_VER >= 1400)
#pragma warning(push)
#pragma warning(disable: 4996) /* To avoid compiler warning (4996 for _CRT_SECURE_NO_WARNINGS equivalent)*/
#endif
#endif
...
void free_db_direct_container(const int unit) {
  ...
  {
    if (NULL == api_info[unit].db_direct_container.db_direct_info_arr[i].result_chunk_mapper_matrix_arr){    /*In case db was not mapped*/
      continue;
    }
...
void free_db_tcam_container(const int unit)
{
  ...
  {
    if (NULL == api_info[unit].db_tcam_container.db_tcam_info_arr[i].result_chunk_mapper_matrix_arr){     /*In case db was not mapped*/
...
void free_db_lpm_container(const int unit)
{
  ...
  {
    if (NULL == api_info[unit].db_lpm_container.db_lpm_info_arr[i].logical_lpm_info.result_chunk_mapper_matrix_arr){      /*In case db was not mapped*/
...
void free_db_em_container(const int unit)
{
  ...
  {
    if (NULL == api_info[unit].db_em_container.db_em_info_arr[i].logical_em_info.result_chunk_mapper_matrix_arr){      /*In case db was not mapped*/
