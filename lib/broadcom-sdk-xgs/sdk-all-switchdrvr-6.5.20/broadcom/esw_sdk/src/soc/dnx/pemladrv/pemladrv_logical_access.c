/** \file src/soc/dnx/pemladrv/pemladrv_logical_access.c
 *
...
/*
STATIC void dnx_pemladrv_get_db_name(const int unit, DbType_t db_type, const int db_id, char** target_name){
  switch (db_type){
    case PEMLA_DB_TYPE_DIRECT_DB:{dnx_pemladrv_get_reg_or_reg_field_name(api_info[unit].db_direct_container.db_direct_info_arr[db_id].name, 0, target_name); break; }
    case PEMLA_DB_TYPE_TCAM_DB:  {dnx_pemladrv_get_reg_or_reg_field_name(api_info[unit].db_tcam_container.db_tcam_info_arr[db_id].name, 0, target_name); break; }
    case PEMLA_DB_TYPE_LPM_DB:   {dnx_pemladrv_get_reg_or_reg_field_name(api_info[unit].db_lpm_container.db_lpm_info_arr[db_id].logical_lpm_info.name, 1, target_name); break; }
    case PEMLA_DB_TYPE_EM_DB:    {dnx_pemladrv_get_reg_or_reg_field_name(api_info[unit].db_em_container.db_em_info_arr[db_id].logical_em_info.name, 1, target_name); break; }
  }
}
*/
    ...
    int *index)
{
  ...
  dnx_build_virtual_db_data(0, key_fields_arr, key, virtual_key_data);    /*needed only to search for existing key in db*/
    ...
    pemladrv_mem_t * result)
{
  ...
  /*if(api_info[unit].db_em_container.db_em_info_arr[table_id].em_cache.next_free_index == index)*/
    ...
    int *index)
{
  ...
  *index = dnx_pemladrv_does_key_exists_in_physical_lpm_db(&api_info[unit].db_lpm_container.db_lpm_info_arr[table_id].logical_lpm_info, uint_len_in_bits, &api_info[unit].db_lpm_container.db_lpm_info_arr[table_id].lpm_cache, virtual_key_data);
  if (*index != -1) {
    ...
    sal_free(virtual_key_data);
    return dnx_pem_lpm_set_by_id(unit, table_id, *index, key, &length_in_bits, result);/*return error of key already existing*/
  }
  sal_free(virtual_key_data);
    ...
    unsigned int **virtual_key_data)
{
  ...
  *virtual_key_mask = (unsigned int*)sal_calloc((*virtual_key_mask), (size_t)pemladrv_ceil(total_width, UINT_WIDTH_IN_BITS), sizeof(uint32));
  ...
  dnx_build_virtual_db_mask(key_width,     field_bit_range_arr,              mask,      *virtual_key_mask);  /*Done for Mask. includes the offset of the key*/
  dnx_build_virtual_db_mask(2 * key_width, valid_field_bit_range_arr,        valid,     *virtual_key_mask);  /*Done for Valid. includes the offset of the key and the mask*/
  ...
  *virtual_key_data = (unsigned int*)sal_calloc((*virtual_key_data), (size_t)pemladrv_ceil(total_width, UINT_WIDTH_IN_BITS), sizeof(uint32));
  ...
  dnx_build_virtual_db_data(  key_width, field_bit_range_arr,              mask,      *virtual_key_data);  /*Done for Mask. includes the offset of the key*/
  dnx_build_virtual_db_data(2*key_width, valid_field_bit_range_arr,        valid,     *virtual_key_data);  /*Done for Valid. includes the offset of the key and the mask*/
    ...
    unsigned int **virtual_result_data)
{
  ...
  *virtual_result_mask = (unsigned int*)sal_calloc((*virtual_result_mask), (size_t)pemladrv_ceil(total_width, UINT_WIDTH_IN_BITS), sizeof(uint32));
  ...
  if (is_init_mask_to_ones_at_field_location) {
    dnx_build_virtual_db_mask(0, field_bit_range_arr, result, *virtual_result_mask);       /*offset for RESULT is 0 */
  }
    ...
    PhysicalWriteInfo ** target_physical_memory_entry_data_list)
{
  ...
  entry_data_arr_nof_entries += 1; /*For all those damn ECC bits*/
    ...
    unsigned int *virtual_db_data_arr)
{
  ...
  const unsigned int is_egress = 0; /*is_ingress bit is not relavant fot this case*/
    ...
    unsigned int *virtual_db_data_arr)
{
  ...
  unsigned int* read_data;
  phy_mem_t*    phy_mem;
  const unsigned int is_ingress = 0;  /*is_ingress bit is a don't-care in this case*/
  const unsigned int is_mem = 1;
  read_data = (unsigned int*)sal_calloc(read_data, 2, sizeof(unsigned int));
  phy_mem   = (phy_mem_t*)sal_calloc(phy_mem, 1, sizeof(phy_mem_t));
...
void dnx_pemladrv_update_lpm_cache_from_physical(int unit) {
  ...
  for (index = 0; index < api_info[unit].db_lpm_container.nof_lpm_dbs; ++index) {
    if (NULL == api_info[unit].db_lpm_container.db_lpm_info_arr[index].logical_lpm_info.result_chunk_mapper_matrix_arr)     /*In case db was not mapped*/
      continue;
...
#else
    tcam_data          = (unsigned int *) calloc((size_t)ceil((2.0f*total_key_width + 1) / UINT_WIDTH_IN_BITS), sizeof(uint32)); /*KEY + MASK + VALID*/
#endif /* BCM_DNX_SUPPORT */
...
void dnx_pemladrv_update_em_cache_from_physical(int unit) {
  ...
  for (index = 0; index < api_info[unit].db_em_container.nof_em_dbs; ++index) {
    if (NULL == api_info[unit].db_em_container.db_em_info_arr[index].logical_em_info.result_chunk_mapper_matrix_arr)     /*In case db was not mapped*/
      continue;
    /*finding relevant fields and allocating memory*/
    ...
    pemladrv_mem_t * result)
{
  ...
  dnx_set_pem_mem_accesss_fldbuf_from_physical_sram_data_array(api_info[unit].db_em_container.db_em_info_arr[db_id].em_cache.em_key_entry_arr[index],
                                                               api_info[unit].db_em_container.db_em_info_arr[db_id].logical_em_info.key_field_bit_range_arr,    /*Sram - because only key is needed, without mask and valid*/
                                                               key);
  return 0;
}
    ...
    pemladrv_mem_t * result)
{
  ...
  dnx_set_pem_mem_accesss_fldbuf_from_physical_sram_data_array(api_info[unit].db_lpm_container.db_lpm_info_arr[db_id].lpm_cache.lpm_key_entry_arr[index],
                                                               api_info[unit].db_lpm_container.db_lpm_info_arr[db_id].logical_lpm_info.key_field_bit_range_arr,    /*Sram - because only key is needed, without mask and valid*/
                                                               key);
  return 0;
}
    ...
    LogicalTcamInfo * tcam_info)
{
  ...
  DbChunkMapper *chunk_mapper;
  phy_mem_t      phy_mem;
  const unsigned int is_ingress = 0; /*This is_ingress bit is don't-care in this case*/
  const unsigned int is_mem = 1;
...
#else
  if (currently_writing_applets == 0) {  /*if not currently writing applets then just read from the design*/
