    ...
    int *app_db_size)
{
    ...
    /** Return NOT_FOUND error if KBP DB ID is 0 (not set) */
    if ((logical_tables_info[unit]->logical_tables[table_id].access_method == DBAL_ACCESS_METHOD_KBP) &&
    ...
    *app_db_id = logical_tables_info[unit]->logical_tables[table_id].app_id;
    ...
    /** app_db_size is not relevant for KBP tables */
    if (logical_tables_info[unit]->logical_tables[table_id].access_method != DBAL_ACCESS_METHOD_KBP)
    {
    ...
    uint32 mode)
{
    ...
    if (!sw_state_is_warm_boot(unit))
    {
        /** currently when working with collection we are also working without validations otherwise we can have a scenario
         *  that two threads are accessing the same place */
