    ...
    int unit)
{
    ...
    macro_idx_max = dnx_data_mdb.dh.macro_type_info_get(unit, macro_type)->nof_macros;
    ...
    uint32 row_idx)
{
    ...
    if (cluster_idx >= dnx_data_mdb.dh.macro_type_info_get(unit, macro_type)->nof_clusters)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Error. macro_type==%d, cluster_idx==%d is not supported.\n", macro_type,
                     cluster_idx);
    }

    if (macro_index >= dnx_data_mdb.dh.macro_type_info_get(unit, macro_type)->nof_macros)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Error. macro_type==%d, macro_index %d is not supported.\n",
                     macro_type, macro_index);
    }

    if (row_idx >= dnx_data_mdb.dh.macro_type_info_get(unit, macro_type)->nof_rows)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Error. macro_type==%d, row_idx %d is not supported.\n", macro_type, row_idx);
    }
    ...
    uint32 entry_idx)
{
    ...
    if (array_idx >= dnx_data_mdb.dh.macro_type_info_get(unit, macro_type)->nof_clusters)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Error. array_idx==%d is not supported.\n", array_idx);
    }

    if (entry_idx >= dnx_data_mdb.dh.macro_type_info_get(unit, macro_type)->nof_rows)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Error. macro_type==%d, entry_idx %d is not supported.\n", macro_type, entry_idx);
    }
    ...
    uint32 cluster_idx)
{
    ...
    row_index_max = dnx_data_mdb.dh.macro_type_info_get(unit, macro_type)->nof_rows;
    ...
    uint32 *macro_index)
{
    ...
    uint32 global_macro_index, tmp_index, macro_idx_max;
    soc_mem_t mem1;
    int blk1;

    SHR_FUNC_INIT_VARS(unit);

    macro_idx_max = dnx_data_mdb.dh.total_nof_macroes_get(unit);
    for (global_macro_index = 0; global_macro_index < macro_idx_max; global_macro_index++)
    {
        tmp_type = dnx_data_mdb.dh.logical_macro_info_get(unit, global_macro_index)->macro_type;
        tmp_index = dnx_data_mdb.dh.logical_macro_info_get(unit, global_macro_index)->macro_index;
    ...
    void *entry_data)
{
    ...
    MDB_EEDB_ACCESS_UNLOCK(unit, soc_mem_array_write(unit, mem, array_index, copyno, index, entry_data));
exit:
    SHR_FUNC_EXIT;
}
    ...
    int flags)
{
    ...
    MDB_EEDB_ACCESS_UNLOCK(unit, soc_mem_array_read_flags(unit, mem, array_index, copyno, index, entry_data, flags));
exit:
    SHR_FUNC_EXIT;
}
