...
/**
 * this array holds the max legal number of bits per pointer type
 */
...
/** MACRO to validate that the handle status is OK */
...
/** MACRO to validate that the instance ID is valid */
...
{                                                                        \
    int rv1;                                                             \
    /** If both 'source,layer' combinations indicate 'disable' then 'return' */      \
...
/** DBAL internal info is saved in struct per unit, no need to keep it in SW state as it is not needed for WB \ ISSU */
...
/*
 * This functions defined in order to save compilation time and save the need on including drv.h in dbal header file.
 */
uint8
dbal_is_j2c_a0(
    int unit)
    ...
    int unit)
{
    ...
    /** initialize the global variable G_dbal_field_full_mask */
    ...
    int unit)
{
    ...
    /** first allocating the handles pool according to number of handles*/
...
/** worker thread procedure, thread is waiting on semaphore until receives signal to wake up, once wake up then it is
 *  synced with the other worker thread and processing pool/performing destroy according to signal type */
STATIC void
    ...
    void *cookie)
{
    ...
    while (1)
    {
        ...
        /** DBAL_COLLECTION_PRINTS((BSL_META("signal received %d \n"), my_id)); */
        ...
        if (dbal_mngr[unit]->collection_signals[my_id] == DBAL_COLLECTION_SIGNAL_DESTROY)
        {
            ...
            /*
             * Destroy the relevant mutex and sem along with the thread
             */
        ...
        for (curr_handle = first_entry_in_pool; curr_handle < first_entry_in_pool + collection_pool_size; curr_handle++)
        {
            ...
            {
                /** entry status is not collected, we expect that the main thread will fill all entries in the pool currently if
                 *  we find an entry with status not collected, we just skipping it */
                /** another reason to reach to this place is when main thread receive a trigger call that the
                 *  operation was completed but the pool was not full (in this case the last entries will be with status DBAL_HANDLE_STATUS_AVAILABLE - suggestion
                 *  to add a different collection_signals for this case to indicate that it is valid) */
                /** DBAL_COLLECTION_PRINTS((BSL_META("\n ENTRY SKIPPED! skipping handle %d since status not collected\n"),
                                        curr_handle)); */
            }
            else
            {
                ...
                if (rv)
                {
                    /** entry commit failed, need a mechanism to indicate about the error, currently it exits and the main thread will see
                     *  that not all entries were processed so it will be the signal that something went wrong. */
        ...
        /** allow the second thread to start working. */
        sal_mutex_give(dbal_mngr[unit]->collection_worker_thread_sync);

        /** allow the main thread to fill this pool */
    ...
    int unit)
{
    ...
    if (dnx_data_dbal.db_init.dbal_collection_pool_size_get(unit))
    {
        ...
        for (worker_thread_id = 0; worker_thread_id < DBAL_NOF_COLLECTION_POOLS; worker_thread_id++)
        {
            ...
            sal_thread_create(thread_name, SAL_THREAD_STKSZ, 50 /* priority */ , dbal_collection_worker_thread,
    ...
    int unit)
{
    ...
    /*
     * Destroy all worker threads mutexes and semaphores 
     */
    if (dnx_data_dbal.db_init.dbal_collection_pool_size_get(unit))
    {
        for (worker_thread_id = 0; worker_thread_id < DBAL_NOF_COLLECTION_POOLS; worker_thread_id++)
        {
            /** wait on mutex letting the thread to complete its work, and signal to thread to destroy. */
    ...
    dbal_collection_trigger_type_e trigger_type)
{
    ...
    /** if the trigger type is WAIT, need to verify that the worker thread completed its work. */
    ...
    int *pool_id)
{
    ...
    while (!is_found)
    {
        ...
        /** assuming that the next pool should be available by waiting on the pools mutex there could be a sync issue here
         *  since the main thread can gain faster the pool before the worker thread started to process it. this is why
         *  checking that the first entry is not available.  */
    ...
    uint8 operation)
{
    ...
    {
        /** when allocating handle need to lock or using same handle for new table.. */
        if ((operation == DBAL_HANDLE_TAKE_ALLOC)
            || (dbal_mngr[unit]->entry_handles_pool[*entry_handle_id].table_id != table_id))
        {
            /** this protection is done before handle_mutex protection to prevent race condition between the two mutexes. */
    ...
    if (operation == DBAL_HANDLE_TAKE_ALLOC)
    {
        ...
        {
            /** checking if current pool is full */
            ...
            {
                /** continue allocating handles from existing pool */
                dbal_mngr[unit]->collection_counter++;
            }
            ...
            *entry_handle_id = handle_index;
            ...
            if (dbal_mngr[unit]->entry_handles_pool[handle_index].handle_status == DBAL_HANDLE_STATUS_ACTION_COLLECTED)
            {
                /** if the status of an handle is collected it means that the worker thread couldn't process this entry for some
                 *  reason, for now we are exiting. */
        ...
        else
        {
            ...
            /** no available handles dump the handle state.. */
    ...
    /** memory for user_output_info will be allocated when dbal_entry_value_field_request() is called by the user */

    /** update handle fields to initial values */
    ...
    /** handle multiple result type table case   */
    ...
    uint32 *dst_handle_id)
{
    ...
    /** find availiable destination handle */
    for (handle_index = 0; handle_index < DBAL_SW_NOF_ENTRY_HANDLES; handle_index++)
    {
        if (dbal_mngr[unit]->entry_handles_pool[handle_index].handle_status == DBAL_HANDLE_STATUS_AVAILABLE)
        {
    ...
    *dst_handle_id = handle_index;
    ...
    /** copy user_output_info if allocated */
    ...
    uint32 entry_handle_id)
{
    ...
    if (entry_handle_id >= DBAL_SW_NOF_ENTRY_HANDLES)
    {
        if (dnx_data_dbal.db_init.dbal_collection_pool_size_get(unit))
        {
            ...
            /** collection handle - no need to do anything */
            if (SHR_IS_BITSET(table->indications_bm, DBAL_TABLE_IND_COLLECT_MODE))
            {
                SHR_EXIT();
            }
    ...
    {
        dbal_logical_table_t *table = dbal_mngr[unit]->entry_handles_pool[entry_handle_id].table;
        /** In Handle release, destroy all pending iterators */
        if (dbal_mngr[unit]->iterators_pool[entry_handle_id].is_init)
        {
            SHR_IF_ERR_EXIT(dbal_iterator_destroy_macro(unit, entry_handle_id));
        }
        /** In Handle release, free user_output_info if allocated */
        DBAL_ENTRY_HANDLE_USER_OUTPUT_INFO_FREE(dbal_mngr[unit]->entry_handles_pool[entry_handle_id]);
        if (SHR_IS_BITSET(table->indications_bm, DBAL_TABLE_IND_THREAD_PROTECTION_ENABLED))
        {
...
/** field core ID can be set even if it is not exists on the table incase that the table is SBC */
static shr_error_e
...
/** function handles mask, set the internal entry_handle key mask buffer with the correct mask value. */
static shr_error_e
    ...
    uint32 *field_mask)
{
    ...
    /** incase of subfield the mask should be affect the encoding part suffix/prefix. in those cases the suffix/prefix
     *  ,ask should be al "1" */
    ...
    dbal_pointer_type_e pointer_type)
{
    ...
    /** validate legal bit size according to the pointer type, (if the field is too big for the pointer type) */
    DBAL_FIELD_ERR_HANDLE(((dbal_field_legal_sizes[pointer_type]) &&
        ...
        /** in case of field is core id, (DPC mode) save it directly in the handle  */
    ...
    /** Can only do validations to field type lower than 32bit */
        ...
        && !SHR_BITGET(table_field->field_indication_bm, DBAL_FIELD_IND_IS_FIELD_ADDRESS))
    {
        ...
        /** not need to validate for enums since for enum it is validate in the decode */
    ...
    {
        /** for enum taking the complete logical value received by the user, after decoded taking only the required bits. */
    ...
    if ((SHR_BITGET(table_field->field_indication_bm, DBAL_FIELD_IND_IS_FIELD_ENCODED)) || is_sub_field)
    {
        ...
        if (ret_val != _SHR_E_NONE)
        {
            ...
            {
                /** during device init no need to dump errors for key not mapped */
        ...
        /** setting the value in the entry buffer */
    ...
    else
    {
        if (entry_handle->error_info.error_exists == _SHR_E_UNAVAIL)
        {
            ...
            {
                /** in case we are not in std1, we are checking if we are in key not mapped case according to error
                 * if we are updating the same key field with a valid value need to update the error info to _SHR_E_NONE */
    ...
    {
        /** if it was set to be ranged before now should return it to default state */
    ...
    uint32 max_val)
{
    ...
    {
        /** incase this is not std_1 the min value for enums should be checked since it can be invalid */
        int jj;
        ...
        if (field_type_info->encode_info.encode_mode == DBAL_VALUE_FIELD_ENCODE_ENUM)
        {
            ...
            {
                /** looking for new min enum value */
    ...
    /** setting the min value of the field in the handle */
    ...
    if (!is_core_field)
    {
        if (!entry_handle->error_info.error_exists)
        {
            ...
            /** updating the range of values that should be set in commit */
...
/**
 * \brief
 *  this is used for multiple result table, when the result type is not resolved yet (the result type should be the
 *  first value field to add). it updates the cur_res_type and the phy entry buffer.
 */
void
    ...
    int res_type)
{
    ...
    else
    {
        if (entry_handle->table_id == DBAL_TABLE_ETM_PP_DESCRIPTOR_EXPANSION)
        {
            ...
            /** incase that the result type is superset, the cur_res type should be the last result type of the table */
...
/**
 * \brief
 * used to update the value for all instances of a value field incase all instances has the same value.
 */
static shr_error_e
    ...
    uint32 *field_val)
{
    ...
    for (inst_iter = 1; inst_iter < nof_instances; inst_iter++)
    {
        ...
        /** first time field is being set (== !is_field_updated) */
...
/**
* \brief
* used to unset the value for all instances of a value field incase all instances has the same value.
*/
static shr_error_e
    ...
    uint32 *field_mask)
{
    ...
    /** multiple result types support - first field has to be result type */
    ...
    /** Getting the field info from the table */
    ...
    /** incase of valid indication (and all other encoding that adds bits to the field the user can use a smaller pointer
     *  type to add the value (for example for if the field_size is 32bit and with valid indication it is 33 bit user
     *  will add the value by uint32_set() (because its max value is valid) */
    if (!SHR_BITGET(table_field->field_indication_bm, DBAL_FIELD_IND_IS_VALID_INDICATION))
    {
        /** validate legal bit size.. */
        DBAL_FIELD_ERR_HANDLE(((dbal_field_legal_sizes[pointer_type]) &&
                               (table_field->field_nof_bits > dbal_field_legal_sizes[pointer_type])),
                              "field size too big for pointer type");
    }
    ...
    /** field value validations applicable only for fields smaller than 32 bits. */
    if (!SHR_IS_BITSET(entry_handle->table->indications_bm, DBAL_TABLE_IND_NO_VALIDATIONS)
        && DBAL_RUNTIME_VERIFICATIONS_ENABLED(unit) && (table_field->field_nof_bits <= 32)
        && !SHR_BITGET(table_field->field_indication_bm, DBAL_FIELD_IND_IS_FIELD_ADDRESS))
    {
        ...
        {
            /** for arr_prefix validations are made inside dbal_field_arr_prefix_decode */
        ...
        /** not need to validate for enums since for enum it is validate in the decode  because of the arr_prefix */
        if (!(SHR_BITGET(table_field->field_indication_bm, DBAL_FIELD_IND_IS_FIELD_ENUM)))
        {
            ret_val = dbal_fields_is_illegal_value(unit, field_id, field_val[0], &is_illegal);
            DBAL_FIELD_ERR_HANDLE((is_illegal || ret_val), "Illegal value for field, in illegals list");
    ...
    if (!(SHR_BITGET(table_field->field_indication_bm, DBAL_FIELD_IND_IS_FIELD_ENUM)))
    {
        ...
        if (SHR_BITGET(table_field->field_indication_bm, DBAL_FIELD_IND_IS_VALID_INDICATION))
        {
            /** incase that the field has valid indication, the actual input is one less bit than the field size, only afterwards
             *  the valid indication is added to the field */
    ...
    /** setting the field value in the buffer */
    ret_val = utilex_bitstream_set_any_field(curr_field_value_ptr, table_field->bits_offset_in_buffer,
                                             table_field->field_nof_bits, &(entry_handle->phy_entry.payload[0]));
    ...
    /** if the field is a new field, update info */
    if (!is_field_updated)
    {
    ...
    /** in case of multiple instances updating all other instances of the field */
    ...
    void *field_mask)
{
    ...
    /*
     * In case the field that found in table is the field+instance, it should be requested.
     * If not, it means that the requested field is a child field, and the child should be requested
     */
    ...
    /** Only allocate user output info when needed */
    ...
    dbal_pointer_type_e pointer_type)
{
    ...
    /** if field is an enum. Get the hw value */
...
/** checks if the value field was set on the handle.
 *  returns _SHR_E_NONE if the field was set
 *  _SHR_E_NOT_FOUND if field was not set
 *  _SHR_E_PARAM if field is not part of the table
 */
shr_error_e
    ...
    int in_inst_id)
{
    ...
    else
    {
        ...
        {
            /** field was not found in the table return _SHR_E_PARAM*/
            SHR_IF_ERR_EXIT_NO_MSG(_SHR_E_PARAM);
        }
    }

    /** validate permission: writeOnly is not allowed  */
    ...
    /** if the field was not set befor the call it means that it was not added */
    if (entry_handle->value_field_ids[field_index_in_table] == DBAL_FIELD_EMPTY)
    {
    ...
    dbal_pointer_type_e pointer_type)
{
    ...
    else
    {
        ...
        /*
         * In case the field that found in table is the field+instance, it should be called from buffer.
         * If not, it means that the requested field is a child field, and the child should be called from buffer
         */
        ...
        /** incase the result type is superset, if the field was not set befor the call it means that it was not added */
        ...
        {
            /** for enum, the arr_prefix is handled inside  dbal_field_from_buffer_get   */
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    {
        /*
         * Update the result fields which are not set
         */
        for (field_pos = 0; field_pos < DBAL_RES_INFO.nof_result_fields; field_pos++)
        {
            if (entry_handle->value_field_ids[field_pos] == DBAL_FIELD_EMPTY)
            {
                    ...
                    (entry_handle->table->hl_mapping_multi_res[entry_handle->cur_res_type].is_default_non_standard))
                {
                    ...
                    /**SHR_IF_ERR_EXIT(dbal_hl_non_standard_default_entry_build(unit, entry_handle, default_val)); */
                    SHR_ERR_EXIT(_SHR_E_INTERNAL, "HL TCAM none standard default is not supported for table %s\n",
                                 entry_handle->table->table_name);
                }
                /** Set field value to zero and set field mask */
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    if (shift_left)
    {
        ...
        /** copying extra bits to make sure that we are cleaning the buffer */
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    for (iter = 0; iter < table->multi_res_info[entry_handle->cur_res_type].nof_result_fields; iter++)
    {
        ...
        {
            /** in case the result type was requested return the value, it is also possible that the result type was set to
             *  indicate the result type and not to retrieve it */
        ...
        {
            /*
             * If those 3 conditions are satisfied, this is a false alarm.
             * The field was not requested, but was set as part of dbal_handle_take_update_init_result_value
             * The field ID should be cleared from entry_handle->value_field_ids array.
             */
...
/** update buffer_to_print the corresponding string for the struct field and all sub fields in the following format:
 *  struct field complete value 0x%x (f1_val, f2_val...) */
static shr_error_e
    ...
    char buffer_to_print[DBAL_MAX_PRINTABLE_BUFFER_SIZE])
{
    ...
    for (ii = nof_struct_fields - 1; ii >= 0; ii--)
    {
        ...
        /** if field is enum need to print enum values, if field is parent field need to print the subfield if field is has
         *  defines need to print is defines
         */
        ...
        if (field_type_info->struct_field_info[ii].struct_field_id != DBAL_FIELD_RESERVED)
        {
            ...
            /** in case of parent field need to update the buffer with the subfield info */
...
/** update buffer_to_print the corresponding string for the child field */
static shr_error_e
    ...
    char buffer_to_print[DBAL_MAX_PRINTABLE_BUFFER_SIZE])
{
    ...
    if (sub_field_id != DBAL_FIELD_EMPTY)
    {
        ...
        /** Found a child field - add it to the printable string */
        char_count =
        ...
        {
            /** Recursive call - used to print the sub-field according to it's print type */
...
/**
 *\brief
 *Inserts a string representation of input field's name and
 *value into buffer_to_print.
 *
 *   result_info    - In case that the field is a
 *          result type, the result info is needed (else set to
 *          NULL)
 *   is_full_buffer - If it is full buffer, Enum values are HW values, in
 *       case single field value, enum already paresed to SW
 *       values
 *   parent_field   - Should be set In case that the
 *          desired field is a part of a table, and has a parent
 *          field in the table. Else set to DBAL_FIELD_EMPTY
 *   size           - Actual nof bits
 */
shr_error_e
    ...
    char buffer_to_print[DBAL_MAX_PRINTABLE_BUFFER_SIZE])
{
    ...
    {
        /** if the field is encoded decoding the value (transforming it to logical value) */
        SHR_IF_ERR_EXIT(dbal_fields_field_decode(unit, field_id, parent_field, size, field_val, decoded_field_val));
        is_field_encoded = 1;
    }

    /** Handle defines - if field has define return the define name. */
    ...
    /** Handle parent field, if field has child field return the child field string */
    ...
    /** Handle struct field, printing all struct fields from the fields */
    ...
    switch (field_type_info->print_type)
    {
        ...
        case DBAL_FIELD_PRINT_TYPE_IPV6:
        {
            ...
            start_byte_to_print -= start_byte_to_print & 0x1;   /* Start always from even index */
    ...
    char buffer_to_print[DBAL_MAX_PRINTABLE_BUFFER_SIZE])
{
    ...
    {
        /** In case not full buffer, enum values are already SW values, and the above parsing is all wrong */
        field_val[0] = buffer_val[0];
        size = SAL_UINT32_NOF_BITS; /** size of DBAL enum is always 32bit */
        if (buffer_mask)
        {
    ...
    else
    {
        ...
        /*
         * Copy the field from buffer to uint32 array. If not from
         * buffer, just copy the relevant bits.
         * mask is relevant only if called with full buffer
         */
        if (is_full_buffer)
        {
...
/**
 * fill the k_mask buffer on phy_entry according to prefix length and key size
 */
shr_error_e
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    /** For KBP tables prefix_length = 0 is a valid case */
...
/**
 * Make sure that the key mask is valid
 * Expected format 1....10....0
 * Update the mask prefix length in phy_entry
 */
shr_error_e
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    /** appID is part of the key, but is not set by the user, for LPM need it to calculate the prefix length */
    ...
    /** currently test each bit, we can improve the performance by checking bytes.
     * we skip the app_id bits, since we just set it.
     */
    for (bit_index = max_bit_index - entry_handle->table->app_id_size; bit_index >= min_bit_index; --bit_index)
    {
        ...
        else
        {
            if (is_bit_on)
            {
                ...
                /** check that the hole in the prefix is caused by the zero padding. if so remove the padding. */
...
/**
 *\brief
 *  this function check if the current access should be
 *  Performed according to the Key added.
 */
shr_error_e
        ...
        if (condition.field_id != DBAL_FIELD_EMPTY)
        {
            ...
            else
            {
                ...
                *is_passed = 0;
                ...
                /*
                 * When using iterator on table with multiple result types it is possible that one or more
                 * of the result types does not include the iterator rule field*/
        ...
        switch (condition.type)
        {
            case DBAL_CONDITION_BIGGER_THAN:
                ...
                for (ii = words_to_compare - 1; ii >= 0; ii--)
                {
                    ...
                    {
                        /** Equality return FALSE condition only if it the least significant word to check */
                        is_condition_true = FALSE;
                        break;
                    }
                }
...
/*
 *  checks if the key buffer created is legal according to the fields legal values used in iterator
 *  In case the key is illegal key according to XML definition, is_valid will be 0.
 *  In case the key is not valid due to iterator rules, is_valid will return DBAL_KEY_IS_OUT_OF_ITERATOR_RULE
 *
 */
shr_error_e
    ...
    dbal_key_value_validity_e * is_valid)
{
    ...
    *is_valid = DBAL_KEY_IS_VALID;
    ...
    for (ii = 0; ii < table->nof_key_fields; ii++)
    {
        ...
        {
            /** Global lif/rif em table the key returned can be out of range of the dbal table but stil valid, since
             *    GLOBAL_LIF_EM & GLOBAL_RIF_EM has the same app_id */
    ...
    dbal_key_value_validity_e * is_valid_entry)
{
    ...
    if (is_allocator)
    {
        ...
        {
            /** if the field size is smaller than the actual resource need to validate that the entry is not excceded  */
        ...
        if ((index != DNX_ALGO_RES_DBAL_ILLEGAL_ELEMENT) && (index <= table_field_info.max_value))
        {
            ...
            /** update the relevant field in the phy_entry.key */
        ...
        {
            /** in case not found, setting the max value of the resource */
...
/** this function copy all the existing key and result fields to the entry handle from the table   */
shr_error_e
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    for (ii = 0; ii < table->nof_key_fields; ii++)
    {
        ...
        if (table->keys_info[ii].field_id == DBAL_FIELD_CORE_ID)
        {
            ...
            {
                /** taking only from core 0, should be the same value for both cores.. */
    ...
    uint32 *field_mask)
{
    ...
    {
        /** First rule to alloc - no FREE buffer */
    ...
    uint32 *field_mask)
{
    ...
    {
        /** First action to alloc - no FREE buffer */
...
/**
 * \brief - Get internal dbal handle and update the field ids
 *
    ...
    uint32 entry_handle_id)
{
    ...
    /** Get internal dbal handle */
    SHR_IF_ERR_EXIT(dbal_entry_handle_get_internal(unit, entry_handle_id, &entry_handle));
    /** Update the field ids in entry_handle*/
    ...
    char *top_node_name)
{
    ...
    while ((dirp = sal_readdir(dir)) != NULL)
    {
        ...
        /**In case of a subdir - recursive call with the subdir*/
        ...
        /**Validate that the xml file matches the soc file image*/
        ...
        /**Found a dynamic file - iterate on its tables*/
