    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    for (cur_res_type = 0; (cur_res_type < entry_handle->table->nof_result_types) && (found == FALSE); cur_res_type++)
    {
        ...
        {
            /** The result type is not used in the device.*/
            continue;
        }
    ...
    uint32 *payload_buffer)
{
    ...
    if (res_type_resolution_needed)
    {
        ...
        /*
         * after result type updated, should align all fields according to result type
         */
    ...
    dbal_entry_action_flags_e flags)
{
    ...
    /** this is an optimization to skip entry get for tables that uses ITERATOR_OPTIMIZED, this is relevant only if the
     *  default entry is all zeros, if is_default_non_standard we need to update the payload to the no standard values.
     *  to is_default_non_standard we prefert just get the entry from the HW anyway.. */
        ...
        !SHR_IS_BITSET(table->indications_bm, DBAL_TABLE_IND_IS_DEFAULT_NON_STANDARD))
    {
        ...
        if (!is_exists)
        {
            ...
            {
                /** handling error code for non-direct tables. */
                SHR_SET_CURRENT_ERR(_SHR_E_NOT_FOUND);
            }
        }
        else
    ...
    dbal_entry_action_flags_e flags)
{
    ...
    if (entry_handle_id >= DBAL_SW_NOF_ENTRY_HANDLES)
    {
        ...
        {
            /** Last entry in the pool - signal to worker thread to start working.*/
    ...
    dbal_result_type_update_mode_e mode)
{
    ...
    for (field_index = 0; field_index < table->multi_res_info[new_result_type].nof_result_fields; field_index++)
    {
        ...
        /** if a field exists in old result type - take its value */
        sal_memset(&field_info, 0x0, sizeof(dbal_table_field_info_t));
        SHR_IF_ERR_EXIT_EXCEPT_IF(dbal_tables_field_info_get_no_err
        ...
        if (field_info.field_nof_bits)
        {
            ...
            /*
             * Get field value and attributes
             */
            ...
            /*
             * Those are the only cases where we can use the value from the previous result-type.
             * Yet, the value should be verified for ranges
             */
            if ((sub_field_id == DBAL_FIELD_EMPTY) || (sub_field_id == new_field_id))
            {
                /*
                 * Verify that the value from the previous result-type is suitable for the
                 * new result-type
                 */
        ...
        /*
         * Set the value from the previous result-type if it was found to be in range.
