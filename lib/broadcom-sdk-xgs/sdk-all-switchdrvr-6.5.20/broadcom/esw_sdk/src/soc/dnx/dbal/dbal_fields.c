    ...
    int unit)
{
    ...
    for (field_type = DBAL_FIELD_TYPE_DEF_EMPTY + 1; field_type < DBAL_NOF_FIELD_TYPES_DEF; field_type++)
    {
        ...
        /** callback assign for illegal values  */
...
/** this API is used only in init to receive the hw value from the string of the logical value. */
shr_error_e
    ...
    shr_error_e ret_val)
{
    ...
    if (ret_val == _SHR_E_UNAVAIL)
    {
      ...
      *  for key fields also need to set the handle status to ignore the action TBD. */
        ...
        {
            /** In standard 1 we always return error for invalid enum value */
            SHR_SET_CURRENT_ERR(_SHR_E_INTERNAL);
        }
