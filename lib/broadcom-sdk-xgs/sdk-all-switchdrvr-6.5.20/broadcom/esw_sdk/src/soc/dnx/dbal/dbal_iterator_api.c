...
/**
 *  \brief check if a given action_type is an entry action
 *  entry action is an action which affect the entire entry
 */
static uint8
...
/**
 *  \brief check if a given action_type is a field action
 *  field action is an action which affect a specific field in an entry
 */
static uint8
...
/**
 *  \brief perform some rules and action validations before starting the action
 */
static shr_error_e
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    for (ii = 0; ii < iterator_info->nof_actions; ii++)
    {
        ...
        /** build the action bitmap */
        actions |= iterator_info->actions_info[ii].action_type;

        /** action is not allowed on RESULT_TYPE field */
    ...
    /*
     * hit bit actions cannot be set with hit bit rules
     */
    ...
    /*
     * hit bit action and rules is supported for MDB
     */
    if (has_hit_bit_get_action || has_hit_bit_clear_action || iterator_attrib_info->hit_bit_rule_valid)
    {
...
/**
 *  \brief build payload buffer for action update in SW
 */
static shr_error_e
...
/**
 *  \brief sync two handles with the action payload information
 */
static shr_error_e
    ...
    uint32 iterated_entry_handle_id)
{
    ...
    /** Copy user_output_info if it exists */
    ...
    uint32 entry_handle_id)
{
    ...
    /** For MDB tables, check if we set the entry to APPLY_NONE */
    if (dbal_logical_table_is_mact(unit, entry_handle->table_id))
    {
        ...
        {
                        /** Reset flag for future use, and do nothing */
...
/**
 *  \brief get next relevant entry (entry which satisfies the iterator rule)
 */
static shr_error_e
    ...
    dbal_iterator_mode_e mode)
{
    ...
    sal_memset(iterator_info, 0, sizeof(dbal_iterator_info_t));
        /**In case of non default permission write - check if there're ro or trigger fields*/
    if (mode == DBAL_ITER_MODE_GET_NON_DEFAULT_PERMISSION_WRITE)
    {
        mode = DBAL_ITER_MODE_GET_ALL_EXCEPT_DEFAULT;
                /**This iterator mode is only relevant for hl access method*/
    ...
    uint32 attr_val)
{
    ...
    switch (attr_type)
    {
        ...
        case (DBAL_ENTRY_ATTR_HIT_GET | DBAL_ENTRY_ATTR_HIT_PRIMARY | DBAL_ENTRY_ATTR_HIT_SECONDARY):
            /** Verify hit bit attribute rule value is NOT_SET(0) or SET(1) */
    ...
    uint32 entry_handle_id)
{
    ...
    /** If the action is to delete, no need to configure anything, Run over the entries with iterator and remove them */
    ...
    int *is_end)
{
    ...
    /** first time calling iterator, need to update the action to get_next */
    ...
    /** If there is an action to do other then get perform it */
