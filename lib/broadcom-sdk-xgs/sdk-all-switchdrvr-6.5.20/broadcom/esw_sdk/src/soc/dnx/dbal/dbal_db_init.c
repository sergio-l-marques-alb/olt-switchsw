...
/** Precondition. field_id has been set to field
 *  access_method has ben set to table  */
static void
...
/** Precondition. nof_instances have been set to field.
 *  access_method has been set to table
 *  Instances have been duplicated
 */
    ...
    *sw_length_bits = 0;
    ...
    for (ii = 0; ii < table_params->hl_access[acc_map_idx].nof_access; ii++)
    {
        ...
        */
         ...
         /** set the packed fields indication for the hard logic mapping of the dbal table. */
            ...
            && (!hl_mapping->l2p_hl_info[access_type].is_packed_fields))
        {
            ...
            /** incase that the field is not valid in the interface the access is not required - no need to allocate memory. */
        }

        /** note that for packed fields tables we didn't look for the field in the interface since the access field is empty. */
        if (rv != _SHR_E_NOT_FOUND)
        {
    ...
    for (ii = 0; ii < table_params->hl_access[acc_map_idx].nof_access; ii++)
    {
        ...
        if (access_field_id)
        {
            ...
            if (rv == _SHR_E_NOT_FOUND)
            {
                /** field was not found in the interface, this can be when the field is not valid for this specific device, in this
                 *  case we are still updating the num_of_access_fields since the memory was allocated above (and should be deallocate
                 *  in deinit), we also setting the field_id to empty that DBAL will not access it. */
                continue;
            }
        }
        else
        {
            if (table_entry->table_type == DBAL_TABLE_TYPE_DIRECT
                && (!hl_mapping->l2p_hl_info[access_type].is_packed_fields))
            {
                /** no access field (this can happen when there is interface  */
                continue;
            }
        }
 ...
 *        performed whenever there's an additional apperance of
 ...
 * \param [in] field_idx - An index to the field IDs array
 *        within the result type struct. That's the location of
 *        the existing apperance of the field.
 * \param [in] field_info - A pointer to the new field apperance
 *        that is added.
 * \return
 *   shr_error_e
 * \see
    ...
    dbal_table_field_info_t * field_info)
{
    ...
    for (field_idx = 0; field_idx < (superset_res_type->nof_result_fields); field_idx++)
    {
         ...
         * In that case store the index of the subfield in an arrat of subfields 
         */
