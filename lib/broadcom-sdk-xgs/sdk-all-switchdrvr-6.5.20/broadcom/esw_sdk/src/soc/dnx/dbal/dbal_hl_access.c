...
#include <soc/drv.h>    /* SOC_REG_BLOCK_NOF_INSTANCES */
 ...
 *  key fields from the handle, hardlogic formula can be only one of the following - callback that was created by the
 *  autocoder from the XML or int value that was calculated by the autocoder and was set in the formula->val;
 */
shr_error_e
    ...
    int *num_of_blocks)
{
    ...
    else
    {
        ...
        if (core_id != DBAL_CORE_ALL)
        {
            ...
            /** in this case the number of blocks depends on the number of cores X blocks per core */
    ...
    int logger_enable)
{
    ...
    /** this part takes the field value from the buffer, taking only the required bits */
    ...
    int result_type_get)
{
    ...
    /** updating the entry result type valid incase of result_type_get != DBAL_INVALID_PARAM */
    ...
    int group_offset)
{
    ...
    /** if block = REG_PORT_ANY, nof_blocks should be 1. */
    ...
    else
    {
        ...
        if (l2p_info->hw_entity_group_id != DBAL_HW_ENTITY_GROUP_EMPTY)
        {
            /** in case group offset is -1 and we have a group need to update all registers in group   */
            all_reg_in_group = 1;
        }
    }
    ...
    {
        /*
         * check if reg is array and there is no specific array index to set 
         */
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    for (core_iteration = start_iteration; core_iteration < end_iteration; core_iteration++)
    {
        ...
        {
            int field_pos;
            /** saves the last valid memory info that was used */
            ...
            {
                int nof_blocks;
                /** indicates that access occurred remove the error indication   */
                entry_handle->error_info.error_exists = 0;
            ...
            if (is_valid_entry == DBAL_KEY_IS_VALID)
            {
                ...
                if (SHR_IS_BITSET(entry_handle->table->indications_bm, DBAL_TABLE_IND_HAS_RESULT_TYPE))
                {
                    ...
                    if (rv != _SHR_E_NONE)
                    {
                        ...
                        {
                            /**  if we get an error at the last element set is_end to TRUE, to indicate the iterator has ended */
