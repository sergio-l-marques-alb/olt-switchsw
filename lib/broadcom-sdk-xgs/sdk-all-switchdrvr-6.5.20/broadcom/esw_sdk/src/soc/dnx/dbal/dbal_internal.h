...
/**
 * \brief collection mode init, collection mode will be enabled only when the pool size is > 0. this function creates 
     *        the worker threads and mutexes for the collection mechanism.
 */
...
/**
 * \brief - Enables/Disables Table collection mode to improve performance
 *
 * \par DIRECT_INPUT:
 *   \param [in] unit -
 *   \param [in] table_id - the table to look for\n
 *   \param [in] mode - 0 no collection, 1 enable collection
 * \par DIRECT OUTPUT:
 *   shr_error_e
 */
...
/**
 * \brief
 * indicate the worker thread to start working
 */
