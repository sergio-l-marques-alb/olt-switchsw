    ...
    sh_sand_control_t * sand_control)
{
    ...
    int current_core, lif_idx;
    int nof_inlif1_lifs, nof_inlif2_lifs;
    ...
    /*
     * Clean all In-LIFs, later restore them through journal
     */
    ...
    for (lif_idx = 0; lif_idx < nof_inlif1_lifs; lif_idx++)
    {
        SHR_IF_ERR_EXIT(lif_mngr_db.local_lif_info.ingress_lif1.set(unit, lif_idx, &ingress_lif_info));
    }
    for (lif_idx = 0; lif_idx < nof_inlif2_lifs; lif_idx++)
    {
        SHR_IF_ERR_EXIT(lif_mngr_db.local_lif_info.ingress_lif2.set(unit, lif_idx, &ingress_lif_info));
    }
     ...
     */
    cli_out("2. Free all allocated inlifs.\n");
