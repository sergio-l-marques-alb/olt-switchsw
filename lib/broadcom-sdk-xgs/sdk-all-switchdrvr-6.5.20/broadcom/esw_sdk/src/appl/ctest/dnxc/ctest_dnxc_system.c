 ...
 * When if CTEST_ALL or CTEST_PASS flag on, will always execute.
 */
#define CTEST_ELIGIBLE(cmd_flags, test_flags) \
                    (( !((cmd_flags & SH_CMD_SKIP_EXEC) && (test_flags & SH_CMD_SKIP_EXEC)) &&          \
                      ...
                      (!(cmd_flags & CTEST_PRECOMMIT) || (test_flags & CTEST_PRECOMMIT)) ) ||             \
                      (test_flags & CTEST_PASS) || (cmd_flags & CTEST_ALL))
    ...
    sh_sand_test_t * sh_sand_test)
{
     ...
     */
    if (sal_strcmp(test_name, "AT_Dnx_") != 0)
    {
        /*
    ...
    char *label)
{
    ...
    SHR_CLI_EXIT_IF_ERR(dbx_file_get_sdk_path(unit, "system_type_labels.xml", "/tools/dnx/", filepath),
    ...
    for (i = 0; i < tokens; i++)
    {
        ...
        RHDATA_ITERATOR(curr_node, top_node, "entry")
        {
            ...
            if (!(sal_strcasecmp(sys_mode, system_mode_tokens[i])))
            {
                found = TRUE;
                if (ISEMPTY(label))
                {
                    sal_strncat(label, sys_mode_label, sal_strlen(sys_mode_label));
                }
                else
                {
    ...
    sh_sand_control_t * sand_control)
{
    ...
    char *inputname;
    char filepath[RHFILE_MAX_SIZE];
    int flags = 0, precommit_flag, random_flag, all_flag;
    char *label;
    char *system_mode;
    ...
    if (dbx_file_get_sdk_path(unit, filename, "/tests/dnx/", filepath) < 0)
    {
        SHR_CLI_EXIT_IF_ERR(dbx_file_get_sdk_path(unit, filename, "/regress/bcm/tests/dnx/", filepath),
                            "could not get sdk path");
    }
    ...
    if (dbx_file_get_sdk_path(unit, filename, "/tests/dnx/ctest/", filepath) < 0)
    {
        SHR_CLI_EXIT_IF_ERR(dbx_file_get_sdk_path(unit, filename, "/regress/bcm/tests/dnx/ctest/", filepath),
                            "could not get sdk path");
    }
    ...
    RHITERATOR(sh_sand_test, test_list)
    {
        ...
        for (core = 0; core < nof_cores; core++)
        {
            ...
            if (SOC_IS_DNX(unit) && ((sh_sand_cmd->flags & SH_CMD_ROLLBACK) || (sh_sand_test->flags & SH_CMD_ROLLBACK)))
            {
                ...
                SHR_CLI_EXIT_IF_ERR(cmd_ctest_run_test(unit, sh_sand_cmd, sh_sand_test, flex_ptr, flags, &core),
                                    "Test failed");
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (sand_control->sh_sand_cmd_a)
    {
        SHR_CLI_EXIT_IF_ERR(sh_sand_cmd_traverse(unit, args, sand_control->sh_sand_cmd_a, cmd_ctest_run, (void *) &shell_flex, flags, NULL      /* label 
                                                                                                                                                 */ ),
...
static sh_sand_man_t sys_ctest_export_man = {
    ...
    "ctest [commands] export [pre] [all] [rnd] [label] [system_mode]",
    "ct export pre\n" "ct fld export\n" "ct mdb export\n" "ct export all\n" "ct export label=kbp\n"
...
static sh_sand_man_t sys_ctest_list_man = {
    ...
    "ctest [commands] list [pre] [rnd] [label] [system_mode]",
    "ct list\n" "ct tm list pre\n" "ct export label=kbp\n" "ct export system_mode=kbp"
};
...
static sh_sand_man_t sys_ctest_run_man = {
    ...
    "ctest [commands] run [pre] [rnd] [lg] [fail] [um]",
    "ct run\n" "ct tm run pre"
};
