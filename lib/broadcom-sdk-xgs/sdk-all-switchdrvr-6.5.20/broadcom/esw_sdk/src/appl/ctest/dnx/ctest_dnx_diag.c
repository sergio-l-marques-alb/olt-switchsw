    ...
    sh_sand_control_t * sand_control)
{
     ...
     * Output xml file
     */
    if (dbx_file_get_sdk_path(unit, "xml/", "/tests/dnx/", folder) < 0)
    {
        SHR_CLI_EXIT_IF_ERR(dbx_file_get_sdk_path(unit, "xml/", "/regress/bcm/tests/dnx/", folder),
...
sh_sand_option_t dnx_diag_counter_g_uc_options[] = {
    {"port", SAL_FIELD_TYPE_PORT, "port to send packet", "eth15"}
    ,
