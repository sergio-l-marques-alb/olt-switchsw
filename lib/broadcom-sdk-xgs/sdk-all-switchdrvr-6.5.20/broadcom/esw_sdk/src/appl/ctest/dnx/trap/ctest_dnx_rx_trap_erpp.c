    ...
    sh_sand_control_t * sand_control)
{
    ...
    SHR_IF_ERR_EXIT(bcm_rx_trap_get(unit, trap_id[0], &trap_config[1]));

    if (trap_config[0].flags != trap_config[1].flags)
    {
        LOG_ERROR(BSL_LS_APPL_COMMON,
                  (BSL_META_U(unit, "set.flags(0x%08X) are not equal to get.flags(0x%08X) %s.\n"), trap_config[0].flags,
                   trap_config[1].flags, EMPTY));
    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (sal_strcasecmp(test_type, CTEST_DNX_RX_TRAP_ERPP_TEST_TYPE_POSITIVE) == 0)
    {
        if (sal_strcasecmp(test_mode, CTEST_DNX_RX_TRAP_ERPP_TEST_MODE_ACTIONS) == 0)
        {
    ...
    else if (sal_strcasecmp(test_type, CTEST_DNX_RX_TRAP_ERPP_TEST_TYPE_NEGATIVE) == 0)
    {
        if (sal_strcasecmp(test_mode, CTEST_DNX_RX_TRAP_ERPP_TEST_MODE_ACTIONS) == 0)
        {
