...
static const ctest_dnx_dram_operation_rate_params_t Default_rate_params = {
    ...
    .ctest_dnx_dram_num_port_in_snake = 68,
    ...
    .expected_rate_deviation_in_percents = {6, 6, 0},
...
/**
 * \brief
 *   Set dram bound threshold.
 * \param [in] unit -
 *   HW identifier of unit.
 * \param [in] threshold -
 *   dram bound threshold.
 * \return
 *   Error code (as per 'shr_error_e').
 * \remark
 *   This setup works on both cores (SBC)
 ...
 */
static shr_error_e
ctest_dnx_dram_bound_threshold_set(
    int unit,
    uint32 threshold)
{
    uint32 entry_handle_id;
 ...
 */
static shr_error_e
ctest_dnx_dram_bound_threshold_get(
    int unit,
    uint32 *threshold)
{
    uint32 entry_handle_id;
    ...
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_INGRESS_CONG_CGM_DRAM_BOUND_STATE_TH, &entry_handle_id));
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_DRAM_BOUND_VOQ_SRAM_WORDS_QSIZE_TH, INST_SINGLE,
                               threshold);
    ...
    sh_sand_control_t * sand_control)
{
    ...
    bcm_pbmp_t nif_ports_bitmap;
    bcm_port_t nif_port;
    uint32 original_dram_bound_threshold;
    ...
    }
    {
        ...
        {
            /*
             * Configure snake between 2 ports, and expect
             * about 210G of dram rate.
             */
            operation_rate_params_p->ctest_dnx_dram_num_port_in_snake = 8;
            operation_rate_params_p->expected_dram_rate_in_mbps[0] = 210000;
        }
        else if (soc_is(unit, J2C_DEVICE))
        {
            /*
             * Configure snake between 9 CGE (100G) ports, and expect
             * about 950G of dram rate.
             */
            operation_rate_params_p->ctest_dnx_text_ports = "CGE";
            operation_rate_params_p->ctest_dnx_dram_num_port_in_snake = 36;
            operation_rate_params_p->expected_rate_in_mbps[0] = 95000;
            operation_rate_params_p->expected_dram_rate_in_mbps[0] = 950000;
        }
        /*
        ...
        SHR_CLI_EXIT_IF_ERR(sh_process_command(unit, "tr 141"), "failed on tr 141\r\n");
        {
            ...
            LOG_DEBUG_EX(BSL_LOG_MODULE,
                         "Force connect ports %d->%d on core %d %s\r\n", from_port_p[core_under_test], port_under_test,
                         core_under_test, EMPTY);
            SHR_IF_ERR_EXIT(ctest_dnx_dram_bound_threshold_get(unit, &original_dram_bound_threshold));
            /*
             * Config for 'snake' in order to activate all ports
             * we assume the number of active ports is larger than '1'
             */
                 ...
                 cycle_index++)
            {
                ...
                if (operation_rate_params_p->ctest_dnx_use_dram_only[cycle_index] != FALSE)
                {
                    ...
                    SHR_IF_ERR_EXIT(ctest_dnx_dram_bound_threshold_set(unit, 0));
                }
                else
                {
                    ...
                    SHR_IF_ERR_EXIT(ctest_dnx_dram_bound_threshold_set(unit, original_dram_bound_threshold));
                }
                SHR_IF_ERR_EXIT(bcm_port_get
                                (unit, from_port_p[core_under_test], &flags, &interface_info, &mapping_info));
                SHR_IF_ERR_EXIT(diag_sand_packet_create(unit, &packet_h));
