...
sh_sand_invoke_t dnx_mdb_tests[] = {
    ...
    ,
    {"lem_capacity", "table=LEM mode=1 app_id=0", CTEST_POSTCOMMIT}
    ,
    {"lem_capacity_mact", "table=LEM mode=1 app_id=2", CTEST_POSTCOMMIT}
    ,
    ...
    ,
    {"kaps_1_ser", "table=kaps1 mode=4", CTEST_POSTCOMMIT}
    ,
    {"kaps_2_ser", "table=kaps2 mode=4", CTEST_POSTCOMMIT}
    ,
#endif
    {NULL}
};
...
sh_sand_option_t dnx_mdb_test_options[] = {
    ...
    {"MoDe", SAL_FIELD_TYPE_INT32,
     "The test mode: 0-fast (tests ~1/10 of the capacity), 1-full(direct)/capacity test (EM/LPM) 2-capacity test mixed entry encoding (EM) 3-MDB iterators test(SW and HW) 4-SER KAPS TCAM only",
     "0"}
    ,
