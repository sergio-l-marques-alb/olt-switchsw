...
/** allow memregs.h include explicitely */
 ...
 */
static shr_error_e
diag_dbal_test_key_fields_allocation_serial(
    int unit,
    int *nof_entries,
    CONST dbal_logical_table_t * table,
    int max_entries_in_test)
...
*/
static shr_error_e
diag_dbal_test_key_fields_allocation_random(
    int unit,
    dbal_tables_e table_id,
    int *nof_entries,
    CONST dbal_logical_table_t * table,
    ...
    int *final_invalid_entries)
{
    ...
    if (table_id != DBAL_TABLE_EGRESS_PP_PORT || !(SHR_IS_BITSET(table->indications_bm, DBAL_TABLE_IND_IS_HOOK_ACTIVE)))
    {
        if (key_size < MIN_KEY_SIZE_FOR_RANDOMIZING)
        {
             ...
             */
            SHR_IF_ERR_EXIT(diag_dbal_test_key_fields_allocation_serial(unit, &nof_entries, table,
                                                                        max_entries_in_test));
        }
        else
        {
            /*
             * When key size is or larger than MIN_KEY_SIZE_FOR_RANDOMIZING bits, we check 32 entries:
             * minimum entry, maximum entry and 30 random entries
             */
            SHR_IF_ERR_EXIT(diag_dbal_test_key_fields_allocation_random(unit, table_id, &nof_entries,
    ...
    if (curr_l2p_field->hw_field != INVALIDf)
    {
        ...
        if ((is_read_only
             && (result_type_set->results_info[curr_l2p_field->field_pos_in_interface].permission !=
                 DBAL_PERMISSION_READONLY)) || (is_write_only
                                                &&
                                                (result_type_set->results_info
                                                 [curr_l2p_field->field_pos_in_interface].permission !=
...
/**
 * \brief
 * The function check if 2 fields are duplicated. DBAL doen't allow such definition. duplication can be also between
 * child fields.
 */
static shr_error_e
    ...
    dbal_tables_e table_id)
{
    ...
    for (jj = 0; jj < table->nof_key_fields; jj++)
    {
        ...
        for (kk = jj + 1; kk < table->nof_key_fields; kk++)
        {
            ...
            SHR_IF_ERR_EXIT(dbal_db_check_field_duplication
                            (unit, field_info->field_id, table->keys_info[kk].field_id, &is_duplication));
            if (is_duplication)
            {
    ...
    dnx_dbal_ltt_input_t * ltt_input)
{
    ...
    if (table_id == DBAL_TABLE_KBP_PCC_STATE_TABLE)
    {
        LOG_CLI((BSL_META("table %s not supported\n"), dbal_logical_table_to_string(unit, table_id)));
        SHR_EXIT();
    }
