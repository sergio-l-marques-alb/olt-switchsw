    ...
    int test_clean)
{
    ...
    bcm_field_flush_entry_info_t flush_entry_info_neg;
     ...
     * Negative tests
     */
    sal_memcpy(&flush_entry_info_neg, &flush_entry_info, sizeof(flush_entry_info_neg));
    flush_entry_info_neg.payload.hit_clear_cmd = FALSE;
    flush_entry_info_neg.payload.delete_cmd = TRUE;
    flush_entry_info_neg.payload.transplant_cmd = TRUE;
    ...
    rv = bcm_field_flush_entry_add(unit, flush_entry_flags, fg_id, flush_entry_id, &flush_entry_info_neg);
    ...
    if (rv != _SHR_E_PARAM)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Accepted both delete and transplant.\r\n");
    }
    ...
    if (test_traffic == FALSE)
    {
        ...
        flush_entry_info.payload.transplant_cmd = FALSE;
        flush_entry_info.payload.delete_cmd = TRUE;
        SHR_SET_SEVERITY_FOR_MODULE_SPECIFIC(BSL_LS_BCMDNX_FLDPROCDNX, bslSeverityFatal);
        SHR_SET_SEVERITY_FOR_MODULE_SPECIFIC(BSL_LS_SOCDNX_DBALDNX, bslSeverityFatal);
    ...
    /*
     * Replace the flush entry with a delete command entry.
     */
    SHR_IF_ERR_EXIT(bcm_field_flush_entry_delete(unit, flush_entry_flags, fg_id, flush_entry_id));
    flush_entry_info.payload.transplant_cmd = FALSE;
    flush_entry_info.payload.delete_cmd = TRUE;
    flush_entry_info.key.entry_action[0].value[0] = 3;
    SHR_SET_SEVERITY_FOR_MODULE_SPECIFIC(BSL_LS_BCMDNX_FLDPROCDNX, bslSeverityFatal);
    SHR_SET_SEVERITY_FOR_MODULE_SPECIFIC(BSL_LS_SOCDNX_DBALDNX, bslSeverityFatal);
    rv = bcm_field_flush_entry_add(unit, flush_entry_flags, fg_id, flush_entry_id, &flush_entry_info);
    SHR_SET_SEVERITY_FOR_MODULE_SPECIFIC(BSL_LS_BCMDNX_FLDPROCDNX, original_severity_fldprocdnx);
    SHR_SET_SEVERITY_FOR_MODULE_SPECIFIC(BSL_LS_SOCDNX_DBALDNX, original_severity_dbaldnx);
    if (rv != _SHR_E_PARAM)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Added flush entry with delete command but with payload actions.\r\n");
    }
    flush_entry_info.payload.nof_entry_actions = 0;
    SHR_IF_ERR_EXIT(bcm_field_flush_entry_add(unit, flush_entry_flags, fg_id, flush_entry_id, &flush_entry_info));
    flush_entry_info.payload.entry_action[0].type = bcmFieldActionCount;
    flush_entry_info.payload.entry_action[0].value[0] = 0;
    flush_entry_info.payload.entry_action[0].mask[0] = 0;
    flush_entry_info.payload.entry_action[0].valid_bit_value = 0;
    flush_entry_info.payload.entry_action[0].valid_bit_mask = 0;
    SHR_IF_ERR_EXIT(bcm_field_flush_entry_get(unit, 0, fg_id, flush_entry_id, &flush_entry_info_get));
    if (sal_memcmp(&flush_entry_info_get, &flush_entry_info, sizeof(flush_entry_info_get)))
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Get does not equal set.\r\n");
    ...
    if (test_traffic)
    {
        SHR_IF_ERR_EXIT(ctest_dnx_exem_learn_flush_traffic_test(unit, field_stage, 3));
        sal_sleep(1);
        /** Thresholds do not trigger delete entries, only transplant. Flush scan pulse deletes entries.*/
        SHR_IF_ERR_EXIT(ctest_dnx_exem_learn_flush_traffic_test(unit, field_stage, 3));
    }
