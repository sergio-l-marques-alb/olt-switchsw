...
#define CTEST_DNX_KBP_FWD_IPMC_LARGE     "IPMC_LARGE"
    ...
    uint8 *ipmc_large)
{
    ...
    else if ((!strncasecmp(*test, CTEST_DNX_KBP_FWD_IPMC, 16)))
    {
        ...
        *rpf = *host = *ipmc_large = FALSE;
        *ipmc = TRUE;
    }
    else if ((!strncasecmp(*test, CTEST_DNX_KBP_FWD_IPMC_LARGE, 16)))
    {
        *test = CTEST_DNX_KBP_FWD_IPMC_LARGE;
        *rpf = *host = FALSE;
        *ipmc = *ipmc_large = TRUE;
    }
    else
    {
...
static sh_sand_invoke_t dnx_kbp_fwd_capacity_ipv4_tests[] = {
    ...
    {CTEST_DNX_KBP_FWD_RPF, CTEST_DNX_KBP_CAPACITY_DEINIT_INIT "=YES " CTEST_DNX_KBP_TEST "=" CTEST_DNX_KBP_FWD_RPF " " CTEST_DNX_KBP_MEASURE, CTEST_POSTCOMMIT},
    {CTEST_DNX_KBP_FWD_RPF_PROFILE1, CTEST_DNX_KBP_CAPACITY_DEINIT_INIT "=YES " CTEST_DNX_KBP_TEST "=" CTEST_DNX_KBP_FWD_RPF_PROFILE1 " " CTEST_DNX_KBP_MEASURE " " CTEST_DNX_KBP_PROFILE "=1", CTEST_POSTCOMMIT},
    {CTEST_DNX_KBP_FWD_RPF_NO_PAYLOAD, CTEST_DNX_KBP_CAPACITY_DEINIT_INIT "=YES " CTEST_DNX_KBP_TEST "=" CTEST_DNX_KBP_FWD_RPF_NO_PAYLOAD " " CTEST_DNX_KBP_MEASURE, CTEST_POSTCOMMIT},
    ...
    {CTEST_DNX_KBP_FWD_IPMC_LARGE, CTEST_DNX_KBP_CAPACITY_DEINIT_INIT "=NO " CTEST_DNX_KBP_TEST "=" CTEST_DNX_KBP_FWD_IPMC_LARGE " " CTEST_DNX_KBP_MEASURE, CTEST_POSTCOMMIT},
    {NULL}
};
    ...
    sh_sand_control_t * sand_control)
{
    ...
    uint32 key_size = 0;
    uint32 res_length = 0;
    uint8 measure = FALSE, skip_config = FALSE, skip_deinit = FALSE;
    int start_time = 0;
    char *layer = CTEST_DNX_KBP_PERFORMANCE_LAYER_BCM;
    ...
    SHR_IF_ERR_EXIT(dnx_kbp_acl_capacity_entries
                    (unit, qual_list, nof_quals, action_list, nof_actions, fg_id, &capacity));
    LOG_CLI((BSL_META("Finished filling the table for %ds\n\n"), ((sal_time_usecs() - start_time) / (1000 * 1000))));
...
static sh_sand_invoke_t dnx_kbp_acl_capacity_tests[] = {
    {"K80_R32", CTEST_DNX_KBP_ACL_KEY_SIZE "=80 " CTEST_DNX_KBP_ACL_RESULT_LENGTH "=32 " CTEST_DNX_KBP_MEASURE, CTEST_POSTCOMMIT},
    {"K160_R32", CTEST_DNX_KBP_ACL_KEY_SIZE "=160 " CTEST_DNX_KBP_ACL_RESULT_LENGTH "=32 " CTEST_DNX_KBP_MEASURE, CTEST_POSTCOMMIT},
