    ...
    diag_dnx_bfd_semantic_test_struct * test)
{
    ...
    uint32 highest_mep_db_index, max_num_of_entries_in_bank, bank;
    ...
    if (for_modification == FALSE)
    {
        ...
        if ((endpoint_info->flags & BCM_BFD_ENDPOINT_IN_HW) != 0)
        {
            ...
            if ((mep_db_threshold != 0))
            {
                ...
                discr_low =
                    bank * max_num_of_entries_in_bank + ((test->gen_value >> ACC_DISCR_OFFSET) % highest_mep_db_index);
    ...
    bcm_bfd_endpoint_info_t * endpoint_info2)
{
    ...
    if ((endpoint_info->flags & BCM_BFD_ENDPOINT_IN_HW) != 0)
    {
        ...
        if ((endpoint_info->flags & BCM_BFD_ENDPOINT_IPV6) != 0)
        {
            ...
            SHR_CLI_EXIT_IF_ERR(dnx_oamp_mep_db_bfd_endpoint_get(unit, endpoint_info->id, entry_values),
                                "Test failed.  Could not read MEP profile\n");
            SHR_CLI_EXIT_IF_ERR(dnx_oam_oamp_mep_profile_get(unit, entry_values->mep_profile, mep_profile),
                                "Test failed.  Could not read MEP profile\n");
    ...
    int unit)
{
    ...
    for (i = 0; i < nof_iterations; i++)
    {
        ...
        if (is_allocated)
        {
            SHR_IF_ERR_EXIT(dnx_algo_oamp_mep_id_dealloc(unit, mep_id));
        }
    ...
    uint8 *is_free_ret)
{
    ...
    for (i = 0; i < nof_iterations; i++)
    {
        ...
        for (j = 0; j < nof_bfd_ipv6_extra_data_entries; j++)
        {
            ...
            {
                SHR_IF_ERR_EXIT(dnx_algo_oamp_mep_id_is_allocated(unit, current_mep_id, &is_allocated[j]));
            }
    ...
    int generator)
{
    ...
    for (i = 0; i < prime; i++)
    {
        ...
        else
        {
            ...
            SHR_IF_ERR_EXIT(free_all_mep_ids(unit));
        ...
        {
            dnx_algo_oamp_mep_id_is_allocated(unit, current_mep_id, &is_allocated);

            if (!is_allocated)
            {
                dnx_algo_oamp_mep_id_alloc(unit, DNX_ALGO_RES_ALLOCATE_WITH_ID, &mep_id_memory_type, &current_mep_id);
            }
        }
        current_mep_id -= oamp_mep_full_entry_threshold;
    ...
    diag_dnx_bfd_semantic_test_struct * test)
{
    ...
    SHR_ALLOC(bfd_endpoint_info, sizeof(bcm_bfd_endpoint_info_t), "Struct for creating BFD endpoints",
              "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
    SHR_ALLOC(bfd_endpoint_info_2, sizeof(bcm_bfd_endpoint_info_t), "Struct for reading BFD endpoints",
              "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SHR_ALLOC_SET_ZERO(endpoint_id, num_eps * sizeof(bcm_bfd_endpoint_t), "Array of endpoint IDs", "%s%s%s\r\n",
                       EMPTY, EMPTY, EMPTY);
    SHR_ALLOC_SET_ZERO(test, sizeof(diag_dnx_bfd_semantic_test_struct), "Struct for test variables", "%s%s%s\r\n",
                       EMPTY, EMPTY, EMPTY);
