...
int test_bfd_period[TEST_BFD_MAX_NUM_PERIODS];
/** Predefined values for BFD transmission rate for local endpoints */
int test_bfd_def_period[TEST_BFD_NUM_DEF_PERIODS];
/** IPv4 addresses */
int test_bfd_IP_address[TEST_BFD_NUM_IP];
/** IPv6 addresses */
bcm_ip6_t test_bfd_IPv6_address[TEST_BFD_NUM_IP];
/** Values for Desired Min TX and Required Min RX Intervals */
int test_bfd_intervals[TEST_BFD_MAX_NUM_INTERVALS];
...
diag_dnx_oamp_test_bfd_lock_t test_bfd_info_lock;
...
diag_dnx_oamp_test_bfd_lock_init(
    int unit)
{
    diag_dnx_oamp_test_bfd_lock_t *temp_state = &test_bfd_info_lock;
    SHR_FUNC_INIT_VARS(unit);

    if (temp_state->is_init == 0)
    {
        sal_memset(temp_state, 0, sizeof(diag_dnx_oamp_test_bfd_lock_t));
        temp_state->dnx_bfd_lock[unit] = sal_mutex_create("test_bfd_info_lock");
...
{
    SHR_FUNC_INIT_VARS(unit);
    if (0 != sal_mutex_take(test_bfd_info_lock.dnx_bfd_lock[unit], sal_mutex_FOREVER))
    {
...
{
    SHR_FUNC_INIT_VARS(unit);
    if (0 != sal_mutex_give(test_bfd_info_lock.dnx_bfd_lock[unit]))
    {
    ...
    int *_tunnel_id)
{
    ...
    *_tunnel_id = encap_id[0];
    ...
    for (i_ip = 0; i_ip < 8; i_ip++)
    {
        ...
        {
            SHR_CLI_EXIT_IF_ERR(diag_dnx_oamp_test_bfd_add_host(unit, test_bfd_IP_address[i_ip], vrf, fec[0]),
                                "{9}Error, create egress object, in_sysport=%d, in unit %d \n", in_sysport, unit);
        }
    }
    ...
    int endpoint_id)
{
    ...
    bfd_endpoint_info.local_diag = 7; /** validate SDK-165337 */
    ...
    int endpoint_2_id)
{
    ...
    LOG_CLI((BSL_META("Update MEP with period [%d] and detect %d\n"), test_bfd_period[period_i],
             (detTime * test_bfd_period[period_i])));
     ...
     */
    LOG_CLI((BSL_META("Wait %d ms to occur the events\n"), (((detTime * test_bfd_period[period_i]) + 1000000) / 1000)));
    sal_usleep((detTime * test_bfd_period[period_i]) + 1000000);
    ...
    */
    for (endpoint_id = 0; endpoint_id < (tot_nof_of_entries - nof_short_entries_created); endpoint_id++)
    {
        ...
        */
        ...
        src_ip_addr.ipv4_addr = test_bfd_IP_address[ip_i_1];
        remote_ip_addr.ipv4_addr = test_bfd_IP_address[ip_i_2];
...
sh_sand_invoke_t diag_dnx_oamp_bfd_tests[] = {
    {"bfd_ipv4_scale_test", "port=13 port2=14 Num_eNTries=1000 is_IPv6=0 Run_with_Updates=0 State_UPDate=0"}
    ,
    {"bfd_ipv6_scale_test", "port=13 port2=15 Num_eNTries=100 is_IPv6=1 Run_with_Updates=10 State_UPDate=5"}
    ,
    {"bfd_ipv4_time_interval_test", "port=13 port2=14 Num_eNTries=10 is_IPv6=0 Run_with_Updates=1 State_UPDate=0"}
    ,
    {"bfd_ipv4_state_test", "port=13 port2=14 Num_eNTries=10 is_IPv6=0 Run_with_Updates=1 State_UPDate=5"}
    ,
    {"bfd_ipv6_10_eps_test", "Port=13 Port2=14 Num_ENTries=10"}
    ,
    {"bfd_ipv6_2_eps_events_test", "Port=13 Port2=14 Num_ENTries=2 Run_with_Updates=1"}
    ,
    {"bfd_ipv6_2_eps_no_updates", "Port=13 Port2=15 Num_ENTries=2 is_IPv6=1 Run_with_Updates=0 State_UPDate=0"}
    ,
    {NULL}
};
