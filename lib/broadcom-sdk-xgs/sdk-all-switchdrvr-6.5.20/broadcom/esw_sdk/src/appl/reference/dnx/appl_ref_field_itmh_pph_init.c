    ...
    bcm_field_context_t context_id)
{
    ...
    if (stage == bcmFieldStageIngressPMF1)
    {
         ...
         * Set the key Gen Var.
         *   Bits usage:
         *      0-6 (0x49) ---> adj_size_layer_idx_egr_prs_idx_q_pph qualifier 7b (SYS_HDR_ADJUST_SIZE_FWD_LAYER_INDEX_EGR_PRS_IDX_QUAL_LENGTH)
         *          - bit 0 ---> SYS HDR ADJUST (1b) ---> Const value of 1 (0b1)
         *          - bits 1-3 ---> FWD_LAYER_INDEX (3b) ---> Const value of 4 (0b100)
         *          - bits 4-6 ---> EGR_PRS_IDX (3b) ---> Const value of 4 (0b100)
         *      7-12 (0x25) --->key_gen_var_q_pph qualifier 6b (used for compare)
         *          - bits 7- ---> FHEI_SIZE (2b) ---> Const value of 2 (0b10) (indicates FHEI size is 5B)
         *          - bits -12 ---> FHEI_TYPE (4b) ---> Const value of 5 (0b0101) (indicates FHEI type is TRAP)
         */
        bcm_field_context_param_info_t_init(&param_info);
    ...
    field_itmh_pph_fg_info_t * itmh_pph_fg_info)
{
    ...
    for (qual_index = 0; qual_index < fg_info.nof_quals; qual_index++)
    {
        ...
        /** Value for 'adj_size_layer_idx_egr_prs_idx_q_pph' qualifier, should be taken from KeyGenVar. */
        if ((itmh_pph_fg_info->itmh_pph_qual_info[qual_index].name != NULL) &&
            (!sal_strncmp
    ...
    */
    ...
    /*
     * for iPMF2 FG3 we need to add the offset of the compare operand
     */
