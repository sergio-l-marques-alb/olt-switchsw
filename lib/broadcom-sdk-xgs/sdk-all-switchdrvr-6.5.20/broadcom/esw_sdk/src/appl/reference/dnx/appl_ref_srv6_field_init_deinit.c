 ...
 *          -x2 Actions:
 *                  1. Action to set the Parsing-Start-Type system header signal
 *                  2. Action to set the egress_parsing_idx and fwd_layer_idx signals
 *
 *   \param [in] unit - device id
 *
 * \return
    ...
    int unit)
{
     ...
     * egress_parsing_idx is used so that the LBP can correctly calculate the ETH offset,
     * which should be removed in the ITPP, if such is not present, the PPH will account for it.
     * The fwd_layer_idx is used in egress to select the correct fwd_code.
     * the pph_fwd_layer_idx = fwd_layer_idx - egress_parsing_idx
     */
    bcm_field_qualifier_info_create_t_init(&qual_info);
    qual_info.size = 3;
    ...
    int unit)
{
    ...
    /**Parsing-Start-Offset up to FWD layer ETH + IPv6 (up to 62b) 6bits signal */
    bcm_field_qualifier_info_create_t_init(&qual_info);
    qual_info.size = 6;
     ...
     *                  using Dummy ETHERNET becuause sometimes full ETH is not
     *                  seen in system headers therefore Parser could fail in identify if using regular
     *                  PARSING_START_TYPE_ETHERNET, which requires to parse the VIDs of ETH.
