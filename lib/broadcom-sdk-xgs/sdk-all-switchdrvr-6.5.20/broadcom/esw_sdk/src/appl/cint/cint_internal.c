/*
 * $Id: cint_internal.c,v 1.44 Broadcom SDK $
...
cint_array_combine_dimensions(cint_parameter_desc_t * type,
    ...
    for(dim_index = t_array_count - 1; 
        ...
        type->dimensions[index] = type->dimensions[dim_index];
    }
                      ...
                      void* addr)
{
    ...
    rc = cint_variable_create_no_add(rvar, vname, desc, vflags,addr);
    if ((rc == CINT_E_NONE) && rvar && *rvar) {

        cint_variable_add(*rvar);
    }
...
cint_fparams_print(cint_parameter_desc_t* pd)
{
    ...
    if(cint_parameter_void(pd)) {
        ...
        CINT_PRINTF("("); 
        while((pd+1)->basetype) {
            CINT_PRINTF("%s %s, ", cint_datatype_format_pd(pd, _rstr, 0), pd->name); 
            pd++; 
        }
        CINT_PRINTF("%s %s)", cint_datatype_format_pd(pd, _rstr, 0), pd->name); 
    }   
