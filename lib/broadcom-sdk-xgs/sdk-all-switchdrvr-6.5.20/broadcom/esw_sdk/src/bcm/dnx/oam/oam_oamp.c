...
/** Number of bits and mask for NUM_CLOCKS_SCAN */
    ...
    uint16 *nof_hw_scans)
{
     ...
     *   rmep_scan_nof_clocks = rmep_max_self_contained * rmep_sys_clocks
    ...
    oam_oamp_profile_hw_data_t * mep_hw_profiles_delete_data)
{
    ...
    if (_SHR_IS_FLAG_SET(endpoint_info->flags, BCM_OAM_ENDPOINT_REPLACE))
    {
        ...
        /** Check if one-way DM is enabled */
   ...
   /** Map bcm ep type to mep db type */
    ...
    mep_profile.lmm_offset =
        dnx_oam_lm_dm_set_offset(unit, mep_db_entry, is_dual_ended_lm, 0 /** LM type */ , 0 /** LMM */ ,
    ...
    mep_profile.lmr_offset =
        dnx_oam_lm_dm_set_offset(unit, mep_db_entry, is_dual_ended_lm, 0 /** LM type */ , 1 /** LMR */ ,
                                 (endpoint_info->type == bcmOAMEndpointTypeMplsLmDmLsp),
                                 _SHR_IS_FLAG_SET(endpoint_info->flags2,
                                                  BCM_OAM_ENDPOINT_FLAGS2_EGRESS_INJECTION_DOWN));
    mep_profile.dmm_offset =
        dnx_oam_lm_dm_set_offset(unit, mep_db_entry, is_dual_ended_lm, 1 /** DM type */ , 0 /** DMM */ ,
                                 (endpoint_info->type == bcmOAMEndpointTypeMplsLmDmLsp),
                                 _SHR_IS_FLAG_SET(endpoint_info->flags2,
                                                  BCM_OAM_ENDPOINT_FLAGS2_EGRESS_INJECTION_DOWN));
    mep_profile.dmr_offset =
        dnx_oam_lm_dm_set_offset(unit, mep_db_entry, is_dual_ended_lm, 1 /** DM type */ , 1 /** DMR */ ,
                                 (endpoint_info->type == bcmOAMEndpointTypeMplsLmDmLsp),
                                 _SHR_IS_FLAG_SET(endpoint_info->flags2,
                                                  BCM_OAM_ENDPOINT_FLAGS2_EGRESS_INJECTION_DOWN));
    ...
    if (!DNX_OAM_IS_ENDPOINT_RFC6374(endpoint_info))
    {
        ...
        {
            /*
             * In case we are Down-MEP with UDH and in Jericho1 mode(inter-op) mep_pe_profile is already set
             */
            if (!((system_headers_mode == DBAL_ENUM_FVAL_SYSTEM_HEADERS_MODE_JERICHO_MODE) &&
    ...
    dnx_oam_mep_profile_t * mep_profile_data)
{
    ...
    if (opcode == OAMP_MEP_TX_OPCODE_OP0 || opcode == OAMP_MEP_TX_OPCODE_OP1)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_oam_oamp_get_transmit_rate_for_opcode
                        (unit, other_opcode, 0 /* dont-care for CCM rate */ , mep_profile_data, &other_opcode_rate));
    ...
    uint8 is_mpls_lm_dm_without_bfd_endpoint)
{
    ...
    {
        /** In case of disabled period, set the cnt to maximum value */
