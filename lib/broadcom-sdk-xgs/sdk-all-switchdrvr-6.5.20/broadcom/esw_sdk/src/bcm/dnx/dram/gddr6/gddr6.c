    ...
    int unit)
{
    ...
    /*
     * Signal that dynamic calibration should be enabled.
     * Note that dynamic calibration shouldn't be enabled before running tunning,
     * so don't enable it when running with 'SKIP_TUNE' on init.
     */
    ...
    int unit)
{
    ...
    {
        /*
         * For debug reasons, we can run the tune without the CDR step (which is the last step).
    ...
    int unit)
{
    ...
    /** Make sure master phys are initialized before slave phys */
    SHR_BIT_ITER(dram_bitmap, nof_drams, dram_index)
    {
        /** Init master phys (dram is master phy of itself) */
    ...
    SHR_BIT_ITER(dram_bitmap, nof_drams, dram_index)
    {
        /** Init slave phys (dram is not master phy of itself) */
        if (dram_index != dnx_data_dram.gddr6.master_phy_get(unit, dram_index)->master_dram_index)
        {
            SHR_IF_ERR_EXIT(soc_g6phy16_shmoo_phy_init(unit, dram_index));
        }
...
/**
 * \brief - Read dram temperature through the periodic temperature readout mechanism.
 *
 * \param [in] unit - unit number
 * \param [in] dram_index - dram index
 * \param [out] temp_readout_status - temperature read status
 *
 * \return
 *   shr_error_e
 *
 * \remark
 *   * Can be used only if periodic temperature readout is supported:
 *   *   dnx_data_dram.gddr6.periodic_temp_readout
 * \see
 *   * None
 */
static shr_error_e
    ...
    dnx_gddr6_periodic_temp_readout_status_t * temp_readout_status)
{
    ...
    /** Configure periodic temperature readout */
    ...
    /*
     * All-bank refresh must be enabled in order for the preiodic temperature
     * readout to work, since the algorithm waits for a ab_refresh in order
    ...
    SHR_BIT_ITER(refresh_ab_bitmap, nof_channels, channel)
    {
        ...
        /** Disable periodic temperature readout */
    ...
    bcm_switch_thermo_sensor_t * sensor_data)
{
    ...
    if (SAL_BOOT_PLISIM)
    {
        /** if on a simulation return valid temp and get out */
