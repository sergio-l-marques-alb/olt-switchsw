...
/**
 * \brief
 *   This function builds a 128 byte buffer from
 *   the input buffer for CRC calculations
 * \param [in] buffer  -
 *   data buffer.
 ...
 * \param [out] target_buffer-
 *  128 byte target buffer
 * \return
 *   * None
 * \remark
 *   The buffer is assumed to be cleared (all 0s)
 *   The simple method of doing this is in two stages:
 *   1.  Copy the data buffer to a 128 byte buffer,
 *   starting at index 10 (target_buffer[start_index] = buffer[0],
 *   target_buffer[start_index+1] = buffer[1] ... target_buffer[start_index + data_length - 1] = buffer[data_length - 1]
 *   2.  Reverse each 16 byte segment of the buffer:
 *   new buffer[0] = buffer[15], new_buffer[1] = buffer[14],
 *   ... new_buffer[15] = buffer[0], new_buffer[16] = buffer[31]...
 *   It is easy to see that step 2 can be done with a simple for
 *   loop: new_buffer[index-(index%16)+15-(index%16)]=buffer[index]
 *   (to reverse, take the part of index that divides by 16,
 *   index-(index%16), and add to it 15-[the remainder])
 *   Since only byte (start_index) - (start_index+data_length-1) of buffer are non-zero, we can just loop
 *   those indexes and use buffer[index-start_index] instead of buffer[index].
 * \see
 *  * None
 */
static void
