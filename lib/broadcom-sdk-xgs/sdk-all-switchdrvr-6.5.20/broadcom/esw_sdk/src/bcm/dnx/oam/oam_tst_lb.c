...
typedef struct
{
    uint32 flag;                /* 1:lb enalbe / 2:tst enable */
    bcm_oam_endpoint_t mepid;   /* Endpoint ID of Local MEP. */
    int lb_tlv_num;
    bcm_oam_tlv_t tlv;          /* flag1:tlv for LB, flag 2 tlv for tst tx */
    uint32 pkt_pri;             /* Optional priority value to be set on Ethernet outer/sole VLAN tag PDP and DEI fields 
                                 */
    uint32 inner_pkt_pri;       /* Optional priority value to be set on Ethernet inner VLAN tag PDP and DEI fields */
...
typedef struct
{
    uint32 flag;                /* 1:lb enalbe / 2:tst enable */
    bcm_oam_endpoint_t mepid;   /* Endpoint ID of Local MEP. */
    uint8 identifier_tc;        /* Class of service */
...
typedef struct
{
    ...
    bcm_mac_t mac_address;      /* DA MAC address of outgoing LBMs */
    ...
    int is_jericho_mode)
{
    ...
    if (!is_jericho_mode)
    {
        ...
        buffer[offset++] = ((fwd_dp & 0x3) << 6) | ((fwd_dest_info & 0x1F8000) >> 15);  /* 15-20 */
        buffer[offset++] = ((fwd_dest_info & 0x7F80) >> 7);     /* 7-14 */
        buffer[offset++] = ((fwd_dest_info & 0x7F) << 1) | (snoop_cmd & 0x8);   /* 0-6 */
...
/*
 * \brief
 * The function gets the gal value defined
 * for Y1731 MPLS TP
 * \param [in] unit -
 *   Relevant unit.
 * \param [out] value -
    ...
    int *header_offset)
{
    ...
    {   /* Prepare Ethernet header */
        if (!_SHR_IS_FLAG_SET(mep_db_entry.flags, DNX_OAMP_OAM_CCM_MEP_IS_UPMEP) && (dnx_drv_soc_property_get(unit, "injection_with_user_header_enable", 0      /* DEFAULT
                                                                                                                                                                 * VALUE */
                                                                                     ) == 1))
        {
    ...
    else
    {
        ...
        pph.field1 = 0;
        /*
         * fwd_code set to OAM_FWD_CODE_MPLS *mpls/PWE
         */
        pph.field2 = 0x4;
        pph.field3 = udh_size;
        pph.field4 = 0;
        pph.field5 = 0;
        /*
         * We assume here that 'soc_reg_above_64_get' lays chip data as per the various
         * combinations of COMPILER_64_* (See compiler.h), includeing the case of !COMPILER_HAS_LONGLONG.
         * If this becomes false, we would need here COMPILER_64_SET() with different input
    ...
    const bcm_oam_tst_tx_t * tst_ptr)
{
    ...
    {   /* flag is update, don't need to create a new gtf */
    ...
    bcm_oam_tst_rx_t * tst_ptr)
{
    ...
    ctf_info.flag = DNX_PP_OAM_LOOPBACK_TST_INFO_FLAGS_TST;     /* 2:tst enable */
    ...
    if (!(tst_ptr->flags & BCM_OAM_TST_RX_UPDATE))
    {
        ...
        {       /* lb&tst share one session_id in the same mep */
            for (session_id = 0; session_id < max_session_id; session_id++)
            {
                ...
                rv = bcm_dnx_sat_ctf_trap_data_to_session_map(unit, trap_data, 0, session_id);  /* OAM-ID is endpoint
                                                                                                 * ID and session ID */
                SHR_IF_ERR_EXIT(rv);
                break;
            }
    ...
    {
        packet_info.offsets.payload_offset = DNX_TST_LB_PKT_PLAYLOAD_OFFSET + 1;        /* for test tlv */
    }
...
/**
* \brief
*  Get OAM tst rx entry .
* \par DIRECT INPUT:
*   \param [in] unit  -
*    Relevant unit.
*   \param [in,out] tst_ptr  -
*    Pointer to the tst structure containing the information.
    ...
    bcm_oam_tst_rx_t * tst_ptr)
{
    ...
    ctf_info.session_map_flag--;
    if (ctf_info.session_map_flag == 0) /* no one using this map */
    {
    ...
    bcm_oam_tst_tx_t * tst_ptr)
{
    ...
    gtf_info.flag = DNX_PP_OAM_LOOPBACK_TST_INFO_FLAGS_TST;     /* 2:tst enable */
    ...
    sat_header->pkt_data->len = offset; /* whole packet( from internal header -to- TLV length and pattern type)- before
                                         * TLV patten */
    ...
    for (priority = 0; priority < BCM_SAT_GTF_NUM_OF_PRIORITIES; priority++)
    {
        ...
        for (i = 0; i < BCM_SAT_GTF_PACKET_LENGTH_NUM_OF_PATTERNS; i++)
        {
             ...
             */
            p_config->packet_edit[priority].packet_length[i] = offset - (is_test_tlv ? 1 : 0) + tst_ptr->tlv.tlv_length + 1;    /* add
                                                                                                                                 * end
                                                                                                                                 * tlv */
            ...
            {
                p_config->packet_edit[priority].packet_length[i] = min_size + udh_size; /* 67total(62 for Up mep) -
                                                                                         * 7sys_header(2 for Up mep) -
                                                                                         * udh + 4crc = 64 eth packet */
            }
        }
    ...
    bcm_oam_loopback_t * lb_ptr)
{
     ...
     * CTF configuration
     */
    if (lb_ptr->flags & BCM_OAM_LOOPBACK_WITH_CTF_ID)   /* if flag is update, user should give the cft_id */
    {
    ...
    else if (lb_ptr->flags & BCM_OAM_LOOPBACK_UPDATE)   /* flag is update, don't need to create a new Ctf */
    {
        ctf_id = lb_ptr->ctf_id;
        rv = _dnx_oam_sat_ctf_info_get(unit, ctf_id, &ctf_info, &found);
        if (!found)
    ...
    lb_ptr->ctf_id = ctf_id;    /* save ctf_id */
    ctf_info.flag = DNX_PP_OAM_LOOPBACK_TST_INFO_FLAGS_LBM;     /* 1:lb enable */
    ...
    bcm_oam_loopback_t * lb_ptr)
{
     ...
     */
    lb_ptr->gtf_id = gtf_id;    /* save gtf_id */

    gtf_info.flag = DNX_PP_OAM_LOOPBACK_TST_INFO_FLAGS_LBM;     /* 1:lb enable */
    ...
    for (i = 0; i < lb_ptr->num_tlvs; i++)
    {
        ...
        sat_header->pkt_data->len = offset;     /* whole packet( from internal header -to- TLV length and pattern
                                                 * type)- before TLV patten */
        ...
        if ((lb_ptr->tlvs[i].tlv_type == bcmOamTlvTypeData))
        {
            p_config->payload.payload_type = bcmSatPayloadConstant4Bytes;       /* tst_ptr->tlv.four_byte_repeatable_pattern 
                                                                                 */
    ...
    for (priority = 0; priority < BCM_SAT_GTF_NUM_OF_PRIORITIES; priority++)
    {
        for (i2 = 0; i2 < BCM_SAT_GTF_PACKET_LENGTH_NUM_OF_PATTERNS; i2++)
        {
             ...
             * length shouldn't include tlv pattern for test tlv, so reduce 1 byte
             */
            p_config->packet_edit[priority].packet_length[i2] = offset - (is_test_tlv ? 1 : 0) + lb_ptr->tlvs[i].tlv_length + 1;        /* add
                                                                                                                                         * end
                                                                                                                                         * tlv */
            if (p_config->packet_edit[priority].packet_length[i2] < (min_size + udh_size))
            {
                p_config->packet_edit[priority].packet_length[i2] = min_size + udh_size;        /* 66total(62 for Up
                                                                                                 * mep) - 6sys_header(2
                                                                                                 * * * * * * * * * * * *
                                                                                                 * * * * * * * * * * * *
                                                                                                 * for Up mep) - udh +
...
*    Relevant unit.
*   \param [in,out] lb_ptr  -
*    Pointer to the lb structure containing the information.
*    The information from the get procedure will be written
*    here.
* \par INDIRECT INPUT:
*   * none
