    ...
    dbal_enum_value_field_mdb_physical_table_e mdb_physical_table)
{
    ...
    row_width =
        ...
        dh.dh_info_get(unit, mdb_physical_table)->row_width;
    ...
    mdb_dh_macro_info_t * macros)
{
    ...
    if (dnx_data_mdb.pdbs.pdbs_info_get(unit, mdb_physical_table)->db_type == DBAL_ENUM_FVAL_MDB_DB_TYPE_EEDB)
    {
    ...
    if (dnx_data_mdb.pdbs.pdbs_info_get(unit, mdb_physical_table)->db_type == DBAL_ENUM_FVAL_MDB_DB_TYPE_EM)
    {
        ...
        if (MDB_EM_TABLE_IS_EMP(mdb_physical_table))
        {
    ...
    uint32 *nof_aging_bits)
{
    ...
    if (nof_entries > 0)
    {
        ...
        available_aging_bits =
            dnx_data_mdb.em.em_aging_info_get(unit, mdb_emp_table_id)->total_nof_aging_bits / nof_entries;
    ...
    dh_em_bank_info * em_bank_info)
{
    ...
    if (dnx_data_mdb.pdbs.pdbs_info_get(unit, mdb_physical_table)->db_type == DBAL_ENUM_FVAL_MDB_DB_TYPE_EM)
    {
        ...
        int row_width = dnx_data_mdb.dh.dh_info_get(unit, mdb_physical_table)->row_width;
        ...
        for (bank_iter = 0; bank_iter < nof_banks; bank_iter++)
        {
            ...
            if (em_bank_info[bank_iter].bank_size_used_macro_b > 0)
            {

                uint32 bucket_size_log2 =
                    utilex_log2_round_up(em_bank_info[bank_iter].
                                         bank_size_used_macro_b >> em_bank_info[bank_iter].is_flex);
                ...
                if (MDB_EM_TABLE_IS_EMP(mdb_physical_table))
                {
                    ...
                    uint32 nof_entries_in_bank =
                        em_bank_info[bank_iter].bank_size_used_macro_b << dnx_data_mdb.dh.macro_type_info_get(unit,
                                                                                                              MDB_MACRO_B)->nof_address_bits;
                    ...
                    for (emp_bank_iter = 0; emp_bank_iter < banks_to_update; emp_bank_iter++)
                    {
                        dbal_entry_value_field32_set(unit, entry_handle_mdb_24_table, DBAL_FIELD_MDB_ITEM_1,
                                                     bank_iter + (nof_banks * emp_bank_iter), nof_entries >> 3);
                        ...
                        nof_entries += nof_entries_in_bank;
                    }
                }
        ...
        if (MDB_EM_TABLE_IS_EMP(mdb_physical_table))
        {
            ...
            SHR_IF_ERR_EXIT(mdb_dh_em_emp_get_nof_aging_bits
                            (unit, dbal_physical_table_id, nof_entries, &nof_aging_bits));
...
mdb_dh_set_bpu_setting_macro(
    ...
    int macro_idx)
{
    dbal_enum_value_field_mdb_physical_table_e mdb_physical_table_id_array[DNX_DATA_MAX_MDB_DH_MAX_NOF_CLUSTERS];
    int cluster_idx;
    ...
    for (cluster_idx = 0; cluster_idx < max_nof_macro_clusters; cluster_idx++)
    {
        ...
        if ((cluster_idx == 0) || (mdb_physical_table_id_array[cluster_idx] == DBAL_NOF_ENUM_MDB_PHYSICAL_TABLE_VALUES)
        ...
        if (mdb_physical_table_id_array[cluster_idx] == DBAL_NOF_ENUM_MDB_PHYSICAL_TABLE_VALUES)
        {
            bpu_mode = 0;
        }
        else
        {
            db_type = dnx_data_mdb.pdbs.pdbs_info_get(unit, mdb_physical_table_id_array[cluster_idx])->db_type;
            row_width = dnx_data_mdb.dh.dh_info_get(unit, mdb_physical_table_id_array[cluster_idx])->row_width;
    ...
    int unit)
{
    ...
    for (global_macro_index = 0; global_macro_index < dnx_data_mdb.dh.total_nof_macroes_get(unit); global_macro_index++)
    {
    ...
    if (!MDB_DH_IS_EEDB_DATA_TABLE(mdb_physical_table_id) && !MDB_DH_IS_EEDB_LL_TABLE(mdb_physical_table_id))
    {
        ...
        for (cluster_idx = 0; cluster_idx < nof_clusters; cluster_idx++)
        {
            ...
            else if (MDB_INIT_IS_FEC_TABLE(mdb_physical_table_id))
            {
                ...
                start_address_mask = ~(((1 << dnx_data_mdb.direct.fec_address_mapping_bits_relevant_get(unit)) - 1)
                                       << fec_address_mapping_bits_offset);
                start_address_remapped =
                    (cluster_info.start_address & start_address_mask) | (cluster_pair_index <<
                                                                         fec_address_mapping_bits_offset);
            }
            ...
            {
                if (dnx_data_mdb.dh.macro_interface_mapping_get(unit,
                                                                global_macro_index)->interfaces[cluster_interface_id] ==
                    mdb_physical_table_id)
                {
                    break;
                }
            ...
            {
                key_format_info.em_bank_info[dh_serial_number_in_mag].bank_size_used_macro_b +=
                 ...
                 dnx_data_mdb.dh.macro_type_info_get(unit, cluster_info.macro_type)->nof_rows != 0))
            {
                SHR_ERR_EXIT(_SHR_E_CONFIG,
                             "%s: The address %d is not a multiple of the basic size of macro type %d.\n",
                             dbal_physical_table_to_string(unit, mdb_physical_table_id), cluster_info.start_address,
                             cluster_info.macro_type);
            }
