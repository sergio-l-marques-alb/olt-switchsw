...
/*
 * The MPLS reserved label(EL) always included in hash J2P
 */
#define SWITCH_LB_MPLS_EL_LABEL_INCLUDED_IN_HASH 0x80
/*
 * The largest NOF supported fields that have matching enabler flags.
 */
#define SWITCH_MAX_HEADER_HASH_ENABLE_FLAGS 12
 /*
  * The MAX NOF DBAL fields that can be associated with a single hash flag.
  */
 ...
 */
/*
 * Return true if all the flags_bits are set in flags
 */
 ...
 */
/*
 * hold information of the relevant DBAL fields that are associated with an hashing enabler field
 */
typedef struct
{
    /*
    ...
    uint32 flag;
    /*
     * NOF of DBAL fields that are associated with this flag
     */
    uint32 nof_fields;
    /*
     * Header field DBAL field
     */
...
/*
 * Hold an hashing header enablers information
 */
typedef struct
{
    /*
    ...
    /*
     * This header will hold the symmetrical flag in supported headers.
     * It is used for setting the HW in the correct way.
     */
    ...
    int nof_field_flags;
    /*
     * Index in the SW state array for the field enablers hashing data.
     */
...
/**
 * \brief - return all relevant information for a header type.
 *
 * \param [in] unit - Relevant unit.
 * \param [in] type - The type of header.
 * \param [out] header_info - holds all the relevant header info.
 * \return
...
/**
 * \brief
 * Enable using the layer record in hashing for all combinations of
 * Layer protocol, port LB profile and IN-LIF LB profile.
 * \param [in] unit - Relevant unit.
 *
 * \return
 *   Error indication according to shr_error_e enum
    ...
    int unit)
{
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);
    /** Initialize IPPB_USE_LAYER_RECORD_LB_INFO */
    ...
    {
        /** Initialize the SW state for Load balancing field enablers per header */
    ...
    if (dnx_data_switch.feature.feature_get(unit, dnx_data_switch_feature_mpls_labels_included_in_hash))
    {
        ...
        for (idx = 0; idx < nof_tables; idx++)
        {
            ...
            /*
             * Set access ID (index), this TCAM has only one valid entry on JR2_A0, JR2_Bx.
             */
    ...
    {
        /** Force only the EL(reserved MPLS label) to participate in the hash of the LB keys(if not terminated)*/
        uint32 entry_handle_id;
        /** Create handle */
...
/*
 * This function return for all the supported nibble values their
 * - dedicated protocol
 * - default protocol
 * - DBAL field in the table
 *
 * It used by the set/get function of the nibble speculation.
 */
static shr_error_e
...
/**
 * \brief - Verify that the flags that are been used for the hashing enablers are the expected flags.
 *
 * \param [in] unit - Relevant unit.
 * \param [in] flags - the flags to verify.
 * \param [in] header_info - header fields information.
 * \return
 *   Error indication according to shr_error_e enum
 *
 * \remark
    ...
    dnx_switch_hash_header_attr_t * header_info)
{
    ...
    /*
     * In case this header has a symmetrical option and the non symmetrical field is set, make sure that also the related field flag is on (e.t IP address, MAC address...).
     */
    ...
    /*
     * Verify that only the expected fields are used by removing the already verified flags from the mask only if all
     * the flag bits are present in the mask.
     * In case of unsupported flags or partial flags the mask will still contain bits and the verification will fail.
     */
    ...
    uint32 mask)
{
    ...
    /*
     * Due to HW limitation the symmetrical and un-symmetrical virtual registers must be turn off
     * before setting one of them on.
     */
    ...
    /*
     * In case the non-symmetrical field is set the symmetrical field should be omitted as both should be configure together
     * by the user but only one of them should be set in the HW.
     */
    ...
    /*
     * Update all the fields enablers
     */
    for (flag_iter = 0; flag_iter < header_info.nof_field_flags; flag_iter++)
    {
        ...
        /*
         * In case this is the symmetrical field or the non symmetrical field the VRs were already been set to zero.
         * They should now only be set to 1 as setting one of them to 1 and the other to zero will cause
         * misbehavior as they share HW bits configuration.
         */
    ...
    /*
     * As the HW limit some of the writes to the VRs, in some cases the DBAL won't be used and the empty commit will fail.
     */
    if (dbal_commit)
    {
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
