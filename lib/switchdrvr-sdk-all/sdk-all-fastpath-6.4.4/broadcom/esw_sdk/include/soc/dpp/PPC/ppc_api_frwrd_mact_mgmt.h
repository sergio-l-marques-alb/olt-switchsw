...
typedef enum
{
  ...
  /*
   *  Trap performed for unknown DA. Relevant only for
   *  Soc_petra-B. Note: there is also forwarding according to the
   *  VSI default, see soc_ppd_vsi_default_frwrd_info_set.   
   */
  SOC_PPC_FRWRD_MACT_TRAP_TYPE_DA_UNKNOWN = 2,
  /*
   *  Number of types in SOC_PPC_FRWRD_MACT_TRAP_TYPE
   */
  SOC_PPC_NOF_FRWRD_MACT_TRAP_TYPES = 3
...
typedef struct
{
   ...
   *  SOC_PPD_RIF_INFO) Then - If this field is set to TRUE then
...
typedef struct
{
   ...
   *  SOC_PPD_FRWRD_MACT_EVENT_TYPE_REFRESH
   */
...
typedef struct
{
   ...
   *  See SOC_PPD_FRWRD_MACT_SHADOW_MODE. However the user can use
