/* $Id: arad_tbl_access.h,v 1.100 Broadcom SDK $
...
/*
 *  The biggest, here according to ARAD_IHP_KEY_PROGRAM4_TBL_ENTRY_SIZE
 */
...
#define ARAD_EGQ_QDCT_TBL_ENTRY_SIZE                                                    5 /* to support jer*/
#define ARAD_EGQ_QQST_TBL_ENTRY_SIZE                                                    5 /* to support jer*/
...
#define ARAD_MAX_NOF_REPS                                                   (0xfffffff) /* 7F's */
...
#define ARAD_TCAM_NOF_LINES_SMALL      SOC_PPC_TCAM_NOF_LINES_ARAD_SMALL

#define ARAD_TCAM_NOF_LINES_PER_BANK(unit, bank_ndx)   ((bank_ndx < SOC_DPP_DEFS_GET(unit, nof_tcam_big_banks))? SOC_DPP_DEFS_GET(unit, nof_tcam_big_bank_lines):ARAD_TCAM_NOF_LINES_SMALL)
...
/* ARAD_EPNI_LLVP_TABLE_TBL_DATA: \{TPID-Profile (3), Packet-Tag-Format (5)\} */
#define ARAD_TBL_EPNI_LLVP_KEY_ENTRY_OFFSET(tpid_profile, pkt_tag_format)   \
  ((tpid_profile<<5)+(pkt_tag_format))


/* ARAD_VTT_LLVP_TABLE: \{Leaf-context(4), TPID-Profile (3), Packet-Tag-Format (5)\} */
...
#define ARAD_VSI_DEFAULT_FRWRD_KEY_ENTRY_OFFSET(key)                                                                                                               \
          SOC_SAND_SET_FLD_IN_PLACE((key)->da_type, ARAD_VSI_DEFAULT_FRWRD_KEY_DA_TYPE_SHIFT, ARAD_VSI_DEFAULT_FRWRD_KEY_DA_TYPE_MASK) |                                     \
          SOC_SAND_SET_FLD_IN_PLACE(((key)->orientation == SOC_SAND_PP_HUB_SPOKE_ORIENTATION_HUB) , ARAD_VSI_DEFAULT_FRWRD_KEY_ORIENTATION_SHIFT, ARAD_VSI_DEFAULT_FRWRD_KEY_ORIENTATION_MASK) |                         \
          SOC_SAND_SET_FLD_IN_PLACE((key)->default_forward_profile, ARAD_VSI_DEFAULT_FRWRD_KEY_FORWARD_PROFILE_SHIFT, ARAD_VSI_DEFAULT_FRWRD_KEY_FORWARD_PROFILE_MASK)
...
/* {Incoming-Tag-Format (4), VLAN-Edit-Profile (4/5) } */
...
#define ARAD_PP_EPNI_EGRESS_EDIT_CMD_TBL_FLD_SIZE   16
/* Bits[n+1:n+0] - Number of tags to remove, Bits[n+3:n+2] - Inner PCP DEI source Bits[n+5:n+4] - Outer PCP DEI source 
   Bits[n+8:n+6] - Inner VID source , Bits[n+11:n+9] - Outer VID source , Bits[n+13:n+12] - Inner TPID Index ,Bits[n+15:n+14] - Outer TPID Index*/
#define ARAD_PP_EPNI_EGRESS_EDIT_CMD_TBL_INFO(outer_tpid, inner_tpid, outer_vid, inner_vid, outer_pcp_dei, inner_pcp_dei, tags_to_remove) \
   ((outer_tpid << 14) +  (inner_tpid << 12) + (outer_vid << 9) + (inner_vid << 6) + (outer_pcp_dei << 4) \
     + (inner_pcp_dei << 2) + tags_to_remove);
...
/* {Type = Ethernet (3'b0001), 1'b0, Packet-Format-Qualifier[1]. Outer-Tag (1) UP-PCP (3), DEI (1)} */
#define ARAD_IHP_TC_DP_MAP_TABLE_TBL_KEY_LSB_L2_PREFIX                   (0x2) /* 3'b0001 for type + 1'b0 */
...
#define ARAD_IHP_TC_DP_MAP_TABLE_TBL_KEY_LSB_MPLS_PREFIX                 (0x1<<5) /* (2'b01) + 5'b0 */
...
/* {Type = TC-DP (4'b0000), 1'b0, Forward-Action. TC (3), Forward-Action. DP (2)} */
...
typedef struct
{
  ...
  uint32 fwd_prcessing_profile;
  uint32 lem_1st_lkp_found;
  uint32 lem_2nd_lkp_found;
  uint32 lpm_1st_lkp_found;
  uint32 lpm_2nd_lkp_found;
  uint32 tcam_found;
  uint32 tcam_traps_found;
  uint32 elk_error;
  uint32 elk_found_result;
  uint32 stamp_native_vsi;
  uint32 mask_parser_leaf_context;
  uint32 mask_in_port_profile;
  uint32 mask_ptc_profile;
  uint32 mask_packet_format_code;
  uint32 mask_packet_format_qualifier_0;
  uint32 mask_packet_format_qualifier_fwd;
  uint32 mask_forwarding_code;
  uint32 mask_forwarding_offset_index;
  uint32 mask_forwarding_offset_index_ext;
  uint32 mask_cpu_trap_code;
  uint32 mask_in_lif_profile;
  uint32 mask_out_lif_range;
  uint32 mask_llvp_incoming_tag_structure;
  uint32 mask_vt_processing_profile;
  uint32 mask_vt_lookup_0_found;
  uint32 mask_vt_lookup_1_found;
  uint32 mask_tt_processing_profile;
  uint32 mask_tt_lookup_0_found;
  uint32 mask_tt_lookup_1_found;
  uint32 mask_fwd_prcessing_profile;
  uint32 mask_lem_1st_lkp_found;
  uint32 mask_lem_2nd_lkp_found;
  uint32 mask_lpm_1st_lkp_found;
  uint32 mask_lpm_2nd_lkp_found;
  uint32 mask_tcam_found;
  uint32 mask_tcam_traps_found;
  uint32 mask_elk_error;
  uint32 mask_elk_found_result;
  uint32 mask_stamp_native_vsi;
  uint32 program;
  uint32 valid;
  /* Jericho fields */
  uint32 in_rif_profile;
  uint32 mask_in_rif_profile;
  uint32 tcam_found_1;
  uint32 mask_tcam_found_1;
  uint32 tcam_traps_found_1;
  uint32 mask_tcam_traps_found_1;
  uint32 rpf_stamp_native_vsi;
  uint32 mask_rpf_stamp_native_vsi;
...
typedef struct
{
  uint32 tpid_profile;
  uint32 vsi;
  uint64 learn_info; /* Until arad+: this field is 16b. For jericho: 40b */
  uint32 service_type;
  uint32 tt_learn_enable;
  uint32 vsi_assignment_mode;
  uint32 oam_lif_set;
...
typedef struct
{
  ...
  uint32 key_16b_inst2; /* used in Jericho only */
  uint32 key_16b_inst3; /* used in Jericho only */
...
typedef struct
{
  uint32 key_16b_inst2; /* not used in Jericho */
  uint32 key_16b_inst3; /* not used in Jericho */
  uint32 key_16b_inst4; /*used in Jericho only */
  uint32 key_16b_inst5; /*used in Jericho only */
  uint32 key_16b_inst6; /*used in Jericho only */
  uint32 key_16b_inst7; /*used in Jericho only */
...
typedef struct
{
  ...
  uint32 instruction_3_32b; /* 16b in Jericho */
  uint32 instruction_4_32b;
  uint32 instruction_5_32b;
  uint32 key_a_inst_0_to_5_valid; /* Up to 7 in Jericho */
  uint32 key_b_inst_0_to_5_valid; /* Up to 7 in Jericho */
  uint32 key_c_inst_0_to_5_valid; /* Up to 7 in Jericho */
...
typedef struct
{
  ...
  uint32 edit_command_inner_vid_source;
  uint32 edit_command_inner_pcp_dei_source;
  uint32 edit_command_bytes_to_remove; /*corresponds to tags_to_remove field*/
  uint32 outer_tpid_ndx;
  uint32 inner_tpid_ndx;
...
/* code generated from the register/memory file */
...
typedef struct
{
  ...
  uint32 rif_profile;  
  /* RIF profile is 6 bits, the 2 MSB bits are used for urpf and default routing. 
  	 the last 4 bits for routing enablers combinations and intf class.
     the number of bits used for class ID is defined according to the number of routing enablers combinations.
     for example, if we have 8 routing enablers combinations there is 3 bits for routing enablers combinations, and one bit for intf class.
 */
...
#define ARAD_PP_IHB_FEC_FORMAT_A (0) /* 1 bit*/
#define ARAD_PP_IHB_FEC_FORMAT_B (4) /* 3 bits */
#define ARAD_PP_IHB_FEC_FORMAT_C (5) /* 3 bits */
#define ARAD_PP_IHB_FEC_FORMAT_NULL (3) /* 2 bits */
...
typedef struct
{
  ...
  uint32 uc_rpf_mode;
  uint32 mc_rpf_mode;
  uint32 format; /*0'1b-A, 4'3b-B, 5'3b-B, 3'2b-NULL*/
  uint32 forwarding_info;
  uint32 eei; /* format C */
  uint32 out_lif; /* format B, A */
  uint32 arp_ptr; /* format A */
...
typedef struct
{
  uint32 program_selection_profile; /* this parameter is two bits each bit represent different port profile. when modifing, change only the bit that related to profile..*/
  uint32 action_profile_sa_drop_index;
...
typedef struct
{
  SOC_SAND_PP_IP_TTL ttl_scope; /* 8 bit value stored in the EGQ_TTL_SCOPE register */
} ARAD_PP_EGQ_TTL_SCOPE_REG_TBL_DATA;

typedef struct
{
  uint32 ttl_scope_index; /* 3 bit vlaue stored in the EGQ_TTL_SCOPE array pointing to a value in the EGQ_TTL_SCOPE register */
...
/* FEM sizes: 4/16/19 in Arad, 4/19/24 in Jericho */
 ...
 */
int arad_tbl_is_dynamic(int unit, soc_mem_t mem);
...
uint32
    arad_pp_ihb_mrr_snp_act_profile_tbl_get_unsafe(
        SOC_SAND_IN  int  unit,
        SOC_SAND_IN  uint32   entry_offset,
        SOC_SAND_IN  SOC_TMC_CMD_TYPE cmnd_type,
        SOC_SAND_OUT  uint32 *sampling_probability
    );

uint32
    arad_pp_ihb_mrr_snp_act_profile_tbl_set_unsafe(
        SOC_SAND_IN  int  unit,
        SOC_SAND_IN  uint32   entry_offset,
        SOC_SAND_IN  SOC_TMC_CMD_TYPE cmnd_type,
        SOC_SAND_IN  uint32 sampling_probability
    );
...
/*
 * Set indirect table Tcam Bank from block IHB,
 * doesn't take semaphore.
 */
uint32
...
/* Get the id of the next entry in the table. */
uint32
...
/* Set the id of the next entry in an existing entry if possible (if the format permits it). */
uint32
 ...
 * DESCRIPTION 
 *   This callback is meant to be used with arad_fill_table_with_variable_values_by_caching (for more info see
 *   arad_fill_table_with_variable_values_by_caching).
 *   For each entry written with arad_fill_table_with_variable_values_by_caching, this callback will be called
 ...
 *   value - The value to be written at index to the specified memory (to arad_fill_table_with_variable_values_by_caching).
 *           Mapping to HW:
 ...
 *            Etc...
 *   opaque - Opaque user data that was passed to arad_fill_table_with_variable_values_by_caching.
 */
...
/* } __ARAD_TBL_ACCESS_H_INCLUDED__*/
#endif
