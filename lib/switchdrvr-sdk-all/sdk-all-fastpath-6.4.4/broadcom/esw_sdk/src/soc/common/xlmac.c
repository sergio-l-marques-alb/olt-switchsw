...
mac_xl_init(int unit, soc_port_t port)
{
    ...
    if (SOC_IS_SABER2(unit)) {
        soc_reg64_field32_set(unit, XLMAC_RX_MAX_SIZEr, &rval,
                 RX_MAX_SIZEf, 0x3000);
    } else
#endif
    {
        soc_reg64_field32_set(unit, XLMAC_RX_MAX_SIZEr, &rval,
                RX_MAX_SIZEf, JUMBO_MAXSZ);
    }
 ...
 */
STATIC int
mac_xl_enable_set_common(int unit, soc_port_t port, int enable)
{
    uint64 ctrl, octrl;
    pbmp_t mask;
    uint32 soft_reset = 0;
    ...
    LOG_VERBOSE(BSL_LS_SOC_10G,
                (BSL_META_U(unit,
                        "mac_xl_enable_set_common: unit %d port %s enable=%s\n"),
                 unit, SOC_PORT_NAME(unit, port),
                 enable ? "True" : "False"));
...
mac_xl_enable_set(int unit, soc_port_t port, int enable)
{
    ...
    if(SOC_IS_SABER2(unit)) {
        ...
        /* In saber2 chips, port needs to enabled before MAC */
        SOC_IF_ERROR_RETURN(soc_saber2_port_enable_set(unit, port, enable));
    }
#endif

    SOC_IF_ERROR_RETURN(mac_xl_enable_set_common(unit, port, enable));
