...
STATIC int
_combo28_calib_2D(int unit, int drc_ndx, combo28_shmoo_container_t *scPtr, uint32 calibMode, uint32 calibPos)
{
    uint32 x;
    uint32 y;
    uint32 xStart;
...
_shmoo_combo28_calib_2D(int unit, int drc_ndx, combo28_shmoo_container_t *scPtr)
{
    ...
    {
        case SHMOO_COMBO28_ADDR_CTRL_SHORT:
            return _combo28_calib_2D(unit, drc_ndx, scPtr, SHMOO_COMBO28_WORD, SHMOO_COMBO28_CALIB_CENTER_PASS);
        case SHMOO_COMBO28_DQS2CK:
            return _combo28_calib_2D(unit, drc_ndx, scPtr, SHMOO_COMBO28_BYTE, SHMOO_COMBO28_CALIB_RISING_EDGE);
        case SHMOO_COMBO28_WCK2CK:
            return _combo28_calib_2D(unit, drc_ndx, scPtr, SHMOO_COMBO28_HALFWORD, SHMOO_COMBO28_CALIB_FALLING_EDGE);
        case SHMOO_COMBO28_RD_EN_FISH:
            break;
        case SHMOO_COMBO28_RD_VALID_FISH:
            break;
        ...
        case SHMOO_COMBO28_EDC_START_FISH:
            break;
        case SHMOO_COMBO28_RD_SHORT_FISH:
            return _combo28_calib_2D(unit, drc_ndx, scPtr, SHMOO_COMBO28_BYTE, SHMOO_COMBO28_CALIB_CENTER_PASS);
        case SHMOO_COMBO28_RD_DESKEW:
            return _combo28_calib_2D(unit, drc_ndx, scPtr, SHMOO_COMBO28_BIT, SHMOO_COMBO28_CALIB_CENTER_PASS);
        case SHMOO_COMBO28_DBI_EDC_RD_DESKEW:
            return _combo28_calib_2D(unit, drc_ndx, scPtr, SHMOO_COMBO28_BIT, SHMOO_COMBO28_CALIB_CENTER_PASS);
        case SHMOO_COMBO28_RD_SHORT:
            return _combo28_calib_2D(unit, drc_ndx, scPtr, SHMOO_COMBO28_BYTE, SHMOO_COMBO28_CALIB_CENTER_PASS);
        ...
        case SHMOO_COMBO28_RD_SHORT:
            switch(calibMode)
            {
                ...
                case SHMOO_COMBO28_BYTE:
                    ...
                    READ_DDRC28_CONTROL_REGS_RESERVED_REGr(unit, drc_ndx, &data);
                    if(xMin <= (*scPtr).endUI[0])
                    {
                        if((xMax - xMin) < (*scPtr).endUI[0])
                        {
                            data = ((((xMax - xMin) << 4) | 0x4) & 0x1FFC) | (data & 0xFFFFE003);
                        }
                        else
                        {
                    ...
                    {
                        data = ((((xMax - xMin) << 4) | 0x8) & 0x1FFC) | (data & 0xFFFFE003);
                    }
                    WRITE_DDRC28_CONTROL_REGS_RESERVED_REGr(unit, drc_ndx, data);
...
/*A11*/                         "A11. ZQ calibration\n")));
        ...
        if(shmoo_dram_info.zq_cal_array[ndx] != ndx)
        {
            ...
            temp2 = soc_phy_ddrc28_reg_field_get(unit, SOC_PHY_REG_COMMON_IO_CONFIGr, data, SOC_PHY_FLD_NDf) - 3;                   /* Offset unrolling */
        }
