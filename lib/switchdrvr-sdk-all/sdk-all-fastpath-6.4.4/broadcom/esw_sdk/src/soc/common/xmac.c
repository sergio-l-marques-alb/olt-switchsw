...
mac_x_init(int unit, soc_port_t port)
{
    ...
    if (SOC_IS_CALADAN3(unit)) { 
        if (soc_property_get(unit, spn_LRP_BYPASS, 0)) {
            /* Loopback tests mode */
            soc_reg64_field32_set(unit, XMAC_TX_CTRLr, &tx_ctrl, CRC_MODEf, 
                                  XMAC_CRC_REGEN);
        } else {
            soc_reg64_field32_set(unit, XMAC_TX_CTRLr, &tx_ctrl, CRC_MODEf, 
                                  XMAC_CRC_APPEND);
    ...
    defined(BCM_CALADAN3_SUPPORT) || defined(BCM_KATANA_SUPPORT)
   ...
   if (SOC_IS_KATANA2(unit) && SOC_PBMP_MEMBER(si->linkphy_pbm, port)) {
      soc_reg64_field32_set(unit, XMAC_RX_CTRLr, &rx_ctrl, STRIP_CRCf, 1);
          ...
          BCM_CALADAN3_SUPPORT || BCM_KATANA_SUPPORT */
   SOC_IF_ERROR_RETURN(WRITE_XMAC_RX_CTRLr(unit, port, rx_ctrl));
...
#if defined(BCM_KATANA2_SUPPORT)
    if (SOC_IS_KATANA2(unit)) {
        sgn_det = soc_property_get(unit, spn_SERDES_RX_LOS, 0);
...
mac_x_speed_set(int unit, soc_port_t port, int speed)
{
	...
	if ( SOC_IS_CALADAN3(unit) ){
            if ((speed < 10000) || (port == si->management_port)) {
                 xpc_enable = 1;
            }
        }
        ...
        if (IS_XT_PORT(unit, port) && (speed >= 10000)) {
            ...
            if ((speed >= 10000) && (speed <= 11000)) {
                ...
                if (si->port_num_lanes[port] == 1) { /* XFI */
                    ...
                    core_mode = 1;
                    if (IS_XW_PORT(unit, port) && SOC_IS_HELIX4(unit)) {
                        phy_mode = 1;
                    }
                }
...
#if defined(BCM_TRIUMPH3_SUPPORT) || defined(BCM_KATANA2_SUPPORT)
    if (SOC_IS_TRIUMPH3(unit) || SOC_IS_KATANA2(unit) ) {
        runt = (mode == SOC_ENCAP_HIGIG2) ? XMAC_RUNT_THRESHOLD_HG2 : 
                ((mode == SOC_ENCAP_HIGIG) ? XMAC_RUNT_THRESHOLD_HG1 : XMAC_RUNT_THRESHOLD_XE);
        SOC_IF_ERROR_RETURN
                  ...
                  int value)
{
    ...
    case SOC_MAC_CONTROL_FAULT_LOCAL_STATUS:
	    if (SOC_IS_CALADAN3(unit)) {
            SOC_IF_ERROR_RETURN
                (soc_reg_field32_modify(unit, XMAC_CLEAR_RX_LSS_STATUSr, port, CLEAR_LOCAL_FAULT_STATUSf, 1));
            SOC_IF_ERROR_RETURN
                (soc_reg_field32_modify(unit, XMAC_CLEAR_RX_LSS_STATUSr, port, CLEAR_LOCAL_FAULT_STATUSf, 0));
        }
        break;
    ...
    case SOC_MAC_CONTROL_FAULT_REMOTE_STATUS:
	    if (SOC_IS_CALADAN3(unit)) {
            SOC_IF_ERROR_RETURN
                (soc_reg_field32_modify(unit, XMAC_CLEAR_RX_LSS_STATUSr, port, CLEAR_REMOTE_FAULT_STATUSf, 1));
            SOC_IF_ERROR_RETURN
                (soc_reg_field32_modify(unit, XMAC_CLEAR_RX_LSS_STATUSr, port, CLEAR_REMOTE_FAULT_STATUSf, 0));
        }
        break;
#endif
