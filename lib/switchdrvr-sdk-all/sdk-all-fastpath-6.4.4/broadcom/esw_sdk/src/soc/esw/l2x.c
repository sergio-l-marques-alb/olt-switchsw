...
soc_l2x_attach(int unit)
{
    ...
    soc->l2x_age_timeout = soc_property_get(unit, spn_ARL_CLEAN_TIMEOUT_USEC,
                                            15000000);
    soc->l2x_mode = soc_property_get(unit, spn_L2XMSG_MODE, L2MODE_POLL);
...
soc_l2x_port_age(int unit, soc_reg_t reg0, soc_reg_t reg1)
{
    ...
    if (!SOC_IS_TOMAHAWK(unit) && SOC_IS_TD2_TT2(unit) &&
        reg0 == L2_BULK_CONTROLr && reg1 == INVALIDr) {
...
_soc_l2x_frozen_cml_restore(int unit)
{
    ...
    if (f_cml->frozen == 1) {
        SOC_PBMP_ASSIGN(e_pbmp, PBMP_E_ALL(unit));
#ifdef BCM_KATANA2_SUPPORT
        if (soc_feature(unit, soc_feature_linkphy_coe) ||
            soc_feature(unit, soc_feature_subtag_coe)) {
...
int
soc_l2x_is_frozen(int unit, int *frozen)
{
...
soc_l2x_freeze(int unit)
{
    ...
    /* Tomahawk uses software based L2 aging mechanism. The l2 lock
     * in effect prevents the L2 aging action from happening until
     * it is released.
     */
    if (!SOC_IS_TOMAHAWK(unit)) {
