...
int pm4x25_port_soft_reset_toggle(int unit, int port, pm_info_t pm_info, int idx)
{
    ...
    if(!SOC_IS_JERICHO(unit)) { /*FIME - temporary WA for Jericho PCID crash*/
...
int pm4x25_port_tsc_reset_set(int unit, int port, int in_reset)
{
    uint32 reg_val;
    SOC_INIT_FUNC_DEFS;
...
static int _clport_mode_get(int unit, int port, int bindex, 
...
int pm4x25_port_cntmaxsize_get(int unit, int port, pm_info_t pm_info, int *val)
{
    ...
    _SOC_IF_ERR_EXIT(READ_CLPORT_CNTMAXSIZEr(unit, port, (uint32_t *)val));
...
int pm4x25_port_cntmaxsize_set(int unit, int port, pm_info_t pm_info, int val)
{
    ...
    _SOC_IF_ERR_EXIT(WRITE_CLPORT_CNTMAXSIZEr(unit, port, (uint32_t)val));
...
int pm4x25_port_modid_set (int unit, int port, pm_info_t pm_info, int value)
{
    ...
    _SOC_IF_ERR_EXIT(READ_CLPORT_CONFIGr(unit, port, &rval));
    soc_reg_field_set(unit, CLPORT_CONFIGr, &rval, MY_MODIDf, modid);
    _SOC_IF_ERR_EXIT(WRITE_CLPORT_CONFIGr(unit, port, rval));
...
int pm4x25_port_led_chain_config (int unit, int port, pm_info_t pm, int value)
{
    ...
    _SOC_IF_ERR_EXIT(WRITE_CLPORT_LED_CHAIN_CONFIGr (unit, port, value));
...
#endif /* PORTMOD_PM4X25TD_SUPPORT */
    {
        ...
        if(enable) {
            ...
            if (PM_4x25_INFO(pm_info)->nof_phys > 0) {
                /* Get Serdes OOR */
                _SOC_IF_ERR_EXIT(pm4x25_port_tsc_reset_set(unit, port, 1));
                _SOC_IF_ERR_EXIT(pm4x25_port_tsc_reset_set(unit, port, 0));
...
int pm4x25_port_enable_set(int unit, int port, pm_info_t pm_info, int flags, int enable)
{
    ...
    rv = PM4x25_IS_BYPASSED_SET(unit, pm_info, is_bypassed);
    _SOC_IF_ERR_EXIT(rv);
...
int pm4x25_port_interface_config_set(int unit, int port, pm_info_t pm_info, const portmod_port_interface_config_t* config)
{
    ...
    if(!(PM_4x25_INFO(pm_info)->in_pm12x10)){
        ...
        _SOC_IF_ERR_EXIT(portmod_intf_to_phymod_intf(unit, config->interface, &phy_config.interface_type));
    ...
    switch(loopback_type){
        ...
        case portmodLoopbackPhyRloopPCS: /*slide*/
            if(PM_4x25_INFO(pm_info)->nof_phys == 0) {
                ...
                params.sys_side = PORTMOD_SIDE_LINE;
                _SOC_IF_ERR_EXIT(portmod_port_phy_lane_access_get(unit, port, &params, 1, &access, &nof_phys, NULL));
...
int pm4x25_port_lag_failover_status_toggle (int unit, soc_port_t port, pm_info_t pm_info)
{
    ...
    _SOC_IF_ERR_EXIT(READ_CLPORT_LAG_FAILOVER_CONFIGr(unit, port, &rval));
    soc_reg_field_set(unit, CLPORT_LAG_FAILOVER_CONFIGr, &rval, LINK_STATUS_UPf, 1);
    _SOC_IF_ERR_EXIT(WRITE_CLPORT_LAG_FAILOVER_CONFIGr(unit, port, rval));
                     ...
                     pm_info_t pm_info, portmod_port_mode_info_t *mode)
{
    ...
    return (_clport_mode_get(unit, port, port_index, &mode->cur_mode, &mode->lanes));
}
...
{
    return (clmac_drain_cell_get(unit, port,
...
{
    return (clmac_drain_cell_stop (unit, port, 
               drain_cells->rx_pfc_en,
               drain_cells->llfc_en,
               drain_cells->rx_pause,
               drain_cells->tx_pause));
...
{
    return (clmac_soft_reset_set(unit, port, val));
}
