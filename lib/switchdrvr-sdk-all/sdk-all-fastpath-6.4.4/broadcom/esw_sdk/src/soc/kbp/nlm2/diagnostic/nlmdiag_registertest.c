                                       ...
                                       )
{
    ...
    for(devNum = 0; devNum < moduleInfo_p->m_numCascadeDev; devNum++)
    {
        ...
        {
            if( devConfigRegReadInfo.m_dbParityErrEntryInvalidate == NLMDEV_ENABLE &&
                devConfigRegReadInfo.m_dbSoftErrProtectMode == NLMDEV_DISABLE && 
                devConfigRegReadInfo.m_eccScanType == NLMDEV_DISABLE &&
                devConfigRegReadInfo.m_lowPowerModeEnable == NLMDEV_DISABLE &&
                devConfigRegReadInfo.m_rangeEngineEnable == NLMDEV_DISABLE &&
                devConfigRegReadInfo.m_softErrorScanEnable == NLMDEV_DISABLE )
            {
        ...
        { 
            ...
            {
                if( blkConfigRegReadInfo.m_blockWidth != NLMDEV_BLK_WIDTH_80 &&
                    blkConfigRegReadInfo.m_blockEnable != NLMDEV_DISABLE)
                {                    
    ...
    switch(globalRegRdInfo_p->m_globalRegType)
    {
        ...
        case NLMDEV_RESULT0_REG:
            ...
            if( (devResultReg0ReadInfo.m_hitAddress[0] != 0x0) ||
                ...
                (devResultReg0ReadInfo.m_hitOrMiss[0] != NLMDEV_MISS) ||
                (devResultReg0ReadInfo.m_hitOrMiss[1] != NLMDEV_MISS))
            {
                NlmCmFile__fprintf(logFile, "\n\t => Compare of Read Data Fails :");  
