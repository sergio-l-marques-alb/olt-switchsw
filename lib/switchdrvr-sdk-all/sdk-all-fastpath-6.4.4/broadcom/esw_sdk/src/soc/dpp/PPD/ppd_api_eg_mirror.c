  ...
  )
{
  ...
  SOC_PPD_ARAD_TMP_DEVICE_CALL(eg_mirror_port_dflt_set,(unit, core_id, local_port_ndx,  config));

  SOC_PPD_DO_NOTHING_AND_EXIT;
exit:
  SOC_PPD_FUNC_PRINT(soc_ppd_eg_mirror_port_dflt_set_print,(unit,local_port_ndx,NULL));
  SOC_SAND_EXIT_AND_SEND_ERROR("error in soc_ppd_eg_mirror_port_dflt_set()", local_port_ndx, 0);
}
  ...
  )
{
  ...
  SOC_PPD_ARAD_ONLY_DEVICE_CALL(eg_mirror_port_appl_set,(unit, core_id, local_port_ndx, enable));
