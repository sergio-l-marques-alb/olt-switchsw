...
#define ARAD_PP_EG_FILTER_PVLAN_PORT_TYPE_MAX                    (ARAD_PP_NOF_EG_FILTER_PVLAN_PORT_TYPES-1)
#define ARAD_PP_EG_FILTER_ORIENTATION_MAX                        (SOC_SAND_PP_NOF_HUB_SPOKE_ORIENTATIONS-1)
#define ARAD_PP_EG_FILTER_FILTER_MASK_MAX                        (ARAD_PP_EG_FILTER_PORT_ENABLE_ALL)
    ...
    Arad_pp_procedure_desc_element_eg_filter[] =
{
   ...
   */
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_EG_FILTER_PORT_INFO_SET),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_EG_FILTER_PORT_INFO_SET_PRINT),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_EG_FILTER_PORT_INFO_SET_UNSAFE),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_EG_FILTER_PORT_INFO_SET_VERIFY),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_EG_FILTER_PORT_INFO_GET),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_EG_FILTER_PORT_INFO_GET_PRINT),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_EG_FILTER_PORT_INFO_GET_VERIFY),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_EG_FILTER_PORT_INFO_GET_UNSAFE),
  ...
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_EG_FILTER_PVLAN_PORT_TYPE_SET),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_EG_FILTER_PVLAN_PORT_TYPE_SET_PRINT),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_EG_FILTER_PVLAN_PORT_TYPE_SET_UNSAFE),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_EG_FILTER_PVLAN_PORT_TYPE_SET_VERIFY),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_EG_FILTER_PVLAN_PORT_TYPE_GET),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_EG_FILTER_PVLAN_PORT_TYPE_GET_PRINT),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_EG_FILTER_PVLAN_PORT_TYPE_GET_VERIFY),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_EG_FILTER_PVLAN_PORT_TYPE_GET_UNSAFE),
    ...
    Arad_pp_error_desc_element_eg_filter[] =
{
  ...
  },
  {
    ...
    "The range is: 0 - ARAD_PP_NOF_EG_FILTER_PVLAN_PORT_TYPES-1.\n\r ",
    SOC_SAND_SVR_ERR,
    FALSE
  },
  {
...
STATIC uint32
  arad_pp_eg_filter_pvlan_type_to_hw_val(
    SOC_SAND_IN ARAD_PP_EG_FILTER_PVLAN_PORT_TYPE pvlan_type
  )
{
  switch(pvlan_type)
  {
  case ARAD_PP_EG_FILTER_PVLAN_PORT_TYPE_ISOLATED:
    return 1;
  case ARAD_PP_EG_FILTER_PVLAN_PORT_TYPE_COMMUNITY:
    return 3;
  default:
  case ARAD_PP_EG_FILTER_PVLAN_PORT_TYPE_PROMISCUOUS:
    return 0;
  }
}
  ...
  )
{
  ...
  if (SOC_IS_JERICHO(unit)) {
	  ...
	  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  500,  exit, ARAD_REG_ACCESS_ERR, WRITE_EPNI_CFG_STP_STATE_TRAPr(unit, SOC_CORE_ALL, data32));

	  /* EPNI_CFG_ACC_FRAME_TYPE_TRAPr */
	  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  15,  exit, ARAD_REG_ACCESS_ERR, READ_EPNI_CFG_ACC_FRAME_TYPE_TRAPr(unit, SOC_CORE_ALL, &data32));
	  ...
	  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  16,  exit, ARAD_REG_ACCESS_ERR, WRITE_EPNI_CFG_ACC_FRAME_TYPE_TRAPr(unit, SOC_CORE_ALL, data32));


	  /* EPNI_CFG_MTU_TRAPr */
	  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  17,  exit, ARAD_REG_ACCESS_ERR, READ_EPNI_CFG_MTU_TRAPr(unit, SOC_CORE_ALL, &data32));
	  ...
	  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  18,  exit, ARAD_REG_ACCESS_ERR, WRITE_EPNI_CFG_MTU_TRAPr(unit, SOC_CORE_ALL, data32));

	  /* EPNI_CFG_PROTECTION_PATH_TRAPr */
	  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  17,  exit, ARAD_REG_ACCESS_ERR, READ_EPNI_CFG_PROTECTION_PATH_TRAPr(unit, SOC_CORE_ALL, &data32));
	  ...
	  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  18,  exit, ARAD_REG_ACCESS_ERR, WRITE_EPNI_CFG_PROTECTION_PATH_TRAPr(unit, SOC_CORE_ALL, data32));

	  /* EPNI_CFG_SPLIT_HORIZON_TRAPr */
	  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  19,  exit, ARAD_REG_ACCESS_ERR, READ_EPNI_CFG_SPLIT_HORIZON_TRAPr(unit, SOC_CORE_ALL, &data32));
	  ...
	  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  503,  exit, ARAD_REG_ACCESS_ERR, WRITE_EPNI_CFG_SPLIT_HORIZON_TRAPr(unit, SOC_CORE_ALL, data32));

	  /* EPNI_CFG_GLEM_TRAPr */
          /* For now disabled in Jericho */
	  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  20,  exit, ARAD_REG_ACCESS_ERR, READ_EPNI_CFG_GLEM_TRAPr(unit, SOC_CORE_ALL, &data32));
	  ...
	  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  21,  exit, ARAD_REG_ACCESS_ERR, WRITE_EPNI_CFG_GLEM_TRAPr(unit, SOC_CORE_ALL, data32));
  ...
  )
{
  ...
  {
  case 1:
    return ARAD_PP_EG_FILTER_PVLAN_PORT_TYPE_ISOLATED;
  case 3:
    return ARAD_PP_EG_FILTER_PVLAN_PORT_TYPE_COMMUNITY;
  default:
  case 0:
    return ARAD_PP_EG_FILTER_PVLAN_PORT_TYPE_PROMISCUOUS;
  }
}
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_PP_EG_FILTER_PORT_INFO_SET_UNSAFE);
   ...
   */
  egq_pp_ppct_tbl.disable_filtering = (filters == ARAD_PP_EG_FILTER_PORT_ENABLE_NONE)?TRUE:FALSE;
  ...
  epni_pp_pct_tbl.egress_vsi_filter_enable = 
    ARAD_PP_MASK_IS_ON(filters, ARAD_PP_EG_FILTER_PORT_ENABLE_VSI_MEMBERSHIP);

  egq_pp_ppct_tbl.enable_same_interface_filters_ports = 
    ARAD_PP_MASK_IS_ON(filters, ARAD_PP_EG_FILTER_PORT_ENABLE_SAME_INTERFACE);

  egq_pp_ppct_tbl.enable_unicast_same_interface_filter = 
    ARAD_PP_MASK_IS_ON(filters, ARAD_PP_EG_FILTER_PORT_ENABLE_SAME_INTERFACE);

  egq_pp_ppct_tbl.unknown_uc_da_action_filter =
    ARAD_PP_MASK_IS_ON(filters, ARAD_PP_EG_FILTER_PORT_ENABLE_UC_UNKNOW_DA);

  egq_pp_ppct_tbl.unknown_mc_da_action_filter =
    ARAD_PP_MASK_IS_ON(filters, ARAD_PP_EG_FILTER_PORT_ENABLE_MC_UNKNOW_DA);

  egq_pp_ppct_tbl.unknown_bc_da_action_filter =
    ARAD_PP_MASK_IS_ON(filters, ARAD_PP_EG_FILTER_PORT_ENABLE_BC_UNKNOW_DA);

  epni_pp_pct_tbl.egress_stp_filter_enable =
    ARAD_PP_MASK_IS_ON(filters, ARAD_PP_EG_FILTER_PORT_ENABLE_STP);

  /*
  epni_pp_pct_tbl.acceptable_frame_type_profile = 
    ARAD_PP_MASK_IS_ON(filters, ARAD_PP_EG_FILTER_PORT_ENABLE_PEP_ACCEPTABLE_FRM_TYPES); */
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_PP_EG_FILTER_PORT_INFO_SET_VERIFY);

  SOC_SAND_ERR_IF_ABOVE_MAX(out_port_ndx, ARAD_PP_PORT_MAX, ARAD_PP_PORT_OUT_OF_RANGE_ERR, 10, exit);
  ARAD_PP_STRUCT_VERIFY(ARAD_PP_EG_FILTER_PORT_INFO, port_info, 20, exit);
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_PP_EG_FILTER_PORT_INFO_GET_VERIFY);
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_PP_EG_FILTER_PORT_INFO_GET_UNSAFE);

  SOC_SAND_CHECK_NULL_INPUT(port_info);

  ARAD_PP_EG_FILTER_PORT_INFO_clear(port_info);
  ...
  {
    port_info->filter_mask = ARAD_PP_EG_FILTER_PORT_ENABLE_NONE;
    goto exit;
  }
  if (epni_pp_pct_tbl.egress_vsi_filter_enable)
  {
    filters |= ARAD_PP_EG_FILTER_PORT_ENABLE_VSI_MEMBERSHIP;
  }

  if (egq_pp_ppct_tbl.enable_same_interface_filters_ports)
  {
    filters |= ARAD_PP_EG_FILTER_PORT_ENABLE_SAME_INTERFACE;
  }
  if (egq_pp_ppct_tbl.unknown_uc_da_action_filter)
  {
    filters |= ARAD_PP_EG_FILTER_PORT_ENABLE_UC_UNKNOW_DA;
  }

  if (egq_pp_ppct_tbl.unknown_mc_da_action_filter)
  {
    filters |= ARAD_PP_EG_FILTER_PORT_ENABLE_MC_UNKNOW_DA;
  }

  if (egq_pp_ppct_tbl.unknown_bc_da_action_filter)
  {
    filters |= ARAD_PP_EG_FILTER_PORT_ENABLE_BC_UNKNOW_DA;
  }
  if (epni_pp_pct_tbl.egress_stp_filter_enable)
  {
    filters |= ARAD_PP_EG_FILTER_PORT_ENABLE_STP;
  }
  if (epni_pp_pct_tbl.acceptable_frame_type_profile)
  {
    filters |= ARAD_PP_EG_FILTER_PORT_ENABLE_PEP_ACCEPTABLE_FRM_TYPES;
  }
  /* as it always enabled */
  filters |= ARAD_PP_EG_FILTER_PORT_ENABLE_SPLIT_HORIZON;
  filters |= ARAD_PP_EG_FILTER_PORT_ENABLE_MTU;
  ...
  )
{
  ...
  SOC_SAND_ERR_IF_ABOVE_MAX(out_port_ndx, ARAD_PP_PORT_MAX, ARAD_PP_PORT_OUT_OF_RANGE_ERR, 20, exit);
  ...
  )
{
  ...
  sal_memset(ports, 0, ARAD_PP_VLAN_MEMBERSHIP_BITMAP_SIZE * sizeof(uint32));
  ...
  )
{
  ...
  ARAD_PP_STRUCT_VERIFY(ARAD_PP_LLP_PARSE_INFO, eg_prsr_out_key, 20, exit);
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_PP_EG_FILTER_PVLAN_PORT_TYPE_SET_UNSAFE);
  ...
  */
  ...
  {
    ARAD_ALLOC(in_info, ARAD_PORTS_LAG_INFO, 1, "arad_pp_eg_filter_pvlan_port_type_set_unsafe.in_info");
    ARAD_ALLOC(out_info, ARAD_PORTS_LAG_INFO, 1, "arad_pp_eg_filter_pvlan_port_type_set_unsafe.out_info");
    ...
    for (lag_info_idx = 0; lag_info_idx < in_info->nof_entries; lag_info_idx++)
    {
      res = arad_ports_logical_sys_id_build(
        ...
        TRUE,
        src_sys_port_ndx->sys_id,
        in_info->lag_member_sys_ports[lag_info_idx].member_id,
        0,
        &phy_sys_port_ndx);
      ...
      res = arad_sys_phys_to_local_port_map_get_unsafe(
              unit,
              in_info->lag_member_sys_ports[lag_info_idx].sys_port,
              &fap_id,
              &local_port
            );
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_PP_EG_FILTER_PVLAN_PORT_TYPE_SET_VERIFY);

  res = arad_pp_SAND_PP_SYS_PORT_ID_verify(src_sys_port_ndx);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);

  SOC_SAND_ERR_IF_ABOVE_MAX(pvlan_port_type, ARAD_PP_EG_FILTER_PVLAN_PORT_TYPE_MAX, ARAD_PP_EG_FILTER_PVLAN_PORT_TYPE_OUT_OF_RANGE_ERR, 20, exit);
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_PP_EG_FILTER_PVLAN_PORT_TYPE_GET_VERIFY);
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_PP_EG_FILTER_PVLAN_PORT_TYPE_GET_UNSAFE);
  ...
  )
{
  ...
  SOC_SAND_ERR_IF_ABOVE_NOF(lif_eep_ndx, SOC_DPP_DEFS_GET(unit, nof_out_lifs), ARAD_PP_LIF_ID_OUT_OF_RANGE_ERR, 10, exit);
  ...
  )
{
  ...
  if (info->filter_mask != ARAD_PP_EG_FILTER_PORT_ENABLE_ALL)
  {
    SOC_SAND_ERR_IF_ABOVE_MAX(info->filter_mask, ARAD_PP_EG_FILTER_FILTER_MASK_MAX, ARAD_PP_EG_FILTER_FILTER_MASK_OUT_OF_RANGE_ERR, 10, exit);

    if ((info->filter_mask & ARAD_PP_EG_FILTER_PORT_ENABLE_SPLIT_HORIZON) == 0)
    {
      SOC_SAND_SET_ERROR_CODE(ARAD_PP_EG_FLTER_PORT_SPLIT_DISABLE_ERR, 20, exit);
    }
    if ((info->filter_mask & ARAD_PP_EG_FILTER_PORT_ENABLE_MTU) == 0)
    {
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in ARAD_PP_EG_FILTER_PORT_INFO_verify()",0,0);
}
