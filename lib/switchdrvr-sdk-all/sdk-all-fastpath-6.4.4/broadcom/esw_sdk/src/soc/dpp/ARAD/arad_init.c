/* $Id: arad_init.c,v 1.309 Broadcom SDK $
...
#define ARAD_INIT_CLAC_M_N_AND_DIFF(dram_freq, freq_diff, m, n, freq_diff_best, m_best, n_best)\
  /* The exact frequency is ARAD_MGMT_INIT_DDR_PLL_FREF_EFF*n/m */\
...
typedef enum
{
  ...
  /*
   * number of arad init serdes types  
   */
...
soc_reg_t ardon_interrupts_mask_registers[] = {
    ...
    DRCA_ECC_INTERRUPT_REGISTER_MASKr,
    DRCB_INTERRUPT_MASK_REGISTERr,
    DRCB_ECC_INTERRUPT_REGISTER_MASKr,
    DRCBROADCAST_INTERRUPT_MASK_REGISTERr, /* access error */
    DRCBROADCAST_ECC_INTERRUPT_REGISTER_MASKr, /* access error */
    DRCC_INTERRUPT_MASK_REGISTERr,
    DRCC_ECC_INTERRUPT_REGISTER_MASKr,
    DRCD_INTERRUPT_MASK_REGISTERr,
    DRCD_ECC_INTERRUPT_REGISTER_MASKr,
...
soc_reg_t ardon_interrupt_monitor_mem_reg[] = {
    ...
    DRCBROADCAST_PAR_ERR_MEM_MASKr,
    EGQ_ECC_ERR_1B_MONITOR_MEM_MASKr,
    EGQ_ECC_ERR_2B_MONITOR_MEM_MASKr,
    EGQ_PAR_ERR_MEM_MASKr,
    EPNI_ECC_ERR_1B_MONITOR_MEM_MASKr,
    ...
    SOC_SAND_OUT uint32                     *pdm_nof_entries)
{
    ...
    if(pdm_mode == ARAD_INIT_PDM_MODE_SIMPLE) {
        *pdm_nof_entries = SOC_DPP_DEFS_GET(unit, pdm_size); /* For Arad: 1.5M */
    } else { /* PDM mode reduced */
        *pdm_nof_entries = SOC_DPP_DEFS_GET(unit, pdm_size)*3/4; /* For Arad: 0.75*1.5M */
    }
  ...
  )
{
  ...
  if (SOC_DPP_CONFIG(unit)->arad->init.dram.fmc_dbuff_mode == ARAD_INIT_FMC_64_REP_128K_DBUFF_MODE) {
      ...
      /* ARAD_INIT_FMC_4K_REP_64K_DBUFF_MODE */
      fmc_size = 64*1024;
  }
   ...
   *  ARAD_INIT_DRAM_BUFF_TO_FBC_DELTA_MIN between regular
   *  and FBC buffers.
   */
    ...
    SOC_SAND_OUT ARAD_INIT_DBUFFS_BDRY       *dbuffs)
{
    ...
    /* Number of ocb dbuff when 0 Drams = min(MAX Number of OCB buff in memory according to OCB buffer size, max OCB buff in cache) */
    ...
    SOC_SAND_OUT ARAD_INIT_DBUFFS_BDRY       *dbuffs)
{
    ...
    /* Number of ocb dbuff calculated according to ocb dbuff size: If 16 * (1024 * 1024) bits size -> 2MB. if buff size = 1k we have 2k OCB buffers. */
    ...
    SOC_SAND_IN  uint32           synt_dram_freq)
{
    ...
    /* The basic equations is:
    * Fref_eff =  Fref * (1 / Pdiv)
    * Fvco =  Fref_eff * Ndiv
    * Dram_freq = Fref * (1 / Pdiv) * Ndiv * (1 / Mdiv)
    */

    /* set ECI DDR PLL Config register
    * ddr_config[2:0]     = 0;
    * ddr_config[12:3]    = n_best;
    * ddr_config[20:13]   = m_best;
    * ddr_config[26:21]   = 0;
    * ddr_config[30:27]   = kp;
    * ddr_config[33:31]   = ki;
    * ddr_config[36:34]   = 2; 
    * ddr_config[42:37]   = 0;    
    * ddr_config[45:43]   = p;
    * ddr_config[74:46]   = 0x08000000;
    * ddr_config[82:75]   = 0;
    * ddr_config[98:83]   = 0;
    * ddr_config[120:99]  = 0;
    * ddr_config[121]     = 0;
    * ddr_config[133:122] = 0;
    * ddr_config[134]     = 0;
    * Ddr0PllConfig_Resetb [135] = 0 
    * PostResetb [136]           = 0 
    * Pwrdwn [137]               = 0 
    * I8PhaseEn [138]            = 0 
    * NdivFrac [158:139]         = 0
    */  
    ...
    /* Calculating the best m and n */
    /* We dram frequency as near as posible to the parameter dram_freq */
    ...
    /* KA parameter ARAD_MGMT_INIT_DDR_PLL_FREF_EFF is 25, and henece it alwayas 2 */
    fld_val = 2;
    ...
    SOC_SAND_IN  ARAD_MGMT_INIT           *init)
{
    ...
    for(ddr_id = 0; ddr_id < SOC_DPP_DEFS_GET(unit, hw_dram_interfaces_max)/2; ++ddr_id) {
        if(init->dram.is_valid[2*ddr_id] || init->dram.is_valid[2*ddr_id+1]) {
            ...
            /* Clear DDR PLL Config/status register */
            SOC_REG_ABOVE_64_CLEAR(field_above_64);
            SOC_REG_ABOVE_64_CLEAR(reg_above_64);
  ...
  )
{
  ...
  if ((hw_adj->dram.enable == TRUE) ||  (hw_adj->ocb.ocb_enable != OCB_DISABLED )){
   ...
   /* IDR_CONTEXT_MRU table init
    ...
    if (SOC_IS_ARADPLUS(unit)) {
        ...
        /* 
         * When MMC buffers run out, UC buffers are used (if allowed  by configuration). 
         * If this happens in the middle of the packet, packets are lost. 
         * configurable threshold for buffer changes from MMC to UC when MMC occupied reaches threshold.
         * set: 
         * MmcDbuffOccTh threshold = MMC-DBuff-size - ((16K/DRAM-Buffer-size) * #Ports) + 50 
         * MmcDbOccCntOverTh=0x1 (just after setting IDR.EnableBufferChange) 
         *  
         * The DRAM-Buffer-size is actually the DRAM-Buffer-size if enabled, otherwise the OCB-Buffer-size         
        */ 

        /*
         * DbuffsToReserve = minimulticat_buffer_size - (dram.enable ?
         *            (MaxPacketSize / dram_dbuff_size) * (#FAP_PORT) + 50: 
         *            (MaxPacketSize / ocb_databuffer_size) * (#FAP_PORT) + 50);
         *  Where MaxPacketSize is 16000
         *  MmcDbuffOccTh = Min(TotalNofMmcBuffer buffsToReserve, TotalNofMmcBuffer*90%)
         * 
         */
...
#endif /*BCM_88660_A0*/
    val32 = soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "8865002", 0); /* Get override for egress multicast workwround with a given number (1,2) of replicators */
    SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  60,  exit, ARAD_REG_ACCESS_ERR,READ_EGQ_EGRESS_REPLICATION_GENERAL_CONFIGr(unit, SOC_CORE_ALL, &fld_val));
    /* configure replicators used for high and low priority egress multicast */
    if (val32 > 2 || (val32 <= 0 && SOC_IS_ARADPLUS(unit))) { /* now workaround for hardware bugs */
    ...
    } else if (val32 == 2) { /* use two replicators */
    ...
    } else { /* work around using one replicator */
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  100,  exit, ARAD_REG_ACCESS_ERR,WRITE_EGQ_EGRESS_REPLICATION_GENERAL_CONFIGr(unit, SOC_CORE_ALL,  fld_val));

#ifdef BCM_88660_A0
    if (SOC_IS_ARADPLUS(unit)) {
        /* set the Arad+ multicast modes in hardware */
        ...
        if (!SOC_IS_ARDON(unit)) {
            ...
            soc_reg_field_set(unit, EGQ_CFG_BUG_FIX_CHICKEN_BITS_REG_2r, &val32, CFG_CUD_MAPPING_ENABLEf,
              (SOC_DPP_CONFIG(unit)->tm.mc_mode & DPP_MC_EGR_17B_CUDS_127_PORTS_MODE) ? 1 : 0);/*Enable 16 bit to 17 bit CUD extension in the EGQ*/
            ...
            soc_reg_field_set(unit, EPNI_CFG_BUG_FIX_CHICKEN_BITS_REG_2r, &val32, CFG_CUD_MAPPING_ENABLEf,
              (SOC_DPP_CONFIG(unit)->tm.mc_mode & DPP_MC_CUD_EXTENSION_MODE) ? 1 : 0); /*enable egress CUD mapping*/
            SOC_SAND_SOC_IF_ERROR_RETURN(res, 120, exit, WRITE_EPNI_CFG_BUG_FIX_CHICKEN_BITS_REG_2r(unit, REG_PORT_ANY, val32)); 
        }
        /*If 17 bit to 21 bit CUD mapping is enabled then set default CUD to intself*/
        if ((SOC_DPP_CONFIG(unit)->tm.mc_mode) & DPP_MC_CUD_EXTENSION_MODE) { 
            SOC_SAND_CHECK_FUNC_RESULT(arad_pp_eg_encap_data_local_to_global_cud_init(unit), 221, exit);
        }
    }
#endif /* BCM_88660_A0 */
    /*
    ...
    /*
     * Configure IDR Ocb Buffer Type
     */
    if (hw_adj->ocb.ocb_enable != OCB_DISABLED) { 
        
        /* 
         * Configure OCB Buffer for:
         * 1) Fwd = FMC or UC                    + Q-eligible  + No mirror                              , set UC OCB Buff
         * 2) Fwd = FMC or UC                    + Q-eligible  + With mirror & bcm886xx_mirror_use_ocb=1, set MC OCB Buff (not looking on mc-in-range although  using MC buff)
         * 3) Fwd = Ing-MC with 1 replication    + MC in range + No mirror                              , set UC OCB Buff
         * 4) Fwd = Ing-MC with 1 replication    + MC in range + With mirror & bcm886xx_mirror_use_ocb=1, set MC OCB Buff (2 copies: mc + mirror ==> minimulticast)
         * 5) Fwd = Ing-MC with n>1 replications + MC in range + Any mirror  & bcm886xx_mirror_use_ocb=1, set MC OCB Buff 
         *
         * Table Entry:
         * {fwd, snoop, mirror, queue-eligibility, mcid-in-range}
         ...
         * mcid-in-range (1 bits) is set if the mcid is in one of the ocb ranges
         *
         * Table value:
         * 0 - don't use ocb buffer
         * 1 - use uc ocb buffer
         * 2 - use flmc ocb buffer
         *
         * Remarks:
         * Ing-MC 1 - Uses for Fabric multicast (use UC OCB buf)
         * Ocb dont have minimulticast - Use FullMulticast 
         * snoop not using OCB
         */
        ...
        /* Configure IQM OCBPRM table with ocb defaults */
        ...
        /* 
         * Configure OCB Dynamic talbe defaults 
         */
...
#ifdef BCM_88660_A0
        if (SOC_IS_ARADPLUS(unit)) {
            /* Default value when working without One-way-bypass */
            fld_val = 0x1;
            /* 
             * In ARADPLUS, this field reset value is 0x0, but we are setting it anyway just to make sure. 
             * In 88650 the reset value is 0x1, and we are not changing it
             */
            ...
            if ((hw_adj->ocb.ocb_enable == OCB_ONLY) || (hw_adj->ocb.ocb_enable == OCB_ONLY_1_DRAM) || (hw_adj->ocb.ocb_enable == OCB_DRAM_SEPARATE)){
                ...
                /* Inverse value from one-way-bypass */
                fld_val = 0x0;
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 420, exit, READ_IQM_IQM_ENABLERSr(unit, REG_PORT_ANY, &val32)); /* read IQM enables */
    /*
     * IQM - VSQ CD Enable
     */
    ...
    if (hw_adj->dram.fmc_dbuff_mode == ARAD_INIT_FMC_64_REP_128K_DBUFF_MODE) {
        /* 64 replication --> iqm_flull_user_count_memory */
        fld_val = 0x0;
    } else {
        /* ARAD_INIT_FMC_4K_REP_64K_DBUFF_MODE */
        fld_val = 0x1;
    ...
    /* set action type source from queue signature or from packet header */
    soc_reg_field_set(unit, IQM_IQM_ENABLERSr, &val32, FWD_ACT_SELf, 
      IS_ACTION_TYPE_FROM_QUEUE_SIGNATURE(unit) ? 1 : 0);

    /* select if hardware guarantees BDs or DBs for queues */
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 425, exit, WRITE_IQM_IQM_ENABLERSr(unit, REG_PORT_ANY, val32)); /* write IQM enables */
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 430, exit, WRITE_IQM_ECN_CONFIGURATIONr(unit, 255)); /* Set ECN capable packets not not be dropped by any RED type when ECN is enabled */
...
#ifdef BCM_88660_A0 
    /*
     * Set configuration for double lb-key range table feature
     */
    ...
    if (SOC_IS_ARADPLUS(unit)) { 
        ...
        if(hw_adj->fabric.ftmh_lb_ext_mode == ARAD_MGMT_FTMH_LB_EXT_MODE_STANDBY_MC_LB) {
            ...
            override_msb = 0x1;
            /* Set SwapLbKeyMsbsEnable */
            reg = SOC_IS_JERICHO(unit)? IHB_LBP_GENERAL_CONFIG_0r: IHB_REG_0092r;
            field = SOC_IS_JERICHO(unit)? SWAP_LB_KEY_MSBS_ENABLEf: ITEM_3_3f;
            SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  1800,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, reg, SOC_CORE_ALL, 0, field,  1));
        } 
        /*     
        * If set to 1 then each OTM-Port has single Load-Balance range. Otherwise, each OTM-Port has two ranges 
        */
        ...
        if (!SOC_IS_ARDON(unit)) { /* The following fields are not present in Ardon */
            /*
            * if set to 1 then the msb bit of the FTMH.LbKey will be overriden by the next register
            */
            ...
            /*
            * this register holds the value to be used when overriding the FTMH.LBKey
            */    
            if (override_msb == 0x1) {
        ...
        /*
         * Configurations for Hash-key manipulation in ARAD+ 
         */
        ...
        if(hw_adj->fabric.trunk_hash_format == ARAD_MGMT_TRUNK_HASH_FORMAT_INVERTED){
           ...
           /* set DuplicateLagLbKeyMsbsToLsbs to 1 */
            ...
            /* set LbKeyExtUseMsbBits to 1 */
            /*SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  225,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, IHB_LBP_GENERAL_CONFIG_0r, SOC_CORE_ALL, 0, LB_KEY_EXT_USE_MSB_BITSf,  1));*/
        }
    }
  ...
  /* Do soft init to IQM block after setting Boundries and VSQ CD Enable bit */
  ...
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 432, exit, WRITE_ECI_BLOCKS_SOFT_INITr(unit, wr_mask_val));
...
#ifdef BCM_88660_A0
    /* Enable Arad Plus Bug fixes/Features */
    if (SOC_IS_ARADPLUS(unit)) {
     ...
     */
    if (SOC_IS_ARDON(unit)) {
        COMPILER_64_SET(val64, 0x0, 0x0);
        SOC_SAND_SOC_IF_ERROR_RETURN(res, 450, exit, WRITE_ECI_PVT_MON_A_CONTROL_REGr(unit, val64));
        sal_usleep(20000);
        COMPILER_64_SET(val64, 0x2, 0x0);
        SOC_SAND_SOC_IF_ERROR_RETURN(res, 470, exit, WRITE_ECI_PVT_MON_A_CONTROL_REGr(unit, val64));
 ...
 */
STATIC uint32 arad_mgmt_ipt_init(SOC_SAND_IN int unit);
  ...
  )
{
  ...
  /* FC should be enabled after ING TM FC thresholds are configures (in arad_itm_init) */
  ...
  )
{
   ...
   SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_MGMT_HW_SET_DEFAULTS);
    ...
    if (!SOC_WARM_BOOT(unit)) { 
        ...
        reg32_val = 0;
        /*clear the IHB_TCAM_PROTECTION_ERROR bit after the TCAM banks are cleared*/
   ...
   )
{
  ...
  for (addr_idx = 0; addr_idx < ARAD_INIT_HM_NOF_ADDRS; addr_idx++) {
    ...
    /* SHR_BITCOPY_RANGE(hmif_pph_learn_ext_asd_type_bits,           
                        0, addr_idx, ARAD_INIT_HMI_LEARN_EXT_VALID_LSB, ARAD_INIT_HMI_LEARN_EXT_VALID_NOF_BITS); */ /* Unused */
    ...
    )
{
   ...
   */
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  10,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, IHP_SYNC_COUNTERr, SOC_CORE_ALL, 0, SYNC_COUNTERf,  0x3ff/* should be examined*/));
  }
  ...
  )
{
    ...
    if (!SOC_IS_ARDON(unit)) {
        ...
        SOC_SAND_SOC_IF_ERROR_RETURN(res, 22, exit, WRITE_PORT_MLD_CTRL_REGr(unit, SOC_BLOCK_PORT(unit, CLP_BLOCK(unit, 0)) /*###CLP0*/, reg32_val));

        /* PORT_MLD_CTRL_REG.CLP1 */
        reg32_val = 0;
        soc_reg_field_set(unit, PORT_MLD_CTRL_REGr, &reg32_val, RST_B_HWf, 1);
        soc_reg_field_set(unit, PORT_MLD_CTRL_REGr, &reg32_val, RST_B_MDIOREGSf, 1);
        SOC_SAND_SOC_IF_ERROR_RETURN(res, 24, exit, WRITE_PORT_MLD_CTRL_REGr(unit, SOC_BLOCK_PORT(unit, CLP_BLOCK(unit, 1))  /*###CLP1*/, reg32_val));

        /* initalizing gaps between ports info for led processor use */
        SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  26,  exit, ARAD_REG_ACCESS_ERR,WRITE_REG_2029700r(unit,  SOC_BLOCK_PORT(unit, CLP_BLOCK(unit, 0))/*###CLP0*/,  0x6));
        SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  27,  exit, ARAD_REG_ACCESS_ERR,WRITE_REG_2029700r(unit,  SOC_BLOCK_PORT(unit, CLP_BLOCK(unit, 1))/*###CLP1*/,  0x6));
        SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  28,  exit, ARAD_REG_ACCESS_ERR,WRITE_REG_2029700r(unit,  SOC_BLOCK_PORT(unit, XLP_BLOCK(unit, 0))/*###XLP0*/,  0x6));
        SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  29,  exit, ARAD_REG_ACCESS_ERR,WRITE_REG_2029700r(unit,  SOC_BLOCK_PORT(unit, XLP_BLOCK(unit, 1))/*###XLP1*/,  0x6));
...
STATIC uint32 arad_mgmt_stk_init(
    SOC_SAND_IN  int                                unit,
    SOC_SAND_IN  ARAD_MGMT_INIT               *init)
{
    uint32
    ...
    SOC_PBMP_ITER(pbmp,port_i) {
        ...
        res = soc_port_sw_db_peer_tm_domain_get(unit, port_i, &ppct_tbl_data.peer_tm_domain_id);
    ...
    for (i=0; i < 64 ; i++) {
        ...
        res = arad_ipq_stack_fec_map_stack_lag_set_unsafe(unit, i /*TM domain*/, ARAD_IPQ_STACK_LAG_STACK_FEC_RESOLVE_ENTRY_ALL, i /* LAG stack */);
        SOC_SAND_CHECK_FUNC_RESULT(res, 90, exit);
    }
    ...
    SOC_SAND_IN  ARAD_MGMT_INIT               *init)
{
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 10, exit, READ_IPS_IPS_GENERAL_CONFIGURATIONSr(unit, REG_PORT_ANY, &val32));
    soc_reg_field_set(unit, IPS_IPS_GENERAL_CONFIGURATIONSr, &val32, ENABLE_SYSTEM_REDf, 0x1);
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 20, exit, WRITE_IPS_IPS_GENERAL_CONFIGURATIONSr(unit, REG_PORT_ANY, val32));
    ...
    SOC_SAND_IN  int                                unit)
{
    ...
    /* Initiate the swap Enable state to be set for all PON ports */
    ...
    SOC_PBMP_ITER(pon_port_bm, port){
        ...
        /* SWAP Tunnel-ID (PON-Channel) from VLAN-tag to the start of the header */
        res = arad_ports_swap_set_unsafe(unit,
                                         port,
                                         &ports_swap_info);
        SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);

        /* Set header start to be 4 bytes after for each PP port */
        ...
        /* Set FLP PTC profile for PON port */    
	  	...
	  	if (SOC_DPP_CONFIG(unit)->pp.compression_spoof_ip6_enable || SOC_DPP_CONFIG(unit)->pp.compression_ip6_enable) {
    ...
    SOC_SAND_IN  int                                unit)
{
    ...
    /* Initiate the swap Enable state to be set for all COE ports */
    ports_swap_info.enable = TRUE;

    SOC_PBMP_ITER(coe_port_bm, port){
        /* SWAP port externder ID from VLAN-tag to the start of the header */
        res = arad_ports_swap_set_unsafe(unit,
                                         port,
                                         &ports_swap_info);
        SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
  ...
  )
{
  ...
  if (init->dram.pdm_mode != ARAD_INIT_PDM_MODE_SIMPLE /*soc_property_get(unit,spn_BCM886XX_PDM_MODE, 0)*/) {
  ...
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 220, exit, READ_IPS_IPS_GENERAL_CONFIGURATIONSr(unit, REG_PORT_ANY, &val32));
  ...
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 230, exit, WRITE_IPS_IPS_GENERAL_CONFIGURATIONSr(unit, REG_PORT_ANY, val32));
  ...
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 500, exit, READ_IPS_IPS_GENERAL_CONFIGURATIONSr(unit, REG_PORT_ANY, &val32));
  ...
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 510, exit, WRITE_IPS_IPS_GENERAL_CONFIGURATIONSr(unit, REG_PORT_ANY, val32));
  ...
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 750, exit, WRITE_IQM_INTERRUPT_REGISTERr(unit, REG_PORT_ANY, 0x7fffffff & ~intr_val)); /* 31 bits */
  ...
  )
{
...
#endif
  {
      ...
      if (ser_test_iters) { /* Perform SER testing if configured to do so */
          ...
          if (time_to_wait == 0) { /* if both sec and ms are specified, sec is used */
              ...
              if (time_to_wait == 0) {
                  time_to_wait = 3600 | DCMN_MBIST_TEST_LONG_WAIT_DELAY_IS_SEC; /* default of one hour if not specified */
              }
          } else {
  ...
  /************************************************************************/
    if (init->credit.credit_worth_enable)
    {
      ...
      res = arad_mgmt_credit_worth_set(
              unit,
  ...
  SOC_PBMP_ITER(pbmp,port_i) {
      ...
      if (!(SOC_PORT_IS_ELK_INTERFACE(flags) || SOC_PORT_IS_STAT_INTERFACE(flags))) {
          ...
          res = soc_port_sw_db_shaper_mode_get(unit, port_i, &shaper_mode);
  ...
  /* Swap info init configuration */
  res = arad_mgmt_swap_init(unit, &(init->ports.swap_info));
  SOC_SAND_CHECK_FUNC_RESULT(res, 180, exit);

  /* PON init configuration */
...
/*
 * Perform power savings on the chip, should be called in the end of the startup.
 * Currently only Arad is supported.
 */
int
arad_activate_power_savings(int unit)
{
    ...
    if (soc_property_suffix_num_get(unit, -1, spn_LOW_POWER, "nif_mac", 1)) {
        ...
        /* Iterate over NIF ports to get the NIF blocks and sub blocks which are used */
        ...
        PBMP_ITER(ports, port) {
            ...
            for (block_index = 0; ; ++block_index) {
                if (block_index >= ARAD_NOF_NIF_BLOCKS) {
                    ...
                    if (block_index < SOC_ARAD_NOF_INSTANCES_CLP) { /* This is a CLP block */
                        if (interface_type == SOC_PORT_IF_ILKN) {
                            /* no sub block is used */
                        } else if (interface_type == SOC_PORT_IF_CAUI) {
                            clp_sub_blocks_not_used[ARAD_NOF_CLP_SUB_BLOCKS * block_index + ARAD_NOF_XMACS_IN_CLP] = 0; /* Mark CMAC in use */
                        } else {
                            ...
                            clp_sub_blocks_not_used[ARAD_NOF_CLP_SUB_BLOCKS * block_index + xmac_index] = 0; /* Mark XMAC in use */
                        }
                    } else { /* This is a XLP block */
                    }
                    block_not_used[block_index] = 0;
                    break;
                }
            }/*soc_reg32_get soc_reg_addr_get*/
        }
#ifdef BCM_88660_A0
        if (SOC_IS_ARADPLUS(unit)) {
            uint8 
           ...
           /* 
            * In ARAD Plus
            * there is a speciale register in the NBI to disable clp/xlp blocks 
            *  
            * If a CLP/XLP has no ports defined for it, turn it off
            * Array index 0 or 1 based on wether this is instance of the block (C/XLP 0 or 1) 
            *  
            * To disable a block we set the value 2 
            * bit[1] : CDC (clock domain crossing) Option
            * 1b1 - Enable CDC - use for all ports less than 100G 
            * 1b0 - Disable CDC - - use for all ports 100G and above.
            * bit[0] : Enable       
            * 1b1 - Enable PORT - Hard enable for all ports.
            * 1b0 - Disable PORT - Hard disable for all ports.
            */
            val32 = 0;
...
#endif /*BCM_88660_A0*/
        /* power down unused XMACs and CMACs per CLP/XLP */
        for (block_index = 0; block_index < ARAD_NOF_NIF_BLOCKS; ++block_index) {
            port = SOC_BLOCK_PORT(unit, nif_blocks[block_index]); /* A port that will translate to the required block */
            val32 = 0;
            if (block_index < SOC_ARAD_NOF_INSTANCES_CLP) { /* This is a CLP block */
                /* When dynamic port configuration is enabled, turn off sub blocks only if all the block is turned off.
                   This is because A block for which no port is defined may not be dynamically configured */
    ...
    if (soc_property_suffix_num_get(unit, -1, spn_LOW_POWER, "fabric_mac", 1)) {
        /* handle power savings for fabric MAC */
        /* Get the quads not in use by any port */
        ...
        /* find the disabled fabric link quads */
        ...
        for (quad = 0; quad < ARAD_NOF_ARAD_FABRIC_QUADS(unit); ++quad) {
            ...
            if (quad_disabled) {
                ++nof_disabled_quad; /* increase nof disables quads, and mark the quad FMAC to be held in reset */
                soc_reg_above_64_field32_set(unit, ECI_BLOCKS_SOFT_RESETr, val_above64, one_quad_fmac_reset_fields[quad], 1);
            }
            if (quad % 3 == 2) { /* If this is the last quad in a three quad FMAC */
                if (nof_disabled_quad == nof_disabled_quad_before_this_3 + 3) {
                    if (!disabling_3_quads) {
                        disabling_3_quads = 1;
                        SOCDNX_IF_ERR_EXIT(READ_ECI_BLOCKS_POWER_DOWNr(unit, &val64)); /* power down the three quad FMAC */
                    }
        ...
        /* perform the register writes saving power for fabric MAC */
        if (nof_disabled_quad) {
            ...
            if (disabling_3_quads) {
                ...
                if (nof_disabled_quad == ARAD_NOF_ARAD_FABRIC_QUADS(unit)) { /* If all quads are disabled */
                    ...
                    SOCDNX_IF_ERR_EXIT(WRITE_ECI_SRD_0_PLL_CONFIGr(unit, val_above64)); /* Power down Fabric Serdes PLL */
                }
            }
        }
    ...
    if (soc_property_suffix_num_get(unit, -1, spn_LOW_POWER, "pll", 1)) {
        /* Power down TS PLL based on the phy_1588_dpll_frequency_lock soc property */
    ...
    if (soc_property_suffix_num_get(unit, -1, spn_LOW_POWER, "dram", 1)) {
        ...
        if (SOC_DPP_CONFIG(unit)->arad->init.dram.enable) { /* Find unused DRAM PLLs */
        ...
        /* power down PLLs for couples of non used external DRAMs */
    ...
    SOC_SAND_IN void *opaque)
{
  ...
  for (lag_size = 0; lag_size < 16; ++lag_size)
  {
    for (hash_indx = 0; hash_indx < 256 ; ++hash_indx)
    {
      ...
      res = arad_irr_smooth_division_tbl_set_unsafe(
              unit,
              lag_size,
              hash_indx,
  ...
  *value = 0;
  ...
  soc_mem_field32_set(unit, IRR_SMOOTH_DIVISIONm, value, MEMBER_0f, member);

  hash_index++;
  member = ((hash_index >> 4) | (hash_index << 4)) % (lag_size+1);  
  soc_mem_field32_set(unit, IRR_SMOOTH_DIVISIONm, value, MEMBER_1f, member);
  ...
  )
{
  ...
  soc_mem_field32_set(unit, IRR_DESTINATION_TABLEm, table_entry, TC_PROFILEf, 0); /* ARAD_IPQ_TC_PROFILE_DFLT is 0 */
  res = arad_fill_table_with_entry(unit, IRR_DESTINATION_TABLEm, MEM_BLOCK_ANY, table_entry); /* fill table with the entry */
  ...
  res = arad_fill_table_with_entry(unit, IRR_SNOOP_MIRROR_DEST_TABLEm, MEM_BLOCK_ANY, table_entry); /* fill table with the entry */
  SOC_SAND_CHECK_FUNC_RESULT(res, 121, exit);

  /*snoop mirror table 1*/
  table_entry[0] = table_entry[1] = 0;
  res = arad_fill_table_with_entry(unit, IRR_SNOOP_MIRROR_TABLE_1m, MEM_BLOCK_ANY, table_entry); /* fill table with the entry */
  ...
  )
{
  ...
  ARAD_IHB_FEM_MAP_TABLE_TBL_DATA
    ...
    uint32
        ...
        table_entry[128] = {0},
        fld_val = 0,
        fes_tbl_data[(SOC_SAND_MAX(99, 297) / 32) + 1] = {0};

  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_MGMT_IHB_TBLS_INIT);


  /* Set all to db profiles to invalid value 0x3e to disable the lookups
   * Note! Do not set value to all ones, since it causes an incorrect
   * key to TCAM (previous packet instead of current one).
   */
  pmf_profile_data = SOC_IS_JERICHO(unit)? 0x7FBEFBE :0xFBEFBE;
  ...
  /* Set the instruction back to NOP according to the encoding */
  fld_val = 0;
  for (table_line = 0; table_line < arad_pmf_fem_action_type_array_size_get_unsafe(unit); ++table_line) {
      if (SOC_PPC_FP_ACTION_TYPE_NOP == arad_pmf_fem_action_type_array_element_get_unsafe(unit, table_line, 0)) {
          soc_mem_field32_set(unit, IHB_PMF_FES_PROGRAMm, &fld_val, FES_ACTION_TYPEf, arad_pmf_fem_action_type_array_element_get_unsafe(unit, table_line, 1));
          break;
      }
  }
  ...
  res = arad_fill_table_with_entry(unit, IHB_PMF_FES_PROGRAMm, MEM_BLOCK_ANY, fes_tbl_data); /* fill memory with the entry in data */
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 40, exit);


  /* Init the TCAM - in Jericho, TCAM_CPU_CMD_WRf is enlarged to allow independent 80b entry configurations */
  if (!SOC_IS_ARDON(unit)) {
      ...
      if (SOC_IS_JERICHO(unit)) {
          /* Due to emulation constraints, init only banks 12-13 */
          res = arad_fill_partial_table_with_entry(unit, PPDB_A_TCAM_BANK_COMMANDm, 0, 0, MEM_BLOCK_ANY, 
                                                   2*1024*12 /* index_start */, 2*1024*14-1 /* index_end */, table_entry);
  ...
  )
{
  ...
  res = arad_fill_table_with_entry(unit, IQM_PQWQm, MEM_BLOCK_ANY, table_entry); /* fill table with the entry */
  SOC_SAND_CHECK_FUNC_RESULT(res, 110, exit);
  ...
  res = arad_fill_table_with_entry(unit, IQM_CNREDm, MEM_BLOCK_ANY, cnred_table_entry); /* fill table with the entry */
  ...
  )
{
  ...
  if (ARAD_IS_VOQ_MAPPING_INDIRECT(unit)) { /* indirect VOQ mapping mode */
    ...
    soc_mem_field32_set(unit, IPS_QPM_1_SYS_REDm, table_entry, SYS_PHY_PORTf, SOC_TMC_MAX_SYSTEM_PHYSICAL_PORT_ID); /* mark as disabled entry 0xfff */
    res = arad_fill_table_with_entry(unit, IPS_QPM_1_SYS_REDm, MEM_BLOCK_ANY, table_entry); /* fill table with the entry */
    SOC_SAND_CHECK_FUNC_RESULT(res, 50, exit);
    ...
    res = arad_fill_table_with_entry(unit, IPS_QPM_1_NO_SYS_REDm, MEM_BLOCK_ANY, table_entry); /* fill table with the entry */
    SOC_SAND_CHECK_FUNC_RESULT(res, 70, exit);
  ...
  res = arad_fill_table_with_entry(unit, IPS_QPM_2_NO_SYS_REDm, MEM_BLOCK_ANY, table_entry);
  SOC_SAND_CHECK_FUNC_RESULT(res, 80, exit);
  /*
...
{   
    ...
    for(i=0;i<ARAD_FBC_PRIORITY_NDX_NOF ; i++) {
        ...
        /*dp = (i & ARAD_FBC_PRIORITY_NDX_DP_MASK) >> ARAD_FBC_PRIORITY_NDX_DP_OFFSET;*/
        ...
        /*is_hp = (i & ARAD_FBC_PRIORITY_NDX_IS_HP_MASK) >> ARAD_FBC_PRIORITY_NDX_IS_HP_OFFSET;*/
...
{
  uint32 res = SOC_SAND_OK;
  uint32 table_entry[SOC_DPP_DEFS_MAX(EGQ_PPCT_NOF_LONGS)];
  ...
  res = arad_fill_table_with_entry(unit, EGQ_PPCTm, MEM_BLOCK_ANY, data); /* fill table with the entry */
  SOC_SAND_CHECK_FUNC_RESULT(res, 60, exit);

  SOC_REG_ABOVE_64_CLEAR(data);
  soc_mem_field32_set(unit, EGQ_PCTm, data, CGM_PORT_PROFILEf, ARAD_EGR_PORT_THRESH_TYPE_15);
  res = arad_fill_table_with_entry(unit, EGQ_PCTm, MEM_BLOCK_ANY, data); /* fill table with the entry */
  SOC_SAND_CHECK_FUNC_RESULT(res, 70, exit);
  ...
  )
{
  ...
  if (SOC_DPP_CONFIG(unit)->emulation_system == 0) {
      ...
      errnum = 12000; /* IDR */
      res = arad_fill_table_with_entry(unit, IDR_CONTEXT_MRUm, MEM_BLOCK_ANY, table_entry); /* Overwritten later */
      errnum += 10;
      SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
  ...
  if (SOC_DPP_CONFIG(unit)->emulation_system == 0) {
      ...
      SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
      if (!SOC_IS_ARDON(unit)) {
          res = arad_fill_table_with_entry(unit, IHB_PATH_SELECTm, MEM_BLOCK_ANY, table_entry);
          errnum += 10;
          SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
      }
      ...
      if (!SOC_IS_ARDON(unit)) {
          res = arad_fill_table_with_entry(unit, PPDB_A_TCAM_BANKm, MEM_BLOCK_ANY, table_entry);
          errnum += 10;
          SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
      }
  ...
  if (SOC_DPP_CONFIG(unit)->emulation_system == 0) {
      ...
      errnum += 10;
      SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
      res = arad_fill_table_with_entry(unit, (SOC_IS_JERICHO(unit) ? IPS_CRBALTHm : IPS_CRBALTHm), MEM_BLOCK_ANY, table_entry);
      errnum += 10;
      SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
      res = arad_fill_table_with_entry(unit, (SOC_IS_JERICHO(unit) ? IPS_CRWDTHm : IPS_CRWDTHm), MEM_BLOCK_ANY, table_entry);
      errnum += 10;
      SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
      res = arad_fill_table_with_entry(unit, (SOC_IS_JERICHO(unit) ? IPS_EMPTYQCRBALm : IPS_EMPTYQCRBALm), MEM_BLOCK_ANY, table_entry);
      errnum += 10;
      SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
      res = arad_fill_table_with_entry(unit, (SOC_IS_JERICHO(unit)? IPS_FLWIDm : IPS_FLWIDm), MEM_BLOCK_ANY, table_entry);
      errnum += 10;
      SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
  ...
  errnum += 10;
  SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
  res = arad_fill_table_with_entry(unit, (SOC_IS_JERICHO(unit) ? IPS_QSZTHm : IPS_QSZTHm), MEM_BLOCK_ANY, table_entry);
  errnum += 10;
  SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
  res = arad_fill_table_with_entry(unit, (SOC_IS_JERICHO(unit)? IPS_QTYPEm : IPS_QTYPEm), MEM_BLOCK_ANY, table_entry);
  errnum += 10;
  SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
