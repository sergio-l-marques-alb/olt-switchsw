    ...
    else
    {
      ...
      if (!ignore_key)
      {
        ...
        if (!found_isem || (found_isem && !found_lem)) {
          res = arad_sw_db_src_bind_spoof_id_ref_count_set(unit, sem_entry.sem_result_ndx, TRUE);
 ...
 *success = SOC_SAND_SUCCESS;
  ...
  if(found)
  {
    ...
    else
    {
      ...
      res = arad_sw_db_src_bind_spoof_id_ref_count_set(unit, sem_entry.sem_result_ndx, FALSE);
      SOC_SAND_CHECK_FUNC_RESULT(res, 32, exit);
      arad_sw_db_src_bind_spoof_id_ref_count_get(unit, sem_entry.sem_result_ndx, &ref_count);
      SOC_SAND_CHECK_FUNC_RESULT(res, 33, exit);
      /* last one */
      if (!ref_count) {
