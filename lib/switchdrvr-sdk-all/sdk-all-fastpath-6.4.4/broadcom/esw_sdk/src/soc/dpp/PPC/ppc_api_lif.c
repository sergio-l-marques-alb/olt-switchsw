/* $Id: ppc_api_lif.c,v 1.27 Broadcom SDK $
...
/******************************************************************
*
* FILENAME:       DuneDriver/ppc/src/soc_ppc_api_lif.c
*
* MODULE PREFIX:  DuneDriver/ppc/src/
*
* FILE DESCRIPTION:
*
* REMARKS:
  ...
  )
{
  ...
  /* Must be last! (Printing the if profiles externaly) */
  LOG_CLI((BSL_META_U(unit,
                      "in_lif_profile: %u\n\r"),info->lif_profile));
...
int
  SOC_PPC_OCC_PROFILE_USAGE_print(
    ...
    LOG_CLI((BSL_META_U(unit, "%.*s\n\r"), (int)(sal_strlen(apps[0])), "======================")); /* prints "underscore" */
    for (i=1; i <= size; i++) {
...
int
  SOC_PPC_OCC_ALL_PROFILES_print(
    ...
    if (SOC_IS_JERICHO(unit)) {
        ...
        /*rv = SOC_PPC_OCC_PROFILE_USAGE_print(unit, SOC_OCC_MGMT_TYPE_RIF, SOC_OCC_MGMT_RIF_SIZE);
        if (soc_sand_get_error_code_from_error_word(rv) != SOC_SAND_OK) { 
            return SOC_SAND_ERR; 
        }*/
    }
