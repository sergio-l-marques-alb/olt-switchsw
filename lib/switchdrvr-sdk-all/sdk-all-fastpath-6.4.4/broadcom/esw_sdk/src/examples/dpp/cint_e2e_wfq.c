...
/*
 * $Id: cint_tm_fap_config3.c,v 1.2 Broadcom SDK $
 *
...
{
    int rv = bcm_stk_modid_get(unit, &my_modid);
    if (rv != BCM_E_NONE) {
...
{
    e2e_wfq_cl_init(500000, 3000);
    e2e_wfq_fq_init(500000, 2500);
    e2e_wfq_voq_conn_init(500000, 2500);
    e2e_wfq_fq_weight_init(2, 4, 8, 16);
    e2e_wfq_wfq_mode_init(0);
    e2e_wfq_margin_cl_init(15000, 75);
    e2e_wfq_margin_fq_init(15000, 775);
    e2e_wfq_margin_voq_conn_init(15000, 775);
...
int e2e_wfq_start_run(int unit, int port, int mode)
{
    ...
    rv = bcm_cosq_gport_add(unit, dest_gport, num_cos, 
...
int e2e_wfq_verify(int unit)
{
    ...
    rc = bcm_cosq_gport_bandwidth_get(unit,g_e2e_wfq_gports.e2e_gport,0, 0,&kbits_sec_max, 0);
    if (rc != BCM_E_NONE) {
