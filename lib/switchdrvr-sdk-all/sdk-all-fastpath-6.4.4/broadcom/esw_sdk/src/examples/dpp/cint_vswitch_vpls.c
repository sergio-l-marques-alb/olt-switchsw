...
/* This is utility to translate encap_id returned by bcm_mpls_port_add
   to encap_id interface used as input to bcm_mpls_tunnel_initiator_create */
void vswitch_vpls_interface_encap_id_set(bcm_if_t encap_id, bcm_if_t * interface_encap_id) {
     *interface_encap_id = encap_id | (2 << 29);
}
...
vswitch_vlps_info_init(int extend_example, int ac_port, int pwe_in_port, int pwe_out_port, int ac_port1_outer_vlan, int ac_port1_inner_vlan,
    ...
    vswitch_vpls_info_1.access_index_3 = (is_gal?2:3);
    if (extend_example) {
        ...
        vswitch_vpls_info_2.access_index_3 = (is_gal?2:3); 
    }
    printf("vswitch_vlps_info_init %d\n", vswitch_vpls_info_1.vpn_id);
...
mpls_tunnels_config(int unit, int second_unit, int extend_example){
    ...
    int rv;
    int flags;
    int label_for_pop_entry_in, label_for_pop_entry_out;
    ...
    if (second_unit >= 0)
    {
        rv = create_l3_intf(second_unit, CINT_NO_FLAGS, mpls_lsr_info_1.eg_port, mpls_lsr_info_1.eg_vid, mpls_lsr_info_1.my_mac, &ingress_intf);
        if (rv != BCM_E_NONE) {
            printf("Error, in create_l3_intf\n");
            return rv;
        }
    ...
    if (second_unit >= 0)
    {
        ...
        rv = create_l3_egress(second_unit, BCM_L3_WITH_ID|flags, mpls_lsr_info_1.eg_port, mpls_lsr_info_1.eg_vid, ingress_intf, mpls_lsr_info_1.next_hop_mac, &egress_intf, &encap_id);
        if (rv != BCM_E_NONE) {
            printf("Error, in create_l3_egress\n");
            return rv;
        }
    ...
    else
    {
        egress_intf = 0;
        encap_id = 0;
        rv = create_l3_egress(unit, flags, mpls_lsr_info_1.eg_port, mpls_lsr_info_1.eg_vid,(pwe_php?0:ingress_intf), mpls_lsr_info_1.next_hop_mac, &egress_intf, &encap_id);
        if (rv != BCM_E_NONE) {
            printf("Error, in create_l3_egress\n");
            return rv;
        }
    ...
    if (extend_example) {
        ...
        /*  create ingress object, packet will be routed to */
        rv = create_l3_intf(unit, CINT_NO_FLAGS, mpls_lsr_info_1.eg_port, mpls_lsr_info_1.in_vid, mpls_lsr_info_1.my_mac, &ingress_intf);
        if (rv != BCM_E_NONE) {
            printf("Error, in create_l3_intf\n");
            return rv;
        }
        if (second_unit >= 0)
        {
            rv = create_l3_intf(second_unit, CINT_NO_FLAGS, mpls_lsr_info_1.eg_port, mpls_lsr_info_1.in_vid, mpls_lsr_info_1.my_mac, &ingress_intf);
            if (rv != BCM_E_NONE) {
                printf("Error, in create_l3_intf\n");
                return rv;
            }
        ...
        rv = bcm_mpls_tunnel_initiator_create(unit,0,is_gal?1:2,label_array);
        if (rv != BCM_E_NONE) {
            printf("Error, in bcm_mpls_tunnel_initiator_create\n");
            return rv;
        }
        ...
        if (second_unit >= 0)
        {
            rv = bcm_mpls_tunnel_initiator_create(second_unit,0,is_gal?1:2,label_array);
            if (rv != BCM_E_NONE) {
                printf("Error, in bcm_mpls_tunnel_initiator_create\n");
                return rv;
            }
 ...
 */
int multicast_mpls_port_add(int unit, int mc_group_id, int sys_port, int gport){
...
vswitch_vpls_add_network_port_1(int unit, int second_unit,  bcm_gport_t *port_id){
    ...
    /* Setting network group according to pwe_network_group_id_1 */
    if (is_device_or_above(unit,JERICHO) && soc_property_get(unit , "split_horizon_forwarding_groups_mode",1)) {
        if (pwe_network_group_id_1) {
            mpls_port_1.network_group_id = pwe_network_group_id_1;
    ...
    if (test_default_ttl_inheritance) {
        mpls_port_1.egress_label.flags = BCM_MPLS_EGRESS_LABEL_TTL_COPY | BCM_MPLS_EGRESS_LABEL_EXP_COPY;
    } else {
        mpls_port_1.egress_label.flags = BCM_MPLS_EGRESS_LABEL_TTL_SET;
    }
    
    if (vccv_type3_ttl) {
        mpls_port_1.vccv_type = bcmMplsPortControlChannelTtl;
    }
    ...
    if (ingress_egress_separate_ports) {
        /* Ingress_only */
        /* WITH_ID flags must be set in case of ingress-egress configuration */
        mpls_port_1.criteria = BCM_MPLS_PORT_MATCH_LABEL;
        mpls_port_1.flags |= BCM_MPLS_PORT_WITH_ID|BCM_MPLS_PORT_ENCAP_WITH_ID;
        mpls_port_1.egress_tunnel_if = vswitch_vpls_shared_info_1.egress_intf;
        if (mpls_termination_label_index_enable) {
        ...
        mpls_port_1.mpls_port_id = 0x18805000;
        mpls_port_1.encap_id = 0x5000;
        mpls_port_1.flags2 |= BCM_MPLS_PORT2_INGRESS_ONLY;
        if (encap_optimized) {
            mpls_port_1.flags2 |= BCM_MPLS_PORT2_LEARN_ENCAP;
        }
        rv = bcm_mpls_port_add(unit_or_second, vswitch_vpls_shared_info_1.vpn, &mpls_port_1);
        if (rv != BCM_E_NONE) {
            printf("Error, bcm_mpls_port_add #1\n");
            return rv;
        }
        ...
        mpls_lsr_info_1.mpls_port_id = mpls_port_1.mpls_port_id;
        
        /* handle of the created gport */
        *port_id = mpls_port_1.mpls_port_id;

        /* Egress_only */
        mpls_port_1.criteria = BCM_MPLS_PORT_MATCH_INVALID;
        mpls_port_1.flags2 &= ~BCM_MPLS_PORT2_INGRESS_ONLY;
        mpls_port_1.flags2 &= ~BCM_MPLS_PORT2_LEARN_ENCAP;
        mpls_port_1.flags2 |= BCM_MPLS_PORT2_EGRESS_ONLY;
        mpls_port_1.port = 0; /* Egress_only no port is required */
        mpls_port_1.match_label = 0;
        mpls_port_1.egress_tunnel_if = vswitch_vpls_info_1.tunnel_id;


        rv = bcm_mpls_port_add(unit_or_second, 0, &mpls_port_1);
        if (rv != BCM_E_NONE) {
            printf("Error, bcm_mpls_port_add #2\n");
            return rv;
        }
        printf("EGRESS: add port 0x%08x to vpn \n\r",mpls_port_1.mpls_port_id);
        egress_mpls_port_id = mpls_port_1.mpls_port_id;
    } else {
        mpls_port_1.criteria = BCM_MPLS_PORT_MATCH_LABEL;
        if (encap_optimized) {
            mpls_port_1.flags2 |= BCM_MPLS_PORT2_LEARN_ENCAP;
        ...
        if (mpls_termination_label_index_enable) {
            BCM_MPLS_INDEXED_LABEL_SET(&mpls_port_1.match_label, vswitch_vpls_info_1.in_vc_label, vswitch_vpls_info_1.access_index_3);
        }
        else {
            mpls_port_1.match_label = vswitch_vpls_info_1.in_vc_label;
        }
        ...
        rv = bcm_mpls_port_add(unit_or_second, vswitch_vpls_shared_info_1.vpn, &mpls_port_1);
        if (rv != BCM_E_NONE) {
            printf("Error, bcm_mpls_port_add #3\n");
            return rv;
        }
		vswitch_vpls_pairs.id2 =  mpls_port_1.mpls_port_id ;
		vswitch_vpls_pairs.label2 = mpls_port_1.match_label ;
        if(verbose1){
            printf("add port 0x%08x to vpn \n\r",mpls_port_1.mpls_port_id);
        }
        mpls_lsr_info_1.mpls_port_id = mpls_port_1.mpls_port_id;
        ...
        /* handle of the created gport */
        *port_id = egress_mpls_port_id = mpls_port_1.mpls_port_id;
    }

    /* GAL support for BFDoGACHoGAKoPWE encapsulation */
    if (is_gal) {
        mpls_port_1.tunnel_id = mpls_port_1.mpls_port_id;
        mpls_port_1.mpls_port_id = 0x18808010;
        mpls_port_1.flags2 = BCM_MPLS_PORT2_INGRESS_ONLY;
        mpls_port_1.flags |= BCM_MPLS_PORT_WITH_GAL | BCM_MPLS_PORT_CONTROL_WORD | BCM_MPLS_PORT_WITH_ID;

        rv = bcm_mpls_port_add(unit_or_second, vswitch_vpls_shared_info_1.vpn, &mpls_port_1);
        if (rv != BCM_E_NONE) {
            printf("Error, bcm_mpls_port_add #4\n");
            return rv;
        }
		vswitch_vpls_pairs.id3 =  mpls_port_1.mpls_port_id ;
		vswitch_vpls_pairs.label3 = mpls_port_1.match_label ;
    }

    vswitch_vpls_interface_encap_id_set(mpls_port_1.encap_id, &pwe_encap_id);
    ...
    bcm_stk_modid_get(unit_or_second, &modid);
    printf("modid is: %d\n", modid);
    ...
    rv = multicast_mpls_port_add(unit_or_second, vswitch_vpls_shared_info_1.vpn, modport, egress_mpls_port_id);
    if (rv != BCM_E_NONE) {
...
vswitch_vpls_add_network_port_2(int unit,  bcm_gport_t *port_id){
    ...
    /* if testing make before break, the vc label should be the same for both mpls ports */
    vc_label = make_before_break ? vswitch_vpls_info_1.in_vc_label : vswitch_vpls_info_2.in_vc_label;
    
    /* set port attribures, key <BC>*/
    if (mpls_termination_label_index_enable) {
        BCM_MPLS_INDEXED_LABEL_SET(&mpls_port_1.match_label, vc_label, vswitch_vpls_info_2.access_index_3);
    }
    else {
        mpls_port_1.match_label = vc_label;
    }


    mpls_port_1.egress_tunnel_if = vswitch_vpls_shared_info_1.egress_intf2;
    mpls_port_1.flags = BCM_MPLS_PORT_EGRESS_TUNNEL;

    /* Setting network group according to pwe_network_group_id_1 */
    if (is_device_or_above(unit,JERICHO) && soc_property_get(unit , "split_horizon_forwarding_groups_mode",1)) {
        if (pwe_network_group_id_2) {
            mpls_port_1.network_group_id = pwe_network_group_id_2;
            mpls_port_1.flags |= BCM_MPLS_PORT_NETWORK;
        }
    }
    else {
    ...
    mpls_port_1.port = vswitch_vpls_info_2.pwe_in_port;
    mpls_port_1.egress_label.label = vswitch_vpls_info_2.eg_vc_label;
    mpls_port_1.egress_label.ttl = 10;
    ...
    if (ingress_egress_separate_ports) {
        ...
        mpls_port_1.criteria = BCM_MPLS_PORT_MATCH_LABEL;
        mpls_port_1.flags |= BCM_MPLS_PORT_WITH_ID|BCM_MPLS_PORT_ENCAP_WITH_ID;
        mpls_port_1.mpls_port_id = 0x18805002;
        mpls_port_1.encap_id = 0x5002;
        mpls_port_1.flags2 |= BCM_MPLS_PORT2_INGRESS_ONLY;
        if (encap_optimized) {
            mpls_port_1.flags2 |= BCM_MPLS_PORT2_LEARN_ENCAP;
        }		
        ...
        *port_id = mpls_port_1.mpls_port_id;
        ...
        mpls_port_1.port = 0; /* Egress_only no port is required */
        mpls_port_1.match_label = 0;
        mpls_port_1.egress_tunnel_if = vswitch_vpls_info_2.tunnel_id;
    ...
    /* update Multicast to have the added port  */
    rv = multicast_mpls_port_add(unit, vswitch_vpls_shared_info_1.vpn, vswitch_vpls_info_2.pwe_in_port , egress_mpls_port_id);
    if (rv != BCM_E_NONE) {
        printf("Error, multicast_mpls_port_add\n");
        return rv;
    }
...
vswitch_vpls_add_access_port_1(int unit, int second_unit,  bcm_gport_t *port_id){
    ...
    BCM_GPORT_MODPORT_SET(modport, modid, vswitch_vpls_info_1.ac_in_port);
    ...
    *port_id = vlan_port_1.vlan_port_id;
    ...
    /* update Multicast to have the added port  */
    rv = multicast_vlan_port_add(unit, vswitch_vpls_info_1.vpn_id, modport , vlan_port_1.vlan_port_id);
    if (rv != BCM_E_NONE) {
        printf("Error, multicast_port_add\n");
        return rv;
    }
    if (is_two_dev)
    {
        rv = multicast_vlan_port_add(second_unit, vswitch_vpls_info_1.vpn_id, modport , vlan_port_1.vlan_port_id);
        if (rv != BCM_E_NONE) {
            printf("Error, multicast_port_add\n");
            return rv;
        }
...
vswitch_vpls_add_access_port_2(int unit,  bcm_gport_t *port_id){
    ...
    *port_id = vlan_port_1.vlan_port_id;
    ...
    /* update Multicast to have the added port  */
    rv = multicast_vlan_port_add(unit, vswitch_vpls_info_1.vpn_id, vswitch_vpls_info_2.ac_in_port , vlan_port_1.vlan_port_id);
    if (rv != BCM_E_NONE) {
        printf("Error, multicast_port_add\n");
        return rv;
    }
...
/* multicast utilities */
int 
open_ingress_mc_group(int unit, int mc_group_id){
    int rv = BCM_E_NONE;
    int flags;
 ...
 */ 
int
vswitch_vpls_allocate_push_profile(int unit, int exp, int ttl, int label, int has_cw){
    int rv = BCM_E_NONE;
    int push_profile;
    ...
    push_profile = 3;
    system_aux_push_profile_to_push_profile_id(push_profile, &mpls_port_push.mpls_port_id);
    mpls_port_push.egress_label.exp = exp;
    mpls_port_push.egress_label.ttl = ttl;
    mpls_port_push.egress_label.label = label;
