...
*      BCM> cint
*      cint> port_extender_dynamic_swithing_coe_info_init(0, 1, 10);
*      cint> port_extender_dynamic_swithing_coe_info_init(1, 2, 20);
...
coe_port_info_s coe_port_info[2];
bcm_port_t regular_port = 5;
int nof_coe_ports = 2;

/* Init coe port info */
int port_extender_dynamic_swithing_coe_info_init(uint32 index, uint32 coe_port, bcm_port_t coe_vlan)
{
...
int port_extender_dynamic_swithing_coe_mapping_set(int unit, int coe_vlan, bcm_port_t coe_port)
{
    int rv = 0;
    ...
    if (enable) {
        for (index = 0; index < nof_coe_ports; index++) {
            if (coe_port_info[index].coe_port == coe_port) {
                rv = port_extender_dynamic_swithing_coe_mapping_set(unit, coe_port_info[index].coe_vlan, coe_port_info[index].coe_port);
                if (rv != BCM_E_NONE) {
