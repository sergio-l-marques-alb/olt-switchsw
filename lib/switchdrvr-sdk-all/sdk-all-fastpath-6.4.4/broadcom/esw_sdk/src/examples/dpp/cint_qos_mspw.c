 ...
 * To Activate Above Settings Run:
   ...
   cint> qos_map_vpls(0);
   cint> vswitch_vpls_run_with_defaults(0,15,16,mpls_pipe_mode_exp_set);
...
/* Map Ingress COS-profile to do EXP-> TC for PWE LIF. */
...
int mpls_tunnels_config(int unit, mpls_info_s *info, int in_tunnel_label, int out_tunnel_label) 
{
    ...
    rv = create_l3_intf(unit, CINT_NO_FLAGS, open_vlan, cross_connect_info.in_port, cross_connect_info.in_vid, vrf, cross_connect_info.my_mac, &(info->l3_intf1));
    if (rv != BCM_E_NONE) {
        printf("Error, in create_l3_intf $rv\n");
        return rv;
    }
    
    /* create egress router interface */
    rv = create_l3_intf(unit, CINT_NO_FLAGS, open_vlan, cross_connect_info.eg_port, cross_connect_info.eg_vid, vrf, cross_connect_info.my_mac, &(info->l3_intf2));
    if (rv != BCM_E_NONE) {
        printf("Error, in create_l3_intf $rv\n");
        return rv;
    } 
