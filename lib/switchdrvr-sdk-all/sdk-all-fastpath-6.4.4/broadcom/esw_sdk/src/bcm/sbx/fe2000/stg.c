...
_bcm_fe2000_stg_recover(int unit)
{
    ...
    if (BCM_FAILURE(rv) || (size == 0)) {
        LOG_WARN(BSL_LS_BCM_STG,
                 (BSL_META_U(unit,
                             "Level 2 not enabled.  Application "
                              "must restate STGs\n")));
        return BCM_E_NONE;
    }
     ...
     */
    count = BCM_VLAN_COUNT * SBX_MAX_PORTS;
...
bcm_fe2000_stg_init(int unit)
{
    ...
    if (SOC_WARM_BOOT(unit)) {
        ...
        if (BCM_FAILURE(result)) {
            LOG_ERROR(BSL_LS_BCM_STG,
                      (BSL_META_U(unit,
                                  "Failed to access recovery data\n")));
            return result;
        }
