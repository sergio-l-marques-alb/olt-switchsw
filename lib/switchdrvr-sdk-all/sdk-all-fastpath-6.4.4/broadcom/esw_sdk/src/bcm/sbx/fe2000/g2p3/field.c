/*
 * $Id: field.c,v 1.91 Broadcom SDK $
...
typedef enum _bcm_fe2k_g2p3_fld_entry_flags_e {
    ...
    _FE2K_G2P3_ENTRY_POLFLAGS = (int)0xE0000000, /* policer flags mask */
    _FE2K_G2P3_ENTRY_TYPEPOL  = 0x20000000, /*  policer is typed */
    _FE2K_G2P3_ENTRY_MEFCOS   = 0x40000000, /*  polcier is MEF by CoS */
    _FE2K_G2P3_ENTRY_MEF      = (int)0x80000000  /*  policer is MEF */
...
typedef struct _bcm_fe2k_g2p3_field_entry_s {
    ...
    uint32 entryFlags;                              /* internal flags */
                                    ...
                                    const _bcm_fe2k_field_entry_index entry)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_FP,
              (BSL_META_U(glob->unit,
                          "unit %d entry %08X allocated from free list\n"),
               glob->unit,
               entry));
    return BCM_E_NONE;
}
                                ...
                                const _bcm_fe2k_field_entry_index entry)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_FP,
              (BSL_META_U(glob->unit,
                          "unit %d entry %08X returned to free list\n"),
               glob->unit,
               entry));
    return BCM_E_NONE;
}
                                   ...
                                   _bcm_fe2k_g2p3_hardware_rule_t *rule)
{
    ...
    default:
        LOG_ERROR(BSL_LS_BCM_FP,
                  (BSL_META("invalid rulebase %d for unit %d group %08X"
                            " entry %08X\n"),
    ...
    LOG_DEBUG(BSL_LS_BCM_FP,
              (BSL_META_U(glob->unit,
                          "set initial defaults for unit %d group %08X"
                          " entry %08X\n"),
               glob->unit,
                 thisEntry->group,
                 entry));
                                            ...
                                            _bcm_fe2k_g2p3_hardware_rule_t *rule)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_FP,
              (BSL_META_U(glob->unit,
                          "set no qualifiers for unit %d group %08X"
                          " entry %08X\n"),
               glob->unit,
                 thisEntry->group,
                 entry));
                                         ...
                                         _bcm_fe2k_g2p3_hardware_rule_t *rule)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_FP,
              (BSL_META_U(glob->unit,
                          "set no actions for unit %d group %08X"
                          " entry %08X\n"),
               glob->unit,
                 thisEntry->group,
                 entry));
                                ...
                                _bcm_fe2k_g2p3_hardware_rule_t *rule)
{
    ...
    if (thisEntry->hwRule) {
        ...
        default:
            LOG_ERROR(BSL_LS_BCM_FP,
                      (BSL_META("invalid rulebase %d for unit %d"
                                " entry %08X (group %08X)\n"),
                       glob->group[thisEntry->group].rulebase,
                       glob->unit,
                       entry,
        ...
        if (BCM_E_NONE != result) {
            LOG_ERROR(BSL_LS_BCM_FP,
                      (BSL_META("unable to read unit %d group %08X"
    ...
    LOG_DEBUG(BSL_LS_BCM_FP,
              (BSL_META_U(glob->unit,
                          "load current rules for unit %d entry %08X\n"),
               glob->unit,
               entry));
    return result;
}
                                 ...
                                 _bcm_fe2k_g2p3_hardware_rule_t *rule)
{
    ...
    if (thisEntry->hwRule) {
        ...
        if (BCM_E_NONE != result) {
            LOG_ERROR(BSL_LS_BCM_FP,
                      (BSL_META("unable to write unit %d group %08X"
                                " entry %08X rule (rb %02X"
                                " rule %08X): %d (%s)\n"),
                       glob->unit,
                       thisEntry->group,
    ...
    LOG_DEBUG(BSL_LS_BCM_FP,
              (BSL_META_U(glob->unit,
                          "write new rules for unit %d entry %08X\n"),
               glob->unit,
               entry));
    return result;
}
                                  ...
                                  _bcm_fe2k_g2p3_hardware_rule_t *rule)
{
    ...
    if (haveRule & (~needRule) ) {
        ...
        if (BCM_E_NONE == result) {
            ...
            LOG_ERROR(BSL_LS_BCM_FP,
                      (BSL_META("unable to remove unit %d group %08X"
                                " entry %08X rule (rb %02X"
                                " rule %08X): %d (%s)\n"),
                       glob->unit,
                       thisEntry->group,
    ...
    LOG_DEBUG(BSL_LS_BCM_FP,
              (BSL_META_U(glob->unit,
                          "entry ports " FIELD_PBMP_FORMAT "\n"),
               FIELD_PBMP_SHOW(targetPorts)));
    LOG_DEBUG(BSL_LS_BCM_FP,
              (BSL_META_U(glob->unit,
                          "have rule %02X; need rule %02X\n"),
               haveRule,
               needRule));
    ...
    if (needRule == 1) {
        ...
        if (haveRule == 1) {
            ...
            if (BCM_E_NONE != result) {
                LOG_ERROR(BSL_LS_BCM_FP,
                          (BSL_META("unable to update unit %d group"
            ...
            LOG_DEBUG(BSL_LS_BCM_FP,
                      (BSL_META_U(glob->unit,
                                  "Adding new entry after %02X:%08X\n"),
                       thisRb->dbId,
            ...
            if (BCM_E_NONE == result) {
                ...
                LOG_ERROR(BSL_LS_BCM_FP,
                          (BSL_META("unable to create unit %d group"
                               ...
                               _bcm_fe2k_g2p3_hardware_rule_t *rule)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_FP,
              (BSL_META_U(glob->unit,
                          "added entry %08X to group %08X on unit %d\n"),
               entry,
               group,
                 glob->unit));
                                          ...
                                          soc_sbx_g2p3_counter_value_t *counts)
{
    ...
    if (thisEntry->hwRule) {
        LOG_DEBUG(BSL_LS_BCM_FP,
                  (BSL_META_U(glob->unit,
                              "%s counter for unit %d entry %08X"
                              " (rule %02X:%08X)\n"),
                   clear?"clear":"read",
                     glob->unit,
                     entry,
                     glob->rulebase[glob->group[thisEntry->group].rulebase].dbId,
                     (uint32)(thisEntry->hwRule)));
        COMPILER_64_ZERO(currCounts.bytes);
        COMPILER_64_ZERO(currCounts.packets);
        switch (glob->group[thisEntry->group].rulebase) {
        case 0:
        ...
        LOG_DEBUG(BSL_LS_BCM_FP,
                  (BSL_META_U(glob->unit,
                              "byte counter for unit %d entry %08X"
                              " (rule %02X:%08X)"
                              " = %08X%08X\n"),
                     glob->unit,
                     entry,
        ...
        LOG_DEBUG(BSL_LS_BCM_FP,
                  (BSL_META_U(glob->unit,
                              "frame counter for unit %d entry %08X"
                              " (rule %02X:%08X)"
                              " = %08X%08X\n"),
                     glob->unit,
                     entry,
        ...
        if ((thisEntry->entryFlags & _FE2K_G2P3_ENTRY_COUNTER) &&
            ...
            COMPILER_64_ADD_64(counts->bytes, currCounts.bytes);
            COMPILER_64_ADD_64(counts->packets, currCounts.packets);
            if (COMPILER_64_LT(counts->bytes, currCounts.bytes)) {
                COMPILER_64_SET(counts->bytes, 0xFFFFFFFF, 0xFFFFFFFF);
            }
            if (COMPILER_64_LT(counts->packets, currCounts.packets)) {
                                    ...
                                    const _bcm_fe2k_field_group_index group)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_FP,
              (BSL_META_U(glob->unit,
                          "unit %d group %08X allocated from free list\n"),
               glob->unit,
               group));
    return BCM_E_NONE;
}
                                ...
                                const _bcm_fe2k_field_group_index group)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_FP,
              (BSL_META_U(glob->unit,
                          "unit %d group %08X returned to free list\n"),
               glob->unit,
               group));
    return BCM_E_NONE;
}
                               ...
                               const _bcm_fe2k_field_group_index group)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_FP,
              (BSL_META_U(glob->unit,
                          "unit %d group %08X added to rulebase %d"
                          " at priority %d\n"),
               glob->unit,
                 group,
                 rulebase & 0xFF,
                               ...
                               const _bcm_fe2k_field_group_index group)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_FP,
              (BSL_META_U(glob->unit,
                          "unit %d group %08X removed from rulebase %d\n"),
               glob->unit,
               group,
                 thisGroup->rulebase));
    soc_sbx_g2p3_ace_group_del(glob->unit,thisGroup->rulebase, (int16)group);
                                  ...
                                  _bcm_fe2k_g2p3_field_group_t *groupData)
{
    ...
    _bcm_fe2k_field_group_index currGroup;
    _bcm_fe2k_g2p3_field_group_t *thisGroup;
    uint8 tempPri[_FIELD_FE2K_G2P3_MAX_GROUPS];
    uint8 rulebase;
    ...
    if (BCM_E_NONE  == result) {
        ...
        if (!SOC_WARM_BOOT(glob->unit)) {
            ...
            soc_sbx_g2p3_ace_group_create(glob->unit, rulebase, (int16) *group,
                                     ...
                                     unsigned int rulebase)
{
    ...
    uint32 entryFlags;/* working entry flags */
    int result = BCM_E_NONE;                    /* working result */
    ...
    if (BCM_E_NONE == result) {
        LOG_DEBUG(BSL_LS_BCM_FP,
                  (BSL_META_U(glob->unit,
                              "update 'in hardware' state for entries\n")));
        ...
        if (BCM_E_NONE  != result) {
            LOG_ERROR(BSL_LS_BCM_FP,
                      (BSL_META("unable to commit rules to scache for"
    ...
    if (enabled) {
        ...
        if (BCM_E_NONE == result) {
            ...
            FIELD_PRINT(("%s  [Unable to read rule counts: %d (%s)]\n",
                         ...
                         _SHR_ERRMSG(result)));
            LOG_ERROR(BSL_LS_BCM_FP,
                      (BSL_META("unable to read unit %d rule %02X:%08X"
                                " counts: %d (%s)\n"),
                       glob->unit,
                       glob->group[thisEntry->group].rulebase,
                       (uint32)(thisEntry->hwRule),
...
_bcm_fe2000_g2p3_field_scache_store(_bcm_fe2k_g2p3_field_glob_t *thisUnit, 
    ...
    if (thisUnit->group != NULL) {
        ...
        LOG_ERROR(BSL_LS_BCM_FP,
                  (BSL_META("unable to restore unit 0x%x groups\n"),
                   thisUnit->unit));
        return BCM_E_MEMORY;
    }
...
_bcm_fe2000_g2p3_field_scache_recover(_bcm_fe2k_g2p3_field_glob_t *thisUnit){
    ...
    for (i=0; i<_FIELD_FE2K_G2P3_MAX_GROUPS; i++){
        if (group[i].rulebase < thisUnit->uMaxSupportedStages) {
            ...
            if (BCM_E_NONE != result) {
                soc_scache_handle_unlock(thisUnit->unit, _field_cache[thisUnit->unit].wb_hdl);
                LOG_ERROR(BSL_LS_BCM_FP,
                          (BSL_META("unable to recover unit 0x%x group %08x: %d (%s)\n"),
    ...
    for (i=0; i<_FIELD_FE2K_G2P3_STAGES; i++) {
        ...
        if (result != BCM_E_NONE) {
            LOG_ERROR(BSL_LS_BCM_FP,
                      (BSL_META("unable to recover handle unit %d entry %08X:"
                                " %d (%s); aborting\n"),
                       thisUnit->unit,
                       curEntry,
                       result,
        ...
        for (e=0; e<entries; e++) {
            ...
            if (entry[curEntry].entryFlags & _FE2K_G2P3_ENTRY_IN_HW) {
                ...
                if (BCM_E_NONE != result) {
                    LOG_ERROR(BSL_LS_BCM_FP,
                              (BSL_META("unable to recover handle unit %d group %08X"
                                        " entry %08X rule (rb %02X"
                                        " rule %08X): %d (%s)\n"),
                               thisUnit->unit,
                               entry[curEntry].group,
                ...
                if (BCM_E_NONE != result) {
                    LOG_ERROR(BSL_LS_BCM_FP,
                              (BSL_META("unable to reallocate unit %d entry %08X:"
                                        " %d (%s); aborting\n"),
                               thisUnit->unit,
                               curEntry,
                               result,
            ...
            if (entry[curEntry].entryFlags & _FE2K_G2P3_ENTRY_IN_HW) {
                    ...
                    if (thisUnit->entry[curEntry].range[r]) {
                        ...
                        if (result == BCM_E_NOT_FOUND){
                            ...
                            if (BCM_E_NONE != result) {
                                ...
                                LOG_ERROR(BSL_LS_BCM_FP,
                                          (BSL_META("error recovring unit %d range %08X,"
                                                    " destroying it\n"),
                                           thisUnit->unit,
                                           tempRange));
                ...
                if (BCM_E_NONE != result) {
                    LOG_ERROR(BSL_LS_BCM_FP,
                              (BSL_META("unable to recover entry, unit %d entry %08X:"
                                        " %d (%s); aborting\n"),
                               thisUnit->unit,
                               curEntry,
                               result,
                ...
                if (BCM_E_NONE != result) {
                    LOG_ERROR(BSL_LS_BCM_FP,
                              (BSL_META("unable to add entry, unit %d entry %08X:"
                                        " %d (%s); aborting\n"),
                               thisUnit->unit,
                               curEntry,
                               result,
                ...
                LOG_WARN(BSL_LS_BCM_FP,
                         (BSL_META("unable to recover entry, unit %d entry %08X: not committed to hardware\n"),
                          thisUnit->unit,
                          curEntry));
            }
            ...
            if ( entry[curEntry].nextEntryRb < thisUnit->entryTotal) {
                ...
                if (BCM_E_NONE != result) {
                    LOG_ERROR(BSL_LS_BCM_FP,
                              (BSL_META("unable to get next handle unit %d group %08X"
                                        " entry %08X rule (rb %02X"
                                        " rule %08X): %d (%s)\n"),
                               thisUnit->unit,
                               entry[curEntry].group,
...
bcm_fe2000_g2p3_field_init(int unit, void **unitData)
{
    ...
    if (BCM_E_NONE == result) {
        ...
        LOG_DEBUG(BSL_LS_BCM_FP,
                  (BSL_META_U(unit,
                              "Unit %d ingress rules = %d\n"),
                   unit,
                   rulesIngress));
    } else { /* if (BCM_E_NONE == result) */
        LOG_ERROR(BSL_LS_BCM_FP,
                  (BSL_META_U(unit,
                              "unable to get unit %d ingress rules:"
                              " %d (%s)\n"),
                   unit,
                   result,
                   _SHR_ERRMSG(result)));
    ...
    if (BCM_E_NONE == result) {
        ...
        LOG_DEBUG(BSL_LS_BCM_FP,
                  (BSL_META_U(unit,
                              "Unit %d egress rules = %d\n"),
                   unit,
                   rulesEgress));
    } else { /* if (BCM_E_NONE == result) */
        LOG_ERROR(BSL_LS_BCM_FP,
                  (BSL_META_U(unit,
                              "unable to get unit %d egress rules:"
                              " %d (%s)\n"),
                   unit,
                   result,
                   _SHR_ERRMSG(result)));
    ...
    LOG_DEBUG(BSL_LS_BCM_FP,
              (BSL_META_U(unit,
                          "unit %d supports %d ranges (computed)\n"),
               unit,
               rangesTotal));
    LOG_DEBUG(BSL_LS_BCM_FP,
              (BSL_META_U(unit,
                          "unit %d supports %d groups (configured)\n"),
               unit,
               groupsTotal));
    ...
    LOG_DEBUG(BSL_LS_BCM_FP,
              (BSL_META_U(unit,
                          "allocate unit description (%d bytes)\n"),
    ...
    LOG_DEBUG(BSL_LS_BCM_FP,
              (BSL_META_U(unit,
                          "allocate unit %d index space (%d bytes)\n"),
               unit,
    ...
    if (BCM_E_NONE == result) {
        ...
        LOG_DEBUG(BSL_LS_BCM_FP,
                  (BSL_META_U(unit,
                              "init unit %d entries into free list\n"),
                   unit));
        ...
        LOG_DEBUG(BSL_LS_BCM_FP,
                  (BSL_META_U(unit,
                              "init unit %d groups into free list\n"),
                   unit));
                    ...
                    if (tmpRes == BCM_E_NONE) {
                        ...
                        if (SOC_WARM_BOOT(unit)) {
                            ...
                            if (result != BCM_E_NONE) {
                                LOG_ERROR(BSL_LS_BCM_FP,
                                          (BSL_META_U(unit,
                                                      "unit %d to initialize field WB cache: %d (%s)\n"), 
                                           thisUnit->unit, 
                                           result, 
                                           _SHR_ERRMSG(result)));
                            }
    ...
    if ((result == BCM_E_NONE) && !SOC_WARM_BOOT(thisUnit->unit)) {
        ...
        LOG_DEBUG(BSL_LS_BCM_FP,
                  (BSL_META_U(unit,
                              "clear unit %d rulebases\n"), unit));
        for (i = 0; i < thisUnit->uMaxSupportedStages; i++) {
            LOG_DEBUG(BSL_LS_BCM_FP,
                      (BSL_META_U(unit,
                                  "clear unit %d rulebase %02X \n"),
                       thisUnit->unit,
                       thisUnit->rulebase[i].dbId));
            switch (i) {
            case 0:
            ...
            if (BCM_E_NONE != tmpRes) {
                ...
                LOG_ERROR(BSL_LS_BCM_FP,
                          (BSL_META_U(unit,
                                      "unable to clear unit %d rulebase %02X: %d (%s)\n"),
                           thisUnit->unit,
                           thisUnit->rulebase[i].dbId,
                           result,
                           _SHR_ERRMSG(result)));
            ...
            if (BCM_E_NONE != tmpRes) {
                ...
                LOG_ERROR(BSL_LS_BCM_FP,
                          (BSL_META_U(unit,
                                      "unable to commit unit %d rulebase %02X: %d (%s)\n"),
                           thisUnit->unit,
                           thisUnit->rulebase[i].dbId,
                           result,
                           _SHR_ERRMSG(result)));
        ...
        *unitData = (void*)thisUnit;
        ...
        if (thisUnit) {
            if (thisUnit->rangeFree) {
                ...
                LOG_DEBUG(BSL_LS_BCM_FP,
                          (BSL_META_U(unit,
                                      "get rid of unit %d range freelist\n"),
                           unit));
            ...
            LOG_DEBUG(BSL_LS_BCM_FP,
                      (BSL_META_U(unit,
                                  "get rid of unit %d private data\n"),
                       unit));
            sal_free(thisUnit);
            thisUnit = NULL;
        } /* if (thisUnit) */
...
bcm_fe2000_g2p3_field_detach(void *unitData)
{
    ...
    for (i = 0; i < thisUnit->entryTotal; i++) {
        ...
        if (thisEntry->hwRule) {
            ...
            if (BCM_E_NONE != tmpRes) {
                result = tmpRes;
                LOG_ERROR(BSL_LS_BCM_FP,
                          (BSL_META("unable to remove unit %d entry %08X"
                                    " rule %02X:%08X: %d (%s)\n"),
                           thisUnit->unit,
                           i,
    ...
    for (i = 0; i < thisUnit->uMaxSupportedStages; i++) {
        ...
        if (BCM_E_NONE != tmpRes) {
            result = tmpRes;
            LOG_ERROR(BSL_LS_BCM_FP,
                      (BSL_META("unable to clear unit %d rulebase %02X: %d (%s)\n"),
                       thisUnit->unit,
                       thisUnit->rulebase[i].dbId,
                       result,
                       _SHR_ERRMSG(result)));
        ...
        if (!SOC_WARM_BOOT(thisUnit->unit)) {
            ...
            default:
                LOG_ERROR(BSL_LS_BCM_FP,
                          (BSL_META("unit %d unexpected stage %d\n"),
                           thisUnit->unit,
                           i));
                tmpRes = BCM_E_INTERNAL;
            }
            if (BCM_E_NONE != tmpRes) {
                result = tmpRes;
                LOG_ERROR(BSL_LS_BCM_FP,
                          (BSL_META("unable to commit unit %d rulebase %02X: %d (%s)\n"),
                           thisUnit->unit,
                           thisUnit->rulebase[i].dbId,
                           result,
                           _SHR_ERRMSG(result)));
    ...
    if (BCM_E_NONE == tmpRes) {
        ...
        if (BCM_E_NONE != tmpRes) {
            result = tmpRes;
            LOG_ERROR(BSL_LS_BCM_FP,
                      (BSL_META("unable to get %d:xt[%08X]: %d (%s)\n"),
            ...
            if (BCM_E_NONE != tmpRes) {
                result = tmpRes;
                LOG_ERROR(BSL_LS_BCM_FP,
                          (BSL_META("unable to set %d:xt[%08X]: %d (%s)\n"),
                           thisUnit->unit,
                           excId,
                           result,
                           _SHR_ERRMSG(result)));
        ...
        result = tmpRes;
        LOG_ERROR(BSL_LS_BCM_FP,
                  (BSL_META("unable to get unit %d copyToCpu exception ID:"
                            " %d (%s)\n"),
                   thisUnit->unit,
                   result,
                   _SHR_ERRMSG(result)));
...
bcm_fe2000_g2p3_field_wb_state_sync(void *unitData, int sync)
{
    ...
    if (SOC_WARM_BOOT(thisUnit->unit)) {
        LOG_ERROR(BSL_LS_BCM_FP,
                  (BSL_META("Cannot write to SCACHE during WarmBoot\n")));
        return BCM_E_INTERNAL;
    }
    ...
    if (thisGroup->entries) {
        ...
        LOG_DEBUG(BSL_LS_BCM_FP,
                  (BSL_META_U(thisUnit->unit,
                              "updated unit %d group %08X qset in place\n"),
                   thisUnit->unit,
                   group));
        return BCM_E_NONE;
    } /* if (thisGroup->entries) */
                                    ...
                                    bcm_field_entry_t entry)
{
    ...
    if (thisEntry->entryFlags & _FE2K_G2P3_ENTRY_IN_HW) {
        ...
        LOG_ERROR(BSL_LS_BCM_FP,
                  (BSL_META("unit %d entry %08X is already installed\n"),
                   thisUnit->unit,
                   entry));
        return BCM_E_EXISTS;
    }
                                    ...
                                    const int apply)
{
    ...
    default:
        /* should never reach this */
        LOG_ERROR(BSL_LS_BCM_FP,
                  (BSL_META("unit %d group %08X qset inconsistent"
                            " with supported\n"),
                   thisUnit->unit,
                   thisUnit->entry[entry].group));
        return BCM_E_INTERNAL;
                                     ...
                                     const int apply)
{
    ...
    if (BCM_PBMP_IS_NULL(ports)) {
        LOG_ERROR(BSL_LS_BCM_FP,
                  (BSL_META("must provide at least one acceptable port\n")));
        return BCM_E_CONFIG;
    }
                                      ...
                                      bcm_pbmp_t mask)
{
    ...
    if (BCM_PBMP_NOT_NULL(pbmp)) {
        LOG_ERROR(BSL_LS_BCM_FP,
                  (BSL_META("ports " FIELD_PBMP_FORMAT " in data but not"
                                         ...
                                         bcm_pbmp_t *pbmask)
{
    ...
    if (thisUnit->entry[entry].hwRule) {
        ...
        LOG_ERROR(BSL_LS_BCM_FP,
                  (BSL_META("unable to locate a rule for unit %d"
                            " entry %08X\n"),
                   thisUnit->unit,
                   entry));
        return BCM_E_INTERNAL;
    ...
    if (BCM_E_NOT_FOUND == result) {
        /* no port restriction */
        LOG_ERROR(BSL_LS_BCM_FP,
                  (BSL_META("unit %d entry %08X has no port qualification\n"),
                   thisUnit->unit,
                   entry));
        return BCM_E_NOT_FOUND;
    }
    ...
    case bcmFieldQualifyOutPort:
        ...
        BCM_PBMP_ITER(ports, pIndex) {
            if (multiple) {
                LOG_ERROR(BSL_LS_BCM_FP,
                          (BSL_META("a single-port qualifier can not be"
        ...
        if (multiple) {
            ...
            *gpmask = ~0;
            ...
            LOG_ERROR(BSL_LS_BCM_FP,
                      (BSL_META("could not find any ports to which unit"
                                " %d entry %08X applies\n"),
                       thisUnit->unit,
                       entry));
            return BCM_E_NOT_FOUND;
    ...
    default:
        ...
        LOG_ERROR(BSL_LS_BCM_FP,
                  (BSL_META("unknown port class qualifier %d\n"),
                   qual));
        return BCM_E_INTERNAL;
    } /* switch (qual) */
    return result;
                                        ...
                                        bcm_vlan_t mask)
{
    ...
    if (0x8000 & mask) {
        LOG_WARN(BSL_LS_BCM_FP,
                 (BSL_META("unit %d does not support CFI matching\n"),
                  thisUnit->unit));
    }
    /* get the entry qualifiers and actions so we can update them */
    result = _bcm_fe2k_g2p3_field_rules_read(thisUnit, entry, &rule);
    ...
    LOG_DEBUG(BSL_LS_BCM_FP,
              (BSL_META_U(thisUnit->unit,
                          "write OuterVlan(%03X,%03X) qualifier to"
                          " unit %d entry %08X\n"),
               data,
                 mask,
                 thisUnit->unit,
                                        ...
                                        uint16 mask)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_FP,
              (BSL_META_U(thisUnit->unit,
                          "write EtherType(%04X,%04X) qualifier to"
                          " unit %d entry %08X\n"),
               data,
                 mask,
                 thisUnit->unit,
                                         ...
                                         uint8 mask)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_FP,
              (BSL_META_U(thisUnit->unit,
                          "write IpProtocol(%02X,%02X) qualifier to"
                          " unit %d entry %08X\n"),
               data,
                 mask,
                 thisUnit->unit,
                                    ...
                                    bcm_ip_t mask)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_FP,
              (BSL_META_U(thisUnit->unit,
                          "write SrcIp(%08X,%08X) qualifier to"
                          " unit %d entry %08X\n"),
               data,
                 mask,
                 thisUnit->unit,
                                    ...
                                    bcm_ip_t mask)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_FP,
              (BSL_META_U(thisUnit->unit,
                          "write DstIp(%08X,%08X) qualifier to"
                          " unit %d entry %08X\n"),
               data,
                 mask,
                 thisUnit->unit,
                                   ...
                                   uint8 mask)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_FP,
              (BSL_META_U(thisUnit->unit,
                          "write DSCP(%02X,%02X) qualifier to"
                          " unit %d entry %08X\n"),
               data,
                 mask,
                 thisUnit->unit,
                                         ...
                                         uint8 mask)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_FP,
              (BSL_META_U(thisUnit->unit,
                          "write TcpControl(%02X,%02X) qualifier to"
                          " unit %d entry %08X\n"),
               data,
                 mask,
                 thisUnit->unit,
                                         ...
                                         int invert)
{
    ...
    if (invert) {
        LOG_ERROR(BSL_LS_BCM_FP,
                  (BSL_META("invert option is not supported on unit %d\n"),
                   thisUnit->unit));
        return BCM_E_PARAM;
    }
    ...
    LOG_DEBUG(BSL_LS_BCM_FP,
              (BSL_META_U(thisUnit->unit,
                          "write RangeCheck(%08X) qualifier to"
                          " unit %d entry %08X\n"),
               range,
                 thisUnit->unit,
                 entry));
                                     ...
                                     bcm_mac_t mask)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_FP,
              (BSL_META_U(thisUnit->unit,
                          "write SrcMac(" FIELD_MACA_FORMAT ","
                                     ...
                                     bcm_mac_t mask)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_FP,
              (BSL_META_U(thisUnit->unit,
                          "write DstMac(" FIELD_MACA_FORMAT ","
                          FIELD_MACA_FORMAT ") qualifier to"
                          " unit %d entry %08X\n"),
                 FIELD_MACA_SHOW(data),
                 FIELD_MACA_SHOW(mask),
                                     ...
                                     bcm_ip6_t mask)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_FP,
              (BSL_META_U(thisUnit->unit,
                          "write SrcIp6(" FIELD_IPV6A_FORMAT ","
                                     ...
                                     bcm_ip6_t mask)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_FP,
              (BSL_META_U(thisUnit->unit,
                          "write DstIp6(" FIELD_IPV6A_FORMAT ","
                          FIELD_IPV6A_FORMAT ") qualifier to"
                          " unit %d entry %08X\n"),
                 FIELD_IPV6A_SHOW(data),
                 FIELD_IPV6A_SHOW(mask),
                                 ...
                                 uint32 param1)
{
    _bcm_fe2k_g2p3_field_glob_t *thisUnit;      /* working unit data */
    _bcm_fe2k_g2p3_hardware_rule_t rule;        /* working rule */
    uint32 newFlags;  /* working flags */
    uint32 oldFlags;  /* working flags */
    uint32 chkFlags;  /* working flags */
    int result;                                 /* working result */
    unsigned int mirrorId = 0;                  /* working mirror ID */
    ...
    case bcmFieldActionDrop:
        if (thisUnit->dropPolicer) {
            ...
            LOG_ERROR(BSL_LS_BCM_FP,
                      (BSL_META_U(unit,
                                  "no blackhole policer available for %s"
                                  " action on unit %d\n"),
                       _sbx_fe2000_field_action_name[action],
                       thisUnit->unit));
    ...
    case bcmFieldActionVlanNew:
        if (param0 > SBX_DYNAMIC_VSI_END(thisUnit->unit)) {
            LOG_ERROR(BSL_LS_BCM_FP,
                      (BSL_META_U(unit,
                                  "unit %d only supports VSI %04X..%04X\n"),
                       thisUnit->unit,
                       0,
    ...
    case bcmFieldActionCosQNew:
        if (param0 >= SBX_MAX_COS) {
            LOG_ERROR(BSL_LS_BCM_FP,
                      (BSL_META_U(unit,
                                  "unit %d only supports COS queues %d..%d\n"),
                       thisUnit->unit,
                       0,
                       SBX_MAX_COS));
            return BCM_E_PARAM;
    ...
    case bcmFieldActionL3Switch:
        if (!(_FE2K_L3_FTE_VALID(thisUnit->unit, param0))) {
            LOG_ERROR(BSL_LS_BCM_FP,
                      (BSL_META_U(unit,
                                  "unit %d only supports FT values"
                                  " %d..%d\n"),
                       thisUnit->unit,
                       0,
                       0x0003FFFF));
    ...
    case bcmFieldActionRedirect:
        ...
        if (BCM_E_NONE == result) {
            ...
            LOG_ERROR(BSL_LS_BCM_FP,
                      (BSL_META_U(unit,
                                  "unable to locate redirect target for mod"
                                  " %d port %d: %d (%s)\n"),
                       param0,
                       param1,
                       result,
    ...
    case bcmFieldActionRedirectMcast:
        ...
        if (BCM_E_NONE == result) {
            ...
            LOG_ERROR(BSL_LS_BCM_FP,
                      (BSL_META_U(unit,
                                  "unable to allocate FT entry for %s(%08X)"
                                  " action: %d (%s)\n"),
                       _sbx_fe2000_field_action_name[bcmFieldActionRedirectMcast],
                       param0,
                       result,
    ...
    if (oldFlags & chkFlags) {
        ...
        LOG_ERROR(BSL_LS_BCM_FP,
                  (BSL_META_U(unit,
                              "unit %d entry %08X already has %s action\n"),
                   thisUnit->unit,
                   entry,
                   _sbx_fe2000_field_action_name[action]));
    ...
    if ((BCM_E_NONE == result) && (bcmFieldActionUpdateCounter == action)) {
        ...
        if (!(thisUnit->entry[entry].entryFlags & _FE2K_G2P3_ENTRY_COUNTER)) {
            LOG_ERROR(BSL_LS_BCM_FP,
                      (BSL_META_U(unit,
                                  "unit %d entry %08X does not have an"
                                  " assigned counter\n"),
                       thisUnit->unit,
                       entry));
            return BCM_E_CONFIG;
        ...
        if (!(BCM_FIELD_COUNTER_MODE_ALL & param0)) {
            LOG_ERROR(BSL_LS_BCM_FP,
                      (BSL_META_U(unit,
                                  "unit %d assuming request to count NO frame"
        ...
        if (BCM_FIELD_COUNTER_MODE_ALL !=
            ...
            LOG_ERROR(BSL_LS_BCM_FP,
                      (BSL_META_U(unit,
                                  "unit %d can only count ALL colours\n"),
                       thisUnit->unit));
            return BCM_E_PARAM;
        }
        if ((~(BCM_FIELD_COUNTER_MODE_ALL | BCM_FIELD_COUNTER_MODE_BYTES |
            ...
            LOG_ERROR(BSL_LS_BCM_FP,
                      (BSL_META_U(unit,
                                  "unit %d does not support bits %08X in"
                                  " counter mode\n"),
                       thisUnit->unit,
        ...
        default:
            ...
            LOG_ERROR(BSL_LS_BCM_FP,
                      (BSL_META_U(unit,
                                  "unit %d does not support counter mode %d\n"),
                       thisUnit->unit,
                       param0 & 0xF));
            return BCM_E_PARAM;
        }
    ...
    if (BCM_E_NONE == result) {
        ...
        case _SBX_FE2K_FIELD_OVERLAP_ACTION_ERR:
            LOG_ERROR(BSL_LS_BCM_FP,
                      (BSL_META_U(unit,
                                  "unit %d entry %08X already has an"
        ...
        default:
            LOG_ERROR(BSL_LS_BCM_FP,
                      (BSL_META_U(unit,
                                  "unable to add %s action to unit %d"
                                  " entry %08X: %d (%s)\n"),
                       _sbx_fe2000_field_action_name[action],
                       thisUnit->unit,
                       entry,
    ...
    if (BCM_E_NONE == result) {
        ...
        LOG_DEBUG(BSL_LS_BCM_FP,
                  (BSL_META_U(thisUnit->unit,
                              "free any resources allocated for action\n")));
...
{
    _bcm_fe2k_g2p3_field_glob_t *thisUnit;      /* working unit data */
    uint32 flags;     /* working flags */
    int result = BCM_E_NONE;                    /* working result */
    unsigned int ftidx, vlan;
    unsigned int mirrorId;

    thisUnit = (_bcm_fe2k_g2p3_field_glob_t*)unitData;
    COMPILER_REFERENCE(mirrorId);
    ...
    if (flags & _FE2K_G2P3_ENTRY_REDIRECT) {
        ...
        if (BCM_FAILURE(result)) {
            LOG_ERROR(BSL_LS_BCM_FP,
                      (BSL_META("Failed to convert fabric port to switch"
                                " port: %s\n"), bcm_errmsg(result)));
            return result;
        }
    ...
    if (flags & _FE2K_G2P3_ENTRY_REDIRMC) {
        ...
        if (BCM_E_NONE != result) {
            LOG_ERROR(BSL_LS_BCM_FP,
                      (BSL_META("unable to recover FTE resource, unit %d entry %08X"
                                 ...
                                 uint32 *param1)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_FP,
              (BSL_META_U(thisUnit->unit,
                          "read %s action from unit %d entry %08X\n"),
               _sbx_fe2000_field_action_name[action],
               thisUnit->unit,
                 entry));
    switch (thisUnit->group[thisUnit->entry[entry].group].rulebase) {
                                    ...
                                    bcm_field_action_t action)
{
    ...
    if (BCM_E_NONE == result) {
        LOG_DEBUG(BSL_LS_BCM_FP,
                  (BSL_META_U(thisUnit->unit,
                              "remove %s action from unit %d entry %08X\n"),
                   _sbx_fe2000_field_action_name[action],
                   thisUnit->unit,
                     entry));
    ...
    if (BCM_E_NONE == result) {
        ...
        LOG_DEBUG(BSL_LS_BCM_FP,
                  (BSL_META_U(thisUnit->unit,
                              "release resources used by %s action"
                                        ...
                                        bcm_field_entry_t entry)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_FP,
              (BSL_META_U(thisUnit->unit,
                          "remove all actions from unit %d entry %08X\n"),
               thisUnit->unit,
               entry));
    result = _bcm_fe2k_g2p3_field_rules_write(thisUnit, entry, &rule);
    if (BCM_E_NONE == result) {
        ...
        LOG_DEBUG(BSL_LS_BCM_FP,
                  (BSL_META_U(thisUnit->unit,
                              "release resources used by actions"
                              " from unit %d entry %08X\n"),
                   thisUnit->unit,
                     entry));
        if (mirrorId) {
                                     ...
                                     bcm_field_entry_t entry)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_FP,
              (BSL_META_U(thisUnit->unit,
                          "enabling counter on unit %d entry %08X\n"),
               thisUnit->unit,
               entry));
                                      ...
                                      bcm_field_entry_t entry)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_FP,
              (BSL_META_U(thisUnit->unit,
                          "disabling counter on unit %d entry %08X\n"),
               thisUnit->unit,
               entry));
                                          ...
                                          int action)
{
    ...
    if (!(thisEntry->entryFlags & _FE2K_G2P3_ENTRY_DROP)) {
        ...
        case 3:
            /* egress v6 */
            LOG_ERROR(BSL_LS_BCM_FP,
                      (BSL_META("policing not supported on egress v6 rules"
                                " for unit %d\n"),
                       glob->unit));
            return BCM_E_UNAVAIL;
            break;
