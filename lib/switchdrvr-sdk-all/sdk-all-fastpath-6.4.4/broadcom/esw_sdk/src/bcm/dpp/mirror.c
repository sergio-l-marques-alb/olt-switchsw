...
bcm_petra_mirror_destination_erspan_lif_alloc(int unit, uint32 alloc_flags, int *global_outlif, int *local_outlif) {
    ...
    if (SOC_IS_JERICHO(unit)) {
        ...
        if (alloc_flags & BCM_DPP_AM_FLAG_ALLOC_WITH_ID) {
            rv = _bcm_dpp_lif_id_is_mapped(unit, FALSE, *global_outlif, &is_mapped_lif);
        ...
        if (is_mapped_lif) {
            rv = bcm_dpp_am_global_lif_alloc(unit, alloc_flags | BCM_DPP_AM_FLAG_ALLOC_EGRESS, global_outlif);
            BCMDNX_IF_ERR_EXIT(rv);
        } else {
            local_lif_flags = alloc_flags;
            local_outlif_info.base_lif_id = *global_outlif;
        }

        local_outlif_info.glif = *global_outlif;
        local_outlif_info.app_alloc_info.pool_id =  dpp_am_res_eg_data_erspan;
        local_outlif_info.app_alloc_info.application_type = 0;
        local_outlif_info.local_lif_flags = (is_mapped_lif) ? BCM_DPP_AM_OUT_LIF_FLAG_COUPLED : BCM_DPP_AM_OUT_LIF_FLAG_DIRECT;

        rv = _bcm_dpp_am_local_outlif_alloc(unit, local_lif_flags, &local_outlif_info);
        BCMDNX_IF_ERR_EXIT(rv);

        *local_outlif = local_outlif_info.base_lif_id;
        *global_outlif = local_outlif_info.glif;

        /* Write global lif mapping to the glem. */
        if (SOC_IS_JERICHO(unit) && is_mapped_lif) {
            rv = MBCM_PP_DRIVER_CALL(unit, mbcm_pp_lif_glem_access_entry_add, (unit, *global_outlif, *local_outlif));
            BCMDNX_IF_ERR_EXIT(rv);
        }
