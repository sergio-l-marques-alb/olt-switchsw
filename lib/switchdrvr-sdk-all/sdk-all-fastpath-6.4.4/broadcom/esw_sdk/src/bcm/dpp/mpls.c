    ...
    int unit)
{
    ...
    if (SOC_DPP_CONFIG(unit)->pp.mldp_support
        ...
        for(i=0; i < 2; i++){
            ...
            BCM_GPORT_TUNNEL_ID_SET(mldp_tunnel_switch.tunnel_id, SOC_DPP_CONFIG(unit)->pp.mldp_dummy_lif_id[i]);
    ...
    bcm_gport_t mpls_port_id)
{
    ...
    rv = _bcm_dpp_gport_to_global_and_local_lif(unit, mpls_port_id, &global_lif, &local_in_lif, NULL, NULL, &is_local);
    BCMDNX_IF_ERR_EXIT(rv);

    if (local_in_lif==BCM_GPORT_INVALID) {
        is_member = 1;
        is_local = 1;
    }
    else {
    ...
    bcm_vpn_t vpn)
{
    ...
    for (iter = 0; iter != -1;) {
        ...
        in_lif_val = iter - 1;
        if ((in_lif_val == SOC_DPP_CONFIG(unit)->pp.local_lif_index_drop) ||
		    (in_lif_val == SOC_DPP_CONFIG(unit)->pp.local_lif_index_simple)) {
            continue;
        }
    ...
    bcm_mpls_port_t * mpls_port)
{
    ...
    uint8 found;
    uint32 lif_idx;
    int local_in_lif_id, local_out_lif_id, global_lif;
    ...
    rv = _bcm_dpp_gport_to_global_and_local_lif(unit, mpls_port->mpls_port_id, &global_lif, &local_in_lif_id, &local_out_lif_id, NULL, &is_local);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    if (is_ingress) {
        /* Ensure InLIF is PWE */
        BCMDNX_IF_ERR_EXIT( _bcm_dpp_lif_usage_get(soc_sand_dev_id, local_in_lif_id, -1, &lif_usage, NULL) );
        ...
        rv = _bcm_dpp_in_lif_mpls_match_get(unit, mpls_port, local_in_lif_id);
        BCMDNX_IF_ERR_EXIT(rv);

        rv = _bcm_dpp_am_template_lif_term_data_get(unit, local_in_lif_id, &term_profile_info);
        BCMDNX_IF_ERR_EXIT(rv);
    ...
    if (is_egress) {
        ...
        rv = _bcm_dpp_lif_usage_get(soc_sand_dev_id, -1, local_out_lif_id, NULL, &lif_usage);
        BCMDNX_IF_ERR_EXIT(rv);
        if (lif_usage != _bcmDppLifTypeMplsPort) {
    ...
    if (is_egress || (gport_type == _BCM_DPP_GPORT_IN_TYPE_FEC_VC)) {   
    ...
    for (start_indx = num_labels; start_indx > 0; start_indx = start_indx - 1 - skip_entries) {
        ...
        if (label_action_type == _bcm_dpp_mpls_egress_action_type_swap) { /* if swap */
            ...
            if (SOC_IS_JERICHO(unit) && is_mapped_lif && !eep_alloced) {
                rv = MBCM_PP_DRIVER_CALL(unit, mbcm_pp_lif_glem_access_entry_add, (unit, global_lif_id, local_lif_id));
                BCMDNX_IF_ERR_EXIT(rv);
            }
        ...
        if (encap_entry_info[indx].entry_type == SOC_PPD_EG_ENCAP_ENTRY_TYPE_MPLS_ENCAP) {  /* mpls encap */
            ...
            for (cur_indx = 0; cur_indx < cur_count; ++cur_indx) {
                ...
                if (SOC_IS_JERICHO(unit)) {
                    ...
                    rv = _bcm_dpp_local_lif_is_wide_entry(unit, local_lif_id, 0, &is_wide_entry);
                    BCMDNX_IF_ERR_EXIT(rv);
    ...
    if (rv == BCM_E_NOT_FOUND) {
        ...
        if (BCM_GPORT_IS_TUNNEL(info->tunnel_id) || (info->flags & BCM_MPLS_SWITCH_WITH_ID)) {
            ...
            if (SOC_DPP_CONFIG(unit)->pp.mldp_support && (info->flags & BCM_MPLS_SWITCH_LOOKUP_NONE) &&
                (lif_indx == SOC_DPP_CONFIG(unit)->pp.mldp_dummy_lif_id[0] || lif_indx == SOC_DPP_CONFIG(unit)->pp.mldp_dummy_lif_id[1])) {
        ...
        if (update) {/* update check LIF already allocated */
            ...
            if (SOC_IS_JERICHO(unit)) {
                ...
                /* 
                 * First, allocate global lif. If update, we deallocate the old global lif at the end, 
                 *      because the local lif deallocation needs it to remove the mapping.
                 */
                global_lif_flags |= ((alloc_flags & SHR_RES_ALLOC_WITH_ID) ? SHR_RES_ALLOC_WITH_ID : 0) | BCM_DPP_AM_FLAG_ALLOC_INGRESS;
                global_lif_id = lif_indx;
                ...
                local_inlif_info.glif = global_lif_id;
        ...
        if (SOC_IS_ARAD(unit)) {
            ...
            /* Special case for mLDP: termination type is concluded according to LIF-ID. LIF-ID is determined by SOC property */
            if (SOC_DPP_CONFIG(unit)->pp.mldp_support && (info->flags & BCM_MPLS_SWITCH_LOOKUP_NONE) &&
                (lif_indx == SOC_DPP_CONFIG(unit)->pp.mldp_dummy_lif_id[1])) {
                    inner_flags |= _BCM_DPP_MPLS_LIF_TERM_GAL;
            }
    ...
    bcm_mpls_tunnel_switch_t * info)
{
    ...
    if (rv == BCM_E_NOT_FOUND) {
        ...
        if (rif_key.flags & SOC_PPD_RIF_MPLS_LABEL_LOOKUP_NONE) {
           ...
           rv = _bcm_dpp_gport_to_global_and_local_lif(unit, info->tunnel_id, &global_lif_id, &local_in_lif_id, NULL, NULL, NULL);
           BCMDNX_IF_ERR_EXIT(rv);
