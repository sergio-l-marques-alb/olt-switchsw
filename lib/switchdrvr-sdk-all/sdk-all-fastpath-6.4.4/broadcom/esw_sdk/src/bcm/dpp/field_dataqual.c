/*
 * $Id: field_dataqual.c,v 1.33 Broadcom SDK $
                                    ...
                                    bcm_field_data_qualifier_t *qual)
{
    ...
    if (!SHR_BITGET(unitData->dataFieldInUse, qual_id)) {
    ...
    if (control_info.val[0] != SOC_TMC_NOF_PMF_CE_SUB_HEADERS) { 
        ...
        int type;
        int qual_valid = 0;
        SOC_PPD_FP_QUAL_TYPE *soc_ppd_qual_ptr;
        ...
        for (qualifier = bcmFieldQualifySrcIp6; qualifier < bcmFieldQualifyCount; ++qualifier) {
            for (type = 0; type < unitData->devInfo->types; ++type) {
                if (BCM_FIELD_QSET_TEST(unitData->stageD[hwStageId].qsetType[type],qualifier)) {
                    qual_valid = 1;
                }
            }
            ...
            if (qual_valid) {
                ...
                if (soc_ppd_qual_ptr[0] == control_info.val[3] && soc_ppd_qual_ptr[1] == SOC_PPD_NOF_FP_QUAL_TYPES) {
                    break;
                }
            }
        }
    ...
    if(control_info.val[1] & SOC_PPD_FP_CONTROL_TYPE_HDR_USER_DEF_FLAG_NEGATIVE) {
        qual->flags |= BCM_FIELD_DATA_QUALIFIER_OFFSET_NEGATIVE;
        control_info.val[1] &= ~SOC_PPD_FP_CONTROL_TYPE_HDR_USER_DEF_FLAG_NEGATIVE; /* will have the positive value */
    }
    if (SHR_BITGET(unitData->dataFieldOfsBit, qual_id)) {
        /* PPD bits -> BCM bits */
    ...
    if (SHR_BITGET(unitData->dataFieldLenBit, qual_id)) {
        /* PPD bits -> BCM bits */
...
_bcm_petra_field_data_qualifier_init(_bcm_dpp_field_info_t *unitData)
{
    ...
    limit = _BCM_PETRA_FIELD_DQ_LIMIT(unitData->unit);
    unitData->dqLimit = limit;

    /* Ingress qualifiers, programmable */
    
    for (index = 0; index < limit; index++) {
        SHR_BITSET(unitData->unitQset.udf_map, index);
        SHR_BITSET(unitData->stageD[0].stgQset.udf_map, index);
        for (offset = 0; offset < unitData->devInfo->types; offset++) {
            SHR_BITSET(unitData->stageD[0].qsetType[offset].udf_map, index);
        }
    }
                                    ...
                                    unsigned int *bcmIndex)
{
    ...
    if (SOC_PPC_FP_IS_QUAL_TYPE_USER_DEFINED(hwType)) {
        *bcmIndex = hwType - SOC_PPD_FP_QUAL_HDR_USER_DEF_0;
    }
    else {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
                         (_BSL_BCM_MSG_NO_UNIT("unit %d HW qualifier %d (%s)"
                                           " not valid data qualifier"),
                          unit,
                          hwType,
                          SOC_PPD_FP_QUAL_TYPE_to_string(hwType)));
    }
    if (*bcmIndex > unitData->dqLimit) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
                                    ...
                                    SOC_PPD_FP_QUAL_TYPE *hwType)
{
    ...
    if (bcmIndex > unitData->dqLimit) {
        ...
        *hwType = SOC_PPD_FP_QUAL_HDR_USER_DEF_0 + bcmIndex;
    }
exit:
    BCMDNX_FUNC_RETURN;
}
                                      ...
                                      bcm_field_data_qualifier_t *data_qualifier)
{
    ...
    SOC_PPD_FP_DATABASE_STAGE stage;
    BCMDNX_INIT_FUNC_DEFS;
    ...
    /* get HW stage ID */
    _bcm_dpp_ppd_stage_from_bcm_stage(unitData, data_qualifier->stage, &stage);
    ...
    if (data_qualifier->flags & BCM_FIELD_DATA_QUALIFIER_WITH_ID) {
        ...
        if ((0 > qual_id) || (unitData->dqLimit <= qual_id)) {
            LOG_ERROR(BSL_LS_BCM_FP,
                      (BSL_META_U(unit,
    ...
    if (BCM_E_NONE == result) {
        ...
        if (!(data_qualifier->flags & BCM_FIELD_DATA_QUALIFIER_OFFSET_PREDEFINED)) {
            ...
            case bcmFieldDataOffsetBasePacketStart:
                /* The base header is Header_0, the L2 header is header_1 */
                control_info.val[0] = SOC_PPD_FP_BASE_HEADER_TYPE_HEADER_0;
            ...
            case bcmFieldDataOffsetBaseL2Header:
                control_info.val[0] = SOC_PPD_FP_BASE_HEADER_TYPE_HEADER_1;
                break;
                /*
            ...
            case bcmFieldDataOffsetBaseFirstHeader:
                control_info.val[0] = SOC_PPD_FP_BASE_HEADER_TYPE_HEADER_2;
                break;
            case bcmFieldDataOffsetBaseSecondHeader:
                control_info.val[0] = SOC_PPD_FP_BASE_HEADER_TYPE_HEADER_3;
                break;
            case bcmFieldDataOffsetBaseThirdHeader:
                control_info.val[0] = SOC_PPD_FP_BASE_HEADER_TYPE_HEADER_4;
                break;
            case bcmFieldDataOffsetBaseFourthHeader:
                control_info.val[0] = SOC_PPD_FP_BASE_HEADER_TYPE_HEADER_5;
                break;
            case bcmFieldDataOffsetBaseForwardingHeader:
                control_info.val[0] = SOC_PPD_FP_BASE_HEADER_TYPE_FWD;
                break;
            case bcmFieldDataOffsetBaseNextForwardingHeader:
                control_info.val[0] = SOC_PPD_FP_BASE_HEADER_TYPE_FWD_POST;
                break;
            default:
                LOG_ERROR(BSL_LS_BCM_FP,
                          (BSL_META_U(unit,
            ...
            for (types=1; types <= 0xf; types = (types << 1) + 1) {
                result = _bcm_dpp_ppd_qual_from_bcm_qual(unitData, stage,0xf,data_qualifier->qualifier,&soc_ppd_qual_ptr);
                if(BCM_FAILURE(result)) {
                    _DPP_FIELD_UNIT_UNLOCK(unitData);
                    BCM_RETURN_VAL_EXIT(result);
                }
                if (SOC_PPD_NOF_FP_QUAL_TYPES != soc_ppd_qual_ptr[1]) {
                    LOG_ERROR(BSL_LS_BCM_FP,
                              (BSL_META_U(unit,
        ...
        if(data_qualifier->flags & BCM_FIELD_DATA_QUALIFIER_OFFSET_NEGATIVE) {
            control_info.val[1] |= SOC_PPD_FP_CONTROL_TYPE_HDR_USER_DEF_FLAG_NEGATIVE;
        }
    } /* if (BCM_E_NONE == result) */
    if (BCM_E_NONE == result) {
    ...
    if (BCM_E_NONE == result) {
        ...
        SHR_BITSET(unitData->dataFieldInUse, qual_id);
        unitData->dataFieldRefs[qual_id] = 0;
        if (data_qualifier->flags & BCM_FIELD_DATA_QUALIFIER_OFFSET_BIT_RES) {
            SHR_BITSET(unitData->dataFieldOfsBit, qual_id);
        } else {
            SHR_BITCLR(unitData->dataFieldOfsBit, qual_id);
        }
        if (data_qualifier->flags & BCM_FIELD_DATA_QUALIFIER_LENGTH_BIT_RES) {
            SHR_BITSET(unitData->dataFieldLenBit, qual_id);
        } else {
            SHR_BITCLR(unitData->dataFieldLenBit, qual_id);
        }
...
bcm_petra_field_data_qualifier_destroy(int unit, int qual_id)
{
    ...
    if ((0 > qual_id) || (unitData->dqLimit <= qual_id)) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_NOT_FOUND,
                         (_BSL_BCM_MSG("data qualifier %d invalid"),
                          qual_id));
    }
    ...
    if (SHR_BITGET(unitData->dataFieldInUse, qual_id)) {
        if (unitData->dataFieldRefs[qual_id]) {
            ...
            LOG_ERROR(BSL_LS_BCM_FP,
                      (BSL_META_U(unit,
                       ...
                       unit,
                       qual_id,
                       unitData->dataFieldRefs[qual_id],
                       (1 != unitData->dataFieldRefs[qual_id])?"s":""));
            result = BCM_E_CONFIG;
        } else {
    ...
    if (BCM_E_NONE == result) {
        SHR_BITCLR(unitData->dataFieldInUse, qual_id);
        SHR_BITCLR(unitData->dataFieldLenBit, qual_id);
        SHR_BITCLR(unitData->dataFieldOfsBit, qual_id);
#ifdef BCM_WARM_BOOT_SUPPORT
        _bcm_dpp_field_datafield_all_wb_save(unitData, NULL, NULL);
                                   ...
                                   bcm_field_data_qualifier_t *qual)
{
    ...
    if ((0 > qual_id) || (qual_id >= unitData->dqLimit)) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_NOT_FOUND,
                         (_BSL_BCM_MSG_NO_UNIT("unit %d data qualifier %d"
                                           " not valid"),
                          unitData->unit,
