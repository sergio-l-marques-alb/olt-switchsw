/*
 * $Id: common.c,v 1.160 Broadcom SDK $
...
typedef struct _bcm_common_bookkeeping_s {
    ...
    int misc_port_profile_created;  /* Flag to check Egr LPORT profile creation */
...
#endif /* BCM_TRIUMPH3_SUPPORT */
#ifdef BCM_TRIDENT2_SUPPORT
    if (SOC_IS_TRIDENT2X(unit) || SOC_IS_TOMAHAWK(unit)) {
        ...
        if (SOC_MEM_IS_VALID(unit, mem)) {
            if (NULL == info->egr_lport_profile) {
                ...
                if (SOC_WARM_BOOT(unit)) {
                    ...
                    rv = _bcm_egr_lport_profile_mem_reference
                        (unit,
                         _bcm_trx_lport_tab_default_entry_index_get(unit), 1);
                } else {
    ...
    if(soc_feature(unit, soc_feature_misc_i2e_hgclass_combo_profile)) {
        ...
        if (SOC_MEM_IS_VALID(unit, mem)) {
            if (NULL == info->misc_port_profile) {
                ...
                index_min[mems_cnt] = SOC_MAX_NUM_PORTS;
                index_max[mems_cnt] = soc_mem_index_max(unit, mem);
                mems_cnt++;
    ...
    if(soc_feature(unit, soc_feature_egr_qos_combo_profile)) {
        ...
        if (SOC_MEM_IS_VALID(unit, mem)) {
            if (NULL == info->egr_qos_profile) {
                ...
                mems_cnt = 0;
                mems[mems_cnt] = mem;
                mem_words[mems_cnt] = sizeof(egr_pri_cng_map_entry_t) / sizeof(uint32);
                index_min[mems_cnt] = soc_mem_index_min(unit, mem);
                index_max[mems_cnt] = soc_mem_index_max(unit, mem);
                mems_cnt++;
                ...
                if (SOC_MEM_IS_VALID(unit, mem)) {
                    ...
                    mems[mems_cnt] = mem;
                    mem_words[mems_cnt] = sizeof(egr_dscp_table_entry_t) /
                        sizeof(uint32);
                    index_min[mems_cnt] = soc_mem_index_min(unit, mem);
                    index_max[mems_cnt] = soc_mem_index_max(unit, mem);
   ...
   if (!soc_feature(unit,soc_feature_advanced_flex_counter)) {
    ...
    if (SOC_MEM_IS_VALID(unit, mem)) {
        if (NULL == info->ing_vlan_vfi_mbrship_profile) {
             ...
             */
            if (SOC_WARM_BOOT(unit)) {
                rv = _bcm_vlan_vfi_memebership_profile_mem_reference
                    (unit,
                ...
                BCM_IF_ERROR_RETURN(
            	    soc_profile_mem_add(unit, info->ing_vlan_vfi_mbrship_profile,
    ...
    if (SOC_MEM_IS_VALID(unit, mem)) {
        if (NULL == info->egr_vlan_vfi_mbrship_profile) {
            ...
            if (SOC_WARM_BOOT(unit)) {
                rv = _bcm_vlan_vfi_memebership_profile_mem_reference
                    (unit, soc_mem_index_min(unit, EGR_VLAN_VFI_MEMBERSHIPm), 1, 0); 
            } else {
                /* coverity[stack_use_callee] */
                /* coverity[stack_use_overflow] */
                ...
                BCM_IF_ERROR_RETURN(
            	    soc_profile_mem_add(unit, info->egr_vlan_vfi_mbrship_profile,
                                        entries, 1, &prof_index));
            }
 ...
 * Parameters:
 *      unit        - (IN) Device number
 *      table_id    - (IN) Index of table to modify, EGR_LPORT or RTAG7
 *      field_count - (IN) Number of fields to modify
 *      fields      - (IN) Array of fields to modify
 *      values      - (IN) Array of new field values
 * Returns:
 ...
 */
int
_bcm_egr_lport_profile_fields32_modify(int unit, int table_id, int field_count,
                                   soc_field_t *fields, uint32 *values)
{
    int rv;
    int i;
    ...
    if (table_id == EGR_LPORT_PROFILE_LPORT_TAB) {
        mem = EGR_LPORT_PROFILEm;
    } else if (table_id == EGR_LPORT_PROFILE_VLAN_CONTROL_1) {
        mem = EGR_VLAN_CONTROL_1m;
    } else if (table_id == EGR_LPORT_PROFILE_VLAN_CONTROL_2) {
        mem = EGR_VLAN_CONTROL_2m;
    } else if (table_id == EGR_LPORT_PROFILE_VLAN_CONTROL_3) {
        mem = EGR_VLAN_CONTROL_3m;
    } else if (table_id == EGR_LPORT_PROFILE_IPMC_CFG2) {
        mem = EGR_IPMC_CFG2m;
    } else if (table_id == EGR_LPORT_PROFILE_MTU) {
        mem = EGR_MTUm;
    } else if (table_id == EGR_LPORT_PROFILE_PORT_1) {
        mem = EGR_PORT_1m;
    } else if (table_id == EGR_LPORT_PROFILE_COUNTER_CONTROL) {
        mem = EGR_COUNTER_CONTROLm;
    } else if (table_id == EGR_LPORT_PROFILE_SHAPING_CONTROL) {
        mem = EGR_SHAPING_CONTROLm;
    } else {
        return BCM_E_INTERNAL;
    }
    ...
    COMMON_LOCK(unit);
    rv = soc_profile_mem_fields32_modify(unit, EGR_LPORT_PROFILE(unit), table_id,
                                         field_count, fields, values);
    COMMON_UNLOCK(unit);
 ...
 */
int
_bcm_egr_lport_profile_field32_modify(int unit, int table_id,
                                  soc_field_t field, uint32 value)
{
    return _bcm_egr_lport_profile_fields32_modify(unit, table_id,
                                              1, &field, &value);
}
 ...
 * Purpose:
 *      Update EGR_MISC_PORT_PROFILE_TABLE entry reference count and entries_per_set 
 *      information for given index.
 * Parameters:
 *      unit            - (IN) Device number
 *      index           - (IN) Base index for entry to update
                                   ...
                                   soc_field_t *fields, uint32 *values)
{
    ...
    if (table_id == FP_I2E_CLASSID_SELECTm) {
        mem = LPORT_TABm;
    } else if (table_id == FP_HG_CLASSID_SELECTm) {
        mem = RTAG7_PORT_BASED_HASHm;
    } else {
        return BCM_E_INTERNAL;
    }
...
#define _BCM_TRX_MYSTA_MEMACC(u, memacc_type) \
        &(_bcm_trx_mysta_memacc_list[u][memacc_type])

#define _BCM_TRX_MYSTA_FIELD_MEMACC(u, fld) \
        &(_bcm_trx_mysta_memacc_list[u][fld])

#define _BCM_TRX_MYSTA_FIELD32_MEMACC_GET(u, ent, fld) \
        soc_memacc_field32_get(_BCM_TRX_MYSTA_FIELD_MEMACC(u, \
                            _BCM_TRX_MYSTA_MEMACC_##fld), ent)
#define _BCM_TRX_MYSTA_FIELD_MEMACC_GET(u, ent, fld, fldbuf) \
        soc_memacc_field_get(_BCM_TRX_MYSTA_FIELD_MEMACC(u, \
                            _BCM_TRX_MYSTA_MEMACC_##fld), ent, fldbuf)
#define _BCM_TRX_MYSTA_MAC_ADDR_MEMACC_GET(u, ent, fld, mac) \
        soc_memacc_mac_addr_get(_BCM_TRX_MYSTA_FIELD_MEMACC(u, \
                            _BCM_TRX_MYSTA_MEMACC_##fld), ent, mac)                                           
#define _BCM_TRX_MYSTA_FIELD32_MEMACC_SET(u, ent, fld, val) \
        soc_memacc_field32_set(_BCM_TRX_MYSTA_FIELD_MEMACC(u, \
                            _BCM_TRX_MYSTA_MEMACC_##fld), ent, val)
#define _BCM_TRX_MYSTA_FIELD_MEMACC_SET(u, ent, fld, fldbuf) \
        soc_memacc_field_set(_BCM_TRX_MYSTA_FIELD_MEMACC(u, \
                            _BCM_TRX_MYSTA_MEMACC_##fld), ent, fldbuf)
#define _BCM_TRX_MYSTA_MAC_ADDR_MEMACC_SET(u, ent, fld, mac) \
        soc_memacc_mac_addr_set(_BCM_TRX_MYSTA_FIELD_MEMACC(u, \
                            _BCM_TRX_MYSTA_MEMACC_##fld), ent, mac) 
...
_bcm_trx_mysta_memacc_init(int unit)
{
    ...
    do {
        _bcm_trx_mysta_memacc_list[unit] =
            sal_alloc(alloc_size, "L2 tables memacc data");
        if (NULL == _bcm_trx_mysta_memacc_list[unit]) {
            rv = BCM_E_MEMORY;
            break;
        }

        memacc_map = _bcm_trx_mysta_field_memacc_map;
        memacc_list = _bcm_trx_mysta_memacc_list[unit];
        ...
        for (mam_ix = 0; mam_ix < memacc_map_size; mam_ix++) {
            ...
            rv = soc_memacc_init(unit, MY_STATION_TCAMm,
                                 memacc_map[mam_ix],
                                 &(memacc_list[mam_ix]));
            if (BCM_FAILURE(rv)) {
                break;
                                ...
                                my_station_tcam_entry_t *entry)
{
    ...
    _BCM_TRX_MYSTA_MAC_ADDR_MEMACC_GET(unit, entry, MAC_ADDRf, l2caddr->mac);
    _BCM_TRX_MYSTA_MAC_ADDR_MEMACC_GET(unit, entry, MAC_ADDR_MASKf, l2caddr->mac_mask);
    l2caddr->vlan = 
        _BCM_TRX_MYSTA_FIELD32_MEMACC_GET(unit, entry, VLAN_IDf);
    l2caddr->vlan_mask = 
        _BCM_TRX_MYSTA_FIELD32_MEMACC_GET(unit, entry, VLAN_ID_MASKf);
    l2caddr->src_port = 
        _BCM_TRX_MYSTA_FIELD32_MEMACC_GET(unit, entry, ING_PORT_NUMf);
    l2caddr->src_port_mask = 
        _BCM_TRX_MYSTA_FIELD32_MEMACC_GET(unit, entry, ING_PORT_NUM_MASKf);

    l2caddr->flags |= BCM_L2_CACHE_L3;
    if (_BCM_TRX_MYSTA_FIELD32_MEMACC_GET(unit, entry, DISCARDf)) {
        l2caddr->flags |= BCM_L2_CACHE_DISCARD;
    }
    if (_BCM_TRX_MYSTA_FIELD32_MEMACC_GET(unit, entry, COPY_TO_CPUf)) {
                              ...
                              bcm_l2_cache_addr_t *l2caddr)
{
    ...
    _BCM_TRX_MYSTA_FIELD32_MEMACC_SET(unit, entry, VALIDf, 1);
    _BCM_TRX_MYSTA_FIELD32_MEMACC_SET(unit, entry, VLAN_IDf,
                        l2caddr->vlan);
    _BCM_TRX_MYSTA_FIELD32_MEMACC_SET(unit, entry, VLAN_ID_MASKf,
                        l2caddr->vlan_mask);
    _BCM_TRX_MYSTA_MAC_ADDR_MEMACC_SET(unit, entry, MAC_ADDRf,
                         l2caddr->mac);
    _BCM_TRX_MYSTA_MAC_ADDR_MEMACC_SET(unit, entry, MAC_ADDR_MASKf,
                         l2caddr->mac_mask);
    _BCM_TRX_MYSTA_FIELD32_MEMACC_SET(unit, entry, ING_PORT_NUMf,
                        l2caddr->src_port);
    _BCM_TRX_MYSTA_FIELD32_MEMACC_SET(unit, entry, ING_PORT_NUM_MASKf,
                        l2caddr->src_port_mask);
    ...
    if (l2caddr->flags & BCM_L2_CACHE_DISCARD) {
        _BCM_TRX_MYSTA_FIELD32_MEMACC_SET(unit, entry, DISCARDf, 1);
    }
    if (l2caddr->flags & BCM_L2_CACHE_CPU) {
        _BCM_TRX_MYSTA_FIELD32_MEMACC_SET(unit, entry, COPY_TO_CPUf, 1);
    }
}
                           ...
                           my_station_tcam_entry_t *entry)
{
    ...
    _BCM_TRX_MYSTA_MAC_ADDR_MEMACC_GET(unit, entry, MAC_ADDRf,
                         l2addr->mac);
    l2addr->vid = _BCM_TRX_MYSTA_FIELD32_MEMACC_GET(unit, entry, VLAN_IDf);

    if (_BCM_TRX_MYSTA_FIELD32_MEMACC_GET(unit, entry,
                            IPV4_TERMINATION_ALLOWEDf)) {
                         ...
                         bcm_l2_addr_t *l2addr, int incl_key_mask)
{
    ...
    _BCM_TRX_MYSTA_FIELD32_MEMACC_SET(unit, entry, DISCARDf, fval);

    fval = l2addr->flags & BCM_L2_COPY_TO_CPU ? 1 : 0;
    _BCM_TRX_MYSTA_FIELD32_MEMACC_SET(unit, entry, COPY_TO_CPUf, fval);
}
                          ...
                          int *entry_index, int *alt_index)
{
    ...
    if (soc_mem_field_valid(unit, MY_STATION_TCAMm, SOURCE_FIELDf)) {
        port_mask =
            (1 << soc_mem_field_length(unit, MY_STATION_TCAMm, SOURCE_FIELDf)) - 1;
    } else {
        port_mask =
    ...
    _BCM_TRX_MYSTA_FIELD32_MEMACC_SET(unit, &entry0, VALIDf, 1);
    _BCM_TRX_MYSTA_FIELD32_MEMACC_SET(unit, &entry0, VLAN_IDf, vlan);
    _BCM_TRX_MYSTA_MAC_ADDR_MEMACC_SET(unit, &entry0, MAC_ADDRf, mac);
    _BCM_TRX_MYSTA_FIELD32_MEMACC_SET(unit, &entry0, VLAN_ID_MASKf, 0xfff);
    _BCM_TRX_MYSTA_MAC_ADDR_MEMACC_SET(unit, &entry0, MAC_ADDR_MASKf,
                         mac_mask);
    mask0 = entry0;
    _BCM_TRX_MYSTA_FIELD32_MEMACC_SET(unit, &mask0, VLAN_IDf, 0xfff);
    _BCM_TRX_MYSTA_MAC_ADDR_MEMACC_SET(unit, &mask0, MAC_ADDRf, mac_mask);

    if (soc_mem_field_valid(unit, MY_STATION_TCAMm, SOURCE_FIELDf)) {
        if (SOC_IS_KATANAX(unit)) {
            _BCM_TRX_MYSTA_FIELD32_MEMACC_SET(unit, &mask0, SOURCE_TYPEf, 1);
            _BCM_TRX_MYSTA_FIELD32_MEMACC_SET(unit, &mask0, 
                            SOURCE_TYPE_MASKf, 1);             
        }
        _BCM_TRX_MYSTA_FIELD32_MEMACC_SET(unit, &mask0, SOURCE_FIELDf,
                            port_mask);
        _BCM_TRX_MYSTA_FIELD32_MEMACC_SET(unit, &mask0, SOURCE_FIELD_MASKf,
                            port_mask);
    } else {
        _BCM_TRX_MYSTA_FIELD32_MEMACC_SET(unit, &mask0, ING_PORT_NUMf,
                            port_mask);
        _BCM_TRX_MYSTA_FIELD32_MEMACC_SET(unit, &mask0, ING_PORT_NUM_MASKf,
                            port_mask);
    }
    if (alt_index != NULL) {
        entry1 = entry0;
        ...
        if (soc_mem_field_valid(unit, MY_STATION_TCAMm, SOURCE_FIELDf)) {
            if (SOC_IS_KATANAX(unit)) {
                if ((1 << SOC_TRUNK_BIT_POS(unit)) & port){
                    /* trunk */
                    if (SOC_MEM_FIELD_VALID(unit, MY_STATION_TCAMm, SOURCE_TYPEf)) {  
                        _BCM_TRX_MYSTA_FIELD32_MEMACC_SET(unit, &entry0, 
                                        SOURCE_TYPEf, 1);
                        _BCM_TRX_MYSTA_FIELD32_MEMACC_SET(unit, &entry0, 
                                        SOURCE_TYPE_MASKf, 1);  
            ...
            _BCM_TRX_MYSTA_FIELD32_MEMACC_SET(unit, &entry0, SOURCE_FIELDf, port);
            _BCM_TRX_MYSTA_FIELD32_MEMACC_SET(unit, &entry0,
                            SOURCE_FIELD_MASKf, port_mask);
            
        } else {
            _BCM_TRX_MYSTA_FIELD32_MEMACC_SET(unit, &entry0, ING_PORT_NUMf, port);
            _BCM_TRX_MYSTA_FIELD32_MEMACC_SET(unit, &entry0,
    ...
    if (alt_index != NULL) {
        if (port == -1) {
            if (soc_mem_field_valid(unit, MY_STATION_TCAMm, SOURCE_FIELDf)) {
                if (SOC_IS_KATANAX(unit)) 
                {
                    _BCM_TRX_MYSTA_FIELD32_MEMACC_SET(unit, &mask1, SOURCE_TYPEf,
                                        0);              
                }
                _BCM_TRX_MYSTA_FIELD32_MEMACC_SET(unit, &mask1, SOURCE_FIELDf,
                                    0);
            } else {      
                _BCM_TRX_MYSTA_FIELD32_MEMACC_SET(unit, &mask1, ING_PORT_NUMf,
                                    0);
            }
        }
        ...
        _BCM_TRX_MYSTA_FIELD32_MEMACC_SET(unit, &entry2, VALIDf, 1);
    }
                           ...
                           int *l2_index)
{
    ...
    if (BCM_SUCCESS(rv)) {
        ...
        if (i == entry_words) {
            ...
            _BCM_TRX_MYSTA_FIELD32_MEMACC_SET(unit, entry, DISCARDf, 0);
            _BCM_TRX_MYSTA_FIELD32_MEMACC_SET(unit, entry, COPY_TO_CPUf,
                                0);
        }
        rv = soc_mem_write(unit, MY_STATION_TCAMm, MEM_BLOCK_ALL, index,
                           entry);
...
bcm_td_l2cache_myStation_set(int unit, int index, bcm_l2_cache_addr_t *l2caddr)
{
    ...
    entry_index = -1;
    entry = &info->my_station_shadow[index];
    if (!_BCM_TRX_MYSTA_FIELD32_MEMACC_GET(unit, entry, VALIDf)) {
    ...
    vlan = _BCM_TRX_MYSTA_FIELD32_MEMACC_GET(unit, entry, VLAN_IDf);
    _BCM_TRX_MYSTA_MAC_ADDR_MEMACC_GET(unit, entry, MAC_ADDRf, mac);
    if (_BCM_TRX_MYSTA_FIELD32_MEMACC_GET(unit, entry,
                            ING_PORT_NUM_MASKf) == 0) {
        port = -1;
    } else {
        port = _BCM_TRX_MYSTA_FIELD32_MEMACC_GET(unit, entry,
                                   ING_PORT_NUMf);
    }
...
bcm_td_l2cache_myStation_delete(int unit, int index)
{
    ...
    if (i == entry_words) {
        ...
        _BCM_TRX_MYSTA_FIELD32_MEMACC_SET(unit, entry, DISCARDf, 0);
        _BCM_TRX_MYSTA_FIELD32_MEMACC_SET(unit, entry, COPY_TO_CPUf, 0);
    }
    rv = soc_mem_write(unit, MY_STATION_TCAMm, MEM_BLOCK_ALL, index, entry);
                             ...
                             int *result_index)
{
    ...
    _BCM_TRX_MYSTA_FIELD32_MEMACC_SET(unit, &mask0, VALIDf, 1);
    _BCM_TRX_MYSTA_FIELD32_MEMACC_SET(unit, &mask0, VLAN_IDf,
                        l2caddr->vlan_mask);
    _BCM_TRX_MYSTA_MAC_ADDR_MEMACC_SET(unit, &mask0, MAC_ADDRf,
                         l2caddr->mac_mask);
    _BCM_TRX_MYSTA_FIELD32_MEMACC_SET(unit, &mask0, ING_PORT_NUMf,
                        l2caddr->src_port_mask);
    _BCM_TRX_MYSTA_FIELD32_MEMACC_SET(unit, &mask0, VLAN_ID_MASKf, 0xfff);
    _BCM_TRX_MYSTA_MAC_ADDR_MEMACC_SET(unit, &mask0, MAC_ADDR_MASKf,
                         mac_mask);
    _BCM_TRX_MYSTA_FIELD32_MEMACC_SET(unit, &mask0, ING_PORT_NUM_MASKf,
                        port_mask);
    for (i = 0; i < entry_words; i++) {
                           ...
                           bcm_port_t port, int tunnel_term)
{
    ...
    if (tunnel_term) {
        term_mask = info->my_station_tunnel_mask.entry_data;
    } else {
        term_mask = info->my_station_l3_mask.entry_data;
    }
    entry_words = soc_mem_entry_words(unit, MY_STATION_TCAMm);
    ...
    if (BCM_SUCCESS(rv)) {
        ...
        entry = &info->my_station_shadow[index];
        for (i = 0; i < entry_words; i++) {
            entry->entry_data[i] |= term_mask[i];
        }
        rv = soc_mem_write(unit, MY_STATION_TCAMm, MEM_BLOCK_ALL, index,
                           entry);
    } else if (rv == BCM_E_NOT_FOUND && index == -1) {
    ...
    sal_memset(entry, 0, sizeof(*entry));
    _BCM_TRX_MYSTA_FIELD32_MEMACC_SET(unit, entry, VALIDf, 1);
    _BCM_TRX_MYSTA_FIELD32_MEMACC_SET(unit, entry, VLAN_IDf, vlan);
    _BCM_TRX_MYSTA_FIELD32_MEMACC_SET(unit, entry, VLAN_ID_MASKf, 0xfff);
    _BCM_TRX_MYSTA_MAC_ADDR_MEMACC_SET(unit, entry, MAC_ADDRf, mac);
    sal_memset(&mac_mask, 0xff, sizeof(mac_mask));
    _BCM_TRX_MYSTA_MAC_ADDR_MEMACC_SET(unit, entry, MAC_ADDR_MASKf,
                         mac_mask);
    if (port != -1) {
        if (soc_mem_field_valid(unit, MY_STATION_TCAMm, SOURCE_FIELDf))
        {
            if (SOC_IS_KATANAX(unit))
            {
                SOC_IF_ERROR_RETURN(soc_mem_field32_fit(unit, MY_STATION_TCAMm, 
                            SOURCE_FIELD_MASKf,
                            1 << (SOC_TRUNK_BIT_POS(unit) - 1 )));
                if ((1 << SOC_TRUNK_BIT_POS(unit)) & port){
                    /* trunk */
                    if (SOC_MEM_FIELD_VALID(unit, MY_STATION_TCAMm, SOURCE_TYPEf)) {
                        _BCM_TRX_MYSTA_FIELD32_MEMACC_SET(unit, entry, 
                                            SOURCE_TYPEf, 1);
                        _BCM_TRX_MYSTA_FIELD32_MEMACC_SET(unit, entry, 
                                            SOURCE_TYPE_MASKf, 1);
                        port &= ~(1 << (SOC_TRUNK_BIT_POS(unit)));
            ...
            _BCM_TRX_MYSTA_FIELD32_MEMACC_SET(unit, entry, SOURCE_FIELDf,
                                port);
            port_mask = (1 << soc_mem_field_length(unit, MY_STATION_TCAMm,
                                                   SOURCE_FIELDf)) - 1;
            _BCM_TRX_MYSTA_FIELD32_MEMACC_SET(unit, entry, SOURCE_FIELD_MASKf,
                            port_mask);   
        } else {
            _BCM_TRX_MYSTA_FIELD32_MEMACC_SET(unit, entry, ING_PORT_NUMf,
                                port);
                              ...
                              bcm_port_t port, int tunnel_term)
{
    ...
    if (tunnel_term) {
        mask = info->my_station_l3_mask.entry_data;
        mask_remove = info->my_station_tunnel_mask.entry_data;        
    } else {
        mask = info->my_station_tunnel_mask.entry_data;
        mask_remove = info->my_station_l3_mask.entry_data;
    }
    entry_words = soc_mem_entry_words(unit, MY_STATION_TCAMm);
    ...
    if (BCM_SUCCESS(rv)) {
        ...
        for (i = 0; i < entry_words; i++) {
            if (entry->entry_data[i] & mask_remove[i]) {
                break;
            }
        }
        if (i == entry_words) {
        ...
        if (i == entry_words) {
            ...
            for (i = 0; i < entry_words; i++) {
                entry->entry_data[i] &= ~mask_remove[i];
            }
        }
        rv = soc_mem_write(unit, MY_STATION_TCAMm, MEM_BLOCK_ALL, index,
                           entry);
...
bcm_td_metro_myStation_delete_all(int unit)
{
    ...
    _BCM_TRX_MYSTA_FIELD32_MEMACC_SET(unit, &valid_mask, VALIDf, 1);
...
/*
 * Function:
 *      _bcm_vlan_vfi_memebershipship_profile_entry_op
 * Purpose:
...
int _bcm_vlan_vfi_memebershipship_profile_entry_op(int unit,
                                                   void **entries, 
                                                   int entries_per_set,
                                                   int add,
                                                   int egress,
...
/*
 * Function:
 *      _bcm_vlan_vfi_memebership_profile_mem_reference
 * Purpose:
 ...
 *      BCM_E_XXX
 */
int _bcm_vlan_vfi_memebership_profile_mem_reference(int unit,
                                                    int index,
                                                    int entries_per_set,
                                                    int egress)
{
