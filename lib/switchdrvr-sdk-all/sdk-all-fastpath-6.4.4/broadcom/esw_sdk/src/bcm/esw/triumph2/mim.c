    ...
    *is_local = 0;
    ...
    if (soc_feature(unit, soc_feature_mim_bvid_insertion_control)) {
        ...
        if (soc_reg_field_get(unit, EGR_CONFIGr, regval,
            ...
            egr_nh_info.dvp_is_network = (mim_port->flags & 
                    BCM_MIM_PORT_TYPE_BACKBONE) ? 1 : 0;
        }
    }
#endif
...
bcm_tr2_mim_port_delete_all(int unit, bcm_vpn_t vpn)
{
    ...
    for (vp = 0; vp < num_vp; vp++) { 
        ...
        if (soc_SOURCE_VPm_field32_get(unit, psvp, ENTRY_TYPEf) != 0) {
            /* Network ports do not have VFI set in SOURCE_VP table and so, it
             * should not be checked. */
            if (soc_feature(unit, soc_feature_multiple_split_horizon_group)) {
                network_port = soc_SOURCE_VPm_field32_get(unit, psvp, NETWORK_GROUPf);
            } else {
                network_port = soc_SOURCE_VPm_field32_get(unit, psvp, NETWORK_PORTf);
            }
            ...
            else if(vfi == soc_SOURCE_VPm_field32_get(unit, psvp, VFIf)) { 
                rv = _bcm_tr2_mim_port_delete(unit, vpn, vp); 
            }
            if (rv < 0) { 
                    goto done; 
    ...
    *port_count = 0;
    ...
    for (vp = 0; vp < num_vp; vp++) {
        ...
        if (soc_SOURCE_VPm_field32_get( unit, &svp, ENTRY_TYPEf) == 1) {
             ...
             * should not be checked. */
            if (soc_feature(unit, soc_feature_multiple_split_horizon_group)) {
                network_port = soc_SOURCE_VPm_field32_get(unit, &svp, NETWORK_GROUPf);
            } else {
            ...
            else if(vfi == soc_SOURCE_VPm_field32_get(unit, &svp, VFIf)){ 
                rv = _bcm_tr2_mim_port_get(unit, vpn, vp, 
                                        &port_array[*port_count]);
            }
