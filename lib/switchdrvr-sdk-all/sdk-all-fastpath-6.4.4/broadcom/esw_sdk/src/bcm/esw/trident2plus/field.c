        ...
        uint32 stat_mode_id, int8 *flex_stat_pool_id)
{
    ...
    if (BCM_SUCCESS(rv)) {
        sal_memcpy (flex_stat_pool_id, &pool_id, sizeof (int8));
    }
    return rv;
}
        ...
        for (idx = 0; idx < f_st->nstat; idx++) {
            ...
            if (BCM_FAILURE(rv)) {
                ...
                if (0 == (((_BCM_FIELD_STAGE_LOOKUP == f_st->stage_id)
                                || (_BCM_FIELD_STAGE_INGRESS == f_st->stage_id))
                            && (BCM_E_NOT_FOUND == rv))) {
                    return rv;
                }
            } else {
                                   ...
                                   _field_oam_action_recover_t *oam_action_hw)
{
    ...
    while (oam_stat_node != NULL) {
        for (idx = 0; idx < 3; idx++) {
            if ((oam_stat_node->flex_pool_id == oam_action_hw[idx].flex_pool_id)
                    && (oam_action_hw[idx].action != 0)) {
                /* Set Pool instance */
