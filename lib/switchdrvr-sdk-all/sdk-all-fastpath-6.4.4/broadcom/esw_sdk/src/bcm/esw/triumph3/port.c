...
int bcm_tr3_port_drain_cells(int unit, int port)
{
    ...
    /* Disable Pause*/
    rv = MAC_PAUSE_GET(macd, unit, port, &pause_tx, &pause_rx);
    if (!BCM_SUCCESS(rv)) {
        PORT_UNLOCK(unit);
        return rv;
    }
    rv = MAC_PAUSE_SET(macd, unit, port, pause_tx, 0);
    if (!BCM_SUCCESS(rv)) {
        PORT_UNLOCK(unit);
        return rv;
    }
    /* Disable PFC */
    rv = MAC_CONTROL_GET(macd, unit, port, SOC_MAC_CONTROL_PFC_RX_ENABLE, &pfc_rx);
    if (!BCM_SUCCESS(rv)) {
        goto reconfigure3;
    }
    rv = MAC_CONTROL_SET(macd, unit, port, SOC_MAC_CONTROL_PFC_RX_ENABLE, 0);
    if (!BCM_SUCCESS(rv)) {
        goto reconfigure3;
    }
    /* Disable LLFC */
    rv = MAC_CONTROL_GET(macd, unit, port, SOC_MAC_CONTROL_LLFC_RX_ENABLE, &llfc_rx);
    if (!BCM_SUCCESS(rv)) {
        goto reconfigure2;
    }
    rv = MAC_CONTROL_SET(macd, unit, port, SOC_MAC_CONTROL_LLFC_RX_ENABLE, 0);
    if (!BCM_SUCCESS(rv)) {
        goto reconfigure2;
    }
...
reconfigure1:
    /* Set llfc config back */
    MAC_CONTROL_SET(macd, unit, port, SOC_MAC_CONTROL_LLFC_RX_ENABLE, llfc_rx);
reconfigure2:
    /* Set pfc config back */
    MAC_CONTROL_SET(macd, unit, port, SOC_MAC_CONTROL_PFC_RX_ENABLE, pfc_rx);
reconfigure3:
    MAC_PAUSE_SET(macd, unit, port, pause_tx, pause_rx);
