...
cmd_ppd_api_diag_oam_counters(int unit, args_t *a) {
    ...
    if (next_arg && (!sal_strncasecmp(next_arg, "id=", 3))) {
        ...
        return arad_pp_oam_diag_print_oamp_counter(unit);
    } else {
        return CMD_USAGE;
    }
...
cmd_ppd_api_diag_oam_em(int unit, args_t *a) {
    ...
    if (next_arg && (!sal_strncasecmp(next_arg, "lif=", 4))) {
         ...
         if (arad_pp_oam_diag_print_em(unit,lif) !=0 ) {
             cli_out("Diag failed.\n");
         }
    } else {
        return CMD_USAGE;
...
cmd_ppd_api_diag_oam_ak(int unit, args_t *a) {
    ...
    ARAD_PP_OAM_ACTION_KEY_PARAMS key_params = {0};
    parse_table_t    pt;

    /* Parse input */
    parse_table_init(unit, &pt);
    parse_table_add(&pt, "lif", PQ_INT, 0, &key_params.lif, NULL);
    parse_table_add(&pt, "mdl", PQ_INT, 0, &key_params.level, NULL);
    parse_table_add(&pt, "op",  PQ_INT, 0, &key_params.opcode, NULL);
    parse_table_add(&pt, "ing", PQ_BOOL, 0, &key_params.ing, NULL);
    parse_table_add(&pt, "bfd", PQ_BOOL, 0, &key_params.bfd, NULL);
    if (SOC_IS_JERICHO(unit)) {
        parse_table_add(&pt, "ydv", PQ_BOOL, 0, &key_params.your_disc, NULL);
    }

    if (parse_arg_eq(a, &pt) < (SOC_IS_JERICHO(unit) ? 6 : 5)) {
        return CMD_USAGE;
    }

    if (arad_pp_oam_diag_print_ak(unit, &key_params) != 0) {
        cli_out("Diag failed.\n");
    }
...
cmd_ppd_api_diag_oam_debug(int unit, args_t *a) {
    ...
    return arad_pp_oam_diag_print_debug(unit, is_cfg, mode);
#else /*LINK_ARAD_LIBRARIES*/
    cli_out("Diag unsupported when compiling without LINK_ARAD_LIBRARIES");
    return 0;
#endif /*LINK_ARAD_LIBRARIES*/
...
cmd_ppd_api_diag_oam_oam_id(int unit, args_t *a) {
    ...
    return arad_pp_oam_diag_print_oam_id(unit);
...
void print_oam_usage(int unit) {
    char cmd_dpp_diag_oam_usage[] =
       ...
       "\nAction_Key -\tDisplay OAM-1/2 search key."
       "\n\tParameters:"
       "\n\t\tLIF=<LIF> Key for O-EM-1 entry"
       "\n\t\tMDL=<Level> Level of the packet"
       "\n\t\tOP=<op-code> OAM Op-Code"
       "\n\t\tING=<yes/no> Is classification in Ingress?"
       "\n\t\tBFD=<yes/no> Is packet BFD?"
    ...
    cli_out(cmd_dpp_diag_oam_usage,
            SOC_IS_JERICHO(unit) ? "\n\t\tYDV=<yes/no> Is Your-Disc-Valid set?" : ""
            );
}
