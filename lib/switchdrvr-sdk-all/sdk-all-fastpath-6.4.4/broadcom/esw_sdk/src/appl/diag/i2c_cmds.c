...
baseboards[] = {
    ...
    { G40, BB_VOLT | BB_CURR,
     "Katana2 Platform" },
    { G41, BB_VOLT | BB_REGULATOR | BB_SEQ,
    ...
    { G45, BB_VOLT | BB_REGULATOR,
     "Greyhound/Elkhound 25x25 QSGMII Platform"},
    { G46, BB_VOLT | BB_REGULATOR,
     "Greyhound/Ranger2 27x27 Platform"},
    { G47, BB_VOLT | BB_REGULATOR,
...
typedef enum {    
    /* core voltage working range from 0.85V - 1.05V*/
    DAC_CORE = 0,
    /* 10% range */
    DAC_10P_3P3 = 1,
    DAC_10P_2P5 = 2,
    ...
    DAC_10P_1P0 = 5,
    DAC_MAX_TYPE = 6
}regulator_dac_param_type_e;

/* regulator voltage range in {uV_max, uV_min} */
static 
i2c_regulator_volt_dac_param_t dac_param[DAC_MAX_TYPE] = {    
   {1050000, 850000},   /* DAC_CORE */
   {3630000, 2970000},  /* DAC_10P_3P3  */
   {2750000, 2250000},  /* DAC_10P_2P5  */
   {1980000, 1620000},  /* DAC_10P_1P8  */
   {1650000, 1350000},  /* DAC_10P_1P5  */
   {1100000, 900000},   /* DAC_10P_1P0  */
};

static i2c_regulator_volt_config_t
bb_regulator_config[NUM_BOARD_TYPES][MAX_VOLTAGES_PER_BOARD] = {
    {{0}},  /* G5 */
    {{0}},  /* G13 */
    {{0}},  /* G15 */
    {{0}},  /* G16 */
    ...
    {{0}},  /* G40 */        
    /* G41 */
    {{"3.3V", 0, I2C_LTC3880_60, I2C_LTC3880_60, NULL, &dac_param[DAC_10P_3P3]},
     {"2.5V", 1, I2C_LTC3880_60, I2C_LTC3880_60, NULL, &dac_param[DAC_10P_2P5]},
     {"1.8V", 0, I2C_LTC3880_61, I2C_LTC3880_61, NULL, &dac_param[DAC_10P_1P8]},
     {"1.5V", 1, I2C_LTC3880_61, I2C_LTC3880_61, NULL, &dac_param[DAC_10P_1P5]},
     {"Core", 0, I2C_LTC3880_62, I2C_LTC3880_62, NULL, &dac_param[DAC_CORE]},
     {"Analog", 1, I2C_LTC3880_62, I2C_LTC3880_62, NULL, &dac_param[DAC_10P_1P0]},
     {"3.3V(EXT_IO)", 0, I2C_LTC3880_64, NULL, NULL, NULL},
     {"5V(EXT_IO)", 1, I2C_LTC3880_64, NULL, NULL, NULL},
     {"DDR_CORE", 0, I2C_LTC3880_65, I2C_LTC3880_65, NULL, &dac_param[DAC_10P_1P0]},
...
/* MAP of regulator config table lengths.
 * One per board, each one is the number of table entries in the
 * table above.
 * INDEXED_BY_BOARD_TYPE
 */
int bb_regulator_config_len[NUM_BOARD_TYPES] = {
    0,  /* G5 */
    0,  /* G13 */
    0,  /* G15 */
    0,  /* G16 */
...
STATIC int
bb_regulator_volt_init(int unit, regulator_config_t *rcfg)
{
    int rv = SOC_E_NONE;
    ...
    board_type = soc_board_idx;
    if (board_type < 0) {
        return SOC_E_UNAVAIL;
    }

    if (bb_regulator_config_len[board_type] == 0) {
        return SOC_E_UNAVAIL;
    }
    vcfgs = bb_regulator_config[board_type];
    vcfgs_num = bb_regulator_config_len[board_type];

    volt = sal_alloc(sizeof(regulator_volt_t), "bb_regulator_volt");
    if (volt != NULL) {
...
STATIC int
bb_regulator_volt_deinit(int unit, regulator_config_t *rcfg)
{
...
STATIC int
bb_regulator_callback(int unit, regulator_func_t func,
                      regulator_type_t type, 
                      regulator_config_t *rcfg)
{
    int rv = SOC_E_PARAM;
    if (type == REGULATOR_TYPE_VOLT) {
        if (func == REGULATOR_FUNC_INIT) {
            rv = bb_regulator_volt_init(unit, rcfg);
        } else if (func == REGULATOR_FUNC_DEINIT) {
            rv = bb_regulator_volt_deinit(unit, rcfg);
        }
    }
    return rv;
}
 ...
 */
STATIC int
bb_regulator_voltage(int unit, int bx, char *source, char* value)
{
    int rv;
    void *vcfg;
    ...
    rv = soc_regulator_init(unit, REGULATOR_TYPE_VOLT, 
                            bb_regulator_callback, &hndl);
    if (rv != SOC_E_NONE) {
        cli_out("gh soc_regulator_init for REGULATOR_TYPE_VOLT failed (%s)\n", 
                bcm_errmsg(rv));
        return CMD_FAIL;
    }
    ...
    if (source == NULL) {
        ...
        for (i=0; i<cfg_num; i++) {
            rv = soc_regulator_get_voltcfg_and_name_by_index(unit, hndl, i, 
                                                            &vcfg, &name);
            if (rv != SOC_E_NONE) {
                cli_out("failed to get index %d of volt config\n", i);
                goto err_cmd_fail;
            }
    ...
    (void)soc_regulator_deinit(unit, hndl);
    return CMD_OK;
err_cmd_fail:
    return CMD_FAIL;
err_cmd_usage:
...
#undef BB_DEBUG
    ...
    if(!sal_strncasecmp(function,"voltage",sal_strlen(function))) {
        ...
        if( baseboard->bbcmd_flags & BB_REGULATOR) {
            return bb_regulator_voltage(unit, bx, source, value);
        }
