...
#if 0
    if (aux_mode != NULL) {
        /* Select AN Master lane for port0 and port1 as applicable configuration specified by the user */
        PHYMOD_IF_ERR_RETURN
            (_sesto_core_an_master_lane_port0_set(pm_acc, aux_mode->an_mst_lane_p0));
        PHYMOD_IF_ERR_RETURN
            (_sesto_core_an_master_lane_port1_set(pm_acc, aux_mode->an_mst_lane_p1));
    }
#endif
...
int _sesto_config_dut_mode_reg(const phymod_phy_access_t *pa, const phymod_phy_inf_config_t* config) 
{
    ...
    if (config->data_rate == SESTO_SPD_100G) {
        mode_ctrl0.fields.mode_falcon_line = 1;
    }
...
int _sesto_interface_set(const phymod_access_t *pa, const phymod_phy_inf_config_t* config)
{
    ...
    if (intf == phymodInterfaceSR4 || intf == phymodInterfaceLR4 ||/* intf == phymodInterfaceER4 ||*/
   ...
   /* } else if (intf == phymodInterfaceCAUI4 || intf == phymodInterfaceVSR) {
        ...
        dfe_en = 1;
        dfe_lp_en = 1; */
    } else if (intf == phymodInterfaceXLAUI2) {
        media_type = 0;
        dfe_en = 0;
        dfe_lp_en = 0;
    }
...
int _sesto_phy_interface_config_set(const phymod_phy_access_t* pa, uint32_t flags, const phymod_phy_inf_config_t* config)
{
    ...
    /* Configure DUT MODE */
    PHYMOD_IF_ERR_RETURN(
        _sesto_config_dut_mode_reg(pa, config));
    
    /* Enable UDMS for non-an MODE */
    PHYMOD_DEBUG_VERBOSE(("Configuring UDMS\n"));
    PHYMOD_IF_ERR_RETURN(
        _sesto_udms_config(pa, config, SESTO_AN_NONE));
    ...
    if ((config->data_rate != cfg.data_rate) && (config->ref_clock != cfg.ref_clock)) {
        ...
        PHYMOD_IF_ERR_RETURN(
            _sesto_config_pll_div(&pa->access, *config));
    }
    PHYMOD_USLEEP(100000);

    /* Configure Interface */
    PHYMOD_IF_ERR_RETURN(
        _sesto_interface_set(&pa->access, config));
...
int _sesto_get_pcs_link_status(const phymod_access_t *pa, uint32_t *link_sts)
{
    ...
    *link_sts = 0xffff;
    PHYMOD_IF_ERR_RETURN(
         _sesto_pcs_link_monitor_enable_set(pa, 0));
...
int _sesto_autoneg_set(const phymod_access_t* pa, const phymod_autoneg_control_t* an)
{
    ...
    if ((config.data_rate == SESTO_SPD_10G || config.data_rate == SESTO_SPD_20G) ||
        (config.data_rate == SESTO_SPD_100G && aux_mode->gearbox_100g_inverse_mode)) {
        PHYMOD_DEBUG_ERROR(("Invalid config for CL73, AN does not support 10G, 20G and 100G Inverse mode\n"));
        return PHYMOD_E_CONFIG;
    }
    ...
    if (an->an_mode == phymod_AN_MODE_CL73) {
        ...
        if (!aux_mode->pass_thru && config.data_rate != SESTO_SPD_100G) {
            ...
            if (lane_mask & 0xF) {
                ...
                } else if((an_mst_sel.data & 0xF0) >= 7) {
                    SESTO_CHIP_FIELD_WRITE(pa, DP_MER_AN_LANE_SELECT,
                                           mer_an_lane_sel4to7, (((an_mst_sel.data >> 4) & 0xF) - 4));
                } else {
                    PHYMOD_DEBUG_ERROR(("Invalid AN master lane, should be b/w 0 to 3 or 4 to 7\n"));
                }
                if (an->enable) {
            ...
            if (ip == SESTO_FALCON_CORE) {
                ...
                if ((an_mst_sel.data & 0xF) <= 1) {
                    ...
                    /* AN Enable */
                    PHYMOD_DEBUG_ERROR(("Invalid AN master lane, should be b/w 0 to 1 or 4 to 5\n"));
                    if (an->enable) {
                        gpreg11.fields.an_enable |= 0x1;
                    } else {
                        gpreg11.fields.an_enable &= ~(0x1);
    ...
    PHYMOD_IF_ERR_RETURN( _sesto_fw_enable(pa, 1)); 
...
int _sesto_core_an_master_lane_port0_set(const phymod_access_t *pa, uint32_t an_mst_lane_p0)
{
    ...
    if (an_mst_lane_p0 <= 5) {
        ...
        an_mst_sel.data |= an_mst_lane_p0;
        WRITE_SESTO_PMA_PMD_REG(pa, SES_GEN_CNTRLS_MICRO_SYNC_4_ADR, an_mst_sel.data);
    } else {
        PHYMOD_DEBUG_ERROR(("Invalid AN Master lane config, should be between 0 to 5 depends on mode configuration\n"));
    }
...
int _sesto_core_an_master_lane_port1_set(const phymod_access_t *pa, uint32_t an_mst_lane_p1)
{
    ...
    if ((an_mst_lane_p1 <= 7) && (an_mst_lane_p1 >= 4)) {
        READ_SESTO_PMA_PMD_REG(pa, SES_GEN_CNTRLS_MICRO_SYNC_4_ADR, an_mst_sel.data);
        an_mst_sel.data &= 0xFF0F;
        an_mst_sel.data |= (an_mst_lane_p1 << 4);
        WRITE_SESTO_PMA_PMD_REG(pa, SES_GEN_CNTRLS_MICRO_SYNC_4_ADR, an_mst_sel.data);
    } else {
        PHYMOD_DEBUG_ERROR(("Invalid AN Master lane config, should be between 4 to 7 depends on mode configuration\n"));
    }
...
int _sesto_phy_repeater_mode_get(const phymod_access_t *pa, uint32_t *enable)
{
    ...
    if (ip == SESTO_FALCON_CORE) { /* Lane side is 10G SerDes */
        *enable = !((gpreg11.fields.dis_rptr_20g_to_10g >> port) & 0x1);
    } else {
        *enable = !((gpreg11.fields.dis_rptr_10g_to_20g >> port) & 0x1);
    }
...
int _sesto_phy_cfg_gpio_pin_set(const phymod_access_t *pa, phymod_gpio_ctrl_args_t *gpio_ctrl)
{
    ...
    if (gpio_ctrl->gpio_pin_number > 3) {
        PHYMOD_RETURN_WITH_ERR(PHYMOD_E_PARAM,
               (_PHYMOD_MSG("Invalid GPIO pin selected, Valid GPIO pins are GPIO0-GPIO3..")));
    }

    READ_SESTO_PMA_PMD_REG(pa,
                          (SES_PAD_CTRL_GPIO_0_CONTROL_ADR + (gpio_ctrl->gpio_pin_number * 2)),
                           pad_ctrl_gpio_ctrl.data);

    pad_ctrl_gpio_ctrl.fields.gpio_0_oeb = (gpio_ctrl->cfg_direction) ? 1 : 0;
    switch (gpio_ctrl->cfg_pull) {
        case 0:
            pad_ctrl_gpio_ctrl.fields.gpio_0_pup = 0;
            pad_ctrl_gpio_ctrl.fields.gpio_0_pdn = 1;
        break;
        case 1:
            pad_ctrl_gpio_ctrl.fields.gpio_0_pup = 1;
            pad_ctrl_gpio_ctrl.fields.gpio_0_pdn = 0;
        break;
        case 2:
            pad_ctrl_gpio_ctrl.fields.gpio_0_pup = 0;
            pad_ctrl_gpio_ctrl.fields.gpio_0_pdn = 0;
    ...
    pad_ctrl_gpio_ctrl.fields.gpio_0_out_frcval = gpio_ctrl->pin_value;
    WRITE_SESTO_PMA_PMD_REG(pa,
                          (SES_PAD_CTRL_GPIO_0_CONTROL_ADR + (gpio_ctrl->gpio_pin_number * 2)),
                           pad_ctrl_gpio_ctrl.data);
...
int _sesto_phy_cfg_gpio_pin_get(const phymod_access_t *pa, phymod_gpio_ctrl_args_t *gpio_ctrl)
{
    ...
    if (pad_ctrl_gpio_ctrl.fields.gpio_0_pup) {
        gpio_ctrl->cfg_pull = 1;
    } else if (pad_ctrl_gpio_ctrl.fields.gpio_0_pdn) {
        gpio_ctrl->cfg_pull = 0;
    } else {
        gpio_ctrl->cfg_pull = 2;
    }

    READ_SESTO_PMA_PMD_REG(pa,
                          (SES_PAD_CTRL_GPIO_0_STATUS_ADR + (gpio_ctrl->gpio_pin_number * 2)),
                           pad_sts_gpio.data);
    gpio_ctrl->pin_value = pad_sts_gpio.fields.gpio_0_din;
...
int _sesto_phy_modctrl_cfg_qsfp_linecard_set(const phymod_access_t *pa, phymod_modctrl_qsfp_io_pins_t *qsfp_io_pins)
{
    ...
    SES_PAD_CTRL_GPIO_0_CONTROL_TYPE_T gpio_ctrl;
    SES_PAD_CTRL_MOD_TX_DIS_LOPWR2_CONTROL_TYPE_T lp_mod_0;
    SES_PAD_CTRL_MOD_RX_LOS_ABS2_CONTROL_TYPE_T mod_prsl_0;
    SES_PAD_CTRL_MOD_LOPWR_CONTROL_TYPE_T lp_mod_1;
    SES_PAD_CTRL_MOD_ABS_CONTROL_TYPE_T mod_prsl_1;
    SES_PAD_CTRL_MOD_RSTN_CONTROL_TYPE_T resetl_1;
    SES_PAD_CTRL_MOD_INTRN_CONTROL_TYPE_T intl_1;

    PHYMOD_MEMSET(&gen_ctrl, 0 , sizeof(SES_GEN_CNTRLS_GEN_CONTROL3_TYPE_T));
    PHYMOD_MEMSET(&gpio_ctrl, 0 , sizeof(SES_PAD_CTRL_GPIO_0_CONTROL_TYPE_T));
    PHYMOD_MEMSET(&lp_mod_0, 0 , sizeof(SES_PAD_CTRL_MOD_TX_DIS_LOPWR2_CONTROL_TYPE_T));
    PHYMOD_MEMSET(&mod_prsl_0, 0 , sizeof(SES_PAD_CTRL_MOD_RX_LOS_ABS2_CONTROL_TYPE_T));
    PHYMOD_MEMSET(&lp_mod_1, 0 , sizeof(SES_PAD_CTRL_MOD_LOPWR_CONTROL_TYPE_T));
    PHYMOD_MEMSET(&mod_prsl_1, 0 , sizeof(SES_PAD_CTRL_MOD_ABS_CONTROL_TYPE_T));
    PHYMOD_MEMSET(&resetl_1, 0 , sizeof(SES_PAD_CTRL_MOD_RSTN_CONTROL_TYPE_T));
    PHYMOD_MEMSET(&intl_1, 0 , sizeof(SES_PAD_CTRL_MOD_INTRN_STATUS_TYPE_T));
    ...
    if (lane_mask <= 0xF) {
        ...
        /* Configure LPMode as output pin and force the user specified value using SES_PAD_CTRL_mod_tx_dis_lopwr2*/
        if (qsfp_io_pins->lpmod.enable) {
            READ_SESTO_PMA_PMD_REG(pa, SES_PAD_CTRL_MOD_TX_DIS_LOPWR2_CONTROL_ADR, lp_mod_0.data);
            lp_mod_0.fields.mod_tx_dis_lopwr2_oeb = 0;
            lp_mod_0.fields.mod_tx_dis_lopwr2_ibof = 1;
            lp_mod_0.fields.mod_tx_dis_lopwr2_out_frcval = qsfp_io_pins->lpmod.value;
            WRITE_SESTO_PMA_PMD_REG(pa, SES_PAD_CTRL_MOD_TX_DIS_LOPWR2_CONTROL_ADR, lp_mod_0.data);
        }

        /* Configure ResetL as output pin and force the user specified value using SES_PAD_CTRL_gpio_2 */ 
        if (qsfp_io_pins->resetl.enable) {
            READ_SESTO_PMA_PMD_REG(pa, (SES_PAD_CTRL_GPIO_0_CONTROL_ADR + 4), gpio_ctrl.data);
            gpio_ctrl.fields.gpio_0_oeb = 0;
            gpio_ctrl.fields.gpio_0_ibof = 1;
            gpio_ctrl.fields.gpio_0_out_frcval = qsfp_io_pins->resetl.value;
            WRITE_SESTO_PMA_PMD_REG(pa, (SES_PAD_CTRL_GPIO_0_CONTROL_ADR + 4), gpio_ctrl.data);
        }  

        /* Configure IntL as input pin and force the user specified value using SES_PAD_CTRL_gpio_0 */
        if (qsfp_io_pins->intl.enable) {
            /* Input Overloading en_mod_intrn_gpio_0 = 1 for GIOP_0 */
            SESTO_CHIP_FIELD_WRITE(pa, SES_CTRL_GENERAL_PURPOSE_IO_CTRL1, en_mod_intrn_gpio_0, 1);

            READ_SESTO_PMA_PMD_REG(pa, SES_PAD_CTRL_GPIO_0_CONTROL_ADR, gpio_ctrl.data);
            gpio_ctrl.fields.gpio_0_oeb = 1;
            gpio_ctrl.fields.gpio_0_ibof = 0;
            gpio_ctrl.fields.gpio_0_ind = 0;
            gpio_ctrl.fields.gpio_0_out_frcval = qsfp_io_pins->intl.value;
            WRITE_SESTO_PMA_PMD_REG(pa, SES_PAD_CTRL_GPIO_0_CONTROL_ADR, gpio_ctrl.data);
        }

        /* Configure ModselL as output pin and force the user specified value using SES_PAD_CTRL_gpio_3 */
        if (qsfp_io_pins->mod_sell.enable) {
            READ_SESTO_PMA_PMD_REG(pa, (SES_PAD_CTRL_GPIO_0_CONTROL_ADR + 6), gpio_ctrl.data);
            gpio_ctrl.fields.gpio_0_oeb = 0;
            gpio_ctrl.fields.gpio_0_ibof = 1;
            gpio_ctrl.fields.gpio_0_out_frcval = qsfp_io_pins->mod_sell.value;
            WRITE_SESTO_PMA_PMD_REG(pa, (SES_PAD_CTRL_GPIO_0_CONTROL_ADR + 6), gpio_ctrl.data);
        }

        /* Configure ModPrsL as input pin and force the user specified value using SES_PAD_CTRL_mod_rx_los_abs2 */
        if (qsfp_io_pins->mod_prsl.enable) {
            READ_SESTO_PMA_PMD_REG(pa, SES_PAD_CTRL_MOD_RX_LOS_ABS2_CONTROL_ADR, mod_prsl_0.data);
            mod_prsl_0.fields.mod_rx_los_abs2_oeb = 1;
            mod_prsl_0.fields.mod_rx_los_abs2_ibof = 0;
            mod_prsl_0.fields.mod_rx_los_abs2_ind = 0;
            mod_prsl_0.fields.mod_rx_los_abs2_out_frcval = qsfp_io_pins->mod_prsl.value;
            WRITE_SESTO_PMA_PMD_REG(pa, SES_PAD_CTRL_MOD_RX_LOS_ABS2_CONTROL_ADR, mod_prsl_0.data);
        }
        ...
        /* Configure ModselL as output pin and force the user specified value using SES_PAD_CTRL_gpio_1 */
        if (qsfp_io_pins->mod_sell.enable) {
            READ_SESTO_PMA_PMD_REG(pa, (SES_PAD_CTRL_GPIO_0_CONTROL_ADR + 2), gpio_ctrl.data);
            gpio_ctrl.fields.gpio_0_oeb = 0;
            gpio_ctrl.fields.gpio_0_ibof = 1;
            gpio_ctrl.fields.gpio_0_out_frcval = qsfp_io_pins->mod_sell.value;
            WRITE_SESTO_PMA_PMD_REG(pa, (SES_PAD_CTRL_GPIO_0_CONTROL_ADR + 2), gpio_ctrl.data);
        }

        /* Configure IntL as input pin and force the user specified value using SES_PAD_CTRL_mod_intrn_control */
        if (qsfp_io_pins->intl.enable) {
            READ_SESTO_PMA_PMD_REG(pa, SES_PAD_CTRL_MOD_INTRN_CONTROL_ADR, intl_1.data);
            intl_1.fields.mod_intrn_oeb = 1;
            intl_1.fields.mod_intrn_ibof = 0;
            intl_1.fields.mod_intrn_ind = 0;
            intl_1.fields.mod_intrn_out_frcval = qsfp_io_pins->intl.value;
            WRITE_SESTO_PMA_PMD_REG(pa, SES_PAD_CTRL_MOD_INTRN_CONTROL_ADR, intl_1.data);
        }

        /* Configure LPMode as output pin and force the user specified value using SES_PAD_CTRL_mod_lopwr */
        if (qsfp_io_pins->lpmod.enable) {
            READ_SESTO_PMA_PMD_REG(pa, SES_PAD_CTRL_MOD_LOPWR_CONTROL_ADR, lp_mod_1.data);
            lp_mod_1.fields.mod_lopwr_oeb = 0;
            lp_mod_1.fields.mod_lopwr_ibof = 1;
            lp_mod_1.fields.mod_lopwr_out_frcval = qsfp_io_pins->lpmod.value;
            WRITE_SESTO_PMA_PMD_REG(pa, SES_PAD_CTRL_MOD_LOPWR_CONTROL_ADR, lp_mod_1.data);
        }

        /* Configure ResetL as output pin and force the user specified value using SES_PAD_CTRL_mod_rstn */
        if (qsfp_io_pins->resetl.enable) {
            READ_SESTO_PMA_PMD_REG(pa, SES_PAD_CTRL_MOD_RSTN_CONTROL_ADR, resetl_1.data);
            resetl_1.fields.mod_rstn_oeb = 0;
            resetl_1.fields.mod_rstn_ibof = 1;
            resetl_1.fields.mod_rstn_out_frcval = qsfp_io_pins->resetl.value;
            WRITE_SESTO_PMA_PMD_REG(pa, SES_PAD_CTRL_MOD_RSTN_CONTROL_ADR, resetl_1.data);
        }

        /* Configure ModPrsL as input pin and force the user specified value using SES_PAD_CTRL_mod_abs*/
        if (qsfp_io_pins->mod_prsl.enable) {
            READ_SESTO_PMA_PMD_REG(pa, SES_PAD_CTRL_MOD_ABS_CONTROL_ADR, mod_prsl_1.data);
            mod_prsl_1.fields.mod_abs_oeb = 1;
            mod_prsl_1.fields.mod_abs_ibof = 0;
            mod_prsl_1.fields.mod_abs_ind = 0;
            mod_prsl_1.fields.mod_abs_out_frcval = qsfp_io_pins->mod_prsl.value;
            WRITE_SESTO_PMA_PMD_REG(pa, SES_PAD_CTRL_MOD_ABS_CONTROL_ADR, mod_prsl_1.data);
        }
    }
...
int _sesto_phy_modctrl_cfg_cfp_linecard_set(const phymod_access_t *pa, phymod_modctrl_cfp_io_pins_t *cfp_io_pins)
{
    uint16_t lane_mask = 0;
    SES_GEN_CNTRLS_GEN_CONTROL3_TYPE_T gen_ctrl;
    SES_PAD_CTRL_MOD_TX_DIS_LOPWR2_CONTROL_TYPE_T tx_dis;
    SES_PAD_CTRL_MOD_RX_LOS_ABS2_CONTROL_TYPE_T rx_los;
    SES_PAD_CTRL_MOD_LOPWR_CONTROL_TYPE_T mod_lopwr;
    SES_PAD_CTRL_MOD_ABS_CONTROL_TYPE_T mod_abs;
    SES_PAD_CTRL_MOD_RSTN_CONTROL_TYPE_T mod_rstn;
    SES_PAD_CTRL_MOD_INTRN_CONTROL_TYPE_T glb_alrmn;

    PHYMOD_MEMSET(&gen_ctrl, 0 , sizeof(SES_GEN_CNTRLS_GEN_CONTROL3_TYPE_T));
    PHYMOD_MEMSET(&tx_dis, 0 , sizeof(SES_PAD_CTRL_MOD_TX_DIS_LOPWR2_CONTROL_TYPE_T));
    PHYMOD_MEMSET(&rx_los, 0 , sizeof(SES_PAD_CTRL_MOD_RX_LOS_ABS2_CONTROL_TYPE_T));
    PHYMOD_MEMSET(&mod_lopwr, 0 , sizeof(SES_PAD_CTRL_MOD_LOPWR_CONTROL_TYPE_T));
    PHYMOD_MEMSET(&mod_abs, 0 , sizeof(SES_PAD_CTRL_MOD_ABS_CONTROL_TYPE_T));
    PHYMOD_MEMSET(&mod_rstn, 0 , sizeof(SES_PAD_CTRL_MOD_RSTN_CONTROL_TYPE_T));
    PHYMOD_MEMSET(&glb_alrmn, 0 , sizeof(SES_PAD_CTRL_MOD_INTRN_STATUS_TYPE_T));


    /* qsfp mode or legacy mode */
    READ_SESTO_PMA_PMD_REG(pa, SES_GEN_CNTRLS_GEN_CONTROL3_ADR, gen_ctrl.data);
    gen_ctrl.fields.qsfp_mode = 0;
    WRITE_SESTO_PMA_PMD_REG(pa, SES_GEN_CNTRLS_GEN_CONTROL3_ADR, gen_ctrl.data);
    ...
    /* Configure tx_dis as output pin and force the user specified value using SES_PAD_CTRL_mod_tx_dis_lopwr2*/
    if (cfp_io_pins->tx_dis.enable) {
        READ_SESTO_PMA_PMD_REG(pa, SES_PAD_CTRL_MOD_TX_DIS_LOPWR2_CONTROL_ADR, tx_dis.data);
        tx_dis.fields.mod_tx_dis_lopwr2_oeb = 0;
        tx_dis.fields.mod_tx_dis_lopwr2_ibof = 1;
        tx_dis.fields.mod_tx_dis_lopwr2_out_frcval = cfp_io_pins->tx_dis.value;
        WRITE_SESTO_PMA_PMD_REG(pa, SES_PAD_CTRL_MOD_TX_DIS_LOPWR2_CONTROL_ADR, tx_dis.data);
    }
    /* Configure rx_los as input pin and force the user specified value using SES_PAD_CTRL_mod_rx_los_abs2 */
    if (cfp_io_pins->rx_los.enable) {
        READ_SESTO_PMA_PMD_REG(pa, SES_PAD_CTRL_MOD_RX_LOS_ABS2_CONTROL_ADR, rx_los.data);
        rx_los.fields.mod_rx_los_abs2_oeb = 1;
        rx_los.fields.mod_rx_los_abs2_ibof = 0;
        rx_los.fields.mod_rx_los_abs2_ind = 0;
        rx_los.fields.mod_rx_los_abs2_out_frcval = cfp_io_pins->rx_los.value;
        WRITE_SESTO_PMA_PMD_REG(pa, SES_PAD_CTRL_MOD_RX_LOS_ABS2_CONTROL_ADR, rx_los.data);
    }
    /* Configure LPMode as output pin and force the user specified value using SES_PAD_CTRL_mod_lopwr */
    if (cfp_io_pins->mod_lopwr.enable) {
        READ_SESTO_PMA_PMD_REG(pa, SES_PAD_CTRL_MOD_LOPWR_CONTROL_ADR, mod_lopwr.data);
        mod_lopwr.fields.mod_lopwr_oeb = 0;
        mod_lopwr.fields.mod_lopwr_ibof = 1;
        mod_lopwr.fields.mod_lopwr_out_frcval = cfp_io_pins->mod_lopwr.value;
        WRITE_SESTO_PMA_PMD_REG(pa, SES_PAD_CTRL_MOD_LOPWR_CONTROL_ADR, mod_lopwr.data);
    }
    /* Configure mod_abs as input pin and force the user specified value using SES_PAD_CTRL_mod_abs*/
    if (cfp_io_pins->mod_abs.enable) {
        READ_SESTO_PMA_PMD_REG(pa, SES_PAD_CTRL_MOD_ABS_CONTROL_ADR, mod_abs.data);
        mod_abs.fields.mod_abs_oeb = 1;
        mod_abs.fields.mod_abs_ibof = 0;
        mod_abs.fields.mod_abs_ind = 0;
        mod_abs.fields.mod_abs_out_frcval = cfp_io_pins->mod_abs.value;
        WRITE_SESTO_PMA_PMD_REG(pa, SES_PAD_CTRL_MOD_ABS_CONTROL_ADR, mod_abs.data);
    }
    /* Configure mod_rstn as output pin and force the user specified value using SES_PAD_CTRL_mod_rstn */
    if (cfp_io_pins->mod_rstn.enable) {
        READ_SESTO_PMA_PMD_REG(pa, SES_PAD_CTRL_MOD_RSTN_CONTROL_ADR, mod_rstn.data);
        mod_rstn.fields.mod_rstn_oeb = 0;
        mod_rstn.fields.mod_rstn_ibof = 1;
        mod_rstn.fields.mod_rstn_out_frcval = cfp_io_pins->mod_rstn.value;
        WRITE_SESTO_PMA_PMD_REG(pa, SES_PAD_CTRL_MOD_RSTN_CONTROL_ADR, mod_rstn.data);
    }
    /* Configure glb_alrmn as input pin and force the user specified value using SES_PAD_CTRL_mod_intrn_control */
    if (cfp_io_pins->glb_alrmn.enable) {
        READ_SESTO_PMA_PMD_REG(pa, SES_PAD_CTRL_MOD_INTRN_CONTROL_ADR, glb_alrmn.data);
        glb_alrmn.fields.mod_intrn_oeb = 1;
        glb_alrmn.fields.mod_intrn_ibof = 0;
        glb_alrmn.fields.mod_intrn_ind = 0;
        glb_alrmn.fields.mod_intrn_out_frcval = cfp_io_pins->glb_alrmn.value;
        WRITE_SESTO_PMA_PMD_REG(pa, SES_PAD_CTRL_MOD_INTRN_CONTROL_ADR, glb_alrmn.data);
    }
...
int _sesto_phy_modctrl_cfg_qsfp_linecard_get(const phymod_access_t *pa, phymod_modctrl_qsfp_io_pins_t *qsfp_io_pins)
{
    uint16_t lane_mask = 0;
    SES_GEN_CNTRLS_GEN_CONTROL3_TYPE_T gen_ctrl;
    SES_PAD_CTRL_GPIO_0_STATUS_TYPE_T gpio_ctrl;
    SES_PAD_CTRL_MOD_TX_DIS_LOPWR2_STATUS_TYPE_T lp_mod_0;
    SES_PAD_CTRL_MOD_RX_LOS_ABS2_STATUS_TYPE_T mod_prsl_0;
    SES_PAD_CTRL_MOD_LOPWR_STATUS_TYPE_T lp_mod_1;
    SES_PAD_CTRL_MOD_ABS_STATUS_TYPE_T mod_prsl_1;
    SES_PAD_CTRL_MOD_RSTN_STATUS_TYPE_T resetl_1;
    SES_PAD_CTRL_MOD_INTRN_STATUS_TYPE_T intl_1;

    PHYMOD_MEMSET(&gen_ctrl, 0 , sizeof(SES_GEN_CNTRLS_GEN_CONTROL3_TYPE_T));
    PHYMOD_MEMSET(&gpio_ctrl, 0 , sizeof(SES_PAD_CTRL_GPIO_0_STATUS_TYPE_T));
    PHYMOD_MEMSET(&lp_mod_0, 0 , sizeof(SES_PAD_CTRL_MOD_TX_DIS_LOPWR2_STATUS_TYPE_T));
    PHYMOD_MEMSET(&mod_prsl_0, 0 , sizeof(SES_PAD_CTRL_MOD_RX_LOS_ABS2_STATUS_TYPE_T));
    PHYMOD_MEMSET(&lp_mod_1, 0 , sizeof(SES_PAD_CTRL_MOD_LOPWR_STATUS_TYPE_T));
    PHYMOD_MEMSET(&mod_prsl_1, 0 , sizeof(SES_PAD_CTRL_MOD_ABS_STATUS_TYPE_T));
    PHYMOD_MEMSET(&resetl_1, 0 , sizeof(SES_PAD_CTRL_MOD_RSTN_STATUS_TYPE_T));
    PHYMOD_MEMSET(&intl_1, 0 , sizeof(SES_PAD_CTRL_MOD_INTRN_STATUS_TYPE_T));
    ...
    if (lane_mask <= 0xF) {
        ...
        /* Read LPMode status */
        READ_SESTO_PMA_PMD_REG(pa, SES_PAD_CTRL_MOD_TX_DIS_LOPWR2_STATUS_ADR, lp_mod_0.data);
        qsfp_io_pins->lpmod.value = lp_mod_0.fields.mod_tx_dis_lopwr2_din;

        /* Read ModPrsL status */
        READ_SESTO_PMA_PMD_REG(pa, SES_PAD_CTRL_MOD_RX_LOS_ABS2_STATUS_ADR, mod_prsl_0.data);
        qsfp_io_pins->mod_prsl.value = mod_prsl_0.fields.mod_rx_los_abs2_din;

        /* Read IntL status */
        READ_SESTO_PMA_PMD_REG(pa, SES_PAD_CTRL_GPIO_0_STATUS_ADR, gpio_ctrl.data);
        qsfp_io_pins->intl.value = gpio_ctrl.fields.gpio_0_din;

        /* Read ResetL status */
        READ_SESTO_PMA_PMD_REG(pa, (SES_PAD_CTRL_GPIO_0_STATUS_ADR + 4), gpio_ctrl.data);
        qsfp_io_pins->resetl.value = gpio_ctrl.fields.gpio_0_din;

        /* Read ModselL status */
        READ_SESTO_PMA_PMD_REG(pa, (SES_PAD_CTRL_GPIO_0_STATUS_ADR + 6), gpio_ctrl.data);
        qsfp_io_pins->mod_sell.value = gpio_ctrl.fields.gpio_0_din;
    } else if (lane_mask <= 0xF0) {  /* QSFP module 1 */
        /* Active Low Select to read NVRAM for QSFP module 1 */
        READ_SESTO_PMA_PMD_REG(pa, SES_GEN_CNTRLS_GEN_CONTROL3_ADR, gen_ctrl.data);
        gen_ctrl.fields.qsfp_sel0b = 1;
        ...
        /* Read LPMode status */
        READ_SESTO_PMA_PMD_REG(pa, SES_PAD_CTRL_MOD_LOPWR_STATUS_ADR, lp_mod_1.data);
        qsfp_io_pins->lpmod.value = lp_mod_1.fields.mod_lopwr_din;

        /* Read ModPrsL status */
        READ_SESTO_PMA_PMD_REG(pa, SES_PAD_CTRL_MOD_ABS_STATUS_ADR, mod_prsl_1.data);
        qsfp_io_pins->mod_prsl.value = mod_prsl_1.fields.mod_abs_din;

        /* Read IntL status */
        READ_SESTO_PMA_PMD_REG(pa, SES_PAD_CTRL_MOD_INTRN_STATUS_ADR, intl_1.data);
        qsfp_io_pins->intl.value = intl_1.fields.mod_intrn_din;

        /* Read ResetL status */
        READ_SESTO_PMA_PMD_REG(pa, SES_PAD_CTRL_MOD_RSTN_STATUS_ADR, resetl_1.data);        
        qsfp_io_pins->resetl.value = resetl_1.fields.mod_rstn_din;

        /* ModselL status */
        READ_SESTO_PMA_PMD_REG(pa, (SES_PAD_CTRL_GPIO_0_STATUS_ADR + 2), gpio_ctrl.data);
        qsfp_io_pins->mod_sell.value = gpio_ctrl.fields.gpio_0_din;
    }
...
int _sesto_phy_modctrl_cfg_cfp_linecard_get(const phymod_access_t *pa, phymod_modctrl_cfp_io_pins_t *cfp_io_pins)
{
    uint16_t lane_mask = 0;
    SES_GEN_CNTRLS_GEN_CONTROL3_TYPE_T gen_ctrl;
    SES_PAD_CTRL_MOD_TX_DIS_LOPWR2_STATUS_TYPE_T tx_dis;
    SES_PAD_CTRL_MOD_RX_LOS_ABS2_STATUS_TYPE_T rx_los;
    SES_PAD_CTRL_MOD_LOPWR_STATUS_TYPE_T mod_lopwr;
    SES_PAD_CTRL_MOD_ABS_STATUS_TYPE_T mod_abs;
    SES_PAD_CTRL_MOD_RSTN_STATUS_TYPE_T mod_rstn;
    SES_PAD_CTRL_MOD_INTRN_STATUS_TYPE_T glb_alrmn;

    PHYMOD_MEMSET(&gen_ctrl, 0 , sizeof(SES_GEN_CNTRLS_GEN_CONTROL3_TYPE_T));
    PHYMOD_MEMSET(&tx_dis, 0 , sizeof(SES_PAD_CTRL_MOD_TX_DIS_LOPWR2_STATUS_TYPE_T));
    PHYMOD_MEMSET(&rx_los, 0 , sizeof(SES_PAD_CTRL_MOD_RX_LOS_ABS2_STATUS_TYPE_T));
    PHYMOD_MEMSET(&mod_lopwr, 0 , sizeof(SES_PAD_CTRL_MOD_LOPWR_STATUS_TYPE_T));
    PHYMOD_MEMSET(&mod_abs, 0 , sizeof(SES_PAD_CTRL_MOD_ABS_STATUS_TYPE_T));
    PHYMOD_MEMSET(&mod_rstn, 0 , sizeof(SES_PAD_CTRL_MOD_RSTN_STATUS_TYPE_T));
    PHYMOD_MEMSET(&glb_alrmn, 0 , sizeof(SES_PAD_CTRL_MOD_INTRN_STATUS_TYPE_T));
    ...
    /* Read TX_DIS status */
    READ_SESTO_PMA_PMD_REG(pa, SES_PAD_CTRL_MOD_TX_DIS_LOPWR2_STATUS_ADR, tx_dis.data);
    cfp_io_pins->tx_dis.value = tx_dis.fields.mod_tx_dis_lopwr2_din;

    /* Read MOD_ABS status */
    READ_SESTO_PMA_PMD_REG(pa, SES_PAD_CTRL_MOD_ABS_STATUS_ADR, mod_abs.data);
    cfp_io_pins->mod_abs.value = mod_abs.fields.mod_abs_din;

    /* Read RX_LOS status */
    READ_SESTO_PMA_PMD_REG(pa, SES_PAD_CTRL_MOD_RX_LOS_ABS2_STATUS_ADR, rx_los.data);
    cfp_io_pins->rx_los.value = rx_los.fields.mod_rx_los_abs2_din;

    /* Read MOD_LOPWR status */
    READ_SESTO_PMA_PMD_REG(pa, SES_PAD_CTRL_MOD_LOPWR_STATUS_ADR, mod_lopwr.data);
    cfp_io_pins->mod_lopwr.value = mod_lopwr.fields.mod_lopwr_din;

    /* Read GLB_ALRMN status */
    READ_SESTO_PMA_PMD_REG(pa, SES_PAD_CTRL_MOD_INTRN_STATUS_ADR, glb_alrmn.data);
    cfp_io_pins->glb_alrmn.value = glb_alrmn.fields.mod_intrn_din;

    /* Read MOD_RSTN status */
    READ_SESTO_PMA_PMD_REG(pa, SES_PAD_CTRL_MOD_RSTN_STATUS_ADR, mod_rstn.data);
    cfp_io_pins->mod_rstn.value = mod_rstn.fields.mod_rstn_din;

    return PHYMOD_E_NONE;
}


int _sesto_phy_module_write(const phymod_access_t *pa, uint32_t slv_dev_addr, uint32_t start_addr,
                            uint32_t no_of_bytes, uint8_t *write_data)
{
    uint16_t lane_mask = 0;
    uint32_t index = 0;
...
int _sesto_phy_module_read(const phymod_access_t *pa, uint32_t slv_dev_addr, uint32_t start_addr,
