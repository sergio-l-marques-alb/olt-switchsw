 ...
 *  Revision      :  $Id: falcon_furia_dv_functions.c 844 2015-01-12 20:58:16Z kirand $ *
 *                                                                                    *
...
err_code_t falcon_furia_dig_lpbk_rptr( const phymod_access_t *pa, uint8_t enable, enum srds_rptr_mode_enum mode) {
  ...
  switch(mode) {
    case DATA_IN_SIDE:
        ...
        if (enable) {
          err_code = falcon_furia_loop_timing( pa, 0);
          if (err_code) {
            return(err_code);
          }
          err_code = falcon_furia_loop_timing( pa, 1);     /* Program DATA_IN_SIDE side TX_PI for loop-timing mode to lock SYS TX to SYS TX clock freq. */
          if (err_code) {
            return(err_code);
          }
        }
...
err_code_t falcon_furia_rmt_lpbk( const phymod_access_t *pa, uint8_t enable) {
  ...
  if (enable) {
    err_code = falcon_furia_loop_timing( pa, enable);
    if (err_code) {
      return(err_code);
    }
  ...
  err_code_t falcon_furia_rmt_lpbk_from_ull( const phymod_access_t *pa ) {
    ...
    err_code = falcon_furia_rmt_lpbk( pa, 0);                      /* Reset TX_PI */
    if (err_code) {
      return (err_code);
    }
    err_code = falcon_furia_rmt_lpbk( pa, 1);                      /* Enable RMT LPBK */
    return (err_code);
  }
...
err_code_t falcon_furia_tx_pi_jitt_gen( const phymod_access_t *pa, uint8_t enable, int16_t freq_override_val, enum srds_tx_pi_freq_jit_gen_enum jit_type, uint8_t tx_pi_jit_freq_idx, uint8_t tx_pi_jit_amp) {
  ...
  err_code = falcon_furia_tx_pi_freq_override( pa, enable, freq_override_val);
  if (err_code) {
    return (err_code);
  }
...
err_code_t falcon_furia_core_pwrdn( const phymod_access_t *pa, enum srds_core_pwrdn_mode_enum mode) {
  ...
  switch(mode) {
    case PWR_ON:
            err_code = _falcon_furia_core_clkgate( pa, 0);
    ...
    case PWRDN:
            ...
            err_code = falcon_furia_delay_ns(500);                     /* wait for more than 50 comclk cycles  */
    ...
    case PWRDN_DEEP:
            ...
            err_code = falcon_furia_delay_ns(500);                     /* wait for more than 50 comclk cycles  */
            wrc_afe_s_pll_pwrdn(0x1);
            err_code = _falcon_furia_core_clkgate( pa, 1);
            if (err_code) return (err_code);
            break;
    default : return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
  }
...
err_code_t falcon_furia_lane_pwrdn( const phymod_access_t *pa, enum srds_core_pwrdn_mode_enum mode) {
  ...
  switch(mode) {
    case PWR_ON:
            ...
            err_code = _falcon_furia_lane_clkgate( pa, 0);
            if (err_code) return (err_code);
            break;
    case PWRDN:
    ...
    case PWRDN_DEEP:
            ...
            wr_ln_rx_s_pwrdn(0x1);
            wr_ln_tx_s_pwrdn(0x1);
            err_code = _falcon_furia_lane_clkgate( pa, 1);
            if (err_code) return (err_code);
            wr_ln_dp_s_rstb(0x0);
            break;
    case PWRDN_TX:
...
err_code_t falcon_furia_dig_lpbk_get(const phymod_access_t *pa,uint8_t *enable) {
  ...
  err_code_t falcon_furia_rdblk_uc_ram( const phymod_access_t *pa, uint8_t *mem, uint16_t addr, uint16_t cnt) {      
      ...
      falcon_furia_rdb_ram(pa, mem,addr,cnt);
      return(ERR_CODE_NONE);
  } 
