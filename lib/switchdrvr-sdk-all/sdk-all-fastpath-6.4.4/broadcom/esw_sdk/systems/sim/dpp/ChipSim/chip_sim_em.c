            ...
            )
{
  ...
  *success = TRUE;
  ...
  multi_set = &Em.blocks[id].multi_set[unit];
             ...
             )
{
  ...
  for (blk_ndx = 0; blk_ndx < block_cnt; blk_ndx++) {
    ...
    for (i=0; i<CHIP_SIM_NOF_CHIPS; i++)
    {
      ...
      block_p->multi_set[i].init_info.get_entry_fun = chip_sim_em_key_map_get_entry;
      block_p->multi_set[i].init_info.set_entry_fun = chip_sim_em_key_map_set_entry;
      block_p->multi_set[i].init_info.max_duplications = 1;/*maximal packets to store from each content*/
      block_p->multi_set[i].init_info.sec_handle = 0;
      block_p->multi_set[i].init_info.prime_handle = i;
      block_p->multi_set[i].init_info.member_size = block_p->key_size; /* Soc_pb_em_blocks[table_type] */
      block_p->multi_set[i].init_info.nof_members = block_p->end_address - block_p->start_address + 1; /*maximal packets to store*/

      block_p->multi_set[i].init_info.wb_var_index = SOC_DPP_WB_ENGINE_VAR_NONE;
