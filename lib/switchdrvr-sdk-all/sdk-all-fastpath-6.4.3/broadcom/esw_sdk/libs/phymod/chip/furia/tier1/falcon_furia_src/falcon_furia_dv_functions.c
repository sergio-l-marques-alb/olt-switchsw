 ...
 *  Revision      :  $Id: falcon_furia_dv_functions.c 714 2014-09-18 19:54:56Z kirand $ *
 *                                                                                    *
...
err_code_t falcon_furia_config_shared_tx_pattern(uint8_t patt_length, const char pattern[]) {
  ...
  if ((str_len > 2) && ((USR_STRNCMP(pattern, "0x", 2)) == 0)) {
    /* Hexadecimal Pattern */
    for (i=2; i < str_len; i++) {
      err_code = _compute_bin(pattern[i],bin);
      if (err_code) {
        return (err_code);
      }
...
err_code_t falcon_furia_write_rx_afe(enum srds_rx_afe_settings_enum param, int8_t val) {
  ...
  switch(param) {
    ...
    case RX_AFE_DFE6:           
	return (_set_rx_dfe6(val));  

    case RX_AFE_DFE7:          
	return (_set_rx_dfe7(val));  

    case RX_AFE_DFE8:          
	return (_set_rx_dfe8(val));  
    
    case RX_AFE_DFE9:          
	return (_set_rx_dfe9(val));  
    
    case RX_AFE_DFE10:         
	return (_set_rx_dfe10(val));  
    
    case RX_AFE_DFE11:     
        return (_set_rx_dfe11(val));  

    case RX_AFE_DFE12: 
        return (_set_rx_dfe12(val));  
    
    case RX_AFE_DFE13: 
        return (_set_rx_dfe13(val));  
    
    case RX_AFE_DFE14: 
        return (_set_rx_dfe14(val));  
    default:
	return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
  }
}
...
err_code_t falcon_furia_read_rx_afe(enum srds_rx_afe_settings_enum param, int8_t *val) {
  ...
  switch(param) {
    ...
    case RX_AFE_DFE5:
    	...
    	break;
    case RX_AFE_DFE6:
    	EFUN(_get_rx_dfe6(val));
    	break;
    case RX_AFE_DFE7:
    	EFUN(_get_rx_dfe7(val));
    	break;
    case RX_AFE_DFE8:
    	EFUN(_get_rx_dfe8(val));
    	break;
    case RX_AFE_DFE9:
    	EFUN(_get_rx_dfe9(val));
    	break;
    case RX_AFE_DFE10:
    	EFUN(_get_rx_dfe10(val));
    	break;
    case RX_AFE_DFE11:
    	EFUN(_get_rx_dfe11(val));
    	break;
    case RX_AFE_DFE12:
    	EFUN(_get_rx_dfe12(val));
    	break;
    case RX_AFE_DFE13:
    	EFUN(_get_rx_dfe13(val));
    	break;
    case RX_AFE_DFE14:
    	EFUN(_get_rx_dfe14(val));
    	break;
    default:
    	return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
  }
...
err_code_t falcon_furia_apply_txfir_cfg(int8_t pre, int8_t main, int8_t post1, int8_t post2, int8_t post3) {
  
  err_code_t failcode = falcon_furia_validate_txfir_cfg(pre, main, post1, post2, post3);
...
err_code_t falcon_furia_prbs_err_count_ll(uint32_t *prbs_err_cnt) {
  ...
  EFUN(falcon_furia_pmd_rdt_reg(TLB_RX_PRBS_CHK_ERR_CNT_MSB_STATUS, &rddata));
...
err_code_t falcon_furia_prbs_err_count_state(uint32_t *prbs_err_cnt, uint8_t *lock_lost) {
  ...
  EFUN(falcon_furia_prbs_err_count_ll(prbs_err_cnt));
...
err_code_t falcon_furia_dig_lpbk_rptr(uint8_t enable, enum srds_rptr_mode_enum mode) {
  ...
  switch(mode) {
    case DATA_IN_SIDE:
        ...
        if (enable) {
          err_code = falcon_furia_loop_timing(0);
          if (err_code) {
            return(err_code);
          }
          err_code = falcon_furia_loop_timing(1);     /* Program DATA_IN_SIDE side TX_PI for loop-timing mode to lock SYS TX to SYS TX clock freq. */
          if (err_code) {
            return(err_code);
          }
        }
...
err_code_t falcon_furia_loop_timing(uint8_t enable) {
  ...
  if (enable) {
    ...
    err_code = falcon_furia_delay_us(25);                     /* Wait for tclk to lock to CDR */
    if (err_code) {
      return(err_code);
    }
  } else {
...
err_code_t falcon_furia_rmt_lpbk(uint8_t enable) {
  ...
  if (enable) {
    err_code = falcon_furia_loop_timing(enable);
    if (err_code) {
      return(err_code);
    }
      wr_tx_pi_ext_ctrl_en(0x1);                        /* PD path enable: 0 = diabled, 1 = enabled */
    wr_rmt_lpbk_en(0x1);                                /* Remote Loopback Enable: 0 = diabled, 1 = enable  */
    err_code = falcon_furia_delay_us(50);                     /* Wait for rclk and tclk phase lock before expecing good data from rmt loopback */
  ...
  err_code_t falcon_furia_rmt_lpbk_from_ull(void) {
    ...
    err_code = falcon_furia_rmt_lpbk(0);                      /* Reset TX_PI */
    if (err_code) {
      return (err_code);
    }
    err_code = falcon_furia_rmt_lpbk(1);                      /* Enable RMT LPBK */
    return (err_code);
  }
...
err_code_t falcon_furia_tx_pi_jitt_gen(uint8_t enable, int16_t freq_override_val, enum srds_tx_pi_freq_jit_gen_enum jit_type, uint8_t tx_pi_jit_freq_idx, uint8_t tx_pi_jit_amp) {
  ...
  err_code = falcon_furia_tx_pi_freq_override(enable, freq_override_val);
  if (err_code) {
    return (err_code);
  }
...
err_code_t falcon_furia_core_pwrdn(enum srds_core_pwrdn_mode_enum mode) {
  ...
  switch(mode) {
    case PWR_ON:
            err_code = _falcon_furia_core_clkgate(0);
    ...
    case PWRDN_DEEP:
            wrc_afe_s_pll_pwrdn(0x1);
            wrc_core_dp_s_rstb(0x0);
            err_code = _falcon_furia_core_clkgate(1);
            if (err_code) return (err_code);
            break;
    default : return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
  }
...
err_code_t falcon_furia_lane_pwrdn(enum srds_core_pwrdn_mode_enum mode) {
  ...
  switch(mode) {
    case PWR_ON:
            ...
            err_code = _falcon_furia_lane_clkgate(0);
            if (err_code) return (err_code);
            break;
    case PWRDN:
    ...
    case PWRDN_DEEP:
            wr_ln_tx_s_pwrdn(0x1);
            wr_ln_rx_s_pwrdn(0x1);
            err_code = _falcon_furia_lane_clkgate(1);
            if (err_code) return (err_code);
            break;
    case PWRDN_TX:
            wr_ln_tx_s_pwrdn(0x1);
...
err_code_t falcon_furia_isolate_ctrl_pins(uint8_t enable) {
