...
STATIC int falcon_diagnostics_eyescan_run_uc(const phymod_phy_access_t* phy, uint32 flags)   
{
    int                 rc = PHYMOD_E_NONE, ii;
    uint32_t            stripe[64];
    uint16_t            status;

#if 0
    if(!(PHYMOD_EYESCAN_F_PROCESS_GET(flags))) {
        rc = falcon_tsc_display_eye_scan(&(phy->access));
        if(rc != PHYMOD_E_NONE) {
           _falcon_diag_uc_reg_dump(&(phy->access));
    ...
    if(PHYMOD_EYESCAN_F_ENABLE_GET(flags)) {
        PHYMOD_IF_ERR_RETURN(falcon_tsc_meas_eye_scan_start(&(phy->access), 0));
    ...
    if(PHYMOD_EYESCAN_F_PROCESS_GET(flags)) {
        ...
        for (ii = 62; ii >= -62; ii-=2) { 
            if (ii == 62) {
                ...
                PHYMOD_IF_ERR_RETURN(falcon_tsc_display_eye_scan_header(&(phy->access), 1));
            }

            rc  = falcon_tsc_read_eye_scan_stripe(&(phy->access), stripe, &status);
            if(rc != PHYMOD_E_NONE) {
                _falcon_diag_uc_reg_dump(&(phy->access));
                PHYMOD_IF_ERR_RETURN(rc);
            }
            PHYMOD_IF_ERR_RETURN(falcon_tsc_display_eye_scan_stripe(&(phy->access), ii, &stripe[0]));

            PHYMOD_DIAG_OUT(("\n"));
        }

        if (rc == PHYMOD_E_NONE) {
            PHYMOD_IF_ERR_RETURN(falcon_tsc_display_eye_scan_footer(&(phy->access), 1));
            PHYMOD_DIAG_OUT(("\n"));
        }
    }
    
    if(PHYMOD_EYESCAN_F_DONE_GET(flags)) {
        PHYMOD_IF_ERR_RETURN(falcon_tsc_meas_eye_scan_done(&(phy->access)));
    }
    ...
    )   
{
    ...
    if(PHYMOD_EYESCAN_F_PROCESS_GET(flags)) {
        ...
        PHYMOD_IF_ERR_RETURN(falcon_phy_meas_lowber_eye (&(phy->access), eyescan_options, buffer));

        PHYMOD_IF_ERR_RETURN(falcon_phy_display_lowber_eye (&(phy->access), eyescan_options, buffer));
    }
