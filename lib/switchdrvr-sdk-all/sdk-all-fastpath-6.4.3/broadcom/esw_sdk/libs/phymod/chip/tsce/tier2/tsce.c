...
{  
    ...
    if(PHYMOD_INTF_MODES_TRIPLE_CORE_GET(config)) {
        if(config->data_rate>=120000) {
            PHYMOD_IF_ERR_RETURN
                (temod_tsc12_control(&pm_phy_copy.access, 2, -1));
        } else if(config->data_rate>=100000) {
            PHYMOD_IF_ERR_RETURN
                (temod_tsc12_control(&pm_phy_copy.access, 1, -1));
        } else {
            PHYMOD_IF_ERR_RETURN
                (temod_tsc12_control(&pm_phy_copy.access, 0, -1));
        }
    }
...
int tsce_phy_autoneg_set(const phymod_phy_access_t* phy, const phymod_autoneg_control_t* an)
{
    ...
    if (an->enable) {
      PHYMOD_IF_ERR_RETURN
          (temod_set_an_port_mode(&phy->access, num_lane_adv_encoded, start_lane, 1));
    } else {
      PHYMOD_IF_ERR_RETURN
