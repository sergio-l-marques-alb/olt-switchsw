...
err_code_t falcon_osr_mode_set(const phymod_access_t *pa, int osr_mode)
{
  err_code_t __err;
  __err=ERR_CODE_NONE;
  __err=wr_osr_mode_frc_val(osr_mode-1);
  if(__err) return(__err);
...
err_code_t falcon_osr_mode_get(const phymod_access_t *pa, int *osr_mode)
{
  ...
  if(osr_forced) {
    *osr_mode = rd_osr_mode_frc_val()+1;
    if(__err) return(__err);
  } else {
    *osr_mode = rd_osr_mode_pin()+1;
    if(__err) return(__err);
  }
...
err_code_t falcon_clause72_control(const phymod_access_t *pa, uint32_t cl_72_en)                /* CLAUSE_72_CONTROL */
{
  ...
  if (cl_72_en) {
    ...
    __err=wr_cl93n72_ieee_restart_training(1); if(__err) return(__err);
    __err=wr_cl93n72_rx_training_en(1);        if(__err) return(__err);
    __err=wr_ln_dp_s_rstb(0);                  if(__err) return(__err);
    __err=wr_ln_dp_s_rstb(1);                  if(__err) return(__err);
  } else {
    __err=wr_cl93n72_rx_training_en(0);        if(__err) return(__err);
    __err=wr_cl93n72_ieee_restart_training(0); if(__err) return(__err);
