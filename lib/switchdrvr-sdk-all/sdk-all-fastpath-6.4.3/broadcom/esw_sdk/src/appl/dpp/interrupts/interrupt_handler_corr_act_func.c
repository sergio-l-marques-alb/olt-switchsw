...
int arad_interrupt_data_collection_for_ihpparityerror(int unit,
                                                      int block_instance,
                                                      arad_interrupt_type interrupt_id,
                                                      char* special_msg,
                                                      uint32 *addr,
                                                      arad_int_corr_act_type* corrective_action)
{
    ...
    soc_mem_t mem;
    char* memory_name; 
    uint32 address, address_valid, counter, counter_overflow;
    uint32 block;

    SOCDNX_INIT_FUNC_DEFS;

    SOCDNX_NULL_CHECK(addr);
    SOCDNX_NULL_CHECK(corrective_action);

    rc = arad_pp_em_ser_parity_error_info_get(unit, ARAD_PP_EM_SER_BLOCK_IHP, &address, &address_valid, &counter, &counter_overflow);
    SOCDNX_IF_ERR_EXIT(rc);

    block = ARAD_INTERRUPT_IHP_BLOCK;

    if (address_valid != 0) {
        mem = soc_addr_to_mem_extended(unit, block, 0xff, address);
        memory_name = SOC_MEM_NAME(unit, mem);

        sal_sprintf(special_msg, "cntf=0x%04x, cnt_overflowf=0x%01x, memory address=0x%08x memory=%s",
                    counter, counter_overflow, address, memory_name);

        *addr = address;
        *corrective_action = ARAD_INT_CORR_ACT_IHP_EM_SOFT_RECOVERY;
    } else {
         sal_sprintf(special_msg, "memory address=0x%08x address is not valid", address);
    }
...
int arad_interrupt_data_collection_for_ihbparityerror(int unit,
                                                      int block_instance,
                                                      arad_interrupt_type interrupt_id,
                                                      char* special_msg,
                                                      uint32 *addr,
                                                      arad_int_corr_act_type* corrective_action)
{
    ...
    rc = arad_pp_em_ser_parity_error_info_get(unit, ARAD_PP_EM_SER_BLOCK_IHB, &address, &address_valid, &counter, &counter_overflow);
    SOCDNX_IF_ERR_EXIT(rc);
...
int arad_interrupt_data_collection_for_epniparityerror(int unit,
                                                      int block_instance,
                                                      arad_interrupt_type interrupt_id,
                                                      char* special_msg,
                                                      uint32 *addr,
                                                      arad_int_corr_act_type* corrective_action)
{
    ...
    rc = arad_pp_em_ser_parity_error_info_get(unit, ARAD_PP_EM_SER_BLOCK_EPNI, &address, &address_valid, &counter, &counter_overflow);
    SOCDNX_IF_ERR_EXIT(rc);

    if (address_valid != 0) {
        sal_sprintf(special_msg, "cntf=0x%04x, cnt_overflowf=0x%01x, memory address=0x%08x",
                    counter, counter_overflow, address);
...
int arad_interrupt_data_collection_for_oampparityerror(int unit,
                                                      int block_instance,
                                                      arad_interrupt_type interrupt_id,
                                                      char* special_msg,
                                                      uint32 *addr,
                                                      arad_int_corr_act_type* corrective_action)
{
    ...
    rc = arad_pp_em_ser_parity_error_info_get(unit, ARAD_PP_EM_SER_BLOCK_OAMP, &address, &address_valid, &counter, &counter_overflow);
    SOCDNX_IF_ERR_EXIT(rc);
