  ...
  )
{
  ...
  SOC_PPD_LLP_TRAP_PORT_INFO_clear(&prm_port_info);
  ...
  { 
    ...
    if (param_val->numeric_equivalent == SOC_PPD_LLP_TRAP_PORT_ENABLE_NONE)
    {
                                  ...
                                  )
{
  ...
  SOC_PPD_LLP_TRAP_PORT_INFO_print(&prm_port_info);
...
{   
  ...
  unit = soc_ppd_get_default_unit(); 
  SOC_PPD_LLP_TRAP_ARP_INFO_clear(&prm_arp_info);
...
{   
  ...
  SOC_PPD_LLP_TRAP_ARP_INFO_print(&prm_arp_info);
  ...
  )
{
  ...
  unit = soc_ppd_get_default_unit();
  SOC_PPD_LLP_TRAP_RESERVED_MC_KEY_clear(&prm_reserved_mc_key);
  SOC_PPD_ACTION_PROFILE_clear(&prm_trap_action);
  ...
  if(!search_param_val_pairs(current_line,&match_index,SOC_PARAM_PPD_LLP_TRAP_RESERVED_MC_INFO_SET_RESERVED_MC_INFO_SET_TRAP_ACTION_PROFILE_TRAP_CODE_LSB_ID,1))
  {
    ...
    UI_MACROS_CHECK_GET_VAL_OF_ERROR;
    prm_trap_action.trap_code = (SOC_PPD_TRAP_CODE)param_val->numeric_equivalent;
  }
  */
  if(!search_param_val_pairs(current_line,&match_index,SOC_PARAM_PPD_LLP_TRAP_RESERVED_MC_INFO_SET_RESERVED_MC_INFO_SET_TRAP_ACTION_PROFILE_TRAP_CODE_LSB_ID,1))
  {
    ...
    UI_MACROS_CHECK_GET_VAL_OF_ERROR;
    prm_trap_action.trap_code = (SOC_PPD_TRAP_CODE)param_val->value.ulong_value;
  }
  ...
  )
{
  ...
  SOC_PPD_ACTION_PROFILE_print(&prm_trap_action);
  ...
  )
{
  ...
  unit = soc_ppd_get_default_unit();
  SOC_PPD_LLP_TRAP_PROG_TRAP_QUALIFIER_clear(&prm_general_trap_info);
  SOC_PPD_ACTION_PROFILE_clear(&prm_trap_action);
  ...
  )
{
  ...
  SOC_PPD_LLP_TRAP_PROG_TRAP_QUALIFIER_print(&prm_general_trap_info);
...
{   
  ...
  uint32   
    prm_local_port_ndx;
  SOC_PPD_LLP_MIRROR_PORT_DFLT_INFO   
  ...
  unit = soc_ppd_get_default_unit(); 
  SOC_PPD_LLP_MIRROR_PORT_DFLT_INFO_clear(&prm_dflt_mirroring_info);
...
{   
  ...
  SOC_PPD_LLP_MIRROR_PORT_DFLT_INFO_print(&prm_dflt_mirroring_info);
