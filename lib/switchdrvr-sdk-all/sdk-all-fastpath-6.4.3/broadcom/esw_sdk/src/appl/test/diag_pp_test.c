...
"Diag PP Test Usage:\n"
"  Bridge=<value>         1: The test will be applied for bridge application. \n"
"  Router=<value>         1: The test will be applied for router application. \n"
"  Mpls=<value>           1: The test will be applied for mpls application. \n"
"  Vpls=<value>           1: The test will be applied for vpls application, sending a packet from PWE to AC.\n"
"                         2: The test will be applied for vpls application, sending a packet from AC to PWE.\n"
;
...
{
    SOC_PPD_DIAG_LEM_LKUP_TYPE type;
    SOC_PPD_DIAG_LEM_KEY key;
    SOC_PPD_DIAG_LEM_VALUE value;
    uint8 valid;
...
int
diag_pp_received_packet_get(int unit, diag_pp_received_packets_fields_t *pkt_info)
{
    int rv = 0;
    ...
    rv = soc_ppd_diag_received_packet_info_get(soc_sand_dev_id, &prm_rcvd_pkt_info);
...
int
diag_pp_parsing_info_get(int unit, diag_pp_parsing_info_fields_t *pars_info)
{
    int rv = 0;
    SOC_PPD_DIAG_PARSING_INFO prm_pars_info;
    uint32 soc_sand_dev_id;

    soc_sand_dev_id = (unit);
    SOC_PPD_DIAG_PARSING_INFO_clear(&prm_pars_info);
    rv = soc_ppd_diag_parsing_info_get(soc_sand_dev_id, &prm_pars_info);
    if (soc_sand_get_error_code_from_error_word(rv) != SOC_SAND_OK) 
    { 
    ...
    pars_info->hdr_type = prm_pars_info.hdr_type;
...
int
diag_pp_termination_info_get(int unit, diag_pp_termination_info_fields_t *term_info)
{
    int rv = 0;
    SOC_PPD_DIAG_TERM_INFO prm_term_info;
    uint32 soc_sand_dev_id;

    soc_sand_dev_id = (unit);
    SOC_PPD_DIAG_TERM_INFO_clear(&prm_term_info);
    rv = soc_ppd_diag_termination_info_get(soc_sand_dev_id, &prm_term_info);
    if (soc_sand_get_error_code_from_error_word(rv) != SOC_SAND_OK) 
    { 
...
int
diag_pp_vlan_edit_info_get(int unit, diag_pp_vlan_edit_info_fields_t *vlan_edit_info)
{
    int rv = 0;
    ...
    soc_sand_dev_id = (unit);
    SOC_PPD_DIAG_VLAN_EDIT_RES_clear(&prm_vlan_edit_info);
    rv = soc_ppd_diag_ing_vlan_edit_info_get(soc_sand_dev_id, &prm_vlan_edit_info);
    if (soc_sand_get_error_code_from_error_word(rv) != SOC_SAND_OK) 
    { 
...
int
diag_pp_pkt_tm_info_get(int unit, diag_pp_pkt_tm_info_fields_t *pkt_tm_info)
{
    int rv = 0;
    ...
    rv = soc_ppd_diag_pkt_associated_tm_info_get(soc_sand_dev_id, &prm_pkt_tm_info);
    if (soc_sand_get_error_code_from_error_word(rv) != SOC_SAND_OK) 
    { 
...
int
diag_pp_lif_lkup_info_get(int unit, diag_pp_lif_lkup_info_fields_t *lif_lkup_info, SOC_PPC_DIAG_DB_USE_INFO *db_info)
{
    int rv = 0;
    ...
    soc_sand_dev_id = (unit);
    SOC_PPD_DIAG_LIF_LKUP_INFO_clear(&prm_lif_lkup_info);
    rv = soc_ppd_diag_db_lif_lkup_info_get(soc_sand_dev_id, db_info, &prm_lif_lkup_info);
    if (soc_sand_get_error_code_from_error_word(rv) != SOC_SAND_OK) 
    { 
...
int
diag_pp_frwrd_trace_info_get(int unit, SOC_PPD_DIAG_FRWRD_DECISION_TRACE_INFO *frwrd_trace_info)
{
    int rv = 0;
    uint32 soc_sand_dev_id;

    soc_sand_dev_id = (unit);
    SOC_PPD_DIAG_FRWRD_DECISION_TRACE_INFO_clear(frwrd_trace_info);
    rv = soc_ppd_diag_frwrd_decision_trace_get(soc_sand_dev_id, frwrd_trace_info);
    if (soc_sand_get_error_code_from_error_word(rv) != SOC_SAND_OK) 
    { 
...
int
diag_pp_traps_info_get(int unit, SOC_PPD_DIAG_TRAPS_INFO *traps_info)
{
    int rv = 0;
    uint32 soc_sand_dev_id;

    soc_sand_dev_id = (unit);
    SOC_PPD_DIAG_TRAPS_INFO_clear(traps_info);
    rv = soc_ppd_diag_traps_info_get(soc_sand_dev_id, traps_info);
    if (soc_sand_get_error_code_from_error_word(rv) != SOC_SAND_OK) 
    { 
...
int
diag_pp_encap_info_get(int unit, SOC_PPD_DIAG_ENCAP_INFO *encap_info)
{
    int rv = 0;
    uint32 soc_sand_dev_id;

    soc_sand_dev_id = (unit);
    SOC_PPD_DIAG_ENCAP_INFO_clear(encap_info);
    rv = soc_ppd_diag_encap_info_get(soc_sand_dev_id, encap_info);
    if (soc_sand_get_error_code_from_error_word(rv) != SOC_SAND_OK) 
    { 
...
int
diag_pp_frwrd_lkup_info_get(int unit, SOC_PPD_DIAG_FRWRD_LKUP_INFO *frwrd_lkup_info)
{
    int rv = 0;
    uint32 soc_sand_dev_id;

    soc_sand_dev_id = (unit);
    SOC_PPD_DIAG_FRWRD_LKUP_INFO_clear(frwrd_lkup_info);
    rv = soc_ppd_diag_frwrd_lkup_info_get(soc_sand_dev_id, frwrd_lkup_info);
    if (soc_sand_get_error_code_from_error_word(rv) != SOC_SAND_OK) 
    { 
...
int
diag_pp_frwrd_lpm_lkup_get(int unit, diag_pp_frwrd_lpm_lkup_fields_t *lpm_lkup_info)
{
    int rv = 0;
    uint32 soc_sand_dev_id;
    SOC_PPD_DIAG_IPV4_VPN_KEY prm_lpm_key;

    soc_sand_dev_id = (unit);
    SOC_PPD_DIAG_IPV4_VPN_KEY_clear(&prm_lpm_key);
    prm_lpm_key.key.subnet.ip_address = 0x7fffff00;
    prm_lpm_key.key.subnet.prefix_len = 24;
    prm_lpm_key.vrf = 0;
...
int
diag_pp_lem_lkup_info_get(int unit, diag_pp_lem_lkup_info_fields_t *lem_lkup_info)
{
    int rv = 0;
    uint32 soc_sand_dev_id;
    SOC_PPD_DIAG_DB_USE_INFO prm_db_info;
...
int
diag_pp_egress_vlan_edit_info_get(int unit, SOC_PPD_DIAG_EGRESS_VLAN_EDIT_RES *egress_vlan_edit_info)
{
    int rv = 0;
    uint32 soc_sand_dev_id;

    soc_sand_dev_id = (unit);
    SOC_PPD_DIAG_EGRESS_VLAN_EDIT_INFO_clear(egress_vlan_edit_info);
    rv = soc_ppd_diag_egress_vlan_edit_info_get(soc_sand_dev_id, egress_vlan_edit_info);
...
diag_pp_parsing_info_compare(diag_pp_parsing_info_fields_t *pars_info, diag_pp_parsing_info_fields_t *expected_pars_info, uint8 num_of_headers)
{
    ...
    if(pars_info->hdr_type != expected_pars_info->hdr_type)
    {
        cli_out("Unexpected hdr_type. Received: %d Expected: %d\n", pars_info->hdr_type, expected_pars_info->hdr_type);
        rv = CMD_FAIL;
    }
...
diag_pp_lif_lkup_info_compare(diag_pp_lif_lkup_info_fields_t *lif_lkup_info, diag_pp_lif_lkup_info_fields_t *expected_lif_lkup_info)
{
    ...
    if(memcmp(&lif_lkup_info->key, &expected_lif_lkup_info->key, sizeof(SOC_PPC_DIAG_LIF_KEY)) != 0)
    {
...
int
diag_pp_frwrd_trace_info_compare(SOC_PPD_DIAG_FRWRD_DECISION_TRACE_INFO *frwrd_trace_info, SOC_PPD_DIAG_FRWRD_DECISION_TRACE_INFO *expected_frwrd_trace_info)
{
    ...
    for(frwrd_decision_phase = 0; frwrd_decision_phase < SOC_PPC_NOF_DIAG_FRWRD_DECISION_PHASES; frwrd_decision_phase++)
    {
        ...
        if(frwrd_trace_info->frwrd[frwrd_decision_phase].is_valid != expected_frwrd_trace_info->frwrd[frwrd_decision_phase].is_valid)
        {
            cli_out("Unexpected frwrd is_valid in phase %d. Received: %d Expected: %d\n", 
                    frwrd_decision_phase, 
                    frwrd_trace_info->frwrd[frwrd_decision_phase].is_valid, 
                    expected_frwrd_trace_info->frwrd[frwrd_decision_phase].is_valid);
            rv = CMD_FAIL;
        }
...
int
diag_pp_traps_info_compare(SOC_PPD_DIAG_TRAPS_INFO *traps_info, SOC_PPD_DIAG_TRAPS_INFO *expected_traps_info)
{
    int rv = 0;
...
int
diag_pp_encap_info_compare(SOC_PPD_DIAG_ENCAP_INFO *encap_info, SOC_PPD_DIAG_ENCAP_INFO *expected_encap_info)
{
    int eep_index;
    int rv = 0;

    for(eep_index = 0; eep_index < encap_info->nof_eeps; eep_index++)
    {
...
int
diag_pp_frwrd_lkup_info_compare(SOC_PPD_DIAG_FRWRD_LKUP_INFO *frwrd_lkup_info, SOC_PPD_DIAG_FRWRD_LKUP_INFO *expected_frwrd_lkup_info)
{
    int rv = 0;
...
diag_pp_lem_lkup_info_compare(diag_pp_lem_lkup_info_fields_t *lem_lkup_info, diag_pp_lem_lkup_info_fields_t *expected_lem_lkup_info)
{
    ...
    if(memcmp(&lem_lkup_info->key, &expected_lem_lkup_info->key, sizeof(SOC_PPD_DIAG_LEM_KEY)) != 0)
    {
        cli_out("Unexpected differences in key\n");
        rv = CMD_FAIL;
    }
    if(memcmp(&lem_lkup_info->value, &expected_lem_lkup_info->value, sizeof(SOC_PPD_DIAG_LEM_VALUE)) != 0)
    {
...
int 
diag_pp_egress_vlan_edit_info_compare(SOC_PPD_DIAG_EGRESS_VLAN_EDIT_RES  *egress_vlan_edit_info, SOC_PPD_DIAG_EGRESS_VLAN_EDIT_RES *expected_egress_vlan_edit_info)
{
   int rv = 0;
...
int
diag_pp_received_packet_bridge_test(int unit)
{
    int rv = 0;
...
int
diag_pp_parsing_info_bridge_test(int unit)
{
    int rv = 0;
    ...
    expected_pars_info.hdr_type = SOC_PPC_PKT_HDR_STK_TYPE_ETH;
    ...
    if((rv = diag_pp_parsing_info_get(unit, &received_pars_info )) < 0)
    { 
...
int
diag_pp_termination_info_bridge_test(int unit)
{
    int rv = 0;
    ...
    if((rv = diag_pp_termination_info_get(unit, &received_term_info )) < 0)
    { 
...
int
diag_pp_vlan_edit_info_bridge_test(int unit)
{
    int rv = 0;
    ...
    if((rv = diag_pp_vlan_edit_info_get(unit, &received_vlan_edit_info )) < 0)
    { 
...
int
diag_pp_pkt_tm_info_bridge_test(int unit)
{
    int rv = 0;
    ...
    expected_pkt_tm_info.frwrd_decision.dest_id = 201;
    ...
    expected_pkt_tm_info.eth_meter_ptr = 1001;
    ...
    if((rv = diag_pp_pkt_tm_info_get(unit, &received_pkt_tm_info )) < 0)
    { 
...
int
diag_pp_lif_lkup_info_bridge_test(int unit)
{
    int rv = 0;
    ...
    SOC_PPD_DIAG_DB_USE_INFO_clear(db_info);
    db_info->lkup_num = 1;
    db_info->bank_id = 0;
    ...
    expected_lif_lkup_info.key.ac.key_type = SOC_PPC_L2_LIF_AC_MAP_KEY_TYPE_PORT_VLAN;
    expected_lif_lkup_info.key.ac.vlan_domain = 0;
    expected_lif_lkup_info.key.ac.outer_vid = 20;
    expected_lif_lkup_info.key.ac.inner_vid = SOC_PPC_LIF_IGNORE_INNER_VID;
    ...
    expected_lif_lkup_info.base_index = 4097;

    if((rv = diag_pp_lif_lkup_info_get(unit, &received_lif_lkup_info, db_info )) < 0)
    { 
...
int
diag_pp_frwrd_trace_info_bridge_test(int unit)
{
    int rv = 0;
    SOC_PPD_DIAG_FRWRD_DECISION_TRACE_INFO received_frwrd_trace_info;
    SOC_PPD_DIAG_FRWRD_DECISION_TRACE_INFO expected_frwrd_trace_info;

    /* Initialize expected info */
    SOC_PPD_DIAG_FRWRD_DECISION_TRACE_INFO_clear(&expected_frwrd_trace_info);
    ...
    expected_frwrd_trace_info.frwrd[SOC_PPC_DIAG_FRWRD_DECISION_PHASE_LIF].is_valid = 1;
    ...
    expected_frwrd_trace_info.frwrd[SOC_PPC_DIAG_FRWRD_DECISION_PHASE_LKUP_FOUND].frwrd_decision.dest_id = 201;
    expected_frwrd_trace_info.frwrd[SOC_PPC_DIAG_FRWRD_DECISION_PHASE_LKUP_FOUND].is_valid = 1;
    ...
    expected_frwrd_trace_info.frwrd[SOC_PPC_DIAG_FRWRD_DECISION_PHASE_PMF].frwrd_decision.dest_id = 201;
    ...
    expected_frwrd_trace_info.frwrd[SOC_PPC_DIAG_FRWRD_DECISION_PHASE_TRAP].frwrd_decision.dest_id = 201;
    ...
    expected_frwrd_trace_info.frwrd[SOC_PPC_DIAG_FRWRD_DECISION_PHASE_ING_RESOLVED].frwrd_decision.dest_id = 201;


    if((rv = diag_pp_frwrd_trace_info_get(unit, &received_frwrd_trace_info)) < 0)
    { 
...
int
diag_pp_traps_info_bridge_test(int unit)
{
    int rv = 0;
    SOC_PPD_DIAG_TRAPS_INFO received_traps_info;
    SOC_PPD_DIAG_TRAPS_INFO expected_traps_info;

    /* Initialize expected info */
    SOC_PPD_DIAG_TRAPS_INFO_clear(&expected_traps_info);
    ...
    if((rv = diag_pp_traps_info_get(unit, &received_traps_info)) < 0)
    { 
...
int
diag_pp_encap_info_bridge_test(int unit)
{
    int rv = 0;
    SOC_PPD_DIAG_ENCAP_INFO received_encap_info;
    SOC_PPD_DIAG_ENCAP_INFO expected_encap_info;

    /* Initialize expected info */
    SOC_PPD_DIAG_ENCAP_INFO_clear(&expected_encap_info);
    expected_encap_info.encap_info[0].entry_type = SOC_PPC_NOF_EG_ENCAP_ENTRY_TYPES_ARAD;
    expected_encap_info.nof_eeps = 1;
    expected_encap_info.ll_vsi = 30;
    expected_encap_info.out_ac = 4097;
    expected_encap_info.tm_port = 201;
    expected_encap_info.pp_port = 201;
  

    if((rv = diag_pp_encap_info_get(unit, &received_encap_info)) < 0)
    { 
...
int
diag_pp_frwrd_lkup_info_bridge_test(int unit)
{
    int rv = 0;
    SOC_PPD_DIAG_FRWRD_LKUP_INFO received_frwrd_lkup_info;
    SOC_PPD_DIAG_FRWRD_LKUP_INFO expected_frwrd_lkup_info;

    /* Initialize expected info */
    SOC_PPD_DIAG_FRWRD_LKUP_INFO_clear(&expected_frwrd_lkup_info);
    ...
    expected_frwrd_lkup_info.lkup_res.mact.frwrd_info.forward_decision.dest_id = 201;
    expected_frwrd_lkup_info.lkup_res.mact.aging_info.age_status = 6;



    if((rv = diag_pp_frwrd_lkup_info_get(unit, &received_frwrd_lkup_info)) < 0)
    { 
...
int
diag_pp_frwrd_lpm_lkup_bridge_test(int unit)
{
    int rv = 0;
    ...
    if((rv = diag_pp_frwrd_lpm_lkup_get(unit, &received_lpm_lkup_info)) < 0)
    { 
...
int
diag_pp_lem_lkup_info_bridge_test(int unit)
{
    int rv = 0;
    ...
    /* Initialize expected info */
    SOC_PPD_DIAG_LEM_KEY_clear(&expected_lem_lkup_info.key);
    SOC_PPD_DIAG_LEM_VALUE_clear(&expected_lem_lkup_info.value);
    ...
    expected_lem_lkup_info.value.mact.frwrd_info.forward_decision.dest_id = 201;
    expected_lem_lkup_info.value.mact.aging_info.age_status = 0;

    if((rv = diag_pp_lem_lkup_info_get(unit, &received_lem_lkup_info)) < 0)
    { 
...
int 
diag_pp_egress_vlan_edit_info_bridge_test(int unit)
{
   int rv = 0;
   SOC_PPD_DIAG_EGRESS_VLAN_EDIT_RES received_egress_vlan_edit_info;
   SOC_PPD_DIAG_EGRESS_VLAN_EDIT_RES expected_egress_vlan_edit_info;

   /* Initialize expected info */
   SOC_PPD_DIAG_EGRESS_VLAN_EDIT_INFO_clear(&expected_egress_vlan_edit_info);
   /* command key */
   ...
   if ((rv = diag_pp_egress_vlan_edit_info_get(unit, &received_egress_vlan_edit_info)) < 0)
   {
        cli_out("Error: diag_pp_egress_vlan_edit_info_get() Failed:\n");
        return rv;
   }
...
int
diag_pp_received_packet_router_test(int unit)
{
    int rv = 0;
    diag_pp_received_packets_fields_t received_pkt_info;
    diag_pp_received_packets_fields_t expected_pkt_info;
...
int
diag_pp_parsing_info_router_test(int unit)
{
    int rv = 0;
    diag_pp_parsing_info_fields_t received_pars_info;
    diag_pp_parsing_info_fields_t expected_pars_info;
    ...
    /* ETH header */
    SOC_PPC_DIAG_PARSING_L2_INFO_clear(&expected_pars_info.hdrs_stack[0].eth);
    expected_pars_info.hdr_type = SOC_PPC_PKT_HDR_STK_TYPE_IPV4_ETH;
    expected_pars_info.hdrs_stack[0].hdr_offset = 0;
    expected_pars_info.hdrs_stack[0].hdr_type = SOC_PPC_PKT_HDR_TYPE_ETH;
    expected_pars_info.hdrs_stack[0].eth.encap_type = SOC_SAND_PP_ETH_ENCAP_TYPE_ETH2;
...
int
diag_pp_termination_info_router_test(int unit)
{
    int rv = 0;
    diag_pp_termination_info_fields_t received_term_info;
    diag_pp_termination_info_fields_t expected_term_info;
...
int
diag_pp_vlan_edit_info_router_test(int unit)
{
    int rv = 0;
    diag_pp_vlan_edit_info_fields_t received_vlan_edit_info;
    diag_pp_vlan_edit_info_fields_t expected_vlan_edit_info;
...
int
diag_pp_pkt_tm_info_router_test(int unit)
{
    int rv = 0;
    diag_pp_pkt_tm_info_fields_t received_pkt_tm_info;
    diag_pp_pkt_tm_info_fields_t expected_pkt_tm_info;
...
int
diag_pp_lif_lkup_info_router_test(int unit)
{
    int rv = 0;
    diag_pp_lif_lkup_info_fields_t received_lif_lkup_info;
    diag_pp_lif_lkup_info_fields_t expected_lif_lkup_info;
    ...
    expected_lif_lkup_info.type = SOC_PPC_DIAG_LIF_LKUP_TYPE_IPV4_TUNNEL;
    expected_lif_lkup_info.key.ip_tunnel.dip = 0x7fffff03;
    expected_lif_lkup_info.key.ip_tunnel.dip_prefix_len = 0;
    expected_lif_lkup_info.key.ip_tunnel.sip = 0x02010101;
    expected_lif_lkup_info.key.ip_tunnel.sip_prefix_len = 0;
    expected_lif_lkup_info.key.ip_tunnel.flags = 3;
    expected_lif_lkup_info.found = 0;
...
int
diag_pp_frwrd_trace_info_router_test(int unit)
{
    int rv = 0;
    SOC_PPD_DIAG_FRWRD_DECISION_TRACE_INFO received_frwrd_trace_info;
    SOC_PPD_DIAG_FRWRD_DECISION_TRACE_INFO expected_frwrd_trace_info;
    ...
    expected_frwrd_trace_info.frwrd[SOC_PPC_DIAG_FRWRD_DECISION_PHASE_LKUP_FOUND].frwrd_decision.dest_id = 1024;
    expected_frwrd_trace_info.frwrd[SOC_PPC_DIAG_FRWRD_DECISION_PHASE_LKUP_FOUND].is_valid = 1;
    expected_frwrd_trace_info.trap[SOC_PPC_DIAG_FRWRD_DECISION_PHASE_LKUP_FOUND].action_profile.trap_code = SOC_PPC_TRAP_CODE_ACCEPTABLE_FRAME_TYPE_ACCEPT;
    expected_frwrd_trace_info.trap[SOC_PPC_DIAG_FRWRD_DECISION_PHASE_LKUP_FOUND].action_profile.frwrd_action_strength = 3;
    ...
    expected_frwrd_trace_info.frwrd[SOC_PPC_DIAG_FRWRD_DECISION_PHASE_PMF].frwrd_decision.dest_id = 1024;
    ...
    expected_frwrd_trace_info.frwrd[SOC_PPC_DIAG_FRWRD_DECISION_PHASE_TRAP].frwrd_decision.additional_info.eei.val.outlif = 4096;
    ...
    expected_frwrd_trace_info.frwrd[SOC_PPC_DIAG_FRWRD_DECISION_PHASE_ING_RESOLVED].frwrd_decision.additional_info.eei.val.outlif = 4096;
...
int
diag_pp_traps_info_router_test(int unit)
{
    int rv = 0;
    SOC_PPD_DIAG_TRAPS_INFO received_traps_info;
    SOC_PPD_DIAG_TRAPS_INFO expected_traps_info;
...
int
diag_pp_encap_info_router_test(int unit)
{
    int rv = 0;
    SOC_PPD_DIAG_ENCAP_INFO received_encap_info;
    SOC_PPD_DIAG_ENCAP_INFO expected_encap_info;
    ...
    expected_encap_info.eep[1] = 4096;
...
int
diag_pp_frwrd_lkup_info_router_test(int unit)
{
    int rv = 0;
    SOC_PPD_DIAG_FRWRD_LKUP_INFO received_frwrd_lkup_info;
    SOC_PPD_DIAG_FRWRD_LKUP_INFO expected_frwrd_lkup_info;
    ...
    expected_frwrd_lkup_info.key_found = 1;
    expected_frwrd_lkup_info.lkup_res.host_info.frwrd_decision.dest_id = 1024;
...
int
diag_pp_frwrd_lpm_lkup_router_test(int unit)
{
    int rv = 0;
    diag_pp_frwrd_lpm_lkup_fields_t received_lpm_lkup_info;
    diag_pp_frwrd_lpm_lkup_fields_t expected_lpm_lkup_info;
...
int
diag_pp_lem_lkup_info_router_test(int unit)
{
    int rv = 0;
    diag_pp_lem_lkup_info_fields_t received_lem_lkup_info;
    diag_pp_lem_lkup_info_fields_t expected_lem_lkup_info;
    ...
    expected_lem_lkup_info.value.host.frwrd_decision.dest_id = 1024;
...
int 
diag_pp_egress_vlan_edit_info_router_test(int unit)
{
   int rv = 0;
   SOC_PPD_DIAG_EGRESS_VLAN_EDIT_RES received_egress_vlan_edit_info;
   SOC_PPD_DIAG_EGRESS_VLAN_EDIT_RES expected_egress_vlan_edit_info;
...
int
diag_pp_received_packet_mpls_test(int unit)
{
    int rv = 0;
    diag_pp_received_packets_fields_t received_pkt_info;
    diag_pp_received_packets_fields_t expected_pkt_info;
...
int
diag_pp_parsing_info_mpls_test(int unit)
{
    int rv = 0;
    diag_pp_parsing_info_fields_t received_pars_info;
    diag_pp_parsing_info_fields_t expected_pars_info;
    ...
    /* ETH header */
    SOC_PPC_DIAG_PARSING_L2_INFO_clear(&expected_pars_info.hdrs_stack[0].eth);
    expected_pars_info.hdr_type = SOC_PPC_PKT_HDR_STK_TYPE_MPLS2_ETH;
    expected_pars_info.hdrs_stack[0].hdr_offset = 0;
    expected_pars_info.hdrs_stack[0].hdr_type = SOC_PPC_PKT_HDR_TYPE_ETH;
    expected_pars_info.hdrs_stack[0].eth.encap_type = SOC_SAND_PP_ETH_ENCAP_TYPE_ETH2;
...
int
diag_pp_termination_info_mpls_test(int unit)
{
    int rv = 0;
    diag_pp_termination_info_fields_t received_term_info;
    diag_pp_termination_info_fields_t expected_term_info;
...
int
diag_pp_vlan_edit_info_mpls_test(int unit)
{
    int rv = 0;
    diag_pp_vlan_edit_info_fields_t received_vlan_edit_info;
    diag_pp_vlan_edit_info_fields_t expected_vlan_edit_info;
...
int
diag_pp_pkt_tm_info_mpls_test(int unit)
{
    int rv = 0;
    diag_pp_pkt_tm_info_fields_t received_pkt_tm_info;
    diag_pp_pkt_tm_info_fields_t expected_pkt_tm_info;
...
int
diag_pp_lif_lkup_info_mpls_test(int unit)
{
    int rv = 0;
    diag_pp_lif_lkup_info_fields_t received_lif_lkup_info;
    diag_pp_lif_lkup_info_fields_t expected_lif_lkup_info;
    ...
    expected_lif_lkup_info.key.mpls.label_id = 400;
    ...
    expected_lif_lkup_info.found = 1; 
    expected_lif_lkup_info.base_index = 16384;
    expected_lif_lkup_info.opcode_id = 0;
...
int
diag_pp_frwrd_trace_info_mpls_test(int unit)
{
    int rv = 0;
    SOC_PPD_DIAG_FRWRD_DECISION_TRACE_INFO received_frwrd_trace_info;
    SOC_PPD_DIAG_FRWRD_DECISION_TRACE_INFO expected_frwrd_trace_info;
    ...
    expected_frwrd_trace_info.frwrd[SOC_PPC_DIAG_FRWRD_DECISION_PHASE_TRAP].frwrd_decision.type = SOC_PPC_FRWRD_DECISION_TYPE_UC_PORT;
    expected_frwrd_trace_info.frwrd[SOC_PPC_DIAG_FRWRD_DECISION_PHASE_TRAP].frwrd_decision.dest_id = 201;
    expected_frwrd_trace_info.frwrd[SOC_PPC_DIAG_FRWRD_DECISION_PHASE_TRAP].outlif = 4096;
    ...
    expected_frwrd_trace_info.frwrd[SOC_PPC_DIAG_FRWRD_DECISION_PHASE_ING_RESOLVED].frwrd_decision.type = SOC_PPC_FRWRD_DECISION_TYPE_UC_PORT;
    expected_frwrd_trace_info.frwrd[SOC_PPC_DIAG_FRWRD_DECISION_PHASE_ING_RESOLVED].frwrd_decision.dest_id = 201;
    expected_frwrd_trace_info.frwrd[SOC_PPC_DIAG_FRWRD_DECISION_PHASE_ING_RESOLVED].outlif = 4096;
...
int
diag_pp_traps_info_mpls_test(int unit)
{
    int rv = 0;
    SOC_PPD_DIAG_TRAPS_INFO received_traps_info;
    SOC_PPD_DIAG_TRAPS_INFO expected_traps_info;
...
int
diag_pp_encap_info_mpls_test(int unit)
{
    int rv = 0;
    SOC_PPD_DIAG_ENCAP_INFO received_encap_info;
    SOC_PPD_DIAG_ENCAP_INFO expected_encap_info;
    ...
    expected_encap_info.eep[0] = 4096;
    ...
    expected_encap_info.eep[1] = 8192;
    expected_encap_info.encap_info[1].entry_type = SOC_PPC_EG_ENCAP_ENTRY_TYPE_LL_ENCAP;
...
int
diag_pp_frwrd_lkup_info_mpls_test(int unit)
{
    int rv = 0;
    SOC_PPD_DIAG_FRWRD_LKUP_INFO received_frwrd_lkup_info;
    SOC_PPD_DIAG_FRWRD_LKUP_INFO expected_frwrd_lkup_info;
    ...
    expected_frwrd_lkup_info.lkup_res.frwrd_decision.dest_id = 1024;
...
int
diag_pp_frwrd_lpm_lkup_mpls_test(int unit)
{
    int rv = 0;
    diag_pp_frwrd_lpm_lkup_fields_t received_lpm_lkup_info;
    diag_pp_frwrd_lpm_lkup_fields_t expected_lpm_lkup_info;
...
int
diag_pp_lem_lkup_info_mpls_test(int unit)
{
    int rv = 0;
    diag_pp_lem_lkup_info_fields_t received_lem_lkup_info;
    diag_pp_lem_lkup_info_fields_t expected_lem_lkup_info;
    ...
    expected_lem_lkup_info.value.ilm.dest_id = 1024;
...
int 
diag_pp_egress_vlan_edit_info_mpls_test(int unit)
{
   int rv = 0;
   SOC_PPD_DIAG_EGRESS_VLAN_EDIT_RES received_egress_vlan_edit_info;
   SOC_PPD_DIAG_EGRESS_VLAN_EDIT_RES expected_egress_vlan_edit_info;
...
int
diag_pp_received_packet_vpls_test(int unit, uint32 ac_to_pwe)
{
    int rv = 0;
    diag_pp_received_packets_fields_t received_pkt_info;
    diag_pp_received_packets_fields_t expected_pkt_info;
    ...
    if (ac_to_pwe) {
        ...
        expected_pkt_info.in_pp_port    = 201;
    }
    expected_pkt_info.in_tm_port    = 0;
    expected_pkt_info.packet_qual   = 0;
    expected_pkt_info.pp_context    = SOC_TMC_PORT_HEADER_TYPE_ETH;
    expected_pkt_info.sys_id        = expected_pkt_info.in_pp_port;
    expected_pkt_info.sys_port_type = SOC_SAND_PP_SYS_PORT_TYPE_SINGLE_PORT;
...
int
diag_pp_parsing_info_vpls_test(int unit, uint32 ac_to_pwe)
{
    int 
        rv = 0,
        number_of_headers;
    ...
    if (ac_to_pwe) {
        ...
        number_of_headers = 3;
        expected_pars_info.hdr_type = SOC_PPC_PKT_HDR_STK_TYPE_ETH_MPLS2_ETH;
...
int
diag_pp_termination_info_vpls_test(int unit, uint32 ac_to_pwe)
{
    int rv = 0;
    diag_pp_termination_info_fields_t received_term_info;
    diag_pp_termination_info_fields_t expected_term_info;
...
int
diag_pp_vlan_edit_info_vpls_test(int unit, uint32 ac_to_pwe)
{
    int rv = 0;
    diag_pp_vlan_edit_info_fields_t received_vlan_edit_info;
    diag_pp_vlan_edit_info_fields_t expected_vlan_edit_info;
...
int
diag_pp_encap_info_vpls_test(int unit, uint32 ac_to_pwe)
{
    int rv = 0;
    SOC_PPD_DIAG_ENCAP_INFO received_encap_info;
    SOC_PPD_DIAG_ENCAP_INFO expected_encap_info;
    ...
    if (ac_to_pwe) {
        ...
        expected_encap_info.out_ac = 12288;
        expected_encap_info.tm_port = 200;
        expected_encap_info.pp_port = 200;
...
int
diag_pp_frwrd_lkup_info_vpls_test(int unit, uint32 ac_to_pwe)
{
    int rv = 0;
    SOC_PPD_DIAG_FRWRD_LKUP_INFO received_frwrd_lkup_info;
    SOC_PPD_DIAG_FRWRD_LKUP_INFO expected_frwrd_lkup_info;
    ...
    if (ac_to_pwe){   
        ...
        expected_frwrd_lkup_info.lkup_res.mact.frwrd_info.forward_decision.dest_id = 1024;
        ...
        expected_frwrd_lkup_info.lkup_res.mact.frwrd_info.forward_decision.type = SOC_PPC_FRWRD_DECISION_TYPE_UC_PORT;
        expected_frwrd_lkup_info.lkup_res.mact.frwrd_info.forward_decision.dest_id = 200;
        expected_frwrd_lkup_info.lkup_res.mact.aging_info.age_status = 6;
        ...
        expected_frwrd_lkup_info.lkup_res.mact.frwrd_info.forward_decision.additional_info.outlif.val = 12288;
    }
...
int 
diag_pp_egress_vlan_edit_info_vpls_test(int unit, uint32 ac_to_pwe)
{
    int rv = 0;
    SOC_PPD_DIAG_EGRESS_VLAN_EDIT_RES received_egress_vlan_edit_info;
    SOC_PPD_DIAG_EGRESS_VLAN_EDIT_RES expected_egress_vlan_edit_info;
...
int
diag_pp_lem_lkup_info_vpls_test(int unit, uint32 ac_to_pwe)
{
    int rv = 0;
    diag_pp_lem_lkup_info_fields_t received_lem_lkup_info;
    diag_pp_lem_lkup_info_fields_t expected_lem_lkup_info;
    ...
    if (ac_to_pwe){
        ...
        expected_lem_lkup_info.value.mact.frwrd_info.forward_decision.dest_id = 1024;
        ...
        expected_lem_lkup_info.value.mact.frwrd_info.forward_decision.type = SOC_PPC_FRWRD_DECISION_TYPE_UC_PORT;
        expected_lem_lkup_info.value.mact.frwrd_info.forward_decision.dest_id = 200;

        expected_lem_lkup_info.value.mact.frwrd_info.forward_decision.additional_info.outlif.type = SOC_PPC_OUTLIF_ENCODE_TYPE_RAW;
        expected_lem_lkup_info.value.mact.frwrd_info.forward_decision.additional_info.outlif.val  = 12288;
    }
...
int
diag_pp_frwrd_trace_info_vpls_test(int unit, uint32 ac_to_pwe)
{
    int rv = 0;
    SOC_PPD_DIAG_FRWRD_DECISION_TRACE_INFO received_frwrd_trace_info;
    SOC_PPD_DIAG_FRWRD_DECISION_TRACE_INFO expected_frwrd_trace_info;
    ...
    if (ac_to_pwe) {
        ...
        expected_frwrd_trace_info.frwrd[SOC_PPC_DIAG_FRWRD_DECISION_PHASE_LKUP_FOUND].frwrd_decision.type = SOC_PPC_FRWRD_DECISION_TYPE_UC_PORT;
        expected_frwrd_trace_info.frwrd[SOC_PPC_DIAG_FRWRD_DECISION_PHASE_LKUP_FOUND].frwrd_decision.dest_id = 200;
        expected_frwrd_trace_info.frwrd[SOC_PPC_DIAG_FRWRD_DECISION_PHASE_LKUP_FOUND].is_valid = 1;
    ...
    if (ac_to_pwe) {
        ...
        expected_frwrd_trace_info.frwrd[SOC_PPC_DIAG_FRWRD_DECISION_PHASE_PMF].frwrd_decision.type = SOC_PPC_FRWRD_DECISION_TYPE_UC_PORT;
        expected_frwrd_trace_info.frwrd[SOC_PPC_DIAG_FRWRD_DECISION_PHASE_PMF].frwrd_decision.dest_id = 200;
    ...
    if (ac_to_pwe) {
        ...
        expected_frwrd_trace_info.frwrd[SOC_PPC_DIAG_FRWRD_DECISION_PHASE_TRAP].frwrd_decision.type = SOC_PPC_FRWRD_DECISION_TYPE_UC_PORT;
        expected_frwrd_trace_info.frwrd[SOC_PPC_DIAG_FRWRD_DECISION_PHASE_TRAP].frwrd_decision.dest_id = 200;
    ...
    if (ac_to_pwe) {
        ...
        expected_frwrd_trace_info.frwrd[SOC_PPC_DIAG_FRWRD_DECISION_PHASE_ING_RESOLVED].frwrd_decision.type = SOC_PPC_FRWRD_DECISION_TYPE_UC_PORT;
        expected_frwrd_trace_info.frwrd[SOC_PPC_DIAG_FRWRD_DECISION_PHASE_ING_RESOLVED].frwrd_decision.dest_id = 200;
...
int
diag_pp_lif_lkup_info_vpls_test(int unit, uint32 ac_to_pwe)
{
    int rv = 0;
    diag_pp_lif_lkup_info_fields_t received_lif_lkup_info;
    diag_pp_lif_lkup_info_fields_t expected_lif_lkup_info;
    ...
    if (ac_to_pwe) {
        SOC_PPC_L2_LIF_AC_KEY_clear(&expected_lif_lkup_info.key.ac);
        expected_lif_lkup_info.type = SOC_PPC_DIAG_LIF_LKUP_TYPE_AC;
        expected_lif_lkup_info.base_index = 12288;
        ...
        expected_lif_lkup_info.key.mpls.flags = SOC_PPC_RIF_MPLS_LABEL_EXPECT_BOS;
...
diag_pp_test(int unit, args_t *a, void *p)
{
    ...
    if(dpt->bridge)
    {
        if((rv = diag_pp_received_packet_bridge_test(unit)) < 0)
        {
            cli_out("Error: diag_pp_received_packet_bridge_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_parsing_info_bridge_test(unit)) < 0)
        {
            cli_out("Error: diag_pp_parsing_info_bridge_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_termination_info_bridge_test(unit)) < 0)
        {
            cli_out("Error: diag_pp_termination_info_bridge_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_vlan_edit_info_bridge_test(unit)) < 0)
        {
            cli_out("Error: diag_pp_vlan_edit_info_bridge_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_pkt_tm_info_bridge_test(unit)) < 0)
        {
            cli_out("Error: diag_pp_pkt_tm_info_bridge_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_lif_lkup_info_bridge_test(unit)) < 0)
        {
            cli_out("Error: diag_pp_lif_lkup_info_bridge_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_frwrd_trace_info_bridge_test(unit)) < 0)
        {
            cli_out("Error: diag_pp_frwrd_trace_info_bridge_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_traps_info_bridge_test(unit)) < 0)
        {
            cli_out("Error: diag_pp_traps_info_bridge_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_encap_info_bridge_test(unit)) < 0)
        {
            cli_out("Error: diag_pp_encap_info_bridge_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_frwrd_lkup_info_bridge_test(unit)) < 0)
        {
            cli_out("Error: diag_pp_frwrd_lkup_info_bridge_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_frwrd_lpm_lkup_bridge_test(unit)) < 0)
        {
            cli_out("Error: diag_pp_frwrd_lpm_lkup_bridge_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_lem_lkup_info_bridge_test(unit)) < 0)
        {
            cli_out("Error: diag_pp_lem_lkup_bridge_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_egress_vlan_edit_info_bridge_test(unit)) < 0)
        {
            cli_out("Error: diag_pp_lem_lkup_bridge_test () Failed:\n");
            goto exit;
        }
    ...
    else if(dpt->vpls)
    {
        ...
        if((rv = diag_pp_received_packet_vpls_test(unit, ac_to_pwe)) < 0)
        {
            cli_out("Error: diag_pp_received_packet_vpls_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_parsing_info_vpls_test(unit, ac_to_pwe)) < 0)
        {
            cli_out("Error: diag_pp_parsing_info_vpls_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_termination_info_vpls_test(unit, ac_to_pwe)) < 0)
        {
            cli_out("Error: diag_pp_termination_info_vpls_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_vlan_edit_info_vpls_test(unit, ac_to_pwe)) < 0)
        {
            cli_out("Error: diag_pp_vlan_edit_info_vpls_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_lif_lkup_info_vpls_test(unit, ac_to_pwe)) < 0)
        {
            cli_out("Error: diag_pp_lif_lkup_info_vpls_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_frwrd_trace_info_vpls_test(unit, ac_to_pwe)) < 0)
        {
            cli_out("Error: diag_pp_frwrd_trace_info_vpls_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_encap_info_vpls_test(unit, ac_to_pwe)) < 0)
        {
            cli_out("Error: diag_pp_encap_info_vpls_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_frwrd_lkup_info_vpls_test(unit, ac_to_pwe)) < 0)
        {
            cli_out("Error: diag_pp_frwrd_lkup_info_vpls_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_lem_lkup_info_vpls_test(unit, ac_to_pwe)) < 0)
        {
            cli_out("Error: diag_pp_lem_lkup_info_vpls_test () Failed:\n");
            goto exit;      
        }
        if ((rv = diag_pp_egress_vlan_edit_info_vpls_test(unit, ac_to_pwe)) < 0) 
        {
