...
#ifdef BCM_ARAD_SUPPORT 
    uint8       xd_ptch_src_port;       /* PTCH src port*/
#endif /*BCM_ARAD_SUPPORT*/
...
dpp_tx_parse(int u, args_t *a, xd_t *xd)
{
    ...
    parse_table_add(&pt, "Version",  PQ_DFL|PQ_BOOL, 0, 
                    &xd->itmh_version, NULL);
    parse_table_add(&pt, "Pph_present",  PQ_DFL|PQ_BOOL, "True", 
                    &xd->itmh_pph_present, NULL);
    parse_table_add(&pt, "Out_mirr_dis",  PQ_DFL|PQ_BOOL, 0, 
                    &xd->itmh_out_mirr_dis, NULL);
    parse_table_add(&pt, "In_mirr_dis",  PQ_DFL|PQ_BOOL, 0, 
                    &xd->itmh_in_mirr_dis, NULL);
    parse_table_add(&pt, "Snoop_cmd",  PQ_DFL|PQ_INT, 0, 
                    &xd->itmh_snoop_cmd, NULL);
    parse_table_add(&pt, "Exclude_src",  PQ_DFL|PQ_BOOL, 0, 
                    &xd->itmh_exclude_src, NULL);
...
#ifdef BCM_ARAD_SUPPORT 
    xd->xd_ptch_src_port = (uint8)ptch_src_port;
#endif /*BCM_ARAD_SUPPORT*/
...
parse_user_packet_payload(int unit, char *packet_data, int *length)
{
    ...
    pkt_len = UNTAGGED_PACKET_LENGTH < data_len ? data_len : UNTAGGED_PACKET_LENGTH;
...
{
    int                     soc_sand_dev_id, rv = BCM_E_NONE;
    bcm_pkt_t               *pkt_info = &xd->pkt_info;
    soc_dpp_itmh_t          itmh;               /* itmh header contents */
    unsigned char           ext_hdr_valid;
    SOC_PETRA_PORT_HEADER_TYPE  incoming, outgoing;

    soc_sand_dev_id = (xd->xd_tx_unit);
    incoming = outgoing = SOC_PETRA_PORT_HEADER_TYPE_NONE;


#ifdef BCM_PETRAB_SUPPORT
    if (SOC_IS_PETRAB(xd->xd_tx_unit)) { 
        int soc_sand_rv=0;
        soc_sand_rv = soc_pb_port_header_type_get_unsafe(soc_sand_dev_id, 0 /* cpu port */,
                                             &incoming, &outgoing);
    ...
    if ( SOC_IS_ARAD(xd->xd_tx_unit)) { 
        rv = arad_port_header_type_get_unsafe(soc_sand_dev_id, SOC_CORE_DEFAULT,  0 /* cpu port */,
                                                &incoming, &outgoing);
    }
#endif /* BCM_ARAD_SUPPORT */
    if (BCM_FAILURE(rv) || 
        (incoming != SOC_PETRA_PORT_HEADER_TYPE_TM)) {
    ...
    sal_memcpy(&pkt_info->_dpp_hdr, &itmh.base.raw.bytes[0], 
               DPP_HDR_ITMH_BASE_LEN);
    LOG_INFO(BSL_LS_APPL_TX,
             (BSL_META("ITMH: 0x%02x-%02x-%02x-%02x \n"), pkt_info->_dpp_hdr[0],
pkt_info->_dpp_hdr[1], pkt_info->_dpp_hdr[2], pkt_info->_dpp_hdr[3]));
...

    {
        ...
        LOG_INFO(BSL_LS_APPL_TX,
                 (BSL_META("PPH: 0x%02x-%02x-%02x-%02x-%02x \n"), pph[0], pph[1], pph[2], pph[3], pph[4]));
        LOG_INFO(BSL_LS_APPL_TX,
                 (BSL_META("       %02x-%02x-%02x-%02x-%02x \n"), pph[5], pph[6], pph[7], pph[8], pph[9]));
        LOG_INFO(BSL_LS_APPL_TX,
                 (BSL_META("       %02x-%02x-%02x-%02x-%02x \n"), pph[10], pph[11], pph[12], pph[13], pph[14]));

        sal_memcpy(&pkt_info->_dpp_hdr[DPP_HDR_ITMH_BASE_LEN], &pph[0], 15);
    }
    ...
    if (SOC_IS_PETRAB(xd->xd_tx_unit)) { 
        int soc_sand_rv=0;
        soc_sand_rv = soc_pb_ports_itmh_extension_get_unsafe(soc_sand_dev_id, 0/* cpu port */,
                                                 &ext_hdr_valid);
        rv = handle_sand_result(soc_sand_rv);
    } 
#endif /* BCM_PETRAB_SUPPORT */
    ...
    if (ext_hdr_valid) {
        /* Source port extension is valid */
        pkt_info->_dpp_hdr_type = DPP_HDR_itmh_sp_ext;
        sal_memcpy(&(pkt_info->_dpp_hdr[DPP_HDR_ITMH_BASE_LEN]),
                   &itmh.ext.raw.bytes[0], 
                   (DPP_HDR_ITMH_SP_EXT_LEN - DPP_HDR_ITMH_BASE_LEN));
    }
...
dpp_do_tx(xd_t *xd)
{
    ...
    uint32             ptch_header_size = 0;
    bcm_port_config_t  port_config;
    uint8             *pkt_data_with_ptch;
    int                soc_sand_rv=0;
    int                core;
    uint32             pp_port;
#endif /*BCM_ARAD_SUPPORT*/
    ...
    if (SOC_IS_ARAD(xd->xd_unit)) {
        ...
        if ((bcm_port_config_get(0, &port_config)) != BCM_E_NONE) {
            LOG_ERROR(BSL_LS_APPL_SHELL,
...
xd->xd_tx_unit, bcm_errmsg(rv)));
        ...
        soc_sand_rv = arad_port_header_type_get((xd->xd_unit), core, port_ndx, &header_type_incoming, &header_type_outgoing);
        ...
        if ((header_type_incoming==SOC_TMC_PORT_HEADER_TYPE_INJECTED_2) ||
            ...
            pkt_data_with_ptch = (uint8 *)soc_cm_salloc(xd->xd_unit, xd->xd_pkt_len+2, "TX packet with ptch");
            if (pkt_data_with_ptch == NULL) {
                LOG_ERROR(BSL_LS_APPL_SHELL,
                          (BSL_META("Unable to allocate packet memory\n")));
                return(CMD_FAIL);
            }
            pkt_data_with_ptch[0] = 0xd0; /*setting Parser-Program-Control=1*/
            pkt_data_with_ptch[1] = xd->xd_ptch_src_port; 
            /*copy packet payload to new packet and delete old packet*/
            sal_memcpy(&pkt_data_with_ptch[2], pkt_info->pkt_data[0].data, pkt_info->pkt_data[0].len);
            soc_cm_sfree(xd->xd_unit, pkt_info->pkt_data[0].data);
            pkt_info->pkt_data[0].data = pkt_data_with_ptch;
            ep = (enet_hdr_t *)(&pkt_info->pkt_data[0].data[2]);
            ptch_header_size = 2;
        }
        else {
    ...
    payload_len = pkt_info->pkt_data[0].len - sizeof(enet_hdr_t) - 
        sizeof(uint32) /* CRC */;
#ifdef BCM_ARAD_SUPPORT        
    payload_len = payload_len - ptch_header_size;
#endif /*BCM_ARAD_SUPPORT*/
...
char cmd_dpp_tx_usage[] =
    ...
    "      Pph_present=[0|1]        - Packet processing header is present\n"
    "      Out_mirr_dis=[0|1]       - Disable out-bound port mirroring\n"
    "      In_mirr_dis=[0|1]        - Disable in-bound port mirroring\n"
    "      Snoop_cmd=<value>        - One of 15 snoop cmds \n"
    "      Exclude_src=[0|1]        - Egress control to filter if src and \n"
    "                                 dest system ports are same\n"
    ...
    "      Source_sys_port_flag=[0|1] - Source port is global or local\n"
    "      Source_sys_port=<value>  - Global sys port \n"
    "      Source_local_port=<value>- Local sys port \n"
#endif
    ;
