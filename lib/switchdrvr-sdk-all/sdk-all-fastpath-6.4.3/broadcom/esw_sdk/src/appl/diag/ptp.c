...
cmd_ptp_register(int unit, args_t *a)
{
    ...
    while (arg) {
        if (parse_cmp("ARP", arg, 0)) {
            ...
            PTP_IF_ERROR_RETURN(_bcm_ptp_register_signaling_arbiter(unit, diag_signaling_arbiter, INT_TO_PTR(ARBITER_DENY_FLAG)));
    ...
    void *user_data)
{
    ...
    time_t host_time_as_time_t = (time_t)host_time;
    struct tm *timestruct = sal_localtime(&host_time_as_time_t);
    if (verboseEvents == 2) {
