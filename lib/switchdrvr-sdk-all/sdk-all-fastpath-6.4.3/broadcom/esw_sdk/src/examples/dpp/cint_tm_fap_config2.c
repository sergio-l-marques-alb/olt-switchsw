...
/*
 * $Id: cint_tm_fap_config2.c,v 1.6 Broadcom SDK $
 *
...
int C2_DEFAULT_E2E_SCHED_FLAGS = (BCM_COSQ_GPORT_SCHEDULER | BCM_COSQ_GPORT_SCHEDULER_HR_DUAL_WFQ);
...
int tm_config2_form_handles(int port_id, int port_type)
{
    ...
    BCM_GPORT_SCHEDULER_SET( c2_gport_ucast_scheduler[idx], (c2_base_sched + (port_id * 4)) );

    BCM_GPORT_SCHEDULER_SET( c2_gport_ucast_scheduler_fq0[idx], (c2_base_fq_sched + (port_id * 16)) );
    BCM_GPORT_SCHEDULER_SET( c2_gport_ucast_scheduler_fq1[idx], (c2_base_fq_sched + (port_id * 16) + 4) );
    BCM_GPORT_SCHEDULER_SET( c2_gport_ucast_scheduler_fq2[idx], (c2_base_fq_sched + (port_id * 16) + 8) );
    BCM_GPORT_SCHEDULER_SET( c2_gport_ucast_scheduler_fq3[idx], (c2_base_fq_sched + (port_id * 16) + 12) );

    BCM_COSQ_GPORT_VOQ_CONNECTOR_SET( c2_gport_ucast_voq_connector_group0[idx], (c2_base_voq_connector + (port_id * 8)) );
    BCM_COSQ_GPORT_VOQ_CONNECTOR_SET( c2_gport_ucast_voq_connector_group1[idx], (c2_base_voq_connector + (port_id * 8) + 4) );

    BCM_GPORT_UNICAST_QUEUE_GROUP_SET( c2_gport_ucast_queue_group0[idx], (c2_base_voq + (port_id * 8)) );
    BCM_GPORT_UNICAST_QUEUE_GROUP_SET( c2_gport_ucast_queue_group1[idx], (c2_base_voq + (port_id * 8) + 4) );
...
int tm_config2_setup(int port_id, int port_type, int res_type)
{
    ...
    if (port_type == C2_FRONT_PANEL_PORT_TYPE) {
        ...
        BCM_GPORT_MODPORT_SET(dest_gport[idx], c2_ingr_unit, port);
        ...
        BCM_COSQ_GPORT_ISQ_ROOT_SET(dest_gport[idx]);
    ...
    port = BCM_GPORT_MODPORT_PORT_GET(dest_gport[idx]);    
    printf("idx(%d) dest_gport(%d) port(%d) modid(%d)\n",  idx, dest_gport[idx], port, c2_egr_modid);    
    
    BCM_GPORT_MODPORT_SET(src_gport[idx], c2_ingr_modid, 0);
    ...
    rv = bcm_cosq_gport_add(c2_egr_unit, 0 /* unused port field */, 1, 
    ...
    if (port_type == C2_ISQ_PORT_TYPE) {
        BCM_COSQ_GPORT_ISQ_ROOT_SET(c2_e2e_gport[idx]);
    } else { 
        BCM_COSQ_GPORT_E2E_PORT_SET(c2_e2e_gport[idx], port);
    }
    ...
    rv = bcm_cosq_gport_add(c2_egr_unit, 0 /* unused port */, 1, 
    ...
    rv = bcm_cosq_gport_add(c2_egr_unit, 0 /* unused */, 1, 
    ...
    rv = bcm_cosq_gport_add(c2_egr_unit, 0, 1, 
    ...
    flags = C2_CONN_FLAGS | default_flags;
    rv = bcm_cosq_gport_add(c2_egr_unit, src_gport[idx], num_cos, 
                flags, &c2_gport_ucast_voq_connector_group0[idx]);
    ...
    rv = bcm_cosq_gport_add(c2_ingr_unit, dest_gport[idx], num_cos, 
    ...
    flags = C2_CONN_FLAGS | default_flags;
    rv = bcm_cosq_gport_add(c2_egr_unit, src_gport[idx], num_cos, 
                flags, &c2_gport_ucast_voq_connector_group1[idx]);
    ...
    rv = bcm_cosq_gport_add(c2_egr_unit, dest_gport[idx], num_cos, 
