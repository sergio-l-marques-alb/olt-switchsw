/* $Id: cint_pon_ipv4_acl.c,v 1.3 Broadcom SDK $
...
* IPv4 ACL Init
*******************************************************************************/
void pon_ipv4_acl_init(bcm_port_t pon_port)
{
    int idx = 0;
...
{ 
    ...
    for (index = 0; index < num_of_n_1_services; index++)    
    {
        ...
        if (index == 0) {                        
            /* Create NNI LIF */            
            rv = nni_lif_create(unit, nni_port, nni_lif_type, 0, down_ovlan, down_ivlan, &nni_gport, &encap_id);    
            if (rv != BCM_E_NONE) {
