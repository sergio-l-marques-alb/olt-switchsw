...
/* $Id: cint_l2_learning.c,v 1.8 Broadcom SDK $
...
int l2_learning_run_single_unit(int *units_ids, int nof_units, int curr_unit){
    ...
    rv = open_ingress_mc_group_num(unit, l2_learning_info1.mc_id, ports, num_ports);
    if (rv != BCM_E_NONE) {
...
int 
open_ingress_mc_group_num(int unit, int mc_group_id, bcm_gport_t *gports, int num_of_ports) {
    ...
    for(i=0; i<len ; i++) {
        rv = bcm_multicast_ingress_add(unit, mc_group, gports[i], 0);
        if (rv != BCM_E_NONE) {
            printf("Error, in mc ingress add, mc_group=%d, dest_gport=0x%x \n", mc_group, gports[i]);
            return rv;
        }
    }
