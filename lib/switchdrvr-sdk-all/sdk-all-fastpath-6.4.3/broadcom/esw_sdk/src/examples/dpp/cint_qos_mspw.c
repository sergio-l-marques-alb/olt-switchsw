 ...
 * To Activate Above Settings Run:
   ...
   BCM> cint
   cint> pipe_mode_exp_set=0(1);
   cint> qos_map_vpls(0);
   cint> vswitch_vpls_run_with_defaults(0,15,16,pipe_mode_exp_set);
...
int is_arad_plus(int unit, int *yesno)
{
    ...
    if (rv != BCM_E_NONE) {
        printf("Error in bcm_info_get $rv\n");
        return rv;
    }
...
int mpls_pipe_mode_exp_set(int unit, int pipe_mode_exp_set) 
{
    int rv = BCM_E_NONE;
...
int mpls_tunnels_config(int unit, mpls_info_s *info, int in_tunnel_label, int out_tunnel_label) 
{
    ...
    if (rv < 0) {
        printf("Error checking whether the device is arad+ $rv.\n");
        return rv;
    }
...
int vswitch_vpls_run_with_defaults(int unit, int inPort, int outPort, int pipe_mode_exp_set) 
{
    ...
    /* set the mpls tunnel as pipe or uniform */
    rv = mpls_pipe_mode_exp_set(unit, pipe_mode_exp_set);
    if (rv != BCM_E_NONE) {
