...
/* $Id: cint_l2_learning.c,v 1.7 Broadcom SDK $
...
l2_learning_info_init(int unit, int port){
    ...
    rv = bcm_port_internal_get(unit,BCM_PORT_INTERNAL_OLP,1,&olp,&count);
    if (rv != BCM_E_NONE) {
...
int l2_learning_run_single_unit(int unit, int other_unit){
    ...
    int i = 0;
    int modid;
    int is_jericho, count;
    bcm_gport_t olp;

    rv = is_jericho(unit, &is_jericho);
    if (rv != BCM_E_NONE) {
        printf("Error, in is_jericho\n");
        return rv;
    }
    ...
    if (!is_jericho) { 
        ...
        rv = open_ingress_mc_group_num(unit, l2_learning_info1.mc_id, ports, cuds, num_ports);
        if (rv != BCM_E_NONE) {
...
int 
open_ingress_mc_group_num(int unit, int mc_group_id, bcm_gport_t *gports, int *cud, int num_of_ports) {
    ...
    /* First destroy to make sure it is not exist, no need to check error */
    ...
    for(i=0; i<len ; i++) {
        rv = bcm_multicast_ingress_add(unit, mc_group, gports[i], cud[i]);
        if (rv != BCM_E_NONE) {
            printf("Error, in mc ingress add, mc_group=%, dest_gport=0x%x \n", mc_group, gports[i]);
            return rv;
        }
    }
...
int open_ingress_mc_group_2(int unit, int mc_group_id, int port1, int port2){
    int dest_arr[2];
    int cud_arr[2];
    ...
    return open_ingress_mc_group_num(unit,mc_group_id, dest_arr, cud_arr, 2);
}

int l2_learning_run_with_defaults(int unit, int other_unit){    	
    int port = 3;
    int rv;
