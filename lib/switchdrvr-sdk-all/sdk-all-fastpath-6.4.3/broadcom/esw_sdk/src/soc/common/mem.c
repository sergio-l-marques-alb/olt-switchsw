                       ...
                       uint32 *xy_entry, uint32 *dm_entry)
{
    ...
    if ( soc_feature(unit, soc_feature_tcam_shift)) {
        if ((mem == FP_GLOBAL_MASK_TCAMm   ) ||
            ...
            /*update key part*/
            soc_mem_field_get(unit, mem, xy_entry, F1f, f1);
            soc_mem_field_get(unit, mem, xy_entry, IPBMf, ipbm);

            _soc_mem_tcam_shift(unit, f1, ipbm);

            soc_mem_field_set(unit, mem, xy_entry, F1f, f1);
            soc_mem_field_set(unit, mem, xy_entry, IPBMf, ipbm);

            /*update mask part*/
            soc_mem_field_get(unit, mem, xy_entry, F1_MASKf, f1);
            soc_mem_field_get(unit, mem, xy_entry, IPBM_MASKf, ipbm);

            _soc_mem_tcam_shift(unit, f1, ipbm);

            soc_mem_field_set(unit, mem, xy_entry, F1_MASKf, f1);
            soc_mem_field_set(unit, mem, xy_entry, IPBM_MASKf, ipbm);
        }
    }
              ...
              int copyno, int index, void *entry_data)
{
    ...
    if (ok) {
        ...
        _soc_mem_read_schan_msg_send(unit, flags, mem, copyno, index,
                      ...
                      int *index_ptr, uint32 *cache, uint8 *vmap)
{
    ...
    if (soc_feature(unit, soc_feature_shared_hash_mem) && ((mem == L2Xm) ||
        ...
        if (SOC_IS_TOMAHAWK(unit)) {
            SOC_IF_ERROR_RETURN
                (soc_tomahawk_hash_bank_bitmap_get(unit, mem, &bmap));
        } else
#endif /* BCM_TOMAHAWK_SUPPORT */
        {
            SOC_IF_ERROR_RETURN
                             ...
                             int32 bank, void *entry)
{
    ...
    case HASH_SHARED:
        ...
        if (SOC_IS_TOMAHAWK(unit)) {
            SOC_IF_ERROR_RETURN
                (soc_th_hash_bank_count_get(unit, mem, &num_banks));
        } else
