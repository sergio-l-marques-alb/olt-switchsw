...
struct pm4x10_s{
    ...
    uint8 in_pm12x10;
    portmod_port_interface_config_t port_config[PM4X10_LANES_PER_CORE];
};
...
STATIC
int pm4x10_phy_core_init(int unit, int port, pm_info_t pm_info, const portmod_port_add_info_t* add_info)
{
    ...
    core_config.interface.data_rate = add_info->interface_config.speed;
    core_config.interface.interface_modes = add_info->interface_config.interface_modes;
    core_config.interface.interface_type = add_info->interface_config.interface;
    ...
    phymod_core_init(PM_4x10_INFO(pm_info)->core_access, &core_config, &status);

    SOC_FUNC_RETURN;
}

STATIC
int _pm4x10_pm_enable(int unit, int port, pm_info_t pm_info, int port_mode, const portmod_port_add_info_t* add_info)
{
    uint32 reg_val;
    int rv, is_core_initialized = 0;
    SOC_INIT_FUNC_DEFS;

    _SOC_IF_ERR_EXIT(READ_XLPORT_MODE_REGr(unit, port, &reg_val));

    if(PM_4x10_INFO(pm_info)->in_pm12x10){
        soc_reg_field_set(unit, XLPORT_MODE_REGr, &reg_val, FIELD_10_10f, 1);
    }
    ...
    if(!is_core_initialized){
        _SOC_IF_ERR_EXIT(pm4x10_phy_core_init(unit, port, pm_info, add_info));
                                    ...
                                    int max_phys, phymod_phy_access_t* phy_access, int* nof_phys)
{
    ...
    if(params->phyn > PM_4x10_INFO(pm_info)->nof_phys){
        _SOC_EXIT_WITH_ERR(SOC_E_PARAM, (_SOC_MSG("phyn exceeded. max allowed %d. got %d"), PM_4x10_INFO(pm_info)->nof_phys - 1, params->phyn));
    }
...
int pm4x10_port_interface_config_get(int unit, int port, pm_info_t pm_info, portmod_port_interface_config_t* config)
{
    ...
    phymod_phy_interface_config_get(&phy, 0, PM_4x10_INFO(pm_info)->ref_clk, &phy_config);
...
{
    int         phy, port_mode=0, rv=0, port_index = -1, phys_count=0, i=0, nof_phys, port_i;
    ...
    if(!pm_is_active){
        /*init the PM*/
        rv = _pm4x10_pm_enable(unit, port, pm_info, port_mode, add_info);
        _SOC_IF_ERR_EXIT(rv);
    ...
    for( i = 0 ; i < PHYMOD_MAX_LANES_PER_CORE ; i++ ){
        _SOC_IF_ERR_EXIT(phymod_phy_media_type_tx_get(&phy_access, phymodMediaTypeChipToChip, &(init_config.tx[i])));
    }

    init_config.an_en = add_info->autoneg_en;
    init_config.cl72_en = add_info->link_training_en;
    for(i=0 ; i<PM4X10_LANES_PER_CORE ; i++) {
        rv = SOC_WB_ENGINE_GET_ARR(unit, SOC_WB_ENGINE_PORTMOD, pm_info->wb_vars_ids[ports], &port_i, i);
        _SOC_IF_ERR_EXIT(rv);
        ...
        sal_memcpy(&init_config.polarity, &PM_4x10_INFO(pm_info)->polarity, sizeof(init_config.polarity));
...
#ifndef __NEED_TO_FIX__
/*
bitmap = 0;
orig_reg_val=0;
reg_val = orig_reg_val;
port_index = orig_reg_val;
*/
#else
/*
...
int pm4x10_port_core_access_get(int unit, int port, pm_info_t pm_info, int phyn, int max_cores, phymod_core_access_t* core_access_arr, int* nof_cores)
{
    ...
    if(phyn > PM_4x10_INFO(pm_info)->nof_phys){
        _SOC_EXIT_WITH_ERR(SOC_E_PARAM, (_SOC_MSG("phyn exceeded. max allowed %d. got %d"), PM_4x10_INFO(pm_info)->nof_phys - 1, phyn));
    }
    if(phyn < 0)
    {
        phyn = PM_4x10_INFO(pm_info)->nof_phys - 1;
...
/*! 
 * pm4x10_frame_spacing_stretch_set
 *
 ...
 * @param [in]  spacing         - 
 */
int pm4x10_frame_spacing_stretch_set(int unit, int port, int spacing)
{
    return(xlmac_frame_spacing_stretch_set(unit, port, spacing));
}

/*! 
 * pm4x10_frame_spacing_stretch_get
 *
 * @brief Port Mac Control Spacing Stretch 
 *
 * @param [in]  unit            - unit id
 ...
 * @param [in]  spacing         - 
 */
int pm4x10_frame_spacing_stretch_get(int unit, int port, int *spacing)
{
    return (xlmac_frame_spacing_stretch_get(unit, port, spacing));
}


/*! 
 * pm4x10_diag_fifo_status_get
 *
 ...
 * @param [in]  diag_info       - 
 */
int pm4x10_diag_fifo_status_get (int unit, int port, 
 ...
 * @param [in]  pfc_cfg         - 
 */
int pm4x10_pfc_config_set (int unit, int port, 
 ...
 * @param [in]  pfc_cfg         - 
 */
int pm4x10_pfc_config_get (int unit, int port, 
                           const portmod_pfc_config_t* pfc_cfg)
{
 ...
 * @param [in]  eee             - 
 */
int pm4x10_eee_set(int unit, int port, const portmod_eee_t* eee)
{
 ...
 * @param [in]  eee             - 
 */
int pm4x10_eee_get(int unit, int port, const portmod_eee_t* eee)
{
 ...
 * @param [in]  vlan_tag        - 
 */
int pm4x10_vlan_tag_set(int unit, int port, const portmod_vlan_tag_t* vlan_tag)
{
 ...
 * @param [in]  vlan_tag        - 
 */
int pm4x10_vlan_tag_get(int unit, int port, const portmod_vlan_tag_t* vlan_tag)
{
