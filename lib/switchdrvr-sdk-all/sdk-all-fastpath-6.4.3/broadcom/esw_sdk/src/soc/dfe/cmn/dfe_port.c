...
soc_dfe_port_loopback_set(int unit, soc_port_t port, soc_dcmn_loopback_mode_t loopback)
{
    ...
    if (loopback != soc_dcmn_loopback_mode_none) {
        if (lb_start == soc_dcmn_loopback_mode_none) {
            SOCDNX_IF_ERR_EXIT(MBCM_DFE_DRIVER_CALL(unit,mbcm_dfe_port_phy_control_get,(unit, port, SOC_PHY_CONTROL_CL72, &cl72_start)));
            rc = SOC_DFE_WARM_BOOT_ARR_VAR_SET(unit, PORT_CL72_CONF, port, &cl72_start);
            SOCDNX_IF_ERR_EXIT(rc);
            if (cl72_start == 1) {
                SOCDNX_IF_ERR_EXIT(MBCM_DFE_DRIVER_CALL(unit,mbcm_dfe_port_phy_control_set,(unit, port, SOC_PHY_CONTROL_CL72, 0)));
            }
        }
    }
    ...
    if (loopback == soc_dcmn_loopback_mode_none) {
        if (lb_start != soc_dcmn_loopback_mode_none) {
            ...
            if (cl72_start == 1) {
                SOCDNX_IF_ERR_EXIT(MBCM_DFE_DRIVER_CALL(unit,mbcm_dfe_port_phy_control_set,(unit, port, SOC_PHY_CONTROL_CL72, 1)));
            }
        }
    }
