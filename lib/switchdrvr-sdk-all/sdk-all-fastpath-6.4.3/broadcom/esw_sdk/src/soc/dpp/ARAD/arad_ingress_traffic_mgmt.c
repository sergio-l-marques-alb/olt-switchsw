  ...
  if (hw_q_type == ARAD_SW_DB_QUEUE_TYPE_NOT_AVAILABLE) {
      ...
      if (SOC_IS_JERICHO(unit)) {
        ...
        SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res, 410, exit, ARAD_REG_ACCESS_ERR,
                                             READ_IPS_QUEUE_PRIORITY_TABLEm(unit, MEM_BLOCK_ANY, hw_q_type, &queue_ptiority_data));
        ...
        if (ref_count) {
            soc_mem_field_get(unit, IPS_QUEUE_PRIORITY_TABLEm, &queue_ptiority_data, QUEUE_IS_OCB_ONLYf, &is_ocb_only);
            if (is_ocb_only != info->is_ocb_only) {
                SOC_SAND_SET_ERROR_CODE(ARAD_ITM_CR_REQUEST_OCB_ONLY_ERR, 420, exit);
            }
        }
        soc_mem_field_set(unit, IPS_QUEUE_PRIORITY_TABLEm, &queue_ptiority_data, QUEUE_IS_OCB_ONLYf, (uint32*)&info->is_ocb_only);
        soc_mem_field_set(unit, IPS_QUEUE_PRIORITY_TABLEm, &queue_ptiority_data, QUEUE_IS_HIGH_PRIORITYf, (uint32*)&info->is_high_priority);
        SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res, 430, exit, ARAD_REG_ACCESS_ERR,
                                             WRITE_IPS_QUEUE_PRIORITY_TABLEm(unit, MEM_BLOCK_ANY, hw_q_type, &queue_ptiority_data));
        ...
        for (slow_level = 0; slow_level < SOC_TMC_ITM_CR_SLOW_LEVELS; slow_level++) {
            soc_mem_field_set(unit,
                              IPS_SLOW_FACTOR_THRESHOLDS_TABLEm,
        ...
        SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res, 430, exit, ARAD_REG_ACCESS_ERR,
                                             WRITE_IPS_SLOW_FACTOR_THRESHOLDS_TABLEm(unit, MEM_BLOCK_ANY, hw_q_type, slow_thresholds_data));
  ...
  )
{
  ...
  if (SOC_IS_JERICHO(unit)) {
      ...
      SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res, 420, exit, ARAD_REG_ACCESS_ERR,
                                         READ_IPS_QUEUE_PRIORITY_TABLEm(unit, MEM_BLOCK_ANY, hw_q_type, &queue_ptiority_data));
      soc_mem_field_get(unit, IPS_QUEUE_PRIORITY_TABLEm, &queue_ptiority_data, QUEUE_IS_OCB_ONLYf, (uint32*)&info->is_ocb_only);
      soc_mem_field_get(unit, IPS_QUEUE_PRIORITY_TABLEm, &queue_ptiority_data, QUEUE_IS_HIGH_PRIORITYf, (uint32*)&info->is_high_priority);

      sal_memset(slow_thresholds_data, 0x0, sizeof(slow_thresholds_data));
      SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res, 430, exit, ARAD_REG_ACCESS_ERR,
                                           READ_IPS_SLOW_FACTOR_THRESHOLDS_TABLEm(unit, MEM_BLOCK_ANY, hw_q_type, slow_thresholds_data));
      for (slow_level = 0; slow_level < SOC_TMC_ITM_CR_SLOW_LEVELS; slow_level++) {
