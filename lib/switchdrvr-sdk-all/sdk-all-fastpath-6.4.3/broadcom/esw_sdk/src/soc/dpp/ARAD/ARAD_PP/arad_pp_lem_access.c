...
#define ARAD_PP_LEM_ACCESS_KEY_TYPE_IN_BITS_4                   (4)
...
#define ARAD_PP_LEM_LIF_VALID_LSB (SOC_IS_JERICHO(unit)?37:35)
       ...
       SOC_SAND_OUT  uint32                        *prefix)
{
    ...
    *prefix = ARAD_PP_FLP_MAP_PROG_NOT_SET;
    ...
    for (indx = 0; indx < ARAD_PP_LEM_ACCESS_NOF_PREFIXES; ++indx) {
        res = arad_pp_sw_db_lem_prefix_to_app_get(unit,indx,&cur_app_id);
       ...
       SOC_SAND_OUT  uint32                        *app_type)
{
    ...
    /* remove */
    res = arad_pp_sw_db_lem_prefix_to_app_get(unit,key_type_msb,&cur_app_id);
    SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
    ...
    )
{
    ...
    if (flags & ARAD_PP_LEM_ACCESS_PREFIX_ALLOC_WITH_ID) {
        ...
        for (i = 0; i < key_type_msb_nof; i++) {
            ...
            res = arad_pp_sw_db_lem_prefix_to_app_get(unit, key_type_msb_tmp, &cur_app_id);
    ...
    /* set as used */
    for ( i = 0; i < key_type_msb_nof; i++) {
        res = arad_pp_sw_db_lem_prefix_to_app_set(unit, (key_type_msb_lcl + i) ,app_id);
    ...
    )
{
  ...
  age_without_aged_bit = age_fld >> 1;
  ...
  else
  {
    ...
    switch (age_without_aged_bit)
      ...
      case 5:
      case 6:      
          age_lcl = age_without_aged_bit - 1;
          break;
    ...
    )
{
  ...
  if(entry_type == ARAD_PP_LEM_ENTRY_TYPE_OUTLIF)
  {
      ...
      if (is_destination_fec) {
          /* destination is FEC */
          arad_pp_sw_db_fec_entry_type_get(unit, payload->dest & 0xffff, &fec_type);
      }
  ...
  else if(entry_type == ARAD_PP_LEM_ENTRY_TYPE_EEI_FEC)
  {
      ...
      SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
      /* encode as FEC */
      payload->dest =  ((SOC_IS_JERICHO(unit)?2:4) << SOC_DPP_DEFS_GET(unit, fec_nof_bits) )| tmp;
  ...
  else if(entry_type == ARAD_PP_LEM_ENTRY_TYPE_EEI_MC)
  {
      ...
      SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
      /* encode as MC */
      payload->dest =  (5 << SOC_DPP_DEFS_GET(unit, multicast_group_nof_bits) )| tmp;
   ...
   */
      ...
      if (SOC_IS_JERICHO(unit)) {
          ...
          payload->asd = (ARAD_PP_EEI_IDENTIFIER_ENCAP_VAL_2_MSBS << ARAD_PP_ASD_BIT_POSITION_IDENTIFIER_2_MSBS) 
  ...
  )
{
  ...
  /* entry is of EEI type */
      ...
      if(payload->dest >> SOC_DPP_DEFS_GET(unit, multicast_group_nof_bits) == 5) {
          entry_type = ARAD_PP_LEM_ENTRY_TYPE_EEI_MC;
      }
      else
      {
  ...
  else if (entry_type == ARAD_PP_LEM_ENTRY_TYPE_FORMAT_3B) 
  {
      ...
      ARAD_PP_LEM_ACCESS_HI_FROM_EEI(hi_value, payload->asd);
...
if (key_type_msb != ARAD_PP_LEM_ACCESS_KEY_PREFIX_FOR_SA_AUTH)
  {
      ...
      res = soc_sand_bitstream_get_any_field(
              key_in_buffer->buffer,
              SOC_DPP_DEFS_GET(unit, lem_width) - ARAD_PP_LEM_ACCESS_KEY_TYPE_IN_BITS_4,
              ARAD_PP_LEM_ACCESS_KEY_TYPE_IN_BITS_4,
              &(key_type_msb)
            );
      SOC_SAND_CHECK_FUNC_RESULT(res, 15, exit);
  }
    ...
    )
{
  ...
  ARAD_PP_LEM_ACCESS_KEY_ENCODED
    key_in_buffer;
  uint32
    key[SOC_DPP_DEFS_MAX_LEM_WIDTH_IN_UINT32S],
  ...
  *is_poll_success = poll_success;
   ...
   */
    ...
    if (SOC_DPP_IS_EM_SIM_ENABLE(unit)) {
      ...
      if (request_all->request.command == ARAD_PP_LEM_ACCESS_CMD_DELETE)
      {
        /*
         *  Deletion command
         */
  ...
  )
{
  ...
  {
    ack_status->is_success = FALSE;
    /* Update ack status reason only in case it wasnt fill untill now */
...
#ifdef PLISIM
  uint32
    key_in[SOC_DPP_DEFS_MAX_LEM_WIDTH_IN_UINT32S],
...
*/
  ...
  if (SAL_BOOT_PLISIM) {
      ...
      *is_found = success;
      COMPILER_64_SET(reg_val1, data_out[0], data_out[1]);
      
      /*
      ARAD_FLD_FROM_REG64(PPDB_B_LARGE_EM_CPU_REQUEST_REQUESTr, MACT_REQ_PAYLOADf, fld_val64, reg_val1, 205, exit);
  ...
  )
{
  ...
  *asd_lcl = 0;
  ...
  res = soc_sand_bitstream_set_any_field(
          ...
          ARAD_PP_LEM_ACCESS_ASD_TRILL_SA_AUTH_EEP_LEN,
          asd_lcl
        );
  SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
...
/* in ipv4 single hop extended mode the LEM is used as the clasiffier
...
int arad_pp_bfd_ipv4_1_hop_lem_entry_add(int unit,int local_discriminator, int is_accelerated) {
...
int arad_pp_bfd_ipv4_1_hop_lem_entry_remove(int unit,int local_discriminator) {
