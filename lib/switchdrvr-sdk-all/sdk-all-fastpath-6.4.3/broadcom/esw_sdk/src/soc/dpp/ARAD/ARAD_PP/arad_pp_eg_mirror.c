  ...
  )
{
    ...
    if ((!SOC_IS_JERICHO(unit))) {
        SOCDNX_SAND_IF_ERR_EXIT(arad_port_ingr_map_write_val_unsafe(unit,context,is_mapped,map_info));
    }
    else {

        if (is_mapped) {
            SOCDNX_IF_ERR_EXIT(soc_jer_port_to_interface_ingress_map_set(unit,context));
        }
        else {
            SOCDNX_IF_ERR_EXIT(soc_jer_port_to_interface_ingress_unmap_set(unit,context));
     ...
     SOC_SAND_OUT uint32 *base_address)
{
    ...
    field =(core == 0 ? INTERFACE_EGQ_0_PORTf : INTERFACE_EGQ_1_PORTf);
...
/*
 * for a given port,channel return it recycling/termination port from the context map
*/
...
uint32 
 get_recycling_port(
     ...
     SOC_SAND_OUT uint32 *logical_port)
{
    uint32 base_address,index,data,tm_port;
    int core=0; 
    SOCDNX_INIT_FUNC_DEFS;

    SOCDNX_IF_ERR_EXIT(get_context_map_base_address(unit, port,&base_address));
    ...
    SOCDNX_IF_ERR_EXIT(soc_port_sw_db_tm_to_local_port_get(unit, core, tm_port, (soc_port_t *)logical_port));
...
/* copy group of  bits from uint8  var into a specific location on a memory buffer*/
STATIC 
void uint8_2_mem(
    SOC_SAND_OUT uint32 *dst,
    SOC_SAND_IN uint32 dst_start,
    SOC_SAND_IN uint8 *src,
    SOC_SAND_IN uint32 src_start,
    SOC_SAND_IN uint32 len)
{
    uint32 src32 = *src;
    SHR_BITCOPY_RANGE(dst,dst_start,&src32,src_start,len);

}

/* copy  group of bits  bits from a specific location on a memory buffer  into  uint8  var*/

STATIC void mem_to_uint8(
    SOC_SAND_OUT uint8 *dst,
    SOC_SAND_IN uint32 dst_start,
...
{
    uint32 dst32;
    SHR_BITCOPY_RANGE((&dst32),dst_start,src,src_start,len);
    *dst =  (uint8)dst32;
     ...
     )
{
    ...
    uint32 mirror_command_lcl[1];
    uint32 entry_start, start,length,table_offset,entry[]={0,0,0};
    int core;
    uint32 pp_port;
    ...
    table_offset = ARAD_PP_EG_MIRROR_PROFILE_TABLE_CALC_ENTRY_OFFSET(pp_port, vid);
    SOCDNX_IF_ERR_EXIT(READ_EPNI_MIRROR_PROFILE_TABLEm(unit, EPNI_BLOCK(unit, core), table_offset, entry));

    entry_start = DPP_EG_MIRROR_PROFILE_TABLE_ENTRY_SHIFT(pp_port, vid);

    start = entry_start + DPP_EG_MIRROR_PROFILE_TABLE_MIRROR_COMMAND_LENGTH_START;
    length = DPP_EG_MIRROR_PROFILE_TABLE_MIRROR_COMMAND_LENGTH_LENGTH;
    *mirror_command_lcl=0;
    SHR_BITCOPY_RANGE(mirror_command_lcl, 0, entry, start, length);
    config->mirror_command = *mirror_command_lcl;
    if (!SOC_IS_JERICHO(unit)) {
        SOC_EXIT;
    }
    start = entry_start + JER_EG_MIRROR_PROFILE_TABLE_FWD_STRENGTH_START; 
    length = JER_EG_MIRROR_PROFILE_TABLE_FWD_STRENGTH_LENGTH;
    mem_to_uint8(&(config->forward_strength), 0, entry, start, length);

    start = entry_start + JER_EG_MIRROR_PROFILE_TABLE_MIRROR_STRENGTH_START;
    length = JER_EG_MIRROR_PROFILE_TABLE_MIRROR_STRENGTH_LENGTH;
    mem_to_uint8(&(config->mirror_strength), 0, entry, start, length);

    start = entry_start + JER_EG_MIRROR_PROFILE_TABLE_FWD_EN_START;
    length = JER_EG_MIRROR_PROFILE_TABLE_FWD_EN_LENGTH;
    mem_to_uint8(&(config->forward_strength), 0, entry, start, length);


    start = entry_start + JER_EG_MIRROR_PROFILE_TABLE_MIRROR_EN_START;
    length = JER_EG_MIRROR_PROFILE_TABLE_MIRROR_EN_LENGTH;
    mem_to_uint8(&(config->mirror_en), 0, entry, start, length);
     ...
     )
{
    ...
    uint32 entry_start, start, length;
    ...
    if ((read_first)) {
         SOCDNX_IF_ERR_EXIT(READ_EPNI_MIRROR_PROFILE_TABLEm(unit, EPNI_BLOCK(unit,core), index, entry));
    }
    entry_start = DPP_EG_MIRROR_PROFILE_TABLE_ENTRY_SHIFT(pp_port, vid);
    
    start = entry_start + DPP_EG_MIRROR_PROFILE_TABLE_MIRROR_COMMAND_LENGTH_START;
    length = DPP_EG_MIRROR_PROFILE_TABLE_MIRROR_COMMAND_LENGTH_LENGTH;
    SHR_BITCOPY_RANGE( entry, start, &(config->mirror_command), 0,length);

    if (SOC_IS_JERICHO(unit)) {
       
        start = DPP_EG_MIRROR_PROFILE_TABLE_ENTRY_SHIFT(pp_port, vid) + JER_EG_MIRROR_PROFILE_TABLE_FWD_STRENGTH_START;
        length = JER_EG_MIRROR_PROFILE_TABLE_FWD_STRENGTH_LENGTH;
        uint8_2_mem(entry, start, &(config->forward_strength), 0, length);

        start = DPP_EG_MIRROR_PROFILE_TABLE_ENTRY_SHIFT(pp_port, vid) + JER_EG_MIRROR_PROFILE_TABLE_MIRROR_STRENGTH_START;
        length = JER_EG_MIRROR_PROFILE_TABLE_MIRROR_STRENGTH_LENGTH;
        uint8_2_mem(entry, start, &(config->mirror_strength), 0, length);

        start = DPP_EG_MIRROR_PROFILE_TABLE_ENTRY_SHIFT(pp_port, vid) + JER_EG_MIRROR_PROFILE_TABLE_FWD_EN_START;
        length = JER_EG_MIRROR_PROFILE_TABLE_FWD_EN_LENGTH;
        uint8_2_mem( entry, start, &(config->forward_en), 0, length);


        start = DPP_EG_MIRROR_PROFILE_TABLE_ENTRY_SHIFT(pp_port, vid) + JER_EG_MIRROR_PROFILE_TABLE_MIRROR_EN_START;
        length = JER_EG_MIRROR_PROFILE_TABLE_MIRROR_EN_LENGTH;
        uint8_2_mem( entry,start, &(config->mirror_en), 0, length);
    }

    SOCDNX_IF_ERR_EXIT(WRITE_EPNI_MIRROR_PROFILE_TABLEm(unit,  EPNI_BLOCK(unit, core), index, entry));
  ...
  )
{
  ...
  {
      SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res, 10, exit, ARAD_REG_ACCESS_ERR,WRITE_IRR_MIRROR_SIZEr(unit, 0x3fffffff)); 
  ...
  )
{
  ...
  res = soc_port_sw_db_local_to_pp_port_get(unit, out_port_ndx, &pp_port, &core);
  SOC_SAND_CHECK_FUNC_RESULT(res, 4, exit); /* error if local port translation failed */

  res = soc_port_sw_db_pp_port_to_out_port_priority_get(unit, core, pp_port, &end_q_pair);
  ...
  if (first_appear) { /* New VLAN, a VID was just created for it, add it to hardware */
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res, 30, exit, ARAD_REG_ACCESS_ERR,
      READ_EPNI_MIRROR_VID_REGr(unit, core, reg_above_64_val)); /* read current hardware mapping */
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res, 40, exit, ARAD_REG_ACCESS_ERR,
      WRITE_EPNI_MIRROR_VID_REGr(unit, core, reg_above_64_val)); /* write the modified hardware mapping */
  }
  ...
  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res, 50, exit, ARAD_REG_ACCESS_ERR, READ_EGQ_PCTm(unit, EGQ_BLOCK(unit ,core), base_q_pair, entry));
  ...
  if (!soc_mem_field32_get(unit, EGQ_PCTm, entry, MIRROR_ENABLEf)) { /* if disabled, need to allocate and set a context */
    ...
    res = alloc_reassembly_context_and_recycle_channel_unsafe(unit, core, pp_port, &context, &channel); /* allocate reassembly context */
    ...
    for (q_pair = base_q_pair; q_pair < end_q_pair; ++q_pair) {
      if (q_pair > base_q_pair) { /* entry for base_q_pair already read */
        SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res, 70, exit, ARAD_REG_ACCESS_ERR,
          READ_EGQ_PCTm(unit, EGQ_BLOCK(unit ,core), q_pair, entry));
      }
      ...
      SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res, 80, exit, ARAD_REG_ACCESS_ERR,
        WRITE_EGQ_PCTm(unit, EGQ_BLOCK(unit ,core), q_pair, entry));
    }
    ...
    res = map_mirrored_port(unit, out_port_ndx, context, TRUE, &port_mapping_info);
    SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 90, exit); /* error if no port / reassembly context is available */
  }
  ...
  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  130, exit, ARAD_REG_ACCESS_ERR,
    READ_EPNI_MIRROR_PROFILE_TABLEm(unit, EPNI_BLOCK(unit ,core), mirror_profile_table_offset, entry));
  ...
  /* Add to SW */
  res = arad_pp_sw_db_eg_mirror_port_vlan_is_exist_set(unit, pp_port, internal_vid_ndx, TRUE);
...
exit:
  ...
  if (ex != no_err) { /* cleanup on error */
    if (clean_stage & 2) { /* deallocate reassembly context */
      for (q_pair = base_q_pair; q_pair < end_q_pair; ++q_pair) {
        if (READ_EGQ_PCTm(unit, EGQ_BLOCK(unit ,core), q_pair, entry) == SOC_E_NONE) {                                    \
          soc_mem_field32_set(unit, EGQ_PCTm, entry, MIRROR_ENABLEf, 0);
          SOC_SAND_SOC_IF_ERROR_RETURN(res, 160, exit, WRITE_EGQ_PCTm(unit, EGQ_BLOCK(unit ,core), q_pair, entry));
        }
      }
    ...
    if (clean_stage & 1) { /* decrease count of and possibly deallocate vlan index */
      ...
      if (first_appear) { /* Remove from HW */
        soc_reg_above_64_val_t fld_value = {0}, reg_above_64_val;
        if (READ_EPNI_MIRROR_VID_REGr(unit, core, reg_above_64_val) == SOC_E_NONE) { /* read current hardware mapping */
          fld_value[0] = ARAD_PP_EG_MIRROR_VID_NOT_IN_USE; /* set hardware mapping from the removed index to not mapped */
          soc_reg_above_64_field_set(unit, EPNI_MIRROR_VID_REGr, reg_above_64_val, arad_pp_eg_mirror_vlan_id_flds[internal_vid_ndx], fld_value);
          res = WRITE_EPNI_MIRROR_VID_REGr(unit, core, reg_above_64_val); /* write the modified hardware mapping */
        }
      }
    }
  }
  ...
  )
{
  ...
  if (ref_count) { /* If the vid was found */
    ...
    /* Remove from SW */
    res = arad_pp_sw_db_eg_mirror_port_vlan_is_exist_set(unit, pp_port, internal_vid_ndx, FALSE);
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res, 80, exit, ARAD_REG_ACCESS_ERR,
                                         dpp_outbound_mirror_config_write(unit,out_port_ndx, internal_vid_ndx, mirror_profile_table_offset,TRUE,&config ));
    ...
    for (internal_vid_ndx = 0; internal_vid_ndx < ARAD_PP_EG_MIRROR_VID_DFLT_NDX; ++internal_vid_ndx) {
      res = arad_pp_sw_db_eg_mirror_port_vlan_is_exist_get(unit, pp_port, internal_vid_ndx, &last_appear);
    ...
    if (!last_appear && !(config.mirror_command)) { /* no vlan id nor the default are mirrored for the port */
      ...
      res = soc_port_sw_db_pp_port_to_base_q_pair_get(unit,core, pp_port,&q_pair);
      SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 195, exit);
      res = soc_port_sw_db_pp_port_to_out_port_priority_get(unit, core, pp_port, &nof_priorities);
      ...
      SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res, 221, exit, ARAD_REG_ACCESS_ERR,get_recycling_port(unit,out_port_ndx, channel, &context));

      /* remove recycle interface channel to reassembly context (port) mapping */
      port_mapping_info.if_id = SOC_TMC_IF_ID_RCY;
      port_mapping_info.channel_id = channel;
      res = map_mirrored_port(unit, out_port_ndx, context, FALSE, &port_mapping_info);
  ...
  )
{
  ...
  if (ref_count) { /* If the vid was found, and we have an index for it */
    res = arad_pp_sw_db_eg_mirror_port_vlan_is_exist_get(unit, pp_port, internal_vid_ndx, &port_vlan_exists);
    SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
    if (port_vlan_exists) { /* If port-vlan exists in the SWDB */
      ...
      SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res, 
                                           ...
                                           exit, 
                                           ARAD_REG_ACCESS_ERR, 
                                           dpp_outbound_mirror_config_read(unit, out_port_ndx, internal_vid_ndx, config));
    }
  }
  ...
  )
{
  ...
  *prev_profile =  local_config.mirror_command;
  ...
  if (config->mirror_command && !(*prev_profile)) { /* port default will start being mirrored */
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res, 30, exit, ARAD_REG_ACCESS_ERR,
      READ_EGQ_PCTm(unit, EGQ_BLOCK(unit,core), base_q_pair, entry_context));
      ...
      for (q_pair = base_q_pair; q_pair < end_q_pair; ++q_pair) {
        if (q_pair > base_q_pair) { /* entry for base_q_pair already read */
          SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res, 50, exit, ARAD_REG_ACCESS_ERR,
            READ_EGQ_PCTm(unit, EGQ_BLOCK(unit,core), q_pair, entry_context));
        }
        ...
        SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res, 60, exit, ARAD_REG_ACCESS_ERR,
          WRITE_EGQ_PCTm(unit, EGQ_BLOCK(unit,core), q_pair, entry_context));
      }
      /* map recycle interface channel to reassembly context (port) */
      port_mapping_info.if_id = SOC_TMC_IF_ID_RCY;
      port_mapping_info.channel_id = channel;
      res = map_mirrored_port(unit, local_port_ndx, context, TRUE, &port_mapping_info);
      SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 70, exit); /* error if no port / reassembly context is available */
    }
  }
  ...
  if (is_disable && !has_mirrored_vlans) {
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res, 221, exit, ARAD_REG_ACCESS_ERR,get_recycling_port(unit,local_port_ndx, channel, &context));

    /* remove recycle interface channel to reassembly context (port) mapping */
    port_mapping_info.if_id = SOC_TMC_IF_ID_RCY;
    port_mapping_info.channel_id = channel;
    res = map_mirrored_port(unit, local_port_ndx, context, FALSE, &port_mapping_info);
    SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 240, exit);
    SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res, 250, exit, ARAD_REG_ACCESS_ERR,
      enabled ? SOC_E_NONE : SOC_E_INTERNAL); /* if disabled for any queue pair, it is an internal error */
  ...
  )
{
  ...
  res = soc_port_sw_db_local_to_pp_port_get(unit, local_port_ndx, &pp_port, &core);
  SOC_SAND_CHECK_FUNC_RESULT(res, 5, exit); /* error if local port translation failed */
  ...
  if (is_reserved != (enable ? 1 : 0)) { /* need to do work only if the wanted state is different from the current one */
    ...
    if (enable) { /* reserve the mapping, if a reassembly context + channel are not allocated, allocate and set them */
      ...
      SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res, 30, exit, ARAD_REG_ACCESS_ERR,
        READ_EGQ_PCTm(unit, EGQ_BLOCK(unit,core), base_q_pair, entry));
      ...
      if (!(config.mirror_command)) {
        ...
        for (internal_vid_ndx = 0; internal_vid_ndx < ARAD_PP_EG_MIRROR_VID_DFLT_NDX; ++internal_vid_ndx) {
          if (!SOC_DPP_PP_ENABLE(unit)) {
              ...
              res = arad_pp_sw_db_eg_mirror_port_vlan_is_exist_get(unit, pp_port, internal_vid_ndx, &port_vlan_exist);
              SOC_SAND_CHECK_FUNC_RESULT(res, 110, exit);
          }
          if (port_vlan_exist) {
            break;
  ...
  )
{
  ...
  else
  {
    ...
    info->outbound_port_ndx =  context ; /*soc_mem_field32_get(unit,  get_context_map_id(unit), &context, PORT_TERMINATION_CONTEXTf);    */
  }
