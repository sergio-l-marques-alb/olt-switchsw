...
soc_jer_lbp_init(SOC_SAND_IN int unit) {
    ...
    /* set in HW */
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 20, exit, WRITE_IHB_OUT_RIF_RANGEr(unit, REG_PORT_ANY, SOC_DPP_DEFS_GET(unit, nof_rifs))); 
...
* |                         INDEX                            ||       VALUE     |    Note               | 
* ------------------------------------------------------------------------------------------------------| 
* |  Forwarding code     | inLif ==0  |  Forwarding-Offset   ||  fhei size      |                       | 
* |      (4b)            |    (1b)    |  -Index (3b)         ||     (2b)        |                       | 
* ------------------------------------------------------------------------------------------------------| 
*/
...
#define SOC_JER_PP_LBP_INIT_PPH_FHEI_FWD_SIZE_INDEX_FWD_OFFSET_INDEX_LSB (0)
#define SOC_JER_PP_LBP_INIT_PPH_FHEI_FWD_SIZE_INDEX_INLIF_EQUAL_0_LSB    (SOC_JER_PP_LBP_INIT_PPH_FHEI_FWD_SIZE_INDEX_FWD_OFFSET_INDEX_LSB + 3)
#define SOC_JER_PP_LBP_INIT_PPH_FHEI_FWD_SIZE_INDEX_FWD_CODE             (SOC_JER_PP_LBP_INIT_PPH_FHEI_FWD_SIZE_INDEX_INLIF_EQUAL_0_LSB + 1)

/* get the index according to index fields */
#define SOC_JER_PP_LBP_INIT_PPH_FHEI_FWD_SIZE_INDEX_GET(fwd_code, inLif_equal_0, fwd_offset_index) \
...
/* get the offset of the register according to index */
#define SOC_JER_PP_LBP_INIT_PPH_FHEI_FWD_SIZE_OFFSET_GET(fwd_code, inLif_equal_0, fwd_offset_index) \
         (ARAD_IHB_PPH_FHEI_FWD_SIZE_NOF_BITS * SOC_JER_PP_LBP_INIT_PPH_FHEI_FWD_SIZE_INDEX_GET(fwd_code, inLif_equal_0, fwd_offset_index))
...
soc_jer_php_fhei_fwd_size_init(SOC_SAND_IN int unit) {
      ...
      * ------------------------------------------------------------------------------------------------------| 
      * | Eth, IPvX-XC,        |            |                      ||                 |                       |  
      * | Trill, CPU-trap,     |  X         |         X            ||      3B         | Arad value            | 
      * | eth-after-termination|            |                      ||                 |                       | 
      * ------------------------------------------------------------------------------------------------------|
      * | others               |  X         |         X            ||      0B         | Arad value            | 
      * ------------------------------------------------------------------------------------------------------| */
             ...
             for (fwd_offset_index=0;fwd_offset_index <= SOC_JER_PP_LBP_INIT_PPH_FHEI_FWD_SIZE_INDEX_FWD_OFFSET_INDEX_MAX; fwd_offset_index++) {
                 SHR_BITCOPY_RANGE(pph_fhei_fwd_size, 
                                   SOC_JER_PP_LBP_INIT_PPH_FHEI_FWD_SIZE_OFFSET_GET(
                                        ...
                                        boolean_values[inLif_equal_0_index], fwd_offset_index), 
         ...
         if ((fwd_code != ARAD_PP_FWD_CODE_ETHERNET) && (fwd_code != ARAD_PP_FWD_CODE_IPV4_UC) 
            ...
            for(inLif_equal_0_index = 0; inLif_equal_0_index < 2; inLif_equal_0_index++) {
                for (fwd_offset_index=0;fwd_offset_index < SOC_JER_PP_LBP_INIT_PPH_FHEI_FWD_SIZE_INDEX_FWD_OFFSET_INDEX_MAX; fwd_offset_index++) {
                    SHR_BITCOPY_RANGE(pph_fhei_fwd_size, 
                                      SOC_JER_PP_LBP_INIT_PPH_FHEI_FWD_SIZE_OFFSET_GET(
                                         fwd_code, boolean_values[inLif_equal_0_index], fwd_offset_index), 
                                      &value, 0, ARAD_IHB_PPH_FHEI_FWD_SIZE_NOF_BITS); 
                }
            }
         }
     ...
     if (system_header_mode == ARAD_PP_SYSTEM_HEADERS_MODE_JERICHO) {
          ...
          * |      (4b)            |    (1b)    |  -Index (3b)         ||     (2b)        |                                   | 
          * ------------------------------------------------------------------------------------------------------------------|   
          * |  IPvX-MC             |  1         |         2            ||      3B         | Routing over dot1q (like Arad)    | 
          * ------------------------------------------------------------------------------------------------------------------|    
          * |  IPvX-MC             |  X         |         3            ||      3B         | Routing out of tunnel (like Arad) | 
          * ------------------------------------------------------------------------------------------------------------------|  
          * |  IPvX-MC             |  X         |         4            ||      8B         | ROO                               | 
          * ------------------------------------------------------------------------------------------------------------------|  
          * |  IPvX-MC             |  0         |         2            ||      8B         | Routing over VPN                  | 
          * ------------------------------------------------------------------------------------------------------------------|    */
         ...
         for (fwd_code_index=0;fwd_code_index<2;fwd_code_index++) {
             ...
             * ------------------------------------------------------------------------------------------------------| */
             ...
             for (inLif_equal_0_index=0;inLif_equal_0_index<2;inLif_equal_0_index++) {
                 SHR_BITCOPY_RANGE(pph_fhei_fwd_size, 
                                   SOC_JER_PP_LBP_INIT_PPH_FHEI_FWD_SIZE_OFFSET_GET(
                                      fwd_codes_IPvX_MC[fwd_code_index], boolean_values[inLif_equal_0_index], 4), 
                                   &value, 0, ARAD_IHB_PPH_FHEI_FWD_SIZE_NOF_BITS); 
             }
            /* ------------------------------------------------------------------------------------------------------| 
             * |                         INDEX                            ||       VALUE     |    Note               | 
             ...
             * ------------------------------------------------------------------------------------------------------| */
             SHR_BITCOPY_RANGE(pph_fhei_fwd_size, 
                                  SOC_JER_PP_LBP_INIT_PPH_FHEI_FWD_SIZE_OFFSET_GET(
                                  fwd_codes_IPvX_MC[fwd_code_index], 0, 2), 
                               &value, 0, ARAD_IHB_PPH_FHEI_FWD_SIZE_NOF_BITS); 
         }
     }
