  ...
  )
{
    ...
    /*initialize features unique to arad*/
  ...
  )
{
  ...
  int i;
    ...
    fbr_nof_macs = SOC_DPP_DEFS_GET(unit, nof_fabric_macs);
  ...
  )
{
  ...
  if (rx_enable) {
      ...
      soc_reg_field_set(unit, SCH_RCI_PARAMSr, &reg_val,FAP_RCI_ENAf, rx_field);
  ...
  )
{
  ...
  fmac_index = SOC_SAND_DIV_ROUND_DOWN(link_ndx,SOC_DPP_DEFS_GET(unit, nof_fabric_macs));
  blk_id = link_ndx % SOC_DPP_DEFS_GET(unit, nof_fabric_macs);
       ...
       )
{
  struct values_table_entry
      {
          int speed;
          uint32 value[SOC_TMC_PORT_PCS_NOF_TYPES];
      };
  ...
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_fabric_is_mesh()",0,0);
}
       ...
       )
{
    ...
    SOC_SAND_CHECK_FUNC_RESULT(_arad_fabric_mesh_value1_get(unit, &value_1, speed , pcs), 10, exit);
    SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  20,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, MESH_TOPOLOGY_REG_010Cr, REG_PORT_ANY, 0, FIELD_0_10f,  value_1));
    if(value_1 != 0){
        value_1++;
    }
    SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  30,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, MESH_TOPOLOGY_SYNC_MSG_RX_ADJ_FACTORr, REG_PORT_ANY, 0, SYNC_MSG_RX_ADJ_FACTORf,  value_1));    
       ...
       )
{
  ...
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 10, exit, READ_RTP_RTP_ENABLEr(unit,&reg_val32));
  soc_reg_field_set(unit, RTP_RTP_ENABLEr, &reg_val32,EN_LOCAL_LINK_REDUCTION_MCf, field_val);

  SOC_SAND_SOC_IF_ERROR_RETURN(res, 20, exit, WRITE_RTP_RTP_ENABLEr(unit,reg_val32));
  ...
  case ARAD_FABRIC_CONNECT_MODE_BACK2BACK:
    ...
    PBMP_SFI_ITER(unit, port)
    {
      ...
      if(speed != -1)
      {
        ...
        if(speed == 0)
        {
          SOC_SAND_CHECK_FUNC_RESULT(arad_port_speed_max(unit, port, &speed) , 50 , exit);
        }
        SOC_SAND_CHECK_FUNC_RESULT(arad_port_control_pcs_get(unit, SOC_DPP_FABRIC_PORT_TO_LINK(unit, port), &pcs), 52, exit);
        SOC_SAND_CHECK_FUNC_RESULT(_arad_fabric_mesh_value1_get(unit, &value_1 , speed, pcs), 52, exit);
        break;
      }
    }
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_LINK_ON_OFF_SET_UNSAFE);

  SOC_SAND_CHECK_NULL_INPUT(info);

  
  port = SOC_DPP_FABRIC_LINK_TO_PORT(unit, link_ndx);
  ...
  {  
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 1230, exit, READ_FMAC_RECEIVE_RESET_REGISTERr(unit, blk_id, &reg_val));
    ...
    *tx_field = soc_reg_field_get(unit, FMAC_RECEIVE_RESET_REGISTERr, reg_val, FMAC_TX_RST_Nf);
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 1240, exit, WRITE_FMAC_RECEIVE_RESET_REGISTERr(unit, blk_id, reg_val)); 
      
    /* RTP */
    SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  20,  exit, ARAD_REG_ACCESS_ERR,READ_RTP_ALLOWED_LINKSr(unit, &reg_64val));
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  30,  exit, ARAD_REG_ACCESS_ERR,WRITE_RTP_ALLOWED_LINKSr(unit,  reg_64val));
    
    /* MAC-RX */
    soc_reg_field_set(unit, FMAC_RECEIVE_RESET_REGISTERr, &reg_val, FMAC_RX_RST_Nf, *rx_field);
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 1250, exit, WRITE_FMAC_RECEIVE_RESET_REGISTERr(unit, blk_id, reg_val));
  ...
  else 
  {
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 1260, exit, READ_FMAC_RECEIVE_RESET_REGISTERr(unit, blk_id, &reg_val));
    *rx_field = soc_reg_field_get(unit, FMAC_RECEIVE_RESET_REGISTERr, reg_val, FMAC_RX_RST_Nf);
    *tx_field = soc_reg_field_get(unit, FMAC_RECEIVE_RESET_REGISTERr, reg_val, FMAC_TX_RST_Nf);
    ...
    /* MAC-RX */
    soc_reg_field_set(unit, FMAC_RECEIVE_RESET_REGISTERr, &reg_val, FMAC_RX_RST_Nf, *rx_field);
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 1270, exit, WRITE_FMAC_RECEIVE_RESET_REGISTERr(unit, blk_id, reg_val));
    

    /* RTP */
    SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  60,  exit, ARAD_REG_ACCESS_ERR,READ_RTP_ALLOWED_LINKSr(unit, &reg_64val));
    high_64bits = COMPILER_64_HI(reg_64val);
    low_64bits = COMPILER_64_LO(reg_64val);
    if(reg_idx == 0)
    {
    ...
    COMPILER_64_SET(reg_64val, high_64bits, low_64bits);
    SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  70,  exit, ARAD_REG_ACCESS_ERR,WRITE_RTP_ALLOWED_LINKSr(unit,  reg_64val));

    /* MAC-TX */
    soc_reg_field_set(unit, FMAC_RECEIVE_RESET_REGISTERr, &reg_val, FMAC_TX_RST_Nf, *tx_field);
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 1280, exit, WRITE_FMAC_RECEIVE_RESET_REGISTERr(unit, blk_id, reg_val));
  }

exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_link_on_off_set_unsafe()",link_ndx,0);
}
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_LINK_ON_OFF_GET_UNSAFE);
  ...
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 1380, exit, READ_FMAC_RECEIVE_RESET_REGISTERr(unit, blk_id, &reg_val));
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_link_on_off_get_unsafe()",link_ndx,0);
}
    ...
    )
{
    ...
    if(SOC_DPP_IS_MESH(unit))
    {
      ...
      SOC_SAND_CHECK_FUNC_RESULT(arad_port_control_pcs_get(unit, SOC_DPP_FABRIC_PORT_TO_LINK(unit, port), &pcs), 34, exit);
      SOC_SAND_CHECK_FUNC_RESULT(arad_fabric_mesh_topology_values_config(unit, speed, pcs), 35, exit);
    }
exit:
    ...
    )
{
    ...
    SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_FABRIC_PORT_SPEED_GET);
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_fabric_port_speed_get()",port, 0);
  ...
  )
{
    ...
    if (!SOC_DPP_CONFIG(unit)->tdm.is_tdm_over_primary_pipe) {
        if (fabric_priority >= ARAD_FBC_PRIORITY_NOF-1 /*last one for tdm only*/) {
            SOCDNX_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_SOC_MSG("fabric priority out of range")));
        }
    } else {
        if (fabric_priority >= ARAD_FBC_PRIORITY_NOF-2 /*2 for tdm only priorities*/) {
            SOCDNX_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_SOC_MSG("fabric priority out of range")));
        }
    }
    ...
    offset = index*ARAD_FBC_PRIORITY_LENGTH /*bits length*/;
  ...
  )
{
  ...
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 1480, exit, READ_RTP_LINK_ACTIVE_MASKr(unit, &mask));

  COMPILER_64_NOT(mask);

  for(link_id = link_index_min; link_id <= link_index_max; ++link_id)
  {
    ...
    if(is_active)
    {
      connectivity_map->link_info[link_id].far_unit = soc_reg_field_get(unit, FCR_CONNECTIVITY_MAP_REGISTERSr, reg_val, SOURCE_DEVICE_ID_Nf);
      ...
      if((source_lvl & 0x2) == 0)
      {
        connectivity_map->link_info[link_id].far_dev_type = ARAD_FAR_DEVICE_TYPE_FAP;
      }
      else if (source_lvl == 0x2)
      {
        connectivity_map->link_info[link_id].far_dev_type = ARAD_FAR_DEVICE_TYPE_FE3;
      }
      else if((source_lvl & 0x3) == 0x3)
      {
        connectivity_map->link_info[link_id].far_dev_type = ARAD_FAR_DEVICE_TYPE_FE2;
      }
      else
      {
        connectivity_map->link_info[link_id].far_dev_type = ARAD_FAR_DEVICE_TYPE_FE1;
      }

      connectivity_map->link_info[link_id].far_link_id = soc_reg_field_get(unit, FCR_CONNECTIVITY_MAP_REGISTERSr, reg_val, SOURCE_DEVICE_LINK_Nf);
      connectivity_map->link_info[link_id].is_logically_connected=1; 
    }
    else
    {
      connectivity_map->link_info[link_id].far_link_id = DCMN_FABRIC_LINK_NO_CONNECTIVITY;
    }      
  }
exit:
...
arad_fabric_reachability_status_get(int unit, int moduleid, int links_max, uint32 *links_array, int *links_count)
{
    ...
    SOCDNX_IF_ERR_EXIT(READ_RTP_LINK_ACTIVE_MASKr(unit, &link_active_mask));
...
/*
 * Function:
 *      arad_dpp_fabric_link_status_all_get
 * Purpose:
...
arad_fabric_link_status_all_get(int unit, int links_array_max_size, uint32* link_status, uint32* errored_token_count, int* links_array_count)
{
    ...
    PBMP_SFI_ITER(unit, port) {
        i = SOC_DPP_FABRIC_PORT_TO_LINK(unit, port);
        if (i > links_array_max_size) {
            break;
        }
        rc = arad_fabric_link_status_get(unit, i, &(link_status[i]), &(errored_token_count[i]));
...
arad_fabric_force_signal_detect_set(int unit, int mac_instance)
{
   ...
   for(i=0 ; i<SOC_DPP_DEFS_GET(unit, nof_fabric_links_in_mac) ; i++) {
       SOCDNX_IF_ERR_EXIT(arad_fabric_loopback_get(unit, base_link+i, &loopback));
       ...
       soc_sand_rv = arad_port_control_pcs_get_unsafe(unit, base_link+i, &pcs);
       if(SOC_SAND_FAILURE(soc_sand_rv)) {
           SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL,(_BSL_SOCDNX_MSG("failed in arad_port_control_pcs_get_unsafe")));
       }
       if(ARAD_PORT_PCS_64_66_FEC == pcs || ARAD_PORT_PCS_64_66_BEC == pcs) {
           force_signal_detect = 1;
           break;
       }
    ...
    SOCDNX_IF_ERR_EXIT(READ_FMAC_GENERAL_CONFIGURATION_REGISTERr(unit, mac_instance, &reg_val));
    soc_reg_field_set(unit, FMAC_GENERAL_CONFIGURATION_REGISTERr, &reg_val, FORCE_SIGNAL_DETECTf, force_signal_detect);
    SOCDNX_IF_ERR_EXIT(WRITE_FMAC_GENERAL_CONFIGURATION_REGISTERr(unit, mac_instance, reg_val));
...
*      unit      - (IN)  Unit number.
*      link      - (IN)  link number 
*      loopback  - (IN)  soc_dcmn_loopback_mode_t
* Returns:
*      SOC_E_xxx
* Notes:
*/
soc_error_t 
arad_fabric_loopback_set(int unit, int link_id, soc_dcmn_loopback_mode_t loopback)
{
  soc_port_t inner_port, port;
  int blk_id;
  uint32 reg_val, field_val[1], soc_sand_rv;
  ...
  locked = 0;
     ...
     * First disable FMAC
     */
  soc_sand_rv = arad_link_on_off_get_unsafe(unit, link_id, &info);
  SOCDNX_SAND_IF_ERR_RETURN(soc_sand_rv);
  on_off = info.on_off;
    if(info.on_off == SOC_TMC_LINK_STATE_ON) {
      info.on_off = SOC_TMC_LINK_STATE_OFF;
      info.serdes_also = TRUE;
      soc_sand_rv = arad_link_on_off_set_unsafe(unit, link_id, &info);
      SOCDNX_SAND_IF_ERR_RETURN(soc_sand_rv);
    }

  blk_id = (int)link_id/4;
  inner_port = link_id % 4;
  port = SOC_DPP_FABRIC_LINK_TO_PORT(unit, link_id);

    /*MAC PCS llopback is valid only for 64/66 FEC encoding*/
    if(soc_dcmn_loopback_mode_mac_pcs == loopback) {
        soc_sand_rv = arad_port_control_pcs_get_unsafe(unit, port, &pcs);
        SOCDNX_SAND_IF_ERR_RETURN(soc_sand_rv);
        if(ARAD_PORT_PCS_64_66_FEC != pcs) {
    ...
    SOCDNX_IF_ERR_EXIT(arad_fabric_loopback_get(unit, link_id, &prev_loopback));
...
/*
* Function:
*      arad_fabric_port_loopback_get
* Purpose:
*      Get port loopback
* Parameters:
*      unit      - (IN)  Unit number.
*      link_id   - (IN)  link number 
*      loopback  - (OUT) soc_dcmn_loopback_mode_t
* Returns:
*      SOC_E_xxx
* Notes:
*/
soc_error_t 
arad_fabric_loopback_get(int unit, int link_id, soc_dcmn_loopback_mode_t* loopback)
{
  soc_port_t inner_port, port;
  int blk_id;
...
*/
soc_error_t 
arad_link_control_strip_crc_set(int unit, bcm_port_t port, int strip_crc)
{
  bcm_port_t inner_port;
  int blk_id;
...
*/
soc_error_t 
arad_link_control_strip_crc_get(int unit, bcm_port_t port, int* strip_crc)
{
  bcm_port_t inner_port;
  int blk_id;
  uint32 general_config;
...
*/
soc_error_t 
arad_link_control_rx_enable_set(int unit, bcm_port_t port, int enable)
{
  bcm_port_t inner_port;
  int blk_id;
  uint32 reg_val, field_val[1];
...
*/
soc_error_t 
arad_link_control_tx_enable_set(int unit, bcm_port_t port, int enable)
{
  bcm_port_t inner_port;
  int blk_id;
  uint32 reg_val, field_val[1];
...
*/
soc_error_t 
arad_link_control_rx_enable_get(int unit, bcm_port_t port, int* enable)
{
  bcm_port_t inner_port;
  int blk_id;
  uint32 reg_val, field_val[1];
...
*/
soc_error_t 
arad_link_control_tx_enable_get(int unit, bcm_port_t port, int* enable)
{
  bcm_port_t inner_port;
  int blk_id;
  uint32 reg_val, field_val[1];
...
arad_link_port_fault_get(int unit, bcm_port_t link_id, uint32* flags)
{
  ...
  (*flags) = 0;
  SOCDNX_IF_ERR_EXIT(READ_RTP_LOCALLY_GENERATED_ACLr(unit, &reg_64val)); /* RTP_LINK_STATE_VECTORr in FE1600 */
  ...
  SOCDNX_IF_ERR_EXIT(READ_RTP_ACL_RECEIVEDr(unit, &reg_64val)); /* RTP_ACL_VECTORr in FE1600 */
  arr_64bits[1] = COMPILER_64_HI(reg_64val);
  arr_64bits[0] = COMPILER_64_LO(reg_64val);
  if(!SHR_BITGET(arr_64bits,link_id)) {
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_FABRIC_LINK_UP_GET_UNSAFE);
  
  /*check if link is disabled*/
  soc_sand_rv = arad_link_on_off_get_unsafe(unit, link_idx, &info);
  SOCDNX_SAND_IF_ERR_RETURN(soc_sand_rv);
  ...
  if(info.on_off == SOC_TMC_LINK_STATE_ON) {
      ...
      *fld_val = (1 << inner_link);
      ...
      SOC_DPP_ALLOW_WARMBOOT_WRITE(WRITE_FMAC_INTERRUPT_REGISTER_2r(unit,blk_id,reg_val),rv);
      SOC_SAND_SOC_CHECK_FUNC_RESULT(rv, 20, exit);
      /*check rxlos interrupt - link is up*/

      SOC_SAND_SOC_IF_ERROR_RETURN(res, 30, exit, READ_FMAC_INTERRUPT_REGISTER_2r(unit,blk_id,&reg_val));
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_fabric_link_up_get_unsafe()",0,0);
}

uint32
  arad_fabric_link_up_get_verify(
    SOC_SAND_IN  int                 unit,
    SOC_SAND_IN  uint32                 link_ndx,
    SOC_SAND_OUT uint32                 *up
  )
{
    SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_FABRIC_LINK_UP_GET_VERIFY);
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_fabric_link_up_get_verify()",0,0);
}
...
arad_fabric_wcmod_ucode_load(int unit ,int port, uint8 *arr, int arr_len) {
    ...
    soc_timeout_t to;
    int clause = 22; /*Arad access intenal phy with CL22*/
    soc_pbmp_t pbmp_sfi;
  ...
  )
{
    ...
    switch (type)
    {
       case ARAD_FABRIC_GCI_TYPE_LEAKY_BUCKET:
           /*GCI LB to IPT DTQ*/
           registers_gci_map_masking[0] = IPT_IPT_INTRNL_FMC_FC_MAPr;
           fields_gci_map_masking[0] = GCI_LB_2_IPT_DTQ_MC_FC_MAPf;
           /*GCI LB to IPT GFMC*/
           registers_gci_map_masking[1] = IPT_IPT_INTRNL_FMC_FC_MAPr;
           fields_gci_map_masking[1] = GCI_LB_2_IPT_GFMC_FC_MAPf;
           /*GCI LB to IPT BFMC*/
           registers_gci_map_masking[2] = IPT_IPT_INTRNL_FMC_FC_MAPr;
           fields_gci_map_masking[2] = GCI_LB_2_IPT_BFMC_FC_MAPf;
           /*GCI LB to IPS GFMC*/
           registers_gci_map_masking[3] = IPT_IPT_FMC_IPS_FC_MAP_1r ;
           fields_gci_map_masking[3] = GCI_LB_2_IPS_GFMC_FC_MAPf;
           /*GCI LB to IPT BFMC 0*/
           registers_gci_map_masking[4] = IPT_IPT_FMC_IPS_FC_MAP_1r;
           fields_gci_map_masking[4] = GCI_LB_2_IPS_BFMC_0_FC_MAPf;
           /*GCI LB to IPT BFMC 1*/
           registers_gci_map_masking[5] = IPT_IPT_FMC_IPS_FC_MAP_1r;
           fields_gci_map_masking[5] = GCI_LB_2_IPS_BFMC_1_FC_MAPf;
           /*GCI LB to IPT BFMC 2*/
           registers_gci_map_masking[6] = IPT_IPT_FMC_IPS_FC_MAP_1r;
  ...
  )
{
    ...
    switch (type)
    {
        ...
        case ARAD_FABRIC_GCI_CONFIG_TYPE_BACKOFF_TH_0:
            SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  1,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_read(unit, IPT_GCI_BACKOFF_RANGE_THRESHOLDSr, REG_PORT_ANY, 0, CNGST_LVL_THRESH_0f, (uint32*)value));
            break;
        case ARAD_FABRIC_GCI_CONFIG_TYPE_BACKOFF_TH_1:
            SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  2,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_read(unit, IPT_GCI_BACKOFF_RANGE_THRESHOLDSr, REG_PORT_ANY, 0, CNGST_LVL_THRESH_1f, (uint32*)value));
            break;
        case ARAD_FABRIC_GCI_CONFIG_TYPE_BACKOFF_TH_2:
            SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  3,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_read(unit, IPT_GCI_BACKOFF_RANGE_THRESHOLDSr, REG_PORT_ANY, 0, CNGST_LVL_THRESH_2f, (uint32*)value));
            break;

        /*GCI Leaky Bucket congested*/
        case ARAD_FABRIC_GCI_CONFIG_TYPE_LB_1_CONGESTION_TH:
            SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  4,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_read(unit,IPT_GCI_LEAKY_BUCKET_CONFIGURATION_REGISTER_2r,REG_PORT_ANY,0, LKY_BKT_CNG_TH_1f,(uint32*)value));
            break;
        case ARAD_FABRIC_GCI_CONFIG_TYPE_LB_2_CONGESTION_TH:
            SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  5,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_read(unit,IPT_GCI_LEAKY_BUCKET_CONFIGURATION_REGISTER_2r,REG_PORT_ANY,0, LKY_BKT_CNG_TH_2f,(uint32*)value));
            break;
        case ARAD_FABRIC_GCI_CONFIG_TYPE_LB_3_CONGESTION_TH:
            SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  6,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_read(unit,IPT_GCI_LEAKY_BUCKET_CONFIGURATION_REGISTER_2r,REG_PORT_ANY,0, LKY_BKT_CNG_TH_3f,(uint32*)value));
            break;
        case ARAD_FABRIC_GCI_CONFIG_TYPE_LB_4_CONGESTION_TH:
            SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  7,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_read(unit,IPT_GCI_LEAKY_BUCKET_CONFIGURATION_REGISTER_2r,REG_PORT_ANY,0, LKY_BKT_CNG_TH_4f,(uint32*)value));
            break;

         /*GCI Leaky Bucket full*/
       case ARAD_FABRIC_GCI_CONFIG_TYPE_LB_1_FULL:
            SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  8,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_read(unit,IPT_GCI_LEAKY_BUCKET_CONFIGURATION_REGISTER_0r,REG_PORT_ANY,0, LKY_BKT_MAX_CNT_1f,(uint32*)value));
            break;
        case ARAD_FABRIC_GCI_CONFIG_TYPE_LB_2_FULL:
            SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  9,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_read(unit,IPT_GCI_LEAKY_BUCKET_CONFIGURATION_REGISTER_0r,REG_PORT_ANY,0, LKY_BKT_MAX_CNT_2f,(uint32*)value));
            break;
        case ARAD_FABRIC_GCI_CONFIG_TYPE_LB_3_FULL:
            SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  10,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_read(unit,IPT_GCI_LEAKY_BUCKET_CONFIGURATION_REGISTER_0r,REG_PORT_ANY,0, LKY_BKT_MAX_CNT_3f,(uint32*)value));
            break;
        case ARAD_FABRIC_GCI_CONFIG_TYPE_LB_4_FULL:
            SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  11,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_read(unit,IPT_GCI_LEAKY_BUCKET_CONFIGURATION_REGISTER_0r,REG_PORT_ANY,0, LKY_BKT_MAX_CNT_4f,(uint32*)value));
            break;
  ...
  )
{
    ...
    {
       case ARAD_FABRIC_GCI_CONFIG_TYPE_BACKOFF_TH_0:
            SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  1,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit,IPT_GCI_BACKOFF_RANGE_THRESHOLDSr,REG_PORT_ANY,0, CNGST_LVL_THRESH_0f, (uint32)value));
            break;
        case ARAD_FABRIC_GCI_CONFIG_TYPE_BACKOFF_TH_1:
            SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  2,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit,IPT_GCI_BACKOFF_RANGE_THRESHOLDSr,REG_PORT_ANY,0, CNGST_LVL_THRESH_1f, (uint32)value));
            break;
        case ARAD_FABRIC_GCI_CONFIG_TYPE_BACKOFF_TH_2:
            SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  3,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit,IPT_GCI_BACKOFF_RANGE_THRESHOLDSr,REG_PORT_ANY,0, CNGST_LVL_THRESH_2f, (uint32)value));
            break;

        /*GCI Leaky Bucket congested*/
        case ARAD_FABRIC_GCI_CONFIG_TYPE_LB_1_CONGESTION_TH:
            SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  4,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit,IPT_GCI_LEAKY_BUCKET_CONFIGURATION_REGISTER_2r,REG_PORT_ANY,0, LKY_BKT_CNG_TH_1f, (uint32)value));
            break;
        case ARAD_FABRIC_GCI_CONFIG_TYPE_LB_2_CONGESTION_TH:
            SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  5,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit,IPT_GCI_LEAKY_BUCKET_CONFIGURATION_REGISTER_2r,REG_PORT_ANY,0, LKY_BKT_CNG_TH_2f, (uint32)value));
            break;
        case ARAD_FABRIC_GCI_CONFIG_TYPE_LB_3_CONGESTION_TH:
            SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  6,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit,IPT_GCI_LEAKY_BUCKET_CONFIGURATION_REGISTER_2r,REG_PORT_ANY,0, LKY_BKT_CNG_TH_3f, (uint32)value));
            break;
        case ARAD_FABRIC_GCI_CONFIG_TYPE_LB_4_CONGESTION_TH:
            SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  7,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit,IPT_GCI_LEAKY_BUCKET_CONFIGURATION_REGISTER_2r,REG_PORT_ANY,0, LKY_BKT_CNG_TH_4f, (uint32)value));
            break;

         /*GCI Leaky Bucket full*/
       case ARAD_FABRIC_GCI_CONFIG_TYPE_LB_1_FULL:
            SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  8,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit,IPT_GCI_LEAKY_BUCKET_CONFIGURATION_REGISTER_0r,REG_PORT_ANY,0, LKY_BKT_MAX_CNT_1f, (uint32)value));
            break;
        case ARAD_FABRIC_GCI_CONFIG_TYPE_LB_2_FULL:
            SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  9,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit,IPT_GCI_LEAKY_BUCKET_CONFIGURATION_REGISTER_0r,REG_PORT_ANY,0, LKY_BKT_MAX_CNT_2f, (uint32)value));
            break;
        case ARAD_FABRIC_GCI_CONFIG_TYPE_LB_3_FULL:
            SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  10,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit,IPT_GCI_LEAKY_BUCKET_CONFIGURATION_REGISTER_0r,REG_PORT_ANY,0, LKY_BKT_MAX_CNT_3f, (uint32)value));
            break;
        case ARAD_FABRIC_GCI_CONFIG_TYPE_LB_4_FULL:
            SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  11,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit,IPT_GCI_LEAKY_BUCKET_CONFIGURATION_REGISTER_0r,REG_PORT_ANY,0, LKY_BKT_MAX_CNT_4f, (uint32)value));
            break;
   ...
   )
{
   ...
   SOC_SAND_ERR_IF_BELOW_MIN(minimum_links,0,SOC_TMC_INPUT_OUT_OF_RANGE,4,exit);
   SOC_SAND_ERR_IF_ABOVE_MAX(minimum_links,SOC_DPP_DEFS_GET(unit, nof_fabric_links),SOC_TMC_INPUT_OUT_OF_RANGE,3,exit);
    
#ifdef BCM_88660_A0
   SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  1,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, RTP_SPARE_REGISTER_2r, REG_PORT_ANY, 0, ECO_MIN_NOF_LINKSf,  minimum_links));
   SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  2,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, RTP_SPARE_REGISTER_2r, REG_PORT_ANY, 0, ECO_ENABLEDf,  minimum_links? 1:0));
#endif

exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_fabric_minimal_links_to_dest_set()",0,0);
}
  ...
  )
{
    ...
    *value=0;
    SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  1,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_read(unit, SCH_RCI_PARAMSr, SOC_CORE_ALL, 0, FAP_RCI_ENAf, &fap_rci_en));
  ...
  )
{
   ...
   /*Verify*/
    ...
    /*Configure registers*/
    SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  1,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, SCH_RCI_PARAMSr, SOC_CORE_ALL, 0, FAP_RCI_ENAf,  fap_rci_en));
   ...
   )
{
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  30,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, FMAC_SPARE_REGISTER_2r,  fmac_index,  0, EMPTY_CELL_SIZEf,  cell_size_hw_format)); /*Enable enhance shaper mode*/
    SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  40,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, FMAC_SPARE_REGISTER_2r,  fmac_index,  0, OVERRIDE_EMPTY_CELL_SIZEf,  0x1)); /*Enable cell size configuration*/
   ...
   SOC_SAND_IN  int                                 unit)
{
          ...
          port_index++)
    {
        ...
        res = soc_phyctrl_enable_get(unit, port_index, &enable);
        ...
        res = arad_fabric_port_speed_get(unit, port_index, &speed);
        SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
        if (speed == 0)
        {
            /*port is disabled*/
        ...
        res = arad_port_control_pcs_get_unsafe(unit, SOC_DPP_FABRIC_PORT_TO_LINK(unit, port_index), &encoding);
        SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);

        if (encoding == ARAD_PORT_PCS_8_10)
        {
            /*Effective rate*/
    ...
    res = READ_FDR_FDR_ENABLERS_REGISTER_1r(unit, &reg64_val);
    SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 30, exit);
    soc_reg64_field32_set(unit, FDR_FDR_ENABLERS_REGISTER_1r , &reg64_val, FIELD_19_22f, highest_aldwp);
    res = WRITE_FDR_FDR_ENABLERS_REGISTER_1r(unit, reg64_val);
    SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 40, exit);

exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_fabric_aldwp_config()", 0, 0);
}
...
arad_fabric_link_tx_traffic_disable_set(int unit, soc_port_t link, int disable)
{
    ...
    rv = READ_RTP_ALLOWED_LINKSr(unit, &reg64_val);
    SOCDNX_IF_ERR_EXIT(rv);
    if (disable)
    {
    ...
    rv = WRITE_RTP_ALLOWED_LINKSr(unit, reg64_val);
    SOCDNX_IF_ERR_EXIT(rv);

    /*Disable/enable MC cells*/
    rv = READ_RTP_MULTICAST_LINK_UPr(unit, &reg64_val);
    SOCDNX_IF_ERR_EXIT(rv);
    if (disable)
    {
        COMPILER_64_BITCLR(reg64_val, link);
    ...
    rv = WRITE_RTP_MULTICAST_LINK_UPr(unit, reg64_val);
    SOCDNX_IF_ERR_EXIT(rv);
