  ...
  )
{
  ...
  res = soc_port_sw_db_tm_port_to_base_q_pair_get(unit,core,  port_ndx, &base_port_tc);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 15, exit);

  res = soc_port_sw_db_tm_port_to_out_port_priority_get(unit,core, port_ndx, &nof_priorities);
  ...
  )
{
  ...
  res = soc_port_sw_db_tm_port_to_out_port_priority_get(unit, core, port_id, &nof_priorities); 
  ...
  )
{
  ...
  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  20,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_read(unit, SCH_REBOUNDED_CREDIT_CONFIGURATIONr, core, 0, REBOUNDED_CREDIT_WORTHf, &assigned_val));
  ...
  soc_SCH_PIR_SHAPERS_STATIC_TABEL_PSSTm_field32_set(unit,&tbl_data,MAX_BURSTf,info->max_burst);
  ...
  )
{
  ...
  soc_SCH_CIR_SHAPERS_STATIC_TABEL_CSSTm_field32_set(unit,&tbl_data,MAX_BURSTf,info->max_burst);
  ...
  )
{
  ...
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 30, exit, READ_SCH_CIR_SHAPERS_CONFIGURATIONr(unit, core,&data));
  /* cal length is +1 from register value */
  cal_length = soc_reg_field_get(unit, SCH_CIR_SHAPERS_CONFIGURATIONr, data, CIR_SHAPERS_CAL_LENGTHf) + 1;
  field_val = soc_reg_field_get(unit, SCH_CIR_SHAPERS_CONFIGURATIONr, data, CIR_SHAPERS_CAL_ACCESS_PERIODf);
  access_period = (field_val >> 4);
