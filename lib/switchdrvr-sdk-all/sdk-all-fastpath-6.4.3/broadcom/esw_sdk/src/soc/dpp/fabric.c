/*
 * $Id: fabric.c,v 1.34 Broadcom SDK $
...
if SOC_TMC_FAR_DEVICE_TYPE_FE1 or number of hops >= 3 -> TRUE
if SOC_TMC_FAR_DEVICE_TYPE_FE2 -> FALSE
*/  
...
)
{
  ...
  /*Check that all the indecies are in the required range and are valid*/
  for (route_index = 0; route_index <route_count; route_index++){
    ...
    if (route.number_of_hops == 0) {/*i.e. The route is not valid*/
      rv = SOC_E_PARAM;
      return rv;
    }
  }
...
)
{
  ...
  /* verifying that the links are in the adequate range*/
  ...
  if(route->number_of_hops == 0) {/* Invalid the entry*/
...
)
{
  ...
  if(!(SOC_DPP_CONTROL(unit))->inband_route_entries[route_id].is_valid)/*i.e. the route is invalid*/
  {
    rv = SOC_E_PARAM;
    goto soc_dpp_attach_error;
  }
...
soc_dpp_fabric_sr_cell_send( 
  ...
  uint32 data_in_size, /*Units: Bytes*/
  uint32 *data_in
)
{
    dcmn_sr_cell_link_list_t sr_link_list;
...
soc_dpp_fabric_sr_cell_receive(
  ...
  uint32 *data_out_size, /*Units: Bytes*/
  uint32 *data_out
)
{
...
int 
  soc_dpp_fabric_reg_read(
    ...
    soc_reg_t *reg, /* For SOC_SAND_FE600, represents the address */ 
    int *port,
    int *index,
    uint64 *data, 
    int *array_count
  )
{
  ...
  /*Checking that all the registers are not above 64 bits*/
  ...
  /*Verifyibg that the route is valid*/
  ...
  /*Getting the sr_link_list for the route*/
#ifdef BCM_PETRAB_SUPPORT
  ...
  for (regs_index=0; regs_index < max_count; regs_index += nof_commands)
  {
...
#ifdef BCM_PETRAB_SUPPORT
      if(SOC_IS_PETRAB(unit)) {
        soc_sand_rv = soc_petra_transaction_with_fe1600(
          soc_sand_dev_id,
          &sr_link_list,
          reg[regs_index],
          INVALIDm,
        ...
        sal_memcpy(&data[regs_index], temp_data_32, 64/8);
      }
#endif /* BCM_PETRAB_SUPPORT */
   ...
   if (SOC_IS_ARAD(unit)) {
    ...
    else
    {
      ...
      if(SOC_IS_PETRAB(unit))
      {
        soc_sand_rv = soc_petra_read_from_fe600(
          soc_sand_dev_id,
          &sr_link_list,
          sizeof(uint32),
          reg[regs_index],
   ...
   if (SOC_IS_ARAD(unit)) {
      ...
      SOCDNX_SAND_IF_ERR_RETURN(soc_sand_rv);
      sal_memcpy(&data[regs_index], temp_data_32, 32/8);/*All the registers of fe600 are 32-bits.*/
    }
  }
  ...
  )
{
  ...
  /*Getting the pointer of the groups for SOC_PETRAB*/
  if(SOC_IS_PETRAB(unit))
  {
    if (route_id > SOC_DPP_NUM_OF_ROUTES)
    {
  ...
  for(regs_index = 0; regs_index < array_count; regs_index += nof_commands) {
...
#ifdef BCM_PETRAB_SUPPORT
      if(SOC_IS_PETRAB(unit)){
        sal_memcpy(&soc_petra_temp_data_32[i*(128/32)], &data[regs_index+i], (128/8));
      }
#endif 
    }
    ...
    if(SOC_IS_ARAD(unit) && SOC_FABRIC_INBAND_IS_GROUP(route_id)){
      /*verifying that the group is valid*/
    ...
    /*Getting the groups ids if group */
    if(SOC_FABRIC_INBAND_IS_GROUP(route_id)){
      /*verifying that the group is valid*/
      SOCDNX_IF_ERR_EXIT(soc_dpp_fabric_inband_route_group_is_valid(unit, SOC_FABRIC_INBAND_ROUTE_GRP_GET(route_id), &is_valid));
      if(!is_valid) {
 ...
 #ifdef BCM_PETRAB_SUPPORT
        if(SOC_IS_PETRAB(unit)){
          soc_sand_rv = soc_petra_write_to_fe600(
            soc_sand_dev_id,
            &sr_link_list,
            sizeof(uint32),
            reg[regs_index],
  ...
  ) 
{
  ...
  if(SOC_IS_PETRAB(unit)) {
    if (route_id > SOC_DPP_NUM_OF_ROUTES)
    {
      /* We may latter change the macro SOC_FABRIC_INBAND_IS_GROUP auch that the criteria will not br route_id >= 2048*/
...
#ifdef BCM_PETRAB_SUPPORT
      if(SOC_IS_PETRAB(unit)){
        soc_sand_rv = soc_petra_indirect_read_from_fe600(
          soc_sand_dev_id,
          &sr_link_list,
          nof_mems_in_cell,
          mem[mems_index],
  ...
  ) 
{
  ...
  /*Getting the pointer to the routes for SOC_PETRAB*/
  if(SOC_IS_PETRAB(unit))
  {
    if (route_id > SOC_DPP_NUM_OF_ROUTES)
    {
...
#ifdef BCM_PETRAB_SUPPORT
      if(SOC_IS_PETRAB(unit)){
        sal_memcpy(&soc_petra_temp_data_32[i*(128/32)], entry_data[mems_index+i], (128/8));
      }
#endif
    }
...
#ifdef BCM_PETRAB_SUPPORT
int
  soc_fabric_link_status_get(
     int unit, 
     bcm_port_t link_id,  
     SOC_PETRA_FABRIC_LINKS_STATUS_SINGLE* link_status
  )
{
  ...
  soc_petra_PETRA_FABRIC_LINKS_STATUS_SINGLE_clear(link_status);

  
  unit = 0;
  mac_group_i = SOC_PETRA_FBC_MAC_OF_LINK(link_id); 
  ...
  link_per_mac_i = SOC_PETRA_FBC_LINK_IN_MAC(link_id);
  ...
  if(TRUE == link_status->valid)
  {
    ...
    else
    {
      ...
      link_status->up_rx = SOC_SAND_NUM2BOOL_INVERSE(is_reset);
      ...
      {
        link_status->indications_bm |= SOC_PETRA_LINK_INDICATE_TYPE_SIG_LOCK;
      }
      ...
      if(fec_en & SOC_SAND_BIT(link_per_mac_i))
      {
        ...
        {
          link_status->errors_bm |= SOC_PETRA_LINK_ERR_TYPE_MISALIGN;
        }
        if(decode_fec & SOC_SAND_BIT(link_per_mac_i))
        {
          link_status->errors_bm |= SOC_PETRA_LINK_ERR_TYPE_CODE_GROUP;
        }
      }
      else
      {
      ...
      if(link_i >= 18)
      {
        ...
        {
          link_status->indications_bm |= SOC_PETRA_LINK_INDICATE_TYPE_ACCEPT_CELL;
        }
      }
      else
      {
    ...
    link_status->crc_err_counter = crc_err_cnt;
  } /* link_status->valid */
...
{    
    ...
    rv = soc_dcmn_drv_fabric_pipe_map_initalize_valid_configurations(unit, 3, fabric_pipe_map_valid_config);
    ...
    SOC_DPP_FABRIC_PIPES_CONFIG(unit).config_uc[3] = soc_property_suffix_num_get(unit, 3, spn_FABRIC_PIPE_MAP, "uc", SOC_DPP_PROPERTY_UNAVAIL); /* if we have one pipe then pipe should be 0, if two pipes then pipe should be 1, if three pipes then pipe should be 2 (default) */
    ...
    if (SOC_DPP_FABRIC_PIPES_CONFIG(unit).mapping_type == soc_dcmn_drv_uc_hp_mc_lp_mc_config_triple_pipe){
...
{ 
    ...
    topo_stat->is_mesh_mc = SOC_DPP_CONFIG(unit)->arad->init.fabric.fabric_mesh_multicast_enable;
...
uint32
soc_dpp_fabric_sync_e_enable_set(int unit, int is_master, int port_id)
{
    SOCDNX_INIT_FUNC_DEFS;
    
    if (!SOC_PORT_VALID_RANGE(unit, port_id)) {
        SOCDNX_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_SOCDNX_MSG("Link %d is invalid"), unit, port_id));
    }
    SOCDNX_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_fabric_sync_e_enable_set, (unit, is_master, port_id)));
...
soc_dpp_fabric_sync_e_enable_get(int unit, int is_master, int *is_fabric_synce)
{
    ...
    *is_fabric_synce = 0;
    ...
    SOCDNX_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_fabric_sync_e_enable_get, (unit, is_master, is_fabric_synce)));
