  ...
  )
{
  ...
  res = arad_sch_port_id_verify_unsafe(unit, port_id);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);

  /* SOC_SAND_TAKE_DEVICE_SEMAPHORE; */

  res = arad_sch_port_tc2se_id_get_unsafe(unit,core,port_id,tc,se_id);
  SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit_semaphore);
...
exit_semaphore:
  ...
  ARAD_DO_NOTHING_AND_EXIT;
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_sch_port_tc2se_id(). port_id %d, tc 0x%08lX", port_id, tc);
}
...
exit_semaphore:
  ...
  ARAD_DO_NOTHING_AND_EXIT;
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_sch_port_tcg_weight_set()", port_id, tcg_ndx);
}
...
exit_semaphore:
  ...
  ARAD_DO_NOTHING_AND_EXIT;
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_sch_port_tcg_weight_get()", port_id, tcg_ndx);
}  
  ...
  )
{
  ...
  if(port_id != ARAD_FAP_EGRESS_REPLICATION_SCH_PORT_ID)
  {
