...
/* bfd ipv4 single hop is selected if Parser-Leaf-Context = x (FIX)
   The parser sets the Parser-Leaf-Context to X if UDP.Dst-Port = 3784*/
uint32
   arad_pp_flp_prog_sel_cam_bfdSingleHop(
     int unit,
	 int32  *prog_id
   )
{
  ...
  /* For static, program id and app id is same */
  res = arad_pp_flp_get_next_avaialable(unit,ARAD_PP_FLP_KEY_PROGRAM_BFD_SINGLE_HOP,&cam_sel,prog_id);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
...
uint32
   arad_pp_flp_key_const_bfdSingleHop(
     int unit,
	 int32  prog
   )
{
...
exit:
 SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_pp_flp_key_const_bfdSingleHop", 0, 0);
}
...
uint32
   arad_pp_flp_lookups_bfdSingleHop(
     int unit,
	 int prog
   )
{
  ...
  SOC_SAND_CHECK_FUNC_RESULT(res, 42, exit);
exit:
 SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_pp_flp_lookups_bfdSingleHop", 0, 0);
}
...
uint32
   arad_pp_flp_process_bfdSingleHop(
     int unit,
	 int prog
   )
{
  ...
  ARAD_SOC_REG_FIELD   strength_fld_fwd,
    ...
    /* Used the pre-allocated User_Defined_Trap code and map it to the HW Code */
    res = arad_pp_trap_mgmt_trap_code_to_internal(SOC_PPC_TRAP_CODE_BFD_IPV4_YOUR_DISCR_NOT_FOUND, &trap_code_internal, &strength_fld_fwd, &strength_fld_snp);
    SOC_SAND_CHECK_FUNC_RESULT(res, 240, exit);  
    ...
    flp_process_tbl.enable_rpf_check            = 0;
    flp_process_tbl.not_found_trap_code         = trap_code_internal;
    flp_process_tbl.not_found_trap_strength     = 6;
   ...
   )
{
  ...
  if (SOC_DPP_CONFIG(unit)->pp.bfd_ipv4_single_hop == 1) {
	    int32 bfd_single_hop_prog_id; 
	  arad_pp_flp_prog_sel_cam_bfdSingleHop(unit, &bfd_single_hop_prog_id); 
	  arad_pp_flp_key_const_bfdSingleHop(unit, bfd_single_hop_prog_id);
      arad_pp_flp_lookups_bfdSingleHop(unit, bfd_single_hop_prog_id);
      arad_pp_flp_process_bfdSingleHop(unit, bfd_single_hop_prog_id);
  }
                ...
                switch (key->key) {
                    case 0:
                        ...
                        if ((flp_key_construction_tbl_0_5.key_b_inst_0_to_5_valid & inst_0_to_5_valid) ||
                            ...
                            uint8 app_id;
                            res = arad_pp_sw_db_flp_prog_index_to_app_get(unit, prog_id, &app_id);
                            SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);
...
)
{
	...
	case ARAD_PP_FLP_KEY_PROGRAM_BFD_SINGLE_HOP:
        return "ARAD_PP_FLP_KEY_PROGRAM_BFD_SINGLE_HOP";
    default:
