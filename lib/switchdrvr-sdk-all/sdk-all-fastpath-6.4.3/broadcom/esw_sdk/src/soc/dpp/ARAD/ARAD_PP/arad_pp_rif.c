...
#define ARAD_RIF_SERVICE_TYPE_MAX                7
  ...
  )
{
  ...
  for (; rif_id < SOC_DPP_DEFS_GET(unit, nof_rifs); ++ rif_id) {
  ...
  if (rif_id < SOC_DPP_DEFS_GET(unit, nof_rifs)) { /* a match was found in the loop */
  ...
  )
{
  ...
  for (rif_id = 0; rif_id <= SOC_DPP_DEFS_GET(unit, nof_rifs); ++ rif_id) {
  ...
  )
{
  ...
  res = MBCM_PP_DRIVER_CALL(data->dev_id,mbcm_pp_profile_mgmt_if_profile_val_set,(data->dev_id, SOC_PPC_PROFILE_MGMT_TYPE_INLIF, SOC_PPC_PROFILE_MGMT_INLIF_PROFILE_TYPE_STRICT_URPF, ((data->mode == ARAD_PP_FRWRD_FEC_RPF_MODE_UC_STRICT) ? 1 : 0), inlif_profile_ptr));
  ...
  )
{
  ...
  if (SOC_IS_ARADPLUS(unit) && soc_property_get(unit, spn_BCM886XX_L3_INGRESS_URPF_ENABLE, 0)) {
    ...
    res = SOC_DPP_WB_ENGINE_SET_ARR(unit, SOC_DPP_WB_ENGINE_VAR_ARAD_PP_RIF_URPF_MODE, &urpf_mode, rif_id);
  ...
  )
{
  ...
  if (SOC_IS_ARADPLUS(unit) && soc_property_get(unit, spn_BCM886XX_L3_INGRESS_URPF_ENABLE, 0)) {
    ...
    res = SOC_DPP_WB_ENGINE_GET_ARR(unit, SOC_DPP_WB_ENGINE_VAR_ARAD_PP_RIF_URPF_MODE, &urpf_mode, rif_id);
    SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 40, exit);
  ...
  )
{
  ...
  if (SOC_IS_ARADPLUS_A0(unit) && soc_property_get(unit, spn_BCM886XX_L3_INGRESS_URPF_ENABLE, 0)) {
      /* Get the mask of inlif profile for strict urf mode */
      res = MBCM_PP_DRIVER_CALL(unit, mbcm_pp_profile_mgmt_if_profile_val_set, (unit, SOC_PPC_PROFILE_MGMT_TYPE_INLIF, SOC_PPC_PROFILE_MGMT_INLIF_PROFILE_TYPE_STRICT_URPF, 1/*value*/,&inlif_profile_urpf_mask));
  ...
  /* 
  if soc property l3_ingress_urpf_enable==0 update the table (RPF-Model-is-Strict) with the default value.
  */
    ...
    if(SOC_IS_JERICHO(unit)){        
        ...
        if (soc_property_get(unit, spn_BCM886XX_L3_INGRESS_URPF_ENABLE, 0)) {
          /* this means that the urpf alwas equals to 0 no matter what is the profile */
          COMPILER_64_SET(field64,0,0x0);          
        }else {
          /* default configuration, the MSB of the rif profile indicate the uRPF (64 profiles),
             so the 32 high values set to "1" and the low 32 set to "0" */
          COMPILER_64_SET(field64,0xffff,0x0);
          
        }
        soc_reg64_field_set(unit, IHB_IN_RIF_UC_RPF_MODEL_IS_STRICTr, &val64, IN_RIF_UC_RPF_MODEL_IS_STRICTf, field64);               
  ...
  )
{
  ...
  sem_res_tbl_data.orientation_is_hub = FALSE;
  ...
  if (SOC_IS_JERICHO(unit)) {
    ...
    sem_res_tbl_data.sys_in_lif = lif_index;
  }
  ...
  )
{
    ...
    arad_pp_rif_ip_tunnel_term_lookups_get(unit,&dip_exact_enabled,&sip_exact_enabled,&dip_sip_tcam_enabled,&dip_sip_tcam_next_protocol_enabled);
    ...
    if(nof_fields != 1 || !exact_match || exact_blocked) {
        if (!dip_sip_tcam_enabled && !dip_sip_tcam_next_protocol_enabled) { /* tcam not enabled */
  ...
  )
{
  ...
  sem_res_tbl_data.protection_path = SOC_SAND_NUM2BOOL(term_info->protection_pass_value);
  sem_res_tbl_data.protection_pointer = term_info->protection_pointer;
  sem_res_tbl_data.service_type = term_info->service_type;
  sem_res_tbl_data.tpid_profile = term_info->tpid_profile;
  sem_res_tbl_data.vsi = term_info->vsi; 
  sem_res_tbl_data.vsi_assignment_mode = term_info->vsi_assignment_mode;
#ifdef BCM_88660_A0
  ...
  )
{
  ...
  term_info->protection_pass_value = sem_res_tbl_data.protection_path;
  term_info->protection_pointer = sem_res_tbl_data.protection_pointer;
  term_info->service_type = sem_res_tbl_data.service_type;
  term_info->tpid_profile = sem_res_tbl_data.tpid_profile;
  term_info->vsi = sem_res_tbl_data.vsi;
  term_info->vsi_assignment_mode = sem_res_tbl_data.vsi_assignment_mode;
  ...
  )
{
  ...
  if (vsid_ndx < SOC_DPP_DEFS_GET(unit, nof_rifs)) { /* low VSI with its own RIF */
    rif_id = vsid_ndx;
  } else { /* high VSI with default RIF */
    res = READ_IHP_HIGH_VSI_CONFIGr(unit, &reg32_val);
    SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 10, exit);
    rif_id = soc_reg_field_get(unit, NBI_TX_MLF_CONFIGr, reg32_val, HIGH_VSI_IN_RIFf);
  }
  ...
  )
{
  ...
  SOC_SAND_ERR_IF_ABOVE_NOF(rif_ndx, SOC_DPP_DEFS_GET(unit, nof_rifs), ARAD_PP_RIF_ID_OUT_OF_RANGE_ERR, 10, exit);
  res = ARAD_PP_RIF_INFO_verify(unit, rif_info);
  SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
   ...
   )
{
    ...
    SOC_SAND_ERR_IF_ABOVE_NOF(intf_id, SOC_DPP_DEFS_GET(unit, nof_rifs), ARAD_PP_RIF_ID_OUT_OF_RANGE_ERR, 10, exit);
   ...
   )
{
    ...
    else if (SOC_IS_JERICHO(unit)) 
    {
        ...
        uint16 data;
        uint32 data_32;
        uint32 native_routing_vlan_tags_32; 
        int entry_offset = intf_id/8;
  ...
  )
{
  ...
  SOC_SAND_ERR_IF_ABOVE_MAX_AND_NOT_NULL(info->rif, SOC_DPP_DEFS_GET(unit, nof_rifs) - 1, ARAD_PP_RIF_NULL, ARAD_PP_RIF_ID_OUT_OF_RANGE_ERR, 10, exit);
  ...
  SOC_SAND_ERR_IF_ABOVE_MAX(info->service_type, ARAD_RIF_SERVICE_TYPE_MAX, ARAD_RIF_SERVICE_TYPE_OUT_OF_RANGE_ERR, 20, exit); 
