...
#define ARAD_OFP_RATES_EGQ_CAL_LEN_EGQ(unit, chan_arb_id) \
  (((chan_arb_id < SOC_DPP_DEFS_GET(unit, nof_big_channelized_calendars)) || \
    (chan_arb_id > SOC_DPP_DEFS_GET(unit, nof_channelized_calendars))) ? \
   (ARAD_OFP_RATES_CAL_LEN_EGQ_BIG_MAX) : (ARAD_OFP_RATES_CAL_LEN_EGQ_SMALL_MAX))  
  ...
  )
{
  ...
  /* Enable resolution decrese for TCG and QPair shapers */
  ...
  /* empy queues are ignored (field name is confusing, 0 means ignore) */
  ...
  res = arad_sw_db_egr_ports_egq_tcg_qpair_shaper_enable_set(unit,qpair_tcg_shapers_enable);
  SOCDNX_SAND_IF_ERR_EXIT(res);
  ...
  )
{
    ...
    if(found) {
        ...
        *chan_arb_id = ARAD_OFP_RATES_EGQ_CHAN_ARB_INVALID;
    }
              ...
              )
{
    ...
    uint32 
        ...
        rem, 
        expected_credits,
        mask = ~((1 << ARAD_EGQ_NIF_MAL_SCM_TBL_ENTRY_PORT_CR_TO_ADD_NOF_BITS) - 1);
    ...
    for (idx = 0; idx < nof_ports; ++idx) {
        ...
        if (ports_rates[idx] > 0 ) {
            while(TRUE) {
                 ...
                 /*Continue until num of bits for credit per slot below ARAD_EGQ_NIF_MAL_SCM_TBL_ENTRY_PORT_CR_TO_ADD_NOF_BITS */
  ...
  )
{
    ...
    *sch_cal_len = 0;
    ...
    res = READ_SCH_CH_NIF_CALENDAR_CONFIGURATION__CNCCm(unit, SCH_BLOCK(unit, core), sch_offset, &data);
    SOCDNX_IF_ERR_EXIT(res);

    /* Check which calendars (EGQ & SCH - Calendars get 'A' or 'B') are currently active. */
    fld_val = soc_SCH_CH_NIF_CALENDAR_CONFIGURATION__CNCCm_field32_get(unit, &data, DVS_CALENDAR_SEL_CH_NI_FXXf);
    sch_to_get = fld_val;
  
    /* Calendar Length */
    sch_len = soc_SCH_CH_NIF_CALENDAR_CONFIGURATION__CNCCm_field32_get(unit, &data, sch_to_get == 0 ? CAL_A_LENf : CAL_B_LENf);
  ...
  )
{
  ...
  */
  ...
  {
    calcal_length = arad_sw_db_ofp_rates_calcal_length_get(unit, core);
    calcal_instances = arad_sw_db_ofp_rates_nof_instances_get(unit, core, cal_info->chan_arb_id);
  }
  else
  {
  ...
  )
{
    ...
    switch (reg_type) {
        case EGQ_TCG_SPR_DISr:
              /*Reading register*/
               result = READ_EGQ_TCG_SPR_DISr(unit, REG_PORT_ANY, reg_256_val);
               SOCDNX_IF_ERR_EXIT(result);

               SOCDNX_IF_ERR_EXIT(soc_port_sw_db_tm_port_to_out_port_priority_get(unit, core, port, &nof_pairs));
               SOCDNX_IF_ERR_EXIT(soc_port_sw_db_tm_port_to_base_q_pair_get(unit, core, port, &base_q_pair));
               ...
               result=WRITE_EGQ_TCG_SPR_DISr(unit, REG_PORT_ANY, reg_256_val);
               SOCDNX_IF_ERR_EXIT(result);
               break;
       ...
       default:
               /*Reading register*/
               result = READ_EGQ_QPAIR_SPR_DISr(unit, REG_PORT_ANY, reg_256_val);
               SOCDNX_IF_ERR_EXIT(result);
               ...
               /*Writing back to register*/
               result=WRITE_EGQ_QPAIR_SPR_DISr(unit, REG_PORT_ANY, reg_256_val);
               SOCDNX_IF_ERR_EXIT(result);
               break;
       }
exit:
  ...
  )
{
    ...
    int res;
    uint32 q_pair_shp_en_f,reg_val;

    SOCDNX_INIT_FUNC_DEFS;

    if (SOC_IS_ARAD_B0_AND_ABOVE(unit)){

        res=READ_EGQ_EGRESS_SHAPER_ENABLE_SETTINGSr(unit, REG_PORT_ANY,&reg_val);
        SOCDNX_SAND_IF_ERR_EXIT(res);
  ...
  )
{
    ...
    res=arad_port_to_interface_map_get(unit, 0, port,  &if_id, NULL);
    SOCDNX_IF_ERR_EXIT(res);
    ...
    if (otm_shp_en_f) {
        ...
        SOCDNX_IF_ERR_EXIT(soc_port_sw_db_local_to_tm_port_get(unit, port, &fap_port, &core));

        /*store previous rate in user parameter*/
        SOCDNX_IF_ERR_EXIT(soc_port_sw_db_tm_port_to_base_q_pair_get(unit, core, fap_port, &base_q_pair));
        SOCDNX_SAND_IF_ERR_EXIT(arad_sw_db_egq_port_rate_get(unit, core, base_q_pair, &queue_rates[base_q_pair]));

        /* Enable port shaper */
        res=arad_ofp_rates_egq_single_port_rate_sw_set(
             unit,
             core,
             fap_port,
             egq_rate
        ...
        res=arad_ofp_rates_egq_single_port_rate_hw_set(
                unit,
                core,
                fap_port
            );
...
STATIC uint32
  arad_ofp_if_spr_rate_by_reg_val_get(
     SOC_SAND_IN  int                   unit,
     SOC_SAND_IN  ARAD_INTERFACE_ID     if_id,                            
     ...
     )
{
    ...
    res = arad_ofp_rates_if_id2chan_arb_get_unsafe(unit,if_id,&(cal_info.chan_arb_id));
    SOCDNX_IF_ERR_EXIT(res);

    cal_info.cal_type = ARAD_OFP_RATES_EGQ_CAL_CHAN_ARB;

    /* Retreive register and field names for egress shaper rate */
    res = arad_ofp_rates_retrieve_egress_shaper_reg_field_names(
            unit,
            &cal_info,
            ARAD_OFP_RATES_CAL_SET_A,
    ...
    /* Get reg value*/
    (*reg_val) = 0;
    SOC_FIELD32_REG64_GET(reg_shpr_name, field_shpr_rate_name, *reg_val);
  ...
  )
{
    ...
    /* Getting Interface ID */
    res=arad_port_to_interface_map_get(unit, core, tm_port,  &if_id, NULL);
    SOCDNX_IF_ERR_EXIT(res);

    /* Save current interface shaper rate */
    res=arad_ofp_if_spr_rate_by_reg_val_get(unit, if_id,shpr_rate_reg_val);
    SOCDNX_IF_ERR_EXIT(res);
    ...
    /* Set conifguration */    
    res = arad_ofp_rates_egq_interface_shaper_set(unit,core, tm_port, SOC_TMC_OFP_SHPR_UPDATE_MODE_OVERRIDE,max_rate);
    SOCDNX_IF_ERR_EXIT(res);
  ...
  )
{
  ...
  for (slot = 0; slot < egq_cal_len; ++slot)
  {
    ...
    if (egq_data.port_cr_to_add > (1 << ARAD_EGQ_NIF_MAL_SCM_TBL_ENTRY_PORT_CR_TO_ADD_NOF_BITS)) {
          SOCDNX_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_SOCDNX_MSG_STR( "loc_calendar_len is above max")));
    }
  ...
  )
{
    ...
    /* Calendar length SCH */ 
    soc_SCH_CH_NIF_CALENDAR_CONFIGURATION__CNCCm_field32_set(unit, &data, (sch_to_set == 0 ? CAL_A_LENf : CAL_B_LENf), sch_len);
    soc_SCH_CH_NIF_CALENDAR_CONFIGURATION__CNCCm_field32_set(unit, &data, DVS_CALENDAR_SEL_CH_NI_FXXf, sch_to_set);

    res = WRITE_SCH_CH_NIF_CALENDAR_CONFIGURATION__CNCCm(unit, SCH_BLOCK(unit, core), sch_offset, &data);    
    SOCDNX_IF_ERR_EXIT(res);
        ...
        )
{
    ...
    *nof_valid_queues = 0;
    ...
    SOC_PBMP_ITER(pbmp, port_i) {
      ...
      res = arad_sw_db_egr_ports_port_priority_get(unit,core,&rates);
      SOCDNX_SAND_IF_ERR_EXIT(res);
        ...
        )
{
    ...
    *nof_valid_entries = 0;
    ...
    SOC_PBMP_ITER(pbmp, port_i) 
    {
      ...
      res = arad_sw_db_egr_ports_tcg_get(unit,core,&rates);
      SOCDNX_SAND_IF_ERR_EXIT(res);
  ...
  )
{
  ...
  case ARAD_OFP_RATES_GENERIC_FUNC_STATE_TCG_SCH_RATE:
      ...
      max_kbps = (is_sch_valid) ? ARAD_IF_MAX_RATE_KBPS:0;
      if (rate > max_kbps) {
          SOCDNX_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_SOCDNX_MSG_STR( "rate is above max")));
      }
      break;
  ...
  case ARAD_OFP_RATES_GENERIC_FUNC_STATE_TCG_EGQ_RATE:
      ...
      max_kbps = (is_egq_valid) ? ARAD_IF_MAX_RATE_KBPS:0;
      if (rate > max_kbps) {
          SOCDNX_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_SOCDNX_MSG_STR( "rate is above max")));
      }
      break;
    ...
    )
{
    ...
    {
    case ARAD_OFP_RATES_EGQ_NOF_CHAN_ARB:    
        SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG_STR( "Chan arbiter not allocated for the port")));
        break;
    default:
        *chan_arb_id = chan_arb_val;
        break;
    ...
    )
{
    ...
    res = MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_egr_is_channelized, (unit, core, tm_port, &is_channelized_id));
    SOCDNX_IF_ERR_EXIT(res);
    ...
    )
{
    ...
    if (is_channelized_id)
    {
         ...
         res = READ_EGQ_MAP_PS_TO_IFCm(unit, EGQ_BLOCK(unit, core), egress_offset, ps_bmap);
         SOCDNX_IF_ERR_EXIT(res);
        ...
        /* assuming symetric between EGQ PS and SCH PS*/
        for(ps = 0; ps < ARAD_EGR_NOF_PS ; ps++) {
            if(SHR_BITGET(ps_bmap, ps)) {
                res = soc_port_sw_db_tm_port_to_out_port_priority_get(unit, core, tm_port, &nof_prio);
                SOCDNX_IF_ERR_EXIT(res);
                for(prio = 0; prio < ARAD_EGR_NOF_Q_PAIRS_IN_PS ; prio += nof_prio) {
                    base_q_pair = (ps*ARAD_EGR_NOF_Q_PAIRS_IN_PS) + prio;
                    res = arad_sw_db_is_port_valid_get(unit, core, base_q_pair, &is_valid);
                    SOCDNX_IF_ERR_EXIT(res);

                    if(is_valid) {
                        res = arad_sw_db_sch_port_rate_get(unit, core, base_q_pair, &sch_rates[base_q_pair]);
                        SOCDNX_SAND_IF_ERR_EXIT(res);
        ...
        /* in order to avoid momentary packet loss, the order of seting the traversal rate and calendar is important.
         * If rate is decreased then first calender shoould be set. Otherwise, if rate is increased, first traversal rate 
         * should be set. 
         */
    ...
    )
{
    ...
    modified_cals_occ = arad_sw_db_egr_modified_channelized_cals_occ_get(unit, core);

    res = MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_ofp_rates_port2chan_cal_get, (unit, core, tm_port, &chan_arb));
    SOCDNX_IF_ERR_EXIT(res);

    /* mark calendar as modified */
    SOCDNX_SAND_IF_ERR_EXIT(soc_sand_occ_bm_occup_status_set(modified_cals_occ, chan_arb, TRUE));
    ...
    )
{
    ...
    res = arad_sw_db_ofp_rates_calcal_length_set(unit, core, egq_calcal_calendar_len);
    SOCDNX_SAND_IF_ERR_EXIT(res);
    ...
    {
        res = arad_sw_db_ofp_rates_nof_instances_set(unit, core, chan_arb_i, egq_calcal_instances[chan_arb_i]);
        SOCDNX_SAND_IF_ERR_EXIT(res);
    }
    ...
    )
{
    ...
    SOC_DPP_CORES_ITER(SOC_CORE_ALL, core_i) {
        ...
        modified_cals_occ = arad_sw_db_egr_modified_channelized_cals_occ_get(unit, core_i);

        for (chan_arb_i = 0 ; chan_arb_i < SOC_DPP_DEFS_GET(unit, nof_channelized_calendars) ; ++chan_arb_i) {
            SOCDNX_SAND_IF_ERR_EXIT(soc_sand_occ_bm_is_occupied(modified_cals_occ, chan_arb_i, &is_cal_modified));
            if (is_cal_modified) {
                break;
            }
        }

        /* no midified calendar was found on this core, can skip the core */
        if (!is_cal_modified) {
            continue;
        }
        ...
        for (chan_arb_i = 0 ; chan_arb_i < SOC_DPP_DEFS_GET(unit, nof_channelized_calendars) ; ++chan_arb_i) {
            ...
            if (arad_sw_db_ofp_rates_nof_instances_get(unit,core_i,chan_arb_i) > 0) {
                /*Retrive rates from the software database*/
                egq_if_rate_requested = 0;
                ...
                if (is_cal_modified == TRUE) {
                    ...
                    SOCDNX_SAND_IF_ERR_EXIT(soc_sand_occ_bm_occup_status_set(modified_cals_occ, chan_arb_i, FALSE));
                }
                ...
                if (SOC_IS_JERICHO(unit) && calendar_port != -1) {
                    ...
                    if (is_single_cal_mode) { /* single calendar mode => cal rate is set to maximum (becomes irrelevant) */
                        if_internal_rate = SOC_DPP_DEFS_GET(unit, cal_internal_rate_max);

                    } else { /* dual calendar mode => cal rate is set to interface rate */
    ...
    )
{
    ...
    if (arad_sw_db_egr_ports_egq_tcg_qpair_shaper_enable_get(unit))
    {
        /* Getting the fap port's base_q_pair*/
        res = soc_port_sw_db_tm_port_to_base_q_pair_get(unit, core, tm_port, &base_q_pair);
        SOCDNX_IF_ERR_EXIT(res);
  ...
  )
{
  ...
  res = arad_ofp_rates_if_id2chan_arb_get_unsafe(unit,if_id,&chan_arb_id);
  SOCDNX_IF_ERR_EXIT(res);
    ...
    )
{
...
#endif
  {
        ...
        res = arad_sw_db_egr_ports_tcg_set(
                unit,
                core,
                ps,
                tcg_ndx,
    ...
    )
{
    ...
    if (arad_sw_db_egr_ports_egq_tcg_qpair_shaper_enable_get(unit))
    {
        ...
        res = arad_sw_db_egr_ports_port_priority_set(
                unit,
                core,
    ...
    )
{
...
#endif
  {
        ...
        SOC_PBMP_ITER(pbmp, port_i)
        {
          ...
          res = arad_sw_db_egr_ports_tcg_get(unit,core_i,&rates);
          SOCDNX_SAND_IF_ERR_EXIT(res);
    ...
    )
{
    ...
    if (arad_sw_db_egr_ports_egq_tcg_qpair_shaper_enable_get(unit))
    {
        ...
        SOC_PBMP_ITER(pbmp, port_i)
        {
          ...
          res = arad_sw_db_egr_ports_port_priority_get(unit,core_i,&rates);
...
STATIC uint32
    ...
    SOC_SAND_IN  int                   unit,  
    SOC_SAND_IN  int                   core,  
    SOC_SAND_IN  ARAD_FAP_PORT_ID       fap_port, 
    ...
    )
{
    ...
    ARAD_SCH_TCG_RATE_INFO
        tcg_sch_rate_info,
        tcg_sch_rate_configured;
    ARAD_SCH_PORT_PRIORITY_RATE_INFO
        ptc_sch_rate_info,
        ptc_sch_rate_configured;
    ...
    case ARAD_OFP_RATES_GENERIC_FUNC_STATE_PTC_SCH_RATE:
        ...
        switch (func_state) {
        case ARAD_OFP_RATES_GENERIC_FUNC_STATE_PTC_SCH_RATE:
             ptc_sch_rate_info.rate = rate;
             break;
        case ARAD_OFP_RATES_GENERIC_FUNC_STATE_PTC_SCH_BURST:
             ptc_sch_rate_info.max_burst = rate;
             break;
        default:
            SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG_STR( "Illegal enum")));
        }

        res = arad_sch_port_priority_shaper_rate_set_unsafe(
              unit, core,
              fap_port,
              octet_ndx,
              &ptc_sch_rate_info,
    ...
    case ARAD_OFP_RATES_GENERIC_FUNC_STATE_TCG_SCH_BURST:
        ...
        switch (func_state) {
        case ARAD_OFP_RATES_GENERIC_FUNC_STATE_TCG_SCH_RATE:
             tcg_sch_rate_info.rate = rate;
             break;
        case ARAD_OFP_RATES_GENERIC_FUNC_STATE_TCG_SCH_BURST:
             tcg_sch_rate_info.max_burst = rate;
             break;
        ...
        res = arad_sch_tcg_shaper_rate_set_unsafe(
              unit, core,
              fap_port,
              octet_ndx,
              &tcg_sch_rate_info,
