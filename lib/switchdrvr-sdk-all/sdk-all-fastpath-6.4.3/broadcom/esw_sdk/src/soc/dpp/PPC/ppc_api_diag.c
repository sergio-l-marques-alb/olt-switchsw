/* $Id: ppc_api_diag.c,v 1.37 Broadcom SDK $
...
/******************************************************************
*
* FILENAME:       DuneDriver/ppc/src/soc_ppc_api_diag.c
*
* MODULE PREFIX:  ppc
*
* FILE DESCRIPTION:
...
void
  SOC_PPC_DIAG_TRAP_PACKET_INFO_clear(
    SOC_SAND_OUT SOC_PPC_DIAG_TRAP_PACKET_INFO *info
  )
{
  SOC_SAND_INIT_ERROR_DEFINITIONS_NO_DEVID(0);
  SOC_SAND_CHECK_NULL_INPUT(info);

  sal_memset(info, 0x0, sizeof(SOC_PPC_DIAG_TRAP_PACKET_INFO));
  SOC_PPC_TRAP_PACKET_INFO_clear(&(info->pkt_info));
  SOC_SAND_MAGIC_NUM_SET;
exit:
  SOC_SAND_VOID_EXIT_AND_SEND_ERROR(0, 0, 0);
}
  ...
  )
{
  ...
  info->hdr_type = SOC_PPC_NOF_PKT_HDR_STK_TYPES;
  info->hdr_type_arad = SOC_TMC_PARSER_NOF_PKT_HDR_STK_TYPES;
...
void
  SOC_PPC_DIAG_TERMINATED_MPLS_LABEL_INFO_clear(
    SOC_SAND_OUT SOC_PPC_DIAG_TERMINATED_MPLS_LABEL_INFO *info
  )
{
  SOC_SAND_INIT_ERROR_DEFINITIONS_NO_DEVID(0);
  SOC_SAND_CHECK_NULL_INPUT(info);

  sal_memset(info, 0x0, sizeof(SOC_PPC_DIAG_TERMINATED_MPLS_LABEL_INFO));
  info->term_type = SOC_PPC_NOF_DIAG_MPLS_TERM_TYPES;
  info->label = 0;
  SOC_PPC_DIAG_TERM_MPLS_LABEL_INFO_clear(&(info->range_match));
  info->range_bit = 0;
  info->is_bos = 0;
  SOC_SAND_MAGIC_NUM_SET;
exit:
...
void
  SOC_PPC_DIAG_TERM_MPLS_INFO_clear(
    SOC_SAND_OUT SOC_PPC_DIAG_TERM_MPLS_INFO *info
  )
{
  uint32
    ind;
  ...
  sal_memset(info, 0x0, sizeof(SOC_PPC_DIAG_TERM_MPLS_INFO));
  for (ind = 0; ind < 3; ++ind)
  {
    SOC_PPC_DIAG_TERMINATED_MPLS_LABEL_INFO_clear(&(info->lbls_info[ind]));
  }
  SOC_SAND_MAGIC_NUM_SET;
exit:
  SOC_SAND_VOID_EXIT_AND_SEND_ERROR(0, 0, 0);
  ...
  )
{
  ...
  SOC_PPC_DIAG_TERM_MPLS_INFO_clear(&(info->mpls));
...
void
  SOC_PPC_DIAG_PACKET_INFO_clear(
    SOC_SAND_OUT SOC_PPC_DIAG_PACKET_INFO *info
  )
{
  SOC_SAND_INIT_ERROR_DEFINITIONS_NO_DEVID(0);
  SOC_SAND_CHECK_NULL_INPUT(info);

  sal_memset(info, 0x0, sizeof(SOC_PPC_DIAG_PACKET_INFO));
  info->type = SOC_PPC_NOF_PKT_HDR_STK_TYPES;
  info->in_tm_port = 0;
  info->total_size = 0;
  info->nof_packets = 0;
  SOC_PPC_DIAG_ETH_PACKET_INFO_clear(&(info->ether_info));
  SOC_SAND_MAGIC_NUM_SET;
exit:
  SOC_SAND_VOID_EXIT_AND_SEND_ERROR(0, 0, 0);
}
...
void 
  SOC_PPC_DIAG_EGRESS_VLAN_EDIT_INFO_clear(
    SOC_SAND_OUT SOC_PPC_DIAG_EGRESS_VLAN_EDIT_RES *prm_vec_res
  )
{
    SOC_SAND_INIT_ERROR_DEFINITIONS_NO_DEVID(0);
    SOC_SAND_CHECK_NULL_INPUT(prm_vec_res);

    sal_memset(prm_vec_res, 0x0, sizeof(SOC_PPC_DIAG_EGRESS_VLAN_EDIT_RES));
...
void 
  SOC_PPC_DIAG_LIF_PROFILE_INFO_clear(
    SOC_SAND_OUT SOC_PPC_DIAG_LIF_PROFILE_INFO *info
  )
{
  SOC_SAND_INIT_ERROR_DEFINITIONS_NO_DEVID(0);
  SOC_SAND_CHECK_NULL_INPUT(info);

  sal_memset(info, 0x0, sizeof(SOC_PPC_DIAG_LIF_PROFILE_INFO));
  SOC_SAND_MAGIC_NUM_SET;
exit:
  SOC_SAND_VOID_EXIT_AND_SEND_ERROR(0, 0, 0);
}
...
void
  SOC_PPC_DIAG_TRAP_PACKET_INFO_print(
    SOC_SAND_IN  SOC_PPC_DIAG_TRAP_PACKET_INFO *info
  )
{
  SOC_SAND_INIT_ERROR_DEFINITIONS_NO_DEVID(0);
  SOC_SAND_CHECK_NULL_INPUT(info);
  ...
  LOG_CLI((BSL_META_U(unit,
                      "pkt_info:")));
  SOC_PPC_TRAP_PACKET_INFO_print(&(info->pkt_info));
exit:
  SOC_SAND_VOID_EXIT_AND_SEND_ERROR(0, 0, 0);
}
  ...
  )
{
  ...
  {
    LOG_CLI((BSL_META_U(unit,
                        "code %s \n\r"), SOC_PPC_TRAP_CODE_to_string(info->code)));
  ...
  )
{
  ...
  LOG_CLI((BSL_META_U(unit,
                      "code %s "), SOC_PPC_TRAP_CODE_to_string(info->code)));
  LOG_CLI((BSL_META_U(unit,
                      "info:")));
  ...
  )
{
  ...
  for (ind = 0; ind < SOC_PPC_NOF_TRAP_CODES; ++ind)
  {
    if (soc_sand_bitstream_test_bit(info->trap_stack,ind))
    {
      have_traps = TRUE;
      LOG_CLI((BSL_META_U(unit,
                          " %s \n\r"), SOC_PPC_TRAP_CODE_to_string((SOC_PPC_TRAP_CODE)ind)));
    }
  }
  ...
  )
{
  ...
  break;
  case SOC_PPC_DIAG_LEM_LKUP_TYPE_EXTEND_P2P:
    SOC_PPC_FRWRD_EXTEND_P2P_FRWRD_INFO_print(&(info->extend_p2p));
    break;
  default:
    for (ind = 0; ind < 2; ++ind)
    {
  ...
  )
{
  ...
  LOG_CLI((BSL_META_U(unit,
                      "hdr_type %s \n\r"), SOC_PPC_PKT_HDR_STK_TYPE_to_string(info->hdr_type)));
...
void
  SOC_PPC_DIAG_TERMINATED_MPLS_LABEL_INFO_print(
    SOC_SAND_IN  SOC_PPC_DIAG_TERMINATED_MPLS_LABEL_INFO *info
  )
{
  SOC_SAND_INIT_ERROR_DEFINITIONS_NO_DEVID(0);
  SOC_SAND_CHECK_NULL_INPUT(info);
  ...
  LOG_CLI((BSL_META_U(unit,
                      "term_type %s "), SOC_PPC_DIAG_MPLS_TERM_TYPE_to_string(info->term_type)));
  if (info->term_type == SOC_PPC_DIAG_MPLS_TERM_TYPE_NONE)
  {
    goto exit;
  }
  if (info->term_type == SOC_PPC_DIAG_MPLS_TERM_TYPE_LIF_LKUP)
  {
    LOG_CLI((BSL_META_U(unit,
                        "label: %u\n\r"),info->label));
  }
  ...
  if (info->term_type == SOC_PPC_DIAG_MPLS_TERM_TYPE_RANGE)
  {
    LOG_CLI((BSL_META_U(unit,
                        "range_match:")));
    SOC_PPC_DIAG_TERM_MPLS_LABEL_INFO_print(&(info->range_match));
    LOG_CLI((BSL_META_U(unit,
                        "range_bit: %u\n\r"),info->range_bit));
    LOG_CLI((BSL_META_U(unit,
                        "is_bos: %u\n\r"),info->is_bos));
...
void
  SOC_PPC_DIAG_TERM_MPLS_INFO_print(
    SOC_SAND_IN  SOC_PPC_DIAG_TERM_MPLS_INFO *info
  )
{
  uint32
    ind;
  ...
  for (ind = 0; ind < 3; ++ind)
  {
    LOG_CLI((BSL_META_U(unit,
                        "lbls_info[%u]:"),ind));
    SOC_PPC_DIAG_TERMINATED_MPLS_LABEL_INFO_print(&(info->lbls_info[ind]));
  }
exit:
  SOC_SAND_VOID_EXIT_AND_SEND_ERROR(0, 0, 0);
}
  ...
  )
{
  ...
  LOG_CLI((BSL_META_U(unit,
                      "term_type %s "), SOC_PPC_PKT_TERM_TYPE_to_string(info->term_type)));
  if (info->term_type == SOC_PPC_PKT_TERM_TYPE_NONE)
  {
    goto exit;
  }
  if (info->term_type != SOC_PPC_PKT_TERM_TYPE_IPV4_ETH &&
      info->term_type != SOC_PPC_PKT_TERM_TYPE_ETH &&
      info->term_type != SOC_PPC_PKT_TERM_TYPE_TRILL
     )
  {
    LOG_CLI((BSL_META_U(unit,
                        "mpls:")));
    SOC_PPC_DIAG_TERM_MPLS_INFO_print(&(info->mpls));
  }
  LOG_CLI((BSL_META_U(unit,
  ...
  )
{
  ...
  LOG_CLI((BSL_META_U(unit, "|   Phase       | Type  | Val  | Type |                     |V|outlif|   trap-code     |str| qual |\n\r")));
  LOG_CLI((BSL_META_U(unit, " -------------------------------------------------------------------------------------------------\n\r")));
  for (ind = 0; ind < SOC_PPC_NOF_DIAG_FRWRD_DECISION_PHASES; ++ind)
  {
    LOG_CLI((BSL_META_U(unit,
  ...
  )
{
  ...
  for (ind = 0; ind < info->nof_eeps; ++ind)
  {
    ...
    LOG_CLI((BSL_META_U(unit,
                        "encap_info[%u]:"),ind));
    SOC_PPC_EG_ENCAP_ENTRY_INFO_print(&(info->encap_info[ind]));
    LOG_CLI((BSL_META_U(unit,
                        "\n\r")));
  }  
  LOG_CLI((BSL_META_U(unit,
  ...
  )
{
  ...
  for (ind = 0; ind < (SOC_PPC_NOF_DIAG_EG_DROP_REASONS - 1); ++ind)
  {
    ...
    /* bit 31, is not drop reason, it's indication to move to second set of reasons*/
    if(ind == 31) {
    ...
    {
      LOG_CLI((BSL_META_U(unit,
                          " %s \n\r"), SOC_PPC_DIAG_EG_DROP_REASON_to_string((SOC_PPC_DIAG_EG_DROP_REASON)(enum_set | SOC_SAND_BIT(ind)))));
    }
  }
...
void
  SOC_PPC_DIAG_PACKET_INFO_print(
    SOC_SAND_IN  SOC_PPC_DIAG_PACKET_INFO *info
  )
{
  SOC_SAND_INIT_ERROR_DEFINITIONS_NO_DEVID(0);
  SOC_SAND_CHECK_NULL_INPUT(info);
  ...
  LOG_CLI((BSL_META_U(unit,
                      "type %s "), SOC_PPC_PKT_HDR_STK_TYPE_to_string(info->type)));
  LOG_CLI((BSL_META_U(unit,
                      "in_tm_port: %u\n\r"),info->in_tm_port));
  LOG_CLI((BSL_META_U(unit,
                      "total_size: %u\n\r"),info->total_size));
  ...
  LOG_CLI((BSL_META_U(unit,
                      "ether_info:")));
  SOC_PPC_DIAG_ETH_PACKET_INFO_print(&(info->ether_info));
exit:
  SOC_SAND_VOID_EXIT_AND_SEND_ERROR(0, 0, 0);
}
...
void 
  SOC_PPC_DIAG_EGRESS_VLAN_EDIT_INFO_print(
     SOC_SAND_IN SOC_PPC_DIAG_EGRESS_VLAN_EDIT_RES *prm_vec_res
  )
{
  SOC_SAND_INIT_ERROR_DEFINITIONS_NO_DEVID(0);
  SOC_SAND_CHECK_NULL_INPUT(prm_vec_res);
  ...
  )
{
  ...
  if (info->trill_key_type == 0) /* TRILL_MC_RPF */
  { 
    LOG_CLI((BSL_META_U(unit,
  ...
  if (info->trill_key_type == 1) /* TRILL_NICK */
  { 
    LOG_CLI((BSL_META_U(unit,
  ...
  if (info->trill_key_type == 2) /* TRILL_APPOINTED_FORWARDER */
  { 
    LOG_CLI((BSL_META_U(unit,
  ...
  if (info->trill_key_type == 3) /* TRILL_NATIVE_INNER_TPID */
  {
     LOG_CLI((BSL_META_U(unit,
  ...
  if (info->trill_key_type == 4) /* TRILL_VSI */
  {    
    LOG_CLI((BSL_META_U(unit,
                        "trill key TRILL VSI:\n\r")));
    LOG_CLI((BSL_META_U(unit,
...
void
  SOC_PPC_DIAG_LIF_PROFILE_INFO_print(
    SOC_SAND_IN SOC_PPC_DIAG_LIF_PROFILE_INFO *info
  ){
  uint8
    pfl_bit;
  ...
  LOG_CLI((BSL_META_U(unit,
                      "InLif Profile\n\r=============\n\r")));
  for (pfl_bit = 0; pfl_bit < SOC_PPC_PROFILE_MGMT_INLIF_PROFILE_NOF_BITS; pfl_bit++) {
    LOG_CLI((BSL_META_U(unit,
                      "\tBit %d : %s\n\r"), pfl_bit,
                          SOC_PPC_INLIF_PROFILE_TYPE_to_string((info->inlif_bits[pfl_bit]))));
  }
  LOG_CLI((BSL_META_U(unit,
                      "\n\rOutLif Profile\n\r==============\n\r")));
  for (pfl_bit = 0; pfl_bit < SOC_PPC_PROFILE_MGMT_OUTLIF_PROFILE_NOF_BITS_MAX; pfl_bit++) {
    LOG_CLI((BSL_META_U(unit,
                      "\tBit %d : %s\n\r"), pfl_bit,
                          SOC_PPC_OUTLIF_PROFILE_TYPE_to_string((info->outlif_bits[pfl_bit]))));
  }
  LOG_CLI((BSL_META_U(unit,
                      "\n\rRif Profile\n\r===========\n\r")));
  for (pfl_bit = 0; pfl_bit < SOC_PPC_PROFILE_MGMT_RIF_PROFILE_NOF_BITS; pfl_bit++) {
    LOG_CLI((BSL_META_U(unit,
                      "\tBit %d : %s\n\r"), pfl_bit,
                          SOC_PPC_RIF_PROFILE_TYPE_to_string((info->rif_bits[pfl_bit]))));
  }
exit:
  SOC_SAND_VOID_EXIT_AND_SEND_ERROR(0, 0, 0);
}

const char * SOC_PPC_INLIF_PROFILE_TYPE_to_string(
	...
	case SOC_PPC_PROFILE_MGMT_INLIF_PROFILE_TYPE_SIMPLE_SAME_INTERFACE:
		str = "EVB / MPLS MP (Same interface filtering)";
		break;
	case SOC_PPC_PROFILE_MGMT_INLIF_PROFILE_TYPE_OAM:
		str = "OAM default lif profile";
		break;
	case SOC_PPC_PROFILE_MGMT_INLIF_PROFILE_TYPE_TAGGED_MODE:
		str = "PWE tagged mode";
		break;
	case SOC_PPC_PROFILE_MGMT_INLIF_PROFILE_TYPE_SIMPLE_DSCP_MARKING:
		str = "DSCP/EXP marking profile when bridging";
		break;
...
const char * SOC_PPC_OUTLIF_PROFILE_TYPE_to_string(
       SOC_SAND_IN SOC_PPC_PROFILE_MGMT_OUTLIF_PROFILE_TYPE enum_val
    ) {
	const char* str = NULL;
	switch (enum_val) {
	case SOC_PPC_PROFILE_MGMT_OUTLIF_PROFILE_TYPE_OAM_PCP:
		str = "OAM PCP lif profile";
		break;
	case SOC_PPC_PROFILE_MGMT_OUTLIF_PROFILE_TYPE_OAM_DEFAULT_MEP:
		str = "OAM default lif profile";
		break;
	case SOC_PPC_PROFILE_MGMT_OUTLIF_PROFILE_TYPE_TAGGED_MODE:
		str = "PWE tagged mode";
		break;
	default:
		str = "Unreserved";
...
const char * SOC_PPC_RIF_PROFILE_TYPE_to_string(
...
const char * SOC_PPC_INLIF_PROFILE_TYPE_to_short_string(
       SOC_SAND_IN SOC_PPC_PROFILE_MGMT_INLIF_PROFILE_TYPE enum_val
    ) {
	const char* str = NULL;
	switch (enum_val) {
...
const char * SOC_PPC_OUTLIF_PROFILE_TYPE_to_short_string(
       SOC_SAND_IN SOC_PPC_PROFILE_MGMT_OUTLIF_PROFILE_TYPE enum_val
    ) {
	const char* str = NULL;
	switch (enum_val) {
...
const char * SOC_PPC_RIF_PROFILE_TYPE_to_short_string(
       SOC_SAND_IN SOC_PPC_PROFILE_MGMT_RIF_PROFILE_TYPE enum_val
    ) {
	const char* str = NULL;
	switch (enum_val) {
