...
/* $Id: arad_tdm.c,v 1.36 Broadcom SDK $
...
#define ARAD_TDM_CELL_SIZE_MAX                                   (252)
...
#define ARAD_TDM_FTMH_OPT_MC_MC_ID_MAX                           (16*1024-1)
...
#define ARAD_TDM_FTMH_ARAD_OPT_MODE_VAL_FLD                      (0x4)
#define ARAD_TDM_FTMH_ARAD_STA_MODE_VAL_FLD                      (0x5)
...
/*  uint32
  ...
  if (!(tdm_mode == ARAD_MGMT_TDM_MODE_PACKET)) /* TDM bypass is enabled at least in one port */
  {
   ...
   /*  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  22,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, EPNI_GLOBAL_10r, REG_PORT_ANY, 0, EGRESS_TDM_MODEf,  dram_enable)); */
        
    /* Enable 2 bytes ftmh only in optimize ftmh mode */
    fld_val = (tdm_mode == ARAD_MGMT_TDM_MODE_TDM_OPT)?0x1:0x0;
    /*ARAD_FLD_SET(ECI_TDM_CONFIGURATIONr, TDM_2BYTES_FTMHf, fld_val, 30, exit);*/
  ...
  else
  {
    ...
    /*SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  45,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, EGQ_TDM_CONFIGURATIONr, REG_PORT_ANY, 0, EGRESS_TDM_MODEf,  fld_val));*/
    /* SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  47,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, EPNI_GLOBAL_10r, REG_PORT_ANY, 0, EGRESS_TDM_MODEf,  fld_val)); */
  }

  /* Fragment number */
  /* Enable EGQ frag-num */
  fld_val = 0x1;
  /*SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  50,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, EGQ_TDM_, SOC_CORE_ALL, 0, TDM_FDR_FRAG_NUM1_ENf,  fld_val));    */
...
/*    SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  61,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, EPNI_GLOBAL_10r, REG_PORT_ANY, 0, TDM_FRG_NUMf,  fld_val)); */ 
  ...
  fld_val = (vsc_mode /* 128 */) ? ARAD_TDM_PB_CELL_SIZE_MAX:ARAD_TDM_CELL_SIZE_MAX;
  ...
  { /* Always enable as Packet mode can be set parallel to any mode */
    ...
    /* Strip crc for TDM packets, always disable as in ARAD we always add Fabric CRC at the ingress */
    fld_val = 0x0;
  ...
  )
{
  ...
  switch(ftmh_mode)
  {
    ...
    case ARAD_TDM_FTMH_INFO_MODE_STANDARD_UC:
      ...
      /* User define more than 32bits only in ARAD FTMH */
      if (!is_petrab_in_system)
      {
        tmp = 0;
        res = soc_sand_bitstream_get_any_field(
  ...
  )
{
  ...
  switch(action_eg)
  {
    case ARAD_TDM_EG_ACTION_REMOVE:
      header_type = ARAD_PORT_HEADER_TYPE_TDM; /* No matter it is TDM or TDM_RAW */
      break;
    case ARAD_TDM_EG_ACTION_CUSTOMER_EXTRACT: /* PMM */
      header_type = ARAD_PORT_HEADER_TYPE_TDM_PMM;
      break;
    case ARAD_TDM_EG_ACTION_NO_CHANGE:  /* Don't touch */
  ...
  )
{
  ...
  switch (arad_ire_tdm_config_tbl_data.mode)
  {
    case ARAD_TDM_FTMH_EXTERNAL_MODE_VAL_FLD:
      *action_ing = ARAD_TDM_ING_ACTION_CUSTOMER_EMBED;
      break;
    case ARAD_TDM_FTMH_UNCHANGED_MODE_VAL_FLD:
      *action_ing = ARAD_TDM_ING_ACTION_NO_CHANGE;
      break;
    case ARAD_TDM_FTMH_PB_OPT_MODE_VAL_FLD:
    case ARAD_TDM_FTMH_PB_STA_MODE_VAL_FLD:
    case ARAD_TDM_FTMH_ARAD_OPT_MODE_VAL_FLD:
    case ARAD_TDM_FTMH_ARAD_STA_MODE_VAL_FLD:
    ...
    else
    {
      ...
      if (tmp == ARAD_TDM_FTMH_STA_TYPE_UC)
      {
        ...
        *is_mc = FALSE;
        ...
        /* User define fields for more than 32bits. Valid only when working in ARAD FTMH */
        if (!is_petrab_in_system)
        {
          tmp = 0;
          res = soc_sand_bitstream_get_any_field(
  ...
  )
{
  ...
  if(soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "user_header_always_remove", 0) == 1) { 
    res = arad_egr_prog_editor_tm_profile_enum_get(unit, egq_pct_data.prog_editor_profile, &eg_tm_profile);
  ...
  switch(eg_tm_profile) {
  case ARAD_PRGE_TM_SELECT_REMOVE_SYSTEM_HEADER:
  case ARAD_PRGE_TM_SELECT_REMOVE_TDM_OPT_FTMH:
    *action_eg = ARAD_TDM_EG_ACTION_REMOVE;
    break;
  case ARAD_PRGE_TM_SELECT_TDM_PMM_HEADER:
    *action_eg = ARAD_TDM_EG_ACTION_CUSTOMER_EXTRACT;
    break;
  case ARAD_PRGE_TM_SELECT_NONE:
  ...
  )
{
  ...
  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  20, exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, IRE_TDM_SIZEr, REG_PORT_ANY, 0, TDM_MIN_SIZEf,  fld_val));

  fld_val = size_range->end;
  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  30, exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, IRE_TDM_SIZEr, REG_PORT_ANY, 0, TDM_MAX_SIZEf,  fld_val));
  ...
  )
{
  ...
  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  20, exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_read(unit, IRE_TDM_SIZEr, REG_PORT_ANY, 0, TDM_MIN_SIZEf, &fld_val));
  size_range->start = fld_val;

  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  30, exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_read(unit, IRE_TDM_SIZEr, REG_PORT_ANY, 0, TDM_MAX_SIZEf, &fld_val));
  ...
  )
{
  ...
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 1220, exit, soc_port_sw_db_tm_port_to_base_q_pair_get(unit, core, port_ndx, &base_q_pair));
  ...
  )
{
    ...
    reassembly_context = arad_sw_db_local_to_reassembly_context_get(unit, port);
    ...
    {  
        ...
        if (is_tdm) 
        {
            ...
            for (find_context_map_val = 0; find_context_map_val < ARAD_NOF_TDM_CONTEXT_MAP; find_context_map_val++) {
                SOC_SAND_IF_ERR_EXIT(arad_sw_db_tdm_context_map_get(unit,find_context_map_val,&used_if_id));

                if (used_if_id == ARAD_IF_ID_NONE || used_if_id == info_incoming.if_id) {
                    /* We found a match */
                    context_map_val = find_context_map_val;

                    /* Update SW DB if neccesary */
                    if (used_if_id == ARAD_IF_ID_NONE) {
                        SOC_SAND_IF_ERR_EXIT(arad_sw_db_tdm_context_map_set(unit,find_context_map_val,info_incoming.if_id));
                    }
                    break;
                }
            }
  ...
  )
{
  ...
  if (configure_ingress) {
      ...
      uint32 reassembly_context = arad_sw_db_local_to_reassembly_context_get(unit, port_ndx);
  ...
  )
{
  ...
  if (is_egress_enabled) { /* get egress configuration */
      soc_reg_above_64_val_t data, field_val;
  ...
  )
{
  ...
  if (info->link_bitmap.arr[1] >> (SOC_DPP_DEFS_GET(unit, nof_fabric_links) - 32))
  {
