  ...
  )
{
  ...
  if(shaper->max_rate != SOC_TMC_ING_SCH_DONT_TOUCH) {
      ...
      /* if delay is not in 2 clocks resolution */
      if (!is_delay_2_clocks) 
      {
         shaper_delay_2_clocks *= 2; 
      }
  ...
  )
{
  ...
  )
  {
    ...
    if (delay_value > (ARAD_ING_SCH_MAX_DELAY_VAL*2))
    {
      cal_value /= 2;
    }
    else if (delay_value < (ARAD_ING_SCH_MIN_DELAY_VAL*2))
    {
      cal_value *= 2;
  ...
  if(delay_value > (ARAD_ING_SCH_MAX_DELAY_VAL*2))
  {
    delay_value = (ARAD_ING_SCH_MAX_DELAY_VAL*2);
  }
