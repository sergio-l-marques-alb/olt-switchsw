    ...
    )
{
  ...
  SOC_PBMP_ITER(pbmp, logical_port)
  {
    ...
    res = soc_port_sw_db_tm_port_to_base_q_pair_get(unit, core, tm_port, &base_queue_pair);
  ...
  )
{
  ...
  res = READ_NBI_FC_PFC_MLF_MASKr(unit, reg_offset, &pfc_map_rx_mlf_reg);
  SOC_SAND_CHECK_FUNC_RESULT(res, 60, exit);
  
  soc_reg_field_set(unit, NBI_FC_PFC_MLF_MASKr, &pfc_map_rx_mlf_reg, rx_mlf_pfc_fld[fld_offset], info->nif_cls_bitmap);

  res = WRITE_NBI_FC_PFC_MLF_MASKr(unit, reg_offset, pfc_map_rx_mlf_reg);
  SOC_SAND_CHECK_FUNC_RESULT(res, 70, exit);
  ...
  )
{
  ...
  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  20,  exit, ARAD_REG_ACCESS_ERR, 
    soc_reg_field32_modify(unit, PORT_CONFIGr, SOC_INFO((unit)).port_p2l_mapping[nif_intern_id+1], LLFC_ENf, fld_val));

  fld_val = SOC_SAND_BOOL2NUM(info->mode != SOC_TMC_FC_INBND_MODE_LL);
  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  20,  exit, ARAD_REG_ACCESS_ERR, 
    soc_reg_field32_modify(unit, PORT_CONFIGr, SOC_INFO((unit)).port_p2l_mapping[nif_intern_id+1], PFC_ENABLEf, fld_val));
  ...
  )
{
    ...
    /* LLFC */
    if (nif_type == ARAD_NIF_TYPE_100G_CGE || nif_type == ARAD_NIF_TYPE_ILKN) {  
        /* LLFC */
  ...
  )
{
    ...
    res = arad_fc_port_fifo_threshold_verify(unit, nif_ndx, info);
