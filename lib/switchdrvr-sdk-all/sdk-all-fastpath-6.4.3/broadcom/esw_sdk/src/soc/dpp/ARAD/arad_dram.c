...
int soc_dpp_jer_dram_mmu_set(int unit, soc_dpp_drc_combo28_info_t *drc_info) 
{
    ...
    /* 
     * MMU_GLOBAL_ADDR_TRANS_CFG configuration
     */
    SOCDNX_IF_ERR_EXIT(READ_MMU_GLOBAL_ADDR_TRANS_CFGr(unit, &reg32_val));
    ...
    SOC_DPP_CONFIG(unit)->arad->init.dram.interleaving_bits = interleaving_bits;
    soc_reg_field_set (unit, MMU_GLOBAL_ADDR_TRANS_CFGr, &reg32_val, BANK_INTERLEAVING_MODEf, field32_val);
    soc_reg_field_set (unit, MMU_GLOBAL_ADDR_TRANS_CFGr, &reg32_val, INTERLEAVING_RVRSE_MODEf, 0);
    soc_reg_field_set (unit, MMU_GLOBAL_ADDR_TRANS_CFGr, &reg32_val, LAYER_PERIODICITYf, (((drc_info->dram_num == 3) || (drc_info->dram_num == 6)) ? 1 : 0));
    soc_reg_field_set (unit, MMU_GLOBAL_ADDR_TRANS_CFGr, &reg32_val, ADDRESS_MAP_CONFIG_GENf, 1);
    soc_reg_field_set (unit, MMU_GLOBAL_ADDR_TRANS_CFGr, &reg32_val, BANK_GROUP_TOPOf, bank_group_topo);
    soc_reg_field_set (unit, MMU_GLOBAL_ADDR_TRANS_CFGr, &reg32_val, ENABLE_BG_ALGOf, 1);
    soc_reg_field_set(unit, MMU_GLOBAL_ADDR_TRANS_CFGr, &reg32_val, ONE_DRAM_MODEf, 0);
    ...
    soc_reg_field_set(unit, MMU_GLOBAL_ADDR_TRANS_CFGr, &reg32_val, DRAM_NUMf, dram_num);

    SOCDNX_IF_ERR_EXIT(WRITE_MMU_GLOBAL_ADDR_TRANS_CFGr(unit, reg32_val));
    ...
    case 4: /* default drams are A,B,C and D, when using symetric mode switching to A,C,F and H */
        ...
        if (SOC_PBMP_MEMBER(drc_info->dram_bitmap, 7)) {
    ...
    SOCDNX_IF_ERR_EXIT(READ_MMU_GENERAL_CONFIGURATION_REGISTERr(unit, &reg32_val));
    ...
    SOCDNX_IF_ERR_EXIT(WRITE_MMU_GENERAL_CONFIGURATION_REGISTERr(unit, reg32_val));
...
/*
 * Function:
 *      soc_dpp_drc_initiate_drc_with_bmap
 * Purpose:
 *      initialize relevant DRCs according to set bits in bitmap
 * Parameters:
 *      unit                - Device Number
 *      drc_info            - drc info relevant for the initialization rutines
 *      bitmap_to_iterate   - bitmap according to which DRCs are initialized
 * Returns:
 *      SOC_E_XXX 
 */
int soc_dpp_drc_initiate_drc_with_bmap(int unit, soc_dpp_drc_combo28_info_t *drc_info, soc_pbmp_t bitmap_to_iterate)
{
    int drc_ndx;
    SOCDNX_INIT_FUNC_DEFS;

    SOC_PBMP_ITER(bitmap_to_iterate, drc_ndx) {
...
int soc_dpp_drc_combo28_dram_init(int unit, soc_dpp_drc_combo28_info_t *drc_info)
{
    ...
    SOC_PBMP_CLEAR(bitmap_to_iterate);
    SOC_PBMP_OR(bitmap_to_iterate, drc_info->dram_bitmap);
    SOC_PBMP_AND(bitmap_to_iterate, drc_info->ref_clk_bitmap);
    soc_dpp_drc_initiate_drc_with_bmap(unit, drc_info, bitmap_to_iterate);

    /* initiate slave DRCs */
    SOC_PBMP_CLEAR(bitmap_to_iterate);
    SOC_PBMP_NEGATE(bitmap_to_iterate, drc_info->ref_clk_bitmap);
    SOC_PBMP_AND(bitmap_to_iterate, drc_info->dram_bitmap);
    soc_dpp_drc_initiate_drc_with_bmap(unit, drc_info, bitmap_to_iterate);
