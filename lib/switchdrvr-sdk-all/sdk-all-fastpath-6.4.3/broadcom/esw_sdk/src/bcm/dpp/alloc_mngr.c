 ...
 *
 *  The initial configuration that is established here, and exported to the
 *  rest of the DPP code.
 *
 *  Hardware VSI resource: 64K elements, split into 3 pools
 *      TB VSI: 0..4095, used for traditional bridging VLANs
 *      MSTP: 4096..16383, MyMac disabled / mapped to nonzero MSTP topo ID
 *      Dynamic: 16384..65535, dynamic mapping
 *
 *  Hardware LIF resource: 16K[???] elements, split into 2 pools
 *      TB VSI: 0..4095, used for traditional bridging VLANs
 *      Dynamic: 4096..16384[???], used for other purposes
 *
 *  Hardware VD resource: [???] elements, one pool
 *      Dynamic: 1..[???], used for VLAN mapping and similar
 *               note that element 0 is allocated by default during startup
 *
 *  Multicast groups: [???] elements, split into 2 pools
 *      TB Flood: 0..4095, used for flooding on tradifional bridging VLANs
 *      Dynamic: 4096..[???], used fobcm_dpp_user_defined_traps_tr other multicasting
 *
 *  FEC entries: 16K[???] elements, split into two pools
 *      Global: [???]..[???], used for global VSI membership
 *      Local: [???]..[???], used for ECMP or failover or similar
 *
 *  EEP entries: [???] elements, split into two pools
 *      Global: [???]..[???], used for aggregates and similar
 *      Local: [???]..[???], used for local purposes
 *
 *  All of the parameters can be changed by making changes to the configuration
 *  that is established in the _bcm_dpp_resources_create function, below.
 */
...
#define _DPP_AM_TEMPLATE_COSQ_EGR_INTERFACE_UNICAST_THRESH_MAX_ENTITIES (SOC_TMC_IF_NOF_NIFS)
...
#define _DPP_AM_TEMPLATE_COSQ_EGR_INTERFACE_MULTICAST_THRESH_MAX_ENTITIES (SOC_TMC_IF_NOF_NIFS)
...
#define _DPP_AM_TEMPLATE_USER_DEFINED_TRAPS_COUNT (SOC_PPC_TRAP_CODE_USER_DEFINED_LAST - SOC_PPD_TRAP_CODE_USER_DEFINED_0 + 1)
#define _DPP_AM_TEMPLATE_USER_DEFINED_TRAPS_MAX_ENTITIES(unit) (_BCM_PETRA_NOF_PP_PORTS(unit))
...
#define _DPP_AM_TEMPLATE_SNOOP_CMD_MAX_ENTITIES(unit) (_BCM_PETRA_NOF_PP_PORTS(unit) + SOC_PPD_NOF_TRAP_CODES)
...
#define _DPP_AM_TEMPLATE_L2_RSRVD_MC_MAX_ENTITIES(unit) (_BCM_PETRA_NOF_PP_PORTS(unit))
...
#define _DPP_AM_TEMPLATE_PORT_TPID_CLASS_MAX_ENTITIES(unit) (_BCM_PETRA_NOF_PP_PORTS(unit))
...
#define _DPP_AM_TEMPLATE_PORT_TPID_CLASS_EGRESS_ACCEPTABLE_FRAME_TYPE_MAX_ENTITIES(unit) (_BCM_PETRA_NOF_PP_PORTS(unit))   
...
#define _DPP_AM_TEMPLATE_FID_AGING_PROFILE_LOW_ID (0)
#define _DPP_AM_TEMPLATE_FID_AGING_PROFILE_COUNT (4)
#define _DPP_AM_TEMPLATE_FID_AGING_PROFILE_MAX_ENTITIES ((SOC_DPP_CONFIG(unit))->l2.nof_vsis + 1)  /* nof vsis + adding 1 dummy for default profiles 0(static) and 1(dynamic) an in some cases 3(slb)*/
#define _DPP_AM_TEMPLATE_FID_AGING_PROFILE_SIZE         (sizeof(int))

/* l2 flooding, retreive LIF infomation  */
...
#define _DPP_AM_TEMPLATE_VLAN_PORT_PROTOCOL_PROFILE_MAX_ENTITIES (_BCM_PETRA_NOF_PP_PORTS(unit) + 1) /* 1 dummy port so NOF protocol will be reserved */  
...
#define _DPP_AM_TEMPLATE_IP_TUNNEL_SRC_IP_SIZE (sizeof(uint32))   /* sip zero is not valid  */
...
#define _DPP_AM_TEMPLATE_L2_TRAP_RSRVD_MC_MAX_ENTITIES(unit) (_BCM_PETRA_NOF_PP_PORTS(unit)*_BCM_PETRA_L2_NOF_RSRV_MC)
...
#define _DPP_AM_TEMPLATE_MEP_ID_SHORT_COUNT (SOC_PPD_OAM_MAX_NUMBER_OF_LOCAL_MEPS(unit) - _DPP_AM_TEMPLATE_MEP_ID_SHORT_LOW_ID )


/* OAM icc map ndx */
#define _DPP_AM_TEMPLATE_OAM_ICC_MAP_NDX_LOW_ID (0)
#define _DPP_AM_TEMPLATE_OAM_ICC_MAP_NDX_COUNT (15)
#define _DPP_AM_TEMPLATE_OAM_ICC_MAP_NDX_MAX_ENTITIES (SOC_PPD_OAM_MAX_NUMBER_OF_LOCAL_MEPS(unit)) 
#define _DPP_AM_TEMPLATE_OAM_ICC_MAP_NDX_SIZE (sizeof(SOC_PPD_OAM_ICC_MAP_DATA))

/* OAM SA MSB profiles (Jericho only)*/
#define _DPP_AM_TEMPLATE_OAM_SA_MSB_LOW_ID (0)
#define _DPP_AM_TEMPLATE_SA_MSB_NDX_COUNT (2)
#define _DPP_AM_TEMPLATE_OAM_SA_MSB_MAX_ENTITIES (SOC_PPD_OAM_MAX_NUMBER_OF_LOCAL_MEPS(unit)) 
#define _DPP_AM_TEMPLATE_OAM_SA_MSB_SIZE (5) /* representing the 5 MSBs of the SA MAC address.*/


/* OAM OAMP punt event hendling profile */
#define _DPP_AM_TEMPLATE_OAM_OAMP_PUNT_PROFILE_LOW_ID (0)
#define _DPP_AM_TEMPLATE_OAM_OAMP_PUNT_PROFILE_COUNT (SOC_PPD_OAM_OAMP_NUMBER_OF_PUNT_PROFILES)
#define _DPP_AM_TEMPLATE_OAM_OAMP_PUNT_PROFILE_MAX_ENTITIES (SOC_PPD_OAM_MAX_NUMBER_OF_REMOTE_MEPS(unit)) 
#define _DPP_AM_TEMPLATE_OAM_OAMP_PUNT_PROFILE_SIZE (sizeof(SOC_PPD_OAM_OAMP_PUNT_PROFILE_DATA))

/* OAM mep profile non accelerated */
#define _DPP_AM_TEMPLATE_OAM_MEP_PROFILE_NON_ACC_LOW_ID (0)
#define _DPP_AM_TEMPLATE_OAM_MEP_PROFILE_NON_ACC_COUNT (SOC_PPD_OAM_NON_ACC_PROFILES_NUM-1) /* Arad only Highest profile is reseved for passive */
#define _DPP_AM_TEMPLATE_OAM_MEP_PROFILE_NON_ACC_COUNT_ARAD_PLUS (SOC_PPD_OAM_NON_ACC_PROFILES_ARAD_PLUS_NUM) /* Arad + only*/
#define _DPP_AM_TEMPLATE_BFD_MEP_PROFILE_NON_ACC_MAX_ENTITIES (SOC_PPD_OAM_MAX_NUMBER_OF_LOCAL_MEPS(unit)) 
#define _DPP_AM_TEMPLATE_OAM_MEP_PROFILE_NON_ACC_MAX_ENTITIES (SOC_PPD_OAM_MAX_NUMBER_OF_LOCAL_MEPS(unit)*8/*levels per lif*/*2/*directions*/) 
#define _DPP_AM_TEMPLATE_OAM_MEP_PROFILE_NON_ACC_SIZE (sizeof(SOC_PPD_OAM_LIF_PROFILE_DATA))

/* OAM mep profile accelerated */
#define _DPP_AM_TEMPLATE_OAM_MEP_PROFILE_ACC_LOW_ID (0)
#define _DPP_AM_TEMPLATE_OAM_MEP_PROFILE_ACC_COUNT (SOC_PPD_OAM_ACC_PROFILES_NUM-1) /* Highest profile is reseved for passive */
#define _DPP_AM_TEMPLATE_OAM_MEP_PROFILE_ACC_MAX_ENTITIES (SOC_PPD_OAM_MAX_NUMBER_OF_LOCAL_MEPS(unit)) 
#define _DPP_AM_TEMPLATE_OAM_MEP_PROFILE_ACC_SIZE (sizeof(SOC_PPD_OAM_LIF_PROFILE_DATA))

/* OAM priority profil */
#define _DPP_AM_TEMPLATE_OAM_TX_PRIORITY_PROFILE_LOW_ID (0)
#define _DPP_AM_TEMPLATE_OAM_TX_PRIORITY_PROFILE_COUNT (8)
#define _DPP_AM_TEMPLATE_OAM_TX_PRIORITY_PROFILE_MAX_ENTITIES (SOC_PPD_OAM_MAX_NUMBER_OF_LOCAL_MEPS(unit)) 
#define _DPP_AM_TEMPLATE_OAM_TX_PRIORITY_PROFILE_SIZE (sizeof(SOC_PPD_OAMP_TX_ITMH_ATTRIBUTES))

/* BFD Profiles */
#define _DPP_AM_TEMPLATE_BFD_IPV4_DIP_PROFILE_LOW_ID (0)
#define _DPP_AM_TEMPLATE_BFD_IPV4_DIP_PROFILE_COUNT (16)
#define _DPP_AM_TEMPLATE_BFD_IPV4_DIP_PROFILE_MAX_ENTITIES (SOC_PPD_OAM_MAX_NUMBER_OF_LOCAL_MEPS(unit)) 
#define _DPP_AM_TEMPLATE_BFD_IPV4_DIP_PROFILE_SIZE (sizeof(uint32))

#define _DPP_AM_TEMPLATE_BFD_PUSH_PROFILE_LOW_ID (0)
#define _DPP_AM_TEMPLATE_BFD_PUSH_PROFILE_COUNT (16)
#define _DPP_AM_TEMPLATE_BFD_PUSH_PROFILE_MAX_ENTITIES (SOC_PPD_OAM_MAX_NUMBER_OF_LOCAL_MEPS(unit)) 
#define _DPP_AM_TEMPLATE_BFD_PUSH_PROFILE_SIZE (sizeof(SOC_PPD_MPLS_PWE_PROFILE_DATA))

#define _DPP_AM_TEMPLATE_BFD_REQ_INTERVAL_LOW_ID (0)
#define _DPP_AM_TEMPLATE_BFD_REQ_INTERVAL_COUNT (8)
#define _DPP_AM_TEMPLATE_BFD_REQ_INTERVAL_MAX_ENTITIES (SOC_PPD_OAM_MAX_NUMBER_OF_LOCAL_MEPS(unit)) 
#define _DPP_AM_TEMPLATE_BFD_REQ_INTERVAL_SIZE (sizeof(uint32))

#define _DPP_AM_TEMPLATE_BFD_TOS_TTL_PROFILE_LOW_ID (0)
#define _DPP_AM_TEMPLATE_BFD_TOS_TTL_PROFILE_COUNT (16 - (1 && SOC_DPP_CONFIG(unit)->pp.bfd_ipv4_single_hop))
#define _DPP_AM_TEMPLATE_BFD_TOS_TTL_PROFILE_MAX_ENTITIES (SOC_PPD_OAM_MAX_NUMBER_OF_LOCAL_MEPS(unit)) 
...
#define _DPP_AM_TEMPLATE_BFD_SRC_IP_PROFILE_MAX_ENTITIES (SOC_PPD_OAM_MAX_NUMBER_OF_LOCAL_MEPS(unit)) 
#define _DPP_AM_TEMPLATE_BFD_SRC_IP_PROFILE_SIZE (sizeof(uint32))

#define _DPP_AM_TEMPLATE_BFD_TX_RATE_PROFILE_LOW_ID (0)
#define _DPP_AM_TEMPLATE_BFD_TX_RATE_PROFILE_COUNT (8)
#define _DPP_AM_TEMPLATE_BFD_TX_RATE_PROFILE_MAX_ENTITIES (SOC_PPD_OAM_MAX_NUMBER_OF_LOCAL_MEPS(unit)) 
#define _DPP_AM_TEMPLATE_BFD_TX_RATE_PROFILE_SIZE (sizeof(uint32))

#define _DPP_AM_TEMPLATE_BFD_FLAGS_PROFILE_LOW_ID (1) /* HW constraint */
#define _DPP_AM_TEMPLATE_BFD_FLAGS_PROFILE_COUNT (15)
#define _DPP_AM_TEMPLATE_BFD_FLAGS_PROFILE_MAX_ENTITIES (SOC_PPD_OAM_MAX_NUMBER_OF_LOCAL_MEPS(unit)) 
...
#define _DPP_AM_TEMPLATE_OAM_LM_DA_TABLES_MAX_ENTITIES (SOC_PPD_OAM_MAX_NUMBER_OF_LOCAL_MEPS(unit)) 
#define _DPP_AM_TEMPLATE_OAM_LM_DA_TABLES_SIZE (sizeof(uint32))

#define _DPP_AM_TEMPLATE_OAM_ETH1731_PROFILE_LOW_ID 0
#define _DPP_AM_TEMPLATE_OAM_ETH1731_PROFILE_COUNT SOC_PPC_OAM_MAX_NUMBER_OF_ETH1731_MEP_PROFILES(unit)
#define _DPP_AM_TEMPLATE_OAM_ETH1731_PROFILE_MAX_ENTITIES (SOC_PPD_OAM_MAX_NUMBER_OF_LOCAL_MEPS(unit)) 
#define _DPP_AM_TEMPLATE_OAM_ETH1731_PROFILE_SIZE (sizeof(SOC_PPD_OAM_ETH1731_MEP_PROFILE_ENTRY))
...
#define _DPP_AM_TEMPLATE_OAM_LOCAL_PORT_2_SYS_PORT_MAX_ENTITIES (SOC_PPD_OAM_MAX_NUMBER_OF_LOCAL_MEPS(unit)) 
...
#define _DPP_AM_TEMPLATE_OAM_OAMP_PE_GEN_MEM_MAX_ENTITIES (SOC_PPD_OAM_MAX_NUMBER_OF_LOCAL_MEPS(unit)) 
...
/* Profile 0 means means NO ACTION
   ...
   Entity SOC_PPD_NOF_TRAP_EG_TYPES will make sure NO ACTION wil always exist, and wont be deleted
   Entity SOC_PPD_NOF_TRAP_EG_TYPES + 1 will make sure DROP ACTION wil always exist, and wont be deleted
   Entity SOC_PPD_NOF_TRAP_EG_TYPES + 2 will make sure CPU DROP ACTION wil always exist, and wont be deleted
   Entity SOC_PPD_NOF_TRAP_EG_TYPES + 3 + entry-id indicates the Field Processor entry action profile  */
...
#define _DPP_AM_TEMPLATE_TRAP_EGRESS_SIZE   (sizeof(SOC_PPD_TRAP_EG_ACTION_PROFILE_INFO))  


/* Prog traps */
#define _DPP_AM_TEMPLATE_PROG_TRAP_LOW_ID (0)
#define _DPP_AM_TEMPLATE_PROG_TRAP_COUNT (4)
#define _DPP_AM_TEMPLATE_PROG_TRAP_MAX_ENTITIES(unit) (_BCM_PETRA_NOF_PP_PORTS(unit)*4)
...
#define _DPP_AM_TEMPLATE_QUEUE_RATE_CLS_MAX_ENTITIES(unit) SOC_DPP_DEFS_GET(unit, nof_queues)
...
#define _DPP_AM_TEMPLATE_VSQ_RATE_CLS_COUNT (SOC_TMC_ITM_VSQ_QT_RT_CLS_MAX+1)
#define _DPP_AM_TEMPLATE_VSQ_RATE_CLS_MAX_ENTITIES (SOC_TMC_ITM_NOF_VSQS)
#define _DPP_AM_TEMPLATE_VSQ_RATE_CLS_SIZE   (sizeof(bcm_dpp_cosq_vsq_rate_class_info_t))

/* Ingress Queue Discount Class */
#define _DPP_AM_TEMPLATE_QUEUE_DISCNT_CLS_LOW_ID (0)
#define _DPP_AM_TEMPLATE_QUEUE_DISCNT_CLS_COUNT (SOC_TMC_ITM_NOF_CR_DISCNT_CLS_NDXS)
#define _DPP_AM_TEMPLATE_QUEUE_DISCNT_CLS_MAX_ENTITIES(unit) SOC_DPP_DEFS_GET(unit, nof_queues)
#define _DPP_AM_TEMPLATE_QUEUE_DISCNT_CLS_SIZE   (sizeof(int))
...
#define _DPP_AM_TEMPLATE_EGRESS_PORT_DISCNT_CLS_MAX_ENTITIES(unit) (SOC_IS_PETRAB(unit) ? SOC_TMC_NOF_FAP_PORTS_PETRA : SOC_TMC_NOF_FAP_PORTS_ARAD)
#define _DPP_AM_TEMPLATE_EGRESS_PORT_DISCNT_CLS_SIZE   (sizeof(SOC_TMC_PORT_EGR_HDR_CR_DISCOUNT_INFO))

/* PUSH profile */

#define _DPP_AM_TEMPLATE_MPLS_PUSH_PROFILE_LOW_ID(unit) (SOC_IS_PETRAB(unit)? 0:1)
#define _DPP_AM_TEMPLATE_MPLS_PUSH_PROFILE_COUNT(unit) (SOC_IS_PETRAB(unit)? 8:7)
#define _DPP_AM_TEMPLATE_MPLS_PUSH_PROFILE_MAX_ENTITIES (64 * 1024) 
#define _DPP_AM_TEMPLATE_MPLS_PUSH_PROFILE_SIZE   (sizeof(SOC_PPD_EG_ENCAP_PUSH_PROFILE_INFO))

/* LIF Termination profile */
#define _DPP_AM_TEMPLATE_LIF_TERM_PROFILE_LOW_ID       (0)
#define _DPP_AM_TEMPLATE_LIF_TERM_PROFILE_COUNT       (8)
#define _DPP_AM_TEMPLATE_LIF_TERM_PROFILE_MAX_ENTITIES (64 * 1024) 
#define _DPP_AM_TEMPLATE_LIF_TERM_PROFILE_SIZE   (sizeof(SOC_PPD_MPLS_TERM_PROFILE_INFO))
...
#define _DPP_AM_TEMPLATE_PORT_MACT_SA_DROP_PROFILE_MAX_ENTITIES(unit) (_BCM_PETRA_NOF_PP_PORTS(unit) + _BCM_DPP_PORT_TRAP_CODE_SA_DROP_NOF_PROFILES) /* With 4 dummies one per profile*/ 
...
#define _DPP_AM_TEMPLATE_PORT_MACT_DA_UNKNOWN_PROFILE_MAX_ENTITIES(unit) (_BCM_PETRA_NOF_PP_PORTS(unit)+_BCM_DPP_PORT_TRAP_CODE_DA_UNKNOWN_NOF_PROFILES) /* With 4 dummies one per profile*/ 
...
#define _DPP_AM_TEMPLATE_METER_HIGH_PROFILE_LOW_ID(unit) (_DPP_AM_TEMPLATE_METER_PROFILE_COUNT(unit))
#define _DPP_AM_TEMPLATE_METER_HIGH_PROFILE_COUNT(unit) (SOC_DPP_CONFIG(unit)->meter.nof_high_rate_profiles) /* each group */
...
#define _DPP_AM_TEMPLATE_COSQ_SCHED_CLASS_MAX_ENTRIES(unit) (SOC_IS_PETRAB(unit) ? ((SOC_TMC_CL_SE_ID_MAX_PETRA + 1) * 2) : ((SOC_TMC_CL_SE_ID_MAX_ARAD + 1) * 2))
...
#define _DPP_AM_TEMPLATE_PTP_PORT_PROFILE_COUNT        (SOC_PPD_PTP_IN_PP_PORT_PROFILES_NUM)
#define _DPP_AM_TEMPLATE_PTP_PORT_PROFILE_MAX_ENTITIES (_BCM_PETRA_NOF_PP_PORTS(unit))
#define _DPP_AM_TEMPLATE_PTP_PORT_PROFILE_CLS_SIZE     (sizeof(SOC_PPD_PTP_PORT_INFO))
...
#define _DPP_AM_TEMPLATE_L3_RIF_MAC_TERMINATION_COMBINTION_MAX_ENTITIES ((SOC_DPP_CONFIG(unit))->l3.nof_rifs)
...
_bcm_dpp_pp_resource_setup(int unit, int res_id, int core_id, int pool_id, bcm_dpp_am_pool_info_t *p_info) {
    ...
    char *desc_str;
    shr_res_idxres_extras_t idxresExtra;
    shr_res_aidxres_extras_t aidxresExtra;
    shr_res_mdb_extras_t  mdbExtra;
    void *extra = NULL;
    int rv = BCM_E_NONE;
#ifdef BCM_ARAD_SUPPORT
    ...
    switch (res_id) {
        case dpp_am_res_vsi_vlan:
            p_info->res_type = SHR_RES_ALLOCATOR_BITMAP;
            p_info->start = 1;
            p_info->count = 4096;
        ...
        case dpp_am_res_vsi_mstp:
            p_info->res_type = SHR_RES_ALLOCATOR_IDXRES;
            p_info->start = 4096;
            p_info->count = ((SOC_DPP_CONFIG(unit))->l2.nof_vsis)-4096;
            p_info->max_elements_per_allocation = 1;
            desc_str = "VSIs for MSTP";
            idxresExtra.scaling_factor = 1; /* each entry is one element */
            extra = &idxresExtra;
            break;     
        case dpp_am_res_lif_pwe:
            if (SOC_DPP_PP_ENABLE(unit) && SOC_IS_PETRAB(unit)) {
                /* mostly direct allocation */
                p_info->res_type = SHR_RES_ALLOCATOR_BITMAP;
                p_info->start = SOC_DPP_CONFIG(unit)->pp.max_egress_encap_ip_tunnel_range+1;
                p_info->count = _DPP_MAX_NUM_OF_LIFS-(SOC_DPP_CONFIG(unit)->pp.max_egress_encap_ip_tunnel_range+1);
                p_info->max_elements_per_allocation = 2;
                desc_str = "LIFs for PWE allocation";
            } else {
                BCM_EXIT;
            }
            break;
        case dpp_am_res_lif_ip_tnl:
            if (SOC_DPP_PP_ENABLE(unit) && SOC_IS_PETRAB(unit)) {
                p_info->res_type = SHR_RES_ALLOCATOR_BITMAP;
                p_info->start = SOC_DPP_CONFIG(unit)->pp.min_egress_encap_ip_tunnel_range;
                p_info->count = SOC_DPP_CONFIG(unit)->pp.max_egress_encap_ip_tunnel_range;
                p_info->max_elements_per_allocation = 1;
                desc_str =  "LIFs for IP tunnels";
            } else {
                BCM_EXIT;
            }
            break;
        case dpp_am_res_trap_single_instance:
            p_info->res_type = SHR_RES_ALLOCATOR_BITMAP;
            p_info->start = 0;
            p_info->count = SOC_PPD_NOF_TRAP_CODES;
            p_info->max_elements_per_allocation = 1;
        ...
        case dpp_am_res_trap_user_define:
            /* mostly direct allocation */
            p_info->res_type = SHR_RES_ALLOCATOR_BITMAP;
            p_info->start = SOC_PPD_TRAP_CODE_USER_DEFINED_0;
        ...
        case dpp_am_res_lif_dynamic:
            if (SOC_DPP_PP_ENABLE(unit) && SOC_IS_PETRAB(unit)) {
                /* mostly 'next available' allocation[???] */
                p_info->res_type = SHR_RES_ALLOCATOR_IDXRES;
                p_info->start = 1;
                p_info->count = _DPP_MAX_NUM_OF_LIFS;
                desc_str = "LIFs for dynamic use";
                idxresExtra.scaling_factor = 1; /* each entry is one element */
                p_info->max_elements_per_allocation = 1;
                extra = &idxresExtra;
            } else {
        ...
        case dpp_am_res_mc_dynamic:
            if (SOC_DPP_CONFIG(unit)->tm.nof_mc_ids) {
                p_info->res_type = SHR_RES_ALLOCATOR_BITMAP;
                p_info->start = 0;
                p_info->count = SOC_IS_PETRAB(unit) ? SOC_DPP_CONFIG(unit)->tm.nof_mc_ids * 2 : 1;
                p_info->max_elements_per_allocation = 1;
        ...
        case dpp_am_res_fec_global:
            {
                ...
                p_info->max_elements_per_allocation = 2;
                aidxresExtra.blocking_factor = 4; /* don't expect blocks > 16 */
                extra = &aidxresExtra;
            }
            break;
    ...
    case dpp_am_res_rp_id:
        if (SOC_IS_ARAD(unit) && _BCM_DPP_IPMC_NOF_RPS(unit) != 0)  {
            p_info->res_type = SHR_RES_ALLOCATOR_BITMAP;
            p_info->start = 0;
    ...
    break;
        ...
        case dpp_am_res_eep_global:
            if (SOC_DPP_PP_ENABLE(unit) && SOC_IS_PETRAB(unit)) {
                /* mostly 'next available' with possibly 256-elem blocks[???] */
                p_info->res_type = SHR_RES_ALLOCATOR_BITMAP;
                p_info->start = 2;
                p_info->count = SOC_DPP_CONFIG(unit)->pp.min_egress_encap_ip_tunnel_range-1;
                p_info->max_elements_per_allocation = 2;
                desc_str = "EEPs for global use";
            }
            else {
                BCM_EXIT;
            }
        ...
        case dpp_am_res_eep_mpls_tunnel:
            if (SOC_DPP_PP_ENABLE(unit) && SOC_IS_PETRAB(unit)) {
               p_info->res_type = SHR_RES_ALLOCATOR_BITMAP;
                p_info->start = SOC_DPP_CONFIG(unit)->pp.max_egress_encap_ip_tunnel_range+1;
                p_info->count = 0x3000-1;
                p_info->max_elements_per_allocation = 2;
                desc_str = "MPLS/PWE tunnels";
            }
            else {
                BCM_EXIT;
            }
        ...
        case dpp_am_res_eep_local:
            if (SOC_DPP_PP_ENABLE(unit)) {
                /* mostly 'next available' with possibly 256-elem blocks[???] */
                p_info->res_type = SHR_RES_ALLOCATOR_MDB;
                p_info->start = 32768; 
                p_info->count = 32768; 
                desc_str = "EEPs for local use";
                extra = &mdbExtra;
                mdbExtra.bank_size = 1024;
                mdbExtra.free_lists = 10;
                mdbExtra.free_counts[0] = 2; /* powers of two are a nice default */
                mdbExtra.free_counts[1] = 4;
                mdbExtra.free_counts[2] = 8;
                mdbExtra.free_counts[3] = 16;
                mdbExtra.free_counts[4] = 32;
                mdbExtra.free_counts[5] = 64;
                mdbExtra.free_counts[6] = 128;
                mdbExtra.free_counts[7] = 256;
                mdbExtra.free_counts[8] = 512;
                mdbExtra.free_counts[9] = 1024;
                p_info->max_elements_per_allocation = 2;
            }
            else {
                BCM_EXIT;
        ...
        case dpp_am_res_eg_out_ac:
            if (SOC_DPP_PP_ENABLE(unit) && SOC_IS_PETRAB(unit)) {
                ...
                p_info->start = 1;
                p_info->count = 16384;
                desc_str = "Out-ACs for dynamic use";
                extra = &idxresExtra;
                idxresExtra.scaling_factor = 1; /* each entry is one element */
                p_info->max_elements_per_allocation = 1;
            } else {
        ...
        case dpp_am_res_qos_egr_remark_id: 
            p_info->res_type = SHR_RES_ALLOCATOR_BITMAP;
            p_info->start = SOC_IS_ARAD(unit) ? 1 : 0;
            p_info->count = SOC_IS_ARAD(unit) ? SOC_DPP_CONFIG(unit)->qos.nof_egr_remark_id - 1 : SOC_DPP_CONFIG(unit)->qos.nof_egr_remark_id;
            p_info->max_elements_per_allocation = 1;
        ...
        case dpp_am_res_qos_egr_pcp_vlan:
             ...
             */
            p_info->res_type = SHR_RES_ALLOCATOR_BITMAP;
            p_info->start = SOC_IS_ARAD(unit) ? 0 : 1;
            p_info->count = SOC_IS_ARAD(unit) ? (SOC_DPP_CONFIG(unit)->qos.nof_egr_pcp_vlan) : (SOC_DPP_CONFIG(unit)->qos.nof_egr_pcp_vlan - 1);
            p_info->max_elements_per_allocation = 1;
        ...
        case dpp_am_res_meter_a:
            p_info->res_type = SHR_RES_ALLOCATOR_BITMAP;
            p_info->start = 1;
            p_info->count =  SOC_DPP_CONFIG(unit)->meter.nof_meter_a - 1;
        ...
        case dpp_am_res_meter_b:
            p_info->res_type = SHR_RES_ALLOCATOR_BITMAP;
            p_info->start = 1;
            p_info->count =  SOC_DPP_CONFIG(unit)->meter.nof_meter_b - 1;
            p_info->max_elements_per_allocation = 4;
            desc_str = "number of meters in processor B";
        break;
        case dpp_am_res_ether_policer:
            /* mostly 'next available' with blocks up to 8[???] */
            p_info->res_type = SHR_RES_ALLOCATOR_BITMAP;
            p_info->start = 1;
            p_info->count = 7;
            ...
            extra = &aidxresExtra;
            p_info->max_elements_per_allocation = 4;
            aidxresExtra.blocking_factor = 3; /* don't expect blocks > 8 */
            break;
        case dpp_am_res_oam_ma_index:
            if (SOC_IS_ARAD(unit)) {
                p_info->res_type = SHR_RES_ALLOCATOR_BITMAP;
                p_info->start = 0;
                p_info->count = SOC_PPD_OAM_MAX_NUMBER_OF_LOCAL_MEPS(unit);
        ...
        case dpp_am_res_oam_mep_id_long:
            if (SOC_IS_ARAD(unit)) {
                p_info->res_type = SHR_RES_ALLOCATOR_BITMAP;
                p_info->start = 0;
                p_info->count = SOC_PPD_OAM_MAX_NUMBER_OF_LOCAL_MEPS(unit)/2; /* lower half of the maximal number of MEPs */
                desc_str = "OAM MEP Index for ccm MA name format";
                idxresExtra.scaling_factor = 1;
                p_info->max_elements_per_allocation = 1;
                extra = &idxresExtra;
            }
        ...
        case dpp_am_res_oam_rmep_id:
            if (SOC_IS_ARAD(unit)) {
                ...
                p_info->count = SOC_PPD_OAM_MAX_NUMBER_OF_REMOTE_MEPS(unit); /* maximal number of RMEPs is 16k*/
        ...
        case dpp_am_res_oam_trap_code_upmep_ftmh_header:
            if (SOC_IS_ARAD(unit)) {
                p_info->res_type = SHR_RES_ALLOCATOR_BITMAP;
                p_info->start = SOC_PPD_TRAP_CODE_OAM_FTMH_MIRROR_TRAP_CODES_FIRST;
                p_info->count = SOC_PPD_TRAP_CODE_OAM_FTMH_MIRROR_TRAP_CODES_NUM;
...
#ifdef BCM_ARAD_SUPPORT
#if (!BCM_ARAD_OPTIMIZE_MEMORY_ALLOCATION)
    if (DPP_AM_RES_ID_IS_EGRESS_LIF(unit, res_id)) {
        rv = BCM_E_NONE;
        if (SOC_IS_ARAD(unit) && SOC_DPP_PP_ENABLE(unit)){
            p_info->res_type = SHR_RES_ALLOCATOR_BITMAP;
            bank_id = _BCM_DPP_AM_EGRESS_ENCAP_POOL_ID_TO_BANK_ID(res_id);
            p_info->start = _BCM_DPP_AM_EGRESS_ENCAP_BANK_LOW_ID(bank_id);
    ...
    else if (DPP_AM_RES_ID_IS_INGRESS_LIF(unit, res_id)) {
        rv = BCM_E_NONE;
        if (SOC_IS_ARAD(unit) && SOC_DPP_PP_ENABLE(unit)){
            p_info->res_type = SHR_RES_ALLOCATOR_BITMAP;
            bank_id = _BCM_DPP_AM_INGRESS_LIF_POOL_ID_TO_BANK_ID(res_id);
            p_info->start = _BCM_DPP_AM_INGRESS_LIF_BANK_LOW_ID(bank_id);
    ...
    if (_BCM_DPP_AM_RES_ID_IS_EGRESS_GLOBAL_LIF(unit, res_id)) {
        rv = BCM_E_NONE;
        if (SOC_IS_JERICHO(unit) && SOC_DPP_PP_ENABLE(unit)){
            p_info->res_type = SHR_RES_ALLOCATOR_BITMAP;
            bank_id = _BCM_DPP_AM_GLOBAL_LIF_EGRESS_LIF_RES_ID_TO_BANK_ID(unit, res_id);
            p_info->start = _BCM_DPP_AM_GLOBAL_LIF_EGRESS_BANK_LOW_ID(bank_id);
            p_info->count = _BCM_DPP_AM_GLOBAL_LIF_NOF_ENTRIES_PER_BANK;
            extra = NULL;
            desc_str = "Egress global lif bank allocation";
        }
        else {
            BCM_EXIT;
        }
    } else if (_BCM_DPP_AM_RES_ID_IS_INGRESS_GLOBAL_LIF(unit, res_id)) {
        rv = BCM_E_NONE;
        if (SOC_IS_JERICHO(unit) && SOC_DPP_PP_ENABLE(unit)){
            p_info->res_type = SHR_RES_ALLOCATOR_BITMAP;
            bank_id = _BCM_DPP_AM_GLOBAL_LIF_INGRESS_LIF_RES_ID_TO_BANK_ID(unit, res_id);
            p_info->start = _BCM_DPP_AM_GLOBAL_LIF_INGRESS_BANK_LOW_ID(bank_id);
            p_info->count = _BCM_DPP_AM_GLOBAL_LIF_NOF_ENTRIES_PER_BANK;
            extra = NULL;
            desc_str = "Ingress global LIF bank allocation";
        }
        else {
            BCM_EXIT;
        }
    ...
    bcm_vlan_t *vsi)
{
    ...
    rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_vsi_vlan,flags,1,&elem);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    *vsi = elem;
    ...
    if (!SOC_WARM_BOOT(unit)) {
       /* Vlan SW DB has WB implementation of its own. If we are recovering
          data, Vlan SW DB will be recovered in its own context.
          No need to set it from here */
       rv = _bcm_dpp_vlan_info_vlan_exist_set(unit, *vsi, TRUE);
       BCMDNX_IF_ERR_EXIT(rv);
    }
#else
       rv = _bcm_dpp_vlan_info_vlan_exist_set(unit, *vsi, TRUE);
    ...
    bcm_vlan_t vsi)
{
    ...
    rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_vsi_vlan,1,elem);
    BCMDNX_IF_ERR_EXIT(rv);

    rv = _bcm_dpp_vlan_info_vlan_exist_set(unit, vsi, FALSE);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    SOC_PPD_LIF_ID *lif)
{
    ...
    *vsi = vid;
    ...
    vlan = *vsi;
    rv = bcm_dpp_am_l2_vsi_vlan_alloc(unit, SHR_RES_ALLOC_WITH_ID, &vlan);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    SOC_PPD_LIF_ID lif)
{
    ...
    rv = bcm_dpp_am_l2_vsi_vlan_dealloc(unit, SHR_RES_ALLOC_WITH_ID, vsi);
    BCMDNX_IF_ERR_EXIT(rv);
    BCM_EXIT;
exit:
    BCMDNX_FUNC_RETURN;
        ...
        bcm_vlan_t *vsi)
{
    ...
    if ((flags & SHR_RES_ALLOC_WITH_ID) &&
        *vsi <= BCM_VLAN_MAX) {
        rv = bcm_dpp_am_l2_vsi_vlan_alloc(unit, SHR_RES_ALLOC_WITH_ID, vsi);
        BCMDNX_IF_ERR_EXIT(rv);
    }
    else {
        rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_vsi_mstp,flags,1,&elem);
        BCMDNX_IF_ERR_EXIT(rv);
        ...
        bcm_vlan_t vsi)
{
    ...
    if (vsi <= BCM_VLAN_MAX) {
        ...
        rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_vsi_mstp,1,elem);
        BCMDNX_IF_ERR_EXIT(rv);
    }
    ...
    bcm_vlan_t vsi)
{
    ...
    if (vsi <= BCM_VLAN_MAX) {
        BCM_RETURN_VAL_EXIT(dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_vsi_vlan,1,elem));
    }
    else {
        BCM_RETURN_VAL_EXIT(dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_vsi_mstp,1,elem));
    }
exit:
    BCMDNX_FUNC_RETURN;
}
    ...
    bcm_failover_t *failover_id)
{
    ...
    if (nof_alloc_ids == 2) {
        flags |= SHR_RES_ALLOC_ALIGN_ZERO;
        rv = dpp_am_res_alloc_align(unit, BCM_DPP_AM_DEFAULT_CORE_ID, alloc_buffer_pool_id, flags, 2, 0, nof_alloc_ids, &elem);
    } else {
        /* Call the general alloc manager function to perform the allocation */
        rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID, alloc_buffer_pool_id, flags, nof_alloc_ids, &elem);
    }
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    bcm_failover_t failover_id)
{
    ...
    rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID, alloc_buffer_pool_id, nof_dealloc_ids, elem);
    BCMDNX_IF_ERR_EXIT(rv);
exit:
    BCMDNX_FUNC_RETURN;
}
    ...
    bcm_failover_t failover_id)
{
    ...
    BCM_RETURN_VAL_EXIT(dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID, alloc_buffer_pool_id, 1, elem));
exit:
    BCMDNX_FUNC_RETURN;
}
    ...
    bcm_ecmp_t *ecmp_id)
{
    ...
    rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_ecmp_id,flags,1,&elem);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    bcm_ecmp_t ecmp_id)
{
    ...
    rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_ecmp_id,1,elem);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    bcm_ecmp_t ecmp_id)
{
    ...
    BCM_RETURN_VAL_EXIT(dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_ecmp_id,1,elem));
exit:
    BCMDNX_FUNC_RETURN;
}
    ...
    int *rp_id)
{
    ...
    rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_rp_id,flags,1,&elem);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    int rp_id)
{
    ...
    rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_rp_id,1,elem);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    int rp_id)
{
    ...
    BCM_RETURN_VAL_EXIT(dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_rp_id,1,elem));
exit:
    BCMDNX_FUNC_RETURN;
}
        ...
        int *eep)
{
    ...
    if(lif) {
        ...
        if (SOC_IS_ARAD(unit)) {
            ...
            rv = _bcm_dpp_am_sync_lif_alloc_align(unit,&allocation_info, flags& (~BCM_DPP_AM_FLAG_ALLOC_GLOBAL),2,0,1,&elem);
        } else 
#endif /* BCM_ARAD_SUPPORT */
        {
            rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_lif_pwe,flags,count,&elem);
            BCMDNX_IF_ERR_EXIT(rv);

            *lif = elem;

            /* marked as alloc in total range */
            flags |= SHR_RES_ALLOC_WITH_ID;

            elem = *lif;
            rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_lif_dynamic,flags,count,&elem);
    ...
    if(eep) {
        /* Not supported by ARAD */
#ifdef BCM_ARAD_SUPPORT
        if (SOC_IS_ARAD(unit)) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_INTERNAL, (_BSL_BCM_MSG("Not supported allocation in ARAD")));
        }
#endif /* BCM_ARAD_SUPPORT */

        elem = *eep;
        rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_eep_local,flags,1,&elem);
        BCMDNX_IF_ERR_EXIT(rv);
        *eep = elem;
...
#endif /* BCM_ARAD_SUPPORT */
    {
      rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_lif_pwe,1,elem);
      BCMDNX_IF_ERR_EXIT(rv);

      rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_lif_dynamic,1,elem);
      BCMDNX_IF_ERR_EXIT(rv);
    }
...
#endif /* BCM_ARAD_SUPPORT */
    {
        rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_eep_local,1,elem);
        BCMDNX_IF_ERR_EXIT(rv);
    }
        ...
        int *lif)
{
    ...
    if (SOC_IS_ARAD(unit)) {
        ...
        /* types parameter is only needed for ARAD information */
        if (types == _BCM_DPP_AM_L3_LIF_MPLS_TERM) { /* MPLS termination */
            allocation_info.application_type = bcm_dpp_am_ingress_lif_app_mpls_term;
        } else if (types == _BCM_DPP_AM_L3_LIF_IP_TERM) { /* IP termination*/
            allocation_info.application_type = bcm_dpp_am_ingress_lif_app_ip_term;
        } else if (types == _BCM_DPP_AM_L3_LIF_MPLS_TERM_INDEXED_1) { /* MPLS termination indexed 1 */
            allocation_info.application_type = bcm_dpp_am_ingress_lif_app_mpls_term_indexed_1;        
        } else if (types == _BCM_DPP_AM_L3_LIF_MPLS_TERM_INDEXED_2) { /* MPLS termination indexed 2 */
            allocation_info.application_type = bcm_dpp_am_ingress_lif_app_mpls_term_indexed_2;        
        } else if (types == _BCM_DPP_AM_L3_LIF_MPLS_TERM_INDEXED_3) { /* MPLS termination indexed 3 */
            allocation_info.application_type = bcm_dpp_am_ingress_lif_app_mpls_term_indexed_3;        
        } else if (types == _BCM_DPP_AM_L3_LIF_MPLS_TERM_FRR) { /* MPLS FRR termination */
            allocation_info.application_type = bcm_dpp_am_ingress_lif_app_mpls_frr_term;        
        } else if (types == _BCM_DPP_AM_L3_LIF_MPLS_TERM_INDEXED_1_2) { /* MPLS termination indexed 1,2 */
            allocation_info.application_type = bcm_dpp_am_ingress_lif_app_mpls_term_indexed_1_2;        
        } else if (types == _BCM_DPP_AM_L3_LIF_MPLS_TERM_INDEXED_1_3) { /* MPLS termination indexed 1,3 */
            allocation_info.application_type = bcm_dpp_am_ingress_lif_app_mpls_term_indexed_1_3;        
        } else if (types == _BCM_DPP_AM_L3_LIF_MPLS_TERM_EXPLICIT_NULL) { /* MPLS termination Explicit NULL */
            allocation_info.application_type = bcm_dpp_am_ingress_lif_app_mpls_term_explicit_null;        
        } else {
...
#endif /* BCM_ARAD_SUPPORT */
    {
        rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_lif_dynamic,flags,1,lif);
        BCMDNX_IF_ERR_EXIT(rv);
    }
...
#endif /* BCM_ARAD_SUPPORT */
    {
        BCM_RETURN_VAL_EXIT(dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_lif_dynamic, 1,lif));
    }
        ...
        int lif)
{
    ...
    if (SOC_IS_ARAD(unit)) {
        ...
        allocation_info.pool_id = dpp_am_res_lif_dynamic;
        /* rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_eep_global,SOC_IS_ARAD(unit) ? 2 : 1,eep); */
        rv = _bcm_dpp_am_ingress_lif_dealloc(unit,&allocation_info,1,lif);
        BCMDNX_IF_ERR_EXIT(rv);
    } else 
#endif /* BCM_ARAD_SUPPORT */
    {
        rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_lif_dynamic,1,lif);
        BCMDNX_IF_ERR_EXIT(rv);
        ...
        int *eep)
{
    ...
    if (SOC_IS_ARAD(unit)) {
        ...
        /*rv = shr_res_alloc_align(unit,dpp_res_pool_eep_global,flags,2,0,2,eep);*/
        rv = _bcm_dpp_am_egress_encap_alloc_align(unit,&allocation_info,flags,2,0,2,eep);
    }
    else 
#endif /* BCM_ARAD_SUPPORT */
    {
        rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_eep_global,flags,1,eep);
        ...
        int eep)
{
    ...
    if (SOC_IS_ARAD(unit)) {
        ...
        allocation_info.pool_id = dpp_am_res_eep_global;
        /* BCM_RETURN_VAL_EXIT(dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_eep_global, SOC_IS_ARAD(unit) ? 2 : 1,eep));*/
        BCM_RETURN_VAL_EXIT(_bcm_dpp_am_egress_encap_is_allocated(unit,&allocation_info,2,eep));
    } else 
#endif /* BCM_ARAD_SUPPORT */
    {
        BCM_RETURN_VAL_EXIT(dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_eep_global, 1,eep));
    }
...
#endif /* BCM_ARAD_SUPPORT */
    rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_eep_global,1,eep);
    BCMDNX_IF_ERR_EXIT(rv);
...
#endif /* BCM_ARAD_SUPPORT */
        {
            rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_lif_ip_tnl,flags,1,&elem);
            BCMDNX_IF_ERR_EXIT(rv);
            ...
            *lif = elem;
            ...
            elem = *lif;
            rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_lif_dynamic,flags,1,&elem);
            BCMDNX_IF_ERR_EXIT(rv);
        }
        *lif = elem;
    }
...
#endif /* BCM_ARAD_SUPPORT */
    {
        rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_lif_ip_tnl,1,elem);
        BCMDNX_IF_ERR_EXIT(rv);
        ...
        int *eep)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
         ...
         /* ipv6 alloc is not supported for PB */
    if(SOC_IS_PETRAB(unit)) {
        if(flags & (BCM_DPP_AM_FLAG_ALLOC_IPV6)) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("ipv6 tunnel not supported for PB, flags = 0x%08x"), flags));
        }
    }
    ...
    if (SOC_IS_ARAD(unit)) {
        ...
        /*rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_eep_ip_tnl,flags,1,eep);*/

        if (types == _BCM_DPP_AM_IP_TUNNEL_EEP_TYPE_ROO) {
            allocation_info.application_type = bcm_dpp_am_egress_encap_app_ip_tunnel_roo;
        }

        /*rv = shr_res_alloc(unit,dpp_res_pool_eep_ip_tnl,flags,1,eep);*/
        rv = _bcm_dpp_am_egress_encap_alloc_align(unit,&allocation_info,flags,2,0,2,eep);
        BCMDNX_IF_ERR_EXIT(rv);
    } else 
#endif /* BCM_ARAD_SUPPORT */
        ...
        int eep)
{
    ...
    if (SOC_IS_ARAD(unit)) {
        ...
        allocation_info.pool_id = dpp_am_res_eep_ip_tnl;
        /* BCM_RETURN_VAL_EXIT(dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_eep_ip_tnl,1,eep));*/
        BCM_RETURN_VAL_EXIT(_bcm_dpp_am_egress_encap_is_allocated(unit,&allocation_info,2,eep));
    } else 
#endif /* BCM_ARAD_SUPPORT */
    {
        ...
        int eep)
{
    ...
    if (SOC_IS_ARAD(unit)) {
        ...
        allocation_info.pool_id = dpp_am_res_eep_ip_tnl;
        /* rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_eep_ip_tnl,1,eep); */
        rv = _bcm_dpp_am_egress_encap_dealloc(unit,&allocation_info,2,eep);
    } else 
#endif /* BCM_ARAD_SUPPORT */
    {
        ...
        int *eep)
{
    ...
    if (SOC_IS_ARAD(unit)) {
        ...
        if (type == _BCM_DPP_AM_MPLS_EEP_TYPE_PWE) {
            ...
            rv = _bcm_dpp_am_sync_lif_alloc_align(unit,&sync_allocation_info, flags&(~BCM_DPP_AM_FLAG_ALLOC_GLOBAL),2,0,2,eep); 
...
#endif /* BCM_ARAD_SUPPORT */
    {
        rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_eep_mpls_tunnel,flags,count,eep);
        BCMDNX_IF_ERR_EXIT(rv);
    }
...
#endif /* BCM_ARAD_SUPPORT */
    {
        BCM_RETURN_VAL_EXIT(dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_eep_mpls_tunnel,1,eep));
    }
exit:
    BCMDNX_FUNC_RETURN;
}
...
#endif /* BCM_ARAD_SUPPORT */
        rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_eep_mpls_tunnel,1,eep);
        BCMDNX_IF_ERR_EXIT(rv);
        ...
        int *idx)
{
    ...
    int elem = *idx;
    BCMDNX_INIT_FUNC_DEFS;
    rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_glbl_src_ip,flags,1,&elem);
    BCMDNX_IF_ERR_EXIT(rv);
        ...
        int idx)
{
    int elem = idx;
    BCMDNX_INIT_FUNC_DEFS;
    BCM_RETURN_VAL_EXIT(dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_glbl_src_ip,1,elem));
exit:
    BCMDNX_FUNC_RETURN;
}
        ...
        int idx)
{
    ...
    int elem = idx;
    BCMDNX_INIT_FUNC_DEFS;
    rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_glbl_src_ip,1,elem);
    BCMDNX_IF_ERR_EXIT(rv);
        ...
        int *idx)
{
    ...
    int elem = *idx;
    BCMDNX_INIT_FUNC_DEFS;
    rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_glbl_ttl,flags,1,&elem);
    BCMDNX_IF_ERR_EXIT(rv);
        ...
        int idx)
{
    int elem = idx;
    BCMDNX_INIT_FUNC_DEFS;
    BCM_RETURN_VAL_EXIT(dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_glbl_ttl,1,elem));
exit:
    BCMDNX_FUNC_RETURN;
}
        ...
        int idx)
{
    ...
    int elem = idx;
    BCMDNX_INIT_FUNC_DEFS;
    rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_glbl_ttl,1,elem);
    BCMDNX_IF_ERR_EXIT(rv);
exit:
    BCMDNX_FUNC_RETURN;
}
        ...
        int *idx)
{
    ...
    int elem = *idx;
    BCMDNX_INIT_FUNC_DEFS;
    rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_glbl_tos,flags,1,&elem);
    BCMDNX_IF_ERR_EXIT(rv);
        ...
        int idx)
{
    int elem = idx;
    BCMDNX_INIT_FUNC_DEFS;
    BCM_RETURN_VAL_EXIT(dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_glbl_tos,1,elem));
exit:
    BCMDNX_FUNC_RETURN;
}
        ...
        int idx)
{
    ...
    int elem = idx;
    BCMDNX_INIT_FUNC_DEFS;
    rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_glbl_tos,1,elem);
    BCMDNX_IF_ERR_EXIT(rv);
...
_bcm_dpp_am_eg_trap_index_get(
        int unit,
        SOC_PPD_TRAP_EG_TYPE trap_id, /* from the enum */
        ...
        int *elem)
{
    ...
    SOC_PPD_TRAP_FRWRD_ACTION_PROFILE_INFO_clear(&(data.trap));
    ...
    *elem = trap_id;
    ...
    if ( (SOC_PPD_TRAP_CODE_USER_DEFINED_0 <= trap_id) && (trap_id <= _BCM_LAST_USER_DEFINE) ) { 
        LOG_DEBUG(BSL_LS_BCM_COMMON,
                  (BSL_META_U(unit,
                              "Unit:%d, SOC_PPD_TRAP_CODE_USER_DEFINED alloc , trap id %d\n"), unit,trap_id));
        ...
        data.ud_unique = *elem;
        template = *elem-SOC_PPD_TRAP_CODE_USER_DEFINED_0; 
        rv = dpp_am_template_allocate(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_user_defined_traps, SHR_TEMPLATE_MANAGE_SET_WITH_ID, &data, &is_allocated,&template);
        BCMDNX_IF_ERR_EXIT(rv);
        ...
        LOG_DEBUG(BSL_LS_BCM_COMMON,
                  (BSL_META_U(unit,
                              ...
                              unit, trap_id, trap_index));
        *elem = trap_index;
        rv = _bcm_dpp_am_trap_egress_alloc(unit,SHR_RES_ALLOC_WITH_ID,elem);
        BCMDNX_IF_ERR_EXIT(rv);
        ...
        *elem = trap_id;
        rv = _bcm_dpp_am_trap_single_instance_alloc(unit, SHR_RES_ALLOC_WITH_ID, elem);
                               ...
                               int *elem)
{
    ...
    rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_trap_virtual, flags, 1, elem);
                              ...
                              int *elem)
{
    ...
    rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_trap_egress, flags, 1, elem);
                                   ...
                                   int *elem)
{
    ...
    rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_trap_user_define, flags, 1, elem);
                               ...
                               int *elem)
{
    ...
    rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_trap_single_instance,flags,1,elem);
        ...
        int elem)
{
    ...
    if ( (SOC_PPD_TRAP_CODE_USER_DEFINED_0 <= trap_id) && (trap_id <= _BCM_LAST_USER_DEFINE) ) { 
        LOG_DEBUG(BSL_LS_BCM_COMMON,
                  (BSL_META_U(unit,
                              "Unit:%d, SOC_PPD_TRAP_CODE_USER_DEFINED dealloc, trap id is = %d\n"), unit,trap_id));
        rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_trap_user_define,SHR_RES_ALLOC_WITH_ID,trap_id);

        BCMDNX_IF_ERR_EXIT(rv);

        /*Sync rx.c traps allocations with port.c traps allocations (templates) for USER_DEFINED traps*/
        template = trap_id-SOC_PPD_TRAP_CODE_USER_DEFINED_0; /*template counting is from 0-59*/
        rv = dpp_am_template_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_user_defined_traps, template, &is_last);
        BCMDNX_IF_ERR_EXIT(rv);
       ...
       /*Clears virtual bit before allocation*/
        ...
        rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_trap_virtual,SHR_RES_ALLOC_WITH_ID,trap_id);
        BCMDNX_IF_ERR_EXIT(rv);
        ...
        LOG_DEBUG(BSL_LS_BCM_COMMON,
                  (BSL_META_U(unit,
                              ...
                              unit, trap_id, trap_index));
        rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_trap_egress,SHR_RES_ALLOC_WITH_ID,trap_index);    
        BCMDNX_IF_ERR_EXIT(rv);
        ...
        rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_trap_single_instance,SHR_RES_ALLOC_WITH_ID,trap_id);    
        BCMDNX_IF_ERR_EXIT(rv);
        ...
        int trap_id)
{
    ...
    /* If in the range of PPD user defined*/
    if ( (SOC_PPD_TRAP_CODE_USER_DEFINED_0 <= trap_id) && (trap_id <= _BCM_LAST_USER_DEFINE) ) { 
        rv = dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_trap_user_define,SHR_RES_ALLOC_WITH_ID,trap_id);

    } else if (_BCM_TRAP_ID_IS_VIRTUAL(trap_id)) { /* If virtual bit is marked */
       /*Clears virtual bit before allocation*/
        trap_id = _BCM_TRAP_ID_TO_VIRTUAL(trap_id);
        rv = dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_trap_virtual,SHR_RES_ALLOC_WITH_ID,trap_id);
        ...
        rv = _bcm_dpp_am_eg_trap_index_get(unit, 
                                           ...
                                           &trap_index); /* 0...NOF_TRAPS */
        BCMDNX_IF_ERR_EXIT(rv);
        rv = dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_trap_egress,SHR_RES_ALLOC_WITH_ID,trap_index);    

    } else { /* Otherwise, use single instance pool (ingress) */
        rv = dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_trap_single_instance,SHR_RES_ALLOC_WITH_ID,trap_id);  
    }
        ...
        int *elem)
{
    ...
    rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_snoop_cmd,flags,1,elem);
    BCMDNX_IF_ERR_EXIT(rv);
        ...
        int *elem)
{
    ...
    rv = dpp_am_template_allocate(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_snoop_cmd, SHR_TEMPLATE_MANAGE_SET_WITH_ID, &data, &is_allocated,&template);
    BCMDNX_IF_ERR_EXIT(rv);
        ...
        int elem)
{
    ...
    rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_snoop_cmd,SHR_RES_ALLOC_WITH_ID,snoop_cmnd);
    BCMDNX_IF_ERR_EXIT(rv);

    /*Sync rx.c traps allocations with port.c traps allocations (templates) for USER_DEFINED traps*/
    template = snoop_cmnd; 
    rv = dpp_am_template_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_snoop_cmd, template, &is_last);
    BCMDNX_IF_ERR_EXIT(rv);
        ...
        int snoop_cmnd)
{
    ...
    BCM_RETURN_VAL_EXIT(dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_snoop_cmd,SHR_RES_ALLOC_WITH_ID,snoop_cmnd));
exit:
    BCMDNX_FUNC_RETURN;
}

int
bcm_dpp_am_eep_is_alloced(
        int unit,
        uint32 eep)
{
    int elem = eep;
    BCMDNX_INIT_FUNC_DEFS;
    BCMDNX_IF_ERR_EXIT(dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_eep_local,1,elem));
exit:
    BCMDNX_FUNC_RETURN;
}
...
#ifdef BCM_88660_A0
        if (types == _BCM_DPP_AM_L2_VPN_AC_TYPE_BIG_OUT_AC) {
            /* If types is BIG_OUT_AC, allocates 2 entries for InLIF and 2 entries for outLIF. */
            rv = _bcm_dpp_am_sync_lif_alloc_align(unit,&allocation_info, flags& (~BCM_DPP_AM_FLAG_ALLOC_GLOBAL),2,0,2,&elem);
            BCMDNX_IF_ERR_EXIT(rv);
        } else
#endif
        {
            if (nof_entries == 1) {
                rv = _bcm_dpp_am_sync_lif_alloc(unit,&allocation_info, flags& (~BCM_DPP_AM_FLAG_ALLOC_GLOBAL),1,&elem);
...
#endif /* BCM_ARAD_SUPPORT */
        {
            rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_lif_dynamic,flags& (~BCM_DPP_AM_FLAG_ALLOC_GLOBAL),1,&elem);
        ...
        SOC_PPD_LIF_ID lif)
{
    ...
    if (SOC_IS_ARAD(unit)) {
        ...
        allocation_info.sync_flags = _BCM_DPP_AM_SYNC_LIF_INGRESS | _BCM_DPP_AM_SYNC_LIF_EGRESS;
        if (types == _BCM_DPP_AM_L2_VPN_AC_TYPE_PON_3_TAGS_DATA) {
            count = 2;
        } else {
            count = 1;
        } 
...
#ifdef BCM_88660_A0
        /*
         * If types is BIG_OUT_AC requires 2 entries in EEDB and 1 entry in InLIF.
         * Call twice _bcm_dpp_am_sync_lif_dealloc,
         * once with count = 1 but sync_flags is Ingress and once with count = 2 
         * and sync_flags is Egress.
         */
        if (types == _BCM_DPP_AM_L2_VPN_AC_TYPE_BIG_OUT_AC) {
            allocation_info.sync_flags = _BCM_DPP_AM_SYNC_LIF_INGRESS;
            count = 1;
...
#endif /* BCM_ARAD_SUPPORT */
    {
      BCM_RETURN_VAL_EXIT(dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_lif_dynamic,1,elem));
    }
        ...
        SOC_PPD_AC_ID *out_ac)
{
    ...
    if (SOC_IS_ARAD(unit)) {
        ...
        if (nof_entries == 1) {
            /*rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_eg_out_ac,flags,1,&elem);*/
        ...
        SOC_PPD_AC_ID out_ac)
{
    ...
    if (SOC_IS_ARAD(unit)) {
        ...
        allocation_info.pool_id = dpp_am_res_eg_out_ac;
        /* rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_eg_out_ac,1,elem); */
        ...
        SOC_PPD_AC_ID out_ac)
{
    ...
    if (SOC_IS_ARAD(unit)) {
        ...
        allocation_info.pool_id = dpp_am_res_eg_out_ac;
        /*  BCM_RETURN_VAL_EXIT(dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_eg_out_ac,1,elem));*/
        ...
        int *qos_id)
{
    ...
    int elem = *qos_id;
    BCMDNX_INIT_FUNC_DEFS;
    rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_qos_ing_elsp,flags,1,&elem);
    BCMDNX_IF_ERR_EXIT(rv);
        ...
        int qos_id)
{
    ...
    int elem = qos_id;
    BCMDNX_INIT_FUNC_DEFS;
    rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_qos_ing_elsp,1,elem);
    BCMDNX_IF_ERR_EXIT(rv);
        ...
        int qos_id)
{
    int elem = qos_id;
    BCMDNX_INIT_FUNC_DEFS;
    BCM_RETURN_VAL_EXIT(dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_qos_ing_elsp,1,elem));
exit:
    BCMDNX_FUNC_RETURN;
}
        ...
        int *qos_id)
{
    ...
    int elem = *qos_id;
    BCMDNX_INIT_FUNC_DEFS;
    rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_qos_ing_lif_cos,flags,1,&elem);
    BCMDNX_IF_ERR_EXIT(rv);
        ...
        int qos_id)
{
    ...
    int elem = qos_id;
    BCMDNX_INIT_FUNC_DEFS;
    rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_qos_ing_lif_cos,1,elem);
    BCMDNX_IF_ERR_EXIT(rv);
        ...
        int qos_id)
{
    int elem = qos_id;
    BCMDNX_INIT_FUNC_DEFS;
    BCM_RETURN_VAL_EXIT(dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_qos_ing_lif_cos,1,elem));
exit:
    BCMDNX_FUNC_RETURN;
}
        ...
        int *qos_id)
{
    ...
    int elem = *qos_id;
    BCMDNX_INIT_FUNC_DEFS;
    rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_qos_ing_pcp_vlan,flags,1,&elem);
    BCMDNX_IF_ERR_EXIT(rv);
        ...
        int qos_id)
{
    ...
    int elem = qos_id;
    BCMDNX_INIT_FUNC_DEFS;
    rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_qos_ing_pcp_vlan,1,elem);
    BCMDNX_IF_ERR_EXIT(rv);
        ...
        int qos_id)
{
    int elem = qos_id;
    BCMDNX_INIT_FUNC_DEFS;
    BCM_RETURN_VAL_EXIT(dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_qos_ing_pcp_vlan,1,elem));
exit:
    BCMDNX_FUNC_RETURN;
}
        ...
        int *qos_id)
{
    ...
    int elem = *qos_id;
    BCMDNX_INIT_FUNC_DEFS;
    rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_qos_egr_remark_id,flags,1,&elem);
    BCMDNX_IF_ERR_EXIT(rv);
        ...
        int *qos_id)
{
    ...
    int elem = *qos_id;
    BCMDNX_INIT_FUNC_DEFS;
    rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_qos_egr_mpls_php_id,flags,1,&elem);
    BCMDNX_IF_ERR_EXIT(rv);
        ...
        int qos_id)
{
    ...
    int elem = qos_id;
    BCMDNX_INIT_FUNC_DEFS;
    rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_qos_egr_remark_id,1,elem);
    BCMDNX_IF_ERR_EXIT(rv);
        ...
        int qos_id)
{
    ...
    int elem = qos_id;
    BCMDNX_INIT_FUNC_DEFS;
    rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_qos_egr_mpls_php_id,1,elem);
    BCMDNX_IF_ERR_EXIT(rv);
        ...
        int qos_id)
{
    int elem = qos_id;
    BCMDNX_INIT_FUNC_DEFS;
    BCM_RETURN_VAL_EXIT(dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_qos_egr_remark_id,1,elem));
exit:
    BCMDNX_FUNC_RETURN;
}
        ...
        int qos_id)
{
    int elem = qos_id;
    BCMDNX_INIT_FUNC_DEFS;
    BCM_RETURN_VAL_EXIT(dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_qos_egr_mpls_php_id,1,elem));
exit:
    BCMDNX_FUNC_RETURN;
}
        ...
        int *qos_id)
{
    ...
    int elem = *qos_id;
    BCMDNX_INIT_FUNC_DEFS;
    rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_qos_egr_pcp_vlan,flags,1,&elem);
    BCMDNX_IF_ERR_EXIT(rv);
        ...
        int qos_id)
{
    ...
    int elem = qos_id;
    BCMDNX_INIT_FUNC_DEFS;
    rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_qos_egr_pcp_vlan,1,elem);
    BCMDNX_IF_ERR_EXIT(rv);
        ...
        int qos_id)
{
    int elem = qos_id;
    BCMDNX_INIT_FUNC_DEFS;
    BCM_RETURN_VAL_EXIT(dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_qos_egr_pcp_vlan,1,elem));
exit:
    BCMDNX_FUNC_RETURN;
}
        ...
        int *qos_id)
{
    ...
    int elem = *qos_id;
    BCMDNX_INIT_FUNC_DEFS;
    rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_qos_ing_cos_opcode,flags,1,&elem);
    BCMDNX_IF_ERR_EXIT(rv);
        ...
        int qos_id)
{
    ...
    int elem = qos_id;
    BCMDNX_INIT_FUNC_DEFS;
    rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_qos_ing_cos_opcode,1,elem);
    BCMDNX_IF_ERR_EXIT(rv);
        ...
        int qos_id)
{
    int elem = qos_id;
    BCMDNX_INIT_FUNC_DEFS;
    BCM_RETURN_VAL_EXIT(dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_qos_ing_cos_opcode,1,elem));
exit:
    BCMDNX_FUNC_RETURN;
}
        ...
        int *qos_id)
{
    ...
    int elem = *qos_id;
    BCMDNX_INIT_FUNC_DEFS;
    rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_qos_egr_l2_i_tag,flags,1,&elem);
    BCMDNX_IF_ERR_EXIT(rv);
        ...
        int qos_id)
{
    ...
    int elem = qos_id;
    BCMDNX_INIT_FUNC_DEFS;
    rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_qos_egr_l2_i_tag,1,elem);
    BCMDNX_IF_ERR_EXIT(rv);
        ...
        int qos_id)
{
    ...
    int elem = qos_id;
    BCMDNX_INIT_FUNC_DEFS;
    BCM_RETURN_VAL_EXIT(dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_qos_egr_l2_i_tag,1,elem));
exit:
    BCMDNX_FUNC_RETURN;
}
        ...
        int *qos_id)
{
    ...
    int elem = *qos_id;
    BCMDNX_INIT_FUNC_DEFS;
    rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_qos_egr_dscp_exp_marking,flags,1,&elem);
    BCMDNX_IF_ERR_EXIT(rv);
        ...
        int qos_id)
{
    ...
    int elem = qos_id;
    BCMDNX_INIT_FUNC_DEFS;
    rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_qos_egr_dscp_exp_marking,1,elem);
    BCMDNX_IF_ERR_EXIT(rv);
        ...
        int qos_id)
{
    ...
    int elem = qos_id;
    BCMDNX_INIT_FUNC_DEFS;
    BCM_RETURN_VAL_EXIT(dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_qos_egr_dscp_exp_marking,1,elem));
exit:
    BCMDNX_FUNC_RETURN;
}
        ...
        int *meter_id) /* if nof_metrs != 1: this is the meter_id of the first one allocated. the next ones will be meter_id+1...meter_id+nof_meters-1 */
{
    ...
    if (meter_group < BCM_DPP_AM_METER_GROUP_ANY) {
        *act_meter_group = meter_group;
        ...
        if (nof_meters == 1) {
            rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_res_meter_a + meter_group, flags, nof_meters, &elem);
            BCMDNX_IF_ERR_EXIT(rv);
        }
        else if (nof_meters > 1) {
            rv = dpp_am_res_alloc_align(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_res_meter_a + meter_group, flags | SHR_RES_BITMAP_ALLOC_ALIGN_ZERO, 
        ...
        for (*act_meter_group = 0; *act_meter_group < BCM_DPP_AM_METER_GROUP_ANY; ++(*act_meter_group)) {
            /* try from any group */
            if (nof_meters == 1) {
                rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_res_meter_a + (*act_meter_group), flags, nof_meters, &elem);
            }
            else if (nof_meters > 1) {
                rv = dpp_am_res_alloc_align(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_res_meter_a + (*act_meter_group), flags | SHR_RES_BITMAP_ALLOC_ALIGN_ZERO, 
                                         nof_meters, 0, nof_meters, &elem);
            }
            else {
                BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("nof_meters to allocate must be at least 1")));
        ...
        int meter_id) /* if nof_metrs != 1: this is the meter_id of the first one to deallocate. the next ones will be meter_id+1...meter_id+nof_meters-1 */
{
    ...
    rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_meter_a + meter_group,nof_meters,elem);
    BCMDNX_IF_ERR_EXIT(rv);
        ...
        int meter_id) /* if nof_metrs != 1: this is the meter_id of the first one. the next ones will be meter_id+1...meter_id+nof_meters-1 */
{
    ...
    BCM_RETURN_VAL_EXIT(dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_meter_a + meter_group,nof_meters,elem));
exit:
    BCMDNX_FUNC_RETURN;
}
        ...
        uint8 is_egress)
{
    ...
    rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_mc_dynamic,flags,1,&elem);
        ...
        uint8 is_egress)
{
    ...
    rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_res_mc_dynamic, 1, elem);
    BCMDNX_IF_ERR_EXIT(rv);
        ...
        uint8 is_egress)
{
    ...
    return dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_res_mc_dynamic, 1, elem);
}
...
bcm_dpp_am_oam_ma_id_alloc(
        int unit,
        uint32 flags, /* flags may be SHR_RES_ALLOC_WITH_ID or 0; */
        uint32  *ma_id) 
{
    int elem = *ma_id;
    BCMDNX_INIT_FUNC_DEFS;
    BCMDNX_IF_ERR_EXIT(dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_oam_ma_index,flags,1,&elem));
    *ma_id = elem;
...
{
    BCMDNX_INIT_FUNC_DEFS;
    BCMDNX_IF_ERR_EXIT(dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_res_oam_ma_index, 1, ma_id));
exit:
    BCMDNX_FUNC_RETURN;
}
...
{
    BCMDNX_INIT_FUNC_DEFS;
    BCM_RETURN_VAL_EXIT(dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_res_oam_ma_index, 1, ma_id));
        ...
        uint32  *mep_id) 
{
    ...
    int rv;
    BCMDNX_INIT_FUNC_DEFS;
    rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_res_oam_mep_id_short, flags, 1, &elem);
    if (rv != BCM_E_NONE) {
...
{
    BCMDNX_INIT_FUNC_DEFS;
    BCMDNX_IF_ERR_EXIT(dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_res_oam_mep_id_short, 1, mep_id));
exit:
    BCMDNX_FUNC_RETURN;
}
...
{
    BCMDNX_INIT_FUNC_DEFS;
    BCM_RETURN_VAL_EXIT(dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_res_oam_mep_id_short, 1, mep_id));
        ...
        uint32  *mep_id) 
{
    int elem = *mep_id;
    BCMDNX_INIT_FUNC_DEFS;
    BCMDNX_IF_ERR_EXIT(dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_oam_mep_id_long, flags, 1, &elem));
    *mep_id = elem;
exit:
    BCMDNX_FUNC_RETURN;
...
{
    BCMDNX_INIT_FUNC_DEFS;
    BCMDNX_IF_ERR_EXIT(dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_res_oam_mep_id_long, 1, mep_id));
...
{
    BCMDNX_INIT_FUNC_DEFS;
    BCM_RETURN_VAL_EXIT(dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_res_oam_mep_id_long, 1, mep_id));
        ...
        uint32  *rmep_id) 
{
    int elem = *rmep_id;
    BCMDNX_INIT_FUNC_DEFS;
    BCMDNX_IF_ERR_EXIT(dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_oam_rmep_id,flags,1,&elem));
    *rmep_id = elem;
exit:
    BCMDNX_FUNC_RETURN;
...
{
    BCMDNX_INIT_FUNC_DEFS;
    BCMDNX_IF_ERR_EXIT(dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_res_oam_rmep_id, 1, rmep_id));
exit:
    BCMDNX_FUNC_RETURN;
}
...
bcm_dpp_am_oam_rmep_id_is_alloced(
        ...
        uint32 rmep_id)
{
    return dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_res_oam_rmep_id, 1, rmep_id);
}
        ...
        uint32 *trap_code)
{
    int elem = *trap_code;
    BCMDNX_INIT_FUNC_DEFS;
    BCMDNX_IF_ERR_EXIT(dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_res_oam_trap_code_upmep_ftmh_header, flags, 1, &elem));
    *trap_code = elem;
exit:
    BCMDNX_FUNC_RETURN;
...
{
    BCMDNX_INIT_FUNC_DEFS;
    BCMDNX_IF_ERR_EXIT(dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_res_oam_trap_code_upmep_ftmh_header, 1, trap_code));
...
int
bcm_dpp_am_oam_trap_code_upmep_ftmh_header_alloced_check(
        int unit,
        uint32 trap_code)
{
    return dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_res_oam_trap_code_upmep_ftmh_header, 1, trap_code);
}
    ...
    SOC_PPD_FEC_ID *fec_id)
{
    ...
    if(usage!= 1) {
        ...
        if(!(flags & BCM_DPP_AM_FLAG_ALLOC_GLOBAL)) {/* only if not ECMP */
            if (size == 1) {
        ...
        /* allocate aligned, odd entries */
        if(flags & BCM_DPP_AM_FLAG_ALLOC_GLOBAL) {
            align = TRUE;
            rv = bcm_dpp_am_fec_global_alloc(unit, flags &(~BCM_DPP_AM_FLAG_ALLOC_GLOBAL), usage, size, align, &elem);
            BCMDNX_IF_ERR_EXIT(rv);
            /* fix alloc base to even, due to change dpp_am_res_fec_global base to 0*/
            *fec_id = elem+1; 
        }
    }
    /* for ecmp */
...
int _bcm_dpp_am_fec_verify_fec_id(int unit, SOC_PPD_FEC_ID fec_id)
{
    BCMDNX_INIT_FUNC_DEFS;
                            ...
                            int *elem)                        
{
    ...
    if (SOC_IS_JERICHO(unit)) {
        ...
        if ((flags & SHR_RES_ALLOC_WITH_ID) != 0) {
            int used_count;
            ...
            rv = SOC_DPP_WB_ENGINE_GET_ARR(unit, SOC_DPP_WB_ENGINE_VAR_FEC_BANK_GROUPS, &bank_group, fec_bank_id); 
            BCMDNX_IF_ERR_EXIT(rv);

            rv = dpp_am_res_used_count(unit, fec_bank_id, dpp_am_res_fec_global, &used_count); 
            BCMDNX_IF_ERR_EXIT(rv);
            ...
            if (used_count == 0) {
                ...
                rv = SOC_DPP_WB_ENGINE_SET_ARR(unit, SOC_DPP_WB_ENGINE_VAR_FEC_BANK_GROUPS, &bank_group, fec_bank_id);
                BCMDNX_IF_ERR_EXIT(rv);
            }
            ...
            for (bank_idx = 0; bank_idx < SOC_DPP_DEFS_GET(unit, nof_fec_banks); bank_idx++) {
                ...
                rv = SOC_DPP_WB_ENGINE_GET_ARR(unit, SOC_DPP_WB_ENGINE_VAR_FEC_BANK_GROUPS, &bank_group, bank_idx);
                BCMDNX_IF_ERR_EXIT(rv);

                rv = dpp_am_res_used_count(unit, bank_idx, dpp_am_res_fec_global, &used_count); 
                BCMDNX_IF_ERR_EXIT(rv);
            ...
            for (bank_idx = 0; bank_idx < SOC_DPP_DEFS_GET(unit, nof_fec_banks); bank_idx++) {
                ...
                if (used_count == 0) {
                    ...
                    rv = SOC_DPP_WB_ENGINE_SET_ARR(unit, SOC_DPP_WB_ENGINE_VAR_FEC_BANK_GROUPS, &bank_group, bank_idx);
                    BCMDNX_IF_ERR_EXIT(rv);
 ...
 */
int bcm_dpp_am_fec_get_usage(int unit, SOC_PPD_FEC_ID fec_id, _bcm_dpp_am_fec_alloc_usage *usage)
{
    int rv;
    int bank_id = 0;
    int used_count;
    ...
    *usage = 0;
    if (SOC_IS_JERICHO(unit)) {
        ...
        rv = dpp_am_res_used_count(unit, bank_id, dpp_am_res_fec_global, &used_count); 
        BCMDNX_IF_ERR_EXIT(rv);
        BCMDNX_VERIFY(used_count > 0);

        rv = SOC_DPP_WB_ENGINE_GET_ARR(unit, SOC_DPP_WB_ENGINE_VAR_FEC_BANK_GROUPS, &bank_group, bank_id);
        BCMDNX_IF_ERR_EXIT(rv);
    ...
    int *policer_id)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_ether_policer,flags,size,policer_id);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    int policer_id)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_ether_policer,size,policer_id);
    BCMDNX_IF_ERR_EXIT(rv);
...
{
    BCMDNX_INIT_FUNC_DEFS;
    BCM_RETURN_VAL_EXIT(dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_ether_policer,1,policer_id));
        ...
        uint16 *pon_channel_profile)
{
    ...
    rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_res_pon_channel_profile, flags, 1, &elem);
        ...
        uint16 pon_channel_profile)
{
    ...
    rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_res_pon_channel_profile, 1, elem);
    BCMDNX_IF_ERR_EXIT(rv);
        ...
        uint16 pon_channel_profile)
{
    ...
    return dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_res_pon_channel_profile, 1, elem);
}
/*
 * PON MOdule - End
 */
    ...
    int *action_id)
{
    ...
    rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID, res_pool, alloc_flags, 1, action_id);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    int action_id)
{
    ...
    rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID, res_pool, 1, action_id);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    int action_id)
{
    ...
    BCM_RETURN_VAL_EXIT(dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID, res_pool, 1 ,action_id));
exit:
    BCMDNX_FUNC_RETURN;
}
    ...
    int *virtual_nick_name_index)
{
    ...
    rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_res_trill_virtual_nick_name, alloc_flags, 1, virtual_nick_name_index);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    int virtual_nick_name_index)
{
    ...
    rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_res_trill_virtual_nick_name, 1, virtual_nick_name_index);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    int virtual_nick_name_index)
{
    ...
    BCM_RETURN_VAL_EXIT(dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_res_trill_virtual_nick_name, 1 ,virtual_nick_name_index));
exit:
    BCMDNX_FUNC_RETURN;
}
...
/* Sync global lif */
int 
bcm_dpp_am_global_sync_lif_alloc(int unit, uint32 flags, int *global_lif){
    BCMDNX_INIT_FUNC_DEFS;
...
int 
bcm_dpp_am_global_sync_lif_dealloc(int unit, int global_lif){
    BCMDNX_INIT_FUNC_DEFS;

    BCMDNX_IF_ERR_EXIT(_bcm_dpp_am_global_sync_lif_internal_dealloc(unit, global_lif));
exit:
    BCMDNX_FUNC_RETURN;
...
/* Egress global lif */
int 
bcm_dpp_am_global_egress_lif_alloc(int unit, uint32 flags, int *global_lif){
    BCMDNX_INIT_FUNC_DEFS;

    BCMDNX_IF_ERR_EXIT(_bcm_dpp_am_global_egress_lif_internal_alloc(unit, flags, global_lif));
exit:
    BCMDNX_FUNC_RETURN;
}

int 
bcm_dpp_am_global_egress_lif_is_alloced(int unit, int global_lif){
    BCMDNX_INIT_FUNC_DEFS;

    BCM_RETURN_VAL_EXIT(_bcm_dpp_am_global_egress_lif_internal_is_alloced(unit, global_lif));
exit:
    BCMDNX_FUNC_RETURN;
}

int 
bcm_dpp_am_global_egress_lif_dealloc(int unit, int global_lif){
    BCMDNX_INIT_FUNC_DEFS;

    BCMDNX_IF_ERR_EXIT(_bcm_dpp_am_global_egress_lif_internal_dealloc(unit, global_lif));
exit:
    BCMDNX_FUNC_RETURN;
}
...
/* Ingress global lif */
int 
bcm_dpp_am_global_ingress_lif_alloc(int unit, uint32 flags, int *global_lif){
    BCMDNX_INIT_FUNC_DEFS;

    BCMDNX_IF_ERR_EXIT(_bcm_dpp_am_global_ingress_lif_internal_alloc(unit, flags, global_lif));
exit:
    BCMDNX_FUNC_RETURN;
}

int 
bcm_dpp_am_global_ingress_lif_is_alloced(int unit, int global_lif){
    BCMDNX_INIT_FUNC_DEFS;

    BCM_RETURN_VAL_EXIT(_bcm_dpp_am_global_ingress_lif_internal_is_alloced(unit, global_lif));
exit:
    BCMDNX_FUNC_RETURN;
}

int 
bcm_dpp_am_global_ingress_lif_dealloc(int unit, int global_lif){
    BCMDNX_INIT_FUNC_DEFS;

    BCMDNX_IF_ERR_EXIT(_bcm_dpp_am_global_ingress_lif_internal_dealloc(unit, global_lif));
exit:
    BCMDNX_FUNC_RETURN;
}
 ...
 _bcm_dpp_am_template_user_defined_trap_from_stream(void* data,const void* from_stream, size_t data_size)
{
    ...
    BCMDNX_INIT_FUNC_DEFS_NO_UNIT;
    SOC_PPD_TRAP_FRWRD_ACTION_PROFILE_INFO_clear(&(user_defined_trap->trap));
...
_bcm_dpp_template_setup(int unit, int template_id, int core_id, int pool_id, bcm_dpp_am_template_info_t *t_info) {
    ...
    t_info->hashExtra.to_stream = _bcm_dpp_am_template_memcpy_to_stream;
    t_info->hashExtra.from_stream = _bcm_dpp_am_template_memcpy_from_stream;    
    ...
    switch (template_id) 
    {
        case dpp_am_template_egress_thresh:
            ...
            t_info->hashExtra.orig_data_size = t_info->data_size;
    ...
    case dpp_am_template_egress_interface_unicast_thresh:
            ...
            t_info->hashExtra.orig_data_size = t_info->data_size;
            desc_str = "Egress threshold for interface unicat flow control";
            break;
        case dpp_am_template_egress_interface_multicast_thresh:
            ...
            t_info->hashExtra.orig_data_size = t_info->data_size;
            desc_str = "Egress threshold for interface multicat flow control";
            break;
        case dpp_am_template_user_defined_traps:
            ...
            t_info->hashExtra.orig_data_size = sizeof(bcm_dpp_user_defined_traps_t);
            t_info->hashExtra.to_stream = _bcm_dpp_am_template_user_defined_trap_to_stream;
            t_info->hashExtra.from_stream = _bcm_dpp_am_template_user_defined_trap_from_stream;
        ...
        case dpp_am_template_snoop_cmd:
            ...
            t_info->hashExtra.orig_data_size = sizeof(bcm_dpp_snoop_t);
            t_info->hashExtra.to_stream = _bcm_dpp_am_template_snoop_cmd_to_stream;
            t_info->hashExtra.from_stream = _bcm_dpp_am_template_snoop_cmd_from_stream;
            desc_str = "Snoop info Per PP port";
            break;
        case dpp_am_template_tpid_profile:
            t_info->manager = SHR_TEMPLATE_MANAGE_HASH;
            t_info->start = _DPP_AM_TEMPLATE_TPID_PROFILE_LOW_ID;
            t_info->count = _DPP_AM_TEMPLATE_TPID_PROFILE_COUNT;
        ...
        case dpp_am_template_oam_tx_priority:
          ...
          break;
        case dpp_am_template_bfd_ipv4_dip:
            if (SOC_IS_ARAD(unit)) {
                t_info->manager = SHR_TEMPLATE_MANAGE_HASH;
                t_info->start = _DPP_AM_TEMPLATE_BFD_IPV4_DIP_PROFILE_LOW_ID;
                t_info->count = _DPP_AM_TEMPLATE_BFD_IPV4_DIP_PROFILE_COUNT;
                t_info->max_entities = _DPP_AM_TEMPLATE_BFD_IPV4_DIP_PROFILE_MAX_ENTITIES;
                t_info->data_size = _DPP_AM_TEMPLATE_BFD_IPV4_DIP_PROFILE_SIZE;
                t_info->hashExtra.orig_data_size = t_info->data_size;
        ...
        case dpp_am_template_oam_punt_event_hendling_profile:
            if (SOC_IS_ARADPLUS(unit)) {
                ...
                t_info->hashExtra.orig_data_size = t_info->data_size;
                desc_str ="oam punt event hendling profile";
            } else {
                BCM_EXIT;
            }
          break; 
        ...
        case dpp_am_template_bfd_tx_rate_profile:
            if (SOC_IS_ARAD(unit)) {
                t_info->manager = SHR_TEMPLATE_MANAGE_HASH;
                t_info->start = _DPP_AM_TEMPLATE_BFD_TX_RATE_PROFILE_LOW_ID;
                t_info->count = _DPP_AM_TEMPLATE_BFD_TX_RATE_PROFILE_COUNT;
                t_info->max_entities = _DPP_AM_TEMPLATE_BFD_TX_RATE_PROFILE_MAX_ENTITIES;
                t_info->data_size = _DPP_AM_TEMPLATE_BFD_TX_RATE_PROFILE_SIZE;
                t_info->hashExtra.orig_data_size = t_info->data_size;
                desc_str ="bfd tx rate profile";
            } else {
                BCM_EXIT;
            }
          break;
        ...
        case dpp_am_template_system_red_dp_pr:
            ...
            break;
        case dpp_am_template_vsq_rate_cls:
            t_info->manager = SHR_TEMPLATE_MANAGE_HASH;
            t_info->start = _DPP_AM_TEMPLATE_VSQ_RATE_CLS_LOW_ID;
            t_info->count = _DPP_AM_TEMPLATE_VSQ_RATE_CLS_COUNT;
            t_info->max_entities = _DPP_AM_TEMPLATE_VSQ_RATE_CLS_MAX_ENTITIES;
            t_info->data_size = _DPP_AM_TEMPLATE_VSQ_RATE_CLS_SIZE;
            t_info->hashExtra.orig_data_size = t_info->data_size;
            desc_str = "VSQ Rate class";
            break;
        case dpp_am_template_queue_discount_cls:
            t_info->manager = SHR_TEMPLATE_MANAGE_HASH;
            t_info->start = _DPP_AM_TEMPLATE_QUEUE_DISCNT_CLS_LOW_ID;
            t_info->count = _DPP_AM_TEMPLATE_QUEUE_DISCNT_CLS_COUNT;
            ...
            t_info->hashExtra.to_stream = _bcm_dpp_am_template_discount_cls_to_stream;
            t_info->hashExtra.from_stream = _bcm_dpp_am_template_discount_cls_from_stream;
            t_info->hashExtra.orig_data_size = sizeof(SOC_TMC_ITM_CR_DISCOUNT_INFO);
        ...
        case dpp_am_template_meter_profile_b_low:
            t_info->manager = SHR_TEMPLATE_MANAGE_HASH;
            if (template_id == dpp_am_template_meter_profile_a_low || template_id == dpp_am_template_meter_profile_b_low) {
                t_info->start = _DPP_AM_TEMPLATE_METER_PROFILE_LOW_ID;
                t_info->count = _DPP_AM_TEMPLATE_METER_PROFILE_COUNT(unit);
            }
            else { /* high */
                if (SOC_IS_ARAD(unit)) {
                    /* no high_profiles in ARAD */
                    BCM_EXIT;
                }
                t_info->start = _DPP_AM_TEMPLATE_METER_PROFILE_LOW_ID;
                t_info->count = _DPP_AM_TEMPLATE_METER_HIGH_PROFILE_COUNT(unit);
            }
        ...
        case dpp_am_template_l2_vsi_learn_profile:
            ...
            desc_str = "Ingress UC TC Mapping Profiles";
            break;
        case dpp_am_template_fid_aging_profile:
            t_info->manager = SHR_TEMPLATE_MANAGE_HASH;
            t_info->start = _DPP_AM_TEMPLATE_FID_AGING_PROFILE_LOW_ID;            
            t_info->count = _DPP_AM_TEMPLATE_FID_AGING_PROFILE_COUNT;                      
            t_info->max_entities = _DPP_AM_TEMPLATE_FID_AGING_PROFILE_MAX_ENTITIES;
            t_info->data_size = _DPP_AM_TEMPLATE_FID_AGING_PROFILE_SIZE;
            t_info->hashExtra.orig_data_size = t_info->data_size;
            desc_str = "Ingress FID Aging Profiles";
            break;
...
STATIC int
_bcm_dpp_am_template_cosq_egr_thresh_port_profile_mapping_get(int unit, int port, int *profile)
{
    BCMDNX_INIT_FUNC_DEFS;
    *profile = _bcm_dpp_egr_thresh_map[unit][port];
...
int
_bcm_dpp_am_template_cosq_egr_thresh_port_profile_mapping_set(int unit, int port, int profile)
{
    int rv;

    BCMDNX_INIT_FUNC_DEFS;

    /* _bcm_dpp_egr_thresh_map[unit].map_port_thresh_type[port] = profile; */
...
int _bcm_dpp_am_template_cosq_egr_thresh_init(int unit,int template_init_id,bcm_dpp_cosq_egress_thresh_key_info_t *egr_thresh_data_init)
{
    ...
    result = dpp_am_template_allocate_group(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_egress_thresh, flags, egr_thresh_data_init, 
    ...
    for (index = 0; index < _DPP_AM_TEMPLATE_COSQ_EGR_THRESH_MAX_ENTITIES(unit); index++) {

        _bcm_dpp_egr_thresh_map[unit][index] = template_init_id;
...
int _bcm_dpp_am_template_cosq_egr_thresh_data_get(int unit, int port, bcm_dpp_cosq_egress_thresh_key_info_t * data)
{
    int old_profile;
    int rc = BCM_E_NONE;

    BCMDNX_INIT_FUNC_DEFS;
    _bcm_dpp_am_template_cosq_egr_thresh_port_profile_mapping_get(unit,port,&old_profile);
    ...
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_egress_thresh,old_profile,data);
...
int _bcm_dpp_am_template_cosq_egr_thresh_exchange(int unit, int port, bcm_dpp_cosq_egress_thresh_key_info_t * data, int *old_template, int *is_last, int *template,int *is_allocated)
{
    int rc = BCM_E_NONE;
    int temp;
    int temp_is_last;
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = _bcm_dpp_am_template_cosq_egr_thresh_port_profile_mapping_get(unit,port,&temp);
    BCMDNX_IF_ERR_EXIT(rc);
    ...
    rc = dpp_am_template_exchange(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_egress_thresh,0,data,temp,&temp_is_last,template,is_allocated);
    BCMDNX_IF_ERR_EXIT(rc);
    ...
    _bcm_dpp_am_template_cosq_egr_thresh_port_profile_mapping_set(unit,port,*template);
    BCMDNX_IF_ERR_EXIT(rc);
...
_bcm_dpp_am_template_cosq_egr_thresh_ref_get(int unit, int template, uint32 *ref_count)
{
    ...
    rc = dpp_am_template_ref_count_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_egress_thresh, template, ref_count);
...
_bcm_dpp_am_template_cosq_egr_thresh_allocate_group(int unit, uint32 flags, bcm_dpp_cosq_egress_thresh_key_info_t *data, int ref_count, int *is_allocated, int *template)
{
    ...
    rc = dpp_am_template_allocate_group(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_egress_thresh, flags, (void *)data, ref_count, is_allocated, template);
...
_bcm_dpp_am_template_cosq_egr_thresh_tdata_get(int unit, int template, bcm_dpp_cosq_egress_thresh_key_info_t *data)
{
    ...
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_egress_thresh, template, (void *)data);
...
STATIC int
_bcm_dpp_am_template_cosq_egr_interface_unicast_thresh_port_profile_mapping_get(int unit, int port, int *profile)
{
    BCMDNX_INIT_FUNC_DEFS;
    *profile = _bcm_dpp_egr_interface_unicast_thresh_map[unit][port];
...
STATIC int
_bcm_dpp_am_template_cosq_egr_interface_multicast_thresh_port_profile_mapping_get(int unit, int port, int *profile)
{
    BCMDNX_INIT_FUNC_DEFS;


    *profile = _bcm_dpp_egr_interface_multicast_thresh_map[unit][port];
...
STATIC int
_bcm_dpp_am_template_cosq_egr_interface_unicast_thresh_port_profile_mapping_set(int unit, int port, int profile)
{
    int rv;
                                              ...
                                              bcm_dpp_cosq_egress_interface_unicast_thresh_key_info_t *egr_interface_unicast_thresh_data_init)
{
    ...
    /* Add all entities to template init id with given data */
    result = dpp_am_template_allocate_group(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_egress_interface_unicast_thresh, flags, egr_interface_unicast_thresh_data_init, 
    ...
    for (index = 0; index < _DPP_AM_TEMPLATE_COSQ_EGR_INTERFACE_UNICAST_THRESH_MAX_ENTITIES; index++) {
        ...
        _bcm_dpp_egr_interface_unicast_thresh_map[unit][index] = template_init_id;
...
int _bcm_dpp_am_template_cosq_egr_interface_unicast_thresh_data_get(int unit, int interface_id, bcm_dpp_cosq_egress_interface_unicast_thresh_key_info_t * data)
{
    int old_profile;
    int rc = BCM_E_NONE;

    BCMDNX_INIT_FUNC_DEFS;
    _bcm_dpp_am_template_cosq_egr_interface_unicast_thresh_port_profile_mapping_get(unit,interface_id,&old_profile);
    BCMDNX_IF_ERR_EXIT(rc);
    ...
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_egress_interface_unicast_thresh,old_profile,data);
    BCMDNX_IF_ERR_EXIT(rc);
exit:
    BCMDNX_FUNC_RETURN;
}

int _bcm_dpp_am_template_cosq_egr_interface_unicast_thresh_exchange(int unit, int interface_id, bcm_dpp_cosq_egress_interface_unicast_thresh_key_info_t * data, int *old_template, int *is_last, int *template,int *is_allocated)
{
    int rc = BCM_E_NONE;
    int temp;
    int temp_is_last;
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = _bcm_dpp_am_template_cosq_egr_interface_unicast_thresh_port_profile_mapping_get(unit,interface_id,&temp);
    BCMDNX_IF_ERR_EXIT(rc);
    ...
    rc = dpp_am_template_exchange(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_egress_interface_unicast_thresh,0,data,temp,&temp_is_last,template,is_allocated);
    BCMDNX_IF_ERR_EXIT(rc);
    ...
    /* Since port-profile mapping is SW, update also the mapping. */
    _bcm_dpp_am_template_cosq_egr_interface_unicast_thresh_port_profile_mapping_set(unit,interface_id,*template);
    BCMDNX_IF_ERR_EXIT(rc);
exit:
    BCMDNX_FUNC_RETURN;
}
...
STATIC int
_bcm_dpp_am_template_cosq_egr_interface_multicast_thresh_port_profile_mapping_set(int unit, int interface_id, int profile)
{
    int rv;
                                              ...
                                              bcm_dpp_cosq_egress_interface_multicast_thresh_key_info_t *egr_interface_multicast_thresh_data_init)
{
    ...
    result = dpp_am_template_allocate_group(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_egress_interface_multicast_thresh, flags, egr_interface_multicast_thresh_data_init, 
    ...
    for (index = 0; index < _DPP_AM_TEMPLATE_COSQ_EGR_INTERFACE_MULTICAST_THRESH_MAX_ENTITIES; index++) {
        ...
        _bcm_dpp_egr_interface_multicast_thresh_map[unit][index] = template_init_id;
...
int _bcm_dpp_am_template_cosq_egr_interface_multicast_thresh_data_get(int unit, int interface_id, bcm_dpp_cosq_egress_interface_multicast_thresh_key_info_t * data)
{
    int old_profile;
    int rc = BCM_E_NONE;

    BCMDNX_INIT_FUNC_DEFS;
    _bcm_dpp_am_template_cosq_egr_interface_multicast_thresh_port_profile_mapping_get(unit,interface_id,&old_profile);
    BCMDNX_IF_ERR_EXIT(rc);
    ...
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_egress_interface_multicast_thresh,old_profile,data);
...
int _bcm_dpp_am_template_cosq_egr_interface_multicast_thresh_exchange(int unit, int interface_id, bcm_dpp_cosq_egress_interface_multicast_thresh_key_info_t * data, int *old_template, int *is_last, int *template,int *is_allocated)
{
    int rc = BCM_E_NONE;
    int temp;
    int temp_is_last;
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = _bcm_dpp_am_template_cosq_egr_interface_multicast_thresh_port_profile_mapping_get(unit,interface_id,&temp);
    BCMDNX_IF_ERR_EXIT(rc);
    ...
    rc = dpp_am_template_exchange(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_egress_interface_multicast_thresh,0,data,temp,&temp_is_last,template,is_allocated);
    BCMDNX_IF_ERR_EXIT(rc);
    ...
    /* Since port-profile mapping is SW, update also the mapping. */
    _bcm_dpp_am_template_cosq_egr_interface_multicast_thresh_port_profile_mapping_set(unit,interface_id,*template);
    BCMDNX_IF_ERR_EXIT(rc);
...
{
    int rc = BCM_E_NONE;
	uint32 data_dummy = 0xfff ;/*insiginificant number*/
	...
	if (flags & SHR_TEMPLATE_MANAGE_IGNORE_DATA) {
		...
		for (prof_ind=DPP_MIRROR_ACTION_NDX_MIN ; prof_ind<= DPP_MIRROR_ACTION_NDX_MAX ; ++prof_ind) {
			if ( (flags &SHR_TEMPLATE_MANAGE_SET_WITH_ID)==0 || ((flags &SHR_TEMPLATE_MANAGE_SET_WITH_ID) && prof_ind==*profile ) ) {
				...
				if (ref_count==0) {
					...
					rc = dpp_am_template_allocate(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_mirror_action_profile, SHR_TEMPLATE_MANAGE_SET_WITH_ID, data_to_give, is_allocateded_to_give, &prof_ind);
					BCMDNX_IF_ERR_EXIT(rc); 
					is_alloced=1;
					*profile = prof_ind;
					break;
		...
		rc = dpp_am_template_allocate(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_mirror_action_profile, flags, data, is_allocateded_to_give, profile);
		BCMDNX_IF_ERR_EXIT(rc);
	}
...
_bcm_dpp_am_template_mirror_action_profile_free(int unit, int profile, int *is_last)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_mirror_action_profile, profile, is_last);
	BCMDNX_IF_ERR_EXIT(rc);
exit:
    BCMDNX_FUNC_RETURN;
}
...
_bcm_dpp_am_template_mirror_action_profile_get(int unit, int mirror_profile, uint32 *data)
{
    ...
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_mirror_action_profile, mirror_profile, data);
	BCMDNX_IF_ERR_EXIT(rc);
exit:
    BCMDNX_FUNC_RETURN;
}
...
_bcm_dpp_am_template_mirror_action_profile_ref_get(int unit, int profile, uint32 *ref_count)
{
    ...
    rc = dpp_am_template_ref_count_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_mirror_action_profile, profile, ref_count);
...
#endif
    /* Add all entities to template init id with given data */
    result = dpp_am_template_allocate_group(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_user_defined_traps, flags, data, _DPP_AM_TEMPLATE_USER_DEFINED_TRAPS_MAX_ENTITIES(unit), &is_allocated,&template);
    BCMDNX_IF_ERR_EXIT(result);
...
_bcm_dpp_am_template_user_defined_port_trap_mapping_get(int unit, int port,int *user_defined_trap)
{
    ...
    if ((trap_code < SOC_PPD_TRAP_CODE_USER_DEFINED_0) || (trap_code - SOC_PPD_TRAP_CODE_USER_DEFINED_0 >= _DPP_AM_TEMPLATE_USER_DEFINED_TRAPS_COUNT)) {
        /* Expect trap code is not user defined */
        BCMDNX_ERR_EXIT_MSG(BCM_E_INTERNAL, (_BSL_BCM_MSG("Expect trap code is not user defined")));
    }

    *user_defined_trap = trap_code - SOC_PPD_TRAP_CODE_USER_DEFINED_0;
...
int _bcm_dpp_am_template_user_defined_trap_allocate(int unit, uint32 flags, int port, const bcm_dpp_user_defined_traps_t* data, int *is_allocated, int *user_defined_trap)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_allocate(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_user_defined_traps, flags, data, is_allocated, user_defined_trap);
...
int _bcm_dpp_am_template_user_defined_trap_free(int unit, int port, int* is_last, int *user_defined_trap)
{
    ...
    rc = dpp_am_template_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_user_defined_traps,*user_defined_trap,is_last);
...
int _bcm_dpp_am_template_user_defined_trap_exchange(int unit, int port, bcm_dpp_user_defined_traps_t * data, int *old_template, int *is_last, int *template,int *is_allocated)
{
    ...
    rc = dpp_am_template_exchange(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_user_defined_traps,SHR_TEMPLATE_MANAGE_IGNORE_NOT_EXIST_OLD_TEMPLATE,data,old_ud,is_last,template,is_allocated);
    BCMDNX_IF_ERR_EXIT(rc);
    ...
    if (*is_last) {
       ...
       trap_id = old_ud + SOC_PPD_TRAP_CODE_USER_DEFINED_0;
       rc = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_trap_user_define,SHR_RES_ALLOC_WITH_ID,trap_id);
       BCMDNX_IF_ERR_EXIT(rc);

    }   

    if (*is_allocated) {
       /*is_allocated means first time in use, there for allocte only the first time*/
       trap_id = *template + SOC_PPD_TRAP_CODE_USER_DEFINED_0;
       rc = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_trap_user_define,SHR_RES_ALLOC_WITH_ID,1,&trap_id);
       BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_snoop_cmd_init(int unit,int template_init_id,const bcm_dpp_snoop_t* data)
{
    ...
    /* Add all entities to template init id with given data */
    result = dpp_am_template_allocate_group(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_snoop_cmd, flags, data,_DPP_AM_TEMPLATE_SNOOP_CMD_MAX_ENTITIES(unit), &is_allocated,&template_init_id);
    BCMDNX_IF_ERR_EXIT(result);
...
int _bcm_dpp_am_template_snoop_cmd_allocate(int unit, uint32 flags, int port, const bcm_dpp_snoop_t* data, int *is_allocated, int *snoop_cmd)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_allocate(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_snoop_cmd, flags, data, is_allocated, snoop_cmd);
    BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_snoop_cmd_free(int unit, int port, int* is_last, int *snoop_cmd)
{
    ...
    rc = dpp_am_template_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_snoop_cmd,snoop,is_last);
    BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_snoop_cmd_exchange(int unit, int port, bcm_dpp_snoop_t * data, int *old_snoop_cmd, int *is_last, int *new_snoop_cmd,int *is_allocated)
{
    ...
    rc = dpp_am_template_exchange(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_snoop_cmd,0,data,snoop_cmd,&is_last_temp,new_snoop_cmd,is_allocated);
    BCMDNX_IF_ERR_EXIT(rc);
    ...
    if (is_last_temp) {
        /*is_last means not in use anymore, there for free allocation*/
        rc = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_snoop_cmd,SHR_RES_ALLOC_WITH_ID,snoop_cmd);
        BCMDNX_IF_ERR_EXIT(rc);
    ...
    if (*is_allocated) {
        ...
        rc = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_snoop_cmd,SHR_RES_ALLOC_WITH_ID,1,new_snoop_cmd);
        BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_snoop_cmd_data_get(int unit, int port, bcm_dpp_snoop_t * data)
{
    ...
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_snoop_cmd,snoop_cmd,data);
...
/* TPID profile - Start */
STATIC int
_bcm_dpp_am_template_tpid_profile_port_tpid_profile_mapping_get(int unit, int port,int *tpid_profile)
{
    uint32 soc_sand_rv;
    int soc_sand_dev_id;
    SOC_PPD_PORT_INFO port_info;
...
int _bcm_dpp_am_template_tpid_profile_init(int unit,int template_init_id,_bcm_petra_tpid_profile_t profile_type, int accept_mode, _bcm_petra_dtag_mode_t dtag_mode, _bcm_petra_ac_key_map_type_t ac_key_map_type)
{
    ...
    for (index = 0; index < _DPP_AM_TEMPLATE_TPID_PROFILE_MAX_ENTITIES(unit); index++) {
        result = dpp_am_template_allocate(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_tpid_profile, flags, &data, &is_allocated,&template);
        BCMDNX_IF_ERR_EXIT(result);
    }
...
int _bcm_dpp_am_template_tpid_profile_allocate(int unit, uint32 flags, int port, _bcm_petra_tpid_profile_t profile_type, int accept_mode, _bcm_petra_dtag_mode_t dtag_mode, _bcm_petra_ac_key_map_type_t ac_key_map_type, int *is_allocated, int *tpid_profile)
{
    ...
    rc = dpp_am_template_allocate(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_tpid_profile, flags, &data, is_allocated, tpid_profile);
    BCMDNX_IF_ERR_EXIT(rc);
exit:
    BCMDNX_FUNC_RETURN;
}

int _bcm_dpp_am_template_tpid_profile_free(int unit, int port, int* is_last, int *tpid_profile)
{
    int rc = BCM_E_NONE;

    BCMDNX_INIT_FUNC_DEFS;
    rc = _bcm_dpp_am_template_tpid_profile_port_tpid_profile_mapping_get(unit,port,tpid_profile);
    BCMDNX_IF_ERR_EXIT(rc);

    rc = dpp_am_template_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_tpid_profile,*tpid_profile,is_last);
...
int _bcm_dpp_am_template_tpid_profile_exchange(int unit, int port, _bcm_petra_tpid_profile_t profile_type, int accept_mode, _bcm_petra_dtag_mode_t dtag_mode, _bcm_petra_ac_key_map_type_t ac_key_map_type, int *old_tpid_profile, int *is_last, int *new_tpid_profile,int *is_allocated)
{
    int rc = BCM_E_NONE;
    int data = 0;
    int tmp_old_profile;
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = _bcm_dpp_am_template_tpid_profile_port_tpid_profile_mapping_get(unit,port,&tmp_old_profile);
    BCMDNX_IF_ERR_EXIT(rc);
    ...
    rc = dpp_am_template_exchange(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_tpid_profile,0,&data,tmp_old_profile,is_last,new_tpid_profile,is_allocated);
    BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_tpid_profile_data_get(int unit,int port,_bcm_petra_tpid_profile_t *profile_type, int *accept_mode, _bcm_petra_dtag_mode_t *dtag_mode, _bcm_petra_ac_key_map_type_t *ac_key_map_type)
{
    int index;
    int val=0;
    int rc = BCM_E_NONE;
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = _bcm_dpp_am_template_tpid_profile_port_tpid_profile_mapping_get(unit,port,&index);
    BCMDNX_IF_ERR_EXIT(rc);
    ...
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_tpid_profile,index,&val);
    BCMDNX_IF_ERR_EXIT(rc);
...
_bcm_dpp_am_template_tpid_profile_ref_get(int unit, int template, uint32 *ref_count)
{
    ...
    rc = dpp_am_template_ref_count_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_tpid_profile, template, ref_count);
...
_bcm_dpp_am_template_tpid_profile_allocate_group(int unit, uint32 flags, uint32 *data, int ref_count, int *is_allocated, int *template)
{
    ...
    rc = dpp_am_template_allocate_group(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_tpid_profile, flags, (void *)data, ref_count, is_allocated, template);
    BCMDNX_IF_ERR_EXIT(rc);
...
_bcm_dpp_am_template_tpid_profile_tdata_get(int unit, int template, uint32 *data)
{
    ...
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_tpid_profile, template, data);
    BCMDNX_IF_ERR_EXIT(rc);
...
STATIC int
_bcm_dpp_am_template_vlan_edit_profile_mapping_get(int unit, SOC_PPD_LIF_ID lif_id, int *profile)
{
    bcm_error_t rc = BCM_E_NONE;
    uint32 soc_sand_rv;
    int soc_sand_dev_id;    
    ...
    if(lif_entry_info.type != SOC_PPD_LIF_ENTRY_TYPE_AC && lif_entry_info.type != SOC_PPD_LIF_ENTRY_TYPE_FIRST_AC_IN_GROUP
       && lif_entry_info.type != SOC_PPD_LIF_ENTRY_TYPE_EMPTY) {
        found = 0;
    }
...
int _bcm_dpp_am_template_vlan_edit_profile_mapping_init(int unit,int template_init_id,const bcm_dpp_vlan_edit_profile_mapping_info_t *mapping_profile)
{
    ...
    result = dpp_am_template_allocate_group(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_vlan_edit_profile_mapping,flags, mapping_profile,_DPP_AM_TEMPLATE_VLAN_EDIT_PROFILE_MAPPING_MAX_ENTITIES(unit),&is_allocated,&template);
    BCMDNX_IF_ERR_EXIT(result);
...
int _bcm_dpp_am_template_vlan_edit_profile_mapping_data_get_by_template(int unit,int index, bcm_dpp_vlan_edit_profile_mapping_info_t *mapping_profile)
{
    ...
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_vlan_edit_profile_mapping,index,mapping_profile);
    BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_vlan_edit_profile_mapping_data_get(int unit,SOC_PPD_LIF_ID lif_id, bcm_dpp_vlan_edit_profile_mapping_info_t *mapping_profile)
{
    int index;
    int rc = BCM_E_NONE;
...
int _bcm_dpp_am_template_vlan_edit_profile_mapping_exchange(int unit, SOC_PPD_LIF_ID lif_id, bcm_dpp_vlan_edit_profile_mapping_info_t *mapping_profile, int *old_profile, int *is_last, int *new_profile,int *is_allocated)
{
    int rc = BCM_E_NONE;
    int tmp_old_profile;
    int tmp_is_last;
    ...
    rc = dpp_am_template_exchange(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_vlan_edit_profile_mapping,0,mapping_profile,tmp_old_profile,&tmp_is_last,new_profile,is_allocated);
    BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_vlan_edit_profile_mapping_alloc(int unit, int flags, SOC_PPD_LIF_ID lif_id, bcm_dpp_vlan_edit_profile_mapping_info_t* mapping_profile, int *template,int *is_allocated)
{
    int rc = BCM_E_NONE;
    ...
    rc = dpp_am_template_allocate(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_vlan_edit_profile_mapping, flags, mapping_profile, is_allocated, template);
    BCMDNX_IF_ERR_EXIT(rc);
exit:
    BCMDNX_FUNC_RETURN;
}

int _bcm_dpp_am_template_vlan_edit_profile_mapping_free(int unit, SOC_PPD_LIF_ID lif_id, int *old_template,int *is_last)
{
    int rc = BCM_E_NONE;
    ...
    rc = dpp_am_template_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_vlan_edit_profile_mapping, *old_template, is_last);
    BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_vlan_edit_profile_mapping_dummy_allocate(int unit, uint32 flags, bcm_dpp_vlan_edit_profile_mapping_info_t *mapping_profile, int *template_id, int *is_allocated)
{
    ...
    rc = dpp_am_template_allocate(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_vlan_edit_profile_mapping, flags, mapping_profile, is_allocated, template_id);
    BCMDNX_IF_ERR_EXIT(rc);
...
STATIC int
_bcm_dpp_am_template_vlan_edit_profile_eg_mapping_get(int unit, SOC_PPD_AC_ID out_ac, int *profile)
{
    uint32 soc_sand_rv;
    int soc_sand_dev_id;
                                                        ...
                                                        const bcm_dpp_vlan_egress_edit_profile_info_t *mapping_profile)
{
    ...
    /* Add all entities to template init id with given data */
    result = dpp_am_template_allocate_group(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_vlan_edit_profile_mapping_eg,flags,mapping_profile,_DPP_AM_TEMPLATE_VLAN_EDIT_PROFILE_MAPPING_EG_MAX_ENTITIES(unit),&is_allocated,&template);
    BCMDNX_IF_ERR_EXIT(result);
...
int _bcm_dpp_am_template_vlan_edit_profile_eg_mapping_allocate(int unit, uint32 flags, bcm_dpp_vlan_egress_edit_profile_info_t *mapping_profile, uint32 *template_id, int *is_allocated)
{
    ...
    rc = dpp_am_template_allocate(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_vlan_edit_profile_mapping_eg, flags, mapping_profile, is_allocated, &template);
    BCMDNX_IF_ERR_EXIT(rc);
                                                            ...
                                                            bcm_dpp_vlan_egress_edit_profile_info_t *mapping_profile)
{
    ...
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_vlan_edit_profile_mapping_eg,index,mapping_profile);
    BCMDNX_IF_ERR_EXIT(rc);
                                                            ...
                                                            int *is_allocated)
{
    ...
    rc = dpp_am_template_exchange(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_vlan_edit_profile_mapping_eg,0,mapping_profile,tmp_old_profile,&tmp_is_last,new_profile,is_allocated);
    BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_l2_cache_rm_init(int unit,int template_init_id,_bcm_petra_l2_rsrvd_mc_profile_info_t* init_data)
{
    ...
    for (index = 0; index < _DPP_AM_TEMPLATE_L2_RSRVD_MC_MAX_ENTITIES(unit); index++) {
        result = dpp_am_template_allocate(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_reserved_mc, flags, init_data, &is_allocated,&template);
        BCMDNX_IF_ERR_EXIT(result);
    }
...
int _bcm_dpp_am_template_l2_cache_rm_data_get(int unit, int port, _bcm_petra_l2_rsrvd_mc_profile_info_t * data)
{
    ...
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_reserved_mc,old_profile,data);
...
int _bcm_dpp_am_template_l2_cache_rm_exchange(int unit, int flags, int port, _bcm_petra_l2_rsrvd_mc_profile_info_t * data, int *old_template, int *is_last, int *template,int *is_allocated)
{
    ...
    if(!(flags & BCM_DPP_AM_TEMPLATE_FLAG_CHECK)) {
        rc = dpp_am_template_exchange(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_reserved_mc,0,data,*old_template,is_last,template,is_allocated);
        BCMDNX_IF_ERR_EXIT(rc);
    }
    else
    {
        rc = dpp_am_template_exchange_test(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_reserved_mc,0,data,*old_template,is_last,template,is_allocated);
    }
...
{
    BCMDNX_INIT_FUNC_DEFS;
    BCMDNX_IF_ERR_EXIT(dpp_am_template_clear(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_reserved_mc));
exit:
    BCMDNX_FUNC_RETURN;
}
...
int _bcm_dpp_am_template_port_tpid_class_init(int unit,int template_init_id,_bcm_dpp_port_tpid_class_info_t* init_data)
{
    ...
    for (index = 0; index < _DPP_AM_TEMPLATE_PORT_TPID_CLASS_MAX_ENTITIES(unit); index++) {
        result = dpp_am_template_allocate(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_tpid_class, flags, init_data, &is_allocated,&template);
        BCMDNX_IF_ERR_EXIT(result);
    }
...
int _bcm_dpp_am_template_port_tpid_class_data_get(int unit, int port, _bcm_dpp_port_tpid_class_info_t * data)
{
    ...
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_tpid_class,old_profile,data);
...
int _bcm_dpp_am_template_port_tpid_class_exchange(int unit, int flags, int port, _bcm_dpp_port_tpid_class_info_t * data, int *old_template, int *is_last, int *template,int *is_allocated)
{
    ...
    if(!(flags & BCM_DPP_AM_TEMPLATE_FLAG_CHECK)) {
        rc = dpp_am_template_exchange(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_tpid_class,0,data,*old_template,is_last,template,is_allocated);
        BCMDNX_IF_ERR_EXIT(rc);

    }
    else
    {
        rc = dpp_am_template_exchange_test(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_tpid_class,0,data,*old_template,is_last,template,is_allocated);
    }
...
{
    BCMDNX_INIT_FUNC_DEFS;
    BCMDNX_IF_ERR_EXIT(dpp_am_template_clear(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_tpid_class));
exit:
    BCMDNX_FUNC_RETURN;
}
...
int _bcm_dpp_am_template_port_tpid_class_egress_acceptable_frame_type_init(int unit,int template_init_id, _bcm_dpp_port_tpid_class_acceptable_frame_type_info_t *init_data) {
    ...
    /* Add all entities to template init id with given data */
    result = dpp_am_template_allocate_group(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_port_tpid_class_egress_acceptable_frame_type, flags, init_data, 
...
_bcm_dpp_am_template_port_tpid_class_egress_acceptable_frame_type_port_profile_mapping_get(int unit, int port, int *profile) {
    ...
    unsigned int soc_sand_dev_id;
    uint32 soc_sand_rv;
    SOC_PPD_EG_FILTER_PORT_INFO port_info;
    int core=0; 

    BCMDNX_INIT_FUNC_DEFS;
    soc_sand_dev_id = (unit);

    soc_sand_rv = soc_ppd_eg_filter_port_info_get(soc_sand_dev_id, core, port, &port_info);
    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
...
int _bcm_dpp_am_template_port_tpid_class_egress_acceptable_frame_type_data_get(int unit, int port, _bcm_dpp_port_tpid_class_acceptable_frame_type_info_t *data) {
    ...
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_port_tpid_class_egress_acceptable_frame_type,old_profile,data);
...
int _bcm_dpp_am_template_port_tpid_class_egress_acceptable_frame_type_exchange(int unit, int flags, int port, _bcm_dpp_port_tpid_class_acceptable_frame_type_info_t *data, int *old_template, int *is_last, int *template,int *is_allocated) {
    ...
    if(!(flags & BCM_DPP_AM_TEMPLATE_FLAG_CHECK)) {
        rc = dpp_am_template_exchange(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_port_tpid_class_egress_acceptable_frame_type,0,data,*old_template,is_last,template,is_allocated);
        BCMDNX_IF_ERR_EXIT(rc);

    }
    else
    {
        rc = dpp_am_template_exchange_test(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_port_tpid_class_egress_acceptable_frame_type,0,data,*old_template,is_last,template,is_allocated);
    }
...
int _bcm_dpp_am_template_port_tpid_class_egress_acceptable_frame_type_clear(int unit) {
    BCMDNX_INIT_FUNC_DEFS;
    BCMDNX_IF_ERR_EXIT(dpp_am_template_clear(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_port_tpid_class_egress_acceptable_frame_type));
exit:
    BCMDNX_FUNC_RETURN;
...
/* Trap- egress - Start */
int _bcm_dpp_am_template_trap_egress_init(int unit,int template_init_id,SOC_SAND_OUT SOC_PPD_TRAP_EG_ACTION_PROFILE_INFO *init_data)
{
    int is_allocated, rc = BCM_E_NONE;
    uint32 flags = 0, cpu_port_exist;
    SOC_PPD_TRAP_EG_ACTION_PROFILE_INFO eg_drop_info;
    int template;
    uint32 soc_sand_rv;
    unsigned int    soc_sand_dev_id;
    ...
    /* Add all entities to template init id with given data
       entity SOC_PPD_NOF_TRAP_EG_TYPES will always point to this data, so it
       will never be cleared (this is the reason for using <= and not <) */
    rc = dpp_am_template_allocate_group(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_trap_egress, flags, init_data,SOC_PPD_NOF_TRAP_EG_TYPES, &is_allocated,&template_init_id);
    BCMDNX_IF_ERR_EXIT(rc);
    ...
    rc = dpp_am_template_allocate(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_trap_egress,flags,&eg_drop_info,&is_allocated,&template);
    BCMDNX_IF_ERR_EXIT(rc);
    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        rc = _bcm_dpp_rx_egress_set(unit,SOC_PPD_TRAP_EG_TYPE_HAIR_PIN,&eg_drop_info);
        BCMDNX_IF_ERR_EXIT(rc);
        
        rc = _bcm_dpp_rx_egress_set(unit,SOC_PPD_TRAP_EG_TYPE_SPLIT_HORIZON,&eg_drop_info);
        BCMDNX_IF_ERR_EXIT(rc);
        
        rc = _bcm_dpp_rx_egress_set(unit,SOC_PPD_TRAP_EG_TYPE_PRIVATE_VLAN,&eg_drop_info);    
        BCMDNX_IF_ERR_EXIT(rc);
        /*set invalid out port to discard pkts*/
        rc = _bcm_dpp_rx_egress_set(unit,SOC_PPD_TRAP_EG_TYPE_INVALID_OUT_PORT,&eg_drop_info);    
        BCMDNX_IF_ERR_EXIT(rc);
    }

    cpu_port_exist = SOC_PBMP_NOT_NULL(PBMP_CMIC(unit));
...
int _bcm_dpp_am_template_trap_egress_data_get(int unit,
                                              SOC_PPD_TRAP_EG_TYPE trap_id, /* Enum trap */
                                              uint32 egress_trap_entry, /* Egress Trap or Field Entry */                                              
                                              SOC_SAND_OUT SOC_PPD_TRAP_EG_ACTION_PROFILE_INFO *data)
{
    int rc = BCM_E_NONE;
    ...
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID ,dpp_am_template_trap_egress, old_profile, data);
    BCMDNX_IF_ERR_EXIT(rc);
                                          ...
                                          int *template)
{
    int isAlloc;
    BCMDNX_INIT_FUNC_DEFS;
    BCMDNX_IF_ERR_EXIT(dpp_am_template_allocate(unit, BCM_DPP_AM_DEFAULT_CORE_ID,
...
_bcm_dpp_am_template_trap_egress_free(int unit,
                                      int template)
{
    SOC_PPD_TRAP_EG_ACTION_PROFILE_INFO defaults;
    int isLast;
    BCMDNX_INIT_FUNC_DEFS;
    BCMDNX_IF_ERR_EXIT(dpp_am_template_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID,
                                      dpp_am_template_trap_egress,
                                      template,
                                      &isLast));
    if (isLast) {
        SOC_PPD_TRAP_EG_ACTION_PROFILE_INFO_clear(&defaults);
        if (!SOC_WARM_BOOT(unit)) {        
            BCM_SAND_IF_ERR_EXIT(soc_ppd_trap_eg_profile_info_set((unit),
                                                                  template,
                                                                  &defaults));
...
int _bcm_dpp_am_template_trap_egress_exchange(int unit, int flags, 
                                              SOC_PPD_TRAP_EG_TYPE trap_id, /* Enum trap */
                                              uint32 egress_trap_entry, /* Egress Trap or Field Entry */
                                              SOC_SAND_IN SOC_PPD_TRAP_EG_ACTION_PROFILE_INFO *data, int *old_template, int *is_last, int *template,int *is_allocated)
{
    int rc = BCM_E_NONE;
    int temp_is_last;
    ...
    rc = dpp_am_template_exchange(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_trap_egress,flags,data,temp,&temp_is_last,template,is_allocated);
    BCMDNX_IF_ERR_EXIT(rc);
...
{  
    ...
    else if (egress_trap_entry != SOC_PPD_TRAP_EG_TYPE_NONE)
    {
    ...
    if (*profile == SOC_PPD_TRAP_EG_NO_ACTION) {
        /* Set BCM NO ACTION */
        *profile = _BCM_DPP_TRAP_EG_NO_ACTION;
    }
...
int _bcm_dpp_am_template_l2_trap_rm_init(int unit,int template_init_id,bcm_dpp_user_defined_traps_t* init_data)
{
    ...
    for (index = 0; index < _DPP_AM_TEMPLATE_L2_TRAP_RSRVD_MC_MAX_ENTITIES(unit); index++) {
        result = dpp_am_template_allocate(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_trap_reserved_mc, flags, init_data, &is_allocated,&template);
        BCMDNX_IF_ERR_EXIT(result);
    }
...
_bcm_dpp_am_template_l2_trap_rm_port_profile_mapping_get(int unit, int port, int mac_lsb, int *trap_code)
{
    ...
    SOC_PPD_ACTION_PROFILE
        action_profile;
    SOC_PPD_LLP_TRAP_RESERVED_MC_KEY rm_mac_key;
    uint32  pp_port;
    int     core;
    ...
    SOC_PPD_LLP_TRAP_RESERVED_MC_KEY_clear(&rm_mac_key);
    ...
    {
        *trap_code = action_profile.trap_code - SOC_PPD_TRAP_CODE_RESERVED_MC_0 + 1;
    }
...
int _bcm_dpp_am_template_l2_trap_rm_data_get(int unit, int port, int mac_lsb, bcm_dpp_user_defined_traps_t * data)
{
    ...
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_trap_reserved_mc,old_profile,data);
...
{
    int rc = BCM_E_NONE;
    int trap_id, new_trap_code, virtual_trap_id;
    bcm_dpp_rx_virtual_traps_t new_virtual_traps_p;

    BCMDNX_INIT_FUNC_DEFS;
    _bcm_dpp_am_template_l2_trap_rm_port_profile_mapping_get(unit,port,mac_lsb, old_template);
    BCMDNX_IF_ERR_EXIT(rc);

    if(!(flags & BCM_DPP_AM_TEMPLATE_FLAG_CHECK)) {
        rc = dpp_am_template_exchange(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_trap_reserved_mc,SHR_TEMPLATE_MANAGE_IGNORE_NOT_EXIST_OLD_TEMPLATE,data,*old_template,is_last,template,is_allocated);
        BCMDNX_IF_ERR_EXIT(rc);   

    
        /*Sync rx.c traps allocations with l2.c traps allocations (templates) for virtual traps*/
        /*PPD trap code*/
        trap_id = *template;

        if (trap_id != 0) {
            new_trap_code = (*template-1) + SOC_PPD_TRAP_CODE_RESERVED_MC_0;

            if (virtual_traps_p == NULL) 
            {
                if (*is_allocated) {
                    /*is_allocated means first time in use, there for allocte only the first time
                       alloction without ID*/
                    
                    rc = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_trap_virtual,0,1,&virtual_trap_id);
                    BCMDNX_IF_ERR_EXIT(rc);


                    /*update _rx_virtual_traps array with PPD prog virtual trap - get from sw db and save to a local copy*/            
                    ...
                    new_virtual_traps_p.soc_ppd_reserved_mc = new_trap_code;

                    /* write the local copy back to sw_db */
                ...
                /*assumes allocation was done becuse gport exist 
                  there for only update PPD trap code*/
                virtual_traps_p->soc_ppd_reserved_mc = new_trap_code;

                /* virtual_traps_p should be wriiten back to sw_db by the calling function */
            }
        }

        if ((*is_last) && (trap_id != 0)) {
            /* Is_last means not in use anymore, there for free allocation*/
            
            /*Scan virtual array. Free relevant virtual trap. Reset Hw configuration.*/
    ...
    else
    {
        ...
        rc = dpp_am_template_exchange_test(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_trap_reserved_mc,SHR_TEMPLATE_MANAGE_IGNORE_NOT_EXIST_OLD_TEMPLATE,data,*old_template,is_last,template,is_allocated);
        BCMDNX_IF_ERR_EXIT(rc);

        /* Check if we'll need a virtual trap */
        if ( (*template != 0) && (virtual_traps_p == NULL) && (*is_allocated) ){
            rc = dpp_am_res_check_all(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_trap_virtual,_BCM_NUMBER_OF_VIRTUAL_TRAPS,0);
            if (rc == BCM_E_EXISTS || rc == BCM_E_EMPTY) {
                rc = BCM_E_NONE;
            }
            BCMDNX_IF_ERR_EXIT(rc);
        }
...
{
    BCMDNX_INIT_FUNC_DEFS;
    BCMDNX_IF_ERR_EXIT(dpp_am_template_clear(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_trap_reserved_mc));
exit:
    BCMDNX_FUNC_RETURN;
}
...
int _bcm_dpp_am_template_vsi_egress_profile_init(int unit, bcm_dpp_vsi_egress_profile_t* init_data)
{
    ...
    for (index = 0; index < _DPP_AM_TEMPLATE_EGRESS_VSI_PROFILE_MAX_ENTITIES(unit); index++) {
        result = dpp_am_template_allocate(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_vsi_egress_profile, flags, init_data, &is_allocated, &template);
        BCMDNX_IF_ERR_EXIT(result);
    }
...
int _bcm_dpp_am_template_vsi_egress_profile_data_get(int unit, int vsi, bcm_dpp_vsi_egress_profile_t * data)
{
    ...
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_vsi_egress_profile, old_profile, data);
                                             ...
                                             int *old_template, int *is_last, int *template, int *is_allocated)
{
    ...
    rc = dpp_am_template_exchange(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_vsi_egress_profile, SHR_TEMPLATE_MANAGE_IGNORE_NOT_EXIST_OLD_TEMPLATE,
...
int _bcm_dpp_am_template_vsi_ingress_profile_init(int unit, bcm_dpp_vsi_ingress_profile_t* init_data)
{
    ...
    for (index = 0; index < _DPP_AM_TEMPLATE_INGRESS_VSI_PROFILE_MAX_ENTITIES(unit); index++) {
        result = dpp_am_template_allocate(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_vsi_ingress_profile, flags, init_data, &is_allocated, &template);
        BCMDNX_IF_ERR_EXIT(result);
    }
...
_bcm_dpp_am_template_vsi_ingress_profile_mapping_get(int unit, int vsi, int *profile)
{
    ...
    soc_sand_rv = soc_ppd_vsi_info_get(soc_sand_dev_id, vsi, &vsi_info);
    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    *profile = vsi_info.profile_ingress & 0x3; /* only 2 lsbs are rellevant */
...
int _bcm_dpp_am_template_vsi_ingress_profile_data_get(int unit, int vsi, bcm_dpp_vsi_ingress_profile_t * data)
{
    ...
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_vsi_ingress_profile, old_profile, data);
                                             ...
                                             int *old_template, int *is_last, int *template, int *is_allocated)
{
    ...
    rc = dpp_am_template_exchange(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_vsi_ingress_profile, SHR_TEMPLATE_MANAGE_IGNORE_NOT_EXIST_OLD_TEMPLATE,
                               data, *old_template, is_last, template, is_allocated);
    BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_oam_icc_map_ndx_data_get(int unit, int mep_id, SOC_PPD_OAM_ICC_MAP_DATA* data)
{
    int old_icc_ndx;
    int rc = BCM_E_NONE;
    ...
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_oam_icc_map, old_icc_ndx, data);
...
int _bcm_dpp_am_template_oam_icc_map_ndx_exchange(int unit, int flags, int mep_id, SOC_PPD_OAM_ICC_MAP_DATA * data,
                                             int *old_icc_ndx, int *is_last, int *icc_ndx, int *is_allocated)
{
    ...
    rc = dpp_am_template_exchange(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_oam_icc_map, SHR_TEMPLATE_MANAGE_IGNORE_NOT_EXIST_OLD_TEMPLATE,
...
int
_bcm_dpp_am_template_oam_icc_map_ndx_alloc(int unit, int flags, SOC_PPD_OAM_ICC_MAP_DATA *data, int *is_allocated, int *icc_ndx)
{
    int rc = BCM_E_NONE;

    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_allocate(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_oam_icc_map, flags, data, is_allocated, icc_ndx);
    BCMDNX_IF_ERR_EXIT(rc);
exit:
    BCMDNX_FUNC_RETURN;
}
...
_bcm_dpp_am_template_oam_icc_map_ndx_free(int unit, int icc_ndx, int *is_last)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_oam_icc_map, icc_ndx, is_last);
    BCMDNX_IF_ERR_EXIT(rc);
exit:
    BCMDNX_FUNC_RETURN;
}
...
_bcm_dpp_am_template_oam_sa_mac_address_alloc(int unit, int flags, uint8 *data, int *is_allocated, int *profile)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_allocate(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_oam_sa_mac, flags, data, is_allocated, profile);
    BCMDNX_IF_ERR_EXIT(rc);
exit:
    BCMDNX_FUNC_RETURN;
}
...
_bcm_dpp_am_template_oam_sa_mac_address_free(int unit, int profile, int *is_last)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_oam_sa_mac, profile, is_last);
    BCMDNX_IF_ERR_EXIT(rc);
exit:
    BCMDNX_FUNC_RETURN;
}
...
int _bcm_dpp_am_template_oam_sa_mac_address_get(int unit, int profile, uint8* data)
{
    ...
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_oam_sa_mac, profile, data);
    BCMDNX_IF_ERR_EXIT(rc);
...
#define _DPP_ALLOC_MNGR_OAM_PUNT_EVENT_HENDLING_PROFILES_MATCH(profile_1, profile_2) \
...
int _bcm_dpp_am_template_oam_punt_event_hendling_profile_alloc_internal(int unit, int flags, SOC_PPD_OAM_OAMP_PUNT_PROFILE_DATA *data, int *is_allocated, int *profile) {
    int rc = BCM_E_NONE;

    BCMDNX_INIT_FUNC_DEFS;

    rc = dpp_am_template_allocate(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_oam_punt_event_hendling_profile, flags, data, is_allocated, profile);
    BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_oam_punt_event_hendling_profile_alloc(int unit, int flags, SOC_PPD_OAM_OAMP_PUNT_PROFILE_DATA *data, int *is_allocated, int *profile)
{
    int rc = BCM_E_NONE;
...
/* profile 0 must remain all zeros.*/
    if (data->punt_rate || data->punt_enable || data->rx_state_update_enable || data->scan_state_update_enable || data->mep_rdi_update_loc_enable 
        ...
        int i;
        SOC_PPD_OAM_OAMP_PUNT_PROFILE_DATA existing_profile;
        for (i=1 ; i<_DPP_AM_TEMPLATE_OAM_OAMP_PUNT_PROFILE_COUNT ; ++i) {
            rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_oam_punt_event_hendling_profile, i ,&existing_profile );
            if (rc == BCM_E_NONE) {
                /* check if the existing profile matches the given profile. If so get outta here*/
                if (_DPP_ALLOC_MNGR_OAM_PUNT_EVENT_HENDLING_PROFILES_MATCH((*data), existing_profile)) {
                    *profile = i;
                    return _bcm_dpp_am_template_oam_punt_event_hendling_profile_alloc_internal(unit,SHR_RES_ALLOC_WITH_ID,data,is_allocated,profile);
                }
            }
        }
...
int _bcm_dpp_am_template_oam_punt_event_hendling_profile_free(int unit, int profile, int *is_last)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_oam_punt_event_hendling_profile, profile, is_last);
    BCMDNX_IF_ERR_EXIT(rc);
exit:
    BCMDNX_FUNC_RETURN;
}
...
int
_bcm_dpp_am_template_oam_mep_profile_alloc(int unit, _dpp_am_template_t template_type, int flags, SOC_PPD_OAM_LIF_PROFILE_DATA *data, int *is_allocated, int *profile)
{
    int rc = BCM_E_NONE;

    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_allocate(unit, BCM_DPP_AM_DEFAULT_CORE_ID, template_type, flags, data, is_allocated, profile);
    BCMDNX_IF_ERR_EXIT(rc);
...
_bcm_dpp_am_template_oam_mep_profile_free(int unit, _dpp_am_template_t template_type, int profile, int *is_last)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID, template_type, profile, is_last);
    BCMDNX_IF_ERR_EXIT(rc);
exit:
    BCMDNX_FUNC_RETURN;
}

int _bcm_dpp_am_template_oam_mep_profile_data_from_profile_get(int unit, _dpp_am_template_t template_type, int profile, SOC_PPD_OAM_LIF_PROFILE_DATA* data)
{
    int rc = BCM_E_NONE;

    BCMDNX_INIT_FUNC_DEFS;

    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, template_type, profile, data);
...
int _bcm_dpp_am_template_oam_mep_profile_data_get(int unit, int mep_id, _dpp_am_template_t template_type, int is_accelerated, int is_passive, SOC_PPD_OAM_LIF_PROFILE_DATA* data)
{
    int old_profile;
    int rc = BCM_E_NONE;
    ...
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, template_type, old_profile, data);
...
int _bcm_dpp_am_template_oam_mep_profile_exchange(int unit, _dpp_am_template_t template_type, int is_accelerated, int is_passive, int flags, int mep_id, SOC_PPD_OAM_LIF_PROFILE_DATA * data,
                                             int *old_profile, int *is_last, int *profile, int *is_allocated)
{
    ...
    rc = dpp_am_template_exchange(unit, BCM_DPP_AM_DEFAULT_CORE_ID, template_type, SHR_TEMPLATE_MANAGE_IGNORE_NOT_EXIST_OLD_TEMPLATE,
...
int
_bcm_dpp_am_template_oam_mep_profile_accelerated_alloc(int unit, int flags, SOC_PPD_OAM_LIF_PROFILE_DATA *data, int *is_allocated, int *profile)
{
    int rc = BCM_E_NONE;
...
int _bcm_dpp_am_template_oam_mep_profile_accelerated_data_get(int unit, int mep_id, SOC_PPD_OAM_LIF_PROFILE_DATA* data)
{
    int rc = BCM_E_NONE;
...
int _bcm_dpp_am_template_oam_mep_profile_accelerated_exchange(int unit, int flags, int mep_id, SOC_PPD_OAM_LIF_PROFILE_DATA * data,
                                             int *old_profile, int *is_last, int *profile, int *is_allocated)
{
    int rc = BCM_E_NONE;
...
int
_bcm_dpp_am_template_oam_mep_profile_non_accelerated_alloc(int unit, int flags, SOC_PPD_OAM_LIF_PROFILE_DATA *data, int *is_allocated, int *profile)
{
    int rc = BCM_E_NONE;
...
int _bcm_dpp_am_template_oam_mep_profile_non_accelerated_data_from_profile_get(int unit, int profile, SOC_PPD_OAM_LIF_PROFILE_DATA* data)
{
    int rc = BCM_E_NONE;
...
int _bcm_dpp_am_template_oam_mep_profile_non_accelerated_data_get(int unit, int mep_id, uint8 is_passive, SOC_PPD_OAM_LIF_PROFILE_DATA* data)
{
    int rc = BCM_E_NONE;
...
int _bcm_dpp_am_template_oam_mep_profile_non_accelerated_exchange(int unit, int flags, int mep_id, int is_passive, SOC_PPD_OAM_LIF_PROFILE_DATA * data,
                                             int *old_profile, int *is_last, int *profile, int *is_allocated)
{
    int rc = BCM_E_NONE;
...
int
_bcm_dpp_am_template_bfd_mep_profile_accelerated_alloc(int unit, int flags, SOC_PPD_OAM_LIF_PROFILE_DATA *data, int *is_allocated, int *profile)
{
    int rc = BCM_E_NONE;
...
int _bcm_dpp_am_template_bfd_mep_profile_accelerated_data_get(int unit, int mep_id, SOC_PPD_OAM_LIF_PROFILE_DATA* data)
{
    int rc = BCM_E_NONE;
...
int _bcm_dpp_am_template_bfd_mep_profile_accelerated_exchange(int unit, int flags, int mep_id, SOC_PPD_OAM_LIF_PROFILE_DATA * data,
                                             int *old_profile, int *is_last, int *profile, int *is_allocated)
{
    int rc = BCM_E_NONE;
...
int
_bcm_dpp_am_template_bfd_mep_profile_non_accelerated_alloc(int unit, int flags, SOC_PPD_OAM_LIF_PROFILE_DATA *data, int *is_allocated, int *profile)
{
    int rc = BCM_E_NONE;
...
int _bcm_dpp_am_template_bfd_mep_profile_non_accelerated_data_get(int unit, int mep_id, SOC_PPD_OAM_LIF_PROFILE_DATA* data)
{
    int rc = BCM_E_NONE;
...
int _bcm_dpp_am_template_bfd_mep_profile_non_accelerated_exchange(int unit, int flags, int mep_id, SOC_PPD_OAM_LIF_PROFILE_DATA * data,
                                             int *old_profile, int *is_last, int *profile, int *is_allocated)
{
    int rc = BCM_E_NONE;
...
/* OAM Priority - Start */
int _bcm_dpp_am_template_oam_tx_priority_alloc(int unit, int flags, SOC_PPD_OAMP_TX_ITMH_ATTRIBUTES *data, int *is_allocated, int *profile_ndx)
{
    int rc = BCM_E_NONE;

    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_allocate(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_oam_tx_priority, flags, data, is_allocated, profile_ndx);
    BCMDNX_IF_ERR_EXIT(rc);
exit:
    BCMDNX_FUNC_RETURN;
}
...
int _bcm_dpp_am_template_oam_tx_priority_free(int unit, int profile_ndx, int *is_last)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_oam_tx_priority, profile_ndx, is_last);
    BCMDNX_IF_ERR_EXIT(rc);
exit:
    BCMDNX_FUNC_RETURN;
}

int _bcm_dpp_am_template_oam_tx_priority_data_get(int unit, int profile_ndx, SOC_PPD_OAMP_TX_ITMH_ATTRIBUTES* data)
{
    int rc = BCM_E_NONE;

    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_oam_tx_priority, profile_ndx, data);
...
/* BFD IPV4 DIP - Start */
int _bcm_dpp_am_template_bfd_ipv4_dip_alloc(int unit, int flags,const uint32 *data, int *is_allocated, int *profile_ndx)
{
    int rc = BCM_E_NONE;

    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_allocate(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_bfd_ipv4_dip, flags, data, is_allocated, profile_ndx);
    BCMDNX_IF_ERR_EXIT(rc);
exit:
    BCMDNX_FUNC_RETURN;
}

int _bcm_dpp_am_template_bfd_ipv4_dip_free(int unit, int profile_ndx, int *is_last)
{
    int rc = BCM_E_NONE;

    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_bfd_ipv4_dip, profile_ndx, is_last);
    BCMDNX_IF_ERR_EXIT(rc);
exit:
    BCMDNX_FUNC_RETURN;
}

int _bcm_dpp_am_template_bfd_ipv4_dip_data_get(int unit, int profile_ndx, uint32* data)
{
    int rc = BCM_E_NONE;

    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_bfd_ipv4_dip, profile_ndx, data);
...
int _bcm_dpp_am_template_mpls_pwe_push_profile_alloc(int unit, int flags, SOC_PPD_MPLS_PWE_PROFILE_DATA *data, int *is_allocated, int *mpls_pwe_profile)
{
    int rc = BCM_E_NONE;

    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_allocate(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_mpls_pwe_push_profile, flags, data, is_allocated, mpls_pwe_profile);
    BCMDNX_IF_ERR_EXIT(rc);
exit:
    BCMDNX_FUNC_RETURN;
}

int _bcm_dpp_am_template_mpls_pwe_push_profile_data_get(int unit, int mpls_pwe_profile, SOC_PPD_MPLS_PWE_PROFILE_DATA* data)
{
    int rc = BCM_E_NONE;

    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_mpls_pwe_push_profile, mpls_pwe_profile, data);
...
int _bcm_dpp_am_template_mpls_pwe_push_profile_free(int unit, int mpls_pwe_profile, int *is_last)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_mpls_pwe_push_profile, mpls_pwe_profile, is_last);
    BCMDNX_IF_ERR_EXIT(rc);
exit:
    BCMDNX_FUNC_RETURN;
}
...
int _bcm_dpp_am_template_bfd_req_interval_pointer_alloc(int unit, int flags, uint32 *data, int *is_allocated, int *rx_rate_pointer)
{
    int rc = BCM_E_NONE;

    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_allocate(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_bfd_req_interval_pointer, flags, data, is_allocated, rx_rate_pointer);
    BCMDNX_IF_ERR_EXIT(rc);
exit:
    BCMDNX_FUNC_RETURN;
}
...
int _bcm_dpp_am_template_bfd_req_interval_pointer_data_get(int unit, int rx_rate_pointer, uint32* data)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_bfd_req_interval_pointer, rx_rate_pointer, data);
...
int _bcm_dpp_am_template_bfd_req_interval_pointer_free(int unit, int rx_rate_pointer, int *is_last)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_bfd_req_interval_pointer, rx_rate_pointer, is_last);
    BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_bfd_tos_ttl_profile_alloc(int unit, int flags, SOC_PPD_BFD_IP_MULTI_HOP_TOS_TTL_DATA *data, int *is_allocated, int *tos_ttl_profile)
{
    int rc = BCM_E_NONE;

    BCMDNX_INIT_FUNC_DEFS;

    rc = dpp_am_template_allocate(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_bfd_tos_ttl_profile, flags, data, is_allocated, tos_ttl_profile);
    BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_bfd_tos_ttl_profile_data_get(int unit, int tos_ttl_profile, SOC_PPD_BFD_IP_MULTI_HOP_TOS_TTL_DATA* data)
{
    int rc = BCM_E_NONE;

    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_bfd_tos_ttl_profile, tos_ttl_profile, data);
...
int _bcm_dpp_am_template_bfd_tos_ttl_profile_free(int unit, int tos_ttl_profile, int *is_last)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_bfd_tos_ttl_profile, tos_ttl_profile, is_last);
    BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_bfd_src_ip_profile_alloc(int unit, int flags, uint32 *data, int *is_allocated, int *src_ip_add_profile)
{
    int rc = BCM_E_NONE;

    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_allocate(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_bfd_src_ip_profile, flags, data, is_allocated, src_ip_add_profile);
    BCMDNX_IF_ERR_EXIT(rc);
exit:
    BCMDNX_FUNC_RETURN;
}
...
int _bcm_dpp_am_template_bfd_src_ip_profile_data_get(int unit, int src_ip_add_profile, uint32* data)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_bfd_src_ip_profile, src_ip_add_profile, data);
...
int _bcm_dpp_am_template_bfd_src_ip_profile_free(int unit, int src_ip_add_profile, int *is_last)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_bfd_src_ip_profile, src_ip_add_profile, is_last);
    BCMDNX_IF_ERR_EXIT(rc);
...
/* BFD tx rate profile - Start */
int _bcm_dpp_am_template_bfd_tx_rate_profile_alloc(int unit, int flags, uint32 *data, int *is_allocated, int *tx_rate_profile)
{
    int rc = BCM_E_NONE;

    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_allocate(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_bfd_tx_rate_profile, flags, data, is_allocated, tx_rate_profile);
    BCMDNX_IF_ERR_EXIT(rc);
exit:
    BCMDNX_FUNC_RETURN;
}

int _bcm_dpp_am_template_bfd_tx_rate_profile_data_get(int unit, int tx_rate_profile, uint32* data)
{
    int rc = BCM_E_NONE;

    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_bfd_tx_rate_profile, tx_rate_profile, data);
...
int _bcm_dpp_am_template_bfd_tx_rate_profile_free(int unit, int tx_rate_profile, int *is_last)
{
    int rc = BCM_E_NONE;

    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_bfd_tx_rate_profile, tx_rate_profile, is_last);
    BCMDNX_IF_ERR_EXIT(rc);

exit:
    BCMDNX_FUNC_RETURN;
}
/* BFD tx rate profile - End */


/* BFD flags profile*/
int _bcm_dpp_am_template_bfd_flags_profile_alloc(int unit, int flags, uint32 *data, int *is_allocated, int *profile)
{
    int rc = BCM_E_NONE;

    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_allocate(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_bfd_flags_profile, flags, data, is_allocated,profile);
    BCMDNX_IF_ERR_EXIT(rc);
exit:
    BCMDNX_FUNC_RETURN;
}
...
int _bcm_dpp_am_template_bfd_flags_profile_data_get(int unit, int profile, uint32* data)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_bfd_flags_profile,profile, data);
...
int _bcm_dpp_am_template_bfd_flags_profile_free(int unit, int profile, int *is_last)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_bfd_flags_profile, profile, is_last);
    BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_oam_lmm_nic_tables_alloc(int unit, int flags, uint32* data, int *is_allocated, int *profile_ndx)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_allocate(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_oam_lmm_nic_tables_profile, flags, data, is_allocated, profile_ndx);
    BCMDNX_IF_ERR_EXIT(rc);
exit:
    BCMDNX_FUNC_RETURN;
}
...
int _bcm_dpp_am_template_oam_lmm_nic_tables_free(int unit, int mep_id, int *profile_ndx, int *is_last)
{
    ...
    *profile_ndx = profile;
    ...
    rc = dpp_am_template_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_oam_lmm_nic_tables_profile, *profile_ndx, is_last);
    BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_oam_lmm_nic_tables_get(int unit, int mep_id, int *profile_ndx, uint32* data)
{
    ...
    *profile_ndx = profile; 
    ...
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_oam_lmm_nic_tables_profile, *profile_ndx, data);
...
int _bcm_dpp_am_template_oam_lmm_oui_tables_alloc(int unit, int flags, uint32* data, int *is_allocated, int *profile_ndx)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_allocate(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_oam_lmm_oui_tables_profile, flags, data, is_allocated, profile_ndx);
    BCMDNX_IF_ERR_EXIT(rc);
exit:
    BCMDNX_FUNC_RETURN;
}
...
int _bcm_dpp_am_template_oam_lmm_oui_tables_free(int unit, int mep_id, int *profile_ndx, int *is_last)
{
    ...
    *profile_ndx = profile;
    ...
    rc = dpp_am_template_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_oam_lmm_oui_tables_profile, *profile_ndx, is_last);
    BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_oam_lmm_oui_tables_get(int unit, int mep_id, int *profile_ndx, uint32* data)
{
    ...
    *profile_ndx = profile;
    ...
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_oam_lmm_oui_tables_profile, *profile_ndx, data);
...
int _bcm_dpp_am_template_oam_local_port_2_sys_port_data_get_internal(int unit, int local_port, _dpp_oam_am_local_port_2_sys_port_internal_data_t *data)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_oam_local_port_2_system_port, local_port, data);
    if (rc == BCM_E_NOT_FOUND) {
...
int _bcm_dpp_am_template_oam_local_port_2_sys_port_alloc(int unit, int flags, const uint32* data, int *is_allocated, int *profile_ndx)
{
    ...
    rc = dpp_am_template_allocate(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_oam_local_port_2_system_port, flags, &internal_data, is_allocated, profile_ndx);
    if (rc!=BCM_E_NONE) {
        BCMDNX_IF_ERR_EXIT(rc); 
    }
exit:
...
int _bcm_dpp_am_template_oam_local_port_2_sys_port_free(int unit, int mep_id, int *is_last)
{
    int rc = BCM_E_NONE;
    SOC_PPD_OAM_OAMP_MEP_DB_ENTRY  mep_db_entry;
    uint32 soc_sand_rv;
    ...
    rc = dpp_am_template_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_oam_local_port_2_system_port, mep_db_entry.local_port, is_last);
    BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_oam_oamp_pe_gen_mem_data_get_internal(int unit, int gen_mem_data, _dpp_oam_oamp_pe_gen_mem_internal_data_t *data)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_oam_oamp_pe_gen_mem, gen_mem_data, data);
    if (rc == BCM_E_NOT_FOUND) {
        BCM_ERR_EXIT_NO_MSG(BCM_E_NOT_FOUND);
    }
    BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_oam_oamp_pe_gen_mem_alloc(int unit, int flags, const uint32* data, int *is_allocated, int *profile_ndx)
{
    ...
    rc = dpp_am_template_allocate(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_oam_oamp_pe_gen_mem, flags, &internal_data, is_allocated, profile_ndx);
    if (rc!=BCM_E_NONE) {
        BCMDNX_IF_ERR_EXIT(rc); 
    }
exit:
...
int _bcm_dpp_am_template_oam_oamp_pe_gen_mem_free(int unit, int mep_id, int *is_last)
{
    ...
    rc = dpp_am_template_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_oam_oamp_pe_gen_mem, mep_db_entry.local_port, is_last);
    BCMDNX_IF_ERR_EXIT(rc);
...
/** Always first  (for comilation)   */
int _bcm_dpp_am_template_oam_eth1731_mep_profile_alloc(int unit, SOC_PPD_OAM_ETH1731_MEP_PROFILE_ENTRY* data,int flags, int *is_allocated, int *profile_ndx)
{
    int rc = BCM_E_NONE;

    BCMDNX_INIT_FUNC_DEFS;

    rc = dpp_am_template_allocate(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_oam_eth1731_profile, flags, data, is_allocated, profile_ndx);
    BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_oam_eth1731_mep_profile_alloc_with_id(int unit, int profile_ndx)
{
    int rc = BCM_E_NONE;
    int who_cares;
    SOC_PPD_OAM_ETH1731_MEP_PROFILE_ENTRY data;

    BCMDNX_INIT_FUNC_DEFS;

    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_oam_eth1731_profile, profile_ndx, &data);
    BCMDNX_IF_ERR_EXIT(rc);

     return _bcm_dpp_am_template_oam_eth1731_mep_profile_alloc(unit, &data, SHR_RES_ALLOC_WITH_ID, &who_cares, &profile_ndx);
exit:
    BCMDNX_FUNC_RETURN;
}
...
int _bcm_dpp_am_template_oam_eth_1731_mep_profile_RDI_alloc(int unit,SOC_PPD_OAM_ETH1731_MEP_PROFILE_ENTRY *data , int * is_allocated, int * profile_ndx)
{
    int rc,profile;
    SOC_PPD_OAM_ETH1731_MEP_PROFILE_ENTRY  old_eth1731_profile;

    /* First, search for an existing entry with the same RDI. */
    for (profile =0 ; profile< _DPP_AM_TEMPLATE_OAM_ETH1731_PROFILE_COUNT ; ++profile) {
        rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_oam_eth1731_profile, profile, &old_eth1731_profile);
        if (rc == BCM_E_NONE && old_eth1731_profile.rdi_gen_method == data->rdi_gen_method && old_eth1731_profile.piggy_back_lm==0 && !old_eth1731_profile.report_mode
            && !old_eth1731_profile.slm_lm) {
...
int _bcm_dpp_am_template_oam_eth_1731_mep_profile_alloc_by_component( int unit, int mep_id,SOC_PPD_OAM_ETH1731_MEP_PROFILE_ENTRY *data , int *is_allocated, int *profile_ndx, 
                                                             int *old_profile, int *is_last, int is_first , int one_lm_two_dm_three_loopback )
{
    int rc,profile;
    SOC_PPD_OAM_ETH1731_MEP_PROFILE_ENTRY old_eth1731_profile;
                ...
                if (one_lm_two_dm_three_loopback == 1 && old_eth1731_profile.lmm_rate == data->lmm_rate &&
                    ...
                    *is_allocated = *is_last =0;
                    if (*old_profile != *profile_ndx) {
                        rc = dpp_am_template_exchange(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_oam_eth1731_profile, 
...
int _bcm_dpp_am_template_oam_eth_1731_mep_profile_loss_alloc(int unit, int mep_id,SOC_PPD_OAM_ETH1731_MEP_PROFILE_ENTRY *data , int *is_allocated, int *profile_ndx, 
                                                             int *old_profile, int *is_last, int is_first )
{

    return _bcm_dpp_am_template_oam_eth_1731_mep_profile_alloc_by_component(unit, mep_id, data,is_allocated, profile_ndx, old_profile, is_last,is_first,1 /* one for lm*/ );
}

int _bcm_dpp_am_template_oam_eth_1731_mep_profile_delay_alloc(int unit, int mep_id ,SOC_PPD_OAM_ETH1731_MEP_PROFILE_ENTRY *data, int *is_allocated, int *profile_ndx, 
                                                             int *old_profile, int *is_last, int is_first )
{
    return _bcm_dpp_am_template_oam_eth_1731_mep_profile_alloc_by_component(unit, mep_id, data, is_allocated, profile_ndx, old_profile, is_last,is_first ,2 /* two for dm*/);
}
int _bcm_oam_am_template_oam_eth_1731_mep_profile_loopback_alloc(int unit, int mep_id ,SOC_PPD_OAM_ETH1731_MEP_PROFILE_ENTRY *data, int *is_allocated, int *profile_ndx, 
                                                             int *old_profile, int *is_last , int is_first)
{
    return _bcm_dpp_am_template_oam_eth_1731_mep_profile_alloc_by_component(unit, mep_id, data, is_allocated, profile_ndx, old_profile, is_last, is_first,3 /* three for LB*/);
}
...
int _bcm_dpp_am_template_oam_eth1731_mep_profile_free_internal(int unit, int profile_ndx, int * is_last)
{
    ...
    rc = dpp_am_template_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_oam_eth1731_profile, profile_ndx, is_last);
    BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_oam_eth1731_mep_profile_get(int unit ,int mep_id, int* profile_ndx, SOC_PPD_OAM_ETH1731_MEP_PROFILE_ENTRY* data)
{
    int rc = BCM_E_NONE;
    uint32 soc_sand_rv;
    uint32 dont_care, eth_profile;
...
int _bcm_dpp_am_template_oam_eth1731_mep_profile_exchange(int unit, int old_profile_indx,SOC_PPD_OAM_ETH1731_MEP_PROFILE_ENTRY* data, int *is_allocated,int *is_last, int *profile_ndx )
{
    int rc = 0;
    BCMDNX_INIT_FUNC_DEFS;

    rc = dpp_am_template_exchange(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_oam_eth1731_profile, SHR_TEMPLATE_MANAGE_IGNORE_NOT_EXIST_OLD_TEMPLATE,
...
int _bcm_dpp_am_template_l2_prog_trap_init(int unit,int template_init_id,SOC_PPD_LLP_TRAP_PROG_TRAP_QUALIFIER *init_data)
{
    int is_allocated, result = BCM_E_NONE;
    uint32 flags = 0;
    int template;
    ...
    /* Add all entities to template init id with given data */
    result = dpp_am_template_allocate_group(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_prog_trap, flags, init_data,_DPP_AM_TEMPLATE_PROG_TRAP_MAX_ENTITIES(unit), &is_allocated,&template);
    BCMDNX_IF_ERR_EXIT(result);
...
_bcm_dpp_am_template_l2_prog_trap_port_profile_mapping_get(int unit, int port, int type, int *profile)
{
    ...
    trap_bit = SOC_PPD_LLP_TRAP_PORT_ENABLE_PROG_TRAP_0 << type;
...
int _bcm_dpp_am_template_l2_prog_trap_data_get(int unit, int port, int type, SOC_PPD_LLP_TRAP_PROG_TRAP_QUALIFIER *qual, bcm_dpp_user_defined_traps_t *trap_res)
{
    bcm_dpp_prog_trap_t
        trap_t;
    int old_profile=0;
    ...
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_prog_trap,old_profile,&trap_t);
    BCMDNX_IF_ERR_EXIT(rc);

    sal_memcpy(qual, &trap_t.qual, sizeof(SOC_PPD_LLP_TRAP_PROG_TRAP_QUALIFIER));
...
int _bcm_dpp_am_template_l2_prog_trap_alloc(int unit, int flags, SOC_PPD_LLP_TRAP_PROG_TRAP_QUALIFIER *qual,
                                             bcm_dpp_user_defined_traps_t *trap_res, int *template,int *is_allocated,
                                             bcm_dpp_rx_virtual_traps_t *virtual_traps_p,
                                             int is_virtual_traps_p_set)
{
    bcm_dpp_prog_trap_t trap_t;
    int rc = BCM_E_NONE,virtual_trap_id,new_trap_code;
    bcm_dpp_rx_virtual_traps_t new_virtual_traps_p;

    BCMDNX_INIT_FUNC_DEFS;
    sal_memcpy(&trap_t.qual, qual, sizeof(SOC_PPD_LLP_TRAP_PROG_TRAP_QUALIFIER));
    sal_memcpy(&trap_t.result, trap_res, sizeof(bcm_dpp_user_defined_traps_t));
        
    rc = dpp_am_template_allocate(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_prog_trap,flags,&trap_t,is_allocated,template);
    BCMDNX_IF_ERR_EXIT(rc);

    /*PPD trap code*/
    new_trap_code = *template + SOC_PPD_TRAP_CODE_PROG_TRAP_0;
    ...
    if (is_virtual_traps_p_set == FALSE) 
    {
        ...
        if (*is_allocated) {
            ...
            new_virtual_traps_p.soc_ppd_prog = new_trap_code;
            if (_bcm_dpp_rx_virtual_trap_set(unit,virtual_trap_id,&new_virtual_traps_p) != BCM_E_NONE)
            {
        ...
        /* assumes allocation was done becuse gport exist 
           there for only update PPD trap code*/
        virtual_traps_p->soc_ppd_prog = new_trap_code;
...
int _bcm_dpp_am_template_l2_prog_trap_free(int unit, int is_virtual_traps_p_set, int type, int *template,int *is_last)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_prog_trap,type,is_last);
    ...
    *template = type;
    ...
    if (*is_last) {
        ...
        {
            /*is_last means not in use anymore, there for free allocation*/
            rc = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_res_trap_virtual, SHR_RES_ALLOC_WITH_ID, type);
        BCMDNX_IF_ERR_EXIT(rc);

        }

        /*update _rx_virtual_traps array with PPD prog virtual trap*/ 
        if (_bcm_dpp_rx_virtual_trap_get(unit, type, &new_virtual_traps_p) != BCM_E_NONE)
        {
            BCMDNX_ERR_EXIT_MSG(BCM_E_EXISTS, (_BSL_BCM_MSG("Function _bcm_dpp_rx_virtual_trap_get failed, on unit:%d \n"), unit));
        }
        new_virtual_traps_p.soc_ppd_prog = 0;
        if (_bcm_dpp_rx_virtual_trap_set(unit, type, &new_virtual_traps_p) != BCM_E_NONE)
        {
            BCMDNX_ERR_EXIT_MSG(BCM_E_EXISTS, (_BSL_BCM_MSG("Function _bcm_dpp_rx_virtual_trap_set failed, on unit:%d \n"), unit));
        }              
...
{
    BCMDNX_INIT_FUNC_DEFS;
    BCMDNX_IF_ERR_EXIT(dpp_am_template_clear(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_prog_trap));
exit:
    BCMDNX_FUNC_RETURN;
}
...
int _bcm_dpp_am_template_l2_event_handle_init(int unit,int template_init_id, int handle_flags)
{
    ...
    /* Add all entities to template init id with given data */
    result = dpp_am_template_allocate_group(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_l2_event_handle, flags, &handle_flags,_DPP_AM_TEMPLATE_L2_EVENT_HANDLE_MAX_ENTITIES, &is_allocated,&template);
    BCMDNX_IF_ERR_EXIT(result);
...
int _bcm_dpp_am_template_l2_event_handle_data_get(int unit, int vsi, int *handle_flags)
{
    ...
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_l2_event_handle,old_handle_profile,handle_flags);
...
int _bcm_dpp_am_template_l2_event_handle_exchange(int unit, int flags, int vsi, int handle_flags, int *old_template, int *is_last, int *template,int *is_allocated)
{
    ...
    if(!(flags & BCM_DPP_AM_TEMPLATE_FLAG_CHECK)) {
        rc = dpp_am_template_exchange(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_l2_event_handle,SHR_TEMPLATE_MANAGE_IGNORE_NOT_EXIST_OLD_TEMPLATE,&handle_flags,*old_template,is_last,template,is_allocated);
        BCMDNX_IF_ERR_EXIT(rc);
    }
    else
    {
        rc = dpp_am_template_exchange_test(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_l2_event_handle,SHR_TEMPLATE_MANAGE_IGNORE_NOT_EXIST_OLD_TEMPLATE,&handle_flags,*old_template,is_last,template,is_allocated);
    }
...
{
    BCMDNX_INIT_FUNC_DEFS;
    BCMDNX_IF_ERR_EXIT(dpp_am_template_clear(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_l2_event_handle));
exit:
    BCMDNX_FUNC_RETURN;
}
...
int _bcm_dpp_am_template_l2_learn_profile_init(int unit,int learn_profile_id, int limit, int event_handle_profile, int fid_aging_profile)
{
    ...
    /* Add all entities to template init id with given data */
    result = dpp_am_template_allocate_group(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_l2_vsi_learn_profile, flags, &data, _DPP_AM_TEMPLATE_L2_VSI_LEARN_PROFILE_MAX_ENTITIES, &is_allocated,&template);
    BCMDNX_IF_ERR_EXIT(result);
...
int _bcm_dpp_am_template_l2_learn_limit_active(int unit, int *is_active)
{
    ...
    *is_active = 0;
    ...
    for (index = 0; index < _DPP_AM_TEMPLATE_L2_FID_RPOFILES_COUNT; index++) {
        rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_l2_vsi_learn_profile, index, &data);
        if (rc == BCM_E_NOT_FOUND) {
            continue;
        }
        BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_l2_learn_profile_data_get(int unit, int vsi, int *limit, int *event_handle_profile, int *fid_aging_profile)
{
    ...
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_l2_vsi_learn_profile,old_handle_profile,&data);
    BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_l2_learn_profile_exchange(int unit, int flags, int vsi, int limit,int event_handle_profile, int fid_aging_profile, int *old_template, int *is_last, int *new_profile,int *is_allocated)
{
    ...
    if(!(flags & BCM_DPP_AM_TEMPLATE_FLAG_CHECK)) {
        rc = dpp_am_template_exchange(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_l2_vsi_learn_profile,SHR_TEMPLATE_MANAGE_IGNORE_NOT_EXIST_OLD_TEMPLATE,&data,*old_template,is_last,new_profile,is_allocated);

    }
    else
    {
        rc = dpp_am_template_exchange_test(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_l2_vsi_learn_profile,SHR_TEMPLATE_MANAGE_IGNORE_NOT_EXIST_OLD_TEMPLATE,&data,*old_template,is_last,new_profile,is_allocated);
    }
...
{
    BCMDNX_INIT_FUNC_DEFS;
    BCMDNX_IF_ERR_EXIT(dpp_am_template_clear(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_l2_vsi_learn_profile));
exit:
    BCMDNX_FUNC_RETURN;
}
...
int _bcm_dpp_am_template_fid_aging_profile_init(int unit,int fid_aging_profile_id, int aging_cycle)
{
    int is_allocated, result = BCM_E_NONE;
    uint32 flags = 0;
    int template = fid_aging_profile_id;
    ...
    /* Add all entities to template init id with given data */
    result = dpp_am_template_allocate_group(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_fid_aging_profile, flags, &aging_cycle, _DPP_AM_TEMPLATE_FID_AGING_PROFILE_MAX_ENTITIES, &is_allocated,&template);
    BCMDNX_IF_ERR_EXIT(result);
...
int
_bcm_dpp_am_template_fid_aging_profile_fid_profile_mapping_get(int unit, int vsi, int *fid_aging_profile)
{
    SOC_PPD_VSI_INFO vsi_info;
    unsigned int soc_sand_dev_id;
    uint32 soc_sand_rv;
    uint32
        fid_aging_profile_tmp;
    ...
    soc_sand_rv = soc_ppd_frwrd_mact_fid_aging_profile_get(soc_sand_dev_id,vsi_info.mac_learn_profile_id,&fid_aging_profile_tmp);
    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);

    *fid_aging_profile = fid_aging_profile_tmp;
...
int _bcm_dpp_am_template_fid_aging_profile_data_get(int unit, int vsi, int *aging_cycle){

    int old_fid_aging_profile;
    int rc = BCM_E_NONE;

    BCMDNX_INIT_FUNC_DEFS;
    _bcm_dpp_am_template_fid_aging_profile_fid_profile_mapping_get(unit,vsi, &old_fid_aging_profile);
    BCMDNX_IF_ERR_EXIT(rc);

    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_fid_aging_profile, old_fid_aging_profile, aging_cycle); 
    BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_fid_aging_profile_exchange(int unit, int flags, int vsi, int aging_cycle, int *old_fid_aging_profile, int *is_last, int *template,int *is_allocated){

    int rc = BCM_E_NONE;

    BCMDNX_INIT_FUNC_DEFS;
    rc = _bcm_dpp_am_template_fid_aging_profile_fid_profile_mapping_get(unit,vsi, old_fid_aging_profile);
    
    BCMDNX_IF_ERR_EXIT(rc);

    if(!(flags & BCM_DPP_AM_TEMPLATE_FLAG_CHECK)) {
        rc = dpp_am_template_exchange(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_fid_aging_profile,SHR_TEMPLATE_MANAGE_IGNORE_NOT_EXIST_OLD_TEMPLATE,&aging_cycle,*old_fid_aging_profile,is_last,template,is_allocated);
        BCMDNX_IF_ERR_EXIT(rc);
    }
    else
    {
        rc = dpp_am_template_exchange_test(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_fid_aging_profile,SHR_TEMPLATE_MANAGE_IGNORE_NOT_EXIST_OLD_TEMPLATE,&aging_cycle,*old_fid_aging_profile,is_last,template,is_allocated);
        if (rc == BCM_E_MEMORY) {
            LOG_ERROR(BSL_LS_BCM_COMMON,(BSL_META_U(unit, "Unit:%d, out of aging profiles\n"), unit));
        }
    }
...
int _bcm_dpp_am_template_fid_aging_profile_clear(int unit)
{
    BCMDNX_INIT_FUNC_DEFS;
    BCMDNX_IF_ERR_EXIT(dpp_am_template_clear(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_fid_aging_profile));
exit:
    BCMDNX_FUNC_RETURN;
}

/* L2 fid-aging profile - End */


/* COSQ Ingress rate class - Start */
STATIC int
_bcm_dpp_am_template_queue_rate_cls_mapping_get(int unit, int queue,int *rate_cls)
{
    uint32 soc_sand_rv;    
...
int _bcm_dpp_am_template_queue_rate_cls_init(int unit,int template_init_id,const bcm_dpp_cosq_ingress_rate_class_info_t* data_rate_cls)
{
    ...
    /* Add all entities to template init id with given data */ 
    result = dpp_am_template_allocate_group(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_queue_rate_cls,flags,data_rate_cls,_DPP_AM_TEMPLATE_QUEUE_RATE_CLS_MAX_ENTITIES(unit),&is_allocated,&template);
    BCMDNX_IF_ERR_EXIT(result);

exit:
    BCMDNX_FUNC_RETURN;
}
int _bcm_dpp_am_template_queue_rate_cls_data_get(int unit,int queue,bcm_dpp_cosq_ingress_rate_class_info_t * data_rate_cls)
{
    int rate_cls;
    int rc = BCM_E_NONE;

    BCMDNX_INIT_FUNC_DEFS;
    rc = _bcm_dpp_am_template_queue_rate_cls_mapping_get(unit,queue,&rate_cls);
    BCMDNX_IF_ERR_EXIT(rc);
    ...
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_queue_rate_cls,rate_cls,data_rate_cls);
    BCMDNX_IF_ERR_EXIT(rc);
exit:
    BCMDNX_FUNC_RETURN;
}
...
int _bcm_dpp_am_template_queue_rate_cls_exchange(int unit, int queue, bcm_dpp_cosq_ingress_rate_class_info_t * data_rate_cls, int *old_rate_cls, int *is_last, int *new_rate_cls,int *is_allocated)
{
    int rc = BCM_E_NONE;
    ...
    rc = _bcm_dpp_am_template_queue_rate_cls_mapping_get(unit,queue,&tmp_old_rate_cls);
    BCMDNX_IF_ERR_EXIT(rc);

    rc = dpp_am_template_exchange(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_queue_rate_cls,0,data_rate_cls,tmp_old_rate_cls,&tmp_is_last,new_rate_cls,is_allocated);
    BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_queue_rate_cls_exchange_test(int unit, int queue, bcm_dpp_cosq_ingress_rate_class_info_t * data_rate_cls, int *old_rate_cls, int *is_last, int *new_rate_cls,int *is_allocated)
{
    int rc = BCM_E_NONE;
    int tmp_old_rate_cls;
    int tmp_is_last;
    ...
    rc = dpp_am_template_exchange_test(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_queue_rate_cls,0,data_rate_cls,tmp_old_rate_cls,&tmp_is_last,new_rate_cls,is_allocated);
    BCMDNX_IF_ERR_EXIT(rc);
...
_bcm_dpp_am_template_queue_rate_cls_ref_get(int unit, int template, uint32 *ref_count)
{
    ...
    rc = dpp_am_template_ref_count_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_queue_rate_cls, template, ref_count);
...
_bcm_dpp_am_template_queue_rate_cls_allocate_group(int unit, uint32 flags, bcm_dpp_cosq_ingress_rate_class_info_t *data_rate_cls, int ref_count, int *is_allocated, int *template)
{
    ...
    rc = dpp_am_template_allocate_group(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_queue_rate_cls, flags, (void *)data_rate_cls, ref_count, is_allocated, template);
...
_bcm_dpp_am_template_queue_rate_cls_tdata_get(int unit, int template, bcm_dpp_cosq_ingress_rate_class_info_t *data_rate_cls)
{
    ...
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_queue_rate_cls, template, (void *)data_rate_cls);
...
STATIC int
_bcm_dpp_am_template_vsq_rate_cls_mapping_get(int unit, SOC_TMC_ITM_VSQ_NDX vsq_id,int *rate_cls)
{
    bcm_error_t rc = BCM_E_NONE;
    uint32 soc_sand_rv;
    SOC_TMC_ITM_VSQ_GROUP vsq_group = SOC_TMC_ITM_VSQ_GROUP_LAST_ARAD;
    ...
    soc_sand_rv = (MBCM_DPP_DRIVER_CALL_WITHOUT_DEV_ID(unit,mbcm_dpp_itm_vsq_index_global2group,( vsq_id, &vsq_group, &vsq_in_group_ndx)));
    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);

    soc_sand_rv = (MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_itm_vsq_qt_rt_cls_get,(unit, vsq_group, vsq_in_group_ndx, &tmp)));
    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
...
int _bcm_dpp_am_template_vsq_rate_cls_init(int unit,int template_init_id,const bcm_dpp_cosq_vsq_rate_class_info_t* data_rate_cls)
{
    ...
    /* Add all entities to template init id with given data */
    result = dpp_am_template_allocate_group(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_vsq_rate_cls, flags, data_rate_cls, _DPP_AM_TEMPLATE_VSQ_RATE_CLS_MAX_ENTITIES, &is_allocated,&template);
    BCMDNX_IF_ERR_EXIT(result);
...
int _bcm_dpp_am_template_vsq_rate_cls_data_get(int unit,int vsq_id,bcm_dpp_cosq_vsq_rate_class_info_t * data_rate_cls)
{
    int rate_cls;
    int rc = BCM_E_NONE;

    BCMDNX_INIT_FUNC_DEFS;
    rc = _bcm_dpp_am_template_vsq_rate_cls_mapping_get(unit,vsq_id,&rate_cls);
    BCMDNX_IF_ERR_EXIT(rc);
    ...
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_vsq_rate_cls,rate_cls,data_rate_cls);
...
int _bcm_dpp_am_template_vsq_rate_cls_exchange(int unit, int vsq_id, bcm_dpp_cosq_vsq_rate_class_info_t * data_rate_cls, int *old_rate_cls, int *is_last, int *new_rate_cls,int *is_allocated)
{
    int rc = BCM_E_NONE;
    int tmp_old_rate_cls;
    int tmp_is_last;
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = _bcm_dpp_am_template_vsq_rate_cls_mapping_get(unit,vsq_id,&tmp_old_rate_cls);
    BCMDNX_IF_ERR_EXIT(rc);

    rc = dpp_am_template_exchange(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_vsq_rate_cls,0,data_rate_cls,tmp_old_rate_cls,&tmp_is_last,new_rate_cls,is_allocated);
    BCMDNX_IF_ERR_EXIT(rc);
...
int
_bcm_dpp_am_template_vsq_rate_cls_ref_get(int unit, int template, uint32 *ref_count)
{
    int rc = BCM_E_NONE;
    ...
    rc = dpp_am_template_ref_count_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_vsq_rate_cls, template, ref_count);
...
int
_bcm_dpp_am_template_vsq_rate_cls_allocate_group(int unit, uint32 flags, bcm_dpp_cosq_vsq_rate_class_info_t *data_rate_cls, int ref_count, int *is_allocated, int *template)
{
    int rc = BCM_E_NONE;
    ...
    rc = dpp_am_template_allocate_group(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_vsq_rate_cls, flags, (void *)data_rate_cls, ref_count, is_allocated, template);
...
int
_bcm_dpp_am_template_vsq_rate_cls_tdata_get(int unit, int template, bcm_dpp_cosq_vsq_rate_class_info_t *data_rate_cls)
{
    int rc = BCM_E_NONE;
    ...
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_vsq_rate_cls, template, (void *)data_rate_cls);
...
int _bcm_dpp_am_template_system_red_dp_pr_init(int unit,int template_init_id,const bcm_dpp_cosq_ingress_system_red_dp_pr_info_t* data_sys_red_dp_pr)
{
    ...
    /* Add all entities to template init id with given data */ 
    result = dpp_am_template_allocate_group(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_system_red_dp_pr, flags,data_sys_red_dp_pr, 
...
int _bcm_dpp_am_template_system_red_dp_pr_data_get(int unit, int dp_pr_ndx, bcm_dpp_cosq_ingress_system_red_dp_pr_info_t * data_sys_red_dp_pr)
{
    ...
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_system_red_dp_pr, sys_red_dp_pr_template_ndx, data_sys_red_dp_pr);
    BCMDNX_IF_ERR_EXIT(rc);
exit:
    BCMDNX_FUNC_RETURN;
}
                                                   ...
                                                   int *old_sys_red_dp_pr_ndx, int *is_last, int *new_sys_red_dp_pr_ndx, int *is_allocated)
{
    ...
    rc = dpp_am_template_exchange(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_system_red_dp_pr, 0, data_sys_red_dp_pr, tmp_old_sys_red_dp_pr_ndx,
...
_bcm_dpp_am_template_system_red_dp_pr_ref_get(int unit, int template, uint32 *ref_count)
{
    ...
    rc = dpp_am_template_ref_count_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_system_red_dp_pr, template, ref_count);
...
_bcm_dpp_am_template_system_red_dp_pr_allocate_group(int unit, uint32 flags, bcm_dpp_cosq_ingress_system_red_dp_pr_info_t *data_sys_red_dp_pr, int ref_count, int *is_allocated, int *template)
{
    ...
    rc = dpp_am_template_allocate_group(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_system_red_dp_pr, flags, (void *)data_sys_red_dp_pr, ref_count, is_allocated, template);
...
_bcm_dpp_am_template_system_red_dp_pr_tdata_get(int unit, int template, bcm_dpp_cosq_ingress_system_red_dp_pr_info_t *data_sys_red_dp_pr)
{
    ...
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_system_red_dp_pr, template, (void *)data_sys_red_dp_pr);
...
STATIC int
_bcm_dpp_am_template_queue_discount_cls_mapping_get(int unit, int queue,int *discount_cls)
{
    bcm_error_t rc = BCM_E_NONE;
    uint32 soc_sand_rv;
    SOC_TMC_ITM_QUEUE_INFO queue_info;
...
int _bcm_dpp_am_template_queue_discount_cls_init(int unit,int template_init_id,const SOC_TMC_ITM_CR_DISCOUNT_INFO* data_discount_cls)
{
    ...
    /* Add all entities to template init id with given data */
    result = dpp_am_template_allocate_group(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_queue_discount_cls, flags, data_discount_cls, _DPP_AM_TEMPLATE_QUEUE_DISCNT_CLS_MAX_ENTITIES(unit), &is_allocated,&template);
    BCMDNX_IF_ERR_EXIT(result);
...
int _bcm_dpp_am_template_queue_discount_cls_data_get(int unit,int queue,SOC_TMC_ITM_CR_DISCOUNT_INFO * data_discount_cls)
{
    int discount_cls;
    int rc = BCM_E_NONE;

    BCMDNX_INIT_FUNC_DEFS;
    rc = _bcm_dpp_am_template_queue_discount_cls_mapping_get(unit,queue,&discount_cls);
    BCMDNX_IF_ERR_EXIT(rc);

    if (discount_cls < 0) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Falied to get discount class")));
    }

    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_queue_discount_cls,discount_cls,data_discount_cls);
...
int _bcm_dpp_am_template_queue_discount_cls_exchange(int unit, int queue, SOC_TMC_ITM_CR_DISCOUNT_INFO * data_discount_cls, int *old_discount_cls, int *is_last, int *new_discount_cls,int *is_allocated)
{
    int rc = BCM_E_NONE;
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = _bcm_dpp_am_template_queue_discount_cls_mapping_get(unit,queue,&tmp_old_discount_cls);
    BCMDNX_IF_ERR_EXIT(rc);

    rc = dpp_am_template_exchange(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_queue_discount_cls,0,data_discount_cls,tmp_old_discount_cls,&tmp_is_last,new_discount_cls,is_allocated);
    if (rc != BCM_E_NONE) {
...
int
_bcm_dpp_am_template_queue_discount_cls_ref_get(int unit, int template, uint32 *ref_count)
{
    int rc = BCM_E_NONE;
    ...
    rc = dpp_am_template_ref_count_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_queue_discount_cls, template, ref_count);
...
int
_bcm_dpp_am_template_queue_discount_cls_allocate_group(int unit, uint32 flags, SOC_TMC_ITM_CR_DISCOUNT_INFO *data_discount_cls, int ref_count, int *is_allocated, int *template)
{
    int rc = BCM_E_NONE;
    ...
    rc = dpp_am_template_allocate_group(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_queue_discount_cls, flags, (void *)data_discount_cls, ref_count, is_allocated, template);
...
int
_bcm_dpp_am_template_queue_discount_cls_tdata_get(int unit, int template, SOC_TMC_ITM_CR_DISCOUNT_INFO *data_discount_cls)
{
    int rc = BCM_E_NONE;
    ...
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_queue_discount_cls, template, (void *)data_discount_cls);
...
int _bcm_dpp_am_template_egress_port_discount_cls_data_get(int unit, int core, uint32 tm_port,SOC_TMC_PORT_EGR_HDR_CR_DISCOUNT_INFO * data_discount_cls)
{
    ...
    if (discount_profile < 0) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Falied to get discount profile")));
    }
    switch (header_type_map) {
        case SOC_TMC_PORT_HEADER_TYPE_RAW:
...
static _dpp_cosq_port_hr_fc_map_t _bcm_dpp_cosq_port_hr_fc_map[BCM_MAX_NUM_UNITS];

STATIC int
_bcm_dpp_am_template_cosq_port_hr_fc_profile_mapping_get(int unit, int port, int *profile)
{
    BCMDNX_INIT_FUNC_DEFS;

    (*profile) = _bcm_dpp_cosq_port_hr_fc_map[unit].profile[port];
...
int
_bcm_dpp_am_template_cosq_port_hr_fc_profile_mapping_set(int unit, int port, int profile)
{
    BCMDNX_INIT_FUNC_DEFS;

    BCM_DPP_UNIT_CHECK(unit);

    _bcm_dpp_cosq_port_hr_fc_map[unit].profile[port] = profile;
...
int
_bcm_dpp_am_template_cosq_port_hr_fc_init(int unit, int template_init_id, int *fc_data_init)
{
    int index, is_allocated, result = BCM_E_NONE;
    uint32 flags = 0;
    int template;
    ...
    result = dpp_am_template_allocate_group(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_cosq_port_hr_flow_control, flags, fc_data_init,_BCM_PETRA_NOF_TM_PORTS(unit), &is_allocated, &template);
    BCMDNX_IF_ERR_EXIT(result);

    /* Add all entities to template init id with given data */
    for (index = 0; index <= _BCM_PETRA_NOF_TM_PORTS(unit); index++) {
        result = _bcm_dpp_am_template_cosq_port_hr_fc_profile_mapping_set(unit, index, template_init_id);
        BCMDNX_IF_ERR_EXIT(result);
    }
...
int
_bcm_dpp_am_template_cosq_port_hr_fc_data_get(int unit, int port, int *data)
{
    int old_profile;
    int rc = BCM_E_NONE;
    ...
    BCMDNX_INIT_FUNC_DEFS;
    _bcm_dpp_am_template_cosq_port_hr_fc_profile_mapping_get(unit, port, &old_profile);
    BCMDNX_IF_ERR_EXIT(rc);
    ...
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_cosq_port_hr_flow_control, old_profile, data);
...
int
_bcm_dpp_am_template_cosq_port_hr_fc_exchange(int unit, int port, int *data, int *old_template, int *is_last, int *template, int *is_allocated)
{
    int rc = BCM_E_NONE;
    int temp;
    int temp_is_last;
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = _bcm_dpp_am_template_cosq_port_hr_fc_profile_mapping_get(unit, port, &temp);
    BCMDNX_IF_ERR_EXIT(rc);
    ...
    rc = dpp_am_template_exchange(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_cosq_port_hr_flow_control, 0, data, temp, &temp_is_last, template, is_allocated);
    BCMDNX_IF_ERR_EXIT(rc);
    ...
    /* Since port-profile mapping is implememnted as s/w cache. */
    _bcm_dpp_am_template_cosq_port_hr_fc_profile_mapping_set(unit, port, (*template));
    BCMDNX_IF_ERR_EXIT(rc);
exit:
    BCMDNX_FUNC_RETURN;
}
...
int
_bcm_dpp_am_template_cosq_hr_fc_ref_get(int unit, int template, uint32 *ref_count)
{
    int rc = BCM_E_NONE;
    ...
    rc = dpp_am_template_ref_count_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_cosq_port_hr_flow_control, template, ref_count);
...
int
_bcm_dpp_am_template_cosq_hr_fc_allocate_group(int unit, uint32 flags, int *data, int ref_count, int *is_allocated, int *template)
{
    int rc = BCM_E_NONE;
    ...
    rc = dpp_am_template_allocate_group(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_cosq_port_hr_flow_control, flags, (void *)data, ref_count, is_allocated, template);
...
int
_bcm_dpp_am_template_cosq_sched_class_allocate(int unit, int flags, int *data, int *is_allocated, int *class_template)
{
    int rc = BCM_E_NONE;

    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_allocate(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_cosq_sched_class, flags, data, is_allocated, class_template);
    BCMDNX_IF_ERR_EXIT(rc);
exit:
    BCMDNX_FUNC_RETURN;
}
...
int
_bcm_dpp_am_template_cosq_sched_class_free(int unit, int class_template, int *is_last)
{
    int rc = BCM_E_NONE;

    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_cosq_sched_class, class_template, is_last);
    BCMDNX_IF_ERR_EXIT(rc);
exit:
    BCMDNX_FUNC_RETURN;
}
...
int
_bcm_dpp_am_template_cosq_sched_class_data_get(int unit, int class_template, int *data)
{
    int rc = BCM_E_NONE;

    
    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_cosq_sched_class, class_template, data);
    BCMDNX_IF_ERR_EXIT(rc);
exit:
    BCMDNX_FUNC_RETURN;
}
...
int
_bcm_dpp_am_template_cosq_sched_class_exchange(int unit, int flags, int *data, int old_class_template, int *is_last, int *class_template, int *is_allocated)
{
    int rc = BCM_E_NONE;
    int temp_is_last;
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_exchange(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_cosq_sched_class, flags, data, old_class_template, &temp_is_last, class_template, is_allocated);
    BCMDNX_IF_ERR_EXIT(rc);
...
int
_bcm_dpp_am_template_cosq_sched_class_ref_get(int unit, int template, uint32 *ref_count)
{
    int rc = BCM_E_NONE;
    ...
    rc = dpp_am_template_ref_count_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_cosq_sched_class, template, ref_count);
...
int
_bcm_dpp_am_template_cosq_sched_class_allocate_group(int unit, uint32 flags, int *data, int ref_count, int *is_allocated, int *template)
{
    int rc = BCM_E_NONE;
    ...
    rc = dpp_am_template_allocate_group(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_cosq_sched_class, flags, (void *)data, ref_count, is_allocated, template);
...
int _bcm_dpp_am_template_mpls_push_profile_exchange(int unit, int push_profile, SOC_PPD_EG_ENCAP_PUSH_PROFILE_INFO *push_profile_info,  int *is_last, 
                                                    int *new_push_profile,int *is_allocated, uint8 with_id)
{
    ...
    rc = dpp_am_template_exchange(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_mpls_push_profile,
...
int _bcm_dpp_am_template_mpls_push_profile_alloc(int unit, int flags, int push_profile, SOC_PPD_EG_ENCAP_PUSH_PROFILE_INFO *push_profile_info, int *new_push_profile,int *is_allocated)
{
    int rc = BCM_E_NONE;


    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_allocate(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_mpls_push_profile, flags, push_profile_info, is_allocated, new_push_profile);
    BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_mpls_push_profile_free(int unit, int push_profile, int *is_last)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_mpls_push_profile,push_profile,is_last);
    BCMDNX_IF_ERR_EXIT(rc);
...
STATIC int
_bcm_dpp_am_template_lif_term_mapping_get(int unit, SOC_PPD_LIF_ID lif_id,int *template)
{
    bcm_error_t rc = BCM_E_NONE;
    uint32 soc_sand_rv;
    int soc_sand_dev_id;      
    ...
    *template = 0;
    ...
    if(lif_entry_info.type != SOC_PPD_LIF_ENTRY_TYPE_PWE && lif_entry_info.type != SOC_PPD_LIF_ENTRY_TYPE_EMPTY
       && lif_entry_info.type != SOC_PPD_LIF_ENTRY_TYPE_MPLS_TUNNEL_RIF) {
...
int _bcm_dpp_am_template_lif_term_init(int unit,int template_init_id,const SOC_PPD_MPLS_TERM_PROFILE_INFO* term_profile)
{
    int is_allocated, result = BCM_E_NONE;
    uint32 flags = 0;
    int template;
    ...
    /* Add all entities to template init id with given data */
    result = dpp_am_template_allocate_group(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_lif_term_profile, flags, term_profile, _DPP_AM_TEMPLATE_LIF_TERM_PROFILE_MAX_ENTITIES, &is_allocated,&template);
    BCMDNX_IF_ERR_EXIT(result);
...
int _bcm_dpp_am_template_lif_term_data_get(int unit, SOC_PPD_LIF_ID lif_id, SOC_PPD_MPLS_TERM_PROFILE_INFO *term_profile_info)
{
    int template;
    int rc = BCM_E_NONE;

    BCMDNX_INIT_FUNC_DEFS;
    rc = _bcm_dpp_am_template_lif_term_mapping_get(unit,lif_id,&template);
    BCMDNX_IF_ERR_EXIT(rc);
    ...
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_lif_term_profile,template,term_profile_info);
...
int _bcm_dpp_am_template_lif_term_profile_exchange(int unit, SOC_PPD_LIF_ID lif_id, SOC_PPD_MPLS_TERM_PROFILE_INFO *term_profile_info,  int *old_template, int *is_last, int *new_term_profile,int *is_allocated)
{
    int rc = BCM_E_NONE;
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = _bcm_dpp_am_template_lif_term_mapping_get(unit,lif_id,&tmp_old_template);
    BCMDNX_IF_ERR_EXIT(rc);

    rc = dpp_am_template_exchange(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_lif_term_profile,0,term_profile_info,tmp_old_template,&tmp_is_last,new_term_profile,is_allocated);
    BCMDNX_IF_ERR_EXIT(rc);
...
STATIC int
_bcm_dpp_am_template_port_mact_sa_drop_mapping_get(int unit, int port,int *template)
{
    bcm_error_t rc = BCM_E_NONE;
    uint32 soc_sand_rv;
    int soc_sand_dev_id;      
    SOC_PPD_FRWRD_MACT_PORT_INFO port_info;
    ...
    SOC_PPD_FRWRD_MACT_PORT_INFO_clear(&port_info);

    soc_sand_rv = soc_ppd_frwrd_mact_port_info_get(soc_sand_dev_id,port,&port_info);      
    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);    
...
int _bcm_dpp_am_template_port_mact_sa_drop_init(int unit,int template_init_id,const _bcm_petra_port_mact_mgmt_action_profile_t* action_profile)
{
    ...
    /* Add all entities to template init id with given data */
    result = dpp_am_template_allocate_group(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_port_mact_sa_drop_profile, flags, action_profile, ref_cnt, &is_allocated,&template);
    BCMDNX_IF_ERR_EXIT(result);
    ...
    for (template = _DPP_AM_TEMPLATE_PORT_MACT_SA_DROP_PROFILE_LOW_ID; template < _DPP_AM_TEMPLATE_PORT_MACT_SA_DROP_PROFILE_LOW_ID+_DPP_AM_TEMPLATE_PORT_MACT_SA_DROP_PROFILE_COUNT; ++template) {
        result = dpp_am_template_allocate(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_port_mact_sa_drop_profile, flags, &template, &is_allocated,&template);
        BCMDNX_IF_ERR_EXIT(result);
...
int _bcm_dpp_am_template_port_mact_sa_drop_data_get(int unit,int port,_bcm_petra_port_mact_mgmt_action_profile_t* action_profile)
{
    int template;
    int rc = BCM_E_NONE;

    BCMDNX_INIT_FUNC_DEFS;
    rc = _bcm_dpp_am_template_port_mact_sa_drop_mapping_get(unit,port,&template);
    BCMDNX_IF_ERR_EXIT(rc);

    if (template < 0) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Failed to get port mact source addresss drop")));
    }

    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_port_mact_sa_drop_profile,template,action_profile);
...
int _bcm_dpp_am_template_port_mact_sa_drop_exchange(int unit, int port, _bcm_petra_port_mact_mgmt_action_profile_t* action_profile, int *old_template, int *is_last, int *new_template,int *is_allocated)
{
    int rc = BCM_E_NONE;
    int tmp_old_template;
    int tmp_is_last;
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = _bcm_dpp_am_template_port_mact_sa_drop_mapping_get(unit,port,&tmp_old_template);
    BCMDNX_IF_ERR_EXIT(rc);

    rc = dpp_am_template_exchange(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_port_mact_sa_drop_profile,0,action_profile,tmp_old_template,&tmp_is_last,new_template,is_allocated);
    BCMDNX_IF_ERR_EXIT(rc);
...
STATIC int
_bcm_dpp_am_template_port_mact_da_unknown_mapping_get(int unit, int port,int *template)
{
    bcm_error_t rc = BCM_E_NONE;
    uint32 soc_sand_rv;
    int soc_sand_dev_id;      
...
int _bcm_dpp_am_template_port_mact_da_unknown_init(int unit,int template_init_id,const _bcm_petra_port_mact_mgmt_action_profile_t* action_profile)
{
    ...
    /* Add all entities to template init id with given data */
    result = dpp_am_template_allocate_group(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_port_mact_da_unknown_profile, flags, action_profile,ref_cnt, &is_allocated,&template);
    BCMDNX_IF_ERR_EXIT(result);
    ...
    for (template = _DPP_AM_TEMPLATE_PORT_MACT_DA_UNKNOWN_PROFILE_LOW_ID; template < _DPP_AM_TEMPLATE_PORT_MACT_DA_UNKNOWN_PROFILE_LOW_ID+_DPP_AM_TEMPLATE_PORT_MACT_DA_UNKNOWN_PROFILE_COUNT; ++template) {
        result = dpp_am_template_allocate(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_port_mact_da_unknown_profile, flags, &template, &is_allocated,&template);
        BCMDNX_IF_ERR_EXIT(result);
...
int _bcm_dpp_am_template_port_mact_da_unknown_data_get(int unit,int port,_bcm_petra_port_mact_mgmt_action_profile_t* action_profile)
{
    int template;
    int rc = BCM_E_NONE;

    BCMDNX_INIT_FUNC_DEFS;
    rc = _bcm_dpp_am_template_port_mact_da_unknown_mapping_get(unit,port,&template);
    BCMDNX_IF_ERR_EXIT(rc);

    if (template < 0) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Failed to get port mact destination address unkown")));
    }

    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_port_mact_da_unknown_profile,template,action_profile);
...
int _bcm_dpp_am_template_port_mact_da_unknown_exchange(int unit, int port, _bcm_petra_port_mact_mgmt_action_profile_t* action_profile, int *old_template, int *is_last, int *new_template,int *is_allocated)
{
    int rc = BCM_E_NONE;
    int tmp_old_template;
    int tmp_is_last;
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = _bcm_dpp_am_template_port_mact_da_unknown_mapping_get(unit,port,&tmp_old_template);
    BCMDNX_IF_ERR_EXIT(rc);

    rc = dpp_am_template_exchange(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_port_mact_da_unknown_profile,0,action_profile,tmp_old_template,&tmp_is_last,new_template,is_allocated);
    BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_meter_init(int unit, int template_init_id, int high_rate, bcm_dpp_am_meter_entry_t *meter_profile_info)
{
    int is_allocated, result = BCM_E_NONE;
    uint32 flags = SHR_TEMPLATE_MANAGE_SET_WITH_ID;
    int template = template_init_id;
    ...
    if (high_rate == 1) {
        template -= _DPP_AM_TEMPLATE_METER_HIGH_PROFILE_LOW_ID(unit);
    }

    /* Add all entities to template init id with given data */
    if (SOC_IS_PETRAB(unit)) {
        template_type = dpp_am_template_meter_profile_a_high;
    }
    else {
    ...
    /* Add all entities to template init id with given data */
    result = dpp_am_template_allocate_group(unit, BCM_DPP_AM_DEFAULT_CORE_ID, template_type, flags, meter_profile_info, SOC_DPP_CONFIG(unit)->meter.nof_meter_a, &is_allocated,&template);
    BCMDNX_IF_ERR_EXIT(result);

    if (SOC_IS_PETRAB(unit)) {
        template_type = dpp_am_template_meter_profile_b_high;
    }
    else {
    ...
    /* Add all entities to template init id with given data */
    result = dpp_am_template_allocate_group(unit, BCM_DPP_AM_DEFAULT_CORE_ID, template_type, flags, meter_profile_info, SOC_DPP_CONFIG(unit)->meter.nof_meter_b, &is_allocated,&template);
    BCMDNX_IF_ERR_EXIT(result);
   ...
   )
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    tmp_tmplt_type += group * 2;

    if (high_rate == 1) {
        tmp_tmplt_type += 1;
        meter_profile -= _DPP_AM_TEMPLATE_METER_HIGH_PROFILE_LOW_ID(unit);
    }

    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID,tmp_tmplt_type,meter_profile,meter_profile_info);
    if (rc == BCM_E_NOT_FOUND) {
    ...
    )
{
    ...
    rc = dpp_am_template_exchange(unit, BCM_DPP_AM_DEFAULT_CORE_ID,tmp_tmplt_type,flags,meter_profile_info,meter_profile,&tmp_is_last,new_meter_profile,is_allocated);
    BCMDNX_IF_ERR_EXIT(rc);

    if (high_rate == 1) {
        *new_meter_profile += _DPP_AM_TEMPLATE_METER_HIGH_PROFILE_LOW_ID(unit);
    }
...
int _bcm_dpp_am_template_meter_alloc(int unit, int flags, int group, int high_rate, bcm_dpp_am_meter_entry_t *meter_profile_info, int *meter_profile,int *is_allocated)
{
    int rc = BCM_E_NONE;
    _dpp_am_template_t tmp_tmplt_type = dpp_am_template_meter_profile_a_low;
    ...
    rc = dpp_am_template_allocate(unit, BCM_DPP_AM_DEFAULT_CORE_ID, tmp_tmplt_type, flags, meter_profile_info, is_allocated, meter_profile);
    BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_meter_free(int unit, int meter_profile, int group, int high_rate, int *is_last)
{
    int rc = BCM_E_NONE;
    _dpp_am_template_t tmp_tmplt_type = dpp_am_template_meter_profile_a_low;
    ...
    rc = dpp_am_template_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID,tmp_tmplt_type,meter_profile,is_last);
    BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_meter_clear(int unit)
{
    int rc = BCM_E_NONE;

    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_clear(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_meter_profile_a_low);
    BCMDNX_IF_ERR_EXIT(rc);

    rc = dpp_am_template_clear(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_meter_profile_a_high);
    BCMDNX_IF_ERR_EXIT(rc);

    rc = dpp_am_template_clear(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_meter_profile_b_low);
    BCMDNX_IF_ERR_EXIT(rc);

    rc = dpp_am_template_clear(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_meter_profile_b_high);
    BCMDNX_IF_ERR_EXIT(rc);
exit:
    BCMDNX_FUNC_RETURN;
}   
...
int _bcm_dpp_am_template_fabric_tdm_link_ptr_init(int unit,int template_init_id,const SOC_TMC_TDM_DIRECT_ROUTING_INFO* routing_info)
{
    ...
    for (index = 0; index < _DPP_AM_TEMPLATE_FABRIC_TDM_LINK_PTR_MAX_ENTITIES; index++) {
        result = dpp_am_template_allocate(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_fabric_tdm_link_ptr, flags, routing_info, &is_allocated,&template);
        BCMDNX_IF_ERR_EXIT(result);
    }
...
int _bcm_dpp_am_template_fabric_tdm_link_ptr_exchange(int unit, int port, SOC_TMC_TDM_DIRECT_ROUTING_INFO * routing_info, int *old_link_ptr, int *is_last, int *new_link_ptr,int *is_allocated)
{
    ...
    rc = dpp_am_template_exchange(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_fabric_tdm_link_ptr,0,routing_info,tmp_old_link_ptr,&tmp_is_last,new_link_ptr,is_allocated);
    BCMDNX_IF_ERR_EXIT(rc);
...
STATIC int
_bcm_dpp_am_template_l2_flooding_mapping_get(int unit, SOC_PPD_LIF_ID lif_index,int *profile)
{
    bcm_error_t rc = BCM_E_NONE;
    uint32 soc_sand_rv;
    int soc_sand_dev_id;   
    ...
    BCMDNX_ALLOC(lif_entry_info, sizeof(SOC_PPD_LIF_ENTRY_INFO), "_bcm_dpp_am_template_l2_flooding_mapping_get.lif_entry_info");
    ...
    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    if(lif_entry_info->type != SOC_PPD_LIF_ENTRY_TYPE_AC && lif_entry_info->type != SOC_PPD_LIF_ENTRY_TYPE_FIRST_AC_IN_GROUP
       && lif_entry_info->type != SOC_PPD_LIF_ENTRY_TYPE_EMPTY && lif_entry_info->type != SOC_PPD_LIF_ENTRY_TYPE_PWE) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_INTERNAL, (_BSL_BCM_MSG("LIF entry type is not as expected AC, PWE")));      /* expected to be AC, PWE type*/
    }
...
int _bcm_dpp_am_template_l2_flooding_init(int unit, int template_init_id, const bcm_dpp_vlan_flooding_profile_info_t* flooding_info)
{
    ...
    rc = dpp_am_template_allocate_group(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_l2_flooding_profile, flags, flooding_info, _DPP_AM_TEMPLATE_L2_FLOODING_LIF_PROFILE_MAX_ENTITIES, &is_allocated,&template);
    BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_l2_flooding_data_get(int unit, SOC_PPD_LIF_ID lif_index, bcm_dpp_vlan_flooding_profile_info_t* flooding_info)
{
    int rc = BCM_E_NONE;
    int template;
    ...
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_l2_flooding_profile,template,flooding_info);
    BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_l2_flooding_exchange(int unit, SOC_PPD_LIF_ID lif_index, bcm_dpp_vlan_flooding_profile_info_t* flooding_info, int *old_template, int *is_last, int *new_template,int *is_allocated)
{
    int rc = BCM_E_NONE;
    ...
    rc = dpp_am_template_exchange(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_l2_flooding_profile,0,flooding_info,tmp_old_template,&tmp_is_last,&tmp_new_template,is_allocated);
    BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_vlan_port_protocol_init(int unit,int template_init_id,const bcm_dpp_vlan_port_protocol_entries_t* port_protocol_info)
{
    ...
    rc = dpp_am_template_allocate_group(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_vlan_port_protocol_profile, flags, port_protocol_info,_DPP_AM_TEMPLATE_VLAN_PORT_PROTOCOL_PROFILE_MAX_ENTITIES, &is_allocated,&template);
    BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_vlan_port_protocol_data_get(int unit, int port, bcm_dpp_vlan_port_protocol_entries_t* port_protocol_info)
{
    ...
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_vlan_port_protocol_profile,template,port_protocol_info);
    BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_vlan_port_protocol_exchange(int unit, int port, bcm_dpp_vlan_port_protocol_entries_t* port_protocol_info, int *old_template, int *is_last, int *new_template,int *is_allocated)
{
    ...
    rc = dpp_am_template_exchange(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_vlan_port_protocol_profile,0,port_protocol_info,tmp_old_template,&tmp_is_last,&tmp_new_template,is_allocated);
    BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_ip_tunnel_init(int unit,int sip_profile ,int sip_val, int ttl_profile, int ttl_val, int tos_profile, int tos_val)
{
    int rc = BCM_E_NONE;
    uint32 flags = 0;
    int template;
    ...
    rc = dpp_am_template_allocate_group(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_ip_tunnel_src_ip, flags, &sip_val, _DPP_AM_TEMPLATE_IP_TUNNEL_SRC_IP_MAX_ENTITIES, &is_allocated,&template);
    BCMDNX_IF_ERR_EXIT(rc);
    
    /* init TTL */
    /* -1 to indicate use dummy profile, good for driver init */
    template = (ttl_profile != -1)?ttl_profile:_DPP_AM_TEMPLATE_IP_TUNNEL_TTL_COUNT-1;
    rc = dpp_am_template_allocate_group(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_ip_tunnel_ttl, flags, &ttl_val, _DPP_AM_TEMPLATE_IP_TUNNEL_TTL_MAX_ENTITIES, &is_allocated,&template);
    BCMDNX_IF_ERR_EXIT(rc);
    ...
    rc = dpp_am_template_allocate_group(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_ip_tunnel_tos, flags, &tos_val, _DPP_AM_TEMPLATE_IP_TUNNEL_TOS_MAX_ENTITIES, &is_allocated,&template);
    BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_ip_tunnel_data_get(int unit, int tunnel_id, int *sip_val, int *ttl_val, int *tos_val)
{
    int rc = BCM_E_NONE;
    ...
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_ip_tunnel_src_ip,sip_profile,sip_val);
    BCMDNX_IF_ERR_EXIT(rc);

    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_ip_tunnel_ttl,ttl_profile,ttl_val);
    BCMDNX_IF_ERR_EXIT(rc);

    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_ip_tunnel_tos,tos_profile,tos_val);
    BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_ip_tunnel_exchange(int unit, int tunnel_id, int is_new_ip_tunnel, int sip, int ttl, int tos, int *sip_template, int *ttl_template, int *tos_template,int *is_allocated)
{
    int rc = BCM_E_NONE;
    ...
    rc = dpp_am_template_exchange_test(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_ip_tunnel_src_ip,0,&sip,old_profile_sip,&tmp_is_last,sip_template,&cur_is_allocated);
    BCMDNX_IF_ERR_EXIT(rc);
    rc = dpp_am_template_exchange_test(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_ip_tunnel_ttl,0,&ttl,old_profile_ttl,&tmp_is_last,ttl_template,&cur_is_allocated);
    BCMDNX_IF_ERR_EXIT(rc);
    rc = dpp_am_template_exchange_test(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_ip_tunnel_tos,0,&tos,old_profile_tos,&tmp_is_last,tos_template,&cur_is_allocated);
    BCMDNX_IF_ERR_EXIT(rc);
    ...
    *is_allocated = 0;
    ...
    rc = dpp_am_template_exchange(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_ip_tunnel_src_ip,0,&sip,old_profile_sip,&tmp_is_last,sip_template,&cur_is_allocated);
    BCMDNX_IF_ERR_EXIT(rc);

    *is_allocated |= cur_is_allocated;

    rc = dpp_am_template_exchange(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_ip_tunnel_ttl,0,&ttl,old_profile_ttl,&tmp_is_last,ttl_template,&cur_is_allocated);
    BCMDNX_IF_ERR_EXIT(rc);

    *is_allocated |= cur_is_allocated;

    rc = dpp_am_template_exchange(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_ip_tunnel_tos,0,&tos,old_profile_tos,&tmp_is_last,tos_template,&cur_is_allocated);
    BCMDNX_IF_ERR_EXIT(rc);
...
STATIC int
_bcm_dpp_am_template_ingress_flow_tc_mapping_get(int unit, int dest_ndx ,uint32 *profile_ndx)
{
  bcm_error_t rc = BCM_E_NONE;
  uint32 soc_sand_rv;
  ...
  soc_sand_rv = (MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_ipq_tc_profile_get,(unit,1, dest_ndx , profile_ndx)));    
  BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
...
int _bcm_dpp_am_template_ingress_flow_tc_mapping_data_get(int unit,int dest_ndx,SOC_TMC_ITM_TC_MAPPING *tc_mapping)
{
  uint32 profile_ndx;
  int rc = BCM_E_NONE;

  BCMDNX_INIT_FUNC_DEFS;
  rc = _bcm_dpp_am_template_ingress_flow_tc_mapping_get(unit,dest_ndx,&profile_ndx);
  BCMDNX_IF_ERR_EXIT(rc);

  rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_ingress_flow_tc_mapping,profile_ndx,tc_mapping);
...
int _bcm_dpp_am_template_ingress_flow_tc_mapping_init(int unit,int template_init_id,const SOC_TMC_ITM_TC_MAPPING* tc_mapping)
{
  ...
  result = dpp_am_template_allocate_group(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_ingress_flow_tc_mapping, flags, tc_mapping,SOC_DPP_DEFS_GET(unit, nof_flows), &is_allocated,&template);
  BCMDNX_IF_ERR_EXIT(result);
...
int _bcm_dpp_am_template_ingress_flow_tc_mapping_exchange(int unit, int dest_ndx, SOC_TMC_ITM_TC_MAPPING *tc_mapping, uint32 *old_mapping_profile, int *is_last, int *new_mapping_profile,int *is_allocated)
{
  int rc = BCM_E_NONE;
  ...
  BCMDNX_INIT_FUNC_DEFS;
  rc = _bcm_dpp_am_template_ingress_flow_tc_mapping_get(unit,dest_ndx,&tmp_old_mapping_profile);
  BCMDNX_IF_ERR_EXIT(rc);

  rc = dpp_am_template_exchange(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_ingress_flow_tc_mapping,0,tc_mapping,tmp_old_mapping_profile,&tmp_is_last,new_mapping_profile,is_allocated);
  BCMDNX_IF_ERR_EXIT(rc);
...
STATIC int
_bcm_dpp_am_template_ingress_uc_tc_mapping_get(int unit, int dest_ndx ,uint32 *profile_ndx)
{
  bcm_error_t rc = BCM_E_NONE;
  uint32 soc_sand_rv;
  ...
  soc_sand_rv = (MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_ipq_tc_profile_get,(unit,0, dest_ndx , profile_ndx)));    
  BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
...
int _bcm_dpp_am_template_ingress_uc_tc_mapping_data_get(int unit,int dest_ndx,SOC_TMC_ITM_TC_MAPPING *tc_mapping)
{
  uint32 profile_ndx;
  int rc = BCM_E_NONE;

  BCMDNX_INIT_FUNC_DEFS;
  rc = _bcm_dpp_am_template_ingress_uc_tc_mapping_get(unit,dest_ndx,&profile_ndx);
  BCMDNX_IF_ERR_EXIT(rc);

  rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_ingress_uc_tc_mapping,profile_ndx,tc_mapping);
...
int _bcm_dpp_am_template_ingress_uc_tc_mapping_init(int unit,int template_init_id,const SOC_TMC_ITM_TC_MAPPING* tc_mapping)
{
  ...
  result = dpp_am_template_allocate_group(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_ingress_uc_tc_mapping, flags, tc_mapping, _DPP_AM_TEMPLATE_INGRESS_UC_TC_MAPPING_MAX_ENTITIES(unit), &is_allocated,&template);
  BCMDNX_IF_ERR_EXIT(result);
...
int _bcm_dpp_am_template_ingress_uc_tc_mapping_exchange(int unit, int dest_ndx, SOC_TMC_ITM_TC_MAPPING *tc_mapping, uint32 *old_mapping_profile, int *is_last, int *new_mapping_profile,int *is_allocated)
{
  int rc = BCM_E_NONE;
  uint32 tmp_old_mapping_profile;
  int tmp_is_last;
  ...
  BCMDNX_INIT_FUNC_DEFS;
  rc = _bcm_dpp_am_template_ingress_uc_tc_mapping_get(unit,dest_ndx,&tmp_old_mapping_profile);
  BCMDNX_IF_ERR_EXIT(rc);

  rc = dpp_am_template_exchange(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_ingress_uc_tc_mapping,0,tc_mapping,tmp_old_mapping_profile,&tmp_is_last,new_mapping_profile,is_allocated);
  BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_fc_generic_pfc_mapping_init(int unit, int template_id, SOC_TMC_FC_PFC_GENERIC_BITMAP *generic_bm)
{
  int rc = BCM_E_NONE;
  int is_allocated;
  BCMDNX_INIT_FUNC_DEFS;
  ...
  rc = dpp_am_template_allocate_group(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_fc_generic_pfc_mapping, SHR_TEMPLATE_MANAGE_SET_WITH_ID, 
...
int _bcm_dpp_am_template_fc_generic_pfc_mapping_data_get(int unit, int generic_bm_id, SOC_TMC_FC_PFC_GENERIC_BITMAP *generic_bm)
{
  int rc = BCM_E_NONE;

  BCMDNX_INIT_FUNC_DEFS;

  rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_fc_generic_pfc_mapping,generic_bm_id,generic_bm);
  BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_fc_generic_pfc_mapping_exchange(int unit, int old_generic_bm_id, SOC_TMC_FC_PFC_GENERIC_BITMAP *new_generic_bm, int *is_last, int *new_mapping_profile,int *is_allocated)
{
  int rc = BCM_E_NONE;
  int flags;
  int tmp_is_last;
  ...
  rc = dpp_am_template_exchange(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_fc_generic_pfc_mapping, flags, new_generic_bm,
...
int _bcm_dpp_am_template_ttl_scope_init(int unit, int template_init_id, uint16 *ttl)
{
    ...
    /* Add all entities to template init id with given data */
    result = dpp_am_template_allocate_group(unit, BCM_DPP_AM_DEFAULT_CORE_ID, template_type, flags, ttl, _DPP_AM_TEMPLATE_TTL_SCOPE_MAX_ENTITIES, &is_allocated,&template);
    BCMDNX_IF_ERR_EXIT(result);
...
int _bcm_dpp_am_template_ttl_scope_exchange(int unit, int ttl_scope_index, uint16 *ttl, int *is_last, int *new_ttl_scope_index, int *is_allocated)
{
    ...
    rc = dpp_am_template_exchange(unit, BCM_DPP_AM_DEFAULT_CORE_ID, template_type, 0, ttl, ttl_scope_index, &tmp_is_last, new_ttl_scope_index, is_allocated);
    BCMDNX_IF_ERR_EXIT(rc);
...
_bcm_dpp_am_template_ttl_scope_index_ttl_mapping_get(int unit, int ttl_scope_index, uint16 *ttl)
{
    ...
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, template_type, ttl_scope_index, ttl);
    BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_pool_cnm_queue_init(int unit, SOC_TMC_CNM_CP_PROFILE_INFO *profile_info, int template)
{
  ...
  rc = dpp_am_template_allocate_group(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_cnm_queue_profile, SHR_TEMPLATE_MANAGE_SET_WITH_ID, 
...
int _bcm_dpp_am_template_pool_cnm_queue_profile_data_get(int unit, int profile_id, SOC_TMC_CNM_CP_PROFILE_INFO *profile_info)
{
  ...
  rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_cnm_queue_profile, profile_id, profile_info);
  BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_pool_cnm_queue_profile_exchange(int unit, int old_profile_id, SOC_TMC_CNM_CP_PROFILE_INFO *new_profile_info, int *new_mapping_profile,int *is_allocated)
{
  ...
  rc = dpp_am_template_exchange(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_cnm_queue_profile, 0, new_profile_info,
...
STATIC int
_bcm_dpp_am_template_ptp_port_profile_mapping_get(int unit, int port_ndx, SOC_PPD_PTP_IN_PP_PORT_PROFILE *profile_ndx)
{
  uint32 soc_sand_rv;
...
int _bcm_dpp_am_template_ptp_port_profile_init(int unit, SOC_PPD_PTP_IN_PP_PORT_PROFILE profile_ndx, SOC_PPD_PTP_PORT_INFO *profile_data)
{
  int rc = BCM_E_NONE;
  int is_allocated;
  uint32 flags = SHR_TEMPLATE_MANAGE_SET_WITH_ID;
  ...
  rc = dpp_am_template_allocate_group(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_ptp_port_profile, flags, profile_data, 
...
int _bcm_dpp_am_template_ptp_port_profile_exchange(int unit, int port_ndx, SOC_PPD_PTP_PORT_INFO *new_profile_data, 
                                                   SOC_PPD_PTP_IN_PP_PORT_PROFILE *new_profile_ndx)
{
    int rc = 0;
    int is_last;
    int is_allocated;
    SOC_PPD_PTP_IN_PP_PORT_PROFILE old_profile_ndx;
    ...
    rc = dpp_am_template_exchange(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_ptp_port_profile, 0, new_profile_data, old_profile_ndx, &is_last, (int *)new_profile_ndx, &is_allocated);
    BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_ptp_port_profile_tdata_get(int unit, SOC_PPD_PTP_IN_PP_PORT_PROFILE profile_ndx, SOC_PPD_PTP_PORT_INFO* profile_data)
{
    int rc = BCM_E_NONE;

    BCMDNX_INIT_FUNC_DEFS;

    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_ptp_port_profile, profile_ndx, profile_data);
    BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_l3_rif_mact_ermination_combination_init(int unit, uint32 *profile_info, int template)
{
  ...
  rc = dpp_am_template_allocate_group(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_l3_rif_mac_termination_combination, SHR_TEMPLATE_MANAGE_SET_WITH_ID, 
...
int _bcm_dpp_am_template_l3_rif_mac_termination_combination_data_get(int unit, int profile_id, uint32 *profile_info)
{
  ...
  rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_l3_rif_mac_termination_combination, profile_id, profile_info);
  BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_l3_rif_mac_termination_combination_alloc(int unit, int flags, uint32 *profile_info, uint8 *new_profile, int *is_allocated)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_allocate(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_l3_rif_mac_termination_combination, flags, profile_info, is_allocated, &profile);
    (*new_profile) = profile;    
    BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_l3_rif_mac_termination_combination_free(int unit, uint8 profile, int *is_last)
{
    int rc = BCM_E_NONE;
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_l3_rif_mac_termination_combination,profile_as_int,is_last);
    BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_l3_rif_mac_termination_combination_exchange(int unit, int old_profile_id, uint32 *new_profile_info, int *new_mapping_profile,int *is_allocated)
{
  int rc = BCM_E_NONE;
  int tmp_is_last;
  int tmp_old_profile_id;
  ...
  rc = dpp_am_template_exchange(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_l3_rif_mac_termination_combination, 0, new_profile_info,
                              tmp_old_profile_id, &tmp_is_last, new_mapping_profile, is_allocated);
  BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_vrrp_alloc(int unit, SOC_PPC_VRRP_CAM_INFO *cam_info, int *is_first, int *profile_ndx)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_allocate(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_l3_vrrp, 0, cam_info, is_first, profile_ndx);
    BCMDNX_IF_ERR_EXIT(rc);
exit:
    BCMDNX_FUNC_RETURN;
}
 ...
 *      BCM_E_*
 */
int _bcm_dpp_am_template_vrrp_alloc_all(int unit, int profile_index, int *alloc_count)
{
    int rc = BCM_E_NONE;
    uint32 ref_count;
    int is_allocated;
    ...
    rc = dpp_am_template_ref_count_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_l3_vrrp, profile_index, &ref_count);
    BCMDNX_IF_ERR_EXIT(rc);

    *alloc_count = _DPP_AM_TEMPLATE_L3_VRRP_PROFILE_MAX_ENTITIES(unit) - ref_count;

    rc = dpp_am_template_allocate_group(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_l3_vrrp, flags, &cam_info, *alloc_count, &is_allocated, &profile_index);
    BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_vrrp_get(int unit, int profile_ndx, SOC_PPC_VRRP_CAM_INFO *cam_info)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_l3_vrrp, profile_ndx, cam_info);
...
int _bcm_dpp_am_template_vrrp_dealloc(int unit, int profile_ndx, int *is_last)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_l3_vrrp, profile_ndx, is_last);
    BCMDNX_IF_ERR_EXIT(rc);
 ...
 *      BCM_E_*
 */
int _bcm_dpp_am_template_vrrp_dealloc_all(int unit, int profile_index, int *dealloc_count)
{
    int rc = BCM_E_NONE;
    int is_last = 0;
    int count = 0;
    ...
    /* Dealloc until the last entry is dealloced. */
    do {
        rc = _bcm_dpp_am_template_vrrp_dealloc(unit, profile_index, &is_last);
        BCMDNX_IF_ERR_EXIT(rc);
        count++;
    }
    while (!is_last); 
...
int _bcm_dpp_am_template_vrrp_index_get(int unit, SOC_PPC_VRRP_CAM_INFO *cam_info, int *profile_ndx)
{
    ...
    rc = dpp_am_template_template_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_l3_vrrp, cam_info, profile_ndx);
    BCMDNX_IF_ERR_EXIT(rc);
...
int bcm_dpp_am_template_eedb_roo_ll_format_eth_type_index_get(int unit, int eth_type_index_ndx, SOC_PPC_EG_ENCAP_ETHER_TYPE_INDEX_INFO* data)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_eedb_roo_ll_format_eth_type_index, eth_type_index_ndx, data);
...
{    
    ...
    rv = dpp_am_template_allocate(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_eedb_roo_ll_format_eth_type_index, flags, data, is_allocated, eth_type_index_ndx);    
    BCMDNX_IF_ERR_EXIT(rv); 
...
int bcm_dpp_am_template_eedb_roo_ll_format_eth_type_index_free(int unit, int eth_type_index_ndx, int *is_last)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_eedb_roo_ll_format_eth_type_index, eth_type_index_ndx, is_last);
    BCMDNX_IF_ERR_EXIT(rc);
exit:
    BCMDNX_FUNC_RETURN;
}
