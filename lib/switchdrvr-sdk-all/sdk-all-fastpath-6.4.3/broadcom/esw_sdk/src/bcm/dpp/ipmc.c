...
bcm_dpp_ipmc_info_t  ipmc_info[BCM_MAX_NUM_UNITS];
...
#define DPP_IPMC_UNIT_VALID_CHECK \
    do {                                                                \
        ...
        if (IPMC_CNTL(unit).init == FALSE) return BCM_E_INIT;           \
        if (IPMC_CNTL(unit).init != TRUE) return IPMC_CNTL(unit).init;  \
    } while (0);
  ...
  )
{
    ...
    else{/* ARAD VID 0 is ok to indicate ignore vlan */
  ...
  )
{
    ...
    soc_sand_rv = soc_ppd_frwrd_mact_entry_remove(unit,&route_key);
    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    ...
    int unit)
{
   ...
   _BCM_DPP_SWITCH_API_START(unit); 
    ...
    status = soc_dpp_wb_engine_init_buffer(unit,SOC_DPP_WB_ENGINE_BUFFER_ARAD_IPMC_INFO_INIT);
    BCMDNX_IF_ERR_EXIT(status);

    if (!SOC_WARM_BOOT(unit)) {
       status = bcm_petra_ipmc_enable(unit, 1); /* Enable IPMC Support */
       BCMDNX_IF_ERR_EXIT(status);
    }
...
{
    bcm_error_t rv;
    bcm_dpp_ipmc_info_t *ipmc_i;

    BCMDNX_INIT_FUNC_DEFS;
    _BCM_DPP_SWITCH_API_START(unit);
    DPP_IPMC_UNIT_VALID_CHECK;

    ipmc_i = &IPMC_CNTL(unit);
    ...
    int enable)
{
    ...
    BCMDNX_IF_ERR_EXIT(SOC_DPP_WB_ENGINE_SET_VAR(unit, SOC_DPP_WB_ENGINE_VAR_ARAD_IPMC_INFO_INIT, &(ipmc_i->init)));
