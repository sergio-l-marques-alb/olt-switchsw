    ...
    switch (version) {
        case BCM_DPP_WB_QOS_VERSION_1_0:
            ...
            if (SOC_IS_ARADPLUS(unit)) {
                ...
                entry_size += SHR_BITALLOCSIZE((_BCM_QOS_MAP_EGR_RESOLVED_DP_MAX*_BCM_QOS_MAP_TC_MAX*SOC_PPC_PROFILE_MGMT_NOF_INLIF_PROFILES_TRANSFERED_TO_EGRESS) * 
...
#ifdef BCM_88660
            if (SOC_IS_ARADPLUS(unit)) {
                size = SHR_BITALLOCSIZE((_BCM_QOS_MAP_EGR_RESOLVED_DP_MAX*_BCM_QOS_MAP_TC_MAX*SOC_PPC_PROFILE_MGMT_NOF_INLIF_PROFILES_TRANSFERED_TO_EGRESS) * 
                    SOC_DPP_CONFIG(unit)->qos.nof_egr_dscp_exp_marking);
...
_bcm_dpp_wb_qos_update_egr_dscp_exp_marking_bitmap_state(int unit, int profile_ndx)
{
    ...
    switch(wb_info->version) {
        case BCM_DPP_WB_QOS_VERSION_1_0:
            end_index = SHR_BITALLOCSIZE((_BCM_QOS_MAP_EGR_RESOLVED_DP_MAX*_BCM_QOS_MAP_TC_MAX*SOC_PPC_PROFILE_MGMT_NOF_INLIF_PROFILES_TRANSFERED_TO_EGRESS)*(profile_ndx + 1)) - 1;
            if (profile_ndx == 0) {
                start_index = 0;
            }
            else {
                start_index = SHR_BITALLOCSIZE((_BCM_QOS_MAP_EGR_RESOLVED_DP_MAX*_BCM_QOS_MAP_TC_MAX*SOC_PPC_PROFILE_MGMT_NOF_INLIF_PROFILES_TRANSFERED_TO_EGRESS)*profile_ndx) - 1;
            }
