    ...
    BCM_DPP_ASSYMETRIC_CORES_ITER(BCM_CORE_ALL, core) {
        switch(wb_info->version) {
            case BCM_DPP_WB_COSQ_VERSION_1_0:
                ...
                vsq_category = gbl_state->config[core].vsq_category_mode;
                ucast_qid_start = gbl_state->config[core].ucast_qid_start;
                ucast_qid_end = gbl_state->config[core].ucast_qid_end;
        ...
        /* set cosq state */
        rc = _bcm_petra_cosq_ucast_qid_range_set(unit, core, ucast_qid_start, ucast_qid_end);
        BCMDNX_IF_ERR_EXIT(rc);
        ...
        if ( (ucast_qid_start >= 0) && (ucast_qid_end > 0) && (ucast_qid_end > ucast_qid_start) ) {
            ...
            rc = bcm_dpp_am_cosq_queue_region_config_get(unit, core, queue_region_config, DPP_DEVICE_COSQ_QUEUE_REGION_UNICAST);
            BCMDNX_IF_ERR_EXIT(rc);
            ...
            rc = _bcm_dpp_am_cosq_process_queue_region(unit, core, queue_region_config);
            BCMDNX_IF_ERR_EXIT(rc);
        }
        ...
        if ( !((mcast_queue_range.start == 0) && (mcast_queue_range.end == 0))  &&
            ...
            rc = bcm_dpp_am_cosq_queue_region_config_get(unit, core, queue_region_config, DPP_DEVICE_COSQ_QUEUE_REGION_MULTICAST);
            BCMDNX_IF_ERR_EXIT(rc);
        ...
        /* ISQ */ 
        if (isp_info.enable) {
            rc = bcm_dpp_am_cosq_queue_region_config_get(unit, core, queue_region_config, DPP_DEVICE_COSQ_QUEUE_REGION_ISQ);
            BCMDNX_IF_ERR_EXIT(rc);
...
_bcm_dpp_wb_cosq_save_gbl_state(int unit)
{
    ...
    BCM_DPP_ASSYMETRIC_CORES_ITER(BCM_CORE_ALL, core) {
        rc = _bcm_petra_cosq_ucast_qid_range_get(unit, core, &(gbl_state->config[core].ucast_qid_start), &(gbl_state->config[core].ucast_qid_end));
        BCMDNX_IF_ERR_EXIT(rc);

        rc = bcm_petra_cosq_fmq_vsq_category_mode_get(unit, core, (bcm_fabric_vsq_category_mode_t *)&(gbl_state->config[core].vsq_category_mode));
        BCMDNX_IF_ERR_EXIT(rc);
    }
exit:
    BCMDNX_FUNC_RETURN;
...
_bcm_dpp_wb_cosq_update_vsq_data(int unit, int core, int category_mode)
{
    ...
    BCM_DPP_ASSYMETRIC_CORES_ITER(core, core_index) {
        switch(wb_info->version) {
            case BCM_DPP_WB_COSQ_VERSION_1_0:
                gbl_state = (bcm_dpp_wb_cosq_gbl_data_t *)(wb_info->scache_ptr + wb_info->gbl_data_off);
                gbl_state->config[core_index].vsq_category_mode = category_mode;

                data = (uint8 *)&(gbl_state->config[core_index].vsq_category_mode);
                offset = (uint32)(data - wb_info->scache_ptr);
...
_bcm_dpp_wb_cosq_update_ucast_data(int unit, int core, int qid_start, int qid_end)
{
    ...
    BCM_DPP_ASSYMETRIC_CORES_ITER(core, core_index) {
        switch(wb_info->version) {
            case BCM_DPP_WB_COSQ_VERSION_1_0:
                gbl_state = (bcm_dpp_wb_cosq_gbl_data_t *)(wb_info->scache_ptr + wb_info->gbl_data_off);
                gbl_state->config[core_index].ucast_qid_start = qid_start;
                gbl_state->config[core_index].ucast_qid_end = qid_end;

                data = (uint8 *)&(gbl_state->config[core_index].ucast_qid_start);
                offset = (uint32)(data - wb_info->scache_ptr);
