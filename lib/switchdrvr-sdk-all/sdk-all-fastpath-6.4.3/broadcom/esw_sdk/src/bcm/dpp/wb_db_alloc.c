 ...
 **********************/
STATIC int 
_bcm_wb_alloc_restore_pool_vsi_vlan(int unit,  
                                    bcm_dpp_am_pool_info_t *pool_info, 
                                    uint32 entry_id,
                                    uint32 count)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    vlan = (bcm_vlan_t)entry_id;
    rc = bcm_dpp_am_l2_vsi_vlan_alloc(unit, SHR_RES_ALLOC_WITH_ID, &vlan);
    BCMDNX_IF_ERR_EXIT(rc);
exit:
    BCMDNX_FUNC_RETURN;
...
STATIC int 
_bcm_wb_alloc_restore_pool_vsi_mstp(int unit,  
                                    bcm_dpp_am_pool_info_t *pool_info, 
                                    uint32 entry_id, 
                                    uint32 count)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    vsi = (bcm_vlan_t)entry_id;
    rc = bcm_dpp_am_l2_vpn_vsi_alloc(unit, SHR_RES_ALLOC_WITH_ID, &vsi);
    BCMDNX_IF_ERR_EXIT(rc);
exit:
    BCMDNX_FUNC_RETURN;
}
                                   ...
                                   uint32 count)
{
    ...
    int *act_meter_group = (int*)&entry_id; 
    
    BCMDNX_INIT_FUNC_DEFS;
   
    rc = bcm_dpp_am_meter_alloc(unit, SHR_RES_ALLOC_WITH_ID, meter_group, (int)count, (int*)&entry_id, act_meter_group); 
    BCMDNX_IF_ERR_EXIT(rc);
                                         ...
                                         shr_wb_alloc_restore_cb_t *cb)
{
    ...
    switch (res_id) 
    {
        case dpp_am_res_vsi_vlan:
            *cb = _bcm_wb_alloc_restore_pool_vsi_vlan;
            break;
        case dpp_am_res_vsi_mstp:
            *cb = _bcm_wb_alloc_restore_pool_vsi_mstp;
            break;
...
#ifdef BCM_WARM_BOOT_SUPPORT
    if (res_id >= (dpp_res_arad_pool_egress_encap_bank_start) && \
        res_id <= (dpp_res_arad_pool_egress_encap_bank_end)) {
        *cb = _bcm_wb_alloc_restore_pool;
        rc = BCM_E_NONE;
    }
    else if (res_id >= (dpp_res_arad_pool_ingress_lif_bank_start) && \
             res_id <= (dpp_res_arad_pool_ingress_lif_bank_end)) {
        *cb = _bcm_wb_alloc_restore_pool;
        rc = BCM_E_NONE;
    }
#endif /*BCM_WARM_BOOT_SUPPORT*/
    ...
    if (_BCM_DPP_AM_RES_ID_IS_EGRESS_GLOBAL_LIF(unit, res_id)
        || _BCM_DPP_AM_RES_ID_IS_INGRESS_GLOBAL_LIF(unit, res_id)) {
        *cb = _bcm_wb_alloc_restore_pool;
        rc = BCM_E_NONE;
    }
