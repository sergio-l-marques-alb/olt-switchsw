...
#define DPP_FRWRD_IP_ALL_VRFS_ID SOC_PPD_FRWRD_IP_ALL_VRFS_ID

#define DPP_SWITCH_MAX_NUM_ADDITIONAL_TPIDS  4

#ifdef BCM_WARM_BOOT_SUPPORT
bcm_dpp_switch_state_t *wb_switch_state[SOC_MAX_NUM_DEVICES] = {0};
#endif
...
_bcm_dpp_switch_init(int unit)
{
...
#ifdef BCM_WARM_BOOT_SUPPORT    
    if (wb_switch_state[unit] == NULL) {
        BCMDNX_ALLOC(wb_switch_state[unit], sizeof(bcm_dpp_switch_state_t), "Switch Info");
        if (wb_switch_state[unit] == NULL) {
           BCMDNX_ERR_EXIT_MSG(BCM_E_MEMORY, (_BSL_BCM_MSG("Unable to allocate switch state")));
        }
        sal_memset(wb_switch_state[unit], 0, sizeof(bcm_dpp_switch_state_t));
        SOC_DPP_STATE(unit)->switch_state                    = wb_switch_state[unit];
        SOC_DPP_STATE(unit)->switch_state->scache_consistent = 0;
        SOC_DPP_STATE(unit)->switch_state->init              = TRUE;
    }
#endif

#ifdef BCM_WARM_BOOT_SUPPORT
    rv = _bcm_dpp_wb_switch_state_init(unit);
    if (rv != BCM_E_NONE) {
       LOG_ERROR(BSL_LS_BCM_SWITCH,
                 (BSL_META_U(unit,
    ...
    /* Validate scache is consistent if warmboot */
    if (SOC_IS_PETRAB(unit) && SOC_DPP_STATE(unit)->switch_state->scache_consistent != 0) {
       LOG_ERROR(BSL_LS_BCM_SWITCH,
                 (BSL_META_U(unit,
                             "%s: Unit %d, scache inconsistent - coldboot required\n"),FUNCTION_NAME(), unit));
        BCMDNX_IF_ERR_EXIT(rv);
    }
#endif /*BCM_ARAD_SUPPORT*/
#endif /*BCM_WARM_BOOT_SUPPORT*/
...
#ifdef BCM_WARM_BOOT_SUPPORT
    rv = soc_dpp_wb_engine_init_buffer(unit, SOC_DPP_WB_ENGINE_BUFFER_SWITCH);
    if (rv != BCM_E_NONE) {
       LOG_ERROR(BSL_LS_BCM_SWITCH,
                 (BSL_META_U(unit,
                             "%s: Unit %d, switch module failed wb_engine init, error 0x%x\n"),FUNCTION_NAME(), unit, rv));
        BCMDNX_IF_ERR_EXIT(rv);
    }
#endif /*BCM_WARM_BOOT_SUPPORT*/
...
_bcm_dpp_switch_detach(int unit)
{
    ...
    if (!SOC_WARM_BOOT(unit)) {
        BCM_FREE(wb_switch_state[unit]);
    }
#endif
#ifdef BCM_WARM_BOOT_SUPPORT
    /* tear down warm boot state */
    BCMDNX_IF_ERR_EXIT(_bcm_dpp_wb_switch_state_deinit(unit));
#endif /* BCM_WARM_BOOT_SUPPORT */
    BCM_EXIT;
exit:
    BCMDNX_FUNC_RETURN;
...
_dpp_switch_eg_trap_enable_mask_get(int unit,
                                 SOC_PPD_TRAP_EG_TYPE trap_code_ndx,
                                 ...
                                 int *is_port_relevant)
{
    ...
    *is_port_relevant = 1;
    ...
    switch (trap_code_ndx)
        {
            case SOC_PPD_TRAP_EG_TYPE_VSI_MEMBERSHIP:
                *trap_enable_mask = SOC_PPD_EG_FILTER_PORT_ENABLE_VSI_MEMBERSHIP;
                break;
            case SOC_PPD_TRAP_EG_TYPE_MTU_VIOLATION:
                *trap_enable_mask = SOC_PPD_EG_FILTER_PORT_ENABLE_MTU;
                break;           
    default:
        *is_port_relevant = 0;
    }
...
_dpp_eg_trap_code_switch_control_set(int unit,
                                  SOC_PPD_TRAP_CODE trap_code_ndx,
                                  int cpu_drop_flood, /* 1:cpu */
                                  int enable)
{
    int                               rv = BCM_E_NONE;  
    ...
    if (enable == FALSE)
    {
        eg_action_profile = SOC_PPD_TRAP_EG_NO_ACTION;      
    }
    else
    {       
        if (cpu_drop_flood == DPP_SWITCH_TRAP_TO_CPU) {
                                 ...
                                 int *is_port_relevant)
{
    ...
    *is_port_relevant = 1;
    ...
    switch (trap_code_ndx)
        {
            case SOC_PPD_TRAP_CODE_MY_ARP:
            case SOC_PPD_TRAP_CODE_ARP:
            case SOC_PPD_TRAP_CODE_MY_MAC_AND_ARP:
                *trap_enable_mask = SOC_PPD_LLP_TRAP_PORT_ENABLE_ARP;
                break;
            case SOC_PPD_TRAP_CODE_IGMP_MEMBERSHIP_QUERY:
    case SOC_PPD_TRAP_CODE_IGMP_REPORT_LEAVE_MSG:
    case SOC_PPD_TRAP_CODE_IGMP_UNDEFINED:
        *trap_enable_mask = SOC_PPD_LLP_TRAP_PORT_ENABLE_IGMP;
        break;
    case SOC_PPD_TRAP_CODE_DHCP_SERVER:
    case SOC_PPD_TRAP_CODE_DHCP_CLIENT:
    case SOC_PPD_TRAP_CODE_DHCPV6_SERVER:
    case SOC_PPD_TRAP_CODE_DHCPV6_CLIENT:
        *trap_enable_mask = SOC_PPD_LLP_TRAP_PORT_ENABLE_DHCP;
        break;
    case SOC_PPD_TRAP_CODE_ETH_FL_ICMPV6_MLD_MC_LISTENER_QUERY:
    case SOC_PPD_TRAP_CODE_ETH_FL_ICMPV6_MLD_REPORT_DONE:
    case SOC_PPD_TRAP_CODE_ETH_FL_ICMPV6_MLD_UNDEFINED:
    case SOC_PPD_TRAP_CODE_ICMPV6_MLD_MC_LISTENER_QUERY:
    case SOC_PPD_TRAP_CODE_ICMPV6_MLD_REPORT_DONE_MSG:
    case SOC_PPD_TRAP_CODE_ICMPV6_MLD_UNDEFINED:
        *trap_enable_mask = SOC_PPD_LLP_TRAP_PORT_ENABLE_MLD;
        break;
    default:
        *is_port_relevant = 0;
    }
...
{  
  ...
  uint32             soc_sand_dev_id;
  uint32              soc_sand_rv;
  SOC_PPD_LLP_TRAP_ARP_INFO arp_info;
                                           ...
                                           int priority /* 0-7 */)
{
    ...
    if ((profile_info.dest_info.frwrd_dest.type == SOC_PB_PP_FRWRD_DECISION_TYPE_UC_PORT) &&
        (profile_info.dest_info.frwrd_dest.dest_id == SOC_PETRA_FRST_CPU_PORT_ID) &&
        (profile_info.cos_info.tc != 0))
    {
      /* if it is trapped to cpu - overwrite tc*/
      profile_info.bitmap_mask |= SOC_PPD_TRAP_ACTION_PROFILE_OVERWRITE_TC;
    }
    else
    {
                                  ...
                                  int enable)
{
    ...
    uint32                              soc_sand_rv;
    SOC_PPD_LLP_TRAP_PORT_INFO              port_info;
    SOC_PPD_TRAP_FRWRD_ACTION_PROFILE_INFO  profile_info;
    SOC_PPD_TRAP_SNOOP_ACTION_PROFILE_INFO  snoop_info;
    uint32                              trap_enable_mask;
    pbmp_t                              ports_map;
    ...
    *is_enabled = TRUE;
    ...
    if (cpu_drop_flood == DPP_SWITCH_TRAP_TO_CPU || cpu_drop_flood == DPP_SWITCH_SNOOP_TO_CPU) {
        if (((profile_info.dest_info.frwrd_dest.dest_id !=
                 SOC_PETRA_FRST_CPU_PORT_ID) ||
             (profile_info.dest_info.frwrd_dest.type !=
             SOC_PB_PP_FRWRD_DECISION_TYPE_UC_PORT) ||
             !(profile_info.bitmap_mask & SOC_PPD_TRAP_ACTION_PROFILE_OVERWRITE_DEST))
    ...
    if (cpu_drop_flood == DPP_SWITCH_DROP) {
        if (((profile_info.dest_info.frwrd_dest.type != SOC_PPD_FRWRD_DECISION_TYPE_DROP) && 
    ...
    if (cpu_drop_flood == DPP_SWITCH_FLOOD) {
        if (profile_info.dest_info.frwrd_dest.type != SOC_PPD_FRWRD_DECISION_TYPE_MC ||
            profile_info.dest_info.frwrd_dest.dest_id != 0 ||
            profile_info.dest_info.add_vsi != TRUE ||
            !(profile_info.bitmap_mask & SOC_PPD_TRAP_ACTION_PROFILE_OVERWRITE_DEST)
            ) {
...
_dpp_trap_to_cpu_if_dos_attack(int unit, bcm_switch_control_t bcm_type, int enable)
{
    ...
    switch (bcm_type)
    {
        case bcmSwitchDosAttackSipEqualDip:
            rv = _dpp_trap_code_switch_control_set(unit, 
                                            SOC_PPD_TRAP_CODE_IPV4_TERM_SIP_EQUAL_DIP, DPP_SWITCH_TRAP_TO_CPU, to_enable);
            BCMDNX_IF_ERR_EXIT(rv);
            break;
    ...
    case bcmSwitchDosAttackV4FirstFrag:
        rv = _dpp_trap_code_switch_control_set(unit,
                                               SOC_PPD_TRAP_CODE_TCP_FRAGMENT_OFFSET_LT8, DPP_SWITCH_TRAP_TO_CPU, to_enable);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchDosAttackTcpFlags:
        rv = _dpp_trap_code_switch_control_set(unit,
                                               SOC_PPD_TRAP_CODE_TCP_SN_FLAGS_ZERO, DPP_SWITCH_TRAP_TO_CPU, to_enable);
        BCMDNX_IF_ERR_EXIT(rv);
        rv = _dpp_trap_code_switch_control_set(unit,
                                               SOC_PPD_TRAP_CODE_TCP_SN_ZERO_FLAGS_SET, DPP_SWITCH_TRAP_TO_CPU, to_enable);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchDosAttackL4Port:
        rv = _dpp_trap_code_switch_control_set(unit,
                                               SOC_PPD_TRAP_CODE_TCP_EQUAL_PORTS, DPP_SWITCH_TRAP_TO_CPU, to_enable);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchDosAttackTcpFrag:
        rv = _dpp_trap_code_switch_control_set(unit,
                                               SOC_PPD_TRAP_CODE_TCP_FRAGMENT_INCOMPLETE_HEADER, DPP_SWITCH_TRAP_TO_CPU, to_enable);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchDosAttackIcmp:
        rv = _dpp_trap_code_switch_control_set(unit,
                                               SOC_PPD_TRAP_CODE_ICMP_DATA_GT_576, DPP_SWITCH_TRAP_TO_CPU, to_enable);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    ...
    case bcmSwitchDosAttackMACSAEqualMACDA:
        rv = _dpp_trap_code_switch_control_set(unit,
                                               SOC_PPD_TRAP_CODE_SA_EQUALS_DA, DPP_SWITCH_TRAP_TO_CPU, to_enable);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    ...
    case bcmSwitchDosAttackIcmpFragments:
        rv = _dpp_trap_code_switch_control_set(unit,
                                               SOC_PPD_TRAP_CODE_ICMP_FRAGMENTED, DPP_SWITCH_TRAP_TO_CPU, to_enable);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchDosAttackUdpPortsEqual:
        rv = _dpp_trap_code_switch_control_set(unit,
                                               SOC_PPD_TRAP_CODE_UDP_EQUAL_PORTS, DPP_SWITCH_TRAP_TO_CPU, to_enable);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    ...
    case bcmSwitchDosAttackSynFlood:
        rv = _dpp_trap_code_switch_control_set(unit,
                                               SOC_PPD_TRAP_CODE_TCP_SYN_FIN, DPP_SWITCH_TRAP_TO_CPU, to_enable);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchDosAttackL3Header:
        rv = _dpp_trap_code_switch_control_set(unit,
                                               SOC_PPD_TRAP_CODE_IPV4_HEADER_LENGTH_ERROR, DPP_SWITCH_TRAP_TO_CPU, to_enable);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
...
int static _bcm_dpp_switch_L2LearnMode_set(int unit, int arg)
{
    ...
    /* learning distribution is not needed if event handled by CPU and not OLP or when learning mode is egress distributed */
    if (!(arg & BCM_L2_EGRESS_INDEPENDENT) && !(arg & BCM_L2_LEARN_CPU) && !(arg & BCM_L2_LEARN_DISABLE)) {
        soc_sand_rv = soc_ppd_frwrd_mact_learn_msgs_distribution_info_get(soc_sand_dev_id,
                                                                          &distr_info);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);

        if (distr_info.header_type == SOC_PPD_FRWRD_MACT_MSG_HDR_TYPE_RAW) {
        ...
        sal_memcpy(&(oper_mode_info.learn_msgs_info.info), &distr_info,
                   sizeof(SOC_PPD_FRWRD_MACT_MSG_DISTR_INFO));
    ...
    case BCM_L2_INGRESS_CENT:
        oper_mode_info.learning_mode =
           SOC_PPD_FRWRD_MACT_LEARNING_MODE_INGRESS_CENTRALIZED;
        break;
    case BCM_L2_INGRESS_DIST:
        oper_mode_info.learning_mode =
           SOC_PPD_FRWRD_MACT_LEARNING_MODE_INGRESS_DISTRIBUTED;
        break;
    case BCM_L2_EGRESS_DIST:
        oper_mode_info.learning_mode =
           SOC_PPD_FRWRD_MACT_LEARNING_MODE_EGRESS_DISTRIBUTED;
        break;
    case BCM_L2_EGRESS_CENT:
        oper_mode_info.learning_mode =
           SOC_PPD_FRWRD_MACT_LEARNING_MODE_EGRESS_CENTRALIZED;
        break;
    case BCM_L2_EGRESS_INDEPENDENT:
        oper_mode_info.learning_mode =
           SOC_PPD_FRWRD_MACT_LEARNING_MODE_EGRESS_INDEPENDENT;
        break;
    ...
    if(oper_mode_info.shadow_mode >= SOC_PPD_NOF_FRWRD_MACT_SHADOW_MODES)
    {
      oper_mode_info.shadow_mode = SOC_PPD_FRWRD_MACT_SHADOW_MODE_NONE;
      oper_mode_info.shadow_msgs_info.type = 
          SOC_PPD_FRWRD_MACT_EVENT_PATH_TYPE_MSG_AGGR;
    }
    /* event handler can be OLP or CPU */
    oper_mode_info.learn_msgs_info.type = 
    (arg & BCM_L2_LEARN_CPU) ? SOC_PPD_FRWRD_MACT_EVENT_PATH_TYPE_DIRECT_ACCESS : SOC_PPD_FRWRD_MACT_EVENT_PATH_TYPE_MSG_AGGR;
...
int static _bcm_dpp_switch_L2LearnMode_get(int unit, int *arg)
{
    ...
    case SOC_PPD_FRWRD_MACT_LEARNING_MODE_INGRESS_CENTRALIZED:
        *arg = BCM_L2_INGRESS_CENT;
        break;
    case SOC_PPD_FRWRD_MACT_LEARNING_MODE_INGRESS_DISTRIBUTED:
        *arg = BCM_L2_INGRESS_DIST;
        break;
    case SOC_PPD_FRWRD_MACT_LEARNING_MODE_EGRESS_DISTRIBUTED:
        *arg = BCM_L2_EGRESS_DIST;
        break;
    case SOC_PPD_FRWRD_MACT_LEARNING_MODE_EGRESS_CENTRALIZED:
        *arg = BCM_L2_EGRESS_CENT;
        break;
    case SOC_PPD_FRWRD_MACT_LEARNING_MODE_EGRESS_INDEPENDENT:
    ...
    if (oper_mode_info.learn_msgs_info.type == SOC_PPD_FRWRD_MACT_EVENT_PATH_TYPE_DIRECT_ACCESS) {
...
int static _bcm_dpp_switch_traverse_mode_set(int unit, int arg)
{
    unsigned int soc_sand_dev_id;
    uint32 soc_sand_rv;
    SOC_PPD_FRWRD_MACT_TRAVERSE_MODE_INFO trvrs_mode_info;
    ...
    case bcmSwitchTableUpdateNormal:
         trvrs_mode_info.state = SOC_PPD_FRWRD_MACT_TRAVERSE_STATE_NORMAL;
    break;
    case bcmSwitchTableUpdateRuleAdd:
         trvrs_mode_info.state = SOC_PPD_FRWRD_MACT_TRAVERSE_STATE_AGGREGATE;
    break;
    case bcmSwitchTableUpdateRuleClear:
         trvrs_mode_info.state = SOC_PPD_FRWRD_MACT_TRAVERSE_STATE_RESET;
    break;
    case bcmSwitchTableUpdateRuleCommit:
         trvrs_mode_info.state = SOC_PPD_FRWRD_MACT_TRAVERSE_STATE_RUN;
    break;
    default:
...
int static _bcm_dpp_switch_traverse_mode_get(int unit, int *arg)
{
     ...
     */
    if (trvrs_mode_info.state == SOC_PPD_FRWRD_MACT_TRAVERSE_STATE_AGGREGATE) {
...
_bcm_dpp_switch_control_sync(int unit, int arg)
{
    ...
    rv = soc_sw_db_sync(unit);
    BCMDNX_IF_ERR_EXIT(rv);

    rv = _bcm_sw_db_sync(unit);
    BCMDNX_IF_ERR_EXIT(rv);
                              ...
                              uint8 *dpp_hash_func_id)
{
    ...
    if (SOC_IS_PETRAB(unit)) {
        ...
        case BCM_HASH_CONFIG_CRC8_0x14D:
            *dpp_hash_func_id = 0;
            break;
        case BCM_HASH_CONFIG_CRC8_0x1C3:
            *dpp_hash_func_id = 1;
            break;
        case BCM_HASH_CONFIG_CRC8_0x1CF:
            *dpp_hash_func_id = 2;
            break;
                              ...
                              int *bcm_hash_config)
{
    ...
    if (SOC_IS_PETRAB(unit)) {
        ...
        case 0:
            *bcm_hash_config = BCM_HASH_CONFIG_CRC8_0x14D;
            break;
        case 1:
            *bcm_hash_config = BCM_HASH_CONFIG_CRC8_0x1C3;
            break;
        case 2:
            *bcm_hash_config = BCM_HASH_CONFIG_CRC8_0x1CF;
            break;
        case 3:
                                      ...
                                      int arg)
{
    ...
    SOC_PPD_LAG_HASH_GLOBAL_INFO    info;
    uint32                    soc_sand_rv;
    uint32                   soc_sand_dev_id;

    BCMDNX_INIT_FUNC_DEFS;
    SOC_PPD_LAG_HASH_GLOBAL_INFO_clear(&info);
    soc_sand_dev_id = (unit);
    ...
    case bcmSwitchTrunkHashSeed:
        if (arg >> 8) {
           LOG_WARN(BSL_LS_BCM_SWITCH,
                    (BSL_META_U(unit,
                                "unit %d: ignoring upper bytes for hash"
                                "seed. Using only the last byte\n"), unit));
        }
        info.seed = arg & 0xff; /* take only the last byte */
        break;
                                    ...
                                    bcm_switch_control_t bcm_type, int arg)
{
    ...
    SOC_PPD_LAG_HASH_PORT_INFO  info;
    uint32               soc_sand_dev_id;
    uint32                soc_sand_rv;
    ...
    if (soc_ppd_port > SOC_DPP_DEFS_GET(unit, nof_pp_ports)) {
       LOG_ERROR(BSL_LS_BCM_SWITCH,
                 (BSL_META_U(unit,
                             "unit %d: Invalid local port(%d) to soc_ppd_port(%d) ."
                             "Valid values: 0-%d\n"), unit, local_port, soc_ppd_port, SOC_DPP_DEFS_GET(unit, nof_pp_ports)));
        rv = BCM_E_PARAM;
    }
    BCMDNX_IF_ERR_EXIT(rv);

    SOC_PPD_LAG_HASH_PORT_INFO_clear(&info);
    soc_sand_dev_id = (unit);
    ...
    case bcmSwitchTrunkHashPktHeaderSelect:
        if (arg == BCM_HASH_HEADER_FORWARD) {
            info.first_header_to_parse = SOC_PPD_LAG_HASH_FRST_HDR_FARWARDING;
        } else if (arg == BCM_HASH_HEADER_TERMINATED) {
            info.first_header_to_parse = SOC_PPD_LAG_HASH_FRST_HDR_LAST_TERMINATED;
        } else {
           LOG_ERROR(BSL_LS_BCM_SWITCH,
                     (BSL_META_U(unit,
    ...
    case bcmSwitchTrunkHashMPLSLabelBOS:
        ...
        if ((arg != BCM_HASH_MPLS_LABEL_BOS_START) && (arg != BCM_HASH_MPLS_LABEL_BOS_SKIP)) {
           LOG_ERROR(BSL_LS_BCM_SWITCH,
                     (BSL_META_U(unit,
                                 "unit %d: Invalid value(%d) specified "
                                 "valid: BCM_HASH_MPLS_LABEL_BOS_START or BCM_HASH_MPLS_LABEL_BOS_SKIP\n"), unit, arg));
            rv = BCM_E_PARAM;
        } else {
            info.start_from_bos = 1;
            info.include_bos = (arg == BCM_HASH_MPLS_LABEL_BOS_START ? 1 : 0);
        }
        break;
    default:
        rv = BCM_E_PARAM;
                                    ...
                                    bcm_switch_control_t bcm_type, int *arg)
{
    ...
    case bcmSwitchTrunkHashPktHeaderCount:
        ...
        break;
    case bcmSwitchTrunkHashPktHeaderSelect:
        if (info.first_header_to_parse == SOC_PPD_LAG_HASH_FRST_HDR_FARWARDING) {
            *arg = BCM_HASH_HEADER_FORWARD;
        } else if (info.first_header_to_parse ==
                      SOC_PPD_LAG_HASH_FRST_HDR_LAST_TERMINATED) {
    ...
    case bcmSwitchTrunkHashMPLSLabelBOS:
        ...
        /*if (info.include_bos != info.start_from_bos) {
           LOG_ERROR(BSL_LS_BCM_SWITCH,
                     (BSL_META_U(unit,
                                 "unit %d: Invalid value(%d) (%d) "
                                 "configured\n"), unit, info.include_bos, info.start_from_bos));
            rv = BCM_E_PARAM;
        } else {*/
...
{
    int                 rv = BCM_E_NONE;
    SOC_PPD_HASH_MASK_INFO  info;
    uint32           soc_sand_dev_id;
    uint32            soc_sand_rv;

    BCMDNX_INIT_FUNC_DEFS;
    SOC_PPD_HASH_MASK_INFO_clear(&info);
    soc_sand_dev_id = (unit);
        ...
        if (((arg & BCM_HASH_FCOE_FIELD_DST_ID_LO) && !(arg & BCM_HASH_FCOE_FIELD_DST_ID_HI)) ||
           ...
           LOG_ERROR(BSL_LS_BCM_SWITCH,
            ...
            if(arg & BCM_HASH_FCOE_FIELD_ORIGINATOR_EXCHANGE_ID) {
                info.mask &= ~SOC_PPD_HASH_MASKS_FC_ORG_EX_ID;
            }
            else {
                info.mask |= SOC_PPD_HASH_MASKS_FC_ORG_EX_ID;
            }
            if(arg & BCM_HASH_FCOE_FIELD_RESPONDER_EXCHANGE_ID) {
                info.mask &= ~SOC_PPD_HASH_MASKS_FC_RES_EX_ID;
            }
            else {
                info.mask |= SOC_PPD_HASH_MASKS_FC_RES_EX_ID;
            }
            ...
            if(arg & BCM_HASH_FCOE_FIELD_DST_ID_LO ||
               arg & BCM_HASH_FCOE_FIELD_DST_ID_HI) {
                info.mask &= ~SOC_PPD_HASH_MASKS_FC_DEST_ID;
            }
            else {
                info.mask |= SOC_PPD_HASH_MASKS_FC_DEST_ID;
            }
            ...
            if(arg & BCM_HASH_FCOE_FIELD_SRC_ID_LO ||
               arg & BCM_HASH_FCOE_FIELD_SRC_ID_HI) {
                info.mask &= ~SOC_PPD_HASH_MASKS_FC_SRC_ID;
            }
            else {
                info.mask |= SOC_PPD_HASH_MASKS_FC_SRC_ID;
            }
            ...
            if(arg & BCM_HASH_FCOE_FIELD_VIRTUAL_FABRIC_ID) {
                info.mask &= ~SOC_PPD_HASH_MASKS_FC_VFI;
            }
            else {
                info.mask |= SOC_PPD_HASH_MASKS_FC_VFI;
            }
            ...
            if(arg & BCM_HASH_FCOE_FIELD_CNTAG_FLOW_ID) {
                info.mask &= ~SOC_PPD_HASH_MASKS_FC_SEQ_ID;
            }
            else {
                info.mask |= SOC_PPD_HASH_MASKS_FC_SEQ_ID;
            }
                              ...
                              uint8 *dpp_hash_func_id)
{
    ...
    if (SOC_IS_PETRAB(unit)) {
        switch (bcm_hash_config) {
        case BCM_HASH_CONFIG_CRC16_0x17111:
            *dpp_hash_func_id = 0;
            break;
        case BCM_HASH_CONFIG_CRC16_0x10491:
            *dpp_hash_func_id = 1;
            break;
        case BCM_HASH_CONFIG_CRC16_0x155F5:
            *dpp_hash_func_id = 2;
            break;
        case BCM_HASH_CONFIG_CRC16_0x1013D:
            *dpp_hash_func_id = 3;
            break;
        case BCM_HASH_CONFIG_CRC16_0x13965:
            *dpp_hash_func_id = 4;
                              ...
                              int *bcm_hash_config)
{
    ...
    if (SOC_IS_PETRAB(unit)) {
        switch (dpp_hash_func_id) {
        case 0:
            *bcm_hash_config = BCM_HASH_CONFIG_CRC16_0x17111;
            break;
        case 1:
            *bcm_hash_config = BCM_HASH_CONFIG_CRC16_0x10491;
            break;
        case 2:
            *bcm_hash_config = BCM_HASH_CONFIG_CRC16_0x155F5;
            break;
        case 3:
            *bcm_hash_config = BCM_HASH_CONFIG_CRC16_0x1013D;
            break;
        case 4:
            *bcm_hash_config = BCM_HASH_CONFIG_CRC16_0x13965;
            break;
...
{
    int                                    rv = BCM_E_NONE;
    SOC_PPD_FRWRD_FEC_ECMP_HASH_GLOBAL_INFO    info;
    uint32                               soc_sand_rv;
    uint32                              soc_sand_dev_id;

    BCMDNX_INIT_FUNC_DEFS;
    SOC_PPD_FRWRD_FEC_ECMP_HASH_GLOBAL_INFO_clear(&info);
    soc_sand_dev_id = (unit);
...
{
    int                                  rv = BCM_E_NONE, core;
    SOC_PPD_FRWRD_FEC_ECMP_HASH_PORT_INFO    info;
    uint32                            soc_sand_dev_id;
    uint32                             soc_sand_rv;
    bcm_port_t                           local_port = 0;
    SOC_PPD_PORT                             soc_ppd_port = 0;
    ...
    SOC_PPD_FRWRD_FEC_ECMP_HASH_PORT_INFO_clear(&info);
    soc_sand_dev_id = (unit);
...
_bcm_dpp_switch_control_color_select_port_set(int unit, bcm_port_t port, int arg)
{
    ...
    SOC_PPD_LLP_COS_PORT_INFO_clear(&port_info);
...
{
    uint32                              soc_sand_rv;
    SOC_PPD_FRWRD_TRILL_GLOBAL_INFO     glbl_info;
    uint32                              soc_sand_dev_id;
...
_bcm_dpp_switch_mim_bvid_range_set(int unit, int high, int low)
{
    uint32 soc_sand_rv;
    uint32 soc_sand_dev_id;
    SOC_PPD_BMACT_PBB_TE_VID_RANGE bvid_ranges;
...
_bcm_dpp_switch_control_routed_learning_set(int unit, int bcmAppFlags)
{
    ...
    if (bcmAppFlags & BCM_SWITCH_CONTROL_L3_LEARN_IPV4_UC) {
        ppdAppFlags |= SOC_PPD_FRWRD_MACT_L3_LEARN_IPV4_UC;
    }
    if (bcmAppFlags & BCM_SWITCH_CONTROL_L3_LEARN_IPV4_MC) {
        ppdAppFlags |= SOC_PPD_FRWRD_MACT_L3_LEARN_IPV4_MC;
    }
    if (bcmAppFlags & BCM_SWITCH_CONTROL_L3_LEARN_IPV6_UC) {
        ppdAppFlags |= SOC_PPD_FRWRD_MACT_L3_LEARN_IPV6_UC;
    }
    if (bcmAppFlags & BCM_SWITCH_CONTROL_L3_LEARN_IPV6_MC) {
        ppdAppFlags |= SOC_PPD_FRWRD_MACT_L3_LEARN_IPV6_MC;
    }
    if (bcmAppFlags & BCM_SWITCH_CONTROL_L3_LEARN_MPLS) {
        ppdAppFlags |= SOC_PPD_FRWRD_MACT_L3_LEARN_MPLS;
    }
...
_bcm_dpp_switch_control_routed_learning_get(int unit, int *bcmAppFlags)
{
    ...
    if (ppdAppFlags & SOC_PPD_FRWRD_MACT_L3_LEARN_IPV4_UC) {
        bcmAppTypes |= BCM_SWITCH_CONTROL_L3_LEARN_IPV4_UC;
    }
    if (ppdAppFlags & SOC_PPD_FRWRD_MACT_L3_LEARN_IPV4_MC) {
        bcmAppTypes |= BCM_SWITCH_CONTROL_L3_LEARN_IPV4_MC;
    }
    if (ppdAppFlags & SOC_PPD_FRWRD_MACT_L3_LEARN_IPV6_UC) {
        bcmAppTypes |= BCM_SWITCH_CONTROL_L3_LEARN_IPV6_UC;
    }
    if (ppdAppFlags & SOC_PPD_FRWRD_MACT_L3_LEARN_IPV6_MC) {
        bcmAppTypes |= BCM_SWITCH_CONTROL_L3_LEARN_IPV6_MC;
    }
    if (ppdAppFlags & SOC_PPD_FRWRD_MACT_L3_LEARN_MPLS) {
...
#if defined(BCM_ARAD_SUPPORT) && defined(BCM_RCPU_SUPPORT)
...
#endif /* #if defined(BCM_ARAD_SUPPORT) && defined(BCM_RCPU_SUPPORT) */
                            ...
                            int *arg)
{
    ...
    SOC_PPD_MTR_GROUP_INFO          mtr_group_info;
    SOC_PPD_EG_ENCAP_PWE_GLBL_INFO  pwe_encap_glbl;
    SOC_PPD_EG_ENCAP_GLBL_INFO      eg_encap_glbl;
    SOC_PPD_FRWRD_MACT_AGING_EVENTS_HANDLE aging_info;
    SOC_PPD_FRWRD_IPV4_GLBL_INFO    route_glbl_info;
    uint32  tmp32 = 0;
    ...
    case bcmSwitchStgInvalidToCpu:
        rv = _dpp_trap_code_switch_control_get(unit,
                                               SOC_PPD_TRAP_CODE_STP_STATE_BLOCK, DPP_SWITCH_TRAP_TO_CPU, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        rv = _dpp_trap_code_switch_control_get(unit,
                                               SOC_PPD_TRAP_CODE_STP_STATE_LEARN, DPP_SWITCH_TRAP_TO_CPU, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchUnknownUcastToCpu:
        rv = _dpp_trap_code_switch_control_get(unit,
                                               SOC_PPD_TRAP_CODE_DA_NOT_FOUND_0, DPP_SWITCH_TRAP_TO_CPU, &tmp1);
        rv = _dpp_trap_code_switch_control_get(unit, 
                                            SOC_PPD_TRAP_CODE_DA_NOT_FOUND_1, DPP_SWITCH_TRAP_TO_CPU, &tmp2);
        rv = _dpp_trap_code_switch_control_get(unit, 
                                            SOC_PPD_TRAP_CODE_DA_NOT_FOUND_2, DPP_SWITCH_TRAP_TO_CPU, &tmp3);
        rv = _dpp_trap_code_switch_control_get(unit, 
                                            SOC_PPD_TRAP_CODE_DA_NOT_FOUND_3, DPP_SWITCH_TRAP_TO_CPU, &tmp4);
    ...
    case bcmSwitchNonIpL3ErrToCpu:
        rv = _dpp_trap_code_switch_control_get(unit, 
                                            SOC_PPD_TRAP_CODE_MY_MAC_AND_UNKNOWN_L3, DPP_SWITCH_TRAP_TO_CPU, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchSourceRouteToCpu:
        rv = _dpp_trap_code_switch_control_get(unit,
                                               SOC_PPD_TRAP_CODE_IPV6_MULTICAST_SOURCE, DPP_SWITCH_TRAP_TO_CPU, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
...
/************************************************************************/
    case bcmSwitchIgmpPktDrop:
        rv = _dpp_trap_code_switch_control_get(unit,SOC_PPD_TRAP_CODE_IGMP_MEMBERSHIP_QUERY, DPP_SWITCH_DROP,arg);
        BCMDNX_IF_ERR_EXIT(rv);
        if (*arg==FALSE)
        {
        ...
        rv = _dpp_trap_code_switch_control_get(unit,SOC_PPD_TRAP_CODE_IGMP_REPORT_LEAVE_MSG, DPP_SWITCH_DROP,arg);
        BCMDNX_IF_ERR_EXIT(rv);
        if (*arg==FALSE)
        {
          /* if one of the options isnt enabled return */
        ...
        rv = _dpp_trap_code_switch_control_get(unit,SOC_PPD_TRAP_CODE_IGMP_UNDEFINED, DPP_SWITCH_DROP,arg);
        BCMDNX_IF_ERR_EXIT(rv);
        if (*arg==FALSE)
        {
          /* if one of the options isnt enabled return */
    ...
    case bcmSwitchIgmpPktToCpu:
        rv = _dpp_trap_code_switch_control_get(unit,SOC_PPD_TRAP_CODE_IGMP_MEMBERSHIP_QUERY, DPP_SWITCH_SNOOP_TO_CPU,arg);
        BCMDNX_IF_ERR_EXIT(rv);
        if (*arg==FALSE)
        {
          /* if one of the options isnt enabled return */
        ...
        rv = _dpp_trap_code_switch_control_get(unit,SOC_PPD_TRAP_CODE_IGMP_REPORT_LEAVE_MSG, DPP_SWITCH_SNOOP_TO_CPU,arg);
        BCMDNX_IF_ERR_EXIT(rv);
        if (*arg==FALSE)
        {
          /* if one of the options isnt enabled return */
        ...
        rv = _dpp_trap_code_switch_control_get(unit,SOC_PPD_TRAP_CODE_IGMP_UNDEFINED, DPP_SWITCH_SNOOP_TO_CPU,arg);
        BCMDNX_IF_ERR_EXIT(rv);
        if (*arg==FALSE)
        {
          /* if one of the options isnt enabled return */
    ...
    case bcmSwitchIgmpQueryToCpu:
        rv = _dpp_trap_code_switch_control_get(unit, 
                                               SOC_PPD_TRAP_CODE_IGMP_MEMBERSHIP_QUERY, DPP_SWITCH_SNOOP_TO_CPU, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchIgmpQueryDrop:
        rv = _dpp_trap_code_switch_control_get(unit,
                                               SOC_PPD_TRAP_CODE_IGMP_MEMBERSHIP_QUERY, DPP_SWITCH_DROP, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchIgmpQueryFlood:
        rv = _dpp_trap_code_switch_control_get(unit,
                                               SOC_PPD_TRAP_CODE_IGMP_MEMBERSHIP_QUERY, DPP_SWITCH_FLOOD, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;

    case bcmSwitchIgmpReportLeaveToCpu:
        rv = _dpp_trap_code_switch_control_get(unit,
                                               SOC_PPD_TRAP_CODE_IGMP_REPORT_LEAVE_MSG, DPP_SWITCH_SNOOP_TO_CPU, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchIgmpReportLeaveDrop:
        rv = _dpp_trap_code_switch_control_get(unit,
                                               SOC_PPD_TRAP_CODE_IGMP_REPORT_LEAVE_MSG, DPP_SWITCH_DROP, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchIgmpReportLeaveFlood:
        rv = _dpp_trap_code_switch_control_get(unit,
                                               SOC_PPD_TRAP_CODE_IGMP_REPORT_LEAVE_MSG, DPP_SWITCH_FLOOD, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchIgmpUnknownToCpu:
        rv = _dpp_trap_code_switch_control_get(unit,
                                               SOC_PPD_TRAP_CODE_IGMP_UNDEFINED, DPP_SWITCH_SNOOP_TO_CPU, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchIgmpUnknownDrop:
        rv = _dpp_trap_code_switch_control_get(unit,
                                               SOC_PPD_TRAP_CODE_IGMP_UNDEFINED, DPP_SWITCH_DROP, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchIgmpUnknownFlood:
        rv = _dpp_trap_code_switch_control_get(unit,
                                               SOC_PPD_TRAP_CODE_IGMP_UNDEFINED, DPP_SWITCH_FLOOD, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchCpuProtoIgmpPriority:
        rv = _dpp_trap_code_switch_control_priority_get(unit, 
                                            SOC_PPD_TRAP_CODE_IGMP_MEMBERSHIP_QUERY, &tmp1);
        rv = _dpp_trap_code_switch_control_priority_get(unit, 
                                            SOC_PPD_TRAP_CODE_IGMP_REPORT_LEAVE_MSG, &tmp2);
        rv = _dpp_trap_code_switch_control_priority_get(unit, 
                                            SOC_PPD_TRAP_CODE_IGMP_UNDEFINED, &tmp3);
...
/************************************************************************/
    case bcmSwitchMldPktToCpu:
        rv = _dpp_trap_code_switch_control_get(unit,SOC_PPD_TRAP_CODE_ICMPV6_MLD_REPORT_DONE_MSG, DPP_SWITCH_SNOOP_TO_CPU,arg);
        BCMDNX_IF_ERR_EXIT(rv);
        if (*arg==FALSE)
        {
          /* if one of the options isnt enabled return */
        ...
        rv = _dpp_trap_code_switch_control_get(unit,SOC_PPD_TRAP_CODE_ICMPV6_MLD_MC_LISTENER_QUERY, DPP_SWITCH_SNOOP_TO_CPU,arg);
        BCMDNX_IF_ERR_EXIT(rv);
        if (*arg==FALSE)
        {
          /* if one of the options isnt enabled return */
    ...
    case bcmSwitchMldPktDrop:
        rv = _dpp_trap_code_switch_control_get(unit,SOC_PPD_TRAP_CODE_ICMPV6_MLD_REPORT_DONE_MSG, DPP_SWITCH_DROP,arg);
        BCMDNX_IF_ERR_EXIT(rv);
        if (*arg==FALSE)
        {
          /* if one of the options isnt enabled return */
        ...
        rv = _dpp_trap_code_switch_control_get(unit,SOC_PPD_TRAP_CODE_ICMPV6_MLD_MC_LISTENER_QUERY, DPP_SWITCH_DROP,arg);
        BCMDNX_IF_ERR_EXIT(rv);
        if (*arg==FALSE)
        {
          /* if one of the options isnt enabled return */
    ...
    case bcmSwitchMldReportDoneToCpu  :
        rv = _dpp_trap_code_switch_control_get(unit, 
                                               SOC_PPD_TRAP_CODE_ICMPV6_MLD_REPORT_DONE_MSG, DPP_SWITCH_SNOOP_TO_CPU, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchMldReportDoneDrop   :
        rv = _dpp_trap_code_switch_control_get(unit,
                                               SOC_PPD_TRAP_CODE_ICMPV6_MLD_REPORT_DONE_MSG, DPP_SWITCH_DROP, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchMldReportDoneFlood  :
        rv = _dpp_trap_code_switch_control_get(unit,
                                               SOC_PPD_TRAP_CODE_ICMPV6_MLD_REPORT_DONE_MSG, DPP_SWITCH_FLOOD, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchMldQueryToCpu       :
        rv = _dpp_trap_code_switch_control_get(unit,
                                               SOC_PPD_TRAP_CODE_ICMPV6_MLD_MC_LISTENER_QUERY, DPP_SWITCH_SNOOP_TO_CPU, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchMldQueryDrop        :
        rv = _dpp_trap_code_switch_control_get(unit,
                                               SOC_PPD_TRAP_CODE_ICMPV6_MLD_MC_LISTENER_QUERY, DPP_SWITCH_DROP, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchMldQueryFlood       :
        rv = _dpp_trap_code_switch_control_get(unit,
                                               SOC_PPD_TRAP_CODE_ICMPV6_MLD_MC_LISTENER_QUERY, DPP_SWITCH_FLOOD, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
/************************************************************************/
/*                              arp                                     */
/************************************************************************/
    case bcmSwitchArpRequestMyStationIPToCPU:
        rv = _dpp_trap_code_switch_control_get(unit,
                                               SOC_PPD_TRAP_CODE_MY_ARP, DPP_SWITCH_SNOOP_TO_CPU, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchArpRequestMyStationIPDrop:
        rv = _dpp_trap_code_switch_control_get(unit,
                                               SOC_PPD_TRAP_CODE_MY_ARP, DPP_SWITCH_DROP, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchArpRequestMyStationIPFwd:
        rv = _dpp_trap_code_switch_control_get(unit,
                                               SOC_PPD_TRAP_CODE_MY_ARP, DPP_SWITCH_FWD, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchArpReplyToCpu:
        rv = _dpp_trap_code_switch_control_get(unit,
                                               SOC_PPD_TRAP_CODE_MY_MAC_AND_ARP, DPP_SWITCH_SNOOP_TO_CPU, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchArpReplyDrop:
        rv = _dpp_trap_code_switch_control_get(unit,
                                               SOC_PPD_TRAP_CODE_MY_MAC_AND_ARP, DPP_SWITCH_DROP, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchArpRequestToCpu:
        rv = _dpp_trap_code_switch_control_get(unit,
                                               SOC_PPD_TRAP_CODE_ARP, DPP_SWITCH_SNOOP_TO_CPU, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchArpRequestDrop:
        rv = _dpp_trap_code_switch_control_get(unit,
                                               SOC_PPD_TRAP_CODE_ARP, DPP_SWITCH_DROP, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
...
/************************************************************************/
    ...
    case bcmSwitchV6L3ErrToCpu:
        rv = _dpp_trap_code_switch_control_get(unit,
                                               SOC_PPD_TRAP_CODE_IPV6_VERSION_ERROR, DPP_SWITCH_TRAP_TO_CPU, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        if (*arg == FALSE) {
            break;
        }
        ...
        rv = _dpp_trap_code_switch_control_get(unit,
                                               SOC_PPD_TRAP_CODE_IPV6_NEXT_HEADER_NULL, DPP_SWITCH_TRAP_TO_CPU, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        if (*arg == FALSE) {
            break;
        }
        rv = _dpp_trap_code_switch_control_get(unit,
                                               SOC_PPD_TRAP_CODE_IPV6_UNSPECIFIED_SOURCE, DPP_SWITCH_TRAP_TO_CPU, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        if (*arg == FALSE) {
            break;
        }
        rv = _dpp_trap_code_switch_control_get(unit,
                                               SOC_PPD_TRAP_CODE_IPV6_UNSPECIFIED_DESTINATION, DPP_SWITCH_TRAP_TO_CPU, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchL3HeaderErrToCpu:
        rv = _dpp_trap_code_switch_control_get(unit,
                                               SOC_PPD_TRAP_CODE_IPV4_CHECKSUM_ERROR, DPP_SWITCH_TRAP_TO_CPU, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        if (*arg == FALSE) {
            break;
        }
        rv = _dpp_trap_code_switch_control_get(unit,
                                               SOC_PPD_TRAP_CODE_IPV4_HEADER_LENGTH_ERROR, DPP_SWITCH_TRAP_TO_CPU, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        if (*arg == FALSE) {
            break;
        }
        rv = _dpp_trap_code_switch_control_get(unit,
                                               SOC_PPD_TRAP_CODE_IPV4_TOTAL_LENGTH_ERROR, DPP_SWITCH_TRAP_TO_CPU, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        if (*arg == FALSE) {
            break;
        }
        rv = _dpp_trap_code_switch_control_get(unit,
                                               SOC_PPD_TRAP_CODE_IPV4_SIP_EQUAL_DIP, DPP_SWITCH_TRAP_TO_CPU, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        if (*arg == FALSE) {
            break;
        }
        rv = _dpp_trap_code_switch_control_get(unit,
                                               SOC_PPD_TRAP_CODE_IPV4_DIP_ZERO, DPP_SWITCH_TRAP_TO_CPU, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        if (*arg == FALSE) {
            break;
        }
        rv = _dpp_trap_code_switch_control_get(unit,
                                               SOC_PPD_TRAP_CODE_IPV4_SIP_IS_MC, DPP_SWITCH_TRAP_TO_CPU, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        if (*arg == FALSE) {
            break;
        }
...
/************************************************************************/
    case bcmSwitchDhcpPktToCpu:
        rv = _dpp_trap_code_switch_control_get(unit,
                                               SOC_PPD_TRAP_CODE_DHCP_CLIENT, DPP_SWITCH_SNOOP_TO_CPU, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchDhcpPktDrop:
        rv = _dpp_trap_code_switch_control_get(unit,
                                               SOC_PPD_TRAP_CODE_DHCP_CLIENT, DPP_SWITCH_DROP, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchCpuProtoDhcpPriority:
        rv = _dpp_trap_code_switch_control_priority_get(unit,
                                                        SOC_PPD_TRAP_CODE_DHCP_CLIENT, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
/************************************************************************/
    ...
    case bcmSwitchCpuProtoIpOptionsPriority:
        rv = _dpp_trap_code_switch_control_priority_get(unit,
                                                        SOC_PPD_TRAP_CODE_IPV4_HAS_OPTIONS, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchV4L3ErrToCpu:
        rv = _dpp_trap_code_switch_control_get(unit,
                                               SOC_PPD_TRAP_CODE_IPV4_VERSION_ERROR, DPP_SWITCH_TRAP_TO_CPU, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        if (*arg == FALSE) {
            break;
        }
    ...
    case bcmSwitchCpuProtoExceptionsPriority:
        rv = _dpp_trap_code_switch_control_priority_get(unit, 
                                            SOC_PPD_TRAP_CODE_IPV4_TTL1, &tmp1);        
        rv = _dpp_trap_code_switch_control_priority_get(unit, 
                                            SOC_PPD_TRAP_CODE_IPV6_HOP_COUNT1, &tmp2);
        if (tmp1==tmp2)
        {
          *arg = tmp1;
        } 
    ...
    case bcmSwitchL3UcastTtl1ToCpu:
        rv = _dpp_trap_code_switch_control_get(unit, 
                                          SOC_PPD_TRAP_CODE_IPV4_TTL1, DPP_SWITCH_TRAP_TO_CPU, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        rv = _dpp_trap_code_switch_control_get(unit,
                                               SOC_PPD_TRAP_CODE_IPV6_HOP_COUNT1, DPP_SWITCH_TRAP_TO_CPU, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchL3UcTtlErrToCpu:
        rv = _dpp_trap_code_switch_control_get(unit,
                                               SOC_PPD_TRAP_CODE_IPV4_TTL0, DPP_SWITCH_TRAP_TO_CPU, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        rv = _dpp_trap_code_switch_control_get(unit,
                                               SOC_PPD_TRAP_CODE_IPV6_HOP_COUNT0, DPP_SWITCH_TRAP_TO_CPU, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;    
    case bcmSwitchCpuProtoArpPriority:
        rv = _dpp_trap_code_switch_control_priority_get(unit, 
                                            SOC_PPD_TRAP_CODE_MY_ARP, &tmp1);        
        rv = _dpp_trap_code_switch_control_priority_get(unit, 
                                            SOC_PPD_TRAP_CODE_ARP, &tmp2);
        if (tmp1==tmp2)
        {
          *arg = tmp1;
        } 
    ...
    case bcmSwitchUnknownVlanToCpu:
        rv = _dpp_eg_trap_code_switch_control_get(unit, 
                                        SOC_PPD_TRAP_EG_TYPE_VSI_MEMBERSHIP, DPP_SWITCH_TRAP_TO_CPU, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        rv = _dpp_trap_code_switch_control_get(unit,
                                               SOC_PPD_TRAP_CODE_PORT_NOT_VLAN_MEMBER, DPP_SWITCH_TRAP_TO_CPU, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchIcmpRedirectToCpu:
        rv = _dpp_trap_code_switch_control_get(unit,
                                               SOC_PPD_TRAP_CODE_ICMP_REDIRECT, DPP_SWITCH_TRAP_TO_CPU, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchCpuIcmpRedirectPrio:
        rv = _dpp_trap_code_switch_control_priority_get(unit,
                                                        SOC_PPD_TRAP_CODE_ICMP_REDIRECT, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchTunnelErrToCpu:
        rv = _dpp_trap_code_switch_control_get(unit,
                                               SOC_PPD_TRAP_CODE_PBP_TE_UNKNOWN_TUNNEL, DPP_SWITCH_TRAP_TO_CPU, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    ...
    case bcmSwitchMplsLabelMissToCpu:
        rv = _dpp_trap_code_switch_control_get(unit,
                                               SOC_PPD_TRAP_CODE_MPLS_UNKNOWN_LABEL, DPP_SWITCH_TRAP_TO_CPU, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchMplsTtlErrToCpu:
        rv = _dpp_trap_code_switch_control_get(unit,
                                               SOC_PPD_TRAP_CODE_MPLS_TTL0, DPP_SWITCH_TRAP_TO_CPU, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        rv = _dpp_trap_code_switch_control_get(unit,
                                               SOC_PPD_TRAP_CODE_MPLS_TTL1, DPP_SWITCH_TRAP_TO_CPU, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchL3SrcUrpfErrToCpu:
    case bcmSwitchL3UrpfFailToCpu:
        rv = _dpp_trap_code_switch_control_get(unit, 
                                        SOC_PPD_TRAP_CODE_UC_LOOSE_RPF_FAIL, DPP_SWITCH_TRAP_TO_CPU, &tmp1);
        rv = _dpp_trap_code_switch_control_get(unit, 
                                        SOC_PPD_TRAP_CODE_UC_STRICT_RPF_FAIL, DPP_SWITCH_TRAP_TO_CPU, &tmp2);
        if (tmp1 == tmp2)
        {
          *arg = tmp1;
        } 
    ...
    case bcmSwitchSharedVlanMismatchToCpu:
        rv = _dpp_eg_trap_code_switch_control_get(unit, 
                                        SOC_PPD_TRAP_EG_TYPE_PRIVATE_VLAN, DPP_SWITCH_TRAP_TO_CPU, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchL3MtuFailToCpu:
        rv = _dpp_eg_trap_code_switch_control_get(unit,
                                                  SOC_PPD_TRAP_EG_TYPE_MTU_VIOLATION, DPP_SWITCH_TRAP_TO_CPU, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    ...
    case bcmSwitchMplsShortPipe:
        if (SOC_IS_ARADPLUS(unit)) {
            SOC_PPD_FRWRD_ILM_GLBL_INFO frwrd_ilm_glbl_info;
            SOC_PPD_FRWRD_ILM_GLBL_INFO_clear(&frwrd_ilm_glbl_info);
    ...
    case bcmSwitchFieldCache:
        ...
        if (SOC_DPP_IS_ELK_ENABLE(unit)) {
            ...
            control_ndx.type = SOC_PPD_FP_CONTROL_TYPE_KBP_CACHE;
...
#ifdef BCM_WARM_BOOT_SUPPORT
        rv = SOC_DPP_WB_ENGINE_GET_VAR(unit,
...
#endif /* BCM_WARM_BOOT_SUPPORT */
        break;
    case bcmSwitchStableConsistent:
#ifdef BCM_WARM_BOOT_SUPPORT
        (*arg) = (SOC_DPP_STATE(unit)->switch_state->scache_consistent == 0) ? TRUE : FALSE;
        rv = BCM_E_NONE;
#endif /* BCM_WARM_BOOT_SUPPORT */
        break;
...
/************************************************************************/
    ...
    case bcmSwitchPolicerFairness:
        ...
        if (SOC_IS_ARAD(unit)) {
            ...
            soc_sand_rv = soc_ppd_mtr_meters_group_info_get(soc_sand_dev_id, 0, &mtr_group_info);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
...
/************************************************************************/
    ...
    case bcmSwitchCongestionCntagEthertype:
      ...
      /*
      case bcmSwitchCongestionCnmMode:
        if (SOC_IS_ARAD(unit))
        {
          rv = _bcm_dpp_switch_control_congestion_mode_get(unit, arg);
          BCMDNX_IF_ERR_EXIT(rv);
        }
        break;
        */
...
/************************************************************************/
    ...
    case bcmSwitchMeterAdjustInterframeGap:
        ...
        if (SOC_IS_ARADPLUS(soc_sand_dev_id)) {
            /* Since the flag is global it doesn't matter which meter group is used. */
            soc_sand_rv = soc_ppd_mtr_meters_group_info_get(soc_sand_dev_id, 0, &mtr_group_info);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
            
            *arg  = mtr_group_info.is_ipg_compensation_enabled;
            rv = BCM_E_NONE;
        } else {
            unsupported_switch = 1;
        }
    ...
    case bcmSwitchColorL3L2Marking:
        if (SOC_IS_ARADPLUS(soc_sand_dev_id)) {
             SOC_PPD_EG_QOS_GLOBAL_INFO info;
             int col_idx;
             int flag_num = 4;
    ...
    case bcmSwitchResilientHashAgeTimer:
        if (SOC_IS_ARADPLUS(unit)) {
            if (soc_property_get(unit, spn_RESILIENT_HASH_ENABLE, FALSE)) {
                ...
                soc_sand_rv = arad_pp_sw_db_slb_aging_time_get(soc_sand_dev_id, &(aging.age_time_in_seconds));
                BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        ...
        break;
    case bcmSwitchResilientHashConfig:
      if (SOC_IS_ARADPLUS(unit)) {
        if (soc_property_get(unit, spn_RESILIENT_HASH_ENABLE, FALSE)) {
          SOC_PPC_SLB_CONFIGURATION_ITEM_HASH_FUNCTION func;

          soc_sand_rv = soc_ppd_slb_get_global_cfg(soc_sand_dev_id, SOC_PPC_SLB_DOWNCAST(&func, SOC_PPC_SLB_CONFIGURATION_ITEM));
          BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
          
          rv = _bcm_dpp_trunk_ppd_hash_to_bcm_hash(unit, func.hash_function, arg);
          BCMDNX_IF_ERR_EXIT(rv);
    ...
    case bcmSwitchEcmpResilientHashOffset:
      if (SOC_IS_ARADPLUS(unit)) {
        if (soc_property_get(unit, spn_RESILIENT_HASH_ENABLE, FALSE)) {
          SOC_PPC_SLB_CONFIGURATION_ITEM_HASH_OFFSET hash_offset;
          ...
          case bcmSwitchTrunkResilientHashOffset:
          case bcmSwitchEcmpResilientHashOffset:
            SOC_PPC_SLB_CLEAR(&hash_offset, SOC_PPC_SLB_OBJECT_TYPE_CONFIGURATION_ITEM_HASH_OFFSET);
            cfg_item = (SOC_PPC_SLB_CONFIGURATION_ITEM *)&hash_offset;
            break;
          default:
            BCMDNX_VERIFY(FALSE);
            break;
                            ...
                            int arg)
{
    ...
    SOC_PPD_EG_ENCAP_GLBL_INFO  eg_encap_glbl;
    SOC_PPD_EG_ENCAP_PWE_GLBL_INFO  pwe_encap_glbl;
    SOC_PPD_FRWRD_MACT_AGING_ONE_PASS_INFO pass_info;
    SOC_PPD_FRWRD_MACT_AGING_EVENTS_HANDLE aging_info;
    SOC_PPD_FRWRD_IPV4_GLBL_INFO route_glbl_info;
    SOC_PPD_FRWRD_IPV4_MEM_DEFRAG_INFO defrag_info;
#ifdef BCM_88660
    SOC_PPD_FRWRD_ILM_GLBL_INFO frwrd_ilm_glbl_info;
#endif /* BCM_88660 */
#if defined(INCLUDE_KBP) && !defined(BCM_88030)
    ...
    case bcmSwitchControlAutoSync:
        ...
        if (BCM_SWITCH_STABLE_NONE != stable_select) {
            ...
            rv = SOC_DPP_WB_ENGINE_SET_VAR(unit,
                                  SOC_DPP_WB_ENGINE_VAR_SWITCH_WARMBOOT_AUTO_SYNC,
                                  &arg);
            /* mark scache as dirty so that sync will occur at the end of current api*/
    ...
    case bcmSwitchUnknownUcastToCpu:
        rv = _dpp_trap_code_switch_control_set(unit,
                                               SOC_PPD_TRAP_CODE_DA_NOT_FOUND_0, DPP_SWITCH_TRAP_TO_CPU, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        rv = _dpp_trap_code_switch_control_set(unit,
                                               SOC_PPD_TRAP_CODE_DA_NOT_FOUND_1, DPP_SWITCH_TRAP_TO_CPU, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        rv = _dpp_trap_code_switch_control_set(unit,
                                               SOC_PPD_TRAP_CODE_DA_NOT_FOUND_2, DPP_SWITCH_TRAP_TO_CPU, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        rv = _dpp_trap_code_switch_control_set(unit,
                                               SOC_PPD_TRAP_CODE_DA_NOT_FOUND_3, DPP_SWITCH_TRAP_TO_CPU, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchCpuProtoExceptionsPriority:
        rv = _dpp_trap_code_switch_control_priority_set(unit,
                                                        SOC_PPD_TRAP_CODE_IPV4_TTL1, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        rv = _dpp_trap_code_switch_control_priority_set(unit,
                                                        SOC_PPD_TRAP_CODE_IPV6_HOP_COUNT1, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchL3UcastTtl1ToCpu:
        rv = _dpp_trap_code_switch_control_set(unit,
    ...
    case bcmSwitchCpuProtoArpPriority:
        rv = _dpp_trap_code_switch_control_priority_set(unit,
                                                        SOC_PPD_TRAP_CODE_ARP, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        rv = _dpp_trap_code_switch_control_priority_set(unit,
                                                        SOC_PPD_TRAP_CODE_MY_ARP, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchCpuProtoIgmpPriority:
        rv = _dpp_trap_code_switch_control_priority_set(unit,
                                                        SOC_PPD_TRAP_CODE_IGMP_MEMBERSHIP_QUERY, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        rv = _dpp_trap_code_switch_control_priority_set(unit,
                                                        SOC_PPD_TRAP_CODE_IGMP_REPORT_LEAVE_MSG, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        rv = _dpp_trap_code_switch_control_priority_set(unit,
                                                        SOC_PPD_TRAP_CODE_IGMP_UNDEFINED, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchNonIpL3ErrToCpu:
        rv = _dpp_trap_code_switch_control_set(unit,
...
/************************************************************************/
    case bcmSwitchIgmpPktDrop:
        rv = _dpp_trap_code_switch_control_set(unit, SOC_PPD_TRAP_CODE_IGMP_MEMBERSHIP_QUERY, DPP_SWITCH_DROP, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        rv = _dpp_trap_code_switch_control_set(unit, SOC_PPD_TRAP_CODE_IGMP_REPORT_LEAVE_MSG, DPP_SWITCH_DROP, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        rv = _dpp_trap_code_switch_control_set(unit, SOC_PPD_TRAP_CODE_IGMP_UNDEFINED, DPP_SWITCH_DROP, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchIgmpPktToCpu:
        rv = _dpp_trap_code_switch_control_set(unit, SOC_PPD_TRAP_CODE_IGMP_MEMBERSHIP_QUERY, DPP_SWITCH_SNOOP_TO_CPU, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        rv = _dpp_trap_code_switch_control_set(unit, SOC_PPD_TRAP_CODE_IGMP_REPORT_LEAVE_MSG, DPP_SWITCH_SNOOP_TO_CPU, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        rv = _dpp_trap_code_switch_control_set(unit, SOC_PPD_TRAP_CODE_IGMP_UNDEFINED, DPP_SWITCH_SNOOP_TO_CPU, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchIgmpQueryToCpu:
        rv = _dpp_trap_code_switch_control_set(unit,
...
/************************************************************************/
    case bcmSwitchMldPktToCpu:
        rv = _dpp_trap_code_switch_control_set(unit, SOC_PPD_TRAP_CODE_ICMPV6_MLD_MC_LISTENER_QUERY, DPP_SWITCH_SNOOP_TO_CPU, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        rv = _dpp_trap_code_switch_control_set(unit, SOC_PPD_TRAP_CODE_ICMPV6_MLD_REPORT_DONE_MSG, DPP_SWITCH_SNOOP_TO_CPU, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchMldPktDrop:
        rv = _dpp_trap_code_switch_control_set(unit, SOC_PPD_TRAP_CODE_ICMPV6_MLD_MC_LISTENER_QUERY, DPP_SWITCH_DROP, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        rv = _dpp_trap_code_switch_control_set(unit, SOC_PPD_TRAP_CODE_ICMPV6_MLD_REPORT_DONE_MSG, DPP_SWITCH_DROP, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchMldQueryToCpu:
        rv = _dpp_trap_code_switch_control_set(unit,
...
/************************************************************************/
    ...
    case bcmSwitchArpRequestMyStationIPToCPU:
        rv = _dpp_trap_code_switch_control_set(unit,
                                               SOC_PPD_TRAP_CODE_MY_ARP, DPP_SWITCH_SNOOP_TO_CPU, 1);
        BCMDNX_IF_ERR_EXIT(rv);
    ...
    case bcmSwitchArpRequestMyStationIPDrop:
        rv = _dpp_trap_code_switch_control_set(unit,
                                               SOC_PPD_TRAP_CODE_MY_ARP, DPP_SWITCH_DROP, 1);
        BCMDNX_IF_ERR_EXIT(rv);
        rv = _dpp_switch_my_arp_set(unit, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
...
/************************************************************************/
    ...
    case bcmSwitchL3UrpfFailToCpu:
        rv = _dpp_trap_code_switch_control_set(unit,
                                               SOC_PPD_TRAP_CODE_UC_LOOSE_RPF_FAIL, DPP_SWITCH_TRAP_TO_CPU, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        rv = _dpp_trap_code_switch_control_set(unit,
                                               SOC_PPD_TRAP_CODE_UC_STRICT_RPF_FAIL, DPP_SWITCH_TRAP_TO_CPU, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchIp4Compress:
        SOC_PPD_FRWRD_IPV4_MEM_DEFRAG_INFO_clear(&defrag_info);
    ...
    case bcmSwitchL2AgeScan:
        SOC_PPD_FRWRD_MACT_AGING_ONE_PASS_INFO_clear(&pass_info);
    ...
    case bcmSwitchL3RouteCache:
        ...
        tmp = (arg == 0) ? SOC_PPD_FRWRD_IP_CACHE_MODE_NONE : SOC_PPC_FRWRD_IP_CACHE_MODE_IPV4_UC_LPM;
    ...
    case bcmSwitchFieldCommit:
        ...
        if (SOC_DPP_IS_ELK_ENABLE(unit)) {
            ...
            SOC_PPD_FP_CONTROL_INDEX_clear(&control_ndx);
            SOC_PPD_FP_CONTROL_INFO_clear(&control_info);
            control_ndx.type = SOC_PPD_FP_CONTROL_TYPE_KBP_COMMIT;
    ...
    case bcmSwitchSnapNonZeroOui:
        break;
    case bcmSwitchSynchronousPortClockSource:
        SOCDNX_IF_ERR_EXIT(soc_dpp_fabric_sync_e_enable_set(unit, 1, arg));
        ...
        break;
    case bcmSwitchSynchronousPortClockSourceBkup:
        SOCDNX_IF_ERR_EXIT(soc_dpp_fabric_sync_e_enable_set(unit, 0, arg));
        if (arg >= SOC_INFO(unit).fabric_logical_port_base) {
...
/************************************************************************/
    ...
    case  bcmSwitchPolicerFairness:
        ...
        if (SOC_IS_ARAD(unit)) {
            ...
            /* update both meters groups */
            SOC_PPD_MTR_GROUP_INFO_clear(&mtr_group_info);
            mtr_group_info.rand_mode_enable = arg;

            soc_sand_rv = soc_ppd_mtr_meters_group_info_set(soc_sand_dev_id, 0, &mtr_group_info);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);

            soc_sand_rv = soc_ppd_mtr_meters_group_info_set(soc_sand_dev_id, 1, &mtr_group_info);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        }
        break;
/************************************************************************/
...
/************************************************************************/
    ...
    case bcmSwitchCongestionCntagEthertype:
      ...
      case bcmSwitchCongestionCnmMode:
        if (SOC_IS_ARAD(unit))
        {
          rv = _bcm_dpp_switch_control_congestion_mode_set(unit, arg);
          BCMDNX_IF_ERR_EXIT(rv);
        }
        break;
        */
...
/************************************************************************/
    ...
    case bcmSwitchMeterAdjustInterframeGap:
        ...
        if (SOC_IS_ARADPLUS(soc_sand_dev_id)) {
            ...
            if (mtr_group_info.is_ipg_compensation_enabled != is_ipg_enabled) {

                /* And then set the info. */
                mtr_group_info.is_ipg_compensation_enabled = is_ipg_enabled;
                soc_sand_rv = soc_ppd_mtr_meters_group_info_set(soc_sand_dev_id, 0, &mtr_group_info);
                BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
            }
    ...
    case bcmSwitchResilientHashAgeTimer:
      if (SOC_IS_ARADPLUS(unit)) {
        if (soc_property_get(unit, spn_RESILIENT_HASH_ENABLE, FALSE)) {
          ...
          soc_sand_rv = arad_pp_sw_db_slb_aging_time_set(soc_sand_dev_id, arg);
          BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    ...
    case bcmSwitchResilientHashConfig:
      if (SOC_IS_ARADPLUS(unit)) {
        if (soc_property_get(unit, spn_RESILIENT_HASH_ENABLE, FALSE)) {
          ...
          BCMDNX_IF_ERR_EXIT(rv);
          rv = _bcm_dpp_switch_ecmp_hash_global_get(unit, bcmSwitchTrunkHashConfig, &ecmp_hash_index);
          BCMDNX_IF_ERR_EXIT(rv);

          BCMDNX_VERIFY(lag_hash_index != ecmp_hash_index);

          if (lag_hash_index == arg) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("The stateful load balacing hash index must be different from the LAG hash index.\n")));
          }
          if (ecmp_hash_index == arg) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("The stateful load balacing hash index must be different from the ECMP hash index.\n")));
          }
          ...
          /* Set the seed in HW. */
          {
            ...
            SOC_PPC_SLB_CLEAR(&func,SOC_PPC_SLB_OBJECT_TYPE_CONFIGURATION_ITEM_HASH_FUNCTION);

            func.hash_function = dpp_hash_idx;

            soc_sand_rv = soc_ppd_slb_set_global_cfg(soc_sand_dev_id, SOC_PPC_SLB_DOWNCAST(&func, SOC_PPC_SLB_CONFIGURATION_ITEM));
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
          }
        } else {
          BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("Stateful load balancing is disabled on this device.\n")));
                                 ...
                                 int *arg)
{
    ...
    case bcmSwitchIgmpQueryDrop:
    case bcmSwitchIgmpQueryFlood:
        rv = _dpp_trap_code_enable_switch_control_port_get(unit, SOC_PPD_TRAP_CODE_IGMP_MEMBERSHIP_QUERY, port, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchIcmpRedirectToCpu:
        rv = _bcm_dpp_gport_to_phy_port(unit, port, 0, &gport_info); 
        ...
        BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_i) {
            ...
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
            *arg = (llp_trap_port_info.trap_enable_mask & SOC_PPD_LLP_TRAP_PORT_ENABLE_ICMP_REDIRECT)?1:0;
            break;
        }
    ...
    case bcmSwitchIgmpReportLeaveDrop:
    case bcmSwitchIgmpReportLeaveFlood:
        rv = _dpp_trap_code_enable_switch_control_port_get(unit, SOC_PPD_TRAP_CODE_IGMP_REPORT_LEAVE_MSG, port, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchIgmpUnknownToCpu:
    case bcmSwitchIgmpUnknownDrop:
    case bcmSwitchIgmpUnknownFlood:
        rv = _dpp_trap_code_enable_switch_control_port_get(unit, SOC_PPD_TRAP_CODE_IGMP_UNDEFINED, port, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchMldPktToCpu:
    case bcmSwitchMldPktDrop:
    ...
    case bcmSwitchMldReportDoneDrop:
    case bcmSwitchMldReportDoneFlood:
        rv = _dpp_trap_code_enable_switch_control_port_get(unit, SOC_PPD_TRAP_CODE_ICMPV6_MLD_REPORT_DONE_MSG, port, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchMldQueryToCpu:
    case bcmSwitchMldQueryDrop:
    case bcmSwitchMldQueryFlood:
        rv = _dpp_trap_code_enable_switch_control_port_get(unit, SOC_PPD_TRAP_CODE_ETH_FL_ICMPV6_MLD_MC_LISTENER_QUERY, port, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchDhcpPktToCpu:
    case bcmSwitchDhcpPktDrop:
        rv = _dpp_trap_code_enable_switch_control_port_get(unit, SOC_PPD_TRAP_CODE_DHCP_CLIENT, port, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchArpReplyToCpu:
    case bcmSwitchArpReplyDrop:
        rv = _dpp_trap_code_enable_switch_control_port_get(unit, SOC_PPD_TRAP_CODE_MY_MAC_AND_ARP, port, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchArpRequestToCpu:
    case bcmSwitchArpRequestDrop:
        rv = _dpp_trap_code_enable_switch_control_port_get(unit, SOC_PPD_TRAP_CODE_ARP, port, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchECMPHashPktHeaderCount:
    ...
    case bcmSwitchPFCClass7Queue:
        ...
        {
           BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("Switch control not supported on this device")));
        }
        break;
/************************************************************************/
/*                                            Header                                                 */
/************************************************************************/
    case bcmSwitchPortHeaderType:
		...
		if (*arg == BCM_SWITCH_PORT_HEADER_TYPE_ETH) {
            soc_sand_rc = soc_ppd_port_info_get(unit, core, (SOC_PPD_PORT)pp_port, &port_info);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rc);
			if (port_info.tunnel_termination_profile == SOC_PPC_PORT_DEFINED_TT_PROFILE_FORCE_MY_MAC) {
    ...
    case bcmSwitchMcastTrunkHashMin:
        if (SOC_IS_ARAD(unit)) {
            soc_sand_rc = (MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_port_direct_lb_key_min_get,(unit, core, pp_port, (uint32 *)arg)));
            BCM_SAND_IF_ERR_EXIT(soc_sand_rc);
        } else {
           BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("Supported only in Arad")));
        }
    ...
    case bcmSwitchMcastTrunkHashMax:
        if (SOC_IS_ARAD(unit)) {
            soc_sand_rc = (MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_port_direct_lb_key_max_get,(unit, core, pp_port, (uint32 *)arg)));
            BCM_SAND_IF_ERR_EXIT(soc_sand_rc);
        } else {
           BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("Supported only in Arad")));
        }
    ...
    case bcmSwitchPrependTagEnable:
        if (SOC_IS_ARAD(unit)) {
           ...
           BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("Switch control bcmSwitchPrependTagEnable is supported only in Arad")));
        }
        break;
                                 ...
                                 int arg)
{
    ...
    case bcmSwitchIgmpQueryDrop:
    case bcmSwitchIgmpQueryFlood:
        rv = _dpp_trap_code_enable_switch_control_port_set(unit, SOC_PPD_TRAP_CODE_IGMP_MEMBERSHIP_QUERY, port, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchIcmpRedirectToCpu:
        rv = _bcm_dpp_gport_to_phy_port(unit, port, 0, &gport_info); 
        ...
        BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_i) {
            ...
            if (arg != 0) {
                /*enable filter*/
                llp_trap_port_info.trap_enable_mask |= SOC_PPD_LLP_TRAP_PORT_ENABLE_ICMP_REDIRECT;
            } else {
                /*disable filter */
                llp_trap_port_info.trap_enable_mask &= ~SOC_PPD_LLP_TRAP_PORT_ENABLE_ICMP_REDIRECT;
            }
    ...
    case bcmSwitchIgmpReportLeaveDrop:
    case bcmSwitchIgmpReportLeaveFlood:
        rv = _dpp_trap_code_enable_switch_control_port_set(unit, SOC_PPD_TRAP_CODE_IGMP_REPORT_LEAVE_MSG, port, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchIgmpUnknownToCpu:
    case bcmSwitchIgmpUnknownDrop:
    case bcmSwitchIgmpUnknownFlood:
        rv = _dpp_trap_code_enable_switch_control_port_set(unit, SOC_PPD_TRAP_CODE_IGMP_UNDEFINED, port, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchMldPktToCpu:
    case bcmSwitchMldPktDrop:
    ...
    case bcmSwitchMldReportDoneDrop:
    case bcmSwitchMldReportDoneFlood:
        rv = _dpp_trap_code_enable_switch_control_port_set(unit, SOC_PPD_TRAP_CODE_ICMPV6_MLD_REPORT_DONE_MSG, port, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchMldQueryToCpu:
    case bcmSwitchMldQueryDrop:
    case bcmSwitchMldQueryFlood:
        rv = _dpp_trap_code_enable_switch_control_port_set(unit, SOC_PPD_TRAP_CODE_ETH_FL_ICMPV6_MLD_MC_LISTENER_QUERY, port, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchDhcpPktToCpu:
    case bcmSwitchDhcpPktDrop:
        rv = _dpp_trap_code_enable_switch_control_port_set(unit, SOC_PPD_TRAP_CODE_DHCP_CLIENT, port, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchArpReplyToCpu:
    case bcmSwitchArpReplyDrop:
        rv = _dpp_trap_code_enable_switch_control_port_set(unit, SOC_PPD_TRAP_CODE_MY_MAC_AND_ARP, port, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchArpRequestToCpu:
    case bcmSwitchArpRequestDrop:
        rv = _dpp_trap_code_enable_switch_control_port_set(unit, SOC_PPD_TRAP_CODE_ARP, port, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchECMPHashPktHeaderCount:
        ...
        if ((bcm_type == bcmSwitchPortHeaderType) || (SOC_IS_ARAD_B0_AND_ABOVE(unit))) {
            if (bcm_type == bcmSwitchPortHeaderType) {
				if (arg == BCM_SWITCH_PORT_HEADER_TYPE_MPLS_RAW){
					...
					soc_sand_rc = soc_ppd_port_info_set(unit, core, (SOC_PPD_PORT)pp_port, &port_info);
					BCM_SAND_IF_ERR_EXIT(soc_sand_rc);
				}
                /* Convert bcm port header type to tmc port header type */
                ...
                if (arg == BCM_SWITCH_PORT_HEADER_TYPE_TDM_RAW || arg == BCM_SWITCH_PORT_HEADER_TYPE_TDM) {
    ...
    uint32 value)
{
            ...
            for (inter = 0; inter < nof_interrupts; inter++) {
                ...
                for (bi_index = 0;  SOC_BLOCK_INFO(unit, bi_index).type >= 0; bi_index++) {
                    ...
                    if (is_valid) {
                        ...
                        rc = soc_interrupt_force(unit, port, &(interrupts[inter]), value);
                        BCMDNX_IF_ERR_EXIT(rc);
                    }
                }
            }
        } else {
            rc = soc_interrupt_force(unit, type.index, &(interrupts[type.event_id]), value);
            BCMDNX_IF_ERR_EXIT(rc);
        }
    ...
    case bcmSwitchEventStormNominal:
        if (type.event_id == BCM_SWITCH_EVENT_CONTROL_ALL) {
            ...
            SOCDNX_IF_ERR_EXIT(SOC_DPP_WB_ENGINE_SET_VAR(unit, SOC_DPP_WB_ENGINE_VAR_DPP_INTERRUPTS_BCM_SWITCH_EVENT_STORM_NOMINAL, &value));
            for (inter = 0; inter < nof_interrupts; inter++) {
                for (bi_index = 0;  SOC_BLOCK_INFO(unit, bi_index).type >= 0; bi_index++) {
...
bcm_petra_switch_l3_protocol_group_set(int unit, uint32 group_members, bcm_l3_protocol_group_id_t group_id){
    ...
    for (i = DPP_SWITCH_L3_PROTOCOL_GROUP_LOWEST_VALID ; i < SOC_PPC_L3_NOF_PROTOCOL_GROUPS(unit) ; i++) {
        /* Read SW DB */
        rv = SOC_DPP_WB_ENGINE_GET_ARR(unit, SOC_DPP_WB_ENGINE_VAR_DPP_L3_VRRP_PROTOCOL_GROUP_MEMBERS, &curr_group_members, i);
        BCMDNX_IF_ERR_EXIT(rv);
        if (i == group_id || (internal_flags & curr_group_members)) {
            /* verify that group is not in use. */
            rv = SOC_DPP_WB_ENGINE_GET_ARR(unit, SOC_DPP_WB_ENGINE_VAR_DPP_L3_VRRP_PROTOCOL_GROUP_COUNT, &nof_members, group_id);
            BCMDNX_IF_ERR_EXIT(rv);
            if (nof_members) {
    ...
    rv = SOC_DPP_WB_ENGINE_GET_ARR(unit, SOC_DPP_WB_ENGINE_VAR_DPP_L3_VRRP_PROTOCOL_GROUP_MEMBERS, &old_group_members, group_id);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    for (i = 0 ; i < SOC_PPC_L3_NOF_PROTOCOL_GROUPS(unit) ; i++) {
        ...
        /* Write to SW DB. */
        rv = SOC_DPP_WB_ENGINE_SET_ARR(unit, SOC_DPP_WB_ENGINE_VAR_DPP_L3_VRRP_PROTOCOL_GROUP_MEMBERS, &curr_group_members, i);
        BCMDNX_IF_ERR_EXIT(rv);
    }
exit:
    BCMDNX_FUNC_RETURN;
    ...
    *group_members = 0;
    ...
    for (i = 0; i < SOC_PPC_L3_NOF_PROTOCOL_GROUPS(unit) ; i++) {
        if ((i & group_mask) == (group_id & group_mask)) {
            rv = SOC_DPP_WB_ENGINE_GET_ARR(unit, SOC_DPP_WB_ENGINE_VAR_DPP_L3_VRRP_PROTOCOL_GROUP_MEMBERS, &internal_flags, i);
            BCMDNX_IF_ERR_EXIT(rv);
...
bcm_dpp_switch_l3_protocol_group_get_group_by_members(int unit, uint32 protocol_flags, uint32 *group_id, uint32 *group_mask, uint8 exact_match){
    ...
    *group_mask = DPP_SWITCH_L3_PROTOCOL_GROUP_MASK_EXACT;
    ...
    for (i = 0; i < SOC_PPC_L3_NOF_PROTOCOL_GROUPS(unit) ; i++) {
        ...
        if (DPP_BMP_IS_SUBSET(internal_flags, protocol_flags)) {
    ...
    if (exact_match) {
        if (existing_members != protocol_flags){
...
bcm_petra_switch_l3_protocol_group_get(int unit, uint32 *group_members, bcm_l3_protocol_group_id_t *group_id){
    ...
    if (*group_id == SOC_PPC_L3_VRRP_PROTOCOL_GROUP_INVALID) {
        ...
        if (group_mask != DPP_SWITCH_L3_PROTOCOL_GROUP_MASK_EXACT) {
            *group_id = SOC_PPC_L3_VRRP_PROTOCOL_GROUP_INVALID;
...
bcm_dpp_switch_l3_protocol_group_change_count(int unit, uint32 group_id, uint32 group_mask, int diff){
    ...
    for (i = 0; i < SOC_PPC_L3_NOF_PROTOCOL_GROUPS(unit) ; i++) {
        if ((i & group_mask) == (group_id & group_mask)) {
            rv = SOC_DPP_WB_ENGINE_GET_ARR(unit, SOC_DPP_WB_ENGINE_VAR_DPP_L3_VRRP_PROTOCOL_GROUP_COUNT, &count, i);
            BCMDNX_IF_ERR_EXIT(rv);
            ...
            rv = SOC_DPP_WB_ENGINE_SET_ARR(unit, SOC_DPP_WB_ENGINE_VAR_DPP_L3_VRRP_PROTOCOL_GROUP_COUNT, &count, i);
            BCMDNX_IF_ERR_EXIT(rv);
        }
    }
...
bcm_dpp_switch_l3_protocol_group_init(int unit){
    ...
    rv = SOC_DPP_WB_ENGINE_SET_ARR(unit, SOC_DPP_WB_ENGINE_VAR_DPP_L3_VRRP_PROTOCOL_GROUP_MEMBERS, &internal_flags, 0);
    BCMDNX_IF_ERR_EXIT(rv);
...
int bcm_petra_switch_tpid_add(
    ...
    int rv = BCM_E_NONE;
    uint32 addtional_tpid_enable;
    uint32 addtional_tpids[DPP_SWITCH_MAX_NUM_ADDITIONAL_TPIDS];
    ...
    if (SOC_IS_ARAD(unit)){
        ...
        sal_memset(addtional_tpids, 0 , sizeof(addtional_tpids));
        addtional_tpids[0] = tpid_info->tpid_value;
        rv = (MBCM_PP_DRIVER_CALL(unit,mbcm_pp_port_additional_tpids_set,(unit, addtional_tpids)));
        BCMDNX_IF_ERR_EXIT(rv);
    } else {
    ...
    bcm_switch_tpid_info_t *tpid_info)
{
    ...
    if (SOC_IS_ARAD(unit)){
        ...
        sal_memset(addtional_tpids, 0 , sizeof(addtional_tpids));
        rv = (MBCM_PP_DRIVER_CALL(unit,mbcm_pp_port_additional_tpids_get,(unit, addtional_tpids)));
        BCMDNX_IF_ERR_EXIT(rv);

        if (tpid_info->tpid_value != addtional_tpids[0]) {
    ...
    bcm_switch_tpid_info_t *tpid_info)
{
    ...
    if (SOC_IS_ARAD(unit)){
        ...
        if (addtional_tpids[0] != 0) {
            tpid_info->tpid_type = bcmTpidTypeInner;
            tpid_info->tpid_value= (uint16)(addtional_tpids[0]);
        }
    } else {
        BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("Addtional TPID not supported")));
    }
        ...
        if (size == 0) {
            *count = 1;
            ...
            if (addtional_tpids[0] != 0) {
                ...
                tpid_info_array[0].tpid_value= (uint16)(addtional_tpids[0]);
                *count = 1;
            }
        }
    } else {
    ...
    bcm_switch_network_group_config_t *config)
{
    ...
    if (!soc_property_get(unit, spn_SPLIT_HORIZON_FORWARDING_GROUPS_MODE, 0)
        && (source_network_group_id > 1 || config->dest_network_group_id > 1)) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG(
           "source_network_group_id and config->dest_network_group_id must be 0 or 1 when soc property split_horizon_forwarding_groups_mode is unset")));
    }
