...
_bcm_tr3_l2gre_egr_xlate_entry_get( int unit, int vfi, egr_vlan_xlate_entry_t *vxlate_entry)
{
    ...
    if (SOC_IS_TRIUMPH3(unit)) {
        soc_EGR_VLAN_XLATEm_field32_set(
           unit, &key_vxlate, KEY_TYPEf, _BCM_TR3_L2GRE_KEY_TYPE_LOOKUP_VFI);
    } else if (SOC_IS_TRIDENT2X(unit) || SOC_IS_TOMAHAWK(unit)) {
        soc_EGR_VLAN_XLATEm_field32_set(
           unit, &key_vxlate, ENTRY_TYPEf, _BCM_TD2_L2GRE_KEY_TYPE_LOOKUP_VFI);
    }
    soc_EGR_VLAN_XLATEm_field32_set(unit, &key_vxlate, VALIDf, 1);
    soc_EGR_VLAN_XLATEm_field32_set(unit, &key_vxlate, L2GRE_VFI__VFIf, vfi);
...
bcm_tr3_l2gre_vpn_destroy(int unit, bcm_vpn_t l2vpn)
{
    ...
    BCM_IF_ERROR_RETURN(_bcm_tr3_l2gre_egr_xlate_entry_get(unit, vfi, &vxlate_entry));
    vpnid = soc_EGR_VLAN_XLATEm_field32_get(unit, &vxlate_entry, L2GRE_VFI__VPNIDf);
    BCM_IF_ERROR_RETURN(bcm_tr3_l2gre_match_vpnid_entry_reset(unit, vpnid));
...
bcm_tr3_l2gre_vpn_get( int unit, bcm_vpn_t l2vpn, bcm_l2gre_vpn_config_t *info)
{
    ...
    /* Get VPNID */
    BCM_IF_ERROR_RETURN(_bcm_tr3_l2gre_egr_xlate_entry_get(unit, vfi_index, &vxlate_entry));
    info->vpnid = soc_EGR_VLAN_XLATEm_field32_get(unit, &vxlate_entry, L2GRE_VFI__VPNIDf);
    info->flags |= BCM_L2GRE_VPN_WITH_VPNID;

    /* Get SDTAG settings */
    (void)_bcm_tr3_l2gre_sd_tag_get( unit, info, NULL, NULL, &vxlate_entry, 1);
...
int
bcm_tr3_l2gre_match_vpnid_entry_reset(int unit, uint32 vpnid)
{
    mpls_entry_entry_t ment;
