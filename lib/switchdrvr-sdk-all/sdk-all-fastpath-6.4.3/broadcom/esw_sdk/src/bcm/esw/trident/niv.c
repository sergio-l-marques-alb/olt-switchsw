    ...
    for (idx = 0; idx < num_local_ports; idx++) {
        ...
        if (BCM_VLAN_VALID(match_vlan)) {
            ...
            if (soc_feature(unit, soc_feature_egr_vlan_control_is_memory)) {
                ...
                SOC_IF_ERROR_RETURN(READ_EGR_VLAN_CONTROL_1m(unit,
                                    MEM_BLOCK_ANY, local_ports[idx], &entry));
                soc_EGR_VLAN_CONTROL_1m_field32_set(unit, &entry,
                                                    VT_ENABLEf, 1);
                SOC_IF_ERROR_RETURN(WRITE_EGR_VLAN_CONTROL_1m(unit,
                                    MEM_BLOCK_ANY, local_ports[idx], &entry));
            } else
#endif
            {
...
#ifdef BCM_TOMAHAWK_SUPPORT
bcm_error_t
_bcm_niv_gport_get(int unit, bcm_gport_t glp,
                   int dst_vif, bcm_gport_t *niv_gport)
{
    ...
    for (vp = soc_mem_index_min(unit, SOURCE_VPm);
        ...
        if (_bcm_vp_used_get(unit, vp, _bcmVpTypeNiv)) {
            ...
            if (!(NIV_PORT_INFO(unit, vp)->flags & BCM_NIV_PORT_MULTICAST)) {
                if (NULL != curr_ptr) {
                    if (curr_ptr->port == glp &&
                        curr_ptr->virtual_interface_id == dst_vif) {
