 ...
 */
int
ext_lpm_vrf_get(int unit, int vrf_id, int vrf_mask, int global)
{
    int vrf;
...
_bcm_tr3_ext_lpm_state_recover(int unit, int v6)
{
    ...
    for (idx = 0; idx < soc_mem_index_count(unit, mem); idx++) {
        ...
        if (v6) { 
        _TR3_LPM_FIB6(unit)[idx].vrf = ext_lpm_vrf_get(unit, vrf, 
                vrf_mask, soc_mem_field32_get(unit, mem, buf, GLOBAL_ROUTEf));
        } else {
        _TR3_LPM_FIB4(unit)[idx].vrf = ext_lpm_vrf_get(unit, vrf, 
                vrf_mask, soc_mem_field32_get(unit, mem, buf, GLOBAL_ROUTEf));
        }
