...
_bcm_port_control_pcs_set(int unit, bcm_port_t port, int value)
{
    ...
    rc = MBCM_DFE_DRIVER_CALL(unit,mbcm_dfe_port_control_pcs_set,(unit, port, (soc_dfe_port_pcs_t)value));
    ...
    int value)
{
    ...
    switch(type)
    {
        ...
        case bcmPortControlLinkDownPowerOn:
            rc = MBCM_DFE_DRIVER_CALL(unit,mbcm_dfe_port_control_power_set,(unit, port, 0, value ? soc_dfe_port_power_on :  soc_dfe_port_power_off));
            BCMDNX_IF_ERR_EXIT(rc);
            break;
    ...
    int *value)
{
    ...
    {
        case bcmPortControlPCS:
            rc = MBCM_DFE_DRIVER_CALL(unit,mbcm_dfe_port_control_pcs_get,(unit, port, (soc_dfe_port_pcs_t*)value));
            BCMDNX_IF_ERR_EXIT(rc);
            break;
          
        case bcmPortControlLinkDownPowerOn:
            rc = MBCM_DFE_DRIVER_CALL(unit,mbcm_dfe_port_control_power_get,(unit, port, (soc_dfe_port_power_t*)value));
            BCMDNX_IF_ERR_EXIT(rc);
            break;
...
exit:
    ...
    if (counter_paused)
    {
        soc_counter_start(unit, flags, usec, PBMP_SFI_ALL(unit));
    }
    BCMDNX_FUNC_RETURN; 
}
...
bcm_dfe_port_enable_set(int unit, bcm_port_t port, int enable)
{
    ...
    if (enable) {
        ...
        rc = bcm_dfe_port_control_set(unit,port,bcmPortControlLinkDownPowerOn, 1);
        BCMDNX_IF_ERR_EXIT(rc);
        ...
        rc = bcm_dfe_port_control_set(unit,port,bcmPortControlLinkDownPowerOn, 0);
        BCMDNX_IF_ERR_EXIT(rc);
    }
...
bcm_dfe_port_phy_control_set(int unit, bcm_port_t port, bcm_port_phy_control_t type, uint32 value)
{
    ...
    DFE_UNIT_LOCK_TAKE(unit);
    rc = MBCM_DFE_DRIVER_CALL(unit,mbcm_dfe_port_phy_control_set,(unit, port, (soc_phy_control_t) type, value));
    DFE_UNIT_LOCK_RELEASE(unit);
                             ...
                             bcm_port_phy_control_t type, uint32 *value)
{
    ...
    DFE_UNIT_LOCK_TAKE(unit);
    rc = MBCM_DFE_DRIVER_CALL(unit,mbcm_dfe_port_phy_control_get,(unit, port, (soc_phy_control_t) type, value));
    BCMDNX_IF_ERR_EXIT(rc);
