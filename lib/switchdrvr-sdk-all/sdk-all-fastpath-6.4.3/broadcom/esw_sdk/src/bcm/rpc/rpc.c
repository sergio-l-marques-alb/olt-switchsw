...
#define _RPC_SERVER_NAME_MAX 50
...
static char *rpc_server_name;             /* Hold the RPC Server Name */
#ifdef  BROADCOM_DEBUG
...
bcm_rpc_thread(void *cookie)
{
    ...
    for (;;) {
        ...
        /* Update the Perticular ThreadID is ready for use */
        /* Update the server DB , thread_id is already updated*/
        db.thread_id = id;
...
bcm_rpc_start(void)
{
    ...
    rpc_server_name = sal_alloc((_num_rpc_threads * _RPC_SERVER_NAME_MAX),
                                             "bcm_rpc_server_name_locks");
    if (rpc_server_name == NULL) {
        return BCM_E_MEMORY;
    }
    _rpc_server_locks =
    ...
    for ( thread_id = 0; thread_id < _num_rpc_threads; ++thread_id ) {
        rpc_server_name_ptr = rpc_server_name + (thread_id *
                                                   _RPC_SERVER_NAME_MAX);
        memset(rpc_server_name_ptr, 0, sizeof(_RPC_SERVER_NAME_MAX));
        sal_strncpy ( rpc_server_name_ptr, _RPC_SERVER_NAME, 
                                                   sizeof (_RPC_SERVER_NAME)); 
        _rpc_sreqs [ thread_id ] = _rpc_sreqs_tail [ thread_id ] = NULL;

        rpc_name = rpc_server_name_ptr + _RPC_SERVER_NAME_EXT_PTR;
        rpc_name [ 0 ] = '0' + (thread_id / 10); /* convert id to character */
        rpc_name [ 1 ] = '0' + (thread_id % 10); /* convert id to character */

        _rpc_server_locks [thread_id] = sal_mutex_create(rpc_server_name_ptr);
        _rpc_server_sems  [thread_id] = sal_sem_create(rpc_server_name_ptr, sal_sem_BINARY, 0);

        tid = sal_alloc(sizeof(*tid), rpc_server_name_ptr);
        if (tid) {
            *tid = thread_id;
            _rpc_server_thread = sal_thread_create(rpc_server_name_ptr,
                                                   RPC_THREAD_STACK,
                                                   RPC_THREAD_PRIO,
                                                   bcm_rpc_thread,
                                                   tid);
