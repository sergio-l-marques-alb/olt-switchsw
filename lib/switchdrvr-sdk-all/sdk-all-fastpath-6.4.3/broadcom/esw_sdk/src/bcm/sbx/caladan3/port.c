...
_bcm_caladan3_port_probe_port(int unit, bcm_port_t port, int *okay)
{
    ...
    *okay = FALSE;
    ...
    if (!SOC_PBMP_MEMBER(SOC_CONTROL(unit)->all_skip_pbm,port) && !SOC_WARM_BOOT(unit)) {
        /* Leave the mac in disabled state */
...
_bcm_caladan3_port_probe(int unit, pbmp_t pbmp, pbmp_t *okay_pbmp)
{
    ...
    if (SAL_BOOT_BCMSIM || SAL_BOOT_PLISIM) return BCM_E_NONE;
    ...
    PBMP_ITER(pbmp, port) {
        ...
        if (rv) {
            LOG_ERROR(BSL_LS_BCM_PORT,
                      (BSL_META_U(unit,
                                  "_bcm_caladan3_port_probe: channelized check failed unit=%d rv=%d(%s)\n"),
                       unit, rv, bcm_errmsg(rv)));
        }
        
        if ((INTERFACE_MAC(unit, port)) && (is_channelized == FALSE || requires_phy_setup == TRUE)) {
...
_bcm_caladan3_port_enable_set(int unit, bcm_port_t port, int enable)
{
    ...
    if (SOC_FAILURE(rv)) {
      LOG_WARN(BSL_LS_BCM_PORT,
               (BSL_META_U(unit,
                           "Unit %d Port %s: Enable of MAC failed %s\n"),
                unit, SOC_PORT_NAME(unit, port), soc_errmsg(rv)));
    }
...
_bcm_caladan3_port_interface_init(int unit, int enable)
{
    ...
    PBMP_ITER(okay_ports, port) {
        ...
        if (is_channelized == TRUE ) continue;
        ...
        if ((rv = _bcm_caladan3_port_mode_setup(unit, port, TRUE)) < 0) {
            LOG_WARN(BSL_LS_BCM_PORT,
                     (BSL_META_U(unit,
                                 "Warning: Failed to set initial mode \n"
        ...
        if (BCM_FAILURE(rv = _bcm_caladan3_port_settings_init(unit, port))) {
            LOG_WARN(BSL_LS_BCM_PORT,
                     (BSL_META_U(unit,
                                 "Warning: Failed to configure initial settings \n"
                                  "unit=%d port=%s rv=%d(%s)\n"),
                      unit, SOC_PORT_NAME(unit, port), rv, bcm_errmsg(rv)));
        }
        ...
        if (BCM_FAILURE(rv = _bcm_caladan3_port_enable_set(unit, port, enable))) {
            LOG_WARN(BSL_LS_BCM_PORT,
                     (BSL_META_U(unit,
                                 "Warning: Failed to %s unit=%d port=%s rv=%d(%s)\n"),
                                        ...
                                        _bcm_caladan3_nvid_action_t action) 
{
    ...
    if (SOC_IS_SBX_G3P1(unit)) {
        ...
        if (BCM_FAILURE(result)) {
            result = _bcm_caladan3_mpls_port_info_get(unit, port, vid, pv2e.vlan, pv2e.vpws,
                                                    &keepUntaggedFlags);
                                    ...
                                    bcm_vlan_t vid)
{
    ...
    if (BCM_GPORT_IS_VLAN_PORT(port)) {
        result =  _bcm_caladan3_port_lp_untagged_vlan_access(unit, port, &vid,
                                                        1 /* set */);
    }
