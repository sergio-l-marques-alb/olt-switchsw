/*
 * $Id: allocator.c,v 1.80 Broadcom SDK $
...
_sbx_gu2_hw_res_t    _g_mapped_hw_res[] = {
    ...
    SBX_GU2K_HW_RES_VPXS_PW_FO_FTE            /* FTE_VPLS_PW_FO */
};
...
_sbx_gu2_hw_res_attrs_t _g_sbx_gu2_res_attrs[SBX_GU2K_HW_RES_MAX] = {
   ...
   /* SBX_GU2K_HW_PROTECTION */
    ...
    /* SBX_GU2K_HW_RES_VPXS_PW_FO_FTE */
    {SBX_GU2K_ALLOC_STYLE_VERSATILE,
     SBX_GU2K_DYNAMIC_BASE_ADDR,
     SBX_GU2K_DYNAMIC_MAX_COUNT,
     1,
...
#define _SBX_ALLOCATOR_GET_RESV_ATTR_NODE(e, var) \
            (var) = DQ_ELEMENT(_sbx_gu2_resv_list_attrs_t*,\
                               (e), (var), listNode)
...
int
_sbx_gu2_vpxs_pw_fo_fte_res_attrs_set(int unit, 
                             _sbx_gu2_hw_res_attrs_t *fte_res_attrs)
{
    uint32 vpws_offset;
#ifdef BCM_FE2000_P3_SUPPORT
    if (SOC_IS_SBX_G2P3(unit)) {
...
_sbx_gu2_resource_to_str(_sbx_gu2_usr_res_types_t type)
{
    ...
    case SBX_GU2K_USR_RES_FTE_VPXS_PW_FO: return "SBX_GU2K_USR_RES_FTE_VPXS_PW_FO";
...
_sbx_gu2_resource_init(int unit)
{
    ...
    _g_sbx_gu2_res_attrs[SBX_GU2K_HW_RES_ETE].base = base;
    _g_sbx_gu2_res_attrs[SBX_GU2K_HW_RES_ETE].max_count = ete_area_size;
    ...
    res_attrs = &_g_sbx_gu2_res_attrs[SBX_GU2K_HW_RES_VPXS_PW_FO_FTE];
    status = _sbx_gu2_vpxs_pw_fo_fte_res_attrs_set(unit, res_attrs);
    if (BCM_FAILURE(status)) {
        return status;
    }
...
STATIC
int _sbx_gu2_resource_check_reserved_range(int unit,
                                           _sbx_gu2_hw_res_attrs_t *res_attrs,
                                           uint32                *elements,
                                           uint32 count)
{
    dq_p_t   resvNodeElem;
    _sbx_gu2_resv_list_attrs_t *resvNode=NULL;
    int index =0 ;
                        ...
                        uint32                     flags)
{
    ...
    if (flags & _SBX_GU2_RES_FLAGS_RESERVE) {
        int checkVal = _sbx_gu2_resource_check_reserved_range(unit, res_attrs, elements, count);
                       ...
                       uint32                     flags)
{
    ...
    if(BCM_E_EXISTS == _sbx_gu2_resource_check_reserved_range(unit, res_attrs, elements, count)) {
        _SBX_GU2_RESOURCE_UNLOCK(unit);
        return BCM_E_NONE;
    }
                           ...
                           int highOrLow, uint32 val)
{
    ...
    int rv = BCM_E_NONE;
    dq_p_t   resvNodeElem;
    _sbx_gu2_resv_list_attrs_t *resvNode=NULL, *allocNode=NULL;
    ...
    if (BCM_SUCCESS(rv) && val) {
        ...
        allocNode = (_sbx_gu2_resv_list_attrs_t*)(sal_alloc(
                                                    sizeof(_sbx_gu2_resv_list_attrs_t),
                                                    "resv list node"));     
        if (!allocNode) {
            rv = BCM_E_MEMORY;
            LOG_ERROR(BSL_LS_BCM_INIT,
            ...
            sal_memset(allocNode, 0, sizeof(_sbx_gu2_resv_list_attrs_t));
            if (highOrLow) {
                allocNode->high = val;
            } else {
                allocNode->low = val;
...
int soc_sbx_g2p3_allocator_reservation_shell_print(int unit)
{
    int j, k;
    int fw = 10;
    dq_p_t   resvNodeElem;
    ...
    for (j = 0; j < SBX_GU2K_TABLE_MAX; j++) {
        ...
        for (k = 0; k < SBX_GU2K_HW_RES_MAX; k++) {
            ...
            if (_g_sbx_gu2_res_attrs[k].tables & 
                ...
                DQ_TRAVERSE(&_g_sbx_gu2_res_attrs[k].resvList, resvNodeElem) {
                    _SBX_ALLOCATOR_GET_RESV_ATTR_NODE(resvNodeElem, resvNode);
                ...
                } DQ_TRAVERSE_END(&_g_sbx_gu2_res_attrs[k].resvList, resvNodeElem);
            }
        }
    }
    ...
    LOG_CLI((BSL_META_U(unit,
                        "OAM RESERVATION\n")));
    bcm_fe2000_oam_dump_reservation(unit);
