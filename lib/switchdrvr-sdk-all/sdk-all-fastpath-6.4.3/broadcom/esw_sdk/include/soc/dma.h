...
/* SOC DMA mode access */
/* Continuous mode uses reload descriptors to chain DVs CMICd only */
typedef enum dma_mode_e {
    SOC_DMA_MODE_DEFAULT = 0,
    SOC_DMA_MODE_CONTINUOUS
} dma_mode_t;
...
typedef struct sdc_s {
    ...
    dma_mode_t  sc_mode;                /* Continuous mode or not */
    int         sc_dma_started;         /* Continuous DMA chan started */
} sdc_t;

#define SOC_DMA_MODE(unit, chan)(SOC_CONTROL(unit)->soc_channels[chan].sc_mode)
...
extern int      soc_dma_start_reload(int unit, dma_chan_t, dv_t *);
