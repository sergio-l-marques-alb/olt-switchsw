...
/*
 * This file contains joint multicast implementation between mutiple dpp devices
 */
 ...
 *  0-25: Reserved for later usage.
 * 26-26: A bit used only by testing code.
 * 27-27: For Arad egress bitmap groups (0 till at most 8191) store if the group is open.
 * 28-28: For used entries: The msb bit of the previous entry of the group. The 16 lsb bits are in the prev_entries table.
...
#define DPP_MCDS_TEST_BIT_SHIFT 26 /* bit saying if the bitmap group (egress 0-8191) is open */
#define DPP_MCDS_BITMAP_OPEN_SHIFT 27 /* bit previously used for marking if the bitmap groups (egress 0-8191) were open */
#define DPP_MCDS_MSB_PREV_BIT_SHIFT 28 /* bit not fitting in prev_entries */
...
#define DPP_MCDS_MSB_PREV_BIT_MASK 0x10000 /* mask of the bit in the entry index */
...
#define DPP_MCDS_IS_BITMAP_OPEN(   mcds, mcid) DPP_MCDS_ENTRY_IS_BITMAP_OPEN(   (mcds)->mcdb + ((mcid)+ARAD_MULT_NOF_MULTICAST_GROUPS))
#define DPP_MCDS_SET_BITMAP_OPEN(  mcds, mcid) DPP_MCDS_ENTRY_SET_BITMAP_OPEN(  (mcds)->mcdb + ((mcid)+ARAD_MULT_NOF_MULTICAST_GROUPS))
#define DPP_MCDS_SET_BITMAP_CLOSED(mcds, mcid) DPP_MCDS_ENTRY_SET_BITMAP_CLOSED((mcds)->mcdb + ((mcid)+ARAD_MULT_NOF_MULTICAST_GROUPS))
...
#define DPP_MCDS_GET_EGRESS_FORMAT(mcds, index) (((mcds)->mcdb[index].word1 & 0x70000) >> 16)
...
#define DPP_MCDS_IS_EGRESS_FORMAT_CONSECUTIVE_NEXT(format) (((format) & 5) == 5)
...
#if 0
#define ARAD_MC_OUTLIF_UNOCCUPIED 0xffff        /* value of outlif field used in ARAD/JER_MC_UNOCCUPIED_ENTRY_LOW */
#define JER_MC_OUTLIF_UNOCCUPIED 0x3ffff        /* value of outlif field used in ARAD_MC_UNOCCUPIED_ENTRY_LOW, ARAD_MC_EGR_UNOCCUPIED_ENTRY_LOW */
#endif
...
typedef struct { /* free block lists of a region */
  ...
  uint32 range_start, range_end; /* range start and range end */
...
#define DPP_MCDS_REP_TYPE_EGR_PORT_CUD 1
#define DPP_MCDS_REP_TYPE_EGR_CUD 2
#define DPP_MCDS_REP_TYPE_EGR_BM_CUD 3
...
/* get the MCDB index of the start of the egress linked list of the give MCID and core */
...
#define DPP_CUD2CORE_INITIAL_BTYE_VALUE (DPP_CUD2CORE_UNDEF_VALUE | DPP_CUD2CORE_UNDEF_VALUE << DPP_CUD2CORE_BITS_PER_CUD | DPP_CUD2CORE_UNDEF_VALUE << DPP_CUD2CORE_BITS_PER_CUD*2 | DPP_CUD2CORE_UNDEF_VALUE << DPP_CUD2CORE_BITS_PER_CUD*3) /* 0xff */
#define DPP_CUD2CORE_GET_CORE(mcds, cud) (((mcds)->cud2core[(cud) / DPP_CUD2CORE_CUDS_PER_WORD] >> DPP_CUD2CORE_BITS_PER_CUD * ((cud) % DPP_CUD2CORE_CUDS_PER_WORD)) & DPP_CUD2CORE_CUD_MASK)
#define DPP_CUD2CORE_SET_CORE(mcds, cud, core) do { \
  uint32 *_cud2core_p = (mcds)->cud2core + (cud) / DPP_CUD2CORE_CUDS_PER_WORD; \
  unsigned _cud2core_shift = DPP_CUD2CORE_BITS_PER_CUD * ((cud) % DPP_CUD2CORE_CUDS_PER_WORD); \
  *_cud2core_p &= ~(DPP_CUD2CORE_CUD_MASK << _cud2core_shift); \
  *_cud2core_p |= (core) << _cud2core_shift; \
  } while (0)
...
typedef uint32 (*dpp_get_replications_from_entry_f)(
    ...
    SOC_SAND_INOUT uint16  *max_size,   /* the maximum number of replications to return from the group, decreased by the number of returned replicaitons */
...
typedef struct {
    ...
    uint32 *cud2core;           /* For Jericho dual core, store for each CUD to port mapping, what is the core of the port */
    ...
    dpp_free_entries_blocks_region_t ingress_alloc_free, egress_alloc_free; /* free lists of entries that are automatically allocated; ingress and egress separately */
    ...
    dpp_rep_data_t *egr_port_outlif_reps, *egr_outlif_reps, *egr_bitmap_reps;
    /* number of replications left of  to the start of or to the current location at the block of each replication type in sorted replications */
    ...
    uint16 nof_egr_bitmap_reps;      /* number of egress replications of type bitmap + outlif */
...
/*
 * This function the processes the replications of an egress group linked list.
...
uint32 dpp_mult_process_egress_replications(
    SOC_SAND_IN  int           unit,
    SOC_SAND_IN  uint32        core,                     /* linked list core to process */
