...
#define DPP_DEVICE_PETRA_VOQ_HD_SIZE(unit)                      ((SOC_DPP_CORE_MODE_IS_SYMMETRIC(unit))   ?  \
                                                                 (SOC_DPP_DEFS_GET(unit, max_queues) - 1) :  \
                                                                 ((SOC_DPP_DEFS_GET(unit, max_queues) * SOC_DPP_CONFIG(unit)->core_mode.nof_active_cores) - 1))

#define DPP_DEVICE_PETRA_VOQ_HD_CORE(unit, element_index)       (element_index / SOC_DPP_DEFS_GET(unit, max_queues))
#define DPP_DEVICE_PETRA_VOQ_HD_ID(unit, core, element_index)   ((core != BCM_CORE_ALL) ? (element_index - (core * SOC_DPP_DEFS_GET(unit, max_queues))) : element_index)
#define DPP_DEVICE_PETRA_VOQ_HD_ELEMENT(unit, core, voq_index)  ((core != BCM_CORE_ALL) ? (voq_index + (core * SOC_DPP_DEFS_GET(unit, max_queues))) : voq_index)
