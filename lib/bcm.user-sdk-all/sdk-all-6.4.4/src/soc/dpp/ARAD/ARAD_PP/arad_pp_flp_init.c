...
/* Generic in build n zeros, ones using 16 bits instruction */
...
#define ARAD_PP_FLP_16B_INST_P6_VSI_D_12_BITS                                                        (0xba18)
...
#define ARAD_PP_FLP_PROG_SEL_CAM_TM                        0
#define ARAD_PP_FLP_PROG_SEL_CAM_P2P_BASE                  1 /* 1 through 2 */
#define ARAD_PP_FLP_PROG_SEL_CAM_IPV4COMPMC_RPF            9
#define ARAD_PP_FLP_PROG_SEL_CAM_IPV4MC_RPF                10
#define ARAD_PP_FLP_PROG_SEL_CAM_COUPLING_LSR              12
#define ARAD_PP_FLP_PROG_SEL_CAM_LSR                       13
#define ARAD_PP_FLP_PROG_SEL_CAM_IPV6MC                    14
#define ARAD_PP_FLP_PROG_SEL_CAM_IPV6UC                    15
#define ARAD_PP_FLP_PROG_SEL_CAM_IPV4UC_L3VPN              16 /* IPV4UC_L3VPN has a higher priority than IPV4UC with a lower line id */
#define ARAD_PP_FLP_PROG_SEL_CAM_IPV4UC_RPF                17
#define ARAD_PP_FLP_PROG_SEL_CAM_IPV4UC                    18
#define ARAD_PP_FLP_PROG_SEL_CAM_ETH_ING_LRN               19
#define ARAD_PP_FLP_PROG_SEL_CAM_TRILL_UC                  20 /* Trill */
#define ARAD_PP_FLP_PROG_SEL_CAM_TRILL_MC_OVERLAY          21 /* Trill */
#define ARAD_PP_FLP_PROG_SEL_CAM_ETH_TK_EPON_UNI_V4_STATIC 21 /* Spoof static V4 */
#define ARAD_PP_FLP_PROG_SEL_CAM_TRILL_MC_ONE_TAG          22 /* Trill */
#define ARAD_PP_FLP_PROG_SEL_CAM_ETH_TK_EPON_UNI_V6_STATIC 22 /* Spoof static V6 */
#define ARAD_PP_FLP_PROG_SEL_CAM_ETH_MAC_IN_MAC            23
#define ARAD_PP_FLP_PROG_SEL_CAM_ETH_PBP                   24
#define ARAD_PP_FLP_PROG_SEL_CAM_ETH_TK_EPON_UNI_V4_DHCP   25 /* Spoof DHCP V4 */
#define ARAD_PP_FLP_PROG_SEL_CAM_TRILL_MC_TWO_TAGS         25 /* Trill */
#define ARAD_PP_FLP_PROG_SEL_CAM_ETH_TK_EPON_UNI_V6_DHCP   26 /* Spoof DHCP V6 */
#define ARAD_PP_FLP_PROG_SEL_CAM_ETH_PON_DEFAULT_UPSTREAM  27 /* PON default upstream  */
#define ARAD_PP_FLP_PROG_SEL_CAM_IPV6UC_2PASS_FWD          28 /* IPv6 UC with RPF 2PASS, prog for second pass (DIP), 
                                                               * must be higher than ARAD_PP_FLP_PROG_SEL_CAM_IPV6UC_2PASS 
                                                               */
#define ARAD_PP_FLP_PROG_SEL_CAM_IPV6UC_2PASS              29 /* IPv6 UC with RPF 2PASS, prog for First Pass (SIP), 
                                                               * must be lower than ARAD_PP_FLP_PROG_SEL_CAM_IPV6UC 
                                                               */
...
STATIC
    ARAD_FLP_CUSTOM_LPM_LOOKUP_INFO
...
#if 0
        {
          ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_MAC_IN_MAC,  
        ...
        {
          ARAD_PP_FLP_KEY_PROGRAM_MAC_IN_MAC_AFTER_TERMINATIOM,
          /* lookup_num, lookup_to_use, {, {}, ... } */
          2, 0,
              {
                  { 2, 0, 2, /*  key C, key OR val, number of instructions */
...
#endif
        {
          ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_ING_LEARN,
          /* lookup_num, {, {}, ... } */
          2, 0,
              {
        ...
        {
          ARAD_PP_FLP_KEY_PROGRAM_IPV4UC_WITH_RPF,
          /* lookup_num, {, {}, ... } */
          2, 0,
              {
                  { 2, 0, 2, /*  key index, key OR val, number of instructions */
        ...
        {
          ARAD_PP_FLP_KEY_PROGRAM_IPV4UC,
          /* lookup_num, {, {}, ... } */
          2, 0,
              {
                  { 2, 0, 2, /*  key index, key OR val, number of instructions */
        ...
        {
          ARAD_PP_FLP_KEY_PROGRAM_IPV6UC,
          /* lookup_num, {, {}, ... } */
          2, 0,
              {
         ...
         },
         {
...
#if 0
          {
            ARAD_PP_FLP_KEY_PROGRAM_COUPLING_LSR,
            /* lookup_num, lookup_to_use, {, {}, ... } */
            2, 0,
              {
                  { 1, 0, 2, /*  key B, key OR val, number of instructions */
...
#endif
          {
            ARAD_PP_FLP_KEY_PROGRAM_IPV4COMPMC_WITH_RPF,
            /* lookup_num, lookup_to_use, {, {}, ... } */
            2, 0,
              {
                  { 2, 0, 2, /*  key C, key OR val, number of instructions */
          ...
          {
            ARAD_PP_FLP_KEY_PROGRAM_IPV6MC,
            /* lookup_num, {, {}, ... } */
            2, 0,
              {
                  { 1, 0, 2, /*  key B, key OR val, number of instructions */
...
STATIC int arad_pp_get_first_zero_bit(int unit, uint8 is_prog_sel, int32* val)
{
    uint32 indx;
    uint32 val_lcl[1];
    *val_lcl = *val;
    for (indx=0; indx < (is_prog_sel? SOC_DPP_DEFS_GET(unit, nof_flp_program_selection_lines): SOC_DPP_DEFS_GET(unit, nof_flp_programs)); ++indx) {
        if(SHR_BITGET(val_lcl,indx) == 0) {
            return indx;
        }
    }
    return -1;
...
STATIC uint32 arad_pp_flp_get_next_avaialable(
        int unit,
        uint32 prog_usage,
        int32 *cam_sel,
        int32 *prog
   )
{
  ...
  for (indx = 0; indx < SOC_DPP_DEFS_GET(unit, nof_flp_program_selection_lines); ++indx) {
      ...
      if (prog_selection_cam_tbl.valid == 1) {
          /* used line */
          cam_status |= 1 << indx;
          /* used prog */
          prog_status |= 1 << prog_selection_cam_tbl.program;
      }
  }

  if(ARAD_PP_L3_ROUTING_ENABLED(unit)) {
      /* The program is configured, even if not used with program selection. */
      prog_status |= 1 << ARAD_PP_FLP_KEY_PROGRAM_IPV4MC_WITH_RPF;
  }

  if (cam_sel != NULL) {
    *cam_sel = arad_pp_get_first_zero_bit(unit, TRUE, &cam_status);
  }

  *prog = arad_pp_get_first_zero_bit(unit, FALSE, &prog_status);

  if (((cam_sel != NULL) && (*cam_sel == -1)) || (*prog == -1)) {
      SOC_SAND_SET_ERROR_CODE(SOC_PPD_ERR_OUT_OF_RESOURCES, 20, exit);
  }

  res = sw_state_access[unit].dpp.soc.arad.pp.fec.flp_progs_mapping.set(unit,*prog,prog_usage);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 10, exit);

exit:
    SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_pp_flp_get_next_avaialable", 0, 0);
}


STATIC uint32 arad_pp_flp_cam_table_next_avaialable_get(
        int unit,
        int32 *cam_sel
   )
{
  ...
  if (((cam_sel != NULL) && (*cam_sel == -1))) {
      SOC_SAND_SET_ERROR_CODE(SOC_PPD_ERR_OUT_OF_RESOURCES, 20, exit);
  }

exit:
    SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_pp_flp_cam_table_next_avaialable_get", 0, 0);
}
   ...
   )
{
  ...
  if (SOC_DPP_PP_ENABLE(unit)) {
      /* Set this SOC property only in PP mode, since the SW DB is saved in PP and not allocated in TM */
      ARAD_PP_FLP_SW_DB_FLP_PROGRAM_ID_SET(unit, ARAD_PP_FLP_KEY_PROGRAM_TM, ARAD_PP_FLP_KEY_PROGRAM_TM);
  }

  res = arad_pp_ihb_flp_program_selection_cam_tbl_get_unsafe(unit, ARAD_PP_FLP_PROG_SEL_CAM_TM, &prog_selection_cam_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
    
  prog_selection_cam_tbl.packet_format_code  = 0x20; /* 6'b10_0000;*/
  prog_selection_cam_tbl.packet_format_code_mask  = 0x1F; /* 6'b01_1111;*/

  prog_selection_cam_tbl.program = ARAD_PP_FLP_KEY_PROGRAM_TM;
  ...
  res = arad_pp_ihb_flp_program_selection_cam_tbl_set_unsafe(unit, ARAD_PP_FLP_PROG_SEL_CAM_TM, &prog_selection_cam_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
exit:
   ...
   )
{
  ...
  uint32
    i = 0;
  int service_type = 1; /*AC to PWE */

  SOC_SAND_INIT_ERROR_DEFINITIONS(0);

  /* For static, program id and app id is same */
  ARAD_PP_FLP_SW_DB_FLP_PROGRAM_ID_SET(unit, ARAD_PP_FLP_KEY_PROGRAM_P2P, ARAD_PP_FLP_KEY_PROGRAM_P2P);

  for (i=0; i<2; i++)
  {
    res = arad_pp_ihb_flp_program_selection_cam_tbl_get_unsafe(unit, ARAD_PP_FLP_PROG_SEL_CAM_P2P_BASE+i, &prog_selection_cam_tbl);
    SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);

    prog_selection_cam_tbl.service_type = service_type;
    /* Consider alternative: select according to the Packet-Format-Code MSB unset (Eth-based packets) */
    prog_selection_cam_tbl.parser_leaf_context = ARAD_PMF_PGM_SEL_PARSER_PROFILE_PP; 
    ...
    prog_selection_cam_tbl.program = ARAD_PP_FLP_KEY_PROGRAM_P2P;
    prog_selection_cam_tbl.valid = 1;
    res = arad_pp_ihb_flp_program_selection_cam_tbl_set_unsafe(unit, ARAD_PP_FLP_PROG_SEL_CAM_P2P_BASE+i, &prog_selection_cam_tbl);    
    SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
    service_type = 0; /*PWE to PWE */  /*actually any p2p service except for ac->pwe will hit this service type*/
  }
exit:
   ...
   )
{
  ...
  for (i = 0; i < 2; i++) { /* DHCP, static */
    ...
    flp_prog_sel = (i == 0) ? ARAD_PP_FLP_PROG_SEL_CAM_ETH_TK_EPON_UNI_V4_DHCP:ARAD_PP_FLP_PROG_SEL_CAM_ETH_TK_EPON_UNI_V4_STATIC;      
    /* For static, program id and app id is same */
    program_id = (flp_prog_sel == ARAD_PP_FLP_PROG_SEL_CAM_ETH_TK_EPON_UNI_V4_DHCP) ? 
          ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_TK_EPON_UNI_V4_DHCP:ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_TK_EPON_UNI_V4_STATIC;
    ARAD_PP_FLP_SW_DB_FLP_PROGRAM_ID_SET(unit, program_id, program_id);
    res = arad_pp_ihb_flp_program_selection_cam_tbl_get_unsafe(unit, flp_prog_sel, &prog_selection_cam_tbl);
    SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);

    prog_selection_cam_tbl.packet_format_code = ARAD_PARSER_PKT_HDR_STK_TYPE_IPV4_ETH;
    ...
    prog_selection_cam_tbl.forwarding_code = ARAD_PP_FWD_CODE_ETHERNET;  
    prog_selection_cam_tbl.forwarding_code_mask = 0x00;    
    prog_selection_cam_tbl.cos_profile = ARAD_PP_FLP_COS_PROFILE_ANTI_SPOOFING;
    prog_selection_cam_tbl.cos_profile_mask = 0x1F; /* 6'b011111 */
    prog_selection_cam_tbl.tt_lookup_0_found = (flp_prog_sel == ARAD_PP_FLP_PROG_SEL_CAM_ETH_TK_EPON_UNI_V4_DHCP) ? 0x1:0x0;
    prog_selection_cam_tbl.tt_lookup_0_found_mask = 0x0;
    prog_selection_cam_tbl.program = (flp_prog_sel == ARAD_PP_FLP_PROG_SEL_CAM_ETH_TK_EPON_UNI_V4_DHCP) ? 
          ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_TK_EPON_UNI_V4_DHCP:ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_TK_EPON_UNI_V4_STATIC;
    prog_selection_cam_tbl.valid = 1;
    res = arad_pp_ihb_flp_program_selection_cam_tbl_set_unsafe(unit, flp_prog_sel, &prog_selection_cam_tbl);
   ...
   )
{
  ...
  for (i = 0; i < 2; i++) { /* DHCP, static */
    ...
    flp_prog_sel = (i == 0) ? ARAD_PP_FLP_PROG_SEL_CAM_ETH_TK_EPON_UNI_V6_DHCP:ARAD_PP_FLP_PROG_SEL_CAM_ETH_TK_EPON_UNI_V6_STATIC;      

    /* For static, program id and app id is same */
    program_id = (flp_prog_sel == ARAD_PP_FLP_PROG_SEL_CAM_ETH_TK_EPON_UNI_V6_DHCP) ? 
          ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_TK_EPON_UNI_V6_DHCP:ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_TK_EPON_UNI_V6_STATIC;
    ARAD_PP_FLP_SW_DB_FLP_PROGRAM_ID_SET(unit, program_id, program_id);

    res = arad_pp_ihb_flp_program_selection_cam_tbl_get_unsafe(unit, flp_prog_sel, &prog_selection_cam_tbl);
    SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);

    prog_selection_cam_tbl.packet_format_code = ARAD_PARSER_PKT_HDR_STK_TYPE_IPV6_ETH;
    prog_selection_cam_tbl.packet_format_code_mask = 0x18 /* 6'b011000 */;
    prog_selection_cam_tbl.parser_leaf_context = ARAD_PMF_PGM_SEL_PARSER_PROFILE_PP;
    prog_selection_cam_tbl.parser_leaf_context_mask = 0x03; /* ignore 2 LSBs because 4 higher Parser-Leaf-Contexts are considered regular PP for FLP */
    prog_selection_cam_tbl.port_profile = 0x0;
    ...
    prog_selection_cam_tbl.cos_profile = ARAD_PP_FLP_COS_PROFILE_ANTI_SPOOFING;
    prog_selection_cam_tbl.cos_profile_mask = 0x1F; /* 6'b011111 */
    if (flp_prog_sel == ARAD_PP_FLP_PROG_SEL_CAM_ETH_TK_EPON_UNI_V6_DHCP){
    ...
    prog_selection_cam_tbl.program = (flp_prog_sel == ARAD_PP_FLP_PROG_SEL_CAM_ETH_TK_EPON_UNI_V6_DHCP) ? 
          ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_TK_EPON_UNI_V6_DHCP:ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_TK_EPON_UNI_V6_STATIC;
    prog_selection_cam_tbl.valid = 1;
    res = arad_pp_ihb_flp_program_selection_cam_tbl_set_unsafe(unit, flp_prog_sel, &prog_selection_cam_tbl);
    SOC_SAND_CHECK_FUNC_RESULT(res, 45, exit);
   ...
   )
{
  ...
  res = arad_pp_flp_get_next_avaialable(unit, ARAD_PP_FLP_KEY_PROGRAM_PON_ARP_DOWNSTREAM, &cam_sel, prog_id);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
   
  res = arad_pp_ihb_flp_program_selection_cam_tbl_get_unsafe(unit, cam_sel, &prog_selection_cam_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
  ...
  res = arad_pp_ihb_flp_program_selection_cam_tbl_set_unsafe(unit, cam_sel, &prog_selection_cam_tbl);
   ...
   )
{
  ...
  res = arad_pp_flp_get_next_avaialable(unit, ARAD_PP_FLP_KEY_PROGRAM_PON_ARP_UPSTREAM, &cam_sel, prog_id);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
   ...
   )
{
  ...
  /* For static, program id and app id is same */
  ARAD_PP_FLP_SW_DB_FLP_PROGRAM_ID_SET(unit, ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_ING_LEARN, ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_ING_LEARN);

  res = arad_pp_ihb_flp_program_selection_cam_tbl_get_unsafe(unit, ARAD_PP_FLP_PROG_SEL_CAM_ETH_ING_LRN, &prog_selection_cam_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  ...
  prog_selection_cam_tbl.ptc_profile_mask = 0x03;
  prog_selection_cam_tbl.forwarding_code_mask = 0x00;
  prog_selection_cam_tbl.program = ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_ING_LEARN;
  prog_selection_cam_tbl.valid = 1;
  res = arad_pp_ihb_flp_program_selection_cam_tbl_set_unsafe(unit, ARAD_PP_FLP_PROG_SEL_CAM_ETH_ING_LRN, &prog_selection_cam_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);

  if (mac_in_mac_enabled) {

      res = arad_pp_flp_get_next_avaialable(unit,ARAD_PP_FLP_KEY_PROGRAM_MAC_IN_MAC_AFTER_TERMINATIOM,&cam_sel, mac_in_mac_prog_id);
      SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
      ...
      prog_selection_cam_tbl.valid = 1;
      res = arad_pp_ihb_flp_program_selection_cam_tbl_set_unsafe(unit, ARAD_PP_FLP_PROG_SEL_CAM_ETH_PBP, &prog_selection_cam_tbl);
      SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);
  }
exit:
   ...
   )
{
  ...
  res = arad_pp_flp_get_next_avaialable(unit,ARAD_PP_FLP_KEY_PROGRAM_OAM_STATISTICS,&cam_sel,prog_id);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
   ...
   )
{
  ...
  res = arad_pp_flp_get_next_avaialable(unit,ARAD_PP_FLP_KEY_PROGRAM_OAM_DOWN_UNTAGGED_STATISTICS,&cam_sel,prog_id);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
   ...
   )
{
  ...
  res = arad_pp_flp_get_next_avaialable(unit,ARAD_PP_FLP_KEY_PROGRAM_OAM_SINGLE_TAG_STATISTICS,&cam_sel,prog_id);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
   ...
   )
{
  ...
  res = arad_pp_flp_get_next_avaialable(unit,ARAD_PP_FLP_KEY_PROGRAM_OAM_DOUBLE_TAG_STATISTICS,&cam_sel,prog_id);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
   ...
   )
{
  ...
  res = arad_pp_flp_get_next_avaialable(unit,ARAD_PP_FLP_KEY_PROGRAM_BFD_MPLS_STATISTICS,&cam_sel,prog_id);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
   ...
   )
{
  ...
  res = arad_pp_flp_get_next_avaialable(unit,ARAD_PP_FLP_KEY_PROGRAM_BFD_PWE_STATISTICS,&cam_sel,prog_id);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
   ...
   )
{
  ...
  res = arad_pp_flp_get_next_avaialable(unit,ARAD_PP_FLP_KEY_PROGRAM_BFD_STATISTICS,&cam_sel,prog_id);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
   ...
   )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(0);
  /* Set input parameter cam_sel as NULL to ask only for program. */
  res = arad_pp_flp_get_next_avaialable(unit,ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_PON_DEFAULT_UPSTREAM,NULL,prog_id);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  /* Always want to the priority of pon upstream default program selector is lower than
   * the priority of L3 source binding program selector.
   * Here use a static for pon upstream default program selector, and the value is
   * ARAD_PP_FLP_PROG_SEL_CAM_ETH_PON_DEFAULT_UPSTREAM(27).
   */
  cam_sel = ARAD_PP_FLP_PROG_SEL_CAM_ETH_PON_DEFAULT_UPSTREAM;
   ...
   )
{
  ...
  res = arad_pp_flp_get_next_avaialable(unit,ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_PON_DEFAULT_DOWNSTREAM,&cam_sel,prog_id);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
   ...
   )
{
  ...
  res = arad_pp_flp_get_next_avaialable(unit,ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_PON_LOCAL_ROUTE,&cam_sel,prog_id);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  ...
  prog_selection_cam_tbl.forwarding_code = ARAD_PP_FWD_CODE_ETHERNET;  
  prog_selection_cam_tbl.forwarding_code_mask = 0x00;
  prog_selection_cam_tbl.cos_profile = ARAD_PP_l2_LIF_AC_LOCAL_SWITCHING_COS_PROFILE;
  prog_selection_cam_tbl.cos_profile_mask = 0x1F; 
  prog_selection_cam_tbl.program = *prog_id;
  prog_selection_cam_tbl.valid = 1;
  res = arad_pp_ihb_flp_program_selection_cam_tbl_set_unsafe(unit, cam_sel, &prog_selection_cam_tbl);
  ...
  if (mac_in_mac_enabled) {
      int32 mim_prog_id;
      res = arad_pp_flp_get_next_avaialable(unit,ARAD_PP_FLP_KEY_PROGRAM_MAC_IN_MAC_AFTER_TERMINATIOM,&cam_sel,&mim_prog_id);
      SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);




      res = arad_pp_ihb_flp_program_selection_cam_tbl_get_unsafe(unit, ARAD_PP_FLP_PROG_SEL_CAM_ETH_PBP, &prog_selection_cam_tbl);
      SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);
   ...
   )
{
  ...
  if(is_fcoe_enabled){      
      res = arad_pp_flp_get_next_avaialable(unit,ARAD_PP_FLP_KEY_PROGRAM_FC_TRANSIT,&cam_sel,prog_id);
      SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);

  }else{
      (*prog_id) = ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_ING_LEARN;
      res = arad_pp_flp_cam_table_next_avaialable_get(unit, &cam_sel);
  }
  ...
  /* pfc is FCoE*/
  /* ARAD_PARSER_PKT_HDR_STK_TYPE_FC_STD_ETH or ARAD_PARSER_PKT_HDR_STK_TYPE_FC_ENCAP_ETH*/
  prog_selection_cam_tbl.packet_format_code  = ARAD_PARSER_PKT_HDR_STK_TYPE_FC_STD_ETH; /* 6'b000100;*/
  prog_selection_cam_tbl.packet_format_code_mask  = 0x1; /* 6'b00_0100;*/

  /* PLC is ARAD_PMF_PGM_SEL_PARSER_PROFILE_FCOE_VFT or ARAD_PMF_PGM_SEL_PARSER_PROFILE_FCoE*/
  prog_selection_cam_tbl.parser_leaf_context = ARAD_PMF_PGM_SEL_PARSER_PROFILE_FCOE_VFT; /* b0100*/
     ...
     )
{
  ...
  if (is_npv == 1) {
      prog_usage = ARAD_PP_FLP_KEY_PROGRAM_FC_WITH_VFT_N_PORT;
  }else{
      prog_usage = ARAD_PP_FLP_KEY_PROGRAM_FC_WITH_VFT;
  }
    
  res = arad_pp_flp_get_next_avaialable(unit,prog_usage,&cam_sel,prog_id);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  ...
  prog_selection_cam_tbl.parser_leaf_context     = ARAD_PMF_PGM_SEL_PARSER_PROFILE_FCOE_VFT; /* with-vft*/
  prog_selection_cam_tbl.tt_processing_profile   = ARAD_PP_IHP_VTT_TT_PROCESSING_PROFILE_FC; /* 3 */
  prog_selection_cam_tbl.packet_format_code      = ARAD_PARSER_PKT_HDR_STK_TYPE_FC_STD_ETH;
   ...
   )
{
  ...
  if (is_npv == 1) {
      prog_usage = ARAD_PP_FLP_KEY_PROGRAM_FC_N_PORT;
  }else{
      prog_usage = ARAD_PP_FLP_KEY_PROGRAM_FC;
  }
  ...
  prog_selection_cam_tbl.tt_lookup_0_found              = 0x1; /* local */
  prog_selection_cam_tbl.parser_leaf_context            = ARAD_PMF_PGM_SEL_PARSER_PROFILE_FCOE_VFT; /* with-vft*/
  prog_selection_cam_tbl.packet_format_code             = ARAD_PARSER_PKT_HDR_STK_TYPE_FC_ENCAP_ETH; /* both header according to mask*/
   ...
   )
{
  ...
  res = arad_pp_flp_get_next_avaialable(unit,ARAD_PP_FLP_KEY_PROGRAM_FC_WITH_VFT_REMOTE,&cam_sel,prog_id);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
   ...
   )
{
  ...
  res = arad_pp_flp_get_next_avaialable(unit,ARAD_PP_FLP_KEY_PROGRAM_FC_REMOTE,&cam_sel,prog_id);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  ...
  prog_selection_cam_tbl.tt_lookup_0_found              = 0x0;  /* remote */
  prog_selection_cam_tbl.parser_leaf_context            = ARAD_PMF_PGM_SEL_PARSER_PROFILE_FCOE_VFT; /* with-vft */
  prog_selection_cam_tbl.packet_format_code             = SOC_TMC_PARSER_PKT_HDR_STK_TYPE_FC_ENCAP_ETH; 
   ...
   )
{
  ...
  /* For static, program id and app id is same */
  ARAD_PP_FLP_SW_DB_FLP_PROGRAM_ID_SET(unit, ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_MAC_IN_MAC, ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_MAC_IN_MAC);

  res = arad_pp_ihb_flp_program_selection_cam_tbl_get_unsafe(unit, ARAD_PP_FLP_PROG_SEL_CAM_ETH_MAC_IN_MAC, &prog_selection_cam_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  ...
  prog_selection_cam_tbl.forwarding_code_mask = 0x00;
  prog_selection_cam_tbl.forwarding_offset_index_mask = 0;
  prog_selection_cam_tbl.program = ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_MAC_IN_MAC;
  prog_selection_cam_tbl.valid = 1;
  res = arad_pp_ihb_flp_program_selection_cam_tbl_set_unsafe(unit, ARAD_PP_FLP_PROG_SEL_CAM_ETH_MAC_IN_MAC, &prog_selection_cam_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
exit:
...
uint32
   arad_pp_flp_prog_sel_cam_ipv4uc_with_l3vpn(
     int unit
   )
{
  uint32
  ...
  /* For static, program id and app id is same */
  ARAD_PP_FLP_SW_DB_FLP_PROGRAM_ID_SET(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV4UC_WITH_DEFAULT, ARAD_PP_FLP_KEY_PROGRAM_IPV4UC_WITH_DEFAULT);

  res = arad_pp_ihb_flp_program_selection_cam_tbl_get_unsafe(unit, ARAD_PP_FLP_PROG_SEL_CAM_IPV4UC_L3VPN, &prog_selection_cam_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  ...
  prog_selection_cam_tbl.program                   = ARAD_PP_FLP_KEY_PROGRAM_IPV4UC_WITH_DEFAULT;
  prog_selection_cam_tbl.valid                     = 1;
  res = arad_pp_ihb_flp_program_selection_cam_tbl_set_unsafe(unit, ARAD_PP_FLP_PROG_SEL_CAM_IPV4UC_L3VPN, &prog_selection_cam_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
exit:
   ...
   )
{
  ...
  /* For static, program id and app id is same */
  /* ARAD_PP_FLP_SW_DB_FLP_PROGRAM_ID_SET(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV6UC_WITH_RPF_2PASS, ARAD_PP_FLP_KEY_PROGRAM_IPV6UC_WITH_RPF_2PASS); */
  res = arad_pp_flp_get_next_avaialable(unit,ARAD_PP_FLP_KEY_PROGRAM_IPV6UC_WITH_RPF_2PASS,NULL,prog_id);
  SOC_SAND_CHECK_FUNC_RESULT(res, 210, exit);

  res = arad_pp_ihb_flp_program_selection_cam_tbl_get_unsafe(unit, ARAD_PP_FLP_PROG_SEL_CAM_IPV6UC_2PASS, &prog_selection_cam_tbl);
  ...
  prog_selection_cam_tbl.program                   = *prog_id;
  prog_selection_cam_tbl.valid                     = 1;
  res = arad_pp_ihb_flp_program_selection_cam_tbl_set_unsafe(unit, ARAD_PP_FLP_PROG_SEL_CAM_IPV6UC_2PASS, &prog_selection_cam_tbl);
   ...
   )
{
  ...
  /* For static, program id and app id is same */
  ARAD_PP_FLP_SW_DB_FLP_PROGRAM_ID_SET(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV4UC_WITH_RPF, ARAD_PP_FLP_KEY_PROGRAM_IPV4UC_WITH_RPF);

  res = arad_pp_ihb_flp_program_selection_cam_tbl_get_unsafe(unit, ARAD_PP_FLP_PROG_SEL_CAM_IPV4UC_RPF, &prog_selection_cam_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  ...
  prog_selection_cam_tbl.program                   = ARAD_PP_FLP_KEY_PROGRAM_IPV4UC_WITH_RPF;
  prog_selection_cam_tbl.valid                     = 1;
  res = arad_pp_ihb_flp_program_selection_cam_tbl_set_unsafe(unit, ARAD_PP_FLP_PROG_SEL_CAM_IPV4UC_RPF, &prog_selection_cam_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
exit:
   ...
   )
{
  ...
  /* For static, program id and app id is same */
  ARAD_PP_FLP_SW_DB_FLP_PROGRAM_ID_SET(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV4UC, ARAD_PP_FLP_KEY_PROGRAM_IPV4UC);

  res = arad_pp_ihb_flp_program_selection_cam_tbl_get_unsafe(unit, ARAD_PP_FLP_PROG_SEL_CAM_IPV4UC, &prog_selection_cam_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  ...
  prog_selection_cam_tbl.forwarding_code_mask        = 0x00;
  prog_selection_cam_tbl.in_rif_uc_rpf_enable_mask   = 0x00;
  prog_selection_cam_tbl.program                   = ARAD_PP_FLP_KEY_PROGRAM_IPV4UC;
  prog_selection_cam_tbl.valid                     = 1;
  res = arad_pp_ihb_flp_program_selection_cam_tbl_set_unsafe(unit, ARAD_PP_FLP_PROG_SEL_CAM_IPV4UC, &prog_selection_cam_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
exit:
   ...
   )
{
  ...
  prog_usage = (is_ipv6 ? ARAD_PP_FLP_KEY_PROGRAM_BFD_IPV6_SINGLE_HOP : ARAD_PP_FLP_KEY_PROGRAM_BFD_IPV4_SINGLE_HOP);      
  ...
  prog_selection_cam_tbl.parser_leaf_context         = ARAD_PMF_PGM_SEL_PARSER_PROFILE_BFD_SINGLE_HOP;
  prog_selection_cam_tbl.port_profile                = 0;
  prog_selection_cam_tbl.ptc_profile                 = 0;
   ...
   )
{
  ...
  /* For static, program id and app id is same */
  /* Alreday done: ARAD_PP_FLP_SW_DB_FLP_PROGRAM_ID_SET(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV6UC, ARAD_PP_FLP_KEY_PROGRAM_IPV6UC); */

  /* CAM selection for second pass (IPv6 with RPF RCY port) must be higher than first pass */
  res = arad_pp_ihb_flp_program_selection_cam_tbl_get_unsafe(unit, ARAD_PP_FLP_PROG_SEL_CAM_IPV6UC_2PASS_FWD, &prog_selection_cam_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  ...
  prog_selection_cam_tbl.program                   = ARAD_PP_FLP_KEY_PROGRAM_IPV6UC;
  prog_selection_cam_tbl.valid                     = 1;
  res = arad_pp_ihb_flp_program_selection_cam_tbl_set_unsafe(unit, ARAD_PP_FLP_PROG_SEL_CAM_IPV6UC_2PASS_FWD, &prog_selection_cam_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
exit:
   ...
   )
{
  ...
  /* For static, program id and app id is same */
  ARAD_PP_FLP_SW_DB_FLP_PROGRAM_ID_SET(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV6UC, ARAD_PP_FLP_KEY_PROGRAM_IPV6UC);

  res = arad_pp_ihb_flp_program_selection_cam_tbl_get_unsafe(unit, ARAD_PP_FLP_PROG_SEL_CAM_IPV6UC, &prog_selection_cam_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  ...
  prog_selection_cam_tbl.program                   = ARAD_PP_FLP_KEY_PROGRAM_IPV6UC;
  prog_selection_cam_tbl.valid                     = 1;
  res = arad_pp_ihb_flp_program_selection_cam_tbl_set_unsafe(unit, ARAD_PP_FLP_PROG_SEL_CAM_IPV6UC, &prog_selection_cam_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
exit:
...
STATIC
  uint32
    arad_pp_flp_elk_instruction_rsrc_set(
...
STATIC
  uint32
    arad_pp_flp_elk_all_progs_instruction_set(
       SOC_SAND_IN  int  unit
    )
{
  uint32
  ...
  {
      res = arad_pp_flp_elk_instruction_rsrc_set(unit, prog_idx);
      SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  }

exit:
 SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_pp_flp_elk_all_progs_instruction_set", 0, 0);
}
    ...
    )
{
    ...
    /* 2. Configure Result Processing:
     *    Set the ELK result format, and how to process
     *    ELK results in forwarding.
     */

    res = arad_pp_ihb_flp_process_tbl_get_unsafe(
            unit, 
            prog_id, 
            &flp_process_tbl
          );
    ...
    res = arad_pp_ihb_flp_process_tbl_set_unsafe(
            unit, 
            prog_id, 
            &flp_process_tbl
          );
    ...
    )
{
   ...
   res = arad_pp_ihb_flp_lookups_tbl_get_unsafe(
    ...
    SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
    *max_key_size_in_bits = 160 - (flp_lookups_tbl.elk_key_c_valid_bytes * SOC_SAND_NOF_BITS_IN_CHAR);
   ...
   )
{
    ...
    /* program selection */
    /* allocate resources */
    res = arad_pp_flp_get_next_avaialable(unit,ARAD_PP_FLP_KEY_PROGRAM_IPV6UC_WITH_RPF,&cam_sel,&prog_id);
    SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
    ...
    SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);
    flp_key_construction_tbl.instruction_0_16b        = ARAD_PP_FLP_16B_INST_P6_VRF_D;
    ...
    /* reference: ARAD_PP_FLP_KEY_PROGRAM_IPV4UC_WITH_RPF */
    /* flp lookups */
    ...
    flp_process_tbl.not_found_trap_code         = ARAD_PP_TRAP_CODE_INTERNAL_FLP_DEFAULT_UCV6;
    ...
    SHR_BITCOPY_RANGE(reg_val,3*ARAD_PP_FLP_KEY_PROGRAM_IPV6UC,&tmp,0,3);
   ...
   )
{
  ...
  /* For static, program id and app id is same */
  ARAD_PP_FLP_SW_DB_FLP_PROGRAM_ID_SET(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV6MC, ARAD_PP_FLP_KEY_PROGRAM_IPV6MC);

  res = arad_pp_ihb_flp_program_selection_cam_tbl_get_unsafe(unit, ARAD_PP_FLP_PROG_SEL_CAM_IPV6MC, &prog_selection_cam_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  ...
  prog_selection_cam_tbl.forwarding_code_mask        = 0x00;
  prog_selection_cam_tbl.in_rif_uc_rpf_enable_mask   = 0x00;
  prog_selection_cam_tbl.program                     = ARAD_PP_FLP_KEY_PROGRAM_IPV6MC;
  prog_selection_cam_tbl.valid                       = 1;
  res = arad_pp_ihb_flp_program_selection_cam_tbl_set_unsafe(unit, ARAD_PP_FLP_PROG_SEL_CAM_IPV6MC, &prog_selection_cam_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
exit:
   ...
   )
{
  ...
  res = arad_pp_flp_get_next_avaialable(unit,ARAD_PP_FLP_KEY_PROGRAM_VPLSOGRE,&cam_sel,prog_id);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
   ...
   )
{
  ...
  /* For static, program id and app id is same */
  ARAD_PP_FLP_SW_DB_FLP_PROGRAM_ID_SET(unit, ARAD_PP_FLP_KEY_PROGRAM_LSR, ARAD_PP_FLP_KEY_PROGRAM_LSR);

  res = arad_pp_ihb_flp_program_selection_cam_tbl_get_unsafe(unit, ARAD_PP_FLP_PROG_SEL_CAM_LSR, &prog_selection_cam_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  ...
  prog_selection_cam_tbl.ptc_profile_mask            = 0x03;
  prog_selection_cam_tbl.forwarding_code_mask        = 0x00;
  prog_selection_cam_tbl.program                     = ARAD_PP_FLP_KEY_PROGRAM_LSR;
  prog_selection_cam_tbl.valid                       = 1;
  res = arad_pp_ihb_flp_program_selection_cam_tbl_set_unsafe(unit, ARAD_PP_FLP_PROG_SEL_CAM_LSR, &prog_selection_cam_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
exit:
...
uint32
   arad_pp_flp_prog_sel_cam_coupling_lsr(
     int unit
   )
{
  uint32
  ...
  /* For static, program id and app id is same */
  ARAD_PP_FLP_SW_DB_FLP_PROGRAM_ID_SET(unit, ARAD_PP_FLP_KEY_PROGRAM_COUPLING_LSR, ARAD_PP_FLP_KEY_PROGRAM_COUPLING_LSR);

  res = arad_pp_ihb_flp_program_selection_cam_tbl_get_unsafe(unit, ARAD_PP_FLP_PROG_SEL_CAM_COUPLING_LSR, &prog_selection_cam_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  prog_selection_cam_tbl.parser_leaf_context        = ARAD_PMF_PGM_SEL_PARSER_PROFILE_PP;
  prog_selection_cam_tbl.port_profile               = 0;
  prog_selection_cam_tbl.ptc_profile                = 0;
  prog_selection_cam_tbl.forwarding_code            = ARAD_PP_FWD_CODE_MPLS;
  prog_selection_cam_tbl.in_rif_uc_rpf_enable       = 0x00;
  prog_selection_cam_tbl.vt_processing_profile      = ARAD_PP_IHP_VTT_VT_PROCESSING_PROFILE_COUPLING;
  prog_selection_cam_tbl.parser_leaf_context_mask   = 0x03; /* ignore 2 LSBs because 4 higher Parser-Leaf-Contexts are considered regular PP for FLP */
  prog_selection_cam_tbl.port_profile_mask          = ARAD_PP_FLP_PORT_PROFILE_MASK_CONSIDER_PBP;
  prog_selection_cam_tbl.ptc_profile_mask           = 0x03;
  prog_selection_cam_tbl.forwarding_code_mask       = 0x00;
  prog_selection_cam_tbl.in_rif_uc_rpf_enable_mask  = 0x00;
  prog_selection_cam_tbl.vt_processing_profile_mask = 0x00;
  prog_selection_cam_tbl.program                    = ARAD_PP_FLP_KEY_PROGRAM_COUPLING_LSR;
  prog_selection_cam_tbl.valid                      = 1;
  res = arad_pp_ihb_flp_program_selection_cam_tbl_set_unsafe(unit, ARAD_PP_FLP_PROG_SEL_CAM_COUPLING_LSR, &prog_selection_cam_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
exit:
 SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_pp_flp_prog_sel_cam_coupling_lsr", 0, 0);
}
   ...
   )
{
  ...
  /* For static, program id and app id is same */
  ARAD_PP_FLP_SW_DB_FLP_PROGRAM_ID_SET(unit, ARAD_PP_FLP_KEY_PROGRAM_TRILL_UC, ARAD_PP_FLP_KEY_PROGRAM_TRILL_UC);

  res = arad_pp_ihb_flp_program_selection_cam_tbl_get_unsafe(unit, ARAD_PP_FLP_PROG_SEL_CAM_TRILL_UC, &prog_selection_cam_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  ...
  prog_selection_cam_tbl.program                 = ARAD_PP_FLP_KEY_PROGRAM_TRILL_UC;
  prog_selection_cam_tbl.valid                   = 1;
  res = arad_pp_ihb_flp_program_selection_cam_tbl_set_unsafe(unit, ARAD_PP_FLP_PROG_SEL_CAM_TRILL_UC, &prog_selection_cam_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
exit:
   ...
   )
{
  ...
  /* For static, program id and app id is same */
  ARAD_PP_FLP_SW_DB_FLP_PROGRAM_ID_SET(unit, ARAD_PP_FLP_KEY_PROGRAM_TRILL_MC_ONE_TAG, ARAD_PP_FLP_KEY_PROGRAM_TRILL_MC_ONE_TAG);

  /* One tag (VL) */
  res = arad_pp_ihb_flp_program_selection_cam_tbl_get_unsafe(unit, ARAD_PP_FLP_PROG_SEL_CAM_TRILL_MC_ONE_TAG, &prog_selection_cam_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  ...
  prog_selection_cam_tbl.program                  = ARAD_PP_FLP_KEY_PROGRAM_TRILL_MC_ONE_TAG;
  prog_selection_cam_tbl.valid                    = 1;
  res = arad_pp_ihb_flp_program_selection_cam_tbl_set_unsafe(unit, ARAD_PP_FLP_PROG_SEL_CAM_TRILL_MC_ONE_TAG, &prog_selection_cam_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);

  /* TWO tags (FGL) */
  /* For static, program id and app id is same */
  ARAD_PP_FLP_SW_DB_FLP_PROGRAM_ID_SET(unit, ARAD_PP_FLP_KEY_PROGRAM_TRILL_MC_TWO_TAGS, ARAD_PP_FLP_KEY_PROGRAM_TRILL_MC_TWO_TAGS);

  res = arad_pp_ihb_flp_program_selection_cam_tbl_get_unsafe(unit, ARAD_PP_FLP_PROG_SEL_CAM_TRILL_MC_TWO_TAGS, &prog_selection_cam_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
  ...
  prog_selection_cam_tbl.trill_mc_mask            = 0x0;  
  prog_selection_cam_tbl.vt_lookup_1_found_mask   = 0x0;
  prog_selection_cam_tbl.program                  = ARAD_PP_FLP_KEY_PROGRAM_TRILL_MC_TWO_TAGS;
  prog_selection_cam_tbl.valid                    = 1;
  res = arad_pp_ihb_flp_program_selection_cam_tbl_set_unsafe(unit, ARAD_PP_FLP_PROG_SEL_CAM_TRILL_MC_TWO_TAGS, &prog_selection_cam_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);
exit:
   ...
   )
{
  ...
  /* For static, program id and app id is same */
  ARAD_PP_FLP_SW_DB_FLP_PROGRAM_ID_SET(unit, ARAD_PP_FLP_KEY_PROGRAM_TRILL_AFTER_TERMINATION, ARAD_PP_FLP_KEY_PROGRAM_TRILL_AFTER_TERMINATION);

  res = arad_pp_ihb_flp_program_selection_cam_tbl_get_unsafe(unit, ARAD_PP_FLP_PROG_SEL_CAM_TRILL_MC_OVERLAY, &prog_selection_cam_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  ...
  prog_selection_cam_tbl.forwarding_code_mask    = 0x00;
  prog_selection_cam_tbl.trill_mc_mask           = 0x0;  
  prog_selection_cam_tbl.program                 = ARAD_PP_FLP_KEY_PROGRAM_TRILL_AFTER_TERMINATION;
  prog_selection_cam_tbl.valid                   = 1;
  res = arad_pp_ihb_flp_program_selection_cam_tbl_set_unsafe(unit, ARAD_PP_FLP_PROG_SEL_CAM_TRILL_MC_OVERLAY, &prog_selection_cam_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
exit:
   ...
   )
{
  ...
  res = arad_pp_flp_get_next_avaialable(unit, ARAD_PP_FLP_KEY_PROGRAM_VMAC_UPSTREAM, &cam_sel, prog_id);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  ...
  prog_selection_cam_tbl.forwarding_code            = ARAD_PP_FWD_CODE_ETHERNET;
  prog_selection_cam_tbl.forwarding_code_mask       = 0x00;
  prog_selection_cam_tbl.cos_profile                = ARAD_PP_FLP_COS_PROFILE_VMAC;
  prog_selection_cam_tbl.cos_profile_mask           = 0x1F;
  prog_selection_cam_tbl.program                    = *prog_id;
  prog_selection_cam_tbl.valid                      = 1;
   ...
   )
{
  ...
  res = arad_pp_flp_get_next_avaialable(unit, ARAD_PP_FLP_KEY_PROGRAM_VMAC_DOWNSTREAM, &cam_sel, prog_id);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
...
uint32
   arad_pp_flp_lookups_ethernet_tk_epon_uni_v4(
     int unit,
     uint8 sa_auth_enabled,
     uint8 slb_enabled
   )
{
  ...
  for (i = 0; i < 2; i++) { /* DHCP, static */
    ...
    flp_prog_sel = (i == 0) ? ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_TK_EPON_UNI_V4_DHCP:ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_TK_EPON_UNI_V4_STATIC;
    ...
    flp_lookups_tbl.lem_2nd_lkp_and_value  = (flp_prog_sel == ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_TK_EPON_UNI_V4_DHCP) ? 0x7:0x0;
    flp_lookups_tbl.lem_2nd_lkp_or_value   = (flp_prog_sel == ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_TK_EPON_UNI_V4_DHCP) ? ARAD_PP_FLP_IP_SPOOF_DHCP_KEY_OR_MASK:ARAD_PP_FLP_IPV4_SPOOF_STATIC_KEY_OR_MASK;

    if (SOC_DPP_L3_SRC_BIND_IPV4_SUBNET_ENABLE(unit) &&
      flp_prog_sel == ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_TK_EPON_UNI_V4_STATIC) {
...
exit:
 SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_pp_flp_lookups_ethernet_tk_epon_uni_v4", 0, 0);
}
   ...
   )
{
  ...
  flp_prog_sel = ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_TK_EPON_UNI_V6_STATIC;
  res = arad_pp_ihb_flp_lookups_tbl_get_unsafe(unit, flp_prog_sel, &flp_lookups_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 42, exit);
  ...
  /* Don't install SAV DHCP because prefix value is conflict */
  if (!SOC_DPP_CONFIG(unit)->pp.compression_spoof_ip6_enable) {
    flp_prog_sel = ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_TK_EPON_UNI_V6_DHCP;
    res = arad_pp_ihb_flp_lookups_tbl_get_unsafe(unit, flp_prog_sel, &flp_lookups_tbl);
   ...
   )
{
  ...
  res = arad_pp_ihb_flp_lookups_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_TK_EPON_UNI_V6_STATIC, &flp_process_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 41, exit);
...
uint32
   arad_pp_flp_lookups_oam_statistics(
     int unit,
     int prog_id
   )
{
  ...
  flp_lookups_tbl.lem_1st_lkp_key_type   = 0; /* no AGET access */
  flp_lookups_tbl.lem_1st_lkp_and_value  = 0x0;
  res = arad_pp_lem_access_app_to_prefix_get(unit,ARAD_PP_FLP_OAM_STATISTICS_KEY_OR_MASK,&flp_lookups_tbl.lem_1st_lkp_or_value);
  SOC_SAND_CHECK_FUNC_RESULT(res, 42, exit);
  flp_lookups_tbl.lem_2nd_lkp_valid     = 1;
  ...
  SOC_SAND_CHECK_FUNC_RESULT(res, 42, exit);
exit:
 SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_pp_flp_lookups_oam_statistics", 0, 0);
}

uint32
   arad_pp_flp_lookups_oam_down_untagged_statistics(
     int unit,
     int prog_id
   )
{
  ...
  flp_lookups_tbl.lem_1st_lkp_key_type   = 0; /* no AGET access */
  flp_lookups_tbl.lem_1st_lkp_and_value  = 0x0;
  res = arad_pp_lem_access_app_to_prefix_get(unit,ARAD_PP_FLP_OAM_DOWN_UNTAGGED_STATISTICS_KEY_OR_MASK,&flp_lookups_tbl.lem_1st_lkp_or_value);
  SOC_SAND_CHECK_FUNC_RESULT(res, 42, exit);
  flp_lookups_tbl.lem_2nd_lkp_valid     = 1;
  flp_lookups_tbl.lem_2nd_lkp_key_select = ARAD_PP_FLP_LKP_KEY_SELECT_FID_FWD_MAC_KEY_HW_VAL; 
  flp_lookups_tbl.lem_2nd_lkp_and_value  = 0x0;
  ...
  SOC_SAND_CHECK_FUNC_RESULT(res, 42, exit);
exit:
 SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_pp_flp_lookups_oam_down_untagged_statistics", 0, 0);
}


uint32
   arad_pp_flp_lookups_oam_single_tag_statistics(
     int unit,
     int prog_id
   )
{
  ...
  flp_lookups_tbl.lem_1st_lkp_key_type   = 0; /* no AGET access */
  flp_lookups_tbl.lem_1st_lkp_and_value  = 0x0;
  res = arad_pp_lem_access_app_to_prefix_get(unit,ARAD_PP_FLP_OAM_SINGLE_TAG_STATISTICS_KEY_OR_MASK,&flp_lookups_tbl.lem_1st_lkp_or_value);
  SOC_SAND_CHECK_FUNC_RESULT(res, 42, exit);
  flp_lookups_tbl.lem_2nd_lkp_valid     = 1;
  flp_lookups_tbl.lem_2nd_lkp_key_select = ARAD_PP_FLP_LKP_KEY_SELECT_FID_FWD_MAC_KEY_HW_VAL; 
  flp_lookups_tbl.lem_2nd_lkp_and_value  = 0x0;
...
uint32
   arad_pp_flp_lookups_oam_double_tag_statistics(
     int unit,
     int prog_id
   )
{
  ...
  flp_lookups_tbl.lem_1st_lkp_key_type   = 0; /* no AGET access */
  flp_lookups_tbl.lem_1st_lkp_and_value  = 0x0;
  res = arad_pp_lem_access_app_to_prefix_get(unit,ARAD_PP_FLP_OAM_DOUBLE_TAG_STATISTICS_KEY_OR_MASK,&flp_lookups_tbl.lem_1st_lkp_or_value);
  SOC_SAND_CHECK_FUNC_RESULT(res, 42, exit);
  flp_lookups_tbl.lem_2nd_lkp_valid     = 1;
  flp_lookups_tbl.lem_2nd_lkp_key_select = ARAD_PP_FLP_LKP_KEY_SELECT_FID_FWD_MAC_KEY_HW_VAL; 
  flp_lookups_tbl.lem_2nd_lkp_and_value  = 0x0;
...
uint32
   arad_pp_flp_lookups_bfd_statistics(
     int unit,
     int prog_id
   )
{
  ...
  flp_lookups_tbl.lem_1st_lkp_key_type   = 0; /* no AGET access */
  flp_lookups_tbl.lem_1st_lkp_and_value  = 0x0;
  res = arad_pp_lem_access_app_to_prefix_get(unit,ARAD_PP_FLP_BFD_STATISTICS_KEY_OR_MASK,&flp_lookups_tbl.lem_1st_lkp_or_value);
  SOC_SAND_CHECK_FUNC_RESULT(res, 42, exit);
  flp_lookups_tbl.lem_2nd_lkp_valid     = 1;
  flp_lookups_tbl.lem_2nd_lkp_key_select = ARAD_PP_FLP_LKP_KEY_SELECT_FID_FWD_MAC_KEY_HW_VAL; 
  flp_lookups_tbl.lem_2nd_lkp_and_value  = 0x0;
   ...
   )
{
  ...
  res = arad_pp_ihb_flp_lookups_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_ING_LEARN, &flp_lookups_tbl); 
  SOC_SAND_CHECK_FUNC_RESULT(res, 42, exit);
  ...
  res = arad_pp_ihb_flp_lookups_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_ING_LEARN, &flp_lookups_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 42, exit);
exit:
   ...
   )
{
  ...
  res = arad_pp_ihb_flp_lookups_tbl_get_unsafe(unit,ARAD_PP_FLP_KEY_PROGRAM_TRILL_AFTER_TERMINATION , &flp_lookups_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 42, exit);
  ...
  res = arad_pp_ihb_flp_lookups_tbl_set_unsafe(unit,ARAD_PP_FLP_KEY_PROGRAM_TRILL_AFTER_TERMINATION , &flp_lookups_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 42, exit);
exit:
   ...
   )
{
  ...
  res = arad_pp_ihb_flp_lookups_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_MAC_IN_MAC, &flp_lookups_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 42, exit);
  ...
  res = arad_pp_ihb_flp_lookups_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_MAC_IN_MAC, &flp_lookups_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 42, exit);
exit:
   ...
   )
{
  ...
  flp_key_construction_tbl.instruction_0_16b        =  (SOC_IS_JERICHO(unit)? JERICHO_PP_FLP_16B_INST_P6_IN_MAPPED_PORT_D :ARAD_PP_FLP_16B_INST_P6_IN_PORT_D);
   ...
   )
{
  ...
  res = arad_pp_ihb_flp_process_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_TM, &flp_process_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 41, exit);
  tmp = 0;
  SOC_REG_ABOVE_64_CLEAR(reg_val);
  res = READ_IHP_PROTOCOL_TRAPS_PROGRAM_SELECTr(unit, 0, reg_val);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 40, exit);
  SHR_BITCOPY_RANGE(reg_val,3*ARAD_PP_FLP_KEY_PROGRAM_TM,&tmp,0,3);
  res = WRITE_IHP_PROTOCOL_TRAPS_PROGRAM_SELECTr(unit, SOC_CORE_ALL, reg_val);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 45, exit);
   ...
   )
{
  ...
  res = arad_pp_ihb_flp_process_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_TM, &flp_process_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 41, exit);
   ...
   )
{
  ...
  res = arad_pp_ihb_flp_lookups_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_P2P, &flp_lookups_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 41, exit);
  ...
  res = arad_pp_ihb_flp_lookups_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_P2P, &flp_lookups_tbl);
   ...
   )
{
  ...
  res = arad_pp_ihb_flp_process_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_P2P, &flp_process_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 41, exit);
  ...
  flp_process_tbl.not_found_trap_strength      = 0;
  flp_process_tbl.not_found_trap_code = ARAD_PP_TRAP_CODE_INTERNAL_FLP_P2P_MISCONFIGURATION;
  ...
  res = arad_pp_ihb_flp_process_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_P2P, &flp_process_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 41, exit);
  tmp = 0; 
  SOC_REG_ABOVE_64_CLEAR(reg_val);
  res = READ_IHP_PROTOCOL_TRAPS_PROGRAM_SELECTr(unit, 0, reg_val);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 40, exit);
  SHR_BITCOPY_RANGE(reg_val,3*ARAD_PP_FLP_KEY_PROGRAM_P2P,&tmp,0,3);
  res = WRITE_IHP_PROTOCOL_TRAPS_PROGRAM_SELECTr(unit, SOC_CORE_ALL, reg_val);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 45, exit);
   ...
   )
{
  ...
  res = arad_pp_ihb_flp_key_construction_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_P2P, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
  ...
  flp_key_construction_tbl.key_b_inst_0_to_5_valid = 0x0 /*6'b000000*/;
  flp_key_construction_tbl.key_c_inst_0_to_5_valid = 0x0 /*6'b000000*/;
  res = arad_pp_ihb_flp_key_construction_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_P2P, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);

  res = arad_pp_ihb_flp_key_construction_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_P2P+ARAD_PP_FLP_INSTRUCTIONS_NOF, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
  flp_key_construction_tbl.key_a_inst_0_to_5_valid = 0x0 /*6'b000000*/;
  flp_key_construction_tbl.key_b_inst_0_to_5_valid = 0x0 /*6'b000000*/;
  flp_key_construction_tbl.key_c_inst_0_to_5_valid = 0x0 /*6'b000000*/;
  res = arad_pp_ihb_flp_key_construction_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_P2P+ARAD_PP_FLP_INSTRUCTIONS_NOF, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
exit:
   ...
   )
{
  ...
  for (i = 0; i < 2; i++) { /* DHCP, static */
    ...
    flp_prog_sel = (i == 0) ? ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_TK_EPON_UNI_V6_DHCP:ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_TK_EPON_UNI_V6_STATIC;
   ...
   )
{
  ...
  res = arad_pp_ihb_flp_process_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_ING_LEARN, &flp_process_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 41, exit);
...
#endif
  res = arad_pp_ihb_flp_process_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_ING_LEARN, &flp_process_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 41, exit);
  ...
  res = READ_IHP_PROTOCOL_TRAPS_PROGRAM_SELECTr(unit, 0, reg_val);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 40, exit);
  SHR_BITCOPY_RANGE(reg_val,3*ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_ING_LEARN,&tmp,0,3);
  res = WRITE_IHP_PROTOCOL_TRAPS_PROGRAM_SELECTr(unit, SOC_CORE_ALL, reg_val);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 45, exit);
   ...
   )
{
  ...
  res = arad_pp_ihb_flp_process_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_TRILL_AFTER_TERMINATION, &flp_process_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 41, exit);
  ...
  res = arad_pp_ihb_flp_process_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_TRILL_AFTER_TERMINATION, &flp_process_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 41, exit);
  ...
  res = READ_IHP_PROTOCOL_TRAPS_PROGRAM_SELECTr(unit, 0, reg_val);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 40, exit);
  SHR_BITCOPY_RANGE(reg_val,3*ARAD_PP_FLP_KEY_PROGRAM_TRILL_AFTER_TERMINATION,&tmp,0,3);
  res = WRITE_IHP_PROTOCOL_TRAPS_PROGRAM_SELECTr(unit, SOC_CORE_ALL, reg_val);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 45, exit);
   ...
   )
{
  ...
  res = arad_pp_ihb_flp_process_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_MAC_IN_MAC, &flp_process_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 41, exit);
  ...
  res = arad_pp_ihb_flp_process_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_MAC_IN_MAC, &flp_process_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 41, exit);
  ...
  SHR_BITCOPY_RANGE(reg_val, 3*ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_MAC_IN_MAC, &tmp, 0, 3);
...
uint32
   arad_pp_flp_key_const_ethernet_tk_epon_uni_v4(
     int unit
   )
{
  uint32
  ...
  if (!SOC_DPP_CONFIG(unit)->pp.compression_spoof_ip6_enable) {
    /* DHCP */
    res = arad_pp_ihb_flp_key_construction_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_TK_EPON_UNI_V4_DHCP, &flp_key_construction_tbl);
    SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);

    flp_key_construction_tbl.instruction_0_16b        = ARAD_PP_FLP_16B_INST_P6_TT_LOOKUP0_PAYLOAD_D; /* spoof id */
    flp_key_construction_tbl.instruction_1_16b        = ARAD_PP_FLP_16B_INST_P6_IN_LIF_D; /* In-AC */
    ...
    flp_key_construction_tbl.key_b_inst_0_to_5_valid    = 0x0 /*6'b000000*/;
    flp_key_construction_tbl.key_c_inst_0_to_5_valid    = 0x0 /*6'b000000*/;
    res = arad_pp_ihb_flp_key_construction_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_TK_EPON_UNI_V4_DHCP, &flp_key_construction_tbl);
    SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);

    res = arad_pp_ihb_flp_key_construction_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_TK_EPON_UNI_V4_DHCP+ARAD_PP_FLP_INSTRUCTIONS_NOF, &flp_key_construction_tbl);
    SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
    flp_key_construction_tbl.instruction_0_16b        = ARAD_PP_FLP_16B_INST_P6_FID_D;
    ...
    flp_key_construction_tbl.key_c_inst_0_to_5_valid    = 0x0 /*6'b000000*/;
    res = arad_pp_ihb_flp_key_construction_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_TK_EPON_UNI_V4_DHCP+ARAD_PP_FLP_INSTRUCTIONS_NOF, &flp_key_construction_tbl);
    SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
  }
  /* Static */
  res = arad_pp_ihb_flp_key_construction_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_TK_EPON_UNI_V4_STATIC, &flp_key_construction_tbl);
  ...
  flp_key_construction_tbl.key_b_inst_0_to_5_valid    = 0x0 /*6'b000000*/;
  flp_key_construction_tbl.key_c_inst_0_to_5_valid    = 0x0 /*6'b000000*/;
  res = arad_pp_ihb_flp_key_construction_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_TK_EPON_UNI_V4_STATIC, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 144, exit);

  res = arad_pp_ihb_flp_key_construction_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_TK_EPON_UNI_V4_STATIC+ARAD_PP_FLP_INSTRUCTIONS_NOF, &flp_key_construction_tbl);
  ...
  flp_key_construction_tbl.key_b_inst_0_to_5_valid    = 0xB /*6'b001011*/;
  flp_key_construction_tbl.key_c_inst_0_to_5_valid    = 0x0 /*6'b000000*/;
  res = arad_pp_ihb_flp_key_construction_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_TK_EPON_UNI_V4_STATIC+ARAD_PP_FLP_INSTRUCTIONS_NOF, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 146, exit);


exit:
 SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_pp_flp_key_const_ethernet_tk_epon_uni_v4", 0, 0);
}
   ...
   )
{
  ...
  if (!SOC_DPP_CONFIG(unit)->pp.compression_spoof_ip6_enable) {
    /* DHCP */
    res = arad_pp_ihb_flp_key_construction_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_TK_EPON_UNI_V6_DHCP, &flp_key_construction_tbl);
    SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
    ...
    flp_key_construction_tbl.key_b_inst_0_to_5_valid    = 0x0 /*6'b000000*/;
    flp_key_construction_tbl.key_c_inst_0_to_5_valid    = 0x0 /*6'b000000*/;
    res = arad_pp_ihb_flp_key_construction_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_TK_EPON_UNI_V6_DHCP, &flp_key_construction_tbl);
    SOC_SAND_CHECK_FUNC_RESULT(res, 44, exit);

    res = arad_pp_ihb_flp_key_construction_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_TK_EPON_UNI_V6_DHCP+ARAD_PP_FLP_INSTRUCTIONS_NOF, &flp_key_construction_tbl);
    SOC_SAND_CHECK_FUNC_RESULT(res, 45, exit);
    flp_key_construction_tbl.instruction_0_16b        = ARAD_PP_FLP_16B_INST_P6_FID_D;
    flp_key_construction_tbl.instruction_1_16b        = ARAD_PP_CE_SA_FWD_HEADER_16_MSB;
    flp_key_construction_tbl.instruction_3_32b        = ARAD_PP_CE_SA_FWD_HEADER_32_LSB;  
    ...
    flp_key_construction_tbl.key_b_inst_0_to_5_valid    = 0xB /*6'b001011*/;
    flp_key_construction_tbl.key_c_inst_0_to_5_valid    = 0x0 /*6'b000000*/;  
    res = arad_pp_ihb_flp_key_construction_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_TK_EPON_UNI_V6_DHCP+ARAD_PP_FLP_INSTRUCTIONS_NOF, &flp_key_construction_tbl);
    SOC_SAND_CHECK_FUNC_RESULT(res, 46, exit);
  }
  /* static */
  res = arad_pp_ihb_flp_key_construction_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_TK_EPON_UNI_V6_STATIC, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 143, exit);
  if (SOC_DPP_CONFIG(unit)->pp.compression_spoof_ip6_enable) {
    flp_key_construction_tbl.instruction_0_16b        = ARAD_PP_FLP_16B_INST_P6_TT_LOOKUP1_PAYLOAD_D_BITS_9; /* TCAM lookup result */
  ...
  res = arad_pp_ihb_flp_key_construction_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_TK_EPON_UNI_V6_STATIC, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 144, exit);

  res = arad_pp_ihb_flp_key_construction_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_TK_EPON_UNI_V6_STATIC+ARAD_PP_FLP_INSTRUCTIONS_NOF, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 145, exit);
  ...
  res = arad_pp_ihb_flp_key_construction_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_TK_EPON_UNI_V6_STATIC+ARAD_PP_FLP_INSTRUCTIONS_NOF, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 146, exit);
exit:
   ...
   )
{
   ...
   */
  res = arad_pp_ihb_flp_key_construction_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_ING_LEARN, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
  flp_key_construction_tbl.instruction_0_16b        = ARAD_PP_FLP_16B_INST_P6_FID_D;
  ...
  res = arad_pp_ihb_flp_key_construction_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_ING_LEARN, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);

  res = arad_pp_ihb_flp_key_construction_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_ING_LEARN+ARAD_PP_FLP_INSTRUCTIONS_NOF, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
  flp_key_construction_tbl.instruction_3_32b        = ARAD_PP_CE_SA_FWD_HEADER_32_LSB;  
  flp_key_construction_tbl.key_a_inst_0_to_5_valid    = 0x8 /*6'b001000*/;
  flp_key_construction_tbl.key_b_inst_0_to_5_valid    = 0x0 /*6'b000000*/;
  flp_key_construction_tbl.key_c_inst_0_to_5_valid    = 0x0 /*6'b000000*/;    
  res = arad_pp_ihb_flp_key_construction_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_ING_LEARN+ARAD_PP_FLP_INSTRUCTIONS_NOF, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
exit:
...
uint32
   arad_pp_flp_key_const_oam_statistics(
     int unit,
   ...
   )
{
  uint32
    res;
  SOC_DPP_DBAL_KEY_TO_TABLE keys_to_table_id;
  uint8 use_32_bit_ce[SOC_DPP_DBAL_PROGRAM_NOF_KEYS] [SOC_PPC_FP_NOF_QUALS_PER_DB_MAX] = {{0}};
  ...
  res= arad_pp_dbal_program_to_tables_associate(unit, oam_statistics_prog_id, ARAD_FP_DATABASE_STAGE_INGRESS_FLP,
                                                                 &keys_to_table_id, use_32_bit_ce, 1);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
exit:
 SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_pp_flp_key_const_oam_statistics", 0, 0);
}

uint32
   arad_pp_flp_key_const_oam_down_untagged_statistics(
     int unit,
	 int oam_statistics_prog_id
   )
{
...
uint32
   arad_pp_flp_key_const_oam_single_tag_statistics(
     int unit,
	 int oam_statistics_prog_id
   )
{
...
uint32
   arad_pp_flp_key_const_oam_double_tag_statistics(
     int unit,
	 int oam_statistics_prog_id
   )
{
  ...
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
exit:
 SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_pp_flp_key_const_oam_double_tag_statistics", 0, 0);
}



uint32
   arad_pp_flp_key_const_bfd_statistics(
     int unit,
   ...
   )
{
  ...
  res= arad_pp_dbal_program_to_tables_associate(unit, bfd_statistics_prog_id, ARAD_FP_DATABASE_STAGE_INGRESS_FLP,
                                                                 &keys_to_table_id, use_32_bit_ce, 1);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
exit:
 SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_pp_flp_key_const_oam_statistics", 0, 0);
...
uint32
   arad_pp_flp_key_const_bfd_mpls_statistics(
     int unit,
	 int bfd_statistics_prog_id
   )
{
...
uint32
   arad_pp_flp_key_const_bfd_pwe_statistics(
     int unit,
	 int bfd_statistics_prog_id
   )
{
   ...
   )
{
  ...
  if (SOC_DPP_CONFIG(unit)->pp.compression_ip6_enable) {
    flp_key_construction_tbl.instruction_0_16b        = ARAD_PP_FLP_16B_INST_P6_TT_LOOKUP1_PAYLOAD_D_BITS_12; /* TCAM lookup result */
    ...
    flp_key_construction_tbl.key_c_inst_0_to_5_valid    = 0x20/*6'b100000*/;
  } else {
  ...
  if (SOC_DPP_CONFIG(unit)->pp.compression_ip6_enable) {
    ...
    flp_key_construction_tbl.key_c_inst_0_to_5_valid    = 0x38/*6'b111000*/;
  } else {
    flp_key_construction_tbl.key_a_inst_0_to_5_valid    = 0x0 /*6'b000000*/;
    flp_key_construction_tbl.key_b_inst_0_to_5_valid    = 0x0 /*6'b000000*/;
    flp_key_construction_tbl.key_c_inst_0_to_5_valid    = 0x0 /*6'b000000*/;   
   ...
   )
{
  ...
  res = arad_pp_ihb_flp_key_construction_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_TRILL_AFTER_TERMINATION, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
  ...
  flp_key_construction_tbl.key_c_inst_0_to_5_valid    = 0x0  /*6'b000000*/;
  res = arad_pp_ihb_flp_key_construction_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_TRILL_AFTER_TERMINATION, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);

  res = arad_pp_ihb_flp_key_construction_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_TRILL_AFTER_TERMINATION+ARAD_PP_FLP_INSTRUCTIONS_NOF, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
  flp_key_construction_tbl.instruction_0_16b        = ARAD_PP_FLP_16B_INST_P6_FID_D;
  ...
  flp_key_construction_tbl.key_b_inst_0_to_5_valid    = 0x0  /*6'b000000*/;
  flp_key_construction_tbl.key_c_inst_0_to_5_valid    = 0x0  /*6'b000000*/;    
  res = arad_pp_ihb_flp_key_construction_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_TRILL_AFTER_TERMINATION+ARAD_PP_FLP_INSTRUCTIONS_NOF, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
exit:
...
uint32
   arad_pp_flp_key_const_ipv4uc_with_l3vpn(
     int unit
   )
{
  uint32
  ...
  res = arad_pp_ihb_flp_key_construction_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV4UC_WITH_DEFAULT, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
  ...
  flp_key_construction_tbl.key_b_inst_0_to_5_valid    = 0x0 /*6'b000000*/;
  flp_key_construction_tbl.key_c_inst_0_to_5_valid    = 0x0 /*6'b000000*/;
  res = arad_pp_ihb_flp_key_construction_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV4UC_WITH_DEFAULT, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);

  res = arad_pp_ihb_flp_key_construction_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV4UC_WITH_DEFAULT+ARAD_PP_FLP_INSTRUCTIONS_NOF, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
  flp_key_construction_tbl.instruction_3_32b     = ARAD_PP_CE_DIP_FWD_HEADER;
#if defined(INCLUDE_KBP) && !defined(BCM_88030)
  ...
  res = arad_pp_ihb_flp_key_construction_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV4UC_WITH_DEFAULT+ARAD_PP_FLP_INSTRUCTIONS_NOF, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
exit:
   ...
   )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(0);
    ...
    /* key construction */
/*    res = arad_pp_ihb_flp_key_construction_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV6UC_WITH_RPF_2PASS, &flp_key_construction_tbl); */
    res = arad_pp_ihb_flp_key_construction_tbl_get_unsafe(unit, prog_id, &flp_key_construction_tbl);
...
/*    res = arad_pp_ihb_flp_key_construction_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV6UC_WITH_RPF_2PASS, &flp_key_construction_tbl); */
    res = arad_pp_ihb_flp_key_construction_tbl_set_unsafe(unit, prog_id, &flp_key_construction_tbl);
...
uint32
   arad_pp_flp_key_const_ipv4uc_with_rpf(
     int unit
   )
{
  uint32
  ...
  res = arad_pp_ihb_flp_key_construction_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV4UC_WITH_RPF, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
  ...
  res = arad_pp_ihb_flp_key_construction_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV4UC_WITH_RPF, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);

  res = arad_pp_ihb_flp_key_construction_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV4UC_WITH_RPF+ARAD_PP_FLP_INSTRUCTIONS_NOF, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);

  flp_key_construction_tbl.instruction_3_32b     = ARAD_PP_CE_DIP_FWD_HEADER;
  flp_key_construction_tbl.instruction_4_32b     = ARAD_PP_FLP_32B_INST_N_ZEROS(4);
  flp_key_construction_tbl.instruction_5_32b     = ARAD_PP_FLP_32B_INST_P6_IN_RIF_D;
  ...
  res = arad_pp_ihb_flp_key_construction_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV4UC_WITH_RPF+ARAD_PP_FLP_INSTRUCTIONS_NOF, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
exit:
   ...
   )
{
  ...
  res = arad_pp_ihb_flp_key_construction_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV4UC, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
  ...
  flp_key_construction_tbl.key_b_inst_0_to_5_valid    = 0x0 /*6'b000000*/;
  flp_key_construction_tbl.key_c_inst_0_to_5_valid    = 0x0 /*6'b000000*/;
  res = arad_pp_ihb_flp_key_construction_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV4UC, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);

  res = arad_pp_ihb_flp_key_construction_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV4UC+ARAD_PP_FLP_INSTRUCTIONS_NOF, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
  flp_key_construction_tbl.instruction_3_32b     = ARAD_PP_CE_DIP_FWD_HEADER;
#if defined(INCLUDE_KBP) && !defined(BCM_88030)
  if(ARAD_KBP_ENABLE_IPV4_UC || ARAD_KBP_ENABLE_IPV4_MPLS_EXTENDED){
  ...
  res = arad_pp_ihb_flp_key_construction_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV4UC+ARAD_PP_FLP_INSTRUCTIONS_NOF, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
exit:
...
uint32
   arad_pp_flp_dbal_tables_associate_ipv4uc(
     int unit
   )
{
  uint32
  ...
  res = arad_pp_dbal_program_to_tables_associate(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV4UC, ARAD_FP_DATABASE_STAGE_INGRESS_FLP,
                                                                 keys_to_table_id, use_32_bit_ce, 4);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);   

exit:
 SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_pp_flp_dbal_tables_associate_ipv4uc", 0, 0);
}


uint32
   arad_pp_flp_dbal_tables_associate_ipv4compmc_with_rpf(
     int unit
   )
{
  uint32
  ...
  res = arad_pp_dbal_program_to_tables_associate(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV4COMPMC_WITH_RPF, ARAD_FP_DATABASE_STAGE_INGRESS_FLP,
                                                                 keys_to_table_id, use_32_bit_ce, 4);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);   
...
uint32
   arad_pp_flp_dbal_tables_associate_ipv4uc_with_rpf(
     int unit
   )
{
  uint32
  ...
  keys_to_table_id[3].key_id = SOC_DPP_DBAL_PROGRAM_KEY_D;
  keys_to_table_id[3].sw_table_id = SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_KAPS;
  res = arad_pp_dbal_program_to_tables_associate(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV4UC_WITH_RPF, ARAD_FP_DATABASE_STAGE_INGRESS_FLP,
                                                                 keys_to_table_id, use_32_bit_ce, 4);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);   

exit:
 SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_pp_flp_dbal_tables_associate_ipv4uc_with_rpf", 0, 0);
}

uint32
   arad_pp_flp_dbal_tables_associate_ipv4uc_with_l3vpn(
     int unit
   )
{
  uint32
  ...
  keys_to_table_id[0].sw_table_id = SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_KAPS;

  res = arad_pp_dbal_program_to_tables_associate(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV4UC_WITH_DEFAULT, ARAD_FP_DATABASE_STAGE_INGRESS_FLP,
                                                                 keys_to_table_id, use_32_bit_ce, 1);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);   

exit:
 SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_pp_flp_dbal_tables_associate_ipv4uc_with_l3vpn", 0, 0);
}

uint32
   arad_pp_flp_dbal_tables_associate_ipv6uc(
     int unit
   )
{
  uint32
  ...
  res = arad_pp_dbal_program_to_tables_associate(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV6UC, ARAD_FP_DATABASE_STAGE_INGRESS_FLP,
                                                                 keys_to_table_id, use_32_bit_ce, 2);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);   

exit:
 SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_pp_flp_dbal_tables_associate_ipv6uc", 0, 0);
}

uint32
   arad_pp_flp_dbal_tables_associate_ipv6mc(
     int unit
   )
{
  uint32
  ...
  res = arad_pp_dbal_program_to_tables_associate(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV6MC, ARAD_FP_DATABASE_STAGE_INGRESS_FLP,
                                                                 keys_to_table_id, use_32_bit_ce, 2);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);   

exit:
 SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_pp_flp_dbal_tables_associate_ipv6mc", 0, 0);
}
   ...
   )
{
  ...
  flp_key_construction_tbl.instruction_0_16b        = ARAD_PP_FLP_16B_INST_P6_IN_TTL_D;

  flp_key_construction_tbl.instruction_1_16b        = ARAD_PP_FLP_16B_INST_IPR2DSP_6EQ7_COMPENSATED_HEADER_OFFSET_IHP_HDR_OFFSET_BITS_4;
   ...
   )
{
  ...
  res = arad_pp_ihb_flp_key_construction_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV6UC, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
  flp_key_construction_tbl.instruction_0_16b        = ARAD_PP_FLP_16B_INST_P6_VRF_D;
  flp_key_construction_tbl.instruction_1_16b        = ARAD_PP_CE_SIP_IPV6_FWD_HEADER_63_48
  flp_key_construction_tbl.instruction_2_16b        = ARAD_PP_CE_SIP_IPV6_FWD_HEADER_47_32
  ...
  res = arad_pp_ihb_flp_key_construction_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV6UC, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);

  res = arad_pp_ihb_flp_key_construction_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV6UC+ARAD_PP_FLP_INSTRUCTIONS_NOF, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
  flp_key_construction_tbl.instruction_0_16b     = ARAD_PP_FLP_16B_INST_P6_VRF_D;
  flp_key_construction_tbl.instruction_1_16b     = ARAD_PP_CE_DIP_IPV6_FWD_HEADER_127_112
  flp_key_construction_tbl.instruction_2_16b     = ARAD_PP_CE_DIP_IPV6_FWD_HEADER_111_96
...
#endif
  {
      flp_key_construction_tbl.key_a_inst_0_to_5_valid = 0x0 /*6'b000000*/;
      flp_key_construction_tbl.key_b_inst_0_to_5_valid = 0x0 /*6'b000000*/;
      flp_key_construction_tbl.key_c_inst_0_to_5_valid = SOC_IS_JERICHO(unit)? 0x3C03 /*MSB: 6'b111100, LSB: 6'b000011*/: 0x3F /*hex 6'b111111*/;
  }
  res = arad_pp_ihb_flp_key_construction_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV6UC+ARAD_PP_FLP_INSTRUCTIONS_NOF, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
exit:
   ...
   )
{
  ...
  res = arad_pp_ihb_flp_key_construction_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV6MC, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
#if defined(INCLUDE_KBP) && !defined(BCM_88030)
  if(ARAD_KBP_ENABLE_IPV6_MC){
      /* Key A */
      flp_key_construction_tbl.instruction_0_16b = ARAD_PP_FLP_16B_INST_P6_VRF_D | 0xf000; 
      flp_key_construction_tbl.instruction_1_16b = ARAD_PP_FLP_16B_INST_P6_IN_RIF_D | 0xf000;
      flp_key_construction_tbl.instruction_3_32b = ARAD_PP_CE_SIP_IPV6_FWD_HEADER_127_96;
...
#else /* ARAD_KBP_IPV6_INSERT_SIP_IN_MASTER_KEY_FOR_VRF */
      flp_key_construction_tbl.key_a_inst_0_to_5_valid = 0x3;  /* No SIP */
      flp_key_construction_tbl.key_b_inst_0_to_5_valid = 0x0; 
      flp_key_construction_tbl.key_c_inst_0_to_5_valid = 0x0; 
#endif /* ARAD_KBP_IPV6_INSERT_SIP_IN_MASTER_KEY_FOR_VRF */
  }
...
#endif
  {
      flp_key_construction_tbl.instruction_0_16b        = ARAD_PP_FLP_16B_INST_P6_IN_RIF_D;
  ...
  res = arad_pp_ihb_flp_key_construction_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV6MC, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
  
  res = arad_pp_ihb_flp_key_construction_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV6MC+ARAD_PP_FLP_INSTRUCTIONS_NOF, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
#if defined(INCLUDE_KBP) && !defined(BCM_88030)
  if(ARAD_KBP_ENABLE_IPV6_MC){
      /* Key B */
...
#else /* ARAD_KBP_IPV6_INSERT_SIP_IN_MASTER_KEY_FOR_VRF */
      flp_key_construction_tbl.key_b_inst_0_to_5_valid = 0x0;  /* No SIP */
#endif /* ARAD_KBP_IPV6_INSERT_SIP_IN_MASTER_KEY_FOR_VRF */
      flp_key_construction_tbl.key_c_inst_0_to_5_valid = 0x3e; /* hex 6'b111110 */
  }
  else
  ...
  res = arad_pp_ihb_flp_key_construction_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV6MC+ARAD_PP_FLP_INSTRUCTIONS_NOF, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
exit:
   ...
   )
{
  ...
  res = arad_pp_sw_db_flp_prog_app_to_index_get(unit, ARAD_PP_FLP_KEY_PROGRAM_VPLSOGRE, &prog);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);

  res = arad_pp_ihb_flp_key_construction_tbl_get_unsafe(unit, prog, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);

  flp_key_construction_tbl.instruction_0_16b     = (in_rif)  ? ARAD_PP_FLP_16B_INST_P6_IN_RIF_D:ARAD_PP_FLP_16B_INST_12_ZEROS;
  flp_key_construction_tbl.instruction_1_16b     = (in_port) ? 
                                                     (SOC_IS_JERICHO(unit)? JERICHO_PP_FLP_16B_INST_P6_IN_MAPPED_PORT_D :ARAD_PP_FLP_16B_INST_P6_IN_PORT_D):
                                                      ARAD_PP_FLP_16B_INST_8_ZEROS;
  flp_key_construction_tbl.instruction_2_16b     =             ARAD_PP_FLP_16B_INST_IPR2DSP_6EQ7_MPLS_EXP;
   ...
   )
{
  ...
  res = arad_pp_ihb_flp_key_construction_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_LSR, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
  ...
  res = arad_pp_ihb_flp_key_construction_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_LSR, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);

  res = arad_pp_ihb_flp_key_construction_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_LSR+ARAD_PP_FLP_INSTRUCTIONS_NOF, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
  ...
  res = arad_pp_ihb_flp_key_construction_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_LSR+ARAD_PP_FLP_INSTRUCTIONS_NOF, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
  ...
  /* have to put it here due to that arad_pp_flp_key_const_lsr is called twice, 
     ...
     */
  res = arad_pp_flp_lpm_custom_lookup_enable(unit, ARAD_PP_FLP_KEY_PROGRAM_LSR);
  SOC_SAND_CHECK_FUNC_RESULT(res, 50, exit);
exit:
...
uint32
   arad_pp_flp_key_const_coupling_lsr(
     int unit
   )
{
  uint32
  ...
  ARAD_PP_IHB_FLP_KEY_CONSTRUCTION_TBL_DATA
    flp_key_construction_tbl;
  SOC_SAND_INIT_ERROR_DEFINITIONS(0);
  res = arad_pp_ihb_flp_key_construction_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_COUPLING_LSR, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
  flp_key_construction_tbl.instruction_0_16b     = ARAD_PP_CE_MPLS_FWD_HEADER_15_0;
  flp_key_construction_tbl.instruction_1_16b     = ARAD_PP_CE_MPLS_FWD_HEADER_19_16;
  flp_key_construction_tbl.instruction_2_16b     = ARAD_PP_CE_MPLS_AFTER_FWD_HEADER_15_0;
  
  flp_key_construction_tbl.key_a_inst_0_to_5_valid = 0xf /*hex 6'b001111*/;
  flp_key_construction_tbl.key_b_inst_0_to_5_valid = 0x0 /*6'b000000*/;
  flp_key_construction_tbl.key_c_inst_0_to_5_valid = 0x0 /*6'b000000*/;
  
  res = arad_pp_ihb_flp_key_construction_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_COUPLING_LSR, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
  res = arad_pp_ihb_flp_key_construction_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_COUPLING_LSR+ARAD_PP_FLP_INSTRUCTIONS_NOF, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
  flp_key_construction_tbl.key_a_inst_0_to_5_valid = 0x0 /*6'b000000*/;
  flp_key_construction_tbl.key_b_inst_0_to_5_valid = 0x0 /*6'b000000*/;
  flp_key_construction_tbl.key_c_inst_0_to_5_valid = 0x0 /*6'b000000*/;
  res = arad_pp_ihb_flp_key_construction_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_COUPLING_LSR+ARAD_PP_FLP_INSTRUCTIONS_NOF, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
exit:
 SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_pp_flp_key_const_coupling_lsr", 0, 0);
}
   ...
   )
{
    ...
    if (is_vsan_from_vsi == 0) {
        ...
        vsi_vft_instuction = ARAD_PP_FLP_16B_INST_P6_VSI_D_12_BITS;
    }
    ...
    if (is_vsan_from_vsi == 0) {
      ...
      flp_key_construction_tbl.instruction_1_16b = ARAD_PP_FLP_16B_INST_P6_VSI_D_12_BITS;}
   ...
   )
{
  ...
  if (is_vsan_from_vsi == 0) {
      ...
      flp_key_construction_tbl.instruction_0_16b     = ARAD_PP_FLP_16B_INST_P6_VSI_D_12_BITS;}
  flp_key_construction_tbl.instruction_1_16b     = ARAD_PP_FLP_16B_INST_N_ZEROS(1);
   ...
   )
{
  ...
  res = arad_pp_ihb_flp_key_construction_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_TRILL_UC, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
  flp_key_construction_tbl.instruction_0_16b = ARAD_PP_CE_TRILL_EGRESS_NICK;
  flp_key_construction_tbl.instruction_1_16b =  (SOC_IS_JERICHO(unit)? JERICHO_PP_FLP_16B_INST_P6_IN_MAPPED_PORT_D :ARAD_PP_FLP_16B_INST_P6_IN_PORT_D);
  ...
  flp_key_construction_tbl.key_c_inst_0_to_5_valid    = 0x0 /*6'b000000*/;
  res = arad_pp_ihb_flp_key_construction_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_TRILL_UC, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);

  res = arad_pp_ihb_flp_key_construction_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_TRILL_UC+ARAD_PP_FLP_INSTRUCTIONS_NOF, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
  flp_key_construction_tbl.key_a_inst_0_to_5_valid = 0x0 /*6'b000000*/;
  flp_key_construction_tbl.key_b_inst_0_to_5_valid = 0x0 /*6'b000000*/;
  flp_key_construction_tbl.key_c_inst_0_to_5_valid = 0x0 /*6'b000000*/;
  res = arad_pp_ihb_flp_key_construction_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_TRILL_UC+ARAD_PP_FLP_INSTRUCTIONS_NOF, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
exit:
   ...
   )
{
  ...
  for (is_two_tags = 0; is_two_tags <= 1; ++is_two_tags) {
      prog = is_two_tags ?
          ARAD_PP_FLP_KEY_PROGRAM_TRILL_MC_TWO_TAGS :
          ARAD_PP_FLP_KEY_PROGRAM_TRILL_MC_ONE_TAG;

      res = arad_pp_ihb_flp_key_construction_tbl_get_unsafe(unit, prog, &flp_key_construction_tbl);
      SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
#if defined(INCLUDE_KBP) && !defined(BCM_88030)
      if(ARAD_KBP_ENABLE_TRILL_MC && 
         (prog == ARAD_PP_FLP_KEY_PROGRAM_TRILL_MC_ONE_TAG))
      {
          flp_key_construction_tbl.instruction_0_16b = ARAD_PP_FLP_16B_INST_IPR2DSP_6EQ7_ESADI;
          flp_key_construction_tbl.instruction_1_16b = SOC_DPP_CONFIG(unit)->trill.mc_prune_mode ? ARAD_PP_FLP_16B_INST_P6_FID_D : ARAD_PP_FLP_16B_INST_15_ZEROS;
      ...
      if(ARAD_KBP_ENABLE_TRILL_MC && 
         (prog == ARAD_PP_FLP_KEY_PROGRAM_TRILL_MC_ONE_TAG))
      {
          flp_key_construction_tbl.key_a_inst_0_to_5_valid  = 0x0; /*6'b000111*/
          flp_key_construction_tbl.key_b_inst_0_to_5_valid  = 0x0; /*6'b000000*/
          flp_key_construction_tbl.key_c_inst_0_to_5_valid  = 0x0; /*6'b000000*/
      }
      else
#endif
      {
          if (SOC_IS_DPP_TRILL_FGL(unit) && SOC_DPP_CONFIG(unit)->trill.mc_prune_mode) {
              ...
              flp_key_construction_tbl.instruction_2_16b = ARAD_PP_FLP_16B_INST_IPR2DSP_6EQ7_ESADI;
              ...
              flp_key_construction_tbl.key_b_inst_0_to_5_valid = 0x0 /*6'b000000*/;
              flp_key_construction_tbl.key_c_inst_0_to_5_valid = 0xe /*6'b001110*/;
          } else {
            ...
            /* VL key: <FID or 0x0,ESADI,DIST-TREE> */
              flp_key_construction_tbl.instruction_0_16b =
                SOC_DPP_CONFIG(unit)->trill.mc_prune_mode ?
                ARAD_PP_FLP_16B_INST_P6_FID_D :
                ARAD_PP_FLP_16B_INST_15_ZEROS;
              flp_key_construction_tbl.instruction_1_16b = ARAD_PP_FLP_16B_INST_IPR2DSP_6EQ7_ESADI;
              flp_key_construction_tbl.instruction_2_16b = ARAD_PP_CE_TRILL_DIST_TREE_NICK_16;
              flp_key_construction_tbl.key_a_inst_0_to_5_valid = 0x7 /*6'b000111*/;
              flp_key_construction_tbl.key_b_inst_0_to_5_valid = 0x0 /*6'b000000*/;
              flp_key_construction_tbl.key_c_inst_0_to_5_valid = 0x0 /*6'b000000*/;
...
uint32
   arad_pp_flp_lookups_ipv4uc_with_l3vpn(
     int unit
   )
{
  uint32
  ...
  res = arad_pp_ihb_flp_lookups_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV4UC_WITH_DEFAULT, &flp_lookups_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 42, exit);
  ...
  if(ARAD_KBP_ENABLE_IPV4_UC || ARAD_KBP_ENABLE_IPV4_MPLS_EXTENDED){
    ...
    flp_lookups_tbl.lpm_2nd_lkp_valid        = 1;
    flp_lookups_tbl.lpm_2nd_lkp_key_select   = 3; /* key D */
    flp_lookups_tbl.lpm_2nd_lkp_and_value    = 7; /* 00111  */;
    flp_lookups_tbl.lpm_2nd_lkp_or_value     = JER_KAPS_IPV4_UC_TABLE_PREFIX << 3; /* only 2 bit prefix */
    ...
    flp_lookups_tbl.lpm_public_2nd_lkp_key_size     = 36;
    flp_lookups_tbl.lpm_public_2nd_lkp_and_value    = 7; /* 00111  */;
    flp_lookups_tbl.lpm_public_2nd_lkp_or_value     = JER_KAPS_IPV4_UC_TABLE_PREFIX << 3; /* only 2 bit prefix */
  }
#endif

  res = arad_pp_ihb_flp_lookups_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV4UC_WITH_DEFAULT, &flp_lookups_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 42, exit);
exit:
...
/*   res = arad_pp_ihb_flp_lookups_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV6UC_WITH_RPF_2PASS, &flp_lookups_tbl); */
  res = arad_pp_ihb_flp_lookups_tbl_get_unsafe(unit, prog_id, &flp_lookups_tbl);
...
/*  res = arad_pp_ihb_flp_lookups_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV6UC_WITH_RPF_2PASS, &flp_lookups_tbl); */
  res = arad_pp_ihb_flp_lookups_tbl_set_unsafe(unit, prog_id, &flp_lookups_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 243, exit);
...
uint32
   arad_pp_flp_lookups_ipv4uc_with_rpf(
     int unit
   )
{
  uint32
  ...
  res = arad_pp_ihb_flp_lookups_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV4UC_WITH_RPF, &flp_lookups_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 42, exit);
  ...
  if(ARAD_KBP_ENABLE_IPV4_UC || ARAD_KBP_ENABLE_IPV4_MPLS_EXTENDED){
      ...
      flp_lookups_tbl.lpm_1st_lkp_valid         = 1;
      flp_lookups_tbl.lpm_1st_lkp_key_select    = 2;  /* key C   */
      flp_lookups_tbl.lpm_1st_lkp_and_value     = 7;  /* 5`b00111 */
      flp_lookups_tbl.lpm_1st_lkp_or_value = JER_KAPS_IPV4_UC_TABLE_PREFIX << 3; /* only 2 bit prefix */
      
      flp_lookups_tbl.lpm_public_1st_lkp_valid        = 1;
      flp_lookups_tbl.lpm_public_1st_lkp_key_select   = 2; /* key C   */
      flp_lookups_tbl.lpm_public_1st_lkp_and_value    = 7; /* 00111  */;
      flp_lookups_tbl.lpm_public_1st_lkp_or_value     = JER_KAPS_IPV4_UC_TABLE_PREFIX << 3; /* only 2 bit prefix */         

      flp_lookups_tbl.lpm_2nd_lkp_valid         = 1;
      flp_lookups_tbl.lpm_2nd_lkp_key_select    = 3;  /* key D   */
      flp_lookups_tbl.lpm_2nd_lkp_and_value     = 7;  /* 5`b00111 */
      flp_lookups_tbl.lpm_2nd_lkp_or_value      = JER_KAPS_IPV4_UC_TABLE_PREFIX << 3; /* only 2 bit prefix */
      
      flp_lookups_tbl.lpm_public_2nd_lkp_valid        = 1;
      flp_lookups_tbl.lpm_public_2nd_lkp_key_select   = 3; /* key D   */
      flp_lookups_tbl.lpm_public_2nd_lkp_and_value    = 7; /* 00111  */;
      flp_lookups_tbl.lpm_public_2nd_lkp_or_value     = JER_KAPS_IPV4_UC_TABLE_PREFIX << 3; /* only 2 bit prefix */         
  }
#endif

  res = arad_pp_ihb_flp_lookups_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV4UC_WITH_RPF, &flp_lookups_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 42, exit);
exit:
   ...
   )
{
  ...
  res = arad_pp_ihb_flp_lookups_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV4UC, &flp_lookups_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 42, exit);
  ...
  res = arad_pp_ihb_flp_lookups_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV4UC, &flp_lookups_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 42, exit);
exit:
   ...
   )
{
  ...
  SOC_SAND_CHECK_FUNC_RESULT(res, 42, exit);
exit:
 SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_pp_flp_lookups_ipv4uc_tcam_profile_set", 0, 0);
}
   ...
   )
{
  ...
  SOC_SAND_OUT ARAD_PP_TRAP_CODE_INTERNAL   trap_code_internal;
    ...
    */
/*    res = arad_pp_ihb_flp_process_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV6UC_WITH_RPF_2PASS, &flp_process_tbl); */
    res = arad_pp_ihb_flp_process_tbl_get_unsafe(unit, prog_id, &flp_process_tbl);
    SOC_SAND_CHECK_FUNC_RESULT(res, 241, exit);
...
/*    res = arad_pp_ihb_flp_process_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV6UC_WITH_RPF_2PASS, &flp_process_tbl); */
    res = arad_pp_ihb_flp_process_tbl_set_unsafe(unit, prog_id, &flp_process_tbl);
...
/*    SHR_BITCOPY_RANGE(reg_val,3*ARAD_PP_FLP_KEY_PROGRAM_IPV6UC_WITH_RPF_2PASS,&tmp,0,3); */
    SHR_BITCOPY_RANGE(reg_val,3*prog_id,&tmp,0,3);
    res = WRITE_IHP_PROTOCOL_TRAPS_PROGRAM_SELECTr(unit, SOC_CORE_ALL, reg_val);
...
uint32
   arad_pp_flp_process_ipv4uc_with_rpf(
     int unit
   )
{
  uint32
  ...
  res = arad_pp_ihb_flp_process_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV4UC_WITH_RPF, &flp_process_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 41, exit);
  ...
  res = arad_pp_ihb_flp_process_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV4UC_WITH_RPF, &flp_process_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 41, exit);
  tmp = 1; /* 0: Apply Ethernet traps on forwarding-header 1: Apply IPv4 traps on forwarding-header and L4 Traps on the following header 2: Apply IPv6 traps on forwarding-header and L4 Traps on the following header 3: Apply MPLS traps on forwarding-header 4: Apply FC traps on forwarding-header Else: Don't apply any protocol traps */
  SOC_REG_ABOVE_64_CLEAR(reg_val);
  res = READ_IHP_PROTOCOL_TRAPS_PROGRAM_SELECTr(unit, 0, reg_val);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 40, exit);
  SHR_BITCOPY_RANGE(reg_val,3*ARAD_PP_FLP_KEY_PROGRAM_IPV4UC_WITH_RPF,&tmp,0,3);
  res = WRITE_IHP_PROTOCOL_TRAPS_PROGRAM_SELECTr(unit, SOC_CORE_ALL, reg_val);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 45, exit);
   ...
   )
{
  ...
  res = arad_pp_ihb_flp_lookups_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV6UC, &flp_lookups_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 42, exit);
  ...
  if (SOC_IS_JERICHO(unit)) {
      ...
      flp_lookups_tbl.lpm_1st_lkp_key_select    = 2;    /* key C    */
      flp_lookups_tbl.lpm_1st_lkp_and_value     = 7;    /* 5`b00111  */
      flp_lookups_tbl.lpm_1st_lkp_or_value = JER_KAPS_IPV6_UC_TABLE_PREFIX << 3; /* only 2 bit prefix */

      /* public RPF */
      flp_lookups_tbl.lpm_public_1st_lkp_valid         = 1;
      flp_lookups_tbl.lpm_public_1st_lkp_key_select    = 4; /* : Lpm_1stLkpKey[0 +: 4*LpmPublic_1ndLkpKeySize] */
      flp_lookups_tbl.lpm_public_1st_lkp_key_size      = 36;
      flp_lookups_tbl.lpm_public_1st_lkp_and_value     = 7;    /* 5`b00111  */
      flp_lookups_tbl.lpm_public_1st_lkp_or_value = JER_KAPS_IPV6_UC_TABLE_PREFIX << 3; /* only 2 bit prefix */

      /* private forwarding */
      flp_lookups_tbl.lpm_2nd_lkp_valid         = 1;
      flp_lookups_tbl.lpm_2nd_lkp_key_select    = 3;    /* key D    */
      flp_lookups_tbl.lpm_2nd_lkp_and_value     = 7;    /* 5`b00111  */
      flp_lookups_tbl.lpm_2nd_lkp_or_value = JER_KAPS_IPV6_UC_TABLE_PREFIX << 3; /* only 2 bit prefix */

      /* public forwarding */
      flp_lookups_tbl.lpm_public_2nd_lkp_valid         = 1;
      flp_lookups_tbl.lpm_public_2nd_lkp_key_select    = 4; /* : Lpm_2stLkpKey[0 +: 4*LpmPublic_2ndLkpKeySize] */
      flp_lookups_tbl.lpm_public_2nd_lkp_key_size      = 36;
      flp_lookups_tbl.lpm_public_2nd_lkp_and_value     = 7;    /* 5`b00111  */
      flp_lookups_tbl.lpm_public_2nd_lkp_or_value = JER_KAPS_IPV6_UC_TABLE_PREFIX << 3; /* only 2 bit prefix */
  }
  else
  ...
  res = arad_pp_ihb_flp_lookups_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV6UC, &flp_lookups_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 42, exit);
exit:
   ...
   )
{
  ...
  res = arad_pp_ihb_flp_lookups_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV6MC, &flp_lookups_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 42, exit);
  ...
  if (SOC_IS_JERICHO(unit)) {
      ...
      flp_lookups_tbl.lpm_2nd_lkp_key_select    = 3;    /* key D    */
      flp_lookups_tbl.lpm_2nd_lkp_and_value     = 7;    /* 5`b00111  */
      flp_lookups_tbl.lpm_2nd_lkp_or_value = JER_KAPS_IPV6_MC_TABLE_PREFIX << 3; /* only 2 bit prefix */
      ...
      flp_lookups_tbl.lpm_public_2nd_lkp_key_size      = 36;
      flp_lookups_tbl.lpm_public_2nd_lkp_and_value     = 7;    /* 5`b00111  */
      flp_lookups_tbl.lpm_public_2nd_lkp_or_value = JER_KAPS_IPV6_MC_TABLE_PREFIX << 3; /* only 2 bit prefix */
  }
  else 
#endif /* defined(BCM_88675_A0) && defined(INCLUDE_KBP) && !defined(BCM_88030) */
  {
      ...
      if(ARAD_KBP_ENABLE_IPV6_MC){
          ...
          flp_lookups_tbl.elk_key_c_valid_bytes = 19; /* take 21 bytes from key C */
    #else /* ARAD_KBP_IPV6_INSERT_SIP_IN_MASTER_KEY_FOR_VRF */
          flp_lookups_tbl.elk_key_a_valid_bytes = 4;  
          flp_lookups_tbl.elk_key_b_valid_bytes = 0;  
          flp_lookups_tbl.elk_key_c_valid_bytes = 15; 
    #endif /* ARAD_KBP_IPV6_INSERT_SIP_IN_MASTER_KEY_FOR_VRF */
      }
    #endif
  }

  res = arad_pp_ihb_flp_lookups_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV6MC, &flp_lookups_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 42, exit);
exit:
   ...
   )
{
  ...
  res = arad_pp_ihb_flp_lookups_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_LSR, &flp_lookups_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 42, exit);
  ...
  if(ARAD_KBP_ENABLE_MPLS || ARAD_KBP_ENABLE_IPV4_MPLS_EXTENDED){
      ...
      if(ARAD_KBP_ENABLE_IPV4_MPLS_EXTENDED) {
          ...
          flp_lookups_tbl.elk_opcode = ARAD_KBP_FRWRD_TABLE_OPCODE_LSR;
          flp_lookups_tbl.elk_key_a_valid_bytes = 6; 
      }
  ...
  res = arad_pp_ihb_flp_lookups_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_LSR, &flp_lookups_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 42, exit);
exit:
 SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_pp_flp_lookups_lsr", 0, 0);
}
uint32
   arad_pp_flp_lookups_coupling_lsr(
     int unit
   )
{
  uint32
  ...
  ARAD_PP_IHB_FLP_LOOKUPS_TBL_DATA
    flp_lookups_tbl;
  SOC_SAND_INIT_ERROR_DEFINITIONS(0);
  res = arad_pp_ihb_flp_lookups_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_COUPLING_LSR, &flp_lookups_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 42, exit);
  flp_lookups_tbl.lem_2nd_lkp_valid     = 1;
  flp_lookups_tbl.lem_2nd_lkp_key_select = 0;
  flp_lookups_tbl.lem_2nd_lkp_and_value  = 0x0;
  flp_lookups_tbl.lem_2nd_lkp_or_value   = ARAD_PP_FLP_COUPLING_LSR_KEY_OR_MASK;
  flp_lookups_tbl.learn_key_select      = ARAD_PP_FLP_LKP_KEY_SELECT_FID_FWD_MAC_KEY_HW_VAL;
  res = arad_pp_ihb_flp_lookups_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_COUPLING_LSR, &flp_lookups_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 42, exit);
exit:
 SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_pp_flp_lookups_coupling_lsr", 0, 0);
}
   ...
   )
{
  ...
  res = arad_pp_ihb_flp_lookups_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_TRILL_UC, &flp_lookups_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 42, exit);
  ...
  res = arad_pp_ihb_flp_lookups_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_TRILL_UC, &flp_lookups_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 42, exit);
exit:
...
uint32
   arad_pp_flp_process_ipv4uc_with_l3vpn(
     int unit
   )
{
  uint32
  ...
  res = arad_pp_ihb_flp_process_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV4UC_WITH_DEFAULT, &flp_process_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 41, exit);
  ...
  res = arad_pp_ihb_flp_process_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV4UC_WITH_DEFAULT, &flp_process_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 41, exit);
  tmp = 1; /* 0: Apply Ethernet traps on forwarding-header 1: Apply IPv4 traps on forwarding-header and L4 Traps on the following header 2: Apply IPv6 traps on forwarding-header and L4 Traps on the following header 3: Apply MPLS traps on forwarding-header 4: Apply FC traps on forwarding-header Else: Don't apply any protocol traps */
  SOC_REG_ABOVE_64_CLEAR(reg_val);
  res = READ_IHP_PROTOCOL_TRAPS_PROGRAM_SELECTr(unit, 0, reg_val);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 40, exit);
  SHR_BITCOPY_RANGE(reg_val,3*ARAD_PP_FLP_KEY_PROGRAM_IPV4UC_WITH_DEFAULT,&tmp,0,3);
  res = WRITE_IHP_PROTOCOL_TRAPS_PROGRAM_SELECTr(unit, SOC_CORE_ALL, reg_val);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 45, exit);
exit:
   ...
   )
{
  ...
  res = arad_pp_ihb_flp_process_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV4UC, &flp_process_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 41, exit);
  ...
  res = arad_pp_ihb_flp_process_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV4UC, &flp_process_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 41, exit);
  tmp = 1; /* 0: Apply Ethernet traps on forwarding-header 1: Apply IPv4 traps on forwarding-header and L4 Traps on the following header 2: Apply IPv6 traps on forwarding-header and L4 Traps on the following header 3: Apply MPLS traps on forwarding-header 4: Apply FC traps on forwarding-header Else: Don't apply any protocol traps */
  SOC_REG_ABOVE_64_CLEAR(reg_val);
  res = READ_IHP_PROTOCOL_TRAPS_PROGRAM_SELECTr(unit, 0, reg_val);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 40, exit);
  SHR_BITCOPY_RANGE(reg_val,3*ARAD_PP_FLP_KEY_PROGRAM_IPV4UC,&tmp,0,3);
  res = WRITE_IHP_PROTOCOL_TRAPS_PROGRAM_SELECTr(unit, SOC_CORE_ALL, reg_val);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 45, exit);
exit:
   ...
   )
{
  ...
  /*ARAD_PP_FLP_INIT_PRINT_ADVANCE("ARAD_PP_FLP_KEY_PROGRAM_TRILL_UC");*/
  
  res = arad_pp_ihb_flp_process_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_TRILL_UC, &flp_process_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 41, exit);
  ...
  flp_process_tbl.not_found_trap_code          = ARAD_PP_TRAP_CODE_INTERNAL_FLP_TRILL_UNKNOWN_UC;
  flp_process_tbl.result_a_format            = 0;
  flp_process_tbl.result_b_format            = 0;
  ...
  res = arad_pp_ihb_flp_process_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_TRILL_UC, &flp_process_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 41, exit);
  ...
  res = READ_IHP_PROTOCOL_TRAPS_PROGRAM_SELECTr(unit, 0, reg_val);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 40, exit);
  SHR_BITCOPY_RANGE(reg_val,3*ARAD_PP_FLP_KEY_PROGRAM_TRILL_UC,&tmp,0,3);
  res = WRITE_IHP_PROTOCOL_TRAPS_PROGRAM_SELECTr(unit, SOC_CORE_ALL, reg_val);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 45, exit);
exit:
   ...
   )
{
  ...
  for (is_two_tags = 0; is_two_tags <= 1; ++is_two_tags) {
      ...
      flp_process_tbl.include_tcam_in_result_a       = 1;
      flp_process_tbl.apply_fwd_result_a           = 1; /*  apply alwayes, even if not found, i.e. do Trap if not found */
      flp_process_tbl.not_found_trap_code          = ARAD_PP_TRAP_CODE_INTERNAL_FLP_TRILL_UNKNOWN_MC;
      flp_process_tbl.result_a_format            = 0;
      flp_process_tbl.result_b_format            = 0;
      flp_process_tbl.procedure_ethernet_default  = 2; /*  if found do procedure_ethernet_default -i.e. BC */
      flp_process_tbl.enable_hair_pin_filter       = 1;
   ...
   )
{
  ...
  res = arad_pp_ihb_flp_process_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV6UC, &flp_process_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 41, exit);
  ...
  res = arad_pp_ihb_flp_process_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV6UC, &flp_process_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 41, exit);
   ...
   )
{
  ...
  res = arad_pp_ihb_flp_process_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV6MC, &flp_process_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 41, exit);
  ...
  /* take VRF default destination */
  flp_process_tbl.not_found_trap_strength       = 0;
  flp_process_tbl.not_found_trap_code = ARAD_PP_TRAP_CODE_INTERNAL_FLP_DEFAULT_MCV6;
  /* 0x0 - Use VRF Default Unicast
     0x1 - Use VRF Default Multicast
     Else: Use NotFoundTrapCode from program */
  ...
  res = arad_pp_ihb_flp_process_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV6MC, &flp_process_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 41, exit);
  ...
  res = READ_IHP_PROTOCOL_TRAPS_PROGRAM_SELECTr(unit, 0, reg_val);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 40, exit);
  SHR_BITCOPY_RANGE(reg_val,3*ARAD_PP_FLP_KEY_PROGRAM_IPV6MC,&tmp,0,3);
  res = WRITE_IHP_PROTOCOL_TRAPS_PROGRAM_SELECTr(unit, SOC_CORE_ALL, reg_val);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 45, exit);
exit:
   ...
   )
{
  ...
  res = arad_pp_ihb_flp_process_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_LSR, &flp_process_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 41, exit);
  ...
  flp_process_tbl.not_found_trap_code = ARAD_PP_TRAP_CODE_INTERNAL_FLP_MPLS_UNKNOWN_LABEL;
  ...
  res = arad_pp_ihb_flp_process_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_LSR, &flp_process_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 41, exit);
  tmp = 3; /* 0: Apply Ethernet traps on forwarding-header 1: Apply IPv4 traps on forwarding-header and L4 Traps on the following header 2: Apply IPv6 traps on forwarding-header and L4 Traps on the following header 3: Apply MPLS traps on forwarding-header 4: Apply FC traps on forwarding-header Else: Don't apply any protocol traps */
  SOC_REG_ABOVE_64_CLEAR(reg_val);
  res = READ_IHP_PROTOCOL_TRAPS_PROGRAM_SELECTr(unit, 0, reg_val);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 40, exit);
  SHR_BITCOPY_RANGE(reg_val,3*ARAD_PP_FLP_KEY_PROGRAM_LSR,&tmp,0,3);
  res = WRITE_IHP_PROTOCOL_TRAPS_PROGRAM_SELECTr(unit, SOC_CORE_ALL, reg_val);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 45, exit);
exit:
...
uint32
   arad_pp_flp_process_coupling_lsr(
     int unit
   )
{
  uint32
  ...
  ARAD_PP_IHB_FLP_PROCESS_TBL_DATA
    flp_process_tbl;
  SOC_SAND_INIT_ERROR_DEFINITIONS(0);
  res = arad_pp_ihb_flp_process_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_COUPLING_LSR, &flp_process_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 41, exit);
  flp_process_tbl.include_lem_2nd_in_result_a    = 1;
  flp_process_tbl.include_lem_1st_in_result_b    = 1;
  flp_process_tbl.include_lpm_2nd_in_result_a    = 1;
  ...
  flp_process_tbl.enable_lsr_p2p_service         = 0;
  flp_process_tbl.not_found_trap_code = ARAD_PP_TRAP_CODE_INTERNAL_FLP_MPLS_UNKNOWN_LABEL;
  res = arad_pp_ihb_flp_process_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_COUPLING_LSR, &flp_process_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 41, exit);
  tmp = 3; /* 0: Apply Ethernet traps on forwarding-header 1: Apply IPv4 traps on forwarding-header and L4 Traps on the following header 2: Apply IPv6 traps on forwarding-header and L4 Traps on the following header 3: Apply MPLS traps on forwarding-header 4: Apply FC traps on forwarding-header Else: Don't apply any protocol traps */
  SOC_REG_ABOVE_64_CLEAR(reg_val);
  res = READ_IHP_PROTOCOL_TRAPS_PROGRAM_SELECTr(unit, 0, reg_val);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 40, exit);
  SHR_BITCOPY_RANGE(reg_val,3*ARAD_PP_FLP_KEY_PROGRAM_COUPLING_LSR,&tmp,0,3);
  res = WRITE_IHP_PROTOCOL_TRAPS_PROGRAM_SELECTr(unit, SOC_CORE_ALL, reg_val);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 45, exit);
exit:
 SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_pp_flp_process_coupling_lsr", 0, 0);
}
   ...
   )
{
 ...
 /* forwarding not found */
  flp_process_tbl.not_found_trap_code = ARAD_PP_TRAP_CODE_INTERNAL_FLP_FCF;
  flp_process_tbl.not_found_trap_strength = 7;
  flp_process_tbl.not_found_snoop_strength = 0;
   ...
   )
{
  ...
  res = arad_pp_ihb_flp_program_selection_cam_tbl_get_unsafe(unit, ARAD_PP_FLP_PROG_SEL_CAM_IPV4MC_RPF, &prog_selection_cam_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);
  ...
  prog_selection_cam_tbl.program                        = ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_ING_LEARN;
  prog_selection_cam_tbl.valid                          = 1;
  res = arad_pp_ihb_flp_program_selection_cam_tbl_set_unsafe(unit, ARAD_PP_FLP_PROG_SEL_CAM_IPV4MC_RPF, &prog_selection_cam_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);
exit:
   ...
   )
{
  ...
  res = arad_pp_ihb_flp_key_construction_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV4MC_WITH_RPF, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
  flp_key_construction_tbl.instruction_0_16b = ARAD_PP_FLP_16B_INST_P6_FID_D;
  ...
  flp_key_construction_tbl.key_c_inst_0_to_5_valid = 0x0 /*hex 6'b101001*/; 
  res = arad_pp_ihb_flp_key_construction_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV4MC_WITH_RPF, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
  
  res = arad_pp_ihb_flp_key_construction_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV4MC_WITH_RPF+ARAD_PP_FLP_INSTRUCTIONS_NOF, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
  ...
  flp_key_construction_tbl.key_b_inst_0_to_5_valid = 0x0 /*6'b000000*/;
  flp_key_construction_tbl.key_c_inst_0_to_5_valid = 0x0 /*6'b000000*/;
  res = arad_pp_ihb_flp_key_construction_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV4MC_WITH_RPF+ARAD_PP_FLP_INSTRUCTIONS_NOF, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
exit:
   ...
   )
{
  ...
  res = arad_pp_ihb_flp_lookups_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV4MC_WITH_RPF, &flp_lookups_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 42, exit);
  ...
  flp_lookups_tbl.learn_key_select      = 0;
  res = arad_pp_ihb_flp_lookups_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV4MC_WITH_RPF, &flp_lookups_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 42, exit);
exit:
   ...
   )
{
  ...
  res = arad_pp_ihb_flp_process_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV4MC_WITH_RPF, &flp_process_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 41, exit);
  ...
  flp_process_tbl.not_found_trap_strength      = 0;
  flp_process_tbl.unknown_address              = 3;
  res = arad_pp_ihb_flp_process_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV4MC_WITH_RPF, &flp_process_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 41, exit);
  ...
  res = READ_IHP_PROTOCOL_TRAPS_PROGRAM_SELECTr(unit, 0, reg_val);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 40, exit);
  SHR_BITCOPY_RANGE(reg_val,3*ARAD_PP_FLP_KEY_PROGRAM_IPV4MC_WITH_RPF,&tmp,0,3);
  res = WRITE_IHP_PROTOCOL_TRAPS_PROGRAM_SELECTr(unit, SOC_CORE_ALL, reg_val);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 45, exit);
exit:
   ...
   )
{
  ...
  /* For static, program id and app id is same */
  ARAD_PP_FLP_SW_DB_FLP_PROGRAM_ID_SET(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV4COMPMC_WITH_RPF, ARAD_PP_FLP_KEY_PROGRAM_IPV4COMPMC_WITH_RPF);

  res = arad_pp_ihb_flp_program_selection_cam_tbl_get_unsafe(unit, ARAD_PP_FLP_PROG_SEL_CAM_IPV4COMPMC_RPF, &prog_selection_cam_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);
  ...
  prog_selection_cam_tbl.program                   = ARAD_PP_FLP_KEY_PROGRAM_IPV4COMPMC_WITH_RPF;
  prog_selection_cam_tbl.valid                     = 1;
  res = arad_pp_ihb_flp_program_selection_cam_tbl_set_unsafe(unit, ARAD_PP_FLP_PROG_SEL_CAM_IPV4COMPMC_RPF, &prog_selection_cam_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);
exit:
   ...
   )
{
  ...
  res = arad_pp_ihb_flp_key_construction_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV4COMPMC_WITH_RPF, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
  ...
  if(ARAD_KBP_ENABLE_IPV4_MC){
      ...
      flp_key_construction_tbl.instruction_2_16b = ARAD_PP_FLP_16B_INST_P6_IN_RIF_D;
  ...
  {
      flp_key_construction_tbl.instruction_0_16b = ARAD_PP_FLP_16B_INST_P6_IN_RIF_D;
      flp_key_construction_tbl.instruction_1_16b = ARAD_PP_FLP_16B_INST_P6_VRF_D;
      ...
      flp_key_construction_tbl.key_c_inst_0_to_5_valid = 0x29 /*hex 6'b101001*/; /*  RIF,SIP,DIP[27:0] */
  }


  res = arad_pp_ihb_flp_key_construction_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV4COMPMC_WITH_RPF, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
  
  res = arad_pp_ihb_flp_key_construction_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV4COMPMC_WITH_RPF+ARAD_PP_FLP_INSTRUCTIONS_NOF, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
  ...
  flp_key_construction_tbl.key_b_inst_0_to_5_valid = 0x0 /*6'b000000*/;
  flp_key_construction_tbl.key_c_inst_0_to_5_valid = 0x0 /*6'b000000*/;
  res = arad_pp_ihb_flp_key_construction_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV4COMPMC_WITH_RPF+ARAD_PP_FLP_INSTRUCTIONS_NOF, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
exit:
   ...
   )
{
  ...
  res = arad_pp_ihb_flp_lookups_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV4COMPMC_WITH_RPF, &flp_lookups_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 42, exit);

#if defined(BCM_88675_A0) && defined(INCLUDE_KBP) && !defined(BCM_88030)
  if (SOC_IS_JERICHO(unit)) {
      /* LEM1(RPF): {VRF,SIP} - key-A */
      flp_lookups_tbl.lem_1st_lkp_valid     = 1;
      flp_lookups_tbl.lem_1st_lkp_key_select = 0;
      flp_lookups_tbl.lem_1st_lkp_and_value  = 0x0;
      flp_lookups_tbl.lem_1st_lkp_or_value   = ARAD_PP_FLP_IPV4_KEY_OR_MASK;
      ...
      flp_lookups_tbl.lpm_2nd_lkp_key_select    = 3;    /* key D    */
      flp_lookups_tbl.lpm_2nd_lkp_and_value     = 7;    /* 5`b00111  */
      flp_lookups_tbl.lpm_2nd_lkp_or_value = JER_KAPS_IPV4_MC_TABLE_PREFIX << 3; /* only 2 bit prefix */
      ...
      flp_lookups_tbl.lpm_public_2nd_lkp_key_size      = 36;
      flp_lookups_tbl.lpm_public_2nd_lkp_and_value     = 7;    /* 5`b00111  */
      flp_lookups_tbl.lpm_public_2nd_lkp_or_value = JER_KAPS_IPV4_MC_TABLE_PREFIX << 3; /* only 2 bit prefix */
  }
  else 
#endif /* defined(BCM_88675_A0) && defined(INCLUDE_KBP) && !defined(BCM_88030) */
  {
  ...
  res = arad_pp_ihb_flp_lookups_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV4COMPMC_WITH_RPF, &flp_lookups_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 42, exit);
exit:
   ...
   )
{
  ...
  res = arad_pp_ihb_flp_process_tbl_get_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV4COMPMC_WITH_RPF, &flp_process_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 41, exit);
  ...
  res = arad_pp_ihb_flp_process_tbl_set_unsafe(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV4COMPMC_WITH_RPF, &flp_process_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 41, exit);
  tmp = 1;
  SOC_REG_ABOVE_64_CLEAR(reg_val);
  res = READ_IHP_PROTOCOL_TRAPS_PROGRAM_SELECTr(unit, 0, reg_val);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 40, exit);
  SHR_BITCOPY_RANGE(reg_val,3*ARAD_PP_FLP_KEY_PROGRAM_IPV4COMPMC_WITH_RPF,&tmp,0,3);
  res = WRITE_IHP_PROTOCOL_TRAPS_PROGRAM_SELECTr(unit, SOC_CORE_ALL, reg_val);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 45, exit);
exit:
   ...
   )
{
  ...
  res = arad_pp_flp_get_next_avaialable(unit, ARAD_PP_FLP_KEY_PROGRAM_GLOBAL_IPV4COMPMC_WITH_RPF, &cam_sel, prog_id);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
   ...
   )
{
  ...
  /* LEM2: {RIF,SIP,DIP} - key-C */
  flp_lookups_tbl.lem_2nd_lkp_valid     = 1;
  flp_lookups_tbl.lem_2nd_lkp_key_select = 2;
   ...
   )
{
  ...
  for(addr = 0; addr < SOC_DPP_DEFS_GET(unit, nof_flp_programs); ++addr)
  {
    ...
    if (SOC_IS_ARADPLUS(unit)) { 
        ...
        fld_val = 0x9; /* bits 119:112 is lookup-result-0 */
        ...
        fld_val = 0x7; /* bits 103:56 is lookup-result-1 */
        soc_mem_field_set(unit, IHP_ELK_PAYLOAD_FORMATm, &mem_val, EXT_DATA_OFFSETf, &fld_val);
        res = WRITE_IHP_ELK_PAYLOAD_FORMATm(unit, MEM_BLOCK_ANY, ARAD_PP_FLP_KEY_PROGRAM_IPV4COMPMC_WITH_RPF, &mem_val);
        SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 101 + addr, exit);
        ...
        if(ARAD_KBP_ENABLE_IPV6_EXTENDED) {
            ...
            res = WRITE_IHP_ELK_PAYLOAD_FORMATm(unit, MEM_BLOCK_ANY, ARAD_PP_FLP_KEY_PROGRAM_IPV6UC, &mem_val);
            SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 101 + addr, exit);
		}
		...
		{                
			...
			res = WRITE_IHP_ELK_PAYLOAD_FORMATm(unit, MEM_BLOCK_ANY, ARAD_PP_FLP_KEY_PROGRAM_LSR, &mem_val);
			SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 101 + addr, exit);

			res = WRITE_IHP_ELK_PAYLOAD_FORMATm(unit, MEM_BLOCK_ANY, ARAD_PP_FLP_KEY_PROGRAM_IPV4UC_WITH_RPF, &mem_val);
			SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 101 + addr, exit);

			res = WRITE_IHP_ELK_PAYLOAD_FORMATm(unit, MEM_BLOCK_ANY, ARAD_PP_FLP_KEY_PROGRAM_IPV4UC, &mem_val);
			SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 101 + addr, exit);
		}
   ...
   )
{
  ...
  /*ARAD_PP_FLP_INIT_PRINT_ADVANCE("arad_pp_flp_lookups_initial_programs ingress_learn_oppurtunistic: %h",ingress_learn_oppurtunistic);*/
  arad_pp_flp_prog_sel_cam_key_program_tm(unit);
  arad_pp_flp_process_key_program_tm(unit);


  arad_pp_flp_process_tm(unit);

  /* Exit here in case of TM - Enable the Management unit since the MACT is not enabled */
#if defined(INCLUDE_KBP) && !defined(BCM_88030)
  if (ARAD_KBP_ENABLE_P2P_EXTENDED) {
    arad_pp_flp_prog_sel_cam_p2p(unit);
    arad_pp_flp_key_const_p2p(unit);
    arad_pp_flp_lookups_p2p(unit);
    arad_pp_flp_process_p2p(unit);
  }
#endif 

  if (SOC_DPP_L3_SRC_BIND_IPV4_ENABLE(unit)) {
    /* Spoof v4 */
    arad_pp_flp_prog_sel_cam_ethernet_tk_epon_uni_v4(unit);
    arad_pp_flp_key_const_ethernet_tk_epon_uni_v4(unit);
    arad_pp_flp_lookups_ethernet_tk_epon_uni_v4(unit, sa_auth_enabled, slb_enabled);
    arad_pp_flp_process_ethernet_tk_epon_uni_v4(unit, TRUE,sa_auth_enabled, slb_enabled);
   }
  if (SOC_DPP_L3_SRC_BIND_IPV6_ENABLE(unit)) {
    /* Spoof v6 */
    arad_pp_flp_prog_sel_cam_ethernet_tk_epon_uni_v6(unit);
    arad_pp_flp_key_const_ethernet_tk_epon_uni_v6(unit);
    arad_pp_flp_lookups_ethernet_tk_epon_uni_v6(unit, ARAD_TCAM_ACCESS_PROFILE_INVALID, sa_auth_enabled, slb_enabled);
    arad_pp_flp_process_ethernet_tk_epon_uni_v6(unit, TRUE,sa_auth_enabled, slb_enabled);
  }

  if (mac_in_mac_enabled) {
  
      arad_pp_flp_prog_sel_cam_ethernet_mac_in_mac(unit);
      /* key construction is not needed because lookup key selection is configured to use {FID,DA} as key */
      arad_pp_flp_lookups_ethernet_mac_in_mac(unit);
      arad_pp_flp_process_ethernet_mac_in_mac(unit); 
  }   
  ...
  if(ARAD_PP_L3_ROUTING_ENABLED(unit))
  {
      ...
      /* ----------- IPv4 UC + L3VPN -------------*/
      /* -----------------------------------------*/
      arad_pp_flp_prog_sel_cam_ipv4uc_with_l3vpn(unit);
#if defined(BCM_88675_A0) && defined(INCLUDE_KBP) && !defined(BCM_88030)
      if(JER_KAPS_ENABLE(unit)) {
          arad_pp_flp_dbal_tables_associate_ipv4uc_with_l3vpn(unit);
      }
      else 
#endif
      {
          arad_pp_flp_key_const_ipv4uc_with_l3vpn(unit);
      }
      arad_pp_flp_lookups_ipv4uc_with_l3vpn(unit);
      arad_pp_flp_process_ipv4uc_with_l3vpn(unit);


      /* ---------------------------------------*/
      /* ----------- IPv4 UC + RPF -------------*/
      /* ---------------------------------------*/
      arad_pp_flp_prog_sel_cam_ipv4uc_with_rpf(unit);
#if defined(BCM_88675_A0) && defined(INCLUDE_KBP) && !defined(BCM_88030)
      if(JER_KAPS_ENABLE(unit)) {
          arad_pp_flp_dbal_tables_associate_ipv4uc_with_rpf(unit);
      }
      else 
#endif
      {
          arad_pp_flp_key_const_ipv4uc_with_rpf(unit);
      }
      arad_pp_flp_lookups_ipv4uc_with_rpf(unit);
      arad_pp_flp_process_ipv4uc_with_rpf(unit);

      /* ---------------------------------*/
      /* ----------- IPv4 UC -------------*/
      /* ---------------------------------*/
      arad_pp_flp_prog_sel_cam_ipv4uc(unit);
#if defined(BCM_88675_A0) && defined(INCLUDE_KBP) && !defined(BCM_88030)
      if(JER_KAPS_ENABLE(unit)) {
          arad_pp_flp_dbal_tables_associate_ipv4uc(unit);
      }
      else 
#endif 
      {
          arad_pp_flp_key_const_ipv4uc(unit);
      }
      arad_pp_flp_lookups_ipv4uc(unit);
      arad_pp_flp_process_ipv4uc(unit);

      /* ----------------------------------------*/
      /* ----------- IPv4 MC bridge -------------*/
      /* ----------------------------------------*/
      arad_pp_flp_prog_sel_cam_ipv4mc_bridge(unit);
      arad_pp_flp_key_const_ipv4mc_bridge(unit);
      arad_pp_flp_lookups_ipv4mc_bridge(unit,sa_auth_enabled, slb_enabled);
      arad_pp_flp_process_ipv4mc_bridge(unit,TRUE,sa_auth_enabled, slb_enabled);
      ...
      arad_pp_flp_prog_sel_cam_ipv4compmc_with_rpf(unit);
#if defined(BCM_88675_A0) && defined(INCLUDE_KBP) && !defined(BCM_88030)
      if(JER_KAPS_ENABLE(unit)) {
          arad_pp_flp_dbal_tables_associate_ipv4compmc_with_rpf(unit);
      }
      else 
#endif 
      {
          arad_pp_flp_key_const_ipv4compmc_with_rpf(unit);
      }
      arad_pp_flp_lookups_ipv4compmc_with_rpf(unit, ARAD_TCAM_ACCESS_PROFILE_INVALID);
      arad_pp_flp_process_ipv4compmc_with_rpf(unit);
      }

      /* ---------------------------------*/
      /* ----------- IPv6 UC -------------*/
      /* ---------------------------------*/
      arad_pp_flp_prog_sel_cam_ipv6uc(unit);
#if defined(BCM_88675_A0) && defined(INCLUDE_KBP) && !defined(BCM_88030)
      if(JER_KAPS_ENABLE(unit)) {
          arad_pp_flp_dbal_tables_associate_ipv6uc(unit);
      }
      else 
#endif 
      {
          arad_pp_flp_key_const_ipv6uc(unit);
      }
      arad_pp_flp_lookups_ipv6uc(unit,ARAD_TCAM_ACCESS_PROFILE_INVALID);
      arad_pp_flp_process_ipv6uc(unit);

      /* ---------------------------------*/
      /* ----------- IPv6 MC -------------*/
      /* ---------------------------------*/
      arad_pp_flp_prog_sel_cam_ipv6mc(unit);
...
#if defined(BCM_88675_A0) && defined(INCLUDE_KBP) && !defined(BCM_88030)
          if(JER_KAPS_ENABLE(unit)) {
              arad_pp_flp_dbal_tables_associate_ipv6mc(unit);
          }
          else 
#endif 
          {
              arad_pp_flp_key_const_ipv6mc(unit);
          }
          arad_pp_flp_lookups_ipv6mc(unit,ARAD_TCAM_ACCESS_PROFILE_INVALID);
          arad_pp_flp_process_ipv6mc(unit);
      }                  

      arad_pp_flp_prog_sel_cam_lsr(unit);
      arad_pp_flp_key_const_lsr(unit, FALSE, FALSE, FALSE);
      arad_pp_flp_lookups_lsr(unit);
      arad_pp_flp_process_lsr(unit);
      arad_pp_flp_prog_sel_cam_coupling_lsr(unit);
      arad_pp_flp_key_const_coupling_lsr(unit);
      arad_pp_flp_lookups_coupling_lsr(unit);
      arad_pp_flp_process_coupling_lsr(unit);
      
      if (SOC_DPP_CONFIG(unit)->trill.mode) {        
        arad_pp_flp_prog_sel_cam_TRILL_uc(unit);
        arad_pp_flp_key_const_TRILL_uc(unit);
        arad_pp_flp_lookups_TRILL_uc(unit);
        arad_pp_flp_process_TRILL_uc(unit);

        arad_pp_flp_prog_sel_cam_TRILL_mc(unit);
        arad_pp_flp_key_const_TRILL_mc(unit,ingress_learn_enable);
        arad_pp_flp_lookups_TRILL_mc(unit,ingress_learn_enable,ARAD_TCAM_ACCESS_PROFILE_INVALID);
        arad_pp_flp_process_TRILL_mc(unit,ingress_learn_enable);

        if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
          arad_pp_flp_prog_sel_cam_TRILL_mc_after_recycle_overlay(unit);
          arad_pp_flp_key_const_TRILL_mc_after_recycle_overlay(unit);
          arad_pp_flp_lookups_TRILL_mc_after_recycle_overlay(unit, sa_auth_enabled, slb_enabled);
          arad_pp_flp_process_TRILL_mc_after_recycle_overlay(unit,TRUE,sa_auth_enabled, slb_enabled);
        }
      }
      ...
      if(ARAD_KBP_ENABLE_IPV6_EXTENDED){
          ...
          {
              int32 prog_id;
              arad_pp_flp_prog_sel_cam_ipv6uc_with_rpf_2pass(unit,&prog_id);
              arad_pp_flp_key_const_ipv6uc_with_rpf_2pass(unit,prog_id);
              arad_pp_flp_lookups_ipv6uc_with_rpf_2pass(unit,prog_id);
              arad_pp_flp_process_ipv6uc_with_rpf_2pass(unit,prog_id);
              arad_pp_flp_prog_sel_cam_ipv6uc_with_rpf_2pass_fwd(unit);
          }
      }
  ...
  if (!(SOC_DPP_CONFIG(unit)->l3.ipmc_vpn_lookup_enable)) {
      ...
      arad_pp_flp_prog_sel_cam_global_ipv4compmc_with_rpf(unit, &program_id);
      arad_pp_flp_key_const_global_ipv4compmc_with_rpf(unit, program_id);
      arad_pp_flp_lookups_global_ipv4compmc_with_rpf(unit, program_id, ARAD_TCAM_ACCESS_PROFILE_INVALID);
      arad_pp_flp_process_global_ipv4compmc_with_rpf(unit, program_id);
  }
  ...
  if (SOC_DPP_CONFIG(unit)->pp.pon_application_enable) {
      ...
      if (SOC_DPP_L3_SRC_BIND_ARP_RELAY_ENABLE(unit)) {
        arad_pp_flp_pon_arp_prog_init(unit, sa_auth_enabled, slb_enabled);
      }

      if (SOC_DPP_CONFIG(unit)->pp.local_switching_enable) {
          /* ARAD FLP for local route upstream: DA+SA+COS */
          arad_pp_flp_prog_sel_cam_ethernet_pon_local_route(unit, mac_in_mac_enabled, &prog_id);
          arad_pp_flp_key_const_ethernet_pon_local_route(unit, prog_id);
          arad_pp_flp_lookups_ethernet_pon_local_route(unit, sa_auth_enabled, slb_enabled, prog_id);
          arad_pp_flp_process_ethernet_pon_local_route(unit, TRUE,sa_auth_enabled, slb_enabled, prog_id);
      } 

      /* ARAD FLP for non local route upstream: SA+PTC */
      arad_pp_flp_prog_sel_cam_ethernet_pon_default_upstream(unit, &prog_id);
      arad_pp_flp_key_const_ethernet_pon_default_upstream(unit, prog_id);  
      arad_pp_flp_lookups_ethernet_pon_default_upstream(unit, sa_auth_enabled, slb_enabled, prog_id);
      arad_pp_flp_process_ethernet_pon_default_upstream(unit, TRUE,sa_auth_enabled, slb_enabled, prog_id);
      /* ARAD FLP for non local route downstream: DA */
      arad_pp_flp_prog_sel_cam_ethernet_pon_default_downstream(unit, &prog_id);
      arad_pp_flp_key_const_ethernet_pon_default_downstream(unit, prog_id);  
      arad_pp_flp_lookups_ethernet_pon_default_downstream(unit, ARAD_TCAM_ACCESS_PROFILE_INVALID, sa_auth_enabled, slb_enabled, prog_id);
      arad_pp_flp_process_ethernet_pon_default_downstream(unit, TRUE,sa_auth_enabled, slb_enabled, prog_id);
  } else {
      int32 mac_in_mac_prog_id;
      arad_pp_flp_prog_sel_cam_ethernet_ing_learn(unit, mac_in_mac_enabled, &mac_in_mac_prog_id);
      arad_pp_flp_key_const_ethernet_ing_learn(unit);  
      arad_pp_flp_lookups_ethernet_ing_learn(unit, sa_auth_enabled, slb_enabled);
      arad_pp_flp_process_ethernet_ing_learn(unit, TRUE,sa_auth_enabled, slb_enabled);

      if (mac_in_mac_enabled) {
          /* MAC termination */      
          arad_pp_flp_key_const_mac_in_mac_after_termination(unit, mac_in_mac_prog_id);      
          arad_pp_flp_lookups_mac_in_mac_after_termination(unit, mac_in_mac_prog_id);
          arad_pp_flp_process_mac_in_mac_after_termination(unit,TRUE, mac_in_mac_prog_id);      
      }
  }
  ...
  res = arad_pp_flp_elk_all_progs_instruction_set(unit);
  SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);
#endif
  ...
  if (SOC_DPP_CONFIG(unit)->pp.bfd_ipv4_single_hop == 1) {
	  ...
	  arad_pp_flp_prog_sel_cam_bfd_single_hop(unit, 0, &bfd_ipv4_single_hop_prog_id); 
	  arad_pp_flp_key_const_bfd_single_hop(unit, bfd_ipv4_single_hop_prog_id);
      arad_pp_flp_lookups_bfd_single_hop(unit, bfd_ipv4_single_hop_prog_id);
      arad_pp_flp_process_bfd_single_hop(unit, 0, bfd_ipv4_single_hop_prog_id);
  }

  if (SOC_DPP_CONFIG(unit)->pp.bfd_ipv6_enable) {
      int bfd_ipv6_single_hop_prog_id; 
	  arad_pp_flp_prog_sel_cam_bfd_single_hop(unit, 1, &bfd_ipv6_single_hop_prog_id); 
	  arad_pp_flp_key_const_bfd_single_hop(unit, bfd_ipv6_single_hop_prog_id);
      arad_pp_flp_lookups_bfd_single_hop(unit, bfd_ipv6_single_hop_prog_id);
      arad_pp_flp_process_bfd_single_hop(unit, 1, bfd_ipv6_single_hop_prog_id);
  }
   ...
   if (SOC_DPP_CONFIG(unit)->pp.oam_statistics == 1) {
	  ...
	  /* OAM up mep*/
	  arad_pp_flp_prog_sel_cam_oam_statistics(unit, &oam_statistics_prog_id); 
	  arad_pp_flp_key_const_oam_statistics(unit, oam_statistics_prog_id);
      arad_pp_flp_lookups_oam_statistics(unit, oam_statistics_prog_id);
      arad_pp_flp_process_oam_statistics(unit, oam_statistics_prog_id);

      /* BFD o IPV4*/
	  arad_pp_flp_prog_sel_cam_bfd_statistics(unit, &bfd_statistics_prog_id); 
	  arad_pp_flp_key_const_bfd_statistics(unit, bfd_statistics_prog_id);
      arad_pp_flp_lookups_bfd_statistics(unit, bfd_statistics_prog_id);
      arad_pp_flp_process_bfd_statistics(unit, bfd_statistics_prog_id); 

	  /* OAM down mep untagged*/
	  arad_pp_flp_prog_sel_cam_oam_down_untagged_statistics(unit, &oam_down_statistics_prog_id); 
	  arad_pp_flp_key_const_oam_down_untagged_statistics(unit, oam_down_statistics_prog_id);
      arad_pp_flp_lookups_oam_down_untagged_statistics(unit, oam_down_statistics_prog_id);
      arad_pp_flp_process_oam_statistics(unit, oam_down_statistics_prog_id);

	  /* OAM down mep single tag*/
	  arad_pp_flp_prog_sel_cam_oam_single_tag_statistics(unit, &oam_single_tag_statistics_prog_id); 
	  arad_pp_flp_key_const_oam_single_tag_statistics(unit, oam_single_tag_statistics_prog_id);
      arad_pp_flp_lookups_oam_single_tag_statistics(unit, oam_single_tag_statistics_prog_id);
      arad_pp_flp_process_oam_statistics(unit, oam_single_tag_statistics_prog_id);

	  /* OAM down mep double tag*/
	  arad_pp_flp_prog_sel_cam_oam_double_tag_statistics(unit, &oam_double_tag_statistics_prog_id); 
	  arad_pp_flp_key_const_oam_double_tag_statistics(unit, oam_double_tag_statistics_prog_id);
      arad_pp_flp_lookups_oam_double_tag_statistics(unit, oam_double_tag_statistics_prog_id);
      arad_pp_flp_process_oam_statistics(unit, oam_double_tag_statistics_prog_id);

      /* BFD o MPLS*/
	  arad_pp_flp_prog_sel_cam_bfd_mpls_statistics(unit, &bfd_mpls_statistics_prog_id); 
	  arad_pp_flp_key_const_bfd_mpls_statistics(unit, bfd_mpls_statistics_prog_id);
      arad_pp_flp_lookups_bfd_statistics(unit, bfd_mpls_statistics_prog_id);
      arad_pp_flp_process_oam_statistics(unit, bfd_mpls_statistics_prog_id);
      
      /* BFD o PWE*/
	  arad_pp_flp_prog_sel_cam_bfd_pwe_statistics(unit, &bfd_pwe_statistics_prog_id); 
	  arad_pp_flp_key_const_bfd_pwe_statistics(unit, bfd_pwe_statistics_prog_id);
      arad_pp_flp_lookups_bfd_statistics(unit, bfd_pwe_statistics_prog_id);
      arad_pp_flp_process_oam_statistics(unit, bfd_pwe_statistics_prog_id); 
  }
   ...
   )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(0);
      ...
      /* SourceStep      MMWriteLine             MMWriteValid           TableOffset      MMReadLine */
    data.step = 0x7;  data.write_line = 0;  data.write_valid = 1;  data.offset = 7;  data.read_line  = 0x7; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 0, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 0, exit);
    data.step = 0 ;  data.write_line = 1;   data.write_valid = 1;  data.offset = 1;  data.read_line  = 0; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 1, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 1, exit);
    data.step = 0 ;  data.write_line = 2;   data.write_valid = 1;  data.offset = 2;  data.read_line  = 0; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 2, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 2, exit);
    data.step = 0 ;  data.write_line = 3;   data.write_valid = 1;  data.offset = 3;  data.read_line  = 0; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 3, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 3, exit);
    data.step = 0 ;  data.write_line = 4;   data.write_valid = 1;  data.offset = 4;  data.read_line  = 0; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 4, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 4, exit);
    data.step = 0 ;  data.write_line = 5;   data.write_valid = 1;  data.offset = 5;  data.read_line  = 0; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 5, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 5, exit);
    data.step = 0 ;  data.write_line = 6;   data.write_valid = 1;  data.offset = 6;  data.read_line  = 0; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 6, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 6, exit);
    data.step = 0 ;  data.write_line = 7;   data.write_valid = 1;  data.offset = 7;  data.read_line  = 0; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 7, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 7, exit);
    data.step = 1 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 1;  data.read_line  = 1; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 8, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 8, exit);
    data.step = 1 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 2;  data.read_line  = 1; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 9, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 9, exit);
    data.step = 1 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 3;  data.read_line  = 1; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 10, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
    data.step = 1 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 4;  data.read_line  = 1; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 11, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 11, exit);
    data.step = 1 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 5;  data.read_line  = 1; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 12, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 12, exit);
    data.step = 1 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 6;  data.read_line  = 1; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 13, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 13, exit);
    data.step = 1 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 7;  data.read_line  = 1; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 14, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 14, exit);
    data.step = 2 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 1;  data.read_line  = 2; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 15, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 15, exit);
    data.step = 2 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 2;  data.read_line  = 2; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 16, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 16, exit);
    data.step = 2 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 3;  data.read_line  = 2; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 17, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 17, exit);
    data.step = 2 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 4;  data.read_line  = 2; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 18, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 18, exit);
    data.step = 2 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 5;  data.read_line  = 2; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 19, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 19, exit);
    data.step = 2 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 6;  data.read_line  = 2; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 20, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
    data.step = 2 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 7;  data.read_line  = 2; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 21, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 21, exit);
    data.step = 3 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 1;  data.read_line  = 3; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 22, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 22, exit);
    data.step = 3 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 2;  data.read_line  = 3; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 23, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 23, exit);
    data.step = 3 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 3;  data.read_line  = 3; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 24, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 24, exit);
    data.step = 3 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 4;  data.read_line  = 3; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 25, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 25, exit);
    data.step = 3 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 5;  data.read_line  = 3; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 26, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 26, exit);
    data.step = 3 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 6;  data.read_line  = 3; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 27, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 27, exit);
    data.step = 3 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 7;  data.read_line  = 3; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 28, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 28, exit);
    data.step = 4 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 1;  data.read_line  = 4; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 29, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 29, exit);
    data.step = 4 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 2;  data.read_line  = 4; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 30, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
    data.step = 4 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 3;  data.read_line  = 4; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 31, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 31, exit);
    data.step = 4 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 4;  data.read_line  = 4; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 32, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 32, exit);
    data.step = 4 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 5;  data.read_line  = 4; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 33, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 33, exit);
    data.step = 4 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 6;  data.read_line  = 4; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 34, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 34, exit);
    data.step = 4 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 7;  data.read_line  = 4; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 35, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 35, exit);
    data.step = 5 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 1;  data.read_line  = 5; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 36, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 36, exit);
    data.step = 5 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 2;  data.read_line  = 5; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 37, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 37, exit);
    data.step = 5 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 3;  data.read_line  = 5; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 38, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 38, exit);
    data.step = 5 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 4;  data.read_line  = 5; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 39, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 39, exit);
    data.step = 5 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 5;  data.read_line  = 5; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 40, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);
    data.step = 5 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 6;  data.read_line  = 5; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 41, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 41, exit);
    data.step = 5 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 7;  data.read_line  = 5; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 42, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 42, exit);
    data.step = 6 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 1;  data.read_line  = 6; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 43, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
    data.step = 6 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 2;  data.read_line  = 6; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 44, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 44, exit);
    data.step = 6 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 3;  data.read_line  = 6; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 45, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 45, exit);
    data.step = 6 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 4;  data.read_line  = 6; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 46, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 46, exit);
    data.step = 6 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 5;  data.read_line  = 6; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 47, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 47, exit);
    data.step = 6 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 6;  data.read_line  = 6; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 48, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 48, exit);
    data.step = 6 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 7;  data.read_line  = 6; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 49, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 49, exit);
    data.step = 7 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 1;  data.read_line  = 7; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 50, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 50, exit);
    data.step = 7 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 2;  data.read_line  = 7; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 51, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 51, exit);
    data.step = 7 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 3;  data.read_line  = 7; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 52, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 52, exit);
    data.step = 7 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 4;  data.read_line  = 7; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 53, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 53, exit);
    data.step = 7 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 5;  data.read_line  = 7; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 54, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 54, exit);
    data.step = 7 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 6;  data.read_line  = 7; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 55, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 55, exit);
    data.step = 7 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 7;  data.read_line  = 7; 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 56, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 56, exit);
    data.step = 8 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 1;  data.read_line  = 0;/*8; */
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 57, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 57, exit);
    data.step = 8 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 2;  data.read_line  = 0;/*8; */ 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 58, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 58, exit);
    data.step = 8 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 3;  data.read_line  = 0;/*8; */ 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 59, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 59, exit);
    data.step = 8 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 4;  data.read_line  = 0;/*8; */ 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 60, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 60, exit);
    data.step = 8 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 5;  data.read_line  = 0;/*8; */ 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 61, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 61, exit);
    data.step = 8 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 6;  data.read_line  = 0;/*8; */ 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 62, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 62, exit);
    data.step = 8 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 7;  data.read_line  = 0;/*8; */ 
    res = arad_tbl_access_em_step_set(unit, tbl_indx, 63, &data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 63, exit);

    if (SOC_IS_JERICHO(unit)) {
        data.step = 0x7;  data.write_line = 0;  data.write_valid = 1;  data.offset = 5;  data.read_line  = 0x7; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 0, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 0, exit);
        data.step = 0 ;  data.write_line = 1;   data.write_valid = 1;  data.offset = 1;  data.read_line  = 0; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 1, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 1, exit);
        data.step = 0 ;  data.write_line = 2;   data.write_valid = 1;  data.offset = 2;  data.read_line  = 0; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 2, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 2, exit);
        data.step = 0 ;  data.write_line = 3;   data.write_valid = 1;  data.offset = 3;  data.read_line  = 0; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 3, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 3, exit);
        data.step = 0 ;  data.write_line = 4;   data.write_valid = 1;  data.offset = 4;  data.read_line  = 0; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 4, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 4, exit);
        data.step = 0 ;  data.write_line = 5;   data.write_valid = 1;  data.offset = 5;  data.read_line  = 0; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 5, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 5, exit);
        data.step = 1 ;  data.write_line = 6;   data.write_valid = 1;  data.offset = 1;  data.read_line  = 1; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 6, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 6, exit);
        data.step = 1 ;  data.write_line = 7;   data.write_valid = 1;  data.offset = 2;  data.read_line  = 1; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 7, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 7, exit);
        data.step = 1 ;  data.write_line = 8 ;  data.write_valid = 1;  data.offset = 3;  data.read_line  = 1; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 8, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 8, exit);
        data.step = 1 ;  data.write_line = 9 ;  data.write_valid = 1;  data.offset = 4;  data.read_line  = 1; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 9, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 9, exit);
        data.step = 1 ;  data.write_line = 10 ;  data.write_valid = 1;  data.offset = 5;  data.read_line  = 1; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 10, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
        data.step = 2 ;  data.write_line = 11 ;  data.write_valid = 1;  data.offset = 1;  data.read_line  = 2; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 11, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 11, exit);
        data.step = 2 ;  data.write_line = 12 ;  data.write_valid = 1;  data.offset = 2;  data.read_line  = 2; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 12, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 12, exit);
        data.step = 2 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 3;  data.read_line  = 2; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 13, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 13, exit);
        data.step = 2 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 4;  data.read_line  = 2; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 14, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 14, exit);
        data.step = 2 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 5;  data.read_line  = 2; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 15, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 15, exit);
        data.step = 3 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 1;  data.read_line  = 3; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 16, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 16, exit);
        data.step = 3 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 2;  data.read_line  = 3; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 17, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 17, exit);
        data.step = 3 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 3;  data.read_line  = 3; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 18, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 18, exit);
        data.step = 3 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 4;  data.read_line  = 3; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 19, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 19, exit);
        data.step = 3 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 5;  data.read_line  = 3; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 20, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
        data.step = 4 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 1;  data.read_line  = 4; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 21, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 21, exit);
        data.step = 4 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 2;  data.read_line  = 4; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 22, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 22, exit);
        data.step = 4 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 3;  data.read_line  = 4; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 23, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 23, exit);
        data.step = 4 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 4;  data.read_line  = 4; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 24, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 24, exit);
        data.step = 4 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 5;  data.read_line  = 4; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 25, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 25, exit);
        data.step = 5 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 1;  data.read_line  = 5; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 26, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 26, exit);
        data.step = 5 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 2;  data.read_line  = 5; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 27, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 27, exit);
        data.step = 5 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 3;  data.read_line  = 5; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 28, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 28, exit);
        data.step = 5 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 4;  data.read_line  = 5; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 29, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 29, exit);
        data.step = 5 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 5;  data.read_line  = 5; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 30, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
        data.step = 6 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 1;  data.read_line  = 6; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 31, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 31, exit);
        data.step = 6 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 2;  data.read_line  = 6; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 32, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 32, exit);
        data.step = 6 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 3;  data.read_line  = 6; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 33, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 33, exit);
        data.step = 6 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 4;  data.read_line  = 6; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 34, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 34, exit);
        data.step = 6 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 5;  data.read_line  = 6; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 35, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 35, exit);

        data.step = 7 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 1;  data.read_line  = 7; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 36, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 36, exit);
        data.step = 7 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 2;  data.read_line  = 7; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 37, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 37, exit);
        data.step = 7 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 3;  data.read_line  = 7; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 38, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 38, exit);
        data.step = 7 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 4;  data.read_line  = 7; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 39, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 39, exit);
        data.step = 7 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 5;  data.read_line  = 7; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 40, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);

        data.step = 8 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 1;  data.read_line  = 8; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 41, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 41, exit);
        data.step = 8 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 2;  data.read_line  = 8; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 42, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 42, exit);
        data.step = 8 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 3;  data.read_line  = 8; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 43, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
        data.step = 8 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 4;  data.read_line  = 8; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 44, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 44, exit);
        data.step = 8 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 5;  data.read_line  = 8; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 45, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 45, exit);

        data.step = 9 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 1;  data.read_line  = 9; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 46, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 46, exit);
        data.step = 9 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 2;  data.read_line  = 9; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 47, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 47, exit);
        data.step = 9 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 3;  data.read_line  = 9; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 48, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 48, exit);
        data.step = 9 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 4;  data.read_line  = 9; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 49, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 49, exit);
        data.step = 9 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 5;  data.read_line  = 9; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 50, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 50, exit);

        data.step = 10 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 1;  data.read_line  = 10; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 51, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 51, exit);
        data.step = 10 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 2;  data.read_line  = 10; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 52, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 52, exit);
        data.step = 10 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 3;  data.read_line  = 10; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 53, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 53, exit);
        data.step = 10 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 4;  data.read_line  = 10; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 54, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 54, exit);
        data.step = 10 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 5;  data.read_line  = 10; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 55, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 55, exit);

        data.step = 11 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 1;  data.read_line  = 11; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 56, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 56, exit);
        data.step = 11 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 2;  data.read_line  = 11;
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 57, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 57, exit);
        data.step = 11 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 3;  data.read_line  = 11; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 58, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 58, exit);
        data.step = 11 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 4;  data.read_line  = 11; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 59, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 59, exit);
        data.step = 11 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 5;  data.read_line  = 11;  
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 60, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 60, exit);

        data.step = 12 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 1;  data.read_line  = 12; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 61, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 61, exit);
        data.step = 12 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 2;  data.read_line  = 12; 
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 62, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 62, exit);
        data.step = 12 ;  data.write_line = 0x7 ;  data.write_valid = 0;  data.offset = 3;  data.read_line  = 12;
        res = arad_tbl_access_glem_step_set(unit, tbl_indx, 63, &data);
        SOC_SAND_CHECK_FUNC_RESULT(res, 63, exit);
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_pp_flp_em_init", 0, 0);
}
   ...
   )
{
  ...
  /*ARAD_PP_FLP_INIT_PRINT_ADVANCE("arad_pp_flp_ethernet_prog_update ingress_learn_enable: %h ingress_learn_enable %h sa_lookup_type %h",
                          ingress_learn_enable,ingress_learn_oppurtunistic,sa_lookup_type);*/

  res = arad_pp_flp_lookups_ethernet_ing_learn(unit,sa_auth_enabled,slb_enabled);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  ...
  {
    uint32 prog_id;
    res = arad_pp_sw_db_flp_prog_app_to_index_get(unit, ARAD_PP_FLP_KEY_PROGRAM_MAC_IN_MAC_AFTER_TERMINATIOM, &prog_id);
    SOC_SAND_CHECK_FUNC_RESULT(res, 15, exit);
  ...
  if (SOC_DPP_CONFIG(unit)->trill.mode) 
  {
    res = arad_pp_flp_key_const_TRILL_mc(unit,ingress_learn_enable);
    SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
    ...
    if (SOC_DPP_CONFIG(unit)->trill.transparent_service) {
      ...
      res = arad_pp_flp_lookups_TRILL_mc(unit,ingress_learn_enable,tcam_access_profile_id);
    } else {
  ...
  if (SOC_DPP_L3_SRC_BIND_IPV4_ENABLE(unit))
  {
    ...
    res = arad_pp_flp_lookups_ethernet_tk_epon_uni_v4(unit,sa_auth_enabled,slb_enabled);
    SOC_SAND_CHECK_FUNC_RESULT(res, 45, exit);
  ...
  if (SOC_DPP_CONFIG(unit)->pp.l3_src_bind_arp_relay & SOC_DPP_L3_SRC_BIND_FOR_ARP_RELAY_UP) {
    res = arad_pp_flp_app_to_prog_index_get(unit,
                                            ARAD_PP_FLP_KEY_PROGRAM_PON_ARP_UPSTREAM,
                                            &prog_id);
...
exit:
 SOC_SAND_EXIT_AND_SEND_ERROR("arad_pp_flp_app_to_prog_index_get", 0, 0);
}
   ...
   )
{
  ...
  int lines[2]; /* current max number of lines, used when programs are not consecutive */

  SOC_SAND_INIT_ERROR_DEFINITIONS(0);

  switch(trap_code_internal) {
  case ARAD_PP_TRAP_CODE_INTERNAL_FLP_MPLS_UNKNOWN_LABEL:
      line = ARAD_PP_FLP_KEY_PROGRAM_LSR;
      if (SOC_DPP_CONFIG(unit)->pp.ipv4_tunnel_term_bitmap_enable & SOC_DPP_IP_TUNNEL_TERM_DB_NVGRE)
      {
        map_id = 1;
        nof_lines = 2;
        use_lines_array = 1;
        lines[0] = ARAD_PP_FLP_KEY_PROGRAM_LSR;
        lines[1] = ARAD_PP_FLP_KEY_PROGRAM_VPLSOGRE;
      }
      break;
  case ARAD_PP_TRAP_CODE_INTERNAL_FLP_P2P_MISCONFIGURATION:
      line = ARAD_PP_FLP_KEY_PROGRAM_P2P;
      break;
  case ARAD_PP_TRAP_CODE_INTERNAL_FLP_TRILL_UNKNOWN_UC:
      line = ARAD_PP_FLP_KEY_PROGRAM_TRILL_UC;
      break;
  case ARAD_PP_TRAP_CODE_INTERNAL_FLP_TRILL_UNKNOWN_MC:
      line = ARAD_PP_FLP_KEY_PROGRAM_TRILL_MC_ONE_TAG;
      nof_lines = 2; /* for consecutive progs */

      map_id = 1; 
      break;
  case ARAD_PP_TRAP_CODE_INTERNAL_FLP_DEFAULT_UCV6:
      line = ARAD_PP_FLP_KEY_PROGRAM_IPV6UC;
      break;
  case ARAD_PP_TRAP_CODE_INTERNAL_FLP_DEFAULT_MCV6:
      line = ARAD_PP_FLP_KEY_PROGRAM_IPV6MC;
      break;
  case ARAD_PP_TRAP_CODE_INTERNAL_FLP_FCF:
      line = ARAD_PP_FLP_KEY_PROGRAM_FC_REMOTE;
  ...
  for (indx = 0;indx < nof_lines; line++,indx++) {
      ...
      if (map_id != 0) {          
          res = arad_pp_flp_app_to_prog_index_get(unit,(uint32)line,&act_line);
          SOC_SAND_CHECK_FUNC_RESULT(res, 100, exit);
      }
      else {
          act_line = (uint8)line;
      ...
      if ((act_line == ARAD_PP_FLP_KEY_PROGRAM_IPV6UC) || (act_line == ARAD_PP_FLP_KEY_PROGRAM_IPV6MC)) {
          /* 0x0 - Use VRF Default Unicast
             0x1 - Use VRF Default Multicast
             Else: Use NotFoundTrapCode from program */
          ...
          if (flp_process_tbl.not_found_trap_strength) {
              ...
              /* take VRF default destination */
              if (act_line == ARAD_PP_FLP_KEY_PROGRAM_IPV6UC) {
                  flp_process_tbl.select_default_result_a = 0; 
                  flp_process_tbl.select_default_result_b = 0;
              }
              else { /* MC */
  ...
  )
{
  ...
  res = arad_pp_flp_app_to_prog_index_get(unit,(uint32)prog_id, &act_line);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  ...
  if(learn_enable)
  {
      res = arad_pp_ihb_flp_lookups_tbl_get_unsafe(unit, act_line, &flp_lookups_tbl);
      SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);

      flp_lookups_tbl.learn_key_select = ARAD_PP_FLP_LKP_KEY_SELECT_FID_FWD_MAC_KEY_HW_VAL;

      res = arad_pp_ihb_flp_lookups_tbl_set_unsafe(unit, act_line, &flp_lookups_tbl);
      SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);
  }
   ...
   )
{
  ...
  flp_process_tbl.lpm_2nd_lkp_enable_default = !flood; 
  flp_process_tbl.lpm_public_2nd_lkp_enable_default = !flood;
  ...
  if (SOC_DPP_CONFIG(unit)->l3.ipmc_vpn_lookup_enable) {
    /* ipv4 mc comp */  
    program_id = ARAD_PP_FLP_KEY_PROGRAM_IPV4COMPMC_WITH_RPF;
  } else {
    /* Global ipv4 mc comp */
    res = arad_pp_flp_app_to_prog_index_get(unit,
   ...
   )
{
  ...
  /* For static, program id and app id is same */
  program_id = (mode == 0)?ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_ING_LEARN:ARAD_PP_FLP_KEY_PROGRAM_IPV4MC_WITH_RPF;
  ARAD_PP_FLP_SW_DB_FLP_PROGRAM_ID_SET(unit, program_id, program_id);
  ...
  /* update program */
  prog_selection_cam_tbl.program      = (mode == 0)?ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_ING_LEARN:ARAD_PP_FLP_KEY_PROGRAM_IPV4MC_WITH_RPF;
   ...
   )
{
  ...
  *mode  = (prog_selection_cam_tbl.program == ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_ING_LEARN)?0:1;
   ...
   )
{
  ...
  flp_process_tbl.not_found_trap_strength      = 7;
  flp_process_tbl.unknown_address              = 3;
  flp_process_tbl.not_found_trap_code          = ARAD_PP_TRAP_CODE_INTERNAL_LLR_ACCEPTABLE_FRAME_TYPE0;
   ...
   )
{
  ...
  SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
  res = arad_pp_flp_key_const_pwe_gre(unit, FALSE, FALSE, FALSE);
  SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
   ...
   )
{
    ...
    res = arad_pp_sw_db_flp_prog_app_to_index_get(unit, ARAD_PP_FLP_KEY_PROGRAM_FC_WITH_VFT_N_PORT, &prog_id);
    SOC_SAND_CHECK_FUNC_RESULT(res, 15, exit);
    res = arad_pp_flp_program_disable(unit,prog_id);
    SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);

    res = arad_pp_sw_db_flp_prog_app_to_index_get(unit, ARAD_PP_FLP_KEY_PROGRAM_FC_N_PORT, &prog_id);
    SOC_SAND_CHECK_FUNC_RESULT(res, 25, exit);
    res = arad_pp_flp_program_disable(unit,prog_id);
    SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
   ...
   )
{
    ...
    res = arad_pp_sw_db_flp_prog_app_to_index_get(unit, ARAD_PP_FLP_KEY_PROGRAM_FC, (uint32*)&(progs[0]));
    SOC_SAND_CHECK_FUNC_RESULT(res, 5, exit);
    res = arad_pp_sw_db_flp_prog_app_to_index_get(unit, ARAD_PP_FLP_KEY_PROGRAM_FC_WITH_VFT, (uint32*)&(progs[1]));
    SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
    res = arad_pp_sw_db_flp_prog_app_to_index_get(unit, ARAD_PP_FLP_KEY_PROGRAM_FC_REMOTE, (uint32*)&(progs[2]));
    SOC_SAND_CHECK_FUNC_RESULT(res, 15, exit);
    res = arad_pp_sw_db_flp_prog_app_to_index_get(unit, ARAD_PP_FLP_KEY_PROGRAM_FC_WITH_VFT_REMOTE, (uint32*)&(progs[3]));
    SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);    

    res = arad_pp_flp_lookups_fcf(unit, progs, (!is_npv));
   ...
   )
{
    ...
    if (!is_zoning_enabled) {
        res = arad_pp_sw_db_flp_prog_app_to_index_get(unit, ARAD_PP_FLP_KEY_PROGRAM_FC_N_PORT, (uint32*)&(progs[0]));
        SOC_SAND_CHECK_FUNC_RESULT(res, 35, exit);
        res = arad_pp_sw_db_flp_prog_app_to_index_get(unit, ARAD_PP_FLP_KEY_PROGRAM_FC_WITH_VFT_N_PORT, (uint32*)&(progs[1]));
        SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);
   ...
   )
{
    ...
    res = arad_pp_sw_db_flp_prog_app_to_index_get(unit, ARAD_PP_FLP_KEY_PROGRAM_FC, &prog);
    SOC_SAND_CHECK_FUNC_RESULT(res, 5, exit);

    res = arad_pp_ihb_flp_key_construction_tbl_get_unsafe(unit, prog, &flp_key_construction_tbl);
    SOC_SAND_CHECK_FUNC_RESULT(res, 15, exit);

    if(flp_key_construction_tbl.instruction_0_16b == ARAD_PP_FLP_16B_INST_P6_VSI_D_12_BITS){
   ...
   )
{
    ...
    /* program selection */
    /* allocate resources */
    res = arad_pp_flp_get_next_avaialable(unit,ARAD_PP_FLP_KEY_PROGRAM_BIDIR,&cam_sel,&prog_id);
    SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
    ...
    flp_key_construction_tbl.instruction_1_16b        = ARAD_PP_FLP_16B_INST_P6_IN_RIF_D; /* in-RIF */
    flp_key_construction_tbl.key_a_inst_0_to_5_valid    = 0x3 /*6'b000011*/;
    flp_key_construction_tbl.key_b_inst_0_to_5_valid    = 0x0 /*6'b000000*/;
    flp_key_construction_tbl.key_c_inst_0_to_5_valid    = 0x0 /*6'b000000*/;
    res = arad_pp_ihb_flp_key_construction_tbl_set_unsafe(unit, prog_id, &flp_key_construction_tbl);
...
)
{
    ...
    case ARAD_PP_FLP_KEY_PROGRAM_TM:
      return "ARAD_PP_FLP_KEY_PROGRAM_TM";
    case ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_ING_LEARN:
      return "ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_ING_LEARN";
    case ARAD_PP_FLP_KEY_PROGRAM_IPV4UC_WITH_DEFAULT:
      return "ARAD_PP_FLP_KEY_PROGRAM_IPV4UC_WITH_DEFAULT";
    case ARAD_PP_FLP_KEY_PROGRAM_IPV4UC_WITH_RPF:
      return "ARAD_PP_FLP_KEY_PROGRAM_IPV4UC_WITH_RPF";
    case ARAD_PP_FLP_KEY_PROGRAM_IPV4UC:
      return "ARAD_PP_FLP_KEY_PROGRAM_IPV4UC";
    case ARAD_PP_FLP_KEY_PROGRAM_IPV6UC:
      return "ARAD_PP_FLP_KEY_PROGRAM_IPV6UC";
    case ARAD_PP_FLP_KEY_PROGRAM_P2P:
      return "ARAD_PP_FLP_KEY_PROGRAM_P2P";
    case ARAD_PP_FLP_KEY_PROGRAM_IPV6MC:
      return "ARAD_PP_FLP_KEY_PROGRAM_IPV6MC";
    case ARAD_PP_FLP_KEY_PROGRAM_LSR:
      return "ARAD_PP_FLP_KEY_PROGRAM_LSR";
    case ARAD_PP_FLP_KEY_PROGRAM_TRILL_UC:
      if (SOC_DPP_CONFIG(unit)->trill.mode) { 
        return "ARAD_PP_FLP_KEY_PROGRAM_TRILL_UC";
      } else {
        return "ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_TK_EPON_UNI_V4_STATIC";
      }
    case ARAD_PP_FLP_KEY_PROGRAM_TRILL_MC_ONE_TAG:
      if (SOC_DPP_CONFIG(unit)->trill.mode) { 
        return "ARAD_PP_FLP_KEY_PROGRAM_TRILL_MC_ONE_TAG";
      } else {
        return "ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_TK_EPON_UNI_V6_STATIC";
      }
    case ARAD_PP_FLP_KEY_PROGRAM_TRILL_MC_TWO_TAGS:
      if (SOC_DPP_CONFIG(unit)->trill.mode) { 
        return "ARAD_PP_FLP_KEY_PROGRAM_TRILL_MC_TWO_TAGS";
      } else {
        return "ARAD_PP_FLP_KEY_PROGRAM_MAC_IN_MAC_AFTER_TERMINATIOM";
      }
    case ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_TK_EPON_UNI_V4_DHCP:
      return "ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_TK_EPON_UNI_V4_DHCP";
    case ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_TK_EPON_UNI_V6_DHCP:
      return "ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_TK_EPON_UNI_V6_DHCP";
    case ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_MAC_IN_MAC:
      return "ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_MAC_IN_MAC";
    case ARAD_PP_FLP_KEY_PROGRAM_TRILL_AFTER_TERMINATION:
      return "ARAD_PP_FLP_KEY_PROGRAM_TRILL_AFTER_TERMINATION";
    case ARAD_PP_FLP_KEY_PROGRAM_IPV4MC_WITH_RPF:
      return "ARAD_PP_FLP_KEY_PROGRAM_IPV4MC_WITH_RPF";
    case ARAD_PP_FLP_KEY_PROGRAM_IPV4COMPMC_WITH_RPF:
      return "ARAD_PP_FLP_KEY_PROGRAM_IPV4COMPMC_WITH_RPF";
    case ARAD_PP_FLP_KEY_PROGRAM_FC_WITH_VFT_N_PORT:
      return "ARAD_PP_FLP_KEY_PROGRAM_FC_WITH_VFT_N_PORT";
    case ARAD_PP_FLP_KEY_PROGRAM_FC_N_PORT:
      return "ARAD_PP_FLP_KEY_PROGRAM_FC_N_PORT";
    case ARAD_PP_FLP_KEY_PROGRAM_FC_REMOTE:
      return "ARAD_PP_FLP_KEY_PROGRAM_FC_REMOTE";
    case ARAD_PP_FLP_KEY_PROGRAM_FC_WITH_VFT_REMOTE:
      return "ARAD_PP_FLP_KEY_PROGRAM_FC_WITH_VFT_REMOTE";
    case ARAD_PP_FLP_KEY_PROGRAM_FC:
      return "ARAD_PP_FLP_KEY_PROGRAM_FC";
    case ARAD_PP_FLP_KEY_PROGRAM_FC_WITH_VFT:
      return "ARAD_PP_FLP_KEY_PROGRAM_FC_WITH_VFT";
    case ARAD_PP_FLP_KEY_PROGRAM_COUPLING_LSR:
      return "ARAD_PP_FLP_KEY_PROGRAM_COUPLING_LSR";
    case ARAD_PP_FLP_KEY_PROGRAM_BIDIR:
      return "ARAD_PP_FLP_KEY_PROGRAM_BIDIR";
    case ARAD_PP_FLP_KEY_PROGRAM_IPV6UC_WITH_RPF:
      return "ARAD_PP_FLP_KEY_PROGRAM_IPV6UC_WITH_RPF";
    case ARAD_PP_FLP_KEY_PROGRAM_VMAC_UPSTREAM:
      return "ARAD_PP_FLP_KEY_PROGRAM_VMAC_UPSTREAM";
    case ARAD_PP_FLP_KEY_PROGRAM_VMAC_DOWNSTREAM:
      return "ARAD_PP_FLP_KEY_PROGRAM_VMAC_DOWNSTREAM";
    case ARAD_PP_FLP_KEY_PROGRAM_VPLSOGRE:
      return "ARAD_PP_FLP_KEY_PROGRAM_VPLSOGRE";
    case ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_PON_LOCAL_ROUTE:
      return "ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_PON_LOCAL_ROUTE";
    case ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_PON_DEFAULT_UPSTREAM:
      return "ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_PON_DEFAULT_UPSTREAM";
    case ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_PON_DEFAULT_DOWNSTREAM:
      return "ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_PON_DEFAULT_DOWNSTREAM";
    case ARAD_PP_FLP_KEY_PROGRAM_GLOBAL_IPV4COMPMC_WITH_RPF:
      return "ARAD_PP_FLP_KEY_PROGRAM_GLOBAL_IPV4COMPMC_WITH_RPF";
    case ARAD_PP_FLP_KEY_PROGRAM_PON_ARP_DOWNSTREAM:
      return "ARAD_PP_FLP_KEY_PROGRAM_PON_ARP_DOWNSTREAM";
    case ARAD_PP_FLP_KEY_PROGRAM_PON_ARP_UPSTREAM:
      return "ARAD_PP_FLP_KEY_PROGRAM_PON_ARP_UPSTREAM";
    case ARAD_PP_FLP_KEY_PROGRAM_FC_TRANSIT:
        return "ARAD_PP_FLP_KEY_PROGRAM_FC_TRANSIT";
    case ARAD_PP_FLP_KEY_PROGRAM_IPV6UC_WITH_RPF_2PASS:
        return "ARAD_PP_FLP_KEY_PROGRAM_IPV6UC_WITH_RPF_2PASS";
	case ARAD_PP_FLP_KEY_PROGRAM_BFD_IPV4_SINGLE_HOP:
        return "ARAD_PP_FLP_KEY_PROGRAM_BFD_IPV4_SINGLE_HOP";
	case ARAD_PP_FLP_KEY_PROGRAM_BFD_IPV6_SINGLE_HOP:
        return "ARAD_PP_FLP_KEY_PROGRAM_BFD_IPV6_SINGLE_HOP";
	case ARAD_PP_FLP_KEY_PROGRAM_OAM_STATISTICS:
        return "ARAD_PP_FLP_KEY_PROGRAM_OAM_STATISTICS";
    case ARAD_PP_FLP_KEY_PROGRAM_OAM_DOWN_UNTAGGED_STATISTICS:
        return "ARAD_PP_FLP_KEY_PROGRAM_OAM_DOWN_UNTAGGED_STATISTICS";
    case ARAD_PP_FLP_KEY_PROGRAM_OAM_SINGLE_TAG_STATISTICS:
        return "ARAD_PP_FLP_KEY_PROGRAM_OAM_SINGLE_TAG_STATISTICS";
	case ARAD_PP_FLP_KEY_PROGRAM_OAM_DOUBLE_TAG_STATISTICS:
        return "ARAD_PP_FLP_KEY_PROGRAM_OAM_DOUBLE_TAG_STATISTICS";
    case ARAD_PP_FLP_KEY_PROGRAM_BFD_STATISTICS:
        return "ARAD_PP_FLP_KEY_PROGRAM_BFD_STATISTICS"; 
    case ARAD_PP_FLP_KEY_PROGRAM_BFD_MPLS_STATISTICS:
        return "ARAD_PP_FLP_KEY_PROGRAM_BFD_MPLS_STATISTICS"; 
    case ARAD_PP_FLP_KEY_PROGRAM_BFD_PWE_STATISTICS:
        return "ARAD_PP_FLP_KEY_PROGRAM_BFD_PWE_STATISTICS";      
    default:
  ...
  )
{
     ...
     */
    *prog_id = -1;
    SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  10,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_read(unit, IHP_DBG_FLP_SELECTED_PROGRAMr, REG_PORT_ANY, 0, DBG_FLP_SELECTED_PROGRAMf, &progs_bmp));
    if (to_print) {
        LOG_CLI((BSL_META_U(unit,
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  20,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, IHP_DBG_FLP_SELECTED_PROGRAMr, REG_PORT_ANY, 0, DBG_FLP_SELECTED_PROGRAMf,  fld_val));
