...
uint32 dpp_mult_cud_to_port_map_set(
    ...
    SOC_SAND_IN SOC_TMC_FAP_PORT_ID port  /* input TM port */
)
{
    ...
    if (SOC_IS_JERICHO(unit)) {
        ...
        entry[0] &= JER_CUD2PORT_MAPPING_PORT_MASK << shift;
        entry[0] |= port << shift;
...
uint32 dpp_mult_cud_to_port_map_get(
    SOC_SAND_IN  int                 unit, /* input device */
    SOC_SAND_IN  uint32              cud,  /* input cud/outlif */
    SOC_SAND_OUT SOC_TMC_FAP_PORT_ID *port /* output TM port */
)
{
    dpp_mcds_base_t *mcds = dpp_get_mcds(unit);
    uint32 entry[2];
    ...
    if (SOC_IS_JERICHO(unit)) {
        ...
        *port = (entry[0] >> (JER_CUD2PORT_MAPPING_NOF_PORT_BITS * (cud % JER_CUD2PORT_MAPPING_PORTS_PER_ENTRY))) &
          JER_CUD2PORT_MAPPING_NOF_PORT_BITS;
    } else {
...
)
{
  ...
  *is_open = dpp_mult_does_group_exist(unit, group_mcid, TRUE) ; /* check if the group is open */
...
)
{
    ...
    } else if (!dpp_egress_group_open_get(unit, group_mcid)) { /* group is not open */
        SOCDNX_EXIT_WITH_ERR(SOC_E_NOT_FOUND, (_BSL_SOCDNX_MSG("MC group is not created")));
    }
...
)
{
    ...
    /* recreate the group with the extra replication */
    {
        ...
        if (*out_err) { /* If failed due to lack of free entries */
            ...
            DPP_MC_FOREACH_CORE(changed_cores, core_bm, core) {
                ret1 = dpp_mcds_remove_replications_from_egress_group(unit, core, group_mcid, port, cud);
                if (ret2 == SOC_E_NONE) {
                    ret2 = ret1;
                }
            }
...
/*********************************************************************
*   Add port replications to an Egress-Multicast bitmap group.
*********************************************************************/
...
/*********************************************************************
*   Removes a port replication of an egress multicast bitmap group.
*********************************************************************/
