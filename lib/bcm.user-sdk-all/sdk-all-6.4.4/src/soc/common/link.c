...
soc_linkscan_hw_init(int unit)
{
    ...
    if (SOC_IS_XGS3_SWITCH(unit) || SOC_IS_SIRIUS(unit) || SOC_IS_CALADAN3(unit)) {
        ...
        /* For Katana2, PBMP_PORT_ALL does not include physical ports not
         * enabled during init. This is causing issues when they are enabled
         * by Hot-swap */
        ...
        if (SOC_IS_SABER2(unit)) {
            ...
            LOG_ERROR(BSL_LS_SOC_COMMON,
                      (BSL_META_U(unit,
                               "soc_linkscan_hw_init: TBD\n")));
        } else
#endif
        if (SOC_IS_KATANA2(unit)) {
        /* In KT2, All Physical ports (1-40) lie between the CMIC port(0)
         * and LB port numbers(40) */
            SOC_PBMP_CLEAR(phy_port_pbmp);
            for(port = CMIC_PORT(unit) + 1; port < LB_PORT(unit); port++) {
                SOC_PBMP_PORT_ADD(phy_port_pbmp, port);
            }
        }
#endif
