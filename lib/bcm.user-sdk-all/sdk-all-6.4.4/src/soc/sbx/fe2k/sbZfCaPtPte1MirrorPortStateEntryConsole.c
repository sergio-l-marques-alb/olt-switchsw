...
#include "sbZfCaPtPte1MirrorPortStateEntryConsole.hx"



/* Print members in struct */
void
sbZfCaPtPte1MirrorPortStateEntry_Print(sbZfCaPtPte1MirrorPortStateEntry_t *pFromStruct) {
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("CaPtPte1MirrorPortStateEntry:: hdrpresent=0x%01x"), (unsigned int)  pFromStruct->m_uHdrPresent));
  LOG_WARN(BSL_LS_SOC_COMMON,
  ...
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("CaPtPte1MirrorPortStateEntry:: numpageshi=0x%01x"), (unsigned int)  pFromStruct->m_uNumPagesHi));
  LOG_WARN(BSL_LS_SOC_COMMON,
  ...
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("CaPtPte1MirrorPortStateEntry:: sourcequeue=0x%02x"), (unsigned int)  pFromStruct->m_uSourceQueue));
  LOG_WARN(BSL_LS_SOC_COMMON,
...
char *
sbZfCaPtPte1MirrorPortStateEntry_SPrint(sbZfCaPtPte1MirrorPortStateEntry_t *pFromStruct, char *pcToString, uint32 lStrSize) {
  uint32 WrCnt = 0x0;

  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"CaPtPte1MirrorPortStateEntry:: hdrpresent=0x%01x", (unsigned int)  pFromStruct->m_uHdrPresent);
  WrCnt += SB_SPRINTF(&pcToString[WrCnt]," pktlength=0x%04x", (unsigned int)  pFromStruct->m_uPktLength);
  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"\n");

  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"CaPtPte1MirrorPortStateEntry:: numpageshi=0x%01x", (unsigned int)  pFromStruct->m_uNumPagesHi);
  ...
  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"CaPtPte1MirrorPortStateEntry:: sourcequeue=0x%02x", (unsigned int)  pFromStruct->m_uSourceQueue);
...
int
sbZfCaPtPte1MirrorPortStateEntry_Validate(sbZfCaPtPte1MirrorPortStateEntry_t *pZf) {
...
int
sbZfCaPtPte1MirrorPortStateEntry_SetField(sbZfCaPtPte1MirrorPortStateEntry_t *s, char* name, int value) {
