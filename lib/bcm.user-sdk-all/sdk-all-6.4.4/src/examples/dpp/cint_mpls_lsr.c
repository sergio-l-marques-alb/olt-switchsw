...
mpls_add_term_entry(int unit, int term_label, uint32 next_prtcl)
{
	...
	if (!mpls_termination_label_index_enable) {
		...
		rv = mpls_add_index_term_entry_info(unit, 1, term_label, flags, next_prtcl);
		if (rv != BCM_E_NONE) {
			printf("Error, in mpls_add_index_term_entry_info\n");
			return rv;
		}
		rv = mpls_add_index_term_entry_info(unit, 2, term_label, flags, next_prtcl);
		if (rv != BCM_E_NONE) {
			printf("Error, in mpls_add_index_term_entry_info\n");
			return rv;
		}
...
/* Enable mpls raw processing: packets sent to/from the given port
   will be received/sent without an ethernet header */
int
enable_mpls_raw_processing(int unit, bcm_port_t port){
	int rv = BCM_E_NONE;
	bcm_vlan_port_t vp;

	rv = bcm_switch_control_port_set(unit, port, bcmSwitchPortHeaderType, BCM_SWITCH_PORT_HEADER_TYPE_MPLS_RAW);
    if (rv != BCM_E_NONE) {
	...
	vp.vsi = 20;    /*Default vsi value (could be any value - for incoming mpls raw packet we don't search the mac table */
