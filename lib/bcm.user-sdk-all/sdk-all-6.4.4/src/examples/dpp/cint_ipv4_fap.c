 ...
 */
int create_l3_intf(int unit, uint32 flags, int vlan, int *intf, int vrf) {
    ...
    *intf = l3if.l3a_intf_id;
    ...
    l3_ing_if.qos_map_id = qos_map_id_l3_ingress_get(0);
    printf("cos_profile(0x%08x) rif(%d)\n", l3_ing_if.qos_map_id, l3if.l3a_intf_id);
    
    /* Set uRPF */
    print bcm_switch_control_set(0, bcmSwitchL3UrpfMode, bcmL3IngressUrpfLoose);
    
    l3_ing_if.urpf_mode = bcmL3IngressUrpfLoose;
    l3_ing_if.flags = BCM_L3_INGRESS_WITH_ID;
    l3_ing_if.vrf = vrf;
    rc = bcm_l3_ingress_create(0, l3_ing_if, l3if.l3a_intf_id);
    if (rc != BCM_E_NONE) {
        printf("bcm_l3_ingress_create failed: %d", rc);
        return rc;
    }
...
int bcm_l3_main(int unit, int second_unit, int dst_port) {
    ...
    if (second_unit<0)
    {
        for (i=0; i<MAX_PORTS; i++) { 
            ...
            rc = create_l3_intf(unit ,CINT_NO_FLAGS, vlan, &ingress_intf[i],vrf);
           if (rc != BCM_E_NONE) {
    ...
    {  
        for (i=0; i<MAX_PORTS; i++) {    
            ...
            {
                rc = create_l3_intf(unit ,CINT_NO_FLAGS, vlan, &ingress_intf[i]);
                if (rc != BCM_E_NONE) {
                 printf ("create_l3_intf failed: %d \n", rc);
                 return rc;
                }
                rc = create_l3_intf(second_unit ,CINT_NO_FLAGS, vlan, &ingress_intf[i]); 
                if (rc != BCM_E_NONE) {
                  printf ("create_l3_intf failed: %d \n", rc);
                  return rc;
                }    
