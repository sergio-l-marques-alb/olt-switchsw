...
static int diag_reg_set(int unit, char *name, reg_val value)
{
    ...
    if ((SOC_IS_PETRAB(unit)) && isint(name)) {    /* Numerical address given */
        ...
        /* cli_out("write 0x%x=0x%x %ssuccessful\n",regaddr, value[0], rv ? "un" : "");
        */
    } else
#endif /* BCM_PETRAB_SUPPORT */
...
static int diag_soc_mem_read_range(int unit, char *name, int index1, int index2, void *buffer)
{
    int         copyno;
    soc_mem_t   mem;
    int         rv = SOC_E_NONE;
...
static int diag_soc_mem_write_range(int unit, char *name, int index1, int index2, void *buffer)
{
    int         copyno;
    soc_mem_t   mem;
    int         rv = SOC_E_NONE;
...
static int diag_mbist(int unit, int skip_errors)
{
    ...
    if (SOC_IS_JERICHO(unit) || SOC_IS_ARDON(unit) ) {
...
static int diag_test_mc(int unit, char *test_name)
{
    ...
    if (!sal_strcmp(test_name, "get_mc_asserts")) { /* Get the number of MC asserts that occurred */
        ...
        return num;
    } else if (!sal_strcmp(test_name, "get_nof_mcids")) { /* get unmber of egress MCIDs */
    ...
    cli_out("ERROR: unknown test name, use one of: "
            ...
            "test_assert_0, test_assert_none, perform_mc_assert, get_mc_asserts, get_nof_mcids, "
            "get_nof_egress_bitmap_groups\n");
    return SOC_E_PARAM;
}
...
static int init_device_for_mems_ser_test(int unit, ser_mem_read_t *ser_mem_read)
{
    ...
    if (!SOC_IS_ARAD(unit) && !SOC_IS_FE1600_B0(unit)){
        return -1;
    }

     dcmn_init_fill_table(unit);
    ...
    if (SOC_IS_ARAD(unit)) {
        ...
        if (SOC_E_NONE != soc_reg_fields32_modify(unit, ECI_GLOBAL_1r, REG_PORT_ANY,2,fields,values))
            return -1;
...
ser_mem_write_cb(int unit, soc_mem_t mem, void* data)
{
  ...
  return dcmn_fill_table_with_entry(unit, mem, MEM_BLOCK_ALL, patterns);
}
...
ser_mem_read_and_compare_cb(int unit, soc_mem_t mem, void* data)
{
    ...
    for (i=0;i< numels;i++) {
        ...
        ret = soc_mem_array_read_range(unit, mem,i, MEM_BLOCK_ANY,memp->index_min,memp->index_max,ser_mem_read->mem_ptr);
        if (ret != SOC_E_NONE) {
            return -1;
        }
        ...
        for (entry_data = ser_mem_read->mem_ptr,entry_index = memp->index_min; entry_index <= memp->index_max; ++entry_index) {
            ...
            if ( nof_error_bits>0 ) {
                cli_out("SER:  %s,%d,%d,%d,%d,%08x,%s,%s,%d,%d\n",SOC_MEM_NAME(unit,mem),soc_mem_index_count(unit, mem),soc_mem_entry_bits(unit, mem),i,entry_index,expected_value, err_print_buffer,err_bits_buffer,nof_error_bits,ser_mem_read->current_read_iteration); 
    ...
    if (table_nof_error_bits) {
        cli_out("==> %u/%u bits failed in %s\n",table_nof_error_bits, table_num_of_bits*numels, SOC_MEM_NAME(unit,mem));
    }
   return 0;
}
                         ...
                         )
{
    ...
    */
    ...
    for (i=0; i < MAX(num_of_iterations,1);i++) {
...
#if defined(INCLUDE_I2C) && (defined(__DUNE_GTO_BCM_CPU__) || defined(__DUNE_WRX_BCM_CPU__))
CINT_FWRAPPER_CREATE_RP4(int, int, 0, 0,
                         cpu_i2c_write,
                         int, int, chip, 0, 0,
                         int, int, addr, 0, 0,
