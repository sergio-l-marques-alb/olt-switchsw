        ...
        int *base_index)
{
    ...
    *base_index = i;
    ...
    if(soc_feature(unit, soc_feature_egress_lport)) {
    ...
    if(SOC_IS_KATANA(unit)) {
        ...
        for (i = 0; i < COUNTOF(src_mod_port_mems); i++) {
            ...
            } else if (soc_feature(unit, soc_feature_egress_lport)) {
...
int _bcm_esw_stk_multi_nexthops_modid_valid(int modid, uint32 mod_mask)
{
    ...
    for (; i < BCM_MAX_NUM_MODID_MASK; i++) {
        if(modid == modid_mask_data[i].num_mods) {
            break;
        }
    }
...
int _bcm_esw_stk_multi_nexthops_module_mask_get(int num_mods, uint32 *mod_mask)
{
    ...
    for (i = BCM_MAX_NUM_MODID_MASK - 1; i > 0; i--) {
        if((num_mods % modid_mask_data[i].num_mods) == 0) {
            break;
        }
    }
