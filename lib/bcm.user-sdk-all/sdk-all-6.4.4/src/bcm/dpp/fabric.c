                             ...
                             int arg)
{
    ...
    switch (type) {
        ...
        case bcmFabricVsqCategory:
            ...
            rc = _bcm_dpp_wb_cosq_update_vsq_data(unit, SOC_CORE_ALL, arg);
            BCMDNX_IF_ERR_EXIT(rc);
       ...
       case bcmFabricMinimalLinksToDestination:
           if (SOC_IS_ARADPLUS(unit) || SOC_IS_JERICHO(unit)) {
               if (!BCM_FABRIC_NUM_OF_LINKS_IS_VALID(unit, arg)){
                   BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("number os links %d is out of range"), arg));
               }
            ...
            if (editing->flags & BCM_FABRIC_TDM_EDITING_MULTICAST) {
                ...
                if (SOC_IS_PETRAB(unit) || BCM_GPORT_IS_SYSTEM_PORT(editing->destination_port)) {
                    ...
                    if (tm_port == -1) {
                        ...
                        modid = SOC_DPP_CORE_TO_MODID(my_mod, core);
                    }
                }
