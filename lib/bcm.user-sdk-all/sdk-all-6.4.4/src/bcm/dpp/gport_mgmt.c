        ...
        if (gport_type == _BCM_DPP_GPORT_IN_TYPE_FEC) {
            ...
            /* Else, get the port information from the forwarding SW DB */
                ...
                /* For FEC VC and Ring Protection, the physical port is retieved via
                   the FEC destination */
                if ((phy_port->type == _BCM_DPP_GPORT_IN_TYPE_FEC_VC) || (phy_port->type == _BCM_DPP_GPORT_IN_TYPE_RING) || (phy_port->type == _BCM_DPP_GPORT_IN_TYPE_FEC_EEP)) {
                    /* get dest from FEC*/
...
{
    int gport_val, global_lif_id = 0, local_lif_id = 0;
    _BCM_GPORT_PHY_PORT_INFO *phy_port=NULL;
    ...
    *encap_id = _BCM_GPORT_ENCAP_ID_LIF_INVALID;
    ...
    if (*gport_type == _BCM_DPP_GPORT_TYPE_ENCAP) {
        ...
        global_lif_id =  SOC_DPP_LIF_ID_FROM_ENCAP_ID(gport_val);
...
int _bcm_dpp_l2_gport_fill_fec(int unit, void *gport, int gport_type, SOC_PPD_FEC_ID fec_id, SOC_PPD_FRWRD_FEC_ENTRY_INFO *work_fec){
    ...
    if(gport_type == BCM_GPORT_VLAN_PORT) {
        ...
        if (SOC_IS_JERICHO(unit) && _BCM_PETRA_L3_ITF_IS_FEC(mpls_port->egress_tunnel_if)) { /* HVPLS */
            SOC_PPD_FRWRD_DECISION_INFO  fwd_decision;
    ...
    else if ((gport_type == BCM_GPORT_MPLS_PORT) || (gport_type == BCM_GPORT_MIM_PORT))
    {
        ...
        if (SOC_IS_ARAD(unit)) {
            ...
            if (SOC_IS_JERICHO(unit) && _BCM_PETRA_L3_ITF_IS_FEC(mpls_port->egress_tunnel_if)) { /* HVPLS */
                fec_entry[0].type = SOC_PPD_FEC_TYPE_TUNNELING_EEI_OUTLIF;
            } 
            else {
                fec_entry[0].type = SOC_PPD_FEC_TYPE_TUNNELING;
            }
    ...
    int gport_type)
{
    ...
    if (gport_type == BCM_GPORT_VLAN_PORT) {
        ...
        if (SOC_IS_JERICHO(unit) && (fec_entry[0].dest.dest_type == SOC_SAND_PP_DEST_FEC)) { /* HVPLS */
            SOC_PPD_FRWRD_DECISION_INFO  fwd_decision;
...
int _bcm_dpp_vlan_port_create(int unit, bcm_vlan_port_t *vlan_port)
{
    ...
    rv = l2_interface_protection_validation(unit, vlan_port, BCM_GPORT_VLAN_PORT, TRUE, &is_protected,
    ...
    */
    ...
    if (encap_with_id) {
        ...
        rv = l2_interface_get_lif_from_port_id(vlan_port, is_protected, is_mc_ingress, (SOC_PPD_LIF_ID*) &global_lif_id, BCM_GPORT_VLAN_PORT);
        BCMDNX_IF_ERR_EXIT(rv);
        ...
        rv = l2_interface_calculate_port_id(unit, act_fec, global_lif_id, vlan_port, BCM_GPORT_VLAN_PORT);
        BCMDNX_IF_ERR_EXIT(rv);
    ...
    if (update) {
        if (!with_id) {
            ...
            if (SOC_IS_JERICHO(unit) && 
                ((vlan_port->flags & BCM_VLAN_PORT_CASCADED) != (vlan_port_o.flags & BCM_VLAN_PORT_CASCADED)) &&
                /* Make sure both old and new vlan_ports have FECs (otherwise this check is irrelevant). */
                (vlan_port->failover_id != 0) && (vlan_port_o.failover_id != 0)) {
    ...
    if (_BCM_DPP_GPORT_INFO_IS_LOCAL_PORT(gport_info) && !remote_lif)
    {
      ...
      BCM_PBMP_ITER(gport_info.pbmp_local_ports,local_port) {
          ...
          if (SOC_IS_PETRAB(unit)) {
            rv = _bcm_dpp_am_template_tpid_profile_data_get(unit, pp_port, core, &profile_type, &accept_mode, &dtag_mode, &ac_key_map_type);
            BCMDNX_IF_ERR_EXIT(rv);
          }
          break;
      }
    ...
    */
    ...
    if(!remote_lif) {
        ...
        if(_BCM_DPP_GPORT_INFO_IS_LAG(gport_info)) {
            global_lif_flags |= BCM_DPP_AM_FLAG_ALLOC_GLOBAL;
        }
        ...
        if ((!update) || 
           ((encap_with_id) && BCM_ENCAP_ID_GET(vlan_port->encap_id) != BCM_ENCAP_ID_GET(vlan_port_o.encap_id))) {
            ...
            if (SOC_IS_JERICHO(unit)) {
                 ...
                 * If lif is direct lif, we don't allocate egress global lif. 
                 *  
                 */ 

                /* Check if lif is mapped or direct */
                if (global_lif_flags & BCM_DPP_AM_FLAG_ALLOC_WITH_ID) {
                ...
                global_lif_flags |= ((is_ingress) ? BCM_DPP_AM_FLAG_ALLOC_INGRESS : 0)
                                 | ((is_egress && !skip_egress_alloc && is_mapped_lif) ? BCM_DPP_AM_FLAG_ALLOC_EGRESS  : 0);
                global_lif_flags &= ~BCM_DPP_AM_FLAG_ALLOC_GLOBAL;

                /* It is possible that lif is direct and egress only. In that case, don't allocate global lif */
                if (global_lif_flags & (BCM_DPP_AM_FLAG_ALLOC_INGRESS | BCM_DPP_AM_FLAG_ALLOC_EGRESS)) {
                ...
                if (update) {
                    rv = _bcm_dpp_gport_to_global_and_local_lif(unit, vlan_port->vlan_port_id, &global_lif_id_o, &in_lif_id_o, &out_lif_id_o, NULL, NULL);
                    BCMDNX_IF_ERR_EXIT(rv);
                ...
                if (is_ingress) {
                    ...
                    rv = _bcm_dpp_am_local_inlif_alloc(unit, local_lif_flags, &local_inlif_info);
                    BCMDNX_IF_ERR_EXIT(rv);
                ...
                if (is_egress && !skip_egress_alloc) {   
                    /* Allocate an egress lif */
                    bcm_dpp_am_local_out_lif_info_t local_out_lif_info;

                    /* If egress lif is direct, allocate the lif id in the local lif. */
                    if (!is_mapped_lif) {
            ...
            rv = _bcm_dpp_gport_to_global_and_local_lif(unit, vlan_port->vlan_port_id, &global_lif_id, &local_in_lif_id, &local_out_lif_id, NULL, NULL);
            BCMDNX_IF_ERR_EXIT(rv);
    ...
    if(is_forward_group) {
        ...
        if(!update) {
            if (with_id) {
                rv = l2_interface_get_lif_from_port_id(vlan_port, is_protected, is_mc_ingress, &fec_id, BCM_GPORT_VLAN_PORT);
                BCMDNX_IF_ERR_EXIT(rv);
           ...
           if (vlan_port->flags & BCM_VLAN_PORT_VSI_BASE_VID){
            ...
            if (is_egress && !skip_egress_alloc) {
                ...
                if (SOC_IS_PETRAB(unit) &&
                    ...
                    soc_sand_rv = soc_ppd_eg_filter_split_horizon_out_ac_orientation_set(unit,active_local_out_lif_id,in_ac_info.orientation);
                    SOC_SAND_IF_ERR_RETURN(soc_sand_rv);
                }
                ...
                /* Add the outlif to the glem. */
                if (SOC_IS_JERICHO(unit) && (!update || global_lif_id != global_lif_id_o) && is_mapped_lif) {
                    rv = MBCM_PP_DRIVER_CALL(unit, mbcm_pp_lif_glem_access_entry_add, (unit, global_lif_id, local_out_lif_id));
                    BCMDNX_IF_ERR_EXIT(rv);
                }
 ...
 )
{
    ...
    _bcm_petra_tpid_profile_t profile_type;
    _bcm_petra_dtag_mode_t dtag_mode;
    _bcm_petra_ac_key_map_type_t ac_key_map_type = _bcm_petra_ac_key_map_type_normal;
    _bcm_dpp_gport_info_t gport_info;
 ...
 )
{
    ...
    *remote_lif = 0;
    ...
    if (BCM_GPORT_IS_MPLS_PORT(gport_id)){
        ...
        if (!is_ingress && !is_egress) { /* MPLS Forwarding group - don't check inlif. */
    ...
    rv = _bcm_dpp_gport_resolve(unit,gport_id,flags,&dest_id,&encap_id,&gport_type);
    BCMDNX_IF_ERR_EXIT(rv);

    /* if gport is FEC, then check if LIF configured seperately, if so,
       then LIF configuration has to be done using the LIF gport.*/
    if((gport_type == _BCM_DPP_GPORT_IN_TYPE_FEC) && (BCM_GPORT_IS_VLAN_PORT(gport_id) || BCM_GPORT_IS_MPLS_PORT(gport_id))) {
#ifdef BCM_ARAD_SUPPORT
            rv = bcm_dpp_gport_mgmt_fec_remote_lif_get(unit, dest_id.dest_val, remote_lif);
            BCMDNX_IF_ERR_EXIT(rv);
#else
            *remote_lif = 0;
#endif
...
int _bcm_dpp_gport_to_global_and_local_lif(int                   unit,
    ...
    if (is_local && is_ingress && is_egress && global_in_lif != global_out_lif) {
 ...
 )
{
    ...
    if(BCM_GPORT_IS_TUNNEL(gport_id) && (in_lif_id != NULL || out_lif_id != NULL)) {
        ...
        if(gport_type_encoding == _BCM_DPP_GPORT_TYPE_ENCAP) {
            if (in_lif_id != NULL) {
              *in_lif_id = SOC_DPP_LIF_ID_FROM_ENCAP_ID(gport_val);
            }
 ...
 )
{
    ...
    *is_member = FALSE;
    ...
    rv = _bcm_dpp_gport_to_global_and_local_lif(unit,gport_id, &global_lif, &local_lif, NULL, NULL, is_local);
    BCMDNX_IF_ERR_EXIT(rv);
...
{
    int is_local, local_in_lif_id, local_out_lif_id, global_lif, fec_id, is_3_tags_data = 0;
    bcm_mpls_port_t mpls_port;
    bcm_vlan_port_t vlan_port;
    uint32 soc_sand_rv;
    int rv = BCM_E_NONE;
    ...
    */
    ...
    rv = _bcm_dpp_gport_to_global_and_local_lif(unit,port_id, &global_lif, &local_in_lif_id, &local_out_lif_id, &fec_id, &is_local);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    /* Get LIF type */
    if (is_egress) {
        BCMDNX_IF_ERR_EXIT(_bcm_dpp_lif_usage_get(unit, -1, local_out_lif_id, NULL, &lif_usage));
    }
    else if (is_ingress) {
    ...
    if ((BCM_GPORT_IS_VLAN_PORT(port_id) && ((!remote_lif) && is_egress && 
                        ((fec_id == -1) || (gport_type != _BCM_DPP_GPORT_TYPE_FEC) || (SOC_DPP_ENCAP_TYPE_GET(BCM_GPORT_VLAN_PORT_ID_GET(port_id)) == _DPP_ENCAP_ID_FORWARD_VAL)))) ||
        (BCM_GPORT_IS_MPLS_PORT(port_id) && ((!remote_lif && is_ingress && (fec_id == -1  || (gport_type != _BCM_DPP_GPORT_TYPE_FEC))) || (is_ingress && !is_egress))))  {   
    ...
    if (!is_local) {
        ...
        if (_Bcm_dpp_sync_all && !remote_lif)  {
            if (BCM_GPORT_IS_VLAN_PORT(port_id)) {
                if (SOC_IS_JERICHO(unit)) {
                    rv = _bcm_dpp_gport_delete_global_and_local_lif(unit, global_lif, local_in_lif_id, local_out_lif_id, FALSE);
                    BCMDNX_IF_ERR_EXIT(rv);
                    ...
                    if (is_ingress) {
                        /* Deallocate ingress if it's an ingress object */
                        if (bcm_dpp_am_l2_ac_is_alloced(unit, local_in_lif_id) == BCM_E_EXISTS) {
                            rv = bcm_dpp_am_l2_ac_dealloc(unit, _BCM_DPP_AM_L2_AC_TYPE_VLAN_EDITING, local_in_lif_id);
                            BCMDNX_IF_ERR_EXIT(rv);
                        }
                    }
                    ...
                    if (is_egress) {
                        ...
                        if (bcm_dpp_am_out_ac_is_alloced(unit, local_out_lif_id) == BCM_E_EXISTS) {
                            rv = bcm_dpp_am_out_ac_dealloc(unit, _BCM_DPP_AM_OUT_AC_TYPE_DEFAULT, 0, local_out_lif_id);
                            BCMDNX_IF_ERR_EXIT(rv);
                        }
                    }
                }
                ...
                if (is_ingress) {
                    rv = _bcm_dpp_in_lif_ac_match_get(unit, &vlan_port, local_in_lif_id);
                    BCMDNX_IF_ERR_EXIT(rv);
                    ...
                    rv = _bcm_dpp_in_lif_match_delete(unit, local_in_lif_id);
                    BCMDNX_IF_ERR_EXIT(rv);

                    rv = _bcm_dpp_in_lif_match_vsi_set(unit, local_in_lif_id, 0);
                    BCMDNX_IF_ERR_EXIT(rv);
                }
                ...
                if (is_egress) {
                    ...
                    BCMDNX_IF_ERR_EXIT(_bcm_dpp_out_lif_match_delete(unit, local_out_lif_id));
                }
    ...
    if (BCM_GPORT_IS_MPLS_PORT(port_id)){
        if (!remote_lif && ((is_local) || (_Bcm_dpp_sync_all))) {
            if (is_ingress) {
                ...
                if (removed_lif == local_in_lif_id) {
                    ...
                    removed_lif = (uint32)local_in_lif_id;
            ...
            if (SOC_IS_JERICHO(unit)) {
                rv = _bcm_dpp_gport_delete_global_and_local_lif(unit, global_lif, local_in_lif_id, local_out_lif_id, TRUE);
                BCMDNX_IF_ERR_EXIT(rv);

            } else if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
                if (is_egress && is_ingress) {
                    rv = bcm_dpp_am_l2_vpn_pwe_eep_dealloc(unit, local_out_lif_id);
                    BCMDNX_IF_ERR_EXIT(rv);

                    rv = bcm_dpp_am_l2_vpn_pwe_lif_dealloc(unit, local_in_lif_id);
                    BCMDNX_IF_ERR_EXIT(rv);
                }
                else if (is_egress && !is_ingress) {
                    rv = bcm_dpp_am_mpls_eep_dealloc(unit, local_out_lif_id);
                    BCMDNX_IF_ERR_EXIT(rv);
                }
                else if (!is_egress && is_ingress) {
            ...
            if (is_egress) {
                /* Remove entry from SW DB */
                rv = _bcm_dpp_out_lif_match_delete(unit, local_out_lif_id);
                BCMDNX_IF_ERR_EXIT(rv);
            }
        }
    }
        ...
        if (is_egress) { 
            ...
            if (SOC_IS_ARADPLUS(unit)) {
                soc_sand_rv = soc_ppd_eg_ac_info_get(unit, local_out_lif_id, &eg_ac_info_o);
                BCM_SAND_IF_ERR_EXIT(soc_sand_rv);           
            }
#endif
            eg_ac_info.edit_info.nof_tags = 2;
            ...
            if ((SOC_DPP_CONFIG(unit)->pp.pon_application_enable) && (BCM_E_EXISTS == bcm_dpp_am_out_ac_is_alloced(unit, local_out_lif_id))) {
                eedb_id_3_tags = local_in_lif_id;
        ...
        if (SOC_IS_JERICHO(unit)) {
            ...
            if (BCM_E_EXISTS == bcm_dpp_am_l2_ac_is_alloced(unit, local_in_lif_id)) {
                rv = bcm_dpp_am_l2_vpn_ac_dealloc(unit,_BCM_DPP_AM_L2_VPN_AC_TYPE_PON_3_TAGS_DATA, 0, local_in_lif_id);
                BCMDNX_IF_ERR_EXIT(rv);
            }
            ...
            if (BCM_E_EXISTS == bcm_dpp_am_out_ac_is_alloced(unit, local_out_lif_id)) {
                rv = bcm_dpp_am_out_ac_dealloc(unit, _BCM_DPP_AM_OUT_AC_TYPE_BIG_OUT_AC, 0, local_out_lif_id);
                BCMDNX_IF_ERR_EXIT(rv);
            }
        }
#endif
    ...
    SOC_PPC_EG_ENCAP_PWE_INFO *old_pwe_info)
{
    ...
    int local_out_lif = 0, global_lif_id = 0, o_local_outlif = 0;
    int is_lag=0;
    ...
    if (is_gal) {
        rv = _bcm_dpp_gport_to_global_lif(unit, mpls_port->tunnel_id, &base_pwe_lif, NULL, NULL, NULL);
        BCMDNX_IF_ERR_EXIT(rv);
    }

    /* Find the protection type on port (if any) and verify it's legal. */
    rv = l2_interface_protection_validation(unit, mpls_port, BCM_GPORT_MPLS_PORT, TRUE, &is_1_to_1_protected,
    ...
    */
    if(encap_with_id) {
        ...
        rv = l2_interface_get_lif_from_port_id(mpls_port, is_1_to_1_protected, is_mc_ingress, (SOC_PPD_LIF_ID*) &global_lif_id, BCM_GPORT_MPLS_PORT);
        BCMDNX_IF_ERR_EXIT(rv);
        ...
        /* Usually, the given mpls_port_id doesn't take into consideration the type of mpls port used (fec, mc or lif). This is
           here so we find the relevant type */
        rv = l2_interface_calculate_port_id(unit, fec_id, global_lif_id, mpls_port, BCM_GPORT_MPLS_PORT);
        BCMDNX_IF_ERR_EXIT(rv);
    }
        ...
        */
        ...
        if (!remote_lif) {
            if(!update) {
                ...
                if(((mp) || (is_1_to_1_protected) || (is_1_plus_1) || (is_egress_protection) || (!_BCM_PETRA_L3_ITF_IS_FEC(mpls_port->egress_tunnel_if))) && is_egress){
                ...
                if (SOC_IS_JERICHO(unit)) {
                    ...
                    /* If egress lif is a direct lif, don't allocate it as a global lif */
                    global_lif_flags |= ((is_ingress) ? BCM_DPP_AM_FLAG_ALLOC_INGRESS : 0)
                                        | ((allocate_egress && is_mapped_lif) ? BCM_DPP_AM_FLAG_ALLOC_EGRESS : 0);
                    rv =  bcm_dpp_am_global_lif_alloc(unit, global_lif_flags, &global_lif_id);
                    BCMDNX_IF_ERR_EXIT(rv);
                ...
                if (SOC_IS_JERICHO(unit)) {
                    rv = _bcm_dpp_gport_to_global_and_local_lif(unit, mpls_port->mpls_port_id, &global_lif_id, (int*)&local_in_lif, &local_out_lif, NULL, NULL);
                    BCMDNX_IF_ERR_EXIT(rv);
                } else if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
                    local_in_lif = local_out_lif = global_lif_id;
                }
        ...
        if (mpls_port->egress_label.label != 0) {
            ...
            if(!remote_lif && (is_local_conf /* local */ || (!_BCM_PPD_IS_VALID_FAILOVER_ID(mpls_port->failover_id)))) /* Tunnel protected */ {
                ...
                if (update) {
                    ...
                    int dummy_global_lif = 0;
                    /* o_encap_id = BCM_ENCAP_ID_GET(mpls_port->encap_id);*/
                    rv = _bcm_dpp_gport_to_global_and_local_lif(unit, mpls_port->mpls_port_id, &dummy_global_lif, NULL, &o_local_outlif, NULL, NULL);
                    BCM_IF_ERROR_RETURN(rv);
                    /* If outlif is not allocated for this mpls port, fetch push profile from SW DB */
                    if (o_local_outlif == _BCM_GPORT_ENCAP_ID_LIF_INVALID) {
                        rv = _bcm_dpp_sw_db_hash_vlan_find(unit,
                        ...
                        if (temp_phy_port->type == _BCM_DPP_GPORT_IN_TYPE_FEC_VC) {
                                /* In case of FEC_VC push profile is allocated */
    ...
    if(is_forward_group) {
        ...
        if(!update) {
            if (with_id) {
                rv = l2_interface_get_lif_from_port_id(mpls_port, is_1_to_1_protected, is_mc_ingress, &fec_id, BCM_GPORT_MPLS_PORT);
                BCMDNX_IF_ERR_EXIT(rv);
    ...
    */ 
    ...
    if(eep_required == 1 && !remote_lif && is_egress) {
        rv = _bcm_dpp_mpls_port_pwe_eep_set(unit, mpls_port, local_out_lif,
                                            push_profile, is_local_conf, old_pwe_info, is_egress_protection);
        BCMDNX_IF_ERR_EXIT(rv);

        /* Map global lif to this out local lif */
        if (SOC_IS_JERICHO(unit)) {
   ...
   _bcm_lif_type_e used_type;
    ...
    if (update) {
        if (!with_id) {
            ...
            /* Check whether outlif is allocated for this mpls port*/
            rv = _bcm_dpp_gport_to_global_and_local_lif(unit, mpls_port_o.mpls_port_id, &dummy_global_lif, NULL, &o_local_out_lif, NULL, NULL);
            BCM_IF_ERROR_RETURN(rv);

            /* Fetch outlif only if it is allocated for this mpls port */
            if (o_local_out_lif != _BCM_GPORT_ENCAP_ID_LIF_INVALID) {
                soc_sand_rv = soc_ppd_eg_encap_entry_get(unit,
                                     SOC_PPC_EG_ENCAP_EEP_TYPE_LIF_EEP, o_local_out_lif, 0,
