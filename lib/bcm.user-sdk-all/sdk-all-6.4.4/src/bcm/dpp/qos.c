...
bcm_petra_qos_init(int unit)
{
    ...
    if (SOC_IS_ARADPLUS(unit)) {
        ...
        _DPP_QOS_ALLOC(SOC_DPP_STATE(unit)->qos_state->egr_dscp_exp_marking_bitmap, 
            SHR_BITALLOCSIZE((_BCM_QOS_MAP_EGR_RESOLVED_DP_MAX * _BCM_QOS_MAP_TC_MAX * SOC_OCC_MGMT_NOF_INLIF_PROFILES_TRANSFERED_TO_EGRESS) * 
                          ...
                          int map_id)
{
    ...
    rv = _bcm_dpp_gport_to_global_and_local_lif(unit, gport, NULL, &lif_id, NULL, NULL, &is_local_conf);
    BCMDNX_IF_ERR_EXIT(rv);
...
_bcm_petra_qos_ingress_gport_map_validate(int unit, bcm_gport_t gport, int ing_map)
{
    ...
    int is_local_conf;
    uint32 soc_sand_rv;
    int global_lif_id = 0, ingress_lif_id = 0;
    int profile_ndx;
    ...
    rv = _bcm_dpp_gport_to_global_and_local_lif(unit, gport, &global_lif_id, &ingress_lif_id, NULL, NULL, &is_local_conf);
    BCMDNX_IF_ERR_EXIT(rv);

    if(is_local_conf) {
        soc_sand_rv = soc_ppd_lif_table_entry_get(unit, ingress_lif_id, &lif_entry_info);
        SOC_SAND_IF_ERR_RETURN(soc_sand_rv);

        if (lif_entry_info.type == SOC_PPD_LIF_ENTRY_TYPE_PWE) {
            rv = _bcm_dpp_in_lif_match_vsi_get(unit, ingress_lif_id, &vsi);
            BCMDNX_IF_ERR_EXIT(rv);
...
_bcm_petra_qos_egress_gport_map_set(int unit, bcm_gport_t gport, int egr_map)
{
    ...
    int global_lif_id, local_lif_id = 0;
    int is_local_conf;
    int port_i;
    int tunnel_id, core;
    int is_uniform;
    ...
    if (BCM_INT_QOS_MAP_IS_REMARK(egr_map)) {
        ...
        if (BCM_INT_QOS_MAP_IS_EGRESS_PCP_VLAN(egr_map)) {
            if (profile_ndx >= SOC_DPP_CONFIG(unit)->qos.nof_egr_pcp_vlan) {
                ...
                /* map gport to used LIF */
                rv = _bcm_dpp_gport_to_global_and_local_lif(unit, gport, &global_lif_id, NULL, &local_lif_id, NULL, &is_local_conf);
                BCMDNX_IF_ERR_EXIT(rv);

                if(is_local_conf) {
                    SOC_SAND_IF_ERROR_RETURN(soc_ppd_eg_ac_info_get(unit, local_lif_id, &eg_ac_info));
            
                    eg_ac_info.edit_info.pcp_profile = profile_ndx;
            
                    SOC_SAND_IF_ERROR_RETURN(soc_ppd_eg_ac_info_set(unit, local_lif_id, &eg_ac_info));
                }
            }
        } 
        else {
    ...
    if (BCM_GPORT_IS_LOCAL(gport) || BCM_GPORT_IS_SYSTEM_PORT(gport) || BCM_GPORT_IS_MODPORT(gport) || (SOC_PORT_VALID(unit,gport))) {
      ...
      if (flags & BCM_QOS_MAP_EGRESS) {
        ...
        if (BCM_GPORT_IS_TUNNEL(gport)) {
            ...
            /* map gport to used LIF */
            rv = _bcm_dpp_gport_to_global_and_local_lif(unit, gport, NULL, NULL, &lif_id, NULL, &is_local_conf);
            BCMDNX_IF_ERR_EXIT(rv);
    ...
    if (BCM_INT_QOS_MAP_IS_EGRESS_PCP_VLAN(egr_map)) {
        ...
        if (profile_ndx >= SOC_DPP_CONFIG(unit)->qos.nof_egr_pcp_vlan) {
            ...
            BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_i) {
                ...
                rv = _bcm_dpp_qos_egress_port_vlan_map_ac_key_to_local_outlif(unit, port, vid, &ac_key, &lif_id);
                BCMDNX_IF_ERR_EXIT(rv);
