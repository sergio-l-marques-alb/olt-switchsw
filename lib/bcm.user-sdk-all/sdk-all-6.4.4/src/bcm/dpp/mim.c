    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        if (!SOC_DPP_IS_VLAN_TRANSLATE_MODE_ADVANCED(unit)) { /* Standard vlan translate mode has a default ac for MIM, while AVT mode doesn't */
            ...
            if (SOC_IS_JERICHO(unit)) {
                ...
                rv =  bcm_dpp_am_global_lif_alloc(unit, BCM_DPP_AM_FLAG_ALLOC_INGRESS | SHR_RES_ALLOC_WITH_ID, (int*)&SOC_DPP_CONFIG(unit)->pp.mim_global_lif_ndx);
                BCMDNX_IF_ERR_EXIT(rv);

                /* Allocate the local ingress lif */
                local_inlif_info.glif = SOC_DPP_CONFIG(unit)->pp.mim_global_lif_ndx;
                ...
                rv = _bcm_dpp_am_local_inlif_alloc(unit, lif_flags, &local_inlif_info);
                BCMDNX_IF_ERR_EXIT(rv);

                SOC_DPP_CONFIG(unit)->pp.mim_local_lif_ndx = local_inlif_info.base_lif_id;
                ...
                /* Check if mim default out lif is mapped or direct.
                 * If it's a direct lif, don't allocate it as a global lif.
                 */
                rv = _bcm_dpp_lif_id_is_mapped(unit, FALSE, SOC_DPP_CONFIG(unit)->pp.mim_global_out_ac, &is_mapped_lif);
                BCMDNX_IF_ERR_EXIT(rv);

                if (is_mapped_lif) {
                    /* Allocate the egress mim global lif */
                    rv =  bcm_dpp_am_global_lif_alloc(unit, BCM_DPP_AM_FLAG_ALLOC_EGRESS | SHR_RES_ALLOC_WITH_ID, (int*)&SOC_DPP_CONFIG(unit)->pp.mim_global_out_ac);
                    BCMDNX_IF_ERR_EXIT(rv);
                }


                /* Allocate the egress local lif */
                if (!is_mapped_lif) {
                    local_out_lif_info.base_lif_id = SOC_DPP_CONFIG(unit)->pp.mim_global_out_ac;
                ...
                local_out_lif_info.glif = SOC_DPP_CONFIG(unit)->pp.mim_global_out_ac;
                local_out_lif_info.local_lif_flags = (is_mapped_lif) ? BCM_DPP_AM_OUT_LIF_FLAG_COUPLED : BCM_DPP_AM_OUT_LIF_FLAG_DIRECT;;

                rv = _bcm_dpp_am_local_outlif_alloc(unit, lif_flags, &local_out_lif_info);
                BCMDNX_IF_ERR_EXIT(rv);

                SOC_DPP_CONFIG(unit)->pp.mim_local_out_ac = local_out_lif_info.base_lif_id;
            }
            if (SOC_IS_ARADPLUS_AND_BELOW(unit) || SOC_IS_PETRAB(unit)) {
                ...
                SOC_DPP_CONFIG(unit)->pp.mim_local_lif_ndx = SOC_DPP_CONFIG(unit)->pp.mim_global_lif_ndx;
                ...
                SOC_DPP_CONFIG(unit)->pp.mim_local_out_ac = SOC_DPP_CONFIG(unit)->pp.mim_global_out_ac;
            ...
            if (SOC_IS_ARAD(unit)) {
                ...
                soc_sand_rv = soc_ppd_eg_ac_info_set(soc_sand_dev_id, SOC_DPP_CONFIG(unit)->pp.mim_local_out_ac, &ac_info);
                BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
            }

            /* Add out lif mapping to GLEM */
            if (SOC_IS_JERICHO(unit) && is_mapped_lif) {
                rv = MBCM_PP_DRIVER_CALL(unit, mbcm_pp_lif_glem_access_entry_add, (unit, SOC_DPP_CONFIG(unit)->pp.mim_global_out_ac, SOC_DPP_CONFIG(unit)->pp.mim_local_out_ac));
                BCMDNX_IF_ERR_EXIT(rv);
            }
        ...
        /* write mim_out_ac to default_sem_index reg */
        if (SOC_IS_PETRAB(unit)) {
            soc_sand_rv = soc_pb_pp_frwrd_bmact_default_sem_index_set(soc_sand_dev_id, SOC_DPP_CONFIG(unit)->pp.mim_local_out_ac);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        }
#endif
...
/* coverity[uninit_use_in_call] */
        ...
        if (!update) {
            if (SOC_IS_JERICHO(unit)) {
                ...
                rv =  bcm_dpp_am_global_lif_alloc(unit, BCM_DPP_AM_FLAG_ALLOC_INGRESS | l2_ac_alloc_flags, &isid_global_lif_id);
                BCMDNX_IF_ERR_EXIT(rv);

                /* Allocate the local ingress lif */
                sal_memset(&local_inlif_info, 0, sizeof(local_inlif_info));

                local_inlif_info.glif = isid_global_lif_id;
                local_inlif_info.local_lif_flags = BCM_DPP_AM_IN_LIF_FLAG_COMMON;
    ...
    bcm_mim_port_t * mim_port)
{
    ...
    uint32 fec_flags = 0, eep_flags = 0, lif_flags = 0, local_lif_flags = 0;
    ...
    if (!update) {
        ...
        if (SOC_IS_JERICHO(unit)) {
            ...
            /* Check if the given encap id is mapped or direct.
             * If it's a direct lif, don't allocate it as a global lif.
             */
            if (mim_port->flags & BCM_MIM_PORT_ENCAP_WITH_ID) {
                rv = _bcm_dpp_lif_id_is_mapped(unit, FALSE, global_out_lif_id, &is_mapped_lif);
                BCMDNX_IF_ERR_EXIT(rv);
            }

            if (is_mapped_lif) {
                /* Allocate the egress global lif */
                rv =  bcm_dpp_am_global_lif_alloc(unit, BCM_DPP_AM_FLAG_ALLOC_EGRESS | eep_flags, &global_out_lif_id);
                BCMDNX_IF_ERR_EXIT(rv);
            }
    ...
    /* Add out lif mapping to GLEM */
    if (SOC_IS_JERICHO(unit) && is_mapped_lif && !update) {
        rv = MBCM_PP_DRIVER_CALL(unit, mbcm_pp_lif_glem_access_entry_add, (unit, global_out_lif_id, local_out_lif_id));
        BCMDNX_IF_ERR_EXIT(rv);
    }
    ...
    if (vpn == BCM_PETRA_MIM_VPN_INVALID) { /* P2P */
        ...
        if (!update) {
            ...
            if (SOC_IS_JERICHO(unit)) {
                ...
                /* Allocate the ingress mim global lif */
                rv =  bcm_dpp_am_global_lif_alloc(unit, BCM_DPP_AM_FLAG_ALLOC_INGRESS | lif_flags, &global_in_lif_id);
                BCMDNX_IF_ERR_EXIT(rv);

                /* Allocate the local ingress lif */
                sal_memset(&local_inlif_info, 0, sizeof(local_inlif_info));

                local_inlif_info.glif = global_in_lif_id;
                local_inlif_info.local_lif_flags = BCM_DPP_AM_IN_LIF_FLAG_COMMON;
                local_inlif_info.app_type = bcm_dpp_am_ingress_lif_app_ingress_isid;
