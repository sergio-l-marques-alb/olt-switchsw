...
/*
 * Convert local ports bitmap to a SOC_TMC PP ports bitmap.
 */
int
_bcm_petra_multicast_local_ports_to_mult_vlan_pbmp(
    int unit,
    const bcm_pbmp_t *pbmp,                              /* input: local ports bitmap */
    SOC_TMC_MULT_EG_VLAN_MEMBERSHIP_GROUP *bitmap_group) /* output: PP ports bitmap */
{
    int i;
...
{
    uint32 nof_mc_ids = is_egress ? SOC_DPP_CONFIG(unit)->tm.nof_mc_ids : (SOC_IS_JERICHO(unit) ?
      SOC_DPP_CONFIG(unit)->tm.nof_ingr_mc_ids - 1 : SOC_DPP_CONFIG(unit)->tm.nof_ingr_mc_ids); /* Jericho reserved one ingress group for workaround */
    BCMDNX_INIT_FUNC_DEFS;
...
_bcm_petra_multicast_egress_encode_replication(int unit, uint32 *out_port, uint32 *out_cud, bcm_port_t local_port,bcm_if_t encap_id)
{
    ...
    bcm_port_t max_local_port = SOC_IS_JERICHO(unit) ? SOC_TMC_NOF_FAP_PORTS_JERICHO - 1 /*511*/ :
...
/*
 * Add/remove a replication to/from an egress multicast linked list group.
 */
...
int _bcm_petra_multicast_egress_vlan_membership_port_add_or_remove(
    int unit,                               /* input: unit/device */
    SOC_TMC_MULT_ID multicast_id_ndx,       /* input: egress MC group to set */
    bcm_port_t local_port,                  /* input: replicaiton port */
    bcm_if_t encap_id,                      /* input: replication CUD */
    uint8 is_delete)                        /* input: if non zero this is a delete operation, otherwise an add */
{
    SOC_TMC_ERROR mc_err = BCM_E_NONE;
    mbcm_dpp_mult_eg_vlan_membership_port_add_f soc_func = is_delete ? dpp_mult_eg_bitmap_group_port_remove : dpp_mult_eg_bitmap_group_port_add;
    BCMDNX_INIT_FUNC_DEFS;
    BCMDNX_IF_ERR_EXIT(soc_func(unit, multicast_id_ndx, local_port, &mc_err));
    if (mc_err) {
    ...
    uint8 is_delete)                        /* input: if non zero this is a delete operation, otherwise an add */
{
    ...
    if (!is_in_range && rep_array->port == BCM_GPORT_INVALID ) { /* outlif only replication (port mapped from outlif) */
        if (!rep_array->encap1) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("outlif only replication can not use outlif 0")));
        }
        BCMDNX_IF_ERR_EXIT(_bcm_petra_multicast_egress_entry_port_add_remove(unit, multicast_id_ndx, BCM_GPORT_INVALID, rep_array->encap1, is_delete));
    } else if (!is_in_range && BCM_GPORT_IS_MCAST(rep_array->port)) { /* bitmap replication */
        if (!_SHR_GPORT_MCAST_GET(rep_array->port)) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("bitmap replication can not use bitmap 0")));
        }
        BCMDNX_IF_ERR_EXIT(_bcm_petra_multicast_egress_entry_port_add_remove(unit, multicast_id_ndx, BCM_GPORT_MCAST_GET(rep_array->port) | ARAD_MC_EGR_IS_BITMAP_BIT, rep_array->encap1, is_delete));
    } else {
        /* Derive local ports */
        uint32 local_port;
        _bcm_dpp_gport_info_t gport_info;

        BCMDNX_IF_ERR_EXIT(_bcm_dpp_gport_to_phy_port(unit, rep_array->port, _BCM_DPP_GPORT_TO_PHY_OP_LOCAL_IS_MANDATORY, &gport_info)); 

        BCM_PBMP_ITER(gport_info.pbmp_local_ports,local_port) { /* add/remove ports in bitmap */
            if (is_in_range) {
               BCMDNX_IF_ERR_EXIT(_bcm_petra_multicast_egress_vlan_membership_port_add_or_remove(unit, multicast_id_ndx, local_port, rep_array->encap1, is_delete));
            } else {
               BCMDNX_IF_ERR_EXIT(_bcm_petra_multicast_egress_entry_port_add_remove(unit, multicast_id_ndx, local_port ,rep_array->encap1, is_delete));
            }
        }
    }
    ...
    uint8 is_delete)        /* input: if non zero this is a delete operation, otherwise an add */
{
    ...
    if (!SOC_IS_PETRAB(unit)) { /* non Petra B */
        bcm_multicast_replication_t rep = {0};
        rep.port = port;
        rep.encap1 = encap_id;
        BCMDNX_IF_ERR_EXIT(_bcm_arad_multicast_egress_add_or_delete(unit, group, 1, &rep, is_delete));
        BCM_EXIT;
    }

    /* Check mc group type is supported */
    BCMDNX_IF_ERR_EXIT(_bcm_petra_multicast_is_supported_mc_group(group));
    BCMDNX_IF_ERR_EXIT(_bcm_petra_multicast_is_group_exist(unit, BCM_DPP_MULTICAST_TYPE_EGRESS_ONLY, multicast_id_ndx, SOC_SAND_TRUE));
    BCMDNX_IF_ERR_EXIT(_bcm_petra_multicast_egress_vlan_membership_range_id_check(unit, multicast_id_ndx, &is_in_range)); /* check if this is a vlan group */
    ...
    BCM_PBMP_ITER(gport_info.pbmp_local_ports, local_port) { /* add/remove ports in bitmap */
        if (is_in_range) {
            BCMDNX_IF_ERR_EXIT(_bcm_petra_multicast_egress_vlan_membership_port_add_or_remove(unit, multicast_id_ndx, local_port, encap_id, is_delete));
        } else {
            BCMDNX_IF_ERR_EXIT(_bcm_petra_multicast_egress_entry_port_add_remove(unit, multicast_id_ndx, local_port, encap_id, is_delete));
        }
    }
    ...
    int *rep_count)                          /* output: number of entries/replications in the group or rep_max */
{
    ...
    if (is_in_range) { /* this is a bitmap */
        ...
        if (rep_max == 0 && port_array == NULL && encap_id_array == NULL) {
            BCM_PBMP_COUNT(l2_pbmp, nof_reps);
        }
        
    } else { /* group is not a bitmap */

        BCMDNX_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_mult_eg_get_group,(unit, multicast_id_ndx, rep_max, port_array, encap_id_array, &group_size, &is_open))); /* do the actual work */
        if (!is_open) {
    ...
    int *port_count)
{
    ...
    BCMDNX_IF_ERR_EXIT(_bcm_petra_multicast_is_group_exist(
      unit, BCM_DPP_MULTICAST_TYPE_EGRESS_ONLY, multicast_id_ndx, SOC_SAND_TRUE)); /* Verify that the multicast ID (group) exists */
    BCMDNX_IF_ERR_EXIT(_bcm_petra_multicast_egress_vlan_membership_range_id_check(unit, multicast_id_ndx, &is_in_range)); /* check if this is a vlan group */
...
int _bcm_arad_multicast_set_egress(
    ...
    bcm_multicast_replication_t *rep_array) /* input: replication array */
{
    uint32 local_port, max_reps = DPP_MULT_MAX_EGRESS_REPLICATIONS;
    SOC_TMC_MULT_ID mcid;
    int ndx, port_local_arr_ndx = 0;
    int is_arad_mechanism = 0, is_bitmap_group;
    bcm_pbmp_t pbmp;
    uint8 is_in_range, open;
    ...
    if (!is_in_range) {
        ...
        BCMDNX_ALLOC(arad_local_port_arr, sizeof(uint32) * max_reps, "local ports from gports");
        BCMDNX_ALLOC(encap_id_arr, sizeof(bcm_if_t) * max_reps, "encap ids");
    ...
    for (ndx = 0; ndx < rep_count ; ndx++) {
        if (port_local_arr_ndx >= max_reps) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Too many replications in input")));
        } else if (is_arad_mechanism == 1 && port_array[ndx] == BCM_GPORT_INVALID) { /* outlif only replication (port mapped from outlif) */
    ...
    if (is_in_range == TRUE) {
        BCMDNX_IF_ERR_EXIT(_bcm_petra_multicast_egress_vlan_membership_set(unit, mcid, pbmp));
    } else {
        SOC_TMC_ERROR mc_err = BCM_E_NONE;
        ...
        BCMDNX_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_mult_eg_group_set,
            (unit, mcid, 0, port_local_arr_ndx, arad_local_port_arr, encap_id_arr, &mc_err)));
        if (mc_err) {
            BCMDNX_ERR_EXIT_MSG(mc_err, (_BSL_BCM_MSG("%s"), (mc_err == (SOC_TMC_ERROR) BCM_E_FULL ?  "not enough multicast entries for the operation" :
    ...
    bcm_if_t *encap_id)
{
    ...
    if(encap_type == _BCM_DPP_GPORT_IN_TYPE_FEC_VC) {
        rv = _bcm_dpp_gport_to_global_lif(unit, mpls_port_id, &lif_id, NULL, NULL, &is_local);
        *encap_id = lif_id;
    }
    ...
    SOC_TMC_DEST_INFO *out_dest) /* output: destination */
{
    ...
    /* In Arad, a CUD of 0 for a destination which is not an ERP system port or FMQ needs to be changed */
    if (SOC_IS_ARAD(unit) && !*inout_cud && !BCM_GPORT_IS_MCAST_QUEUE_GROUP(in_dest)) {
    ...
    int *rep_count)
{
    ...
    if (reps_array_size > BCM_DPP_MULTICAST_SMALL_ARRAY_SIZE) {
        BCMDNX_IF_ERR_EXIT(dcmn_alloc_mem(unit, &dest_arr, sizeof(soc_gport_t) * reps_array_size, "destinations")); /* Allocate dests */
        BCMDNX_IF_ERR_EXIT(dcmn_alloc_mem(unit, &cud_array, sizeof(bcm_if_t) * reps_array_size, "CUDs")); /* Allocate CUD array */
    } else {
        dest_arr = small_dest_array;
        cud_array = small_cud_array;
    }

    BCMDNX_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_mult_ing_get_group, (unit, mcid, reps_array_size, dest_arr, cud_array, &group_size, &open))); /* do the actual work */
    if (!open) {
...
exit:
    if (dest_arr && dest_arr != small_dest_array) {
        SOCDNX_IF_ERR_EXIT(dcmn_free_mem(unit, (void*)&dest_arr));
    }
    if (cud_array && cud_array != small_cud_array) {
        SOCDNX_IF_ERR_EXIT(dcmn_free_mem(unit, (void*)&cud_array));
    }
    BCMDNX_FUNC_RETURN;
}
    ...
    int *rep_count)
{
    ...
    if (!open) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_NOT_FOUND, (_BSL_BCM_MSG("%gress multicast group %u was not created"), is_egress ? "E" : "In", multicast_id_ndx));
    }
    ...
    bcm_multicast_replication_t *rep_array)
{
    ...
    if (nof_replications > BCM_DPP_MULTICAST_SMALL_ARRAY_SIZE) {
        BCMDNX_IF_ERR_EXIT(dcmn_alloc_mem(unit, &dest_info_arr, sizeof(SOC_TMC_DEST_INFO) * nof_replications,
          "dest info entries")); /* Allocate dest info entries */
        BCMDNX_IF_ERR_EXIT(dcmn_alloc_mem(unit, &cud_array, sizeof(bcm_if_t) * nof_replications, "CUDs")); /* Allocate CUD array */
    } else {
...
exit:
    if (dest_info_arr && dest_info_arr != small_dests_array) {
        SOCDNX_IF_ERR_EXIT(dcmn_free_mem(unit, (void*)&dest_info_arr));
    }
    if (cud_array && cud_array != small_cud_array) {
        SOCDNX_IF_ERR_EXIT(dcmn_free_mem(unit, (void*)&cud_array));
    }
    ...
    bcm_multicast_replication_t *rep_array)
{
    ...
    BCM_DPP_UNIT_CHECK(unit);                      
    if (SOC_IS_PETRAB(unit)) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("The API is not supported for Petra")));
    } else if (nof_replications > 0) {
        BCMDNX_NULL_CHECK(rep_array);
    }
