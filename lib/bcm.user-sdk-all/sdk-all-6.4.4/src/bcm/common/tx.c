        ...
        if (SOC_DCB_TYPE(unit) == 23) {
            ...
            if ((soc_feature(unit, soc_feature_visibility)) && 
               ...
               if (loopback_port == -1) {
                ...
                PBS_MH_V9_W2_SMOD_COS_SET(pbh, src_mod, 0, pkt->cos);
                PBS_MH_V9_W2_L2BM_SET(pbh, 1);              
            } 
            else 
            {
                PBS_MH_V9_W2_SMOD_COS_SET(pbh, src_mod, 1, pkt->cos);
            }
    } else {
...
_bcm_tx(int unit, bcm_pkt_t *pkt, void *cookie)
{
    ...
    if ((dv->dv_vcnt > 0) || (SOC_IS_ARAD(unit))) {
        ...
        /* Only free DV here if not continuous mode */
        if (SOC_DMA_MODE(unit) != SOC_DMA_MODE_CONTINUOUS) {
            if (dv != NULL) {
                _tx_dv_free(unit, dv);
            }
...
_bcm_tx_chain_send(int unit, dv_t *dv, int async)
{
    ...
    if (async) { /* Send packet(s) asynchronously */
        ...
        if (SOC_DMA_MODE(unit) == SOC_DMA_MODE_CONTINUOUS) {
            _bcm_tx_desc_done_cb(unit, dv, null_dcb);
        } else {
...
int _bcm_olp_l2_hdr_get(int unit,int dglp,soc_olp_l2_hdr_t *l2hdr)
{
    int rv = BCM_E_NONE;
    uint64 rval;
    uint64 mac_field;
    BCM_IF_ERROR_RETURN(_bcm_olp_port_mac_get(
                        unit, CMIC_PORT(unit), (bcm_mac_t *)(l2hdr->src_mac)));
...
int _bcm_tx_olp_hdr_fill(int unit, bcm_pkt_t *pkt,soc_olp_tx_hdr_t *otxhdr)
{
    ...
    if((pkt->flags2 & BCM_PKT_F2_REPLACEMENT_OFFSET)) {
        ...
        SOC_OLP_TX_OAM_OFFSET(otxhdr) = (pkt->oam_replacement_offset - _OLP_OAM_MIN_OFFSET) / 2;
    }
    /*Counter index*/
    if(pkt->oam_counter_size) {
        ...
        for(index = 0; index < max_index; index++) {
            _bcm_esw_stat_get_counter_id_info(
                    pkt->oam_counter[index].counter_object_id,
...
_tx_pkt_olp_desc_add(int unit, bcm_pkt_t *pkt, dv_t *dv, int pkt_idx)
{
    ...
    pkt_hg_hdr = NULL;
    _bcm_olp_l2_hdr_get(unit,pkt->dest_port,&(opkt.l2_hdr));
...
_tx_pkt_desc_add(int unit, bcm_pkt_t *pkt, dv_t *dv, int pkt_idx)
{
    ...
    if(SOC_IS_TRIDENT2PLUS(unit) && BCM_PKT_TX_OLP(pkt)) {
...
if ((soc_feature(unit, soc_feature_visibility)) && 
    (pkt->flags2 & BCM_PKT_F2_RX_PORT)) {
        ...
        uint16 pkt_trace_src_modid, pkt_trace_src_pp_port, pkt_trace_src;
        ...
        pkt_trace_src_modid   = BCM_GPORT_MODPORT_MODID_GET(visibility_source_gport);
        pkt_trace_src_pp_port = BCM_GPORT_MODPORT_PORT_GET(visibility_source_gport);
