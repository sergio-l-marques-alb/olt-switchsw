/* 
 * $Id: olp_pkt.h,v 1.0 Broadcom SDK $
...
#elif defined(BCM_SABER2_SUPPORT)
    union {
        uint32 h3;
        struct {
            uint32 _ctr3_id1:4,
...
#define SOC_OLP_RX_TIMESTAMP_GET(_orh)     \
    (((_orh)->u3.s3._rx_timestamp1 << 22) | ((_orh)->u4.s4._rx_timestamp2 ))
#define SOC_OLP_RX_OAM_PDU_OFFSET_GET(_orh)     \
    ((_orh)->u4.s4.oam_pdu )

#ifdef BCM_SABER2_SUPPORT
#define SOC_OLP_RX_OAM_SAMPLE_TYPE_EXT_GET(_orh)     \
    ((_orh)->u4.s4.sample_type_ext )
#define SOC_OLP_RX_SAMPLE_VALUE_EXT_GET(_orh)     \
    (((_orh)->u4.s4._sample_value_ext1 << 31) | ((_orh)->u5.s5._sample_value_ext2 ))
...
#define SOC_OLP_TX_CTR2_ID_GET(_oth)     \
    (((_oth)->u2.s2._ctr2_id1 << 10) | ((_oth)->u3.s3._ctr2_id2))
#define SOC_OLP_TX_CTR2_ACTION_GET(_oth)     \
    ((_oth)->u3.s3.ctr2_action )
#define SOC_OLP_TX_TIMESTAMP_ACTION_GET(_oth)     \
    ((_oth)->u3.s3.timestamp_action )
#define SOC_OLP_TX_OAM_OFFSET_GET(_oth)     \
    ((_oth)->u3.s3.oam_replacement_offset )
...
#define SOC_OLP_TX_CTR2_ID_SET(_oth,val)    {           \
    ...
    (_oth)->u3.s3._ctr2_id2 = (val) & (0x3ff);        \
    }

#ifdef BCM_TRIDENT2PLUS_SUPPORT     
#define SOC_OLP_TX_CLASSID_GET(_oth)     \
    (((_oth)->u3.s3._class_id1 << 6) | ((_oth)->u4.s4._class_id2))
...
#define SOC_OLP_TX_CTR3_ID(_oth)            SOC_OLP_TX_CTR3_ID_GET(_oth)
...
#define SOC_OLP_TX_CTR(pool,idx)    \
        ((((pool) << 12) & 0xe000) | ((base_index) & 0xfff))
