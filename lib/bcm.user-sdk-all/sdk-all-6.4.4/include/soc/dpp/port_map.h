...
{                                                      \
    if (SOC_IS_ARAD(unit))                             \
        _soc_dpp_wb_pp_port_use_set(unit, port, TRUE); \
    (soc_dpp_pp_ports[unit][port].in_use = TRUE);      \
}
#define SOC_DPP_PP_PORT_FREE(unit, port)               \
{                                                      \
    if (SOC_IS_ARAD(unit))                             \
        _soc_dpp_wb_pp_port_use_set(unit, port, FALSE);\
...
int _soc_dpp_wb_pp_port_use_set(int unit, soc_port_t port, uint8 in_use);
