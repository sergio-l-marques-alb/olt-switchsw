...
typedef struct
{
  ...
  ARAD_SW_DB_REASSBMEBLY_CTXT
    arad_sw_db_reassembly_ctxt;

  ARAD_SWDB_MULTICAST
    arad_sw_db_multicast;

  uint32
    q_type_ref_count[SOC_TMC_ITM_NOF_QT_NDXS];
  ...
  ARAD_SYSPORT modport2sysport[ARAD_NOF_MODPORT];
...
/*Get reference count for credit request profiles*/
uint32
  arad_sw_db_queue_type_ref_count_get(
    SOC_SAND_IN  int            unit,
    SOC_SAND_IN  uint8          q_type,
    SOC_SAND_OUT uint32*        ref_count);
/*Set reference count for credit request profiles*/
uint32
