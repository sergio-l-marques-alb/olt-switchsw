...
#define DPP_DEVICE_COSQ_TOTAL_FLOW_REGIONS(unit)                (SOC_IS_PETRAB(unit) ? DPP_DEVICE_PETRA_COSQ_TOTAL_FLOW_REGIONS:SOC_TMC_COSQ_TOTAL_FLOW_REGIONS)
...
typedef struct bcm_dpp_cosq_config_s {
    ...
    int                               default_se[SOC_DPP_DEFS_MAX(NOF_CORES)] ;
...
typedef enum {
  ...
  */
   ...
   * VSQ PFC (VSQE)
   */
  BCM_DPP_COSQ_VSQ_TYPE_PFC = _SHR_COSQ_VSQ_SRC_PORT,
  /*
   * VSQ LLFC (VSQF)
   */
  BCM_DPP_COSQ_VSQ_TYPE_LLFC = _SHR_COSQ_VSQ_PG,
  /*
   * Last entry
   */
...
int
_bcm_petra_cosq_queue_range_set(int unit, int core_id, int type, int configuration_type, SOC_SAND_U32_RANGE* queue_range);
