...
char if_dpp_phy_usage[] =
    ...
    "                       [Loopback=<true|false>] [Invert=<value>]\n"
...
_phy_portmod_sym_access(int u, args_t *a, int intermediate, soc_pbmp_t *pbm)
{
    ...
    DPORT_SOC_PBMP_ITER(u, *pbm, dport, p) {
        rv = portmod_port_phy_lane_access_get(u, p, &params, MAX_CORE_ACCESS_STRUCTURES_PER_PORT, phy_access, &nof_phys_structs);
        if (rv != CMD_OK) {
...
_phy_prbs_dcmn(int unit, args_t *args)
{
    ...
    bcm_pbmp_t          pbmp;
    int                 rv, cmd, enable, mode = 0, lb_mode = BCM_PORT_LOOPBACK_PHY;
...
_phy_diag_portmod_dsc(int unit, bcm_pbmp_t pbmp,args_t *args)
{
    ...
    DPORT_BCM_PBMP_ITER(unit, pbmp, dport, port) {
        rv = portmod_port_phy_lane_access_get(unit, port, &params, MAX_CORE_ACCESS_STRUCTURES_PER_PORT, phy_access, &nof_phys_structs);
        if(rv < 0 ){
            return CMD_FAIL;
        }
...
_phy_diag_dcmn(int unit, args_t *args)
{
    ...
    if ((port_str = ARG_GET(args)) == NULL) {
        cli_out("        diag: Subcommands:\n"
                ...
                "                   [time_upper_bound=<value>] [nof_threshold_links=<value>]\n"
    ...
    if (sal_strcasecmp(cmd_str, "dsc") == 0) {
        ...
        if(soc_feature(unit, soc_feature_portmod)){
            cli_out("The unit does not support phy diag eyescan\n"); 
            return CMD_FAIL;
        }
...
_phy_arad_nif_measure(int unit, bcm_port_t port, uint32 *type_of_bit_clk, int *one_clk_time_measured_int ,int *one_clk_time_measured_remainder, int *serdes_freq_int, int *serdes_freq_remainder, uint32 *lane) {
    ...
    uint32 reg_val, default_configuration;
    int counter;
...
STATIC void
   _phy_measure_ch0_calc(soc_reg_above_64_val_t reg_val, int *ch0_out_int, int *ch0_out_remainder) {
...
_phy_measure(int unit, args_t *args){
    ...
    bcm_pbmp_t          pbmp;
    bcm_stat_val_t type = snmpBcmRxAsynFifoRate;
    uint64 reg_val64;
    uint64 val, period;
    bcm_port_t port, dport, lane;
    uint32 rate, rate_int, rate_remainder;
    uint64 rate_start, rate_end;
    int rv, counter_disabled = 0;
    int clock_speed_int = 0, clock_speed_remainder = 0;
   ...
   soc_reg_above_64_val_t reg_val;
    ...
    if ((cmd_str = ARG_GET(args)) != NULL) {
        if (sal_strcasecmp(cmd_str, "tx") == 0) {
            type = snmpBcmTxAsynFifoRate;
        } else if (sal_strcasecmp(cmd_str, "rx") == 0) {
            type = snmpBcmRxAsynFifoRate;
        } else {
    ...
    DPORT_BCM_PBMP_ITER(unit, pbmp, dport, port) {
        ...
        if(SOC_IS_DFE(unit) || (SOC_IS_ARAD(unit) && IS_SFI_PORT(unit, port))){
            ...
            if (SOC_IS_DFE(unit) && !counter_disabled) {
                COMPILER_64_SUB_64(rate_end, rate_start);
            }
            rate = COMPILER_64_LO(rate_end);

            rate_int = (rate * 40 * 1000)/ (COMPILER_64_LO(period) * (clock_speed_int * 1000 + (clock_speed_remainder%1000)));
            rate_remainder = ((rate * 40 * 1000)/ ((COMPILER_64_LO(period)/1000) * (clock_speed_int * 1000 + (clock_speed_remainder%1000)))) % 1000;
            ...
            if (SOC_IS_ARAD(unit)) {
                ...
                _phy_measure_ch0_calc(reg_val, &ch0_out_int, &ch0_out_remainder);
            ...
            rv = _phy_arad_nif_measure(unit, port, &type_of_bit_clk, &one_clk_time_measured_int, &one_clk_time_measured_remainder,
                                  &serdes_freq_int, &serdes_freq_remainder, &port_lane);
            if(BCM_E_NONE != rv) {
                return CMD_FAIL;
            }
