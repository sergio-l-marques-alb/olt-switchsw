...
STATIC int diag_print_rejected(int unit) {
   int                 f;
...
int jericho_counters_graphic_print(int unit, uint32 print_none_zero, uint32 hex_base, uint32 core_disp) {
    ...
    print_counter_val_if_neccesary(unit, "|  %-30s %c %-24s            ", "RCY_PACKET_COUNTERr", frst_cntbuf);
    ...
    print_counter_val_if_neccesary(unit, "|  %-30s %c %-24s            ->", "EGQ_0_PKT_CNT", third_cntbuf);
    ...
    print_counter_val_if_neccesary(unit, "|  %-30s %c %-24s            ", "IPT_DESC_CELL_COUNTERr", frst_cntbuf);
    ...
    print_counter_val_if_neccesary(unit, "|  %-30s %c %-24s            ", "IRE_DESC_CELL_COUNTERr", frst_cntbuf);
    print_counter_val_if_neccesary(unit, "|  %-30s %c %-24s            |\n", "P1_CP2_CELL_IN_CNTELL_IN_CNT", sec_cntbuf);
...
int arad_packet_flow_counter_get(int unit, uint32 print_none_zero, uint32 hex_base) {
   ...
   counter_OVF = get_field(counter_MSB, 0, 0);
#ifdef BCM_88660_A0
   if (counter_LSB) print_reason=1; /* if counter is on, we'll want to print the reasons */
#endif
...
#ifdef BCM_88660_A0
   if (print_reason && SOC_IS_ARADPLUS(unit)) diag_print_pqp_discard_reasons(unit); /* if ARAD+ and counter was on, print reasons*/
   print_reason=0;       
#endif
   ...
   if (diag_print_rejected(unit) == CMD_FAIL) return CMD_FAIL;
...
char cmd_dpp_diag_usage[] =
   ...
   "\n\t\tvoq - display all congested VOQs"
...
cmd_dpp_nif(int unit, args_t *a) {
    ...
    int ch0_out_int = 0, ch0_out_remainder = 0, p_div, n_div, m0_div;
    ...
    BCM_PBMP_ITER(pbmp_nif, port) {
        ...
        if (enable){
            ...
            one_clk_time_measured_remainder = 0;
            type_of_bit_clk = 0;
            one_clk_time_measured_int = 0;
            lane = 0;
            serdes_freq_remainder = 0;
            serdes_freq_int = 0;
        }

        rv = bcm_port_to_nif_id(unit, port, &nif_id);
        if (rv != BCM_E_NONE) {
            return CMD_FAIL;
        }

        /*get port_type_str*/
        port_type = SOC_TMC_INTERFACE_ID_to_string(nif_id);

        if (SOC_IS_ARAD(unit)) {
            uint32 reg32_val[1];
            int port_i;
            int i;
            uint32 nof_lanes, base_lane;
            ARAD_NIF_TYPE nif_type;
            pbmp_t phy_ports;
            char *serdes_num_str, serdes_num_str_buf[30];

            nif_type = arad_nif_id2type(nif_id);
            /*get serdes #*/
            ...
            if (nif_type == ARAD_NIF_TYPE_100G_CGE) {
                /*for case of swap*/
            ...
            /*get is_locked*/
            if (ARAD_NIF_TYPE_ILKN == nif_type) {
            rv = READ_NBI_RX_ILKN_STATUSr(unit, nif_id - SOC_TMC_NIF_ID_ILKN_0, reg32_val);
            if (rv != BCM_E_NONE) {
                return CMD_FAIL;
            }
            is_locked = soc_reg_field_get(unit, NBI_RX_ILKN_STATUSr, *reg32_val, RX_N_STAT_ALIGNED_RAWf);
            } else if (ARAD_NIF_TYPE_100G_CGE == nif_type) {
             ...
             /*PLL - ch0 out print*/
            if (READ_ECI_SRD_1_PLL_CONFIGr(unit, reg_val) != BCM_E_NONE) {return CMD_FAIL;}

            p_div = get_field(reg_val[1], 11, 13);
            n_div = get_field(reg_val[0], 3, 10);
            if (p_div == 0) {p_div = 8;};
            if (n_div == 0) {n_div = 1024;};

            m0_div = get_field(reg_val[0], 13, 20);
            if (m0_div == 0) {m0_div = 256;}
            ch0_out_int = (125 / p_div * n_div) / m0_div;
            ch0_out_remainder = (125000 / p_div * n_div) / m0_div - ch0_out_int * 1000;

            cli_out("   %2d   | %-15s |%-21s|    0x%-6x    |     %3s     |   %2d.%-3d    |   %3d.%-3d  \n",
                    port,
                    port_type,
                    serdes_num_str,
                    seq_done,
                    (is_locked ? " + " : " - "),
                    serdes_freq_int, serdes_freq_remainder,
...
int dpp_diag_counter_queue(int unit, args_t *a) {
   ...
   if (queue != -2) {
      ...
      if (!is_voq) {
         ...
         rv = soc_dpp_cosq_vsq_index_global_to_group_get(unit, vsq_id, &soc_vsq_group_type, &vsq_index_in_grp);
         if (SOC_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_APPL_SHELL,
                      (BSL_META_U(unit,
...
#ifdef BCM_DPP_SUPPORT
   uint32 interval = 0;
   char *block_names_tmp[] = {"EGQ", "EPNI", "FDR", "FDT", "IRE", "IPT", "IQM", "NBI"};
...
#ifdef BCM_DPP_SUPPORT
      if (interval != 0) {
         block_names_cur = block_names_tmp;
         block_num = sizeof(block_names_tmp)/sizeof(char *);
...
cmd_dpp_diag_rate(int unit, args_t *a) {
   ...
   while (NULL != option) {
      ...
      if (!sal_strncasecmp(option, "SCH", strlen(option))) {
         ...
         else if (!sal_strncasecmp(sub_option, "GROUP", strlen(option))){
            if (calc_sch_groups_bw(unit) != CMD_OK) retCode = CMD_FAIL;
         }
         else {
            parse_arg_eq_done(&pt);
            return CMD_USAGE;            
      ...
      } else if (!sal_strncasecmp(option, "EPEI", strlen(option))) {
         mode_given++;
         if (if_num == -2) {
            cli_out("option EPEI requires if value\n");
            parse_arg_eq_done(&pt);
            return CMD_FAIL;
         }
         if (calc_epe_if_rate(unit, if_num) != CMD_OK) retCode = CMD_FAIL;
...
cmd_dpp_diag_wb(int unit, args_t *a) {
   ...
   while (NULL != option) {
      ...
      if (!sal_strncasecmp(option, "DUMP", strlen(option))) {
         ...
         mode_given++;
         if (soc_wb_engine_dump(unit, engine_id, 0, var_id, buf_id, 0) != CMD_OK) retCode = CMD_FAIL;
         if (file) {
...
#ifdef BCM_ARAD_SUPPORT
STATIC void
dpp_diag_print_egr_congestion_statistics(ARAD_EGR_CGM_CONGENSTION_STATS *stats,
                                         int is_max,
                                         int is_non_zero,
                                         int port,
                                         int queue,
   ...
   if (0 <= interface && interface < 32 && all != 1) { /* if interface was specified */
...
dpp_diag_egr_congestion(int unit, args_t *a) {
   ...
   unsigned char current = 1, max = 1, all = 1, non_zero = 0, global = 0; /* default options */
   ...
   option = ARG_GET(a);
   while (NULL != option) {
      if (!sal_strncasecmp(option, "current", strlen(option))) {
         current = 1; max = 0;
         ...
         no_updates = 1;
      } else if (!sal_strncasecmp(option, "nz", strlen(option))) {
         non_zero = 1;
   ...
   if (res == CMD_OK) {
      ...
      if (current) {
         rc = soc_arad_get_congestion_statistics(unit, stats, 0, 0, 1);
         if (SOC_FAILURE(rc)) {
            cli_out("\nFailed to get current statistics\n\n");
            res = CMD_FAIL;
         } else {
            dpp_diag_print_egr_congestion_statistics(stats,
                                                     0,
                                                     non_zero,
                                                     port,
                                                     queue,
      ...
      if (max) {
         rc = soc_arad_get_congestion_statistics(unit, 0, stats, 0, no_updates);
         if (SOC_FAILURE(rc)) {
...
STATIC cmd_result_t
dpp_diag_voq_non_empty_print(int unit, uint32 specific_queue,uint32 is_flow_also,uint32 num_most_congested,uint32 poll_nof_times,uint32 poll_delay) {
    uint32 i=0,j=0,k=0;
    soc_error_t rc;
    cmd_result_t result;
...
STATIC cmd_result_t
dpp_diag_voq_details(int unit, int voq_id)
{
   cmd_result_t                 res = CMD_OK;
   soc_error_t rc;
   bcm_error_t rv;
   ...
   int pri_profile_id, pri_count, voq_priority;
   ...
   if (!isisq) {
      ...
      rv = bcm_cosq_gport_threshold_get(unit, gport_connect.voq_connector, 0, &threshold);
      if (BCM_FAILURE(rv)) {
         cli_out("\nbcm_cosq_gport_threshold_get voq_connector(0x%8.8x) queue_index(%d) failed(%d)!", gport_connect.voq_connector, 0, rv);
         return CMD_FAIL;
      }   
   ...
   /* 2.a The device's credit watchdog mode,
      ...
      /* 2.b Credit watchdog message time,
                    ...
                    Empty-Queue-Credit-Balance-Thresholds,
                    Queue-Size-Thresholds */
      diag_dpp_voq_credit_request_threshold_print(&dt);
...
STATIC cmd_result_t
dpp_diag_voq_congestion(int unit, int voq_id, int most) {
   cmd_result_t                 res = CMD_OK;
   ...
   res=dpp_diag_voq_non_empty_print(unit,voq_id,0,most,1,0);
   if (SOC_FAILURE(res)) {
                                       ...
                                       uint32 short_format)
{
    ...
    rv = soc_dpp_cosq_flow_and_up_info_get(unit, is_flow, dest_id, print_status, &flow_and_up_info);
    if (BCM_FAILURE(rv))
    {
        return CMD_FAIL;
    }
    ...
    while (flow_and_up_info.credit_sources_nof != 0)
    {
        ...
        for (credit_source_i = 0; credit_source_i < flow_and_up_info.credit_sources_nof; credit_source_i++){
          if (flow_and_up_info.is_port_sch[credit_source_i]) {        
             ...
             * Get hr2ps info
             */
            rv = soc_dpp_cosq_hr2ps_info_get(unit, 
...
dpp_diag_cosq(int unit, args_t *a) {
   ...
   if (SOC_IS_DPP(unit)) {
      ...
      if (res == CMD_OK) {
         if (print_voqs) {
            if (detailed == 1) {
                res = dpp_diag_voq_details(unit, voq_id);
            }
            else {		
                res = dpp_diag_voq_congestion(unit, voq_id, most);
            }
            set_autocredit = 0;
         ...
         if (print_flow_and_up) {
            ...
            res = dpp_diag_cosq_flow_and_up_print(unit, is_flow, dest_id, print_status, short_format);
            if (SOC_FAILURE(res)) {
         ...
         if (non_empty_queues) {
            unit = (unit);
            res=dpp_diag_voq_non_empty_print(unit,SAL_UINT32_MAX,is_flow_also,0,poll_nof_times,poll_delay);
            if (SOC_FAILURE(res)) {
               cli_out("\nFailed to get cosq non empty queues\n\n");
               res = CMD_FAIL;
            }
         ...
         if (scheduler_alloc_manager) {
            res = dpp_diag_alloc_cosq_sched_print(unit, flow_id, flow_count, allocated_resource);
            if (SOC_FAILURE(res)) {
...
cmd_diag_header_diff(int unit, args_t* a)
{
    ...
    char *block_names_ingress[] = {"IQM", "NBI"};
	char *block_names_egress[] = {"EGQ", "NBI"};
    int block_num = 2;
	int rv;
	...
	if (sal_strcasecmp(subcmd, "Ingress") == 0) {
		...
		rv = WRITE_IQM_ENQUEUE_BYTE_COUNTERr(unit, REG_PORT_ANY, reg64);
        if(rv != SOC_E_NONE) {
            return CMD_FAIL;
        }
		rv = WRITE_IQM_ENQUEUE_PACKET_COUNTERr(unit, REG_PORT_ANY, 0);
        if(rv != SOC_E_NONE) {
            return CMD_FAIL;
        }
		...
		/* read IQM counters */
		rv = READ_IQM_ENQUEUE_PACKET_COUNTERr(unit, REG_PORT_ANY, &pckts_num32);
        if(rv != SOC_E_NONE) {
            return CMD_FAIL;
        }
		rv = READ_IQM_ENQUEUE_BYTE_COUNTERr(unit, REG_PORT_ANY, &bytes_num64);
        if(rv != SOC_E_NONE) {
            return CMD_FAIL;
        }
		...
		cli_out("Ingress header diff: %d\n", (int)avg_bytes_num2);
	}

	if (sal_strcasecmp(subcmd, "Egress") == 0) {
		/* init counters */
        COMPILER_64_ZERO(reg64);
		rv = WRITE_EPNI_EPE_BYTES_COUNTERr(unit, REG_PORT_ANY, reg64);
        if(rv != SOC_E_NONE) {
            return CMD_FAIL;
        }
		rv = WRITE_EPNI_EPE_PACKET_COUNTERr(unit, REG_PORT_ANY, reg64);
        if(rv != SOC_E_NONE) {
            return CMD_FAIL;
        }
		rv = WRITE_EGQ_PQP_UNICAST_BYTES_COUNTERr(unit, REG_PORT_ANY, reg64);
        if(rv != SOC_E_NONE) {
            return CMD_FAIL;
        }
		rv = WRITE_EGQ_PQP_UNICAST_PACKET_COUNTERr(unit, REG_PORT_ANY, reg64);
        if(rv != SOC_E_NONE) {
            return CMD_FAIL;
        }
		...
		/* read EPNI counters */
		rv = READ_EPNI_EPE_PACKET_COUNTERr(unit, REG_PORT_ANY, &pckts_num64);
        if(rv != SOC_E_NONE) {
            return CMD_FAIL;
        }
		rv = READ_EPNI_EPE_BYTES_COUNTERr(unit, REG_PORT_ANY, &bytes_num64);
        if(rv != SOC_E_NONE) {
            return CMD_FAIL;
        }
		COMPILER_64_UDIV_64(bytes_num64, pckts_num64);
		...
		/* read EGQ counters */
		rv = READ_EGQ_PQP_UNICAST_PACKET_COUNTERr(unit, REG_PORT_ANY, &pckts_num64);
        if(rv != SOC_E_NONE) {
            return CMD_FAIL;
        }
		rv = READ_EGQ_PQP_UNICAST_BYTES_COUNTERr(unit, REG_PORT_ANY, &bytes_num64);
        if(rv != SOC_E_NONE) {
            return CMD_FAIL;
        }
		COMPILER_64_UDIV_64(bytes_num64, pckts_num64); 
...
dpp_diag_prge_last(int unit, args_t *a) {
    ...
    arad_egr_prog_editor_print_chosen_program(unit, first_instruction);
    return CMD_OK;
}
...
print_rate_usage(int unit) {
   char cmd_dpp_diag_rate_usage[] =
      ...
      "\nSCH GROUP-\tSCH group rates calculation."
...
cmd_dpp_diag(int unit, args_t *a) {
    ...
    {"HeaderDiff",   cmd_diag_header_diff,       NULL,              DNX_DIAG_ID_UNAVAIL},
    {"ssdump",       cmd_dpp_diag_sw_state_dump, print_sw_state_dump_usage, DNX_DIAG_ID_UNAVAIL},
    {"ocb",          cmd_dpp_diag_ocb,           cmd_dpp_diag_ocb_usage, 	DNX_DIAG_ID_UNAVAIL}
#endif
    };
