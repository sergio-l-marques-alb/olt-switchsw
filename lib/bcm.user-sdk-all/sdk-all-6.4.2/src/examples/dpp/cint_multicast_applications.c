...
int device_level_scheduled_multicast(int unit,int multicast_id,bcm_gport_t* destinations,int* cuds,int nof_destinations)
{
    ...
    for (index = 0; index < nof_destinations; index++) {
         if (BCM_GPORT_IS_LOCAL(destinations[index])) {
             ...
             /* System port */
             rv = bcm_stk_sysport_gport_get(unit,destinations[index],&temp);
             if (rv != BCM_E_NONE) {
                 printf("(%s) \n",bcm_errmsg(rv));
                 return rv;
             }
             modid = BCM_GPORT_MODPORT_MODID_GET(temp);
         }
         /* CUD on multicast ingress must be the multicast egress group id */
         cud = multicast_id;             

         /* Call ERP system port */      
         rv = bcm_port_internal_get(unit,BCM_PORT_INTERNAL_EGRESS_REPLICATION,1,&temp,&count_erp);
         if (rv != BCM_E_NONE) {
             printf("(%s) \n",bcm_errmsg(rv));
             return rv;
         }
         ...
         rv = bcm_stk_gport_sysport_get(unit,temp,&dest_erp);
         if (rv != BCM_E_NONE) {
             printf("(%s) \n",bcm_errmsg(rv));
             return rv;
         }
    ...
    for (index = 0; index < nof_destinations; index++) {
        if (BCM_GPORT_IS_LOCAL(destinations[index])) {
            ...
            port = BCM_GPORT_MODPORT_PORT_GET(temp);
        }
        /* CUD on multicast ingress must be the multicast egress group id */
        cud = cuds[index];                             
        BCM_GPORT_LOCAL_SET(temp,port);
        rv = bcm_multicast_egress_add(unit,multicast_group,temp,cud);
        if (rv != BCM_E_NONE && rv != BCM_E_EXISTS) {
            printf("(%s) \n",bcm_errmsg(rv));
            return rv;
        }      
