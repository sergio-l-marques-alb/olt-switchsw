...
int l2_mact_run(int unit, int port1, int port2){
    ...
    rv = l2_entry_add(unit, mac, l2_mact_table_info1.vid1, 1, 0, is_jericho?l2_mact_table_info1.dest_port_1:l2_mact_table_info1.mc_id);
    if (rv != BCM_E_NONE) {
        return rv;
    }
    generat_mac_address(mac);
	
    rv = l2_entry_add(unit, mac, l2_mact_table_info1.vid2, 1, 0, is_jericho?l2_mact_table_info1.dest_port_2:l2_mact_table_info1.mc_id);
    if (rv != BCM_E_NONE) {
        return rv;
    }
