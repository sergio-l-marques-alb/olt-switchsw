    ...
    void **unused)
{
    ...
    /* If a packet is sent, we will exit out and reschedule at a minimum time, to prevent flooding */
    sal_usecs_t reschedule_time = (sal_usecs_t)BCM_PTP_SIGNAL_MIN_MASTER_DPC_TIME_USEC;
    ...
    for (i = 0; i < num_signaled_masters && reschedule_time == BCM_PTP_SIGNAL_DEFAULT_MASTER_DPC_TIME_USEC; ++i) {
