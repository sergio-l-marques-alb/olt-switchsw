 ...
 */
int
_bcm_th_ipmc_src_port_compare(int unit, int ipmc_index,
                              bcm_ipmc_addr_t *ipmc, int *match)
{
    _bcm_esw_ipmc_l3entry_t *ipmc_l3entry;
    int no_src_check = FALSE;
    int  is_trunk = 0;
    int tgid_f = -1, mod_f = -1, port_f = -1;
    *match = FALSE;
    
    ipmc_l3entry = IPMC_GROUP_INFO(unit, ipmc_index)->l3entry_list;
    if (ipmc_l3entry) { /* IPMC group being used by IP multicast entries */
        _bcm_l3_cfg_t l3cfg;
        sal_memset(&l3cfg, 0, sizeof(_bcm_l3_cfg_t));
        /* Get the first one in the list as all the entires share same IPMC-Repl Group */
        l3cfg.l3c_flags = ipmc_l3entry->l3info.flags;
        l3cfg.l3c_vrf = ipmc_l3entry->l3info.vrf;
        l3cfg.l3c_vid = ipmc_l3entry->l3info.vid;
        l3cfg.l3c_ip_addr = ipmc_l3entry->l3info.ip_addr;
        ...
        if (l3cfg.l3c_flags & BCM_IPMC_IP6) {
        ...
        if (l3cfg.l3c_flags & BCM_IPMC_SOURCE_PORT_NOCHECK) {
            mod_f = SOC_MODID_MAX(unit);
            port_f = TH_IPMC_NO_SRC_CHECK_PORT;
            no_src_check = TRUE;
        } else {
            if (l3cfg.l3c_flags & BCM_L3_TGID) {
                tgid_f = l3cfg.l3c_port_tgid;
                is_trunk = 1;
            } else {
                port_f = l3cfg.l3c_port_tgid;
            }
            mod_f = l3cfg.l3c_modid;
        }
        ...
        if ((ipmc->flags & BCM_IPMC_SOURCE_PORT_NOCHECK) ||
            ...
            if (is_trunk && (tgid_f == ipmc->port_tgid)) {
                *match = TRUE;
            }
        } else {                                            /* source port */
            ...
            if ((mod_f == mod_out) && (port_f == port_out)) {
                *match = TRUE;
            }
        }
    } else { /* No entries using this IPMC group,
              * first IP multicast entry being added using this group
              */
        *match = TRUE;
    }
    return BCM_E_NONE;
