/* $Id: field.c,v 1.581 Broadcom SDK $
                           ...
                           uint8 *dvp_type)
{
    ...
    *eid = *sid = *pid = 0;
    ...
    /* Retrieve DVP_tpe */
    if (version >= BCM_WB_VERSION_1_6) {
        *dvp_type =  buf[fc->scache_pos];
        fc->scache_pos++;
    }
                           ...
                           int slice_idx)
{
    ...
    while (fg_ptr != NULL) {
        ...
        if (fc->l2warm) {
            ...
            BCM_IF_ERROR_RETURN
                (_field_group_info_retrieve(unit, port, &gid, &priority, 
                                            &group_flags, &qset, fc));
            if (gid != -1) {
        ...
        for (entry_index = 0; entry_index < fs_p->entry_count; ++entry_index)
        {
            ...
            if (fc->l2warm)
            {
                rv = _field_entry_info_retrieve(unit, &entry_id, &prio, fc,
                    multigroup, &prev_prio, &stat_id, &policer_id, &dvp_type);
