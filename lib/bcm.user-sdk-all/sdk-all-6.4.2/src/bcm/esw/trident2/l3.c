                             ...
                             _bcm_l3_cfg_t *l3cfg)
{
    ...
    if (SOC_IS_TOMAHAWK(unit)) {
        if (l3cfg->l3c_flags & BCM_IPMC_SOURCE_PORT_NOCHECK) {
            /* For no source port check, set following fields */
            soc_mem_field32_set(unit, mem, buf_p, trunk_port, 1);
            soc_mem_field32_set(unit, mem, buf_p, module_id, 0xff);
            soc_mem_field32_set(unit, mem, buf_p, port_num, 0xff);
        } else {
            if (l3cfg->l3c_flags & BCM_L3_TGID) {
                soc_mem_field32_set(unit, mem, buf_p, trunk_port, 1);
                soc_mem_field32_set(unit, mem, buf_p, trunk_id, l3cfg->l3c_port_tgid);
            } else {
                soc_mem_field32_set(unit, mem, buf_p, port_num, l3cfg->l3c_port_tgid);
            }
            soc_mem_field32_set(unit, mem, buf_p, module_id, l3cfg->l3c_modid);
        }
    }
    return;
}
        ...
        if (SOC_IS_TOMAHAWK(unit)) {
            ...
            if (soc_mem_field32_get(unit, mem, buf_p, svp_valid)) {
                ...
                if (soc_mem_field32_get(unit, mem, buf_p, trunk_port)) {
                    ...
                    if ((l3cfg->l3c_port_tgid == 0xFF) &&
                        soc_mem_field32_get(unit, mem, buf_p, module_id) == 0xFF) {
