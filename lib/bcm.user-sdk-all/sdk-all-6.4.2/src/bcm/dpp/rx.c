...
#define _BCM_RX_SNOOP_CONFIG_PROB_TO_TMC_ACTION_CMD_SNOOP_INFO_PROB(snoop_config_prob, tmc_action_command_prob) \
    ...
    /* the math is  x ---> x* (2^16 -1)/ 100,000. Unfortunately for this we need a uint64*/\
        ...
        COMPILER_64_UMUL_32(prob_res, (SOC_IS_JERICHO(unit) ? 64 :1)*64*1024-1);\
    ...
    bcm_rx_snoop_config_t *config)
{
    ...
    if(SOC_IS_PETRAB(unit)){
        ...
        soc_sand_rv = arad_action_cmd_snoop_get_unsafe(unit,snoop_cmnd,&prm_profile_info);
        SOC_SAND_IF_ERR_RETURN(soc_sand_rv);
    }
#endif
    else {
    ...
    bcm_pkt_t *pkt)
{
    ...
    if (SOC_IS_ARAD(unit)) {
        ...
        PBMP_ITER(bmp, port){
            ...
            rv = soc_port_sw_db_tm_port_to_base_q_pair_get(unit, port, 0, &queue_base);
            if(BCM_FAILURE(rv)) {
                return;
            }
            rv = soc_port_sw_db_pp_port_to_out_port_priority_get(unit, port, 0, &queue_offset);
            if(BCM_FAILURE(rv)) {
                return;
            }
    ...
    uint8 device_access_allowed)
{
    ...
    if (SOC_IS_ARAD(unit)) {
        ...
        PBMP_ITER(bmp, port){
            ...
            BCMDNX_IF_ERR_EXIT(soc_port_sw_db_tm_port_to_base_q_pair_get(unit, port, 0, &queue_base));
            BCMDNX_IF_ERR_EXIT(soc_port_sw_db_pp_port_to_out_port_priority_get(unit, port, 0, &queue_offset));
