...
#define _BCM_L2_EGRESS_SUPPORTED_FLAGS (BCM_L2_EGRESS_WITH_ID)
...
int l2_initialized[BCM_LOCAL_UNITS_MAX]={0};

l2_data_t                  l2_data[BCM_LOCAL_UNITS_MAX];

/* define locks 
   #define L2_LOCK(unit)  sal_mutex_take(l2_data[unit].l2_mutex, sal_mutex_FOREVER)
...
#define _DPP_L2_INIT_GET(unit)                  \
    (l2_initialized[unit])

#define _DPP_L2_INIT_SET(unit, val)             \
    l2_initialized[unit] = (val);

#if 1
#define _DPP_L2_INIT_CHECK(unit)                                        \
    if (_DPP_L2_INIT_GET(unit) == FALSE) {                              \
...
#define _BCM_PETRA_L2_CACHE_INDX_TYPE_RSRVD_MC (0x0)
#define _BCM_PETRA_L2_CACHE_INDX_TYPE_PROG (0x1)
#define _BCM_PETRA_L2_CACHE_INDX_TYPE_VSI_RSRVD_MC (0x2)
...
#define _BCM_PETRA_L2_CACHE_INDX_MAX_VAL  (0x3FFFFFFF)
...
_bcm_petra_l2_freeze_t _bcm_petra_l2_freeze_state[BCM_LOCAL_UNITS_MAX];
...
int _bcm_petra_l2_freeze_set_no_wb(int unit, int frozen)
{
    ...
    _bcm_petra_l2_freeze_state[unit].frozen = frozen;
...
int _bcm_petra_l2_freeze_inc(int unit, int *frozen)
{
    ...
    *frozen = ++_bcm_petra_l2_freeze_state[unit].frozen;
#ifdef BCM_WARM_BOOT_SUPPORT
    BCMDNX_IF_ERR_EXIT(_bcm_dpp_wb_l2_update_freeze_state(unit));
#endif   
exit:
    BCMDNX_FUNC_RETURN;
}
...
int _bcm_petra_l2_freeze_dec(int unit, int *frozen)
{
    ...
    if (_bcm_petra_l2_freeze_state[unit].frozen == 0) {
        return -1;
    }
    *frozen = --_bcm_petra_l2_freeze_state[unit].frozen;
#ifdef BCM_WARM_BOOT_SUPPORT
...
int _bcm_petra_l2_is_freezed(int unit)
{
    return (_bcm_petra_l2_freeze_state[unit].frozen > 0);
}
...
int _bcm_petra_l2_freeze_info_set(int unit, _bcm_petra_l2_freeze_t* freeze_info)
{
    ...
    _bcm_petra_l2_freeze_state[unit].save_age_ena = freeze_info->save_age_ena;
    _bcm_petra_l2_freeze_state[unit].save_learn_ena = freeze_info->save_learn_ena;
#ifdef BCM_WARM_BOOT_SUPPORT
    BCMDNX_IF_ERR_EXIT(_bcm_dpp_wb_l2_update_freeze_state(unit));
#endif
exit:
...
void _bcm_petra_l2_freeze_info_get(int unit, _bcm_petra_l2_freeze_t* freeze_info)
{
    sal_memcpy(freeze_info, &_bcm_petra_l2_freeze_state[unit], sizeof (_bcm_petra_l2_freeze_t));
}

#ifdef BCM_WARM_BOOT_SUPPORT
void _bcm_l2_cb_entry_get(int unit, int idx, bcm_l2_addr_callback_t *fn, void **fn_data) {
    *fn_data = _BCM_PETRA_L2_CB_ENTRY(unit, idx).fn_data;
    *fn = _BCM_PETRA_L2_CB_ENTRY(unit, idx).fn;
}
#endif
...
#define L2_EGRESS_ETHERTYPE_IP 0x0800
...
_bcm_petra_l2_mymac_msb_is_set_set(int unit, uint8 val)
{
    ...
    if (unit < BCM_LOCAL_UNITS_MAX) {
        l2_data[unit].mymac_msb_is_set = val;
    }
#ifdef BCM_WARM_BOOT_SUPPORT
    BCMDNX_IF_ERR_EXIT(_bcm_dpp_wb_l2_update_mymac_msb_state(unit));
#endif
...
uint8
_bcm_petra_l2_mymac_msb_is_set_get(int unit)
{
    if (unit < BCM_LOCAL_UNITS_MAX) {
        return l2_data[unit].mymac_msb_is_set;
    }
    return 0;
}
...
{ 
    ...
    if (l2_entry_val->frwrd_info.forward_decision.type == SOC_PPD_FRWRD_DECISION_TYPE_MC){
        ...
        if(l2_entry_val->frwrd_info.forward_decision.additional_info.trap_info.action_profile.trap_code == _BCM_PETRA_UD_DROP_TRAP){
            ...
            rv = _bcm_dpp_gport_from_fwd_decision(unit,&(l2addr->port),&(l2_entry_val->frwrd_info.forward_decision));
            BCMDNX_IF_ERR_EXIT(rv);
        }
    }
    else
    {
        ...
        rv = _bcm_dpp_port_encap_from_fwd_decision(unit,&(l2addr->port),&(l2addr->encap_id),&(l2_entry_val->frwrd_info.forward_decision),_BCM_DPP_GPORT_ENCAP_USAGE_GENERAL);
        BCMDNX_IF_ERR_EXIT(rv);
    }
...
{ 
    ...
    if (l2_entry_val->mc_id != 0) {
        ...
        rv = _bcm_dpp_mod_port_from_sys_port(unit, &modid, &mode_port, l2_entry_val->sys_port_id);
        BCMDNX_IF_ERR_EXIT(rv);
...
_bcm_petra_l2_llm_callback_thread(int unit)
{
    ...
    while (1) {
        ...
        if (event_msg.s.subclass == MOS_MSG_SUBCLASS_LLM_EVENT_MAC_MOVE
            ...
            gport_id = SOC_DPP_ENCAP_ID_FROM_LIF(mac_move_event.existing_ac);
            ...
            gport_id = SOC_DPP_ENCAP_ID_FROM_LIF(mac_move_event.incoming_ac);
            BCM_GPORT_VLAN_PORT_ID_SET(gport, gport_id);
                                   ...
                                   SHR_LLM_SERVICE_TYPE *serv_type)
{
    ...
    rv = _bcm_dpp_gport_to_lif(unit, gport_id, (int*)&lif_id, NULL, &fec_id, &is_local);
    BCMDNX_IF_ERR_EXIT(rv);
                         ...
                         bcm_vlan_t vid)
{
    ...
    if (_BCM_DPP_VLAN_IS_BVID(l2addr.vid)) { /* vid is B-VID */
...
#endif
        {
            ...
            BCMDNX_IF_ERR_EXIT(rv);
            soc_sand_rv = soc_ppd_frwrd_mact_entry_remove(soc_sand_dev_id,&l2_entry_key);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        }
    }
...
/* internal function to perfrom action on match entries*/
/* used for
                                        ...
                                        )
{
    ...
    *restore = 0;
    ...
    if(!_bcm_petra_l2_is_freezed(unit))
    {
    ...
    if (trvrs_flags & _BCM_PETRA_TRAVERSE_MATCH_TRUNK) {
        ...
        if (match_mask_l2addr != NULL) {
            ...
            rule.value_rule.val_mask.frwrd_info.forward_decision.dest_id = 0x3ff;
        } 
    }
    /* port/gport in rule, has to be after trunk */
    if (trvrs_flags & _BCM_PETRA_TRAVERSE_MATCH_PORT) {

        if (!_BCM_DPP_PORT_ENCAP_INVALID(match_l2addr->encap_id)) {
    ...
    */
    ...
    else if ((match_l2addr->flags & BCM_L2_STATIC)) {/* just consider static entries*/
        rule.value_rule.compare_mask |= SOC_PPD_FRWRD_MACT_TRAVERSE_MATCH_SELECT_IS_DYNAMIC;
    ...
    else if ((match_l2addr->flags & BCM_L2_STATIC)) {/* just consider SA-discard 1*/
        rule.value_rule.compare_mask |= SOC_PPD_FRWRD_MACT_TRAVERSE_MATCH_SELECT_DROP_SA;
    ...
    else /* do it one by one, not in ARAD this is get-by rule is hw-accelared */
    {
        ...
        while(!SOC_SAND_TBL_ITER_IS_END(&(block_range.iter))) {
            soc_sand_rv = soc_ppd_frwrd_mact_get_block(unit,&rule,&block_range,_bcm_l2_traverse_mact_keys[unit],_bcm_l2_traverse_mact_vals[unit],&nof_entries);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
            if(nof_entries == 0) {
                break;
            }
            for (indx = 0; indx < nof_entries; ++indx) {
...
#endif
                    {
                        soc_sand_rv = soc_ppd_frwrd_mact_entry_remove(unit,&(_bcm_l2_traverse_mact_keys[unit][indx]));
                        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                    }
                    /* invoke call back */
                ...
                if (trvrs_flags & _BCM_PETRA_TRAVERSE_ACTION_REPLACE) {
                    ...
                    if (SOC_MAC_LIMIT_PER_TUNNEL_ENABLE_GET(unit) && BCM_GPORT_IS_SET(l2addr.port)) {
                        ...
                        soc_sand_rv = soc_ppd_frwrd_mact_entry_pack(unit,
                                                                    TRUE,
                                                                    SOC_PPD_FRWRD_MACT_ADD_TYPE_TRANSPLANT,
                                                                    &(_bcm_l2_traverse_mact_keys[unit][indx]),
                                                                    &(_bcm_l2_traverse_mact_vals[unit][indx]),
                                                                    l2_entry.entry,
                                                                    &data_len);
...
#endif
                    {
                        soc_sand_rv = soc_ppd_frwrd_mact_entry_add(unit,SOC_PPD_FRWRD_MACT_ADD_TYPE_TRANSPLANT,&(_bcm_l2_traverse_mact_keys[unit][indx]),&(_bcm_l2_traverse_mact_vals[unit][indx]),&failure_indication);
                        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                    }
                                ...
                                uint32 flags)
{
    ...
    if(restore) {
        rv = bcm_petra_l2_addr_thaw(unit);
        BCMDNX_IF_ERR_EXIT(rv);
    }
    BCMDNX_IF_ERR_EXIT(rv);
exit:
...
#ifdef BCM_WARM_BOOT_SUPPORT
            rc = _bcm_dpp_wb_l2_update_data_state(unit, i);
            if (rc != BCM_E_NONE) {
                L2_UNLOCK(unit);
                BCM_EXIT;
            }
    ...
    void *user_data)
{
    ...
    flags = BCM_L2_TRAVERSE_IGNORE_DISCARD_SRC | BCM_L2_TRAVERSE_IGNORE_DES_HIT|BCM_L2_TRAVERSE_MATCH_STATIC ;
                             ...
                             bcm_l2_learn_limit_t *limit)
{
    ...
    if (_L2_LEARN_LIMIT_PER_TUNNEL_IS_ENABLED(unit, limit)) {
        ...
        if (!IS_PON_PORT(unit,gport_info.local_port)) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PORT, (_BSL_BCM_MSG("BCM_L2_LEARN_LIMIT_TUNNEL is not supported in a non PON port")));
        }
        
        soc_sand_rv = soc_ppd_frwrd_mact_tunnel_mac_limit_info_get(soc_sand_dev_id, gport_info.local_port, limit->tunnel_id, &limit->limit);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        limit->limit = _BCM_PETRA_L2_MAC_LIMIT_GET(limit->limit);
        BCM_EXIT;
    }
#endif
    ...
    /* In case of LIF limit, the existing VSI limit table is used. Therefore, the LIF value should be
       mapped to the VSI value, according to exact mapping that is done by the HW */
        ...
        rv = _bcm_dpp_gport_to_lif(unit, limit->port, &lif_id, NULL, NULL, &is_local);
        BCMDNX_IF_ERR_EXIT(rv);

        /* Validate the LIF exists */
        rv = _bcm_dpp_lif_usage_get(unit, lif_id, _BCM_GPORT_ENCAP_ID_LIF_INVALID, &usage, NULL);
        BCMDNX_IF_ERR_EXIT(rv);
        if (usage == _bcmDppLifTypeAny) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("MACT Learn Limit per LIF can be retrieved only for an existing LIF")));
        }
        ...
        soc_sand_rv = soc_ppd_frwrd_mact_limit_mapped_val_to_table_index_get(soc_sand_dev_id,
                                                                             lif_id, &limit_tbl_idx, &is_limit_tbl_idx_reserved);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                             ...
                             bcm_l2_learn_limit_t *limit)
{
    ...
    /* Non system limit: if unlimitted, set to device max limit */
    if(limit->limit == -1){
    ...
    if (limit->flags & BCM_L2_LEARN_LIMIT_PORT) {
        ...
        BCMDNX_IF_ERR_EXIT(rv);
        if (usage == _bcmDppLifTypeAny) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("MACT Learn Limit per LIF can be set only for an existing LIF")));
        }
        ...
        /* Don't allow the set, if the mapping leads to the reserved entry for invalid mappings.
           There's no way to know if it was an expilicit limit for the invalid mapping or misconfiguration. */
    ...
    if (_L2_LEARN_LIMIT_PER_TUNNEL_IS_ENABLED(unit, limit)) {
        ...
        msg.tunnel_count = 1;
        soc_sand_rv = soc_ppd_frwrd_mact_tunnel_mac_limit_info_get(soc_sand_dev_id, msg.port, msg.tunnel, &old_limit);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);

        /* It's vmac limit, set bit 15 of new limit as 1 */
        if (old_limit & SOC_PPD_FRWRD_MACT_LEARN_VMAC_LIMIT) {
            /* It's vmac limit, new value is > 16, return BCM_E_PARAM. */
            if (new_limit > SOC_PPD_FRWRD_MACT_LEARN_MAX_VMAC_LIMIT) {
                BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Invalid mac limit value")));
            }
            
            new_limit |= SOC_PPD_FRWRD_MACT_LEARN_VMAC_LIMIT; 
            msg.type_of_service = LLM_SERVICE_VMAC;
            msg.bitmap = limit->limit;
            type = SHR_LLM_MSG_PON_ATT_MAC_BITMAP_SET;

        } else {
            if (new_limit >= old_limit) {
                msg.bitmap = (new_limit - old_limit);
                type = SHR_LLM_MSG_PON_ATT_MAC_BITMAP_INCREASE;
            } else {
                msg.bitmap = (old_limit - new_limit);
                type = SHR_LLM_MSG_PON_ATT_MAC_BITMAP_DECREASE;
            }
            msg.type_of_service = LLM_SERVICE_MAC_LIMIT;
        }

        /* Send LLM SET message to uC */
        rv = shr_llm_msg_mac_limit_set(unit, type, &msg);
        BCMDNX_IF_ERR_EXIT(rv);

        soc_sand_rv = soc_ppd_frwrd_mact_tunnel_mac_limit_info_set(soc_sand_dev_id, msg.port, msg.tunnel, new_limit);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    }
#endif
                                       ...
                                       bcm_l2_learn_msgs_config_t *learn_msg_config)
{
    ...
    /* set default traffic class attributes for learning messages */
    olp_port_itmh.base.dp = 0;
    olp_port_itmh.base.tr_cls = 0; 
...
int _bcm_petra_l2_event_handle_profile_alloc(
                                             int unit, int check, int vsi, int learn_flags, int trans_flags, int del_flags, int *new_learn_profile, int *first_appear)
{
    int buf;
    int alloc_flags = 0;
    int is_last;
    ...
    rv = _bcm_petra_l2_event_handle_buff_build(learn_flags,trans_flags,del_flags,&buf);
    BCMDNX_IF_ERR_EXIT(rv);
                                         ...
                                         bcm_l2_addr_distribute_t *distribution)
{
    ...
    bcm_l2_addr_distribute_t
        old_distribution;
    int learn_flags=0, trans_flags=0, del_flags=0,new_handle_profile;
    ...
    */ 
    ...
    rv = _bcm_petra_l2_event_handle_profile_alloc(unit, TRUE, vsi,learn_flags, trans_flags, del_flags, &new_handle_profile, &first_handle_appear);
    BCMDNX_IF_ERR_EXIT(rv);

    /* allocate fid-profile with new learn event, old-limit and old aging-profile */
    rv = _bcm_dpp_am_template_l2_learn_profile_exchange(unit, 0, vsi, old_limit, new_handle_profile, old_aging_profile, &old_learn_profile, &is_last, &new_learn_profile,&learn_profile_first_appear);
    BCMDNX_IF_ERR_EXIT(rv);

    /* now that all available allocate event profile */
    rv = _bcm_petra_l2_event_handle_profile_alloc(unit, FALSE, vsi, learn_flags, trans_flags, del_flags, &new_handle_profile, &first_handle_appear);
    BCMDNX_IF_ERR_EXIT(rv);
                                    ...
                                    )
{
    ...
    soc_sand_rv = soc_sand_bitstream_set_any_field(&new_val,indx * 8, 8, buff);
    BCM_SAND_IF_ERR_EXIT_NO_UNIT(soc_sand_rv);
                                       ...
                                       )
{
    ...
    /* packet is sent to the given port if it's not lookuped or dest_port is valid */
    else if ((addr->dest_port != BCM_GPORT_INVALID && BCM_GPORT_IS_SET(addr->dest_port)) || !(addr->flags & BCM_L2_CACHE_LOOKUP)) {
        dest_update = TRUE;
        trap_info->strength = (SOC_DPP_CONFIG(unit)->pp.default_trap_strength);
        trap_info->bitmap_mask |= SOC_PPD_TRAP_ACTION_PROFILE_OVERWRITE_DEST;
    ...
    if(addr->flags & BCM_L2_CACHE_LOOKUP) {
        ...
        dest_update = TRUE;
        /* dont update destination so it will be upadted according to L2 lookup*/
        trap_info->bitmap_mask &= ~SOC_PPD_TRAP_ACTION_PROFILE_OVERWRITE_DEST;
    }
                                      ...
                                      )
{
    ...
    /* if trap set to updare destination then resolve destination */
    if(trap_info->strength != 0 && (trap_info->bitmap_mask & SOC_PPD_TRAP_ACTION_PROFILE_OVERWRITE_DEST)) {
        if(trap_info->dest_info.frwrd_dest.type == SOC_PPD_FRWRD_DECISION_TYPE_MC) {
            addr->flags |= BCM_L2_CACHE_MULTICAST;
            addr->group = trap_info->dest_info.frwrd_dest.dest_id;
        }
        else{

            /* if this drop trap, set discard flag */
            if (trap_info->dest_info.frwrd_dest.type == SOC_PPD_FRWRD_DECISION_TYPE_DROP) {
                addr->flags |= BCM_L2_CACHE_DISCARD;
            }
            else{
                /* prefer return in gport format instead of flags, (yet when set accept flags) --> assymetric set/get */
                rv = _bcm_dpp_gport_from_fwd_decision(unit,&(addr->dest_port),&(trap_info->dest_info.frwrd_dest));
                BCMDNX_IF_ERR_EXIT(rv);
            }
        }
    }
    ...
    if(trap_info->strength != 0 && (trap_info->bitmap_mask & SOC_PPD_TRAP_ACTION_PROFILE_OVERWRITE_TC)) {
                              ...
                              int unit)
{
    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        BCM_PBMP_ITER(PBMP_E_ALL(unit),port_i){       
            ...
            soc_sand_rv = soc_ppd_llp_trap_port_info_set(soc_sand_dev_id, soc_ppd_port, &port_trap_info);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        }
    }
                       ...
                       bcm_l2_cache_addr_t *addr)
{
    ...
    if(index >= _BCM_PETRA_L2_CACHE_INDX_MAX_VAL || index < 0) {
    ...
    is_prog_trap = (indx_type == _BCM_PETRA_L2_CACHE_INDX_TYPE_PROG);
    ...
    if (indx_type == _BCM_PETRA_L2_CACHE_INDX_TYPE_RSRVD_MC) {
        mac_lsb = prog_trap_profile;
    } else if (indx_type == _BCM_PETRA_L2_CACHE_INDX_TYPE_VSI_RSRVD_MC) {
    ...
    /* verify index param */
    if(port >= _BCM_PETRA_NOF_PP_PORTS(unit)) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PORT, (_BSL_BCM_MSG("port is out of range")));
    }

    soc_sand_rv = soc_ppd_llp_trap_port_info_get(soc_sand_dev_id, port, &port_trap_info);
    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    ...
    else if (indx_type == _BCM_PETRA_L2_CACHE_INDX_TYPE_RSRVD_MC){
        addr->vlan_mask = 0;
        /* get trapping/snooping information for the givnen port and MAC */
    ...
    } else if (((SOC_IS_ARAD(unit)) && (indx_type == _BCM_PETRA_L2_CACHE_INDX_TYPE_VSI_RSRVD_MC))) {
        addr->vlan_mask = -1;
                       ...
                       int is_for_delete)
{
    ...
    int old_trap_code, trap_is_last, new_trap_code, trap_is_first;
    int old_profile, rm_is_last, new_profile, rm_is_first;
    int prog_is_first,prog_old_profile, prog_is_last;
    ...
    if (addr->dest_port != BCM_GPORT_TYPE_NONE && (addr->flags & BCM_L2_CACHE_DISCARD)) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Illegal both are set desination and discard flag")));
    }
    ...
    /* errornous flags combinations are checked in _bcm_petra_l2_cache_allocate_trap_fill*/
    ...
    if(SOC_IS_ARAD(unit) && _BCM_PETRA_L2_IS_RM_MAC(addr->mac) && _BCM_PETRA_L2_IS_MAC_ALL_ONES(addr->mac_mask) && (addr->vlan_mask == (bcm_vlan_t)(-1))) {
        mac_lsb = addr->mac[5];
        ...
        *index_used = (_BCM_PETRA_L2_CACHE_INDX_TYPE_VSI_RSRVD_MC  <<_BCM_PETRA_L2_CACHE_INDX_TYPE_SHIFT) |  (vsi << _BCM_PETRA_L2_CACHE_INDX_VSI_RSRVD_MC_VSI_SHIFT ) | (mac_lsb  << _BCM_PETRA_L2_CACHE_INDX_VSI_RSRVD_MC_MAC_SHIFT);
    }
    /*is reserved MC and port */
    /* for reserved MC profile has to be -1 */
    else if(_BCM_PETRA_L2_IS_RM_MAC(addr->mac) && _BCM_PETRA_L2_IS_MAC_ALL_ONES(addr->mac_mask) && !(addr->flags & BCM_L2_CACHE_SUBTYPE) && (addr->ethertype_mask == 0x0) && (addr->vlan_mask == 0)) {
        ...
        BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_i) {
            if (is_for_delete) {
                sal_memset(&rsrvd_info, 0x0, sizeof(_bcm_petra_l2_rsrvd_mc_profile_info_t));
            } else {
        ...
        BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_i) {
            ...
            /* update port configuration */
            soc_sand_rv = soc_ppd_llp_trap_port_info_get(soc_sand_dev_id, port_i, &port_trap_info);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);

            port_trap_info.reserved_mc_profile = new_profile;

            soc_sand_rv = soc_ppd_llp_trap_port_info_set(soc_sand_dev_id, port_i, &port_trap_info);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);

            *index_used = (_BCM_PETRA_L2_CACHE_INDX_TYPE_RSRVD_MC <<_BCM_PETRA_L2_CACHE_INDX_TYPE_SHIFT) |  (port_i << _BCM_PETRA_L2_CACHE_INDX_PORT_SHIFT) | (mac_lsb  << _BCM_PETRA_L2_CACHE_INDX_PROFILE_SHIFT);
    ...
    /*is progrmmable trap */
        ...
        /* or fully masked, or exact ethertpye*/
        if((addr->ethertype_mask != 0xffff) && (addr->ethertype_mask != 0)) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("support only fully masked or exact ethertpye value")));
        }
        ...
        BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_i) {
            if (is_for_delete == FALSE) {
                /* find programable trap, that fit condition and result */
            ...
            *index_used = (_BCM_PETRA_L2_CACHE_INDX_TYPE_PROG <<_BCM_PETRA_L2_CACHE_INDX_TYPE_SHIFT) |  (port_i << _BCM_PETRA_L2_CACHE_INDX_PORT_SHIFT) | (new_profile  << _BCM_PETRA_L2_CACHE_INDX_PROFILE_SHIFT);
            break;
        }
        return BCM_E_NONE;
    }
                    ...
                    int unit)
{
    ...
    /* init freeze info*/
    sal_memset(&_bcm_petra_l2_freeze_state[unit],0x0,sizeof(_bcm_petra_l2_freeze_t));
    ...
    /* clear  handle */
    rv = _bcm_dpp_am_template_fid_aging_profile_clear(unit);
    BCMDNX_IF_ERR_CONT(rv);

    /* clear l2 mymac info */
    l2_data[unit].mymac_msb_is_set = 0;
...
#ifdef BCM_CMICM_SUPPORT
    if (SOC_MAC_LIMIT_PER_TUNNEL_ENABLE_GET(unit)) {
        _bcm_petra_l2_llm_thread_deinit(unit);
        shr_llm_appl_deinit(unit);
    }
#endif
...
#ifdef BCM_WARM_BOOT_SUPPORT
    rv = _bcm_dpp_wb_l2_state_deinit(unit);
#endif
                  ...
                  int unit)
{
    ...
    */
    ...
    rv = _bcm_petra_l2_event_handle_buff_build(BCM_L2_ADDR_DIST_SET_LEARN_DISTRIBUTER,BCM_L2_ADDR_DIST_SET_LEARN_DISTRIBUTER,BCM_L2_ADDR_DIST_SET_LEARN_DISTRIBUTER,&buf);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    /* init fid-aging-profile 0(static-default),1(dynamic-default),3(slb)*/
    /* these are default profiles that couldn't be released*/
    /* Profile 0 initialized with 0 aging_cycles for backward compatibility*/
    /* Profile 1 and 3 initialized with illegal values*/
    rv = _bcm_dpp_am_template_fid_aging_profile_init(unit, 0, 0);
    rv = _bcm_dpp_am_template_fid_aging_profile_init(unit, 1, 7);

    if(soc_property_get(unit, spn_RESILIENT_HASH_ENABLE, 0))
    {
        rv = _bcm_dpp_am_template_fid_aging_profile_init(unit, 3, 8);
        BCMDNX_IF_ERR_EXIT(rv);
    }
    ...
    if (SOC_MAC_LIMIT_PER_TUNNEL_ENABLE_GET(unit)) {
        ...
        if (SOC_DPP_CONFIG(unit)->pp.vmac_enable) {
            ...
            soc_sand_rv = soc_ppd_frwrd_mact_app_to_prefix_get(soc_sand_dev_id, 
        ...
        _bcm_petra_l2_llm_thread_init(unit);
    }
#endif /*BCM_CMICM_SUPPORT*/

#ifdef BCM_WARM_BOOT_SUPPORT
    rv = _bcm_dpp_wb_l2_state_init(unit);
                         ...
                         bcm_l2_station_t *station)
{
    ...
    } else if (station->flags & BCM_L2_STATION_MIM) { /* my-B-Mac */

        /* verify vsi is not valid */
        if (station->vlan_mask != 0) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("vlan cannot be valid if flag BCM_L2_STATION_MIM is set")));
        }
            ...
            *station_id = BCM_PETRA_L2_STATION_GLOBAL_MSB;
            ...
            if (SOC_IS_ARADPLUS(unit) && _BCM_L3_MULTIPLE_MYMAC_ENABLED(unit)) {
                ...
                if (_BCM_PETRA_L3_MULTIPLE_MYMAC_MODE_PROTOCOL_GROUP_IS_ACTIVE(unit)) {
                    ...
                    /* Check if the mac mask is right for the new multiple mymac termination mode.
                    Should be all 1s */
                    soc_sand_rv = soc_sand_pp_mac_address_string_parse("FFFFFFFFFFFF", &mac_mask);
                    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                    soc_sand_rv = soc_sand_pp_mac_address_are_equal(&mac_mask, &my_mac_mask, &is_equal);
                         ...
                         bcm_l2_station_t *station)
{
    ...
    if (BCM_PETRA_L2_STATION_IS_OAM_CFM(station_id)) { /* OAM endpoint */
        ...
        if (vsi == BCM_PETRA_L2_STATION_GLOBAL_MSB) { /* vsi is not valid */
            ...
            if (_bcm_petra_l2_mymac_msb_is_set_get(unit)) {
                                           ...
                                           SOC_PPD_LLP_SA_AUTH_MAC_INFO *mac_auth_info)
{
    ...
    if (mac_auth_info->expect_system_port.sys_id != SOC_PPD_LLP_SA_AUTH_ACCEPT_ALL_PORTS) {
        ...
        rv = _bcm_dpp_mod_port_from_sys_port(unit, &modid, &mode_port, mac_auth_info->expect_system_port.sys_id);
        BCMDNX_IF_ERR_EXIT(rv);
...
_bcm_petra_l2_egress_create_verify(int unit, bcm_l2_egress_t *egr) {
    ...
    /* check flag WITH_ID */
    if ((egr->flags & BCM_L2_EGRESS_WITH_ID) && egr->encap_id == 0) {
    ...
    /* check ethertype is ip: we support only IPoEth */
    if (egr->ethertype != L2_EGRESS_ETHERTYPE_IP) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("bcm_l2_egress_create is only supported for IPoEth. Ethertype must be 0x%x \n"), L2_EGRESS_ETHERTYPE_IP)); 
    }
...
{
    int rv = BCM_E_NONE;
    int outlif =0;  /* used to get allocated outlif/eedb index */
    ...
    if (egr->flags & ~_BCM_L2_EGRESS_SUPPORTED_FLAGS) {
     ...
     *    (could be misleading.. don't do it here, but in bcm_tunnel_intiator_create)
     * 
    ...
    if (egr->flags & BCM_L2_EGRESS_WITH_ID) {
        ...
        /* Check that egr->encap_id is an index of an Overlay LL OutLIF (in SW DB) */
        BCMDNX_IF_ERR_EXIT( _bcm_dpp_lif_usage_get(unit, -1, egr->encap_id, NULL, &usage) );
    ...
    rv = bcm_dpp_am_l3_eep_alloc(unit,_BCM_DPP_AM_L3_EEP_TYPE_ROO_LINKER_LAYER,alloc_flags ,&outlif);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    if (SOC_IS_JERICHO(unit)) {
        ...
        /* inner vid, (see outer_vid) */
        ll_encap_info.inner_vid =  VLAN_CTRL_ID(egr->inner_vlan);

        /* inner PCP DEI (see outer PCP DEI) */
        ...
        rv = bcm_dpp_am_template_eedb_roo_ll_format_eth_type_index_alloc(
           unit, 0, &eth_type_index_entry, &isAllocated, &(ll_encap_info.eth_type_index)); 
        BCMDNX_IF_ERR_EXIT(rv);

        if (isAllocated) {
            /* allocate eth_type_index entry */
    ...
    rv = soc_ppd_eg_encap_overlay_arp_data_entry_add(unit, outlif, &ll_encap_info);
    BCMDNX_IF_ERR_EXIT(rv);

    /* Update SW DB */
    rv = _bcm_dpp_out_lif_overlay_ll_match_add(unit, outlif);
    BCMDNX_IF_ERR_EXIT(rv);

    /* 4. link overlay tunnel eedb entry to overlay arp eedb entry */

    if (egr->l3_intf > 0) {
        if (_BCM_PETRA_L3_ITF_IS_ENCAP(egr->l3_intf)) {
            ...
            rv = _bcm_tunnel_intf_ll_eep_set(unit,egr->l3_intf,tunnel_eep_index, outlif);
            BCMDNX_IF_ERR_EXIT(rv);
        } 
    ...
    bcm_l2_egress_t *egr)
{
    ...
    BCMDNX_IF_ERR_EXIT(_bcm_dpp_lif_usage_get(unit, -1, encap_id, NULL, &usage));
    if (usage != _bcmDppLifTypeOverlayLinkLayer) {
    ...
    /* 1. check outlif (encap_id) is allocated  */
    rv = bcm_dpp_am_l3_eep_is_alloced(unit, encap_id);
    if (rv == BCM_E_EXISTS) {
        SOC_PPC_EG_ENCAP_OVERLAY_ARP_ENCAP_INFO ll_encap_info;
        ...
        if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
            ...
            rv = soc_ppd_eg_encap_entry_get(
               unit, SOC_PPC_EG_ENCAP_EEP_TYPE_DATA, encap_id, 0, encap_entry_info, next_eep, &nof_entries);
            BCM_SAND_IF_ERR_EXIT(rv);
        ...
        /* from jericho */
            ...
            rv = soc_ppd_eg_encap_entry_get(
               unit, SOC_PPC_EG_ENCAP_EEP_TYPE_ROO_LL, encap_id, 0, encap_entry_info, next_eep, &nof_entries);
            BCM_SAND_IF_ERR_EXIT(rv);
    ...
    bcm_if_t encap_id)
{
    ...
    /* check outlif (encap_id) is allocated  */
    rv = bcm_dpp_am_l3_eep_is_alloced(unit, encap_id);
    if (rv == BCM_E_EXISTS) {
    } else {
       BCMDNX_IF_ERR_EXIT(rv);
    ...
    /* free used outlif (SW) */
    rv = bcm_dpp_am_l3_eep_dealloc(unit, encap_id);
    BCMDNX_IF_ERR_EXIT(rv);

    /* free used eedb entry */
    if (SOC_IS_ARADPLUS_A0(unit)) {
        rv = soc_ppd_eg_encap_entry_remove(unit, SOC_PPC_EG_ENCAP_EEP_TYPE_DATA, encap_id);
        BCM_SAND_IF_ERR_EXIT(rv);
    } else {
        SOC_PPD_EG_ENCAP_ENTRY_INFO encap_entry_info[SOC_PPD_NOF_EG_ENCAP_EEP_TYPES];
        uint32 next_eep[SOC_PPD_NOF_EG_ENCAP_EEP_TYPES];
        ...
        rv = soc_ppd_eg_encap_entry_remove(unit, SOC_PPC_EG_ENCAP_EEP_TYPE_ROO_LL, encap_id);
        BCM_SAND_IF_ERR_EXIT(rv);

        /* free template mgr entry (sw db). */
        bcm_dpp_am_template_eedb_roo_ll_format_eth_type_index_free(
    ...
    rv = _bcm_dpp_out_lif_match_delete(unit, encap_id);
    BCMDNX_IF_ERR_EXIT(rv);
