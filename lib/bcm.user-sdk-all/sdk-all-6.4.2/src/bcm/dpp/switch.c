                                        ...
                                        int enable)
{
    ...
    /*update*/
    soc_sand_rv = soc_ppd_eg_filter_port_info_set(soc_sand_dev_id, port, &port_info);
    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                                 ...
                                 int *is_enabled)
{
    ...
    *is_enabled = FALSE;
    ...
    BCM_PBMP_ITER(ports_map, port_i) {          
            ...
            soc_sand_rv = soc_ppd_eg_filter_port_info_get(soc_sand_dev_id, soc_ppd_port_i, &port_info);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                                  ...
                                  int enable)
{
    ...
    if (is_port_relevant > 0)
    {
        ...
        BCM_PBMP_ITER(ports_map, port_i) {            
            ...
            /* update */
            soc_sand_rv = soc_ppd_eg_filter_port_info_set(soc_sand_dev_id, soc_ppd_port_i, &port_info);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        }
    }
exit:
                                  ...
                                  int enable)
{
    ...
    if (is_port_relevant > 0)
    {
        ...
        BCM_PBMP_ITER(ports_map, port_i) {           
            BCMDNX_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_local_to_pp_port_get, (unit, port_i, &soc_ppd_port_i, &core)));
            /* get before update*/
            soc_sand_rv = soc_ppd_llp_trap_port_info_get(unit, soc_ppd_port_i, &port_info);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
            ...
            /* update */
            soc_sand_rv = soc_ppd_llp_trap_port_info_set(unit, soc_ppd_port_i, &port_info);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        }
    }
                                 ...
                                 int *is_enabled)
{
    ...
    *is_enabled = TRUE;
    ...
    if (is_port_relevant > 0)
    {
        ...
        BCM_PBMP_ITER(ports_map, port_i) {       
            BCMDNX_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_local_to_pp_port_get, (unit, port_i, &soc_ppd_port_i, &core)));
            soc_sand_rv = soc_ppd_llp_trap_port_info_get(soc_sand_dev_id,soc_ppd_port_i,&port_info);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                                    ...
                                    int enable)
{
    ...
    /* get before update*/
    soc_sand_rv = soc_ppd_llp_trap_port_info_get(soc_sand_dev_id, port, &port_info);
    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    ...
    /* update */
    soc_sand_rv = soc_ppd_llp_trap_port_info_set(soc_sand_dev_id, port, &port_info);
    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                                    ...
                                    bcm_switch_control_t bcm_type, int arg)
{
    ...
    soc_sand_dev_id = (unit);
    soc_sand_rv = soc_ppd_lag_hashing_port_info_get(soc_sand_dev_id, soc_ppd_port, &info);
    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    ...
    if (BCM_SUCCESS(rv)) {
        soc_sand_rv = soc_ppd_lag_hashing_port_info_set(soc_sand_dev_id, soc_ppd_port, &info);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    }
    BCMDNX_IF_ERR_EXIT(rv);
exit:
                                    ...
                                    bcm_switch_control_t bcm_type, int arg)
{
    ...
    soc_sand_dev_id = (unit);
    soc_sand_rv = soc_ppd_frwrd_fec_ecmp_hashing_port_info_get(soc_sand_dev_id, soc_ppd_port, &info);
    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    ...
    if (BCM_SUCCESS(rv)) {
        soc_sand_rv = soc_ppd_frwrd_fec_ecmp_hashing_port_info_set(soc_sand_dev_id, soc_ppd_port, &info);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    }
    BCMDNX_IF_ERR_EXIT(rv);
exit:
                                 ...
                                 int *arg)
{
    ...
    case bcmSwitchIcmpRedirectToCpu:
        ...
        BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_i) {
            soc_sand_rv = soc_ppd_llp_trap_port_info_get(unit, port_i, &llp_trap_port_info);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
...
/************************************************************************/
    case bcmSwitchPortHeaderType:
        ...
        /* Get TM Header type */
        soc_sand_rc = (MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_port_header_type_get,(unit, core, pp_port, &soc_tmcport_header_type, &soc_tmcport_header_type)));
        BCM_SAND_IF_ERR_EXIT(soc_sand_rc);
    ...
    case bcmSwitchEgressKeepSystemHeader:
        ...
        if (SOC_IS_ARAD_B0_AND_ABOVE(unit)) {
            ...
            soc_sand_rc = (MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_port_pp_port_get,(unit, (uint32)pp_port, &pp_port_info)));
            BCM_SAND_IF_ERR_EXIT(soc_sand_rc);
            ...
            if ((pp_port_info.header_type_out == SOC_TMC_PORT_HEADER_TYPE_ETH) || (pp_port_info.header_type_out == SOC_TMC_PORT_HEADER_TYPE_INJECTED_PP) ||
                (pp_port_info.header_type_out == SOC_TMC_PORT_HEADER_TYPE_INJECTED_2_PP)) {
    ...
    case bcmSwitchMcastTrunkHashMin:
        if (SOC_IS_ARAD(unit)) {
            soc_sand_rc = (MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_port_direct_lb_key_min_get,(unit, core, port, (uint32 *)arg)));
            BCM_SAND_IF_ERR_EXIT(soc_sand_rc);
        } else {
           BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("Supported only in Arad")));
        }
    ...
    case bcmSwitchMcastTrunkHashMax:
        if (SOC_IS_ARAD(unit)) {
            soc_sand_rc = (MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_port_direct_lb_key_max_get,(unit, core, port, (uint32 *)arg)));
            BCM_SAND_IF_ERR_EXIT(soc_sand_rc);
        } else {
           BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("Supported only in Arad")));
        }
    ...
    case bcmSwitchMeterAdjust:
        if (SOC_IS_ARAD(unit)) {
            soc_sand_rc = (MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_cnt_meter_hdr_compensation_get,(unit, port, SOC_TMC_PORT_DIRECTION_INCOMING, arg)));
            BCM_SAND_IF_ERR_EXIT(soc_sand_rc);
            rv = BCM_E_NONE;
        } else {
           BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("Supported only in Arad")));
    ...
    case bcmSwitchCounterAdjust:
        if (SOC_IS_ARAD(unit)) {
            soc_sand_rc = (MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_cnt_meter_hdr_compensation_get,(unit, port, SOC_TMC_PORT_DIRECTION_OUTGOING, arg)));
            BCM_SAND_IF_ERR_EXIT(soc_sand_rc);
            rv = BCM_E_NONE;
        } else {
           BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("Supported only in Arad")));
                                 ...
                                 int arg)
{
    ...
    case bcmSwitchIcmpRedirectToCpu:
        ...
        BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_i) {
            ...
            soc_sand_rv = soc_ppd_llp_trap_port_info_set(unit, port_i, &llp_trap_port_info);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        }
...
/************************************************************************/
    ...
    case bcmSwitchEgressKeepSystemHeader:
        ...
        if ((bcm_type == bcmSwitchPortHeaderType) || (SOC_IS_ARAD_B0_AND_ABOVE(unit))) {
            /* Convert local port to TM/PP port */
            BCMDNX_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_local_to_tm_port_get, (unit, port, &tm_port, &core)));
            BCMDNX_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_local_to_pp_port_get, (unit, port, &pp_port, &core)));
            ...
            soc_sand_rc = (MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_port_pp_port_set,(unit, (uint32)pp_port, core, &pp_port_info, &soc_sand_success)));
            BCM_SAND_IF_ERR_EXIT(soc_sand_rc);
    ...
    case bcmSwitchMcastTrunkHashMin:
        if(SOC_IS_ARAD(unit)){
		    ...
		    BCMDNX_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_local_to_tm_port_get, (unit, gport_info.local_port, &tm_port, &core)));
    ...
    case bcmSwitchMeterAdjust:
        if (SOC_IS_ARAD(unit)) {
            soc_sand_rc = (MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_cnt_meter_hdr_compensation_set,(unit, port, SOC_TMC_PORT_DIRECTION_INCOMING, arg)));
            BCM_SAND_IF_ERR_EXIT(soc_sand_rc);
            rv = BCM_E_NONE;
        } else {
           BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("Supported only in Arad")));
    ...
    case bcmSwitchCounterAdjust:
        if (SOC_IS_ARAD(unit)) {
            soc_sand_rc = (MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_cnt_meter_hdr_compensation_set,(unit, port, SOC_TMC_PORT_DIRECTION_OUTGOING, arg)));
            BCM_SAND_IF_ERR_EXIT(soc_sand_rc);
            rv = BCM_E_NONE;
        } else {
           BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("Supported only in Arad")));
