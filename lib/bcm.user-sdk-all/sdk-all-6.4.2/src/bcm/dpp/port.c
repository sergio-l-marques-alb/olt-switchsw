...
#ifdef  BCM_PETRAB_SUPPORT
  extern void soc_pb_nif_srd_lanes_get(SOC_SAND_IN  int          unit, SOC_SAND_IN  SOC_PETRA_INTERFACE_ID nif_ndx, SOC_SAND_OUT uint32          *nof_lanes,    SOC_SAND_OUT uint32          *base_lane);
#endif
...
bcm_petra_port_init(int unit)
{
    ...
    /* initialize */
    rv = MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_port_init,(unit));
    BCMDNX_IF_ERR_EXIT(rv);
...
bcm_petra_port_local_get(int unit, bcm_gport_t gport, bcm_port_t *local_port)
{
    ...
    if (BCM_GPORT_IS_SET(gport)) {
        ...
        BCM_GPORT_LOCAL_SET(port,gport);
    } else {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PORT, (_BSL_BCM_MSG("Invalid port %d"), gport)); 
    }
    gport = port;

    BCMDNX_IF_ERR_EXIT(bcm_petra_stk_modid_count(unit, &num_modid));
    if ((!num_modid) && !BCM_GPORT_IS_DEVPORT(gport) ){
        /* Only devport gport can be supported on device with no modid */
        BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("Only devport gport can be supported on device with no modid ")));
    }
    ...
    if (BCM_GPORT_IS_LOCAL(gport)) {
        ...
        *local_port = BCM_GPORT_DEVPORT_PORT_GET(gport);
        if (unit != BCM_GPORT_DEVPORT_DEVID_GET(gport)) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PORT, (_BSL_BCM_MSG("Device id doesn't match local unit")));
        }
    } else if (BCM_GPORT_IS_MODPORT(gport)) {
        BCMDNX_IF_ERR_EXIT(bcm_petra_stk_my_modid_get(unit, &my_mod));
        ...
        if (mod_out == my_mod){ 
            ...
            *local_port = port_out;
            while (mod_out != my_mod) {
                mod_out -= 1; 
                *local_port += 32;
            }
        } else {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PORT, (_BSL_BCM_MSG("Invalid MODID %d"), mod_out));
        }
        if (!SOC_PORT_VALID(unit, *local_port)) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PORT, (_BSL_BCM_MSG("Port %d ins't valid for unit"), *local_port));
        }
    } else {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PORT, (_BSL_BCM_MSG("Unsupported gport type")));
    }
...
bcm_petra_port_enable_set(int unit, bcm_port_t port, int enable)
{
    ...
    BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_i) {
         rv = (MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_port_enable_set,(unit, port_i, enable)));
         BCMDNX_IF_ERR_EXIT(rv);
    }
...
bcm_petra_port_speed_set(int unit, bcm_port_t port, int speed)
{
    ...
    if (speed == 0) {
     ...
     /* make sure port isn't in disabled list */
     BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_i) {
         if (BCM_PBMP_MEMBER(SOC_PORT_DISABLED_BITMAP(unit, all), port_i)) {
             BCMDNX_ERR_EXIT_MSG(BCM_E_DISABLED, (_BSL_BCM_MSG("port %d is disabled"), port_i));
         }
     }
...
bcm_petra_port_speed_get(int unit, bcm_port_t port, int *speed)
{
    ...
    BCMDNX_IF_ERR_EXIT(_bcm_dpp_gport_to_phy_port(unit, port, _BCM_DPP_GPORT_TO_PHY_OP_LOCAL_IS_MANDATORY, &gport_info)); 
     ...
     /* make sure port isn't in disabled list */
    if (BCM_PBMP_MEMBER(SOC_PORT_DISABLED_BITMAP(unit, all), gport_info.local_port)) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_DISABLED, (_BSL_BCM_MSG("port %d is disabled"), gport_info.local_port));
    }
...
bcm_petra_port_encap_set(int unit, bcm_port_t port, int mode)
{
    ...
    BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_i) {
        BCMDNX_IF_ERR_EXIT(soc_port_sw_db_encap_mode_set(unit, port_i, mode));
    }
                           ...
                           bcm_port_control_t type, int value)
{
    ...
    uint8 port_is_inlif_profile = FALSE;

    BCMDNX_INIT_FUNC_DEFS;

    if (type == bcmPortControlEgressModifyDscp) {
        port_is_inlif_profile = TRUE;
    }
    ...
    if (port == -1) { 
        ...
        if (BCM_PHY_GPORT_IS_LANE(port)) {
           if (SOC_IS_PETRAB(unit)) {
                BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("Gport is unsupported")));
           }
            local_port = BCM_PHY_GPORT_LANE_PORT_PORT_GET(port);
            if (!SOC_DPP_PHY_GPORT_LANE_VALID(unit, port)) {
                BCMDNX_ERR_EXIT_MSG(BCM_E_PORT, (_BSL_BCM_MSG("Invalid Gport")));
            }
        } else {
            local_port = port;
        }
        
        if (SOC_PORT_VALID_RANGE(unit,local_port) && BCM_PBMP_MEMBER(SOC_PORT_DISABLED_BITMAP(unit, all), local_port)) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_DISABLED, (_BSL_BCM_MSG("port %d is disabled"), local_port));           
        }
     
        if (SOC_PORT_VALID_RANGE(unit,local_port) && BCM_PBMP_MEMBER(PBMP_SFI_ALL(unit), local_port)) {
            BCMDNX_IF_ERR_EXIT(_bcm_petra_port_fabric_control_set(unit, local_port, type, value));
        } else {
            /* TM / PP */
...
{
    int rv = BCM_E_NONE;
    int local_port, core =0;
    SOC_TMC_INTERFACE_ID if_id;
    uint32 tm_port;

    BCMDNX_INIT_FUNC_DEFS;

    BCMDNX_NULL_CHECK(nif_id);
    ...
    rv = (MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_port_to_interface_map_get,(unit, core, tm_port, &if_id, NULL)));
    BCMDNX_IF_ERR_EXIT(rv);
...
#ifdef BCM_PETRAB_SUPPORT
/*this function is relevant for PetraB only*/
STATIC int
_bcm_petra_port_fabric_link_to_serdes_lane(int unit, int link, int *lane)
{
    BCMDNX_INIT_FUNC_DEFS;

    if (link < 0) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("link %d can't be negative"),link));
    } else if (link < SOC_PETRA_NOF_FABRIC_REGULAR_LINKS) {
        *lane = link + 32;
    } else if (link < SOC_PETRA_NOF_FABRIC_REGULAR_LINKS + SOC_PETRA_NOF_FABRIC_LINKS_IN_COMBO) {
        *lane = link - 28 + 12;
    } else if (link < SOC_PETRA_NOF_FABRIC_REGULAR_LINKS + 2 * SOC_PETRA_NOF_FABRIC_LINKS_IN_COMBO) {
        *lane = link - 32 + 28;
    } else {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("illegal link %d"),link));
    }
...
/*
 * Function:
 *      _bcm_petra_port_gport_validate
 * Description:
...
_bcm_petra_port_gport_validate(int unit, bcm_port_t port_in, bcm_port_t *port_out)
{
    ...
    if (BCM_GPORT_IS_SET(port_in)) {
        BCMDNX_IF_ERR_EXIT(bcm_petra_port_local_get(unit, port_in, port_out));
    } else if (SOC_PORT_VALID(unit, port_in)) { 
        *port_out = port_in;
    } else {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PORT, (_BSL_BCM_MSG("port not valid"))); 
    }
...
bcm_petra_port_info_set(int unit, bcm_port_t port, bcm_port_info_t *info)
{
    ...
    BCMDNX_IF_ERR_EXIT(bcm_petra_port_selective_set(unit, gport_info.local_port, info));
...
bcm_petra_port_info_get(int unit, bcm_port_t port, bcm_port_info_t *info)
{
...
#ifdef BCM_ARAD_SUPPORT
    if(SOC_IS_ARAD(unit) && !BCM_PBMP_MEMBER(PBMP_SFI_ALL(unit), port)) {
        uint32 flags;
    ...
    if(SOC_IS_ARDON(unit) && !BCM_PBMP_MEMBER(PBMP_SFI_ALL(unit), port) ) {
                          ...
                          int *linkscan)
{
    ...
    BCMDNX_IF_ERR_EXIT(bcm_common_linkscan_mode_get(unit, port, linkscan));
...
#ifdef BCM_PETRAB_SUPPORT
STATIC int
_bcm_petra_port_serdes_range_prbs_polynomial_set(int          soc_sand_dev_id,
                                                 unsigned int first_lane,
                                                 unsigned int nof_lanes,
                                                 int          poly)
{
    int                 soc_sand_rv;
    ...
    case BCM_PORT_PRBS_POLYNOMIAL_X7_X6_1:
        mode = SOC_PETRA_SRD_PRBS_MODE_POLY_7;
        break;
       
    case BCM_PORT_PRBS_POLYNOMIAL_X15_X14_1:
        mode = SOC_PETRA_SRD_PRBS_MODE_POLY_15;
        break;
        
    case BCM_PORT_PRBS_POLYNOMIAL_X23_X18_1:
        mode = SOC_PETRA_SRD_PRBS_MODE_POLY_23;
        break;
        
    case BCM_PORT_PRBS_POLYNOMIAL_X31_X28_1:
        mode = SOC_PETRA_SRD_PRBS_MODE_POLY_31;
        break;
        
    default:
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG_NO_UNIT("Poly %d not supported"), poly));
        break;
    }
    
    for (lane = first_lane; lane < first_lane + nof_lanes; ++lane) {
        soc_sand_rv = soc_petra_srd_prbs_mode_set(soc_sand_dev_id, lane,
...
STATIC int
_bcm_petra_port_serdes_range_prbs_polynomial_get(int          soc_sand_dev_id,
                                                 unsigned int first_lane,
                                                 unsigned int nof_lanes,
                                                 int          *poly)
{
    int                 soc_sand_rv;
    SOC_PETRA_SRD_PRBS_MODE rx_mode;
    SOC_PETRA_SRD_PRBS_MODE tx_mode;
    SOC_PETRA_SRD_PRBS_MODE port_mode;
    unsigned int        lane;
    
    BCMDNX_INIT_FUNC_DEFS_NO_UNIT;
    /* First, get the RX and TX polynomials of the first serdes lane and make
     * sure that they are the same */
    soc_sand_rv = soc_petra_srd_prbs_mode_get(soc_sand_dev_id, first_lane, &rx_mode, &tx_mode);
    BCM_SAND_IF_ERR_EXIT_NO_UNIT(soc_sand_rv);
    if (rx_mode != tx_mode) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_INTERNAL, (_BSL_BCM_MSG_NO_UNIT("RX and TX polynomials must be the same")));
    }
    port_mode = rx_mode;
    
    /* Verify that all the other serdeses use the same polynomial */
    for (lane = first_lane + 1; lane < first_lane + nof_lanes; ++lane) {
        soc_sand_rv = soc_petra_srd_prbs_mode_get(soc_sand_dev_id, lane, &rx_mode, &tx_mode);
        BCM_SAND_IF_ERR_EXIT_NO_UNIT(soc_sand_rv);
        if (rx_mode != tx_mode || rx_mode != port_mode) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_INTERNAL, (_BSL_BCM_MSG_NO_UNIT("serdeses must use the same polynomial")));
        }
    } 
    
    /* Finally, translate it to the BCM definitions */
    switch (port_mode) {
    case SOC_PETRA_SRD_PRBS_MODE_POLY_7:
        *poly = BCM_PORT_PRBS_POLYNOMIAL_X7_X6_1;
        break;
       
    case SOC_PETRA_SRD_PRBS_MODE_POLY_15:
        *poly = BCM_PORT_PRBS_POLYNOMIAL_X15_X14_1;
        break;
        
    case SOC_PETRA_SRD_PRBS_MODE_POLY_23:
        *poly = BCM_PORT_PRBS_POLYNOMIAL_X23_X18_1;
        break;
    ...
    default:
        BCMDNX_ERR_EXIT_MSG(BCM_E_INTERNAL, (_BSL_BCM_MSG_NO_UNIT("Invalid port mode %d"), port_mode));
        break;
    }
...
STATIC int
_bcm_petra_port_nif_prbs_polynomial_set(int soc_sand_dev_id, int nif_id, int poly)
{
    int          rv = BCM_E_NONE;
    unsigned int nof_lanes;
    unsigned int base_lane;
    
    BCMDNX_INIT_FUNC_DEFS_NO_UNIT;
    soc_pb_nif_srd_lanes_get(soc_sand_dev_id, nif_id, &nof_lanes, &base_lane);
    rv = _bcm_petra_port_serdes_range_prbs_polynomial_set(soc_sand_dev_id,
                                                          base_lane, nof_lanes,
                                                          poly);
    BCMDNX_IF_ERR_EXIT(rv);
...
STATIC int
_bcm_petra_port_nif_prbs_polynomial_get(int soc_sand_dev_id, int nif_id, int *poly)
{
    int          rv = BCM_E_NONE;
    unsigned int nof_lanes;
    unsigned int base_lane;
    ...
    BCMDNX_INIT_FUNC_DEFS_NO_UNIT;
    soc_pb_nif_srd_lanes_get(soc_sand_dev_id, nif_id, &nof_lanes, &base_lane);
    rv = _bcm_petra_port_serdes_range_prbs_polynomial_get(soc_sand_dev_id,
                                                          base_lane,
                                                          nof_lanes, poly);
    BCMDNX_IF_ERR_EXIT(rv);
...
STATIC int
_bcm_petra_port_serdes_range_prbs_set(int soc_sand_dev_id, unsigned int first_lane,
                                      unsigned int nof_lanes,
                                      SOC_TMC_CONNECTION_DIRECTION dir,
                                      int enable)
{
    int          soc_sand_rv;
    unsigned int lane;
    ...
    for (lane = first_lane; lane < first_lane + nof_lanes; ++lane) {
        if (enable) {
            soc_sand_rv = soc_petra_srd_prbs_start(soc_sand_dev_id, lane, dir);
            BCM_SAND_IF_ERR_EXIT_NO_UNIT(soc_sand_rv);
        } else {
            soc_sand_rv = soc_petra_srd_prbs_stop(soc_sand_dev_id, lane, dir);
            BCM_SAND_IF_ERR_EXIT_NO_UNIT(soc_sand_rv);
        }
    } 
...
STATIC int
_bcm_petra_port_nif_prbs_set(int soc_sand_dev_id, int nif_id, 
                         SOC_TMC_CONNECTION_DIRECTION dir, int enable)
{
    int          rv = BCM_E_NONE;
    unsigned int nof_lanes;
    unsigned int base_lane;
    ...
    BCMDNX_INIT_FUNC_DEFS_NO_UNIT;
    soc_pb_nif_srd_lanes_get(soc_sand_dev_id, nif_id, &nof_lanes, &base_lane);
    rv = _bcm_petra_port_serdes_range_prbs_set(soc_sand_dev_id, base_lane,
                                               nof_lanes, dir, enable);
    BCMDNX_IF_ERR_EXIT(rv);
exit:
    BCMDNX_FUNC_RETURN;
}
...
STATIC int
_bcm_petra_port_serdes_range_prbs_result_get(int soc_sand_dev_id,
                                             unsigned int first_lane,
                                             unsigned int nof_lanes,
                                             int *status)
{
    int                      soc_sand_rv;
    unsigned int             lane;
    SOC_PETRA_SRD_PRBS_RX_STATUS rx_status;
    
    BCMDNX_INIT_FUNC_DEFS_NO_UNIT;
    *status = 0;
    
    for (lane = first_lane; lane < first_lane + nof_lanes; ++lane) {
        soc_sand_rv = soc_petra_srd_prbs_get_and_clear_stat(soc_sand_dev_id, lane, &rx_status);
        BCM_SAND_IF_ERR_EXIT_NO_UNIT(soc_sand_rv);
        
        if (rx_status.prbs_signal_stat != SOC_PETRA_SRD_PRBS_SIGNAL_STAT_LOCKED_NO_LOSS) {
            ++*status;
        } else {
            *status += rx_status.error_cnt;
        }
    }
...
STATIC int
_bcm_petra_port_nif_prbs_result_get(int soc_sand_dev_id, int nif_id, int *status)
{
    int          rv = BCM_E_NONE;
    unsigned int nof_lanes;
    unsigned int base_lane;
    ...
    BCMDNX_INIT_FUNC_DEFS_NO_UNIT;
    soc_pb_nif_srd_lanes_get(soc_sand_dev_id, nif_id, &nof_lanes, &base_lane);
    rv = _bcm_petra_port_serdes_range_prbs_result_get(soc_sand_dev_id, base_lane,
                                                      nof_lanes, status);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    _bcm_petra_dtag_mode_t *dtag_mode)
{
   ...
   rv =  _bcm_dpp_am_template_tpid_profile_data_get(unit, soc_ppd_port_i, &profile_type, &accept_mode, &temp_dtag_mode, &ac_key_map_type);
   BCMDNX_IF_ERR_EXIT(rv);
    ...
    _bcm_petra_ac_key_map_type_t ac_key_map_type)
{
    ...
    rv = _bcm_dpp_am_template_tpid_profile_exchange(unit, soc_ppd_port_i, profile_type, accept_mode, dtag_mode, ac_key_map_type, &old_tpid_profile, &last_appear, &new_tpid_profile, &first_appear);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    _bcm_petra_dtag_mode_t dtag_mode)
{
   ...
   BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_i) {
        ...
        rv = _bcm_petra_port_discard_extend_set(unit, soc_ppd_port_i, profile_type, accept_mode, dtag_mode, ac_key_map_type);
        BCMDNX_IF_ERR_EXIT(rv);
   }
    ...
    int mode)
{
   ...
   BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_i) {
        ...
        /* get old data (profile_type) */
        rv =  _bcm_dpp_am_template_tpid_profile_data_get(unit, soc_ppd_port_i, &profile_type, &accept_mode, &dtag_mode, &ac_key_map_type);
        BCMDNX_IF_ERR_EXIT(rv);
        
        /* Exchange in case of update and commit */
        rv = _bcm_petra_port_discard_extend_set(unit, soc_ppd_port_i, profile_type, mode, dtag_mode, ac_key_map_type);
        BCMDNX_IF_ERR_EXIT(rv);
   }
                                ...
                                bcm_port_control_t type, int value)
{
    ...
    SOC_TMC_FC_GEN_INBND_INFO fc_tx_info;
    SOC_TMC_FC_REC_INBND_INFO fc_rx_info;
#ifdef  BCM_ARAD_SUPPORT
    SOC_TMC_FC_PFC_TIMER_INFO fc_pfc_refresh_timer_info;
    ...
    SOC_TMC_FC_GEN_INBND_INFO_clear(&fc_tx_info);
    SOC_TMC_FC_REC_INBND_INFO_clear(&fc_rx_info);
    ...
    rv = _bcm_petra_port_to_nif_id(unit, local_port, &nif_id);
    BCMDNX_IF_ERR_EXIT(rv);

    switch (type) {
    case bcmPortControlPrbsPolynomial:
        rv = _bcm_petra_port_nif_prbs_polynomial_set(soc_sand_dev_id, nif_id, value);
        break;
        
    case bcmPortControlPrbsTxEnable:
        rv = _bcm_petra_port_nif_prbs_set(soc_sand_dev_id, nif_id, 
                                      SOC_TMC_CONNECTION_DIRECTION_TX, value);
        break;
    
    case bcmPortControlPrbsRxEnable:
        rv = _bcm_petra_port_nif_prbs_set(soc_sand_dev_id, nif_id,
                                      SOC_TMC_CONNECTION_DIRECTION_RX, value);
        break;
    ...
    case bcmPortControlLLFCTransmit:
        soc_sand_rv = soc_pb_fc_gen_inbnd_get(soc_sand_dev_id, nif_id, &fc_tx_info);
        BCM_SAND_IF_ERR_EXIT_NO_UNIT(soc_sand_rv);

        if(value == 0)
        {
          fc_tx_info.mode = SOC_TMC_FC_INBND_MODE_DISABLED;
        }
        else
        {
          fc_tx_info.mode = (type == bcmPortControlPFCTransmit) ? SOC_TMC_FC_INBND_MODE_CB : SOC_TMC_FC_INBND_MODE_LL;
        }

        soc_sand_rv = soc_pb_fc_gen_inbnd_set(soc_sand_dev_id, nif_id, &fc_tx_info);
        BCM_SAND_IF_ERR_EXIT_NO_UNIT(soc_sand_rv);
      break;

    case bcmPortControlPFCReceive:
    case bcmPortControlLLFCReceive:
        soc_sand_rv = soc_pb_fc_rec_inbnd_get(soc_sand_dev_id, nif_id, &fc_rx_info);
        BCM_SAND_IF_ERR_EXIT_NO_UNIT(soc_sand_rv);

        if(value == 0)
        {
          fc_rx_info.mode = SOC_TMC_FC_INBND_MODE_DISABLED;
        }
        else
        {
          fc_rx_info.mode = (type == bcmPortControlPFCReceive) ? SOC_TMC_FC_INBND_MODE_CB : SOC_TMC_FC_INBND_MODE_LL;
        }

        soc_sand_rv = soc_pb_fc_rec_inbnd_set(soc_sand_dev_id, nif_id, &fc_rx_info);
        BCM_SAND_IF_ERR_EXIT_NO_UNIT(soc_sand_rv);
      break;
    ...
    switch(type) {
      case bcmPortControlLinkDownPowerOn:
        BCMDNX_IF_ERR_EXIT(soc_pm_enable_set(unit, port, value));
        break;
    ...
    case bcmPortControlPFCRefreshTime:
         ...
         soc_sand_rv = arad_fc_pfc_timer_get(soc_sand_dev_id, nif_id, &fc_pfc_refresh_timer_info);
         BCM_SAND_IF_ERR_EXIT_NO_UNIT(soc_sand_rv);

         fc_pfc_refresh_timer_info.enable = value ? TRUE : FALSE; 
         fc_pfc_refresh_timer_info.refresh_time = value;

         soc_sand_rv = arad_fc_pfc_timer_set(soc_sand_dev_id, nif_id, &fc_pfc_refresh_timer_info);
         BCM_SAND_IF_ERR_EXIT_NO_UNIT(soc_sand_rv);
         break;
    ...
    case bcmPortControlSAFCTransmit:
        soc_sand_rv = arad_fc_gen_inbnd_get(soc_sand_dev_id, nif_id, &fc_tx_info);
        BCM_SAND_IF_ERR_EXIT_NO_UNIT(soc_sand_rv);
        ...
        else
        {
          ...
          {
            fc_tx_info.mode = SOC_TMC_FC_INBND_MODE_PFC;
          }
          if(type == bcmPortControlSAFCTransmit)
          {
            fc_tx_info.mode = SOC_TMC_FC_INBND_MODE_SAFC;
          }
          if(type == bcmPortControlLLFCTransmit)
          {
            fc_tx_info.mode = SOC_TMC_FC_INBND_MODE_LL;
          }
        }

        soc_sand_rv = arad_fc_gen_inbnd_set(soc_sand_dev_id, nif_id, &fc_tx_info);
        BCM_SAND_IF_ERR_EXIT_NO_UNIT(soc_sand_rv);
      break;
    ...
    case bcmPortControlSAFCReceive:
        soc_sand_rv = arad_fc_rec_inbnd_get(soc_sand_dev_id, nif_id, &fc_rx_info);
        BCM_SAND_IF_ERR_EXIT_NO_UNIT(soc_sand_rv);
        ...
        else
        {
          ...
          {
            fc_rx_info.mode = SOC_TMC_FC_INBND_MODE_PFC;
          }
          if(type == bcmPortControlSAFCReceive)
          {
            fc_rx_info.mode = SOC_TMC_FC_INBND_MODE_SAFC;
          }
          if(type == bcmPortControlLLFCReceive)
          {
            fc_rx_info.mode = SOC_TMC_FC_INBND_MODE_LL;
          }
        }

        soc_sand_rv = arad_fc_rec_inbnd_set(soc_sand_dev_id, nif_id, &fc_rx_info);
        BCM_SAND_IF_ERR_EXIT_NO_UNIT(soc_sand_rv);
      break;
      case bcmPortControlLinkFaultLocal:
          BCMDNX_IF_ERR_EXIT(soc_pm_local_fault_clear(unit, port));
          break;
      case bcmPortControlLinkFaultRemote:
          BCMDNX_IF_ERR_EXIT(soc_pm_remote_fault_clear(unit, port));
          break;
...
#ifdef  BCM_ARAD_SUPPORT
STATIC int
_bcm_petra_port_fabric_pcs_control_set(int soc_sand_dev_id,int link,int pcs_val){
   int          soc_sand_rv;
   ARAD_PORT_PCS pcs;
   BCMDNX_INIT_FUNC_DEFS_NO_UNIT;

   pcs = (ARAD_PORT_PCS) pcs_val;
   soc_sand_rv = arad_port_control_pcs_set(soc_sand_dev_id,link,pcs);
   BCM_SAND_IF_ERR_EXIT_NO_UNIT(soc_sand_rv);
...
STATIC int
_bcm_petra_port_fabric_pcs_control_get(int soc_sand_dev_id,int link,int *pcs_val){
   int          soc_sand_rv;
   ARAD_PORT_PCS* pcs;
   BCMDNX_INIT_FUNC_DEFS_NO_UNIT;
   pcs = (ARAD_PORT_PCS*)pcs_val;
   soc_sand_rv = arad_port_control_pcs_get(soc_sand_dev_id,link,pcs);
   BCM_SAND_IF_ERR_EXIT_NO_UNIT(soc_sand_rv);
                                   ...
                                   bcm_port_control_t type, int value)
{
    ...
    if (SOC_IS_PETRAB(unit)) {
        ...
        BCMDNX_IF_ERR_EXIT(_bcm_petra_port_fabric_link_to_serdes_lane(unit, link, &lane));

        switch (type) {
            case bcmPortControlPrbsPolynomial:
                rv = _bcm_petra_port_serdes_range_prbs_polynomial_set(unit, lane,
                                                              1, value);
                break;
        
            case bcmPortControlPrbsTxEnable:
                rv = _bcm_petra_port_serdes_range_prbs_set(unit, lane, 1,
                                          SOC_TMC_CONNECTION_DIRECTION_TX, value);
                break;
    ...
    if (SOC_IS_ARAD(unit)) {
        ...
        switch (type) {
            case bcmPortControlPCS:
                rv = _bcm_petra_port_fabric_pcs_control_set(unit,link,value);
                BCMDNX_IF_ERR_EXIT(rv);
            ...
            case bcmPortControlLinkDownPowerOn:
                {
                    ...
                    soc_sand_rv = arad_link_on_off_set(unit, link, &info);
                    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                }
                break;

            case bcmPortControlStripCRC:
                soc_sand_rv = arad_link_control_strip_crc_set(unit, port, value ? 1 :  0);
                BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                break;

            case bcmPortControlRxEnable:
                rv = arad_link_control_rx_enable_set(unit, link, value);
                BCMDNX_IF_ERR_EXIT(rv);
                break;

            case bcmPortControlTxEnable:
                rv = arad_link_control_tx_enable_set(unit, link, value);
                BCMDNX_IF_ERR_EXIT(rv);
                break;

            case bcmPortControlPrbsPolynomial:
                if(SOC_PBMP_MEMBER(_bcm_dpp_port_prbs_mac_mode[unit],port)) {
                    BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("Control %d isn't supported for MAC PRBS"), type));
                } else if ((value != BCM_PORT_PRBS_POLYNOMIAL_X7_X6_1) && (value != BCM_PORT_PRBS_POLYNOMIAL_X15_X14_1)\
                        && (value != BCM_PORT_PRBS_POLYNOMIAL_X23_X18_1) && (value != BCM_PORT_PRBS_POLYNOMIAL_X31_X28_1)){
            ...
            case bcmPortControlPrbsTxEnable:
                if(SOC_PBMP_MEMBER(_bcm_dpp_port_prbs_mac_mode[unit],port)) {
                    soc_sand_rv = arad_port_prbs_tx_enable_set(unit, link, value);
            ...
            case bcmPortControlPrbsRxEnable:
                if(SOC_PBMP_MEMBER(_bcm_dpp_port_prbs_mac_mode[unit],port)) {
                    soc_sand_rv = arad_port_prbs_rx_enable_set(unit, link, value);
                    rv = handle_sand_result(soc_sand_rv); 
                } else {
                    MIIM_LOCK(unit);
                    locked = 1;
        ...
        case bcmPortControlPrbsRxStatus:
                MIIM_LOCK(unit);
                locked = 1;
                rv = soc_phyctrl_control_set(unit, port, SOC_PHY_CONTROL_PRBS_RX_STATUS, value);
                BCMDNX_IF_ERR_EXIT(rv);
                locked = 0;
                MIIM_UNLOCK(unit);
                break;
            ...
            case bcmPortControlLowLatencyLLFCEnable:
                rv=MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_port_control_low_latency_set,(unit, link,value));
                BCM_SAND_IF_ERR_EXIT(rv);
                break;

            case bcmPortControlFecErrorDetectEnable:
                rv=MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_port_control_fec_error_detect_set,(unit,link,value));
                BCM_SAND_IF_ERR_EXIT(rv);
                break;
...
STATIC int _bcm_petra_port_inlif_profile_set(
   ...
   const uint32 arg, 
   const SOC_PPC_PROFILE_MGMT_INLIF_PROFILE_TYPE kind,
   ...
   SOC_PPD_LIF_ENTRY_INFO *lif_info)
{
  ...
  if ((BCM_E_NONE == rv) && (lif_id != _BCM_GPORT_ENCAP_ID_LIF_INVALID)) {
    ...
    if (kind == SOC_PPC_PROFILE_MGMT_INLIF_PROFILE_TYPE_USER) {
        soc_sand_rv = MBCM_PP_DRIVER_CALL(unit,mbcm_pp_profile_mgmt_if_profile_advanced_val_set,(unit, SOC_PPC_PROFILE_MGMT_TYPE_INLIF, arg, mapped_val_p));
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    }
    else {
        soc_sand_rv = MBCM_PP_DRIVER_CALL(unit,mbcm_pp_profile_mgmt_if_profile_val_set,(unit, SOC_PPC_PROFILE_MGMT_TYPE_INLIF, kind, arg, mapped_val_p));
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    }  
   ...
   SOC_PPD_LIF_ENTRY_INFO *lif_info)
{
  ...
  if ((BCM_E_NONE == rv) && (lif_id != _BCM_GPORT_ENCAP_ID_LIF_INVALID)) {
    ...
    if (kind == SOC_PPC_PROFILE_MGMT_INLIF_PROFILE_TYPE_USER) {
        soc_sand_rv = MBCM_PP_DRIVER_CALL(unit,mbcm_pp_profile_mgmt_if_profile_advanced_get_val_from_map,(unit, SOC_PPC_PROFILE_MGMT_TYPE_INLIF, mapped_val, arg));
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    }
    else {
        soc_sand_rv = (MBCM_PP_DRIVER_CALL(unit,mbcm_pp_profile_mgmt_if_profile_get_val_from_map,(unit, SOC_PPC_PROFILE_MGMT_TYPE_INLIF, kind, mapped_val, arg)));
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    }
  }
    ...
    if (SOC_DPP_PP_ENABLE(unit)) {
        ...
        switch (type)
        {
          ...
          case bcmPortControlFcoeFabricId:
              if(!SOC_DPP_CONFIG(unit)->pp.fcoe_enable ){
                  ...
                  soc_sand_rv = soc_ppd_fp_control_set(unit,&ctrl_indx,&ctrl_info,&failure_indication);
                  BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
              ...
              if (value == 0 || value == 1) { /* disable/enable MiM */ 
                  ...
                  BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_ndx) {
                      ...
                      if (SOC_IS_ARAD(unit)) {
                          if (value == 1) { /* Enable MiM */
                              ...
                              if (port_info.dflt_egress_ac == 0) {
                                  ...
                                  port_info.dflt_egress_ac = __dpp_mim_out_ac_get(unit);                          
                              }
                          }
                          else { /* Disable MiM */
                              /* set out lif is default, set it to L2 default out lif */
                              if (port_info.dflt_egress_ac == __dpp_mim_out_ac_get(unit)) {                              
                      ...
                      if (value == 1) { /* enable MiM */
                          ...
                          if (lif_id == SOC_DPP_CONFIG(unit)->pp.global_lif_index_simple)
                          {                              
                              if (SOC_IS_PETRAB(unit)) {
                              ...
                              soc_sand_rv = soc_ppd_l2_lif_ac_add(unit, &ac_key, __dpp_mim_lif_ndx_get(unit), &ac_info, &success);
                              BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                          ...
                          if (lif_id == SOC_DPP_CONFIG(unit)->pp.mim_lif_ndx) {
        ...
        case bcmPortControlDiscardMacSaAction:
          {
            ...
            BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_ndx) {
              ...
              rv = _bcm_dpp_am_template_port_mact_sa_drop_exchange(unit,soc_ppd_port,&action_profile,&old_template,&is_last,&new_template,&is_new);
              BCMDNX_IF_ERR_EXIT(rv);

              if (old_template != new_template)
              {
                /* Set mapping in HW */
                soc_sand_rv = soc_ppd_frwrd_mact_port_info_get(unit,soc_ppd_port,&frwrd_mact_port_info);
                rv = handle_sand_result(soc_sand_rv); 
                if (rv == BCM_E_NONE) { 
                  /* All OK go on */
                  frwrd_mact_port_info.sa_drop_action_profile = new_template;
                  soc_sand_rv = soc_ppd_frwrd_mact_port_info_set(unit,soc_ppd_port,&frwrd_mact_port_info);
                  rv = handle_sand_result(soc_sand_rv);
                } 
        ...
        case bcmPortControlUnknownMacDaAction:
          {
            ...
            BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_ndx) {
              ...
              rv = _bcm_dpp_am_template_port_mact_da_unknown_exchange(unit,soc_ppd_port,&action_profile,&old_template,&is_last,&new_template,&is_new);
              BCMDNX_IF_ERR_EXIT(rv);
        ...
        case bcmPortControlBridge: 
          {
            ...
            if (BCM_GPORT_IS_VLAN_PORT(port)) {
              ...
              if (simple_mode) {
                ...
                /* 1 - disable, 0 - enable*/
                rv = _bcm_petra_port_inlif_profile_set(unit, port, value, SOC_PPC_PROFILE_MGMT_INLIF_PROFILE_TYPE_SIMPLE_SAME_INTERFACE, lif_info);
                ...
                if (SOC_IS_ARADPLUS(unit)) {
                  ...
                  rv = _bcm_petra_port_inlif_profile_get(unit, port, &inlif_profile, SOC_PPC_PROFILE_MGMT_INLIF_PROFILE_TYPE_USER, lif_info);
                ...
                BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_ndx) {
                  ...
                  soc_sand_rv = soc_ppd_eg_filter_port_info_get(unit, soc_ppd_port, &eg_port_info);
                  BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                  if (value == 1) {
                  ...
                  soc_sand_rv = soc_ppd_eg_filter_port_info_set(unit, soc_ppd_port, &eg_port_info);
                  BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                }
            }
          }
            ...
            if (SOC_IS_ARAD(unit)) {
                ...
                if (gport_type == _BCM_DPP_GPORT_TYPE_SIMPLE) {
                    ...
                    if (BCM_GPORT_IS_VLAN_PORT(port) || BCM_GPORT_IS_MPLS_PORT(port)) {
                        ...
                        rv = _bcm_dpp_gport_to_lif(unit, port, &lif_index, NULL, &fec_id, &is_local);
                        BCMDNX_IF_ERR_EXIT(rv);
              ...
              BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_ndx) {
                  ...
                  if (SOC_IS_ARAD(unit)) {
                      ...
                      if ((!trill_disable_designated_vlan_check) && ((SOC_DPP_CONFIG(unit))->trill.designated_vlan_inlif_enable)) {
                          soc_sand_rv = soc_ppd_llp_filter_designated_vlan_get(unit, soc_ppd_port, &vid, 
                                                                        &accept);
                          BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                          ...
                          lif_index = soc_property_port_suffix_num_get(unit, soc_ppd_port, -1, spn_CUSTOM_FEATURE, "trill_designated_vlan_inlif", -1);
                          if ((lif_index != -1) && (vid != value)) {
                              soc_sand_rv = soc_ppd_port_info_get(unit, soc_ppd_port, &port_info);
                              BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                              ...
                              soc_sand_rv = soc_ppd_l2_lif_ac_add(unit, &ac_key, lif_index, &ac_info, &success);
                              BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                              SOC_SAND_IF_FAIL_RETURN(success);
                          }
                      }
                  }
                  soc_sand_rv = soc_ppd_llp_filter_designated_vlan_set(unit, soc_ppd_port, value, 
                                                                TRUE, &success);
                  BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                  SOC_SAND_IF_FAIL_RETURN(success);
              }        
        ...
        case bcmPortControlL2SaAuthenticaion:
        {
            ...
            BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_ndx) {
                ...
                soc_sand_rv = soc_ppd_llp_sa_auth_port_info_set(unit, soc_ppd_port, &sa_auth_port_info);
                BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
            }
        }
        break;
        ...
        case bcmPortControlTrill: 
          {
              ...
              BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_ndx) {
                ...
                if (value && ((SOC_DPP_CONFIG(unit))->trill.designated_vlan_inlif_enable)) {
                    lif_id = soc_property_port_suffix_num_get(unit, soc_ppd_port, -1, spn_CUSTOM_FEATURE, "trill_designated_vlan_inlif", -1);
                    if (lif_id == -1) {
                ...
                soc_sand_rv = soc_ppd_llp_vid_assign_port_info_get(unit, soc_ppd_port, &vid_assign_port_info);
                BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        
                if (SOC_IS_PETRAB(unit)) { 
                    soc_sand_rv = soc_ppd_llp_sa_auth_port_info_get(unit, soc_ppd_port, &sa_auth_port_info);
                    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                }
                ...
                if (value) {
                    ...
                    soc_sand_rv = soc_ppd_llp_vid_assign_port_info_set(unit, soc_ppd_port, &vid_assign_port_info);
                    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        ...
        case bcmPortControlPreservePacketPriority: 
          {
              ...
              BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_ndx) {
                  ...
                  soc_sand_rv = soc_ppd_eg_vlan_edit_port_vlan_transmit_outer_tag_set(unit, soc_ppd_port, 0, transmit_outer_tag);
                  BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
              }
          }
          break;
        ...
        case bcmPortControlEgressFilterDisable:
              ...
              if (SOC_IS_ARAD(unit)) {
                  BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_ndx) {
                      if (IS_RCY_PORT(unit,port_ndx)) {
                          ...
                          BCM_SAND_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_port_pp_port_get,(unit, soc_ppd_port, &pp_port_info)));
                          ...
                          if ((header_type_incoming == SOC_TMC_PORT_HEADER_TYPE_INJECTED_2) || (header_type_incoming == SOC_TMC_PORT_HEADER_TYPE_INJECTED_2_PP)) {
                              ...
                              BCM_SAND_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_port_pp_port_set,(unit, soc_ppd_port, core, &pp_port_info, &success)));
                              SOC_SAND_IF_FAIL_RETURN(success);
                          }
          ...
          case bcmPortControlEvbType:
          {
              ...
              BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_ndx) {
                  ...
                  soc_sand_rv = soc_ppd_llp_trap_port_info_get(unit, soc_ppd_port, &llp_trap_port_info);
                  BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                  ...
                  soc_sand_rv = soc_ppd_llp_trap_port_info_set(unit, soc_ppd_port, &llp_trap_port_info);
                  BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        ...
        case bcmPortControlVMac:
            {
                ...
                if (SOC_IS_ARAD(unit)) {
                    ...
                    rv = _bcm_dpp_gport_to_lif(unit, port, (int*)&lif_id, NULL, &fec_id, &is_local);
                    BCMDNX_IF_ERR_EXIT(rv);
            ...
            case bcmPortControlErspanEnable:
            {
                if (SOC_IS_ARAD_B1_AND_BELOW(unit)) {
                    ...
                    BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_ndx) {
                        ...
                        soc_sand_rv = soc_ppd_eg_encap_port_erspan_set(unit, soc_ppd_port, value);
                        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                    }
        ...
        case bcmPortControlOamDefaultProfile:
            {
                ...
                if (SOC_IS_ARADPLUS(unit)) {
                    ...
                    case 0:  /* Advanced mode */
                        rv = _bcm_petra_port_inlif_profile_get(unit, port, &inlif_profile, SOC_PPC_PROFILE_MGMT_INLIF_PROFILE_TYPE_USER, &lif_entry);
                        BCMDNX_IF_ERR_EXIT_MSG(rv, (_BSL_BCM_MSG("failed getting the inlif profile")));
                    ...
                    case 2:  /* Simple mode - 2 bits */
                        ...
                        _bcm_petra_port_inlif_profile_set(unit, port, value, SOC_PPC_PROFILE_MGMT_INLIF_PROFILE_TYPE_OAM, &lif_entry);
                        break;
                    default:
...
STATIC int
_bcm_petra_port_control_lanes_set(int unit, bcm_port_t port, int value)
{
    BCMDNX_INIT_FUNC_DEFS;
    ...
    if (SOC_IS_ARAD(unit)) {
        ...
        int dev_id;
        int rv;
        uint32 channel, flags, is_hg, offset;
        uint32 first_port, is_valid, is_initialized;
        bcm_port_if_t intf;
        int i, core;
        ...
        BCMDNX_IF_ERR_EXIT(soc_port_sw_db_interface_type_get(unit, port, &intf));

        /*if port is active - can't change num of lanes*/
        BCMDNX_IF_ERR_EXIT(soc_port_sw_db_is_initialized_get(unit, port, &is_initialized));
        if(is_initialized) {
            if(intf == BCM_PORT_IF_ILKN){
                BCMDNX_IF_ERR_EXIT(bcm_petra_port_enable_set(unit, port, 0));
                rv = soc_pm_ilkn_dynamic_num_of_lanes_set(unit, port , value);
                BCMDNX_IF_ERR_EXIT(bcm_petra_port_enable_set(unit, port, 1));
                BCMDNX_IF_ERR_EXIT(rv);
                BCM_EXIT;
            }
            else {
                BCMDNX_ERR_EXIT_MSG(BCM_E_PORT, (_BSL_BCM_MSG("Port is initialized %d"), port));
            }
        }
       
        /*collect info before removing the port*/
        BCMDNX_IF_ERR_EXIT(soc_port_sw_db_channel_get(unit, port, &channel));
        BCMDNX_IF_ERR_EXIT(soc_port_sw_db_flags_get(unit, port, &flags));
        BCMDNX_IF_ERR_EXIT(soc_port_sw_db_first_phy_port_get(unit, port, &first_port));
        BCMDNX_IF_ERR_EXIT(soc_port_sw_db_is_hg_get(unit, port, &is_hg));
        BCMDNX_IF_ERR_EXIT(soc_port_sw_db_protocol_offset_get(unit, port, &offset)); 
        
        /*remove port*/
        BCMDNX_IF_ERR_EXIT(soc_pm_port_remove(unit, port));
        
        BCM_PBMP_CLEAR(phy_ports);
        
        if (intf == BCM_PORT_IF_ILKN) {
            if(((value > 12) && (first_port != 1)) || (value > 24)){
                BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Number of lanes (%d) is out of range"), value));
            }
            for(i = 0 ; i < value ; i++){
                if(i < 12) {
                    BCM_PBMP_PORT_ADD(phy_ports, first_port + i);
                } else{
                    BCM_PBMP_PORT_ADD(phy_ports, 40 - i);
                }
            }
        } else if(intf == BCM_PORT_IF_CAUI) {
            for(i = 0 ; i < value ; i++){
                BCM_PBMP_PORT_ADD(phy_ports, first_port + i);
            }
        } else{
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("unsupported interface type %d"), intf));
        }
        
        /*add port and restore it's params*/
        BCMDNX_IF_ERR_EXIT(soc_port_sw_db_port_validate_and_add(unit, port, 0, channel, flags, intf, phy_ports));
        BCMDNX_IF_ERR_EXIT(soc_pm_init(unit, phy_ports, SOC_DPP_FIRST_DIRECT_PORT(unit)));

        BCMDNX_IF_ERR_EXIT(soc_port_sw_db_protocol_offset_set(unit, port, offset));
        BCMDNX_IF_ERR_EXIT(soc_port_sw_db_is_hg_set(unit, port, is_hg));

        BCMDNX_IF_ERR_EXIT(soc_pm_default_speed_get(unit, port, &speed));
        BCMDNX_IF_ERR_EXIT(bcm_petra_port_speed_set(unit, port, speed));

        if (intf == BCM_PORT_IF_CAUI) {
            BCMDNX_IF_ERR_EXIT(soc_pm_port_clp_reset(unit, port));
        } else if (intf == BCM_PORT_IF_ILKN) {
        ...
        BCM_SAND_IF_ERR_EXIT(arad_port_to_nif_type(unit, tm_port, &nif_type));
        mapping_info.if_id = arad_nif_intern2nif_id(dev_id, nif_type, first_phy_port-1);
        BCM_SAND_IF_ERR_EXIT(arad_port_to_dynamic_interface_map_set(unit, core, tm_port, ARAD_PORT_DIRECTION_BOTH, &mapping_info, FALSE));
        /*ARAD egress queueing configuration update */
        /*1. Disable Higher bandwidth requests for ports above 100g or higher*/
        /*2. Enable for any other port*/
        BCMDNX_IF_ERR_EXIT(arad_egr_q_fqp_scheduler_config(dev_id));
    }
...
_bcm_petra_port_control_lanes_get(int unit, bcm_port_t port, int* value)
{
    ...
    if (SOC_IS_ARAD(unit)) {
        ...
        BCMDNX_IF_ERR_EXIT(soc_port_sw_db_interface_type_get(unit, port, &intf));
        /*if ilkn read from the hardware*/
        if(intf == BCM_PORT_IF_ILKN){
            BCMDNX_IF_ERR_EXIT(soc_pm_ilkn_dynamic_num_of_lanes_get(unit , port, &lane_count));
        }
        else{
            BCMDNX_IF_ERR_EXIT(soc_port_sw_db_num_lanes_get(unit, port, &lane_count));
        }
        *value = lane_count;
    }
                            ...
                            bcm_port_control_t type, int value)
{
    ...
    switch (type)
    {
      ...
      case bcmPortControlLinkFaultRemote:
        ...
        if (SOC_PORT_VALID_RANGE(unit,local_port) && BCM_PBMP_MEMBER(PBMP_PORT_ALL(unit), local_port) && !BCM_PBMP_MEMBER(PBMP_SFI_ALL(unit), local_port)) {
    ...
    case bcmPortControlLanes:
        /* change nof_lanes */
        rv = _bcm_petra_port_control_lanes_set(unit, port, value);
        break;
    case bcmPortControlRxEnable :
        if (!SOC_IS_PETRAB(unit)) 
        {
            ...
            if (value == 0)
            {
                soc_sand_rv = (MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_tdm_ifp_get,(unit, port,&is_tdm)));
                BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                if (is_tdm)
                {
    ...
    case bcmPortControlEgressModifyDscp:
        if (SOC_IS_ARADPLUS(unit)) {
            ...
            if (simple_mode) {
                ...
                rv = _bcm_petra_port_inlif_profile_set(unit, port, value, SOC_PPC_PROFILE_MGMT_INLIF_PROFILE_TYPE_SIMPLE_DSCP_MARKING, lif_info);
                BCMDNX_IF_ERR_EXIT_MSG(rv, (_BSL_BCM_MSG("failed setting the inlif profile")));
                                ...
                                bcm_port_control_t type, int *value)
{
    ...
    if (SOC_IS_PETRAB(unit)) { 
        ...
        switch (type) {
            case bcmPortControlPrbsPolynomial:
                rv = _bcm_petra_port_nif_prbs_polynomial_get(soc_sand_dev_id, nif_id, value);
                break;
        
            case bcmPortControlPrbsRxStatus:
                rv = _bcm_petra_port_nif_prbs_result_get(soc_sand_dev_id, nif_id, value);
                break;
          ...
          case bcmPortControlLLFCTransmit:
              ...
              if(fc_tx_info.mode == SOC_TMC_FC_INBND_MODE_DISABLED)
              {
                *value = 0;
              }
              else
              {
                if(type == bcmPortControlPFCTransmit)
                {
                  *value = (fc_tx_info.mode == SOC_TMC_FC_INBND_MODE_CB) ? 1 : 0;
                }
                else
                {
                  *value = (fc_tx_info.mode == SOC_TMC_FC_INBND_MODE_LL) ? 1 : 0;
                }
              }
            ...
            case bcmPortControlLLFCReceive:
                ...
                if(fc_rx_info.mode == SOC_TMC_FC_INBND_MODE_DISABLED)
                {
                  *value = 0;
                }
                else
                {
                  if(type == bcmPortControlPFCReceive)
                  {
                    *value = (fc_rx_info.mode == SOC_TMC_FC_INBND_MODE_CB) ? 1 : 0;
                  }
                  else
                  {
                    *value = (fc_rx_info.mode == SOC_TMC_FC_INBND_MODE_LL) ? 1 : 0;
                  }         
                }
    ...
    if (SOC_IS_ARAD(unit)) {
      ...
      switch(type) {
      case bcmPortControlLinkDownPowerOn:
          BCMDNX_IF_ERR_EXIT(soc_pm_enable_get(unit, port, (uint32*)value));
          break;
    ...
    case bcmPortControlPFCRefreshTime:
          ...
          *value = fc_pfc_refresh_timer_info.enable ? fc_pfc_refresh_timer_info.refresh_time : 0;
    ...
    case bcmPortControlSAFCTransmit:
        ...
        else
        {
          if(type == bcmPortControlPFCTransmit)
          {
            *value = (fc_tx_info.mode == SOC_TMC_FC_INBND_MODE_PFC) ? 1 : 0;
          }
          if(type == bcmPortControlSAFCTransmit)
          {
            *value = (fc_tx_info.mode == SOC_TMC_FC_INBND_MODE_SAFC) ? 1 : 0;
          }
          if(type == bcmPortControlLLFCTransmit)
          {
            *value = (fc_tx_info.mode == SOC_TMC_FC_INBND_MODE_LL) ? 1 : 0;
    ...
    case bcmPortControlSAFCReceive:
        ...
        else
        {
          if(type == bcmPortControlPFCReceive)
          {
            *value = (fc_rx_info.mode == SOC_TMC_FC_INBND_MODE_PFC) ? 1 : 0;
          }
          if(type == bcmPortControlSAFCReceive)
          {
            *value = (fc_rx_info.mode == SOC_TMC_FC_INBND_MODE_SAFC) ? 1 : 0;
          }
          if(type == bcmPortControlLLFCReceive)
          {
            *value = (fc_rx_info.mode == SOC_TMC_FC_INBND_MODE_LL) ? 1 : 0;
                                ...
                                bcm_port_control_t type, int *value)
{
    ...
    if (SOC_IS_PETRAB(unit)) { 
        ...
        switch (type) {
            case bcmPortControlPrbsPolynomial:
                rv = _bcm_petra_port_serdes_range_prbs_polynomial_get(soc_sand_dev_id, lane,
                                                              1, value);
                break;
        
            case bcmPortControlPrbsRxStatus:
                rv = _bcm_petra_port_serdes_range_prbs_result_get(soc_sand_dev_id, lane, 1,
                                                          value);
                break;
    ...
    if (SOC_IS_ARAD(unit)) {
        ...
        switch (type) {
            case bcmPortControlPCS:
                rv = _bcm_petra_port_fabric_pcs_control_get(soc_sand_dev_id,link,value);
                break;
            ...
            case bcmPortControlLinkDownPowerOn:
                {
                    ARAD_LINK_STATE_INFO info;
                    arad_ARAD_LINK_STATE_INFO_clear(&info);
                    soc_sand_rv = arad_link_on_off_get(unit, link, &info); 
                    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                    *value = info.on_off;
                }
                break;
            ...
            case bcmPortControlStripCRC:
                soc_sand_rv = arad_link_control_strip_crc_get(unit, port, value);
                BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                break;

            case bcmPortControlRxEnable:
                soc_sand_rv = arad_link_control_rx_enable_get(unit, link, value);
                BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                break;

            case bcmPortControlTxEnable:
                soc_sand_rv = arad_link_control_tx_enable_get(unit, link, value);
                BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                break;

            case bcmPortControlPrbsPolynomial:
                if(SOC_PBMP_MEMBER(_bcm_dpp_port_prbs_mac_mode[unit],port)) {
                    BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("Control %d isn't supported for MAC PRBS\n"), type));
                } else {
                    MIIM_LOCK(unit);
                    locked = 1;
            ...
            case bcmPortControlPrbsTxEnable:
                if(SOC_PBMP_MEMBER(_bcm_dpp_port_prbs_mac_mode[unit],port)) {
                    soc_sand_rv = arad_port_prbs_tx_enable_get(unit, link, value);
                    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                } else {
                    MIIM_LOCK(unit);
                    locked = 1;
            ...
            case bcmPortControlPrbsRxEnable:
                if(SOC_PBMP_MEMBER(_bcm_dpp_port_prbs_mac_mode[unit],port)) {
                    soc_sand_rv = arad_port_prbs_rx_enable_get(unit, link, value);
                    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                } else {
                    MIIM_LOCK(unit);
                    locked = 1;
            ...
            case bcmPortControlPrbsRxStatus:
                if(SOC_PBMP_MEMBER(_bcm_dpp_port_prbs_mac_mode[unit],port)) {
                    soc_sand_rv = arad_port_prbs_rx_status_get(unit, link, value);
                    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                } else {
                    MIIM_LOCK(unit);
                    locked = 1;
            ...
            case bcmPortControlLowLatencyLLFCEnable:
                rv=MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_port_control_low_latency_get,(unit, link,(int *)value));
                BCM_SAND_IF_ERR_EXIT(rv);
                break;

            case bcmPortControlFecErrorDetectEnable:
                rv=MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_port_control_fec_error_detect_get,(unit,link,(int *)value));
                BCM_SAND_IF_ERR_EXIT(rv);
                break;
            default:
    ...
    if (SOC_DPP_PP_ENABLE(unit)) {
      ...
      case bcmPortControlFcoeFabricId:
          ...
          if(!SOC_DPP_CONFIG(unit)->pp.fcoe_enable ){
              ...
              soc_ppd_port = 0;
                  ...
                  ctrl_indx.val_ndx = soc_ppd_port;
                  ctrl_indx.type = SOC_PPD_FP_CONTROL_TYPE_FLP_PP_PORT_DATA;
                  soc_sand_rv = soc_ppd_fp_control_get(unit,&ctrl_indx,&ctrl_info);
                  BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
      ...
      case bcmPortControlDiscardMacSaAction:
      {
            ...
            BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_ndx) {
              ...
              rv = _bcm_dpp_am_template_port_mact_sa_drop_data_get(unit,soc_ppd_port,&action_profile);
              BCMDNX_IF_ERR_EXIT(rv);
      ...
      case bcmPortControlUnknownMacDaAction:
        {
            ...
            BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_ndx) {
              ...
              rv = _bcm_dpp_am_template_port_mact_da_unknown_data_get(unit,soc_ppd_port,&action_profile);
              BCMDNX_IF_ERR_EXIT(rv);
      ...
      case bcmPortControlBridge:
        {
          if (BCM_GPORT_IS_VLAN_PORT(port)) {
            ...
            if (simple_mode) {
              ...
              rv = _bcm_petra_port_inlif_profile_get(unit, port, (uint32*)value, SOC_PPC_PROFILE_MGMT_INLIF_PROFILE_TYPE_SIMPLE_SAME_INTERFACE, lif_info);
              BCMDNX_IF_ERR_EXIT_MSG(rv, (_BSL_BCM_MSG("failed getting the inlif profile")));
      ...
      case bcmPortControlPreservePacketPriority: 
        {
            ...
            BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_ndx) {
                ...
                soc_sand_rv = soc_ppd_eg_vlan_edit_port_vlan_transmit_outer_tag_get(unit, soc_ppd_port, 0, &transmit_outer_tag);
                BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
         ...
         case bcmPortControlL2SaAuthenticaion: 
          {
              ...
              BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_ndx) {
                  ...
                  soc_sand_rv = soc_ppd_llp_sa_auth_port_info_get(unit, soc_ppd_port, &port_auth_info);
                  BCM_SAND_IF_ERR_EXIT(soc_sand_rv);                 
          ...
          case bcmPortControlErspanEnable:
          {
              if (SOC_IS_ARAD_B1_AND_BELOW(unit)) {
                  ...
                  BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_ndx) {
                      ...
                      soc_sand_rv = soc_ppd_eg_encap_port_erspan_get(unit, soc_ppd_port, &is_erspan);
                      BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        ...
        case bcmPortControlOamDefaultProfile:
            {
                ...
                if (SOC_IS_ARADPLUS(unit)) {
                    ...
                    case 1:  /* Simple mode - 1 bit */
                    case 2:  /* Simple mode - 2 bits */
                        _bcm_petra_port_inlif_profile_get(unit, port, &oam_trap_profile, SOC_PPC_PROFILE_MGMT_INLIF_PROFILE_TYPE_OAM, &lif_entry);
                        break;
                    default:
                        BCMDNX_ERR_EXIT_MSG(BCM_E_CONFIG, (_BSL_BCM_MSG("OAM default profile mode unsupported (profile = %d)"), oam_lif_profile_mode));
                    }
    ...
    case bcmPortControlEgressModifyDscp:
        if (SOC_IS_ARADPLUS(unit)) {
            ...
            if (simple_mode) {
                ...
                rv = _bcm_petra_port_inlif_profile_get(unit, port, &inlif_profile, SOC_PPC_PROFILE_MGMT_INLIF_PROFILE_TYPE_SIMPLE_DSCP_MARKING, lif_info);
...
_bcm_petra_global_control_get(int unit, bcm_port_control_t type, int *value)
{
    ...
    case bcmPortControlDropPrecedence:
        SOC_PPD_LLP_COS_GLBL_INFO_clear(&glbl_info);
        soc_sand_rv = soc_ppd_llp_cos_glbl_info_get(soc_sand_dev_id, &glbl_info);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                            ...
                            bcm_port_control_t type, int *value)
{
    ...
    if (port == -1) { 
        ...
        if (SOC_PORT_VALID_RANGE(unit,local_port) && BCM_PBMP_MEMBER(PBMP_SFI_ALL(unit), local_port)) {
            rv = _bcm_petra_port_fabric_control_get(unit, local_port, type, value);
        } else {
            /* TM / PP */
            rv = _bcm_petra_port_control_get(unit, port, type, value);
        } 
 ...
 */
int
_bcm_arad_port_phy_set(int unit, bcm_port_t port, uint32 flags,
                 uint32 phy_reg_addr, uint32 phy_data)
{
    ...
    if (!(flags & BCM_PORT_PHY_NOMAP)) {
        BCMDNX_IF_ERR_EXIT(_bcm_petra_port_gport_validate(unit, port, &port));
    }

    MIIM_LOCK(unit);

    clause = (flags & BCM_PORT_PHY_CLAUSE45 ? 45 : 22);
    ...
    if (flags & (SOC_PHY_I2C_DATA8 | SOC_PHY_I2C_DATA16)) {
        ...
        if (reg_flag & SOC_PHY_REG_INDIRECT) {
            if (flags & BCM_PORT_PHY_NOMAP) {
                 ...
                 */
                BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("indirect register access is not supported if BCM_PORT_PHY_NOMAP flag is set")));
            }
...
int
_bcm_arad_port_phy_get(int unit, bcm_port_t port, uint32 flags,
                 uint32 phy_reg_addr, uint32 *phy_data)
{
    uint32  phy_id;
    uint32 phy_reg;
...
/* 
 *  ARAD only functions for phy control configuration }
 */
#endif /* BCM_ARAD_SUPPORT */

/*
 * Function:
 *      bcm_petra_port_phy_reset
 * Description:
 *      This function performs the low-level PHY reset and is intended to be
 *      called ONLY from callback function registered with
 *      bcm_petra_port_phy_reset_register. Attempting to call it from any other
...
bcm_petra_port_phy_reset(int unit, bcm_port_t port)
{
    ...
    if (SOC_IS_PETRAB(unit)) 
    {
      /* phyctrl is not supported for Petra-B */
      BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("bcm_petra_port_phy_reset unavaliable for Petra"))); 
    }
#endif /* BCM_PETRAB_SUPPORT */
#ifdef BCM_ARAD_SUPPORT
    if (SOC_IS_ARAD(unit))
    {
        ...
        if(!SOC_PORT_IS_NETWORK_INTERFACE(flags)) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PORT, (_BSL_BCM_MSG("Port %d isn't NIF"), port));
        }
 ...
 */
int
bcm_petra_port_phy_reset_register(int unit, bcm_port_t port,
...
#ifdef BCM_PETRAB_SUPPORT
    if (SOC_IS_PETRAB(unit)) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("bcm_petra_port_phy_reset_register is not supported in Petra-B")));
    }
#endif
#ifdef BCM_ARAD_SUPPORT
    if(SOC_IS_ARAD(unit)){
        BCMDNX_IF_ERR_EXIT(soc_phy_reset_register(unit, port, callback, user_data, FALSE));
    }
#endif
 ...
 */
int
bcm_petra_port_phy_reset_unregister(int unit, bcm_port_t port,
                              bcm_port_phy_reset_cb_t callback,
                              void  *user_data)
{
    BCMDNX_INIT_FUNC_DEFS;
...
#ifdef BCM_PETRAB_SUPPORT
    if (SOC_IS_PETRAB(unit)) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("bcm_petra_port_phy_reset_unregister is not supported in Petra-B")));
    }
#endif
#ifdef BCM_ARAD_SUPPORT
    if(SOC_IS_ARAD(unit)){
        BCMDNX_IF_ERR_EXIT(soc_phy_reset_unregister(unit, port, callback, user_data));
    }
#endif
...
int _bcm_external_phy_gport_params_get(int unit, bcm_port_t port, uint8 *is_gport,
                                        bcm_port_t *local_port, int *phy_lane,
                                        int *phyn, int *sys_side){
    BCMDNX_INIT_FUNC_DEFS;
    /*set default values*/
    *phy_lane = -1;
    *phyn = 0;
    ...
    *is_gport = TRUE;
    if(BCM_PHY_GPORT_IS_PHYN(port)) {
        ...
        *phyn = BCM_PHY_GPORT_PHYN_PORT_PHYN_GET(port); /* check this out.. */
...
/*
 * Function:
 *     bcm_petra_port_phy_control_set
 * Description:
...
bcm_petra_port_phy_control_set(int unit, bcm_port_t port, bcm_port_phy_control_t type, uint32 value)
{
...
#ifdef  BCM_PETRAB_SUPPORT
    if(SOC_IS_PETRAB(unit)) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("bcm_petra_port_phy_control_set is not supported in Petra-B")));
    } else 
#endif /* BCM_PETRAB_SUPPORT */
    ...
    if(SOC_IS_ARAD(unit)){
        ...
        BCMDNX_IF_ERR_EXIT(_bcm_external_phy_gport_params_get(unit, port, &is_gport, &local_port, &phy_lane,
                                        &phyn, &sys_side));
        if(is_gport){
            if (!SOC_PORT_VALID(unit, local_port)) {
            ...
            /*PORT_LOCK(unit);*/
            MIIM_LOCK(unit);
            rv = soc_phyctrl_redirect_control_set(unit, local_port, phyn, phy_lane, sys_side, (soc_phy_control_t)type, value);
            MIIM_UNLOCK(unit);
            /*PORT_UNLOCK(unit);*/
            ...
            /*closing port before enable cl72*/
            ...
            /*enable port back in case of cl72*/
            if(BCM_PORT_PHY_CONTROL_CL72 == type) {
                if(enable) {
                    BCMDNX_IF_ERR_EXIT(bcm_petra_port_enable_set(unit, port, 1));
                }
            }
            /*check for error in the control set just after enable the port*/
            BCMDNX_IF_ERR_EXIT(rv);
        }
    } else
#endif /* BCM_ARAD_SUPPORT */

    BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("bcm_petra_port_phy_control_set is not supported")));
...
/*
 * Function:
 *     bcm_petra_port_phy_control_get
 * Description:
 *     Set PHY specific properties 
 * Parameters:
 *     unit        device number
                             ...
                             bcm_port_phy_control_t type, uint32 *value)
{
...
#ifdef  BCM_PETRAB_SUPPORT
    if(SOC_IS_PETRAB(unit)) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("bcm_petra_port_phy_control_get is not supported in Petra-B")));
    } else 
#endif /* BCM_PETRAB_SUPPORT */
#ifdef BCM_ARAD_SUPPORT
    if(SOC_IS_ARAD(unit)){
        ...
        if(is_gport){
            ...
            /*PORT_LOCK(unit);*/
            MIIM_LOCK(unit);
            rv = soc_phyctrl_redirect_control_get(unit, local_port, phyn, phy_lane, sys_side, (soc_phy_control_t)type, value);
            MIIM_UNLOCK(unit);
            /*PORT_UNLOCK(unit);*/
    ...
    BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("bcm_petra_port_phy_control_get is not supported")));
...
#ifdef BCM_ARAD_SUPPORT
    if(SOC_IS_ARAD(unit)) {
        BCMDNX_IF_ERR_EXIT(_bcm_arad_port_phy_get(unit, port, flags, phy_reg_addr, phy_data));
    } else 
#endif /* BCM_ARAD_SUPPORT */

    BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("bcm_petra_port_phy_get isn't supported")));
...
#ifdef BCM_ARAD_SUPPORT
    if(SOC_IS_ARAD(unit)) {
        BCMDNX_IF_ERR_EXIT(_bcm_arad_port_phy_set(unit, port, flags, phy_reg_addr, phy_data));
    } else 
#endif /* BCM_ARAD_SUPPORT */

    BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("bcm_petra_port_phy_set isn't supported")));
    ...
    PBMP_ITER(pbmp, port) {
        ...
        if (SOC_IS_ARAD(unit)) {
            if (SOC_PBMP_MEMBER(SOC_INFO(unit).cmic_bitmap, port)) {
                ...
                SOCDNX_IF_ERR_EXIT(bcm_port_linkscan_set(unit, port, BCM_LINKSCAN_MODE_NONE));
                SOC_IF_ERROR_RETURN(soc_pm_port_deinit(unit, port));
            }
        }
#endif /* BCM_ARAD_SUPPORT */
...
bcm_petra_port_pause_addr_get(int unit, bcm_port_t port, bcm_mac_t mac)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    /* Get the MAL ID from the port number */
    rv = _bcm_petra_port_to_nif_id(unit, port, &nif_id);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    if (SOC_IS_PETRAB(unit)) {
      ...
      mal_id = SOC_PB_IF2MAL_NDX(nif_id);
      
      soc_sand_dev_id = (unit);
      soc_sand_rv = soc_pb_fc_nif_pause_frame_src_addr_get(soc_sand_dev_id, mal_id, &soc_sand_mac);
      BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
      
      /* The SOC_SAND MAC address is little-endian */
      for (i = 0; i < SOC_SAND_PP_MAC_ADDRESS_NOF_U8; ++i) {
          mac[i] = soc_sand_mac.address[SOC_SAND_PP_MAC_ADDRESS_NOF_U8 - i - 1];
      }
    } 
#endif /* BCM_PETRAB_SUPPORT */
#ifdef  BCM_ARAD_SUPPORT
    if (SOC_IS_ARAD(unit)) {
        ...
        soc_sand_dev_id = (unit);
        soc_sand_SAND_PP_MAC_ADDRESS_clear(&soc_sand_mac);

        soc_sand_rv = arad_fc_nif_pause_frame_src_addr_get(soc_sand_dev_id, nif_id, &soc_sand_mac);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);     
...
bcm_petra_port_pause_addr_set(int unit, bcm_port_t port, bcm_mac_t mac)
{
    ...
    if (SOC_IS_PETRAB(unit)) {
      ...
      soc_sand_SAND_PP_MAC_ADDRESS_clear(&soc_sand_mac);
      for (i = 0; i < SOC_SAND_PP_MAC_ADDRESS_NOF_U8; ++i) {
          soc_sand_mac.address[SOC_SAND_PP_MAC_ADDRESS_NOF_U8 - i - 1] = mac[i];
      }
      
      soc_sand_dev_id = (unit);
      soc_sand_rv = soc_pb_fc_nif_pause_frame_src_addr_set(soc_sand_dev_id, mal_id, &soc_sand_mac);
      BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    } 
#endif /* BCM_PETRAB_SUPPORT */
#ifdef  BCM_ARAD_SUPPORT
    if (SOC_IS_ARAD(unit)) {
      ...
      soc_sand_rv = arad_fc_nif_pause_frame_src_addr_set(soc_sand_dev_id, nif_id, &soc_sand_mac);
      BCM_SAND_IF_ERR_EXIT(soc_sand_rv);     
    }
#endif  /* BCM_ARAD_SUPPORT */
...
bcm_petra_port_pause_get(int unit, bcm_port_t port, int *pause_tx, int *pause_rx)
{
    ...
    int                  nif_id;
    int                  soc_sand_rv;
    SOC_TMC_FC_REC_INBND_INFO rec_info;
    SOC_TMC_FC_GEN_INBND_INFO gen_info;
    BCMDNX_INIT_FUNC_DEFS;
    
    /* Get the NIF ID from the port number */
    rv = _bcm_petra_port_to_nif_id(unit, port, &nif_id);
    BCMDNX_IF_ERR_EXIT(rv);


    if ((nif_id == SOC_TMC_NIF_ID_ILKN_0) || (nif_id == SOC_TMC_NIF_ID_ILKN_1)) {
        *pause_rx = *pause_tx = 0;
        BCM_EXIT;
    }
      
    SOC_TMC_FC_REC_INBND_INFO_clear(&rec_info);
    soc_sand_rv = (MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_fc_rec_inbnd_get,(unit, nif_id, &rec_info)));
    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    *pause_rx = rec_info.mode == SOC_TMC_FC_INBND_MODE_LL ? 1 : 0;
      
    SOC_TMC_FC_GEN_INBND_INFO_clear(&gen_info);
    soc_sand_rv = (MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_fc_gen_inbnd_get,(unit, nif_id, &gen_info)));
    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
...
bcm_petra_port_pause_set(int unit, bcm_port_t port, int pause_tx, int pause_rx)
{
    ...
    if ((nif_id == SOC_TMC_NIF_ID_ILKN_0) || (nif_id == SOC_TMC_NIF_ID_ILKN_1)) {
      ...
      SOC_TMC_FC_REC_INBND_INFO_clear(&rec_info);
      rec_info.mode = pause_rx ? SOC_TMC_FC_INBND_MODE_LL : SOC_TMC_FC_INBND_MODE_DISABLED;
      soc_sand_rv = (MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_fc_rec_inbnd_set,(unit, nif_id, &rec_info)));
      BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
      
      SOC_TMC_FC_GEN_INBND_INFO_clear(&gen_info);
      gen_info.mode = pause_tx ? SOC_TMC_FC_INBND_MODE_LL : SOC_TMC_FC_INBND_MODE_DISABLED;
      gen_info.ll.cnm_enable = TRUE;
      soc_sand_rv = (MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_fc_gen_inbnd_set,(unit, nif_id, &gen_info)));
      BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                     ...
                     int ifg)
{
...
#ifdef BCM_PETRAB_SUPPORT
    int rv = BCM_E_NONE;
    SOC_PB_NIF_SPAUI_INFO info_rx, info_tx;
#endif
    BCMDNX_INIT_FUNC_DEFS;
    ...
    { 
      ...
      soc_sand_rv = arad_nif_ipg_set(soc_sand_dev_id, port, ifg);
      BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    ...
    if (SOC_IS_PETRAB(unit)) {
      ...
      if (!SOC_PB_NIF_IS_TYPE_ID(XAUI, nif_id) && !SOC_PB_NIF_IS_TYPE_ID(RXAUI, nif_id)) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("Only supported in XAUI and RXAUI modes")));
      }
        ...
        if ((ifg < 1) || (ifg > 255)) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Interframe gap %d out of range 0 < ifg < 256"), ifg));
        }

        soc_sand_rv = soc_pb_nif_spaui_conf_get(soc_sand_dev_id, nif_id, &info_rx, &info_tx);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        ...
        info_tx.ipg.enable = 1;
        info_tx.ipg.size = ifg;
        info_tx.ipg.dic_mode = SOC_PB_NIF_IPG_DIC_MODE_AVERAGE;

        info_tx.preamble.enable = 1;
        info_tx.preamble.size = SOC_PB_NIF_PRMBL_SIZE_COLUMNS_0;
        info_tx.preamble.skip_SOP = 1;

        /* 
         * CURRENTLY UNCONFIGURED.
         * NEEDS VALID SETTING
         */
        if (info_tx.fault_response.local == SOC_PB_NIF_NOF_FAULT_RESPONSES) {
            info_tx.fault_response.local = SOC_PB_NIF_FAULT_RESPONSE_DATA_AND_IDLE;
        }
        if (info_tx.fault_response.remote == SOC_PB_NIF_NOF_FAULT_RESPONSES) {
            info_tx.fault_response.remote = SOC_PB_NIF_FAULT_RESPONSE_DATA_AND_IDLE;
        }

        soc_sand_rv = soc_pb_nif_spaui_conf_set(soc_sand_dev_id, nif_id, SOC_TMC_CONNECTION_DIRECTION_TX, &info_tx);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);

        /*
         * RX setting (ipg ignored) 
         */

        info_rx.ipg.enable = 1;
        info_rx.ipg.size = ifg;
        info_rx.ipg.dic_mode = SOC_PB_NIF_IPG_DIC_MODE_AVERAGE;

        info_rx.preamble.enable = 1;
        info_rx.preamble.size = SOC_PB_NIF_PRMBL_SIZE_COLUMNS_0;
        info_rx.preamble.skip_SOP = 1;
        
        /* 
         * CURRENTLY UNCONFIGURED.
         * NEEDS VALID SETTING
         */
        if (info_rx.fault_response.local == SOC_PB_NIF_NOF_FAULT_RESPONSES) {
            info_rx.fault_response.local = SOC_PB_NIF_FAULT_RESPONSE_DATA_AND_IDLE;
        }
        if (info_rx.fault_response.remote == SOC_PB_NIF_NOF_FAULT_RESPONSES) {
            info_rx.fault_response.remote = SOC_PB_NIF_FAULT_RESPONSE_DATA_AND_IDLE;
        }

        soc_sand_rv = soc_pb_nif_spaui_conf_set(soc_sand_dev_id, nif_id, SOC_TMC_CONNECTION_DIRECTION_RX, &info_rx);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        BCM_EXIT;
    }
#endif
    SOC_IF_ERROR_RETURN(BCM_E_UNAVAIL);
    BCM_EXIT;
exit:
    _BCM_DPP_SWITCH_API_END(unit);
    BCMDNX_FUNC_RETURN;
                     ...
                     int *ifg)
{
    ...
    {   
      ...
      soc_sand_rv = arad_nif_ipg_get(soc_sand_dev_id, port, &ifg_val);
      BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
...
bcm_petra_port_selective_get(int unit, bcm_port_t port, bcm_port_info_t *info)
{
    ...
    if (BCM_PBMP_MEMBER(SOC_PORT_DISABLED_BITMAP(unit, all), port)) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_DISABLED, (_BSL_BCM_MSG("port %d is disabled"), port));
    }
    ...
    if (!IS_SFI_PORT(unit, port)) {
        ...
        /* Additional masks irrelevant for PB */
        if (SOC_IS_PETRAB(unit)) {
            mask &= ~(BCM_PORT_ATTR_AUTONEG_MASK);
        }
        ...
        if (SOC_IS_ARAD(unit)) {
            ...
            BCMDNX_IF_ERR_EXIT(soc_port_sw_db_is_valid_port_get(unit, port, &is_valid));
            if(!is_valid) {
                BCMDNX_ERR_EXIT_MSG(BCM_E_PORT, (_BSL_BCM_MSG("invalid local port %d"), port));
            }
            BCMDNX_IF_ERR_EXIT(soc_port_sw_db_interface_type_get(unit, port, &interface));
            if(BCM_PORT_IF_ILKN == interface) {
                mask = mask & ~(BCM_PORT_ATTR_FRAME_MAX_MASK);
            }
            BCMDNX_IF_ERR_EXIT(soc_port_sw_db_flags_get(unit, port, &flags));
            if(!SOC_PORT_IS_NETWORK_INTERFACE(flags)) {
    ...
    if (mask & BCM_PORT_ATTR_ENABLE_MASK) {
        rv = bcm_petra_port_enable_get(unit, port, &info->enable);
        if (rv != BCM_E_NONE) {
    ...
    if (mask & BCM_PORT_ATTR_LINKSTAT_MASK) {
        rv = bcm_petra_port_link_status_get(unit, port, &info->linkstatus);
        if (rv != BCM_E_NONE) {
    ...
    if (mask & BCM_PORT_ATTR_SPEED_MASK) {
        if ((rv = bcm_petra_port_speed_get(unit, port, &info->speed)) < 0) {
            if (rv != BCM_E_BUSY) {
                if (rv != BCM_E_NONE) {
    ...
    if (mask & (BCM_PORT_ATTR_PAUSE_TX_MASK |
                BCM_PORT_ATTR_PAUSE_RX_MASK)) {
        rv = bcm_petra_port_pause_get(unit, port,
    ...
    if (mask & BCM_PORT_ATTR_LINKSCAN_MASK) {
        rv = bcm_petra_port_linkscan_get(unit, port, &info->linkscan);
        if (BCM_FAILURE(rv)) {
            BCMDNX_ERR_EXIT_MSG(rv, (_BSL_BCM_MSG("bcm_petra_port_linkscan_get failed: %s"), bcm_errmsg(rv)));
        }
    }

    if (IS_E_PORT(unit, port) && (mask & BCM_PORT_ATTR_LEARN_MASK)) {
        rv = bcm_petra_port_learn_get(unit, port, &info->learn);
        if (BCM_FAILURE(rv)) {
            BCMDNX_ERR_EXIT_MSG(rv, (_BSL_BCM_MSG("bcm_petra_port_learn_get failed: %s"), bcm_errmsg(rv)));
        }
    }

    if (IS_E_PORT(unit, port) && (mask & BCM_PORT_ATTR_DISCARD_MASK)) {
        rv = bcm_petra_port_discard_get(unit, port, &info->discard);
        if (BCM_FAILURE(rv)) {
            BCMDNX_ERR_EXIT_MSG(rv, (_BSL_BCM_MSG("bcm_petra_port_discard_get failed: %s"), bcm_errmsg(rv)));
        }
    }

    if (IS_E_PORT(unit, port) && (mask & BCM_PORT_ATTR_VLANFILTER_MASK)) {
        rv = bcm_petra_port_vlan_member_get(unit, port, &info->vlanfilter);
        if (BCM_FAILURE(rv)) {
    ...
    if (IS_E_PORT(unit, port) && (mask & BCM_PORT_ATTR_UNTAG_PRI_MASK)) {
        rv = bcm_petra_port_untagged_priority_get(unit, port,
    ...
    if (IS_E_PORT(unit, port) && (mask & BCM_PORT_ATTR_UNTAG_VLAN_MASK)) {
        rv = bcm_petra_port_untagged_vlan_get(unit, port, &info->untagged_vlan);
        if (BCM_FAILURE(rv)) {
            BCMDNX_ERR_EXIT_MSG(rv, (_BSL_BCM_MSG("bcm_petra_port_untagged_vlan_get failed: %s"), bcm_errmsg(rv)));
        }
    }

    if (IS_E_PORT(unit, port) && (mask & BCM_PORT_ATTR_STP_STATE_MASK)) {
        rv = bcm_petra_port_stp_get(unit, port, &info->stp_state);
        if (BCM_FAILURE(rv)) {
    ...
    if (mask & BCM_PORT_ATTR_LOOPBACK_MASK) {
        rv = bcm_petra_port_loopback_get(unit, port, &info->loopback);
        if (BCM_FAILURE(rv)) {
    ...
    if (mask & BCM_PORT_ATTR_INTERFACE_MASK) {
        rv = bcm_petra_port_interface_get(unit, port, &info->interface);
        if (BCM_FAILURE(rv)) {
    ...
    if (mask & BCM_PORT_ATTR_SPEED_MAX_MASK) {
        rv = bcm_petra_port_speed_max(unit, port, &info->speed_max);
        if (BCM_FAILURE(rv)) {
    ...
    if (mask & BCM_PORT_ATTR_FRAME_MAX_MASK) {
        rv = bcm_petra_port_frame_max_get(unit, port, &info->frame_max);
        if (BCM_FAILURE(rv)) {
    ...
    rv = bcm_petra_port_encap_get(unit, port, &info->encap_mode);
        if (BCM_FAILURE(rv)) {
    ...
    if (mask & BCM_PORT_ATTR_AUTONEG_MASK) {
        rv = bcm_petra_port_autoneg_get(unit, port, &info->autoneg);
        if (BCM_FAILURE(rv)) {
    ...
    if (mask & BCM_PORT_ATTR_REMOTE_ADVERT_MASK) {
        if ((rv = bcm_petra_port_advert_remote_get(unit, port,
    ...
    if (mask & BCM_PORT_ATTR_DUPLEX_MASK) {
        rv = bcm_petra_port_duplex_get(unit, port, &info->duplex);
        if (BCM_FAILURE(rv)) {
    ...
    if (mask & BCM_PORT_ATTR_PAUSE_MAC_MASK) {
        rv = bcm_petra_port_pause_addr_get(unit, port, info->pause_mac);
        if (BCM_FAILURE(rv)) {
    ...
    if (mask & BCM_PORT_ATTR_ABILITY_MASK) {
        rv = bcm_petra_port_ability_get(unit, port, &info->ability);
        if (BCM_FAILURE(rv)) {
    ...
    if (mask & BCM_PORT_ATTR_FAULT_MASK) {
        rv = bcm_petra_port_fault_get(unit, port, &info->fault);
        if (BCM_FAILURE(rv)) {
...
bcm_petra_port_selective_set(int unit, bcm_port_t port, bcm_port_info_t *info)
{
    ...
    if (mask & BCM_PORT_ATTR_LINKSCAN_MASK) {
        rv = bcm_petra_port_linkscan_set(unit, port, info->linkscan);
        if (BCM_FAILURE(rv)) {
    ...
    if (mask & BCM_PORT_ATTR_ENABLE_MASK) {
        rv = bcm_petra_port_enable_set(unit, port, info->enable);
        if (rv != BCM_E_NONE) {
    ...
    if (mask & BCM_PORT_ATTR_INTERFACE_MASK) {
        rv = bcm_petra_port_interface_set(unit, port, info->interface);
        if (BCM_FAILURE(rv)) {
    ...
    if (mask & BCM_PORT_ATTR_SPEED_MASK) {
        rv = bcm_petra_port_speed_set(unit, port, info->speed);
        if (rv != BCM_E_NONE) {
    ...
    if (mask & (BCM_PORT_ATTR_PAUSE_TX_MASK |
        ...
        rv = bcm_petra_port_pause_set(unit, port, tpause, rpause);
        if (rv != BCM_E_NONE) {
    ...
    if (mask & BCM_PORT_ATTR_LEARN_MASK) {
        rv = bcm_petra_port_learn_set(unit, port, info->learn);
        if (BCM_FAILURE(rv)) {
    ...
    if (mask & BCM_PORT_ATTR_DISCARD_MASK) {
        rv = bcm_petra_port_discard_set(unit, port, info->discard);
        if (BCM_FAILURE(rv)) {
    ...
    if (mask & BCM_PORT_ATTR_VLANFILTER_MASK) {
        rv = bcm_petra_port_vlan_member_set(unit, port, info->vlanfilter);
        if (BCM_FAILURE(rv)) {
    ...
    if (mask & BCM_PORT_ATTR_UNTAG_PRI_MASK) {
        rv = bcm_petra_port_untagged_priority_set(unit, port,
                          info->untagged_priority);
        if (BCM_FAILURE(rv)) {
    ...
    if (mask & BCM_PORT_ATTR_UNTAG_VLAN_MASK) {
        rv = bcm_petra_port_untagged_vlan_set(unit, port, info->untagged_vlan);
        if (BCM_FAILURE(rv)) {
    ...
    if (mask & BCM_PORT_ATTR_LOOPBACK_MASK) {
        rv = bcm_petra_port_loopback_set(unit, port, info->loopback);
        if (BCM_FAILURE(rv)) {
    ...
    if (mask & BCM_PORT_ATTR_AUTONEG_MASK) {
        rv = bcm_petra_port_autoneg_set(unit, port, info->autoneg);
        if (BCM_FAILURE(rv)) {
    ...
    if (mask & BCM_PORT_ATTR_STP_STATE_MASK) {
        rv = bcm_petra_port_stp_set(unit, port, info->stp_state);
        if (BCM_FAILURE(rv)) {
    ...
    if (mask & BCM_PORT_ATTR_FRAME_MAX_MASK) {
        rv = bcm_petra_port_frame_max_set(unit, port, info->frame_max);
        if (BCM_FAILURE(rv)) {
    ...
    if (mask & BCM_PORT_ATTR_ENCAP_MASK) {
        rv = bcm_petra_port_encap_set(unit, port, info->encap_mode);
        if (BCM_FAILURE(rv)) {
    ...
    if (mask & BCM_PORT_ATTR_PAUSE_MAC_MASK) {
        rv = bcm_petra_port_pause_addr_set(unit, port, info->pause_mac);
        if (BCM_FAILURE(rv)) {
    ...
    if (mask & BCM_PORT_ATTR_DUPLEX_MASK) {
        rv = bcm_petra_port_duplex_set(unit, port, info->duplex);
        if (BCM_FAILURE(rv)) {
...
_bcm_petra_port_link_get(int unit, bcm_port_t port, int hw, int *up)
{
    ...
    if (SOC_IS_PETRAB(unit)) {
      int soc_sand_rv = 0;
      int nif_idx = 0;
      SOC_PB_NIF_LINK_STATUS  status;

       rv =_bcm_petra_port_to_nif_id(unit, port, &nif_idx);
       if (rv != BCM_E_NONE) {
           *up = 0;
           BCMDNX_ERR_EXIT_MSG(rv, (_BSL_BCM_MSG("%s: _bcm_petra_port_to_nif_id failed, unit %d"), FUNCTION_NAME(), unit));
       }

      if (hw) {
          BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("hw mode not supported for Petra")));
      }

      soc_sand_rv = soc_pb_nif_link_status_get(unit, nif_idx, &status);
      rv = handle_sand_result(soc_sand_rv);
      
      if (rv != BCM_E_NONE) {
      *up = 0;
      BCMDNX_ERR_EXIT_MSG(rv, (_BSL_BCM_MSG("%s: soc_pb_nif_link_status_get nif_idx %d failed, unit %d"), FUNCTION_NAME(), nif_idx, unit));
      }

      *up = status.link_status;
    } 
#endif /* BCM_PETRAB_SUPPORT */
#ifdef  BCM_ARAD_SUPPORT
    if (SOC_IS_ARAD(unit)) {
      ...
      if (!IS_SFI_PORT(unit, port) /*nif port*/) {
          BCMDNX_IF_ERR_EXIT(soc_pm_link_status_get(unit, port, up, &down));
      } else if (IS_SFI_PORT(unit, port)/*fabric link*/) {
          link = SOC_DPP_FABRIC_PORT_TO_LINK(unit, port);

          soc_sand_rv = arad_fabric_link_up_get(unit, link, &fabric_up);
          rv = handle_sand_result(soc_sand_rv);

          *up = (int) fabric_up;
          if (rv != BCM_E_NONE) {
            *up = 0;
            BCMDNX_ERR_EXIT_MSG(rv, (_BSL_BCM_MSG("%s: arad_fabric_link_status_get link %d failed, unit %d"), FUNCTION_NAME(), link, unit));
          }
...
bcm_petra_port_link_status_get(int unit, bcm_port_t port, int *up)
{
    ...
    if (rv == BCM_E_DISABLED) {
        ...
        rv = bcm_common_linkscan_mode_get(unit, port, &mode);

        if (BCM_SUCCESS(rv) && (mode == BCM_LINKSCAN_MODE_HW)) {
            rv = _bcm_petra_port_link_get(unit, port, 1, up);
        } else {
    ...
    LOG_DEBUG(BSL_LS_BCM_PORT,
              (BSL_META_U(unit,
                          "bcm_petra_port_link_status_get: p=%d up=%d rv=%d"),
               port, *up, rv));
...
#ifdef  BCM_PETRAB_SUPPORT
STATIC int
_bcm_petra_port_serdes_range_polarity_swap_enable_set(int          unit,
                                                      unsigned int first_lane,
                                                      unsigned int nof_lanes,
                                                      int          enable)
{
    ...
    int                     soc_sand_rv;
    int                     soc_sand_dev_id;
    SOC_PETRA_SRD_ALL_LANE_INFO *lane_conf;
    unsigned int            lane;
    
    BCMDNX_INIT_FUNC_DEFS;
    soc_sand_dev_id = (unit);
    lane_conf = SOC_DPP_CONFIG(unit)->pb->hw_adjust.serdes.conf.lane_conf;
   
    for (lane = first_lane; lane < first_lane + nof_lanes; ++lane) {
        if (enable) {
            soc_sand_rv = soc_petra_srd_lane_polarity_set(soc_sand_dev_id, lane,
                                    SOC_TMC_CONNECTION_DIRECTION_TX,
                                    lane_conf[lane].is_swap_polarity_tx);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
            soc_sand_rv = soc_petra_srd_lane_polarity_set(soc_sand_dev_id, lane,
                                    SOC_TMC_CONNECTION_DIRECTION_RX,
                                    lane_conf[lane].is_swap_polarity_rx);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        } else {
            soc_sand_rv = soc_petra_srd_lane_polarity_set(soc_sand_dev_id, lane,
                                        SOC_TMC_CONNECTION_DIRECTION_BOTH, FALSE);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        }
    }
...
STATIC int
_bcm_petra_port_nif_polarity_swap_enable_set(int unit, int nif_id, int enable)
{
    int          rv = BCM_E_NONE;
    int          soc_sand_dev_id;
    unsigned int nof_lanes;
    ...
    soc_pb_nif_srd_lanes_get(soc_sand_dev_id, nif_id, &nof_lanes, &base_lane);
    rv = _bcm_petra_port_serdes_range_polarity_swap_enable_set(unit,
                                                               base_lane,
                                                               nof_lanes,
                                                               enable);
    BCMDNX_IF_ERR_EXIT(rv);
...
STATIC int
_bcm_petra_port_nif_loopback_set(int unit, bcm_port_t port, int enable)
{
    int rv = BCM_E_NONE;
    int soc_sand_rv;    
    int nif_id;
    ...
    if (SOC_IS_PETRAB(unit))
    {
      /* Lane polarity swap has to be disabled when in loopback, regardless of
       * previous configuration */
      rv = _bcm_petra_port_nif_polarity_swap_enable_set(unit, nif_id,
                                                        enable ? 0 : 1);
      BCMDNX_IF_ERR_EXIT(rv);
    }
#endif
        
    soc_sand_rv = (MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_nif_loopback_set,(unit, nif_id, enable ? TRUE : FALSE)));
    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
...
STATIC int
_bcm_petra_port_fabric_loopback_set(int unit, bcm_port_t port, int enable)
{
    int                          rv = BCM_E_NONE;
    int                          soc_sand_rv;
    int                          soc_sand_dev_id;
    SOC_PETRA_SRD_LANE_LOOPBACK_MODE loopback_mode;
    int                          link;
    int                          serdes;
    
    BCMDNX_INIT_FUNC_DEFS;
    loopback_mode = enable ? SOC_PETRA_SRD_LANE_LOOPBACK_NSILB
                                 : SOC_PETRA_SRD_LANE_LOOPBACK_NONE;
    link = SOC_DPP_FABRIC_PORT_TO_LINK(unit, port);
    rv = _bcm_petra_port_fabric_link_to_serdes_lane(unit, link, &serdes);
    BCMDNX_IF_ERR_EXIT(rv);
    
    /* Set the correct polarity */
    rv = _bcm_petra_port_serdes_range_polarity_swap_enable_set(unit, serdes, 1,
                                                               enable ? 0 : 1);
    BCMDNX_IF_ERR_EXIT(rv);
    
    /* Set the loopback mode */
    soc_sand_dev_id = (unit);
    soc_sand_rv = soc_petra_srd_lane_loopback_mode_set(soc_sand_dev_id, serdes,
                                               loopback_mode);
    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
...
#ifdef BCM_ARAD_SUPPORT
STATIC int
_bcm_arad_port_nif_loopback_set(int unit, bcm_port_t port, int loopback)
{
    int locked;
    int res;
    ...
    /*MAC loopback*/
    BCMDNX_IF_ERR_EXIT(soc_pm_mac_loopback_set(unit, port, (loopback == BCM_PORT_LOOPBACK_MAC)));

    /*Phy loopback*/
    MIIM_LOCK(unit);
    locked = 1;
    BCMDNX_IF_ERR_EXIT(soc_phyctrl_loopback_set(unit, port, (loopback == BCM_PORT_LOOPBACK_PHY), TRUE));
    locked = 0;
    MIIM_UNLOCK(unit);
    ...
    MIIM_LOCK(unit);
    locked = 1;
    res = soc_phyctrl_control_set(unit, port, control, (loopback == BCM_PORT_LOOPBACK_PHY_REMOTE));
    locked = 0;
    MIIM_UNLOCK(unit);
    if((res != SOC_E_UNAVAIL) || (loopback == BCM_PORT_LOOPBACK_PHY_REMOTE)){
...
STATIC int
_bcm_arad_port_nif_loopback_get(int unit, bcm_port_t port, int *loopback)
{
    int  lb, locked;
    soc_port_if_t interface;
    uint32 control;
    ...
    /*MAC loopback*/
    BCMDNX_IF_ERR_EXIT(soc_pm_mac_loopback_get(unit, port, (uint32*)&lb));
    if(lb) {
    ...
    MIIM_LOCK(unit);
    locked = 1;
    BCMDNX_IF_ERR_EXIT(soc_phyctrl_loopback_get(unit, port, &lb));
    if(lb) {
    ...
    BCMDNX_IF_ERR_EXIT(soc_phyctrl_control_get(unit, port, control, (uint32*)&lb));
    locked = 0;
    MIIM_UNLOCK(unit);
...
STATIC int
_bcm_arad_port_fabric_loopback_set(int unit, bcm_port_t port, int loopback)
{
    bcm_error_t rc;
    int link;
    ...
    link = SOC_DPP_FABRIC_PORT_TO_LINK(unit, port);
    rc = arad_fabric_loopback_set(unit, link, dcmn_loopback);
...
STATIC int
_bcm_arad_port_loopback_get(int unit, bcm_port_t port, int* loopback)
{
    bcm_error_t rc;
    int link;
    soc_dcmn_loopback_mode_t dcmn_loopback;
  ...
  link = SOC_DPP_FABRIC_PORT_TO_LINK(unit, port);
    rc = arad_fabric_loopback_get(unit, link, &dcmn_loopback);
    BCMDNX_IF_ERR_EXIT(rc);
...
bcm_petra_port_loopback_set(int unit, bcm_port_t port, int loopback)
{
    ...
    if (SOC_IS_PETRAB(unit)) {
        ...
        switch (loopback) {
            ...
            case BCM_PORT_LOOPBACK_MAC:
                BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("MAC loopback isn't supported for Petra")));
                break;

            case BCM_PORT_LOOPBACK_PHY_REMOTE:
                BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("Remote loopback isn't supported for Petra")));
                break;

            default:
                BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Invalid loopback mode %d"),loopback));
                break;
        }
    
        if (BCM_PBMP_MEMBER(PBMP_SFI_ALL(unit), port)) {
            rv = _bcm_petra_port_fabric_loopback_set(unit, port, enable);
        } else if (BCM_PBMP_MEMBER(PBMP_PORT_ALL(unit), port)) {
            rv = _bcm_petra_port_nif_loopback_set(unit, port, enable);
        } else {
            rv = BCM_E_PORT;
        }
    }
    ...
    if(SOC_IS_ARAD(unit)) {
        ...
        BCMDNX_IF_ERR_EXIT(bcm_petra_port_loopback_get(unit, port, &lb_start));
        if ((loopback != 0) && (lb_start == 0) && (EXT_PHY_SW_STATE(unit, port) == NULL)) {
            BCMDNX_IF_ERR_EXIT(bcm_petra_port_phy_control_get(unit, port, BCM_PORT_PHY_CONTROL_CL72, &cl72_start));
            SOC_DPP_PORT_PARAMS(unit).cl72_configured[port] = cl72_start;
            if (cl72_start == 1) {
                BCMDNX_IF_ERR_EXIT(bcm_petra_port_phy_control_set(unit, port, BCM_PORT_PHY_CONTROL_CL72, 0)); 
            }
        }
        
        if (BCM_PBMP_MEMBER(PBMP_SFI_ALL(unit), port)) {
            rv = _bcm_arad_port_fabric_loopback_set(unit, port, loopback);
        } else if (BCM_PBMP_MEMBER(PBMP_PORT_ALL(unit), port)) {
            rv = _bcm_arad_port_nif_loopback_set(unit, port, loopback);
        } else  {
            rv = BCM_E_PORT;
        }
        ...
        if ((loopback == 0) && (lb_start != 0) && (EXT_PHY_SW_STATE(unit, port) == NULL)) {
            cl72_start = SOC_DPP_PORT_PARAMS(unit).cl72_configured[port];
            if (cl72_start == 1) {
                rv = bcm_petra_port_phy_control_set(unit, port, BCM_PORT_PHY_CONTROL_CL72, 1);
            }
        }
    }
#endif /* BCM_ARAD_SUPPORT */
...
bcm_petra_port_loopback_get(int unit, bcm_port_t port, int *loopback)
{
    ...
    if (SOC_IS_PETRAB(unit)) {
        int       soc_sand_rv;    
        int       nif_id;
        uint8 loopback_enable = FALSE;

        /* Get the NIF ID from the port number */
        rv = _bcm_petra_port_to_nif_id(unit, port, &nif_id);
        BCMDNX_IF_ERR_EXIT(rv);

        /* Call the Dune API to read the current NIF configuration */    
        soc_sand_rv = (MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_nif_loopback_get,(unit, nif_id, &loopback_enable)));
        rv = handle_sand_result(soc_sand_rv);
        BCMDNX_IF_ERR_EXIT(rv);

        if (loopback_enable == TRUE) {
            *loopback = BCM_PORT_LOOPBACK_PHY;
        } else {
            *loopback = BCM_PORT_LOOPBACK_NONE;
        }
    ...
    {
        if (BCM_PBMP_MEMBER(PBMP_SFI_ALL(unit), port)) {
            rv = _bcm_arad_port_loopback_get(unit, port, loopback);
        } else if (BCM_PBMP_MEMBER(PBMP_PORT_ALL(unit), port)) {
            rv = _bcm_arad_port_nif_loopback_get(unit, port, loopback);
        } else {
            rv = BCM_E_PORT;
        }
    }
...
bcm_petra_port_untagged_vlan_set(int unit, bcm_port_t port, bcm_vlan_t vid)
{
    ...
    BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_i) {
        ...
        soc_sand_rv = soc_ppd_llp_vid_assign_port_info_get(unit, soc_ppd_port, &port_info);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);

        port_info.pvid = vid; 
        soc_sand_rv = soc_ppd_llp_vid_assign_port_info_set(unit, soc_ppd_port, &port_info);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    }
...
bcm_petra_port_untagged_priority_set(int unit, bcm_port_t port, int priority)
{
   ...
   BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_i) {
       ...
       soc_sand_rv = soc_ppd_llp_cos_port_info_get(unit, soc_ppd_port, &port_info);
       BCM_SAND_IF_ERR_EXIT(soc_sand_rv);

       port_info.default_tc = priority;

       soc_sand_rv = soc_ppd_llp_cos_port_info_set(unit, soc_ppd_port, &port_info);
       BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
   }
...
_bcm_petra_port_traps_init(int unit)
{
   ...
   BCM_PBMP_ITER(ports_map, port) {
        ...
        soc_sand_rv =soc_ppd_eg_filter_port_info_get(unit,soc_ppd_port,&eg_filter_port_info);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        ...
        soc_sand_rv =soc_ppd_eg_filter_port_info_set(unit,soc_ppd_port,&eg_filter_port_info);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    ...
    )
{
  ...
  */
  ...
  soc_sand_rv = soc_ppd_port_info_get(soc_sand_dev_id,ppd_port_id,&port_info);
  BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
...
_bcm_petra_port_tpid_indexes_get(
       int unit, SOC_PPD_PORT ppd_port_id, uint32 tpid_val, uint16 tpid_vals[_BCM_PETRA_PORT_NOF_TPID_VALS], uint32 nof_tpid_vals,
    ...
    )
{
  ...
  *nof_tpid_indexes = 0;
  ...
  for (tpid_val_indx = 0; tpid_val_indx < _BCM_PETRA_PORT_NOF_TPID_VALS; ++tpid_val_indx) {
    ...
    )
{
   ...
   *new_profile = FALSE;
   BCM_PBMP_ITER(pbmp_local_ports, port_idx) {
       ...
       soc_sand_rv = soc_ppd_llp_cos_port_info_get(soc_sand_dev_id,soc_ppd_port_idx,&cos_port_info);
       BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
   ...
   uint32 class_id) 
{
    ...
    rv = _bcm_dpp_gport_to_lif(unit, port, &lif_id, &out_lif_id, &fec_id, &is_local);
    BCMDNX_IF_ERR_EXIT(rv);
   ...
   *class_id = 0;
   ...
   switch (pclass) {
       ...
       case bcmPortClassFieldIngress:
           ...
           if ((BCM_E_NONE == rv) && (lif_id != _BCM_GPORT_ENCAP_ID_LIF_INVALID)) {
             ...
             rv = _bcm_petra_port_inlif_profile_get(unit, port, class_id, SOC_PPC_PROFILE_MGMT_INLIF_PROFILE_TYPE_USER, lif_info);
   ...
   case bcmPortClassL2Lookup:
            ...
            if (phy_port->type != _BCM_DPP_GPORT_IN_TYPE_RING) {
                BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Port is not part of a L2 Lookup group")));
            }
   ...
   BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_i) {
       ...
       case bcmPortClassFieldLookup:
           ...
           soc_sand_rv = soc_ppd_fp_control_get(soc_sand_dev_id,&ctrl_indx,&ctrl_info);
           BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
       ...
       case bcmPortClassFieldIngressVlanTranslation:
           {
               ...
               rv = MBCM_PP_DRIVER_CALL(unit, mbcm_pp_port_property_get, (unit, port, soc_ppc_port_property_vlan_translation, class_id));
               BCMDNX_IF_ERR_EXIT(rv);
           }
           break;
    ...
    int *nof_pp_ports)
{
   ...
   *nof_pp_ports = 0;
   for (local_port = 0; local_port < SOC_DPP_DEFS_GET(unit, nof_pp_ports); local_port++) {
       /* Get the VLAN Domain = Port class */
       SOC_PPD_PORT_INFO_clear(&port_info);
       soc_sand_rv = soc_ppd_port_info_get(soc_sand_dev_id, local_port, &port_info);
       BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
   ...
   switch (pclass) {
       ...
       case bcmPortClassFieldIngress:
           ...
           if ((BCM_E_NONE == rv) && (lif_id != _BCM_GPORT_ENCAP_ID_LIF_INVALID)) {
             ...
             rv = _bcm_petra_port_inlif_profile_set(unit, port, class_id, SOC_PPC_PROFILE_MGMT_INLIF_PROFILE_TYPE_USER, lif_info);
        ...
        case bcmPortClassL2Lookup:
            ...
            rv = _bcm_dpp_gport_to_lif(unit,
                                       ...
                                       &fec_id,
                                       &is_local);
            if ((rv != BCM_E_NONE) || (lif_id == _BCM_GPORT_ENCAP_ID_LIF_INVALID)) {
       ...
       case bcmPortClassForwardEgress:
             /* Only MPLS Tunnel is supported */
             if (BCM_GPORT_IS_TUNNEL(port)) {
               rv = _bcm_dpp_port_class_set_split_horizon(unit, port, class_id);
               BCMDNX_IF_ERR_EXIT(rv);
   ...
   BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_i) {
       ...
       switch (pclass) {
         ...
         case bcmPortClassId:
           ...
           soc_sand_rv = soc_ppd_port_info_set(unit, soc_ppd_port_i, &port_info);
           BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
         break;
       ...
       case bcmPortClassFieldIngressVlanTranslation:
           {
               ...
               rv = MBCM_PP_DRIVER_CALL(unit, mbcm_pp_port_property_set, (unit, port, soc_ppc_port_property_vlan_translation, class_id));
               BCMDNX_IF_ERR_EXIT(rv);
           }
           break;
   ...
   )
{
    ...
    BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_i) {
        ...
        if (BCM_GPORT_IS_TRAP(egr_port)) {
            ...
            /*is useded for sync*/
            ud_trap_info.ud_unique = _BCM_DPP_TRAP_PREVENT_SWAPPING; 
            rv = _bcm_dpp_am_template_user_defined_trap_exchange(unit, soc_ppd_port_i, &ud_trap_info, NULL, &last_appear, &trap_indx, &first_appear);
            ...
            if (!SOC_IS_JERICHO(unit)) {
                if (rv != BCM_E_NONE && ingress_rate) {
                    ...
                    rv = _bcm_dpp_am_template_snoop_cmd_exchange(unit,soc_ppd_port_i,&old_snoop_data,NULL, NULL, &snoop_cmd, &first_appear);
                    BCMDNX_IF_ERR_EXIT(rv);
    ...
    bcm_color_t *color)
{
    ...
    BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_i) {
        ...
        soc_sand_rv = soc_ppd_llp_cos_port_info_get(unit,soc_ppd_port_i,&cos_port_info);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    ...
    bcm_color_t color)
{
   ...
   BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_idx) {
       ...
       soc_sand_rv = soc_ppd_llp_cos_port_info_get(unit,soc_ppd_port_idx,&cos_port_info);
       BCM_SAND_IF_ERR_EXIT(soc_sand_rv);

       cos_port_info.l2_info.tbls_select.up_to_dp_index = profile;
       soc_sand_rv = soc_ppd_llp_cos_port_info_set(unit,soc_ppd_port_idx,&cos_port_info);
       BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
   }
...
STATIC int 
_bcm_petra_gport_outlif_discard_get(
    int unit, 
    bcm_port_t port, 
    int *mode)
{
   ...
   /* map gport to LIF */
   rv = _bcm_dpp_gport_to_lif(unit, port, NULL, &lif_index, &fec_id, &is_local);
   BCMDNX_IF_ERR_EXIT(rv);
   
   if (lif_index==_BCM_GPORT_ENCAP_ID_LIF_INVALID) {
         BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("port %d doesn't have outlif. discard can not be retrieved"),port));  
   }
   ...
   soc_sand_rv = soc_ppd_eg_encap_lif_field_get(soc_sand_dev_id,lif_index,SOC_PPD_EG_ENCAP_ENTRY_UPDATE_DROP,&drop);
   BCM_SAND_IF_ERR_EXIT(soc_sand_rv);


   if(drop == 1) {
       *mode = BCM_PORT_DISCARD_EGRESS;
   }
   else{
       *mode = BCM_PORT_DISCARD_NONE;
   }
    ...
    int *mode)
{
   ...
   /* check if this LIF-gport configuration */
   rv = _bcm_dpp_gport_parse(unit,port,&gport_type, &gport_val, NULL);
   BCMDNX_IF_ERR_EXIT(rv);

   /* if the supplied gport is not simple (i.e. it;s a LIF) handle it in special function*/
   if (gport_type != _BCM_DPP_GPORT_TYPE_SIMPLE) {
       rv = _bcm_petra_gport_outlif_discard_get(unit,port,mode);
       BCMDNX_IF_ERR_EXIT(rv);
       BCM_EXIT;
   }
    ...
    int *mode)
{
   ...
   BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_i) {
       ...
       /* get old data*/
       rv =  _bcm_dpp_am_template_tpid_profile_data_get(unit,soc_ppd_port_i,&profile_type, mode, &dtag_mode, &ac_key_map_type);
       BCMDNX_IF_ERR_EXIT(rv);
 ...
 */
STATIC int 
_bcm_petra_gport_outlif_discard_set(
    int unit, 
    bcm_port_t port, 
    int mode)
{
   ...
   if(mode == BCM_PORT_DISCARD_EGRESS) {
       drop = 1;
   }
       ...
       BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("%s: unsupported mode %x"),FUNCTION_NAME(), mode));
   }
   ...
   if (lif_index==_BCM_GPORT_ENCAP_ID_LIF_INVALID) {
         BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("port %d doesn't have outlif. discard can not be set"),port));  
   }   
   ...
   /* update LIF attribute */
   soc_sand_rv = soc_ppd_eg_encap_next_outlif_update(soc_sand_dev_id,lif_index,SOC_PPD_EG_ENCAP_ENTRY_UPDATE_DROP,drop);
   BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    ...
    int mode)
{
   ...
   if (mode >= BCM_PORT_DISCARD_COUNT && mode != BCM_PORT_DISCARD_INGRESS && mode != BCM_PORT_DISCARD_EGRESS) {
   ...
   /* if the supplied gport is not simple (i.e. it;s a LIF) handle it in special function*/
   if (gport_type != _BCM_DPP_GPORT_TYPE_SIMPLE) {
       rv = _bcm_petra_gport_outlif_discard_set(unit,port,mode);
       BCMDNX_IF_ERR_EXIT(rv);
       BCM_EXIT;
   }
    ...
    int mode)
{
    ...
    BCM_PBMP_ITER(pbmp_local_ports, port_i) {
        ...
        soc_sand_rv = soc_ppd_llp_cos_port_info_set(unit,soc_ppd_port_i,&cos_port_info);
         BCM_SAND_IF_ERR_EXIT(soc_sand_rv);           
    }
    ...
    uint32 *flags)
{
    ...
    soc_sand_rv = soc_ppd_frwrd_mact_port_info_get(soc_sand_dev_id,soc_ppd_port_i,&mact_port_info);
    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    ...
    uint32 remove)
{
    ...
    BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_i) {
        ...
        soc_sand_rv = soc_ppd_frwrd_mact_port_info_get(unit,port_i,&mact_port_info);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        mact_port_info.sa_unknown_action_profile = sa_unknown_action_profile;
        soc_sand_rv = soc_ppd_frwrd_mact_port_info_set(unit,port_i,&mact_port_info);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        ...
        soc_sand_rv = soc_ppd_port_info_get(unit,port_i,&port_info);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);

        if (port_info.enable_learning != learn_enable) {
            port_info.enable_learning = learn_enable;
            soc_sand_rv = soc_ppd_port_info_set(unit,port_i,&port_info);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        }
...
STATIC int
_bcm_petra_mgmt_max_pckt_size_set(int unit, bcm_port_t port, int size)
{
    BCMDNX_INIT_FUNC_DEFS;
   
#ifdef BCM_ARAD_SUPPORT
   if (SOC_IS_ARAD(unit)) {
        if (size < 0 || size > SOC_DPP_DEFS_GET(unit, jumbo_max_size)) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Max size is out of range")));
        }

        BCMDNX_IF_ERR_EXIT(soc_pm_frame_max_set(unit, port, size));
   }
#endif
#ifdef BCM_PETRAB_SUPPORT
   if (SOC_IS_PETRAB(unit)) {
       ...
       BCMDNX_IF_ERR_EXIT(petra_soc_dpp_local_to_tm_port_get(unit, port, &tm_port, &core));

       if (size < 0 || size > SOC_DPP_DEFS_GET(unit, jumbo_max_size)) {
           BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Max size is out of range")));
       }

       soc_sand_rv = MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_mgmt_max_pckt_size_set,
           (unit, tm_port, SOC_TMC_MGMT_PCKT_SIZE_CONF_MODE_INTERN, size));
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
...
bcm_petra_port_frame_max_set(int unit, bcm_port_t port, int size)
{
    ...
    status = _bcm_petra_mgmt_max_pckt_size_set(unit, port, size);
...
STATIC int
_bcm_pb_mgmt_max_pckt_size_get(int unit, bcm_port_t port, int *size)
{
    uint32 tm_port;
    uint32 soc_sand_rv, max_size = 0;

    BCMDNX_INIT_FUNC_DEFS;
    
    BCMDNX_NULL_CHECK(size);

#ifdef BCM_ARAD_SUPPORT
    if(SOC_IS_ARAD(unit)) {
        BCMDNX_IF_ERR_EXIT(soc_pm_frame_max_get(unit, port, size));
    }
#endif
    ...
    if(SOC_IS_PETRAB(unit)) {
        ...
        soc_sand_rv = MBCM_DPP_DRIVER_CALL(unit,  mbcm_dpp_mgmt_max_pckt_size_get, (unit, tm_port, SOC_TMC_MGMT_PCKT_SIZE_CONF_MODE_INTERN, &max_size));
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
...
bcm_petra_port_frame_max_get(int unit, bcm_port_t port, int *size)
{
    ...
    /* Set the max packet size */
    status = _bcm_pb_mgmt_max_pckt_size_get(unit, port, size);
    BCMDNX_IF_ERR_EXIT(status);
...
/* Returns the speed of the FAP port - for now, assume port speed is 10G        */
/* This should be the speed of the FAP port, which may be a channel within a    */
/* channelized NIF.  This should be obtained by getting the port speed and      */
/* subport speed from SOC properties and returning these values.  For now,      */
/* assume speed is 10G.                                                         */
int
bcm_petra_fap_port_speed_get(int unit, int fap_port, int *p_speed_in_kbps)
{
    ...
    *p_speed_in_kbps = 10000000;

    BCMDNX_FUNC_RETURN;
}

/* Returns the nif port parent of the fap port - note that the fap port need not */
/* be mapped 1 to 1 with the pbmp port.                                          */
int
bcm_petra_fap_port_parent_nif_get(int unit, int fap_port, int *p_nif_port)
{
    ...
    rv = MBCM_DPP_SOC_DRIVER_CALL(unit, mbcm_dpp_port_to_interface_map_get, (unit, core, fap_port, &if_id, NULL));

    if (rv != BCM_E_NONE) {
        LOG_ERROR(BSL_LS_BCM_PORT,
              (BSL_META_U(unit,
                          "%s: mbcm_dpp_port_to_interface_map_get fap_port %d failed, unit %d\n"), 
                          FUNCTION_NAME(), fap_port, unit));
        BCMDNX_IF_ERR_EXIT(rv); 
    }
...
/* Return the total number of front panel ports configured (local only) */
int
bcm_petra_port_count_get(int unit, int *p_num_ports)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    BCM_PBMP_ASSIGN(ports, PBMP_E_ALL(unit));

    PBMP_ITER(ports, port) {
        port_count++;
    }

    *p_num_ports = port_count;
    BCM_EXIT;
exit:
    BCMDNX_FUNC_RETURN;
}

/* get the nif_id for a port */
/* calling routine calls this function irespective of Petra/Arad Support     */

/*        In future this will also have to support ports that are not        */
/*        part of pbmp (e.g. erp, replication, etc)                          */
int
bcm_port_to_nif_id(int unit, bcm_port_t port, int *nif_id)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = _bcm_petra_port_to_nif_id(unit, port, nif_id);
...
bcm_port_from_nif_id(int unit, int nif_id, bcm_port_t *port)
{
#ifdef  BCM_ARAD_SUPPORT
    uint32 port_lcl;
    uint8 found = FALSE, is_nif = FALSE;
    int soc_sand_rv = 0;
#endif
    ...
    if (SOC_IS_PETRAB(unit)) {
        ...
        BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("Not supported for Petra-B")));
    } 
#endif /* BCM_PETRAB_SUPPORT */
#ifdef  BCM_ARAD_SUPPORT
    if (SOC_IS_ARAD(unit)) {
        int core;
        soc_sand_rv = arad_nif_id_to_port_get(unit, nif_id, &port_lcl, &core, &found, &is_nif);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);

        
        *port = (found)? port_lcl : 0;
    }
#endif  /* BCM_ARAD_SUPPORT */
...
bcm_port_to_scaled_down_nif_id(int unit, int nif_id, uint32 *scaled_down_nif_id)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    (*scaled_down_nif_id) = nif_id & 0x7F;
    BCM_EXIT;
exit:
    BCMDNX_FUNC_RETURN;
}

/* These functions above are required for cosq TM support */
    ...
    )
{
    ...
    int soc_sand_dev_id = unit, soc_sand_rv = 0;
    uint16 used_tpid_vals[_BCM_PORT_NOF_TPID_VALS] = {0};
    uint16 tpid_val;
    ...
    if(nof_filled == nof_tpids) {
        ...
        if (MIM_IS_INIT(unit) && SOC_DPP_IS_VLAN_TRANSLATE_MODE_NORMAL(unit)) {
            ...
            mim_out_ac = __dpp_mim_out_ac_get(unit);
            ...
            if (tpid_i != _BCM_PORT_NOF_TPID_VALS) { /* B-tag tpid was found */
                ...
                rv = _bcm_dpp_am_template_vlan_edit_profile_eg_mapping_data_get(unit, mim_out_ac, &mapping_profile);
                BCMDNX_IF_ERR_EXIT(rv);
                ...
                rv = _bcm_dpp_am_template_vlan_edit_profile_eg_mapping_exchange(unit, mim_out_ac, &mapping_profile, &old_profile, &is_last, 
                ...
                if (new_profile != 0) {
                    soc_sand_rv = soc_ppd_eg_ac_info_get(soc_sand_dev_id, mim_out_ac, &ac_info);
                    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);

                    ac_info.edit_info.edit_profile = new_profile;

                    soc_sand_rv = soc_ppd_eg_ac_info_set(soc_sand_dev_id, mim_out_ac, &ac_info);
                    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                }
            }
        }
...
/* 
 * Update old counter in case of tpids
 */
STATIC int
_bcm_dpp_port_update_old_tpids(int unit, int tpid_profile, int nof_old_tpids)
{
    int tpid_count;      
    BCMDNX_INIT_FUNC_DEFS;  
   ...
   BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_i) {
      ...
      */
      ...
      if(!_BCM_DPP_TPID_PARSE_ADVANCED_MODE(unit)) {
          ...
          rv = _bcm_dpp_am_template_tpid_profile_exchange(unit,soc_ppd_port_i,new_profile_type,accept_mode,dtag_mode,ac_key_map_type,&old_tpid_profile,&last_appear,&new_tpid_profile,&first_appear);
          BCMDNX_IF_ERR_EXIT(rv);
      ...
      /* now that modification is possible update counters */
      rv = _bcm_dpp_port_update_old_tpids(unit,port_info.tpid_profile,nof_old_tpids);
      BCMDNX_IF_ERR_EXIT(rv);  

      /* if number of new tpids is 0, nothing to allocate, as no tpids are associate to the port */
      if(nof_new_tpids > 0) {
          rv2 = bcm_petra_port_tpid_profile_alloc(unit, new_tpids, nof_new_tpids,FALSE,&tpid_profile);
          /* if fail, restore counter status */
          if(rv2 != BCM_E_NONE) {
              if(nof_old_tpids >= 1) {
                  _BCM_PETRA_PORT_TPID_COUNT_GET(unit, 0, tpid_profile_info.tpid1.index, tpid_count);
                  ++tpid_count;
                  _BCM_PETRA_PORT_TPID_COUNT_SET(unit, 0, tpid_profile_info.tpid1.index, tpid_count);
              }
              if(nof_old_tpids >= 2) {
                  _BCM_PETRA_PORT_TPID_COUNT_GET(unit, 1, tpid_profile_info.tpid2.index, tpid_count);
                  ++tpid_count;
                  _BCM_PETRA_PORT_TPID_COUNT_SET(unit, 1, tpid_profile_info.tpid2.index, tpid_count);
              }
              /* resotre also port-profile */
          ...
          if(nof_new_tpids >= 1) {
              _BCM_PETRA_PORT_TPID_COUNT_GET(unit, 0, port_info.tpid_profile, tpid_count);
              ++tpid_count;
              _BCM_PETRA_PORT_TPID_COUNT_SET(unit, 0, port_info.tpid_profile, tpid_count);
          }
          if(nof_new_tpids  >= 2) {
              _BCM_PETRA_PORT_TPID_COUNT_GET(unit, 1, port_info.tpid_profile, tpid_count);
              ++tpid_count;
              _BCM_PETRA_PORT_TPID_COUNT_SET(unit, 1, port_info.tpid_profile, tpid_count);
          }
...
{   
    ...
    /*                                                                       .
     *  If old_nof_tpids is zero (i.e. operation none) need to add 2 new tpids.
     *  one tpid is as requested and the other is 0.
     * 
     *  If old_nof_tpids is two (i.e. double tag operation) can be two options:
     *  1. double tag => double tag (i.e. operation remove tpid and then add 0)
     *  2. double tag => none (i.e. operation remove tpid and another remove 0)
     */
    ...
    /* If old == none and current operation is one tag run second operation with tpid 0 */
    if (nof_old_tpids == 0 && nof_new_tpids == 1) {
        /* Add inner as zero */
        new_oper = _bcm_petra_tpid_op_inner_set;
        rv = bcm_petra_port_tpid_new_tpids_calc(unit, lif_id, new_tpids, nof_new_tpids, new_profile_type, new_oper, 0, new_tpids, &nof_new_tpids, &new_profile_type,FALSE,FALSE);
        BCMDNX_IF_ERR_EXIT(rv);
    } else if (nof_old_tpids == 2 && nof_new_tpids == 1) {
        /* Decide operation according to tpids numbers */
        if (new_tpids[0] == 0 && new_tpids[1] == 0) {
            /* none, move from double_tag => none */
            new_oper = _bcm_petra_tpid_op_outer_delete_all;
            rv = bcm_petra_port_tpid_new_tpids_calc(unit, lif_id, new_tpids, nof_new_tpids, new_profile_type, new_oper, 0, new_tpids, &nof_new_tpids, &new_profile_type,FALSE,FALSE);
            BCMDNX_IF_ERR_EXIT(rv);
        } else {
            /* second operation to add tpid 0 in order to maintain two tags */
            switch (oper)
            {
            case _bcm_petra_tpid_op_outer_delete:
              new_oper = _bcm_petra_tpid_op_outer_set;
              break;
            case _bcm_petra_tpid_op_inner_remove:
              new_oper = _bcm_petra_tpid_op_inner_set;
              break;
            default:
              BCMDNX_ERR_EXIT_MSG(BCM_E_INTERNAL, (_BSL_BCM_MSG("Tried to add another tpid but operation wasnt as expected")));
              break;
            }
            rv = bcm_petra_port_tpid_new_tpids_calc(unit, lif_id, new_tpids, nof_new_tpids, new_profile_type, new_oper, 0, new_tpids, &nof_new_tpids, &new_profile_type,FALSE,FALSE);
            BCMDNX_IF_ERR_EXIT(rv);
    ...
    if (!(nof_new_tpids == 2 || nof_new_tpids == 0)) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_INTERNAL, (_BSL_BCM_MSG("Expected to obtain new tpids 0 or 2 but got to internal expected number %d"), nof_new_tpids));
    }

    /* Update counters */
    /* now that modification is possible update counters */    
    rv = _bcm_dpp_port_update_old_tpids(unit,tpid_profile,nof_old_tpids);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    /* Update lif profile */
    if (is_pwe) {
        lif_entry_info->value.pwe.tpid_profile_index = tpid_profile;
    }
    else {
        lif_entry_info->value.isid.tpid_profile_index = tpid_profile;
    }
...
{
    uint16 port_tpids[_BCM_PETRA_PORT_NOF_TPID_VALS]; 
    ...
    static int enable_egress_acceptable_frame_type = 1;
    _bcm_dpp_gport_info_t gport_info;
    BCMDNX_INIT_FUNC_DEFS;
    ...
    rv = _bcm_petra_port_assigned_tpids_get(unit, soc_ppd_port_i, port_tpids, &nof_tpids);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    rv = _bcm_dpp_am_template_port_tpid_class_data_get(unit,soc_ppd_port_i,&tpid_class_buffer);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    rv = _bcm_dpp_am_template_port_tpid_class_exchange(unit,0,soc_ppd_port_i,&tpid_class_buffer,&old_template,&is_last,&new_template,&is_first);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    if (enable_egress_acceptable_frame_type) {

        /* convert from port profile entry format to egress acceptable frame port profile */
        _bcm_dpp_port_tpid_class_info_t_to_bcm_dpp_port_tpid_class_acceptable_frame_type_info_t(unit, &tpid_class_buffer, &tpid_class_acceptable_frame_info);
            ...
            for(indx1 = 0; indx1 < port_tpid_nof_indexes[0]; ++indx1) {
                for(indx2 = 0; indx2 < port_tpid_nof_indexes[1]; ++indx2) {
                    for(indx3 = 0; indx3 < nof_prirority_types; ++indx3) {
                        ...
                        eg_filter_port_accept = !(SHR_BITGET(&tpid_class_acceptable_frame_info.buffer, tag_stcture_indx));
        ...
        BCM_PBMP_ITER(gport_info.pbmp_local_ports, local_port) {
            ...
            soc_sand_rv = soc_ppd_eg_filter_port_info_get(unit, soc_ppd_port_i, &eg_filter_port_info);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
            eg_filter_port_info.acceptable_frames_profile  = new_template;
            soc_sand_rv  = soc_ppd_eg_filter_port_info_set(unit,soc_ppd_port_i, &eg_filter_port_info);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        }
    }
    ...
    uint32 *flags)
{
   ...
   *flags = 0;
   ...
   BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_i) {
       ...
       if (SOC_IS_ARAD(unit)) {       
           soc_sand_rv = soc_ppd_llp_trap_port_info_get(unit, soc_ppd_port_i, &llp_trap_port_info);
           BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    ...
    uint32 flags)
{
   ...
   BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_i) {
       ...
       if (SOC_IS_ARAD(unit)) {       
           ...
           soc_sand_rv = soc_ppd_llp_trap_port_info_set(unit, soc_ppd_port_i, &llp_trap_port_info);
           BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
       }
...
bcm_petra_port_autoneg_set(int unit, bcm_port_t port, int autoneg)
{
    ...
    if (SOC_IS_PETRAB(unit)) {
      ...
      /* autoneg is supported only for SGMII and QSGMII */
      if (!(SOC_PB_NIF_IS_TYPE_ID(SGMII, nif_id) || SOC_PB_NIF_IS_TYPE_ID(QSGMII, nif_id)))
      {
          BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("autoneg is supported only for SGMII and QSGMII")));
      }

      if (autoneg == 1) {
          rate = SOC_PB_NIF_GMII_RATE_AUTONEG;
      } else if (autoneg == 0){
          rate = SOC_PB_NIF_GMII_RATE_1000Mbps;
      } else {
          BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("invalid autoneg mode %d"), autoneg));
      }

      /* Call the Dune API and return the result */
      soc_sand_rv = soc_pb_nif_gmii_rate_set(soc_sand_dev_id, nif_id, rate);
      rv = handle_sand_result(soc_sand_rv);
      BCMDNX_IF_ERR_EXIT(rv);
    } 
#endif /* BCM_PETRAB_SUPPORT */
...
bcm_petra_port_gport_get(int unit, bcm_port_t port, bcm_gport_t *gport)
{
    ...
    /* Assuming HW port is same as physical port */
    
    SOC_GPORT_MODPORT_SET(l_gport, my_modid, port);
...
bcm_petra_port_internal_get(int unit, uint32 flags, int internal_ports_max, bcm_gport_t *internal_gport, int *internal_ports_count)
{
    ...
    if ((flags & BCM_PORT_INTERNAL_EGRESS_REPLICATION) && (NUM_ERP_PORT(unit) > 0)) {
        ...
        if(SOC_IS_PETRAB(unit)) {
            BCM_GPORT_LOCAL_SET(gport, BCM_DPP_PORT_INTERNAL_ERP(0));
        } else {
            BCM_GPORT_LOCAL_SET(gport, si->erp_port[0]);
        }
    ...
    if ((flags & BCM_PORT_INTERNAL_OLP)  && (NUM_OLP_PORT(unit) > 0)) {
        ...
        if(SOC_IS_PETRAB(unit)) {
            BCM_GPORT_LOCAL_SET(gport, BCM_DPP_PORT_INTERNAL_OLP(0));
        } else {
            BCM_GPORT_LOCAL_SET(gport, si->olp_port[0]);
        }
        internal_gport[count++] = gport;
    }
    ...
    if ((flags & BCM_PORT_INTERNAL_OAMP)  && (NUM_OAMP_PORT(unit) > 0)) {
        ...
        if(SOC_IS_PETRAB(unit)) {
            BCM_GPORT_LOCAL_SET(gport, BCM_DPP_PORT_INTERNAL_OAMP(0));
        } else {
            BCM_GPORT_LOCAL_SET(gport, si->oamp_port[0]);
        }
        internal_gport[count++] = gport;
    }
                     ...
                     bcm_color_t *color)
{
    ...
    *color = -1;
    ...
    BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_i) {
        ...
        BCM_SAND_IF_ERR_EXIT(soc_ppd_llp_cos_port_info_get(unit,soc_ppd_port_i,&cos_port_info));
...
#ifdef BCM_PETRAB_SUPPORT
  if (SOC_IS_PETRAB(unit)) {
      /* Petra-B only supports Full Duplex */
    ...
    if (SOC_IS_ARAD(unit)) {
        ...
        BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_i) {  
            /* PORT_LOCK(unit); */
            MIIM_LOCK(unit);
            rv = soc_phyctrl_duplex_get(unit, port_i, &phy_duplex);
            MIIM_UNLOCK(unit);
            BCMDNX_IF_ERR_EXIT(rv);
            /* PORT_UNLOCK(unit); */
...
bcm_petra_port_duplex_set(int unit, bcm_port_t port, int duplex)
{
  ...
  if (SOC_IS_ARAD(unit)) {
      ...
      BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_i) {  
          ...
          MIIM_LOCK(unit);
          locked = 1;
          BCMDNX_IF_ERR_EXIT(soc_phyctrl_auto_negotiate_set(unit, port_i, FALSE));
          BCMDNX_IF_ERR_EXIT(soc_phyctrl_duplex_set(unit, port_i, duplex));
          locked = 0;
          MIIM_UNLOCK(unit);
          /* PORT_UNLOCK(unit); */
      }
...
bcm_petra_port_fault_get(int unit, bcm_port_t port, uint32 *flags)
{
    ...
    if(SOC_IS_PETRAB(unit))
    {
        BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("bcm_petra_port_fault_get is not supported in Petra-B")));
    } else
#endif
#ifdef BCM_ARAD_SUPPORT
    if(SOC_IS_ARAD(unit))
    {
        if (BCM_PBMP_MEMBER(PBMP_SFI_ALL(unit), port)) {
            ...
            BCMDNX_IF_ERR_EXIT(arad_link_port_fault_get(unit, link_id, flags));
        } else if (BCM_PBMP_MEMBER(PBMP_PORT_ALL(unit), port)) {
            int local_fault, remote_fault;

            BCMDNX_IF_ERR_EXIT(soc_pm_local_fault_get(unit, port, &local_fault));
            BCMDNX_IF_ERR_EXIT(soc_pm_remote_fault_get(unit, port, &remote_fault));
...
#endif
    {
        BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("bcm_petra_port_fault_get is not supported")));
    }
...
_bcm_petra_port_find_free_port_and_allocate(int unit, bcm_port_if_t interface, int with_id, bcm_port_t *port)
{
    ...
    SOCDNX_IF_ERR_RETURN(soc_port_sw_db_valid_ports_get(unit, 0, &ports_bm));
    ...
    SOC_PBMP_ITER(SOC_DPP_CONFIG(unit)->arad->reserved_ports, port_i)
    {
        ...
        if (!is_valid) {
            if (SOC_PBMP_MEMBER(SOC_DPP_CONFIG(unit)->arad->reserved_ports, port_i))
            {
                (*port) = port_i;
                found = 1;
                break;
    ...
    SOC_PBMP_ITER(ports_bm, port_i){
        ...
        if(interface == interface_i) {
            SOCDNX_IF_ERR_RETURN(soc_port_sw_db_channel_get(unit, port_i, &tchannel));
            channels[tchannel] = 1;
        }
    }
    ...
    BCMDNX_IF_ERR_EXIT(soc_port_sw_db_port_add(unit, *port, 0, tchannel, 0, interface, phy_ports));
    BCMDNX_IF_ERR_EXIT(soc_port_sw_db_local_to_tm_port_set(unit, *port, *port));
    BCMDNX_IF_ERR_EXIT(soc_port_sw_db_local_to_pp_port_set(unit, *port, *port));
    ...
    bcm_gport_t *tunnel_port_id)
{
    ...
    if (!IS_PON_PORT(unit,_BCM_PPD_GPORT_PON_TO_PHY_PORT(gport_info.local_port))) {
    ...
    SOC_IF_ERROR_RETURN(_bcm_petra_port_find_free_port_and_allocate(unit, SOC_PORT_IF_NOCXN, 1, &pon_pp_port));
    ...
    soc_sand_rv = soc_ppd_port_info_set(unit,pon_pp_port,&port_info);
    BCM_SAND_IF_ERR_EXIT(soc_sand_rv); 
    ...
    BCM_SAND_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_port_pp_port_set,(unit, pon_pp_port, core, &pp_port_info, &failure_indication)));
    SOC_SAND_IF_FAIL_RETURN(failure_indication);
    ...
    bcm_port_match_info_t *match)
{
    ...
    rv = _bcm_dpp_gport_to_lif(unit, port, &in_lif_id, &out_lif_id, NULL ,&is_local);
    BCMDNX_IF_ERR_EXIT(rv);
...
int bcm_petra_port_egress_set(int unit, bcm_port_t port, int modid, bcm_pbmp_t pbmp) 
{
    ...
    SOC_PBMP_ITER(pbmp, egr_port) {
        ...
        /* Add to DB */
        BCMDNX_IF_ERR_EXIT(soc_port_sw_db_port_add(unit, egr_port, 0, tchannel, flags, interface, phy_ports));
        ...
        /* Set TM port */
        BCMDNX_IF_ERR_EXIT(arad_egr_dsp_pp_to_base_q_pair_set(unit, egr_port, 0, base_q_pair));
...
{
    uint32      is_valid, out_port_priority, tm_port, pp_port, first_header_size, first_phy_port;
    uint32      temp_flags = 0;
    uint32      mirror_profile, nof_lanes, nof_channels, kbits_sec_max_get, dummy1, dummy2, num_lanes;
    int         i, core, speed, base_q_pair, curr_q_pair, mc_reserved_pds, discount_data, header_type, is_channelized;
    ...
    BCMDNX_IF_ERR_EXIT(soc_port_sw_db_port_validate_and_add(unit, port, core, mapping_info->channel, temp_flags, 
    ...
    if (interface_info->interface == SOC_PORT_IF_ILKN || interface_info->interface == SOC_PORT_IF_CAUI) {
        if (first_phy_port == 1) {
            BCMDNX_IF_ERR_EXIT(soc_port_sw_db_protocol_offset_set(unit, port, 0));
        } else {
            BCMDNX_IF_ERR_EXIT(soc_port_sw_db_protocol_offset_set(unit, port, 1));
        }
    } 
    ...
    BCMDNX_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_parser_nof_bytes_to_remove_set, (unit, mapping_info->tm_port, core, first_header_size)));
    ...
    BCMDNX_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_egr_dsp_pp_to_base_q_pair_set, (unit, tm_port, core, base_q_pair)));
    
    /* Set port priorities */
    BCMDNX_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_egr_dsp_pp_priorities_mode_set, (unit, tm_port, core, out_port_priority)));

    /* Set Schedluing by default to be LOW priority */
    BCM_GPORT_LOCAL_SET(gport, tm_port);
    bcm_petra_cosq_gport_sched_set(unit,gport, 0, BCM_COSQ_SP1, 0);

    /* Set egress port shaper mode */
    BCMDNX_IF_ERR_EXIT(soc_port_sw_db_shaper_mode_get(unit,port,&shaper_mode));
    BCM_SAND_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_egr_dsp_pp_shaper_mode_set, (unit, tm_port, core, shaper_mode)));
    ...
    {
        BCMDNX_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_egr_prog_editor_profile_set, (unit, curr_q_pair, core, prog_editor_profile)));
    }
    ...
    if (!(flags & BCM_PORT_ADD_CGM_AUTO_ADJUST_DISABLE)) {
        BCMDNX_IF_ERR_EXIT(_bcm_petra_cosq_fc_port_threshold_init(unit, port, &mc_reserved_pds));
    ...
    if (interface_info->interface == BCM_PORT_IF_CAUI) {
        BCMDNX_IF_ERR_EXIT(soc_pm_port_clp_reset(unit, port));
    ...
    BCMDNX_IF_ERR_EXIT(_bcm_dpp_am_template_egress_port_discount_cls_init(unit,port,&egr_discount_info));


    /*port to IF mapping*/
    SOC_TMC_PORT2IF_MAPPING_INFO_clear(&port2if_mapping_info);
    BCMDNX_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_ports_port_to_nif_id_get, (unit, tm_port, core, &if_id)));
    port2if_mapping_info.if_id = if_id;
    port2if_mapping_info.channel_id = mapping_info->channel;
    BCM_SAND_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_port_to_dynamic_interface_map_set, (unit, core, tm_port, SOC_TMC_PORT_DIRECTION_BOTH, &port2if_mapping_info, FALSE)));
    ...
    BCM_SAND_IF_ERR_EXIT(soc_ppd_port_info_get(unit,pp_port,&port_info));

    port_info.enable_same_interfac_filter = TRUE;
    port_info.same_interface_filter_skip = FALSE;
    port_info.initial_action_profile.trap_code = _BCM_PETRA_UD_DFLT_TRAP;
    BCM_SAND_IF_ERR_EXIT(soc_ppd_port_info_set(unit,pp_port,&port_info));
    ...
    BCM_SAND_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_port_pp_port_set, (unit, pp_port, core, &conf,&success)));
    ...
    if (!is_channelized) {
        ...
        BCMDNX_IF_ERR_EXIT(soc_port_sw_db_num_lanes_get(unit, port, &num_lanes));
        ...
        if ((kbits_sec_max_get + speed*num_lanes*1000) <= SOC_TMC_IF_MAX_RATE_KBPS_ARAD) {
            BCMDNX_IF_ERR_EXIT(bcm_petra_cosq_gport_bandwidth_set(unit,fap_port,0,0,(kbits_sec_max_get + speed*num_lanes*1000),0));
        } else {
            BCMDNX_IF_ERR_EXIT(bcm_petra_cosq_gport_bandwidth_set(unit,fap_port,0,0,SOC_TMC_IF_MAX_RATE_KBPS_ARAD,0));
        }
    }
exit:
    /* set back to invalid */
...
{
    uint32           is_valid, is_master, counter_flags, num_lanes, dummy1, dummy2, kbits_sec_max_get;
    bcm_port_if_t    interface;
    int              core, old_profile, new_profile, is_last, is_allocated, mc_reserved_pds;
    int              base_q_pair, profile, rv, counter_interval, is_channelized, speed;
    uint32           tm_port, out_port_priority, nof_channels;
    bcm_gport_t      gport, fap_port;
    ...
    if (!is_channelized) {
        ...
        BCMDNX_IF_ERR_EXIT(bcm_petra_cosq_gport_bandwidth_get(unit,fap_port,0,&dummy1,&kbits_sec_max_get,&dummy2));
        if ((kbits_sec_max_get - speed*num_lanes*1000) > 0) {
            BCMDNX_IF_ERR_EXIT(bcm_petra_cosq_gport_bandwidth_set(unit,fap_port,0,0,(kbits_sec_max_get - speed*num_lanes*1000),0));
        } else {
            BCMDNX_IF_ERR_EXIT(bcm_petra_cosq_gport_bandwidth_set(unit,fap_port,0,0,0,0));
        }
    }    
    ...
    BCMDNX_IF_ERR_EXIT(_bcm_dpp_am_template_cosq_egr_thresh_data_get(unit,port,&thresh_info));
    /* Coverity - all tcs are expected to have the same value "mc_reserved_pds" */
    mc_reserved_pds = thresh_info.unsch_drop_packet_descriptors_available_tc[0];
    profile = SOC_DPP_DEFS_GET(unit, cosq_egr_default_thresh_type);
    BCMDNX_IF_ERR_EXIT(_bcm_petra_cosq_egress_thresh_entry_get(unit,profile,&thresh_info));
    BCMDNX_IF_ERR_EXIT(_bcm_dpp_am_template_cosq_egr_thresh_exchange(unit,tm_port,&thresh_info,&old_profile,&is_last,&new_profile,&is_allocated)); 
    /* end - init port cgm thresholds */

    /* reduce reserved pds for sp0 (only sp0 is used in dynamic port) */
    BCMDNX_IF_ERR_EXIT(soc_port_sw_db_local_to_out_port_priority_get(unit, tm_port, &out_port_priority));
    gport = 0; /* global threshold*/
    threshold.flags = BCM_COSQ_THRESHOLD_MULTICAST_SP0 | BCM_COSQ_THRESHOLD_EGRESS | BCM_COSQ_THRESHOLD_DROP;
    threshold.type = bcmCosqThresholdAvailablePacketDescriptors;
    BCMDNX_IF_ERR_EXIT(bcm_petra_cosq_gport_threshold_get(unit, gport, -1, &threshold));
    ...
    BCMDNX_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit ,mbcm_dpp_egr_q_cgm_interface_set, (unit, tm_port, core, ARAD_EGQ_IFC_DEF_VAL)));

    /* set cos_map_profile to 0 */
    BCMDNX_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_egr_q_profile_map_set, (unit, tm_port, core, 0)));

    /* PCT and PPCT cgm_port_profile init to default */
    BCM_SAND_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_egr_ofp_thresh_type_set,(unit, tm_port, core, SOC_TMC_EGR_PORT_THRESH_TYPE_15)));

    /* set TM port to invalid base_q_pair */
    BCM_SAND_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_egr_dsp_pp_to_base_q_pair_set,(unit, tm_port, core, ARAD_EGR_INVALID_BASE_Q_PAIR)));

    /* unmap port */
    BCM_SAND_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_port_to_dynamic_interface_unmap_set,(unit, core, tm_port, SOC_TMC_PORT_DIRECTION_BOTH)));
    
    /* release base q pair (PS) */
    BCMDNX_IF_ERR_EXIT(arad_ps_db_release_binding_ps(unit, port, base_q_pair));
