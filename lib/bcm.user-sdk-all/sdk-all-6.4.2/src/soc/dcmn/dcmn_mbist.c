    ...
    )
{
    ...
    LOG_DEBUG(BSL_LS_SOC_MBIST, (BSL_META_U(unit, "script %s starting in %u Microseconds"),
    ...
    while (nof_commands > 0) {
        ...
        case DCMN_MBIST_COMMAND_WAIT:
            ...
            sal_usleep((val32) / mbist_device->sleep_divisor); 
            break;
        default:
    ...
    )
{
    ...
    SOCDNX_IF_ERR_EXIT(soc_reg32_set(unit, mbist_device->reg_tap_command, REG_PORT_ANY, 0, 1));
    SOCDNX_IF_ERR_EXIT(soc_reg32_set(unit, mbist_device->reg_tap_command, REG_PORT_ANY, 0, 3));
    sal_usleep(10000);
exit:
    SOCDNX_FUNC_RETURN;
