...
int cl45_bus_read(void* user_acc, uint32_t core_addr, uint32_t reg_addr, uint32_t* val){
    portmod_default_user_access_t *cl45_user_data;
    ...
    return soc_dcmn_miim_cl45_read(cl45_user_data->unit, core_addr, reg_addr, (uint16 *) val);
}
...
int cl22_bus_read(void* user_acc, uint32_t core_addr, uint32_t reg_addr, uint32_t* val){
    portmod_default_user_access_t *cl22_user_data;
    ...
    return soc_dcmn_miim_cl22_read(cl22_user_data->unit, core_addr, reg_addr,(uint16 *) val);
}
 ...
 */
STATIC soc_error_t
soc_dcmn_mdio_phy_access_get(int unit, portmod_default_user_access_t *acc_data, int is_clause45, uint16 addr, phymod_access_t *access, int *is_sim)
{
    SOCDNX_INIT_FUNC_DEFS;
...
soc_dcmn_fabric_firmware_loader_callback(const phymod_core_access_t* core,  uint32_t length, const uint8_t* data)
{
    ...
    /*
    PHYMOD_IF_ERR_RETURN(phymod_core_firmware_info_get(core, &fw_info));
    if((fw_info.fw_version == fw_version) && (fw_info.fw_crc == fw_crc)){*/
        /*FW already loaded*/
    ...
    */
    ...
    PHYMOD_IF_ERR_RETURN(WRITE_BRDC_FSRD_WC_UC_MEM_MASK_BITMAPr(user_data->unit, 0xf)); /*all FSRDs*/
    for (i = 0 ; i < length ; i+= UC_TABLE_ENTRY_SIZE){
        ...
        PHYMOD_IF_ERR_RETURN(WRITE_BRDC_FSRD_FSRD_WL_EXT_MEMm(user_data->unit, MEM_BLOCK_ALL , 0, wr_data));
    }
    PHYMOD_IF_ERR_RETURN(WRITE_BRDC_FSRD_WC_UC_MEM_MASK_BITMAPr(user_data->unit, 0x0));
...
soc_dcmn_fabric_pms_add(int unit, int cores_num , int first_port, int use_mutex, int quads_in_fsrd, core_address_get_f address_get_func, void **alloced_buffer)
{
    ...
    portmod_default_user_access_t *user_data = NULL;
    uint32 tx_lane_map;
    int cl, is_cl45;
    SOCDNX_INIT_FUNC_DEFS;

    SOCDNX_IF_ERR_EXIT(portmod_pm_create_info_t_init(unit, &pm_info));
    user_data = sal_alloc(sizeof(portmod_default_user_access_t)*cores_num , "user_data");
    ...
    for(i = 0 ; i < cores_num ; i++){
        ...
        user_data[i].blk_id = -1; /*Not relevant for MDIO access*/
    }
    ...
    for(i = 0; i < cores_num ; i++){
        ...
        pm_info.pm_specific_info.dnx_fabric.fsrd_internal_quad = (i % quads_in_fsrd);
...
soc_error_t
soc_dcmn_fabric_port_probe(int unit, int port, dcmn_port_fabric_init_config_t* port_config)
{
...
soc_dcmn_port_phy_control_set(int unit, soc_port_t port, soc_phy_control_t type, uint32 value)
{
    ...
    SOCDNX_IF_ERR_EXIT(soc_port_control_set_wrapper(unit, is_lane_control, &phys, 1, type, value));
...
soc_dcmn_port_phy_control_get(int unit, soc_port_t port, soc_phy_control_t type, uint32 *value)
{
    ...
    SOCDNX_IF_ERR_EXIT(soc_port_control_get_wrapper(unit, is_lane_control, &phys, 1, type, value));
