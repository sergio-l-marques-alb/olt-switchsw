    ...
    soc_port_phy_eyescan_results_t* results)    /*array of result per port*/
{
    ...
    for (j = 0; j < nof_ports; j++) {
        ...
        for (idx = 0; idx < pmc[j]->num_phys; idx++) {
            ...
            if (rc) {
                 LOG_CLI((BSL_META_U(unit, "Error: Lowber eyescan measure rc = %d\n"), rc));
                 return (rc);
            }
            rc = phymod_phy_display_lowber_eye(pa, eyescan_options, buffer);
            if (rc) {
                 LOG_CLI((BSL_META_U(unit, "Error: Lowber eyescan display rc = %d\n"), rc));
                 return (rc);
            }
        }
    }
    ...
    soc_port_phy_eyescan_results_t* results)    /*array of result per port*/
{
    ...
    rv = _soc_eye_scan_get_local_ports(unit, inst, nof_ports, ports, local_ports);
    if (SOC_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_SOC_PHY,
                  (BSL_META_U(unit,
                              "%s\n"), soc_errmsg(rv)));
    ...
    for(j = 0 ; j < nof_ports ; j++) {
        ...
        for (idx = 0; idx < pmc[j]->num_phys; idx++) {
            ...
            rv = phymod_phy_meas_eye_scan_start(pa, 0); /* 0 = Horiz, 1 = Vertical */
            if (SOC_FAILURE(rv)) {
                LOG_ERROR(BSL_LS_SOC_PHY,
                          (BSL_META_U(unit,
                                      "%s\n"), soc_errmsg(rv)));
