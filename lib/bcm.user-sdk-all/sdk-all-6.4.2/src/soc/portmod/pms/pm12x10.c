...
int pm12x10_port_attach(int unit, int port, pm_info_t pm_info, const portmod_port_add_info_t* add_info)
{
    ...
    rv = WRITE_CXX_CXXPORT_MODE_REGr(unit, PM_12x10_INFO(pm_info)->blk_id | SOC_REG_ADDR_BLOCK_ID_MASK, 1); 
    _SOC_IF_ERR_EXIT(rv);
...
int pm12x10_port_detach(int unit, int port, pm_info_t pm_info)
{
    ...
    rv = WRITE_CXX_CXXPORT_MODE_REGr(unit, PM_12x10_INFO(pm_info)->blk_id | SOC_REG_ADDR_BLOCK_ID_MASK, 0); 
    _SOC_IF_ERR_EXIT(rv);
    for(i = 0 ; i < 3 ; i++)
    {
