...
{    
    ...
    rv = soc_dcmn_drv_fabric_pipe_map_initalize_valid_configurations(unit, 
    ...
    if (nof_pipes == SOC_DFE_PROPERTY_UNAVAIL) /* soc property isn't supported*/
    {
        ...
        SOC_DFE_FABRIC_PIPES_CONFIG(unit).mapping_type = (SOC_DFE_CONFIG(unit).is_dual_mode? soc_dcmn_drv_tdm_non_tdm_config_dual_pipe:soc_dcmn_drv_config_single_pipe);
    ...
    if (!is_valid) /* invalid configuration */
    {
        ...
        {
            SOC_DFE_FABRIC_PIPES_CONFIG(unit).mapping_type = (nof_pipes == 2) ? soc_dcmn_drv_custom_config_dual_pipe:soc_dcmn_drv_custom_config_triple_pipe;
            LOG_CLI((BSL_META_U(unit,
                                "warning: using an invalid configuration for fabric pipes")));
        }
    }

    if (SOC_DFE_FABRIC_PIPES_CONFIG(unit).mapping_type == soc_dcmn_drv_uc_hp_mc_lp_mc_config_triple_pipe &&
...
soc_dfe_drv_soc_properties_port_read(int unit)
{
    ...
    {
        SOC_DFE_CONFIG(unit).fabric_clk_freq_quad_26 = soc_dfe_property_suffix_num_get_only_suffix(unit, 26, spn_SERDES_FABRIC_CLK_FREQ, "quad", 1, -1); 
        SOC_DFE_CONFIG(unit).fabric_clk_freq_quad_35 = soc_dfe_property_suffix_num_get_only_suffix(unit, 35, spn_SERDES_FABRIC_CLK_FREQ, "quad", 1, -1);
    }
...
soc_dfe_drv_soc_properties_general_read(int unit)
{
    ...
    SOC_DFE_CONFIG(unit).custom_feature_lab=soc_dfe_property_suffix_num_get(unit,0,spn_CUSTOM_FEATURE,"lab",1,0);
...
_soc_dfe_drv_soc_property_serdes_qrtt_read(int unit)
{
    ...
    int                 quad, quad_active, disabled_port, quad_index, port, rc;
...
int
soc_dfe_drv_graceful_shutdown_set(int unit, soc_pbmp_t active_links, int shutdown, soc_pbmp_t unisolated_links, int isolate_device) 
{
    int rc;
    ...
    if (SOC_DFE_IS_FE2(unit)) {
        ...
        if (shutdown) {
            rc = MBCM_DFE_DRIVER_CALL(unit,mbcm_dfe_drv_graceful_shutdown_set,(unit, active_links, shutdown, unisolated_links));
            SOCDNX_IF_ERR_EXIT(rc);
            ...
            if (!isolate_device)
            {
                rc = MBCM_DFE_DRIVER_CALL(unit,mbcm_dfe_fabric_topology_isolate_set,(unit, soc_dcmn_isolation_status_active)); 
                SOCDNX_IF_ERR_EXIT(rc);   
            }
            rc = MBCM_DFE_DRIVER_CALL(unit,mbcm_dfe_drv_graceful_shutdown_set,(unit, active_links, shutdown, unisolated_links));
            SOCDNX_IF_ERR_EXIT(rc);
        ...
        rc =  MBCM_DFE_DRIVER_CALL(unit, mbcm_dfe_drv_fe13_graceful_shutdown_set, (unit, active_links, unisolated_links, shutdown));
        SOCDNX_IF_ERR_EXIT(rc);
    } else {
        SOCDNX_EXIT_WITH_ERR(SOC_E_FAIL, (_BSL_SOCDNX_MSG("Unknown fabric device mode")));
    }
...
soc_dfe_init_no_reset(int unit)
{
    ...
    if (!SOC_WARM_BOOT(unit) && !SOC_IS_RELOADING(unit))
    {
        ...
        rc = soc_dfe_drv_graceful_shutdown_set(unit, pbmp_enabled, 1, pbmp_unisolated, 0);
        SOCDNX_IF_ERR_EXIT(rc);
        ...
        rc = soc_dfe_drv_graceful_shutdown_set(unit, pbmp_enabled, 0, pbmp_unisolated, device_isolation_status == soc_dcmn_isolation_status_isolated ? 1:0);
        SOCDNX_IF_ERR_EXIT(rc);
...
soc_dfe_chip_driver_find(int unit , uint16 pci_dev_id, uint8 pci_rev_id)
{
    ...
    if (soc_cm_get_id_driver(pci_dev_id, pci_rev_id, &driver_dev_id, &driver_rev_id) < 0) {
     ...
     case BCM88950_DEVICE_ID:
         if (pci_rev_id == BCM88950_A0_REV_ID)
         {
...
soc_dfe_info_config(int unit, int dev_id)
{
    ...
    for (blk = 0; SOC_BLOCK_INFO(unit, blk).type >= 0; blk++) {
        ...
        if (SOC_IS_FE1600_REDUCED(unit)) 
        {
            ...
            if (SOC_BLOCK_INFO(unit, blk).number >= nof_block_instances)
            {
                si->block_valid[blk] = 0;
            } else {
...
soc_dfe_drv_soc_properties_dump(int unit)
{
    ...
    if (SOC_DFE_IS_FE13_ASYMMETRIC(unit))
    {
        SOC_DFE_DRV_SUPPORTED_SOC_PROPERTY_SUFFIX_NUM_INT_DUMP(unit, spn_SERDES_FABRIC_CLK_FREQ, "quad", 26, SOC_DFE_CONFIG(unit).fabric_clk_freq_quad_26);
        SOC_DFE_DRV_SUPPORTED_SOC_PROPERTY_SUFFIX_NUM_INT_DUMP(unit, spn_SERDES_FABRIC_CLK_FREQ, "quad", 35, SOC_DFE_CONFIG(unit).fabric_clk_freq_quad_35);
    }
