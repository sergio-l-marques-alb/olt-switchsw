...
#define _SOC_FE3200_MAX_FAP_ID (0x3f*SOC_FE3200_STK_FAP_GROUP_SIZE)
...
_soc_fe3200_drv_fabric_device_mode_set(int unit, soc_dfe_fabric_device_mode_t fabric_device_mode)
{
    ...
    soc_reg_field_set(unit, ECI_GLOBAL_GENERAL_FE_CFG_1r, &reg32_val, FE_13_MODEf, 
                            (fabric_device_mode == soc_dfe_fabric_device_mode_multi_stage_fe13) ? 1 : 0);
    /*Asymmetrical FE13*/
    ...
    soc_reg_field_set(unit, ECI_GLOBAL_GENERAL_FE_CFG_1r, &reg32_val, MULTISTAGE_MODEf, 
                            (fabric_device_mode == soc_dfe_fabric_device_mode_multi_stage_fe2) ? 1 : 0);
    ...
    repeater_mode_set = (fabric_device_mode == soc_dfe_fabric_device_mode_repeater) ? 0 : 1;
...
_soc_fe3200_drv_fabric_multicast_config_set(int unit, soc_dfe_multicast_mode_t mc_mode, int fe_mc_priority_map_enable)
{
    ...
    for (i=0 ; i < SOC_DFE_DEFS_GET(unit, nof_instances_dch) ; i++)
    {
        ...
        soc_reg_field_set(unit, DCH_PRIORITY_TRANSLATIONr, &reg_val32, LOW_PRE_MUL_SETf, SOC_DFE_CONFIG(unit).fe_mc_priority_map_enable ? 0 : 1);
        soc_reg_field_set(unit, DCH_PRIORITY_TRANSLATIONr, &reg_val32, MID_PRE_MUL_SETf, SOC_DFE_CONFIG(unit).fe_mc_priority_map_enable ? 0 : 1); 
...
{
	int min_tdm_priority, i, priority_bmp_val = 0;
	uint32 reg_val32, mask = 0;
	uint64 reg_val64;

	SOCDNX_INIT_FUNC_DEFS;

	min_tdm_priority =  (SOC_DFE_CONFIG(unit).fabric_tdm_priority_min == SOC_DFE_FABRIC_TDM_PRIORITY_NONE ? 
                         SOC_FE3200_MAX_NUM_OF_PRIORITIES : 
                         SOC_DFE_CONFIG(unit).fabric_tdm_priority_min);
	if (local_routing_enable == 1)
	{
		for (i = min_tdm_priority ; i > 0  ; i--)
		{
			mask = 1 << (i-1);
			priority_bmp_val |= mask;
		}
	}

	SOCDNX_IF_ERR_EXIT(READ_RTP_DRH_LOAD_BALANCING_GENERAL_CONFIGr(unit, &reg_val64));
	soc_reg64_field32_set(unit, RTP_DRH_LOAD_BALANCING_GENERAL_CONFIGr, &reg_val64, ENABLE_LOCAL_FE_1_ROUTINGf, priority_bmp_val);
...
soc_fe3200_set_fsrd_config(int unit)
{
    ...
    {
        if (SOC_DFE_CONFIG(unit).fabric_clk_freq_quad_26 != -1)
        {
            SOCDNX_IF_ERR_EXIT(READ_FSRD_SRD_QUAD_CTRLr(unit, 2, 2, &reg_val32)); /* 2*12 + 2 */
            soc_reg_field_set(unit, FSRD_SRD_QUAD_CTRLr, &reg_val32, SRD_QUAD_N_LCREF_ENf, 0);
            SOCDNX_IF_ERR_EXIT(WRITE_FSRD_SRD_QUAD_CTRLr(unit, 2, 2, reg_val32));       
        }
        if (SOC_DFE_CONFIG(unit).fabric_clk_freq_quad_35 != -1)
        {
            SOCDNX_IF_ERR_EXIT(READ_FSRD_SRD_QUAD_CTRLr(unit, 11, 2, &reg_val32)); /* 2*12 + 11 */
            soc_reg_field_set(unit, FSRD_SRD_QUAD_CTRLr, &reg_val32, SRD_QUAD_N_LCREF_ENf, 0);
...
soc_fe3200_set_rtp_config(int unit)
{
    ...
    SOCDNX_IF_ERR_EXIT(soc_fe3200_stk_module_max_fap_set(unit, _SOC_FE3200_MAX_FAP_ID));
...
soc_fe3200_set_dch_config(int unit)
{
    ...
    if (SOC_DFE_IS_FE13(unit))
    {
        for (i=0 ; i < nof_instances_dch/2; i++) { /*FE1*/
            ...
            SOCDNX_IF_ERR_EXIT(READ_DCH_GLOBAL_FE_DEST_IDS_2r(unit, i, &reg_val32));
            soc_reg_field_set(unit, DCH_GLOBAL_FE_DEST_IDS_2r, &reg_val32, FE_DEST_ID_2_ENf, 0X0);
            soc_reg_field_set(unit, DCH_GLOBAL_FE_DEST_IDS_2r, &reg_val32, FE_DEST_ID_3_ENf, 0x0);
            SOCDNX_IF_ERR_EXIT(WRITE_DCH_GLOBAL_FE_DEST_IDS_2r(unit, i, reg_val32));
        }
    }
...
soc_fe3200_set_dcm_config(int unit)
{
    ...
    for (i=0 ; i < 3; i++) /* iterate over all possible pipes*/
    {
        ...
        SOCDNX_IF_ERR_EXIT(soc_fe3200_fabric_flow_control_mid_fifo_size_threshold_set(unit, soc_dfe_fabric_link_fifo_type_index_0, pipes[i], dcm_thresholds_default_values.fifo_size[i]));
        SOCDNX_IF_ERR_EXIT(soc_fe3200_fabric_flow_control_mid_fifo_size_threshold_set(unit, soc_dfe_fabric_link_fifo_type_index_1, pipes[i], dcm_thresholds_default_values.fifo_size[i]));

        /* set bcmFabricLinkMidAlmostFull */

        SOCDNX_IF_ERR_EXIT(soc_fe3200_fabric_flow_control_mid_almost_full_threshold_set(unit, soc_dfe_fabric_link_fifo_type_index_0, pipes[i], dcm_thresholds_default_values.almost_full_threshold[i]));
        SOCDNX_IF_ERR_EXIT(soc_fe3200_fabric_flow_control_mid_almost_full_threshold_set(unit, soc_dfe_fabric_link_fifo_type_index_1, pipes[i], dcm_thresholds_default_values.almost_full_threshold[i]));

        /* set bcmFabricLinkMidPrio[0-3]Drop*/

        SOCDNX_IF_ERR_EXIT(soc_fe3200_fabric_flow_control_mid_prio_drop_threshold_set(unit, bcmFabricLinkMidPrio0Drop, soc_dfe_fabric_link_fifo_type_index_0, pipes[i], dcm_thresholds_default_values.prio_0_threshold[i]));
        SOCDNX_IF_ERR_EXIT(soc_fe3200_fabric_flow_control_mid_prio_drop_threshold_set(unit, bcmFabricLinkMidPrio0Drop, soc_dfe_fabric_link_fifo_type_index_1, pipes[i], dcm_thresholds_default_values.prio_0_threshold[i]));

        SOCDNX_IF_ERR_EXIT(soc_fe3200_fabric_flow_control_mid_prio_drop_threshold_set(unit, bcmFabricLinkMidPrio1Drop, soc_dfe_fabric_link_fifo_type_index_0, pipes[i], dcm_thresholds_default_values.prio_1_threshold[i]));
        SOCDNX_IF_ERR_EXIT(soc_fe3200_fabric_flow_control_mid_prio_drop_threshold_set(unit, bcmFabricLinkMidPrio1Drop, soc_dfe_fabric_link_fifo_type_index_1, pipes[i], dcm_thresholds_default_values.prio_1_threshold[i]));

        SOCDNX_IF_ERR_EXIT(soc_fe3200_fabric_flow_control_mid_prio_drop_threshold_set(unit, bcmFabricLinkMidPrio2Drop, soc_dfe_fabric_link_fifo_type_index_0, pipes[i], dcm_thresholds_default_values.prio_2_threshold[i]));
        SOCDNX_IF_ERR_EXIT(soc_fe3200_fabric_flow_control_mid_prio_drop_threshold_set(unit, bcmFabricLinkMidPrio2Drop, soc_dfe_fabric_link_fifo_type_index_1, pipes[i], dcm_thresholds_default_values.prio_2_threshold[i]));

        SOCDNX_IF_ERR_EXIT(soc_fe3200_fabric_flow_control_mid_prio_drop_threshold_set(unit, bcmFabricLinkMidPrio3Drop, soc_dfe_fabric_link_fifo_type_index_0, pipes[i], dcm_thresholds_default_values.prio_3_threshold[i]));
        SOCDNX_IF_ERR_EXIT(soc_fe3200_fabric_flow_control_mid_prio_drop_threshold_set(unit, bcmFabricLinkMidPrio3Drop, soc_dfe_fabric_link_fifo_type_index_1, pipes[i], dcm_thresholds_default_values.prio_3_threshold[i]));

        /* set bcmFabricLinkMidGciLvl[1-3]FC */

        SOCDNX_IF_ERR_EXIT(soc_fe3200_fabric_flow_control_mid_gci_threshold_set(unit, bcmFabricLinkMidGciLvl1FC, soc_dfe_fabric_link_fifo_type_index_0, pipes[i], dcm_thresholds_default_values.gci_low_threshold[i]));
        SOCDNX_IF_ERR_EXIT(soc_fe3200_fabric_flow_control_mid_gci_threshold_set(unit, bcmFabricLinkMidGciLvl1FC, soc_dfe_fabric_link_fifo_type_index_1, pipes[i], dcm_thresholds_default_values.gci_low_threshold[i]));

        SOCDNX_IF_ERR_EXIT(soc_fe3200_fabric_flow_control_mid_gci_threshold_set(unit, bcmFabricLinkMidGciLvl2FC, soc_dfe_fabric_link_fifo_type_index_0, pipes[i], dcm_thresholds_default_values.gci_med_threshold[i]));
        SOCDNX_IF_ERR_EXIT(soc_fe3200_fabric_flow_control_mid_gci_threshold_set(unit, bcmFabricLinkMidGciLvl2FC, soc_dfe_fabric_link_fifo_type_index_1, pipes[i], dcm_thresholds_default_values.gci_med_threshold[i]));

        SOCDNX_IF_ERR_EXIT(soc_fe3200_fabric_flow_control_mid_gci_threshold_set(unit, bcmFabricLinkMidGciLvl3FC, soc_dfe_fabric_link_fifo_type_index_0, pipes[i], dcm_thresholds_default_values.gci_high_threshold[i]));
        SOCDNX_IF_ERR_EXIT(soc_fe3200_fabric_flow_control_mid_gci_threshold_set(unit, bcmFabricLinkMidGciLvl3FC, soc_dfe_fabric_link_fifo_type_index_1, pipes[i], dcm_thresholds_default_values.gci_high_threshold[i]));

        /* set bcmFabricLinkMidRciLvl[1-3]FC */

        SOCDNX_IF_ERR_EXIT(soc_fe3200_fabric_flow_control_mid_rci_threshold_set(unit, bcmFabricLinkMidRciLvl1FC, soc_dfe_fabric_link_fifo_type_index_0, pipes[i], dcm_thresholds_default_values.rci_low_threshold[i]));
        SOCDNX_IF_ERR_EXIT(soc_fe3200_fabric_flow_control_mid_rci_threshold_set(unit, bcmFabricLinkMidRciLvl1FC, soc_dfe_fabric_link_fifo_type_index_1, pipes[i], dcm_thresholds_default_values.rci_low_threshold[i]));

        SOCDNX_IF_ERR_EXIT(soc_fe3200_fabric_flow_control_mid_rci_threshold_set(unit, bcmFabricLinkMidRciLvl2FC, soc_dfe_fabric_link_fifo_type_index_0, pipes[i], dcm_thresholds_default_values.rci_med_threshold[i]));
        SOCDNX_IF_ERR_EXIT(soc_fe3200_fabric_flow_control_mid_rci_threshold_set(unit, bcmFabricLinkMidRciLvl2FC, soc_dfe_fabric_link_fifo_type_index_1, pipes[i], dcm_thresholds_default_values.rci_med_threshold[i]));

        SOCDNX_IF_ERR_EXIT(soc_fe3200_fabric_flow_control_mid_rci_threshold_set(unit, bcmFabricLinkMidRciLvl3FC, soc_dfe_fabric_link_fifo_type_index_0, pipes[i], dcm_thresholds_default_values.rci_high_threshold[i]));
        SOCDNX_IF_ERR_EXIT(soc_fe3200_fabric_flow_control_mid_rci_threshold_set(unit, bcmFabricLinkMidRciLvl3FC, soc_dfe_fabric_link_fifo_type_index_1, pipes[i], dcm_thresholds_default_values.rci_high_threshold[i]));

        /* set DCM Full thresholds */
        for (j=0 ; j < nof_instances_dcm ; j++)
        {
            SOCDNX_IF_ERR_EXIT(READ_DCM_FIFOS_THRESHOLDS_P_TYPE_0r(unit, j, i, &reg_val64));
            soc_reg64_field32_set(unit, DCM_FIFOS_THRESHOLDS_P_TYPE_0r, &reg_val64, DCM_FULL_TH_P_N_TYPE_0f, dcm_thresholds_default_values.full_threshold[i]);
            SOCDNX_IF_ERR_EXIT(WRITE_DCM_FIFOS_THRESHOLDS_P_TYPE_0r(unit, j , i, reg_val64));

            SOCDNX_IF_ERR_EXIT(READ_DCM_FIFOS_THRESHOLDS_P_TYPE_1r(unit, j, i, &reg_val64));
            soc_reg64_field32_set(unit, DCM_FIFOS_THRESHOLDS_P_TYPE_1r, &reg_val64, DCM_FULL_TH_P_N_TYPE_1f, dcm_thresholds_default_values.full_threshold[i]);
            SOCDNX_IF_ERR_EXIT(WRITE_DCM_FIFOS_THRESHOLDS_P_TYPE_1r(unit, j , i, reg_val64));
        }
    }
...
soc_fe3200_drv_pll_config_set(int unit)
{
    ...
    if (!SHR_BITGET(&reg32_val, SOC_FE3200_PORT_ECI_POWER_UP_CONFIG_DYNAMIC_PLL_BIT) && !SAL_BOOT_PLISIM)
    {
    ...
    for(lcpll=0 ; lcpll<SOC_DFE_DEFS_GET(unit, nof_lcpll) ; lcpll++) 
    {
        ...
        SOC_REG_ABOVE_64_WORD_SET(reg_above_64_val, SOC_FE3200_PORT_PLL_CONFIG_DEFAULT_WORD_5, 5);
        ...
        {
            case 0:
                SOCDNX_IF_ERR_EXIT(WRITE_ECI_MISC_PLL_0_CONFIGr(unit, reg_above_64_val));
                break;

            case 1:
                SOCDNX_IF_ERR_EXIT(WRITE_ECI_MISC_PLL_1_CONFIGr(unit, reg_above_64_val));
                break;

            case 2:
                SOCDNX_IF_ERR_EXIT(WRITE_ECI_MISC_PLL_2_CONFIGr(unit, reg_above_64_val));
                break;

            case 3:
                SOCDNX_IF_ERR_EXIT(WRITE_ECI_MISC_PLL_3_CONFIGr(unit, reg_above_64_val));
                break;

            default:
                SOCDNX_EXIT_WITH_ERR(SOC_E_CONFIG, (_BSL_SOCDNX_MSG("lcpll %d is not supported"), lcpll));
                break;
        }
    }
...
soc_fe3200_drv_pvt_monitor_enable(int unit)
{
    ...
    SOCDNX_IF_ERR_EXIT(READ_ECI_PVT_MON_A_CONTROL_REGr(unit, &reg64_val));
    COMPILER_64_BITSET(reg64_val, _SOC_FE3200_ECI_PVT_MON_CONTROL_REG_RESET_BIT);
    SOCDNX_IF_ERR_EXIT(WRITE_ECI_PVT_MON_A_CONTROL_REGr(unit, reg64_val));
    
    SOCDNX_IF_ERR_EXIT(READ_ECI_PVT_MON_B_CONTROL_REGr(unit, &reg64_val));
    COMPILER_64_BITSET(reg64_val, _SOC_FE3200_ECI_PVT_MON_CONTROL_REG_RESET_BIT);
    SOCDNX_IF_ERR_EXIT(WRITE_ECI_PVT_MON_B_CONTROL_REGr(unit, reg64_val));

    SOCDNX_IF_ERR_EXIT(READ_ECI_PVT_MON_C_CONTROL_REGr(unit, &reg64_val));
    COMPILER_64_BITSET(reg64_val, _SOC_FE3200_ECI_PVT_MON_CONTROL_REG_RESET_BIT);
    SOCDNX_IF_ERR_EXIT(WRITE_ECI_PVT_MON_C_CONTROL_REGr(unit, reg64_val));

    SOCDNX_IF_ERR_EXIT(READ_ECI_PVT_MON_D_CONTROL_REGr(unit, &reg64_val));
    COMPILER_64_BITSET(reg64_val, _SOC_FE3200_ECI_PVT_MON_CONTROL_REG_RESET_BIT);
    SOCDNX_IF_ERR_EXIT(WRITE_ECI_PVT_MON_D_CONTROL_REGr(unit, reg64_val));
...
soc_fe3200_drv_test_brdc_blk_filter(int unit, soc_reg_t reg, int *is_filtered)
{
    ...
    *is_filtered = 0;
    ...
    {   
        ...
        case BRDC_FMAC_SBUS_LAST_IN_CHAINr:
...
soc_fe3200_drv_test_brdc_blk_info_get(int unit, int max_size, soc_reg_brdc_block_info_t *brdc_info, int *actual_size)
{
    ...
    *actual_size = 0;
    ...
    if (max_size > *actual_size)
    {
        ...
        for (i = 0, instance = SOC_DFE_DEFS_GET(unit, nof_instances_mac) / 2; instance < 3 * SOC_DFE_DEFS_GET(unit, nof_instances_mac) / 4; instance++, i++)
        {
            brdc_info[*actual_size].blk_ids[i] = FMAC_BLOCK(unit, instance); 
        }
...
soc_fe3200_drv_fe13_graceful_shutdown_set(int unit, soc_pbmp_t active_links, soc_pbmp_t unisolated_links, int shutdown) 
{
    ...
    if (shutdown)
    {
        ...
        {
            rv = soc_fe3200_port_control_rx_enable_set(unit, port, 0, 0);
            SOCDNX_IF_ERR_EXIT(rv);
        }
        ...
        SOC_PBMP_ITER(active_links, port)
        {
            rv = soc_fe3200_port_control_tx_enable_set(unit, port, 0);
            SOCDNX_IF_ERR_EXIT(rv);
        }
        ...
        SOC_PBMP_ITER(active_links, port)
        {
            rv = soc_fe3200_port_control_tx_enable_set(unit, port, 1);
            SOCDNX_IF_ERR_EXIT(rv);
        }
        ...
        SOC_PBMP_ITER(active_links, port)
        {
            rv = soc_fe3200_port_control_rx_enable_set(unit, port, 0, 1);
            SOCDNX_IF_ERR_EXIT(rv);
        }
