  ...
  )
{
  ...
  *success = SOC_SAND_SUCCESS;
  ...
  SOC_SAND_SET_BIT(fld_val, enable_mirror, eq_mirror_enable_reg_offset);
  ...
  )
{
  ...
  SOC_PB_PP_STRUCT_VERIFY(SOC_PB_PP_EG_MIRROR_PORT_DFLT_INFO, dflt_mirroring_info, 20, exit);
  ...
  )
{
  ...
  SOC_PB_PP_EG_MIRROR_PORT_DFLT_INFO_clear(dflt_mirroring_info);
  ...
  SOC_PB_FLD_GET(regs->epni.mirror_enable_reg[eq_mirror_enable_reg_index].mirror_enable , fld_val, 60, exit);
  dflt_mirroring_info->enable_mirror = SOC_SAND_NUM2BOOL(SOC_SAND_GET_BIT(fld_val, eq_mirror_enable_reg_offset));
