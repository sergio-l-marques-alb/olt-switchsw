...
soc_petra_default_config_tm_get(int unit)
{
  ...
  SOCDNX_INIT_FUNC_DEFS;
    ...
    SOC_DPP_CONFIG(unit)->tm.hr_isq = SOC_TMC_SCH_PORT_ID_INVALID_PETRA; /* PORT ISQ */
    for (i = 0; i < 4; i++) {
        SOC_DPP_CONFIG(unit)->tm.hr_fmqs[i] = SOC_TMC_SCH_PORT_ID_INVALID_PETRA; /* PORT FMQ */
    }    
...
soc_petra_info_config(int unit) {
    ...
    if (dpp_pb->op_mode.pp_enable) {
      ...
      if (soc_dpp_str_prop_parse_logical_port_mim(unit, &SOC_DPP_CONFIG(unit)->pp.mim_lif_ndx, &SOC_DPP_CONFIG(unit)->pp.mim_out_ac)) {
          return SOC_E_FAIL;
      }
...
int
soc_petra_allocate_tm_port_in_range_and_recycle_channel(int unit, int max_port, int *tm_port, uint32 *channel)
{
    int port;
