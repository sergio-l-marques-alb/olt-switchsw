...
port_termination_exit:
  ...
  for(core=0 ; core<SOC_DPP_DEFS_GET(unit, nof_cores) ; core++) {
      ...
      if(is_valid) {
          ...
          {
				  res = arad_egq_pct_tbl_get_unsafe(unit, curr_q_pair, core, &pct_tbl_data);
              ...
              if (soc_property_port_suffix_num_get(unit, pp_port, -1, spn_CUSTOM_FEATURE, "map_port_extr_enabled", 0) == 1) {
				  ...
				  res = arad_egq_pct_tbl_set_unsafe(unit, curr_q_pair, core, &pct_tbl_data);
    ...
    )
{
  ...
  */
      ...
      !(is_init)) /*if !is_init then it is a dynamic port - hence need to rewrite it to to the HW*/
  {
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 114, exit, soc_port_sw_db_tm_to_local_port_get(unit, port_ndx, core_id, &local_port));
  ...
  )
{
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 10, exit, soc_port_sw_db_tm_to_local_port_get(unit, tm_port, core_id, &local_port));
    ...
    )
{
    ...
    *if_id = ARAD_IF_ID_NONE;
    ...
    /* Get base qp */
    res = soc_port_sw_db_tm_port_to_base_q_pair_get(unit, tm_port, core_id, &base_q_pair);
  ...
  if (remove)
  {
    ...
    if (fap_id == sys_fap_id_self)
    {
      /* remove configuration of removed port */
        res = (MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_port_direct_lb_key_set,(unit, core, port_id, 0, 1 /* set_min */, 0, 1 /* set_max */)));
        SOC_SAND_CHECK_FUNC_RESULT(res, 42, exit);
    }
  }
...
uint32 alloc_reassembly_context_and_recycle_channel_unsafe(int unit, int core_id, uint32 out_port, uint32 *out_context, uint32 *out_channel)
{
   ...
   */
  res = arad_port_pp_port_get_unsafe(unit, out_port, core_id, &out_info);
   ...
   )
{
  ...
  res = soc_port_sw_db_tm_port_to_base_q_pair_get(unit, local_port, core_id, &base_q_pair);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 10, exit);
    ...
    )
{
  ...
  for(port_i=0; port_i < ARAD_PORT_NDX_MAX ; ++port_i) {
     ...
     */
    res = (MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_port_direct_lb_key_get, (unit, core, port_i, &min_lb_key, &max_lb_key)));
    SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);

    res = (MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_port_direct_lb_key_set,(unit, core, port_i, min_lb_key, TRUE, max_lb_key, TRUE)));
    SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);
    ...
    )
{
  ...
  res = arad_port_pp_port_get_unsafe(unit,index,core_id,&pp_port_info);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 12, exit);

  pp_port_info.header_type_out = ARAD_PORT_HEADER_TYPE_L2_REMOTE_CPU;

  res = arad_port_pp_port_set_unsafe(unit,index,core_id,&pp_port_info,&success);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 15, exit);
