  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_MGMT_CREDIT_WORTH_SET_UNSAFE);
  ...
  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  10,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, credit_value_reg, REG_PORT_ANY, 0, credit_value_lcl_field,  fld_val));
  ...
  BCM_PBMP_ITER(ports_map, port_i) { /* Look for ILKN/CAUI interfaces that need value different than default */
      ...
      if(is_ilkn_used && is_caui_used) break; /* No need to continue iterating */
  }
  ...
  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  20,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, SCH_ASSIGNED_CREDIT_CONFIGURATIONr, SOC_CORE_ALL, 0, ASSIGNED_CREDIT_WORTHf,  fld_val));

  /* Set Rebounded Credit worth equals to assigned credit worth */
  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  30,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, SCH_REBOUNDED_CREDIT_CONFIGURATIONr, SOC_CORE_ALL, 0, REBOUNDED_CREDIT_WORTHf,  fld_val));
  /* SCH } */

exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_mgmt_credit_worth_set_unsafe()",0,0);
}
  ...
  )
{
  ...
  SOC_SAND_ERR_IF_OUT_OF_RANGE(
    credit_worth, ARAD_CREDIT_SIZE_BYTES_MIN, ARAD_CREDIT_SIZE_BYTES_MAX,
  ...
  )
{
    ...
    credit_value_reg = SOC_IS_ARADPLUS(unit)? IPS_IPS_CREDIT_CONFIG_1r: IPS_IPS_CREDIT_CONFIGr;
    credit_value_lcl_field = SOC_IS_ARADPLUS(unit)? CREDIT_VALUE_1f: CREDIT_VALUEf;
    SOCDNX_IF_ERR_EXIT(soc_reg_above_64_field32_read(unit, credit_value_reg, REG_PORT_ANY, 0, credit_value_lcl_field, &fld_val));
  ...
  )
{
    ...
    soc_reg_field_set(unit, IPS_IPS_CREDIT_CONFIG_1r, &reg_val, CREDIT_VALUE_1f, credit_worth_local);
...
exit:
    SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_plus_mgmt_credit_worth_set_unsafe()", unit, credit_worth_local);
}
  ...
  )
{
    uint32 res, reg_val;
    SOC_SAND_INIT_ERROR_DEFINITIONS(0);
    SOC_SAND_CHECK_NULL_INPUT(credit_worth_local);
...
exit:
    SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_plus_mgmt_credit_worth_get_unsafe()", unit, 0);
}

#define ARAD_PLUS_GET_FAP_CREDIT_VALUE_MODE_WITH_OFFSET(per_module_credit_value, word, offset) (((per_module_credit_value)[word] >> (offset)) & 1)
#define ARAD_PLUS_SET_FAP_CREDIT_VALUE_MODE_WITH_OFFSET(per_module_credit_value, word, offset, mode) do { \
    (per_module_credit_value)[word] = ((per_module_credit_value)[word] & ~(1 << (offset))) | ((mode) << (offset)); \
} while (0);
#define ARAD_PLUS_GET_FAP_CREDIT_VALUE_MODE(per_module_credit_value, fap_id) ARAD_PLUS_GET_FAP_CREDIT_VALUE_MODE_WITH_OFFSET( \
    per_module_credit_value, fap_id / ARAD_PLUS_CREDIT_VALUE_MODES_PER_WORD, fap_id % ARAD_PLUS_CREDIT_VALUE_MODES_PER_WORD)
#define ARAD_PLUS_SET_FAP_CREDIT_VALUE_MODE(per_module_credit_value, fap_id, mode) ARAD_PLUS_SET_FAP_CREDIT_VALUE_MODE_WITH_OFFSET( \
    per_module_credit_value, fap_id / ARAD_PLUS_CREDIT_VALUE_MODES_PER_WORD, fap_id % ARAD_PLUS_CREDIT_VALUE_MODES_PER_WORD, mode)
...
{
    uint32 prev_type, *per_module_credit_value;
    soc_error_t
        rv;
    SOC_SAND_INIT_ERROR_DEFINITIONS(0);
    ...
    per_module_credit_value = SOC_DPP_CONFIG(unit)->arad_plus->per_module_credit_value;
    prev_type = ARAD_PLUS_GET_FAP_CREDIT_VALUE_MODE(per_module_credit_value, fap_id);
    if (credit_value_type != prev_type) {
        ARAD_PLUS_SET_FAP_CREDIT_VALUE_MODE(per_module_credit_value, fap_id, credit_value_type);
        if (credit_value_type == ARAD_PLUS_FAP_CREDIT_VALUE_REMOTE) {
            ++SOC_DPP_CONFIG(unit)->arad_plus->nof_remote_faps_with_remote_credit_value;
        } else if (prev_type == ARAD_PLUS_FAP_CREDIT_VALUE_REMOTE) {
            --SOC_DPP_CONFIG(unit)->arad_plus->nof_remote_faps_with_remote_credit_value;
        }
        rv = SOC_DPP_WB_ENGINE_SET_VAR(unit, SOC_DPP_WB_ENGINE_VAR_ARAD_PLUS_CONFIG, SOC_DPP_CONFIG(unit)->arad_plus);  /* update warm boot data */
  ...
  )
{
    ...
    *credit_value_type = ARAD_PLUS_GET_FAP_CREDIT_VALUE_MODE(SOC_DPP_CONFIG(unit)->arad_plus->per_module_credit_value, fap_id);
  ...
  )
{
    ...
    } else if (credit_value_to_use == ARAD_PLUS_FAP_CREDIT_VALUE_LOCAL) {
        SOC_DPP_CONFIG(unit)->arad_plus->nof_remote_faps_with_remote_credit_value = 0;
    } else if (credit_value_to_use == ARAD_PLUS_FAP_CREDIT_VALUE_REMOTE) {
        SOC_DPP_CONFIG(unit)->arad_plus->nof_remote_faps_with_remote_credit_value = ARAD_NOF_FAPS_IN_SYSTEM;
    } else {
        SOC_SAND_SET_ERROR_CODE(SOC_SAND_GEN_ERR, 120, exit);
    }

    /* change all fap modes to credit_value_to_use */
    sal_memset(SOC_DPP_CONFIG(unit)->arad_plus->per_module_credit_value, credit_value_to_use ? 255 : 0, ARAD_PLUS_CREDIT_VALUE_MODE_WORDS * sizeof(uint32));
    res = SOC_DPP_WB_ENGINE_SET_VAR(unit, SOC_DPP_WB_ENGINE_VAR_ARAD_PLUS_CONFIG, SOC_DPP_CONFIG(unit)->arad_plus);  /* update warm boot data */
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_MGMT_SYSTEM_FAP_ID_GET_UNSAFE);
  SOC_SAND_CHECK_NULL_INPUT(sys_fap_id);

  if (SOC_IS_JERICHO(unit)) {
    fld_val = 0;
  } else {
    SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  10,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_read(unit, ECI_GLOBAL_0r, REG_PORT_ANY, 0, PIPEIDf, &fld_val));
  }
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_mgmt_system_fap_id_get_unsafe()",0,0);
}
  ...
  )
{
  ...
  conf[conf_idx].val = (enable == TRUE ? soc_sand_link_fap_reachability_rate_get(arad_chip_ticks_per_sec_get(unit), SOC_DPP_DEFS_GET(unit, nof_fabric_links), SOC_ARAD_NOF_RMGR_UNITS_PER_LINK) : 0x0);
  ...
  conf[conf_idx].val = (enable == TRUE ? soc_sand_link_fap_reachability_watchdog_period_get(arad_chip_ticks_per_sec_get(unit), SOC_DPP_DEFS_GET(unit, nof_fabric_links)) : 0x0);
  conf[conf_idx].instance_id = REG_PORT_ANY;
  conf[conf_idx].polling_func = NULL;
  conf[conf_idx].failure_func = NULL;
  conf[conf_idx].err_on_fail = FALSE;
  ...
  for (inst_idx = 0; inst_idx < SOC_DPP_DEFS_GET(unit, nof_fabric_macs); inst_idx++)
  {
  conf[conf_idx].reg = FMAC_FABRIC_SYSTEM_CONFIG_0r;
  conf[conf_idx].field = FABRIC_SYSTEM_CONFIG_0f;
  ...
  conf[conf_idx].reg = FMAC_FABRIC_SYSTEM_CONFIG_1r ;
  conf[conf_idx].field = FABRIC_SYSTEM_CONFIG_1f;
  conf[conf_idx].val = (enable == TRUE ? 0xf : 0x0);
  conf[conf_idx].instance_id = inst_idx;
  conf[conf_idx].polling_func = NULL;
  conf[conf_idx].failure_func = NULL;
    ...
    */
    if (enable == FALSE) {
        ...
        */
         ...
         SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  20,  exit, ARAD_REG_ACCESS_ERR,WRITE_FDR_FDR_ENABLERS_REGISTER_1r(unit,  reg_val_64)); 
      }
  
      /*
      *  Disable internal data-path - just in case
      */
     SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  16,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, IDR_DYNAMIC_CONFIGURATIONr, REG_PORT_ANY, 0, ENABLE_DATA_PATHf,  enable_val));
     SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  18,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, IRR_DYNAMIC_CONFIGURATIONr, REG_PORT_ANY, 0, ENABLE_DATA_PATH_IDRf,  enable_val));
     SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  20,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, IRR_DYNAMIC_CONFIGURATIONr, REG_PORT_ANY, 0, ENABLE_DATA_PATH_IQMf,  enable_val));
     SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  22,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, IHP_IHP_ENABLERSr, SOC_CORE_ALL, 0, ENABLE_DATA_PATHf,  enable_val));
    ...
    */
    ...
    if (enable == TRUE) {
        ...
        */
         ...
         * Reconfigure ALDWP
         */
        SOC_SAND_CHECK_FUNC_RESULT(arad_fabric_aldwp_config(unit), 120, exit);
  ...
  )
{
  ...
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 10, exit, soc_reg32_get(unit, idr_ocb_multicast_range[range_ndx], REG_PORT_ANY, 0, &reg_val));
  ...
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 20, exit, soc_reg32_set(unit, idr_ocb_multicast_range[range_ndx], REG_PORT_ANY, 0, reg_val));
    ...
    SOC_SAND_IN  int       unit)
{
...
#ifdef ARAD_DRAM_MMU_ARAD_PLUS_FIX
    ...
    /* QPTS - Statistics shaper will not work when setting the shaper period to 1    */
        SOC_SAND_SOC_IF_ERROR_RETURN(res, 45, exit, READ_IQM_STATISTICS_REPORT_CONFIGURATIONS_2r(unit, &reg_val));
    field = 0x1;
    soc_reg_field_set(unit, IQM_STATISTICS_REPORT_CONFIGURATIONS_2r, &reg_val, SOC_IS_ARDON(unit) ? ST_RATE_LIMIT_PRD_FIX_ENf : FIELD_16_16f, field);
        SOC_SAND_SOC_IF_ERROR_RETURN(res, 50, exit, WRITE_IQM_STATISTICS_REPORT_CONFIGURATIONS_2r(unit, reg_val));
    ...
    for (index=0 ; index < ARAD_CNT_NOF_PROCESSOR_IDS ; index++) {
    ...
    if (!SOC_IS_ARDON(unit)) {
        ...
        /* Enable ERSPAN Inbound mirror fix */
        field = 0x0;
...
#define ARAD_PVT_NOF_ITER 3

int arad_mgmt_temp_pvt_get(int unit, int* temp) 
{
    uint32 value, accum, iterations;
    int i;
    SOCDNX_INIT_FUNC_DEFS;

    accum = 0;

    for(i=0 ; i<ARAD_PVT_NOF_ITER ; i++) {
        SOCDNX_IF_ERR_EXIT(READ_ECI_PVT_MON_A_THERMAL_DATAr(unit, &value));
        accum += soc_reg_field_get(unit, ECI_PVT_MON_A_THERMAL_DATAr, value, THERMAL_DATAf);
        SOCDNX_IF_ERR_EXIT(READ_ECI_PVT_MON_B_THERMAL_DATAr(unit, &value));
        accum += soc_reg_field_get(unit, ECI_PVT_MON_B_THERMAL_DATAr, value, THERMAL_DATAf);
    }

    iterations = ARAD_PVT_NOF_ITER*2;
    *temp = accum/(iterations);
    ...
    *avs_val = 0;
    ...
    if (SOC_IS_ARADPLUS(unit)) {
        ...
        *avs_val = soc_reg_field_get(unit, ECI_AVS_STATUSr, reg_val, AVS_STATUS_VALUEf);
        ...
        SOCDNX_IF_ERR_EXIT(soc_reg32_get(unit, ECI_REG_0243r, REG_PORT_ANY, 0, &reg_val));
