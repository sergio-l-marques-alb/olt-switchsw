...
arad_ipq_destination_id_packets_base_queue_id_verify(
   ...
   if ((core < 0 || core > SOC_DPP_DEFS_GET(unit, nof_cores)) && core != SOC_CORE_ALL) {
       SOC_SAND_SET_ERROR_CODE(ARAD_IPQ_DESTINATION_ID_PACKETS_CORE_NDX_OUT_OF_RANGE_ERR, 40, exit);
   }
   ...
   if (!SOC_DPP_CORE_MODE_IS_SYMMETRIC(unit) && 
       ...
       for (core_i = 0;core_i < SOC_DPP_DEFS_GET(unit, nof_cores); core_i++){
           res = arad_sw_db_sysport2queue_get(unit, dest_ndx, core_i, &valid_i[core_i], &sw_only_i[core_i], &base_queue_old[core_i]);
...
arad_ipq_destination_id_packets_base_queue_id_set_unsafe(
   ...
   SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_IPQ_DESTINATION_ID_PACKETS_BASE_QUEUE_ID_SET_UNSAFE);
    ...
    if (!SOC_DPP_CORE_MODE_IS_SYMMETRIC(unit)) {
       res = arad_sw_db_sysport2queue_set(unit, dest_ndx, core, valid, sw_only, base_queue);
       SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 50, exit);
    }
...
arad_ipq_destination_id_packets_base_queue_id_get_unsafe(
   ...
   if ((core < 0 || core > SOC_DPP_DEFS_GET(unit, nof_cores)) && core != SOC_CORE_ALL) {
       SOC_SAND_SET_ERROR_CODE(ARAD_IPQ_DESTINATION_ID_PACKETS_CORE_NDX_OUT_OF_RANGE_ERR, 20, exit);
   }

   if (!SOC_DPP_CORE_MODE_IS_SYMMETRIC(unit)) {
       res = arad_sw_db_sysport2queue_get(unit, dest_ndx, core, valid, sw_only, base_queue);
