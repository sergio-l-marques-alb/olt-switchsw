    ...
    )
{
  ...
  if(multicast_class_valid) {
      res = soc_port_sw_db_tm_port_to_base_q_pair_get(unit, mcast_class_port_id, 0, &base_port_tc);  
  ...
  if (multicast_class_valid)
  {
    ...
    if (if_id != ARAD_IF_ID_NONE)
    {
      ...
      res = arad_ofp_rates_sch_single_port_rate_sw_set(unit, mcast_class_port_id, 0, 0);
      if (res != SOC_E_NONE) {
          SOC_SAND_SET_ERROR_CODE(ARAD_MULT_FABRIC_ILLEGAL_MULTICAST_CLASS_ERR,55,exit);
      }
      res = arad_ofp_rates_sch_single_port_rate_hw_set(unit, mcast_class_port_id, 0); 
      if (res != SOC_E_NONE) {
