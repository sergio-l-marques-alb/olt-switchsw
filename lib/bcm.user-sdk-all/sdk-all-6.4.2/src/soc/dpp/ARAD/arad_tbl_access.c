...
/* 
 * Mark all non-dynamic tables as cachable
 */
void arad_tbl_mark_cachable(int unit) {
    soc_mem_t mem;
    for (mem = 0; mem < NUM_SOC_MEM; mem++) {
        if (!SOC_MEM_IS_VALID(unit, mem) || arad_tbl_is_dynamic(mem)) {
            continue;
        };
  ...
  )
{
  ...
  if ((SOC_IS_JERICHO(unit))) {
      tbl_data->valid[0] = soc_mem_field32_get(unit, IRR_DESTINATION_TABLEm, data, VALID_0f);
      tbl_data->valid[1] = soc_mem_field32_get(unit, IRR_DESTINATION_TABLEm, data, VALID_1f);
  } else {
  ...
  )
{
  ...
  if ((SOC_IS_JERICHO(unit))) {
      soc_mem_field32_set(unit, IRR_DESTINATION_TABLEm, data, VALID_0f, (uint32)tbl_data->valid[0]);
      soc_mem_field32_set(unit, IRR_DESTINATION_TABLEm, data, VALID_1f, (uint32)tbl_data->valid[1]);
  }
  ...
  )
{
  ...
  if (SOC_IS_JERICHO(unit)) {
      ...
      SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 30, exit);
      soc_mem_field_get(unit, PPDB_A_FEC_SUPER_ENTRY_BANKm, super_entry_data, 
  ...
  )
{
  ...
  if(tbl_data->format == ARAD_PP_IHB_FEC_FORMAT_A) {
    ...
    if (SOC_IS_JERICHO(unit)) {
        soc_mem_field32_set(unit, PPDB_A_FEC_ENTRY_FORMAT_Am, data, OUT_LIF_IS_NATIVE_VSIf, 1); 
    }
  ...
  )
{
 ...
 /* tbl_data->enable_same_interface_filters_lifs = soc_mem_field32_get(unit, EGQ_PP_PPCTm, data, ENABLE_SAME_INTERFACE_FILTERS_LIFSf); */
  tbl_data->enable_same_interface_filters_ports = soc_mem_field32_get(unit, EGQ_PP_PPCTm, data, ENABLE_MULTICAST_SAME_INTERFACE_FILTERSf);
...
/*  soc_mem_field32_set(unit, EGQ_PP_PPCTm, data, ENABLE_SAME_INTERFACE_FILTERS_LIFSf, tbl_data->enable_same_interface_filters_lifs); */
  soc_mem_field32_set(unit, EGQ_PP_PPCTm, data, ENABLE_MULTICAST_SAME_INTERFACE_FILTERSf, tbl_data->enable_same_interface_filters_ports);
  ...
  )
{
  ...
  soc_mem_field64_get(unit, EGQ_TTL_SCOPEm, &data, TTL_SCOPEf, &field_64);
  ...
  )
{
  ...
  soc_mem_field64_set(unit, EGQ_TTL_SCOPEm, &data, TTL_SCOPEf, field_64);
  ...
  )
{
  ...
  tbl_data->eei_or_out_lif = soc_mem_field32_get(unit, PPDB_B_LIF_TABLE_AC_2_EEIm, data, OUT_LIF_OR_LABELf);
  ...
  )
{
  ...
  soc_mem_field32_set(unit, PPDB_B_LIF_TABLE_AC_2_EEIm, data, OUT_LIF_OR_LABELf, tbl_data->eei_or_out_lif);
