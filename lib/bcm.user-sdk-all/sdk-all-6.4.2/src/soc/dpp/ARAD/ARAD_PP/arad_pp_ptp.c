...
#define ARAD_PP_1588_PROGRAM_FIELD_PACKET_IS_IEEE_1588_MASK           0x1 /* 1 bit lenght */
#define ARAD_PP_1588_PROGRAM_FIELD_ENCAPSULATION_SHIFT                  1
#define ARAD_PP_1588_PROGRAM_FIELD_ENCAPSULATION_MASK                 0x1 /* 1 bit lenght */
#define ARAD_PP_1588_PROGRAM_FIELD_CARRY_1588_HEADER_INDEX_SHIFT        2
#define ARAD_PP_1588_PROGRAM_FIELD_CARRY_1588_HEADER_INDEX_MASK       0x7 /* 3 bit lenght */
...
#define ARAD_PP_1588_ACTION_FIELD_UPDATE_TIMESTAMP_MASK              0x1 /* 1 bit lenght */
#define ARAD_PP_1588_ACTION_FIELD_COMMAND_SHIFT                        1
#define ARAD_PP_1588_ACTION_FIELD_COMMAND_MASK                       0x3 /* 2 bit lenght */
#define ARAD_PP_1588_ACTION_FIELD_ACTION_INDEX_SHIFT                   3
#define ARAD_PP_1588_ACTION_FIELD_ACTION_INDEX_MASK                  0x7 /* 3 bit lenght */
...
/* we dont have enough entries, so uniting IPV4 & IPV6 by kind of compromize:
...
/* 1588 supported encapsulations */
   int ieee_1588_identification_cam[ARAD_PP_1588_IDENTIFICATION_CAM_TABLE_ENTRIES_NUM][9] =
    ...
    /* when bit is 1 the timestamp is taken when packet is recieved */
    ...
    for(i = 0; i < ARAD_PP_1588_IDENTIFICATION_CAM_TABLE_ENTRIES_NUM; i++) {
        ...
        /* mask=IP_MSK(0011b), accepts QQXXb values (Q - bit vlaue of qualifier, X - any bit, 0 or 1) */
        /* mask=0, accepts only qualifier value (QQQQb)  (Q - bit vlaue of qualifier) */
        ...
        /* bits     4   0 */
        /*          CCCEI */
        /*                                      1588 encupsulation 0-eth, 1-udp     1588 carry header index */
  ...
  )
{
    ...
    for(i = 0; i < index_max + 1; i++) {
        ...
        /* when one step TC is enabled settin command 3, RX record & TX stamping */
        if(0 != (info->flags & ARAD_PP_PTP_PORT_TIMESYNC_ONE_STEP_TIMESTAMP)) {
            /* 1588 protocol TC implementation, need to updated CF in follwing 4 messages */
            ...
            if(ARAD_PP_PTP_1588_PKT_SYNC        == msg_type ||
               ...
               ARAD_PP_PTP_1588_PKT_PDELAY_REQ  == msg_type ||
               ARAD_PP_PTP_1588_PKT_PDELAY_RESP == msg_type) {
                /* when two step TC is enabled settin command 2, TX time recording */
                egress_command = ARAD_PP_1588_EGRESS_COMMAND_2_TX_RECORDING;
            }
        }

        /*
         * pkt_drop, pkt_tocpu bitmaps of 1588 event and general packet types indicating wether to :
        ...
        command = 0;
        /* index to the trap/snoop strenght in IHB_ACTION_PROFILE_IEEE_1588 register */
        ...
        /* setting table field/column acording to the port profile (0-3) */
  ...
  )
{
    ...
    if(0 != info->ptp_enabled) {
        ...
        /* Extenal MAC is relevant only in case PTP is enabled */       
        ...
        if(TRUE == ext_mac_enable) {
            ...
            /*   incomming packets - setting timestamp to 0, 
