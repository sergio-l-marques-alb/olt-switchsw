...
)
{
  ...
  *is_open = dpp_mult_does_group_exist(unit, group_mcid, FALSE) ;
  if (*is_open) {
    uint16 group_size;
    SOCDNX_IF_ERR_EXIT(dpp_mcds_get_group(
      unit, group_mcid, DPP_MCDS_TYPE_VALUE_INGRESS, mc_group_size, &group_size));
    *exact_mc_group_size = group_size;
    SOCDNX_IF_ERR_EXIT(dpp_mcds_copy_replications_to_arrays(unit, 0, mc_group_size,  ports, cuds));
...
)
{
  ...
  if (dpp_mult_does_group_exist(unit, multicast_id_ndx, FALSE)) { /* do nothing if not open */
    dpp_mcds_base_t *mcds = dpp_get_mcds(unit);
...
)
{
  ...
  if (!dpp_mult_does_group_exist(unit, multicast_id_ndx, FALSE)) {
    SOCDNX_EXIT_WITH_ERR(SOC_E_NOT_FOUND, (_BSL_SOCDNX_MSG("MC group is not created")));
  }
