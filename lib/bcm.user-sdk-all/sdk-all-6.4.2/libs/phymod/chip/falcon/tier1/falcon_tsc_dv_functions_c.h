/**************************************************************************************/
/**************************************************************************************/
/*  File Name     :  falcon_api_dv_functions.c                                        */
/*  Created On    :  22/05/2014                                                       */
/*  Created By    :  Kiran Divakar                                                    */
/*  Description   :  APIs for Serdes IPs ported over from DV                          */
/*  Revision      :  $Id: falcon_api_dv_functions.c 641 2014-07-25 21:55:03Z kirand $ */
/*                                                                                    */
/*  All Rights Reserved                                                               */
...
/** @file falcon_api_dv_functions.c
...
err_code_t falcon_config_shared_tx_pattern(const phymod_access_t *pa, uint8_t patt_length, const char pattern[]) {
...
err_code_t falcon_tx_shared_patt_gen_en(const phymod_access_t *pa, uint8_t enable, uint8_t patt_length) {
  uint8_t mode_sel,zero_pad_len;

  EFUN(_calc_patt_gen_mode_sel(pa, &mode_sel,&zero_pad_len,patt_length));

  if (enable) {
    #if defined(MERLIN) && !defined(MERLIN_SESTO) && !defined(MERLIN_COUNTACH)     
      /* Use this for all MERLIN other than SESTO and COUNTACH */
      wr_patt_gen_mode_sel(mode_sel);                 /* Pattern Generator Mode Select */
    #else
...
#ifdef FALCON
err_code_t falcon_display_cl93n72_status(const phymod_access_t *pa) {
...
err_code_t falcon_display_cl72_status(void) {
...
err_code_t falcon_tsc_write_rx_afe(const phymod_access_t *pa, enum _falcon_comn_rx_afe_settings_enum param, int8_t val) {
...
err_code_t falcon_tsc_read_rx_afe(const phymod_access_t *pa, enum _falcon_comn_rx_afe_settings_enum param, int8_t *val) {
  /* Assumes the micro is not actively tuning */
  if(!val) {
  	return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
  }
...
#ifndef MERLIN
err_code_t falcon_validate_txfir_cfg(const phymod_access_t *pa, int8_t pre, int8_t main, int8_t post1, int8_t post2, int8_t post3) {
...
err_code_t falcon_tsc_write_tx_afe(const phymod_access_t *pa, enum _falcon_comn_tx_afe_settings_enum param, int8_t val) {
...
err_code_t falcon_tsc_read_tx_afe(const phymod_access_t *pa, enum _falcon_comn_tx_afe_settings_enum param, int8_t *val) {
  if(!val) {
  	return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
  }
...
err_code_t falcon_apply_txfir_cfg(const phymod_access_t *pa, int8_t pre, int8_t main, int8_t post1, int8_t post2, int8_t post3) {
  
  err_code_t failcode = falcon_validate_txfir_cfg(pa, pre, main, post1, post2, post3);
...
err_code_t falcon_get_tx_prbs_config(const phymod_access_t *pa, enum falcon_tsc_prbs_polynomial_enum *prbs_poly_mode, uint8_t *prbs_inv) {
    uint8_t val;
...
err_code_t falcon_get_tx_prbs_en(const phymod_access_t *pa, uint8_t *enable) {
...
err_code_t falcon_tx_prbs_err_inject(const phymod_access_t *pa, uint8_t enable) {
  /* PRBS Error Insert.
...
err_code_t falcon_tsc_config_rx_prbs(const phymod_access_t *pa, enum falcon_tsc_prbs_polynomial_enum prbs_poly_mode, enum _falcon_comn_prbs_checker_mode_enum prbs_checker_mode, uint8_t prbs_inv) {
...
/* get PRBS Checker */
err_code_t falcon_get_rx_prbs_config(const phymod_access_t *pa, enum falcon_tsc_prbs_polynomial_enum *prbs_poly_mode, enum _falcon_comn_prbs_checker_mode_enum *prbs_checker_mode, uint8_t *prbs_inv) {
  uint8_t val;
  ...
  *prbs_checker_mode = (enum _falcon_comn_prbs_checker_mode_enum)val;
...
err_code_t falcon_get_rx_prbs_en(const phymod_access_t *pa, uint8_t *enable) {
...
err_code_t falcon_prbs_err_count_ll(const phymod_access_t *pa, uint32_t *prbs_err_cnt) {
  uint16_t rddata;
  if(!prbs_err_cnt) {
	  return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
  }

  EFUN(falcon_pmd_rdt_reg(pa, TLB_RX_PRBS_CHK_ERR_CNT_MSB_STATUS, &rddata));
...
err_code_t falcon_tsc_prbs_err_count_state(const phymod_access_t *pa, uint32_t *prbs_err_cnt, uint8_t *lock_lost) {
  ...
  EFUN(falcon_prbs_err_count_ll(pa, prbs_err_cnt));
...
err_code_t falcon_dig_lpbk_rptr(uint8_t enable, enum _falcon_comn_rptr_mode_enum mode) {
  ...
  switch(mode) {
    case DATA_IN_SIDE:
        ...
        if (enable) {
          err_code = falcon_loop_timing(pa, 0);
          if (err_code) {
            return(err_code);
          }
          err_code = falcon_loop_timing(pa, 1);     /* Program DATA_IN_SIDE side TX_PI for loop-timing mode to lock SYS TX to SYS TX clock freq. */
          if (err_code) {
            return(err_code);
          }
        }
    ...
    case DIG_LPBK_SIDE:
        /* DIG_LPBK_SIDE - The side where data is looped back using digital loopback */
          ...
          if ((osr_mode == SERDES_ENUM_OSX16P5) || (osr_mode == SERDES_ENUM_OSX20P625)) {
            wr_rx_pi_manual_mode(0x0);
          }
        #endif
        err_code = _rptr_mode_timing();          /* Program DIG_LPBK_SIDE side TX_PI for repeater mode to lock LINE TX to SYS RX clock freq. */
        if (err_code) {
          return (err_code);
        }
        break;
...
err_code_t falcon_loop_timing(const phymod_access_t *pa, uint8_t enable) {
  ...
  if (enable) {
    ...
    err_code = falcon_delay_us(25);                     /* Wait for tclk to lock to CDR */
    if (err_code) {
      return(err_code);
    }
  }
...
err_code_t falcon_tsc_rmt_lpbk(const phymod_access_t *pa, uint8_t enable) {
  ...
  if (enable) {
    err_code = falcon_loop_timing(pa, enable);
    if (err_code) {
      return(err_code);
    }
    ...
    err_code = falcon_delay_us(50);                     /* Wait for rclk and tclk phase lock before expecing good data from rmt loopback */
  ...
  err_code_t falcon_rmt_lpbk_from_ull(void) {

    err_code_t err_code;

    err_code = falcon_tsc_rmt_lpbk(pa, 0);                      /* Reset TX_PI */
    if (err_code) {
      return (err_code);
    }
    err_code = falcon_tsc_rmt_lpbk(pa, 1);                      /* Enable RMT LPBK */
    return (err_code);
  }
#endif
  ...
  err_code_t falcon_rmt_lpbk_from_nl(void) {
  ...
  err_code_t falcon_ull_from_rmt_lpbk(void) {
    ...
    err_code = _rptr_mode_timing();                     /* Enable jitter filter enable */
    if (err_code) {
      return (err_code);
    }
    err_code = _ull_mode_setup();                       /* Setup ULL Mode */
    return (err_code);
  }
#endif
  ...
  err_code_t falcon_nl_from_rmt_lpbk(void) { 
    ...
    #ifndef MERLIN_COUNTACH
     #ifdef FALCON
      wr_ams_tx_sel_ll(0x0);                            /* Switch the tx data mux to PCS data */
     #else
...
err_code_t falcon_tx_pi_jitt_gen(const phymod_access_t *pa, uint8_t enable, int16_t freq_override_val, enum _falcon_comn_tx_pi_freq_jit_gen_enum jit_type, uint8_t tx_pi_jit_freq_idx, uint8_t tx_pi_jit_amp) {
...
err_code_t falcon_core_config_for_iddq(const phymod_access_t *pa) {

#if defined (MERLIN) && !defined(MERLIN_COUNTACH) 
...
err_code_t falcon_lane_config_for_iddq(const phymod_access_t *pa) {
...
err_code_t falcon_uc_active_enable(const phymod_access_t *pa, uint8_t enable) {
...
err_code_t falcon_tsc_uc_reset(const phymod_access_t *pa, uint8_t enable) {
  ...
  if (enable) { 
    ...
    EFUN(falcon_pmd_wr_reg(pa, 0xD200, 0x0000));
    EFUN(falcon_pmd_wr_reg(pa, 0xD201, 0x0000));
    EFUN(falcon_pmd_wr_reg(pa, 0xD202, 0x0000));
    EFUN(falcon_pmd_wr_reg(pa, 0xD203, 0x0000));
    EFUN(falcon_pmd_wr_reg(pa, 0xD207, 0x0000));
    EFUN(falcon_pmd_wr_reg(pa, 0xD208, 0x0000));
    EFUN(falcon_pmd_wr_reg(pa, 0xD20A, 0x080f));
    EFUN(falcon_pmd_wr_reg(pa, 0xD20C, 0x0000));
    EFUN(falcon_pmd_wr_reg(pa, 0xD20D, 0x0000));
  }
  else {
  ...
  if (enable) { 
    ...
    EFUN(falcon_pmd_wr_reg(pa, 0xD201, 0x0000));
    EFUN(falcon_pmd_wr_reg(pa, 0xD202, 0x0000));
    EFUN(falcon_pmd_wr_reg(pa, 0xD204, 0x0000));
    EFUN(falcon_pmd_wr_reg(pa, 0xD205, 0x0000));
    EFUN(falcon_pmd_wr_reg(pa, 0xD206, 0x0000));
    EFUN(falcon_pmd_wr_reg(pa, 0xD207, 0x0000));
    EFUN(falcon_pmd_wr_reg(pa, 0xD208, 0x0000));
    EFUN(falcon_pmd_wr_reg(pa, 0xD209, 0x0000));
    EFUN(falcon_pmd_wr_reg(pa, 0xD20A, 0x0000));
    EFUN(falcon_pmd_wr_reg(pa, 0xD20B, 0x0000));
    EFUN(falcon_pmd_wr_reg(pa, 0xD20C, 0x0000));
    EFUN(falcon_pmd_wr_reg(pa, 0xD20D, 0x0000));
    EFUN(falcon_pmd_wr_reg(pa, 0xD20E, 0x0000));
    EFUN(falcon_pmd_wr_reg(pa, 0xD211, 0x0000));
    EFUN(falcon_pmd_wr_reg(pa, 0xD212, 0x0000));
    EFUN(falcon_pmd_wr_reg(pa, 0xD213, 0x0000));
    EFUN(falcon_pmd_wr_reg(pa, 0xD214, 0x0000));
    EFUN(falcon_pmd_wr_reg(pa, 0xD215, 0x0000));
    EFUN(falcon_pmd_wr_reg(pa, 0xD216, 0x0007));
    EFUN(falcon_pmd_wr_reg(pa, 0xD217, 0x0000));
    EFUN(falcon_pmd_wr_reg(pa, 0xD218, 0x0000));
    EFUN(falcon_pmd_wr_reg(pa, 0xD219, 0x0000));
    EFUN(falcon_pmd_wr_reg(pa, 0xD21A, 0x0000));
    EFUN(falcon_pmd_wr_reg(pa, 0xD21B, 0x0000));
    EFUN(falcon_pmd_wr_reg(pa, 0xD220, 0x0000));
    EFUN(falcon_pmd_wr_reg(pa, 0xD221, 0x0000));
    EFUN(falcon_pmd_wr_reg(pa, 0xD224, 0x0000));
    EFUN(falcon_pmd_wr_reg(pa, 0xD225, 0x0401));
  }
  else {
    /* De-assert micro reset - Start executing code */
...
err_code_t falcon_core_pwrdn(const phymod_access_t *pa, enum _falcon_comn_core_pwrdn_mode_enum mode) {
  err_code_t err_code;
  switch(mode) {
    case PWR_ON:
            err_code = _falcon_core_clkgate(pa, 0);
    ...
    case PWRDN_DEEP:
            ...
            #endif
            wrc_core_dp_s_rstb(0x0);
            err_code = _falcon_core_clkgate(pa, 1);
            if (err_code) return (err_code);
            break;
    default : return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
  }
...
err_code_t falcon_tsc_lane_pwrdn(const phymod_access_t *pa, enum _falcon_comn_core_pwrdn_mode_enum mode) {
  err_code_t err_code;
  switch(mode) {
    case PWR_ON:
            wr_ln_tx_s_pwrdn(0x0);
            wr_ln_rx_s_pwrdn(0x0);
            err_code = _falcon_lane_clkgate(pa, 0);
            if (err_code) return (err_code);
            break;
    case PWRDN:
    ...
    case PWRDN_DEEP:
            wr_ln_tx_s_pwrdn(0x1);
            wr_ln_rx_s_pwrdn(0x1);
            err_code = _falcon_lane_clkgate(pa, 1);
            if (err_code) return (err_code);
            break;
    case PWRDN_TX:
            wr_ln_tx_s_pwrdn(0x1);
...
err_code_t falcon_isolate_ctrl_pins(const phymod_access_t *pa, uint8_t enable) {
...
/***********************************************/
  ...
  err_code_t falcon_tsc_ucode_mdio_load(const phymod_access_t *pa, uint8_t *ucode_image, uint16_t ucode_len) {
    ...
    err_code = falcon_poll_micro_ra_initdone(pa, 250);    /* Poll for micro_ra_initdone = 1 to indicate initialization done */
    if (err_code) {
      return (err_code);
    }
...
/*************************************************/
  ...
  err_code_t falcon_wrw_uc_ram(const phymod_access_t *pa, uint16_t addr, uint16_t wr_val) {        
  ...
  err_code_t falcon_wrb_uc_ram(const phymod_access_t *pa, uint16_t addr, uint8_t wr_val) {
    wrc_micro_autoinc_wraddr_en(0);
  ...
  /* Micro RAM Word Read */
  uint16_t falcon_rdw_uc_ram(const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr) {      
   uint16_t rddata;
   if(!err_code_p) {
     return(0);
   }
  ...
  /* Micro RAM Byte Read */
  uint8_t falcon_rdb_uc_ram(const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr) {      
    uint8_t rddata;
    if(!err_code_p) {
      return(0);
     }
  ...
  err_code_t falcon_wrw_uc_ram(const phymod_access_t *pa, uint16_t addr, uint16_t wr_val) {        
    ...
    err_code = falcon_delay_ns(pa, 80);                     /* wait for 10 comclk cycles/ 80ns  */
    if (err_code) {                              
      return (err_code);
    } 
    wrc_micro_ram_wrdata(wr_val);                       /* RAM Write value */
    err_code = falcon_delay_ns(pa, 80);                     /* Wait for Data RAM to be written */
    return (err_code);
  }
  ...
  uint16_t falcon_rdw_uc_ram(const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr) {      
    ...
    *err_code_p = falcon_delay_ns(pa, 80);                    /* Wait for the RAM read  */
  ...
  err_code_t falcon_wrws_uc_ram(const phymod_access_t *pa, uint16_t addr, int16_t wr_val) { 
    return (falcon_wrw_uc_ram(pa, addr, wr_val));
  }

  /* Micro RAM Byte Signed Write */
  err_code_t falcon_wrbs_uc_ram(const phymod_access_t *pa, uint16_t addr, int8_t wr_val) {
    return (falcon_wrb_uc_ram(pa, addr, wr_val));  
  }

  /* Micro RAM Word Signed Read */
  int16_t falcon_rdws_uc_ram(const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr) {      
    return ((int16_t)falcon_rdw_uc_ram(pa, err_code_p, addr));
  }

  /* Micro RAM Byte Signed Read */
  int8_t falcon_rdbs_uc_ram(const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr) {      
    return ((int8_t)falcon_rdb_uc_ram(pa, err_code_p, addr));
  }

    /* Micro RAM Byte Read */
  err_code_t falcon_rdblk_uc_ram(const phymod_access_t *pa, uint8_t *mem, uint16_t addr, uint16_t cnt) {      
    ...
    i=0;
    do {
        mem[i] = falcon_rdb_uc_ram(pa, &err_code,addr+i);         /* Value read from the required Data RAM Address */
        if(err_code) {
            return(_error(err_code));
        }
        i++;
