 ...
 */
uint32 soc_dcmn_cache_table_update_all(int unit, dcmn_mbist_script_t *script);
...
#define DCMN_MBIST_TEST_SCRIPT(func, mbist_device) \
    ...
    LOG_DEBUG(BSL_LS_SOC_MBIST, \
              (BSL_META_U(unit, \
                          "us:%lu Testing MBIST script " #func "\n"), \
               (unsigned long)(sal_time_usecs() - dynamic.measured_time))); 
#else
#define DCMN_MBIST_TEST_SCRIPT(func, mbist_device) \
    SOCDNX_IF_ERR_EXIT(soc_dcmn_run_mbist_script(unit, &  func ## _script , &mbist_device, &dynamic));
#endif
