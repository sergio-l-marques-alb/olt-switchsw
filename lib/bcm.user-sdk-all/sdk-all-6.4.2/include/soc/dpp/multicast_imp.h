...
typedef struct {
    ...
    uint32 max_egr_cud_field; /* The maximum value in an ingress CUD field */
    ...
    dpp_rep_data_t reps[DPP_MULT_MAX_REPLICATIONS]; /* used to store replication data of a group */
    ...
    uint16 nof_reps;                 /* total number of replications */
    uint16 nof_ingr_reps;            /* number of ingress replications or egress port replications in non standard egress groups */
    uint16 nof_egr_port_outlif_reps; /* number of egress replications of type port + outlif */
    uint16 nof_egr_outlif_reps;      /* number of egress replications of type outlif */
...
#if 0
/* structure members ordered for sort */
typedef struct {
  dpp_mc_outlif_t       outlif;
  dpp_mc_ingress_dest_t dest;
} dpp_mc_ingr_replication_t; /* the data of an ingress replication */

typedef struct {
  dpp_mc_outlif_t        outlif;
  dpp_mc_local_port_t    port;
} dpp_egr_mc_replication_port_outlif_t; /* the data of egress replications in formats 0,4,5 */

typedef struct {
  dpp_mc_outlif_t        outlif;
} dpp_egr_mc_replication_outlif_t; /* the data of egress replications in formats 2,6,7 */

typedef struct {
  dpp_mc_outlif_t        outlif;
  dpp_mc_bitmap_id_t     bitmap;
} dpp_egr_mc_replication_bitmap_t; /* the data of egress replications in format 1 */

typedef struct {
  dpp_mc_local_port_t          port;
} dpp_egr_mc_replication_port_t; /* the data of non standard egress formats */

typedef union
{
  dpp_mc_ingr_replication_t            ingress;
  dpp_egr_mc_replication_port_outlif_t egress_port_outlif;
  dpp_egr_mc_replication_outlif_t      egress_outlif;
  dpp_egr_mc_replication_bitmap_t      egress_bitmap;
  dpp_egr_mc_replication_port_t        egress_port;
} dpp_mc_replication_information_t; /* can store any replication information */
...
uint16 dpp_mcds_base_get_nof_ingress_replications(dpp_mcds_base_t *mcds);
