/* $Id: chipsim_bde.c,v 1.20 Broadcom SDK $
...
extern uint32 Soc_pb_reg_buffer[CHIP_SIM_NOF_CHIPS][SOC_PB_TOTAL_SIZE_OF_REGS];
#endif
...
#if LINK_PB_LIBRARIES
    addr += PTR_TO_INT((*(Soc_pb_reg_buffer + d)));  /* add base address */
    status = chip_sim_read(addr, &data);
#endif
    if (status) {
...
STATIC uint32*
_chipsim_bde_p2l(int d, uint32 paddr)
{
...
chipsim_eb_bus_probe(chipsim_bde_bus_t *bus)
{
    ...
    if(chipsim_ndev != 0){
        ...
        while (_n_devices < chipsim_ndev) {
            ...
            if (chip_type_str == NULL || sal_strcmp(chip_type_str, "petrab") == 0) {
                cbd->dev_type = BDE_EB_DEV_TYPE;
                cbd->bde_dev.device = PETRAB_DEVICE_ID;
                cbd->bde_dev.rev    = PETRAB_A0_REV_ID;
            } else if (sal_strcmp(chip_type_str, "arad") == 0) {
                cbd->dev_type = BDE_EB_DEV_TYPE;
