...
#define _SOC_TH_ASF_UNIT_VALIDATE(unit) \
...
typedef struct {
    ...
    _soc_th_ep_credit_acc_t          ep_credit_acc;
...
_soc_th_asf_cfg_tbl[] = {
    ...
    {11000,  1,  10, 6, {0, 0},  0, 10, 0,  {0,  0},  {0,   0},  {18, 51, 42, 30}, {18, 60, 48, 27} },    /*  HG[11]   */
    {20000,  3,  10, 0, {2, 8},  7, 0,  29, {13, 7},  {18,  16}, {18, 63, 48, 30}, {18, 54, 42, 21} },    /*  20GE     */
    {21000,  3,  10, 7, {0, 0},  0, 11, 0,  {0,  0},  {0,   0},  {18, 63, 48, 30}, {18, 54, 42, 21} },    /*  HG[21]   */
    {25000,  5,  10, 0, {3, 8},  6, 0,  25, {12, 7},  {16,  15}, {18, 60, 48, 27}, {18, 51, 36, 18} },    /*  25GE     */
    {27000,  5,  10, 7, {0, 0},  0, 11, 0,  {0,  0},  {0,   0},  {18, 60, 48, 27}, {18, 51, 36, 18} },    /*  HG[27]   */
    {40000,  7,  12, 0, {3, 12}, 8, 0,  29, {18, 7},  {25,  19}, {18, 57, 45, 24}, {18, 42, 30, 12} },    /*  40GE     */
    {42000,  7,  12, 7, {0, 0},  0, 11, 0,  {0,  0},  {0,   0},  {18, 57, 45, 24}, {18, 42, 30, 12} },    /*  HG[42]   */
    {50000,  7,  12, 0, {1, 12}, 8, 0,  25, {20, 17}, {27,  23}, {18, 54, 42, 21}, {18, 33, 27, 12} },    /*  50GE     */
    {53000,  7,  12, 7, {0, 0},  0, 11, 0,  {0,  0},  {0,   0},  {18, 54, 42, 21}, {18, 33, 27, 12} },    /*  HG[53]   */
    {100000, 11, 12, 0, {3, 3},  8, 0,  3,  {33, 27}, {44,  36}, {18, 48, 36, 18}, {18, 33, 27, 15} },    /*  100GE    */
    {106000, 11, 12, 6, {0, 0},  0, 10, 0,  {0,  0},  {0,   0},  {18, 48, 36, 18}, {18, 33, 27, 15} }     /*  HG[106]  */
};
    ...
    int          speed)
{
    ...
    ct_class = _soc_th_speed_to_ct_class_map(speed);
    _SOC_TH_ASF_SPEED_CLASS_VALIDATE(ct_class, 1);
    ...
    IN_OUT uint8* const      xmit_cnt)
{
    ...
    soc_port_t                   th_port;
    _soc_th_latency_mode_e       latency = _SOC_TH_FULL_LATENCY;

    _SOC_TH_ASF_UNIT_VALIDATE(unit);
    if (!xmit_cnt ||
    ...
    _SOC_TH_ASF_SPEED_CLASS_VALIDATE(dc, 1);
    ...
    if (_SOC_TH_ASF_RETRV == *xmit_cnt) {
        *xmit_cnt = 0;
        ...
        SOC_IF_ERROR_RETURN(
            READ_EGR_XMIT_START_COUNTm(unit, MEM_BLOCK_ALL, port_idx, &entry));
        *xmit_cnt = soc_mem_field32_get(unit, EGR_XMIT_START_COUNTm,
                        &entry, THRESHOLDf);
        ...
        *xmit_cnt = 0;
        ...
        PBMP_ITER(th_ports, th_port) {
            if (SOC_PBMP_MEMBER(si->oversub_pbm, th_port)) {
                SOC_IF_ERROR_RETURN(
                    soc_th_port_oversub_ratio_get(unit, th_port,
                        &ovs_ratio));
    ...
    IN_OUT uint8* const   max_sp)
{
    ...
    if ((_SOC_TH_ASF_RETRV == *min_sp) &&
        ...
        *min_sp = *max_sp = 0;
        ...
        /* MIN_SRC_PORT_SPEED */
        *min_sp = soc_reg_field_get(unit, ASF_EPORT_CFGr,
                      rval, MIN_SRC_PORT_SPEEDf);
        /* MAX_SRC_PORT_SPEED */
        *max_sp = soc_reg_field_get(unit, ASF_EPORT_CFGr,
                      rval, MAX_SRC_PORT_SPEEDf);
    ...
    IN_OUT uint8* const   mmu_prebuf)
{
    ...
    if (_SOC_TH_ASF_RETRV == *mmu_prebuf) {
        *mmu_prebuf = 0;
        ...
        SOC_IF_ERROR_RETURN(READ_ASF_EPORT_CFGr(unit, port, &rval));
        *mmu_prebuf = soc_reg_field_get(unit, ASF_EPORT_CFGr,
                          rval, MIN_SRC_PORT_XMIT_CNTf);
        ...
        *mmu_prebuf = 0;
        ...
        if (10 == speed_encoding) {   /* special case - HG[53] */
            if ((7 == min_sp) ||
                (8 == min_sp)) {  /* 40GE or HG[42] */
                *mmu_prebuf = 20;
    ...
    IN_OUT uint8* const   fifo_threshold)
{
    ...
    if (_SOC_TH_ASF_RETRV == *fifo_threshold) {
        *fifo_threshold = 0;
        ...
        SOC_IF_ERROR_RETURN(READ_ASF_EPORT_CFGr(unit, port, &rval));
        *fifo_threshold = soc_reg_field_get(unit, ASF_EPORT_CFGr,
                              rval, FIFO_THRESHOLDf);
        ...
        *fifo_threshold = 0;
        ...
        oversub =  (SOC_PBMP_MEMBER
                       (SOC_INFO(unit).oversub_pbm, port) ? 1 : 0);
        ...
        switch(mode) {
            ...
            case _SOC_TH_ASF_MODE_SLOW_TO_FAST:
                ...
                ret = _soc_th_port_asf_mmu_prebuf_get(unit, port,
                          port_speed, min_sp, &mmu_prebuf);
                if (SOC_E_NONE != ret) {
                    return ret;
                }
    ...
    IN_OUT uint8* const   fifo_depth)
{
    ...
    if (_SOC_TH_ASF_RETRV == *fifo_depth) {
        ...
        *fifo_depth = 0;
        SOC_IF_ERROR_RETURN(READ_ASF_EPORT_CFGr(unit, port, &rval));
        *fifo_depth = soc_reg_field_get(unit, ASF_EPORT_CFGr,
                          rval, FIFO_DEPTHf);
    ...
    uint8   ca_thresh)
{
    ...
    SOC_IF_ERROR_RETURN(
        soc_reg32_get(unit, idb_obm_ca_ct_ctrl_reg[0][0],
                      REG_PORT_ANY, 0, &rval));

    soc_reg_field_set(unit, idb_obm_ca_ct_ctrl_reg[0][0], &rval,
                      PORT0_CT_SELf, 0);
    soc_reg_field_set(unit, idb_obm_ca_ct_ctrl_reg[0][0], &rval,
                      THRESHOLD0f, ca_thresh);
    soc_reg_field_set(unit, idb_obm_ca_ct_ctrl_reg[0][0], &rval,
                      PORT1_CT_SELf, 1);
    soc_reg_field_set(unit, idb_obm_ca_ct_ctrl_reg[0][0], &rval,
                      THRESHOLD1f, ca_thresh);
    soc_reg_field_set(unit, idb_obm_ca_ct_ctrl_reg[0][0], &rval,
                      PORT2_CT_SELf, 2);
    soc_reg_field_set(unit, idb_obm_ca_ct_ctrl_reg[0][0], &rval,
                      THRESHOLD2f, ca_thresh);
    soc_reg_field_set(unit, idb_obm_ca_ct_ctrl_reg[0][0], &rval,
                      PORT3_CT_SELf, 3);
    soc_reg_field_set(unit, idb_obm_ca_ct_ctrl_reg[0][0], &rval,
                      THRESHOLD3f, ca_thresh);

    /* IDB_OBM[0-7]_CA_CT_CONTROL_PIPE[0-3]r */
    for (obm = 0; obm <= 7; obm++) {
    ...
    uint8   bubble_mop_disable)
{
    ...
    /* IDB_OBM[0-7]_CONTROL_PIPE[0-3]r */
    for (obm = 0; obm <= 7; obm++) {
        for (pipe = 0; pipe < NUM_PIPE(unit); pipe++) {
            SOC_IF_ERROR_RETURN(
            ...
            soc_reg_field_set(unit, idb_obm_ctrl_reg[obm][pipe], &rval,
                              PORT0_BUBBLE_MOP_DISABLEf, bubble_mop_disable);
            soc_reg_field_set(unit, idb_obm_ctrl_reg[obm][pipe], &rval,
                              PORT1_BUBBLE_MOP_DISABLEf, bubble_mop_disable);
            soc_reg_field_set(unit, idb_obm_ctrl_reg[obm][pipe], &rval,
                              PORT2_BUBBLE_MOP_DISABLEf, bubble_mop_disable);
            soc_reg_field_set(unit, idb_obm_ctrl_reg[obm][pipe], &rval,
                              PORT3_BUBBLE_MOP_DISABLEf, bubble_mop_disable);

            SOC_IF_ERROR_RETURN(
                soc_reg32_set(unit, idb_obm_ctrl_reg[obm][pipe],
                              REG_PORT_ANY, 0, rval));
        }
    }
    ...
    soc_th_asf_mode_e   mode)
{
    ...
    speed_encoding = _soc_th_speed_to_ct_class_map(speed);
    _SOC_TH_ASF_SPEED_CLASS_VALIDATE(speed_encoding, 1);

    /* init CT class */
    if (SOC_E_NONE !=
        (ret = _soc_th_port_asf_class_init(unit, port, speed)) ) {
        return SOC_E_INIT;
    }
    /* init Egress XMIT Start Count */
    if (SOC_E_NONE !=
        (ret = _SOC_TH_PORT_ASF_XMIT_START_COUNT_INIT(unit, port,
                  speed, mode, 0))) {
        return SOC_E_INIT;
    }
    ...
    uint8                extra_cells)
{
    ...
    if (_SOC_TH_ASF_MODE_CFG_UPDATE != mode) {
        ...
        _SOC_TH_ASF_SPEED_CLASS_VALIDATE(dst_class, 1);
    }
    ...
    for (src_class = 0; src_class < 13; src_class++) {
        ...
        soc_mem_field32_set(unit, EGR_XMIT_START_COUNTm, &entry,
                            THRESHOLDf, xmit_cnt);
        port_idx = ((port % 34) * 16) + src_class;
        SOC_IF_ERROR_RETURN(
           WRITE_EGR_XMIT_START_COUNTm(unit, MEM_BLOCK_ALL, port_idx, &entry));
    }
    ...
    soc_th_asf_mode_e   mode)
{
    ...
    uint8         bubble_mop_disable = 0, enable_org = 0;
    uint32        rval;
    soc_info_t   *si = NULL;
    mac_driver_t *macd;
    ...
    switch(mode) {
        case _SOC_TH_ASF_MODE_SAF:
            /* ASF_EPORT_CFG */
            SOC_IF_ERROR_RETURN(READ_ASF_EPORT_CFGr(unit, port, &rval));
            enable_org = soc_reg_field_get(unit, ASF_EPORT_CFGr, rval, ENABLEf);
            if (!enable_org) {
                return SOC_E_NONE;
            }
            break;
    ...
    if (_SOC_TH_ASF_MODE_SAF != mode) {
        ...
        bubble_mop_disable = 0; /* revisit after low latency feature */
        pause_enable = 0;
        ca_thresh = 4;
        ...
        /* OBM Cell Assembly FIFO threshold */
        SOC_IF_ERROR_RETURN(
            _soc_th_asf_obm_ca_fifo_thresh_set(unit, ca_thresh));

        /* BubbleMOP - At present only L3 & Full Latency Mode supported */
        SOC_IF_ERROR_RETURN(
            _soc_th_asf_obm_bubble_mop_set(unit, bubble_mop_disable));
        ...
        SOC_IF_ERROR_RETURN(
            READ_ASF_CREDIT_THRESH_HIr(unit, port, &rval));
        soc_reg_field_set(unit, ASF_CREDIT_THRESH_HIr, &rval, THRESHf,
                          mmu_ep_credits);
        SOC_IF_ERROR_RETURN(
		    WRITE_ASF_CREDIT_THRESH_HIr(unit, port, rval));
        ...
        /* EGR_MMU_CELL_CREDIT */
        SOC_IF_ERROR_RETURN(
            READ_EGR_MMU_CELL_CREDITr(unit, port, &rval));
    ...
    int        port_speed)
{
    ...
    SOC_IF_ERROR_RETURN(
        soc_th_port_asf_mode_get(unit, port, port_speed, &mode));
    SOC_IF_ERROR_RETURN(
        _soc_th_port_asf_speed_limits_get(unit, port, port_speed,
            mode, &min_speed, &max_speed));

    cli_out("%-4s     %-11s      ", SOC_PORT_NAME(unit, port), mode_str[mode]);
    if (_SOC_TH_ASF_MODE_SAF == mode) {
        cli_out("    .. NA ..\n");
    } else if (_SOC_TH_ASF_MODE_FAST_TO_SLOW == mode) {
    ...
    int        port_speed)
{
    ...
    uint32 rval, mmu_ep_credits, egr_mmu_cell_credits, ct_class;
    mac_driver_t *macd;
    soc_th_asf_mode_e mode = _SOC_TH_ASF_RETRV;
    ...
    if (SOC_E_NONE != ret) {
        cli_out("Error retrieving port ASF config: mode\n");
        return ret;
    }
    ...
    SOC_IF_ERROR_RETURN(
        _soc_th_port_asf_fifo_threshold_get(unit, port, port_speed,
            mode, min_sp, &fifo_threshold));

    SOC_IF_ERROR_RETURN(
        _soc_th_port_asf_fifo_depth_get(unit, port, port_speed,
            mode, min_sp, &fifo_depth));
    ...
    for (src_class = 0; src_class <= 12; src_class++) {
        ...
        SOC_IF_ERROR_RETURN(
            _soc_th_port_asf_xmit_start_count_get(unit, port,
                src_class, dst_class, mode, &xmit_start_cnt[src_class]));
    }

    cli_out("%-4s %-3d %-3d %-6d %-6d %-3d %-3d   ", SOC_PORT_NAME(unit, port),
            mode, ct_class, _SOC_TH_CT_CLASS_TO_SPEED_MAP(min_sp),
            _SOC_TH_CT_CLASS_TO_SPEED_MAP(max_sp), fifo_threshold,
            fifo_depth);
    for (src_class = 0; src_class <= 12; src_class++) {
        cli_out("%-3d ", xmit_start_cnt[src_class]);
    }
    ...
    SOC_IF_ERROR_RETURN(READ_EGR_MMU_CELL_CREDITr(unit, port,
                        &rval));
