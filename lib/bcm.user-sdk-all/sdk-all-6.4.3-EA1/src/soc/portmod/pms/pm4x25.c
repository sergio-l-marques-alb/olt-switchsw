...
int pm4x25_pm_init(int unit, const portmod_pm_create_info_internal_t* pm_add_info, int wb_buffer_index, pm_info_t pm_info)
{
    ...
    pm4x25_data = sal_alloc(sizeof(*(pm_info->pm_data.pm4x25_db)), "specific_db");
...
int pm4x25_port_attach(int unit, int port, pm_info_t pm_info, const portmod_port_add_info_t* add_info)
{
    ...
    rv = SOC_WB_ENGINE_SET_ARR(unit, SOC_WB_ENGINE_PORTMOD, pm_info->wb_vars_ids[ports], &port, port_index);
    _SOC_IF_ERR_EXIT(rv);
...
int _pm4x25_port_index_get(int unit, int port, pm_info_t pm_info, int *index)
{
   int i;
   int rv = 0;
   int tmp_port = 0;
...
{   
    ...
    _SOC_IF_ERR_EXIT(_pm4x25_port_index_get(unit, port, pm_info, &port_index));
    /*TBD - num of lanes*/
    phy_access[0].access.lane_mask = (1<<port_index);
