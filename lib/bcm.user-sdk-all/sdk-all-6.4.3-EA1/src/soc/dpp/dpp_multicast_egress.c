...
uint32 dpp_egq_vlan_table_tbl_get(
    ...
    SOC_SAND_OUT  dpp_mc_egr_bitmap_t* EGQ_vlan_table_tbl_data
)
{
    uint32 data[DPP_EGQ_VLAN_TABLE_TBL_ENTRY_SIZE];
    SOCDNX_INIT_FUNC_DEFS;
 
    SOCDNX_IF_ERR_EXIT(READ_EGQ_VLAN_TABLEm(unit, MEM_BLOCK_ANY, entry_offset, data));
    soc_mem_field_get(unit, EGQ_VLAN_TABLEm, data, VLAN_MEMBERSHIPf, EGQ_vlan_table_tbl_data->vlan_membership);
exit:
    SOCDNX_FUNC_RETURN;
}
...
uint32 dpp_egq_vlan_table_tbl_set(
    SOC_SAND_IN   int                  unit,
    SOC_SAND_IN   uint32               entry_offset,
    SOC_SAND_IN   dpp_mc_egr_bitmap_t* EGQ_vlan_table_tbl_data
)
{
    uint32 data[DPP_EGQ_VLAN_TABLE_TBL_ENTRY_SIZE];
    SOCDNX_INIT_FUNC_DEFS;

    data[DPP_EGQ_VLAN_TABLE_TBL_ENTRY_SIZE-1] = 0;
    soc_mem_field_set(unit, EGQ_VLAN_TABLEm, data, VLAN_MEMBERSHIPf, (uint32*)(&(EGQ_vlan_table_tbl_data->vlan_membership)));
    SOCDNX_IF_ERR_EXIT(WRITE_EGQ_VLAN_TABLEm(unit, MEM_BLOCK_ANY, entry_offset, data));
exit:
    SOCDNX_FUNC_RETURN;
}
...
{
  uint32 long_in_bitmap_i;
  dpp_mc_egr_bitmap_t egq_vlan_table_tbl_data;

  SOCDNX_INIT_FUNC_DEFS;
  SOCDNX_NULL_CHECK(group);

  SOCDNX_IF_ERR_EXIT(dpp_egq_vlan_table_tbl_get(unit, bitmap_id, &egq_vlan_table_tbl_data));

  for (long_in_bitmap_i = 0; long_in_bitmap_i < DPP_MC_NOF_EGRESS_BITMAP_WORDS; long_in_bitmap_i++) {
     group->bitmap[long_in_bitmap_i] = egq_vlan_table_tbl_data.vlan_membership[long_in_bitmap_i];
  }
...
)
{
  ...
  if (dpp_mult_does_group_exist(unit, multicast_id_ndx, TRUE)) { /* do nothing if not open */
    if (multicast_id_ndx <= SOC_DPP_CONFIG(unit)->tm.multicast_egress_bitmap_group_range.mc_id_high) { /* this is a vlan bitmap group */
      ...
      SOCDNX_IF_ERR_EXIT(dpp_mult_eg_vlan_membership_group_close(unit, multicast_id_ndx));
  ...
  if (multicast_id_ndx <= SOC_DPP_CONFIG(unit)->tm.multicast_egress_bitmap_group_range.mc_id_high) { /* this is a vlan bitmap group */
    ...
    *mc_group_size = 0;
    if (dpp_mult_does_group_exist(unit, multicast_id_ndx, TRUE)) { /* group open */
      dpp_mc_egr_bitmap_t egq_vlan_table_tbl_data;
      uint32 bits, *bits_ptr = egq_vlan_table_tbl_data.vlan_membership;
      unsigned units_left = DPP_MC_NOF_EGRESS_BITMAP_WORDS, bits_left;

      SOCDNX_IF_ERR_EXIT(dpp_egq_vlan_table_tbl_get(unit, multicast_id_ndx, &egq_vlan_table_tbl_data));
      for (; units_left; --units_left) {
        bits = *(bits_ptr++);
        for (bits_left = SOC_SAND_REG_SIZE_BITS; bits_left; --bits_left) {
...
{
  uint32 indx = SOC_DPP_CONFIG(unit)->tm.nof_mc_ids -1;
...

  {
    while (indx)
    {
      SOCDNX_IF_ERR_EXIT(dpp_mult_eg_group_close(unit, --indx));
    }
  }
...
)
{
  ...
  SOCDNX_IF_ERR_EXIT(dpp_egq_vlan_table_tbl_set(unit, multicast_id_ndx, &egq_vlan_table_tbl_data));
...
)
{
  ...
  uint32 long_in_bitmap_i;
  dpp_mc_egr_bitmap_t egq_vlan_table_tbl_data;
  SOC_TMC_MULT_EG_VLAN_MEMBERSHIP_GROUP_RANGE info;

  SOCDNX_INIT_FUNC_DEFS;

  SOCDNX_NULL_CHECK(group);

  SOCDNX_IF_ERR_EXIT(dpp_mult_eg_vlan_membership_group_range_get(unit, &info));

  tbl_offset = multicast_id_ndx - info.mc_id_low;

  if (tbl_offset >= DPP_MC_EGR_NOF_BITMAPS) {
    SOCDNX_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_SOCDNX_MSG("ID is too high for a multicast bitmap")));
  }

  SOCDNX_IF_ERR_EXIT(dpp_egq_vlan_table_tbl_get(unit, tbl_offset, &egq_vlan_table_tbl_data));
  ...
  SOCDNX_IF_ERR_EXIT(dpp_egq_vlan_table_tbl_set(unit, tbl_offset, &egq_vlan_table_tbl_data));
exit:
  SOCDNX_FUNC_RETURN;
}
 ...
 */
uint32 dpp_mult_eg_vlan_membership_group_close(
    SOC_SAND_IN  int          unit,
    SOC_SAND_IN  dpp_mc_id_t  multicast_id_ndx
)
{
...
{
  uint32 tbl_offset, val;
  uint32 port_inner_i, long_in_bitmap_i;
  dpp_mc_egr_bitmap_t egq_vlan_table_tbl_data;
  SOC_TMC_MULT_EG_VLAN_MEMBERSHIP_GROUP_RANGE info;
  ...
  val = 0x1;
  if (egq_vlan_table_tbl_data.vlan_membership[long_in_bitmap_i] & SOC_SAND_BIT(port_inner_i)) {
    *out_err = _SHR_E_EXISTS; /* port is already replicated to */
  } else {
    *out_err = _SHR_E_NONE;
    SOC_SAND_SET_BIT(egq_vlan_table_tbl_data.vlan_membership[long_in_bitmap_i], val , port_inner_i);
...
)
{
  ...
  val = 0x0;
  if (egq_vlan_table_tbl_data.vlan_membership[long_in_bitmap_i] & SOC_SAND_BIT(port_inner_i)) {
    *out_err = _SHR_E_NONE; /* port is replicated to and needs to be removed */
    SOC_SAND_SET_BIT(egq_vlan_table_tbl_data.vlan_membership[long_in_bitmap_i], val , port_inner_i);
...
)
{
  ...
  SOCDNX_IF_ERR_EXIT(soc_sand_os_memset(&egq_vlan_table_tbl_data, 0, sizeof(dpp_mc_egr_bitmap_t)));
  
  for (tbl_offset = 0 ; tbl_offset < SOC_TMC_MULT_EG_VLAN_MEMBERSHIP_GROUP_RANGE_MAX; tbl_offset++)
  {
    for (long_in_bitmap_i = 0; long_in_bitmap_i < DPP_MC_NOF_EGRESS_BITMAP_WORDS; long_in_bitmap_i++)
    {
      egq_vlan_table_tbl_data.vlan_membership[long_in_bitmap_i] = 0;
