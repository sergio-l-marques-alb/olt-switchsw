...
int soc_dpp_drc_combo28_util_cas_latency_calc(int unit, uint32 dram_type, uint32 c_cas_latency, uint32 *cas_latency_calc)
{
    ...
    *cas_latency_calc = 0x0;
    ...
    switch (dram_type) {   
    case SHMOO_COMBO28_DRAM_TYPE_DDR4:
        if ((c_cas_latency >=9) && (c_cas_latency <= 16)) {
            *cas_latency_calc = c_cas_latency - 9;
        } else if ((c_cas_latency == 18) || (c_cas_latency == 20) || (c_cas_latency == 22) || (c_cas_latency == 24)) {
            *cas_latency_calc = (c_cas_latency / 2) - 1;
        } else {
...
int soc_dpp_drc_combo28_info_struct_convert(int unit, soc_dpp_drc_combo28_info_t *drc_info, combo28_shmoo_dram_info_t *shmoo_info)
{
    ...
    shmoo_info->dram_bitmap             = SOC_PBMP_WORD_GET(drc_info->dram_bitmap, 0x0);
...
int soc_dpp_drc_combo28_drc_dq_swap(int unit, int drc_ndx, soc_dpp_drc_combo28_info_t *drc_info) 
{
    ...
    uint32 
        reg32_val,
        addr_dq_bit_map_val[17] = {9, 11, 13, 15, 25, 27, 29, 31, 30, 10, 8 , 28, 1 , 12, 17, 24, 1}; 
    ...
    static soc_field_t 
        ...
        DRC_GDDR_5_BIST_ADT_ADDR_DQ_BIT_MAPl[] = {ADDR_0f, ADDR_1f, ADDR_2f, ADDR_3f, ADDR_4f, ADDR_5f, ADDR_6f, ADDR_7f, ADDR_8f, ADDR_9f, ADDR_10f, ADDR_11f, ADDR_12f, 
                                                  BANK_0f, BANK_1f, BANK_2f, BANK_3f, DQ_TO_ADDR_MAP_ENf};

    SOCDNX_INIT_FUNC_DEFS;

    for (byte = 0; byte < SOC_DPP_DRC_COMBO28_NOF_PER_INTERFACE_BYTES; byte++) {
        for (bit = 0; bit < SOC_DPP_DRC_COMBO28_NOF_BITS_IN_BYTE ; bit++) {
            if (drc_info->dram_dq_swap[drc_ndx][byte][bit] != SOC_DPP_DRC_COMBO28_DQ_SWAP_VAL_DEFAULT_NO_SWAP) {

                SOCDNX_IF_ERR_EXIT(soc_reg32_get(unit, DRC_DQ_BYTE_MAPl[byte][drc_ndx], REG_PORT_ANY, 0, &reg32_val));
                soc_reg_field_set(unit, DRCA_DQ_BYTE_0_MAPr, &reg32_val, DQ_BIT_MAPl[byte][bit], drc_info->dram_dq_swap[drc_ndx][byte][bit]);
                soc_reg_field_set(unit, DRCA_DQ_BYTE_0_MAPr, &reg32_val, DQ_BIT_MAPl[byte][drc_info->dram_dq_swap[drc_ndx][byte][bit]], bit);
                SOCDNX_IF_ERR_EXIT(soc_reg32_set(unit, DRC_DQ_BYTE_MAPl[byte][drc_ndx], REG_PORT_ANY, 0, reg32_val));

                LOG_VERBOSE(BSL_LS_SOC_DRAM,
                            (BSL_META_U(unit,
                                        "%s(): dq_swap[drc_ndx=%d][byte=%d][bit=%d]=%d, reg32_val=0x%x.\n"),
    ...
    for (i = 0; i < 17; i++) { 
        SHR_BITCOPY_RANGE(field_above64_val, 0, &addr_dq_bit_map_val[i], 0, 32);
        soc_reg_above_64_field_set(unit, DRCA_GDDR_5_BIST_ADT_ADDR_DQ_MAPr, reg_above64_val, DRC_GDDR_5_BIST_ADT_ADDR_DQ_BIT_MAPl[i], field_above64_val);
    }
    reg32_val = 0x0;
    SHR_BITCOPY_RANGE(field_above64_val, 0, &reg32_val, 0, 32);
    soc_reg_above_64_field_set(unit, DRCA_GDDR_5_BIST_ADT_ADDR_DQ_MAPr, reg_above64_val, DQ_TO_ADDR_MAP_ENf, field_above64_val);
...
int soc_dpp_drc_combo28_drc_general_register_set(int unit, int drc_ndx, soc_dpp_drc_combo28_info_t *drc_info, uint32 write_crc) 
{
    ...
    /* Should be aligned to STOP_MMU_PIPE_INSIDE_DPRC in MMU (MMU_GENERAL_CONFIGURATION_REGISTER). Both set to 0x0 */
    field32_val = 0;
    ...
    if (SOC_IS_ARDON(unit)) {
        soc_reg_field_set(unit, DRCA_GENERAL_CONFIGURATIONSr, &reg32_val, ITEM_0_4f, field32_val);
    ...
    COMPILER_64_SET(field64_val, 0x0, ((drc_info->read_crc == 0x0) ? 0x0 : drc_info->dram_param.c_cas_latency + drc_info->dram_int_param.c_crc_rd_latency + 3));
    soc_reg64_field_set(unit, DRCA_DRAM_SPECIAL_FEATURESr, &reg64_val, EDC_RD_LATTENCYf, field64_val);
    COMPILER_64_SET(field64_val, 0x0, ((write_crc == 0x0) ? 0x0 : drc_info->dram_param.c_wr_latency + drc_info->dram_int_param.c_crc_wr_latency + 3));
    ...
    COMPILER_64_SET(field64_val, 0x0, ((drc_info->dram_type == SHMOO_COMBO28_DRAM_TYPE_GDDR5) ? 0 : drc_info->cmd_par_latency));
...
int soc_dpp_drc_combo28_mr_register_info_prepare(int unit, soc_dpp_drc_combo28_info_t *drc_info)
{
    ...
    case SHMOO_COMBO28_DRAM_TYPE_DDR4:
        ...
        /* MR0 */
        drc_info->mr[0]  = 0x0;
        drc_info->mr[0] |= SOC_SAND_SET_BITS_RANGE(0x0 /* val */, 12 /* MSB */, 12 /* LSB */);           /* DDRPD - Normal PD mode */
        ...
        drc_info->mr[5] |= SOC_SAND_SET_BITS_RANGE(0x1, 9, 6);                   /* RTT_Park. ZQ/4 */
...
int soc_dpp_src_combo28_gear_down_mode(int unit, int drc_ndx){
    ...
    SOCDNX_INIT_FUNC_DEFS;
     ...
     COMPILER_64_ADD_32(field64_val, 1);
     COMPILER_64_SHR(field64_val,    1);
     SOCDNX_IF_ERR_EXIT(soc_reg64_set(unit, DRC_DRAM_SPECIAL_FEATURESl[drc_ndx], REG_PORT_ANY, 0, field64_val));
     ...
     SOCDNX_IF_ERR_EXIT(soc_reg32_get(unit, DRC_DRAM_COMPLIANCE_CONFIGURATION_REGISTERl[drc_ndx], REG_PORT_ANY, 0, &reg32_val));
     field32_val = soc_reg_field_get(unit, DRCA_DRAM_COMPLIANCE_CONFIGURATION_REGISTERr, reg32_val, WR_LATENCYf);
     field32_val = SOC_SAND_DIV_ROUND_UP(field32_val, 2);
     soc_reg_field_set(unit, DRCA_DRAM_COMPLIANCE_CONFIGURATION_REGISTERr, &reg32_val, WR_LATENCYf, field32_val);
     SOCDNX_IF_ERR_EXIT(soc_reg32_set(unit, DRC_DRAM_COMPLIANCE_CONFIGURATION_REGISTERl[drc_ndx], REG_PORT_ANY, 0, reg32_val));
...
int soc_dpp_drc_combo28_init_tune_ddr(int unit, int drc_ndx, soc_dpp_drc_combo28_info_t *drc_info)
{
...
#endif
    {
        ...
        if (drc_info->auto_tune == 0x1) {
            rv = soc_combo28_shmoo_ctl(unit, drc_ndx, shmoo_type, stat, plot, SHMOO_COMBO28_ACTION_RUN_AND_SAVE, &(drc_info->shmoo_config_param[drc_ndx]));
            SOCDNX_IF_ERR_EXIT(rv);            
            
            rv = soc_dpp_drc_combo28_shmoo_cfg_set(unit, drc_ndx, &(drc_info->shmoo_config_param[drc_ndx]));
            SOCDNX_IF_ERR_EXIT(rv);            

        } else if (drc_info->auto_tune == 0x0) {
        
            rv = soc_combo28_shmoo_ctl(unit, drc_ndx, shmoo_type, stat, plot, SHMOO_COMBO28_ACTION_RESTORE, &(drc_info->shmoo_config_param[drc_ndx]));
            SOCDNX_IF_ERR_EXIT(rv);
        } 

    } 
    
    /* Enable Write CRC for DDR4 */   
    if ((drc_info->dram_type == SHMOO_COMBO28_DRAM_TYPE_DDR4) && (drc_info->write_crc == 0x1)) {
    
        /* Set General DRC registers (Will also enable refi) */
        rv = soc_dpp_drc_combo28_drc_general_register_set(unit, drc_ndx, drc_info, drc_info->write_crc);
        SOCDNX_IF_ERR_EXIT(rv);

        /* Set AC Operation DRC registers*/
        rv = soc_dpp_drc_combo28_drc_ac_op_register_set(unit, drc_ndx, drc_info, drc_info->write_crc);
        SOCDNX_IF_ERR_EXIT(rv);
        ...
        /* Set Write CRC in MR registers*/
        rv = soc_dpp_drc_combo28_enable_wr_crc(unit, drc_ndx, drc_info->write_crc /* enable */);
        SOCDNX_IF_ERR_EXIT(rv);
