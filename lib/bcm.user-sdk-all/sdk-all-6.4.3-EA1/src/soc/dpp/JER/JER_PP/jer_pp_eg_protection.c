...
#define JER_PP_EGRESS_PROTECTION_PATH_ENTRY_OFFSET_MASK        (0x7)
#define JER_PP_EGRESS_PROTECTION_PATH_ENTRY_IDX_MASK           (0xFFF8)
   ...
   uint8 path_state)
{
    ...
    uint32 epni_outlif_protection_path_entry_data;
    uint8 entry_offset;
    SOCDNX_INIT_FUNC_DEFS;
    ...
    epni_outlif_protection_path_entry_data = 0;
    ...
    tbl_idx = (protection_ndx & JER_PP_EGRESS_PROTECTION_PATH_ENTRY_OFFSET_MASK) >>
              JER_PP_EGRESS_PROTECTION_PATH_NOF_ENTRY_OFFSET_BITS;

    /* Read the required protection path entry from the HW */
    rv = READ_EPNI_PROTECTION_PTR_TABLEm(unit, MEM_BLOCK_ANY, tbl_idx, &epni_outlif_protection_path_entry_data);
    SOCDNX_IF_ERR_EXIT(rv);
    ...
    rv = WRITE_EPNI_PROTECTION_PTR_TABLEm(unit, MEM_BLOCK_ALL, tbl_idx, &epni_outlif_protection_path_entry_data);
    SOCDNX_IF_ERR_EXIT(rv);
