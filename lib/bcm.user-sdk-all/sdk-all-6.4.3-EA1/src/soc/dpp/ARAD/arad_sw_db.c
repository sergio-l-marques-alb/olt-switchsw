  ...
  )
{
  ...
  res = arad_sw_db_tm_queue_to_rate_class_mapping_is_simple_set(unit, TRUE);
  SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
...
uint32 arad_sw_db_modport2sysport_init(SOC_SAND_IN int unit);
uint32 arad_sw_db_modport2sysport_terminate(SOC_SAND_IN int unit);
  ...
  )
{
  ...
  res = arad_sw_db_modport2sysport_terminate(unit);
  SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
  ...
  )
{
    ...
    res = arad_wb_db_lag_update_local_to_sys_state(unit, tm_port); 
    SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_sw_db_local_to_sys_port_set()",0,0);
}
...
arad_sw_db_sw_dump(int unit) 	 
{
    ...
    for (i = 0; i < ARAD_NOF_LOCAL_PORTS; i++) {
        if(db->lag.local_to_sys[i] != 0) {
            LOG_VERBOSE(BSL_LS_SOC_SWDB,
                        (BSL_META_U(unit,
                                    "\n local_to_sys (%03d):  %u\n"), i, db->lag.local_to_sys[i]));
        }
    }
...
uint32 arad_sw_db_modport2sysport_init(
  SOC_SAND_IN int unit
  )
{
  uint32 res = SOC_SAND_OK;
  ...
  Arad_sw_db.arad_device_sw_db[unit]->modport2sysport = NULL;

  /* in case of direct VOQ mapping mode, create the modport2sysport mapping  data structure */
  if (ARAD_IS_VOQ_MAPPING_DIRECT(unit)) {
    res = arad_sw_db_modport2sysport_create(unit);
    SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
  }

exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_sw_db_modport2sysport_init()",unit,0);
}

uint32 arad_sw_db_modport2sysport_terminate(
  SOC_SAND_IN int unit
  )
{
  uint32 res = SOC_SAND_OK;
  ...
  if (Arad_sw_db.arad_device_sw_db[unit]->modport2sysport != NULL) {
    res = arad_sw_db_modport2sysport_destroy(unit);
    SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  }

exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_sw_db_modport2sysport_terminate()",unit,0);
}


/*
Create and allocate fap_id x fap_port_id to system physical port mapping.
  ...
  )
{
  ...
  /* allocate the memory blocks aray and init it to 0 values */
  ARAD_ALLOC(Arad_sw_db.arad_device_sw_db[unit]->modport2sysport, ARAD_SYSPORT*, ARAD_MOD_PORT2SYSPORT_ALLOC_NUM, "modport2sysport");
...
/*
Destroy and deallocate fap_id x fap_port_id to system physical port mapping data structure.
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(0);
  SOC_SAND_CHECK_NULL_PTR(Arad_sw_db.arad_device_sw_db[unit], SOC_SAND_ERR, exit);
  SOC_SAND_CHECK_NULL_INPUT(Arad_sw_db.arad_device_sw_db[unit]->modport2sysport);

  /* deallocate allocated blocks */
  for (i = 0; i < ARAD_MOD_PORT2SYSPORT_ALLOC_NUM; ++i) {
    ARAD_FREE((Arad_sw_db.arad_device_sw_db[unit]->modport2sysport)[i]);
  }
  /* deallocate the memory blocks arrays */
  ARAD_FREE(Arad_sw_db.arad_device_sw_db[unit]->modport2sysport);

exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_sw_db_modport2sysport_destroy()", 0, 0);
}
  ...
  )
{
  ...
  SOC_SAND_ERR_IF_ABOVE_NOF(sysport, ARAD_NOF_SYS_PHYS_PORTS, SOC_SAND_VALUE_ABOVE_MAX_ERR, 20, exit);
  SOC_SAND_CHECK_NULL_PTR(Arad_sw_db.arad_device_sw_db[unit], SOC_SAND_ERR, exit);
  ...
  index = modport / ARAD_MOD_PORT2SYSPORT_SYSPORTS_PER_ALLOC;
  if (!modport2sysport[index]) {
    unsigned i = 0;
    ARAD_SYSPORT* new_block;

    ARAD_ALLOC(modport2sysport[index], ARAD_SYSPORT, ARAD_MOD_PORT2SYSPORT_SYSPORTS_PER_ALLOC, "modport2sysport[index]");
    new_block = modport2sysport[index];
    for (; i < ARAD_MOD_PORT2SYSPORT_SYSPORTS_PER_ALLOC; ++i) {
      new_block[i] = ARAD_NOF_SYS_PHYS_PORTS; /* mark as unmapped */
    }
  }
  modport2sysport[index][modport % ARAD_MOD_PORT2SYSPORT_SYSPORTS_PER_ALLOC] = sysport;
  ...
  )
{
  ...
  index = modport / ARAD_MOD_PORT2SYSPORT_SYSPORTS_PER_ALLOC;
  *sysport = modport2sysport[index] ?
    modport2sysport[index][modport % ARAD_MOD_PORT2SYSPORT_SYSPORTS_PER_ALLOC] :
    ARAD_NOF_SYS_PHYS_PORTS;
  /* SOC_SAND_ERR_IF_ABOVE_NOF(sysport, ARAD_NOF_SYS_PHYS_PORTS, SOC_SAND_HASH_TABLE_KEY_IS_NOT_EXIST_ERR, 30, exit); */
  ...
  )
{
  ...
  for (i = 0; i < ARAD_MOD_PORT2SYSPORT_ALLOC_NUM; ++i) {
    const ARAD_SYSPORT* sysport_arr = modport2sysport[i];
    if (sysport_arr) {
      for (j = 0; j < ARAD_MOD_PORT2SYSPORT_SYSPORTS_PER_ALLOC; ++j) {
        if (sysport_arr[j] == sysport) {
          const uint32 modport = j | (i << ARAD_MOD_PORT2SYSPORT_SYSPORTS_PER_ALLOC_BITS);
          *fap_id = modport & ARAD_DEVICE_NUMBER_MASK;
          *fap_port_id = modport >> ARAD_DEVICE_NUMBER_BITS;
          goto exit;
        }
      }
    }
  ...
  *fap_id = *fap_port_id = ARAD_SW_DB_MODPORT2SYSPORT_REVERSE_GET_NOT_FOUND;
  /* SOC_SAND_SET_ERROR_CODE(SOC_SAND_HASH_TABLE_KEY_IS_NOT_EXIST_ERR, 30, exit); */
...
/*
Remove a fap_id x fap_port_id to system physical port mapping.
Does not perform deallocation if this is the last mapping in the allocated block.
*/
  ...
  )
{
  ...
  if (modport2sysport[index]) {
    /* SOC_SAND_ERR_IF_ABOVE_NOF(sysport, ARAD_NOF_SYS_PHYS_PORTS, SOC_SAND_HASH_TABLE_KEY_IS_NOT_EXIST_ERR, 30, exit); */
    modport2sysport[index][modport % ARAD_MOD_PORT2SYSPORT_SYSPORTS_PER_ALLOC] = ARAD_NOF_SYS_PHYS_PORTS;
  }
  /* else { SOC_SAND_HASH_TABLE_KEY_IS_NOT_EXIST_ERR, 40, exit); } */
