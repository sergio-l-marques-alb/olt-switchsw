  ...
  )
{
  ...
  res = arad_fill_table_with_entry(unit, IHP_VTT_IN_PP_PORT_CONFIGm, MEM_BLOCK_ANY, &vtt_in_pp_port_conf_tbl); 
  SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);

  if (SOC_IS_JERICHO(unit)) {
    soc_sand_os_memset(ihb_flp_pinfo_tbl, 0x0, sizeof(ihb_flp_pinfo_tbl));
    res = arad_fill_table_with_entry(unit, IHP_PINFO_FLP_0m, MEM_BLOCK_ANY, &ihb_flp_pinfo_tbl); 
    SOC_SAND_CHECK_FUNC_RESULT(res, 42, exit);
    soc_sand_os_memset(ihb_flp_pinfo_tbl, 0x0, sizeof(ihb_flp_pinfo_tbl));
    res = arad_fill_table_with_entry(unit, IHP_PINFO_FLP_1m, MEM_BLOCK_ANY, &ihb_flp_pinfo_tbl); 
...
{ 
   ...
   */
  res = arad_pp_ihp_pinfo_llr_tbl_get_unsafe(unit, local_port_ndx, &pinfo_llr_tbl);
  ...
  if (!SOC_DPP_PP_ENABLE(unit)) {
      ...
      res = arad_pp_ihp_pinfo_llr_tbl_set_unsafe(unit, local_port_ndx, &pinfo_llr_tbl);
  ...
  res = arad_pp_ihp_pp_port_info_tbl_get_unsafe(unit, local_port_ndx, &pp_port_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);

  res = arad_pp_ihp_vtt_in_pp_port_config_tbl_get_unsafe(unit, local_port_ndx, &vtt_in_pp_port_conf_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 12, exit);

  res = arad_pp_ihb_pinfo_flp_tbl_get_unsafe(unit, local_port_ndx, &pinfo_flp_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 16, exit);

  res = arad_pp_egq_pp_ppct_tbl_get_unsafe(unit, local_port_ndx, &egq_pp_ppct_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);

  res = arad_egq_pct_tbl_get_unsafe(unit, local_port_ndx, 0, &pct_tbl);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 22, exit);

  res = arad_pp_epni_pp_pct_tbl_get_unsafe(unit, local_port_ndx, 0, &epni_pp_pct_tbl);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 24, exit);

  res = arad_pp_ihp_vtt_pp_port_vsi_profiles_tbl_get_unsafe(unit, local_port_ndx, &vsi_profiles_tbl);
  ...
  res = arad_pp_ihp_pp_port_info_tbl_set_unsafe(unit, local_port_ndx, &pp_port_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);

  res = arad_pp_ihp_vtt_in_pp_port_config_tbl_set_unsafe(unit, local_port_ndx, &vtt_in_pp_port_conf_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 32, exit);

  res = arad_pp_ihp_pinfo_llr_tbl_set_unsafe(unit, local_port_ndx, &pinfo_llr_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 34, exit);

  res = arad_pp_ihb_pinfo_flp_tbl_set_unsafe(unit, local_port_ndx, &pinfo_flp_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 36, exit);

  res = arad_egq_pct_tbl_set_unsafe(unit, local_port_ndx, 0, &pct_tbl);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 38, exit);

  res = arad_pp_epni_pp_pct_tbl_set_unsafe(unit, local_port_ndx, 0, &epni_pp_pct_tbl);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 40, exit);

  res = arad_pp_egq_pp_ppct_tbl_set_unsafe(unit, local_port_ndx, &egq_pp_ppct_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 42, exit);

  res = arad_pp_ihp_vtt_pp_port_vsi_profiles_tbl_set_unsafe(unit, local_port_ndx, &vsi_profiles_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 44, exit);
  
 /*
  * save to SW DB
  */
  res = arad_pp_sw_db_pon_double_lookup_port_set(unit, local_port_ndx, port_info->enable_pon_double_lookup);
  ...
  )
{
  ...
  res = arad_pp_sw_db_pon_double_lookup_port_get(unit, local_port_ndx, &pon_double_lookup_enabled);
  SOC_SAND_CHECK_FUNC_RESULT(res, 26, exit);
  ...
  )
{
  ...
  res = arad_pp_ihp_stp_table_tbl_get_unsafe(unit, local_port_ndx, &ing_stp_tbl_data);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  ...
  res = arad_pp_ihp_stp_table_tbl_set_unsafe(unit, local_port_ndx, &ing_stp_tbl_data);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  ...
  res = arad_pp_epni_stp_tbl_get_unsafe(unit, topology_id_ndx, &eg_stp_tbl_data);
  SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);

  SHR_BITCOPY_RANGE(eg_stp_tbl_data.stp, local_port_ndx, &eg_stp_state_bit, 0, 1);

  res = arad_pp_epni_stp_tbl_set_unsafe(unit, topology_id_ndx, &eg_stp_tbl_data);
  SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
  ...
  )
{
  ...
  SOC_SAND_ERR_IF_ABOVE_NOF(info->dflt_egress_ac, SOC_DPP_DEFS_GET(unit, nof_local_lifs), ARAD_PP_AC_ID_OUT_OF_RANGE_ERR, 26, exit);
...
*********************************************************************/
soc_error_t 
arad_pp_port_property_set(int unit, int port, SOC_PPC_PORT_PROPERTY port_property, uint32 value){
    uint32 data;
    int rv;
    ...
    /* Read table entry. */
    rv = READ_IHP_VTT_PP_PORT_VT_KEY_VARm(unit, SOC_BLOCK_ANY, port, &data);
    ...
    /* Write data to HW. */
    rv = WRITE_IHP_VTT_PP_PORT_VT_KEY_VARm(unit, SOC_BLOCK_ANY, port, &data);
    SOCDNX_IF_ERR_EXIT(rv);
...
soc_error_t arad_pp_port_additional_tpids_set(int unit, uint32 *additional_tpids)
{
    ...
    soc_mem_field32_set(unit, IHP_PARSER_ETH_PROTOCOLSm, &data, ETH_TYPE_PROTOCOLf, additional_tpids[0]);

    rv = WRITE_IHP_PARSER_ETH_PROTOCOLSm(unit, MEM_BLOCK_ANY, ARAD_PARSER_ETHER_PROTO_4_ADD_TPID, &data);
    SOCDNX_IF_ERR_EXIT(rv);
...
soc_error_t arad_pp_port_additional_tpids_get(int unit, uint32 *additional_tpids)
{
    ...
    rv = READ_IHP_PARSER_ETH_PROTOCOLSm(unit, MEM_BLOCK_ANY, ARAD_PARSER_ETHER_PROTO_4_ADD_TPID, &data);
    SOCDNX_IF_ERR_EXIT(rv);

    additional_tpids[0] = soc_mem_field32_get(unit, IHP_PARSER_ETH_PROTOCOLSm, &data, ETH_TYPE_PROTOCOLf);
