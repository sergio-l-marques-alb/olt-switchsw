...
uint32
  arad_pp_fp_action_alloc_fes(
    ...
    SOC_SAND_INOUT ARAD_SW_DB_PMF_FES         fes_info[ARAD_PMF_LOW_LEVEL_NOF_FESS],
  ...
  )
{
    ...
    *found = 0;
    ...
    if (flags & ARAD_PP_FP_FEM_ALLOC_FES_TM) {
        success = SOC_SAND_SUCCESS;
        goto exit_tm;
    }
    ...
    if (success != SOC_SAND_SUCCESS) {
        ...
        for (fem_id_ndx = 0; (fem_id_ndx < ARAD_PMF_LOW_LEVEL_NOF_FEMS_PER_GROUP) && (!is_fem_blocking); ++fem_id_ndx)
        {
              ...
              res = arad_sw_db_fp_fem_entry_get(
                      unit,
                      fem_id_ndx,
                      &fem_entry2
                    );
  ...
  )
{
  ...
  res = arad_sw_db_pgm_all_fes_get(
            unit,
            stage,
            prog_id,
            fes_info
  ...
  )
{
    ...
    *success = SOC_SAND_SUCCESS;
    ...
    if ((stage != ARAD_FP_DATABASE_STAGE_INGRESS_FLP) && (stage != ARAD_FP_DATABASE_STAGE_INGRESS_SLB)) { /* Skip the verification for the FLP stage */
        if (nof_bits <= 20) {
  ...
  )
{
    ...
    if (fp_db_info.flags & SOC_PPC_FP_DATABASE_INFO_FLAGS_STAGE_VT )
    {
         is_vt_match_ipv4 = TRUE;
    }

    if ((fp_db_info.flags & SOC_PPC_FP_DATABASE_INFO_FLAGS_EXTENDED_FWRD_DB) || (fp_db_info.flags & SOC_PPC_FP_DATABASE_INFO_FLAGS_EXTENDED_IPV6_FWRD_DB))
    {
  ...
  )
{
  ...
  res = arad_sw_db_fp_fem_entry_set(
          unit,
          fem_id_dest,
          &fem_entry
        );
        ...
        if (is_for_fem) {
              ...
              && (fem_free_min_ndx == nof_fem_fes_per_group))
          {
            ...
            if(fes_info[fes_id_indx].is_used && (fes_info[fes_id_indx].action_type == entry_ndx->action_type[action_ndx])) {
                res = arad_sw_db_pmf_db_prio_get(unit, stage, fes_info[fes_id_indx].db_id, &fes_prio);
                SOC_SAND_CHECK_FUNC_RESULT(res, 100, exit);
        ...
        if (is_fem_lower[fem_id_ndx] == TRUE)
        {
          ...
          if (is_for_fem) {
              ...
              fes_id_indx_2 = fem_free_first + (cycle_to_use * nof_fem_fes_per_group); /* To */
                ...
                sal_memcpy(&fes_info[fes_id_indx_2],&fes_info[fes_id_indx],sizeof(ARAD_SW_DB_PMF_FES));
                ...
                if (is_for_fem? (!is_other_type_fes_fem): is_other_type_fes_fem) {
                  ...
                  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
                    ...
                    if(fes_info[fes_id_indx].is_used) {
                        ...
                        res = arad_sw_db_pmf_db_prio_get(unit, stage, fes_info[fes_id_indx].db_id, &fem_entry.db_strength);
  ...
  )
{
  ...
  is_for_fem = entry_ndx->is_for_entry;
  if (!is_for_fem) {
      sal_memcpy(fes_info_lcl, fes_info, ARAD_PMF_LOW_LEVEL_NOF_FESS * sizeof(ARAD_SW_DB_PMF_FES));
  }
  else {
