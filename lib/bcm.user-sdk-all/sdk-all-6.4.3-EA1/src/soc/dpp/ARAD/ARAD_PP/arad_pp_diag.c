  ...
  )
{
  ...
  if (!SOC_IS_ARDON(unit)) {
      ...
      SOC_SAND_SOC_IF_ERROR_RETURN(res, 20, exit, READ_IHP_DBG_FLP_PROGRAM_SELECTION_CAM_LINEr(unit, &reg_val64));
      if (!COMPILER_64_IS_ZERO(reg_val64)) {
          *(ret_val) = 1;
          goto exit;
      }
      SOC_SAND_SOC_IF_ERROR_RETURN(res, 30, exit, READ_IHP_DBG_FLP_SELECTED_PROGRAMr(unit, &reg_val));
      if (reg_val != 0) {
          *(ret_val) = 1;
          goto exit;
      }
  ...
  )
{
  ...
  if (pkt_trace & SOC_PPC_DIAG_PKT_TRACE_FLP)
  {
      ...
      SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  44,  exit, ARAD_REG_ACCESS_ERR,WRITE_IHP_DBG_FLP_PROGRAM_SELECTION_CAM_LINEr(unit, clear_val_64));
      SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  45,  exit, ARAD_REG_ACCESS_ERR,WRITE_IHP_DBG_FLP_SELECTED_PROGRAMr(unit, clear_val));
  }
  ...
  if (pkt_trace & ARAD_PP_DIAG_PKT_TRACE_TRAP)
  {
    ...
    /* FLP traps */
    SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  140,  exit, ARAD_REG_ACCESS_ERR,WRITE_IHP_DBG_FLP_TRAP_0r(unit, clear_val));
    SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  150,  exit, ARAD_REG_ACCESS_ERR,WRITE_IHP_DBG_FLP_TRAP_1r(unit, clear_val));
    /* FER traps */
  ...
  )
{
  ...
  /* FLP traps */
  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  60,  exit, ARAD_REG_ACCESS_ERR,READ_IHP_DBG_FLP_TRAP_0r(unit, &tmp_regs_val[0]));
  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  70,  exit, ARAD_REG_ACCESS_ERR,READ_IHP_DBG_FLP_TRAP_1r(unit, &tmp_regs_val[1]));
  ...
  */
  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  180,  exit, ARAD_REG_ACCESS_ERR,READ_IHB_DBG_LAST_TRAP_CHANGE_DESTINATION_REGr(unit, &reg_val));
