...
/* { */
STATIC
uint32 arad_pp_fp_vt_match_key_ces_get(      
          SOC_SAND_IN  int                    unit,
          SOC_SAND_IN  uint8                  vt_match_mode,
          SOC_SAND_OUT ARAD_SW_DB_PMF_CE      *ce_array,
          SOC_SAND_OUT uint8                  *nof_ces
       )
{
    const ARAD_SW_DB_PMF_CE ce_array_ipv4_match[6] = {
                {1,   0, 0, 15, 0, 0, 0, SOC_PPC_FP_QUAL_HDR_IPV4_SRC_PORT},
                {1,   0, 0, 15, 0, 0, 0, SOC_PPC_FP_QUAL_HDR_IPV4_DEST_PORT},
                {1,   0, 0, 31, 0, 0, 0, SOC_PPC_FP_QUAL_HDR_IPV4_SIP},
                {1,   0, 0, 7,  0, 0, 0, SOC_PPC_FP_QUAL_HDR_IPV4_NEXT_PRTCL},
                {1,   0, 0, 31, 0, 0, 0, SOC_PPC_FP_QUAL_HDR_IPV4_DIP},
                {1,   0, 0, 11, 0, 0, 0, SOC_PPC_FP_QUAL_HDR_FWD_IPV4_IN_VID}             
               };
    const ARAD_SW_DB_PMF_CE ce_array_efp_match[8] = {
                {1,   0, 0, 15, 0, 0, 0, SOC_PPC_FP_QUAL_HDR_ETHERTYPE},                
                {1,   0, 0, 2, 0, 0, 0,  SOC_PPC_FP_QUAL_HDR_VLAN_TAG_PRI},
                {1,   0, 0, 0, 0, 0, 0,  SOC_PPC_FP_QUAL_HDR_VLAN_TAG_CFI},
                {1,   0, 0, 11, 0, 0, 0, SOC_PPC_FP_QUAL_VLAN_EDIT_CMD_VID2},
                {1,   0, 0, 11, 0, 0, 0, SOC_PPC_FP_QUAL_VLAN_EDIT_CMD_VID1},
                {1,   0, 0, 7,  0, 0, 0, SOC_PPC_FP_QUAL_HDR_IPV4_NEXT_PRTCL},
                {1,   0, 0, 11, 0, 0, 0, SOC_PPC_FP_QUAL_HDR_2ND_VLAN_TAG_ID},
                {1,   0, 0, 11, 0, 0, 0, SOC_PPC_FP_QUAL_HDR_VLAN_TAG_ID}             
               };
    uint8 nof_ces_in_key = 0;
    switch ( vt_match_mode )
    {
    case ARAD_PP_FP_VT_MATCH_MODE_IPV4:
        nof_ces_in_key = 6;
        sal_memcpy(ce_array,&ce_array_ipv4_match,sizeof(ARAD_SW_DB_PMF_CE)* nof_ces_in_key );
        break;
    case ARAD_PP_FP_VT_MATCH_MODE_EFP:
        nof_ces_in_key = 8;
        sal_memcpy(ce_array,&ce_array_efp_match,sizeof(ARAD_SW_DB_PMF_CE)* nof_ces_in_key );
        break;
    default:
        break;
    }

    *nof_ces = nof_ces_in_key;
    return SOC_SAND_OK;
}
...
uint32 arad_pp_fp_kbp_match_key_ces_get(      
          SOC_SAND_IN  int                    unit,
          SOC_SAND_IN  int                    is_ipv6,
          SOC_SAND_OUT ARAD_SW_DB_PMF_CE      *ce_array,
          SOC_SAND_OUT uint8                  *nof_ces
       )
{
#define NUMBER_OF_CE_USED_FOR_EXTENDED_TABLES   5
    const ARAD_SW_DB_PMF_CE ce_array_kbp_ipv6_match[NUMBER_OF_CE_USED_FOR_EXTENDED_TABLES] = {
    ...
    const ARAD_SW_DB_PMF_CE ce_array_kbp_match[NUMBER_OF_CE_USED_FOR_EXTENDED_TABLES] = {
    ...
    if (is_ipv6) {
        sal_memcpy(ce_array,&ce_array_kbp_ipv6_match,sizeof(ARAD_SW_DB_PMF_CE)* NUMBER_OF_CE_USED_FOR_EXTENDED_TABLES );        
    } else {
        sal_memcpy(ce_array,&ce_array_kbp_match,sizeof(ARAD_SW_DB_PMF_CE)* NUMBER_OF_CE_USED_FOR_EXTENDED_TABLES );        
    }
     
    *nof_ces = NUMBER_OF_CE_USED_FOR_EXTENDED_TABLES;

    return SOC_SAND_OK;
#undef NUMBER_OF_CE_USED_FOR_EXTENDED_TABLES
}
  ...
  )
{
    ...
    res = arad_sw_db_pmf_db_info_get(
            unit,
            stage,
            db_ndx,
            &db_info
    ...
    for(ce_indx = 0; ce_indx < ARAD_PMF_LOW_LEVEL_NOF_CE_IN_PROG; ++ce_indx) {
        res = arad_sw_db_pgm_ce_instr_get(
                unit,
                stage,
                prog_ndx,
                db_prog_info->cycle,
    ...
    )
{
    ...
    for(zone_idx = 0; zone_idx < ARAD_PP_FP_KEY_NOF_ZONES; zone_idx++)
    {
        if(next_available_zone[0] == ARAD_PP_FP_KEY_NOF_ZONES && 
           remaining_bits_in_zone[zone_idx] >= qual_nof_bits[idx])
        {
            next_available_zone[0] = zone_idx;
        }
        if(next_available_zone[1] == ARAD_PP_FP_KEY_NOF_ZONES && 
           remaining_bits_in_zone[zone_idx] >= qual_nof_bits[idx_1])
        {
            next_available_zone[1] = zone_idx;
        }
    }
     ...
     * started and we want it to be the first availabl. e.g. if zone 0 is 
     * all used we want the next one to be zone 1 (rather than 2), and 
     * since it is MSB we willl try to allocate the MSB options first. 
     */
        ...
        if((nof_found == 2) && (int_qual_info[0].info.qual_nof_bits != int_qual_info[1].info.qual_nof_bits)) {
           LOG_ERROR(BSL_LS_SOC_FP,
                     (BSL_META_U(unit,
                      ...
                      unit, SOC_PPC_FP_DATABASE_STAGE_to_string(stage), ARAD_PP_FP_QUAL_TYPE_to_string(qual_type), int_qual_info[0].info.qual_nof_bits, int_qual_info[1].info.qual_nof_bits));
  ...
  )
{
    ...
    res = arad_sw_db_pgm_ce_instr_set(
            unit,
            stage,
            prog_ndx,
            cycle_ndx,
    ...
    )
{
  ...
  *key_bitmap_constraint = 0;
  ...
  if (is_cascaded && (lookup_id == 1)) {
       ...
       */ 
      if (sw_db_info.cascaded_key) {
          *is_key_fixed = TRUE;
          *key_id = ARAD_PP_FP_KEY_FROM_CASCADED_KEY_GET(sw_db_info.cascaded_key);
      }
      else {
          *is_key_fixed = FALSE;
          ...
          {
              if (SHR_BITGET(sw_db_info.progs, pmf_pgm_ndx)) {
                  /* Relevant program. OR on the taken keys */
                  res = arad_sw_db_pgm_key_rsrc_get(unit, stage, pmf_pgm_ndx, lookup_id /* 1 */, &keys_bmp);
                  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
    ...
    if (is_equal) 
    {
      ...
      *key_place = place_cons;
      ...
      for (pmf_pgm_ndx = pmf_pgm_ndx_min; pmf_pgm_ndx < pmf_pgm_ndx_max; ++pmf_pgm_ndx)
      {
          ...
          {
              /* Relevant program. OR on the taken keys */
              res = arad_sw_db_pgm_key_rsrc_get(
                        unit, 
                        stage, 
                        pmf_pgm_ndx, 
                        1 /* cycle */, 
...
/* Look in SW DB, but do not write anything in HW */
#define ARAD_PP_FP_KEY_ALLOC_KEY_CHECK_ONLY      (0x2)
  ...
  )
{
    ...
    *found = 0;
    ...
    if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
        if ((stage == ARAD_FP_DATABASE_STAGE_INGRESS_PMF) && (cycle_ndx == 0)) {
    ...
    *key_id = key_id_lcl;
    ...
    if(*found) {
        if (!(flags & ARAD_PP_FP_KEY_ALLOC_CHECK_ONLY)) {
            res = arad_sw_db_pgm_key_rsrc_set(
                    unit,
                    stage,
                    prog_ndx,
                    cycle_ndx,
            ...
            res = arad_sw_db_pmf_db_info_get(unit, stage, db_id, &sw_db_info);
            SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 30, exit);
            sw_db_info.cascaded_key = ARAD_PP_FP_KEY_TO_CASCADED_KEY_GET((*key_id));
            res = arad_sw_db_pmf_db_info_set(unit, stage, db_id, &sw_db_info);
  ...
  )
{
    ...
    *fit_ce = arad_pp_fp_key_get_available_ce_under_constraint(unit, stage, place_cons,size_cons,group_cons);
    ...
    if(*found == TRUE && !(flags & ARAD_PP_FP_KEY_ALLOC_CE_CHECK_ONLY)) {
        ...
        res = arad_sw_db_pgm_ce_rsrc_get(
                  unit,
                  stage,
                  prog_ndx,
                  cycle_ndx,
        ...
        res = arad_sw_db_pgm_ce_rsrc_set(
                  unit,
                  stage,
                  prog_ndx,
                  cycle_ndx,
...
STATIC
 uint32
  arad_pp_fp_key_alloc_in_prog_commit(
    ...
    SOC_SAND_IN  uint32                       db_id,
    SOC_SAND_IN  uint32                       flags,
    SOC_SAND_IN  ARAD_PP_FP_QUAL_TYPE         qual_types[ARAD_PP_FP_NOF_QUALS_PER_DB_MAX], /* qualifiers*/
    SOC_SAND_IN  ARAD_PP_FP_KEY_ALLOC_INFO   *alloc_info
  )
{
    uint32
    ...
    ARAD_SW_DB_PMF_CE
        sw_db_ce;
    ARAD_SW_DB_PMF_DB_INFO /* update db info */
        sw_db_info; 
            ...
            if(found == 0) {
               LOG_ERROR(BSL_LS_SOC_FP,
                         (BSL_META_U(unit,
                                     "   "
                                     "CE HW commit: "
                                     "qual_type %s unsupported \n\r"), ARAD_PP_FP_QUAL_TYPE_to_string(qual_type)));
    ...
    if (!(flags&ARAD_PP_FP_KEY_ALLOC_CHECK_ONLY)) {
        ...
        res = arad_sw_db_tcam_db_entry_size_get(unit,ARAD_PP_FP_DB_ID_TO_TCAM_DB(db_id), &entry_size);
        SOC_SAND_CHECK_FUNC_RESULT(res, 115, exit);
        nof_access_profiles = 0;
        if ((db_info.db_type != ARAD_PP_FP_DB_TYPE_DIRECT_EXTRACTION) 
            ...
            || is_equal ) 
        {
            ...
            if (SOC_IS_ARADPLUS(unit))
            {
                if (is_equal) 
                {
                    ...
                    SOC_SAND_SOC_IF_ERROR_RETURN(res, 165, exit, READ_IHB_KEY_D_XOR_MASKSr(unit, 0, reg_above_64));
                    ...
                    SOC_SAND_SOC_IF_ERROR_RETURN(res, 166, exit, WRITE_IHB_KEY_D_XOR_MASKSr(unit, 0, reg_above_64));

                    /*int soc_reg_above_64_set(int unit, soc_reg_t reg, int port, int index, soc_reg_above_64_val_t data)*/
    ...
    *prog_used_cycle_bmp_lcl = sw_db_info.prog_used_cycle_bmp;
    ...
    sw_db_info.used_key[prog_ndx][0] = alloc_info->key_id[0];
    sw_db_info.used_key[prog_ndx][1] = alloc_info->key_id[1];
    sw_db_info.is_320b = is_320b;
    sw_db_info.alloc_place = alloc_info->alloc_place;
    sw_db_info.nof_bits_zone[0][0] = nof_bits_zone[0][0];
    sw_db_info.nof_bits_zone[0][1] = nof_bits_zone[0][1];
    sw_db_info.nof_bits_zone[1][0] = nof_bits_zone[1][0];
    sw_db_info.nof_bits_zone[1][1] = nof_bits_zone[1][1];
    
    res = arad_sw_db_pmf_db_info_set(
            unit,
            stage,
            db_id,
            &sw_db_info
  ...
  )
{
  ...
  *key_alloced = 0;
  ...
  res = arad_sw_db_fp_db_info_get(
          unit,
          ARAD_FP_DATABASE_STAGE_INGRESS_PMF, /* All stages equivalent in use */
          db_id,
          &fp_database_info
        );
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
      ...
      for(alloc_exact = 1; alloc_exact >= 0; --alloc_exact) {
          for (cycle = cycle_first; cycle <= cycle_last; ++cycle) {
              ...
              res = arad_pp_fp_key_alloc_key_id(
                       ...
                       flags|ARAD_PP_FP_KEY_ALLOC_KEY_CHECK_ONLY, /* Check-only: commit in case of success */
                       &key_id[0],
                       &success
                    );
              SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
              ...
              for(ce_index = 0; ce_index  < nof_ce_const; ++ce_index) {
                  ...
                  /* if fail, to allocate in CE-16 try to use CE-32 if enabled */
                  if(!success  && 
                        ((!(ce_cons[ce_index].size_cons & ARAD_PP_FP_KEY_CE_SIZE_32))
                         || (ce_cons[ce_index].qual_type == SOC_PPC_FP_QUAL_IRPP_KEY_CHANGED))) {
                      ...
                      res = arad_pp_fp_key_alloc_ce(
                               ...
                               prog_ndx,
                               cycle,
                               ce_cons[ce_index].place_cons & alloc_place,
                               ARAD_PP_FP_KEY_CE_SIZE_32,
                  ...
                  if(success) {
                      ...
                      LOG_DEBUG(BSL_LS_SOC_FP,
                                (BSL_META_U(unit,
                                            "   "
                                            "program: %d cycle: %d CE-ID:%d alloced for qualifier:%s \n\r"), prog_ndx, cycle, ce_id, SOC_PPC_FP_QUAL_TYPE_to_string(ce_cons[ce_index].qual_type)));
                  }

                  /* if fail, to allocate in CE-32 try to use 2 of CE-16*/
                  if(is_split_enable && !success  && !(ce_cons[ce_index].size_cons & ARAD_PP_FP_KEY_CE_SIZE_16)) {
                      for(qual_break_index = 0; qual_break_index < 2; ++qual_break_index) {
                          res = arad_pp_fp_key_alloc_ce(
                                   unit,
                                   stage,
                                   prog_ndx,
  ...
  )
{
  ...
  *key_alloced = 0;
  ...
  if ((stage == SOC_PPC_FP_DATABASE_STAGE_INGRESS_FLP) && (!(flags & ARAD_PP_FP_KEY_ALLOC_CE_CHECK_ONLY))) {
      ...
      res = arad_sw_db_pgm_ce_rsrc_set(unit, stage, prog_ndx, cycle, &ce_rsrc);
  ...
  )
{
    uint32 
        ...
        res;
    uint8
        is_key_found_for_320b[ARAD_PP_FP_NOF_CYCLES];
...
STATIC
 uint32
  arad_pp_fp_key_progs_rsrc_stat(
    ...
    SOC_SAND_OUT  uint8                       cycle_needed[ARAD_PP_FP_NOF_CYCLES],
    SOC_SAND_OUT  uint32                      ce_rsrc[ARAD_PP_FP_NOF_CYCLES],
    SOC_SAND_OUT  uint32                      selected_cycle[ARAD_PMF_LOW_LEVEL_NOF_PROGS_ALL_STAGES],
    SOC_SAND_OUT  uint8                       key_zone_enough[ARAD_PP_FP_NOF_KEY_CE_PLACES][ARAD_PP_FP_NOF_CYCLES],
  ...
  )
{
  ...
  *flp_max_key_size_in_bits = 160; /* Used only in FLP stage */
  /* init the structure */
  sal_memset(key_zone_enough, 0x0, sizeof(uint8) * ARAD_PP_FP_NOF_KEY_CE_PLACES * ARAD_PP_FP_NOF_CYCLES);
        ...
        for(place_ndx = 0; place_ndx < ARAD_PP_FP_NOF_KEY_CE_PLACES_STAGE; place_ndx++) {
                 ...
                 || (is_high_and_low_diff_fgs_allowed && Arad_pp_fp_place_const_convert[place_ndx][5])))
            {
                if(stage == ARAD_FP_DATABASE_STAGE_INGRESS_FLP)
                {
                     ...
                     */
                    res = arad_pp_flp_elk_prog_config_max_key_size_get(unit, cur_prog, &max_key_size_in_bits);
                ...
                else
                {
                    ...
                    res = arad_pp_fp_key_alloc_key_id(
                             ...
                             ARAD_PP_FP_KEY_ALLOC_KEY_CHECK_ONLY, /* Check-only */
                             &key_id[0],
                             &success
                          );
                    SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
...
uint32
  arad_pp_fp_qual_ce_mltpl_info_get(
      ...
      SOC_SAND_IN  ARAD_FP_DATABASE_STAGE       stage,
	  SOC_SAND_IN  uint32                       flags,
      SOC_SAND_INOUT ARAD_PP_FP_QUAL_TYPE       qual_types[ARAD_PP_FP_NOF_QUALS_PER_DB_MAX],
      SOC_SAND_OUT uint32                       *nof_quals,
      SOC_SAND_OUT ARAD_PP_FP_QUAL_CE_INFO      qual_ce_info[ARAD_PP_FP_NOF_QUALS_PER_DB_MAX]
  )
{
  uint8 
      success;
  ...
  *nof_quals = 0;
  ...
  for(qual_indx = 0; (qual_indx < ARAD_PP_FP_NOF_QUALS_PER_DB_MAX) && (qual_types[qual_indx]!= ARAD_PP_NOF_FP_QUAL_TYPES); ++qual_indx)
  {
    /*
 ...
 * 2. Sort first the LSB-only and MSB-only qualifiers, since their location is known 
 * 3. Then sort per qualifier size (regardless the lost bits) 
 *  
 * Special case for FLP: 
...
uint32 
    ...
    SOC_SAND_INOUT ARAD_PP_FP_QUAL_TYPE     qual_types[ARAD_PP_FP_NOF_QUALS_PER_DB_MAX],
	SOC_SAND_INOUT ARAD_PP_FP_QUAL_CE_INFO  qual_ce_info[ARAD_PP_FP_NOF_QUALS_PER_DB_MAX]
	)
{
  uint32 
      qual_idx,
  ...
  ARAD_PP_FP_QUAL_TYPE qual_tmp;

  SOC_SAND_INIT_ERROR_DEFINITIONS_NO_DEVID(0);

  for(swap_idx = 0; swap_idx < ARAD_PP_FP_NOF_QUALS_PER_DB_MAX; swap_idx++) {
    swaped = FALSE;  
        ...
        if ((!qual_ce_info[qual_idx].is_lsb) && (!qual_ce_info[qual_idx].is_msb)) {
           LOG_ERROR(BSL_LS_SOC_FP,
                     (BSL_META_U(unit,
                      ...
                      ARAD_PP_FP_QUAL_TYPE_to_string(qual_types[qual_idx])));
  ...
  )
{
  uint32   
    ...
    total_ce_indx,
    prog_ndx,
      flp_max_key_size_in_bits,
    place_start_lcl, tmp_start_place,
    ce_rsrc_bmp_glbl_lcl[ARAD_PP_FP_NOF_CYCLES], 
  ...
  uint8
      ...
      is_high_group,
      is_key_not_found,
      key_zone_enough[ARAD_PP_FP_NOF_KEY_CE_PLACES][ARAD_PP_FP_NOF_CYCLES],
      direct_ext_2_needed, 
      cycle_needed[ARAD_PP_FP_NOF_CYCLES], 
      zone_used[ARAD_PP_FP_KEY_NOF_ZONES];
  ARAD_PP_FP_QUAL_TYPE          
      new_qual_types[ARAD_PP_FP_NOF_QUALS_PER_DB_MAX];
  ARAD_PP_FP_QUAL_CE_INFO       
      *ce_info = NULL;

  SOC_SAND_INIT_ERROR_DEFINITIONS(0);

  sal_memcpy(new_qual_types, qual_types, sizeof(ARAD_PP_FP_QUAL_TYPE) * ARAD_PP_FP_NOF_QUALS_PER_DB_MAX);
  ARAD_ALLOC(ce_info, ARAD_PP_FP_QUAL_CE_INFO, ARAD_PP_FP_NOF_QUALS_PER_DB_MAX, "arad_pp_fp_key_alloc_constrain_calc.ce_info");
  sal_memset(ce_info, 0x0, sizeof(ARAD_PP_FP_QUAL_CE_INFO) * ARAD_PP_FP_NOF_QUALS_PER_DB_MAX);

  /* get relevant programs for the DB */
  res = arad_sw_db_pmf_db_prog_get(
          unit,
          stage,
          db_id,
          &exist_progs
  ...
  *found = TRUE;
  ...
  for (cycle_idx = cycle_base; cycle_idx < (cycle_base + nof_cycles); cycle_idx++) {
      ...
      if (nof_cycles == 2)
      {
         ...
         * Qualifiers are plcaes by key such that a construction of 80 bit key will be attempted 
        ...
        for (direct_ext_loop_indx = 0; (direct_ext_loop_indx < (1 + is_for_direct_extraction)) && direct_ext_2_needed; direct_ext_loop_indx++) {
            *found = TRUE;
            ...
            for(zone_idx = 0; zone_idx < ARAD_PP_FP_KEY_NOF_ZONES; zone_idx++)
            {
              ...
              /* For Direct extraction, start from High to Low - always single key */
            ...
            for(qual_indx = 0;  (qual_indx < nof_quals) && (*found); ++qual_indx) 
            {
              ...
              /* 
               * Verify that contraints are met: for each zone that was used, 
              ...
              for(zone_idx = 0; zone_idx < ARAD_PP_FP_KEY_NOF_ZONES; zone_idx++)
              {
                ...
                */
                else if ((place_cons == ARAD_PP_FP_KEY_CE_PLACE_ANY_NOT_DOUBLE) && (zone_id > ARAD_PP_FP_KEY_ZONE_MSB_0)) 
                {
                ...
                else if ((stage == SOC_PPC_FP_DATABASE_STAGE_INGRESS_FLP) && (flp_max_key_size_in_bits < total_bits_in_zone[zone_id])) {
                    ...
                    LOG_DEBUG(BSL_LS_SOC_FP,
                              (BSL_META_U(unit,
                               ...
                               db_id, flp_max_key_size_in_bits, total_bits_in_zone[zone_id]));
                    *found = FALSE;
                    total_ce_indx = 0;
         ...
         * otherwize take the hardest constraint if necessary
         */
  ...
  )
{
    ...
    for(ce_indx = 0; ce_indx < db_prog_info.nof_ces; ++ce_indx) {
        ...
        ARAD_CLEAR(&sw_db_ce, ARAD_SW_DB_PMF_CE, 1);
        res = arad_sw_db_pgm_ce_instr_set(
                unit,
                stage,
                prog_ndx,
    ...
    nof_access_profiles = 0;
    if ((db_info.db_type != ARAD_PP_FP_DB_TYPE_DIRECT_EXTRACTION) && (db_info.db_type != ARAD_PP_FP_DB_TYPE_SLB)) {
...
uint32
  arad_pp_fp_key_value_buffer_mapping(
      ...
      SOC_SAND_INOUT  ARAD_PP_FP_QUAL_VAL      qual_vals[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX],
  ...
  )
{
    ...
    SOC_PPC_FP_QUAL_TYPE 
        qual_type_lcl = SOC_PPC_FP_QUAL_IRPP_INVALID;
    ARAD_PP_FP_DATABASE_INFO
        fp_database_info;
    uint8 is_vt_match = FALSE;
    uint8 is_fp_db_extended_fwrd = FALSE;
    uint8 is_fp_db_extended_ipv6_fwrd = FALSE;
    uint8 is_key_320b = 0;
    uint8 nof_ces_to_handle;
    ARAD_SW_DB_PMF_CE ce_array[ARAD_PMF_LOW_LEVEL_NOF_CE_IN_PROG_MAX_ALL_LEVELS];
    ...
    /* Get the database Info */
     ...
     if (fp_database_info.flags & SOC_PPC_FP_DATABASE_INFO_FLAGS_STAGE_VT )
     {
         is_vt_match = TRUE;
     }

     if (fp_database_info.flags & SOC_PPC_FP_DATABASE_INFO_FLAGS_EXTENDED_FWRD_DB )
     {
         is_fp_db_extended_fwrd = TRUE;         
     }

     if(fp_database_info.flags & SOC_PPC_FP_DATABASE_INFO_FLAGS_EXTENDED_IPV6_FWRD_DB)
     {
         is_fp_db_extended_ipv6_fwrd = TRUE;
     }
    /* 
     * Get all the information on this Database: 
     * - a PMF-Porgram it is present in 
     * - the Copy Engine attributes it is using 
     */

    if ( is_vt_match  ) /*Vlan translation classification*/
    {
        /*Build the CEs array*/
        res = arad_pp_fp_vt_match_key_ces_get(unit, fp_database_info.vt_classification_id, ce_array, &nof_ces_to_handle ); /*curently only 1 mode*/
        SOC_SAND_CHECK_FUNC_RESULT(res, 8, exit);
        is_key_320b = 0;
    }
    else if (is_fp_db_extended_fwrd || is_fp_db_extended_ipv6_fwrd) {
        /*Build the CEs array*/
        res = arad_pp_fp_kbp_match_key_ces_get(unit, is_fp_db_extended_ipv6_fwrd, ce_array, &nof_ces_to_handle); /*curently only 1 mode*/
        SOC_SAND_CHECK_FUNC_RESULT(res, 8, exit);
        is_key_320b = 0;
    }
    else
    {
        ...
        nof_ces_to_handle = db_prog_info.nof_ces;
    ...
    if (direction == ARAD_PP_FP_KEY_BUFFER_DIRECTION_BUFFER_TO_KEY) {
        for(ce_indx = 0; ce_indx < nof_ces_to_handle; ++ce_indx) {
            if ( is_vt_match || is_fp_db_extended_fwrd || is_fp_db_extended_ipv6_fwrd)
            {
                if(ce_indx >= ARAD_PMF_LOW_LEVEL_NOF_CE_IN_PROG_LSB) { /*we'll never enter here - just for consistentcy*/
                    buffer_to_key_nof_msb[ce_array[ce_indx].is_second_key] += ce_array[ce_indx].msb + 1;
                }
                else {
                    buffer_to_key_nof_lsb[ce_array[ce_indx].is_second_key] += ce_array[ce_indx].msb + 1;
                }
            }
            else
            {
               res = arad_sw_db_pgm_ce_instr_get(
                ...
                if(db_prog_info.ces[ce_indx] >= ARAD_PMF_LOW_LEVEL_NOF_CE_IN_PROG_LSB) {
                    buffer_to_key_nof_msb[sw_db_ce.is_second_key] += sw_db_ce.msb + 1;
                }
                else {
                    buffer_to_key_nof_lsb[sw_db_ce.is_second_key] += sw_db_ce.msb + 1;
                }
            }
        }
        ...
        for(ce_indx = 0; ce_indx < nof_ces_to_handle; ++ce_indx) {
            ...
            else
            {
                ...
                LOG_DEBUG(BSL_LS_SOC_FP,
                          (BSL_META_U(unit,
                                      "   "
                                      "Key value-buffer: direction:%d, ce-id:%d, type:%s \n\r"), direction, 
                           db_prog_info.ces[ce_indx], ARAD_PP_FP_QUAL_TYPE_to_string(sw_db_ce.qual_type)));

                if(db_prog_info.ces[ce_indx] >= ARAD_PMF_LOW_LEVEL_NOF_CE_IN_PROG_LSB && moved_to_msb == 0) {
            ...
            for(qual_index = 0; qual_index < ARAD_PP_FP_NOF_QUALS_PER_DB_MAX ; ++qual_index) {
                ...
                if (direction == ARAD_PP_FP_KEY_BUFFER_DIRECTION_KEY_TO_BUFFER) {
                    ...
                    if(qual_type_lcl == ARAD_PP_NOF_FP_QUAL_TYPES) {
                        continue;
                    }
                }
            ...
            if(!qual_found) {
                ...
                LOG_DEBUG(BSL_LS_SOC_FP,
                          (BSL_META_U(unit,
                                      "   "
                                      "qual:%s  qual is masked. skip to %d (%d bits)\n\r"), ARAD_PP_FP_QUAL_TYPE_to_string(qual_type_lcl), buf_msb, sw_db_ce.msb + 1));
                continue;
            }
            ...
            LOG_DEBUG(BSL_LS_SOC_FP,
                      (BSL_META_U(unit,
                                  ...
                                  "copied bits:%d\n\r"), sw_db_ce.lsb, buf_msb, qual_lsb[qual_index], sw_db_ce.qual_lsb, nof_copy_bits));
                ...
                if (direction == ARAD_PP_FP_KEY_BUFFER_DIRECTION_KEY_TO_BUFFER) {
                    ...
                    LOG_DEBUG(BSL_LS_SOC_FP,
                              (BSL_META_U(unit,
                                          ...
                                          "qual:%s, "
                                          "copied val:%08x, "
                                          "copied mask:%08x \n\r"), ARAD_PP_FP_QUAL_TYPE_to_string(qual_type), copied_val, copied_maks));
                }
                else if (direction == ARAD_PP_FP_KEY_BUFFER_DIRECTION_BUFFER_TO_KEY) {
                    ...
                    LOG_DEBUG(BSL_LS_SOC_FP,
                              (BSL_META_U(unit,
                                          ...
                                          "qual:%s, "
                                          "copied val:%08x, "
                                          "copied mask:%08x \n\r"), ARAD_PP_FP_QUAL_TYPE_to_string(sw_db_ce.qual_type), copied_val, copied_maks));
                }
        ...
        if (direction == ARAD_PP_FP_KEY_BUFFER_DIRECTION_KEY_TO_BUFFER) {
            ...
            if(buf_msb != ARAD_PP_FP_TCAM_ENTRY_SIZE*32) {
                ...
                if(nof_lsb[key_ndx] > 0) 
                {
                    ...
                    /* In FLP stage, need to stick to MSB (159:X) because the LSBs are not transmitted in the master key */
#if defined(INCLUDE_KBP) && !defined(BCM_88030)
                    if (stage == SOC_PPC_FP_DATABASE_STAGE_INGRESS_FLP) {
                        ...
                        } else if (is_fp_db_extended_ipv6_fwrd) {
...

            {
                uint32
                    print_idx;
                char
                    string_to_print[20*ARAD_PP_FP_TCAM_ENTRY_SIZE + 10] = "";
                LOG_DEBUG(BSL_LS_SOC_FP,
                          (BSL_META_U(unit,
                                      "   "
                                      "Buffer after shifting:\n\r")));
...
uint32
  arad_pp_fp_key_buffer_to_value(
      ...
      SOC_SAND_OUT  ARAD_PP_FP_QUAL_VAL      qual_vals_out[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX]
  )
{
    uint32
      res = SOC_SAND_OK;
    ...
    for(qual_val_ndx = 0; qual_val_ndx < SOC_PPC_FP_NOF_QUALS_PER_DB_MAX; ++qual_val_ndx) {
        ARAD_PP_FP_QUAL_VAL_clear(&qual_vals_out[qual_val_ndx]);
    }
  ...
  )
{
    ...
    sal_memcpy(qual_vals_lcl, qual_vals, SOC_PPC_FP_NOF_QUALS_PER_DB_MAX * sizeof(ARAD_PP_FP_QUAL_VAL));
  ...
  )
{
  ...
  info->lost_bits = 0;
  info->qual_lsb = 0;
  info->qual_type = SOC_PPC_NOF_FP_QUAL_TYPES;
