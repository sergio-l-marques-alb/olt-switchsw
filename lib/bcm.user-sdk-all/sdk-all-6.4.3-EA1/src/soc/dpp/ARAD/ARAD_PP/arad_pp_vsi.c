  ...
  )
{
  ...
  if (vsi_ndx < SOC_DPP_DEFS_GET(unit, nof_vsi_lowers)) {
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 27, exit, READ_EGQ_VSI_PROFILE_MEMORY_VSI_PROFILEm(unit, MEM_BLOCK_ANY, entry_index, entry_data));
    soc_EGQ_VSI_PROFILE_MEMORY_VSI_PROFILEm_field_get(unit, entry_data, VSI_PROFILE_DATAf, &vsi_profile_data_entry); 
    ...
    soc_EGQ_VSI_PROFILE_MEMORY_VSI_PROFILEm_field_set(unit, entry_data, VSI_PROFILE_DATAf, &vsi_profile_data_entry);
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 28, exit, WRITE_EGQ_VSI_PROFILE_MEMORY_VSI_PROFILEm(unit, MEM_BLOCK_ANY, entry_index, entry_data));
  ...
  )
{
  ...
  if (vsi_ndx < SOC_DPP_DEFS_GET(unit, nof_vsi_lowers)) {
    ...
    entry_index = vsi_ndx >> 4; /* each entry contains 16 vsi profiles */
    res = READ_EGQ_VSI_PROFILE_MEMORY_VSI_PROFILEm(unit, MEM_BLOCK_ANY, entry_index, whole_entry_data);
    SOC_SAND_CHECK_FUNC_RESULT(res, 26, exit);
    soc_EGQ_VSI_PROFILE_MEMORY_VSI_PROFILEm_field_get(unit, whole_entry_data, VSI_PROFILE_DATAf, &vsi_profile_data);   
  ...
  )
{
  ...
  SOC_REG_ABOVE_64_CLEAR(reg_above_64);
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 10, exit, READ_IHP_MEF_L_2_CP_PEER_BITMAPr(unit, reg_above_64));
  SHR_BITCOPY_RANGE(reg_above_64, index_ingress, &is_peer, 0, 1);
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 30, exit, WRITE_IHP_MEF_L_2_CP_PEER_BITMAPr(unit, reg_above_64));
  SOC_REG_ABOVE_64_CLEAR(reg_above_64);
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 40, exit, READ_IHP_MEF_L_2_CP_DROP_BITMAPr(unit, reg_above_64));
  SHR_BITCOPY_RANGE(reg_above_64, index_ingress, &is_drop, 0, 1);
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 50, exit, WRITE_IHP_MEF_L_2_CP_DROP_BITMAPr(unit, reg_above_64));
	  ...
	  *handle_type = SOC_PPD_VSI_L2CP_HANDLE_TYPE_TUNNEL;
	  ...
	  if (is_peer) {
		  *handle_type = SOC_PPD_VSI_L2CP_HANDLE_TYPE_PEER;
		  ...
		  SOC_REG_ABOVE_64_CLEAR(reg_above_64);
		  SOC_SAND_SOC_IF_ERROR_RETURN(res, 10, exit, READ_IHP_MEF_L_2_CP_DROP_BITMAPr(unit, reg_above_64));
