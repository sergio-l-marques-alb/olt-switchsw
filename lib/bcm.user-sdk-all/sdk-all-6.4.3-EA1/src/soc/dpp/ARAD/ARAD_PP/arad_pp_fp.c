...
#define ARAD_PP_FP_DE_ENTRY_ID_NDX_MAX                           (15)
  ...
  )
{
  ...
  if(!soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "egress_pmf_lookups_always_valid_disable", 0))
  {
      ...
      res = arad_sw_db_tcam_access_profile_valid_set(unit, access_profile_id, TRUE); /* Never freed */
  ...
  )
{
    ...
    ARAD_PP_FP_PFG_INFO
        pfg_info_current;
    uint32
        pfg_tm_bmp[ARAD_BIT_TO_U32(ARAD_PMF_NOF_GROUPS)],
        res,
        flags = 0,
        line_indx,
        min_indx, max_indx, 
  ...
  *success = SOC_SAND_SUCCESS;
  ...
  ARAD_ALLOC(curr_level, ARAD_SW_DB_PMF_PSL_LEVEL_INFO, 1, "arad_pp_fp_packet_format_group_set_unsafe.curr_level");
  ...
  if (soc_property_get(unit, spn_FIELD_PRESEL_MGMT_ADVANCED_MODE, FALSE)) {
      ...
      if (info->is_for_hw_commit) {
          ...
          if (info->is_array_qualifier) {
              ...
              SOC_SAND_CHECK_FUNC_RESULT(arad_sw_db_level_info_get(unit, info->stage, 1, ARAD_PMF_PSL_TYPE_ETH, curr_level), 27, exit);
  ...
  if (!info->is_array_qualifier) {
      ...
      if (is_pfg_tm) {
          ...
          res = arad_sw_db_pmf_psl_pfg_tms_get(unit, stage, pfg_tm_bmp);
          SOC_SAND_CHECK_FUNC_RESULT(res, 134, exit);
          SHR_BITCLR(pfg_tm_bmp, pfg_ndx);
          res = arad_sw_db_pmf_psl_pfg_tms_set(unit, stage, pfg_tm_bmp);
  ...
  )
{
  ...
  if (soc_property_get(unit, spn_FIELD_PRESEL_MGMT_ADVANCED_MODE, FALSE) ==0 ) 
  {
      ...
      if (!info->is_array_qualifier) {
          res = arad_sw_db_pmf_psl_pfg_dbs_get(unit, stage, pfg_ndx, pfg_db_pmb);
  ...
  )
{
    ...
    uint32
        pfg_ndx,
        default_db_pmb[ARAD_BIT_TO_U32(ARAD_SW_DB_PMF_NOF_DBS)],
        pfgs_db_pmb[ARAD_BIT_TO_U32(ARAD_SW_DB_PMF_NOF_DBS)],
        res;
    ...
    if (is_default_db
        ...
        res = arad_sw_db_pmf_psl_default_dbs_get(unit, stage, default_db_pmb);
        SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 11, exit);
        SHR_BITSET(default_db_pmb, db_id_ndx);
        res = arad_sw_db_pmf_psl_default_dbs_set(unit, stage, default_db_pmb);
        SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 13, exit);
    }
    else {
        for (pfg_ndx = 0; pfg_ndx < ARAD_PP_FP_NOF_PFGS; ++pfg_ndx) {
            res = arad_sw_db_pmf_psl_pfg_dbs_get(unit, stage, pfg_ndx, pfgs_db_pmb);
            SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 15, exit);
            if (SHR_BITGET(db_info->supported_pfgs_arad, pfg_ndx)) {
                SHR_BITSET(pfgs_db_pmb, db_id_ndx);
                res = arad_sw_db_pmf_psl_pfg_dbs_set(unit, stage, pfg_ndx, pfgs_db_pmb);
    ...
    sw_db_info.prio = db_info->strength;
    sw_db_info.valid = 1;
  ...
  )
{
    ...
    if (is_default_db) {
        res = arad_sw_db_pmf_psl_default_dbs_get(unit, stage, default_db_pmb);
        SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 11, exit);
        SHR_BITCLR(default_db_pmb, db_id_ndx);
        res = arad_sw_db_pmf_psl_default_dbs_set(unit, stage, default_db_pmb);
        SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 13, exit);
    }
    else {
        for (pfg_ndx = 0; pfg_ndx < ARAD_PP_FP_NOF_PFGS; ++pfg_ndx) {
            ...
            SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 15, exit);
            if (SHR_BITGET(db_info->supported_pfgs_arad, pfg_ndx)) {
                SHR_BITCLR(pfgs_db_pmb, db_id_ndx);
                res = arad_sw_db_pmf_psl_pfg_dbs_set(unit, stage, pfg_ndx, pfgs_db_pmb);
                SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 17, exit);
            }
        }
    ...
    /* Clear DB info */
    ARAD_CLEAR(&sw_db_info, ARAD_SW_DB_PMF_DB_INFO, 1);
    res = arad_sw_db_pmf_db_info_set(
            unit,
            stage,
            db_id_ndx,
    ...
    for (stage_lcl = 0; stage_lcl < ARAD_NOF_FP_DATABASE_STAGES; stage_lcl ++) {
      ...
      sw_db_fp_entry.nof_db_entries = 0;
      res = arad_sw_db_fp_db_entries_set(
              unit,
              stage,
              db_id_ndx,
              &sw_db_fp_entry
  ...
  for (pfg_long_ndx = 0; pfg_long_ndx < SOC_PPC_FP_NOF_PFGS_IN_LONGS_ARAD; pfg_long_ndx++) {
      for (pfg_bit_ndx = 0; pfg_bit_ndx < 32; pfg_bit_ndx++) {
          if (SHR_BITGET(&(info->supported_pfgs_arad[pfg_long_ndx]), pfg_bit_ndx)) {
              ...
              /* Check if it is TM */
              if (SHR_BITGET(pfg_tm_bmp, pfg_bit_ndx)) {
                  is_for_tm_lcl = TRUE;
              }
  ...
  )
{
  ...
  uint32
      db_id_other_ndx,
      offset_lsb,
      forbidden_db_bmp[ARAD_BIT_TO_U32(ARAD_SW_DB_PMF_NOF_DBS)],
  ...
  *success = SOC_SAND_FAILURE_OUT_OF_RESOURCES;
  ...
  if ((info->flags & SOC_PPC_FP_DATABASE_INFO_FLAGS_EXTENDED_FWRD_DB) || (info->flags & SOC_PPC_FP_DATABASE_INFO_FLAGS_EXTENDED_IPV6_FWRD_DB)) {
      is_fp_db_extended_fwrd = TRUE;
  }

  sal_memset(selected_cycle, 0x0, sizeof(uint32) * ARAD_PMF_LOW_LEVEL_NOF_PROGS_ALL_STAGES);
  sal_memset(forbidden_db_bmp, 0x0, sizeof(uint32) * ARAD_BIT_TO_U32(ARAD_SW_DB_PMF_NOF_DBS));
  

  if ( info->flags & SOC_PPC_FP_DATABASE_INFO_FLAGS_STAGE_VT )
  {
         is_vt_match = TRUE;
  }
  /*
  ...
  if ( is_vt_match || is_fp_db_extended_fwrd )
  {
      *success = SOC_SAND_SUCCESS;
      ARAD_DO_NOTHING_AND_EXIT;
  }
  ...
  res = arad_sw_db_pmf_db_prog_set(
            unit,
            stage,
            db_id_ndx,
            pmf_pgm_bmp_used
 ...
 /* Check if there is a key-change action or qualifier */
  ...
  else if(info->db_type == ARAD_PP_FP_DB_TYPE_FLP) {
    /* 
    ...
    */
       ...
       && (info->db_type != ARAD_PP_FP_DB_TYPE_SLB))
    {
        ...
        if(info->db_type != ARAD_PP_FP_DB_TYPE_DIRECT_TABLE) {
            res = arad_sw_db_pmf_psl_default_dbs_get(unit, stage, forbidden_db_bmp);
        }
...
exit_tm:
  *success = SOC_SAND_SUCCESS;
  ...
  else 
  {
    ...
    ARAD_ALLOC(fes_info, ARAD_SW_DB_PMF_FES, ARAD_PMF_LOW_LEVEL_NOF_FESS, "arad_pp_fp_database_create_unsafe.fes_info");
    for (pmf_pgm_ndx = pmf_pgm_ndx_min; pmf_pgm_ndx < pmf_pgm_ndx_max; ++pmf_pgm_ndx)
    {
        if (SOC_SAND_GET_BIT(pmf_pgm_bmp_used, pmf_pgm_ndx) != 0x1){
            /* PMF Program not used */
        ...
        res = arad_sw_db_pmf_psl_pgm_dbs_get(unit, stage, pmf_pgm_ndx_duplicate, pgms_db_pmb);
        SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 230, exit);
        SHR_BITSET(pgms_db_pmb, db_id_ndx);
        res = arad_sw_db_pmf_psl_pgm_dbs_set(unit, stage, pmf_pgm_ndx_duplicate, pgms_db_pmb);
  ...
  )
{
   ...
   */
     ...
     */
        ...
        if ((stage != ARAD_FP_DATABASE_STAGE_INGRESS_FLP) && (stage != ARAD_FP_DATABASE_STAGE_INGRESS_SLB)) { /* do not check for FLP */
  ...
  )
{
  uint32
    tcam_db_id,
      ...
      pmf_pgm_ndx_max,
      pgms_db_pmb[ARAD_BIT_TO_U32(ARAD_SW_DB_PMF_NOF_DBS)],
      pgms_db_pmb_new[ARAD_BIT_TO_U32(ARAD_SW_DB_PMF_NOF_DBS)],
    res = SOC_SAND_OK;
  ARAD_PP_FP_DATABASE_INFO
    fp_database_info;
  ARAD_SW_DB_FP_ENTRY
  ...
  {
      res = arad_sw_db_fp_db_entries_get(
              unit,
              stage,
              db_id_ndx,
              &sw_db_fp_entry
      ...
      SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
      if (sw_db_fp_entry.nof_db_entries)
      {
      LOG_ERROR(BSL_LS_SOC_FP,
                (BSL_META_U(unit,
  ...
  res = arad_sw_db_pmf_db_prog_get(unit, stage, db_id_ndx, &pmf_pgm_bmp_used);
  SOC_SAND_CHECK_FUNC_RESULT(res, 50, exit);

  res = arad_sw_db_pgm_program_rsrc_get(unit, stage, &prog_rsrc);
  SOC_SAND_CHECK_FUNC_RESULT(res, 60, exit);
  ...
  for (pmf_pgm_ndx_used = pmf_pgm_ndx_min; pmf_pgm_ndx_used < pmf_pgm_ndx_max; ++pmf_pgm_ndx_used)
  {
      ...
      res = arad_sw_db_pmf_psl_pgm_dbs_get(unit, stage, pmf_pgm_ndx_used, pgms_db_pmb);
      SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 84, exit);
      SHR_BITCLR(pgms_db_pmb, db_id_ndx);
      res = arad_sw_db_pmf_psl_pgm_dbs_set(unit, stage, pmf_pgm_ndx_used, pgms_db_pmb);
      ...
      for (pmf_pgm_ndx_new = pmf_pgm_ndx_min; pmf_pgm_ndx_new < pmf_pgm_ndx_max; ++pmf_pgm_ndx_new)
      {
          ...
          res = arad_sw_db_pmf_psl_pgm_dbs_get(unit, stage, pmf_pgm_ndx_new, pgms_db_pmb_new);
          SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 112, exit);
          if(sal_memcmp(pgms_db_pmb_new, pgms_db_pmb, sizeof(pgms_db_pmb)) == 0) {
              /* Update the Preselector line's PMF program at SW DB and HW */
...
exit_without_field_hardware_update:
    ...
    if ((fp_database_info.db_type == ARAD_PP_FP_DB_TYPE_TCAM) 
        ...
        || (fp_database_info.db_type == ARAD_PP_FP_DB_TYPE_EGRESS))
    {
      tcam_db_id = ( is_vt_match ) ? (ARAD_PP_ISEM_ACCESS_VT_CLASSIFICATIONS_TCAM_BASE + fp_database_info.vt_classification_id) :   ARAD_PP_FP_DB_ID_TO_TCAM_DB(db_id_ndx);
      res = arad_tcam_access_destroy_unsafe(
              unit,
              tcam_db_id
            );
  ...
  )
{
  ...
  {
    res = arad_sw_db_tcam_db_bank_used_get(
            unit, 
            ARAD_PP_FP_DB_ID_TO_TCAM_DB(db_id_ndx), 
            bank_ndx,
            &is_used
  ...
  )
{
  ...
  if(fp_database_info.db_type == ARAD_PP_FP_DB_TYPE_DIRECT_TABLE) 
  {
    ...
    if(0 == sw_db_fp_entry.nof_db_entries)
    {
        /* allocate bank */
    ...
    res = arad_sw_db_tcam_db_action_bitmap_ndx_get(
            unit,
            ARAD_PP_FP_DB_ID_TO_TCAM_DB(db_id_ndx),
            &action_bitmap_ndx
          );
  ...
  else 
  {
    ...
    /* 
     * Different insertion procedures for internal and external TCAMs 
     */
    if (stage == SOC_PPC_FP_DATABASE_STAGE_INGRESS_FLP) {
#if defined(INCLUDE_KBP) && !defined(BCM_88030)
        ...
        /* Initialize the key */
        sal_memset(&route_key, 0x0, sizeof(route_key));
        route_key.type = ARAD_IP_TCAM_ENTRY_TYPE_KBP_FIRST_ACL + db_id_ndx; /* Use NOF for the ACL table id */
        route_key.key.elk_fwd.priority = info->priority;

        if (is_fp_db_extended_fwrd) {
          frwrd_table_id = ARAD_KBP_FRWRD_TBL_ID_LSR_IP_SHARED;
          route_key.type = ARAD_IP_TCAM_FROM_KBP_FRWRD_IP_TBL_ID(frwrd_table_id);
        } else if (is_fp_db_extended_ipv6_fwrd){
            frwrd_table_id = ARAD_KBP_FRWRD_TBL_ID_EXTENDED_IPV6;
            route_key.type = ARAD_IP_TCAM_FROM_KBP_FRWRD_IP_TBL_ID(frwrd_table_id);
        } else {
          /* Encode the buffer according to KBP constraints */
          frwrd_table_id = (ARAD_KBP_ACL_TABLE_ID_OFFSET + db_id_ndx);          
        }
        ...
        res = arad_pp_frwrd_ip_tcam_route_to_kbp_payload_buffer_encode(
                unit,
                table_payload_in_bytes,
                action.value, /* limited to 96b */
                action.elk_ad_value);
        SOC_SAND_CHECK_FUNC_RESULT(res,  76, exit);

        /* Insertion in KBP */
        if (is_fp_db_extended_fwrd || is_fp_db_extended_ipv6_fwrd) {
          route_key.key.elk_fwd.priority = entry_id_ndx;/* only in those cases the priority saves the entry ID */
          res = arad_pp_frwrd_ip_tcam_route_add_unsafe(unit,
                                                       &route_key,
                                                       &action,
                                                       success);
        }
    ...
    res = arad_pp_frwrd_ip_tcam_route_kbp_compare(
        ...
        /* No use fo FLP stage without KBP */
    ...
    { /* internal TCAM */
        ...
        /* Indicate if it is for update */
        entry.is_for_update = info->is_for_update;

        /* Indicate if it is for top of bank or middle */
  ...
  if ((*success == SOC_SAND_SUCCESS) && (!entry.is_for_update))
  {
     ...
     * In TCAM, a verification is done in lower-level 
     */
    res = arad_sw_db_fp_db_entries_get(
            unit,
            stage,
  ...
  )
{
  ...
  else
  {
     ...
     if (is_fp_db_extended_fwrd || is_fp_db_extended_ipv6_fwrd)
     {
        ...
        uint32 logical_entry_size_in_bytes, table_payload_in_bytes, tbl_id;
        tbl_id = (is_fp_db_extended_fwrd == 1)? ARAD_KBP_FRWRD_TBL_ID_LSR_IP_SHARED:ARAD_KBP_FRWRD_TBL_ID_EXTENDED_IPV6;
        soc_sand_os_memset(&key, 0x0, sizeof(key));
        res = arad_kbp_table_size_get(unit, tbl_id, &logical_entry_size_in_bytes, &table_payload_in_bytes);
        SOC_SAND_CHECK_FUNC_RESULT(res,  71, exit);
        key.type = ARAD_IP_TCAM_FROM_KBP_FRWRD_IP_TBL_ID(tbl_id); 

        /* map key qualifiers to tcam buffer */
        res = arad_pp_fp_key_value_to_buffer(unit, stage, entry_info.qual_vals,db_id_ndx,entry.value,entry.mask);
        ...
        res = arad_pp_frwrd_ip_tcam_entry_hw_get(unit, &key, &action ,&found);
     }
     else
#endif /*INCLUDE_KBP */
     {
       if ( is_vt_match )
       {
          tcam_db_id = (ARAD_PP_ISEM_ACCESS_VT_CLASSIFICATIONS_TCAM_BASE + fp_database_info.vt_classification_id);
          tcam_key_type = ARAD_IP_NOF_TCAM_ENTRY_TYPES;
       }
       else
       {
  ...
  )
{
...
#if defined(INCLUDE_KBP) && !defined(BCM_88030)    
    if ((fp_database_info.flags & SOC_PPC_FP_DATABASE_INFO_FLAGS_EXTENDED_FWRD_DB) || (fp_database_info.flags & SOC_PPC_FP_DATABASE_INFO_FLAGS_EXTENDED_IPV6_FWRD_DB)) {
      ARAD_IP_TCAM_ENTRY_TYPE entry_type = ((fp_database_info.flags & SOC_PPC_FP_DATABASE_INFO_FLAGS_EXTENDED_FWRD_DB)? 
        ARAD_IP_TCAM_FROM_KBP_FRWRD_IP_TBL_ID(ARAD_KBP_FRWRD_TBL_ID_LSR_IP_SHARED): ARAD_IP_TCAM_FROM_KBP_FRWRD_IP_TBL_ID(ARAD_KBP_FRWRD_TBL_ID_EXTENDED_IPV6));
    ...
    if (fp_database_info.db_type == ARAD_PP_FP_DB_TYPE_DIRECT_TABLE) {
        if (0 == sw_db_fp_entry.nof_db_entries) {
            /* allocate bank */
            res = arad_tcam_managed_db_direct_table_bank_remove(
               unit,
               ARAD_PP_FP_DB_ID_TO_TCAM_DB(db_id_ndx),
               &success
               );
  ...
  )
{
  ...
  if ( (!is_large_direct_extraction) && (!use_fes)) {
      ...
      *success = SOC_SAND_SUCCESS;
       ...
       */ 
      place_start = ARAD_PP_FP_KEY_CE_HIGH;
      key_alloc_ce_flag = ( is_large_direct_extraction) ? ARAD_PP_FP_KEY_ALLOC_CE_USE_QUAL_ORDER : ARAD_PP_FP_KEY_ALLOC_CE_SINGLE_MAPPING ; 
      res = arad_pp_fp_key_alloc_constrain_calc(
                unit,
                stage,
                db_id_ndx,
                ARAD_PP_FP_KEY_ALLOC_CHECK_ONLY | key_alloc_ce_flag,
                qual_types,
      ...
      res = arad_pp_fp_action_alloc(
              unit,
              db_id_ndx,
              ARAD_PP_FP_FEM_ALLOC_FES_CHECK_ONLY,
              fp_action_type,
      ...
      res = arad_sw_db_pmf_db_prog_get(unit, stage, db_id_ndx, &exist_progs);
          ...
          if (is_large_direct_extraction) {
            ...
            LOG_ERROR(BSL_LS_SOC_FP,
             ...
             SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 200, exit);
              ...
              /* Allocate the Action */
              action_const.tcam_res_id[0] = sw_db_info.used_key[pmf_pgm_ndx][0];
              action_const.tcam_res_id[1] = (sw_db_info.alloc_place == ARAD_PP_FP_KEY_CE_HIGH)? 80: 0; /* Which LSB bit */
    ...
    for (cycle_ndx = 0; (cycle_ndx < ARAD_PMF_NOF_CYCLES) && (fes_is_found == FALSE); ++cycle_ndx)
    {
      ...
      for(pgm_idx = pgm_idx_min; pgm_idx < pgm_idx_max; pgm_idx++)
      {
        for(fes_group_idx = 0; fes_group_idx < ARAD_PMF_LOW_LEVEL_NOF_FESS_PER_GROUP; fes_group_idx++)
        {
          ...
          sal_memset(&fes_info, 0x0, sizeof(ARAD_SW_DB_PMF_FES));
     ...
     if (fes_is_found) {
       ...
       *is_fes = TRUE;
        ...
        info->actions[0].type = fp_database_info.action_types[0];
  }
  ...
  )
{
    ...
    case ARAD_PP_FP_CONTROL_TYPE_EGR_PP_PORT_DATA:
        port_table = EGQ_PACKET_PROCESSING_PER_PORT_CONFIGURATION_TABLE_PP_PPCTm;
        break;
  ...
  )
{
  ...
  *success = SOC_SAND_SUCCESS;
  ...
  case ARAD_PP_FP_CONTROL_TYPE_ING_TM_PORT_DATA:
      ...
      port_table = arad_pp_fp_port_table_get(unit, control_ndx->type);
      res = soc_mem_read(unit, port_table, MEM_BLOCK_ANY, control_ndx->val_ndx, tbl_data);
      SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 17, exit);
      fld_data[0] = info->val[0];
      soc_mem_field_set(unit, port_table, tbl_data, arad_pp_fp_port_field_get(unit, control_ndx->type), fld_data);
      res = soc_mem_write(unit, port_table, MEM_BLOCK_ANY, control_ndx->val_ndx, tbl_data);
  ...
  case ARAD_PP_FP_CONTROL_TYPE_ACE_POINTER_PP_PORT:
      sal_memset(tbl_data, 0x0, ARAD_PP_FP_KEY_GEN_VAR_MAX_IN_LONGS * sizeof(uint32));
      /* Set / get the table field */
      res = soc_mem_read(unit, EPNI_ACE_TO_OUT_PP_PORTm, MEM_BLOCK_ANY, control_ndx->val_ndx / ARAD_PP_FP_NOF_ACE_POINTERS_IN_LINE, tbl_data);
      SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 20, exit);
      ...
      res = soc_mem_write(unit, EPNI_ACE_TO_OUT_PP_PORTm, MEM_BLOCK_ANY, control_ndx->val_ndx / ARAD_PP_FP_NOF_ACE_POINTERS_IN_LINE, tbl_data);
  ...
  case ARAD_PP_FP_CONTROL_TYPE_INNER_ETH_NOF_VLAN_TAGS:
    res = arad_sw_db_fp_inner_eth_nof_tags_set(
      unit,
      ARAD_FP_DATABASE_STAGE_INGRESS_PMF,
      control_ndx->val_ndx,
      info->val[0]
  ...
  case ARAD_PP_FP_CONTROL_TYPE_KEY_CHANGE_SIZE:
    res = arad_sw_db_fp_key_change_size_set(
      unit,
      ARAD_FP_DATABASE_STAGE_INGRESS_PMF,
      (uint8)   info->val[0]
      );
    ...
    for(profile_object_idx = 0; profile_object_idx < ARAD_PP_FP_NOF_PROFILE_OBJECTS(unit, control_ndx->type); profile_object_idx++)
    {
      ...
      if(control_ndx->clear_val || (SHR_BITGET(info->val, profile_object_idx)))
      {
         ...
         */
          switch (control_ndx->type) {
          case SOC_PPC_FP_CONTROL_TYPE_IN_PORT_PROFILE:
            res = arad_ihb_pinfo_pmf_tbl_get_unsafe(unit, profile_object_idx, &pinfo_tbl_data);
            SOC_SAND_CHECK_FUNC_RESULT(res, 110, exit);
            profile_id = pinfo_tbl_data.port_pmf_profile;
            break;
          case SOC_PPC_FP_CONTROL_TYPE_OUT_PORT_PROFILE:
              res = arad_pp_egq_pp_ppct_tbl_get_unsafe(unit, profile_object_idx, &pp_ppct_tbl_data);
              SOC_SAND_CHECK_FUNC_RESULT(res, 130, exit);
           ...
           || (profile_id == control_ndx->val_ndx))
        {
            ...
            case SOC_PPC_FP_CONTROL_TYPE_IN_PORT_PROFILE:
                ...
                res = arad_ihb_pinfo_pmf_tbl_set_unsafe(unit, profile_object_idx, &pinfo_tbl_data);
                SOC_SAND_CHECK_FUNC_RESULT(res, 120, exit);
              break;
            case SOC_PPC_FP_CONTROL_TYPE_OUT_PORT_PROFILE:
                pp_ppct_tbl_data.pmf_profile = profile_id;
                res = arad_pp_egq_pp_ppct_tbl_set_unsafe(unit, profile_object_idx, &pp_ppct_tbl_data);
  ...
  case SOC_PPC_FP_CONTROL_TYPE_KBP_CACHE:
    /* set KBP cache value */
    res = arad_sw_db_frwrd_ip_kbp_cache_mode_set(unit, info->val[0]);
    SOC_SAND_CHECK_FUNC_RESULT(res, 150, exit)
    break;
  ...
  )
{
  ...
  case ARAD_PP_FP_CONTROL_TYPE_OUT_LIF_RANGE:
    ...
    break;
  case ARAD_PP_FP_CONTROL_TYPE_INNER_ETH_NOF_VLAN_TAGS:
    res = arad_sw_db_fp_inner_eth_nof_tags_get(
            unit,
            ARAD_FP_DATABASE_STAGE_INGRESS_PMF,
            control_ndx->val_ndx,
            &(info->val[0])
  ...
  case ARAD_PP_FP_CONTROL_TYPE_KEY_CHANGE_SIZE:
    res = arad_sw_db_fp_key_change_size_get(unit, ARAD_FP_DATABASE_STAGE_INGRESS_PMF, &key_change_size);
  ...
  case SOC_PPC_FP_CONTROL_TYPE_KBP_CACHE:
    /* set KBP cache value */
    res = arad_sw_db_frwrd_ip_kbp_cache_mode_get(unit, &info->val[0]);
    SOC_SAND_CHECK_FUNC_RESULT(res, 140, exit);
    break;
#endif /* defined(INCLUDE_KBP) && !defined(BCM_88030) */
  default:
...
exit_ingress_pmf:
  ...
  for (db_id_ndx = 0; db_id_ndx < ARAD_PP_FP_NOF_DBS; ++db_id_ndx)
  {
    ...
    cycle_db_id = SHR_BITGET(&sw_db_info.prog_used_cycle_bmp, pmf_pgm_ndx) ? 1 : 0;
    key_ndx = sw_db_info.used_key[pmf_pgm_ndx][0];

    /* Key: map from tcam buffer to key qualifiers */
    SHR_BITCOPY_RANGE(data_in, 0, key_built[cycle_db_id][key_ndx], 0, 160); 
    is_320b = (sw_db_info.used_key[pmf_pgm_ndx][1] == sw_db_info.used_key[pmf_pgm_ndx][0] + 1)? 1: 0; 
    if (is_320b) { 
        key_ndx = sw_db_info.used_key[pmf_pgm_ndx][1];
    ...
    for (key_id = 0; key_id < 2; key_id++) {
        ...
        key_ndx = sw_db_info.used_key[pmf_pgm_ndx][key_id];
   ...
   * 6. FEM - FES
   */
  ARAD_ALLOC(fes_info, ARAD_SW_DB_PMF_FES, ARAD_PMF_LOW_LEVEL_NOF_FESS, "arad_pp_fp_packet_diag_get_unsafe.fes_info");
  /* 
  ...
  for (cycle_ndx = 0; cycle_ndx < ARAD_PMF_NOF_CYCLES; ++cycle_ndx)
  {
      for (fem_macro_ndx = 0; fem_macro_ndx < SOC_PPC_FP_NOF_MACRO_SIMPLES; ++fem_macro_ndx)
      {
          ...
          if (fes_info[fes_ndx].is_used == TRUE) {
              info->macro_simple[cycle_ndx][fem_macro_ndx].db_id = fes_info[fes_ndx].db_id;
  ...
  for (qual_index = 0; qual_index < ARAD_PP_FP_EGRESS_NOF_CAM_PARAMS; qual_index++) {
      ...
      if (reg_fld.base != 0) {
          ...
          case 0:
              ...
              res = arad_pp_egq_pp_ppct_tbl_get_unsafe(unit, pp_port_ndx, &egq_pp_ppct_tbl);
        ...
        if (is_found) {
            ...
            if (reg_fld.base != 0) {
                ...
                if (qual_info.info.irpp_field == SOC_PPC_FP_QUAL_ERPP_OUT_PP_PORT_PMF_DATA) {
                    res = arad_pp_egq_pp_ppct_tbl_get_unsafe(unit, val[0], &egq_pp_ppct_tbl);
                    ...
                    res = arad_egq_ppct_tbl_get_unsafe(unit, val[0], 0, &egq_ppct_tbl);
  ...
  )
{
  uint32
    ...
    *key_cur_loc = NULL,
    diag_ce_bitmap[SOC_PPC_NOF_FP_DATABASE_STAGES][ARAD_SW_DB_PMF_NOF_PROGS][ARAD_SW_DB_PMF_NOF_CYCLES],
    diag_key_bitmap[SOC_PPC_NOF_FP_DATABASE_STAGES][ARAD_SW_DB_PMF_NOF_PROGS][ARAD_SW_DB_PMF_NOF_CYCLES],
    diag_fes_bitmap[SOC_PPC_NOF_FP_DATABASE_STAGES][ARAD_SW_DB_PMF_NOF_PROGS],
    nof_entries;
  ...
  sal_memset(diag_key_bitmap, 0x0, sizeof(uint32) * SOC_PPC_NOF_FP_DATABASE_STAGES * ARAD_SW_DB_PMF_NOF_PROGS * ARAD_SW_DB_PMF_NOF_CYCLES);
  value_idx = 0;
  ...
  for(db_idx = 0; db_idx < ARAD_PP_FP_NOF_DBS; db_idx++)
  {
    ...
    sal_memset(&pmf_db_info, 0x0, sizeof(ARAD_SW_DB_PMF_DB_INFO));
    res = arad_sw_db_pmf_db_info_get(unit, info->db[db_idx].stage, db_idx, &pmf_db_info);
    SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit); 
    ...
    if(info->db[db_idx].valid)
    {
      ...
      res = arad_sw_db_fp_db_info_get(unit, info->db[db_idx].stage, db_idx, &fp_db_info);
      SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit); 
      ...
      res = arad_sw_db_pmf_db_prio_get(
              unit, 
              stage, 
              db_idx, 
              &db_priority
      ...
      for(bank_id = 0; bank_id < SOC_DPP_DEFS_GET_TCAM_NOF_BANKS(unit); bank_id++)
      {
        if(! (((stage == SOC_PPC_FP_DATABASE_STAGE_INGRESS_FLP) || (stage == SOC_PPC_FP_DATABASE_STAGE_INGRESS_SLB)) 
             && (bank_id == 0)) ) {
            res = arad_sw_db_tcam_db_bank_used_get(unit, tcam_db_idx, bank_id, &is_used);
            SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
        }
           ...
           || is_used)
        {
             ...
             || info->db[db_idx].type == SOC_PPC_FP_DB_TYPE_EGRESS)
          {
             ...
             */
            res = arad_sw_db_tcam_db_entry_size_get(unit,tcam_db_idx, &entry_size);
            SOC_SAND_CHECK_FUNC_RESULT(res, 25, exit);
            ...
            {
              res = arad_sw_db_tcam_db_access_profile_id_get(unit, tcam_db_idx, key_idx, &profile_idx);
              SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
             ...
             || (info->db[db_idx].type == SOC_PPC_FP_DB_TYPE_FLP))
          {
                ...
                info->db[db_idx].db_tcam.bank[bank_id].entries_used = sw_db_fp_entry.nof_db_entries;
                info->db[db_idx].db_tcam.bank[bank_id].entries_free = 0;
          }
          else
          {
                /* bank information per tcam db - only for TCAM */
                res = arad_sw_db_tcam_bank_nof_entries_free_get(
                        unit,
                        bank_id,
  ...
  sal_memset(diag_ce_bitmap, 0x0, sizeof(uint32) * SOC_PPC_NOF_FP_DATABASE_STAGES * ARAD_SW_DB_PMF_NOF_PROGS * ARAD_SW_DB_PMF_NOF_CYCLES);
  ...
  ARAD_ALLOC(db_used_program, uint32, ARAD_SW_DB_PMF_NOF_DBS, "arad_pp_fp_resource_diag_get_unsafe.db_used_program");
  for(db_idx = 0; db_idx < ARAD_PP_FP_NOF_DBS; db_idx++)
  {
      db_used_program[db_idx] = ARAD_SW_DB_PMF_NOF_PROGS;
      ...
      for(cycle_idx = 0; cycle_idx < ARAD_PMF_LOW_LEVEL_NOF_CYCLES; cycle_idx++)
      {
        ...
        for(lsb_msb = 0; lsb_msb < ARAD_PMF_LOW_LEVEL_NOF_LSB_MSB; lsb_msb++)
        {
          for(ce_idx = 0; ce_idx < ARAD_PMF_LOW_LEVEL_NOF_CE_IN_PROG_LSB; ce_idx++)
          {
            ...
            sal_memset(&pgm_ce, 0x0, sizeof(ARAD_SW_DB_PMF_CE));
            res = arad_sw_db_pgm_ce_instr_get(
                    unit, 
                    stage, 
                    pgm_idx, 
            ...
            if(pgm_ce.is_used)
            {
                ...
                if(mode >= ARAD_PP_FP_RESOURCE_MODE_WITH_AVAILABLE_RESOURCES) {
                  ...
                  info->available.key[stage][pgm_idx][cycle_idx][pmf_db_info.used_key[pgm_idx][key_idx]].is_lsb_db =
                        info->db[pgm_ce.db_id].type == ARAD_PP_FP_DB_TYPE_DIRECT_EXTRACTION ? FALSE : TRUE;
                  info->available.key[stage][pgm_idx][cycle_idx][pmf_db_info.used_key[pgm_idx][key_idx]].is_msb_db =
                        info->db[pgm_ce.db_id].type == ARAD_PP_FP_DB_TYPE_DIRECT_EXTRACTION ? TRUE : pgm_ce.is_msb;
              }
              ...
              if(db_used_program[pgm_ce.db_id] != ARAD_SW_DB_PMF_NOF_PROGS 
  ...
  sal_memset(diag_fes_bitmap, 0x0, sizeof(uint32) * SOC_PPC_NOF_FP_DATABASE_STAGES * ARAD_SW_DB_PMF_NOF_PROGS);
  for(db_idx = 0; db_idx < SOC_PPC_FP_NOF_DBS; db_idx++)
  {
      db_used_program[db_idx] = ARAD_SW_DB_PMF_NOF_PROGS;
  }
  ...
  for(cycle_idx = 0; cycle_idx < ARAD_PMF_LOW_LEVEL_NOF_CYCLES; cycle_idx++)
  {
    ...
    /* Each cycle starts with 16 FESs */
      ...
      for(pgm_idx = pgm_idx_min; pgm_idx < pgm_idx_max; pgm_idx++)
      {
        if(stage == SOC_PPC_FP_DATABASE_STAGE_INGRESS_PMF && mode >= ARAD_PP_FP_RESOURCE_MODE_WITH_AVAILABLE_RESOURCES) {
          ...
          info->available.fes_free[pgm_idx][cycle_idx].fes_free = ARAD_PMF_LOW_LEVEL_NOF_FESS /ARAD_SW_DB_PMF_NOF_CYCLES;
    ...
    for(fem_group_idx = 0; fem_group_idx < SOC_PPC_FP_NOF_MACROS; fem_group_idx++)
    {
         ...
         fem_entry.action_type[0] != SOC_PPC_FP_ACTION_TYPE_INVALID)
      {
            ...
            ARAD_PP_FP_FEM_ENTRY_clear(&diag_fem_entry);
            res = arad_pp_fp_fem_configuration_de_get(
                    unit,
                    fem_id,
                    cycle_idx,
  ...
  for(db_idx = 0; db_idx < SOC_PPC_FP_NOF_DBS; db_idx++)
  {
    ...
    if(SOC_PPC_FP_DB_TYPE_FLP != info->db[db_idx].type) /* internal TCAM specific verification */
    {
        ...
        res = arad_sw_db_tcam_db_action_bitmap_ndx_get(unit, tcam_db_idx, &action_bmp);
  ...
  for(bank_id = 0; bank_id < SOC_DPP_DEFS_GET_TCAM_NOF_BANKS(unit); bank_id++)
  {
    res = arad_sw_db_tcam_bank_valid_get(unit, bank_id, &(info->bank[bank_id].is_used));
    SOC_SAND_CHECK_FUNC_RESULT(res, 313, exit);
    if(info->bank[bank_id].is_used)
    {
      res = arad_sw_db_tcam_bank_nof_entries_free_get(unit, bank_id, &(info->bank[bank_id].entries_free));
      SOC_SAND_CHECK_FUNC_RESULT(res, 315, exit);
      res = arad_sw_db_tcam_bank_entry_size_get(unit, bank_id, &(info->bank[bank_id].entry_size));
      SOC_SAND_CHECK_FUNC_RESULT(res, 318, exit);
  ...
  for(stage = 0; stage < SOC_PPC_NOF_FP_DATABASE_STAGES; stage++)
  {
    ...
    if(mode >= ARAD_PP_FP_RESOURCE_MODE_WITH_AVAILABLE_RESOURCES) {
      ...
      for(pfg_idx = 0; pfg_idx < ARAD_PMF_NOF_GROUPS; ++pfg_idx) {
        res = arad_sw_db_pmf_psl_pfg_dbs_get(
                unit,
                stage,
                pfg_idx,
            ...
            if(!soc_sand_u64_is_zero(&pfg_info->qual_vals[qual_idx].is_valid)) {
              ...
              if ((stage == SOC_PPC_FP_DATABASE_STAGE_INGRESS_FLP) 
                  && (pfg_info->qual_vals[qual_idx].type == SOC_PPC_FP_QUAL_FWD_PRCESSING_PROFILE)) {
                      ...
                      res = arad_pp_fp_control_get_unsafe(unit, &control_ndx, &control_info);
  ...
  for(stage = 0; stage < SOC_PPC_NOF_FP_DATABASE_STAGES; stage++)
  {
    ...
    for(line_idx = 0 ; line_idx < ARAD_PMF_NOF_LINES; line_idx++) 
    {
      ...
      if(valid)
      {
        ...
        res = arad_sw_db_pmf_psl_pgm_dbs_get(
                unit,
                stage,
                info->presel[stage][line_idx].pmf_pgm,
  ...
  if(mode >= ARAD_PP_FP_RESOURCE_MODE_DIAG)
  {
    ...
    {
      tcam_db_idx = ARAD_PP_FP_DB_ID_TO_TCAM_DB(db_idx);
      res = arad_sw_db_tcam_db_valid_get(unit, tcam_db_idx, &is_valid);
      ...
      {
        res = arad_sw_db_tcam_db_access_profile_id_get(unit, tcam_db_idx, key_idx, &profile_id);
        SOC_SAND_CHECK_FUNC_RESULT(res, 348, exit);
        ...
        arad_sw_db_tcam_db_prefix_get(unit, tcam_db_idx, &prefix);
        ...
        not_valid = FALSE;
        res = arad_sw_db_tcam_db_is_direct_get(unit, tcam_db_idx, &is_direct);
    ...
    for(bank_id = 0; bank_id < SOC_DPP_DEFS_GET_TCAM_NOF_BANKS(unit); bank_id++)
    {
      res = arad_sw_db_tcam_bank_valid_get(unit, bank_id, &is_valid);
      SOC_SAND_CHECK_FUNC_RESULT(res, 310, exit);
      if(!is_valid)
      {
        continue;
      ...
      {
        /* Verify TCAM entries */
        res = arad_sw_db_tcam_bank_entry_size_get(unit, bank_id, &entry_size);
        SOC_SAND_CHECK_FUNC_RESULT(res, 377, exit);
           ...
           ((ARAD_TCAM_BANK_ENTRY_SIZE_320_BITS == entry_size) && !(bank_id % 2)))
        {
          ...
          for(entry_idx = 0; entry_idx < bank_nof_lines; entry_idx++)
          {
            ...
            if(tcam_entry.valid)
            {
              ...
              res = arad_sw_db_tcam_managed_bank_prefix_db_get(unit, bank_id, prefix_id, &tcam_db_idx);
              SOC_SAND_CHECK_FUNC_RESULT(res, 383, exit);
              ...
              if(!found)
              {
                  ...
                  if(!error_params[param_idx].is_error)
                  {
                      ...
                      res = arad_sw_db_tcam_db_access_profile_id_get(unit, tcam_db_idx, bank_id%2, &(error_params[param_idx].value[value_idx++]));
          ...
          if((bank_nof_lines != 0))
          {
            res = arad_sw_db_tcam_bank_nof_entries_free_get(unit, bank_id, &nof_entries);
            SOC_SAND_CHECK_FUNC_RESULT(res, 61, exit);
    ...
    for(stage = 0; stage < SOC_PPC_NOF_FP_DATABASE_STAGES; stage++)
    {
      ...
      for(pgm_idx = pgm_idx_min; pgm_idx < pgm_idx_max; pgm_idx++)
      {
        for(cycle_idx = 0; cycle_idx < ARAD_PMF_LOW_LEVEL_NOF_CYCLES; cycle_idx++)
        {
          ...
          res = arad_sw_db_pgm_ce_rsrc_get(unit, stage, pgm_idx, cycle_idx, &ce_rsrc);
          SOC_SAND_CHECK_FUNC_RESULT(res, 400, exit);
          ...
          if(mode >= ARAD_PP_FP_RESOURCE_MODE_DIAG) {
              ...
              res = arad_sw_db_pgm_key_rsrc_get(unit, stage, pgm_idx, cycle_idx, &key_rsrc);
              SOC_SAND_CHECK_FUNC_RESULT(res, 410, exit);
