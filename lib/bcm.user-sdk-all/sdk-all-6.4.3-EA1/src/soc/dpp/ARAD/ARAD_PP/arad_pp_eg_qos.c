...
/* For DSCP/EXP Marking { */
    ...
    #define ARAD_PP_EG_QOS_IP_DSCP_NOF_BITS                      (7)
  ...
  )
{
  ...
  res = arad_pp_epni_pp_pct_tbl_get_unsafe(unit, local_port_ndx, 0, &tbl_data);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 10, exit);
  
  tbl_data.exp_map_profile  = port_qos_info->exp_map_profile;

  res = arad_pp_epni_pp_pct_tbl_set_unsafe(unit, local_port_ndx, 0, &tbl_data);
