...
soc_pm_ports_init(int unit, soc_pbmp_t pbmp, soc_pbmp_t* okay_ports)
{
    ...
    soc_pbmp_t quads_in_use, quads_oor, virtual_pbmp;
    soc_port_if_t interface;
    CONST mac_driver_t* md;
    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        SOC_PBMP_CLEAR(virtual_pbmp);
        SOCDNX_IF_ERR_EXIT(soc_port_sw_db_valid_ports_get(unit, 0, &virtual_pbmp));
        SOC_PBMP_REMOVE(virtual_pbmp, *okay_ports);
        
        SOC_PBMP_ITER(virtual_pbmp, port){
            SOCDNX_IF_ERR_EXIT(soc_port_sw_db_interface_type_get(unit, port, &interface));
            switch (interface) {
            case SOC_PORT_IF_CPU:
            case SOC_PORT_IF_TM_INTERNAL_PKT:
