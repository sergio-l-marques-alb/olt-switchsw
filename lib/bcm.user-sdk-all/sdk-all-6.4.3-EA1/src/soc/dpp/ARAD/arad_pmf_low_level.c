...
#define ARAD_PMF_DESTINATION_MULTICAST_ID_PREFIX    (0x5) /* 101 - MC-ID */
...
#define ARAD_XGS_TM_DEPRECTED_KEY_CONSTRUCTION (0)
...
CONST STATIC
    ...
    {2, 1,  0, SOC_PPC_FP_QUAL_IRPP_ALL_ONES,                   1,  0,  SOC_SAND_BIT(ARAD_PMF_KEY_A)}, /* 1 bit one*/
...
/* Copied before:{2, 1,  2, SOC_PPC_FP_QUAL_HDR_DSP_EXTENSION_AFTER_FTMH,   16,  0,  SOC_SAND_BIT(ARAD_PMF_KEY_A)}, *//* DSP_EXTENSION - inverted with Out-LIF if custom_feature_egress_snooping_advanced = 1*/
    ...
    {2, 0,  1, SOC_PPC_FP_QUAL_IRPP_ALL_ZEROES,                 1,  0,  SOC_SAND_BIT(ARAD_PMF_KEY_B)}, /* 1 bit zero*/
    {2, 0,  2, SOC_PPC_FP_QUAL_IRPP_ALL_ONES,                   1,  0,  SOC_SAND_BIT(ARAD_PMF_KEY_B)}, /* 1 bit one*/
...
#if (ARAD_XGS_TM_DEPRECTED_KEY_CONSTRUCTION)

/* 
 * XGS deprected Key construction. Takes System port 16 bits as is.
 */
static 
    ARAD_PMF_KEY_CONSTR
        Arad_pmf_key_construction_info_xgs[] = {
            /* Regular TM Program */
            /* Is-Not-TM, Is-MSB            , CE-id   Qualifier type,   Nof-bits, Qual-LSB,   Key-Bitmap        */
    ...
    {0, 1, 1,  SOC_PPC_FP_QUAL_IRPP_ALL_ZEROES  , 2 ,  0, SOC_SAND_BIT(ARAD_PMF_KEY_A)}, /*this and the above are destination-system-port-prefix*/
    {0, 1, 2,  SOC_PPC_FP_QUAL_HDR_HIGIG_FRC    , 16, 16, SOC_SAND_BIT(ARAD_PMF_KEY_A)}, /*destination-system-port*/
  ...
  )
{
  ...
  res = arad_pmf_low_level_ce_key_construction_unsafe(
            ...
            pmf_pgm_ndx,
            in_port_profile,
            (sizeof(Arad_pmf_key_construction_info_tm) / sizeof(ARAD_PMF_KEY_CONSTR)),
            Arad_pmf_key_construction_info_tm
        );
  SOC_SAND_CHECK_FUNC_RESULT(res, 70, exit);
  ...
  res = arad_pmf_low_level_fes_action_construction_unsafe(
            ...
            pmf_pgm_ndx,
            in_port_profile,
            (sizeof(Arad_pmf_fes_construction_info_tm) / sizeof(ARAD_PMF_FES_CONSTR)),
            Arad_pmf_fes_construction_info_tm
        );
  ...
  )
{
  ...
  if (SOC_DPP_CONFIG((unit))->arad->xgs_compatability_tm_system_port_encoding == SOC_DPP_XGS_TM_7_MODID_8_PORT ) 
  {
    res = arad_pmf_low_level_ce_key_construction_unsafe(
              ...
              (sizeof(Arad_pmf_key_construction_info_xgs_7_modid_8_port) / sizeof(ARAD_PMF_KEY_CONSTR)),
              Arad_pmf_key_construction_info_xgs_7_modid_8_port
          );
    SOC_SAND_CHECK_FUNC_RESULT(res, 70, exit);
  } 
  ...
  { 
    ...
    res = arad_pmf_low_level_ce_key_construction_unsafe(
              ...
              pmf_pgm_ndx,
              ARAD_PMF_PORT_PROFILE_RAW,
              (sizeof(Arad_pmf_key_construction_info_xgs_8_modid_7_port) / sizeof(ARAD_PMF_KEY_CONSTR)),
              Arad_pmf_key_construction_info_xgs_8_modid_7_port
          );
