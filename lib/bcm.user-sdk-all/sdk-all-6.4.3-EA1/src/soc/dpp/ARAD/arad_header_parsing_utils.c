...
#define ARAD_HPU_ITMH_SNOOP_CMD_MSB      28
#define ARAD_HPU_ITMH_SNOOP_CMD_LSB      25

#define ARAD_HPU_ITMH_TR_CLS_MSB             24
#define ARAD_HPU_ITMH_TR_CLS_LSB             22

#define ARAD_HPU_ITMH_DP_MSB                 21
#define ARAD_HPU_ITMH_DP_LSB                 20
...
#define ARAD_HPU_ITMH_EXT_SRC_PORT_MSB       19
    ...
    SOC_SAND_OUT ARAD_HPU_ITMH_HDR        *itmh)
{
    ...
    SOC_SAND_CHECK_NULL_INPUT(itmh);
        ...
        itmh_hdr_base |= SOC_SAND_SET_BITS_RANGE(info->base.snoop_cmd_ndx, ARAD_HPU_ITMH_SNOOP_CMD_MSB, ARAD_HPU_ITMH_SNOOP_CMD_LSB);
        itmh_hdr_base |= SOC_SAND_SET_BITS_RANGE(info->base.tr_cls, ARAD_HPU_ITMH_TR_CLS_MSB, ARAD_HPU_ITMH_TR_CLS_LSB);
        itmh_hdr_base |= SOC_SAND_SET_BITS_RANGE(info->base.dp, ARAD_HPU_ITMH_DP_MSB, ARAD_HPU_ITMH_DP_LSB);


        arad_hpu_itmh_fwd_dest_info_build(unit, info->base.destination, &dest);
        SOC_SAND_CHECK_FUNC_RESULT(res, 30,exit);

        itmh_hdr_base |= SOC_SAND_SET_BITS_RANGE(dest, ARAD_HPU_ITMH_DESTINATION_MSB, ARAD_HPU_ITMH_DESTINATION_LSB);

        if (info->extension.enable == TRUE) {

            dest = 0;
            arad_hpu_itmh_fwd_dest_info_build(unit, info->extension.destination, &dest);
            SOC_SAND_CHECK_FUNC_RESULT(res, 40,exit);

            itmh_hdr_ext_src_port |= SOC_SAND_SET_BITS_RANGE(dest, ARAD_HPU_ITMH_EXT_SRC_PORT_MSB, ARAD_HPU_ITMH_EXT_SRC_PORT_LSB);
        }
    ...
    SOC_SAND_OUT ARAD_PORTS_ITMH            *info)
{
    ...
    SOC_SAND_CHECK_NULL_INPUT(info);
        ...
        info->base.snoop_cmd_ndx = SOC_SAND_GET_BITS_RANGE(itmh_hdr_base,ARAD_HPU_ITMH_SNOOP_CMD_MSB, ARAD_HPU_ITMH_SNOOP_CMD_LSB);
        info->base.tr_cls =SOC_SAND_GET_BITS_RANGE(itmh_hdr_base,ARAD_HPU_ITMH_TR_CLS_MSB,ARAD_HPU_ITMH_TR_CLS_LSB);
        info->base.dp =SOC_SAND_GET_BITS_RANGE(itmh_hdr_base,ARAD_HPU_ITMH_DP_MSB,ARAD_HPU_ITMH_DP_LSB);

        dest = SOC_SAND_GET_BITS_RANGE(itmh_hdr_base,ARAD_HPU_ITMH_DESTINATION_MSB,ARAD_HPU_ITMH_DESTINATION_LSB);
        res = arad_hpu_itmh_fwd_dest_info_parse(unit, dest, &(info->base.destination));
        SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
        ...
        if(info->base.destination.type == ARAD_DEST_TYPE_ISQ_FLOW_ID ||
            ...
            dest = SOC_SAND_GET_BITS_RANGE(itmh->extention_src_port, ARAD_HPU_ITMH_EXT_SRC_PORT_MSB, ARAD_HPU_ITMH_EXT_SRC_PORT_LSB);
            arad_hpu_itmh_fwd_dest_info_parse(unit, dest, &(info->extension.destination));
            SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);
        }
