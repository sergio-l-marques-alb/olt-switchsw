    ...
    )
{
  ...
  *is_free = TRUE;
  ...
  {
    res = arad_sw_db_tcam_managed_bank_prefix_db_get(unit, bank_id, base | ndx, &tcam_db_id);
    ...
    )
{
  ...
  *success = FALSE;
  ...
  for (msb_ndx = 0; !(*success) && (msb_ndx < nof_msb); ++msb_ndx)
  {
    ...
    {
      res = arad_sw_db_tcam_managed_bank_prefix_db_get(
              unit,
              bank_id,
    ...
    )
{
  ...
  {
    res = arad_sw_db_tcam_managed_bank_prefix_db_set(
      unit,
      bank_id,
      base | ndx,
      tcam_db_id
    ...
    )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(0);
    ...
    *success = FALSE;
    res = arad_sw_db_tcam_bank_valid_get(unit, bank_ndx, &valid);
    ...
    {
        res = arad_sw_db_tcam_bank_nof_entries_free_get(unit, bank_ndx, &nof_entries);
        ...
        res = arad_sw_db_tcam_bank_entry_size_get(unit, bank_ndx, &bank_entry_size);
        SOC_SAND_CHECK_FUNC_RESULT(res, 3, exit);

        res = arad_sw_db_tcam_bank_has_direct_table_get(unit, bank_ndx, &has_direct_table);
        ...
        if ((bank_entry_size == entry_size) 
          ...
          if (bank_owner == curr_bank_owner)
          {
            ...
            res = arad_sw_db_tcam_bank_has_direct_table_get(unit, bank_ndx, &direct_table_exists);
            ...
            {
                res = arad_sw_db_tcam_db_bank_used_get(unit, tcam_db_other_ndx, bank_ndx, &is_used);
                    ...
                    /* In case of 160/320-bit size bank and in FP, possible conflicts with direct table DBs */
                        ...
                        /* Direct Tables can't share the same bank with each other or with 80-bit DBs */
                            ...
                            *success = FALSE;
                            ...
                            res = arad_sw_db_tcam_db_action_bitmap_ndx_get(unit, tcam_db_other_ndx, &action_bmp);
                            SOC_SAND_CHECK_FUNC_RESULT(res, 70, exit);

                            /* Bank ID is odd - action tables 2 and 3 */
                            res = arad_sw_db_tcam_db_entry_size_get(unit,tcam_db_other_ndx, &bank_entry_size);
    ...
    )
{
  ...
  res = arad_sw_db_tcam_use_small_banks_get(unit, tcam_db_id, &use_small_banks);
  SOC_SAND_CHECK_FUNC_RESULT(res, 3, exit);
      ...
      if (two_banks_to_find && (bank_ndx % 2 != 0)) {
          ...
          for (bank_other_ndx = bank_start; bank_other_ndx < (bank_start + nof_banks); ++bank_other_ndx)
          {
              if ((bank_other_ndx % 2) == 0) {
                  res = arad_sw_db_tcam_db_bank_used_get(unit, tcam_db_id, bank_other_ndx, &is_used);
                  SOC_SAND_CHECK_FUNC_RESULT(res, 6, exit);
                  if(is_used) {
                      res = arad_sw_db_tcam_db_bank_used_get(unit, tcam_db_id, bank_other_ndx + 1, &is_used);
  ...
  )
{
  ...
  if (uniform_prefix)
  {
    ...
    {
      res = arad_sw_db_tcam_db_bank_used_get(unit, tcam_db_id, bank_id, &is_used);
      SOC_SAND_CHECK_FUNC_RESULT(res, 1, exit);
   ...
   *  compatible prefix
   */
  res = arad_sw_db_tcam_db_entry_size_get(
          unit,
          tcam_db_id,
          &entry_size
        );
  ...
  res = arad_sw_db_tcam_db_prefix_size_get(
          unit,
          tcam_db_id,
          &prefix_size
        );
  ...
  if (bank_found)
  {
    res = arad_sw_db_tcam_bank_valid_get(unit, bank_id, &is_used);
    SOC_SAND_CHECK_FUNC_RESULT(res, 6, exit);      
    if (!is_used)
    {
      /*
    ...
    {
        res = arad_sw_db_tcam_bank_entry_size_get(unit, bank_id, &entry_size_1);
        ...
        if(entry_size_1 == ARAD_TCAM_NOF_BANK_ENTRY_SIZES
            ...
            res = arad_sw_db_tcam_bank_entry_size_set(unit, bank_id, entry_size);
            SOC_SAND_CHECK_FUNC_RESULT(res, 8, exit);
        }
    }

    res = arad_sw_db_tcam_managed_bank_nof_db_get(
            unit,
            bank_id,
            &nof_dbs
          );
    SOC_SAND_CHECK_FUNC_RESULT(res, 8, exit);
    res = arad_sw_db_tcam_managed_bank_nof_db_set(
      unit,
      bank_id,
      nof_dbs + 1
    );
    ...
    res = arad_sw_db_tcam_db_action_bitmap_ndx_set(
           unit, 
           tcam_db_id, 
           action_bitmap_ndx
    );
  ...
  )
{
  ...
  res = arad_sw_db_tcam_access_profile_valid_get(unit, access_profile_id, &is_valid);
  SOC_SAND_CHECK_FUNC_RESULT(res, 5, exit);
  if (is_valid)
  {
  ...
  res = arad_sw_db_tcam_db_valid_get(unit, tcam_db_id, &is_valid);
  ...
  res = arad_sw_db_tcam_db_access_profile_id_set(
    unit,
    tcam_db_id,
    access_profile_array_id,
    access_profile_id
  ...
  res = arad_sw_db_tcam_access_profile_min_banks_set(
    unit,
    access_profile_id,
    min_banks
  );
  ...
  res = arad_sw_db_tcam_access_profile_user_data_set(
    unit,
    access_profile_id,
    user_data
  );
  ...
  res = arad_sw_db_tcam_access_profile_bank_owner_set(
    unit,
    access_profile_id,
    bank_owner
  );
  ...
  *success = SOC_SAND_SUCCESS;
  ...
  {    
    ...
    res = arad_sw_db_tcam_access_profile_valid_set(
      unit,
      access_profile_id,
      TRUE
    );
  ...
  )
{
  ...
  *success = SOC_SAND_SUCCESS;
  ...
  for (access_profile_ndx = 0; (access_profile_ndx < ARAD_TCAM_NOF_ACCESS_PROFILE_IDS) && (access_profile_array_id != nof_access_profiles); access_profile_ndx++) {
      res = arad_sw_db_tcam_access_profile_valid_get(unit, access_profile_ndx, &access_profile_valid);
  ...
  )
{
  ...
  *success = SOC_SAND_SUCCESS;
  ...
  if(SOC_SAND_SUCCESS == *success) {
      access_profile_array_id = 0;
      res = arad_sw_db_tcam_db_access_profile_id_get(unit, tcam_db_id, access_profile_array_id, &access_profile_id);
      SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
      res = arad_sw_db_tcam_access_profile_bank_owner_get(unit, access_profile_id, &bank_owner);
      SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
      res = arad_tcam_access_profile_bank_add(
                unit,
                access_profile_id,
  ...
  )
{
  ...
  {
      /* Add a bank */
      res = arad_sw_db_tcam_db_entry_size_get(unit, tcam_db_id, &entry_size);
      ...
      *success = SOC_SAND_SUCCESS;
      for (access_profile_array_id = 0; (access_profile_array_id < nof_access_profiles) && (*success == SOC_SAND_SUCCESS); access_profile_array_id++) {
          ...
          res = arad_sw_db_tcam_access_profile_bank_owner_get(
                  unit,
                  access_profile_id,
                  &bank_owner
                );
          ...
          res = arad_sw_db_tcam_db_is_direct_get(unit, tcam_db_id, &is_direct);
  ...
  )
{
  ...
  for (tcam_db_other_ndx = 0; tcam_db_other_ndx < ARAD_TCAM_MAX_NOF_LISTS; tcam_db_other_ndx++) {
      if (SHR_BITGET(info->forbidden_dbs, tcam_db_other_ndx)) {
          res = arad_sw_db_tcam_db_valid_get(unit, tcam_db_other_ndx, &valid);
