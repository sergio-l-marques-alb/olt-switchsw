...
#define ARAD_OFP_RATES_EGQ_CAL_LEN_EGQ(chan_arb_id) \
  ((chan_arb_id == ARAD_OFP_RATES_EGQ_CHAN_ARB_00 || chan_arb_id == ARAD_OFP_RATES_EGQ_CHAN_ARB_01) ? \
  (ARAD_OFP_RATES_CAL_LEN_EGQ_0_1_MAX): (ARAD_OFP_RATES_CAL_LEN_EGQ_REG_MAX))  

/* Calculate offset for SCM table */
#define ARAD_OFP_RATES_EGQ_SCM_OFFSET_GET(cal_info,cal2set) \
  ((cal_info->cal_type == ARAD_OFP_RATES_EGQ_CAL_CHAN_ARB) ? \
  (cal2set * ARAD_OFP_RATES_EGQ_CAL_LEN_EGQ(cal_info->chan_arb_id) ) : \
...
/* 
 * Internal representation of field names for egress shaper channelize arbiter register.
 */
typedef enum
{
  ARAD_OFP_RATES_EGQ_CHAN_ARB_FIELD_RATE = 0,

  ARAD_OFP_RATES_EGQ_CHAN_ARB_FIELD_MAX_BURST = 1,

  ARAD_OFP_RATES_EGQ_CHAN_ARB_FIELD_CAL_LEN = 2,

  ARAD_OFP_RATES_EGQ_CHAN_ARB_NOF_FIELD_TYPE = 3
} ARAD_OFP_RATES_EGQ_CHAN_ARB_FIELD_TYPE;
  ...
  )
{
  ...
  uint8
    ...
    oamp_enable;
  uint32 init_max_burst = ARAD_OFP_RATES_BURST_LIMIT_MAX;
  ...
  )
{
    ...
    res = arad_port2egress_offset(unit, tm_port, 0, &egr_interface_id);
    SOCDNX_IF_ERR_EXIT(res);
  ...
  )
{
    ...
    if(found) {
        res = arad_ofp_rates_port2chan_arb_get_unsafe(unit, tm_port, chan_arb_id);
        SOCDNX_IF_ERR_EXIT(res);
    } else {
  ...
  )
{
    ...
    *sch_cal_len = sch_len + 1;
    ...
    res = MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_sch_cal_tbl_get, (unit, sch_offset, core, sch_to_get, *sch_cal_len, sch_cal->slots));
    SOCDNX_IF_ERR_EXIT(res);
  ...
  )
{
  ...
  */
  ...
  {
    calcal_length = arad_sw_db_ofp_rates_calcal_length_get(unit);
    calcal_instances = arad_sw_db_ofp_rates_nof_instances_get(unit, cal_info->chan_arb_id);
  }
  else
  {
  ...
  )
{
  ...
  *egq_cal_len = 0;
  ...
  SOC_FIELD_GET(EGQ_EGRESS_SHAPER_ENABLE_SETTINGSr, field_cal_set, fld_val);
  egq_to_get = fld_val;

  /* Get suitable cal_len from the current requested calender */
  cal2set = egq_to_get;
  res = arad_ofp_rates_retrieve_egress_shaper_reg_field_names(
          unit,
          cal_info,
          cal2set,
          ARAD_OFP_RATES_EGQ_CHAN_ARB_FIELD_CAL_LEN,
          &reg_egress_shpr,
          &field_egress_shpr_cal_len
        );
  SOCDNX_IF_ERR_EXIT(res);
   ...
   *  Calendar Length
   */
  SOC_FIELD64_GET(reg_egress_shpr, field_egress_shpr_cal_len, field64);
  egq_len=COMPILER_64_LO(field64);
  /*
   ...
   */
  /* 
   * Calculate offset (depends on calender length and cal2set)
   */
  offset = ARAD_OFP_RATES_EGQ_SCM_OFFSET_GET(cal_info,cal2set);
   ...
   */
    for (cal2set = ARAD_OFP_RATES_CAL_SET_A; cal2set < ARAD_OFP_NOF_RATES_CAL_SETS; ++cal2set) {
        ...
        res = arad_ofp_rates_retrieve_egress_shaper_reg_field_names(
                ...
                cal_info,
                cal2set,
                ARAD_OFP_RATES_EGQ_CHAN_ARB_FIELD_RATE,
        ...
        res = arad_ofp_rates_retrieve_egress_shaper_reg_field_names(
                ...
                cal_info,
                cal2set,
                ARAD_OFP_RATES_EGQ_CHAN_ARB_FIELD_MAX_BURST,
                &(reg_shpr_name[cal2set]),
     ...
     )
{
  ...
  if (SOC_IS_ARADPLUS(unit)) {
      ...
      SOC_FIELD_SET(EGQ_EGRESS_SHAPER_ENABLE_SETTINGSr, EMPTY_PORT_STOP_COLLECTING_ENf, (arg ? 1 : 0));

       /* Setting shapers */
      SOC_FIELD_SET(EGQ_EMPTY_PORT_MAX_CREDIT_VALUEr, EMPTY_PORT_MAX_CREDITf, arg);
  }
#endif
     ...
     )
{
  ...
  if (SOC_IS_ARADPLUS(unit)) {
       /* Getting shapers */
      SOC_FIELD_GET(EGQ_EMPTY_PORT_MAX_CREDIT_VALUEr, EMPTY_PORT_MAX_CREDITf, *arg);
  }
#endif
    ...
    switch (reg_type) {
        case EGQ_TCG_SPR_DISr:
              /*Reading register*/
               ...
               SOCDNX_IF_ERR_EXIT(soc_port_sw_db_tm_port_to_out_port_priority_get(unit, port, 0, &nof_pairs));
               SOCDNX_IF_ERR_EXIT(soc_port_sw_db_tm_port_to_base_q_pair_get(unit, port, 0, &base_q_pair));
  ...
  )
{
    ...
    res=arad_port_to_interface_map_get(unit, port, 0, &if_id, NULL);
    SOCDNX_IF_ERR_EXIT(res);
    ...
    if (otm_shp_en_f) {
        ...
        SOCDNX_IF_ERR_EXIT(soc_port_sw_db_tm_port_to_base_q_pair_get(unit, fap_port, core, &base_q_pair));
        SOCDNX_SAND_IF_ERR_EXIT(arad_sw_db_egq_port_rate_get(unit, base_q_pair, core, &queue_rates[base_q_pair]));
  ...
  )
{
    ...
    /* Getting Interface ID */
    res=arad_port_to_interface_map_get(unit, tm_port, core, &if_id, NULL);
    SOCDNX_IF_ERR_EXIT(res);
    ...
    /* Set conifguration */    
    res = arad_ofp_rates_egq_interface_shaper_set(unit,tm_port,SOC_TMC_OFP_SHPR_UPDATE_MODE_OVERRIDE,max_rate);
    SOCDNX_IF_ERR_EXIT(res);
  ...
  )
{
    ...
    soc_field_t
            field_cal_set_name,
            field_cal_len_name[ARAD_OFP_NOF_RATES_CAL_SETS];   
    uint64  field64;
   ...
   */
    if ((egq_cal_len > ARAD_OFP_RATES_EGQ_CAL_LEN_EGQ(cal_info->chan_arb_id)) ||  (egq_cal_len < 1)){
  ...
  SOC_FIELD_GET(EGQ_EGRESS_SHAPER_ENABLE_SETTINGSr, field_cal_set_name, fld_val);
  ...
  res = arad_ofp_rates_retrieve_egress_shaper_reg_field_names(
            ...
            egq_to_set,
            ARAD_OFP_RATES_EGQ_CHAN_ARB_FIELD_CAL_LEN,
            &(reg_shpr_name[egq_to_set]),
            &(field_cal_len_name[egq_to_set])
          );
  SOCDNX_IF_ERR_EXIT(res);
   ...
   *  Calendar length EGQ
   */
  COMPILER_64_SET(field64,0,egq_len);
  SOC_FIELD64_SET(reg_shpr_name[egq_to_set], field_cal_len_name[egq_to_set], field64);
  ...
  {
    offset = ARAD_OFP_RATES_EGQ_SCM_OFFSET_GET(cal_info,egq_to_set);
  ...
  )
{
    ...
    res = MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_sch_cal_tbl_set, (unit, sch_offset, core, sch_to_set, sch_cal_len, sch_cal->slots));
    SOCDNX_IF_ERR_EXIT(res);
        ...
        )
{
    ...
    *nof_valid_queues = 0;
    SOCDNX_IF_ERR_EXIT(soc_port_sw_db_tm_port_to_base_q_pair_get(unit, port_id, 0, &port_base_q_pair));

    SOCDNX_IF_ERR_EXIT(soc_port_sw_db_valid_ports_get(unit, 0, &pbmp));

    SOC_PBMP_ITER(pbmp, port_i) {

      SOCDNX_IF_ERR_EXIT(soc_port_sw_db_local_to_tm_port_get(unit, port_i, &fap_port_ndx, &core));

      SOCDNX_IF_ERR_EXIT(soc_port_sw_db_tm_port_to_base_q_pair_get(unit,fap_port_ndx, core, &base_q_pair));
      
      if (base_q_pair == ARAD_EGR_INVALID_BASE_Q_PAIR)
      {
        continue;
      }
      res = soc_port_sw_db_tm_port_to_out_port_priority_get(unit, fap_port_ndx, core, &nof_priorities);
      SOCDNX_IF_ERR_EXIT(res);

      res = arad_sw_db_egr_ports_port_priority_get(unit,&rates);
      SOCDNX_SAND_IF_ERR_EXIT(res);
        ...
        )
{
    ...
    *nof_valid_entries = 0;
    ...
    SOCDNX_IF_ERR_EXIT(soc_port_sw_db_tm_port_to_base_q_pair_get(unit, port_id, 0, &base_q_pair));
    ...
    SOC_PBMP_ITER(pbmp, port_i) 
    {
      ...
      res = arad_sw_db_egr_ports_tcg_get(unit,&rates);
      SOCDNX_SAND_IF_ERR_EXIT(res);
  ...
  )
{
  ...
  {
    SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG_STR( "Illegal if_id")));
  }

  res = arad_fap_port_id_verify(unit, port_id);
  SOCDNX_SAND_IF_ERR_EXIT(res);
  ...
  case ARAD_OFP_RATES_GENERIC_FUNC_STATE_TCG_SCH_RATE:
      ...
      res = arad_ofp_rates_tcg_id_sch_verify(unit, port_id, tcg_ndx, &is_sch_valid); 
      SOCDNX_IF_ERR_EXIT(res);
  ...
  case ARAD_OFP_RATES_GENERIC_FUNC_STATE_TCG_EGQ_RATE:
      ...
      res = arad_ofp_rates_tcg_id_egq_verify(unit,port_id,tcg_ndx,&is_egq_valid);
      SOCDNX_IF_ERR_EXIT(res);
  ...
  res = arad_ofp_rates_sw_db_tcg_nof_valid_entries_get(unit,port_id,tcg_ndx,&nof_valid_entries);
  SOCDNX_IF_ERR_EXIT(res);
  ...
  )
{
  ...
  SOCDNX_IF_ERR_EXIT(soc_port_sw_db_tm_port_to_out_port_priority_get(unit, port_id, 0, &nof_priorities));
    ...
    )
{
    ...
    res = arad_port2egress_offset(unit, fap_port, 0, &egr_interface_id);
    SOCDNX_IF_ERR_EXIT(res);
    ...
    )
{
    ...
    res = MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_egr_is_channelized, (unit, tm_port, core, &is_channelized_id));
    SOCDNX_IF_ERR_EXIT(res);
    ...
    if (is_channelized_id)
    {
        ...
        res = MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_port2egress_offset, (unit, tm_port, core, &egress_offset));
        SOCDNX_IF_ERR_EXIT(res);

        /* Get scheduler offset */
        SOCDNX_IF_ERR_EXIT(READ_SCH_FC_MAP__FCMm(unit, SCH_BLOCK(unit, core), egress_offset, &data));
        sch_offset = soc_SCH_FC_MAP__FCMm_field32_get(unit,&data,FC_MAP__FCMf);
        ...
        *rate = sch_rates[base_q_pair];
        ...
        res = arad_sch_if_shaper_rate_get(unit,tm_port, core, rate);
        SOCDNX_IF_ERR_EXIT(res);
    }
    ...
    )
{
    ...
    res = arad_sw_db_sch_port_rate_set(unit, base_q_pair, core, rate);
    SOCDNX_SAND_IF_ERR_EXIT(res);
    res = arad_sw_db_is_port_valid_set(unit, base_q_pair, core, TRUE);
    SOCDNX_SAND_IF_ERR_EXIT(res);
    ...
    if (is_channelized_id)
    {
        ...
        for(ps = 0; ps < ARAD_EGR_NOF_PS ; ps++) {
            if(SHR_BITGET(ps_bmap, ps)) {
                res = soc_port_sw_db_tm_port_to_out_port_priority_get(unit, tm_port, core, &nof_prio);
                SOCDNX_IF_ERR_EXIT(res);
                for(prio = 0; prio < ARAD_EGR_NOF_Q_PAIRS_IN_PS ; prio += nof_prio) {
                    base_q_pair = (ps*ARAD_EGR_NOF_Q_PAIRS_IN_PS) + prio;
                    res = arad_sw_db_is_port_valid_get(unit, base_q_pair, core, &is_valid);
                    SOCDNX_IF_ERR_EXIT(res);

                    if(is_valid) {
                        res = arad_sw_db_sch_port_rate_get(unit, base_q_pair, core, &sch_rates[base_q_pair]);
                        SOCDNX_SAND_IF_ERR_EXIT(res);
         ...
         * should be set. 
         */
        res = arad_sch_ch_if_rate_get_unsafe(unit, sch_offset, core, &current_if_rate);
        SOCDNX_IF_ERR_EXIT(res);
        ...
        res = soc_port_sw_db_tm_port_to_base_q_pair_get(unit, tm_port, core, &base_q_pair);
        SOCDNX_IF_ERR_EXIT(res);
        res = arad_sw_db_is_port_valid_get(unit, base_q_pair, core, &is_valid);
        SOCDNX_IF_ERR_EXIT(res);
        if (is_valid)
        {
            nof_valid_ports += 1;
            /*Get sch rate*/
            res = arad_sw_db_sch_port_rate_get(unit, base_q_pair, core, &sch_rates[0]);
            SOCDNX_SAND_IF_ERR_EXIT(res);
            res = arad_sch_if_shaper_rate_set(unit, tm_port, core, sch_rates[0]);
            SOCDNX_IF_ERR_EXIT(res);
        }
        else
        {
    ...
    )
{
    ...
    /*Get chnl arbiter id*/
    res = arad_ofp_rates_port2chan_arb_unsafe(unit,fap_port,&cal_info.chan_arb_id);
    SOCDNX_IF_ERR_EXIT(res);
    cal_info.cal_type = ARAD_OFP_RATES_EGQ_CAL_CHAN_ARB;
    ...
    res = soc_port_sw_db_tm_port_to_base_q_pair_get(unit, fap_port, 0, &base_q_pair);
    SOCDNX_IF_ERR_EXIT(res);
    if (base_q_pair == ARAD_EGR_INVALID_BASE_Q_PAIR)
    {
        SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG_STR( "Illegal port")));
    ...
    )
{
    ...
    /*
     * Getting the channel arbiter which the fap port is mapped to
     */
    res = arad_ofp_rates_port2chan_arb_unsafe(unit,fap_port,&chan_arb_id);
    SOCDNX_IF_ERR_EXIT(res);
    /*
     * Getting the fap port's base_q_pair
     */
     ...
     * Setting rate to sw
     */
    res = arad_sw_db_egq_port_rate_set(unit, base_q_pair, 0, rate);
    SOCDNX_SAND_IF_ERR_EXIT(res);
    res = arad_sw_db_is_port_valid_set(unit, base_q_pair, 0, TRUE);
    SOCDNX_SAND_IF_ERR_EXIT(res);
    ...
    )
{
    uint32 
        ...
        egq_calcal_rates[ARAD_OFP_RATES_EGQ_NOF_CHAN_ARB],
        egq_calcal_instances[ARAD_OFP_RATES_EGQ_NOF_CHAN_ARB],
        base_q_pair,
        egq_rate,
        port_i;
    uint8  
    ...
    /*Clear*/
    egq_calcal_chan_arb_rate_requested = 0;
    sal_memset(egq_calcal_rates,0,sizeof(uint32)*ARAD_OFP_RATES_EGQ_NOF_CHAN_ARB);
    sal_memset(egq_calcal_instances,0,sizeof(uint32)*ARAD_OFP_RATES_EGQ_NOF_CHAN_ARB);
    ...
    SOC_PBMP_ITER(pbmp, port_i)
    {
        ...
        res = arad_ofp_rates_port2chan_arb_get_unsafe(unit, tm_port, &chan_arb_i);
        SOCDNX_IF_ERR_EXIT(res);
        ...
        res = arad_sw_db_egq_port_rate_get(unit, base_q_pair, core, &egq_rate); 
        SOCDNX_SAND_IF_ERR_EXIT(res);
    ...
    SOC_FIELD_GET(EGQ_EGRESS_SHAPER_ENABLE_SETTINGSr, OTM_SPR_SET_SELf, act_cal);
    ...
    {
      SOC_FIELD_SET(EGQ_EGRESS_SHAPER_CALENDARS_ARBITRATION_CYCLE_LENGTHr, CAL_CAL_LEN_Af, egq_calcal_calendar_len - 1);
    }
    else
    {
      SOC_FIELD_SET(EGQ_EGRESS_SHAPER_CALENDARS_ARBITRATION_CYCLE_LENGTHr, CAL_CAL_LEN_Bf, egq_calcal_calendar_len - 1);
    }
    
    res = arad_sw_db_ofp_rates_calcal_length_set(unit, egq_calcal_calendar_len);
    SOCDNX_SAND_IF_ERR_EXIT(res);

    /* Write CalCal entries, calculate nof_calal instances per chan_arb*/
    ...
    for (chan_arb_i = 0; chan_arb_i < ARAD_OFP_RATES_EGQ_NOF_CHAN_ARB; ++chan_arb_i)
    {
        res = arad_sw_db_ofp_rates_nof_instances_set(unit, chan_arb_i, egq_calcal_instances[chan_arb_i]);
        SOCDNX_SAND_IF_ERR_EXIT(res);
    }
    ...
    )
{
    ...
    /*Configure calcal*/
    res = arad_ofp_rates_calcal_config_unsafe(unit);
    SOCDNX_IF_ERR_EXIT(res);
    /*Get channel arbiter*/
    res = arad_ofp_rates_port2chan_arb_unsafe(unit,fap_port,&chan_arb_id);
    SOCDNX_IF_ERR_EXIT(res);
    ...
    for (chan_arb_i = 0 ; chan_arb_i < ARAD_OFP_RATES_EGQ_NOF_CHAN_ARB ; ++chan_arb_i) {
        ...
        if (arad_sw_db_ofp_rates_nof_instances_get(unit,chan_arb_i) > 0) {
            /*Retrive rates from the software database*/
            egq_if_rate_requested = 0;
            ...
            SOC_PBMP_ITER(pbmp, port_i)
            {
                ...
                res = arad_ofp_rates_port2chan_arb_get_unsafe(unit, fap_port_ndx, &cur_chan_arb_i);
                SOCDNX_IF_ERR_EXIT(res);
                ...
                {
                    /*Get egq rate*/
                    res = arad_sw_db_egq_port_rate_get(unit, base_q_pair, core, &egq_rates[base_q_pair]);
                    SOCDNX_SAND_IF_ERR_EXIT(res);
            ...
            cal_info.cal_type = ARAD_OFP_RATES_EGQ_CAL_CHAN_ARB;
            recalc = ((chan_arb_i == chan_arb_id) || arad_sw_db_ofp_rates_update_dev_changed_get(unit)) ? TRUE : FALSE;
            res = arad_ofp_rates_from_egq_ports_rates_to_generic_calendar(
                      ...
                      ARAD_OFP_RATES_EGQ_CAL_LEN_EGQ(cal_info.chan_arb_id),
                      recalc,
                      TRUE,
                      egq_calendar,
                      &egq_calendar_len  
    ...
    /*Set active calendar*/
     SOC_FIELD_SET(EGQ_EGRESS_SHAPER_ENABLE_SETTINGSr, OTM_SPR_SET_SELf, cal2set);
    ...
    )
{
     ...
     * Setting max_rate
     */
    offset = ARAD_OFP_RATES_EGQ_PMC_OFFSET_GET(cal_info,egq_to_get,fap_port);
       ...
       )
{
  ...
  if (SOC_IS_ARADPLUS(unit)) {
      ...
      res=WRITE_EGQ_EGRESS_SHAPER_ENABLE_SETTINGSr(unit, REG_PORT_ANY, reg_val);
      SOCDNX_SAND_IF_ERR_EXIT(res);
       /* Setting shapers */
      res=READ_EGQ_QP_MAX_CREDIT_VALUEr(unit, REG_PORT_ANY,&reg_val);
      SOCDNX_SAND_IF_ERR_EXIT(res);
      soc_reg_field_set(unit, EGQ_QP_MAX_CREDIT_VALUEr, &reg_val, EMPTY_QP_MAX_CREDITf, max_burst_empty_queues);
      res=WRITE_EGQ_QP_MAX_CREDIT_VALUEr(unit, REG_PORT_ANY, reg_val);
      SOCDNX_SAND_IF_ERR_EXIT(res);
  }
  /*Max burst for empty and fc queues }*/
#endif
    ...
    )
{
...
#endif
  {
        ...
        /*Getting the fap port's base_q_pair*/
        SOCDNX_IF_ERR_EXIT(soc_port_sw_db_tm_port_to_base_q_pair_get(unit, fap_port, 0, &base_q_pair));
    ...
    )
{
  ...
  {
        /*Get active calendar*/
        SOC_FIELD_GET(EGQ_EGRESS_SHAPER_ENABLE_SETTINGSr, TCG_SPR_SET_SELf, act_cal);
        cal2set = (act_cal == ARAD_OFP_RATES_CAL_SET_A)?ARAD_OFP_RATES_CAL_SET_B:ARAD_OFP_RATES_CAL_SET_A;
        ...
        SOC_PBMP_ITER(pbmp, port_i)
        {
          ...
          SOCDNX_IF_ERR_EXIT(soc_port_sw_db_tm_port_to_out_port_priority_get(unit, fap_port_ndx, core, &nof_priorities));
        ...
        /*Set active calendar*/
        SOC_FIELD_SET(EGQ_EGRESS_SHAPER_ENABLE_SETTINGSr, TCG_SPR_SET_SELf, cal2set);
  }
    ...
    )
{
    ...
    /*Get active calendar*/
    SOC_FIELD_GET(EGQ_EGRESS_SHAPER_ENABLE_SETTINGSr, QPAIR_SPR_SET_SELf, act_cal);
    cal2set = (act_cal == ARAD_OFP_RATES_CAL_SET_A)?ARAD_OFP_RATES_CAL_SET_B:ARAD_OFP_RATES_CAL_SET_A;
    ...
    if (arad_sw_db_egr_ports_egq_tcg_qpair_shaper_enable_get(unit))
    {
        ...
        /*Set active calendar*/
        SOC_FIELD_SET(EGQ_EGRESS_SHAPER_ENABLE_SETTINGSr, QPAIR_SPR_SET_SELf, cal2set);
    }
...
#endif
  {
        res = arad_ofp_rates_tcg_ptc_sch_generic_set_unsafe(unit,
                                                          fap_port, 
                                                          tcg_ndx,   
                                                          tcg_rate,
