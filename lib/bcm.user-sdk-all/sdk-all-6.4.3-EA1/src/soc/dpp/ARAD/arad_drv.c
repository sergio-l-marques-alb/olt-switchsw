...
soc_arad_default_config_tm_get(int unit)
{
    ...
    rv = soc_dpp_arad_str_prop_mc_tbl_mode(unit);
...
soc_arad_default_config_general_get(int unit)
{
    ...
    if (SOC_IS_ARADPLUS(unit)) {
        dpp_pp->oam_classifier_advanced_mode = soc_property_get(unit, spn_OAM_CLASSIFIER_ADVANCED_MODE, 1) && SOC_IS_ARADPLUS_A0(unit);
...
soc_arad_info_config(int unit) 
{
    ...
    /*
     * init multicast auto allocation ranges
     */
        ...
        /* TDM Always High Priority Scheduling { */
        {
            ...
            for (port_i = 0; port_i < SOC_MAX_NUM_PORTS; ++port_i) {
                ...
                if (IS_TDM_PORT(unit, port_i)) {
                    soc_sand_rv = arad_egr_ofp_scheduling_get(unit, tm_port, &ofp_sch_info);
                    rv = handle_sand_result(soc_sand_rv);
                    SOCDNX_IF_ERR_EXIT(rv);

                    ofp_sch_info.nif_priority = SOC_TMC_EGR_OFP_INTERFACE_PRIO_HIGH;

                    soc_sand_rv = arad_egr_ofp_scheduling_set(unit, tm_port, &ofp_sch_info);
                    rv = handle_sand_result(soc_sand_rv);
                    SOCDNX_IF_ERR_EXIT(rv);
                }
            }
