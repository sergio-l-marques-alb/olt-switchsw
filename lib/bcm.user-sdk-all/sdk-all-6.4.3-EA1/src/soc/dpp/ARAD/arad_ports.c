  ...
  )
{
  ...
  int core;
  uint32 is_vendor_pp_port_enable = 0;
  ...
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 12, exit, soc_port_sw_db_valid_ports_get(unit, SOC_PORT_FLAGS_VENDOR_PP_PORT, &vendor_pp_port_bm));
  ...
  SOC_PBMP_ITER(pbmp, port_i) {
      ...
      res = arad_ports_multicast_id_offset_set(unit, tm_port , multicast_offset);
...
port_termination_exit:
  ...
  core = 0;
  for (pp_port = 0; pp_port < ARAD_NOF_FAP_PORTS; ++pp_port)
  {
      res = READ_IHP_PTC_VIRTUAL_PORT_CONFIGm(unit, MEM_BLOCK_ANY, pp_port, tbl_data_64b);
      ...
      res = WRITE_IHP_PTC_VIRTUAL_PORT_CONFIGm(unit, MEM_BLOCK_ANY, pp_port, tbl_data_64b);
      ...
      res = soc_port_sw_db_pp_is_valid_get(unit, pp_port, core, &is_valid);
      SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 28, exit);
      if(is_valid) {
          res = soc_port_sw_db_pp_port_to_base_q_pair_get(unit, pp_port, core, &base_q_pair);
          SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 29, exit);
          res = soc_port_sw_db_pp_port_to_out_port_priority_get(unit, pp_port, core, &nof_pairs);
          SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 30, exit);

          /* go over all q-pairs */
          for (curr_q_pair = base_q_pair; curr_q_pair < base_q_pair + nof_pairs; curr_q_pair++) 
          {
              res = arad_egq_pct_tbl_get_unsafe(unit, curr_q_pair, 0, &pct_tbl_data);
              ...
              if (soc_property_port_suffix_num_get(unit, pp_port, -1, spn_CUSTOM_FEATURE, "map_port_extr_enabled", 0) == 1) {
                  ...
                  (SOC_DPP_CONFIG(unit))->pp.port_extender_map_lc_exists=1;
              }

              res = arad_egq_pct_tbl_set_unsafe(unit, curr_q_pair, 0, &pct_tbl_data);
  ...
  {  
    ...
    {    
       ...
       * Action is 14bits. 
       * Action should be:Action[3:0] from PTC (there are only 8 PON ports 0-7) and Action[13:4] Tunnel-ID from header
       */
    virtual_port_fem_field_select_map.virtual_port_fem_field_select[fem_bit_ndx] = 
  ...
  else if (is_vendor_pp_port_enable) {
  ...
  )
{
    ...
    SOCDNX_IF_ERR_EXIT(soc_port_sw_db_port_with_channel_get(unit, port, 0, &port_match_channel));
    if(port_match_channel == SOC_MAX_NUM_PORTS)
    {
         SOCDNX_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_SOCDNX_MSG("No matching port was found")));
    }
    xgs_mac_ext_info->ptc_port = port_match_channel;
  ...
  )
{
    ...
    SOCDNX_IF_ERR_EXIT(arad_pp_epni_pp_pct_tbl_get_unsafe(unit, pp_port, core, &pp_pct_tbl_data));
  ...
  )
{
  ...
  if (is_xgs_mac_ext) 
  {
      res = arad_ports_local_to_xgs_mac_extender_info_get(unit, tm_port_id, &xgs_mac_ext_info);
  ...
  )
{
  ...
  res = MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_mgmt_system_fap_id_get,(unit , &sys_fap_id_self));
  SOC_SAND_CHECK_FUNC_RESULT(res, 5, exit);
  ...
  if (mapped_fap_id >= sys_fap_id_self && mapped_fap_id < sys_fap_id_self + SOC_DPP_CONFIG(unit)->core_mode.nof_active_cores)
  {
    ...
    res = arad_pp_egq_dsp_ptr_map_tbl_get_unsafe(unit, mapped_fap_port_id, core_id, &dsp_tbl_data);
    SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 1130, exit);

    dsp_tbl_data.dst_system_port = egq_sys_phys_port_ndx;

    res = arad_pp_egq_dsp_ptr_map_tbl_set_unsafe(unit, mapped_fap_port_id, core_id, &dsp_tbl_data);
    ...
    /* store in SW DB */
    res = arad_sw_db_local_to_sys_port_set(
            unit,
            mapped_fap_port_id,
            sys_phys_port_ndx
          );
  ...
  )
{
  ...
  if (fap_ndx == sys_fap_id_self)
  {
    ...
    res = arad_sw_db_local_to_sys_port_get(
            unit,
            fap_local_port_ndx,
            sys_phys_port_id
          );
  ...
  } else { /* not a local port */
    if (ARAD_IS_VOQ_MAPPING_INDIRECT(unit))
    { /* indirect voq to modport mapping mode */
      do {
        res = arad_sys_phys_to_local_port_map_get_unsafe(unit, curr_sys_port_id, &curr_fap_id, &curr_port_id);
        SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
      ...
      } while((curr_sys_port_id <= ARAD_MAX_SYSTEM_PHYSICAL_PORT_ID_INDIRECT));

      *sys_phys_port_id = found ? curr_sys_port_id : ARAD_SYS_PHYS_PORT_INVALID_INDIRECT;
    }
    else
    { /* direct voq to modport mapping mode */
      ARAD_SYSPORT sysport;
      res = arad_sw_db_modport2sysport_get(unit, fap_ndx, fap_local_port_ndx, &sysport);
      SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
      *sys_phys_port_id = sysport != ARAD_NOF_SYS_PHYS_PORTS ? sysport : ARAD_SYS_PHYS_PORT_INVALID_DIRECT;
    }
  }
  ...
  )
{
  ...
  if (!is_mapped) 
  {
      ...
      if (map_info->if_id != SOC_TMC_IF_ID_CPU && map_info->if_id != SOC_TMC_IF_ID_OLP && 
         ...
         SOC_SAND_SOC_IF_ERROR_RETURN(res, 5, exit,soc_port_sw_db_tm_to_local_port_get(unit, port_ndx, 0, &port));
    ...
    if (is_channelized_interface && !(nif_type == ARAD_NIF_TYPE_ILKN)) 
    {
        ...
        if(is_master){
            ...
            if (!is_mapped) { /* if master port is unmaped, the next master should used instead */
                SOC_SAND_SOC_IF_ERROR_RETURN(res, 10, exit,soc_port_sw_db_tm_to_local_port_get(unit, port_ndx, 0, &port));
  ...
  )
{
  ...
  rv = soc_port_sw_db_tm_port_to_base_q_pair_get(unit, port_ndx, 0, &base_q_pair);
  if(SOC_FAILURE(rv)) {
      SOC_SAND_SET_ERROR_CODE(ARAD_UNKNOWN_NIF_TYPE_ERR, 1, exit);
  }
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 5 ,exit, soc_port_sw_db_tm_port_to_out_port_priority_get(unit, port_ndx, 0, &nof_q_pairs));

  rv = soc_port_sw_db_tm_to_local_port_get(unit, port_ndx, 0, &local_port);
  if(SOC_FAILURE(rv)) {
  ...
  ifc = (is_mapped) ? ifc:ARAD_EGQ_IFC_DEF_VAL;
    ...
    )
{
  ...
  */
  ...
  do
  {
    ...
    else
    {
      if (curr_info.if_id != ARAD_IF_ID_NONE)
      {
        ...
        SOC_SAND_SOC_IF_ERROR_RETURN(res, 64, exit, soc_port_sw_db_tm_to_local_port_get(unit, port_ndx, 0, &local_port));
      ...
      !(is_init)) /*if !is_init then it is a dynamic port - hence need to rewrite it to to the HW*/
  {
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 114, exit, soc_port_sw_db_tm_to_local_port_get(unit, port_ndx, 0, &local_port));
    ...
    if(!SOC_PORT_IS_STAT_INTERFACE(flags)) {
       ...
       if(!is_init) { /*for dynamic need to update FQP and PQP cals*/
          res = arad_egr_q_nif_cal_set_unsafe(unit);
  ...
  )
{
  ...
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 3, exit,soc_port_sw_db_tm_to_local_port_get(unit,port_ndx,0,&local_port));
  ...
  )
{
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 10, exit, soc_port_sw_db_tm_to_local_port_get(unit, tm_port, core, &local_port));
    ...
    )
{
    ...
    *if_id = ARAD_IF_ID_NONE;
    ...
    SOCDNX_IF_ERR_EXIT(READ_IQM_ITMPMm(unit, IQM_BLOCK(unit,core), reassembly_context, &to_iqm_itmpm));
  ...
  )
{
    ...
    if (in_enable)
    {
        ...
        if (!skip_config)
        {
            ...
            /* IHP-Port Info */
            SOCDNX_IF_ERR_EXIT(arad_pp_ihp_tm_port_parser_program_pointer_config_tbl_get_unsafe(unit, pp_port, core, &tm_port_parser_program_pointer));

            /* The Parser Program Pointer is bits 3:0 */
            /* Skip the first junk bits and after that the PTCH, just before Ethernet / ITMH */
            SOCDNX_IF_ERR_EXIT(arad_parser_nof_bytes_to_remove_get(unit, tm_port, core, &fld_val));
            ...
            SOCDNX_IF_ERR_EXIT(arad_pp_ihp_tm_port_parser_program_pointer_config_tbl_set_unsafe(unit, pp_port, core, &tm_port_parser_program_pointer));
            ...
            SOCDNX_IF_ERR_EXIT(arad_ihp_tm_port_sys_port_config_tbl_get_unsafe(unit, pp_port, core, &ihp_port2sys_data));
            ...
            SOCDNX_IF_ERR_EXIT(arad_ihp_tm_port_sys_port_config_tbl_set_unsafe(unit, pp_port, core, &ihp_port2sys_data));
            ...
            if ((header_type == SOC_TMC_PORT_HEADER_TYPE_TM) || (arad_port_header_type_is_injected_any(header_type))
                ...
                SOCDNX_IF_ERR_EXIT(soc_port_sw_db_pp_to_local_port_get(unit, pp_port, core, &local_port));
    ...
    if (out_enable)
    {
        ...
        if (!skip_config)
        {
            ...
            {
                SOCDNX_IF_ERR_EXIT(arad_egq_pct_tbl_get_unsafe(unit, curr_q_pair, core, &egq_pct_data));
          ...
          if (soc_property_port_suffix_num_get(unit, port,-1, spn_CUSTOM_FEATURE,"vendor_custom_tm_port", FALSE)==1  && port) {
                ...
                SOCDNX_IF_ERR_EXIT(arad_egq_pct_tbl_set_unsafe(unit, curr_q_pair, core, &egq_pct_data));

            }        

            /* EGQ-PPCT - Retreive for base_q_pair only */
            SOCDNX_IF_ERR_EXIT(arad_egq_ppct_tbl_get_unsafe(unit, base_q_pair, core, &egq_ppct_data));
            egq_ppct_data.port_type = port_type_val;
            SOCDNX_IF_ERR_EXIT(arad_egq_ppct_tbl_set_unsafe(unit, base_q_pair, core, &egq_ppct_data));
            ...
            if(header_type == ARAD_PORT_HEADER_TYPE_STACKING) {
                SOCDNX_IF_ERR_EXIT(arad_pp_lag_hashing_port_lb_profile_set(unit, pp_port, core, 1));
            } else {
                SOCDNX_IF_ERR_EXIT(arad_pp_lag_hashing_port_lb_profile_set(unit, pp_port, core, 0));
            }
  ...
  )
{
    ...
    if (tm_port_parser_program_pointer.parser_program_pointer_profile == ARAD_PORTS_FEM_PROFILE_PP_PORT_PROFILE_INJECTED_PTCH2)
    {
        ...
        header_type_in = (header_type_in != ARAD_PORT_HEADER_TYPE_ETH)?ARAD_PORT_HEADER_TYPE_INJECTED_2:ARAD_PORT_HEADER_TYPE_INJECTED_2_PP;
    }
  ...
  )
{
    ...
    {   
        ...
        switch (tm_port_profile) {
            case ARAD_EGR_PROG_TM_PORT_PROFILE_TM_SRC_NO_DEST_NO_CUD_NEV: /*NEVER*/
                /*[9:0]   = {5'prge_prog_pointer, 4'h0, 1'h0};
                  [21:12] = {5'prge_prog_pointer, 4'h0, 1'h0};
                  [11:10] = 2'h0;
                  [31:22] = 10'h0;*/
                prog_editor_value   = (prge_prog_pointer << 5) + (prge_prog_pointer << 17); /* OTMH_BASE */
                break;
            case ARAD_EGR_PROG_TM_PORT_PROFILE_TM_SRC_NO_DEST_NO_CUD_MC:  /*IF_MC*/
                /*[9:0]   ={5'h10,4'h0,1'h0}; CUD if MC
                  [21:12] = {5'prge_prog_pointer, 4'h0, 1'h0};
                  [11:10] = 2'h0;
                  [31:22] = 10'h0;*/
                prog_editor_value = (0x10 << 5) + (prge_prog_pointer << 17); /* OTMH_WITH_CUD_EXT if Multicast */
                break;
            case ARAD_EGR_PROG_TM_PORT_PROFILE_TM_SRC_NO_DEST_NO_CUD_ALW: /*ALWAYS*/
                /*[9:0]   = {5'h14, 4'h0, 1'h0}; CUD Extension         - entry 40
                  [21:12] = {5'prge_prog_pointer, 4'h0, 1'h0};
                  [11:10] = 2'h0;
                  [31:22] = 10'h0;*/
                prog_editor_value = (0x14 << 5) + (prge_prog_pointer << 17); /* OTMH_WITH_CUD_EXT */
                break;
            case ARAD_EGR_PROG_TM_PORT_PROFILE_TM_SRC_DEST_NO_CUD_NEV: /*NEVER*/
                /*[9:0]   = {5'prge_prog_pointer, 4'h0, 1'h0};
                  [21:12] = {5'h14, 4'h0, 1'h0}; Source Port Extension - entry 40
                  [11:10] = 2'h0;
                  [31:22] = 10'h0;*/
                prog_editor_value   = (prge_prog_pointer << 5) + (0x14 << 17); /* OTMH_WITH_SRC_EXT */
                break;
            case ARAD_EGR_PROG_TM_PORT_PROFILE_TM_SRC_DEST_NO_CUD_MC:
                /*[9:0]   = {5'h10, 4'h0, 1'h0}; CUD if MC
                  ...
                  [11:10] = 2'h0;
                  [31:22] = 10'h0;*/
                prog_editor_value = 0x280200; /* OTMH_WITH_SRC_AND_MC_ID_EXT */
                break;
            case ARAD_EGR_PROG_TM_PORT_PROFILE_TM_SRC_DEST_NO_CUD_ALW:
                /*[9:0]   = {5'h14, 4'h0, 1'h0}; CUD Extension         - entry 40
                  ...
                  [11:10] = 2'h0;
                  [31:22] = 10'h0;*/
                prog_editor_value = 0x280280; /* OTMH_WITH_SRC_AND_CUD_EXT */
                break;
            default:
    ...
    if (update_prog_editor_value || is_double_tag || is_otmh_enlarge)
    {
        ...
        for (curr_q_pair = base_q_pair; curr_q_pair < base_q_pair + nof_pairs; curr_q_pair++) {
            ...
            SOCDNX_IF_ERR_EXIT(arad_egq_pct_tbl_get_unsafe(unit, curr_q_pair, core, &pct_tbl_data));
            ...
            SOCDNX_IF_ERR_EXIT(arad_egq_pct_tbl_set_unsafe(unit, curr_q_pair, core, &pct_tbl_data));
        }
    }
  ...
  )
{
  ...
  if (header_type_outgoing != ARAD_PORT_HEADER_TYPE_TM)
  {
    SOC_SAND_SET_ERROR_CODE(ARAD_PORTS_NOT_TM_PORT_ERR, 20, exit);
  }
  ...
  )
{
  ...
  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  50,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, uc_reg, REG_PORT_ANY, 0, uc_fld,  value));
  ...
  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  70,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, mc_reg, REG_PORT_ANY, 0, mc_fld,  value));
  ...
  )
{
    ...
    */
    ...
    SOCDNX_IF_ERR_EXIT(arad_pp_epni_pp_pct_tbl_set_unsafe(unit, pp_port, core, &pp_pct_tbl_data));
  ...
  )
{
    ...
    if ((direction_ndx == ARAD_PORT_DIRECTION_INCOMING)
        ...
        if (is_xgs_mac_ext) 
        {
            SOCDNX_IF_ERR_EXIT(arad_ports_local_to_xgs_mac_extender_info_get(unit, tm_port, &xgs_mac_ext_info));
    ...
    if ((direction_ndx == ARAD_PORT_DIRECTION_OUTGOING)
        ...
        SOCDNX_IF_ERR_EXIT(arad_pp_egq_dsp_ptr_map_tbl_get_unsafe(unit, tm_port, core, &dsp_tbl_data));
        dsp_tbl_data.out_pp_port = pp_port;
        SOCDNX_IF_ERR_EXIT(arad_pp_egq_dsp_ptr_map_tbl_set_unsafe(unit, tm_port, core, &dsp_tbl_data));
  ...
  )
{
  ...
  *pp_port_in = pp_port;
   ...
   *  Outgoing
   */
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 2090, exit, READ_EGQ_DSP_PTR_MAPm(unit, MEM_BLOCK_ANY, port_ndx, dsp_ptr_map));
   ...
   )
{
  ...
  res = soc_port_sw_db_tm_port_to_base_q_pair_get(unit, local_port, 0, &base_q_pair);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 30, exit); 
  res = soc_port_sw_db_tm_port_to_out_port_priority_get(unit, local_port, 0, &nof_pairs);
   ...
   )
{
  ...
  res = (MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_port_direct_lb_key_set,(unit, local_port, min_lb_key,1,0,0)));
  SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);
   ...
   )
{
  ...
  res = (MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_port_direct_lb_key_set,(unit, local_port,0,0,max_lb_key,1)));
  SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);
    ...
    *nof_rep = 0;
    ...
    for(i=0; i<lag_size; ++i) {
        ...
        if(irr_glag_mapping_tbl_data.destination == sys_port) {
            if(i < member_id) {
                /* the number of replication is givven only for the first member ID*/ 
                break;    
            } else {
                ++(*nof_rep); 
            }
  ...
  ARAD_IRR_LAG_MAPPING_TBL_DATA
    ...
    for (i = 0; i < max_size; ++i) {
        ...
        if (irr_glag_mapping_tbl_data.destination == sys_port) {
            *member_id = i;    
            break;    
        } else if(irr_glag_mapping_tbl_data.destination == ARAD_SYS_PHYS_PORT_INVALID(unit)){
            *member_id = -1;    
            break; 
        }
    }
     ...
     */
      ...
      */
      ...
      if (fap_id == sys_fap_id_self)
      {
          ...
          res = (MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_port_direct_lb_key_set,(unit, port_id, curr_rng, 1 /* set_min */, lb_key_max, 1 /* set_max */)));
          SOC_SAND_CHECK_FUNC_RESULT(res, 42, exit);
  ...
  if (remove)
  {
    ...
    if (fap_id == sys_fap_id_self)
    {
      /* remove configuration of removed port */
        res = (MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_port_direct_lb_key_set,(unit, port_id, 0, 1 /* set_min */, 0xff, 1 /* set_max */)));
        SOC_SAND_CHECK_FUNC_RESULT(res, 42, exit);
    }
  }
  ...
  if (out_enable)
  {
    ...
    */
     ...
     */
      ...
      res = arad_pp_egq_dsp_ptr_map_tbl_get_unsafe(unit,port_id,SOC_CORE_ALL,&dsp_tbl_data);
      SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 70, exit);

      dsp_tbl_data.dst_system_port = sys_logic_port_id;

      res = arad_pp_egq_dsp_ptr_map_tbl_set_unsafe(unit,port_id,SOC_CORE_ALL,&dsp_tbl_data);
    ...
    if((lag_member->flags & SOC_PPC_LAG_MEMBER_INGRESS_DISABLE) == 0x0) {
        ...
        */
         ...
         *     range = nof_entries
         */
        if ((irr_glag_to_lag_range_tbl_data.mode == 1)
            ...
            )
        {
            ...
            irr_glag_to_lag_range_tbl_data.range = out_lag->nof_entries + 1 ; 
        }
...
#endif
            {
                range = irr_glag_to_lag_range_tbl_data.range + 1;
            } else {
                range = irr_glag_to_lag_range_tbl_data.range;
            }
  ...
  )
{
  ...
  res = _arad_ports_lag_members_get_num_of_replication(unit, lag_ndx, 0, lag_member->sys_port, in_lag->nof_entries, &member_rep);
  ...
  if (out_enable == TRUE)
  {
    ...
    if (removed_member_index != out_lag->nof_entries-1)
    {
       ...
       */    
        ...
        res = arad_sys_phys_to_local_port_map_get_unsafe(
          unit,
          irr_glag_mapping_tbl_data.destination,
          &moved_member_fap_id,
          &moved_member_port_id
        );
        SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);
        ...
        {               
         ...
         if((lag_member->flags & SOC_PPC_LAG_MEMBER_EGRESS_DISABLE) == 0x0) {
           ...
           res = arad_pp_egq_dsp_ptr_map_tbl_get_unsafe(unit, moved_member_port_id, SOC_CORE_ALL,&dsp_tbl_data);
           SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 70, exit);

           dsp_tbl_data.dst_system_port = moved_member_logic_sysport;

           res = arad_pp_egq_dsp_ptr_map_tbl_set_unsafe(unit, moved_member_port_id, SOC_CORE_ALL,&dsp_tbl_data);
           SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 80, exit);
 ...
 */        
...
#endif
        {
            irr_glag_to_lag_range_tbl_data.range = out_lag->nof_entries - 2; 
        } else {
            irr_glag_to_lag_range_tbl_data.range = out_lag->nof_entries - 1; 
        }
    }
    ...
    if (!is_stateful) {
    /* empty invalid entry */
        irr_glag_mapping_tbl_data.destination = ARAD_SYS_PHYS_PORT_INVALID(unit);
        res = arad_irr_lag_mapping_tbl_set_unsafe(
                  unit,
                  lag_ndx,
                   move_member_index,
...
{         
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 40, exit, READ_EGQ_DSP_PTR_MAPm(unit, MEM_BLOCK_ANY, port_id, dsp_ptr_map));  
...
#endif
      {
          irr_glag_to_lag_range_tbl_data.range = info->nof_entries - 1 - nof_ing_disable_members; /* fixing hw behavior */
      } else {
          irr_glag_to_lag_range_tbl_data.range = info->nof_entries - nof_ing_disable_members;
      }
...
#endif
    {
        nof_members = irr_glag_to_lag_range_tbl_data.range + 1; /* fixing hw behavior */
    } else {
        nof_members = irr_glag_to_lag_range_tbl_data.range ;
    }
    ...
    SOC_SAND_IN  ARAD_PORTS_LAG_INFO      *info)
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(0);
    ...
    res = arad_mgmt_system_fap_id_get_unsafe(unit, &sys_fap_id_self);
    SOC_SAND_CHECK_FUNC_RESULT(res, 15, exit);
  ...
  )
{
  ...
  res = soc_port_sw_db_tm_port_to_base_q_pair_get(unit, port_ndx, 0, &base_q_pair);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 10, exit);
  res = soc_port_sw_db_tm_port_to_out_port_priority_get(unit, port_ndx, 0, &nof_pairs);
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_port_egr_hdr_credit_discount_select_set_unsafe()",port_ndx,0);
}
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_port_egr_hdr_credit_discount_select_get_unsafe()",port_ndx,0);
}
    ...
    SOC_SAND_IN  uint32                              peer_tmd)
{
    ...
    res = soc_port_sw_db_tm_port_to_base_q_pair_get(unit, local_port_ndx, 0, &base_q_pair);
    SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 5, exit);

    res = arad_egq_ppct_tbl_get_unsafe(unit, base_q_pair, 0, &ppct_tbl_data);
    SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 10, exit);

    ppct_tbl_data.is_stacking_port =  is_stacking;
    ppct_tbl_data.peer_tm_domain_id =  peer_tmd;

    res = arad_egq_ppct_tbl_set_unsafe(unit, base_q_pair, SOC_CORE_ALL, &ppct_tbl_data);
    SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 20, exit);
    
    if (is_stacking) {
        res = arad_port_stacking_route_history_bitmap_set_unsafe(unit, local_port_ndx, SOC_TMC_STACK_EGR_PROG_TM_PORT_PROFILE_STACK1, 0x0);
        SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
    }
    ...
    SOC_SAND_IN  uint32                              bitmap)
{
   ...
   ARAD_EGQ_PCT_TBL_DATA
    ...
    res = soc_port_sw_db_tm_port_to_base_q_pair_get(unit, local_port_ndx, 0, &base_q_pair);
    SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 5, exit);
    res = soc_port_sw_db_tm_port_to_out_port_priority_get(unit, local_port_ndx, 0, &nof_pairs);
...
uint32 alloc_reassembly_context_and_recycle_channel_unsafe(int unit, uint32 out_port, uint32 *out_context, uint32 *out_channel)
{
  int rv, port;
  uint32 res;
   ...
   */
  res = arad_port_pp_port_get_unsafe(unit, out_port, 0, &out_info);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 40, exit);

  res = arad_port_header_type_get_unsafe(unit, out_port, &header_type_incoming, &header_type_outgoing);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 50, exit);
  ...
  rv = arad_port_pp_port_set_unsafe(unit, port, 0, &in_info, &success);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 60, exit);
   ...
   */
  if (header_type_outgoing == ARAD_PORT_HEADER_TYPE_ETH && out_info.header_type_out == ARAD_PORT_HEADER_TYPE_XGS_MAC_EXT) 
  {  
  ...
  rv = arad_port_header_type_set_unsafe(unit, port, ARAD_PORT_DIRECTION_INCOMING, new_header_type_incoming);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 70, exit);
   ...
   )
{
  ...
  res = (MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_port_direct_lb_key_get, (unit, local_port, min_lb_key, NULL)));
  SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);
   ...
   )
{
  ...
  res = (MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_port_direct_lb_key_get, (unit, local_port, NULL, max_lb_key)));
  SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);
    ...
    )
{
  ...
  for(port_i=0; port_i < ARAD_PORT_NDX_MAX ; ++port_i) {
     ...
     */
    res = (MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_port_direct_lb_key_get, (unit, port_i, &min_lb_key, &max_lb_key)));
    SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);

    res = (MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_port_direct_lb_key_set,(unit, port_i, min_lb_key, TRUE, max_lb_key, TRUE)));
    SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);
    ...
    SOC_SAND_IN  uint32     port)
{
    ...
    res = arad_pp_egq_dsp_ptr_map_tbl_set_unsafe(unit,port,SOC_CORE_ALL,&dsp_tbl_data);
    SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 30, exit);
    ...
    SOC_SAND_OUT uint32     enable)
{
    ...
    if(enable) 
    {
        ...
        res = arad_pp_egq_dsp_ptr_map_tbl_get_unsafe(unit,tm_port,SOC_CORE_ALL,&dsp_tbl_data);
        SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 100, enable_shapers);

        dsp_tbl_data.out_tm_port = field_val;

        res = arad_pp_egq_dsp_ptr_map_tbl_set_unsafe(unit,tm_port,SOC_CORE_ALL,&dsp_tbl_data);
...
enable_shapers:
       ...
       if(is_channelized)
       {
           res=arad_ofp_rates_egq_interface_shaper_set(unit,tm_port,SOC_TMC_OFP_SHPR_UPDATE_MODE_OVERRIDE,if_spr_reg_val);
    ...
    )
{
  ...
  res = arad_egq_pct_tbl_get_unsafe(unit, base_q_pair, 0, &pct_tbl_data);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 30, exit);
    ...
    )
{
  ...
  res = arad_port_header_type_set_unsafe(unit,index,ARAD_PORT_DIRECTION_OUTGOING,ARAD_PORT_HEADER_TYPE_L2_REMOTE_CPU);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 10, exit);

  res = arad_port_pp_port_get_unsafe(unit, index, 0, &pp_port_info);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 12, exit);

  pp_port_info.header_type_out = ARAD_PORT_HEADER_TYPE_L2_REMOTE_CPU;

  res = arad_port_pp_port_set_unsafe(unit,index,0,&pp_port_info,&success);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 15, exit);
    ...
    ) 
{
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 20, exit, READ_IHB_PINFO_LBPm(unit, MEM_BLOCK_ANY, port_ndx, data));
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 30, exit, WRITE_IHB_PINFO_LBPm(unit, MEM_BLOCK_ANY, port_ndx, data));
    ...
    SOC_SAND_IN ARAD_PORTS_APPLICATION_MAPPING_INFO *info)
{
  ...
  if (info->type == ARAD_PORTS_APPLICATION_MAPPING_TYPE_PP_PORT_EXTENDER) {
    /* 
        ...
        */
    res = READ_IHP_VIRTUAL_PORT_TABLEm(unit, MEM_BLOCK_ANY, info->value.pp_port_ext.index, &tbl_data_32b);
    SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 100, exit);
    
    soc_IHP_VIRTUAL_PORT_TABLEm_field32_set(unit, &tbl_data_32b, IN_PP_PORTf, info->value.pp_port_ext.pp_port);

    res = WRITE_IHP_VIRTUAL_PORT_TABLEm(unit, MEM_BLOCK_ANY, info->value.pp_port_ext.index, &tbl_data_32b);
    ...
    SOC_SAND_IN ARAD_PORTS_APPLICATION_MAPPING_INFO *info)
{
  ...
  if (info->type == ARAD_PORTS_APPLICATION_MAPPING_TYPE_PP_PORT_EXTENDER) 
  {
    ...
    if (PBMP_MEMBER(vendor_pp_port_bm, port_ndx)) {
        SOC_SAND_ERR_IF_ABOVE_MAX(
        (info->value.pp_port_ext.index&0x3F), ARAD_PORTS_VENDOR_PP_SOURCE_PORT_MAX,
    ...
    SOC_SAND_INOUT ARAD_PORTS_APPLICATION_MAPPING_INFO *info)
{
  ...
  if (info->type == ARAD_PORTS_APPLICATION_MAPPING_TYPE_XGS_MAC_EXTENDER) 
  {
     ...
     * Retreive Higig Port
     */
    res = arad_ports_local_to_xgs_mac_extender_info_get(unit, port_ndx, &mac_ext_info);
    SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 20, exit);
   ...
   )
{
    ...
    if (interface != SOC_PORT_IF_ERP) {
        ...
        SOCDNX_IF_ERR_EXIT(arad_parser_nof_bytes_to_remove_set(unit, tm_port, core, first_header_size));

        SOCDNX_IF_ERR_EXIT(arad_port_header_type_set_unsafe(unit,port,ARAD_PORT_DIRECTION_INCOMING,header_type_in));
        SOCDNX_IF_ERR_EXIT(arad_port_header_type_set_unsafe(unit,port,ARAD_PORT_DIRECTION_OUTGOING,header_type_out));
        ...
        if (header_type_out == ARAD_PORT_HEADER_TYPE_TM) {
            ...
            SOCDNX_IF_ERR_EXIT(arad_ports_otmh_extension_set(unit,port,&otmh_ext_en));
        }
        ...
        SOCDNX_IF_ERR_EXIT(arad_port_pp_port_set_unsafe(unit, pp_port, core, &conf,&success));
