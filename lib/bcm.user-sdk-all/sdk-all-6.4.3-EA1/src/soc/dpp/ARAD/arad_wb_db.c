...
#define ARAD_WB_DB_LAG_LOCAL_TO_SYS_NUM        \
...
typedef struct arad_wb_db_lag_local_to_sys_config_s {
    uint32     local_to_sys;
}  __ATTRIBUTE_PACKED__ arad_wb_db_lag_local_to_sys_config_t;
...
#define ARAD_WB_DB_LAYOUT_INIT_VARIABLE(_module, _var, _var_num)    \
            ...
            ARAD_WB_DB_LAYOUT_INIT_VARIABLE(lag, 
                                            local_to_sys, 
                                            ARAD_WB_DB_LAG_LOCAL_TO_SYS_NUM);
...
STATIC uint32
_arad_wb_db_lag_restore_local_to_sys_state(SOC_SAND_IN int unit)
{
    ARAD_WB_DB_RESTORE_ARR_STATE(ARAD_WB_DB_LAG_RESTORE_LOCAL_TO_SYS_STATE,
                                 lag, 
                                 local_to_sys,
                                 lag.local_to_sys[arr_indx]);
}
...
_arad_wb_db_restore_modport2sysport_state(SOC_SAND_IN int unit)
{
    ...
    if (!ARAD_IS_VOQ_MAPPING_INDIRECT(unit)) {
        res = arad_populate_sw_db_modport2sysport_from_hw_unsafe(unit);
        SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
    }

exit:
    SOC_SAND_EXIT_AND_SEND_ERROR("error in _arad_wb_db_restore_modport2sysport_state()", 0, 0);
}
...
_arad_wb_db_restore_state(SOC_SAND_IN int unit)
{
    ...
    res = _arad_wb_db_lag_restore_local_to_sys_state(unit);
...
STATIC uint32
_arad_wb_db_lag_save_local_to_sys_state(SOC_SAND_IN int unit)
{
    ARAD_WB_DB_SAVE_ARR_STATE(ARAD_WB_DB_LAG_SAVE_LOCAL_TO_SYS_STATE,
                              lag, 
                              local_to_sys, 
                              lag.local_to_sys[arr_indx]);
}
...
arad_wb_db_sync(SOC_SAND_IN int unit)
{
    ...
    res = _arad_wb_db_lag_save_local_to_sys_state(unit);
    SOC_SAND_CHECK_FUNC_RESULT(res, 110, exit);
...
arad_wb_db_lag_update_local_to_sys_state(SOC_SAND_IN int unit, 
                                         SOC_SAND_IN uint32 arr_indx)
{
    ARAD_WB_DB_UPDATE_ARR_STATE(ARAD_WB_DB_LAG_UPDATE_LOCAL_TO_SYS_STATE,
                                lag, 
                                local_to_sys,
                                lag.local_to_sys[arr_indx],
                                arr_indx);
