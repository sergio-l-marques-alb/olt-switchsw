...
soc_fe1600_fabric_links_aldwp_config(int unit, soc_port_t port)
{
    ...
    highest_aldwp = 0;
    dch_instance = port / SOC_DFE_DEFS_GET(unit, nof_links_in_dcq);
    SOCDNX_IF_ERR_EXIT(MBCM_DFE_DRIVER_CALL(unit,mbcm_dfe_fabric_link_device_mode_get,(unit,port, 1/*rx*/, &device_mode)));
    for (port_index = dch_instance * SOC_DFE_DEFS_GET(unit, nof_links_in_dcq); 
          port_index < (dch_instance + 1) * SOC_DFE_DEFS_GET(unit, nof_links_in_dcq); 
          port_index++)
    {
