...
{
    uint32 soc_sand_rv;
    uint32 event_enable = llm_appl_info[unit].event_enable;
    ...
    soc_sand_rv = soc_sand_bitstream_set(&event_enable, event_id, value);
    return soc_sand_rv;
}
...
int shr_llm_mac_move_event_get(int unit, uint32 event_id, uint32 *value)
{
    ...
    soc_sand_rv = soc_sand_bitstream_get_field(&event_enable, event_id, 1, value);
    return soc_sand_rv;
}
...
int shr_llm_msg_pon_attr_set(int unit, uint32 serv_type, uint32 data_type, void *data)
{
    ...
    msg.data_len  = sizeof(shr_llm_msg_pon_att_data_t);
    switch (data_type) {      
    ...
    /* Pack control message data into DMA buffer */
    buffer     = llm_appl_info[unit].dma_buffer;
    shr_llm_msg_pon_att_update_pack(buffer, &msg);
    send_len = sizeof(shr_llm_msg_pon_att_update_t);
