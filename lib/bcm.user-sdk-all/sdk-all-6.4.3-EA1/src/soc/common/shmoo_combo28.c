...
uint32
_shmoo_combo28_check_dram(int drc_ndx)
{
...
_shmoo_combo28_record_dq_swap(int unit, int drc_ndx)
{
    ...
    for(byte = 0; byte < SHMOO_COMBO28_BYTES_PER_INTERFACE; byte++)
    {
        for(bit = 0; bit < SHMOO_COMBO28_BYTE; bit++)
        {
            if(shmoo_dram_info.dq_swap[drc_ndx][byte][bit] != SHMOO_COMBO28_DQ_BIT_NOT_SWAPPED)
            {
...
STATIC int
_initialize_bist(int unit, int drc_ndx, int bit, combo28_shmoo_container_t *scPtr, combo28_bist_info_t *biPtr, combo28_gddr5_bist_info_t *gbiPtr)
{
    uint32 i;
    uint8 dbi_pattern;
    uint8 edc_pattern;
    ...
    switch ((*scPtr).shmooType) {
        ...
        case SHMOO_COMBO28_RD_START_FISH:
            if((*scPtr).dramType == SHMOO_COMBO28_DRAM_TYPE_GDDR5)
            {
                ...
                for(i = 0; i < SHMOO_COMBO28_BIST_NOF_PATTERNS; i++)
                {
                    COMPILER_64_SET((*gbiPtr).data_pattern[i], _get_random28(), _get_random28());
                    (*gbiPtr).dbi_pattern[i]  = (uint8) (_get_random28() & 0xFF);
                    (*gbiPtr).edc_pattern[i]  = (uint8) (_get_random28() & 0xFF);
                }
        ...
        case SHMOO_COMBO28_RD_SHORT:
            ...
            else if((*scPtr).dramType == SHMOO_COMBO28_DRAM_TYPE_DDR4)
            {
                ...
                {
                    (*biPtr).mpr_pattern[i] = (uint8) (_get_random28() & 0xFF);
                }
...
STATIC int
_run_bist(int unit, int drc_ndx, combo28_shmoo_container_t *scPtr, combo28_bist_info_t *biPtr, combo28_gddr5_bist_info_t *gbiPtr, combo28_shmoo_error_array_t *seaPtr)
{
    uint32 i;
    ...
    switch ((*scPtr).shmooType) {
        case SHMOO_COMBO28_ADDR_CTRL_SHORT:
            if((*scPtr).dramType == SHMOO_COMBO28_DRAM_TYPE_GDDR5)
            {
                ...
                (*seaPtr)[0] = gbe.bist_adt_err_occur;
            }
            else
            {
                LOG_ERROR(BSL_LS_SOC_DDR,
...
STATIC uint32
_test_interface(int unit, int drc_ndx, combo28_shmoo_container_t *scPtr, uint32 clear)
{
    uint32 sizeX;
    uint32 sizeY;
    uint32 yCapMin;
...
_shmoo_combo28_wck2ck(int unit, int drc_ndx, combo28_shmoo_container_t *scPtr)
{
    ...
    (*scPtr).sizeX = SHMOO_COMBO28_MAX_VDL_LENGTH;
    (*scPtr).sizeY = 1;
    (*scPtr).yCapMin = 0;
    (*scPtr).yCapMax = 1;
    ...
    for(x = 0; x < SHMOO_COMBO28_MAX_VDL_LENGTH; x++)
    {
        READ_DDRC28_DQ_BYTE0_WRITE_MAX_VDL_DQSr(unit, drc_ndx, &data0);
        soc_phy_ddrc28_reg_field_set(unit, SOC_PHY_REG_DQ_BYTE0_WRITE_MAX_VDL_DQSr, &data0, SOC_PHY_FLD_MAX_VDL_STEPf, x);
        ...
        if(result0 == SHMOO_COMBO28_REPEAT)
        {
            wckInv &= 0xFFFFFFFB;
            result |= 0x0000FFFF;
        }
        ...
        if(result2 == SHMOO_COMBO28_REPEAT)
        {
            wckInv &= 0xFFFFFFF7;
            result |= 0xFFFF0000;
        }
...
/*    uint32 temp; */
    ...
    _initialize_bist(unit, drc_ndx, -1, scPtr, &bi, NULL);
    ...
    {        
        ...
        for(x = 0; x < (((*scPtr).endUI[1] + 1) >> jump); x++)
        {
            ...
            _run_bist(unit, drc_ndx, scPtr, &bi, NULL, NULL);
...
_shmoo_combo28_rd_valid_fish(int unit, int drc_ndx, combo28_shmoo_container_t *scPtr)
{
    ...
    for(d = 0; d < 32; d++)
    {
        for(x = 0; x < (SHMOO_COMBO28_MAX_VDL_LENGTH >> jump); x++)
        {
            ...
            data = _test_interface(unit, drc_ndx, scPtr, 1);
...
_shmoo_combo28_rd_start_fish(int unit, int drc_ndx, combo28_shmoo_container_t *scPtr)
{
    ...
    _initialize_bist(unit, drc_ndx, -1, scPtr, NULL, &gbi);
    ...
    {        
        ...
        for(x = 0; x < (((*scPtr).endUI[2] + 1) >> jump); x++)
        {
            READ_DDRC28_DQ_BYTE0_READ_MAX_VDL_DQSPr(unit, drc_ndx, &data);
            soc_phy_ddrc28_reg_field_set(unit, SOC_PHY_REG_DQ_BYTE0_READ_MAX_VDL_DQSPr, &data, SOC_PHY_FLD_MAX_VDL_STEPf, x << jump);
            ...
            _run_bist(unit, drc_ndx, scPtr, NULL, &gbi, &sea);
    ...
    temp100 = (*scPtr).size1000UI / 1000;
    temp200 = ((*scPtr).size1000UI * 2) / 1000;
    
    data = 0;
    soc_phy_ddrc28_reg_field_set(unit, SOC_PHY_REG_DQ_BYTE0_READ_MAX_VDL_FSMr, &data, SOC_PHY_FLD_MAX_VDL_STEPf, (fishSite0 + temp100) % temp200);
    WRITE_DDRC28_DQ_BYTE0_READ_MAX_VDL_FSMr(unit, drc_ndx, data);
    
    soc_phy_ddrc28_reg_field_set(unit, SOC_PHY_REG_DQ_BYTE1_READ_MAX_VDL_FSMr, &data, SOC_PHY_FLD_MAX_VDL_STEPf, (fishSite1 + temp100) % temp200);
    WRITE_DDRC28_DQ_BYTE1_READ_MAX_VDL_FSMr(unit, drc_ndx, data);
    
    soc_phy_ddrc28_reg_field_set(unit, SOC_PHY_REG_DQ_BYTE2_READ_MAX_VDL_FSMr, &data, SOC_PHY_FLD_MAX_VDL_STEPf, (fishSite2 + temp100) % temp200);
    WRITE_DDRC28_DQ_BYTE2_READ_MAX_VDL_FSMr(unit, drc_ndx, data);
    
    soc_phy_ddrc28_reg_field_set(unit, SOC_PHY_REG_DQ_BYTE3_READ_MAX_VDL_FSMr, &data, SOC_PHY_FLD_MAX_VDL_STEPf, (fishSite3 + temp100) % temp200);
...
_shmoo_combo28_edc_start_fish(int unit, int drc_ndx, combo28_shmoo_container_t *scPtr)
{
    ...
    {        
        if(fish0)
        {
            ...
            soc_phy_ddrc28_reg_field_set(unit, SOC_PHY_REG_DQ_BYTE0_EDCEN_FIFO_CONFIGr, &data, SOC_PHY_FLD_RD_2G_DELAYf, d);
            WRITE_DDRC28_DQ_BYTE0_EDCEN_FIFO_CONFIGr(unit, drc_ndx, data);
        }
        if(fish1)
        {
            READ_DDRC28_DQ_BYTE1_EDCEN_FIFO_CONFIGr(unit, drc_ndx, &data);
            soc_phy_ddrc28_reg_field_set(unit, SOC_PHY_REG_DQ_BYTE1_EDCEN_FIFO_CONFIGr, &data, SOC_PHY_FLD_RD_2G_DELAYf, d);
            WRITE_DDRC28_DQ_BYTE1_EDCEN_FIFO_CONFIGr(unit, drc_ndx, data);
        }
        if(fish2)
        {
            READ_DDRC28_DQ_BYTE2_EDCEN_FIFO_CONFIGr(unit, drc_ndx, &data);
            soc_phy_ddrc28_reg_field_set(unit, SOC_PHY_REG_DQ_BYTE2_EDCEN_FIFO_CONFIGr, &data, SOC_PHY_FLD_RD_2G_DELAYf, d);
            WRITE_DDRC28_DQ_BYTE2_EDCEN_FIFO_CONFIGr(unit, drc_ndx, data);
        }
        if(fish3)
        {
            READ_DDRC28_DQ_BYTE3_EDCEN_FIFO_CONFIGr(unit, drc_ndx, &data);
            soc_phy_ddrc28_reg_field_set(unit, SOC_PHY_REG_DQ_BYTE3_EDCEN_FIFO_CONFIGr, &data, SOC_PHY_FLD_RD_2G_DELAYf, d);
...
_shmoo_combo28_rd_short_fish(int unit, int drc_ndx, combo28_shmoo_container_t *scPtr)
{
    ...
    {        
        READ_DDRC28_DQ_BYTE0_REN_FIFO_CONFIGr(unit, drc_ndx, &data);
        soc_phy_ddrc28_reg_field_set(unit, SOC_PHY_REG_DQ_BYTE0_REN_FIFO_CONFIGr, &data, SOC_PHY_FLD_RD_2G_DELAYf, y);
        WRITE_DDRC28_DQ_BYTE0_REN_FIFO_CONFIGr(unit, drc_ndx, data);
        WRITE_DDRC28_DQ_BYTE1_REN_FIFO_CONFIGr(unit, drc_ndx, data);
        WRITE_DDRC28_DQ_BYTE2_REN_FIFO_CONFIGr(unit, drc_ndx, data);
        WRITE_DDRC28_DQ_BYTE3_REN_FIFO_CONFIGr(unit, drc_ndx, data);
...
_shmoo_combo28_wr_deskew(int unit, int drc_ndx, combo28_shmoo_container_t *scPtr)
{
    ...
    {
        _initialize_bist(unit, drc_ndx, b, scPtr, &bi, &gbi);
        ...
        for(x = 0; x < (*scPtr).sizeX; x++)
        {
            ...
            _run_bist(unit, drc_ndx, scPtr, &bi, &gbi, &sea);
...
_shmoo_combo28_dbi_wr_deskew(int unit, int drc_ndx, combo28_shmoo_container_t *scPtr)
{
    ...
    _initialize_bist(unit, drc_ndx, -1, scPtr, &bi, &gbi);
...
_shmoo_combo28_wr_extended(int unit, int drc_ndx, combo28_shmoo_container_t *scPtr)
{
    ...
    switch(dramType)
    {
        case SHMOO_COMBO28_DRAM_TYPE_DDR4:
            ...
            (*scPtr).sizeY = 74;    /* Actual Vref Range crossover at SHMOO_COMBO28_DDR4_VREF_RANGE_CROSSOVER */
            break;
    ...
    for(y = yCapMin; y < yCapMax; y++)
    {
        switch(dramType)
        {
            ...
            case SHMOO_COMBO28_DRAM_TYPE_GDDR5:
                ...
                break;
            /* must default. Otherwize - compilation error */
        ...
        for(x = 0; x < (*scPtr).sizeX; x++)
        {
            ...
            _run_bist(unit, drc_ndx, scPtr, &bi, NULL, &sea);
...
_shmoo_combo28_dbi_edc_rd_deskew(int unit, int drc_ndx, combo28_shmoo_container_t *scPtr)
{
    ...
    switch (data) {
        ...
        default:
            LOG_ERROR(BSL_LS_SOC_DDR,
                      (BSL_META_U(unit,
                                  "RD_DESKEW shmoo has not been run, or Macro Reserved Registers have been tempered with.\n")));
            return SOC_E_FAIL;
    }
...
_shmoo_combo28_rd_extended(int unit, int drc_ndx, combo28_shmoo_container_t *scPtr)
{
    ...
    (*scPtr).engageUIshift = 0;
    (*scPtr).sizeY = SHMOO_COMBO28_MAX_VREF_RANGE;
    (*scPtr).yCapMin = yCapMin;
    (*scPtr).yCapMax = yCapMax;
...
STATIC int
_calib_2D(int unit, int drc_ndx, combo28_shmoo_container_t *scPtr, uint32 calibMode, uint32 calibPos)
{
    uint32 x;
    uint32 y;
    uint32 xStart;
...
_shmoo_combo28_calib_2D(int unit, int drc_ndx, combo28_shmoo_container_t *scPtr)
{
    switch ((*scPtr).shmooType) {
        case SHMOO_COMBO28_ADDR_CTRL_SHORT:
            return _calib_2D(unit, drc_ndx, scPtr, SHMOO_COMBO28_WORD, SHMOO_COMBO28_CALIB_CENTER_PASS);
        case SHMOO_COMBO28_DQS2CK:
            return _calib_2D(unit, drc_ndx, scPtr, SHMOO_COMBO28_BYTE, SHMOO_COMBO28_CALIB_RISING_EDGE);
        case SHMOO_COMBO28_WCK2CK:
            return _calib_2D(unit, drc_ndx, scPtr, SHMOO_COMBO28_HALFWORD, SHMOO_COMBO28_CALIB_FALLING_EDGE);
        case SHMOO_COMBO28_RD_EN_FISH:
            break;
        case SHMOO_COMBO28_RD_VALID_FISH:
            break;
        ...
        case SHMOO_COMBO28_EDC_START_FISH:
            break;
        case SHMOO_COMBO28_RD_SHORT_FISH:
            return _calib_2D(unit, drc_ndx, scPtr, SHMOO_COMBO28_BYTE, SHMOO_COMBO28_CALIB_CENTER_PASS);
        case SHMOO_COMBO28_RD_DESKEW:
            return _calib_2D(unit, drc_ndx, scPtr, SHMOO_COMBO28_BIT, SHMOO_COMBO28_CALIB_CENTER_PASS);
        case SHMOO_COMBO28_DBI_EDC_RD_DESKEW:
            return _calib_2D(unit, drc_ndx, scPtr, SHMOO_COMBO28_BIT, SHMOO_COMBO28_CALIB_CENTER_PASS);
        case SHMOO_COMBO28_RD_SHORT:
            return _calib_2D(unit, drc_ndx, scPtr, SHMOO_COMBO28_BYTE, SHMOO_COMBO28_CALIB_CENTER_PASS);
...
STATIC int
_calculate_ui_position(uint32 linearPos, combo28_shmoo_container_t *scPtr, combo28_ui_position_t *upPtr)
{
    uint32 ui;
    uint32 position;
        ...
        case SHMOO_COMBO28_WCK2CK:
            switch(calibMode)
            {
                ...
                case SHMOO_COMBO28_BYTE:
                    ...
                    return SOC_E_FAIL;
            case SHMOO_COMBO28_HALFWORD:
                    val0 = ((*scPtr).resultData[0] & 0xFFFF); /* + SHMOO_COMBO28_WCK2CK_OFFSET; */
                    ...
                    val2 = ((*scPtr).resultData[1] & 0xFFFF); /* + SHMOO_COMBO28_WCK2CK_OFFSET; */
        ...
        case SHMOO_COMBO28_WR_DESKEW:
            switch(calibMode)
            {
                case SHMOO_COMBO28_BIT:
                    ...
                    if(engageUIshift)
                    {
                        _calculate_ui_position(min0 - initAdjustment, scPtr, &up);
                        ...
                        _calculate_ui_position(min1 - initAdjustment, scPtr, &up);
                        ...
                        _calculate_ui_position(min2 - initAdjustment, scPtr, &up);
                        ...
                        _calculate_ui_position(min3 - initAdjustment, scPtr, &up);
...
STATIC int
_plot(int unit, int drc_ndx, combo28_shmoo_container_t *scPtr, uint32 plotMode)
{
    uint32 x;
    uint32 y;
    uint32 xStart;
...
_shmoo_combo28_plot(int unit, int drc_ndx, combo28_shmoo_container_t *scPtr)
{
    switch ((*scPtr).shmooType) {
        case SHMOO_COMBO28_ADDR_CTRL_SHORT:
            return _plot(unit, drc_ndx, scPtr, SHMOO_COMBO28_WORD);
        case SHMOO_COMBO28_DQS2CK:
            return _plot(unit, drc_ndx, scPtr, SHMOO_COMBO28_BYTE);
        case SHMOO_COMBO28_WCK2CK:
            return _plot(unit, drc_ndx, scPtr, SHMOO_COMBO28_HALFWORD);
        case SHMOO_COMBO28_RD_EN_FISH:
            break;
        case SHMOO_COMBO28_RD_VALID_FISH:
            break;
        ...
        case SHMOO_COMBO28_RD_SHORT_FISH:
            return _plot(unit, drc_ndx, scPtr, SHMOO_COMBO28_BYTE);
        case SHMOO_COMBO28_RD_DESKEW:
            return _plot(unit, drc_ndx, scPtr, SHMOO_COMBO28_BIT);
        case SHMOO_COMBO28_DBI_EDC_RD_DESKEW:
            return _plot(unit, drc_ndx, scPtr, SHMOO_COMBO28_BIT);
        case SHMOO_COMBO28_RD_SHORT:
            return _plot(unit, drc_ndx, scPtr, SHMOO_COMBO28_BIT);
...
int
_calculate_step_size(int unit, int drc_ndx, combo28_step_size_t *ssPtr)
{
    uint32 data;
    ...
    switch ((*scPtr).shmooType) {
        case SHMOO_COMBO28_ADDR_CTRL_SHORT:
            if((*scPtr).dramType == SHMOO_COMBO28_DRAM_TYPE_GDDR5)
            {
                ...
                _calculate_step_size(unit, drc_ndx, &ss);
...
int
soc_combo28_shmoo_ctl(int unit, int drc_ndx, int shmoo_type, int stat, int plot, int action, combo28_shmoo_config_param_t *config_param)
{
    ...
    if(!stat)
    {
        ...
        for(; ndx < ndxEnd; ndx++)
        {
            ...
            else if((action == SHMOO_COMBO28_ACTION_RUN) || (action == SHMOO_COMBO28_ACTION_RUN_AND_SAVE))
            {
                switch(ctlType)
                {
                    case SHMOO_COMBO28_CTL_TYPE_RSVP:
...
soc_combo28_shmoo_phy_cfg_pll(int unit, int drc_ndx)
{
    ...
    int pdiv10, ndiv_int10, vco_gain10;
    uint32 dramType;
    ...
    if(freq_vco >= 4320)
    {
        vco_gain10 = ((-18 * freq_vco) + 260000) / 3000;
    }
    else
    {
        vco_gain10 = ((-83 * freq_vco) + 420000) / 1000;
    }
    
    if((vco_gain10 % 10) >= 5)
    {
        pll_info.vco_gain = (vco_gain10 / 10) + 1;
    }
    else
    {
        pll_info.vco_gain = (vco_gain10 / 10);
    }
    
    differential_ref_clk_mode = 1;                                                  /* Reference clock: Differential */
    pll_info.msc_ctrl = (0x0020 | (0x2 << differential_ref_clk_mode) | pll_info.div2_mode);
    
    COMPILER_64_SET(pll_info.pll_ctrl, 0x00805200, 0x00000020);
    
    if(freq_vco >= 4450)
    {
        uint64 tmp;
        COMPILER_64_SET(tmp, 0x00000000, 0x00000200);
...
/*A06*/                         "A06. Compute VDL step size\n")));
        _calculate_step_size(unit, ndx, &ss);
        
        step1000 = ss.step1000;
        size1000UI = ss.size1000UI;
        LOG_VERBOSE(BSL_LS_SOC_DDR,
                    (BSL_META_U(unit,
                                "     VDL step size........: %3d.%0d ps\n"),
                     (step1000 / 1000), (step1000 % 1000)));
        LOG_VERBOSE(BSL_LS_SOC_DDR,
                    (BSL_META_U(unit,
                                "     UI size..............: %3d.%0d steps\n"),
...
/*A11*/                         "A11. ZQ calibration\n")));
        ...
        do
        {
            ...
            {
                if(soc_phy_ddrc28_reg_field_get(unit, SOC_PHY_REG_COMMON_STATUS_RESCALr, data, SOC_PHY_FLD_COMP_ERRORf))
                {
                    LOG_ERROR(BSL_LS_SOC_DDR,
                              (BSL_META_U(unit,
                                          "     WARNING: ZQ calibration error - Manual IO programming required for correct operation\n")));
                    /* return SOC_E_FAIL; */
                }
                break;
            }
        ...
        soc_phy_ddrc28_reg_field_set(unit, SOC_PHY_REG_COMMON_IO_CONFIGr, &data, SOC_PHY_FLD_PDf, temp1);
        soc_phy_ddrc28_reg_field_set(unit, SOC_PHY_REG_COMMON_IO_CONFIGr, &data, SOC_PHY_FLD_PTERMf, temp1);
        soc_phy_ddrc28_reg_field_set(unit, SOC_PHY_REG_COMMON_IO_CONFIGr, &data, SOC_PHY_FLD_NDf, temp2);
        soc_phy_ddrc28_reg_field_set(unit, SOC_PHY_REG_COMMON_IO_CONFIGr, &data, SOC_PHY_FLD_NTERMf, temp2);
        WRITE_DDRC28_COMMON_IO_CONFIGr(unit, ndx, data);

        READ_DDRC28_AQ_IO_CONFIGr(unit, ndx, &data);
        soc_phy_ddrc28_reg_field_set(unit, SOC_PHY_REG_AQ_IO_CONFIGr, &data, SOC_PHY_FLD_PDf, temp1);
        soc_phy_ddrc28_reg_field_set(unit, SOC_PHY_REG_AQ_IO_CONFIGr, &data, SOC_PHY_FLD_PTERMf, temp1);
        soc_phy_ddrc28_reg_field_set(unit, SOC_PHY_REG_AQ_IO_CONFIGr, &data, SOC_PHY_FLD_NDf, temp2);
        soc_phy_ddrc28_reg_field_set(unit, SOC_PHY_REG_AQ_IO_CONFIGr, &data, SOC_PHY_FLD_NTERMf, temp2);
        WRITE_DDRC28_AQ_IO_CONFIGr(unit, ndx, data);

        READ_DDRC28_DQ_BYTE0_DATA_IO_CONFIGr(unit, ndx, &data);
        soc_phy_ddrc28_reg_field_set(unit, SOC_PHY_REG_DQ_BYTE0_DATA_IO_CONFIGr, &data, SOC_PHY_FLD_PDf, temp1);
        soc_phy_ddrc28_reg_field_set(unit, SOC_PHY_REG_DQ_BYTE0_DATA_IO_CONFIGr, &data, SOC_PHY_FLD_PTERMf, temp1);
        soc_phy_ddrc28_reg_field_set(unit, SOC_PHY_REG_DQ_BYTE0_DATA_IO_CONFIGr, &data, SOC_PHY_FLD_NDf, temp2);
        soc_phy_ddrc28_reg_field_set(unit, SOC_PHY_REG_DQ_BYTE0_DATA_IO_CONFIGr, &data, SOC_PHY_FLD_NTERMf, temp2);
        ...
        soc_phy_ddrc28_reg_field_set(unit, SOC_PHY_REG_DQ_BYTE0_DQS_IO_CONFIGr, &data, SOC_PHY_FLD_PDf, temp1);
        soc_phy_ddrc28_reg_field_set(unit, SOC_PHY_REG_DQ_BYTE0_DQS_IO_CONFIGr, &data, SOC_PHY_FLD_PTERMf, temp1);
        soc_phy_ddrc28_reg_field_set(unit, SOC_PHY_REG_DQ_BYTE0_DQS_IO_CONFIGr, &data, SOC_PHY_FLD_NDf, temp2);
        soc_phy_ddrc28_reg_field_set(unit, SOC_PHY_REG_DQ_BYTE0_DQS_IO_CONFIGr, &data, SOC_PHY_FLD_NTERMf, temp2);
...
/*A13*/                         "A13. Configure FIFOs\n")));
        ...
        data = 0;
        soc_phy_ddrc28_reg_field_set(unit, SOC_PHY_REG_DQ_BYTE0_REN_FIFO_CONFIGr, &data, SOC_PHY_FLD_RD2_2G_DELAYf, 4);
        WRITE_DDRC28_DQ_BYTE0_REN_FIFO_CONFIGr(unit, drc_ndx, data);
        WRITE_DDRC28_DQ_BYTE1_REN_FIFO_CONFIGr(unit, drc_ndx, data);
        WRITE_DDRC28_DQ_BYTE2_REN_FIFO_CONFIGr(unit, drc_ndx, data);
        WRITE_DDRC28_DQ_BYTE3_REN_FIFO_CONFIGr(unit, drc_ndx, data);
        ...
        data = 0;
        soc_phy_ddrc28_reg_field_set(unit, SOC_PHY_REG_DQ_BYTE0_EDCEN_FIFO_CONFIGr, &data, SOC_PHY_FLD_RD2_2G_DELAYf, 4);
        WRITE_DDRC28_DQ_BYTE0_EDCEN_FIFO_CONFIGr(unit, drc_ndx, data);
        WRITE_DDRC28_DQ_BYTE1_EDCEN_FIFO_CONFIGr(unit, drc_ndx, data);
        WRITE_DDRC28_DQ_BYTE2_EDCEN_FIFO_CONFIGr(unit, drc_ndx, data);
        WRITE_DDRC28_DQ_BYTE3_EDCEN_FIFO_CONFIGr(unit, drc_ndx, data);
            ...
            else
            {
                if((*config_param).reset_n == 1)
                {
                    ...
                    if(!field)
                    {
                        ...
                        _calculate_step_size(unit, drc_ndx, &ss);
                        step2UI = ((ss.size1000UI) << 1) / 1000;
                        
                        READ_DDRC28_DQ_BYTE0_CDR_VDL_COMMON_CONFIGr(unit, ndx, &data);
                        soc_phy_ddrc28_reg_field_set(unit, SOC_PHY_REG_DQ_BYTE0_CDR_VDL_COMMON_CONFIGr, &data, SOC_PHY_FLD_VDL_VALUE_CAPf, step2UI);
                        WRITE_DDRC28_DQ_BYTE0_CDR_VDL_COMMON_CONFIGr(unit, ndx, data);
                        READ_DDRC28_DQ_BYTE1_CDR_VDL_COMMON_CONFIGr(unit, ndx, &data);
                        soc_phy_ddrc28_reg_field_set(unit, SOC_PHY_REG_DQ_BYTE1_CDR_VDL_COMMON_CONFIGr, &data, SOC_PHY_FLD_VDL_VALUE_CAPf, step2UI);
                        WRITE_DDRC28_DQ_BYTE1_CDR_VDL_COMMON_CONFIGr(unit, ndx, data);
                        READ_DDRC28_DQ_BYTE2_CDR_VDL_COMMON_CONFIGr(unit, ndx, &data);
                        soc_phy_ddrc28_reg_field_set(unit, SOC_PHY_REG_DQ_BYTE2_CDR_VDL_COMMON_CONFIGr, &data, SOC_PHY_FLD_VDL_VALUE_CAPf, step2UI);
                        WRITE_DDRC28_DQ_BYTE2_CDR_VDL_COMMON_CONFIGr(unit, ndx, data);
                        READ_DDRC28_DQ_BYTE3_CDR_VDL_COMMON_CONFIGr(unit, ndx, &data);
                        soc_phy_ddrc28_reg_field_set(unit, SOC_PHY_REG_DQ_BYTE3_CDR_VDL_COMMON_CONFIGr, &data, SOC_PHY_FLD_VDL_VALUE_CAPf, step2UI);
                        WRITE_DDRC28_DQ_BYTE3_CDR_VDL_COMMON_CONFIGr(unit, ndx, data);
                ...
                if(((*config_param).byte == -1) || ((*config_param).byte == 0))
                {
                    if  (   ((*config_param).p != SHMOO_COMBO28_CDR_UNDEFINED_VALUE)
                            || ((*config_param).n != SHMOO_COMBO28_CDR_UNDEFINED_VALUE)
                            || ((*config_param).i != SHMOO_COMBO28_CDR_UNDEFINED_VALUE)
                            || ((*config_param).q != SHMOO_COMBO28_CDR_UNDEFINED_VALUE)
                            || ((*config_param).ib != SHMOO_COMBO28_CDR_UNDEFINED_VALUE)
                        )
                    {
                        READ_DDRC28_DQ_BYTE0_CDR_PN_VDL_CONFIGr(unit, ndx, &data);
                        if((*config_param).p != SHMOO_COMBO28_CDR_UNDEFINED_VALUE)
                        {
                            soc_phy_ddrc28_reg_field_set(unit, SOC_PHY_REG_DQ_BYTE0_CDR_PN_VDL_CONFIGr, &data, SOC_PHY_FLD_P_VDL_LOAD_VALUEf, (*config_param).p);
                        }
                        if((*config_param).n != SHMOO_COMBO28_CDR_UNDEFINED_VALUE)
                        {
                            soc_phy_ddrc28_reg_field_set(unit, SOC_PHY_REG_DQ_BYTE0_CDR_PN_VDL_CONFIGr, &data, SOC_PHY_FLD_N_VDL_LOAD_VALUEf, (*config_param).n);
                        }
                        ...
                        {
                            soc_phy_ddrc28_reg_field_set(unit, SOC_PHY_REG_DQ_BYTE0_CDR_IQ_VDL_CONFIGr, &data, SOC_PHY_FLD_I_VDL_LOAD_VALUEf, (*config_param).i);
                        }
                        if((*config_param).q != SHMOO_COMBO28_CDR_UNDEFINED_VALUE)
                        {
                            soc_phy_ddrc28_reg_field_set(unit, SOC_PHY_REG_DQ_BYTE0_CDR_IQ_VDL_CONFIGr, &data, SOC_PHY_FLD_Q_VDL_LOAD_VALUEf, (*config_param).q);
                        }
                        if((*config_param).ib != SHMOO_COMBO28_CDR_UNDEFINED_VALUE)
                        {
                            soc_phy_ddrc28_reg_field_set(unit, SOC_PHY_REG_DQ_BYTE0_CDR_IQ_VDL_CONFIGr, &data, SOC_PHY_FLD_IB_VDL_LOAD_VALUEf, (*config_param).ib);
                        }
                ...
                if(((*config_param).byte == -1) || ((*config_param).byte == 1))
                {
                        ...
                        )
                    {
                        ...
                        if((*config_param).p != SHMOO_COMBO28_CDR_UNDEFINED_VALUE)
                        {
                            soc_phy_ddrc28_reg_field_set(unit, SOC_PHY_REG_DQ_BYTE1_CDR_PN_VDL_CONFIGr, &data, SOC_PHY_FLD_P_VDL_LOAD_VALUEf, (*config_param).p);
                        }
                        if((*config_param).n != SHMOO_COMBO28_CDR_UNDEFINED_VALUE)
                        {
                            soc_phy_ddrc28_reg_field_set(unit, SOC_PHY_REG_DQ_BYTE1_CDR_PN_VDL_CONFIGr, &data, SOC_PHY_FLD_N_VDL_LOAD_VALUEf, (*config_param).n);
                        }
                        ...
                        if((*config_param).i != SHMOO_COMBO28_CDR_UNDEFINED_VALUE)
                        {
                            soc_phy_ddrc28_reg_field_set(unit, SOC_PHY_REG_DQ_BYTE1_CDR_IQ_VDL_CONFIGr, &data, SOC_PHY_FLD_I_VDL_LOAD_VALUEf, (*config_param).i);
                        }
                        if((*config_param).q != SHMOO_COMBO28_CDR_UNDEFINED_VALUE)
                        {
                            soc_phy_ddrc28_reg_field_set(unit, SOC_PHY_REG_DQ_BYTE1_CDR_IQ_VDL_CONFIGr, &data, SOC_PHY_FLD_Q_VDL_LOAD_VALUEf, (*config_param).q);
                        }
                        if((*config_param).ib != SHMOO_COMBO28_CDR_UNDEFINED_VALUE)
                        {
                            soc_phy_ddrc28_reg_field_set(unit, SOC_PHY_REG_DQ_BYTE1_CDR_IQ_VDL_CONFIGr, &data, SOC_PHY_FLD_IB_VDL_LOAD_VALUEf, (*config_param).ib);
                        }
                ...
                if(((*config_param).byte == -1) || ((*config_param).byte == 2))
                {
                        ...
                        )
                    {
                        ...
                        if((*config_param).p != SHMOO_COMBO28_CDR_UNDEFINED_VALUE)
                        {
                            soc_phy_ddrc28_reg_field_set(unit, SOC_PHY_REG_DQ_BYTE2_CDR_PN_VDL_CONFIGr, &data, SOC_PHY_FLD_P_VDL_LOAD_VALUEf, (*config_param).p);
                        }
                        if((*config_param).n != SHMOO_COMBO28_CDR_UNDEFINED_VALUE)
                        {
                            soc_phy_ddrc28_reg_field_set(unit, SOC_PHY_REG_DQ_BYTE2_CDR_PN_VDL_CONFIGr, &data, SOC_PHY_FLD_N_VDL_LOAD_VALUEf, (*config_param).n);
                        }
                        ...
                        if((*config_param).i != SHMOO_COMBO28_CDR_UNDEFINED_VALUE)
                        {
                            soc_phy_ddrc28_reg_field_set(unit, SOC_PHY_REG_DQ_BYTE2_CDR_IQ_VDL_CONFIGr, &data, SOC_PHY_FLD_I_VDL_LOAD_VALUEf, (*config_param).i);
                        }
                        if((*config_param).q != SHMOO_COMBO28_CDR_UNDEFINED_VALUE)
                        {
                            soc_phy_ddrc28_reg_field_set(unit, SOC_PHY_REG_DQ_BYTE2_CDR_IQ_VDL_CONFIGr, &data, SOC_PHY_FLD_Q_VDL_LOAD_VALUEf, (*config_param).q);
                        }
                        if((*config_param).ib != SHMOO_COMBO28_CDR_UNDEFINED_VALUE)
                        {
                            soc_phy_ddrc28_reg_field_set(unit, SOC_PHY_REG_DQ_BYTE2_CDR_IQ_VDL_CONFIGr, &data, SOC_PHY_FLD_IB_VDL_LOAD_VALUEf, (*config_param).ib);
                        }
                ...
                if(((*config_param).byte == -1) || ((*config_param).byte == 3))
                {
                        ...
                        )
                    {
                        ...
                        if((*config_param).p != SHMOO_COMBO28_CDR_UNDEFINED_VALUE)
                        {
                            soc_phy_ddrc28_reg_field_set(unit, SOC_PHY_REG_DQ_BYTE3_CDR_PN_VDL_CONFIGr, &data, SOC_PHY_FLD_P_VDL_LOAD_VALUEf, (*config_param).p);
                        }
                        if((*config_param).n != SHMOO_COMBO28_CDR_UNDEFINED_VALUE)
                        {
                            soc_phy_ddrc28_reg_field_set(unit, SOC_PHY_REG_DQ_BYTE3_CDR_PN_VDL_CONFIGr, &data, SOC_PHY_FLD_N_VDL_LOAD_VALUEf, (*config_param).n);
                        }
                        ...
                        if((*config_param).i != SHMOO_COMBO28_CDR_UNDEFINED_VALUE)
                        {
                            soc_phy_ddrc28_reg_field_set(unit, SOC_PHY_REG_DQ_BYTE3_CDR_IQ_VDL_CONFIGr, &data, SOC_PHY_FLD_I_VDL_LOAD_VALUEf, (*config_param).i);
                        }
                        if((*config_param).q != SHMOO_COMBO28_CDR_UNDEFINED_VALUE)
                        {
                            soc_phy_ddrc28_reg_field_set(unit, SOC_PHY_REG_DQ_BYTE3_CDR_IQ_VDL_CONFIGr, &data, SOC_PHY_FLD_Q_VDL_LOAD_VALUEf, (*config_param).q);
                        }
                        if((*config_param).ib != SHMOO_COMBO28_CDR_UNDEFINED_VALUE)
                        {
                            soc_phy_ddrc28_reg_field_set(unit, SOC_PHY_REG_DQ_BYTE3_CDR_IQ_VDL_CONFIGr, &data, SOC_PHY_FLD_IB_VDL_LOAD_VALUEf, (*config_param).ib);
                        }
                        ...
                        if((!field) && (((*config_param).byte == -1) || ((*config_param).byte == 0)))
                        {
                            ...
                            do
                            {
                                ...
                                if(soc_phy_ddrc28_reg_field_get(unit, SOC_PHY_REG_DQ_BYTE0_STATUS_CDR_OPERATIONr, init_operation_status, SOC_PHY_FLD_INIT_LOCK_STATUSf))
                                {
                                    ...
                                    while((current_vdl_status == init_vdl_status) && (current_operation_status == init_operation_status));
                                ...
                                if (soc_timeout_check(&to))
                                {
                                    LOG_VERBOSE(BSL_LS_SOC_DDR,
                                                (BSL_META_U(unit,
                                                            "WARNING: Byte 0 could not lock (Lock indication error). Please adjust I/Q/IB manual and try again.\n")));
                                    break;
                                }
                            }
                            while(TRUE);
                        ...
                        if((!field) && (((*config_param).byte == -1) || ((*config_param).byte == 1)))
                        {
                            ...
                            do
                            {
                                ...
                                if (soc_timeout_check(&to))
                                {
                                    LOG_VERBOSE(BSL_LS_SOC_DDR,
                                                (BSL_META_U(unit,
                                                            "WARNING: Byte 1 could not lock (Lock indication error). Please adjust I/Q/IB manual and try again.\n")));
                                    break;
                                }
                            }
                            while(TRUE);
                        ...
                        if((!field) && (((*config_param).byte == -1) || ((*config_param).byte == 2)))
                        {
                            ...
                            do
                            {
                                ...
                                if (soc_timeout_check(&to))
                                {
                                    LOG_VERBOSE(BSL_LS_SOC_DDR,
                                                (BSL_META_U(unit,
                                                            "WARNING: Byte 2 could not lock (Lock indication error). Please adjust I/Q/IB manual and try again.\n")));
                                    break;
                                }
                            }
                            while(TRUE);
                        ...
                        if((!field) && (((*config_param).byte == -1) || ((*config_param).byte == 3)))
                        {
                            ...
                            do
                            {
                                ...
                                if (soc_timeout_check(&to))
                                {
                                    LOG_VERBOSE(BSL_LS_SOC_DDR,
                                                (BSL_META_U(unit,
                                                            "WARNING: Byte 3 could not lock (Lock indication error). Please adjust I/Q/IB manual and try again.\n")));
                                    break;
                                }
                            }
                            while(TRUE);
