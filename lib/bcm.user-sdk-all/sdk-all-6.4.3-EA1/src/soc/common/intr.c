...
void soc_cmn_error(void *unit_vp, void *d1, void *d2, void *d3, void *d4)
{
    ...
    LOG_VERBOSE(BSL_LS_SOC_INTR,
                (BSL_META_U(unit,
                            "%s(): interrupt_num=%d, max_interrupts_size=%d, total_interrupts=%d\n"),
...
soc_active_interrupts_get(int unit, int flags, int max_interrupts_size, soc_interrupt_cause_t *interrupts, int *total_interrupts)
{
    ...
    while (1) {
       ...
       if(SOC_IS_ARAD(unit) &&  (bi->type == SOC_BLK_CLP || bi->type == SOC_BLK_XLP )) {
        ...
        LOG_VERBOSE(BSL_LS_SOC_INTR,
                    (BSL_META_U(unit,
                                "%s(): block_int_data=0x%x, block_int_mask_data=0x%x, block_int_bitmap=0x%x,\n"),
        ...
        for (; int_bit_idx < SOC_INTERRUPT_INTERRUPT_PER_REG_NUM_MAX; int_bit_idx++) {
            ...
            if (int_id == -1) {
                LOG_VERBOSE(BSL_LS_SOC_INTR,
                            (BSL_META_U(unit,
                                        "%s(): Reached hidden interrupt. int_bit_idx=%d,\n"),
            ...
            if ((!(block_int_bitmap[0] & (1<<int_bit_idx))) && (!(vector_id == 1 && is_unmasked_flag == 0))) {
                LOG_DEBUG(BSL_LS_SOC_INTR,
                          (BSL_META_U(unit,
                                      "%s(): no interrupt for int_bit_idx=%d\n"),
                           FUNCTION_NAME(), int_bit_idx));
                continue;
            }
            ...
            LOG_VERBOSE(BSL_LS_SOC_INTR,
                        (BSL_META_U(unit,
                                    "%s(): \tgettind int_id: blk=%d, int_bit_idx=%d, int_id=%d, name=%s, vector_id=%d,\n"),
                         FUNCTION_NAME(), blk, int_bit_idx, int_id, interrupts_arr[int_id].name, vector_id));
#else
            LOG_VERBOSE(BSL_LS_SOC_INTR,
                        (BSL_META_U(unit,
                                    "%s(): \tgettind int_id: blk=%d, int_bit_idx=%d, int_id=%d, vector_id=%d,\n"),
            ...
            if ((finfop->len == 0x1) && (finfop->bp != int_bit_idx)) {
                LOG_ERROR(BSL_LS_SOC_INTR,
                          (BSL_META_U(unit,
                                      "%s(): Error: Where finfop->len=%d. finfop->bp=%d != int_bit_idx=%d.\n"),
            ...
            if (vector_id == 0) {
                ...
                if (index < max_interrupts_size ) {
                    ...
                    LOG_VERBOSE(BSL_LS_SOC_INTR,
                                (BSL_META_U(unit,
                                            "%s(): \tinterrupts[%d].index=%d, interrupts[%d].id=%d,\n"),
                ...
                LOG_VERBOSE(BSL_LS_SOC_INTR,
                            (BSL_META_U(unit,
                                        "%s(): \t\tvector_int_data=0x%x, vector_int_mask_data=0x%x, vector_int_bitmap=0x%x,\n"),
                ...
                for (; vector_int_bit_idx < SOC_INTERRUPT_INTERRUPT_PER_REG_NUM_MAX; vector_int_bit_idx++) {
                    if (!(vector_int_bitmap[0] & (1<<vector_int_bit_idx))) {
                        LOG_DEBUG(BSL_LS_SOC_INTR,
                                  (BSL_META_U(unit,
                                              "%s(): no interrupt for vector_int_bit_idx=%d\n"),
                    ...
                    if (vector_int_id == -1) {
                        LOG_VERBOSE(BSL_LS_SOC_INTR,
                                    (BSL_META_U(unit,
                                                "%s(): Reached hidden interrupt. vector_int_bit_idx=%d,\n"),
                                     FUNCTION_NAME(), vector_int_bit_idx));
                        continue;
                    }
                    ...
                    LOG_VERBOSE(BSL_LS_SOC_INTR,
                                (BSL_META_U(unit,
                                            "%s(): \t\tblk=%d, int_id=%d, vector_int_bit_idx=%d, vector_int_id=%d, name=%s,\n"),
                                 FUNCTION_NAME(), blk, int_id, vector_int_bit_idx, vector_int_id, interrupts_arr[vector_int_id].name));
#else
                    LOG_VERBOSE(BSL_LS_SOC_INTR,
                                (BSL_META_U(unit,
                                            "%s(): \t\tblk=%d, int_id=%d, vector_int_bit_idx=%d, vector_int_id=%d,\n"),
                    ...
                    if ((finfop->len == 0x1) && (finfop->bp != vector_int_bit_idx)) {
                        LOG_ERROR(BSL_LS_SOC_INTR,
                                  (BSL_META_U(unit,
                                              "%s(): Error: Where finfop->len=%d. finfop->bp=%d != vector_int_bit_idx=%d.\n"),
    ...
    *total_interrupts = index;
    LOG_VERBOSE(BSL_LS_SOC_INTR,
                (BSL_META_U(unit,
                            "%s(): index=%d, *total_interrupts=%d, cont_prev_i_blk=%d, cont_prev_bit_idx=%d, cont_prev_vector_bit_idx=%d.\n"),
...
soc_get_interrupt_id_specific(int unit, int reg_adress, int reg_block, int field_bit, int* interrupt_id)
{
    ...
    *interrupt_id = -1;
    ...
    for(i=0; i < nof_interrupts; i++) {
        /* check block */
        if(soc_interrupt_is_supported(unit, blk_indx, i) == 0x0) {
            continue; 
        }
...
#endif /* defined(BCM_ESW_SUPPORT) || defined(BCM_SIRIUS_SUPPORT) || defined(BCM_DFE_SUPPORT)|| defined(BCM_PETRA_SUPPORT) || defined(PORTMOD_SUPPORT)*/
