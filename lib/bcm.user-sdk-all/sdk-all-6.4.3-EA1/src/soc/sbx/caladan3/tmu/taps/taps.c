...
int _cache_taps_work[SOC_MAX_NUM_DEVICES] = {0};
...
#define TAPS_CACHE_LOCK


static volatile sal_thread_t _taps_autocache[SOC_TAPS_SHARE_CACHE_MAX_UNIT_NUM] = {NULL};
static sal_mutex_t _taps_autocache_mutex[SOC_TAPS_SHARE_CACHE_MAX_UNIT_NUM] = {NULL};
...
#define TAPS_CACHE_LOCK_GET     sal_mutex_take(_taps_sal_mutex[unit],sal_mutex_FOREVER);
#define TAPS_CACHE_LOCK_FREE    sal_mutex_give(_taps_sal_mutex[unit]);
#else
#include <pthread.h>
static pthread_spinlock_t _taps_cache_lock[SOC_TAPS_SHARE_CACHE_MAX_UNIT_NUM];
#define TAPS_CACHE_LOCK_GET     pthread_spin_lock(&_taps_cache_lock[unit]);
#define TAPS_CACHE_LOCK_FREE    pthread_spin_unlock(&_taps_cache_lock[unit]);
#endif
...
static void _taps_autocache_thread(int unit) {
  uint last_cache_count=0;

#ifdef TAPS_USE_NANO_TIMER
 struct timespec sleep_time, sleep_rep;
  /* Set up the time interval between cache flushes */
  sleep_time.tv_sec=0;
...
#ifdef TAPS_USE_NANO_TIMER
    nanosleep(&sleep_time, &sleep_rep);
#else
        ...
        uint32 work_type_count)
{
  ...
  /* Check if we have room for the new item */
  if(_taps_cache_size[unit] >=  TAPS_MAX_CACHE)
    {
      /* If not then flush out the existing items */
      taps_flush_cache(unit);
    }
...
void taps_cached_cmd_dma(void * p_elem, int unit)
{
    ...
    /* If the DMA Is full then flush it */
    if (rv == SOC_E_FULL)
    {
      rv = soc_sbx_caladan3_tmu_post_cmd(unit, SOC_SBX_CALADAN3_TMU_DEF_CMD_FIFO,
      ...
      if(!SOC_SUCCESS(rv))
        {
          LOG_ERROR(BSL_LS_SOC_COMMON,
                    (BSL_META_U(unit,
                                "%s: unit %d Failed to Flush DMA%d\n"),
      ...
      if(!SOC_SUCCESS(rv))
        {
          LOG_ERROR(BSL_LS_SOC_COMMON,
                    (BSL_META_U(unit,
                                "%s: unit %d Failed command send on retry%d\n"),
                     FUNCTION_NAME(), unit, rv));
          return;
        }
    }
...
void taps_cached_cmd_share_dma(void * p_elem, taps_share_cache_cookie_data_t *cookie)
{
    ...
    {
        soc_sbx_caladan3_tmu_master_flush_cmd(cookie->master_unit, &cookie->slave_units[0], 
    ...
    {
        LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META("%s: unit %d Failed to DMA command command %d\n"),
                                ...
                                int *split)
{
    ...
    _taps_find_prefix_objects(unit, &arg, &obj);
    assert(obj.sph);
    if (obj.sph != obj.sph->sbh->wsph) {
...
static int _taps_sbucket_redist_spivot_get(taps_spivot_handle_t sph, void *user_data)
{
    ...
    if (cbdata->taps->param.mode == TAPS_OFFCHIP_ALL) {
        if(sph == sph->sbh->wsph) {
            ...
            taps_dbucket_stat_get(cbdata->unit, cbdata->taps, sph->dbh, &bkt_stat);
                            ...
                            taps_tcam_pivot_handle_t tph, taps_spivot_handle_t *sph_array, int sph_cnt)
{
    ...
    if (!sbucket_split) {
        ...
        taps_sbucket_stat_get(unit, old_sbh, &bkt_stat);
    ...
    while(state != INS_ST_DONE) {
        ...
        case INS_ST_DOMAIN_PROPAGATE_SPLIT:
            ...
            for (sph_idx = 0; sph_idx < cookie->added_sph_count; sph_idx++) {
                ...
                /* Destory dbh */
                taps_dbucket_destroy(unit, cookie->arg.taps, obj->wgroup[unit], dbh);
...
TIME_STAMP("# INS_ST_ERROR state")
    ...
    /* destroy work group */
    if (obj->wgroup[unit]) {
        taps_work_group_destroy_for_all_devices(unit, arg->taps,
