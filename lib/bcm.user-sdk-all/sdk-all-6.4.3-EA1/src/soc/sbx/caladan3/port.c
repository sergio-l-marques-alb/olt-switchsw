...
extern int _soc_sbx_caladan3_port_enable(int unit, int port, int subport, int enable);
/*
...
static const soc_field_t port_mac_control_reset_field[] = {
    XMAC0_RESETf,
    XMAC1_RESETf,
    XMAC2_RESETf
};
...
int soc_sbx_caladan3_is_expanded_port_space(int unit)
{
 ...
 *
 * Function:
 *     soc_sbx_caladan3_reconfig_port_cleanup_port
 * Purpose:
 *     During reconfiguration clean up port state for a port
 * that is no longer present. Three things are done:
 ...
 */
int soc_sbx_caladan3_reconfig_port_cleanup_port(int unit, soc_port_t port)
{
    soc_sbx_caladan3_port_map_t         *port_map = NULL;
    int                                 subport;
    int                                 status;
    int                                 portf[] = { PORT0f, PORT1f, PORT2f,
                                                    PORT3f, PORT4f, PORT5f,
                                                    PORT6f, PORT7f, PORT8f,
                                                    PORT9f, PORT10f, PORT11f };
    uint32                              portmode;
    ...
    /* Reset the warp core */
    soc_sbx_caladan3_xgxs_reset(unit, port,
        port_map->line_port_info[port].intf_instance);

    /* Clear the port enable bit */
    subport = (port_map->line_port_info[port].intf_instance << 2) +
        port_map->line_port_info[port].bindex;
    status = soc_reg_field32_modify(unit, PORT_ENABLE_REGr, port,
        portf[subport], 0);

    if ((subport % 4) != 0) {
        return BCM_E_NONE;
    }
    /* Set the core port mode to disabled */
    status = READ_PORT_MODE_REGr(unit, port, &portmode);
    if (status != SOC_E_NONE) {
        LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META_U(unit,
                              "Unit %d %s: READ_PORT_MODE_REGr failed: %s\n"),
                   unit, FUNCTION_NAME(), soc_errmsg(status)));
    }
    if (port_map->line_port_info[port].intf_instance == 0) {
        soc_reg_field_set(unit, PORT_MODE_REGr, &portmode, 
                          XPORT0_CORE_PORT_MODEf,
                          SOC_SBX_CALADAN3_CORE_PORT_DISABLE);
        soc_reg_field_set(unit, PORT_MODE_REGr, &portmode, 
                          XPORT0_PHY_PORT_MODEf, 0);
    } else if (port_map->line_port_info[port].intf_instance == 1) {
        soc_reg_field_set(unit, PORT_MODE_REGr, &portmode, 
                          XPORT1_CORE_PORT_MODEf,
                          SOC_SBX_CALADAN3_CORE_PORT_DISABLE);
        soc_reg_field_set(unit, PORT_MODE_REGr, &portmode, 
                          XPORT1_PHY_PORT_MODEf, 0);
    } else if (port_map->line_port_info[port].intf_instance == 2) {
        soc_reg_field_set(unit, PORT_MODE_REGr, &portmode, 
                          XPORT2_CORE_PORT_MODEf,
                          SOC_SBX_CALADAN3_CORE_PORT_DISABLE);
        soc_reg_field_set(unit, PORT_MODE_REGr, &portmode, 
                          XPORT2_PHY_PORT_MODEf, 0);
    } else {
        LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META_U(unit,
                              "Unit %d soc_sbx_caladan3_reconfig_port_cleanup: error\n"),
                   unit));
        return SOC_E_INTERNAL;
    }
    status = WRITE_PORT_MODE_REGr(unit, port, portmode);
    if (status != SOC_E_NONE) {
        LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META_U(unit,
                              "Unit %d %s: WRITE_PORT_MODE_REGr failed: %s\n"),
                   unit, FUNCTION_NAME(), soc_errmsg(status)));
    }
...
int soc_sbx_caladan3_reconfig_port_cleanup(int unit)
{
    ...
    for (port = 0; port < SOC_SBX_CALADAN3_MAX_LINE_PORT; port++) {
        ...
        if (spn_str == NULL ||
            ...
            subport = (port_map->line_port_info[port].intf_instance << 2) +
                port_map->line_port_info[port].bindex;
            _soc_sbx_caladan3_port_enable(unit, port, subport, 0);
...
int soc_sbx_caladan3_port_info_load(int unit)
{
    ...
    char *spn_str;
    int  port, ucode_num_ports, max_port, num_ports_found, ucode_num_port_override;
    soc_sbx_caladan3_port_map_t *port_map = NULL;
...
/*
 * Function:
 *    _soc_sbx_caladan3_port_enable
 * Purpose:
 *    Enable ports in the mac core
 */
int
_soc_sbx_caladan3_port_enable(int unit, int port, int subport, int enable)
{
    uint32 rval;
    int rv = SOC_E_NONE;
                               ...
                               int core_mode, int phy_mode, int gmii_mode)
{
    ...
    mac_in_reset = soc_reg_field_get(unit, PORT_MAC_CONTROLr, mac_control,
                                     port_mac_control_reset_field[core]);
    if (!mac_in_reset) {
        soc_reg_field_set(unit, PORT_MAC_CONTROLr, &mac_control,
                          port_mac_control_reset_field[core], 1);
    ...
    if (!mac_in_reset) {
        soc_reg_field_set(unit, PORT_MAC_CONTROLr, &mac_control,
                          port_mac_control_reset_field[core], 0);
        SOC_IF_ERROR_RETURN(WRITE_PORT_MAC_CONTROLr(unit, port, mac_control)); 
    }
...
int
_soc_sbx_caladan3_mac_reset(int unit, int port, int subport, int mac_mode) 
{
    _soc_sbx_caladan3_mac_in_reset(unit, port, subport, mac_mode);
    _soc_sbx_caladan3_mac_out_reset(unit, port, subport, mac_mode);
    return SOC_E_NONE;
}
...
_soc_sbx_caladan3_mac_aggregated(int unit, int port, int subport)
{
    ...
    if (SOC_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META_U(unit,
                              "Unit %d *** ERRROR: Mac Reset failed port %d\n"), unit, port));
        return rv;
    }
    soc_reg_field_set(unit, PORT_MODE_REGr, &portmode, 
                      XPORT0_CORE_PORT_MODEf, core_mode);
...
soc_sbx_caladan3_mac_init(int unit) 
{
    ...
    SOC_PBMP_ITER(PBMP_PORT_ALL(unit), port) {
        ...
        /* skip all aggregate ports */
        if (IS_XE_PORT(unit, port) ||
            IS_GE_PORT(unit, port) ||
            (IS_HG_PORT(unit,port) && 
             (si->port_speed_max[port] == (10*SOC_SBX_CALADAN3_GIG_SPEED)))) {
            int mac_in_reset = 0;

            /* make sure port_mode_reg default is disabled */
            if (soc_feature(unit, soc_feature_logical_port_num)) {
                phy_port = si->port_l2p_mapping[port];
            } else {
                phy_port = port;
            ...
            core_mode = SOC_SBX_CALADAN3_CORE_PORT_DISABLE;
            if (core == 0) {
                soc_reg_field_set(unit, PORT_MODE_REGr, &portmode, XPORT0_CORE_PORT_MODEf, core_mode);
            } else if (core == 1) {
                soc_reg_field_set(unit, PORT_MODE_REGr, &portmode, XPORT1_CORE_PORT_MODEf, core_mode);
            } else {
                soc_reg_field_set(unit, PORT_MODE_REGr, &portmode, XPORT2_CORE_PORT_MODEf, core_mode);
            }
    ...
    SOC_PBMP_ITER(PBMP_PORT_ALL(unit), port) {
        ...
        /* Disable Port */
        rv = _soc_sbx_caladan3_port_enable(unit, port, subport, 0);
        if (SOC_FAILURE(rv)) {
            LOG_WARN(BSL_LS_SOC_COMMON,
                     (BSL_META_U(unit,
                                 "Unit %d Port %d disable failed \n"), unit, port));
        }
        ...
        if (soc_property_port_get(unit, port, spn_SERDES_RXAUI_MODE, 0) && ((subport % 2) != 0)) {
            ...
            /* Enable */
            rv = _soc_sbx_caladan3_port_enable(unit, port, subport, 1);
            if (SOC_FAILURE(rv)) {
                LOG_WARN(BSL_LS_SOC_COMMON,
                         (BSL_META_U(unit,
                                     "Unit %d Port %d enable failed \n"), unit, port));
            }
...
int soc_sbx_caladan3_port_remove(int unit, soc_port_t lport)
{
    ...
    subport = (port_map->line_port_info[lport].intf_instance << 2) +
        port_map->line_port_info[lport].bindex;
    _soc_sbx_caladan3_port_enable(unit, lport, subport, 0);
    ...
    if (wc_count == 0) {
        ...
        soc_reg_field_set(unit, PORT_MAC_CONTROLr, &mac_control,
            port_mac_control_reset_field[wcidx], 1);
