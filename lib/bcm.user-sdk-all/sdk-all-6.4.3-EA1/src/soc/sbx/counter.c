...
/*
 * Function:
 *     _soc_sbx_counter_bset_collect
 * Purpose:
 *     Collect statistics for specified counter set.
 * Parameters:
 *     unit    - Device number
 *     discard - If true, the software counters are not updated; this
 *               results in only synchronizing the previous count buffer
 ...
 */
STATIC int
_soc_sbx_counter_bset_collect(int unit, int discard, int block, int set)
{
    _counter_data_t  *data = NULL;
    int              counter = 0;
    int              num_counters = 0;
    ...
    for (counter = 0; counter < num_counters; counter++) {
        ...
        /* Read value into new */
        SOC_IF_ERROR_RETURN
            (COUNTER_READ(unit, block, set, counter, &val_new, &width));
            
        /* if prev == new, then delta = 0 */
        if (COMPILER_64_EQ(val_new, val_prev)) {
            COUNTER_ATOMIC_BEGIN(s);
            COMPILER_64_ZERO(data->delta);
            COUNTER_ATOMIC_END(s);
            continue;
        ...
        if (COMPILER_64_LT(val_diff, val_prev)) {
            uint64  wrap_amt;
            ...
            if (width < 32) {
                ...
                COMPILER_64_ADD_64(val_diff, wrap_amt);
            } else if (width < 64) {
        ...
        COMPILER_64_SUB_64(val_diff, val_prev);
...
soc_sbx_counter_collect64(int unit, int discard)
{
    ...
    int                 index = 0;
    int                 port_base = 0, port_base_dma = 0;
    int                 ar_idx = 0;
...
_soc_sbx_counter_collect(int unit, int discard)
{
    ...
    for (block = 0; block < COUNTER_NUM_BLOCKS(unit); block++) {
        ...
        for (set = 0; set < COUNTER_BLOCK_NUM_SETS(unit, block); set++) {
            ...
            if ((SOC_IS_SBX_FE2000(unit)) || (SOC_IS_SBX_FE2KXT(unit))) { 
                _soc_sbx_counter_bset_collect(unit, discard, block, set);
            } else {
