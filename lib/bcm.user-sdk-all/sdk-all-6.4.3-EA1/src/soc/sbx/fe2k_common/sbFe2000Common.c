...
/*****************************************************************************
 * $Id: sbFe2000Common.c,v 1.16 Broadcom SDK $
 *
 ...
 */
int
sbFe2000SwsConfigCompute(uint32 hpp_freq, uint32 epoch,
                         ...
                         sbFe2000QmParams_t *hpp_to_mac)
{
    ...
    double   clock_period;
    uint32 page_sz     = FE2K_SWS_PAGE_SIZE;
    uint32 sws_latency = FE2K_SWS_LATENCY;
    double   pkt_rate_64 = FE2K_PKT_RATE_64;
    uint32 overshoot = 64 * 16;
    uint32 frames_per_epoch = 48;
    uint32 batch_latency, factor;
    double   dev_latency;
    uint32 jumbo_pages = ceilx((mtu / page_sz), 1);
    double   min_thresh, jumbo_thresh, output_thresh;
    uint32 lines_per_ts;
    uint32 ts_pages;
    uint32 e2ecc_period = 256;
	...
	if (hpp_to_mac->max_pages < spi_to_hpp->max_pages) {
	  hpp_to_mac->max_pages = spi_to_hpp->max_pages;
	}
    }
 ...
 */
int
sbFe2000SwsConfigGet(uint32 hpp_freq, uint32 epoch, uint32 contexts,
