/*
 * $Id: rx_los_db.c,v 1.6 Broadcom SDK $
 *
...
/* Database: */
static rx_los_unit_info_t* rx_los_units[SOC_MAX_NUM_DEVICES];
static soc_pbmp_t rx_los_units_pbmp;
static uint32 rx_los_long_sleep = RX_LOS_LONG_SLEEP_DEFAULT;
static uint32 rx_los_short_sleep = RX_LOS_SHORT_SLEEP_DEFAULT;
static uint32 rx_los_allowed_retries = RX_LOS_ALLOWED_RETRIES_DEFAULT;
static uint32 rx_los_priority = RX_LOS_THREAD_PRIORITY_DEFAULT;
static uint32 rx_los_link_down_cycle_time = RX_LOS_LINK_DOWN_CYCLE_TIME_DEFAULT;
static uint32 rx_los_link_down_cycle_count = RX_LOS_LINK_DOWN_CYCLE_COUNT_DEFAULT;
static uint32 rx_los_rx_seq_change_cycle_time = RX_LOS_RX_SEQ_CHANGE_CYCLE_TIME_DEFAULT;
static uint32 rx_los_rx_seq_change_cycle_count = RX_LOS_RX_SEQ_CHANGE_CYCLE_COUNT_DEFAULT;
...
{
    rx_los_long_sleep = long_sleep_usec;
    rx_los_short_sleep = short_sleep_usec;
    rx_los_allowed_retries = allowed_retries;
    rx_los_priority = priority;
    rx_los_link_down_cycle_count = link_down_count_max;
    rx_los_link_down_cycle_time = link_down_cycle_time;
...
{
    BCM_PBMP_ASSIGN(*pbmp, rx_los_units_pbmp);
...
{
    if (rx_los_units[unit]==NULL) {
        return BCM_E_FAIL;
    }

    BCM_PBMP_PORT_ADD(rx_los_units[unit]->supported_links, port);
...
int rx_los_db_remove_link_from_support(int unit, int port)
{
    ...
    BCM_PBMP_PORT_REMOVE(rx_los_units[unit]->supported_links, port);
...
int rx_los_db_remove_link_from_steady(int unit, int port)
{
    ...
    BCM_PBMP_PORT_ADD(rx_los_units[unit]->not_steady_state,port);
...
int rx_los_db_move_link_to_steady(int unit, int port)
{
    ...
    BCM_PBMP_PORT_REMOVE(rx_los_units[unit]->not_steady_state,port);
...
{
    BCM_PBMP_ASSIGN(*pbmp,rx_los_units[unit]->not_steady_state);
...
{
    BCM_PBMP_ASSIGN(*pbmp, rx_los_units[unit]->supported_links);
...
rx_los_state_t rx_los_db_state_get(int unit, int port)
{
    ...
    state = rx_los_units[unit]->links[port].current_state;
...
int rx_los_db_state_set(int unit,int port,rx_los_state_t state)
{
    ...
    rv = rx_los_db_state_name_get(rx_los_units[unit]->links[port].current_state, &old_name);
    if(rv != BCM_E_NONE) {
        return rv;
    }
    ...
    rx_los_units[unit]->links[port].current_state = state;
...
{
    sal_usecs_t timeStamp;
    timeStamp = rx_los_units[unit]->links[port].timeStamp;
...
{
    rx_los_units[unit]->links[port].timeStamp = sal_time_usecs();
}

int rx_los_db_tries_get(int unit, int port)
{
    int rv;
    rv = rx_los_units[unit]->links[port].current_retries;
...
{
    rx_los_units[unit]->links[port].current_retries = 0;

}

void rx_los_db_tries_increment(int unit, int port)
{
    rx_los_units[unit]->links[port].current_retries++;
}

int rx_los_db_unit_null_check(int unit)
{
    if ((rx_los_units[unit] == NULL)||(rx_los_units[unit]->links == NULL)) {
...
rx_los_db_init(void) {
    ...
    BCM_PBMP_CLEAR(rx_los_units_pbmp);
    sal_memset(rx_los_units, 0, (sizeof(rx_los_unit_info_t*))*SOC_MAX_NUM_DEVICES);
}
...
rx_los_db_unit_init(int unit, pbmp_t links_pbmp) {
    ...
    rx_los_units[unit] = unit_info;
    BCM_PBMP_PORT_ADD(rx_los_units_pbmp, unit);
...
rx_los_db_unit_deinit(int unit) {
    ...
    unit_info = rx_los_units[unit];
    rx_los_units[unit] = NULL;
    BCM_PBMP_PORT_REMOVE(rx_los_units_pbmp, unit);
...
rx_los_db_dump_port(int unit, bcm_port_t port) {
    ...
    if (!BCM_PBMP_MEMBER(rx_los_units[unit]->supported_links, port)) {
        return BCM_E_PORT;
    }

    rv = rx_los_db_state_name_get(rx_los_units[unit]->links[port].current_state, &state_name);

    cli_out("Port %d: state %s, current retries %d, timestamp %u link_down_count %u rx_seq_change_count %u \n", port, 
            state_name,
            rx_los_units[unit]->links[port].current_retries,
            rx_los_units[unit]->links[port].timeStamp,
            rx_los_units[unit]->links[port].link_down_cycle_count,
            rx_los_units[unit]->links[port].rx_seq_change_cycle_count);

    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_APPL_SHELL,
                  (BSL_META_U(unit,
                              "ERROR: link %u - state number is %d\n"), port, rx_los_units[unit]->links[port].current_state));
        return rv;
    }
...
{
    *cycle_start = rx_los_units[unit]->links[port].link_down_cycle_start;
    *cycle_count = rx_los_units[unit]->links[port].link_down_cycle_count;
...
{
    rx_los_units[unit]->links[port].link_down_cycle_start = cycle_start;
    rx_los_units[unit]->links[port].link_down_cycle_count = cycle_count;
...
{
    *cycle_time = rx_los_link_down_cycle_time;
    *cycle_max_count = rx_los_link_down_cycle_count;
...
{
    rx_los_link_down_cycle_time = cycle_time;
    rx_los_link_down_cycle_count = cycle_max_count;
...
{
    *cycle_start = rx_los_units[unit]->links[port].rx_seq_change_cycle_start;
    *cycle_count = rx_los_units[unit]->links[port].rx_seq_change_cycle_count;
...
{
    rx_los_units[unit]->links[port].rx_seq_change_cycle_start = cycle_start;
    rx_los_units[unit]->links[port].rx_seq_change_cycle_count = cycle_count;
...
{
    *cycle_time = rx_los_rx_seq_change_cycle_time;
    *cycle_max_count = rx_los_rx_seq_change_cycle_count;
...
{
    rx_los_rx_seq_change_cycle_time = cycle_time;
    rx_los_rx_seq_change_cycle_count = cycle_max_count;
...
{
    rx_los_units[unit]->callback = callback;
...
{
    *callback = rx_los_units[unit]->callback;
