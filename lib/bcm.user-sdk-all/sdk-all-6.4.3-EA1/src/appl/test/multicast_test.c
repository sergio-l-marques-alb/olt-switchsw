/*
 * $Id: multicast_test.c,v 1.42 Broadcom SDK $
...
#if defined(BCM_88650_A0) && !defined(_ARAD_MC_NO_IMPLEMENTATION_SPECIFIC_TESTS)
...
#define MC_MEMBER_CUD_BITS 18 /* number of CUD bits */
#define MC_MEMBER_CUD_MASK ((((uint32)1) << MC_MEMBER_CUD_BITS) - 1)
#define MC_NOF_OUTLIF_IN_FIELD (1 << MC_MEMBER_CUD_BITS)

#define MC_MEMBER_DATA0_IS_TDM_BIT MC_MEMBER_CUD_BITS /* tdm bit in data0: 18 */
#define MC_MEMBER_DATA0_IS_USED_BIT (MC_MEMBER_DATA0_IS_TDM_BIT + 1) /* is used bit in data0: 19 */
#define MC_MEMBER_DATA0_IS_INGRESS_BIT (MC_MEMBER_DATA0_IS_USED_BIT + 1) /* is ingress bit in data0: 20 */

#define MC_MEMBER_DATA0_DEST_START_BIT (MC_MEMBER_DATA0_IS_INGRESS_BIT + 1) /* start bit of dest in data0: 21 */
...
#define MC_MEMBER_DATA0_DEST_BITS (32 - MC_MEMBER_DATA0_DEST_START_BIT) /* number of dest bits in data0: 11 */
#define MC_MEMBER_DATA1_DEST_BITS (MC_MEMBER_DEST_BITS - MC_MEMBER_DATA0_DEST_BITS) /* number of dest bits in data1: 7 */
...
#define MC_MEMBER_NEXT_BITS 18 /* number of bits for next pointer (in data1) */
#define MC_MEMBER_DATA1_NEXT_BIT MC_MEMBER_DATA1_DEST_BITS /* start bit of next pointer in data1: 7 */
#define MC_MEMBER_NEXT_MAX_VAL ((((uint32)1) << MC_MEMBER_NEXT_BITS) - 1)

#define MC_MEMBER_DATA1_ALLOC_BIT (MC_MEMBER_DATA1_NEXT_BIT + MC_MEMBER_NEXT_BITS) /* start bit of allocation number in data1: 25 */
#define MC_MEMBER_ALLOC_BITS (32 - MC_MEMBER_DATA1_ALLOC_BIT) /* number of bits for allocation number in data1: 7 */
...
#define MAX_ADDITIONAL_MEMBERS (512*1024) /* max number of non initial members supported by the test */
#define ADDITIONAL_MEMBERS_PER_ALLOC 4096 /* number of non initial members to allocate per allocation */
...
typedef enum { /* which assert technique to use in multicast tests */
  MC_ASSERT_FUCNTION = 0, /* divides be zero */
  MC_ASSERT_NORMAL   = 1, /* regular SDK assert */
...
void mc_test_stop(int unit, unsigned line, const char *expr, int i) { /* function used to handle filed asserts and can be caught the debugger. In gdb use:   b mc_test_stop */
    ...
    if (assert_type[unit] == MC_ASSERT_NORMAL) { /* regular SDK assert */
        assert(i);
    } else if (assert_type[unit] == MC_ASSERT_FUCNTION) { /* divides be zero */
        i = i / i;
    } /* else continue */
}
...
{
    member->data0 = ((uint32)1) << MC_MEMBER_DATA0_IS_INGRESS_BIT;
    member->data1 = 0;
}
...
int multicast_arad_test_init(int unit)
{
    ...
    BCMDNX_ALLOC(data->egress_mc_groups, sizeof(mc_member) * SOC_DPP_CONFIG(unit)->tm.nof_mc_ids, "multicast_egress_groups");
    ...
    for (i = SOC_DPP_CONFIG(unit)->tm.nof_mc_ids; i ;) {
        mc_member_clear_ingress(data->egress_mc_groups + --i); /* mark as closed */
    }

    {
        BCMDNX_ALLOC(data->ingress_mc_groups, sizeof(mc_member) * SOC_DPP_CONFIG(unit)->tm.nof_mc_ids, "multicast_ingress_groups");
        ...
        for (i = SOC_DPP_CONFIG(unit)->tm.nof_mc_ids; i ;) {
            mc_member_clear_egress(data->ingress_mc_groups + --i); /* mark as closed */
        }

        BCMDNX_IF_ERR_EXIT(READ_EGQ_EGRESS_REPLICATION_GENERAL_CONFIGr(unit, REG_PORT_ANY, &data->nof_vlan_egress_mc_groups));
    ...
    BCMDNX_ALLOC(data->vlan_bitmaps, sizeof(uint32) * UINT32SPER_VLAN * data->nof_vlan_egress_mc_groups, "multicast_vlan_bitmaps");
    if (!data->vlan_bitmaps) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_MEMORY, (_BCM_MSG("Failed allocating test memory for vlan bitmaps")));
    }
    for (i = data->nof_vlan_egress_mc_groups * UINT32SPER_VLAN; i ;) {
...
int multicast_arad_test_is_group_open(int unit, const mc_member* group, int* out_is_open)
{
    ...
    if (data->egress_mc_groups) {
        ...
        if (diff < SOC_DPP_CONFIG(unit)->tm.nof_mc_ids) {
            not_found = 0;
    ...
    if (data->ingress_mc_groups && not_found) {
        ...
        if (diff < ARAD_MULT_NOF_MULTICAST_GROUPS) { 
            not_found = 0;
            *out_is_open = mc_member_is_ingress_get(group);
        }
    }
...
int multicast_arad_test_add_member(int unit, mc_member* group_to_add_to, uint32 cud, uint32 dest)
{
    ...
    if (!is_ingress && mc_id < data->nof_vlan_egress_mc_groups) { /* This is a VLAN bitmap MC group */
        ...
        data->vlan_bitmaps[mc_id * UINT32SPER_VLAN + (dest / 32)] |= ((uint32)1) << (dest % 32);
  ...
  /* res = arad_mult_rplct_tbl_entry_unoccupied_set_all(unit); */
{
    ...
    for (i = 0; i < data->nof_vlan_egress_mc_groups; ++i) { /* first test VLAN bitmap egress Mc groups */
        ...
        is_open = mc_member_is_ingress_get(group_ptr) ^ 1; /* group marked open in data structure */
    }
#endif
    ...
    for (i = 0; i < SOC_DPP_CONFIG(unit)->tm.nof_mc_ids; ++i) {
        ...
        if ((data->ingress_mc_groups && i < ARAD_MULT_NOF_MULTICAST_GROUPS && mc_member_is_ingress_get(data->ingress_mc_groups + i)) || 
            (data->egress_mc_groups && i >= data->nof_vlan_egress_mc_groups && (mc_member_is_ingress_get(data->egress_mc_groups + i) ^ 1))) {
...
int test_bcm_multicast_create(int unit, uint32 flags, bcm_multicast_t *group, int may_run_out_of_space)
{
    ...
    if ((flags & (BCM_MULTICAST_EGRESS_GROUP | BCM_MULTICAST_TYPE_TRILL)) || (~flags & BCM_MULTICAST_INGRESS_GROUP)) {
        ...
        TEST_ASSERT(mc_member_is_ingress_get(data->egress_mc_groups + multicast_id)); /* should not already be open */
        mc_member_is_ingress_set(unit, data->egress_mc_groups + multicast_id, 0); /* mark group as open */
        mc_member_is_used_set(unit, data->egress_mc_groups + multicast_id, 0); /* mark group as empty */
    }
...
{
    mc_test_data_t *data = test_data[unit];
    mc_member* group = data->egress_mc_groups + multicast_id;
    mc_member* next_member;
    BCMDNX_INIT_FUNC_DEFS;
                
    TEST_ASSERT(!mc_member_is_ingress_get(group)); /* assert that egress group is open */
    if (close_group) {
        mc_member_is_ingress_set(unit, group, 1); /* mark group as closed */
    }
...
{
    SOC_TMC_MULT_ID multicast_id;
    int is_ingress_open = 0, is_egress_open;
    mc_test_data_t *data = test_data[unit];
    BCMDNX_INIT_FUNC_DEFS;
    ...
    is_egress_open = !mc_member_is_ingress_get(data->egress_mc_groups + multicast_id); /* is ingress group open? */
                                    ...
                                    bcm_gport_t port, bcm_if_t encap_id, int may_run_out_of_space)
{
    ...
    BCMDNX_IF_ERR_EXIT(_bcm_petra_multicast_group_to_id(group, &multicast_id));
    testgroup = test_data[unit]->egress_mc_groups + multicast_id;
    ...
    if (!is_bitmap_group && port == BCM_GPORT_INVALID ) { /* outlif only replication (port mapped from outlif) */
        ...
        BCMDNX_IF_ERR_EXIT(_multicast_arad_test_remove_eg_member(unit, testgroup, &member_to_find, encap_id, BCM_GPORT_MCAST_GET(port) + MC_MEMBER_DEST_BITMAP_OFFSET, &found));
    } else {
        uint32 local_port;
        bcm_pbmp_t pbmp_local_ports;
        bcm_if_t port_encap = encap_id; 
                                  ...
                                  int may_run_out_of_space)
{
    ...
    for (i = 0; i < port_count ; ++i) {
        if (!is_bitmap_group && port_array[i] == BCM_GPORT_INVALID ) { /* outlif only replication (port mapped from outlif) */
            ...
            BCMDNX_IF_ERR_EXIT(multicast_arad_test_add_member(unit, testgroup, encap_id_array[i], BCM_GPORT_MCAST_GET(port_array[i]) + MC_MEMBER_DEST_BITMAP_OFFSET));
        } else {
            uint32 local_port;
            bcm_pbmp_t pbmp_local_ports;
                                           ...
                                           const bcm_gport_t *port_array, const bcm_if_t *encap_id_array, const unsigned size)
{
    ...
    if (mc_group - data->egress_mc_groups >= 0 && mc_group - data->egress_mc_groups < SOC_DPP_CONFIG(unit)->tm.nof_mc_ids) { /* egress MC group */
        ...
        for (bcm_buf_i = 0; bcm_buf_i < size ; ++bcm_buf_i) {
            ...
            { /* direct Queue_id */
                dest = destination.id | ARAD_MC_ING_DESTINATION_FLD_TYPE_TM_FLOW;
                ...
                dest |= ARAD_MC_ING_DESTINATION_FLD_TYPE_PORT;
            }
    ...
    if (is_open && mc_member_is_used_get(mc_group) ) {
        for (; ;) {
...
int test_bcm_multicast_verify_all(int unit)
{
    ...
    uint8* outlif2dsp; /* will contain the outlif to dsp mapping from EGQ_MAP_OUTLIF_TO_DSPm */
    ...
    if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
      ...
      max_ingr_reps_in_mini = 4 - 2;
    } else {
      switch (SOC_DPP_CONFIG(unit)->arad->init.dram.fmc_dbuff_mode) {
        case JERICHO_INIT_FMC_4K_REP_256K_DBUFF_MODE:
          max_ingr_reps = 4096 - 2;
          break;
    ...
    BCMDNX_IF_ERR_EXIT(READ_EGQ_EGRESS_REPLICATION_GENERAL_CONFIGr(unit, REG_PORT_ANY, &temp));
    ...
    if (soc_mem_dmaable(unit, IRR_MCDBm, SOC_MEM_BLOCK_ANY(unit, IRR_MCDBm))) { /* use DMA, separate allocation for memory to be filled by DMA */
        nof_uint32s = (MCDS_INGRESS_LINK_END(mcds) + 1) * ARAD_MC_TABLE_ENTRY_SIZE + /* MCDB */
                      ARAD_IRDB_NOF_ENTRIES * ARAD_IRDB_ENTRY_SIZE + /* IDR_IRDB */
                      ((ARAD_MC_VLAN_BITMAP_ENTRY_SIZE * data->nof_egress_bitmaps /* VLAN table */
        ...
        nof_uint32s = ((SOC_DPP_CONFIG(unit)->tm.nof_mc_ids / 16 /* used bitmap */
                      + 1) / 2 * 2); /* add padding to 8 byte boundary */
        BCMDNX_ALLOC(alloced_mem, 4 * nof_uint32s + ARAD_MC_MAX_GROUP_REPLICATIONS_ANY * 2 * sizeof(mc_member_data) + MC_NOF_OUTLIF_TO_PORT, "mc_verify_all");
        if (!alloced_mem) {
        ...
        vlan_table = irdb + ARAD_IRDB_NOF_ENTRIES * ARAD_IRDB_ENTRY_SIZE;
        ...
        outlif2dsp = (uint8*)(mcg_buf2 + ARAD_MC_MAX_GROUP_REPLICATIONS_ANY);
        TEST_ASSERT(outlif2dsp == ((uint8*)alloced_mem) + 4 * nof_uint32s + ARAD_MC_MAX_GROUP_REPLICATIONS_ANY * 2 * sizeof(mc_member_data));
    } else { /* do not use DMA, everything in the same memory allocation */
        nof_uint32s = (MCDS_INGRESS_LINK_END(mcds) + 1) * ARAD_MC_TABLE_ENTRY_SIZE + /* MCDB */
                      ARAD_IRDB_NOF_ENTRIES * ARAD_IRDB_ENTRY_SIZE + /* IDR_IRDB */
                      ((ARAD_MC_VLAN_BITMAP_ENTRY_SIZE * data->nof_egress_bitmaps + /* VLAN table */
                      SOC_DPP_CONFIG(unit)->tm.nof_mc_ids / 16 /* used bitmap */
                      + 1) / 2 * 2); /* add padding to 8 byte boundary */
        BCMDNX_ALLOC(alloced_mem, 4 * nof_uint32s + ARAD_MC_MAX_GROUP_REPLICATIONS_ANY * 2 * sizeof(mc_member_data) + MC_NOF_OUTLIF_TO_PORT, "mc_verify_all");
        if (!alloced_mem) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_MEMORY, (_BCM_MSG("Failed allocating test memory for test_bcm_multicast_verify_all")));
        ...
        irdb = mcdb + (MCDS_INGRESS_LINK_END(mcds) + 1) * ARAD_MC_TABLE_ENTRY_SIZE;
        vlan_table = irdb + ARAD_IRDB_NOF_ENTRIES * ARAD_IRDB_ENTRY_SIZE;
        used_bitmap = vlan_table + ARAD_MC_VLAN_BITMAP_ENTRY_SIZE * data->nof_egress_bitmaps;
    ...
    for (i = SOC_DPP_CONFIG(unit)->tm.nof_mc_ids / 16; i;) { /* init used_bitmap to 0 */
        used_bitmap[--i] = 0;
    }
    /* initialize outlif2dsp from EGQ_MAP_OUTLIF_TO_DSPm */
    BCMDNX_IF_ERR_EXIT(soc_mem_read_range(unit, EGQ_MAP_OUTLIF_TO_DSPm, MEM_BLOCK_ANY, 0, MC_NOF_OUTLIF_TO_PORT - 1, mcdb));
    for (i = 0; i < MC_NOF_OUTLIF_TO_PORT; ++i) {
        outlif2dsp[-(int32)i] = soc_mem_field32_get(unit, EGQ_MAP_OUTLIF_TO_DSPm, mcdb + i, DSPf);
    }
    ...
    BCMDNX_IF_ERR_EXIT(soc_mem_read_range(unit, IDR_IRDBm, MEM_BLOCK_ANY, 0, ARAD_IRDB_NOF_ENTRIES - 1, irdb));
    if (data->nof_egress_bitmaps)
        BCMDNX_IF_ERR_EXIT(soc_mem_read_range(unit, EGQ_VLAN_TABLEm, MEM_BLOCK_ANY, 0, data->nof_egress_bitmaps - 1, vlan_table));

#ifndef _ARAD_MC_NO_IMPLEMENTATION_SPECIFIC_TESTS
    {
        arad_mcdb_entry_t *entry = MCDS_GET_MCDB_ENTRY(mcds, 0);
        uint32* mcdb_ptr = mcdb;
        for (i = ARAD_MULT_NOF_MULTICAST_GROUPS; i; --i, ++entry, mcdb_ptr += ARAD_MC_TABLE_ENTRY_SIZE) { 
    ...
    if (data->ingress_mc_groups) {
        for (i = 0; i < SOC_DPP_CONFIG(unit)->tm.nof_ingr_mc_ids; ++i) {
            ...
            if (is_open) { /* if group is open compare its members */
                ...
                mcg_size1 = mcg_size2 > max_ingr_reps_in_mini ? 1 : 3; /* expected value in IRDB */
            } else {
    ...
    for (; i < SOC_DPP_CONFIG(unit)->tm.nof_mc_ids; ++i) {
        ...
        if (is_open) { /* if group is open compare its members */
            ...
            /* read MC group from mcdb into mcg_buf2 */
            mcg_size2 = 0;
            for (j = ARAD_MULT_NOF_MULTICAST_GROUPS + i; j != DPP_MC_EGRESS_LINK_PTR_END;) { 
                uint32* mcdb_ptr = mcdb + j * ARAD_MC_TABLE_ENTRY_SIZE;
                uint32* used_bitmap_ptr = used_bitmap + (j / 32);
                uint32 used_bitmap_mask = 1 << (j % 32);
                uint32 cud_f;
...
int test_bcm_mc_arad_basic(int unit)
{
    ...
    data = test_data[unit];
    BCMDNX_ALLOC(ids, sizeof(SOC_TMC_MULT_ID)*ARAD_MULT_NOF_MULTICAST_GROUPS, "test_bcm_mc_arad_basic.ids"); 
        
    if (data->ingress_mc_groups) { /* create and set ingress MC groups */
        for (i = 0; i < ARAD_MULT_NOF_MULTICAST_GROUPS; ++i) { /* init random group id selection array */ 
            ids[i] = i;
        }
...
int test_bcm_mc_arad_mid_size(int unit)
{
    ...
    test_iter_info iters[] = { /* use all the MCDB */
...
#define REP_TEST_GET_EGRESS_MCID(rand) (((rand) % (ARAD_MULT_NOF_MULTICAST_GROUPS - nof_bitmap_groups)) + nof_bitmap_groups) 
...
#endif
#define REP_TEST_GET_EGRESS_BITMAP_REPS_NUM(rand, reps) ((reps) > 14 ? (rand) % 7 : 0)
#define REP_TEST_GET_EGRESS_PORT_CUD_REPS_NUM(rand, reps, bitmap_reps) ((((reps) - (bitmap_reps)) * ((rand) % 53 + 48)) / 100)
...
int test_bcm_mc_arad_replications(int unit, unsigned iters_num)
{
    ...
    egress_groups = ingress_groups + (ARAD_MULT_NOF_MULTICAST_GROUPS/32); 
    ...
    for (iteration = 1; iteration <= iters_num; ++iteration) { /* start a test iteration */
        ...
        while (!full) { /* while we think we should not run out of mcdb entries */
            ...
            if (REP_TEST_IS_INGRESS(rand_val)) { /* create and fill an ingress group */
                ...
                unsigned bitmap_reps = REP_TEST_GET_EGRESS_BITMAP_REPS_NUM(rand_val, reps);
                unsigned port_cud_reps = REP_TEST_GET_EGRESS_PORT_CUD_REPS_NUM(rand_val, reps, bitmap_reps);
        ...
        for (; k < j; --k) {
            ...
            if (REP_TEST_IS_INGRESS(rand_val)) { /* ingress group */
                ...
                bitmap_reps = REP_TEST_GET_EGRESS_BITMAP_REPS_NUM(rand_val, reps);
                port_cud_reps = REP_TEST_GET_EGRESS_PORT_CUD_REPS_NUM(rand_val, reps, bitmap_reps);
...
{
    unsigned i, eg_reps = 0, ing_groups, initialized = 0;
    ...
    TEST_ASSERT(ing_groups < SOC_DPP_CONFIG(unit)->tm.nof_mc_ids - 8 && ing_groups > SOC_DPP_CONFIG(unit)->tm.nof_mc_ids - MY_NOF_EGRESS_GROUP_REPS);
    ...
    BCMDNX_IF_ERR_EXIT(test_bcm_multicast_verify_all(unit)); 
    /* TEST_PRINT((_BCM_MSG("should be: 3 free entries"))); */
    ...
    /* BCMDNX_IF_ERR_EXIT(test_bcm_multicast_verify_all(unit)); */
    /* TEST_PRINT((_BCM_MSG("should be: 4 free entries"))); */
    ...
    BCMDNX_IF_ERR_EXIT(test_bcm_multicast_egress_delete(unit, mcid, gport, cuds_port_reps[3], may_fail));
    BCMDNX_IF_ERR_EXIT(test_bcm_multicast_verify_all(unit));
    /* TEST_PRINT((_BCM_MSG("should be: 6 free entries"))); */
    ...
    /* BCMDNX_IF_ERR_EXIT(test_bcm_multicast_verify_all(unit)); */
    /* TEST_PRINT((_BCM_MSG("should be: 7 free entries"))); */
    ...
    BCMDNX_IF_ERR_EXIT(test_bcm_multicast_verify_all(unit));
    /* TEST_PRINT((_BCM_MSG("should be: 8 free entries"))); */
    ...
    for (i = 0, full = 0; !full; ++i) {
        REP_TEST_IF_ERR_FAIL(test_bcm_multicast_ingress_add(unit, 0, gport, 0xe100 + i, may_fail));
    }
    may_fail = FALSE;
    BCMDNX_IF_ERR_EXIT(test_bcm_multicast_verify_all(unit));
    TEST_PRINT((_BCM_MSG("created a 2 replications group and filled MCDB again")));
    TEST_ASSERT(i <= 9); /* verify the number of free entries */  
    /* BCMDNX_IF_ERR_EXIT(test_bcm_multicast_verify_all(unit)); */

    BCMDNX_IF_ERR_EXIT(test_bcm_multicast_egress_delete(unit, mcid - 1, port_array[4], cud_array[4], may_fail));
    REP_TEST_IF_ERR_FAIL(test_bcm_multicast_ingress_add(unit, 0, gport, 0xe100 + i, may_fail)); /* occupy the deleted entry */
    /* BCMDNX_IF_ERR_EXIT(test_bcm_multicast_verify_all(unit)); */
    /* TEST_PRINT((_BCM_MSG("should be: 0 free entries"))); */
    BCMDNX_IF_ERR_EXIT(test_bcm_multicast_egress_delete(unit, mcid - 1, port_array[5], cud_array[5], may_fail));
    BCMDNX_IF_ERR_EXIT(test_bcm_multicast_verify_all(unit));
    /* TEST_PRINT((_BCM_MSG("should be: 1 free entries"))); */
    may_fail = TRUE;
    full = 0;
    REP_TEST_IF_ERR_FAIL(test_bcm_multicast_ingress_add(unit, 0, gport, 0xe100 + ++i, may_fail)); /* occupy the deleted entry */
    TEST_ASSERT(full);
...
#endif /* defined(BCM_88650_A0) || defined(BCM_88750_A0) */
