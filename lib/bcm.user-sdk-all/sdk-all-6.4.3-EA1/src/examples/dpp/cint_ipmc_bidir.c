...
int ipv4_mc_bidir_example(int unit, int in_port, int out_port){
    ...
    /* init WA PMF to drop upon admission test fail
     ...
     */
    ipmc_bidir_wa_setup(unit,0,0);

  	/* Create ingress intf, over vlan 1 */
	create_l3_intf(unit, 0, in_port, source_local_port_id, in_vlan, vrf, my_mac, &ingress_intf[0]);

    /* Create ingress intf over vlan 2 */
    create_l3_intf(unit, 0, in_port, source_local_port_id, in_vlan_2, vrf, my_mac, &ingress_intf[1]);
    ...
    for(group_index = 0; group_index < nof_groups; ++group_index) {
        ...
        create_l3_intf(unit, 0, 1, dest_local_port_id, out_vlan, vrf, my_mac, &egress_intf);
    ...
    for(group_index = 0; group_index < nof_groups; ++group_index) {
        ...
        mc_groups[group_index] = create_ip_mc_group(unit, BCM_MULTICAST_EGRESS_GROUP, mc_groups[group_index], dest_local_port_id, nof_dest_ports_per_group, out_vlan);
	...
	rv = add_ip4mc_entry(unit, ip_groups[2], 0, in_vlan, mc_groups[2]);
	if (rv != BCM_E_NONE) {
        printf("fail at :<*,G,*> %d \n",ip_groups[2]);
        return rv;
    }

    rv = add_ip4mc_entry(unit, ip_groups[2], 0, in_vlan_2, mc_groups[3]);
    if (rv != BCM_E_NONE) {
        printf("fail at :<*,G,*> %d \n",ip_groups[3]);
        return rv;
    }
