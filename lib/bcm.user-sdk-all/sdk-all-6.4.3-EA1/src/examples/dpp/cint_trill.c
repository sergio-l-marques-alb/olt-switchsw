/* $Id: cint_trill.c,v 1.45 Broadcom SDK $
...
bcm_multicast_t l2mc_group = 6162;
...
int port_config(int unit, int port, int designated_vlan) {
    ...
    /* 
     * Take revision information on device. 
     * In case revision is A0 (0x0) then Disable Same-interface-filter. 
     * In ARAD-B bug is fixed and so there is no need to disable the filter.                                                               .
     */
    if (is_arad_a0(unit)) {
...
add_remote_rbridge(int unit, bcm_mac_t remote_mac_rb, bcm_vlan_t vsi, int port, int nickname, int flags, bcm_trill_port_t *trill_port) {
    ...
    bcm_l3_egress_t_init(egr_obj_rb);
    sal_memcpy(egr_obj_rb.mac_addr, remote_mac_rb, 6);
    egr_obj_rb.vlan     = vsi; /* VSI < 4K */
    egr_obj_rb.port     = port;
    egr_obj_rb.flags    = BCM_L3_TRILL_ONLY;

    rv = bcm_l3_egress_create(unit, 0, &egr_obj_rb, &egr_if_id_rb);
    if (rv != BCM_E_NONE) {
...
mult_adjacency(int unit, bcm_gport_t port_id, int add_adjacency) {
    ...
    printf(" multicast adjacency add/remove %d\n", port_id);
    if (port_id <= 0) {
    ...
    mult_adjacency.encap_intf = egr.encap_id;
    mult_adjacency.port = egr.port;
...
config_local_rbridge(int unit, int nickname_local) {
    ...
    printf("Created trill_port_local: %d \n", trill_port_local.trill_port_id);
...
add_trill_port_mc(int unit, int mc_group, int nickname, int fec_id, int *eg_if, bcm_trill_port_t *trill_port) {
    ...
    BCM_GPORT_MCAST_SET(egr_obj.port, mc_group );
    egr_obj.flags    = BCM_L3_TRILL_ONLY ;
    ...
    rv = bcm_l3_egress_create(unit, flags, &egr_obj, eg_if);
    BCM_IF_ERROR_RETURN(rv);
    printf("Created egress obj: %d \n", *eg_if);
...
assign_flooding_to_MC_trill_port(int unit, bcm_gport_t id, int vsi, int flood_group ) {
    ...
    bcm_l3_egress_t_init(egr_obj);
    BCM_GPORT_MCAST_SET(egr_obj.port, flood_group);
    egr_obj.flags    = BCM_L3_TRILL_ONLY ;
...
create_mc_group(int unit, int group, int with_trill_copies, int with_host_copies) {
    ...
    if (with_trill_copies) {
        ...
        rv = bcm_multicast_egress_add(unit, group, port_1, trill_encap);
        BCM_IF_ERROR_RETURN(rv);
        printf ("Added port %d to group. (Trill copy)\n", port_1);

     
        rv = bcm_multicast_trill_encap_get(unit, -1, trill_port_rb2.trill_port_id ,0, &trill_encap); 
        BCM_IF_ERROR_RETURN(rv);

        rv = bcm_multicast_egress_add(unit, group, port_2, trill_encap);
        BCM_IF_ERROR_RETURN(rv);
        printf ("Added port %d to group. (Trill copy)\n", port_2);
    }
...
remove_flooding_port(int unit, int vsi, int mc_group, int nick, int flood_port_id ) {
    ...
    rv = remove_remote_rbridge(unit,flood_port_id);
    BCM_IF_ERROR_RETURN(rv);


    rv = bcm_multicast_egress_get(0,mc_group,10,p,e,&port_cnt);
    BCM_IF_ERROR_RETURN(rv);
...
add_root_rbridge(int unit, int rootname, bcm_trill_port_t *trill_port, bcm_multicast_t flooding_mc) {
    ...
    if (flooding_mc != -1) {
        bcm_l3_egress_t_init(egr_obj);
        BCM_GPORT_MCAST_SET(egr_obj.port, flooding_mc );
        egr_obj.flags    = BCM_L3_TRILL_ONLY ;
...
int
remove_root_rbridge(int unit, bcm_gport_t id) {
    bcm_trill_port_t            trill_port;
     int rv = 0;
...
trill_uni(int unit) {
    ...
    /* Add TRILL Virtual Port for local RB (configures my nickname) */
    rv = config_local_rbridge(unit, nickname_rb_local);
    if (rv != BCM_E_NONE) {
...
trill_mc_transit_fecless_config(int unit, int is_fgl, int double_tag, int is_2pass) {
    ...
    /*enable RPF Trap*/
    rv = rpf_drop_set(0,10, rpf_group, &qual_id_ing_nickname, &qual_id_eg_nickname);
    BCM_IF_ERROR_RETURN(rv);
...
trill_mc_ingress_ecmp(int unit) {
    uint32              rv = 0;
    uint32              fecs[ecmp_mc_ing_nof_ifs] = {1300, 1301, 1302};
    ...
    for (i = 0; i < ecmp_mc_ing_nof_ifs; ++i) {
        ...
        BCM_IF_ERROR_RETURN(rv);
        printf ("Added port %d to group %d with encap %d.\n", ports[i],ecmp_mc_ing_mc_groups[i], trill_encap);
...
int
test_trill_mc_uc_transit(int unit, int p1, int p2, int trill_ether_type,int hop, bcm_multicast_t group) {
    port_1 =  p1;
    port_2  = p2;
    trill_ethertype = trill_ether_type;
    hopcount = hop;
...
test_clear_trill_uni(int unit) {
    ...
    if (trill_port_local.trill_port_id <= 0) {
        printf("Error, Try to delete invalid id of local port %d \n", trill_port_local.trill_port_id );
    }
...
test_clear_trill_mc_transit_fecless_config(int unit, int is_fgl, int double_tag, int is_2pass) {
    ...
    rv = bcm_multicast_egress_delete(unit, l2mc_group, port_1, encap_id);
    BCM_IF_ERROR_RETURN(rv);
    
     /* Get trill encapsulation */   
    rv = bcm_multicast_trill_encap_get(unit, l2mc_group, trill_port_rb2.trill_port_id ,0, &encap_id); 
    BCM_IF_ERROR_RETURN(rv);

    /* Delete port + encapsulation from MC group */
    rv = bcm_multicast_egress_delete(unit, l2mc_group, port_2, encap_id);
    BCM_IF_ERROR_RETURN(rv);
    host_encap = get_encap_id_to_bridge(unit);

    if (!is_2pass) {
        rv = bcm_multicast_egress_delete(unit, l2mc_group, port_1, host_encap);
        BCM_IF_ERROR_RETURN(rv);

        rv = bcm_multicast_egress_delete(unit, l2mc_group, port_2, host_encap);
        BCM_IF_ERROR_RETURN(rv);
    }
    /* old implementation that configure sw db (mc-id to nickname)*/
    if (mc_id) {
...
trill_mc_transit_fecless_2pass(int unit, int is_ptch, int is_fgl, int double_tag) {
    ...
    rv = port_config(unit, recycle_port, vid_2_desig);
    BCM_IF_ERROR_RETURN(rv);
    ...
    rv = bcm_port_class_set(unit, recycle_port, bcmPortClassFieldIngress, 0x1);
    BCM_IF_ERROR_RETURN(rv);
    ...
    rv = two_pass_snoop_set(0, bcmRxTrapUserDefine, recycle_port, &snoop_trap_gport_id);
    BCM_IF_ERROR_RETURN(rv);

    rv = trill_2pass_field_group_set(0, snoop_trap_gport_id, &snoop_group);
    BCM_IF_ERROR_RETURN(rv);
    ...
    rv = trill_mc_transit_fecless_2pass_config_second_pass(0);
    BCM_IF_ERROR_RETURN(rv);
...
int
test_trill_mc_transit_fecless_2pass(int unit, int p1, int p2, int trill_ether_type,int hop, 
                                    bcm_multicast_t group, int rcy_port, int is_ptch,
...
test_clear_trill_mc_transit_fecless_2pass(int unit,  int is_fgl, int double_tag) {
    ...
    rv = port_config_reset(unit, recycle_port, vid_2_desig);
    if (rv != BCM_E_NONE) {
        printf("Error, in port_config_reset\n");
        return rv;
    }
