...
*/
bcm_multicast_t create_ip_mc_group(int unit, int flags, int ipmc_index, int dest_local_port_id, int nof_ports, int vlan){
    ...
    for(port_indx = 0; port_indx < nof_ports; ++port_indx) {
      ...
      rv = bcm_multicast_l3_encap_get(unit, mc_id, dest_gport+port_indx, vlan,&cud);
      if (rv != BCM_E_NONE) {
...
int run_with_ports_ipv4_mc_example(int unit, int iport, int oport){
    ...
    bcm_multicast_t mc_id = create_ip_mc_group(unit, BCM_MULTICAST_EGRESS_GROUP, ipmc_index, dest_local_port_id, nof_dest_ports, vlan);
    ...
    create_l3_intf(unit, 0, 1, source_local_port_id, vlan, 0, mac_l3_ingress, &ingress_intf);
...
int ipv4_mc_best_match_example(int unit, int in_port, int out_port){
	...
	create_l3_intf(unit, 0, 1, source_local_port_id, in_vlan, 0, my_mac, &ingress_intf);

    /* Create ingress intf, for ethernet termination for vlan 2 (mismatch). */
    create_l3_intf(unit, 0, 1, source_local_port_id, in_vlan_2, 0, my_mac, &ingress_intf);
    ...
    for(group_index = 0; group_index < nof_groups; ++group_index) {
        ...
        if(out_vlan != in_vlan && out_vlan != in_vlan_2) {
            create_l3_intf(unit, 0, 1, source_local_port_id, out_vlan, 0, my_mac, &ingress_intf);
        }
        if(out_vlan_step == 0) {
            break;
        }
    ...
    for(group_index = 0; group_index < nof_groups; ++group_index) {
        ...
        ipmc_indexes[group_index] = create_ip_mc_group(unit, BCM_MULTICAST_EGRESS_GROUP, ipmc_indexes[group_index], dest_local_port_id, nof_dest_ports_per_group, out_vlan);
...
int ipv4_mc_comp_bridge(int unit, int in_port, int out_port){
    ...
    /* Create the IP MC entry with <FID,DIP>
       ...
       - bcm_switch_control_set(0,bcmSwitchL3McastL2,1);
     */
    ...
    /* Create the IP MC entry with <FID,MC-DA>
        packet is to be forwarded according to this entry if
        - IPMC routing is disabled on intf
        - bcm_switch_control_set(0,bcmSwitchL3McastL2,0);
       */
...
int ipv6_mc_example(int unit){
	...
	/* Create the IPMC group, same as we did for IPv6	 */
	bcm_multicast_t mc_id = create_ip_mc_group(unit, BCM_MULTICAST_EGRESS_GROUP, ipmc_index, dest_local_port_id, vlan);
