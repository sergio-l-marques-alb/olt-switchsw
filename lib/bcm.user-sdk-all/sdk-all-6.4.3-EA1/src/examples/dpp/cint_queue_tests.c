 ...
 */
int test_voq_attributes(int unit, int chip_rev) /* chip_rev == 0 means ARAD_A0, 3 is Arad+ */
{
    int rv, i, j;
    ...
    if ((rv = bcm_cosq_delay_tolerance_level_set(unit, BCM_COSQ_DELAY_TOLERANCE_14, &dt1)) != BCM_E_NONE) {
        printf ("failed to set delay tolerance 15\n");
        return BCM_E_FAIL;
    }
    ...
    for (i = test_first_queue, j = 0; i <= test_last_queue; i += test_q_num_in_q_bundle) {
        ...
        for (cosq = 0; cosq < test_q_num_in_q_bundle; ++cosq) {
            ...
            if ((rv = bcm_cosq_gport_sched_get(unit, base_q, cosq, &mode, &weight)) != BCM_E_NONE) {
                ...
                printf("queue base: 0x%x, cosq: %d retrieved credit request (tolerance level) profile 0x%x is not the one that was set\n", i, cosq, mode);
                return BCM_E_FAIL;
    ...
    if (chip_rev == 3) { /* check Arad+ two credit values selected per Q type */
...
int verify_credit_watchdog_config(int unit, int required_mode, int first_q, int last_q, int required_common_fsm) {
    ...
    reg_val f;
    char *reg_name = "IPS_CREDIT_WATCHDOG_CONFIGURATION";
...
int get_delete_ths_common_fsm(int unit, int nof_profiles, int *profiles, char **profile_names, int common_fsm_time, int *out_delete_ths)
{
    ...
    for (i = 0; i < nof_profiles; ++i) {
        ...
        v[0] = -1;
        if (diag_mem_field_get(unit, "IPS_CRWDTH", "WD_DELETE_Q_TH", index, v) || v[0] != t) {
            ...
            return BCM_E_FAIL;
        } else if (diag_mem_field_get(unit, "IPS_CRWDTH", "WD_STATUS_MSG_GEN_PERIOD", index, v) || v[0] != exp + fsm_exp) {
