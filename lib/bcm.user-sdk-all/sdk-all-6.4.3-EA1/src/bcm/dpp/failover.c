    ...
    bcm_failover_t *failover_id)
{
    ...
    uint8                           fec_success;
    int32 failover_type, nof_alloc_ids = 1;
    ...
    if ((SOC_IS_ARAD(unit)) && (failover_type == DPP_FAILOVER_TYPE_L2_LOOKUP)) {
        ...
        /* For all other failover types, a failover object should be allocated */
        rv = bcm_dpp_am_failover_alloc(unit, alloc_flags, failover_type, nof_alloc_ids, &tmp_failover_id);
        BCMDNX_IF_ERR_EXIT(rv);

        /* Clean up the HW status of this failover-id */
        rv = _bcm_dpp_failover_state_hw_set(unit, tmp_failover_id, failover_type, 1);
    ...
    bcm_failover_t  failover_id)
{
    ...
    DPP_FAILOVER_ID_GET(failover_id_val, failover_id);
    rv = bcm_dpp_am_failover_dealloc(unit, failover_type, 1, failover_id_val);
    BCMDNX_IF_ERR_EXIT(rv);
