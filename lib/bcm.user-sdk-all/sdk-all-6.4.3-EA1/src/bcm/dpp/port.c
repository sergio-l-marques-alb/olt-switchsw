...
_bcm_petra_port_to_nif_id(int unit, bcm_port_t port, int *nif_id)
{
    ...
    rv = (MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_port_to_interface_map_get,(unit, tm_port, core, &if_id, NULL)));
    BCMDNX_IF_ERR_EXIT(rv);
              ...
              if (SOC_IS_ARAD(unit)) {
                  BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_ndx) {
                      if (IS_RCY_PORT(unit,port_ndx)) {
                          ...
                          soc_sand_rv = arad_port_header_type_get(unit, soc_ppd_port, &header_type_incoming, &header_type_outgoing); 
                          BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
...
_bcm_petra_port_control_lanes_set(int unit, bcm_port_t port, int value)
{
    ...
    if (SOC_IS_ARAD(unit)) {
        ...
        BCM_SAND_IF_ERR_EXIT(arad_port_to_dynamic_interface_map_set(unit, tm_port, ARAD_PORT_DIRECTION_BOTH, &mapping_info, FALSE));
...
bcm_petra_fap_port_parent_nif_get(int unit, int fap_port, int *p_nif_port)
{
    ...
    rv = MBCM_DPP_SOC_DRIVER_CALL(unit, mbcm_dpp_port_to_interface_map_get, (unit, fap_port, 0, &if_id, NULL));
        ...
        if (flags & BCM_PORT_PON_TUNNEL_WITH_ID)
        {
            if (pon_channel_profile == _BCM_PPD_PON_CHANNEL_DEFAULT_PROFILE) {
                *tunnel_port_id = gport_info.local_port;
                BCM_EXIT; /* It's not necessary to add default profile. */
            } else if (pon_channel_profile >= _BCM_PPD_NOF_PON_CHANNEL_PROFILE(nof_channels)) {
    ...
    bcm_port_encap_config_t *encap_config)
{
    ...
    soc_sand_dev_id = (unit);
    {
        ...
        rv = arad_port_encap_config_set(soc_sand_dev_id,port_ndx,&info);
        if (rv != 0)
        {
    ...
    bcm_port_encap_config_t *encap_config)
{
   ...
   soc_sand_dev_id = (unit);
   {
       ...
       SOC_TMC_L2_ENCAP_INFO_clear(&info);
       rv = arad_port_encap_config_get(soc_sand_dev_id,port_ndx,&info);
       if (rv != 0)
       {
...
int bcm_petra_port_add(int unit, bcm_port_t port, uint32 flags, bcm_port_interface_info_t *interface_info, bcm_port_mapping_info_t *mapping_info)  
{
    ...
    uint32      temp_flags = 0;
    uint32      mirror_profile, nof_lanes, nof_channels;
    int         i, core, speed, base_q_pair, curr_q_pair, mc_reserved_pds, discount_data, header_type;
    ...
    if (flags & BCM_PORT_ADD_CONFIG_CHANNELIZED) 
    {
        BCMDNX_IF_ERR_EXIT(soc_port_sw_db_is_channelized_port_set(unit, port, TRUE));
    }
    ...
    BCM_SAND_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_port_to_dynamic_interface_map_set, (unit, tm_port, SOC_TMC_PORT_DIRECTION_BOTH, &port2if_mapping_info, FALSE)));
...
int bcm_petra_port_get(int unit, bcm_port_t port, bcm_port_interface_info_t *interface_info, bcm_port_mapping_info_t *mapping_info) 
{
    bcm_pbmp_t phy_ports;
...
{
    uint32           is_valid, is_master, counter_flags;
    bcm_port_if_t    interface;
    int              core, old_profile, new_profile, is_last, is_allocated, mc_reserved_pds;
    int              base_q_pair, profile, rv, counter_interval;
    ...
    /* make sure port queues are empty before reducing cgm reserved pds */
    BCM_SAND_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_action_cmd_cosq_flush,(unit,port,FALSE)));
    ...
    /* unmap port */
    BCM_SAND_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_port_to_dynamic_interface_unmap_set,(unit, tm_port, core, SOC_TMC_PORT_DIRECTION_BOTH)));
