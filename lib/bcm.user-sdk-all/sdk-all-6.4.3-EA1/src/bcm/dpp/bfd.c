...
int bcm_petra_bfd_endpoint_create(int unit, bcm_bfd_endpoint_info_t *endpoint_info) {
    ...
    SOC_SAND_PP_IPV4_ADDRESS ipv4_address;
    int is_allocated;
    int dip_ndx = 0;
    ...
    uint8 port_is_equal;
	uint8 found_profile;
	uint8 is_mp_type_flexible_dummy;
    ...
    correct_flags |= BCM_BFD_ENDPOINT_UPDATE | BCM_BFD_ENDPOINT_WITH_ID | BCM_BFD_ENDPOINT_IN_HW | BCM_BFD_ENDPOINT_MPLS_TP_POLL_SEQUENCE_ENABLE |
                     ...
                     BCM_BFD_ENDPOINT_RDI_AUTO_UPDATE | BCM_BFD_ENDPOINT_RDI_ON_LOC | BCM_BFD_ENDPOINT_RDI_CLEAR_ON_LOC_CLEAR | BCM_BFD_ENDPOINT_HW_ACCELERATION_SET;
    ...
    if (endpoint_info->flags & BCM_BFD_ENDPOINT_UPDATE) {
        ...
        if ((endpoint_info->flags & BCM_BFD_ENDPOINT_WITH_ID) && (endpoint_info->id == -1)) {
			/* Default endpoint */
            ...
            soc_sand_rv = soc_ppd_oam_classifier_oam1_entries_insert_default_profile(unit, &profile_data, NULL, 1);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        ...
        if (_BCM_BFD_ENDPOINT_IS_ACCELERATED_TO_OAMP(*endpoint_info) &&
            ...
            if (endpoint_info->type == bcmBFDTunnelTypeMpls)  {
                ...
                if (endpoint_info->flags & BCM_BFD_ENDPOINT_MULTIHOP) {
                    ipv4_address.address[0] = endpoint_info->dst_ip_addr;
                    rv = _bcm_dpp_am_template_bfd_ipv4_dip_alloc(unit, 0 /*flags*/, &ipv4_address, &is_allocated, &dip_ndx);
                    BCMDNX_IF_ERR_EXIT(rv);
                    if (is_allocated) {
                        soc_sand_rv = soc_ppd_oam_bfd_my_bfd_dip_ipv4_set(unit, dip_ndx, ipv4_address);
                        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                    }
                } else {
        ...
        if (endpoint_info->flags & BCM_BFD_ENDPOINT_HW_ACCELERATION_SET) {
            ...
            if (!(endpoint_info->flags & BCM_BFD_ENDPOINT_IN_HW)) {
                ...
                soc_sand_rv = soc_ppd_oam_classifier_find_mep_and_profile_by_lif_and_mdlevel(
                    unit, oem_key, 0/*mdlevel*/, 0/*is_upmep*/, &found, &profile_temp, &found_profile, &is_mp_type_flexible_dummy);
                BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                if (found) {
                    BCMDNX_ERR_EXIT_MSG(BCM_E_EXISTS,
            ...
            if (endpoint_info->flags & BCM_BFD_ENDPOINT_IN_HW) {
                classifier_mep_entry.non_acc_profile = 0; 
            }
            else {
            ...
            soc_sand_rv = soc_ppd_oam_classifier_oam1_2_entries_insert_according_to_profile(
                unit, &classifier_mep_entry, &profile_data, &profile_data_acc_dummy, trap_code_to_mp_map_dummy);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);

            if (endpoint_info->type == bcmBFDTunnelTypeMpls) {
                soc_sand_rv = soc_ppd_oam_tcam_bfd_lsp_ipv4_entry_add(unit, 
                                                                      classifier_mep_entry.your_discriminator, 
...
int bcm_petra_bfd_endpoint_get(
   ...
   bcm_bfd_endpoint_info_t *endpoint_info) {
    ...
    /* Default BFD trap */
    if (endpoint == -1) {
        rv = _bcm_dpp_oam_bfd_mep_info_db_get(unit, -2, &classifier_mep_entry, &found);
        BCMDNX_IF_ERR_EXIT(rv);
    ...
    if (endpoint_info->type == bcmBFDTunnelTypeMpls) {
        ...
        if (endpoint_info->flags & BCM_BFD_ENDPOINT_MULTIHOP) {
            rv = _bcm_dpp_am_template_bfd_ipv4_dip_data_get(unit, dip_ndx, &ipv4_address);
            endpoint_info->dst_ip_addr = ipv4_address.address[0];
        }
    }
...
int bcm_petra_bfd_endpoint_destroy(
   ...
   bcm_bfd_endpoint_t endpoint) {
    ...
    SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY classifier_mep_entry;
    SOC_PPD_OAM_OAMP_MEP_DB_ENTRY mep_db_entry;
    SOC_SAND_PP_IPV4_ADDRESS ipv4_dip;
    int is_last;
    uint32 trap_code;
    uint32 rmep_index_internal;
    uint32 mpls_udp_sport_ref_counter;
    ...
    if (endpoint == -1) {
        ...
        if (classifier_mep_entry.non_acc_profile != SOC_PPD_OAM_PROFILE_DEFAULT) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_INTERNAL,
                             (_BSL_BCM_MSG("Error: Internal error in default profile configuration.\n")));
        }
        rv = _bcm_dpp_am_template_oam_mep_profile_non_accelerated_free(unit, classifier_mep_entry.non_acc_profile, &is_last);
        BCMDNX_IF_ERR_EXIT(rv);

        rv = _bcm_dpp_oam_bfd_mep_info_db_delete(unit, -2);
        BCMDNX_IF_ERR_EXIT(rv);
	...
	if ((classifier_mep_entry.flags & SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_FLAG_ACCELERATED) && (classifier_mep_entry.remote_gport == BCM_GPORT_INVALID)) {
		if ((classifier_mep_entry.mep_type == SOC_PPD_OAM_MEP_TYPE_BFD_O_IPV4_1_HOP) ||
			...
			if (classifier_mep_entry.mep_type == SOC_PPD_OAM_MEP_TYPE_BFD_O_IPV4_M_HOP) {
				...
				BCMDNX_IF_ERR_EXIT(rv);
				if (is_last) {
					soc_sand_SAND_PP_IPV4_ADDRESS_clear(&ipv4_dip);
					ipv4_dip.address[0] = 0;
					soc_sand_rv = soc_ppd_oam_bfd_my_bfd_dip_ipv4_set(unit, classifier_mep_entry.dip_profile_ndx, ipv4_dip);
					BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
				}
			}
    ...
    if (!(classifier_mep_entry.flags & SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_FLAG_SERVER)) {
        ...
        if (classifier_mep_entry.mep_type == SOC_PPD_OAM_MEP_TYPE_BFD_O_MPLS) {
            soc_sand_rv = soc_ppd_oam_tcam_bfd_lsp_ipv4_entry_delete(unit, classifier_mep_entry.counter);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        }
    }
