    ...
    bcm_vlan_port_t *vlan_port)
{
    ...
    if (BCM_GPORT_IS_VLAN_PORT(vlan_port->vlan_port_id) ) {
        ...
        if(!remote_lif && (is_local || _Bcm_dpp_sync_all)) {
            ...
            if (is_egress) {
                ...
                if (SOC_IS_ARAD(unit) && SOC_DPP_CONFIG(unit)->pp.pon_application_enable) {
					...
					if(soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "evc_filter", 0) && eg_ac_info.edit_info.use_as_data_entry)
					{
						vlan_port->egress_tunnel_value = eg_ac_info.edit_info.data[0];
					}
#endif
                }
            }
        ...
        /* get PON egress info */
        if (SOC_IS_ARAD(unit) && SOC_DPP_CONFIG(unit)->pp.pon_application_enable) {
            rv = _bcm_dpp_vlan_port_pon_egress_info_get(unit, lif_id, vlan_port);
            BCMDNX_IF_ERR_EXIT(rv);
        } else {
            /* get egress info */
            soc_sand_rv = soc_ppd_eg_ac_info_get(soc_sand_dev_id, lif_id, &eg_ac_info);
        ...
        /* Finally, determine wether gport is ingress only or egress only according to the vlan_port_id */
