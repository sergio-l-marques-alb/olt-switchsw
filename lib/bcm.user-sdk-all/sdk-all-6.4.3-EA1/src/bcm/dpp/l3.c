...
#define DPP_L3_UNIT_INIT_CHECK \
    if ((!(SOC_DPP_STATE(unit)->l3_state))) { \
        BCMDNX_ERR_EXIT_MSG(BCM_E_INIT, (_BSL_BCM_MSG("unit %d not initialised"), unit)); \
    }

#define DPP_L3_LOCK_TAKE \
    if (sal_mutex_take(SOC_DPP_STATE(unit)->l3_state->lock, sal_mutex_FOREVER)) { \
...
#define DPP_L3_LOCK_RELEASE \
    if ((unit) >= 0 && ((unit) < (BCM_MAX_NUM_UNITS)) && SOC_DPP_STATE(unit)->l3_state && sal_mutex_give(SOC_DPP_STATE(unit)->l3_state->lock)) { \
...
static bcm_dpp_l3_state_t *_dpp_l3_state[BCM_MAX_NUM_UNITS];

bcm_dpp_l3_info_t *_dpp_l3_info[BCM_MAX_NUM_UNITS];
  ...
  )
{
    ...
    if(SOC_IS_ARAD(unit)){
        /* need tunnel EEP */
        if ((egr->mpls_label != BCM_MPLS_LABEL_INVALID) || egr->mpls_flags != 0 || (egr->flags & BCM_L3_ROUTE_LABEL)) {
        ...
        rv = bcm_dpp_am_l3_eep_alloc(unit, _BCM_DPP_AM_L3_EEP_TYPE_DEFAULT, alloc_flags,ll_eep_index);
        BCM_IF_ERROR_RETURN(rv);

        *eep_index = *ll_eep_index;
        BCM_EXIT;
    }
    ...
    if(alloc_flags & SHR_RES_ALLOC_WITH_ID) {
        ...
        /* link layer?*/
        if(*eep_index <= eg_encap_range_info.ll_limit) {
            /* if the eep-id the user asked for (WITH_ID flag) already exists -
                   it can be used, no error should be returned */
            if (BCM_E_EXISTS != (bcm_dpp_am_l3_eep_is_alloced(unit,*ll_eep_index))) {
                rv = bcm_dpp_am_l3_eep_alloc(unit,_BCM_DPP_AM_L3_EEP_TYPE_DEFAULT, alloc_flags,ll_eep_index);
                BCM_IF_ERROR_RETURN(rv);
            }
        }        
        else if(*eep_index <= eg_encap_range_info.ip_tnl_limit) {
            ...
            rv = bcm_dpp_am_ip_tunnel_eep_alloc(unit, _BCM_DPP_AM_IP_TUNNEL_EEP_TYPE_DEFAULT, alloc_flags, eep_index);
            BCM_IF_ERROR_RETURN(rv);
        }
        else {
            ...
            rv = bcm_dpp_am_mpls_eep_alloc(unit,_BCM_DPP_AM_MPLS_EEP_TYPE_SIMPLE_MPLS_TUNNEL,alloc_flags,2,eep_index);
            BCM_IF_ERROR_RETURN(rv);
            tunneled = 1;
        }
    }/* at any place */
  ...
  )
{
  ...
  if (_bcm_petra_l2_mymac_msb_is_set_get(unit)) { /* MyMac msb has been set already */
  ...
  )
{
  ...
  PBMP_ITER(all_ports, port_i){
    ...
    ret = soc_ppd_port_info_get(unit, port_i, &port_info);
    if (soc_sand_get_error_code_from_error_word(ret) != SOC_SAND_OK)
    { 
      LOG_ERROR(BSL_LS_BCM_L3,
                (BSL_META_U(unit,
    ...
    int *eep)
{
	...
	*eep = 
	    SOC_DPP_STATE(unit)->l3_state->_bcm_tunnel_intf_to_eep[intf - 
					       _BCM_PETRA_L3_ITF_ID_BASE];
    }
    BCM_EXIT;
exit:
    BCMDNX_FUNC_RETURN;
    ...
    int eep)
{
    ...
    SOC_DPP_STATE(unit)->l3_state->_bcm_tunnel_intf_to_eep[intf - _BCM_PETRA_L3_ITF_ID_BASE] = eep;

#ifdef BCM_WARM_BOOT_SUPPORT
    BCMDNX_IF_ERR_EXIT(_bcm_dpp_wb_l3_update_tnl_to_eep(unit, (intf - _BCM_PETRA_L3_ITF_ID_BASE) ));
#endif
  ...
  )      
{
  ...
  if (SOC_IS_ARAD(unit)) {
      ...
      case SOC_PPD_FEC_TYPE_TUNNELING_EEI:
        if (fec_entry->app_info.out_rif != 0) { /* must be null */
        ...
        if (fec_entry->type == SOC_PPD_FEC_TYPE_TUNNELING_EEI) {
            egr->flags |= BCM_BCM_L3_ENCAP_SPACE_OPTIMIZED;
        }
        break;
  ...
  )
{
  ...
  {
      egr->failover_if_id = fec_index + 1;
  ...
  if(BCM_GPORT_IS_MCAST(egr->port)) {
     is_local_conf = FALSE;
  } else {
  ...
  if (is_local_conf)
  {
      if (_BCM_PETRA_L3_ITF_IS_ENCAP(egr->encap_id)) {
          BCMDNX_IF_ERR_EXIT( _bcm_dpp_lif_usage_get(unit, -1, _BCM_PETRA_L3_ITF_VAL_GET(egr->encap_id), NULL, &usage));
...
int
  _bcm_ppd_frwrd_fec_entry_add(
    ...
    int eep_index,
    int out_rif,
    uint8 is_tunnel_outlif, /* If TRUE, FEC should be of type Tunnel and points to eep_index using format B */
    uint8 is_tunnel_eei, /* If TRUE, FEC should be of type Tunnel and points to eep_index using format C */
    bcm_l3_egress_t *egr
  )
{
  ...
  */ 
  ...
  working_fec.rpf_info.rpf_mode = _dpp_l3_state[unit]->urpf_mode;

  /* In case we should reset the L3 Hit indicator - clear on read */
  ...
  if (SOC_IS_ARAD(unit)) {
      ...
      } else if (is_tunnel_outlif) { /* out-rif Is tunnel means FEC should point to EEP outlif */
          working_fec.type = SOC_PPD_FEC_TYPE_TUNNELING;
          working_fec.eep = eep_index;
      } else if (is_tunnel_eei) { /* out-rif Is tunnel means FEC should point to EEP eei */
          working_fec.type = SOC_PPD_FEC_TYPE_TUNNELING_EEI;
          working_fec.eep = eep_index;
  ...
  )
{
  ...
  l3info->l3info_used_vrf = _dpp_l3_info[unit]->used_vrf; /* Number of virtual routers created. */
  ...
  l3info->l3info_used_intf = _dpp_l3_info[unit]->used_intf;    /* L3 interfaces used. */
  l3info->l3info_used_host = _dpp_l3_info[unit]->used_host;    /* L3 host entries used. */
  l3info->l3info_used_route = _dpp_l3_info[unit]->used_route;  /* L3 route entries used. */
      ...
      if ((((_BCM_GPORT_IS_OVERLAY(info->l3a_port_tgid)) && (SOC_IS_JERICHO(unit))) /* check FEC: check is overlay gport */
         ...
         && (info->encap_id != 0))/* check ARP */ { 
          if ((SOC_IS_ARADPLUS(unit)) &&  (BCM_GPORT_IS_FORWARD_PORT(info->l3a_port_tgid))) {
              ...
              uint8 found_fec = 0;
              uint32 fec_id;
              rv = _bcm_dpp_gport_to_fec(unit, info->l3a_port_tgid, &fec_id, &found_fec);
              BCMDNX_IF_ERR_EXIT(rv);
              if (found_fec) {/*return frwrd_fec or return error */
                  frwrd_type = 4;
              } else {
...
STATIC int _bcm_dpp_l3_check_init(int unit)
{
    ...
    if (FALSE == SOC_DPP_STATE(unit)->l3_state->init) {
        return BCM_E_INIT;
    } else {
        return BCM_E_NONE;
    }
...
{
    int status;
    bcm_dpp_l3_state_t *temp_state = NULL;
    bcm_dpp_l3_info_t  *temp_info = NULL;
    int vrrp_mode = 0;
    uint32 vrrp_flags = 0;
    ...
    BCMDNX_ALLOC(temp_state, sizeof(bcm_dpp_l3_state_t), "bcm_petra_l3_init");
    if (temp_state) {
        sal_memset(temp_state, 0, sizeof(bcm_dpp_l3_state_t));
        temp_state->lock = sal_mutex_create("_dpp_l3_unit_lock");
        if (!temp_state->lock) {
            BCM_FREE(temp_state);
    ...
    SOC_DPP_STATE(unit)->l3_state = _dpp_l3_state[unit] = temp_state;
    SOC_DPP_STATE(unit)->l3_state->urpf_mode = bcmL3IngressUrpfStrict;
#ifdef BCM_88660_A0
    ...
    if (SOC_IS_ARADPLUS(unit) && soc_property_get(unit, spn_BCM886XX_L3_INGRESS_URPF_ENABLE, 0)) {
        SOC_DPP_STATE(unit)->l3_state->urpf_mode = bcmL3IngressUrpfLoose;
    }
#endif /* BCM_88660_A0 */

    /* create new unit info information */
    BCMDNX_ALLOC(temp_info, sizeof(bcm_dpp_l3_info_t), "bcm_petra_l3_init");
    if (temp_info) {
        sal_memset(temp_info, 0, sizeof(bcm_dpp_l3_info_t));
    } else {
        BCMDNX_ERR_EXIT_MSG(BCM_E_MEMORY, (_BSL_BCM_MSG("Memory allocation failure")));
    }
    _dpp_l3_info[unit] = temp_info;
    _dpp_l3_state[unit]->l3_info = temp_info;

    status = _bcm_l3_sw_init(unit);

    /* mapping l3 interface to EEP */
    SOC_DPP_STATE(unit)->l3_state->_bcm_tunnel_intf_to_eep = NULL;
    BCMDNX_ALLOC(SOC_DPP_STATE(unit)->l3_state->_bcm_tunnel_intf_to_eep,
              sizeof(int) * _BCM_PETRA_L3_NOF_ITFS,
              "_bcm_tunnel_intf_to_eep");
    if (SOC_DPP_STATE(unit)->l3_state->_bcm_tunnel_intf_to_eep == NULL) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_MEMORY, (_BSL_BCM_MSG("Memory allocation failure")));
    }
    sal_memset(SOC_DPP_STATE(unit)->l3_state->_bcm_tunnel_intf_to_eep, 0x0, sizeof(int) * _BCM_PETRA_L3_NOF_ITFS);
    ...
    */
    ...
    /* Initialise WB SW DB */
    BCMDNX_IF_ERR_EXIT(soc_dpp_wb_engine_init_buffer(unit, SOC_DPP_WB_ENGINE_BUFFER_DPP_L3));
    ...
    SOC_DPP_STATE(unit)->l3_state->init = TRUE;

#ifdef BCM_WARM_BOOT_SUPPORT
    status = _bcm_dpp_wb_l3_state_init(unit);
    if (status != BCM_E_NONE) {
        LOG_ERROR(BSL_LS_BCM_L3,
                  (BSL_META_U(unit,
    ...
    int unit)
{
    ...
    if ((!SOC_DPP_STATE(unit)) || (!(SOC_DPP_STATE(unit)->l3_state))) {
        BCM_EXIT;
    }        
    
    if (SOC_DPP_STATE(unit)->l3_state->init == TRUE) {

	/* mapping interface to EEP */
	BCM_FREE(SOC_DPP_STATE(unit)->l3_state->_bcm_tunnel_intf_to_eep);
	...
	if (SOC_DPP_STATE(unit)->l3_state->lock != NULL) {
	    sal_mutex_destroy(SOC_DPP_STATE(unit)->l3_state->lock);
	}
	
	if (_dpp_l3_info[unit] != NULL) {
	    BCM_FREE(_dpp_l3_info[unit]);
	}

    BCM_FREE(SOC_DPP_STATE(unit)->l3_state);
    SOC_DPP_STATE(unit)->l3_state = NULL;

#ifdef BCM_WARM_BOOT_SUPPORT
    rv = _bcm_dpp_wb_l3_state_deinit(unit);
    if (rv != BCM_E_NONE) {
        LOG_ERROR(BSL_LS_BCM_L3,
                  (BSL_META_U(unit,
                              "%s: Unit %d, failed warmboot cleanup, error 0x%x\n"), 
    ...
    bcm_l3_intf_t *intf)
{
    ...
    if (intf->l3a_vid && !BCM_VLAN_VALID(intf->l3a_vid)) {
        LOG_ERROR(BSL_LS_BCM_L3,
                  (BSL_META_U(unit,
                              "invalid vid(0x%x)\n"), intf->l3a_vid));
        BCM_RETURN_VAL_EXIT(BCM_E_PARAM);
  ...
  if (!BCM_DPP_NATIVE_ROUTING_VLAN_TAGS_VALID(intf->native_routing_vlan_tags)) {
    ...
    _dpp_l3_info[unit]->used_intf++;
#ifdef BCM_WARM_BOOT_SUPPORT
    BCMDNX_IF_ERR_EXIT(_bcm_dpp_wb_l3_update_info_state(unit));
#endif
    ...
    int ll_eep)
{
    ...
    } else if (encap_entry_info[0].entry_type == SOC_PPD_EG_ENCAP_ENTRY_TYPE_MPLS_ENCAP) {
      soc_sand_rv =
    ...
    bcm_if_t *if_id)
{
    ...
    if(egr->mpls_flags != 0) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("DPP devices do not support mpls flags 0x%x\n"), egr->mpls_flags));
    }
    ...
    if (encap_with_id) {
        _bcm_l3_encap_id_to_eep(unit, _BCM_PETRA_L3_ITF_VAL_GET(egr->encap_id), &ll_eep_index);
    }
    ...
    if (update) {
        ...
        _bcm_l3_encap_id_to_eep(unit, _BCM_PETRA_L3_ITF_VAL_GET(egr_ori.encap_id), &ll_eep_index_ori);        
    ...
    bcm_if_t *if_id)
{
    ...
    int ll_eep_index = 0;
    int tunnel_eep_index = 0;
    int fec_eep_pointer = 0;
    bcm_l3_intf_t l3_intf;
    int rif_entry_id=0;
    int out_rif = 0;    
    int intf_tunnel_index = 0;
    SOC_PPD_FEC_ID fec_id = 0,protect_fec_id = 0;
    uint8 is_tunnel_outlif = FALSE;
    uint8 is_tunnel_eei = FALSE;
    ...
    if (egr->intf > 0) {
        if (_BCM_PETRA_L3_ITF_IS_RIF(egr->intf)) {
            ...
            status = bcm_petra_l3_intf_get(unit, &l3_intf) ;
            /* Don't check return value because l3 interface can be allocated on remote device */

            intf_tunnel_index = l3_intf.l3a_tunnel_idx;
            is_tunnel_outlif = FALSE;
            is_tunnel_eei = FALSE;
            
        } else if (_BCM_PETRA_L3_ITF_IS_ENCAP(egr->intf)) {
            intf_tunnel_index = _BCM_PETRA_L3_ITF_VAL_GET(egr->intf);
			/* This flag is used to signal that the FEC output is EEI (Format C). 
			 * Otherwise it is OutLif (Format B).
			 */
            if (egr->flags & BCM_BCM_L3_ENCAP_SPACE_OPTIMIZED) {
                is_tunnel_eei = TRUE;
            } else {
                is_tunnel_outlif = TRUE;
            }
        } else {
            DPP_L3_LOCK_RELEASE;
        ...
        if (flags & BCM_L3_WITH_ID) {
            ...
            fec_flags |= SHR_RES_ALLOC_WITH_ID;
             if ((flags & BCM_L3_REPLACE)) {
                ...
                if (!_BCM_PPD_L3_IS_PROTECTED_IF(egr)) {
                    status = _bcm_l3_alloc_fec(unit, fec_flags, 0, 1, &fec_index);
                    if (status != BCM_E_NONE) {
                        DPP_L3_LOCK_RELEASE;
                        BCMDNX_IF_ERR_EXIT(status);
                    }
                ...
                /* protection, if protecting alloc two*/
                    ...
                    status = _bcm_l3_alloc_fec(unit, fec_flags, 0, 2, &protect_fec_index);
                    if (status != BCM_E_NONE) {
                        DPP_L3_LOCK_RELEASE; 
                        BCMDNX_IF_ERR_EXIT(status);
                    }
            ...
            /* no protection alloc one */
            if (!_BCM_PPD_L3_IS_PROTECTED_IF(egr)) {
                status = _bcm_l3_alloc_fec(unit, 0, 0, 1, &fec_index);
                if (status != BCM_E_NONE) {
                    DPP_L3_LOCK_RELEASE;           
                    BCMDNX_IF_ERR_EXIT(status);     
                }
            ...
            /* protection, if protecting alloc two*/
                ...
                status = _bcm_l3_alloc_fec(unit, 0, 0, 2, &fec_index);
                if (status != BCM_E_NONE) {
                    DPP_L3_LOCK_RELEASE; 
                    BCMDNX_IF_ERR_EXIT(status);
                }
    ...
    if (is_egress) {    
        ...
        if ((encap_with_id) && (flags & BCM_L3_REPLACE)) { /* second call */
            tunnel_eep_index = ll_eep_index;
            status = _bcm_l3_alloc_eep(unit, egr, &tunnel_eep_index, &ll_eep_index); 
            ...
            fec_eep_pointer = tunnel_eep_index;         
        } else {/* egress */
            ret = _bcm_l3_alloc_eep(unit, egr, &tunnel_eep_index, &ll_eep_index);
            if (ret != BCM_E_NONE) {
                DPP_L3_LOCK_RELEASE; 
                BCMDNX_IF_ERR_EXIT(ret);
            }
            fec_eep_pointer = tunnel_eep_index;
            _BCM_PETRA_L3_ITF_SET(egr->encap_id,_BCM_PETRA_L3_ITF_ENCAP,ll_eep_index); 
        }    
        ...
        */
        ...
        if ((!encap_with_id) || ((flags & (BCM_L3_KEEP_DSTMAC | BCM_L3_KEEP_VLAN | BCM_L3_REPLACE)) != (BCM_L3_KEEP_DSTMAC | BCM_L3_KEEP_VLAN | BCM_L3_REPLACE))) {
            status = _bcm_ppd_eg_encap_entry_add(unit, fec_index, tunnel_eep_index, ll_eep_index, &fec_eep_pointer, egr);
            if (status != BCM_E_NONE) {
                DPP_L3_LOCK_RELEASE; 
                BCMDNX_IF_ERR_EXIT(status);
            }
        ...
        if (intf_tunnel_index != 0) {
            status = _bcm_tunnel_intf_ll_eep_set(unit, egr->intf, intf_tunnel_index, ll_eep_index);
            if (status != BCM_E_NONE) {
                DPP_L3_LOCK_RELEASE;
                BCMDNX_IF_ERR_EXIT(status);
            }
            fec_eep_pointer = intf_tunnel_index;
            is_tunnel_outlif = TRUE;
            is_tunnel_eei = FALSE;
        }
    } else {
    ...
    bcm_if_t intf)
{
    ...
    if (_BCM_PETRA_L3_ITF_IS_ENCAP(intf)) { /* LL */
      ...
      _bcm_l3_encap_id_to_eep(unit, intf, &eep_index);
      
      rv = _bcm_ppd_eg_encap_entry_remove(unit, eep_index);
      BCMDNX_IF_ERR_EXIT(rv);

      /* free ENCAP entry usage */
      rv = _bcm_l3_dealloc_eep(unit, eep_index);
      BCMDNX_IF_ERR_EXIT(rv);
    ...
    bcm_l3_egress_t *egr)
{
    ...
    if (_BCM_PETRA_L3_ITF_IS_ENCAP(intf)) { /* LL */
      ...
      status = _bcm_ppd_eg_encap_entry_get(unit, eep_index,egr);      
  ...
  )
{
  ...
  if ((ecmp_obj_copy.max_paths < 1) || (ecmp_obj_copy.max_paths > SOC_DPP_CONFIG(unit)->l3.ecmp_max_paths)) {
    BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG_STR("Error - max_paths must be a positive integer.")));
  }
    ...
    bcm_if_t *intf_id)
{
    ...
    if ((if_id >= SOC_DPP_DEFS_GET(unit, nof_rifs)) && (if_id != SOC_PPD_RIF_NULL)) {
        DPP_L3_LOCK_RELEASE;
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Invalid interface")));
    }
    ...
    if (if_id != SOC_PPD_RIF_NULL) {
        ...
        switch (ing_intf->urpf_mode) {
            case bcmL3IngressUrpfLoose:
                if (SOC_DPP_STATE(unit)->l3_state->urpf_mode == bcmL3IngressUrpfLoose) {
                    rif_info.uc_rpf_enable = SOC_SAND_TRUE;
                } else {
                    LOG_WARN(BSL_LS_BCM_L3,
                             (BSL_META_U(unit,
                ...
                break;
            case bcmL3IngressUrpfStrict:
                if (SOC_DPP_STATE(unit)->l3_state->urpf_mode == bcmL3IngressUrpfStrict) {
                    rif_info.uc_rpf_enable = SOC_SAND_TRUE;
                } else {
                    LOG_WARN(BSL_LS_BCM_L3,
                             (BSL_META_U(unit,
        ...
        if ((ing_intf->flags & BCM_L3_INGRESS_ROUTE_DISABLE_IP4_MCAST) || (ing_intf->flags & BCM_L3_INGRESS_ROUTE_DISABLE_IP6_MCAST)) {
            rif_info.routing_enablers_bm &= ~SOC_PPD_RIF_ROUTE_ENABLE_TYPE_IP_MC;
        }
        else {
            rif_info.routing_enablers_bm |= SOC_PPD_RIF_ROUTE_ENABLE_TYPE_IP_MC;
    ...
    bcm_l3_ingress_t *ing_intf)
{
    ...
    } else if (SOC_DPP_STATE(unit)->l3_state->urpf_mode == SOC_PPD_FRWRD_FEC_RPF_MODE_UC_LOOSE) {
...
#endif /* BCM_88660_A0 */
    ing_intf->flags = 0;
    if (!(rif_info.routing_enablers_bm & SOC_PPD_RIF_ROUTE_ENABLE_TYPE_IP_UC)) {
        ing_intf->flags |= (BCM_L3_INGRESS_ROUTE_DISABLE_IP4_UCAST | BCM_L3_INGRESS_ROUTE_DISABLE_IP6_UCAST);
    }
    if (!(rif_info.routing_enablers_bm & SOC_PPD_RIF_ROUTE_ENABLE_TYPE_IP_MC)) {
        ing_intf->flags |= (BCM_L3_INGRESS_ROUTE_DISABLE_IP4_MCAST | BCM_L3_INGRESS_ROUTE_DISABLE_IP6_MCAST);
    }
    ...
    int enable)
{
    ...
    soc_sand_rv = soc_ppd_lif_table_entry_update(soc_sand_dev_id, lif_id, lif_info);
    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    ...
    bcm_l3_source_bind_t *info)
{
    ...
    rv = _bcm_dpp_gport_to_lif(unit, info->port, &lif_id, NULL, &fec_id, &is_local);
    BCMDNX_IF_ERR_EXIT(rv);
  ...
  )
{
    ...
    while(!SOC_SAND_TBL_ITER_IS_END(&(block_range.iter)))
    {
        ...
        for(idx = 0; idx < nof_entries; idx++)
        {
            ...
            BCMDNX_IF_ERR_EXIT(_bcm_dpp_lif_to_gport(unit, src_bind_ipv4s[idx].lif_ndx, 1, &entry.port));
            /* Invoke user callback. */
    ...
    for (vrf_ndx=BCM_L3_VRF_DEFAULT; vrf_ndx<vrf_max; vrf_ndx++)
    {
      ...
      while(!SOC_PPC_IP_ROUTING_TABLE_ITER_IS_END(&block_range.start.payload))
      {
        ...
        for(idx = 0; idx < nof_entries; idx++)
        {
          ...
          if (fec_ids[idx])
          {
              BCMDNX_IF_ERR_EXIT(_bcm_dpp_lif_to_gport(unit, fec_ids[idx], 1, &entry.port));
              if (vrf_ndx == BCM_L3_VRF_DEFAULT)
              {
    ...
    while(!SOC_PPC_IP_ROUTING_TABLE_ITER_IS_END(&block_range_key.start.payload))
    {
      ...
      for(idx = 0; idx < nof_entries; idx++)
      {
          ...
          rv = _bcm_l3_sand_ipv6_addr_to_bcm_ipv6_addr(unit,
                               ...
                               &(entry.ip6));
          BCMDNX_IF_ERR_EXIT(rv);
          BCMDNX_IF_ERR_EXIT(_bcm_dpp_lif_to_gport(unit, src_bind_ipv6s[idx].lif_ndx, 1, &entry.port));
    ...
    uint8* mode)
{
    ...
    /* ARADPLUS and jericho can tolerate vlan 0, and jericho can tolerate vlan < 32k */
    if ((!(BCM_VLAN_VALID(vlan)) && SOC_IS_ARAD_B1_AND_BELOW(unit))
        || /* soc is aradplus or jericho and */ vlan >= _BCM_PETRA_L3_VRRP_MAX_VSI(_unit)) {
 ...
 * Purpose:
 *      Assings VRRP mac address according to protocol (IPV4/ IPV6) and vrid.
 * Parameters: 
...
_bcm_l3_vrrp_setting_add(int unit, SOC_PPC_VRRP_CAM_INFO *cam_info, int vlan){
    ...
    if (SOC_IS_JERICHO(unit)) {
        bcm_dpp_switch_l3_protocol_group_change_count(unit, cam_info->protocol_group, cam_info->protocol_group_mask, alloc_count);
    }
