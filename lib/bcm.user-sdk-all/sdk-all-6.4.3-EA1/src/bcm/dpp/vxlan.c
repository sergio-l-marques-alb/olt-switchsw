...
#define DPP_VXLAN_UNIT_CHECK(unit) \
do { \
    if (!((unit) >= 0 && ((unit) < (BCM_MAX_NUM_UNITS)))) { \
       BCMDNX_ERR_EXIT_MSG(BCM_E_UNIT, (_BSL_BCM_MSG("%s: Invalid unit \n"), FUNCTION_NAME())); \
    } \
} while (0)
      ...
      )          
{
    ...
    *out_lif = -1;
    /* in_lif */
    *in_lif = _bcm_petra_vxlan_port_id_to_lif(unit,vxlan_port_id);
    ...
    if(phy_port->type == _BCM_DPP_GPORT_IN_TYPE_VXLAN_EG_FEC) {
    ...
    else if (phy_port->type == _BCM_DPP_GPORT_IN_TYPE_VXLAN){/* else store port + outlif*/
    ...
    /* get outlif+port from FEC if valid */
    /* this code currently is not reached as outlif is not needed */
    if(*eg_fec != -1 && out_lif_needed) {
        soc_sand_dev_id = (unit);
        soc_sand_rv = soc_ppd_frwrd_fec_entry_get(soc_sand_dev_id,*eg_fec,&protect_type,&fec_entry[0],&fec_entry[1],&protect_info);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        *out_lif = fec_entry[0].eep; /* EEP */
        BCM_EXIT;
    }
...
_bcm_petra_vxlan_resources_get(
    ...
    *fec_index = -1;
    ...
    if(ingress) {
        ...
        rv = _bcm_dpp_gport_to_lif(unit,vxlan_port->match_tunnel_id,in_lif,NULL,NULL,&is_local);
        BCMDNX_IF_ERR_EXIT(rv);
        ...
        if (vxlan_port->egress_tunnel_id != 0) {
            /* get match tunnel (terminator) */
            rv = _bcm_dpp_gport_to_lif(unit,vxlan_port->egress_tunnel_id,NULL,out_lif,NULL,&is_local);
            BCMDNX_IF_ERR_EXIT(rv);
        }
    }
...
int
_bcm_petra_vxlan_check_lif(int unit, int lif, int ingress){
    int rv = BCM_E_NONE;
...
bcm_petra_vxlan_port_sw_state_add(
    ...
    int update)
 {
    ...
    if(eg_fec != -1) {
        phy_port->type = _BCM_DPP_GPORT_IN_TYPE_VXLAN_EG_FEC;
        ...
        phy_port->type = _BCM_DPP_GPORT_IN_TYPE_VXLAN;
    ...
    /* update lif usage */
    if (in_lif != -1) {
        rv = _bcm_dpp_in_lif_usage_set(unit, in_lif, _bcmDppLifTypeVxlan); 
        BCMDNX_IF_ERR_EXIT(rv);
    ...
    if (out_lif != -1) {
        rv = _bcm_dpp_out_lif_usage_set(unit, out_lif, _bcmDppLifTypeVxlan);
        BCMDNX_IF_ERR_EXIT(rv);
    }
    ...
    bcm_vxlan_port_t *vxlan_port)
{
    ...
    rv = _bcm_petra_vxlan_resources_get(unit,vxlan_port,ingress,egress,&in_lif,&out_lif,&fec_index);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    /* 1. LIF, learn-info + VSI-assignment mode.
       2. map VXLAN-vpn to VSI and vice-versa. remove, not this done in vpn-create
       3. store SW state
     */
    ...
    if(in_lif != -1 && is_local_config) {
        ...
        rv = _bcm_petra_vxlan_check_lif(unit, in_lif, 1);
        BCMDNX_IF_ERR_EXIT(rv);
       ...
       /* update LIF info */
        ...
        rv = _bcm_petra_vxlan_learn_info_map_set(unit,out_lif,vxlan_port->match_port,fec_index, &(lif_info.value.ip_term_info.learn_record));
        BCMDNX_IF_ERR_EXIT(rv);
    ...
    if(out_lif != -1 && is_local_config) {
        ...
        rv = _bcm_petra_vxlan_check_lif(unit, out_lif,0);
        BCMDNX_IF_ERR_EXIT(rv);
    ...
    if(!(vxlan_port->flags & BCM_VXLAN_PORT_WITH_ID)) {
        _bcm_petra_vxlan_port_id_from_lif(unit,in_lif,&(vxlan_port->vxlan_port_id));
    }

    /* store SW state */
    bcm_petra_vxlan_port_sw_state_add(unit,l2vpn,vxlan_port,is_local_config,fec_index,in_lif,out_lif,update);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    bcm_gport_t vxlan_port_id)
{
    ...
    rv = _bcm_petra_vxlan_port_id_to_resources(unit,vxlan_port_id,&in_lif,&out_lif,&eg_fec,&phy_port,&is_local);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    rv = bcm_petra_vxlan_port_sw_state_remove(unit,l2vpn,vxlan_port_id,is_local,eg_fec,in_lif,out_lif);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    bcm_vxlan_port_t *vxlan_port)
{
    ...
    in_lif = _bcm_petra_vxlan_port_id_to_lif(unit, vxlan_port->vxlan_port_id);
    if (in_lif == -1) {
        return BCM_E_PARAM;
    }
    ...
    /* get <port,in-lif, out-lif, FEC associated with this GPORT */
    rv = _bcm_petra_vxlan_port_id_to_resources(unit,vxlan_port->vxlan_port_id,&in_lif,&out_lif,&eg_fec,&phy_port,&is_local);
    BCMDNX_IF_ERR_EXIT(rv);

    /* Verify outLIF is really VXLAN. */
    BCMDNX_IF_ERR_EXIT(_bcm_dpp_lif_usage_get(unit, -1, out_lif, NULL, &usage));
    if (usage != _bcmDppLifTypeVxlan) {
    ...
    if(eg_fec != -1) {
        ...
        BCM_GPORT_TUNNEL_ID_SET(vxlan_port->egress_tunnel_id, out_lif);
    }

    /* match tunnel */
    BCM_GPORT_TUNNEL_ID_SET(vxlan_port->match_tunnel_id, in_lif);
