    ...
    bcm_vpn_t vpn)
{
    ...
    /* check type */
    if (!_BCM_DDP_IS_MP_VPN(vpn) && !_BCM_DDP_IS_CROSS_CONNECT_P2P_VPN(vpn)) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("VPN is not set with supported types : MP or Cross Connect")));
    }
    ...
    bcm_gport_t mpls_port_id)
{
    ...
    if (is_member || !is_local) {
        _bcm_dpp_gport_delete(unit, vpn, mpls_port_id);
    }
    ...
    bcm_mpls_port_t * mpls_port)
{
    ...
    if (is_ingress) {
        ...
        if (lif_usage != _bcmDppLifTypeMplsPort) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PORT, (_BSL_BCM_MSG("InLIF is not PWE")));
        }
    ...
    bcm_mpls_tunnel_switch_t * info)
{
    ...
    if (!glbl_info.key_info.mask_port) {
        ...
        rv = MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_pp_to_local_port_get, (unit, ilm_key->in_local_port, 0, &local_port));
        BCMDNX_IF_ERR_EXIT(rv);
    ...
    bcm_mpls_tunnel_switch_t * info)
{
    ...
    if (info->action != BCM_MPLS_SWITCH_ACTION_POP) {
        ...
        if ((info->action != BCM_MPLS_SWITCH_EGRESS_ACTION_PUSH) && (!(info->flags & BCM_MPLS_SWITCH_TTL_DECREMENT))) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("BCM_MPLS_SWITCH_TTL_DECREMENT must be set in case of ILM SWAP and PHP")));     /* TTL always decemented (even if inherented/copied */
        }
