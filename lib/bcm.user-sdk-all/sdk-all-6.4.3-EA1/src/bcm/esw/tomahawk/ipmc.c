...
#if 0
        SOC_IF_ERROR_RETURN(soc_mem_write(unit, MMU_REPL_HEAD_TBLm,
                    MEM_BLOCK_ALL, REPL_HEAD_TBL_INDEX(unit, pipe, 0),
                    soc_mem_entry_null(unit, MMU_REPL_HEAD_TBLm)));
#endif
    }
...
_bcm_th_repl_list_entry_info_init(int unit)
{
    ...
    for (pipe = 0; pipe < NUM_PIPES(unit); pipe++) {
        ...
        SOC_IF_ERROR_RETURN(soc_mem_write(unit, MMU_REPL_LIST_TBLm, MEM_BLOCK_ALL,
                    REPL_LIST_TBL_INDEX(unit, pipe, 0),
                    soc_mem_entry_null(unit, MMU_REPL_LIST_TBLm)));
#endif
    }
                ...
                int intf_count)
{
    ...
    SOC_IF_ERROR_RETURN(READ_MMU_REPL_GROUP_INITIAL_COPY_COUNTm(unit,
                MEM_BLOCK_ANY, REPL_GROUP_ICC_TBL_INDEX(unit, slice, repl_group),
    ...
    if (slice == 0) {
        ...
        if (phy_port == 133) {
            soc_MMU_REPL_GROUP_INITIAL_COPY_COUNTm_field32_set(unit,
                    &initial_copy_count_entry, ICC_BIT_139_137f, copy_count);
        } else if (phy_port == 132) {
            soc_MMU_REPL_GROUP_INITIAL_COPY_COUNTm_field32_set(unit,
                    &initial_copy_count_entry, ICC_BIT_136_134f, copy_count);
        } else if (phy_port == 129) {
            soc_MMU_REPL_GROUP_INITIAL_COPY_COUNTm_field32_set(unit,
                    &initial_copy_count_entry, ICC_BIT_133_131f, copy_count);
        } else if (IS_CPU_PORT(unit, port)) {
            soc_MMU_REPL_GROUP_INITIAL_COPY_COUNTm_field32_set(unit,
                    &initial_copy_count_entry, ICC_BIT_130_128f, copy_count);
        } else
#endif
        if (phy_port <= 64 && phy_port >= 33) {
    ...
    SOC_IF_ERROR_RETURN(WRITE_MMU_REPL_GROUP_INITIAL_COPY_COUNTm(unit,
                MEM_BLOCK_ALL, REPL_GROUP_ICC_TBL_INDEX(unit, slice, repl_group),
                &initial_copy_count_entry));
#endif
                ...
                int start_ptr, int intf_count)
{
    ...
    SOC_IF_ERROR_RETURN(soc_mem_read(unit, repl_group_table, MEM_BLOCK_ANY, 
                REPL_GROUP_TBL_INDEX(unit, pipe, repl_group),
                &repl_group_entry));
#endif
    ...
    if (add_member) {
        ...
        if (SOC_PBMP_MEMBER(old_member_bitmap, member_bitmap_index)) {
            ...
            SOC_IF_ERROR_RETURN(WRITE_MMU_REPL_HEAD_TBLm(unit, MEM_BLOCK_ALL,
                        REPL_HEAD_TBL_INDEX(unit, pipe, old_head_index + member_id),
                        &repl_head_entry));
#endif
            ...
            SOC_PBMP_ITER(new_member_bitmap, member) {
                if (member == member_bitmap_index) {
                    ...
                    SOC_IF_ERROR_RETURN(WRITE_MMU_REPL_HEAD_TBLm(unit,
                                MEM_BLOCK_ALL, REPL_HEAD_TBL_INDEX(unit, pipe,
                                    new_head_index + new_member_id),
                                &repl_head_entry));
#endif
                } else {
                    ...
                    SOC_IF_ERROR_RETURN(READ_MMU_REPL_HEAD_TBLm(unit,
                                MEM_BLOCK_ANY, REPL_HEAD_TBL_INDEX(unit, pipe,
                                    old_head_index + old_member_id),
                                &old_repl_head_entry));
#endif
                    SOC_IF_ERROR_RETURN
        ...
        SOC_IF_ERROR_RETURN
            (soc_mem_write(unit, repl_group_table, MEM_BLOCK_ALL,
                           repl_group, &repl_group_entry));
#if 0
        SOC_IF_ERROR_RETURN(soc_mem_write(unit, repl_group_table, MEM_BLOCK_ALL,
                    REPL_GROUP_TBL_INDEX(unit, pipe, repl_group),
                    &repl_group_entry));
#endif
...
#if 0
    SOC_IF_ERROR_RETURN(READ_MMU_REPL_HEAD_TBLm(unit, MEM_BLOCK_ANY,
                REPL_HEAD_TBL_INDEX(unit, pipe, head_index),
                &repl_head_entry));
#endif
...
_bcm_th_repl_list_free(int unit, int pipe, int start_ptr)
{
    ...
    while (repl_entry_ptr != prev_repl_entry_ptr) {
        ...
        SOC_IF_ERROR_RETURN(READ_MMU_REPL_LIST_TBLm(unit, MEM_BLOCK_ANY,
                    REPL_LIST_TBL_INDEX(unit, pipe, repl_entry_ptr),
                    &repl_list_entry));
#endif
        BCM_IF_ERROR_RETURN
            ...
            if (prev_repl_entry_ptr == -1) {
                ...
                *start_ptr = repl_entry_ptr;
                ...
                SOC_IF_ERROR_RETURN(WRITE_MMU_REPL_LIST_TBLm(unit,
                            MEM_BLOCK_ALL,
                            REPL_LIST_TBL_INDEX(unit, pipe, prev_repl_entry_ptr),
                            &repl_list_entry));
#endif
                if (no_more_free_repl_entries) {
    ...
    if (prev_repl_entry_ptr > 0) {
        ...
        SOC_IF_ERROR_RETURN
            (soc_mem_write(unit, repl_list_table, MEM_BLOCK_ALL, 
                           prev_repl_entry_ptr, &repl_list_entry));
#if 0
        SOC_IF_ERROR_RETURN(WRITE_MMU_REPL_LIST_TBLm(unit, MEM_BLOCK_ALL,
                    REPL_LIST_TBL_INDEX(unit, pipe, prev_repl_entry_ptr),
                    &repl_list_entry));
#endif
    }
...
#if 0
        index_min = REPL_GROUP_TBL_INDEX(unit, pipe, 0);
        index_max = REPL_GROUP_TBL_INDEX(unit, pipe,
                _th_repl_info[unit]->num_repl_groups - 1);
#endif
        index_min = 0;
        ...
        for (i = index_min; i <= index_max; i++) {
            ...
            SOC_PBMP_ITER(member_bitmap, member) {
                ...
                rv = READ_MMU_REPL_HEAD_TBLm(unit, MEM_BLOCK_ANY,
                        REPL_HEAD_TBL_INDEX(unit, pipe, head_index + member_id),
                        &repl_head_entry);
#endif
                if (SOC_FAILURE(rv)) {
                    goto cleanup;
 ...
 */
int
_bcm_th_ipmc_src_port_compare(int unit, bcm_ipmc_addr_t *ipmc, int *match)
{
    /* Tomahawk dummy function for port compare */ 
    *match = TRUE;
    return BCM_E_NONE;

#if 0    
    _bcm_l3_cfg_t l3cfg;
    l3_entry_ipv4_multicast_entry_t v4entry;
    l3_entry_ipv6_multicast_entry_t v6entry;
    int ipmc_index;
    ...
    *match = FALSE;
    ...
    if (ipmc->flags & BCM_IPMC_IP6) {
        ...
        if (BCM_FAILURE(mbcm_driver[unit]->mbcm_l3_ip6_get(unit, &l3cfg))) {
            return BCM_E_NONE;
        }
        ...
        /* Read L3_ENTRY_IPV6 */
        BCM_IF_ERROR_RETURN
             (soc_mem_read(unit, L3_ENTRY_IPV6_MULTICASTm, MEM_BLOCK_ANY, ipmc_index,
                          &v6entry));
        
        t_f    = soc_L3_ENTRY_IPV6_MULTICASTm_field32_get(unit, &v6entry, Tf);
        tgid_f = soc_L3_ENTRY_IPV6_MULTICASTm_field32_get(unit, &v6entry, TGIDf);
        mod_f  = soc_L3_ENTRY_IPV6_MULTICASTm_field32_get(unit, &v6entry, MODULE_IDf);
        port_f = soc_L3_ENTRY_IPV6_MULTICASTm_field32_get(unit, &v6entry, PORT_NUMf);
    } else {
        l3cfg.l3c_src_ip_addr = ipmc->s_ip_addr;
        l3cfg.l3c_ipmc_group = ipmc->mc_ip_addr;
       
        if (BCM_FAILURE(mbcm_driver[unit]->mbcm_l3_ip4_get(unit, &l3cfg))) {
            return  BCM_E_NONE;
        }
        ...
        /* Read L3_ENTRY_IPV4 */
        BCM_IF_ERROR_RETURN
            (soc_mem_read(unit, L3_ENTRY_IPV4_MULTICASTm, MEM_BLOCK_ANY, ipmc_index,
                          &v4entry));

        t_f    = soc_L3_ENTRY_IPV4_MULTICASTm_field32_get(unit, &v4entry, Tf);
        tgid_f = soc_L3_ENTRY_IPV4_MULTICASTm_field32_get(unit, &v4entry, TGIDf);
        mod_f  = soc_L3_ENTRY_IPV4_MULTICASTm_field32_get(unit, &v4entry, MODULE_IDf);
        port_f = soc_L3_ENTRY_IPV4_MULTICASTm_field32_get(unit, &v4entry, PORT_NUMf);
    }

    if ((t_f == 1) && (mod_f == SOC_MODID_MAX(unit)) &&
            (port_f == SOC_IS_TD_TT(unit))) {
        no_src_check = TRUE;
    } else if (t_f == 1) {
        is_trunk = 1;
        tgid = tgid_f;
