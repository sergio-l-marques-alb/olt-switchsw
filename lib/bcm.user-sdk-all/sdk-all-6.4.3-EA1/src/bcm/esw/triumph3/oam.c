...
_bcm_oam_misc_config(int unit)
{
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "OAM(unit %d) Error: Misc config - Failed.\n"),
                   unit));
        return (rv);
    }
                               ...
                               uint32 *entry, void *ma_rmep)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_OAM,
              (BSL_META_U(unit,
                          "OAM(unit %d) Info: _bcm_tr3_oam_read_clear_faults index=%d "
                           "Table=%d.\n"), unit, index, mem));
                           ...
                           bcm_oam_endpoint_t ep_id)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_OAM,
              (BSL_META_U(unit,
                          "OAM(unit %d) Info: _bcm_oam_group_ep_list_add (GID=%d) (EP=%d).\n"),
               unit, group_id, ep_id));
    return (BCM_E_NONE);
}
                           ...
                           const bcm_oam_endpoint_info_t *ep_info_p)
{
    ...
    if (!(h_data_p->in_use)) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "OAM(unit %d) Error: %s EP valid check failed.\n"),
                   unit, FUNCTION_NAME()));
        return (BCM_E_INTERNAL);
    }
                       ...
                       uint32 *l3_entry_p)
{
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "OAM(unit %d) Error: L3 entry lookup vlan=%d port=%x %s.\n"),
                             ...
                             const bcm_oam_endpoint_info_t *ep_info_p)
{
    ...
    if (BCM_SUCCESS
        ...
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                                  "OAM(unit %d) Error: L3_ENTRY table update failed for EP=%d"
                                   "  %s.\n"), unit, ep_p->id, bcm_errmsg(rv)));
            L3_UNLOCK(unit);
            return (rv);
        }
        ...
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                                  "OAM(unit %d) Error: L3_ENTRY table insert failed for EP=%d"
                                   "  %s.\n"), unit, ep_p->id, bcm_errmsg(rv)));
            L3_UNLOCK(unit);
            return (rv);
        }
    ...
    if (1 == h_data_p->local_rx_enabled) {
        ...
        if ( h_data_p->flags & 
            ...
            if (BCM_FAILURE(rv)) {
                LOG_ERROR(BSL_LS_BCM_OAM,
                          (BSL_META_U(unit,
                                      "OAM(unit %d) Error: LM DM delete failed (EP=%d)"
                                       "- %s.\n"), unit, h_data_p->ep_id, bcm_errmsg(rv)));
                return (rv);
            }
        }
                                  ...
                                  bcm_oam_endpoint_info_t *ep_info_p)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_OAM,
              (BSL_META_U(unit,
                          "OAM(unit %d) Info: _bcm_tr3_oam_endpoint_params_validate.\n"),
               unit));
    ...
    if (ep_info_p->flags & BCM_OAM_ENDPOINT_CCM_COPYFIRSTTOCPU) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "OAM (unit %d) Error: Unsupported flag\n"),
                   unit));
        return (BCM_E_PARAM);
    }
...
_bcm_tr3_oam_fp_entry_id_allocate(int unit, bcm_field_group_t fp_group, 
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "OAM(unit %d) Error: FP Entry Tx creation (tx), %s.\n"),
                   unit, bcm_errmsg(rv)));
        return (rv);
    }
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "OAM(unit %d) Error: FP Entry Tx Prio Set (tx), %s.\n"),
                   unit, bcm_errmsg(rv)));
        return (rv);
    }
...
_bcm_tr3_oam_fp_entry_action_add(int unit, _bcm_oam_hash_data_t *hash_data, 
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "OAM(unit %d) Error: Adding action OamLmepEnable, EP=%d %s.\n"),
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "OAM(unit %d) Error: Adding action OamLmEnable, EP=%d %s.\n"),
                   unit, hash_data->ep_id, bcm_errmsg(rv)));
        return (rv);
    }
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "OAM(unit %d) Error: Adding action OamDmEnable, EP=%d %s.\n"),
                   unit, hash_data->ep_id, bcm_errmsg(rv)));
        return (rv);
    }
    ...
    if (hash_data->flags & BCM_OAM_ENDPOINT_DELAY_MEASUREMENT){
        ...
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                                  "OAM(unit %d) Error: Adding action DmTimeFormat, "
                                  "EP=%d %s.\n"), unit, hash_data->ep_id, bcm_errmsg(rv)));
            return (rv);
        }
    }
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "OAM(unit %d) Error: Adding action LmepMdl (tx), EP=%d %s.\n"),
                   unit, hash_data->ep_id, bcm_errmsg(rv)));
        return (rv);
    }
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "OAM(unit %d) Error: Adding action OamUpMep (tx), EP=%d %s.\n"),
                   unit, hash_data->ep_id, bcm_errmsg(rv)));
        return (rv);
    }
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "OAM(unit %d) Error: Adding action OamTx (tx), EP=%d %s.\n"),
                   unit, hash_data->ep_id, bcm_errmsg(rv)));
        return (rv);
    }
    ...
    if (hash_data->flags & BCM_OAM_ENDPOINT_LOSS_MEASUREMENT)
    {
        ...
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                                  "OAM(unit %d) Error: Adding action OamUpMep (tx), "
                                  "EP=%d %s.\n"), unit, hash_data->ep_id, bcm_errmsg(rv)));
            return (rv);
        }
...
_bcm_tr3_oam_fp_trunk_create(int unit, _bcm_oam_control_t *oc, 
    ...
    if( oc->fp_glp_group == -1 ){
         LOG_ERROR(BSL_LS_BCM_OAM,
                   (BSL_META_U(unit,
                               "OAM(unit %d) Error: FP Croup not present, EP=%d %s.\n"),
                    unit, hash_data->pri_map_index, bcm_errmsg(rv)));
         return (BCM_E_INTERNAL);
    }
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "OAM(unit %d) Error: FP Trunk Entry allocate (tx), EP=%d %s\n"),
                   unit, hash_data->ep_id, bcm_errmsg(rv)));
        return (rv);
    }
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "OAM(unit %d) Error: Qualifying OuterVlanId (tx), EP=%d %s.\n"),
                   unit, hash_data->ep_id, bcm_errmsg(rv)));
        return (rv);
    }
    ...
    modid = _BCM_OAM_GLP_MODULE_ID_GET(unit, hash_data->dglp);
    LOG_VERBOSE(BSL_LS_BCM_OAM,
                (BSL_META_U(unit,
                            "OAM(unit %d): %s - Local Port %d, modid %d.\n"), 
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "OAM(unit %d) Error: Qualifying DstPort (tx), EP=%d %s.\n"),
                   unit, hash_data->ep_id, bcm_errmsg(rv)));
        return (rv);
    }
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "OAM(unit %d) Error: EtherType (tx), EP=%d %s.\n"),
                   unit, hash_data->ep_id, bcm_errmsg(rv)));
        return (rv);
    }
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "OAM(unit %d) Error: Qualifying MDL (tx), EP=%d %s.\n"),
                   unit, hash_data->ep_id, bcm_errmsg(rv)));
        return (rv);
    }
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "OAM(unit %d) Error: Adding action (tx), EP=%d %s.\n"),
                   unit, hash_data->ep_id, bcm_errmsg(rv)));
        return (rv);
    }
...
_bcm_tr3_oam_fp_create(int unit, _bcm_oam_control_t *oc, 
    ...
    if (oc->fp_glp_group == -1) {
        ...
        rv = bcm_esw_field_group_create(unit, oc->fp_glp_qs, 
         ...
         if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                                  "OAM(unit %d) Error: Group creation, EP=%d %s.\n"),
                       unit, hash_data->ep_id, bcm_errmsg(rv)));
            return (rv);
        }
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "OAM(unit %d) Error: FP Entry allocate (tx), EP=%d %s.\n"),
                   unit, hash_data->ep_id, bcm_errmsg(rv)));
        return (rv);
    }
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "OAM(unit %d) Error: FP Entry allocate (rx), EP=%d %s.\n"),
                   unit, hash_data->ep_id, bcm_errmsg(rv)));
        return (rv);
    }
    ...
    if (hash_data->vp != 0) {
        ...
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                                  "OAM(unit %d) Error: Qualifying DstMimGport (tx), "
                                  "EP=%d %s.\n"), unit, hash_data->ep_id, bcm_errmsg(rv)));
            return (rv);
        }
    	...
    	if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                                  "OAM(unit %d) Error: Qualifying SrcMimGport (rx), "
                                  "EP=%d %s.\n"), unit, hash_data->ep_id, bcm_errmsg(rv)));
            return (rv);
        }
        ...
        /* If its not VP case then we need to qualify on outer vlan for rest */
         ...
         if (BCM_FAILURE(rv)) {
             LOG_ERROR(BSL_LS_BCM_OAM,
                       (BSL_META_U(unit,
                                   "OAM(unit %d) Error: Qualifying OuterVlanId (tx), "
                                   "EP=%d %s.\n"), unit, hash_data->ep_id, bcm_errmsg(rv)));
             return (rv);
         }
        ...
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                                  "OAM(unit %d) Error: Qualifying OuterVlanId (rx), "
                                  "EP=%d %s.\n"), unit, hash_data->ep_id, bcm_errmsg(rv)));
            return (rv);
        }
        ...
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                                  "OAM(unit %d) Error: call to _bcm_esw_gport_resolve "
                                  "failed, EP=%d %s.\n"), unit, hash_data->ep_id, bcm_errmsg(rv)));
            return (rv);
        }
        LOG_VERBOSE(BSL_LS_BCM_OAM,
                    (BSL_META_U(unit,
                                "OAM(unit %d): %s - Local Port %d, modid %d, trunkid %d.\n"), 
        ...
        if (SOC_GPORT_IS_TRUNK(hash_data->gport)) {
            LOG_VERBOSE(BSL_LS_BCM_OAM,
                        (BSL_META_U(unit,
                                    "OAM(unit %d): MEP on Trunk\n"),
                         unit));
            
            if (BCM_TRUNK_INVALID == tgid)  {
                LOG_ERROR(BSL_LS_BCM_OAM,
                          (BSL_META_U(unit,
                                      "OAM(unit %d) Error: Invalid Trunkid, EP=%d %s.\n"), 
                           unit, hash_data->ep_id, bcm_errmsg(rv)));
                return (rv);
            }
            ...
            if (BCM_FAILURE(rv)) {
                LOG_ERROR(BSL_LS_BCM_OAM,
                          (BSL_META_U(unit,
                                      "OAM(unit %d) Error: Qualifying DstTrunk (tx), "
                                      "EP=%d %s.\n"), unit, hash_data->ep_id, bcm_errmsg(rv)));
                return (rv);
            }
            ...
            if (BCM_FAILURE(rv)) {
                LOG_ERROR(BSL_LS_BCM_OAM,
                          (BSL_META_U(unit,
                                      "OAM(unit %d) Error: Qualifying SrcTrunk (rx), "
                                      "EP=%d %s.\n"), unit, hash_data->ep_id, bcm_errmsg(rv)));
                return (rv);
            }
            ...
            if (!(hash_data->flags & BCM_OAM_ENDPOINT_UP_FACING)){
                ...
                if (BCM_FAILURE(rv)) {
                    LOG_ERROR(BSL_LS_BCM_OAM,
                              (BSL_META_U(unit,
                                          "OAM(unit %d) Error: Creating Fp Entry on Trunk Tx "
                                          "port, EP=%d %s\n"), unit, hash_data->ep_id, bcm_errmsg(rv)));
                    return (rv);
                }
            }
            ...
            if (BCM_FAILURE(rv)) {
                LOG_ERROR(BSL_LS_BCM_OAM,
                          (BSL_META_U(unit,
                                      "OAM(unit %d) Error: Qualifying DstPort (tx), "
                                      "EP=%d %s.\n"), unit, hash_data->ep_id, bcm_errmsg(rv)));
                return (rv);
            }
            ...
            if (BCM_FAILURE(rv)) {
                LOG_ERROR(BSL_LS_BCM_OAM,
                          (BSL_META_U(unit,
                                      "OAM(unit %d) Error: Qualifying SrcPort (rx), "
                                      "EP=%d %s.\n"), unit, hash_data->ep_id, bcm_errmsg(rv)));
                return (rv);
            }
        }   
    ...
    LOG_VERBOSE(BSL_LS_BCM_OAM,
                (BSL_META_U(unit,
                            "OAM(unit %d): %s _bcm_oam_lm_dm_search_data.match_count %d, "
    ...
    if (add_mdl) {
        ...
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                                  "OAM(unit %d) Error: EtherType (rx), EP=%d %s.\n"),
                       unit, hash_data->ep_id, bcm_errmsg(rv)));
            return (rv);
        }
        ...
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                                  "OAM(unit %d) Error: Qualifying MDL (rx), EP=%d %s.\n"),
                       unit, hash_data->ep_id, bcm_errmsg(rv)));
            return (rv);
        }
    }
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "OAM(unit %d) Error: Adding action (rx), EP=%d %s.\n"),
                   unit, hash_data->ep_id, bcm_errmsg(rv)));
        return (rv);
    }
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "OAM(unit %d) Error: FP Install failed (tx), EP=%d %s.\n"),
                   unit, hash_data->ep_id, bcm_errmsg(rv)));
        return (rv);
    }
    /* Rx Entry */
    ...
    if (hash_data->fp_entry_trunk[0] != -1){
        ...
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                                  "OAM(unit %d) Error: FP Install failed (Trunk), "
                                  "EP=%d %s.\n"), unit, hash_data->ep_id, bcm_errmsg(rv)));
            return (rv);
        }
    }
    ...
    for (i=0; i<2; i++){
        if (modify_entry[i] != -1) {
            ...
            if (BCM_FAILURE(rv)) {
                LOG_ERROR(BSL_LS_BCM_OAM,
                          (BSL_META_U(unit,
                                      "OAM(unit %d) Error: FP Modify failed, EP=%d %s.\n"),
                           unit, hash_data->ep_id, bcm_errmsg(rv)));
                return (rv);
            }
        }
...
_bcm_tr3_oam_pri_map_profile_create(int unit, _bcm_oam_control_t *oc, 
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "OAM(unit %d) Error: service map profile add, EP=%d %s.\n"), 
                   unit, hash_data->ep_id, bcm_errmsg(rv)));
        return (rv);
    }
...
_bcm_tr3_oam_loss_delay_measurement_add(int unit, _bcm_oam_control_t *oc, 
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "OAM(unit %d) Error: LM Search failed, EP=%d %s.\n"),
                   unit, hash_data->ep_id, bcm_errmsg(rv)));
        return (rv);
    }
    ...
    LOG_VERBOSE(BSL_LS_BCM_OAM,
                (BSL_META_U(unit,
                            "OAM(unit %d): Existing EP on vlan port count=%d, "
        ...
        if (_bcm_oam_lm_dm_search_data.match_count > 0) {
          ...
          hash_data->lm_counter_index = existing_ep_hash_data->lm_counter_index;
            ...
            if (BCM_FAILURE(rv)) {
                LOG_ERROR(BSL_LS_BCM_OAM,
                          (BSL_META_U(unit,
                                      "OAM(unit %d) Error: ING_SERVICE_PRI_MAP profile "
                                      "creation, EP=%d %s\n"), unit, hash_data->ep_id, bcm_errmsg(rv)));
                return (rv);
            }
            ...
            if (BCM_FAILURE(rv)) {
                LOG_ERROR(BSL_LS_BCM_OAM,
                          (BSL_META_U(unit,
                                      "OAM(unit %d) Error: lm counter idx alloc failed, "
                                      "EP=%d %s.\n"), unit, hash_data->ep_id, bcm_errmsg(rv)));
                return (rv);
            }
        }
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "OAM(unit %d) Error: FP Create failed, EP=%d %s.\n"),
                   unit, hash_data->ep_id, bcm_errmsg(rv)));
    ...
    LOG_VERBOSE(BSL_LS_BCM_OAM,
                (BSL_META_U(unit,
                            "OAM(unit %d): oc->fp_glp_entry_cnt=%d\n"), 
...
_bcm_tr3_oam_loss_delay_measurement_delete(int unit, _bcm_oam_control_t *oc, 
    ...
    if (hash_data->flags & BCM_OAM_ENDPOINT_LOSS_MEASUREMENT) {
        ...
        if (_bcm_oam_lm_dm_search_data.match_count == 0){
            ...
            if (BCM_FAILURE(rv)) {
                LOG_ERROR(BSL_LS_BCM_OAM,
                          (BSL_META_U(unit,
                                      "OAM(unit %d) Error: lm counter idx free failed, "
                                      "EP=%d %s.\n"), unit, hash_data->ep_id, bcm_errmsg(rv)));
                return (rv);
            }
        }
    ...
    if ( _bcm_oam_lm_dm_search_data.match_count > 0 && 
        ...
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                                  "OAM(unit %d) Error: Eth Type dequlaify (tx), EP=%d %s.\n"),
                       unit, hash_data->ep_id, bcm_errmsg(rv)));
                return (rv);
        }
        ...
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                                  "OAM(unit %d) Error: Level dequlaify (tx), EP=%d %s.\n"),
                       unit, hash_data->ep_id, bcm_errmsg(rv)));
                return (rv);
        }
        ...
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                                  "OAM(unit %d) Error: Eth Type dequlaify (rx), EP=%d %s.\n"),
                       unit, hash_data->ep_id, bcm_errmsg(rv)));
                return (rv);
        }
        ...
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                                  "OAM(unit %d) Error: Level dequlaify (rx), EP=%d %s.\n"),
                       unit, hash_data->ep_id, bcm_errmsg(rv)));
                return (rv);
        }
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "OAM(unit %d) Error: FP Destroy failed (tx), EP=%d %s.\n"),
                   unit, hash_data->ep_id, bcm_errmsg(rv)));
        return (rv);
    }
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "OAM(unit %d) Error: FP Destroy failed (rx), EP=%d %s.\n"),
                   unit, hash_data->ep_id, bcm_errmsg(rv)));
        return (rv);
    }
    ...
    if (hash_data->fp_entry_trunk[0] != -1) {
        ...
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                                  "OAM(unit %d) Error: FP Destroy failed (Trunk), "
                                  "EP=%d %s.\n"), unit, hash_data->ep_id, bcm_errmsg(rv)));
            return (rv);
        }
    ...
    if (!(oc->fp_glp_entry_cnt)) {
        ...
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                                  "OAM(unit %d) Error: FP Group Destroy failed, EP=%d %s.\n"),
                       unit, hash_data->ep_id, bcm_errmsg(rv)));
            return (rv);
        }
                          ...
                          uint32 *l3_entry)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_OAM,
              (BSL_META_U(unit,
                          "OAM(unit %d) Info: _bcm_tr3_oam_rmep_recover.\n"),
               unit));
    /*
     * Get OAM control structure.
   ...
   /* Has to be a valid RMEP, return error otherwise. */
    ...
    if (1 == source_type) {
        ...
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                                  "OAM(unit %d) Error: Gport construct failed - %s.\n"),
                       unit, bcm_errmsg(rv)));
            goto cleanup;
        }
    ...
    LOG_DEBUG(BSL_LS_BCM_OAM,
              (BSL_META_U(unit,
                          "OAM(unit %d) Info: _bcm_tr3_oam_rmep_recover - done.\n"),
               unit));
...
cleanup:
    ...
    LOG_DEBUG(BSL_LS_BCM_OAM,
              (BSL_META_U(unit,
                          "OAM(unit %d) Info: _bcm_tr3_oam_rmep_recover - error_done.\n"),
               unit));
    return (rv);
}
                                    ...
                                    uint32 *l3_entry)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_OAM,
              (BSL_META_U(unit,
                          "OAM(unit %d) Info: _bcm_tr3_oam_lmep_rx_config_recover.\n"),
               unit));
    /*
     * Get OAM control structure.
     *     Note: Lock taken by calling routine.
...
_bcm_tr3_oam_lmep_tx_config_recover(int unit)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_OAM,
              (BSL_META_U(unit,
                          "OAM(unit %d) Info: _bcm_tr3_oam_lmep_tx_config_recover.\n"),
               unit));
                ...
                switch (soc_mem_field32_get(unit, mem, &l3_entry, KEY_TYPEf)) {
                    ...
                    case SOC_MEM_KEY_L3_ENTRY_LMEP:
                        ...
                        if (BCM_FAILURE(rv)) {
                            LOG_ERROR(BSL_LS_BCM_OAM,
                                      (BSL_META_U(unit,
                                                  "OAM(unit %d) Error: Local endpoint"
                                                   " (index=%d) reconstruct failed  - %s.\n"),
                                       unit, index, bcm_errmsg(rv)));
                            return (rv);
                        }
...
bcm_tr3_oam_group_create(int unit, bcm_oam_group_info_t *group_info)
{
    ...
    if (group_info->flags & BCM_OAM_GROUP_REPLACE) {
        if (group_info->flags & BCM_OAM_GROUP_WITH_ID) {
            ...
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                                  "OAM(unit %d) Error: Replace command needs a valid Group ID.\n"),
                       unit));
            return (BCM_E_PARAM);
        }
                      ...
                      bcm_oam_group_info_t *group_info)
{
    ...
    if (BCM_FAILURE(rv)) {
        ...
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "OAM(unit %d) Error: bcm_tr3_oam_group_get Group ID=%d "
                               "- Failed.\n"), unit, group));
        return (rv);
    }
                          ...
                          bcm_oam_group_t group)
{
    ...
    if (BCM_FAILURE(rv)) {
        ...
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "OAM(unit %d) Error: bcm_tr3_oam_endpoint_destroy_all"
                               " (GID=%d) - %s.\n"), unit, group, bcm_errmsg(rv)));
        return (rv);
    }
    ...
    for (grp_idx = 0; grp_idx < oc->group_count; grp_idx++) {
        ...
        if (BCM_E_EXISTS
            ...
            if (BCM_FAILURE(rv)) {
                ...
                LOG_ERROR(BSL_LS_BCM_OAM,
                          (BSL_META_U(unit,
                                      "OAM(unit %d) Error: _bcm_tr3_oam_get_group "
                                       "(GID=%d) - %s.\n"), unit, grp_idx, bcm_errmsg(rv)));
                return (rv);
            }
...
bcm_tr3_oam_endpoint_create(int unit, bcm_oam_endpoint_info_t *endpoint_info)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_OAM,
              (BSL_META_U(unit,
                          "OAM(unit %d) Info: bcm_tr3_oam_endpoint_create "
    ...
    if (1 == remote) {
        ...
        if (1 == mep_ccm_rx) {
            ...
            if (BCM_FAILURE(rv)) {
                ...
                LOG_ERROR(BSL_LS_BCM_OAM,
                          (BSL_META_U(unit,
                                      "OAM(unit %d) Error: LMEP Rx index alloc failed EP:%d "
                                       "%s.\n"), unit, endpoint_info->id, bcm_errmsg(rv)));
                return (rv);
            }
        }
            ...
            if (BCM_FAILURE(rv)) {
                ...
                if ( endpoint_info->flags & 
                    ...
                    if (BCM_FAILURE(rv)) {
                        LOG_ERROR(BSL_LS_BCM_OAM,
                                  (BSL_META_U(unit,
                                              "OAM(unit %d) Error: FP insert failed for "
                                              "LM DM over EP=%d %s.\n"), unit, endpoint_info->id, 
                                   bcm_errmsg(rv)));
        ...
        if (1 == h_data_p->is_remote) {
            ...
            if (1 == h_data_p->local_tx_enabled) {
                ...
                if (BCM_FAILURE(rv)) {
                    LOG_ERROR(BSL_LS_BCM_OAM,
                              (BSL_META_U(unit,
                                          "OAM(unit %d) Error: LMEP table read failed for"
                                           " EP=%d %s.\n"), unit, endpoint, bcm_errmsg(rv)));
                    _BCM_OAM_UNLOCK(oc);
                    return (rv);
                }
                ...
                if (BCM_FAILURE(rv)) {
                    LOG_ERROR(BSL_LS_BCM_OAM,
                              (BSL_META_U(unit,
                                          "OAM(unit %d) Error: LMEP_DA table read failed for"
                                           " EP=%d %s.\n"), unit, endpoint, bcm_errmsg(rv)));
                    _BCM_OAM_UNLOCK(oc);
                    return (rv);
                }
                        ...
                        bcm_oam_endpoint_info_t *endpoint_info)
{
    ...
    if(hash_data->flags & BCM_OAM_ENDPOINT_LOSS_MEASUREMENT) {
        ...
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                                  "BHH(unit %d) Error: Adding ING_SERVICE_PRI_MAP entry"
                                   " - %s.\n"), unit, bcm_errmsg(rv)));
                                ...
                                _bcm_oam_hash_key_t  *hash_key)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_OAM,
              (BSL_META_U(unit,
                          "BHH(unit %d) Info: bcm_tr3_oam_bhh_endpoint_create"
                           "Endpoint ID=%d.\n"), unit, endpoint_info->id));
    ...
    /* hash collision!!! Is this needed for BHH? */
        ...
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                                  "BHH(unit %d) Error: Hash table insert failed EP=%d %s.\n"),
                       unit, endpoint_info->id, bcm_errmsg(rv)));
            return (rv);
        }
#endif
...
_bcm_tr3_oam_bhh_hw_init(int unit)
{
    ...
    if(BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "BHH(unit %d) Error:hw init. Write CPU Control_0 Reg %s.\n"),
                   unit, bcm_errmsg(rv)));
        _BCM_OAM_UNLOCK(oc);
        return (rv);
    }
    ...
    if(BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "BHH(unit %d) Error:hw init. Write CPU Control_m Reg %s.\n"),
                   unit, bcm_errmsg(rv)));
        _BCM_OAM_UNLOCK(oc);
        return (rv);
    }
    ...
    if (i < min_cosq) {
        ...
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "BHH(unit %d) Error:hw init. queue channel set %s.\n"),
                   unit, bcm_errmsg(rv)));
        _BCM_OAM_UNLOCK(oc);
        return (rv);
    }
...
_bcm_tr3_oam_bhh_callback_thread(void *param)
{
    ...
    while (1) {
        ...
        if (sess_id < 0 || 
            sess_id >= oc->ep_count) {
            LOG_CLI((BSL_META("%s: Invalid sess_id:%d \n"), FUNCTION_NAME(), sess_id));
        } else {
            /* Check endpoint status. */
