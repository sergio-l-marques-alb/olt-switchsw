...
STATIC int
_tr3_l2_delete_all_by_hw(int unit, int vfi)
{
    int rv = BCM_E_NONE;
...
_tr3_l2x_delete_all(int unit)
{
    ...
    if(soc_property_get(unit, spn_MEM_CLEAR_HW_ACCELERATION, 1) && 
       !SAL_BOOT_SIMULATION) {
        return _tr3_l2_delete_all_by_hw(unit, -1);
    }
...
_bcm_tr3_l2_hash_dynamic_replace(int unit, _bcm_tr3_l2_entries_t *l2_entries)
{
    ...
    if (cf_unhit >= 0) {
        ...
        if (mem == L2_ENTRY_2m) {
            ...
            if (SOC_FAILURE(rv) || (narrow_avail == 0)) {
    ...
    if (SOC_SUCCESS(rv) && (narrow_avail < 2)) {
        rv = soc_mem_delete_index(unit, mem, MEM_BLOCK_ALL, l2_index);
    }
    if (SOC_SUCCESS(rv)) {
 ...
 *      l2addr - (OUT) Pointer to bcm_l2_addr_t structure to receive result
 * Returns:
 *      BCM_E_xxx
 * Notes:
 */
...
{    
    ...
    if (failover_ring->flags & 
        ...
        if (BCM_TRUNK_INVALID != trunk_id) {
            ...
            soc_reg_field_set(unit, FLOOD_LEARN_MATCH_PORT_Ar, &rval, PORT_NUMf, 
                              port_out);
            soc_reg_field_set(unit, FLOOD_LEARN_MATCH_PORT_Ar, &rval, 
                              MODULE_IDf, mod_out);
            soc_reg_field_set(unit, FLOOD_LEARN_MATCH_PORT_Ar, &rval, 
        ...
        if (BCM_TRUNK_INVALID != trunk_id) {
            ...
            soc_reg_field_set(unit, FLOOD_LEARN_MATCH_PORT_Br, &rval, PORT_NUMf, 
                              port_out);
            soc_reg_field_set(unit, FLOOD_LEARN_MATCH_PORT_Br, &rval, 
                              MODULE_IDf, mod_out);
            soc_reg_field_set(unit, FLOOD_LEARN_MATCH_PORT_Br, &rval, 
        ...
        soc_reg_field_set(unit, FLOOD_LEARN_CONTROLr, &rval, MATCH_PORT_Af, 1);
        soc_reg_field_set(unit, FLOOD_LEARN_CONTROLr, &rval, MATCH_PORT_Bf, 1);
        soc_reg_field_set(unit, FLOOD_LEARN_CONTROLr, &rval, 
...
bcm_tr3_failover_ring_config_get(int unit, bcm_failover_ring_t *failover_ring)
{
    ...
    _bcm_gport_dest_t dest;
    SHR_BITDCL *vlan_map = NULL;
    uint32 map_size, rval, dest_type;
...
{
    int vfi = 0, rv = BCM_E_NONE, gport_id;
    ...
    if (l2_ring->flags & BCM_L2_REPLACE_PROTECTION_RING) {
        ...
        if (BCM_TRUNK_INVALID != trunk_id) {
            soc_reg_field_set(unit, L2_BULK_MATCH_PORT_Ar, &rval, DESTINATIONf, 
                              trunk_id);
            soc_reg_field_set(unit, L2_BULK_MATCH_PORT_Ar, &rval, DEST_TYPEf, 
                              1);
        } else if (!(-1 == port_out || -1 == mod_out)) {
            soc_reg_field_set(unit, L2_BULK_MATCH_PORT_Ar, &rval, PORT_NUMf, 
                              port_out);
            soc_reg_field_set(unit, L2_BULK_MATCH_PORT_Ar, &rval, MODULE_IDf, 
                              mod_out);
            soc_reg_field_set(unit, L2_BULK_MATCH_PORT_Ar, &rval, DEST_TYPEf, 
                              0);
        } else if (-1 != gport_id) {
        ...
        if (BCM_TRUNK_INVALID != trunk_id) {
            soc_reg_field_set(unit, L2_BULK_MATCH_PORT_Br, &rval, DESTINATIONf, 
                              trunk_id);
            soc_reg_field_set(unit, L2_BULK_MATCH_PORT_Br, &rval, DEST_TYPEf, 
                              1);
        } else if (!(-1 == port_out || -1 == mod_out)) {
            soc_reg_field_set(unit, L2_BULK_MATCH_PORT_Br, &rval, PORT_NUMf, 
                              port_out);
            soc_reg_field_set(unit, L2_BULK_MATCH_PORT_Br, &rval, MODULE_IDf, 
                              mod_out);
            soc_reg_field_set(unit, L2_BULK_MATCH_PORT_Br, &rval, DEST_TYPEf, 
                              0);
        } else if (-1 != gport_id) {
        ...
        if (l2_ring->flags & BCM_FAILOVER_LOOKUP_DISABLE) {
            soc_reg_field_set(unit, AUX_L2_BULK_CONTROLr, &rval, FLOOD_FORCEf, 
                              1);
        }
        if (l2_ring->flags & BCM_FAILOVER_LEARN_DISABLE) {
            soc_reg_field_set(unit, AUX_L2_BULK_CONTROLr, &rval, LEARN_DISABLEf, 
                              1);
        }
        ...
        soc_reg_field_set(unit, AUX_L2_BULK_CONTROLr, &rval, 
                          MATCH_KEY_TYPE_PORT_Bf, 1);
        soc_reg_field_set(unit, AUX_L2_BULK_CONTROLr, &rval, MATCH_PORT_Af, 1);
        soc_reg_field_set(unit, AUX_L2_BULK_CONTROLr, &rval, MATCH_PORT_Bf, 1);
        soc_reg_field_set(unit, AUX_L2_BULK_CONTROLr, &rval, 
                          MATCH_VLANS_PORT_Af, 1);
        soc_reg_field_set(unit, AUX_L2_BULK_CONTROLr, &rval, 
                          MATCH_VLANS_PORT_Bf, 1);
