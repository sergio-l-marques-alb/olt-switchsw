/*
 * $Id: timesync.c,v 1.17 Broadcom SDK $
            ...
            defined(BCM_KATANA2_SUPPORT)
                ...
                if (SOC_IS_HURRICANE2(unit) || SOC_IS_GREYHOUND(unit)) {
                    if (!IS_XL_PORT(unit, port)) {
                        SOC_IF_ERROR_RETURN(
                            READ_UMAC_TIMESTAMP_ADJUSTr(unit, port, &rval));
                        soc_reg_field_set(unit, UMAC_TIMESTAMP_ADJUSTr, &rval, 
                                          AUTO_ADJUSTf, 0);
                        soc_reg_field_set(unit, UMAC_TIMESTAMP_ADJUSTr, &rval, 
                                          ADJUSTf, 0);
                        soc_reg_field_set(unit, UMAC_TIMESTAMP_ADJUSTr, &rval, 
                                          EN_1588f, 1);
                        SOC_IF_ERROR_RETURN(
                            WRITE_UMAC_TIMESTAMP_ADJUSTr(unit, port, rval));
                    }
                }
 ...
 * Parameters:
 *      unit                - (IN) bcm device
 *      timestamping_mode   - (IN) 32bit ot 48bit timestamping mode
 *
 * Returns:
 *      BCM_E_NONE - Success
 *      BCM_E_XXX
            ...
            if (val == SOC_BLK_XLPORT) {
                ...
                SOC_IF_ERROR_RETURN(
                    soc_reg_field32_modify(
                        ...
                        port, EGR_1588_TIMESTAMPING_MODEf, 0));
            }
        }
    }
#endif /* BCM_HURRICANE2_SUPPORT || BCM_GREYHOUND_SUPPORT */
