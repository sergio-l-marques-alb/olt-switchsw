...
_bcm_virtual_init(int unit, soc_mem_t vp_mem, soc_mem_t vfi_mem)
{
    ...
    if (soc_feature(unit, soc_feature_wlan)) {
        rv = _bcm_vp_alloc(unit, 0, (num_vp - 1), 1, SOURCE_VPm, _bcmVpTypeWlan, &vp);
        if (vp != 0) {
            rv = BCM_E_INTERNAL;
        }
        if (BCM_FAILURE(rv)) {
 ...
 */
int
_bcm_vp_ing_dvp_config(int unit, _bcm_vp_ing_dvp_config_op_t op, int vp, int nh_index)
{
    int rv = BCM_E_NONE;
    ...
    /* Configure DVP table */
    if ((op != _bcmVpIngDvpConfigClear) && (nh_index > 0)) {
        soc_ING_DVP_TABLEm_field32_set(unit, &dvp_entry,
                                       NEXT_HOP_INDEXf, nh_index);

#if defined (BCM_TRIDENT_SUPPORT)
        if (SOC_MEM_IS_VALID(unit, ING_DVP_2_TABLEm)) {
            soc_ING_DVP_TABLEm_field32_set(unit, &dvp_2_entry,
                                           NEXT_HOP_INDEXf, nh_index);
        }
#endif /* BCM_TRIDENT_SUPPORT */
    }
