...
static int tdpll_input_state_machine_running = 0;
static _bcm_ptp_mutex_t tdpll_input_state_machine_lock; /* protects state_machine_running */ 
    ...
    int stack_id)
{
    ...
    if (!tdpll_input_state_machine_lock) {
        tdpll_input_state_machine_lock = _bcm_ptp_mutex_create("tdpll_input_state");
    }
...
bcm_common_tdpll_input_clock_control(
    ...
    int enable)
{
    PTP_MUTEX_TAKE(tdpll_input_state_machine_lock, TDPLL_STATE_MACHINE_DPC_TIME_USEC_DEFAULT);

    /* Cancel prior input clock selector DPC process (if any). */
    sal_dpc_cancel(INT_TO_PTR(&bcm_tdpll_input_clock_state_machine));

    if (enable) {
        tdpll_input_state_machine_running = 1;
        ...
        tdpll_input_state_machine_running = 0;
    }

    PTP_MUTEX_RELEASE_RETURN(tdpll_input_state_machine_lock, BCM_E_NONE);
}
    ...
    void *unused1)
{
    ...
    if ((rv = _bcm_ptp_mutex_take(tdpll_input_state_machine_lock, TDPLL_STATE_MACHINE_DPC_TIME_USEC_DEFAULT))) {
        PTP_ERROR_FUNC("Unable to get TDPLL DPC protection mutex.  Will retry\n");
    ...
    if (!tdpll_input_state_machine_running) {
        /* This DPC starting probably raced the cancellation of the DPC */
        /* By not rescheduling the DPC, we stop its operation */
 ...
 dpc_release_return:
    _bcm_ptp_mutex_give(tdpll_input_state_machine_lock);
