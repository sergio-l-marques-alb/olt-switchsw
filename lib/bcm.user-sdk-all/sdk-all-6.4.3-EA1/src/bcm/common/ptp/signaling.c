...
typedef struct unit_sig_info_s {
    ...
    int send_pending_signaling_to_masters_running;
    _bcm_ptp_mutex_t dpc_lock; /* Protects send_pending_signaling_to_masters_running */
} unit_sig_info_t;
...
static int _bcm_ptp_slave_signaling_send(int unit, bcm_ptp_stack_id_t ptp_id,
    ...
    int8 log_intermessage_period, uint32 duration, uint16 sequenceId, _bcm_ptp_clock_peer_ext_t *slave);
    ...
    bcm_ptp_stack_id_t ptp_id)
{
    ...
    if (!unit_sig_array[unit].dpc_lock) {
        unit_sig_array[unit].dpc_lock = _bcm_ptp_mutex_create("slave_dpc");
    }
...
int _bcm_ptp_signaling_teardown(int unit, bcm_ptp_stack_id_t ptp_id)
{
    int rv = BCM_E_NONE;
    PTP_MUTEX_TAKE(unit_sig_array[unit].dpc_lock, BCM_PTP_SIGNAL_SLAVE_MUTEX_TIMEOUT_USEC);

    unit_sig_array[unit].send_pending_signaling_to_masters_running = 0;
    sal_dpc_cancel(INT_TO_PTR(&send_pending_signaling_to_masters));

    PTP_MUTEX_RELEASE_RETURN(unit_sig_array[unit].dpc_lock, rv);
}
    ...
    bcm_ptp_stack_id_t ptp_id)
{
    ...
    unit_sig_array[unit].send_pending_signaling_to_masters_running = 1;
    ...
    PTP_MUTEX_RELEASE_RETURN(unit_sig_array[unit].dpc_lock, BCM_E_NONE);
}
    ...
    bcm_ptp_clock_peer_t *slave_table)
{
    ...
    *num_slaves = signaled_slave_table.num_entries;
    ...
    for (i = 0; i < (*num_slaves); ++i) {
        slave_table[i] = signaled_slave_table.entry[i].slave.peer;
    }
    ...
    bcm_ptp_clock_peer_t *master)
{
    ...
    if (num_tlvs > 1 && multi_tlv_en == 0) {
        LOG_VERBOSE(BSL_LS_BCM_COMMON,
                    (BSL_META_U(unit,
                                "%s() failed %s\n"), FUNCTION_NAME(), "Multiple TLV support not enabled.\n"));
        return BCM_E_CONFIG;
    }
    ...
    if (BCM_FAILURE(rv = _bcm_ptp_update_peer_counts(unit, master->local_port_number, 0,
    ...
    bcm_ptp_message_type_t messageType)
{
    ...
    if (BCM_FAILURE(rv = _bcm_ptp_update_peer_counts(unit, entry->master.local_port_number, 0,
            &master_addr, 0, 1, 0, 0, 0, 0, 0, 0, 1, 0))) {
        PTP_ERROR_FUNC("_bcm_ptp_update_peer_counts()");
        return rv;
    }
    ...
    _bcm_ptp_clock_peer_ext_t *slave)
{
    ...
    default:
        LOG_VERBOSE(BSL_LS_BCM_COMMON,
                    (BSL_META_U(unit,
                                "%s() failed %s\n"), FUNCTION_NAME(), "Unknown or unsupported TLV type.\n"));
        return BCM_E_PARAM;
    }
    ...
    _bcm_ptp_construct_udp_packet(&slave->peer, &slave->local_address, udp, udpLen, packet, &packet_len);

    if (BCM_FAILURE(rv = _bcm_ptp_peer_address_convert(&slave->peer.peer_address, &slave_addr))) {
        PTP_ERROR_FUNC("_bcm_ptp_peer_address_convert()");
        return rv;
    }
    if (BCM_FAILURE(rv = _bcm_ptp_update_peer_counts(unit, slave->peer.local_port_number, 0, &slave_addr,
            0, 1, 0, 0, 0, 0, 0, 0, 1, 0))) {
        PTP_ERROR_FUNC("_bcm_ptp_update_peer_counts()");
        return rv;
    }
                                                          ...
                                                          packet_len, packet)))
    {
        LOG_ERROR(BSL_LS_BCM_COMMON,
                  (BSL_META_U(unit,
                              "In %s, _bcm_ptp_tunnel_message_to_world failed\n"), FUNCTION_NAME()));
    }
    ...
    void *unused)
{
    ...
    if ((rv = _bcm_ptp_mutex_take(unit_sig_array[unit].dpc_lock, BCM_PTP_SIGNAL_SLAVE_MUTEX_TIMEOUT_USEC))) {
        PTP_ERROR_FUNC("Unable to get signaling DPC protection mutex.  Will retry\n");
    ...
    if (!unit_sig_array[unit].send_pending_signaling_to_masters_running) {
        goto dpc_release_return;
        return;
    }
...
process_tlvs:
 ...
 dpc_release_return:
    _bcm_ptp_mutex_give(unit_sig_array[unit].dpc_lock);
    ...
    _bcm_ptp_clock_peer_ext_t *peer_ext)
{
    ...
    case bcmPTPUDPIPv4:
        ...
        if (peer->peer_address.raw_l2_header_length > PTP_MAX_L2_HEADER_LENGTH) {
            ...
            LOG_VERBOSE(BSL_LS_BCM_COMMON,
                        (BSL_META_U(unit,
                                    "%s() failed %s\n"), FUNCTION_NAME(), "Incompatible L2 header size.\n"));
            return BCM_E_CONFIG;
        }
        ...
        if (peer->local_port_number == any_port) {
            ...
            for (port_num = 1; port_num <= ci.num_ports; ++port_num) {
                ...
                if (protocol == pi.port_address.addr_type) {
                    /* See if the address matches, or if the port will accept any incoming address */
                    if (!sal_memcmp(data + src_addr_offset + PTP_IPV4_ADDR_SIZE_BYTES,
                                    pi.port_address.address, PTP_IPV4_ADDR_SIZE_BYTES) ||
                        (pi.rx_packets_criteria_mask & BCM_PTP_RXMAP_MATCH_ANY_ADDR)) {
                        break;
                    }
                }
            }
    ...
    uint8 *data)
{
    ...
    if (BCM_FAILURE(rv = _bcm_ptp_make_peer_from_signaling_msg(unit, ptp_id, clock_num,
            data_length, data, protocol, src_addr_offset, ptp_msg_offset, &peer_ext))) {
        return bcmPTPCallbackReject;
    }
        ...
        case bcmPTPTlvTypeRequestUnicastTransmission:
            {
                ...
                if (BCM_FAILURE(rv = _bcm_ptp_signaled_unicast_slave_exists(unit, ptp_id, &peer_ext, &i))) {
                    ...
                    sal_memcpy(signaled_slave_table.entry[i].slave.peer.clock_identity, peer_ext.peer.clock_identity, sizeof(bcm_ptp_clock_identity_t));
