...
/*
 * Function:
 *     bcm_field_compress_attr_t_init
 * Purpose:
 *     Initializes a bcm_field_compress_attr structure to all zeros
 * Parameters:
 *     c_attr - (IN/OUT) Pointer to field compression attribute structure
 * Returns:
 *     None
 * Notes:
 *     1. This function initializes size variable to -1, meaning utilize the 
 *        maximum available size for this compression identifier during create 
 *        operation.
 *     2. priority value will be initialized to 0.
 *     3. qual is set to bcmFieldQualifyCount.
 *     4. compress_id is set to 0. 
 */
void
bcm_field_compress_attr_t_init(bcm_field_compress_attr_t *c_attr)
{
    if (NULL != c_attr) {
       sal_memset(c_attr, 0x00, sizeof(*c_attr));
       c_attr->size = -1;
       c_attr->priority = 0;
       c_attr->compress_id = 0;
       c_attr->qual = bcmFieldQualifyCount;
    }
}
