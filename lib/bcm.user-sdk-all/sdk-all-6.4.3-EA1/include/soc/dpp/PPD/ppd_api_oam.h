...
#define SOC_PPD_OAM_IS_CLASSIFIER_ADVANCED_MODE(unit)   SOC_DPP_CONFIG(unit)->pp.oam_classifier_advanced_mode
 ...
 *     Your discriminator - packet identification
 *   SOC_SAND_IN  uint32                                 in_lif -
 *     In lif for classification
 *    SOC_SAND_OUT  uint32                                 tcam_entry -
 *     TCAM entry to be used.
 * REMARKS:
 *   None.
 * RETURNS:
 *   OK or ERROR indication.
 ...
 *   PROC
 * FUNCTION:
 *   Delete BFD LSP IPv4 entry from TCAM
 * INPUT:
 *   SOC_SAND_IN  int                   unit -
 *     Identifier of the device to access.
 *   SOC_SAND_IN  uint32                   tcam entry -
 *     TCAM entry used for endpoint.
 * REMARKS:
 *   None.
 * RETURNS:
 *   OK or ERROR indication.
