...
typedef SOC_PPC_EG_AC_VBP_KEY                                  SOC_PPD_EG_AC_VBP_KEY;
typedef SOC_PPC_EG_AC_CEP_PORT_KEY                             SOC_PPD_EG_AC_CEP_PORT_KEY;
typedef SOC_PPC_EG_VLAN_EDIT_CEP_INFO                          SOC_PPD_EG_VLAN_EDIT_CEP_INFO;
typedef SOC_PPC_EG_VLAN_EDIT_VLAN_INFO                         SOC_PPD_EG_VLAN_EDIT_VLAN_INFO;
typedef SOC_PPC_EG_AC_VLAN_EDIT_INFO                           SOC_PPD_EG_AC_VLAN_EDIT_INFO;
typedef SOC_PPC_EG_AC_INFO                                     SOC_PPD_EG_AC_INFO;
typedef SOC_PPC_EG_AC_MP_INFO                                  SOC_PPD_EG_AC_MP_INFO;
 ...
 *     Identifier of the device to access.
 *   SOC_SAND_IN  SOC_PPD_EG_AC_CEP_PORT_KEY                  *cep_key -
 *     The key (port(vlan-domain) x CVID).
 *   SOC_SAND_OUT SOC_PPD_AC_ID                               *out_ac_id -
 *     Out AC ID, the key is mapped to
 * REMARKS:
 *   The returned out_ac_id is used as key when calling the
 *   API soc_ppd_cnt_counter_map_set for assigning a counter
 *   which will count all packets egressing a specific CEP
 *   port and CVID. Note that packets that are forwarded to
 *   this CEP port with an explicit outAC assigned by the
 *   ingress PP will not be counted by this counter.
 * RETURNS:
 *   OK or ERROR indication.
*********************************************************************/
uint32
 ...
 *     Identifier of the device to access.
 *   SOC_SAND_IN  SOC_PPD_EG_AC_VBP_KEY                       *vbp_key -
 *     The key (port(vlan-domain) x VSI).
 *   SOC_SAND_OUT SOC_PPD_AC_ID                               *out_ac_id -
 *     Out AC ID, the key is mapped to
 * REMARKS:
 *   The returned out_ac_id is used as key when calling the
 *   API soc_ppd_cnt_counter_map_set for assigning a counter
 *   which will count all packets egressing a specific port
 *   and VSI. Note that packets that are forwarded to this
 *   port with an explicit outAC assigned by the ingress PP
 *   will not be counted by this counter.
 * RETURNS:
 *   OK or ERROR indication.
*********************************************************************/
uint32
...
CONST SOC_PROCEDURE_DESC_ELEMENT*
  soc_ppd_eg_ac_get_procs_ptr(void);
void
