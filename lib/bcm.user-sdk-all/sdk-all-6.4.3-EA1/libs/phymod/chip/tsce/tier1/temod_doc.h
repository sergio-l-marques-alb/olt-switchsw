...
Various acronyms used in this document is available at http://www.broadcom.com/press/glossary.php

TEMod has an unambiguous interface to system platforms: Software platforms can
...
Tier1 procedures don't have a fixed argument list lik in the past. They accept
...
platform. For most chips it will be an SBUS/PBUS protocol which the hardware
translates to TO_TSC protocol bus of TSCE. For Eagle PHY, the the hardware
translates SBUS/PBUS protocol to MDIO protocol. Note, both PCS and PMD registers
are accessed similarly, differing only in their devid fields.

Some user level examples for broadcom-SDK diagshells are listed below.
...
Forced speed and auto negotiation links require different treeatment. In forced
...
to compensate the lane swap in the board routing.   The notation for 
xgxs_tx_lane_map is physical lane base, and xgxs_rx_lane_map is the logic lane 
base.  The physical lane base notation is that the digit positions are based on 
physical lane as P3P2P1P0.  

- For example, xgxs_tx_lane_map=0x2031 means 
...
- Another example for xgxs_rx_lane_map=0x1320 which is logic lane base: it has
