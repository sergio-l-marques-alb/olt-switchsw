/*----------------------------------------------------------------------
 * $Id: tefmod_diagnostics.c,v 1.2 Broadcom SDK $
...
int tefmod_diag_topology(PHYMOD_ST *pc)
{
  ...
  {
  uint16_t pcs_lcl_lb = MAIN0_LPBK_CTLr_LOCAL_PCS_LOOPBACK_ENABLEf_GET(pcs_loopback_ctrl);
  uint16_t pcs_rmt_lb = MAIN0_LPBK_CTLr_REMOTE_PCS_LOOPBACK_ENABLEf_GET(pcs_loopback_ctrl);
  uint16_t pmd_dig_lb = TLB_RX_DIG_LPBK_CFGr_DIG_LPBK_ENf_GET(pmd_loopback_dig);
  uint16_t pmd_rmt_lb = TLB_TX_RMT_LPBK_CFGr_RMT_LPBK_ENf_GET(pmd_loopback_remote);

  pcs_lcl_lb = pcs_lcl_lb;
  pcs_rmt_lb = pcs_rmt_lb;
  pmd_dig_lb = pmd_dig_lb; 
  pmd_rmt_lb = pmd_rmt_lb; 
...
int _get_no_of_pcs_lane(PHYMOD_ST* pc)
{
  int port_type = 0; 
  int port_num  = 0; 
  switch(port_type) {
    case TEFMOD_MULTI_PORT:      return 1;
    case TEFMOD_TRI1_PORT:       return ((port_num==2) ? 2:1);
    case TEFMOD_TRI2_PORT:       return ((port_num==0) ? 2:1);
    case TEFMOD_DXGXS:           return 2;
    case TEFMOD_SINGLE_PORT:     return 4;
    default:                    return 0;
  }
  return 0;
}
