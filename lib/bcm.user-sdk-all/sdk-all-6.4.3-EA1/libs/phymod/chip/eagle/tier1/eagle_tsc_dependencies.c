...
void eagle_tsc_pmd_mwr_reg(const phymod_access_t *pa, uint16_t address, uint16_t mask, uint8_t lsb, uint16_t val) {
    ...
    for(i=1; i <= 0x8; i = i << 1) {
        if ( i & pa->lane_mask ) {
            pa_copy.lane_mask = i;
            phymod_tsc_iblk_write(&pa_copy, (PHYMOD_REG_ACC_TSC_IBLK | 0x10000 | (uint32_t) address), data);
        }
    }
}
...
void eagle_tsc_pmd_wr_reg(const phymod_access_t *pa, uint16_t address, uint16_t val){
