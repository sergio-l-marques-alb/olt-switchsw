...
int phymod_phy_ladder_setting_to_mV(const phymod_phy_access_t* phy, int8_t y, int16_t* level)
{
    ...
    if(NULL != __phymod_diagnostics__dispatch__[__type__]->f_phymod_phy_ladder_setting_to_mV) {
        PHYMOD_LOCK_TAKE(phy);
        __rv__ = __phymod_diagnostics__dispatch__[__type__]->f_phymod_phy_ladder_setting_to_mV(phy, y, level);
        PHYMOD_LOCK_GIVE(phy);
        PHYMOD_IF_ERR_RETURN(__rv__);
    } else { 
        PHYMOD_RETURN_WITH_ERR(PHYMOD_E_UNAVAIL, (_PHYMOD_MSG("phymod_phy_ladder_setting_to_mV isn't implemented for driver type")));
    }
