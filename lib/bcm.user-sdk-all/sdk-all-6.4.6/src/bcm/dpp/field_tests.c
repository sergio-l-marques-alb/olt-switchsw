...
#define _BCM_PETRA_FIELD_GROUP_PRIO_HIGHEST           121 /*127 (higest priority - 6 proiorities used for ITMH programmable mode)*/
...
#define _BCM_PETRA_FIELD_TEST_MAXIMAL_GROUP_ID        121 /*127 (higest group id - 6 groups used for ITMH programmable mode)*/
        ...
        if(qualifier == bcmFieldQualifyPacketRes) {
            ...
            if(qualifiers_info[qual_index].size == 0) {
                ...
                if(_BCM_PETRA_FIELD_TEST_DEBUG_CHECK_ERR) {
                    LOG_INFO(BSL_LS_BCM_FP,
                             (BSL_META_U(unit,
                                         "The qualifier %d in not supported \n"), qualifier));
                }
                _BCM_PETRA_FIELD_TEST_FAIL_MSG;
                return CMD_FAIL;
            }
                                                                     ...
                                                                     SOC_PPD_FP_PACKET_DIAG_INFO **info_out)
{
    ...
    /* get the information */
    soc_sand_res = soc_ppd_fp_packet_diag_get(unit, SOC_CORE_DEFAULT, &info);
    if(SOC_SAND_FAILURE(soc_sand_res)) {
        sal_free(packet);
        _BCM_PETRA_FIELD_TEST_FAIL_MSG;
        return CMD_FAIL;
                                                     ...
                                                     uint32 *mask)
{
    ...
    switch(qualifier) {
        ...
        case bcmFieldQualifyOutPorts:
            ...
            sal_memset(mask, 0xffffffff, _BCM_PETRA_FIELD_MAX_QUALIFY_LENGTH/8);
            break;
...
STATIC cmd_result_t _bcm_petra_field_test_itmh_parsing_test_tag(int unit, int core_id,  _bcm_dpp_field_stage_idx_t stage, uint32 x, _bcmPetraFieldTestSpeedMode mode, uint8 do_warmboot)
{
#ifdef BCM_ARAD_SUPPORT
    /*bcm_error_t rv;*/
    cmd_result_t result;
    ...
    if(SOC_IS_ARAD(unit)) {
        ...
        for(iteration_idx = 0; iteration_idx < nof_iterations; ++iteration_idx) { 
            ...
            for(flip = 0; flip < _BCM_PETRA_FIELD_TEST_ITMH_PARSING_TEST_NOF_FLIPS; ++flip) {
                ...
                soc_sand_res = soc_ppd_diag_pkt_associated_tm_info_get(unit, core, &packet_tm_info);
                if(SOC_SAND_FAILURE(soc_sand_res)) {
                    _BCM_PETRA_FIELD_TEST_FAIL_MSG;
                    return CMD_FAIL;
                }
...
cmd_result_t _bcm_petra_field_test_itmh_parsing_test_pb(int unit, _bcm_dpp_field_stage_idx_t stage, uint32 x, _bcmPetraFieldTestSpeedMode mode, uint8 do_warmboot)
{
    ...
    result = _bcm_petra_field_test_itmh_parsing_test_tag(unit, core, stage, x, mode, do_warmboot);
    if(result != CMD_OK) {
        _BCM_PETRA_FIELD_TEST_FAIL_MSG;
        return result;
    }
