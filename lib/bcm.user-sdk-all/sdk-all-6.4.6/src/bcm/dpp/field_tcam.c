                                          ...
                                          int backingStore)
{
    ...
    if(backingStore == FALSE){
        if ((groupData->groupFlags & _BCM_DPP_FIELD_GROUP_EXTENDED_DATABASES) && 
            ((groupData->internal_table_id == _BCM_DPP_INTERNAL_TABLE_ID_EXTENDED_IPV6) || 
             (groupData->internal_table_id == _BCM_DPP_INTERNAL_TABLE_ID_INRIF_MAPPING) || 
             (groupData->internal_table_id == _BCM_DPP_INTERNAL_TABLE_ID_EXTENDED_P2P)))
        {
            is_sw_remove_only = 1;
        }
    }
    ...
    if (_BCM_DPP_FIELD_TCAM_ENTRY_FLAGS(isExternalTcam, entry) & _BCM_DPP_FIELD_ENTRY_IN_HW) 
    {
        ...
        ppdr = soc_ppd_fp_entry_remove(unitData->unitHandle,
                                       groupData->hwHandle,
                                       _BCM_DPP_FIELD_TCAM_ENTRY_HW_HANDLE(isExternalTcam, entry), is_sw_remove_only);
...
/* coverity[uninit_use_in_call] */
            ...
            case bcmFieldActionIngressGportSet:
                ...
                if (BCM_GPORT_IS_SET(propActs[bcmIndex].bcmParam0)) {
                    ...
                    if ((BCM_GPORT_IS_VLAN_PORT(propActs[bcmIndex].bcmParam0)) &&  (0 == BCM_GPORT_VLAN_PORT_ID_GET(propActs[bcmIndex].bcmParam0))) {
                        ...
                        result = _bcm_dpp_gport_to_hw_resources(unit, propActs[bcmIndex].bcmParam0, _BCM_DPP_GPORT_HW_RESOURCES_GLOBAL_LIF_INGRESS|_BCM_DPP_GPORT_HW_RESOURCES_FEC, &gport_hw_resources);
                        inLif = gport_hw_resources.global_in_lif;
            ...
            case bcmFieldActionExternalValue0Set: /*48 bit so param1 can be used*/                
                ...
                case bcmFieldActionExternalValue0Set:
                    ...
                    if (propActs[bcmIndex].bcmParam1) {
                        hwAct[ppdIndex + 1].hwType = SOC_PPD_FP_ACTION_TYPE_FLP_ACTION_0;
                        hwAct[ppdIndex + 1].hwParam = propActs[bcmIndex].bcmParam1;
                        /* two hardware actions are used here */
                        actCount = 2;
                    }
    ...
    for (ppdIndex = 0; ppdIndex < SOC_PPD_FP_NOF_ACTIONS_PER_DB_MAX; ppdIndex++) {
        for (index = ppdIndex + 1;
            ...
            if (SOC_PPD_FP_ACTION_TYPE_FLP_ACTION_0 == hwAct[ppdIndex].hwType) {
                ...
                if (BCM_E_NONE == result) {
                    if (found) {
                        ...
                        if (index < SOC_PPD_FP_NOF_ACTIONS_PER_DB_MAX) {
                            ...
                            *param1 = 0;
                            if (entInfo.actions[index].type == SOC_PPC_FP_ACTION_TYPE_FLP_ACTION_0) {
