   ...
   ) 
{
    ...
    if (SOC_IS_ARAD(unit)) {
        ...
        if (meter_id.group == 0) {
            mem = SOC_IS_JERICHO(unit) ? MRPS_MCDA_PRFSELm : IDR_MCDA_PRFSELm;
        } else {
            mem = SOC_IS_JERICHO(unit) ? MRPS_MCDB_PRFSELm : IDR_MCDB_PRFSELm;
        }
        if (meter_id.id < 16384 || SOC_IS_JERICHO(unit)) {
            array_index = 0;
            index = meter_id.id / 4;
        } else {
            array_index = 1;
    ...
    *old_profile = tmp_old_profile;
    ...
    if (old_is_high_rate == is_high_rate) {
        ...
        if (*new_profile != 0) {
            ...
            rv = _bcm_dpp_am_template_meter_data_get(unit, core, *new_profile, group, is_high_rate, &old_sw_state_info);
        ...
        rv = _bcm_dpp_am_template_meter_exchange(unit, core, *old_profile, group, is_high_rate, sw_state_info, &is_last, new_profile, change_profile, &is_allocated);
        BCMDNX_IF_ERR_EXIT(rv);
    } else {
        /* if not, alloc in new rate */
        rv = _bcm_dpp_am_template_meter_alloc(unit, core, 0, group, is_high_rate, sw_state_info, new_profile, &is_allocated);
        BCMDNX_IF_ERR_EXIT(rv);
        /* remove old rate */
        rv = _bcm_dpp_am_template_meter_free(unit, core, *old_profile, group, old_is_high_rate, &is_last);
        BCMDNX_IF_ERR_EXIT(rv);
    }
    ...
    *policer_id = _DPP_POLICER_ID_FROM_METER_GROUP_CORE(unit, meter, act_meter_group, act_meter_core);
    ...
    for (meter_id_incr = 0; meter_id_incr < nof_meters; ++meter_id_incr) {
        ...
        rv = bcm_petra_meter_profile_alloc(unit, act_meter_core, act_meter_group, meter + meter_id_incr, &sw_state_info, 
										   &dpp_profile_info, is_high_rate, &new_profile, &old_profile, change_profile);
        if (rv == BCM_E_NOT_FOUND) {
    ...
    if (SOC_IS_ARAD(unit)) {
        ...
        if (meter < 16384 || SOC_IS_JERICHO(unit)) {
            array_index = 0;
            index = meter / 4;
        } else {
            array_index = 1;
		...
		if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
			...
			soc_sand_rv = soc_mem_read(soc_sand_dev_id, mem, MRPS_BLOCK(unit, act_meter_core), index, mem_val); 
		}
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
		...
		if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
			...
			soc_sand_rv = soc_mem_write(soc_sand_dev_id, mem, MRPS_BLOCK(unit, act_meter_core), index, mem_val); 
		}
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    }
...
bcm_petra_policer_init(int unit) 
{
    ...
    BCMDNX_IF_ERR_EXIT(POLICER_ACCESS.is_allocated(unit, &is_allocated));
    if(!is_allocated) {
   ...
   int *npolicers) 
{
	...
	if (SOC_IS_JERICHO(unit)) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("%s: ethernet policers not available for Jericho.\n"), FUNCTION_NAME()));
    }
