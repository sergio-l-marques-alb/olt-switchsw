...
_bcm_dpp_gport_trill_info_state_restore_cb(int unit, int version, shr_wb_htb_key_t key, shr_wb_htb_data_t data)
{
    ...
    switch(version) {
        case BCM_DPP_WB_GPORT_VERSION_1_0:
            /* determine if the element is already allocated during init() phase. */
            /* NOTE: this check can be optimized                                  */
            rc = _bcm_dpp_trill_sw_db_hash_trill_info_find(unit, (shr_htb_key_t)key, htb_pointer, FALSE);
            if (rc == BCM_E_NONE) {
                found = TRUE;   
            }
            ...
            if (found == FALSE) {
                /* insert in hash list */
                rc = _bcm_dpp_trill_sw_db_hash_trill_info_insert(unit, key, trill_info_state);
                BCMDNX_IF_ERR_EXIT(rc);
                found = TRUE;
            }
...
_bcm_dpp_gport_trill_vpn_state_restore_cb(int unit, int version, shr_wb_htb_key_t key, shr_wb_htb_data_t data)
{
    ...
    switch(version) {
        case BCM_DPP_WB_GPORT_VERSION_1_0:
            /* determine if the element is already allocated during init() phase. */
            /* NOTE: this check can be optimized                                  */
            rc = _bcm_dpp_trill_sw_db_hash_trill_vpn_find(unit, (shr_htb_key_t)key, htb_pointer, FALSE);
            if (rc == BCM_E_NONE) {
                found = TRUE;   
            }
            ...
            if (found == FALSE) {
                /* insert in hash list */
                rc = _bcm_dpp_trill_sw_db_hash_trill_vpn_insert(unit, key, trill_vpn_state);
                BCMDNX_IF_ERR_EXIT(rc);
                found = TRUE;
            }
...
_bcm_dpp_gport_mc_trill_state_restore_cb(int unit, int version, shr_wb_htb_key_t key, shr_wb_htb_data_t data)
{
    ...
    switch(version) {
        case BCM_DPP_WB_GPORT_VERSION_1_0:
            /* determine if the element is already allocated during init() phase. */
            /* NOTE: this check can be optimized                                  */
            rc = _bcm_dpp_trill_sw_db_hash_mc_trill_find(unit, (shr_htb_key_t)key, htb_pointer, FALSE);
            if (rc == BCM_E_NONE) {
                found = TRUE;   
            }
            ...
            if (found == FALSE) {
                /* insert in hash list */
                rc = _bcm_dpp_trill_sw_db_hash_mc_trill_insert(unit, key, mc_trill_state);
                BCMDNX_IF_ERR_EXIT(rc);
                found = TRUE;
            }
...
_bcm_dpp_gport_trill_route_state_restore_cb(int unit, int version, shr_wb_htb_key_t key, shr_wb_htb_data_t data)
{
    ...
    switch(version) {
        case BCM_DPP_WB_GPORT_VERSION_1_0:
            /* determine if the element is already allocated during init() phase. */
            /* NOTE: this check can be optimized                                  */
            rc = _bcm_pb_trill_sw_db_hash_trill_route_info_find(unit, (shr_htb_key_t)key, htb_pointer, FALSE);
            if (rc == BCM_E_NONE) {
                found = TRUE;   
            }
            ...
            if ( found == FALSE) {
                /* insert in hash list */
                rc = _bcm_pb_trill_sw_db_hash_trill_route_info_insert(unit, key, trill_route_state);
                BCMDNX_IF_ERR_EXIT(rc);
                found = TRUE;
            }
            break;
...
_bcm_dpp_gport_trill_src_state_restore_cb(int unit, int version, shr_wb_htb_key_t key, shr_wb_htb_data_t data)
{
    ...
    switch(version) {
        case BCM_DPP_WB_GPORT_VERSION_1_0:
            /* determine if the element is already allocated during init() phase. */
            /* NOTE: this check can be optimized                                  */
            rc = _bcm_pb_trill_sw_db_hash_trill_root_src_find(unit, (shr_htb_key_t)key, htb_pointer, FALSE);
            if (rc == BCM_E_NONE) {
                found = TRUE;   
            }
            ...
            if (found == FALSE) {
                /* insert in hash list */
                rc = _bcm_pb_trill_sw_db_hash_trill_root_src_insert(unit, key, trill_src_state);
                BCMDNX_IF_ERR_EXIT(rc);
                found = TRUE;
            }
...
_bcm_dpp_wb_gport_save_trill_info_state(int unit)
{
    ...
    switch(wb_info->version) {
        case BCM_DPP_WB_GPORT_VERSION_1_0:
            ...
            /* save state */
            rc = _bcm_dpp_trill_sw_db_hash_trill_info_iterate(unit, _bcm_dpp_gport_trill_info_state_save_cb);
            BCMDNX_IF_ERR_EXIT(rc);
...
_bcm_dpp_wb_gport_save_mc_trill_state(int unit)
{
    ...
    switch(wb_info->version) {
        case BCM_DPP_WB_GPORT_VERSION_1_0:
            ...
            /* save state */
            rc = _bcm_dpp_trill_sw_db_hash_mc_trill_iterate(unit, _bcm_dpp_gport_mc_trill_state_save_cb);
            BCMDNX_IF_ERR_EXIT(rc);
...
_bcm_dpp_wb_gport_save_trill_route_state(int unit)
{
    ...
    switch(wb_info->version) {
        case BCM_DPP_WB_GPORT_VERSION_1_0:
            ...
            /* save state */
            rc = _bcm_dpp_trill_sw_db_hash_trill_route_info_iterate(unit, _bcm_dpp_gport_trill_route_state_save_cb);
            BCMDNX_IF_ERR_EXIT(rc);
...
_bcm_dpp_wb_gport_save_trill_src_state(int unit)
{
    ...
    switch(wb_info->version) {
        case BCM_DPP_WB_GPORT_VERSION_1_0:
            ...
            /* save state */
            rc = _bcm_dpp_trill_sw_db_hash_trill_src_iterate(unit, _bcm_dpp_gport_trill_src_state_save_cb);
            BCMDNX_IF_ERR_EXIT(rc);
...
_bcm_dpp_wb_gport_save_trill_vpn_state(int unit)
{
    ...
    switch(wb_info->version) {
        case BCM_DPP_WB_GPORT_VERSION_1_0:
            ...
            /* save state */
            rc = _bcm_dpp_trill_sw_db_hash_trill_vpn_iterate(unit, _bcm_dpp_gport_trill_vpn_state_save_cb);
            BCMDNX_IF_ERR_EXIT(rc);
...
_bcm_dpp_wb_gport_pack_inlif_match_state(int unit, int version, _bcm_dpp_inlif_match_info_t *state, _bcm_dpp_inlif_match_info_t  *wb_state)
{
    ...
    switch(version) {
        case BCM_DPP_WB_GPORT_VERSION_1_0:
            ...
            _bcm_dpp_wb_store32(state->additional_flags, (uint8*)&wb_info->additional_flags);
            break;
...
_bcm_dpp_wb_gport_unpack_inlif_match_state(int unit, int version, _bcm_dpp_inlif_match_info_t  *wb_state, _bcm_dpp_inlif_match_info_t *state)
{
    ...
    switch(version) {
        case BCM_DPP_WB_GPORT_VERSION_1_0:
            ...
            state->additional_flags              = _bcm_dpp_wb_load32((uint8 *)&wb_info->additional_flags);
            break;
