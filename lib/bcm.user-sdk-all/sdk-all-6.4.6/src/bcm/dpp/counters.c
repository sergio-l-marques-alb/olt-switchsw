    ...
    *defer = TRUE;
    ...
    if (sal_mutex_take(unitData->cacheLock, sal_mutex_FOREVER)) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_INTERNAL,
                         (_BSL_BCM_MSG("unable to take unit %d"
                                       " counter mutex"),
                          unit));
    }
    ...
    if (sal_mutex_give(unitData->cacheLock)) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_INTERNAL,
                         (_BSL_BCM_MSG("unable to give unit %d"
                                       " counter mutex"),
                          unit));
    }
                        ...
                        uint64 *counter)
{
    ...
    for (index = 0; index < num_counters; index++) {
        if(unitData->proc[proc].avail_offsets[type[index]] == UNSUPPORTED_COUNTER_OFFSET){
             BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
                     (_BSL_BCM_MSG("unit %d counter processor %u unavailable counter type %s "),
    ...
    if (unitData->proc[proc].num_counters <= (base + num - 1)) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
                         (_BSL_BCM_MSG("unit %d"
                       ...
                       unsigned int *set_id)
{
    ...
    if (flags & BCM_DPP_COUNTER_WITH_ID) {
        ...
        if (*set_id >= unitData->proc[*proc].num_sets) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
                             (_BSL_BCM_MSG("unit %d counter processor %u"
        ...
        if (inUse & mask) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_EXISTS,
                             (_BSL_BCM_MSG("unit %d counter processor %d"
                ...
                if (is_alloc) {
                        ...
                        if (sIndex < sLimit) {
                            ...
                            if (sLimit < unitData->proc[pIndex].num_sets) {
                                ...
                                inUse |= mask;
#ifdef BCM_WARM_BOOT_SUPPORT
                                rv = _bcm_dpp_wb_counters_proc_inUse_set(unit, inUse, pIndex, sIndex);
                                BCMDNX_IF_ERR_EXIT(rv);
                      ...
                      unsigned int set_id)
{
    ...
    if (!is_alloc) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_CONFIG,
                         (_BSL_BCM_MSG("unit %d counter processor %u sets"
    ...
    if (0 == (inUse & mask)) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_NOT_FOUND,
                         (_BSL_BCM_MSG("unit %d counter processor %d set"
                                       " %d is not in use"),
                          unit,
                          proc,
                          set_id));
...
int _bcm_dpp_counter_bg_thread_detach(_bcm_dpp_counter_state_t *unit_data){
    ...
    if (unit_data->running){
        ...
        rv = _bcm_dpp_wb_counters_update_background_collection_enable_state(unit);
        if(rv != BCM_E_NONE) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_INTERNAL,(_BSL_BCM_MSG("unit %d error updating warm "
...
_bcm_dpp_counter_proc_inUse_config(_bcm_dpp_counter_state_t *unitData, soc_port_t crps_index){
    ...
    switch (unitData->proc[crps_index].source) {
        ...
        case SOC_TMC_CNT_SRC_TYPE_EGR_PP:
            ...
            allocSize = ((unitData->proc[crps_index].num_sets + 7) / 8) * sizeof(uint32);
            LOG_DEBUG(BSL_LS_BCM_CNT,
                      (BSL_META_U(unit,
        ...
        if (unitData->proc[crps_index].mode.src_type   == unitData->proc[j].mode.src_type &&
            ...
            if(unitData->proc[crps_index].mode.mode_statistics != unitData->proc[j].mode.mode_statistics) {
                ...
                if (unitData->proc[crps_index].mode.src_type == SOC_TMC_CNT_SRC_TYPE_EGR_PP ||
                    (unitData->proc[crps_index].mode.src_type == SOC_TMC_CNT_SRC_TYPE_ING_PP && 
                     (unitData->proc[crps_index].mode.mode_eg.type == SOC_TMC_CNT_MODE_EG_TYPE_VSI || 
                      unitData->proc[crps_index].mode.mode_eg.type == SOC_TMC_CNT_MODE_EG_TYPE_PMF))
                    ) { 
    ...
    for (index = 0; (index < unitData->num_counter_procs); index++) {
        if (unitData->proc[index].mode.src_type == SOC_TMC_CNT_SRC_TYPE_OAM) {
            if (unitData->proc[index].mode.command_id == 0 ||
                unitData->proc[index].mode.command_id == 1) {
                    ...
                    break;
            } else if (unitData->proc[index].mode.command_id == 2 ||
                       unitData->proc[index].mode.command_id == 3) {
...
bcm_dpp_counter_init(int unit)
{
    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        for (index = 0; (index < tempUnit->num_counter_procs); index++) {
            result = _bcm_dpp_wb_counters_update_mode_state(unit, index);
            BCMDNX_IF_ERR_EXIT(result);

            result = _bcm_dpp_wb_counters_update_valid_state(unit, index);
            BCMDNX_IF_ERR_EXIT(result);
        }
...
_bcm_dpp_counter_egress_que_build_pointer(_bcm_dpp_counter_state_t *unitData,
    ...
    BCMDNX_IF_ERR_EXIT(_bcm_dpp_counters_per_set(unitData, proc, &counters_in_set));
    ...
    for (cosq = 0 ; cosq < nof_priorities; cosq++) {
        ...
        if (unitData->proc[proc].mode.src_type == SOC_TMC_CNT_SRC_TYPE_EGR_PP) {
            /* 
             * EGQ TM Macro build 
             */ 
            ...
            if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {               
                ...
                proc_set_array[*set_cnt].ctr_set_id = (lval % unitData->proc[proc].num_sets);
            }
            (*set_cnt)++;
             ...
             * EPNI TM Macro build 
             */ 
            lval = (((out_port_field + cosq) & 0xff) << 5) | ((priority & 0x7) << 2) | (dp & 0x3);
                         ...
                         unsigned int *set_cnt)
{
    ...
    SOC_TMC_CNT_COUNTER_WE_BITMAP we_val_mask = 0;
    BCMDNX_INIT_FUNC_DEFS;
    _COUNTER_UNIT_CHECK(unit, unitData);

    for (proc_index = 0; proc_index < unitData->num_counter_procs; proc_index++) {
        if (!unitData->proc[proc_index].valid) {
            continue;
        }
        ...
        if (unitData->proc[proc_index].mode.mode_eg.type == SOC_TMC_CNT_MODE_EG_TYPE_TM || 
            ...
            if (unitData->proc[proc_index].mode.src_type == SOC_TMC_CNT_SRC_TYPE_EGR_PP && system_mc) {
                /*No system mc for PP source type*/
                BCM_EXIT;
            }

            /*the system multicast are in the upper 4k ID - which means that they are in the MSB - have the check reflecting that*/
            if (unitData->proc[proc_index].mode.src_type == SOC_TMC_CNT_SRC_TYPE_EPNI) {
                if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
                    if(counters_in_set >= 2){
                        if ((!is_mc) && (unitData->proc[proc_index].mode.we_val)) {
                            continue;
                        }
                        if ((is_mc) && !(unitData->proc[proc_index].mode.we_val)) {
                            continue;
                    ...
                    we_val_mask = (0x1 << 13) / unitData->proc[proc_index].num_sets;
                    if (!system_mc && (we_val_mask == (unitData->proc[proc_index].mode.we_val & we_val_mask))) {
                        continue;
                    }
                    if (system_mc && !(we_val_mask == (unitData->proc[proc_index].mode.we_val & we_val_mask))) {
                        continue;
                    }
                    we_val_mask = (0x1 << 14) / unitData->proc[proc_index].num_sets;
                    if (!is_mc && (we_val_mask == (unitData->proc[proc_index].mode.we_val & we_val_mask))) {
                        continue;
                    }
                    if (is_mc && !(we_val_mask == (unitData->proc[proc_index].mode.we_val & we_val_mask))) { 
                        continue;
            ...
            for (dp = 0; dp <= 3; dp++) {
                ...
                BCMDNX_IF_ERR_EXIT(result);
                if (match) {
                    result = _bcm_dpp_counter_egress_que_build_pointer(unitData, max_set_cnt, proc_index, tm_port, pp_port, offset, system_mc, is_mc, dp, proc_set_array, &set_index);
                    BCMDNX_IF_ERR_EXIT(result);
                }
            }
        ...
        if (SOC_TMC_CNT_SRC_TYPE_VOQ == unitData->proc[index].source && unitData->proc[index].mode.src_core == core_id) {
            ...
            if ((voq >= lfirst) && (voq < (lfirst + lcount))) {
                ...
                if ((ctrStat == bcm_dpp_counter_count)
                    ...
                    if (proc_and_set) {
                        proc_and_set->ctr_proc_id = index;
                        proc_and_set->ctr_set_id = lindex;
                    }
                    if (first) {
