...
_field_presel_entry_sync(int unit)
{
    ...
    uint32 endmarkerpresel  = _FIELD_WB_EM_PRESEL;
     ...
     int type_presel[11] =  {             /* types in _field_presel_entry_t */
    (int)((_bcmFieldInternalVariable << _FP_WB_TLV_BASIC_TYPE_SHIFT)
           | _bcmFieldInternalPreselId),
    (int)((_bcmFieldInternalVariable << _FP_WB_TLV_BASIC_TYPE_SHIFT)
           | _bcmFieldInternalPreselFlags),
    ...
    TLV_CREATE(_bcmFieldInternalControlPreselInfo, _bcmFieldInternalVariable,
                (presel_ent_count | (11 << _FP_WB_TLV_TYPE_SHIFT)), &tlv);
...
_field_group_sync(int unit, _field_group_t *fg)
{
    ...
    TLV_CREATE(_bcmFieldInternalEMGroupAset,
                                ...
                                _field_stage_t *stage_fc)
{
    ...
    int group_count = 0, rv = 0, i = 0; /* group coutn, ret val, loop var */
    uint32 end_marker = _FIELD_WB_EM_CLASS; /* end marker */
    ...
    for (pipe=0; pipe < num_pipes; pipe++) {
        rv  = _field_class_info_sync(unit, stage_fc->class_info_arr[pipe],
    ...
    while (fg != NULL) {
        ...
        for (i=0; i < fg->group_status.entry_count; i++) {
            ...
            f_ent = fg->entry_arr[i];
            /* parts_count hardcoded to 1, as compression entries cannot
...
_field_em_group_entries_sync(int unit, _field_group_t *fg)
{
    ...
    if (fg->em_mode == _FieldExactMatchMode320) {
        ...
        while  (count  != (num_attr * ent_count)) {
            ...
            _bcm_field_th_val_get(f_ent->tcam.key_hw, tbuf, 0, 101);
            soc_mem_field_set(unit, mem, bufp, MODE320__KEY_0_ONLYf, tbuf);
            _bcm_field_th_val_get(f_ent->tcam.key_hw, tbuf, 101, 59);
            ...
            _bcm_field_th_val_get(f_ent_part->tcam.key_hw, tbuftemp, 0, 42);
            _bcm_field_th_val_set(tbuf, tbuftemp, 59, 42);
            soc_mem_field_set(unit, mem, bufp, MODE320__KEY_1_ONLYf, tbuf);;
            _bcm_field_th_val_get(f_ent_part->tcam.key_hw, tbuf, 42, 101);
        ...
        if (fg->em_mode == _FieldExactMatchMode128) {
            while  (count  != (num_attr * ent_count)) {
                ...
                _bcm_field_th_val_get(f_ent->tcam.key_hw, tbuf, 101, 27);
                             ...
                             _field_stage_t *stage_fc)
{
    ...
    _field_slice_t *curr_slice = NULL;  /* Pointer to curr slice, lt slice */
    _field_lt_slice_t *curr_ltslice = NULL;
    int rv = 0, i = 0;            /* return value, loop variable */
    _field_group_t *fg = NULL;
     /* Number of groups, partitions in each group */
    int group_count = 0, parts_count = 0;
    _field_lt_entry_t *f_lt_ent = NULL;   /* pointer to lt entry */
    ...
    TLV_CREATE(_bcmFieldInternalEMOperMode, _bcmFieldInternalVariable, 0, &tlv);
    tlv->value = &stage_fc->oper_mode;
    TLV_WRITE(unit, tlv, ptr, pos);
...
_field_wb_size_calc(int unit, uint32* total_size, _field_control_t  *fc)
{
    ...
    int slice_count = 14;               /*number of slices (12 +2exactmatch ) */
    ...
    size_presel = basic_size_presel + (BCM_FIELD_QUALIFY_MAX/8);
    ...
    *total_size = size_fc + size_datafc + size_stage + (slice_count
     ...
     type_fc = 64 + (13 * 256) + 16; /* added 16 for presel */
     ...
     /* Exactmatch has 64k entries per pipe, and each entry has at max 5 attr */
     exactmatch_size = size_stage + sizeof(int) * 5 * 256000;
    ...
    for (i = 0; i < num_instances; i++) {
        ...
        while  ((tlv2.type != _bcmFieldInternalEndStructPreselInfo)
            ...
            switch (tlv2.type) {
                ...
                default:
                    LOG_ERROR(BSL_LS_BCM_FP,(BSL_META_U(unit,
                               "unknown type detected : PRESEL recovery\n")));
                    rv = BCM_E_INTERNAL;
                    goto cleanup;
            }
            type_pos++;
        ...
        if (f_pr->group !=NULL)
        {
            ...
            for (idx = 0; (idx < parts_count && f_presel_p != NULL); idx++) {
                ...
                if (0 != idx) {
                    ...
                    /*
                     * Given entry part TCAM index, determine the slice number                       * and slice offset index.
                     */
    ...
    while (tlv.type != _bcmFieldInternalEndStructSlice) {
        ...
        switch (tlv.type) {
            ...
            case _bcmFieldInternalSliceFlags:
                  /* Field is populated during init */
                  break;
    ...
    switch (act ->action) {
        case bcmFieldActionFabricEHAddOrUpdate:
            ...
            if ((a_offset.value[3] == 6)) {
                ...
                soc_mem_read(unit, EH_MASK_PROFILEm,
...
_field_actions_recover(int unit, _field_action_bmp_t act_bmp, uint32 *e_buf,
                       ...
                       int *hw_idx_array, uint32 action_count)
{
    ...
    for (action_id = 0; action_id < action_count; action_id++) {
        ...
        switch (tlv.type)
        {
            ...
            case _bcmFieldInternalEntryActionsPbmp:
                 _FP_XGS3_ALLOC(act_bmp->w,
                                (tlv.length * static_type_map[tlv.type].size),
                                "action_bmp");
                 *action_count = (tlv.length * static_type_map[tlv.type].size * 8);
...
_field_entry_recover(int unit, _field_group_t **new_fg)
{
    ...
    uint32 action_count = 0 ;           /* Action Count */
        ...
        _field_stage_t *stage_fc)
{
    ...
    uint32 flags[1];               /* compression entries have only one part */
    _field_action_bmp_t act_bmp;   /* bitmap of actions */
    int *hw_idx_array = NULL;      /* pointer to array of hw indexes */
    int *val_array = NULL;         /* pointer to array of values */
    uint32 action_count = -1 ;     /* Number of Actions */
    ...
    FP_LOCK(fc);
    WB_FIELD_CONTROL_GET(fc, ptr, pos);
    BCM_IF_ERROR_RETURN(_field_scache_stage_hdr_chk(fc,
    ...
    while (tlv.type != _bcmFieldInternalEndStageClass) {
        ...
        case _bcmFieldInternalGroupEntry:
             ...
             for (idx = 0; idx < entry_count; idx++) {
                ...
                _field_entry_info_recover(unit, f_ent, flags, 1, new_fg,
                                         &act_bmp, &val_array, &hw_idx_array,
                                         &action_count);
               /* Actions recover for entry */
               if (f_ent->flags & _FP_ENTRY_INSTALLED) {
                    rv =  _field_actions_recover(unit, act_bmp, NULL,
                                            f_ent, val_array,
                                            hw_idx_array, action_count);
               } else {
                    rv = _bcm_field_th_class_type_qset_get(unit,
    ...
    BCM_IF_ERROR_RETURN(_field_scache_stage_hdr_chk(fc, _FIELD_CLASS_DATA_END));
cleanup:
    if (curr_fg != NULL) {
        sal_free(curr_fg);
        curr_fg = NULL;
...
#define ACTION_DATA_SHIFT(act_data, shift)     \
    ...
    act_data[0] |= act_data[1] & umask;        \
    act_data[1] = (act_data[1] >> shift);      \
/*
 * Function:
 *      _field_wb_em_meter_set_recover
 * Purpose:
 ...
 */
int
_field_wb_em_meter_set_recover(int unit, _field_entry_t *f_ent,
                               uint32 *act_data) {
    /* check here */
    return BCM_E_NONE;
}
/*
 * Function:
 *      _field_wb_em_green_to_pid_set_recover
 * Purpose:
 ...
 */
int
_field_wb_em_green_to_pid_set_recover(int unit, _field_entry_t *f_ent,
                                      uint32 *act_data) {
/*    _bcm_field_action_offset_t a_offset; */
    _field_action_t *f_act = NULL, *prev_act = NULL;
...
/*
 * Function:
 *      _field_wb_em_mirror_override_set_recover
 * Purpose:
 ...
 */
int
_field_wb_em_mirror_override_set_recover(int unit, _field_entry_t *f_ent,
                                         uint32 *ebuf)
{
    _bcm_field_action_offset_t a_offset;
    _field_action_t *f_act = NULL, *prev_act = NULL;
...
/*
 * Function:
 *      _field_wb_em_nat_override_set_recover
 * Purpose:
 ...
 */
int
_field_wb_em_nat_override_set_recover(int unit, _field_entry_t *f_ent,
                                      uint32 *ebuf)
{
    _bcm_field_action_offset_t a_offset;
    _field_action_t *f_act = NULL, *prev_act = NULL;
...
/*
 * Function:
 *      _field_wb_em_sflow_set_recover
 * Purpose:
 ...
 */
int
_field_wb_em_sflow_set_recover(int unit,_field_entry_t *f_ent, uint32 *ebuf)
{
    _bcm_field_action_offset_t a_offset;
    _field_action_t *f_act = NULL, *prev_act = NULL;
    bcm_field_action_t action_idx;
...
/*
 * Function:
 *      _field_wb_em_cut_thru_override_set_recover
 * Purpose:
 ...
 */
int
_field_wb_em_cut_thru_override_set_recover(int unit, _field_entry_t *f_ent,
                                            uint32 *ebuf)
{
    return BCM_E_NONE;
}
/*
 * Function:
 *      _field_wb_em_urpf_override_set_recover
 * Purpose:
 ...
 */
int
_field_wb_em_urpf_override_set_recover(int unit, _field_entry_t *f_ent,
                                       uint32 *ebuf)
{
   _bcm_field_action_offset_t a_offset;
    _field_action_t *f_act = NULL, *prev_act = NULL;
...
/*
 * Function:
 *      _field_wb_em_ttl_override_set_recover
 * Purpose:
 ...
 */
int
_field_wb_em_ttl_override_set_recover(int unit, _field_entry_t *f_ent,
                                      uint32 *ebuf)
{
   _bcm_field_action_offset_t a_offset;
    _field_action_t *f_act = NULL, *prev_act = NULL;
...
/*
 * Function:
 *      _field_wb_em_lb_controls_set_recover
 * Purpose:
 ...
 */
int
_field_wb_em_lb_controls_set_recover(int unit, _field_entry_t *f_ent,
                                     uint32 *ebuf)
{
   _bcm_field_action_offset_t a_offset;
    _field_action_t *f_act = NULL, *prev_act = NULL;
 ...
 */
int
_field_wb_em_drop_set_recover(int unit, _field_entry_t *f_ent, uint32 *ebuf)
{
   _bcm_field_action_offset_t a_offset;
    _field_action_t *f_act = NULL, *prev_act = NULL;
    bcm_field_action_t action_idx;
...
/*
 * Function:
 *      _field_wb_em_change_cpu_cos_set_recover
 * Purpose:
 ...
 */
int
_field_wb_em_change_cpu_cos_set_recover(int unit, _field_entry_t *f_ent,
                                        uint32 *ebuf)
{
   _bcm_field_action_offset_t a_offset;
    _field_action_t *f_act = NULL, *prev_act = NULL;
    ...
    for (action_idx = 0; action_idx < (sizeof(action_arr)/sizeof(int)); action_idx++) {
        ...
        switch (action_arr[action_idx]) {
            case bcmFieldActionCosQCpuNew:
                ...
                break;
            case bcmFieldActionServicePoolIdNew:
                if ((a_offset.value[1] == 1) && (a_offset.value[2] == 2)){
                    valid = 1;
                    param[0] = a_offset.value[0];
                }
                break;
 ...
 */
int
_field_wb_em_nat_set_recover(int unit, _field_entry_t *f_ent,  uint32 *ebuf)
{
   _bcm_field_action_offset_t a_offset;
...
/*
 * Function:
 *      _field_wb_em_mirror_set_recover
 * Purpose:
 ...
 */
int
_field_wb_em_mirror_set_recover(int unit, _field_entry_t *f_ent, uint32 *ebuf)
{
   _bcm_field_action_offset_t a_offset;
    _field_action_t *f_act = NULL, *prev_act = NULL;
        ...
        switch (action_arr[action_idx]) {
            case bcmFieldActionMirrorIngress:
                if ((a_offset.value[0] & 0x1) && !(reg_value & 0x1)) {
                    ...
                    _bcm_esw_mirror_mtp_to_modport(unit, hw_index, FALSE,
...
/*
 * Function:
 *      _field_wb_em_copy_to_cpu_recover
 * Purpose:
 ...
 */
int
_field_wb_em_copy_to_cpu_recover(int unit, _field_entry_t *f_ent, uint32 *ebuf)
{
   _bcm_field_action_offset_t a_offset;
    _field_action_t *f_act = NULL, *prev_act = NULL;
    bcm_field_action_t action_idx;
...
/*
 * Function:
 *      _field_wb_em_l3swl2_set_recover
 * Purpose:
 ...
 */
int
_field_wb_em_l3swl2_set_recover(int unit, _field_entry_t *f_ent, uint32 *ebuf)
{
   _bcm_field_action_offset_t a_offset;
    _field_action_t *f_act = NULL, *prev_act = NULL;
    int  action_idx = 0;
        ...
        switch (action_arr[action_idx]) {
            ...
            case bcmFieldActionSrcMacNew:
                if ((a_offset.offset[1]==19) && (a_offset.width[1]==4)
                    && (a_offset.value[1]==1)) {
                    soc_mem_read(unit, EGR_L3_NEXT_HOPm, MEM_BLOCK_ANY, a_offset.value[0], &entry);
                    sa_disable = soc_mem_field32_get(unit, EGR_L3_NEXT_HOPm, &entry, IFP_ACTIONS__L3_SA_DISABLEf);
                    da_disable = soc_mem_field32_get(unit, EGR_L3_NEXT_HOPm, &entry, IFP_ACTIONS__L3_DA_DISABLEf);
                /*    ov_new = soc_mem_field32_get(unit, EGR_L3_NEXT_HOPm, &entry, IFP_ACTIONS__L3_VLAN_DISABLEf);
                    vn_action = soc_mem_field32_get(unit, EGR_L3_NEXT_HOPm, &entry, IFP_ACTIONS__VNTAG_ACTIONf); */
                }
                if (sa_disable == 1) {
                    valid = 1;
                    hw_index = a_offset.value[0];
                    soc_mem_field_get(unit, EGR_L3_NEXT_HOPm, (uint32 *)&entry, IFP_ACTIONS__MAC_ADDRESSf, temp);
                    for (i=0; i < 6; i++) {
                        param[i] = mac[i];
                    }
                }
            ...
            case bcmFieldActionOuterVlanNew:
                if ((a_offset.offset[1]==19) && (a_offset.width[1]==4)
                    ...
                    if (ov_new == 1) {
                        valid = 1;
                        param[0] = soc_mem_field32_get(unit, EGR_L3_NEXT_HOPm, &entry,  IFP_ACTIONS__INTF_NUMf);
                    }
                }
                break;
            case bcmFieldActionDstMacNew:
            ...
            case bcmFieldActionEtagNew:
                if ((a_offset.offset[1]==19) && (a_offset.width[1]==4)
                    ...
                    if (vn_action == 0x2) {
                        valid = 1;
                        hw_index = a_offset.value[0];
                        param[0] = soc_mem_field32_get(unit, EGR_L3_NEXT_HOPm, &entry, IFP_ACTIONS__VNTAGf);
                    }
                }
                break;
            case bcmFieldActionVnTagNew:
...
/*
 * Function:
 *      _field_wb_em_emredirect_set_recover
 * Purpose:
 ...
 */
int
_field_wb_em_emredirect_set_recover(int unit, _field_entry_t *f_ent,  uint32 *ebuf)
{
   _bcm_field_action_offset_t a_offset;
    _field_action_t *f_act = NULL, *prev_act = NULL;
    bcm_field_action_t action_idx;
    ...
    BCM_IF_ERROR_RETURN(_field_stage_control_get(unit,_BCM_FIELD_STAGE_EXACTMATCH, &stage_fc));
    BCM_IF_ERROR_RETURN(_field_trx_redirect_profile_get(unit, &redirect_profile));
    for (action_idx = 0; action_idx < (sizeof(action_arr)/sizeof(int)); action_idx++) {
        valid  = 0;
        ACTION_GET(unit, f_ent, ebuf, action_arr[action_idx], 0, &a_offset);
        switch (action_arr[action_idx]) {
            case bcmFieldActionRedirect:
                 if ((a_offset.value[1] == 6) && (a_offset.value[2] == 1)) {
                    valid = 1;
                    param[0] = a_offset.value[0];
                 }
                 break;
            ...
            case bcmFieldActionEgressMask:
                if (a_offset.value[1] == 4) {
                    hw_index = a_offset.value[0];
                    soc_mem_read(unit, IFP_REDIRECTION_PROFILEm, MEM_BLOCK_ANY, hw_index, entry);
                    soc_mem_field_get(unit, IFP_REDIRECTION_PROFILEm, (uint32 *)entry, BITMAPf, pbmp.pbits);
                    for (i=0; i < 4; i++) {
        ...
        if (valid) {
            ...
            SOC_PROFILE_MEM_ENTRIES_PER_SET(unit, redirect_profile, hw_index, 1);
        }
...
/*
 * Function:
 *      _field_wb_em_counter_set_recover
 * Purpose:
 ...
 */
int
_field_wb_em_counter_set_recover(int unit, _field_entry_t *f_ent, uint32 *ebuf) {
    return BCM_E_NONE;
}

#define ACTION_PROFILES_COUNT  17
/*
 * Function:
 ...
 */
int
_field_wb_action_profile_parse(int unit, _field_entry_t *f_ent, uint32 *ebuf, uint32 *act_data)
{
    uint32 prof;
    int i, temp;
    uint32 umask;
    ...
    prof = *ebuf;
    for (i=0; i < ACTION_PROFILES_COUNT; i++) {
        temp = (prof >> i);
        if (!(temp & 0x1)) {
            continue;
        }
        ...
        {
            case 0:
                _field_wb_em_meter_set_recover(unit, f_ent, act_data);
                ACTION_DATA_SHIFT(act_data, 28);
                break;
            case 1:
                _field_wb_em_green_to_pid_set_recover(unit, f_ent, act_data);
                ACTION_DATA_SHIFT(act_data, 1);
                break;
            case 2:
                _field_wb_em_mirror_override_set_recover(unit, f_ent, act_data);
               ACTION_DATA_SHIFT(act_data, 1);
                break;
            case 3:
                _field_wb_em_nat_override_set_recover(unit, f_ent,  act_data);
                ACTION_DATA_SHIFT(act_data, 1);
                break;
            case 4:
                _field_wb_em_sflow_set_recover(unit, f_ent,  act_data);
                ACTION_DATA_SHIFT(act_data, 1);
                break;
            case 5:
                _field_wb_em_cut_thru_override_set_recover(unit, f_ent, act_data);
                ACTION_DATA_SHIFT(act_data, 1);
                break;
           case 6:
                _field_wb_em_urpf_override_set_recover(unit, f_ent, act_data);
                ACTION_DATA_SHIFT(act_data, 1);
                break;
            case 7:
                _field_wb_em_ttl_override_set_recover(unit, f_ent, act_data);
                ACTION_DATA_SHIFT(act_data, 1);
                break;
            case 8:
                _field_wb_em_lb_controls_set_recover(unit, f_ent, act_data);
                 ACTION_DATA_SHIFT(act_data, 4);
                break;
            case 9:
                _field_wb_em_drop_set_recover(unit,f_ent,  act_data);
                 ACTION_DATA_SHIFT(act_data, 6);
                break;
            case 10:
                _field_wb_em_change_cpu_cos_set_recover(unit, f_ent, act_data);
                 ACTION_DATA_SHIFT(act_data, 8);
                break;
            case 11:
                _field_wb_em_mirror_set_recover(unit, f_ent, act_data);
                     ACTION_DATA_SHIFT(act_data, 12);
                break;
            case 12:
                _field_wb_em_nat_set_recover(unit,f_ent, act_data);
                 ACTION_DATA_SHIFT(act_data, 12);
                break;
            case 13:
                _field_wb_em_copy_to_cpu_recover(unit, f_ent, act_data);
                 ACTION_DATA_SHIFT(act_data, 19);
                break;
            case 14:
                _field_wb_em_l3swl2_set_recover(unit, f_ent, act_data);
                 ACTION_DATA_SHIFT(act_data, 23);
                break;
            case 15:
                _field_wb_em_emredirect_set_recover(unit,f_ent, act_data);
                 ACTION_DATA_SHIFT(act_data, 25);
                break;
            case 16:
                _field_wb_em_counter_set_recover(unit, f_ent, act_data);
                 ACTION_DATA_SHIFT(act_data, 25);
                break;
                             ...
                             uint32 *act_data)
{
    ...
    _field_wb_action_profile_parse(unit, f_ent, e_buf, act_data);
    return BCM_E_NONE;
    ...
    while (i < tlv.length) {
        ...
        if (hw_index != -1) {
            ...
            if (fg->em_mode == _FieldExactMatchMode128) {
                ...
                _bcm_field_th_val_set(tbuf, tbuftemp, 101, 27);
        ...
        _FP_XGS3_ALLOC(f_ent->tcam.key_hw, f_ent->tcam.key_size, "em entry key");
        sal_memcpy(f_ent->tcam.key_hw, tbuf, f_ent->tcam.key_size);
        sal_memcpy(f_ent->tcam.key, tbuf, f_ent->tcam.key_size);
        sal_memset(f_ent->tcam.mask, 0xff, f_ent->tcam.key_size);
        _field_wb_qos_actions_recover(unit, f_ent, qos_idx);
                                    ...
                                    _field_stage_t *stage_fc)
{
    ...
    FP_LOCK(fc);
    WB_FIELD_CONTROL_GET(fc, ptr, pos);
    BCM_IF_ERROR_RETURN(_field_scache_stage_hdr_chk(fc, _FIELD_EM_DATA_START));
    ...
    BCM_IF_ERROR_RETURN(_field_scache_stage_hdr_chk(fc, _FIELD_EM_DATA_END));
cleanup:
    if (new_fg != NULL) {
        sal_free(new_fg);
    }
