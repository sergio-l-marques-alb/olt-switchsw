...
STATIC int bcmi_xgs4_udf_offset_install_per_pipe(int unit,
    int e, uint32 hw_bmap, int base, int offset, int pipe_num);
#endif

/* tcam help apis */
STATIC int bcmi_xgs4_udf_tcam_entry_move(int unit,
    bcmi_xgs4_udf_tcam_entry_t *tcam_entry_arr, int src, int dst);

STATIC int bcmi_xgs4_udf_tcam_move_up(int unit,
    bcmi_xgs4_udf_tcam_entry_t *tcam_entry_arr, int dest, int free_slot);
...
#if defined (BCM_TOMAHAWK_SUPPORT)
STATIC int bcmi_xgs4_udf_tcam_entry_move_per_pipe(int unit, 
    bcmi_xgs4_udf_tcam_entry_t *tcam_entry_arr, int src, int dst, int pipe);

STATIC int bcmi_xgs4_udf_tcam_entry_insert_per_pipe(int unit,
    bcmi_xgs4_udf_tcam_info_t *tcam_new);
#endif
...
#if defined (BCM_TOMAHAWK_SUPPORT)
STATIC bcm_udf_oper_mode_t udf_oper_mode = bcmUdfOperModeGlobal;
#endif
...
#if defined (BCM_TOMAHAWK_SUPPORT)
    /* operation mode */
    alloc_size += sizeof(udf_oper_mode);
#endif
...
#if defined (BCM_TOMAHAWK_SUPPORT)
        offset_info->associated_pipe = scache_offset_p->associated_pipe;
 ...
 */
STATIC int
bcmi_xgs4_udf_offset_uninstall(int unit, int e, uint32 hw_bmap)
{
...
/*
 * Function:
 *      bcmi_xgs4_udf_offset_uninstall_per_pipe
 * Purpose:
 *      To uninstall the UDF offsets in the hardware
 * Parameters:
 *      unit            - (IN)      Unit number.
 ...
 */
STATIC int
bcmi_xgs4_udf_offset_uninstall_per_pipe(int unit, int e, uint32 hw_bmap, int pipe_num)
{
    int udf_idx, user_idx;
    int rv;
    int base = 0x0;
...
/*
 * Function:
 *      bcmi_xgs4_udf_offset_install_per_pipe
 * Purpose:
 *      To install the UDF offsets in the hardware
 * Parameters:
 *      unit            - (IN)      Unit number.
 ...
 *      base            - (IN)      value to write in BASE_OFFSETx.
 *      offset          - (IN)      Value to write in the OFFSETx.
 *      pipe_num        - (IN)      pipe number where the entry is installed
 * Returns:
 *      BCM_E_XXX
 * Notes:
 */
STATIC int
bcmi_xgs4_udf_offset_install_per_pipe(int unit, int e, uint32 hw_bmap,
...
/*
 * Function:
 *     bcmi_xgs4_udf_tcam_entry_move_per_pipe
 * Purpose:
 *     Move a single entry[pipe] in  udf tcam and offset tables.
 * Parameters:
 *     unit      - (IN) BCM device number.
 *     tcam_entry_arr - (IN) Data control structure.
 *     src       - (IN) Move from index.
 ...
 */
STATIC int
bcmi_xgs4_udf_tcam_entry_move_per_pipe(int unit,
                         bcmi_xgs4_udf_tcam_entry_t *tcam_entry_arr,
                         int src, int dst,
                         int pipe)
{
    ...
    offset_mem = SOC_MEM_UNIQUE_ACC(unit, UDF_CTRL(unit)->offset_mem)[pipe];
    if (offset_mem == INVALIDm) {
        return BCM_E_INTERNAL;
    }
                      ...
                      int dest, int free_slot)
{
    ...
    for (idx = free_slot; idx > dest; idx --) {
        rv = bcmi_xgs4_udf_tcam_entry_move(unit, tcam_entry_arr, idx - 1, idx);
        BCM_IF_ERROR_RETURN(rv);
    }
...
/*
 * Function:
 *     bcmi_xgs4_udf_tcam_move_up_per_pipe
 * Purpose:
 *     Moved udf tcam entries up.
 * Parameters:
 *     unit      - (IN) BCM device number.
 ...
 */
STATIC int
bcmi_xgs4_udf_tcam_move_up_per_pipe(int unit,
                      bcmi_xgs4_udf_tcam_entry_t *tcam_entry_arr,
                      int dest, int free_slot, int pipe)
{
    int idx;          /* Entries iterator.        */
    int rv;           /* Operation return status. */

    for (idx = free_slot; idx > dest; idx --) {
        rv = bcmi_xgs4_udf_tcam_entry_move_per_pipe(unit, tcam_entry_arr, idx - 1, idx, pipe);
        BCM_IF_ERROR_RETURN(rv);
    }
...
/*
 * Function:
 *     bcmi_xgs4_udf_tcam_move_down_per_pipe
 * Purpose:
 *     Moved udf tcam entries down.
 * Parameters:
 *     unit      - (IN) BCM device number.
 ...
 */
STATIC int
bcmi_xgs4_udf_tcam_move_down_per_pipe(int unit,
                      bcmi_xgs4_udf_tcam_entry_t *tcam_entry_arr,
                      int dest, int free_slot, int pipe)
{
    int idx;          /* Entries iterator.        */
...
/*
 * Function:
 *      bcmi_xgs4_udf_tcam_entry_insert_per_pipe
 * Purpose:
 *     To insert an entry in UDF_TCAM.pipe at an available index.
 * Parameters:
 *     unit        - (IN) BCM device number.
 *     tcam_new    - (IN) Tcam info to be inserted.
 * Returns:
 ...
 */
STATIC int
bcmi_xgs4_udf_tcam_entry_insert_per_pipe(int unit, bcmi_xgs4_udf_tcam_info_t *tcam_new)
{
    int idx;                       /* FP UDF tcam table interator.    */
    int rv;                        /* Operation return status.        */
    int idx_max;                   /* FP UDF tcam table index max.    */
    ...
    if (unused_entry_min > range_min) {
        ...
        rv = bcmi_xgs4_udf_tcam_move_up_per_pipe(unit, tcam_entry_arr,
                                         range_max, unused_entry_max, pipe_num);

        BCM_IF_ERROR_RETURN(rv);
        tcam_idx = range_max;
    } else if (unused_entry_max == -1) {
        rv = bcmi_xgs4_udf_tcam_move_down_per_pipe(unit, tcam_entry_arr,
                                           range_min, unused_entry_min, pipe_num);
        BCM_IF_ERROR_RETURN(rv);
        tcam_idx = range_min;
    } else if ((range_min - unused_entry_min) >
               (unused_entry_max - range_max)) {
...
{
#if defined (BCM_TOMAHAWK_SUPPORT)
        int port_count = 0, port = 0, first_pipe = -1;
        soc_info_t  *si;
        si = &SOC_INFO(unit);
#endif
    ...
    bcm_udf_pkt_format_id_t pkt_format_id)
{
    ...
    if ((tcam_info->num_udfs == 0)) {
        ...
        if (soc_feature(unit, soc_feature_udf_multi_pipe_support) &&
           (udf_oper_mode == bcmUdfOperModePipeLocal)) {
            rv = bcmi_xgs4_udf_tcam_entry_insert_per_pipe(unit, tcam_info);
    ...
    if (soc_feature(unit, soc_feature_udf_multi_pipe_support) &&
       (udf_oper_mode == bcmUdfOperModePipeLocal)) {
        rv = bcmi_xgs4_udf_offset_install_per_pipe(unit, tcam_info->hw_idx,
    ...
    bcm_udf_pkt_format_id_t pkt_format_id)
{
    ...
    if (soc_feature(unit, soc_feature_udf_multi_pipe_support) &&
       (udf_oper_mode == bcmUdfOperModePipeLocal)) { 
        rv = bcmi_xgs4_udf_offset_uninstall_per_pipe(
                                            unit, tcam_info->hw_idx,
                                            offset_info->hw_bmap, pipe_num);
    } else 
#endif
    {
