...
#define _BCM_SB2_LINKPHY_COUNTER_MAX   64
    ...
    if (BCM_PBMP_NOT_NULL(SOC_INFO(unit).linkphy_allowed_pbm)) {
        ...
        for (index = 0; index < SOC_MAX_NUM_PP_PORTS; index++) {
            ...
            if (valid) {
                int rv;
                bcm_port_t port;
                uint16 int_stream_id_1 = 0, int_stream_id_2 = 0;
                ...
                rv = _bcm_linkphy_port_int_stream_id_base_get(unit, port, &int_stream_id_base);
                BCM_IF_ERROR_RETURN(rv);

                rv = _bcm_linkphy_port_ext_stream_id_base_get(unit, port, &ext_stream_id_base);
                BCM_IF_ERROR_RETURN(rv);

                int_stream_id_1 = stream_id_1 - ext_stream_id_base;
                dev_int_stream_id_1 = int_stream_id_1 + int_stream_id_base;

                int_stream_id_2 = stream_id_2 - ext_stream_id_base;
                dev_int_stream_id_2 = int_stream_id_2 + int_stream_id_base;

                _BCM_KT2_LINKPHY_DEV_INT_STREAM_ID_USED_SET(unit, dev_int_stream_id_1);
                _BCM_KT2_LINKPHY_DEV_INT_STREAM_ID_USED_SET(unit, dev_int_stream_id_2);
    ...
    if (SOC_MEM_FIELD_VALID(unit, mem, INTERNAL_STREAM_IDf)) {
        ...
        */
        ...
        rxlp_int_stream_map_port_idx =
                              ((lp_block)? 128: 0) + int_stream_id -
                              SOC_INFO(unit).port_linkphy_s1_base[port];
...
_bcm_linkphy_subport_stat_callback(int unit)
{
    ...
    int linkphy_counter_max = _BCM_KT2_LINKPHY_COUNTER_MAX;
...
#if defined(BCM_SABER2_SUPPORT)
    if (SOC_IS_SABER2(unit)) {
        linkphy_counter_max = _BCM_SB2_LINKPHY_COUNTER_MAX;
    }
#endif
...
_bcm_linkphy_subport_init(int unit)
{
    ...
    int subport_port_max = _BCM_KT2_SUBPORT_PORT_MAX;
    int subport_group_max = _BCM_KT2_SUBPORT_GROUP_MAX;
    int index = 0;
...
#if defined(BCM_SABER2_SUPPORT)
    if (SOC_IS_SABER2(unit)) {
        subport_group_max = _BCM_SB2_SUBPORT_GROUP_MAX;
        subport_port_max = _BCM_SB2_SUBPORT_PORT_MAX;
    }
#endif
                              ...
                              bcm_gport_t *gport)
{
    ...
    *******************************/
    ...
    dev_int_stream_id_base = si->port_linkphy_s1_base[port];
    ...
    for (i = 0; i < config->num_streams; i++) {
        ...
        */
        ext_stream_id = config->stream_id_array[i] - ext_stream_id_base;
        dev_int_stream_id = ext_stream_id + dev_int_stream_id_base;
        int_stream_id = (lp_block) ? dev_int_stream_id - 128 : dev_int_stream_id;
        ...
        if (dev_int_stream_id > soc_mem_index_max(unit,
            ...
            LOG_ERROR(BSL_LS_BCM_SUBPORT,
                      (BSL_META_U(unit,
                       ...
                       dev_int_stream_id, port, dev_int_stream_id_base, int_stream_id));
                                 ...
                                 bcm_gport_t subport_port_gport)
{
    ...
    dev_int_stream_id_base = SOC_INFO(unit).port_linkphy_s1_base[port];
    ...
    */
    ...
    for (i = 0; i < num_streams; i++) {
        ...
        dev_int_stream_id = int_stream_id + dev_int_stream_id_base;
        mem = DEVICE_STREAM_ID_TO_PP_PORT_MAPm;
        if (dev_int_stream_id > soc_mem_index_max(unit,mem)) {
            LOG_ERROR(BSL_LS_BCM_SUBPORT,
                      (BSL_META_U(unit,
                       ...
                       dev_int_stream_id_base, int_stream_id, pp_port_index));
            return BCM_E_PARAM;
        }
                              ...
                              bcm_subport_config_t *config)
{
    ...
    uint16 int_stream_id = 0, dev_int_stream_id_base = 0, dev_int_stream_id = 0;
    uint16 ext_stream_id_base = 0;
    device_stream_id_to_pp_port_map_entry_t dev_sid_to_pp_entry;
    soc_mem_t mem;
                             ...
                             bcm_gport_t *gport)
{
    ...
    ***********************************************/
    ...
    soc_SUBPORT_TAG_TO_PP_PORT_MAPm_field32_set(unit,
        (uint32 *)&subtag_to_pp_entry, VALIDf, SOC_IS_SABER2(unit) ? 0x3 : 0x1);
...
bcm_kt2_subport_init(int unit)
{
    int rv = BCM_E_NONE;
    soc_info_t *si = &SOC_INFO(unit);
    int pp_port_index_max = _BCM_KT2_SUBPORT_PP_PORT_INDEX_MAX;
    int subport_group_max = _BCM_KT2_SUBPORT_GROUP_MAX;

#if defined(BCM_SABER2_SUPPORT)
    if (SOC_IS_SABER2(unit)) {
        pp_port_index_max = _BCM_SB2_SUBPORT_PP_PORT_INDEX_MAX;
        subport_group_max = _BCM_SB2_SUBPORT_GROUP_MAX;
    }
#endif
                              ...
                              _bcm_kt2_subport_info_t *subport_info)
{
    ...
    int pp_port_index_min = _BCM_KT2_SUBPORT_PP_PORT_INDEX_MIN;
    int pp_port_index_max = _BCM_KT2_SUBPORT_PP_PORT_INDEX_MAX;
...
#if defined(BCM_SABER2_SUPPORT)
    if (SOC_IS_SABER2(unit)) {
        pp_port_index_min = _BCM_SB2_SUBPORT_PP_PORT_INDEX_MIN;
        pp_port_index_max = _BCM_SB2_SUBPORT_PP_PORT_INDEX_MAX;
    }
#endif
    ...
    dev_int_stream_id_base =
        SOC_INFO(unit).port_linkphy_s1_base[subport_info->port];
    ...
    if (subport_info->port_type == _BCM_KT2_SUBPORT_TYPE_SUBTAG) {
        ...
        for (i = 0; i < subport_port_config.num_streams; i++) {
            ...
            subport_info->dev_int_stream_id[i] =
                int_stream_id + dev_int_stream_id_base;
        }
    }
        ...
        for (direction = 0; direction < _BCM_KT2_LINKPHY_DIRECTION_MAX;
            ...
            for (pool_id = 0; pool_id < _BCM_KT2_LINKPHY_POOL_MAX; pool_id++) {
                ...
                if (!(COMPILER_64_IS_ZERO(*vptr))) {
                    LOG_CLI((BSL_META_U(unit,
                            "DevStreamID = %d  %s%d  %s  0x%08x%08x\n"),
                            ((block_id) ? _BCM_KT2_LINKPHY_COUNTER_MAX +
                                          index : index),
                             ((direction) ? "TXLP" : "RXLP"),
                             block_id,
                             stat[stat_type_id],
                                 ...
                                 bcm_port_t port, int value)
{
    ...
    soc_mem_t mem;
    soc_info_t *si = &SOC_INFO(unit);
    int min_subport_index = KT2_MIN_SUBPORT_INDEX;
...
{
    int idx;
    int subport_group_max = _BCM_SB2_SUBPORT_GROUP_MAX;
