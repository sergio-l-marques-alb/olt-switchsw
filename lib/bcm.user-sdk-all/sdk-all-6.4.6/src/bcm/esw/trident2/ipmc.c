...
bcm_td2_ipmc_pim_bidir_init(int unit)
{
    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        if (BCM_FAILURE(rv)) {
         ...
         /* Program the table */
        mbuf = soc_cm_salloc(unit, SOC_MEM_TABLE_BYTES(unit, L3_IIFm), "L3_iif");
        if (NULL == mbuf) {
            bcm_td2_ipmc_pim_bidir_detach(unit);
            return BCM_E_MEMORY;
        }
                                          ...
                                          bcm_ip6_t ip6_mask)
{
    ...
    soc_mem_t mem;
    uint32    ip6_fields[4];
    ing_ipv6_mc_reserved_address_entry_t  entry_buf;

    mem = ING_IPV6_MC_RESERVED_ADDRESSm;
    /* clear entry memory */
    sal_memset(&entry_buf, 0, sizeof(ing_ipv6_mc_reserved_address_entry_t));
    /* encode v6 address into ip6_fields */
    SAL_IP6_ADDR_TO_UINT32(ip6_addr, ip6_fields);
    /* set address in entry */
    soc_ING_IPV6_MC_RESERVED_ADDRESSm_field_set(unit, &entry_buf, ADDRESSf,
                                                &(ip6_fields[0]));
    /* encode v6 mask into ip6_fields */
    SAL_IP6_ADDR_TO_UINT32(ip6_mask, ip6_fields);
    /* set the mask in entry */
    soc_ING_IPV6_MC_RESERVED_ADDRESSm_field_set(unit, &entry_buf, MASKf,
                                                &(ip6_fields[0]));
                                          ...
                                          bcm_ip6_t *ip6_mask)
{
    ...
    BCM_IF_ERROR_RETURN(soc_mem_read(unit, mem, MEM_BLOCK_ALL, 0,
                                     &entry_buf));
    /* deduce the address from entry */
    soc_ING_IPV6_MC_RESERVED_ADDRESSm_field_get(unit, &entry_buf, ADDRESSf,
                                                &(ip6_fields[0]));
    /* set the address in user buf */
    SAL_IP6_ADDR_FROM_UINT32(*ip6_addr, ip6_fields);

    /* deduce the mask from entry */
    soc_ING_IPV6_MC_RESERVED_ADDRESSm_field_get(unit, &entry_buf, MASKf,
                                                &(ip6_fields[0]));
