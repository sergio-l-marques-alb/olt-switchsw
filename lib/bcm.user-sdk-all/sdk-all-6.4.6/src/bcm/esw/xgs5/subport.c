/*
 * $Id: subport.c,v 1.28 Broadcom SDK $
...
/* Local var used for WB operations */
static uint32 _scache_ofst_;
#endif
...
bcmi_xgs5_subtag_subport_init(int unit)
{
    ...
    BCM_IF_ERROR_RETURN
        (soc_mem_clear(unit, SUBPORT_TAG_SGPP_MAPm, COPYNO_ALL, TRUE));
    ...
    if (_bcm_subtag_group_bitmap[unit] == NULL) {
        ...
        _bcm_subtag_group_bitmap[unit] = 
             sal_alloc(SHR_BITALLOCSIZE(_BCM_TD2P_SUBPORT_COE_GROUP_MAX),
    ...
    sal_memset(_bcm_subtag_group_bitmap[unit], 0,
        SHR_BITALLOCSIZE(_BCM_TD2P_SUBPORT_COE_GROUP_MAX));
    ...
    if (_bcm_subtag_subport_port_info[unit] == NULL) {
        _bcm_subtag_subport_port_info[unit] =
            sal_alloc((_BCM_TD2P_SUBPORT_COE_PORT_MAX *
    ...
    for (index = 0; index < _BCM_TD2P_SUBPORT_COE_PORT_MAX; index++) {
...
/* Do this only for the Newer CoE implementations */
    ...
    if (soc_feature(unit, soc_feature_hgproxy_subtag_coe)) {
        ...
        sal_memset(_bcm_subport_group_subport_port_count[unit], 0,
            (_BCM_TD2P_SUBPORT_COE_GROUP_MAX * sizeof(int)));
                                     ...
                                     bcm_subport_config_t *config)
{
    ...
    if (_BCM_COE_GPORT_IS_SUBTAG_SUBPORT_PORT(unit, subport_port_gport)) {
        ...
        for (i=0; i < _BCM_TD2P_SUBPORT_COE_PORT_MAX; i++) {
...
STATIC int
_bcm_coe_subtag_subport_default_lport_entry_set(int unit, int stm_index)
{
    ...
    if (SOC_MEM_FIELD_VALID(unit, LPORT_TABm, OUTER_TPIDf)) {
        soc_PORT_TABm_field32_set(unit, &lport_entry, OUTER_TPIDf, 0x8100);
    }
    ...
    if (SOC_MEM_FIELD_VALID(unit, LPORT_TABm, V4L3_ENABLEf)) {
         soc_LPORT_TABm_field32_set(unit, &lport_entry, V4L3_ENABLEf, 1);
    }

    /* Enable V6 L3 termination */
    if (SOC_MEM_FIELD_VALID(unit, LPORT_TABm, V6L3_ENABLEf)) {
         soc_LPORT_TABm_field32_set(unit, &lport_entry, V6L3_ENABLEf, 1);
    }

    /* Enable V4 IPMC */
    if (SOC_MEM_FIELD_VALID(unit, LPORT_TABm, V4IPMC_ENABLEf)) {
         soc_LPORT_TABm_field32_set(unit, &lport_entry, V4IPMC_ENABLEf, 1);
    }

    /* Enable V6 IPMC */
    if (SOC_MEM_FIELD_VALID(unit, LPORT_TABm, V6IPMC_ENABLEf)) {
         soc_LPORT_TABm_field32_set(unit, &lport_entry, V6IPMC_ENABLEf, 1);
    }
    ...
    /* Write the data back */
    BCM_IF_ERROR_RETURN
        (WRITE_SOURCE_TRUNK_MAP_TABLEm(unit, MEM_BLOCK_ANY,
                                       stm_index, &stm_entry)); 
...
_bcm_coe_subtag_subport_port_delete(int unit, bcm_gport_t subport)
{
    ...
    for (idx=0; idx < _BCM_TD2P_SUBPORT_COE_PORT_MAX; idx++) {
...
int bcmi_xgs5_subport_coe_group_create(int unit,
                                     ...
                                     bcm_gport_t *group)
{
    int rv = BCM_E_NONE, group_id = -1, i = 0;
    bcm_port_t port;
    soc_info_t *si = &SOC_INFO(unit);
    ...
    if (config->flags & BCM_SUBPORT_GROUP_WITH_ID) {
        ...
        * Check that only subtag group id bits (8-0) are configured*/
        if ((group_id >> _BCM_SUBPORT_COE_GROUP_PORT_SHIFT)
            ...
            LOG_ERROR(BSL_LS_BCM_SUBPORT,
                      (BSL_META_U(unit,"ERROR: subport group id = %d "
                                       ...
                                       " %d\n"), 
                                       group_id, 
                                       _BCM_TD2P_SUBPORT_COE_GROUP_MAX-1));
            return BCM_E_PARAM;
        }
        ...
        for (i =0; i < _BCM_TD2P_SUBPORT_COE_GROUP_MAX; i++) {
...
_bcm_coe_subport_free_entry_find(int unit, int *free_index)
{
    ...
    soc_mem_t mem = SUBPORT_COE_HW_SUBPORT_SUBTAG_SGPP_MEM(unit);
    ...
    for (index = start; index != end; index++) {
        ...
        if (SOC_SUCCESS(rv)) {
            if (!soc_mem_field32_get(unit, mem, (uint32 *)&entry, 
                        SUBPORT_COE_HW_SUBPORT_SUBTAG_SGPP(unit)->valid)) {
                /* Return the first entry that has the VALIDf unset */
                *free_index = index;
                return BCM_E_NONE;
            }
        }
...
{
    int i, j, k;
    bcm_stk_modid_config_t modIdInfo[BCM_TD2P_MAX_COE_MODULES];
    ...
    _bcm_stk_aux_modport_info_get(unit, BCM_TD2PLUS_MODID_TYPE_COE, modIdInfo);

    /* Get a (mod,port) from the CoE modules, also ensure it is not already
       used */
    for (i = 0; i < si->num_coe_modules; i++) {
        if((modIdInfo[i].modid != BCM_MODID_INVALID) &&
           (modIdInfo[i].num_ports != -1)) {
                for(j = 0; j < modIdInfo[i].num_ports; j++) {
                    /* Put together a GPORT_MODPORT with this
                       (mod,port) combo */
                    BCM_GPORT_MODPORT_SET(coe_system_gport, 
                                          modIdInfo[i].modid, j);
                    for(k = 0; k < _BCM_TD2P_SUBPORT_COE_PORT_MAX; k++) {
                        if(_BCM_SUBPORT_COE_SUBTAG_PORT_INFO_VALID_GET(unit, k)) {
                            if(_BCM_SUBPORT_COE_SUBTAG_PORT_INFO_SUBPORT_MODPORT_GET(unit, k) == 
                                                    coe_system_gport) {
                                /* Subport already in use, break */
                                break;
                            }
                        }
                    }

                    /* If no match was found in the added groups, we are good to use this (mod,port),
                       send it back to caller */
                    if(k == _BCM_TD2P_SUBPORT_COE_PORT_MAX) {
    ...
    /* Return BOC_E_FULL by default, we do not have a free entry available */
    return BCM_E_FULL;
}
...
int _bcm_xgs5_subport_coe_mod_port_local(int unit, int modid, bcm_port_t portid)
{
    ...
    rv = _bcm_stk_aux_modport_valid(unit, BCM_TD2PLUS_MODID_TYPE_COE, modid, portid);
...
int _bcm_xgs5_subport_coe_gport_local(int unit, bcm_gport_t gport)
{
    ...
    /* Check if the passed in (mod,port) is valid on the local device */
    rv = _bcm_stk_aux_modport_valid(unit, BCM_TD2PLUS_MODID_TYPE_COE, 
...
int _bcm_xgs5_subport_coe_mod_local(int unit, int modid)
{
    ...
    /* Check if the passed in (mod,port) is valid on the local device */
    rv = _bcm_stk_aux_mod_valid(unit, BCM_TD2PLUS_MODID_TYPE_COE, modid);
...
_bcm_coe_subtag_subport_port_add(int unit,
                                 ...
                                 bcm_gport_t *gport)
{
    int rv = BCM_E_NONE, subtag_index = -1;
    int sp_group_idx = -1, mod_port_index = 0, modid, portid, port;
    uint16 vlan_id = 0, phb_val = 0;
    soc_mem_t mem;
    subport_tag_sgpp_map_entry_t        subtag_to_sgpp_entry;
    ...
    rv = soc_mem_write(unit, mem, MEM_BLOCK_ANY, subtag_index,
                       &subtag_to_sgpp_entry);
    ...
    SOC_IF_ERROR_RETURN(
        _bcm_esw_src_mod_port_table_index_get(unit, modid, 
    ...
    /* Update the ports SOURCE_TRUNK_MAP entry with a default
       LPORT porperty  */
    BCM_IF_ERROR_RETURN(
       _bcm_coe_subtag_subport_default_lport_entry_set(unit, mod_port_index));
    ...
    if (SOC_FAILURE(rv)) {
       ...
       BCM_IF_ERROR_RETURN(soc_mem_field32_modify(unit,
                   SUBPORT_COE_HW_SUBPORT_SUBTAG_SGPP_MEM(unit), subtag_index,
                                                  VALIDf, 0));
        return rv;
    }
    ...
    rv = bcm_xgs3_nh_add(unit, 0, &nh_info, &nh_index);
    ...
    _bcm_subtag_subport_port_info[unit][subtag_index].nh_index = 
                                    BCM_XGS3_EGRESS_IDX_MIN + nh_index;
    ...
    _BCM_SUBPORT_COE_SUBTAG_PORT_INFO_SET(unit, subtag_index,
                                               ...
                                               bcm_qos_map_t *map)
{
    ...
    soc_mem_field32_set(unit, mem, &egr_subtag_dot1p_map, 
                                            ...
                                            bcm_qos_map_t *map)
{
    ...
    sal_memset(&egr_subtag_dot1p_map, 0, sizeof(egr_subtag_dot1p_map));
    soc_mem_field32_set(unit, mem, &egr_subtag_dot1p_map, 
                        SUBPORT_TAG_PRIORITYf, 0);
    soc_mem_field32_set(unit, mem, &egr_subtag_dot1p_map, 
                        SUBPORT_TAG_COLORf, 0);
    SOC_IF_ERROR_RETURN(WRITE_EGR_SUBPORT_TAG_DOT1P_MAPm(unit,
                            MEM_BLOCK_ALL, index, &egr_subtag_dot1p_map));
    return BCM_E_NONE;
}
    ...
    *trunk_id = BCM_TRUNK_INVALID;
    ...
    if ((soc_feature(unit, soc_feature_hgproxy_subtag_coe) &&
        _BCM_COE_GPORT_IS_SUBTAG_SUBPORT_PORT(unit, subport_port_gport))) {
            ...
            bcm_subport_config_t_init(&config);
            if(_bcm_coe_subtag_subport_port_get(unit, subport_port_gport, &config)
                == BCM_E_NONE) {
...
bcmi_xgs5_subport_wb_scache_alloc(int unit, uint16 version, int *alloc_sz)
{
    ...
    SCACHE_ALLOC_SIZE_INCR(*alloc_sz,
        (_BCM_TD2P_SUBPORT_COE_PORT_MAX * sizeof(_bcm_subtag_subport_port_info_t)));
...
_bcm_coe_subport_sw_dump(int unit)
{
    ...
    for(idx = 0; idx < _BCM_TD2P_SUBPORT_COE_GROUP_MAX; idx++) {
