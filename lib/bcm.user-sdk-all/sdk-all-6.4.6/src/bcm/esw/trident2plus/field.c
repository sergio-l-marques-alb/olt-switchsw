                             ...
                             soc_mem_t mem, uint32 *buf)
{
    ...
    while (oam_stat_node != NULL) {
        if (oam_stat_node->flags & _FP_OAM_STAT_ACTION_VALID) {
            ...
            /*        Enum                   S/W value     H/W value */
            /* StatActionIncrement                 0          0      */
            /* StatActionIncrementCancel           1          2      */
            /* StatActionSample                    2          3      */

            if (oam_action->action == bcmFieldStatActionIncrement) {
                action = oam_action->action;
            } else {
                action = oam_action->action + 1;
            }
                                          ...
                                          bcm_field_stat_action_t stat_action)
{
    ...
    if ((stat_action < 0) || (stat_action >= bcmFieldStatActionCount)) {
        LOG_ERROR(BSL_LS_BCM_FP,                                        
                (BSL_META("FP(unit %d) Error: Stat Action exceeeds the"
                          " maximum value\r\n"), unit));
        return BCM_E_PARAM;
    }
    ...
    while (oam_stat_node != NULL) {
        for (idx = 0; idx < 3; idx++) {
            if ((oam_stat_node->flex_pool_id
                ...
                oam_action = oam_stat_node->oam_stat_action;
                if (oam_action != NULL) {
                    if (oam_action_hw[oam_stat_node->pool_instance].action == 0) {
                        oam_action->action =
                            (oam_action_hw[oam_stat_node->pool_instance].action);
                    } else {
                        oam_action->action =
                            (oam_action_hw[oam_stat_node->pool_instance].action - 1);
                    }
                }
                break;
            }
