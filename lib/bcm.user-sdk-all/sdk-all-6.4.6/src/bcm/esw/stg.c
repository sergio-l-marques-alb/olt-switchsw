...
bcm_esw_stg_stp_set(int unit, bcm_stg_t stg, bcm_port_t port, int stp_state)
{
...
#ifdef BCM_TRIDENT2PLUS_SUPPORT
    _bcm_vp_group_port_type_t  port_type = _bcmVpGrpPortLocal;
    ...
    if (soc_feature(unit, soc_feature_vlan_vfi_membership)) {
        _bcm_td2p_vp_group_port_type_get(unit, port_in, &port_type);

        if (port_type == _bcmVpGrpPortLocal) {
    ...
    if (soc_feature(unit, soc_feature_vlan_vfi_membership)) {
        ...
        if (port_type == _bcmVpGrpPortLocal) {
            ...
            rv = bcm_td2p_vp_group_stg_move(unit, stg, port_in, 
                                            pvp, BCMI_VP_GROUP_INGRESS);
            soc_mem_unlock(unit, STG_TABm);
            if (rv == BCM_E_NONE) {
                soc_mem_lock(unit, EGR_VLAN_STGm);
                rv = bcm_td2p_vp_group_stg_move(unit, stg, port_in, 
                                                pvp, BCMI_VP_GROUP_EGRESS);
                soc_mem_unlock(unit, EGR_VLAN_STGm);
            }
        }
    } else 
#endif
	...
	*stp_state = BCM_STG_STP_FORWARD;
    ...
    if (soc_feature(unit, soc_feature_vlan_vfi_membership)) {
        ...
        if (port_type == _bcmVpGrpPortLocal) {
            ...
            soc_mem_lock(unit, STG_TABm);
            rv = bcm_td2p_vp_group_stg_get(unit, stg, port,
                                           BCMI_VP_GROUP_INGRESS, &pvp);
            if(rv == BCM_E_NONE) {
                /* Translate from hardware PVP_xxx state to BCM_STG_STP_xxx state. */
                BCM_IF_ERROR_RETURN
                   (_bcm_stg_pvp_translate(unit, pvp, stp_state));
            }
            soc_mem_unlock(unit, STG_TABm);
        }
    } else 
