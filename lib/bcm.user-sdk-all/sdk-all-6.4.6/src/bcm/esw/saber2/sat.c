...
#define _BCM_SAT_OAMP_PHY_PORT_NUMBER 5
 ...
 */
void 
bcm_sat_endpoint_info_t_init(bcm_sat_endpoint_info_t *endpoint_info)
{
   ...
   /* Calculate the hash key for given enpoint input parameters. */
    ...
    if(up_mep) {
        ...
        if (BCM_FAILURE(rv)) {
            LOG_DEBUG(BSL_LS_BCM_SAT, 
                      (BSL_META_U(unit,
                       "SAT(unit %d) Error: UP-MEP hw set failed EP=%d %s.\n"),
                         unit, endpoint_info->ep_id, bcm_errmsg(rv)));
            _bcm_sb2_sat_endpoint_cleanup(unit, up_mep, hash_key, hash_data);
            _BCM_SAT_UNLOCK(satc);
            return (rv);
        ...
        if (BCM_FAILURE(rv)) {
            LOG_DEBUG(BSL_LS_BCM_SAT, 
                      (BSL_META_U(unit,
                     "SAT(unit %d) Error: Down-MEP hw set failed EP=%d %s.\n"),
                         unit, endpoint_info->ep_id, bcm_errmsg(rv)));
            _bcm_sb2_sat_endpoint_cleanup(unit, up_mep, hash_key, hash_data);
            _BCM_SAT_UNLOCK(satc);
            return (rv);
...
_bcm_sb2_sat_wb_upmep_endpoints_recover(int unit)
{
    ...
    for (index = 0; index < satc->upsamp_ep_count; index++) {
        ...
        if (soc_EGR_SAT_SAMP_TCAMm_field32_get(unit, &entry, VALIDf)) {
            ...
            rv = _bcm_kt2_pp_port_to_modport_get(unit, sglp,
                                                  &modid, &port_id);
            if (BCM_FAILURE(rv)) {
                goto cleanup;
            }
            ...
            rv = _bcm_kt2_pp_port_to_modport_get(unit, dglp,
                                                  &modid, &port_id);
            if (BCM_FAILURE(rv)) {
                goto cleanup;
            }
                        ...
                        uint16 sglp, uint16 dglp)
{
	...
	/* Store INT_PRI */
    mask_field = (1 << soc_mem_field_length(unit, ING_SAT_SAMP_TCAMm, 
                                            INT_PRIf)) - 1;
    ...
    if (ep_info_p->flags & BCM_SAT_ENDPOINT_DOWNMEP) {
        ...
        soc_mem_field32_set(unit, data_mem, ptr_dentry, OLP_HDR_TYPE_COMPRESSEDf, 
                            BCM_SAT_DOWNMEP_OLP_HDR_TYPE & mask_field);			
    }
    else if (ep_info_p->flags & BCM_SAT_ENDPOINT_UPMEP) {
        mask_field = (1 << soc_mem_field_length(unit, data_mem, 
                                                OLP_HDR_TYPE_COMPRESSEDf)) - 1;
        soc_mem_field32_set(unit, data_mem, ptr_dentry, OLP_HDR_TYPE_COMPRESSEDf, 
                            BCM_SAT_UPMEP_OLP_HDR_TYPE & mask_field);			
    }
 ...
 */
int
bcm_sb2_sat_oamp_enable(int unit, int enable)
{
    int rv = SOC_E_NONE;
    uint32 reg_val;
    uint32 fld_val;
    ...
    if (enable) {
        ...
        fld_val = _BCM_SAT_OAMP_PHY_PORT_NUMBER;
