...
int global_oam_wd = 0;      /* WD timeout callbacks to host app. */
int global_oam_fifo = 0;    /* 8-entry FIFO overflowed. */
int global_oam_drop = 0;    /* WD queue overflow, dropped events. */
int global_oam_error = 0;   /* Group traversal complete and FIFO still overflowed, error. */
int global_oam_sdk = 0;     /* SDK table writes timed out. */
int global_oam_get = 1;     /* Streamline the WD callback processing. */
int global_oam_dpc = 0;     /* Flag for DPC feature, to delay fast-rate Watchdog timeout. */
int global_oam_timeout = 3000000; /* Watchdog timeout in usec.  default to 3 seconds. */
...
STATIC int _oam_get_endpoint_by_timer(int unit, uint32 timer, 
                                      uint32 *ep_rec_idx);
...
STATIC int is_oam_group_id_app_reserved(int unit, int id);
STATIC int is_oam_ep_id_app_reserved(int unit, int id);
...
_oam_oamep_prepare(int unit, soc_sbx_g2p3_oamep_t *oamep)
{
    ...
    if (tmp.function == _state[unit]->ep_type_ids.ccm_first) {
        ...
        oamep->delmentry = tmp.delmentry;

    } else if (tmp.function == _state[unit]->ep_type_ids.psc) {
        oamep->nextentry = tmp.nextentry;
        oamep->function = tmp.function;
        oamep->type = tmp.type;
        oamep->intpripm = tmp.intpripm;
        oamep->ftidx = tmp.ftidx;
        oamep->tx = tmp.tx;
        oamep->path = tmp.path;
        ...
        oamep->tx_path = tmp.tx_path;
        oamep->tx_fpath = tmp.tx_fpath;
        oamep->tx_reserved = tmp.tx_reserved;
        oamep->tx_revertive = tmp.tx_revertive;
        oamep->tx_pt = tmp.tx_pt;
        oamep->tx_request = tmp.tx_request;
        oamep->tx_version = tmp.tx_version;
        oamep->tx_pschdr = tmp.tx_pschdr;
        ...
        oamep->singletohost = tmp.singletohost;
        oamep->ftidx = tmp.ftidx;
        oamep->rxtxinterval = tmp.rxtxinterval;
        oamep->ipda = tmp.ipda;
...
_oam_remove_ep_from_trunk_list(int unit, bcm_trunk_t tid, dq_p_t ep_list_node)
{
    ...
    LOG_VERBOSE(BSL_LS_BCM_OAM,
                (BSL_META_U(unit,
                            "Removing endpoint-id from trunk-id %d\n"),
                 tid));
    ...
    if (trunk_data==NULL) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "Failed to find trunk %d entry: %d %s\n"),
...
bcm_fe2000_oam_init(int unit)
{
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "Failed to get oam timer params: %d %s\n"),
                   rv, bcm_errmsg(rv)));
        return rv;
    }
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "Failed to get oam timer policer bank params: "
                               "%d %s\n"), rv, bcm_errmsg(rv)));
        return rv;
    }
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "failed to init timer calendar manager\n")));
        _oam_cleanup(unit);
        return rv;
    }
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "failed to init loss threshold manager\n")));
        _oam_cleanup(unit);
        return rv;
    }
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "failed to configure exceptions\n")));
        _oam_cleanup(unit);
        return rv;
    }
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "failed ot allocate timer queue\n")));
        _oam_cleanup(unit);
        return rv;
    }
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "failed to start oam cb thread rv=%d\n"),
                   rv));
        _oam_cleanup(unit);
        return rv;
    }
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "Only level 2 warm boot is supported by OAM\n")));
        _oam_cleanup(unit);
        return rv;
    }
    ...
    if (!SOC_WARM_BOOT(unit) && (size > 0)) {
        ...
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                                  "Failed to allocate scache for warm boot: %s\n"),
                       bcm_errmsg(rv)));
            _oam_cleanup(unit);
            return rv;
        }
                                  ...
                                  bcm_oam_endpoint_info_t *endpoint_info)
{
    ...
    if(!endpoint_info) {
        ...
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "Invalid endpoint id.  Remote EPs must supply "
                            ...
                            bcm_oam_endpoint_info_t *endpoint_info)
{
    ...
    if (oamlsp1731 == _state[unit]->hash_data_store[endpoint].oam_type) {
        ...
        /* return RDI value from remote endpoint record */
        if (endpoint_info->flags & BCM_OAM_ENDPOINT_REMOTE) {
            ...
            if (BCM_FAILURE(rv)) {
                ...
                if (oamep.peerrdi) {
                    endpoint_info->persistent_faults |= BCM_OAM_ENDPOINT_FAULT_REMOTE;
                }
            }
        }
        ...
        LOG_VERBOSE(BSL_LS_BCM_OAM,
                    (BSL_META_U(unit,
                                "Persistent-Flags = 0x%8x, Flags = 0x%8x, Current-Faults = 0x%8x.\n"),
                     clear_persistent_faults, endpoint_info->flags, endpoint_info->persistent_faults));
                               ...
                               bcm_oam_endpoint_info_t *endpoint_info)
{
    ...
    trunk_info = sal_alloc(sizeof(bcm_trunk_add_info_t), "trunk_info");
    if (trunk_info == NULL) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "Failed to allocate trunk_info\n")));
        rv = BCM_E_MEMORY;
        goto exit;
    }
    ...
    if (BCM_GPORT_IS_TRUNK(endpoint_info->gport)) {
        ...
        if(!OAM_IS_ETHERNET(endpoint_info->type)) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                                  "Can not create endpoint of type %d, on trunks.\n"),
                       endpoint_info->type));
            rv = BCM_E_PARAM;
            goto exit;
        }
         ...
         */
        rv = bcm_fe2000_trunk_get_old (unit, tid, &(trunk_data->add_info));
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                                  "Failed to get trunk info: %d %s\n"),
                       rv, bcm_errmsg(rv)));
            goto exit;
        }
            ...
            if ((OAM_IS_MPLS_Y1731(endpoint_info->type)) || \
                ...
                if (rv == BCM_E_EXISTS) {
                    if (is_oam_ep_id_app_reserved(unit, ep_rec_index)) {
                        ...
                        if (!(endpoint_info->flags & BCM_OAM_ENDPOINT_REMOTE) && (rv2 == BCM_E_EXISTS)) {
    ...
    if (BCM_FAILURE(rv)) {
        ...
        if (!(endpoint_info->flags & BCM_OAM_ENDPOINT_REPLACE)) {
            ...
            if (BCM_GPORT_IS_TRUNK(endpoint_info->gport) || egrPath.is_trunk) {
                ...
                LOG_VERBOSE(BSL_LS_BCM_OAM,
                            (BSL_META_U(unit,
                                        "Adding hash_data %p, ep_idx %d to trunk"
                                         " list\n"),
                             (void *)hash_data, ep_rec_index));
            }
        }

        /* 
         * COPYFIRSTTOCPU is cleard by ucode
         * SDK will reflect this and autoclear thse flags
         */
        if (endpoint_info->flags & BCM_OAM_ENDPOINT_CCM_COPYFIRSTTOCPU) {
            endpoint_info->flags &= ~BCM_OAM_ENDPOINT_CCM_COPYFIRSTTOCPU;
        }
...
bcm_fe2000_oam_endpoint_destroy(int unit, bcm_oam_endpoint_t endpoint)
{
    ...
    /* For PSC, verify remote is destroyed before local endpoint */
    if (OAM_IS_PSC(ep_info.type)) {
        if (OAM_GROUP_EP_LIST_EMPTY(unit, ep_info.group)) {
            ...
            if (ep_info.flags & BCM_OAM_ENDPOINT_REMOTE) {
                ...
                if (!local_ep_count) {
                    ...
                    LOG_ERROR(BSL_LS_BCM_OAM,
                              (BSL_META_U(unit,
                                          "Peer PSC endpoint must be destroyed" 
    ...
    if (BCM_GPORT_IS_TRUNK(ep_info.gport)) {
        ...
        if(!OAM_IS_ETHERNET(ep_info.type)) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                                  "Can not create endpoint of type %d, "
                                   "on trunks.\n"), 
                       ep_info.type));
            return BCM_E_PARAM;
        }
    ...
    if (is_trunk) {
        ...
        if (trunk_info == NULL) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                                  "Unable to allocate %d for trunk_ifno\n"), 
                       sizeof(bcm_trunk_add_info_t)));
            return BCM_E_MEMORY;
        }
        trunk_data = &(_state[unit]->trunk_data[tid]);
    ...
    if(!OAM_GROUP_EP_LIST_EMPTY(unit, group)) {
        ...
        DQ_TRAVERSE(&OAM_GROUP_EP_LIST(unit,group), ep_elem) {
            ...
            if (BCM_FAILURE(tmp_rv)) {
                LOG_ERROR(BSL_LS_BCM_OAM,
                          (BSL_META_U(unit,
                                      "Failed to get epInfo on active endpoint 0x%x:"
...
{    
        ...
        (group_info->flags & BCM_OAM_GROUP_REPLACE)) 
    {
        ...
        if (rv != BCM_E_EXISTS) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                                  "Attempted to replace non-existent oam group id %d\n"),
                       group_info->id));
            rv = BCM_E_PARAM;
            goto Exit;
        }
        ...
        rv = shr_idxres_list_elem_state(_state[unit]->group_pool, group_info->id);
        if (rv == BCM_E_NOT_FOUND) { 
            /* Group not in reserved range */
            rv = shr_idxres_list_reserve(_state[unit]->group_pool,
                                     group_info->id, group_info->id);
            if (rv != BCM_E_NONE) {
                LOG_ERROR(BSL_LS_BCM_OAM,
                          (BSL_META_U(unit,
                                      "Failed to reserve oam group id %d: %d %s\n"),
        ...
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "Invalid flag - must replace with id. Flags "
                               "passed=0x%x\n"), group_info->flags));
        rv = BCM_E_PARAM;
        goto Exit;
    } else {
        ...
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                                  "Failed to allocate oam group id: %d %s\n"),
                       rv, bcm_errmsg(rv)));
            goto Exit;
        }
    }
...
bcm_fe2000_oam_group_destroy(int unit, bcm_oam_group_t group)
{
    ...
    if (rv != BCM_E_EXISTS) {
        ...
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "Attempted to destroy non-existent oam group id %d\n"),
                   group));
        return BCM_E_PARAM;
    }
...
_oam_trunk_cb(int unit, bcm_trunk_t tid, bcm_trunk_add_info_t *trunk_info, void *user_data)
{
    ...
    LOG_VERBOSE(BSL_LS_BCM_OAM,
                (BSL_META_U(unit,
                            "Updating OAM endpoints after a trunk change\n")));
    ...
    if (BCM_SUCCESS(rv) && trunk_data) {
                ...
                if (OAM_IS_ETHERNET(hash_data->type)) {
                    ...
                    if (BCM_SUCCESS(rv)) {
                        ...
                        if (!(ep_info.flags & BCM_OAM_ENDPOINT_REMOTE)) {
                            ...
                            if (BCM_FAILURE(rv)) {
                                LOG_ERROR(BSL_LS_BCM_OAM,
                                          (BSL_META_U(unit,
                                                      "Failed to get egrPath for epid %d: %d %s\n"),
                        ...
                        if (BCM_FAILURE(rv)) {
                            LOG_ERROR(BSL_LS_BCM_OAM,
                                      (BSL_META_U(unit,
                                                  "Failed to delete endpoint %d: %d %s\n"),
                                       save_ep_id, rv, bcm_errmsg(rv)));
                            status = rv;
                        }
                        ...
                        if (ep_info.flags & BCM_OAM_ENDPOINT_REMOTE) {
                            ...
                            if (BCM_FAILURE(rv)) {
                                LOG_ERROR(BSL_LS_BCM_OAM,
                                          (BSL_META_U(unit,
                                                      "Failed to update egrPath for epid %d: %d %s\n"),
                                           save_ep_id, rv, bcm_errmsg(rv)));
                                status = rv;
                            }
                        }
                        ...
                        if (BCM_FAILURE(rv)) {
                            LOG_ERROR(BSL_LS_BCM_OAM,
                                      (BSL_META_U(unit,
                                                  "Failed to re-create endpoint %d: %d %s\n"),
                                       save_ep_id, rv, bcm_errmsg(rv)));
                            status = rv;
                        }
                    } else {
                     ...
                     */
                     ...
                     if (BCM_FAILURE(status)) {
                         LOG_ERROR(BSL_LS_BCM_OAM,
                                   (BSL_META_U(unit,
                                               "Failed to get endpoint info %d: %d %s\n"),
                                    hash_data->rec_idx, rv, bcm_errmsg(rv)));
                     } else {
        ...
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "Failed to lookup trunk info for tid %d: %d %s\n"),
                   tid, rv, bcm_errmsg(rv)));
        status = BCM_E_PARAM;
    }
...
oam_cb_handler_thread(void *param)
{
    ...
    LOG_INFO(BSL_LS_BCM_OAM,
             (BSL_META("expired timer callback thread starting\n")));
...
thread_done:
    ...
    oam_control.cb_tid = SAL_THREAD_ERROR;
    LOG_INFO(BSL_LS_BCM_OAM,
             (BSL_META("expired timer thread exitting\n")));
...
_oam_process_expired_timers(int unit)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_OAM,
              (BSL_META_U(unit,
                          "Processing expired timers on unit %d\n"),
               unit));
    ...
    for (i=0; i<count; i++) {
        ...
        if (BCM_FAILURE(rv) ) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                                  "failed to find endpoint with timer id=%d\n"), 
                       elt.timer));
            continue;
        }

        if (_state[unit]->wd_cb_data.epCb) {
            (_state[unit]->wd_cb_data.epCb)(unit, 0, bcmOAMEventGroupCCMTimeout, 
                                        0, epRecIdx,
                                        _state[unit]->wd_cb_data.userData);
            global_oam_wd = global_oam_wd + 1;
        }
...
_oam_fe2000_oam_timer_expiration_reset(int unit, int timerid)
{
    ...
    if (sts != SB_OK) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "Internal error sbFe2000PmOamTimerRawGet failed. "
                               "rv(%d)\n"), sts));
    ...
    if (sts != SB_OK) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "Error resetting timer(id:%d). rv (%d) \n"),
                   timerid, sts));
        return soc_sbx_translate_status(sts);
    }
                                       ...
                                       uint32 *deadline, uint32 *time)
{
    ...
    if (sts != SB_OK) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "Internal error, sbFe2000PmOamTimerRead failed. "
                               "rv(%d) id %x\n"), sts, real_timerid));
        return soc_sbx_translate_status(sts);
    }
...
{
    uint32 fifo_status, timerid, timeout, temp, v, rdi, error;
    soc_sbx_g2p3_policer_params_t   timerParams;
    int                             rv, notify=0;
    oam_timer_queue_t               *queue = OAM_TIMER_QUEUE(unit);
    ...
    if (rv != SOC_E_NONE) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "Internal error reading oamtimer policer params \n")));
        return;
    }
    ...
    while (fifo_status && (timeout > 0) && (error == 0)) {
        ...
        LOG_VERBOSE(BSL_LS_BCM_OAM,
                    (BSL_META_U(unit,
                                " Iterating through timers to see what expired\n")));
        ...
        for (timerid = 1; timerid < timerParams.size; timerid++) {
            ...
            if (rv != BCM_E_NONE) {
                LOG_ERROR(BSL_LS_BCM_OAM,
                          (BSL_META_U(unit,
                                      "Internal error handling PM_ERROR.TIMER_EXPIRE"
            ...
            if (time >= deadline) {
                ...
                if (_oam_tq_queue_full(queue)) {
                    ...
                    if (_state[unit]->wd_cb_data.epCb)
                    {
                        ...
                        LOG_DEBUG(BSL_LS_BCM_OAM,
                                  (BSL_META_U(unit,
                                              "timer %d expired but no callback "
                                               "registered \n"),timerid));
                    }
                }
                ...
                if (rv != BCM_E_NONE) {
                    LOG_ERROR(BSL_LS_BCM_OAM,
                              (BSL_META_U(unit,
                                          "Error resetting timer(id:%d) handling "
    ...
    if (timeout <= 0) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "Timeout occurred while handling "
                               "PM_ERROR.TIMER_EXPIRED_FIFO_OVERFLOW condition\n")));
        global_oam_error = global_oam_error + 1;
    }
...
_oam_fe2000_wd_timeout_cb(void *unit_)
{
    ...
    while(fifo_status) {
        ...
        if (_oam_tq_queue_full(queue)) {
            LOG_WARN(BSL_LS_BCM_OAM,
                     (BSL_META_U(unit,
                                 "Timer queue Full on unit %d, dropping event.\n"),
                      unit));
            notify = 1;
            global_oam_drop = global_oam_drop + 1;
        } else {
            if (_state[unit]->wd_cb_data.epCb)
            {
                LOG_DEBUG(BSL_LS_BCM_OAM,
        ...
        LOG_DEBUG(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "Timer Expired:  id=%d (0x%08x) fifo_status=%d\n"),
...
_oam_cleanup(int unit)
{
    ...
    if (_state[unit]->mamep_htbl) {
        status = shr_htb_destroy(&_state[unit]->mamep_htbl, NULL);
        if( BCM_FAILURE(status)) {
            LOG_INFO(BSL_LS_BCM_OAM,
                     (BSL_META_U(unit,
                                 "error(%s) freeing mamep_htbl failed\n"),
                      bcm_errmsg(status)));
        }
    }
    if (_state[unit]->timer_htbl) {
        status = shr_htb_destroy(&_state[unit]->timer_htbl, NULL);
        if( BCM_FAILURE(status)) {
            LOG_INFO(BSL_LS_BCM_OAM,
                     (BSL_META_U(unit,
                                 "error(%s) freeing timer_htbl failed\n"),
                      bcm_errmsg(status)));
        }
    }
 ...
 */
int
_oam_egr_path_get(int unit, egr_path_desc_t *egrPath, uint32 epIdx)
{
    int rv = BCM_E_NONE;
    soc_sbx_g2p3_oamep_t oamep;
    soc_sbx_g2p3_oamep_t oamep2;
    uint32 dir = OAM_DIR_DOWN;

    egrPath->allocated = 0;

    LOG_INFO(BSL_LS_BCM_OAM,
             (BSL_META_U(unit,
                         "Getting egress path for oamepi=0x%x\n"), epIdx));
    /* use the given epIdx to find the egress path */
    rv = soc_sbx_g2p3_oamep_get(unit, epIdx, &oamep);
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "Failed to get oamep 0x%x on replace: %d %s\n"), 
    ...
    if (oamep.function == _state[unit]->ep_type_ids.ccm_first) {
        ...
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                                  "Failed to get (second) oamep 0x%x on replace: %d %s\n"), 
    ...
    } else if (oamep.function == _state[unit]->ep_type_ids.psc) {
        egrPath->ftIdx = oamep.ftidx;
        dir = OAM_DIR_DOWN;  /* only down mode supported for PSC */
        ...
        /* types: Second-CCM and Peer-CCM are not valid for egr_path_get (they do
           not contain an ftidx). */
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "_oam_egr_path_get called with an invalid epidx 0x%x \n"), 
                   epIdx));
        return BCM_E_PARAM;
    }
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "Failed to get fte 0x%x on replace: %d %s\n"),
    ...
    /* Up MEP CCM Endpoints do not have the OI -> ETE egress path
     * configured because they are an mcast forward and get flooded
     * on the vlan.  Down MEP PM are unicast and require the ETE encap.
     */
    if ((dir == OAM_DIR_DOWN) ||
        ...
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                                  "Failed to get oi 0x%x on replace: %d %s\n"),
        ...
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                                  "Failed to get eteencap 0x%x on replace: %d %s\n"),
        ...
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                                  "Failed to get etel2 0x%x on replace: %d %s\n"),
...
/* all three must be set to see this dump */
    ...
    if (LOG_CHECK(BSL_LS_BCM_OAM | BSL_DEBUG)) {
        LOG_INFO(BSL_LS_BCM_OAM,
                 (BSL_META_U(unit,
                             "Retrieved egress path :\n")));
        LOG_INFO(BSL_LS_BCM_OAM,
                 (BSL_META_U(unit,
                             "fte Idx=0x%x\n"),
                  egrPath->ftIdx));
...
int _oam_egr_path_alloc(int unit, bcm_mac_t smac, egr_path_desc_t *egrPath)
{
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "failed to allocate egress smac " L2_6B_MAC_FMT 
                               "  %s\n"),
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "Failed to allocate ete encap: %d %s\n"),
                   rv, bcm_errmsg(rv)));
        return rv;
    }
    ALLOC_SET(ETEENCAP, egrPath->allocated);
    ...
    LOG_INFO(BSL_LS_BCM_OAM,
             (BSL_META_U(unit,
                         "FTE=0x%08X OI=0x%08X ETEENCAP=0x%08X ETEL2=0x%08X\n"),
...
int
_oam_egr_path_clone_downmep(int unit, egr_path_desc_t *clone, uint32 rec_idx)
{
    int                  rv;
    egr_path_desc_t      src;
    bcm_mac_t            smac;
    ...
    /* This function called for down MEP LM & DM endpoints. */

    /* This function copies the down MEP CCM's EgrPath to the cloned
     * egress path for the LM or DM service attached to the same MEP.
     */
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "Failed to get src copy of egr path: %s\n"),
                   bcm_errmsg(rv)));
        return rv;
    }
...
_oam_egr_path_clone_upmep(int unit, egr_path_desc_t *clone, bcm_gport_t gport, int vid, bcm_mac_t smac)
{
    ...
    /* This function called for up MEP LM & DM endpoints. */

    /* This function copies the data (or service) packet's egrPath
     * and modifies it for the unicast OAM (PM) egrPath.
     */
    ...
    LOG_INFO(BSL_LS_BCM_OAM,
             (BSL_META_U(unit,
                         "UP MEP cloned smac:  %02x:%02x:%02x:%02x:%02x:%02x \n"), 
...
_oam_egr_path_commit(int unit, egr_path_desc_t *egrPath)
{
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "unable to write ete[%08X]L2: %d (%s)\n"),
                   egrPath->eteL2Idx, rv, bcm_errmsg(rv)));
        return rv;
    }
                     ...
                     bcm_trunk_add_info_t *trunk_info)
{
    ...
    if (dir == OAM_DIR_UP) {
        ...
        LOG_INFO(BSL_LS_BCM_OAM,
                 (BSL_META_U(unit,
                             "mapped Vlan 0x%08X to FTE 0x%08X \n"),
...
int _oam_egr_path_free(int unit, egr_path_desc_t *egrPath)
{
    ...
    LOG_INFO(BSL_LS_BCM_OAM,
             (BSL_META_U(unit,
                         "Freeing resources: fti=0x%08x oi=0x%08x "
    ...
    if (ALLOC_GET(ESMAC, egrPath->allocated)) {
        ...
        if (rv == BCM_E_EMPTY) {
            ...
            LOG_VERBOSE(BSL_LS_BCM_OAM,
                        (BSL_META_U(unit,
                                    "Found no more users of idx %d, clearing hw\n"),
            ...
            if (BCM_FAILURE(rv)) {
                LOG_WARN(BSL_LS_BCM_OAM,
                         (BSL_META_U(unit,
                                     "Failed to delete egress smac: %d %s\n"),
                          rv, bcm_errmsg(rv)));
            }
        }
        ...
        LOG_INFO(BSL_LS_BCM_OAM,
                 (BSL_META_U(unit,
                             "Freed smac idx %d - " L2_6B_MAC_FMT " -> %s\n"), 
    ...
    if (ALLOC_GET(ETEL2, egrPath->allocated)) {
        ...
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                                  "unable to clear ete[%08X]L2: %d (%s)\n"),
                       egrPath->eteL2Idx, rv, bcm_errmsg(rv)));
        }
    ...
    if (ALLOC_GET(ETEENCAP, egrPath->allocated)) {
        ...
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                                  "unable to clear ete[%08X]Encap: %d (%s)\n"),
                       egrPath->eteEncapIdx, rv, bcm_errmsg(rv)));
        }
...
int _oam_egr_path_dmac_set(int unit, egr_path_desc_t *egrPath, bcm_mac_t dmac, int dmaclsbset)
{
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "Failed to write eteEncap[0x%04x]: %d %s\n"),
                   egrPath->eteEncapIdx, rv, bcm_errmsg(rv)));
        return rv;
    }
...
int _oam_egr_path_smac_set(int unit, egr_path_desc_t *egrPath, bcm_mac_t smac)
{
    int rv = BCM_E_NONE;
    soc_sbx_g2p3_esmac_t esmac;
    uint32             esmacIdx = egrPath->eteL2.smacindex;
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "Failed to write esmac idx 0x%x \n"),
                   esmacIdx));
        return rv;
    }
                       ...
                       uint32 *idx)
{
    ...
    if (ep_info->flags & BCM_OAM_ENDPOINT_WITH_ID) {
        ...
        *idx = 0;
        ...
        if (BCM_FAILURE(rv)) {
            ...
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                                  "found null data at valid sw hash entry\n")));
        } 
    ...
    if (((ep_info->flags & BCM_OAM_ENDPOINT_UP_FACING) == 0) &&
         ...
         */
        if (OAM_IS_ETHERNET(ep_info->type)) {
            ep_info->pkt_pri = oamep.intpri & 0x1f;
            ep_info->int_pri = (oamep.intpri & 0xe0) >> 3;
        } else {
            ep_info->int_pri = oamep.intpri;
        }
    }
...
STATIC int
_oam_get_endpoint_by_timer(int unit, uint32 timer, uint32 *ep_rec_idx)
{
    int rv;
                        ...
                        int started, uint32 *id)
{
    ...
    if (cfg.ms < 1) {cfg.ms = 1;} /* 0 timer doesn't work anyway */

    cfg.started    = started; /* start before first ucode access */
    ...
    if (rv != BCM_E_NONE) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "Failed to insert timer_id=%d and ep_id=%d: %d %s\n"),
                   *id, ep_rec_index, rv, bcm_errmsg(rv)));
    }

    LOG_VERBOSE(BSL_LS_BCM_OAM,
                (BSL_META_U(unit,
                            "allocated timer: id=%d (0x%x) rate=%d started=%d ep=%d\n"),
                 *id, *id, (int)interval_ms, started, ep_rec_index));
...
int _oam_dump_ep(int unit, bcm_oam_endpoint_info_t *ep_info)
{
    ...
    LOG_INFO(BSL_LS_BCM_OAM,
             (BSL_META_U(unit,
                         "EP: flags=0x%08x type=%s\n"),
              ep_info->flags, 
              (OAM_IS_ETHERNET(ep_info->type) ? "enet" : "unknown")));
#endif /* defined(INCLUDE_BFD) */
    LOG_INFO(BSL_LS_BCM_OAM,
             (BSL_META_U(unit,
                         "  id=%d gport=0x%08x vid=0x%04x\n"),
              ep_info->id, 
...
void print_ep_idx(dq_t* ep_node, int* idx) 
{
    ...
    if (oam_sw != NULL) {
        if (idx == NULL) {
            LOG_CLI((BSL_META("%d"), oam_sw->rec_idx));
            return;
        }
...
_bcm_fe2000_oam_sw_dump(int unit)
{
    ...
    if (BCM_OAM_IS_INIT(unit) == FALSE) {
        LOG_CLI((BSL_META_U(unit,
                            "Oam sub system not initialized.\n")));
        return;
    }

    LOG_CLI((BSL_META_U(unit,
                        "Oam Internal SW state\n")));
    LOG_CLI((BSL_META_U(unit,
                        "max_pids=%d (0x%x), max_endpoints=%d (0x%0x) "
                        "timer_base=0x%x\n"),
    ...
    LOG_CLI((BSL_META_U(unit,
                        "ucode Types/Functions: bfd=%d lm=%d dm=%d ccm_first=%d "
                        "ccm_second=%d \n"),
...
/*
 *   Function
 *      bcm_fe2000_oam_endpoint_range_reserve
 *   Purpose
 ...
 */
int
bcm_fe2000_oam_endpoint_range_reserve(int unit, int highOrLow, uint32 val)
{
    ...
    if (!BCM_OAM_IS_INIT(unit)) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "OAM Module not initialized\n")));
        return BCM_E_INIT;
    }

    if (val > _state[unit]->max_endpoints) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "Invalid value for endpoint id \n")));
        return BCM_E_PARAM;
    }

    OAM_LOCK (unit);

    first = _state[unit]->ep_reservedLow;
    last = _state[unit]->ep_reservedHigh;

    /* Zero for any value, high or low, will clear the known range */
    if (val == 0) {
        clearIt = 1;
        _state[unit]->ep_reservedLow = 0;
        _state[unit]->ep_reservedHigh = 0;
    } else {

        if (highOrLow) {
            _state[unit]->ep_reservedHigh = val; 
        } else {
            _state[unit]->ep_reservedLow = val;
        }

        if ((_state[unit]->ep_reservedHigh != 0) && 
            (_state[unit]->ep_reservedLow != 0)) 
        {
            if (_state[unit]->ep_reservedHigh < 
                _state[unit]->ep_reservedLow) 
            {
                LOG_ERROR(BSL_LS_BCM_OAM,
                          (BSL_META_U(unit,
                                      "Upper bounds is set less than lower"
                                       " bounds: 0x%x < 0x%x\n"),
                           _state[unit]->ep_reservedHigh, 
                           _state[unit]->ep_reservedLow));
    ...
    if (reserveIt) {
        ...
        LOG_VERBOSE(BSL_LS_BCM_OAM,
                    (BSL_META_U(unit,
                                "Reserved oam endpoint ids: 0x%08x-0x%08x rv=%d %s\n"),
        ...
        if (first && last) {
            ...
            for (elt = first; elt <= last; elt++) {
                ignoreRv = shr_idxres_list_free(_state[unit]->ep_pool,
                                                elt);
...
/*
 *   Function
 *      bcm_fe2000_oam_endpoint_range_get
 *   Purpose
 ...
 *   Notes
 */
int bcm_fe2000_oam_endpoint_range_get(int unit, uint32 *low, uint32 *high)
{
    int rv = BCM_E_NONE;
    ...
    rv = shr_idxres_list_state(_state[unit]->ep_pool, &first, &last, 
...
/*
 *   Function
 *      bcm_fe2000_oam_group_range_reserve
 *   Purpose
 ...
 */
int
bcm_fe2000_oam_group_range_reserve(int unit, int highOrLow, uint32 val)
{
    ...
    if (val > _state[unit]->max_endpoints) {
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "Invalid value for group id \n")));
        return BCM_E_PARAM;
    }

    OAM_LOCK (unit);

    first = _state[unit]->grp_reservedLow;
    last = _state[unit]->grp_reservedHigh;

    /* Zero for any value, high or low, will clear the known range */
    if (val == 0) {
        clearIt = 1;
        _state[unit]->grp_reservedLow = 0;
        _state[unit]->grp_reservedHigh = 0;
    } else {

        if (highOrLow) {
            _state[unit]->grp_reservedHigh = val; 
        } else {
            _state[unit]->grp_reservedLow = val;
        }

        if ((_state[unit]->grp_reservedHigh != 0) && 
            (_state[unit]->grp_reservedLow != 0)) 
        {
            if (_state[unit]->grp_reservedHigh < 
                _state[unit]->grp_reservedLow) 
            {
                LOG_ERROR(BSL_LS_BCM_OAM,
                          (BSL_META_U(unit,
                                      "Upper bounds is set less than lower"
                                       " bounds: 0x%x < 0x%x\n"),
                           _state[unit]->grp_reservedHigh, 
                           _state[unit]->grp_reservedLow));
    ...
    if (reserveIt) {
        ...
        if (first && last) {
            ...
            for (elt = first; elt <= last; elt++) {
                ignoreRv = shr_idxres_list_free(_state[unit]->group_pool,
                                                elt);
...
/*
 *   Function
 *      bcm_fe2000_oam_group_range_get
 *   Purpose
 ...
 *   Notes
 */
int bcm_fe2000_oam_group_range_get(int unit, uint32 *low, uint32 *high)
{
    int rv = BCM_E_NONE;
    uint32 validLow, validHigh, freeCount, allocCount, first, last;
    ...
    rv = shr_idxres_list_state(_state[unit]->group_pool, &first, &last, 
                               &validLow, &validHigh, &freeCount, &allocCount);
...
int bcm_fe2000_oam_endpoint_action_set(
    int unit, bcm_oam_endpoint_t id, 
...
/*
 *   Function
 *      bcm_fe2000_oam_dump_reservation
 *   Purpose
 ...
 *   Notes
 */
void bcm_fe2000_oam_dump_reservation(int unit)
{
    int fw = 10;
    ...
    if (_state[unit]->grp_reservedLow || _state[unit]->grp_reservedHigh) {
        LOG_CLI((BSL_META_U(unit,
                            "  %-20s %-*s0x%08x %-*s0x%08x\n\n"), "RESERVATION:", fw, "",
                 _state[unit]->grp_reservedLow, fw, "",
                 _state[unit]->grp_reservedHigh));
    } else {           
        LOG_CLI((BSL_META_U(unit,
                            "***** No Reservations ******\n")));
    ...
    if (_state[unit]->ep_reservedLow || _state[unit]->ep_reservedHigh) {
        LOG_CLI((BSL_META_U(unit,
                            "  %-20s %-*s0x%08x %-*s0x%08x\n\n"), "RESERVATION:", fw, "",
                 _state[unit]->ep_reservedLow, fw, "",
                 _state[unit]->ep_reservedHigh));
    } else {           
        LOG_CLI((BSL_META_U(unit,
                            "***** No Reservations ******\n")));
...
STATIC int is_oam_group_id_app_reserved(int unit, int id) {
    if ((_state[unit]->grp_reservedHigh < id) ||
        (_state[unit]->grp_reservedLow  > id)) {
        /* Outside range */
        return 0;
    } else {
        return 1;
...
int is_oam_ep_id_app_reserved(int unit, int id) {
    if ((_state[unit]->ep_reservedHigh < id) ||
        (_state[unit]->ep_reservedLow  > id)) {
        /* Outside range */
        return 0;
    } else {
        return 1;
...
int _bcm_fe2000_oam_ep_check_and_reserve(int unit, int id) {
    int rv = BCM_E_NONE;

    rv = shr_idxres_list_elem_state(_state[unit]->ep_pool, id);
    if (rv == BCM_E_NOT_FOUND) {
        /* endpoint not in app reserved range,
         * reserve the requested endpoint-id */
        rv = shr_idxres_list_reserve(_state[unit]->ep_pool, id, id);
        if (rv != BCM_E_NONE) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                                  "Couldn't reserve Endpoint"
                                   " %d %d %s\n"), id, rv, bcm_errmsg(rv)));
        }
    } else if (rv == BCM_E_EXISTS) {
        if (!(is_oam_ep_id_app_reserved(unit, id))) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                                  "Endpoint %d already in use:"
                                   "%d %s\n"), id, rv, bcm_errmsg(rv)));
...
int _bcm_fe2000_oam_ep_check_and_free(int unit, int id) {

    int rv = BCM_E_NONE;

    if (!is_oam_ep_id_app_reserved(unit, id)) {
        rv = shr_idxres_list_free(_state[unit]->ep_pool, id);
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                                  "Failed to free oam endpoint id:%d %d %s\n"),
                       id, rv, bcm_errmsg(rv)));
        }
    }
    return rv;
