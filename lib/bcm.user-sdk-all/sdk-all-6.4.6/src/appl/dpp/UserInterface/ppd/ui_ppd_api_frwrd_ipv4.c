/* $Id: ui_ppd_api_frwrd_ipv4.c,v 1.19 Broadcom SDK $
...
{   
  ...
  unit = soc_ppd_get_default_unit(); 
  SOC_PPD_FRWRD_IPV4_GLBL_INFO_clear(&prm_glbl_info);
  ...
  { 
    ...
    UI_MACROS_CHECK_GET_VAL_OF_ERROR; 
    prm_glbl_info.router_info.mc_default_action.value.action_profile.trap_code = (SOC_PPD_TRAP_CODE)param_val->numeric_equivalent;
  } 
  */
  if(!search_param_val_pairs(current_line,&match_index,SOC_PARAM_PPD_FRWRD_IPV4_GLBL_INFO_SET_GLBL_INFO_SET_GLBL_INFO_ROUTER_INFO_MC_DEFAULT_ACTION_VALUE_ACTION_PROFILE_TRAP_CODE_LSB_ID,1)) 
  { 
    ...
    UI_MACROS_CHECK_GET_VAL_OF_ERROR; 
    prm_glbl_info.router_info.mc_default_action.value.action_profile.trap_code = (SOC_PPD_TRAP_CODE)param_val->value.ulong_value;
  } 
  ...
  { 
    ...
    UI_MACROS_CHECK_GET_VAL_OF_ERROR; 
    prm_glbl_info.router_info.uc_default_action.value.action_profile.trap_code = (SOC_PPD_TRAP_CODE)param_val->numeric_equivalent;
  } 
  */
  if(!search_param_val_pairs(current_line,&match_index,SOC_PARAM_PPD_FRWRD_IPV4_GLBL_INFO_SET_GLBL_INFO_SET_GLBL_INFO_ROUTER_INFO_UC_DEFAULT_ACTION_VALUE_ACTION_PROFILE_TRAP_CODE_LSB_ID,1)) 
  { 
    ...
    UI_MACROS_CHECK_GET_VAL_OF_ERROR; 
    prm_glbl_info.router_info.uc_default_action.value.action_profile.trap_code = (SOC_PPD_TRAP_CODE)param_val->value.ulong_value;
  } 
...
{   
  ...
  SOC_PPD_FRWRD_IPV4_GLBL_INFO_print(&prm_glbl_info);
...
{   
  ...
  unit = soc_ppd_get_default_unit(); 
  SOC_PPD_FRWRD_IPV4_UC_ROUTE_KEY_clear(&prm_route_key);
  ...
  if (count>1)
  {
      ...
      if (prm_success != SOC_SAND_SUCCESS)
      {
        cli_out("success: %s\n\r",soc_sand_SAND_SUCCESS_FAILURE_to_string(prm_success));
        cli_out("index: %u \n\r", i);
        SOC_PPD_FRWRD_IPV4_UC_ROUTE_KEY_print(&prm_route_key);
        cli_out("\n\r");
...
{   
  ...
  cli_out("route_status: %s\n\r",SOC_PPD_FRWRD_IP_ROUTE_STATUS_to_string(prm_route_status));

  cli_out("location: %s\n\r",SOC_PPD_FRWRD_IP_ROUTE_LOCATION_to_string(prm_location));
...
{   
  ...
  unit = soc_ppd_get_default_unit(); 
  SOC_PPD_IP_ROUTING_TABLE_RANGE_clear(&prm_block_range);
  prm_block_range.start.type = SOC_PPD_IP_ROUTING_TABLE_ITER_TYPE_PREFIX_IP_ORDERED;
		...
		for(i = 0; i < nof_entries_to_print; i++)
		{
			if (fec_id != -1 && fec_id == prm_fec_ids[i])
			{
				if (!count_only)
				{
					...
					cli_out("|%-13s", SOC_PPD_FRWRD_IP_ROUTE_STATUS_to_string(prm_routes_status[i]));
					cli_out("|%-36s|", SOC_PPD_FRWRD_IP_ROUTE_LOCATION_to_string(prm_routes_location[i]));
...
{   
  ...
  cli_out("route_status: %s\n\r",SOC_PPD_FRWRD_IP_CACHE_MODE_to_string(prm_route_types));
...
{   
  ...
  ret = soc_ppd_frwrd_ip_routes_cache_commit(
          unit,
          SOC_PPD_FRWRD_IP_CACHE_MODE_IPV4_UC_LPM,
          prm_vrf_ndx,
          &prm_success
        );
  if (soc_sand_get_error_code_from_error_word(ret) != SOC_SAND_OK) 
...
{   
  ...
  unit = soc_ppd_get_default_unit(); 
  SOC_PPD_FRWRD_IPV4_HOST_KEY_clear(&prm_host_key);
  SOC_PPD_FRWRD_IPV4_HOST_ROUTE_INFO_clear(&prm_routing_info);
  ...
  if (count>1)
  {
      ...
      if (prm_routing_info.eep != 0xffffffff)
      {
        ...
        if (prm_success != SOC_SAND_SUCCESS)
        {
          cli_out("success: %s\n\r",soc_sand_SAND_SUCCESS_FAILURE_to_string(prm_success));
          cli_out("index: %u \n\r", i);
          SOC_PPD_FRWRD_IPV4_HOST_KEY_print(&prm_host_key);
          cli_out("\n\r");
          SOC_PPD_FRWRD_IPV4_HOST_ROUTE_INFO_print(&prm_routing_info);
          cli_out("\n\r");
          goto exit;
        }
      }
...
{   
  ...
  ret = soc_ppd_frwrd_ipv4_host_get(
          unit,
          &prm_host_key,
          SOC_PPD_FRWRD_IP_HOST_CLEAR_ON_GET,
          &prm_routing_info,
          &prm_route_status,
          &prm_location,
          &prm_found
...
{   
  ...
  uint32
	...
	fec_id = -1,
	count_only = 0,
    flags=SOC_PPD_FRWRD_IP_HOST_GET_ACCESSS_STATUS,
	print_count = 0;
  uint8
    done = FALSE,
	printed_once = FALSE;
  ...
  {  
      ...
      UI_MACROS_CHECK_GET_VAL_OF_ERROR;   
      flags |= SOC_PPD_FRWRD_IP_HOST_GET_ACCESSED_ONLY;
  } 
  ...
  {  
      ...
      UI_MACROS_CHECK_GET_VAL_OF_ERROR;   
      flags |= SOC_PPD_FRWRD_IP_HOST_CLEAR_ON_GET;
  } 
    ...
    for (indx = 0; indx < prm_nof_entries; ++indx)
    {
		if ((eep == -1 || eep == prm_routes_info[indx].eep) && (fec_id == -1 || fec_id == prm_routes_info[indx].fec_id))
		{
			...
			if (!count_only)
			{
                ...
                if (prm_routes_status[indx] & SOC_PPD_FRWRD_IP_ROUTE_STATUS_ACCESSED) {
                    cli_out("| V ");
                }
                else{
                    cli_out("| X ");
...
{   
  ...
  {  
    ...
    switch (key.type) {
        ...
        case ARAD_PP_LEM_ACCESS_KEY_TYPE_COUPLING_ILM:
          prefix = ARAD_PP_LEM_ACCESS_KEY_TYPE_PREFIX_COUPLING_ILM;
        break;
  ...
  {  
      UI_MACROS_GET_NUMMERIC_VAL(SOC_PARAM_PPD_FRWRD_IPV4_HOST_GET_BLOCK_HOST_GET_BLOCK_ID_RULE_KEY_ACCESSED_ONLY);  
      UI_MACROS_CHECK_GET_VAL_OF_ERROR;   
      access_only = SOC_PPD_FRWRD_IP_HOST_GET_ACCESSED_ONLY;
  } 
  ...
  {  
      UI_MACROS_GET_NUMMERIC_VAL(SOC_PARAM_PPD_FRWRD_IPV4_HOST_GET_BLOCK_HOST_GET_BLOCK_ID_ACCESSED_CLEAR_ID);  
      UI_MACROS_CHECK_GET_VAL_OF_ERROR;   
      access_only |= SOC_PPD_FRWRD_IP_HOST_CLEAR_ON_GET;
  } 
...
{   
  ...
  unit = soc_ppd_get_default_unit(); 
  SOC_PPD_FRWRD_IPV4_MC_ROUTE_KEY_clear(&prm_route_key);
  SOC_PPD_FRWRD_IPV4_MC_ROUTE_INFO_clear(&prm_route_info);
...
{   
  ...
  { 
    ...
    UI_MACROS_CHECK_GET_VAL_OF_ERROR; 
    prm_exact_match = ((uint8)param_val->value.ulong_value == 0x1)?SOC_PPD_FRWRD_IP_EXACT_MATCH:0;
  } 
  ...
  { 
    UI_MACROS_GET_NUMMERIC_VAL(SOC_PARAM_PPD_FRWRD_IPV4_MC_ROUTE_GET_MC_ROUTE_GET_EXACT_MATCH_ID);  
    UI_MACROS_CHECK_GET_VAL_OF_ERROR; 
    prm_exact_match |= ((uint8)param_val->value.ulong_value == 0x1)?SOC_PPD_FRWRD_IP_CLEAR_ON_GET:0;
  } 
  ...
  SOC_PPD_FRWRD_IPV4_MC_ROUTE_INFO_print(&prm_route_info);
  ...
  */
  ...
  while(!SOC_PPC_IP_ROUTING_TABLE_ITER_IS_END(&prm_block_range.start.payload))
  {
    ...
    for(i = 0; i < prm_nof_entries; i++)
    {
      ...
      cli_out("|%-9s", SOC_PPD_FRWRD_IP_ROUTE_STATUS_to_string(prm_routes_status[i]));
      cli_out("|%-13s|", SOC_PPD_FRWRD_IP_ROUTE_LOCATION_to_string(prm_routes_location[i]));
...
{   
  ...
  unit = soc_ppd_get_default_unit(); 
  SOC_PPD_FRWRD_IPV4_VRF_INFO_clear(&prm_vrf_info);
  ...
  { 
    ...
    UI_MACROS_CHECK_GET_VAL_OF_ERROR; 
    prm_vrf_info.router_info.mc_default_action.value.action_profile.trap_code = (SOC_PPD_TRAP_CODE)param_val->numeric_equivalent;
  } 
  */
  if(!search_param_val_pairs(current_line,&match_index,SOC_PARAM_PPD_FRWRD_IPV4_VRF_INFO_SET_VRF_INFO_SET_VRF_INFO_ROUTER_INFO_MC_DEFAULT_ACTION_VALUE_ACTION_PROFILE_TRAP_CODE_LSB_ID,1)) 
  { 
    ...
    UI_MACROS_CHECK_GET_VAL_OF_ERROR; 
    prm_vrf_info.router_info.mc_default_action.value.action_profile.trap_code = (SOC_PPD_TRAP_CODE)param_val->value.ulong_value;
  } 
  ...
  { 
    ...
    UI_MACROS_CHECK_GET_VAL_OF_ERROR; 
    prm_vrf_info.router_info.uc_default_action.value.action_profile.trap_code = (SOC_PPD_TRAP_CODE)param_val->numeric_equivalent;
  } 
  */
  if(!search_param_val_pairs(current_line,&match_index,SOC_PARAM_PPD_FRWRD_IPV4_VRF_INFO_SET_VRF_INFO_SET_VRF_INFO_ROUTER_INFO_UC_DEFAULT_ACTION_VALUE_ACTION_PROFILE_TRAP_CODE_LSB_ID,1)) 
  { 
    ...
    UI_MACROS_CHECK_GET_VAL_OF_ERROR; 
    prm_vrf_info.router_info.uc_default_action.value.action_profile.trap_code = (SOC_PPD_TRAP_CODE)param_val->value.ulong_value;
  } 
...
{   
  ...
  SOC_PPD_FRWRD_IPV4_VRF_INFO_print(&prm_vrf_info);
...
{   
  ...
  unit = soc_ppd_get_default_unit(); 
  SOC_PPD_FRWRD_IPV4_VPN_ROUTE_KEY_clear(&prm_route_key);
  ...
  if (count>1)
  {
      ...
      if (prm_success != SOC_SAND_SUCCESS)
      {
        cli_out("success: %s\n\r",soc_sand_SAND_SUCCESS_FAILURE_to_string(prm_success));
        cli_out("index: %u \n\r", i);
        SOC_PPD_FRWRD_IPV4_VPN_ROUTE_KEY_print(&prm_route_key);
        cli_out("\n\r");
        cli_out("prm_fec_id: %u \n\r", prm_fec_id);
        goto exit;
      }
    ...
    for(i = 0; i < prm_nof_entries; i++)
    {
		if (fec_id == -1 || fec_id == prm_fec_ids[i])
		{
			if (!count_only)
			{
				...
				cli_out("|%-13s", SOC_PPD_FRWRD_IP_ROUTE_STATUS_to_string(prm_routes_status[i]));
				cli_out("|%-30s|", SOC_PPD_FRWRD_IP_ROUTE_LOCATION_to_string(prm_routes_location[i]));
...
{   
  uint32 
    ret;   
  uint32 
    flags= 0; /* SOC_PPD_FRWRD_IP_HOST_ONLY | SOC_PPD_FRWRD_IP_LPM_ONLY;   */
  ...
  {  
      flags = SOC_PPD_FRWRD_IP_LPM_ONLY;
  }
  ...
  {  
      if (flags == SOC_PPD_FRWRD_IP_LPM_ONLY) {
          cli_out("only one can be setl, lpm_only or host_only  \n\r");
          goto exit;
      }
      flags = SOC_PPD_FRWRD_IP_HOST_ONLY;
  }
...
{   
  ...
  unit = soc_ppd_get_default_unit(); 
  SOC_PPD_FRWRD_IPV4_MEM_STATUS_clear(&prm_mem_status);
  ...
  SOC_PPD_FRWRD_IPV4_MEM_STATUS_print(&prm_mem_status);
...
{   
  ...
  uint32   
    prm_mem_ndx;
  SOC_PPD_FRWRD_IPV4_MEM_DEFRAG_INFO   
    prm_defrag_info;
  ...
  unit = soc_ppd_get_default_unit(); 
  SOC_PPD_FRWRD_IPV4_MEM_DEFRAG_INFO_clear(&prm_defrag_info);
