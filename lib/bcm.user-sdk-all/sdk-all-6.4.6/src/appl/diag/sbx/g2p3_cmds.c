/*
 * $Id: g2p3_cmds.c,v 1.45 Broadcom SDK $
...
cmd_sbx_g2p3_get (int unit, args_t *args)
{
    ...
    if (sbx->ucodetype != SOC_SBX_UCODE_TYPE_G2P3) {
        cli_out("ERROR: not running gu2 v1.3 microcode\n");
        return CMD_FAIL;
    }
...
rem_entries_1:
  cli_out("adding %d entries using soc_sbx_g2p3_pvv2e_add took %dus\n", added, SAL_USECS_SUB(sal_time_usecs(), start));

  cli_out("Removing entries ...\n");
  ovid = ivid = 1;
...
rem_entries_2:
  cli_out("adding %d entries using soc_sbx_g2p3_pvv2e_set took %dus\n", added, SAL_USECS_SUB(sal_time_usecs(), start));
...
int bcm_acl_dma_add_thread(int feUnit, bcm_field_group_t group, uint32 entryStart, uint32 entryEnd);
int bcm_acl_dma_del_thread(int feUnit, bcm_field_group_t gid, uint32 entryStart, uint32 entryEnd);
void acl_test_loop();
uint32 gTick = 5;
uint32 entryCur = 0;
uint32 ifShutdown = 0;
...
void acl_test_loop()
{
  ...
  cli_out("ACL test thread finished.\n");
}
...
"  g2p3util reservation            - displays G2P3 SDK table reservations\n"
#endif
;
...
extern int soc_sbx_g2p3_allocator_reservation_shell_print(int unit);
...
cmd_sbx_g2p3_util(int unit, args_t *a)
{
    ...
    if (sal_strcasecmp(subcmd, "help") == 0) {
      ...
      cmd_sbx_g2p3_svid_cap(unit, NULL);
        ...
        if (parseEndOk(a, &pt, &rv)) {
            soc_sbx_g2p3_pvv2e_keys_get(unit, ivid, ovid, port,
                                        &left, &right, &seed, &b);
            cli_out("pvv2e left key = 0x%x, ", left);
            cli_out("right key = 0x%x, ", right);
            cli_out("seed = 0x%x, ", seed);
            cli_out(" bank: %s\n", b ? "b" : "a");
        }
        ...
        uint32 uSecTmp = 0, factorTmp = 0, factorTmp2;
        uint64 count;
        uint64 fcount1;
        uint64 fcount2;
        uint64 fcountTmp;
        ...
        if (parseEndOk(a, &pt, &rv)) {
            ...
            COMPILER_64_SET(count,0,vids);
            COMPILER_64_UMUL_32(count, 100000);
            COMPILER_64_UMUL_32(count, passes);
            fcount1 = count;
            fcount2 = count;
            COMPILER_64_UMUL_32(fcount1, factor1);
            COMPILER_64_UMUL_32(fcount2, factor2);
            ...
            if (!fastonly) {
                ...
                for (pass = 0; pass < passes; pass++) {
                    for (i = svid; i <= evid; i++) {
                        ...
                        if (s != SOC_E_NONE) {
                            cli_out("ERROR: pv2e get port %d, vid %d failed %d: "
                                    "%s\n",
                ...
                if (soc_sbx_div64(count, iget10usec, &uSecTmp) == SOC_E_PARAM) {
                    sal_free(pv2es);
                    sal_free(epv2es);
                    sal_free(istates);
                    sal_free(edrops);
                ...
                cli_out("Got %d pv2es in %u microseconds (%d/sec)\n",
                        vids * passes,
                        igetusec,
                        iget10usec ? uSecTmp : -1);
                ...
                for (pass = 0; pass < passes; pass++) {
                    for (i = svid; i <= evid; i++) {
                        s = soc_sbx_g2p3_epv2e_get(unit, i, port,
                                                   &epv2es[i - svid]);
                        if (s != SOC_E_NONE) {
                            cli_out("ERROR: epv2e get port %d, vid %d failed %d: "
                                    "%s\n",
                                    port, i, s, bcm_errmsg(s));
                            sal_free(pv2es);
                            sal_free(epv2es);
                ...
                if (soc_sbx_div64(count,eget10usec, &factorTmp) == SOC_E_PARAM) {
                    sal_free(pv2es);
                    sal_free(epv2es);
                    sal_free(istates);
                    sal_free(edrops);
                ...
                cli_out("Got %d epv2es in %u microseconds (%d/sec)\n",
                        vids * passes,
                        egetusec,
                        eget10usec ? factorTmp : -1);
                if (soc_sbx_div64(count,(iget10usec + eget10usec), &factorTmp) == SOC_E_PARAM) {
                    sal_free(pv2es);
                    sal_free(epv2es);
                    sal_free(istates);
                    sal_free(edrops);
                ...
                cli_out("Got %d pv2es & epv2es in %u microseconds (%d/sec)\n",
                        vids * passes,
                        igetusec + egetusec,
                        iget10usec + eget10usec ? factorTmp : -1);
                ...
                for (pass = 0; pass < passes; pass++) {
                    for (i = svid; i <= evid; i++) {
                        ...
                        if (s != SOC_E_NONE) {
                            cli_out("ERROR: pv2e set port %d, vid %d failed %d: "
                                    "%s\n",
                                    port, i, s, bcm_errmsg(s));
                            sal_free(pv2es);
                            sal_free(epv2es);
                ...
                if (soc_sbx_div64(count,iset10usec, &factorTmp) == SOC_E_PARAM) {
                    sal_free(pv2es);
                    sal_free(epv2es);
                    sal_free(istates);
                    sal_free(edrops);
                ...
                cli_out("Set %d pv2es in %u microseconds (%d/sec)\n",
                        vids * passes,
                        isetusec,
                        iset10usec ? factorTmp : -1);
                if (soc_sbx_div64(count,(iget10usec + iset10usec), &factorTmp) == SOC_E_PARAM) {
                    sal_free(pv2es);
                    sal_free(epv2es);
                    sal_free(istates);
                    sal_free(edrops);
                ...
                cli_out("Got/set %d pv2es in %u microseconds (%d/sec)\n",
                        vids * passes,
                        igetusec + isetusec,
                        iget10usec + iset10usec ? factorTmp : -1);
                st = sal_time_usecs();
                for (pass = 0; pass < passes; pass++) {
                    for (i = svid; i <= evid; i++) {
                        s = soc_sbx_g2p3_epv2e_set(unit, i, port,
                                                   &epv2es[i - svid]);
                        if (s != SOC_E_NONE) {
                            cli_out("ERROR: epv2e set port %d, vid %d failed %d: "
                                    "%s\n",
                                    port, i, s, bcm_errmsg(s));
                            sal_free(pv2es);
                            sal_free(epv2es);
                ...
                if (soc_sbx_div64(count,eset10usec, &factorTmp) == SOC_E_PARAM) {
                    sal_free(pv2es);
                    sal_free(epv2es);
                    sal_free(istates);
                    sal_free(edrops);
                ...
                cli_out("Set %d epv2es in %u microseconds (%d/sec)\n",
                        vids * passes,
                        esetusec,
                        eset10usec ? factorTmp : -1);
                if (soc_sbx_div64(count,(eget10usec + eset10usec), &factorTmp) == SOC_E_PARAM) {
                    sal_free(pv2es);
                    sal_free(epv2es);
                    sal_free(istates);
                    sal_free(edrops);
                ...
                cli_out("Got/set %d epv2es in %u microseconds (%d/sec)\n",
                        vids * passes,
                        egetusec + esetusec,
                        eget10usec + eset10usec ? factorTmp : -1);
                if (soc_sbx_div64(count,(iset10usec + eset10usec), &factorTmp) == SOC_E_PARAM) {
                    sal_free(pv2es);
                    sal_free(epv2es);
                    sal_free(istates);
                    sal_free(edrops);
                ...
                cli_out("Set %d pv2es & epv2es in %u microseconds (%d/sec)\n",
                        vids * passes,
                        isetusec + esetusec,
                        iset10usec + eset10usec ? factorTmp : -1);
            }
            ...
            for (pass = 0; pass < passes * factor1; pass++) {
                s = soc_sbx_g2p3_pv2e_stpstate_fast_set(unit, svid, port,
                                                       evid, port, NULL,
                                                        istates, vids);
                if (s != SOC_E_NONE) {
                    cli_out("ERROR: pv2e stpstate fast set failed %d: %s\n",
                            s, bcm_errmsg(s));
                            sal_free(pv2es);
                            sal_free(epv2es);
                            sal_free(istates);
            ...
            if (soc_sbx_div64(fcount1,ifast10usec, &factorTmp) == SOC_E_PARAM) {
                    sal_free(pv2es);
                    sal_free(epv2es);
                    sal_free(istates);
                    sal_free(edrops);
            ...
            cli_out("Fast set %d pv2e.stpstates in %u microseconds (%d/sec)\n",
                    vids * passes * factor1,
                    ifastusec,
                    ifast10usec ? factorTmp : -1);

            if (!fastonly) {
                cli_out("Fast pv2e.stpstate set gives %dx original performance\n",
            ...
            for (pass = 0; pass < passes * factor2; pass++) {
                s = soc_sbx_g2p3_epv2e_drop_fast_set(unit, svid, port,
                                                     ...
                                                     edrops, vids);
                if (s != SOC_E_NONE) {
                    cli_out("ERROR: epv2e stpdrop fast set failed %d: %s\n",
                            s, bcm_errmsg(s));
                            sal_free(pv2es);
                            sal_free(epv2es);
                            sal_free(istates);
            ...
            if (soc_sbx_div64(fcount2,efast10usec, &factorTmp) == SOC_E_PARAM) {
                    sal_free(pv2es);
                    sal_free(epv2es);
                    sal_free(istates);
                    sal_free(edrops);
            ...
            cli_out("Fast set %d epv2e.stpdrops in %u microseconds (%d/sec)\n",
                    vids * passes * factor2,
                    efastusec,
                    efast10usec ? factorTmp : -1);

            if (!fastonly) {
                cli_out("Fast set epv2e.stpdrops gives %dx original performance\n\n",
                        efastusec ? (egetusec + esetusec) / (efastusec / factor2) : -1);

                if (soc_sbx_div64(count,(iget10usec + iget10usec + eget10usec + eset10usec), &factorTmp) == SOC_E_PARAM) {
                    sal_free(pv2es);
                    sal_free(epv2es);
                    sal_free(istates);
                    sal_free(edrops);
                ...
                cli_out("Normal update %d in %u microseconds (%d/sec)\n",
                        vids * passes,
                        igetusec + igetusec + egetusec + esetusec,
                        iget10usec + iget10usec + eget10usec + eset10usec ? factorTmp : -1);
            }

            COMPILER_64_SET(fcountTmp, 0, efastusec);
            COMPILER_64_UMUL_32(fcountTmp, factor1);
            if (soc_sbx_div64(fcountTmp, factor2, &factorTmp2) == SOC_E_PARAM) {
                    sal_free(pv2es);
                    sal_free(epv2es);
                    sal_free(istates);
                    sal_free(edrops);
            ...
            COMPILER_64_SET(fcountTmp, 0, efast10usec);
            COMPILER_64_UMUL_32(fcountTmp, factor1);
            if (soc_sbx_div64(fcountTmp, factor2, &factorTmp) == -1) {
                    sal_free(pv2es);
                    sal_free(epv2es);
                    sal_free(istates);
                    sal_free(edrops);
            ...
            if (soc_sbx_div64(fcount1, (ifast10usec + factorTmp), &uSecTmp) == SOC_E_PARAM) {
                    sal_free(pv2es);
                    sal_free(epv2es);
                    sal_free(istates);
                    sal_free(edrops);
            ...
            cli_out("Fast update %d in %u microseconds (%d/sec)\n",
                    vids * passes * factor1,
                    ifastusec + factorTmp2,
                    ifast10usec + efast10usec ? uSecTmp : -1);

            if (!fastonly) {
                cli_out("Fast update provides %dx original performance\n",
        ...
        if (parseEndOk(a, &pt, &rv)) {
            ...
            if (BCM_E_NONE == result) {
                ...
                if (-1 == qeUnit) {
                    ...
                    cli_out("there must be at least one port in the vid!\n");
                    return CMD_FAIL;
                }
                for (pass = 0; pass < passes; pass++) {
                    cli_out("pass %d of %d\n", pass, passes);
                    for (group = 0; group < groups; group++) {
                        ...
                        if (BCM_E_NONE == result) {
                            for (addr = 0;
                                ...
                                if (BCM_E_NONE != result) {
                                    cli_out("unable to add multicast group %d address %02X:%06X to unit %d: %d (%s)\n",
                                            groupId,
                                            group,
                                            addr,
                                            unit,
                            ...
                            while (addr > 0) {
                                ...
                                if (BCM_E_NONE != result) {
                                    cli_out("unable to remove multicast group %d address %02X:%06X from unit %d: %d (%s)\n",
                                            groupId,
                                            group,
                                            addr,
                                            unit,
                            ...
                            if (BCM_E_NONE != result) {
                                cli_out("unable to destroy multicast group %d on QE unit %d: %d (%s)\n",
                                        groupId,
                                        unit,
                                        result,
                                        _SHR_ERRMSG(result));
                            ...
                            cli_out("unable to create multicast group on QE unit %d: %d (%s)\n",
                                    qeUnit,
                                    result,
                                    _SHR_ERRMSG(result));
                        }
                ...
                cli_out("unable to get VLAN %03X ports: %d (%s)\n",
                        vid,
                        result,
                        _SHR_ERRMSG(result));
            }
      ...
      if (!sal_strncasecmp(pArg, "ingress", 7)){
          ...
          cli_out("Unrecognized Rule Database\n");
          return CMD_USAGE;
      }
      
      if ((pArg = ARG_GET(a)) == NULL) {
          cli_out("Missing index\n");
          return CMD_USAGE;
      }
      ...
      if ( rv != SOC_E_NONE ) {
          if (rv == SOC_E_NOT_FOUND) {
              cli_out("Entry 0x%x not found\n", index);
          }else{
              cli_out("Get failed: 0x%x \n", rv);
          }
          return rv;
      }
      /* print rule handle */
	...
	return CMD_USAGE;
      ...
      if (c_file) {
        ...
        while((ch = (char)fgetc(rf))!='}') {
          ...
          if (((ch=='{') || (ch==',')) && (size_found)) {
              /* coverity[secure_coding] */
              err = fscanf(rf,"%x", &rvalue);
              if (err < 0) {
                 printf("fscanf Read cfile failed!\n");
    ...
    } else if (!sal_strcasecmp(subcmd, "reservation")) {
        return soc_sbx_g2p3_allocator_reservation_shell_print(unit);
	...
	cli_out("ACL test thread pointer: %u\n", (uint32)acl_test_thread_id);
      } else {
	cli_out("ACLs added: %d\n", acl_test_count);
      }
#endif
    } else {
        return CMD_USAGE;
