/*
 * $Id: fe2000_cmds.c,v 1.87 Broadcom SDK $
...
static int
_fe2000_rx_port_data_read(sbhandle sbh, uint32 uQueueId,
                   sbZfFe2000PpRxPortDataEntry_t *pRxPortData)
{
  /* Note that this zframe is identical for CA/C2 */
  uint32 uBuffer[SB_ZF_FE2000_PP_RX_PORT_DATA_ENTRY_SIZE_IN_WORDS];
  ...
  sbZfFe2000PpRxPortDataEntry_Unpack(pRxPortData, (uint8 *) &uBuffer[0],
                             SB_ZF_FE2000_PP_RX_PORT_DATA_ENTRY_SIZE_IN_BYTES);
  return FALSE;
}

static int
_fe2000xt_rx_port_data_read(sbhandle sbh, uint32 uQueueId,
                   sbZfC2PpRxPortDataEntry_t *pRxPortData)
{
  /* Note that this zframe is identical for CA/C2 */
  uint32 uBuffer[(SB_ZF_C2PPRXPORTDATAENTRY_SIZE_IN_BYTES+3)/4];
  ...
  sbZfC2PpRxPortDataEntry_Unpack(pRxPortData, (uint8 *) &uBuffer[0],
                             SB_ZF_C2PPRXPORTDATAENTRY_SIZE_IN_BYTES);
  return FALSE;
}

static int
_fe2000_queue_priority_group_read(sbhandle sbh, uint32 uQueueId,
                           sbZfFe2000PpQueuePriorityGroupEntry_t *qpg)
{
  ...
  sbZfFe2000PpQueuePriorityGroupEntry_Unpack(qpg, (uint8 *) &uBuffer[0], 4);
  return FALSE;
}

static int
_fe2000xt_queue_priority_group_read(sbhandle sbh, uint32 uQueueId,
                           sbZfC2PpQueuePriorityGroupEntry_t *qpg)
{
  ...
  sbZfC2PpQueuePriorityGroupEntry_Unpack(qpg, (uint8 *) &uBuffer[0], 4);
  return FALSE;
}
  ...
  if (!SOC_IS_SBX_FE2KXT(unit)){
      ...
      for (i = SB_FE2000_IF_PTYPE_SPI0; i < SB_FE2000_IF_PTYPE_MAX; i++) {
        for (j = 0; j < _ports_per_iface[i]; j++) {
          ...
          if (qn == queue) {
            cli_out("is fed by %s.%d ", _iface_names[i], j);
          }
        }
      }
      for (i = SB_FE2000_IF_PTYPE_SPI0; i < SB_FE2000_IF_PTYPE_MAX; i++) {
        for (j = 0; j < _ports_per_iface[i]; j++) {
          ...
          if (qn == queue) {
            cli_out("feeds %s.%d ", _iface_names[i], j);
          }
        }
      }
      cli_out("\n");
...
cmd_sbx_fe2000_sep(int unit, args_t *a)
{
    ...
    if (err != SOC_E_NONE) {
        cli_out("soc_sbx_fe2000_egress_pages_set failed: %d\n", err);
        rv = CMD_FAIL;
    }
    return rv;
}
...
cmd_sbx_g2k_util(int unit, args_t *a)
{
    ...
    if (sal_strcasecmp(subcmd, "ppearm") == 0) {
      if (SOC_IS_SBX_FE(unit)) {
          if ((subcmd = ARG_GET(a)) == NULL) {
              cli_out("Missing Queue Id for ppearm\n");
              return CMD_USAGE;
          }
          ...
          cli_out("PPE Armed to capture on queue %d\n", sbFeQueue);
          rv = CMD_OK;
      } else {
          cli_out("PPE Arm command valid only on FE\n");
          rv = CMD_FAIL;
      }
    } else if (sal_strcasecmp(subcmd, "ppedump") == 0) {
      if (SOC_IS_SBX_FE(unit)) {
          if (sbFeQueue == -1) {
              cli_out("Run this command after arming PPE with a valid source queue\n");
              return CMD_FAIL;
          }
          ...
          if (SAND_HAL_FE2000_GET_FIELD(sbh, PP_DEBUG_PARSING_CAM_CTRL, TRACE_CAPTURED,
              ...
              cli_out("No PPE CAM trace captured\n");
              return CMD_OK;
          }
      } else {
          cli_out("PPE Dump command valid only on FE\n");
          rv = CMD_USAGE;
      }
        ...
        sbStatus_t sts;
        if ((arg = ARG_GET(a)) == NULL) {
            cli_out("Missing stream for get instruction\n");
            return CMD_USAGE;
        }
        stream = sal_ctoi(arg, 0);
        if ((arg = ARG_GET(a)) == NULL) {
            cli_out("Missing instruction for get instruction\n");
            return CMD_USAGE;
        }
        ...
        for (i = startpc; i <= endpc; i++) {
            ...
            if (sts == SB_BAD_ARGUMENT_ERR_CODE) {
                cli_out("illegal stream (%d) or pc (%d)\n", stream, i);
                return CMD_USAGE;
            }
            if (sts != SB_OK) {
                cli_out("get instruction %d:%d failed: %d\n", stream, i, sts);
                return CMD_FAIL;
            }
            thread = (data2 >> 29) & 0x7;
            ...
            regb = (data2 >> 18) & 1;
            tar = data2 & 0x3ffff;
            cli_out("%d:%d %s %d. op=0x%x, tar=0x%x, srca=0x%x%s, "
                    "srcb=0x%x%s\n",
                    stream, i, task ? "<" : ">", thread, opcode,
                    tar,
                    data1, rega ? "r" : "",
                    data0 >> 16, regb ? "r" : "");
            cli_out("          0x%08x %08x %08x\n", data2, data1, data0);
        }
      ...
      if (SOC_IS_SBX_FE(unit)) {
        ...
        cli_out("IFACE   (uPort)---> QID  ---> QID  --->   IFACE\n");
        for (i=0; i < queues->n; i++) {
          ...
          if (!c->bEgress){
              cli_out("%c%4s.%-2d(0x%02x) ---> 0x%02x ---> 0x%02x ---> %c%4s.%-2d\n",
          ...
          cli_out("swsdump command valid only on FE\n");
          rv = CMD_USAGE;
      }
      ...
      if (SOC_IS_SBX_FE(unit)) {
          ...
          int32 destport, qid;
          if (!SOC_IS_SBX_FE2KXT(unit)){
              cli_out("CC block mapping only on FE2KXT\n");
          }else{
              for (i=SB_FE2000_IF_PTYPE_AGM0; i<=SB_FE2000_IF_PTYPE_XGM1; i++) {
                ...
                while ( sbFe2000XtCcPort2QidMapGet((sbhandle)unit, i, e, &destport, &qid) == SB_OK) {
                    cli_out("Iface: 0x%x (%4s) Entry: 0x%x ERHDestPort: 0x%02x ---> qid: 0x%02x\n", i, _iface_names[i], e, destport, qid);
                    e++;
                }
              }
          }
      }else{
          cli_out("ccdump command valid only on FE\n");
          rv = CMD_USAGE;
      }
      ...
      if (SOC_IS_SBX_FE(unit)) {
          ...
          uint32 port, qid;
          if ((arg = ARG_GET(a)) == NULL) {
              cli_out("Missing iface info for ccmod command\n");
              return CMD_USAGE;
          }
          iface = sal_ctoi(arg, 0);
          if ((arg = ARG_GET(a)) == NULL) {
              cli_out("Missing entry info for ccmod command\n");
              return CMD_USAGE;
          }
          entry = sal_ctoi(arg, 0);
          if ((arg = ARG_GET(a)) == NULL) {
              cli_out("Missing port info for ccmod command\n");
              return CMD_USAGE;
          }
          port = sal_ctoi(arg, 0);
          if ((arg = ARG_GET(a)) == NULL) {
              cli_out("Missing qid info for ccmod command\n");
              return CMD_USAGE;
          }
          qid = sal_ctoi(arg, 0);
          if (!SOC_IS_SBX_FE2KXT(unit)){
          ...
          cli_out("ccmod command valid only on FE\n");
          rv = CMD_USAGE;
      }
...
cmd_result_t
cmd_sbx_fe2000_hc(int unit, int queue, int on)
{
    sbhandle sbh;
...
ppe_print(sbZfFe2000PpeDescriptor_t *pFromStruct) {
  cli_out("Fe2000PpeDescriptor:: contbyte=0x%02x", (unsigned int)  pFromStruct->m_uContinueByte);
  cli_out(" hdr0=0x%01x", (unsigned int)  pFromStruct->m_uHeaderType0);
  cli_out(" hdr1=0x%01x", (unsigned int)  pFromStruct->m_uHeaderType1);
  cli_out(" hdr2=0x%01x", (unsigned int)  pFromStruct->m_uHeaderType2);
  cli_out(" hdr3=0x%01x", (unsigned int)  pFromStruct->m_uHeaderType3);
  cli_out("\n");

  cli_out("Fe2000PpeDescriptor:: hdr4=0x%01x", (unsigned int)  pFromStruct->m_uHeaderType4);
  cli_out(" hdr5=0x%01x", (unsigned int)  pFromStruct->m_uHeaderType5);
  cli_out(" hdr6=0x%01x", (unsigned int)  pFromStruct->m_uHeaderType6);
  cli_out(" hdr7=0x%01x", (unsigned int)  pFromStruct->m_uHeaderType7);
  cli_out(" hdr8=0x%01x", (unsigned int)  pFromStruct->m_uHeaderType8);
  cli_out(" hdr9=0x%01x", (unsigned int)  pFromStruct->m_uHeaderType9);
  cli_out("\n");

  cli_out("Fe2000PpeDescriptor:: locn0=0x%02x", (unsigned int)  pFromStruct->m_uLocation0);
  cli_out(" locn1=0x%02x", (unsigned int)  pFromStruct->m_uLocation1);
  cli_out(" locn2=0x%02x", (unsigned int)  pFromStruct->m_uLocation2);
  cli_out(" locn3=0x%02x", (unsigned int)  pFromStruct->m_uLocation3);
  cli_out("\n");

  cli_out("Fe2000PpeDescriptor:: locn4=0x%02x",
          (unsigned int)  pFromStruct->m_uLocation4);
  cli_out(" locn5=0x%02x", (unsigned int)  pFromStruct->m_uLocation5);
  cli_out(" locn6=0x%02x", (unsigned int)  pFromStruct->m_uLocation6);
  cli_out(" locn7=0x%02x", (unsigned int)  pFromStruct->m_uLocation7);
  cli_out("\n");

  cli_out("Fe2000PpeDescriptor:: locn8=0x%02x",
          (unsigned int)  pFromStruct->m_uLocation8);
  cli_out(" locn9=0x%02x", (unsigned int)  pFromStruct->m_uLocation9);
  cli_out(" str=0x%01x", (unsigned int)  pFromStruct->m_uStreamSelector);
  cli_out(" buffer=0x%01x", (unsigned int)  pFromStruct->m_uBuffer);
  cli_out(" expidx=0x%02x", (unsigned int)  pFromStruct->m_uExceptIndex);
  cli_out("\n");

  cli_out("Fe2000PpeDescriptor:: sque=0x%02x",
          (unsigned int)  pFromStruct->m_uSourceQueue);
  cli_out(" rxport0=0x%02x", (unsigned int)  pFromStruct->m_uRxPortData0);
  cli_out(" repcnt=0x%03x", (unsigned int)  pFromStruct->m_uReplicantCnt);
  cli_out(" repbit=0x%01x", (unsigned int)  pFromStruct->m_bReplicantBit);
  cli_out("\n");

  cli_out("Fe2000PpeDescriptor:: resv0=0x%01x",
          (unsigned int)  pFromStruct->m_uResv0);
  cli_out(" drop=0x%01x", (unsigned int)  pFromStruct->m_uDrop);
  cli_out(" enq=0x%01x", (unsigned int)  pFromStruct->m_uEnqueue);
  cli_out(" dqueue=0x%02x", (unsigned int)  pFromStruct->m_uDqueue);
  cli_out("\n");

  cli_out("Fe2000PpeDescriptor:: rxport1=0x%05x",
          (unsigned int)  pFromStruct->m_uRxPortData1);
  cli_out(" frmlen=0x%04x", (unsigned int)  pFromStruct->m_uFrameLength);
  cli_out(" aggrhash=0x%08x", (unsigned int)  pFromStruct->m_uAggrHash);
  cli_out("\n");

  cli_out("Fe2000PpeDescriptor:: variable=0x%08x",
          (unsigned int)  pFromStruct->m_uVariable);
  cli_out("\n");
}
static void
ppe_c2_print(sbZfC2PpeDescriptor_t *pFromStruct) {
  cli_out("C2PpeDescriptor:: contbyte=0x%02x", (unsigned int)  pFromStruct->m_uContinueByte);
  cli_out(" hdr0=0x%01x", (unsigned int)  pFromStruct->m_uHeaderType0);
  cli_out(" hdr1=0x%01x", (unsigned int)  pFromStruct->m_uHeaderType1);
  cli_out(" hdr2=0x%01x", (unsigned int)  pFromStruct->m_uHeaderType2);
  cli_out(" hdr3=0x%01x", (unsigned int)  pFromStruct->m_uHeaderType3);
  ...
  cli_out("C2PpeDescriptor:: hdr4=0x%01x", (unsigned int)  pFromStruct->m_uHeaderType4);
  cli_out(" hdr5=0x%01x", (unsigned int)  pFromStruct->m_uHeaderType5);
  cli_out(" hdr6=0x%01x", (unsigned int)  pFromStruct->m_uHeaderType6);
  cli_out(" hdr7=0x%01x", (unsigned int)  pFromStruct->m_uHeaderType7);
  cli_out(" hdr8=0x%01x", (unsigned int)  pFromStruct->m_uHeaderType8);
  ...
  cli_out("C2PpeDescriptor:: locn0=0x%02x", (unsigned int)  pFromStruct->m_uLocation0);
  cli_out(" locn1=0x%02x", (unsigned int)  pFromStruct->m_uLocation1);
  cli_out(" locn2=0x%02x", (unsigned int)  pFromStruct->m_uLocation2);
  cli_out(" locn3=0x%02x", (unsigned int)  pFromStruct->m_uLocation3);
  cli_out("\n");

  cli_out("C2PpeDescriptor:: locn4=0x%02x",
          (unsigned int)  pFromStruct->m_uLocation4);
  cli_out(" locn5=0x%02x", (unsigned int)  pFromStruct->m_uLocation5);
  cli_out(" locn6=0x%02x", (unsigned int)  pFromStruct->m_uLocation6);
  cli_out(" locn7=0x%02x", (unsigned int)  pFromStruct->m_uLocation7);
  ...
  cli_out("C2PpeDescriptor:: locn8=0x%02x",
          (unsigned int)  pFromStruct->m_uLocation8);
  cli_out(" locn9=0x%02x", (unsigned int)  pFromStruct->m_uLocation9);
  cli_out(" str=0x%01x", (unsigned int)  pFromStruct->m_uStreamSelector);
  cli_out(" buffer=0x%01x", (unsigned int)  pFromStruct->m_uBuffer);
  ...
  cli_out("C2PpeDescriptor:: sque=0x%02x",
          (unsigned int)  pFromStruct->m_uSourceQueue);
  cli_out(" rxport0=0x%02x", (unsigned int)  pFromStruct->m_uRxPortData0);
  cli_out(" repcnt=0x%03x", (unsigned int)  pFromStruct->m_uReplicantCnt);
  cli_out(" repbit=0x%01x", (unsigned int)  pFromStruct->m_bReplicantBit);
  ...
  cli_out("C2PpeDescriptor:: resv0=0x%01x",
          (unsigned int)  pFromStruct->m_uResv0);
  cli_out(" drop=0x%01x", (unsigned int)  pFromStruct->m_uDrop);
  cli_out(" enq=0x%01x", (unsigned int)  pFromStruct->m_uEnqueue);
  cli_out(" dqueue=0x%02x", (unsigned int)  pFromStruct->m_uDqueue);
  ...
  cli_out("C2PpeDescriptor:: rxport1=0x%05x",
          (unsigned int)  pFromStruct->m_uRxPortData1);
  cli_out(" frmlen=0x%04x", (unsigned int)  pFromStruct->m_uFrameLength);
  cli_out(" aggrhash=0x%08x", (unsigned int)  pFromStruct->m_uAggrHash);
  cli_out("\n");

  cli_out("C2PpeDescriptor:: variable=0x%08x",
          (unsigned int)  pFromStruct->m_uVariable);
  cli_out("\n");
}
...
pedin_print(sbZfFe2000LrpDescriptor_t *pFromStruct) {
  cli_out("Fe2000LrpDescriptor:: contbyte=0x%02x",
  ...
  cli_out("Fe2000LrpDescriptor:: hdr4=0x%01x",
  ...
  cli_out("Fe2000LrpDescriptor:: locn0=0x%02x",
  ...
  cli_out("Fe2000LrpDescriptor:: locn4=0x%02x",
          (unsigned int)  pFromStruct->m_uLocation4);
  cli_out(" locn5=0x%02x", (unsigned int)  pFromStruct->m_uLocation5);
  cli_out(" locn6=0x%02x", (unsigned int)  pFromStruct->m_uLocation6);
  cli_out(" locn7=0x%02x", (unsigned int)  pFromStruct->m_uLocation7);
  ...
  cli_out("Fe2000LrpDescriptor:: locn8=0x%02x",
          (unsigned int)  pFromStruct->m_uLocation8);
  cli_out(" locn9=0x%02x", (unsigned int)  pFromStruct->m_uLocation9);
  cli_out(" str=0x%01x", (unsigned int)  pFromStruct->m_uStreamSelector);
  cli_out(" buffer=0x%04x", (unsigned int)  pFromStruct->m_uBuffer);
  cli_out("\n");

  cli_out("Fe2000LrpDescriptor:: expidx=0x%02x",
          (unsigned int)  pFromStruct->m_uExceptIndex);
  cli_out(" sque=0x%02x", (unsigned int)  pFromStruct->m_uSourceQueue);
  cli_out(" tc=0x%01x", (unsigned int)  pFromStruct->m_uTruncationCommand);
  cli_out(" hccpy=0x%01x", (unsigned int)  pFromStruct->m_uHeaderCopy);
  cli_out(" mirridx=0x%01x", (unsigned int)  pFromStruct->m_uMirrorIndex);
  cli_out("\n");

  cli_out("Fe2000LrpDescriptor:: cpycnt=0x%03x",
          (unsigned int)  pFromStruct->m_uCopyCount);
  cli_out(" crcmode=0x%01x", (unsigned int)  pFromStruct->m_uCrcMode);
  cli_out(" dropbit=0x%01x", (unsigned int)  pFromStruct->m_uDropBit);
  cli_out(" enq=0x%01x", (unsigned int)  pFromStruct->m_uEnqueue);
  cli_out("\n");

  cli_out("Fe2000LrpDescriptor:: dqueue=0x%02x",
          (unsigned int)  pFromStruct->m_uDestinationQueue);
  cli_out(" xferlen=0x%04x", (unsigned int)  pFromStruct->m_uXferLength);
  cli_out(" packmark=0x%01x", (unsigned int)  pFromStruct->m_uPacketMarking);
  cli_out(" resv1=0x%01x", (unsigned int)  pFromStruct->m_uResv1);
  cli_out("\n");

  cli_out("Fe2000LrpDescriptor:: frmlen=0x%04x",
          (unsigned int)  pFromStruct->m_uFrameLength);
  cli_out(" resv2=0x%08x", (unsigned int)  pFromStruct->m_uResv2);
  cli_out(" resv3=0x%08x", (unsigned int)  pFromStruct->m_uResv3);
  cli_out("\n");
}

static void
pedin_c2_print(sbZfC2LrpDescriptor_t *pFromStruct) {
  cli_out("C2LrpDescriptor:: contbyte=0x%02x",
          (unsigned int)  pFromStruct->m_uContinueByte);
  cli_out(" hdr0=0x%01x", (unsigned int)  pFromStruct->m_uHeaderType0);
  cli_out(" hdr1=0x%01x", (unsigned int)  pFromStruct->m_uHeaderType1);
  cli_out(" hdr2=0x%01x", (unsigned int)  pFromStruct->m_uHeaderType2);
  ...
  cli_out("C2LrpDescriptor:: hdr4=0x%01x",
          (unsigned int)  pFromStruct->m_uHeaderType4);
  cli_out(" hdr5=0x%01x", (unsigned int)  pFromStruct->m_uHeaderType5);
  cli_out(" hdr6=0x%01x", (unsigned int)  pFromStruct->m_uHeaderType6);
  cli_out(" hdr7=0x%01x", (unsigned int)  pFromStruct->m_uHeaderType7);
  ...
  cli_out("C2LrpDescriptor:: locn0=0x%02x",
          (unsigned int)  pFromStruct->m_uLocation0);
  cli_out(" locn1=0x%02x", (unsigned int)  pFromStruct->m_uLocation1);
  cli_out(" locn2=0x%02x", (unsigned int)  pFromStruct->m_uLocation2);
  cli_out(" locn3=0x%02x", (unsigned int)  pFromStruct->m_uLocation3);
  ...
  cli_out("C2LrpDescriptor:: locn4=0x%02x",
          (unsigned int)  pFromStruct->m_uLocation4);
  cli_out(" locn5=0x%02x", (unsigned int)  pFromStruct->m_uLocation5);
  cli_out(" locn6=0x%02x", (unsigned int)  pFromStruct->m_uLocation6);
  cli_out(" locn7=0x%02x", (unsigned int)  pFromStruct->m_uLocation7);
  ...
  cli_out("C2LrpDescriptor:: locn8=0x%02x",
          (unsigned int)  pFromStruct->m_uLocation8);
  cli_out(" locn9=0x%02x", (unsigned int)  pFromStruct->m_uLocation9);
  cli_out(" str=0x%01x", (unsigned int)  pFromStruct->m_uStreamSelector);
  cli_out(" buffer=0x%04x", (unsigned int)  pFromStruct->m_uBuffer);
  ...
  cli_out("C2LrpDescriptor:: expidx=0x%02x",
          (unsigned int)  pFromStruct->m_uExceptIndex);
  cli_out(" sque=0x%02x", (unsigned int)  pFromStruct->m_uSourceQueue);
  cli_out(" tc=0x%01x", (unsigned int)  pFromStruct->m_uTruncationCommand);
  cli_out(" hccpy=0x%01x", (unsigned int)  pFromStruct->m_uHeaderCopy);
  ...
  cli_out("C2LrpDescriptor:: cpycnt=0x%03x",
          (unsigned int)  pFromStruct->m_uCopyCount);
  cli_out(" crcmode=0x%01x", (unsigned int)  pFromStruct->m_uCrcMode);
  cli_out(" dropbit=0x%01x", (unsigned int)  pFromStruct->m_uDropBit);
  cli_out(" enq=0x%01x", (unsigned int)  pFromStruct->m_uEnqueue);
  ...
  cli_out("C2LrpDescriptor:: dqueue=0x%02x",
          (unsigned int)  pFromStruct->m_uDestinationQueue);
  cli_out(" xferlen=0x%04x", (unsigned int)  pFromStruct->m_uXferLength);
  cli_out(" packmark=0x%01x", (unsigned int)  pFromStruct->m_uPacketMarking);
  cli_out(" resv1=0x%01x", (unsigned int)  pFromStruct->m_uResv1);
  ...
  cli_out("C2LrpDescriptor:: frmlen=0x%04x",
          (unsigned int)  pFromStruct->m_uFrameLength);
  cli_out(" resv2=0x%08x", (unsigned int)  pFromStruct->m_uResv2);
  cli_out(" resv3=0x%08x", (unsigned int)  pFromStruct->m_uResv3);
  cli_out("\n");
...
pedout_print(sbZfFe2000PedDescriptor_t *pFromStruct) {
  cli_out("Fe2000PedDescriptor:: cntbyte=0x%02x",
          (unsigned int)  pFromStruct->m_uContinueByte);
  cli_out(" hdrlen=0x%02x", (unsigned int)  pFromStruct->m_uHeaderLength);
  cli_out(" resv0=0x%04x", (unsigned int)  pFromStruct->m_uResv0);
  cli_out(" strsel=0x%01x", (unsigned int)  pFromStruct->m_uStreamSelector);
  cli_out("\n");

  cli_out("Fe2000PedDescriptor:: buffer=0x%04x",
          (unsigned int)  pFromStruct->m_uBuffer);
  cli_out(" excpidx=0x%02x", (unsigned int)  pFromStruct->m_uExceptionIndex);
  cli_out(" squeue=0x%02x", (unsigned int)  pFromStruct->m_uSourceQueue);
  cli_out(" resv1=0x%01x", (unsigned int)  pFromStruct->m_uResv1);
  cli_out("\n");

  cli_out("Fe2000PedDescriptor:: mridx=0x%01x",
          (unsigned int)  pFromStruct->m_uMirrorIndex);
  cli_out(" cpycnt=0x%03x", (unsigned int)  pFromStruct->m_uCopyCount);
  cli_out(" crcmode=0x%01x", (unsigned int)  pFromStruct->m_uCrcMode);
  cli_out(" dbit=0x%01x", (unsigned int)  pFromStruct->m_uDropBit);
  cli_out(" enque=0x%01x", (unsigned int)  pFromStruct->m_uEnqueue);
  cli_out("\n");

  cli_out("Fe2000PedDescriptor:: dque=0x%02x",
          (unsigned int)  pFromStruct->m_uDestinationQueue);
  cli_out(" xferlen=0x%04x", (unsigned int)  pFromStruct->m_uXferLength);
  cli_out(" packmark=0x%01x", (unsigned int)  pFromStruct->m_uPacketMarking);
  cli_out(" resv3=0x%01x", (unsigned int)  pFromStruct->m_uResv3);
  cli_out("\n");

  cli_out("Fe2000PedDescriptor:: srcfrmlen=0x%04x",
          (unsigned int)  pFromStruct->m_uSourceFrameLength);
  cli_out("\n");
}
static void
pedout_c2_print(sbZfC2PedDescriptor_t *pFromStruct) {
  cli_out("C2PedDescriptor:: cntbyte=0x%02x",
          (unsigned int)  pFromStruct->m_uContinueByte);
  cli_out(" hdrlen=0x%02x", (unsigned int)  pFromStruct->m_uHeaderLength);
  cli_out(" resv0=0x%04x", (unsigned int)  pFromStruct->m_uResv0);
  cli_out(" strsel=0x%01x", (unsigned int)  pFromStruct->m_uStreamSelector);
  ...
  cli_out("C2PedDescriptor:: buffer=0x%04x",
          (unsigned int)  pFromStruct->m_uBuffer);
  cli_out(" excpidx=0x%02x", (unsigned int)  pFromStruct->m_uExceptionIndex);
  cli_out(" squeue=0x%02x", (unsigned int)  pFromStruct->m_uSourceQueue);
  cli_out(" resv1=0x%01x", (unsigned int)  pFromStruct->m_uResv1);
  ...
  cli_out("C2PedDescriptor:: mridx=0x%01x",
          (unsigned int)  pFromStruct->m_uMirrorIndex);
  cli_out(" cpycnt=0x%03x", (unsigned int)  pFromStruct->m_uCopyCount);
  cli_out(" crcmode=0x%01x", (unsigned int)  pFromStruct->m_uCrcMode);
  cli_out(" dbit=0x%01x", (unsigned int)  pFromStruct->m_uDropBit);
  ...
  cli_out("C2PedDescriptor:: dque=0x%02x",
          (unsigned int)  pFromStruct->m_uDestinationQueue);
  cli_out(" xferlen=0x%04x", (unsigned int)  pFromStruct->m_uXferLength);
  cli_out(" packmark=0x%01x", (unsigned int)  pFromStruct->m_uPacketMarking);
  cli_out(" resv3=0x%01x", (unsigned int)  pFromStruct->m_uResv3);
  ...
  cli_out("C2PedDescriptor:: srcfrmlen=0x%04x",
          (unsigned int)  pFromStruct->m_uSourceFrameLength);
  cli_out("\n");
}
...
_cmd_sbx_fe2000_dump_header(int unit, header_dump_type_t type,
                            int raw, int parsed)
{
    int hdr_words = SB_FE2000_HEADER_CAPTURE_SIZE_IN_BYTES/sizeof(uint32);
    uint32 h[SB_FE2000_HEADER_CAPTURE_SIZE_IN_BYTES/sizeof(uint32)];
    uint32 hr[SB_FE2000_HEADER_CAPTURE_SIZE_IN_BYTES/sizeof(uint32)];
    sbhandle sbh = SOC_SBX_CONTROL(unit)->sbhdl;
    ...
    if ((0 == reg) || (NULL == name)) {
        cli_out("Unknown header dump type=%d", type);
        return CMD_FAIL;
    }
    ...
    for (i = 0; i < hdr_words / 8; i++) {
        if (sbFe2000UtilReadIndir(sbh, FALSE, descending, reg, i, 8,
                                  &h[j * 8])) {
            cli_out("%s header read failed at %d\n", name, i);
            return CMD_FAIL;
        }
    ...
    if ( parsed ) {
        cli_out("%s descriptor:\n", name);
        switch (type) {
        case HEADER_DUMP_PPE_OUT:
            if (!SOC_IS_SBX_FE2KXT(unit)){
                sbZfFe2000PpeDescriptor_Unpack(&ppd, (uint8 *) hr,
                                               hdr_words * sizeof(uint32));
                ppe_print(&ppd);
            }else{
                sbZfC2PpeDescriptor_Unpack(&ppd_c2, (uint8 *) hr,
                                               hdr_words * sizeof(uint32));
                ppe_c2_print(&ppd_c2);
            }
            break;
        case HEADER_DUMP_PED_IN:
            if (!SOC_IS_SBX_FE2KXT(unit)){
                sbZfFe2000LrpDescriptor_Unpack(&lrd, (uint8 *) hr,
                                               hdr_words * sizeof(uint32));
                pedin_print(&lrd);
            }else{
                sbZfC2LrpDescriptor_Unpack(&lrd_c2, (uint8 *) hr,
                                               hdr_words * sizeof(uint32));
        ...
        case HEADER_DUMP_PED_OUT:
            ...
            if (!SOC_IS_SBX_FE2KXT(unit)){
                sbZfFe2000PedDescriptor_Unpack(&ped, (uint8 *) hr,
                                               hdr_words * sizeof(uint32));
                pedout_print(&ped);
            }else{
                sbZfC2PedDescriptor_Unpack(&ped_c2, (uint8 *) hr,
                                               hdr_words * sizeof(uint32));
    ...
    if ( raw ) {
        cli_out("%s descriptor dump:", name);
...
cmd_result_t
cmd_sbx_fe2000_hd(int unit, int ppe, int pedin, int pedout, 
                  int raw, int parsed)
{
    sbhandle sbh;
    int rv, i, in, out, n;
    ...
    if (ppe) {
        if (SAND_HAL_FE2000_GET_FIELD(sbh, PP_COPY_OUT, INTERRUPT,
                               SAND_HAL_FE2000_READ(sbh, PP_COPY_OUT))) {
            cli_out("dumping captured PPE header\n");
    ...
    if (n) {
        cli_out("dumping %d captured PED headers\n", n);
    }
    for (i = 0; i < n; i++) {
        cli_out("PED header %d ---------------------------------------------\n",
                i);
        if (i < in) {
    ...
    if (pedin
        && SAND_HAL_FE2000_GET_FIELD(sbh, PD_COPY_IN_EVENT, HDR_REC_OVERFLOW,
                                    SAND_HAL_FE2000_READ(sbh, PD_COPY_IN_EVENT))) {
        cli_out("PED_IN header capture buffer overflowed\n");
    ...
    if (pedout
        && SAND_HAL_FE2000_GET_FIELD(sbh, PD_COPY_OUT_EVENT, HDR_REC_OVERFLOW,
                              SAND_HAL_FE2000_READ(sbh, PD_COPY_OUT_EVENT))) {
        cli_out("PED_OUT header capture buffer overflowed\n");
...
cmd_sbx_fe2000_hg(int unit, args_t *a)
{
    ...
    if (dumpold) {
        ...
        if (!(SAND_HAL_FE2000_GET_FIELD(sbh, PP_COPY_OUT, INTERRUPT,
                           SAND_HAL_FE2000_READ(sbh, PP_COPY_OUT)))) {
            cli_out("No packet header captured on port %d\n", queue);
            return CMD_OK;
        }
    ...
    for (i = 0; i < hdr_words / 8; i++) {
        if (sbFe2000UtilReadIndir(sbh, TRUE, descending, reg, i, 8,
                                  &hr[j * 8])) {
            cli_out("PPE header read failed at %d\n", i);
            return CMD_FAIL;
        }
        j = j + 1;
    }
    ...
    if ((tcl_path == NULL) || (tcl_path[0] == '\0')) {
        cli_out("TCL_LIBRARY environment variable not set\n");
        return CMD_FAIL;
    }
#endif

    cli_out("%d byte packet on port %d, captured header ",
            ppd.m_uFrameLength, queue);
    if ((sal_strlen(tcl_path)) > 150) {
        cli_out("TCL_LIBRARY environment variable too long: %s\n", tcl_path);
        return CMD_FAIL;
    }
    ...
    if (fabric) {
      sbxpkt_from_byte (ERH_QE, &ppd.m_uPacketDataByte0, ppd.m_uFrameLength, pkt);
    }else {
      sbxpkt_from_byte (MAC, &ppd.m_uPacketDataByte0, ppd.m_uFrameLength, pkt);
    }
    sbxpkt_print(pkt);
...
cmd_sbx_fe2000_pkt_dump(int unit, args_t *args)
{
    ...
    if (pkt_type > COUNTOF(cmd_pkt_dump_parse_type) || pkt_type == 0) {
        cli_out("Uknown packet type: %d\n", pkt_type);
        return CMD_USAGE;
    }

    buffer = sal_alloc(buffer_max, "pkt_buffer");
    if (buffer == NULL) {
        cli_out("Failed to alloc temp buffer\n");
        return CMD_FAIL;
    }
    ...
    /* Format with sbx packet class */
    pkt = sbxpkt_alloc();
    sbxpkt_from_byte(pkt_type, buffer, byte_pos, pkt);
...
cmd_result_t
cmd_sbx_fe2000_nic_config(int unit,
                          int ingress, int egress, int free)
{
    sbhandle sbh;
    ...
    if (!SOC_IS_SBX_FE2KXT(unit)){
        ...
        if (ingress == 1) {
            ...
            if (!found) {
                cli_out("couldn't find system-side queue for PCI\n");
                return CMD_FAIL;
            }
            CaPtQueueToPortRead(sbh, ihppq, &q2p);
            ...
            if (qn == ehppq) {
                cli_out("PCI NIC is currently captive to egress\n");
            } else {
                CaPtPortToQueueRead(sbh, SB_FE2000_IF_PTYPE_PCI, 0, &qn);
                if (qn == isysq) {
                    cli_out("PCI NIC is currently captive to ingress\n");
                } else {
                    cli_out("PCI NIC currently connects to the system\n");
                }
            }
        }
    }else{ /* FE2KXT */
                   ...
                   bcm_policer_config_t *info, void *cookie_unused)
{
    ...
    if (rv == BCM_E_NONE) {
        ...
        cli_out("BCM Policer id - %d \n", id);
        cli_out("    flags:      0x%x -> ", info->flags);
        if (info->flags) {
            if (info->flags & BCM_POLICER_WITH_ID)
                cli_out("BCM_POLICER_WITH_ID ");
            if (info->flags & BCM_POLICER_REPLACE)
                cli_out("BCM_POLICER_REPLACE ");
            if (info->flags & BCM_POLICER_COLOR_BLIND)
                cli_out("BCM_POLICER_COLOR_BLIND ");
            if (info->flags & BCM_POLICER_DROP_RED)
                cli_out("BCM_POLICER_DROP_RED ");
            cli_out(" \n");
        } else {
            cli_out("EMPTY\n");
        }
	...
	case bcmPolicerModeSrTcmModified:
            cli_out("SrTcmModified\n");
            break;
	case bcmPolicerModeCoupledTrTcmDs:
            cli_out("CoupledTrTcmDs\n");
            break;
	default:
            cli_out("%d\n", info->mode);
            break;
        }
        cli_out("    Committed:  Burst - %dB, CIR - %dkbps \n",
                KBITS_TO_BYTES(info->ckbits_burst), info->ckbits_sec);
        cli_out("    Excess:     Burst - %dB, EIR - %dkbps \n",
                KBITS_TO_BYTES(info->pkbits_burst), info->pkbits_sec);
        cli_out("    Current:    Rate - %dkbps \n", info->kbits_current);
    }
    return rv;
}
...
int
_bcm_sbx_policer_stat_display(int unit, int stat_type, uint64 stat_value)
{
    int rv = CMD_OK;

    switch (stat_type) {
    case bcmPolicerStatGreenToGreenPackets: 
        cli_out("Green To Green Packets:     0x%x%08x \n", COMPILER_64_HI(stat_value),COMPILER_64_LO(stat_value) );
        break;
    case bcmPolicerStatGreenToGreenBytes: 
        cli_out("Green To Green Bytes:       0x%x%08x \n", COMPILER_64_HI(stat_value),COMPILER_64_LO(stat_value));
        break;
    case bcmPolicerStatGreenToYellowPackets: 
        cli_out("Green To Yellow Packets:    0x%x%08x \n", COMPILER_64_HI(stat_value),COMPILER_64_LO(stat_value));
        break;
    case bcmPolicerStatGreenToYellowBytes: 
        cli_out("Green To Yellow Bytes:      0x%x%08x \n", COMPILER_64_HI(stat_value),COMPILER_64_LO(stat_value));
        break;
    case bcmPolicerStatGreenToRedPackets: 
        cli_out("Green To Red Packets:       0x%x%08x \n", COMPILER_64_HI(stat_value),COMPILER_64_LO(stat_value));
        break;
    case bcmPolicerStatGreenToRedBytes: 
        cli_out("Green To Red Bytes:         0x%x%08x \n", COMPILER_64_HI(stat_value),COMPILER_64_LO(stat_value));
        break;
    case bcmPolicerStatGreenToDropPackets: 
        cli_out("Green To Drop Packets:      0x%x%08x \n", COMPILER_64_HI(stat_value),COMPILER_64_LO(stat_value));
        break;
    case bcmPolicerStatGreenToDropBytes: 
        cli_out("Green To Drop Bytes:        0x%x%08x \n", COMPILER_64_HI(stat_value),COMPILER_64_LO(stat_value));
        break;
    case bcmPolicerStatYellowToGreenPackets: 
        cli_out("Yellow To Green Packets:    0x%x%08x \n", COMPILER_64_HI(stat_value),COMPILER_64_LO(stat_value));
        break;
    case bcmPolicerStatYellowToGreenBytes: 
        cli_out("Yellow To Green Bytes:      0x%x%08x \n", COMPILER_64_HI(stat_value),COMPILER_64_LO(stat_value));
        break;
    case bcmPolicerStatYellowToYellowPackets: 
        cli_out("Yellow To Yellow Packets:   0x%x%08x \n", COMPILER_64_HI(stat_value),COMPILER_64_LO(stat_value));
        break;
    case bcmPolicerStatYellowToYellowBytes: 
        cli_out("Yellow To Yellow Bytes:     0x%x%08x \n", COMPILER_64_HI(stat_value),COMPILER_64_LO(stat_value));
        break;
    case bcmPolicerStatYellowToRedPackets: 
        cli_out("Yellow To Red Packets:      0x%x%08x \n", COMPILER_64_HI(stat_value),COMPILER_64_LO(stat_value));
        break;
    case bcmPolicerStatYellowToRedBytes: 
        cli_out("Yellow To Red Bytes:        0x%x%08x \n", COMPILER_64_HI(stat_value),COMPILER_64_LO(stat_value));
        break;
    case bcmPolicerStatYellowToDropPackets: 
        cli_out("Yellow To Drop Packets:     0x%x%08x \n", COMPILER_64_HI(stat_value),COMPILER_64_LO(stat_value));
        break;
    case bcmPolicerStatYellowToDropBytes: 
        cli_out("Yellow To Drop Bytes:       0x%x%08x \n", COMPILER_64_HI(stat_value),COMPILER_64_LO(stat_value));
        break;
    case bcmPolicerStatRedToGreenPackets: 
        cli_out("Red To Green Packets:       0x%x%08x \n", COMPILER_64_HI(stat_value),COMPILER_64_LO(stat_value));
        break;
    case bcmPolicerStatRedToGreenBytes: 
        cli_out("Red To Green Bytes:         0x%x%08x \n", COMPILER_64_HI(stat_value),COMPILER_64_LO(stat_value));
        break;
    case bcmPolicerStatRedToYellowPackets: 
        cli_out("Red To Yellow Packets:      0x%x%08x \n", COMPILER_64_HI(stat_value),COMPILER_64_LO(stat_value));
        break;
    case bcmPolicerStatRedToYellowBytes: 
        cli_out("Red To Yellow Bytes:        0x%x%08x \n", COMPILER_64_HI(stat_value),COMPILER_64_LO(stat_value));
        break;
    case bcmPolicerStatRedToRedPackets: 
        cli_out("Red To Red Packets:         0x%x%08x \n", COMPILER_64_HI(stat_value),COMPILER_64_LO(stat_value));
        break;
    case bcmPolicerStatRedToRedBytes: 
        cli_out("Red To Red Bytes:           0x%x%08x \n", COMPILER_64_HI(stat_value),COMPILER_64_LO(stat_value));
        break;
    case bcmPolicerStatRedToDropPackets: 
        cli_out("Red To Drop Packets:        0x%x%08x \n", COMPILER_64_HI(stat_value),COMPILER_64_LO(stat_value));
        break;
    case bcmPolicerStatRedToDropBytes: 
        cli_out("Red To Drop Bytes:          0x%x%08x \n", COMPILER_64_HI(stat_value),COMPILER_64_LO(stat_value));
        break;
    case bcmPolicerStatPackets:
        cli_out("Policed Packets             0x%x%08x \n", COMPILER_64_HI(stat_value),COMPILER_64_LO(stat_value));
        break;
    case bcmPolicerStatBytes:
        cli_out("Policed Bytes               0x%x%08x \n", COMPILER_64_HI(stat_value),COMPILER_64_LO(stat_value));
        break;
    case bcmPolicerStatUnknownUnicastPackets:
        cli_out("Policed Unknown UC Packets  0x%x%08x \n", COMPILER_64_HI(stat_value),COMPILER_64_LO(stat_value));
        break;
    case bcmPolicerStatUnknownUnicastBytes:
        cli_out("Policed Unknown UC Bytes    0x%x%08x \n", COMPILER_64_HI(stat_value),COMPILER_64_LO(stat_value));
        break;
    case bcmPolicerStatUnicastPackets:
        cli_out("Policed UC Packets          0x%x%08x \n", COMPILER_64_HI(stat_value),COMPILER_64_LO(stat_value));
        break;
    case bcmPolicerStatUnicastBytes:
        cli_out("Policed UC Bytes            0x%x%08x \n", COMPILER_64_HI(stat_value),COMPILER_64_LO(stat_value));
        break;
    case bcmPolicerStatMulticastPackets:
        cli_out("Policed MC Packets          0x%x%08x \n", COMPILER_64_HI(stat_value),COMPILER_64_LO(stat_value));
        break;
    case bcmPolicerStatMulticastBytes:
        cli_out("Policed MC Bytes            0x%x%08x \n", COMPILER_64_HI(stat_value),COMPILER_64_LO(stat_value));
        break;
    case bcmPolicerStatBroadcastPackets:
        cli_out("Policed BC Packets          0x%x%08x \n", COMPILER_64_HI(stat_value),COMPILER_64_LO(stat_value));
        break;
    case bcmPolicerStatBroadcastBytes:
        cli_out("Policed BC Bytes            0x%x%08x \n", COMPILER_64_HI(stat_value),COMPILER_64_LO(stat_value));
        break;
    case bcmPolicerStatDropPackets:
        cli_out("Dropped Packets             0x%x%08x \n", COMPILER_64_HI(stat_value),COMPILER_64_LO(stat_value));
        break;
    case bcmPolicerStatDropBytes:
        cli_out("Dropped Bytes               0x%x%08x \n", COMPILER_64_HI(stat_value),COMPILER_64_LO(stat_value));
        break;
    case bcmPolicerStatDropUnknownUnicastPackets:
        cli_out("Dropped Unknown UC Packets  0x%x%08x \n", COMPILER_64_HI(stat_value),COMPILER_64_LO(stat_value));
        break;
    case bcmPolicerStatDropUnknownUnicastBytes:
        cli_out("Dropped Unknown UC Bytes    0x%x%08x \n", COMPILER_64_HI(stat_value),COMPILER_64_LO(stat_value));
        break;
    case bcmPolicerStatDropUnicastPackets:
        cli_out("Dropped UC Packets          0x%x%08x \n", COMPILER_64_HI(stat_value),COMPILER_64_LO(stat_value));
        break;
    case bcmPolicerStatDropUnicastBytes:
        cli_out("Dropped UC Bytes            0x%x%08x \n", COMPILER_64_HI(stat_value),COMPILER_64_LO(stat_value));
        break;
    case bcmPolicerStatDropMulticastPackets:
        cli_out("Dropped MC Packets          0x%x%08x \n", COMPILER_64_HI(stat_value),COMPILER_64_LO(stat_value));
        break;
    case bcmPolicerStatDropMulticastBytes:
        cli_out("Dropped MC Bytes            0x%x%08x \n", COMPILER_64_HI(stat_value),COMPILER_64_LO(stat_value));
        break;
    case bcmPolicerStatDropBroadcastPackets:
        cli_out("Dropped BC Packets          0x%x%08x \n", COMPILER_64_HI(stat_value),COMPILER_64_LO(stat_value));
        break;
    case bcmPolicerStatDropBroadcastBytes:
        cli_out("Dropped BC Bytes            0x%x%08x \n", COMPILER_64_HI(stat_value),COMPILER_64_LO(stat_value));
        break;
    case bcmPolicerStatGreenPackets:
        cli_out("Green marked Packets        0x%x%08x \n", COMPILER_64_HI(stat_value),COMPILER_64_LO(stat_value));
        break;
    case bcmPolicerStatGreenBytes:
        cli_out("Green marked Bytes          0x%x%08x \n", COMPILER_64_HI(stat_value),COMPILER_64_LO(stat_value));
        break;
    case bcmPolicerStatYellowPackets:
        cli_out("Yellow marked Packets       0x%x%08x \n", COMPILER_64_HI(stat_value),COMPILER_64_LO(stat_value));
        break;
    case bcmPolicerStatYellowBytes:
        cli_out("Yellow marked Bytes         0x%x%08x \n", COMPILER_64_HI(stat_value),COMPILER_64_LO(stat_value));
        break;
    case bcmPolicerStatRedPackets:
        cli_out("Red marked Packets          0x%x%08x \n", COMPILER_64_HI(stat_value),COMPILER_64_LO(stat_value));
        break;
    case bcmPolicerStatRedBytes:
        cli_out("Red marked Bytes            0x%x%08x \n", COMPILER_64_HI(stat_value),COMPILER_64_LO(stat_value));
        break;
    default:
        rv = CMD_FAIL;
        break;
...
_cmd_pol_list(int unit, args_t *args)
{
    ...
    if (!sal_strncasecmp(param, "all", 3)) {
        ...
        if (rv != BCM_E_NONE) {
            cli_out("Internal Error displaying policer info. \n");
            return CMD_FAIL;
        }
    } else {
        ...
        if (bcm_policer_get(unit, _polid, &_pol_cfg) != BCM_E_NONE) {
            cli_out("Invalid policer id (%d) specified. \n", _polid);
            return CMD_FAIL;
...
_cmd_pol_group_rate_get(int unit, args_t *args)
{
    ...
    if ((group < 0) || (group >= SB_FE_2000_PM_NUM_OF_GROUPS)) {
        cli_out("Invalid group specified. \n");
        return CMD_FAIL;
    }

    sbsts = sbFe2000PmPolicerGroupRead(pPm, group, &zGroupConfig);
    if(sbsts != SB_OK) {
        cli_out("%s: sbFe2000PmPolicerGroupRead failed. \n",FUNCTION_NAME());
        return CMD_FAIL;
    }
...
_cmd_pol_group_rate_set(int unit, args_t *args)
{
    ...
    if (_cmd_pol_rate_valid(rate, &sb_rate) != BCM_E_NONE) {
        cli_out("Invalid rate (%s) specified. \n", rate);
        parse_arg_eq_done(&pt);
        return CMD_USAGE;
    }
    ...
    if (sbsts != SB_OK) {
        cli_out("%s: sbFe2000PmPolicerGroupRateChange failed. \n",
                FUNCTION_NAME());
        parse_arg_eq_done(&pt);
        return CMD_FAIL;
    }
    cli_out("Group %d Base rate changed to %s \n", group, rate);
    parse_arg_eq_done(&pt);
    return CMD_OK;
}
...
_cmd_pol_stat(int unit, args_t *args)
{
    ...
    if (!sal_strncasecmp(subcmd, "enable", (sc_len>6 ? 6:sc_len))) {
        if ((rv = bcm_policer_stat_enable_set(unit, polid, 1)) != BCM_E_NONE) {
            cli_out("Stat enable for policer id %d failed (rv=%d: %s) \n",
        ...
        if ((rv = bcm_policer_stat_enable_set(unit, polid, 0)) != BCM_E_NONE) {
            cli_out("Stat disable for policer id %d failed (rv=%d: %s) \n",
                    polid, rv, bcm_errmsg(rv));
            return CMD_FAIL;
        }
        ...
        if ((stat_type_min < 1) || 
            ...
            cli_out("Invalid stat_type specified (%d) \n", type);
            return CMD_USAGE;
        }
        ...
        if (value) {
            cli_out("stat value can only be 0 \n");
            return CMD_FAIL;
        }
        if (type < 0) {
            stat_type_min = 1;
        ...
        for (idx=stat_type_min; idx<=stat_type_max; idx++) {
            ...
            for (cos = cos_min; cos < cos_max; cos++) {
                if (_bcm_sbx_policer_stat_valid(unit, polid, idx) 
                    == BCM_E_NONE) {
                    COMPILER_64_SET(stat_value, 0, value);
                    if (bcm_policer_stat_set(unit, polid, cos, idx, stat_value) 
                        != BCM_E_NONE) {
                        return CMD_FAIL;
                    }
                }
...
_cmd_pol_oam_timer(int unit, args_t *args)
{
    ...
    if (!sal_strncasecmp(subcmd, "add", (sc_len > 3? 3: sc_len))) {
        ...
        if ((id < 0) || (deadline < 0) || (started < 0) || (strict < 0) ||
            (reset < 0) || (intr < 0)) {
            cli_out("One or more input params missing for \"time add\" OAM "
                    "sub-command\n");
            parse_arg_eq_done(&pt);
            return CMD_USAGE;
        }
        ...
        sb_sts = sbFe2000PmOamTimerCreate(pPm, (uint32)id, &zTimer, &id_ret);
        if (sb_sts != SB_OK) {
            cli_out("sbFe2000PmOamTimerCreate failed. \n");
            parse_arg_eq_done(&pt);
            return CMD_FAIL;
        }
        cli_out("Timer Id - %d created successfully\n", id);
    } else if (!sal_strncasecmp(subcmd, "remove", (sc_len > 6? 6: sc_len))) {
        temp = ARG_CUR(args);
        sc_len = strlen(temp);
        if (!sal_strncasecmp(temp, "all", (sc_len > 3? 3: sc_len))) {
            cli_out("TODO: remove all not supported yet. \n");
            return CMD_FAIL;
        } else {
            parse_table_init(unit, &pt);
            parse_table_add(&pt, "id",          PQ_DFL|PQ_INT, 0, &id,       NULL);
            ...
            if (id < 0) {
                cli_out("Timer id not specified. \n");
                parse_arg_eq_done(&pt);
                return CMD_USAGE;
            }
            min = id;
        ...
        for(idx = min; idx < max; idx++) {
            sb_sts = sbFe2000PmOamTimerDelete(pPm, (uint32)idx);
            if(sb_sts != SB_OK) {
                cli_out("sbFe2000PmOamTimerDelete failed. \n");
                parse_arg_eq_done(&pt);
                return CMD_FAIL;
            }
            cli_out("Timer %d Deleted\n", idx);
        }
        ...
        for(idx = min; idx < max; idx++) {
            sb_sts = sbFe2000PmOamTimerRead(pPm, (uint32)idx, &zTimer);
            if(sb_sts != SB_OK) {
                cli_out("sbFe2000PmOamTimerRead: idx %d failed. \n", idx);
                if (parsed) {
                    parse_arg_eq_done(&pt);
                }
                return CMD_FAIL;
        ...
        cli_out("Invalid OAM sub-command (%s) \n", subcmd);
        return CMD_USAGE;
    }
...
_cmd_pol_oam_seq_gen(int unit, args_t *args)
{
    ...
    if (!sal_strncasecmp(subcmd, "add", (sc_len > 3? 3: sc_len))) {
        ...
        if (id < 0) {
            cli_out("Id not specified. \n");
            parse_arg_eq_done(&pt);
            return CMD_USAGE;
        }
        sb_sts = sbFe2000PmOamSeqGenCreate(pPm, (uint32)id, &id_ret);
        if (sb_sts != SB_OK) {
            cli_out("sbFe2000PmOamSeqGenCreate failed. \n");
            parse_arg_eq_done(&pt);
            return CMD_FAIL;
        }
        cli_out("SeqGen Id - %d created successfully\n", id);
    } else if (!sal_strncasecmp(subcmd, "remove", (sc_len > 6 ? 6: sc_len))) {
        temp = ARG_CUR(args);
        sc_len = strlen(temp);
        if (!sal_strncasecmp(temp, "all", (sc_len > 3? 3: sc_len))) {
            ...
            if (id < 0) {
                cli_out("SeqGen id not specified. \n");
                parse_arg_eq_done(&pt);
                return CMD_USAGE;
            }
            min = id;
        ...
        for(idx = min; idx < max; idx++) {
            sb_sts = sbFe2000PmOamSeqGenDelete(pPm, (uint32)idx);
            if(sb_sts != SB_OK) {
                cli_out("sbFe2000PmOamSeqGenDelete failed. \n");
                if (parsed) {
                    parse_arg_eq_done(&pt);
                }
                return CMD_FAIL;
            }
            cli_out("SeqGen %d Deleted\n", idx);
        }
    } else {
        cli_out("Invalid SequenceGenerator sub-command (%s) \n", subcmd);
        return CMD_FAIL;
    }
...
_cmd_pol_oam(int unit, args_t *args) 
{
    ...
    if (!sal_strncasecmp(subcmd, "timer", (sc_len > 5? 5: sc_len))) {
        ...
        cli_out("Invalid OAM Sub-Command. \n");
        return CMD_USAGE;
    }
...
_cmd_pol_monitor(int unit, args_t *args)
{
    ...
    if (!sal_strncasecmp(subcmd, "attach", (sc_len>6 ? 6:sc_len))) {
        ...
        if (rv != BCM_E_NONE) {
          cli_out("Internal error attaching monitor to policer: %d\n", polid);
          return CMD_FAIL;
        }
        ...
        if (rv != BCM_E_NONE) {
          cli_out("Internal error detaching monitor to policer: %d\n", polid);
          return CMD_FAIL;
        }
        ...
        if (rv == BCM_E_NONE) {
            if (_bcm_fe2000_is_monitor_stat_enabled(unit, polid, &monid)){
                stat_min = ((SOC_INFO(unit).chip_type == SOC_INFO_CHIP_TYPE_FE2000) ?
                ...
                cli_out("Monitor stats not enabled on policer %d \n",
                        polid);
            }
            _bcm_fe2000_policer_unit_unlock(unit);
        }
        if (rv != BCM_E_NONE) {
            cli_out("Internal error displaying monitor stats for "
                    "policer: %d \n", polid);
            return CMD_FAIL;
        }
    } else {
        cli_out("Invalid Monitor sub-command (%s) \n", subcmd);
        return CMD_USAGE;
    }
...
{
    if (!SOC_IS_SBX_FE2000(unit)) {
        cli_out("Policer commands only supported on FE-2000 \n");
        return CMD_FAIL;
    }
...
cmd_sbx_ddr_tune(int unit, args_t *args)
{
  ...
  if (!SOC_IS_SBX_FE2KXT(unit)){
    cli_out("only supported on BCM88025\n");
    return CMD_FAIL;
  }
...
cmd_sbx_fe2000_sync_ethernet(int unit, args_t *a)
{
  ...
  if (sal_strcasecmp(subcmd,"off") == 0) {
    ...
    if (ge_port < 0 || ge_port > 23 ) {
      cli_out("%s:Invalid ge port (%d)\n",SOC_CHIP_STRING(unit),ge_port);
      return CMD_FAIL;
    }
    cli_out("Using %s for recovered clock\n",SOC_PORT_NAME(unit,ge_port));
    if (ge_port <= 11) {
...
cmd_sbx_fe2kxt_ddrconfig(int unit, args_t *a)
{
    ...
    if (!SOC_IS_SBX_FE2KXT(unit)) {
        cli_out("ddrconfig command valid only on FE2KXT\n");
        rv = CMD_FAIL;
    }
    if (sal_strcasecmp(subcmd, "load") == 0) {
        cli_out("loading DDR tuning config from EEPROM\n");
        ...
        cli_out("storing DDR tuning config to EEPROM\n");
        ...
        cli_out("current DDR tuning configuration \n");
