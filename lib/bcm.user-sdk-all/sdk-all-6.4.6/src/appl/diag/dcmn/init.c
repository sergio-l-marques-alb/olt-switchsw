 ...
 */
STATIC int
appl_dpp_misc_diag_init (int unit, int mymodid, int base_modid, int nof_devices, int nof_active_cores, int is_symmetric)
{
    int         rv = BCM_E_NONE;
    int         port_i = 0;
    int         modid_idx = 0;
    ...
    g_dii.nof_active_cores = nof_active_cores;
    g_dii.is_symmetric = is_symmetric;
...
appl_dpp_bcm_diag_init(int unit, appl_dcmn_init_param_t* init_param)
{
    ...
    rv = appl_dpp_misc_diag_init(unit, init_param->modid, init_param->base_modid, init_param->nof_devices, init_param->nof_active_cores, init_param->is_symmetric);
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_APPL_COMMON,
                  (BSL_META_U(unit,
