...
/*------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
    ...
    {  "EEPs for local use"                   , "EEP"           , "EEP"    , dpp_am_res_eep_local                      , NULL                              , NULL                              },
...
cmd_result_t diag_alloc_gport_info_get(int unit, 
   ...
   ret = _bcm_dpp_gport_to_hw_resources(unit, gport_info->gport_id, _BCM_DPP_GPORT_HW_RESOURCES_GLOBAL_LIF_INGRESS|_BCM_DPP_GPORT_HW_RESOURCES_GLOBAL_LIF_EGRESS|_BCM_DPP_GPORT_HW_RESOURCES_FEC, &gport_hw_resources);
   if (ret != BCM_E_NONE) {
       cli_out("Diag failed, error in _bcm_dpp_gport_to_lif\n");
       return CMD_FAIL; 
   }

   gport_info->lif_id = gport_hw_resources.global_in_lif;
   gport_info->out_lif_id = gport_hw_resources.global_out_lif;
...
cmd_diag_alloc_init(int unit, args_t *a, int pool_idx) {
    ...
    for (core_id=0 ; core_id < nof_cores_per_res ; core_id++) {
        ...
        /* get the lowest id for the current pool and the number of entries */
        rv = shr_res_pool_get(unit,pool_id,0,&low_id,&nbr_entries,0,0);
        if (rv != BCM_E_NONE) {
...
cmd_result_t cmd_diag_alloc_my_out_test(int unit)
{
       ...
       uint32 flags = 0; /* BCM_DPP_AM_LIF_FLAG_WITH_ID*/
        ...
        outlif_info.glif = 5001; /* pre-alloced Global LIF ID*/ 
