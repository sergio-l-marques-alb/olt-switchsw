...
portmod_eyescan_diag(int unit, args_t *a){
    ...
    if (!ARG_CNT(a)) {  
        ...
        if( sys_lane_mask != 0xFFFFFFFF) {
            cli_out("Running EyeScan for System Side lane_mask 0x%x.\n",sys_lane_mask);
            for (i = 0;i < nof_phys ;i++){
                phy_access[i].access.lane_mask = sys_lane_mask;
                /* setting bit 31, which is sys side for now until global sys side support. */
                phy_access[i].access.flags |= PHYMOD_ACC_DEVAD_0_OVERRIDE_MASK;
            }
        }
