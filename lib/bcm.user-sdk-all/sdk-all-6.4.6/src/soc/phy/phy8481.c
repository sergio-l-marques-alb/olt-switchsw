...
phy_8481_init(int unit, soc_port_t port)
{
    ...
    if ((PHYCTRL_INIT_STATE(pc) == PHYCTRL_INIT_STATE_PASS3) ||
        ...
        if ( PHY_IS_BCM8486X(pc) ) {    /* Mako supports 5G/2.5G speed */
            /* set 5G/2.5G over XFI mode,  command handler code 0x8017 */
            SOC_IF_ERROR_RETURN(
                    _phy8486x_xfi_2p5g_mode_set(unit, pc,
                                    PHY84834_DIAG_CMD_XFI_2P5G_MODE_XFI) );
...
_phy_8481_check_firmware(int unit,phy_ctrl_t *pc) {
    ...
    if ( (PHY_IS_BCM8485X(pc) && ((fw_ver_main > 0) || ((fw_ver_main == 0) && (fw_ver_branch >= 3 )))) ||
        ...
        if ( ((fw_ver_main == 1) && (fw_ver_branch >= 1)) || (fw_ver_main > 1) ) {
            FLAGS(pc) |= PHY8483X_ATOMIC_ACCESS_SHD;
        }
...
_phy_8481_copper_lb_set(int unit, soc_port_t port, int enable)
{
    ...
    if ( enable ) {         /* enable loopback */
        data16 = 0;
        SOC_IF_ERROR_RETURN
            (_phy_8481_copper_lb_get(unit, port, &lbmood));
        ...
        if ( speed >= 2500 ) {     /*  10G / 5G / 2.5G  */
            ...
            if ( speed == 2500 ) {
                data16 = (MGBASE_AN_CTRL_SPEED_2P5G | MGBASE_AN_CTRL_SPEED_MASK);
            } else if ( speed == 5000 ) {
                data16 = (MGBASE_AN_CTRL_SPEED_5G   | MGBASE_AN_CTRL_SPEED_MASK);
            }
        } else {                    /* 1G / 100M / 10M */
            /* 1000BASE-T/100BASE-TX MII Contol Reg. DEVAD=7, Addr=0xFFE0    */
                                   ...
                                   soc_port_ability_t *ability)
{
    ...
    if ( PHY_IS_BCM8486X(pc) ) {      /* Mako supports 5G/2.5G */
        ability->speed_full_duplex |= (SOC_PA_SPEED_5000MB |
                                       SOC_PA_SPEED_2500MB);
    }
