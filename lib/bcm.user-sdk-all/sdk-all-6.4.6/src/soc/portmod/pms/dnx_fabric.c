...
STATIC
int dnx_fabric_port_phy_index_get(int unit, int port, uint32 *phy_index){
    int first_phy = 0;
    int sub_phy = 0;
    SOCDNX_INIT_FUNC_DEFS;

    SOCDNX_IF_ERR_EXIT(portmod_port_first_phy_get(unit, port, &first_phy, &sub_phy));
    /* *phy_index = first_phy - pm_info->pm_data.dnx_fabric->first_lane;*/
    *phy_index = first_phy % 4;
exit:
    SOCDNX_FUNC_RETURN;
}
...
int dnx_fabric_wb_buffer_init(int unit, int wb_buffer_index, pm_info_t pm_info)
{
    ...
    SOC_WB_ENGINE_ADD_BUFF(SOC_WB_ENGINE_PORTMOD, wb_buffer_index, "dnx_fabric", NULL, VERSION(1), 1, SOC_WB_ENGINE_PRE_RELEASE);
...
{   
    const portmod_dnx_fabric_create_info_t *info = &pm_add_info->pm_specific_info.dnx_fabric;
    ...
    fabric_data = NULL;
    fabric_data = sal_alloc(sizeof(*(pm_info->pm_data.dnx_fabric)), "specific_db");    
    ...
    {
        fabric_data->core_type = phymodDispatchTypeCount; 
    } else {
        /* Probe */
        SOCDNX_IF_ERR_EXIT(phymod_core_probe(&info->access, &(fabric_data->core_type)));
    }
    ...
    SOC_PBMP_ASSIGN(fabric_data->phys, pm_add_info->phys);
...
int dnx_fabric_core_init_phase1(int unit, int port, pm_info_t pm_info, const portmod_port_add_info_t* add_info)
{
    ...
    int phyn, lane_index, core_index;
    uint32 reg_val, is_core_probed;
    ...
    if(!PHYMOD_CORE_INIT_F_RESUME_AFTER_FW_LOAD_GET(add_info)) {
        ...
        for(lane_index = 0; lane_index < DNX_FABRIC_LANES_PER_CORE;lane_index++){            
            ...
            SOCDNX_IF_ERR_EXIT(portmod_port_phychain_core_probe(core_access, fabric_data->nof_phys));
            for (phyn = 0; phyn < fabric_data->nof_phys; phyn++) {
                if (0 == phyn) { 
    ...
    if(PORTMOD_PORT_ADD_F_FIRMWARE_LOAD_VERIFY_GET(add_info)) {
        ...
        PHYMOD_CORE_INIT_F_FIRMWARE_LOAD_VERIFY_CLR(&core_config);
    }
    
    /* Init Core */
    /* lane of inner phy might be connected to the lane in differrent cores in outer phy,
          and all the different cores should be initialized in outer phy*/
    /*for(lane_index = 0; lane_index < DNX_FABRIC_LANES_PER_CORE;lane_index++){
        SOCDNX_IF_ERR_EXIT(dnx_fabric_lane_phychain_core_access_get(unit, lane_index, 1, pm_info, core_access));
        SOCDNX_IF_ERR_EXIT(portmod_port_phychain_core_init(core_access, fabric_data->nof_phys, &core_config, &status));
    }*/

    SOCDNX_IF_ERR_EXIT(dnx_fabric_lane_phychain_core_access_get(unit, 0, 1, pm_info, core_access));
    SOCDNX_IF_ERR_EXIT(portmod_port_phychain_core_init(core_access, fabric_data->nof_phys, &core_config, &status));
...
int dnx_fabric_port_attach(int unit, int port, pm_info_t pm_info, const portmod_port_add_info_t* add_info)
{
    ...
    SOCDNX_IF_ERR_EXIT(rv);
    if(!is_core_probed || (is_core_probed == 1 && PORTMOD_PORT_ADD_F_INIT_RESUME_AFTER_FW_LOAD_GET(add_info))){ 
    ...
    rv = SOC_WB_ENGINE_GET_ARR(unit, SOC_WB_ENGINE_PORTMOD, pm_info->wb_vars_ids[wb_polarity_rx], &init_config.polarity.rx_polarity, port % DNX_FABRIC_LANES_PER_CORE);
    SOCDNX_IF_ERR_EXIT(rv);
    rv = SOC_WB_ENGINE_GET_ARR(unit, SOC_WB_ENGINE_PORTMOD, pm_info->wb_vars_ids[wb_polarity_tx], &init_config.polarity.tx_polarity, port % DNX_FABRIC_LANES_PER_CORE);
    SOCDNX_IF_ERR_EXIT(rv);
...
int dnx_fabric_port_enable_set(int unit, int port, pm_info_t pm_info, int flags, int enable)
{
    ...
    if ((flags & PORTMOD_PORT_ENABLE_RX) && (flags & PORTMOD_PORT_ENABLE_MAC))
    {
        SOCDNX_IF_ERR_EXIT(dnx_fabric_port_phy_index_get(unit, port, &phy_index));
        ...
        if (enable)
        {
            ...
            *field_val = soc_reg_field_get(unit, FMAC_RECEIVE_RESET_REGISTERr, reg_val, FMAC_RX_RST_Nf);
            SHR_BITCLR(field_val, phy_index);
        ...
        else
        {
            SOCDNX_IF_ERR_EXIT(READ_FMAC_RECEIVE_RESET_REGISTERr(unit, fabric_data->fmac_schan_id | SOC_REG_ADDR_SCHAN_ID_MASK, &reg_val));
            *field_val = soc_reg_field_get(unit, FMAC_RECEIVE_RESET_REGISTERr, reg_val, FMAC_RX_RST_Nf);
            SHR_BITSET(field_val, phy_index);
            soc_reg_field_set(unit, FMAC_RECEIVE_RESET_REGISTERr, &reg_val, FMAC_RX_RST_Nf, *field_val);
            SOCDNX_IF_ERR_EXIT(WRITE_FMAC_RECEIVE_RESET_REGISTERr(unit, fabric_data->fmac_schan_id | SOC_REG_ADDR_SCHAN_ID_MASK, reg_val));
    ...
    else
    {
        ...
        if(enable){
                ...
                SOCDNX_IF_ERR_EXIT(phymod_phy_tx_lane_control_set(&phy_access, phymodTxElectricalIdleEnable));
                

                SOCDNX_IF_ERR_EXIT(phymod_phy_reset_get(&phy_access, &phy_reset));           
                phy_reset.tx = phymodResetDirectionOut;
                phy_reset.rx = phymodResetDirectionOut;
                ...
                *field_val = soc_reg_field_get(unit, FMAC_RECEIVE_RESET_REGISTERr, reg_val, FMAC_TX_RST_Nf);
                ...
                SOCDNX_IF_ERR_EXIT(phymod_phy_tx_lane_control_set(&phy_access, phymodTxElectricalIdleDisable));
        ...
        else
        {
                ...
                *field_val = soc_reg_field_get(unit, FMAC_RECEIVE_RESET_REGISTERr, reg_val, FMAC_TX_RST_Nf);
                ...
                SOCDNX_IF_ERR_EXIT(phymod_phy_reset_get(&phy_access, &phy_reset));
                phy_reset.tx = phymodResetDirectionIn;
                phy_reset.rx = phymodResetDirectionIn;
                SOCDNX_IF_ERR_EXIT(phymod_phy_reset_set(&phy_access, &phy_reset));
...
int dnx_fabric_port_enable_get(int unit, int port, pm_info_t pm_info, int flags, int *enable)
{
    ...
    *field_val = soc_reg_field_get(unit, FMAC_RECEIVE_RESET_REGISTERr, reg_val, FMAC_RX_RST_Nf);
    ...
    {
        SOCDNX_IF_ERR_EXIT(phymod_phy_reset_get(&phy_access, &phy_reset)); 
        *enable = ((!mac_reset) && (phy_reset.rx == phymodResetDirectionOut));
    }
...
int dnx_fabric_port_interface_config_set(int unit, int port, pm_info_t pm_info, const portmod_port_interface_config_t* config, int all_phy)
{
    phymod_phy_inf_config_t interface_config;
    phymod_phy_access_t phy;
    uint32 phy_index = 0, val, reg_val;
    int phys_num;
    int fsrd_schan_id, fsrd_internal_quad;
    dnx_fabric_t fabric_data = pm_info->pm_data.dnx_fabric;
    portmod_access_get_params_t params;
...
int dnx_fabric_port_interface_config_get(int unit, int port, pm_info_t pm_info, portmod_port_interface_config_t* config)
{
...
int dnx_fabric_port_encoding_set(int unit, int port, pm_info_t pm_info, uint32 properties, portmod_port_pcs_t encoding)
{
    ...
    SOCDNX_IF_ERR_EXIT(READ_FMAC_ASYNC_FIFO_CONFIGURATIONr(unit, fmac_blk_id_masked, phy_index, &reg64_val));
    soc_reg64_field32_set(unit, FMAC_ASYNC_FIFO_CONFIGURATIONr, &reg64_val, FMAL_N_RX_SLOW_READ_RATEf, slow_read_rate);
    SOCDNX_IF_ERR_EXIT(WRITE_FMAC_ASYNC_FIFO_CONFIGURATIONr(unit, fmac_blk_id_masked, phy_index, reg64_val));
    ...
    SOCDNX_IF_ERR_EXIT(WRITE_FMAC_FPS_RX_FEC_CONFIGURATIONr(unit, fmac_blk_id_masked, phy_index, reg_val));

    SOCDNX_IF_ERR_EXIT(READ_FMAC_FPS_TX_CONFIGURATIONr(unit, fmac_blk_id_masked, phy_index, &reg_val));
    soc_reg_field_set(unit,FMAC_FPS_TX_CONFIGURATIONr,&reg_val,FPS_N_TX_FEC_ENf, fec_enable);
    SOCDNX_IF_ERR_EXIT(WRITE_FMAC_FPS_TX_CONFIGURATIONr(unit, fmac_blk_id_masked, phy_index, reg_val));
    ...
    SOCDNX_IF_ERR_EXIT(WRITE_FMAC_FMAL_RX_GENERAL_CONFIGURATIONr(unit, fmac_blk_id_masked, phy_index, reg_val));

    SOCDNX_IF_ERR_EXIT(READ_FMAC_RSF_CONFIGURATIONr(unit, fmac_blk_id_masked, phy_index, &reg64_val));
    soc_reg64_field32_set(unit, FMAC_RSF_CONFIGURATIONr, &reg64_val, RSF_N_RX_ERR_MARK_ENf, rsf_err_mark);
    soc_reg64_field32_set(unit, FMAC_RSF_CONFIGURATIONr, &reg64_val, RSF_N_RX_HALF_FRAMEf, rx_half_frame);
    soc_reg64_field32_set(unit, FMAC_RSF_CONFIGURATIONr, &reg64_val, RSF_N_TX_HALF_FRAMEf, tx_half_frame);
    soc_reg64_field32_set(unit, FMAC_RSF_CONFIGURATIONr, &reg64_val, RSF_N_TX_SCRAMBLE_LLFC_BITS_ENf, 0x1);
    soc_reg64_field32_set(unit, FMAC_RSF_CONFIGURATIONr, &reg64_val, RSF_N_RX_SCRAMBLE_LLFC_BITS_ENf, 0x1);
    SOCDNX_IF_ERR_EXIT(WRITE_FMAC_RSF_CONFIGURATIONr(unit, fmac_blk_id_masked, phy_index, reg64_val));
...
int dnx_fabric_port_encoding_get(int unit, int port, pm_info_t pm_info, uint32 *properties, portmod_port_pcs_t *encoding)
{
    ...
    *properties = 0;
    ...
    case 4:
        SOCDNX_IF_ERR_EXIT(READ_FMAC_RSF_CONFIGURATIONr(unit, fmac_blk_id_masked, phy_index, &reg64_val));
        rx_half_frame = soc_reg64_field32_get(unit, FMAC_RSF_CONFIGURATIONr, reg64_val, RSF_N_RX_HALF_FRAMEf);
        if(rx_half_frame == 0){
    ...
    {
        SOCDNX_IF_ERR_EXIT(READ_FMAC_RSF_CONFIGURATIONr(unit, fmac_blk_id_masked, phy_index, &reg64_val));
        err_mark = soc_reg64_field32_get(unit, FMAC_RSF_CONFIGURATIONr, reg64_val, RSF_N_RX_ERR_MARK_ENf);
...
_dnx_fabric_force_signal_detect_set(int unit, pm_info_t pm_info)
{
    ...
    SOCDNX_IF_ERR_EXIT(READ_FMAC_GENERAL_CONFIGURATION_REGISTERr(unit, fmac_blk_id_masked, &reg_val));
    soc_reg_field_set(unit, FMAC_GENERAL_CONFIGURATION_REGISTERr, &reg_val, FORCE_SIGNAL_DETECTf, force_signal_detect);
    SOCDNX_IF_ERR_EXIT(WRITE_FMAC_GENERAL_CONFIGURATION_REGISTERr(unit, fmac_blk_id_masked, reg_val));
...
int dnx_fabric_mac_loopback_set(int unit, int port, pm_info_t pm_info, portmod_loopback_mode_t loopback_type, int enable)
{
    ...
    case portmodLoopbackMacAsyncFifo:
        SOCDNX_IF_ERR_EXIT(READ_FMAC_FMAL_GENERAL_CONFIGURATIONr(unit, fmac_blk_id_masked, phy_index, &reg_val));
        soc_reg_field_set(unit, FMAC_FMAL_GENERAL_CONFIGURATIONr, &reg_val, FMAL_CORE_LOOPBACKf, enable);
        SOCDNX_IF_ERR_EXIT(WRITE_FMAC_FMAL_GENERAL_CONFIGURATIONr(unit, fmac_blk_id_masked, phy_index, reg_val));
...
int dnx_fabric_port_loopback_get(int unit, int port, pm_info_t pm_info, portmod_loopback_mode_t loopback_type, int* enable)
{
    ...
    case portmodLoopbackMacAsyncFifo:
        SOCDNX_IF_ERR_EXIT(READ_FMAC_FMAL_GENERAL_CONFIGURATIONr(unit, fmac_blk_id_masked, phy_index, &reg_val));
        *enable = soc_reg_field_get(unit, FMAC_FMAL_GENERAL_CONFIGURATIONr, reg_val, FMAL_CORE_LOOPBACKf);
        break;
    case portmodLoopbackMacOuter:
        SOCDNX_IF_ERR_EXIT(READ_FMAC_LOOPBACK_ENABLE_REGISTERr(unit, fmac_blk_id_masked, &reg_val));
        *field = soc_reg_field_get(unit, FMAC_LOOPBACK_ENABLE_REGISTERr, reg_val, LCL_LPBK_ONf);
...
int dnx_fabric_port_link_get(int unit, int port, pm_info_t pm_info, int* link)
{
    int    enable;
    uint32 phy_index;
    uint32 
...
{   portmod_lane_connection_t *candidate_lane_connection = NULL;
    ...
    lane_index = 0;
    SOC_PBMP_ITER(pm_info->pm_data.dnx_fabric->phys, phy_index) {
        if (phy_index == iphy) {
            break;
        }
        lane_index++;
...
int dnx_fabric_port_cl72_set(int unit, int port, pm_info_t pm_info, int enable)
{
    phymod_phy_access_t phy_access;
    int chain_length = 0;
    SOCDNX_INIT_FUNC_DEFS;
    ...
    SOCDNX_IF_ERR_EXIT(portmod_port_chain_phy_access_get(unit, port, pm_info, &phy_access,
                                                        1, &chain_length));
    SOCDNX_IF_ERR_EXIT(portmod_port_phychain_cl72_set(&phy_access, chain_length, enable));

exit:
    SOCDNX_FUNC_RETURN;
}

int dnx_fabric_port_cl72_get(int unit, int port, pm_info_t pm_info, int* enable)
{
    phymod_phy_access_t phy_access;
    int chain_length = 0;
    uint32_t tmp_enable;
    ...
    SOCDNX_IF_ERR_EXIT(portmod_port_chain_phy_access_get(unit, port, pm_info, &phy_access,
                                                        1, &chain_length));
    SOCDNX_IF_ERR_EXIT(portmod_port_phychain_cl72_get(&phy_access, chain_length, &tmp_enable));
    *enable = tmp_enable;
