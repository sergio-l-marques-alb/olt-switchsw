...
#if defined(PORTMOD_PM4X10Q_SUPPORT)
...
int unimac_init(int unit, soc_port_t port, int is_auto_cfg, uint32 rsv_mask, int init_flags)
{
    ...
    /*lcc_drain_timeout[unit] = soc_property_get(unit, spn_LCCDRAIN_TIMEOUT_USEC, 250000);
    ...
    frame_max = /*IS_ST_PORT(unit, port) ? JUMBO_MAXSZ :*/ SOC_UNIMAC_MAX_FRAME_SIZE;
    unimac_frame_max_set(unit, port, frame_max);
    ...
    soc_reg_field_set(unit, COMMAND_CONFIGr, &command_config, ENA_EXT_CONFIGf, auto_cfg ? 1 : 0);
    ...
    if (rsv_mask) { /* default val is 0x070 */
        SOC_IF_ERROR_RETURN(WRITE_GPORT_RSV_MASKr(unit, port, rsv_mask)); 
        SOC_IF_ERROR_RETURN(WRITE_GPORT_STAT_UPDATE_MASKr(unit, port, rsv_mask));
    }

    /* Bring the UniMAC out of reset */
    SOC_IF_ERROR_RETURN(unimac_port_soft_reset_set(unit, port, FALSE));
...
int unimac_loopback_set(int unit, soc_port_t port, int enable)
{
    ...
    SOC_IF_ERROR_RETURN(unimac_port_soft_reset_set(unit, port, TRUE));
...
int unimac_soft_reset_get(int unit, soc_port_t port, int &enable)
{
    ...
    SOC_IF_ERROR_RETURN(READ_COMMAND_CONFIGr(unit, port, &command_config));
    *enable = soc_reg_field_set(unit, COMMAND_CONFIGr, &command_config, SW_RESETf, is_reset);
...
int unimac_duplex_set(int unit, soc_port_t port, int duplex)
{
    ...
    /* First put the MAC in reset */ 
    SOC_IF_ERROR_RETURN(unimac_port_soft_reset(unit, port, TRUE));

    soc_reg_field_set(unit, COMMAND_CONFIGr, &command_config, SW_RESETf, 1);
    SOC_IF_ERROR_RETURN(WRITE_COMMAND_CONFIGr(unit, port, command_config));
 
    /* Set IPG to match new duplex */
    SOC_IF_ERROR_RETURN(unimac_ipg_update(unit, port));

    /* Bring the MAC out of reset */
    SOC_IF_ERROR_RETURN(unimac_port_soft_reset(unit, port, FALSE));
...
int unimac_speed_set(int unit, soc_port_t port, int speed)
{
    ...
    SOC_IF_ERROR_RETURN(READ_COMMAND_CONFIGr(unit, port, &command_config));
    cur_speed = soc_reg_field_get(unit, COMMAND_CONFIGr, command_config, ETH_SPEEDf);
...
int unimac_rx_enable_set (int unit, soc_port_t port, int enable)
{
    ...
    soc_reg32_field_set(unit, COMMAND_CONFIGr, &reg_val, RX_ENAf, enable ? 1: 0);
...
int unimac_rx_enable_get (int unit, soc_port_t port, int *enable)
{
    ...
    SOC_IF_ERROR_RETURN(READ_COMMAND_CONFIGr(unit, port, &reg_val));
    *enable = soc_reg32_field_get(unit, COMMAND_CONFIGr, &reg_val, RX_ENAf);
...
int unimac_tx_preamble_length_set(int unit, soc_port_t port, int length)
{
    ...
    soc_reg32_field_set(unit, TX_PREAMBLEr, &reg_val, TX_PREAMBLEf, length);
...
int unimac_eee_set(int unit, int port, const portmod_eee_t* eee)
{
    ...
    soc_reg32_field_set(unit, UMAC_EEE_CTRLr, &reg_val, EEE_ENf, eee->enable);
    SOC_IF_ERROR_RETURN(WRITE_UMAC_EEE_CTRLr(unit ,port, reg_val));

    SOC_IF_ERROR_RETURN(unimac_speed_get(unit, port, &speed));
    if (speed > SOC_UNIMAC_MAX_EEE_SPEED) {
        LOG_VERBOSE(BSL_LS_SOC_COMMON,(BSL_META_U(unit, "max speed for EEE is 1000Mbps"), length));
        return (SOC_E_PARAM);
    }
    ...
    soc_reg32_field_set(unit, reg, &reg_val, field, eee->tx_wake_time);
    ...
    field = (speed == 1000) ? GMII_EEE_DELAY_ENTRY_TIMERf : MII_EEE_DELAY_ENTRY_TIMERf;

    SOC_IF_ERROR_RETURN(soc_reg32_get(unit, reg, port, 0, &reg_val));
    soc_reg32_field_set(unit, reg, &reg_val, field, eee->tx_idle_time);
    SOC_IF_ERROR_RETURN(soc_reg32_set(unit, reg, port, 0, reg_val));
...
int unimac_eee_get(int unit, int port, const portmod_eee_t* eee)
{
    ...
    SOC_IF_ERROR_RETURN(READ_UMAC_EEE_CTRLr(unit, port, &reg_val));
    pEEE->enable = soc_reg32_field_get (unit, UMAC_EEE_CTRLr, reg_val, EEE_ENf);
    ...
    SOC_IF_ERROR_RETURN(soc_reg32_get(unit, reg, port, 0, &reg_val));
    pEEE->tx_wake_time = soc_reg32_field_get(unit, reg, &reg_val, field);

    reg = (speed == 1000) ? GMII_EEE_DELAY_ENTRY_TIMERr : MII_EEE_DELAY_ENTRY_TIMERr;
    field = (speed == 1000) ? GMII_EEE_DELAY_ENTRY_TIMERf : MII_EEE_DELAY_ENTRY_TIMERf;

    SOC_IF_ERROR_RETURN(soc_reg32_get(unit, reg, port, 0, &reg_val));
    pEEE->tx_idle_time = soc_reg32_field_get(unit, reg, &reg_val, field);
                          ...
                          const portmod_pfc_config_t* pfc_cfg)
{
    ...
    soc_reg32_field_set(unit, MAC_PFC_TYPEr, &reg_val, PFC_ETH_TYPEf, pfc_cfg->type);
    SOC_IF_ERROR_RETURN(WRITE_MAC_PFC_TYPEr(unit ,port, reg_val));

    SOC_IF_ERROR_RETURN(READ_MAC_PFC_OPCODEr(unit ,port, &reg_val));
    soc_reg32_field_set(unit, MAC_PFC_OPCODEr, &reg_val, PFC_OPCODEf, pfc_cfg->opcode);
    ...
    soc_reg32_field_set(unit, MAC_PFC_DA_1r, &reg_val, PFC_MACDA_1f, pfc_cfg->da_oui >> 8);
                          ...
                          const portmod_pfc_config_t* pfc_cfg)
{
    ...
    SOC_IF_ERROR_RETURN(READ_MAC_PFC_TYPEr(unit, port, &reg_val));
    pcfg->type = soc_reg32_field_get(unit, MAC_PFC_TYPEr, reg_val, PFC_ETH_TYPEf);

    SOC_IF_ERROR_RETURN(READ_MAC_PFC_OPCODEr(unit, port, &reg_val));
    pcfg->opcode = soc_reg32_field_get(unit, MAC_PFC_OPCODEr, reg_val, PFC_OPCODEf);

    SOC_IF_ERROR_RETURN(READ_MAC_PFC_DA_0r(unit, port, &reg_val));
    fval0 = soc_reg32_field_get(unit, MAC_PFC_DA_0r, reg_val, PFC_MACDA_0f);

    SOC_IF_ERROR_RETURN(READ_MAC_PFC_DA_1r(unit, port, &reg_val));
    fval1 = soc_reg32_field_get(unit, MAC_PFC_DA_1r, reg_val, PFC_MACDA_1f);
    pcfg->da_oui = (fval0 >> 24) | (fval1 << 8);

    SOC_IF_ERROR_RETURN(READ_MAC_PFC_DA_0r(unit, port, &reg_val));
    fval = soc_reg32_field_get(unit, MAC_PFC_DA_0r, reg_val, PFC_MACDA_0f);
                                ...
                                const portmod_fifo_status_t* diag_info)
{
    ...
    if (soc_reg_field_get(unit, unimac_TX_TIMESTAMP_FIFO_STATUSr, reg_val, TX_TS_FIFO_EMPTYf)) {
        return SOC_E_EMPTY;
    }
...
int unimac_pass_control_frame_set(int unit, int port, int value)
{
    ...
    SOC_IF_ERROR_RETURN(READ_MAC_PFC_CTRLr(unit, port, &rval));
    soc_reg_field_set(unit, MAC_PFC_CTRLr, &rval, RX_PASS_PFC_FRMf, value ? 1 : 0);
    SOC_IF_ERROR_RETURN(WRITE_MAC_PFC_CTRLr(unit, port, rval));
...
{   
    ...
    drain_cells->rx_pfc_en = soc_reg64_field32_get(unit, MAC_PFC_CTRLr, reg_val, PFC_RX_ENBLf);

    
    drain_cells->llfc_en = 0;
                                
    SOC_IF_ERROR_RETURN(READ_MAC_MODEr(unit, port , &reg_val));
    drain_cells->rx_pause  = soc_reg_field_get(unit, MAC_MODEr, rval, MAC_RX_PAUSEf);
    drain_cells->tx_pause  = soc_reg_field_get(unit, MAC_MODEr, rval, MAC_TX_PAUSEf);
...
STATIC int
_unimac_drain_cells(int unit, soc_port_t port)
{
    int                 rv;
    uint32              flush_ctrl;
    ...
    SOC_IF_ERROR_RETURN(READ_FLUSH_CONTROLr(unit, port, &flush_ctrl));
	soc_reg_field_set(unit, FLUSH_CONTROLr, &flush_ctrl, FLUSHf, 1);
	SOC_IF_ERROR_RETURN(WRITE_FLUSH_CONTROLr(unit, port, flush_ctrl));

    /* Disable pause function */
    SOC_IF_ERROR_RETURN(soc_mac_uni.md_pause_get(unit, port, &pause_tx, &pause_rx));
    SOC_IF_ERROR_RETURN(soc_mac_uni.md_pause_set(unit, port, 0, 0));
    ...
    SOC_IF_ERROR_RETURN(soc_mac_uni.md_pause_set(unit, port, pause_tx, pause_rx));
