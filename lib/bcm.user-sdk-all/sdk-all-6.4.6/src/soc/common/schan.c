...
_soc_schan_cmicm_poll_wait(int unit, schan_msg_t *msg)
{
    ...
    if (schanCtrl & SC_CMCx_MSG_NAK) {
        ...
        resp_msg.dwords[1] =
            soc_pci_read(unit, CMIC_CMCx_SCHAN_MESSAGEn(cmc, 1));            
...
# if 0  /* #ifdef PLISIM */
        if (SAL_BOOT_PLISIM) {
            /*
             * Reach here if code is running under PCID. In that case,
             * artificially make sure the SC_CMCx_MSG_DONE flag is set.
             */
            uint32 schanCtrl ;
            int cmc = SOC_PCI_CMC(unit);

            schanCtrl = soc_pci_read(unit, CMIC_CMCx_SCHAN_CTRL_OFFSET(cmc)) ;
...
int phy_mem_read(int unit, phy_mem_t *mem, void *entry_data)
{
  ...
  /*
   * Always access memory (but be ready to change to register, if required).
   */
  is_mem = 1 ;
  /*
