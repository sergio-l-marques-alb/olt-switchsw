    ...
    while ((interval = soc->mem_scan_interval) != 0) {
        ...
        if (SOC_IS_TOMAHAWK(unit)) {
            ...
            if (SOC_MEM_SER_CORRECTION_TYPE(unit, mem) == 0) {
                LOG_VERBOSE(BSL_LS_SOC_SER, (BSL_META_U(unit, "nti_scrub: skipping mem %s (ERR_CORRECTION = 0)\n"), SOC_MEM_NAME(unit, mem)));
                continue;
            }
            if (SOC_MEM_INFO(unit, mem).flags & SOC_MEM_FLAG_SER_ENTRY_CLEAR) {
                LOG_VERBOSE(BSL_LS_SOC_SER, (BSL_META_U(unit, "nti_scrub: skipping mem %s (SER_RESPONSE = SER_ENTRY_CLEAR)\n"), SOC_MEM_NAME(unit, mem)));
                continue;
            }
            if (soc_th_check_scrub_skip(unit, mem, 0)) {
                continue;
            }
        }
#endif /* BCM_TOMAHAWK_SUPPORT */
            ...
            if (!soc_mem_cache_get(unit, mem, MEM_BLOCK_ALL)) {
               ...
               if (SOC_IS_TOMAHAWK(unit)) {
                   ...
                   case EGR_VLAN_XLATE_LPm:
                       ...
                       /*scan these, even if they are non-cacheable */
                       skip = 0; break;
                   default:
                       break;
                   }
...
#if defined(BCM_TOMAHAWK_SUPPORT)
                if (SOC_IS_TOMAHAWK(unit)) {
                    int k, num_inst_to_scrub = 0, acc_type_list[8],
                        copyno = blk;
                    (void) soc_th_check_scrub_info(unit, mem, blk, copyno,
                                                   &num_inst_to_scrub,
                                                   acc_type_list);
