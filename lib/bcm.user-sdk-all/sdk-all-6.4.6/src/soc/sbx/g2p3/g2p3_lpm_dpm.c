...
/*
 * $Id: g2p3_lpm_dpm.c,v 1.6 Broadcom SDK $
 *
...
/* invalid extra bits uint16 value */
...
g2p3_lpmDPMInit(g2p3_lpmDPMHandle_t *pDpm,
                 ...
                 uint32 paylDmaBankA, uint32 paylDmaBankB, 
                 uint32 paylBanks,
    ...
    if (status != SB_OK) {
        ...
        if(ignore_status != SB_OK) {
          LOG_WARN(BSL_LS_SOC_COMMON,
                   (BSL_META_U(unit,
                               "g2p3_lpmDPMInit:  g2p3_lpmHostMemFree Failed "
                               "host %p"), vp));
        }
        return status;
    }
...
findDevicePayload(g2p3_lpmDPMHandle_t dpm,
                  ...
                  uint32 dropMaskLo, uint32 dropMaskHi)
                   ...
                   uint32 dropMaskLo, uint32 dropMaskHi)
{
...
#if LPM_DPM_DEBUG
            LOG_WARN(BSL_LS_SOC_COMMON,
                     (BSL_META("DPM ref: %p, %x, %x "), payload, clsA, clsB));
#endif
    if (dpm->busy)
    ...
    if (dp != NULL) {
        LOG_WARN(BSL_LS_SOC_COMMON,
                 (BSL_META("(found at host %p, device 0x%08x, refs %d)\n"), dp, dp->address, dp->refs));
    }
#endif
    if (dp != NULL) {
        SB_ASSERT(dp->refs > 0);
    ...
    if (status != SB_OK) {
        ...
        if(ignore_status != SB_OK) {
          LOG_WARN(BSL_LS_SOC_COMMON,
                   (BSL_META("g2p3_lpmDPMRef:  g2p3_lpmHostMemFree Failed "
                             "host %p"), vp));
        }
        return status;
    }
    ...
    if (dpm->numBanks > 1) {
      ...
      if (status != SB_OK) {
        ...
        if(ignore_status != SB_OK) {
          LOG_WARN(BSL_LS_SOC_COMMON,
                   (BSL_META("g2p3_lpmDPMRef:  g2p3_lpmDeviceMemFree[A] Failed "
                             "host %p Device 0x%08x"), (void *)dp, dp->address));
        }
        return status;
      }
...
#if LPM_DPM_DEBUG
    LOG_WARN(BSL_LS_SOC_COMMON,
             (BSL_META("(new at host %p, device 0x%08x)\n"), dp, dp->address));
#endif
    ...
    if (status != SB_OK) {
      ...
      if(ignore_status != SB_OK) {
        LOG_WARN(BSL_LS_SOC_COMMON,
                 (BSL_META("DPM deref: g2p3_lpmDeviceMemFree[A] Failed "
                           "host %p device 0x%08x"), (void *)dp, dp->address));
      }
      ...
      if (dpm->numBanks > 1) {
        ...
        if(ignore_status != SB_OK) {
          LOG_WARN(BSL_LS_SOC_COMMON,
                   (BSL_META("DPM deref: g2p3_lpmDeviceMemFree[B] Failed "
                             "host %p device 0x%08x"), (void *)dp, dp->address));
        }
      }
      ...
      if(ignore_status != SB_OK) {
        LOG_WARN(BSL_LS_SOC_COMMON,
                 (BSL_META("DPM deref:  g2p3_lpmHostMemFree Failed "
                           "host %p"), vp));
      }
      return status;
    }
                  ...
                  g2p3_lpmDevicePayload_p_t devicePayload)
{
    ...
    if(ignore_status != SB_OK) {
      LOG_WARN(BSL_LS_SOC_COMMON,
               (BSL_META("freeDevicePayload: g2p3_lpmDeviceMemFree Failed "
                         "host %p dev[A]Address 0x%08x"), (void *)dpm->deviceMemA, devicePayload->address));
    ...
    if (dpm->numBanks > 1) {
      ...
      LOG_WARN(BSL_LS_SOC_COMMON,
               (BSL_META("freeDevicePayload: g2p3_lpmDeviceMemFree Failed "
                         "host %p dev[B]Address 0x%08x"), (void *)dpm->deviceMemB, devicePayload->address));
      status = ignore_status;
    }
    ...
    if(ignore_status != SB_OK) {
      LOG_WARN(BSL_LS_SOC_COMMON,
               (BSL_META("freeDevicePayload:  g2p3_lpmHostMemFree Failed "
                         "host %p"), (void *)devicePayload));
      status = ignore_status;
    }
    return status;
}
...
#if LPM_DPM_DEBUG
            LOG_WARN(BSL_LS_SOC_COMMON,
                     (BSL_META("DPM deref: %p, %x, %x "), payload, clsA, clsB));
#endif
    dp = findDevicePayload(dpm, payload, clsA, clsB, dropMaskLo, dropMaskHi);
#if LPM_DPM_DEBUG
    if (dp == NULL)
...
#if LPM_DPM_DEBUG
              LOG_WARN(BSL_LS_SOC_COMMON,
                       (BSL_META("DPM commit: writing @0x%08x\n"), dmaAddr));
#endif
...
#if LPM_DPM_DEBUG
    LOG_WARN(BSL_LS_SOC_COMMON,
             (BSL_META("DPM commit: calling g2p3_lpmDmaFlush\n")));
#endif
...
g2p3_lpmDPMUninit(g2p3_lpmDPMHandle_t *pDpm)
{
    ...
    if(status != SB_OK) {
      LOG_WARN(BSL_LS_SOC_COMMON,
               (BSL_META("g2p3_lpmDPMUninit:  g2p3_lpmHostMemFree Failed "
                         "host %p"), (void *)dpm));
    }
    return status;
}
