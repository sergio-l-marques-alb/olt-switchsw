/*
 * $Id: g2p3_ace.c,v 1.134 Broadcom SDK $
 *
...
typedef struct soc_sbx_g2p3_ace_group_s {
    ...
    int16               uGroupId;
    uint32              uRuleCount;   /* counts only enabled rules */
#ifdef BCM_WARM_BOOT_SUPPORT
    soc_sbx_g2p3_ace_qset_t qset;
    uint8                 bAllQualifiers;
#endif
...
static sbStatus_t
soc_sbx_g2p3_ace_rt_qos_action_add(soc_sbx_g2p3_state_t *pFe , uint8 bBank, uint32 uIndex,
               soc_sbx_g2p3_irt_wrap_t *pAction);

static sbStatus_t
soc_sbx_g2p3_ace_rt_egr_action_add(soc_sbx_g2p3_state_t *pFe , uint8 bBank, uint32 uIndex,
...
soc_sbx_g2p3_ace_scache_recover(soc_sbx_g2p3_state_t *pFe){
    ...
    uint32 i, unit;
    soc_sbx_g2p3_ace_mgr_t *pClsAce;
    uint32 uRuleCount[SB_MAX_GROUPS];
        ...
        if (group[i].uGroupId != SOC_SBX_G2P3_ACE_GROUPID_INVALID) {
            ...
            if (err !=  SB_OK) {
                LOG_ERROR(BSL_LS_SOC_COMMON,
                          (BSL_META_U(unit,
                                      "ACE group 0x%x recovery failed (0x%x)"),
                           i, err));
                return err;
            }
            /* save rulecount */
        ...
        ACEDQ_TRAVERSE(&pDb->listGroups, pGroup, soc_sbx_g2p3_ace_group_t) {
            if (pGroup->uGroupId >= IFP_DEF_GROUP_IDX) {
                ...
                if (err != SB_OK) {
                    LOG_ERROR(BSL_LS_SOC_COMMON,
                              (BSL_META_U(unit,
                                          "ACE rule recovery for group 0x%x recovery failed (0x%x)"),
                               pGroup->uGroupId, err));
                    return err;
                }
            }else{
    ...
    LOG_CLI((BSL_META("DUMPING all recovered rules in ORDER!\n")));
    ACEDQ_TRAVERSE(&pClsAce->listRuleDbs, pDb, soc_sbx_g2p3_ace_db_t) {
                 ...
                 sbIngressConfigParams_p_t iParams)
{
  ...
  int                rv;
  uint32           uNumRules = 0;
  uint32           uSchema, uSchemaRules = 0;
  soc_sbx_g2p3_counter_params_t cp;
  uint32 isC2 = SAND_HAL_IS_FE2KXT(pFe->regSet);
  uint32 uMaxDb;
  uint8 v6Enable;
  ...
  sal_memset(&qset_dummy, 0, sizeof(soc_sbx_g2p3_ace_qset_t));
...
#ifdef RULE_ADD_DBG
  LOG_CLI((BSL_META("MAX_RULES = 0x%x\n"), uNumRules));
  LOG_CLI((BSL_META("RULE PER CHUNK: 0x%x, CHUNKS: 0x%x\n"), pClsAce->ulRulesPerChunk, MAX_MEM_CHUNKS));
#endif
                    ...
                    int16 uGroupId)
{
  ...
  soc_sbx_g2p3_ace_db_t *pDb = NULL;
  uint32 uCapacity, uLength = 0, isC2;
  uint8 bAvailable = 0;
  ...
  if (!isC2) {
      ...
      if ( uLength >= uCapacity) {
          ...
          LOG_CLI((BSL_META(" cannot add additional rules\n")));
#endif
...
soc_sbx_g2p3_ace_rule_handle_print(soc_sbx_g2p3_state_t *pFe, soc_sbx_g2p3_ace_mgr_rule_t *pRuleHandle)
{
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("RuleHandle = %p\n"), (void *)pRuleHandle));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("RuleHandle.ulMemChunkId    = 0x%x\n"), pRuleHandle->ulMemChunkId));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("RuleHandle.bEnable         = 0x%x\n"), pRuleHandle->bEnable     ));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("RuleHandle.bcommitted       = 0x%x\n"), pRuleHandle->bcommitted   ));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("RuleHandle.uRuleIdx        = 0x%x\n"), pRuleHandle->uRuleIdx    ));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("RuleHandle.uRuleOffset     = 0x%x\n"), pRuleHandle->uRuleOffset ));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("RuleHandle.uOldRuleIdx     = 0x%x\n"), pRuleHandle->uOldRuleIdx    ));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("RuleHandle.uOldRuleOffset  = 0x%x\n"), pRuleHandle->uOldRuleOffset ));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("RuleHandle.uNumPktHit   = 0x%x\n"), COMPILER_64_LO((pRuleHandle->counts).packets)    ));
  /* print pattern */
                                ...
                                soc_sbx_g2p3_cls_rule_db_e_t eDb)
{
    ...
    soc_sbx_g2p3_ace_mgr_rule_t *pRule       = NULL;
    uint8 bRuleEnabled                     = FALSE;
    soc_sbx_g2p3_counter_value_t cv;

    uint32 uRuleCount = 0, uRSet=0; /* uRcInstance=0,*/
    uint32 uRuleIdx = 0;
    uint32 uIndex = 0;
    uint32 oldRuleOffset = 0;
    uint8  uProgNumber = 0;
    sbStatus_t err = SB_OK;
    soc_sbx_g2p3_ace_mgr_t *pClsAce  = pFe->pClsAce;
    uint32 isC2 = SAND_HAL_IS_FE2KXT(pFe->regSet);
                ...
                if(bRuleEnabled) {
                    ...
                    while (err!=SB_OK) {
                        if(pRule->Rule.eActionType == SB_G2P3_CLS_QOS) {
                            ...
                            LOG_WARN(BSL_LS_SOC_COMMON,
                                     (BSL_META("ERROR: Wrong ActionType\n")));
                        }
                        if (err != SB_OK) {
                            LOG_WARN(BSL_LS_SOC_COMMON,
                                     (BSL_META("WARNING: Adding rule to table failed, err is %d\n"), err));
                            return err;
                        }
                    }

                    uRSet = uRuleCount/pSchemaHandler->layout.uNumRulesPerRs;
                    COMPILER_REFERENCE(uRSet);
                               ...
                               uint32 uRsOffset)
{
    ...
    if (pRule) {
        ...
        LOG_WARN(BSL_LS_SOC_COMMON,
                 (BSL_META("ERROR: unexpectedly NULL rule pointer\n")));
        SB_ASSERT(0);
    }
    ...
    if (pSchema == NULL) {
        LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META("ERROR: pSchema NULL\n")));
        return SB_FAILED;
    }
    SB_ASSERT(pPatternMemory != NULL);
    if (pPatternMemory == NULL) {
        LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META("ERROR: pPatternMemory NULL\n")));
        return SB_FAILED;
    }
...
#if 0
        LOG_CLI((BSL_META("encoding rule 0x%x\n"), uRuleIndex + pRule->uRuleOffset));
                                ...
                                soc_sbx_g2p3_cls_rule_db_e_t eDb)
{
    ...
    uint8 uGroupCount = 0; /*, uLastGroupWithRule = -1; */
    soc_sbx_g2p3_ace_group_t *pGroup;
...
{
  uint32               ulLen, ulChunkTodo, ulChunkId, ulWorkChunk, ulListLen;
  soc_sbx_g2p3_ace_mgr_t     *pClsAce;
  soc_sbx_g2p3_ace_db_t      *pDb;
...
{
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("G2P3FeClsAcePattern:: dbtypeany=0x%01x"), (unsigned int)  pFromStruct->bDbTypeAny));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" portany=0x%01x"), (unsigned int)  pFromStruct->bPortAny));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" priorityany=0x%01x"), (unsigned int)  pFromStruct->bPriorityAny));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" fragmentany=0x%01x"), (unsigned int)  pFromStruct->bFragmentAny));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("\n")));

  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("G2P3FeClsAcePattern:: tcpflagurgany=0x%01x"), (unsigned int)  pFromStruct->bTcpFlagURGAny));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" tcpflagackany=0x%01x"), (unsigned int)  pFromStruct->bTcpFlagACKAny));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" tcpflagpshany=0x%01x"), (unsigned int)  pFromStruct->bTcpFlagPSHAny));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("\n")));

  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("G2P3FeClsAcePattern:: tcpflagrstany=0x%01x"), (unsigned int)  pFromStruct->bTcpFlagRSTAny));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" tcpflagsynany=0x%01x"), (unsigned int)  pFromStruct->bTcpFlagSYNAny));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" tcpflagfinany=0x%01x"), (unsigned int)  pFromStruct->bTcpFlagFINAny));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("\n")));

  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("G2P3FeClsAcePattern:: dscpany=0x%01x"), (unsigned int)  pFromStruct->bDscpAny));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" ecnany=0x%01x"), (unsigned int)  pFromStruct->bEcnAny));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" protocolany=0x%01x"), (unsigned int)  pFromStruct->bProtocolAny));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" ipv4saany=0x%01x"), (unsigned int)  pFromStruct->bIpv4SaAny));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("\n")));

  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("G2P3FeClsAcePattern:: ipv4daany=0x%01x"), (unsigned int)  pFromStruct->bIpv4DaAny));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" ethertypeany=0x%01x"), (unsigned int)  pFromStruct->bEtherTypeAny));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" smacany=0x%01x"), (unsigned int)  pFromStruct->bSmacAny));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" vlanany=0x%01x"), (unsigned int)  pFromStruct->bVlanAny));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("\n")));

  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("G2P3FeClsAcePattern:: dmacany=0x%01x"), (unsigned int)  pFromStruct->bDmacAny));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" dbtype=0x%02x"), (unsigned int)  pFromStruct->uDbType));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" port=0x%02x"), (unsigned int)  pFromStruct->uPort));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" priority=0x%01x"), (unsigned int)  pFromStruct->uPriority));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" fragment=0x%01x"), (unsigned int)  pFromStruct->uFragment));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("\n")));

  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("G2P3FeClsAcePattern:: tcpflagurg=0x%01x"), (unsigned int)  pFromStruct->uTcpFlagURG));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" tcpflagack=0x%01x"), (unsigned int)  pFromStruct->uTcpFlagACK));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" tcpflagpsh=0x%01x"), (unsigned int)  pFromStruct->uTcpFlagPSH));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("\n")));

  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("G2P3FeClsAcePattern:: tcpflagrst=0x%01x"), (unsigned int)  pFromStruct->uTcpFlagRST));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" tcpflagsyn=0x%01x"), (unsigned int)  pFromStruct->uTcpFlagSYN));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" tcpflagfin=0x%01x"), (unsigned int)  pFromStruct->uTcpFlagFIN));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" dscp=0x%02x"), (unsigned int)  pFromStruct->uDscp));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("\n")));

  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("G2P3FeClsAcePattern:: ecn=0x%01x"), (unsigned int)  pFromStruct->uEcn));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" protocol=0x%02x"), (unsigned int)  pFromStruct->uProtocol));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" l4srcporthigh=0x%04x"), (unsigned int)  pFromStruct->uL4SrcPortHigh));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("\n")));

  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("G2P3FeClsAcePattern:: l4srcportlow=0x%04x"), (unsigned int)  pFromStruct->uL4SrcPortLow));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" l4dstporthigh=0x%04x"), (unsigned int)  pFromStruct->uL4DstPortHigh));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" l4dstportlow=0x%04x"), (unsigned int)  pFromStruct->uL4DstPortLow));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("\n")));

  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("G2P3FeClsAcePattern:: bdsapany=0x%01x"), (unsigned int) pFromStruct->bDsapAny));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" llcdsapssaphigh=0x%04x"), (unsigned int)  pFromStruct->uLlcDsapSsapHigh));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" llcdsapssaplow=0x%08x"), (unsigned int)  pFromStruct->uLlcDsapSsapLow));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("\n")));

  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("G2P3FeClsAcePattern:: bllcctrlany=0x%01x"), (unsigned int) pFromStruct->bLlcCtrlAny));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" llcctrldatahigh=0x%04x"), (unsigned int)  pFromStruct->uLlcCtrlDataHigh));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" llcctrldatalow=0x%04x"), (unsigned int)  pFromStruct->uLlcCtrlDataLow));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("\n")));

  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("G2P3FeClsAcePattern:: ipv4sa=0x%08x"), (unsigned int)  pFromStruct->uIpv4Sa));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" ipv4salen=0x%02x"), (unsigned int)  pFromStruct->uIpv4SaLen));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("\n")));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("G2P3FeClsAcePattern:: ipv4da=0x%08x"), (unsigned int)  pFromStruct->uIpv4Da));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" ipv4dalen=0x%02x"), (unsigned int)  pFromStruct->uIpv4DaLen));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" ethertype=0x%04x"), (unsigned int)  pFromStruct->uEtherType));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("\n")));

  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("G2P3FeClsAcePattern:: smac=0x%04x%08x"),   COMPILER_64_HI(pFromStruct->uuSmac), COMPILER_64_LO(pFromStruct->uuSmac)));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" smaclen=0x%02x"), (unsigned int)  pFromStruct->uuSmacLen));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" vlan=0x%04x"), (unsigned int)  pFromStruct->uVlan));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" dmac=0x%04x%08x"),   COMPILER_64_HI(pFromStruct->uuDmac), COMPILER_64_LO(pFromStruct->uuDmac)));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("\n")));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("G2P3FeClsAcePattern:: dmaclen=0x%02x"), (unsigned int)  pFromStruct->uuDmacLen));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" pbmp=0x%05x%08x"),   COMPILER_64_HI(pFromStruct->uuPbmp), COMPILER_64_LO(pFromStruct->uuPbmp)));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" enabled=0x%01x"), (unsigned int)  pFromStruct->bEnabled));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("\n")));
}
...
{
  uint64 uuMac = COMPILER_64_INIT(0xFFFF, 0xFFFFFFFF);
  ...
  if (COMPILER_64_GT(pZf->uuSmac, uuMac)) return 0;
  if (pZf->uuSmacLen > 0x30) return 0;
  if (pZf->uVlan > 0x3fff) return 0;
  if (COMPILER_64_GT(pZf->uuDmac, uuMac)) return 0;
...
soc_sbx_g2p3_cls_ace_pattern_init(soc_sbx_g2p3_cls_ace_pattern_t *pFrame)
{
  ...
  COMPILER_64_ZERO(pFrame->uuSmac);
  pFrame->uuSmacLen =  (unsigned int)  0;
  pFrame->uVlan =  (unsigned int)  0;
  COMPILER_64_ZERO(pFrame->uuDmac);
  pFrame->uuDmacLen =  (unsigned int)  0;
  COMPILER_64_ZERO(pFrame->uuPbmp);
...
soc_sbx_g2p3_cls_ace_pattern_ipv6_set(soc_sbx_g2p3_cls_ace_pattern_ipv6_t *s, char* name, int value)
{
  if (SB_STRCMP(name, "") == 0 ) {
    ...
    COMPILER_64_SET(s->uuIpv6SaMS, 0, value);
  } else if (SB_STRCMP(name, "uuipv6sals") == 0) {
    COMPILER_64_SET(s->uuIpv6SaLS, 0, value);
  } else if (SB_STRCMP(name, "uuipv6salen") == 0) {
    s->uuIpv6SaLen = value;
  } else if (SB_STRCMP(name, "uuipv6dams") == 0) {
    COMPILER_64_SET(s->uuIpv6DaMS,0,value);
  } else if (SB_STRCMP(name, "uuipv6dals") == 0) {
    COMPILER_64_SET(s->uuIpv6DaLS,0,value);
...
soc_sbx_g2p3_cls_ace_pattern_ipv6_init(soc_sbx_g2p3_cls_ace_pattern_ipv6_t *pFrame)
{
  ...
  pFrame->uL4DstPortHigh =  (unsigned int)  0;
  pFrame->uL4DstPortLow =  (unsigned int)  0;
  COMPILER_64_ZERO(pFrame->uuIpv6SaMS);
  COMPILER_64_ZERO(pFrame->uuIpv6SaLS);
  pFrame->uuIpv6SaLen = (unsigned int) 0;
  COMPILER_64_ZERO(pFrame->uuIpv6DaMS);
  COMPILER_64_ZERO(pFrame->uuIpv6DaLS);
                                    ...
                                    soc_sbx_g2p3_ace_rule_t *r)
{
    ...
    COMPILER_64_ZERO(r->uPattern.g2p3.uuSmac);
    for (i = 0; i < 6; i++) {
        COMPILER_64_SHL(r->uPattern.g2p3.uuSmac, 8);
        COMPILER_64_SET(uuTmp, 0, e->smac[i]);
        COMPILER_64_OR(r->uPattern.g2p3.uuSmac, uuTmp);
    }
    ...
    COMPILER_64_ZERO(r->uPattern.g2p3.uuDmac);
    for (i = 0; i < 6; i++) {
        COMPILER_64_SHL(r->uPattern.g2p3.uuDmac, 8);
        COMPILER_64_SET(uuTmp, 0, e->dmac[i]);
        COMPILER_64_OR(r->uPattern.g2p3.uuDmac, uuTmp);
    }
    r->uPattern.g2p3.bDmacAny = 0;
    r->uPattern.g2p3.uuDmacLen = e->dmacwidth;

    COMPILER_64_ZERO(r->uPattern.g2p3.uuPbmp);
    for (i = 0; i < 7; i++) {
        COMPILER_64_SHL(r->uPattern.g2p3.uuPbmp, 8);
        COMPILER_64_SET(uuTmp, 0, 0xFF);
        COMPILER_64_OR(r->uPattern.g2p3.uuPbmp, uuTmp);
        COMPILER_64_SET(uuTmp, 0, ~e->pbmpn[i]);
        COMPILER_64_AND(r->uPattern.g2p3.uuPbmp, uuTmp);
    }
                                  ...
                                  soc_sbx_g2p3_ace_rule_t *r)
{
    ...
    for (i = 0; i < 6; i++) {
        uuTmp =  r->uPattern.g2p3.uuSmac;
        COMPILER_64_SHR(uuTmp, (8 * (5 - i)));
        e->smac[i] = COMPILER_64_LO(uuTmp) & 0xFF;
    }
    for (i = 0; i < 6; i++) {
        uuTmp =  r->uPattern.g2p3.uuDmac;
        COMPILER_64_SHR(uuTmp, (8 * (5 - i)));
        e->dmac[i] = COMPILER_64_LO(uuTmp) & 0xFF;
    }
    ...
    for (i = 0; i < 7; i++) {
        uuTmp =  r->uPattern.g2p3.uuPbmp;
        COMPILER_64_SHR(uuTmp, (8 * (6 - i)));
        COMPILER_64_NOT(uuTmp);
        e->pbmpn[i] = COMPILER_64_LO(uuTmp) & 0xFF;
    }
                                    ...
                                    soc_sbx_g2p3_ace_rule_t *r)
{
    ...
    for (i = 0; i < 8; i++) {
        COMPILER_64_SHL(r->uPattern.g2p3_ipv6.uuIpv6SaMS, 8);
        COMPILER_64_SET(uuTmp, 0, e->sa[i]);
	COMPILER_64_OR(r->uPattern.g2p3_ipv6.uuIpv6SaMS, uuTmp);
    }
    for (i = 8; i < 16; i++) {
        COMPILER_64_SHL(r->uPattern.g2p3_ipv6.uuIpv6SaLS, 8);
        COMPILER_64_SET(uuTmp, 0, e->sa[i]);
        COMPILER_64_OR(r->uPattern.g2p3_ipv6.uuIpv6SaLS, uuTmp);
    }
    r->uPattern.g2p3_ipv6.uuIpv6SaLen = e->sawidth;

    for (i = 0; i < 8; i++) {
        COMPILER_64_SHL(r->uPattern.g2p3_ipv6.uuIpv6DaMS, 8);
        COMPILER_64_SET(uuTmp, 0, e->da[i]);
        COMPILER_64_OR(r->uPattern.g2p3_ipv6.uuIpv6DaMS, uuTmp);
    }
    for (i = 8; i < 16; i++) {
        COMPILER_64_SHL(r->uPattern.g2p3_ipv6.uuIpv6DaLS, 8);
        COMPILER_64_SET(uuTmp, 0, e->da[i]);
        COMPILER_64_OR(r->uPattern.g2p3_ipv6.uuIpv6DaLS, uuTmp);
    }
    ...
    COMPILER_64_ZERO(r->uPattern.g2p3_ipv6.uuPbmp);
    for (i = 0; i < 7; i++) {
        COMPILER_64_SET(uuTmp, 0, 0xFF);
        COMPILER_64_SHL(r->uPattern.g2p3_ipv6.uuPbmp, 8);
        COMPILER_64_OR(r->uPattern.g2p3_ipv6.uuPbmp, uuTmp);
        COMPILER_64_SET(uuTmp, 0, ~e->pbmpn[i]);
        COMPILER_64_AND(r->uPattern.g2p3_ipv6.uuPbmp, uuTmp);
    }
                                  ...
                                  soc_sbx_g2p3_ace_rule_t *r)
{
    ...
    for (i = 0; i < 8; i++) {
	uuTmp =  r->uPattern.g2p3_ipv6.uuIpv6SaMS;
        COMPILER_64_SHR(uuTmp, (8 * (7 - i)));
        e->sa[i] = COMPILER_64_LO(uuTmp) & 0xFF;
    }
    for (i = 8; i < 16; i++) {
	uuTmp =  r->uPattern.g2p3_ipv6.uuIpv6SaLS;
        COMPILER_64_SHR(uuTmp, (8 * (15 - i)));
        e->sa[i] = COMPILER_64_LO(uuTmp) & 0xFF;
    }
    e->sawidth = r->uPattern.g2p3_ipv6.uuIpv6SaLen;

    for (i = 0; i < 8; i++) {
        uuTmp =  r->uPattern.g2p3_ipv6.uuIpv6DaMS;
	COMPILER_64_SHR(uuTmp, (8 * (7 - i)));
        e->da[i] = COMPILER_64_LO(uuTmp) & 0xFF;
    }
    for (i = 8; i < 16; i++) {
        uuTmp =  r->uPattern.g2p3_ipv6.uuIpv6DaMS;
	COMPILER_64_SHR(uuTmp, (8 * (15 - i)));
    ...
    for (i = 0; i < 7; i++) {
        uuTmp =  r->uPattern.g2p3_ipv6.uuPbmp;
        COMPILER_64_SHR(uuTmp, (8 * (6 - i)));
        COMPILER_64_NOT(uuTmp);
        e->pbmpn[i] = COMPILER_64_LO(uuTmp) & 0xFF;
                                  ...
                                  soc_sbx_g2p3_ace_rule_t *r)
{
    ...
    for (i = 8; i < 16; i++) {
        uuTmp =  r->uPattern.g2p3_ipv6.uuIpv6DaLS;
        COMPILER_64_SHR(uuTmp, (8 * (15 - i)));
        e->da[i] = COMPILER_64_LO(uuTmp) & 0xFF;
    }
    e->dawidth = r->uPattern.g2p3_ipv6.uuIpv6DaLen;
                            ...
                            soc_sbx_g2p3_counter_value_t *v)
{
    ...
    soc_sbx_g2p3_counter_value_t cv;
    int rv;
    uint32 oldRuleOffset = 0, ruleCount = 0, rulePresent = 0;
...
static int
soc_sbx_g2p3_rule_modify(uint32 unit,
                       soc_sbx_g2p3_ace_rule_handle_t  handle,
                       soc_sbx_g2p3_ace_rule_t         *pRule,
                       soc_sbx_g2p3_cls_rule_db_e_t eDb)
{
...
static int
soc_sbx_g2p3_rule_from_handle(uint32 unit,
                              soc_sbx_g2p3_cls_rule_db_e_t eDb,
                              soc_sbx_g2p3_ace_rule_handle_t handle,
                              soc_sbx_g2p3_ace_rule_t *pUserCopy)
{
...
static
int soc_sbx_g2p3_fp_clear_all_ext(uint32 unit,
                                  soc_sbx_g2p3_cls_rule_db_e_t eDb)
{
    soc_sbx_g2p3_state_t *pFe =
        (soc_sbx_g2p3_state_t *) SOC_SBX_CONTROL(unit)->drv;
...
#if 0
    LOG_CLI((BSL_META("Recovered rule 0x%x group 0x%x\n"), uRule, uGroupId));
    if (err == SB_OK) {
