...
/*
 * $Id: sbFe2000DmaMgr.c,v 1.15 Broadcom SDK $
 *
...
struct sbFe2000DmaMgr_s {
  ...
  uint32 *pRingMem;
  uint32 *pRingPtr;
  uint32 ulProducerOffs;
  uint32 ulConsumerOffs;
  ...
  sbFe2000DmaMgr_t *slaveDmaCtxt[SOC_MAX_NUM_DEVICES];  /* fe2k DMA Mgr ptr of slave devices */
  sbFe2000DmaSlabOp_t *slaveSlabOp[SOC_MAX_NUM_DEVICES]; /* slab operation for DMA on slave devices */
};
...
static sbStatus_t
sbFe2000DmaWaitBankIdle(sbFe2000DmaMgr_t *state, uint32 feAddress);
static sbStatus_t
sbFe2000DmaWaitIndirect(sbFe2000DmaMgr_t *state, uint32 timeout, sbFe2000DmaPioRegs_t *bank);
...
static uint8 sbFe2000DmaSlabStartChunk(sbFe2000DmaMgr_t *state);
static uint32 sbFe2000DmaRingNext(sbFe2000DmaMgr_t *state, uint32 addr);
...
{
  SB_MEMSET(params, 0, sizeof(*params)); 

  params->pciReadsPerEpoch = 20;  
  params->maxPio = 0x80;          
  params->pioTimeout = 1024;      
}
                ...
                sbFe2000DmaConfig_t *params)
{
  ...
  *pCtl = state = vp;
  ...
  /* We assume uint32s are densely packed in slabs */
  SB_ASSERT(((uint8 *) &s[0].data[1]) - ((uint8 *) &s[0].data[0]) == sizeof(uint32));
  /* We assume slabs are densely packed in arrays */
  SB_ASSERT(((uint8 *) &(s[1])) - ((uint8 *) &(s[0])) == sizeof(sbFe2000DmaSlab_t));
  ...
  if (params->bUseRing) {
    ...
    base = ((uint32) state->pRingMem) & ~state->ulRingMask;
    offs = ((uint32) state->pRingMem) & state->ulRingMask;

    /* see if we need to align  */
    if (offs) {
      /* round up to next aligned location */
      state->pRingPtr = (uint32 *) (base + rsize);
    } else {
    ...
    SAND_HAL_FE2000_WRITE(params->regSet,  PC_DMA_SG_RING_PTR, (uint32) state->pRingPtr);
   ...
   * Broke into regsets due to Pedantic Build complaint of string size too large
   *
   * SB_ASSERT((SAND_HAL_FE2000_REG_OFFSET(params->regSet, MM0_INT0_MEM_ACC_DATA1) + sizeof(uint32))
   *            == SAND_HAL_FE2000_REG_OFFSET(params->regSet, MM0_INT0_MEM_ACC_DATA0));
   * SB_ASSERT((SAND_HAL_FE2000_REG_OFFSET(params->regSet, MM1_INT0_MEM_ACC_DATA1) + sizeof(uint32))
   *            == SAND_HAL_FE2000_REG_OFFSET(params->regSet, MM1_INT0_MEM_ACC_DATA0));
   */
  { uint32 regset1, regset2;
    regset1 = (SAND_HAL_FE2000_REG_OFFSET(params->regSet, MM0_INT0_MEM_ACC_DATA1) + sizeof(uint32));
    regset2 = SAND_HAL_FE2000_REG_OFFSET(params->regSet, MM0_INT0_MEM_ACC_DATA0);
    SB_ASSERT(regset1 == regset2);
    regset1 = (SAND_HAL_FE2000_REG_OFFSET(params->regSet, MM1_INT0_MEM_ACC_DATA1) + sizeof(uint32));
    regset2 = SAND_HAL_FE2000_REG_OFFSET(params->regSet, MM1_INT0_MEM_ACC_DATA0);
    SB_ASSERT(regset1 == regset2);
  }
  ...
  for (i = 0; i < SOC_MAX_NUM_DEVICES; i++) {
      state->slaveDmaCtxt[i] = NULL;
      state->slaveSlabOp[i] = NULL;
  }
...
sbStatus_t
sbFe2000DmaInitAddSlave(int master_unit,
			sbFe2000DmaMgr_t *masterDmaMgr,
			int slave_unit,
			sbFe2000DmaMgr_t *slaveDmaMgr)
{
    void *vp;
    sbStatus_t status = SB_OK;

    /* 
     * After slave units created dma manager, it add itself
     * to master dma manager. It also create an area for slapOp
     * to be used by master slab dma operation.
     */

    /* allocate slabOp */
    status = thin_malloc((void *)master_unit, SB_ALLOC_INTERNAL, sizeof(sbFe2000DmaSlabOp_t), (void**)&vp, NULL);
    if (status != SB_OK)
	return status;
    if (!vp)
	return SB_MALLOC_FAILED;  
    SB_MEMSET(vp, 0, sizeof(sbFe2000DmaSlabOp_t));  

    SB_ASSERT(masterDmaMgr->slaveDmaCtxt[slave_unit]==NULL);
    SB_ASSERT(masterDmaMgr->slaveSlabOp[slave_unit]==NULL);

    masterDmaMgr->slaveDmaCtxt[slave_unit] = slaveDmaMgr;
    masterDmaMgr->slaveSlabOp[slave_unit] = (sbFe2000DmaSlabOp_t *)vp;
                   ...
                   sbFe2000DmaOp_t *op)
{
    ...
    if (action == _dmaOpDma) {
      LOG_VERBOSE(BSL_LS_SOC_DMA,
                  (BSL_META_U(unit,
                              "DMA request.  FeAddr=0x%08x words=%d "
                              "forcePio= %d opcode=%d\n"),
                    ...
                    sbFe2000DmaOp_t *op)
{
  sbStatus_t status;
  sbSyncToken_t sync;
  int unit = (int)state->clientData;

#ifdef BCM_WARM_BOOT_SUPPORT  
  int wb = SOC_WARM_BOOT(unit);
#endif
  ...
  if (status == SB_OK) {
    sync = (state->pCfg->isrSync)((void*)unit);
    ...
    (state->pCfg->isrUnsync)((void *)unit, (int)sync);
  }
...
sbFe2000DmaIsStartable(sbFe2000DmaMgr_t *state, sbFe2000DmaOp_t *op)
{
  uint32 ulMax = state->bUseRing ? state->ulRingEntries : SB_FE2000_DMA_CHANNELS;
...
uint32 g_bSbG2FeDmaStartDump = 0;
...
sbFe2000DmaStart(sbFe2000DmaMgr_t *state, sbFe2000DmaOp_t *op)
{
  ...
  uint8 write = opcode == SB_FE2000_DMA_OPCODE_WRITE;
  uint8 clear = opcode == SB_FE2000_DMA_OPCODE_CLEAR;
  uint8 clearOnRead = opcode == SB_FE2000_DMA_OPCODE_READ_CLEAR;
  sbStatus_t status = SB_OK;
  uint32 bank, reg, offs, wide, bankWords, i, pio, w2, w3, immediate, ctrlReg, *pEntry;
  uint32 indicesPerBlock;
  _fe2kDmaOp_t action;
  int unit = (int)state->clientData;
  ...
  if (g_bSbG2FeDmaStartDump)
    LOG_WARN(BSL_LS_SOC_COMMON,
             (BSL_META_U(unit,
                         "DmaStart: unit: %d, %s, opcode: 0x%x, %d words, hba: 0x%x, fea: 0x%x\n"),
              unit, pio ? "pio" : "dma", opcode, op->words,
  ...
  if (opcode == SB_FE2000_DMA_OPCODE_SYNC) {
    for (i = 0; i < state->pCfg->pciReadsPerEpoch; i++) {
      ...
      SAND_HAL_FE2000_READ_OFFS(unit, state->dmaRegs[0].request);
    }
  } else {
    ...
    if (pio) {
      ...
      for (i = 0; i < op->words / bankWords; i++) {
        if (write || clear) {
          uint32 *data = clear ? NULL : &op->hostAddress[i * bankWords];
          status =
      ...
      if (!state->bUseRing) { /* initiate thread-based dma */
        ...
        reg = (SAND_HAL_FE2000_SET_FIELD(unit,  PC_DMA_CTRL0, ACK, 1) |
               SAND_HAL_FE2000_SET_FIELD(unit,  PC_DMA_CTRL0, REQ, 1) |
               SAND_HAL_FE2000_SET_FIELD(unit,  PC_DMA_CTRL0, RD_WR_N, !write) |
               SAND_HAL_FE2000_SET_FIELD(unit,  PC_DMA_CTRL0, COR, clearOnRead) |
               SAND_HAL_FE2000_SET_FIELD(unit,  PC_DMA_CTRL0, DMA_SRC, bank) |
               SAND_HAL_FE2000_SET_FIELD(unit,  PC_DMA_CTRL0, DMA_SRC_TYPE, wide));

        SAND_HAL_FE2000_WRITE_OFFS(unit, regs->hostAddress, op->hostBusAddress);
        SAND_HAL_FE2000_WRITE_OFFS(unit, regs->devAddress, offs);
        SAND_HAL_FE2000_WRITE_OFFS(unit, regs->wordCount, op->words);
        SAND_HAL_FE2000_WRITE_OFFS(unit, regs->request, reg);
...
#if 0 
        /* Disable immediate mode not supported in current model */
        ...
        if (immediate) {
          ...
          w2 = *(((uint32 *) op->hostBusAddress) + 0);
          w3 = *(((uint32 *) op->hostBusAddress) + 1);
        ...
        pEntry = state->pRingPtr + state->ulProducerOffs / sizeof(uint32);
        ...
        *(pEntry + 0) = SAND_SWAP_32(ctrlReg);
        ...
        SAND_HAL_FE2000_WRITE(unit,  PC_DMA_SG_RING_PRODUCER, state->ulProducerOffs);
      }
    }
  }
  return status;
...
sbFe2000DmaWaitBankIdle(sbFe2000DmaMgr_t *state, uint32 feAddress)
{
  ...
  status = sbFe2000DmaWaitIndirect(state, state->pCfg->pioTimeout,
                                   &state->pioRegs[bank]);
...
sbFe2000DmaWaitIndirect(sbFe2000DmaMgr_t *state, uint32 timeout, sbFe2000DmaPioRegs_t *bank)
{
  ...
  for (i = 0; i < timeout; i++) {
    reg = SAND_HAL_FE2000_READ_OFFS(unit, bank->request);
    if (reg & SAND_HAL_FE2000_FIELD_MASK(unit, MM_INT0, MEM_ACC_CTRL_ACK)) {
      reg &= ~(SAND_HAL_FE2000_FIELD_MASK(unit, MM_INT0, MEM_ACC_CTRL_REQ));
      reg |= SAND_HAL_FE2000_FIELD_MASK(unit, MM_INT0, MEM_ACC_CTRL_ACK);
      SAND_HAL_FE2000_WRITE_OFFS(unit, bank->request, reg);
      return SB_OK;
    }
  }
                         ...
                         uint32 *data)
{
  ...
  sbStatus_t status;
  uint32 i, j, bank, offs, reg, bankWords, clsAdjust;
  int unit = (int)state->clientData;
  ...
  /* Deal with the way the classifier dmas are presented to
   * pio subsystem. There are 64 words in the dma chunk.
   * Words 0->3 and 32-63 need to be packed into the 36 word
   * data registers for the pio transaction. Words 4->31
   * (28 words) are padding. The pio data registers are
   * ajacent. We need to transfer the dma data into data regs */
  if ((bank == SB_FE2000_MEM_CLS_0) || (bank == SB_FE2000_MEM_CLS_1))
    clsAdjust = 1;
  else
    clsAdjust = 0;
  ...
  for (i = 0; i < bankWords; i++) {
    ...
    SAND_HAL_FE2000_WRITE_OFFS_RAW(unit,
                            pRegs->data + j * sizeof(uint32),
                            data ? data[i] : 0);
  }

  /* compose request using physical addressing */
  reg = (SAND_HAL_FE2000_SET_FIELD(unit, MM_INT0_MEM_ACC_CTRL, ACK, 1) |
         SAND_HAL_FE2000_SET_FIELD(unit, MM_INT0_MEM_ACC_CTRL, REQ, 1) |
         SAND_HAL_FE2000_SET_FIELD(unit, MM_INT0_MEM_ACC_CTRL, RD_WR_N, 0) |
         SAND_HAL_FE2000_SET_FIELD(unit, MM_NARROW_PORT0_MEM_ACC_CTRL, ADDRESS, offs));

  SAND_HAL_FE2000_WRITE_OFFS(unit, pRegs->request, reg);
...
{
  uint32 i, bank, offs, *pdata, bwords, addr;
  sbStatus_t sts;
  ...
  /* uint32 words per memory address */
                         ...
                         uint32 *data, uint8 clearOnRead)
{
  ...
  reg = (SAND_HAL_FE2000_SET_FIELD(unit, MM_INT0_MEM_ACC_CTRL, ACK, 1) |
         SAND_HAL_FE2000_SET_FIELD(unit, MM_INT0_MEM_ACC_CTRL, REQ, 1) |
         SAND_HAL_FE2000_SET_FIELD(unit, MM_INT0_MEM_ACC_CTRL, RD_WR_N, 1) |
         SAND_HAL_FE2000_SET_FIELD(unit, MM_NARROW_PORT0_MEM_ACC_CTRL, ADDRESS, offs));

  SAND_HAL_FE2000_WRITE_OFFS(unit, pRegs->request, reg);

  if ((status = sbFe2000DmaWaitIndirect(state, state->pCfg->pioTimeout, pRegs)) != SB_OK) {
    return status;
  }
  for (i = 0; i < bankWords; i++) {
    pRegs->bReverseLayout ? (j = bankWords - i - 1) : (j = i);
    ...
    data[i] = SAND_HAL_READ_OFFS_RAW(unit,
                                     pRegs->data + j * sizeof(uint32));
  }
...
sbFe2000DmaReadIndirectMult(sbFe2000DmaMgr_t *state, sbFe2000DmaAddr_t feAddress,
                            uint32 *data, uint8 clearOnRead, uint32 words)
{
  uint32 i, bank, offs, *pdata, bwords, addr;
  sbStatus_t sts;
  ...
  while (state->dmasActive && again) {
    ...
    while (state->dmasActive != 0) {
      ...
      if (!state->bUseRing) {
        ...
        if ((SAND_HAL_FE2000_READ_OFFS(unit, regs->request)
             & SAND_HAL_FE2000_FIELD_MASK(unit, PC_DMA, CTRL0_ACK)) == 0) {
          again = 0;
          break;
        }
        SAND_HAL_FE2000_WRITE_OFFS(unit, regs->request,
                            SAND_HAL_FE2000_FIELD_MASK(unit, PC_DMA, CTRL0_ACK));
        state->regsConsumer = (state->regsConsumer + 1) % SB_FE2000_DMA_CHANNELS;
      } else {
        /* ring based dma */
        pEntry = state->pRingPtr + state->ulConsumerOffs / sizeof(uint32);
    ...
    DQ_MAP(&done, sbFe2000DmaDone, unit);
  }
...
sbFe2000DmaDone(dq_t *op0, int a)
{
  ...
  if (g_bSbG2FeDmaStartDump)
    LOG_WARN(BSL_LS_SOC_COMMON,
             (BSL_META("DmaDone: unit: %d, opcode: 0x%x, %d words, hba: 0x%x, fea: 0x%x\n"),
              a, op->opcode, op->words,
              (unsigned int) op->hostAddress, (unsigned int) op->feAddress));
                           ...
                           sbFe2000DmaSlabOp_t *slabOp)
{
    ...
    soc_sbx_control_t *sbx = SOC_SBX_CONTROL(unit), *slave_sbx = NULL;
    void *data0;
    sbStatus_t status = SB_OK;
    sbStatus_t slave_status[SOC_MAX_NUM_DEVICES];
    int slave, dev;
    
    for (dev = 0; dev < SOC_MAX_NUM_DEVICES; dev++) {
      slave_status[dev] = SB_OK;
    }
    
    /* debug: g_bSbG2FeDmaStartDump = 1; */

    /* check if it's master unit */
    if (SOC_IS_SBX_MASTER(unit)) {
      /* for master unit, issue sbFe2000DmaSlabRequest for itself and then all
       * slave units. To improve efficiency, we start DMA on all units before
       * wait on any of unit's DMA. We only use master unit's sync_sem though
       */
      sal_sem_take(pDmaMgr->sync_sem, sal_sem_FOREVER);

      /* save original slabOp->data */
      data0 = slabOp->data;      

      /* init slabOp for all units, for slave units, only thing different
       * is the unit (slabOp->data). Note that all slabOp share same 
       * slabOp->slab.
       */
      slabOp->cb = sbFe2000DmaSlabRequestCallback;
      slabOp->data = (void *) unit;
      for (slave = 0; slave < sbx->num_slaves; slave++) {
        dev = sbx->slave_units[slave];
        sal_memcpy(pDmaMgr->slaveSlabOp[dev], slabOp, sizeof(sbFe2000DmaSlabOp_t));
        pDmaMgr->slaveSlabOp[dev]->data = (void *) dev;
      }

      /* start Dma on Master itself */
      status = sbFe2000DmaSlabRequestI(pDmaMgr, (void *)unit, slabOp);

      /* start Dma on all slave units */
      for (slave = 0; slave < sbx->num_slaves; slave++) {
        dev = sbx->slave_units[slave];
        slave_status[slave] = sbFe2000DmaSlabRequestI(pDmaMgr->slaveDmaCtxt[dev],
                                                      (void *)dev,
                                                      pDmaMgr->slaveSlabOp[dev]);
      }

      /* wait Master to be done */
      if (status == SB_IN_PROGRESS) {
        sal_sem_take(sbx->dma_sem, sal_sem_FOREVER);
        status = slabOp->status;
      }
      
      /* wait all slaves to be done */
      for (slave = 0; slave < sbx->num_slaves; slave++) {
        dev = sbx->slave_units[slave];      
        slave_sbx = SOC_SBX_CONTROL(dev);
        if (slave_status[slave] == SB_IN_PROGRESS) {
          sal_sem_take(slave_sbx->dma_sem, sal_sem_FOREVER);
          slave_status[slave] = pDmaMgr->slaveSlabOp[dev]->status;
        }
      }
      ...
      for (slave = 0; slave < sbx->num_slaves; slave++) {
        dev = sbx->slave_units[slave];
        pDmaMgr->slaveSlabOp[dev]->data = data0;
      }
 
      sal_sem_give(pDmaMgr->sync_sem);

      /* update status based on slave status, report error if any
       * slave unit has error. Note that we will not have 
       * SB_IN_PROGRESS state
       */
      if (status == SB_OK) {
        for (slave = 0; slave < sbx->num_slaves; slave++) {
          if (slave_status[slave] != SB_OK) {
            status = slave_status[slave];
            break;
          }
        }
      }
                        ...
                        sbFe2000DmaSlabOp_t *slabOp)
{
  ...
  if (slabOp->hostBusAddress == SB_FE2000_DMA_HBA_COMPUTE) {
    status =
      (state->pCfg->getHba)(slabOp->dmaHandle, (uint32 *) slabOp->slab,
                           slabSize / sizeof(uint32),
  ...
  (state->pCfg->isrUnsync)((void *)unit, sync);
  return status;
}
...
sbFe2000DmaSlabStartChunk(sbFe2000DmaMgr_t *state)
{
  ...
  sbStatus_t status = SB_OK;
  uint8 readwords = chunk->opcode == SB_FE2000_DMA_OPCODE_READ_WORDS;
  uint8 noMore;
  int unit = (int)state->clientData;
  ...
  dmaOp->hostBusAddress =
    ...
    + ((sbFeDmaHostBusAddress_t)
       (((uint8 *) dmaOp->hostAddress) - ((uint8 *) slabOp->slab)));
  dmaOp->dmaHandle = slabOp->dmaHandle;

  status = sbFe2000DmaRequestI(state, (void *)unit, dmaOp);
...
{
  uint32 left = maxContigWords - (*totalWords % maxContigWords);
...
uint32
sbFe2000DmaRingNext(sbFe2000DmaMgr_t *state, uint32 ulAddr)
{
