...
#define RSP_TCAM_OPCODE_TO_OAM_TS_FORMAT(opcode)\
    (opcode==SOC_PPC_OAM_ETHERNET_PDU_OPCODE_LMM || SOC_PPC_OAM_ETHERNET_PDU_OPCODE_SLM)? _ARAD_PP_OAM_SUBTYPE_LM : \
...
do {\
    ...
    SHR_BITCOPY_RANGE(&data,0,&format__,0,3);\
    SHR_BITCOPY_RANGE(&data,3,&opcode__,0,16);\
    SHR_BITCOPY_RANGE(&data,19,&gen__,0,1);\
} while (0)
...
soc_error_t soc_jer_pp_oam_init_response_table(int unit) {
      ...
      int valid_opcodes[] = {SOC_PPC_OAM_ETHERNET_PDU_OPCODE_LBM, SOC_PPC_OAM_ETHERNET_PDU_OPCODE_LTM, SOC_PPC_OAM_ETHERNET_PDU_OPCODE_LMM,
      SOC_PPC_OAM_ETHERNET_PDU_OPCODE_DMM, SOC_PPC_OAM_ETHERNET_PDU_OPCODE_SLM, SOC_PPC_OAM_ETHERNET_PDU_OPCODE_EXM};
      int i;
      SOCDNX_INIT_FUNC_DEFS;
      ...
      for (i = 0; i < sizeof(valid_opcodes) / sizeof(int); ++i) {
          ...
          rv = WRITE_OAMP_RSP_TCAMm(unit,0,i,reg_above_64);
          SOCDNX_IF_ERR_EXIT(rv);
      }
