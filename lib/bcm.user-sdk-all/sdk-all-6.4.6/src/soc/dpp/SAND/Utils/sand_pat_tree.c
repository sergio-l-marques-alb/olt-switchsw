  ...
  )
{
  ...
  if(!tree_info_ndx->pat_tree_data.tree_memory) {
    ...
    {
        btmp_init_info.wb_var_index = tree_info_ndx->wb_var_index + WB_ENGINE_PAT_TREE_AGREGATION_MEMORY_USE;
        btmp_init_info.unit    = tree_info_ndx->prime_handle;      
    }
  ...
  )
{
  ...
  if(tree_info_ndx->pat_tree_data.memory_use) {
      ...
      tree_info_ndx->pat_tree_data.memory_use = NULL;
  }
  ...
  )
{
  ...
  */
  res = soc_sand_occ_bm_cache_set(unit, tree_info_ndx->pat_tree_data.memory_use,enable_cache);
  SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
 /*
  ...
  )
{
  ...
  */
  res = soc_sand_occ_bm_cache_commit(unit, tree_info_ndx->pat_tree_data.memory_use,flags);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
 /*
  * if cache enabled, copy status to cache instance
  */
  ...
  )
{
  ...
  * rollback bitmap
  */
  res = soc_sand_occ_bm_cache_rollback(unit, tree_info_ndx->pat_tree_data.memory_use,flags);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
 /*
  * if cache enabled, copy status to cache instance
  */
