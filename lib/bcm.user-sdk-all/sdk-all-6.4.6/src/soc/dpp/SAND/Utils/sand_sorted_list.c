...
uint32
  ...
  *sorted_list_ptr = soc_sand_os_malloc_any_size(sizeof(SOC_SAND_SORTED_LIST_INFO), "sorted_list_ptr");
  soc_sand_SAND_SORTED_LIST_INFO_clear(*sorted_list_ptr);
  sorted_list = *sorted_list_ptr;

  sorted_list->init_info.prime_handle  = init_info.prime_handle;
  sorted_list->init_info.sec_handle    = init_info.sec_handle;
  sorted_list->init_info.list_size     = init_info.list_size;
  sorted_list->init_info.key_size      = init_info.key_size;
  sorted_list->init_info.data_size     = init_info.data_size;
  sorted_list->init_info.get_entry_fun = init_info.get_entry_fun;
  sorted_list->init_info.set_entry_fun = init_info.set_entry_fun;
  sorted_list->init_info.cmp_func_type = init_info.cmp_func_type;
  sorted_list->init_info.wb_var_index  = init_info.wb_var_index;
  sorted_list->init_info.unit          = init_info.unit;
    ...
    )
{
  ...
  soc_sand_os_free_any_size(sorted_list);
  ...
  )
{
  ...
  if (init_info_ptr->cmp_func_type == SOC_SAND_SORTED_LIST_CMP_FUNC_TYPE_TCAM) {
  ...
  */
  ...
  if (prev_node != SOC_SAND_SORTED_LIST_ITER_END(sorted_list) &&  prev_node != SOC_SAND_SORTED_LIST_ITER_BEGIN(sorted_list))
  {
    ...
    if (cmp_key_fun(list_data_ptr->tmp_key, key, init_info_ptr->key_size) > 0 )
    {
  ...
  if (next_node != SOC_SAND_SORTED_LIST_ITER_END(sorted_list) &&  next_node != SOC_SAND_SORTED_LIST_ITER_BEGIN(sorted_list))
  {
    ...
    if (cmp_key_fun(key, list_data_ptr->tmp_key, init_info_ptr->key_size) > 0 )
    {
  ...
  )
{
  ...
  {
    set_entry_fun = soc_sand_sorted_list_default_entry_set;
  }
  ...
  if (data)
  {
      if(SOC_DPP_WB_ENGINE_VAR_NONE == init_info_ptr->wb_var_index)
      {
          res = set_entry_fun(init_info_ptr->prime_handle,
  ...
  )
{
  ...
  {
    get_entry_fun = soc_sand_sorted_list_default_entry_get;
  }
  ...
  )
{
  ...
  res = soc_sand_sorted_list_create(unit, &sorted_list, sorted_list->init_info);
  SOC_SAND_CHECK_FUNC_RESULT(res,20, exit);
  ...
  )
{
  ...
  info->init_info.cmp_func_type = SOC_SAND_SORTED_LIST_CMP_FUNC_TYPE_DEFAULT; 
  ...
  )
{
  ...
  if(SOC_DPP_WB_ENGINE_VAR_NONE == init_info_ptr->wb_var_index)
  {
      set_entry_fun(init_info_ptr->prime_handle,
                                   init_info_ptr->sec_handle,
                                   list_data_ptr->keys,
                                   new_node_ptr,
                                   init_info_ptr->key_size,
