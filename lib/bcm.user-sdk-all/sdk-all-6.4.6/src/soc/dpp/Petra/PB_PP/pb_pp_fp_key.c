  ...
  )
{
  ...
  *nof_bits   = 0;
  ...
  {
    if (db_info->qual_types[idx] != SOC_PB_PP_NOF_FP_QUAL_TYPES)
    {
  ...
  )
{
  ...
  if (db_has_format)
  {
    ...
    for (qual_idx = 0 ; db_info->qual_types[qual_idx] == SOC_PB_PP_NOF_FP_QUAL_TYPES; ++qual_idx);
  ...
  {
    if (db_info->qual_types[qual_idx] == SOC_PB_PP_NOF_FP_QUAL_TYPES)
    {
      break;
    }
  ...
  {
    for ( ; db_info->qual_types[qual_idx] == SOC_PB_PP_NOF_FP_QUAL_TYPES; ++qual_idx);
  ...
  )
{
  ...
  if (stage == SOC_PB_PP_FP_DATABASE_STAGE_INGRESS_PMF) {
    ...
    qual_type_arr[1] = SOC_PB_PP_NOF_FP_QUAL_TYPES;
