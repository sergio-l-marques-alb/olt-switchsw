  ...
  )
{
  ...
  if(!SOC_WARM_BOOT(unit)) {
      ...
      {
          rv = sw_state_access[unit].dpp.soc.arad.pp.mac_limit_info.is_allocated(unit, &is_allocated);
          SOC_SAND_SOC_CHECK_FUNC_RESULT(rv, 32, exit);
      ...
      {
          rv = sw_state_access[unit].dpp.soc.arad.pp.arad_sw_db_src_binds.is_allocated(unit, &is_allocated);
          SOC_SAND_SOC_CHECK_FUNC_RESULT(rv, 32, exit);
