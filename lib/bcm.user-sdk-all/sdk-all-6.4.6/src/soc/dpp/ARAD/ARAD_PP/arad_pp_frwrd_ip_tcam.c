    ...
    )
{
    ...
    *found = FALSE;
    ...
    res = sw_state_access[unit].dpp.soc.arad.tm.kbp_frwrd_ip.location_tbl.get(unit, data_indx, &location);
    ...
    )
{
    ...
    res = sw_state_access[unit].dpp.soc.arad.tm.kbp_frwrd_ip.kbp_cache_mode.get(unit, &kbp_cache_mode);
    ...
    res = sw_state_access[unit].dpp.soc.arad.tm.kbp_frwrd_ip.location_tbl.set(unit, data_indx, &location);
...
uint32
  arad_pp_frwrd_ip_tcam_kbp_tcam_entry_add(int unit, ARAD_KBP_FRWRD_IP_TBL_ID frwrd_table_id, uint32 entry_id_ndx, uint8 is_for_update, uint32 priority, uint32 data[ARAD_TCAM_ENTRY_MAX_LEN], uint32  mask[ARAD_TCAM_ENTRY_MAX_LEN], uint32 value[ARAD_TCAM_ACTION_MAX_LEN], SOC_SAND_SUCCESS_FAILURE *success)
{
