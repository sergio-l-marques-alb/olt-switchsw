...
uint32
  arad_pp_eg_mirror_port_appl_set_unsafe(
	...
	SOC_SAND_IN  int        core_id,
    SOC_SAND_IN  ARAD_PP_PORT  pp_port, /* Local port ID */
  ...
  )
{
  ...
  res = arad_sw_db_is_port_reserved_for_reassembly_context(unit, pp_port, &is_reserved);
  ...
  if (is_reserved != (enable ? 1 : 0)) { /* need to do work only if the wanted state is different from the current one */
    ...
    res = arad_sw_db_set_port_reserved_for_reassembly_context(unit, pp_port, enable);
  ...
  )
{
  ...
  uint32 res;
  uint32 entry_context[3]; /* will hold an EGQ_PCTm entry */
  uint32 channel; /* reassembly context and recycle interface channel */
  ...
  else
  {
    ...
    if (interface_type == SOC_PORT_IF_ILKN) {
      ...
      SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res, 221, exit, ARAD_REG_ACCESS_ERR,get_recycling_port(unit, core_id, pp_port, channel, &rcy_port));
    }
