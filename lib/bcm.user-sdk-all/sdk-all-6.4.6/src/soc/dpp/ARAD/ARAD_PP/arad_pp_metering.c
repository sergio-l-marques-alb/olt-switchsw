...
/* 
    The InPpPortMapAndMtrPrfMap tables (one for each mcd on Jericho) map the MapTo3 profile in the meter profile
    and the InPpPortMap into a 8 bit.
	The table has 16 lines, each with 4 records, meaning a total of 64 records.
    The key structure: {in_pp_port_data_after_map, profile_map_to3}
*/
#define ARAD_PP_MTR_IN_PP_PORT_MAP_AND_MTR_PRF_MAP_NUM_LINES 16
  ...
  )
{
  ...
  if (mtr_group_ndx == 0)
  {
	...
	mem = MRPS_MCDA_IN_PP_PORT_MAP_AND_MTR_PRF_MAPm;
	field_ipg = MCDA_IN_PP_PORT_MAP_AND_MTR_PRF_MAP_0f;
  }
  else /* if (mtr_group_ndx == 1) */
  {
	...
	mem = MRPS_MCDB_IN_PP_PORT_MAP_AND_MTR_PRF_MAPm;
	field_ipg = MCDB_IN_PP_PORT_MAP_AND_MTR_PRF_MAP_0f;
  }
  ...
  if (SOC_IS_ARADPLUS(unit)){
	  if (!SOC_IS_JERICHO(unit)) {
		  /*On Arad+, there's a global bit to enable it */
		  res = soc_reg_field32_modify(unit, IDR_SPARE_REGISTER_2r, MEM_BLOCK_ANY, IPG_COMPENSATION_ENf,  mtr_group_info->is_ipg_compensation_enabled); 
		  SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
	  }
	  else{
		  /* 
		    On Jericho, all meters have MtrPrflMapTo3=0.
		    We'll configure the IN_PP_PORT_MAP_AND_MTR_PRF_MAP to add 20 bytes to the packet size for entries matching this profile.																												   .
		  */
		    fldval = 20;
			for(table_index = 0; table_index < ARAD_PP_MTR_IN_PP_PORT_MAP_AND_MTR_PRF_MAP_NUM_LINES; table_index += 2) {
				res = soc_mem_read(unit, mem, ARAD_PP_MTR_MEM_BLOCK(unit, core_id), table_index, &table_line);
				SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);

				soc_mem_field_set(unit, mem, &table_line, field_ipg, &fldval);

				res = soc_mem_write(unit, mem, ARAD_PP_MTR_MEM_BLOCK(unit, core_id), table_index, &table_line);
				SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
			}
		}
  }
...
#ifdef BCM_88660_A0
  if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
	  /* On Arad+, IPG compensation is the same for both groups. */
	  ...
	  /* On Jericho, all meter profiles are configured with MtrPrflMapTo3=0.
	     If the IN_PP_PORT_MAP_AND_MTR_PRF_MAP table is configured to add 20 bytes to all entries matching this profile, then IPG is enabled.
		 We check only entry 0.*/

	  res = soc_mem_read(unit, MRPS_MCDA_IN_PP_PORT_MAP_AND_MTR_PRF_MAPm, ARAD_PP_MTR_MEM_BLOCK(unit, core_id), 0, &table_line);
	  SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);

	  soc_mem_field_get(unit, MRPS_MCDA_IN_PP_PORT_MAP_AND_MTR_PRF_MAPm, &table_line, MCDA_IN_PP_PORT_MAP_AND_MTR_PRF_MAP_0f, &fldval);

	  mtr_group_info->is_ipg_compensation_enabled = (fldval == 20);
  }
#endif /* BCM_88660_A0*/
  ...
  )
{
  ...
  if (SOC_IS_JERICHO(unit)) {
	  /* 
		Allow later activation of IPG compenstaion by setting profile_map_to3 to zero and 
	    enabling the MRPS_MCDA_IN_PP_PORT_MAP_AND_MTR_PRF_MAP by default for every profile.
	  */
	  ...
	  */
	  soc_mem_field32_set(unit, format_mem, (void*)reg_above64_val, MTR_PROFILE_MAP_TO_4f, 8);
  }
  ...
  )
{
  ...
  soc_reg32_get(unit, IDR_ENABLE_DYNAMIC_MEMORY_ACCESSr, ARAD_PP_MTR_REG_PORT(unit,core_id), 0, &write_access_enable);
  if (0 == write_access_enable) {
	  res = soc_reg32_set(unit, IDR_ENABLE_DYNAMIC_MEMORY_ACCESSr, ARAD_PP_MTR_REG_PORT(unit,core_id), 0, 1);
	  SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
  }

  /* get bucket entries */
  if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
	  res = soc_mem_array_read(unit, buckets_mem, array_index, ARAD_PP_MTR_MEM_BLOCK(unit, core_id), index, buckets_mem_val); 
  }
  else{
	  res = soc_mem_read(unit, buckets_mem, ARAD_PP_MTR_MEM_BLOCK(unit, core_id), index, buckets_mem_val); 
  }
  SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);
  ...
  if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
	  res = soc_mem_array_write(unit, buckets_mem, array_index, ARAD_PP_MTR_MEM_BLOCK(unit, core_id), index, buckets_mem_val);
  }
  else{
	  res = soc_mem_write(unit, buckets_mem, ARAD_PP_MTR_MEM_BLOCK(unit, core_id), index, buckets_mem_val);
  }
  SOC_SAND_CHECK_FUNC_RESULT(res, 50, exit);

  /* if we took write access, disable it*/
  if (0 == write_access_enable) {
	  res = soc_reg32_set(unit, IDR_ENABLE_DYNAMIC_MEMORY_ACCESSr, ARAD_PP_MTR_REG_PORT(unit,core_id), 0, 0);
	  SOC_SAND_CHECK_FUNC_RESULT(res, 60, exit);
  }
exit:
