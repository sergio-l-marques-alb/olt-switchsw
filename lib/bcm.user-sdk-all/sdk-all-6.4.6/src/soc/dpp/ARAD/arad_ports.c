  ...
  )
{
  ...
  uint32 reg_val, is_valid, flags, offset;
...
port_termination_exit:
   ...
   */
    ...
    SOC_DPP_CORES_ITER(SOC_CORE_ALL, core) {
        for (offset = 0; offset < SOC_DPP_DEFS_MAX(NOF_INTERLAKEN_PORTS); offset++) {
            res = sw_state_access[unit].dpp.soc.arad.tm.tm_info.rcy_single_context_port.set(unit, core, offset, ARAD_PORT_INVALID_RCY_CONTEXT);
            SOC_SAND_CHECK_FUNC_RESULT(res, 96, exit);
        }
    }
    ...
    SOC_DPP_CORES_ITER(SOC_CORE_ALL, core) {
        for (offset = 0; offset < SOC_DPP_MAX_NOF_CHANNELS; offset++) {
            res = sw_state_access[unit].dpp.soc.arad.tm.tm_info.rcy_channel_ilkn_offset.set(unit, core, offset, ARAD_PORT_INVALID_PROTOCOL_OFFSET);
            SOC_SAND_CHECK_FUNC_RESULT(res, 96, exit);
        }
    }
        ...
        if (!skip_config)
        {
            ...
            if (!(SOC_PORT_IS_ELK_INTERFACE(flags) || SOC_PORT_IS_STAT_INTERFACE(flags))) {
                ...
                if (header_type != ARAD_PORT_HEADER_TYPE_TM)
                {
                    ...
                    {
                        SOCDNX_IF_ERR_EXIT(sw_state_access[unit].dpp.soc.arad.tm.arad_egr_ports.ports_prog_editor_profile.set(unit,port,eg_tm_profile));
                    }
    ...
    )
{
    ...
    if (out_enable)
    {
         ...
         */
            ...
            core_id = fap_id - sys_fap_id_self;
        ...
        if((lag_member->flags & SOC_PPC_LAG_MEMBER_INGRESS_DISABLE) == 0x0) 
        {
            ...
            res = sw_state_access[unit].dpp.soc.arad.tm.lag.in_use.set(unit, lag_ndx, in_use);
        ...
        if (removed_member_index != out_lag->nof_entries-1)
        {
            ...
            */
            ...
            {               
                ...
                {
                    core_id = moved_member_fap_id - sys_fap_id_self;
    ...
    )
{
    ...
    */
    ...
    if (out_enable)
    {
        ...
        res = sw_state_access[unit].dpp.soc.arad.tm.lag.in_use.set(unit, lag_ndx, in_use);
        SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 260, exit);
    }
  ...
  )
{
    SOCDNX_INIT_FUNC_DEFS;
    if (!SOC_IS_JERICHO(unit)) {
        SOCDNX_SAND_IF_ERR_EXIT(arad_port_ingr_map_write_val_unsafe(unit,port,is_mapped,map_info));
    }
...
uint32 alloc_reassembly_context_and_recycle_channel_unsafe(int unit, int core_id, uint32 out_pp_port, uint32 *channel, uint32 *reassembly_context)
{
  ...
  uint32 flags, pp_rcy_port, offset, tchannel;
  int core_rcy_port, offset_i;
  ...
  *channel = 0;
  ...
  /* single context for all channels */
  if (interface_type == SOC_PORT_IF_ILKN) {

      res = soc_port_sw_db_protocol_offset_get(unit, local_dest_port, 0, &offset);
      SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 20, exit);

      res = sw_state_access[unit].dpp.soc.arad.tm.tm_info.rcy_single_context_port.get(unit, core_id, offset, &rcy_port);
      SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 25, exit);
      if (rcy_port == ARAD_PORT_INVALID_RCY_CONTEXT) { /* first channel for this interface, allocate a port */
          res = MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_drv_allocate_rcy_port, (unit, core_id, &rcy_port));
          SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 30, exit);
          res = sw_state_access[unit].dpp.soc.arad.tm.tm_info.rcy_single_context_port.set(unit, core_id, offset, rcy_port);
          SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 35, exit);
          res = soc_port_sw_db_channel_get(unit, rcy_port, channel);
          SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 36, exit);

          if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
              *reassembly_context = rcy_port;
          } else {
          ...
          res = sw_state_access[unit].dpp.soc.arad.tm.tm_info.rcy_reassembly_ctxt.set(unit, core_id, offset, *reassembly_context);
          SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 46, exit);

      } else { /* find a free channel */

            /* loop over all local ports, marking used recycle channels, marking used rcy channels */
           ...
           for (*channel = 0; *channel < SOC_DPP_MAX_NOF_CHANNELS; (*channel)++) {
               res = sw_state_access[unit].dpp.soc.arad.tm.tm_info.rcy_channel_ilkn_offset.get(unit, core_id, *channel, &offset_i);
               SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 37, exit);

               if ((offset_i == ARAD_PORT_INVALID_PROTOCOL_OFFSET) && (!channels[*channel])) {
                   break;
               }
           }
           ...
           res = sw_state_access[unit].dpp.soc.arad.tm.tm_info.rcy_reassembly_ctxt.get(unit, core_id, offset, reassembly_context);
           SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 46, exit);
      }

      /* mark channel as used by the interface */
      res = sw_state_access[unit].dpp.soc.arad.tm.tm_info.rcy_channel_ilkn_offset.set(unit, core_id, *channel, offset);
      ...
      res = map_mirrored_port(unit, rcy_port, TRUE, &port_mapping_info);
      SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 90, exit);

      /* get reassembly context */
      res= MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_port_ingr_reassembly_context_get, (unit, rcy_port, &pp_rcy_port, reassembly_context));
...
STATIC 
uint32 
 get_context_map_base_address(
     SOC_SAND_IN int unit,
     SOC_SAND_IN int core_id,
     SOC_SAND_IN soc_port_t  pp_port, 
     SOC_SAND_OUT uint32 *base_address)
{
    uint32 data[2], cpu_offset, tm_port,reg32;
    soc_field_t field;
    ...
    SOCDNX_IF_ERR_EXIT(soc_port_sw_db_pp_to_local_port_get(unit, core_id, pp_port, &local_port));
    SOCDNX_IF_ERR_EXIT(soc_port_sw_db_local_to_tm_port_get(unit, local_port, &tm_port, &tmp_core));
    field =(core_id == 0 ? INTERFACE_EGQ_0_PORTf : INTERFACE_EGQ_1_PORTf);
    SOCDNX_IF_ERR_EXIT(READ_IRE_INTERFACE_PORT_CONFIGURATIONr(unit, &reg32));
    cpu_offset = soc_reg_field_get(unit, IRE_INTERFACE_PORT_CONFIGURATIONr, reg32, field);
    SOCDNX_IF_ERR_EXIT(READ_IRE_PORT_TO_BASE_ADDRESS_MAPm(unit, MEM_BLOCK_ANY, cpu_offset, data));
    *base_address_lcl=0;
     ...
     SOC_SAND_OUT soc_port_t *logical_port)
{
    ...
    SOCDNX_IF_ERR_EXIT(get_context_map_base_address(unit,core_id,pp_port,&base_address));
...
{
  int i, res, rcy_core, offset_i;
  uint32 offset, nof_rcy_channels = 0, tm_rcy_port;
  ...
  if (interface_type == SOC_PORT_IF_ILKN) {
      ...
      res = sw_state_access[unit].dpp.soc.arad.tm.tm_info.rcy_single_context_port.get(unit, core, offset, &rcy_port);
      SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 25, exit);
      if (rcy_port == ARAD_PORT_INVALID_RCY_CONTEXT) { /* no rcy channel exist */
          SOC_SAND_SET_ERROR_CODE(SOC_SAND_GEN_ERR, 30, exit);
      }

      /* count channels */
      for (i = 0; i < SOC_DPP_MAX_NOF_CHANNELS; i++) {
           res = sw_state_access[unit].dpp.soc.arad.tm.tm_info.rcy_channel_ilkn_offset.get(unit, core, i, &offset_i);
           SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 35, exit);
           if ((offset_i != ARAD_PORT_INVALID_RCY_CONTEXT) && (offset_i == offset)) {
               nof_rcy_channels++;
           }
      }
      ...
      if (nof_rcy_channels == 1) { /* if last rcy channel */
          ...
          if (SOC_IS_JERICHO(unit)) {
              res = sw_state_access[unit].dpp.soc.arad.tm.tm_info.rcy_reassembly_ctxt.get(unit, core, offset, &reassembly_context);
              SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 36, exit);
      ...
      res = soc_port_sw_db_local_to_tm_port_get(unit, rcy_port, &tm_rcy_port, &rcy_core);
      ...
      res = sw_state_access[unit].dpp.soc.arad.tm.tm_info.rcy_channel_ilkn_offset.set(unit, core, channel, ARAD_PORT_INVALID_PROTOCOL_OFFSET);
      SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 101, exit);

  } else {

      SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res, 221, exit, ARAD_REG_ACCESS_ERR,get_recycling_port(unit,core,out_pp_port, channel, &rcy_port));

      /* remove recycle interface channel to reassembly context (port) mapping */
      port_mapping_info.if_id = SOC_TMC_IF_ID_RCY;
      port_mapping_info.channel_id = channel;
      res = map_mirrored_port(unit, rcy_port, FALSE, &port_mapping_info);
    ...
    SOC_SAND_OUT uint32     enable)
{
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 3, exit, soc_port_sw_db_base_q_pair_get(unit, port, &base_q_pair));
   ...
   )
{
    ...
    if (interface != SOC_PORT_IF_ERP && (!SOC_PORT_IS_STAT_INTERFACE(flags)) ) {
