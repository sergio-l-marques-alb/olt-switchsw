...
#define ARAD_IPQ_MIN_INTERDIGIT_FLOW_QUARTET   (64 * 1024 / 4)
...
arad_ipq_traffic_class_profile_map_set_unsafe(
   ...
   for (is_ingress_shape = 0; is_ingress_shape < nof_entries; ++is_ingress_shape) {
      if (SOC_IS_JERICHO(unit)) {
           ...
           */
           entry_offset = ((is_flow_profile & 0x1) << 6) | ((profile_ndx & 0x3) << 4) | ((is_ingress_shape & 0x1) << 3) | (tr_cls_ndx & 0x7);
          ...
          if (is_flow_profile) {
             ...
             entry_offset = ARAD_IPQ_UC_FLOW_ID_TC_ENTRY(profile_ndx, is_ingress_shape);
          } else {
             /* Offset is for system port destination */
             entry_offset = ARAD_IPQ_UC_SYS_PORT_TC_ENTRY(profile_ndx, is_ingress_shape);
          }
         /*
...
arad_ipq_destination_id_packets_base_queue_id_set_unsafe(
   ...
   SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_IPQ_DESTINATION_ID_PACKETS_BASE_QUEUE_ID_SET_UNSAFE);
    ...
    if (!sw_only) {
       ...
       irr_destination_table_tbl_data.queue_number = (valid) ? base_queue : ARAD_IPQ_DESTINATION_ID_INVALID_QUEUE;
   ...
   )
{
    ...
    SOC_SAND_INIT_ERROR_DEFINITIONS(0);
     ...
     res = arad_iqm_dynamic_tbl_get_unsafe(unit,core,queue_id,&iqm_dynamic_tbl);
   ...
   if (soc_mem_dmaable(unit, IQM_PQDMDm, SOC_MEM_BLOCK_ANY(unit, IQM_PQDMDm))) {
      ...
      if (entry_array == NULL) {
         SOCDNX_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_SOCDNX_MSG("failed to allocate memory")));
          ...
          _rv = soc_mem_array_read_range(unit, IQM_PQDMDm, 0, IQM_BLOCK(unit, core), index_min, index_max, entry_array);
   ...
   SOC_DPP_CORES_ITER(core, core_index) {
       for (queue_id = first_queue; queue_id < nof_queues; ++queue_id) {
          if (entry_array != NULL) {
             ...
             res = arad_iqm_dynamic_tbl_get_unsafe(unit,core_index,queue_id,&iqm_dynamic_tbl);
