/*
 * $Id: alpm_trie.c,v 1.10 Broadcom SDK $
...
int print_trie_node(trie_node_t *trie, void *datum)
{
	...
	LOG_CLI((BSL_META("trie: %p, type %s, skip_addr 0x%x skip_len %d "
                 ...
                 trie->count, trie->child[0].child_node,
    ...
    if (lcp == length) { /* guaranteed: length < _MAX_SKIP_LEN_ */
        ...
        if (!trie->child[bit].child_node) {
            ...
            if (SOC_SUCCESS(rv)) {
                ...
                LOG_CLI((BSL_META("\n Error on trie skip node allocaiton "
                                  "[%d]!!!!\n"), rv));
            }
        } else { 
...
int tmu_trie_split_ut(unsigned int seed) 
{
    ...
    for (id=0; id < 4; id++) {
       ...
       break;
        ...
        if (SOC_SUCCESS(rv)) {
            LOG_CLI((BSL_META("\n Split Trie Pivot: 0x%08x 0x%08x Length: %d "
                              "Root: %p \n"),
            ...
            for(index=0; index < numkey && rv == SOC_E_NONE; index++) {
                ...
                if (rv != SOC_E_NONE) {
                    ...
                    if (rv != SOC_E_NONE) {
                        LOG_CLI((BSL_META("SEARCH: Key=0x%x 0x%x len %d "
                                          "SEARCH idx:%d failed on both trie!!!!\n"), 
...
int _trie_splitable(trie_node_t *trie)
{
    if (trie == NULL) {
        return 0;
    }

    if (trie->count <= 1 && trie->type == PAYLOAD) {
        return 0;
    }
	...
	if ((*length > max_split_len) && (trie->count != max_count)) {
		...
		*state = TRIE_SPLIT_STATE_PRUNE_NODES;
		return rv;
		        ...
		        (_trie_splitable(trie->child[bit].child_node) == 0) ||
		        ((ABS(trie->child[bit].child_node->count * 2 - max_count) >
	              ABS(trie->count * 2 - max_count)) && trie->count <= free_space)
		      ) {
	    /* 
		...
		*state = TRIE_SPLIT_STATE_PRUNE_NODES;
	    ...
	    rv = _trie_split(trie->child[bit].child_node, 
			     ...
			     split_to_pair, bpm, state, free_space);
	}
    }
    ...
    *length = 0;
	...
	if (trie->v6_key) {	    
	    ...
	    rv = _trie_split(trie->trie, pivot, length, &split_count, split_trie_root,
			     &child, max_count, max_split_len, split_to_pair, bpm, &state, free_space);
	}
