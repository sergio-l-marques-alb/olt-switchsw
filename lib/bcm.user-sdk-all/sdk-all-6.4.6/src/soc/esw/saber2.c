...
int _soc_saber2_hw_reset_control_init(int unit)
{
    ...
    soc_reg_field_set(unit, RXLP_HW_RESET_CONTROLr, &rval, COUNTf, 0x84);
...
int _soc_saber2_misc_init(int unit)
{
    ...
    if(!(SAL_BOOT_BCMSIM || SAL_BOOT_PLISIM || SAL_BOOT_XGSSIM)) {
       ...
       /* initialize LED UP0 */
        SOC_IF_ERROR_RETURN(_saber2_ledup_init(unit));
    }
...
_soc_saber2_mmu_port_init(int unit) {
    ...
    PBMP_ALL_ITER(unit, port) {
        if (si->port_speed_max[port] <= 2500) {
            depth = 11;
            /* For internally mapped port (using device fifo), threshold is zero */
            threshold = (IS_EXT_MEM_PORT(unit, port)) ? 8 : 0;
            port_bw = 25;
            start_threshold = 7;
        } else if (si->port_speed_max[port] <= 13000) {
            depth = 44;
            /* For internally mapped port (using device fifo), threshold is zero */
            threshold = (IS_EXT_MEM_PORT(unit, port)) ? 40 : 0;
            port_bw = 250;
            start_threshold = 48;
        } else { /* > 13G */
            return SOC_E_PARAM;
...
soc_sb2_mem_config(int unit, int dev_id)
{
    ...
    if (!(SAL_BOOT_BCMSIM || SAL_BOOT_PLISIM)) {
        ...
        case BCM56268_DEVICE_ID:
             ...
             sop->memState[MP_GROUPm].index_max = 1023;
             sop->memState[EGR_MP_GROUPm].index_max = 1023;
...
int soc_saber2_xl_port_speed_get(int unit, int port, int *speed) {
    ...
    SOC_IF_ERROR_RETURN(_soc_saber2_get_cfg_num(unit, &cfg_num));
    *speed = bcm56260_speed[unit][cfg_num][port-1];

    if ((*speed == 10000) && !(SOC_PBMP_MEMBER(my_pbmp_xport_xe, port))) {
        *speed = 11000;
    }
