...
soc_trident2_port_config_init(int unit, uint16 dev_id, uint8 rev_id)
{
...
#ifdef BCM_TRIDENT2PLUS_SUPPORT
    int flexport_reserve_HSP = 0;
    int flexport_reserve_T2OQ = 0;
    int flexport_max_speed = -1;
    int num_HSP_per_pipe_to_reserve = 0;
#endif
    si = &SOC_INFO(unit);
    ...
    if (SOC_IS_TRIDENT2PLUS(unit)) {
        if (soc_td2p_is_any_port_flex_enable(unit)) {
            ...
            if(flexport_max_speed > 42000) {
                flexport_reserve_HSP = 1;
                flexport_reserve_T2OQ = 1;
                /* only going to reserve 4 HSP and 4 T2OQ b/c this is per TSC-12 */
                num_HSP_per_pipe_to_reserve = _TD2_PGWS_PER_PIPE;
            }
            /* otherwise flexing only goes up to 40 gig */
            else {
                num_HSP_per_pipe_to_reserve = SOC_TD2_NUM_HSP_MMU_PORTS_PER_PIPE;

                /* Table 6-16 of TD2+ FlexPort uArch Spec */
                /* don't need to preallocate for frequencies at and above 960000 */
                if(si->bandwidth >= 960000) {
                }
                /* Need to preallocate 1 HSP port per port macro for above this freq */
                else if(si->bandwidth >= 720000) {
                    flexport_reserve_HSP = 1;
                    flexport_reserve_T2OQ = 0;
                }
                /* Otherwise, for freq 480000 and below need to preallocate 1 HSP and 1 T2OQ
                   per port macro */
                else {
                    flexport_reserve_HSP = 1;
                    flexport_reserve_T2OQ = 1;
    ...
    for (pipe = 0; pipe < _TD2_PIPES_PER_DEV; pipe++) {
        ...
        if (SOC_IS_TRIDENT2PLUS(unit)) {
            ...
            if (flexport_reserve_HSP) {
                for (i = 0; i < num_HSP_per_pipe_to_reserve; i++) {
                    /* mmu port 0-15 on each pipe; 0-15 on X Pipe, 64-79 on Y Pipe */
                    SOC_PBMP_PORT_ADD(pbmp_mmu_reserved, i + (pipe * _TD2_PORTS_PER_PIPE));
                    if(flexport_reserve_T2OQ == 1) {
                        /* mmu port 36-51 on each pipe; 36-51 on X Pipe, 100-115 on Y Pipe */
                        SOC_PBMP_PORT_ADD(pbmp_mmu_reserved, i + (pipe * _TD2_PORTS_PER_PIPE)+36);
                    }
                }
            }
        }
...
soc_trident2_chip_reset(int unit)
{
    ...
    if (soc_property_get(unit, "td2_sbus_mdio", SOC_IS_TRIDENT2PLUS(unit))) {
...
_soc_trident2_tdm_t *soc_td2_td2p_tdm_sched_info_get(int unit)
{
    return &_soc_td2_tdm;
}
                               ...
                               _soc_mmu_device_info_t *devcfg, int lossless)
{
    ...
    LOG_VERBOSE(BSL_LS_SOC_MMU,
                (BSL_META_U(unit,
                            "MMU config set HW: Cells rsvd Total : %d ing %d egr %d asf %d\n"),
                            (ing_rsvd + egr_rsvd + asf_rsvd), ing_rsvd, egr_rsvd, asf_rsvd));
    ...
    for (idx = 0; idx < _TD2_MMU_NUM_POOL; idx++) {
        ...
        limit = buf_pool->total - (pool_pg_headroom +
    ...
    for (idx = 0; idx < _TD2_MMU_NUM_POOL; idx++) {
        ...
        limit = buf_pool->total - (queue_guarantee + asf_rsvd)
            + (lossless ? 10 : 0);
    ...
    if (rev_id >= BCM56850_A2_REV_ID) {
        ...
        if (SOC_IS_TRIDENT2PLUS(unit)) {
            ...
            soc_td2p_set_obm_registers (unit, reg, speed, index, 
                    obm_inst, 1, PGW_OBM_INIT_SW_DEFAULT);
#endif
            }
        } else {
            _soc_trident2_quad_bandwidth_calculate
