...
_soc_td2_l2mod_fifo_process(int unit, uint32 flags, l2_mod_fifo_entry_t *entry)
{
    ...
    case 7: /* L2_BULK_REPLACE */
            /* When l2x_mode is L2MODE_FIFO, L2_BULK_REPLACE JUST FOR DELETE. */
#if defined(BCM_ESW_SUPPORT) && defined(BCM_CMICM_SUPPORT) && defined(BCM_XGS3_SWITCH_SUPPORT)
        (void)_soc_l2mod_sbus_fifo_field_get(0, L2_MOD_FIFO_ENABLE_L2_DELETEf, &delete_enable);
        if (SOC_CONTROL(unit)->l2x_mode == L2MODE_FIFO && 0 == delete_enable) {
            break;
        }
#endif
