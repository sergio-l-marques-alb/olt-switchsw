...
tdm_td2p_vmap_alloc( tdm_mod_t *_tdm )
{
	...
	for (j=0; j<(TDM_AUX_SIZE); j++) {
		...
		lr25[j] = TD2P_NUM_EXT_PORTS; 
		lr40[j] = TD2P_NUM_EXT_PORTS;
		lr50[j] = TD2P_NUM_EXT_PORTS; 
	...
	if(_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.y5 > 0){
		...
		if (tdm_td2p_check_pipe_ethernet(lr50[_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.y5],_tdm) && _tdm->_chip_data.soc_pkg.soc_vars.td2p.higig_mgmt==BOOL_FALSE) {
			if (tdm_core_postalloc(_tdm->_core_data.vmap, _tdm->_chip_data.soc_pkg.clk_freq, 50, &_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.yy, &(_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.y5), _tdm->_chip_data.soc_pkg.lr_idx_limit, lr50, TOKEN_50G, "50", TD2P_NUM_EXT_PORTS)!=PASS) {
				TDM_ERROR0("Critical error in 50G time vector preallocation\n");
				return 0;
			}
		}
		else {
			if (tdm_core_postalloc(_tdm->_core_data.vmap, _tdm->_chip_data.soc_pkg.clk_freq, 53, &_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.yy, &(_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.y5), _tdm->_chip_data.soc_pkg.lr_idx_limit, lr50, TOKEN_50G, "50", TD2P_NUM_EXT_PORTS)!=PASS) {
				TDM_ERROR0("Critical error in 50G time vector preallocation\n");
				return 0;
			}
		}
	...
	if (_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.y6 > 0) {
		...
		if (tdm_td2p_check_pipe_ethernet(lr25[_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.y6],_tdm) && _tdm->_chip_data.soc_pkg.soc_vars.td2p.higig_mgmt==BOOL_FALSE) {
			if (tdm_core_postalloc(_tdm->_core_data.vmap, _tdm->_chip_data.soc_pkg.clk_freq, 25, &_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.yy, &(_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.y6), _tdm->_chip_data.soc_pkg.lr_idx_limit, lr25, TOKEN_25G, "25", TD2P_NUM_EXT_PORTS)!=PASS) {
				TDM_ERROR0("Critical error in 25G time vector preallocation\n");
				return 0;
			}
		}
		else {
			if (tdm_core_postalloc(_tdm->_core_data.vmap, _tdm->_chip_data.soc_pkg.clk_freq, 27, &_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.yy, &(_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.y6), _tdm->_chip_data.soc_pkg.lr_idx_limit, lr25, TOKEN_25G, "25", TD2P_NUM_EXT_PORTS)!=PASS) {
				TDM_ERROR0("Critical error in 25G time vector preallocation\n");
				return 0;
			}
		}
