/*
 * $Id: fe2kxt_diags.h,v 1.4 Broadcom SDK $
...
void Fe2kxt_WrI(sbhandle hdl, uint32 mem, uint32 offs, uint32 data);
uint32 Fe2kxt_RdI(sbhandle hdl, uint32 mem, uint32 offs);
uint32 Fe2kxt_MemSizeEnc(uint32 bits);
void Fe2kxt_SetupCtlRegs(sbxDiagsInfo_t *pDiagsInfo);
uint32 Fe2kxt_WaitForPhase(sbhandle hdl, uint32 phase);
uint32 Fe2kxt_CheckErrors(sbhandle hdl,int32 seed,uint32 phase);
uint32 Fe2kxt_HashData(int32 seed, uint32 ulOffs, uint32 Inv);
uint32 Fe2kxt_crc32(uint32 x);
...
/* foward declarations */
extern void sigcatcher_fe2kxt(int); /* handler for ctrl-c */
...
int sbFe2kxtDiagsDumpExceptionPkt(sbxDiagsInfo_t *pDiagsInfo, uint32 qe_unit);

int32 sbFe2kxtDiagsInjectPciPacket(sbxDiagsInfo_t *pDiagsInfo);
int32 sbFe2kxtDiagsReceivePciPacket(sbxDiagsInfo_t *pDiagsInfo);
...
int sbFe2kxtCheckLoopbackStats(sbxDiagsInfo_t *pDiagsInfo, uint8 bExternal);

/* clear the queues */
void sbFe2kxtClearQueues(sbhandle userDeviceHandle);
int sbFe2kxt1GLoopbackSet(uint32 unit,uint32 loopback);
