...
#define SB_ZF_C2PPCAMCONFIGURATIONINSTANCE0ENTRY_GET_MASK6(nToData,pFromData,nMaxFromDataIndex) \
          do { \
           (nToData) = (uint32) ((pFromData)[50] >> 6) & 0x03; \
           (nToData) |= (uint32) (pFromData)[51] << 2; \
           (nToData) |= (uint32) (pFromData)[52] << 10; \
           (nToData) |= (uint32) ((pFromData)[53] & 0x3f) << 18; \
          } while(0)

#define SB_ZF_C2PPCAMCONFIGURATIONINSTANCE0ENTRY_GET_MASK5(nToData,pFromData,nMaxFromDataIndex) \
          do { \
           (nToData) = (uint32) ((pFromData)[46] >> 6) & 0x03; \
           (nToData) |= (uint32) (pFromData)[47] << 2; \
           (nToData) |= (uint32) (pFromData)[48] << 10; \
           (nToData) |= (uint32) (pFromData)[49] << 18; \
           (nToData) |= (uint32) ((pFromData)[50] & 0x3f) << 26; \
          } while(0)

#define SB_ZF_C2PPCAMCONFIGURATIONINSTANCE0ENTRY_GET_MASK4(nToData,pFromData,nMaxFromDataIndex) \
          do { \
           (nToData) = (uint32) ((pFromData)[42] >> 6) & 0x03; \
           (nToData) |= (uint32) (pFromData)[43] << 2; \
           (nToData) |= (uint32) (pFromData)[44] << 10; \
           (nToData) |= (uint32) (pFromData)[45] << 18; \
           (nToData) |= (uint32) ((pFromData)[46] & 0x3f) << 26; \
          } while(0)

#define SB_ZF_C2PPCAMCONFIGURATIONINSTANCE0ENTRY_GET_MASK3(nToData,pFromData,nMaxFromDataIndex) \
          do { \
           (nToData) = (uint32) ((pFromData)[38] >> 6) & 0x03; \
           (nToData) |= (uint32) (pFromData)[39] << 2; \
           (nToData) |= (uint32) (pFromData)[40] << 10; \
           (nToData) |= (uint32) (pFromData)[41] << 18; \
           (nToData) |= (uint32) ((pFromData)[42] & 0x3f) << 26; \
          } while(0)

#define SB_ZF_C2PPCAMCONFIGURATIONINSTANCE0ENTRY_GET_MASK2(nToData,pFromData,nMaxFromDataIndex) \
          do { \
           (nToData) = (uint32) ((pFromData)[34] >> 6) & 0x03; \
           (nToData) |= (uint32) (pFromData)[35] << 2; \
           (nToData) |= (uint32) (pFromData)[36] << 10; \
           (nToData) |= (uint32) (pFromData)[37] << 18; \
           (nToData) |= (uint32) ((pFromData)[38] & 0x3f) << 26; \
          } while(0)

#define SB_ZF_C2PPCAMCONFIGURATIONINSTANCE0ENTRY_GET_MASK1(nToData,pFromData,nMaxFromDataIndex) \
          do { \
           (nToData) = (uint32) ((pFromData)[30] >> 6) & 0x03; \
           (nToData) |= (uint32) (pFromData)[31] << 2; \
           (nToData) |= (uint32) (pFromData)[32] << 10; \
           (nToData) |= (uint32) (pFromData)[33] << 18; \
           (nToData) |= (uint32) ((pFromData)[34] & 0x3f) << 26; \
          } while(0)
...
#define SB_ZF_C2PPCAMCONFIGURATIONINSTANCE0ENTRY_GET_MASK0(nToData,pFromData,nMaxFromDataIndex) \
          do { \
           ...
           (nToData) |= (uint32) ((pFromData)[30] & 0x3f) << 24; \
          } while(0)
