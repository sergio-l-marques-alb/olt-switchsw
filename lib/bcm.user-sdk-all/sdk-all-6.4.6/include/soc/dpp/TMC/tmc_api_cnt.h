...
#define SOC_TMC_CNT_LIF_COUNTING_MASK_SET(lif_counting_mask, lif_range_stack_0, lif_range_stack_1, lif_range_stack_2, lif_range_stack_3) \
            (lif_counting_mask = \
            ((0x3 & lif_range_stack_0) << 0) | \
            ((0x3 & lif_range_stack_1) << 2) | \
            ((0x3 & lif_range_stack_2) << 4) | \
            ((0x3 & lif_range_stack_3) << 6))

#define SOC_TMC_CNT_LIF_COUNTING_MASK_GET(lif_counting_mask, lif_stack_level) ((lif_counting_mask & (0x2 << (lif_stack_level * 2))) >> (lif_stack_level * 2))

#define SOC_TMC_CNT_LIF_COUNTING_MASK_TO_FIELD_VAL(lif_counting_mask_i)  ((lif_counting_mask_i < 2) ? (lif_counting_mask_i + 2) : (lif_counting_mask_i - 2))

#define SOC_TMC_CNT_LIF_COUNTING_MASK_FIELD_VAL_GET(lif_counting_mask, lif_stack_level) \
            SOC_TMC_CNT_LIF_COUNTING_MASK_TO_FIELD_VAL(SOC_TMC_CNT_LIF_COUNTING_MASK_GET(lif_counting_mask, lif_stack_level))
/* } */
/*************
 * GLOBALS   *
 *************/
