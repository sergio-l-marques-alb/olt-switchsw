...
#ifdef BCM_TRIDENT2PLUS_SUPPORT
/* For START_CNT values below, refer Appendix of 
 * TD2_Cutthrough_programming_guideline.pdf. These values are used when a 
 * port is in cut-through mode and both EGR_EDB_XMIT_CTRL.WAIT_FOR_2ND_MOP and
 * EGR_EDB_XMIT_CTRL.WAIT_FOR_MOP need to be set to 0.
 */
#define _TD2P_LINERATE_START_CNT(speed) \
        (speed <= 10000 ? 7 : ((si->frequency <= 415) ? 8 : 7))

#define _TD2P_OVERSUB_START_CNT ((si->frequency >= 760 ? 12 :\
...
extern _soc_trident2_tdm_t *soc_td2_td2p_tdm_sched_info_get(int unit);
...
extern int 
soc_td2p_mmu_port_resource_set(int unit, soc_port_t port, int speed);
...
extern int soc_td2p_mmu_flexport_allocate_deallocate_ports(int unit, 
        int nport, soc_port_resource_t * flexport_data, int apply);
