 ...
 * $Id: bcmutils.c,v 1.3 Broadcom SDK $
 */
...
/* debatable */
#include <bcmstdlib.h>
#endif /* NDIS || vxworks || __vxworks || _CFE_ */
#include <bcmendian.h>
#include <bcmdevs.h>
...
pktq_pflush(osl_t *osh, struct pktq *pq, int prec, bool dir)
{
	...
	while (p) {
		q->head = PKTLINK(p);
		PKTSETLINK(p, NULL);
		PKTFREE(osh, p, dir, MEMORY_DDRRAM);
		q->len--;
		pq->len--;
		p = q->head;
	}
...
#ifndef BUILD_VXBOOT
	/* then query env from CFE */
	res = cfe_getenv((char *)name, str, sizeof(str));
	if (res != CFE_OK) {
	    return NULL;
	}
