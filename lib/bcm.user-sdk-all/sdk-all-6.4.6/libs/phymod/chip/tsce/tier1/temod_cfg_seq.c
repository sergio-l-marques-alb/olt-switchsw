...
int temod_get_plldiv(PHYMOD_ST *pc,  uint32_t *pll_div)
{
  ...
  *pll_div = PLL_CAL_CTL7r_PLL_MODEf_GET(reg_ctl7);
  TEMOD_DBG_IN_FUNC_VOUT_INFO(pc,("temod_get_pll_div: reg=0x%x\n", *pll_div));
...
int temod_set_an_port_mode(PHYMOD_ST* pc, int num_of_lanes, int starting_lane, int single_port)    /* SET_AN_PORT_MODE */
{
  ...
  TEMOD_DBG_IN_FUNC_INFO(pc);
  TEMOD_DBG_IN_FUNC_VIN_INFO(pc,("num_of_lanes: %d, starting_lane: %d, single_port: %d\n", num_of_lanes, starting_lane, single_port));
...
#ifdef _SDK_TEMOD_
int temod_refclk_set(PHYMOD_ST* pc, temod_ref_clk_t refclk) 
{
    MAIN0_SETUPr_t  reg_setup;
...
int temod_scrambling_dis_lkup_get(PHYMOD_ST* pc, temod_spd_intfc_type spd_intf, uint32_t* scrambling_dis)
{
  ...
  *scrambling_dis = eagle_sc_pmd_entry[speed_id].scrambling_dis;
  TEMOD_DBG_IN_FUNC_VOUT_INFO(pc,("scrambling_dis: %d", *scrambling_dis));
