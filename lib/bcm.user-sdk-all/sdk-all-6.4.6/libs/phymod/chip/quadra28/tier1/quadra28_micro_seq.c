                                 ...
                                 uint8_t prg_eeprom)
{
    ...
    chip_id =  _quadra28_get_chip_id(&pa);
    q28_max_lane = (chip_id == QUADRA28_82752_CHIP_ID) ? 2 : QUADRA28_MAX_LANE;
    PHYMOD_IF_ERR_RETURN(
        _quadra28_clear_mode_config(&pa));

    /* Step1: Enable broadcast mode */
    for (index = 0; index <= q28_max_lane - 1; index++)
    {  
    ...
    for (index = 0; index <= (q28_max_lane - 1); index += 2)
    {
         pa.addr = (phy_addr + index);
         if (((lane_map >> index) & 1) == 0x1) 
         {
