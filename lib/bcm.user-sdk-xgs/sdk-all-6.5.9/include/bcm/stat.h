...
/* Enable/disable the engine counting and eviction. */
typedef struct bcm_stat_engine_enable_s {
...
} bcm_stat_engine_enable_t;

#define BCM_STAT_COUNTER_ENGINE_RESET   0x00000001 /* Reset engine configuration
                                                      and counters memory. */

#ifndef BCM_HIDE_DISPATCHABLE

/* 
 * API enable/disable the engine counting and eviction (input and output.
 * Enable should be made after configure all parameters of the engine.
 * Changing engine configuration is allowed only if engine is fully
 * disabled. (counting and evcition).
 */
extern int bcm_stat_counter_engine_enable_set(
    int unit, 
    uint32 flags, 
    bcm_stat_engine_t *engine, 
    bcm_stat_engine_enable_t *enable);

/* determine the enabler status of the engine. */
extern int bcm_stat_counter_engine_enable_get(
    int unit, 
    uint32 flags, 
    bcm_stat_engine_t *engine, 
    bcm_stat_engine_enable_t *enable);
...
typedef enum bcm_stat_counter_engine_control_e {
    ...
    bcmStatCounterThreadEnable = 2,     /* General enabler for the SW BG thread
                                           of counter collecting. Irelevant for
                                           specific engine. (one thread for all
                                           engines) */
    bcmStatCounterNextConsecutiveEngine = 3 /* Allow the user to concatenate new
                                           engine to a data base (not part of
                                           the initial configuration). the arg
                                           represent the next engine id */
} bcm_stat_counter_engine_control_t;

#define BCM_STAT_COUNTER_ENGINE_CONTROL_CLEAR_SW_ONLY 0x1        /* Refer to control enum
                                                          bcmStatCounterClearAll
                                                          - clear only SW
                                                          counters memory. */
#define BCM_STAT_COUNTER_ENGINE_CONTROL_CLEAR_HW_ONLY 0x2        /* Refer to control enum
                                                          bcmStatCounterClearAll
                                                          - clear only HW
                                                          counters memory. */
...
typedef enum bcm_stat_counter_interface_type_e {
    ...
    bcmStatCounterInterfaceIngressMeter = 2, 
    bcmStatCounterInterfaceIngressVoq = 3, 
    bcmStatCounterInterfaceIngressTransmitPp = 4, 
    bcmStatCounterInterfaceEgressReceivePp = 5, 
    bcmStatCounterInterfaceEgressReceiveQueue = 6, 
    bcmStatCounterInterfaceEgressTransmitPp = 7, 
    bcmStatCounterInterfaceEgressOam = 8, 
    bcmStatCounterInterfaceEgressMeter = 9, 
    bcmStatCounterInterfaceCount = 10 
...
typedef struct bcm_stat_counter_interface_s {
    bcm_stat_counter_interface_type_t engine_source; /* Counting source. */
    ...
    int database_id;                    /* database_id should be the same for
                                           all the engines that belong to one
                                           database. */
    int next_engine_id;                 /* Define the next engine id that is
                                           follow after the current engine. (-1)
                                           means none. */
...
/* API is used to configure the engine interface. */
...
/* API is used to get the engine interface configuration. */
...
/* API set the counter set size and the counter set mapping. */
...
/* API get the counter set size and the counter set mapping. */
...
/* init bcm_stat_engine_enable_t */
extern void bcm_stat_engine_enable_t_init(
