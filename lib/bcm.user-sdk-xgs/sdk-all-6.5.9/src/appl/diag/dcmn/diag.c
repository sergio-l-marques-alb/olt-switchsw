...
STATIC int diag_discard_interrupt_to_string(int unit, 
    ...
    uint8               reject_or_discard  = reg == EGQ_ERPP_DISCARD_INTERRUPT_REGISTERr;
...
STATIC int diag_print_iqm_rejects_and_erpp_discards(int unit, uint8 core_disp) {
    ...
    {
        if (SOC_IS_QAX(unit)){
            if (BCM_E_NONE != READ_CGM_REJECT_STATUS_BITMAPr(unit, &cgm_reg_val)) {
               return CMD_FAIL;
            }
        }
        else{
    ...
    do {
...
#endif
        {
            ...
            num_of_rejects += diag_discard_interrupt_to_string(unit, 
                                                              EGQ_ERPP_DISCARD_INTERRUPT_REGISTERr, 
                                                              erpp_reg_val64[core], 
...
int jericho_counters_graphic_print(int unit, uint32 print_none_zero, uint32 hex_base, uint32 core_disp) {
    ...
    if (!core_disp) {
        ...
        /* remove IPT EGQ0/1 PKT CNT (not reliable due to a bug) */
...
int qax_counters_graphic_print(int unit, uint32 print_none_zero, uint32 hex_base, uint32 core_disp) {
    ...
    for (i = 0; i < QAX_NOF_GRAPHIC_COUNTERS; ++i)
    {
       if (i == QAX_TXQ_EGQ_SRAM_COUNTER ||
           ...
           i == SPB_ALMOST_FULL_SRAM_REJECT_COUNTER)
        {
    ...
    rqp_counters_sum_array[0] = SPB_ALMOST_FULL_SRAM_REJECT_COUNTER;
    qax_compute_sum_rqp_counters64(unit, rqp_reg[SPB_ALMOST_FULL_SRAM_REJECT_COUNTER], rqp_counters_sum_array, 1, qax_counters, print_none_zero, hex_base, &sum_counters);
    print_counter_val_to_string(unit, sum_counters, overflow, print_none_zero, hex_base, cntbuf[0]);
    ...
    print_g_line("SPB_AF_SRAM_RJ_PKT_CNT", NULL, "PQP_DSCRD_UC_PKT_CNT", NULL, cntbuf);

    COMPILER_64_ZERO(sum_counters);
    COMPILER_64_ADD_64(sum_counters ,counters_data[SPB_ALMOST_FULL_0_SRAM_REJECT_COUNTER]);
    COMPILER_64_ADD_64(sum_counters ,counters_data[SPB_ALMOST_FULL_1_SRAM_REJECT_COUNTER]);
    COMPILER_64_ADD_64(sum_counters ,counters_data[SPB_ALMOST_FULL_2_SRAM_REJECT_COUNTER]);
    print_counter_val_to_string(unit, sum_counters, overflow, print_none_zero, hex_base, cntbuf[0]);
    ...
    print_g_line("SPB_AF_N_SRAM_RJ_PKT_CNT", NULL, "PQP_UC_BYTES_CNT", NULL, cntbuf);
    ...
    print_g_line(NULL, NULL, "DELETED_PKT_CNT", NULL, cntbuf);
...
int qux_counters_graphic_print(int unit, uint32 print_none_zero, uint32 hex_base, uint32 core_disp)
{
    ...
    {
        if (i == QUX_TXQ_EGQ_SRAM_COUNTER ||
            i == QUX_TXQ_EGQ_DRAM_COUNTER ||
            i == QUX_MMU_IDR_PACKET_COUNTER)
        {
    ...
    if (!core_disp) {
        ...
        compute_counter_val_into_string_2(unit, dual_counters_data[QUX_EPNI_EPE_DISCARDED_PACKETS_COUNTER], EPNI_EPE_DISCARDED_PACKETS_COUNTERr,
                                          EPE_DISCARDED_PACKETS_COUNTERf, 46, print_none_zero, hex_base, cntbuf[2]);
        print_g_line("OLP_PACKET_COUNTER", counter_alignment, "EPE_DSCRD_PKT_CNT", "EPE_DSCRD_PKT_CNT", cntbuf);
    }
    ...
    rqp_counters_sum_array[0] = QUX_MMU_IDR_PACKET_COUNTER;
    qax_compute_sum_rqp_counters32(unit, rqp_reg[QUX_MMU_IDR_PACKET_COUNTER], rqp_counters_sum_array, 1, qux_counters, print_none_zero, hex_base, &sum_counters);
    print_counter_val_to_string(unit, sum_counters, overflow, print_none_zero, hex_base, cntbuf[0]);
    ...
    print_g_line("CGM_VOQ_DRAM_DEL_PKT_CTR", NULL, NULL, NULL, cntbuf);
...
print_dpp_pll_val(int unit, char *pll, soc_dpp_pll_info_t *pll_info) {
    ...
    if (!sal_strncasecmp(pll, "CORE", strlen(pll))) {
        current_pll=&(pll_info->core_pll);
    } else if (!sal_strncasecmp(pll, "UC", strlen(pll))) {
        current_pll=&(pll_info->uc_pll);
    } else if (!sal_strncasecmp(pll, "TS", strlen(pll))) {
        current_pll=&(pll_info->ts_pll);
    } else if (!sal_strncasecmp(pll, "BS0", strlen(pll))) {
        current_pll=&(pll_info->bs_pll[0]);
    } else if (!sal_strncasecmp(pll, "BS1", strlen(pll))) {
        current_pll=&(pll_info->bs_pll[1]);
    } else if (!sal_strncasecmp(pll, "PMH", strlen(pll))) {
        current_pll=&(pll_info->pmh_pll);
    } else if (!sal_strncasecmp(pll, "PML0", strlen(pll))) {
        current_pll=&(pll_info->pml_pll[0]);
    } else if (!sal_strncasecmp(pll, "PML1", strlen(pll))) {
        current_pll=&(pll_info->pml_pll[1]);
    } else if (!sal_strncasecmp(pll, "FAB0", strlen(pll))) {
        current_pll=&(pll_info->fab_pll[0]);
    } else if (!sal_strncasecmp(pll, "FAB1", strlen(pll))) {
        current_pll=&(pll_info->fab_pll[1]);
    } else if (!sal_strncasecmp(pll, "SRD0", strlen(pll))) {
        current_pll=&(pll_info->srd_pll[0]);
    } else if (!sal_strncasecmp(pll, "SRD1", strlen(pll))) {
        current_pll=&(pll_info->srd_pll[1]);
    } else if (!sal_strncasecmp(pll, "DDR0", strlen(pll))) {
        current_pll=&(pll_info->ddr_pll[0]);
    } else if (!sal_strncasecmp(pll, "DDR1", strlen(pll))) {
        current_pll=&(pll_info->ddr_pll[1]);
    } else if (!sal_strncasecmp(pll, "DDR2", strlen(pll))) {
        current_pll=&(pll_info->ddr_pll[2]);
    } else if (!sal_strncasecmp(pll, "DDR3", strlen(pll))) {
...
cmd_dpp_nif(int unit, args_t *a) {
    ...
    while (NULL != option) {
        if (!sal_strncasecmp(option, "norates", strlen(option))) {
            calc_rates = 0;
        }
        option = ARG_GET(a);
    }
...
cmd_dpp_multicast_parse(int unit, args_t *a, int *is_egress, int *members, int *mcdb, int *mcid_min, int *mcid_max) {
    ...
    if (option == NULL) {
        ...
        return CMD_FAIL;
    } else if (!sal_strncasecmp(option, "ingress", strlen(option))) {
        *is_egress=0;
    } else if (!sal_strncasecmp(option, "egress", strlen(option))) {
        *is_egress=1;
    } else if (!sal_strncasecmp(option, "?", strlen(option))) {
        cmd_dpp_multicast_usage();
        return CMD_OK;
    } else {
        cli_out("Invalid option: %s \n", option);
    ...
    if (option == NULL) {
        ...
        *mcid_min=sal_ctoi(option,0);
        ...
        if (option == NULL) {
            cmd_dpp_multicast_usage();
            return CMD_FAIL;
        } else if (!sal_strncasecmp(option, "members", strlen(option))) {
            *mcid_max=*mcid_min;
            *members=1;
        } else if (!sal_strncasecmp(option, "mcdb", strlen(option))) {
            *mcid_max=*mcid_min;
            *mcdb=1;
        } else {
    ...
    if (mcid_min==mcid_max) {
        ...
        if (mcdb) {
            ...
            if(((flags & BCM_MULTICAST_INGRESS_GROUP)&&(!is_egress)) || ((flags & BCM_MULTICAST_EGRESS_GROUP)&&(is_egress))) {
                ...
                rv=dpp_mcdb_index_get(unit, current_mcid, is_egress, &group_size, index_array0, index_array1);
                if (BCM_FAILURE(rv)) {
                ...
                cli_out(" --------------------- \n");
                for (repl=0; repl<group_size; repl++) {
                ...
                if ((SOC_DPP_CONFIG(unit)->core_mode.nof_active_cores==2) && is_egress) {
                    cli_out("\n");
                    cli_out("\n");
...
dpp_diag_counters_print(int unit, diag_counter_data_t *counters, int ignore_zero, uint32 hex_base, char **block_names, int num_of_block_names) {
   ...
   for (i = 0; INVALIDr != counters[i].reg; i++) {
      ...
      SOC_BLOCKS_ITER(unit, blk, SOC_REG_INFO(unit, counters[i].reg).block) {
         ...
         if (SOC_BLOCK_IN_LIST(SOC_REG_INFO(unit, counters[i].reg).block, SOC_BLK_PORT) && !SOC_PORT_VALID(unit,SOC_BLOCK_INFO(unit, blk).number)) {
             continue;
         }
...
cmd_dpp_diag_rate(int unit, args_t *a) {
   ...
   while (NULL != option) {
      ...
      } else if (!SOC_IS_QAX(unit) && !sal_strncasecmp(option, "IQM", strlen(option))) {
         mode_given++;
         if (port_num == -2) {
      ...
      } else if (SOC_IS_QAX(unit) && !sal_strncasecmp(option, "CGM", strlen(option))) {
         mode_given++;
         if (calc_cgm_rate(unit) != CMD_OK) retCode = CMD_FAIL;
      } 


      else if (!SOC_IS_QAX(unit) && !sal_strncasecmp(option, "IPT", strlen(option))) {
         mode_given++;
         if (calc_ipt_rate(unit) != CMD_OK) retCode = CMD_FAIL;
      }
      else if (SOC_IS_QAX(unit) && !sal_strncasecmp(option, "PTS", strlen(option))) {
         mode_given++;
         if (calc_pts_rate(unit) != CMD_OK) retCode = CMD_FAIL;
      }
      else if (SOC_IS_QAX(unit) && !sal_strncasecmp(option, "TXQ", strlen(option))) {
         mode_given++;
...
dpp_diag_resource_allocation(int unit, args_t *a)
{
    ...
    if (res == CMD_OK) {
        ...
        } else if (!sal_strncasecmp(type, "dst", strlen(type))) {
   ...
   if (SOC_IS_DPP(unit)) {
      ...
      while (NULL != option) {
         if (!sal_strncasecmp(option, "print_flow_and_up", strlen(option))) {
            ...
            print_flow_and_up = 0;
            local_to_sys = 1;
         } else if (!sal_strncasecmp(option, "print_mapping", strlen(option))) {
            print_flow_and_up = 0;
            print_mapping = 1;
...
cmd_diag_header_diff(int unit, args_t* a)
{
    ...
    BCM_DPP_CORES_ITER(BCM_CORE_ALL, core) {
        if (!sal_strncasecmp(subcmd, "Ingress", strlen(subcmd))) {
            ...
            if (SOC_IS_QAX(unit)) {
                rv = WRITE_CGM_VOQ_SRAM_ENQ_BYTE_CTRr(unit, reg64); 
                if(rv != SOC_E_NONE) {
                    return CMD_FAIL;
                }
                rv = WRITE_CGM_VOQ_SRAM_ENQ_PKT_CTRr(unit, 0);
                if(rv != SOC_E_NONE) {
                    return CMD_FAIL;
                }
            } else {
            ...
            if (SOC_IS_QAX(unit)) {
                rv = READ_CGM_VOQ_SRAM_ENQ_PKT_CTRr(unit, &pckts_num32);
                if(rv != SOC_E_NONE) {
                    return CMD_FAIL;
                }
                rv = READ_CGM_VOQ_SRAM_ENQ_BYTE_CTRr(unit, &bytes_num64);
                if(rv != SOC_E_NONE) {
                    return CMD_FAIL;
                }
            } else {
...
cmd_dpp_diag(int unit, args_t *a) {
...
#ifdef BCM_PETRA_SUPPORT
    if (!sal_strncasecmp(function, "pll", strlen(function))) {
      if (SOC_IS_ARAD(unit)) {
         return cmd_dpp_pll(unit, a);
      } else {
         return CMD_USAGE;
      }
    } else if (!sal_strncasecmp(function, "lag", strlen(function))) {
      if (SOC_IS_ARAD(unit)) {
         return cmd_dpp_lag(unit, a);
      } else {
         return CMD_USAGE;
      }
    } else if (!sal_strncasecmp(function, "nif", strlen(function))) {
      if (SOC_IS_ARAD(unit)) {
    ...
    } else if (!sal_strncasecmp(function, "multicast", strlen(function))) {
      if (SOC_IS_ARAD(unit)) {
...
#ifdef BCM_JERICHO_SUPPORT
    else if (! sal_strncasecmp(function, "resource_allocation", strlen(function)) ) {
