...
appl_dpp_convert_modport_to_sysport_id(int unit, int modid_idx,int port)
{
    ...
    {
        /* 
         * Only ERP is expected to have bigger values then APPL_DNX_NOF_SYS_PORTS_PER_MODID
         * So for brining up an interop system ERP port should be later reconfigured in both devices 
         *   
         */
        if (port < APPL_DNX_NOF_SYS_PORTS_PER_MODID)
        {
            rv = appl_dnx_modid_to_sysport_convert(unit, modid_idx, port, &sys_port);
            if (rv < 0)
            {
                sys_port = -1;
            }
...
/* Run DNX voqs allocation */
    ...
    if (is_dnx_app)
    {
        rv = appl_dnx_sysport_id_to_base_voq_id_convert(unit, sysport, &base_voq);
        if (rv < 0)
        {
            return -1;
        }
...
/* Run DNX voqs allocation */
    ...
    if (is_dnx_app)
    {
        ingress_modid = ingress_modid/MAX_MODIDS_PER_DEVICE;
        rv = appl_dnx_modid_to_base_voq_connector_id_convert(unit, local_port, core_id, ingress_modid, g_dii.nof_devices, &voq_conn_id);
        if (rv < 0)
        {
            return -1;
        }
...
#endif  /*(defined(__DUNE_GTO_BCM_CPU__) || defined(__DUNE_WRX_BCM_CPU__) || defined(__DUNE_SLK_BCM_CPU__)) && defined(BCM_PETRA_SUPPORT) */
