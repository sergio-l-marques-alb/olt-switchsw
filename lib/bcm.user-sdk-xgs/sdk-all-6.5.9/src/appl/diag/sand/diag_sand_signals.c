...
static sh_sand_man_t signal_parser_man = {
    ...
    "sig prs SRv6_UNTERM_Qualifier=0x1264\n"
...
static sh_sand_option_t signal_parser_options[] = {
    ...
    {"size",  SAL_FIELD_TYPE_STR, "value size in bits",                 "0"},
    {"block", SAL_FIELD_TYPE_STR, "expansion specific block name",      ""},
    {"from",  SAL_FIELD_TYPE_STR, "expansion specific from stage name", ""},
    {"order", SAL_FIELD_TYPE_ENUM, "print values in certain endian order <little/big>", "big", (void *)order_enum_table},
    {NULL}
};
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SH_SAND_GET_ITERATOR(sand_arg)
    {
        ...
        value       = sand_arg->param.array_uint32;
        sand_signal_parse_get(unit, expansion_n, block_n, from_n, output_order, value, size_in, dsig_list);
    }
    ...
    RHITERATOR(signal_output, dsig_list)
    {
        ...
        SHR_CLI_EXIT_IF_ERR(sand_signal_expand_print(signal_output->field_list, 1, prt_ctr, attr_offset, SIGNALS_PRINT_VALUE), "");
    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    RHITERATOR(cur_sigstruct, device->struct_list)
    {
         ...
         */
        if (!ISEMPTY(from_n) && (ISEMPTY(cur_sigstruct->from_n) || (sal_strcasestr(cur_sigstruct->from_n, from_n) == NULL)))
            continue;
        /*
         * If there is specific block name, only signal belonging to this block may be matched to it
         */
        if (!ISEMPTY(block_n) && (ISEMPTY(cur_sigstruct->block_n) || (sal_strcasestr(cur_sigstruct->block_n, block_n) == NULL)))
            continue;
