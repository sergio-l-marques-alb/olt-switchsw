    ...
    switch(reginfo->regtype)
    {
        ...
        case soc_genreg:
            ...
            SHR_IF_ERR_EXIT_WITH_LOG(soc_reg_above_64_get(unit, reg, SOC_BLOCK_PORT(unit, block_id), reg_index, value),
                                    ...
                                    SOC_REG_NAME(unit, reg), reg_index, SOC_BLOCK_NAME(unit, block_id));
            break;
        case soc_portreg:
            ...
            case soc_portreg:
                PBMP_ITER(PBMP_PORT_ALL(unit), port)
                {
                    ...
                    if (SOC_REG_BLOCK_IN_LIST(unit, reg, SOC_PORT_TYPE(unit, port)))
                    {
                        ...
                        {
                            access_reg_print(prt_ctr, unit, reg, SOC_PORT_NAME(unit, port), 0, SOC_REG_BASE(unit, reg), reg_value, field_flag, changed, fld_match_list);
                        }
                    }
                }
                break;
            ...
            default:
                ...
                {   /*
                    ...
                    for (i_ind = 0; i_ind < numels; i_ind++)
                    {
                        ...
                        {
                            access_reg_print(prt_ctr, unit, reg, SOC_BLOCK_NAME(unit, i_bl), i_ind, address, reg_value, field_flag, changed, fld_match_list);
                        }
                    }
                }
                break;
...
static sh_sand_man_t access_hit_man = {
    "Obtain hit entries from cmodel server and read the content",
...
static sh_sand_option_t access_hit_options[] = {
    ...
    {"property",    SAL_FIELD_TYPE_STR,  "Name or name substring for regs/mems properties/n"
                                         "<reg/mem/array/ro/wo regtype name:<cpu/general/iproc/custom/port>",   ""},
    {"cache",       SAL_FIELD_TYPE_BOOL, "flag used to obtain non-cached memory value",                         "Yes"},
    {"raw",         SAL_FIELD_TYPE_BOOL, "Show raw numbers from C-model",                                       "No"},
    {NULL}
};
    ...
    sh_sand_control_t * sand_control)
{
    ...
    while(hit_data_length >= CMODEL_HIT_INFO_SIZE)
    {
        int unit;
        opcode = *hit_data++;
        unit          = bcm_ntohl(*((int *)hit_data));    hit_data += sizeof(int);
        ...
        if((opcode == UPDATE_TO_SOC_REG_READ) || (opcode == UPDATE_TO_SOC_REG_WRITE))
        {
            ...
            for (reg = 0; reg < NUM_SOC_REG; reg++)
            {
                ...
                {
                    cli_out("Block:0x%08x Offset:0x%08x Base:0x%08x Register:%s Invalid Offset", i_bl, offset, base_address, SOC_REG_NAME(unit, reg));
                    continue;
                }
                else if((i_bl < 0) || (i_bl >= SOC_MAX_NUM_BLKS))
                {
                    cli_out("Block:0x%08x Offset:0x%08x Base:0x%08x Register:%s Invalid Block", i_bl, offset, base_address, SOC_REG_NAME(unit, reg));
                    continue;
                }
                ...
                switch(reginfo->regtype)
                {
                    ...
                    default:
                        ...
                        else
                        {
                            ...
                            SOC_REG_ABOVE_64_CLEAR(reg_value);
                            address = soc_reg_addr_get(unit, reg, SOC_BLOCK_PORT(unit, i_bl), offset, SOC_REG_ADDR_OPTION_NONE, &blk, &acc_type);
                        }
                        access_reg_print(prt_ctr, unit, reg, SOC_BLOCK_NAME(unit, i_bl), offset, address, reg_value, field_flag, changed, fld_match_list);
                        break;
                }
...
#ifdef CMODEL_SERVER_MODE
    {"hit",    access_hit_cmd,  NULL, access_hit_options,   &access_hit_man},
#endif
    {"property", access_property_cmd,  NULL, access_property_options,  &access_property_man},
    {NULL}
};

sh_sand_cmd_t sh_sand_access_test_cmds[] = {
