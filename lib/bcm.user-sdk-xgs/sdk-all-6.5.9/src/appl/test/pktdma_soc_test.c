...
#define NUM_LB_PORTS_PARAM_DEFAULT 10
...
soc_get_chans_per_cmc(int unit)
{
    ...
    if (SOC_IS_TRIDENT3(unit)) {
        return (SOC_VCHAN_NUM(unit) / SOC_CMCS_NUM(unit));
    } else
#endif
...
/* VCHANS_PER_DEVICE */
static uint32
soc_get_vchans_per_device(int unit)
{
    return (soc_get_cmcs_per_device(unit) * soc_get_chans_per_cmc(unit));
}
...
#ifdef BCM_TRIDENT3_SUPPORT
    if (SOC_IS_TRIDENT3(unit)) {
        soc_dma_chan_ctrl_reg_get(unit, vchan, rdata);
    } else
#endif
    if (SOC_IS_TOMAHAWK(unit) || SOC_IS_TRIDENT2PLUS(unit)) {
        soc_get_cmc_ch_from_vchan(unit, vchan, &cmc, &ch);
...
pktdma_parse_test_params(int unit, args_t *a)
{
    ...
    /* print device CHANNEL info: parsed from pci_cmcs_num=xxx in config.bcm */
    cli_out("\n\nDEVICE CHANNEL INFO");
    cli_out("\nCMC_PER_DEVICE   = %0d", soc_get_cmcs_per_device(unit));
    cli_out("\nCH_PER_CMC       = %0d", soc_get_chans_per_cmc(unit));
    cli_out("\nVCHAN_PER_DEVICE = %0d", soc_get_vchans_per_device(unit));
    cli_out("\n");
    ...
    for (channel = 0; channel < MAX_CHANNELS; channel++) {
        ...
        if (channel < soc_get_vchans_per_device(unit)) {
    ...
    if (pktdma_p->test_type_rx_param == DESC_CHAIN_PKT_RELOAD ||
        ...
        if (pktdma_p->stream_rx_param == 0 ||
            ...
            test_error(unit,
                "\n*WARN: TestTypeRx/Tx=4 sets up an infinite descriptor "
...
pktdma_set_source_chan(int unit)
{
    ...
    for (rx_ch = 0; rx_ch < MAX_CHANNELS; rx_ch++) {
        if (pktdma_p->rx_channel_en[rx_ch]) {
            while (tx_ch < MAX_CHANNELS) {
                if (pktdma_p->tx_channel_en[tx_ch]) {
...
#ifdef BCM_TRIDENT3_SUPPORT
    if (SOC_IS_TRIDENT3(unit)) {
        soc_dma_chan_poll_done(unit, vchan, type, detected);
    } else
#endif
    if (SOC_IS_TOMAHAWK(unit) || SOC_IS_TRIDENT2PLUS(unit)) {
        soc_get_cmc_ch_from_vchan(unit, vchan, &cmc, &ch);
        ...
        for (j = 0; j < pktdma_p->num_pkts_rx_param; j++) {
            ...
            for (k = 0; k < pkt_size; k++) {
                if (packet[k] != exp_packet[k]) {
                    test_error(unit, "\n*ERROR Corrupt packet received on "
...
/*
 * Function:
 *      pktdma_set_cos_4_cpu_td3
 * Purpose:
 *      Program CPU_COS_MAP table for 16 priorities. Maps each internal
 *      priority to an unique COS queue. (Trident3)
 * Parameters:
 *      unit: StrataSwitch Unit #.
 *
 * Returns:
...
#ifdef BCM_TRIDENT3_SUPPORT
static void
pktdma_set_cos_4_cpu_td3(int unit)
{
...
#ifdef BCM_TRIDENT3_SUPPORT
static void
pktdma_set_cos_4_ipep_td3(int unit)
{
    uint32 i, j, entry_size, entry_num;
    ...
    if (SOC_IS_TRIDENT3(unit)) {
        ...
        if (SOC_MEM_IS_VALID(unit, EGR_VLAN_CONTROL_1m)) {
            entry_size = sizeof(egr_vlan_control_1_entry.entry_data) /
                         sizeof(uint32);
            for (j = 0; j < entry_size; j++) {
                egr_vlan_control_1_entry.entry_data[j] = 0x0;
            }
        ...
        if (SOC_MEM_IS_VALID(unit, ING_OUTER_DOT1P_MAPPING_TABLEm)) {
            ...
            entry_size = sizeof(ing_outer_dot1p_mapping_table_entry.entry_data) /
                         sizeof(uint32);
            cli_out("\n[ING_OUTER_DOT1P_MAPPING_TABLEm] entry_num %d, entry_size %d",
        ...
        if (SOC_MEM_IS_VALID(unit, PHB_MAPPING_TBL_1m)) {
            entry_num = soc_mem_index_max(unit, PHB_MAPPING_TBL_1m);
            entry_size = sizeof(phb_mapping_tbl_1_entry.entry_data) /
                         sizeof(uint32);
            cli_out("\n[PHB_MAPPING_TBL_1m] entry_num %d, entry_size %d",
                    entry_num, entry_size);
            for (i = 0; i < entry_num; i++) {
                for (j = 0; j < entry_size; j++) {
                    phb_mapping_tbl_1_entry.entry_data[j] = 0x0;
                }
        ...
        if (SOC_MEM_IS_VALID(unit, PORT_COS_MAPm)) {
            ...
            entry_size = sizeof(port_cos_map_entry.entry_data) /
                         sizeof(uint32);
            cli_out("\n[PORT_COS_MAPm] entry_num %d, entry_size %d",
                    entry_num, entry_size);
            for (i = 0; i < entry_num; i++) {
                for (j = 0; j < entry_size; j++) {
...
/*                 soc_mem_field32_set(unit, PORT_COS_MAPm, port_cos_map_entry.entry_data,
        ...
        if (SOC_MEM_IS_VALID(unit, EGR_ZONE_3_DOT1P_MAPPING_TABLE_3m)) {
            entry_num = soc_mem_index_max(unit, EGR_ZONE_3_DOT1P_MAPPING_TABLE_3m);
            entry_size = sizeof(egr_zone_3_dot1p_mapping_table_3_entry.entry_data) /
                         sizeof(uint32);
            cli_out("\n[EGR_ZONE_3_DOT1P_MAPPING_TABLE_3m] entry_num %d, entry_size %d",
                    entry_num, entry_size);
            for (i = 0; i < entry_num; i++) {
                for (j = 0; j < entry_size; j++) {
                    egr_zone_3_dot1p_mapping_table_3_entry.entry_data[j] = 0x0;
                }
                /* i[5:2] */
                soc_mem_field32_set(unit, EGR_ZONE_3_DOT1P_MAPPING_TABLE_3m,
                                    egr_zone_3_dot1p_mapping_table_3_entry.entry_data,
                                    CFIf, (((i >> 2) & 0xf) % 2));
                soc_mem_field32_set(unit, EGR_ZONE_3_DOT1P_MAPPING_TABLE_3m,
                                    egr_zone_3_dot1p_mapping_table_3_entry.entry_data,
                                    PRIf, (((i >> 2) & 0xf) / 2));
                (void) soc_mem_write(unit, EGR_ZONE_3_DOT1P_MAPPING_TABLE_3m,
                                     COPYNO_ALL, i,
                                     egr_zone_3_dot1p_mapping_table_3_entry.entry_data);
            }
        }
        ...
        if (SOC_MEM_IS_VALID(unit, EGR_MAP_MHm)) {
            entry_num = soc_mem_index_max(unit, EGR_MAP_MHm);
            entry_size = sizeof(egr_map_mh_entry.entry_data) /
                         sizeof(uint32);
            cli_out("\n[EGR_MAP_MHm] entry_num %d, entry_size %d",
                    entry_num, entry_size);
            for (i = 0; i < entry_num; i++) {
                for (j = 0; j < entry_size; j++) {
                    egr_map_mh_entry.entry_data[j] = 0x0;
                }
                soc_mem_field32_set(unit, EGR_MAP_MHm,
...
#ifdef BCM_TRIDENT3_SUPPORT
    if (SOC_IS_TRIDENT3(unit)) {
        pktdma_set_cos_4_ipep_td3(unit);
    } else
#endif
    if (SOC_IS_TOMAHAWK(unit) || SOC_IS_TRIDENT2PLUS(unit)) {
...
/*
 * Function:
 *      pktdma_set_cos_4_cmic_td3
 * Purpose:
 *      Program COS_CTRL bitmaps. (Trident3)
 * Parameters:
 *      unit: StrataSwitch Unit #.
 *      rx_bitmap: bitmap of RX channels
 *
...
#ifdef BCM_TRIDENT3_SUPPORT
static void
pktdma_set_cos_4_cmic_td3(int unit, uint32 rx_bitmap)
{
    uint32 i, cmc = 0, ch = 0, vchan = 0, cmc_max, ch_max;
    uint32 cos_bmp0 = 0, cos_bmp1 = 0;
    int numels;
    soc_reg_t reg;
    ...
    if (SOC_IS_TRIDENT3(unit)) {
        /* MMU: Q_SCHED_CPU_L1_MC_QUEUE_L0_MAPPING
         *      -- desc: map 48 cos queues to 10 logical queue of L0 node
         *      -- rule:
         *            0 -  7 : 0 - 7
         *            8 - 15 : 8
         *           16 - 47 : 9
        ...
        /* CMIC: CMIC to MMU mapping
         *      -- desc: map 16 RX channels to 48 cos queues (48-bit cosmap)
         *      -- rule:
         *           0 - 15 :  0 - 15
         *                  : 16 - 47 (unused)
         */
...
pktdma_set_cos_4_cmic_th(int unit, uint32 rx_bitmap)
{
    ...
    cmc_max = soc_get_cmcs_per_device(unit);
    ch_max  = soc_get_chans_per_cmc(unit);
    vchan_max = soc_get_vchans_per_device(unit);
...
pktdma_set_cos_4_cmic(int unit, uint32 rx_bitmap)
{
    ...
    if (SOC_IS_TRIDENT3X(unit)) {
        pktdma_set_cos_4_cmic_td3(unit, rx_bitmap);
    } else
#endif
    if (SOC_IS_TOMAHAWK(unit) || SOC_IS_TRIDENT2PLUS(unit)) {
        pktdma_set_cos_4_cmic_th(unit, rx_bitmap);
    }
    /* print out cos mapping */
    {
        uint32 i, cos_bmp0 = 0, cos_bmp1 = 0;
        uint32 cmc = 0, ch = 0, vchan = 0, cmc_max, ch_max;
        uint32 rval = 0, fval = 0, numels = 0;
        soc_reg_t reg;

        cmc_max = soc_get_cmcs_per_device(unit);
        ch_max = soc_get_chans_per_cmc(unit);
        /* CMIC to MMU cos mapping */
        for (cmc = 0; cmc < cmc_max; cmc++) {
            for (ch = 0; ch < ch_max; ch++) {
                ...
                LOG_INFO(BSL_LS_APPL_TESTS, (BSL_META_U(unit,
                         ...
                         "cos_bmp 0x%08x_0x%08x"),
                         vchan, cos_bmp1, cos_bmp0));
            }
        }
        /* MMU logical queue to cos mapping */
        if (SOC_REG_IS_VALID(unit, Q_SCHED_CPU_L1_MC_QUEUE_L0_MAPPINGr)) {
            reg = Q_SCHED_CPU_L1_MC_QUEUE_L0_MAPPINGr;
            numels = SOC_REG_NUMELS(unit, reg);
            for (i=0; i<numels; i++) {
...
pktdma_clear_dma_ctrl_regs(int unit)
{
    ...
    if (SOC_IS_TRIDENT3(unit)) {
        {
            uint32 pkt_endian = 0, desc_endian = 0, header_endian = 0;
...
/*
 * Function:
 *      pktdma_turn_off_fc_td3
 * Purpose:
 *      Turn off flow control at the MAC, IDB and MMU. (Trident3)
 * Parameters:
 *      unit: StrataSwitch Unit #.
 *
 * Returns:
...
#ifdef BCM_TRIDENT3_SUPPORT
static void
pktdma_turn_off_fc_td3(int unit)
{
    int p, num_regs;
    ...
    if (SOC_IS_TRIDENT3(unit)) {
        ...
        num_regs = sizeof(idb_fcc_regs_td3) / sizeof(soc_reg_t);
        pktdma_turn_off_idb_fc_handle(unit, idb_fcc_regs_td3, num_regs);
    }
}
#endif
    ...
    for (cmc = 0; cmc < cmc_max; cmc++) {
        for (ch = 0; ch < ch_max; ch++) {
            ...
            if (tx_pkt_e[vchan] != 0 &&
                ...
                test_error(unit, "\n*WARN, TXDMA on vchan %0d stalled", vchan);
                result = BCM_E_FAIL;
            }
            if (rx_pkt_e[vchan] != 0 &&
                rx_pkt_e[vchan] == rx_pkt_s[vchan]) {
                test_error(unit, "\n*WARN, RXDMA on vchan %0d stalled", vchan);
                result = BCM_E_FAIL;
            }
        }
    }
    ...
    for (cmc = 0; cmc < cmc_max; cmc++) {
        for (ch = 0; ch < ch_max; ch++) {
            ...
            if ((((tx_bitmap >> vchan) & 0x1) != 0) &&
                ...
                result = BCM_E_FAIL;
                test_error(unit, "\n*ERROR, TXDMA enabled on CMC %0d, CH %0d, but"
                                 " no packets sent.", cmc, ch);
            }
            if ((((rx_bitmap >> vchan) & 0x1) != 0) &&
                (rate_calc->ch_rx_pkt[1][vchan] == 0)) {
                result = BCM_E_FAIL;
                test_error(unit, "\n*ERROR, RXDMA enabled on CMC %0d, CH %0d, but"
...
/*
 * Function:
 *      pktdma_set_cmicx_dma_loopback
 * Purpose:
 *      Set up dma loopback for cmicx.
 * Parameters:
 *      unit: StrataSwitch Unit #.
 *
 * Returns:
 ...
 */
static void
pktdma_set_cmicx_dma_loopback(int unit) {
#ifdef BCM_TRIDENT3_SUPPORT
    if (SOC_IS_TRIDENT3(unit)) {
...
/*
 * Function:
 *      pktdma_drain_rx_pkts
 * Purpose:
 *      Drain out packets in RX direction.
 * Parameters:
 *      unit: StrataSwitch Unit #.
 *
 * Returns:
 ...
 */
static void
pktdma_drain_rx_pkts(int unit)
{
    int timeout, drain_done;
    int pkt_counter[MAX_CHANNELS];
    uint32 vchan;
    uint32 tpkt = 0, rpkt = 0;
    pktdma_t *pktdma_p = pktdma_parray[unit];
    ...
    for (vchan = 0; vchan < MAX_CHANNELS; vchan++) {
        ...
        if (pktdma_p->rx_channel_en[vchan]) {
            soc_get_chan_pkt_counter(unit, vchan, &tpkt, &rpkt);
            pkt_counter[vchan] = rpkt;
        }
    }

    timeout = 10; /* 10 seconds */
    drain_done = 0;
    while (timeout > 0 && drain_done == 0) {
        drain_done = 1;
        /* sleep 1 second */
        sal_usleep(1000000);
        /* check rx pkt counter to determine whether all pkts are drained */
        for (vchan = 0; vchan < MAX_CHANNELS; vchan++) {
            if (pktdma_p->rx_channel_en[vchan] == 1) {
                soc_get_chan_pkt_counter(unit, vchan, &tpkt, &rpkt);
                if (rpkt != pkt_counter[vchan]) {
                    LOG_INFO(BSL_LS_APPL_TESTS, (BSL_META_U(unit,
                             "\ndraining packets ..., timeout = %0d,"
                             "vchan = %2d, pkt_prev %0d, pkt_curr %0d"),
                             timeout, vchan, pkt_counter[vchan], rpkt));
                    drain_done = 0;
                    pkt_counter[vchan] = rpkt;
                }
            }
    ...
    if (timeout == 0) {
        cli_out("\nWARN: Packets may not be drained out.");
    }
}
...
pktdma_stop_tx_rx_threads(int unit)
{
    ...
    if(pktdma_p->tx_thread_done == 0) {
        test_error(unit, "\n*WARN, TX thread still running");
        pktdma_p->test_fail = 1;
    }

    /* drain packets in buffer */
    pktdma_drain_rx_pkts(unit);

    /* kill RX thread */
    if(pktdma_p->rx_thread_done == 0) {
        test_error(unit, "\n*WARN, RX thread still running");
        pktdma_p->test_fail = 1;
    }
...
pktdma_soc_test_init(int unit, args_t *a, void **pa)
{
    ...
    /* set env: cmicx_loopback_param */
    if (pktdma_p->cmicx_loopback_param == 1) {
        pktdma_set_cmicx_dma_loopback(unit);
