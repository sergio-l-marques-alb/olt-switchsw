...
#else
#include <include/soc/dnx/swstate_legacy/sw_state_legacy_access.h>
#endif
...
#else 
#define RES_BMP_ACCESS sw_state_legacy_access[unit].shareddnx.res_tag_bmp_info
#endif
/*
    ...
    if (flags & SW_STATE_RES_TAG_BITMAP_ALLOC_WITH_ID) {
        ...
        if (_SHR_E_NONE == result) {
            ...
            if (_SHR_E_NONE != result) {
                ...
                index = offs;
                while (index < bmp_data.nextAlloc) {
                    result = RES_BMP_ACCESS.bitmap_lists.data.bit_get(unit, bmp_index, index, &temp);
                    SW_STATE_ACCESS_ERROR_CHECK(result);
