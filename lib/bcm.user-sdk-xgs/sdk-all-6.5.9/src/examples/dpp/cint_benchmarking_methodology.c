...
/* adding entry in database */
    ...
    for ( i = 0; i < reflector_port_num; i++ ) {
        ...
        if (!is_jer) {
            ...
            for (core_id = 0 ; core_id < cores_num ; core_id++) {
                /* Retrieve the old value of out-PP-port, using the signals from ERPP. In ETPP it will be changed already*/
                rv = dpp_dsig_read(unit, core_id, "ERPP", "Filter", "PMF", "Out_PP_Port", &signal_value, 1);
                old_outpp_port[core_id] = signal_value;
                BCM_GPORT_SYSTEM_PORT_ID_SET(old_outpp_gport[core_id], old_outpp_port[core_id]);
                core_config_arr[core_id].param0 = statId ;
                core_config_arr[core_id].param1 = old_outpp_gport[core_id] ;
                core_config_arr[core_id].param2 = BCM_ILLEGAL_ACTION_PARAMETER ;
            }
            statId_as_long = statId;
            printf("%s(): JERICHO: Call bcm_field_action_config_add(): bcmFieldActionStat\n",proc_name) ;
            printf("==> entry %d : statId = %d (0x%08lX)",entry,statId,statId_as_long) ;
            for (core_id = 0 ; core_id < cores_num ; core_id++) {
                printf(", old_outpp_port[core %d] = %d",core_id,old_outpp_port[core_id]) ;
            }
            printf("\n");
