...
int cint_jer_test_fc_src_vsq_trigger_set_example(int unit, int is_pfc, int target_port, int shaped_port, bcm_gport_t *vsq_gport)
{
    int rv = BCM_E_NONE;
    bcm_gport_t local_gport;
    bcm_cosq_pfc_config_t fc_threshold;
    ...
    if (*vsq_gport == 0) {
        ...
        rv = bcm_cosq_src_vsqs_gport_add(unit, target_port, &config, &src_port_vsq_gport, &pg_base_vsq_gport);
    ...
    if (rv != BCM_E_NONE) {
        printf("Failed(%d) to set VSQ fc threshold in cint_jer_test_fc_src_vsq_trigger_set_example.\n", rv);
        return rv;
    }
...
int cint_jer_test_fc_src_vsq_trigger_unset_example(int unit, int target_port, int shaped_port, bcm_gport_t vsq_gport)
{
    int rv = BCM_E_NONE;
    bcm_gport_t local_gport;
    bcm_cosq_pfc_config_t fc_threshold;
    ...
    rv = bcm_cosq_pfc_config_set(unit, vsq_gport, 0 /* cosq is not relevant */, 0, &fc_threshold);
    if (rv != BCM_E_NONE) {
        printf("Failed(%d) to set VSQ threshold in cint_jer_test_fc_src_vsq_trigger_unset_example\n", rv);
        return rv;
    }
