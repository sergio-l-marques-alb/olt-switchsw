...
/*
 * $Id: cint_vsq_tc_examples.c,v 1.2 Broadcom SDK $
 *
...
int set_wred_settings(int unit, bcm_gport_t gport, int is_enable, int drop_precedence, int max_thresh, int min_thresh, int max_prob, int gain)
{
    ...
    printf("Set WRED Settings unit %d, vsq 0x%x, is_enable %d, drop index %d, max thresh %d, min thresh %d, max prob %d gain %d \n",
...
/* 
 *  WRED Example
 *  This example sets the port's shaper (to create over subscription)
 *  and enable WRED.
 *  
 *  The process: 
...
int run_wred_example(int unit,int destination_local_port, int base_queue)
{
    ...
    rv = set_ofp_bandwidth(unit,destination_local_port,bandwidth);
    if (rv != BCM_E_NONE) {
        printf("(%s) \n",bcm_errmsg(rv));
        return rv;
    }
    ...
    /* 2. Create VSQs gport with category 2 traffic class 0 */
