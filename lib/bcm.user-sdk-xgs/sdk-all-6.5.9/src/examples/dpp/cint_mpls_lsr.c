 ...
 */
int
mpls_second_lsr_tunnel_create(int *units_ids, int nof_units, int in_sysport, int out_sysport){
    bcm_error_t rv;
    int term_label = 2000;
    int flags, ingress_intf, encap_id, egress_intf;
    int unit, i;
...
int lsr_field_egress_counter_set( int unit,int outP)
{
   ...
   if (BCM_E_NONE != result) {
    ...
    if(!is_qax)
    {
        /*action config for stat:
         para 0 is stat id
         para 1 is system port
         para 2 is outlif*/
        bcm_field_action_core_config_t core_config_jericho[2]={{0,stat_id,sysport_gport,0},{0,stat_id,sysport_gport,0}};
