                ...
                /* in */ int counter_id)
{
    ...
    for(ii = 0; ii < nof_dqs; ii++) { 
        ...
        dq.qualifier = dq_base[ii];
        dq.offset = dq_offset[ii];
        dq.length = dq_length[ii];
    ...
    rv = bcm_field_qualify_Stage(unit,
                                 pid | BCM_FIELD_QUALIFY_PRESEL | presel_flags,
                                 bcmFieldStageEgress);
    if (rv != BCM_E_NONE) {
        printf("Error, bcm_field_qualify_Stage\n");
    }
    rv = bcm_field_qualify_ForwardingType(unit, pid | BCM_FIELD_QUALIFY_PRESEL | presel_flags, bcmFieldForwardingTypeAny);
    if (rv != BCM_E_NONE) {
    ...
    case 0:
        ...
        for (ii =0; ii < qsize; ii++) {
            qdata[ii] = (sip >> ((qsize - ii - 1) * 8)) % 256;
            qmask[ii] = (ip_mask >> ((qsize - ii - 1) * 8)) % 256;
        }
    ...
    case 1:
        ...
        sal_memset(&qmask, 0, sizeof(qmask));
        for (ii =0; ii < qsize; ii++) {
            qdata[ii] = (dip >> ((qsize - ii) * 8)) % 256;
            qmask[ii] = (ip_mask >> ((qsize - ii) * 8)) % 256;
        }
                 ...
                 /* in */ bcm_field_group_t group) 
{
    ...
    for(ii = 0; ii< nof_dqs; ii++) {
        rv = bcm_field_data_qualifier_destroy(unit, dq_id[ii]);
        if (rv != BCM_E_NONE) {
            printf("%s():Error in function bcm_field_group_flush(), returned %d (%s)\n", proc_name, rv, bcm_errmsg(rv));
            return rv;
        }
