...
void erspan_set_eei_invalid(int unit, bcm_gport_t mirror_id)
{
    ...
    if (BCM_GPORT_IS_MIRROR_MIRROR(mirror_id)) {
        ...
        if (is_qax) {
            ...
            /* Add UDH length to forwarding header offset */
            for (i = 0; i < 4; i++) {
                user_header_size += soc_property_port_get((unit), i, spn_FIELD_CLASS_ID_SIZE, 0);
            }
            val[1] = ((user_header_size / 8) << 9);
            sprintf(cmd, "mod IPT_ARAD_SNP_MIR_STAMP_VALUES %d 1 PPH_BASE=0x%x%08x", offset, val[1], val[0]);
            bshell(unit,cmd);
        }
    } else {
