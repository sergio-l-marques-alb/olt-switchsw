...
int dc_enable_raw_data(int unit, int out_port) {
    ...
    uint32 hit1val = 0x1234;
    uint32 hit2val = 0x4321;
    uint32 zero = 0x0;
    ...
    COMPILER_64_SET(hit_value, zero, hit1val);
    ...
    rv = bcm_field_action_add(unit, ent, bcmFieldActionRedirect, 0, out_port);
    if(rv) {
    ...
    presel_id = 1; /* Advanced mode allows same presel ID in different stages */
    if(soc_property_get(unit, spn_FIELD_PRESEL_MGMT_ADVANCED_MODE, FALSE)) { 
        presel_flags = BCM_FIELD_QUALIFY_PRESEL_ADVANCED_MODE_STAGE_INGRESS; 
        rv = bcm_field_presel_create_stage_id(unit, bcmFieldStageExternal, presel_id); 
        if (BCM_E_NONE != rv) { 
    ...
    COMPILER_64_ZERO(hit_value);
    COMPILER_64_ZERO(hit_mask);
    COMPILER_64_SET(hit_value, zero, hit2val);
    COMPILER_64_SET(hit_mask, zero, hitmask);
