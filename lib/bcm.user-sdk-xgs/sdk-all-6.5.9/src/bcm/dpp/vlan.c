    ...
    if (is_port_flooding || (old_template != new_template)) {
        ...
        if (is_lif_flooding) {        
          ...
          if ((lif_entry_info->type != SOC_PPC_LIF_ENTRY_TYPE_AC) && (lif_entry_info->type != SOC_PPC_LIF_ENTRY_TYPE_FIRST_AC_IN_GROUP) &&
              ...
              BCMDNX_ERR_EXIT_MSG(BCM_E_INTERNAL, (_BSL_BCM_MSG("expected to be AC,PWE,Extender type")));
          }
    ...
    bcm_gport_t gport)
{
    ...
    */
    ...
    if (gport_hw_resources.fec_id != -1) {
        ...
        remove_fec = TRUE;
        if (BCM_GPORT_SUB_TYPE_IS_FORWARD_GROUP(BCM_GPORT_VLAN_PORT_ID_GET(gport))) {
    ...
    bcm_vlan_port_t *vlan_port)
{
    ...
    if (fec_id != -1 && (0 == (vlan_port->flags & BCM_VLAN_PORT_FORWARD_GROUP))) {
    ...
    bcm_vlan_port_translation_t *vlan_port_translation)
{
    ...
    rv = _bcm_dpp_gport_to_hw_resources(unit, vlan_port_translation->gport,
                                        _BCM_DPP_GPORT_HW_RESOURCES_LOCAL_LIF,
                                        &gport_hw_resources);
    BCMDNX_IF_ERR_EXIT(rv);
...
int _bcm_dpp_vlan_port_create(int unit, bcm_vlan_port_t *vlan_port)
{
    ...
    if ((SOC_IS_JERICHO(unit) && (soc_property_get(unit, spn_SPLIT_HORIZON_FORWARDING_GROUPS_MODE, 1)<2)) && 
        ...
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("vlan network group id must be 0 when soc property split_horizon_forwarding_groups_mode is not set to 2"))); 
    }

    if ((SOC_IS_JERICHO(unit) && (soc_property_get(unit, spn_SPLIT_HORIZON_FORWARDING_GROUPS_MODE, 1)==2)) && (vlan_port->flags & BCM_VLAN_PORT_NETWORK)) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("BCM_VLAN_PORT_NETWORK flag must be unset when soc property split_horizon_forwarding_groups_mode is set to 2"))); 
    }
           ...
           ((encap_with_id) && BCM_ENCAP_ID_GET(vlan_port->encap_id) != BCM_ENCAP_ID_GET(vlan_port_o.encap_id))) {
            ...
            if (SOC_IS_JERICHO(unit)) {
                ...
                if (SOC_DPP_CONFIG(unit)->pp.pon_dscp_remarking && is_tunnel_matched && !is_3_tags_manipulated && !_BCM_PPD_PON_PP_PORT_MAPPING_BY_PASS(unit)) {
                    local_out_lif_info_p->local_lif_flags |= BCM_DPP_AM_OUT_LIF_FLAG_EVEN_HALF;
                }
