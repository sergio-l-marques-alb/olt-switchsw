    ...
    bcm_l3_intf_t *intf)
{
...
#ifdef BCM_ARAD_SUPPORT
    SOC_PPC_EG_ENCAP_VSI_ENCAP_INFO out_vsi_info;
#endif
    ...
    if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE,
        ...
        if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
            /* In this mode (custom_feature_mtu_advanced_mode) MTU profile
             * for VSIs 1-3 must be equal to the VSI value, and is destined
             * to WA a hardware limitation.
             */
            if ((vsi >= 1) && (vsi <= 3) &&
  ...
  if(enable && !(intf->l3a_flags & BCM_L3_TRILL_ONLY)) {
      int outrif_profile_index = 0;
      SOC_PPC_EG_ENCAP_VSI_ENCAP_INFO_clear(&out_vsi_info);

      out_vsi_info.outrif_profile_index = outrif_profile_index;
  ...
  )
{
  ...
  if(info->l3a_flags2 &  BCM_L3_FLAGS2_SCALE_ROUTE){
      if (SOC_IS_JERICHO(unit)) {
          ...
          if(ARAD_KBP_ENABLE_IPV4_DC && ARAD_KBP_IPV4DC_24BIT_FWD && !(info->l3a_flags & BCM_L3_HOST_LOCAL)) {
	          BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("bcm_l3_host_add is not supported for IPv4 DC 24bit FWD in KBP; Use bcm_l3_route_add instead")));
          }
#endif
...
#if defined(INCLUDE_KBP) && !defined(BCM_88030)
          if(ARAD_KBP_ENABLE_IPV4_DC && ARAD_KBP_IPV4DC_24BIT_FWD && !(info->l3a_flags & BCM_L3_HOST_LOCAL)) {
	          BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("bcm_l3_host_find is not supported for IPv4 DC 24bit FWD in KBP; Use bcm_l3_route_get instead")));
          }
#endif
          host_key.route_scale = 1;
      }else{
                         ...
                         uint8 *is_found)
{
    ...
    if (is_found_tmp) {
        ...
        /* dealloc the out-rif profile if necessary */
        if (SOC_IS_QAX(unit)) {
            int is_last;
            int outrif_profile_index;
            QAX_PP_EG_ENCAP_ACCESS_OUT_RIF_PROFILE_ENTRY_FORMAT outrif_profile;

            sal_memset(&outrif_profile, 0,
                sizeof(QAX_PP_EG_ENCAP_ACCESS_OUT_RIF_PROFILE_ENTRY_FORMAT));

            status  = _bcm_dpp_am_template_out_rif_profile_index_get(unit,
                                        &outrif_profile, &outrif_profile_index);
            if (status == BCM_E_NONE) {
                BCMDNX_IF_ERR_EXIT(
                        _bcm_dpp_am_template_out_rif_profile_dealloc(unit,
                                            outrif_profile_index, &is_last));
                /* No need to delete profile since this rif profile is 0 */
            } else if (status == BCM_E_NOT_FOUND) {
                status = BCM_E_NONE;
            } else {
                BCMDNX_ERR_EXIT_VERB_MSG(status,
                            (_BSL_BCM_MSG("Out-RIF profile index get failed")));
            }
        }
    }
    ...
    bcm_l3_host_t *info)
{
    ...
    if (info->l3a_flags & BCM_L3_IP6) {
        ...
        if(info->l3a_flags2 &  BCM_L3_FLAGS2_SCALE_ROUTE){
            if (SOC_IS_JERICHO(unit)) {
#if defined(INCLUDE_KBP) && !defined(BCM_88030)
                if(ARAD_KBP_ENABLE_IPV4_DC && ARAD_KBP_IPV4DC_24BIT_FWD) {
    	            BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("bcm_l3_host_delete is not supported for IPv4 DC 24bit FWD; Use bcm_l3_route_delete instead")));
                }
#endif
            }
        }
