                                        ...
                                        int *q_count_increased)
{
    ...
    if (SOC_IS_KATANA2(unit) && !SOC_IS_SABER2(unit)) {
        rv = _bcm_kt2_intf_ref_cnt_increment(unit, queue_id, if_count);
    } else
#endif
    {
                                           ...
                                           int *q_count_decreased)
{
...
#ifdef BCM_KATANA2_SUPPORT
        if (SOC_IS_KATANA2 (unit)){ 
            rv = _bcm_kt2_ipmc_egress_intf_delete_all(unit, ipmc_id, ipmc_ptr);
        } else 
#endif
        {
            rv = _bcm_kt_ipmc_egress_intf_delete_all(unit, ipmc_id, ipmc_ptr);
        }
        if (BCM_FAILURE(rv)){
            goto intf_add_done;
        }
    ...
    /*for the list  we can use the base queue id */
    while(q_num){
        queue_id = q_array[q_num];
...
#ifdef BCM_KATANA2_SUPPORT
    if (SOC_IS_KATANA2(unit) && !SOC_IS_SABER2(unit)) {
        rv = _bcm_kt2_intf_ref_cnt_delete_all(unit);
    } else
#endif
    {
        rv = _bcm_kt_intf_ref_cnt_delete_all(unit);
    }
intf_add_done:
    IPMC_REPL_UNLOCK(unit);
    if (q_repl_ptr != NULL) {
...
#ifdef BCM_KATANA2_SUPPORT
    if (SOC_IS_KATANA2(unit) && !SOC_IS_SABER2(unit)) {
        rv = _bcm_kt2_intf_ref_cnt_decrement(unit, queue_id, 1);
    } else
#endif
    {
