...
bcmi_td3_port_dev_info_init(int unit)
{
    ...
    for (pipe = 0; pipe < _TD3_PIPES_PER_DEV; pipe++) {
        ...
        dev_info->pipe_phy_port_base[pipe] = TRIDENT3_PHY_PORTS_PER_PIPE * pipe +
                                             (pipe ? 1 : 0);
...
{
    phb_mapping_tbl_1_entry_t pri_map[16];
    ing_untagged_phb_entry_t phb;
    int index, pkt_pri, cfi, base = 0;
    void *entries[2];

    sal_memset(pri_map, 0, sizeof(phb_mapping_tbl_1_entry_t) * 16);
...
_bcm_td3_port_cfg_init(int unit, bcm_port_t port, bcm_vlan_data_t *vlan_data)
{
...
#endif
    {
        ...
        if (soc_feature(unit, soc_feature_egr_vxlate_supports_dgpp)) {
            soc_mem_set_field_value_array(egr_port_field_ids, VT_SYS_PORT_OVERRIDEf,
                                          egt_port_field_values, 1,
                                          egr_port_field_count);
        }
...
#define _GET_FIELD_VALUE(_f, _v)                \
    ...
    _GET_FIELD_VALUE(TRUST_DSCP_V6f, port_cfg->pc_dse_mode);
                             ...
                             uint32 burst, uint32 mode)
{
    ...
    mem = SOC_TD3_PMEM(unit, port, MMU_MTRO_EGRMETERINGCONFIG_MEM_Am,
