    ...
    switch(type) {
        case _TCB_SCOPE_UCQ:
            ...
            local_mmu_port = (entity_num & _TD3_TCB_UCQ_SCOPE_QUEUE_MASK) /
                             _BCM_TD3_NUM_UCAST_QUEUE_PER_PORT;
            mmu_port = pipe * SOC_TD3_MMU_PORT_STRIDE + local_mmu_port;
        ...
        case _TCB_SCOPE_ING_PORT:
            ...
            entity_num = soc_reg64_field32_get(unit, TCB_CONFIGr, rval64,
                                               SCOPE_ENTITYf);
            pipe = entity_num >> _TD3_TCB_PORT_SCOPE_PIPE_OFFSET;
            mmu_port = pipe * SOC_TD3_MMU_PORT_STRIDE +
                       (entity_num & _TD3_TCB_PORT_SCOPE_PORT_MASK);
            phy_port = si->port_m2p_mapping[mmu_port];
            local_port = si->port_p2l_mapping[phy_port];
                            ...
                            bcm_cosq_tcb_config_t *config)
{
    ...
    switch(config->scope_type) {
        ...
        case bcmCosqTcbScopeIngressPort:
            ...
            soc_reg64_field32_set(unit, TCB_CONFIGr, &rval64, SCOPE_ENTITY_TYPEf,
                              _TCB_SCOPE_ING_PORT);
            arg = (pipe << _TD3_TCB_PORT_SCOPE_PIPE_OFFSET) + mmu_port;
            break;
