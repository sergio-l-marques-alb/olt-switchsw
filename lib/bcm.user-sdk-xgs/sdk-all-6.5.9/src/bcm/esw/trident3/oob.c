    ...
    if (logical_queue_id == _TD3_OOB_STATS_END_QUEUE_REPORT) {
        *gport = _TD3_OOB_STATS_END_QUEUE_REPORT;
        ...
        if (logical_queue_id >= 1024) {
            q_offset = 1024;
            port_offset = SOC_TD3_MMU_PORT_STRIDE;
        } else {
            q_offset = 0;
            port_offset = 0;
        }
        cosq = (logical_queue_id - q_offset) % _TD3_NUM_UCAST_QUEUE_PER_PORT;
        mmu_port = (slice_id * SOC_TD3_MMU_PORT_STRIDE * _TD3_NUM_PIPE_PER_SLICE)
                    + ((logical_queue_id - q_offset) / _TD3_NUM_UCAST_QUEUE_PER_PORT) +
                    port_offset;
                        ...
                        int offset, bcm_gport_t gport)
{
    ...
    if (gport == _TD3_OOB_STATS_END_QUEUE_REPORT) {
        ...
        */
        ...
        slice_id = pipe / _TD3_NUM_PIPE_PER_SLICE;
    }

    BCM_IF_ERROR_RETURN
        (_bcm_td3_oob_stats_queue_mapping_set(unit, offset, slice_id,
                                             logical_queue_id));
