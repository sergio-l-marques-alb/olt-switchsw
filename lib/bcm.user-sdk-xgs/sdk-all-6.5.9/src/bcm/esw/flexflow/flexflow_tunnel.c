...
/*
 * Function:
 *      _bcm_flow_fragment_field_info_get
 * Purpose:
 *      Get the fragment field information. The fragment field is defined as 
 * that a user field is split into two parts in the memory, such as the IPv6 
 * field which splits into two in the first and second memory entry. Each 
 * doesn't end up in the 32bit, or 16bit boundary. In fact, it can split at 
 * any bit position. These fields are defined as internal logical field. 
 * They should always be in a pair. Example: FRAGMENT_DIP6_0_96, 
 * FRAGMENT_DIP6_97_127.
 * Format: FRAGMENT_<field_id>_<pos_min>_<pos_max>
 *
 * Parameters:
...
int _bcm_flow_fragment_field_info_get(int unit, int mem_view_id,char *id_str,
                                    _bcm_flow_frag_field_info_t *field_info)
{       
    ...
    int i;
    int res;
    char *fix_name = "FRAGMENT_";   

    sal_strcpy(name_buf,fix_name);
    sal_strcpy(name_buf+strlen(fix_name),id_str);
    ...
    for (ix = 0,j = 0; j < key_count; j++) {
        field_name = NULL;
        rv = soc_flow_db_mem_view_field_name_get(unit, mem_view_id,
                 key_list[j], &field_name);
        SOC_IF_ERROR_RETURN(rv);
        if (sal_strncmp(field_name, name_buf,strlen(name_buf)) == 0) {
            num_ptr = field_name + strlen(name_buf);
            /* field name must have format: <str_name>_nnn_nnn 
             * n: digit
             */
            if (ix > 1) {
                /* out of range */
            ...
            if (strlen(num_ptr) > sizeof(str_buf)) {
                return BCM_E_INTERNAL;
            }
            sal_strcpy(str_buf,num_ptr);
            num_ptr = sal_strtok_r(str_buf,"_",&next_str);
            if (num_ptr == NULL) {
                return BCM_E_INTERNAL;
            }
            /* atoi */
            for (res=0,i=0; num_ptr[i] != '\0'; ++i ) {
                res = res * 10 + num_ptr[i] - '0';
            }
            field_info[ix].pos_min = res;
            num_ptr = sal_strtok_r(NULL,"_",&next_str);
            if (num_ptr == NULL) {
                return BCM_E_INTERNAL;
            }
            /* atoi */
            ...
            field_info[ix].pos_max = res;
            field_info[ix].id  = key_list[j];
            ix++;
        }
    }
    if (ix != 2) {
        /* a pair of fragment field not found */
        return BCM_E_NOT_FOUND;
    }
    return BCM_E_NONE;                     
}

/* filter out internal logical fields */
int _bcm_flow_usr_logical_field_get (int unit, uint32 mem_view_id,
                                    int field_type,
                                    int size,
                                    uint32  *field_id,
                                    uint32  *field_count)
{
    uint32 tmp_fields[_BCM_FLOW_LOGICAL_FIELD_MAX];
    uint32 num_fields = 0;
    int i;
    int cnt;
    int rv;
    char *field_name = NULL;
    char *frag_str = "FRAGMENT";  /* internal logical name */

    rv = soc_flow_db_mem_view_field_list_get(unit,
                         mem_view_id,
                         field_type == SOC_FLOW_DB_FIELD_TYPE_LOGICAL_KEY?
                              SOC_FLOW_DB_FIELD_TYPE_LOGICAL_KEY:
                              SOC_FLOW_DB_FIELD_TYPE_LOGICAL_POLICY_DATA,
                         _BCM_FLOW_LOGICAL_FIELD_MAX, tmp_fields, &num_fields);
    SOC_IF_ERROR_RETURN(rv);
    for (cnt = 0, i = 0; i < num_fields; i++) {
        rv = soc_flow_db_mem_view_field_name_get(unit, mem_view_id,
                 tmp_fields[i], &field_name);
        SOC_IF_ERROR_RETURN(rv);
        
        if (sal_strncmp(field_name, frag_str,strlen(frag_str))) {
            /* not internal logical name */
            if (cnt < size) {
                field_id[cnt] = tmp_fields[i];
                cnt++;
            }
        }
    }
...
int _bcm_flow_frag_to_usr_field_convert(uint32 *usr_field, 
                     ...
                     uint32 *frag_field)
{
    ...
    bit_end    = pos_max%32;
    word_num = (pos_max/32 - pos_min/32) + 1;
    l_shifter = bit_start;
    ...
    for (last_idx = 0, i = 0; i < (word_num - 1); i++) {
...
int _bcm_flow_usr_to_frag_field_convert(uint32 *usr_field, int pos_min, 
...
void
_bcm_flow_ip6_addr_t_2_mem_field(ip6_addr_t ip6, uint32 *ip6_field)
{
...
void
_bcm_flow_mem_field_2_ip6_addr_t(ip6_addr_t ip6, uint32 *ip6_field)
{
...
STATIC int
_bcm_flow_ip6_addr_is_valid(bcm_ip6_t addr)
{
    uint32 idx = 0;
            ...
            uint32 *field_cnt)
{
    ...
    if (fixed_view == TRUE) {  /* fixed view */
        ...
        if (qos_f) {    
            ...
            info->dscp_sel = soc_format_field32_get(unit,QOS_A_ACTION_SETfmt,
                               &qos,QOS_ACTION_SELf);
            if (info->dscp_sel == bcmTunnelDscpMap) {
                /* Set DSCP_MAP value. */
                int hw_map_idx = 0;
    ...
    if (!_BCM_FLOW_IS_FLEX_VIEW(info)) {
        ...
        if (info->valid_elements & BCM_FLOW_TUNNEL_INIT_DSCP_VALID) {
            soc_format_field32_set(unit,QOS_A_ACTION_SETfmt,
                               &qos,QOS_ACTION_SELf, info->dscp_sel);
            if (info->dscp_sel == bcmTunnelDscpMap) {
                /* Set DSCP_MAP value. */
                int hw_map_idx = 0;
                rv = _bcm_tr2_qos_id2idx(unit, info->dscp_map, &hw_map_idx);
...
STATIC
int _bcm_esw_flow_entry_to_tnl_term(int unit, 
                                    uint32 *entry, 
...
STATIC
int _bcm_esw_flow_tnl_term_to_entry(int unit,
                                    soc_tunnel_term_t *tnl_term,
                                    uint32 *entry)
{
   uint32 num_hw_entries = 0;
    ...
    bcm_flow_tunnel_terminator_t *info)
{
    ...
    if (info->valid_elements & BCM_FLOW_TUNNEL_TERM_DIP6_VALID) {
        if (!_bcm_flow_ip6_addr_is_valid(info->dip6)) {
            LOG_ERROR(BSL_LS_BCM_FLOW, (BSL_META_U(unit,
            "Flow Tunnel Termination : \
    ...
    if (info->valid_elements & BCM_FLOW_TUNNEL_TERM_SIP6_VALID) {
        if (!_bcm_flow_ip6_addr_is_valid(info->sip6)) {
            LOG_ERROR(BSL_LS_BCM_FLOW, (BSL_META_U(unit,
            "Flow Tunnel Termination : \
    ...
    if (info->valid_elements & BCM_FLOW_TUNNEL_TERM_SIP6_MASK_VALID) {
        if (!_bcm_flow_ip6_addr_is_valid(info->sip6_mask)) {
            LOG_ERROR(BSL_LS_BCM_FLOW, (BSL_META_U(unit,
            "Flow Tunnel Termination : \
    ...
    if (info->valid_elements & BCM_FLOW_TUNNEL_TERM_DIP6_MASK_VALID) {
        if (!_bcm_flow_ip6_addr_is_valid(info->dip6_mask)) {
            LOG_ERROR(BSL_LS_BCM_FLOW, (BSL_META_U(unit,
            "Flow Tunnel Termination : \
    ...
    soc_flow_db_mem_view_info_t vinfo)
{
    ...
    if (vinfo.mem == L3_TUNNELm) {
        BCM_IF_ERROR_RETURN(
        _bcm_esw_flow_entry_to_tnl_term(unit, entry, &tnl_term));
    ...
    if (_BCM_FLOW_IS_FLEX_VIEW(info)) {
        ...
        if ((info->flow_handle == SOC_FLOW_DB_FLOW_ID_LEGCAY_VXLAN) &&
            (info->type == bcmTunnelTypeVxlan)) {
            if (info->multicast_flag == BCM_VXLAN_MULTICAST_TUNNEL_STATE_BUD_ENABLE) {
                soc_mem_field32_set(unit, mem, entry,
                VXLAN_DIP__NETWORK_RECEIVERS_PRESENTf, 0x01);
            } else if (info->multicast_flag == BCM_VXLAN_MULTICAST_TUNNEL_STATE_BUD_DISABLE) {
                soc_mem_field32_set(unit, mem, entry,
                VXLAN_DIP__NETWORK_RECEIVERS_PRESENTf, 0x00);
            }  
        } else if ((info->flow_handle == SOC_FLOW_DB_FLOW_ID_LEGACY_L2GRE) &&
            (info->type == bcmTunnelTypeL2Gre)) {
            if (info->multicast_flag == BCM_L2GRE_MULTICAST_TUNNEL_STATE_BUD_ENABLE) {
                soc_mem_field32_set(unit, mem, entry,
                L2GRE_DIP__NETWORK_RECEIVERS_PRESENTf, 0x01);
            } else if (info->multicast_flag == 
                       BCM_L2GRE_MULTICAST_TUNNEL_STATE_BUD_DISABLE) {
                soc_mem_field32_set(unit, mem, entry,
    ...
    soc_flow_db_mem_view_info_t vinfo) 
{
    ...
    if (vinfo.mem == L3_TUNNELm) {
        BCM_IF_ERROR_RETURN(
        _bcm_esw_flow_entry_to_tnl_term(unit, entry, &tnl_entry));
        BCM_IF_ERROR_RETURN(
           soc_tunnel_term_match(unit, &tnl_entry,
                              &ret_tnl_entry, &index));
       _bcm_esw_flow_tnl_term_to_entry(unit, &ret_tnl_entry, rentry);
    } else {
        BCM_IF_ERROR_RETURN(
    ...
    if (_BCM_FLOW_IS_FLEX_VIEW(info)) {
        ...
        if ((info->flow_handle == SOC_FLOW_DB_FLOW_ID_LEGCAY_VXLAN) &&
            ...
            if(soc_mem_field32_get(unit, mem, rentry,
                    VXLAN_DIP__NETWORK_RECEIVERS_PRESENTf) == 0x01) {
               info->multicast_flag |= BCM_VXLAN_MULTICAST_TUNNEL_STATE_BUD_ENABLE;
            } else if (soc_mem_field32_get(unit, mem, rentry,
                    VXLAN_DIP__NETWORK_RECEIVERS_PRESENTf) == 0x00) {
               info->multicast_flag |= BCM_VXLAN_MULTICAST_TUNNEL_STATE_BUD_DISABLE;
            }
            ...
            if(soc_mem_field32_get(unit, mem, rentry,
                    L2GRE_DIP__NETWORK_RECEIVERS_PRESENTf) == 0x01) {
               info->multicast_flag |= BCM_L2GRE_MULTICAST_TUNNEL_STATE_BUD_ENABLE;
            } else if (soc_mem_field32_get(unit, mem, rentry,
                    L2GRE_DIP__NETWORK_RECEIVERS_PRESENTf) == 0x00) {
               info->multicast_flag |= BCM_L2GRE_MULTICAST_TUNNEL_STATE_BUD_DISABLE;
            }
    ...
    if (_BCM_FLOW_IS_FLEX_VIEW(info)) {
        ...
        for (i = 0; i < key_count; i++) {
            if (key_list[i] == IPV4__DIPf) {
                ...
                if (info->valid_elements & BCM_FLOW_TUNNEL_TERM_SIP6_VALID) {
                    rv = (_bcm_flow_ip6_addr_is_valid(info->sip6)) ?
                ...
                if (info->valid_elements & BCM_FLOW_TUNNEL_TERM_DIP6_VALID) {
                    rv = (_bcm_flow_ip6_addr_is_valid(info->dip6)) ?
                          BCM_E_NONE :  BCM_E_PARAM;
                    BCM_IF_ERROR_RETURN(rv);
                    soc_mem_ip6_addr_set(unit, mem, entry, 
                ...
                if (info->valid_elements & BCM_FLOW_TUNNEL_TERM_SIP6_MASK_VALID) {
                    rv = (_bcm_flow_ip6_addr_is_valid(info->sip6_mask)) ?
                          BCM_E_NONE :  BCM_E_PARAM;
                    BCM_IF_ERROR_RETURN(rv);
                    soc_mem_ip6_addr_set(unit, mem, entry,
                ...
                if (info->valid_elements & BCM_FLOW_TUNNEL_TERM_DIP6_MASK_VALID) {
                    rv = (_bcm_flow_ip6_addr_is_valid(info->dip6_mask)) ?
                          BCM_E_NONE :  BCM_E_PARAM;
                    BCM_IF_ERROR_RETURN(rv);
                    soc_mem_ip6_addr_set(unit, mem, entry,
        ...
        if ((info->valid_elements & BCM_FLOW_TUNNEL_TERM_DIP6_VALID) &&
            (!SOC_MEM_FIELD_VALID(unit,mem_view_id,IPV6__DIPf)) ) {
            ...
            BCM_IF_ERROR_RETURN(_bcm_flow_fragment_field_info_get(unit, 
                          mem_view_id, "IPV6__DIP", f_info));
            ...
            _bcm_flow_ip6_addr_t_2_mem_field(info->dip6, ip6_value);
            BCM_IF_ERROR_RETURN(_bcm_flow_usr_to_frag_field_convert(ip6_value, 
                 f_info[0].pos_min, f_info[0].pos_max, ip6_field));
            soc_mem_field_set(unit, mem, entry, f_info[0].id, ip6_field);
            BCM_IF_ERROR_RETURN(_bcm_flow_usr_to_frag_field_convert(ip6_value, 
                 f_info[1].pos_min, f_info[1].pos_max, ip6_field));
            soc_mem_field_set(unit, mem, entry, f_info[1].id, ip6_field);
        }
                             ...
                             bcm_flow_logical_field_t *field)
{
    ...
    if (rv == BCM_E_NOT_FOUND) {
       ...
       info->flow_option = ffo[0].option_id;
        ...
        if (!(info->valid_elements & BCM_FLOW_TUNNEL_TERM_DIP6_VALID)) {
            /* check if fragment ipv6 address */
            _bcm_flow_frag_field_info_t f_info[2];
            ...
            rv = _bcm_flow_fragment_field_info_get(unit, mem_view_id, "IPV6__DIP", 
                               f_info);
            ...
            if (rv == SOC_E_NONE) {
                ...
                soc_mem_field_get(unit, mem_view_id, entry, f_info[0].id, ip6_frag);
                soc_mem_field_get(unit, mem_view_id, entry, f_info[1].id, ip6_frag1);
                ...
                _bcm_flow_mem_field_2_ip6_addr_t(info->dip6, ip6_field);
            }
        }
        
        BCM_IF_ERROR_RETURN(
                    _bcm_flow_usr_logical_field_get(unit,
                                mem_view_id,
                                SOC_FLOW_DB_FIELD_TYPE_LOGICAL_KEY,
                                SOC_FLOW_DB_MAX_VIEW_FIELDS,
                                key_list,
    ...
    for (chnk_idx = soc_mem_index_min(unit, mem);
        ...
        for (ent_idx = 0 ; ent_idx <= chnk_end; ) {
            ...
            if (mem == L3_TUNNELm) {
                ...
                for (i= 0; i < num_hw_entries; i++) {                    
                    sal_memcpy(trav_entry + entry_width * i , entry, entry_width);
                    ent_idx++;
                    entry = soc_mem_table_idx_to_pointer(unit, mem, uint32 *,
                                             vt_tbl_chnk, ent_idx);
                }
            } else {
                sal_memcpy(trav_entry, entry, sizeof(trav_entry));
                ent_idx++;
            }   
            bcm_flow_tunnel_terminator_t_init(&info);
             
            rv = _bcm_flow_tunnel_terminator_entry_parse(unit, mem, entry, &info,
                    &num_of_fields,field);
            if (rv == BCM_E_NOT_FOUND) { /* for entries not exposed to user */
                continue;
            } else if (BCM_FAILURE(rv)) {
