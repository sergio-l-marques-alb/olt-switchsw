...
#if defined(BCM_TRIDENT2_SUPPORT)
static sbusdma_desc_handle_t bst_tbl_handles[BCM_MAX_NUM_UNITS] = {0};
static _bcm_bst_tbl_ctrl_t **bst_tbl_ctrl[BCM_MAX_NUM_UNITS] = {NULL};
#endif
...
#if defined(BCM_TRIDENT2_SUPPORT)
STATIC int _bcm_td2_bst_sbusdma_desc_sync(int unit);
STATIC int _bcm_td2_bst_sbusdma_desc_init(int unit);
STATIC int _bcm_td2_bst_sbusdma_desc_deinit(int unit);
#endif
...
_bcm_bst_td2_init(int unit)
{
    ...
    if (SOC_IS_TRIDENT2(unit)) {
        bst_info->hw_sbusdma_desc_init = _bcm_td2_bst_sbusdma_desc_init;
        bst_info->hw_sbusdma_desc_deinit = _bcm_td2_bst_sbusdma_desc_deinit;
        bst_info->hw_sbusdma_desc_sync = _bcm_td2_bst_sbusdma_desc_sync;
    }
...
soc_bst_sbusdma_desc_free(int unit)
{
    ...
    for (bid = bcmBstStatIdDevice; bid < bcmBstStatIdHeadroomPool; bid++) {
...
soc_bst_sbusdma_desc_setup(int unit)
{
    ...
    bst_tbl_ctrl[unit] = sal_alloc(bcmBstStatIdHeadroomPool *
...
#define BST_TBLS "BST TBLS"
    ...
    for (bid = bcmBstStatIdDevice; bid < bcmBstStatIdHeadroomPool; bid++) {
        ...
        for (pipe = 0; pipe < pipe_factor; pipe++) {
            ...
            reg = resInfo->stat_reg[pipe];
            if (mem != INVALIDm) {
                cfg[mi].flags = SOC_SBUSDMA_CFG_COUNTER_IS_MEM | SOC_SBUSDMA_CFG_USE_FLAGS;
                cfg[mi].blk = SOC_MEM_BLOCK_ANY(unit, mem);
                cfg[mi].addr = soc_mem_addr_get(unit, mem,
                                               0, cfg[mi].blk, 0, &acc_type);
                cfg[mi].acc_type = acc_type;
                cfg[mi].blk = SOC_BLOCK2SCH(unit, cfg[mi].blk);
                cfg[mi].width = soc_mem_entry_words(unit, mem);
                cfg[mi].count = resInfo->num_stat_pp / pipe_factor;
                cfg[mi].addr_shift = 0;
                size += cfg[mi].count * cfg[mi].width * 4;
                tbl_ctrl->blk = cfg[mi].blk;
                mi++;
            }else if (reg != INVALIDr) {
                regp = SOC_REG_PTR(unit, reg);
                cfg[mi].addr = soc_reg_addr_get(unit, reg, REG_PORT_ANY, 0,
                    SOC_REG_ADDR_OPTION_NONE, &blk,
                    &acc_type);
                cfg[mi].flags = 0;
                cfg[mi].acc_type = acc_type;
                cfg[mi].blk = blk;
                cfg[mi].width = BYTES2WORDS(soc_reg_bytes(unit, reg)) * regp->nFields;
                cfg[mi].count = resInfo->num_stat_pp / pipe_factor;
                cfg[mi].addr_shift = 8;
                size += cfg[mi].count * cfg[mi].width * 4;
                tbl_ctrl->blk = cfg[mi].blk;
...
STATIC int
_bcm_td2_bst_sbusdma_desc_init(int unit)
{
    _bcm_bst_cmn_unit_info_t *bst_info;
    _bcm_bst_resource_info_t *resInfo;
...
STATIC int
_bcm_bst_sw_sbusdma_desc_sync(int unit, int bid)
{
    _bcm_bst_cmn_unit_info_t *bst_info;
    _bcm_bst_resource_info_t *resInfo;
    _bcm_bst_tbl_ctrl_t *tbl_ctrl;
...
STATIC int
_bcm_td2_bst_sbusdma_desc_sync(int unit)
{
    _bcm_bst_cmn_unit_info_t *bst_info;
    bcm_bst_stat_id_t bid;
    int enable = 0;
    ...
    for (bid = bcmBstStatIdDevice; bid < bcmBstStatIdHeadroomPool; bid++) {
        BCM_IF_ERROR_RETURN(_bcm_bst_sw_sbusdma_desc_sync(unit, bid));
    }
...
STATIC int
_bcm_td2_bst_sbusdma_desc_deinit(int unit)
{
    _bcm_bst_cmn_unit_info_t *bst_info;
    int rv = BCM_E_INIT;
