...
STATIC const soc_field_t e2efc_rmod_enable[GH2_E2EFC_RMOD_NUM] = {
        V0f, V1f, V2f, V3f};
STATIC const soc_reg_t
e2efc_ibp_hg_rmodf[GH2_E2EFC_RMOD_NUM] = {
        TX_HG_RMOD0f, TX_HG_RMOD1f,
        TX_HG_RMOD2f, TX_HG_RMOD3f};
    ...
    bcm_port_e2efc_remote_module_config_t *e2efc_rmod_cfg)
{
    ...
    if (mode == bcmPortE2efcModeTx && (e2efc_rmod_cfg->local_port < 0 ||
        e2efc_rmod_cfg->local_port >= NUM_ALL_PORT(unit))) {
        LOG_ERROR(BSL_LS_BCM_PORT,
                  (BSL_META_U(unit,
        ...
        for (i = 0; i < GH2_E2EFC_RMOD_NUM; i++) {
            ...
            if (!(soc_reg_field_get(unit, e2efc_rmod_id_r[mode][i], val,
                ...
                if (mode == bcmPortE2efcModeTx) {
                    soc_reg_t reg = E2EFC_IBP_HG_RMODr;
                    BCM_IF_ERROR_RETURN(soc_reg32_get(unit, reg,
                                                      REG_PORT_ANY, 0, &val));
                    soc_reg_field_set(unit, reg, &val, e2efc_ibp_hg_rmodf[i],
                                      e2efc_rmod_cfg->local_port);
                    BCM_IF_ERROR_RETURN(soc_reg32_set(unit, reg, REG_PORT_ANY,
                                                      0, val));
                }
                modid = i;
                break;
    ...
    int *rport_handle_id)
{
    ...
    if (e2efc_rport_cfg->remote_port < 0 ||
        e2efc_rport_cfg->remote_port >= NUM_ALL_PORT(unit)) {
        LOG_ERROR(BSL_LS_BCM_PORT,
                  (BSL_META_U(unit,
                  "Invalid Port Range. Should be >= 0 && < %d\n"),
                  NUM_ALL_PORT(unit)));
