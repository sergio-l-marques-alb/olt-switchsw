...
{
    return ((id == MIRROR_EDIT_CTRL_ID_PSAMP)?1:0);
}
...
/* Define mirror encap profile index */
#define ENCAP_PROF_PSAMP_LOCAL_MTP                                       1
#define ENCAP_PROF_PSAMP_REMOTE_MTP                                      2
#define ENCAP_PROF_RSPAN_INGRESS_MIRROR_LOCAL_MTP                        3
#define ENCAP_PROF_RSPAN_EGRESS_MIRROR_LOCAL_MTP                         4
#define ENCAP_PROF_SFLOW_ETAG_LOCAL_MTP                                  5
#define ENCAP_PROF_SFLOW_ETAG_OTAG_LOCAL_MTP                             5
#define ENCAP_PROF_SFLOW_INGRESS_MIRROR_LOCAL_MTP                        5
#define ENCAP_PROF_PSAMP_UNTAG_LOCAL_MTP                                 5
#define ENCAP_PROF_SFLOW_VNTAG_LOCAL_MTP                                 5
#define ENCAP_PROF_SFLOW_VNTAG_OTAG_LOCAL_MTP                            5
#define ENCAP_PROF_SFLOW_VNTAG_OTAG_REMOTE_MTP                           6
#define ENCAP_PROF_SFLOW_VNTAG_REMOTE_MTP                                6
#define ENCAP_PROF_PSAMP_UNTAG_REMOTE_MTP                                6
#define ENCAP_PROF_SFLOW_ETAG_OTAG_REMOTE_MTP                            6
#define ENCAP_PROF_SFLOW_ETAG_REMOTE_MTP                                 6
#define ENCAP_PROF_SFLOW_INGRESS_MIRROR_REMOTE_MTP                       6
#define ENCAP_PROF_SFLOW_UNTAG_INGRESS_MIRROR_LOCAL_MTP                  7
#define ENCAP_PROF_SFLOW_UNTAG_INGRESS_MIRROR_REMOTE_MTP                 8
#define ENCAP_PROF_ERSPAN_EGRESS_MIRROR_LOCAL_MTP                        9
#define ENCAP_PROF_ERSPAN_EGRESS_MIRROR_UNTAG_PAYLOAD_LOCAL_MTP          9
#define ENCAP_PROF_ERSPAN_EGRESS_MIRROR_REMOTE_MTP                      10
#define ENCAP_PROF_ERSPAN_EGRESS_MIRROR_UNTAG_PAYLOAD_REMOTE_MTP        10
#define ENCAP_PROF_ERSPAN_INGRESS_MIRROR_LOCAL_MTP                      11
#define ENCAP_PROF_ERSPAN_INGRESS_MIRROR_UNTAG_PAYLOAD_LOCAL_MTP        11
#define ENCAP_PROF_ERSPAN_UNTAG_INGRESS_MIRROR_LOCAL_MTP                11
#define ENCAP_PROF_ERSPAN_UNTAG_INGRESS_MIRROR_UNTAG_PAYLOAD_LOCAL_MTP  11
#define ENCAP_PROF_ERSPAN_UNTAG_INGRESS_MIRROR_REMOTE_MTP               12
#define ENCAP_PROF_ERSPAN_UNTAG_INGRESS_MIRROR_UNTAG_PAYLOAD_REMOTE_MTP 12
#define ENCAP_PROF_ERSPAN_INGRESS_MIRROR_REMOTE_MTP                     12
#define ENCAP_PROF_ERSPAN_INGRESS_MIRROR_UNTAG_PAYLOAD_REMOTE_MTP       12
#define ENCAP_PROF_ERSPAN_UNTAG_EGRESS_MIRROR_LOCAL_MTP                 13
#define ENCAP_PROF_ERSPAN_UNTAG_EGRESS_MIRROR_UNTAG_PAYLOAD_LOCAL_MTP   13
#define ENCAP_PROF_ERSPAN_UNTAG_EGRESS_MIRROR_REMOTE_MTP                14
#define ENCAP_PROF_ERSPAN_UNTAG_EGRESS_MIRROR_UNTAG_PAYLOAD_REMOTE_MTP  14

STATIC int 
_bcm_td3_mirror_encap_profile_index_get(int unit, int edit_ctrl_id, int local)
{
                    ...
                    bcm_mirror_destination_t  *mirror_dest)
{
    ...
    uint8 *p_profile_1_hdr, *p_profile_2_hdr, *p_profile_3_hdr;

    p_profile_1_hdr = (uint8*) &profile_1;
    p_profile_2_hdr = (uint8*) &profile_2;
    ...
    if (_is_edit_ctrl_id_sflow(edit_ctrl_id) ||
        ...
        len_offset += 12;  /* size of mac header */
        ...
        if (edit_ctrl_id == MIRROR_EDIT_CTRL_ID_SFLOW_INGRESS_MIRROR) {
            ...
            *(uint16*)p_profile_header = soc_ntohs(mirror_dest->vlan_id);
            p_profile_header+=sizeof(uint16);

            len_offset += 4;  /* Vlan Tag length */
        }

        /* for VNTAG  */ 
        if (edit_ctrl_id == MIRROR_EDIT_CTRL_ID_SFLOW_VNTAG) {
            ...
            *(uint16*)p_profile_header = soc_ntohs(mirror_dest->niv_dst_vif);
            if (mirror_dest->niv_flags & BCM_MIRROR_NIV_LOOP) {
                *(uint16*)p_profile_header |= _BCM_TD_MIRROR_NIV_LOOP_BIT;
            }
            p_profile_header+=sizeof(uint16);
            ...
            *(uint16*)p_profile_header = soc_ntohs(mirror_dest->niv_src_vif);
            ...
            len_offset += 6;  /* Ether type and VNTAG length */
        }

        /* Add Ether Type */
        *(uint16*)p_profile_header = soc_ntohs(0x0800);
        p_profile_header+=sizeof(uint16);
        len_offset += 2;  /* ETHER TYPE */

        /* Construct IPV4 Information */ 
        *p_profile_header = mirror_dest->version? mirror_dest->version : 0x45;
        p_profile_header+=1;
        *p_profile_header = mirror_dest->tos;   
        p_profile_header+=7;

        len_offset += 2;  /* Version/TOS */ 
        ...
        *(uint16*)p_profile_header = soc_ntohs(mirror_dest->udp_dst_port);
        ...
        } else if (edit_ctrl_id == MIRROR_EDIT_CTRL_ID_PSAMP) {
        ...
        *p_profile_header = (mirror_dest->version & 0xF) << 4;
        p_profile_header+=1;
    ...
    {
        int mcnt;
        for (mcnt = 0; mcnt < EGR_MIRROR_FLEX_EDITOR_WIDTH; mcnt++) {
            p_profile_1_hdr[mcnt] = 
                 p_profile_hdr[EGR_MIRROR_FLEX_EDITOR_WIDTH-1-mcnt]; 
            p_profile_2_hdr[mcnt] = 
                 p_profile_hdr[(2*EGR_MIRROR_FLEX_EDITOR_WIDTH)-1-mcnt];
         }
     }

    lcl_port = mirror_dest->gport & 0x7FF;
    encap_profile_index = _bcm_td3_mirror_encap_profile_index_get(unit, 
                          edit_ctrl_id, 
                                        ...
                                        void **entries)
{
    ...
    if (mtp_cfg->egress) {
        edit_ctrl_id = (mirror_dest->flags & BCM_MIRROR_DEST_PAYLOAD_UNTAGGED)?
                     MIRROR_EDIT_CTRL_ID_ERSPAN_EGRESS_MIRROR_UNTAG_PAYLOAD :
                     MIRROR_EDIT_CTRL_ID_ERSPAN_EGRESS_MIRROR;
    } else {
        edit_ctrl_id = (mirror_dest->flags & BCM_MIRROR_DEST_PAYLOAD_UNTAGGED)?
                     MIRROR_EDIT_CTRL_ID_ERSPAN_INGRESS_MIRROR_UNTAG_PAYLOAD :
                     MIRROR_EDIT_CTRL_ID_ERSPAN_INGRESS_MIRROR;
    }
                                   ...
                                   void **entries)
{
    ...
    soc_mem_field32_set (unit, EGR_MIRROR_TABLEm, mirror_table_p, 
                         IPFIX_VERSIONf, 0xa);
    soc_mem_field32_set (unit, EGR_MIRROR_TABLEm, mirror_table_p, 
                         IPFIX_OBSERVATION_DOMAINf, 100);
    soc_mem_field32_set (unit, EGR_MIRROR_TABLEm, mirror_table_p, 
                         PSAMP_TEMPLATE_IDf, 0x2222);
    soc_mem_field32_set (unit, EGR_MIRROR_TABLEm, mirror_table_p, 
                         PSAMP_PADf, 0x00FF);
                ...
                
{
    ...
    p_profile_header = p_profile_hdr;
    sal_memcpy(p_profile_header, p_profile_1_hdr, 
               EGR_MIRROR_FLEX_EDITOR_WIDTH);

    sal_memcpy(p_profile_header+EGR_MIRROR_FLEX_EDITOR_WIDTH, 
               p_profile_2_hdr, EGR_MIRROR_FLEX_EDITOR_WIDTH);
    ...
    if (_is_edit_ctrl_id_sflow(edit_ctrl_id) ||
        ...
        if (edit_ctrl_id == MIRROR_EDIT_CTRL_ID_SFLOW_VNTAG) {
            ...
            mirror_dest->niv_flags = (mirror_dest->niv_dst_vif & 0x8000) ? 
        ...
        *****************************************/
        ...
        mirror_dest->version  = *p_profile_header >> 4;
        p_profile_header+=1;
        ...
        /* RSPAN recovery */
        mirror_dest->tpid    = *(uint16*)p_profile_header;
        p_profile_header+=sizeof(uint16);
        mirror_dest->vlan_id = *(uint16*)p_profile_header;
        ...
        /* ETAG recovery */
        mirror_dest->etag_src_vid    = *(uint16*)p_profile_header;
        p_profile_header+=sizeof(uint16);
        mirror_dest->etag_dst_vid = *(uint16*)p_profile_header;
...
_bcm_td3_mirror_tunnel_reload(int unit, bcm_gport_t dest_id,
                             uint32 profile_index)
{
    int     edit_ctrl_id, lcl_port, encap_profile_index = 0;
    void   *entries[EGR_MIRROR_TABLE_ENTRIES_NUM];
...
void _bcm_td3_mirror_flags_update (int unit, int edit_ctrl_id, uint32 *flags)
{
    if (_is_edit_ctrl_id_sflow(edit_ctrl_id)) {
    ...
    if (edit_ctrl_id == MIRROR_EDIT_CTRL_ID_PSAMP) {
    ...
    if (soc_feature(unit, soc_feature_mirror_encap_profile)) {
        ...
        if (soc_mem_field32_get(unit, mtp_index_mem,
            ...
            if (SOC_IS_TRIDENT3X(unit)) {
                ...
                _bcm_td3_mirror_flags_update(unit, edit_ctrl_id, flags);
            } else
#endif /* BCM_TRIDENT3_SUPPORT */
            {
