...
bcmi_xgs5_port_attach_dscp(int unit, bcm_port_t port)
{
    BCM_IF_ERROR_RETURN
        (_bcm_esw_port_tab_set(unit, port, _BCM_CPU_TABS_ETHER,
                               TRUST_DSCP_PTRf,
                               0));
    SOC_IF_ERROR_RETURN(_bcm_dscp_table_entry_reference(unit, 0,
                                                        64));
                                      ...
                                      bcm_port_resource_t *resource)
{
    ...
    for (i = 0; i < nport; i++) {
        if (resource[i].physical_port == -1) {
           BCM_PBMP_PORT_ADD(detached_pbmp, resource[i].port);
        } else {
           BCM_PBMP_PORT_ADD(attached_pbmp, resource[i].port);
        }
    }
