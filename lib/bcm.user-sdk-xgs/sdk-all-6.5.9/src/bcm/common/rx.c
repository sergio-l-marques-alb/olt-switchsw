...
/* } */
    ...
    if(soc_feature(unit, soc_feature_cmicx)) {
        ...
        for (ix = 0; ix < chan_id_max; ix++) {
            ...
            bit = 1 << (queue_id % 32);
            reg_val = cmicx_pktdma_cos_ctrl_queue_id_get(unit, cmc,
...
cmicx_rx_default_parser(int unit,  bcm_pkt_t *pkt)
{
    ...
    sal_memcpy(hdr, pkt->_pkt_data.data, hdr_size);

    if (bsl_check(bslLayerSoc, bslSourceRx, bslSeverityVerbose, unit)) {
        soc_dma_ep_to_cpu_hdr_dump(unit, "EP_2_CPU ", pkt->_pkt_data.data, hdr_size, 0);
    }
