...
)
{
    ...
    if (dma_buf_mutexes[unit]==0 && soc_mem_slamable(unit, 0, 0)) { /* check if DMA is enabled */
        SOCDNX_IF_ERR_EXIT(dcmn_alloc_dma_mem(unit, TRUE, dma_buffers + unit, DCMN_MAX_U32S_IN_MEM_ENTRY * sizeof(uint32), "fill_table"));
  ...
  )
{
   ...
   if (entry_data == NULL) {
    ...
    } else if ((buffer != NULL) && (soc_mem_entry_words(unit, mem) < DCMN_MAX_U32S_IN_MEM_ENTRY)) {
        SOCDNX_IF_ERR_EXIT(sal_mutex_take(dma_buf_mutexes[unit], sal_mutex_FOREVER));
        should_release = 1;
  ...
  )
{
    ...
    if (entry_data == NULL) {
        ...
        if (soc_mem_entry_words(unit, mem) > DCMN_MAX_U32S_IN_MEM_ENTRY) {
