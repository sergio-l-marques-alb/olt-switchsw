...
soc_arad_str_prop_nif_elk_tcam_connect_mode_get(int unit, ARAD_NIF_ELK_TCAM_CONNECT_MODE *kbp_connect_mode)
{
    ...
    if (propval) {
        ...
        } else if ((sal_strcmp(propval, "DUAL") == 0)){
            *kbp_connect_mode = ARAD_NIF_ELK_TCAM_CONNECT_MODE_DUAL;
        } else {
            SOCDNX_EXIT_WITH_ERR(SOC_E_FAIL, (_BSL_SOCDNX_MSG("Unexpected property value (\"%s\") for %s"), propval, propkey));
        }
    } else {
...
soc_arad_str_prop_parse_mpls_termination_database_mode(int unit, uint8 mpls_termination_label_index_enable, uint8 mpls_termination_rif_enable, uint8 *mpls_database, SOC_PPC_MPLS_TERM_NAMESPACE_TYPE  *mpls_namespace)
{
    ...
    SOC_DPP_CONFIG(unit)->pp.tunnel_termination_ordered_by_index = (mode == 11)  ? 1:0;
    ...
    switch (mode) {
        ...
        case 10:
            ...
            break;
        case 11:
            /* 11: MPLS_1 refers to label namespace L2 and located in SEM-A. MPLS_2 refers to label namespace L1,L3 and located in SEM-B. */
            mpls_database[0] = 1;
            mpls_database[1] = 0;        
            mpls_namespace[0] = SOC_PPC_MPLS_TERM_NAMESPACE_L1_L3;
            mpls_namespace[1] = SOC_PPC_MPLS_TERM_NAMESPACE_L2;    
...
#if defined(INCLUDE_KBP) && !defined(BCM_88030)
{
    ...
    if (dpp_arad->init.elk.tcam_dev_type == ARAD_NIF_ELK_TCAM_DEV_TYPE_BCM52311) {
        ...
        elk->kbp_start_lane[0] = soc_property_suffix_num_get(unit, 0, spn_EXT_TCAM_START_LANE, "port", 0x0);
        if (dpp_arad->init.elk.kbp_connect_mode == ARAD_NIF_ELK_TCAM_CONNECT_MODE_DUAL) {
            elk->kbp_start_lane[1] = soc_property_suffix_num_get(unit, 1, spn_EXT_TCAM_START_LANE, "port", 8);
            elk->kbp_start_lane[2] = soc_property_suffix_num_get(unit, 2, spn_EXT_TCAM_START_LANE, "port", 16);
            elk->kbp_start_lane[3] = soc_property_suffix_num_get(unit, 3, spn_EXT_TCAM_START_LANE, "port", 24);
            if (unit % 2 != 0) {
                elk->is_master = FALSE;
            }
        } else {
            elk->kbp_start_lane[1] = soc_property_suffix_num_get(unit, 1, spn_EXT_TCAM_START_LANE, "port", 20);
        }
    }
}
exit:
