...
/* From this place PFC groups defined by masks start */
    ...
    { DPP_PFC_ANY_MPLS1,        "Any MPLSx1",     ARAD_PARSER_PFC_MPLS1_ETH,      ARAD_PARSER_PFC_MATCH_ANY_ENCAP,IS_ETH | IS_MPLS, 0, DPP_PFC_TYPE_GROUP,     0,       ARAD_PARSER_PFC_MPLS1_ETH,      ARAD_PARSER_PFC_MPLS1_ETH,      DPP_PLC_PROFILE_MPLS},
    { DPP_PFC_ANY_MPLS2,        "Any MPLSx2",     ARAD_PARSER_PFC_MPLS2_ETH,      ARAD_PARSER_PFC_MATCH_ANY_ENCAP,IS_ETH | IS_MPLS, 0, DPP_PFC_TYPE_GROUP,     0,       ARAD_PARSER_PFC_MPLS2_ETH,      ARAD_PARSER_PFC_MPLS2_ETH,      DPP_PLC_PROFILE_MPLS},
    { DPP_PFC_ANY_MPLS3,        "Any MPLSx3",     ARAD_PARSER_PFC_MPLS3_ETH,      ARAD_PARSER_PFC_MATCH_ANY_ENCAP,IS_ETH | IS_MPLS, 0, DPP_PFC_TYPE_GROUP,     0,       ARAD_PARSER_PFC_MPLS3_ETH,      ARAD_PARSER_PFC_MPLS3_ETH,      DPP_PLC_PROFILE_MPLS},
    { DPP_PFC_ANY_MPLS,         "Any MPLS",       ARAD_PARSER_PFC_MPLS1_ETH,      ARAD_PARSER_PFC_MATCH_ANY_MPLS, IS_ETH | IS_MPLS, 0, DPP_PFC_TYPE_GROUP,     0,       ARAD_PARSER_PFC_MPLS3_ETH,      ARAD_PARSER_PFC_MPLS3_ETH,      DPP_PLC_PROFILE_MPLS},
...
dpp_parser_pfc_info_t *parser_pfc_info_get_by_sw(DPP_PFC_E pfc_sw)
{
    ...
    LOG_ERROR(BSL_LS_SOC_PKT,(BSL_META_U(0, "Invalid SW PFC:%d\n"), pfc_sw));
    return NULL;
}
...
static dpp_parser_pfc_info_t *parser_pfc_info_get_by_hw(int unit, int pfc_hw)
{
    ...
    {
        LOG_ERROR(BSL_LS_SOC_PKT,(BSL_META_U(0, "Invalid HW PFC:%d\n"), pfc_hw));
        return NULL;
    }
    DPP_PARSER_PFC_GET_STATE(pfc_hw, pfc_sw);
    if(pfc_sw == DPP_PFC_NA)
    {
        LOG_ERROR(BSL_LS_SOC_PKT,(BSL_META_U(0, "HW PFC:%d was not activated\n"), pfc_hw));
        return NULL;
    }
    if((pfc_info = parser_pfc_info_get_by_sw(pfc_sw)) != NULL)
    {
        if(pfc_info->hw != pfc_hw)
        {
             LOG_ERROR(BSL_LS_SOC_PKT,(BSL_META_U(0, "Inconsistency between Inout HW PFC:%d and Internal %d\n"), pfc_hw, pfc_info->hw));
        }
    };
...
static dpp_parser_pfc_info_t *parser_pfc_info_get_by_acl_pair(int unit, int pfc_acl, int pfc_acl_mask)
{
    ...
    LOG_ERROR(BSL_LS_SOC_PKT,(BSL_META_U(0, "PFC with ACL pair PFC:%d MASK:%d not found\n"), pfc_acl, pfc_acl_mask));
    return NULL;
}

static dpp_parser_plc_profile_t *parser_plc_profile_get(DPP_PLC_PROFILE_E plc_profile_id)
{
    ...
    LOG_ERROR(BSL_LS_SOC_PKT,(BSL_META_U(0, "Invalid PLC Profile:%d\n"), plc_profile_id));

    return NULL;
}

static dpp_parser_plc_info_t *parser_plc_info_get_by_sw(DPP_PLC_PROFILE_E plc_profile_id, DPP_PLC_E plc_sw)
{
    dpp_parser_plc_info_t *plc_info;
    dpp_parser_plc_profile_t *plc_profile;
    int i_plc;

    if((plc_profile = parser_plc_profile_get(plc_profile_id)) == NULL)
        return NULL;
    ...
    LOG_ERROR(BSL_LS_SOC_PKT,(BSL_META_U(0, "Invalid SW PLC:%d for profile:%s\n"), plc_sw, plc_profile->name));
    return NULL;
}

static dpp_parser_plc_info_t *parser_plc_info_get_by_hw(DPP_PLC_PROFILE_E plc_profile_id, int plc_hw, int plc_hw_mask)
{
    dpp_parser_plc_profile_t *plc_profile;
    dpp_parser_plc_info_t *plc_info;
    int i_plc;
    ...
    LOG_ERROR(BSL_LS_SOC_PKT,(BSL_META_U(0, "Invalid HW PLC:%d for profile:%s\n"), plc_hw, plc_profile->name));
    return NULL;
}
...
soc_error_t dpp_parser_pfc_get_hw_by_sw(int unit, DPP_PFC_E pfc_sw, uint32 *pfc_hw_p, uint32 *pfc_hw_mask_p)
{
    ...
    {
        LOG_INFO(BSL_LS_SOC_PKT,(BSL_META_U(0, "PFC:%s is not active\n"), pfc_info->name));
...
soc_error_t dpp_parser_pfc_get_vtt_by_sw(int unit, DPP_PFC_E pfc_sw, uint32 *pfc_vtt_p, uint32 *pfc_vtt_mask_p)
{
    ...
    if((pfc_info->type != DPP_PFC_TYPE_GROUP) && (mapped_pfc_sw != pfc_sw))
    {
        LOG_ERROR(BSL_LS_SOC_PKT,(BSL_META_U(0, "PFC:%s is not activated\n"), pfc_info->name));
        return SOC_E_DISABLED;
    }

    if(pfc_info->vtt == ARAD_PARSER_PFC_NOF)
    {
        LOG_ERROR(BSL_LS_SOC_PKT,(BSL_META_U(0, "PFC:%s is not supported for VTT\n"), pfc_info->name));
...
soc_error_t dpp_parser_pfc_activate_hw_by_sw(int unit, DPP_PFC_E pfc_sw, uint32 *pfc_hw_p)
{
    ...
    { /* If it was allocated check if it was allocated to another SW PFC */
        LOG_ERROR(BSL_LS_SOC_PKT,(BSL_META_U(0, "HW PFC:%d was already allocated to SW PFC:%s\n"),
                                               pfc_hw, dpp_parser_pfc_string_by_sw(mapped_pfc_sw)));
        return SOC_E_BUSY;
    }
...
soc_error_t dpp_parser_plc_hw_by_sw(int unit, DPP_PFC_E pfc_sw, DPP_PLC_E plc_sw, uint32 *plc_hw_p, uint32 *plc_hw_mask_p)
{
    ...
    if((plc_info = parser_plc_info_get_by_sw(profile_id, plc_sw)) == NULL)
...
soc_error_t dpp_parser_plc_sw_by_acl(int unit, DPP_PFC_E pfc_sw, uint32 plc_acl, uint32 plc_acl_mask, DPP_PLC_E *plc_sw_p)
{
    ...
    if((plc_info = parser_plc_info_get_by_hw(profile_id, plc_acl, (~plc_acl_mask) & ARAD_PARSER_PLC_MAX)) == NULL)
        return SOC_E_NOT_FOUND;
...
char *dpp_parser_plc_string_by_hw(int unit, int pfc_hw, int plc_hw)
{
    ...
    if((plc_info = parser_plc_info_get_by_hw(profile_id, plc_hw, -1)) == NULL)
        goto exit;
...
char *dpp_parser_plc_profile_string_by_sw(int unit, DPP_PFC_E pfc_sw)
{
    ...
    if((plc_profile = parser_plc_profile_get(profile_id)) == NULL)
        goto exit;
...
dpp_parser_pfc_l4_location_init(int unit)
{
    ...
    for(pfc_hw = 0; pfc_hw < ARAD_PARSER_PFC_NOF; pfc_hw++)
    {
        ...
        if((pfc_info = parser_pfc_info_get_by_sw(pfc_sw)) == NULL)
        {
            LOG_ERROR(BSL_LS_SOC_PKT,(BSL_META_U(0, "Bad SW PFC:%d for HW PFC:%d\n"), pfc_sw, pfc_hw));
            goto exit;
        }
