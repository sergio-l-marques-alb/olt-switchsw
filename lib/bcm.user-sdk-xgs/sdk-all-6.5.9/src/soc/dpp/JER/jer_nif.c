...
#define SOC_AN_MODE_CL73_MSA            (3)
#define SOC_AN_MODE_MSA_ONLY            (4)
...
typedef struct soc_jer_portctrl_phy_toplogy_s {
    ...
    uint16_t is_default;
    uint16_t map_index;       /* The map index for the same physical port*/
} soc_jer_portctrl_phy_toplogy_t;

/*
 * Function:
 *      soc_jer_portctrl_ext_phy_config_topology_get
 * Purpose:
...
soc_jer_portctrl_ext_phy_config_topology_get (int unit, soc_jer_portctrl_phy_toplogy_t *topology)
{
    ...
    /*
    * phy_topology_<pport>_<pport_xphy_index>_<item_index>=<xphy_id>:<sys_lane>:<line_lane>
    */
    ...
    if (config_str == NULL) {
        /* No config found. Try to find phy_topology_<pport>_<pport_xphy_index>*/
...
soc_jer_portmod_fabric_init(int unit)
{
    ...
    {
        /* fabric without fabric link feature, don't init fabric interface */
        SOC_EXIT;
    }
...
soc_jer_portmod_register_external_phys(int unit)
{
     ...
     *  NIF Exteranl PHY
     */
    /* Although we should skip soc_phyctrl_software_init in portmod, we keep it for phy legacy mode. */
    ...
    for (port_i = 0; port_i < SOC_MAX_NUM_PORTS; ++port_i) {
        ...
        tmp_ext_phy_access.type = phymodDispatchTypeInvalid; /* Make sure it is invalid. */
...
soc_jer_port_ilkn_bypass_interface_rx_check_and_enable(int unit, int port) {
    ...
    if (SOC_JER_NIF_IS_ILKN_IF_OVER_FABRIC(unit, offset)) {
        ...
        We are setting it only if need_to_enable_rx != current_rx_status
        */
        ...
        SOC_PBMP_ITER(phy_lanes, phy){
            ...
            shift += SOC_DPP_DEFS_GET(unit, first_fabric_link_id); /*For QMX, ILKN over fabric starts at quad 2, i.e. lane 8)*/
            rst_lanes |= (1 << shift);
        ...
        /*if need_to_enable_rx is up but interface is down - enable the bypass_interface_rx*/
        /*if need_to_enable_rx is down but interface is up - disable the bypass_interface_rx*/
        if (current_rx_status != need_to_enable_rx){
            ...
            PORTMOD_PORT_ENABLE_MAC_SET(flags);
            PORTMOD_PORT_ENABLE_RX_SET(flags);
            SOCDNX_IF_ERR_EXIT(portmod_port_enable_set(unit, port, flags, need_to_enable_rx));
        }
    }else{
 ...
 * Purpose:
 *      Get the info of whether ILKN Rx bypass interface is enabled or disabled.
 * Parameters:
 *      unit - Device unit number.
 *      port - Port number.
 *      rx_enable - return info.
 * Returns:
 *      BCM_E_NONE
 *      BCM_E_XXX
 */
...
soc_jer_portmod_probe(int unit, pbmp_t pbmp, pbmp_t *okay_pbmp, int is_init_sequence)
{
    ...
    if (SOC_WARM_BOOT(unit)) {
        ...
        if (soc_feature(unit, soc_feature_packet_tdm_marking) && is_init_sequence) {
			...
			/* enable link status masking. If set  table data is masked with the link status. */
        ...
        SOC_PBMP_ITER(pbmp, port) {
            ...
            if (SOC_PORT_IS_STAT_INTERFACE(flags)) {
                ...
                SOCDNX_IF_ERR_EXIT(soc_port_sw_db_interface_type_get(unit, port, &interface_type));
                /*if sif port is CAUI,need to adjust CLMAC_TX_CTRL.TX_THRESHOLD to avoid mac underrun*/
        ...
        SOC_PBMP_ITER(pbmp, port) {
            ...
            if ((add_info.interface_config.interface != SOC_PORT_IF_ILKN) &&
                ...
                /* Don't reconfigure the PLL for ports on shared quad*/
                ...
                SOC_PBMP_ITER(shared_ports_bm, shared_port) {
                    ...
                    if (is_initialized == 1)  {
                        /*Raise a flag to tell the driver to not configure the PLL again for this quad*/
        ...
        if( SOC_PBMP_NOT_NULL(SOC_PORT_BITMAP(unit, sfi)) && broadcast_load_fabric && (!SOC_IS_QAX(unit) || SOC_IS_QAX_WITH_FABRIC_ENABLED(unit)) && !SOC_IS_FLAIR(unit)) {
...
soc_jer_portmod_port_speed_set(int unit, soc_port_t port, int speed)
{
    ...
    if (IS_IL_PORT(unit, port) && (BCM_E_NONE == bcm_petra_init_check(unit))) {
        ...
        /* SDK-118544 It's a mandate to reset FMAC TX (through portmod_port_enable_set(unit, port, 0, 0)) before
         * configuring NBIH_NIF_PMH_FGS_RSTN.
         */
        SOCDNX_IF_ERR_EXIT(portmod_port_enable_set(unit, port, 0, 0));
        SOCDNX_IF_ERR_EXIT(MBCM_DPP_SOC_DRIVER_CALL(unit, mbcm_port_fabric_o_nif_bypass_interface_enable, (unit, port, 0)));
    }
    ...
    if (IS_IL_PORT(unit, port) && (BCM_E_NONE == bcm_petra_init_check(unit))) {
        ...
        SOCDNX_IF_ERR_EXIT(MBCM_DPP_SOC_DRIVER_CALL(unit, mbcm_port_fabric_o_nif_bypass_interface_enable, (unit, port, 1)));
        SOCDNX_IF_ERR_EXIT(portmod_port_enable_set(unit, port, 0, enabled));
    }
exit:
    SOCDNX_FUNC_RETURN;
}
...
soc_jer_portmod_check_for_qmlf_conflict(int unit, int new_port)
{
    ...
    SOCDNX_INIT_FUNC_DEFS;
    if (SOC_IS_QUX(unit)) {   /* This function is not relevant for QUX as QUX doesn't support ILKN */
        SOCDNX_FUNC_RETURN;
    }
    ...
    if (ilkn_logical_port != SOC_JER_INVALID_PORT) {                /* There is ILKN on this 3 PM offsets! */
        ...
        SOCDNX_IF_ERR_EXIT(soc_jer_port_ilkn_is_tdm_channel_exist_get(unit, new_port, &tdm_channel_exist));
        if (tdm_channel_exist) {
            SOCDNX_IF_ERR_EXIT(soc_jer_port_ilkn_tdm_and_data_share_hrf_get(unit, new_port, &shr_hrf));
        }
...
soc_jer_port_ilkn_hrf_config_close_path(int unit, int port, int offset){
    ...
    if (nof_segments == 2) {
        ...
        /* when we are working with 4 segments we allocate two PM memories (extended memory) 
         * ILKN0/2/4 -> PM0 and PM2 will be occupied. ILKN1/3/5 -> PM3 and PM5 will be occupied. 
         * before removing the port we need t clear this configuration. 
         */
...
soc_jer_port_ilkn_hrf_config_open_path(int unit, int port, int offset)
{
    ...
    if (nof_segments == 2) {
        ...
        if (!is_master && !tdm_channel_exist) {
        ...
        if (!tdm_channel_exist || shr_hrf) {
            ...
            /* tdm traffic should have a seperate PM memory.
             * to make sure the correct value is set, clear the TDM_AND_DATA_ON_SAME_HRF indication */
            SOCDNX_IF_ERR_EXIT(READ_NBIH_ILKN_RX_TDM_AND_DATA_TRAFFIC_ON_SAME_HRFr(unit, &reg_val));
        ...
        /* when we are working with 4 segments we should allocate two PM memories (extended memory) 
         * ILKN0/2/4 -> PM0 and PM2 will be occupied. ILKN1/3/5 -> PM3 and PM5 will be occupied.
         */
        if (!SOC_PORT_IS_ELK_INTERFACE(flags)) { /* ELK port takes no QMLF memory */
            reg = (offset < 2) ? NBIH_HRF_RX_USE_EXTENDED_MEM_HRFr : NBIL_HRF_RX_USE_EXTENDED_MEM_HRFr;
            SOCDNX_IF_ERR_EXIT(soc_reg32_get(unit, reg, reg_port, offset & 1, &reg_val));
...
soc_jer_ilkn_memory_qmlfs_get(int unit, soc_port_t port, SHR_BITDCL* memory_qmlfs)
{
    ...
    /* Check if Retransmit is enabled */
    ...
    if (nof_segments <= 2 && (!tdm_channel_exist || shr_hrf) && (!retransmit_ena)) {
        ...
        /* get all qmlf used by ethernet ports */
        ...
        /* check if qmlf collision between ethernet ports and ILKN port */
...
soc_jer_port_open_ilkn_path(int unit, int port) {
    ...
    if (is_master) {
        ...
        if (SOC_JER_NIF_IS_ILKN_IF_OVER_FABRIC(unit, offset)) {
            ...
            SOC_PBMP_ITER(phy_lanes, phy){
                il_over_fabric |= 1 << (((phy)  % JER_NIF_ILKN_MAX_NOF_LANES) + SOC_DPP_DEFS_GET(unit, first_fabric_link_id)); /*For QMX, ILKN over fabric starts at quad 2, i.e. lane 8)*/
            }
    ...
    if (is_tdm) {
         ...
         * This register should not be set for data traffic, only for TDM. */
        index = 2;
...
soc_jer_port_open_nbi_path(int unit, int port) {
    ...
    /* if port is SIF on qux, need to take MLF ooo */
...
soc_jer_port_close_ilkn_path(int unit, soc_port_t port) {
    ...
    if (nof_channels == 1 || IS_TDM_PORT(unit, port)/*last TDM channel*/) {
        ...
        if (nof_channels == 1) {
            /* remove last channel, then no qmlf is needed */
            SHR_BITCLR_RANGE(memory_qmlfs, 0, SOC_JERICHO_NOF_QMLFS);
        }
        else {
            /* remove last tdm channel and retransmit is disabled */
            if (!retransmit_ena) {
...
soc_jer_phy_nif_measure(int unit, soc_port_t port, uint32 *type_of_bit_clk, int *one_clk_time_measured_int, int *one_clk_time_measured_remainder, int *serdes_freq_int, int *serdes_freq_remainder, uint32 *lane)
{
    ...
    int nof_phys, i, speed, is_tscf = FALSE, is_tsce = FALSE, is_viper = FALSE;
    SOCDNX_INIT_FUNC_DEFS;
    ...
    if(SOC_IS_DIRECT_PORT(unit, port)) {
        ...
        *lane = SOC_INFO(unit).port_l2p_mapping[port]; /*lane should be 1-based*/
        /* The current implementation doesn't support ILKN over Fabric*/
    ...
    *one_clk_time_measured_remainder = ((((total_time_measured_int*10000) + (total_time_measured_remainder)) / (sync_sth_cnt * vco_div)) % 10000);
    ...
    /*  The HW is connected in a way that the oversample affects the SyncE->SerDesRate calculation
        only on TSCE and TSCF SGMII/GMII. Otherwise - we should ignore it ! */
...
soc_jer_port_loopback_set(int unit, soc_port_t port, soc_dcmn_loopback_mode_t loopback)
{
    ...
    if (IS_IL_PORT(unit, port) && (BCM_E_NONE == bcm_petra_init_check(unit))) {
         ...
         *  Therefore, when ilkn_over_fabric and first_packet_sw_bypass are set => checking enable_get only for Tx MAC.
