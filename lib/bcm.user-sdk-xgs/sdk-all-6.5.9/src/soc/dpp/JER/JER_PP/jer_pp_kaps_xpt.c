                                            ...
                                            uint8 *bytes)
{
    ...
    if (kaps_tcm_write_p[unit] + entry_size_counter * 8 > JER_KAPS_CONFIG_CFIFO_MAX_ADD_8) {
        ...
        rv = jer_sbusdma_desc_add_fifo_dma(unit, KAPS_TCMm, 0/*index*/, KAPS_BLOCK(unit, 0), kaps_tcm_write_p[unit], &tcm_entry[0], remaining_writes, 3, 0);
        if (rv != SOC_SAND_OK) {
        ...
        if ((entry_size_counter - remaining_writes) > 0) {
            ...
            rv = jer_sbusdma_desc_add_fifo_dma(unit, KAPS_TCMm, 0/*index*/, KAPS_BLOCK(unit, 0), kaps_tcm_write_p[unit], &tcm_entry[remaining_writes*2], entry_size_counter - remaining_writes, 3, 1);
            if (rv != SOC_SAND_OK) {
                LOG_CLI((BSL_META_U(0,"%s(), jer_sbusdma_desc_add failed.\n"),FUNCTION_NAME()));
                return KBP_FATAL_TRANSPORT_ERROR;
            }
        ...
        rv = jer_sbusdma_desc_add_fifo_dma(unit, KAPS_TCMm, 0/*index*/, KAPS_BLOCK(unit, 0), kaps_tcm_write_p[unit], &tcm_entry[0], entry_size_counter, 3, new_descriptor);
        if (rv != SOC_SAND_OK) {
            LOG_CLI((BSL_META_U(0,"%s(), jer_sbusdma_desc_add failed.\n"),FUNCTION_NAME()));
            return KBP_FATAL_TRANSPORT_ERROR;
        }
...
uint32 jer_pp_xpt_init(int unit, void **xpt)
{
    ...
    *xpt = soc_sand_os_malloc(sizeof(JER_KAPS_XPT), "kaps_xpt");
    ...
    if (SOC_IS_JERICHO_PLUS(unit)) {
        ...
        /* HITBIT dump machine is not available in QAX */
