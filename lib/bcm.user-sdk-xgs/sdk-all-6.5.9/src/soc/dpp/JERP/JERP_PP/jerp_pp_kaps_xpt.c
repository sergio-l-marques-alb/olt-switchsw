...
/* The number of bits/bytes/uint32 in a single HB reply line, composed of 16 reply HB entries, each entry is JERP_KAPS_HB_BYTES */
...
/* The number of bytes/uint32 in a single HB command line, composed of 7 single HB commands, each command is 64bits */
...
/* The hitbits are duplicated per core */
...
/*#define JERP_KAPS_XPT_PRINTS_ENABLED*/
...
/*
 * This function translates from KBPSDK block numbers to HW block number
 * 0-31: BB's-hitbits; 0-15 Core 0, 16-31 Core 1.
 * Each KAPS_RPB_TCAM_HIT_INDICATION holds the hitbits of 2 BBs.
 * 1022-1023: ADS-prefix-hitbits
 */
kbp_status jerp_kaps_xpt_translate_blk(int unit, uint32 blk_nr, uint32 row_nr, uint32 *blk_hw_nr, uint32 *row_hw_nr) {
    if ((blk_nr >= JERP_BB_FIRST_BLK_ID) && (blk_nr <= JERP_BBS_LAST_BLK_ID)) {
        /*2 BBs per hitbit instance for JER+, JERP_KAPS_HB_HW_ROWS_PER_BB rows for each BB*/
...
kbp_status jerp_kaps_xpt_hb_write(void *xpt, uint32_t blk_nr, uint32_t row_nr, uint8_t *bytes) {
    ...
    if (desc_dma_enabled == 0) {
        if (!SOC_IS_QAX(unit)) {
             /*Need to change mask before and after write to TCAM_HIT_INDICATION*/
        ...
        if (!SOC_IS_QAX(unit)) {
            /*Need to change mask before and after write to TCAM_HIT_INDICATION*/
            SOC_REG_ABOVE_64_ALLONES(reg_above64_val);
            rv = jer_sbusdma_desc_add_reg(unit, KAPS_INDIRECT_WR_MASKr, REG_PORT_ANY, 0/*array_index*/, reg_above64_val);
            if (rv != SOC_SAND_OK) {
                                       ...
                                       uint8 clear_on_read, soc_reg_above_64_val_t reply_line)
{
    ...
    if (!SAL_BOOT_PLISIM) {
        /* polling state machine control to validate it's done */
...
kbp_status jerp_kaps_xpt_hb_dump(void *xpt, uint32_t start_blk_num, uint32_t start_row_num, uint32_t end_blk_num,
    ...
    if ((start_hw_blk_num == JERP_KAPS_HB_HW_PRIVATE_RPB_BLKID) || (start_hw_blk_num == JERP_KAPS_HB_HW_PUBLIC_RPB_BLKID)) {
        ...
        /*Dump from both cores and OR the result line*/
        for (i = 0; i < JERP_KAPS_HB_HW_BB_DUPLICATES(unit); i++) {
            ...
            SHR_BITOR_RANGE(reply_line_core, reply_line, 0/*first*/, JERP_KAPS_HB_STM_REPLY_BITS, reply_line);
        }
    }

    for (i = 0; i < replies_num; i++) {
        for (j = 0; j < JERP_KAPS_HB_BYTES; j++) {
            data[(i*JERP_KAPS_HB_BYTES) + j] = reply_line[i/(SOC_SAND_NOF_BITS_IN_UINT32/SOC_SAND_NOF_BITS_IN_BYTE)] >>
                (((i % (SOC_SAND_NOF_BITS_IN_UINT32/SOC_SAND_NOF_BITS_IN_BYTE)) + j) * SOC_SAND_NOF_BITS_IN_BYTE);
        }
    }
                                       ...
                                       uint16_t source_mask, uint8_t rotate_right, uint8_t perform_clear)
{
    ...
    if (desc_dma_enabled == 0) {
        ...
        /*
         * Assume the hitbit machine mechanism can handle requests faster than the descriptor dma can issue them,
         * therefore no need for polling. Also assume the KBPSDK issues correct hitbit machine commands.
         */
        rv = jer_sbusdma_desc_add_reg(unit, KAPS_HITBIT_COMMANDS_LINEr, REG_PORT_ANY, 0/*array_index*/, command_line);
        if (rv != SOC_SAND_OK) {
            LOG_CLI((BSL_META_U(0,"%s(), jer_sbusdma_desc_add_reg failed, 0xffffffff.\n"),FUNCTION_NAME()));
            return KBP_FATAL_TRANSPORT_ERROR;
        }
...
kbp_status jerp_kaps_xpt_hb_copy(void *xpt, uint32_t src_blk_num, uint32_t src_row_num, uint32_t dest_blk_num,
    ...
    if ((src_hw_blk_num == JERP_KAPS_HB_HW_PRIVATE_RPB_BLKID) || (src_hw_blk_num == JERP_KAPS_HB_HW_PUBLIC_RPB_BLKID)) {
        ...
        /*Write to both cores if necessary*/
        for (i = 0; i < JERP_KAPS_HB_HW_BB_DUPLICATES(unit); i++) {
