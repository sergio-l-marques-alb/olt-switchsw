...
/* used in the soft corrective action - runs mini shmoo */
...
/* clear read and write CRCs interrupt indication from given interface */
static int soc_dpp_drc_combo28_rd_wr_crc_clear(int unit, int drc_ndx)
{
    ...
    /* those interrupt registers are clear on set, need to set to 1 only those 2 to clear the interrupts */
...
/* checks for read and write CRCs from given interface */
...
/* corrective action - doesn't stop (OCB) traffic - soft reset is requiered if during the corrective action another wr/rd crc happend as a result of read/writing the FBC*/
static int soc_dpp_drc_combo28_rd_wr_crc_corrective_action(int unit, int drc_ndx, int* soft_reset_is_requiered)
{
    ...
    *soft_reset_is_requiered = 0;
    ...
    /* check fbc size, and check buffer size to allocate accordingly */
    ...
    /* get range of fbc buffers: min buffer is as indicated in the HW(plus shifts) and max index is and indicated in the HW(plus shifts) + size of buffer - 1 to compensate for the last buffer */
    ...
    /* change FMC buffer ranges to invalid to prevent trashing of the LBM */
    ...
    /* Indirect Access to the MMU seems to be trashing the LBM, need to set LBM_USE_ONLY_FULL_MCf to 1 then back to 0 after done to prevent this. */
    ...
    /* clear interrupt before reading FBC */
    ...
    /* clear interrupt before writting FBC */
    SOCDNX_IF_ERR_EXIT( soc_dpp_drc_combo28_rd_wr_crc_clear(unit, drc_ndx));    
    
    /* write FBC values back */
    ...
    /* change FMC buffer ranges back to valid ranges */
    SOCDNX_IF_ERR_EXIT( soc_reg64_get(unit, MMU_GLOBAL_FULL_MC_DB_RANGE_0r, REG_PORT_ANY, 0, &reg64_val));
...
/*
 * Function:
 *      soc_dpp_drc_combo28_rd_wr_crc_handle_interrupt
 * Purpose:
 *      handle unexplaind drc crc interrupts --> WA for unknown as of yet dram CRC in GDDR5
 * Parameters:
 *      unit                        - Device Number
 *      drc_ndx                     - DRC index
 * Returns:
...
/*
 * Function:
 *      soc_dpp_drc_combo28_rd_wr_crc_control
 * Purpose:
 *      update dram rd/wr crc interrupt mask: restore saved state/mask interrupts
 * Parameters:
 *      unit                        - Device Number
 *      drc_index                   - drc index
 *      restore_saved_state         - restore saved state of interrupt
 *      mask_interrupt              - mask interrupts
 * Returns:
 *      SOC_E_XXX 
 */
    ...
    int         mask_interrupt )
{
    ...
    if (mask_interrupt)
    {
        /* save RD/WR crc interrupts state */
        ...
        /* mask RD/WR crc interrupts of given interface - value of 0 means that the interrupt is masked */
    ...
    {
        /* clear interrupts if were triggered during the time they were masked */
        ...
        /* restore RD/WR crc interrupt state */
        SOCDNX_IF_ERR_EXIT(soc_reg32_get(unit, DRC_INTERRUPT_MASK_REGISTERl[drc_index], REG_PORT_ANY, 0, &reg32_val));
