 ...
 */
STATIC void
td3_pm_port_polarity_get (int unit, int logical_port, int physical_port, int lane,
                            uint32 *tx_pol, uint32* rx_pol)
{
    uint32 rx_polarity, tx_polarity, num_lanes = 0;
                           ...
                           uint32 *txlane_map, uint32* rxlane_map)       
{
    ...
    *rxlane_map = soc_property_phy_get (unit, physical_port, 0, 0, 0, 
    ...
    if (*rxlane_map == 0xFFFFFFFF) {
        *rxlane_map = soc_property_port_suffix_num_get(unit, logical_port, 0,
    ...
    *txlane_map = soc_property_phy_get (unit, physical_port, 0, 0, 0, 
    ...
    if (*txlane_map == 0xFFFFFFFF) {
        *txlane_map = soc_property_port_suffix_num_get(unit, logical_port, 0,
...
_soc_td3_portctrl_pm4x25_portmod_init(int unit, int second_mgmt_port)
{
    ...
    for (pmid = 0; PORTMOD_SUCCESS(rv) && (pmid < SOC_TD3_PM4X25_COUNT); pmid++) {
        ...
        for (lane = 0; lane < SOC_PM4X25_NUM_LANES; lane++) {
            ...
            td3_pm_port_polarity_get (unit, logical_port, phy, lane, &tx_polarity, &rx_polarity);
        ...
        td3_pm_port_lanemap_get (unit, logical_port, first_port, &txlane_map, &rxlane_map);       
...
_soc_td3_portctrl_pm4x10_portmod_init(int unit, int second_mgmt_port)
{
    ...
    for (pmid = 0; PORTMOD_SUCCESS(rv) && (pmid < SOC_TD3_PM4X10_COUNT); pmid++) {
        ...
        if (logical_port != -1) {
            td3_pm_port_lanemap_get (unit, logical_port, pPort[0], &txlane_map, &rxlane_map);
        }
...
soc_td3_portctrl_pm_port_config_get(int unit, soc_port_t port, void *port_config)
{
    ...
    if (i == SOC_TD3_PM4X25_COUNT) {
        for (i=0; i<SOC_TD3_PM4X10_COUNT; i++) {
            if (core_num == (31+i)) {
    ...
    for (core_index = 0; core_index < core_count; core_index++) {
        ...
        for (lane = 0; lane < SOC_PM4X25_NUM_LANES; lane++) {
            ...
            td3_pm_port_polarity_get (unit, logical_port, phy_port, lane, &tx_polarity, &rx_polarity);
