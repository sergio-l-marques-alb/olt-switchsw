...
int soc_cancun_init (uint32 unit) {
    ...
    if(cc == NULL) {
        ...
        cc->flags |= (val & SOC_PROPERTY_CANCUN_VER_MATCH)?
                     SOC_CANCUN_FLAG_SKIP_VERSION_MATCH: 0;
...
int soc_cancun_file_info_get(int unit, soc_cancun_file_t* ccf, char *filename,
    ...
    soc_cancun_t *cc = soc_cancun_info[unit];
    soc_cancun_file_header_t *ccfh = NULL;
    soc_cancun_file_header_t *ccfh_buf = (soc_cancun_file_header_t *) buf;
    char *reg_ver_str, *tmp_str, *prefix_str = "_AV_";
    uint16 dev_id, prefix_str_len = sal_strlen(prefix_str);
    uint8 rev_id;
    uint32 reg_ver, crc, *file_crc;
    ...
    /* Copy file header from buffer to cancun packed format structure */
    ccfh->file_identifier = soc_letohl_load(&ccfh_buf->file_identifier);
    ccfh->file_type       = soc_letohl_load(&ccfh_buf->file_type);
    ccfh->chip_rev_id     = soc_letohl_load(&ccfh_buf->chip_rev_id);
    ccfh->version         = soc_letohl_load(&ccfh_buf->version);
    ccfh->file_length     = soc_letohl_load(&ccfh_buf->file_length);
    ccfh->num_data_blobs  = soc_letohl_load(&ccfh_buf->num_data_blobs);
    ccfh->sdk_version     = soc_letohl_load(&ccfh_buf->sdk_version);
    ccfh->rsvd2           = soc_letohl_load(&ccfh_buf->rsvd2);

    ccf->valid = 0;
    /* 1. File identifier */
    if(ccfh->file_identifier != SOC_CANCUN_FILE_ID) {
        LOG_ERROR(BSL_LS_SOC_CANCUN, (BSL_META_U(unit,
            "ERROR: Not a CANCUN file. Abort\n")));
        return SOC_E_INTERNAL;
    }
    /* 2. File type */
    ...
    if((cc->flags & SOC_CANCUN_FLAG_SKIP_VALIDITY) == 0) {
        ...
        if(ccfh->chip_rev_id != ((dev_id << 16) | rev_id)) {
            LOG_ERROR(BSL_LS_SOC_CANCUN, (BSL_META_U(unit,
        ...
        if(crc != *file_crc) {
            LOG_ERROR(BSL_LS_SOC_CANCUN, (BSL_META_U(unit,
                "ERROR: CRC check fails. Abort\n")));
            return SOC_E_INTERNAL;
        }
    }

    if((cc->flags & SOC_CANCUN_FLAG_SKIP_VERSION_MATCH) == 0) {
        /* 6. REGSFILE version */
        reg_ver_str = sal_strdup(SOC_ORIGIN(unit));
        reg_ver_str = sal_strstr(reg_ver_str, prefix_str);
        if(reg_ver_str != NULL) {
            reg_ver_str += prefix_str_len;
            reg_ver = CANCUN_VERSION_PREFIX_AV << CANCUN_VERSION_OFFSET_PREFIX;
            tmp_str = sal_strtok_r(reg_ver_str, ".", &reg_ver_str);
            reg_ver |= sal_ctoi(tmp_str, 0) << CANCUN_VERSION_OFFSET_MAJOR;
            tmp_str = sal_strtok_r(reg_ver_str, ".", &reg_ver_str);
            reg_ver |= sal_ctoi(tmp_str, 0) << CANCUN_VERSION_OFFSET_MINOR;
            tmp_str = sal_strtok_r(reg_ver_str, " ", &reg_ver_str);
            reg_ver |= sal_ctoi(tmp_str, 0) << CANCUN_VERSION_OFFSET_BUILD;
            if((ccfh->version & CANCUN_VERSION_REGSFILE_MASK) != reg_ver) {
                LOG_ERROR(BSL_LS_SOC_CANCUN, (BSL_META_U(unit,
                    "ERROR: Version mismatch. Abort\n")));
                return SOC_E_INTERNAL;
            }
        }
        else {
            LOG_WARN(BSL_LS_SOC_CANCUN, (BSL_META_U(unit,
                "WARNING: Not a AV labeled REGSFILE. Skip version check\n")));
        }
    }
...
int soc_cancun_cmh_mem_set (int unit, soc_mem_t mem, int index,
    ...
    if(mem == EGR_PORTm && field == PORT_TYPEf) {
        ...
        soc_cancun_app_dest_entry_set (unit, EGR_PORTm, index, INVALIDf, CANCUN_APP__EGR_PORT__QOS_LAYERED_RESOLUTION, 0);
...
int soc_cancun_cch_reg_set(int unit, soc_reg_t reg, int index, uint64 data) {
    ...
    soc_cancun_hash_table_t *hash_table_header;
    soc_cancun_list_t *field_list;
    uint32 *hash_table_entry, *p_field, *hash_entry, *p, *p_val;
    uint32 i, j, k, l, key = 0, offset, val_idx = 0, bit_offset;
    uint32 d_ind_num, d_fld_num, d_mem, d_ind, d_fld, fld_max, fld_min, fld_bits, fld_words;
    uint32 entbuf[SOC_MAX_MEM_FIELD_WORDS] = {0};
    uint32 fldbuf[SOC_MAX_MEM_FIELD_WORDS] = {0};
    uint64 rval, orval, odata64, rdata64, fdata64;
    uint64 p64;
    int rv, cch_d_mem_is_reg;
        ...
        for(i = 0; i < cch_map->dest_mem_num; i++) {
            ...
            for(j = 0; j < d_ind_num; j++) {
                ...
                if(d_mem & SOC_CANCUN_FLAG_REG_ENUM) {
                    ...
                    cch_d_mem_is_reg = 0;
                    SOC_IF_ERROR_RETURN(
                        soc_mem_read(unit, d_mem, COPYNO_ALL, d_ind, entbuf));
                }
                ...
                for(k = 0; k < d_fld_num; k++) {
                    ...
                    if(cch_d_mem_is_reg == 1) {
                        ...
                        if(finfop->len != fld_bits) {
                            ...
                            if(fld_bits % BYTES2BITS(1) == 0 &&
                               bit_offset % BYTES2BITS(1) == 0) {
                                sal_memcpy(((uint8 *)fldbuf) + BITS2BYTES(bit_offset)
                                           , p_val, BITS2BYTES(fld_bits));
                            } else {
                                LOG_ERROR(BSL_LS_SOC_CANCUN, (BSL_META_U(unit,
                                    "doesn't support non-byte align shift yet\n")));
                                return SOC_E_INTERNAL;
                            }
                        } else {
                            sal_memcpy(fldbuf, p_val, BITS2BYTES(fld_bits));
                        }

                        soc_mem_field_set(unit, d_mem, entbuf, d_fld, fldbuf);

                        if(cc->flags & SOC_CANCUN_FLAG_DEBUG_MODE) {
                            char fldbuf_str[(SOC_MAX_MEM_FIELD_WORDS * 8) + 3];
        ...
        for(i = 0; i < hash_table_header->pd; i++) {
            ...
            for(j = 0; j < entry_num; j++) {
                if(cch_map->src_mem != 0 && cch_map->src_field != 0) {
                    ...
                    if (cch_map->format == 1 || cch_map->format == 2) {
...
static int _soc_cancun_cih_load(int unit, uint8* buf, int num_data_blobs) {
    ...
    for (i = 0; i < num_data_blobs; i++) {
         ...
         * each data blob */
        length = soc_letohl_load((buf + SOC_CANCUN_CIH_LENGTH_OFFSET));
        flags = soc_letohl_load((buf + SOC_CANCUN_CIH_FLAG_OFFSET));
