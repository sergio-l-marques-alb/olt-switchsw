...
{
    return (tdm_td3_cmn_get_port_pm(_tdm->_core_data.vars_pkg.port, _tdm));
}
...
tdm_td3_shim_get_pipe_ethernet(tdm_mod_t *_tdm)
{
    ...
    if (_tdm->_core_data.vars_pkg.port<=64) {
        result = tdm_td3_cmn_get_pipe_ethernet(0, _tdm);
    } else if (_tdm->_core_data.vars_pkg.port<=128){
        result = tdm_td3_cmn_get_pipe_ethernet(1, _tdm);
    }
...
**/
int
tdm_td3_shim_core_vbs_scheduler_ovs(tdm_mod_t *_tdm)
{
    if (_tdm->_chip_data.soc_pkg.flex_port_en) {
        /* Used for FlexPort */
        /* return tdm_td3_vbs_scheduler_ovs_flex_port(_tdm); */
        /* return tdm_td3_vbs_scheduler_ovs_flex_port_delta(_tdm) */;
