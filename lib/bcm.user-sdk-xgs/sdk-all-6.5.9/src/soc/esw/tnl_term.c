...
_soc_tunnel_term_flex_hash_key_set(int unit, soc_tunnel_term_t *entry,
    ...
    uint32 key_count = 0;
    uint32 key_value = 0;
    uint32 trav_entry[SOC_MAX_MEM_WORDS];
                          ...
                          int *type, int *entry_type)
{
    ...
    if (soc_feature(unit,soc_feature_flex_flow) &&
        ((soc_mem_field32_get(unit, L3_TUNNELm,
    ...
    } else if (SOC_MEM_FIELD_VALID(unit, L3_TUNNELm, MODEf)) {
        key = soc_mem_field32_get(unit, L3_TUNNELm, 
    ...
    SOC_IF_ERROR_RETURN(_soc_tunnel_term_type_get(unit, entry, &type, entry_type));
     ...
     if (soc_feature(unit,soc_feature_flex_flow) &&
        ...
        for (idx = 1; idx < idx_max; idx++) {
            SOC_IF_ERROR_RETURN
                   (READ_L3_TUNNELm(unit, MEM_BLOCK_ANY, (index + idx) ,
