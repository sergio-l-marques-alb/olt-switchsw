...
#define _SOC_TD3_DEFIP_TCAM_DEPTH   2048
    ...
    sop->memState[L3_ENTRY_LPm].index_max = (7 * 1024) +
                                            (shared_l3_banks * 14 * 1024);

    /* Adjust FP exact match table size */
    sop->memState[EXACT_MATCH_DOUBLEm].index_max = fpem_entries - 1;
    sop->memState[EXACT_MATCH_ONLY_DOUBLEm].index_max = fpem_entries - 1;
    sop->memState[EXACT_MATCH_QUADm].index_max = fpem_entries / 2 - 1;
    sop->memState[EXACT_MATCH_ONLY_QUADm].index_max = fpem_entries / 2 - 1;
    sop->memState[EXACT_MATCH_HIT_ONLYm].index_max = fpem_entries / 2 - 1;
    /* 14k per shared FPEM bank */
    sop->memState[EXACT_MATCH_LPm].index_max = shared_fpem_banks * 14 * 1024;
...
STATIC void
_soc_trident3_speed_to_slot_mapping(int unit, int speed, int *slot)
{
    if (speed >= 40000) {
        if (speed >= 100000) {
            *slot = 40;
        ...
        for (blk_idx = 0; blk_idx < SOC_MAX_NUM_BLKS; blk_idx++) {
            ...
            if (blk_type == SOC_BLK_CLPORT) {
                if (soc_property_phys_port_get(unit,
                 ...
                 || soc_property_suffix_num_get(unit, pm,
                           spn_PORT_FLEX_ENABLE, "core", 0)) {
...
{
#define _SOC_TD3_LCPLL_COUNT    4
#define _SOC_TD3_PLL_CFG_FIELDS 4
#define _SOC_TD3_DEF_TEMP_MAX   125
    int index;
    ...
    soc_reg_field_set(unit, TOP_TS_PLL_CTRL_1r, &rval, POST_RST_HOLD_SELf, 3);
    SOC_IF_ERROR_RETURN(WRITE_TOP_TS_PLL_CTRL_1r(unit, rval));
    
    SOC_IF_ERROR_RETURN(READ_TOP_TS_PLL_CTRL_2r(unit,&rval));
    soc_reg_field_set(unit, TOP_TS_PLL_CTRL_2r, &rval, KAf,
                      soc_property_get(unit, spn_PTP_TS_KA, 0));
    soc_reg_field_set(unit, TOP_TS_PLL_CTRL_2r, &rval, KIf,
                      soc_property_get(unit, spn_PTP_TS_KI, 2));
    soc_reg_field_set(unit, TOP_TS_PLL_CTRL_2r, &rval, KPf,
    ...
    soc_reg_field_set(unit, TOP_TS_PLL_CTRL_4r, &rval, NDIV_FRAC_MODE_SELf, 0);
    ...
    soc_reg_field_set(unit, TOP_BS_PLL0_CTRL_1r, &rval, POST_RST_HOLD_SELf, 3);
    SOC_IF_ERROR_RETURN(WRITE_TOP_BS_PLL0_CTRL_1r(unit, rval));

    SOC_IF_ERROR_RETURN(READ_TOP_BS_PLL0_CTRL_2r(unit, &rval));
    soc_reg_field_set(unit, TOP_BS_PLL0_CTRL_2r, &rval, KAf,
                      soc_property_get(unit, spn_PTP_BS_KA, 0));
    soc_reg_field_set(unit, TOP_BS_PLL0_CTRL_2r, &rval, KIf,
                      soc_property_get(unit, spn_PTP_BS_KI, 2));
    soc_reg_field_set(unit, TOP_BS_PLL0_CTRL_2r, &rval, KPf,
    ...
    soc_reg_field_set(unit, TOP_BS_PLL0_CTRL_4r, &rval, NDIV_FRAC_MODE_SELf, 0);
    ...
    if (!SAL_BOOT_SIMULATION) {
        ...
        for (index = 0; index < _SOC_TD3_LCPLL_COUNT; index++) {
    ...
    for (index = 0; index <COUNTOF(temp_thr_reg); index++) {
         ...
         */
        temp_thr = soc_property_get(unit, temp_thr_prop[index], _SOC_TD3_DEF_TEMP_MAX);
        if (temp_thr > 410) {
            LOG_ERROR(BSL_LS_SOC_COMMON,
                      (BSL_META_U(unit,
...
soc_trident3_uft_uat_config(int unit) {
    ...
    int num_entries, offset_base, offset_multiplier ;
    ...
    SOC_IF_ERROR_RETURN
        (soc_trident3_hash_bank_count_get(unit, L3_ENTRY_ONLY_SINGLEm,
                                          &num_l3_banks));
    SOC_IF_ERROR_RETURN
        (soc_trident3_hash_bank_count_get(unit, EXACT_MATCH_DOUBLEm,
                                          &num_fpem_banks));
    SOC_IF_ERROR_RETURN
    ...
    mpls_16k_banks = uat_16k_banks;
    mpls_8k_banks = uat_8k_banks;
    if (mpls_16k_banks + mpls_8k_banks) {
        offset_base = 64 % (mpls_16k_banks + mpls_8k_banks);
        offset_multiplier = 64 / (mpls_16k_banks + mpls_8k_banks);
    }
    ...
    vlan_xlate_1_16k_banks = uat_16k_banks - mpls_16k_banks;
    vlan_xlate_1_8k_banks = uat_8k_banks - mpls_8k_banks;
    if (vlan_xlate_1_16k_banks + vlan_xlate_1_8k_banks) {
        offset_base = 64 % (vlan_xlate_1_16k_banks + vlan_xlate_1_8k_banks);
        offset_multiplier = 64 / (vlan_xlate_1_16k_banks +
    ...
    for(index = 0; index < vlan_xlate_1_8k_banks; index++) {
        ...
        soc_mem_field32_set(unit, IP_UAT_8K_SHARED_BANKS_CONTROLm,
                            shared_8k_hash_control_entry, shared_fields[index +
                            mpls_8k_banks], offset_base + (index *
                            offset_multiplier));
    }
    ...
    for(index = 0; index < vlan_xlate_1_16k_banks; index++) {
        ...
        soc_mem_field32_set(unit, IP_UAT_16K_SHARED_BANKS_CONTROLm,
                            shared_hash_control_entry, shared_fields[index +
                            mpls_16k_banks], offset_base + (index *
                            offset_multiplier));
    }
    SOC_IF_ERROR_RETURN
    ...
    vlan_xlate_2_8k_banks = uat_8k_banks - mpls_8k_banks -
                                vlan_xlate_1_8k_banks;
    if (vlan_xlate_2_16k_banks + vlan_xlate_2_8k_banks) {
        offset_base = 64 % (vlan_xlate_2_16k_banks + vlan_xlate_2_8k_banks);
        offset_multiplier = 64 / (vlan_xlate_2_16k_banks +
                                    vlan_xlate_2_8k_banks);
    }
    ...
    for(index = 0; index < vlan_xlate_2_8k_banks; index++) {
        ...
        soc_mem_field32_set(unit, IP_UAT_8K_SHARED_BANKS_CONTROLm,
                            shared_8k_hash_control_entry, shared_fields[index +
                            mpls_8k_banks + vlan_xlate_1_8k_banks], offset_base
                            + (index * offset_multiplier));
    }
                             ...
                             int index, uint64 *data, int write)
{
    ...
    if (sed >= 0 && base_index >= 0) {
        SOC_IF_ERROR_RETURN
            (_soc_trident3_xpe_reg_check(unit, reg, sed, base_index));
    }
...
STATIC int
_soc_trident3_clear_all_memory(int unit)
{
    uint32 pipe_map;
    int block_info_idx;
    soc_reg_t reg;
...
STATIC int
_soc_trident3_tdm_idb_calendar_set(int unit,  int calendar_id)
{
    soc_info_t *si;
    _soc_tomahawk_tdm_t *tdm;
    uint32 pipe_map;
    ...
    for (pipe = 0; pipe < NUM_PIPE(unit); pipe++) {
        ...
        for (length = _TDM_LENGTH; length > 0; length--) {
            if (tdm->idb_tdm[pipe][length - 1] != _SOC_TH_TDM_NUM_EXT_PORTS) {
                break;
            }
        }
        ...
        for (slot = 0; slot < length; slot += 2) {
            ...
            if (phy_port == TH_OVSB_TOKEN) {
                idb_port = _SOC_TD3_TDM_OVERSUB_TOKEN;
            } else if (phy_port == TH_IDL1_TOKEN) {
                idb_port = _SOC_TD3_TDM_IDL1_TOKEN;
            } else if (phy_port == TH_IDL2_TOKEN) {
                idb_port = _SOC_TD3_TDM_IDL2_TOKEN;
            } else if (phy_port == NULL_TOKEN) {
                idb_port = _SOC_TD3_TDM_NULL_TOKEN;
            } else if (phy_port >= _SOC_TH_TDM_NUM_EXT_PORTS) {
                idb_port = _SOC_TD3_TDM_UNUSED_TOKEN;
            } else {
                port = si->port_p2l_mapping[phy_port];
                idb_port = si->port_l2i_mapping[port];
            }
            ...
            if (tdm->idb_tdm[pipe][slot + 2] == _SOC_TH_TDM_NUM_EXT_PORTS) {
                soc_reg_field_set(unit, reg, &rval, field,
                                  phy_port == _SOC_TH_TDM_NUM_EXT_PORTS ?
                                              slot : slot + 1);
                break;
            }
        }
...
STATIC int
_soc_trident3_tdm_mmu_calendar_set(int unit, int calendar_id)
{
    soc_info_t *si;
    _soc_tomahawk_tdm_t *tdm;
    uint32 pipe_map;
    ...
    for (pipe = 0; pipe < NUM_PIPE(unit); pipe++) {
        ...
        for (length = _TDM_LENGTH; length > 0; length--) {
            if (tdm->mmu_tdm[pipe][length - 1] != _SOC_TH_TDM_NUM_EXT_PORTS) {
                break;
            }
        }
        ...
        for (slot = 0; slot < length; slot += 2) {
            ...
            if (phy_port == TH_OVSB_TOKEN) {
                mmu_port = _SOC_TD3_TDM_OVERSUB_TOKEN;
            } else if (phy_port == TH_IDL1_TOKEN) {
                mmu_port = _SOC_TD3_TDM_IDL1_TOKEN;
            } else if (phy_port == TH_IDL2_TOKEN) {
                mmu_port = _SOC_TD3_TDM_IDL2_TOKEN;
            } else if (phy_port == NULL_TOKEN) {
                mmu_port = _SOC_TD3_TDM_NULL_TOKEN;
            } else if (phy_port >= _SOC_TH_TDM_NUM_EXT_PORTS) {
                mmu_port = _SOC_TD3_TDM_UNUSED_TOKEN;
            } else {
                mmu_port = si->port_p2m_mapping[phy_port];
            }
            ...
            SOC_IF_ERROR_RETURN
                (soc_mem_write(unit, mem, MEM_BLOCK_ALL, slot / 2, entry));
            if (tdm->mmu_tdm[pipe][slot + 2] == _SOC_TH_TDM_NUM_EXT_PORTS) {
                soc_reg_field_set(unit, reg, &rval, field,
                                  phy_port == _SOC_TH_TDM_NUM_EXT_PORTS ?
                                              slot : slot + 1);
                break;
...
STATIC int
_soc_trident3_tdm_calculation(int unit, _soc_tomahawk_tdm_t *tdm)
{
    soc_info_t *si;
    int port, phy_port;
    ...
    SOC_BLOCK_ITER(unit, block_info_idx, SOC_BLK_CLPORT) {
        port = SOC_BLOCK_PORT(unit, block_info_idx);
        clport = SOC_BLOCK_NUMBER(unit, block_info_idx);
        soc_trident3_port_ratio_get(unit, clport, &tdm->port_ratio[clport]);
    ...
    SOC_PBMP_ITER(si->management_pbm, port) {
        phy_port = si->port_l2p_mapping[port];
        tdm->pm_encap_type[_TD3_PBLKS_PER_DEV(unit)] = mode[si->port_pipe[port]];
        tdm->speed[phy_port] = si->port_speed_max[port];
    }
    ...
    if (LOG_CHECK(BSL_LS_SOC_TDM | BSL_INFO)) {
        ...
        LOG_CLI((BSL_META_U(unit,
                            "port speed:")));
        for (index = 0; index < _SOC_TH_TDM_NUM_EXT_PORTS; index++) {
            if (index % 8 == 0) {
                LOG_CLI((BSL_META_U(unit,
                                    "\n    ")));
            }
        ...
        for (index = 0; index < _SOC_TH_TDM_NUM_EXT_PORTS; index++) {
            ...
            if (index == 0 || index == (_SOC_TH_TDM_NUM_EXT_PORTS - 1)) {
                LOG_CLI((BSL_META_U(unit,
                                    " ---")));
            } else {
                LOG_CLI((BSL_META_U(unit,
        ...
        LOG_CLI((BSL_META_U(unit,
                            "pm encap type:")));
        for (index = 0; index < _SOC_TH_TDM_NUM_PM_MOD; index++) {
            if (index % 16 == 0) {
                LOG_CLI((BSL_META_U(unit,
                                    "\n    ")));
            }
    ...
    for (index = 1; index < _SOC_TH_TDM_NUM_EXT_PORTS; index++) {
        tdm->tdm_globals.port_rates_array[index-1] = tdm->port_state[index];
    }
    for (index = 1; index < _SOC_TH_TDM_NUM_PM_MOD; index++) {
        ...
        for (group = 0; group < _OVS_GROUP_COUNT; group++) {
            ...
            for (index = 0; index < _OVS_GROUP_TDM_LENGTH; index++) {
                ...
                if (port_slot_count == 0) {
                    _soc_trident3_speed_to_slot_mapping
...
STATIC int
_soc_trident3_hash_init(int unit)
{
...
_soc_trident3_idb_init(int unit)
{
    ...
    static const struct _obm_setting_s {
        ...
        { /* 1 lane */
            1148, 129, 760,
            { 618, 618, 618 }, /* port_xoff */
            { 240, 240, 240 }, /* lossy_low_pri */
            { 580, 580, 580 },  /* lossy_discard */
            { 0,    1152, 2304, 3456}, /* buffer_start */
            { 1151, 2303, 3455, 4607}  /* buffer_end */
        },
        { /* 2 lanes */
            2300, 321, 1912,
            { 1475, 1475, 1475 }, /* port_xoff */
            { 240, 240, 240 }, /* lossy_low_pri */
            { 580, 580, 580 },  /* lossy_discard */
                               ...
                               int *obm_id, int *lane)
{
    ...
    if (obm_id != NULL) {
        *obm_id = pipe & 1 ? (15 - (clport & 0xF)) : (clport & 0xF);
    }
...
STATIC int
_soc_trident3_init_idb_memory(int unit)
{
    uint32 pipe_map;
    soc_reg_t reg;
...
_soc_trident3_misc_init(int unit)
{
    ...
    SOC_IF_ERROR_RETURN(_soc_trident3_init_idb_memory(unit));
    if (parity_enable) {
        if ((!SAL_BOOT_SIMULATION || SAL_BOOT_QUICKTURN) &&
            ...
            SOC_IF_ERROR_RETURN(_soc_trident3_clear_mmu_memory(unit, INVALIDm));
    ...
    if (!SOC_IS_RELOADING(unit) && !SOC_WARM_BOOT(unit)) {
        SOC_IF_ERROR_RETURN(_soc_trident3_clear_all_memory(unit));
    }
    ...
    SOC_IF_ERROR_RETURN(_soc_trident3_hash_init(unit));
                            ...
                            int lossless)
{
    ...
    if ((speed >= 1000) && (speed <= 10000)) {
        ...
        hdrm = port_oversub ? 262 : 181;
    } else if ((speed > 20000) && (speed <= 42000)) {
        hdrm = port_oversub ? 361 : 280;
    } else if (speed >= 100000) {
        hdrm = port_oversub ? 699 : 507;
    } else {
        hdrm = port_oversub ? 196 : 163;
    }
    return hdrm;
...
_soc_trident3_mmu_init(int unit)
{
    uint32 pipe_map;
    int pipe;
    uint64 enable[_TD3_PIPES_PER_DEV];
    int test_only;
    int alloc_size;
...
#endif
    {
        uint64 const1_64 = COMPILER_64_INIT(0,0x1);
        soc_trident3_pipe_map_get(unit, &pipe_map);
...
STATIC int
_soc_trident3_update_ep_credit(int unit, soc_td3_port_lanes_t *lanes_ctrl)
{
    soc_reg_t mem = EGR_MMU_CELL_CREDITm;
    uint32 vco, fval;
    ...
    if (lanes_ctrl->lanes == 1) {
        ...
        if (lanes_ctrl->mode == SOC_TD3_PORT_MODE_TRI_012 ||
            lanes_ctrl->mode == SOC_TD3_PORT_MODE_TRI_023) {
            if (lanes_ctrl->mode == SOC_TD3_PORT_MODE_TRI_012) {
    ...
    sal_memset(entry, 0, sizeof(egr_mmu_cell_credit_entry_t));
    soc_mem_field32_set(unit, mem, entry, CREDITf, fval);
    for (i = 0; i < pc; i+=n) {
        SOC_IF_ERROR_RETURN(WRITE_EGR_MMU_CELL_CREDITm(unit, MEM_BLOCK_ALL, port+i, entry));
    }
    return SOC_E_NONE;
}
...
#if !defined(BCM_TD3_ASF_EXCLUDE)
STATIC int
_soc_trident3_xmt_start_cnt_update(int unit, int extra_delay)
{
    int                 port;
    int                 port_speed;
    soc_td3_asf_mode_e  mode = _SOC_TD3_ASF_MODE_SAF;
    soc_info_t *si;
    ...
    PBMP_ALL_ITER(unit, port) {
        ...
        port_speed = (si->port_init_speed[port]? si->port_init_speed[port]:
...
STATIC int
_soc_trident3_port_lanes_update_tdm(int unit, soc_td3_port_lanes_t *lanes_ctrl)
{
    soc_control_t *soc;
    soc_info_t *si;
    _soc_tomahawk_tdm_t *tdm;
    ...
    if (!SOC_PBMP_MEMBER(si->oversub_pbm, port_base)) {
        SOC_IF_ERROR_RETURN(_soc_trident3_xmt_start_cnt_update(unit, 9));
        SOC_IF_ERROR_RETURN(_soc_trident3_tdm_calculation(unit, tdm));
        SOC_IF_ERROR_RETURN(_soc_trident3_tdm_idb_calendar_set(unit, -1));
        SOC_IF_ERROR_RETURN(_soc_trident3_tdm_mmu_calendar_set(unit, -1));
        SOC_IF_ERROR_RETURN(_soc_trident3_update_ep_credit(unit, lanes_ctrl));
        SOC_IF_ERROR_RETURN(_soc_trident3_xmt_start_cnt_update(unit, 0));
    } else {
        pipe = si->port_pipe[port_base];
...
/*
 * Function:
 *      soc_trident3_port_lanes_set
 * Description:
 *      Configure number of lanes used by a port
 * Parameters:
 *      unit          - Device number
 *      lanes_ctrl    - Control structure
 * Notes: Caller needs to call soc_trident3_port_lanes_validate to fill
 *        in the control structure.
 */
int
soc_trident3_port_lanes_set(int unit, soc_td3_port_lanes_t *lanes_ctrl)
{
    uint32 rval;
    soc_reg_t reg;
    uint32 values[2];
...
#undef RECONFIGURE_PORT_TYPE_INFO
    ...
    if (lanes_ctrl->phy_ports_len > 0) {
        /* Update TDM */
        SOC_IF_ERROR_RETURN
            (_soc_trident3_port_lanes_update_tdm(unit, lanes_ctrl));
    }
