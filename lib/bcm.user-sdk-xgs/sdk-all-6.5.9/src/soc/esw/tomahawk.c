...
soc_th_port_config_validate(int unit)
{
    ...
    int port_sister_speed, port_sister_ovs;
    ...
    for (blk = 0; blk < SOC_MAX_NUM_BLKS; blk++) {
        if (SOC_BLOCK_TYPE(unit, blk) == SOC_BLK_CLPORT) {
            ...
            SOC_PBMP_ITER(si->block_bitmap[blk], lport) {
                ...
                port_sister_speed = si->port_speed_max[lport];
                port_sister_ovs = SOC_PBMP_MEMBER(si->oversub_pbm, lport) ? 1 : 0;
                if (ovs != port_sister_ovs) {
                    return SOC_E_CONFIG;
                }

                if (ovs &&
                    (port_base_speed == 100000) &&
                    (port_sister_speed == 50000)) {
