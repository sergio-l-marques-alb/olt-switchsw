...
#define APACHE_PORTS_PER_TSC         4
#define APACHE_TSCS_PER_PGW          9
#define APACHE_NUM_XLP_PER_PGW       7
#define APACHE_NUM_CLP_PER_PGW       2
#define APACHE_PGWS_PER_DEV          2
#define APACHE_PGW0                  0
#define APACHE_PGW1                  1

#define APACHE_PORTS_PER_PGW         (APACHE_PORTS_PER_TSC * APACHE_TSCS_PER_PGW)
#define APACHE_PORTS_PER_PIPE        (APACHE_PORTS_PER_PGW * APACHE_PGWS_PER_DEV)
...
static  _soc_apache_ser_info_t _soc_apache_clport_ser_info[] = {
    ...
    { _SOC_PARITY_TYPE_MAC_RX_TS, NULL, 0,
      MAC_RX_TS_CDC_MEM_ERRf, INVALIDf,
...
soc_apache_port_ca_mode_get(int unit, int flexport, int xlp, int *mode)
{
    ...
    int num_lanes[APACHE_PORTS_PER_TSC];
    int speed_max[APACHE_PORTS_PER_TSC];

    si = &SOC_INFO(unit);
    *mode = SOC_AP_PORT_CA_MODE_QUAD;

    phy_port_base = 1 + (xlp * APACHE_PORTS_PER_TSC);
    for (lane = 0; lane < APACHE_PORTS_PER_TSC; lane++) {
...
_soc_apache_idb_port_init(int unit, int reset, int lossless, soc_port_t port)
{
    ...
    */ 
    ...
    obm %= APACHE_TSCS_PER_PGW;

    pgw = si->port_group[port];
    pgw_inst = (pgw | SOC_REG_ADDR_INSTANCE_MASK);
    subport = (phy_port - 1) % APACHE_PORTS_PER_TSC;
    ...
    if (lossless) {
        mem = obm_pri_map_port_mem[(phy_port - 1) % APACHE_PORTS_PER_PGW];
...
_soc_apache_port_blk_ca_mode_set(int unit, int flexport, int xlp)
{
    ...
    r32val0 = 0;
    reg = obm_ca_ctrl_regs[xlp % APACHE_TSCS_PER_PGW];
    pgw_inst = ((xlp / APACHE_TSCS_PER_PGW) | SOC_REG_ADDR_INSTANCE_MASK);
    /* Debug output */
    LOG_VERBOSE(BSL_LS_SOC_PORT,
                (BSL_META_U(unit,
                            "Port Block CellAssembly Mode: "
                            "xlp=%d  pgw=%d reg=%s ca_mode=%d \n"),
                 xlp, (xlp / APACHE_TSCS_PER_PGW), SOC_REG_NAME(unit, reg), ca_port_mode));
...
STATIC int
_soc_apache_l3_mem_sram_info_get(int unit, int *base, int *count,
                                 soc_mem_t *view,
                               ...
                               _soc_ser_sram_info_t *sram_info)
{
    ...
    case ING_L3_NEXT_HOPm:
        sram_info->ram_count = SOC_APACHE_NUM_EL_ING_L3_NEXT_HOP;
    ...
    case L2_ENTRY_ISS_LPm:
        ...
        sram_info->ram_count = 1;
        sram_info->view[0] = L2_ENTRY_ONLYm;
        break;
    case L3_ENTRY_ISS_LPm:
        base = index;
        SOC_IF_ERROR_RETURN(_soc_apache_l3_mem_sram_info_get(unit, &base,
                                 ...
                                 char *prefix_str)
{
    ...
    do {
        ...
        if (spci.mem != INVALIDm) {
            ...
            _soc_apache_ser_control_reg_get(unit,
                                            _soc_bcm56560_a0_mmu_mem_ser_info,
                                            spci.mem,
                                            &parity_enable_reg,
                                            &parity_enable_field);
        } else {
        ...
        spci.log_id = _soc_apache_populate_ser_log(unit, parity_enable_reg,
                                                   parity_enable_field,
                                                   spci.mem, blk_idx,
...
soc_apache_port_is_2p5g_forced(int unit, int phy_port)
{
    ...
    tsc_id = ((phy_port - 1) / APACHE_PORTS_PER_TSC);
     ...
     */
    bit_pos = (tsc_id >= APACHE_TSCS_PER_PGW) ? (tsc_id - APACHE_NUM_CLP_PER_PGW) : tsc_id;
...
soc_apache_port_config_bandwidth_check(int unit)
{
    ...
    int pgw_port[APACHE_PORTS_PER_PGW];
    int pgw_port_bandwidth[APACHE_PORTS_PER_PGW];
    int pgw_linerate_bandwidth[APACHE_PGWS_PER_DEV], pgw_oversub_bandwidth[APACHE_PGWS_PER_DEV];
    int max_pgw_core_bandwidth[APACHE_PGWS_PER_DEV];
    int max_pgw_oversub_bandwidth[APACHE_PGWS_PER_DEV];
    int os_ratio[APACHE_PGWS_PER_DEV];
    ...
    for (pgw = 0; pgw < APACHE_PGWS_PER_DEV; pgw++) {
        max_pgw_core_bandwidth[pgw] = (SOC_INFO(unit).bandwidth) / APACHE_PGWS_PER_DEV;
        max_pgw_oversub_bandwidth[pgw] = (SOC_INFO(unit).io_bandwidth) / APACHE_PGWS_PER_DEV;
    }
    ...
    for (pgw = 0; pgw < APACHE_PGWS_PER_DEV; pgw++) {
        ...
        for (index = 0; index < APACHE_PORTS_PER_PGW; index++) {
            phy_port = 1 + pgw * APACHE_PORTS_PER_PGW + index;
        ...
        for (blk = 0; blk < APACHE_TSCS_PER_PGW; blk++) {

            blk_port = &pgw_port[blk * APACHE_PORTS_PER_TSC];
            blk_port_bandwidth = &pgw_port_bandwidth[blk * APACHE_PORTS_PER_TSC];

            blk_configured_map = 0;
            for (index = 0; index < APACHE_PORTS_PER_TSC; index++) {
                if (blk_port[index] != -1) {
            ...
            if (port_bandwidth > speed_lanes_1) {
                ...
                if (blk_port[0] == -1) {
                    LOG_CLI((BSL_META_U(unit,
                               ...
                               "port %d\n"),
                               1 + pgw * APACHE_PORTS_PER_PGW +
                               blk * APACHE_PORTS_PER_TSC,
                               blk_port[2]));
                    rv = SOC_E_FAIL;
                }
            }
            ...
            if (blk_inactive_map != 0) {
                if (blk_oversub_map != 0 &&
                    ...
                    LOG_CLI((BSL_META_U(unit, "Flex port")));
                    for (index = 0; index < APACHE_PORTS_PER_TSC;
                         index++) {
                        if (blk_port[index] != -1) {
                            LOG_CLI((BSL_META_U(unit, " %d"), blk_port[index]));
                        }
    ...
    if (oversub_mode == 2) {
         ...
         */
        if ((os_ratio[APACHE_PGW0] > def_os_ratio) && (os_ratio[APACHE_PGW0] < max_os_ratio)) {
            if (os_ratio[APACHE_PGW1] > def_os_ratio) {
                LOG_CLI((BSL_META_U(unit,
                                    "PGW_CL1 total oversub bandwidth "
                                    "(%d Gb) exceeds limit\n"),
                                    ((pgw_oversub_bandwidth[APACHE_PGW1]) / 1000)));
                rv = SOC_E_FAIL;
            }
        } else if ((os_ratio[APACHE_PGW1] > def_os_ratio) && (os_ratio[APACHE_PGW1] < max_os_ratio)) {
            if (os_ratio[APACHE_PGW0] > def_os_ratio) {
                LOG_CLI((BSL_META_U(unit,
                                    "PGW_CL0 total oversub bandwidth "
                                    "(%d Gb) exceeds limit\n"),
                                    ((pgw_oversub_bandwidth[APACHE_PGW0]) / 1000)));
                rv = SOC_E_FAIL;
            }
        }
    }
...
soc_apache_mmu_ports_assign(int unit)
{
    ...
    int port_is_hsp;
    int num_phy_port = APACHE_PORTS_PER_PIPE;
    soc_info_t *si;
    soc_pbmp_t pbmp;
     ...
     */
    for (phy_port = 1; phy_port <= num_phy_port; phy_port += APACHE_PORTS_PER_TSC) {
        if (si->port_p2m_mapping[phy_port] != -1) {
            continue;
        }
     ...
     */
    for (phy_port = 3; phy_port <= num_phy_port; phy_port += APACHE_PORTS_PER_TSC) {
        if (si->port_p2m_mapping[phy_port] != -1) {
            continue;
        }
...
soc_apache_port_config_init(int unit, uint16 dev_id, uint8 rev_id)
{
    ...
    for (port = 0; port <= num_port; port++) {
        ...
        /* 26 ports per PGW_CL */
        si->port_group[port] = (phy_port - 1) / APACHE_PORTS_PER_PGW;
        si->port_serdes[port] = (phy_port - 1) / APACHE_PORTS_PER_TSC;
...
soc_apache_lls_bmap_alloc(int unit)
{
    ...
    for (port = 0; port < APACHE_PORTS_PER_PIPE + 2; port++) {
...
soc_apache_tsc_disable(int unit)
{
    ...
    for (phy_port = 1;
         phy_port <= APACHE_PORTS_PER_PIPE;
         phy_port = (phy_port + APACHE_PORTS_PER_TSC)) {

        tsc = (phy_port - 1) / APACHE_PORTS_PER_TSC;
        for (lane = 0; lane < APACHE_PORTS_PER_TSC; lane++) {
...
typedef struct soc_apache_tdm_s {
    ...
    int pgw_tdm[APACHE_PGWS_PER_DEV][APACHE_PGW_TDM_LENGTH];
    int mmu_tdm[APACHE_MMU_TDM_LENGTH + 1];

    struct {
        int pgw_ovs_tdm[APACHE_PGWS_PER_DEV][APACHE_PGW_TDM_LENGTH];
        int pgw_ovs_spacing[APACHE_PGWS_PER_DEV][APACHE_PGW_TDM_LENGTH];
    } ingress_ovs_tdm;
 ...
 */
static const soc_reg_t pgw_hsp_config_regs[APACHE_NUM_CLP_PER_PGW * APACHE_PORTS_PER_TSC] = {
...
static const soc_reg_t pgw_hsp_config_fields[APACHE_NUM_CLP_PER_PGW * APACHE_PORTS_PER_TSC] = {
    PHY_PORT_NO_1f, PHY_PORT_NO_5f, PHY_PORT_NO_3f, PHY_PORT_NO_7f,
    PHY_PORT_NO_0f, PHY_PORT_NO_4f, PHY_PORT_NO_2f, PHY_PORT_NO_6f
};
static const soc_mem_t ap_mmu_tdm_mem[] = {ES_PIPE0_TDM_TABLE_0m, ES_PIPE1_TDM_TABLE_1m};
 ...
 */
#define APACHE_TDM_SPEED_ADJUST(unit, port, speed)          \
    do {                                                    \
...
soc_apache_tdm_calc(int unit, soc_apache_tdm_t *apache_tdm)
{
    ...
    switch (dev_id) {
        case BCM56565_DEVICE_ID:
            apache_chip_pkg.clk_freq = 794;
            break;
        case BCM56567_DEVICE_ID:
            apache_chip_pkg.clk_freq = 795;
            break;
        case BCM56568_DEVICE_ID:
            apache_chip_pkg.clk_freq = 796;
            SOC_PBMP_ITER(PBMP_PORT_ALL(unit), port) {
                if ((si->port_serdes[port] == 6) ||
                    (si->port_serdes[port] == 16) ||
                    (si->port_serdes[port] == 17)) {
                    /* TSC6, TSC16 or TSC 17 is in oversub */
                    apache_chip_pkg.clk_freq = IS_OVERSUB_PORT(unit, port) ? 796 : 797;
                    break;
                }
            }
            break;
        ...
        case BCM56068_DEVICE_ID:
            apache_chip_pkg.clk_freq = 840;
            break;
        default:
            apache_chip_pkg.clk_freq = 793;
            break;
    }
                      ...
                      soc_apache_tdm_t *apache_tdm)
{
    ...
    for (pgw = 0; pgw < APACHE_PGWS_PER_DEV; pgw++) {
        ...
        for (phy_port = pgw_clp_ports[pgw];
             phy_port < pgw_clp_ports[pgw] + (APACHE_NUM_CLP_PER_PGW * APACHE_PORTS_PER_TSC);
             phy_port++)
        {
            port = SOC_INFO(unit).port_p2l_mapping[phy_port];
...
soc_apache_tdm_init(int unit)
{
    ...
    PBMP_PORT_ITER(unit, port) {
        ...
        APACHE_TDM_SPEED_ADJUST(unit, port, apache_tdm->speed[phy_port]);
...
soc_apache_port_edb_config(int unit, int asf, soc_port_t port)
{
    ...
    soc_field_t asm_ct_fields[APACHE_NUM_XLP_PER_PGW + APACHE_NUM_CLP_PER_PGW] = {
    ...
    soc_cm_get_id(unit, &dev_id, &rev_id);
    if ((dev_id == BCM56069_DEVICE_ID)) {
    pm_id = ((phy_port - 1) / APACHE_PORTS_PER_TSC) % APACHE_TSCS_PER_PGW;
        if (pm_id < APACHE_NUM_XLP_PER_PGW) { /* Eagle Cores */
            ...
            if (si->port_speed_max[port] <= 11000) {  /* 0-11G */
                ...
                edb_values[0] = (linerate) ? (asf ? (cl91_enabled ? 10 : 3) : 0) : (asf ? 0 : 4);
                edb_values[1] = (linerate) ? (asf ? 0 : 1) : 0;
                edb_values[2] = 0; /* linerate/oversub and cut-thru or no cut-thru */
...
_soc_apache_misc_init (int unit)
{
    ...
    SOC_IF_ERROR_RETURN
        (soc_hbx_higig2_mcast_sizes_set(unit,
             ...
             soc_property_get(unit, spn_HIGIG2_MULTICAST_L2_RANGE,
                              (soc_mem_index_count(unit, L2MCm) / 3) * 2),
             soc_property_get(unit, spn_HIGIG2_MULTICAST_L3_RANGE,
                              soc_mem_index_count(unit, L2MCm) / 3)));
    SOC_IF_ERROR_RETURN
    ...
    SOC_IF_ERROR_RETURN
        (soc_hbx_ipmc_size_set(unit, soc_property_get(unit,
             spn_MULTICAST_L3_RANGE, soc_mem_index_count(unit, L2MCm) / 3)));
                                     ...
                                     _soc_mmu_rsvd_buffer_t *rsvd_buffer)
{
    ...
    for (pm = start_pm; pm < end_pm; pm++) {
        ...
        if (pm_flex_enabled == 1) {
            ...
            asf_cells = 0;
            mcq_entry = 0;
            start_port = (APACHE_PORTS_PER_TSC * pm ) + 1 ;
            for (j = start_port; j < start_port + APACHE_PORTS_PER_TSC; j++) {
                               ...
                               soc_port_t input_port)
{
    ...
    PBMP_PORT_ITER(unit, port) {
        ...
        for (idx = 0; idx < numq; idx++) {
            ...
            if (buf_queue->discard_enable) {
                soc_mem_field32_set(unit, mem2, entry0, QGROUP_LIMIT_ENABLEf, 1);
                     ...
                     int post_num_res, soc_port_resource_t *post_res)
{
    ...
    for (i = 0; i < pre_num_res; i++) {
        ...
        xlp = (pre_res[i].physical_port % (APACHE_TSCS_PER_PGW * APACHE_PGWS_PER_DEV));
...
soc_ap_cut_thru_enable_disable(int unit, soc_port_t port, int enable)
{
    ...
    if (soc_property_port_get(unit, port, "pfc_optimized_settings", 1)) {
        ...
        phy_port = SOC_INFO(unit).port_l2p_mapping[port];
        SOC_IF_ERROR_RETURN(READ_EGR_PFC_CONTROLm(unit, MEM_BLOCK_ALL, phy_port, &pfc_control));
        soc_mem_field32_set(unit, EGR_PFC_CONTROLm, &pfc_control, ENABLEf, enable ? 0 : 1);
...
soc_ap_port_resource_tdm_calculate(int unit, soc_apache_tdm_t *tdm)
{
    ...
    PBMP_PORT_ITER(unit, port) {
        ...
        APACHE_TDM_SPEED_ADJUST(unit, port, tdm->speed[phy_port]);
...
soc_ap_port_macro_flex_enabled(int unit, int port_macro, int * flex_enabled)
{
    ...
    port_num = (APACHE_PORTS_PER_TSC * port_macro) + 1;
