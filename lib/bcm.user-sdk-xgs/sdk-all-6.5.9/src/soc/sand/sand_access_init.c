/** \file sand_access_init.c
 * Init common access data structures.
 *
 */
...
/* Macro for a block with multiple instances for block that use <block>_block */
#define SAND_MULTI_BLOCK_CASE(uc_block_name, lc_block_name) \
            case SOC_BLK_##uc_block_name: \
                if (blknum >= SOC_MAX_NUM_##uc_block_name##_BLKS) { \
                    SHR_ERR_EXIT(_SHR_E_INTERNAL, "soc_sand_info_config_blocks: too many " #uc_block_name " blocks\n"); \
                } \
                si->lc_block_name##_block[blknum] = blk; \
                si->block_port[blk] = blknum | SOC_REG_ADDR_INSTANCE_MASK; \
                break;

/* Macro for a block with multiple instances for block that use <block>_blocks */
...
/*
 * This functions sets the soc_info_t structure of the device,
 * which contains the information about the device's blocks.
 * nof_cores_to_broadcast is the number of block instances = cores to write to
 * in broadcast writes, if the device supports broadcast writes to per-core
 * block instances
 */
    ...
    uint8 nof_cores_to_broadcast)
{
    ...
    for (blk = 0; SOC_BLOCK_INFO(unit, blk).type >= 0; blk++) {
        ...
        if (blknum < 0) {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "soc_sand_info_config_blocks: illegal block instance number %d\n", blknum);
        }

        if (blk >= SOC_MAX_NUM_BLKS) {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "soc_sand_info_config_blocks: too many blocks for device\n");
        }
        ...
        switch (blktype)
        {
            ...
            SAND_MULTI_BLOCK_CASE(PM, pm)
            SAND_SINGLE_BLOCK_CASE(FDTL, fdtl)
            ...
            SAND_SINGLE_BLOCK_CASE(FDA, fda)
            SAND_SINGLE_BLOCK_CASE(FDT, fdt)
            ...
            SAND_SINGLE_BLOCK_CASE(OLP, olp)
            ...
            SAND_SINGLE_BLOCK_CASE(TAR, tar)
            ...
            SAND_SINGLE_BLOCK_CASE(GCK, gck)
            ...
            SAND_SINGLE_BLOCK_CASE(BRDC_FSRD, brdc_fsrd)
            SAND_SINGLE_BLOCK_CASE(BRDC_FMAC, brdc_fmac)
            ...
            SAND_SINGLE_BLOCK_CASE(MACT, mact)
            SAND_MULTIS_BLOCK_CASE(MCP, mcp)
            SAND_SINGLE_BLOCK_CASE(MDB, mdb)
            SAND_SINGLE_BLOCK_CASE(MTM, mtm)
