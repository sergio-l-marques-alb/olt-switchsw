...
#if defined(BCM_ESW_SUPPORT) || defined(BCM_SBX_SUPPORT) || defined(BCM_SAND_SUPPORT) || defined(BCM_CALADAN3_SUPPORT)
...
soc_reg_get(int unit, soc_reg_t reg, int port, int index, uint64 *data)
{
    ...
    if (SOC_IS_DONE_INIT(unit))
    {
        if (BCM_UNIT_DO_HW_READ_WRITE(unit))
        {
            if(Hw_Log_List[unit].Access_cb.soc_reg_get)
            {
                return Hw_Log_List[unit].Access_cb.soc_reg_get(unit, reg, port, index, data);
            }
        }
    }
#endif /* CRASH_RECOVERY_SUPPORT */
   /* Use user defined callback for access */
    ...
    if (SOC_IS_DNX_TEST_DEVICE(unit)
        ...
        return soc_dnxtestchip_reg_get(unit, reg, port, data);
    }
#endif /* DNX_TEST_CHIPS_SUPPORT */
...
#define SOC_REG_ADDR_INVALID_PORT 0 /* for asserts */
    ...
    if (!block_id_mask && !schan_id_mask && !instance_mask && port >= 0) {
        if (SOC_BLOCK_IN_LIST(regblktype, portblktype)) {
            if (preserve_port) {
                phy_port = port;
            } else if (phy_acc_mask) {
#ifdef BCM_JERICHO_SUPPORT
                if (SOC_IS_JERICHO(unit)) {
                    SOC_IF_ERROR_RETURN(MBCM_DPP_SOC_DRIVER_CALL(unit, mbcm_dpp_qsgmii_offsets_add, (unit, port, &phy_port))); 
                }
    ...
    if ((!block_id_mask) && (!schan_id_mask) && (REG_PORT_ANY == port || instance_mask || !SOC_BLOCK_IN_LIST(regblktype, portblktype))) {
		...
		case SOC_BLK_MDB_ARM:
            ...
            break;
		case SOC_BLK_PM:
			block = PM_BLOCK(unit, blkport);
            break;
        case SOC_BLK_FDTL:
            block = FDTL_BLOCK(unit);
            break;
        case SOC_BLK_ECI:
            block = ECI_BLOCK(unit);
            break;
    ...
    if(schan_id_mask) {
        ...
        if (SOC_IS_DNX(unit)) {
            ...
            access_info->num_blks = 2; 
            for (block_core = 0; block_core < access_info->num_blks; ++block_core) { 
                access_info->blk_list[block_core] = SOC_BLOCK_INFO(unit, soc_blocks[block_core]).cmic;
            }
...
#endif /* defined(BCM_ESW_SUPPORT) || defined(BCM_SIRIUS_SUPPORT) || defined(BCM_SAND_SUPPORT) || defined(BCM_CALADAN3_SUPPORT) || defined(PORTMOD_SUPPORT)*/
