...
#define _BCM_MOD_INIT_CER(unit, rtn, dispname, init) {                      \
    if (init) {                                                             \
        ...
        if (SOC_WARM_BOOT(unit)) {                                          \
            soc_cm_print("bcm_init: %8s   took %10d usec\n",                \
...
_bcm_modules_init(int unit)
{
    ...
    if (soc_feature(unit, soc_feature_l2gre)) {
        _BCM_MOD_INIT_IER(unit, bcm_esw_l2gre_init, "l2gre", init_cond);
    }
    if (soc_feature(unit, soc_feature_vxlan)) {
        _BCM_MOD_INIT_IER(unit, bcm_esw_vxlan_init, "vxlan", init_cond);
    }
    if (soc_feature(unit, soc_feature_port_extension)) {
        _BCM_MOD_INIT_IER(unit, bcm_esw_extender_init, "extender", init_cond);
    }
    ...
    /* Service meter */
    if (soc_feature(unit, soc_feature_global_meter)) {
        _BCM_MOD_INIT_IER(unit, bcm_esw_global_meter_init, "global_meter", TRUE);
    }
#endif /* BCM_KATANA_SUPPORT  or BCM_TRIUMPH3_SUPPORT */
#ifdef BCM_KATANA_SUPPORT
	...
	_BCM_MOD_INIT_IER(unit, bcm_esw_fcoe_init, "fcoe", init_cond);
    }
    if (SOC_IS_TRIDENT2(unit)) {
