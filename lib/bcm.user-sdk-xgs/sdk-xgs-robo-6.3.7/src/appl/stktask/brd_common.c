/* 
 * $Id: brd_common.c 1.21 Broadcom SDK $
...
_bcm_board_trunk_make(int unit, _trunk_info_t *trunk,
                      int start_tid, int status[BCM_TRUNK_MAX_PORTCNT])
{
    int tid_offset;
    int tid_available;
    int tid;
    ...
    tid_available = -1;
    for (tid_offset = 0; tid_offset < BCM_TRUNK_MAX_PORTCNT; tid_offset++) {
            ...
            if (BCM_FAILURE(rv)) {
                if (rv == BCM_E_NOT_FOUND) {
                    if (tid_available < 0) {
                        ...
                        TOPO_VERB(("Available trunk %d\n", tid));
                    } else {
                        TOPO_VERB(("Free trunk %d\n", tid));
                    }
                    status[tid_offset] = TRUNK_STATUS_FREE;
                } else if (rv == BCM_E_PARAM) {
                    TOPO_VERB(("Illegal trunk %d\n", tid));
                    break;
                } else {
                    TOPO_VERB(("Trunk %d error %d\n", tid, rv));
                    return rv;
                }
    ...
    if (tid_available < 0) {
        TOPO_VERB(("No free trunk was found\n"));
                      ...
                      int start, int flags)
{
    ...
    int trunk_status[BCM_TRUNK_MAX_PORTCNT];
    ...
    if (flags & BCM_BOARD_TRUNK_DESTROY) {
        ...
        if (tid != start) {
            TOPO_VERB(("TOPO: destroyed %d trunks\n", tid-start));
        }
        return BCM_E_NONE;
    }

    sal_memset(trunk_status, 0, sizeof(trunk_status));
        ...
        if (trunk.num_ports > 1) {
            if (bcm_board_trunk_callback_func) {
                BCM_IF_ERROR_RETURN(bcm_trunk_create(unit, BCM_TRUNK_FLAG_WITH_ID,
                                                     &tid));
                BCM_IF_ERROR_RETURN(bcm_board_trunk_callback_func(unit, tid,
                                    &trunk.info, trunk.num_ports, trunk.member,
                                    bcm_board_trunk_callback_cookie));
            } else {
                rv = _bcm_board_trunk_make(unit, &trunk, start, trunk_status);
    ...
    rv = _bcm_board_trunk_remove(unit, start, trunk_status);
    BCM_IF_ERROR_RETURN(rv);
    ...
    for (i = 0; i < entry->base.num_stk_ports; i++) {
        ...
        if (bcm_st_reserved_modid_enable_get() ||
            ...
            if (nh_tx_src_get(&nhmod, &nhport) < 0 || nhmod < 0) {
                TOPO_VERB(("TOPO: Could not get default NH src modid.\n"));
            }
        }
...
_bcm_board_xgs_stk_port_add(int funit, topo_cpu_t *tp_cpu, int *unitport)
{
    ...
    for (unit = 0; unit < l_entry->base.num_units; unit++) {
        ...
        if (bcm_stk_port_get(unit, BCM_STK_USE_HG_IF, &flags) < 0) {
            TOPO_WARN(("Could not get IPIC stack info for unit %d\n", unit));
            continue;
        }
        if (!(flags & BCM_STK_ENABLE)) {
                              ...
                              int num_connections)
{
    ...
    for (unit = 0; unit < num_connections; unit++) {
        ...
        if (bcm_stk_port_get(unit, sport, &flags) < 0) {
            TOPO_WARN(("Could not get stack info for (%d,%d)\n", unit, sport));
            continue;
        }
        if (!(flags & BCM_STK_ENABLE)) {
    ...
    if (!mirr_dir) {
        ...
        if (port == fail_sp) {
            if (BCM_FAILURE(bcm_mirror_to_set(unit, repl_sp))) {
                TOPO_ERR(("Cannot set MTP on unit %d, port %d\n",
                          unit, repl_sp));
                return BCM_E_FAIL;
            }
        }
