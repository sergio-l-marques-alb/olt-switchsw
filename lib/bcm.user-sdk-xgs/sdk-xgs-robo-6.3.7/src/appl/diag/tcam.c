...
cmd_tcam(int u, args_t * a)
{
    ...
    if (!SOC_IS_TRIUMPH3(u)) {
        ...
        type=0; subtype=0;
        if (tcam_info == NULL) {
            printk("TCAM info not found. Not initialized\n");
            return CMD_FAIL;
        }
    }
	...
	rv = soc_tcam_part_index_to_mem_index(u, part, index, mem, &mem_index);
        ...
        if((cmd == _PHY_PRBS_SET_CMD) || 
            ...
            for(lane=0; lane <= max_lane; ++lane) {
                if (poly <= 3) {
                    rv = wcmod_esm_serdes_control_set(u, 
                            ...
                            &poly);
                    if( rv != BCM_E_NONE) {
                        printk("Setting prbs polynomial failed on ESM SerDes\n");
                        return CMD_FAIL;
                    }
                    /* Tx enable */
                    rv = wcmod_esm_serdes_control_set(u, 
                            ...
                            &enable);
                    if( rv != BCM_E_NONE) {
                        printk("Setting prbs tx enable failed\n");
                        return CMD_FAIL;
                    }
                } else {
                    printk("Polynomial must be between 0 & 3, inclusive.\n");
                    return CMD_FAIL;
                }
            ...
            for(nl_dev_id=0; nl_dev_id < num_tcams; ++nl_dev_id) {
                ...
                if(rv < 0) {
                    printk("Enabling prbs checker on tcam(mdio_portid %d) failed \n",mdio_portid);
                    return CMD_FAIL;
                }
            }
            ...
            for(nl_dev_id=0; nl_dev_id < num_tcams; ++nl_dev_id) {
                ...
                if(rv < 0) {
                    printk("Enabling prbs generator on tcam(mdio_portid %d) failed \n",mdio_portid);
                    return CMD_FAIL;
                }
            }
            ...
            for(lane=12; lane <= max_lane; ++lane) {
                if (poly <= 3) {
                    rv = wcmod_esm_serdes_control_set(u, 
                            ...
                            &poly);
                    if( rv != BCM_E_NONE) {
                        printk("Enabling prbs checker on ESM SerDes failed\n");
                        return CMD_FAIL;
                    }
                    /* Tx enable */
                    rv = wcmod_esm_serdes_control_set(u, 
            ...
            for(nl_dev_id=0; nl_dev_id < num_tcams; ++nl_dev_id) {
                ...
                /* CRX lanes active for all tcams except the first
                   ...
                   line(tx) of serdes (nl_dev_id = 0)*/
                crx_lanes = nl_dev_id? 1 : 0;
                printk("CHECKING PRBS: NL Tcam (%d)\n", mdio_portid);
                rv = nl_mdio_prbs_chk_err(u, mdio_portid, crx_lanes);
                if(rv < 0) {
                    printk("Getting prbs status on tcam(mdio_portid %d) failed \n",mdio_portid);
                    return CMD_FAIL;
                }
            ...
            printk("CHECKING PRBS: TR3 ESM SerDes\n");
            max_lane = 23;
            for(lane=12; lane <= max_lane; ++lane) {
                rv = wcmod_esm_serdes_control_get(u, 
                        ...
                        &status);
                if( rv != BCM_E_NONE) {
                    printk("ESM SerDes: Getting prbs status failed (lane:%d) \n", lane);
                    return CMD_FAIL;
                }
                switch (status) {
                    case 0:
                        ...
                        break;
                    case 0xffffffff: /* -1 */
                        printk("\tPRBS not locked on lane %d of RX interface\n", lane - 12);
                        prbs_pass = 0;
                        break;
                    case 0xfffffffe: /* -2 */
                        printk("\tPRBS lost lock on lane %d of RX interface\n", lane - 12);
                        prbs_pass = 0;
                        break;
                    default:
                        printk("\t%d PRBS errors seen on lane %d of RX interface\n", status, lane - 12);
                        prbs_pass = 0;
                        break;
                }
            }         
        ...
        if (SOC_IS_TRIUMPH3(u)) {
            if (soc_feature(u, soc_feature_esm_support) && SOC_CONTROL(u)->tcam_info) {
                ...
                printk("Calculating, please wait...");
                rv = hwtl_find_idle_req_rsp_latency(u, ptr_fme_req, 30, &latency_ns);
                
                if( rv != BCM_E_NONE) {
                    printk("Get external tcam request response latency failed\n");
                    return CMD_FAIL;
                } else {
