/*
 * $Id: linux-user-bde.c 1.103.6.3 Broadcom SDK $
...
#if defined(__DUNE_GTO_BCM_CPU__) && defined(INCLUDE_I2C)
#define DUNE_GTO_I2C
#endif
...
typedef struct bde_dev_s {
    ...
    unsigned int       *vbase1;         /* Secondary virtual base address #1 */
    unsigned int       *vbase2;         /* Secondary virtual base address #2 */
#ifdef DUNE_GTO_I2C
            ...
            if (_ioctl(LUBDE_DEV_RESOURCE, &devio) == 0) {
                ...
                if (pbase) {
                    ...
                    for (j = 0; j < IPROC_SUBWIN_MAX; j++) {
                        ...
                        if (rval & 1) { /* Valid Bit */
                            iproc_map[0]->subwin[j].addr_min = rval & ~(0xfff);
                            iproc_map[0]->subwin[j].addr_max = rval | 0xfff;
                        } else {
                            iproc_map[0]->subwin[j].addr_min = 0;
                            iproc_map[0]->subwin[j].addr_max = 0;
                        }
                    }
...
static uint32
_iproc_offset(uint32 map_no, int d, uint32 addr)
{
    _iproc_map_t *map;
...
_iproc_read(int d, unsigned int addr)
{
    ...
    return _devices[d]->vbase1[addr/sizeof(uint32)];
}
...
_cpu_write(int d, uint32 addr, uint32 *buf)
{
    ...
    if (_ioctl(LUBDE_CPU_WRITE_REG, &_devio) != 0) {
        printf("linux-user-bde: warnning: _cpu_write failed unit=%d addr=0x%x, buf=%p, *buf=0x%x", d, addr, buf, *buf);
        return -1;
    }
