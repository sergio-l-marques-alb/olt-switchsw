...
int vlan_port_create(int port, int vlan_ing, int inner_vlan_ing, int vlan_egr, int inner_vlan_egr, bcm_vlan_port_t *vp)
{
 ...
 if (!advanced_vlan_editing)
 {
  ...
  if (vlan_egr > 0 && vlan_egr < 4096)
  {
   ...
   if (rv != BCM_E_NONE) {
    printf("Error, in bcm_vlan_translate_egress_action_add of vp: rv=%d\n", rv);
    return rv;
   }
  }
 }
...
int vlan_port_destroy(unsigned int vlan_port_id)
{
 ...
 if (!advanced_vlan_editing)
 {
  ...
  if (rv != BCM_E_NONE && rv != BCM_E_EXISTS) {
   printf("Error, in bcm_vlan_translate_egress_action_delete: rv=%d\n", rv);
   return rv;
  }
 }
...
int vlan_port_find(unsigned int vlan_port_id, bcm_vlan_port_t *vp)
{
 ...
 if (rv != BCM_E_NONE) {
  printf("Error, bcm_vlan_port_find: rv=%d\n", rv);
  return rv;
 }
...
int vswitch_add(int port, int vlan, int inner_vlan, int vsi)
{
 ...
 if (vsi < 4096)
 {
  ...
  if (rv != BCM_E_NONE && rv != BCM_E_EXISTS) {
   printf("Error, in vlan_add(vsi=%u, port=%u): rv=%d\n", vsi, port, rv);
   return rv;
  }
 }
 ...
 if (rv != BCM_E_NONE && rv != BCM_E_EXISTS) {
  printf("Error creating vlan_port for port %u: rv=%d\n", port, rv);
  return rv;
 }
 ...
 if (rv != BCM_E_NONE)
 {
  printf("error: bcm_port_learn_set for vlan_port_id 0x%x: rv=%d\n", vp.vlan_port_id, rv);
  return rv;
 }
...
int vswitch_remove(int vlan_port_id, int vsi)
{
 ...
 if (rv != BCM_E_NONE && rv != BCM_E_EXISTS) {
  printf("Error destroying vlan_port %u: rv=%d\n", vlan_port_id, rv);
  return rv;
 }
...
int crossconnect_add(int port1, int port2, int vlan1, int vlan2, int inner_vlan1, int inner_vlan2)
{
 ...
 if (rv != BCM_E_NONE && rv != BCM_E_EXISTS) {
  printf("Error, in vlan_add of port %u to vlan %d: rv=%d\n", port1, vlan1, rv);
  return rv;
 }
...
int crossconnect_remove(int vlan_port_1, int vlan_port_2)
{
 ...
 if (rv != BCM_E_NONE) {
  printf("Error, vlan_port_find(&vp1): rv=%d\n", rv);
  return rv;
 }
 rv = vlan_port_find(vlan_port_2, &vp2);
 if (rv != BCM_E_NONE) {
  printf("Error, vlan_port_find(&vp2): rv=%d\n", rv);
  return rv;
 }
 ...
 if (rv != BCM_E_NONE && rv != BCM_E_EXISTS) {
  printf("Error destroying vlan_port 0x%x: rv=%d\n", vlan_port_1, rv);
  return rv;
 }
 ...
 if (rv != BCM_E_NONE) {
  printf("Error, vlan_remove port1: rv=%d\n", rv);
  return rv;
 }
 rv = vlan_remove(vp2.match_vlan, vp2.port);
 if (rv != BCM_E_NONE) {
  printf("Error, vlan_remove port2: rv=%d\n", rv);
  return rv;
 }
