...
void init_outer_label(int unit, bcm_mpls_egress_label_t* label_conf,
    ...
    label_conf->exp = ext_mpls_exp;
    label_conf->flags = (BCM_MPLS_EGRESS_LABEL_TTL_SET|BCM_MPLS_EGRESS_LABEL_TTL_DECREMENT);
...
mpls__create_tunnel_initiator__set(int unit, mpls__egress_tunnel_utils_s *mpls_tunnel_properties) {
    ...
    if (mpls_tunnel_properties->label_out>0) {
        ...
        init_outer_label(unit, &label_array[1], mpls_tunnel_properties, mpls_tunnel_properties->label_out);
        num_labels++;

        if (mpls_tunnel_properties->label_out_2>0) {

            init_outer_label(unit, &label_array[2], mpls_tunnel_properties, mpls_tunnel_properties->label_out_2);
            num_labels++;

            if (mpls_tunnel_properties->label_out_3>0) {

                init_outer_label(unit, &label_array[3], mpls_tunnel_properties, mpls_tunnel_properties->label_out_3);
                num_labels++;
