  ...
  )
{
  ...
  res = soc_petra_sch_device_if_weight_idx_set(unit, 0, weight_index);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);

exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in soc_petra_sch_device_if_weight_idx_set_dispatch()",0,0);
}
/*********************************************************************
*     Sets, for a specified device interface, (NIF-Ports,
*     recycling, OLP, ERP) its weight index. Range: 0-7. The
  ...
  )
{
  ...
  *weight_index = 0;
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in soc_petra_sch_device_if_weight_idx_get_dispatch()",0,0);
}
