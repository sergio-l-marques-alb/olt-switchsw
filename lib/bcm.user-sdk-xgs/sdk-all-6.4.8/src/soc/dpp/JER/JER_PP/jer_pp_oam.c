...
soc_error_t soc_jer_pp_oam_init(int unit)
{
    ...
    */
    ...
    rv = WRITE_IHP_OAM_MP_TYPE_MAPr_REG64(unit,SOC_CORE_ALL,reg_64);
    SOCDNX_IF_ERR_EXIT(rv);
    ...
    SOC_DPP_CORES_ITER(SOC_CORE_ALL, core) {
		...
		rv = READ_IHP_FLP_GENERAL_CFGr_REG64(unit,core,&reg_64);
		SOCDNX_IF_ERR_EXIT(rv);
		...
		soc_reg64_field32_set(unit,IHP_FLP_GENERAL_CFGr,&reg_64,EGRESS_NON_OAM_PACKET_SUB_TYPEf,0 );
		rv = WRITE_IHP_FLP_GENERAL_CFGr_REG64(unit,core,reg_64);
		SOCDNX_IF_ERR_EXIT(rv);
  ...
  )
{
    ...
    if (!update_action_only) {
        if (mep_index >= ARAD_PP_OAM_DEFAULT_EP_EGRESS_0) {
            ...
            SOC_DPP_CORES_ITER(SOC_CORE_ALL, core) {
                /* Set counter index for all OAM-lif-profiles*/
                SOCDNX_IF_ERR_EXIT(READ_IHP_OAM_DEFAULT_COUNTERSr(unit, core, reg_above_64));
                for (eg_mep_idx = COUNTER_FOR_EGRESS_PROFILE_0f; eg_mep_idx <= COUNTER_FOR_EGRESS_PROFILE_3f; eg_mep_idx++) {
                    soc_reg_above_64_field32_set(unit, IHP_OAM_DEFAULT_COUNTERSr, reg_above_64, eg_mep_idx, classifier_mep_entry->counter);
                }
                ...
                for (eg_mep_idx=ARAD_PP_OAM_DEFAULT_EP_EGRESS_0; eg_mep_idx<=ARAD_PP_OAM_DEFAULT_EP_EGRESS_3; eg_mep_idx++) {
                    JER_PP_OAM_DEFAULT_EGRESS_MEP_MDL_SET(eg_mep_idx, classifier_mep_entry->md_level, reg64);
                }
  ...
  )
{
    ...
    eth1731_profile->report_mode  = soc_OAMP_MEP_PROFILEm_field32_get(unit, profile_entry, REPORT_MODEf);
    ...
    for (oam1_key.ingress = 0; oam1_key.ingress <= 1; ++oam1_key.ingress) {
        ...
        for (oam1_key.my_cfm_mac = 0; oam1_key.my_cfm_mac <= 1; ++oam1_key.my_cfm_mac) {
            for (oam1_key.inject = 0; oam1_key.inject <= 1; ++oam1_key.inject) {
                for (oam1_key.opcode = 0; oam1_key.opcode < SOC_PPD_OAM_OPCODE_MAP_COUNT; ++oam1_key.opcode) {
                    for (oam1_key.mp_type_jr = 0; oam1_key.mp_type_jr < SOC_PPC_OAM_MP_TYPE_JERICHO_COUNT; ++oam1_key.mp_type_jr) {
                        /* Map key->payload */
                        soc_sand_rv = arad_pp_oam_classifier_oam1_entry_set_on_buffer(unit, &oam1_key, &oam_payload, &oama_buffer);
                        SOCDNX_SAND_IF_ERR_RETURN(soc_sand_rv);
                    }
...
{
     int offset = byte_start_offset;
     uint8 opcode = is_lbm? SOC_PPD_OAM_ETHERNET_PDU_OPCODE_LBM: 
     ...
     SOC_SAND_OUT int *header_offset)
{
    ...
    if (next_is_itmh) {
     ...
     if(soc_property_get(unit, spn_ITMH_PROGRAMMABLE_MODE_ENABLE, FALSE) ==0 ){
       ...
       offset = _insert_itmh_for_tst_lb_arad(header_buffer, offset, fwd_dst_info, tc, dp, 0, 0, (mep_db_entry.mep_type==SOC_PPD_OAM_MEP_TYPE_Y1731_O_MPLSTP  || mep_db_entry.mep_type==SOC_PPD_OAM_MEP_TYPE_Y1731_O_PWE));
     }
     else{/*jericho Style*/
         if (mep_db_entry.mep_type == SOC_PPD_OAM_MEP_TYPE_ETH_OAM) {
         ...
         offset = _insert_itmh_for_tst_lb_jericho(header_buffer, offset, fwd_dst_info, tc, dp, 0, 0, (mep_db_entry.mep_type==SOC_PPD_OAM_MEP_TYPE_Y1731_O_MPLSTP  || mep_db_entry.mep_type==SOC_PPD_OAM_MEP_TYPE_Y1731_O_PWE));
     }
    }
    
    if (mep_db_entry.mep_type == SOC_PPD_OAM_MEP_TYPE_ETH_OAM) {/*Prepare Ethernet header */
    ...
    offset = _insert_eth_for_tst_lb(header_buffer, offset,peer_d_mac, gtf_smac, tpids,vlans, mep_db_entry.tags_num,ether_type);
     ...
     exp = (reg_field[0] & 0x700) >> 8 ; /* next 3 bits*/
      ...
      if (mep_db_entry.mep_type == SOC_PPD_OAM_MEP_TYPE_Y1731_O_MPLSTP) {
         ...
         offset= _insert_4_bytes_for_tst_lb(header_buffer ,gal_gach,offset);
      } else if(mep_db_entry.mep_type == SOC_PPD_OAM_MEP_TYPE_Y1731_O_PWE){
