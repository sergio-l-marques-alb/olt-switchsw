   ...
   )
{
    ...
    {   
        ...
        if(interface == SOC_PORT_IF_ILKN) {
            ...
            SOCDNX_IF_ERR_EXIT(soc_jer_port_ilkn_nif_port_get(unit, protocol_offset, &nif_port));
        ...
        /* TDM ports have their own contexts */
   ...
   )
{
    ...
    uint32 offset, channel, if_rate_mbps, otm_port_number, ifc_delay, ifc_val;
    ...
    if(interface != SOC_PORT_IF_ERP) {
        ...
        ifc_val = interface == SOC_PORT_IF_QSGMII ? 1 : 0;
   ...
   )
{
    ...
    uint32 base_q_pair, base_q_pair_i, reg32[1], if_rate_mbps, higher_req_en;
  ...
  )
{
    ...
    /* set or clear TDM configuration */    
    if ((SOC_PORT_IS_NETWORK_INTERFACE(flags)) || (interface_type == SOC_PORT_IF_CPU)) {
        ...
        if (interface_type == SOC_PORT_IF_CPU) {
            ...
            if (interface_type == SOC_PORT_IF_ILKN) {
                ...
                if (core_id) {
                    reg = NBIH_RX_REQ_PIPE_1_TDM_ENr;
                } else {
                    reg = NBIH_RX_REQ_PIPE_0_TDM_ENr;
                }
                SOCDNX_IF_ERR_EXIT(soc_reg32_get(unit, reg, REG_PORT_ANY, 0, reg32_val));

                bit_val = 0;
                SOCDNX_IF_ERR_EXIT(soc_port_sw_db_ports_to_same_interface_get(unit, logical_port, &pbmp));
                SOC_PBMP_ITER(pbmp, port_i) {
                ...
                case 0:
                    SOCDNX_IF_ERR_EXIT(soc_reg_field32_modify(unit, NBIH_RX_SCH_CONFIG_FOR_TDM_HRFSr, REG_PORT_ANY, HRF_RX_PIPE_2f, core_id));
                    SHR_BITWRITE(reg32_val, 1, bit_val);
                    SHR_BITWRITE(reg32_val, 18, bit_val);
                    SHR_BITWRITE(reg32_val, 19, bit_val);
                    break;
                case 1:
                    SOCDNX_IF_ERR_EXIT(soc_reg_field32_modify(unit, NBIH_RX_SCH_CONFIG_FOR_TDM_HRFSr, REG_PORT_ANY, HRF_RX_PIPE_3f, core_id));
                    SHR_BITWRITE(reg32_val, 4, bit_val);
                    SHR_BITWRITE(reg32_val, 20, bit_val);
                    SHR_BITWRITE(reg32_val, 21, bit_val);
                    break;
                case 2:
                    SOCDNX_IF_ERR_EXIT(soc_reg_field32_modify(unit, NBIH_RX_SCH_CONFIG_FOR_TDM_HRFSr, REG_PORT_ANY, HRF_RX_PIPE_6f, core_id));
                    SHR_BITWRITE(reg32_val, 7, bit_val);
                    SHR_BITWRITE(reg32_val, 22, bit_val);
                    SHR_BITWRITE(reg32_val, 23, bit_val);
                    break;
                case 3:
                    SOCDNX_IF_ERR_EXIT(soc_reg_field32_modify(unit, NBIH_RX_SCH_CONFIG_FOR_TDM_HRFSr, REG_PORT_ANY, HRF_RX_PIPE_7f, core_id));
                    SHR_BITWRITE(reg32_val, 10, bit_val);
                    SHR_BITWRITE(reg32_val, 24, bit_val);
                    SHR_BITWRITE(reg32_val, 25, bit_val);
                    break;
                case 4:
                    SOCDNX_IF_ERR_EXIT(soc_reg_field32_modify(unit, NBIH_RX_SCH_CONFIG_FOR_TDM_HRFSr, REG_PORT_ANY, HRF_RX_PIPE_10f, core_id));
                    SHR_BITWRITE(reg32_val, 13, bit_val);
                    SHR_BITWRITE(reg32_val, 26, bit_val);
                    SHR_BITWRITE(reg32_val, 27, bit_val);
                    break;
                case 5:
                    SOCDNX_IF_ERR_EXIT(soc_reg_field32_modify(unit, NBIH_RX_SCH_CONFIG_FOR_TDM_HRFSr, REG_PORT_ANY, HRF_RX_PIPE_11f, core_id));
                    SHR_BITWRITE(reg32_val, 16, bit_val);
                    SHR_BITWRITE(reg32_val, 28, bit_val);
                    SHR_BITWRITE(reg32_val, 29, bit_val);
                    break;
                default:
                    SOCDNX_EXIT_WITH_ERR(SOC_E_CONFIG, (_BSL_SOCDNX_MSG("Invalid ILKN offset")));
                }
                SOCDNX_IF_ERR_EXIT(soc_reg32_set(unit, reg, REG_PORT_ANY, 0, *reg32_val));
            } else {
...
soc_jer_port_first_link_in_fsrd_get(int unit, soc_port_t port, int *is_first_link, int enable)
{
    ...
    for (i=fsrd_block*SOC_DPP_DEFS_GET(unit, nof_links_in_fsrd) ; i<fsrd_block*SOC_DPP_DEFS_GET(unit, nof_links_in_fsrd) + SOC_DPP_DEFS_GET(unit, nof_links_in_fsrd); i++) /* create a mask for all the links in the fsrd*/
    {
