...
#define JER_ITM_MC_HP_FIFO_DP_0_THRESHOLD(limit)                6
#define JER_ITM_MC_HP_FIFO_DP_1_THRESHOLD(limit)                (6 + ((limit) - 6) / 4)
#define JER_ITM_MC_HP_FIFO_DP_2_THRESHOLD(limit)                (6 + ((limit) - 6) / 2)
#define JER_ITM_MC_HP_FIFO_DP_3_THRESHOLD(limit)                (6 + ((limit) - 6) * 3 / 4)
#define JER_ITM_MC_LP_FIFO_DP_0_THRESHOLD(limit)                6
#define JER_ITM_MC_LP_FIFO_DP_1_THRESHOLD(limit)                (6 + ((limit) - 6) / 4)
#define JER_ITM_MC_LP_FIFO_DP_2_THRESHOLD(limit)                (6 + ((limit) - 6) / 2)
#define JER_ITM_MC_LP_FIFO_DP_3_THRESHOLD(limit)                (6 + ((limit) - 6) * 3 / 4)
  ...
  )
{
  ...
  if (SOC_DPP_CONFIG(unit)->jer->tm.cgm_mgmt_guarantee_mode == SOC_TMC_ITM_CGM_MGMT_GUARANTEE_LOOSE) {
      ...
      soc_reg_field_set(unit, IQM_STE_ENABLERSr, &data, LOSSY_BLOCK_STRICTf, TRUE);
      soc_reg_field_set(unit, IQM_STE_ENABLERSr, &data, LOSSLESS_BLOCK_STRICTf, TRUE);
  ...
  /*
   * VOQ guaranteed reject test mask bitmap - Enable all in strict mode
  ...
  SOCDNX_IF_ERR_EXIT(res);
  if (SOC_DPP_CONFIG(unit)->jer->tm.cgm_mgmt_guarantee_mode == SOC_TMC_ITM_CGM_MGMT_GUARANTEE_LOOSE) {
      /*mask out: bit 7 - Mask reject test based on queue WRED*/
      soc_reg64_field32_set(unit, IQM_RJCT_GLBL_TESTS_MASKr, &reg64, VOQ_GRNTD_RJCT_MASKf, (0x1 << 7));
      soc_reg64_field32_set(unit, IQM_RJCT_GLBL_TESTS_MASKr, &reg64, VSQ_GRNTD_RJCT_MASKf, (0x1 << 7));
      soc_reg64_field32_set(unit, IQM_RJCT_GLBL_TESTS_MASKr, &reg64, VSQ_LOSSLESS_RJCT_MASKf, (0x1 << 7));
      soc_reg64_field32_set(unit, IQM_RJCT_GLBL_TESTS_MASKr, &reg64, ECN_RJCT_MASKf, (0x1 << 7));
  } else if (SOC_DPP_CONFIG(unit)->jer->tm.cgm_mgmt_guarantee_mode == SOC_TMC_ITM_CGM_MGMT_GUARANTEE_STRICT) {
      soc_reg64_field32_set(unit, IQM_RJCT_GLBL_TESTS_MASKr, &reg64, VOQ_GRNTD_RJCT_MASKf, 0);
      soc_reg64_field32_set(unit, IQM_RJCT_GLBL_TESTS_MASKr, &reg64, VSQ_GRNTD_RJCT_MASKf, 0);
      soc_reg64_field32_set(unit, IQM_RJCT_GLBL_TESTS_MASKr, &reg64, VSQ_LOSSLESS_RJCT_MASKf, 0);
      soc_reg64_field32_set(unit, IQM_RJCT_GLBL_TESTS_MASKr, &reg64, ECN_RJCT_MASKf, 0);
  }
  res = WRITE_IQM_RJCT_GLBL_TESTS_MASKr(unit, SOC_CORE_ALL, reg64);
  SOCDNX_IF_ERR_EXIT(res);

  /*
   * VOQ guaranteed reject test mask bitmap
   *               bit 0 - Mask VSQ-A max Q-Size test
   *               bit 1 - Mask VSQ-B max Q-Size test - only in strict mode.
   *               bit 2 - Mask VSQ-C max Q-Size test - only in strict mode.
   *               bit 3 - Mask VSQ-D max Q-Size test - only in strict mode.
   *               bit 4 - Mask VSQ-E max Q-Size test - only in strict mode.
   *               bit 5 - Mask VSQ-F max Q-Size test - only in strict mode.
   *
   */
  COMPILER_64_ZERO(reg64);
  ...
  SOCDNX_IF_ERR_EXIT(res);
  if (SOC_DPP_CONFIG(unit)->jer->tm.cgm_mgmt_guarantee_mode == SOC_TMC_ITM_CGM_MGMT_GUARANTEE_LOOSE) {
      /*mask out: bits 1-5: VSQB-VSQF max Q-size test*/
      soc_reg64_field32_set(unit, IQM_STE_RJCT_TESTS_MASKr, &reg64, VOQ_GRNTD_VQ_MX_SIZE_RJCT_MASKf, 0x3E);
      soc_reg64_field32_set(unit, IQM_STE_RJCT_TESTS_MASKr, &reg64, VSQ_GRNTD_VQ_MX_SIZE_RJCT_MASKf, 0x3E);
      soc_reg64_field32_set(unit, IQM_STE_RJCT_TESTS_MASKr, &reg64, VSQ_LOSSLESS_VQ_MX_SIZE_RJCT_MASKf, 0x3E);
      soc_reg64_field32_set(unit, IQM_STE_RJCT_TESTS_MASKr, &reg64, ECN_VQ_MX_SIZE_RJCT_MASKf, 0x3E);
      soc_reg64_field32_set(unit, IQM_STE_RJCT_TESTS_MASKr, &reg64, VOQ_GRNTD_VQ_WRED_RJCT_MASKf, 0x3E);
      soc_reg64_field32_set(unit, IQM_STE_RJCT_TESTS_MASKr, &reg64, VSQ_GRNTD_VQ_WRED_RJCT_MASKf, 0x3E);
      soc_reg64_field32_set(unit, IQM_STE_RJCT_TESTS_MASKr, &reg64, VSQ_LOSSLESS_VQ_WRED_RJCT_MASKf, 0x3E);
      soc_reg64_field32_set(unit, IQM_STE_RJCT_TESTS_MASKr, &reg64, ECN_VQ_WRED_RJCT_MASKf, 0x3E);
  } else if (SOC_DPP_CONFIG(unit)->jer->tm.cgm_mgmt_guarantee_mode == SOC_TMC_ITM_CGM_MGMT_GUARANTEE_STRICT) {
      soc_reg64_field32_set(unit, IQM_STE_RJCT_TESTS_MASKr, &reg64, VOQ_GRNTD_VQ_MX_SIZE_RJCT_MASKf, 0);
      soc_reg64_field32_set(unit, IQM_STE_RJCT_TESTS_MASKr, &reg64, VSQ_GRNTD_VQ_MX_SIZE_RJCT_MASKf, 0);
      soc_reg64_field32_set(unit, IQM_STE_RJCT_TESTS_MASKr, &reg64, VSQ_LOSSLESS_VQ_MX_SIZE_RJCT_MASKf, 0);
      soc_reg64_field32_set(unit, IQM_STE_RJCT_TESTS_MASKr, &reg64, ECN_VQ_MX_SIZE_RJCT_MASKf, 0);
      soc_reg64_field32_set(unit, IQM_STE_RJCT_TESTS_MASKr, &reg64, VOQ_GRNTD_VQ_WRED_RJCT_MASKf, 0);
      soc_reg64_field32_set(unit, IQM_STE_RJCT_TESTS_MASKr, &reg64, VSQ_GRNTD_VQ_WRED_RJCT_MASKf, 0);
      soc_reg64_field32_set(unit, IQM_STE_RJCT_TESTS_MASKr, &reg64, VSQ_LOSSLESS_VQ_WRED_RJCT_MASKf, 0);
      soc_reg64_field32_set(unit, IQM_STE_RJCT_TESTS_MASKr, &reg64, ECN_VQ_WRED_RJCT_MASKf, 0);
  }
  ...
  SOCDNX_SAND_IF_ERR_EXIT(arad_init_pdm_nof_entries_calc(unit, SOC_DPP_CONFIG(unit)->arad->init.dram.pdm_mode, &pdm_nof_entries));
  SOC_DPP_CORES_ITER(SOC_DPP_CORE_ALL(unit), core_index) {
      glbl_drop.bdbs[0].set       = 128;
      glbl_drop.bdbs[0].clear     = 512;

      glbl_drop.ocb_bdbs[0].set       = 128;
      glbl_drop.ocb_bdbs[0].clear     = 512;
      for (idx = 1; idx < ARAD_NOF_DROP_PRECEDENCE; idx++)
      {
        glbl_drop.bdbs[idx].set   = 256;
        glbl_drop.bdbs[idx].clear = 1024;

        glbl_drop.ocb_bdbs[idx].set   = 256;
        glbl_drop.ocb_bdbs[idx].clear = 1024;
      }
      for (idx = 0; idx < SOC_TMC_NOF_DROP_PRECEDENCE; idx++)
      {
        glbl_drop.bds[idx].set   = (1008 * (pdm_nof_entries / 1024)) / SOC_DPP_CONFIG(unit)->core_mode.nof_active_cores;
        glbl_drop.bds[idx].clear = (928 * (pdm_nof_entries / 1024)) / SOC_DPP_CONFIG(unit)->core_mode.nof_active_cores; 

        glbl_drop.ocb_bds[idx].set   = (((core_index == 0) ? dbuffs_bdries->ocb_0.size : dbuffs_bdries->ocb_1.size) * 1008) / 1024; 
        glbl_drop.ocb_bds[idx].clear = (((core_index == 0) ? dbuffs_bdries->ocb_0.size : dbuffs_bdries->ocb_1.size) * 928) / 1024; 
      }
...
*/
uint32 jer_itm_congestion_statistics_get(
  SOC_SAND_IN int unit,
  SOC_SAND_IN int core,
...
Get Jericho ingress congestion statistics.
*/
uint32 qax_itm_congestion_statistics_get(
  SOC_SAND_IN int unit,
  SOC_SAND_IN int core,
  SOC_SAND_OUT ARAD_ITM_CGM_CONGENSTION_STATS *stats /* place current statistics output here */
  )
