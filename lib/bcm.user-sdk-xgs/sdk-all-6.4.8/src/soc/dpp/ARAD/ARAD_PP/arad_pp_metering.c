...
/*
	Convert bucket level from 23bit 2s compliment to integer
*/
	  ...
	  SOC_SAND_IN int unit)
{
	...
	for (addr_idx = 0; addr_idx < ARAD_PP_MTR_PCD_OPT_NUM; addr_idx++) {
		...
		if (mcda_mtr_color == ARAD_PP_MTR_PCD_COL_INVALID) {
			...
			final_color = mcda_update_color = mcdb_update_color;
		} 
  ...
  )
{
  ...
  if (SOC_IS_ARADPLUS(unit)){
	  if (!SOC_IS_JERICHO(unit)) {
		  ...
		  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res, __LINE__, exit, ARAD_REG_ACCESS_ERR, soc_reg_above_64_field32_modify(unit, IDR_SPARE_REGISTER_2r, REG_PORT_ANY, 0, IPG_COMPENSATION_ENf, mtr_group_info->is_ipg_compensation_enabled));
	  }
	  else{
		  /* 
  ...
  )
{
  ...
  SOC_SAND_ERR_IF_BELOW_MIN(policer_info->cir, 
							policer_info->is_packet_mode ? ARAD_PP_POLICER_PROFILE_ADD_VERIFY_IR_MIN * ARAD_PP_MTR_PACKET_MODE_CORRECTION 
							: ARAD_PP_POLICER_PROFILE_ADD_VERIFY_IR_MIN,
							ARAD_PP_METERING_CIR_OUT_OF_RANGE_ERR, 40, exit); 
  ...
  SOC_SAND_ERR_IF_BELOW_MIN(policer_info->cbs, policer_info->is_packet_mode ? ARAD_PP_BW_PROFILE_ADD_VERIFY_BS_MIN * ARAD_PP_MTR_PACKET_MODE_CORRECTION
							: ARAD_PP_BW_PROFILE_ADD_VERIFY_BS_MIN,
							ARAD_PP_METERING_CBS_OUT_OF_RANGE_ERR, 60, exit);
