...
#if (defined(LINUX))
      bcm_port_loopback_set(nUnit, nPort, BCM_PORT_LOOPBACK_PHY);
#else
      bcm_port_loopback_set(nUnit, nPort, BCM_PORT_LOOPBACK_MAC);
#endif
    }
  }
  ...
  if ( !SAL_BOOT_QUICKTURN && nField ) {
    ...
    */
    ...
    if ( pIlControlInfo->bExternalLoopback == 0 && pIlControlInfo->bL1Loopback == 0 ) {
      bcm_port_loopback_set(nUnit, nWCport, BCM_PORT_LOOPBACK_PHY);
    }
