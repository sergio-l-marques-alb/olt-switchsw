...
#define LPORT_TAB_SYNC(_u_, _p_, _fld_, _val_)                              \
    ...
    if (SOC_MEM_FIELD_VALID(unit, PORT_TABm, SRC_SYS_PORT_IDf)) {
        soc_PORT_TABm_field32_set(unit, &ptab, SRC_SYS_PORT_IDf, port);
    ...
    if (soc_feature(unit, soc_feature_separate_ing_lport_rtag7_profile)) {
        ...
        rtag7_profile_idx =
            ((soc_mem_index_max(unit, RTAG7_PORT_BASED_HASHm) -
              soc_mem_index_max(unit, PORT_TABm)) + port);
        soc_PORT_TABm_field32_set(unit, &ptab, RTAG7_PORT_PROFILE_INDEXf, rtag7_profile_idx);
    }
#endif
    ...
    if (lp_field_count > 0) {
        BCM_IF_ERROR_RETURN
            (bcm_esw_port_lport_fields_set(unit, port, LPORT_PROFILE_LPORT_TAB, 
                                           lp_field_count, lp_field_ids, lp_field_values));
    }
    ...
    if (soc_feature(unit, soc_feature_egr_vlan_control_is_memory)) {
        if (soc_feature(unit, soc_feature_hgproxy_subtag_coe) &&
            ...
            BCM_IF_ERROR_RETURN(
                ...
                unit, port, EGR_LPORT_PROFILE_VLAN_CONTROL_1,
                field, value));
        } else {
    ...
    if (soc_feature(unit, soc_feature_td2p_style_egr_outer_tpid)) {
        if (soc_feature(unit, soc_feature_hgproxy_subtag_coe) &&
            ...
            rv = bcm_esw_port_egr_lport_field_set(
                unit, port, EGR_LPORT_PROFILE_PORT_1,
