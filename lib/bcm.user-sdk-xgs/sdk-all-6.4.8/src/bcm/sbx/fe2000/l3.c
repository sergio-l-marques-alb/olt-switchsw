...
STATIC int
_bcm_fe2000_l3_intf_alloc_l2_ete(_fe2k_l3_fe_instance_t *l3_fe,
                                uint32                 *reserve_etes,
...
int
_bcm_fe2000_l3_egress_dmac_update(_fe2k_l3_fe_instance_t *l3_fe,
...
l3_wb_store_intf(_fe2k_l3_fe_instance_t *l3_fe, _fe2k_l3_intf_t *l3_intf)
{
    ...
    LOG_VERBOSE(BSL_LS_BCM_L3,
                (BSL_META("if=%d base_ete=0x%x\n"),
                 l3_intf->if_info.l3a_intf_id, l3_intf->if_l2_ete[0].ete_idx));

    if (l3_intf->if_info.l3a_intf_id >= L3_RCVR_MAX_IFID) {
        LOG_ERROR(BSL_LS_BCM_L3,
                  (BSL_META("Insufficient space for %d interface ids"
                       ...
                       fe2k_l3_wb_mem_layout_t *layout)
{
    ...
    LOG_VERBOSE(BSL_LS_BCM_L3,
                (BSL_META_U(unit,
                            "recovering and reconstructing interface ids\n")));
    for (idx = 0; idx < L3_RCVR_MAX_IFID; idx++) {
        if (layout->rif[idx].ete[0]) {
            ...
            if (BCM_FAILURE(rv)) {
                LOG_ERROR(BSL_LS_BCM_L3,
                          (BSL_META_U(l3_fe->fe_unit,
                                      "Failed to recover (add) intf 0x%x:%s\n"),
                           idx, bcm_errmsg(rv)));
                return rv;
            }
                        ...
                        _fe2k_l3_ete_t         **sw_ete)
{
    ...
    if (l3_sw_ete->l3_l2encap.ete_idx.ete_idx != _FE2K_INVALID_ETE_IDX) {
        ...
        if (status != BCM_E_NONE) {
            LOG_ERROR(BSL_LS_BCM_L3,
                      (BSL_META_U(l3_fe->fe_unit,
                                  "error(%s) could not free L2 ETE %d\n"),
                       bcm_errmsg(status),
                       l3_sw_ete->l3_l2encap.ete_idx.ete_idx));
            return status;
        }
    ...
    if (_FE2K_ETE_TYPE_NEEDS_OHI(l3_sw_ete->l3_ete_key.l3_ete_hk.type)) {
        ...
        case  SOC_SBX_UCODE_TYPE_G2P3:
        {
            ...
            if (status != BCM_E_NONE) {
                LOG_ERROR(BSL_LS_BCM_L3,
                          (BSL_META_U(l3_fe->fe_unit,
                                      "error %s in soc_sbx_g2p3_oi2e_set"),
                           bcm_errmsg(status)));
                return status;
            }
            break;
    ...
    *sw_ete     = NULL;
    ...
    if (!_FE2K_L2_ETE_SHARED(l3_sw_ete)) {
        ...
        if (status != BCM_E_NONE) {
            LOG_ERROR(BSL_LS_BCM_L3,
                      (BSL_META_U(l3_fe->fe_unit,
                                  "Could not allocate L2 Encap Ete: %s\n"),
                       bcm_errmsg(status)));
            _bcm_fe2000_free_l3_ete(l3_fe, &l3_sw_ete);
            return status;
        }
        LOG_DEBUG(BSL_LS_BCM_L3,
                  (BSL_META_U(l3_fe->fe_unit,
                              "Allocated L2 Encap Ete Index (0x%x)\n"),
                   l3_sw_ete->l3_l2encap.ete_idx.ete_idx));
    }
                         ...
                         _fe2k_l3_ete_t        **sw_ete)
{
    ...
    *sw_ete    = NULL;
    ...
    if (!(_FE2K_L2_ETE_SHARED(l3_ete))) {
        ...
        if (!(l3_intf->if_flags & _FE2K_L3_INTF_ETE_L2_ALLOTTED)) {
            /* if no l2 ete's were allotted to l3 intf, allot now */
            status = _bcm_fe2000_l3_intf_alloc_l2_ete(l3_fe, NULL, l3_intf);
            if (BCM_FAILURE(status)) {
                LOG_ERROR(BSL_LS_BCM_L3,
                          (BSL_META_U(l3_fe->fe_unit, 
                                      "error(%s) could not allocate l2 ete\n"),
                           bcm_errmsg(status)));
                            ...
                            _fe2k_fte_idx_t        *fte_data)
{
    ...
    if (chg_flags == L3_EGR_DMAC_CHG) {
        ...
        if ( (l3_fe->fe_my_modid == bcm_egr->module) || 
              (bcm_egr->flags && BCM_L3_TGID)) {
            status = _bcm_fe2000_l3_egress_dmac_update(l3_fe, bcm_egr);
            if (status != BCM_E_NONE) {
                LOG_ERROR(BSL_LS_BCM_L3,
                          (BSL_META_U(l3_fe->fe_unit,
...
/*
 * Function:
 *     _bcm_fe2000_l3_egress_dmac_update
 * Purpose:
                                  ...
                                  bcm_l3_egress_t        *bcm_egr)
{
    ...
    int                      ignore_status;
    _fe2k_l3_ete_t          *l3_sw_ete;
    soc_sbx_g2p3_eteencap_t  l3_hw_ete;
    _fe2k_ohi_t              ohi;
    _fe2k_l3_intf_t         *l3_intf;
    int                      ete_hash_idx;
    ...
    _FE2K_MAKE_UCAST_IP_SW_ETE_KEY(&l3_sw_ete->l3_ete_key, bcm_egr, l3_intf);
    ...
    if (status != BCM_E_NONE) {
        LOG_ERROR(BSL_LS_BCM_L3,
                  (BSL_META("unit %d: [%s] error %s in setting L3 ete in HW"),
                   l3_fe->fe_unit, FUNCTION_NAME(), bcm_errmsg(status)));
        return status;
    } else {
        LOG_DEBUG(BSL_LS_BCM_L3,
    ...
    if (l3_intf->if_flags & _FE2K_L3_MPLS_TUNNEL_SET) {
        ...
        if (ignore_status != BCM_E_NONE) {
            LOG_ERROR(BSL_LS_BCM_L3,
                      (BSL_META_U(l3_fe->fe_unit,
                                  "Could not enable mpls tunnel on interface 0x%x\n"),
                       _FE2K_USER_HANDLE_FROM_IFID((l3_sw_ete)->l3_intf_id)));
        }
    }
                                    ...
                                    bcm_if_t               *intf)
{
    ...
    if (SOC_IS_SBX_G2P3(l3_fe->fe_unit)) {
        if(bcm_egr->flags & BCM_L3_UNTAG) {
            LOG_ERROR(BSL_LS_BCM_L3,
                      (BSL_META_U(l3_fe->fe_unit,
                                  "BCM_L3_UNTAG not yet supported in G2P3\n")));
            return (BCM_E_PARAM);
        }
    }
...
/*
 * Function:
 *     _bcm_fe2000_l3_find_intf_by_egr_label
 * Purpose:
 ...
 */
int
_bcm_fe2000_l3_find_intf_by_egr_label(_fe2k_l3_fe_instance_t *l3_fe,
                                     ...
                                     _fe2k_l3_intf_t       **ret_l3_intf)
{
    ...
    status      = BCM_E_NOT_FOUND;
    hash_index  = _FE2K_GET_INTF_EGR_LBL_HASH(egr_label);
    hash_bucket = &l3_fe->fe_intf_by_egr_lbl[hash_index];

    DQ_TRAVERSE(hash_bucket, l3_intf_elem) {
        _FE2K_L3INTF_FROM_EGR_LBL_DQ(l3_intf_elem, l3_intf);
        ...
        for (ii = _FE2K_L3_ETE_VIDOP__NOP;
            ...
            if (l3_intf->if_l2_ete[ii].ete_idx ) {
                ...
                if (status != BCM_E_NONE) {
                    LOG_ERROR(BSL_LS_BCM_L3,
                              (BSL_META_U(l3_fe->fe_unit,
                                          "Could not free L2 ete 0x%x: %s"),
                               l3_intf->if_l2_ete[ii].ete_idx, bcm_errmsg(status)));
                }
            }
        }
        l3_intf->if_flags = l3_intf->if_flags &
                                ~(_FE2K_L3_INTF_ETE_L2_ALLOTTED);
    }
    sal_free(l3_intf);
...
bcm_fe2000_l3_cleanup(int unit)
{
    ...
    if (rv != BCM_E_NONE) {
        /* Lock destroyed in sw uninit unless routine failes */
        L3_UNLOCK(unit);
    }
    return rv;
}
...
_bcm_fe2000_l3_sw_init(int unit)
{
    ...
    for (ii = 0; ii < _FE2K_INTF_EGR_LBL_HASH_SIZE; ii++) {
                                     ...
                                     bcm_l3_intf_t          *bcm_intf)
{
    ...
    if (bcm_intf->l3a_flags & ~(L3_INTF_SUPPORTED_FLAGS)) {
        LOG_ERROR(BSL_LS_BCM_L3,
                  (BSL_META_U(l3_fe->fe_unit,
                              "unsupported flags 0x%x"), bcm_intf->l3a_flags));
        return BCM_E_PARAM;
    }
    ...
    if ((bcm_intf->l3a_flags & BCM_L3_IP6) && bcm_intf->l3a_vrf) {
        LOG_ERROR(BSL_LS_BCM_L3,
                  (BSL_META_U(l3_fe->fe_unit,
                              "VRFs not supported on IPV6 interfaces\n")));
        return BCM_E_PARAM;
    }
                                  ...
                                  uint32                 *ul_egr_idx)
{
    ...
    if (op == L3_INTF_DEL_SMAC) {
        ...
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_L3,
                      (BSL_META_U(l3_fe->fe_unit,
                                  "failed to allocate esmac idx: %d %s\n"),
                       rv, bcm_errmsg(rv)));
            return rv;
        }
        ...
        if (BCM_FAILURE(rv)) {
            ...
            COMPILER_REFERENCE(junk);
            return rv;
        }

        LOG_VERBOSE(BSL_LS_BCM_L3,
                    (BSL_META_U(l3_fe->fe_unit,
                                "%s lsmi=%d esmi=%d rv=%d\n"),
                     ((flags & _SBX_GU2_RES_FLAGS_RESERVE) ? 
...
/*
 * Function:
 *    _bcm_fe2000_l3_intf_alloc_l2_ete
 * Purpose:
 *    To alloc l2 ete's for l3intf that could be shared by l3 egress
 */
STATIC int
_bcm_fe2000_l3_intf_alloc_l2_ete(_fe2k_l3_fe_instance_t *l3_fe,
                                uint32                 *reserve_etes,
...
{
    int                 status, flags = 0;
    uint32              etes[_FE2K_L3_ETE_VIDOP__MAX_TYPES] , ii;
    ...
    if (BCM_FAILURE(status)) {
        LOG_ERROR(BSL_LS_BCM_L3,
                  (BSL_META_U(l3_fe->fe_unit,
                              "Could not allocate L2 ete: %s\n"), 
                   bcm_errmsg(status)));
        return status;
    }

    l3_intf->if_flags = l3_intf->if_flags | _FE2K_L3_INTF_ETE_L2_ALLOTTED;
    for (ii = 0; ii < _FE2K_L3_ETE_VIDOP__MAX_TYPES; ii++) {
        l3_intf->if_l2_ete[ii].ete_idx = etes[ii];
        LOG_VERBOSE(BSL_LS_BCM_L3,
                    (BSL_META_U(l3_fe->fe_unit, 
                                "allocated L2 ete (vid op %d) ete addr 0x%x\n"),
...
_bcm_fe2000_l3_alloc_intf(_fe2k_l3_fe_instance_t *l3_fe,
                          ...
                          _fe2k_l3_intf_t        **ret_intf)
{
    int                            status, flags, ete_valid = 0, ii;
    ...
    if (reserve_etes) {
        ete_valid = 1;
        for (ii = 0; ii < _FE2K_L3_ETE_VIDOP__MAX_TYPES; ii++) {
            if (reserve_etes[ii] == _FE2K_INVALID_ETE_IDX) {
                ete_valid = 0;
            }
        }
        if (ete_valid == 1) {
            status = _bcm_fe2000_l3_intf_alloc_l2_ete(l3_fe, 
                                                      reserve_etes,
                                                      l3_intf);
            if (BCM_FAILURE(status)) {
                LOG_ERROR(BSL_LS_BCM_L3,
                          (BSL_META_U(l3_fe->fe_unit,
                                      "Warm boot: Could not allocate L2 ete: %s\n"), 
                           bcm_errmsg(status)));
            }
        } else {
            LOG_ERROR(BSL_LS_BCM_L3,
                      (BSL_META_U(l3_fe->fe_unit,
                       "Warm boot: Atleast one of the L2 ete is invalid\n"))); 
        }
    }
                        ...
                        bcm_l3_route_t *info)
{
    ...
    if ((info->l3a_flags & BCM_L3_IP6) && info->l3a_vrf) {
        LOG_ERROR(BSL_LS_BCM_L3,
                  (BSL_META_U(unit,
                              "VRFs not supported on IPV6 routes\n")));
        return BCM_E_PARAM;
    }
                                   ...
                                   bcm_if_t intf)
{
    ...
    if (status != BCM_E_NONE) {
        LOG_ERROR(BSL_LS_BCM_L3,
                  (BSL_META_U(l3_fe->fe_unit,
                              "Could not find mpath object with base %d"), mpintf));
        L3_UNLOCK(unit);
        return  status;
    }
                       ...
                       uint32 vrid)
{
    ...
    if (status != BCM_E_NONE) {
        LOG_ERROR(BSL_LS_BCM_L3,
                  (BSL_META_U(unit,
                              "error %s in soc_sbx_g2p3_v2e1_get"), 
                   bcm_errmsg(status)));
        return status;
    }
    ...
    if (status != BCM_E_NONE) {
        LOG_ERROR(BSL_LS_BCM_L3,
                  (BSL_META_U(unit,
                              "error %s in soc_sbx_g2p3_v2e1_set"),
                   bcm_errmsg(status)));
        return status;
    }
