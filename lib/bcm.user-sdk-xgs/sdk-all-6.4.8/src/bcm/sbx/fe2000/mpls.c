/*
 * $Id: mpls.c,v 1.242 Broadcom SDK $
                                      ...
                                      bcm_mpls_egress_label_t *label_array)
{
    ...
    if (status != BCM_E_NONE) {
        LOG_ERROR(BSL_LS_BCM_MPLS,
                  (BSL_META_U(l3_fe->fe_unit,
                              "error(%s) map-set l2-ete(0x%x) for mpls\n"),
                   bcm_errmsg(status),
                                    ...
                                    _fe2k_l3_intf_t         *l3_intf)
{
    ...
    if (l3_intf->if_flags & _FE2K_L3_INTF_IN_EGR_LBL_LIST) {
    ...
    if (BCM_SUCCESS(rv)) {
        ...
        if (rv != BCM_E_NONE) {
            LOG_ERROR(BSL_LS_BCM_MPLS,
                      (BSL_META_U(l3_fe->fe_unit,
                                  "error(%s) setting ipv4 ete on intf(0x%x)\n"),
                       bcm_errmsg(rv), ifid));
            return rv;
        }
    }
                                       ...
                                       uint32 port, uint32 vid, uint32 vsi)
{
    ...
    if (local_gport) {
        LOG_VERBOSE(BSL_LS_BCM_MPLS,
                    (BSL_META_U(l3_fe->fe_unit,
                                "Processing port=%d vid=0x%03x vsi=0x%04x\n"),
                     port, vid, vsi));
    }
    ...
    if (ve.v4route == 0) {
        ...
        LOG_VERBOSE(BSL_LS_BCM_MPLS,
                    (BSL_META_U(l3_fe->fe_unit,
                                "Found valid GPORT at fti 0x%0x on "
                                "vpn 0x%0x\n"), fti, vpn_id));
        ...
        if (local_gport) {
            ...
            if (BCM_FAILURE(rv)) {
                LOG_ERROR(BSL_LS_BCM_MPLS,
                          (BSL_META_U(l3_fe->fe_unit,
                                      "Failed to allocate vpnId 0x%x: %s\n"),
        ...
        LOG_VERBOSE(BSL_LS_BCM_MPLS,
                    (BSL_META_U(l3_fe->fe_unit,
                                "Recovering mplsGPort 0x%x, target "
                                "modport=%d:%d\n"), 
...
_bcm_fe2000_mpls_recover(_fe2k_l3_fe_instance_t  *l3_fe)
{
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_MPLS,
                  (BSL_META_U(l3_fe->fe_unit,
                              "Failed to get non tb vsi list: %s\n"),
                   bcm_errmsg(rv)));
        return rv;
    }
         ...
         vsi++) 
    {
        ...
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_MPLS,
                      (BSL_META_U(l3_fe->fe_unit,
                                  "Failed to recover gport/vsi 0x%04x: %s\n"),
                       vsi, bcm_errmsg(rv)));
            /* keep trying */
        }
...
_bcm_fe2000_mplstp_init(int unit)
{
    ...
    soc_sbx_g2p3_xt_t  xt;    /* exception table */  
    ...
    BCM_IF_ERROR_RETURN(
       soc_sbx_g2p3_exc_dcn_idx_get(unit, &xt_index));
    ...
    BCM_IF_ERROR_RETURN
      (soc_sbx_g2p3_xt_get(unit, xt_index, &xt));

    xt.forward = 1;

    BCM_IF_ERROR_RETURN
      (soc_sbx_g2p3_xt_set(unit, xt_index, &xt));
                      ...
                      _fe2k_l3_fe_instance_t *l3_fe)
{
    ...
    if (vpws_vpn_low && vpws_vpn_high) {
        ...
        if (rv != BCM_E_NONE) {
            ...
            if (rv != BCM_E_NONE) {
                ...
                _sbx_gu2_range_reserve(unit, SBX_GU2K_USR_RES_LINE_VSI,
                                       FALSE, 0);
            }
        }
    }
    if (vpws_pw_gport_low && vpws_pw_gport_high) {
        ...
        rv = _sbx_gu2_range_reserve(unit, SBX_GU2K_USR_RES_FTE_MPLS,
...
*/
    status = bcm_fe2000_mpls_gport_get(l3_fe->fe_unit, gport, vpn_sap);
    if (BCM_FAILURE(status)) {
        if (vpn_sap) {
            *vpn_sap = NULL;
        }
...
/*
 * Function:
 *      _bcm_fe2000_free_vpncb_for_vlan_vsi
 * Purpose:
...
int
_bcm_fe2000_free_vpncb_for_vlan_vsi(int                     unit,
                                    _fe2k_l3_fe_instance_t  *l3_fe,
                                    _fe2k_vpn_control_t     **vpnc)
{
...
/*
 * Function:
 *      _bcm_fe2000_alloc_vpncb_for_vlan_vsi
 * Purpose:
...
int
_bcm_fe2000_alloc_vpncb_for_vlan_vsi(int                     unit,
                                  _fe2k_l3_fe_instance_t  *l3_fe,
                                  bcm_vlan_t              vlan_vsi,
                                  _fe2k_vpn_control_t     **vpnc)
{
    ...
    hidx = _FE2K_GET_MPLS_VPN_HASH(vlan_vsi);
	    ...
	    if (BCM_FAILURE(status)) {
           ...
           if( SOC_SBX_CFG(l3_fe->fe_unit)->mplstp_ena ==1) {
            ...
            if (BCM_FAILURE(status)) {
                LOG_ERROR(BSL_LS_BCM_MPLS,
                          (BSL_META_U(l3_fe->fe_unit,
                                      "error(%s) calling soc_sbx_g2p3_l2e_set "
                                      "for label(0x%x)\n"),
                           bcm_errmsg(status),
                           mpls_port->match_label));
                return status;
                               ...
                               _fe2k_vpn_sap_t      **vpn_sap)
{
    ...
    *vpn_sap = sal_alloc(sizeof(_fe2k_vpn_sap_t),
    ...
    LOG_VERBOSE(BSL_LS_BCM_MPLS,
                (BSL_META_U(l3_fe->fe_unit,
                            "vpn(0x%x) allocated fte(0x%x), ohi(0x%x), ete(0x%x) l2-ete(0x%x)\n"),
                 vpnc->vpn_id, fte_idx,
                 (*vpn_sap)->vc_ohi.ohi,
                 (*vpn_sap)->vc_ete_hw_idx.ete_idx,
                 (*vpn_sap)->vc_l2ete_hw_idx.ete_idx));
                                   ...
                                   bcm_mpls_port_t        *mpls_port)
{
    ...
    if (mpls_port->pw_failover_port_id) {
        if (BCM_GPORT_IS_MPLS_PORT(mpls_port->pw_failover_port_id)) {
            ...
            if (!_FE2K_MPLS_PORT_FTE_VALID(l3_fe->fe_unit, failover_pw_port_fte)) {
        ...
        if ((SOC_SBX_CFG(l3_fe->fe_unit)->mplstp_ena == 0) &&
            ...
            if (status == BCM_E_NOT_FOUND) {
                ...
                if (BCM_FAILURE(status)) {
                    LOG_ERROR(BSL_LS_BCM_MPLS,
                              (BSL_META_U(l3_fe->fe_unit,
                                          "failed to get logical port: %d (%s)\n"),
                                   ...
                                   _fe2k_vpn_sap_t **vpn_sap)
{
    uint32 search_flag;
    uint32 mask_flags;
    _fe2k_vpn_sap_t *tmp_vpn_sap = NULL, *vpws_vpn_sap = NULL;
    
    mask_flags = (BCM_MPLS_PORT_FAILOVER |
                 BCM_MPLS_PORT_NETWORK  |
                 BCM_MPLS_PORT_PW_FAILOVER);
    switch (port_type) {
    case _BCM_FE2K_MPLS_VPWS_UNI_PORT:
        search_flag = 0;
        break;
    case _BCM_FE2K_MPLS_VPWS_UNI_PORT_FO:
        search_flag = BCM_MPLS_PORT_FAILOVER;
        break;
    case _BCM_FE2K_MPLS_VPWS_NW_PORT:
        search_flag = BCM_MPLS_PORT_NETWORK;
        break;
    case _BCM_FE2K_MPLS_VPWS_NW_PORT_FO:
        
        search_flag = BCM_MPLS_PORT_NETWORK | BCM_MPLS_PORT_FAILOVER;
        break;
    case _BCM_FE2K_MPLS_VPWS_NW_PORT_PW_FO:
        search_flag = BCM_MPLS_PORT_NETWORK | BCM_MPLS_PORT_PW_FAILOVER;
        break;
    case _BCM_FE2K_MPLS_VPWS_NW_PORT_PW_FO2:
...
#define _MPLS_VPN_SAP_SWAP(f)  \
  tmp = vpnSaps[0]->f; vpnSaps[0]->f = vpnSaps[1]->f; vpnSaps[1]->f = tmp;
    ...
    if (SOC_WARM_BOOT(l3_fe->fe_unit) == 0) { 
        rv = _bcm_fe2000_g2p3_mpls_vpws_fte_connect(l3_fe, 
...
/*
 * Function:
 *    _bcm_fe2000_mpls_port_is_failover_update
 * Purpose:
 ...
 *    0 - Otherwise
 */
int _bcm_fe2000_mpls_port_is_failover_update(bcm_mpls_port_t *mpls_port_old,
                                             bcm_mpls_port_t *mpls_port)
{
                              ...
                              _fe2k_vpn_sap_t        *vpn_sap) 
{
    ...
    _fe2k_vpn_sap_t   *tmp_vpn_sap = NULL, *vpws_ac_sap = NULL;
    int fo_update;
    ...
    if (mpls_port->flags & BCM_MPLS_PORT_REPLACE) {
        ...
        if (vpnc->vpn_flags & BCM_MPLS_VPN_VPWS) {
            ...
            fo_update = _bcm_fe2000_mpls_port_is_failover_update(
                                     &vpn_sap->vc_mpls_port, mpls_port);
            if (fo_update) {
                LOG_DEBUG(BSL_LS_BCM_MPLS,
                          (BSL_META_U(unit,
                ...
                if ((mpls_port->failover_id != 0) || 
                    ...
                    status = _bcm_fe2000_g2p3_mpls_failover_update(l3_fe, vpnc,
                                                      vpn_sap, mpls_port, 0);
                ...
                status = _bcm_fe2000_g2p3_mpls_vpws_port_failover_update(
                                                       l3_fe->fe_unit,
                                                       fte_idx_to_update,
                                                       fte_idx_fo,
                                                       mpls_port->failover_id);
        ...
        if (vpnc->vpn_flags & BCM_MPLS_VPN_VPWS) {
            ...
            if (num_vpn_ports == 2) {
                ...
                if ((is_pw_fo_exists == 1) && (vpws_ac_sap != NULL)) {
                    status = _bcm_fe2000_g2p3_mpls_vpws_pw_fo_connect(l3_fe,
                          ...
                          int                     num_vpn_ports)
{
    ...
    _fe2k_vpn_sap_t              *vpn_saps[2], *vpws_ac_sap = NULL;
    bcm_fe2k_mpls_trunk_association_t *trunkAssoc=NULL;
    uint32                      trunkid = -1, is_failover = 0;
    uint8                       is_trunk = 0, is_pw_fo_exists = 0;
    uint8                       is_mplstp_vpwsuni = 0, is_same_label = 0;
    ...
    if (mpls_port->flags & BCM_MPLS_PORT_PW_FAILOVER) {
        if (vpnc->vpn_flags & BCM_MPLS_VPN_VPLS) {
            resType = SBX_GU2K_USR_RES_FTE_VPXS_PW_FO;
    ...
    if (!SOC_SBX_CFG(unit)->mplstp_ena) {
        if (!(mpls_port->flags & BCM_MPLS_PORT_FAILOVER)) {
            ...
            if (status != BCM_E_NONE) {
                LOG_ERROR(BSL_LS_BCM_MPLS,
                          (BSL_META_U(l3_fe->fe_unit,
                                      "error(%s) programming vlan2etc for port-vsi(0x%x)\n"),
                             ...
                             _fe2k_vpn_sap_t        **vpn_sap)
{
    ...
    bcm_mpls_port_t            *mpls_port;
    int                         freeLp = 0;
    uint32                    logicalPort = 0, is_failover = 0;
    _sbx_gu2_usr_res_types_t    restype;
                                      ...
                                      bcm_vlan_vector_t vlan_vec)
{
    ...
    int                          status = BCM_E_NONE;
    bcm_port_t                   phy_port[BCM_TRUNK_MAX_PORTCNT];
    ...
    for (vid = BCM_VLAN_MIN + 1; vid < BCM_VLAN_MAX; vid++) {
        for (index = 0; index < num_ports; index++) {
            ...
            status = SOC_SBX_G2P3_PV2E_GET(unit, phy_port[index],
                                           vid, &p3pv2e);
            if (BCM_FAILURE(status)) {
                break;
            }
            ...
            status = _soc_sbx_g2p3_pv2e_set(unit, phy_port[index], vid,
                                            &p3pv2e);
    ...
    if (!l3_fe) {
        ...
        if (!_FE2K_IS_MPLS_INITIALIZED(l3_fe)) {
            ...
            if (BCM_SUCCESS(status)) {
                ...
                if (status == BCM_E_NOT_FOUND) {
                    if (is_vlan_vsi) {
                        status = _bcm_fe2000_alloc_vpncb_for_vlan_vsi(unit, 
                                                                      l3_fe,
                                                                      vpn,
                                                                      &vpnc);
                        ...
                        if (BCM_SUCCESS(status)) {
                            ...
                            if (vpnc->vpn_flags & BCM_MPLS_VPN_VPLS &&
                                ...
                                if (!(mpls_port->match_label & _FEK2_L3_MPLS_PWE3_LBL_ID)) {
                                    LOG_ERROR(BSL_LS_BCM_MPLS,
                                              (BSL_META_U(unit,
                                                          "Match label 0x%x does not have bit 0x%x as "
                                                          "required for VPLS\n"),
                            ...
                            bcm_gport_t       mpls_port_id)
{
    ...
    if (!_FE2K_IS_MPLS_INITIALIZED(l3_fe)) {
        LOG_ERROR(BSL_LS_BCM_MPLS,
                  (BSL_META_U(l3_fe->fe_unit, "mpls is not initialized\n")));
        L3_UNLOCK(unit);
        return BCM_E_INIT;
    }
    ...
    if (vpnc->vpn_flags & BCM_MPLS_VPN_VPWS) {
        ...
        if (num_vpn_ports == 2 && !(vpn_sap->vc_mpls_port.flags & 
            ...
            status = _bcm_fe2000_mpls_vpws_fte_connect(l3_fe, vpnSaps, 
                                               _FE2K_MPLS_PORT_MATCH_DELETE, 0);
            if (BCM_FAILURE(status)) {
                LOG_ERROR(BSL_LS_BCM_MPLS,
                          (BSL_META_U(unit,
                                      "error(%s) failed to unlink VPWS\n"),
    ...
    if ((vpnc->vpn_flags & BCM_MPLS_VPN_VPLS) && (vpn > 0) && (vpn < BCM_VLAN_MAX)) {
        if (DQ_EMPTY(&vpnc->vpn_sap_head)) {
            ...
            _bcm_fe2000_free_vpncb_for_vlan_vsi(unit, l3_fe, &vpnc);
        }
    }
                                     ...
                                     bcm_mpls_egress_label_t *label_array)
{
    ...
    l3_intf->if_flags |= _FE2K_L3_INTF_IN_EGR_LBL_LIST;
/*    for (i = 0; i < num_labels; i++) { */
        i = 0;
        l3_intf->if_flags |= _FE2K_L3_INTF_IN_EGR_LBL_LIST;
        hash_index = _FE2K_GET_INTF_EGR_LBL_HASH(label_array[i].label);
        ...
        if (status  == BCM_E_NOT_FOUND) {
            *swinfo = *info;
            ...
            if (BCM_FAILURE(status)) {
                LOG_ERROR(BSL_LS_BCM_MPLS,
                          (BSL_META_U(unit,
                                      "error(%s) Updating switch_info Dbase (0x%x)\n"),
                                     ...
                                     bcm_mpls_tunnel_switch_t *info)
{
    ...
    if (BCM_FAILURE(status)) {
        LOG_ERROR(BSL_LS_BCM_MPLS,
                  (BSL_META_U(l3_fe->fe_unit,
                              "failed to free MPLS switch_info dbase element: %d (%s)\n"),
                   status, bcm_errmsg(status)));
    } else {
        sal_free(datum);
    }
                                ...
                                uint64 *val)
{
    ...
    _fe2k_l3_fe_instance_t      *l3_fe = NULL;
    _fe2k_vpn_sap_t             *vpn_sap = NULL;
    soc_sbx_g2p3_label2e_t       label2etc;
    _fe2k_l3_intf_t             *l3_intf = NULL;
    ...
    case bcmMplsInBytes:
        if (label != 0) {
            if (SOC_SBX_CFG(unit)->mplstp_ena == 1) {
                soc_sbx_g2p3_label2e_t_init(&label2etc);
                status = soc_sbx_g2p3_label2e_get(unit, label, &label2etc);
                COMPILER_REFERENCE(status);
                lpi = label2etc.lpidx;
            }
        } else {
    ...
    case bcmMplsOutBytes:
        ...
        if (label) {
            status = _bcm_fe2000_l3_find_intf_by_egr_label(l3_fe,
                                                          label,
                                                         &l3_intf);
            if (BCM_SUCCESS(status)) {
                status = _bcm_fe2000_get_ete_by_type_on_intf(l3_fe,
                                                      l3_intf,
                                                       ...
                                                       &l3_ete);
                if (BCM_SUCCESS(status)) {
                    ohi = _FE2K_L3_G2P3_ADJUST_TB_OFFSET(l3_ete->l3_ohi.ohi);
                } else {
                    LOG_ERROR(BSL_LS_BCM_MPLS,
                              (BSL_META_U(unit,
...
int
_bcm_fe2000_mpls_egress_stat_enable(int unit,
                                    uint32 ohi,
                                    int enable)
{
    int                          status = BCM_E_NONE;
...
int
_bcm_fe2000_mpls_ingress_stat_enable(int unit,
                                     uint32 lpi,
                                     int enable)
{
    int                          status = BCM_E_NONE;
...
int
bcm_fe2000_mpls_label_stat_enable_set(int unit, 
                                      bcm_mpls_label_t label,
                                      bcm_gport_t port,
                                      int enable)
{
    ...
    if (BCM_SUCCESS(status)) {
        lpi = label2etc.lpidx;
        status = _bcm_fe2000_mpls_ingress_stat_enable(unit, lpi, enable);
        if (BCM_FAILURE(status)) {
            LOG_ERROR(BSL_LS_BCM_MPLS,
                      (BSL_META_U(unit,
    ...
    if (BCM_SUCCESS(status)) {
        ...
        if (BCM_SUCCESS(status)) {
            ...
            ohi = _FE2K_L3_G2P3_ADJUST_TB_OFFSET(l3_ete->l3_ohi.ohi);
            status = _bcm_fe2000_mpls_egress_stat_enable(unit, ohi, enable);
            if (BCM_FAILURE(status)) {
                LOG_ERROR(BSL_LS_BCM_MPLS,
                          (BSL_META_U(unit,
                               ...
                               int *keepUntagged)
{
    ...
    _fe2k_vpn_control_t    *vpncb = NULL;
    _fe2k_vpn_sap_t        *vpnsap = NULL; 
    uint16                num_ports = 1;
    ...
    if (SOC_SBX_CFG(unit)->mplstp_ena) {
        if (vpwsuni) {
            ...
            rv = _bcm_fe2000_mpls_find_vpn_sap_by_gport(l3_fe, mpls_port_id, &vpnsap); 
            if (BCM_FAILURE(rv)) {
                LOG_ERROR(BSL_LS_BCM_MPLS,
                          (BSL_META_U(l3_fe->fe_unit,
            ...
            rv = _bcm_fe2000_find_mpls_vpncb_by_id(unit, l3_fe, 
                                                   vsi, &vpncb);
            if (BCM_FAILURE(rv)) {
                LOG_ERROR(BSL_LS_BCM_MPLS,
                          (BSL_META_U(l3_fe->fe_unit,
                ...
                rv = _bcm_fe2000_find_vpn_sap_by_port_vlan(l3_fe, vpncb, 
                                                           modgport, vid, &vpnsap);
                if (BCM_FAILURE(rv)) {
                    /* try to find gport with port only match */
                    rv = _bcm_fe2000_find_vpn_sap_by_port(l3_fe, vpncb, 
                                                           modgport, &vpnsap);
                }
                if (BCM_FAILURE(rv)) {
                    LOG_ERROR(BSL_LS_BCM_MPLS,
        ...
        *keepUntagged = !(vpnsap->vc_mpls_port.flags & BCM_MPLS_PORT_INNER_VLAN_ADD);
        ...
        rv = _bcm_fe2000_mpls_port_vlan_vector_internal(unit, mpls_port_id,
                                                 ...
                                                 &mpls_port, NULL, &num_ports);
        LOG_VERBOSE(BSL_LS_BCM_MPLS,
                    (BSL_META_U(l3_fe->fe_unit,
                                " rv=%d vsi=0x%x (fti 0x%x)--> \nmplsPort=0x%x"
        ...
        /* _bcm_fe2000_mpls_port_vlan_vector_internal returns error,
         *  if the mpls_port.port doesn't belong to the local module.
...
_bcm_fe2000_mpls_label_commit_enable_set(int unit, uint32 enable)
{
    int status;
    BCM_IF_ERROR_RETURN(L3_LOCK(unit));
    status = soc_sbx_g2p3_label2e_commit_set(unit, enable);
    if (BCM_FAILURE(status)) {
        LOG_ERROR(BSL_LS_BCM_MPLS,
                  (BSL_META_U(unit,
...
_bcm_fe2000_mpls_label_commit_enable_get(int unit, uint32 *enable)
{
    int status;
    BCM_IF_ERROR_RETURN(L3_LOCK(unit));
    status = soc_sbx_g2p3_label2e_commit_get(unit, enable);
    if (BCM_FAILURE(status)) {
        LOG_ERROR(BSL_LS_BCM_MPLS,
                  (BSL_META_U(unit,
...
_bcm_fe2000_mpls_label_commit(int unit)
{
    int status;
    BCM_IF_ERROR_RETURN(L3_LOCK(unit));
    status = soc_sbx_g2p3_label2e_commit(unit, -1);
    if (BCM_FAILURE(status)) {
        LOG_ERROR(BSL_LS_BCM_MPLS,
                  (BSL_META_U(unit,
