...
/************************************************************************/
    ...
    case bcmSwitchMeterAdjustInterframeGap:
        ...
        if (SOC_IS_ARADPLUS(unit)) {
            /* Since the flag is global it doesn't matter which meter group/core is used. */
            soc_sand_rv = soc_ppd_mtr_meters_group_info_get(soc_sand_dev_id, 0/*core*/, 0, &mtr_group_info);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    ...
    case  bcmSwitchPolicerFairness:
        ...
        if (SOC_IS_ARAD(unit)) {
			...
			_DPP_POLICER_METER_CORES_ITER(core_id){
				soc_sand_rv = soc_ppd_mtr_meters_group_info_set(soc_sand_dev_id, core_id, 0, &mtr_group_info);
				BCM_SAND_IF_ERR_EXIT(soc_sand_rv);

				soc_sand_rv = soc_ppd_mtr_meters_group_info_set(soc_sand_dev_id, core_id, 1, &mtr_group_info);
				BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
			}
        }
        break;
    ...
    case bcmSwitchMeterAdjustInterframeGap:
        ...
        if (SOC_IS_ARADPLUS(unit)) {
            ...
            if (mtr_group_info.is_ipg_compensation_enabled != is_ipg_enabled) {
                ...
                /* Configuration is global(all groups)*/
                mtr_group_info.is_ipg_compensation_enabled = is_ipg_enabled;
				if (!SOC_IS_JERICHO(unit)) {
					soc_sand_rv = soc_ppd_mtr_meters_group_info_set(soc_sand_dev_id, 0/*core*/, 0/*group*/, &mtr_group_info);
                BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
				}
				else{
					/* In Jericho, configuration is per core/group*/
					_DPP_POLICER_METER_CORES_ITER(core_id){
						soc_sand_rv = soc_ppd_mtr_meters_group_info_set(soc_sand_dev_id, core_id, 0, &mtr_group_info); 
						BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
						soc_sand_rv = soc_ppd_mtr_meters_group_info_set(soc_sand_dev_id, core_id, 1, &mtr_group_info); 
