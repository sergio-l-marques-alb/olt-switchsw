    ...
    _bcm_petra_trill_mc_trill_src_list_t    *src_list)
{
    ...
    rv =  _bcm_pb_trill_sw_db_hash_trill_root_src_find(unit,
                                                 (sw_state_htb_key_t) &(root_name),
                                                 (sw_state_htb_data_t) src_list,
                                                 FALSE); 
    ...
    _bcm_petra_trill_mc_trill_route_list_t  *route_list)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rv = _bcm_pb_trill_sw_db_hash_trill_route_info_find(unit,
                                                   (sw_state_htb_key_t) &(root_name),
                                                   (sw_state_htb_data_t) route_list,
                                                   FALSE);  
    if ((rv == BCM_E_NONE) ||  (rv == BCM_E_NOT_FOUND) ){
        BCM_RETURN_VAL_EXIT(rv);
    }
    ...
    int                                     *route_ndx)
{
    ...
    *route_ndx = -1;
    ...
    for (i = 0; i < route_list->nof_routes; i++) {
        if ((route_list->route[i].flags == flags)
            &&
            (route_list->route[i].vlan == vlan)
            &&
            (route_list->route[i].group == group))
         {
            *route_ndx = i;
            BCM_EXIT;
         }
    ...
    bcm_if_t intf) 
{
    ...
    if (rv == BCM_E_NOT_FOUND ) {
        ...
        rv = _bcm_pb_trill_sw_db_hash_trill_root_src_insert(unit,
                                                       (sw_state_htb_key_t) &(root_name),
                                                       (sw_state_htb_data_t) &src_list);        
        if (BCM_FAILURE(rv)) {
...
int _bcm_pb_trill_multicast_entry_add(int unit, bcm_trill_multicast_entry_t *trill_mc) 
{
    ...
    if (rv == BCM_E_NOT_FOUND ) {
        ...
        rv = _bcm_pb_trill_sw_db_hash_trill_route_info_insert(unit,
                                                         (sw_state_htb_key_t) &(trill_mc->root_name),
                                                         (sw_state_htb_data_t) &route_list);        
        if (BCM_FAILURE(rv)) {
            BCMDNX_ERR_EXIT_MSG(rv, (_BSL_BCM_MSG("error(%s) adding route info (0x%x)\n"),
...
int
_bcm_pb_trill_sw_db_hash_trill_root_src_find(int unit, sw_state_htb_key_t key, sw_state_htb_data_t data, int remove)
{
    bcm_error_t rv = BCM_E_NONE;
    int mc_trill_root_src_db;
    BCMDNX_INIT_FUNC_DEFS;

    BCMDNX_IF_ERR_EXIT(sw_state_access[unit].dpp.bcm.trill.mc_trill_root_src_db_htb_handle.get(unit, &mc_trill_root_src_db));
    rv = sw_state_htb_find(unit, mc_trill_root_src_db, key, data, remove);
    if (rv != BCM_E_NOT_FOUND) {
        BCMDNX_IF_ERR_EXIT(rv);
    }
    BCM_RETURN_VAL_EXIT(rv);
...
int 
_bcm_pb_trill_sw_db_hash_trill_root_src_insert(int unit, sw_state_htb_key_t key, sw_state_htb_data_t data)
{
    int mc_trill_root_src_db;
    BCMDNX_INIT_FUNC_DEFS;

    BCMDNX_IF_ERR_EXIT(sw_state_access[unit].dpp.bcm.trill.mc_trill_root_src_db_htb_handle.get(unit, &mc_trill_root_src_db));
    BCMDNX_IF_ERR_EXIT(sw_state_htb_insert(unit, mc_trill_root_src_db, key, data));
...
int
_bcm_pb_trill_sw_db_hash_trill_route_info_find(int unit, sw_state_htb_key_t key, sw_state_htb_data_t data, int remove)
{
    bcm_error_t rv = BCM_E_NONE;
    ...
    BCMDNX_IF_ERR_EXIT(sw_state_access[unit].dpp.bcm.trill.mc_trill_route_info_db_htb_handle.get(unit, &mc_trill_route_info_db));
    rv = sw_state_htb_find(unit, mc_trill_route_info_db, key, data, remove);
    if (rv != BCM_E_NOT_FOUND) {
        BCMDNX_IF_ERR_EXIT(rv);
    }
    BCM_RETURN_VAL_EXIT(rv);
...
int 
_bcm_pb_trill_sw_db_hash_trill_route_info_insert(int unit, sw_state_htb_key_t key, sw_state_htb_data_t data)
{
    int mc_trill_route_info_db;
    BCMDNX_INIT_FUNC_DEFS;

    BCMDNX_IF_ERR_EXIT(sw_state_access[unit].dpp.bcm.trill.mc_trill_route_info_db_htb_handle.get(unit, &mc_trill_route_info_db));
    BCMDNX_IF_ERR_EXIT(sw_state_htb_insert(unit, mc_trill_route_info_db, key, data));
exit:
    BCMDNX_FUNC_RETURN;
}
