...
#define _DPP_RATE_PACKET_MODE_CORRECTION (2)
                           ...
                           uint32 kbits_sec, uint32 kbits_burst)
{
    ...
    if (kbits_sec != 0 && kbits_burst != 0) {
		...
		min_rate = policer.is_packet_mode ? dpp_meter->policer_min_rate * _DPP_RATE_PACKET_MODE_CORRECTION : dpp_meter->policer_min_rate;
		
        if (kbits_sec < min_rate) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (DPP_RATE_MSG("rate is lower than the device min-rate: %d. rate can be either 0 or above the min-rate \n"), min_rate));
		} 
