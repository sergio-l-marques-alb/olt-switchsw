    ...
    switch(dload_method)
    {
        case phymodFirmwareLoadMethodInternal:
            if (PHYMOD_CORE_INIT_F_UNTIL_FW_LOAD_GET(init_config)) {
                ...
                return quadra28_bcast_after_fw_load(core, chip_id);
            } else {
                if (init_config->flags == 0) {
                   PHYMOD_DIAG_OUT(("Firware download success\n"));
                    /* Doing Softreset to clear Broadcast*/
                   PHYMOD_IF_ERR_RETURN(
                           quadra28_bcast_disable(core));
                    phy.port_loc = core->port_loc;
                    PHYMOD_MEMCPY(&phy.access, &core->access, sizeof(phymod_access_t));
                    phy.type = core->type; 
                    PHYMOD_IF_ERR_RETURN
                    ...
                    PHYMOD_IF_ERR_RETURN (
                        _quadra28_set_datapath(&core->access, init_config->interface.data_rate, init_config->op_datapath));
                }
            }
        break;
    ...
    *is_identified = 0;
    ...
    if ((id0.v[0] == QUADRA28_PMD_ID0 &&
        ...
        if (chip_id == QUADRA28_82780_CHIP_ID || chip_id == QUADRA28_82752_CHIP_ID 
                || chip_id == QUADRA28_82758_CHIP_ID) {
