/* 
 * $Id: pcidappl.c,v 1.46 Broadcom SDK $
...
main(int argc, char *argv[])
{
    ...
    if (SOC_CONTROL(0)->chip_driver == NULL && SOC_IS_DNX_TYPE(pci_dev_id)) {
        SOC_CONTROL(0)->chip_driver = soc_dnx_chip_driver_find(pci_dev_id, pci_rev_id);
    }
#endif
...
#endif
#ifdef BCM_SBX_SUPPORT
    if (SOC_CONTROL(0)->chip_driver == NULL && SOC_IS_SBX_TYPE(pci_dev_id)) {
