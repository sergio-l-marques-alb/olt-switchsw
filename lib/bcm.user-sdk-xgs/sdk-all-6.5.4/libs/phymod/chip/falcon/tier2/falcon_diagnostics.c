...
int falcon_phy_pattern_enable_set(const phymod_phy_access_t* phy, uint32_t enable, phymod_pattern_t* pattern)
{
    PHYMOD_IF_ERR_RETURN
...
int falcon_diagnostics_eye_margin_proj( const phymod_phy_access_t* phy, uint32_t flags,
                                        const phymod_phy_eyescan_options_t* eyescan_options) {
#ifdef SERDES_API_FLOATING_POINT
  int osr_mode;
  USR_DOUBLE data_rate;
  struct   falcon_tsc_uc_core_config_st core_cfg;
  if(PHYMOD_EYESCAN_F_PROCESS_GET(flags)) {
    PHYMOD_IF_ERR_RETURN(falcon_tsc_get_uc_core_config (&(phy->access), &core_cfg));
    PHYMOD_IF_ERR_RETURN(falcon_osr_mode_get(&phy->access, &osr_mode));
    data_rate = (core_cfg.vco_rate_in_Mhz * 1000.0 * 1000.0) / (1<<osr_mode);
    PHYMOD_DIAG_OUT((" data rate       = %f\n", data_rate ));

    falcon_tsc_eye_margin_proj(&phy->access, data_rate, eyescan_options->ber_proj_scan_mode,
