                       ...
                       const phymod_phy_inf_config_t* config)
{
    ...
    phy_type = (config_copy.interface_modes & PHYMOD_INTF_MODES_OTN) ? 2 : phy_type;
    link_type = _furia_get_link_type(config_copy.interface_type, config_copy.data_rate);
    ...
    for(lane_index = 0; lane_index < num_lanes; lane_index++) {
        if (((lane_map >> lane_index) & 1) == 0x1) {
            ...
            PHYMOD_IF_ERR_RETURN
                (_furia_set_intf_type(pa, phy_type, link_type, config_copy.interface_type, lane_index));
        }
    }
    ...
    if (pkg_side == SIDE_B) {
        pll_mode = curr_pll_mode[SIDE_B];
    }else{
        pll_mode = curr_pll_mode[SIDE_A];
    }
            ...
            if (((lane_map >> lane_index) & 1) == 0x1) {
                ...
                if((config_copy.interface_type != actual_intf) ||
                   (config_copy.ref_clock != actual_ref_clk)) {
                    PHYMOD_IF_ERR_RETURN
                        (_furia_set_intf_type(pa, phy_type_1, link_type_1, curr_intf[lane_index], lane_index));
    ...
    *ref_clk = 0; 
    ...
    if (!an_ctrl_reg.fields.auto_negotiationenable) {
        ...
        for(lane_index = 0; lane_index < num_lanes; lane_index++) {
            ...
            PHYMOD_IF_ERR_RETURN(
                _furia_get_intf_type(pa, phy_type, link_type, lane_index, &saved_intf));
            }
        }
    } else {
        PHYMOD_IF_ERR_RETURN
    ...
    switch(ref_clk_scaler_val) {
        case 0x0:
            ...
            *interface_modes = PHYMOD_INTF_MODES_OTN; 
        break;
        case 0x1:
...
STATIC uint16_t _furia_get_phy_type(phymod_interface_t intf, uint32_t speed)
{
    ...
    switch (intf) {
        ...
        case phymodInterfaceCAUI4_C2M:
        case phymodInterfaceXLAUI:
            if (((speed == SPEED_25G) || (speed == SPEED_27G) || (speed == SPEED_50G) || (speed == SPEED_53G)) &&
                ((intf == phymodInterfaceCR) || (intf == phymodInterfaceCR2))) {
                phy_type = 0x1; 
            } else {
                phy_type = 0x4; 
            }
...
STATIC uint16_t _furia_get_link_type(phymod_interface_t intf, uint32_t speed)
{
    ...
    switch (intf) {
        ...
        case phymodInterfaceCAUI4_C2C:
        case phymodInterfaceCAUI4_C2M:
            if (speed == SPEED_25G || speed == SPEED_27G) {
                link_type = 0x7;
...
int  _furia_get_intr_reg (const phymod_phy_access_t *phy, uint32_t intr_type, uint32_t *bit_pos, uint32_t *int_grp) {
    ...
    switch (intr_type) {
        case PHYMOD_INTR_AUTONEG_EVENT:
                ...
                PHYMOD_IF_ERR_RETURN(
			_furia_cfg_an_master_lane_get(pa,&an_master_lane));
		    intr = INT_CL73_AN_COMPLETE_0 + an_master_lane ;
                }else{
	 	    pkg_ln_des = _furia_pkg_ln_des_lane_swap(chip_id, pa, lane_index, 1);
		    PHYMOD_NULL_CHECK(pkg_ln_des);
...
int _furia_phy_reset_set(const phymod_access_t *pa, const phymod_phy_reset_t* reset)
{
    ...
    LINE_FALCON_IF_C_AND_R_CNTL_t c_and_r_ctrl;
    LINE_FALCON_IF_SOFT_MODE0_t soft_mode0;
                        ...
                        PHYMOD_IF_ERR_RETURN
                            ...
                            PHYMOD_IF_ERR_RETURN
                                (READ_FURIA_PMA_PMD_REG(pa,\
                                                        LINE_FALCON_IF_c_and_r_cntl_Adr,\
                                                        &c_and_r_ctrl.data));
                            c_and_r_ctrl.fields.pmd_ln_dp_h_rstb &= ~(1 << rx_pkg_ln_des->die_lane_num);
                            c_and_r_ctrl.fields.pmd_ln_dp_h_rstb |= (1 << rx_pkg_ln_des->die_lane_num);
                            PHYMOD_IF_ERR_RETURN
                                (WRITE_FURIA_PMA_PMD_REG(pa,\
                                                         LINE_FALCON_IF_c_and_r_cntl_Adr,\
                            ...
                            PHYMOD_IF_ERR_RETURN
                                (READ_FURIA_PMA_PMD_REG(pa,\
                                                        LINE_FALCON_IF_soft_mode0_Adr,\
                                                        &pwrdwn_ctrl.data));
                            soft_mode0.fields.software_mode_pmd_ln_dp_h_rstb  &= ~(1 << rx_pkg_ln_des->die_lane_num);
                            soft_mode0.fields.software_mode_pmd_ln_dp_h_rstb  |= (1 << rx_pkg_ln_des->die_lane_num);
                            PHYMOD_IF_ERR_RETURN
                                (WRITE_FURIA_PMA_PMD_REG(pa,\
                                                         LINE_FALCON_IF_soft_mode0_Adr,\
        ...
        case FURIA_PHYTYPE_FP:
        {
            switch (link_type) {
                ...
                case FURIA_LINKTYPE_5:
                    ...
                    if (phymod_interface_type == phymodInterfaceCR2) {
                        FURIA_REG_SET(gen_ctrls_intf_type.fields.intf_type_ln, lane_index, FURIA_IF_CR2);
                    }
                    break;
                default:
                    break;
...
STATIC int _furia_get_intf_type(const phymod_access_t *pa, uint16_t phy_type, 
        uint16_t link_type, int lane_index, uint16_t *saved_intf) 
{
    FUR_GEN_CNTRLS_micro_sync_7_t gen_ctrls_intf_type;
    uint32_t chip_id = 0;
    PHYMOD_MEMSET(&gen_ctrls_intf_type, 0 , sizeof(FUR_GEN_CNTRLS_micro_sync_7_t));
    ...
    for(lane_index = 0; lane_index < num_lanes; lane_index++) {
        if (((lane_map >> lane_index) & 1) == 0x1) {
            ...
            if (FURIA_IF_RX_SIDE(chip_id, pkg_side)) {
                ...
                l_pa.lane_mask = 0x1 << lane_index;
                PHYMOD_IF_ERR_RETURN
                    (furia_get_config_mode(&l_pa, &intf, &data_rate, &ref_clk, &interface_modes));
