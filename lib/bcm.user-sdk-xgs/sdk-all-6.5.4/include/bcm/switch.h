...
typedef enum bcm_switch_control_e {
    ...
    bcmSwitchHashUseFlowSelVxlanEcmp = 1009, /* Enable/Disable flow based hashing for
                                           VXLAN ECMP. */
    bcmSwitchHashEcmpBitsCount = 1010,  /* Hash Select for ECMP to determine the
                                           number of bits of the 16-bit ECMP
                                           hash value (valid range: 10 ~ 16). */
    bcmSwitchDefaultNativeOutVlanPort = 1011, /* Default native out VLAN Port. */
    bcmSwitchDefaultEgressVlanEditClassId = 1012, /* Default Vlan Edit Class Id for Egress
                                           VLAN translation. */
    bcmSwitchGtpDetectEnable = 1013,    /* Enable/disable the capability for GTP
                                           packet detection. */
    bcmSwitchGtpHdrFirstByte = 1014,    /* Setup the value for GTP Header First
                                           Byte. */
    bcmSwitchGtpHdrFirstByteMask = 1015, /* Setup the mask for GTP Header First
                                           Byte. */
    bcmSwitchHashGtpTeidEnable0 = 1016, /* Enable using the Tunnel endpoint id
                                           for hashing, for GTP pkts. */
    bcmSwitchHashGtpTeidEnable1 = 1017, /* Enable using the Tunnel endpoint id
                                           for hashing, for GTP pkts. */
    bcmSwitchEccSingleBitErrorEvent = 1018, /* Enable/Disable 1bit ECC error
                                           reporting for all the memories which
                                           are protected by ECC logic and whose
                                           Parity/ECC error could only be
                                           corrected by H/W. */
    bcmSwitchHeaderCompensationPerPacket = 1019, /* Set the header compensation of
                                           packets according to the MSBs of its
                                           CUD. */
    bcmSwitch__Count = 1020             /* Must be last.  Not a usable value. */
