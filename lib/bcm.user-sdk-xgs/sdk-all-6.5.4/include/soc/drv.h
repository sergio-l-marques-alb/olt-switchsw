...
#define SOC_IS_SAND(unit) (SOC_IS_DPP(unit) || SOC_IS_DFE(unit) || SOC_IS_DNX(unit))
...
typedef struct {
   ...
   int              lp_streams_per_slice_max; /* Max strreams per slice */
   int              lp_streams_per_port_max;  /* Max strreams per slice */
#endif
} soc_info_t;
...
#ifdef CRASH_RECOVERY_SUPPORT 
#define SOC_CR_ENABALED(unit) (SOC_UNIT_NUM_VALID(unit) && SOC_IS_JERICHO(unit) && SOC_IS_DONE_INIT(unit) && (SOC_CONTROL(unit))->crash_recovery)
#else
