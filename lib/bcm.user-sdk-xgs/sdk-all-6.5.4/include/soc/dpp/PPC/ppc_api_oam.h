...
#define SOC_PPC_OAM_OAMP_CRC_MASK_LSB_BIT_SIZE  (64)
#define SOC_PPC_OAM_OAMP_CRC_MASK_LSB_BYTE_SIZE (SOC_PPC_OAM_OAMP_CRC_MASK_LSB_BIT_SIZE / SOC_SAND_NOF_BITS_IN_BYTE)
#define SOC_PPC_OAM_OAMP_CRC_MASK_MSB_BYTE_SIZE (120)

typedef struct {
    uint8       msb_mask[SOC_PPC_OAM_OAMP_CRC_MASK_LSB_BYTE_SIZE];
    SHR_BITDCL  lsbyte_mask[_SHR_BITDCLSIZE(SOC_PPC_OAM_OAMP_CRC_MASK_MSB_BYTE_SIZE)];
} SOC_PPC_OAM_OAMP_CRC_MASK;

typedef struct {
    uint16                      crc16_val;      /* Calculated crc val. */
...
typedef struct {
    ...
    int                         mask_tbl_index; /* The index in the mask table where the mask will be stored. */
    int                         crc_tcam_index; /* The index in the flex crc tcam */
