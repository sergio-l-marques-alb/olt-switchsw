...
/* #define SOC_APACHE_NUM_MMU_PORTS_PER_PIPE               53 */
#define SOC_APACHE_NUM_MMU_PORTS_PER_PIPE               75

#define SOC_APACHE_56855_LOOPBACK_HIGIG_PORT_START      73
#define SOC_APACHE_56855_LOOPBACK_HIGIG_PORT_END        86
...
#if 0
typedef struct soc_apache_port_lanes_s {
...
} soc_apache_port_lanes_t;

extern int soc_apache_port_lanes_validate(int unit,
                                            soc_apache_port_lanes_t *lanes_ctrl);
extern int soc_apache_port_lanes_set(int unit,
                                       soc_apache_port_lanes_t *lanes_ctrl);
extern int soc_apache_port_lanes_get(int unit, soc_port_t port_base,
                                       int *cur_lanes);
#endif
...
extern soc_mem_t
_soc_apache_pmem(int unit, int port, soc_mem_t memx, soc_mem_t memy);

#define SOC_APACHE_PMEM(u,p,m1,m2)   _soc_apache_pmem((u),(p),(m1),(m2))

extern soc_reg_t
_soc_apache_preg(int unit, int port, soc_reg_t reg_x, soc_reg_t reg_y);

#define SOC_APACHE_PREG(u,p,m1,m2)   _soc_apache_preg((u),(p),(m1),(m2))
...
#define _SOC_APACHE_NODE_CONFIG_MEM(u,p,n)                                 \
    (((n)==SOC_APACHE_NODE_LVL_S1) ? \
        SOC_APACHE_PMEM((u),(p),LLS_S1_CONFIGm,    \
                             LLS_S1_CONFIGm) : \
     (((n) == SOC_APACHE_NODE_LVL_L0) ? \
        SOC_APACHE_PMEM((u),(p),LLS_L0_CONFIGm, \
                             LLS_L0_CONFIGm) : \
      ((n) == SOC_APACHE_NODE_LVL_L1 ? \
        SOC_APACHE_PMEM((u),(p),LLS_L1_CONFIGm, \
                             LLS_L1_CONFIGm) : -1  )))

#define _SOC_APACHE_NODE_CONFIG_MEM2(u,p,n)                                 \
    (((n)==SOC_APACHE_NODE_LVL_S1) ? \
        SOC_APACHE_PMEM((u),(p),LLS_S1_CONFIGm,    \
                             LLS_S1_CONFIGm) : \
     (((n) == SOC_APACHE_NODE_LVL_L0) ? \
...
#define _SOC_APACHE_NODE_PARENT_MEM(u,p,n)                                 \
    (((n)==SOC_APACHE_NODE_LVL_L2) ? \
        SOC_APACHE_PMEM((u),(p), LLS_L2_PARENTm, \
                              LLS_L2_PARENTm) : \
     (((n) == SOC_APACHE_NODE_LVL_L1) ? \
        SOC_APACHE_PMEM((u),(p), LLS_L1_PARENTm, \
                              LLS_L1_PARENTm) : \
      (((n) == SOC_APACHE_NODE_LVL_L0) ? \
       SOC_APACHE_PMEM((u),(p), LLS_L0_PARENTm, \
           LLS_L0_PARENTm) : \
       (((n) == SOC_APACHE_NODE_LVL_S1) ? \
        SOC_APACHE_PMEM((u),(p), LLS_S1_PARENTm, \
                              LLS_S1_PARENTm) : -1  ))))

#define _SOC_APACHE_NODE_WIEGHT_MEM(u,p,n)                                 \
    (((n)==SOC_APACHE_NODE_LVL_S1) ? \
        SOC_APACHE_PMEM((u),(p), LLS_S1_CHILD_WEIGHT_CFG_CNTm , \
                              LLS_S1_CHILD_WEIGHT_CFG_CNTm) : \
     (((n)==SOC_APACHE_NODE_LVL_L0) ? \
        SOC_APACHE_PMEM((u),(p), LLS_L0_CHILD_WEIGHT_CFG_CNTm , \
                              LLS_L0_CHILD_WEIGHT_CFG_CNTm) : \
     (((n) == SOC_APACHE_NODE_LVL_L1) ? \
        SOC_APACHE_PMEM((u),(p), LLS_L1_CHILD_WEIGHT_CFG_CNTm , \
                              LLS_L1_CHILD_WEIGHT_CFG_CNTm) : \
      (((n) == SOC_APACHE_NODE_LVL_L2) ? \
        SOC_APACHE_PMEM((u),(p), LLS_L2_CHILD_WEIGHT_CFG_CNTm , \
                              LLS_L2_CHILD_WEIGHT_CFG_CNTm) : -1 ))))
...
extern int
soc_ap_set_obm_registers(int unit, soc_reg_t reg, int speed,
                         int index, int obm_inst, int lossless, int
                         default_flag );


extern int
soc_ap_obm_prob_drop_default_set(int unit, int xlp, int index,
                                 int obm_inst, int default_flag);
extern
int soc_ap_obm_cut_through_threshold_set (int unit, int xlp, int index,
                                          int obm_inst, int default_flag);

extern int
soc_ap_mem_config(int unit, uint16 dev_id, uint8 rev_id);

extern int
soc_ap_get_shared_bank_size(int unit, uint16 dev_id, uint8 rev_id);

extern uint32*
soc_ap_mmu_params_arr_get(uint16 dev_id, uint8 rev_id);

extern int
tdm_ap_set_iarb_tdm_table(int core_bw, int is_x_ovs, int is_y_ovs,
...
extern int
soc_ap_get_prio_map(int unit, int port, uint16 *pri_mask);
