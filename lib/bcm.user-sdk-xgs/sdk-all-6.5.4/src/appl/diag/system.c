...
#if (defined(BCM_PETRA_SUPPORT) || defined(BCM_DNX_SUPPORT) || defined(BCM_DFE_SUPPORT))
#include <shared/bslenum.h>
#include <appl/diag/bsldnx.h>
#endif /* (defined(BCM_PETRA_SUPPORT) || defined(BCM_DNX_SUPPORT) || defined(BCM_DFE_SUPPORT)) */
#if defined(VXWORKS)
#include <vxWorks.h>
#include <sysLib.h>
#include <version.h>
...
diag_shell(void)
{
    ...
    for (i = 0; i < soc_ndev; i++) {
        ...
        if (soc_property_get_str(SOC_NDEV_IDX2DEV(i), "scache_nh") != NULL) {
            ...
            if ((SOC_CONTROL(SOC_NDEV_IDX2DEV(i)) != NULL) &&
               SOC_IS_TOMAHAWK(SOC_NDEV_IDX2DEV(i))) {
