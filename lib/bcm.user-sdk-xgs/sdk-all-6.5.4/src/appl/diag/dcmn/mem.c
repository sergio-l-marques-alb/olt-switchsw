...
STATIC int
dpp_active_intr_check(int unit, int inter)
{
...
dpp_mem_list_ser_action(int unit)
{
    ...
    for (inter = 0; inter < nof_interrupts; inter++) {
        ...
        if (dpp_active_intr_check(unit, inter)) {
...
cmd_dpp_mem_list(int unit, args_t *a)
{
    ...
    if ((tab != NULL) && (sal_strcasecmp(tab, "cache-table") == 0)) {
        bsl_log_add(BSL_APPL_SHELL, bslSeverityNormal, bslSinkIgnore, unit, "Current enabled cached memory: \n");
        if (soc_mem_iterate(unit, dpp_mem_list_cache, NULL) < 0) {
            LOG_ERROR(BSL_LS_APPL_SOCMEM, (BSL_META_U(unit, "ERROR: Get all cache table error.\n")));
            return CMD_FAIL;
        }
        return CMD_OK;
    }
