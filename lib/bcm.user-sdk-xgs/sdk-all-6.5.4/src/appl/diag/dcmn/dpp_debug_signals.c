...
static void *dpp_get_top(char *descr, char *filename, char *topname, int flags)
{
    ...
    if(ISEMPTY(db_dir) || (dataio_dir_exists(db_dir) != SOC_SAND_OK)) {
...
dpp_qual_signals_init(char *db_name, char *qual_signals_db, pp_stage_t *pp_stage)
{
    ...
    if ((curTop = dpp_get_top(db_name, qual_signals_db, "SignalInfo", 0)) == NULL) {
    ...
    RHDATA_ITERATOR(cur, curTop, "signal") {
        ...
        xml_get_content_str(cur, "verilog",cur_internal_signal->hw, RHFILE_MAX_SIZE);
        cur_internal_signal++;
    }
...
dpp_dsig_handle_get(device_t *device, char *block, char *from, char *to, char *name, debug_signal_t **signal_p)
{
    ...
    if((dsig_list = rhlist_create("prt_print", sizeof(signal_output_t), 0)) == NULL) {
        goto exit;
    }
    ...
    if(dpp_dsig_get_list(device, unit, core, &match, dsig_list) != _SHR_E_NONE) {
        ...
        if((signal_output = rhlist_get_first(dsig_list)) != NULL) {
...
static int dpp_dsig_init(device_t *device)
{
    ...
    if ((curTop = dpp_get_top(RHNAME(device), "PP", "top", 0)) == NULL) {
        res = _SHR_E_NOT_FOUND;
        goto exit;
    }
    ...
    RHDATA_ITERATOR(curBlock, curSubTop, "block") {
        ...
        signal_num = 0;
        if ((curSignalTop = dpp_get_top(RHNAME(device), cur_pp_block->debug_signals_n, "SignalInfo", 0)) == NULL) {
            res = _SHR_E_NOT_FOUND;
            goto exit;
        }
        ...
        /* loop through entries */
        RHDATA_ITERATOR(curSignal, curSignalTop, "Signal") {
            xml_get_content_str(curSignal, "Name", debug_signal->hw, RHFILE_MAX_SIZE);
            ...
            xml_get_content_int(curSignal, "Perm", &debug_signal->perm);
...
dpp_dsig_read_value(rhlist_t *field_list, char *full_name, uint32 *value, int size)
{
    ...
    tokens = dataio_split_mstr(full_name, ".", maxtokens, &realtokens);
                ...
                char *match_n, int flags, rhlist_t **field_list_p)
{
    ...
    tokens = dataio_split_mstr(match_n, ".", maxtokens, &realtokens);
    if(realtokens && (tokens == NULL)) {
        goto exit;
    }
...
dpp_dsig_get_list(device_t *device, int unit, int core, match_t *match_p, rhlist_t *dsig_list)
{
    ...
    for (i = 0; i < device->block_num; i++) {
        ...
        for (j = 0; j < cur_pp_block->signal_num; j++) {
            ...
            /* Now we can allocate output and fill it */
            if(rhlist_add_tail(dsig_list, debug_signals[j].attribute, RHID_TO_BE_GENERATED, &temp) == -SOC_SAND_ERR) {
                /* No more place any more - return with what you have until now */
                res = _SHR_E_MEMORY;
...
static int
dpp_dsig_expand_print(rhlist_t *field_list, int depth, rhlist_t *prt_list, int attr_offset, int flags)
{
    ...
    RHITERATOR(field_output, field_list) {
        ...
        dpp_dsig_expand_print(field_output->field_list, depth + 1, prt_list, attr_offset, flags);
    }
...
dpp_dsig_print(rhlist_t *dsig_list, int flags)
{
    ...
    attr_offset = PRT_GET_CUR_COLUMN;

    PRT_COPY("Attribute");
    PRT_COPY("Size");

    if (flags & SIGNALS_PRINT_VALUE) {
        value_offset = PRT_GET_CUR_COLUMN;
        PRT_COPY("Value");
    }
    ...
    if (flags & SIGNALS_PRINT_DETAIL) {
        PRT_COPY("Perm");
        PRT_COPY("HW");
        addr_offset = PRT_GET_CUR_COLUMN;
        PRT_COPY("High");
        PRT_COPY("Low");
        PRT_COPY("MSB");
        PRT_COPY("LSB");
    ...
    RHITERATOR(signal_output, dsig_list) {
        ...
        if(flags & SIGNALS_PRINT_DEVICE) {
            if(signal_output->device) {
                PRT_PRINT("%s", RHNAME(signal_output->device));
            }
            else {
                PRT_SKIP(1);
            }
        ...
        if(flags & SIGNALS_PRINT_CORE) {
            PRT_PRINT("%s(%d)", debug_signal->block_n, signal_output->core);
        }
        else {
        ...
        if (flags & SIGNALS_PRINT_DETAIL) {
            PRT_PRINT("%d", debug_signal->perm);
            PRT_PRINT("%s", debug_signal->hw);
            for (j = 0; j < debug_signal->range_num; j++) {
                if (j != 0) {
                    PRT_ALLOC_LINE(LINE_NONE);
                    PRT_SKIP(addr_offset);
                }
        ...
        if(dpp_dsig_expand_print(signal_output->field_list, 1, prt_list, attr_offset, flags) != _SHR_E_NONE) {
...
dpp_sigstruct_init(device_t *device)
{
    ...
    if ((curTop = dpp_get_top(RHNAME(device), temp, "top", 0)) == NULL) {
        res = _SHR_E_NOT_FOUND;
        goto exit;
    }
    ...
    cur_sigparam = device->sigparams = dataio_alloc(device->sigparam_num * sizeof(sigparam_t));
    ...
    cur_sigstruct = device->sigstructs = dataio_alloc(device->sigstruct_num * sizeof(sigstruct_t));
...
int dpp_device_init(rhlist_t **list_p)
{
    ...
    if ((curTop = dpp_get_top(NULL, "DNX-Devices", "top", 0)) == NULL) {
        res = _SHR_E_NOT_FOUND;
        goto exit;
    }

    if ((curSubTop = xml_get_first(curTop, "devices")) == NULL) {
        res = _SHR_E_NOT_FOUND;
        goto exit;
    }

    if((device_list = rhlist_create("devices", sizeof(device_t), 0)) == NULL) {
        res = _SHR_E_MEMORY;
        goto exit;
    }
    ...
    RHDATA_ITERATOR(cur, curSubTop, "device") {
        ...
        if((device = rhlist_get_entry_by_name(device_list, db_name)) == NULL) {
            /* No such db in the list */
            if(rhlist_add_tail(device_list, db_name, RHID_TO_BE_GENERATED, &temp) == -SOC_SAND_ERR) {
                /* No more place any more - return with what you have until now */
                goto exit;
            }
            device = temp;
            /* Create list of chips supported by this device */
            if((device->chip_list = rhlist_create("devices", sizeof(rhentry_t), 0)) == NULL) {
                res = _SHR_E_MEMORY;
                goto exit;
            }
        }

        if(rhlist_add_tail(device->chip_list, chip_n, RHID_TO_BE_GENERATED, &temp) == -SOC_SAND_ERR) {
            /* No more place any more - return with what you have until now */
            goto exit;
        }
    }
...
device_t *dpp_device_get(int unit)
{
    ...
    {
        RHCOND_ITERATOR(device, device_list) {
            if((rhlist_get_entry_by_name(device->chip_list, chip_n)) != NULL) {
                break;
            }
        }
    }
