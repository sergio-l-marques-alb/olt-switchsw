...
_phy_diag_portmod_dsc(int unit, bcm_pbmp_t pbmp,args_t *args)
{
    ...
    DPORT_BCM_PBMP_ITER(unit, pbmp, dport, port) {
        ...
        else if (IS_IL_PORT(unit, port) || IS_SFI_PORT(unit,port)){
            /* ILKN port and fabric port not use portmod */
            portmod_access_get_params_t_init(unit, &params);
            if (PHY_DIAG_DEV_INT == phy_unit) {
                params.phyn = 0;
            }
            if (PHY_DIAG_INTF_SYS == phy_unit_if) {
