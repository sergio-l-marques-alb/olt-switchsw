...
reg_mask_subset_apache(int unit, soc_regaddrinfo_t *ainfo, uint64 *mask)
{
    ...
    if (REG_PORT_ANY != port) {
        ...
        /* XLPORT registers - Only XL and XLB0 ports */
        if (((ainfo->reg >= XLPORT_CNTMAXSIZEr) &&
             (ainfo->reg <= XLPORT_XGXS_COUNTER_MODEr)) &&
            (!(IS_XL_PORT(unit, port) || IS_XLB0_PORT(unit, port)))) {
        ...
        } else if (SOC_BLOCK_IN_LIST(SOC_REG_INFO(unit, ainfo->reg).block, SOC_BLK_PORT)) {
            /* phy port */
...
rval_test_skip_reg(int unit, soc_regaddrinfo_t *ainfo, reg_data_t *rd) {
    ...
    if(SOC_IS_JERICHO(unit)) {
        ...
        /* list that we dont get the reset values back from the registers on jer emulation*/
        switch(ainfo->reg) {

            /* reset value are !=0 but cant be parsed from asic files(Daniel)*/
            case ECI_CM_PLL_CONFIGr:
            case ECI_UC_PLL_CONFIGr:


            /* not existing in jericho(yaar keren)*/
            ...
            case ILKN_SLE_RX_STATS_WT_BYTE_HIGHr:
            case ILKN_SLE_RX_STATS_WT_PKT_HIGHr:
            case ILKN_SLE_TX_CFGr:
            case ILKN_SLE_TX_DEBUG_0r:
            case ILKN_SLE_TX_LANEr:
            case ILKN_SLE_TX_STATS_WT_BYTE_HIGHr:
            case ILKN_SLE_TX_STATS_WT_PKT_HIGHr:
      ...
      /*  this registers  intialized by the driver*/           
        case CGM_SBUS_LAST_IN_CHAINr:
        case DRCH_REG_0087r:
        case EGQ_SBUS_LAST_IN_CHAINr:
        case EPNI_SBUS_LAST_IN_CHAINr:
