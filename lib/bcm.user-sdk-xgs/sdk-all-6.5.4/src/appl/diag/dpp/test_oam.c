...
static uint32 rmeps[2048];
static uint32 lmeps[2048];

/* Global Variables */
int test_oam_timeout_event_counts = 0;
int test_oam_timein_event_counts = 0;
int test_oam_gports[TEST_OAM_NUM_LIFS];
...
int
test_oam_updateLMEP(int unit,int index){
...
test_oam_init_and_surroundings(int unit, int *group_id) {
    ...
    ret = bcm_oam_init(unit);
    /* OAM might be already initialized */
    if (ret != BCM_E_INIT) {
        BCMDNX_IF_ERR_EXIT(ret);
        LOG_VERBOSE(BSL_LS_BCM_OAM,(BSL_META_U(unit,"OAM Inititialization:\t[DONE]\n")));
    }
...
int _vlan_level__to_name(int vlan, int level, int parity) {
...
/* Create endpoints */
   ...
   for (endpoint = 0; endpoint < numEndpoints; ++endpoint) {
      ...
      vlan = starting_vlan + endpoint / (num_levels * 2);
...
/* For every first endpoint per LIF, create VLAN gports */
      ...
      name1 = _vlan_level__to_name(vlan, level, 0);
      name2 = _vlan_level__to_name(vlan, level, 1);

      ep1.name = name1;
      ep1.vlan_port_id = vlan_port_id1;
      ep1.vlan = vlan;
      ep1.level = level;
      ep1.port = port_1;
      ep2.name = name2;
      ep2.vlan_port_id = vlan_port_id2;
      ep2.vlan = vlan;
      ep2.port = port_2;
      ep2.level = level;
...
/* Create conected endpoints on port1 and port2 */
      ...
      ret = test_oam_create_endpoint_with_rmep(unit, &ep1, &ep2, endpoint, group_id);
      BCMDNX_IF_ERR_EXIT(ret);
      
      ret = test_oam_create_endpoint_with_rmep(unit, &ep2, &ep1, endpoint, group_id);
      BCMDNX_IF_ERR_EXIT(ret);
...
/* Sleep to wait for the timeout*/
   sal_sleep(1);
   if (test_oam_timeout_event_counts <  (endpoint / 2) - 1) {
      cli_out("\nIncorrect number of timeouts. expected %d received %d\n", endpoint, test_oam_timeout_event_counts);
      return CMD_FAIL;
   }
...
/* Sleep to wait for the timein */
   sal_sleep(1);
   if (test_oam_timein_event_counts < ((endpoint / 2) - 1)) {
      cli_out("\nIncorrect number of timeins. expected %d received %d.\n", endpoint, test_oam_timein_event_counts);
      return CMD_FAIL;
   }
/* Here we update the period of numUpdate random endpoints */
   test_oam_timein_event_counts = 0;
   test_oam_timeout_event_counts = 0;

   for (update = 1; update <= numUpdate; ++update) {

      index = sal_rand() % endpoint;
      LOG_VERBOSE(BSL_LS_BCM_OAM, (BSL_META_U(unit, "#####################################\n\tUpdate %d, LMEP %d\n"), update, index));
      ret = test_oam_updateLMEP(unit, index);
      BCMDNX_IF_ERR_EXIT(ret);

/* Sleep to wait for the timeout*/
      sal_sleep(1);
      if (test_oam_timeout_event_counts != update) {
         cli_out("\nIncorrect number of timeouts. expected %d received %d\n", update, test_oam_timeout_event_counts);
         return CMD_FAIL;
      }

      LOG_VERBOSE(BSL_LS_BCM_OAM, (BSL_META_U(unit, "#####################################\n\tUpdate %d, RMEP %d\n"), update, index));
      ret = test_oam_updateRMEP(unit, index);
      BCMDNX_IF_ERR_EXIT(ret);

/* Sleep to wait for the timein*/
      sal_sleep(1);
      if (test_oam_timein_event_counts != update) {
         cli_out("\nIncorrect number of timeins. expected %d received %d\n", update, test_oam_timein_event_counts);
         return CMD_FAIL;
      }
   }
