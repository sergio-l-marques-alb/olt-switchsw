...
stream_get_pipe_bandwidth(int unit, uint32 flag)
{
    ...
    /* TH 2 */
        ...
        switch (freq) {
            case TH2_FREQ_1700: pipe_bandwidth = TH2_BW_1700; break;
            case TH2_FREQ_1625: pipe_bandwidth = TH2_BW_1625; break;
            case TH2_FREQ_1525: pipe_bandwidth = TH2_BW_1525; break;
            case TH2_FREQ_1425: pipe_bandwidth = TH2_BW_1425; break;
            case TH2_FREQ_1325: pipe_bandwidth = TH2_BW_1325; break;
            case TH2_FREQ_1275: pipe_bandwidth = TH2_BW_1275; break;
            case TH2_FREQ_1225: pipe_bandwidth = TH2_BW_1225; break;
            case TH2_FREQ_1133: pipe_bandwidth = TH2_BW_1133; break;
            case TH2_FREQ_1050: pipe_bandwidth = TH2_BW_1050; break;
            case TH2_FREQ_950:  pipe_bandwidth = TH2_BW_950;  break;
            case TH2_FREQ_850:  pipe_bandwidth = TH2_BW_850;  break;
            default: pipe_bandwidth = TH2_BW_1700; break;
        }
    }
    /* TD3 */
/*     else if (dev_id BCM56870_DEVICE_ID) {
        switch (freq) {
            case TD3_FREQ_1700: pipe_bandwidth = TD3_BW_1700; break;
            case TD3_FREQ_1625: pipe_bandwidth = TD3_BW_1625; break;
            ...
            case TD3_FREQ_1125: pipe_bandwidth = TD3_BW_1125; break;
            case TD3_FREQ_850:  pipe_bandwidth = TD3_BW_850;  break;
            default: pipe_bandwidth = TD3_BW_1700; break;
        }
    } */
    /* default */
    else {
...
stream_set_lpbk(int unit, pbmp_t pbmp, int loopback)
{
    ...
    if (lp_invalid == 0) {
        PBMP_ITER(pbmp, port) {
            if (port < SOC_MAX_NUM_PORTS) {
                BCM_IF_ERROR_RETURN(bcm_port_loopback_set(unit, port, loopback));
            }
        }
        /* soc_set_lpbk(unit, pbmp, loopback); */
    }
...
stream_gen_random_l3_pkt(uint8 *pkt_ptr, uint8 ipv6_en,
                         ...
                         uint8 mac_da[NUM_BYTES_MAC_ADDR],
                         uint8 mac_sa[NUM_BYTES_MAC_ADDR],
                         uint16 vlan_id, uint32 ip_da, uint32 ip_sa, uint8 ttl)
{
    int i, offset;
    uint32 crc;
 ...
 */
static void
stream_gen_ref_l2_pkt(int port, stream_integrity_t *pkt_intg,
 ...
 */
static void
stream_gen_ref_l3_pkt(int port, stream_integrity_t *pkt_intg, uint8 *rx_pkt,
                       uint8 *ref_pkt, uint32 *ref_pkt_size)
{
    uint32 offset, seq_id, pkt_size;
    uint8 ttl;
 ...
 */
static bcm_error_t
stream_gen_ref_pkt(int port, stream_integrity_t *pkt_intg,
                      uint8 *rx_pkt, uint8 *ref_pkt, uint32 *pkt_size)
{
    bcm_error_t rv = BCM_E_NONE;

    if (pkt_intg->type == PKT_TYPE_L2) {
        stream_gen_ref_l2_pkt(port, pkt_intg, rx_pkt, ref_pkt, pkt_size);
    } else if (pkt_intg->type == PKT_TYPE_IPMC ||
               pkt_intg->type == PKT_TYPE_L3UC) {
        stream_gen_ref_l3_pkt(port, pkt_intg, rx_pkt, ref_pkt, pkt_size);
    } else {
        rv = BCM_E_FAIL;
    }
...
stream_pktdma_tx(int unit, dv_t *dv, uint8 *pkt, uint16 cnt)
{
    ...
    while (channel_done == 0 && (timeout--) > 0) {
        stream_chk_dma_chain_done(unit, TX_CHAN, SOC_DMA_POLL_CHAIN_DONE,
                                  &channel_done);
    }
    if (channel_done == 0) {
        rv = BCM_E_FAIL;
...
if(0) {
            SOC_IF_ERROR_RETURN(READ_L3_MTU_VALUESm(
                unit, MEM_BLOCK_ANY, vlan, &l3_mtu_values_entry));
            soc_L3_MTU_VALUESm_field32_set(
                unit, &l3_mtu_values_entry, MTU_SIZEf, 63);
            SOC_IF_ERROR_RETURN(WRITE_L3_MTU_VALUESm(
                unit, MEM_BLOCK_ALL, vlan, &l3_mtu_values_entry));
}
    ...
    while (channel_done == 0 && (timeout--) > 0) {
        stream_chk_dma_chain_done(unit, RX_CHAN, SOC_DMA_POLL_CHAIN_DONE,
                                  &channel_done);
    }
    if (channel_done == 0) {
        rv = BCM_E_FAIL;
...
{
    int port, port_speed;
    int pipe, pipe_bandwidth;
    int lr_bandwidth, ov_bandwidth;
    bcm_error_t rv = BCM_E_NONE;
 ...
 */
static bcm_error_t
stream_calc_act_port_rate(int unit, uint32 rate_interval, uint64 *port_rate_act)
{
    int port;
    bcm_error_t rv = BCM_E_NONE;
    uint32 interval_len;
    uint64 *port_rate;
    sal_usecs_t *time_start, *time_end, dtime;
    uint64 *tpkt_start, *tpkt_end,
           *tbyt_start, *tbyt_end;
    uint64 rdata1, rdata2;
    uint32 ipg, preamble;
    uint64 tpkt_delta, tbyt_delta;
    uint64 interval_len_64;
    ...
    time_start = (sal_usecs_t *) sal_alloc(SOC_MAX_NUM_PORTS * sizeof(sal_usecs_t),
                                           "time_start");
    time_end   = (sal_usecs_t *) sal_alloc(SOC_MAX_NUM_PORTS * sizeof(sal_usecs_t),
                                           "time_end");
    ...
    sal_memset(time_start, 0, SOC_MAX_NUM_PORTS * sizeof(sal_usecs_t));
    sal_memset(time_end,   0, SOC_MAX_NUM_PORTS * sizeof(sal_usecs_t));
    for (port = 0; port < SOC_MAX_NUM_PORTS; port++) {
        COMPILER_64_ZERO(port_rate[port]);
    ...
    PBMP_ITER(PBMP_PORT_ALL(unit), port) {
        if (port < SOC_MAX_NUM_PORTS) {
            if (stream_get_reg_tpkt_tbyt(unit, port, &reg_tpkt, &reg_tbyt) !=
                BCM_E_FAIL) {
                time_start[port] = sal_time_usecs();
                (void) soc_reg_get(unit, reg_tpkt, port, 0, &rdata1);
                (void) soc_reg_get(unit, reg_tbyt, port, 0, &rdata2);
                COMPILER_64_SET(tpkt_start[port], COMPILER_64_HI(rdata1),
                                                  COMPILER_64_LO(rdata1));
                COMPILER_64_SET(tbyt_start[port], COMPILER_64_HI(rdata2),
                                                  COMPILER_64_LO(rdata2));
            }
        }
    }
    ...
    PBMP_ITER(PBMP_PORT_ALL(unit), port) {
        if (port < SOC_MAX_NUM_PORTS) {
            if (stream_get_reg_tpkt_tbyt(unit, port, &reg_tpkt, &reg_tbyt) !=
                BCM_E_FAIL) {
                time_end[port] = sal_time_usecs();
                (void) soc_reg_get(unit, reg_tpkt, port, 0, &rdata1);
                (void) soc_reg_get(unit, reg_tbyt, port, 0, &rdata2);
                COMPILER_64_SET(tpkt_end[port], COMPILER_64_HI(rdata1),
                                                COMPILER_64_LO(rdata1));
                COMPILER_64_SET(tbyt_end[port], COMPILER_64_HI(rdata2),
                                                COMPILER_64_LO(rdata2));
            }
        }
    }
    /* calculate actual rate for each port */
    PBMP_ITER(PBMP_PORT_ALL(unit), port) {
        if (port < SOC_MAX_NUM_PORTS) {
            if (IS_HG_PORT(unit, port)) {
                ipg = HG2_IPG;
            ...
            dtime = SAL_USECS_SUB(time_end[port], time_start[port]);
            COMPILER_64_SET(interval_len_64, 0, dtime);
            
            COMPILER_64_DELTA(tpkt_delta, tpkt_start[port], tpkt_end[port]);
            COMPILER_64_DELTA(tbyt_delta, tbyt_start[port], tbyt_end[port]);
            COMPILER_64_UMUL_32(tpkt_delta, (ipg + preamble));
            COMPILER_64_ADD_64(tbyt_delta, tpkt_delta);
            COMPILER_64_UMUL_32(tbyt_delta, 8);
            COMPILER_64_SET(port_rate[port], COMPILER_64_HI(tbyt_delta),
                                             COMPILER_64_LO(tbyt_delta));
            COMPILER_64_UDIV_64(port_rate[port], interval_len_64);
        }
    }
...
/*
 * Function:
 *      stream_chk_mib_counters_normal
 * Purpose:
 ...
 */
static bcm_error_t
stream_chk_mib_counters_normal(int unit, pbmp_t pbmp, int flag)
{
    uint32 port, j;
    ...
    good_reg_num = sizeof(good_counters) / sizeof(soc_reg_t);
    error_reg_num = sizeof(error_counters) / sizeof(soc_reg_t);
    cli_out("\n==================================================\n");
        ...
        if (port < SOC_MAX_NUM_PORTS) {
            if (flag != 0) {
                for (j = 0; j < good_reg_num; j++) {
                    if (SOC_REG_IS_VALID(unit, good_counters[j])) {
                        SOC_IF_ERROR_RETURN(
                            soc_reg_get(unit, good_counters[j], port, 0, &rdata));
                        if (COMPILER_64_IS_ZERO(rdata)) {
                            cli_out("*ERROR: (a) Counter %s "
                                    "has a zero value for port %0d\n",
                                    SOC_REG_NAME(unit, good_counters[j]), port);
                            rv = BCM_E_FAIL;
                        }
                    }
                }
            ...
            for (j = 0; j < error_reg_num; j++) {
                if (SOC_REG_IS_VALID(unit, error_counters[j])) {
                    SOC_IF_ERROR_RETURN(
                        soc_reg_get(unit, error_counters[j], port, 0, &rdata));
                    if (!COMPILER_64_IS_ZERO(rdata)) {
                        cli_out("*ERROR: (b) Counter %s "
                                "has a non zero value for port %0d\n",
                                SOC_REG_NAME(unit, error_counters[j]), port);
                        rv = BCM_E_FAIL;
                    }
                }
            }
...
stream_chk_mib_counters(int unit, pbmp_t pbmp, int flag)
{
    bcm_error_t rv = BCM_E_NONE;
    if (SOC_IS_TOMAHAWK(unit) || SOC_IS_TRIDENT2PLUS(unit)) {
        rv = stream_chk_mib_counters_normal(unit, pbmp, flag);
    }
    else {
        cli_out("*ERROR, unrecognized device, stream_chk_mib_counters()\n");
        rv = BCM_E_FAIL;
    }
    return rv;
}
...
{
    int cnt = 0, port, port_speed;
    uint32 mode, interval_len, jitter_lr, jitter_os, tolerance;
    uint64 min_rate, max_rate, margin_of_error;
    uint64 hundred_64;
    ...
    stream_calc_act_port_rate(unit, interval_len, port_rate_act);
    ...
    PBMP_ITER(rate_calc->pbmp, port) {
        if (port < SOC_MAX_NUM_PORTS) {
            ...
            /* max_rate */
            COMPILER_64_SET(max_rate, COMPILER_64_HI(port_rate_exp[port]),
                                      COMPILER_64_LO(port_rate_exp[port]));
            ...
            format_uint64_decimal(exp_rate_str, port_rate_exp[port], 0);
            format_uint64_decimal(act_rate_str, port_rate_act[port], 0);
            LOG_INFO(BSL_LS_APPL_TESTS,
...
stream_chk_pkt_integrity(int unit, stream_integrity_t *pkt_intg)
{
    ...
    rx_pkt  = sal_dma_alloc(MTU * sizeof(uint8), "rx_pkt");
    ...
    PBMP_ITER(pkt_intg->rx_pbmp, port) {
        if (port < SOC_MAX_NUM_PORTS) {
            ...
            for (j = 0; j < port_exp_pkt_cnt; j++) {
                ...
                if (stream_pktdma_rx(unit, rst_vlan, vlan, dv_rx, rx_pkt, MTU)
                    != BCM_E_NONE) {
                    /*                     
                ...
                if (stream_gen_ref_pkt(port, pkt_intg, rx_pkt, ref_pkt,
                                       &pkt_size) != BCM_E_NONE) {
