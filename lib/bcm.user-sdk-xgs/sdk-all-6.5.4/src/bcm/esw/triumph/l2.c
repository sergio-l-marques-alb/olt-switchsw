        ...
        if ( BCM_TRUNK_INVALID != tgid) {
            ...
            if (tid_is_vp_lag) {
                ...
                if (l2addr->flags & BCM_L2_REMOTE_TRUNK) {
                    ...
                    if (soc_feature(unit, soc_feature_l2_remote_trunk_overlay)) {
#ifdef VERDOR_BROADCOM
                        /* Workaround: shouldn't set REMOTE_TRUNK */
#endif
                    } else
#endif
                    {
    ...
    *cf_count = 0;
    ...
    for (bank = 0; bank < num_banks; bank++) {
        ...
        for (slot = 0; slot < entries_per_bucket; slot++) {
            ...
            if (!SOC_IS_TOMAHAWK(unit) && !SOC_IS_APACHE(unit) && SOC_IS_TD2_TT2(unit)) {
                BCM_IF_ERROR_RETURN
