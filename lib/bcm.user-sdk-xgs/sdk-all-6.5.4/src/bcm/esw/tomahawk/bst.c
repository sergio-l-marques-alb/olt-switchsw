    ...
    if (reg != INVALIDr) {
        ...
        for (idx = 0; idx < resInfo->num_stat_pp; idx++) {
            ...
            if (resInfo->flags & _BCM_BST_CMN_RES_F_RES_DEV) {
                /* For CFAP res, threshold share between instance(xpe) */
                thd_idx = 0;
            } else {
...
bcm_bst_th_init(int unit)
{
    ...
    granularity = 1;
    total_thd_count = idx_count; /* all xpe share one threshold */
    ...
    total_stat_count = NUM_XPE(unit) * idx_count; /* per xpe copy */
