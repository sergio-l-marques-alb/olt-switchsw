...
_bcm_bst_default_profile_init(int unit)
{
    ...
    if (SOC_IS_TRIDENT2(unit) || SOC_IS_TOMAHAWK(unit)) {
        ...
        for (ei = 0; ei < pool_num; ei++) {
            SOC_IF_ERROR_RETURN(_bcm_bst_cmn_profile_set_hw(unit, -1,
                ei, bcmBstStatIdIngPool, &profile, NULL));
        }
    }
#endif
    ...
    if (mem != INVALIDm) {
        ...
        if (op == _bcmBstOpGet) {
            *resval = soc_reg_field_get(unit, reg, rval, field);
            ...
            if(SOC_IS_TOMAHAWK(unit) &&(bid == bcmBstStatIdDevice)){
                _bcm_bst_resource_info_t *resInfo;
                int min_hw_idx,max_hw_idx, idx, stats_per_inst, inst, stat_idx;
                resInfo = _BCM_BST_RESOURCE(unit, bid);
                ...
                for (idx = min_hw_idx; idx <= max_hw_idx; idx++) {
                    ...
                    SOC_IF_ERROR_RETURN(
                    soc_reg32_set(unit, reg, inst, stat_idx, rval));
                }
            } else
#endif
            {
