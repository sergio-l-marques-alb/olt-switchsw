...
#endif /* (BCM_TRIDENT2_SUPPORT) || defined(BCM_METROLITE_SUPPORT) */
    /* ECMP max paths info */
    alloc_sz += sizeof(int);
...
_bcm_esw_l3_warm_boot_alloc(int unit)
{
    ...
    if (SOC_IS_TOMAHAWK(unit)) {
        int wb_l3_extended_size = 0;
        /* Get the required scache size */
        bcm_th_l3_extended_required_scache_size_get(unit, &wb_l3_extended_size);
        alloc_sz += wb_l3_extended_size;
    }
#endif /* BCM_TOMAHAWK_SUPPORT */
    ...
    if (SOC_IS_XGS3_SWITCH(unit) && soc_feature(unit, soc_feature_l3)) {
        ...
        if (!BCM_XGS3_L3_MPATH_EGRESS_IDX_VALID(unit, mpintf)) {
            ...
            return (BCM_E_PARAM);
                   ...
                   bcm_th_ecmp_group_is_rh(unit, mpintf - BCM_XGS3_MPATH_EGRESS_IDX_MIN)) {
            rv = bcm_th_ecmp_rh_multipath_get(unit, mpintf - BCM_XGS3_MPATH_EGRESS_IDX_MIN,
                                              intf_size,
                                              intf_array,
                                              intf_count,
                                              NULL);
        ...
        int intf_count, bcm_if_t *intf_array, int op, int count, bcm_if_t *intf)
{
    ...
    if (SOC_IS_XGS3_SWITCH(unit) && soc_feature(unit, soc_feature_l3)) {
        ...
        if (soc_feature(unit, soc_feature_ecmp_resilient_hash_optimized) &&
            ...
            rv = bcm_th_ecmp_rh_set_intf_arr(unit, intf_count, intf_array,
                                             grp_idx, ecmp->dynamic_size, tmp_max_paths);
            rv = bcm_th_ecmp_lb_mode_set(unit, grp_idx,
                                         BCM_ECMP_LB_MODE_RH);
...
STATIC int
_bcm_th_l3_ecmp_agm_update(int unit, bcm_if_t l3_ecmp_id, int agm_pool_id,
                           int agm_index)
{
    int rv, ecmp_index;
    uint32 mpath_flag;
    ...
    /* l3_ecmp group entry */
    SOC_IF_ERROR_RETURN
        (READ_L3_ECMP_COUNTm(unit, MEM_BLOCK_ANY, ecmp_index, &ecmpc_entry));
    soc_L3_ECMP_COUNTm_field32_set(unit, &ecmpc_entry, AGM_MONITOR_POOL_IDf,
                                   agm_pool_id);
    soc_L3_ECMP_COUNTm_field32_set(unit, &ecmpc_entry, AGM_MONITOR_IDf,
                                   agm_index);
    SOC_IF_ERROR_RETURN
        (WRITE_L3_ECMP_COUNTm(unit, MEM_BLOCK_ANY, ecmp_index, &ecmpc_entry));
                          ...
                          bcm_switch_agm_id_t agm_id)
{
    ...
    BCM_IF_ERROR_RETURN(_bcm_th_l3_ecmp_agm_update(unit, l3_ecmp_id, 0, 0));
    ...
    if (SOC_IS_XGS3_SWITCH(unit) && soc_feature(unit, soc_feature_l3)) {
        ...
        if (BCM_SUCCESS(rv) &&
            ...
            if (ecmp->dynamic_mode == BCM_L3_ECMP_DYNAMIC_MODE_RESILIENT) {
                L3_LOCK(unit);
                rv = bcm_th_ecmp_rh_multipath_get(unit,
                           (ecmp->ecmp_intf - BCM_XGS3_MPATH_EGRESS_IDX_MIN),
