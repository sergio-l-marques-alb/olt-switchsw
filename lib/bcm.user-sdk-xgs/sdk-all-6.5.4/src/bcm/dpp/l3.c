  ...
  )
{
    ...
    if (SOC_IS_JERICHO(unit)) {
        ...
        {
            if ((egr->counting_profile != BCM_STAT_LIF_COUNTING_PROFILE_NONE) && (egr->counting_profile >= SOC_TMC_CNT_LIF_COUNTING_NOF_PROFILES_PER_SOURCE)) 
            {
                BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("OutLIF-counting-profile %d is out of range."), egr->counting_profile));
            }
            local_out_lif_info.counting_profile_id = egr->counting_profile;
        } 
        else 
        {
    ...
    bcm_l3_intf_t *intf)
{
  ...
  if(enable && !(intf->l3a_flags & BCM_L3_TRILL_ONLY)) {
      ...
      if (SOC_IS_JERICHO_PLUS(unit)) {
          ...
          sal_memset(&outrif_profile,0,sizeof(QAX_PP_EG_ENCAP_ACCESS_OUT_RIF_PROFILE_ENTRY_FORMAT));

          outrif_profile.drop = out_vsi_info.drop;
          outrif_profile.oam_lif_set = out_vsi_info.oam_lif_set;
          outrif_profile.outlif_profile = out_vsi_info.outlif_profile;
          ret = _bcm_dpp_am_template_out_rif_profile_alloc(unit, &outrif_profile, &is_first, &outrif_profile_index);
          BCMDNX_IF_ERR_EXIT(ret);
          out_vsi_info.outrif_profile_index = outrif_profile_index;
          if (is_first) {
              ret = MBCM_PP_DRIVER_CALL(unit, mbcm_pp_eg_rif_profile_set, (unit, out_vsi_info.outrif_profile_index, &outrif_profile));
              if (ret != BCM_E_NONE) {
                  temp_ret = _bcm_dpp_am_template_out_rif_profile_dealloc(unit, outrif_profile_index, &is_first /* is_last has no meaning here*/);
                  BCMDNX_IF_ERR_EXIT(temp_ret);
                  BCMDNX_IF_ERR_EXIT(ret);
              }
          }
      }
    ...
    for (i=0; i<tables_num; i++) {
        ...
        if (is_table_initiated) {
            ...
            while (found==TRUE) {
                ...
                if ((table_ids[i] == SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_ROUTE) || (table_ids[i] == SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_ROUTE_SCALE_LONG_KAPS) || 
    ...
    bcm_l3_intf_t *intf)
{
    ...
    if (status == BCM_E_NONE)
    {
        ...
        if (status == BCM_E_NONE)
        {
            status = _bcm_ppd_rif_vsid_map_set(unit, intf, 1,(intf->l3a_flags & BCM_L3_REPLACE));
            if (status == BCM_E_NONE)
            {
                         ...
                         uint8 *is_found)
{
    ...
    if (is_found_tmp) {
        ...
        if (status == BCM_E_NONE) {
            status = _bcm_ppd_rif_vsid_map_set(unit, intf, 0,0);
            if (status == BCM_E_NONE) {
                if (SOC_IS_JERICHO_AND_BELOW(unit) && SOC_IS_ROO_ENABLE(unit)) {
    ...
    int ll_eep)
{
    ...
    if ((encap_entry_info[1].entry_type == SOC_PPC_EG_ENCAP_ENTRY_TYPE_MPLS_ENCAP)
       ...
       if (encap_entry_info[0].entry_type == SOC_PPC_EG_ENCAP_ENTRY_TYPE_IPV4_ENCAP) {
        ...
        for (index = 0; index < SOC_PPC_EG_ENCAP_DATA_INFO_MAX_SIZE; index++) {
    ...
    void *user_data)
{
    ...
    if ((flags & BCM_L3_IP6) && (SOC_DPP_CONFIG(unit)->pp.enhanced_fib_scale_prefix_length_ipv6_long == 0)) {
        LOG_VERBOSE(BSL_LS_BCM_L3,
                    (BSL_META_U(unit,
                                "IPv6: flags %x\n"), flags));
        BCM_RETURN_VAL_EXIT(BCM_E_UNAVAIL);
    ...
    void *user_data)
{
    ...
    if (flags & BCM_L3_IP6) {
#if defined(INCLUDE_KBP) && !defined(BCM_88030)  
        if (SOC_IS_JERICHO(unit) && !(soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "l3_ipv6_uc_use_tcam", 0)) && !ARAD_KBP_ENABLE_IPV6_UC) {
    ...
    uint8* mode)
{
    ...
    SOC_PPC_MYMAC_VRRP_INFO vrrp_info;
    ...
    if (vrrp_info.mode == SOC_PPC_MYMAC_VRRP_MODE_ALL_VSI_BASED) {
        ...
        *vsi = 256;
        if (vrrp_info.mode == SOC_PPC_MYMAC_VRRP_MODE_2K_VSI_BASED) {
            max_vlan = 2048;
        }
        else if (vrrp_info.mode == SOC_PPC_MYMAC_VRRP_MODE_1K_VSI_BASED) {
            max_vlan = 1024;
        }
        else if (vrrp_info.mode == SOC_PPC_MYMAC_VRRP_MODE_512_VSI_BASED) {
            max_vlan = 512;
        }
        else {
            max_vlan = 256;
            *mode = SOC_PPC_MYMAC_VRRP_MODE_256_VSI_BASED;
        }
    }
