                                   ...
                                   uint32 *ppd_flp_program)
{
    ...
    *ppd_flp_program = 0;
    ...
    case bcmFieldAppTypeMplsCoupling:
        ppdAppType = PROG_FLP_COUPLING_LSR;
        break;
                               ...
                               const char *prefix)
{
...
#endif  /* _BCM_SPECIAL_SHOW_FOR_RANGE */
    {
        ...
        case bcmFieldQualifyHeaderFormat:
            result = _bcm_dpp_field_HeaderFormat_ppd_to_bcm(COMPILER_64_LO(qualData.qualData),
                                        ...
                                        const uint64 valid)
{
    ...
    if (_BCM_DPP_FIELD_ENTRY_IS_ENTRY(entry)) {
        ...
        if (COMPILER_64_IS_ZERO(qual[index].qualMask) 
            ...
            LOG_DEBUG(BSL_LS_BCM_FP,
                      (BSL_META_U(unit,
                                  "unit %d PRESEL entry %d qualifier %d (%s) (%d (%s))"
                                  " removed since it has no significant bits\n"),
                       unit,
                       entry,
                       ppdQual,
            ...
            qual[index].hwType = BCM_FIELD_ENTRY_INVALID;
            qual[index].qualType = BCM_FIELD_ENTRY_INVALID;
        } else {
            LOG_DEBUG(BSL_LS_BCM_FP,
                      (BSL_META_U(unit,
                                  "unit %d PRESEL entry %d qualifier %d (%s) (%d (%s))"
                                        ...
                                        const uint64 valid)
{
    ...
    if (index < qualLimit) {
        ...
        FIELD_ACCESS.entryDe.entryCmn.entryQual.qualType.set(unit, entry, free, ppdQual);
        FIELD_ACCESS.entryDe.entryCmn.entryQual.qualData.set(unit, entry, free, edata); 
        FIELD_ACCESS.entryDe.entryCmn.entryQual.qualMask.set(unit, entry, free, emask);
        index = free;
    } else {
        BCMDNX_ERR_EXIT_MSG(BCM_E_RESOURCE,
                         (_BSL_BCM_MSG_NO_UNIT("unit %d entry %d does not have hw"
    ...
    if (COMPILER_64_IS_ZERO(entryDe_qualMask) 
        ...
        LOG_DEBUG(BSL_LS_BCM_FP,
                  (BSL_META_U(unit,
                   ...
                   hwType,
                   SOC_PPC_FP_QUAL_TYPE_to_string(hwType)));
        COMPILER_64_ZERO(entryDe_qualData);
        COMPILER_64_ZERO(entryDe_qualMask);
        FIELD_ACCESS.entryDe.entryCmn.entryQual.qualMask.set(unit, entry, index, entryDe_qualMask);
        FIELD_ACCESS.entryDe.entryCmn.entryQual.qualData.set(unit, entry, index, entryDe_qualData); 
                                  ...
                                  _bcm_dpp_field_grp_idx_t *entryGroup)
{
    ...
    if (_BCM_DPP_FIELD_ENTRY_IS_ENTRY(entry)) {
        ...
        *qualLimit = SOC_PPC_FP_NOF_QUALS_PER_DB_MAX;
         ...
         */
        BCMDNX_IF_ERR_EXIT(FIELD_ACCESS_PRESELINUSE_W.bit_get(unit, entry, &bit_value)) ;
        if (!bit_value) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_NOT_FOUND,
