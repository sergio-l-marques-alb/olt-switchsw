...
STATIC uint32
_bcm_dpp_am_local_outlif_bank_alloc(int unit, int flags, bcm_dpp_am_egress_encap_alloc_info_t *app_alloc_info, uint8 tag, uint8 ext_type, int *bank_id , uint8 *bank_found  );
...
#define _BCM_DPP_AM_EGRESS_LIF_ENTRY_TO_LOCAL_LIF(bank_id, offset)      (bank_id * _BCM_DPP_AM_EGRESS_LIF_NOF_ENTRIES_PER_HALF_BANK(unit)  + offset)

#define _BCM_DPP_AM_EGRESS_LIF_LOCAL_LIF_ID_TO_BANK_ID(lif_id)          ( lif_id /_BCM_DPP_AM_EGRESS_LIF_NOF_ENTRIES_PER_HALF_BANK(unit))
#define _BCM_DPP_AM_EGRESS_LIF_LOCAL_LIF_OFFSET_IN_BANK(lif_id)         (lif_id % _BCM_DPP_AM_EGRESS_LIF_NOF_ENTRIES_PER_HALF_BANK(unit))

#define _BCM_DPP_AM_EGRESS_TOP_BANK_TO_EEDB_BANK(unit, top_bank_id)                (_BCM_DPP_AM_EGRESS_LIF_NOF_EEDB_REGULAR_BANKS(unit) * 2 + top_bank_id / 2 )
#define _BCM_DPP_AM_EGRESS_TOP_BANK_TO_ELEMENT_OFFSET_IN_BANK(top_bank_id)   (_BCM_DPP_AM_EGRESS_LIF_NOF_ENTRIES_PER_HALF_BANK(unit) /2 * ( top_bank_id % 2 )  )
...
_bcm_dpp_am_local_inlif_alloc(int unit, int flags, bcm_dpp_am_local_inlif_info_t *lif_info)
{
    ...
    for (bank_id_iter = 0 ; bank_id_iter < _BCM_DPP_AM_INGRESS_LIF_NOF_BANKS ; bank_id_iter++) 
    {
        ...
        {
            rv = _bcm_dpp_am_local_inlif_internal_alloc(unit, inlif_tag, bank_id_iter,(lif_info->local_lif_flags) | BCM_DPP_AM_IN_LIF_FLAG_ALLOC_CHECK_ONLY,&local_inlif_id); /*bank_id_iter = bank to allocate LIF */
            if (rv == BCM_E_RESOURCE) 
            {
                ...
                BCMDNX_IF_ERR_EXIT(_bcm_dpp_am_local_inlif_internal_alloc(unit, inlif_tag, bank_id_iter,(lif_info->local_lif_flags),&local_inlif_id)); /*bank_id_iter = bank to allocate LIF */
                bank_found = TRUE;
...
_bcm_dpp_am_local_outlif_alloc(int unit, int flags, bcm_dpp_am_local_out_lif_info_t *lif_info)
{
    ...
    {
        rv = _bcm_dpp_am_local_outlif_bank_alloc(unit, lif_info->local_lif_flags, &(lif_info->app_alloc_info), outlif_tag, lif_ext_type, &bank_id, &bank_found);
        BCMDNX_IF_ERR_EXIT(rv); 
    }
...
_bcm_dpp_am_local_outlif_bank_alloc(int unit, int flags, bcm_dpp_am_egress_encap_alloc_info_t *app_alloc_info, uint8 tag, uint8 ext_type, int *bank_id , uint8 *bank_found  )
{
    ...
    for (bank_id_iter = 0 ; bank_id_iter < _BCM_DPP_AM_EGRESS_LIF_NOF_EEDB_HALF_BANKS(unit) ; bank_id_iter++) 
    {
        ...
        {
            if ( bcm_dpp_am_local_outlif_alloc_align(unit, bank_id_iter, ( flags | SW_STATE_RES_ALLOC_CHECK_ONLY ) ,&tag, 2,0,2,&tmp_elem)== BCM_E_NONE ) 
            {
                *bank_found = TRUE;
                *bank_id = bank_id_iter;
                break;
