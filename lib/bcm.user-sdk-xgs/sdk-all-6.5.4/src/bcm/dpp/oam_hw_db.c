        ...
        if (SOC_IS_ARAD_B0_AND_ABOVE(unit) && PPC_API_OAM_STORE_LOCAL_PORT_IN_MEP_DB(mep_db_entry->mep_type)) {
            if (_BCM_OAM_IS_SERVER_SERVER(endpoint_info) && _BCM_OAM_DISSECT_IS_ENDPOINT_UPMEP(endpoint_info)) {
                ...
                /* In this special case local port serves three purposes:
                   ...
                   3) This also serves as the LSB of the src mac address, which has already been verified to match the given value. */
                ...
                if (SOC_IS_JERICHO(unit)) {
                    ...
                    /* Store local port for inner PTCH on PE_GEN_MEM.
                       PRGE prgram will take it and paste it on the inner PTCH itself. */
                    ret = _bcm_dpp_am_template_oam_oamp_pe_gen_mem_alloc(unit, SHR_RES_ALLOC_WITH_ID, &local_port_on_inner_ptch, &ignored, &local_port);
                    BCMDNX_IF_ERR_EXIT(ret);

                    ret = MBCM_PP_DRIVER_CALL(unit, mbcm_pp_oam_oamp_gen_mem_set, (unit, local_port, local_port_on_inner_ptch));
                    BCMDNX_IF_ERR_EXIT(ret);
                }
