...
#define _DPP_AM_TEMPLATE_OAM_MEP_PROFILE_NON_ACC_COUNT_ARAD_PLUS (SOC_PPC_OAM_NON_ACC_PROFILES_ARAD_PLUS_NUM) /* Arad + only*/
...
#define _DPP_AM_TEMPLATE_OAM_LM_DA_TABLES_MAX_ENTITIES (SOC_PPC_OAM_MAX_NUMBER_OF_LOCAL_MEPS(unit)) 
...
#define _DPP_AM_TEMPLATE_IP_TUNNEL_ENCAPSULATION_MODE_COUNT        (4) /* support up to 4 encapsulation mode */
    ...
    break;
      ...
      case dpp_am_res_oam_sd_sf_profile_id:
        ...
        case dpp_am_res_vsq_src_port:
            if (!SOC_IS_ARADPLUS_AND_BELOW(unit)) {
                p_info.res_type = SW_STATE_RES_ALLOCATOR_BITMAP;
                p_info.count = SOC_TMC_ITM_VSQ_GROUPE_SZE(unit) - 1;
                p_info.start = 1;
        ...
        int *elem)
{
    ...
    *elem = trap_id;
    ...
    if ( (SOC_PPC_TRAP_CODE_USER_DEFINED_0 <= trap_id) && (trap_id <= _BCM_LAST_USER_DEFINE) ) { 
        ...
        */
        if( trap_id != SOC_PPC_TRAP_CODE_USER_DEFINED_0 )
        {
    ...
    else if (_BCM_TRAP_ID_IS_ETPP(trap_id))  /* etpp create */
    {
        ...
        int elem)
{
    ...
    if ( (SOC_PPC_TRAP_CODE_USER_DEFINED_0 <= trap_id) && (trap_id <= _BCM_LAST_USER_DEFINE) ) { 
        ...
        rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_POOL_IDX,dpp_am_res_trap_user_define,SW_STATE_RES_ALLOC_WITH_ID,trap_id);

        BCMDNX_IF_ERR_EXIT(rv);

        /*Sync rx.c traps allocations with port.c traps allocations (templates) for USER_DEFINED traps*/
        template = trap_id-SOC_PPC_TRAP_CODE_USER_DEFINED_0; /*template counting is from 0-59*/
       ...
       /*Clears virtual bit before allocation*/
        ...
        rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_POOL_IDX,dpp_am_res_trap_virtual,SW_STATE_RES_ALLOC_WITH_ID,trap_id);
        BCMDNX_IF_ERR_EXIT(rv);
    ...
    else if (_BCM_TRAP_ID_IS_ETPP(trap_id))  
    {
        ...
        trap_id = _bcm_dpp_rx_trap_id_to_etpp(trap_id);
        rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_POOL_IDX,dpp_am_res_trap_etpp,SW_STATE_RES_ALLOC_WITH_ID,trap_id);    
        BCMDNX_IF_ERR_EXIT(rv);
    }
        ...
        rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_POOL_IDX,dpp_am_res_trap_single_instance,SW_STATE_RES_ALLOC_WITH_ID,trap_id);    
        BCMDNX_IF_ERR_EXIT(rv);
...
int
bcm_dpp_am_oam_mep_id_is_alloced(
        int unit,
        uint32 mep_id)
{
    BCMDNX_INIT_FUNC_DEFS;
    BCM_RETURN_VAL_EXIT(dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_POOL_IDX, dpp_am_res_oam_mep_id, 1, mep_id));
                                             ...
                                             int *old_template, int *is_last, int *template,int *is_allocated)
{
    ...
    if(!(flags & BCM_DPP_AM_TEMPLATE_FLAG_CHECK)) {
        rc = dpp_am_template_exchange(unit, BCM_DPP_AM_DEFAULT_POOL_IDX, dpp_am_template_trap_reserved_mc,SHR_TEMPLATE_MANAGE_IGNORE_NOT_EXIST_OLD_TEMPLATE,data,*old_template,is_last,template,is_allocated);
        BCMDNX_IF_ERR_EXIT(rc);   
    ...
    else
    {
        ...
        rc = dpp_am_template_exchange_test(unit, BCM_DPP_AM_DEFAULT_POOL_IDX,dpp_am_template_trap_reserved_mc,SHR_TEMPLATE_MANAGE_IGNORE_NOT_EXIST_OLD_TEMPLATE,data,*old_template,is_last,template,is_allocated);
        BCMDNX_IF_ERR_EXIT(rc);
    }
...
int _bcm_dpp_am_template_oam_local_port_2_sys_port_alloc(int unit, int flags, const uint32* data, int *is_allocated, int *profile_ndx)
{
    ...
    if (flags & BCM_DPP_AM_FLAG_ALLOC_RESERVE) {
        ...
        if (rc==BCM_E_NONE && cur_internal_data.data == SOC_DPP_OAM_LOCAL_PORT_2_SYSTEM_PORT_RESERVED_VALUE) {
           ...
           _bcm_dpp_am_template_oam_local_port_2_sys_port_free(unit, *profile_ndx, &is_last);
        }
        internal_data.profile = *profile_ndx;
    } else {
        /* gulp!*/
...
int _bcm_dpp_am_template_l2_prog_trap_free(int unit, int is_virtual_traps_p_set, int type, int *template,int *is_last)
{
    ...
    *template = type;
    ...
    if (*is_last) {
        ...
        for (trap_index = 0; trap_index < _BCM_NUMBER_OF_VIRTUAL_TRAPS ; trap_index++)
        {
...
int _bcm_dpp_am_template_queue_discount_cls_init(int unit, int template_init_id,const SOC_TMC_ITM_CR_DISCOUNT_INFO* data_discount_cls)
{
    ...
    BCM_DPP_CORES_ITER(BCM_CORE_ALL, core) {
        result = dpp_am_template_allocate_group(unit, core, dpp_am_template_queue_discount_cls, flags, data_discount_cls, _DPP_AM_TEMPLATE_QUEUE_DISCNT_CLS_MAX_ENTITIES(unit), &is_allocated,&template);
        BCMDNX_IF_ERR_EXIT(result);
    }
exit:
    BCMDNX_FUNC_RETURN;
...
int bcm_dpp_am_template_ip_tunnel_encapsulation_mode_exchange(int unit, int old_encapsulation_mode, SOC_PPC_EG_ENCAP_IP_TUNNEL_SIZE_PROTOCOL_TEMPLATE_INFO* data, int *old_is_last, int *new_encapsulation_mode,int *is_allocated) {
   int rc = BCM_E_NONE;
