...
#define _BCM_PETRA_L2_LIMIT_FLAGS_SUPPORTED(unit)                       \
    (_BCM_PETRA_L2_LIMIT_FLAGS_SUPPORTED_ARAD)
#endif
...
#define _BCM_L2_CACHE_INDEX_INVALID   (0) /* -1 */
                                    ...
                                    )
{
    ...
    /* trap-code */
    new_val = action_profile->trap_code - SOC_PPC_TRAP_CODE_RESERVED_MC_0;
                                            ...
                                            int *mac_lsb_p,int *port_p, int *prog_profile_p,SOC_PPC_VSI_ID *vsi_p,int index)
{
    ...
    {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("index is not valid")));
    }
...
* Purpose:   Virtual trap can be created by 2 ways (unfortunately)
                    ...
                    In first case we must update the new trap code that was allocated be exchange tamplate (MC or PROG)
                    In second case we must update all parametes of virtual trap
                   Since virtual trap must have SW db to hold its parametrs (since we cant get trap strength for MC reserved)
...
* is_for_delete         - beeing called from delete cache
                ...
                int is_for_delete,bcm_dpp_rx_virtual_traps_t *virtual_traps_p)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    /*If virtual trap was not set means that trap is beeing created withou calling the bcm_rx_trap_type_create 
    ...
    if(virtual_trap_was_set == FALSE)
    {
        if (new_trap_allocated) /*In new trap was allocated by template exchange of MC reserved or Prog trap*/
        {
            ...
            /*New Trap no need to get whats writted in SW DB*/
                       ...
                       int is_for_delete)
{
    ...
    SOC_PPC_ACTION_PROFILE_clear(&action_info_clean);
    ...
    /* unsupported flags */
    if(addr->flags & (BCM_L2_CACHE_MIRROR|BCM_L2_CACHE_L3|BCM_L2_CACHE_REMOTE_LOOKUP|BCM_L2_CACHE_DESTPORTS|BCM_L2_CACHE_PROTO_PKT))
    {
    ...
    {
        /*In case the SOC property of exposed HW id is on we need SW id for following checks*/
        rv = _bcm_dpp_rx_trap_hw_id_to_sw_id(unit,trap_id,&trap_sw_id);
        BCMDNX_IF_ERR_EXIT(rv);
        mac_lsb = addr->mac[5];
        ...
        if (addr->flags & BCM_L2_CACHE_TUNNEL) {
            ...
            if ((trap_sw_id == SOC_PPC_TRAP_CODE_ETH_L2CP_PEER) || (trap_sw_id == SOC_PPC_TRAP_CODE_ETH_L2CP_DROP)) {
                BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Single action per API is supported")));
            }
        }
        else if ((BCM_GPORT_IS_TRAP(addr->dest_port)) && (trap_sw_id == SOC_PPC_TRAP_CODE_ETH_L2CP_PEER)){
            handle_type = SOC_PPC_VSI_L2CP_HANDLE_TYPE_PEER;
        }
        else if ((BCM_GPORT_IS_TRAP(addr->dest_port)) && (trap_sw_id == SOC_PPC_TRAP_CODE_ETH_L2CP_DROP)){
        ...
        if(is_for_delete == FALSE)
        {
            ...
            _bcm_dpp_l2_cache_index_values_set(unit,_BCM_L2_CACHE_INDEX_TYPE_VSI_RESERVED_MC,mac_lsb,vsi,is_virtual_traps_p_set,index_used));
        ...
        (addr->ethertype_mask == 0x0) && (addr->vlan_mask == 0)) 
    {
        ...
        BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_i) 
        {
            rv  = _bcm_dpp_am_template_l2_trap_rm_exchange(unit, BCM_DPP_AM_TEMPLATE_FLAG_CHECK, port_i, mac_lsb, &_bcm_trap_info,
                                                           &old_trap_code, &trap_is_last, &new_trap_code, &trap_is_first);
            BCMDNX_IF_ERR_EXIT(rv);
            break;
        }
        ...
        BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_i) 
        {
            ...
            rv  = _bcm_dpp_am_template_l2_trap_rm_exchange(unit, 0, port_i, mac_lsb, &_bcm_trap_info, &old_trap_code, &trap_is_last, &new_trap_code, &trap_is_first);
            BCMDNX_IF_ERR_EXIT(rv);
            ...
            /*First clean old (last template referance) and only than update configuration
                        Since we use template exchange the old and the new can be same template
                        If we clean old after new configuration than it will override the new config since its same profile/template*/
            if (rm_is_last && (old_trap_code != 0)) {
                /* clean-up previous configuration */
                action_info_clean.trap_code = (old_trap_code-1) + SOC_PPC_TRAP_CODE_RESERVED_MC_0;
                mc_key.reserved_mc_profile = old_profile;
                soc_sand_rv = soc_ppd_llp_trap_reserved_mc_info_set(soc_sand_dev_id, &mc_key, &action_info_clean);
                BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
            }
    ...
    /*is programmable trap */
        ...
        BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_i) {
            ...
            if (is_for_delete == FALSE) {
                ...
                /* De-alloc */
                _bcm_dpp_l2_cache_index_values_get(unit,0,0,0,&new_profile,0,index);

                /* this trap is enabled in the given trap, get it's content*/
                soc_sand_rv = soc_ppd_llp_trap_port_info_get(soc_sand_dev_id, core, pp_port, &port_trap_info);
                BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                if(port_trap_info.trap_enable_mask & (SOC_PPC_LLP_TRAP_PORT_ENABLE_PROG_TRAP_0 << new_profile)) {
            ...
            new_trap_code = new_profile + SOC_PPC_TRAP_CODE_PROG_TRAP_0;
            ...
            if ((is_for_delete == FALSE) && (port_trap_info.trap_enable_mask & (SOC_PPC_LLP_TRAP_PORT_ENABLE_PROG_TRAP_0 << new_profile))) {
                rv  = _bcm_dpp_am_template_l2_prog_trap_free(unit, is_virtual_traps_p_set, new_profile, &prog_old_profile, &prog_is_last);
                BCM_IF_ERROR_RETURN(rv);
            } else {
                if (is_for_delete == FALSE) {
                    port_trap_info.trap_enable_mask |= (SOC_PPC_LLP_TRAP_PORT_ENABLE_PROG_TRAP_0 << new_profile);
                } else {
                    port_trap_info.trap_enable_mask &= ~(SOC_PPC_LLP_TRAP_PORT_ENABLE_PROG_TRAP_0 << new_profile);
                }
                soc_sand_rv = soc_ppd_llp_trap_port_info_set(soc_sand_dev_id,core,pp_port,&port_trap_info);
            ...
            {
                BCM_SAND_IF_ERR_EXIT(
                _bcm_dpp_l2_cache_index_values_set(unit,_BCM_L2_CACHE_INDEX_TYPE_PROG,new_profile,port_i,is_virtual_traps_p_set,index_used));
            }/*In case of deletetion the index is deleted in bcm_petra_l2_cache_delete*/
            break;
        }
        return BCM_E_NONE;
    }
