...
#include <soc/dpp/QAX/QAX_PP/cmodel/cmodel_reg_access.h>
#endif
...
STATIC int _bcm_dpp_trap_create_additional_actions(int unit,bcm_rx_trap_t trap_type)
{
    ...
    switch (trap_type)
    {
        case bcmRxTrapEgTxMtuFilter:
            /*In case bcmRxTrapEgTxMtuFilter need to check that soc property for that trap was enabled
                Since to set this trap conditions bits need to be allocated in outlif profile and will be allocated only if SOC property is enabled
                The trap can be enabled only on jericho B0 and above devices*/
            if((SOC_IS_JERICHO_B0_AND_ABOVE(unit) == TRUE) &&
                (soc_property_get(unit, spn_TRAP_LIF_MTU_ENABLE, 0) == FALSE))
            {
                BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, 
                    (_BSL_BCM_MSG("Need to enable trap_lif_mtu_enable for bcmRxTrapEgTxMtuFilter to be created")));
            }
            else
            {
    ...
    int trap_id)
{
    ...
    if (_BCM_TRAP_ID_IS_ETPP(trap_id_sw)) {
        ...
        if(trap_id_etpp == SOC_PPC_TRAP_CODE_ETPP_MTU_FILTER) {
            BCMDNX_IF_ERR_EXIT(
            MBCM_PP_DRIVER_CALL(unit,mbcm_pp_eg_etpp_out_lif_mtu_check_set,(unit,FALSE)));
        }
    }
...
/*****************************************************************************
* Function:  bcm_petra_rx_lif_mtu_set
* Purpose:   This function is handle for bcm_rx_lif_mtu_set. Should set given MTU value to given LIF
                Will convert gport (which should be global LIF) -> local LIF (Should be Outlif) -> Outlif profile -> Mtu profile
                For MTU profile the MTU value will be set with the given mtu input
* Params:
* unit (IN) - Device number
* flags (IN)- reserved 0
* port (IN) - gport represent Global Lif
* mtu  (IN) - mtu value to set for Lif
* Return:    (int)
*******************************************************************************/
int bcm_petra_rx_lif_mtu_set(int unit, uint32 flags,bcm_port_t port,uint32 mtu)
{
    int is_local = FALSE;
    ...
    if(flags != 0)
    {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Flags has to be set to 0"))); 
    }
    ...
    BCMDNX_IF_ERR_EXIT(
    _bcm_dpp_am_template_lif_mtu_profile_exchange(unit,&mtu,&old_mtu_profile,
    ...
    BCM_SAND_IF_ERR_EXIT(
    soc_ppd_eg_encap_lif_field_set(unit,out_lif_id,
                SOC_PPC_EG_ENCAP_ENTRY_UPDATE_OUT_LIF_PROFILE,out_lif_profile));
    
    
    /*call soc to map Outlif profile to MTU profile*/
   BCMDNX_IF_ERR_EXIT(
   MBCM_PP_DRIVER_CALL(unit,mbcm_pp_eg_etpp_out_lif_mtu_map_set,(unit,out_lif_profile,new_mtu_profile,mtu)));
...
* Purpose:   Get the MTU value to which the Global LIF (Out-LIF) was mapped to
* Params:
* unit (IN) - Device number
* flags (IN)- reserved 0
* port (IN) - gport represent Global LIF
* mtu  (OUT) - Will hold the MTU value to which the Global LIF (Out-LIF) was mapped to
* Return:    (int)
*******************************************************************************/
int bcm_petra_rx_lif_mtu_get(int unit, uint32 flags,bcm_port_t port,uint32 *mtu)
{
    int is_local = FALSE;
    uint32 out_lif_profile = 0;
    int out_lif_id;
    ...
    BCMDNX_IF_ERR_EXIT(
    MBCM_PP_DRIVER_CALL(unit,mbcm_pp_eg_etpp_out_lif_mtu_map_get,(unit,out_lif_profile,mtu)));
exit:
   BCMDNX_FUNC_RETURN;
}
...
void cmodel_convert_rx_data_from_chars(int data_length_in_bits, unsigned char *data_chars, uint8 *data_binary)
{
...
void cmodel_build_rx_packet(int unit, uint32 ms_id, uint32 nof_signals, uint32 src_port, int len, unsigned char *buf, bcm_pkt_t *pkt)
{
    /* Currently there are no signals, only the packet's data. Take the data after the signal ID and data size */
    cmodel_convert_rx_data_from_chars(len, &(buf[8]), pkt->pkt_data->data);
...
STATIC void cmodel_rx_pkt_thread(void *param)
{
    ...
    while (1)
    {
        ...
        pli_cmodel_read_buffer(unit, &ms_id, &nof_signals, &src_port, &len, &(buf[0]));
