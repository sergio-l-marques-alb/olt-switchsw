...
int _bcm_oam_profile_single_opcode_set(int unit, SOC_PPC_OAM_MEP_PROFILE_DATA *profile_data, uint8 opcode, bcm_oam_endpoint_action_t *action, uint32 trap_code)	{
 ...
 */
int insert_event_into_htbl( int unit, shr_htb_hash_table_t htbl, _bcm_oam_event_and_rmep_info *eventt ){
...
int _bcm_dpp_oam_lif_table_set(int unit, SOC_PPC_LIF_ID lif_index, uint8 value) {
                    ...
                    if (event_id_bitmap[event_index]) {
                        ...
                        if (event_index == SOC_PPC_OAM_EVENT_RMEP_STATE_CHANGE) {
                            ...
                            if (event_ind !=-1) {
                                ...
                                rv = insert_event_into_htbl(unit, event_htbl, &event_found);
                                BCMDNX_IF_ERR_EXIT(rv);
                                /* reset "flag" */
                                event_ind =-1;
                            }
                                                  ...
                                                  void *user_data)
{
    ...
    rv = _bcm_dpp_gport_to_hw_resources(unit, endpoint_info->gport, _BCM_DPP_GPORT_HW_RESOURCES_GLOBAL_LIF_INGRESS | _BCM_DPP_GPORT_HW_RESOURCES_LOCAL_LIF_EGRESS, &gport_hw_resources);
...
int _bcm_oam_default_id_from_ep_id(int unit, int epid, ARAD_PP_OAM_DEFAULT_EP_ID *default_id)
{
    ...
    if (SOC_IS_ARAD_B1_AND_BELOW(unit)) {
        ...
        case BCM_OAM_ENDPOINT_DEFAULT_EGRESS1:
            if (SOC_PPC_OAM_NUMBER_OF_BITS_USED_BY_OUTLIF_PCP < 1) { 
        ...
        case BCM_OAM_ENDPOINT_DEFAULT_EGRESS2:
            ...
            if (SOC_PPC_OAM_NUMBER_OF_BITS_USED_BY_OUTLIF_PCP < 2) { 
        ...
        case BCM_OAM_ENDPOINT_DEFAULT_EGRESS3:
            /* coverity[result_independent_of_operands:false]*/
            if (SOC_PPC_OAM_NUMBER_OF_BITS_USED_BY_OUTLIF_PCP < 3) { 
...
int bcm_petra_oam_init(int unit) {
	...
	bcm_gport_t oamp_port[SOC_DPP_DEFS_MAX(NOF_CORES)], cpu_port, recycle_port, recycle_port_iter , lbm_recycle_port;
    int count_erp, snoop_cmnd, trap_code, trap_code_with_meta, fake_destination, core = SOC_CORE_INVALID;
    ...
    lbm_recycle_port = soc_property_get(unit, spn_OAM_RCY_PORT_UP, -1);
    if (lbm_recycle_port != -1) {
        BCM_PBMP_ITER(port_config.rcy, recycle_port_iter) {
            if (recycle_port_iter == lbm_recycle_port) {
                break;
            }
        }
        if (recycle_port_iter != lbm_recycle_port) {
...
int bcm_petra_oam_group_create(int unit, bcm_oam_group_info_t *group_info) {
    ...
    /* fully flexible 48B MAID format */
    if ((soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "maid48_enabled", 0)) && (group_info->flags & BCM_OAM_GROUP_FLEXIBLE_MAID_48_BYTE)) { 
...
STATIC int _bcm_oam_endpoint_local_create(int unit, bcm_oam_endpoint_info_t *endpoint_info_lcl) {
    ...
    if (SOC_IS_JERICHO(unit) && endpoint_info_lcl->type == bcmOAMEndpointTypeBHHPwe &&
...
int bcm_petra_oam_endpoint_action_set_internal(
    ...
    for (opcode = 0; opcode < SOC_PPC_OAM_ETHERNET_PDU_OPCODE_COUNT; opcode++) {
        if (BCM_OAM_OPCODE_GET(*action_lcl, opcode)) {
            ...
            if (!is_mip) {
                if (_BCM_OAM_DISSECT_CLASSIFICATION_BY_OAM2(unit,&classifier_mep_entry,id) && !is_default) {
                     rv = _bcm_oam_profile_single_opcode_set(unit, &(profile_data_acc.mep_profile_data), internal_opcode, action_lcl, trap_code);
                     BCMDNX_IF_ERR_EXIT(rv);
                } else { /* Modify only to non-acc profile */
                     rv = _bcm_oam_profile_single_opcode_set(unit, &(profile_data.mep_profile_data), internal_opcode, action_lcl, trap_code);
                     BCMDNX_IF_ERR_EXIT(rv);
                }
            } else { /*MIP*/
                /* In case of MIP, modify both directions */
                 rv = _bcm_oam_profile_single_opcode_set(unit, &(profile_data.mip_profile_data), internal_opcode, action_lcl, trap_code);
                 BCMDNX_IF_ERR_EXIT(rv);
                 rv = _bcm_oam_profile_single_opcode_set(unit, &(profile_data_passive.mip_profile_data), internal_opcode, action_lcl, trap_code2);
                 BCMDNX_IF_ERR_EXIT(rv);
            }
        }
    }
...
int _manage_DA_with_existing_entries_internal(int unit , int mep_id ,SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY * classifier_mep, uint32 msb, uint32 lsb, 
  ...
  uint32 user_header_egress_pmf_offset_0_dummy, user_header_egress_pmf_offset_1_dummy;
    ...
    if (is_dm && eth_1731_profile->dmm_rate==0) {
		...
		if ( SHR_BITGET(found_bitmap, SOC_PPC_OAM_MEP_TYPE_DM) ==0 && !(old_eth_data.report_mode && old_eth_data.dmm_rate ) )  {
    ...
    if (!_BCM_OAM_DISSECT_IS_CLASSIFIER_MEP_ACCELERATED(&classifier_mep_entry)) {
        ...
        if (SOC_IS_JERICHO(unit)) {
            ...
            if (report_mode && (old_eth1731_profile.report_mode == 0)) {
                ...
                soc_sand_rv = MBCM_PP_DRIVER_CALL(unit,mbcm_pp_oam_oamp_search_for_lm_dm,(unit, loss_ptr->id, &found));
                BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
...
int bcm_petra_oam_loss_get(int unit, bcm_oam_loss_t * loss_ptr) {
    ...
    if (SOC_IS_QAX(unit) || (!eth1731_profile_data.report_mode)) {
    ...
    else if (!eth1731_profile_data.report_mode) {
...
int bcm_petra_oam_loss_delete(int unit, bcm_oam_loss_t *loss_ptr) {
    ...
    if (!_BCM_OAM_DISSECT_IS_CLASSIFIER_MEP_ACCELERATED(&classifier_mep_entry)) {
        /* MEP is Not accelerated, only the classifier settings are required */
...
int bcm_petra_oam_delay_add(int unit, bcm_oam_delay_t *delay_ptr) {
   ...
   SOC_PPC_OAM_OAMP_LM_DM_MEP_DB_ENTRY_clear(&mep_db_entry);
    ...
    if ((SOC_IS_JERICHO(unit) && !SOC_IS_QAX(unit)) && (delay_ptr->period == 0) && ((delay_ptr->id & 0x7) == 0)) {
...
int bcm_petra_oam_ais_add(int unit, bcm_oam_ais_t * ais_ptr)
{
    ...
    if (ais_ptr->flags & BCM_OAM_AIS_WITH_AIS_ID) {
        ...
        if (SOC_IS_QAX(unit)) {
            ...
            rv = bcm_dpp_am_oam_mep_id_alloc(unit, 0, (uint32 *)&(ais_ptr->ais_id));
            BCMDNX_IF_ERR_EXIT(rv);
        } else {
   ...
   case bcmOamControlOampPuntPacketIntPri: 
      {
         ...
         if(SOC_IS_ARADPLUS(unit)){
            ...
            soc_sand_rv = WRITE_OAMP_CPUPORTr(unit, reg_above_64); 
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
         }
      }
      break;
   case  bcmOamControlOampCCMWeight:
      {
         ...
         if(SOC_IS_ARADPLUS(unit)){
            ...
            soc_sand_rv = WRITE_OAMP_ARBITER_WEIGHTr(unit, oamp_arbiter_weight);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
         }
...
_bcm_oam_diag_iterate_cb(int unit, sw_state_htb_key_t key, sw_state_htb_data_t user_data) {
    ...
    if ( (*(int*)key != -2  ) && ((mep_entry->mep_type == SOC_PPC_OAM_MEP_TYPE_ETH_OAM) || (mep_entry->mep_type == SOC_PPC_OAM_MEP_TYPE_Y1731_O_MPLSTP) ||
        (mep_entry->mep_type == SOC_PPC_OAM_MEP_TYPE_Y1731_O_PWE) ) ) { /*found an oam endpoint*/
...
int _bcm_dpp_oam_bfd_diagnostics_endpoint_by_id(int unit, int endpoint_id) {
    ...
    if ((mep_info.mep_type == SOC_PPC_OAM_MEP_TYPE_ETH_OAM) || (mep_info.mep_type == SOC_PPC_OAM_MEP_TYPE_Y1731_O_MPLSTP) ||
        (mep_info.mep_type == SOC_PPC_OAM_MEP_TYPE_Y1731_O_PWE)) { /*found an oam endpoint*/
        rv = bcm_petra_oam_endpoint_get(unit, endpoint_id, &endpoint_info);
        BCMDNX_IF_ERR_EXIT(rv);
        if (endpoint_info.flags & BCM_OAM_ENDPOINT_REMOTE) {
         ...
         if (((mep_info.counter >= config.source.pointer_range.start) && (mep_info.counter <= config.source.pointer_range.end)) &&
            ...
            if (config.source.engine_source == bcmStatCounterSourceIngressOam) { /* seek only for oam counter */
               ...
               if (mep_info.flags & SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY_FLAG_PCP) {
                  for (i = 0; i < 8; i++) {
                     crps_counters->crps_ingress_counter[i] = mep_info.counter - config.source.pointer_range.start + i;
                     crps_counters->crps_egress_counter[i] = mep_info.counter - config.source.pointer_range.start + i + 8;
                  }
               } else {
                  crps_counters->crps_ingress_counter[0] = mep_info.counter - config.source.pointer_range.start;
                  crps_counters->crps_egress_counter[0] = mep_info.counter - config.source.pointer_range.start + 1;
               }
               break;
            }
         }
      ...
      if (mep_info.flags & SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY_FLAG_PCP) {
         for (i = 0; i < 8; i++) {
            ...
            if (eg_val) {
               LOG_INFO(BSL_LS_BCM_OAM,
                        (BSL_META_U(unit,
                                    "Level: %d - Egress counter: %d.\t"),(i + 1), eg_val));
            } else {
               LOG_INFO(BSL_LS_BCM_OAM,
                        (BSL_META_U(unit,
                                    "Level: %d - Egress counter inactive.\t"),(i + 1)));
            }
            if (ing_val) {
               LOG_INFO(BSL_LS_BCM_OAM,
                        (BSL_META_U(unit,
      ...
      bcm_pkt_blk_t *packet_header)
{
    ...
    rv = arad_pmf_db_fes_user_header_sizes_get( unit,
                                                &user_header_0_size,
                                                &user_header_1_size,
                                                &user_header_egress_pmf_offset_0_dummy,
                                                &user_header_egress_pmf_offset_1_dummy);
    ...
    if (SOC_IS_JERICHO(unit) && (user_header_size !=0)) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Error: oam protection packet header can't be set togehter with user-defined header ")));
    }
