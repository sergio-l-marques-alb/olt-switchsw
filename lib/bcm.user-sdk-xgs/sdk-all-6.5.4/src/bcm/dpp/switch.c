...
int static _bcm_dpp_switch_L2LearnMode_set(int unit, int arg)
{
    ...
    if (!((arg & BCM_L2_LEARN_CPU) && SOC_MAC_LIMIT_PER_TUNNEL_DISABLE_GET(unit))) 
    {
     ...
     */
    if ((arg & BCM_L2_LEARN_CPU) &&
        SOC_MAC_LIMIT_PER_TUNNEL_DISABLE_GET(unit)) {
        if (SOC_IS_JERICHO(unit)) {
                                    ...
                                    bcm_switch_control_t bcm_type, int arg)
{
	...
	case bcmSwitchTrunkHashMPLSLabelBOS:
		...
		case BCM_HASH_MPLS_LABEL_BOS_SKIP:
			info.start_from_bos = 1;
			info.include_bos = (arg == BCM_HASH_MPLS_LABEL_BOS_START ? 1 : 0);/*Only valid when BOS search is enabled*/
			break;
		default:
			LOG_ERROR(BSL_LS_BCM_SWITCH,
						 (BSL_META_U(unit,
									 "unit %d: Invalid value(%d) specified "
									 "valid: BCM_HASH_MPLS_LABEL_BOS_START, BCM_HASH_MPLS_LABEL_BOS_SKIP or BCM_HASH_MPLS_LABEL_NONE\n"), unit, arg));
			rv = BCM_E_PARAM;
			break;
		}
		break;
                                    ...
                                    bcm_switch_control_t bcm_type, int *arg)
{
    ...
    case bcmSwitchTrunkHashPktHeaderSelect:
        ...
        break;
    case bcmSwitchTrunkHashMPLSLabelBOS:
		if (info.start_from_bos == 1) {
			*arg = (info.include_bos == 1 ? BCM_HASH_MPLS_LABEL_BOS_START : BCM_HASH_MPLS_LABEL_BOS_SKIP); 
		} else {
			*arg = BCM_HASH_MPLS_LABEL_NONE;
		}
        break;
    default:
        rv = BCM_E_PARAM;
...
int
_bcm_dpp_switch_ecmp_hash_global_set(int unit, bcm_switch_control_t bcm_type, 
                                      int arg)
{
    int                                    rv = BCM_E_NONE;
    ...
    uint32                               soc_sand_rv;
    uint32                              soc_sand_dev_id;
	int                                 max_hash_offset, max_hash_seed;
    ...
    case bcmSwitchECMPHashSeed:
        if (arg > max_hash_seed) {
           LOG_WARN(BSL_LS_BCM_SWITCH,
                    (BSL_META_U(unit,
                                "unit %d: ignoring upper bytes for hash seed.\n"), unit));
        }
        info.seed = arg & max_hash_seed; /* take only the one/two bytes */
        break;
...
int
_bcm_dpp_switch_ecmp_hash_global_get(int unit, bcm_switch_control_t bcm_type, 
                                      int *arg)
{
    int                                    rv = BCM_E_NONE;
    SOC_PPC_FRWRD_FEC_ECMP_HASH_GLOBAL_INFO    info;
                            ...
                            int *arg)
{
    ...
    case bcmSwitchUnknownMcastToCpu:
        rv = bcm_petra_vlan_control_get(unit, bcmVlanUnknownToCpu, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
...
/************************************************************************/
    ...
    case bcmSwitchUnknownIpmcAsMcast:
        if (SOC_IS_JERICHO(unit)) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_CONFIG, (_BSL_BCM_MSG("bcmSwitchUnknownIpmcAsMcast has no effect, "
                                                            "if a default vrf entry has been configured it will be used, "
                                                            "otherwise the packet is flooded on the VSI.")));
        }
        soc_sand_rv = soc_ppd_frwrd_ipv4_glbl_info_get(soc_sand_dev_id, &route_glbl_info);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        /* get clear hit on get*/
    ...
    case bcmSwitchMplsShortPipe:
        if (SOC_IS_ARADPLUS(unit)) {
            SOC_PPC_FRWRD_ILM_GLBL_INFO frwrd_ilm_glbl_info;
            SOC_PPC_FRWRD_ILM_GLBL_INFO_clear(&frwrd_ilm_glbl_info);
            soc_sand_rv = soc_ppd_frwrd_ilm_glbl_info_get(unit, &frwrd_ilm_glbl_info);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
            *arg = frwrd_ilm_glbl_info.short_pipe_enable;
            rv = BCM_E_NONE;
        } else {
    ...
    case bcmSwitchCrLastTransactionStatus:
        rv = sw_state_access[unit].dcmn.crash_recovery.transaction_status.get(unit, arg);
    case bcmSwitchCrCouldNotRecover:
        rv = sw_state_access[unit].dcmn.crash_recovery.not_recoverable_reason.get(unit, (dcmn_cr_no_support *) arg);
#endif
        break;
    ...
    case bcmSwitchSynchronousPortClockSourceDivCtrl:
        SOCDNX_IF_ERR_EXIT(soc_dpp_fabric_sync_e_enable_get(unit, 1 ,&fabric_synce_enable));
        if (fabric_synce_enable) {
            rv = MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_fabric_port_sync_e_divider_get, (unit, arg));
        } else {
                            ...
                            int arg)
{
    ...
    {
    case bcmSwitchUnknownMcastToCpu:
        rv = bcm_petra_vlan_control_set(unit, bcmVlanUnknownToCpu, arg);
        BCMDNX_IF_ERR_EXIT(rv);
        break;    
    case bcmSwitchWarmBoot:
    case bcmSwitchStableSelect:
...
/************************************************************************/
    ...
    case bcmSwitchL3UrpfMode:
...
#endif /* BCM_88660_A0 */        
        {
            ...
            if (curr_urpf_mode != arg) {
                ...
                if (SOC_IS_JERICHO(unit)){
                    if (bcmL3IngressUrpfStrict == arg) {
                        new_urpf_mode = 1;
                    }
                    rv = (MBCM_PP_DRIVER_CALL(unit, mbcm_pp_rif_global_urpf_mode_set,(unit, new_urpf_mode)));
					BCMDNX_IF_ERR_EXIT(rv);
                }
        ...
        if (SOC_IS_ARADPLUS(unit)) {
            ...
            if (arg < 0 || arg > 1) {
               BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("The only valid argument values for switch control"
                                                            " bcmSwitchMplsShortPipe are 0 and 1")));
            }

            soc_sand_rv = soc_ppd_frwrd_ilm_glbl_info_get(unit, &frwrd_ilm_glbl_info);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
            frwrd_ilm_glbl_info.short_pipe_enable = arg;
            soc_sand_rv = soc_ppd_frwrd_ilm_glbl_info_set(unit, &frwrd_ilm_glbl_info);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
            rv = BCM_E_NONE;
        } else {
            unsupported_switch = 1;
    ...
    case bcmSwitchSynchronousPortClockSourceDivCtrl:
        ...
        SOCDNX_IF_ERR_EXIT(soc_dpp_fabric_sync_e_enable_get(unit, 1 ,&fabric_synce_enable));
        if (fabric_synce_enable) {
          BCMDNX_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_fabric_port_sync_e_divider_set, (unit, arg)));
        } else {
...
/************************************************************************/
    case bcmSwitchCongestionCnm:
      if (SOC_IS_ARAD_A0(unit)) {
          BCMDNX_ERR_EXIT_MSG(SOC_E_UNAVAIL, (_BSL_SOC_MSG("CNM congestion isn't supported for Arad A0")));
      }
      else {
...
/************************************************************************/
    ...
    case bcmSwitchTrunkResilientHashConfig:
      if (SOC_IS_ARADPLUS(unit)) {
          ...
          rv = _bcm_dpp_switch_trunk_hash_global_get(unit, bcmSwitchTrunkHashConfig, &lag_hash_index);
          BCMDNX_IF_ERR_EXIT(rv);
          rv = _bcm_dpp_switch_ecmp_hash_global_get(unit, bcmSwitchECMPHashConfig, &ecmp_hash_index);
          BCMDNX_IF_ERR_EXIT(rv);

          BCMDNX_VERIFY(lag_hash_index != ecmp_hash_index);

          if (lag_hash_index == arg) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("The stateful load balacing LAG hash index must be different from the LAG hash index.\n")));
          }
          if (ecmp_hash_index == arg) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("The stateful load balacing LAG hash index must be different from the ECMP hash index.\n")));
          }
                                 ...
                                 int *arg)
{
    ...
    uint32    soc_sand_rv;
    int       port_i, core=SOC_CORE_INVALID;
    _bcm_dpp_gport_info_t gport_info;
...
/************************************************************************/
    ...
    case bcmSwitchMeterAdjust:
        if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
            soc_sand_rc = (MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_cnt_meter_hdr_compensation_get, (unit, core, pp_port, SOC_TMC_PORT_DIRECTION_INCOMING, arg))); 
            BCM_SAND_IF_ERR_EXIT(soc_sand_rc);
        } else {
    ...
    case bcmSwitchCounterAdjust:
        soc_sand_rc = (MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_cnt_meter_hdr_compensation_get,(unit, core, pp_port, SOC_TMC_PORT_DIRECTION_OUTGOING, arg)));
        BCM_SAND_IF_ERR_EXIT(soc_sand_rc);
        rv = BCM_E_NONE;
        break;
                                 ...
                                 int arg)
{
    ...
    SOC_PPC_LLP_TRAP_PORT_INFO llp_trap_port_info;
    uint32    soc_sand_rv;
    int port_i, core, is_last, is_tdm_queuing_on = 0;
    _bcm_dpp_gport_info_t gport_info;
    bcm_gport_t gport;
...
/************************************************************************/
    ...
    case bcmSwitchMeterAdjust:
        if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
            soc_sand_rc = (MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_cnt_meter_hdr_compensation_set, (unit, core, pp_port, SOC_TMC_PORT_DIRECTION_INCOMING, arg))); 
            BCM_SAND_IF_ERR_EXIT(soc_sand_rc);
        } else {
        ...
        break;
    case bcmSwitchCounterAdjust:
        soc_sand_rc = (MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_cnt_meter_hdr_compensation_set,(unit, core, pp_port, SOC_TMC_PORT_DIRECTION_OUTGOING, arg)));
        BCM_SAND_IF_ERR_EXIT(soc_sand_rc);
        rv = BCM_E_NONE;
        break;
    case bcmSwitchPrependTagEnable:
...
bcm_petra_switch_l3_protocol_group_set(int unit, uint32 group_members, bcm_l3_protocol_group_id_t group_id){
    ...
    for (i = DPP_SWITCH_L3_PROTOCOL_GROUP_LOWEST_VALID ; i < SOC_PPC_L3_NOF_PROTOCOL_GROUPS(unit) ; i++) {
        ...
        if (i == group_id || (internal_flags & curr_group_members)) {
            ...
            rv = sw_state_access[unit].dpp.bcm.l3.vrrp_protocol_set_counts.get(unit, group_id, &nof_members);
            BCMDNX_IF_ERR_EXIT(rv);
            if (nof_members) {
    ...
    int *eedb_bank_id)
{
    ...
    /*set property key and validate bank */
    switch (lif_property) {
    ...
    if (SOC_IS_JERICHO_PLUS(unit) && *eedb_bank_id < 0) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_FAIL, (_BSL_BCM_MSG("In QAX the outrifs are not in the eedb. This function can't be used with them.")));
    }
    ...
    bcm_switch_lif_property_config_t *lif_config)
{
    ...
    rv = bcm_petra_switch_lif_property_validate(unit, lif_property, lif_config, &eedb_bank_id);
    BCMDNX_IF_ERR_EXIT(rv); 

    /* In case Phase or extension, both half banks need to be set (per bank property)*/
    start_bank_id = eedb_bank_id;
    ...
    for (bank_id_iter = start_bank_id ; bank_id_iter <= end_bank_id ; bank_id_iter++ ) {
        ...
        case bcmLifPropertyOutGlobalLifRangeType :
            ...
            break;
        case bcmLifPropertyEEDBBankPhase:
            if (  bank_id_iter < ARAD_PP_EG_ENCAP_NOF_RIF_BANKS_PHASE(rif_max_id)  &&   lif_config->value !=  BCM_SWITCH_LIF_PROPERTY_EEDB_PHASE_2  ) 
            {
                BCMDNX_ERR_EXIT_MSG(BCM_E_FAIL, (_BSL_BCM_MSG(" lif property can not be set for bank allocated for RIF")));
            }
    ...
    bcm_switch_lif_property_config_t *lif_config)
{
    ...
    /* Validate input and get the eedb bank id. */
    rv = bcm_petra_switch_lif_property_validate(unit, lif_property, lif_config, &bank_id);
    BCMDNX_IF_ERR_EXIT(rv); 
