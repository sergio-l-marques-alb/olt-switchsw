/*
 * $Id: tscf.c,v 1.1.2.25 Broadcom SDK $
...
#include "../../../libs/phymod/chip/tscf/tier1/tefmod.h"
#include "../../../libs/phymod/chip/tscf/tier1/tefmod_device.h"
...
#define TSCF_IF_NULL   (1 << SOC_PORT_IF_NULL)
#define TSCF_IF_SR     (1 << SOC_PORT_IF_SR)
#define TSCF_IF_SR2  (1 << SOC_PORT_IF_MII)
#define TSCF_IF_SR4    (1 << SOC_PORT_IF_SR4)
#define TSCF_IF_LR4    (1 << SOC_PORT_IF_LR4)
#define TSCF_IF_KR     (1 << SOC_PORT_IF_KR)
#define TSCF_IF_KR2    (1 << SOC_PORT_IF_TBI)
#define TSCF_IF_KR4    (1 << SOC_PORT_IF_KR4)
#define TSCF_IF_CR     (1 << SOC_PORT_IF_CR)
#define TSCF_IF_CR2    (1 << SOC_PORT_IF_RGMII)
#define TSCF_IF_CR4    (1 << SOC_PORT_IF_CR4)
#define TSCF_IF_XFI    (1 << SOC_PORT_IF_XFI)
#define TSCF_IF_SFI    (1 << SOC_PORT_IF_SFI)
#define TSCF_IF_XLAUI  (1 << SOC_PORT_IF_XLAUI)
#define TSCF_IF_XGMII  (1 << SOC_PORT_IF_XGMII)
#define TSCF_IF_ILKN   (1 << SOC_PORT_IF_ILKN)
#define TSCF_IF_CAUI   (1 << SOC_PORT_IF_CAUI)
...
#define TSCF_40G_10G_INTF_ALL   (TSCF_IF_KR4 | TSCF_IF_XLAUI | TSCF_IF_CR4 | TSCF_IF_SR | \
                        ...
                        TSCF_IF_KR | TSCF_IF_CR | TSCF_IF_XFI | TSCF_IF_SFI | TSCF_IF_CAUI)
                 ...
                 phymod_phy_init_config_t  *pm_phy_init_config)
{
    ...
    */
    ...
    /*default leave the crc check on */
    core_init_config->firmware_load_method = 0x100;
    ...
    core_init_config->firmware_load_method &= 0xff00;
    ...
    if (local_line_interface > 0) {
        speed_config->line_interface = 1 << local_line_interface;
    }
...
_phy_tscf_init_pass2(int unit, soc_port_t port)
{
    ...
    for (idx = 0; idx < pmc->num_phys; idx++) {
        ...
        if ((pass2_flag != TSCF_INIT_PASS2)  && (PHYCTRL_INIT_STATE(pc) == PHYCTRL_INIT_STATE_PASS2)) {  
           ...
           if (core->init_config.firmware_load_method & 0xff00) {
                PHYMOD_CORE_INIT_F_FIRMWARE_LOAD_VERIFY_SET(&core->init_config); 
           } else {
                PHYMOD_CORE_INIT_F_FIRMWARE_LOAD_VERIFY_CLR(&core->init_config); 
           }
           core->init_config.firmware_load_method &= 0xff;
           if (!SOC_WARM_BOOT(unit)) {
               SOC_IF_ERROR_RETURN
...
phy_tscf_speed_set(int unit, soc_port_t port, int speed)
{
    ...
    speed_config.port_is_higig = (PBMP_MEMBER(SOC_HG2_PBM(unit), port) || PBMP_MEMBER(PBMP_HG_ALL(unit), port));
    ...
    pCfg->speed_config.port_is_higig = (PBMP_MEMBER(SOC_HG2_PBM(unit), port) || PBMP_MEMBER(PBMP_HG_ALL(unit), port));
                              ...
                              soc_port_ability_t *ability)
{
    ...
    speed_full_duplex|= PHYMOD_BAM_CL73_CAP_40G_CR2_GET(reg73_ability) ?SOC_PA_SPEED_40GB:0;
    speed_full_duplex|= PHYMOD_BAM_CL73_CAP_40G_KR2_GET(reg73_ability) ?SOC_PA_SPEED_40GB:0;
    speed_full_duplex|= PHYMOD_BAM_CL73_CAP_50G_CR2_GET(reg73_ability) ?SOC_PA_SPEED_50GB:0;
    speed_full_duplex|= PHYMOD_BAM_CL73_CAP_50G_KR2_GET(reg73_ability) ?SOC_PA_SPEED_50GB:0;
                               ...
                               soc_port_ability_t *ability)
{
    ...
    speed_full_duplex|= PHYMOD_BAM_CL73_CAP_20G_CR2_GET(reg73_ability) ?SOC_PA_SPEED_20GB:0;
    speed_full_duplex|= PHYMOD_BAM_CL73_CAP_20G_KR2_GET(reg73_ability) ?SOC_PA_SPEED_20GB:0;
    speed_full_duplex|= PHYMOD_BAM_CL73_CAP_25G_KR1_GET(reg73_ability) ?SOC_PA_SPEED_25GB:0;  
    speed_full_duplex|= PHYMOD_BAM_CL73_CAP_25G_CR1_GET(reg73_ability) ?SOC_PA_SPEED_25GB:0;
...
phy_tscf_interface_set(int unit, soc_port_t port, soc_port_if_t pif)
{
    ...
    if (TSCF_40G_10G_INTF(pif)) {
        ...
        if (pif <= 31) {
            intf |= 1 << pif;
        } else if (pif == SOC_PORT_IF_SR2) {
            intf |= TSCF_IF_SR2;
        } else if (pif == SOC_PORT_IF_CR2) {
            intf |= TSCF_IF_CR2;
        } else {
            intf |= TSCF_IF_KR2;
        }     
...
STATIC
int
tscf_sdk_poly_to_phymod_poly(uint32 sdk_poly, uint32 *phymod_poly){
    switch(sdk_poly){
...
tscf_hg2_codec_enable_set(soc_phymod_ctrl_t *pmc, uint32 value)
{
    ...
    for (idx = 0; idx < pmc->num_phys; idx++) {
        ...
        switch (value) {
            case 0: local_copy = phymodBcmHG2CodecOff;
                    break;
            case 1: local_copy = phymodBcmHG2CodecOnWith8ByteIPG;
                    break;
            case 2: local_copy = phymodBcmHG2CodecOnWith9ByteIPG;
                    break;
            default: local_copy = phymodBcmHG2CodecOff;
                    break;
        }
...
phy_tscf_control_get(int unit, soc_port_t port, soc_phy_control_t type, uint32 *value)
{
    ...
    switch(type) {
    case SOC_PHY_CONTROL_INTERFACE:
        rv = phy_tscf_interface_get(unit, port, value);
        break;
    /* PREEMPHASIS */
    case SOC_PHY_CONTROL_PREEMPHASIS_LANE0:
