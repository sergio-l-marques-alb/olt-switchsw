...
STATIC soc_error_t cmodel_memreg_read_response(int unit, int is_mem, uint32 data_len, uint32 *data)
{
    ...
    if( FD_ISSET(cmodelInfo->cmodelMemRegSocFd, &read_vect))
    {
        ...
        if(read_length < MEM_RESPONSE_HEADER_SIZE_CMODEL) {
            cli_out("pli_cmodel_read_buffer: could not read packet length\n");
            rv = SOC_E_FAIL;
            SOCDNX_IF_ERR_EXIT(rv);
        }
...
uint32 pli_cmodel_read_buffer(int devNo, uint32 *ms_id, uint32* nof_signals, uint32 *src_port, int *len, unsigned char *buf)
{
    ...
    LOG_VERBOSE(BSL_LS_SYS_VERINET,
                (BSL_META("pli_cmodel_read_buffer: sockfd=%d\n"), cmodelInfo->cmodelRxTxSocFd));
    ...
    while (1) {
        if(select(cmodelInfo->cmodelRxTxSocFd+1,&read_vect,(fd_set*)0x0,(fd_set*)0x0,NULL)<0)
        {
            if (errno == EINTR) {
                /*
                 * Interrupted by a signal such as a GPROF alarm so
    ...
    if( FD_ISSET(cmodelInfo->cmodelRxTxSocFd, &read_vect))
    {
        ...
        *nof_signals = long_val;
        ...
        LOG_INFO(BSL_LS_SYS_VERINET,
             (BSL_META("pli_cmodel_read_buffer: packet_length=%d ms_id=%d nof_signals=%d\n"), packet_length, *ms_id, *nof_signals));
        ...
        *len = packet_length-2*sizeof(uint32);
        ...
        if(readn(cmodelInfo->cmodelRxTxSocFd, buf, *len) < *len) {
            cli_out("pli_cmodel_read_buffer: could not read packet\n");
            goto fail;
        }
    }
    else
    {
        return -2; /* Time expire with no command */
    }
...
uint32 pli_cmodel_send_buffer(int devNo, uint32 ms_id, uint32 src_port, int len, unsigned char *buf, int nof_signals)
{
    ...
    LOG_INFO(BSL_LS_SYS_VERINET,
             (BSL_META("pli_cmodel_send_buffer: src_port=%d len=0x%x\n"), src_port, len));
    ...
    {
        cli_out("Error: pli_cmodel_send_packet data failed\n");
