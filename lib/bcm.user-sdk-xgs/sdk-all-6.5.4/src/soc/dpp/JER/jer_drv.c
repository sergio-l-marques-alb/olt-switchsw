...
soc_jer_ports_config(int unit) 
{
    ...
    for (port_i = 0; port_i < SOC_DPP_DEFS_GET(unit, nof_logical_ports); ++port_i) 
    {
        ...
        if (dcmn_device_block_for_feature(unit,DCMN_NIF_24_44_FEATURE)) {
        ...
        if (dcmn_device_block_for_feature(unit,DCMN_NIF_44_16_FEATURE)) {

            /*QUADs 6,7 and 12 are invalid*/
            if(SOC_PBMP_MEMBER(port_config.phy_pbmp, 25) ||
               SOC_PBMP_MEMBER(port_config.phy_pbmp, 26) ||
               SOC_PBMP_MEMBER(port_config.phy_pbmp, 27) ||
               SOC_PBMP_MEMBER(port_config.phy_pbmp, 28) ||
               SOC_PBMP_MEMBER(port_config.phy_pbmp, 29) ||
               SOC_PBMP_MEMBER(port_config.phy_pbmp, 30) ||
               SOC_PBMP_MEMBER(port_config.phy_pbmp, 31) ||
               SOC_PBMP_MEMBER(port_config.phy_pbmp, 32) ||
               SOC_PBMP_MEMBER(port_config.phy_pbmp, 49) ||
               SOC_PBMP_MEMBER(port_config.phy_pbmp, 50) ||
               SOC_PBMP_MEMBER(port_config.phy_pbmp, 51) ||
               SOC_PBMP_MEMBER(port_config.phy_pbmp, 52)) {
                SOCDNX_EXIT_WITH_ERR(SOC_E_FAIL, (_BSL_SOCDNX_MSG("device %s cant mapped to quads 6,7 and 12"),
                                                  soc_dev_name(unit)));
            }
        }

        if (dcmn_device_block_for_feature(unit,DCMN_NIF_48_4_FEATURE)) {
            ...
            if(SOC_PBMP_MEMBER(port_config.phy_pbmp, 25) ||
               ...
               SOC_PBMP_MEMBER(port_config.phy_pbmp, 31) ||
               SOC_PBMP_MEMBER(port_config.phy_pbmp, 32) ||
               SOC_PBMP_MEMBER(port_config.phy_pbmp, 33) ||
               SOC_PBMP_MEMBER(port_config.phy_pbmp, 34) ||
               SOC_PBMP_MEMBER(port_config.phy_pbmp, 35) ||
               SOC_PBMP_MEMBER(port_config.phy_pbmp, 36) ||
               SOC_PBMP_MEMBER(port_config.phy_pbmp, 37) ||
               SOC_PBMP_MEMBER(port_config.phy_pbmp, 38) ||
               SOC_PBMP_MEMBER(port_config.phy_pbmp, 39) ||
               SOC_PBMP_MEMBER(port_config.phy_pbmp, 40) ||
               SOC_PBMP_MEMBER(port_config.phy_pbmp, 41) ||
               SOC_PBMP_MEMBER(port_config.phy_pbmp, 42) ||
               SOC_PBMP_MEMBER(port_config.phy_pbmp, 43) ||
               SOC_PBMP_MEMBER(port_config.phy_pbmp, 44)) {
                SOCDNX_EXIT_WITH_ERR(SOC_E_FAIL, (_BSL_SOCDNX_MSG("device %s cant mapped to quads 6,7,8,9 and 10"),
                                                  soc_dev_name(unit)));
            }
        }

        if (dcmn_device_block_for_feature(unit,DCMN_NIF_40_4_FEATURE)) {
            ...
            if(SOC_PBMP_MEMBER(port_config.phy_pbmp, 25) ||
               ...
               SOC_PBMP_MEMBER(port_config.phy_pbmp, 51) ||
               SOC_PBMP_MEMBER(port_config.phy_pbmp, 52) ||
               SOC_PBMP_MEMBER(port_config.phy_pbmp, 53) ||
               SOC_PBMP_MEMBER(port_config.phy_pbmp, 54) ||
               SOC_PBMP_MEMBER(port_config.phy_pbmp, 55) ||
               SOC_PBMP_MEMBER(port_config.phy_pbmp, 56)) {
                SOCDNX_EXIT_WITH_ERR(SOC_E_FAIL, (_BSL_SOCDNX_MSG("device %s cant mapped to quads 6,7,8,9,10,12 and 13"),
                                                  soc_dev_name(unit)));
            }
        }

        /*ilkn restrictions for SKUs */
        if (dcmn_device_block_for_feature(unit,DCMN_2_ILKN_PORTS_FEATURE)) {
            if (port_config.interface == SOC_PORT_IF_ILKN &&
                port_config.protocol_offset >= 2) {
                SOCDNX_EXIT_WITH_ERR(SOC_E_FAIL, (_BSL_SOCDNX_MSG("device %s can use ilkn 0-1 only"),
                                                  soc_dev_name(unit)));
                } else if (port_config.interface == SOC_PORT_IF_ILKN && !port_config.is_kbp) {
                    SOCDNX_EXIT_WITH_ERR(SOC_E_FAIL, (_BSL_SOCDNX_MSG("device %s can use ilkn only for KBP"),
                                                  soc_dev_name(unit)));
                }
        }
        if (dcmn_device_block_for_feature(unit,DCMN_4_ILKN_PORTS_FEATURE)) {
            if (port_config.interface == SOC_PORT_IF_ILKN &&
    ...
    int unit)
{
    ...
    if (SOC_IS_QUX(unit)) {
        ...
        SOCDNX_IF_ERR_EXIT(WRITE_CMIC_SBUS_RING_MAP_112_119r(unit, 0x44554066));
        SOCDNX_IF_ERR_EXIT(WRITE_CMIC_SBUS_RING_MAP_120_127r(unit, 0x00000633));
