  ...
  )
{
    ...
    else if ((interface == SOC_PORT_IF_SAT) || (interface == SOC_PORT_IF_ILKN) || (interface == SOC_PORT_IF_RCY) || (interface == SOC_PORT_IF_CPU) || (is_master_port && is_nif))
    {
        if(interface == SOC_PORT_IF_ILKN) {
            SOCDNX_IF_ERR_EXIT(soc_port_sw_db_protocol_offset_get(unit, port, 0, &protocol_offset /* ilkn id */));
    ...
    if(interface != SOC_PORT_IF_ERP) {
        ...
        if((is_nif || SOC_PORT_IS_LB_MODEM(flags)) && 
           (is_skip_nif_for_lb != 1)) {
            ...
            if (SOC_IS_QAX(unit)) { /* SDK-90226: move code to QAX */
                uint32 field_zero = 0;
                uint32 num_ports_nbih, num_ports_nbil, num_lanes_per_nbi, nbi_inst;
                uint32 pm_num, pm_internal, nif_port;
                ...
                {
					SOCDNX_IF_ERR_EXIT(soc_port_sw_db_first_phy_port_get(unit,
								port, &phy_port /*one based*/));
					nif_port = phy_port;
					--nif_port; /*0-based*/
					SOCDNX_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit,
                ...
                SOCDNX_IF_ERR_EXIT(READ_EPNI_IFC_2_NIF_PORT_MAPm(unit,
                            EPNI_BLOCK(unit, core), egr_if, reg32));
                soc_mem_field_set(unit, EPNI_IFC_2_NIF_PORT_MAPm, reg32,
                        PM_NUMf, &pm_num);
                soc_mem_field_set(unit, EPNI_IFC_2_NIF_PORT_MAPm, reg32,
                        PM_INTERNAL_PORTf, &pm_internal);
                soc_mem_field_set(unit, EPNI_IFC_2_NIF_PORT_MAPm, reg32,
                        BICAST_VALIDf, &field_zero);
                soc_mem_field_set(unit, EPNI_IFC_2_NIF_PORT_MAPm, reg32,
                        NIF_PORT_NUMf, &nif_port);
                SOCDNX_IF_ERR_EXIT(WRITE_EPNI_IFC_2_NIF_PORT_MAPm(unit,
                            EPNI_BLOCK(unit ,core), egr_if, reg32));
            }
        }
    }
