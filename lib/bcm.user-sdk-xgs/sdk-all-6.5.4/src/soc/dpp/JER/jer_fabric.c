  ...
  )
{
    ...
    if (SOC_IS_JERICHO_PLUS_ONLY(unit))
    {
        if(SOC_DPP_CONFIG(unit)->arad->init.fabric.is_128_in_system)
        {
            ...
            SOCDNX_IF_ERR_EXIT(soc_reg_fields32_modify(unit, FDR_REG_012A_1r, REG_PORT_ANY, field_count, fields, values));
            SOCDNX_IF_ERR_EXIT(soc_reg_fields32_modify(unit, FDR_REG_012A_2r, REG_PORT_ANY, field_count, fields, values));
            SOCDNX_IF_ERR_EXIT(soc_reg_fields32_modify(unit, FDR_REG_012A_3r, REG_PORT_ANY, field_count, fields, values));
        }
        else
        {
    ...
    else if (!SOC_IS_KALIA(unit)) {/*elij - temporary bypass since this is part of Kalia part 3 code*/
...
*/
    ...
    if (SOC_IS_JERICHO_PLUS_ONLY(unit)) {
        ...
        field_val = (SOC_DPP_CONFIG(unit)->arad->init.fabric.connect_mode == SOC_TMC_FABRIC_CONNECT_MODE_MESH) ? 0x0 : 0x1;
    ...
    if ((fabric_pipe_map_type == soc_dcmn_fabric_pipe_map_triple_uc_hp_mc_lp_mc) && SOC_IS_JERICHO_AND_BELOW(unit)) {
        ...
        soc_reg_field_set(unit, FDA_FAB_18_SCH_WFQ_CFGr, &reg_val, FAB_18_SCH_WFQ_BACKPRESSURE_ENABLEf, 0x7);
        ...
        soc_reg_field_set(unit, FDA_FAB_36_SCH_WFQ_CFGr, &reg_val, FAB_36_SCH_WFQ_BACKPRESSURE_ENABLEf, 0x7);
  ...
  )
{
    ...
    /* 
     * Configure DTQs according to pipe configuration
     */
    ...
    SOC_SAND_OUT int             *divider)
{
    ...
    SOCDNX_IF_ERR_EXIT(READ_ECI_SYNCE_MASTER_PLL_CONFIGr(unit, reg_val));
    fsrd_block = soc_reg_above_64_field32_get(unit, ECI_SYNCE_MASTER_PLL_CONFIGr, reg_val, SYNCE_MASTER_PLL_CFG_REFRENCE_FABRIC_SELf);
  ...
  )
{
    ...
    if (is_master) {
        ...
        /*update eci register*/
        SOCDNX_IF_ERR_EXIT(READ_ECI_SYNCE_MASTER_PLL_CONFIGr(unit, reg_val));
        soc_reg_above_64_field32_set(unit, ECI_SYNCE_MASTER_PLL_CONFIGr, reg_val, SYNCE_MASTER_PLL_CFG_REFRENCE_FABRIC_SELf, fsrd_block);
        SOCDNX_IF_ERR_EXIT(WRITE_ECI_SYNCE_MASTER_PLL_CONFIGr(unit, reg_val));

        /*update fsrd register*/    
        soc_reg_field_set(unit, FSRD_SYNC_E_SELECTr, &reg_val2, MASTER_CLK_SELf,  inner_fsrd_port);
        ...
        /*update eci register*/
        SOCDNX_IF_ERR_EXIT(READ_ECI_SYNCE_SLAVE_PLL_CONFIGr(unit, reg_val));
        soc_reg_above_64_field32_set(unit, ECI_SYNCE_SLAVE_PLL_CONFIGr, reg_val, SYNCE_SLAVE_PLL_CFG_REFRENCE_FABRIC_SELf, fsrd_block);
        SOCDNX_IF_ERR_EXIT(WRITE_ECI_SYNCE_SLAVE_PLL_CONFIGr(unit, reg_val));

        /*update fsrd register*/
        soc_reg_field_set(unit, FSRD_SYNC_E_SELECTr, &reg_val2, SLAVE_CLK_SELf, inner_fsrd_port);
  ...
  )
{
    ...
    if (is_master) {
        ...
        SOCDNX_IF_ERR_EXIT(READ_ECI_SYNCE_SLAVE_PLL_CONFIGr(unit, reg_val));
        fsrd_block = soc_reg_above_64_field32_get(unit, ECI_SYNCE_SLAVE_PLL_CONFIGr, reg_val, SYNCE_SLAVE_PLL_CFG_REFRENCE_FABRIC_SELf);
  ...
  )
{
    ...
    if(is_master)
    {
        ...
        SOCDNX_IF_ERR_EXIT(READ_ECI_SYNCE_MASTER_PLL_CONFIGr(unit, reg_val));
        soc_reg_above_64_field32_set(unit, ECI_SYNCE_MASTER_PLL_CONFIGr, reg_val, field, set_val);
        SOCDNX_IF_ERR_EXIT(WRITE_ECI_SYNCE_MASTER_PLL_CONFIGr(unit, reg_val));
    }
    else
    {
        ...
        SOCDNX_IF_ERR_EXIT(READ_ECI_SYNCE_SLAVE_PLL_CONFIGr(unit, reg_val));
        soc_reg_above_64_field32_set(unit, ECI_SYNCE_SLAVE_PLL_CONFIGr, reg_val, field, set_val);
        SOCDNX_IF_ERR_EXIT(WRITE_ECI_SYNCE_SLAVE_PLL_CONFIGr(unit, reg_val));
    }
exit:
    SOCDNX_FUNC_RETURN;
  ...
  )
{
    ...
    reg = (is_master) ? ECI_SYNCE_MASTER_PLL_CONFIGr : ECI_SYNCE_SLAVE_PLL_CONFIGr;
  ...
  )
{
    ...
    if (SOC_IS_QUX(unit)) {
        SOCDNX_EXIT_WITH_ERR(SOC_E_UNAVAIL, (_BSL_SOCDNX_MSG("This function is not supported on QUX")));
    }
