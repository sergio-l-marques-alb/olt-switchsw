  ...
  )
{
    ...
    if (nof_bytes == JER_KAPS_AD_BUFFER_NOF_BYTES) 
    {
...
#ifdef CRASH_RECOVERY_SUPPORT
    if (JER_KAPS_IS_CR_MODE == 1) {
        SOCDNX_SAND_IF_ERR_EXIT(sw_state_access[unit].dcmn.crash_recovery.kaps_dirty.set(unit,1));
        SOCDNX_SAND_IF_ERR_EXIT(sw_state_access[unit].dcmn.crash_recovery.kaps_tbl_id.set(unit,table_id));
    } else
#endif
    {
...
#ifdef CRASH_RECOVERY_SUPPORT
    if (JER_KAPS_IS_CR_MODE == 1) {
        SOC_SAND_IF_ERR_EXIT(sw_state_access[unit].dcmn.crash_recovery.kaps_dirty.set(unit,1));
        SOC_SAND_IF_ERR_EXIT(sw_state_access[unit].dcmn.crash_recovery.kaps_tbl_id.set(unit,table_id));
    } else
#endif
    {
#ifdef JER_KAPS_TIME_MEASUREMENTS
    ...
    )
{
    ...
    do {
        ...
        res = arad_pp_dbal_kbp_buffer_to_entry_key(unit, dbal_table, kpb_e_info.prio_len, kpb_e_info.data, qual_vals_array + (*nof_entries)*SOC_PPC_FP_NOF_QUALS_PER_DB_MAX);
        ...
        if ((qual_vals_array[(*nof_entries)*SOC_PPC_FP_NOF_QUALS_PER_DB_MAX + vrf_qual_index].val.arr[0] == vrf_ndx) || (vrf_mode == 0))
        {
  ...
  )
{
    ...
    res =  jer_pp_kaps_data_buffer_to_kaps_payload_buffer_encode(unit,asso_data_buffer, JER_KAPS_AD_BUFFER_NOF_BYTES,asso_data);
...
uint32
  jer_pp_kaps_dma_byffer_to_kaps_payload_buffer_encode(
    SOC_SAND_IN  int                        unit,
    SOC_SAND_IN  uint32                     *asso_data_buffer,
    SOC_SAND_OUT uint8                      *asso_data
  )
{
    ...
    res =  jer_pp_kaps_data_buffer_to_kaps_payload_buffer_encode(unit,asso_data_buffer, JER_KAPS_DMA_BUFFER_NOF_BYTES,asso_data);
                                      ...
                                      SOC_SAND_OUT JER_KAPS_IP_TBL_ID       *kaps_table_id)
{
    ...
    if (((vrf == 0) && JER_KAPS_ENABLE_PUBLIC_DB(unit)) || !JER_KAPS_ENABLE_PRIVATE_DB(unit)) {
        ...
        case SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_RPF_KAPS:
            *kaps_table_id = JER_KAPS_IP_RPF_PUBLIC_IPV4_UC_TBL_ID;
            break;
        ...
        case SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_RPF_KAPS:
            *kaps_table_id = JER_KAPS_IP_RPF_PUBLIC_IPV6_UC_TBL_ID;
            break;
        ...
        case SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_KAPS:
            *kaps_table_id = JER_KAPS_IP_FWD_PRIVATE_IPV4_UC_TBL_ID;
            break;
        case SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_RPF_KAPS:
            *kaps_table_id = JER_KAPS_IP_RPF_PRIVATE_IPV4_UC_TBL_ID;
            break;
        case SOC_DPP_DBAL_SW_TABLE_ID_IPV4MC_KAPS:
        ...
        case SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_ROUTE:
            *kaps_table_id = JER_KAPS_IP_FWD_PRIVATE_IPV6_UC_TBL_ID;
            break;
        case SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_RPF_KAPS:
            *kaps_table_id = JER_KAPS_IP_RPF_PRIVATE_IPV6_UC_TBL_ID;
            break;
        case SOC_DPP_DBAL_SW_TABLE_ID_IPV6MC:
                                     ...
                                     SOC_SAND_IN    uint8 dbal_tbl_id)
{
    ...
    /* RPF tables are clones and can not be directly cleared, the FWD tables must be cleared instead */
                   ...
                   SOC_SAND_OUT SOC_SAND_SUCCESS_FAILURE    *success)
{
    ...
    uint32
        db_id=0, offset=0, word_off=0, mem_off=0, mem_len=0;
    ...
    *success = SOC_SAND_SUCCESS;
    ...
    mem_off = (word_off < 3) ? (JER_KAPS_DMA_BUFFER_NOF_BYTES - (word_off*4)) : 0;
    mem_len = (word_off < 3) ? JER_KAPS_DMA_BUFFER_NOF_BYTES : 4;
