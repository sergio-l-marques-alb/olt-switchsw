...
#define ARAD_PORTS_FEM_PP_PORT_VENDOR2_FIELD_2_SIZE (1)
...
#define ARAD_PORTS_VENDOR2_FIELD_0_MAX_VALUE (48)
#define ARAD_PORTS_VENDOR2_FIELD_1_MAX_VALUE (3)
#define ARAD_PORTS_VENDOR2_FIELD_2_MAX_VALUE (1)
...
#define ARAD_PORTS_VIRTUAL_PORT_PON_LIMITED_PORT  (7)
#define ARAD_PORTS_VIRTUAL_PORT_PON_MAX_TUNNEL_ID_IN_LIMITED_PORT (2047 - (2 * 256)) /* Once for rregular ports, once for TM-injected ports */
#define ARAD_PORTS_VIRTUAL_PORT_PON_NOF_ENTRIES_MODE0(local_port) \
    (local_port == ARAD_PORTS_VIRTUAL_PORT_PON_LIMITED_PORT ? (ARAD_PORTS_VIRTUAL_PORT_PON_MAX_TUNNEL_ID_IN_LIMITED_PORT+1):(ARAD_PORTS_VIRTUAL_PORT_PON_MAX_TUNNEL_ID_MODE0+1))
  ...
  )
{
  ...
  if_pon_pp_port_map_bypass = soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "pon_pp_port_mapping_bypass", 0);
  ...
  if (if_pon_pp_port_map_bypass == 0) {
      PBMP_ITER(pon_port_bm, local_port) {
          if (pon_tunnel_id_alloc_mode == 0) {
              max_tunnel_id = ARAD_PORTS_VIRTUAL_PORT_PON_NOF_ENTRIES_MODE0(local_port);
    ...
    /* PON - END */
      ...
      {    
        if (SOC_DPP_CONFIG_CUSTOM_PP_PORT_IS_VENDOR2()) {
             ...
             * Action is 11bits. Action should be : { U/M[0], SubType[3:0], DelLength[5:0] }
             */
            ...
            virtual_port_fem_field_select_map.virtual_port_fem_field_select[fem_bit_ndx] = 
                ...
                : ((fem_bit_ndx <= ARAD_PORTS_FEM_PP_PORT_VENDOR2_FIELD_1_MSB)?(fem_bit_ndx+ARAD_PORTS_FEM_PP_PORT_VENDOR2_FIELD_1_FEM_LSB-(ARAD_PORTS_FEM_PP_PORT_VENDOR2_FIELD_0_MSB+1))
                :(fem_bit_ndx+ARAD_PORTS_FEM_PP_PORT_VENDOR2_FIELD_2_FEM_LSB-(ARAD_PORTS_FEM_PP_PORT_VENDOR2_FIELD_1_MSB+1)));
        } else {
            /* 
  ...
  )
{
    ...
    pon_tunnel_id_alloc_mode = soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "pon_tunnel_id_alloc_mode", 1);
    if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "pon_pp_port_mapping_bypass", 0) == 0) {
        if (SOC_PBMP_MEMBER(pon_port_bm, local_port)) {
            if (pon_tunnel_id_alloc_mode == 0) {
                for (tunnel_id = 0; tunnel_id < ARAD_PORTS_VIRTUAL_PORT_PON_NOF_ENTRIES_MODE0(local_port); tunnel_id++) {
                ...
                for (tunnel_id = 0; tunnel_id < ARAD_PORTS_VIRTUAL_PORT_PON_MAX_TUNNEL_ID; tunnel_id++) {
                    res = READ_IHP_VIRTUAL_PORT_TABLEm(unit, IHP_BLOCK(unit, core), ARAD_PORTS_VIRTUAL_PORT_PON_ENCODED(local_port,tunnel_id), &tbl_data_32b);
                    SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 142, exit);
                    /* Set System-Port. The System-Port is set per Virtual Port */
                    soc_IHP_VIRTUAL_PORT_TABLEm_field32_set(unit, &tbl_data_32b, VIRTUAL_PORT_SRC_SYS_PORTf, sys_port);
                    res = WRITE_IHP_VIRTUAL_PORT_TABLEm(unit, IHP_BLOCK(unit, core), ARAD_PORTS_VIRTUAL_PORT_PON_ENCODED(local_port,tunnel_id), &tbl_data_32b);
        ...
        switch(header_type) {
            ...
            case ARAD_PORT_HEADER_TYPE_TDM:
                ...
                eg_tm_profile = ARAD_EGR_PROG_TM_PORT_PROFILE_TDM1;
                    if ((tdm_mode == ARAD_MGMT_TDM_MODE_TDM_OPT) || (SOC_IS_QAX(unit) && !(SOC_DPP_CONFIG(unit)->pp.port_extender_map_lc_exists) 
  ...
  )
{
    ...
    */
	...
	/* upmep LBM */ 
    if (soc_property_get(unit, spn_OAM_RCY_PORT_UP, -1) == pp_port) {
    ...
    if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "dedicated_recycle_port", 0)==pp_port &&  pp_port != 0) { 
    ...
    if ((direction_ndx == ARAD_PORT_DIRECTION_INCOMING)
        ...
        if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "pon_pp_port_mapping_bypass", 0) == 0) {
            if ((SOC_PBMP_MEMBER(pon_port_bm, port))) {
                if (pon_tunnel_id_alloc_mode == 0) {
                    for (tunnel_id = 0; tunnel_id < ARAD_PORTS_VIRTUAL_PORT_PON_NOF_ENTRIES_MODE0(port); tunnel_id++) {
                    ...
                    for (tunnel_id = 0; tunnel_id < ARAD_PORTS_VIRTUAL_PORT_PON_MAX_TUNNEL_ID; tunnel_id++) {
                        SOCDNX_IF_ERR_EXIT(READ_IHP_VIRTUAL_PORT_TABLEm(unit, IHP_BLOCK(unit, core), ARAD_PORTS_VIRTUAL_PORT_PON_ENCODED(port, tunnel_id), &tbl_data));
                        soc_IHP_VIRTUAL_PORT_TABLEm_field32_set(unit, &tbl_data, IN_PP_PORTf, pp_port);
                        SOCDNX_IF_ERR_EXIT(WRITE_IHP_VIRTUAL_PORT_TABLEm(unit, IHP_BLOCK(unit, core), ARAD_PORTS_VIRTUAL_PORT_PON_ENCODED(port, tunnel_id), &tbl_data));
                    }
                }
            }
        }
  ...
  )
{
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 10, exit, arad_sys_phys_to_local_port_map_get_unsafe( unit, sys_port, &fap_id, &port_id));

    SOC_SAND_SOC_IF_ERROR_RETURN(res, 20, exit, MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_mgmt_system_fap_id_get, (unit, &sys_fap_id_self)));

    if (SOC_DPP_IS_MODID_AND_BASE_MODID_ON_SAME_FAP(unit, fap_id, sys_fap_id_self) == 1)
    {
        SOC_SAND_SOC_IF_ERROR_RETURN(res, 30, exit, soc_port_sw_db_local_to_tm_port_get(unit, port_id, &tm_port, &core));
        SOC_SAND_SOC_IF_ERROR_RETURN(res, 40, exit, arad_port_src_sys_port_get_unsafe( unit, core, tm_port, &sys_logic_port_id));
        SOC_SAND_SOC_IF_ERROR_RETURN(res, 50, exit, arad_ports_logical_sys_id_parse( unit, sys_logic_port_id, &is_lag, &lag_id, &lag_member_id, &sys_phys_port_id));
    ...
    {
        SOC_SAND_SOC_IF_ERROR_RETURN(res, 60, exit, arad_ports_lag_get_unsafe( unit, lag_id, &in_prev_lag_info, &out_prev_lag_info));
        for (lag_member_id = 0; lag_member_id < out_prev_lag_info.nof_entries; ++lag_member_id)
        {
            SOC_SAND_SOC_IF_ERROR_RETURN(res, 70, exit, arad_irr_lag_mapping_tbl_get_unsafe( unit, lag_id, lag_member_id, &irr_lag_mapping_tbl_data));
...
uint32 alloc_reassembly_context_and_recycle_channel_unsafe(int unit, int core_id, uint32 out_pp_port, uint32 *channel, uint32 *reassembly_context)
{
  ...
  *channel = 0;
  ...
  /* Enable All-VLAN-member on outbound mirroring ports */
  {
      ...
      vtt_in_pp_port_config_tbl_data.all_vlan_member = 1;
...
uint32 release_reassembly_context_and_mirror_channel_unsafe(int unit, int core, uint32 out_pp_port, uint32 channel)
{
  ...
  res = MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_ports_mirrored_channel_and_context_map, (unit, core, 
                                             ARAD_PORTS_IF_UNMAPPED_INDICATION, ARAD_PORTS_IF_UNMAPPED_INDICATION, channel));
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 100, exit); 
  ...
  )
{
  ...
  if (pon_tunnel_id_alloc_mode == 0) {
    ...
    tbl_idx = ARAD_PORTS_VIRTUAL_PORT_PON_ENCODED(port_ndx,tunnel);
  }
    ...
    SOC_SAND_IN ARAD_PORTS_APPLICATION_MAPPING_INFO *info)
{
  ...
  if (info->type == ARAD_PORTS_APPLICATION_MAPPING_TYPE_PP_PORT_EXTENDER) 
  {
    ...
    if (PBMP_MEMBER(vendor_pp_port_bm, port_ndx)) {
      if (SOC_DPP_CONFIG_CUSTOM_PP_PORT_IS_VENDOR2()) {
          SOC_SAND_ERR_IF_ABOVE_MAX(
          (info->value.pp_port_ext.index & ((0x1L<<ARAD_PORTS_FEM_PP_PORT_VENDOR2_FIELD_0_SIZE) - 1)), ARAD_PORTS_VENDOR2_FIELD_0_MAX_VALUE,
            SOC_TMC_INPUT_OUT_OF_RANGE, 80, exit);
          SOC_SAND_ERR_IF_ABOVE_MAX(
          ((info->value.pp_port_ext.index>>(ARAD_PORTS_FEM_PP_PORT_VENDOR2_FIELD_0_MSB+1))&((0x1L<<ARAD_PORTS_FEM_PP_PORT_VENDOR2_FIELD_1_SIZE)-1)),
            ARAD_PORTS_VENDOR2_FIELD_1_MAX_VALUE, SOC_TMC_INPUT_OUT_OF_RANGE, 81, exit);
          SOC_SAND_ERR_IF_ABOVE_MAX((info->value.pp_port_ext.index>>(ARAD_PORTS_FEM_PP_PORT_VENDOR2_FIELD_2_MSB+1)),
            0, SOC_TMC_INPUT_OUT_OF_RANGE, 82, exit);
   ...
   )
{
    ...
    if(SOC_PORT_IF_ERP != interface) {
        ...
        /* in order to support dynamic enable/disable */
        if (port_extender_enable) {
            ...
            if ((header_type_in == SOC_TMC_PORT_HEADER_TYPE_ETH) ||
                ...
                if (SOC_INFO(unit).ether.num > 0) {
                    SOC_INFO(unit).ether.num--;
                }
            }
            if (header_type_in == SOC_TMC_PORT_HEADER_TYPE_STACKING) {
                if (SOC_INFO(unit).st.num > 0) {
                    SOC_INFO(unit).st.num--;
                }
            }
            if (SOC_PORT_IF_OLP != interface && SOC_PORT_IF_OAMP != interface) {
                if ((header_type_out == SOC_TMC_PORT_HEADER_TYPE_TDM) || (header_type_out == SOC_TMC_PORT_HEADER_TYPE_TDM_RAW)) {
                    if (SOC_INFO(unit).tdm.num > 0) {
                        SOC_INFO(unit).tdm.num--;
                    }
                }
            }
        }
    ...
    if (((SOC_DPP_CONFIG(unit))->pp.port_extender_map_lc_exists == 2) && (header_type_outgoing == ARAD_PORT_HEADER_TYPE_ETH)) {
        if (soc_mem_field32_get(unit, EGQ_PCTm, entry_context, MIRROR_ENABLEf)) {
            if ((value != 0) && (SOC_PORT_IS_COE_PORT(flags))) {
                ...
                ports_swap_info.enable = TRUE;
                SOCDNX_IF_ERR_EXIT(arad_ports_swap_set_unsafe(unit, rcy_port, &ports_swap_info));                
