...
soc_dpp_arad_str_prop_mc_tbl_mode(int unit)
{
    ...
    if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
        ...
        if (SOC_IS_QAX(unit)) {
            ...
            if (SOC_DPP_CONFIG(unit)->tm.nof_ingr_mc_ids + SOC_DPP_CONFIG(unit)->core_mode.nof_active_cores * 
...
soc_arad_default_config_l3_get(int unit)
{
    ...
    int    i;
    uint32 ipmc_mode;
    uint32 ipmc_l3mcastl2_mode, ipv4_double_capacity;
    ...
    if(SOC_IS_JERICHO(unit)){
        ...
        ipv4_double_capacity = soc_property_get(unit, spn_EXT_IP4_DOUBLE_CAPACITY_FWD_TABLE_SIZE, 0);

        if((ipmc_l3mcastl2_mode == 1) && (ipv4_double_capacity == 1)) {
            ...
            dpp_l3->nof_dedicated_in_rif_profile_bits = SOC_DPP_IN_RIF_PROFILE_CONST_DEDICATED_BITS + ipmc_l3mcastl2_mode + ipv4_double_capacity;
        }
...
soc_arad_core_frequency_config_get(int unit, int dflt_freq_khz, uint32 *freq_khz) 
{
    ...
    SOCDNX_IF_ERR_EXIT(dcmn_property_get_str(unit, spn_CORE_CLOCK_SPEED, &value));
    if (value != NULL) {
...
STATIC int
soc_arad_prop_stat_if_report_size_get(int unit, int sif_port, uint8 *report_size)
{
        ...
        for (sif_port = 0; sif_port < nof_sif_ports; sif_port++)
        {
            ...
            else if (dpp_arad->init.stat_if.stat_if_info[sif_port].mode == SOC_TMC_STAT_IF_REPORT_QSIZE)
            {
                ...
                if (SOC_IS_JERICHO(unit))
                {
                    ...
                    dpp_arad->init.stat_if.stat_if_info[sif_port].if_qsize_queue_max = soc_property_suffix_num_get(unit, sif_port, spn_STAT_IF_SELECTIVE_REPORT_QUEUE_MAX, "core", 
                ...
                dpp_arad->init.stat_if.stat_if_info[sif_port].if_scrubber_queue_min          = soc_property_suffix_num_get(unit, sif_port, spn_STAT_IF_SCRUBBER_QUEUE_MIN, "core",
                                                                                                                           (SOC_IS_QUX(unit) ? QUX_REPORT_QUEUE_SIZE_MAX :
                                                                                                                           (SOC_IS_QAX(unit) ? QAX_REPORT_QUEUE_SIZE_MAX : ARAD_STAT_IF_REPORT_SCRUBBER_QUEUE_MIN)));
                dpp_arad->init.stat_if.stat_if_info[sif_port].if_scrubber_queue_max          = soc_property_suffix_num_get(unit, sif_port, spn_STAT_IF_SCRUBBER_QUEUE_MAX, "core",
                                                                                                                           (SOC_IS_QUX(unit) ? QUX_REPORT_QUEUE_SIZE_MAX :
                                                                                                                           (SOC_IS_QAX(unit) ? QAX_REPORT_QUEUE_SIZE_MAX : ARAD_STAT_IF_REPORT_SCRUBBER_QUEUE_MIN)));
                ...
                for(ind = 0; ind < ARAD_STAT_IF_REPORT_THRESHOLD_LINES; ++ind) {
                    ...
                    dpp_arad->init.stat_if.stat_if_info[sif_port].if_scrubber_sram_buffers_th[ind] = soc_property_port_suffix_num_get(unit, ind, sif_port, spn_STAT_IF_SCRUBBER_SRAM_BUFFERS_TH, "core", ARAD_STAT_IF_REPORT_THRESHOLD_IGNORED);
                    dpp_arad->init.stat_if.stat_if_info[sif_port].if_scrubber_sram_pdbs_th[ind]  = soc_property_port_suffix_num_get(unit, ind, sif_port, spn_STAT_IF_SCRUBBER_SRAM_PDBS_TH, "core", ARAD_STAT_IF_REPORT_THRESHOLD_IGNORED);                    
                }
            }
        }
