  ...
  )
{
  ...
  if (!is_second_init) {
      ...
      if (SOC_IS_JERICHO_AND_BELOW(unit)) {
          ...
          soc_reg_field_set(unit, IHP_OAM_COUNTER_INCREMENT_BITMAPr, &reg, OAM_EGRESS_SUB_TYPEf,
                            SOC_IS_JERICHO(unit) ? _ARAD_PP_OAM_SUBTYPE_DEFAULT_EGRESS_OAM : 0);
      }
      SOC_SAND_SOC_IF_ERROR_RETURN(res, 80, exit, soc_reg32_set(unit, IHP_OAM_COUNTER_INCREMENT_BITMAPr, SOC_CORE_ALL, 0, reg));
  ...
  )
{
    ...
    if (SOC_IS_QAX(unit) && !is_second_init) {
        res = soc_qax_pp_oam_init(unit);
        SOC_SAND_SOC_CHECK_FUNC_RESULT_ERR_VAL(res, 222,exit,res);
    }
  ...
  )
{
  ...
  if ((dst_mac_address->address[5] & 0x1) != 1) {
      ...
      /*egress*/
      SOC_SAND_SOC_IF_ERROR_RETURN(res, 60, exit, READ_EPNI_MY_CFM_MAC_TABLEm(unit,MEM_BLOCK_ANY, table_index, &entry));
      if (is_first_mep) {
      ...
      SOC_SAND_SOC_IF_ERROR_RETURN(res, 70, exit, WRITE_EPNI_MY_CFM_MAC_TABLEm(unit, MEM_BLOCK_ANY, table_index, entry));
  }
...
soc_error_t
    ...
    if (SOC_IS_JERICHO(unit)) {
        ...
        rv = READ_IHP_OAM_COUNTER_INCREMENT_BITMAPr_REG32(unit, SOC_CORE_ALL, &reg);
        SOCDNX_IF_ERR_EXIT(rv);

        soc_reg_field_set(unit, IHP_OAM_COUNTER_INCREMENT_BITMAPr, &reg, OAM_EGRESS_SUB_TYPEf,
                          is_slm ? _ARAD_PP_OAM_SUBTYPE_LM : _ARAD_PP_OAM_SUBTYPE_DEFAULT_EGRESS_OAM);
        rv = soc_reg32_set(unit, IHP_OAM_COUNTER_INCREMENT_BITMAPr, SOC_CORE_ALL, 0, reg); 
        SOCDNX_IF_ERR_EXIT(rv);
    } else {
  ...
  )
{
  ...
  if (ref_count->ref_count[_ARAD_PP_OAM_MY_MAC_LSB_REF_COUNT_INDEX(table_index, dst_mac_address)]==0) {
      ...
      SOC_SAND_SOC_IF_ERROR_RETURN(res, 40, exit, READ_EPNI_MY_CFM_MAC_TABLEm(unit, EPNI_BLOCK(unit, SOC_CORE_ALL), table_index, &epni_entry));
      ...
      SOC_SAND_SOC_IF_ERROR_RETURN(res, 60, exit, WRITE_EPNI_MY_CFM_MAC_TABLEm(unit, EPNI_BLOCK(unit, SOC_CORE_ALL), table_index, epni_entry));
  }
  ...
  )
{
  ...
  mp_type_bfd = SOC_IS_QAX(unit) ? SOC_PPC_OAM_MP_TYPE_QAX_BFD : SOC_PPC_OAM_MP_TYPE_JERICHO_BFD;
  ...
  )
{
    ...
    if ((soc_property_get(unit, spn_OAM_RCY_PORT_UP, -1)!=-1) &&  (is_unicast && oam_payload->up_map) && (internal_opcode == SOC_PPC_OAM_ETHERNET_PDU_OPCODE_LBM)) {
                                                                            ...
                                                                            SOC_SAND_IN SOC_PPC_OAM_LIF_PROFILE_DATA       *profile_data)
{
    ...
    if (SOC_IS_JERICHO(unit)) {
        oam1_key.mp_type_jr = SOC_IS_QAX(unit) ?  SOC_PPC_OAM_MP_TYPE_QAX_BFD : SOC_PPC_OAM_MP_TYPE_JERICHO_BFD;
            ...
            for (mp_type = 0; mp_type < SOC_PPC_OAM_MP_TYPE_COUNT_PLUS; mp_type++) {
                ...
                case SOC_PPC_OAM_MP_TYPE_BELOW_PLUS: /* Drop Level */
                    ...
                    if (SOC_PPD_OAM_IS_CLASSIFIER_ARAD_MODE(unit)) {
                        ...
                        oam1_key.mp_type_jr = SOC_IS_QAX(unit) ?
                            SOC_PPC_OAM_MP_TYPE_QAX_BELOW_ALL :
                            SOC_PPC_OAM_MP_TYPE_JERICHO_BELLOW_HIGHEST_MEP; /* Used only by Jericho.*/
                        oam_payload.snoop_strength = 0;
                        res = sw_state_access[unit].dpp.soc.arad.pp.oam.mirror_profile_err_level.get(unit, &oam_mirror_profile);
                        SOCDNX_SAND_IF_ERR_EXIT(res);
                        oam_payload.mirror_profile = oam_mirror_profile;
    ...
    for (is_my_cfm_mac = 0; is_my_cfm_mac <=1; is_my_cfm_mac++) {
        ...
        for (internal_opcode = 1; internal_opcode < SOC_PPC_OAM_OPCODE_MAP_COUNT; internal_opcode++) {
            ...
            if (SOC_PPD_OAM_IS_CLASSIFIER_ARAD_MODE(unit)) {
                ...
                SOC_PPC_OAM_MP_TYPE_JERICHO possible_mp_types[] = { SOC_PPC_OAM_MP_TYPE_JERICHO_ACTIVE_MATCH, SOC_PPC_OAM_MP_TYPE_JERICHO_PASSIVE_MATCH,
                    SOC_PPC_OAM_MP_TYPE_JERICHO_MIP_MATCH };
                int cur_mp_type;
                /* Handle the mp-types. Three possibilities:
                   ...
                   2. Passive match - Hitting the entry belonging to the given endpoint, but on the other side.
                          In Jericho there is no passive mp-profile so it is necessary to update all of em'
                   3. MIP match - hitting the MIPs "Down" half.*/
                for (cur_mp_type = 0; cur_mp_type < sizeof(possible_mp_types) / sizeof(SOC_PPC_OAM_MP_TYPE_JERICHO); ++cur_mp_type) {
                    oam1_key.mp_type_jr = possible_mp_types[cur_mp_type];
                    oam1_key.mp_type = possible_mp_types[cur_mp_type] == SOC_PPC_OAM_MP_TYPE_JERICHO_ACTIVE_MATCH ?
                        SOC_PPC_OAM_MP_TYPE_MEP_OR_ACTIVE_MATCH_PLUS : SOC_PPC_OAM_MP_TYPE_MIP_OR_PASSIVE_MATCH_PLUS;
                    if (SOC_PPD_OAM_IS_CLASSIFIER_ADVANCED_MODE(unit) &&
                        possible_mp_types[cur_mp_type] == SOC_PPC_OAM_MP_TYPE_JERICHO_MIP_MATCH) {
                        /* No need for the third case: MIP match and passive match share the same mp type, each handled in the same way.*/
                        continue;
                    }

                    if (possible_mp_types[cur_mp_type] == SOC_PPC_OAM_MP_TYPE_JERICHO_ACTIVE_MATCH ||
                        (possible_mp_types[cur_mp_type] == SOC_PPC_OAM_MP_TYPE_JERICHO_PASSIVE_MATCH && SOC_PPD_OAM_IS_CLASSIFIER_JERICHO_MODE(unit))) {
                        /*Case 1 and case 2 for Jericho: iterate over all possible mp-profiles.*/
                        for (mep_prof = 0; mep_prof < SOC_PPC_OAM_NON_ACC_PROFILES_ARAD_PLUS_NUM; ++mep_prof) {
                            oam1_key.mp_profile = mep_prof;
                            res = arad_pp_oam_classifier_oam1_entry_set_on_buffer(unit, &oam1_key, &oam_payload, oama_buffer);
                            SOCDNX_SAND_IF_ERR_EXIT(res);
                        }
                    } else if (possible_mp_types[cur_mp_type] == SOC_PPC_OAM_MP_TYPE_JERICHO_ACTIVE_MATCH) {
                        /* Case 3: MIP match, applies only to Jericho.*/
                        oam1_key.mp_profile = classifier_mep_entry->non_acc_profile;
                        res = arad_pp_oam_classifier_oam1_entry_set_on_buffer(unit, &oam1_key, &oam_payload, oama_buffer);
                        SOCDNX_SAND_IF_ERR_EXIT(res);
                    } else {
                        /* Case 2 for Arad+: Only option left, passive side.*/
                        oam1_key.mp_profile = classifier_mep_entry->non_acc_profile_passive;
                        res = arad_pp_oam_classifier_oam1_entry_set_on_buffer(unit, &oam1_key, &oam_payload, oama_buffer);
                        SOCDNX_SAND_IF_ERR_EXIT(res);
      ...
      if (mep_db_entry->mep_type == SOC_PPC_OAM_MEP_TYPE_ETH_OAM) { /* Only relevant for Ethernet */
          ...
          if (SOC_IS_ARADPLUS(unit)) {
            ...
            if (mep_db_entry->remote_recycle_port && mep_db_entry->is_11b_maid) {
                ...
                if(SOC_IS_JERICHO(unit))
                {
                    if(user_header_size && mep_db_entry->is_upmep) {
  ...
  if (PPC_API_OAM_STORE_LOCAL_PORT_IN_MEP_DB(mep_db_entry->mep_type)) {
      if (!mep_db_entry->is_upmep || SOC_PPC_OAM_IS_MEP_TYPE_BFD(mep_db_entry->mep_type)) {
          if (SOC_IS_ARAD_B0_AND_ABOVE(unit) || SOC_PPC_OAM_IS_MEP_TYPE_BFD(mep_db_entry->mep_type)) {
              ...
              if (SOC_IS_ARADPLUS(unit) && SOC_IS_JERICHO_AND_BELOW(unit)) { /* Table "accidentally" marked as internal. */
                                                                ...
                                                                uint8                    update )
{
    ...
    SOC_REG_ABOVE_64_CLEAR(reg_above_64);
    /*enter RMEP DB entry*/
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 100, exit,soc_mem_read_no_cache(unit, SOC_MEM_NO_FLAGS, OAMP_RMEP_DBm, MEM_BLOCK_ANY, rmep_index, reg_above_64));
  ...
  )
{
  ...
  /*get RMEP DB entry*/  
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 10, exit,soc_mem_read_no_cache(unit, SOC_MEM_NO_FLAGS, OAMP_RMEP_DBm, MEM_BLOCK_ANY, rmep_index, entry));
  ...
  )
{
  ...
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 10, exit, READ_IHP_OAM_COUNTER_RANGEr_REG32(unit, 0, &reg));
  ...
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 20, exit, soc_reg32_set(unit, IHP_OAM_COUNTER_RANGEr, SOC_CORE_ALL, 0, reg));
  ...
  )
{
    ...
    SOC_REG_ABOVE_64_CLEAR(reg_data);
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 12, exit,soc_mem_read_no_cache(unit, SOC_MEM_NO_FLAGS, OAMP_MEP_DB_LM_DBm, MEM_BLOCK_ANY, lm_info->entry_id +1, reg_data));
    ...
    SOC_REG_ABOVE_64_CLEAR(reg_data);
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 12, exit, soc_mem_read_no_cache(unit, SOC_MEM_NO_FLAGS, OAMP_MEP_DB_LM_STATm, MEM_BLOCK_ANY, lm_info->entry_id + 2, reg_data));
  ...
  )
{
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 12, exit, soc_mem_read_no_cache(unit, SOC_MEM_NO_FLAGS, OAMP_MEP_DB_DM_STATm, MEM_BLOCK_ANY, dm_info->entry_id + offset, reg_data));  
  ...
  )
{
    ...
    if(SOC_IS_JERICHO(unit)) {
        ...
        found_field[1] = SOC_IS_QAX(unit)?OEMA_LAST_LOOKUP_SLAVE_RESULT_FOUNDf:OEMA_LAST_LOOKUP_SLAVE_FOUNDf;
    ...
    if(SOC_IS_JERICHO(unit)) {
        ...
        found_field[1] = SOC_IS_QAX(unit) ? OEMB_LAST_LOOKUP_SLAVE_RESULT_FOUNDf : OEMB_LAST_LOOKUP_SLAVE_FOUNDf;
        nof_last_lookup_regs = 2;
    } else {
    ...
    )
{
    ...
    *val = 0;
    if(SOC_IS_JERICHO(unit)) {
        ...
        last_lookup_fields[1] = SOC_IS_QAX(unit)?OEMA_LAST_LOOKUP_SLAVE_RESULT_FOUNDf:OEMA_LAST_LOOKUP_SLAVE_FOUNDf;
        nof_last_lookup_regs = 2;
    } else {
        last_lookup_regs[0] = IHB_OEMA_LAST_LOOKUPr;
    ...
    if(SOC_IS_JERICHO(unit)) {
        ...
        last_lookup_fields[1] = SOC_IS_QAX(unit) ? OEMB_LAST_LOOKUP_SLAVE_RESULT_FOUNDf : OEMB_LAST_LOOKUP_SLAVE_FOUNDf;
        nof_last_lookup_regs = 2;
    } else {
        last_lookup_regs[0] = IHB_OEMB_LAST_LOOKUPr;
