  ...
  )
{
  ...
  case  ARAD_PP_DEST_ENCODED_PREFIX_TYPE_1001:
    ...
    /* SOC_PPC_FRWRD_DECISION_TYPE_UC_LAG, will be set according to sys-port parsing */
  break;
  ...
  )
{
  ...
  case SOC_PPC_FRWRD_DECISION_TYPE_UC_LAG:
      ...
      res = arad_ports_logical_sys_id_build(
                unit,
                fwd_decision->type == SOC_PPC_FRWRD_DECISION_TYPE_UC_LAG /* is lag*/,
  ...
  else if(fwd_decision->additional_info.eei.type == SOC_PPC_EEI_TYPE_MPLS) /* MPLS */
  {
    ...
    {
      eei_type = SOC_PPC_EEI_IDENTIFIER_POP_VAL;  /*push: push-profile + label*/
  ...
  else if(fwd_decision->additional_info.eei.type == SOC_PPC_EEI_TYPE_OUTLIF) /* outlif */
  {
    ...
    if (SOC_IS_JERICHO(unit)) {
        ...
        /* Fix EEI according to Arad mode:
         * In case of Arad system headers mode in Jericho, EEI should be configured explicitly
         * to adjust the Jericho format.
         * In this case, bits 16:17 must always be set.
         */
  ...
  else if(fwd_decision->additional_info.eei.type == SOC_PPC_EEI_TYPE_MIM) /* outlif */
  {
...
/***********************************************************************************************************
*  Parse an ASD and a destination buffers to get the forward decision struct
*  Flags holds ARAD_PP_FWD_DECISION_PARSE_... values to describe the dest and asd format
*  dest_buffer - Look for destination encoding in the PP Spec for the format of the detination buffer
*  asd_buffer - the second part of the payload (OutLIF/EEI/...) depends which type of payload is parsed
************************************************************************************************************/
  ...
  )
{
  ...
  if(flags & (ARAD_PP_FWD_DECISION_PARSE_LEGACY | ARAD_PP_FWD_DECISION_PARSE_DEST)) {
      ...
      case SOC_PPC_FRWRD_DECISION_TYPE_DROP:
        ...
        SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);
        break;
      case SOC_PPC_FRWRD_DECISION_TYPE_UC_LAG:/* port and lag same case */
      case SOC_PPC_FRWRD_DECISION_TYPE_UC_PORT:
         res = soc_sand_bitstream_get_any_field(
                  &(dest_buffer_lcl),
                  0,
  ...
  if(is_eei == 1 || flags & ARAD_PP_FWD_DECISION_PARSE_EEI)
  {
    ...
    else if(fwd_decision->additional_info.eei.type == SOC_PPC_EEI_TYPE_OUTLIF) /* outlif */
    {
       ...
       * In case of Arad system headers mode in Jericho, EEI should be configured explicitly
       * to adjust the Jericho format.
       * In this case, bits 16:17 are always set, but since they're not part of the lif, they should be unset.
       */
      SOC_SAND_CHECK_FUNC_RESULT(sw_state_access[unit].dpp.soc.arad.pp.header_data.system_headers_mode.get(unit, &system_headers_mode), 73, exit);
...
uint32
  arad_pp_pkt_hdr_type_to_interanl_val_map(
    SOC_SAND_IN  SOC_PPC_PKT_HDR_STK_TYPE    pkt_hdr_type,
    SOC_SAND_OUT uint32                  *internal_val
  )
{

  SOC_SAND_INIT_ERROR_DEFINITIONS_NO_DEVID(SOC_PPC_PKT_HDR_TYPE_TO_INTERANL_VAL_MAP);
  ...
  break;
  default:
    SOC_SAND_SET_ERROR_CODE(SOC_PPC_PKT_HDR_TYPE_NOT_SUPPORTED_ERR, 100, exit);
}exit:
  ...
  )
{
  ...
  case ARAD_PARSER_PKT_HDR_STK_TYPE_ETH:
    *pkt_hdr_type = SOC_PPC_PKT_HDR_STK_TYPE_ETH;
  break;
  case ARAD_PARSER_PKT_HDR_STK_TYPE_ETH_ETH:
    *pkt_hdr_type = SOC_PPC_PKT_HDR_STK_TYPE_ETH_ETH;
  break;
  case ARAD_PARSER_PKT_HDR_STK_TYPE_IPV4_ETH:
    *pkt_hdr_type = SOC_PPC_PKT_HDR_STK_TYPE_IPV4_ETH;
  break;
  case ARAD_PARSER_PKT_HDR_STK_TYPE_ETH_IPV4_ETH:
    *pkt_hdr_type = SOC_PPC_PKT_HDR_STK_TYPE_ETH_IPV4_ETH;
  break;
  case ARAD_PARSER_PKT_HDR_STK_TYPE_IPV6_ETH:
    *pkt_hdr_type = SOC_PPC_PKT_HDR_STK_TYPE_IPV6_ETH;
  break;
  case ARAD_PARSER_PKT_HDR_STK_TYPE_FC_ENCAP_ETH:
    *pkt_hdr_type = SOC_PPC_PKT_HDR_STK_TYPE_FC_ENCAP_ETH;
  break;
  case ARAD_PARSER_PKT_HDR_STK_TYPE_MPLS1_ETH:
    *pkt_hdr_type = SOC_PPC_PKT_HDR_STK_TYPE_MPLS1_ETH;
  break;
  case ARAD_PARSER_PKT_HDR_STK_TYPE_MPLS2_ETH:
    *pkt_hdr_type = SOC_PPC_PKT_HDR_STK_TYPE_MPLS2_ETH;
  break;
  case ARAD_PARSER_PKT_HDR_STK_TYPE_MPLS3_ETH:
    *pkt_hdr_type = SOC_PPC_PKT_HDR_STK_TYPE_MPLS3_ETH;
  break;
  case ARAD_PARSER_PKT_HDR_STK_TYPE_FC_STD_ETH:
    *pkt_hdr_type = SOC_PPC_PKT_HDR_STK_TYPE_FC_STD_ETH;
  break;
  case ARAD_PARSER_PKT_HDR_STK_TYPE_ETH_TRILL_ETH:
    *pkt_hdr_type = SOC_PPC_PKT_HDR_STK_TYPE_ETH_TRILL_ETH;
  break;
  case ARAD_PARSER_PKT_HDR_STK_TYPE_ETH_MPLS1_ETH:
    *pkt_hdr_type = SOC_PPC_PKT_HDR_STK_TYPE_ETH_MPLS1_ETH;
  break;
  case ARAD_PARSER_PKT_HDR_STK_TYPE_ETH_MPLS2_ETH:
    *pkt_hdr_type = SOC_PPC_PKT_HDR_STK_TYPE_ETH_MPLS2_ETH;
  break;
  case ARAD_PARSER_PKT_HDR_STK_TYPE_ETH_MPLS3_ETH:
    *pkt_hdr_type = SOC_PPC_PKT_HDR_STK_TYPE_ETH_MPLS3_ETH;
  break;
  case ARAD_PARSER_PKT_HDR_STK_TYPE_IPV4_IPV4_ETH:
    *pkt_hdr_type = SOC_PPC_PKT_HDR_STK_TYPE_IPV4_IPV4_ETH;
  break;
  case ARAD_PARSER_PKT_HDR_STK_TYPE_IPV4_MPLS1_ETH:
    *pkt_hdr_type = SOC_PPC_PKT_HDR_STK_TYPE_IPV4_MPLS1_ETH;
  break;
  case ARAD_PARSER_PKT_HDR_STK_TYPE_IPV4_MPLS2_ETH:
    *pkt_hdr_type = SOC_PPC_PKT_HDR_STK_TYPE_IPV4_MPLS2_ETH;
  break;
  case ARAD_PARSER_PKT_HDR_STK_TYPE_IPV4_MPLS3_ETH:
    *pkt_hdr_type = SOC_PPC_PKT_HDR_STK_TYPE_IPV4_MPLS3_ETH;
  break;
  case ARAD_PARSER_PKT_HDR_STK_TYPE_IPV6_IPV4_ETH:
    *pkt_hdr_type = SOC_PPC_PKT_HDR_STK_TYPE_IPV6_IPV4_ETH;
  break;
  case ARAD_PARSER_PKT_HDR_STK_TYPE_IPV6_MPLS1_ETH:
    *pkt_hdr_type = SOC_PPC_PKT_HDR_STK_TYPE_IPV6_MPLS1_ETH;
  break;
  case ARAD_PARSER_PKT_HDR_STK_TYPE_IPV6_MPLS2_ETH:
    *pkt_hdr_type = SOC_PPC_PKT_HDR_STK_TYPE_IPV6_MPLS2_ETH;
  break;
  case ARAD_PARSER_PKT_HDR_STK_TYPE_IPV6_MPLS3_ETH:
    *pkt_hdr_type = SOC_PPC_PKT_HDR_STK_TYPE_IPV6_MPLS3_ETH;
  break;
  default:
    *pkt_hdr_type = 0; /* HDR TYPE NOT SUPPORTED */
  }
 ...
 */
  ...
  case SOC_PPC_EEI_TYPE_MIM:
        ...
        break;
  case SOC_PPC_EEI_TYPE_RAW: /* check not out of EEI size */
