...
#ifdef CRASH_RECOVERY_SUPPORT
    if (ARAD_KBP_IS_CR_MODE == 1) {
        SOC_SAND_IF_ERR_EXIT(sw_state_access[unit].dcmn.crash_recovery.kbp_dirty.set(unit,1));
        SOC_SAND_IF_ERR_EXIT(sw_state_access[unit].dcmn.crash_recovery.kbp_tbl_id.set(unit,frwrd_table_id));
    }else
#endif
    {
    ...
    )
{
    ...
    *success = SOC_SAND_SUCCESS;
    ...
    if (!is_update) {
        ...
        if(ARAD_KBP_TO_SOC_RESULT(res) != SOC_SAND_OK) {
           LOG_CLI((BSL_META_U(unit,"Error in %s(): Entry add : kbp_db_install with failed: %s!\n"),
                       FUNCTION_NAME(),
