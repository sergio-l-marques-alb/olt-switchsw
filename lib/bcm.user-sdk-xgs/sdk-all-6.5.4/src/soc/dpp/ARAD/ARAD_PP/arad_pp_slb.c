...
#define ARAD_PP_SLB_LAG_GROUP_MEMBER_NOF_BITS       14
/* Max match rules - at most one LB group and one LB group member. */
...
/* Declare a verify function for each SOC_PPC_SLB_suffix. */
...
static const _arad_pp_slb_verify _arad_pp_slb_verify_arr[] = {
  /* [SOC_PPC_SLB_OBJECT_TYPE_BASE] = */ SOC_PPC_SLB_OBJECT_verify,

  /* For each object link the verify. */
  #define PPC_API_SLB_INTERNAL_OBJECT_LIST_ONLY_OBJECT_ACTION(suffix) /* [SOC_PPC_SLB_OBJECT_TYPE_ ## suffix] = */ SOC_PPC_SLB_ ## suffix ## _verify,
  #define PPC_API_SLB_INTERNAL_OBJECT_LIST_ONLY
...
/* Given the number of bits, generate a mask for that no. of bits. */
/* E.g. 5 -> 0x1F. */
...
/* Given no. of bits, return the maximal value. */
...
{
  return 4 + SOC_DPP_CONFIG(unit)->arad->init.ports.lag_mode;
}
...
STATIC uint32 SOC_PPC_SLB_ENTRY_KEY_verify(SOC_SAND_IN int unit, SOC_SAND_IN SOC_PPC_SLB_OBJECT *untyped_obj)
{
  ...
  /* We don't verify the value since it should not be input. If this changes, then verify should be implemented. */
...
/*
 * Perform checks to make sure the configuration state is valid for SLB. 
 *  
 * Details: 
 * 1) Check that SA authentication is disabled. 
 */
STATIC uint32 _arad_pp_slb_init_verify(SOC_SAND_IN int unit)
{
  ...
  /* Check that SA authentication is not enabled. */
...
/*
 * Initialization of SLB general configuration.
 *  
 * 1. Set the LEM DB ID of SLB to the allocated prefix. 
 * 2. Set SA_LOOKUP_TYPE to 3.
 * 3. Set forbidden traps to 0. 
 * 4. Set the PROG index DB ID to the SLB DB ID. 
 */
STATIC uint32 _arad_pp_slb_init_general_lem_configuration(SOC_SAND_IN int unit, SOC_SAND_IN uint32 slb_prefix)
{
  ...
  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  45,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, ECI_GLOBAL_PP_7r /* Packet Processing Global Register5	*/, REG_PORT_ANY, 0, SA_LOOKUP_TYPEf,  3));
  /* These are actually shadow copies of SA_LOOKUP_TYPE in ECI_GLOBAL_19 so no need to set them. */
  /* IHP_IHP_GLOBAL_REGISTER_8r */
  /* SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  50,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, IHP_REG_00D9r, REG_PORT_ANY, 0, SA_LOOKUP_TYPEf,  3)); */
  /* IHB_IHB_GLOBAL_REGISTER_6r */
  /* SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  60,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, IHB_REG_008Br, REG_PORT_ANY, 0, SA_LOOKUP_TYPEf,  3)); */
  
#ifdef BCM_88675_A0
  if(SOC_IS_JERICHO(unit)) {
      consistent_hashing_configration_reg = PPDB_B_CONSISTENT_HASHING_CONFIGRATIONr;
      /* Need to use 4 Msb for consistent hashing configuration in this section. */
  ...
  /* Set the PROG index (2) DBID to the SLB DBID. */
...
/*
 * Initialization of SLB hashing - set default SLB hash function to 0x8423. 
 */
STATIC uint32 _arad_pp_slb_init_hashing(SOC_SAND_IN int unit)
{
  ...
  /* Set the default LAG hash function to be 0x8423. */
  ...
  if (SOC_IS_JERICHO(unit)) {
	  for (i = 0; i < ARAD_PP_SLB_ECMP_HASH_KEY_OFFSET_NOF; i++) {
	      /* Set ECMP-Lb-Key0 and ECMP-Lb-Key1 to use half~ of the Consistent-Hashing-Lem-Key(47bit) each.
	           Key0 will use the 23 lsb, Key1 will use the 24 mbs*/
...
STATIC uint32 _arad_pp_slb_disable_traps(SOC_SAND_IN int unit)
{
  ...
  /* Initialize forbidden traps to 0. */
...
/*
 * Configure SLB Learning.
 *  
 * 1. Enable learning for SLB (by setting the SLB learn enable bit).
 * 2. Configure the OLP to route to loopback for PROG index events.
 * 3. Check that SA authentication is disabled. 
 *  
 * Notes about OLP: 
 *   We set the OLP configuration for the PROG pointer.
 *   The key is {Key-DB-Profile(2), Command(3), Part-of-Lag(1), FID-DB-Profile(2)}.
 *   Each line contains 8 entries (i.e. part-of-lag and fid-db-profile indices are inside the line).
 *   The line is composed of 8 entries of the format (MSB) <DspEventRouteToDsp1, DspEventRouteToDsp0, DspEventRouteToLoopback> (LSB).
 *   We would like to set loopback to 1, and the other two (RouteToDsp{0,1}) to zero.
 */
STATIC uint32 _arad_pp_slb_init_learning(SOC_SAND_IN int unit)
{
  ...
  /* Prepare a line (8 entries with 001 (3 bits)).*/
  desired_line_val = 0;
  for (entry_idx = 0; entry_idx < entries_per_line; entry_idx++) {
    desired_line_val |= 1 << (entry_idx * 3); /* 001 (3 bits) */
  }
 ...
 *  
 * Details: 
 *   Set PROG index (2) to use AGING-CFG-PTR 3.
 *   The table is IHP_MACT_AGE_AGING_MODE.
 *   The key is {Entry.DB-Index(2bit),Entry.Is-Dynamic(1bit),Entry.FID-Aging-Profile(2bit)}.
 *   The entry format is {LargeEmAgeOwnSystemLagPorts(1), LargeEmAgeOwnSystemPhysicalPorts(1), LargeEmAgeAgingCfgPtr(2)}.
 *  
 *   We set 5'b10XXX -> 
 */ 
STATIC uint32 _arad_pp_slb_init_aging(SOC_SAND_IN int unit)
{
  ...
  for (fid_idx = 0; fid_idx < 4; fid_idx++) {
    for (is_dynamic = 0; is_dynamic < 2; is_dynamic++) {
      ...
      /* We only set the PTR to 3 (first 2 bits of the entry). */
  ...
  /* Initialize the SLB aging time to a default value. */
  cfg_item.age_time_in_seconds = ARAD_PP_SLB_DEFAULT_AGING_TIME_IN_SECONDS;

  /* Set the sw state to the default aging time. */
...
/*
 * Initialize SLB/flexible program variables.
 *
 * Details:
 * 1. Set ENABLE_STANDARD_FORMATS to 0. This means that the LEM format is treated as SLB (Not relevant for flexible hashing).
 * 2. Set ENABLE_LOOKUP to LEM if needed.
 * 3. Set the AND_MASK to 0xf (Not relevant for flexible hashing).
 * 4. Set the OR_MASK to the slb_prefix (Not relevant for flexible hashing).
 * 5. Set KEY_MASK_VALUE to 0xFFFF to un-mask the last 16 bits of the 256 bits vector (FP-LB) that used just before the XOR and CRC functions.
 */
STATIC uint32 _arad_pp_slb_init_program_variables(SOC_SAND_IN int unit, SOC_SAND_IN uint8 slb_enable, SOC_SAND_IN uint32 slb_prefix)
{
  ...
  /* The field name is different in Jericho and above*/
  ...
  for (program_idx = 0; program_idx < ARAD_PP_SLB_NOF_PROGRAMS; program_idx++) {
    ...
    soc_mem_field32_set(unit, IHB_CONSISTENT_HASHING_PROGRAM_VARIABLESm, reg_val, KEY_MASK_VALUEf, 0xFFFF); /* Allow all bits */
...
/*
 * Initialization Sequence (for enable).
 *
 * 1. Initialize the object system.
 * 2. Set the ENABLE_SLB bits.
 * 3. Verify - make sure the configuration is valid for SLB.
 * 4. Initialize the SLB configuration.
 * 5. Configure learning for SLB.
 * 6. Configure aging for SLB.
 * 7. Configure SLB program variables.
 *
 * @enable - TRUE - enable full SLB with learning.
 *           FALSE - enable only SLB hashing without learning and without LEM.
 */
uint32
...
#ifdef BCM_88675_A0
  if(SOC_IS_JERICHO(unit)) {
      regs[0] = PPDB_B_GLOBAL_PP_7r;
      regs[1] = INVALIDr;
      nof_regs = 1;
  } else
#endif /* BCM_88675_A0 */
  {
      /* Not present in Jericho. */
      ...
      regs[1] = IHB_CONSISTENT_HASHING_CONFIGRATIONr; /* Note: it is not the same register */
      nof_regs = 2;
  }
...
/* Check that an object is one of the allowed types (not subtype, but that type exactly). */
/* If not return error. */
STATIC uint32 
...
/* 
 * There should be at least one rule, and at most two.
 *  
 * Possible rule constellations: 
 * 1) One ALL rule. 
 * 2) One LB_GROUP. 
 * 3) One LB_GROUP_MEMBER. 
 * 4) One LB_GROUP and one LB_GROUP_MEMBER. 
 */
STATIC uint32
  ...
  )
{
  ...
  /* Check for a LB_GROUP/LB_GROUP_MEMBER rules/combinations of rules. */
  ...
  /* Check that either both rules are LAG or ECMP. */
...
/* Verify the traverse action is one of the allowed actions for traverse/get_block. */
STATIC uint32
...
/* Update the LEM key by rules. */
/* For LB groups the key is updated. */
/* For LB group members nothing is done. */
uint32 
  ...
  )
{
  ...
  /* Get the destination and is_destination_fec fields from the LB group rule (if present). */
  for (i = 0; i < nof_match_rules; i++) {
    SOC_PPC_SLB_OBJECT_TYPE type;
  ...
  if (use_destination_in_rule) {
    /* The last bit of the destination is cut, since it is in the MAC part of the key, */
    /* which is not matched (MAC is 48 bits, the SLB hash key is 47 bits, followed immediately by */
    /* the destination field. Therefore one bit is in the MAC part)*/
    key_mask->param[1].value[0] = _arad_pp_slb_bit_mask(ARAD_PP_LEM_ACCESS_KEY_PARAM1_IN_BITS_FOR_SLB) & ~1;

    /* For LAG the MSB is also ignored (only 14 bits in LAG)*/
...
/* Fill value for LB group and LB group member rules. */
uint32 
  ...
  )
{
  ...
  /*
   * Match rule logic: 
   *   LB_GROUP_LAG or LB_GROUP_MEMBER present ?
   *   yes -> Use a match value rule with the member. In case of LAG, include the group in the value if the group is present.
   *   no -> Don't use a match value rule with the member (only for getting the right entries).
   *  
   * It is important to match the value for LAG since the LSB of the destination field in the key is not matched.
   *  
   * Consequently, only an LB_GROUP_MEMBER changes the member_match variable.
   */  
  for (i = 0; i < nof_match_rules; i++) {
    SOC_PPC_SLB_OBJECT_TYPE type;
    ...
    case SOC_PPC_SLB_OBJECT_TYPE_TRAVERSE_MATCH_RULE_LB_GROUP_ECMP:
      {
        ...
        is_group_ecmp = TRUE;
        /* group_ndx is irrelevant for an ECMP match value rule. */
        /* group_ndx = ecmp_group->ecmp_ndx; */
      }
      break;
    case SOC_PPC_SLB_OBJECT_TYPE_TRAVERSE_MATCH_RULE_LB_GROUP_MEMBER_LAG:
      {
  ...
  /* How the match value rule is calculated: */
  /*   First, about the prefix (2 upper bits of the value) the matched rules will be either 1* or *1 (therefore we do not catch the 00 entries). */
  /*   This is because we allow at most one match format (ECMP or LAG). */
  /*   Next, the chosen format data may be matched (if match_member is set) or not (if match_member is not set). */
  ...
  )
{
  ...
  /* Transform SLB rules to LEM rules (for key and value). */
  ...
  )
{
  ...
  for (i = 0; i <*nof_entries; i++) {
    ...
    /* We use bitcopy since the flow label field length is bigger than 32. */
    ...
    if (!slb_keys[i].is_fec) {
      /* Remove the (possible) garbage (reserved bit) from the upper bit of the lb_group. */
      slb_keys[i].lb_group.lag_ndx &= (ARAD_PP_SLB_LAG_GROUP_MEMBER_NOF_BITS);
    }

    /* The non-LAG bits may be garbage, so we mask them. */
...
/* Calculate the final age given the SLB aging time and the total aging time and return it. */
/* The final age is the highest age that should be deleted. */
STATIC uint32
  ...
  )
{
  ...
  /* Maximal selectable age is ARAD_PP_SLB_NOF_AGES - 2 (denote it MSA). */
  /* We can choose to delete in one of the following indexes: MSA .. 0 */
  /* Everything bigger than MSA * age_rate automatically gets 0. */
  /* Denote r = AGING_RATE / NOF_AGES*/
  /* We want to map */
  /* 0 .. r -> MSA */
  /* r .. 2r -> MSA - 1 */
  /* ... */
  ...
  /* So we map 0 .. r -> 0, r .. 2r -> 1, ..., MSA * r .. infinity -> MSA. */
  /* Then we do 6 - result. */
  ...
  )
{
  ...
  /* In case of aging, it is also in, so verify it.*/
