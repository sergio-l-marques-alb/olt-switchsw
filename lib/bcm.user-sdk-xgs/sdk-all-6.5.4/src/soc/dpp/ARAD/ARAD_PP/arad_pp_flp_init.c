   ...
   )
{
  ...
  if (SOC_IS_ARADPLUS(unit)) {
      flp_process_tbl.include_lem_2nd_in_result_a    = 2;
      flp_process_tbl.include_lem_1st_in_result_a    = 1; /* VPN entry lower priority */
  } else {
     ...
     int unit)
{
  ...
  if (is_routing_enabled) {
      ...
      /* ipv4mc_bridge */
      if ((!SOC_DPP_CONFIG(unit)->pp.pon_application_enable)&&(SOC_DPP_CONFIG(unit)->pp.ipmc_l3mcastl2_mode == 1)){
  ...
  {      
...
#else
      {
          ...
          if(JER_KAPS_ENABLE(unit) && SOC_DPP_CONFIG(unit)->pp.custom_ip_route){
              ...
              arad_pp_flp_dbal_ipv4uc_l3vpn_program_tables_init(unit,prog_id);
              arad_pp_flp_process_ipv4uc_l3vpn(unit,prog_id);
          }
    #endif
      }
...
#endif
      {
...
#if defined(BCM_88675_A0) && defined(INCLUDE_KBP) && !defined(BCM_88030)
          if(JER_KAPS_ENABLE(unit)){
              arad_pp_flp_dbal_ipv4uc_l3vpn_program_tables_init(unit,0);
          }
          else
#endif
          {
          ...
          arad_pp_flp_process_ipv4uc_l3vpn(unit,0);
      }
