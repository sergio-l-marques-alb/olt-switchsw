  ...
  )
{
    ...
    *nof_entries = 0;
    ...
    if (SOC_IS_DONE_INIT(unit))
    {
        ...
        {
            int                                             hw_access;
            SOC_PPC_FRWRD_MACT_TRAVERSE_MATCH_RULE          new_rule                        = *rule;
            uint8                                           do_dynamic                      = 1,
                                                            do_static                       = 1;
            ...
            if (do_dynamic)
            {
                ...
                new_rule.value_rule.compare_mask |= SOC_PPC_FRWRD_MACT_TRAVERSE_MATCH_SELECT_IS_DYNAMIC;
...
/*    entries_to_scan,*/
  ...
  soc_field_t
    flu_machine_pause = SOC_IS_JERICHO(unit) ? LARGE_EM_FLU_MACHINE_PAUSEf : MACT_FLU_MACHINE_PAUSEf; 
  ...
  LOG_INFO(BSL_LS_SOC_FORWARD,
           (BSL_META_U(unit,
            ...
            if(fifo_size >= ARAD_PP_MACT_REPLY_FIFO_SIZE) {
                ...
                SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  6,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_read(unit, PPDB_B_LARGE_EM_FLU_MACHINE_CURRENT_INDEXr, REG_PORT_ANY, 0, LARGE_EM_FLU_MACHINE_CURRENT_INDEXf, &current_index));
                /* stop flush db*/
                SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  6,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, PPDB_B_LARGE_EM_FLU_MACHINE_CONFIGURATIONr, REG_PORT_ANY, 0, flu_machine_pause,  1));
    ...
    *nof_matched_entries = 0;
    ...
    if (SOC_IS_DONE_INIT(unit))
    {
      ...
      if (BCM_UNIT_DO_HW_READ_WRITE(unit))
        {
            int                                             hw_access;
            SOC_PPC_FRWRD_MACT_TRAVERSE_MATCH_VALUE_RULE    new_rule_val;
            ...
            {
                /* just consider dynamic entries*/
                new_rule_val.compare_mask |= SOC_PPC_FRWRD_MACT_TRAVERSE_MATCH_SELECT_IS_DYNAMIC;
  ...
  )
{
  ...
  LOG_INFO(BSL_LS_SOC_FORWARD,
    ...
    if (action->type == SOC_PPC_FRWRD_MACT_TRAVERSE_ACTION_TYPE_REMOVE)
    {
        res = chip_sim_em_delete_all(unit, ARAD_CHIP_SIM_LEM_BASE, ARAD_CHIP_SIM_LEM_KEY, SOC_DPP_DEFS_GET(unit, lem_width), 
                                     ARAD_CHIP_SIM_LEM_PAYLOAD, rule_key->prefix.value, rule_key->prefix.nof_bits);
        SOC_SAND_CHECK_FUNC_RESULT(res, 120, exit);
    }
exit:
