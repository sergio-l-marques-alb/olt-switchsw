  ...
  )
{
  ...
  */
  ...
  if (SOC_IS_JERICHO(unit))  {
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  220,  exit, ARAD_REG_ACCESS_ERR, soc_reg_above_64_field32_modify(unit, IHP_FLP_GENERAL_CFGr, REG_PORT_ANY, 0, FLP_KAPS_DELAY_IS_EVENf,  SOC_IS_JERICHO_PLUS(unit) ? 0 : 1));
    SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  220,  exit, ARAD_REG_ACCESS_ERR, soc_reg_above_64_field32_modify(unit, IHB_PMF_GENERALr, REG_PORT_ANY, 0, PMF_KAPS_DELAY_IS_EVENf,  SOC_IS_JERICHO_PLUS(unit) ? 0 : 1));
