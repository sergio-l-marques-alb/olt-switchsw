  ...
  )
{
...
#endif
  {
      ...
      /* soc_bist_irdp_arad fails when schanIntrEnb enabled */
  ...
  )
{
  ...
  {
      /* setting one to one mapping TCAM_LOOKUP_SELECT_KEY_i -> i */
  ...
  )
{
  ...
  if (ARAD_IS_VOQ_MAPPING_INDIRECT(unit)) { /* indirect VOQ mapping mode */
    ...
    res = arad_fill_table_with_entry(unit, IPS_QPM_1_SYS_REDm, IPS_BLOCK(unit, SOC_CORE_ALL), table_entry); /* fill table with the entry */
    SOC_SAND_CHECK_FUNC_RESULT(res, 50, exit);
    ...
    res = arad_fill_table_with_entry(unit, IPS_QPM_1_NO_SYS_REDm, IPS_BLOCK(unit, SOC_CORE_ALL), table_entry); /* fill table with the entry */
