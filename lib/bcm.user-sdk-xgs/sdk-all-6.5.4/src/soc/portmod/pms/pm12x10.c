                    ...
                    int wb_buffer_index, pm_info_t pm_info)
{
    ...
    SOC_PBMP_ASSIGN(pm_info->pm_data.pm12x10_db->phys, pm_add_info->phys);
                                       ...
                                       portmod_port_interface_config_t* config)
{
    ...
    rv = PM_4X10_DRIVER(pm_info, 2)->f_portmod_port_interface_config_get(
                            unit, port, PM_4X10_INFO(pm_info, 2), config);
    _SOC_IF_ERR_EXIT(rv);
...
int pm12x10_port_ability_local_get (int unit, int port, pm_info_t pm_info, 
...
/* CHECK IF WE SHOULD USE 4X25 or 4X10
    ...
    rv = PM_4X10_DRIVER(pm_info, 2)->f_portmod_port_ability_local_get (
             unit, port, PM_4X10_INFO(pm_info,2), ability);
    _SOC_IF_ERR_EXIT(rv);
...
int pm12x10_port_autoneg_set(int unit, int port, pm_info_t pm_info, 
                              const phymod_autoneg_control_t* an)
{
    ...
    if (an->enable) {
        ...
        for ( i = 0 ; i < 3 ; i++)
        {
            rv = PM_4X10_DRIVER(pm_info, i)->f_portmod_port_autoneg_set(
                           unit, port, PM_4X10_INFO(pm_info, i), an);
            _SOC_IF_ERR_EXIT(rv);
        }
    ((phymod_autoneg_control_t*)an)->flags &= ~PHYMOD_AN_F_SET_PRIOR_ENABLE;
    }
    ...
    rv = PM_4X10_DRIVER(pm_info, core_seq)->f_portmod_port_autoneg_set(
                           unit, port, PM_4X10_INFO(pm_info, core_seq), an);
    _SOC_IF_ERR_EXIT(rv);
...
int pm12x10_port_autoneg_get (int unit, int port, pm_info_t pm_info, 
...
{     
    ...
    for ( i = 0 ; i < 3 ; i++)
    {
        _SOC_IF_ERR_EXIT(PM_4X10_DRIVER(pm_info, i)->f_portmod_port_link_get( unit, port, 
                                         PM_4X10_INFO(pm_info, i), &tmp_link));
...
int pm12x10_port_ability_remote_get (int unit, soc_port_t port, pm_info_t pm_info,
                                          portmod_port_ability_t *ability)
{
    int rv = 0;
    SOC_INIT_FUNC_DEFS;
...
int pm12x10_port_ability_advert_get (int unit, soc_port_t port, pm_info_t pm_info,
                                          portmod_port_ability_t *ability)
{
    int rv = 0;
    int port_fallback_lane, core_seq;
...
int pm12x10_port_ability_advert_set (int unit, soc_port_t port, pm_info_t pm_info,
                                          portmod_port_ability_t *ability)
{
    int rv = 0, i=0;
    SOC_INIT_FUNC_DEFS;
    ...
    for(i = 0 ; i < 3 ; i++)
    {
        rv = PM_4X10_DRIVER(pm_info, i)->f_portmod_port_ability_advert_set (
             unit, port, PM_4X10_INFO(pm_info,i), ability);
        _SOC_IF_ERR_EXIT(rv);
    }
    ...
    for(i=0; i < nof_phys; i++) {
        ...
        switch(op_cmd) {
            case PHY_DIAG_CTRL_DSC:
                ...
                SOC_IF_ERROR_RETURN
                    (portmod_port_phychain_pmd_info_dump(phy_access, 1,
                                                 (void*)arg));
                break;
            ...
            case PHY_DIAG_CTRL_PCS:
                ...
                SOC_IF_ERROR_RETURN
                    (portmod_port_phychain_pcs_info_dump(phy_access, 1,
                                                         (void*)arg));
                break;
...
int pm12x10_pm4x10_enable(int unit, int port, uint32 phy, uint32 enable) 
{
    ...
    if(!pm_info->pm_data.pm12x10_db->external_top_mode) {
        ...
        i=0;
        quad=-1;
        SOC_PBMP_ITER(PM_12x10_INFO(pm_info)->phys, phy_i) {
            if(i%4 == 0) {
                quad++;
            }
            if(phy_i == phy) {
...
int pm12x10_port_notify(int unit, int port, pm_info_t pm_info,  int link) {
    ...
    for( i = 0 ; i < 3 ; i++){
        ...
        if (tmp_nof_phys > 1) {
            ...
            PORTMOD_USER_ACC_CMD_FOR_PHY_SET(access, 0);
            is_change_speed = 1;
        }
    ...
    for( i = 0 ; i < 3 ; i++){
        ...
        if (tmp_nof_phys > 1) {
            /* set the speed for internal phy */
            PORTMOD_USER_ACC_CMD_FOR_PHY_CLR(access, 0);
        }
...
int pm12x10_port_lane_map_get(int unit, int port, pm_info_t pm_info, phymod_lane_map_t* lane_map)
{
    ...
    if (PM_4X10_DRIVER(pm_info, core_num)->f_portmod_port_lane_map_get) {
