...
typedef enum xphy_wb_vars {
    xphy_is_initialized,
    xphy_phy_type,
    xphy_legacy_phy_present,/* indicates if port has legacy external phy */
    xphy_num_var
}xphy_wb_vars_t;
...
int portmod_xphy_wb_buffer_init(int unit, uint32 wb_buffer_index, portmod_xphy_core_info_t* xphy_core_info)
{
    ...
    _SOC_IF_ERR_EXIT(rv);
    xphy_core_info->wb_var_ids[xphy_legacy_phy_present]= wb_var_id;
...
int portmod_xphy_add(int unit, int xphy_addr, const portmod_xphy_core_info_t* xphy_core_info)
{
    ...
    uint32 phy_type_var_id;
    uint32 is_init_var_id;
    uint32 is_legacy_ext_phy;
   ...
   phy_type_var_id = _xphy_info[unit][xphy_id]->wb_var_ids[xphy_phy_type];
   is_init_var_id = _xphy_info[unit][xphy_id]->wb_var_ids[xphy_is_initialized];
   is_legacy_ext_phy =
        _xphy_info[unit][xphy_id]->wb_var_ids[xphy_legacy_phy_present];

    sal_memcpy(_xphy_info[unit][xphy_id], xphy_core_info, sizeof(portmod_xphy_core_info_t));

   _xphy_info[unit][xphy_id]->wb_var_ids[xphy_phy_type] = phy_type_var_id;
   _xphy_info[unit][xphy_id]->wb_var_ids[xphy_is_initialized] = is_init_var_id ;
   _xphy_info[unit][xphy_id]->wb_var_ids[xphy_legacy_phy_present] =
              is_legacy_ext_phy;

    /* fill some core info */
    if(NULL == xphy_core_info->core_access.access.user_acc) {
        ...
        _xphy_info[unit][xphy_id]->core_access.access.user_acc = local_user_access;
    }
    ...
    if(SOC_WARM_BOOT(unit)){
        ...
        if (is_legacy_phy_present) {
            PORTMOD_IS_LEGACY_PHY_SET(
                &_xphy_info[unit][xphy_id]->core_access.access);
        }
...
int portmod_xphy_delete(int unit, int xphy_addr)
{
    ...
    if( NULL ==  _xphy_info[unit][xphy_id]){
        ...
        xphy_core_info = _xphy_info[unit][xphy_id];
        if(NULL != xphy_core_info->core_access.access.user_acc) {
            local_user_access = (portmod_default_user_access_t *)xphy_core_info->core_access.access.user_acc;
            if(NULL != local_user_access->mutex) {
                sal_mutex_destroy(local_user_access->mutex);
                local_user_access->mutex = NULL;
            }
            sal_free(xphy_core_info->core_access.access.user_acc);
            xphy_core_info->core_access.access.user_acc = NULL;
        }
