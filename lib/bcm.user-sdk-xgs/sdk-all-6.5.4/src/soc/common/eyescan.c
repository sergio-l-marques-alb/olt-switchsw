    ...
    sal_memset(phy_acc, 0, sizeof(phymod_phy_access_t)*array_max_size);
        ...
        for(i=0; i<nof_ports; i++ ) {
            if ( phy_acc[i].access.lane_mask == 0 ) {
                LOG_ERROR(BSL_LS_SOC_PHY, (BSL_META_U(unit, "Illegal lane_mask of 0 on port %d, skipping \n"), ports[i]));
               /* sal_free(phy_acc);
                return SOC_E_INTERNAL; */
            }
        }
        ...
        /*enable eyescan*/
        if(params->type == 4) {
          rv = phymod_diag_eyescan_run(phy_acc, line_rates, phy_acc_length, phymodEyescanModeBERProj, &eyescan_options);
    ...
    soc_port_phy_eyescan_results_t* results)    /*array of result per port*/
{
    ...
    sal_memset(phy_acc, 0, sizeof(phymod_phy_access_t)*nof_ports*PHYMOD_CONFIG_MAX_CORES_PER_PORT);
        ...
        for(i=0; i<nof_ports; i++ ) {
            if ( phy_acc[i].access.lane_mask == 0 ) {
                LOG_ERROR(BSL_LS_SOC_PHY, (BSL_META_U(unit, " Lane_mask of 0 on port %d, skipping\n"), ports[i]));
               /* sal_free(phy_acc);
                return SOC_E_INTERNAL;*/ 
            }
        }
