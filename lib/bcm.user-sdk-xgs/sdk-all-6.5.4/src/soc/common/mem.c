                       ...
                       uint32 *xy_entry, uint32 *dm_entry)
{
    ...
    _soc_mem_fp_global_mask_tcam_shift(unit, mem, xy_entry, 0, count - 1);
...
/*
 * Function:
 *    _soc_mem_hw_cache_read_attempt
 * Purpose:
 *    Read mem from pipe0. If read from pipe0 fails, read from pipe1
 *
 * Returns: TRUE if the read was handled by this function
 *          FALSE if this function did nothing
 * Notes:
 */
STATIC int
_soc_mem_hw_cache_read_attempt(int unit, uint32 flags, soc_mem_t mem,
                               int index, int copyno, void *entry_data,
                               int *rv) {
#ifdef ALPM_ENABLE
    int acc_type = 0;
    ...
    LOG_VERBOSE(BSL_LS_SOC_SOCMEM, (BSL_META_U(unit,
        "soc_mem_alpm_pipe_read: mem = %s, index = %0d, pipe = %d, will read from pipe %0d\n"),
        SOC_MEM_NAME(unit, mem), index, acc_type, 0));
    *rv = soc_mem_pipe_select_read(unit, flags, mem, copyno, acc_type,
                                   index, entry_data);
    if (SOC_E_FAIL == *rv) {
        LOG_WARN(BSL_LS_SOC_SOCMEM, (BSL_META_U(unit,
            "soc_mem_alpm_pipe_read: mem = %s, index = %0d, pipe = %d, saw parity failure: will read from pipe %0d\n"),
            SOC_MEM_NAME(unit, mem), index, acc_type, 1));
        acc_type = 1;
        /* Read data from alternate pipe */
        *rv = soc_mem_pipe_select_read(unit, flags, mem, copyno, acc_type,
                                       index, entry_data);
        if (SOC_FAILURE(*rv)) {
            LOG_ERROR(BSL_LS_SOC_SOCMEM, (BSL_META_U(unit,
                "soc_mem_alpm_pipe_read: mem = %s, index = %0d, also seeing parity failure for pipe %0d, could not recover\n"),
                SOC_MEM_NAME(unit, mem), index, acc_type));
        }
    }
    return TRUE;
#else
                          ...
                          int resp_word, int *rv, int *all_done)
{
    ...
    if (cache != NULL && CACHE_VMAP_TST(vmap, index)
        ...
        if (SOC_IS_TOMAHAWK(unit)) {
            if (_soc_mem_hw_cache_read_attempt(unit, flags, mem, index,
                                               copyno, entry_data, rv)) {
                *all_done = TRUE; /* no need to manipulate entry_data, etc */
                return TRUE;
            }
        }
...
#ifdef BCM_DPP_SUPPORT
    entry_dw = soc_mem_entry_words(unit, mem);
    if (SOC_IS_DPP(unit) && (entry_dw >= CMIC_SCHAN_WORDS(unit))) {
        if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
            SOC_DPP_ALLOW_WARMBOOT_WRITE(_soc_arad_mem_array_wide_access(unit, mem, array_index, copyno, index, entry_data, 1), rv);
            return rv;
        } else {
            SOC_DPP_ALLOW_WARMBOOT_WRITE(dcmn_mem_array_wide_access(unit, mem, array_index, copyno, index, entry_data, 1), rv);
            return rv;
        }
    }
#endif
...
#ifdef BCM_DPP_SUPPORT 
        && (entry_dw < CMIC_SCHAN_WORDS(unit))
#endif
        ) {
#if defined(BCM_SIRIUS_SUPPORT) || defined (BCM_XGS3_SWITCH_SUPPORT) ||\
    defined(BCM_CALADAN3_SUPPORT) || defined(BCM_88750_SUPPORT) ||\
    defined(BCM_ARAD_SUPPORT) || defined(BCM_DFE_SUPPORT)

        if (SOC_IS_SIRIUS(unit) || SOC_IS_XGS3_SWITCH(unit) ||
            SOC_IS_CALADAN3(unit) || SOC_IS_DFE(unit) || SOC_IS_ARAD(unit)) {
            int rv;
            soc_mem_info_t  *meminfo;
...
#endif /* defined(BCM_TRIUMPH3_SUPPORT) && defined(BCM_ISM_SUPPORT) */
              ...
              int index, void *entry_data)
{
    ...
    if (SOC_IS_DPP(unit) && (entry_dw >= CMIC_SCHAN_WORDS(unit))) {
        if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
            SOC_DPP_ALLOW_WARMBOOT_WRITE(_soc_arad_mem_array_wide_access(unit, mem, array_index, copyno, index, entry_data, 0), rv);
            return rv;
        } else {
            SOC_DPP_ALLOW_WARMBOOT_WRITE(dcmn_mem_array_wide_access(unit, mem, array_index, copyno, index, entry_data, 0), rv);
            return rv;
        }
    }
#endif
...
#ifdef BCM_TRIDENT_SUPPORT
                        if ((SOC_IS_TD_TT(unit) && !SOC_IS_TOMAHAWK(unit)) && 
    ...
    if ((nack != 0) || (rv == SOC_E_FAIL)) {
        if (type == SCHAN_GEN_RESP_TYPE_NOT_FOUND) {
            ...
            if (!(SOC_IS_TRIUMPH3(unit) || (SOC_IS_TRIDENT2(unit)
                ...
                || (SOC_IS_TOMAHAWK(unit)) || SOC_IS_APACHE(unit))) {
                       ...
                       void *key, void *result, int *index_ptr)
{
    ...
    if ((nack != 0) || (rv == SOC_E_FAIL)) {
        ...
        if (!index_valid) {
#if defined(BCM_HURRICANE3_SUPPORT)
            if (SOC_IS_HURRICANE3(unit) && (mem == L2Xm)) {
...
_retry:
     ...
     * addr[TD2: 13:0,  TH: 12:0]   - bucket
     * addr[TD2: 17:14, TH: 16:13]  - bank disable[3:0]
     */
#if defined(BCM_TOMAHAWK_SUPPORT)
    if (SOC_IS_TOMAHAWK(unit)) {
...
_retry_op:
    ...
    if ((nack != 0) || (rv == SOC_E_FAIL)) {
        ...
        {
            if (type == SCHAN_GEN_RESP_L2_MOD_FIFO_FULL) { 
                ...
                LOG_INFO(BSL_LS_SOC_SOCMEM,
                         (BSL_META_U(unit,
                                     "L2_MOD_FIFO is full. Retry Insert operation!\n")));
        ...
        if (type == SCHAN_GEN_RESP_TYPE_NOT_FOUND) {
            ...
            if (type == SCHAN_GEN_RESP_L2_MOD_FIFO_FULL) {
                ...
                if (soc_mem_generic_lookup(unit, mem, copyno, banks,
                    ...
                    LOG_INFO(BSL_LS_SOC_SOCMEM,
                             (BSL_META_U(unit,
                                         "L2_MOD_FIFO is full. Retry Delete operation!\n")));
                    goto _retry_op_delete;
                }                
            }
#endif
...
STATIC int
_soc_mem_shared_hash_breadth_l3_move2(int unit, soc_mem_t mem, int copyno,
                                      void *entry, int num_levels)
{
    _soc_breadth_bucket2_t *buckets = NULL;
    ...
    SHR_BITDCL *bkt_trace = NULL;
    _soc_hash_bank_info_t bi = {0};
    uint32 bkt_entry[SOC_MAX_MEM_WORDS];
    ...
    base_mem = L3_ENTRY_IPV6_MULTICASTm;
    SOC_IF_ERROR_RETURN(_soc_mem_shared_hash_init
    ...
    do {
        ...
        for (ent = 0; ent < NUM_ENT; ent += ent_incr) {
            ...
            for (bix = 0; bix < num_banks; bix++) {
                ...
                /* loop sub bucket's entries and try move */
                rv = soc_mem_read(unit, base_mem, copyno, bucket, bkt_entry);
                if (SOC_FAILURE(rv)) {
                    goto cleanup;
                }

                soc_mem_field_get(unit, base_mem, bkt_entry, VALID_3f, &v[3]);
                soc_mem_field_get(unit, base_mem, bkt_entry, VALID_2f, &v[2]);
                soc_mem_field_get(unit, base_mem, bkt_entry, VALID_1f, &v[1]);
                soc_mem_field_get(unit, base_mem, bkt_entry, VALID_0f, &v[0]);
                soc_mem_field_get(unit, base_mem, bkt_entry, KEY_TYPE_3f, &k[3]);
                soc_mem_field_get(unit, base_mem, bkt_entry, KEY_TYPE_2f, &k[2]);
                soc_mem_field_get(unit, base_mem, bkt_entry, KEY_TYPE_1f, &k[1]);
                soc_mem_field_get(unit, base_mem, bkt_entry, KEY_TYPE_0f, &k[0]);
                ...
                for (sent = 0; sent < NUM_ENT; sent += sent_incr) {
                    ...
                    _soc_mem_shared_hash_l3_bucket_read(unit, sent, k[sent],
                            &sent_incr, &orig_mem, bkt_entry, me);
                    ...
                    while (free && pbi->s_bucket != bi.numb) {
                        ...
                        src_width = (L3_ENTRY_IPV6_MULTICASTm == orig_mem) ? 4 :
                                    (L3_ENTRY_IPV4_MULTICASTm == orig_mem ||
                                     L3_ENTRY_IPV6_UNICASTm   == orig_mem) ? 2 : 1;
...
STATIC int
_soc_mem_shared_hash_breadth_l3_move(int unit, soc_mem_t mem, int copyno,
                                     void *entry, int num_levels)
{
    SHR_BITDCL *bkt_trace = NULL;
    _soc_breadth_bucket_t *buckets[MAX_L3_LEVELS + 1] = {0};
    _soc_breadth_bucket_t *cb, *tb, *cbi = NULL, *pbi;
    uint32 bkt_entry[SOC_MAX_MEM_WORDS];
    uint32 v[NUM_ENT], k[NUM_ENT];
    void *se, *me;
    ...
    if (num_levels <= 0) {
        return SOC_E_FULL;
    } else if (num_levels > MAX_L3_LEVELS) {
        return _soc_mem_shared_hash_breadth_l3_move2(unit, mem, copyno,
                                                     entry, num_levels);
    }
            ...
            if (level == 0) {
                ...
                cb->src_width = (L3_ENTRY_IPV6_MULTICASTm == mem) ? 4 :
                                (L3_ENTRY_IPV4_MULTICASTm == mem ||
                                 L3_ENTRY_IPV6_UNICASTm   == mem) ? 2 : 1;
                se = entry;
            } else {
                if (cb->src_mem == 0) {
                    continue;
                                ...
                                SHR_BITDCL *trace)
{
    ...
    for (bix = 0; bix < bi.num_banks; bix++) {
        ...
        if (mem != L2Xm) {
            rv = soc_mem_read(unit, l3_mem, copyno, bucket, bkt_entry);
            if (SOC_FAILURE(rv)) {
                goto cleanup;
            }

            v[3] = soc_mem_field32_get(unit, l3_mem, bkt_entry, VALID_3f);
            v[2] = soc_mem_field32_get(unit, l3_mem, bkt_entry, VALID_2f);
            v[1] = soc_mem_field32_get(unit, l3_mem, bkt_entry, VALID_1f);
            v[0] = soc_mem_field32_get(unit, l3_mem, bkt_entry, VALID_0f);
            k[3] = soc_mem_field32_get(unit, l3_mem, bkt_entry, KEY_TYPE_3f);
            k[2] = soc_mem_field32_get(unit, l3_mem, bkt_entry, KEY_TYPE_2f);
            k[1] = soc_mem_field32_get(unit, l3_mem, bkt_entry, KEY_TYPE_1f);
            k[0] = soc_mem_field32_get(unit, l3_mem, bkt_entry, KEY_TYPE_0f);
        ...
        for (ent = 0; ent < NUM_ENT; ent += ent_incr) {
            if (retry) {
                ...
                if (mem == L2Xm) {
                    ...
                    _soc_mem_shared_hash_l3_bucket_read(unit, ent, k[ent],
                          &ent_incr, &orig_mem, bkt_entry, &orig_entry[ent]);
                }
            }
    ...
    switch (SOC_MULTI_HASH_MOVE_ALGORITHM(unit)) {
        ...
        default:
            if (mem == L2Xm) {
                ...
                rv = _soc_mem_shared_hash_breadth_l3_move(unit, mem, copyno,
                                                          entry_data, recurse_depth);
            }
            break;
    }
...
_soc_hr3_l3iif_hw_mem_init(int unit, int force_init_all) 
{
    ...
    if (!SOC_IS_HURRICANE3(unit) || !SOC_MEM_IS_VALID(unit, L3_IIFm))  {
        return SOC_E_UNAVAIL;
    }
