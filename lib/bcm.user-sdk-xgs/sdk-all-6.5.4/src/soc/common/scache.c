...
#ifdef CRASH_RECOVERY_SUPPORT
    if (SCACHE_IS_SHMEM(unit)) {
        if (!ha_mem_is_already_alloc(unit, SOC_SCACHE_HANDLE_MODULE_GET(handle), SOC_SCACHE_HANDLE_SEQUENCE_GET(handle))) {
            return SOC_E_NOT_FOUND;
        }
        *ptr = (uint8 *)ha_mem_alloc(unit,
                            SOC_SCACHE_HANDLE_MODULE_GET(handle), 
                            SOC_SCACHE_HANDLE_SEQUENCE_GET(handle),
                            0, 0, size);
        return SOC_E_NONE;
    }
#endif
...
soc_stable_size_set(int unit, int arg)
{
    ...
    if (stable->location == _SHR_SWITCH_STABLE_DEVICE_NEXT_HOP) {
        ...
        if (stable->index_max < num_entries) {
            stable->index_min = stable->index_max;
            LOG_ERROR(BSL_LS_SOC_COMMON,
                      (BSL_META_U(unit,
                                  "Stable size(%d) is larger than size of memory EGR_L3_NEXT_HOPm(%d)\n"),
                                  stable->size,
                                  stable->index_max * bytes2));
        } else {
