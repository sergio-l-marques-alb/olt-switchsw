...
#define ALPM_MAX_PFX_ENTRIES              (3 * (64 + 32 + 2 + 1))
#define ALPM_MAX_PFX_INDEX                (ALPM_MAX_PFX_ENTRIES - 1)
#define MAX_VRF_PFX_INDEX            ((ALPM_MAX_PFX_ENTRIES/3) - 1)
...
soc_alpm_db_ent_type_encoding(int u, int vrf, uint32 *db_type, uint32 *ent_type)
{
    ...
    if (vrf == (SOC_VRF_MAX(u) + 1)) {
        dbt = 0;
        ent = 0;
    } else if (SOC_IS_TRIDENT2PLUS(u) || SOC_IS_TOMAHAWK(u)) {
        /* TD2+ and TH */
        vrf_msb = (vrf >> 10) & 0x3;
...
static int _soc_alpm_update_bpm_len(int u, int new_tcam_index, int v6, 
                                    int bpm_len)
{
    int rv, tmp, index;
    defip_aux_table_entry_t entry;

    index = new_tcam_index >> (v6 ? 0 : 1);
    rv = soc_mem_read(u, L3_DEFIP_AUX_TABLEm, MEM_BLOCK_ANY, index, &entry);
    SOC_IF_ERROR_RETURN(rv);
    if (v6) {
        soc_mem_field32_set(u, L3_DEFIP_AUX_TABLEm, &entry, BPM_LENGTH0f, 
                            bpm_len);
        soc_mem_field32_set(u, L3_DEFIP_AUX_TABLEm, &entry, BPM_LENGTH1f, 
                            bpm_len);
        tmp = soc_mem_field32_get(u, L3_DEFIP_AUX_TABLEm, &entry, DB_TYPE0f);
    } else {
        if (new_tcam_index & 1) {
            soc_mem_field32_set(u, L3_DEFIP_AUX_TABLEm, &entry, BPM_LENGTH1f, 
                                bpm_len);
            tmp = soc_mem_field32_get(u, L3_DEFIP_AUX_TABLEm, &entry, 
                                      DB_TYPE1f);
        } else {
            soc_mem_field32_set(u, L3_DEFIP_AUX_TABLEm, &entry, BPM_LENGTH0f, 
                                bpm_len);
    ...
    /* write entry back */
    rv = soc_mem_write(u, L3_DEFIP_AUX_TABLEm, MEM_BLOCK_ANY, index, &entry);
    SOC_IF_ERROR_RETURN(rv);
    ...
    if (SOC_URPF_STATUS_GET(u)) {
        ...
        soc_mem_field32_set(u, L3_DEFIP_AUX_TABLEm, &entry, DB_TYPE0f, tmp);
        soc_mem_field32_set(u, L3_DEFIP_AUX_TABLEm, &entry, DB_TYPE1f, tmp);
        index += (soc_mem_index_count(u, L3_DEFIP_AUX_TABLEm)) / 2;
        rv = soc_mem_write(u, L3_DEFIP_AUX_TABLEm, MEM_BLOCK_ANY, index, 
                           &entry);
    }
    return rv;
...
static int
_soc_alpm_write_pivot_aux(int u, int s_index, int index, void *entry, int convert)
{
    defip_aux_table_entry_t shadow;

    SOC_IF_ERROR_RETURN(_soc_alpm_tcam_shadow_ent_init(u, 
            s_index, index, entry, convert, (void *)&shadow));
                        ...
                        uint32 *e, int *key_index, int v6)
{
    ...
    if (rv == SOC_E_FULL) {
        ...
        if (SOC_ALPM_V6_SCALE_CHECK(u, v6)) {
            return soc_mem_alpm_insert(u, mem, bucket_index+1,
                             MEM_BLOCK_ANY,
                             bank_disable,
                             alpm_data, e, key_index);
        }
...
int
_soc_alpm_raw_bucket_get(int u, soc_mem_t mem, int index)
{
    return SOC_ALPM_BKT_ENTRY_TO_IDX(index % (1 << 16));
}
                          ...
                          void *raw_entry, void *raw_sip_entry)
{
    ...
    for (i = 0; i < SOC_ALPM_RAW_BUCKET_SIZE(u, v6); i++) {
        SOC_IF_ERROR_RETURN(soc_mem_read(u, L3_DEFIP_ALPM_RAWm, MEM_BLOCK_ANY, 
                                    raw_index + i, &ent[i]));
        if (SOC_URPF_STATUS_GET(u)) {
...
static int
_soc_alpm_split_inval(int u, soc_mem_t mem, int v6, 
                      alpm_mem_prefix_array_t *prefix_array, int *new_index)
{
    ...
    if (v6) {
        ...
        bufp_entry_size = sizeof(alpmv4_entry);
        sip_bufp_entry_size = sizeof(alpmv4_sip_entry);
    }
                 ...
                 int tcam_index)
{
    ...
    flex = VRF_FLEX_IS_SET(u, vrf_id, vrf, v6, 1);
    /* Get table memory. */
    ...
    /* Allocate Payload */
    payload = sal_alloc(sizeof(payload_t), "Payload for Key");
    if (payload == NULL) {
        LOG_ERROR(BSL_LS_SOC_ALPM,
                  (BSL_META_U(u,
                              "_soc_alpm_insert: Unable to allocate memory for "
                              "trie node \n")));
        return SOC_E_MEMORY;
    }
    ...
    if (split) {
        ...
        /* Insert PIVOT and assign a bucket, move enteries to new bucket
        ...
        if (SOC_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_SOC_ALPM,
                      (BSL_META_U(u,
                                  "_soc_alpm_insert: Unable to allocate"
                                  "new bucket for split\n")));
            bucket_index = -1;
            goto delete_pfx_trie;
        }

        /* make sure alloced bucket is freed on all subsequent errors 
         * 1024 here used to pass the splitable checker */
        rv = trie_split(trie, v6 ? _MAX_KEY_LEN_144_ : _MAX_KEY_LEN_48_, FALSE, 
                        pivot, &length, &split_trie_root, NULL, FALSE, 1024);
        if (SOC_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_SOC_ALPM,
                      (BSL_META_U(u,
                                  "_soc_alpm_insert: Could not split bucket"
        ...
        if (bucket_handle == NULL) {
            LOG_ERROR(BSL_LS_SOC_ALPM,
                      (BSL_META_U(u,
                                  "_soc_alpm_insert: Unable to allocate memory "
                                  "for PIVOT trie node \n")));
            rv = SOC_E_MEMORY;
            goto delete_pfx_trie;
        }
        ...
        ALPM_TRIE_TO_NORMAL_IP(pivot, length, v6);
        (void) _soc_alpm_lpm_ent_key_init(u, pivot, length, vrf, v6,
                              &lpm_entry, 0);
        ...
        new_index = sal_alloc(sizeof(*new_index)*prefix_array.count,
                    "Temp storage for location of prefixes in new bucket");
        if (new_index == NULL) {
            rv = SOC_E_MEMORY;
            goto delete_pfx_trie;
        }
        ...
        rv = soc_alpm_lpm_insert(u, &lpm_entry, &new_tcam_index, 
                                 src_default, src_discard);
        if (SOC_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_SOC_ALPM,
                      (BSL_META_U(u,
                                  "_soc_alpm_insert: Unable to add new"
                                  "pivot to tcam\n")));
        ...
        new_tcam_index = soc_alpm_physical_idx(u, L3_DEFIPm, 
                                                new_tcam_index, v6);

        rv = _soc_alpm_update_bpm_len(u, new_tcam_index, v6, bpm_len);
        ...
        pivot_added = 1;
        ALPM_TCAM_PIVOT(u, new_tcam_index << (v6 ? 1 : 0)) = pivot_pyld;
        PIVOT_TCAM_INDEX(pivot_pyld) = new_tcam_index << (v6 ? 1 : 0);
...
#else
        rv = _soc_alpm_split_inval(u, mem, v6, &prefix_array, new_index);
        if (SOC_FAILURE(rv)) {
            goto rollback_pivot_add;
        }
#endif 
        ...
        if (new_insert == -1) {
            ...
            rv = _soc_alpm_insert_in_bkt(u, mem, PIVOT_BUCKET_INDEX(old_pivot),
                                bank_disable, alpm_data, e, &key_index, v6);

            if (SOC_FAILURE(rv)) {
                /* recover by matching trie state to h/w state */
                LOG_ERROR(BSL_LS_SOC_ALPM,
                          (BSL_META_U(u,
                                      "_soc_alpm_insert: Could not insert new "
...
rollback_pivot_add:
    ...
    rv2 = soc_alpm_lpm_delete(u, &lpm_entry);
    if (SOC_FAILURE(rv2)) {
        LOG_ERROR(BSL_LS_SOC_ALPM,
                  (BSL_META_U(u,
                              "_soc_alpm_insert: Failure to free new prefix"
                              "at %d\n"), soc_alpm_logical_idx(u, L3_DEFIPm,
                   new_tcam_index, v6)));
    }
...
rollback_bkt_move:
    ...
    for (i = 0; i < prefix_array.count; i ++) {
        ...
        if (new_index[i] != -1) {
            if (v6) {
                sal_memset(bufp, 0, sizeof(defip_alpm_ipv6_64_1_entry_t));
            } else {
                sal_memset(bufp, 0, sizeof(defip_alpm_ipv4_1_entry_t));
            }
            ...
            if (SOC_FAILURE(rv2)) {
                LOG_ERROR(BSL_LS_SOC_ALPM,
                          (BSL_META_U(u,
                                      "_soc_alpm_insert: mem write failure"
                                      "in bkt move rollback\n")));
            }
            if (SOC_URPF_STATUS_GET(u)) {
                rv2 = soc_mem_write(u, mem, MEM_BLOCK_ANY, 
                ...
                if (SOC_FAILURE(rv2)) {
                    LOG_ERROR(BSL_LS_SOC_ALPM,
                              (BSL_META_U(u,
                                          "_soc_alpm_insert: urpf mem write "
        ...
        if (prefix->index > 0) {
            ...
            if (SOC_FAILURE(rv2)) {
                LOG_ERROR(BSL_LS_SOC_ALPM,
                          (BSL_META_U(u,
                                      "_soc_alpm_insert: trie reinsert failure"
                                      "in bkt move rollback\n")));
            }
        } else {
            /* new prefix stays in old bucket trie after split */
            if (payload != NULL) {
                sal_free(payload);
            }
        }
...
_soc_alpm_bkt_count(int u, soc_mem_t mem, int bkt_idx, int v6, void *bufp) 
{
    ...
    for (bank = 0; bank < 4; bank++) {
      for (entry = 0; entry < (v6 ? 4 : 6); entry++) {
        ...
        SOC_IF_ERROR_RETURN(rv);
        if (sal_memcmp(bufp, soc_mem_entry_null(u, mem), soc_mem_entry_words(u, mem) * 4)) {
            return 1;
        }
      }
    }
                 ...
                 int tcam_index, int key_index, int propagate)  /* TCAM entry data */
{
    ...
    if (vrf_id != SOC_L3_VRF_OVERRIDE) {
        ...
        delete_bucket = bucket_index & ~SOC_ALPM_V6_SCALE_CHECK(u, ipv6);
...
soc_alpm_ipmc_war(int unit, int install)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        if (install) {
            ...
            SOC_IF_ERROR_RETURN(soc_alpm_lpm_insert(unit, &lpm_entry, &index, 0, 0));
        } else {
...
soc_alpm_vrf_add(int u, int vrf, int v6)
{
    ...
    rv = soc_alpm_lpm_insert(u, &tmp_lpm, &index, 0, 0);
    if (SOC_FAILURE(rv)) {
        goto cleanup;
    }
                ...
                int pivot_info)
{
    ...
    if (pivot_info & SOC_ALPM_LOOKUP_HIT) {
        ...
        rv = _soc_alpm_insert(u, lpm_entry, mem, bufp, bufp2, &index,
                         ...
                         int do_urpf)
{
    ...
    for (i = index_min; i < index_min + index_cnt; i++) {
        SOC_IF_ERROR_RETURN(soc_mem_read(u, lpm_mem, MEM_BLOCK_ANY, 
                            i, (void *)&lpm_ent));
...
soc_alpm_shared_mem_init(int u)
{
    ...
    alpm_bucket_assign(u, &defip_table_size, 1);
    return SOC_E_NONE;
}

/* Get the free bucket to allocate to the PIVOT */
int
alpm_bucket_assign(int u, int *bucket_pointer, int v6)
{
                      ...
                      uint32 op, int src_default_or_convert, int src_discard)
{
     ...
     */
    rv = (_soc_alpm_write_pivot_aux(u, src_index, index, entry, convert));

    if (rv >= 0) {
        rv = (_lpm_fb_urpf_entry_replicate(u, src_index_uprf, index, entry,
                                          convert, sdefault, sdiscard));
    }
...
int _lpm_free_slot_create(int u, int pfx, int ipv6, void *e, int *free_slot)
{
    ...
    if (SOC_ALPM_LPM_STATE_VENT(u, pfx) == 0) {
        ...
        if (soc_alpm_mode_get(u) == SOC_ALPM_MODE_PARALLEL) {
            if (pfx <= MAX_VRF_PFX_INDEX) {
                curr_pfx = MAX_VRF_PFX_INDEX;
            }
        }
...
soc_alpm_lpm_init(int u)
{
    ...
    if (mode == SOC_ALPM_MODE_PARALLEL) {
        ...
        */
        ...
        SOC_ALPM_LPM_STATE_FENT(u, (MAX_VRF_PFX_INDEX)) = defip_table_size >> 1;

        /* Remaining entries are assigned to Global entries */
        SOC_ALPM_LPM_STATE_FENT(u, (ALPM_MAX_PFX_INDEX)) = 
          (defip_table_size - SOC_ALPM_LPM_STATE_FENT(u, (MAX_VRF_PFX_INDEX)));
    } else {
        /* Remaining entries are assigned to Global entries */
    ...
    if (rv == SOC_E_NONE) {
        ...
        LOG_INFO(BSL_LS_SOC_ALPM,
                 (BSL_META_U(u,
                             ...
                             index, pfx));
        rv = _soc_alpm_write_pivot(u, index, index, entry_data,
                          ...
                          int bkt_ptr)
{
    ...
    soc_L3_DEFIPm_field32_set(unit, lpm_entry, ALG_BKT_PTR0f, bkt_ptr);
...
#define PRINT_BKT_OCCU_ITEM_TOT(u, all_dc) \
    do { \
        LOG_CLI((BSL_META_U(u, \
                 ...
                 "Total", BKT_PERCENT((all_dc)->v4, (all_dc)->v4t), \
