                       ...
                       int to_ent, int erase)
{
    ...
    if (SOC_LPM128_PFX_IS_V6(u, pfx)) {
        ...
        /* coverity[copy_paste_error : FALSE] */
            rv = _lpm128_fb_urpf_entry_replicate(u, to_ent, e, eupr);
        }
        if (rv < 0) {
        ...
        rv = WRITE_L3_DEFIPm(u, MEM_BLOCK_ANY, to_ent, e);
        if (rv >= 0) {
            rv = _lpm128_fb_urpf_entry_replicate(u, to_ent, e, NULL);
        }
        if (rv < 0) {
                         ...
                         void *e, int slot, int *is_deleted)
{
    ...
    if (to_ent != from_ent) {
        ...
        if (rv >= 0) {
            rv = _lpm128_fb_urpf_entry_replicate(u, to_ent, ef, efupr);
        }
        if (rv < 0) {
                     ...
                     int *hw_index)
{
    ...
    if (rv == SOC_E_NONE) {
        ...
        if (!found) {
            LPM128_HASH_INSERT(u, entry_data, entry_data1, index, &rvt_index0, &rvt_index1);
            if (type == socLpmEntryTypeV4) {
        ...
        if (!found && rv < 0) {
            LPM128_HASH_REVERT(u, entry_data, entry_data1, index, rvt_index0, rvt_index1);
        }
    }
