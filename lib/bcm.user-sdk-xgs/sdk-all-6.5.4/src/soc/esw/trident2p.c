...
static int 
soc_td2p_port_macro_to_flex_port_mapping[NUM_TSC4_PORT_MACRO] = 
                                        {1, 1, 1, 13, 17, 
    ...
    if (si->frequency >= 760) {
        ...
        for (port_macro = pipe * _TD2P_NUM_TSC4_PER_PIPE; 
            ...
            if (pm_flex_enabled == 1) {
                ...
                phy_port = soc_td2p_port_macro_to_flex_port_mapping[port_macro];
                port = si->port_p2l_mapping[phy_port];
        ...
        SOC_PBMP_ITER(pbmp, port) {
            ...
            for (port_macro = pipe * _TD2P_NUM_TSC4_PER_PIPE; 
                 port_macro < (pipe * _TD2P_NUM_TSC4_PER_PIPE) + 
                      _TD2P_NUM_TSC4_PER_PIPE; port_macro++) {
                if(phy_port == 
                   soc_td2p_port_macro_to_flex_port_mapping[port_macro]) {
                    skip_port = 1;
                    break;
                }
            }
...
soc_td2p_port_macro_flex_enabled(int unit, int port_macro, int * flex_enabled)
{
    ...
    port_num = soc_td2p_port_macro_to_flex_port_mapping[port_macro];
...
/*
 * Function:
 *      _soc_td2p_tdm_lb_port_indices_get
 * Purpose:
 ...
 */
STATIC int
_soc_td2p_tdm_lb_port_indices_get(int unit, int table_num,
                                  int slot_len, uint32 *arr, uint32 *count)
{
    static const soc_mem_t mmu_tdm_mem[2] = {
...
/*
 * Function:
 *      _soc_td2p_tdm_lb_port_indices_set
 * Purpose:
 ...
 */
STATIC int
_soc_td2p_tdm_lb_port_indices_set(int unit, int table_num, int slot_len,
                                  uint32 *arr, uint32 count, int value)
{

    static const soc_mem_t mmu_tdm_mems[2] = {
        ES_PIPE1_TDM_TABLE_0m,
        ES_PIPE1_TDM_TABLE_1m
    };
    static const soc_mem_t mmu_tdm_fields[2] ={
        PORT_NUM_EVENf, PORT_NUM_ODDf
    };
...
/*
 * Function:
 *      _soc_td2p_mmu_map_phy_port_to_lb
 * Purpose:
 *      This function temporarily maps old physical port to
 *      loopback port in the mmu port mapping registers
 * Parameters:
 *      unit           - (IN) Unit number.
 ...
 */
STATIC int
_soc_td2p_mmu_map_phy_port_to_lb(int unit,
                                 int pre_count,
                                 soc_port_resource_t *pre_resource)
{
    ...
    for (i = 0, pr = &pre_resource[0]; i < pre_count; i++, pr++) {
        LOG_VERBOSE(BSL_LS_SOC_MMU,
                (BSL_META_U(unit,
                            ...
                            "logical_port=%d old physical_port=%d\n"),
                 pr->logical_port, pr->physical_port));

        /* MMU_PORT_TO_PHY_PORT_MAPPING [MMU LB port number]
         * = old physical port number
         * */
        old_phy_port = pr->physical_port;
    ...
    /* MMU_PORT_TO_PHY_PORT_MAPPING [MMU LB port number]
     * = physical LB port number */
    rval = 0;
    soc_reg_field_set(unit, MMU_PORT_TO_PHY_PORT_MAPPINGr, &rval,
            PHY_PORTf, si->port_l2p_mapping[si->lb_port]);
    SOC_IF_ERROR_RETURN
        (soc_reg32_set(unit, MMU_PORT_TO_PHY_PORT_MAPPINGr,
                       si->lb_port, 0, rval));
 ...
 *      temporarily to the Loopback port
 * Parameters:
 *      unit           - (IN) Unit number.
 *      pre_count      - (IN) Array size for pre-FlexPort array.
 *      pre_resource   - (IN) Pre-FlexPort configuration array.
 ...
 *        Turning off Opportunistic Scheduling for the Loopback port
 *        and
 *        Turning ON Opportunistic Scheduling for the Loopback port
 *
 */
int
                                  ...
                                  soc_port_resource_t *pre_resource)
{
    ...
    /* Get all the fields here from Pipe 1 TDM CONFIG
    ...
    /* Get the location of (slot numbers) loopback port in the active TDM calendar */
    SOC_IF_ERROR_RETURN(
            _soc_td2p_tdm_lb_port_indices_get(unit, table_num,
                slot_len, lb_port_tdm_indices,
                &count));

    /* Turn off Opportunistic Scheduling for the Loopback port */
    ...
    LOG_VERBOSE(BSL_LS_SOC_MMU,
            (BSL_META_U(unit,
                        "Writing 0x3f to LB port locations\n")));

    /* Write to the active TDM calendar table,
     * changing all instances of the LB port to 0x3f */
    SOC_IF_ERROR_RETURN(
            _soc_td2p_tdm_lb_port_indices_set(unit, table_num, slot_len,
                lb_port_tdm_indices, count, 0x3f));

    /* Waiting for LB port to be empty */
    SOC_IF_ERROR_RETURN(
            _soc_td2p_lb_port_flush(unit));

    /* Temporarily map each old physical port number to the Loopback Port MMU port */
    SOC_IF_ERROR_RETURN(
            _soc_td2p_mmu_map_phy_port_to_lb(unit, pre_count, pre_resource));
    ...
    SOC_IF_ERROR_RETURN(
            _soc_td2p_tdm_lb_port_indices_set(unit, table_num, slot_len,
                lb_port_tdm_indices, count, mmu_lb_port));
