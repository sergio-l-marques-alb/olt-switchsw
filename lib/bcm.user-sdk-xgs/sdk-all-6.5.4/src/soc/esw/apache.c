...
STATIC _soc_ap_mmu_buffer_config_t soc_ap_buffer_config[_AP_NUM_SKEWS] = {
    { 65536, 61376, 60495, 16, 50, 0, 4, 0, 50, 0, 4, 0 },
    { 53248, 46016, 45372, 13, 72, 0, 0, 0, 72, 0, 0, 0 },
    { 34816, 46016, 45372, 13, 72, 0, 0, 0, 72, 0, 0, 0 }
};
...
static  _soc_apache_ser_info_t _soc_apache_pgw0_cxxport_ser_info[] = {
     ...
     * 3 for TSC and 4 for CORE. No information as to what do with
     * TSC interrupts, hence ignoring them */
                               ...
                               soc_mem_t mem, int enable)
{
    ...
    if ((inst == 0) || (inst == 3)) {
        /* bad way - but what is the alternate? */
        enable_reg = CLPORT_INTR_ENABLEr;
        blk_idx    = SOC_BLK_CLPORT;
    } else {
        enable_reg = XLPORT_INTR_ENABLEr;
    ...
    /* Read reg val here */
    SOC_IF_ERROR_RETURN
        (soc_reg_get(unit, enable_reg, port, 0, &rval));
    SOC_IF_ERROR_RETURN
    ...
    SOC_IF_ERROR_RETURN
        (soc_reg_set(unit, enable_reg, port, 0, rval));
                                ...
                                const _soc_apache_ser_info_t *info)
{
    ...
    port = -1;
    SOC_BLOCK_ITER(unit, block_info_idx, SOC_BLK_CXXPORT) {
        if (SOC_BLOCK_INFO(unit, block_info_idx).number == inst) {
            port = SOC_BLOCK_PORT(unit, block_info_idx);
            break;
        }
                         ...
                         char *prefix_str)
{
    ...
    for (info_index = 0; ; info_index++) {
        ...
        case _SOC_PARITY_TYPE_CXXPORT:
            ...
            SOC_IF_ERROR_RETURN
                (_soc_apache_ser_process_cxxport(unit, inst, pipe, info));
            break;
        case _SOC_PARITY_TYPE_START_ERR:
            SOC_IF_ERROR_RETURN
...
soc_apache_mem_config(int unit)
{
    ...
    max_l2_entries = 272; /* Dedicated (16K) + Shared (256K)*/
    max_l3_entries = 260; /* Dedicated (4K) + Shared (256K)*/
    ...
    for (mmu_port = 0; mmu_port < _APACHE_MMU_PORTS_PER_PIPE; mmu_port++) {
        ...
        if (SOC_PBMP_MEMBER(si->eq_pbm, port)) { /* VBS (HSP) port */
            ...
            *divisible by 4 for PFC to work */ 
            lls_uc_cosq_base = (lls_uc_cosq_base + 3) & (~3);
                                 ...
                                 int port_lanes, int port_bandwidth)
{
    ...
    if (port_lanes == -1) {
        if (soc_apache_port_is_falcon(unit, phy_port)) {
            if (port_bandwidth > 53000) {
                port_lanes =  4;
            } else if (port_bandwidth > 27000) {
                port_lanes =  2;
            } else {
                port_lanes =  1;
            }
    ...
    case 15000:
        ...
        return (!soc_apache_port_is_falcon(unit, phy_port) && ((port_lanes == 2) || (port_lanes == 4)));
    case 30000:
...
soc_apache_port_config_bandwidth_check(int unit)
{
    ...
    for (pgw = 0; pgw < APACHE_PGWS_PER_DEV; pgw++) {
        ...
        if ((oversub_mode == 1) && (pgw_linerate_bandwidth != 0)) {
            LOG_ERROR(BSL_LS_SOC_COMMON,
                      (BSL_META_U(unit, "Oversub mode: Linerate BW for PGW %d is not 0.\n"), pgw));
            rv = SOC_E_FAIL;
        } else if ((oversub_mode == 0) && (pgw_oversub_bandwidth != 0)) {
            LOG_ERROR(BSL_LS_SOC_COMMON,
        ...
        if (pgw_linerate_bandwidth > max_pgw_core_bandwidth[pgw]) {
            LOG_CLI((BSL_META_U(unit,
                                "PGW_CL%d total line rate bandwidth "
                                "(%d Gb) exceeds %d Gb\n"),
                     pgw, (pgw_linerate_bandwidth / 1000),
                     (max_pgw_core_bandwidth[pgw] / 1000)));
            rv = SOC_E_FAIL;
        }

        /* Oversub Bandwidth check */
        if ((pgw_linerate_bandwidth + pgw_oversub_bandwidth) > max_pgw_oversub_bandwidth[pgw]) {
            LOG_CLI((BSL_META_U(unit,
                                "PGW_CL%d total oversub bandwidth "
                                "(%d Gb) exceeds %d Gb\n"),
                     pgw, ((pgw_linerate_bandwidth + pgw_oversub_bandwidth) / 1000),
...
soc_apache_chip_reset(int unit)
{
    ...
    if (si->frequency != def_freq) {
        ...
        SOC_IF_ERROR_RETURN(WRITE_TOP_CORE_CLK_FREQ_SELr(unit, rval));
        soc_reg_field_set(unit, TOP_CORE_CLK_FREQ_SELr, &rval, SW_CORE_CLK_SEL_ENf,
                          1);
        SOC_IF_ERROR_RETURN(WRITE_TOP_CORE_CLK_FREQ_SELr(unit, rval));
        soc_reg_field_set(unit, TOP_CORE_CLK_FREQ_SELr, &rval, SW_CORE_CLK_SEL_ENf,
                          0);
    ...
    fields[0] = XG_PLL0_HO_BYP_ENABLEf;
    values[0] = 1;
    fields[1] = XG_PLL1_HO_BYP_ENABLEf;
    values[1] = 1;
    fields[2] = XG_PLL2_HO_BYP_ENABLEf;
    values[2] = 1;
    fields[3] = XG_PLL3_HO_BYP_ENABLEf;
    values[3] = 1;
    SOC_IF_ERROR_RETURN
        (soc_reg_fields32_modify(unit, TOP_MISC_CONTROL_2r, REG_PORT_ANY, 4,
                                 fields, values));
#endif
    ...
    SOC_IF_ERROR_RETURN(READ_TOP_SOFT_RESET_REG_2r(unit, &rval));
#if 0
    soc_reg_field_set(unit, TOP_SOFT_RESET_REG_2r, &rval, TOP_CORE_PLL_RST_Lf,
                      1);
...
soc_apache_tsc_reset(int unit)
{
    ...
    SOC_BLOCKS_ITER(unit, blk, port_blocks) {
        port = SOC_BLOCK_PORT(unit, blk);
        SOC_IF_ERROR_RETURN(soc_tsc_xgxs_reset(unit, port, 0));
    }
...
{
    switch (speed) {
        case 5000:   *sp_spacing = 0x14; *spg = ap_ovs_speed_5G;   break;
        case 10000:  *sp_spacing = 0x14; *spg = ap_ovs_speed_10G;  break;
        case 20000:  *sp_spacing = 0x0a; *spg = ap_ovs_speed_20G;  break;
        case 25000:  *sp_spacing = 0x0a; *spg = ap_ovs_speed_25G;  break;
        case 40000:  *sp_spacing = 0x0a; *spg = ap_ovs_speed_40G;  break;
        case 50000:  *sp_spacing = 0x0a; *spg = ap_ovs_speed_50G;  break;
        case 100000: *sp_spacing = 0x04; *spg = ap_ovs_speed_100G; break;
        default:     *sp_spacing = 0x00;  *spg = ap_ovs_speed_None; break;
    }
                      ...
                      soc_apache_tdm_t *apache_tdm)
{
    ...
    for (group = 0; group < APACHE_MMU_OVS_GROUP_COUNT; group++) {
        ...
        APACHE_TDM_MMU_SPEED_GET(unit, speed_class);
        _soc_apache_mmu_ovs_speed_class_map_get(unit, speed_class, &speed_group, &speed_spacing);

        if (speed_group == ap_ovs_speed_None) {
            continue;
        }
        ...
        for (slot = 0; slot < APACHE_MMU_OVS_GROUP_TDM_LENGTH; slot++) {
            ...
            if (mmu_port == 0x7f) {
                ...
                if (speed_class == 0) {
                    ...
                    _soc_apache_mmu_ovs_speed_class_map_get(unit, speed_class,
...
extern int (*_phy_tscmod_firmware_set_helper[SOC_MAX_NUM_DEVICES])
            (int, int, uint8 *, int);
STATIC int
_soc_apache_tsc_firmware_set(int unit, int port, uint8 *array, int datalen)
{
...
_soc_portctrl_apache_port_init(int unit)
{
    ...
    SOC_BLOCK_ITER(unit, blk, SOC_BLK_CXXPORT) {
        ...
        if (si->port_speed_max[port] >= 100000) {
            ...
            soc_apache_port_speed_update(unit, port, si->port_speed_max[port]);
...
int
soc_apache_ports_edb_config(int unit, int asf, soc_pbmp_t pbm)
{
#define _AP_PGW_NUM_XLP 7  
#define _AP_PGW_NUM_CLP 2
    int rv;
    uint32 osct_pbm;
    uint64 tmp, lrct_pbm;
    soc_info_t *si;
    ...
    soc_field_t asm_ct_fields[_AP_PGW_NUM_XLP + _AP_PGW_NUM_CLP] = {
    ...
    SOC_PBMP_ITER(pbm, port) {
        ...
        if (pm_id < _AP_PGW_NUM_XLP) { /* Eagle Cores */
...
soc_apache_edb_init(int unit)
{
    ...
    SOC_IF_ERROR_RETURN(soc_apache_ports_edb_config(unit, 0, pbm));
...
_soc_apache_misc_init (int unit)
{
    ...
    uint32 values[2];
    egr_vlan_control_1_entry_t entry_vlan_control;
    ...
    PBMP_PORT_ITER(unit, port) {
        if (IS_XL_PORT(unit, port) || IS_XLB0_PORT(unit, port)) {
            SOC_IF_ERROR_RETURN
            ...
            SOC_IF_ERROR_RETURN
                (soc_reg_field32_modify(unit, XLPORT_CONFIGr, port, HIGIG_MODEf, 1));

        } else if (IS_CL_PORT(unit, port) || IS_CLG2_PORT(unit, port)) {
            SOC_IF_ERROR_RETURN
                (soc_reg_field32_modify(unit, CLPORT_SPARE0_REGr, port, RSVDf, 0));

            if (!IS_HG_PORT(unit, port)) {
                continue;
            }
            SOC_IF_ERROR_RETURN
                (soc_reg_field32_modify(unit, CLPORT_CONFIGr, port, HIGIG_MODEf, 1));
    ...
    SOC_BLOCK_ITER(unit, blk, SOC_BLK_XLPORT) {
        ...
        rval = 0;
        for (bindex = 0; bindex < APACHE_PORTS_PER_TSC; bindex++) {
    ...
    SOC_BLOCK_ITER(unit, blk, SOC_BLK_CLPORT) {
        ...
        if (si->port_speed_max[port] > 53000) {
            ...
            subport0 = si->port_p2l_mapping[phy_port_base];
            subport2 = si->port_p2l_mapping[phy_port_base + 2];
            if (subport0 >= 0 && si->port_speed_max[subport0] > 25000) {
                if (subport2 < 0 || si->port_speed_max[subport2] <= 000) {
                    mode = SOC_APACHE_PORT_MODE_TRI_023;
                } else {
                    mode = SOC_APACHE_PORT_MODE_DUAL;
                }
            } else if (subport2 >= 0 && si->port_speed_max[subport2] > 25000) {
                if (subport0 < 0 || si->port_speed_max[subport0] <= 25000) {
                    mode = SOC_APACHE_PORT_MODE_TRI_012;
                } else {
                    mode = SOC_APACHE_PORT_MODE_DUAL;
    ...
    SOC_IF_ERROR_RETURN(WRITE_ING_DEST_PORT_ENABLEm(unit, MEM_BLOCK_ALL,
                                                     0, &ingr_entry));
     ...
     */
    sal_memset(&entry_vlan_control, 0, sizeof(entry_vlan_control));

    soc_mem_field32_set(unit, EGR_VLAN_CONTROL_1m, &entry_vlan_control,VT_MISS_UNTAGf, 0);
    /* Enable pri/cfi remarking on egress ports. */
    soc_mem_field32_set(unit, EGR_VLAN_CONTROL_1m, &entry_vlan_control,REMARK_OUTER_DOT1Pf, 1);
    PBMP_ALL_ITER(unit, port) {
        SOC_IF_ERROR_RETURN(WRITE_EGR_VLAN_CONTROL_1m(unit,MEM_BLOCK_ALL, port, &entry_vlan_control));
    }
    ...
    _phy_tscmod_firmware_set_helper[unit] = _soc_apache_tsc_firmware_set;
    ...
    /* Enable re-direction */
    rval = 0;
    soc_reg_field_set(unit, EP_REDIR_CONTROLr, &rval,
              ENABLEf, 1);
    ...
    /* Enable RX/TX */
    rval = 0;
    soc_reg_field_set(unit, RDB_ENABLEr, &rval,
              RX_ENABLEf, 1);
    soc_reg_field_set(unit, RDB_ENABLEr, &rval,
              TX_ENABLEf, 1);
    soc_reg_field_set(unit, RDB_ENABLEr, &rval,
    ...
    /* Enable Credit */
    rval = 0;
    soc_reg_field_set(unit, RDB_CREDIT_ENABLEr, &rval,
              ENABLEf, 1);
...
soc_reg_t
_soc_apache_preg(int unit, int port, soc_reg_t regx, soc_reg_t regy)
{
    return regx;
}
                                ...
                                _soc_mmu_device_info_t *devcfg, int lossless)
{
    ...
    PBMP_ALL_ITER(unit, port) {
        mcq_entry_reserved += si->port_num_cosq[port] *
            _AP_MMU_PER_COSQ_EGRESS_QENTRY_RSVD;
        if (IS_LB_PORT(unit, port)) {
            /*QCN queue needs to be excluded*/
            mcq_entry_reserved -= _AP_MMU_PER_COSQ_EGRESS_QENTRY_RSVD;
        }
    ...
    PBMP_ALL_ITER(unit, port) {
        ...
        for (idx = 0; idx < si->port_num_cosq[port]; idx++) {
            ...
            if (lossless) {
                ...
                buf_queue->guarantee = min_cell_per_mcq ?  _soc_apache_min_cell_rsvd_per_mcq(unit, port, default_mtu_cells) : default_mtu_cells;
                               ...
                               soc_port_t input_port)
{
    ...
    PBMP_ALL_ITER(unit, port) {
        ...
        base = soc_apache_l2_hw_index(unit, qbase, 0) - 16384;

        mem1 = SOC_APACHE_PMEM(unit, port, MMU_THDM_DB_QUEUE_OFFSET_0m, 
                                        MMU_THDM_DB_QUEUE_OFFSET_1m);
        mem2 = SOC_APACHE_PMEM(unit, port, MMU_THDM_MCQE_QUEUE_OFFSET_0m, 
        ...
        mem = SOC_APACHE_PMEM(unit, port, MMU_THDM_MCQE_QUEUE_CONFIG_0m, 
        ...
        for (idx = 0; idx < _APACHE_MMU_NUM_POOL; idx++) {
            ...
            mem2 = SOC_APACHE_PMEM(unit, port, MMU_THDM_DB_PORTSP_CONFIG_0m, 
            ...
            mem2 = SOC_APACHE_PMEM(unit, port, MMU_THDM_MCQE_PORTSP_CONFIG_0m, 
    ...
    if (input_port == -1) {
        ...
        for (idx = 0; idx < 11; idx++) {
            ...
            soc_reg_field_set(unit, MMU_THDR_QE_LIMIT_COLOR_PRIQr, 
                    &rval3, SHARED_RED_LIMITf, lossless ? fval : 8);
            soc_reg_field_set(unit, MMU_THDR_QE_LIMIT_COLOR_PRIQr, 
                    &rval3, SHARED_YELLOW_LIMITf, lossless ? fval : 8);
...
soc_apache_mmu_config_shared_buf_recalc(int unit, uint32 spid, int shared_size, int flags)
{
    ...
    PBMP_PORT_ITER(unit, port) {
        ...
        /* Per  port per pool */
        idx = spid;    
        mem = SOC_APACHE_PMEM(unit, port, MMU_THDM_DB_PORTSP_CONFIG_0m, 
                                            MMU_THDM_DB_PORTSP_CONFIG_1m);
            ...
            int *temperature_count)
{
    ...
    for (index = 0; index < num_entries_out; index++) {
        ...
        fval = soc_reg_field_get(unit, reg, rval, MAX_PVT_DATAf);
...
soc_ap_cut_thru_enable_disable(int unit, soc_port_t port, int enable)
{
    ...
    SOC_PBMP_CLEAR(pbm);
    SOC_PBMP_PORT_ADD(pbm, port);
    SOC_IF_ERROR_RETURN(soc_apache_ports_edb_config(unit, enable, pbm));
...
/*
 * This function will be called during flexing operations for clearing, or
 * setting default values for port  registers and/or memories.
 * port_info is an array containing ports for which ipipe registers should
 * be set to default, or should be reset.
 */
STATIC int soc_ap_ip_reg_mem_clear(int unit, 
                                     soc_port_resource_t *port_info,
                                     int port_info_arr_size)
{
#if 0
    port_tab_entry_t ptab_ent;
    int idx;

    /* Clear PORT_TABm entry asscoiated with the port */
    sal_memcpy(&ptab_ent, soc_mem_entry_null(unit, PORT_TABm),
               sizeof (ptab_ent));

    for (idx = 0; idx < port_info_arr_size; idx++) {
        SOC_IF_ERROR_RETURN(soc_mem_write(unit, PORT_TABm, MEM_BLOCK_ALL,
                            port_info[idx].logical_port, &ptab_ent));
    }
                                   ...
                                   int lossless)
{
    ...
    /* No frequency update; no need to configure IARB TDM */
    ...
    for (idx = 0; idx < new_port_info_size; idx++) {
        ...
        if (new_port_info[idx].speed <= 21000) {
            ...
            SOC_IF_ERROR_RETURN(soc_ap_is_cut_thru_enabled(unit, port,
                                &cut_thru_en));

            if (cut_thru_en == TRUE) {
                /* Disable cut-through for this port */
                SOC_IF_ERROR_RETURN(soc_ap_cut_thru_enable_disable(unit,
                                    new_port_info[idx]->logical_port, 0));
            }
        }
    }
    ...
    SOC_IF_ERROR_RETURN(soc_ap_curr_egr_tdm_table_get(unit, 0,
                                                        &table_num));
    /* toggle the active calendar */
    APACHE_TDM_ACTIVE_CAL(unit) ^= 1;

    
    SOC_IF_ERROR_RETURN(soc_ap_curr_egr_tdm_table_set(unit, 0,
                                                        table_num));

    /* Switch over PGW's TDM calendars */
    idx = 0;

    for (pgw = 0; pgw  < APACHE_PGWS_PER_DEV; pgw++, idx++) { 
        obm_inst = pgw | SOC_REG_ADDR_INSTANCE_MASK;
        /* Switch over ingress calendar */

        SOC_IF_ERROR_RETURN(soc_ap_curr_ingr_tdm_table_get(unit,
                        obm_inst, &table_num));
        table_num ^= 0x1;

        SOC_IF_ERROR_RETURN(soc_ap_curr_ingr_tdm_table_set(unit,
                            obm_inst, table_num));
    }
    ...
    for (idx = 0; idx < new_port_info_size; idx++) {
        ...
        if (new_port_info[idx].speed <= 21000) {
            ...
            if (cut_thru_en == FALSE) {
                ...
                SOC_IF_ERROR_RETURN(soc_ap_cut_thru_enable_disable(unit,
                                    new_port_info[idx]->logical_port, 1));
            }
        }
    }

    /* Reconfigure OBM thresholds (Section 6.6.3.2, step #5) */
    SOC_IF_ERROR_RETURN(soc_ap_obm_threshold_set(unit,
                        curr_port_info_size, curr_port_info,
                        new_port_info_size, new_port_info,
                        lossless));
#endif
...
#if 0
    SOC_IF_ERROR_RETURN(soc_ap_pgw_port_hw_default_set(unit,
                                                         pre_num_res, pre_res,
                                                         lossless));
    SOC_IF_ERROR_RETURN(soc_ap_pgw_port_sw_default_set(unit,
                                  ...
                                  soc_ap_info_t *si_info)
{
    ...
    SOC_IF_ERROR_RETURN(soc_ap_ip_reg_mem_clear(unit, new_port_info,
                        new_port_info_size));
                               ...
                               soc_port_resource_t *flexport_data)
{
    ...
    SOC_IF_ERROR_RETURN
        (soc_ap_port_resource_mmu_mapping_set(unit, nport,
                                                flexport_data));
