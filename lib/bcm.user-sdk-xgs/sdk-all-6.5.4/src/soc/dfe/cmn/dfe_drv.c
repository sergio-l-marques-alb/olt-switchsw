 ...
 * $Id: dfe_drv.c,v 1.87 Broadcom SDK $
...
{    
    ...
    rv = soc_dcmn_fabric_pipe_map_initalize_valid_configurations(unit, 
                                                                     SOC_DFE_CONFIG(unit).fabric_tdm_priority_min == SOC_DFE_FABRIC_TDM_PRIORITY_NONE ? SOC_DCMN_FABRIC_PIPE_MAX_NUM_OF_PRIORITIES : SOC_DFE_CONFIG(unit).fabric_tdm_priority_min, /*Set to num of priorities in case no tdm priority*/
    ...
    if (nof_pipes == SOC_DFE_PROPERTY_UNAVAIL) /* soc property isn't supported*/
    {
    ...
    SOC_DFE_FABRIC_PIPES_CONFIG(unit).config_uc[3] = soc_dfe_property_suffix_num_get(unit, 3, spn_FABRIC_PIPE_MAP, "uc",1, SOC_DFE_PROPERTY_UNAVAIL); /* if we have one pipe then pipe should be 0, if two pipes then pipe should be 1, if three pipes then pipe should be 2 (default) */
...
soc_dfe_drv_soc_properties_access_read(int unit)
{
    ...
    SOC_CONTROL(unit)->schanTimeout = soc_dfe_property_get(unit, spn_SCHAN_TIMEOUT_USEC,1, SOC_CONTROL(unit)->schanTimeout);
    SOC_CONTROL(unit)->schanIntrEnb = soc_dfe_property_get(unit, spn_SCHAN_INTR_ENABLE, 0, SOC_DFE_PROPERTY_UNAVAIL);

    /*MIIM access*/
    SOC_CONTROL(unit)->miimTimeout =  soc_dfe_property_get(unit, spn_MIIM_TIMEOUT_USEC,0, SOC_DFE_PROPERTY_UNAVAIL);
    SOC_CONTROL(unit)->miimIntrEnb = soc_dfe_property_get(unit, spn_MIIM_INTR_ENABLE,0, SOC_DFE_PROPERTY_UNAVAIL);
...
soc_dfe_drv_soc_properties_chip_read(int unit)
{
    ...
    if (soc_property_get_str(unit, spn_SYSTEM_REF_CORE_CLOCK) != NULL) {
        ...
        if (SOC_DFE_CONFIG(unit).system_ref_core_clock ==  SOC_DFE_PROPERTY_UNAVAIL) /* system ref core clock wasn't configued in khz */
        {
            SOC_DFE_CONFIG(unit).system_ref_core_clock = soc_dfe_property_get(unit, spn_SYSTEM_REF_CORE_CLOCK, 0, SOC_DFE_PROPERTY_UNAVAIL) * 1000;
        }
    }
    ...
    if (soc_property_get_str(unit, spn_CORE_CLOCK_SPEED) != NULL) {
        ...
        if (SOC_DFE_CONFIG(unit).core_clock_speed ==  SOC_DFE_PROPERTY_UNAVAIL ) /* core clock speed wasn't configured in khz */
        {
            SOC_DFE_CONFIG(unit).core_clock_speed = soc_dfe_property_get(unit, spn_CORE_CLOCK_SPEED, 0, SOC_DFE_PROPERTY_UNAVAIL ) * 1000;
        }
    }
...
soc_dfe_drv_soc_properties_multicast_read(int unit)
{
    ...
    {
        SOC_DFE_CONFIG(unit).fe_mc_priority_map_enable = 1; /*default for fe1600*/
    }
...
soc_dfe_drv_soc_properties_port_read(int unit)
{
    ...
    if (SOC_DFE_CONFIG(unit).serdes_mixed_rate_enable !=  SOC_DFE_PROPERTY_UNAVAIL) /*i.e. supported*/
    {
...
soc_dfe_drv_soc_properties_fabric_cell_read(int unit)
{
    ...
    if (!SOC_DFE_CONFIG(unit).fabric_TDM_over_primary_pipe) {
        ...
        if ( SOC_DFE_CONFIG(unit).vcs128_unicast_priority != SOC_DFE_PROPERTY_UNAVAIL) /*if supported*/
        {
    ...
    if (SOC_DFE_CONFIG(unit).system_is_vcs_128_in_system !=  SOC_DFE_PROPERTY_UNAVAIL && SOC_DFE_CONFIG(unit).system_is_fe600_in_system != SOC_DFE_PROPERTY_UNAVAIL) /*i.e. not supported*/
    {
    ...
    if(SOC_DFE_CONFIG(unit).fabric_TDM_fragment != SOC_DFE_PROPERTY_UNAVAIL){ /* i.e. supported */
...
soc_dfe_drv_soc_properties_general_read(int unit)
{
    ...
    int rv;
    SOCDNX_INIT_FUNC_DEFS;
    if (!SOC_IS_BCM88773(unit) && !SOC_IS_BCM88774(unit) && !SOC_IS_BCM88775(unit) && !SOC_IS_BCM88776(unit)) {
        str = soc_dfe_property_get_str(unit, spn_FABRIC_DEVICE_MODE,0,NULL);
    } else {
        str = "SINGLE_STAGE_FE2";
    }
...
_soc_dfe_drv_soc_property_serdes_qrtt_read(int unit)
{
    ...
    int                 quad, quad_active, disabled_port, quad_index, port, rc, dis;
    ...
    for (quad = 0; quad < SOC_DFE_DEFS_GET(unit, nof_links)/4; quad++) {
         /* 
          * check both SERDES_QRTT_ACTIVE and PB_SERDES_QRTT_ACTIVE to see if one of the signals qrtt as inactive.
          * if one of them marks qrtt as inactive, disable qrtt  
          */
...
soc_dfe_cache_enable_init(int unit)
{
    ...
    if (enable_all) {
        if (soc_mem_iterate(unit, dfe_tbl_mem_cache_mem_set, &cache_enable) < 0)
            LOG_ERROR(BSL_LS_APPL_SOCMEM, (BSL_META_U(unit, "soc_dpe_cache_enable_init: unit %d all_cache enable failed\n"), unit));
    }

    if (enable_parity)
    {
        if (soc_mem_iterate(unit, dfe_tbl_mem_cache_enable_parity_tbl, &cache_enable) < 0)
            LOG_ERROR(BSL_LS_APPL_SOCMEM, (BSL_META_U(unit, "soc_dfe_cache_enable_init: unit %d parity cache enable failed\n"), unit));
    }

    if (enable_ecc)
    {
        if (soc_mem_iterate(unit, dfe_tbl_mem_cache_enable_ecc_tbl, &cache_enable) < 0)
            LOG_ERROR(BSL_LS_APPL_SOCMEM, (BSL_META_U(unit, "soc_dfe_cache_enable_init: unit %d ecc cache enable failed\n"), unit));
    }
...
/*
 * soc_dfe_deinit - 
 * This function main role is to resotore the status to be identical to the status after soc_dfe_attach. 
 * Important! - update deinit function each time soc_dfe_attach is updated. 
 */
int
