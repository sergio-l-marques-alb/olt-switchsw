        ...
        uint32 *ecc)
{
    ...
    SOCDNX_IF_ERR_EXIT(print_data(unit,data_len_bits, data_bits, "input    data", 0));
    SOCDNX_IF_ERR_EXIT(print_data(unit, ecc_len_bits+1, ecc_bits,  "input    ecc", 0));
    ...
    if (!count) {
        ...
        SOCDNX_IF_ERR_EXIT(print_data(unit, data_len_bits, data_bits, "corrcted data", 0));
        SOCDNX_IF_ERR_EXIT(print_data(unit, ecc_len_bits+1, calc_ecc_bits, "calc     ecc", 0));
        SOC_EXIT;
    }
    ...
    if (count==1) {
        ...
        SOCDNX_IF_ERR_EXIT(print_data(unit, ecc_len_bits, calc_ecc_bits, "correct  ecc", 0));
        ...
        uint32 *ecc)
{
    ...
    for (j=nof_portions-1,i=0;i<nof_portions;i++,j--) {
        ...
        SOCDNX_IF_ERR_EXIT(ecc_correction_std(unit,data_portion_len,tmp_data_portion, tmp_ecc_portion));
        ...
        uint32 *ecc)
{
    ...
    if (SOC_MEM_ECC_TYPE(unit,mem) ==SOC_MEM_ECC_TYPE_WIDE ) {
        SOCDNX_IF_ERR_EXIT(ecc_correction_wide(unit,data_len_bits,data, ecc));
