...
int oam_lb_tst_sat_start(int unit, int gtf_id)
 ...
 int oam_max_lb_tst_run(int unit, int port1, int port2, int port3)
 {
       ...
       for(index = 0; index <max_number; index++){
           ...
           print bcm_oam_loopback_add(0, lb[index]);
           
           tst_tx_ptr[index].tlv.tlv_length = 16;
       ...
       for(index = 0; index <max_number; index++){
          ...
          mep_id = 4100 + index;
          lb[index].id = mep_id;
          print bcm_oam_loopback_delete(0, lb[index]);
       }
       
       /*Before testing tst, must make sure the LB were deleted*/
       for(index = 0; index <max_number; index++){
           print bcm_oam_tst_rx_add(unit, tst_rx_ptr[index]);
           print bcm_oam_tst_tx_add(unit, tst_tx_ptr[index]);
         }
       ...
       for(index = 0; index <max_number; index++){
          ...
          print bcm_oam_tst_rx_delete(unit, tst_rx_ptr[index]);
          print bcm_oam_tst_tx_delete(unit, tst_tx_ptr[index]);
       }
