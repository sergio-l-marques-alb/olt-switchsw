...
int evpn_create_egress_tunnels(int unit,int config_wide_data){
    ...
    flags = BCM_MPLS_EGRESS_LABEL_IML | BCM_MPLS_EGRESS_LABEL_EVPN;
    rv = evpn_set_egress_tunnel(unit, add_to_flooding_domain, 1, flags, cint_evpn_info.pe1_pe3_iml_label, cint_evpn_info.pe1_pe3_lsp_label, \ /* 4.6, 6.4, 8.7 */
    ...
    rv = evpn_set_egress_tunnel(unit, add_to_flooding_domain, 1, \
                            flags, cint_evpn_info.pe1_pe4_iml_label, cint_evpn_info.pe1_pe4_lsp_label, \ /* 4.6, 6.4, 8.7 */
    ...
    rv = evpn_set_egress_tunnel(unit, add_to_flooding_domain, 1,  \
                            flags, cint_evpn_info.pe1_pe2_iml_label, cint_evpn_info.pe1_pe2_lsp_label, \ /* 4.6, 6.4, 8.7 */
    ...
    if (evpn_add_frr_label && !is_device_or_above(unit, JERICHO_PLUS)) {
...
int evpn_routing_into_evpn(int unit){
    ...
    */
    ...
    rv = multicast__create_forwarding_entry_dip_sip(unit, &forwarding_entry, cint_evpn_info.mc_id, /* inRif*/0,0); /* 8.9 */
    if (rv != BCM_E_NONE) {
...
int evpn_create_linker_layer_entries(int unit){
    ...
    if (evpn_add_frr_label) {
        arp_entry.mpls_extended_label_value = cint_evpn_info.pe1_frr_label; /* 3.7, 4.9, 7.9, 8.11 */
    }

    rv = roo__overlay_link_layer_create(unit, &arp_entry, 0, &encap_id);
    if (rv != BCM_E_NONE) {
    ...
    if (evpn_add_frr_label) {
        arp_entry.mpls_extended_label_value = cint_evpn_info.pe2_frr_label; /* 3.7, 4.9, 7.9, 8.11 */
    }
