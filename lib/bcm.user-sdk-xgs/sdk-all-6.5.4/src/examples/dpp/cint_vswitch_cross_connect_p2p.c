...
run(int unit, int port1, int type1, int protection1, int second_port1 /* used if protection1 is set */, 
    ...
    case 1:
        /* create vlan port */
        rv = vlan_port_create_dummy(unit,
        ...
        if (protection1) {
           ...
           if (!decoupled_mode) {
            ...
            if(protection1 == 2) {
                ...
                /* add gport to protection MC */
                rv = multicast__mpls_port_add(unit,cross_connect_info.protection_mc1,cross_connect_info.port_1,cross_connect_info.mpls_port_1.mpls_port_id, 0/*is_egress*/);
                if (rv != BCM_E_NONE) {
                    printf("Error, in multicast_vpls_port_add with port %d\n", cross_connect_info.port_1);
                    return rv;
                }
                rv = multicast__mpls_port_add(unit,cross_connect_info.protection_mc1,cross_connect_info.port_3,cross_connect_info.mpls_port_2.mpls_port_id, 0 /*is_egress*/);
                if (rv != BCM_E_NONE) {
                    printf("Error, in multicast_vpls_port_add with port %d\n", cross_connect_info.port_1);
                    return rv;
                }
