...
/*  if (vxlan_roo_param.add_double_vlan_overlay) {
  ...
  vxlan_port_add.add_vxlan_port_to_vsi_flooding_mc_group = 0; 
  ...
  bcm_if_t cud_encap_id;
  rv = bcm_multicast_egress_object_encap_get(unit, g_vxlan_roo.vpn /* VSI flooding MC group */, out_tunnel_intf_ids_only[1], &cud_encap_id); 
  if (rv != BCM_E_NONE) {
      printf("Error, bcm_multicast_egress_object_encap_get\n");
      return rv; 
  }
  rv = bcm_multicast_ingress_add(unit,  g_vxlan_roo.vpn /* VSI flooding MC group */, provider_port, cud_encap_id);
  if (rv != BCM_E_NONE) {
