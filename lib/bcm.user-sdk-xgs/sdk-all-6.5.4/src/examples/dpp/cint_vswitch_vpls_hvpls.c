...
vswitch_vpls_hvpls_info_init(int unit, int ac_port, int pwe_in_port, int pwe_out_port, int ac_port1_outer_vlan, int ac_port1_inner_vlan, int vpn_id, int two_calls) {
    ...
    egress_mc = is_device_or_above(unit,JERICHO_PLUS) ? 0 : 1;
...
vswitch_vpls_hvpls_run(int unit){
    ...
    create_l3_egress_params.l3_flags = BCM_L3_CASCADED|(mpls_port_eei?0:BCM_L3_ENCAP_SPACE_OPTIMIZED); /* MPLS FEC optimized includes EEI, otherwise OutLif */
    ...
    rv = bcm_multicast_set(unit, mc_id, is_device_or_above(unit,JERICHO_PLUS) ? BCM_MULTICAST_INGRESS : 0, 5, reps);
    if (rv != BCM_E_NONE) {
