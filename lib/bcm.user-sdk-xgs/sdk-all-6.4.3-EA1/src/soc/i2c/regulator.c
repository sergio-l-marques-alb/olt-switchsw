...
soc_i2c_regulator_set_volt(int unit, void *voltcfg, int uV)
{
    ...
    if (I2C_REGULATOR_VOLT_WR_DEV(vcfg) == NULL) {
        cli_out("%s is not configured to set!\n",
                                        vcfg->function);
        return SOC_E_FAIL;
    }
    ...
    if (dac_param != NULL) {
        if ((uV > dac_param->uV_max) || (uV < dac_param->uV_min)) {
            cli_out("%s is configured with max=%duV "
                                          "and min=%duV, value %duV is "
                                          "invalid!\n",
                                          vcfg->function,
                                          dac_param->uV_max,
                                          dac_param->uV_min,
    ...
    if (dac < 0) {
        cli_out("Failed to open %s : %s\n",
                    I2C_REGULATOR_VOLT_WR_DEV(vcfg), bcm_errmsg(dac));
        return dac;
    }

    if ((rv = bcm_i2c_ioctl(unit, dac, I2C_REGULATOR_IOC_VOLT_SET,
                        &uV, vcfg->chan)) < 0) {
        cli_out("%s Failed to set voltage : %s\n",
                    I2C_REGULATOR_VOLT_WR_DEV(vcfg), bcm_errmsg(rv));
    }
...
soc_i2c_regulator_get_volt(int unit, void *voltcfg, int *uV)
{
    ...
    if (I2C_REGULATOR_VOLT_RD_DEV(vcfg) == NULL) {
        LOG_ERROR(BSL_LS_SOC_COMMON, (BSL_META_U(unit, "%s is not configured to get!\n"),
                                        vcfg->function));
        return SOC_E_FAIL;
    }
    ...
    if (adc < 0) {
        cli_out("Failed to open %s : %s\n",
                    I2C_REGULATOR_VOLT_RD_DEV(vcfg), bcm_errmsg(adc));
    }

    if ((rv = bcm_i2c_ioctl(unit, adc, I2C_REGULATOR_IOC_VOLT_GET, uV,
                                vcfg->chan)) < 0) {
        cli_out("%s Failed to get voltage : %s\n",
                    I2C_REGULATOR_VOLT_RD_DEV(vcfg), bcm_errmsg(rv));
    }
