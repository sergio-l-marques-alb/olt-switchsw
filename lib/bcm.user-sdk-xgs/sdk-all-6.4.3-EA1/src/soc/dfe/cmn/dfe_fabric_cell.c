...
soc_dfe_fabric_cell_load(int unit)
{
     ...
     * Parse cell
     */
    rv = soc_dfe_fabric_cell_parse(unit, entry, &cell_info);
    SOCDNX_IF_ERR_EXIT(rv);
 ...
 */
soc_error_t
soc_dfe_fabric_cell_parse(int unit, soc_dcmn_fabric_cell_entry_t entry, soc_dcmn_fabric_cell_info_t *cell_info)
{
    int rv;
    uint32 nof_lines;
    ...
    rv = MBCM_DFE_DRIVER_CALL(unit, mbcm_dfe_fabric_cell_parse_table_get, (unit, cell_info->cell_type, SOC_DCMN_FABRIC_CELL_PARSE_TABLE_MAX_NOF_LINES, parse_table, &nof_lines));
    SOCDNX_IF_ERR_EXIT(rv);

    rv = soc_dcmn_fabric_cell_parser(unit, entry, parse_table, nof_lines, cell_info);
    SOCDNX_IF_ERR_EXIT(rv);
