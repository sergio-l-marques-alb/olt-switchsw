...
soc_fe3200_fabric_link_status_get(int unit, soc_port_t link_id, uint32 *link_status, uint32 *errored_token_count)
{
  ...
  /*link status
    DCMN_FABRIC_LINK_STATUS_CRC_ERROR Non-zero CRC rate  
    DCMN_FABRIC_LINK_STATUS_SIZE_ERROR Non-zero size error-count  
    DCMN_FABRIC_LINK_STATUS_CODE_GROUP_ERROR Non-zero code group error-count  
    DCMN_FABRIC_LINK_STATUS_MISALIGN Link down, misalignment error  
    DCMN_FABRIC_LINK_STATUS_NO_SIG_LOCK Link down, SerDes signal lock error  
    DCMN_FABRIC_LINK_STATUS_NO_SIG_ACCEP Link up, but not accepting reachability cells  
    DCMN_FABRIC_LINK_STATUS_ERRORED_TOKENS Low value, indicates bad link connectivity or link down, based on reachability cells */
    ...
    *field_val = soc_reg_field_get(unit, FMAC_FMAC_INTERRUPT_REGISTER_4r, reg_val, DEC_ERR_INTf);
    ...
    srd_arr_id = (link_id % SOC_DFE_DEFS_GET(unit, nof_links_in_fsrd))/SOC_DFE_DEFS_GET(unit, nof_links_in_quad); /* quad number in fsrd */
    ...
    *sig_lock = soc_reg_field_get(unit, FSRD_SRD_QUAD_STATUSr, reg_val, SRD_QUAD_N_SYNC_STATUSf);
    ...
    /*SOCDNX_IF_ERR_EXIT(MBCM_DFE_DRIVER_CALL(unit, mbcm_dfe_port_phy_control_get, (unit, link_id, SOC_PHY_CONTROL_RX_SIGNAL_DETECT, &sig_acc)));
    if(!sig_acc) {
...
/*
 * Function:
 *      soc_fe3200_fabric_link_status_clear
 * Purpose:
