/*
 * $Id: drc_combo28_cb.c,v 1.1.2.9 Broadcom SDK $
...
#include <soc/dpp/ARAD/arad_dram.h> /* For Bist operation */
...
int soc_dpp_drc_combo28_shmoo_drc_bist_conf_set(int unit, int drc_ndx, combo28_bist_info_t info)
{
    ...
    if (info.bist_timer_us == 0x0) {
        ...
        if (info.mpr_mode != 0x0 ) {
            ...
            /* bist_info.arad_bist_flags         |= SOC_DPP_DRC_COMBO28_BIST_FLAGS_STAGGER_INCREMENT_MODE; */
            ...
            /* bist_info.arad_bist_flags |= SOC_DPP_DRC_COMBO28_BIST_FLAGS_BG_INTERLEAVE; */
...
int soc_dpp_drc_combo28_shmoo_drc_bist_err_cnt(int unit, int drc_ndx, combo28_bist_err_cnt_t *info)
{
    ...
    soc_sand_rv = arad_dram_read_bist_err_cnt(unit, 
                                              drc_ndx, 
                                              &(info->bist_err_occur), 
                                              &(info->bist_full_err_cnt), 
                                              &(info->bist_single_err_cnt), 
                                              &(info->bist_global_err_cnt));
...
int soc_dpp_drc_combo28_shmoo_dram_init(int unit, int drc_ndx, int phase)
{
    ...
    if (dram_type == SOC_DPP_DRC_COMBO28_FLD_VAL_DRAM_TYPE_DDR4) {
        ...
        /* Enable and disable to reset the dram divider should be after the dram init */
        if (init_phase == 0) {
...
int soc_dpp_drc_combo28_shmoo_drc_pll_set(int unit, int drc_ndx, CONST combo28_drc_pll_t *pll_info)
{
    ...
    /*soc_reg_field_set(unit, DRCA_DDR_PHY_PLL_RCr, &reg_val, DIV2_MODEf, pll_info->div2_mode);*/
    ...
    LOG_VERBOSE(BSL_LS_SOC_DRAM,
                (BSL_META_U(unit,
                            "%s(): drc_ndx=%d, Wait PLL lock ...\n"), FUNCTION_NAME(), drc_ndx));

     soc_sand_rv = arad_polling(
          unit,
          ARAD_TIMEOUT,
          ARAD_MIN_POLLS,
          DRC_DDR_PHY_PLL_STATUSl[drc_ndx],
          REG_PORT_ANY,
          0,
          PLL_LOCKf,
          1
...
int soc_dpp_drc_combo28_gddr5_shmoo_drc_bist_conf_set(int unit, int drc_ndx, combo28_gddr5_bist_info_t info)
{
    ...
    bist_info.data_pattern_mode = 0; /* custom pattern */
...
int soc_dpp_drc_combo28_shmoo_drc_enable_write_leveling(int unit, int drc_ndx, uint32 command_parity_lattency, int use_continious_gddr5_dqs, int enable)
{
    ...
    if (enable) {
        ...
        /* Make sure only deselect by setting parity latency */
        if (command_parity_lattency == 0) {
            ...
            COMPILER_64_SET(field64_val, 0x0, 0x4);
        ...
        if (use_continious_gddr5_dqs == 1) { 
            ...
            /* Change to GDDR5 and provide continious DQS */ 
            LOG_VERBOSE(BSL_LS_SOC_DRAM,
                        (BSL_META_U(unit,
            ...
            SOCDNX_IF_ERR_EXIT(soc_reg64_set(unit, DRC_DRAM_COMPLIANCE_CONFIGURATION_REGISTERl[drc_ndx], REG_PORT_ANY, 0, reg64_val));
             ...
             LOG_VERBOSE(BSL_LS_SOC_DRAM,
                         (BSL_META_U(unit,
                                     "%s(): Drive DQS low.\n"), FUNCTION_NAME()));
             SOCDNX_IF_ERR_EXIT(soc_reg64_get(unit, DRC_DRAM_COMPLIANCE_CONFIGURATION_REGISTERl[drc_ndx], REG_PORT_ANY, 0, &reg64_val));
             soc_reg64_field32_set(unit, DRCA_DRAM_COMPLIANCE_CONFIGURATION_REGISTERr, &reg64_val, FORCE_DQS_VALf, 0x0);
        ...
        if (1 == use_continious_gddr5_dqs) {
            ...
            /* Change back to ddr4 mode */
            LOG_VERBOSE(BSL_LS_SOC_DRAM,
                        (BSL_META_U(unit,
                                    "%s(): Change back to DDR4 mode.\n"), FUNCTION_NAME()));
            SOCDNX_IF_ERR_EXIT(soc_reg64_get(unit, DRC_DRAM_COMPLIANCE_CONFIGURATION_REGISTERl[drc_ndx], REG_PORT_ANY, 0, &reg64_val));
        ...
        /* Disabling command force */
        LOG_VERBOSE(BSL_LS_SOC_DRAM,
                    (BSL_META_U(unit,
                                "%s(): Disabling command force.\n"), FUNCTION_NAME()));
       SOCDNX_IF_ERR_EXIT(soc_reg_get(unit, DRC_CPU_COMMANDSl[drc_ndx], REG_PORT_ANY, 0, &reg64_val));
        ...
        /* Retriving drc_CommandParityLattency */
        if (command_parity_lattency == 0) {
            LOG_VERBOSE(BSL_LS_SOC_DRAM,
                        (BSL_META_U(unit,
...
int soc_dpp_drc_combo28_shmoo_write_mpr(int unit, int drc_ndx, uint32 intial_calib_mpr_addr, uint32 mpr_mode, uint32 mpr_page, uint32 mrs_readout, int enable_mpr)
{
    ...
    SOCDNX_IF_ERR_EXIT(soc_reg32_get(unit, DRC_INTIAL_CALIB_USE_MPRl[drc_ndx], REG_PORT_ANY, 0, &reg_val));
    SOC_SAND_SET_BIT(intial_calib_mpr_addr, 0x1, 12); /* Use regular burst length 0f 8 */ 
...
int soc_dpp_drc_combo28_shmoo_mpr_load(int unit, int drc_ndx, uint8 *mpr_pattern)
{
    ...
    for (mpr_location = 0; mpr_location < SHMOO_COMBO28_BIST_MPR_NOF_PATTERNS; mpr_location++) {
        ...
        rv = soc_dpp_drc_combo28_dram_cpu_command(unit, drc_ndx, 1 /* ras_n */, 0 /* cas_n */, 0 /* we_n */, 1 /* act_n */, bank /* bank */, cpu_addr /* address */);
        SOCDNX_IF_ERR_EXIT(rv);
    }
...
int soc_dpp_drc_combo28_shmoo_drc_active_gddr5_cmd(int unit, int drc_ndx)
{
    ...
    LOG_VERBOSE(BSL_LS_SOC_DRAM,
                (BSL_META_U(unit,
                            "%s(): Activate command.\n"), FUNCTION_NAME()));    
    rv = soc_dpp_drc_combo28_dram_cpu_command(unit, drc_ndx, 0 /* ras_n */, 1 /* cas_n */, 1 /* we_n */, 1 /* act_n */, 0 /* bank */, 0 /* address */);
    SOCDNX_IF_ERR_EXIT(rv);
...
int soc_dpp_drc_combo28_shmoo_drc_precharge_all(int unit, int drc_ndx)
{
    ...
    rv = soc_dpp_drc_combo28_dram_cpu_command(unit, drc_ndx, 0 /* ras_n */, 1 /* cas_n */, 0 /* we_n */, 1 /* act_n */, 0 /* bank */, cpu_addr /* address */);
    SOCDNX_IF_ERR_EXIT(rv);
...
int soc_dpp_drc_combo28_shmoo_vendor_info_get(int unit, int drc_ndx, combo28_vendor_info_t *info)
{
    ...
    soc_reg_field_set(unit, DRCA_GDDR_5_BIST_CONFIGURATIONSr, &reg_val, GDDR_5_BIST_MODEf, 4 /* DRC_GDDR5_VENDOR_ID_MODE */);
    ...
    info->manufacture_id  = SOC_SAND_SET_BITS_RANGE(SOC_SAND_GET_BITS_RANGE(reg_above64_val[0], 11, 11), 0, 0);
    info->manufacture_id |= SOC_SAND_SET_BITS_RANGE(SOC_SAND_GET_BITS_RANGE(reg_above64_val[0], 10, 10), 1, 1);
    info->manufacture_id |= SOC_SAND_SET_BITS_RANGE(SOC_SAND_GET_BITS_RANGE(reg_above64_val[0], 9, 9), 2, 2);
    info->manufacture_id |= SOC_SAND_SET_BITS_RANGE(SOC_SAND_GET_BITS_RANGE(reg_above64_val[0], 8, 8), 3, 3);
    info->revision_id     = SOC_SAND_SET_BITS_RANGE(SOC_SAND_GET_BITS_RANGE(reg_above64_val[0], 15, 15), 0, 0);
    info->revision_id    |= SOC_SAND_SET_BITS_RANGE(SOC_SAND_GET_BITS_RANGE(reg_above64_val[0], 14, 14), 1, 1);
    info->revision_id    |= SOC_SAND_SET_BITS_RANGE(SOC_SAND_GET_BITS_RANGE(reg_above64_val[0], 13, 13), 2, 2);
    info->revision_id    |= SOC_SAND_SET_BITS_RANGE(SOC_SAND_GET_BITS_RANGE(reg_above64_val[0], 12, 12), 3, 3);
    info->dram_density    = SOC_SAND_SET_BITS_RANGE(SOC_SAND_GET_BITS_RANGE(reg_above64_val[0], 24, 24), 0, 0);
    info->dram_density   |= SOC_SAND_SET_BITS_RANGE(SOC_SAND_GET_BITS_RANGE(reg_above64_val[0], 25, 25), 1, 1);
    info->fifo_depth      = SOC_SAND_SET_BITS_RANGE(SOC_SAND_GET_BITS_RANGE(reg_above64_val[0], 26, 26), 0, 0);
    info->fifo_depth     |= SOC_SAND_SET_BITS_RANGE(SOC_SAND_GET_BITS_RANGE(reg_above64_val[0], 27, 27), 1, 1);
...
int soc_dpp_drc_combo28_shmoo_drc_dqs_pulse_gen(int unit, int drc_ndx, int use_continious_gddr5_dqs)
{
    ...
    if (use_continious_gddr5_dqs) {
        ...
        SOCDNX_IF_ERR_EXIT(soc_reg64_get(unit, DRC_DRAM_COMPLIANCE_CONFIGURATION_REGISTERl[drc_ndx], REG_PORT_ANY, 0, &reg64_val));         
        soc_reg64_field32_set(unit, DRCA_DRAM_COMPLIANCE_CONFIGURATION_REGISTERr, &reg64_val, FORCE_DQS_VALf, 0x1);
        SOCDNX_IF_ERR_EXIT(soc_reg64_set(unit, DRC_DRAM_COMPLIANCE_CONFIGURATION_REGISTERl[drc_ndx], REG_PORT_ANY, 0, reg64_val));
        soc_reg64_field32_set(unit, DRCA_DRAM_COMPLIANCE_CONFIGURATION_REGISTERr, &reg64_val, FORCE_DQS_VALf, 0x0);
        SOCDNX_IF_ERR_EXIT(soc_reg64_set(unit, DRC_DRAM_COMPLIANCE_CONFIGURATION_REGISTERl[drc_ndx], REG_PORT_ANY, 0, reg64_val));
    }
...
int soc_dpp_drc_combo28_shmoo_enable_refresh(int unit, int drc_ndx , int enable, uint32 new_trefi, uint32 * curr_refi)
{
    ...
    static soc_reg_t
        DRC_AC_OPERATING_CONDITIONS_4l[] = {DRCA_AC_OPERATING_CONDITIONS_4r, DRCB_AC_OPERATING_CONDITIONS_4r, DRCC_AC_OPERATING_CONDITIONS_4r,
                                            DRCD_AC_OPERATING_CONDITIONS_4r, DRCE_AC_OPERATING_CONDITIONS_4r};
    ...
    /* Read current tREFI from register (relevant only when disabling refresh) */
