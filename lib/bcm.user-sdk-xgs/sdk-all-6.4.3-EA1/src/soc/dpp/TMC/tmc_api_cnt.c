/* $Id: tmc_api_cnt.c,v 1.12 Broadcom SDK $
...
/******************************************************************
*
* FILENAME:       DuneDriver/tmc/src/soc_tmcapi_cnt.c
*
* MODULE PREFIX:  tmc
*
* FILE DESCRIPTION:
  ...
  )
{
  ...
  sal_memset(info, 0x0, sizeof(SOC_TMC_CNT_VOQ_PARAMS));
  info->start_q = 0;
  ...
  )
{
  ...
  case SOC_TMC_CNT_MODE_ING_FWD_SIMPLE_COLOR:
    ...
    break;
  case SOC_TMC_CNT_MODE_ING_DROP_SIMPLE_COLOR:
  ...
  )
{
  ...
  LOG_CLI((BSL_META_U(unit,
                      "start_q: %u\n\r"),info->start_q));
  LOG_CLI((BSL_META_U(unit,
                      "q_set_size %s "), SOC_TMC_CNT_Q_SET_SIZE_to_string(info->q_set_size)));
exit:
  SOC_SAND_VOID_EXIT_AND_SEND_ERROR(0, 0, 0);
}
  ...
  )
{
  ...
  LOG_CLI((BSL_META_U(unit,
                      "mode_ing %s "), SOC_TMC_CNT_MODE_ING_to_string(info->mode_ing)));
  SOC_TMC_CNT_MODE_EG_print(&(info->mode_eg));
  LOG_CLI((BSL_META_U(unit,
                      "voq_cnt:")));
  SOC_TMC_CNT_VOQ_PARAMS_print(&(info->voq_cnt));
  LOG_CLI((BSL_META_U(unit,
                      "stag_lsb: %u\n\r"),info->stag_lsb));
  if(info->mode_ing == SOC_TMC_CNT_MODE_ING_CONFIGURABLE_OFFSETS){
                           ...
                           )
{
  ...
  info->src_type = SOC_TMC_CNT_NOF_SRC_TYPES;
  info->mode_ing = SOC_TMC_CNT_NOF_MODE_INGS;
 
    info->mode_eg.base_val = 0;
  info->mode_eg.resolution = SOC_TMC_CNT_NOF_MODE_EGS;
  SOC_TMC_CNT_VOQ_PARAMS_clear(&(info->voq_cnt));
