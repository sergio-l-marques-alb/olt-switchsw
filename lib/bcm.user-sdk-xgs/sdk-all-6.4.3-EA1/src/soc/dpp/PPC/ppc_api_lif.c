...
void
  SOC_PPC_INLIF_PROFILE_USAGE_print(
    SOC_SAND_IN uint32 inlif_profile,
    SOC_SAND_IN SOC_PPC_PROFILE_MGMT_INLIF_PROFILE_TYPE *arr_profile_bit_assgn
  ){
    ...
    SOC_SAND_CHECK_NULL_INPUT(arr_profile_bit_assgn);

    for (pfl_type = 0; pfl_type < SOC_PPC_PROFILE_MGMT_INLIF_PROFILE_TYPE_NOF; pfl_type++) {
        pfl_value = -1;
        pfl_mask = 1;
        for (pfl_bit = 0; pfl_bit < SOC_PPC_PROFILE_MGMT_INLIF_PROFILE_NOF_BITS; pfl_bit++) {
            if (arr_profile_bit_assgn[pfl_bit] == pfl_type) {
                if (pfl_value < 0) {
                    pfl_value = 0;
                }
                if (inlif_profile & (1<<pfl_bit)) {
                    pfl_value |= pfl_mask;
                }
                pfl_mask <<= 1;
            }
        ...
        if (pfl_value >= 0) {
            LOG_CLI((BSL_META_U(unit, "\t%s: %d\n\r"),
                        SOC_PPC_INLIF_PROFILE_TYPE_to_short_string(pfl_type), 
                        pfl_value
                    ));
        }
    }
...
void
  SOC_PPC_OUTLIF_PROFILE_USAGE_print(
    SOC_SAND_IN uint32 outlif_profile,
    SOC_SAND_IN SOC_PPC_PROFILE_MGMT_OUTLIF_PROFILE_TYPE *arr_profile_bit_assgn
  ){
    ...
    /* The outlif profile value isn't printed in the normal diag prints
     * because it's a Jericho and later only property
     */
    LOG_CLI((BSL_META_U(unit, "out_lif_profile: %u\n\r"),
                outlif_profile
            ));
    for (pfl_type = 0; pfl_type < SOC_PPC_PROFILE_MGMT_OUTLIF_PROFILE_TYPE_NOF; pfl_type++) {
        pfl_value = -1;
        pfl_mask = 1;
        ...
        if (pfl_value >= 0) {
            LOG_CLI((BSL_META_U(unit, "\t%s: %d\n\r"),
                        SOC_PPC_OUTLIF_PROFILE_TYPE_to_short_string(pfl_type), 
                        pfl_value
                    ));
        }
    }
