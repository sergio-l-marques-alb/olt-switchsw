...
/* Poll that the glem management machine is done. */
#define SOC_JER_LIF_GLEM_POLL_MANAGEMENT_COMPLETE(_unit) \
        (arad_polling(unit, ARAD_TIMEOUT, ARAD_MIN_POLLS, EDB_GLEM_INTERRUPT_REGISTER_ONEr, MEM_BLOCK_ANY, 0, GLEM_MANAGEMENT_COMPLETEDf, 1))

/* Poll that the glem diagnostics machine is done. */
#define SOC_JER_LIF_GLEM_POLL_DIAGNOSTICS_LOOKUP_COMPLETE(_unit) \
        (arad_polling(unit, ARAD_TIMEOUT, ARAD_MIN_POLLS, PPDB_B_LARGE_EM_DIAGNOSTICSr, REG_PORT_ANY, 0, GLEM_DIAGNOSTICS_LOOKUPf, 0))
/* } */
/*************
 * TYPE DEFS *
 *************/
...
soc_jer_lif_glem_access_entry_by_key_get(int unit, int global_lif_id, int *egress_lif_id, uint8 *accessed, uint8 *is_found){
    ...
    *fld_val = 0x1;
     ...
     */
    rv = SOC_JER_LIF_GLEM_POLL_DIAGNOSTICS_LOOKUP_COMPLETE(unit); 
    SOCDNX_IF_ERR_EXIT(rv);
...
soc_jer_lif_glem_access_write_to_machine(int unit, uint32 *buffer){
    ...
    /* 
     *  Assert that the previous GLEM access is done, and GLEM can be accessed.
     */
    ...
    rv = soc_mem_write(unit, EDB_GLEM_MANAGEMENT_REQUESTm, MEM_BLOCK_ALL, 0, buffer);
    SOCDNX_IF_ERR_EXIT(rv);

    /* 
     * Poll that the writing is complete.
     */

    rv = SOC_JER_LIF_GLEM_POLL_MANAGEMENT_COMPLETE(unit);
    SOCDNX_IF_ERR_EXIT(rv);
    ...
    rv = soc_reg_field32_modify(unit, EDB_GLEM_INTERRUPT_REGISTER_ONEr, MEM_BLOCK_ANY, GLEM_MANAGEMENT_COMPLETEDf, 1);
    SOCDNX_IF_ERR_EXIT(rv);
