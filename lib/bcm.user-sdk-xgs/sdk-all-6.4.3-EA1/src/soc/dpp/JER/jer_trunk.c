   ...
   )
{
    ...
    SOCDNX_IF_ERR_EXIT(soc_port_sw_db_tm_port_to_base_q_pair_get(unit, local_port, 0, &base_q_pair));
    SOCDNX_IF_ERR_EXIT(soc_port_sw_db_tm_port_to_out_port_priority_get(unit, local_port, 0, &nof_pairs));
    ...
    for (curr_q_pair = base_q_pair; curr_q_pair - base_q_pair < nof_pairs; curr_q_pair++)
    {
        ...
        if(use_table_2 == 1){
            /* update PPCT */
            if (0x1 == set_min) {
                egq_pp_ppct_data.second_range_lb_key_min = min_lb_key;
            }
            if (0x1 == set_max) {
                egq_pp_ppct_data.second_range_lb_key_max = max_lb_key;
            }
        } else {
            /* update PPCT */
            if (0x1 == set_min) {
   ...
   )
{
    ...
    SOCDNX_IF_ERR_EXIT(
     ...
     if(use_table_2 == 1){
         ...
         if (min_lb_key != NULL) {
           *min_lb_key = egq_pp_ppct_data.second_range_lb_key_min;
         }
         if (max_lb_key != NULL) {
           *max_lb_key = egq_pp_ppct_data.second_range_lb_key_max;
         }
     }
exit:
  SOCDNX_FUNC_RETURN;
...
int soc_jer_trunk_init_tables (int unit)
{
    ...
    mem = EGQ_PER_PORT_CONFIGURATION_TABLE_PPCTm;
    for (index = soc_mem_index_min(unit,mem) ; index < soc_mem_index_count(unit,mem) ; index++) {
        SOCDNX_IF_ERR_EXIT(READ_EGQ_PER_PORT_CONFIGURATION_TABLE_PPCTm (unit, EGQ_BLOCK(unit, 0), index, egq_data));
        ...
        SOCDNX_IF_ERR_EXIT(WRITE_EGQ_PER_PORT_CONFIGURATION_TABLE_PPCTm (unit, EGQ_BLOCK(unit, SOC_CORE_ALL), index, egq_data));
    }
