...
/* $Id: arad_stat_if.c,v 1.34 Broadcom SDK $
  ...
  )
{
    uint32 
     ...
     soc_dpp_config_arad_t 
       ...
       if (info->if_pkt_size == SOC_TMC_NOF_STAT_IF_PKT_SIZES) {
       ...
       /* Get the interface-id for this Wrap Core */
       ...
       core_clock_speed = dpp_arad->init.core_freq.frequency/1000;
        ...
        if(record_rate >= core_clock_speed) {
            *limit_prd_val = 0;
            ...
            /* recored_rate/core_clock_speed = limit_prd_val/(limit_prd_val+1) */
            max_machine_clock_value = (record_rate / (core_clock_speed - record_rate));
#ifdef BCM_88660_A0
            if (SOC_IS_ARADPLUS(unit)) {
                if (max_machine_clock_value < 1) {
                    max_machine_clock_value = 1;
                }
            } else 
#endif
            {
  ...
  )
{
  uint32
   res,
   soc_sand_rv;
  uint32 port_rxaui_14, port_rxaui_15;
  uint8
      found_rxaui_14, found_rxaui_15, is_nif;
  int core_rxaui_14, core_rxaui_15;

  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_MGMT_FUNCTIONAL_INIT);

  /* 
   * Set StatisticsTagConfiguration
   */
  ...
  /* 
   * Set StatisticsReportConfigurations
   */
  ...
  /* 
   * Set StatisticsReportConfigurations2
   */
  ...
  /* SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  40,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, IQM_STATISTICS_REPORT_CONFIGURATIONS_2r, REG_PORT_ANY, 0, ST_RATE_LIMIT_PRDf,  0x0)); */
 
   /* 
   * StatisticsReportConfigurations4 
   */
  ...
  if (SOC_DPP_CONFIG(unit)->arad->init.stat_if.stat_if_enable) {
      soc_sand_rv = arad_nif_id_to_port_get(unit, SOC_TMC_NIF_ID_RXAUI_14, &port_rxaui_14, &core_rxaui_14, &found_rxaui_14, &is_nif);
      SOC_SAND_IF_ERR_EXIT(soc_sand_rv);

      soc_sand_rv = arad_nif_id_to_port_get(unit, SOC_TMC_NIF_ID_RXAUI_15, &port_rxaui_15, &core_rxaui_15, &found_rxaui_15, &is_nif);
      SOC_SAND_IF_ERR_EXIT(soc_sand_rv);

        if (found_rxaui_14 != 0 || found_rxaui_15 != 0) {
...
#ifdef BCM_88650_B0
  /* 
   * In B0 and above, allow the bit 20 of Counter-Pointer-1 to 
   * indicate if to filter the Statistic-Reports 
   */
  ...
  )
{
  ...
  case SOC_TMC_STAT_IF_REPORT_MODE_BILLING_QUEUE_NUMBER:
      if (SOC_IS_ARAD_B0_AND_ABOVE((unit))) {
          /*counter command action 1 will be overwritten by the queue number*/
...
#endif
  /* The missing break is not needed. */
...
#ifdef BCM_88650_B0
  /*statistcs interface enabler per port*/
  if (SOC_IS_ARAD_B0_AND_ABOVE((unit))) {
      int i;
      int rv;
      uint32
      ...
      /* 
       * Enable per port statistic feature
       */
      ...
      PBMP_ITER(stat_port_bm, i) {
          if (SOC_DPP_CONFIG(unit)->arad->init.stat_if.stat_if_info.stat_if_port_enable[i]) {
              /*get the relevant qpair*/

              res = soc_port_sw_db_tm_port_to_base_q_pair_get(unit, i, 0, &base_q_pair);
              SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 81, exit);
              res = soc_port_sw_db_tm_port_to_out_port_priority_get(unit, i, 0, &nof_pairs);
              SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 85, exit);

              for (curr_q_pair = base_q_pair; curr_q_pair - base_q_pair < nof_pairs; curr_q_pair++)
              {
                  rv = READ_EGQ_PORT_CONFIGURATION_TABLE_PCTm(unit, MEM_BLOCK_ANY, curr_q_pair, entry);
                  if(SOC_FAILURE(rv)) {
                      SOC_SAND_SET_ERROR_CODE(ARAD_STAT_IF_INIT_FAILED_ERR, 90, exit);
                  }
                  soc_mem_field32_set(unit, EGQ_PORT_CONFIGURATION_TABLE_PCTm, entry, STATISTICS_ENf, 0x1);
                  rv = WRITE_EGQ_PORT_CONFIGURATION_TABLE_PCTm(unit, MEM_BLOCK_ALL, curr_q_pair, entry);
                  if(SOC_FAILURE(rv)) {
  ...
  case SOC_TMC_STAT_IF_REPORT_QSIZE:
      ...
      /* Divide by 2: the rate must be for a packet every 2 clocks */
      ...
      for(threshold_index = 0; threshold_index < ARAD_STAT_IF_REPORT_THRESHOLD_LINES; threshold_index++) {
          ...
          /* set buffer descriptor threshold */
          ...
          /* set unicast dram buffer threshold */
