    ...
    arad_fast_reg_access_init(int unit)
{
    ...
    for (i = 0; i < ARAD_FAST_REG_TYPE_LAST; i++) {
        ...
        addr = soc_reg_addr_get(unit, g_fast_reg_info_table[unit][i].reg, port, 0, TRUE, &block, &at);
  ...
  )
{
  ...
  for(;;)
  {
    if(SOC_MEM_IS_ARRAY(unit, mem)) {
        res = soc_mem_array_read_range(unit, mem, 0, MEM_BLOCK_ANY, 0, array_type, dma_buf);       
