  ...
  )
{
  ...
  res = arad_port2egress_offset(unit, port_ndx, 0, &egr_interface_id);
  ...
  SOC_REG_ABOVE_64_CLEAR(data);  
  data_32 = 0;
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 1120, exit, READ_SCH_FC_MAP__FCMm(unit,MEM_BLOCK_ANY,egr_interface_id,&data_32));
  if (required_chan_arb_val != ARAD_OFP_RATES_EGQ_CHAN_ARB_NON_CHAN)
  {
  ...
  else
  {
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 1020, exit, READ_SCH_ONE_PORT_NIF_CONFIGURATION__OPNCm(unit,MEM_BLOCK_ANY,new_place,tbl_data));
    soc_SCH_ONE_PORT_NIF_CONFIGURATION__OPNCm_field32_set(unit,tbl_data,PORT_IDf,base_q_pair);
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 1160, exit, WRITE_SCH_ONE_PORT_NIF_CONFIGURATION__OPNCm(unit,MEM_BLOCK_ANY,new_place,tbl_data));    
  }
  soc_SCH_FC_MAP__FCMm_field32_set(unit,&data_32,FC_MAP__FCMf,sch_offset);
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 1120, exit, WRITE_SCH_FC_MAP__FCMm(unit,MEM_BLOCK_ANY,egr_interface_id,&data_32));
  ...
  )
{
  ...
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 1120, exit, READ_SCH_FC_MAP__FCMm(unit,MEM_BLOCK_ANY,egr_interface_id,&data_32));
  sch_offset = soc_SCH_FC_MAP__FCMm_field32_get(unit,&data_32,FC_MAP__FCMf);
  ...
  )
{
  ...
  *is_channelized = FALSE;
  ...
  res = soc_port_sw_db_tm_to_local_port_get(unit, port, core, &local_port);
  ...
  )
{
  ...
  SOCDNX_IF_ERR_EXIT(soc_port_sw_db_tm_to_local_port_get(unit, tm_port, 0, &local_port));
  ...
  switch (interface_type)
  {
      ...
      case SOC_PORT_IF_ERP:
          ...
          SOCDNX_IF_ERR_EXIT(arad_egq_ppct_tbl_get_unsafe(unit, erp_base_q_pair, 0, &egq_ppct_tbl_data));
...
arad_port_post_init(int unit, soc_pbmp_t* ports) 
{
    ...
    if (!SOC_WARM_BOOT(unit) && dpp_arad->init.nif_recovery_enable) 
    {
        ...
        SOC_PBMP_ITER(*ports, port) {
            if (!IS_SFI_PORT(unit, port)) {
                ...
                if (SOC_BLK_XLP == SOC_PORT_TYPE(unit, port) && !SOC_PORT_IS_STAT_INTERFACE(flags)) {
                    SOC_PBMP_PORT_ADD(recover_pbmp, port);
                }
            }
        }
            
        SOCDNX_IF_ERR_EXIT(arad_nif_recover_run_recovery_test(unit, &recover_pbmp, dpp_arad->init.nif_recovery_iter));
    }
