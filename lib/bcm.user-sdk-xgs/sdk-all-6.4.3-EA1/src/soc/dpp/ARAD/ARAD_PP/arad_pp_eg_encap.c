...
#define ARAD_PP_EG_LL_ENCAP_REMARK_PROFILE_INDEX_MAX             (3)
  ...
  )
{
  ...
  fld_val = soc_reg64_field32_get(unit, EPNI_EEDB_OUTLIF_ACCESSr, val64, EEDB_OUTLIF_ACCESSf);

  for (ndx=0; ndx < ARAD_PP_EG_ENCAP_NOF_BANKS; ndx++) {
    ...
    case (ARAD_PP_EG_ENCAP_ACCESS_PHASE_FOUR):
      ...
      res = soc_sand_bitstream_set_any_field(&tmp, ndx*2, 2, &fld_val);
  ...
  soc_reg64_field32_set(unit, EPNI_EEDB_OUTLIF_ACCESSr, &val64, EEDB_OUTLIF_ACCESSf, fld_val);
  ...
  )
{
  ...
  for (ndx=0; ndx < ARAD_PP_EG_ENCAP_NOF_BANKS; ndx++) {
      res = soc_sand_bitstream_get_any_field(&fld_val, ndx*2, 2, &range_info->bank_access_phase[ndx]);
      SOC_SAND_CHECK_FUNC_RESULT(res, 20+ndx, exit);
  }
  ...
  )
{
  ...
  for (eedb_array_index = 0; eedb_array_index < (1 << ARAD_PP_EG_ENCAP_DATA_LOCAL_TO_GLOBAL_CUD_EEDB_ARRAY_INDEX_NOF_BITS) ; eedb_array_index++) 
  {
          ...
          eedb_mem_index++, in_out_cud = 0) 
      {
              ...
              eedb_data_index++) 
          {
              ...
              /*set in_out_cud in the relevent palce*/
   ...
   * Since the incomming cud is never aboth 17 bit, So not action required
   *  
   */
  ...
  )
{
  ...
  ARAD_PP_STRUCT_VERIFY(ARAD_PP_EG_ENCAP_VSI_ENCAP_INFO, vsi_info, 20, exit);
  ...
  )
{
  ...
  res = arad_pp_epni_pp_pct_tbl_get_unsafe(unit, local_port_ndx, 0, &pp_pct_tbl_data);
  ...
  if(1 == set_table) {
      res = arad_pp_epni_pp_pct_tbl_set_unsafe(unit, local_port_ndx, 0, &pp_pct_tbl_data);
  ...
  *nof_entries = 0; 
  ...
  for (depth_i = 0; depth_i <= depth; ++depth_i)
  {
    ...
    if (cur_eep_ndx != ARAD_PP_EG_ENCAP_NEXT_EEP_INVALID) {
      ...
      case ARAD_PP_EG_ENCAP_EEDB_ACCESS_TYPE_MPLS_TUNNEL:
          /* 
                  *In case depth > 0, the first entry is PWE, the second entry could be MPLS tunnel. 
  ...
  )
{
  ...
  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  10,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, IHP_GRE_ETH_TYPE_ETHERNETr, SOC_CORE_ALL, 0, GRE_ETH_TYPE_ETHERNETf,  (uint32)glbl_info->l2_gre_prtcl_type));
  /* Set CUSTOM as Ethernet to eliminate the chance for undefined CUSTOM to hit */
  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  15,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, IHP_GRE_ETH_TYPE_CUSTOMr, SOC_CORE_ALL, 0, GRE_ETH_TYPE_CUSTOMf,  (uint32)glbl_info->l2_gre_prtcl_type)); 
  ...
  )
{
  ...
  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  10,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_read(unit, IHP_GRE_ETH_TYPE_ETHERNETr, SOC_CORE_ALL, 0, GRE_ETH_TYPE_ETHERNETf, &field32));
  ...
  )
{
  ...
  if (info->out_ac_valid) {
      SOC_SAND_ERR_IF_ABOVE_NOF(info->out_ac_lsb, SOC_DPP_DEFS_GET(unit, nof_local_lifs), ARAD_PP_AC_ID_OUT_OF_RANGE_ERR, 30, exit); 
  }
  ...
  )
{
  ...
  SOC_SAND_ERR_IF_ABOVE_NOF(info->out_ac, SOC_DPP_DEFS_GET(unit, nof_local_lifs), ARAD_PP_AC_ID_OUT_OF_RANGE_ERR, 11, exit);
