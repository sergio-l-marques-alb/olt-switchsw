    ...
    )
{
    ...
    *found = FALSE;
    ...
    SOC_SAND_CHECK_NULL_PTR(location.db_entry, 35, exit);
    ...
    SOC_SAND_CHECK_NULL_PTR(ad_entry, 40, exit);
    ...
    res = kbp_entry_get_priority(db_p, location.db_entry, priority);
    if(ARAD_KBP_TO_SOC_RESULT(res) != SOC_SAND_OK) {
    ...
    )
{
    ...
    *success = SOC_SAND_SUCCESS;
    ...
    if (frwrd_table_id == ARAD_KBP_FRWRD_TBL_ID_LSR_IP_SHARED) {
        location.db_entry = db_entry;
        res = arad_sw_db_frwrd_ip_kbp_location_tbl_set(unit, data_indx, &location);
        SOC_SAND_CHECK_FUNC_RESULT(res, 90, exit);
    }
