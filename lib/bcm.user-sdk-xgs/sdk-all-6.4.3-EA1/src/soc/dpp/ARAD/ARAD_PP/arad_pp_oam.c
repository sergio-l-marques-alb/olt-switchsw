...
#define _ARAD_PP_OAM_SUBTYPE_EGRESS_SNOOP 6

#define _ARAD_PP_OAM_MAX_MD_LEVEL                7

#define _ARAD_PP_OAM_RMEP_EMC_SHIFT_BFD 16
  ...
  do {                                                                                                                            \
      ...
      if ((mep_type == SOC_PPD_OAM_MEP_TYPE_ETH_OAM) ||                                                                             \
          ...
          case SOC_PPD_OAM_MEP_TYPE_BFDCC_O_MPLSTP:                                                                                \
              field64 = soc_reg64_field_get(unit, OAMP_BFD_EMC_CONSTr, reg_val64, BFDCC_EMC_CONSTf);                        \
              break;                                                                                                            \
          default:                                                                                                                \
  ...
  Arad_pp_procedure_desc_element_oam[] =
{
  ...
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_OAM_CLASSIFIER_MEP_SET),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_OAM_CLASSIFIER_MEP_SET_UNSAFE),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_OAM_CLASSIFIER_MEP_SET_VERIFY),
  ...
  )
{
    ...
    uint32 i, bfd_gach_cc, bfd_gach_cv, y1731_gach, bfd_pwe_cw;
    ...
    if (is_bfd) {
        ...
        if (SOC_IS_ARADPLUS(unit) && soc_property_get(unit, spn_BFD_ENCAPSULATION_MODE, 0)) {
            ...
            SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  110,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, OAMP_BFD_PWE_CW_0r, REG_PORT_ANY, 0, BFD_PWE_CWf,  bfd_gach_cc));
        }
        else {
            /* Deafult CW for PWE */
            SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  105,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, OAMP_BFD_PWE_CW_0r, REG_PORT_ANY, 0, BFD_PWE_CWf,  bfd_pwe_cw));
        }
    ...
    res = arad_hpu_itmh_fwd_dest_info_build(unit, destination, &destination_for_itmh);
    SOC_SAND_CHECK_FUNC_RESULT(res, 15, exit);
  ...
  )
{
	...
	if ((soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "oam_downmep_pwe_classification",0) == 1) &&
		...
		if (ip_begin_bit > 0) {
			...
			soc_sand_bitstream_set_any_field(&val,  sizeof(uint32) * 8 * ARAD_TCAM_ENTRY_MAX_LEN - ip_begin_bit - 4, 4, entry->value);
			soc_sand_bitstream_set_any_field(&mask, sizeof(uint32) * 8 * ARAD_TCAM_ENTRY_MAX_LEN - ip_begin_bit - 4, 4, entry->mask);

			mask =   0xFFFFFFFF;
			val = 5; /* IPv4.IHL(4) = 5 */
			soc_sand_bitstream_set_any_field(&val,  sizeof(uint32) * 8 * ARAD_TCAM_ENTRY_MAX_LEN - ip_begin_bit - 8, 4, entry->value);
			soc_sand_bitstream_set_any_field(&mask, sizeof(uint32) * 8 * ARAD_TCAM_ENTRY_MAX_LEN - ip_begin_bit - 8, 4, entry->mask);

			mask = 0xFFFFFFFF;
			val = 1; /* IPv4.TTL(8) = 1 */
			soc_sand_bitstream_set_any_field(&val,  sizeof(uint32) * 8 * ARAD_TCAM_ENTRY_MAX_LEN - ip_begin_bit - 72, 8, entry->value);
			soc_sand_bitstream_set_any_field(&mask, sizeof(uint32) * 8 * ARAD_TCAM_ENTRY_MAX_LEN - ip_begin_bit - 72, 8, entry->mask);

			mask = 0xFFFFFFFF;
			val = 17; /* IPv4.protocol(8) = UPD(17) */
			soc_sand_bitstream_set_any_field(&val,  sizeof(uint32) * 8 * ARAD_TCAM_ENTRY_MAX_LEN - ip_begin_bit - 80, 8, entry->value);
			soc_sand_bitstream_set_any_field(&mask, sizeof(uint32) * 8 * ARAD_TCAM_ENTRY_MAX_LEN - ip_begin_bit - 80, 8, entry->mask);

			mask = 0;
			val = 0x7FFFFF; /* IPv4.DIP-msbs(8) = 127.255.255.X */
			soc_sand_bitstream_set_any_field(&val,  sizeof(uint32) * 8 * ARAD_TCAM_ENTRY_MAX_LEN - ip_begin_bit - 152, 24, entry->value);
			soc_sand_bitstream_set_any_field(&mask, sizeof(uint32) * 8 * ARAD_TCAM_ENTRY_MAX_LEN - ip_begin_bit - 152, 24, entry->mask);

			mask = 0xFFFFFFFF;
			val = 3; /* UDP.source-prot-msbs(2) = 3: The source  port MUST be in the range 49152 through 65535 */
			soc_sand_bitstream_set_any_field(&val,  sizeof(uint32) * 8 * ARAD_TCAM_ENTRY_MAX_LEN - ip_begin_bit - 160 - 2, 2, entry->value);
			soc_sand_bitstream_set_any_field(&mask, sizeof(uint32) * 8 * ARAD_TCAM_ENTRY_MAX_LEN - ip_begin_bit - 160 - 2, 2, entry->mask);

			mask = 0xFFFFFFFF;
			val = 0x0ec8; /* UDP.dest-port(16) = 0x0ec8 */
			soc_sand_bitstream_set_any_field(&val,  sizeof(uint32) * 8 * ARAD_TCAM_ENTRY_MAX_LEN - ip_begin_bit - 160 - 32, 16, entry->value);
			soc_sand_bitstream_set_any_field(&mask, sizeof(uint32) * 8 * ARAD_TCAM_ENTRY_MAX_LEN - ip_begin_bit - 160 - 32, 16, entry->mask);
...
{
  SOC_SAND_INIT_ERROR_DEFINITIONS(0);
  SOC_SAND_ERR_IF_ABOVE_MAX(your_discriminator, _ARAD_PP_OAM_BFD_YOUR_DISC_MAX, ARAD_PP_OAM_YOUR_DISC_ABOVE_MAX_ERROR, 10, exit);

    if ( soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "vt_tst1",0) ) {
      SOC_SAND_SET_ERROR_MSG((_BSL_SOCDNX_MSG_STR("BFDoIP can not be used when vt_tst1 application is enabled.")));
    }

exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_pp_oam_tcam_bfd_lsp_ipv4_entry_add_verify()", 0, 0);
}
  ...
  )
{
  ...
  oam_tcam_entry_key.mdl = 0;
  oam_tcam_entry_key.opcode = 0;
  oam_tcam_entry_key.your_disc = your_discriminator;
  ...
  oam_tcam_entry_action.opcode = 0;
  oam_tcam_entry_action.mdl = 0;
  oam_tcam_entry_action.oam_lif_tcam_result = in_lif;/*Lif solution*/
  oam_tcam_entry_action.oam_lif_tcam_result_valid = 1;
  ...
  res = SOC_DPP_WB_ENGINE_GET_VAR(unit, SOC_DPP_WB_ENGINE_VAR_ARAD_PP_OAM_TCAM_LAST_ENTRY_ID, tcam_entry);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 11, exit);

  ++*tcam_entry;
  tcam_entry_id = *tcam_entry;
  ...
  res = SOC_DPP_WB_ENGINE_SET_VAR(unit, SOC_DPP_WB_ENGINE_VAR_ARAD_PP_OAM_TCAM_LAST_ENTRY_ID, tcam_entry);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 12, exit);  
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_pp_oam_tcam_bfd_entry_delete_verify()", 0, 0);
}
  ...
  )
{
  ...
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 30, exit, READ_IHP_OAM_IDENTIFICATION_ENABLEr(unit, &reg));
  if (is_bfd) {
  ...
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 40, exit, WRITE_IHP_OAM_IDENTIFICATION_ENABLEr(unit, reg));
  ...
  if (is_bfd) {
      /* BFD registers */
      SOC_SAND_SOC_IF_ERROR_RETURN(res, 160, exit, READ_IHP_OAM_BFD_YOUR_DISCR_TO_LIF_CFGr(unit, &reg_64));
      ...
      SOC_SAND_SOC_IF_ERROR_RETURN(res, 170, exit, WRITE_IHP_OAM_BFD_YOUR_DISCR_TO_LIF_CFGr(unit, reg_64));
  ...
  if (!is_second_init) {
	  ...
	  if (SOC_IS_ARADPLUS_A0(unit)) { /* Arad plus and only arad plus*/
		  SOC_SAND_SOC_IF_ERROR_RETURN(res, 30, exit, READ_IHP_FLP_GENERAL_CFGr(unit, &reg_64));
		  ...
		  SOC_SAND_SOC_IF_ERROR_RETURN(res, 30, exit, WRITE_IHP_FLP_GENERAL_CFGr(unit, reg_64));
	  }
	  ...
	  SOC_SAND_SOC_IF_ERROR_RETURN(res, 60, exit, WRITE_IHP_OAM_COUNTER_IN_RANGE_CFGr(unit, reg));
	  ...
	  SOC_SAND_SOC_IF_ERROR_RETURN(res, 70, exit, WRITE_IHP_OAM_COUNTER_NOT_IN_RANGE_CFGr(unit, reg));
	  ...
	  SOC_SAND_SOC_IF_ERROR_RETURN(res, 80, exit, WRITE_IHP_OAM_COUNTER_INCREMENT_BITMAPr(unit, reg));
  ...
  )
{
  ...
  if (ref_count->ref_count[_ARAD_PP_OAM_MY_MAC_LSB_REF_COUNT_INDEX(table_index, dst_mac_address)]==0) {
      ...
      SOC_SAND_SOC_IF_ERROR_RETURN(res, 40, exit, READ_EPNI_MY_CFM_MAC_TABLEm(unit, MEM_BLOCK_ANY, table_index, &epni_entry));
      ...
      SOC_SAND_SOC_IF_ERROR_RETURN(res, 60, exit, WRITE_EPNI_MY_CFM_MAC_TABLEm(unit, MEM_BLOCK_ANY, table_index, epni_entry));
  }
  ...
  )
{
  ...
  /* Passive : forward */
  oam1_key.ingress = 0; /* Do not support default upmep */
  res = SOC_DPP_WB_ENGINE_GET_VAR(unit, SOC_DPP_WB_ENGINE_VAR_ARAD_PP_OAM_TRAP_CODE_FRWRD, &oam_trap_code);
...
*********************************************************************/
uint32
  arad_pp_oam_classifier_oam1_2_entries_insert_according_to_profile_mep_match_unsafe(
    SOC_SAND_IN  int                                      unit,
    SOC_SAND_IN  uint32                                      opcode,
    SOC_SAND_IN  SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY            *classifier_mep_entry,
    SOC_SAND_IN  SOC_PPD_OAM_LIF_PROFILE_DATA                *profile_data,
    SOC_SAND_IN  SOC_PPD_OAM_CPU_TRAP_CODE_TO_MIRROR_PROFILE_MAP  cpu_trap_code_to_mirror_profile_map,
    SOC_SAND_INOUT  SOC_PPD_OAM_CLASSIFIER_OAM1_ENTRY_KEY       *oam1_key,
  ...
  )
{
    ...
    *meter_disable_lcl = profile_data->mep_profile_data.meter_disable;
    ...
    oam_trap_code = oam1_key->my_cfm_mac ? profile_data->mep_profile_data.opcode_to_trap_code_unicast_map[internal_opcode] :
  ...
  )
{
  ...
  SOC_PPD_OAM_CLASSIFIER_OAM_ENTRY_PAYLOAD   oam2_payload;
  uint32 oam_trap_code;
  uint8 oam_mirror_profile[4];  /* Stored as 8 uint8's on uit32's*/
  ...
  SOC_PPD_OAM_CLASSIFIER_OAM_ENTRY_PAYLOAD_clear(&oam2_payload);
                  ...
                  case SOC_PPD_OAM_MP_TYPE_MEP_OR_ACTIVE_MATCH_PLUS: /* Mep active match */
                      if (SOC_PPD_OAM_IS_CLASSIFIER_ARAD_MODE(unit)) { /* Arad mode*/
                          ...
                          for (oam1_key.mip_type = SOC_PPD_OAM_MP_TYPE_ABOVE; oam1_key.mip_type <= SOC_PPD_OAM_MP_TYPE_BELOW; oam1_key.mip_type++) {
                              res = arad_pp_oam_classifier_oam1_2_entries_insert_according_to_profile_mep_match_unsafe(unit,
                                                                                                                       opcode,
                      ...
                      if (SOC_PPD_OAM_IS_CLASSIFIER_ARAD_MODE(unit) || (!profile_data->mp_type_passive_active_mix) || SOC_PPD_OAM_IS_CLASSIFIER_JERICHO_MODE(unit)) { /* MIP */
                          ...
                          *meter_disable_lcl = profile_data->mip_profile_data.meter_disable;
                          ...
                          if (egress_snooping_advanced_mode && !oam1_key.ingress && oam_payload.snoop_strength) {
                              ...
                              oam_payload.sub_type = _ARAD_PP_OAM_SUBTYPE_EGRESS_SNOOP;
                          }
                          res = arad_pp_oam_classifier_oam1_entry_set_on_buffer(unit, &oam1_key, &oam_payload, &oama_buffer);
                          SOC_SAND_CHECK_FUNC_RESULT(res, 52, exit);
  ...
  if (is_accelerated) {
      ...
      if (oam2_key.is_bfd) {
          ...
          SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 15, exit);
          oam2_key.opcode = 0;
          oam2_payload.forwarding_strength = _ARAD_PP_OAM_TRAP_STRENGTH;
          oam2_payload.up_map = is_upmep;
          oam2_payload.meter_disable = 0;

          oam_trap_code = profile_data->mep_profile_data.opcode_to_trap_code_multicast_map[internal_opcode];
          _ARAD_PP_OAM_TRAP_CODE_TO_INTERNAL(oam_trap_code, oam2_payload.cpu_trap_code);

           oam2_payload.sub_type = _ARAD_PP_OAM_SUBTYPE_CCM; 
          ...
          for (is_my_cfm_mac = 0; is_my_cfm_mac <=1; is_my_cfm_mac++) {
              ...
              for (opcode = 1; opcode < SOC_PPD_OAM_ETHERNET_PDU_OPCODE_COUNT; opcode++) {
                  ...
                  oam_trap_code = is_my_cfm_mac ? profile_data_acc->mep_profile_data.opcode_to_trap_code_unicast_map[internal_opcode] :
                                                  profile_data_acc->mep_profile_data.opcode_to_trap_code_multicast_map[internal_opcode];
                  res = soc_ppd_trap_snoop_profile_info_get(unit, oam_trap_code, &snoop_profile_info);
                  SOC_SAND_CHECK_FUNC_RESULT(res, 52, exit);

                  if (IS_INGRESS_WHEN_ACTIVE(is_upmep)) { /*ingress*/
                      if (snoop_profile_info.snoop_cmnd > 0) {
                          oam2_payload.snoop_strength = _ARAD_PP_OAM_SNOOP_STRENGTH;
                          oam2_payload.sub_type = _ARAD_PP_OAM_SUBTYPE_DEFAULT;
                      }
                      oam2_payload.forwarding_strength = _ARAD_PP_OAM_TRAP_STRENGTH;
                      oam2_payload.up_map = is_upmep;
                      oam2_payload.meter_disable = (SHR_BITGET(&profile_data_acc->mep_profile_data.meter_disable, internal_opcode)!= 0);
                      _ARAD_PP_OAM_TRAP_CODE_TO_INTERNAL(oam_trap_code, oam2_payload.cpu_trap_code);
                  }
                  else { /*egress - only Ethernet */
                      if ((oam_trap_code == SOC_PPC_TRAP_CODE_USER_DEFINED_DFLT_TRAP) || (snoop_profile_info.strength > 0)) {
                          /* In FWD and snoop dont do fwd disable */
                          oam2_payload.forward_disable = 0;
                      }
                      else {
                          oam2_payload.forward_disable = 1;
                      }
                      oam2_payload.mirror_profile = cpu_trap_code_to_mirror_profile_map[oam_trap_code];
                  }

                  switch (opcode) {
                  case SOC_PPD_OAM_ETHERNET_PDU_OPCODE_CCM:
                      oam2_payload.sub_type = ccm_subtype; 
                      break;
                  case SOC_PPD_OAM_ETHERNET_PDU_OPCODE_LMR:
                  case SOC_PPD_OAM_ETHERNET_PDU_OPCODE_LMM:
                  case SOC_PPD_OAM_ETHERNET_PDU_OPCODE_SLR:
                  case SOC_PPD_OAM_ETHERNET_PDU_OPCODE_SLM:
                      oam2_payload.sub_type = _ARAD_PP_OAM_SUBTYPE_LM; 
                      break;
                  case SOC_PPD_OAM_ETHERNET_PDU_OPCODE_1DM:
                  case SOC_PPD_OAM_ETHERNET_PDU_OPCODE_DMR:
                  case SOC_PPD_OAM_ETHERNET_PDU_OPCODE_DMM:
                      oam2_payload.sub_type = profile_data_acc->is_1588 ? _ARAD_PP_OAM_SUBTYPE_DM_1588 : _ARAD_PP_OAM_SUBTYPE_DM_NTP;
                      break;
                  case SOC_PPD_OAM_ETHERNET_PDU_OPCODE_LBM:
                      oam2_payload.sub_type = _ARAD_PP_OAM_SUBTYPE_ETH_LOOPBACK;
                      break;
                  default:
                      oam2_payload.sub_type = _ARAD_PP_OAM_SUBTYPE_DEFAULT_OAM_MESSAGE; 
                  }

                  /* Get profile counter */
                  _ARAD_PP_OAM_SET_COUNTER_DISABLE(opcode, internal_opcode, (&oam2_payload), profile_data_acc->mep_profile_data, profile_data->is_piggybacked);
          ...
          for (opcode = 1; opcode <SOC_PPD_OAM_ETHERNET_PDU_OPCODE_COUNT; opcode++) {
              ...
              for (mep_type = 0; mep_type < SOC_PPD_OAM_MP_TYPE_COUNT; mep_type++) {
                  for (mip_type = 0; mip_type < SOC_PPD_OAM_MP_TYPE_COUNT; mip_type++) {
                      ...
                      if (is_accelerated) {
                          _ARAD_PP_OAM_SET_COUNTER_DISABLE(opcode, internal_opcode, (&oam_payload), profile_data_acc->mep_profile_data, profile_data->is_piggybacked);
                      } else {
                          _ARAD_PP_OAM_SET_COUNTER_DISABLE(opcode, internal_opcode, (&oam_payload), profile_data->mep_profile_data, profile_data->is_piggybacked);
                      }
                      ...
                      if ((oam1_key.mep_type==SOC_PPD_OAM_MP_TYPE_MATCH) && is_accelerated) {
                          oam2_key.mp_profile = classifier_mep_entry->acc_profile;
  ...
  )
{
  ...
  SOC_PPD_OAM_CLASSIFIER_OEM1_ENTRY_PAYLOAD   oem1_prev_payload, oem1_payload_ingress, oem1_payload_egress;
  ...
  if (SOC_PPD_OAM_IS_CLASSIFIER_ADVANCED_MODE(unit)){
      /* If MP_Type is changing -> is was natural before -> only endpoints on one direction.
      ...
      if (passive_active_enable) {
          ...
          oem1_payload_ingress.mp_profile = classifier_mep_entry->non_acc_profile;
          oem1_payload_ingress.mep_bitmap = oem1_prev_payload.mep_bitmap;
		  if (update_mp_type) {
              /* in this case only the mp_profile is to be updated.*/
              SET_MIP_BITMEP_FROM_MEP_BITMEP(oem1_payload_egress.mip_bitmap,oem1_prev_payload.mep_bitmap );
		  } else {
              oem1_payload_ingress.mip_bitmap = oem1_prev_payload.mip_bitmap;
          }
          oem1_payload_ingress.counter_ndx = oem1_prev_payload.counter_ndx;
          ...
          oem1_payload_egress.mp_profile = classifier_mep_entry->non_acc_profile;
          oem1_payload_egress.mep_bitmap = oem1_prev_payload.mep_bitmap;
		  if (update_mp_type) {
              /* in this case only the mp_profile is to be updated.*/
              SET_MIP_BITMEP_FROM_MEP_BITMEP(oem1_payload_egress.mip_bitmap,oem1_prev_payload.mep_bitmap );
		  } else {
              oem1_payload_egress.mip_bitmap = oem1_prev_payload.mip_bitmap;
          }
          oem1_payload_egress.counter_ndx = oem1_prev_payload.counter_ndx;


          /* Insert ingress and egress entries */
          oem1_key.ingress = 1;
          res = arad_pp_oam_classifier_oem1_entry_set_unsafe(unit, &oem1_key, &oem1_payload_ingress);
          SOC_SAND_CHECK_FUNC_RESULT(res, 70, exit);
          oem1_key.ingress = 0;
          res = arad_pp_oam_classifier_oem1_entry_set_unsafe(unit, &oem1_key, &oem1_payload_egress);
          ...
          oem1_prev_payload.mp_profile = classifier_mep_entry->non_acc_profile;
          oem1_prev_payload.mip_bitmap = update_mp_type? 0 : oem1_prev_payload.mip_bitmap;
          res = arad_pp_oam_classifier_oem1_entry_set_unsafe(unit, &oem1_key, &oem1_prev_payload);
  ...
  )
{
  ...
  if (!SOC_PPD_OAM_IS_MEP_TYPE_BFD(classifier_mep_entry->mep_type) ||
      ...
      if (!SOC_PPD_OAM_IS_MEP_TYPE_BFD(classifier_mep_entry->mep_type)) {
          ...
          if (SOC_PPD_OAM_IS_CLASSIFIER_JERICHO_MODE(unit)) {
              ...
              res = arad_pp_oam_mep_passive_active_enable_get_unsafe(unit, classifier_mep_entry->non_acc_profile, &passive_active_enable);
              SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
              if ((!(is_mep)) || (passive_active_enable) ) {
  ...
  if (((classifier_mep_entry->flags & SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_FLAG_ACCELERATED) != 0) && !update) {
  ...
  )
{
  ...
  eep_shift = SOC_IS_ARADPLUS(unit) ? 1 : 0; /* Outlif is alyays even. Using it in ARAD+ to save bits */
  ...
  )
{
  ...
  eep_shift = SOC_IS_ARADPLUS(unit) ? 1 : 0;
  if ((mep_db_entry->mep_type == SOC_PPD_OAM_MEP_TYPE_ETH_OAM) ||
      (mep_db_entry->mep_type == SOC_PPD_OAM_MEP_TYPE_Y1731_O_MPLSTP) ||
      (mep_db_entry->mep_type == SOC_PPD_OAM_MEP_TYPE_Y1731_O_PWE)) { /* Only relevant for Ethernet and Y1731 */
        ...
        *profile = oem1_payload.mp_profile;
        if (SOC_PPD_OAM_IS_CLASSIFIER_JERICHO_MODE(unit)) {
            ...
            *found_mep = ((oem1_payload.mep_bitmap >> md_level) & 1);
            if (SOC_PPD_OAM_IS_CLASSIFIER_ADVANCED_MODE(unit) && (!(*found_mep))) {
                ...
                *found_mep |= ((oem1_payload.mip_bitmap >> md_level) & 1);
            }
        }
    }

    /* ARAD+ If no mips configured or all mes configured on same direction -> mp type can be changed */
    if (SOC_PPD_OAM_IS_CLASSIFIER_ADVANCED_MODE(unit)) {
        if (oem1_payload.mip_bitmap == 0) {
            *is_mp_type_flexible = TRUE;
        }
        else if (oem1_payload.mep_bitmap == 0) {
            *is_mp_type_flexible = TRUE;
        }
        else {
  ...
  )
{
  ...
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 10, exit, READ_IHP_OAM_COUNTER_RANGEr(unit, &reg));
  ...
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 20, exit, WRITE_IHP_OAM_COUNTER_RANGEr(unit, reg));
  ...
  )
{
  ...
  /* your-discr RX*/
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 10, exit, READ_IHP_OAM_BFD_YOUR_DISCR_TO_LIF_CFGr(unit, &reg));
  ...
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 20, exit, WRITE_IHP_OAM_BFD_YOUR_DISCR_TO_LIF_CFGr(unit, reg));
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 20, exit, WRITE_IHB_OAM_BFD_YOUR_DISCR_TO_LIF_CFGr(unit, reg));
  ...
  )
{
  ...
  rv = arad_pp_epni_pp_pct_tbl_get_unsafe(unit, local_port_ndx, 0, &tbl_data);
  ...
  )
{
  ...
  dest_above_64[0] = dest_system_port;
  ...
  )
{
    ...
    if (!is_lm) { /* Reset the MEP_PE_PROFILE*/
        ...
        soc_OAMP_MEP_DBm_field32_set(unit, entry, MEP_PE_PROFILEf, 0) ;
  ...
  )
{
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN(res,10,exit,READ_IHP_OAM_COUNTER_DISABLE_MAPr(unit, &reg));
    map_key = profile + (packet_is_oam << 4);
    field_val = (uint32)counter_enable;
    SHR_BITCOPY_RANGE(&reg, map_key, &field_val, 0, 1);
    SOC_SAND_SOC_IF_ERROR_RETURN(res,20,exit,WRITE_IHP_OAM_COUNTER_DISABLE_MAPr(unit, reg));
...
{ 
    ...
    char *stikcy_port_msg[] = {"error packet", "trap miss", "type miss", "emc miss", "maid miss", "mdl miss", "ccm interval miss", "my discreminator miss","error src IP miss" , "error your discreminator miss"  ,
