 ...
 * 4. Set default SLB hash function to 0x8423. 
 * 5. Set the PROG index DB ID to the SLB DB ID. 
 */
STATIC uint32 _arad_pp_slb_init_general_configuration(SOC_SAND_IN int unit, SOC_SAND_IN uint32 slb_prefix)
{
...
STATIC uint32 _arad_pp_slb_init_aging(SOC_SAND_IN int unit)
{
  ...
  res = arad_pp_sw_db_slb_aging_time_set(unit, cfg_item.age_time_in_seconds);
  ...
  )
{
  ...
  if (enable) {
    ...
    res = _arad_pp_slb_init_general_configuration(unit, slb_prefix);
    SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
