  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS_NO_DEVID(ARAD_PP_TRAP_MGMT_TRAP_CODE_TO_INTERNAL);
  ...
  )
{
  ...
  res = arad_pp_sw_db_fwd_header_data_system_headers_mode_get(unit, &system_headers_mode);
	if(system_headers_mode != 2) {
		  SOC_SAND_SET_ERROR_CODE(SOC_SAND_GEN_ERR, 10, exit);
	}

  res = arad_pp_sw_db_fwd_header_data_ftmh_lb_key_ext_en_get(unit, &(packet_parse_info->lb_ext_en));
  SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
  res = arad_pp_sw_db_fwd_header_data_ftmh_stacking_ext_enable_get(unit, &(packet_parse_info->stacking_enabled));
  SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);

  /* LSB to MSB according to CMIC*/
  soc_cm_get_endian(unit, &big_pio, &big_packet, &big_other);
  if (big_pio) {
      packet_parse_info->recv_mode = SOC_TMC_PKT_PACKET_RECV_MODE_LSB_TO_MSB;
  } else {
      packet_parse_info->recv_mode = SOC_TMC_PKT_PACKET_RECV_MODE_MSB_TO_LSB;
  }

  /* Get the user-header size */
#ifndef BCM_ARAD_PARSE_PACKET_IN_INTERRUPT_CONTEXT
  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  40,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_read(unit, EGQ_PP_CONFIGr, SOC_CORE_ALL, 0, NETWORK_OFFSETf, &packet_parse_info->user_header_size));
#endif
  ...
  )
{
    ...
    SOC_SAND_INIT_ERROR_DEFINITIONS_NO_DEVID(ARAD_PP_TRAP_PACKET_PARSE_OAM);
  ...
  )
{
  ...
  if(trap_type_bitmap_ndx < SOC_PPC_TRAP_EG_TYPE_INVALID_OUT_PORT) {
