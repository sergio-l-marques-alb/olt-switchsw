...
#define ARAD_PP_BW_PROFILE_ADD_VERIFY_IR_MAX (ARAD_IF_MAX_RATE_KBPS)
 ...
 * This map is used to allow more flexibility to the user when 
 * deciding on out color and charged bucket in case of parallel 
 * metetring. 
 */
STATIC uint32 
  ...
  )
{
  ...
  if (SOC_IS_ARADPLUS(unit)) {
      ...
      if ((range_mode == 32) && (sharing_mode == ARAD_PP_MTR_TWO_METER_SHARING_MODE_PARALLEL)) {
        /* Enable the PCD map only for parallel mode. */
...
#ifdef BCM_88660_A0
  if (SOC_IS_ARADPLUS(unit)) {
      res = arad_pp_metering_pcd_init(unit);
  ...
  )
{
  ...
  ARAD_PP_STRUCT_VERIFY(ARAD_PP_MTR_BW_PROFILE_INFO, bw_profile_info, 30, exit);
  ...
  )
{
  ...
  soc_mem_field_set(unit, mem, mem_val, fld, &fld_val);
 
  res = soc_mem_array_write(unit, mem, array_index, MEM_BLOCK_ANY, index, mem_val);
  SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
  ...
  )
{
  ...
  SOC_SAND_ERR_IF_ABOVE_MAX(policer_info->cir, ARAD_PP_BW_PROFILE_ADD_VERIFY_IR_MAX, ARAD_PP_METERING_CIR_OUT_OF_RANGE_ERR, 30, exit);
  ...
  )
{
  ...
  SOC_SAND_ERR_IF_ABOVE_MAX(info->cir, ARAD_PP_BW_PROFILE_ADD_VERIFY_IR_MAX, ARAD_PP_METERING_CIR_OUT_OF_RANGE_ERR, 10, exit);
  SOC_SAND_ERR_IF_BELOW_MIN(info->cir, ARAD_PP_BW_PROFILE_ADD_VERIFY_IR_MIN, ARAD_PP_METERING_CIR_OUT_OF_RANGE_ERR, 20, exit);

  SOC_SAND_ERR_IF_ABOVE_MAX(info->eir, ARAD_PP_BW_PROFILE_ADD_VERIFY_IR_MAX, ARAD_PP_METERING_EIR_OUT_OF_RANGE_ERR, 30, exit);
  ...
  if (info->is_sharing_enabled && (info->max_cir != ARAD_PP_BW_PROFILE_IR_MAX_UNLIMITED)) {
      SOC_SAND_ERR_IF_ABOVE_MAX(info->max_cir, ARAD_PP_BW_PROFILE_ADD_VERIFY_IR_MAX, ARAD_PP_METERING_CIR_OUT_OF_RANGE_ERR, 15, exit);
  ...
  if (info->max_eir != ARAD_PP_BW_PROFILE_IR_MAX_UNLIMITED) {
      SOC_SAND_ERR_IF_ABOVE_MAX(info->max_eir, ARAD_PP_BW_PROFILE_ADD_VERIFY_IR_MAX, ARAD_PP_METERING_EIR_OUT_OF_RANGE_ERR, 35, exit);
