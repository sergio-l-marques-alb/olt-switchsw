  ...
  Arad_pp_lb_hash_map_table[] =
{
  ...
  { ARAD_PP_LB_PFC_OFFSET(ARAD_PARSER_PKT_HDR_STK_TYPE_ETH_IPV4_ETH,1), ARAD_PP_LB_VECTOR_INDEX_NONE, ARAD_PP_LB_VECTOR_INDEX_NONE, ARAD_PP_LB_VECTOR_INDEX_ETHERNET, ARAD_PP_LB_VECTOR_INDEX_IPV4,         ARAD_PP_LB_VECTOR_INDEX_ETHERNET},
  ...
  )
{
  ...
  /* 1. ARAD_PMF_PGM_SEL_PARSER_PROFILE_FCOE_VFT : (0x20 - bit 5) - for FC plc 4 => VFT exist
     2. ARAD_PMF_PGM_SEL_PARSER_PROFILE_PP_L4 : (0x4000 - bit 14)
   */
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 80, exit, WRITE_IHB_LB_KEY_PARSER_LEAF_CONTEXT_PROFILEr(unit, REG_PORT_ANY, 0x4020));
  ...
  )
{
  ...
  */
  ...
  /* store key in SW DB */
  res = arad_pp_sw_db_lag_lb_key_mask_set(unit, masks);
  SOC_SAND_CHECK_FUNC_RESULT(res, 120, exit);
  res = arad_pp_sw_db_lag_lb_key_is_symtrc_set(unit, is_symmetric);
  ...
  )
{
  ...
  res = arad_pp_sw_db_lag_lb_key_mask_get(unit, &(mask_info->mask));
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);

  res = arad_pp_sw_db_lag_lb_key_is_symtrc_get(unit, &(mask_info->is_symmetric_key));
  SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
