...
#define ARAD_PP_FRWRD_FEC_PROTECT_PTR_VAL_NO_PROTECT   (0x3FFF)
/* protection pointer value for facility protection */
#define ARAD_PP_FRWRD_FEC_PROTECT_PTR_VAL_FAC_PROTECT   (0x3FFE)
/* reserved EEP Value */
  ...
  )
{
  ...
  switch(fec_entry->type)
  {
    ...
    break;
    case ARAD_PP_FEC_TYPE_TUNNELING_EEI:
  ...
  )
{
  ...
  res = arad_pp_sw_db_fec_entry_type_set(
          unit,
          fec_ndx,
          fec_entry->type
        );
  ...
  )
{
  ...
  res = arad_pp_sw_db_fec_entry_type_get(
          unit,
          fec_ndx,
          &fec_entry->type
        );
  ...
  )
{
  ...
  if (ARAD_PP_FRWRD_FEC_EVEN_FEC(fec_ndx) && protect_type != ARAD_PP_FRWRD_FEC_PROTECT_TYPE_NONE)
  {
    ...
    {
      ARAD_PP_STRUCT_VERIFY(ARAD_PP_FRWRD_FEC_PROTECT_INFO, protect_info, 50, exit);
    }
  }
  ...
  )
{
  ...
  tmp = SOC_SAND_NUM2BOOL(glbl_hash_info->use_port_id);
  ...
  )
{
  ...
  {
    SOC_SAND_ERR_IF_ABOVE_NOF(info->out_ac_id, SOC_DPP_DEFS_GET(unit, nof_local_lifs), ARAD_PP_AC_ID_OUT_OF_RANGE_ERR, 10, exit);
  }
  ...
  )
{
  ...
  break;
    case SOC_SAND_PP_DEST_SINGLE_PORT:
      SOC_SAND_ERR_IF_ABOVE_MAX(info->dest.dest_val,SOC_TMC_MAX_SYSTEM_PHYSICAL_PORT_ID,ARAD_PP_FRWRD_FEC_DEST_VAL_OUT_OF_RANGE_ERR,16,exit);
    break;
