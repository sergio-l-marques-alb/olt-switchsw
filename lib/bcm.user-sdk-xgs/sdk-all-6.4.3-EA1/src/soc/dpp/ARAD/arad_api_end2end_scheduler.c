  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_SCH_DEVICE_RATE_ENTRY_SET);
  /* SOC_SAND_CHECK_DRIVER_AND_DEVICE; */

  SOC_SAND_CHECK_NULL_INPUT(exact_rate);

  res = arad_sch_device_rate_entry_verify(
    unit,
    rci_level_ndx,
  ...
  res = arad_sch_device_rate_entry_set_unsafe(
    unit,
    rci_level_ndx,
    nof_active_links_ndx,
    rate,
...
exit_semaphore:
  /* SOC_SAND_GIVE_DEVICE_SEMAPHORE; */
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_sch_device_rate_entry_set()",0,0);
}
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_SCH_DEVICE_RATE_ENTRY_GET);
  /* SOC_SAND_CHECK_DRIVER_AND_DEVICE; */
  ...
  res = arad_sch_device_rate_entry_get_unsafe(
    unit,
    rci_level_ndx,
    nof_active_links_ndx,
    rate
...
exit_semaphore:
  /* SOC_SAND_GIVE_DEVICE_SEMAPHORE; */
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_sch_device_rate_entry_get()",0,0);
}
  ...
  )
{
  ...
  res = arad_sch_port_tc2se_id_get_unsafe(
          unit,
          port_id,
          tc,
          se_id
...
exit_semaphore:
  ...
  ARAD_DO_NOTHING_AND_EXIT;
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_sch_port_tc2se_id()", port_id, tc);
}
