...
{
/*Note: Note: these configurations are based on soc properties parsing done in counters.c; 
  Should move all the soc properties parsing to arad_drv.c, and then these Macros are redundant.*/
...
#define IS_STR_MSB1(propval, string) ((sal_strcmp(propval, (string "_MSB_1")) == 0))
    ...
    soc_port_t 
        port;
    uint32      
        field_value[2] = {0xffffffff ,0xffffffff};
    uint32 
        res = SOC_SAND_OK;
    SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_PMF_LOW_LEVEL_PGM_COUNTER_INIT_UNSAFE);

    for (port = 0; port < SOC_DPP_DEFS_GET(unit, nof_counter_processors); port++)
    {
        ...
        /*If we with to count VSO then  - i.e - field value should be 0x1*/
        if ((IS_STR_LSB0(propval, "INGRESS_VSI")) || 
            (IS_STR_MSB0(propval, "INGRESS_VSI"))) {
            if (field_value[0] == 0x3) {
                LOG_ERROR(BSL_LS_SOC_FP,(BSL_META_U(unit,"Unit %d Conflict in counter engine source PMF configuration.\n\r"), unit));
                SOC_SAND_SET_ERROR_CODE(ARAD_PMF_LOW_LEVEL_PGM_COUNTER_PARSE, 10, exit);
            }
            field_value[0] = 0x1;
        } else if ((IS_STR_LSB1(propval, "INGRESS_VSI")) || 
                   (IS_STR_MSB1(propval, "INGRESS_VSI"))) {
            if (field_value[1] == 0x3) {
                LOG_ERROR(BSL_LS_SOC_FP,(BSL_META_U(unit,"Unit %d Conflict in counter engine source PMF configuration.\n\r"), unit));
                SOC_SAND_SET_ERROR_CODE(ARAD_PMF_LOW_LEVEL_PGM_COUNTER_PARSE, 20, exit);
            }
            field_value[1] = 0x1;
        } else if ((IS_STR_LSB0(propval, "INGRESS_FIELD")) || 
                   (IS_STR_MSB0(propval, "INGRESS_FIELD"))) {
            if (field_value[0] == 0x1) {
                LOG_ERROR(BSL_LS_SOC_FP,(BSL_META_U(unit,"Unit %d Conflict in counter engine source PMF configuration.\n\r"), unit));
                SOC_SAND_SET_ERROR_CODE(ARAD_PMF_LOW_LEVEL_PGM_COUNTER_PARSE, 30, exit);
            }
            field_value[0] = 0x3;
        }  else if ((IS_STR_LSB1(propval, "INGRESS_FIELD")) || 
                    (IS_STR_MSB1(propval, "INGRESS_FIELD"))) {
            if (field_value[1] == 0x1) {
                LOG_ERROR(BSL_LS_SOC_FP,(BSL_META_U(unit,"Unit %d Conflict in counter engine source PMF configuration.\n\r"), unit));
                SOC_SAND_SET_ERROR_CODE(ARAD_PMF_LOW_LEVEL_PGM_COUNTER_PARSE, 40, exit);
            }
            field_value[1] = 0x3;
        }
    }
    /*the default mode for counting ingress PP is counting PMF - i.e - field value should be 0x3*/
    if (field_value[0] == 0xffffffff) {
        field_value[0] = 0x3;
    } 
    if (field_value[1] == 0xffffffff) {
        field_value[1] = 0x3;
    } 

    SOC_REG_ABOVE_64_CLEAR(tbl_data_above_64);
    soc_IHB_PMF_PROGRAM_COUNTERSm_field32_set(unit, tbl_data_above_64, COUNTER_BASE_Af, 0x0);
    soc_IHB_PMF_PROGRAM_COUNTERSm_field32_set(unit, tbl_data_above_64, COUNTER_BASE_Bf, 0x0);
    soc_IHB_PMF_PROGRAM_COUNTERSm_field32_set(unit, tbl_data_above_64, COUNTER_MODE_Af, field_value[0]);
    soc_IHB_PMF_PROGRAM_COUNTERSm_field32_set(unit, tbl_data_above_64, COUNTER_MODE_Bf, field_value[1]);
    res = arad_fill_table_with_entry(unit, IHB_PMF_PROGRAM_COUNTERSm, MEM_BLOCK_ANY, tbl_data_above_64);
  ...
  )
{
  ...
  res = arad_pmf_low_level_pgm_counter_init_unsafe(unit);
  ...
  /* In Normal VLAN edit mode, set different FHEI IVE size per command ID */
      ...
      for(i=1; i<=13; i++) { /* for every vlan edit command */
        SHR_BITCOPY_RANGE(fhei_ive_size, i*2, &val, 0, 2);
      }
      val = 2;
  ...
  soc_IHB_HEADER_PROFILEm_field32_set(unit, &header_profile, ALWAYS_ADD_PPH_LEARN_EXTf, always_add_pph_learn_ext);
  soc_IHB_HEADER_PROFILEm_field32_set(unit, &header_profile, NEVER_ADD_PPH_LEARN_EXTf, 0);
  soc_IHB_HEADER_PROFILEm_field32_set(unit, &header_profile, ENABLE_STACKING_UCf, 0); 
  soc_IHB_HEADER_PROFILEm_field32_set(unit, &header_profile, ENABLE_STACKING_MCf, 0);
  soc_IHB_HEADER_PROFILEm_field32_set(unit, &header_profile, USER_HEADER_1_SIZEf, user_header_size[0]);
  soc_IHB_HEADER_PROFILEm_field32_set(unit, &header_profile, USER_HEADER_2_SIZEf, user_header_size[1]);
  ...
  /* header_profile_tbl_data.st_vsq_ptr_tc_mode        = $random;*/
  /*header_profile_tbl_data.enable_stacking_uc      = (device_cfg_stacking_ext_present ? 1 : 0);*/
  /*header_profile_tbl_data.user_header_1_size       = user_header1_size;*/
  /*header_profile_tbl_data.user_header_2_size       = user_header2_size;*/
  /* header_profile_tbl_data.ftmh_reserved_value     = $random;*/
   ...
   * Enable Stacking
   */
 if (SOC_DPP_CONFIG(unit)->arad->init.ports.is_stacking_system == 0x1) {
