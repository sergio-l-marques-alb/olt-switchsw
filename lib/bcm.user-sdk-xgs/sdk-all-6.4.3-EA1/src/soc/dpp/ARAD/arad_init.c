...
/*
 *  Core & System reference frequency
 */
#define ARAD_MGMT_CORE_FREQ_MIN          150000
#define ARAD_MGMT_CORE_FREQ_MAX          900000
#define ARAD_MGMT_SYSTEM_FREQ_MIN        150000
#define ARAD_MGMT_SYSTEM_FREQ_MAX        900000
...
/*  uint8
   ...
   *  Mesh Mode
   */
  if (init->fabric.connect_mode == ARAD_FABRIC_CONNECT_MODE_MESH)
  {
  ...
  )
{
  ...
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 432, exit, WRITE_ECI_BLOCKS_SOFT_INITr(unit, wr_mask_val));
    ...
    if (!SOC_IS_ARDON(unit)) {
        SOC_SAND_SOC_IF_ERROR_RETURN(res, 460, exit, WRITE_ECI_PVT_MON_B_CONTROL_REGr(unit,val64));
    }
    sal_usleep(20000);
    COMPILER_64_SET(val64, 0, 0x41D0003);
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 470, exit, WRITE_ECI_PVT_MON_A_CONTROL_REGr(unit,val64));
    if (!SOC_IS_ARDON(unit)) {
        SOC_SAND_SOC_IF_ERROR_RETURN(res, 480, exit, WRITE_ECI_PVT_MON_B_CONTROL_REGr(unit,val64));
    }
  ...
  )
{
  ...
  /* FC should be enabled after ING TM FC tresholds are configures (in arad_itm_init) */
  res = arad_flow_control_init(
          unit,
          init->fc
        );
      ...
      )
{
    ...
    PBMP_IL_ITER(unit, port){
       ...
       SOC_SAND_SOC_IF_ERROR_RETURN(res, 11, exit, soc_port_sw_db_protocol_offset_get(unit, port, &offset));
...
arad_ser_init(int unit)
{
    ...
    if (!SOC_WARM_BOOT(unit)) { 
		...
		if(soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "evc_filter", 0))
		{
			SOC_SAND_SOC_IF_ERROR_RETURN(res, 36, exit, soc_reg_above_64_field32_modify(unit, EPNI_PAR_ERR_MEM_MASKr, REG_PORT_ANY, 0, PRGE_INSTRUCTION_11_PARITY_ERR_MASKf, 0));
		}
  ...
  )
{
  ...
  if (SOC_DPP_CONFIG(unit)->tm.is_petrab_in_system) {
      SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  37,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, ECI_GLOBALFr, REG_PORT_ANY, 0, SYSTEM_HEADERS_MODEf,  0x1));
    ...
    )
{
  ...
  if ((init->fabric.enable) && (!SOC_IS_ARDON(unit))) {
    ...
    SOC_SAND_IN  int                                unit)
{
    ...
    SOC_PBMP_ITER(pon_port_bm, port){
        ...
        {
          pp_port = ((idx << 3) | port);
          res = arad_parser_pp_port_nof_bytes_to_remove_set(unit, pp_port, 4);
          SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);
        }    
    ...
    SOC_SAND_IN  int                                unit)
{
    ...
    if (!SOC_IS_ARDON(unit) && SOC_IS_ARADPLUS(unit)) 
    {
        ...
        SOCDNX_IF_ERR_EXIT(READ_IHP_FLP_GENERAL_CFGr(unit, &reg_val_64));
        fld_val = 0x1; 
        ...
        SOCDNX_IF_ERR_EXIT(WRITE_IHP_FLP_GENERAL_CFGr(unit, reg_val_64));
  ...
  )
{
...
#ifdef PLISIM
  if (SAL_BOOT_PLISIM) {
      res = chip_sim_em_init(SOC_SAND_DEV_ARAD);
  ...
  SOC_PBMP_ITER(pbmp,port_i) {
      ...
      res = arad_egr_dsp_pp_to_base_q_pair_set(unit, tm_port, core, base_q_pair);
      SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 145, exit);

      /* Set port priorities */
      res = arad_egr_dsp_pp_priorities_mode_set(unit, tm_port, core, priority_mode);
      SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 145, exit);

      /* Set egress port shaper mode */
      res = arad_egr_dsp_pp_shaper_mode_set_unsafe(unit, tm_port, core, shaper_mode);
...
/*  uint32
  ...
  if (init->core_freq.enable)
  {
    SOC_SAND_ERR_IF_OUT_OF_RANGE(init->core_freq.frequency, ARAD_MGMT_CORE_FREQ_MIN, ARAD_MGMT_CORE_FREQ_MAX, ARAD_CORE_FREQ_OUT_OF_RANGE_ERR, 40, exit);
    SOC_SAND_ERR_IF_OUT_OF_RANGE(init->core_freq.system_ref_clock, ARAD_MGMT_SYSTEM_FREQ_MIN, ARAD_MGMT_SYSTEM_FREQ_MAX, ARAD_CORE_FREQ_OUT_OF_RANGE_ERR, 42, exit);
  }
  ...
  )
{
  ...
  res = arad_fill_table_with_entry(unit, IRR_SNOOP_MIRROR_TM_TABLEm, MEM_BLOCK_ANY, table_entry); /* fill table with the entry */
  ...
  )
{
  ...
  /* Set all to 1 to disable the lookups */
  pmf_profile_data = SOC_IS_JERICHO(unit)? 0x7FFFFFF :0xFFFFFF;
  ...
  )
{
  ...
  soc_mem_field32_set(unit, IQM_PQWQm, table_entry, AVRG_ENf, 1); /* Needed to enable WRED for the rate class */
...
{   
    ...
    for(i=0;i<ARAD_FBC_PRIORITY_NDX_NOF ; i++) {
        ...
        if (SOC_IS_JERICHO(unit)) {
            res = dcmn_fill_partial_table_with_entry(unit, IPT_PRIORITY_BITS_MAPPING_2_FDTm, 0, 0, MEM_BLOCK_ALL, i, i, &fabric_priority); 
            SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 25, exit);
            res = dcmn_fill_partial_table_with_entry(unit, IPT_TDM_BIT_MAPPING_2_FDTm, 0, 0, MEM_BLOCK_ALL, i, i, &is_tdm); 
  ...
  )
{
  ...
  if (SOC_DPP_CONFIG(unit)->emulation_system == 0) {
      ...
      res = arad_fill_table_with_entry(unit, EGQ_PACKET_PROCESSING_PER_PORT_CONFIGURATION_TABLE_PP_PPCTm, MEM_BLOCK_ANY, &mtu_table_entry);
      SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
  }
  ...
  )
{
  ...
  res = arad_fill_table_with_entry(unit, SCH_SCHEDULER_ENABLE_MEMORY__SEMm, MEM_BLOCK_ANY, table_entry);
  SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);

  /* SHC */
  /* all relevant bits initialized to 0 */
  res = arad_fill_table_with_entry(unit, SCH_HR_SCHEDULER_CONFIGURATION__SHCm, MEM_BLOCK_ANY, table_entry);
  SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);

  /* SCT */
  /* all relevant bits initialized to 0 */
  res = arad_fill_table_with_entry(unit, SCH_CL_SCHEDULERS_TYPE__SCTm, MEM_BLOCK_ANY, table_entry);
  SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);

  /* FGM */
  /* all relevant bits initialized to 0 */
  res = arad_fill_table_with_entry(unit, SCH_FLOW_GROUP_MEMORY__FGMm, MEM_BLOCK_ANY, table_entry);
  SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);
...
if (SOC_DPP_CONFIG(unit)->emulation_system == 0) {
      ...
      errnum += 10;
      SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
      res = arad_fill_table_with_entry(unit, EGQ_PORT_CONFIGURATION_TABLE_PCTm, MEM_BLOCK_ANY, table_entry);
      errnum += 10;
      SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
      ...
      errnum += 10;
      SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
      res = arad_fill_table_with_entry(unit, EGQ_PACKET_PROCESSING_PER_PORT_CONFIGURATION_TABLE_PP_PPCTm, MEM_BLOCK_ANY, table_entry);
      errnum += 10;
      SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
      ...
      errnum += 10;
      SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
      res = arad_fill_table_with_entry(unit, EGQ_TTL_SCOPE_MEMORY_TTL_SCOPEm, MEM_BLOCK_ANY, table_entry);
      errnum += 10;
      SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
      ...
      if (!SOC_IS_ARDON(unit)) {
          ...
          errnum += 10;
          SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
          res = arad_fill_table_with_entry(unit, EGQ_AUXILIARY_DATA_MEMORY_AUX_TABLEm, MEM_BLOCK_ANY, table_entry);
          errnum += 10;
          SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
      }
      res = arad_fill_table_with_entry(unit, EGQ_VSI_PROFILE_MEMORY_VSI_PROFILEm, MEM_BLOCK_ANY, table_entry);
      errnum += 10;
      SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
  }
      res = arad_fill_table_with_entry(unit, EGQ_INGRESS_VLAN_EDITING_COMMAND_IVEC_TABLEm, MEM_BLOCK_ANY, table_entry);
      errnum += 10;
      SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
  ...
  if (SOC_DPP_CONFIG(unit)->emulation_system == 0) {
      ...
      errnum += 10;
      SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
      res = arad_fill_table_with_entry(unit, IPS_FLOW_ID_LOOKUP_TABLEm, MEM_BLOCK_ANY, table_entry);
      errnum += 10;
      SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
  ...
  res = arad_fill_table_with_entry(unit, IRE_CTXT_MAPm, MEM_BLOCK_ANY, table_entry);
  errnum += 10;
  SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
  ...
  if (SOC_DPP_CONFIG(unit)->emulation_system == 0) {
      ...
      errnum = 25000; /* RTP */
      res = arad_fill_table_with_entry(unit, RTP_CRMAm, MEM_BLOCK_ANY, table_entry);
      errnum += 10;
      SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
      res = arad_fill_table_with_entry(unit, RTP_DRMAm, MEM_BLOCK_ANY, table_entry);
      errnum += 10;
      SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
      res = arad_fill_table_with_entry(unit, RTP_RRMAm, MEM_BLOCK_ANY, table_entry);
      errnum += 10;
      SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);

      errnum = 26000; /* SCH */
      res = arad_fill_table_with_entry(unit, SCH_CH_NIF_CALENDAR_CONFIGURATION__CNCCm, MEM_BLOCK_ANY, table_entry);
      errnum += 10;
      SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
      res = arad_fill_table_with_entry(unit, SCH_CH_NIF_RATES_CONFIGURATION__CNRCm, MEM_BLOCK_ANY, table_entry);
      errnum += 10;
      SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
      res = arad_fill_table_with_entry(unit, SCH_CIR_SHAPERS_STATIC_TABEL__CSSTm, MEM_BLOCK_ANY, table_entry);
      errnum += 10;
      SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
      res = arad_fill_table_with_entry(unit, SCH_CIR_SHAPER_CALENDAR__CSCm, MEM_BLOCK_ANY, table_entry);
      errnum += 10;
      SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
      res = arad_fill_table_with_entry(unit, SCH_CL_SCHEDULERS_CONFIGURATION__SCCm, MEM_BLOCK_ANY, table_entry);
      errnum += 10;
      SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
      res = arad_fill_table_with_entry(unit, SCH_CL_SCHEDULERS_TYPE__SCTm, MEM_BLOCK_ANY, table_entry);
      errnum += 10;
      SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
      res = arad_fill_table_with_entry(unit, SCH_DEVICE_RATE_MEMORY__DRMm, MEM_BLOCK_ANY, table_entry);
      errnum += 10;
      SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
      res = arad_fill_table_with_entry(unit, SCH_DSP_2_PORT_MAP__DSPPm, MEM_BLOCK_ANY, table_entry);
      errnum += 10;
      SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
      res = arad_fill_table_with_entry(unit, SCH_DUAL_SHAPER_MEMORY__DSMm, MEM_BLOCK_ANY, table_entry);
      errnum += 10;
      SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
      res = arad_fill_table_with_entry(unit, SCH_FC_MAP__FCMm, MEM_BLOCK_ANY, table_entry);
      errnum += 10;
      SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
      res = arad_fill_table_with_entry(unit, SCH_FLOW_DESCRIPTOR_MEMORY_STATIC__FDMSm, MEM_BLOCK_ANY, table_entry);
      errnum += 10;
      SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
      res = arad_fill_table_with_entry(unit, SCH_FLOW_GROUP_MEMORY__FGMm, MEM_BLOCK_ANY, table_entry);
      errnum += 10;
      SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
      res = arad_fill_table_with_entry(unit, SCH_FLOW_SUB_FLOW__FSFm, MEM_BLOCK_ANY, table_entry);
      errnum += 10;
      SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
      res = arad_fill_table_with_entry(unit, SCH_FLOW_TO_FIP_MAPPING__FFMm, MEM_BLOCK_ANY, table_entry);
      errnum += 10;
      SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
      res = arad_fill_table_with_entry(unit, SCH_FLOW_TO_QUEUE_MAPPING__FQMm, MEM_BLOCK_ANY, table_entry);
      errnum += 10;
      SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
      res = arad_fill_table_with_entry(unit, SCH_FORCE_STATUS_MESSAGEm, MEM_BLOCK_ANY, table_entry);
      errnum += 10;
      SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
      res = arad_fill_table_with_entry(unit, SCH_HR_SCHEDULER_CONFIGURATION__SHCm, MEM_BLOCK_ANY, table_entry);
      errnum += 10;
  ...
  if (SOC_DPP_CONFIG(unit)->emulation_system == 0) {
      ...
      res = arad_fill_table_with_entry(unit, SCH_MEM_01F00000m, MEM_BLOCK_ANY, table_entry);
      errnum += 10;
      SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
      res = arad_fill_table_with_entry(unit, SCH_MEM_04700000m, MEM_BLOCK_ANY, table_entry);
      errnum += 10;
      SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
      res = arad_fill_table_with_entry(unit, SCH_MEM_04A00000m, MEM_BLOCK_ANY, table_entry);
      errnum += 10;
      SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
      res = arad_fill_table_with_entry(unit, SCH_MEM_04D00000m, MEM_BLOCK_ANY, table_entry);
      errnum += 10;
      SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
  }
  ...
  if (SOC_DPP_CONFIG(unit)->emulation_system == 0) {
      ...
      res = arad_fill_table_with_entry(unit, SCH_ONE_PORT_NIF_CONFIGURATION__OPNCm, MEM_BLOCK_ANY, table_entry);
      errnum += 10;
      SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
      res = arad_fill_table_with_entry(unit, SCH_PIR_SHAPERS_STATIC_TABEL__PSSTm, MEM_BLOCK_ANY, table_entry);
      errnum += 10;
      SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
      res = arad_fill_table_with_entry(unit, SCH_PIR_SHAPER_CALENDAR__PSCm, MEM_BLOCK_ANY, table_entry);
      errnum += 10;
      SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
      res = arad_fill_table_with_entry(unit, SCH_PORT_ENABLE__PORTENm, MEM_BLOCK_ANY, table_entry);
      errnum += 10;
      SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
      res = arad_fill_table_with_entry(unit, SCH_PORT_GROUP__PFGMm, MEM_BLOCK_ANY, table_entry);
      errnum += 10;
      SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
      res = arad_fill_table_with_entry(unit, SCH_PORT_QUEUE_SIZE__PQSm, MEM_BLOCK_ANY, table_entry);
      errnum += 10;
      SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
      res = arad_fill_table_with_entry(unit, SCH_PORT_SCHEDULER_MAP__PSMm, MEM_BLOCK_ANY, table_entry);
      errnum += 10;
      SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
      res = arad_fill_table_with_entry(unit, SCH_PORT_SCHEDULER_WEIGHTS__PSWm, MEM_BLOCK_ANY, table_entry);
      errnum += 10;
      SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
      res = arad_fill_table_with_entry(unit, SCH_SCHEDULER_CREDIT_GENERATION_CALENDAR__CALm, MEM_BLOCK_ANY, table_entry);
      errnum += 10;
      SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
      res = arad_fill_table_with_entry(unit, SCH_SCHEDULER_ENABLE_MEMORY__SEMm, MEM_BLOCK_ANY, table_entry);
      errnum += 10;
      SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
      res = arad_fill_table_with_entry(unit, SCH_SHAPER_DESCRIPTOR_MEMORY_STATIC__SHDSm, MEM_BLOCK_ANY, table_entry);
      errnum += 10;
      SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
      res = arad_fill_table_with_entry(unit, SCH_TOKEN_MEMORY_CONTROLLER__TMCm, MEM_BLOCK_ANY, table_entry);
      errnum += 10;
      SOC_SAND_CHECK_FUNC_RESULT(res, errnum, exit);
  }
