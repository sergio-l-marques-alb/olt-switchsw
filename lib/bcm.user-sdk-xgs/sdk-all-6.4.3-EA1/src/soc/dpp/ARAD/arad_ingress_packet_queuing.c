...
#define ARAD_IPQ_DESTINATION_ID_STACKING_BASE_QUEUE_MIN     ((0x7ff) << 6)
#define ARAD_IPQ_DESTINATION_ID_STACKING_BASE_QUEUE_MAX     0x1ffff
#define ARAD_IPQ_DESTINATION_ID_INVALID_QUEUE ARAD_IPQ_DESTINATION_ID_STACKING_BASE_QUEUE_MAX
...
#define ARAD_IPQ_MIN_INTERDIGIT_FLOW_QUARTET   6144
...
arad_ipq_traffic_class_profile_map_set_unsafe(
   ...
   nof_entries = (is_flow_profile ? ARAD_IPQ_NOF_UC_FLOW_DEST_TC_ENTRIES : ARAD_IPQ_NOF_UC_SYS_PORT_DEST_TC_ENTRIES);

   for (line_ndx = 0; line_ndx < nof_entries; ++line_ndx) {
      if (is_flow_profile) {
         /* Offset is for flow id destination */
         entry_offset = ARAD_IPQ_UC_FLOW_ID_TC_ENTRY(profile_ndx, line_ndx);
      } else {
         /* Offset is for system port destination */
         entry_offset = ARAD_IPQ_UC_SYS_PORT_TC_ENTRY(profile_ndx, line_ndx);
      }
...
arad_ipq_traffic_class_profile_map_get_unsafe(
   ...
   if (is_flow_profile) {
      entry_offset = ARAD_IPQ_UC_FLOW_ID_TC_ENTRY(profile_ndx, 0);
   } else {
      entry_offset = ARAD_IPQ_UC_SYS_PORT_TC_ENTRY(profile_ndx, 0);
   }
...
arad_ipq_destination_id_packets_base_queue_id_verify(
   ...
   if (!((base_queue >= ARAD_IPQ_DESTINATION_ID_PACKETS_BASE_BASE_QUEUE_MIN) || (base_queue <= (SOC_DPP_DEFS_GET(unit, nof_queues) - 1)) ||
         (base_queue >= ARAD_IPQ_DESTINATION_ID_STACKING_BASE_QUEUE_MIN) || (base_queue <= ARAD_IPQ_DESTINATION_ID_STACKING_BASE_QUEUE_MAX))) {
...
arad_ipq_destination_id_packets_base_queue_id_set_unsafe(
   ...
   res = arad_irr_destination_table_tbl_get_unsafe(
      unit,
      dest_ndx,
      &irr_destination_table_tbl_data
      );
   ...
   if(valid) {
       ...
       irr_destination_table_tbl_data.queue_number = ARAD_IPQ_DESTINATION_ID_INVALID_QUEUE;
   }
...
arad_ipq_destination_id_packets_base_queue_id_get_unsafe(
   ...
   *base_queue = irr_destination_table_tbl_data.queue_number;
   if(*base_queue == ARAD_IPQ_DESTINATION_ID_INVALID_QUEUE) {
       *valid = FALSE;
   } else {
       *valid = TRUE;
   }
...
arad_ipq_queue_interdigitated_mode_set_unsafe(
   ...
   SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  10,  exit, ARAD_REG_ACCESS_ERR,READ_IPS_INTERDIGITATED_MODEr(unit, SOC_CORE_ALL, &buffer));
   ...
   SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  20,  exit, ARAD_REG_ACCESS_ERR,WRITE_IPS_INTERDIGITATED_MODEr(unit, SOC_CORE_ALL,  buffer));
...
arad_ipq_queue_to_flow_mapping_verify(
   ...
   if (queue_quartet_ndx > 0) {
      ...
      if ((is_interdigitated) &&                                                                            /*Invalid connector - can be either intrdigitated or not*/
             (info->flow_quartet_index < ARAD_IPQ_MIN_INTERDIGIT_FLOW_QUARTET) && (info->flow_quartet_index != ARAD_DEVICE_QUEUE_RESERVED_FLOW_START)) {SOC_SAND_SET_ERROR_CODE(ARAD_IPQ_BASE_FLOW_FOR_INTERDIGIT_QUEUE_QUARTET_TOO_LOW_ERR, 60, exit);
...
arad_ipq_queue_to_flow_mapping_set_unsafe(
    ...
    * Set base flow {
    */
   res = arad_ips_flow_id_lookup_table_tbl_get_unsafe(unit, queue_quartet_ndx, &ips_flow_id_lookup_table_tbl_data);
   SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);

   res = indirect_mode ?
      arad_indirect_base_queue_to_system_physical_port_tbl_get_unsafe(unit, queue_quartet_ndx, &system_physical_port_tbl_data) :
      arad_direct_base_queue_to_system_physical_port_tbl_get_unsafe(unit, queue_quartet_ndx, &mod_port_tbl_data);
   ...
   if (!(
         ...
         (indirect_mode ?
          ...
          (info->is_modport ?
           (mod_port_tbl_data.fap_id == ARAD_MAX_FAP_ID && mod_port_tbl_data.fap_port_id == ARAD_MAX_FAP_PORT_ID) :
           info->system_physical_port == SOC_TMC_MAX_SYSTEM_PHYSICAL_PORT_ID_ARAD))
         )) {
      /*
       *    This is a re-map. First unmap and flush all queues
       *  for a clean transition
       */
      res = arad_ipq_queue_qrtt_unmap_unsafe(unit, queue_quartet_ndx);
      SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
   }
   ...
   res = arad_ips_flow_id_lookup_table_tbl_set_unsafe(unit, queue_quartet_ndx, &ips_flow_id_lookup_table_tbl_data);
   ...
   if (indirect_mode) {
      ...
      res = arad_indirect_base_queue_to_system_physical_port_tbl_set_unsafe(unit, queue_quartet_ndx, &system_physical_port_tbl_data);
   } else {
      ...
      res = arad_direct_base_queue_to_system_physical_port_tbl_set_unsafe(unit, queue_quartet_ndx, &mod_port_tbl_data);
   }
  ...
  )
{
  ...
  for(i=0 ; i<iterations ; i++) {
    ...
    if(nof_queues_filled > 0 && 
       queue.queue_id == queue_ndx && 
       queue.queue_byte_size > 0) {
        (*is_empty) = 0;
        break;
    }
  }
...
arad_ipq_queue_qrtt_unmap_unsafe(
   ...
   if(dpp->arad->init.fabric.connect_mode == ARAD_FABRIC_CONNECT_MODE_MESH) {
       for (q_id = baseq_id; q_id <= baseq_id + 3; q_id++){
          res = soc_arad_ipq_queue_empty_check_unsafe(unit, q_id, ARAD_IPQ_QUEUE_EMPTY_ITERATIONS, &is_empty);
...
arad_ipq_queue_to_flow_mapping_get_unsafe(
   ...
   if (ARAD_IS_VOQ_MAPPING_INDIRECT(unit)) { /* indirect mapping mode */
      res = arad_indirect_base_queue_to_system_physical_port_tbl_get_unsafe(unit, queue_quartet_ndx, &system_physical_port_tbl_data);
      SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
      ...
      res = arad_direct_base_queue_to_system_physical_port_tbl_get_unsafe(unit, queue_quartet_ndx, &mod_port_tbl_data);
      SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
      ...
      SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);
      info->system_physical_port = sysport == ARAD_NOF_SYS_PHYS_PORTS ? -1 : sysport;
   }
...
arad_ipq_k_quartet_reset_unsafe(
   ...
   if (ARAD_IS_VOQ_MAPPING_INDIRECT(unit)) { /* indirect mapping mode */
      ARAD_BASE_Q_TO_SYS_PORT_TABLE_TBL_DATA system_physical_port_tbl_data;
      system_physical_port_tbl_data.sys_phy_port = ARAD_MAX_SYSTEM_PHYSICAL_PORT_ID_IN_IPS;
      res = arad_indirect_base_queue_to_system_physical_port_tbl_region_set_unsafe(unit, queue_k_quartet_ndx, region_size, &system_physical_port_tbl_data);
      SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
   } else { /* direct mapping mode */
      ARAD_BASE_Q_TO_MOD_PORT_TABLE_TBL_DATA mod_port_tbl_data;
      mod_port_tbl_data.fap_id = ARAD_MAX_FAP_ID;
      mod_port_tbl_data.fap_port_id = ARAD_MAX_FAP_PORT_ID;
      res = arad_direct_base_queue_to_system_physical_port_tbl_region_set_unsafe(unit, queue_k_quartet_ndx, region_size, &mod_port_tbl_data);
      SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
   }
   /*
    * Set base flow {
   ...
   res = arad_ips_flow_id_lookup_table_tbl_region_set_unsafe(unit, queue_k_quartet_ndx, region_size, &ips_flow_id_lookup_table_tbl_data);
   SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
   /*
    * Set base flow }
    */
   ...
   ) 
{
   ...
   nof_queues = SOC_DPP_DEFS_GET(unit, nof_queues);

   /* If DMA is enabled, try to alloc buffer for it, and if suceeding use DMA */
   if (soc_mem_dmaable(unit, IQM_PQDMDm, SOC_MEM_BLOCK_ANY(unit, IQM_PQDMDm))) {
      /* read the table using DMA */
      size = SOC_MEM_TABLE_BYTES(unit, IQM_PQDMDm);
      index_max = soc_mem_index_max(unit, IQM_PQDMDm);
      index_min = soc_mem_index_min(unit, IQM_PQDMDm); 
      entry_array = soc_cm_salloc(unit, size, "IQM_PQDMDm");
      if (entry_array == NULL) {
         SOCDNX_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_SOCDNX_MSG("failed to allocate memory")));
      }
      _rv = soc_mem_array_read_range(unit, IQM_PQDMDm, 0, MEM_BLOCK_ANY, index_min, index_max, entry_array);
   ...
   for (queue_id = first_queue; queue_id < nof_queues; ++queue_id) {
      if (entry_array != NULL) {
         entry_words = soc_mem_entry_words(unit, IQM_PQDMDm);
         entry = entry_array + (entry_words * queue_id);
         soc_mem_field_get(unit, IQM_PQDMDm, entry, PQ_INST_QUE_SIZEf, &queue_byte_size);
      } else {
         res = arad_iqm_dynamic_tbl_get_unsafe(unit,queue_id,&iqm_dynamic_tbl);
...
arad_ipq_tc_profile_set_unsafe(
   ...
   if (is_flow_ndx) {
      ...
      SOC_SAND_SOC_IF_ERROR_RETURN(res, 1000, exit, READ_IRR_FLOW_TABLEm(unit, MEM_BLOCK_ANY, line_ndx, &tbl_data));
      ...
      SOC_SAND_SOC_IF_ERROR_RETURN(res, 1010, exit, WRITE_IRR_FLOW_TABLEm(unit, MEM_BLOCK_ANY, line_ndx, &tbl_data));
   } else { /* Unicast mapping */
      line_ndx = dest_ndx;

      SOC_SAND_SOC_IF_ERROR_RETURN(res, 1020, exit, READ_IRR_DESTINATION_TABLEm(unit, MEM_BLOCK_ANY, line_ndx, &tbl_data));
...
arad_ipq_tc_profile_get_unsafe(
   ...
   if (is_flow_ndx) {
      ...
      SOC_SAND_SOC_IF_ERROR_RETURN(res, 1130, exit, READ_IRR_FLOW_TABLEm(unit, MEM_BLOCK_ANY, line_ndx, &tbl_data));
      ...
      SOC_SAND_SOC_IF_ERROR_RETURN(res, 1140, exit, READ_IRR_DESTINATION_TABLEm(unit, MEM_BLOCK_ANY, line_ndx, &tbl_data));
...
/*********************************************************************
*     Populate the modport2sysport from the hardware tables.
*     Assumes that the database is allocated and empty.
*     This function should only be called after calling the
*     arad_ipq_queue_interdigitated_mode_set()
*     In direct voq to modport mapping mode, a mapping to modport is performed
*     Details: in the H file. (search for prototype)
*********************************************************************/
int
arad_populate_sw_db_modport2sysport_from_hw_unsafe(
   SOC_SAND_IN int unit
   ) {
   ARAD_SYSPORT sysport;
   uint32 baseq, fap_id, fap_port_id;
   uint32 entry, *entry_p = &entry;
   uint32 *alloced_pci_mem = 0; /* buffer for reading tables by DMA, will hold in order: IRR_DESTINATION_TABLEm, IPS_QPM_1_NO_SYS_REDm, IPS_QPM_2m */
   uint32 *qpm_1 = 0,*qpm_2 = 0; /* locations inside alloced_pci_mem for tables */


   SOCDNX_INIT_FUNC_DEFS;

   if (ARAD_IS_VOQ_MAPPING_INDIRECT(unit)) {
      SOCDNX_EXIT_WITH_ERR(SOC_E_UNAVAIL, (_BSL_SOCDNX_MSG_STR("Not using direct mapping mode\n")));
   }

   /* If DMA is enabled, try to alloc buffer for it, and if suceeding use DMA */
   if (soc_mem_dmaable(unit, IRR_DESTINATION_TABLEm, SOC_MEM_BLOCK_ANY(unit, IRR_DESTINATION_TABLEm)) &&
       (alloced_pci_mem = soc_cm_salloc(unit, 4 * DMA_ALLOC_WORDS, "modport2sysport_dma"))) {
      qpm_1 = alloced_pci_mem + IRR_DESTINATION_TABLE_SIZE;
      qpm_2 = qpm_1 + IPS_QPM_1_NO_SYS_RED_SIZE;

      /* read the three tables using DMA */
      SOCDNX_IF_ERR_EXIT(soc_mem_read_range(unit, IRR_DESTINATION_TABLEm, MEM_BLOCK_ANY, 0, IRR_DESTINATION_TABLE_SIZE - 1, alloced_pci_mem));
      SOCDNX_IF_ERR_EXIT(soc_mem_read_range(unit, IPS_QPM_1_NO_SYS_REDm, MEM_BLOCK_ANY, 0, IPS_QPM_1_NO_SYS_RED_SIZE - 1, qpm_1));
      SOCDNX_IF_ERR_EXIT(soc_mem_read_range(unit, IPS_QPM_2_NO_SYS_REDm, MEM_BLOCK_ANY, 0, IPS_QPM_2_SIZE - 1, qpm_2));
   }

   for (sysport = 0; sysport < SOC_TMC_MAX_SYSTEM_PHYSICAL_PORT_ID_ARAD; ++sysport) { /* loop over legal sysports */
      if (alloced_pci_mem) { /* set entry_p either to previously filled DMA buffer or to value read now from the table */
         entry_p = alloced_pci_mem + sysport;
      } else {
         SOCDNX_IF_ERR_EXIT(READ_IRR_DESTINATION_TABLEm(unit, MEM_BLOCK_ANY, sysport, &entry));
      }
      baseq = soc_mem_field32_get(unit, IRR_DESTINATION_TABLEm, entry_p, QUEUE_NUMBERf); /* get queue from system physical port */

      if (baseq < ARAD_MAX_QUEUE_ID(unit)) { /* If the system physical port to base queue mapping is set, process the base queue */
         /* LOG_INFO(BSL_LS_SOC_INGRESS,
                     (BSL_META_U(unit,
                                 "sysport-q %u --> %u\n"),sysport, baseq)); */
         baseq >>= 2; /* and translate the queue to the base queue */

         /* get the moule from the base queue */
         /* There is no copy-paste mistake. There shouldn't be qpm_1 instead of alloced_pci_mem */
         /* coverity[copy_paste_error:FALSE] */
         if (alloced_pci_mem) { /* set entry_p either to previously filled DMA buffer or to value read now from the table */
            entry_p = qpm_1 + baseq;
         } else {
            SOCDNX_IF_ERR_EXIT(READ_IPS_QPM_1_NO_SYS_REDm(unit, MEM_BLOCK_ANY, baseq, &entry));
         }
         fap_id = soc_mem_field32_get(unit, IPS_QPM_1_NO_SYS_REDm, entry_p, DEST_DEVf);
         if (fap_id < ARAD_MAX_FAP_ID) { /* if the mapping is not empty */
            /* get the moule's port from the base queue */
            fap_port_id = soc_mem_field32_get(unit, IPS_QPM_1_NO_SYS_REDm, entry_p, DEST_PORT_MSBf) << BASE_QUEUES_PER_QPM_FAP_PORT_MSB_OFFSET;
            if (alloced_pci_mem) { /* set entry_p either to previously filled DMA buffer or to value read now from the table */
               entry_p = qpm_2 + (baseq / BASE_QUEUES_PER_QPM_2_ENTRY);
            } else {
               SOCDNX_IF_ERR_EXIT(READ_IPS_QPM_2_NO_SYS_REDm(unit, MEM_BLOCK_ANY, baseq / BASE_QUEUES_PER_QPM_2_ENTRY, &entry));
            }
            fap_port_id |= soc_mem_field32_get(unit, IPS_QPM_2_NO_SYS_REDm, entry_p, qpm2_field_per_base_queue[baseq % BASE_QUEUES_PER_QPM_2_ENTRY]);
            /* LOG_INFO(BSL_LS_SOC_INGRESS,
                        (BSL_META_U(unit,
                                    "baseq-modport       %u --> %u , %u\n"),baseq, fap_id, fap_port_id)); */
            if (arad_sw_db_modport2sysport_set(unit, fap_id, fap_port_id, sysport)) { /* set the found modport to sysport mapping */
               SOCDNX_EXIT_WITH_ERR(SOC_E_FAIL, (_BSL_SOCDNX_MSG_STR("failed setting mapping from modport %u,%u to %u\n"), fap_id, fap_port_id, sysport));
            }
         }
      }
   }
