  ...
  )
{
  ...
  for (indx = 0; indx < nof_entries; ++indx)
  {
    ...
    LOG_CLI((BSL_META_U(unit,
                        "|%-6s"), SOC_PPD_FEC_TYPE_to_string_short(working_fec.type)));
  ...
  )
{
  ...
  SOC_PPD_ARAD_TMP_DEVICE_CALL(frwrd_fec_ecmp_hashing_port_info_set,(unit, port_ndx, port_hash_info));
  ...
  )
{
  ...
  SOC_PPD_ARAD_TMP_DEVICE_CALL(frwrd_fec_ecmp_hashing_port_info_get,(unit, port_ndx, port_hash_info));
