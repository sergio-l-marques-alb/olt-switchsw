...
uint32  _soc_mem_entry_llr_llvp_null[SOC_MAX_MEM_WORDS] = {0x90};
    ...
    defined(BCM_PETRA_SUPPORT) || defined(BCM_DFE_SUPPORT)  || defined(BCM_CALADAN3_SUPPORT) || defined(PORTMOD_SUPPORT)
...
#define _SOC_MEM_SYNC_MAPPED_MEM_STATES(unit, mem) {\
    ...
    case VLAN_TABm: \
        if (SOC_IS_KATANA2(unit)) { \
            SOC_MEM_BLOCK_ITER(unit, EGR_VLANm, blk) {\
                if (copyno != COPYNO_ALL && copyno != blk) {\
                    continue;\
                }\
            SOC_MEM_STATE(unit, EGR_VLANm).cache[blk] = cache;\
            SOC_MEM_STATE(unit, EGR_VLANm).vmap[blk] = vmap;\
            }\
        }\
        break;\
    default: break;\
...
soc_control_defip_scache_init(int unit)
{
    ...
    if (NULL == SOC_L3_DEFIP_SCACHE_HANDLE(unit)) {
        SOC_IF_ERROR_RETURN(soc_stable_size_get(unit, &stable_size));
        if (!stable_size) {
        ...
        alloc_sz += SOC_WB_SCACHE_CONTROL_SIZE;
        if (stable_size - (stable_used + alloc_sz) < 0) {
        ...
        if (!SOC_WARM_BOOT_SCACHE_IS_LIMITED(unit)) {
            ...
            /* Get the pointer for the Level 2 cache */
            rv = soc_scache_ptr_get(unit, scache_handle,
                                    &l3_defip_scache_ptr, &alloc_get);
            if (!SOC_WARM_BOOT(unit) || (SOC_E_NOT_FOUND == rv)) {
                /* Not yet allocated in Cold Boot or in upgrade */
                SOC_IF_ERROR_RETURN
                ...
                if (SOC_FAILURE(rv)) {
                    ...
                    /* Expected size doesn't match retrieved size */
                    return SOC_E_INTERNAL;
                } else if (NULL == l3_defip_scache_ptr) {
                    return SOC_E_MEMORY;
                }
...
soc_mem_dmaable(int unit, soc_mem_t mem, int copyno)
{
    ...
    if(SOC_IS_ARAD(unit)) {
#ifndef PLISIM
        if (!SOC_IS_JERICHO(unit) && (SOC_BLOCK_TYPE(unit, copyno) == SOC_BLK_SCH)) {
            return FALSE;
        }
        return TRUE;
#else
...
#if defined(BCM_FIREBOLT_SUPPORT)
#if defined(SOC_MEM_L3_DEFIP_WAR) || defined(BCM_TRIUMPH3_SUPPORT) || defined(BCM_ENDURO_SUPPORT)
    if (soc_feature(unit, soc_feature_l3_defip_map) &&
        (mem == L3_DEFIPm ||
         mem == L3_DEFIP_ONLYm ||
         mem == L3_DEFIP_DATA_ONLYm ||
                  ...
                  uint32 ser_flags, void *buffer)
{
    ...
    if (rv < 0) {
        if (rv != SOC_E_TIMEOUT) {
            ...
            LOG_ERROR(BSL_LS_SOC_SOCMEM,
                      (BSL_META_U(unit,
                                  "TableDmaTimeOut:%s, %s.%s %s timeout\n"),
                       FUNCTION_NAME(), SOC_MEM_UFNAME(unit, mem),
                       SOC_BLOCK_NAME(unit, copyno),
    ...
    if (!flags || (flags & SOC_MEM_WRITE_STATUS_ONLY)) {
        ...
        if (rv < 0) {
            if (rv != SOC_E_TIMEOUT) {
                ...
                LOG_ERROR(BSL_LS_SOC_SOCMEM,
                          (BSL_META_U(unit,
                                      "SlamDmaTimeOut:%s, %s.%s %s timeout\n"),
                           FUNCTION_NAME(), SOC_MEM_UFNAME(unit, mem),
                           SOC_BLOCK_NAME(unit, copyno),
              ...
              int copyno, int index, void *entry_data)
{
    ...
    if (index_valid && 
        ...
        (cache != NULL) && !no_cache &&
        CACHE_VMAP_TST(vmap, index) && !SOC_MEM_TEST_SKIP_CACHE(unit)) {
#ifdef INCLUDE_TCL
        ...
        if (cache_consistency_check) {
            ...
            sal_memcpy(entry_data_cache,
                       cache + mem_array_cache_offset + index * entry_dw, entry_dw * 4);
        } else
#endif
        {
            sal_memcpy(entry_data, cache + mem_array_cache_offset + index * entry_dw, entry_dw * 4);
    ...
    if (SOC_IS_TRIDENT2(unit)) {
        if (mem == MMU_INTFI_XPIPE_FC_MAP_TBL0m || 
            ...
            rv = soc_trident2_fc_map_shadow_entry_get(unit, mem, index, entry_data);
            goto done;
        }
    }
#endif
            ...
            if (!(flags & SOC_MEM_SCHAN_ERR_RETURN)) {
                if (cache != NULL && CACHE_VMAP_TST(vmap, index) 
                    ...
                    if (SOC_IS_TD2_TT2(unit) &&
                        ...
                        if (schan_msg_cpy.readcmd.header.v2.src_blk == _SOC_MEM_ADDR_ACC_TYPE_PIPE_Y) {
                            schan_msg_cpy.readcmd.header.v2.src_blk = _SOC_MEM_ADDR_ACC_TYPE_PIPE_X;
                        } else {
                            schan_msg_cpy.readcmd.header.v2.src_blk = _SOC_MEM_ADDR_ACC_TYPE_PIPE_Y;
                        }
...
#define _SOC_ENTRY_PARITY_CLEAR(unit, mem, ecc_field)                       \
        ...
        /* 
         * Ignore the comparison of the case that data=1 and mask=0 due to 
         * 28nm TCAM encode methodology.
         */
        if (soc_feature(unit, soc_feature_xy_tcam_28nm) &&
           (meminfo->flags & SOC_MEM_FLAG_CAM) &&
            !(meminfo->flags & SOC_MEM_FLAG_EXT_CAM)) {
            soc_field_t key_field, mask_field;
            uint32 key_val[SOC_MAX_MEM_FIELD_WORDS];
            uint32 word_length = 0, k;
            uint32 mask_val[SOC_MAX_MEM_FIELD_WORDS];
            /* 
             * The values become D=0 and M=0 due to TCAM 28nm eccode mothodology
             * when read from HW.
             */
            key_field = INVALIDf;
            mask_field = INVALIDf;
            if (SOC_MEM_FIELD_VALID(unit, mem, KEYf)) {
                key_field = KEYf;
                mask_field = MASKf;
            } else if (SOC_MEM_FIELD_VALID(unit, mem, FULL_KEYf)) {
                key_field = FULL_KEYf;
                mask_field = FULL_MASKf;
            }
            
            if (key_field != INVALIDf) {
                word_length = (soc_mem_field_length(
                    unit, mem, key_field) + 31) / 32;
                soc_mem_field_get(unit, mem, entry_data_cache,
                    key_field, key_val);
                soc_mem_field_get(unit, mem, entry_data_cache,
                    mask_field, mask_val);
                /* Reset the key value if the mask value is zero */
                for (k = 0; k < word_length; k++) {
                    key_val[k] &= mask_val[k];
                }
                /* Update the key value */
                soc_mem_field_set(unit, mem, entry_data_cache,
                    key_field, key_val);
...
soc_mem_read_no_cache(int unit,
                      ...
                      void *entry_data)
{
    int flags = SOC_MEM_DONT_USE_CACHE;
    int rv;
  
    _SOC_MEM_REPLACE_MEM(unit, mem);
    
    MEM_LOCK(unit, mem);
    rv = soc_mem_array_read_flags(unit, mem, 0, copyno, 
...
soc_mem_array_read(int unit, soc_mem_t mem, unsigned array_index, int copyno, 
                   int index, void *entry_data)
{
    /* Use user defined callbak for access */
    if(SOC_INFO(unit).reg_access.mem_read) {
        return SOC_INFO(unit).reg_access.mem_read(unit, mem, array_index, copyno, index, entry_data);
    }
    ...
    if (SOC_IS_TRIDENT(unit) || SOC_IS_TITAN(unit)) {
        ...
        if (mem == EGR_VLANm) {
            ...
            SOC_IF_ERROR_RETURN
                (_soc_mem_read(unit, 0, mem, array_index, copyno, index, entry_data_y));
            soc_trident_pipe_select(unit, TRUE, 0); /* X pipe */
            SOC_IF_ERROR_RETURN
                (_soc_mem_read(unit, 0, mem, array_index, copyno, index, entry_data));
...
_retry_op:
...
#else
                sal_memcpy(cache + mem_array_cache_offset + index * entry_dw , entry_data, entry_dw * 4);
#endif /* BCM_TRIDENT_SUPPORT || BCM_SHADOW_SUPPORT */
    ...
    if (SOC_IS_TRIDENT2(unit)) {
        if (mem == MMU_INTFI_XPIPE_FC_MAP_TBL0m || 
            ...
            mem == MMU_INTFI_YPIPE_FC_MAP_TBL0m ||
            mem == MMU_INTFI_YPIPE_FC_MAP_TBL1m){
            rv = soc_trident2_fc_map_shadow_entry_set(unit, mem, index, entry_data);
            goto done;
        }
    }
#endif
...
#ifdef BCM_HURRICANE2_SUPPORT
STATIC int
_soc_mem_parity_generate(int unit, soc_mem_t mem, void *entry_data)
{
...
{
    _SOC_MEM_REPLACE_MEM(unit, mem);
    return soc_mem_array_write_extended(unit, flags, mem, 0, copyno,
                                        index, entry_data);
}
              ...
              int index, void *entry_data)
{
    ...
    if (SOC_IS_HURRICANE2(unit)) {
        if ((mem == MMU_IPMC_VLAN_TBLm) || (mem == MMU_IPMC_GROUP_TBL0m)
            ...
            SOC_IF_ERROR_RETURN(
                _soc_mem_parity_generate(unit, mem, entry_data));
        }
    }
                          ...
                          int copyno, int index_min, int index_max, void *buffer)
{
    ...
    if (SOC_IS_ARAD(unit)){
        int copyno_override;
        if ((copyno == COPYNO_ALL || copyno == SOC_CORE_ALL) && (copyno_override = soc_arad_mem_broadcast_block_get(unit,mem))){
                              ...
                              then the buffer must be allocated using soc_cm_salloc(). */
{
    ...
    LOG_INFO(BSL_LS_SOC_SOCMEM, (BSL_META_U(unit,
             "%s: unit %d memory %s[%u:%u].%s [%d:%d]\n"),
              FUNCTION_NAME(), unit, SOC_MEM_UFNAME(unit, mem),
    ...
    if (soc_mem_slamable(unit, mem, copyno)) {
            ...
            if (SHARED_GOOD_START(p)  && SHARED_GOOD_END_DEBUG(p)) {
                if (soc_mem_entry_bytes(unit, mem) > p->size) {
                    LOG_WARN(BSL_LS_SOC_SOCMEM, (BSL_META_U(unit,
                             "Some problem in calling :%s::%d:Desc:%s: Size:%u: length:%d\n"),
                              FUNCTION_NAME(), __LINE__, p->description, p->size, soc_mem_entry_bytes(unit, mem)));
                }
            }
        } else {
            LOG_ERROR(BSL_LS_SOC_SOCMEM, (BSL_META_U(unit,
                      "ERROR:ATTN :%s:%d: Address:%p: probably not in shared memory region\n"),
                      FUNCTION_NAME(), __LINE__, p));
        }
#endif
...
#ifdef BCM_SBUSDMA_SUPPORT
        if (soc_feature(unit, soc_feature_sbusdma)) {
            rv = _soc_mem_array_sbusdma_write(unit, mem, min_ar_index, max_ar_index,
                   copyno, index_min, index_max, (void*)buffer, TRUE, 1);
        } else
#endif
        {
            rv = SOC_E_INTERNAL;
        ...
        if (rv >= 0) {
            ...
            if (cache != NULL && !SOC_MEM_TEST_SKIP_CACHE(unit) && !SOC_MEM_IS_ARRAY(unit, mem)) {
                for (a_ind = min_ar_index; a_ind <= max_ar_index; ++a_ind) {
                    ...
                    mem_array_cache_offset = mem_array_vmap_offset * entry_dw;
                    cache +=  mem_array_cache_offset + index_min * entry_dw;
                    for (; nof_entries_to_write > 0; --nof_entries_to_write) { /* update cache */
                        sal_memcpy(cache, buffer, bytes_to_copy);
                        cache += entry_dw;
                    }
                    for (i = index_min; i <= index_max; i++) {
                        CACHE_VMAP_SET(vmap, mem_array_vmap_offset + i);
...
_soc_mem_entry_get_key(int unit, soc_mem_t mem, void *entry, void *key)
{
...
#endif /* BCM_ISM_SUPPORT */
    {
        ...
        if (SOC_MEM_FIELD_VALID(unit, mem, KEYf)) {
            ...
            LOG_ERROR(BSL_LS_SOC_SOCMEM,
                      (BSL_META_U(unit,
                                  "Unit %d: Unable to retreive key for %s.\n"),
                       unit, SOC_MEM_NAME(unit, mem)));
        }
    }
}
                             ...
                             int32 bank, void *entry)
{
    ...
    case HASH_DUAL:
        ...
        for (bank_idx = 0; bank_idx < 2; bank_idx++) {
            ...
            hash_base = _soc_mem_dual_hash_get(unit, mem, hash_sel, entry);
            if (hash_base == -1) {
                return SOC_E_INTERNAL;
            }
    ...
    case HASH_SHARED:
#if defined(BCM_TRIDENT2_SUPPORT)
        SOC_IF_ERROR_RETURN(soc_trident2_hash_bank_count_get(unit, mem, &num_banks));
        for (bank_idx = 0; bank_idx < num_banks; bank_idx++) {
            SOC_IF_ERROR_RETURN
                (soc_trident2_hash_bank_number_get(unit, mem, bank_idx,
                                                   &bank_ids[bank_idx]));
            SOC_IF_ERROR_RETURN
                (soc_hash_bucket_get(unit, mem, bank_ids[bank_idx], entry, &bucket));
                        ...
                        SHR_BITDCL *bucket_trace, int recurse_depth)
{
    ...
    if (((SOC_IS_TRIUMPH(unit) || SOC_IS_VALKYRIE(unit) ||
         ...
         (mem == EGR_VLAN_XLATEm)) ||
         soc_feature(unit, soc_feature_dual_hash_no_bank)) {
                          ...
                          int recurse_depth)
{
    ...
    if (NULL == bucket_trace) {
        ...
        /* For simplicity, allocate same/max number of bits for 
         * buckets for all banks */
        numb = 65536;
        /* Keep back trace of all buckets affected by recursion. */
    ...
    if (banks != SOC_MEM_HASH_BANK_ALL) {
        SOC_IF_ERROR_RETURN
            (soc_trident2_hash_seq_number_get(unit, mem, banks, &banks_seq_num));
    } else {
 ...
 *    *bank_ids  -  mapping for logical bank id to phyiscal bank id for insert mem
 *    *bank_bmp  -  phyiscal bank id bitmap for this kind of mem table
 * Notes:
 *    None.
 */
...
_soc_mem_shared_hash_init(int unit, soc_mem_t mem, 
                          uint8 *num_ent, uint32 *num_bkt, SHR_BITDCL **trace,
                          int *num_banks, int *bank_ids, int *bank_bmp)
{
    uint8 bix;
    uint32 trace_size;
    ...
    *num_bkt = 65536;
    /* Keep back trace of all buckets affected by recursion. */
    trace_size = SHR_BITALLOCSIZE(*num_bkt * 10);
    /* For simplicity, allocate same number of bits for buckets per bank */
    bkt_trace = sal_alloc(trace_size, "Shared hash");
    if (NULL == bkt_trace) {
        return SOC_E_MEMORY;
    }

    sal_memset(bkt_trace, 0, trace_size);

    /* return trace buffer pointer */
    *trace = bkt_trace;
                                  ...
                                  int recurse, int recurse_depth)
{
    static int mc = 0, ic = 0;
    static SHR_BITDCL *trace;
    static int num_banks, bank_ids[7], bank_bmp = 0;
    static uint8 num_ent = 0;
    ...
    if (NULL == bucket_trace) {
        SOC_IF_ERROR_RETURN(
            _soc_mem_shared_hash_init(unit, mem, &num_ent, &numb, &trace,
                                      &num_banks, bank_ids, &bank_bmp));
    }
    if (banks != SOC_MEM_HASH_BANK_ALL) {
        rv = soc_trident2_hash_seq_number_get(unit, mem, banks, &banks_seq_num);
        if (SOC_FAILURE(rv)) {
            goto clean_up;
        }
    } else {
    ...
    for (bix = 0; bix < num_banks; bix++) {
        ...
        * The dbs only used in breadth iteration loop */
        ...
        if (!recurse) {
            ...
            if (SHR_BITGET(trace, (numb * cbix) + bucket)) {
                continue;
            }
            SHR_BITSET(trace, (numb * cbix) + bucket);
        }
                            ...
                            int recurse_depth)
{
    ...
    /* Time to shuffle the entries */
    soc_mem_lock(unit, mem);
    if (SOC_MULTI_HASH_MOVE_ALGORITHM(unit) == MULTI_HASH_MOVE_ALGO_BREADTH) {
 ...
 *
 * @purpose - Map logical to phyiscal index
 */
