...
soc_cmicm_intr(void *_unit)
{
    ...
    sal_time_t time_start = 0, time_stop = 0;
#endif
...
#ifdef BSL_TRACE_INCLUDE
    time_stop = sal_time_usecs();
    if(time_stop > time_start) {
        if ((time_stop - time_start) > 500000) {
            LOG_DEBUG(BSL_LS_SOC_TRACE,
                      (BSL_META_U(unit,
                                  "Intr Processing Took %ul usecs\n"),
                       (uint32)(time_stop - time_start)));
        }
    }
#endif
