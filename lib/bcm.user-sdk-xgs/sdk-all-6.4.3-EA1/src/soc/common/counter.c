                        ...
                        int ar_idx, uint64 val)
{
    ...
    soc_info_t *si = &SOC_INFO(unit);
    soc_control_t *soc = SOC_CONTROL(unit);
    soc_mem_t mem = soc->counter_non_dma[id - SOC_COUNTER_NON_DMA_START].mem;
    if ((ctrl_info.instance_type == SOC_CTR_INSTANCE_TYPE_PIPE) &&
...
soc_petra_port_counter_get(int unit, soc_port_t port, int ctr_type, uint64 *value)
{
    ...
    rv = MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_port_to_interface_map_get, (unit, port, 0, &if_id, NULL));
                                    ...
                                    soc_reg_t ctr_reg)
{
    ...
    int index_max = 0, pipe, entries_per_pipe;
