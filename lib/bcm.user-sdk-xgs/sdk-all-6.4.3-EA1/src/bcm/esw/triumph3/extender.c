 ...
 */
STATIC int
_bcm_tr3_extender_failover_add(int unit, bcm_extender_port_t *extender_port)
{
    int rv = BCM_E_NONE;
    ing_dvp_table_entry_t dvp_entry;
    int vp, nh_index, failover_vp = -1, failover_nh_index = 0;
    ...
    vp = BCM_GPORT_EXTENDER_PORT_ID_GET(ep->extender_port_id);
                         ...
                         bcm_extender_port_t *extender_port)
{
    ...
    if (!(extender_port->flags & BCM_EXTENDER_PORT_REPLACE)) {
        ...
        if (BCM_SUCCESS(_bcm_esw_failover_extender_check(unit, 
                                                         extender_port))) {
            rv = _bcm_tr3_extender_failover_add(unit, extender_port);
            if (rv < 0) {
                goto cleanup;
            }
        }
        ...
        if (BCM_SUCCESS(_bcm_esw_failover_extender_check(unit, 
                                                         extender_port))) {
            BCM_IF_ERROR_RETURN
                (_bcm_tr3_extender_failover_add(unit, extender_port));
        } else {
