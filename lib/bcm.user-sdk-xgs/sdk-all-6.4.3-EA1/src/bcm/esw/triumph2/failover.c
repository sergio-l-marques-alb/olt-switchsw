...
int
bcm_tr2_failover_ecmp_prot_nhi_set(int unit, int ecmp, int nh_index, 
                                   uint32 prot_nh_index, 
    ...
    for (i = 0; i < count; i++) {
        ...
        if (nhi == nh_index) {
            soc_mem_field32_set(unit, INITIAL_L3_ECMPm, hw_buf, 
                                PROT_NEXT_HOP_INDEXf, prot_nh_index);
            if (failover_id > 0 && 
                failover_id < soc_mem_index_count(unit, 
...
int
bcm_tr2_failover_ecmp_prot_nhi_get(int unit, int ecmp, int nh_index, 
                                   bcm_failover_t *failover_id,
                                   int *prot_nh_index)
{
    int i, base_ptr, count, nhi;
...
int
bcm_tr2_failover_ecmp_prot_nhi_cleanup(int unit, int ecmp, int nh_index)
{
    int i, base_ptr, count, nhi;
    uint32 hw_buf[SOC_MAX_MEM_WORDS]; /* Buffer to read hw entry. */
