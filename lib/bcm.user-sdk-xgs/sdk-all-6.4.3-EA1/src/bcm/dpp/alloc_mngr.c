...
#define _DPP_AM_TEMPLATE_OAM_MEP_PROFILE_NON_ACC_MAX_ENTITIES (SOC_PPD_OAM_MAX_NUMBER_OF_LOCAL_MEPS(unit)) 
...
#define _DPP_AM_TEMPLATE_BFD_IPV4_DIP_PROFILE_SIZE (sizeof(SOC_SAND_PP_IPV4_ADDRESS))
...
_bcm_dpp_pp_resource_setup(int unit, int res_id, int core_id, int pool_id, bcm_dpp_am_pool_info_t *p_info) {
    ...
    switch (res_id) {
        ...
        case dpp_am_res_fec_global:
            ...
            p_info->count = SOC_DPP_CONFIG(unit)->l3.fec_db_size - SOC_DPP_CONFIG(unit)->l3.fec_ecmp_reserved; /* exclude reserved for ECMP */
    ...
    break;
        ...
        case dpp_am_res_reassembly_context:
            p_info->res_type = SHR_RES_ALLOCATOR_BITMAP;
            p_info->start = 0;
            p_info->count =  SOC_DPP_DEFS_GET(unit, num_of_reassembly_context);
            p_info->max_elements_per_allocation = 1;
            desc_str = "reassembly context";
            break;
    ...
    SOC_PPD_FEC_ID *fec_id)
{
    ...
    if(usage!= 1) {
        if(!(flags & BCM_DPP_AM_FLAG_ALLOC_GLOBAL)) {/* only if not ECMP */
            if (size == 1) {
                rv = bcm_dpp_am_fec_global_alloc(unit, flags, size, align, &elem);
                BCMDNX_IF_ERR_EXIT(rv);
        ...
        if(flags & BCM_DPP_AM_FLAG_ALLOC_GLOBAL) {
            align = TRUE;
            rv = bcm_dpp_am_fec_global_alloc(unit, flags &(~BCM_DPP_AM_FLAG_ALLOC_GLOBAL), size, align, &elem);
            BCMDNX_IF_ERR_EXIT(rv);
                            ...
                            int *elem)                        
{
    ...
    if (!aligned) {
        rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_fec_global,flags,size,elem);
        BCMDNX_IF_ERR_EXIT(rv);
        
    }
    else {
        rv = dpp_am_res_alloc_align(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_fec_global,flags,2, 0, size, elem);
        BCMDNX_IF_ERR_EXIT(rv);
    }
    ...
    SOC_PPD_FEC_ID fec_id)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_fec_global,size,fec_id);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    SOC_PPD_FEC_ID fec_id)
{
    int rv;
    BCMDNX_INIT_FUNC_DEFS;
    rv = dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_fec_global,size,fec_id);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    uint32 *context)
{
    ...
    rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_res_reassembly_context, alloc_flags, 1, (int*)context);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    uint32 context)
{
    ...
    rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_res_reassembly_context, 1, (int)context);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    case dpp_am_template_egress_interface_unicast_thresh:
        ...
        case dpp_am_template_bfd_mep_profile_non_accelerated:
            if (SOC_IS_ARAD(unit)) {
                t_info->manager = SHR_TEMPLATE_MANAGE_HASH;
                t_info->start = _DPP_AM_TEMPLATE_OAM_MEP_PROFILE_NON_ACC_LOW_ID;
                t_info->count = _DPP_AM_TEMPLATE_OAM_MEP_PROFILE_NON_ACC_COUNT;
                t_info->max_entities = _DPP_AM_TEMPLATE_OAM_MEP_PROFILE_NON_ACC_MAX_ENTITIES;
                t_info->data_size = _DPP_AM_TEMPLATE_OAM_MEP_PROFILE_NON_ACC_SIZE;
                           t_info->hashExtra.orig_data_size = t_info->data_size;
...
int _bcm_dpp_am_template_oam_mep_profile_mapping_get(int unit, int mep_id, int is_accelerated, int *profile)
{
    int rc = BCM_E_NONE;
    ...
    if (found) {
        *profile = is_accelerated ? mep_info.acc_profile : mep_info.non_acc_profile;
    }
    else {
...
int _bcm_dpp_am_template_oam_mep_profile_data_get(int unit, int mep_id, _dpp_am_template_t template_type, int is_accelerated, SOC_PPD_OAM_LIF_PROFILE_DATA* data)
{
    int old_profile;
    int rc = BCM_E_NONE;

    BCMDNX_INIT_FUNC_DEFS;
    rc = _bcm_dpp_am_template_oam_mep_profile_mapping_get(unit, mep_id, is_accelerated, &old_profile);
    BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_oam_mep_profile_exchange(int unit, _dpp_am_template_t template_type, int is_accelerated, int flags, int mep_id, SOC_PPD_OAM_LIF_PROFILE_DATA * data,
...
int _bcm_dpp_am_template_oam_mep_profile_accelerated_data_get(int unit, int mep_id, SOC_PPD_OAM_LIF_PROFILE_DATA* data)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = _bcm_dpp_am_template_oam_mep_profile_data_get(unit, mep_id, dpp_am_template_oam_mep_profile_accelerated, 1, data);
    BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_oam_mep_profile_accelerated_mapping_get(int unit, int mep_id, int *profile)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = _bcm_dpp_am_template_oam_mep_profile_mapping_get(unit, mep_id, 1, profile);
    BCMDNX_IF_ERR_EXIT(rc);
                                             ...
                                             int *old_profile, int *is_last, int *profile, int *is_allocated)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = _bcm_dpp_am_template_oam_mep_profile_exchange(unit, dpp_am_template_oam_mep_profile_accelerated, 1, flags, mep_id, data,
...
int _bcm_dpp_am_template_oam_mep_profile_non_accelerated_data_get(int unit, int mep_id, SOC_PPD_OAM_LIF_PROFILE_DATA* data)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = _bcm_dpp_am_template_oam_mep_profile_data_get(unit, mep_id, dpp_am_template_oam_mep_profile_non_accelerated, 0, data);
    BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_oam_mep_profile_non_accelerated_mapping_get(int unit, int mep_id, int *profile)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = _bcm_dpp_am_template_oam_mep_profile_mapping_get(unit, mep_id, 0, profile);
    BCMDNX_IF_ERR_EXIT(rc);
                                             ...
                                             int *old_profile, int *is_last, int *profile, int *is_allocated)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = _bcm_dpp_am_template_oam_mep_profile_exchange(unit, dpp_am_template_oam_mep_profile_non_accelerated, 0, flags, mep_id, data,
                                             old_profile, is_last, profile, is_allocated);
    BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_bfd_mep_profile_non_accelerated_data_get(int unit, int mep_id, SOC_PPD_OAM_LIF_PROFILE_DATA* data)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = _bcm_dpp_am_template_oam_mep_profile_data_get(unit, mep_id, dpp_am_template_bfd_mep_profile_non_accelerated, 0, data);
    BCMDNX_IF_ERR_EXIT(rc);
                                             ...
                                             int *old_profile, int *is_last, int *profile, int *is_allocated)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = _bcm_dpp_am_template_oam_mep_profile_exchange(unit, dpp_am_template_bfd_mep_profile_non_accelerated, 0, flags, mep_id, data,
                                             old_profile, is_last, profile, is_allocated);
    BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_queue_rate_cls_mapping_mode_advance_enable(int unit)
{
    ...
    int rc = BCM_E_NONE, rate_class_index;
    uint32 ref_count;
    ...
    if (queue_to_rate_class_mapping_is_simple)
    {
        for (rate_class_index = 0; rate_class_index < SOC_TMC_ITM_NOF_RATE_CLASSES; rate_class_index++) 
        {
            /* 1. get ref_count by rate_class_index - write to SW Data base*/
            rc = _bcm_dpp_am_template_queue_rate_cls_ref_get(unit, rate_class_index, &ref_count);
            BCMDNX_IF_ERR_EXIT(rc);
            BCM_SAND_IF_ERR_EXIT(arad_sw_db_tm_queue_to_rate_class_mapping_ref_count_set(unit,rate_class_index, ref_count));
            if (ref_count == 0) { /*class was not configured - set to default configuration*/
                rc =_bcm_petra_cosq_voq_rate_class_default_set(unit, rate_class_index, &rate_class_info);
                BCMDNX_IF_ERR_EXIT(rc);
            }
        }
        /* 2. destroy table*/
        rc = dpp_am_template_clear(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_queue_rate_cls);
        BCMDNX_IF_ERR_EXIT(rc);

        /*3. enable new mode*/
        BCM_SAND_IF_ERR_EXIT(arad_sw_db_tm_queue_to_rate_class_mapping_is_simple_set(unit, FALSE));
    }
exit:
    BCMDNX_FUNC_RETURN;
}
...
#ifdef BCM_ARAD_SUPPORT
    BCM_SAND_IF_ERR_EXIT(arad_sw_db_tm_queue_to_rate_class_mapping_is_simple_get(unit, &queue_to_rate_class_mapping_is_simple));
    if ((!SOC_IS_ARAD(unit)) || queue_to_rate_class_mapping_is_simple)
#endif /*BCM_ARAD_SUPPORT*/
    {
        flags = SHR_TEMPLATE_MANAGE_SET_WITH_ID;   
        template = template_init_id;
...
_bcm_dpp_am_template_egress_port_discount_cls_mapping_get(int unit, int port,int *discount_profile,SOC_TMC_PORT_HEADER_TYPE *header_type_outgoing)
{
    ...
    *discount_profile = discount_type;
    ...
    soc_sand_rv = (MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_port_header_type_get,(unit,port,&header_type_incoming,header_type_outgoing)));
    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
