...
typedef struct {
    int status[_BCM_PETRA_NOF_MAX_PP_PORTS];/*0-none, 1-bridge, 2-metro*/
} _BCM_GPORT_PORTS_INFO;

int _bcm_dpp_sw_db_hash_trill_info_destroy(int unit);
/*
 * Globals
 */
...
/* port information */
/*  - whether it used for bridging or metro*/
static _BCM_GPORT_PORTS_INFO ports_info[BCM_MAX_NUM_UNITS];
...
_bcm_dpp_gport_mgmt_sw_state_init(int                     unit)
{
    ...
    for(indx = 0 ; indx < SOC_DPP_DEFS_GET(unit, nof_pp_ports) ; ++indx) {
        ports_info[unit].status[indx] = 0;
    }
    ...
    {
        /* for Trill Route store  */
        rv = shr_htb_create(&mc_trill_route_info_db[unit],
                       _BCM_DPP_NOF_GPORTS(unit),
                       sizeof(bcm_gport_t),
                       "Trill Info");
        if( BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_PORT,
                      (BSL_META_U(unit,
                                  "error(%s) allocating Trill Route Info DB failed\n"),
                       bcm_errmsg(rv)));
            BCMDNX_IF_ERR_EXIT(rv);
        }

        shr_htb_cast_key_func_set(mc_trill_route_info_db[unit], (shr_htb_cast_key_f)_bcm_dpp_htb_cast_key_f);

        /* for Trill Src store  */
        rv = shr_htb_create(&mc_trill_root_src_db[unit],
                       _BCM_DPP_NOF_GPORTS(unit),
                       sizeof(bcm_gport_t),
                       "Trill Src Info");
        if( BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_PORT,
                      (BSL_META_U(unit,
                                  "error(%s) allocating Trill Src DB failed\n"),
                      bcm_errmsg(rv)));
            BCMDNX_IF_ERR_EXIT(rv);
        }
    
        shr_htb_cast_key_func_set(mc_trill_root_src_db[unit], (shr_htb_cast_key_f)_bcm_dpp_htb_cast_key_f);
    }
    ...
    */
    ...
    /* init OutLIF in WB SW-DB */
    BCMDNX_IF_ERR_EXIT(soc_dpp_wb_engine_init_buffer(unit, SOC_DPP_WB_ENGINE_BUFFER_OUTLIF_DB));

#ifdef BCM_ARAD_SUPPORT
    /* allocate/reset WB state */    
    rv = soc_dpp_wb_engine_state_init_gport_info(unit);   
    BCMDNX_IF_ERR_EXIT(rv);
#endif
...
_bcm_dpp_gport_mgmt_sw_state_cleanup(int                     unit)
{
    int status = 0;
    _bcm_dpp_inlif_bookkeeping_t *lif_info = INLIF_INFO(unit);
#if defined (BCM_WARM_BOOT_SUPPORT) && defined (BCM_ARAD_SUPPORT)
    uint32 soc_sand_rv;
#endif
    ...
    if(SOC_IS_PETRAB(unit))
    {
        if (mc_trill_route_info_db[unit]) {
            status = shr_htb_destroy(&mc_trill_route_info_db[unit], NULL);
            if( BCM_FAILURE(status)) {
                LOG_ERROR(BSL_LS_BCM_PORT,
                          (BSL_META_U(unit,
                                      "error(%s) freeing trill Route DB failed\n"),
                                      bcm_errmsg(status)));
            }
        }
     
        if (mc_trill_root_src_db[unit]) {
            status = shr_htb_destroy(&mc_trill_root_src_db[unit], NULL);
            if( BCM_FAILURE(status)) {
                LOG_ERROR(BSL_LS_BCM_PORT,
                          (BSL_META_U(unit,
                                      "error(%s) freeing trill Src DB failed\n"),
                                      bcm_errmsg(status)));
            }
        }
    }
...
#ifdef BCM_ARAD_SUPPORT
    soc_sand_rv = soc_dpp_wb_engine_state_deinit_gport_info(unit);
...
{
    SOC_PPD_LIF_ING_VLAN_EDIT_COMMAND_KEY       
        edit_cmd_key;
    SOC_PPD_EG_VLAN_EDIT_COMMAND_KEY
        eg_edit_cmd_key;    
    ...
    bcm_dpp_vlan_edit_profile_mapping_info_t
        ing_profile_info;
    bcm_error_t rv;
    int is_allocated, template_id, nof_tags;

    BCMDNX_INIT_FUNC_DEFS;
    soc_sand_dev_id = (unit);
    SOC_PPD_LIF_ING_VLAN_EDIT_COMMAND_KEY_clear(&edit_cmd_key);
    ...
    */
    ...
    rv = _bcm_dpp_am_template_vlan_edit_profile_mapping_dummy_allocate(unit, SHR_TEMPLATE_MANAGE_SET_WITH_ID, &ing_profile_info, &template_id, &is_allocated);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    SOC_PPD_EG_VLAN_EDIT_COMMAND_KEY_clear(&eg_edit_cmd_key);    
    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        for (eg_edit_cmd_key.tag_format = SOC_SAND_PP_ETHERNET_FRAME_VLAN_FORMAT_NONE; eg_edit_cmd_key.tag_format < SOC_SAND_PP_NOF_ETHERNET_FRAME_VLAN_FORMATS; ++eg_edit_cmd_key.tag_format) {
            SOC_PPD_EG_VLAN_EDIT_COMMAND_INFO_clear(&eg_profile_info.evec[eg_edit_cmd_key.tag_format]);
            eg_profile_info.evec[eg_edit_cmd_key.tag_format].outer_tag.vid_source = SOC_PPD_EG_VLAN_EDIT_TAG_VID_SRC_EMPTY;
            eg_profile_info.evec[eg_edit_cmd_key.tag_format].inner_tag.vid_source = SOC_PPD_EG_VLAN_EDIT_TAG_VID_SRC_EMPTY;
            eg_profile_info.evec[eg_edit_cmd_key.tag_format].outer_tag.pcp_dei_source = SOC_PPD_EG_VLAN_EDIT_TAG_PCP_DEI_NONE;
            eg_profile_info.evec[eg_edit_cmd_key.tag_format].inner_tag.pcp_dei_source = SOC_PPD_EG_VLAN_EDIT_TAG_PCP_DEI_NONE;
            eg_profile_info.evec[eg_edit_cmd_key.tag_format].tags_to_remove = 0;

            /* For Petra-B NOP action should be remove one and add for non untagged formats so Egress transmit tag will be invoked */
            if (SOC_IS_PETRAB(unit)) {
                nof_tags = SOC_SAND_PP_NOF_TAGS_IN_VLAN_FORMAT(eg_edit_cmd_key.tag_format);
                if (nof_tags > 0) {
                    eg_profile_info.evec[eg_edit_cmd_key.tag_format].tags_to_remove = 1;
                    eg_profile_info.evec[eg_edit_cmd_key.tag_format].outer_tag.vid_source = SOC_PPD_EG_VLAN_EDIT_TAG_VID_SRC_OUTER_TAG;
                    eg_profile_info.evec[eg_edit_cmd_key.tag_format].outer_tag.tpid_index = 0;
                    eg_profile_info.evec[eg_edit_cmd_key.tag_format].outer_tag.pcp_dei_source = SOC_PPC_EG_VLAN_EDIT_TAG_PCP_DEI_SRC_OUTER_TAG;
                }
            }
        ...
        if (SOC_IS_ARAD(unit)) {
            ...
            eg_profile_info.evec[eg_edit_cmd_key.tag_format].outer_tag.vid_source = SOC_PPD_EG_VLAN_EDIT_TAG_VID_SRC_VSI;
            eg_profile_info.evec[eg_edit_cmd_key.tag_format].outer_tag.pcp_dei_source = SOC_PPD_EG_VLAN_EDIT_TAG_PCP_DEI_SRC_MAP;
            soc_sand_rv = soc_ppd_eg_vlan_edit_command_info_set(soc_sand_dev_id,&eg_edit_cmd_key,&(eg_profile_info.evec[eg_edit_cmd_key.tag_format]));
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        }
        ...
        rv = _bcm_dpp_am_template_vlan_edit_profile_eg_mapping_allocate(unit, SHR_TEMPLATE_MANAGE_SET_WITH_ID, &eg_profile_info, &eg_edit_cmd_key.edit_profile, &is_allocated);
        BCMDNX_IF_ERR_EXIT(rv);
    }
    ...
    if (!SOC_WARM_BOOT(unit)) {        
        ...
        for (eg_edit_cmd_key.tag_format = SOC_SAND_PP_ETHERNET_FRAME_VLAN_FORMAT_NONE; eg_edit_cmd_key.tag_format < SOC_SAND_PP_NOF_ETHERNET_FRAME_VLAN_FORMATS; ++eg_edit_cmd_key.tag_format) {
            eg_profile_info.evec[eg_edit_cmd_key.tag_format].inner_tag.vid_source = SOC_PPD_EG_VLAN_EDIT_TAG_VID_SRC_EMPTY;
            eg_profile_info.evec[eg_edit_cmd_key.tag_format].outer_tag.vid_source = SOC_PPD_EG_VLAN_EDIT_TAG_VID_SRC_EDIT_INFO_0;
    ...
    if (!SOC_WARM_BOOT(unit)) {        
        ...
        for (eg_edit_cmd_key.tag_format = SOC_SAND_PP_ETHERNET_FRAME_VLAN_FORMAT_NONE; eg_edit_cmd_key.tag_format < SOC_SAND_PP_NOF_ETHERNET_FRAME_VLAN_FORMATS; ++eg_edit_cmd_key.tag_format) {
            eg_profile_info.evec[eg_edit_cmd_key.tag_format].inner_tag.vid_source = SOC_PPD_EG_VLAN_EDIT_TAG_VID_SRC_EDIT_INFO_1;
            eg_profile_info.evec[eg_edit_cmd_key.tag_format].outer_tag.vid_source = SOC_PPD_EG_VLAN_EDIT_TAG_VID_SRC_EDIT_INFO_0;
            eg_profile_info.evec[eg_edit_cmd_key.tag_format].outer_tag.tpid_index = 0;
            eg_profile_info.evec[eg_edit_cmd_key.tag_format].inner_tag.tpid_index = 1;
            eg_profile_info.evec[eg_edit_cmd_key.tag_format].inner_tag.pcp_dei_source = SOC_PPD_EG_VLAN_EDIT_TAG_PCP_DEI_SRC_MAP;
            eg_profile_info.evec[eg_edit_cmd_key.tag_format].outer_tag.pcp_dei_source = SOC_PPD_EG_VLAN_EDIT_TAG_PCP_DEI_SRC_MAP;
            eg_profile_info.evec[eg_edit_cmd_key.tag_format].tags_to_remove = SOC_SAND_PP_NOF_TAGS_IN_VLAN_FORMAT(eg_edit_cmd_key.tag_format);
...
int
_bcm_dpp_gport_port_status_set(int unit, uint32 local_port, int status)
{
    BCMDNX_INIT_FUNC_DEFS;
    if(local_port >= SOC_DPP_DEFS_GET(unit, nof_pp_ports)) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Invalid local port")));
    }
    ports_info[unit].status[local_port] = status;
...
int
_bcm_dpp_gport_port_status_get(int unit, uint32 local_port, int *status)
{
    BCMDNX_INIT_FUNC_DEFS;
    if(local_port >= SOC_DPP_DEFS_GET(unit, nof_pp_ports)) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Invalid local port")));
    }
    *status = ports_info[unit].status[local_port];
...
_bcm_dpp_out_lif_match_delete(int unit, int out_lif)
{
    ...
    BCMDNX_IF_ERR_EXIT(SOC_DPP_WB_ENGINE_SET_ARR(unit, SOC_DPP_WB_ENGINE_VAR_OUTLIF_INFO, &new_match, out_lif));
...
_bcm_dpp_lif_match_valid(int unit, int *in_lif, int *out_lif)
{
    ...
    if (out_lif) {
        rv = SOC_DPP_WB_ENGINE_GET_ARR(unit, SOC_DPP_WB_ENGINE_VAR_OUTLIF_INFO, &outlif_match, *out_lif);
        if (rv != BCM_E_NONE) {
            return FALSE;
        }
...
_bcm_dpp_out_lif_mpls_match_add(int unit, bcm_mpls_port_t *mpls_port, int lif)
{
    ...
    BCMDNX_IF_ERR_EXIT(SOC_DPP_WB_ENGINE_SET_ARR(unit, SOC_DPP_WB_ENGINE_VAR_OUTLIF_INFO, &new_match, lif));
 ...
 * Purpose:
 *      Retrieve the usage type of a LIF. For InLIF, the value is retrieved
 *      from the Ingress SW DB. For OutLIF (intended for Egress only LIFs),
    ...
    if (out_lif_usage) {
        *out_lif_usage = _bcmDppLifTypeAny;
        if (out_lif != _BCM_GPORT_ENCAP_ID_LIF_INVALID) {
            BCMDNX_IF_ERR_EXIT(SOC_DPP_WB_ENGINE_GET_ARR(unit, SOC_DPP_WB_ENGINE_VAR_OUTLIF_INFO, &outlif_match, out_lif));
 ...
 _bcm_dpp_mod_port_to_sys_port(int unit, bcm_module_t modid, bcm_port_t mode_port, unsigned int *sys_port)
{
    ...
    *sys_port = SOC_TMC_SYS_PHYS_PORT_INVALID;/* not really needed - just to avoid coverity defect */
    ...
    else
    {
      ...
      BCM_SAND_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_mgmt_system_fap_id_get,(unit, &local_modid)));
      ...
      if(modid == local_modid) {
          BCMDNX_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_local_to_tm_port_get, (unit, mode_port, &port, &core)));
      } else {
          port = mode_port;
      }
...
_bcm_dpp_gport_to_phy_port(int unit, bcm_gport_t port, uint32  operations, _bcm_dpp_gport_info_t* gport_info)
{
    ...
    if(BCM_GPORT_IS_LOCAL(gport)){ /* local-port*/
        ...
        if(modid == my_modid) {
            ...
            BCM_PBMP_PORT_ADD(gport_info->pbmp_local_ports, mod_port);
        }
        ...
        /* map sys port to fap and tm-port*/
        soc_sand_rv = (MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_sys_phys_to_local_port_map_get,(unit,gport_info->sys_port, (uint32*)&modid, &local_port)));
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        ...
        soc_sand_rv = (MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_ipq_queue_to_flow_mapping_get,(unit,(queue_id/4), &queue_map_info)));
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        ...
        if(operations & _BCM_DPP_GPORT_TO_PHY_OP_RETRIVE_SYS_PORT){
         ...
         BCM_PBMP_PORT_ADD(gport_info->pbmp_local_ports, gport_info->local_port);
         gport_info->flags |= _BCM_DPP_GPORT_INFO_F_IS_LOCAL_PORT;
         gport_info->flags |= _BCM_DPP_GPORT_INFO_F_IS_PER_LANE;
    }
    /* try to resolve logical gport to phy-port */
    else {
        /* map gport to FEC/port */
        rv = _bcm_dpp_gport_parse(unit, gport, &gport_type, &gport_val, NULL);
        BCMDNX_IF_ERR_EXIT(rv);

        /* if FEC, get physical port from the FEC */
        if (gport_type == _BCM_DPP_GPORT_IN_TYPE_FEC) {
            /* get dest from FEC*/
            soc_sand_rv = soc_ppd_frwrd_fec_entry_get(unit,gport_val,&protect_type,&fec_entry[0],&fec_entry[1],&protect_info);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        ...
        else if (gport_type == _BCM_DPP_GPORT_TYPE_SIMPLE) { /* simple gport*/
            ...
            if (BCM_GPORT_IS_VLAN_PORT(gport) && (gport_type == _BCM_DPP_GPORT_TYPE_ENCAP) && SOC_DPP_ENCAP_ID_IS_INGRESS_ONLY(gport)) {

                /* Retrieve Ingress SW DB info according to the gport parsing*/
                rv = _bcm_dpp_in_lif_ac_match_get(unit, &vlan_port, SOC_DPP_LIF_ID_FROM_ENCAP_ID(gport_val));
                BCMDNX_IF_ERR_EXIT(rv);
            ...
            /* Else, get the port information from the forwarding SW DB */
            } else {
                ...
                /* For FEC VC and Ring Protection, the physical port is retieved via
                   the FEC destination */
                if ((phy_port->type == _BCM_DPP_GPORT_IN_TYPE_FEC_VC) || (phy_port->type == _BCM_DPP_GPORT_IN_TYPE_RING)) {
                    /* get dest from FEC*/
...
/* given gport return id stored in gport var: FEC or EEP/AC
   the value stored is either FEC or EEP/AC.
   FEC: when gport is protected.
   The returned ID is LIF_INFO, not only LIF_ID.
 
   type:
   if from SW state:
     then see _BCM_DPP_GPORT_IN_XXX
   if from ID:
     - FEC
     - AC (_BCM_DPP_GPORT_TYPE_ENCAP = LIF)
     - simple
...
int _bcm_dpp_gport_parse(int unit, bcm_gport_t port, int *type, int *val, _BCM_GPORT_PHY_PORT_INFO **phy_port_p)
{
    ...
    if (BCM_GPORT_IS_SET(gport)) {
        ...
        /* This option is not in use. It assumes the Lif isn't Ingress only
           and it also return LIF_ID instead of LIF_INFO */
        if(phy_port_p != NULL) {
            /* check gport info according to stored information */
            rv = _bcm_dpp_sw_db_hash_vlan_find(unit,
                                               (&gport),
                                               (shr_htb_data_t*)(void *)&phy_port,
                                               FALSE);        
            ...
            /* information found */
            if(phy_port != NULL && phy_port->type != _BCM_DPP_GPORT_IN_TYPE_INVALID) {
                *phy_port_p = phy_port;
                *type = phy_port->type;
                *val = phy_port->phy_gport;
                BCM_EXIT;
            }
        }

        /* if fail, then try to get information from gport-id */
        *type = _BCM_DPP_GPORT_TYPE_FEC;
        if (BCM_GPORT_IS_VLAN_PORT(gport)) {
            *val = BCM_GPORT_VLAN_PORT_ID_GET(gport);
        } 
        else if (BCM_GPORT_IS_MPLS_PORT(gport)) {
            *val = BCM_GPORT_MPLS_PORT_ID_GET(gport);
        }
        else if (BCM_GPORT_IS_MIM_PORT(gport)) {
            *val = BCM_GPORT_MIM_PORT_ID_GET(gport);
        }
        else if (BCM_GPORT_IS_TRILL_PORT(gport)) {
            *val = BCM_GPORT_TRILL_PORT_ID_GET(gport);
        }
        else if (BCM_GPORT_IS_TUNNEL(gport)) { /* for IP-tunnel ID, is in/out-LIF ID */
            *val = BCM_GPORT_TUNNEL_ID_GET(gport);
            /* for ARAD, gport-id may include MC ID*/
            if(!SOC_IS_PETRAB(unit) && SOC_DPP_ENCAP_TYPE_GET(gport) == _DPP_ENCAP_ID_MC_VAL) {
                *type = _BCM_DPP_GPORT_TYPE_MC;
            }
            else{
                *type = _BCM_DPP_GPORT_TYPE_ENCAP;
            }
        }
        else if (BCM_GPORT_IS_L2GRE_PORT(gport)) { /* for IP-tunnel ID, is in/out-LIF ID */
            *val = BCM_GPORT_L2GRE_PORT_ID_GET(gport);
            *type = _BCM_DPP_GPORT_TYPE_ENCAP;
        }
        else if (BCM_GPORT_IS_VXLAN_PORT(gport)) { /* for IP-tunnel ID, is in/out-LIF ID */
            *val = BCM_GPORT_VXLAN_PORT_ID_GET(gport);
            *type = _BCM_DPP_GPORT_TYPE_ENCAP;
        }
        else if (BCM_GPORT_IS_FORWARD_PORT(gport)) {
            *val = BCM_GPORT_FORWARD_PORT_GET(gport);
        }
        else {
            *type = _BCM_DPP_GPORT_TYPE_SIMPLE;
            *val = gport;
        }

        /* now check if the gport-value respresent encap */
        if (SOC_DPP_IS_VALID_ENCAP_ID(gport)) {
            *val = SOC_DPP_LIF_FROM_ENCAP_ID(*val);
            /* it can be MC */
            if(SOC_DPP_ENCAP_TYPE_GET(gport) == _DPP_ENCAP_ID_MC_VAL) {
                *type = _BCM_DPP_GPORT_TYPE_MC;
                ...
                /* it can be forward */
                *type = _BCM_DPP_GPORT_IN_TYPE_FEC;
            }
            else {
                /* it can be encap */
                *type = _BCM_DPP_GPORT_TYPE_ENCAP;
...
{
    int gport_val;
    /*ddd _BCM_GPORT_PHY_PORT_INFO *phy_port=NULL;*/
    /*ddd SOC_PPD_FRWRD_DECISION_INFO  dummy_fwd_decsion;*/
    int rv = BCM_E_NONE;
    int gport_type;
    ...
    *is_protected = FALSE;
    ...
    if (gport_type == _BCM_DPP_GPORT_IN_TYPE_FEC) {/*FEC encap is stored in SW*/        
        *is_protected = TRUE;
    }
...
{
    int rv = BCM_E_NONE, in_lif_id, out_lif_id;
    ...
    *used_type = _bcmDppLifTypeAny;
    ...
    rv = _bcm_dpp_gport_to_lif(unit, gport, &in_lif_id, &out_lif_id, NULL, NULL);
    if (rv == BCM_E_NOT_FOUND) {
    ...
    else
    {
        ...
        *used_type = (in_lif_id != _BCM_GPORT_ENCAP_ID_LIF_INVALID) ? in_lif_usage_type : out_lif_usage_type;
    }
exit:
    BCMDNX_FUNC_RETURN;
}
...
/* 
 *  given gport return destination (FEC/phy-port) + encap-id
 *  if gport is FEC then read from FEC
 *  otherwise read from SW state (hash table)
 */
int _bcm_dpp_gport_resolve(int unit, bcm_gport_t gport, int flags, SOC_SAND_PP_DESTINATION_ID *dest_id, int *encap_id, int *gport_type)
{
    int gport_val, lif_id;
    _BCM_GPORT_PHY_PORT_INFO *phy_port=NULL;
    ...
    *encap_id = _BCM_GPORT_ENCAP_ID_LIF_INVALID;
    ...
    if (*gport_type == _BCM_DPP_GPORT_TYPE_MC ) { /* gport is MC, for 1-plus-1 */
        if(SOC_IS_PETRAB(unit)){ 
            BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("MC gport (1+1 protection) is not supported in PB")));
        }
        /* multicast destination can be resolved from ID */

        rv = _bcm_dpp_gport_plus_1_id_to_mc(gport_val,&mc, &working);
        BCMDNX_IF_ERR_EXIT(rv);
    ...
    if (*gport_type == _BCM_DPP_GPORT_TYPE_ENCAP) {
        ...
        lif_id =  SOC_DPP_LIF_ID_FROM_ENCAP_ID(gport_val);
        if (BCM_GPORT_IS_VLAN_PORT(gport) || BCM_GPORT_IS_MPLS_PORT(gport)) {
            if (SOC_DPP_ENCAP_ID_IS_INGRESS_ONLY(gport)) {
                is_ingress_only = TRUE;
            }
            if (SOC_DPP_ENCAP_ID_IS_EGRESS_ONLY(gport)) {
                is_egress_only = TRUE;
            }
        }
        ...
        if (!(flags & _BCM_DPP_GPORT_RESOLVE_FLAG_NO_CHECK)) {
            if (is_egress_only) {
                if (!_bcm_dpp_lif_match_valid(unit, NULL, &lif_id)) {
    ...
    if (*gport_type == _BCM_DPP_GPORT_IN_TYPE_FEC) {/*FEC encap is stored in SW*/
        ...
        if (BCM_GPORT_IS_TRILL_PORT(gport)) {
            ...
            *gport_type = phy_port->type;
            ...
            dest_id->dest_val = BCM_GPORT_MIM_PORT_ID_GET(gport); /* FEC */
            ...
            *gport_type = phy_port->type;
            ...
            if (phy_port->type == _BCM_DPP_GPORT_IN_TYPE_MIM_P2P) { /* P2P */
            ...
            else if (phy_port->type == _BCM_DPP_GPORT_IN_TYPE_MIM_MP) { /* MP */
            ...
            if(!(flags & _BCM_DPP_GPORT_RESOLVE_FLAG_NO_CHECK) && !_bcm_dpp_lif_match_valid(unit, encap_id, NULL)){
                BCMDNX_ERR_EXIT_MSG(BCM_E_NOT_FOUND, (_BSL_BCM_MSG("gport not found ")));
            }
    ...
    if (*gport_type == _BCM_DPP_GPORT_TYPE_SIMPLE) { /* simple gport*/
        ...
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    /* For an Ingress only gport, just use the parse resolution, destination info is invalid */
    } else if ((is_ingress_only && BCM_GPORT_IS_VLAN_PORT(gport)) || (is_egress_only && BCM_GPORT_IS_MPLS_PORT(gport))) {
        *encap_id = lif_id;
        dest_id->dest_type = -1;
    } else {  /* rest of cases: get stored information */
        ...
        if (!phy_port || BCM_FAILURE(rv)) {
            ...
            if ((phy_port->type == _BCM_DPP_GPORT_IN_TYPE_FEC_VC) || (phy_port->type == _BCM_DPP_GPORT_IN_TYPE_FEC_EEP) ||
                (phy_port->type == _BCM_DPP_GPORT_IN_TYPE_RING)) {      /*FEC encap is stored in SW*/
                dest_id->dest_type = SOC_SAND_PP_DEST_FEC;
                dest_id->dest_val = phy_port->phy_gport;
            }
            else if (phy_port->type == _BCM_DPP_GPORT_IN_TYPE_L2_GRE_EG_FEC) { /* l2gre gport is always LIF (in-tunnel)*/
                dest_id->dest_type = SOC_SAND_PP_DEST_FEC;
                dest_id->dest_val = phy_port->phy_gport;
                /* _BCM_DPP_GPORT_IN_TYPE_L2_GRE is handle in the below else */
            ...
            else if (phy_port->type == _BCM_DPP_GPORT_IN_TYPE_VXLAN_EG_FEC) { /* VXLAN gport is always LIF (in-tunnel)*/
                dest_id->dest_type = SOC_SAND_PP_DEST_FEC;
                dest_id->dest_val = phy_port->phy_gport;
                /* _BCM_DPP_GPORT_IN_TYPE_VXLAN is handle in the below else */
            ...
            else if (BCM_GPORT_IS_MPLS_PORT(gport) && (phy_port->type == _BCM_DPP_GPORT_IN_TYPE_FEC)) { /* MPLS PORT FEC */
                dest_id->dest_type = SOC_SAND_PP_DEST_FEC;
                dest_id->dest_val = phy_port->phy_gport;
...
/* 
   encap-id: presents:_BCM_DPP_GPORT_IN_TYPE_AC, encap_id = AC
   _BCM_DPP_GPORT_IN_TYPE_EEP,  PWE (EEP)
   _BCM_DPP_GPORT_IN_TYPE_FEC_VC, VC+push-profile


   _BCM_DPP_GPORT_IN_TYPE_AC, encap_id = AC
   _BCM_DPP_GPORT_IN_TYPE_EEP,  PWE (EEP)
   _BCM_DPP_GPORT_IN_TYPE_FEC_VC, VC+push-profile
 */
    }
...
#ifndef BCM_ARAD_SUPPORT
    BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("mode not supported")));
#else
...
_bcm_dpp_gport_to_fwd_decision(int unit, bcm_gport_t port, SOC_PPD_FRWRD_DECISION_INFO  *fwd_decsion)
{
    ...
    if(BCM_GPORT_IS_SCHEDULER(gport)){  /*CosQ/Flow*/
      param0 = BCM_GPORT_SCHEDULER_GET(gport);
    ...
    rv = _bcm_dpp_gport_parse(unit,gport,&gport_type, &param0, NULL);
    BCMDNX_IF_ERR_EXIT(rv);

    /* 1+1 protection */
    /* no matter what is the application if the gport has 1+1 protection then
       fwd-decision is MC-group */
    if(gport_type == _BCM_DPP_GPORT_TYPE_MC) {

        /* get MC from gport */
        param0 = SOC_DPP_MC_GROUP_FROM_ENCAP_ID(param0);
        SOC_PPD_FRWRD_DECISION_MC_GROUP_SET(unit, fwd_decsion, param0, soc_sand_rv);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        BCM_EXIT;
    }
    ...
    if(BCM_GPORT_IS_VLAN_PORT(gport)){  /*vlan_port*/
        if (gport_type == _BCM_DPP_GPORT_IN_TYPE_FEC) { /* if FEC done */
            SOC_PPD_FRWRD_DECISION_PROTECTED_AC_SET(unit, fwd_decsion, encap_id, param0, soc_sand_rv); /* encap_id is ignored */
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        }
        else {
            ...
            if ((gport_type == _BCM_DPP_GPORT_IN_TYPE_AC) ||   /* phy + AC */
                (gport_type == _BCM_DPP_GPORT_IN_TYPE_RING)) { /* FEC + AC */
                ...
                if (SOC_IS_PETRAB(unit)) {
                    (fwd_decsion)->additional_info.outlif.type = SOC_PPD_OUTLIF_ENCODE_TYPE_AC;
                }
                else{ /* arad has no encoding for outlif, has to be raw*/
                    (fwd_decsion)->additional_info.outlif.type = SOC_PPD_OUTLIF_ENCODE_TYPE_RAW;
                }
    ...
    if(BCM_GPORT_IS_MPLS_PORT(gport)){  /*mpls_port*/
        ...
        if(gport_type == _BCM_DPP_GPORT_IN_TYPE_FEC) { /* FEC: protected PWE/AC */
            SOC_PPD_FRWRD_DECISION_PROTECTED_PWE_SET(unit, fwd_decsion, dest_id.dest_val, soc_sand_rv);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        }
        else if(gport_type == _BCM_DPP_GPORT_IN_TYPE_EEP) { /* phy + pwe */
            /* set physical dest information */
            soc_sand_rv = soc_ppd_sand_dest_to_fwd_decision(unit,&dest_id,fwd_decsion);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);

            /* update oulif to include the PWE/VC */
            (fwd_decsion)->additional_info.outlif.type = SOC_PPD_OUTLIF_ENCODE_TYPE_EEP;
            (fwd_decsion)->additional_info.outlif.val = encap_id;
        }
        else if(gport_type == _BCM_DPP_GPORT_IN_TYPE_AC) { /* phy + AC */
            /* set physical dest information */
            soc_sand_rv = soc_ppd_sand_dest_to_fwd_decision(unit,&dest_id,fwd_decsion);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        ...
        else if(gport_type == _BCM_DPP_GPORT_IN_TYPE_FEC_VC) {  /*VC+FEC: pwe protected Tunnel */
            vc_label = _BCM_DPP_ENCAP_ID_VC_GET(encap_id);
            push_profile = _BCM_DPP_ENCAP_ID_PUSH_PROFILE_GET(encap_id);
            SOC_PPD_FRWRD_DECISION_PWE_PROTECTED_TUNNEL_SET(unit, fwd_decsion, vc_label, push_profile, dest_id.dest_val, soc_sand_rv);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        }
        else if(gport_type == _BCM_DPP_GPORT_IN_TYPE_FEC_EEP) { /* FEC + eep for tunnel, protected PWE */
            /* set FEC*/
            soc_sand_rv = soc_ppd_sand_dest_to_fwd_decision(unit,&dest_id,fwd_decsion);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    ...
    if (BCM_GPORT_IS_MIM_PORT(gport)) {  /* mim_port */
        ...
        if (gport_type == _BCM_DPP_GPORT_IN_TYPE_MIM_P2P) {
        ...
        else if (gport_type == _BCM_DPP_GPORT_IN_TYPE_MIM_MP || gport_type == _BCM_DPP_GPORT_IN_TYPE_FEC) {
    ...
    if(BCM_GPORT_IS_TRILL_PORT(gport)){  
        ...
        is_trill_mc = (gport_type == _BCM_DPP_GPORT_IN_TYPE_TRILL_MC);
        if (is_trill_mc) {
    ...
    if(BCM_GPORT_IS_TUNNEL(gport)){  /* ip tunnnel */
        /* parse gport to encap-id and physical destination */
        rv = _bcm_dpp_gport_resolve(unit,gport,0, &dest_id,&encap_id,&gport_type);
        BCMDNX_IF_ERR_EXIT(rv);
        /* found fec then forwarding decision is FEC */
        if(gport_type == _BCM_DPP_GPORT_IN_TYPE_FEC) {
            SOC_PPD_FRWRD_DECISION_PROTECTED_AC_SET(unit, fwd_decsion, encap_id, dest_id.dest_val, soc_sand_rv);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        }
        else if(gport_type == _BCM_DPP_GPORT_IN_TYPE_IP_TUNNEL) {/* phy + AC */
            /* set physical dest information */
            soc_sand_rv = soc_ppd_sand_dest_to_fwd_decision(unit,&dest_id,fwd_decsion);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
            /* update oulif to include the AC */
            ...
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Tunnel gport does not include enough information"))); /* vlan-port doesn't include relevant/enough information */
        }
        BCM_EXIT;
     }
    ...
    if(BCM_GPORT_IS_L2GRE_PORT(gport)){  
        ...
        /* FEC */
        if(gport_type == _BCM_DPP_GPORT_IN_TYPE_L2_GRE_EG_FEC) {
            SOC_PPD_FRWRD_DECISION_FEC_SET(unit, fwd_decsion, dest_id.dest_val, soc_sand_rv);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        }
        else{ /* port + outlif */
    ...
    if(BCM_GPORT_IS_VXLAN_PORT(gport)){  
        ...
        /* FEC */
        if(gport_type == _BCM_DPP_GPORT_IN_TYPE_VXLAN_EG_FEC) {
            SOC_PPD_FRWRD_DECISION_FEC_SET(unit, fwd_decsion, dest_id.dest_val, soc_sand_rv);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        }
        else{ /* port + outlif */
    ...
    if (BCM_GPORT_IS_FORWARD_PORT(gport)) {
        SOC_PPD_FRWRD_DECISION_FEC_SET(unit, fwd_decsion, param0, soc_sand_rv);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    }
       
    else{   /* not supported. if still relevant, should be handled before calling this API*/
...
_bcm_dpp_gport_to_tm_dest_info(int unit, bcm_gport_t port, SOC_TMC_DEST_INFO  *soc_petra_dest_info)
{
    ...
    if(BCM_GPORT_IS_LOCAL(gport)){ /* local-port*/
        ...
        if (_mode_port == BCM_DPP_PORT_INTERNAL_ERP(0)) { /* ERP special case */
          rv = _bcm_dpp_mod_port_to_sys_port(unit,_modid,_mode_port,&_sys_phy_port_id);
          BCMDNX_IF_ERR_EXIT(rv);          
        } else {
          BCMDNX_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_local_to_tm_port_get, (unit, _mode_port, &tm_port, &core)));
...
int _bcm_dpp_l2_gport_fill_fec(int unit, void *gport, int gport_type, SOC_PPD_FEC_ID fec_id, SOC_PPD_FRWRD_FEC_ENTRY_INFO *work_fec){
    ...
    if (is_work) {
        ...
        sec_fec = 1;
        /* rv = bcm_dpp_am_fec_alloc(unit,BCM_DPP_AM_FLAG_ALLOC_GLOBAL|BCM_DPP_AM_FLAG_ALLOC_WITH_ID,0,1,&work_fec_id);*/
        /* BCMDNX_IF_ERR_EXIT(rv);*/
    }
    ...
    if (gport_type == BCM_GPORT_VLAN_PORT) {
        ...
        if (SOC_IS_PETRAB(unit)) {
            fec_entry[0].type = SOC_PPD_FEC_TYPE_BRIDGING_WITH_AC;
            fec_entry[0].app_info.out_ac_id = fec_encap;
        }
        else{/* ARAD FEC type is tunnel (or forward when using out-lif profile) */
...
int _bcm_gport_vlan_is_valid_replace(int unit, bcm_vlan_port_t *new_vlan_port, bcm_vlan_port_t *old_vlan_port){
    ...
    if (((old_vlan_port->flags & BCM_VLAN_PORT_VSI_BASE_VID ) ||
         ...
         return BCM_E_UNAVAIL; /* cannot update if assigment mode is change*/
    }
    ...
    if(!n_is_protected && (new_vlan_port->flags & BCM_VLAN_PORT_ENCAP_WITH_ID)) {
        if(new_vlan_port->encap_id != old_vlan_port->encap_id) {
            return BCM_E_UNAVAIL; /* cannot change encap if not protected as it part of ID*/
        }
    }
...
int _bcm_gport_mpls_is_valid_replace(int unit, bcm_mpls_port_t *new_mpls_port, bcm_mpls_port_t *old_mpls_port, SOC_PPC_EG_ENCAP_PWE_INFO *old_pwe_info){
    ...
    o_is_protected = _BCM_PPD_IS_VALID_FAILOVER_ID(new_mpls_port->failover_id);
    n_is_protected = _BCM_PPD_IS_VALID_FAILOVER_ID(old_mpls_port->failover_id);
    ...
    /* egress label can be modified only in case PWE is protected */
    if ((!n_is_protected) && (new_mpls_port->egress_label.label != old_pwe_info->label)) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("cannot change egress label if PWE is not protected")));
    }
    ...
    if ((!n_is_protected) && ((new_push_profile != old_pwe_info->push_profile) || (rv != BCM_E_NONE))) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("cannot change push profile if PWE is not protected")));
    }
...
int l2_interface_calculate_act_fec(void *gport, int *act_fec, int gport_type){
    ...
    is_work = (gport_type == BCM_GPORT_VLAN_PORT) ? _BCM_PPD_GPORT_IS_WORKING_PORT_ID(vlan_port->vlan_port_id) : _BCM_PPD_GPORT_IS_WORKING_PORT_ID(mpls_port->mpls_port_id);
...
l2_interface_calculate_port_id(int unit, int act_fec, SOC_PPD_LIF_ID lif_id, void *gport, int gport_type){
    ...
    int is_protected;
    uint8 is_forward_group, is_mc_ingress;
    uint8 lif_gport;
    ...
    if((gport_type != BCM_GPORT_VLAN_PORT) && (gport_type != BCM_GPORT_MPLS_PORT)) {
    ...
    BCMDNX_IF_ERR_EXIT(rv);
    rv = l2_interface_protection_validation(unit, gport, FALSE, &is_protected, NULL, &is_mc_ingress, NULL, gport_type);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    if (   ((is_forward_group || is_protected) && !act_fec)      /* Fec gport but no fec */
        ...
        || (lif_gport && !lif_id)                               /* Lif gport but no lif_id */
 ...
 * Purpose:
 *      When creating a vlan/mpls port WITH_ID (not ENCAP_WITH_ID), use this function to get the used lif. If port is protected or mc_ingress,
 *      the vlan_port_id will stand for fec and mc_group respectivly, and not for lif.
 * Parameters: 
 *      gport               : (IN) the vlan/mpls_port->vlan/mpls_port_id 
 *      is_protected        : (IN) is the port 1:1 protected?
 *      is_mc_ingress       : (IN) is the port 1+1 prtected?
 *      lif_id              : (OUT) the final lif_id. If port id is not based on lif (protected) will be 0.
 *      gport_type          : (IN) port type (MPLS/VLAN) 
 * Returns: 
 *      BCM_E_NONE
 */
int l2_interface_get_lif_from_port_id(void *gport, uint8 is_protected, uint8 is_mc_ingress, SOC_PPD_LIF_ID *lif_id, int gport_type){
    ...
    *lif_id = (gport_type==BCM_GPORT_VLAN_PORT) ? BCM_GPORT_VLAN_PORT_ID_GET(vlan_port->vlan_port_id) : BCM_GPORT_MPLS_PORT_ID_GET(mpls_port->mpls_port_id);
 ...
 * Returns:                
 *      BCM_E_PARAM : If validate is on and either:
 *                      * Chip is PetraB and 1+1 protection is on.
 ...
 *      BCM_E_NONE
 */
int l2_interface_protection_validation(int unit, void *gport, uint8 validate, int *is_1_to_1, uint8 *is_ingress_protected, 
    ...
    if (gport_type == BCM_GPORT_MPLS_PORT) {
        ...
        DPP_IF_NOT_NULL_FILL(is_1_to_1 ,_BCM_PPD_IS_VALID_FAILOVER_ID(mpls_port->failover_id) && !(DPP_VALUE_IF_NOT_NULL(is_mc_ingress)));
    }

    /* Perform all type of Protection validations, if required */
    if (validate) {
        if(SOC_IS_PETRAB(unit) && DPP_VALUE_IF_NOT_NULL(is_1_plus_1)){
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("1_plus_1 protection is not supported by Soc_petra-B")));
        }
...
{
    _BCM_GPORT_PHY_PORT_INFO *phy_port = NULL;
    SOC_PPD_FEC_ID fec_id=0;
    SOC_PPD_LIF_ID lif_id=0, out_ac_id=0, out_lif_id=0;
    int rv = BCM_E_NONE;
    int lif_flags = 0, out_ac_flags = 0;
    uint32 local_port;
    _bcm_dpp_gport_info_t gport_info;
    _bcm_lif_type_e used_type=_bcmDppLifTypeAny;
    int is_protected,is_failover_port_valid;
    int fec_flags=0, act_fec=0, learn_gport;
    ...
    uint32 lif_id_o;
    uint8 found,is_1_plus_1, is_ingress_protected=0,is_mc_ingress=0, is_1_plus_1_o = 0;
    ...
    SOC_PPD_EG_AC_VBP_KEY
        ac_port_vsi_key,
        ac_port_vsi_key_o;
    SOC_PPD_EG_AC_CEP_PORT_KEY
        ac_port_cep_key,
        ac_port_cep_key_o;
    int remote_lif = 0;
    int encap_id_val;
    ...
    /* Verify */
    if (SOC_IS_PETRAB(unit) && vlan_port->flags & BCM_VLAN_PORT_TRANSPARENT) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Invalid flags 0x%x"),vlan_port->flags));
    }
    ...
    default:
        if (SOC_IS_PETRAB(unit) && vlan_port->criteria == BCM_VLAN_PORT_MATCH_PORT_INITIAL_VLAN) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Invalid match criteria")));
        }
        break;
    }
	...
	if(soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "evc_filter", 0) == 0)
	{
		/* In case of EVC Filter, this will not be a valid tunnel ID */
		/* Therefore, only check this value if EVC is disabled.		 */
		BCM_DPP_EGR_TUNNEL_CHK_ID(unit,vlan_port->egress_tunnel_value);
	}
    ...
    rv = l2_interface_protection_validation(unit, vlan_port, TRUE, &is_protected, &is_ingress_protected, &is_mc_ingress, &is_1_plus_1, BCM_GPORT_VLAN_PORT);
    BCMDNX_IF_ERR_EXIT(rv);

    /*
     * manipulate 3 tags with PON application. 
     */
    if ((is_tunnel_matched) && (vlan_port->egress_vlan != 0) && (vlan_port->egress_inner_vlan != 0)) {
     ...
     */
    rv = l2_interface_ingress_egress_only_validation(vlan_port, TRUE, &is_ingress, &is_egress, BCM_GPORT_VLAN_PORT);
    BCMDNX_IF_ERR_EXIT(rv);

    /* Verify VSI assignment mode */
    /*  If VSI assignment mode is vid plu vsi base than vlan_port->vsi have to be some vsi base value and vlan_port->criteria have to be match port */
    if (vlan_port->flags & BCM_VLAN_PORT_VSI_BASE_VID)
    {
        if (is_egress) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("vsi assignment mode vsi_base+vid, supported in ingress mode only")));
        }
        if ((_BCM_DPP_VSI_USE_AS_ASSIGNMENT_MODE(vlan_port->vsi) || !SOC_IS_ARAD(unit) || (vlan_port->criteria != BCM_VLAN_PORT_MATCH_PORT))) 
        {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("In case of vsi assignment mode is vsi_base+vid, vsi has to be set and match criteria has to be port match")));
        }
    }
    ...
    */
    rv = _bcm_dpp_gport_to_phy_port(unit,vlan_port->port, 0, &gport_info); 
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    /* Get the gport usage state. The Encap flag should be tested first as it
       holds the LIF whenever set. The returned value isn't checked as the functions are allowed to
       fail for unallocated LIFs and the used_type value will still be correct. */
    if (encap_with_id) {
        if(!BCM_ENCAP_REMOTE_GET(vlan_port->encap_id)) {
            encap_id_val = BCM_ENCAP_ID_GET(vlan_port->encap_id);
            if (is_ingress) {
                rv = _bcm_dpp_lif_usage_get(unit, encap_id_val, _BCM_GPORT_ENCAP_ID_LIF_INVALID, &used_type, NULL);
            } else {
                rv = _bcm_dpp_lif_usage_get(unit, _BCM_GPORT_ENCAP_ID_LIF_INVALID, encap_id_val, NULL, &used_type);
            }
        }
        if (rv != BCM_E_NOT_FOUND) {
            BCMDNX_IF_ERR_EXIT(rv);
        ...
        rv = l2_interface_get_lif_from_port_id(vlan_port, is_protected, is_mc_ingress, &lif_id, BCM_GPORT_VLAN_PORT);
        BCMDNX_IF_ERR_EXIT(rv);

        if (is_protected) {
            rv = l2_interface_calculate_act_fec(vlan_port, &act_fec, BCM_GPORT_VLAN_PORT);
            BCMDNX_IF_ERR_EXIT(rv);
        } 
        ...
        rv = l2_interface_calculate_port_id(unit, act_fec, lif_id, vlan_port, BCM_GPORT_VLAN_PORT);
        BCMDNX_IF_ERR_EXIT(rv);

        rv = _bcm_dpp_gport_get_used_type(unit, vlan_port->vlan_port_id, &used_type);
        if (rv != BCM_E_NOT_FOUND) {
            BCMDNX_IF_ERR_EXIT(rv);
        }
    }
    ...
    if (update) {
        if (!with_id) {
            ...
            rv = _bcm_gport_vlan_is_valid_replace(unit, vlan_port, &vlan_port_o);
    ...
    if (SOC_DPP_CONFIG(unit)->pp.pon_application_enable) {
        BCM_PBMP_ITER(gport_info.pbmp_local_ports,local_port) {
            if (is_tunnel_matched) {
                if (local_port >= _BCM_PPD_NOF_PON_PP_PORT) {
    ...
    BCM_PBMP_ITER(gport_info.pbmp_local_ports,local_port) {
        ...
        if (is_tunnel_matched) {
            if (IS_PON_PORT(unit,_BCM_PPD_GPORT_PON_TO_PHY_PORT(local_port))) {
                break;
            }
        }
        else {
    ...
    if (SOC_DPP_CONFIG(unit)->pp.pon_application_enable && 
        ...
        if (local_port != pon_pp_port) {
            if (!IS_PON_PORT(unit,_BCM_PPD_GPORT_PON_TO_PHY_PORT(pon_pp_port))) {
    ...
    if (_BCM_DPP_GPORT_INFO_IS_LOCAL_PORT(gport_info) && !remote_lif)
    {
      ...
      BCM_PBMP_ITER(gport_info.pbmp_local_ports,local_port) {
          ...
          soc_sand_rv = soc_ppd_port_info_get(unit,local_port,&port_info);

          BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
          /* this information not to be change at update */

          if ((SOC_DPP_CONFIG(unit)->trill.mode) && 
              (port_info.vlan_translation_profile == SOC_PPD_PORT_DEFINED_VT_PROFILE_TRILL || 
          ...
          if (port_info.enable_learning != TRUE ||
              ...
              /* If Out-LIF bits are used for ACL profile then don't learn AC */
              if (port_info.port_type != SOC_SAND_PP_PORT_L2_TYPE_CEP && !(is_forward_group)) {
                  port_info.is_learn_ac = 1;
              }
              port_info.orientation = SOC_SAND_PP_HUB_SPOKE_ORIENTATION_SPOKE;
              
              soc_sand_rv = soc_ppd_port_info_set(unit,local_port,&port_info);
              BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
          }

          if (SOC_IS_PETRAB(unit)) {
            rv = _bcm_dpp_am_template_tpid_profile_data_get(unit, local_port, &profile_type, &accept_mode, &dtag_mode, &ac_key_map_type);
            BCMDNX_IF_ERR_EXIT(rv);
          }
          break;
      }
    ...
    */
    ...
    if(encap_with_id) {
        ...
        lif_id = BCM_ENCAP_ID_GET(vlan_port->encap_id);
    }
    else{ /* if not with encap, but with id, then take encap from ID if relevant */
        if(with_id) {
            rv = l2_interface_get_lif_from_port_id(vlan_port, is_protected, is_mc_ingress, &lif_id, BCM_GPORT_VLAN_PORT);
            BCMDNX_IF_ERR_EXIT(rv);
            if (lif_id) { /* If lif_id != 0, it means it's valid to be used. */
                lif_flags = BCM_DPP_AM_FLAG_ALLOC_WITH_ID;
            }
        }
    }
     ...
     */
    if (is_tunnel_matched && is_3_tags_manipulated && (vlan_port->egress_policer_id != 0) && SOC_IS_ARAD_B1_AND_BELOW(unit)) {
        out_ac_flags = BCM_DPP_AM_FLAG_ALLOC_WITH_ID;
        out_ac_id = vlan_port->egress_policer_id;
    }
    ...
    /* is local and defined on lag, allocate global*/
    if(_BCM_DPP_GPORT_INFO_IS_LAG(gport_info)) {
        lif_flags |= BCM_DPP_AM_FLAG_ALLOC_GLOBAL;
    }

    if(!remote_lif) {
        if(!update) {
            /* incase of metro use vpn alloc (alloc ingress + gress) otherwise alloc only for ingress, egress will be with default editing*/
            
            if ((!is_egress) || skip_egress_alloc) {
                rv = bcm_dpp_am_l2_ac_alloc(unit, _BCM_DPP_AM_L2_AC_TYPE_VLAN_EDITING, lif_flags, &lif_id);
                BCMDNX_IF_ERR_EXIT(rv);
            } else if (!is_ingress) {   /* egress only */
                if (!is_3_tags_manipulated) {
                    /* Alloc an OUT LIF to manipulate 2 VLAN TAGs (s-tag and c-tag) */
                    rv = bcm_dpp_am_out_ac_alloc(unit, _BCM_DPP_AM_OUT_AC_TYPE_DEFAULT, lif_flags, &lif_id);
                } else {
#ifdef BCM_88660_A0
                    if (SOC_IS_ARADPLUS(unit)) {
                        /* Alloc an OUT LIF to manipulate 3 VLAN TAGs (tunnel-id tag, s-tag and c-tag) */
                        rv = bcm_dpp_am_out_ac_alloc(unit, _BCM_DPP_AM_OUT_AC_TYPE_BIG_OUT_AC, lif_flags, &lif_id);
                    } else
#endif
                    { 
                        /* Alloc an EEDB entry to manipulate Tunnel_TAG */
                        rv = bcm_dpp_am_out_ac_alloc(unit, _BCM_DPP_AM_OUT_AC_TYPE_PON_3_TAGS_DATA, lif_flags, &lif_id);
                        BCMDNX_IF_ERR_EXIT(rv);
                            
                        /* Alloc an OUT AC to manipulate the rest 2 VLAN TAGs (s-tag and c-tag) */
                        rv = bcm_dpp_am_out_ac_alloc(unit, _BCM_DPP_AM_OUT_AC_TYPE_PON_3_TAGS_OUT_AC, out_ac_flags, &out_ac_id);
                    }
                }
                BCMDNX_IF_ERR_EXIT(rv);
            } else {
                if (!is_3_tags_manipulated) {                    
                    /* Alloc an OUT LIF to manipulate 2 VLAN TAGs (s-tag and c-tag) */
                    rv = bcm_dpp_am_l2_vpn_ac_alloc(unit,_BCM_DPP_AM_L2_VPN_AC_TYPE_DEFAULT, lif_flags,&lif_id);
                    BCMDNX_IF_ERR_EXIT(rv);
                } else {
#ifdef BCM_88660_A0
                    if (SOC_IS_ARADPLUS(unit)) {
                        /* Alloc an OUT LIF to manipulate 3 VLAN TAGs (tunnel-id tag, s-tag and c-tag) */
                        rv = bcm_dpp_am_l2_vpn_ac_alloc(unit,_BCM_DPP_AM_L2_VPN_AC_TYPE_BIG_OUT_AC, lif_flags, &lif_id);
                        BCMDNX_IF_ERR_EXIT(rv);
                    } else 
#endif
                    {
                        /* Alloc an EEDB entry to manipulate Tunnel_TAG */
                        rv = bcm_dpp_am_l2_vpn_ac_alloc(unit,_BCM_DPP_AM_L2_VPN_AC_TYPE_PON_3_TAGS_DATA, lif_flags,&lif_id);
                        BCMDNX_IF_ERR_EXIT(rv);
        ...
        else{ /* if update then realloc, if encap is different, otherwise don't change allocation */
            if((encap_with_id) && BCM_ENCAP_ID_GET(vlan_port->encap_id) != BCM_ENCAP_ID_GET(vlan_port_o.encap_id)) {

                if ((!is_egress) || skip_egress_alloc) { /* ingress only */
                    rv = bcm_dpp_am_l2_ac_dealloc(unit, _BCM_DPP_AM_L2_AC_TYPE_VLAN_EDITING, BCM_ENCAP_ID_GET(vlan_port_o.encap_id));
                    BCMDNX_IF_ERR_EXIT(rv);
                    ...
                    if (!is_3_tags_manipulated) {
                        rv = bcm_dpp_am_out_ac_dealloc(unit, _BCM_DPP_AM_OUT_AC_TYPE_DEFAULT, 0, BCM_ENCAP_ID_GET(vlan_port_o.encap_id));
                        BCMDNX_IF_ERR_EXIT(rv);
...
#ifdef BCM_88660_A0
                        if (SOC_IS_ARADPLUS(unit)) {
                            rv = bcm_dpp_am_out_ac_dealloc(unit, _BCM_DPP_AM_OUT_AC_TYPE_BIG_OUT_AC, 0, vlan_port_o.encap_id);
                            BCMDNX_IF_ERR_EXIT(rv);
...
#endif
                        {
                            rv = bcm_dpp_am_out_ac_dealloc(unit, _BCM_DPP_AM_OUT_AC_TYPE_PON_3_TAGS_DATA, 0, vlan_port_o.encap_id);
                            BCMDNX_IF_ERR_EXIT(rv);
                    ...
                    if (!is_3_tags_manipulated) {
                        rv = bcm_dpp_am_l2_vpn_ac_dealloc(unit,_BCM_DPP_AM_L2_VPN_AC_TYPE_DEFAULT, 0, BCM_ENCAP_ID_GET(vlan_port_o.encap_id));
                        BCMDNX_IF_ERR_EXIT(rv);
...
#ifdef BCM_88660_A0
                        if (SOC_IS_ARADPLUS(unit)) {
                            rv = bcm_dpp_am_l2_vpn_ac_dealloc(unit,_BCM_DPP_AM_L2_VPN_AC_TYPE_BIG_OUT_AC, 0, vlan_port_o.encap_id);
                            BCMDNX_IF_ERR_EXIT(rv);
...
#endif
                        {
                            rv = bcm_dpp_am_l2_vpn_ac_dealloc(unit,_BCM_DPP_AM_L2_VPN_AC_TYPE_PON_3_TAGS_DATA, 0, vlan_port_o.encap_id);
                            BCMDNX_IF_ERR_EXIT(rv);
    ...
    if(is_protected) {
        /* allocate global FEC */
        fec_flags |= BCM_DPP_AM_FLAG_ALLOC_GLOBAL;
        /* if with id, then allocate in same place */
        if(with_id) {
    ...
    */
    ...
    if(!(encap_with_id)) {
        vlan_port->encap_id = lif_id;
    }

    /* 
     * in case of ARAD, egress_policer_id is used to store out_ac_id in PON application with 3 TAGs manipulated.
     */
    if (is_3_tags_manipulated && SOC_IS_ARAD_B1_AND_BELOW(unit)) {
        vlan_port->egress_policer_id = out_ac_id;
    }
    ...
    /* 1+1 protection store gport-id (MC + is-primary) to LIF id */
        ...
        if(!update) {
            /* if not protected, then gport is out-AC and phy-port */
            BCMDNX_ALLOC(phy_port, sizeof(_BCM_GPORT_PHY_PORT_INFO), "vlan_gport");
            if (phy_port == NULL) {
                BCMDNX_ERR_EXIT_MSG(BCM_E_MEMORY, (_BSL_BCM_MSG("alloc for vlan gport fail")));
            }
        }
        ...
        phy_port->type = _BCM_DPP_GPORT_TYPE_MC;
        ...
        if (is_egress) {
            if (!update) {
                ...
                BCMDNX_ALLOC(phy_port, sizeof(_BCM_GPORT_PHY_PORT_INFO), "vlan_gport");
                if (phy_port == NULL) {
                    BCMDNX_ERR_EXIT_MSG(BCM_E_MEMORY, (_BSL_BCM_MSG("could not allocate phy port of type VLAN")));
                }
            }
            else { /* if update remove from hash, reusue same memory*/
                rv = _bcm_dpp_sw_db_hash_vlan_find(unit,
            ...
            if (!update || phy_port->type != _BCM_DPP_GPORT_IN_TYPE_RING) {                
                sal_memset(phy_port, 0, sizeof(_BCM_GPORT_PHY_PORT_INFO));
                if (is_forward_group) {
    ...
    if(is_protected || (is_forward_group)) {
        ...
        rv = soc_dpp_wb_fec_remote_lif_set(unit,act_fec,remote_lif);
        BCMDNX_IF_ERR_EXIT(rv);
#endif
    ...
    /* Update the bookkeeping SW DB. Should be done also for non local ports that aren't remote ports */
    if (!remote_lif && is_ingress) {
        rv = _bcm_dpp_in_lif_ac_match_add(unit, vlan_port, lif_id);
        BCMDNX_IF_ERR_EXIT(rv);
    }
     ...
     if (_BCM_DPP_GPORT_INFO_IS_LOCAL_PORT(gport_info) && !remote_lif) {
            ...
            SOC_PPD_L2_LIF_AC_KEY  in_ac_key_o;
            SOC_PPD_L2_LIF_AC_INFO in_ac_info;
            SOC_PPD_EG_AC_INFO     eg_ac_info;
            SOC_PPD_L2_LIF_AC_ING_EDIT_INFO original_ing_edit_info;
            SOC_PPD_EG_AC_VLAN_EDIT_INFO original_eg_edit_info;


            SOC_PPD_L2_LIF_AC_KEY_clear(&in_ac_key);
            SOC_PPD_L2_LIF_AC_INFO_clear(&in_ac_info);
            SOC_PPD_EG_AC_INFO_clear(&eg_ac_info);
            SOC_PPD_L2_LIF_AC_ING_EDIT_INFO_clear(&original_ing_edit_info);
            SOC_PPD_EG_AC_VLAN_EDIT_INFO_clear(&original_eg_edit_info);
            ...
            if(update) {
                ...
                if (is_ingress) {
                    ...
                    soc_sand_rv = soc_ppd_l2_lif_ac_get(unit, &in_ac_key_o, &lif_id_o, &in_ac_info, &found);
                    SOC_SAND_IF_ERR_RETURN(soc_sand_rv);
                }

                if (is_egress) {
                    soc_sand_rv = soc_ppd_eg_ac_info_get(unit, lif_id_o, &eg_ac_info);
                    SOC_SAND_IF_ERR_RETURN(soc_sand_rv);
                }
            } else {
            ...
            switch (vlan_port->criteria) {
            case BCM_VLAN_PORT_MATCH_PORT:            
                in_ac_key.key_type = SOC_PPD_L2_LIF_AC_MAP_KEY_TYPE_PORT;
                eg_ac_info.edit_info.edit_profile = _BCM_DPP_EVE_PROFILE_REMOVE_TAGS; /* remove tags*/
                in_ac_key.vlan_domain = local_port;/* if key is port only, then it's port and not vlan-domain*/
                break;
            case BCM_VLAN_PORT_MATCH_PORT_VLAN:
                if ((SOC_IS_PETRAB(unit)) && (ac_key_map_type == _bcm_petra_ac_key_map_type_compressional)) {
                    in_ac_key.key_type = SOC_PPD_L2_LIF_AC_MAP_KEY_TYPE_PORT_COMP_VLAN;
                }
                else {
                    in_ac_key.key_type = SOC_PPD_L2_LIF_AC_MAP_KEY_TYPE_PORT_VLAN;
                }
                if (_BCM_DPP_VT_TRANSLATION_PROFILE_INITIAL_VID(port_info.vlan_translation_profile))
                {
                    in_ac_key.key_type = SOC_PPD_L2_LIF_AC_MAP_KEY_TYPE_PORT_INITIAL_VLAN;
            ...
            case BCM_VLAN_PORT_MATCH_PORT_VLAN_STACKED:
                if ((SOC_IS_PETRAB(unit)) && (ac_key_map_type == _bcm_petra_ac_key_map_type_compressional)) {
                    in_ac_key.key_type = SOC_PPD_L2_LIF_AC_MAP_KEY_TYPE_PORT_COMP_VLAN_COMP_VLAN;
                }
                else if (_BCM_DPP_VT_TRANSLATION_PROFILE_FLEXIBLE_Q_IN_Q(port_info.vlan_translation_profile)) {
                    in_ac_key.key_type = SOC_PPC_L2_LIF_AC_MAP_KEY_TYPE_FLEXIBLE_Q_IN_Q_PORT_INITIAL_VID_VID;                    
                }
                else {
                    in_ac_key.key_type = SOC_PPD_L2_LIF_AC_MAP_KEY_TYPE_PORT_VLAN_VLAN;
                }
                eg_ac_info.edit_info.edit_profile = _BCM_DPP_EVE_PROFILE_REMOVE_TAGS_PUSH_2; /* remove tags, push 2 tag*/

                break;
            case BCM_VLAN_PORT_MATCH_PORT_PCP_VLAN:
                in_ac_key.key_type = SOC_PPD_L2_LIF_AC_MAP_KEY_TYPE_PORT_PCP_VLAN;
                eg_ac_info.edit_info.edit_profile = _BCM_DPP_EVE_PROFILE_REMOVE_TAGS_PUSH_1; /* remove tags, push 1 tag*/
                break;
            case BCM_VLAN_PORT_MATCH_PORT_PCP_VLAN_STACKED:
                in_ac_key.key_type = SOC_PPD_L2_LIF_AC_MAP_KEY_TYPE_PORT_PCP_VLAN_VLAN;
                eg_ac_info.edit_info.edit_profile = _BCM_DPP_EVE_PROFILE_REMOVE_TAGS_PUSH_2; /* remove tags, push 2 tag*/
                break;
            case BCM_VLAN_PORT_MATCH_PORT_INITIAL_VLAN:            
            case BCM_VLAN_PORT_MATCH_PORT_UNTAGGED:   
                ...
                break;
            case BCM_VLAN_PORT_MATCH_PORT_TUNNEL:
                in_ac_key.key_type = SOC_PPD_L2_LIF_AC_MAP_KEY_TYPE_PORT_TUNNEL;
                break;
            case BCM_VLAN_PORT_MATCH_PORT_TUNNEL_VLAN:
                in_ac_key.key_type = SOC_PPD_L2_LIF_AC_MAP_KEY_TYPE_PORT_TUNNEL_COMP_VLAN;
                break;
            case BCM_VLAN_PORT_MATCH_PORT_TUNNEL_VLAN_STACKED:
                in_ac_key.key_type = SOC_PPD_L2_LIF_AC_MAP_KEY_TYPE_PORT_TUNNEL_COMP_VLAN_COMP_VLAN;
                break;
            case BCM_VLAN_PORT_MATCH_PORT_TUNNEL_PCP_VLAN:
                in_ac_key.key_type = SOC_PPD_L2_LIF_AC_MAP_KEY_TYPE_PORT_TUNNEL_PCP_ETHER_TYPE_COMP_VLAN_COMP_VLAN;
                /* Ignore other fields */
                in_ac_key.inner_vid = SOC_PPD_LIF_IGNORE_INNER_VID;
                in_ac_key.ethertype = SOC_PPD_LIF_IGNORE_ETHER_TYPE;
                break;
            case BCM_VLAN_PORT_MATCH_PORT_TUNNEL_PCP_VLAN_STACKED:
                in_ac_key.key_type = SOC_PPD_L2_LIF_AC_MAP_KEY_TYPE_PORT_TUNNEL_PCP_ETHER_TYPE_COMP_VLAN_COMP_VLAN;
                /* Ignore other fields */
            ...
            case BCM_VLAN_PORT_MATCH_PORT_TUNNEL_ETHERTYPE:
                ...
                /* Ignore other fields */
                in_ac_key.inner_vid = SOC_PPD_LIF_IGNORE_INNER_VID;
                in_ac_key.outer_pcp = SOC_PPD_LIF_IGNORE_OUTER_PCP;
                in_ac_key.outer_vid = SOC_PPD_LIF_IGNORE_OUTER_VID;
                break;
            case BCM_VLAN_PORT_MATCH_PORT_TUNNEL_VLAN_ETHERTYPE:
                in_ac_key.key_type = SOC_PPD_L2_LIF_AC_MAP_KEY_TYPE_PORT_TUNNEL_PCP_ETHER_TYPE_COMP_VLAN_COMP_VLAN;
                /* Ignore other fields */
            ...
            case BCM_VLAN_PORT_MATCH_PORT_TUNNEL_PCP:
                ...
                break;
            case BCM_VLAN_PORT_MATCH_NONE:
                in_ac_key.key_type = SOC_PPD_L2_LIF_AC_MAP_KEY_TYPE_NONE;
                break;
            ...
            if (vlan_port->flags & BCM_VLAN_PORT_TRANSPARENT) {
                in_ac_key.key_type = SOC_PPD_L2_LIF_AC_MAP_KEY_TYPE_TLS_PORT_TUNNEL;
           ...
           if (vlan_port->flags & BCM_VLAN_PORT_VSI_BASE_VID){
                 ...
                 if (!update){
                     ...
                     in_ac_info.default_frwrd.default_frwd_type = SOC_PPD_L2_LIF_DFLT_FRWRD_SRC_VSI; 
                     in_ac_info.learn_record.learn_type = SOC_PPD_L2_LIF_AC_LEARN_DISABLE;
                     in_ac_info.service_type = SOC_PPD_L2_LIF_AC_SERVICE_TYPE_MP;
                 }
            }
            else{
                if(vlan_port->vsi == BCM_VLAN_NONE) {
                    ...
                    /* for arad set learn LIF for vlan port */
                    if (SOC_IS_ARAD(unit)) {
                        in_ac_info.use_lif = 1;
                    }
                    if(!update) {
                        /* by default Gport, inited as P2P with drop destiantion, till attached to service */
                        in_ac_info.default_frwrd.default_frwd_type = SOC_PPD_L2_LIF_DFLT_FRWRD_SRC_LIF; 
                        /* in PB, LIF destination cannot be drop so use trap-code*/
                        if(SOC_IS_PETRAB(unit)){
                            SOC_PPD_FRWRD_DECISION_TRAP_SET(unit, &(in_ac_info.default_frwrd.default_forwarding), _BCM_PETRA_UD_DROP_TRAP, 5, 0, soc_sand_rv);
                        }
                        else{
                            SOC_PPD_FRWRD_DECISION_DROP_SET(unit, &(in_ac_info.default_frwrd.default_forwarding), soc_sand_rv);
                        }
                        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                        in_ac_info.service_type = SOC_PPD_L2_LIF_AC_SERVICE_TYPE_AC2AC;
                        ...
                        if (is_ingress) {
                            ...
                            _bcm_dpp_in_lif_match_learn_gport_set(unit, lif_id, learn_gport);
                        }
                    }
                }
                else {  /* this is LIF for vlan translation */
                    ...
                    if (!update) {
                        in_ac_info.ing_edit_info.ing_vlan_edit_profile = _BCM_DPP_IVE_PORIFLE_NOP;
                        in_ac_info.default_frwrd.default_frwd_type = SOC_PPD_L2_LIF_DFLT_FRWRD_SRC_VSI; 
                        in_ac_info.learn_record.learn_type = SOC_PPD_L2_LIF_AC_LEARN_SYS_PORT;
                        in_ac_info.service_type = SOC_PPD_L2_LIF_AC_SERVICE_TYPE_MP;
                    }
                }
            }
            ...
            if (is_1_plus_1 || is_1_plus_1_o) {
                ...
                in_ac_info.protection_pass_value = _BCM_PPD_GPORT_IS_WORKING_PORT(vlan_port)?0:1;
            } 

            /* Set the orientation */
            if(vlan_port->flags & BCM_VLAN_PORT_NETWORK) {
                in_ac_info.orientation = SOC_SAND_PP_HUB_SPOKE_ORIENTATION_HUB;
            }
            else{
                in_ac_info.orientation = SOC_SAND_PP_HUB_SPOKE_ORIENTATION_SPOKE;
            }
            ...
            if (is_ingress) {
                ...
                if (SOC_DPP_IS_VLAN_TRANSLATE_MODE_NORMAL(unit)) {
                    ...
                    rv = _bcm_dpp_am_template_vlan_edit_profile_mapping_exchange (unit, lif_id, ing_profile_info, &old_profile, &is_last, &new_profile, &is_allocated);
                    BCMDNX_IF_ERR_EXIT(rv);
                }
                if(update) {
                    /* if key is different remove old key */
                    if(sal_memcmp(&in_ac_key_o, &in_ac_key,sizeof(SOC_PPD_L2_LIF_AC_KEY))){
                        soc_sand_rv = soc_ppd_l2_lif_ac_remove(unit, &in_ac_key_o, &lif_id_o);
                        SOC_SAND_IF_ERR_RETURN(soc_sand_rv);
                    }

                    if(lif_id_o != lif_id) {
                        rv = _bcm_dpp_in_lif_match_delete(unit,lif_id_o);
                        BCMDNX_IF_ERR_EXIT(rv);
                        rv = _bcm_dpp_in_lif_match_vsi_set(unit,lif_id_o, 0);
                        BCMDNX_IF_ERR_EXIT(rv);
                    }
                }
                ...
                soc_sand_rv = soc_ppd_l2_lif_ac_add(unit, &in_ac_key, lif_id, &in_ac_info,&failure_indication);
                BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
            ...
            if (is_egress) {
                ...
                if (is_3_tags_manipulated) {
                    ...
                    if (SOC_IS_ARADPLUS(unit)) {
                        ...
                        SOC_PPD_EG_ENCAP_DATA_2ND_PON_TUNNEL_FORMAT_SET(unit, pon_tunnel_tpid, pon_tunnel_pcp, vlan_port->egress_tunnel_value, &eg_ac_info.edit_info);
                    } else 
#endif
                    {
                        rv = _bcm_dpp_pon_encap_data_enty_add(unit, lif_id, pon_tunnel_tpid, pon_tunnel_pcp, vlan_port->egress_tunnel_value, out_ac_id);
                        BCMDNX_IF_ERR_EXIT(rv);
                    }
                }

                out_lif_id = (is_3_tags_manipulated && SOC_IS_ARAD_B1_AND_BELOW(unit)) ? out_ac_id : lif_id;

                if (SOC_DPP_IS_VLAN_TRANSLATE_MODE_NORMAL(unit)) {
                    /* Using one EVE in case of manipulating 2 TAGs(PON Tunnel TAG + c-tag) */
                    if ((is_tunnel_matched) && (!is_3_tags_manipulated)) {                    
                    ...
                    rv = _bcm_dpp_am_template_vlan_edit_profile_eg_mapping_exchange (unit, out_lif_id, eg_profile_info, &old_profile, &is_last, &new_profile, &is_allocated);
                    BCMDNX_IF_ERR_EXIT(rv);

                    /* internal verify */
                    if (new_profile != eg_ac_info.edit_info.edit_profile || is_allocated ) {
                        BCMDNX_ERR_EXIT_MSG(BCM_E_INTERNAL, (_BSL_BCM_MSG("Mismatch in egress vlan editing profile given id: %d is not as expected one: %d"), new_profile, eg_ac_info.edit_info.edit_profile));
                    }
                }

				if(soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "evc_filter", 0) && vlan_port->egress_tunnel_value != 0)
				{
					eg_ac_info.edit_info.use_as_data_entry = 1;
					eg_ac_info.edit_info.data[0] = vlan_port->egress_tunnel_value;
				}

                /* Set the Egress LIF info */
                soc_sand_rv = soc_ppd_eg_ac_info_set(unit, out_lif_id, &eg_ac_info);
                BCM_SAND_IF_ERR_EXIT(soc_sand_rv);

                /* Add to OutLIF SW DB */
                BCMDNX_IF_ERR_EXIT(_bcm_dpp_out_lif_usage_set(unit, out_lif_id, _bcmDppLifTypeVlan));
                
                /* if egress hairpin is enabled set egress state as well*/
#ifdef BCM_PETRAB_SUPPORT
                if (SOC_IS_PETRAB(unit) &&
                    SOC_DPP_CONFIG(unit)->pb->pp_op_mode.split_horizon_filter_enable) {
                    soc_sand_rv = soc_ppd_eg_filter_split_horizon_out_ac_orientation_set(unit,out_lif_id,in_ac_info.orientation);
                    SOC_SAND_IF_ERR_RETURN(soc_sand_rv);
                }
#endif /* BCM_PETRAB_SUPPORT */
#ifdef BCM_ARAD_SUPPORT
                if(SOC_IS_ARAD(unit) &&
                    SOC_DPP_CONFIG(unit)->arad->pp_op_mode.split_horizon_filter_enable) {
                    soc_sand_rv = soc_ppd_eg_filter_split_horizon_out_lif_orientation_set(unit,out_lif_id,in_ac_info.orientation);
                    SOC_SAND_IF_ERR_RETURN(soc_sand_rv);
                }
                 ...
                 */
                if (SOC_IS_ARAD(unit) && vlan_port->criteria == BCM_VLAN_PORT_MATCH_PORT) {                  
                    soc_sand_rv = soc_ppd_port_info_get(unit,local_port,&port_info);
                    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                
                    port_info.dflt_egress_ac = out_lif_id;
                ...
                if ((vlan_port->vsi <= BCM_VLAN_MAX) && (vlan_port->vsi != BCM_VLAN_NONE) && (vlan_port->criteria != BCM_VLAN_PORT_MATCH_NONE)) {
                    ...
                    if ((!cep_port) || 
                        ...
                        soc_sand_rv = soc_ppd_eg_ac_port_vsi_info_add(unit, out_lif_id, &ac_port_vsi_key, &eg_ac_info,&failure_indication);
                    } else {
                        SOC_PPD_EG_AC_CEP_PORT_KEY_clear(&ac_port_cep_key);
                        ...
                        soc_sand_rv = soc_ppd_eg_ac_port_cvid_info_add(unit, out_lif_id, &ac_port_cep_key, &eg_ac_info, &failure_indication);
                    }
                    ...
                    if(update) {
                        if (!cep_port || 
                            vlan_port->criteria == BCM_VLAN_PORT_MATCH_PORT ||
                            vlan_port->criteria == BCM_VLAN_PORT_MATCH_PORT_INITIAL_VLAN) {
                            SOC_PPD_EG_AC_VBP_KEY_clear(&ac_port_vsi_key_o);
                            ...
                            if(sal_memcmp(&ac_port_vsi_key_o, &ac_port_vsi_key,sizeof(SOC_PPD_EG_AC_VBP_KEY))){
                                soc_sand_rv = soc_ppd_eg_ac_port_vsi_info_remove(unit, &ac_port_vsi_key_o, &lif_id_o);
                                SOC_SAND_IF_ERR_EXIT(soc_sand_rv);
                            }
                        } else {
                                SOC_PPD_EG_AC_CEP_PORT_KEY_clear(&ac_port_cep_key_o);
                                ...
                                /* if key was changed, remove old key */
                                if(sal_memcmp(&ac_port_cep_key_o, &ac_port_cep_key,sizeof(SOC_PPD_EG_AC_CEP_PORT_KEY))){
                                    soc_sand_rv = soc_ppd_eg_ac_port_cvid_info_remove(unit, &ac_port_cep_key_o, &lif_id_o);
                                    SOC_SAND_IF_ERR_EXIT(soc_sand_rv);
                            }
                        }
                    }
                ...
                if (!update) {
                    soc_sand_rv = soc_ppd_eg_encap_next_outlif_update(unit, out_lif_id, SOC_PPD_EG_ENCAP_ENTRY_UPDATE_DROP, 0);
                    SOC_SAND_IF_ERR_EXIT(soc_sand_rv);
                }
            }
    }
    ...
    SOC_PPC_EG_ENCAP_PWE_INFO *old_pwe_info)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    soc_sand_dev_id = (unit);
    SOC_PPD_EG_ENCAP_PWE_INFO_clear(&eep_pwe_info);

    /* set EEP/PWE information */
    if(is_local_conf || _Bcm_dpp_sync_all){
            ...
            if (_BCM_PETRA_L3_ITF_IS_ENCAP(mpls_port->egress_tunnel_if)) {
                rv = _bcm_l3_encap_id_to_eep(unit, mpls_port->egress_tunnel_if, &eep_id_intf);
            } else {
            ...
            soc_sand_rv = soc_ppd_eg_encap_pwe_entry_add(soc_sand_dev_id,encap_id,&eep_pwe_info,eep_id_intf);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);

            rv = _bcm_dpp_out_lif_mpls_match_add(unit,mpls_port,encap_id);
            BCMDNX_IF_ERR_EXIT(rv);

#ifdef BCM_ARAD_SUPPORT
            if (SOC_IS_ARAD(unit) && (SOC_DPP_CONFIG(unit)->arad->pp_op_mode.split_horizon_filter_enable)) {
                soc_sand_rv = soc_ppd_eg_filter_split_horizon_out_lif_orientation_set(soc_sand_dev_id, encap_id, eep_pwe_info.orientation);
                BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
            }
#endif /* BCM_ARAD_SUPPORT */
    }
 ...
 )
{
    ...
    rv = _bcm_dpp_gport_to_phy_port(unit, phy_port, 0, &gport_info);
    BCMDNX_IF_ERR_EXIT(rv);

    /* only for local ports */
    if(_BCM_DPP_GPORT_INFO_IS_LOCAL_PORT(gport_info)) {
        /* get vlan domain */
        soc_sand_rv = soc_ppd_port_info_get(soc_sand_dev_id,gport_info.local_port,&port_info);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);

        if (SOC_IS_PETRAB(unit)) {
          rv = _bcm_dpp_am_template_tpid_profile_data_get(unit, gport_info.local_port, &profile_type, &accept_mode, &dtag_mode, &ac_key_map_type);
          BCMDNX_IF_ERR_EXIT(rv);
        }
    }      
    ...
    if(gport_type == BCM_GPORT_VLAN_PORT) {
        ...
        case BCM_VLAN_PORT_MATCH_PORT:
            in_ac_key->key_type = SOC_PPD_L2_LIF_AC_MAP_KEY_TYPE_PORT;
            in_ac_key->vlan_domain = gport_info.local_port;
            break;
        case BCM_VLAN_PORT_MATCH_PORT_VLAN:
            if ((SOC_IS_PETRAB(unit)) && (ac_key_map_type == _bcm_petra_ac_key_map_type_compressional)) {
                in_ac_key->key_type = SOC_PPD_L2_LIF_AC_MAP_KEY_TYPE_PORT_COMP_VLAN;
 ...
 )
{
    ...
    *remote_lif = 0;
    ...
    rv = _bcm_dpp_gport_resolve(unit,gport_id,0,&dest_id,&encap_id,&gport_type);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    if(gport_type == _BCM_DPP_GPORT_IN_TYPE_FEC) {
#ifdef BCM_ARAD_SUPPORT
            rv = soc_dpp_wb_fec_remote_lif_get(unit,dest_id.dest_val,remote_lif);
            BCMDNX_IF_ERR_EXIT(rv);
#else
            *remote_lif = 0;
#endif
 ...
 )
{
    ...
    _BCM_GPORT_PHY_PORT_INFO *phy_port=NULL;
    SOC_SAND_PP_DESTINATION_ID dest_id;
    int encap_id,gport_type_encoding,forwarding_type;
    int gport_val,tmp_lif;
    uint8 remote_lif=0;
     ...
     */
    if(BCM_GPORT_IS_TUNNEL(gport_id) && (lif_id != NULL || out_lif_id != NULL)) {
        rv = _bcm_dpp_gport_parse(unit,gport_id,&gport_type_encoding, &gport_val, NULL);
        BCMDNX_IF_ERR_EXIT(rv);

        if(gport_type_encoding == _BCM_DPP_GPORT_TYPE_ENCAP) {
            if (lif_id != NULL) {
              *lif_id = SOC_DPP_LIF_ID_FROM_ENCAP_ID(gport_val);
            }

            if (out_lif_id != NULL) {
              *out_lif_id = SOC_DPP_LIF_ID_FROM_ENCAP_ID(gport_val);
            }
            /* assume local always*/
            if (is_local) {
                *is_local = 1;
    ...
    rv = _bcm_dpp_gport_resolve(unit,gport_id,0,&dest_id,&encap_id,&forwarding_type);
    BCMDNX_IF_ERR_EXIT(rv);
    if(fec_id) {
        if(forwarding_type == _BCM_DPP_GPORT_IN_TYPE_FEC) {
            *fec_id = dest_id.dest_val;
        }
        else if(forwarding_type == _BCM_DPP_GPORT_IN_TYPE_MIM_MP || forwarding_type == _BCM_DPP_GPORT_IN_TYPE_MIM_P2P) {
    ...
    if (forwarding_type == _BCM_DPP_GPORT_IN_TYPE_FEC_VC) {
        ...
        tmp_lif = SOC_DPP_LIF_ID_FROM_ENCAP_ID(gport_val);
    }
    ...
    if (BCM_GPORT_IS_TRILL_PORT(gport_id)) {
        ...
        if (lif_id != NULL) { /* In LIF is the returned value from unicast_route_get */
           *lif_id = trill_lif_id;
        }
    ...
    if (lif_id && BCM_GPORT_IS_MIM_PORT(gport_id) && !remote_lif){
        ...
        if (phy_port->type != _BCM_DPP_GPORT_IN_TYPE_MIM_P2P) {
    ...
    /* lif not required */
    if (lif_id) {
        *lif_id = (is_ingress) ? tmp_lif : _BCM_GPORT_ENCAP_ID_LIF_INVALID;
    }
    ...
    if (out_lif_id) {
        *out_lif_id = (is_egress) ? tmp_lif : _BCM_GPORT_ENCAP_ID_LIF_INVALID;
        ...
        if ((is_local) && (_BCM_DPP_GPORT_INFO_IS_LOCAL_PORT(gport_info)) && (*out_lif_id != _BCM_GPORT_ENCAP_ID_LIF_INVALID)) {
            ...
            if (is_pon_port) {
                rv = _bcm_dpp_pon_lif_is_3_tags_data(unit, tmp_lif, &is_3_tags_data);
                BCMDNX_IF_ERR_EXIT(rv);

                if (is_3_tags_data) {
                    /* get out_ac or out_lif index from input lif index */
                    rv = _bcm_dpp_pon_encap_data_enty_get(unit, (uint32)tmp_lif, &tpid, &pcp, &tunnel_id, out_lif_id);
                    BCMDNX_IF_ERR_EXIT(rv);
                }
            }
        }
        ...
        if (BCM_GPORT_IS_MPLS_PORT(gport_id)) {
            rv = _bcm_dpp_lif_to_vsi(unit, tmp_lif, &vsi, sw_state, NULL);          
            BCMDNX_IF_ERR_EXIT(rv);
...
/*
 * Given a vlan_port_id/mpls_port_id, determines wether this gport represents ingress only or egress only.
 ...
 * In case sw_state is 1 then resolving lif to gport will be in optimized time which is 
 * from SW DBs as much as possible. 
     ...
     )
{
    ...
    *port_val = BCM_GPORT_INVALID;
     ...
     */
    rv = _bcm_dpp_lif_to_gport(unit, cur_lif, sw_state, port_val);
    BCMDNX_IF_ERR_EXIT(rv);
 ...
 )
{
    ...
    *is_member = FALSE;
    ...
    rv = _bcm_dpp_gport_to_lif(unit,gport_id, &cur_lif, NULL, NULL, is_local);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    if(mp) {
        ...
        /* from LIF get vsi */
        if ((*is_local) || (_Bcm_dpp_sync_all && (!(BCM_GPORT_IS_VLAN_PORT(gport_id))))) {
            soc_sand_rv = soc_ppd_lif_table_entry_get(soc_sand_dev_id,cur_lif,&lif_entry_info);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
   ...
   )
{
    ...
    *vsi = -1;
    ...
    rv = _bcm_dpp_lif_to_gport(unit, lif, sw_state, &tmp_gport);
    BCMDNX_IF_ERR_EXIT(rv);
...
{
    int is_local, in_lif_id, out_lif_id, fec_id, is_3_tags_data = 0;
    bcm_mpls_port_t mpls_port;
    bcm_vlan_port_t vlan_port;
    uint32 soc_sand_rv;
    int rv = BCM_E_NONE;
    ...
    */
    ...
    /* Get the gport type */
    rv = _bcm_dpp_gport_parse(unit,port_id, &gport_type, &gport_val, NULL);
    BCMDNX_IF_ERR_EXIT(rv);

    /* get LIF + FEC used for this gport */
    rv = _bcm_dpp_gport_to_lif(unit,port_id, &in_lif_id, &out_lif_id, &fec_id, &is_local);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    if (BCM_GPORT_IS_VLAN_PORT(port_id) && (lif_usage != _bcmDppLifTypeVlan)) { /* VLAN */
        BCMDNX_ERR_EXIT_MSG(BCM_E_PORT, (_BSL_BCM_MSG("LIF is not a VLAN")));
    }
    else if (BCM_GPORT_IS_MPLS_PORT(port_id) && (lif_usage != _bcmDppLifTypeMplsPort)) { /* PWE */
    ...
    if (fec_id != -1) {
        ...
        if (BCM_GPORT_IS_VLAN_PORT(port_id) &&
            ...
            rv = bcm_dpp_am_fec_dealloc(unit, 0, 0, 1, fec_id);
        } else if (_BCM_PPD_GPORT_IS_WORKING_PORT_ID(fec_id)) {
            rv = bcm_dpp_am_fec_dealloc(unit, 0, 0, 2, fec_id);
        }
        BCMDNX_IF_ERR_EXIT(rv);
    }
    ...
    if ((!remote_lif) && ((fec_id == -1 || (!(gport_type == _BCM_DPP_GPORT_TYPE_FEC))) ||
        (BCM_GPORT_IS_VLAN_PORT(port_id) && SOC_DPP_ENCAP_TYPE_GET(BCM_GPORT_VLAN_PORT_ID_GET(port_id)) == _DPP_ENCAP_ID_FORWARD_VAL)))  {   
        /* In case of VLAN PORT we need to remove vlan information only if egress.
           In case of MPLS PORT we need to remove vlan information anyway. */
        if (is_egress || BCM_GPORT_IS_MPLS_PORT(port_id)) {
            rv = _bcm_dpp_sw_db_hash_vlan_find(unit,
                                             &(port_id),
                                             (shr_htb_data_t*)(void *)&phy_port,
                                             TRUE); /* remove */
    ...
    if (!is_local) {
        ...
        if (_Bcm_dpp_sync_all && !remote_lif)  {
            if (BCM_GPORT_IS_VLAN_PORT(port_id)) {
                if (is_ingress) {
                    /* Deallocate ingress if it's an ingress object */
                    if (bcm_dpp_am_l2_ac_is_alloced(unit, in_lif_id) == BCM_E_EXISTS) {
                        rv = bcm_dpp_am_l2_ac_dealloc(unit, _BCM_DPP_AM_L2_AC_TYPE_VLAN_EDITING, in_lif_id);
                        BCMDNX_IF_ERR_EXIT(rv);
                    }
                }
                ...
                if (is_egress) {
                    ...
                    if (bcm_dpp_am_out_ac_is_alloced(unit, out_lif_id) == BCM_E_EXISTS) {
                        rv = bcm_dpp_am_out_ac_dealloc(unit, _BCM_DPP_AM_OUT_AC_TYPE_DEFAULT, 0, out_lif_id);
                        BCMDNX_IF_ERR_EXIT(rv);
                    }
                }
                ...
                if (SOC_DPP_IS_VLAN_TRANSLATE_MODE_NORMAL(unit)) {
                    if (is_ingress) {
                        ...
                        rv = _bcm_dpp_am_template_vlan_edit_profile_mapping_exchange (unit, in_lif_id, ing_profile_info, &old_profile, &is_last, &new_profile, &is_allocated);
                        BCMDNX_IF_ERR_EXIT(rv);
                    }
                ...
                if (is_ingress) {
                    ...
                    rv = _bcm_dpp_in_lif_match_delete(unit, in_lif_id);
                    BCMDNX_IF_ERR_EXIT(rv);

                    rv = _bcm_dpp_in_lif_match_vsi_set(unit, in_lif_id, 0);
                    BCMDNX_IF_ERR_EXIT(rv);
                }
                ...
                if (is_egress) {
                    ...
                    BCMDNX_IF_ERR_EXIT(_bcm_dpp_out_lif_match_delete(unit,out_lif_id));
                }
    ...
    if (BCM_GPORT_IS_MPLS_PORT(port_id)){
        if (!remote_lif && ((is_local) || (_Bcm_dpp_sync_all))) {
            if (is_ingress) {
                ...
                if (removed_lif == in_lif_id) {
                    ...
                    removed_lif = (uint32)in_lif_id;
                    LOG_WARN(BSL_LS_BCM_PORT,
                             (BSL_META_U(unit,
                                         "When destroy LIF expected LIF and given LIF must be equal. Otherwise SEM entry won't be deleted.\n")));
                    soc_sand_rv = soc_ppd_l2_lif_pwe_remove(unit, mpls_port.match_label, FALSE/*remove from SEM*/, (mpls_port.flags & BCM_MPLS_PORT_WITH_GAL), &removed_lif);
                    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                }
            }
            ...
            if (is_egress && is_ingress) {
                rv = bcm_dpp_am_l2_vpn_pwe_eep_dealloc(unit,in_lif_id);
                BCMDNX_IF_ERR_EXIT(rv);

                rv = bcm_dpp_am_l2_vpn_pwe_lif_dealloc(unit,in_lif_id);
                BCMDNX_IF_ERR_EXIT(rv);
            }
            else if (is_egress && !is_ingress) {
                rv = bcm_dpp_am_mpls_eep_dealloc(unit, in_lif_id);
                BCMDNX_IF_ERR_EXIT(rv);
            }
            else { /* !is_egress && is_ingress */
                 /* Deallocating only InLif */
                 rv = bcm_dpp_am_l3_lif_dealloc(unit, in_lif_id);
                 BCMDNX_IF_ERR_EXIT(rv);
            }
            ...
            if (is_ingress) {
                ...
                /* Free push profile */
                rv = bcm_petra_mpls_tunnel_free_push_profile(unit, in_lif_id);
                BCMDNX_IF_ERR_EXIT(rv);
        ...
        if ((vlan_port.vsi <= BCM_VLAN_MAX) && is_egress) {
            ...
            if (vlan_port.criteria == BCM_VLAN_PORT_MATCH_PORT) {
                ...
                if (SOC_IS_ARAD(unit)) {
                    ...
                    /* get port info */
                    soc_sand_rv = soc_ppd_port_info_get(unit,gport_info.local_port,&port_info);
                    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);

                    port_info.dflt_egress_ac = is_port_control_mim ? __dpp_mim_out_ac_get(unit) : 0;

                    soc_sand_rv = soc_ppd_port_info_set(unit, gport_info.local_port, &port_info);
                    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                }
            }
        }
        ...
        if (is_egress) { 
            ...
            if (SOC_IS_ARADPLUS(unit)) {
                soc_sand_rv = soc_ppd_eg_ac_info_get(unit, out_lif_id, &eg_ac_info_o);
                BCM_SAND_IF_ERR_EXIT(soc_sand_rv);           
            }
#endif
            eg_ac_info.edit_info.nof_tags = 2;
            ...
            if ((SOC_DPP_CONFIG(unit)->pp.pon_application_enable) && (BCM_E_EXISTS == bcm_dpp_am_out_ac_is_alloced(unit, out_lif_id))) {
                eedb_id_3_tags = in_lif_id;
        ...
        if (is_3_tags_data) {
            ...
            if (BCM_E_EXISTS == bcm_dpp_am_l2_ac_is_alloced(unit, in_lif_id)) {
                rv = bcm_dpp_am_l2_vpn_ac_dealloc(unit,_BCM_DPP_AM_L2_VPN_AC_TYPE_PON_3_TAGS_DATA, 0, in_lif_id);
                BCMDNX_IF_ERR_EXIT(rv);
            }
            ...
            if (BCM_E_EXISTS == bcm_dpp_am_out_ac_is_alloced(unit, out_lif_id)) {
                rv = bcm_dpp_am_out_ac_dealloc(unit, _BCM_DPP_AM_OUT_AC_TYPE_BIG_OUT_AC, 0, out_lif_id);
                BCMDNX_IF_ERR_EXIT(rv);
            }
        }
#endif
    ...
    SOC_PPC_EG_ENCAP_PWE_INFO *old_pwe_info)
{
    ...
    uint8 is_1_plus_1, is_mc_ingress=0;
    SOC_PPD_FEC_ID fec_id, protect_fec_id;
    ...
    SOC_PPD_L2_LIF_PWE_INFO_clear(pwe_info);
    ...
    if (is_gal) {
        rv = _bcm_dpp_gport_to_lif(unit, mpls_port->tunnel_id, &base_pwe_lif, NULL, NULL, NULL);
        BCMDNX_IF_ERR_EXIT(rv);
    }

    /* Find the protection type on port (if any) and verify it's legal. */
    rv = l2_interface_protection_validation(unit, mpls_port, TRUE, &is_1_to_1_protected, NULL, &is_mc_ingress, &is_1_plus_1, BCM_GPORT_MPLS_PORT);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    * Set flags for ingress/egress operations and perform related validations 
    */
    rv = l2_interface_ingress_egress_only_validation(mpls_port, TRUE, &is_ingress, &is_egress, BCM_GPORT_MPLS_PORT);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    */
    if(encap_with_id) {
        lif_flags = BCM_DPP_AM_FLAG_ALLOC_WITH_ID;
        lif_id = BCM_ENCAP_ID_GET(mpls_port->encap_id);
        eep = lif_id;
        ...
        rv = l2_interface_get_lif_from_port_id(mpls_port, is_1_to_1_protected, is_mc_ingress, &lif_id, BCM_GPORT_MPLS_PORT);
        BCMDNX_IF_ERR_EXIT(rv);

        if (lif_id) { /* If the lif id is valid, then then WITH_ID flag is for the lif allocation. */
            lif_flags = BCM_DPP_AM_FLAG_ALLOC_WITH_ID;
        }

        if (is_1_to_1_protected) {
            rv = l2_interface_calculate_act_fec(mpls_port, (int*)(&fec_id), BCM_GPORT_MPLS_PORT);
            BCMDNX_IF_ERR_EXIT(rv);
        } 
        ...
        /* Usually, the given mpls_port_id doesn't take into consideration the type of mpls port used (fec, mc or lif). This is
           here so we find the relevant type */
        rv = l2_interface_calculate_port_id(unit, fec_id, lif_id, mpls_port, BCM_GPORT_MPLS_PORT);
        BCMDNX_IF_ERR_EXIT(rv);
    }
        ...
        */
        ...
        if (!remote_lif) {
            if(!update) {
                ...
                if(((mp) || (is_1_to_1_protected) || (is_1_plus_1) || (!_BCM_PETRA_L3_ITF_IS_FEC(mpls_port->egress_tunnel_if))) && is_egress){

                    eep_required = 1;

                    if (is_ingress && is_egress) {
                        rv = bcm_dpp_am_l2_vpn_pwe_alloc(unit,pwe_lif_types,lif_flags,1,&lif_id,NULL);
                        BCMDNX_IF_ERR_EXIT(rv);
                    ...
                    rv = bcm_dpp_am_mpls_eep_alloc(unit,(is_ingress && is_egress) ? _BCM_DPP_AM_MPLS_EEP_TYPE_PWE:_BCM_DPP_AM_MPLS_EEP_TYPE_PWE_EGRESS_ONLY,lif_flags,1,&eep);
                    BCMDNX_IF_ERR_EXIT(rv);
                }
                else {
                    if (is_ingress) {
                        /* Allocating only InLif */
                        rv = bcm_dpp_am_l3_lif_alloc(unit, lif_types, lif_flags, (int*)&lif_id);
                        BCMDNX_IF_ERR_EXIT(rv);
                    }
                }
            }
            else if (is_1_to_1_protected && encap_with_id && is_egress) { /* Update EEP only if it's protected */
                eep_required = 1;
                eep = lif_id;
                rv = bcm_dpp_am_mpls_eep_is_alloced(unit, eep);
                if (rv != BCM_E_EXISTS) {
                    BCMDNX_ERR_EXIT_MSG(BCM_E_NOT_FOUND, (_BSL_BCM_MSG("EEP to be replaced %d does not exist"), eep));
                }       
            }
        }
    ...
    #ifdef BCM_ARAD_SUPPORT
            rv = soc_dpp_wb_fec_remote_lif_set(unit,fec_id,remote_lif);
            BCMDNX_IF_ERR_EXIT(rv);
    #endif
        }
        ...
        if (mpls_port->egress_label.label != 0) {
            /* PUSH PROFILE template management. allocate in case local (for EEP) or FEC+VC (learn data) */
            if(!remote_lif && (is_local_conf /* local */ || (!_BCM_PPD_IS_VALID_FAILOVER_ID(mpls_port->failover_id)))) /* Tunnel protected */ {
                ...
                if (update) {
                    ...
                    if (old_pwe_info->push_profile != push_profile) {
    ...
    */ 
    ...
    if(_BCM_PPD_IS_VALID_FAILOVER_ID(mpls_port->failover_id)) {
        if(is_mc_ingress) {
            if(!update) {
                /* if not protected, then gport is out-AC and phy-port */
                BCMDNX_ALLOC(phy_port, sizeof(_BCM_GPORT_PHY_PORT_INFO), "mpls_gport");
                if (phy_port == NULL) {
                    BCMDNX_ERR_EXIT_MSG(BCM_E_MEMORY, (_BSL_BCM_MSG("alloc for mpls gport fail")));
                }
            }
            else{ /* if update remove from hash, reusue same memory*/
                rv = _bcm_dpp_sw_db_hash_vlan_find(unit,
        ...
        if (is_ingress && !is_egress) {
            if(!update) {
                /* if protected, then gport is FEC */
                BCMDNX_ALLOC(phy_port, sizeof(_BCM_GPORT_PHY_PORT_INFO), "mpls_gport");
                if (phy_port == NULL) {
                    BCMDNX_ERR_EXIT_MSG(BCM_E_MEMORY, (_BSL_BCM_MSG("alloc for mpls gport fail")));
                }
            ...
            failover_encap_id = BCM_GPORT_MPLS_PORT_ID_GET(mpls_port->failover_port_id);
            if (SOC_DPP_ENCAP_ID_IS_FORWARD_GROUP(failover_encap_id)) {
                /* This fec id is used for the learning information */
                phy_port->phy_gport = SOC_DPP_FORWARD_GROUP_FROM_ENCAP_ID(failover_encap_id);
            } else {
                protect_fec_id = failover_encap_id;
                phy_port->phy_gport = _BCM_PPD_GPORT_PROTECT_TO_WORK_FEC(protect_fec_id);
            }
        ...
        if(!update) {
            BCMDNX_ALLOC(phy_port, sizeof(_BCM_GPORT_PHY_PORT_INFO), "mpls_gport");
            if (phy_port == NULL) {
                BCMDNX_ERR_EXIT_MSG(BCM_E_MEMORY, (_BSL_BCM_MSG("Could not allocate Phy port of type MPLS port")));
            }
        }
        else{ /* if update remove from hash, reusue same memory*/
            rv = _bcm_dpp_sw_db_hash_vlan_find(unit,
        ...
        if (_BCM_PETRA_L3_ITF_IS_ENCAP(mpls_port->egress_tunnel_if)) {
            phy_port->type = _BCM_DPP_GPORT_IN_TYPE_EEP;
            phy_port->phy_gport = mpls_port->port; /* physical port */
            phy_port->encap_id = BCM_ENCAP_ID_GET(mpls_port->encap_id); /* encap_id = out-PWE */
        } else {
            if (mpls_port->flags2 & BCM_MPLS_PORT2_LEARN_ENCAP) {
                phy_port->type = _BCM_DPP_GPORT_IN_TYPE_FEC_EEP;
                phy_port->phy_gport =_BCM_PETRA_L3_ITF_VAL_GET(mpls_port->egress_tunnel_if);  /* MPLS FEC */
                phy_port->encap_id = BCM_ENCAP_ID_GET(mpls_port->encap_id); /* PWE Out-Lif */
            }
            else {
                phy_port->type = _BCM_DPP_GPORT_IN_TYPE_FEC_VC;
                phy_port->phy_gport =_BCM_PETRA_L3_ITF_VAL_GET(mpls_port->egress_tunnel_if);  /* FEC */
                _BCM_DPP_ENCAP_ID_VC_SET(phy_port->encap_id,mpls_port->egress_label.label,push_profile); /* encap_id = VC */
            }
        }        
        rv = _bcm_dpp_sw_db_hash_vlan_insert(unit,
                                             &(mpls_port->mpls_port_id),
    ...
    if(eep_required == 1 && !remote_lif && is_egress) {
        rv = _bcm_dpp_mpls_port_pwe_eep_set(unit,vpn,mpls_port,BCM_ENCAP_ID_GET(mpls_port->encap_id),
                                            push_profile, is_local_conf, old_pwe_info);
        BCMDNX_IF_ERR_EXIT(rv);
    }
    ...
    if(!remote_lif && (is_local_conf || _Bcm_dpp_sync_all) && is_ingress) {
      /* 3. LIF for MP */
      /* In case of GAL P2P entry should be added */
      if(_BCM_DDP_IS_MP_VPN(vpn) && !is_gal) {
          pwe_info->service_type = SOC_PPD_L2_LIF_PWE_SERVICE_TYPE_MP;
          pwe_info->vsid = _BCM_DDP_VPN_TO_VSI(vpn);
          pwe_info->default_frwrd.default_frwd_type = SOC_PPD_L2_LIF_DFLT_FRWRD_SRC_VSI;
          /* learn-info*/
  
          /* learning is enabled as MP */
          ...
          if(_BCM_PPD_IS_VALID_FAILOVER_ID(mpls_port->failover_id)) {
              ...
              rv = _bcm_dpp_in_lif_match_learn_gport_set(unit, lif_id, mpls_port->mpls_port_id);
              BCMDNX_IF_ERR_EXIT(rv);
          }
          /* protected case, where need to configure only LIF,
             in this case failover-id is invalid, but failover_port_id includes the FEC id to learn */
              ...
              if (SOC_DPP_ENCAP_ID_IS_FORWARD_GROUP(failover_encap_id)) {
                  /* This fec id is used for the learning information */
                  fec_id = SOC_DPP_FORWARD_GROUP_FROM_ENCAP_ID(failover_encap_id);
              }
              else {
                  protect_fec_id = failover_encap_id;
              ...
              rv = _bcm_dpp_in_lif_match_learn_gport_set(unit, lif_id, mpls_port->failover_port_id);
              BCMDNX_IF_ERR_EXIT(rv);
          }
          /* protected-tunnel: learn FEC + VC */
              ...
              if (_BCM_PETRA_L3_ITF_IS_ENCAP(mpls_port->egress_tunnel_if)) {
                /* learn EEP + Dest */
                SOC_PPD_FRWRD_DECISION_PWE_SET(unit, &(pwe_info->learn_record.learn_info), lif_id, is_lag, sys_port, soc_sand_rv);
              } else {
                if (mpls_port->flags2 & BCM_MPLS_PORT2_LEARN_ENCAP) {
                    /* Learn encap */
                	SOC_PPD_FRWRD_DECISION_PWE_PROTECTED_TUNNEL_WITH_OUTLIF_SET(unit, &(pwe_info->learn_record.learn_info), _BCM_PETRA_L3_ITF_VAL_GET(mpls_port->egress_tunnel_if), mpls_port->encap_id, soc_sand_rv);
                }
                else {
                    /* allocate push-profile according to tunnel information */  
                    SOC_PPD_FRWRD_DECISION_PWE_PROTECTED_TUNNEL_SET(unit, &(pwe_info->learn_record.learn_info), mpls_port->egress_label.label, push_profile, _BCM_PETRA_L3_ITF_VAL_GET(mpls_port->egress_tunnel_if), soc_sand_rv);
                }
                BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                if(SOC_IS_PETRAB(unit)){
                    /* fix FEC format to be ethernet into Tunnel */
                    rv = _bcm_dpp_gport_fix_fec_type(unit, _BCM_PETRA_L3_ITF_VAL_GET(mpls_port->egress_tunnel_if), SOC_PPD_FEC_TYPE_BRIDGING_INTO_TUNNEL);
                    BCMDNX_IF_ERR_EXIT(rv);
                } else {
                    /* fix FEC format to be tunneling */
                    if ((is_ingress && is_egress) || is_forward_group) {
                        rv = _bcm_dpp_gport_fix_fec_type(unit, _BCM_PETRA_L3_ITF_VAL_GET(mpls_port->egress_tunnel_if), SOC_PPD_FEC_TYPE_TUNNELING);
                        BCMDNX_IF_ERR_EXIT(rv);
                    }
                }
              }
          ...
          soc_sand_rv = soc_ppd_l2_lif_pwe_add(unit, in_vc_label, lif_id, pwe_info, &failure_indication);
          BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
          SOC_SAND_IF_FAIL_RETURN(failure_indication);

          /* store match cretiria used for this LIF */
          /* store gport this LIF is mapped to */
          rv = _bcm_dpp_in_lif_mpls_match_add(unit,mpls_port,lif_id);
          BCMDNX_IF_ERR_EXIT(rv);

          /* store vsi */
          rv = _bcm_dpp_in_lif_match_vsi_set(unit, lif_id, vpn);
          BCMDNX_IF_ERR_EXIT(rv);
      }
      else /* p2p */ 
      {
          /* update VPN sw state, */
          if(update) {
              flags |= _BCM_DPP_PORT_REPLACE;
          }
          if(with_id) {
              flags |= _BCM_DPP_PORT_WITH_ID;
          }
          if(encap_with_id) {
              flags |= _BCM_DPP_PORT_ENCAP_WITH_ID;
          }

          flags |= _BCM_DPP_PORT_SAME_PEER_ONLY; /* in mlps API, VPN can include only mpls ports */

          /*1. P2P attributes */
          pwe_info->vsid = _BCM_PPD_VSI_P2P_SERVICE;
          pwe_info->learn_record.enable_learning = FALSE;
          pwe_info->service_type = SOC_PPD_L2_LIF_PWE_SERVICE_TYPE_P2P;
          /* no protection & use egress-tunnel*/
          pwe_info->default_frwrd.default_frwd_type = SOC_PPD_L2_LIF_DFLT_FRWRD_SRC_LIF;
          ...
          if (is_gal) {
              ...
              pwe_info->has_gal = 1;
              pwe_info->lif_table_entry_in_lif_field = base_pwe_lif;
          }
          ...
          /* set flag depending if this is working or protect port  */
          if(is_1_to_1_protected) {
              if(_BCM_PPD_GPORT_IS_WORKING_PORT(mpls_port)) {
                  flags |= _BCM_DPP_PORT_WORK;
              }
              else{
                  flags |= _BCM_DPP_PORT_PORTECT;
              }
          }

          /* considere my peer not exist */
          if(SOC_IS_PETRAB(unit)){
              SOC_PPD_FRWRD_DECISION_TRAP_SET(unit, &pwe_info->default_frwrd.default_forwarding, _BCM_PETRA_UD_DROP_TRAP, 5, 0, soc_sand_rv);
          }
          else {
          ...
          if((is_local_conf || _Bcm_dpp_sync_all)) {
              ...
              if(SOC_IS_PETRAB(unit)){
                  /* fix FEC format to br ethernet into Tunnel */
                  rv = _bcm_dpp_gport_fix_fec_type(unit, _BCM_PETRA_L3_ITF_VAL_GET(mpls_port->egress_tunnel_if), SOC_PPD_FEC_TYPE_BRIDGING_INTO_TUNNEL);
                  BCMDNX_IF_ERR_EXIT(rv);
              }
          }
    ...
    if(update && !remote_lif && is_ingress) {
        ...
        if(mpls_port->match_label != mpls_port_o.match_label){
            ...
            soc_sand_rv = soc_ppd_l2_lif_pwe_remove(unit, in_vc_label, TRUE/*remove from SEM*/, is_gal, &lif_id_o);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    ...
    bcm_mpls_port_t *   mpls_port)
{
   ...
   _bcm_lif_type_e used_type;
    ...
    SOC_PPD_EG_ENCAP_ENTRY_INFO
            eep_encap_info_o[SOC_PPD_NOF_EG_ENCAP_EEP_TYPES];
    uint32 next_eep[SOC_PPD_NOF_EG_ENCAP_EEP_TYPES], nof_entries;
    _bcm_dpp_gport_info_t gport_info;
    int is_local_conf;
    BCMDNX_INIT_FUNC_DEFS;
    ...
    SOC_PPD_EG_ENCAP_ENTRY_INFO_clear(&eep_encap_info_o[0]);
    ...
    /* for PetraB p2p orientation has to be spoke*/
    if(SOC_IS_PETRAB(unit) && _BCM_DDP_IS_CROSS_CONNECT_P2P_VPN(vpn)) {
        if((mpls_port->flags & BCM_MPLS_PORT_NETWORK)) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("For p2p cross connect, the orientation can't be HUB"))); 
        }
    }
    ...
    if (SOC_IS_PETRAB(unit) && (mpls_port->flags & BCM_MPLS_PORT_ENTROPY_ENABLE)) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Entropy is not supported for Soc_petra-B, flags 0x%x "),mpls_port->flags));        
    }

    if (mpls_port->flags2 & BCM_MPLS_PORT2_LEARN_ENCAP) {
        if (!_BCM_PETRA_L3_ITF_IS_FEC(mpls_port->egress_tunnel_if)) {
    ...
    rv = _bcm_dpp_gport_get_used_type(unit, mpls_port->mpls_port_id, &used_type);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    if (update) {
        if (!with_id) {
            ...
            o_encap_id = BCM_ENCAP_ID_GET(mpls_port_o.encap_id);
            soc_sand_rv = soc_ppd_eg_encap_entry_get(unit,
                                 SOC_PPC_EG_ENCAP_EEP_TYPE_LIF_EEP, o_encap_id, 0,
                                 eep_encap_info_o, next_eep, &nof_entries);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);

            if (eep_encap_info_o[0].entry_type != SOC_PPD_EG_ENCAP_ENTRY_TYPE_PWE) {
                BCMDNX_ERR_EXIT_MSG(BCM_E_INTERNAL, (_BSL_BCM_MSG("Encapsulation entry type must be PWE")));  /* has to be pwe */
            }

            /* check replace is possible, i.e. update attribute is possible */
            rv = _bcm_gport_mpls_is_valid_replace(unit, mpls_port, &mpls_port_o, &eep_encap_info_o[0].entry_val.pwe_info);
            BCM_IF_ERROR_RETURN(rv);
        }
    }
    /* If mpls port ingress is configured need to check that it wasn't configured previously */
        ...
        if (with_id && (used_type != _bcmDppLifTypeAny) && (mpls_port->flags2 & BCM_MPLS_PORT_WITH_GAL)) {
    ...
    */
    ...
    /* verify IDs handling */
    /* cannot be remote when ID is not ready */
    if(!is_local_conf && !(mpls_port->flags & BCM_MPLS_PORT_WITH_ID)) {
      BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("In case MPLS port is not local BCM_MPLS_PORT_WITH_ID must be set")));
    }
    
    /* configure the used port to be metro port */
    SOC_PPD_PORT_INFO_clear(&port_info);
    if ((mpls_port->port != BCM_GPORT_INVALID) && is_local_conf)
    {    
      /* get port info */
      BCM_PBMP_ITER(gport_info.pbmp_local_ports,local_port) {
          /* set port attributes to act as metro port */
    ...
    if(!_BCM_PPD_IS_VALID_FAILOVER_ID(mpls_port->failover_id) && (mpls_port->failover_port_id == BCM_GPORT_TYPE_NONE) && !(mpls_port->flags2 & BCM_MPLS_PORT2_EGRESS_ONLY)) {
    ...
    if ((mpls_port->flags2 & BCM_MPLS_PORT2_INGRESS_ONLY) || (mpls_port->flags2 & BCM_MPLS_PORT2_EGRESS_ONLY)) {
        ...
        if ((mpls_port->flags & BCM_MPLS_PORT_WITH_ID) == 0) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Setting flags of BCM_MPLS_PORT2_EGRESS_ONLY/INGRESS_ONLY, is supported only with BCM_MPLS_PORT_WITH_ID flag set")));
        }
    }

    
    if (mpls_port->flags2 & BCM_MPLS_PORT2_INGRESS_ONLY) {
        if (!_BCM_PETRA_L3_ITF_IS_FEC(mpls_port->egress_tunnel_if) && (mpls_port->egress_tunnel_if != 0)) {
    ...
    /* PWE configurations */
    rv = _bcm_dpp_mpls_port_pwe_set(unit,vpn,mpls_port,&pwe_info, &eep_encap_info_o[0].entry_val.pwe_info);
    BCMDNX_IF_ERR_EXIT(rv);
...
_bcm_dpp_gport_mgmt_test(void)
{
    ...
    vp1.criteria = BCM_VLAN_PORT_MATCH_PORT_VLAN_STACKED;
    vp1.egress_vlan = 100;
    vp1.egress_inner_vlan = 200;
    vp1.flags = 0;
    vp1.match_vlan = 10;
    vp1.match_inner_vlan = 20;
    BCM_GPORT_LOCAL_SET(vp1.port,10);
    vp1.vsi = 10;

    rv = bcm_petra_vlan_port_create(unit,&vp1);
    BCMDNX_IF_ERR_EXIT(rv);
exit:
    BCMDNX_FUNC_RETURN;
}
...
STATIC int
_bcm_dpp_gport_trill_route_info_state_dump_cb(int unit, shr_htb_key_t key, shr_htb_data_t data)
{
    int                                    rc = BCM_E_NONE, i;
    _bcm_petra_trill_mc_trill_route_list_t *state = NULL;
    bcm_gport_t                            *gport;


    state = (_bcm_petra_trill_mc_trill_route_list_t *)data;
    gport = (bcm_gport_t *)key;
    LOG_VERBOSE(BSL_LS_BCM_PORT,
                (BSL_META_U(unit,
                            "Key: 0x%x, nof_routes 0x%x: \n"), (*gport), state->nof_routes));
    for (i=0; i < state->nof_routes; i++) {
        LOG_VERBOSE(BSL_LS_BCM_PORT,
                    (BSL_META_U(unit,
                                "\tflags: 0x%x, root_name: 0x%x, vlan 0x%x, group 0x%x\n"), 
                                state->route[i].flags, state->route[i].root_name,
                     state->route[i].vlan,state->route[i].group));
    }
    return(rc);
}

STATIC int
_bcm_dpp_gport_trill_root_src_info_state_dump_cb(int unit, shr_htb_key_t key, shr_htb_data_t data)
{
    int                                    rc = BCM_E_NONE, i;
    _bcm_petra_trill_mc_trill_src_list_t  *state = NULL;
    bcm_gport_t                           *gport;


    state = (_bcm_petra_trill_mc_trill_src_list_t *)data;
    gport = (bcm_gport_t *)key;
    LOG_VERBOSE(BSL_LS_BCM_PORT,
                (BSL_META_U(unit,
                            "Key: 0x%x, nof_src_rbridge 0x%x: \n"), (*gport), state->nof_src_rbridge));
    for (i=0; i < state->nof_src_rbridge; i++) {
        LOG_VERBOSE(BSL_LS_BCM_PORT,
                    (BSL_META_U(unit,
                                "\tsrc_rbridge: 0x%x, Port: 0x%x\n"), 
                                state->src_rbridge[i].src_rbridge, state->src_rbridge[i].port));
    }
...
_bcm_dpp_gport_sw_dump(int unit)
{
    ...
    if(SOC_IS_PETRAB(unit))
    {
        /* dump trill route info state info */
        LOG_VERBOSE(BSL_LS_BCM_PORT,
                    (BSL_META_U(unit,
                                "\nTrill Route Info\n")));
        rc = _bcm_dpp_sw_db_hash_trill_info_iterate(unit, _bcm_dpp_gport_trill_route_info_state_dump_cb);
        if (rc != BCM_E_NONE) {
            return(rc);
        }

        /* dump trill root src state info */
        LOG_VERBOSE(BSL_LS_BCM_PORT,
                    (BSL_META_U(unit,
                                "\nTrill Root Src Info\n")));
        rc = _bcm_dpp_sw_db_hash_trill_info_iterate(unit, _bcm_dpp_gport_trill_root_src_info_state_dump_cb);
        if (rc != BCM_E_NONE) {
            return(rc);
        }
    }
