                              ...
                              uint32 *hwTrapCode)
{
    ...
    *hwTrapCode = 0;
    ...
    if (SOC_IS_ARAD(unit)) {
        ...
        ppdr = arad_pp_trap_mgmt_trap_code_to_internal(ppdTrapCode,
                                                     &intTrapCode_arad,
                                                     &dc0_arad,
                                                     &dc1_arad);
                                          ...
                                          bcm_field_AppType_t *bcmAppType)
{
    ...
    *bcmAppType = bcmFieldAppTypeCount;
    ...
    if (SOC_IS_ARAD(unit)) {
        ...
        sandResult = arad_pp_sw_db_flp_prog_index_to_app_get((unit), ppd_flp_program, &ppdAppType);
        BCMDNX_IF_ERR_EXIT(handle_sand_result(sandResult));
...
int
_bcm_dpp_field_ip_frag_bcm_to_ppd(bcm_field_IpFrag_t bcmFragInfo,
...
_bcm_dpp_field_presel_port_profile_alloc_dealloc(_bcm_dpp_field_info_t *unitData,
                                                 _bcm_dpp_field_profile_type_t profile_type,
                                                 uint8 alloc,
                                                 uint32 port_bitmap[SOC_TMC_NOF_FAP_PORTS_MAX_IN_LONGS], 
                                                 uint64 *profile) /* if alloc=0 then IN else OUT */
{
    uint32 
        ...
        tmp_bitmap[SOC_PPC_FP_NOF_CONTROL_VALS];

    BCMDNX_INIT_FUNC_DEFS_NO_UNIT;

    unit = unitData->unit;

    sal_memset(empty_bitmap, 0, sizeof(uint32) * SOC_PPC_FP_NOF_CONTROL_VALS);
    ...
    if(!alloc)
    {
        ...
        for(profile_idx = 1; profile_idx < _BCM_DPP_PRESEL_NOF_PORT_PROFILES; profile_idx++) 
        {
            ...
            if(0 != sal_memcmp(tmp_bitmap, empty_bitmap, sizeof(uint32) * SOC_PPC_FP_NOF_CONTROL_VALS))
            {
                /* If port bitmaps are equal, then the same profile
                 * should be given to this PFG as well.
                 */
                if(0 == sal_memcmp(port_bitmap, tmp_bitmap, sizeof(uint32) * SOC_PPC_FP_NOF_CONTROL_VALS)) {
                    COMPILER_64_SET(*profile, 0, profile_idx);
                    break;
                }
                else {
                    /* Return error if intersection */
                    SHR_BITAND_RANGE(tmp_bitmap, port_bitmap, 0, nof_profile_objects, tmp_bitmap);
                    if (0 != sal_memcmp(tmp_bitmap, empty_bitmap, sizeof(uint32) * SOC_PPC_FP_NOF_CONTROL_VALS)) {
                                       ...
                                       uint32 port_bitmap[SOC_TMC_NOF_FAP_PORTS_MAX_IN_LONGS])
{
    ...
    sal_memcpy(port_bitmap, control_info.val, sizeof(uint32) * SOC_PPC_FP_NOF_CONTROL_VALS);
...
int
_bcm_dpp_field_ip_frag_ppd_to_bcm(uint8 ppdIpFragmented,
                                    bcm_field_IpFrag_t *bcmFragInfo)
{
    switch (ppdIpFragmented) {
    case 0:
        *bcmFragInfo = bcmFieldIpFragNon;
        break;
    case 1:
        *bcmFragInfo = bcmFieldIpFragAny;
        break;
    default:
        return BCM_E_PARAM;
    }
