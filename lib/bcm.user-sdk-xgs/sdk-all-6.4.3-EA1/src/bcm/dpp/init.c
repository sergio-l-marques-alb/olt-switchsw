...
bcm_dpp_state_t _dpp_state[BCM_MAX_NUM_UNITS];
...
bcm_petra_init(int unit)
{
    ...
    BCM_IF_ERROR_RETURN(_bcm_petra_lock_init(unit));

    /* init state pointer */
    if (!SOC_WARM_BOOT(unit)) {
        sal_memset(&_dpp_state[unit], 0x0, sizeof(bcm_dpp_state_t));
    }
    SOC_DPP_CONTROL(unit)->state = &(_dpp_state[unit]);

    /* first thing we do is recover from scache if warm boot */
    
    rv = _bcm_sw_db_init(unit);
    BCMDNX_IF_ERR_EXIT(rv);

    /* Scache API completion for consistency state must be as early as possible */
    rv = _bcm_dpp_switch_init(unit);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    if (SOC_IS_JERICHO(unit)) {
        BCMDNX_IF_ERR_EXIT(soc_jer_egr_tm_init(unit));
    }
#endif

    rv = bcm_dpp_counter_init(unit);
    BCMDNX_IF_ERR_EXIT(rv);

    if (pp_enable) {
        rv = _bcm_dpp_gport_mgmt_init(unit);
        BCMDNX_IF_ERR_EXIT(rv);
    }
    ...
    if (pp_enable) {
        ...
        if (SOC_DPP_CONFIG(unit)->trill.mode) {
            rv = bcm_petra_trill_init_data(unit);
            BCMDNX_IF_ERR_EXIT(rv);
        }
...
#ifdef BCM_ARAD_SUPPORT
    if (SOC_IS_ARAD(unit) && soc_feature(unit, soc_feature_time_support)) {
        INIT_CALL_CHK(time);
    }
#ifdef INCLUDE_PTP
    if (SOC_IS_ARAD(unit) && soc_feature(unit, soc_feature_ptp)) {
    ...
    if (SOC_IS_ARAD(unit)) {
        if (SOC_WARM_BOOT(unit)) {
            ...
            /*
             * Update and start threads with relevant information
             */
            /*Recover stat module - counter ports bitmap*/
            BCM_IF_ERROR_RETURN(_bcm_petra_stat_recover(unit, okay_nif_ports, counter_flags, counter_interval, counter_pbmp));
...
_bcm_petra_modules_deinit(int unit, int silent)
{
    ...
    /* SOC_PETRA_DETACH(unit, bcm_petra_tx_deinit, rv); no function */
    ...
    if (pp_enable) {
        ...
        SOC_PETRA_DETACH(unit, bcm_petra_oam_detach, rv); /* init is done only in WB */
        SOC_PETRA_DETACH(unit, bcm_petra_bfd_detach, rv); /* init is done only in WB */
