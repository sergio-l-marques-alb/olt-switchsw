...
{   
    ...
    if (SOC_DPP_CONFIG(unit)->tdm.is_bypass && !SOC_WARM_BOOT(unit)) {
      ...
      soc_sand_rc = (MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_tdm_direct_routing_set,(unit, template_init_id, &direct_routing)));
                                  ...
                                  bcm_fabric_tdm_direct_routing_t *routing_info)
{
    ...
    BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_i) {
      ...
      if (is_allocated) {
          soc_sand_rc = (MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_tdm_direct_routing_set,(unit, new_link_ptr, &direct_routing_information)));
          BCM_SAND_IF_ERR_EXIT(soc_sand_rc);
      }
      ...
      if (is_last && (new_link_ptr != old_link_ptr)) {
          soc_sand_rc = (MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_tdm_direct_routing_set,(unit, old_link_ptr, &cleaned_direct_routing_information)));
          BCM_SAND_IF_ERR_EXIT(soc_sand_rc);
      }
    }
exit:
