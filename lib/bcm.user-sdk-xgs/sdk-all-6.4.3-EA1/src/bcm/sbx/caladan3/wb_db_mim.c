...
_bcm_caladan3_wb_mim_state_layout_init (int unit, int version, unsigned int *scache_len)
{
    ...
    SBX_SCACHE_INFO_PTR(unit)->avl_tree_size = max - min;
    ...
    switch(version)
    {
        case BCM_CALADAN3_WB_MIM_VERSION_1_0:
            *scache_len = 0;
            ...
            SBX_WB_DB_LAYOUT_INIT_NV(bcm_caladan3_mim_vpn_control_t, max-min);
            SBX_WB_DB_LAYOUT_INIT_NV(int16, (max-min)*3);
            SBX_WB_DB_LAYOUT_INIT_NV(bcm_caladan3_mim_port_control_t, (max-min)*2);
