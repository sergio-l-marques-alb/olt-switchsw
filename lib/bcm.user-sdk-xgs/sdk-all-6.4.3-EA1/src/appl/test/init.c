...
init_deinit_test(int unit, args_t *a, void *p)
{
    ...
    if (NULL != stable_filename) {
        sal_strncat(file_name1, stable_filename, sizeof(file_name1));
        sal_strncat(file_name2, stable_filename, sizeof(file_name2));
    ...
    {   
        ...
        for (j = 0; j < loop; j++) {
            ...
            if((!init_param->no_sync) && (init_param->warmboot)) {  
                ...
                cli_out("%d: Sync sw state to persistant storage.\n", unit);
