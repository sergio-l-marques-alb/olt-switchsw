...
diag_dfe_fabric_thresholds_types_parse(int unit, char *th_name, diag_dnx_fabric_link_th_info_t *thresholds_info)
{
    ...
    } else if (!sal_strncasecmp(th_name, "GCI", strlen(th_name))) {
        /*RX*/
        thresholds_info->th_types_rx_nof = 3;
    ...
    } else if (!sal_strncasecmp(th_name, "DROP", strlen(th_name))) {
        /*RX*/
        if (SOC_IS_FE1600(unit))
        {
    ...
    } else if (!sal_strncasecmp(th_name, "ALMOST_FULL", strlen(th_name))) {
        if (!SOC_IS_FE1600(unit))
        {
            /*TX*/
            thresholds_info->th_types_tx_nof = 1;
    ...
    } else if (!sal_strncasecmp(th_name, "SIZE", strlen(th_name))) {
        if (!SOC_IS_FE1600(unit))
        {
            /*RX*/
            thresholds_info->th_types_rx_nof = 1;
    ...
    } else if (!sal_strncasecmp(th_name, "FULL", strlen(th_name))) {
        if (!SOC_IS_FE1600(unit))
        {
            /*RX*/
            thresholds_info->th_types_rx_nof = 1;
