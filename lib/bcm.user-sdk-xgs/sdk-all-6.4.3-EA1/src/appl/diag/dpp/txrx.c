...
setup_itmh_tx(xd_t *xd)
{
    ...
    if ( SOC_IS_ARAD(xd->xd_tx_unit)) { 
        ...
        rv = arad_port_header_type_get_unsafe(soc_sand_dev_id, 0 /* cpu port */, 
                                                &incoming, &outgoing);
    }
#endif /* BCM_ARAD_SUPPORT */
    if (BCM_FAILURE(rv) || 
...
xd->xd_tx_unit, bcm_errmsg(rv)));
        ...
        soc_sand_rv = arad_port_header_type_get((xd->xd_unit), port_ndx, &header_type_incoming, &header_type_outgoing); 
