...
int rx_los_port_enable(int unit, bcm_port_t port, int enable, int warm_boot) {
    ...
    if (enable) {
        ...
        if (interrupt_id1 != -1) {
            ...
            rv = soc_interrupt_flags_get(unit, interrupt_id1, &flags);
            BCM_FUNC_IF_ERR(rv) {
                LOG_ERROR(BSL_LS_APPL_PORT,
                          (BSL_META_U(unit,
            ...
            flags |= SOC_INTERRUPT_DB_FLAGS_UNMASK_AND_CLEAR_DISABLE;
            rv = soc_interrupt_flags_set(unit, interrupt_id1, flags);
            BCM_FUNC_IF_ERR(rv) {
                LOG_ERROR(BSL_LS_APPL_PORT,
                          (BSL_META_U(unit,
        ...
        if (interrupt_id2 != -1) {
            rv = soc_interrupt_flags_get(unit, interrupt_id2, &flags);
            BCM_FUNC_IF_ERR(rv) {
                LOG_ERROR(BSL_LS_APPL_PORT,
                          (BSL_META_U(unit,
                                      "ERROR: rx_los_port_enable - soc_interrupt_flags_get FAILED\n")));
            ...
            flags |= SOC_INTERRUPT_DB_FLAGS_UNMASK_AND_CLEAR_DISABLE;
            rv = soc_interrupt_flags_set(unit, interrupt_id2, flags);
            BCM_FUNC_IF_ERR(rv) {
                LOG_ERROR(BSL_LS_APPL_PORT,
                          (BSL_META_U(unit,
...
int rx_los_unit_detach(int unit){
    ...
    PBMP_ITER(pbmp,port){
        rv = rx_los_port_enable(unit, port, 0, 0);
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_APPL_PORT,
                      (BSL_META_U(unit,
