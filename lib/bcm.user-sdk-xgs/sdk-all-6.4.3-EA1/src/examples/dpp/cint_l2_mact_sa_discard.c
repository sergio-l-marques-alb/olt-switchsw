/*
* $Id: cint_l2_mact_sa_discard.c,v 1.5 Broadcom SDK $
...
l2_learning_run_with_defaults(0, -1);	
rv = bcm_switch_control_set(0, bcmSwitchL2LearnMode, BCM_L2_INGRESS_DIST);
rv = bcm_port_tpid_set(0,13,0x8100);
//run SA-discard example
rv = l2_sa_discard_example(0,13,-1,13);
 *  
 */
 ...
 */
int
 l2_sa_discard_example(int unit, int in_port_1, int in_port_2, int dest_port) {
    int rv = 0;
    ...
    rv = l2_sa_discard_config_traps(unit);
    if (rv != BCM_E_NONE) {
   ...
   /* for port_2 trap packet for SA-discard */
    if(in_port_2 != -1){
    	rv = bcm_port_control_set(unit, in_port_2, bcmPortControlDiscardMacSaAction, BCM_PORT_CONTROL_DISCARD_MACSA_DROP|BCM_PORT_CONTROL_DISCARD_MACSA_TRAP);
    	if (rv != BCM_E_NONE) {
...
int
 l2_sa_discard_config_traps(int unit) {
 	int rv = BCM_E_NONE;
  ...
  if(drop_trap_id == 0){
		rv = bcm_rx_trap_type_create(unit,BCM_RX_TRAP_WITH_ID,bcmRxTrapL2DiscardMacsaTrap,&drop_trap_id);
		if (rv != BCM_E_NONE) {
  ...
  if(redirect_trap_id == 0){
  	/* set drop to drop packet, this is also the driver default if filter is enabled*/
		rv = bcm_rx_trap_type_create(unit,BCM_RX_TRAP_WITH_ID,bcmRxTrapL2DiscardMacsaDrop,&redirect_trap_id);
		if (rv != BCM_E_NONE) {
...
int l2_sa_discard_example_run_with_dvapi(int unit, int in_port_1, int in_port_2, int out_port) {
    int rv = 0;
    ...
    print(rv);
    //run SA-discard example
    rv = l2_sa_discard_example(unit,in_port_1,in_port_2,out_port);
    print(rv);
    return rv;
}
