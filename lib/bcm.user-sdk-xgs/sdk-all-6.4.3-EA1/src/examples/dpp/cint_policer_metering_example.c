/* $Id: cint_policer_metering_example.c,v 1.15 Broadcom SDK $
...
/* two_meters_groups: if policer_ingress_count=64 then there is only one group of 64K meters 
 *                    if policer_ingress_count=32 then there are 2 groups of meters, each with 32K meters    
 *  
 * run:
 * cint ../../../../src/examples/dpp/cint_policer_metering_example
 * cint
 * metering_example(0, <is_arad>, <two_meters_groups>); 
 */  
int
metering_example(int unit, int is_arad, int two_meters_groups) {
    ...
    if (with_check){
        rv = compare_policers(metering_info.pol_cfg, metering_info.pol_cfg_get);
        if (rv != BCM_E_NONE) {
    ...
    print metering_info.pol_cfg_get;
    if (with_check){
        rv = compare_policers(metering_info.pol2_cfg, metering_info.pol2_cfg_get);
        if (rv != BCM_E_NONE) {
    ...
    if (with_check){
        rv = compare_policers(metering_info.pol3_cfg, metering_info.pol3_cfg_get);
        if (rv != BCM_E_NONE) {
    ...
    if (with_check){
        rv = compare_policers(metering_info.pol3_cfg, metering_info.pol_cfg_get);
        if (rv != BCM_E_NONE) {
            printf("Error in bcm_policer_get no. 3. Did not get the same policer.\n");
            print rv;
            return rv;
    ...
    if (is_arad) {
        ...
        metering_info.pol4_cfg.ckbits_sec = 300000000; /* max rate 300GB */
        metering_info.pol4_cfg.pkbits_sec = 260000000;
        metering_info.pol4_cfg.max_ckbits_sec = 300000000; /* when sharing is enabled, max_ckbits_sec is also used */
        /* maximal committed rate including tokens transferred from a higher class buckets (in case of sharing) */
        metering_info.pol4_cfg.max_pkbits_sec = 270000000; /* in ARAD, max_pkbits_sec is always used */
        ...
        if (with_check){
            for (i = 0 ; i < 4 ; i++) {
                ...
                rv = compare_policers(metering_info.pol4_cfg, metering_info.pol4_cfg_get);
                if (rv != BCM_E_NONE) {
        ...
        if (with_check){
            for (i = 0 ; i < 4 ; i++) {
                ...
                if (i == 2) {
                    ...
                    rv = compare_policers(metering_info.pol5_1_cfg, metering_info.pol5_cfg_get);
                }
                else {
                    rv = compare_policers(metering_info.pol5_cfg, metering_info.pol5_cfg_get);
                }
                if (rv != BCM_E_NONE) {
        ...
        if (with_check) {
            rv = compare_policers(metering_info.same_prf_1_pol_cfg, metering_info.same_prf_1_pol_cfg_get);
            if (rv != BCM_E_NONE) {
        ...
        if (with_check) {
            rv = compare_policers(metering_info.same_prf_2_pol_cfg, metering_info.same_prf_2_pol_cfg_get);
            if (rv != BCM_E_NONE) {
        ...
        if (with_check) {
            ...
            rv = compare_policers(metering_info.same_prf_2_pol_cfg, metering_info.same_prf_1_pol_cfg_get);
            if (rv != BCM_E_NONE) {
    ...
    metering_info.pol3_cfg.ckbits_burst = 0;
    if (is_arad) {
    ...
    rv = compare_policers(metering_info.pol6_cfg, metering_info.pol6_cfg_get);
    if (rv != BCM_E_NONE) {
...
/* if policer_ingress_count=64 then there is only one group of 64K meters 
 * if policer_ingress_count=32 then there are 2 groups of meters, each with 32K meters    
 *  
 * run:
 * cint ../../../../src/examples/dpp/cint_policer_metering_example.c
 * cint
 * metering_replace_example(0, <is_arad>); 
 */ 
int
metering_replace_example(int unit, int is_arad) {
    ...
    if (with_check){
        rv = compare_policers(metering_info.pol_cfg_get, metering_info.pol2_cfg_get);
        if (rv != BCM_E_NONE) {
    ...
    for (i = 0; i < policer_nof; i++) {
        ...
        if (with_check){
            rv = compare_policers(eth_pol_cfg, eth_pol_cfg_get);
            if (rv != BCM_E_NONE) {
 ...
 * cint
 * traffic_example(0, <in_port>, <out_port>, <is_arad>, <single_rate>, <color_blind>); 
 *  
 * traffic to run: 
 * for Arad (is_arad=1):
 ...
 */
int 
traffic_example(int unit, int in_port, int out_port, int is_arad, int single_rate, int color_blind) {
    ...
    if (!is_arad) {
        ...
        if (rv != BCM_E_NONE) {
            printf("Error in bcm_field_qualify_DstPort with dest_port %d\n", DstPortModValue);
            print rv;
            return rv;
        }
        ...
        rv = bcm_field_entry_policer_attach(unit, ent, 0, policer_id);
        if (rv != BCM_E_NONE) {
 ...
 * cint examples/cint_policer_metering_example
 * cint
 * two_meterw_per_packet_example(0, <out_port>);  
 *  
...
two_meter_per_packet_example(int unit, int out_port) {
    ...
    rv = bcm_field_entry_policer_attach(unit, ent1, 0, policer_id);
    if (rv != BCM_E_NONE) {
        printf("Error in bcm_field_entry_policer_attach with policer_id %d\n", policer_id);
        print rv;
        return rv;
    }
    rv = bcm_field_entry_policer_attach(unit, ent1, 1, policer_id_share);
    if (rv != BCM_E_NONE) {
        printf("Error in bcm_field_entry_policer_attach with policer_id %d\n", policer_id_share);
        print rv;
        return rv;
    ...
    rv = bcm_field_entry_policer_attach(unit, ent2,  0, policer_id);
    if (rv != BCM_E_NONE) {
        printf("Error in bcm_field_entry_policer_attach with policer_id %d\n", policer_id);
        print rv;
        return rv;
    }
    rv = bcm_field_entry_policer_attach(unit, ent2,  1, policer_id_share);
    if (rv != BCM_E_NONE) {
        printf("Error in bcm_field_entry_policer_attach with policer_id %d\n", policer_id_share);
        print rv;
        return rv;
 ...
 * cint examples/cint_policer_metering_example
 * cint
 * discard_traffic_by_dp(0, <in_port>, <out_port>); 
 *  
 * traffic to run: 
...
int metering_example_with_check(int unit, int is_arad, int two_meters_groups) {
    int rv = BCM_E_NONE;
    with_check = 1;
    rv = metering_example(unit,is_arad,two_meters_groups);
    if (rv != BCM_E_NONE) {
...
int compare_policers(bcm_policer_config_t original, bcm_policer_config_t other){
        ...
        if (check_five_percent_variation(original.ckbits_sec, other.ckbits_sec)){
        ...
        if (check_five_percent_variation(original.pkbits_sec, other.pkbits_sec)){
        ...
        if (check_five_percent_variation(original.max_ckbits_sec, other.max_ckbits_sec)){
        ...
        if (check_five_percent_variation(original.max_pkbits_sec, other.max_pkbits_sec)){
        ...
        if (check_five_percent_variation(original.pkbits_burst, other.pkbits_burst)){
        ...
        if (check_five_percent_variation(original.ckbits_burst, other.ckbits_burst)){
...
int check_five_percent_variation(int original, int other){
    if (original != 0 &&
        (other < original * 0.95 || 
         other > original * 1.05)){
        return -1;
