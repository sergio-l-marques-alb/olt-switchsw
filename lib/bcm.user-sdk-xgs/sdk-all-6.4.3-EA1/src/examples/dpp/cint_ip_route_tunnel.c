 ...
 *  
 */
int example_ip_to_tunnel_4_label_push(int *units_ids, int nof_units, int in_sysport, int out_sysport){
    int rv;
    int unit, i;
    int ing_intf_in; 
    int ing_intf_out; 
    ...
    for (i = 0 ; i < nof_units ; i++){
        unit = units_ids[i];
        rv = create_tunnel_initiator_4_label_push(unit, &ing_intf_out, &tunnel_id);
        if (rv != BCM_E_NONE) {
          printf("Error, in create_tunnel_initiator\n");
          return rv;
        }
