...
pvid_port_set(int unit, int port, bcm_vlan_t pvid, bcm_vlan_t vsi) {
    ...
    if (advanced_vlan_translation_mode) { 
        rv = trill_advanced_vlan_translation_map_local_vsi_to_global_vsi_2(unit, vsi, port_vlan.egress_vlan, port_vlan.vlan_port_id, trill_mode);
        if (rv != BCM_E_NONE) {
...
trill_uni_untag_fgl(int unit, int double_tag, int is_tts) {
    ...
    if (is_tts) {
        vpn_config.flags = BCM_BCM_TRILL_VPN_NETWORK | BCM_BCM_TRILL_VPN_ACCESS | BCM_BCM_TRILL_VPN_TRANSPARENT_SERVICE;
    } else {
