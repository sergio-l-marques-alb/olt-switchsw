...
*/
 ...
 printf("1\n");
    ...
    port = idx + 1;
    system_port = idx + 1;
    BCM_GPORT_MODPORT_SET(dest_gport[idx], c6_my_modid, port);
    BCM_GPORT_MODPORT_SET(src_gport[idx], c6_my_modid, port);
    BCM_GPORT_SYSTEM_PORT_ID_SET(sys_gport[idx], system_port);

    /* src=dest in this example application, 1 node */
    /*src_gport[idx] = dest_gport[idx];*/
...
printf("\5\n");
    ...
    BCM_COSQ_GPORT_SCHED_CIR_SET(c6_gport_ucast_scheduler_cir[idx], c6_gport_ucast_scheduler[idx]);
    BCM_COSQ_GPORT_SCHED_PIR_SET(c6_gport_ucast_scheduler_pir[idx], c6_gport_ucast_scheduler[idx]);
...
printf("\7\n");
     ...
     */          
    flags = BCM_COSQ_GPORT_VOQ_CONNECTOR | default_flags;
    rv = bcm_cosq_gport_add(c6_unit, src_gport[idx], num_cos, 
                flags, &c6_gport_ucast_voq_connector_group[idx]);
    if (rv != BCM_E_NONE) {
