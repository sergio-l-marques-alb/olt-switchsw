 ...
 *
 */
int l2gre_ipv4_mc_route(int unit, int in_port, int out_port)
{
    int i;
    ...
    rv = bcm_multicast_create(unit, BCM_MULTICAST_INGRESS_GROUP | BCM_MULTICAST_TYPE_L3 | BCM_MULTICAST_WITH_ID, &ipmc_index); /* 3.1 */
    if (rv != BCM_E_NONE) {
        printf("Error, bcm_multicast_create \n");
        return rv;
    }

    rv = add_ingress_multicast_forwarding(unit,ipmc_index, out_port, 1, vlan); 
    if (rv != BCM_E_NONE) {
...
int l2_gre_mc_bud_node_example(int unit, int in_port, int de_encap_out_port, int route_out_port, int recycle_port)
{
    ...
    rv = l2gre_ipv4_mc_route(unit, in_port, route_out_port);
    if (rv != BCM_E_NONE) {
