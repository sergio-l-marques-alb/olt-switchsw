 ...
 *                bcm_l3_egress_create flag BCM_BCM_L3_ENCAP_SPACE_OPTIMIZED should be set to configure FEC format to OutLif.
...
mpls_tunnels_config(int unit, int second_unit, int extend_example){
    ...
    flags = encap_optimized ? BCM_BCM_L3_ENCAP_SPACE_OPTIMIZED : CINT_NO_FLAGS;
    ...
    if (mpls_termination_label_index_enable) {
        BCM_MPLS_INDEXED_LABEL_SET(&label_for_pop_entry_out, vswitch_vpls_info_1.out_tunnel_label, 2);
    ...
    if (extend_example) {
        if (mpls_termination_label_index_enable) {
            BCM_MPLS_INDEXED_LABEL_SET(&label_for_pop_entry_out, vswitch_vpls_info_2.out_tunnel_label, 2);
...
/* create VPN */
int
vswitch_vpls_vpn_create(int unit, bcm_vpn_t vpn){
...
vswitch_vpls_add_network_port_1(int unit, int second_unit,  bcm_gport_t *port_id){
    ...
    if (ingress_egress_separate_ports) {
        ...
        *port_id = mpls_port_1.mpls_port_id;
        ...
        if (mpls_termination_label_index_enable) {
            BCM_MPLS_INDEXED_LABEL_SET(&mpls_port_1.match_label, vswitch_vpls_info_1.in_vc_label, is_gal?2:3);
        }
        else {
            mpls_port_1.match_label = vswitch_vpls_info_1.in_vc_label;
        }
...
vswitch_vpls_add_network_port_2(int unit,  bcm_gport_t *port_id){
    ...
    /* set port attribures, key <BC>*/
    if (mpls_termination_label_index_enable) {
        BCM_MPLS_INDEXED_LABEL_SET(&mpls_port_1.match_label, vswitch_vpls_info_2.in_vc_label, is_gal?2:3);
    }
    else {
         /* if testing make before break, the vc label should be the same for both mpls ports */
        mpls_port_1.match_label = make_before_break ? vswitch_vpls_info_1.in_vc_label : vswitch_vpls_info_2.in_vc_label;
    }
