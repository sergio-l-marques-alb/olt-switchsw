...
bcm_port_t pon_port;
bcm_port_t nni_port_1;
bcm_port_t nni_port_2;
...
int pon_cpu_service_Init(int unit, bcm_port_t port_pon, bcm_port_t port_nni_1, bcm_port_t port_nni_2)
{
    ...
    pon_port = port_pon;
    nni_port_1 = port_nni_1;
    nni_port_2 = port_nni_2;
...
{ 
    ...
    for (index = 0; index < num_of_tls_services; index++)    
    {
        ...
        {
            /* Create NNI LIF */            
            rv = nni_lif_create(unit, trunk_gport, nni_lif_type, 0, down_ovlan, down_ivlan, &nni_gport, &encap_id);    
            if (rv != BCM_E_NONE) {
            ...
            /*For upstream*/            
            rv = multicast_vlan_port_add(unit, tls_service_vsi, trunk_gport, nni_gport); 
            if (rv != BCM_E_NONE) {
                printf("multicast_vlan_port_add failed (%d) $rv\n", index);
                return rv;
            }
        ...
        pon_tls_service_info[index].nni_gport = nni_gport;
    }
...
{ 
    ...
    for (index = 0; index < num_of_tls_services; index++)
    {
        ...
        if (index == 0) 
        {
            rv = vswitch_delete_port(unit, tls_service_vsi, pon_tls_service_info[index].nni_gport);
            if (rv != BCM_E_NONE) {
                printf("vswitch_delete_port failed (%d) $rv\n", index);
                return rv;
            }
