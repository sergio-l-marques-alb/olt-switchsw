 ...
 * 2) make sure your makefile includes PTP feature.
 * 3) when broadsync is not enabled need to enable TS_PLL clock via soc properties (see 'Broadsync' section bellow).
 *
 * Provides the following functionalities:
 *
 * - 1588 one/step TC (Transparent clock). 
 ...
 *    CF update:
 *        the CF (8 bytes) update in the ARAD is done in bytes number 2,3 (zero based).
 *        also least significant bit can be changed for internal use.
 *
 *    Broadsync:
 *     1588 TC (Transparent clock) relay on clock synchronization between ARAD devices in the system i.e Broadsync,
 *     for Broadsync configuration see BCM shell "bs config" command.
 *     ( IMPORTANT: when Broadsync is not active, in order check TC, the TS_PLL clock should be enabled and running, 
 *                  otherwise the clock would not change, thus the CF won`t be updated. to enable TS_PLL clock,  
 *                  following soc properties need to be un-commented :
 ...
 * how to run:
 *
 * for example: enable one step TC for unit 0 port 13, 14 (both In-port and Out-port need ot be configured.
