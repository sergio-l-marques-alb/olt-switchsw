...
int ip_route_explicit_rif__start_run(int unit,  ip_route_explicit_rif_s *param){
    ...
    for (i=0; i< NUMBER_OF_PORTS; i++) {
        port__tpid__set(unit, g_ip_route_explicit_rif.rif[i].sys_port);
        if (rv != BCM_E_NONE) {
    ...
    rv = l3__egress__create(unit , g_ip_route_explicit_rif.rif[OUT_PORT].sys_port, 0, g_ip_route_explicit_rif.rif[OUT_PORT].rif, g_ip_route_explicit_rif.next_hop_mac, &fec, &encap_id); 
    if (rv != BCM_E_NONE) {
...
int ip_route_explicit_rif__l3_intf_create(int unit, int rif_index) {
    ...
    rc = l2__port_vlan__create(unit, g_ip_route_explicit_rif.rif[rif_index].sys_port, 
