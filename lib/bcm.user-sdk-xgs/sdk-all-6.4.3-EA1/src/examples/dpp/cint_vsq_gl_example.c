 ...
 *  1.  Run traffic from a port to itself (at around 80% load, 64B packets)
 *  2.  Set the above thresholds
 *  3.  You should see the RX traffic load stablize at around 56%
 *  4.  If you lower the Set threshold the traffic load will decrease.
 *  5.  If you raise the Set threshold the traffic load will increase.
 *
 */
 ...
 * vsq_global_init(0,7,5,3,1);
 * vsq_global_example(0); 
 * Run Traffic: Packet Destination 2, 10Gbps. 
 * Expected: Drop percentage 80%. 
 */
