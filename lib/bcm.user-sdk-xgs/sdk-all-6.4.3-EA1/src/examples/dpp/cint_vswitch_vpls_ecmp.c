 ...
 *   |  |0:11|  ||0x8100|    |100||Label:1000-1010|Label:1500-1510||Lable:2010||   ||     | | 
 *   |  +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+ |
 *   |                +~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~+                             |
 ...
 *   |  +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+ |
 *   |  | DA |SA||TIPD1 |Prio|VID||  MPLS         |  MPLS         ||   PWE    ||Eth||Data | |
 *   |  |0:11|  ||0x8100|    |100||Label:1000-1010|Label:1500-1510||Lable:1982||   ||     | | 
 *   |  +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+ |
 *   |                +~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~+                             |
 *   |                | Figure 15: Packets Received from PWE1 |                             |  
 *   ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
void
vswitch_vlps_info_init(int ac_port, int pwe_in_port, int pwe_out_port, int ac_port1_outer_vlan, int ac_port1_inner_vlan,
...
mpls_tunnels_config(int unit, int second_unit){
    ...
    for (i = 0; i < vswitch_vpls_ecmp_info.ecmp_size; i++) {
        /* create FEC */
        rv = create_l3_egress(unit, CINT_NO_FLAGS, mpls_lsr_info_1.eg_port/*can be different ports*/, mpls_lsr_info_1.eg_vid/*can be different vlans*/,
                              tunnel_ids[i], mpls_lsr_info_1.next_hop_mac, &vswitch_vpls_ecmp_info.egress_intfs[i], &encap_id[i]);
        if (rv != BCM_E_NONE) {
    ...
    if (second_unit >= 0) {
        ...
        for (i = 0; i < vswitch_vpls_ecmp_info.ecmp_size; i++) {
            /* create FEC */
            rv = create_l3_egress(second_unit, CINT_NO_FLAGS, mpls_lsr_info_1.eg_port/*can be different ports*/, mpls_lsr_info_1.eg_vid/*can be different vlans*/,
                                  tunnel_ids[i], mpls_lsr_info_1.next_hop_mac, &vswitch_vpls_ecmp_info.egress_intfs[i], &encap_id[i]);
            if (rv != BCM_E_NONE) {
                printf("Error, in create_l3_egress\n");
                return rv;
    ...
    for (i = 0; i < vswitch_vpls_ecmp_info.ecmp_size; i++) {
        ...
        if (mpls_termination_label_index_enable) {
            BCM_MPLS_INDEXED_LABEL_SET(&label_for_pop_entry_out, vswitch_vpls_ecmp_info.out_tunnel_label[i], 2);
...
vswitch_vpls_add_network_port_1(int unit, int second_unit,  bcm_gport_t *port_id){
    ...
    if (mpls_termination_label_index_enable) {
        BCM_MPLS_INDEXED_LABEL_SET(&mpls_port_1.match_label, vswitch_vpls_info_1.in_vc_label, 3);
    }
    else {
    ...
    mpls_port_1.egress_tunnel_if = vswitch_vpls_shared_info_1.egress_intf; /*Learning - MPLS ECMP */
    ...
    *port_id = mpls_port_1.mpls_port_id;
    ...
    mpls_port_1.egress_tunnel_if = vswitch_vpls_info_1.tunnel_id; /* This is used for multicast only */
...
vswitch_vpls_run(int unit, int second_unit){
    ...
    if (second_unit >= 0)
    {
        init_vlan(second_unit,vswitch_vpls_info_1.ac_port1_outer_vlan);
        init_vlan(second_unit,vswitch_vpls_info_1.ac_port1_inner_vlan);
        init_vlan(second_unit,vswitch_vpls_info_1.ac_port2_outer_vlan);
        init_vlan(second_unit,vswitch_vpls_info_1.ac_port2_inner_vlan);
    } 
    ...
    if (second_unit >= 0)
    {
        rv = vswitch_vpls_vpn_create(second_unit, vswitch_vpls_info_1.vpn_id);
        if (rv != BCM_E_NONE) {
            printf("Error, in vswitch_vpls_vpn_create\n");
            return rv;
        }
...
vswitch_vpls_run_with_defaults(int unit, int second_unit){
    ...
    vswitch_vlps_info_init(1,13,13,10,20,15,30,default_vpn_id);
...
vswitch_vpls_run_with_defaults_dvapi(int unit, int acP, int pweP){
    ...
    vswitch_vlps_info_init(acP,pweP,pweP,10,20,15,30,default_vpn_id);
    return vswitch_vpls_run(unit, second_unit);
}
