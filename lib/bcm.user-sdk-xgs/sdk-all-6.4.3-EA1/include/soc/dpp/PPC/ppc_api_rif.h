...
#define  SOC_PPC_RIF_MPLS_LABEL_COUPLING (0x2)
...
typedef enum
{
   ...
   */
  SOC_PPC_RIF_ROUTE_ENABLE_TYPE_IP_UC = 0x1,
  /*
   *  Enable IPv4/v6 MC routing.
   */
  SOC_PPC_RIF_ROUTE_ENABLE_TYPE_IP_MC = 0x2,
  /*
   *  Enable MPLS routing (LSR)
   */
  SOC_PPC_RIF_ROUTE_ENABLE_TYPE_MPLS = 0x4,
  /*
   *  Enable all above routing on RIF.
   */
  SOC_PPC_RIF_ROUTE_ENABLE_TYPE_ALL = (int)0xFFFFFFFF,
  /*
   *  Number of types in SOC_PPC_RIF_ROUTE_ENABLE_TYPE
   */
  SOC_PPC_NOF_RIF_ROUTE_ENABLE_TYPES = 5
...
typedef struct
{
   ...
   *  RIFs see SOC_PPD_RIF_ROUTE_ENABLE_TYPE
   */
...
typedef struct
{
  SOC_SAND_MAGIC_NUM_VAR
  /*
   *  RIF Id. When RIF == SOC_PPD_RIF_NULL, the default RIF ID and
  ...
  /*
   * service type, used in program setting and PMF
   * range: 0-7.
   * Used only in Arad.
   */
  uint32 service_type;
...
const char*
  SOC_PPC_RIF_ROUTE_ENABLE_TYPE_to_string(
    SOC_SAND_IN  SOC_PPC_RIF_ROUTE_ENABLE_TYPE enum_val
  );
