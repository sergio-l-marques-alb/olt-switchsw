/*
 * $Id: fe2000_diags.h,v 1.3 Broadcom SDK $
...
#define DISPLAY_TESTSTR(func,mmu,str) cli_out("\n%s: Starting MM%d %s test..\n",func,mmu,str)
/* from fe2000unimac.c */
#define FE2000_STS_CHECK(status)    \
    if ((status) != SB_FE2000_STS_INIT_OK_K) { return SOC_E_INTERNAL; }

void ConfigCmuHardMapping(char *cntrType,uint32 base_address,uint32 segment_limit);
void WrI(sbhandle hdl, uint32 mem, uint32 offs, uint32 data);
uint32 RdI(sbhandle hdl, uint32 mem, uint32 offs);
uint32 MemSizeEnc(uint32 bits);
void LrpEnable(sbhandle hdl);
uint32 LrpDisable(sbhandle hdl);
uint32 WaitForPhase(sbhandle hdl, uint32 phase);
uint32 CheckErrors(sbhandle hdl,int32 seed,uint32 phase);
uint32 HashData(int32 seed, uint32 ulOffs, uint32 Inv);
uint32 crc32(uint32 x);
...
/* foward declarations */
extern void sigcatcher(int); /* handler for ctrl-c */
int sbFe2000DiagsSnakeTest(sbxDiagsInfo_t *pDiagsInfo);
int sbFe2000DiagsDumpExceptionPkt(sbxDiagsInfo_t *pDiagsInfo, uint32 qe_unit);
int32 sbFe2000DiagsInjectPciPacket(sbxDiagsInfo_t *pDiagsInfo);
int32 sbFe2000DiagsReceivePciPacket(sbxDiagsInfo_t *pDiagsInfo);
...
int sbFe2000CheckLoopbackStats(sbxDiagsInfo_t *pDiagsInfo, uint8 bExternal);

/* clear the queues */
void sbFe2000ClearQueues(sbhandle userDeviceHandle);
int sbFe2000LoopbackSet(uint32 unit,uint32 loopback);
...
int sbFe200010GLoopbackSet(uint32 unit, uint32 port_10g, uint32 loopback);
...
int sbFe2000DiagsPacketReplication(sbxDiagsInfo_t *pDiagsInfo,
				   int32 nPacketRepCount,
				   uint32 uIngressClient,
				   uint32 uEgressInterface,
				   uint32 uEgressPort);
