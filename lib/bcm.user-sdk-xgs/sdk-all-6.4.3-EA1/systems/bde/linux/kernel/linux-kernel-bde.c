...
{BCM53454_DEVICE_ID, 7},
{BCM53455_DEVICE_ID, 7},
{BCM53456_DEVICE_ID, 7},
{BCM53457_DEVICE_ID, 7},
{BCM53422_DEVICE_ID, 7},
{BCM53424_DEVICE_ID, 7},
{BCM53426_DEVICE_ID, 7},
{ 0, 0}
};
...
_pci_probe(struct pci_dev *dev, const struct pci_device_id *ent)
{
    ...
    if (plx_dev) {
        ...
        {
            gprintk("PCI device enabled: vendor_id = %x, device_id = %x\n", dev->vendor, dev->device);
        }
...
_alloc_mpool(size_t size)
{
    ...
    if (_use_himem) {
        ...
        if (debug >= 1)
          gprintk("_alloc_mpool: _dma_vbase:%p pbase:%lx  allocated:%lx (SIMPLE_MEMORY)\n", _dma_vbase, pbase, (unsigned long)size);
        ...
        if (debug >= 1)
          gprintk("_alloc_mpool: _dma_vbase:%p pbase:%lx  allocated:%lx (non SIMPLE_MEMORY)\n", _dma_vbase, pbase, (unsigned long)size);
#endif /* _SIMPLE_MEMORY_ALLOCATION_ */
        if (debug >= 1) gprintk("_alloc_mpool: _dma_vbase:%p pbase:%lx  allocated:%lx (1)\n", _dma_vbase, pbase, (unsigned long)size);
        if (((pbase + size) >> 16) > DMA_BIT_MASK(16)) {
            _dma_vbase = NULL;
        ...
        _dma_pbase = pbase;
        if (debug >= 1) gprintk("_alloc_mpool: _dma_pbase:%x (2)\n", _dma_pbase);
#ifdef REMAP_DMA_NONCACHED
        _dma_vbase = IOREMAP(_dma_pbase, size);
        if (debug >= 1) gprintk("_alloc_mpool: _dma_vbase=%p _dma_pbase:%lx (3)\n", _dma_vbase, _dma_pbase);
#endif
    }
    if (debug >= 1) gprintk("Done!\n");
}
#ifdef BCM_ROBO_SUPPORT
...
_init(void)
{
    ...
    gprintk("MRS: Step 5: _switch_ndevices=%u _ndevices=%u\n", _switch_ndevices, _ndevices);
    ...
    if (_switch_ndevices > 0) {
        ...
        for (i=0; i<_switch_ndevices; i++)
        {
		gprintk("MRS: i=%u: dev_type=0x%x device=0x%x rev=0x%x\n", i, _devices[i].dev_type, _devices[i].bde_dev.device, _devices[i].bde_dev.rev);
        }
