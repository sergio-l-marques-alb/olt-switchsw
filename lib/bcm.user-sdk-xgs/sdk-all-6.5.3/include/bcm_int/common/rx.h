...
#define BCM_RX_F_STARTED          0x1  /* Started? */
    ...
    sal_mutex_t       rx_copy_to_cpu_config_mutex; /* Sync for copy-to-cpu config */
} rx_ctl_t;
...
#define RX_COPY_TO_CPU_LOCK(unit)        \
    sal_mutex_take(rx_ctl[unit]->rx_copy_to_cpu_config_mutex, \
    sal_mutex_FOREVER)
#define RX_COPY_TO_CPU_UNLOCK(unit)      \
    sal_mutex_give(rx_ctl[unit]->rx_copy_to_cpu_config_mutex)
...
extern SHR_BITDCL *(_bcmi_rx_egr_drop_copy_to_cpu_loc_bitmap[BCM_MAX_NUM_UNITS]);
