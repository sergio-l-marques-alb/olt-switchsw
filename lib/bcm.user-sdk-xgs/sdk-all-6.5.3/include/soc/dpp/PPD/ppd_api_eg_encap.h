...
#define SOC_PPD_EG_ENCAP_BFD_IPV4_PROG_VAR        0xa
...
#define SOC_PPD_EG_ENCAP_DATA_COUPLING_SWAP_FORMAT_SET(__unit, __ether_type, __mpls_label, __eg_encap_data_info)  \
      ...
      (__eg_encap_data_info)->data_entry[0] = (((__ether_type) << 28) | ((__mpls_label) << 8) | SOC_PPC_PRGE_DATA_ENTRY_LSBS_COUPLING_SWAP);
...
#define SOC_PPD_EG_ENCAP_DATA_COUPLING_SWAP_TYPE(__unit, __entry_type, __eg_encap_data_info) \
      ( (__entry_type == SOC_PPC_EG_ENCAP_ENTRY_TYPE_DATA) && ((__eg_encap_data_info)->data_entry[0] & SOC_PPC_PRGE_DATA_ENTRY_LSBS_COUPLING_SWAP))
...
#define SOC_PPD_EG_ENCAP_DATA_PON_TUNNEL_FORMAT_SET(__unit, __tpid, __pcp, __tunnel_id, __eg_encap_data_info)  \
      (__eg_encap_data_info)->data_entry[1] = (__tpid>>8) & 0xFF;  \
      (__eg_encap_data_info)->data_entry[0] = ((((__tpid) & 0xFF) << 24) | ((__pcp) << 20) | ((__tunnel_id) << 8) | SOC_PPC_PRGE_DATA_ENTRY_LSBS_PON_TUNNEL);
