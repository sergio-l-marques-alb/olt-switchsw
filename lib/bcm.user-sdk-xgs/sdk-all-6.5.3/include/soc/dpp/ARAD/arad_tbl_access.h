...
#define ARAD_PP_EPNI_EGRESS_EDIT_CMD_TBL_FLD_SIZE   ((SOC_IS_QAX(unit)) ? 19 : 16)
...
#define ARAD_PP_EPNI_EGRESS_EDIT_CMD_TBL_INFO(unit, outer_tpid, inner_tpid, outer_vid, inner_vid, outer_pcp_dei, inner_pcp_dei, tags_to_remove, packet_is_tagged_after_eve) \
   ((outer_tpid << ((SOC_IS_QAX(unit)) ? 15 : 14)) +  (inner_tpid << 12) + (outer_vid << 9) + (inner_vid << 6) + (outer_pcp_dei << 4) \
     + (inner_pcp_dei << 2) + tags_to_remove + (SOC_IS_QAX(unit) ? (packet_is_tagged_after_eve << 18) : 0))
/* Native EVEC entry */
...
#define ARAD_PP_FEC_PATH_SELECT_MEM(_unit)   \
        ((SOC_IS_QAX(unit)) ?  IHB_FEC_PATH_SELECTm : IHB_PATH_SELECTm)
