...
int pm_qtc_wb_buffer_init(int unit, int wb_buffer_index, pm_info_t pm_info)
{
    ...
    SOC_WB_ENGINE_ADD_BUFF(SOC_WB_ENGINE_PORTMOD, wb_buffer_index, "pm_qtc", NULL, VERSION(1), 1, SOC_WB_ENGINE_PRE_RELEASE);
    _SOC_IF_ERR_EXIT(rv);
                   ...
                   int wb_buffer_index, pm_info_t pm_info)
{
    ...
    SOC_PBMP_ASSIGN(pm_qtc_data->phys, pm_add_info->phys);
                        ...
                        const portmod_port_add_info_t* add_info, int enable)
{
    ...
    if(enable) {
       ...
       /* TBD : PGW_GE WRITE : GP0_TDM_MODEf */
        ...
        SOC_PBMP_AND(port_phys_in_pm, PM_QTC_INFO(pm_info)->phys);

        temp = 0;
        SOC_PBMP_ITER(PM_QTC_INFO(pm_info)->phys, phy) {
                            ...
                            int flags, int enable)
{
    ...
    if(enable) {
	    ...
	    rv = unimac_enable_set(unit, port,  1);
            _SOC_IF_ERR_EXIT(rv);
      	    if(PM_QTC_INFO(pm_info)->portmod_mac_soft_reset) {
		        _SOC_IF_ERR_EXIT(pm_qtc_port_soft_reset(unit, port, pm_info, 0));
	        }
        ...
        if((PORTMOD_PORT_ENABLE_MAC_GET(actual_flags))  && (!is_bypassed)){
            rv = unimac_enable_set(unit, port, 0);
            _SOC_IF_ERR_EXIT(rv);
        }
    }
