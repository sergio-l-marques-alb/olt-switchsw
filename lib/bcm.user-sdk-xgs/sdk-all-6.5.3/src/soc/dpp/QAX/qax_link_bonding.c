  ...
  )
{
    ...
    SOCDNX_IF_ERR_EXIT(soc_reg32_get(unit, reg, REG_PORT_ANY, 0, &reg_val32)); 
    soc_reg_field_set(unit, reg, &reg_val32, LBG_CREDIT_INITf, 0x1fff);
    SOCDNX_IF_ERR_EXIT(soc_reg32_set(unit, reg, REG_PORT_ANY, 0, reg_val32));
    ...
    for (lbg_id = 0; lbg_id < SOC_TMC_LB_NOF_LBG; lbg_id++) {
        fifo_size = soc_property_suffix_num_get(unit, lbg_id, spn_LB_MODEM_FIFO_SIZE, "FIFO", 30);
  ...
  )
{
    ...
    SOCDNX_IF_ERR_EXIT(soc_reg32_get(unit, reg, REG_PORT_ANY, 0, &reg_val32)); 
    soc_reg_field_set(unit, reg, &reg_val32, SEQ_NUM_WRAP_WIDTHf, num_width);
    SOCDNX_IF_ERR_EXIT(soc_reg32_set(unit, reg, REG_PORT_ANY, 0, reg_val32));
  ...
  )
{
    ...
    reg = ILB_MISC_CONFIG_REGr;
    SOCDNX_IF_ERR_EXIT(soc_reg32_get(unit, reg, REG_PORT_ANY, 0, &reg_val32)); 
    *num_width = soc_reg_field_get(unit, reg, reg_val32, SEQ_NUM_WRAP_WIDTHf);
  ...
  )
{
    ...
    SOC_TMC_LB_VALUE_MAX_CHECK(map_index->vlan, SOC_TMC_LB_ING_PKT_TO_MODEM_VLAN_MAX, "map_index->vlan");
  ...
  )
{
    ...
    case socLbModemStatusIsEmptyFifo:
        ...
        *status = SHR_BITGET(fld_val32, modem_id);
        break;
    default:
  ...
  )
{
    ...
    SOCDNX_IF_ERR_EXIT(soc_reg32_get(unit, reg, REG_PORT_ANY, 0, &reg_val32)); 
    *fld_val32 = soc_reg_field_get(unit, reg, reg_val32, IFC_IS_LBGf);
    *is_enable = SHR_BITGET(fld_val32, lbg_id);
  ...
  )
{
    ...
    else if (config->pkt_format == socLbgFormatTypeNonChannelize) {
        hw_hdr_type = 1;
    }
  ...
  )
{
    ...
    config->pkt_format = soc_mem_field32_get(unit, modem_table, modem_data, HDR_TYPEf);
  ...
  )
{
    ...
    /* check if parameters are valid */
    SOC_TMC_LB_VALUE_MAX_CHECK(lbg_count, SOC_TMC_LB_LBG_MAX, "lbg_count");
    SOCDNX_NULL_CHECK(lbg_weights);

    for (lbg_temp = 0; lbg_temp < lbg_count; lbg_temp++) {
        sum_of_lbg_weight += lbg_weights[lbg_temp].lbg_weight;
    }
    SOC_TMC_LB_VALUE_MAX_CHECK(sum_of_lbg_weight, SOC_TMC_LB_EGR_NOF_SCHEDULER_ENTRY, "lbg_count");

    mem = EPNI_LBG_CALENDARm;
    entry_start = 0;
    for (lbg_temp = 0; lbg_temp < lbg_count; lbg_temp++) {
        nof_entries = lbg_weights[lbg_temp].lbg_weight;
        for (entry_index = entry_start; entry_index < nof_entries; entry_index++) {
    ...
    for (; entry_index < SOC_TMC_LB_EGR_NOF_SCHEDULER_ENTRY; entry_index++) {
  ...
  )
{
    ...
    SOC_TMC_LB_VALUE_MAX_CHECK(modem_id, SOC_TMC_LB_MODEM_MAX, "modem_id");
    SOC_TMC_LB_VALUE_CHECK(shaper_config->hdr_compensation, SOC_TMC_LB_ING_SEQUENCE_NUM_WIDTH_MIN, SOC_TMC_LB_ING_SEQUENCE_NUM_WIDTH_MAX, "hdr_compensation");
  ...
  )
{
    ...
    case socLbStatusIsEmptyFifo:
        ...
        *status = SHR_BITGET(fld_val32, lbg_id);
        break;
    default:
        if (status_type < socLbStatusCount) {
            return BCM_E_UNAVAIL;
