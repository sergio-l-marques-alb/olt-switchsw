...
)
{
    ...
    if (use_dma) { /* use DMA */
        ...
        SOCDNX_IF_ERR_EXIT(soc_mem_read_range(unit, TAR_MCDBm, MEM_BLOCK_ANY, 0, DPP_LAST_MCDB_ENTRY(mcds), alloced_mem));
        ...
        if (do_not_read) { /* if not in warm boot, the MCDB and IRDB tables that we have just filled */
            ...
            SOCDNX_IF_ERR_EXIT(soc_mem_read_range(unit, TAR_MCDBm, MEM_BLOCK_ANY, 0, DPP_LAST_MCDB_ENTRY(mcds), dest));
            for (i = table_size; i; --i) {
        ...
        for (i = 0 ; i < irdb_table_nof_entries; ++i) {
            ...
            for (j = 0 ; j < QAX_NOF_GROUPS_PER_IRDB_ENTRY; ++j) {
                if (bits & 1) { /* we found an open ingress multicast group and will traverse it */
                    ...
                    uint32 cur_entry = soc_mem_field32_get(unit, TAR_MCDBm, mcdb_entry, LINK_PTRf);
    ...
    if (range_start < mcds->ingress_bitmap_start && range_end >= range_start) { /* If we have a valid egress range */
        ...
        SOCDNX_IF_ERR_EXIT(qax_mcds_build_free_blocks( unit, mcds, last_end + 1, range_start - 1, &mcds->no_starts, McdsFreeBuildBlocksAddOnlyFree));
