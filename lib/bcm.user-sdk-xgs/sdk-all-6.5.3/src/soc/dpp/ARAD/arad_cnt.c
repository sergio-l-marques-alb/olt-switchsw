...
CONST soc_field_t sel[SOC_DPP_DEFS_MAX(NOF_COUNTER_FIFOS)] =  {FIFO_DMA_0_SELf, FIFO_DMA_1_SELf, FIFO_DMA_2_SELf, FIFO_DMA_3_SELf}; 
/* } */
/*************
 * FUNCTIONS *
 *************/
  ...
  )
{
  ...
  for (cnt_proc_ndx = 0; cnt_proc_ndx < ARAD_CNT_NOF_PROCESSOR_IDS(unit); cnt_proc_ndx++) {
      ...
      if (!SOC_IS_JERICHO_PLUS_A0(unit) && !SOC_IS_QAX(unit)) {
          /* 
...
uint32
  arad_cnt_channel_to_fifo_mapping_set(
    ...
    if (channel > SOC_MEM_FIFO_DMA_CHANNEL_3) {
        SOCDNX_IF_ERR_EXIT(BCM_E_PARAM);
    }
    ...
    if (SOC_IS_JERICHO(unit)) {
      /*Mark DMA channel as used for CRPS
        ...
        */
        if (crps_fifo <= 0x3) {
            selected_applicaion = crps_fifo;
        } else if (crps_fifo == SOC_TMC_CNT_INVALID_FIFO) {
            selected_applicaion = DMA_SRC_DEFAULT; /*Return to reset-value*/
        } else {
            SOCDNX_IF_ERR_EXIT(BCM_E_PARAM);
        }
        rv = soc_reg_above_64_field32_modify(unit, ECI_FIFO_DMA_SELr, REG_PORT_ANY, 0, sel[channel], selected_applicaion);
...
if (!SAL_BOOT_PLISIM ) {
  ...
  soc_pci_write(unit, CMIC_CMCx_FIFO_CHy_RD_DMA_SBUS_START_ADDRESS_OFFSET(cmc, channel), start_addr);

  /* Configure Cannels endianess according to the CPU */
  reg_val = soc_pci_read(unit, CMIC_CMCx_FIFO_CHy_RD_DMA_CFG_OFFSET(cmc, channel));
  soc_endian_get(unit, &dont,&care,&endian);
  soc_reg_field_set(unit, CMIC_CMC0_FIFO_CH0_RD_DMA_CFGr, &reg_val, ENDIANESSf, endian != 0);
  soc_pci_write(unit, CMIC_CMCx_FIFO_CHy_RD_DMA_CFG_OFFSET(cmc, channel), reg_val);

  /* Unmask fifo dma channle ARAD_LEARNING_DMA_CHANNEL_USED interrupts */
  soc_cmicm_intr0_disable(unit, IRQ_CMCx_FIFO_CH_DMA(channel));
}
      ...
      SOC_SAND_IN int                     command_id)
{
    ...
    case ARAD_CNT_MODE_EG_TYPE_TM:
      fld_val = (SOC_IS_QAX(unit) ? QAX_CNT_MODE_EG_TYPE_TM_FLD_VAL : ARAD_CNT_MODE_EG_TYPE_TM_FLD_VAL);
      tm_out_port_selector = 1; /* Out-Port is TM */
      break;
    case ARAD_CNT_MODE_EG_TYPE_VSI:
      fld_val = (SOC_IS_QAX(unit) ? QAX_CNT_MODE_EG_TYPE_VSI_FLD_VAL : ARAD_CNT_MODE_EG_TYPE_VSI_FLD_VAL);
      break;
    case ARAD_CNT_MODE_EG_TYPE_OUTLIF:
      fld_val = (SOC_IS_QAX(unit) ? QAX_CNT_MODE_EG_TYPE_OUTLIF_FLD_VAL : ARAD_CNT_MODE_EG_TYPE_OUTLIF_FLD_VAL);
      break;
  ...
  )
{
  ...
  SOC_SAND_ERR_IF_ABOVE_MAX(channel, SOC_DPP_DEFS_GET(unit, nof_counter_fifos) - 1, ARAD_PROCESSOR_NDX_OUT_OF_RANGE_ERR, 20, exit);
  ...
  )
{
  ...
  SOC_SAND_ERR_IF_ABOVE_MAX(channel, SOC_DPP_DEFS_GET(unit, nof_counter_fifos) - 1, ARAD_PROCESSOR_NDX_OUT_OF_RANGE_ERR, 21, exit);
...
if (!SAL_BOOT_PLISIM ) {
   ...
   rval = soc_pci_read(unit, CMIC_CMCx_FIFO_CHy_RD_DMA_STAT_OFFSET(cmc, channel));
   arad_sw_db_cnt_buff_and_index_get(unit, dma_ndx, &host_buff, &buff_index);
  ...
  )
{
  ...
  SOC_SAND_ERR_IF_ABOVE_MAX(channel, SOC_DPP_DEFS_GET(unit, nof_counter_fifos), ARAD_PROCESSOR_NDX_OUT_OF_RANGE_ERR, 20, exit);
