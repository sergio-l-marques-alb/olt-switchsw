  ...
  )
{
 ...
 if ( (SOC_DPP_CONFIG(unit)->arad->init.ports.is_stacking_system == 0x1) || (SOC_DPP_CONFIG(unit)->arad->init.ports.add_dsp_extension_enable) ) {
   ...
   */
    ...
    if (SOC_IS_JERICHO_PLUS(unit)) {
        SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  496,  exit, ARAD_REG_ACCESS_ERR, soc_reg_above_64_field32_modify(unit, IHB_PMF_GENERALr, SOC_CORE_ALL, 0, DISABLE_2ND_PASS_KEY_UPDATEf,  1)); 
    }
