  ...
  )
{
  ...
  int i;
    ...
    for(i = 0; i < fbr_nof_links_in_mac; i++) {
        ...
        if (SOC_IS_KALIA(unit)){ /*elij - temp fix*/
            for (blk_id = 0; blk_id < fbr_nof_macs; blk_id++) {
                SOCDNX_IF_ERR_EXIT(WRITE_FMAC_FPS_CONFIGURATION_RX_SYNCr(unit, blk_id, i, reg_val));
            }
        }else{
   ...
   SOC_SAND_IN  int                                 unit)
{
    ...
    aldwp_field = (SOC_IS_QAX(unit) ? FIELD_0_3f : FIELD_19_22f);
