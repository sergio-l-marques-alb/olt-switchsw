...
{
  uint32                    res = SOC_SAND_OK;
  SOC_PPC_MPLS_TERM_LKUP_INFO mpls_lkup_key;
  ...
  if (mpls_lkup_key.key_type == SOC_PPC_MPLS_TERM_KEY_TYPE_LABEL_RIF)
  {
    if (mpls_key->flags != 0) {
    ...
    if (use_label_index)
    {
      ...
      case SOC_PPC_MPLS_LABEL_INDEX_FIRST:
        ...
        break;
      default:
          SOC_SAND_SET_ERROR_CODE(ARAD_PP_RIF_MPLS_LABEL_KEY_ILLEGAL_ERR, 52, exit);
      }     
    }
    else
    {
  ...
  if (SOC_IS_JERICHO(unit) && soc_property_get(unit, spn_EVPN_ENABLE, 0)) {
      isem_key->key_info.mpls.flags =  (mpls_key->flags & SOC_PPC_RIF_MPLS_LABEL_EVPN_IML) ? SOC_PPC_MPLS_TERM_FLAG_EVPN_IML : 0;
  }
