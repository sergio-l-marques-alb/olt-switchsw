    ...
    arad_pp_dbal_kbp_table_init(int unit, SOC_DPP_DBAL_SW_TABLE_IDS table_id, SOC_PPC_FP_DATABASE_STAGE stage, int lookup_number, int program_id)
{
	...
	}else if(table.additional_table_info == SOC_DPP_DBAL_ATI_KBP_LOOKUP_ONLY){
    ...
    case SOC_DPP_DBAL_PHYSICAL_DB_TYPE_KBP:
        {
            ...
            flp_lookups_tbl.elk_opcode = table_program.program_id;
            if (is_msb) {
    ...
    arad_pp_dbal_iterator_get_next_lem(int unit, SOC_DPP_DBAL_SW_TABLE_IDS table_id, uint32 flags, SOC_PPC_FP_QUAL_VAL qual_vals[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX], void* payload, uint32* priority, uint8* hit_bit, uint8* found)
{
    ...
    if ((table_iterator->read_keys != NULL) && (table_iterator->read_vals != NULL) && (table_iterator->last_nof_entries != 0)) {
        ...
        if (table_iterator->read_vals[entry_index].flags & ARAD_PP_LEM_ACCESS_ENTRY_ACCESSED) {
            *hit_bit = 1;
        } else {
            *hit_bit = 0;
        }
    ...
    arad_pp_dbal_iterator_get_next_kbp(int unit, SOC_DPP_DBAL_SW_TABLE_IDS table_id, uint32 flags, SOC_PPC_FP_QUAL_VAL qual_vals[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX], void* payload, uint32* priority, uint8* hit_bit, uint8* found)
{
    ...
    *found = FALSE;
	...
	host_prefix_len = ((kbp_table_iterator->table_id == ARAD_KBP_FRWRD_TBL_ID_IPV6_UC_RPF_0)||(kbp_table_iterator->table_id == ARAD_KBP_FRWRD_TBL_ID_IPV6_UC_RPF_1)) ? 144 : 48;
	...
	if (*found) {
		...
		if (is_host) {
			...
			if (kbp_lem_payload.flags & ARAD_PP_LEM_ACCESS_ENTRY_ACCESSED) {
				*hit_bit = TRUE;
			}
		}else{
   ...
   arad_pp_dbal_vtt_dynamic_tables_init(int unit)
{
    ...
    SOCDNX_SAND_IF_ERR_EXIT(arad_pp_dbal_program_to_tables_associate(unit, prog_id, SOC_PPC_FP_DATABASE_STAGE_INGRESS_VT, &keys_to_table_id, NULL, 1));    
    ...
    SOCDNX_SAND_IF_ERR_EXIT(arad_pp_dbal_program_to_tables_associate(unit, prog_id, SOC_PPC_FP_DATABASE_STAGE_INGRESS_TT, &keys_to_table_id, NULL, 1));
...
STATIC SOC_DPP_SIGNAL_MNGR_SIGNAL_INFO arad_pp_signal_mngr_qax_signals_table[SOC_DPP_SIGNAL_MNGR_SIGNAL_TYPE_NOF_TYPES] = {                           
    ...
    {/*FLP_2_LPM2_KEY     		*/ TRUE,  ARAD_IHP_ID,  32,       0,      77,    0,     160,       82},
