    ...
    arad_pp_init_dynamic_kbp_result(int unit)
{
	...
	int res2_pad=0, res3_pad=0, res_size_with_pad=0;
	int total_bits_in_lsb = 0, total_bits_in_msb = 0, aling_to_120b_index = -1;
	int total_results_bits_32b_aligned = 0;
    ...
    if (ARAD_KBP_ENABLE_IPV4_UC ||  ARAD_KBP_ENABLE_IPV4_MC || ARAD_KBP_ENABLE_IPV6_UC || ARAD_KBP_ENABLE_IPV6_MC 
        ...
        if (res_sizes[0] != ARAD_PP_FLP_DEFAULT_FWD_RES_SIZE) {
            SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG("illegal value for external res 0 size, when we have fwd in external tcam res 0 has to be %d \n"), ARAD_PP_FLP_DEFAULT_FWD_RES_SIZE));
        }
    }
    ...
    if (ARAD_KBP_ENABLE_IPV4_RPF ||  ARAD_KBP_ENABLE_IPV6_RPF ){
        if (res_sizes[2] != ARAD_PP_FLP_DEFAULT_RPF_RES_SIZE) {
			SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG("illegal value for external res 2 size, when we have rpf in external tcam, its res has to be %d \n"), ARAD_PP_FLP_DEFAULT_RPF_RES_SIZE));
        }
    ...
    /*if double capacity enabled the first 3 results are constant and only the last one can be configured 
      should be 48, 48, 24, (24 pad), x */
    if (ARAD_KBP_ENABLE_IPV4_DC){
        if ((res_sizes[0] != 48) || (res_sizes[1] != 24) || (res_sizes[2] != 24)||
			(res_sizes[3] != 16) || (res_sizes[4] != 32) || (res_sizes[5] != 0)) {
			SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG("illegal value for external res 0-5 size, when double capacity in external tcam, its res has to be fix result sizes should be 48, 24, 24, 16, 32, 0\n")));
        }
		res2_pad = soc_property_suffix_num_get(unit, 0, spn_EXT_TCAM_RESULT_SIZE_SEGMENT, "res2_pad", 0);
		res3_pad = soc_property_suffix_num_get(unit, 0, spn_EXT_TCAM_RESULT_SIZE_SEGMENT, "res3_pad", 0);
		if (res2_pad != 24 || res3_pad != 48 ) {
			SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG("results padding in case of double capacity has to be res2_pad=24b, res3_pad=48b\n")));
		}
		aling_to_120b_index = 2;
    }else{
		/* check which result is align to 120 bit. if none, return an error*/
		/* check if the total length of the results (aligned to 32 bit) exceeds 256 bit (8*32bit)*/
		...
		for (i = 0; i < ARAD_PP_FLP_KBP_MAX_NUMBER_OF_RESULTS; i++) {
			...
			total_results_bits_32b_aligned += (res_sizes[i]+31)/32;
		}
		...
		if (aling_to_120b_index == -1) {
			SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG("results length have to be 120b alignment, total separation between LSB and MSB buffers\n"
																  "result_0 = %d, result_1 = %d, result_2 = %d, result_3 = %d\n"),res_sizes[0],res_sizes[1],res_sizes[2],res_sizes[3]));
		}

		/* total results size is limit to payload buffer size */
		if (total_nof_bits_in_elk_res > (ARAD_PP_FLP_KBP_MAX_ROP_PAYLOAD_SIZE_IN_BITS + ARAD_PP_FLP_KBP_MAX_ROP_PAYLOAD_LSB_SIZE_IN_BITS)) {
			SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG("total external result size has to be %d, total = %d, "
																  "update ext_tcam_result_size_segment_# according\n"), (ARAD_PP_FLP_KBP_MAX_ROP_PAYLOAD_SIZE_IN_BITS + ARAD_PP_FLP_KBP_MAX_ROP_PAYLOAD_LSB_SIZE_IN_BITS), total_nof_bits_in_elk_res));
		}
		
		if (total_results_bits_32b_aligned > ARAD_PP_FLP_KBP_MAX_RESULTS_LENGTH_IN_32b_ALIGNMENT) {
			SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG("total external result sizes (aligned to 32b) can not exceed %d d-words, "
																  "update ext_tcam_result_size_segment_# according\n"), ARAD_PP_FLP_KBP_MAX_RESULTS_LENGTH_IN_32b_ALIGNMENT));
		}
	}
    ...
    for(i = 0; i < SOC_DPP_DEFS_GET(unit, nof_flp_programs); i++){
		...
		if (flp_lookups_tbl.elk_lkp_valid) {
            ...
            if((i == PROG_FLP_IPV4UC_RPF)||(i == ipv6rpf_prog_id)||(i == PROG_FLP_IPV6MC)||(i == PROG_FLP_IPV4COMPMC_WITH_RPF)||(i == PROG_FLP_IPV4_DC)){
                rpf_result_start_offset = ARAD_PP_FLP_KBP_MAX_ROP_PAYLOAD_SIZE_IN_BITS-(res_sizes[0]+res_sizes[1]+res_sizes[2]+res2_pad);
                fec_res_found_bit_offset = ARAD_PP_FLP_KBP_MAX_ROP_PAYLOAD_SIZE_IN_BITS+5;
                is_rpf_lookup_needed = 1;
				if(i == PROG_FLP_IPV4_DC){
					is_rpf_lookup_needed = 2; /* in order to get 48 bit result length, instead of 24*/
				}
            }

            /* configures the FLP */            
            arad_pp_flp_elk_result_configure(unit, i, res_sizes[0], ARAD_PP_FLP_KBP_MAX_ROP_PAYLOAD_SIZE_IN_BITS+7, is_rpf_lookup_needed*24, rpf_result_start_offset, fec_res_found_bit_offset);
	...
	total_bits_in_lsb = total_bits_in_msb = 0;
    for (i = 0; i < ARAD_PP_FLP_KBP_MAX_NUMBER_OF_RESULTS; i++) {
		...
		res_size_with_pad = res_sizes[i];
		if (i==2) {
			res_size_with_pad += res2_pad;
		}
		if (i==3) {
			res_size_with_pad += res3_pad;
		}
		kbp_qualifiers_values.qual_nof_bits = res_size_with_pad;

		if (i <= aling_to_120b_index) {
			total_bits_in_msb += res_size_with_pad;
			...
			total_bits_in_lsb += res_size_with_pad;
    ...
    SOC_SAND_IN  int                                unit)
{
    ...
    if (!SOC_IS_ARDON(unit) && SOC_IS_ARADPLUS(unit)) 
    {
        ...
        if (SOC_IS_ROO_ENABLE(unit) && SOC_IS_QAX(unit)) {
            uint64 fld_val64;
  ...
  )
{
  ...
  */
  ...
  if (SOC_IS_JERICHO(unit))  {
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  220,  exit, ARAD_REG_ACCESS_ERR, soc_reg_above_64_field32_modify(unit, IHP_FLP_GENERAL_CFGr, REG_PORT_ANY, 0, FLP_KAPS_DELAY_IS_EVENf,  SOC_IS_QAX(unit) ? 0 : 1));
    SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  220,  exit, ARAD_REG_ACCESS_ERR, soc_reg_above_64_field32_modify(unit, IHB_PMF_GENERALr, REG_PORT_ANY, 0, PMF_KAPS_DELAY_IS_EVENf,  SOC_IS_QAX(unit) ? 0 : 1));
