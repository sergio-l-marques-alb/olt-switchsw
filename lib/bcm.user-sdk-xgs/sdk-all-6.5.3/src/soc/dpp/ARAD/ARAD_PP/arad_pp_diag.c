 ...
 *********************************************************************/
STATIC SOC_PPC_PKT_HDR_TYPE
  arad_pp_diag_hdr_at_index(
    SOC_SAND_IN  SOC_PPC_PKT_HDR_STK_TYPE   pkt_type,
  ...
  )
{
  ...
  for (indx = 0; indx < nof_traps; ++indx)
  {
    if (soc_sand_bitstream_test_bit(regs_val, indx))
    {
      ...
      res = arad_pp_trap_mgmt_trap_code_to_trap_stength(unit, trap_code, &traps_info->trap_stack_strength[trap_code]);
  ...
  )
{
  ...
  if (SOC_IS_QAX(unit)) {
      ARAD_PP_DIAG_FLD_READ(&fld, core_id, ARAD_IHP_ID,3,0,228,213,20);
  ...
  )
{
  ...
  if (pkt_trace & SOC_PPC_DIAG_PKT_TRACE_TUNNEL_RNG)
  {
  ...
  )
{
  ...
  *ret_val = SOC_PPC_DIAG_OK; 
  ...
  if (SOC_IS_QAX(unit)) {
    ARAD_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHP_ID,1,7,40,33,75);
  } else {
  ...
  )
{
  ...
  *ret_val = SOC_PPC_DIAG_OK;
  ...
  if (SOC_IS_QAX(unit)) {
      ARAD_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHP_ID,3,1,191,186,5);
  } else if (SOC_IS_JERICHO(unit)) {
  ...
  if (outer_eth_vld)
  {
    ...
    if (SOC_IS_QAX(unit)) {
        ARAD_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHP_ID,3,2,25,22,15);
    } else if (SOC_IS_JERICHO(unit)) {
    ...
    if (SOC_IS_QAX(unit)) {
        ARAD_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHP_ID,2,5,89,65,15);
        tmp = regs_val[0];
    } else if (SOC_IS_JERICHO(unit)) {
  ...
  */
  if (SOC_IS_QAX(unit)) {
    ARAD_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHP_ID,1,7,40,33,15);
  } else {
  ...
  )
{
  ...
  *ret_val = SOC_PPC_DIAG_OK; 
  ...
  if (SOC_IS_QAX(unit)) {
      ARAD_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHP_ID,3,7,161,153,5);
  } else if (SOC_IS_JERICHO(unit)) {
  ...
  )
{
  ...
  *ret_val = SOC_PPC_DIAG_OK;
  ...
  if (SOC_IS_QAX(unit)) {
      ARAD_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHP_ID,3,0,242,229,5);
  } else if (SOC_IS_JERICHO(unit)) {
  ...
  if (frwrd_info->frwrd_type == SOC_PPC_DIAG_FWD_LKUP_TYPE_TRILL_UC) {
      if (SOC_IS_QAX(unit)) {
          ARAD_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHP_ID,3,1,72,72,5);
      } else if (SOC_IS_JERICHO(unit)) {
  ...
  if (SOC_IS_QAX(unit)) {
      ARAD_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHP_ID,28,0,79,0,101);
  } else if (SOC_IS_JERICHO(unit)) {
  ...
  else if (frwrd_info->frwrd_type == SOC_PPC_DIAG_FWD_LKUP_TYPE_IPV4_MC)
  {
  ...
  if (diag_flavor & SOC_PPC_DIAG_FLAVOR_RAW)
  {
    ...
    SOC_SAND_CHECK_FUNC_RESULT(res, 410, exit);
            ...
            frwrd_info->frwrd_type == SOC_PPC_DIAG_FWD_LKUP_TYPE_FCF
          )
  {
      if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
        ...
        if (tmp)
        {
          ...
          if (SOC_PPC_DIAG_FWD_LKUP_TYPE_IPV6_MC||SOC_PPC_DIAG_FWD_LKUP_TYPE_IPV6_UC)
          {
  ...
  )
{
  ...
  *ret_val = SOC_PPC_DIAG_OK;
  ...
  if (SOC_IS_QAX(unit)) {
      ARAD_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHP_ID,3,1,92,91,105);
  } else if (SOC_IS_JERICHO(unit)) {
  ...
  if (SOC_IS_QAX(unit)) {
      ARAD_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHP_ID,3,2,154,152,106);
  } else if (SOC_IS_JERICHO(unit)) {
  ...
  if (SOC_IS_QAX(unit)) {
      ARAD_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHP_ID,3,2,197,190,107);
  } else if (SOC_IS_JERICHO(unit)) {
  ...
  arad_pp_trap_cpu_trap_code_from_internal_unsafe(unit,regs_val[0],&trap_code);
  frwrd_trace_info->trap[SOC_PPC_DIAG_FRWRD_DECISION_PHASE_LIF].action_profile.trap_code = trap_code;

  /*dbg: ihp2ihb_vtt2flp_2eq3_forwarding_action_cpu_trap_qual [15 : 0] */
  if (SOC_IS_QAX(unit)) {
      ARAD_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHP_ID,3,2,189,174,108);
  } else if (SOC_IS_JERICHO(unit)) {
  ...
  if (SOC_IS_QAX(unit)) {
      ARAD_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHP_ID,3,2,173,155,102);
  } else if (SOC_IS_JERICHO(unit)) {
   ...
   if (SOC_IS_QAX(unit)) {
       ARAD_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHP_ID,3,2,222,199,110);
   } else if (SOC_IS_JERICHO(unit)) {
  ...
  if (SOC_IS_QAX(unit)) {
      ARAD_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHP_ID,3,1,209,192,111);
  } else if (SOC_IS_JERICHO(unit)) {
 ...
 /* forwarding lookup TRAP */
  ...
  if (SOC_IS_QAX(unit)) {
      ARAD_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHP_ID,4,1,247,246,112);
  } else if (SOC_IS_JERICHO(unit)) {
  ...
  if (SOC_IS_QAX(unit)) {
      ARAD_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHP_ID,4,8,169,167,113);
  } else if (SOC_IS_JERICHO(unit)) {
  ...
  if (SOC_IS_QAX(unit)) {
      ARAD_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHP_ID,4,8,216,209,114);
  } else if (SOC_IS_JERICHO(unit)) {
  ...
  if(arad_pp_diag_is_trap_code(regs_val[0])){
      arad_pp_trap_cpu_trap_code_from_internal_unsafe(unit,regs_val[0],&trap_code);
      frwrd_trace_info->trap[SOC_PPC_DIAG_FRWRD_DECISION_PHASE_LKUP_FOUND].action_profile.trap_code = trap_code;
  }
  else{
    if(frwrd_trace_info->trap[SOC_PPC_DIAG_FRWRD_DECISION_PHASE_LKUP_FOUND].action_profile.frwrd_action_strength > 0)
    {
      frwrd_trace_info->trap[SOC_PPC_DIAG_FRWRD_DECISION_PHASE_LKUP_FOUND].action_profile.trap_code = SOC_PPC_TRAP_CODE_ACCEPTABLE_FRAME_TYPE_ACCEPT;
    }
  }

  /*dbg: flp2pmf_2eq3_forwarding_action_cpu_trap_qual [15 : 0] */
  if (SOC_IS_QAX(unit)) {
      ARAD_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHP_ID,4,8,208,193,115);
  } else if (SOC_IS_JERICHO(unit)) {
 ...
 /* forwarding lookup forwarding decision*/
  ...
  if (SOC_IS_QAX(unit)) {
      ARAD_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHP_ID,4,8,192,174,116);
  } else if (SOC_IS_JERICHO(unit)) {
  ...
  if (SOC_IS_QAX(unit)) {
      ARAD_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHP_ID,4,10,1,0,117);
      tmp = regs_val[0] << 22;
      ARAD_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHP_ID,4,9,255,234,117);
      regs_val[0] += tmp;
  } else if (SOC_IS_JERICHO(unit)) {
  ...
  /*dbg: flp2pmf_1eq2_out_lif [17 : 0] */
  if (SOC_IS_QAX(unit)) {
      ARAD_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHP_ID,4,10,73,56,1181);
  } else if (SOC_IS_JERICHO(unit)) {
  ...
  if (SOC_IS_QAX(unit)) {
      ARAD_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHB_ID,0,13,204,186,1191);
  } else if (SOC_IS_JERICHO(unit)) {
  ...
  if (SOC_IS_QAX(unit)) {
      ARAD_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHB_ID,0,14,103,80,120);
  } else if (SOC_IS_JERICHO(unit)) {
  ...
  /*dbg: pmf2fer_38eq39_out_lif [17 : 0] */
  if (SOC_IS_QAX(unit)) {
      ARAD_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHB_ID,0,12,133,116,121);
  } else if (SOC_IS_JERICHO(unit)) {
  ...
  if (SOC_IS_QAX(unit)) {
      ARAD_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHB_ID,2,3,31+21,31+3,122);
  } else if (SOC_IS_JERICHO(unit)) {
  ...
  if (SOC_IS_QAX(unit)) {
      ARAD_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHB_ID,2,3,215,192,123);
  } else if (SOC_IS_JERICHO(unit)) {
  ...
  /*dbg: fer2lbp_eq2_out_lif [17 : 0] */
  if (SOC_IS_QAX(unit)) {
      ARAD_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHB_ID,2,1,162,145,124);
  } else if (SOC_IS_JERICHO(unit)) {
  ...
  if(arad_pp_diag_is_trap_code(trap)){
      ...
      frwrd_trace_info->trap[SOC_PPC_DIAG_FRWRD_DECISION_PHASE_FEC].action_profile.trap_code = trap_code;
  }
  else{
    if(frwrd_trace_info->trap[SOC_PPC_DIAG_FRWRD_DECISION_PHASE_FEC].action_profile.frwrd_action_strength > 0)
    {
      frwrd_trace_info->trap[SOC_PPC_DIAG_FRWRD_DECISION_PHASE_FEC].action_profile.trap_code = SOC_PPC_TRAP_CODE_ACCEPTABLE_FRAME_TYPE_ACCEPT;
    }
  }
  ...
  if (SOC_IS_QAX(unit)) {
      ARAD_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHB_ID,4,0,19,1,125);
  } else if (SOC_IS_JERICHO(unit)) {
  ...
  if (SOC_IS_QAX(unit)) {
      ARAD_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHB_ID,2,3,215,192,125);
  } else if (SOC_IS_JERICHO(unit)) {
  ...
  )
{
  ...
  *ret_val = SOC_PPC_DIAG_OK;   
  ...
  if (SOC_IS_QAX(unit)) {
      ARAD_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHB_ID,2,2,66,66,101);
  } else if (SOC_IS_JERICHO(unit)) {
  ...
  if (SOC_IS_QAX(unit)) {
      ARAD_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHB_ID,2,1,195,195,101);
  } else if (SOC_IS_JERICHO(unit)) {
  ...
  if (SOC_IS_QAX(unit)) {
      ARAD_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHB_ID,2,0,68,29,101);
  } else if (SOC_IS_JERICHO(unit)) {
  ...
  if (SOC_IS_QAX(unit)) {
      ARAD_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHB_ID,2,2,2,0,101);
      tmp = regs_val[0] << 28;
      ARAD_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHB_ID,2,1,255,196,101);
      regs_val[1] += tmp;
  } else if (SOC_IS_JERICHO(unit)) {
  ...
  )
{
  ...
  *ret_val = SOC_PPC_DIAG_OK;    
  ...
  /* vlan-edit-info*/
  if (SOC_IS_QAX(unit)) {
      ARAD_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHB_ID,2,0,158,125,5);
  ...
  )
{
  ...
  *ret_val = SOC_PPC_DIAG_OK;                                                           
  ...
  /* tm-forwarding-info*/
  if (SOC_IS_QAX(unit)) {
      ARAD_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHB_ID,4,1,75,0,5);
  } else if (SOC_IS_JERICHO(unit)) {
  ...
  if (tmp)
  {
        ...
        res = soc_sand_bitstream_get_any_field(SOC_IS_QAX(unit) ? regs_val : regs_val_part2, SOC_IS_QAX(unit) ? 114 : (SOC_IS_JERICHO(unit)? 27: 42), 2, &pkt_tm_info->dp_meter_cmd);
        SOC_SAND_CHECK_FUNC_RESULT(res, 140, exit);
  }

  /*COUNTER1*/
  tmp = 0;
  res = soc_sand_bitstream_get_any_field(regs_val, SOC_IS_QAX(unit) ? 183 : (SOC_IS_JERICHO(unit)? 244: 242), 1, &tmp);
  SOC_SAND_CHECK_FUNC_RESULT(res, 145, exit);
  if (tmp)
  {
    pkt_tm_info->valid_fields |= SOC_PPC_DIAG_PKT_TM_FIELD_COUNTER1;
    res = soc_sand_bitstream_get_any_field(regs_val, SOC_IS_QAX(unit) ? 166 : (SOC_IS_JERICHO(unit)? 223: 226), SOC_IS_QAX(unit) ? 17 : (SOC_IS_JERICHO(unit)? 21: 16), &pkt_tm_info->counter1);
    SOC_SAND_CHECK_FUNC_RESULT(res, 150, exit);
  }
  /*COUNTER2*/
  tmp = 0;
  res = soc_sand_bitstream_get_any_field(SOC_IS_QAX(unit) ? regs_val : regs_val_part2, SOC_IS_QAX(unit) ? 165 : (SOC_IS_JERICHO(unit)? 10: 3), 1, &tmp);
  ...
  if (tmp)
  {
    ...
    if(!SOC_IS_QAX(unit))
    {
        res = soc_sand_bitstream_get_any_field(regs_val, SOC_IS_JERICHO(unit)? 245: 243, SOC_IS_JERICHO(unit)? 11: 13, &field_part1);
        SOC_SAND_CHECK_FUNC_RESULT(res, 160, exit);
        res = soc_sand_bitstream_get_any_field(regs_val_part2, 0, SOC_IS_JERICHO(unit)? 10: 3, &field_part2);
        SOC_SAND_CHECK_FUNC_RESULT(res, 165, exit);
        pkt_tm_info->counter2 = field_part1 + (field_part2 << (SOC_IS_JERICHO(unit)? 11: 13));
    } else {
        res = soc_sand_bitstream_get_any_field(regs_val, 148 , 17, &pkt_tm_info->counter2);
        SOC_SAND_CHECK_FUNC_RESULT(res, 155, exit);
    }
  }
  ...
  if (tmp)
  {
    ...
    res = soc_sand_bitstream_get_any_field(SOC_IS_QAX(unit) ? regs_val : regs_val_part2, SOC_IS_QAX(unit) ? 52 : (SOC_IS_JERICHO(unit)? 11: 26), 16, &pkt_tm_info->lag_lb_key);
    SOC_SAND_CHECK_FUNC_RESULT(res, 250, exit);
  }
  ...
  /*dbg: fer2lbp_eq2_out_lif [17 : 0] */
  if (SOC_IS_QAX(unit)) {
      ARAD_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHB_ID,2,1,162,145,1751);
  } else if (SOC_IS_JERICHO(unit)) {
  ...
  )
{
  ...
  *ret_val = SOC_PPC_DIAG_OK;                                                           
  ...
  if (SOC_IS_QAX(unit)) {
      ARAD_PP_DIAG_FLD_READ(&fld,core_id,ARAD_EPNI_ID,4,4,8,0,5);
      tmp = regs_val[0] << 20;
      ARAD_PP_DIAG_FLD_READ(&fld,core_id,ARAD_EPNI_ID,4,3,255,236,5);
      regs_val[0] += tmp;
  } else if (SOC_IS_JERICHO(unit)) {
      ARAD_PP_DIAG_FLD_READ(&fld,core_id,ARAD_EPNI_ID,4,3,0,0,5);
      tmp = regs_val[0] << 28;
      ARAD_PP_DIAG_FLD_READ(&fld,core_id,ARAD_EPNI_ID,4,2,255,228,5);
      regs_val[0] += tmp;
  } else {
      ARAD_PP_DIAG_FLD_READ(&fld,core_id,ARAD_EPNI_ID,4,2,104,76,5);
  }
  ...
  if (SOC_IS_QAX(unit)) {
      ARAD_PP_DIAG_FLD_READ(&fld,core_id,ARAD_EPNI_ID,9,5,22,21,5);
  } else if (SOC_IS_JERICHO(unit)) {
  ...
  if (tmp == 1 && tmp2 == 5) {
      ...
      if (SOC_IS_JERICHO(unit)) {
          ARAD_PP_DIAG_FLD_READ(&fld,core_id,ARAD_EPNI_ID,2,0,87,64,5);
      } else {
      ...
      if (SOC_IS_JERICHO(unit)) {
          ARAD_PP_DIAG_FLD_READ(&fld,core_id,ARAD_EPNI_ID,2,0,103,64,5);
      } else {
  ...
  if (SOC_IS_QAX(unit)) {
      ARAD_PP_DIAG_FLD_READ(&fld,core_id,ARAD_EPNI_ID,4,3,235,104,5);
  } else if (SOC_IS_JERICHO(unit)) {
  ...
  /* VSI, Out-AC */
  if (SOC_IS_QAX(unit)) {
      ARAD_PP_DIAG_FLD_READ(&fld,core_id,ARAD_EPNI_ID,8,0,69,52,5);
  } else if (SOC_IS_JERICHO(unit)) {
  ...
  if (cur_eep_type != ARAD_PP_EG_ENCAP_EEDB_ACCESS_TYPE_OUT_AC) {
    ...
    /* VSI is taken from ESEM key */
      if (SOC_IS_QAX(unit)) {
          ARAD_PP_DIAG_FLD_READ(&fld,core_id,ARAD_EPNI_ID,11,0,117,102,5);
      } else if (SOC_IS_JERICHO(unit)) {
