...
exit_ingress_pmf:
  ...
  for (cycle_ndx = 0; cycle_ndx < SOC_PPC_FP_NOF_CYCLES; ++cycle_ndx)
  {
    ...
    for (key_ndx = 0; key_ndx < ARAD_PMF_LOW_LEVEL_NOF_KEYS; ++key_ndx)
    {
        access_profile_id = soc_mem_field32_get(unit, ihb_pmf_pass_lookup[cycle_ndx], &tbl_data, (SOC_IS_QAX(unit) ? fields_qax[key_ndx]:fields[key_ndx]));
  ...
  for (db_id_ndx = 0; db_id_ndx < SOC_PPC_FP_NOF_DBS; ++db_id_ndx)
  {
    ...
    for (key_id = 0; key_id < 2; key_id++) {
        ...
        access_profile_id = soc_mem_field32_get(unit, ihb_pmf_pass_lookup[cycle_db_id], &tbl_data, (SOC_IS_QAX(unit) ? fields_qax[key_ndx]:fields[key_ndx]));
        for (pd_ndx = 0; pd_ndx < ARAD_PP_FP_NOF_PDS; ++pd_ndx)
        {
