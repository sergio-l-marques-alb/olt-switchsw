  ...
  )
{
    ...
    if(0 != info->ptp_enabled) {
        ...
        SOC_SAND_SOC_IF_ERROR_RETURN(res, 10, exit, READ_EPNI_PACKET_PROCESSING_PORT_CONFIGURATION_TABLE_PP_PCTm(unit,  EPNI_BLOCK(unit, core), pp_port, entry));
        soc_EPNI_PACKET_PROCESSING_PORT_CONFIGURATION_TABLE_PP_PCTm_field32_set(unit, entry, IEEE_1588_MAC_ENABLEf, ext_mac_enable ? 0 : 1);
        SOC_SAND_SOC_IF_ERROR_RETURN(res, 20, exit, WRITE_EPNI_PACKET_PROCESSING_PORT_CONFIGURATION_TABLE_PP_PCTm(unit, EPNI_BLOCK(unit, core), pp_port, entry));
    }else {
        if (TRUE == ext_mac_enable) {
            SOC_SAND_SOC_IF_ERROR_RETURN(res, 30, exit, READ_IHB_PINFO_LBPm(unit,  IHB_BLOCK(unit, core), pp_port, entry));           
            soc_IHB_PINFO_LBPm_field32_set(unit, entry, RESET_TIME_STAMPf,  0);            
        ...
        SOC_SAND_SOC_IF_ERROR_RETURN(res, 10, exit, READ_EPNI_PACKET_PROCESSING_PORT_CONFIGURATION_TABLE_PP_PCTm(unit,  EPNI_BLOCK(unit, core), pp_port, entry));
        soc_EPNI_PACKET_PROCESSING_PORT_CONFIGURATION_TABLE_PP_PCTm_field32_set(unit, entry, IEEE_1588_MAC_ENABLEf, 0);
        SOC_SAND_SOC_IF_ERROR_RETURN(res, 20, exit, WRITE_EPNI_PACKET_PROCESSING_PORT_CONFIGURATION_TABLE_PP_PCTm(unit, EPNI_BLOCK(unit, core), pp_port, entry));
    }
