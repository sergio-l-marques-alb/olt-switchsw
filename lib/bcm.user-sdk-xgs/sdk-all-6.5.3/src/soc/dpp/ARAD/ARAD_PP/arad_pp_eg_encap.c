  ...
  )
{
  ...
  for (bank_ndx=0; bank_ndx < SOC_PPC_EG_ENCAP_NOF_BANKS(unit); bank_ndx++) {
    if (range_info->bank_access_phase[bank_ndx] < SOC_PPC_NOF_EG_ENCAP_ACCESS_PHASE_TYPES(unit)) {
        ...
        res = soc_sand_bitstream_set_any_field(&tmp, bank_ndx * nof_bits_per_bank, nof_bits_per_bank, fld_vals);
  ...
  SOC_DPP_ALLOW_WARMBOOT_WRITE(WRITE_EPNI_EEDB_OUTLIF_ACCESSr_REG64(unit, REG_PORT_ANY, val64), rv);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(rv, 30, exit);

  if (SOC_IS_JERICHO_PLUS(unit)) {
      /* Both registers need to be set with the same value in QAX. */
      SOC_DPP_ALLOW_WARMBOOT_WRITE(WRITE_EDB_EEDB_OUTLIF_ACCESSr_REG64(unit, val64), rv);
  ...
  )
{
  ...
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 10, exit, READ_EPNI_EEDB_OUTLIF_ACCESSr_REG64(unit, REG_PORT_ANY, &val64));
  ...
  for (ndx=0; ndx < SOC_PPC_EG_ENCAP_NOF_BANKS(unit); ndx++) {
      ...
      res = soc_sand_bitstream_get_any_field(fld_vals, ndx*nof_bits_per_bank, nof_bits_per_bank, &range_info->bank_access_phase[ndx]);
  ...
  )
{
  ...
  if ((nof_entries >= 1) && (encap_entry_info[0].entry_type == SOC_PPC_EG_ENCAP_ENTRY_TYPE_MPLS_ENCAP) &&
      ...
      tbl_data.mpls2_command = SOC_PPC_MPLS_COMMAND_TYPE_PUSH + encap_entry_info[0].entry_val.mpls_encap_info.tunnels[1].push_profile;
  ...
  )
{
  ...
  if (SOC_IS_QAX(unit) && profile_info->add_entropy_label_indicator && !(profile_info->add_entropy_label)) {
  ...
  )
{
  ...
  ARAD_PP_EG_ENCAP_CHECK_OUTLIF_ID(unit, info->out_ac, SOC_PPC_AC_ID_OUT_OF_RANGE_ERR, 11, exit);
