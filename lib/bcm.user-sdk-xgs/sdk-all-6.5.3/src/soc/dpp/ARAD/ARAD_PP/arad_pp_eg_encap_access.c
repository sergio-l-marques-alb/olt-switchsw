...
/*
 * In QAX, the eedb format names are prefixed with EDB_, and in Jericho and below they are prefixed 
 *  with EPNI_. For simplicity, use this macro to get the correct name.
 */

#define ARAD_PP_EG_ENCAP_FORMAT_BY_CHIP(_unit, _name)   \
      ((SOC_IS_JERICHO_PLUS(unit) || SOC_IS_JERICHO_B0(unit)) ? EDB_##_name##m : EPNI_##_name##m)
     ...
     )
{
    ...
    if (SOC_IS_ARADPLUS(unit) && SOC_IS_JERICHO_AND_BELOW(unit))  {
        ...
        soc_mem_field32_set(unit, ARAD_PP_EG_ENCAP_FORMAT_BY_CHIP(unit,  AC_HALF_ENTRY_FORMAT), table_entry, AC_0_ACTION_USE_AS_DATA_BITf, 0);
        soc_mem_field32_set(unit, ARAD_PP_EG_ENCAP_FORMAT_BY_CHIP(unit,  AC_HALF_ENTRY_FORMAT), table_entry, AC_1_ACTION_USE_AS_DATA_BITf, 1);
        ...
        soc_mem_field32_set(unit, ARAD_PP_EG_ENCAP_FORMAT_BY_CHIP(unit, OUT_RIF_FORMAT), table_entry, OUT_RIF_0_ACTION_DROPf, 1);
        soc_mem_field32_set(unit, ARAD_PP_EG_ENCAP_FORMAT_BY_CHIP(unit, OUT_RIF_FORMAT), table_entry, OUT_RIF_1_ACTION_DROPf, 1);
    }
  ...
  )
{
  ...
  if (SOC_IS_JERICHO_PLUS(unit)) {
      ...
      res = qax_pp_eg_encap_access_init_outrif_max(unit);
  ...
  if (key_prefix == ARAD_PP_EG_ENCAP_EEDB_ACCESS_TYPE_OUT_RIF 
      ...
      if (key_prefix2 == ARAD_PP_EG_ENCAP_EEDB_ACCESS_TYPE_NONE) {
          ...
          if (SOC_IS_JERICHO_AND_BELOW(unit)) {
               ...
               */
              soc_mem_field32_set(unit, ARAD_PP_EG_ENCAP_FORMAT_BY_CHIP(unit, AC_HALF_ENTRY_FORMAT), data, (is_location_at_start) ? AC_0_ENTRY_FORMATf : AC_1_ENTRY_FORMATf, 0);
          }

          /* 3.3 - Set the drop / data bits. */
          if (SOC_IS_ARADPLUS(unit) && SOC_IS_JERICHO_AND_BELOW(unit))  {
              soc_mem_field32_set(unit, ARAD_PP_EG_ENCAP_FORMAT_BY_CHIP(unit, AC_HALF_ENTRY_FORMAT), data, (is_location_at_start) ? AC_0_ACTION_USE_AS_DATA_BITf : AC_1_ACTION_USE_AS_DATA_BITf, 0);
          } else if (SOC_IS_ARAD_B1_AND_BELOW(unit)) {
              soc_mem_field32_set(unit, EPNI_AC_FORMATm, data, (is_location_at_start) ? AC_0_ACTION_DROPf : AC_1_ACTION_DROPf, 1);
              soc_mem_field32_set(unit, ARAD_PP_EG_ENCAP_FORMAT_BY_CHIP(unit, OUT_RIF_FORMAT), data, (is_location_at_start) ? OUT_RIF_0_ACTION_DROPf : OUT_RIF_1_ACTION_DROPf, 1);
          }
    ...
    )
{
    ...
    if ((access_prefix & ARAD_PP_EG_ENCAP_ACCESS_MPLS_ENTRY_PREFIX_MASK) ==  ARAD_PP_EG_ENCAP_ACCESS_PREFIX_TYPE_MPLS){
      *key_prefix = ARAD_PP_EG_ENCAP_EEDB_ACCESS_TYPE_MPLS;
        ...
        mem = ARAD_PP_EG_ENCAP_FORMAT_BY_CHIP(unit, DATA_FORMAT);
        ...
        /* for jericho/qmx: LL format is shared by LL eedb entry and ROO LL eedb entry. 
           To differentiate between them, check additional bit: ROO-Link-Layer-Format-Identifier */
            ...
            mem = ARAD_PP_EG_ENCAP_FORMAT_BY_CHIP(unit, LINK_LAYER_OR_ARP_NEW_FORMAT);
        ...
        *key_prefix = ARAD_PP_EG_ENCAP_EEDB_ACCESS_TYPE_TRILL;
        ...
        if (SOC_IS_JERICHO_PLUS(unit)) {
            ...
            *key_prefix = ARAD_PP_EG_ENCAP_EEDB_ACCESS_TYPE_OUT_AC;
            ...
            access_prefix_2_ent_ac = soc_mem_field32_get(unit, ARAD_PP_EG_ENCAP_FORMAT_BY_CHIP(unit, AC_HALF_ENTRY_FORMAT), data, (is_location_at_start) ? AC_0_ENTRY_FORMATf : AC_1_ENTRY_FORMATf);
            access_prefix_2_ent_rif = soc_mem_field32_get(unit, ARAD_PP_EG_ENCAP_FORMAT_BY_CHIP(unit, OUT_RIF_FORMAT), data, (is_location_at_start) ? OUT_RIF_0_ENTRY_FORMATf : OUT_RIF_1_ENTRY_FORMATf);
    ...
    if(*key_prefix == ARAD_PP_EG_ENCAP_EEDB_ACCESS_TYPE_MPLS) {
        /* mpls command get*/
        mpls1_command = soc_mem_field32_get(unit, ARAD_PP_EG_ENCAP_FORMAT_BY_CHIP(unit, MPLS_PUSH_FORMAT), data, MPLS_ACTION_MPLS_1_CMDf);
        SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
  ...
  )
{
  ...
  uint32
    tmp = 0,
    data[ARAD_PP_EG_ENCAP_ACCESS_FORMAT_TBL_ENTRY_SIZE];
  soc_mem_t mem = ARAD_PP_EG_ENCAP_FORMAT_BY_CHIP(unit, MPLS_SWAP_FORMAT);
  ...
  if (tbl_data->mpls1_command == ARAD_PP_EG_ENCAP_MPLS1_COMMAND_POP_val) {
    ...
    mem = ARAD_PP_EG_ENCAP_FORMAT_BY_CHIP(unit, MPLS_POP_FORMAT);
    ...
    mem = ARAD_PP_EG_ENCAP_FORMAT_BY_CHIP(unit, MPLS_SWAP_FORMAT);
    ...
    /* MPLS-Push-0-7 */  
    mem = ARAD_PP_EG_ENCAP_FORMAT_BY_CHIP(unit, MPLS_PUSH_FORMAT);
  ...
  )
{
  ...
  uint32
    tmp = 0,
    data[ARAD_PP_EG_ENCAP_ACCESS_FORMAT_TBL_ENTRY_SIZE];
  soc_mem_t mem = ARAD_PP_EG_ENCAP_FORMAT_BY_CHIP(unit, MPLS_POP_FORMAT);
  uint32 push_command_upper_limit_val = 0;
  ...
  )
{
  ...
  uint32
    tmp = 0,
    data[ARAD_PP_EG_ENCAP_ACCESS_FORMAT_TBL_ENTRY_SIZE];
  soc_mem_t mem = ARAD_PP_EG_ENCAP_FORMAT_BY_CHIP(unit, IPV4_TUNNEL_FORMAT);
  ...
  )
{
  ...
  uint32
    ...
    data[ARAD_PP_EG_ENCAP_ACCESS_FORMAT_TBL_ENTRY_SIZE];
  uint64 mac64;
  soc_mem_t mem = ARAD_PP_EG_ENCAP_FORMAT_BY_CHIP(unit, LINK_LAYER_OR_ARP_FORMAT);
  ...
  )
{
  ...
  uint32
    ...
    data[ARAD_PP_EG_ENCAP_ACCESS_FORMAT_TBL_ENTRY_SIZE];
  uint64 data64;
  soc_mem_t mem = ARAD_PP_EG_ENCAP_FORMAT_BY_CHIP(unit, DATA_FORMAT);
  ...
  )
{
  ...
  if (SOC_IS_JERICHO(unit)) {
    mem = ARAD_PP_EG_ENCAP_FORMAT_BY_CHIP(unit, TRILL_FORMAT_HALF_ENTRY);
  } else {
  ...
  )
{
  ...
  tmp = soc_mem_field32_get(unit, ARAD_PP_EG_ENCAP_FORMAT_BY_CHIP(unit, OUT_RIF_FORMAT), data, (is_location_at_start) ? OUT_RIF_0_ENTRY_FORMATf : OUT_RIF_1_ENTRY_FORMATf);
  ...
  tbl_data->drop = soc_mem_field32_get(unit, ARAD_PP_EG_ENCAP_FORMAT_BY_CHIP(unit, OUT_RIF_FORMAT), data, (is_location_at_start) ? OUT_RIF_0_ACTION_DROPf : OUT_RIF_1_ACTION_DROPf);
  

  tbl_data->oam_lif_set = soc_mem_field32_get(unit, ARAD_PP_EG_ENCAP_FORMAT_BY_CHIP(unit, OUT_RIF_FORMAT), data, (is_location_at_start) ? OUT_RIF_0_ACTION_OAM_LIF_SETf : OUT_RIF_1_ACTION_OAM_LIF_SETf);
  

  tbl_data->remark_profile = soc_mem_field32_get(unit, ARAD_PP_EG_ENCAP_FORMAT_BY_CHIP(unit, OUT_RIF_FORMAT), data, (is_location_at_start) ? OUT_RIF_0_ACTION_REMARK_PROFILEf : OUT_RIF_1_ACTION_REMARK_PROFILEf);
  
  /* In Jericho, vsi = outrif. Therefore, this field is not configured. */
  tbl_data->next_vsi_lsb = (SOC_IS_JERICHO(unit)) ? outlif : 
        soc_mem_field32_get(unit, ARAD_PP_EG_ENCAP_FORMAT_BY_CHIP(unit, OUT_RIF_FORMAT), data, (is_location_at_start) ? OUT_RIF_0_ACTION_VSI_LSBSf : OUT_RIF_1_ACTION_VSI_LSBSf);
  
  
  tbl_data->next_outlif_valid = soc_mem_field32_get(unit, ARAD_PP_EG_ENCAP_FORMAT_BY_CHIP(unit, OUT_RIF_FORMAT), data, (is_location_at_start) ? OUT_RIF_0_ACTION_ARP_PTR_VALIDf : OUT_RIF_1_ACTION_ARP_PTR_VALIDf);
  
  tbl_data->next_outlif = soc_mem_field32_get(unit, ARAD_PP_EG_ENCAP_FORMAT_BY_CHIP(unit, OUT_RIF_FORMAT), data, (is_location_at_start) ? OUT_RIF_0_ACTION_ARP_PTRf : OUT_RIF_1_ACTION_ARP_PTRf);
  
  if (SOC_IS_JERICHO(unit)) {
      tbl_data->outlif_profile = soc_mem_field32_get(unit, ARAD_PP_EG_ENCAP_FORMAT_BY_CHIP(unit, OUT_RIF_FORMAT), data, (is_location_at_start) ? OUT_RIF_0_ACTION_OUTLIF_PROFILEf : OUT_RIF_1_ACTION_OUTLIF_PROFILEf);
  }
  ...
  )
{
  ...
  soc_mem_field32_set(unit, ARAD_PP_EG_ENCAP_FORMAT_BY_CHIP(unit, OUT_RIF_FORMAT), raw_data, (is_location_at_start) ? OUT_RIF_0_ENTRY_FORMATf : OUT_RIF_1_ENTRY_FORMATf, tmp);
  

  /* Set OUT-RIF data */
  soc_mem_field32_set(unit, ARAD_PP_EG_ENCAP_FORMAT_BY_CHIP(unit, OUT_RIF_FORMAT), raw_data, (is_location_at_start) ? OUT_RIF_0_ACTION_DROPf : OUT_RIF_1_ACTION_DROPf, tbl_data->drop);       
  

  soc_mem_field32_set(unit, ARAD_PP_EG_ENCAP_FORMAT_BY_CHIP(unit, OUT_RIF_FORMAT), raw_data, (is_location_at_start) ? OUT_RIF_0_ACTION_OAM_LIF_SETf : OUT_RIF_1_ACTION_OAM_LIF_SETf, tbl_data->oam_lif_set);
  

  soc_mem_field32_set(unit, ARAD_PP_EG_ENCAP_FORMAT_BY_CHIP(unit, OUT_RIF_FORMAT), raw_data, (is_location_at_start) ? OUT_RIF_0_ACTION_REMARK_PROFILEf : OUT_RIF_1_ACTION_REMARK_PROFILEf, tbl_data->remark_profile);  
  ...
  if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
      soc_mem_field32_set(unit, ARAD_PP_EG_ENCAP_FORMAT_BY_CHIP(unit, OUT_RIF_FORMAT), raw_data, (is_location_at_start) ? OUT_RIF_0_ACTION_VSI_LSBSf : OUT_RIF_1_ACTION_VSI_LSBSf, tbl_data->next_vsi_lsb);  
  }
  

  soc_mem_field32_set(unit, ARAD_PP_EG_ENCAP_FORMAT_BY_CHIP(unit, OUT_RIF_FORMAT), raw_data, (is_location_at_start) ? OUT_RIF_0_ACTION_ARP_PTR_VALIDf : OUT_RIF_1_ACTION_ARP_PTR_VALIDf, tbl_data->next_outlif_valid);
  
  soc_mem_field32_set(unit, ARAD_PP_EG_ENCAP_FORMAT_BY_CHIP(unit, OUT_RIF_FORMAT), raw_data, (is_location_at_start) ? OUT_RIF_0_ACTION_ARP_PTRf : OUT_RIF_1_ACTION_ARP_PTRf, tbl_data->next_outlif);

  if (SOC_IS_JERICHO(unit)) {
      soc_mem_field32_set(unit, ARAD_PP_EG_ENCAP_FORMAT_BY_CHIP(unit, OUT_RIF_FORMAT), raw_data, (is_location_at_start) ? OUT_RIF_0_ACTION_OUTLIF_PROFILEf : OUT_RIF_1_ACTION_OUTLIF_PROFILEf, tbl_data->outlif_profile);
  }
  ...
  )
{
  ...
  uint8
    is_location_at_start;
  uint64 data64;
  soc_mem_t mem = ARAD_PP_EG_ENCAP_FORMAT_BY_CHIP(unit, AC_HALF_ENTRY_FORMAT);
  ...
  if (SOC_IS_JERICHO(unit)) {
      ...
      if (SOC_IS_ARADPLUS(unit) && is_location_at_start) {
          soc_field_t use_as_data_field = SOC_IS_QAX(unit) ? AC_PLUS_DATA_ENTRYf : AC_ACTION_USE_AS_DATA_BITf;
          
          mem = ARAD_PP_EG_ENCAP_FORMAT_BY_CHIP(unit, AC_ENTRY_WITH_DATA_FORMAT);
  ...
  )
{
    ...
    uint32
         is_big_ac_entry=0;
    uint64 data64;
    soc_mem_t data_entry_mem = ARAD_PP_EG_ENCAP_FORMAT_BY_CHIP(unit, AC_ENTRY_WITH_DATA_FORMAT);
    soc_field_t use_as_data_field = SOC_IS_QAX(unit) ? AC_PLUS_DATA_ENTRYf : AC_ACTION_USE_AS_DATA_BITf;
#endif
    soc_mem_t mem = ARAD_PP_EG_ENCAP_FORMAT_BY_CHIP(unit, AC_HALF_ENTRY_FORMAT);
