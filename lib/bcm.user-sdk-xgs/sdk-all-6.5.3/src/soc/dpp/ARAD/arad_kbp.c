	...
	for (qual_ndx = nof_qualifiers-1; qual_ndx >= 0 ; qual_ndx--) {
		...
		if (sal_strcmp(segment_name,next_segment_name) != 0) {
			...
			if (segment_ndx != -1) {
				...
				if (((segment_length_in_bits + zeroes_ones_padding_length) & 0x7) != 0) {
					SOC_SAND_SET_ERROR_CODE(SOC_SAND_GEN_ERR, 3, exit);
				}
				master_key->key_segment[segment_ndx].nof_bytes = (segment_length_in_bits + zeroes_ones_padding_length)/8;
...
exit:
    SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_kbp_convert_quals_to_segments", segment_length_in_bits, zeroes_ones_padding_length);
}
	...
	SOC_SAND_IN int						nof_qualifiers)
{
	...
	if (SHR_BITGET(&(kbp_ltr_config.parallel_srches_bmp), 0) && SHR_BITGET(&(kbp_ltr_config.parallel_srches_bmp), 2)) {
...
int
  arad_kbp_print_diag_all_entries(int unit){
	...
	LOG_CLI((BSL_META_U(unit,"|-------------------------------------------------------------------------------------------\n")));
	LOG_CLI((BSL_META_U(unit,"|------------------------------------  KBP All Entries  ------------------------------------\n")));
	LOG_CLI((BSL_META_U(unit,"|-------------------------------------------------------------------------------------------\n")));

	for (table_id = 0; table_id < ARAD_KBP_MAX_NUM_OF_TABLES;table_id++) {
		i=0;
		half[0] = 0;
		half[1] = 0;
		half[2] = 0;
		...
		if (i != 0) {
			...
			LOG_CLI((BSL_META_U(unit,"|________________________________________________________________________________\n")));
		}
	}
	LOG_CLI((BSL_META_U(unit,"|                                                                                           \n")));
	LOG_CLI((BSL_META_U(unit,"|-----------------------------------  End Of KBP tables ------------------------------------\n")));
...
int
  arad_kbp_print_diag_last_packet(int unit, int core, int flp_program){
	...
	*/
	...
	LOG_CLI((BSL_META_U(unit,"|-------------------------------------------------------------------------------------------\n")));
	LOG_CLI((BSL_META_U(unit,"| Core = %d, FLP program = %s (opcode=%d), LTR index = %d\n"), core_id, ARAD_KBP_FRWRD_IP_DB_TYPE_to_string(db_type), opcode, db_type));
