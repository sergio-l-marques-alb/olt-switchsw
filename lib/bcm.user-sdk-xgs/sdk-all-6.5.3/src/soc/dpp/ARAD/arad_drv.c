...
#define QAX_MAX_NOF_MC_GROUPS (64 * 1024) /* maximum number of QAX MC groups */
#define QAX_MAX_NOF_EGRESS_MESH_MC_GROUPS (32 * 1024) /* maximum number of QAX mesh MC groups */
...
#define SOC_DPP_DEFAULT_NOF_EGR_DSCP_EXP_MARKING(unit)   (SOC_IS_QAX(unit) ? 16 : 4)  /*maximum number of egress dscp exp marking*/
...
soc_arad_default_config_l3_get(int unit)
{
    ...
    if(SOC_IS_JERICHO(unit)){
        ...
        } else if(dpp_l3->actual_nof_routing_enablers_bm > 8 && ipmc_l3mcastl2_mode == 1) {/*4 bits + 1*/
...
int
soc_arad_is_ipsec(int unit, soc_port_t port, uint32* is_ipsec)
{
    soc_port_if_t interface;
    SOCDNX_INIT_FUNC_DEFS;

    SOCDNX_IF_ERR_EXIT(soc_port_sw_db_interface_type_get(unit, port, &interface));
    *is_ipsec = (SOC_PORT_IF_IPSEC == interface ? 1: 0);
    ...
    if (SOC_IS_JERICHO(unit)) {
        if (dpp_arad->init.fabric.connect_mode == SOC_TMC_FABRIC_CONNECT_MODE_MESH) {
            if (SOC_IS_KALIA(unit)){
                dpp_arad->init.fabric.fabric_mesh_multicast_enable = 1; /*In Kalia, MESH is always multi-cast*/
            }else {
