...
static void
l2uc_soc_set_up_mac_lpbk(int unit)
{
    int p;
...
static void
l2uc_turn_off_cmic_mmu_bkp(int unit)
{
    int cmc, ch;
...
calc_oversub_ratio(int unit)
{
    ...
    for (pipe = 0; pipe < si->num_pipe; pipe++) {
        ...
        for (i = 0; i < l2uc_p->num_fp_ports; i++) {
            if (SOC_PBMP_MEMBER(si->pipe_pbm[pipe], l2uc_p->port_list[i])) {
                if (l2uc_p->port_oversub[i] == 1) {
                    if (l2uc_p->dest_port[i] != NULL_DEST) {
                        ov_bandwidth += l2uc_p->port_speed[i];
                    }
                } else {
                    lr_bandwidth += l2uc_p->port_speed[i];
                }
            }
        }

        l2uc_p->ovs_ratio_x1000[pipe] =
            (ov_bandwidth) / ((pipe_bandwidth - lr_bandwidth) / 1000);

        if (l2uc_p->pkt_size_param >
            safe_pkt_size(unit, l2uc_p->ovs_ratio_x1000[pipe])) {
            l2uc_p->ovs_ratio_x1000[pipe] = 1000;
        }

        if (l2uc_p->ovs_ratio_x1000[pipe] < 1000) {
            l2uc_p->ovs_ratio_x1000[pipe] = 1000;
        }
 ...
 *      set_exp_rates
 * Purpose:
 *      Set expected rates array (l2uc_p->exp_rate). This is based on port
...
set_exp_rates(int unit)
{
    ...
    l2uc_p->exp_rate =
        (uint64 *) sal_alloc(l2uc_p->num_fp_ports * sizeof(uint64),
                             "exp_rate");

    for (i = 0; i < l2uc_p->num_fp_ports; i++) {
        if (l2uc_p->port_oversub[i] == 1) {
            COMPILER_64_SET(l2uc_p->exp_rate[i], 0,
                            l2uc_p->port_speed[i]);
            COMPILER_64_UMUL_32(l2uc_p->exp_rate[i], 1000000);
            COMPILER_64_SET(ovs_ratio_x1000_64, 0,
                            l2uc_p->ovs_ratio_x1000[get_pipe(unit,
                                                        l2uc_p->
                                                            port_list[i])]);
            COMPILER_64_UMUL_32(l2uc_p->exp_rate[i], 1000);
            COMPILER_64_UDIV_64(l2uc_p->exp_rate[i], ovs_ratio_x1000_64);
        } else {
            COMPILER_64_SET(l2uc_p->exp_rate[i], 0,
                            l2uc_p->port_speed[i]);
            COMPILER_64_UMUL_32(l2uc_p->exp_rate[i], 1000000);
    ...
    for (i = 0; i < l2uc_p->num_fp_ports; i++) {
        if (l2uc_p->dest_port[i] == NULL_DEST) {
            COMPILER_64_ZERO(l2uc_p->exp_rate[i]);
        }
    }

    for (i = 0; i < l2uc_p->num_fp_ports; i++) {
        if (COMPILER_64_GT(l2uc_p->exp_rate[i],
                                l2uc_p->exp_rate[l2uc_p->dest_port[i]])) {
            COMPILER_64_SET(l2uc_p->exp_rate[i],
                            COMPILER_64_HI(l2uc_p->
                                            exp_rate[l2uc_p->dest_port[i]]),
                            COMPILER_64_LO(l2uc_p->
                                            exp_rate[l2uc_p->dest_port[i]]));
        }
    }
    ...
    LOG_INFO(BSL_LS_APPL_TESTS,
             (BSL_META_U(unit, "\n=======================")));
    for (i = 0; i < l2uc_p->num_fp_ports; i++) {
        format_uint64_decimal(exp_rate_str, l2uc_p->exp_rate[i], 0);
        LOG_INFO(BSL_LS_APPL_TESTS,
...
set_port_property_arrays(int unit)
{
    ...
    for (i = 0; i < l2uc_p->num_fp_ports; i++) {
        ...
        switch(l2uc_p->port_speed[i]) {
            case 11000:
                l2uc_p->port_speed[i] = 10600;
            break;
            case 27000:
                l2uc_p->port_speed[i] = 26500;
            break;
            case 42000:
                l2uc_p->port_speed[i] = 42400;
            break;
        }
    }
    ...
    for (i = 0; i < l2uc_p->num_fp_ports; i++) {
        ...
        if(l2uc_p->dest_port[i] == NULL_DEST) {
            ...
            if(can_be_paired) {
                ...
                cli_out("\n%0d", l2uc_p->num_streams);
                cli_out("\t%0d", l2uc_p->port_list[i]);
                cli_out("\t%0d", l2uc_p->port_list[l2uc_p->dest_port[i]]);
                l2uc_p->num_streams++;
            }
        }
    }
    ...
    LOG_INFO(BSL_LS_APPL_TESTS,
                (BSL_META_U(unit, "\nPRINTING DEST PORT ARRAY")));
    LOG_INFO(BSL_LS_APPL_TESTS,
             (BSL_META_U(unit, "\n===============================")));

    for (i = 0; i < l2uc_p->num_fp_ports; i++) {
        LOG_INFO(BSL_LS_APPL_TESTS,
                 (BSL_META_U(unit, "\nFor port %0d, dest_port[%0d] = %0d"), i,
                  i, l2uc_p->dest_port[i]));
    }
    ...
    for (i = 0; i < l2uc_p->num_fp_ports; i++) {
        for (j = 0; j < TARGET_CELL_COUNT; j++) {
            ...
            } while (num_cells(unit, pkt_size, l2uc_p->port_list[i]) >
...
set_modport_map_sw(int unit, uint32 modid, uint32 index_upper, uint32 dest0)
{
    ...
    soc_mem_read(unit, MODPORT_MAP_SWm, COPYNO_ALL, index,
                 modport_map_sw_entry.entry_data);
    soc_mem_field32_set(unit, MODPORT_MAP_SWm, modport_map_sw_entry.entry_data,
                        DEST0f, dest0);
    soc_mem_field32_set(unit, MODPORT_MAP_SWm, modport_map_sw_entry.entry_data,
                        ENABLE0f, 0x1);
    soc_mem_write(unit, MODPORT_MAP_SWm, COPYNO_ALL, index,
                 modport_map_sw_entry.entry_data);
}
...
static int
l2uc_dma_chan_check_done(int unit, int vchan, soc_dma_poll_type_t type,
                              int *detected)
{
    int rv = SOC_E_NONE;
    ...
    if (pkt_size == 1) {
        while (total_cells < TARGET_CELL_COUNT) {
            total_cells += num_cells(unit,
                                     l2uc_p->rand_pkt_sizes[port][flood_cnt],
                                     l2uc_p->port_list[port]);
            flood_cnt++;
        }
    } else {
        flood_cnt = TARGET_CELL_COUNT / (num_cells(unit, pkt_size,
                                               l2uc_p->port_list[port]));
        flood_cnt = (flood_cnt * MAX_FP_PORTS) / (l2uc_p->num_fp_ports);
    }
...
void
l2uc_gen_random_l2_pkt(uint8 *pkt_ptr, uint32 pkt_size,
...
send_pkts(int unit)
{
    ...
    for (i = 0; i < l2uc_p->num_streams; i++) {
        ...
        if (l2uc_p->flood_pkt_cnt_param == 0) {
            ...
            flood_cnt = l2uc_p->flood_pkt_cnt_param;
        }
        ...
        for (j = 0; j < num_pkts_tx; j++) {
            if (use_random_packet_sizes == 1) {
                pkt_size = l2uc_p->rand_pkt_sizes[port][j];
            }
            packet_store_ptr =
            ...
            sal_srand(l2uc_p->pkt_seed + i + j);
            l2uc_gen_random_l2_pkt(packet_store_ptr, pkt_size, mac_da,
            ...
            while (channel_done == 0) {
                l2uc_dma_chan_check_done(unit, TX_CHAN,
...
get_rates(int unit, uint32 rate_calc_int)
{
    ...
    PBMP_ITER(PBMP_PORT_ALL(unit), p) {
        ...
        COMPILER_64_SET(l2uc_p->tpkt_start[i], COMPILER_64_HI(rdata),
    ...
    PBMP_ITER(PBMP_PORT_ALL(unit), p) {
        ...
        COMPILER_64_SET(l2uc_p->tbyt_start[i], COMPILER_64_HI(rdata),
                        COMPILER_64_LO(rdata));
        i++;
    }
    ...
    PBMP_ITER(PBMP_PORT_ALL(unit), p) {
        (void) soc_reg_get(unit, TBYTr, p, 0, &rdata);
        COMPILER_64_SET(l2uc_p->tbyt_end[i], COMPILER_64_HI(rdata),
                        COMPILER_64_LO(rdata));
        i++;
    }
    ...
    PBMP_ITER(PBMP_PORT_ALL(unit), p) {
        (void) soc_reg_get(unit, TPKTr, p, 0, &rdata);
        COMPILER_64_SET(l2uc_p->tpkt_end[i], COMPILER_64_HI(rdata),
                        COMPILER_64_LO(rdata));
        i++;
    }
    ...
    for (i = 0; i < l2uc_p->num_fp_ports; i++) {
        ...
        COMPILER_64_DELTA(tbyt_delta, l2uc_p->tbyt_start[i],
                          l2uc_p->tbyt_end[i]);
        COMPILER_64_DELTA(tpkt_delta, l2uc_p->tpkt_start[i],
                          l2uc_p->tpkt_end[i]);
        COMPILER_64_UMUL_32(tpkt_delta, (ipg + preamble));
        COMPILER_64_ADD_64(tbyt_delta, tpkt_delta);
        COMPILER_64_UMUL_32(tbyt_delta, 8);
        COMPILER_64_SET(l2uc_p->rate[i], COMPILER_64_HI(tbyt_delta),
                        COMPILER_64_LO(tbyt_delta));
        COMPILER_64_UDIV_64(l2uc_p->rate[i], rate_calc_int_64);
    }
}
...
check_rates(int unit)
{
    ...
    for (i = 0; i < l2uc_p->num_fp_ports; i++) {
        if (!(COMPILER_64_IS_ZERO(l2uc_p->exp_rate[i]))) {
            if (l2uc_p->port_oversub[i] == 1) {
                tolerance = l2uc_p->rate_tolerance_ov_param;
            } else {
                tolerance = l2uc_p->rate_tolerance_lr_param;
            }

            COMPILER_64_SET(margin_of_error,
                            COMPILER_64_HI(l2uc_p->exp_rate[i]),
                            COMPILER_64_LO(l2uc_p->exp_rate[i]));
            ...
            COMPILER_64_SET(port_line_rate, 0, l2uc_p->port_speed[i]);
            ...
            cli_out("\n");
            if ((COMPILER_64_LT(l2uc_p->rate[i], min_rate))
                || (COMPILER_64_GT(l2uc_p->rate[i], max_rate))) {
                cli_out("*ERROR: ");
                fail = 1;
            }
            format_uint64_decimal(avg_rate_str, l2uc_p->rate[i], 0);
...
set_up_ports(int unit)
{
    ...
    PBMP_ITER(PBMP_PORT_ALL(unit), p) {
        if (IS_HG_PORT(unit, p)) {
            ...
            soc_mem_write(unit, PORT_TABm, COPYNO_ALL, p,
...
check_mib_counters(int unit)
{
    ...
    for (i = 0; i < l2uc_p->num_fp_ports; i++) {
        for (j = 0; j < (sizeof(error_counters) / sizeof(soc_reg_t)); j++) {
            (void) soc_reg_get(unit, error_counters[j], l2uc_p->port_list[i], 0,
                        &rdata);
    ...
    for (i = 0; i < l2uc_p->num_streams; i++) {
        ...
        for (j = 0; j < (2 * flood_cnt); j++) {
            ...
            if (l2uc_p->hg_stream[i] == 1) {
                ...
                invalidate_vlan(unit, (XLATE_VLAN0 + i));
            }

            while (channel_done == 0 && timeout > 0) {
                l2uc_dma_chan_check_done(unit, RX_CHAN,
                                              SOC_DMA_POLL_CHAIN_DONE,
                                              &channel_done);
                timeout--;
            }
            ...
            if (timeout > 0) {
                ...
                sal_srand(l2uc_p->pkt_seed + i + seq_id);
                if (l2uc_p->hg_stream[i] == 1) {
                    l2uc_gen_random_l2_pkt(ref_pkt_ptr, pkt_size, mac_da,
                                            mac_sa, TPID, (VLAN + i), seq_id);
                } else {
                    l2uc_gen_random_l2_pkt(ref_pkt_ptr, pkt_size, mac_da,
                                            mac_sa, TPID, (XLATE_VLAN1 + i),
                                            seq_id);
                }
        ...
        if (pkt_count_port < (2 * flood_cnt)) {
            test_error(unit, "\n*ERROR: Stream %0d expected %0d pkts, "
                             "received %0d packets",
                             i, (2 * flood_cnt), pkt_count_port);
            l2uc_p->test_fail = 1;
        }
        pkt_count_port = 0;
    }
...
l2uc_test_init(int unit, args_t *a, void **pa)
{
    ...
    l2uc_soc_set_up_mac_lpbk(unit);
    l2uc_turn_off_cmic_mmu_bkp(unit);
    l2uc_soc_turn_off_fc(unit);
    /* coverity[dont_call : FALSE] */
...
l2uc_test_cleanup(int unit, void *pa)
{
    ...
    get_rates(unit, l2uc_p->rate_calc_interval_param);
