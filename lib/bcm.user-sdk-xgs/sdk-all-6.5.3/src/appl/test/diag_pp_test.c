 ...
 *  take the next bank.
 */ 
#define DIAG_PP_FIRST_DEFAULT_EGRESS_LOCAL_OUTLIF(_unit)  ((SOC_IS_JERICHO(_unit)) ? 8192 : 4096)
 ...
 */
#define DIAG_PP_FIRST_MPLS_DEFAULT_EGRESS_LOCAL_OUTLIF(_unit) ((SOC_IS_JERICHO_PLUS(unit)) ? 8192 : 4096)
...
#define DIAG_PP_DEAFULT_FEC(_unit)  (SOC_IS_JERICHO(_unit) ? 4096 : 1024)

/* 
 *  In multi core devices, since not all ports are on the same core, it's possible that the cpu port is not 0, and
 *  therefore the TM port is also different. In single core devices, this issue is irrelevant, so we always use TM port 0.
 */
#define DIAG_PP_TM_PORT(_unit, _in_local_port) ((SOC_DPP_DEFS_GET(_unit, nof_cores) > 1) ? _in_local_port : 0)
...
diag_pp_pkt_tm_info_get(int unit, int core_id, diag_pp_pkt_tm_info_fields_t *pkt_tm_info)
{
    ...
    if (SOC_IS_JERICHO_PLUS(unit)) {
        ...
        pkt_tm_info->valid_fields &= ~(SOC_PPC_DIAG_PKT_TM_FIELD_COUNTER2|SOC_PPC_DIAG_PKT_TM_FIELD_COUNTER1);
    }
...
int
diag_pp_received_packet_bridge_test(int unit, int core_id) {
    int rv = 0;
    ...
    rv = soc_port_sw_db_local_to_tm_port_get(unit, DIAG_PP_TM_PORT(unit, 200), &expected_pkt_info.in_tm_port, &expected_core);
    if ( rv < 0) {
        cli_out("Error: diag_pp_received_packet_bridge_test() local_to_pp Failed:\n");
        return rv;
    }
...
int
diag_pp_pkt_tm_info_bridge_test(int unit, int core_id) {
    int rv = 0;
...
int
diag_pp_frwrd_trace_info_bridge_test(int unit, int core_id) {
    int rv = 0;
...
cmd_result_t
diag_pp_encap_info_bridge_test(int unit, int core_id) {
    ...
    expected_encap_info->out_ac = (SOC_IS_JERICHO(unit)) ? 8193 : 4097;
...
int
diag_pp_frwrd_lkup_info_bridge_test(int unit, int core_id) {
    int rv = 0;
...
int
diag_pp_lem_lkup_info_bridge_test(int unit, int core_id) {
    int rv = 0;
...
int
diag_pp_received_packet_router_test(int unit, int core_id) {
    int rv = 0;
    diag_pp_received_packets_fields_t received_pkt_info;
    diag_pp_received_packets_fields_t expected_pkt_info;
    int        expected_core;
    ...
    rv = soc_port_sw_db_local_to_pp_port_get(unit, DIAG_PP_TM_PORT(unit, 200), &expected_pkt_info.in_tm_port, &expected_core);
    if ( rv < 0) {
        cli_out("Error: diag_pp_received_packet_router_test() local_to_pp Failed:\n");
        return rv;
    }
...
int
diag_pp_pkt_tm_info_router_test(int unit, int core_id) {
    int rv = 0;
    diag_pp_pkt_tm_info_fields_t received_pkt_tm_info;
    diag_pp_pkt_tm_info_fields_t expected_pkt_tm_info;
...
int
diag_pp_frwrd_trace_info_router_test(int unit, int core_id)
{
    int rv = 0;
    SOC_PPC_DIAG_FRWRD_DECISION_TRACE_INFO received_frwrd_trace_info;
    SOC_PPC_DIAG_FRWRD_DECISION_TRACE_INFO expected_frwrd_trace_info;
...
int
diag_pp_encap_info_router_test(int unit, int core_id)
{
    int rv = 0;
    int enacp_info_port = 201;
    int expected_core;

    SOC_PPC_DIAG_ENCAP_INFO received_encap_info;
    SOC_PPC_DIAG_ENCAP_INFO expected_encap_info;
...
diag_pp_frwrd_lkup_info_router_test(int unit, int core_id)
{
    ...
    expected_frwrd_lkup_info.key_found = (SOC_IS_JERICHO(unit)) ? 0 : 1;
    expected_frwrd_lkup_info.lkup_res.host_info.frwrd_decision.dest_id = (SOC_IS_JERICHO(unit)) ? 0 
                                                                                                  : 1024;
...
int
diag_pp_received_packet_mpls_test(int unit, int core_id)
{
    int rv = 0;
    diag_pp_received_packets_fields_t received_pkt_info;
    diag_pp_received_packets_fields_t expected_pkt_info;
...
int
diag_pp_pkt_tm_info_mpls_test(int unit, int core_id)
{
    int rv = 0;
    diag_pp_pkt_tm_info_fields_t received_pkt_tm_info;
    diag_pp_pkt_tm_info_fields_t expected_pkt_tm_info;
...
int
diag_pp_frwrd_trace_info_mpls_test(int unit, int core_id)
{
    int rv = 0;
    SOC_PPC_DIAG_FRWRD_DECISION_TRACE_INFO received_frwrd_trace_info;
    SOC_PPC_DIAG_FRWRD_DECISION_TRACE_INFO expected_frwrd_trace_info;
...
cmd_result_t
diag_pp_encap_info_mpls_test(int unit, int core_id)
{
    cmd_result_t result = CMD_OK;
    ...
    expected_encap_info->out_ac = 0;
    rv = soc_port_sw_db_local_to_pp_port_get(unit, 201, &expected_encap_info->pp_port, &expected_core);
    if (SOC_FAILURE(rv)) {
        cli_out("Error: diag_pp_received_packet_mpls_test() local_to_pp Failed:\n");
        result = CMD_FAIL;
        goto exit;
    }
    rv = soc_port_sw_db_local_to_tm_port_get(unit, 201, &expected_encap_info->tm_port, &expected_core);
    if (SOC_FAILURE(rv)) {
        cli_out("Error: diag_pp_received_packet_mpls_test() local_to_pp Failed:\n");
        result = CMD_FAIL;
        goto exit;
...
int
diag_pp_received_packet_vpls_test(int unit, int core_id, uint32 ac_to_pwe)
{
    int rv = 0;
    diag_pp_received_packets_fields_t received_pkt_info;
    diag_pp_received_packets_fields_t expected_pkt_info;
...
diag_pp_parsing_info_vpls_test(int unit, int core_id, uint32 ac_to_pwe)
{
    ...
    if (ac_to_pwe) {
        ...
        if (SOC_IS_JERICHO(unit)) { /* In jericho, due to second stage parsing, MPLS2_ETH is parsed as MPLS1_ETH*/
            expected_pars_info.hdr_type = SOC_PPC_PKT_HDR_STK_TYPE_ETH_MPLS1_ETH;
        } else{
...
int
diag_pp_encap_info_vpls_test(int unit, int core_id, uint32 ac_to_pwe)
{
    int rv = 0;
    SOC_PPC_DIAG_ENCAP_INFO received_encap_info;
    SOC_PPC_DIAG_ENCAP_INFO expected_encap_info;
...
int
diag_pp_frwrd_lkup_info_vpls_test(int unit, int core_id, uint32 ac_to_pwe)
{
    int rv = 0;
    SOC_PPC_DIAG_FRWRD_LKUP_INFO received_frwrd_lkup_info;
    SOC_PPC_DIAG_FRWRD_LKUP_INFO expected_frwrd_lkup_info;
...
int
diag_pp_lem_lkup_info_vpls_test(int unit, int core_id, uint32 ac_to_pwe)
{
    int rv = 0;
    diag_pp_lem_lkup_info_fields_t received_lem_lkup_info;
    diag_pp_lem_lkup_info_fields_t expected_lem_lkup_info;
...
int
diag_pp_frwrd_trace_info_vpls_test(int unit, int core_id, uint32 ac_to_pwe)
{
    int rv = 0;
    SOC_PPC_DIAG_FRWRD_DECISION_TRACE_INFO received_frwrd_trace_info;
    SOC_PPC_DIAG_FRWRD_DECISION_TRACE_INFO expected_frwrd_trace_info;
...
l2_count_bcm_command_test(int unit)
{
	...
	if (rv < 0) {
		cli_out("Error: per_vsi_get_count() Failed:\n");
        return rv;
	}else if (counter != 262143) {
		cli_out("Error: expected value 262143 from get_mact_num_entries(), got %d:\n",counter);
        return -1;
	}
...
diag_pp_test_init(int unit, args_t *a, void **p)
{
    ...
    *p = dpt;
    ...
    if (in_port != -1)
    {
        rv = soc_port_sw_db_core_get(unit, in_port, &(dpt->in_core));
        if ( rv < 0) {
    ...
    if (out_port != -1)
    {
        rv = soc_port_sw_db_core_get(unit, out_port, &(dpt->out_core));
        if ( rv < 0) {
...
diag_pp_test(int unit, args_t *a, void *p)
{
    ...
    if(dpt->bridge)
    {
        if((rv = diag_pp_received_packet_bridge_test(unit, dpt->in_core)) < 0)
        {
        ...
        if((rv = diag_pp_pkt_tm_info_bridge_test(unit, dpt->in_core)) < 0)
        {
        ...
        if((rv = diag_pp_frwrd_trace_info_bridge_test(unit, dpt->in_core)) < 0)
        {
        ...
        if((rv = diag_pp_encap_info_bridge_test(unit, dpt->out_core)) < 0)
        {
            cli_out("Error: diag_pp_encap_info_bridge_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_frwrd_lkup_info_bridge_test(unit, dpt->in_core)) < 0)
        {
        ...
        if((rv = diag_pp_lem_lkup_info_bridge_test(unit, dpt->in_core)) < 0)
        {
    ...
    {
        if((rv = diag_pp_received_packet_router_test(unit, dpt->in_core)) < 0)
        {
        ...
        if((rv = diag_pp_pkt_tm_info_router_test(unit, dpt->in_core)) < 0)
        {
        ...
        if((rv = diag_pp_frwrd_trace_info_router_test(unit, dpt->in_core)) < 0)
        {
        ...
        if((rv = diag_pp_encap_info_router_test(unit, dpt->out_core)) < 0)
        {
    ...
    {
        if((rv = diag_pp_received_packet_mpls_test(unit, dpt->in_core)) < 0)
        {
        ...
        if((rv = diag_pp_pkt_tm_info_mpls_test(unit, dpt->in_core)) < 0)
        {
        ...
        if((rv = diag_pp_frwrd_trace_info_mpls_test(unit, dpt->in_core)) < 0)
        {
        ...
        if((rv = diag_pp_encap_info_mpls_test(unit, dpt->out_core)) < 0)
        {
    ...
    else if(dpt->vpls)
    {
        ...
        if((rv = diag_pp_received_packet_vpls_test(unit, dpt->in_core, ac_to_pwe)) < 0)
        {
        ...
        if((rv = diag_pp_frwrd_trace_info_vpls_test(unit, dpt->in_core, ac_to_pwe)) < 0)
        {
            cli_out("Error: diag_pp_frwrd_trace_info_vpls_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_encap_info_vpls_test(unit, dpt->out_core, ac_to_pwe)) < 0)
        {
            cli_out("Error: diag_pp_encap_info_vpls_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_frwrd_lkup_info_vpls_test(unit, dpt->in_core, ac_to_pwe)) < 0)
        {
            cli_out("Error: diag_pp_frwrd_lkup_info_vpls_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_lem_lkup_info_vpls_test(unit, dpt->in_core, ac_to_pwe)) < 0)
        {
