...
/* Comment out this define in the bringup of a new device that not yet supporting MC groups */
#define MC_GROUPS_SUPPORTED
...
STATIC int
appl_dpp_convert_modport_to_base_voq_connector(int local_port, int core_id, int ingress_modid)
{
        ...
        LOG_ERROR(BSL_LS_APPL_COMMON,
                  (BSL_META_U(unit,
                              "appl_dpp_misc_diag_init failed. "
                               ...
                               " between base_modid and (base_modid+nof_devices*MAX_MODIDS_PER_DEVICE-1).\n")));
        return BCM_E_PARAM;
    }
...
appl_dpp_cosq_diag_init(int unit)
{
    ...
    int int_flags, core_idx, core_id, is_pdm_wide_mode, is_hdr_comp_tag_exists;
            ...
            for (idx = 0; idx < g_dii.num_ports; idx++) {
                for (core_id = 0; core_id < g_dii.nof_active_cores; core_id++) {
                    ...
                    for(core_idx = 0 ; core_idx < num_erp_ports ; core_idx++) {
                        if ((local_erp_port[core_idx] == port) && (modid_idx != g_dii.my_modid_idx)) {
                            continue;
                        }
                    }
                    ...
                    if (g_dii.base_modid == 0) {
                        voq_connector_id = appl_dpp_convert_modport_to_base_voq_connector(port, core_id, modid_idx * MAX_MODIDS_PER_DEVICE);
                    ...
                    if (g_dii.is_symmetric) {
                        for (core_id++; core_id < g_dii.nof_active_cores; core_id++) {
    ...
    for (modid_idx = 0; modid_idx < g_dii.nof_devices; modid_idx++) {
        for (idx = 0; idx < g_dii.num_ports; idx++) {
            ...
            if(domain == my_domain) {
                if (SOC_IS_QAX(unit) || (is_pdm_wide_mode && is_hdr_comp_tag_exists)) /* new mechanism */
                {
                    if (!SOC_IS_QAX(unit)) { 
                        for (tc_i = 0; tc_i < g_dii.num_cos; tc_i++) { /* set compensation at ingress to +22 */
                            ...
                            rv = bcm_cosq_gport_pkt_size_adjust_set(unit, &adjust_info, 22);
                            if (BCM_FAILURE(rv)) {
                                LOG_ERROR(BSL_LS_APPL_COMMON,
                                          (BSL_META_U(unit,
        ...
        for (modid_idx = 0; modid_idx < g_dii.nof_devices; modid_idx++) {
            for (idx = 0; idx < g_dii.num_ports; idx++) {
                for (core_id = (g_dii.is_symmetric ? BCM_CORE_ALL : 0);
                    ...
                    if (g_dii.base_modid == 0) {
                        voq_connector_id = appl_dpp_convert_modport_to_base_voq_connector(port, core_id, g_dii.my_modid);
...
int open_ingress_mc_group_for_olp(int unit, bcm_multicast_t *mc_group_id)
{
    ...
    *mc_group_id = (SOC_DPP_CONFIG(unit)->tm.nof_ingr_mc_ids) - 1;
    ...
    for (modid_idx = 0; modid_idx < g_dii.nof_devices; modid_idx++) {
        ...
        sys_port = appl_dpp_convert_modport_to_sysport_id(modid_idx, BCM_GPORT_LOCAL_GET(olp_port));
         ...
         void *userdata)
{
    if (operation == BCM_L2_CALLBACK_LEARN_EVENT) {
        ...
        bcm_l2_addr_delete(unit, l2addr->mac, l2addr->vid);
    }
}
...
#else /* MC_GROUPS_SUPPORTED not defined*/
...
{
    int rv = BCM_E_NONE;
    bcm_field_group_config_t grp, grp2, grp3, grp4, grp5, grp6, grp7;
    ...
    bcm_field_data_qualifier_t data_qual, data_qual2, data_qual3, data_qual4, data_qual5, data_qual6, data_qual7 ; 
   ...
   */
    ...
    bcm_field_data_qualifier_t_init(&data_qual2); 
    data_qual2.flags =  BCM_FIELD_DATA_QUALIFIER_LENGTH_BIT_RES | BCM_FIELD_DATA_QUALIFIER_OFFSET_BIT_RES  | BCM_FIELD_DATA_QUALIFIER_WITH_ID; 
    data_qual2.offset_base = bcmFieldDataOffsetBaseL2Header; 
    data_qual2.offset = 5 ; 
    data_qual2.length = 1; 
    data_qual2.qual_id = 59;
    rv = bcm_field_data_qualifier_create(unit, &data_qual2);
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_APPL_COMMON,
                  (BSL_META_U(unit,
                              "bcm_field_data_qualifier_create failed. Error:%d (%s)\n"),  
    ...
    bcm_field_data_qualifier_t_init(&data_qual3); 
    data_qual3.flags = BCM_FIELD_DATA_QUALIFIER_LENGTH_BIT_RES | BCM_FIELD_DATA_QUALIFIER_OFFSET_BIT_RES  | BCM_FIELD_DATA_QUALIFIER_WITH_ID; 
    data_qual3.offset_base = bcmFieldDataOffsetBaseL2Header; 
    data_qual3.offset = 24 ; 
    data_qual3.length = 4; 
    data_qual3.qual_id = 60;
    rv = bcm_field_data_qualifier_create(unit, &data_qual3);
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_APPL_COMMON,
                  (BSL_META_U(unit,
                              "bcm_field_data_qualifier_create failed. Error:%d (%s)\n"),  
    ...
    bcm_field_data_qualifier_t_init(&data_qual4); 
    data_qual4.flags = BCM_FIELD_DATA_QUALIFIER_LENGTH_BIT_RES | BCM_FIELD_DATA_QUALIFIER_OFFSET_BIT_RES  | BCM_FIELD_DATA_QUALIFIER_WITH_ID; 
    data_qual4.offset_base = bcmFieldDataOffsetBaseL2Header; 
    data_qual4.offset = 35 ; 
    data_qual4.length = 20; 
    data_qual4.qual_id = 61;
    rv = bcm_field_data_qualifier_create(unit, &data_qual4);
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_APPL_COMMON,
                  (BSL_META_U(unit,
                              "bcm_field_data_qualifier_create failed. Error:%d (%s)\n"),  
    ...
    bcm_field_data_qualifier_t_init(&data_qual5); 
    data_qual5.flags = BCM_FIELD_DATA_QUALIFIER_LENGTH_BIT_RES | BCM_FIELD_DATA_QUALIFIER_OFFSET_BIT_RES  | BCM_FIELD_DATA_QUALIFIER_WITH_ID; 
    data_qual5.offset_base = bcmFieldDataOffsetBaseL2Header; 
    data_qual5.offset = 2 ; 
    data_qual5.length = 2; 
    data_qual5.qual_id = 62;
    rv = bcm_field_data_qualifier_create(unit, &data_qual5);
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_APPL_COMMON,
                  (BSL_META_U(unit,
                              "bcm_field_data_qualifier_create failed. Error:%d (%s)\n"),  
    ...
    bcm_field_data_qualifier_t_init(&data_qual6); 
    data_qual6.flags = BCM_FIELD_DATA_QUALIFIER_LENGTH_BIT_RES | BCM_FIELD_DATA_QUALIFIER_OFFSET_BIT_RES  | BCM_FIELD_DATA_QUALIFIER_WITH_ID; 
    data_qual6.offset_base = bcmFieldDataOffsetBaseL2Header; 
    data_qual6.offset = 0 ; 
    data_qual6.length = 17; 
    data_qual6.qual_id = 63;
    rv = bcm_field_data_qualifier_create(unit, &data_qual6);
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_APPL_COMMON,
                  (BSL_META_U(unit,
                              "bcm_field_data_qualifier_create failed. Error:%d (%s)\n"),  
    ...
    if(stat_en)
    {
        ...
        bcm_field_data_qualifier_t_init(&data_qual7);
        data_qual7.flags = BCM_FIELD_DATA_QUALIFIER_LENGTH_BIT_RES | BCM_FIELD_DATA_QUALIFIER_WITH_ID;
        data_qual7.offset_base = bcmFieldDataOffsetBasePacketStart;
        data_qual7.offset = 2 ;
        data_qual7.length = 16;
        data_qual7.qual_id = 57;
        rv = bcm_field_data_qualifier_create(unit, &data_qual7);
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_APPL_COMMON,
                    (BSL_META_U(unit,
                            "bcm_field_data_qualifier_create failed. Error:%d (%s)\n"),
    ...
    */
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_APPL_COMMON,
                  (BSL_META_U(unit,
                              "bcm_field_presel_create failed. Error:%d (%s)\n"),  
                   rv, bcm_errmsg(rv)));
        return rv;
    }
    ...
    /* TM forwarding */
    rv = bcm_field_qualify_ForwardingType(unit, presel_id | BCM_FIELD_QUALIFY_PRESEL, bcmFieldForwardingTypeTrafficManagement);
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_APPL_COMMON,
                  (BSL_META_U(unit,
    ...
    */
    ...
    rv = bcm_field_qset_data_qualifier_add(unit, &grp2.qset, data_qual2.qual_id);
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_APPL_COMMON,
                  (BSL_META_U(unit,
                              "bcm_field_qset_data_qualifier_add failed. Error:%d (%s)\n"),  
    ...
    /*  Create the Field group with type Direct Extraction */
    grp2.priority = 123;
    grp2.flags = BCM_FIELD_GROUP_CREATE_WITH_MODE | BCM_FIELD_GROUP_CREATE_WITH_ID | BCM_FIELD_GROUP_CREATE_WITH_PRESELSET;
    grp2.mode = bcmFieldGroupModeDirectExtraction;
    grp2.preselset = psset;
    ...
    /*  Attach the action set */
    rv = bcm_field_group_action_set(unit, grp2.group, aset);
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_APPL_COMMON,
                  (BSL_META_U(unit,
                              "bcm_field_group_action_set failed. Error:%d (%s)\n"),  
    ...
    bcm_field_group_config_t_init(&grp3);
    grp3.group = 124;

    /* 
     * Define the QSET - use data qualifier. 
     */
    BCM_FIELD_QSET_INIT(grp3.qset);
    BCM_FIELD_QSET_ADD(grp3.qset, bcmFieldQualifyStageIngress);
    rv = bcm_field_qset_data_qualifier_add(unit, &grp3.qset, data_qual3.qual_id);
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_APPL_COMMON,
                  (BSL_META_U(unit,
                              "bcm_field_qset_data_qualifier_add failed. Error:%d (%s)\n"),  
    ...
    rv = bcm_field_qset_data_qualifier_add(unit, &grp3.qset, data_qual4.qual_id);
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_APPL_COMMON,
                  (BSL_META_U(unit,
                              "bcm_field_qset_data_qualifier_add failed. Error:%d (%s)\n"),  
    ...
    /*  Create the Field group with type Direct Extraction */
    grp3.priority = 124;
    grp3.flags = BCM_FIELD_GROUP_CREATE_WITH_MODE | BCM_FIELD_GROUP_CREATE_WITH_ID | BCM_FIELD_GROUP_CREATE_WITH_PRESELSET;
    grp3.mode = bcmFieldGroupModeDirectExtraction;
    grp3.preselset = psset;
    rv = bcm_field_group_config_create(unit, &grp3);
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_APPL_COMMON,
                  (BSL_META_U(unit,
                              "bcm_field_group_config_create failed. Error:%d (%s)\n"),  
    ...
    /*  Attach the action set */
    rv = bcm_field_group_action_set(unit, grp3.group, aset);
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_APPL_COMMON,
                  (BSL_META_U(unit,
                              "bcm_field_group_action_set failed. Error:%d (%s)\n"),  
    ...
    bcm_field_group_config_t_init(&grp4);
    grp4.group = 125;

    /* 
     * Define the QSET - use data qualifier. 
     */
    BCM_FIELD_QSET_INIT(grp4.qset);
    BCM_FIELD_QSET_ADD(grp4.qset, bcmFieldQualifyStageIngress);
    rv = bcm_field_qset_data_qualifier_add(unit, &grp4.qset, data_qual3.qual_id);
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_APPL_COMMON,
                  (BSL_META_U(unit,
                              "bcm_field_qset_data_qualifier_add failed. Error:%d (%s)\n"),  
    ...
    rv = bcm_field_qset_data_qualifier_add(unit, &grp4.qset, data_qual4.qual_id);
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_APPL_COMMON,
                  (BSL_META_U(unit,
                              "bcm_field_qset_data_qualifier_add failed. Error:%d (%s)\n"),  
    ...
    /*  Create the Field group with type Direct Extraction */
    grp4.priority = 125;
    grp4.flags = BCM_FIELD_GROUP_CREATE_WITH_MODE | BCM_FIELD_GROUP_CREATE_WITH_ID | BCM_FIELD_GROUP_CREATE_WITH_PRESELSET;
    grp4.mode = bcmFieldGroupModeDirectExtraction;
    grp4.preselset = psset;
    rv = bcm_field_group_config_create(unit, &grp4);
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_APPL_COMMON,
                  (BSL_META_U(unit,
                              "bcm_field_group_config_create failed. Error:%d (%s)\n"),  
    ...
    /*  Attach the action set */
    rv = bcm_field_group_action_set(unit, grp4.group, aset);
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_APPL_COMMON,
                  (BSL_META_U(unit,
                              "bcm_field_group_action_set failed. Error:%d (%s)\n"),  
    ...
    bcm_field_group_config_t_init(&grp5);
    grp5.group = 126;

    /* 
     * Define the QSET - use data qualifier. 
     */
    BCM_FIELD_QSET_INIT(grp5.qset);
    BCM_FIELD_QSET_ADD(grp5.qset, bcmFieldQualifyStageIngress);
    rv = bcm_field_qset_data_qualifier_add(unit, &grp5.qset, data_qual3.qual_id);
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_APPL_COMMON,
                  (BSL_META_U(unit,
                              "bcm_field_qset_data_qualifier_add failed. Error:%d (%s)\n"),  
    ...
    if(stat_en)
    {
        BCM_FIELD_QSET_ADD(grp5.qset, bcmFieldQualifyPtch);
        rv = bcm_field_qset_data_qualifier_add(unit, &grp5.qset, data_qual7.qual_id);
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_APPL_COMMON,
                    (BSL_META_U(unit,
                            "bcm_field_qset_data_qualifier_add failed. Error:%d (%s)\n"),
    ...
    /*  Create the Field group with type Direct Extraction */
    grp5.priority = 126;
    grp5.flags = BCM_FIELD_GROUP_CREATE_WITH_MODE | BCM_FIELD_GROUP_CREATE_WITH_ID | BCM_FIELD_GROUP_CREATE_WITH_PRESELSET;
    grp5.mode = bcmFieldGroupModeDirectExtraction;
    grp5.preselset = psset;
    rv = bcm_field_group_config_create(unit, &grp5);
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_APPL_COMMON,
                  (BSL_META_U(unit,
                              "bcm_field_group_config_create failed. Error:%d (%s)\n"),  
    ...
    /*  Attach the action set */
    rv = bcm_field_group_action_set(unit, grp5.group, aset);
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_APPL_COMMON,
                  (BSL_META_U(unit,
                              "bcm_field_group_action_set failed. Error:%d (%s)\n"),  
    ...
    bcm_field_group_config_t_init(&grp6);
    grp6.group = 127;

    /* 
     * Define the QSET - use data qualifier. 
     */
    BCM_FIELD_QSET_INIT(grp6.qset);
    BCM_FIELD_QSET_ADD(grp6.qset, bcmFieldQualifyStageIngress);
    rv = bcm_field_qset_data_qualifier_add(unit, &grp6.qset, data_qual5.qual_id);
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_APPL_COMMON,
                  (BSL_META_U(unit,
                              "bcm_field_qset_data_qualifier_add failed. Error:%d (%s)\n"),  
    ...
    rv = bcm_field_qset_data_qualifier_add(unit, &grp6.qset, data_qual6.qual_id);
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_APPL_COMMON,
                  (BSL_META_U(unit,
                              "bcm_field_qset_data_qualifier_add failed. Error:%d (%s)\n"),  
    ...
    /*  Create the Field group with type Direct Extraction */
    grp6.priority = 127;
    grp6.flags = BCM_FIELD_GROUP_CREATE_WITH_MODE | BCM_FIELD_GROUP_CREATE_WITH_ID | BCM_FIELD_GROUP_CREATE_WITH_PRESELSET;
    grp6.mode = bcmFieldGroupModeDirectExtraction;
    grp6.preselset = psset;
    rv = bcm_field_group_config_create(unit, &grp6);
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_APPL_COMMON,
                  (BSL_META_U(unit,
                              "bcm_field_group_config_create failed. Error:%d (%s)\n"),  
    ...
    /*  Attach the action set */
    rv = bcm_field_group_action_set(unit, grp6.group, aset);
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_APPL_COMMON,
                  (BSL_META_U(unit,
                              "bcm_field_group_action_set failed. Error:%d (%s)\n"),  
    ...
    bcm_field_group_config_t_init(&grp7);
    grp7.group = 121;

    /*
    * Define the QSET - use data qualifier.
    */
    BCM_FIELD_QSET_INIT(grp7.qset);
    BCM_FIELD_QSET_ADD(grp7.qset, bcmFieldQualifyStageIngress);
    rv = bcm_field_qset_data_qualifier_add(unit, &grp7.qset, data_qual.qual_id);
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_APPL_COMMON,
                  (BSL_META_U(unit,
                              "bcm_field_qset_data_qualifier_add failed. Error:%d (%s)\n"),
    ...
    */
    ...
    /*  Create the Field group with type Direct Extraction */
    grp7.priority = 121;
    grp7.flags = BCM_FIELD_GROUP_CREATE_WITH_MODE | BCM_FIELD_GROUP_CREATE_WITH_ID | BCM_FIELD_GROUP_CREATE_WITH_PRESELSET;
    grp7.mode = bcmFieldGroupModeDirectExtraction;
    grp7.preselset = psset;
    rv = bcm_field_group_config_create(unit, &grp7);
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_APPL_COMMON,
                  (BSL_META_U(unit,
                              "bcm_field_group_config_create failed. Error:%d (%s)\n"),
    ...
    /*  Attach the action set */
    rv = bcm_field_group_action_set(unit, grp7.group, aset);
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_APPL_COMMON,
                  (BSL_META_U(unit,
                              "bcm_field_group_action_set failed. Error:%d (%s)\n"),
    ...
    for (entry_ndx = 0; entry_ndx < nof_itmh_entries; entry_ndx++) {
     ...
     /*build extraction fields*/
      ...
      case 1: /* in_mirror*/
          ...
          q_data = 0x1;
          q_mask = 0x1;
          rv = bcm_field_qualify_data(unit, ent, data_qual2.qual_id, &q_data, &q_mask, 1);
          if (BCM_FAILURE(rv)) {
              LOG_ERROR(BSL_LS_APPL_COMMON,
                        (BSL_META_U(unit,
      ...
      case 4: /* multicast destination id */
          ...
          q_data = 0x2;
          q_mask = 0x3;
          rv = bcm_field_qualify_data(unit, ent, data_qual5.qual_id, &q_data, &q_mask, 1);
          if (BCM_FAILURE(rv)) {
              LOG_ERROR(BSL_LS_APPL_COMMON,
                        (BSL_META_U(unit,
                                    "bcm_field_qualify_data failed. Error:%d (%s)\n"),  
          ...
          bcm_field_extraction_field_t_init(&ext_field);
          ext_field.flags = BCM_FIELD_EXTRACTION_FLAG_DATA_FIELD;
          ext_field.qualifier = data_qual6.qual_id;
          ext_field.lsb = 0;
          ext_field.bits = 17; 
      ...
      case 5: /* snoop command */
          ...
          ext_field2[0].qualifier = data_qual.qual_id;
          ext_field2[0].bits  = 4;
          ext_field2[0].lsb   = 4;
      ...
      case 7: /* ITMH_DESTINATION_INFO EXTENSION_EXISTS field =1 , type = outlif*/
          q_data = 0x8;
          q_mask = 0xf;
          rv = bcm_field_qualify_data(unit, ent, data_qual3.qual_id, &q_data, &q_mask, 1);
          if (BCM_FAILURE(rv)) {
              LOG_ERROR(BSL_LS_APPL_COMMON,
                        (BSL_META_U(unit,
                                    "bcm_field_qualify_data failed. Error:%d (%s)\n"),  
          ...
          bcm_field_extraction_field_t_init(&ext_field);
          ext_field.flags = BCM_FIELD_EXTRACTION_FLAG_DATA_FIELD;
          ext_field.qualifier = data_qual4.qual_id;
          ext_field.lsb = 0;
          ext_field.bits = 16; 
          if (SOC_IS_JERICHO(unit)) {
              ext_field.bits = 18; 
       ...
       case 14: /* ITMH_DESTINATION_INFO EXTENSION_EXISTS =0- remove ITMH = 4 Bytes*/
        ...
        case 15: /* ITMH_DESTINATION_INFO EXTENSION_EXISTS field =1 , type = ISQ*/
            ...
            if(stat_en)
            {
                ...
                ext_field2[1].qualifier = data_qual7.qual_id;
                ext_field2[1].lsb = 0;
...
appl_dpp_bcm_diag_init(int unit, appl_dcmn_init_param_t* init_param)
{
    ...
    if (prop != NULL && !sal_strcmp(prop, "PP")) {
        ...
        if (soc_property_get(unit, spn_OAM_ENABLE, FALSE)) {
            DCMN_RUNTIME_DEBUG_PRINT(unit);
            LOG_INFO(BSL_LS_BCM_INIT,
        ...
        if (soc_property_get(unit, spn_BFD_ENABLE, FALSE)) {
            DCMN_RUNTIME_DEBUG_PRINT(unit);
            LOG_INFO(BSL_LS_BCM_INIT,
    ...
    if(appl_dpp_diag_sig_init(unit) != CMD_OK) {
        LOG_VERBOSE(BSL_LS_APPL_SHELL, (BSL_META_U(unit, "Signal DB was not initiated\n")));
    }
...
#define _SBUSDMA_DESC_TIME "SBUSDMA_DESC_TI"
        sal_strncpy(ctrl.name, _SBUSDMA_DESC_TIME, sizeof(_SBUSDMA_DESC_TIME));
        LOG_VERBOSE(BSL_LS_APPL_COMMON, (BSL_META_U(unit, "ctrl: ctrl.flags=0x%x, ctrl.cfg_count=%d, ctrl.buff=%p, ctrl.data=%p, ctrl.name=%s\n"), ctrl.flags, ctrl.cfg_count, ctrl.buff, ctrl.data, ctrl.name));
        ...
        SOC_IF_ERROR_RETURN(soc_sbusdma_desc_create(unit, &ctrl, &cfg, &_soc_sbusdma_desc_handles[0]));
...
cmd_dpp_ardon_card(int unit, args_t* a)
{
    ...
    if (! function ) {
        ...
        if (! option ) {
            return CMD_USAGE; 
        } else if (! sal_strncasecmp(option, "ardon_dvt_temp", strlen(option))) {
