...
{   
  ...
  glbl_prf_ptr_field   = SOC_IS_JERICHO_PLUS(unit) ? GLBL_ETH_MTR_PTRf : GLOBAL_METER_PROFILEf ;
  glbl_prf_valid_field = SOC_IS_JERICHO_PLUS(unit) ? GLBL_ETH_MTR_PTR_VALIDf : GLOBAL_METER_PROFILE_VALIDf ;
  eth_mtr_config_mem   = SOC_IS_JERICHO_PLUS(unit) ? CGM_ETH_MTR_PTR_MAPm : IDR_ETHERNET_METER_CONFIGm;
...
#ifndef __KERNEL__
static int diag_get_in_mem(int unit, SOC_PPC_LIF_ENTRY_INFO *lif_entry_p, char **table_name)
{
    int mem = 0;
    ...
    if((SOC_INFO(unit).chip_type == SOC_INFO_CHIP_TYPE_JERICHO) ||
        ...
        if(!SOC_MEM_IS_VALID(unit, mem)) {
            ...
            mem = PPDB_B_LIF_TABLEm;
                                         ...
                                         (SOC_INFO(unit).chip_type == SOC_INFO_CHIP_TYPE_KALIA)) {
        /* Look for table and present appropriately */
        switch(lif_entry_p->type) {
            case SOC_PPC_LIF_ENTRY_TYPE_EMPTY:
                mem = IHP_LIF_TABLEm;
...
static int diag_get_out_mem(int unit, ARAD_PP_EG_ENCAP_EEDB_ACCESS_TYPE access_type, char **table_name)
{
    ...
    else if((SOC_INFO(unit).chip_type == SOC_INFO_CHIP_TYPE_QAX) || (SOC_INFO(unit).chip_type == SOC_INFO_CHIP_TYPE_KALIA)) {
        /* Look for table and present appropriately */
        /* Look for table and present appropriately */
        switch(access_type) {
            case ARAD_PP_EG_ENCAP_EEDB_ACCESS_TYPE_NONE:
...
{   
	...
	if (0 >= nof_arg) {
	    cli_out(
	            ...
	            "\n\t\t[print]                  - Print out the current configuration of tables and DB."
