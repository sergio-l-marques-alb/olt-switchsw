...
int ipv4_dc_intf_creation_with_traffic(int unit, int vrf, int out_port, int num_of_hosts, int num_of_routes, int num_of_entries_in_lem, int is_double_capacity){
	...
	bcm_l3_egress_t  egress_intf2;
	bcm_l3_egress_t  egress_intf3;
	...
	egress_intf.mac_addr = next_hop_mac_host;
	egress_intf.port	 = out_port;
	egress_intf.intf 	 = intf2.l3a_intf_id;
	bcm_l3_egress_create(unit,0,&egress_intf,&eg_int);
	if (rv != BCM_E_NONE) {
		printf("Error, bcm_l3_egress_create 1\n");
        return -1;
	}
	...
	bcm_l3_egress_t_init(&egress_intf2);
	egress_intf2.mac_addr = next_hop_mac_route;
	egress_intf2.port	  = out_port;
	egress_intf2.intf 	  = intf3.l3a_intf_id;
	bcm_l3_egress_create(unit,0,&egress_intf2,&eg_int2);
	if (rv != BCM_E_NONE) {
		printf("Error, bcm_l3_egress_create 2\n");
        return -1;
	}
	...
	bcm_l3_egress_t_init(&egress_intf3);
	egress_intf3.mac_addr = next_hop_mac_lem;
	egress_intf3.port	  = out_port;
	egress_intf3.intf 	  = intf4.l3a_intf_id;
	bcm_l3_egress_create(unit,0,&egress_intf3,&eg_int3);
	if (rv != BCM_E_NONE) {
		printf("Error, bcm_l3_egress_create 3\n");
        return -1;
	}
...
int ipv4_dc_host_traverse_cb(int unit, int index, bcm_l3_host_t *info, void *user_data){
	...
	if (index < 4096) {
		...
		if(!(info->l3a_flags2 & BCM_L3_FLAGS2_SCALE_ROUTE)){
			printf("Error: expected to have BCM_L3_FLAGS2_SCALE_ROUTE set. flags2 = 0x%08x\n",info->l3a_flags2);
			return BCM_E_FAIL;
		}
	}
...
int ipv4_dc_route_traverse_cb(int unit, int index, bcm_l3_route_t *info, void* user_data){
...
int ipv4_dc_traverse(int unit){
	...
	rv = bcm_l3_host_traverse(unit,0,0,0,ipv4_dc_host_traverse_cb,NULL);
	if (rv != BCM_E_NONE) {
	...
	rv = bcm_l3_route_traverse(unit, 0, 0, 0, ipv4_dc_route_traverse_cb, NULL);
	if (rv != BCM_E_NONE) {
