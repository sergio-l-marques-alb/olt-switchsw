...
/* $Id: cint_field_oam_statistics.c,v 1.4 Broadcom SDK $
...
int oam_tx_up_stat(int unit) {
    ...
    if(!is_device_or_above(unit, JERICHO))
    {
       ...
       result = bcm_field_qualify_L2SrcHit(unit, presel_id | BCM_FIELD_QUALIFY_PRESEL, 1, 1);
       if (BCM_E_NONE != result) {
    ...
    result = bcm_field_qualify_Ptch(unit, presel_id | BCM_FIELD_QUALIFY_PRESEL, 0x7, 0x7);
    if (BCM_E_NONE != result) {
...
int oam_rx_down_stat(int unit) {
    ...
    result = bcm_field_qualify_RxTrapCode32(unit, presel_id | BCM_FIELD_QUALIFY_PRESEL, trap_code, 0xff);
    if (BCM_E_NONE != result) {
