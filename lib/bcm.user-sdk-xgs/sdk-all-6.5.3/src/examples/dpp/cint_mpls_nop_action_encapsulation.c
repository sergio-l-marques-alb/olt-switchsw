...
int mpls_nop_action_info_run(int unit) {
    ...
    bcm_l3_egress_t nh_l3_egress_object;
    ...
    bcm_l3_egress_t_init(&nh_l3_egress_object);
    nh_l3_egress_object.vlan   = mpls_nop_action_info.out_vsi;
    nh_l3_egress_object.mac_addr = mpls_nop_action_info.nh_da;
    nh_l3_egress_object.intf = label_stack.tunnel_id;
    nh_l3_egress_object.port   = mpls_nop_action_info.out_port;
    nh_l3_egress_object.flags = BCM_L3_ENCAP_SPACE_OPTIMIZED;

    rv = bcm_l3_egress_create(unit, 0/*flags*/, &nh_l3_egress_object, &nh_fec);
    if (rv != BCM_E_NONE) {
        printf("Error, bcm_l3_egress_create\n");
        return rv;
    }
    printf("NH ARP GLEM = 0x%x , FEC = %x\n", nh_l3_egress_object.encap_id, nh_fec);
    ll_encap_id = nh_l3_egress_object.encap_id;
