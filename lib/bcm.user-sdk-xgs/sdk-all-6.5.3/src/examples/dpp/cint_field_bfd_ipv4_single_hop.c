...
int bfd_ipv4_single_hop_field_action(int unit)
{
	...
	bcm_field_qualify_AppType(unit, presel_id | BCM_FIELD_QUALIFY_PRESEL, bcmFieldAppTypeBfdIp4SingleHop);
	if (BCM_E_NONE != result) {
   ...
   if (BCM_E_NONE != result) {
	...
	bcm_field_qualify_AppType(unit, presel_id2 | BCM_FIELD_QUALIFY_PRESEL, bcmFieldAppTypeBfdIp4SingleHop);
	if (BCM_E_NONE != result) {
          printf("Error in bcm_field_qualify_AppType\n");
          return result;
    }
	...
	result= bfd_ipv4_single_hop_field_trap_qualifier(unit,presel_set);
	if (BCM_E_NONE != result) {
	...
	result= bfd_ipv4_single_hop_field_oam_restore(unit,presel_set);
	if (BCM_E_NONE != result) {
...
int bfd_ipv4_single_hop_field_oam_restore(int unit, bcm_field_presel_set_t presel_set)
{
	...
	udp_qualifier.flags = BCM_FIELD_DATA_QUALIFIER_LENGTH_BIT_RES | BCM_FIELD_DATA_QUALIFIER_OFFSET_PREDEFINED;
...
int bfd_ipv4_single_hop_field_trap_qualifier(int unit, bcm_field_presel_set_t presel_set)
{
