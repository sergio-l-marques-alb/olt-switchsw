...
int example_ip_route_to_tunnel_mpls_additional_labels(int unit, int in_sysport, int out_sysport, int add_EL_ELI, int add_B1_B2, int add_C1_C2, int add_C3, int add_D1) {
    ...
    /* FEC */
    l3_egress.intf = add_B1_B2 ? additional_labels_info.B_intf : additional_labels_info.C_intf /* at least B or C must exist */;
    ...
    BCM_L3_ITF_LIF_TO_GPORT_TUNNEL(additional_labels_info.D_gport, l3_egress.encap_id);
    ...
    rv = add_host(unit, additional_labels_info.host, additional_labels_info.vrf, l3_egress.l3_egress_id);
    if (rv != BCM_E_NONE) {
