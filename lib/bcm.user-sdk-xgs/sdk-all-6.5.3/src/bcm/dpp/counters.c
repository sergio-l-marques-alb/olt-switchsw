...
_bcm_dpp_counter_unit_select_update(int unit, uint32 select_bitmap, _bcm_dpp_counter_state_t *unitData, bcm_dpp_counter_proc_specific_data_s * specific_proc_data) {
    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        if (select_bitmap & COUNTER_UPDATE_SELECT_FIFO_INFO) {
            for (fifo = 0; fifo < SOC_DPP_DEFS_GET(unit, nof_counter_fifos); fifo++) {
                rv = COUNTERS_FIFO_ACCESS.dma_channel.set(unit, fifo, unitData->fifo[fifo].dma_channel);
                BCMDNX_IF_ERR_EXIT(rv);
                rv = COUNTERS_FIFO_ACCESS.ref_count.set(unit, fifo, unitData->fifo[fifo].ref_count);
                BCMDNX_IF_ERR_EXIT(rv);
            }
            rv = COUNTERS_ACCESS.dma_attached.set(unit, unitData->dma_attached);
            BCMDNX_IF_ERR_EXIT(rv);
        }
...
_bcm_dpp_counter_unit_fill(int unit, _bcm_dpp_counter_state_t **unitDataPtr, uint32 select_bitmap, bcm_dpp_counter_proc_specific_data_s * specific_proc_data) {
	...
	if (select_bitmap & COUNTER_UPDATE_SELECT_FIFO_INFO)
	{
	    rv = COUNTERS_ACCESS.dma_attached.get(unit, &unitData->dma_attached);
	    BCMDNX_IF_ERR_EXIT(rv);
	}
    if (select_bitmap & COUNTER_UPDATE_SELECT_LIF_INFO)
    {
...
bcm_dpp_counter_state_diag_unmask(int unit) {
    ...
    /*restore data*/
    rv = _bcm_dpp_counter_unit_select_update(unit, COUNTER_UPDATE_SELECT_BACKROUND_INFO | COUNTER_UPDATE_SELECT_DIAGS | COUNTER_UPDATE_SELECT_THREAD, unitData, NULL);
    BCMDNX_IF_ERR_EXIT(rv);
...
bcm_dpp_counter_state_diag_mask(int unit) {
    ...
    /*write zero data*/
    rv = _bcm_dpp_counter_unit_select_update(unit, COUNTER_UPDATE_SELECT_BACKROUND_INFO | COUNTER_UPDATE_SELECT_DIAGS | COUNTER_UPDATE_SELECT_THREAD, zeroUnitData, NULL);
    BCMDNX_IF_ERR_EXIT(rv);
    _bcm_dpp_counter_state_buffer[unit].dirty_bitmap |= COUNTER_UPDATE_SELECT_BACKROUND_INFO | COUNTER_UPDATE_SELECT_DIAGS | COUNTER_UPDATE_SELECT_THREAD;
                              ...
                              unsigned int *depth)
{
    ...
    for (proc = 0; proc < unitData->num_counter_procs; proc++) {
        ...
        if (unitData->fifo[fifo_dma_index].dma_channel == SOC_TMC_CNT_INVALID_DMA_CHANNEL) {
        ...
        soc_sandResult = (MBCM_DPP_DRIVER_CALL((unit), mbcm_dpp_cnt_algorithmic_read, (unit, fifo_dma_index, unitData->fifo[fifo_dma_index].dma_channel, counter_info_array, data_p)));
...
void _bcm_dpp_counter_bg(void *unitPtr) {
    ...
    if (unitData) {
        ...
        rv = _bcm_dpp_counter_unit_select_update(unit, COUNTER_UPDATE_SELECT_THREAD, unitData, NULL);
        BCMDNX_IF_ERR_EXIT(rv);
        ...
        while (unitData->running) {
            ...
            if ((!unitData->diag.fifo_read_deferred) && (!unitData->background_disable)) {
                ...
                if (!mutexTaken) {
                    ...
                    rv = _bcm_dpp_counter_unit_select_update(unit, COUNTER_UPDATE_SELECT_DIAGS | COUNTER_UPDATE_SELECT_THREAD | COUNTER_UPDATE_SELECT_BACKROUND_INFO, unitData, NULL);
                    BCMDNX_IF_ERR_EXIT(rv);
                    continue;
                }
                        ...
                        uint64 *counter)
{
    ...
    if (!((base + num - 1) < (unitData->proc[proc].mode.num_counters)) && !((base >= base_limit_down) && ((base + num - 1) < base_limit_up)))
    {
        ...
        if ((base < unitData->proc[proc].mode.num_counters) || ((base >= base_limit_down) && (base < base_limit_up)))
        {
            ...
            {
                if( (unitData->proc[proc+1].valid == TRUE) &&
...
int _bcm_dpp_counter_dma_find_free_channel(
    ...
    uint8                       *channel)
{
    /*we want to take channel-0 only if there is no choise, since it is potensily shared with l2-learning*/
    CONST uint8 channels[SOC_DPP_DEFS_MAX(NOF_COUNTER_FIFOS)] = {SOC_MEM_FIFO_DMA_CHANNEL_1, SOC_MEM_FIFO_DMA_CHANNEL_2, SOC_MEM_FIFO_DMA_CHANNEL_3, SOC_MEM_FIFO_DMA_CHANNEL_0};
    int channel_index;
    int unit = unitData->cunit;
    int result = BCM_E_NONE;
    uint8 fifo, fifo_get = 0, found = FALSE, dma_channel;
    BCMDNX_INIT_FUNC_DEFS;
    BCMDNX_NULL_CHECK(unitData);
    ...
    for (fifo = 0; fifo < SOC_DPP_DEFS_GET(unit, nof_counter_fifos); fifo++) {
        if (unitData->fifo[fifo].ref_count == 0 && 
            unitData->fifo[fifo].dma_channel != SOC_TMC_CNT_INVALID_DMA_CHANNEL) {
            /*Have a channel and  don't need it.*/
            result = MBCM_DPP_SOC_DRIVER_CALL(unit, mbcm_dpp_cnt_channel_to_fifo_mapping_get, (unit, unitData->fifo[fifo].dma_channel, &fifo_get));
            BCMDNX_IF_ERR_EXIT(result);

            if (fifo_get != fifo) {
                BCMDNX_ERR_EXIT_MSG(BCM_E_INTERNAL, 
                                    (_BSL_BCM_MSG("unit %d channel %d is mapped to fifo %d in SW, but mapped to fifo %d in HW\n"), unit, fifo, unitData->fifo[fifo].dma_channel, fifo, fifo_get));
            }
            dma_channel = unitData->fifo[fifo].dma_channel;
            result = _bcm_dpp_counter_dma_fifo_dettach(unitData, fifo);
            BCM_SAND_IF_ERR_EXIT(result);

            /*Take this channel only is it wasn't taken by another application*/
            if (reserved_channel != dma_channel) {
                /*We freed this channel so we can take it now.*/
                *channel = dma_channel;
    ...
    /*Find a free channel in channels_to_fifo_index*/
    for (channel_index = 0; channel_index < SOC_DPP_DEFS_GET(unit, nof_counter_fifos); channel_index++) {
         /*Take this channel only is it wasn't taken by another application*/
        if (reserved_channel == channels[channel_index]) {
            continue;
        }
        result = MBCM_DPP_SOC_DRIVER_CALL(unit, mbcm_dpp_cnt_channel_to_fifo_mapping_get, (unit, channels[channel_index], &fifo)); 
        BCMDNX_IF_ERR_EXIT(result);

        if (fifo == SOC_TMC_CNT_INVALID_FIFO) {
            /*we have a match*/
...
int _bcm_dpp_counter_dma_fifo_dettach(_bcm_dpp_counter_state_t *unitData, int fifo){
    ...
    sand_rv = MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_cnt_dma_unset, (unit, fifo, unitData->fifo[fifo].dma_channel));
    BCM_SAND_IF_ERR_EXIT(sand_rv);

    result = MBCM_DPP_SOC_DRIVER_CALL(unit, mbcm_dpp_cnt_channel_to_fifo_mapping_set, (unit, unitData->fifo[fifo].dma_channel, SOC_TMC_CNT_INVALID_FIFO));
    BCMDNX_IF_ERR_EXIT(result);
...
int _bcm_dpp_counter_dma_fifo_update_state(_bcm_dpp_counter_state_t *unitData, int fifo, uint8 reserved_channel) {
    ...
    BCMDNX_NULL_CHECK(unitData);
    /*need to stop DMA*/
    if ((!unitData->dma_attached || unitData->fifo[fifo].ref_count == 0) && 
         unitData->fifo[fifo].dma_channel != SOC_TMC_CNT_INVALID_DMA_CHANNEL) {
        result = _bcm_dpp_counter_dma_fifo_dettach(unitData, fifo);
        BCMDNX_IF_ERR_EXIT(result);
    } 
    if (unitData->dma_attached && 
        (unitData->fifo[fifo].ref_count != 0 && unitData->fifo[fifo].dma_channel == SOC_TMC_CNT_INVALID_DMA_CHANNEL)) {
...
int _bcm_dpp_counter_dma_attach(_bcm_dpp_counter_state_t *unitData){
    ...
    if (!unitData->dma_attached) {
        ...
        for (fifo = 0; fifo < SOC_DPP_DEFS_GET(unit, nof_counter_fifos); fifo++) {
            ...
            if (unitData->fifo[fifo].dma_channel != SOC_TMC_CNT_INVALID_DMA_CHANNEL) {
...
bcm_dpp_counter_config_set(
    ...
    used_dma = unitData->proc[crps_index].valid && !SOC_TMC_CNT_FORMAT_IS_WIDE(unitData->proc[crps_index].mode.format);    
...
bcm_dpp_counter_init(int unit)
{
    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        for (index = 0; index < SOC_DPP_DEFS_GET(unit, nof_counter_fifos); index++) {
            tempUnit->fifo[index].dma_channel = SOC_TMC_CNT_INVALID_DMA_CHANNEL;
    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        tempUnit->bgWait = _BACKGROUND_THREAD_ITER_DEFAULT(tempUnit->cunit);
        result = _bcm_dpp_counter_unit_select_update(unit, COUNTER_UPDATE_SELECT_THREAD | COUNTER_UPDATE_SELECT_FIFO_INFO, tempUnit, NULL);
        BCMDNX_IF_ERR_EXIT(result);
...
_bcm_dpp_modify_proc_and_index(_bcm_dpp_counter_state_t * unitData, unsigned int *proc, unsigned int * finalIndex, uint8 subCount)
{
    ...
    else
    {
        ...
        if( ((*proc+1) < (SOC_DPP_DEFS_GET(unit, nof_counter_processors))) &&
            (unitData->proc[*proc+1].valid == TRUE) &&
            ((unitData->proc[*proc].mode.we_val + 1) == unitData->proc[*proc+1].mode.we_val) &&
            (unitData->proc[*proc].mode.command_id == unitData->proc[*proc+1].mode.command_id) &&
            (unitData->proc[*proc].mode.src_type == unitData->proc[*proc+1].mode.src_type) &&
            (unitData->proc[*proc].mode.mode_eg.type == unitData->proc[*proc+1].mode.mode_eg.type))
