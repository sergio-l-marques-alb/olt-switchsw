...
{   
    ...
    SOC_PPC_LIF_ING_VLAN_EDIT_TAG_VID_SRC outer_vid_source = 0, inner_vid_source = 0;
    SOC_PPC_LIF_ING_VLAN_EDIT_TAG_PCP_DEI_SRC pcp_dei_source;
    SOC_PPC_EG_VLAN_EDIT_TAG_VID_SRC outer_vid_source_eg = 0, inner_vid_source_eg = 0;
    ...
    for (vlan_format = 0; vlan_format < SOC_SAND_PP_NOF_ETHERNET_FRAME_VLAN_FORMATS; vlan_format++) {
        ...
        for (tag_type = 0; tag_type < DPP_VLAN_TRANSLATION_MAX_NOF_TAGS; tag_type++) {
          ...
          if (tag_type == DPP_VLAN_TRANSLATION_OUTER_TAG) {            
            ...
            if ((outer_vid_source != SOC_PPC_LIF_ING_VLAN_EDIT_TAG_VID_SRC_EMPTY) && action_set->outer_tpid_action == bcmVlanTpidActionModify) {
            ...
            if ((inner_vid_source != SOC_PPC_LIF_ING_VLAN_EDIT_TAG_VID_SRC_EMPTY) && action_set->inner_tpid_action == bcmVlanTpidActionModify) {
              /* Match value to index */
        ...
        if (is_ivec_to_set) {
          ...
          for (tag_type = 0; tag_type < DPP_VLAN_TRANSLATION_MAX_NOF_TAGS; tag_type++) {
            ...
            if (tag_type == DPP_VLAN_TRANSLATION_OUTER_TAG) {
              ...
              if (mapping_info->ivec[vlan_format].outer_tag.vid_source != SOC_PPC_LIF_ING_VLAN_EDIT_TAG_VID_SRC_EMPTY) {
              ...
              if (eg_mapping_info->evec[vlan_format].outer_tag.vid_source != SOC_PPC_EG_VLAN_EDIT_TAG_VID_SRC_EMPTY) {
              ...
              /* Take TPID action only in case VID source is not empty (i.e. VID is created from VLAN translation) */
              if (mapping_info->ivec[vlan_format].inner_tag.vid_source != SOC_PPC_LIF_ING_VLAN_EDIT_TAG_VID_SRC_EMPTY) {
              ...
              if (eg_mapping_info->evec[vlan_format].inner_tag.vid_source != SOC_PPC_EG_VLAN_EDIT_TAG_VID_SRC_EMPTY) {
    ...
    )
{
    ...
    if (ing_command_info) {
        ing_command_info->outer_tag.tpid_index = (ing_command_info->outer_tag.vid_source != SOC_PPC_LIF_ING_VLAN_EDIT_TAG_VID_SRC_EMPTY) ?
            outer_tpid_index : 0;
        ing_command_info->inner_tag.tpid_index = (ing_command_info->inner_tag.vid_source != SOC_PPC_LIF_ING_VLAN_EDIT_TAG_VID_SRC_EMPTY) ? 
            inner_tpid_index : 0;
    }

    if (eg_command_info) {
        eg_command_info->outer_tag.tpid_index = (eg_command_info->outer_tag.vid_source != SOC_PPC_EG_VLAN_EDIT_TAG_VID_SRC_EMPTY) ?
            outer_tpid_index : 0;
        eg_command_info->inner_tag.tpid_index = (eg_command_info->inner_tag.vid_source != SOC_PPC_EG_VLAN_EDIT_TAG_VID_SRC_EMPTY) ?
            inner_tpid_index : 0;
    }
    ...
    int *forced_tpid_profile)
{
    ...
    if (eg_command_info) {
        ...
        eg_command_info->packet_is_tagged_after_eve = bcm_dpp_action->packet_is_tagged_after_eve;
    ...
    */
    ...
    /* Handle the Egress variation. The Set is done to a SW DB and to the HW */
        ...
        if (SOC_IS_JERICHO_B0_AND_ABOVE(unit)) {
            ...
            eg_command_info.packet_is_tagged_after_eve = (action->flags & BCM_VLAN_ACTION_SET_OUTER_VID_UNTAGGED) ? 0 : 1;
            bcm_dpp_action.packet_is_tagged_after_eve =  eg_command_info.packet_is_tagged_after_eve;

            /* Jericho_B0 only: Verify BCM_VLAN_ACTION_SET_OUTER_VID_UNTAGGED flag is set only if outer_vid_source is empty */
            if ((SOC_IS_JERICHO_B0(unit) || SOC_IS_QMX_B0(unit)) &&
    ...
    int arg)
{
    ...
    case bcmVlanMemberMismatchLearn:
    {
        ...
        soc_sand_rv = soc_ppd_trap_frwrd_profile_info_set(soc_sand_dev_id, SOC_PPC_TRAP_CODE_PORT_NOT_VLAN_MEMBER, &profile_info); 
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        break;
    }  
    break;    
    ...
    */
    ...
    if (is_egress || is_ingress) {
        ...
        if ((is_ingress && in_lif_usage != _bcmDppLifTypeVlan)
            || (is_egress && out_lif_usage != _bcmDppLifTypeVlan)) { /* VLAN */
    ...
    if (is_egress) {
        ...
        if (SOC_IS_JERICHO(unit)) {
            ...
            if ((is_wide_entry == BCM_DPP_AM_OUT_LIF_FLAG_WIDE) && (ext_type == 0) && eg_ac_info.protection_info.is_protection_valid) {
                ...
                vlan_port->egress_failover_port_id = (eg_ac_info.protection_info.protection_pass_value) ? 0 : 1;
...
int _bcm_dpp_vlan_port_create(int unit, bcm_vlan_port_t *vlan_port)
{
    ...
    if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "pon_pp_port_mapping_bypass", 0) == 1) {
        if (vlan_port->match_tunnel_value > 4095) 
           ...
           ((encap_with_id) && BCM_ENCAP_ID_GET(vlan_port->encap_id) != BCM_ENCAP_ID_GET(vlan_port_o.encap_id))) {
            ...
            if (SOC_IS_JERICHO(unit)) {
                ...
                if (is_egress && !skip_egress_alloc) {   
                    ...
                    local_out_lif_info.app_alloc_info.application_type = (is_3_tags_manipulated) ? bcm_dpp_am_egress_encap_app_out_big_ac
    ...
    if (_BCM_DPP_GPORT_INFO_IS_LOCAL_PORT(gport_info) && !remote_lif) {
            ...
            if (is_egress && !skip_egress_alloc) {
                ...
                if (!update) {
                    soc_sand_rv = soc_ppd_eg_encap_next_outlif_update(unit, active_local_out_lif_id, SOC_PPC_EG_ENCAP_ENTRY_UPDATE_DROP, 0);
                    SOC_SAND_IF_ERR_EXIT(soc_sand_rv);
                }
            }
    }
