 ...
 */
#define _BCM_BFD_MAX_NUM_ENDPOINTS  2047
...
#define BFD_UC_INFO(u_)                     (bfd_dpp_info[(u_)])
#define BFD_ENDPOINT_CONFIG(u_, index_)  \
    (&(BFD_UC_INFO((u_))->endpoints[(index_)]))

/*
 * Utility macros
 */
/* True if BFD module has been initialized */
#define BFD_UC_INIT(u_)                                        \
    ((BFD_UC_INFO(u_) != NULL) && (BFD_UC_INFO(u_)->initialized))
...
#define BFD_UC_INIT_CHECK(u_)                                          \
    if ((BFD_UC_INFO(u_) == NULL) || (!BFD_UC_INFO(u_)->initialized)) {   \
        return BCM_E_INIT;                  \
    }
...
typedef struct bfd_info_s
{
    ...
    bfd_event_handler_t *event_handler_list; /* List of event handlers */
    uint32 event_mask;                       /* Mask of all events */
} bfd_info_t;

static bfd_info_t *bfd_dpp_info[SOC_MAX_NUM_DEVICES] = {0};
...
{
    bfd_info_t *bfd_info = BFD_UC_INFO(unit);;

    if (bfd_dpp_info[unit] != NULL) {
        ...
        sal_free(bfd_dpp_info[unit]);
        bfd_dpp_info[unit] = NULL;
    }
                             ...
                             int num_auth_sha1_keys, int num_auth_sp_keys)
{
    ...
    if ((bfd_dpp_info[unit] =
         _bcm_petra_bfd_uc_alloc_clear(sizeof(bfd_info_t),
                                   "BFD info")) == NULL) {
        return BCM_E_MEMORY;
    }

    bfd_info = BFD_UC_INFO(unit);
    if ((bfd_info->endpoints =
         _bcm_petra_bfd_uc_alloc_clear(num_sessions *
    ...
    if ((bfd_info->endpoints_in_use =
         _bcm_petra_bfd_uc_alloc_clear(SHR_BITALLOCSIZE(num_sessions),
    ...
    if (num_auth_sha1_keys > 0) {
        if ((bfd_info->auth_sha1 =
             _bcm_petra_bfd_uc_alloc_clear(num_auth_sha1_keys *
    ...
    if (num_auth_sp_keys > 0) {
        if ((bfd_info->auth_sp =
             _bcm_petra_bfd_uc_alloc_clear(num_auth_sp_keys *
    ...
    bfd_info->dma_buffer = soc_cm_salloc(unit, bfd_info->dma_buffer_len, "BFD DMA buffer");
...
STATIC int _bcm_petra_bfd_uc_trap_code_create(int unit, bcm_bfd_endpoint_info_t *endpoint_info, uint32 *trap_code) {
    ...
    if (BCM_GPORT_IS_TRAP(endpoint_info->remote_gport)) {
        ...
        rv = _bcm_dpp_oam_bfd_trap_code_get_and_prepare_for_action(unit, &classifier_mep_entry, endpoint_info->remote_gport, BCM_GPORT_INVALID, trap_code, trap_code);
 ...
 * @return int 
 */
STATIC int _bcm_petra_bfd_uc_m_hop_set_classifer(int unit, bcm_bfd_endpoint_info_t *endpoint_info, uint32 trap_code, bfd_endpoint_config_t *endpoint_config) {
    int rv;
    uint32 soc_sand_rv;
...
_bcm_petra_bfd_uc_callback_thread(int unit)
{
    ...
    while (1) {
         ...
         */
        _BCM_BFD_UC_MEP_INDEX_FROM_INTERNAL(sess_id, sess_id);
 ...
 */
STATIC int
_bcm_petra_bfd_uc_find_free_endpoint(SHR_BITDCL *endpoints_in_use,
                                int endpoint_count)
{
    int endpoint_index;
                               ...
                               uint8 *buffer)
{
    ...
    if (packet_flags & _BCM_BFD_ENCAP_PKT_UDP) {
        BCM_IF_ERROR_RETURN
            (_bcm_petra_bfd_uc_udp_header_get(unit, endpoint_info,
                                          packet_flags, &udp));
    }

    if (packet_flags & _BCM_BFD_ENCAP_PKT_IP) {
        BCM_IF_ERROR_RETURN
            (_bcm_petra_bfd_uc_ip_headers_get(endpoint_info,
    ...
    if (packet_flags & _BCM_BFD_ENCAP_PKT_MPLS) {
        ...
        BCM_IF_ERROR_RETURN
            (_bcm_petra_bfd_uc_mpls_labels_get(unit, endpoint_info,
                                           packet_flags,
    ...
    BCM_IF_ERROR_RETURN
        (_bcm_petra_bfd_uc_itmh_ext_header_get(unit, endpoint_info, &itmh_ext));

    /* Always build ITMH Header */
    BCM_IF_ERROR_RETURN
        (_bcm_petra_bfd_uc_itmh_header_get(unit, endpoint_info, &itmh));
...
bcm_petra_bfd_uc_init(int unit)
{
    ...
    if (num_sessions > _BCM_BFD_MAX_NUM_ENDPOINTS) {
        return BCM_E_CONFIG;
    }
     ...
     */
    rv = _bcm_petra_bfd_uc_alloc_resource(unit, num_sessions, num_auth_sha1, num_auth_sp);
    if (BCM_FAILURE(rv)) {
        _bcm_petra_bfd_uc_free_resource(unit);
        return rv;
    }
     ...
     */
    rv = soc_cmic_uc_appl_init(unit, bfd_info->uc_num, MOS_MSG_CLASS_BFD,
                               _BCM_BFD_UC_MSG_TIMEOUT_USECS,
                               BFD_SDK_VERSION,
    ...
    bfd_info->rx_channel = _BCM_BFD_RX_CHANNEL;
    ...
    shr_bfd_msg_ctrl_init_pack(buffer, &msg_init);
    send_len = sizeof(shr_bfd_msg_ctrl_init_t);
                                        ...
                                        bcm_bfd_endpoint_info_t *endpoint_info)
{
    ...
    bfd_info_t *bfd_info = BFD_UC_INFO(unit);
    bfd_endpoint_config_t *endpoint_config;
    bfd_endpoint_config_t  endpoint_config_cur;
    int update;
    int endpoint_index;
    ...
    if (endpoint_info->flags & BCM_BFD_ENDPOINT_WITH_ID) {
        ...
        /* Allocate endpoint */
        endpoint_index =
            _bcm_petra_bfd_uc_find_free_endpoint(bfd_info->endpoints_in_use,
    ...
    if (update && (endpoint_info->flags & BCM_BFD_ENDPOINT_IN_HW)) {
        rv = bcm_petra_bfd_uc_endpoint_get(unit, endpoint_index, &(endpoint_config->info));
        BCMDNX_IF_ERR_EXIT(rv); 
    }
    ...
    endpoint_config_cur.info           = *endpoint_info;
    ...
    if (!update || (endpoint_info->flags & BCM_BFD_ENDPOINT_ENCAP_SET)) {
        ...
        if ((SOC_IS_JERICHO(unit) && 
            ...
            BCM_IF_ERROR_RETURN
                (_bcm_petra_bfd_uc_m_hop_set_classifer(unit, endpoint_info, trap_code, &endpoint_config_cur));
        } else {  
            ARAD_PP_LEM_BFD_ONE_HOP_ENTRY_INFO lem_info;
            ARAD_PP_LEM_BFD_ONE_HOP_ENTRY_INFO_clear(&lem_info);

            fwd_strength = soc_property_get(unit, spn_DEFAULT_TRAP_STRENGTH, 6);
            snp_strength = 0;                
                                    ...
                                    bcm_bfd_endpoint_info_t *endpoint_info)
{
    ...
    if ((endpoint_info->flags & BCM_BFD_ENDPOINT_IN_HW) && 
        ...
        if (BFD_FEATURE_CHECK(BFD_SDK_VERSION, bfd_firmware_version, BFD_FEATURE_POLL_BIT)) {
...
bcm_petra_bfd_uc_endpoint_destroy(int unit, bcm_bfd_endpoint_t endpoint)
{
    ...
    if ((SOC_IS_JERICHO(unit) &&
        ...
        rv= MBCM_PP_DRIVER_CALL(unit, mbcm_pp_lem_access_bfd_one_hop_lem_entry_remove, (unit, endpoint_config->info.local_discr)); 
