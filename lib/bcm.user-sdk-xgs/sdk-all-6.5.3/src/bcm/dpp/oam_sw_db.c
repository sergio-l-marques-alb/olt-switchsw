/*
 * $Id: oam_sw_db.c,v 1.148 Broadcom SDK $
...
*   June 11 2015
* FUNCTION:
*   Get value of 'endpoint list' pointer (See endpoint_list_t)
*   from handle (equivalent to index in 'endp_lsts_array').
* INPUT:
*   int                      unit -
*     Identifier of the device to access.
*   ENDPOINT_LIST_PTR        endpoint_list -
*     Handle to the endpoint list.
*   endpoint_list_t          **endp_lst_ptr_ptr -
*     This procedure loads pointed memory by the pointer to the
*     indicated 'endpoint list' on internal workspace buffer.
* REMARKS:
*   This procedure is exceptional. It is added here so we can use
*   the buffer pointed by item of 'endp_lsts_array' as a work space
*   whose address is passed to various utilities.
* RETURNS:
  ...
  )
{
  ...
  *endp_lst_ptr_ptr = sw_state[unit]->dpp.bcm.oam->endp_lst->endp_lsts_array[endp_lst_index] ;
exit:
  BCMDNX_FUNC_RETURN;
}
/*********************************************************************
...
*   June 11 2015
* FUNCTION:
*   Get value of 'endpoint list member' pointer (See endpoint_list_member_t)
*   from handle (equivalent to index in 'endp_lst_ms_array').
* INPUT:
*   int                      unit -
*     Identifier of the device to access.
*   ENDPOINT_LIST_MEMBER_PTR endpoint_list_member -
*     Handle to the endpoint list.
*   endpoint_list_t          **endp_lst_m_ptr_ptr -
*     This procedure loads pointed memory by the pointer to the
*     indicated 'endpoint list member' on internal workspace buffer.
* REMARKS:
*   This procedure is exceptional. It is added here so we can use
*   the buffer pointed by item of 'endp_lst_ms_array' as a work space
*   whose address is passed to various utilities.
* RETURNS:
*     OK or ERROR indication.
*********************************************************************/
  ...
  )
{
  ...
  *endp_lst_m_ptr_ptr = sw_state[unit]->dpp.bcm.oam->endp_lst_m->endp_lst_ms_array[endp_lst_m_index] ;
exit:
  BCMDNX_FUNC_RETURN;
}
/*********************************************************************
