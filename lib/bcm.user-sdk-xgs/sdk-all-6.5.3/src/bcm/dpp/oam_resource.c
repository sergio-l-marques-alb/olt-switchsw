...
#define BCM_OAM_OAMP_DESTINATION_TRAP_CODE_GET(is_upmep, mep_type, destination_trap_code) \
  do { \
	if (is_upmep) { /* Can be only Eth. OAM */ \
		...
		else if (mep_type == SOC_PPC_OAM_MEP_TYPE_Y1731_O_MPLSTP) { \
...
int _bcm_dpp_bfd_mep_id_alloc(int unit, uint32 flags, uint32 *mep_index) {
    ...
    if (SOC_IS_QAX(unit)) {
        ...
        return bcm_dpp_am_oam_mep_id_alloc(unit, flags, mep_index);
    }
...
int _bcm_dpp_oam_bfd_mep_id_is_alloced(int unit, uint32 mep_id) {
    ...
    if (SOC_IS_QAX(unit)) {
        return bcm_dpp_am_oam_mep_id_is_alloced(unit, mep_id);
    }
    else {
...
int _bcm_dpp_oam_bfd_mep_id_dealloc(int unit, uint32 mep_id) {
    ...
    if (SOC_IS_QAX(unit)) {
        rv = bcm_dpp_am_oam_mep_id_is_alloced(unit, mep_id);
        if (rv == BCM_E_EXISTS) {
            return bcm_dpp_am_oam_mep_id_dealloc(unit, mep_id);
        }
        else {
            return BCM_E_PARAM;
        }
...
int _bcm_oam_default_profile_get(int unit,
   ...
   for (opcode = 1; opcode < SOC_PPC_OAM_ETHERNET_PDU_OPCODE_COUNT; opcode++) {
      ...
      if ((opcode == SOC_PPC_OAM_ETHERNET_PDU_OPCODE_LBM) || (opcode == SOC_PPC_OAM_ETHERNET_PDU_OPCODE_LTM)) {
...
int _bcm_dpp_oam_mep_profile_trap_data_free(int unit, SOC_PPC_OAM_MEP_PROFILE_DATA *profile_data, SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY *classifier_mep_entry, uint8 is_mep) {
    uint32 trap_codes[SOC_PPC_NOF_TRAP_CODES] = {0};
    uint32 trap_code;
    uint32 opcode;
    uint8  internal_opcode;
		...
		if (trap_codes[trap_code] && (trap_code != _BCM_PETRA_UD_DFLT_TRAP)) {
            ...
            if (is_internal_trap) {
                ...
                if (SOC_IS_ARADPLUS(unit)) {
                    ...
                    rv = MBCM_PP_DRIVER_CALL(unit, mbcm_pp_oam_oamp_rx_trap_codes_delete, (unit, classifier_mep_entry->mep_type, trap_code));
                    BCMDNX_IF_ERR_EXIT(rv); 
                }
...
int _bcm_dpp_oam_mep_profile_trap_data_alloc(int unit, SOC_PPC_OAM_MEP_PROFILE_DATA *profile_data) {
...
/* Aloocates a mirror_profile if not yet allocated */
STATIC int _bcm_dpp_oam_map_trap_code_to_mirror_profile(int unit, uint32 trap_code) {
    uint32 mirror_profile, snoop_strength, trap_code_dummy;
    uint32 soc_sand_rv = SOC_SAND_OK;
    int rv = BCM_E_NONE;
    ...
    /* Adding mapping of trap code to mirror profile */
...
STATIC int _bcm_dpp_oam_mip_adv_snoop_set(int unit, uint32 trap_code, bcm_rx_trap_config_t* trap_config) {
    ...
    rv = _bcm_dpp_oam_traps_ref_counter_increase(unit, SOC_PPC_TRAP_CODE_OAM_MIP_EGRESS_SNOOP_WITH_FTMH);
    BCMDNX_IF_ERR_EXIT(rv);
...
int _bcm_dpp_oam_bfd_trap_code_get_and_prepare_for_action(int unit, SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY* classifier_mep_entry,
    ...
    bcm_rx_trap_config_t trap_config;
    uint8 is_mip, is_upmep, is_snoop, is_equal, is_advanced_egress_snooping;
    ...
    if (destination != BCM_GPORT_INVALID) {
        ...
        *trap_code = BCM_GPORT_TRAP_GET_ID(destination);
        trap_code_sw = *trap_code;
        BCMDNX_IF_ERR_EXIT(_bcm_dpp_rx_get_converted_trap_hw_id(unit, trap_code_sw, &trap_code_hw));

        /* Get the trap */
        bcm_rx_trap_config_t_init(&trap_config);
        rv = bcm_petra_rx_trap_get(unit, trap_code_hw, &trap_config);
        BCMDNX_IF_ERR_EXIT(rv);

        is_snoop = trap_config.snoop_cmnd > 0;
        if (is_snoop) {
            if (is_mip && is_advanced_egress_snooping) {
                /* Arad+ MIP with advanced egress snooping */
                rv = _bcm_dpp_oam_mip_adv_snoop_set(unit, *trap_code, &trap_config);
                BCMDNX_IF_ERR_EXIT(rv);
                /* Adcanced egress snooping special case, using same trap_code for both directions */
        ...
        if (is_upmep) {
            /* Allocate mirror profile if required (already done for advanced egress snooping) */
            rv = _bcm_dpp_oam_map_trap_code_to_mirror_profile(unit, *trap_code);
            BCMDNX_IF_ERR_EXIT(rv);
        }
    }
    if (is_mip && !is_advanced_egress_snooping && destination2 != BCM_GPORT_INVALID) {
        ...
        *trap_code2 = BCM_GPORT_TRAP_GET_ID(destination2);
        ...
        rv = _bcm_dpp_oam_map_trap_code_to_mirror_profile(unit, *trap_code2);
        BCMDNX_IF_ERR_EXIT(rv);
    }
...
int _bcm_oam_trap_and_mirror_profile_set(int unit,
	...
	if (set_upmep) {
		...
		if (trap_code_upmep == SOC_PPC_TRAP_CODE_OAM_CPU_SNOOP
