    ...
    uint64 *value)
{
    ...
    if ( (SOC_IS_JERICHO(unit) && (type == snmpEtherStatsDropEvents)) || (_SOC_CONTROLLED_COUNTER_USE(unit, port)) ) {
    ...
    bcm_stat_counter_config_t *config)
{
    ...
    if (we_val > max_we_val) {
        /* 
         * Canot use the we_val calculated from config->source.pointer_range.start / counter_sets_per_counter_processor 
         * Try to use the maximum we_val. 
         */
        we_val = max_we_val;
        command_base_offset = config->source.pointer_range.start - (max_we_val * counter_sets_per_counter_processor);
    }

    if ((src_type == SOC_TMC_CNT_SRC_TYPE_EPNI || src_type == SOC_TMC_CNT_SRC_TYPE_ING_PP) && eg_type == SOC_TMC_CNT_MODE_EG_TYPE_OUTLIF) {
