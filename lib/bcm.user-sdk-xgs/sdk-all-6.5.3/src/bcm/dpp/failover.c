    ...
    int    unit)
{
    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        rv = _bcm_dpp_failover_state_hw_set(unit, _BCM_PROTECTION_POINTER_INVALID, DPP_FAILOVER_TYPE_INGRESS, 0);
        BCMDNX_IF_ERR_EXIT(rv);
    }
    ...
    bcm_failover_t *failover_id)
{
    ...
    if (flags & BCM_FAILOVER_WITH_ID) {
        if (*failover_id == _BCM_PROTECTION_POINTER_INVALID) {
    ...
    bcm_failover_t  failover_id)
{
    ...
    if (failover_id_val == _BCM_PROTECTION_POINTER_INVALID) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Invalid Failover ID")));
    }
