    ...
    bcm_lbg_t *lbg_id)
{
    ...
    if ((bcmLbDirectionRx == direction) || (bcmLbDirectionBoth == direction)) {
        _BCM_LB_SYSTEM_LOCK(unit);
        soc_rc = MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_lb_ing_modem_to_lbg_map_get,(unit, modem_id, &lbg_id_ing));
        _BCM_LB_SYSTEM_UNLOCK(unit);
        if (SOC_FAILURE(soc_rc)) {
            BCMDNX_ERR_EXIT_MSG(soc_rc, (_BSL_BCM_MSG("unit %d, error in mbcm_dpp_lb_ing_modem_to_lbg_map_get modem(%d), error 0x%x\n"), 
                unit, modem_id, soc_rc));
        }

        if (lbg_id_ing == SOC_TMC_LB_LBG_INVALID) {
            lbg_id_ing = BCM_LB_LBG_INVALID;
        }
        *lbg_id = lbg_id_ing;
    }
    ...
    if ((bcmLbDirectionTx == direction) || (bcmLbDirectionBoth == direction)) {
        _BCM_LB_SYSTEM_LOCK(unit);
        soc_rc = MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_lb_egr_modem_to_lbg_map_get,(unit, modem_id, &lbg_id_egr));
        _BCM_LB_SYSTEM_UNLOCK(unit);
        if (SOC_FAILURE(soc_rc)) {
            BCMDNX_ERR_EXIT_MSG(soc_rc, (_BSL_BCM_MSG("unit %d, error in mbcm_dpp_lb_egr_modem_to_lbg_map_get modem(%d), error 0x%x\n"), 
                unit, modem_id, soc_rc));
        }

        if (lbg_id_egr == SOC_TMC_LB_LBG_INVALID) {
            lbg_id_egr = BCM_LB_LBG_INVALID;
        }
        *lbg_id = lbg_id_egr;
    }
    ...
    int number_width)
{
    ...
    if ((bcmLbDirectionTx == direction) || (bcmLbDirectionBoth == direction)) {
        ...
        for (lbg_id_tmp = lbg_id_start; lbg_id_tmp < (lbg_id_end+1); lbg_id_tmp++) {
            _BCM_LB_SYSTEM_LOCK(unit);
            soc_rc = MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_lb_egr_seqeunce_num_width_set,(unit, lbg_id, number_width));
            _BCM_LB_SYSTEM_UNLOCK(unit);
            if (SOC_FAILURE(soc_rc)) {
                BCMDNX_ERR_EXIT_MSG(soc_rc, (_BSL_BCM_MSG("unit %d, error in mbcm_dpp_lb_egr_seqeunce_num_width_set modem(%d) lbg(%d) number_width(%d), error 0x%x\n"), 
                    unit, lbg_id, number_width, soc_rc));
            }
        }
    }
    ...
    int *number_width)
{
    ...
    if ((bcmLbDirectionTx == direction) || (bcmLbDirectionBoth == direction)) {
        ...
        for (lbg_id_tmp = lbg_id_start; lbg_id_tmp < (lbg_id_end+1); lbg_id_tmp++) {
            _BCM_LB_SYSTEM_LOCK(unit);
            soc_rc = MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_lb_egr_seqeunce_num_width_get,(unit, lbg_id, &num_width_egr_tmp));
            _BCM_LB_SYSTEM_UNLOCK(unit);
            if (SOC_FAILURE(soc_rc)) {
    ...
    int arg)
{
    ...
    if (is_global) {
        ...
        case bcmLbControlExpectedSequenceNumber:            
            ...
            else if (bcmLbControlExpectedSequenceNumber == lb_control) {
                reorder_config.expected_sequence_number = arg;
            }
    ...
    if (is_global) {
        ...
        case bcmLbControlExpectedSequenceNumber:            
            ...
            if (bcmLbControlTimeoutThreshold == lb_control) {
                ...
                *arg = soc_reorder_config.expected_sequence_number;
            }
    ...
    bcm_lb_rx_modem_map_config_t *map_config)
{
    ...
    if (BCM_GPORT_IS_SET(map_index->port)) {
        ...
        else if (BCM_GPORT_IS_LOCAL(map_index->port)) {
    ...
    if (mapped_port != user_port) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_CONFIG,
            (_BSL_BCM_MSG("Fail(%s) modem(%d) can't be connect to port(0x%x)\n"),
                             bcm_errmsg(BCM_E_PARAM), (map_config->modem_id), (map_index->port)));
    }
    ...
    bcm_lb_modem_shaper_config_t *shaper)
{
...
#ifdef BCM_LB_SUPPORT
    soc_error_t soc_rc = SOC_E_NONE;
    soc_lb_modem_shaper_config_t soc_shaper;
    ...
    /* get info from HW */
    _BCM_LB_SYSTEM_LOCK(unit);
    soc_rc = MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_lb_egr_modem_shaper_get,(unit, modem_id, &soc_shaper));
    _BCM_LB_SYSTEM_UNLOCK(unit);
    if (SOC_FAILURE(soc_rc)) {
    ...
    /* update info */
    soc_shaper.enable = shaper->enable;
    soc_shaper.rate = shaper->rate;
    soc_shaper.max_burst= shaper->max_burst;

    /* update HW */
    _BCM_LB_SYSTEM_LOCK(unit);
    soc_rc = MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_lb_egr_modem_shaper_set,(unit, modem_id, &soc_shaper));
    _BCM_LB_SYSTEM_UNLOCK(unit);
    if (SOC_FAILURE(soc_rc)) {
    ...
    bcm_lb_modem_shaper_config_t *shaper)
{
    ...
    /* get info from HW */
    sal_memset(&soc_shaper, 0, sizeof(soc_shaper));
    _BCM_LB_SYSTEM_LOCK(unit);
    soc_rc = MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_lb_egr_modem_shaper_get,(unit, modem_id, &soc_shaper));
    _BCM_LB_SYSTEM_UNLOCK(unit);
    if (SOC_FAILURE(soc_rc)) {
    ...
    /* update info */
    shaper->enable = soc_shaper.enable;
    shaper->rate = soc_shaper.rate;
    shaper->max_burst = soc_shaper.max_burst;
