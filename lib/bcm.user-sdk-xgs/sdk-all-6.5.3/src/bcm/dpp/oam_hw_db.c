...
#define _BCM_OAM_COUNTER_BASE_ID_TO_CLASSIFIER_COUNTER(endpoint_info_ptr, classifier_mep_ptr)\
   classifier_mep_ptr->counter = endpoint_info_ptr->lm_counter_base_id;
...
int _bcm_oam_mep_db_entry_dealloc(int unit, uint32 mep_index, SOC_PPC_OAM_OAMP_MEP_DB_ENTRY *mep_db_entry, SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY * mep_info) {
    ...
    if (SOC_IS_ARAD_B0_AND_ABOVE(unit) && PPC_API_OAM_STORE_LOCAL_PORT_IN_MEP_DB(mep_db_entry->mep_type) &&
         ((!(mep_info->flags & SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY_FLAG_UPMEP)) || (mep_info->flags & SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY_FLAG_SERVER && mep_info->port) || mep_db_entry->system_port==0xffff)) {
...
int _bcm_oam_classifier_mep_entry_struct_get(int unit, bcm_oam_endpoint_info_t *endpoint_info, SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY * classifier_mep_entry) {
    ...
    if (classifier_mep_entry->mep_type!= SOC_PPC_OAM_MEP_TYPE_Y1731_O_MPLSTP || soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "oam_over_mpls_ignore_mdl", 0)==0) {
