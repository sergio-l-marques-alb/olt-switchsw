...
int bcm_petra_port_add(int unit, bcm_port_t port, uint32 flags, bcm_port_interface_info_t *interface_info, bcm_port_mapping_info_t *mapping_info)  
{
    ...
    if (interface_info->interface == SOC_PORT_IF_LBG) {
        ...
        if (mapping_info->base_q_pair > SOC_TMC_LB_BASE_Q_PAIR_MAX) {
        ...
        case bcmPortControlEgressFilterDisable:
          {
              ...
              BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_ndx) {
                  ...
                  /* Enable all filters */
                  eg_port_info.filter_mask |= (SOC_PPC_EG_FILTER_PORT_ENABLE_UC_UNKNOW_DA | SOC_PPC_EG_FILTER_PORT_ENABLE_MC_UNKNOW_DA | SOC_PPC_EG_FILTER_PORT_ENABLE_BC_UNKNOW_DA);
    ...
    BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_i) {
        ...
        if (BCM_GPORT_IS_TRAP(egr_port)) {
            ...
            if(enable && first_appear) {
                soc_sand_rv = soc_ppd_trap_frwrd_profile_info_set(unit,trap_indx+SOC_PPC_TRAP_CODE_USER_DEFINED_0,&(ud_trap_info.trap));
                BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
            }
   ...
   uint8 is_out_lif_drop)
{
   ...
   soc_sand_rv = soc_ppd_eg_encap_next_outlif_update(unit, out_lif_index, SOC_PPC_EG_ENCAP_ENTRY_UPDATE_DROP, is_out_lif_drop);
   BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    ...
    bcm_port_match_info_t *match)
{
    ...
    rv = arad_pp_dbal_entry_get(unit, SOC_DPP_DBAL_SW_TABLE_ID_EXTENDER_PE_SEM_A, qual_vals, &payload, &priority, &hit_bit, &found);
    BCM_SAND_IF_ERR_EXIT(rv);

    if (found) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_EXISTS, (_BSL_BCM_MSG("Trying to add existing match. Port: %d, ECID: %d, VID: %d "),
                                           match->port, match->extended_port_vid, match->match_vlan));
    }

    /* Add match to SEM */
    rv = arad_pp_dbal_entry_add(unit, SOC_DPP_DBAL_SW_TABLE_ID_EXTENDER_PE_SEM_A, qual_vals, 0, &lif_id, &success);
    BCM_SAND_IF_ERR_EXIT(rv);
    SOC_SAND_IF_FAIL_RETURN(success);
    ...
    bcm_port_match_info_t *match)
{
    ...
    if (usage == _bcmDppLifTypeVlan) {
        ...
        if (match->match != BCM_PORT_MATCH_PORT_EXTENDED_PORT_VID_VLAN){
    ...
    bcm_port_match_info_t *match)
{
    ...
    rv = arad_pp_dbal_entry_get(unit, SOC_DPP_DBAL_SW_TABLE_ID_EXTENDER_PE_SEM_A, qual_vals, &lif, &priority, &hit_bit, &found);
    BCM_SAND_IF_ERR_EXIT(rv);

    if (!found) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_NOT_FOUND, (_BSL_BCM_MSG("Trying to remove non existing match. port: %d, ECID: %d, VID: %d "),
                                               match->port, match->extended_port_vid, match->match_vlan));
    }
    ...
    rv = arad_pp_dbal_entry_delete(unit, SOC_DPP_DBAL_SW_TABLE_ID_EXTENDER_PE_SEM_A, qual_vals, &success);
    BCM_SAND_IF_ERR_EXIT(rv);
    SOC_SAND_IF_FAIL_RETURN(success);
