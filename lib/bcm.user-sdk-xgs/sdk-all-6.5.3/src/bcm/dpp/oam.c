     ...
     /*    case bcmOAMEventEndpointCCMTimeoutEarly: soc_ppd_event_type = SOC_PPD_OAM_EVENT_ALMOST_LOC_SET; break; Unsupported due to HW bug */
         ...
         case bcmOAMEventEndpointInterfaceLLDown:
            *soc_ppd_event_type = SOC_PPC_OAM_EVENT_RMEP_STATE_CHANGE;
            break;
         default:
            BCMDNX_ERR_EXIT_MSG(BCM_E_INTERNAL, (_BSL_BCM_MSG("Error: Illegal PPD event \n")));
   }
   BCM_EXIT;
exit:
   BCMDNX_FUNC_RETURN;
...
*/
int set_classifier_action( int unit, int mep_id, bcm_oam_endpoint_action_t *action, int is_lm, uint8 is_1dm_or_piggy_back, SOC_PPC_OAM_MEP_TYPE mep_type ){
    ...
    if (!is_lm) {
	    ...
	    int meter_counter_opcode = is_piggy_backed? SOC_PPC_OAM_ETHERNET_PDU_OPCODE_CCM : SOC_PPC_OAM_ETHERNET_PDU_OPCODE_LMM;
        if (is_1dm_or_piggy_back) {
...
int _bcm_dpp_oam_core_from_lif_get(int unit, uint32 global_lif, uint32 *core){
    ...
    uint32 lif2gport_flags = (_BCM_DPP_LIF_TO_GPORT_GLOBAL_LIF |
        _BCM_DPP_LIF_TO_GPORT_INGRESS | _BCM_DPP_LIF_TO_GPORT_USE_SW_STATE);
...
int bcm_petra_oam_init(int unit) {
    ...
    bcm_gport_t oamp_port[SOC_DPP_DEFS_MAX(NOF_CORES)], cpu_port, recycle_port, recycle_port_iter;
    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        for (trap_code_with_meta = SOC_PPC_TRAP_CODE_OAM_FTMH_MIRROR_TRAP_CODES_FIRST;
            ...
            BCMDNX_IF_ERR_EXIT(_bcm_dpp_rx_get_converted_trap_hw_id(unit, trap_code_with_meta, &trap_code));
...
int bcm_petra_oam_endpoint_action_set_internal(
    ...
    if (BCM_OAM_OPCODE_GET(*action_lcl, SOC_PPC_OAM_ETHERNET_PDU_OPCODE_CCM)) {
        if (is_accelerated) {
...
int bcm_petra_oam_endpoint_action_set(
    ...
    rv = bcm_petra_oam_endpoint_action_set_internal(unit,id,action,0,0);
    BCMDNX_IF_ERR_EXIT(rv);
