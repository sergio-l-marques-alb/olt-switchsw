...
bcm_petra_init(int unit)
{
    ...
    BCM_INIT_MODULE(_bcm_sw_db_init(unit), rv, "db init");
    ...
    DISPLAY_MEM;
    DISPLAY_SW_STATE_MEM;
    DISPLAY_MEM_PRINTF(("%s(): unit %d: Going to call %s\r\n",__FUNCTION__,unit,"bcm_dpp_counter_init"));
    rv = bcm_dpp_counter_init(unit);
    BCMDNX_IF_ERR_EXIT(rv);

    if (pp_enable) {
        DISPLAY_SW_STATE_MEM ;
        DISPLAY_MEM_PRINTF(("%s(): unit %d: Going to call %s\r\n",__FUNCTION__,unit,"_bcm_dpp_gport_mgmt_init"));
    ...
    else if (soc_property_get(unit, spn_ITMH_PROGRAMMABLE_MODE_ENABLE, FALSE)) {
        /*TM mode*/
        INIT_CALL_CHK(field);
    }
...
_bcm_petra_modules_deinit(int unit, int silent)
{
    ...
    if (pp_enable) {
        ...
        SOC_PETRA_DETACH(unit, _bcm_petra_oam_detach, rv); /* Detach is an internal function, not currently supported through mkdispatch */
        SOC_PETRA_DETACH(unit, _bcm_petra_bfd_detach, rv); /* Detach is an internal function, not currently supported through mkdispatch */
