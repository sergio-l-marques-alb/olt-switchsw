...
_bcm_dpp_am_local_outlif_init(int unit)
{
    ...
    if (!SOC_WARM_BOOT(unit)) {
         ...
         */
        if (SOC_IS_JERICHO_PLUS(unit) && rif_max_id == 0) {
            /*
             * In the weird case the user wants 0 rifs, we only allocate outlif 0 because it's the null 
             * entry. We do it directly since we don't want to set the bank's phase. 
             */
            elem = 0;
            bank_id = 0;
            outlif_tag = 0;
