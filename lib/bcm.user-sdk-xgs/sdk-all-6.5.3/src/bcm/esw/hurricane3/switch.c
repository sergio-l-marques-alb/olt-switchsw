    ...
    int *table_size)
{
    if (match_service == bcmSwitchMatchServiceMiml) {
        ...
        *table_size = _BCM_HR3_SWITCH_MATCH_TABLE_SIZE_CUSTOM_HDR;
    } else {
        *match_type = 0;
        *match_list = NULL;
        *table_size = 0;
...
bcm_hr3_switch_match_init(int unit)
{
    ...
    match_info->custom_hdr_match_list = 
        sal_alloc(sizeof(uint32) * _BCM_HR3_SWITCH_MATCH_TABLE_SIZE_CUSTOM_HDR, 
    ...
    sal_memset(match_info->custom_hdr_match_list, 0, 
                sizeof(uint32) * _BCM_HR3_SWITCH_MATCH_TABLE_SIZE_CUSTOM_HDR);
