...
_bcm_esw_switch_control_gport_resolve(int unit, bcm_gport_t gport, bcm_port_t *port)
{
    ...
    if (SOC_IS_METROLITE(unit)) {
        min_subport =  _BCM_ML_SUBPORT_PP_PORT_INDEX_MIN ;
        max_subport = _BCM_ML_SUBPORT_PP_PORT_INDEX_MAX ;
    } else 
#endif
...
_bcm_switch_control_sync(int unit, int arg)
{
...
#endif /* BCM_SHADOW_SUPPORT */
    {
         ...
         * Restricting this to Raven as other chips have L3 dependent features
         * like MPLS that will fail if L3 is disabled.
         */
        if (!(SOC_IS_RAVEN(unit)) ||
                (SOC_IS_RAVEN(unit) && soc_feature(unit, soc_feature_l3) &&
                          ...
                          bcm_switch_control_t type, int arg)
{
    ...
    if (SOC_IS_TD_TT(unit) || SOC_IS_TRIUMPH3(unit)) {
        ...
        sub_field_width[1] = 16;  /* HASH_B0 (16bit)   */
        sub_field_width[2] = 4;   /* LBN (4bit)        */
        sub_field_width[3] = 64;  /* MH.DPORT / HASH_A0 B0 A1 B1(64bit) */
                            ...
                            int *max_offset, int *stride_offset)
{
    ...
    int index, offset;
    uint32 fval, sub_sel;
    int sub_sel_f = SUB_SEL_ECMPf;
    int  ecmp_offset_f = OFFSET_ECMPf;

#if defined(BCM_RIOT_SUPPORT) || defined(BCM_MULTI_LEVEL_ECMP_SUPPORT)
    /* Set the overlay fields if this overlay flow */
    if (BCM_MACROFLOW_IS_OVERLAY(type)) {
        if (!(soc_feature(unit, soc_feature_riot) ||
              soc_feature(unit, soc_feature_multi_level_ecmp))) {
            return BCM_E_UNAVAIL;
        }
        sub_sel_f     = SUB_SEL_ECMP_LEVEL1f;
        ecmp_offset_f = OFFSET_ECMP_LEVEL1f;
    }
#endif
...
_bcm_td_macroflow_offset_set(int unit, bcm_switch_control_t type, int arg)
{
    ...
    for (index = 0; index <= soc_mem_index_max(unit, RTAG7_FLOW_BASED_HASHm);
        ...
        soc_mem_field32_set(unit, RTAG7_FLOW_BASED_HASHm, &entry,
                            sub_sel_f, sub_sel);
        soc_mem_field32_set(unit, RTAG7_FLOW_BASED_HASHm, &entry,
                            ecmp_offset_f, offset - sub_field_base[sub_sel]);
...
#ifdef BCM_TRIDENT2_SUPPORT
    if (BCM_SUCCESS(rv) && (SOC_IS_TRIDENT2X(unit) || SOC_IS_TOMAHAWK(unit))) {
...
defined(BCM_TRIDENT_SUPPORT)
    ...
    { bcmSwitchIngParseL2TunnelTermDipSipSelect, TD | TR3 | HX4 | TD2 | TD2P | TH | KT | KT2,
        ...
        NULL, 0},
    { bcmSwitchIngParseL3L4IPv4, TD | TR3 | HX4 | TD2 | TD2P | TH | KT | KT2,
        ING_L2_TUNNEL_PARSE_CONTROLr, IGMP_ENABLEf ,
        NULL, 0},
    { bcmSwitchIngParseL3L4IPv6, TD | TR3 | HX4 | TD2 | TD2P | TH | KT | KT2,
...
#if defined(BCM_TRIDENT_SUPPORT)
    { bcmSwitchHashField0Ip6FlowLabel, TD | TR3 | HX4 | TD2 | TD2P | TH,
       RTAG7_HASH_CONTROL_2r, ENABLE_FLOW_LABEL_IPV6_Af,
        NULL, 0},
    { bcmSwitchHashField1Ip6FlowLabel, TD | TR3 | HX4 | TD2 | TD2P | TH,
    ...
    || defined(BCM_KATANA2_SUPPORT) || defined(BCM_TOMAHAWK_SUPPORT)
        ...
        case bcmSwitchDropSobmhOnLinkDown:
            if (SOC_IS_SABER2(unit)) {
                ...
                BCM_IF_ERROR_RETURN(
                        soc_mem_field32_modify(unit, EPC_LINK_BMAPm, 0,
                            ENABLE_SOBMH_BLOCKINGf, (arg ? 1 : 0)));
 ...
 return BCM_E_NONE;
   ...
   case bcmSwitchEEEPacketLatencyProfile0:
    ...
    case bcmSwitchL3Max128BV6Entries:
        ...
        if (soc_feature(unit, soc_feature_l3) &&
            soc_feature(unit, soc_feature_l3_shared_defip_table)) {
            if (SOC_IS_TRIDENT2X(unit) || SOC_IS_TRIUMPH3(unit) || SOC_IS_TOMAHAWK(unit)) {
#ifdef BCM_TRX_SUPPORT
                           ...
                           int *arg)
{
    ...
    defined(BCM_TRIDENT_SUPPORT)
        ...
        case bcmSwitchMacroFlowHashMaxOffset:
            return _bcm_td_macroflow_param_get(unit, type, NULL, NULL, arg, NULL);
            break;
#if defined(BCM_RIOT_SUPPORT) || defined(BCM_MULTI_LEVEL_ECMP_SUPPORT)
        case bcmSwitchMacroFlowHashOverlayStrideOffset:
            if (soc_feature(unit, soc_feature_riot) ||
        ...
        case bcmSwitchMacroFlowHashStrideOffset:
            ...
            return _bcm_td_macroflow_param_get(unit, type, NULL, NULL, NULL, arg);
            break;
#endif /* BCM_TRIDENT_SUPPORT */
        ...
        if (SOC_IS_TD2_TT2(u)){
            ...
            switch(object) {
                ...
                case bcmSwitchObjectL3RouteV6Routes128bMax:
                    if (soc_property_get(u, spn_IPV6_LPM_128B_ENABLE, 1)) {
                        rv = (* soc_alpm_capacity_get)(u, 
                                L3_DEFIP_MODE_128, entries, NULL);
                    } else {
                        *entries = 0;
                    }
                    return rv;
                ...
                case bcmSwitchObjectL3RouteV6Routes128bMinGuaranteed:
                    if (soc_property_get(u, spn_IPV6_LPM_128B_ENABLE, 1)) {
                        rv = (* soc_alpm_capacity_get)(u,
                                L3_DEFIP_MODE_128, NULL, entries);
                    } else {
                        *entries = 0;
                    }
                    return rv;
...
#ifdef BCM_TOMAHAWK_SUPPORT
        if (SOC_IS_TOMAHAWK(unit)) {
            rv =  soc_th_hash_offset_set(unit, mem, bank_num, offset,
                                         use_lsb);
        } else
#endif /* BCM_TOMAHAWK_SUPPORT */
        {
...
{
    return (_bcm_esw_pkt_trace_info_get(unit, options, 
                                        port, len, data,
                                        pkt_trace_info));
}
...
#ifdef BCM_HURRICANE3_SUPPORT
int 
_bcm_esw_switch_capwap_payload_outer_tpid_add(int unit, 
    uint16 tpid_value)
{
    int rv = BCM_E_NONE;
...
int 
_bcm_esw_switch_capwap_payload_inner_tpid_add(int unit, 
    uint16 tpid_value)
{
    int rv = BCM_E_NONE;
    int tpid_enable;
...
int 
_bcm_esw_switch_capwap_payload_outer_tpid_delete(int unit, 
    uint16 tpid_value)
{
    int rv = BCM_E_NONE;
...
int 
_bcm_esw_switch_capwap_payload_inner_tpid_delete(int unit, 
    uint16 tpid_value)
{
    int rv = BCM_E_NONE;
    int tpid_enable;
...
int 
_bcm_esw_switch_capwap_payload_outer_tpid_delete_all(int unit)
{
    int rv = BCM_E_NONE;
...
int 
_bcm_esw_switch_capwap_payload_inner_tpid_delete_all(int unit)
{
    int rv = BCM_E_NONE;
    uint32  reg_val;
...
int 
_bcm_esw_switch_capwap_payload_outer_tpid_get(int unit, 
    uint16 *tpid_value)
{
    int tpid_enable, tpid_index;
    uint32  reg_val;
...
int 
_bcm_esw_switch_capwap_payload_inner_tpid_get(int unit, 
    uint16 *tpid_value)
{
    int rv = BCM_E_NONE;
    int tpid_enable;
    ...
    bcm_switch_tpid_info_t *tpid_info)
{
    ...
    if (soc_feature(unit, soc_feature_ing_capwap_parser)) {
        switch (tpid_info->tpid_type) {
            case bcmTpidTypeCapwapPayloadOuter:
                rv = _bcm_esw_switch_capwap_payload_outer_tpid_add(unit,
                                tpid_info->tpid_value);
                break;
            case bcmTpidTypeCapwapPayloadInner:
                rv = _bcm_esw_switch_capwap_payload_inner_tpid_add(unit,
                                tpid_info->tpid_value);
                break;
            default:
                return (BCM_E_UNAVAIL);
...
bcm_esw_switch_tpid_delete(int unit, bcm_switch_tpid_info_t *tpid_info)
{
    ...
    if (soc_feature(unit, soc_feature_ing_capwap_parser)) {
        switch (tpid_info->tpid_type) {
            case bcmTpidTypeCapwapPayloadOuter:
                rv = _bcm_esw_switch_capwap_payload_outer_tpid_delete(unit,
                                tpid_info->tpid_value);
                break;
            case bcmTpidTypeCapwapPayloadInner:
                rv = _bcm_esw_switch_capwap_payload_inner_tpid_delete(unit,
                                tpid_info->tpid_value);
                break;
            default:
                return (BCM_E_UNAVAIL);
...
#ifdef BCM_HURRICANE3_SUPPORT
    if (soc_feature(unit, soc_feature_ing_capwap_parser)) {
        rv = _bcm_esw_switch_capwap_payload_outer_tpid_delete_all(unit);
        if (BCM_FAILURE(rv)) {
            return rv;
        }
        rv = _bcm_esw_switch_capwap_payload_outer_tpid_delete_all(unit);
                    ...
                    bcm_switch_tpid_info_t *tpid_info)
{
    ...
    if (soc_feature(unit, soc_feature_ing_capwap_parser)) {
        switch (tpid_info->tpid_type) {
            case bcmTpidTypeCapwapPayloadOuter:
                rv = _bcm_esw_switch_capwap_payload_outer_tpid_get(unit,
                                &tpid_info->tpid_value);
                break;
            case bcmTpidTypeCapwapPayloadInner:
                rv = _bcm_esw_switch_capwap_payload_inner_tpid_get(unit,
                                &tpid_info->tpid_value);
                break;
            default:
                return (BCM_E_UNAVAIL);
    ...
    bcm_switch_tpid_info_t *tpid_info_array, int *count)
{
    ...
    if (soc_feature(unit, soc_feature_ing_capwap_parser)) {
        if (size == 0) {
            if (SOC_IS_HURRICANE3(unit)) {
                /* 
                 * 4 capwap payload outer TPIDs + 1 inner TPID
                 */
                *count = 5;
                return BCM_E_NONE;
            }
    ...
    *count = 0;
    ...
    if (tpid_enable) {
        ...
        tpid = soc_reg_field_get(unit, VLAN_CTRLr, reg_val, 
            INNER_TPIDf);
