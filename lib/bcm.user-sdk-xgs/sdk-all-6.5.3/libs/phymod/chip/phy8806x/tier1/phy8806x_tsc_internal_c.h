/***********************************************************************************
***********************************************************************************
*  File Name     :  phy8806x_tsc_internal.c                                         *
*  Created On    :  13/02/2014                                                    *
*  Created By    :  Justin Gaither                                                *
*  Description   :  APIs for Serdes IPs                                           *
*  Revision      :  $Id: phy8806x_tsc_internal.c 1062 2015-06-02 22:12:41Z kirand $ *
*                                                                                 *
...
/** @file phy8806x_tsc_internal.c
 * Implementation of API functions
 */

#include <phymod/phymod.h>
#include "phy8806x_tsc_internal.h"
#ifndef UINT16_MAX
...
SDK_STATIC char* _e2s_err_code (const phymod_access_t *pa, err_code_t err_code){
	switch (err_code) {
...
SDK_STATIC err_code_t _print_err_msg (const phymod_access_t *pa, uint16_t err_code){
	if (err_code != 0)
		USR_PRINTF(("ERROR: SerDes err_code = %s\n", _e2s_err_code (pa, err_code)));
	return err_code;
}
...
#if 0
err_code_t set_lms_hoffset (const phymod_access_t *pa, char delta){
	if (delta != 0) {
		EFUN(wr_rx_pi_slicers_en(      0x30 ));
		EFUN(wr_rx_pi_phase_step_cnt(         0 ));
		EFUN(wr_rx_pi_phase_step_dir((delta > 0)));
		{   uint8_t loop = (uint8_t)(_abs(delta));
		    while (loop-- > 0)
			    EFUN(wr_rx_pi_manual_strobe(1)); }
		/* Undo setup */
		EFUN(wr_rx_pi_slicers_en(0x0));
		EFUN(wr_rx_pi_phase_step_cnt(  0));
		EFUN(wr_rx_pi_phase_step_dir(  0));
	}
	return ERR_CODE_NONE;
}
#endif

err_code_t phy8806x_tsc_log_full_pmd_state (const phymod_access_t *pa, struct phy8806x_tsc_detailed_lane_status_st *lane_st){
	uint16_t reg_data;
	int8_t tmp;
	...
	if (lane_st->pmd_lock == 1) {
		...
		EFUN(phy8806x_tsc_pmd_uc_control (pa, CMD_UC_CTRL_STOP_IMMEDIATE, 200));
	...
	EFUN(phy8806x_tsc_read_tx_afe (pa, TX_AFE_AMP, &tmp)); lane_st->amp_ctrl = (uint8_t)tmp;
	EFUN(phy8806x_tsc_read_tx_afe (pa, TX_AFE_PRE, &tmp)); lane_st->pre_tap = (uint8_t)tmp;
	EFUN(phy8806x_tsc_read_tx_afe (pa, TX_AFE_MAIN, &tmp)); lane_st->main_tap = (uint8_t)tmp;
	EFUN(phy8806x_tsc_read_tx_afe (pa, TX_AFE_POST1, &tmp)); lane_st->post1_tap = (uint8_t)tmp;
	EFUN(phy8806x_tsc_read_tx_afe (pa, TX_AFE_POST2, &lane_st->post2_tap));
	EFUN(phy8806x_tsc_read_tx_afe (pa, TX_AFE_POST3, &lane_st->post3_tap));
	...
	EFUN(phy8806x_tsc_pmd_rdt_reg (pa, DSC_E_RX_PI_CNT_BIN_D, &reg_data));
	lane_st->ddq_hoffset = (uint8_t)dist_ccw(((reg_data >> 8) & 0xFF), (reg_data & 0xFF));
	EFUN(phy8806x_tsc_pmd_rdt_reg (pa, DSC_E_RX_PI_CNT_BIN_P, &reg_data));
	lane_st->ppq_hoffset = (uint8_t)dist_ccw(((reg_data >> 8) & 0xFF), (reg_data & 0xFF));
	EFUN(phy8806x_tsc_pmd_rdt_reg (pa, DSC_E_RX_PI_CNT_BIN_L, &reg_data));
	lane_st->llq_hoffset = (uint8_t)dist_ccw(((reg_data >> 8) & 0xFF), (reg_data & 0xFF));
	EFUN(phy8806x_tsc_pmd_rdt_reg (pa, DSC_E_RX_PI_CNT_BIN_PD, &reg_data));
	lane_st->dp_hoffset = (uint8_t)dist_cw(((reg_data >> 8) & 0xFF), (reg_data & 0xFF));
	EFUN(phy8806x_tsc_pmd_rdt_reg (pa, DSC_E_RX_PI_CNT_BIN_LD, &reg_data));
	...
	lane_st->prbs_chk_order = (reg_data == 0) ? 7 :
				  ...
				  (reg_data == 5) ? 31 :
				  (reg_data == 6) ? 58 : 0;
	EFUN(phy8806x_tsc_prbs_chk_lock_state (pa, &lane_st->prbs_chk_lock));
	EFUN(phy8806x_tsc_prbs_err_count_ll (pa, &lane_st->prbs_chk_errcnt));
...
err_code_t phy8806x_tsc_disp_full_pmd_state (const phymod_access_t *pa, struct phy8806x_tsc_detailed_lane_status_st *lane_st, uint8_t num_lanes){
	uint8_t i;

	EFUN_PRINTF(( "\n\n"));
	EFUN_PRINTF(( "------------------------------------------------------------------------\n"));
	EFUN_PRINTF(( "Phy8806x PMD State\n"));
...
static err_code_t _ull_tx_mode_setup (const phymod_access_t *pa, uint8_t enable){
	EFUN(wr_ams_tx_pdet_mode(enable));                      /* Randomize bit[0] */
	EFUN(wr_ams_tx_pdet_ll_clk(enable));                    /* New reister bit to enable ull PD mode */

	EFUN(wr_tx_pi_hs_fifo_phserr_invert(0x0));
	EFUN(wr_tx_pi_ext_phase_bwsel_integ(0x7));              /* 0 to 7 */
	EFUN(wr_tx_pi_ext_ctrl_en(enable));                     /* Turn on jitter filter's phase detector input path  */

	EFUN(phy8806x_tsc_delay_us(1024));                       /* Wait for phase detector path to settle */

	EFUN(wr_tx_pi_second_order_loop_en(enable));            /* Turn on 2nd order loop in jitter filter */

	EFUN(wr_ams_tx_fifo_depth(0x6));
	...
	EFUN(phy8806x_tsc_poll_st_afe_tx_fifo_resetb_equals_1 (pa, 1));       /* Check if FIFO reset is reported to be released within 1ms time interval */

	EFUN(wr_ams_tx_sel_ll(enable));                                 /* Switch the tx data mux to ULL traffic and check for data integrity */
	return ERR_CODE_NONE;
}
...
static err_code_t _ull_rx_mode_setup (const phymod_access_t *pa, uint8_t enable){
	EFUN(wr_ams_rx_data12t_ll_en(enable));               /* Enable RX ULL datapath */
	return ERR_CODE_NONE;
}
...
static uint8_t _phy8806x_tsc_rdb_uc_var (const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr){
	uint8_t rddata;

	if (!err_code_p)
		return 0;

	EPSTM(rddata = phy8806x_tsc_rdb_uc_ram (pa, err_code_p, addr)); /* Use Micro register interface for reading RAM */

	return rddata;
}

/* Micro Global RAM Word Read */
static uint16_t _phy8806x_tsc_rdw_uc_var (const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr){
	uint16_t rddata;
...
#endif
	EPSTM(rddata = phy8806x_tsc_rdw_uc_ram (pa, err_code_p, (addr))); /* Use Micro register interface for reading RAM */

	return rddata;
}

/* Micro Global RAM Byte Write  */
static err_code_t _phy8806x_tsc_wrb_uc_var (const phymod_access_t *pa, uint16_t addr, uint8_t wr_val){

	return phy8806x_tsc_wrb_uc_ram (pa, addr, wr_val);                              /* Use Micro register interface for writing RAM */
}
/* Micro Global RAM Word Write  */
static err_code_t _phy8806x_tsc_wrw_uc_var (const phymod_access_t *pa, uint16_t addr, uint16_t wr_val){
	if (addr % 2 != 0)                                                              /* Validate even address */
		return _error(ERR_CODE_INVALID_RAM_ADDR);
	return phy8806x_tsc_wrw_uc_ram (pa, addr, wr_val);                                    /* Use Micro register interface for writing RAM */
}
...
static err_code_t _set_tx_post1 (const phymod_access_t *pa, uint8_t val){
	...
	EFUN(wr_cl93n72_txfir_post(val));
	return ERR_CODE_NONE;
}
...
static err_code_t _set_tx_main (const phymod_access_t *pa, uint8_t val){
	...
	EFUN(wr_cl93n72_txfir_main(val));
	return ERR_CODE_NONE;
}
...
static err_code_t _phy8806x_tsc_core_clkgate (const phymod_access_t *pa, uint8_t enable){
...
static err_code_t _phy8806x_tsc_lane_clkgate (const phymod_access_t *pa, uint8_t enable){
...
SDK_STATIC err_code_t _phy8806x_tsc_get_osr_mode (const phymod_access_t *pa, phy8806x_tsc_osr_mode_st *imode){
	phy8806x_tsc_osr_mode_st mode;

	ENULL_MEMSET(&mode, 0, sizeof(phy8806x_tsc_osr_mode_st));
...
SDK_STATIC err_code_t _phy8806x_tsc_read_lane_state (const phymod_access_t *pa, phy8806x_tsc_lane_state_st *istate){

	phy8806x_tsc_lane_state_st state;
	uint8_t ladder_range = 0;
	uint16_t reg_data;

	ENULL_MEMSET(&state, 0, sizeof(phy8806x_tsc_lane_state_st));

	if (!istate)
		return _error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT);

	EFUN(_phy8806x_tsc_pmd_lock_status (pa, &state.rx_lock, &state.rx_lock_chg));
	...
	{  phy8806x_tsc_osr_mode_st osr_mode;
	   ENULL_MEMSET(&osr_mode, 0, sizeof(phy8806x_tsc_osr_mode_st));
	   EFUN(_phy8806x_tsc_get_osr_mode (pa, &osr_mode));
	   state.osr_mode = osr_mode; }
	ESTM(state.ucv_config = rdv_config_word());
	ESTM(state.reset_state = rd_lane_dp_reset_state());
	EFUN(_phy8806x_tsc_sigdet_status (pa, &state.sig_det, &state.sig_det_chg));
	ESTM(state.rx_ppm = rd_cdr_integ_reg() / 84);
	EFUN(phy8806x_tsc_get_clk90_offset (pa, &state.clk90));
	EFUN(phy8806x_tsc_pmd_rdt_reg (pa, DSC_E_RX_PI_CNT_BIN_LD, &reg_data));
...
static err_code_t _phy8806x_tsc_display_lane_state_no_newline (const phymod_access_t *pa){
	uint16_t lane_idx;
	phy8806x_tsc_lane_state_st state;
	...
	EFUN(_phy8806x_tsc_read_lane_state (pa, &state));

	lane_idx = phy8806x_tsc_get_lane (pa );
...
SDK_STATIC err_code_t _phy8806x_tsc_read_core_state (const phymod_access_t *pa, phy8806x_tsc_core_state_st *istate){
	phy8806x_tsc_core_state_st state;
	struct phy8806x_tsc_uc_core_config_st core_cfg;

	ENULL_MEMSET(&state, 0, sizeof(phy8806x_tsc_core_state_st));
	ENULL_MEMSET(&core_cfg, 0, sizeof(struct phy8806x_tsc_uc_core_config_st));

	if (!istate)
		return _error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT);

	EFUN(phy8806x_tsc_get_uc_core_config (pa, &core_cfg));
	...
	{  int16_t die_temp = 0;
	   EFUN(phy8806x_tsc_read_die_temperature (pa, &die_temp));
	...
	EFUN(_phy8806x_tsc_pll_lock_status (pa, &state.pll_lock, &state.pll_lock_chg));
...
static err_code_t _phy8806x_tsc_display_core_state_no_newline (const phymod_access_t *pa){
	phy8806x_tsc_core_state_st state;
	phy8806x_tsc_core_state_st state_pll1;

	ENULL_MEMSET(&state_pll1, 0, sizeof(state_pll1));
	ENULL_MEMSET(&state, 0, sizeof(state     ));
	{
		uint8_t pll_orig;
		ESTM(pll_orig = phy8806x_tsc_get_pll(pa));
		EFUN(phy8806x_tsc_set_pll(pa, 0));
		EFUN(_phy8806x_tsc_read_core_state (pa, &state));
		EFUN(phy8806x_tsc_set_pll(pa, 1));
		EFUN(_phy8806x_tsc_read_core_state (pa, &state_pll1));
		EFUN(phy8806x_tsc_set_pll(pa, pll_orig));
	}
	EFUN_PRINTF(("%02d  ",  phy8806x_tsc_get_core()));
	...
	EFUN_PRINTF((" %2d.%2d,%2d.%2dGHz ", (state.vco_rate_mhz / 1000),
	...
	EFUN_PRINTF((" (%02d,%02d)",  state.pll_div,
		     state_pll1.pll_div));
	EFUN(_phy8806x_tsc_display_pll_to_divider (pa,   state.pll_div));
	EFUN_PRINTF((","));
	EFUN(_phy8806x_tsc_display_pll_to_divider (pa,   state_pll1.pll_div));
...
SDK_STATIC err_code_t _phy8806x_tsc_pll_lock_status (const phymod_access_t *pa, uint8_t *pll_lock, uint8_t *pll_lock_chg){

	uint16_t rddata;

	EFUN(phy8806x_tsc_pmd_rdt_reg (pa, PLL_STATUS_ADDR, &rddata));
...
SDK_STATIC err_code_t _phy8806x_tsc_pmd_lock_status (const phymod_access_t *pa, uint8_t *pmd_lock, uint8_t *pmd_lock_chg){

	uint16_t rddata;

	EFUN(phy8806x_tsc_pmd_rdt_reg (pa, PMD_LOCK_STATUS_ADDR, &rddata));
...
SDK_STATIC err_code_t _phy8806x_tsc_sigdet_status (const phymod_access_t *pa, uint8_t *sig_det, uint8_t *sig_det_chg){

	uint16_t rddata;

	EFUN(phy8806x_tsc_pmd_rdt_reg (pa, SIGDET_STATUS_ADDR, &rddata));
...
static err_code_t _update_uc_lane_config_st (const phymod_access_t *pa, struct  phy8806x_tsc_uc_lane_config_st *st){
	uint16_t in = st->word;
...
static err_code_t _update_usr_ctrl_disable_functions_st (const phymod_access_t *pa, struct phy8806x_tsc_usr_ctrl_disable_functions_st *st){
	uint16_t in = st->word;
...
static err_code_t _update_usr_ctrl_disable_dfe_functions_st (const phymod_access_t *pa, struct phy8806x_tsc_usr_ctrl_disable_dfe_functions_st *st){
	uint8_t in = st->byte;
...
static err_code_t _update_uc_core_config_st (const phymod_access_t *pa, struct phy8806x_tsc_uc_core_config_st *st){
	uint16_t in = st->word;
...
static err_code_t _update_uc_core_config_word (const phymod_access_t *pa, struct phy8806x_tsc_uc_core_config_st *st){
	uint16_t in = 0;
...
static err_code_t _update_uc_lane_config_word (const phymod_access_t *pa, struct phy8806x_tsc_uc_lane_config_st *st){
	uint16_t in = 0;
...
static err_code_t _update_usr_ctrl_disable_functions_byte (const phymod_access_t *pa, struct phy8806x_tsc_usr_ctrl_disable_functions_st *st){
	uint16_t in = 0;
...
static err_code_t _update_usr_ctrl_disable_dfe_functions_byte (const phymod_access_t *pa, struct  phy8806x_tsc_usr_ctrl_disable_dfe_functions_st *st){
	uint8_t in = 0;
...
static err_code_t _phy8806x_tsc_display_pll_to_divider (const phymod_access_t *pa, uint8_t val){
...
static err_code_t _phy8806x_tsc_display_ber_scan_data (const phymod_access_t *pa, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control){
	...
	uint8_t range250;
	phy8806x_tsc_osr_mode_st osr_mode;
	struct phy8806x_tsc_uc_core_config_st core_config;

	ENULL_MEMSET(&core_config, 0, sizeof(core_config));
	ENULL_MEMSET(&osr_mode, 0, sizeof(osr_mode));

	EFUN(phy8806x_tsc_get_uc_core_config (pa, &core_config));
	EFUN(_phy8806x_tsc_get_osr_mode (pa, &osr_mode));
	...
	EFUN(phy8806x_tsc_start_ber_scan_test (pa, ber_scan_mode, timer_control, max_error_control));
	...
	if (verbose > 5) {
		do {
			EFUN(phy8806x_tsc_delay_us(2000000));
		...
		EFUN(phy8806x_tsc_poll_diag_done (pa, &sts, timer_control * 2000));
	}
	...
	for (i = 0; i < cnt; i++) {
		...
		EFUN(phy8806x_tsc_pmd_uc_cmd (pa,  CMD_READ_DIAG_DATA_WORD, 0, 100));
		...
		EFUN(phy8806x_tsc_pmd_uc_cmd (pa,  CMD_READ_DIAG_DATA_BYTE, 0, 100));
		ESTM(prbs_byte = (uint8_t)rd_uc_dsc_data());
		errors = _float12_to_uint32 (pa, prbs_byte, prbs_multi); /* convert 12bits to uint32 */
		timer_values = (_float12_to_uint32 (pa, time_byte, time_multi) << 3);
		if (verbose < 5) {
			if (!(i % 4))
				EFUN_PRINTF(("\n"));
			if (ber_scan_mode & DIAG_BER_HORZ)
	...
	EFUN_PRINTF(("\n"));
	EFUN(phy8806x_tsc_pmd_uc_cmd (pa, CMD_CAPTURE_BER_END, 0x00, 2000));
