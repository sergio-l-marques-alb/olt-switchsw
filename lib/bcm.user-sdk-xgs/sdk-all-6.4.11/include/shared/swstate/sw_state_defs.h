...
#define SHR_SW_STATE_SET(_unit, location, input, type, flags, dbg_string)\
    location = input;
#define SHR_SW_STATE_MEMSET(_unit, location, input, type, flags, dbg_string)\
    sal_memcpy(&location, input, sizeof(type));

#else /*SHR_THIN_SW_STATE*/

#define SHR_SW_STATE_SET(_unit, location, input, type, flags, dbg_string)\
    shr_sw_state_memcpy(_unit, (uint8 *) &location, (uint8 *) &input, sizeof(type), flags);
#define SHR_SW_STATE_MEMSET(_unit, location, input, type, flags, dbg_string)\
    shr_sw_state_memcpy(_unit,(uint8 *) &location, (uint8 *) input, sizeof(type), flags);

#endif /*SHR_THIN_SW_STATE*/

#define SHR_SW_STATE_ALLOC(_unit, location, size, flags, dbg_string)\
    shr_sw_state_alloc(_unit, (uint8**) &location, size, flags);
#define SHR_SW_STATE_FREE(_unit, location, flags, dbg_string)\
    shr_sw_state_free(_unit, (uint8*) location, flags);
#define SHR_SW_STATE_IS_ALLOC(_unit, location, is_alloc, flags, dbg_string)\
...
#define SHR_SW_STATE_ALLOC_BITMAP(_unit, location, nof_bits_to_alloc, flags, dbg_string)\
    shr_sw_state_alloc(_unit, (uint8**) &location, SHR_BITALLOCSIZE(_nof_bits_to_alloc) + 1, flags);\
    shr_sw_state_shr_bitdcl_nof_bits_update(_unit, (uint8 *)(location), (nof_bits_to_alloc));

#ifdef SHR_THIN_SW_STATE /*SHR_THIN_SW_STATE*/

#define SHR_SW_STATE_BIT_SET(__unit, _flags, _sw_state_bmp, __bit_num)\
    SHR_BITSET(_sw_state_bmp, _bit_num);
#define SHR_SW_STATE_BIT_CLEAR(__unit, _flags ,_sw_state_bmp, __bit_num)\
    SHR_BITCLR(_sw_state_bmp, _bit_num);
#define SHR_SW_STATE_BIT_GET(__unit, _flags, _sw_state_bmp, __bit_num , _bit_result)\
    *(_bit_result) = (SHR_BITGET(_sw_state_bmp, _bit_num) > 0) ? 1: 0;
#define SHR_SW_STATE_BIT_RANGE_READ(__unit, _flags, _sw_state_bmp, _sw_state_bmp_first, _result_first, __range, _result_bmp)\
    SHR_BITCOPY_RANGE(_result_bmp, result_first, _sw_state_bmp, _sw_state_bmp_first, __range);
#define SHR_SW_STATE_BIT_RANGE_WRITE(__unit, _flags, _sw_state_bmp, _sw_state_bmp_first, _input_bmp_first, __range, _input_bmp)\
    SHR_BITCOPY_RANGE(_sw_state_bmp, _sw_state_bmp_first, _input_bmp, _input_bmp_first, __range);
#define SHR_SW_STATE_BIT_RANGE_AND(__unit, _flags, _sw_state_bmp, _input_bmp, _sw_state_bmp_first, __count)\
    SHR_BITAND_RANGE(_input_bmp, _sw_state_bmp, _sw_state_bmp_first, __count, _sw_state_bmp);
#define SHR_SW_STATE_BIT_RANGE_OR(__unit, _flags, _sw_state_bmp, _input_bmp, _sw_state_bmp_first, __count)\
    SHR_BITOR_RANGE(_input_bmp, _sw_state_bmp, _sw_state_bmp_first, __count, _sw_state_bmp);
#define SHR_SW_STATE_BIT_RANGE_XOR(__unit, _flags, _sw_state_bmp, _input_bmp, _sw_state_bmp_first, __count)\
    SHR_BITXOR_RANGE(_input_bmp, _sw_state_bmp, _sw_state_bmp_first, __count, _sw_state_bmp);
#define SHR_SW_STATE_BIT_RANGE_REMOVE(__unit, _flags, _sw_state_bmp, _input_bmp, _sw_state_bmp_first, __count)\
    SHR_BITREMOVE_RANGE(_input_bmp, _sw_state_bmp, _sw_state_bmp_first, __count, _sw_state_bmp);
#define SHR_SW_STATE_BIT_RANGE_NEGATE(__unit, _flags, _sw_state_bmp, _sw_state_bmp_first, __count)\
    SHR_BITNEGATE_RANGE(_sw_state_bmp, _sw_state_bmp_first, __count, _sw_state_bmp);
#define SHR_SW_STATE_BIT_RANGE_CLEAR(__unit, _flags, _sw_state_bmp, _sw_state_bmp_first, __count)\
    SHR_BITCLR_RANGE(_sw_state_bmp, _sw_state_bmp_first, __count);
#define SHR_SW_STATE_BIT_RANGE_SET(__unit, _flags, _sw_state_bmp, _sw_state_bmp_first, __count)\
    SHR_BITSET_RANGE(_sw_state_bmp, _sw_state_bmp_first, __count);
#define SHR_SW_STATE_BIT_RANGE_NULL(__unit, _flags, _sw_state_bmp, _sw_state_bmp_first, __count, _bool_result)\
    *(_bool_result) = (SHR_BITNULL_RANGE(_sw_state_bmp, _sw_state_bmp_first, __count) > 0) ? 1: 0;
#define SHR_SW_STATE_BIT_RANGE_TEST(__unit, _flags, _sw_state_bmp, _sw_state_bmp_first, __count, _bool_result)\
    SHR_BITTEST_RANGE(_sw_state_bmp, _sw_state_bmp_first, __count, *(_bool_result));
#define SHR_SW_STATE_BIT_RANGE_EQ(__unit, _flags, _sw_state_bmp, _sw_state_bmp_first, __count, _input_bmp, _bool_result)\
    *(_bool_result) = (SHR_BITEQ_RANGE(_input_bmp, _sw_state_bmp, _sw_state_bmp_first, __count) > 0) ? 1: 0;
#define SHR_SW_STATE_BIT_RANGE_COUNT(__unit, _flags, _sw_state_bmp, _sw_state_bmp_first, __range, _int_result)\
...
do {\
    ...
    SHR_SW_STATE_BIT_OP(__unit, param);\
} while(0)
#define SHR_SW_STATE_BIT_CLEAR(__unit, _flags ,_sw_state_bmp, __bit_num)\
do {\
    sw_state_bitdcl_param_t param;\
...
#define SHR_SW_STATE_PBMP_NEQ(_unit, path, input_pbmp, result, flags, debug_string)\
    *result = (_SHR_PBMP_NEQ(path, input_pbmp) > 0) ? 1: 0;
#define SHR_SW_STATE_PBMP_EQ(_unit, path, input_pbmp, result, flags, debug_string)\
    *result = (_SHR_PBMP_EQ(path, input_pbmp) > 0) ? 1: 0;
#define SHR_SW_STATE_PBMP_MEMBER(_unit, path, input_port, result, flags, debug_string)\
    *result = (_SHR_PBMP_MEMBER(path, input_port) > 0) ? 1: 0;
#define SHR_SW_STATE_PBMP_NOT_NULL(_unit, path, result, flags, debug_string)\
    *result = (_SHR_PBMP_NOT_NULL(path) > 0) ? 1: 0;
#define SHR_SW_STATE_PBMP_IS_NULL(_unit, path, result, flags, debug_string)\
    *result = (_SHR_PBMP_IS_NULL(path) > 0) ? 1: 0;
#define SHR_SW_STATE_PBMP_COUNT(_unit, path, result, flags, debug_string)\
    _SHR_PBMP_COUNT(path, *result);
#define SHR_SW_STATE_PBMP_XOR(_unit, path, input_pbmp, flags, debug_string)\
    _SHR_PBMP_XOR(path, input_pbmp);
#define SHR_SW_STATE_PBMP_REMOVE(_unit, path, input_pbmp, flags, debug_string)\
    _SHR_PBMP_REMOVE(path, input_pbmp);
#define SHR_SW_STATE_PBMP_ASSIGN(_unit, path, input_pbmp, flags, debug_string)\
    _SHR_PBMP_ASSIGN(path, input_pbmp);
#define SHR_SW_STATE_PBMP_GET(_unit, path, output_pbmp, flags, debug_string)\
    _SHR_PBMP_ASSIGN(*output_pbmp, path);
#define SHR_SW_STATE_PBMP_AND(_unit, path, input_pbmp, flags, debug_string)\
    _SHR_PBMP_AND(path, input_pbmp);
#define SHR_SW_STATE_PBMP_NEGATE(_unit, path, input_pbmp, flags, debug_string)\
    _SHR_PBMP_NEGATE(path, input_pbmp);
#define SHR_SW_STATE_PBMP_OR(_unit, path, input_pbmp, flags, debug_string)\
    _SHR_PBMP_OR(path, input_pbmp);
#define SHR_SW_STATE_PBMP_CLEAR(_unit, path, flags, debug_string)\
    _SHR_PBMP_CLEAR(path);
#define SHR_SW_STATE_PBMP_PORT_ADD(_unit, path, input_port, flags, debug_string)\
    _SHR_PBMP_PORT_ADD(path, input_port);
#define SHR_SW_STATE_PBMP_PORT_FLIP(_unit, path, input_port, flags, debug_string)\
    _SHR_PBMP_PORT_FLIP(path, input_port);
#define SHR_SW_STATE_PBMP_PORT_REMOVE(_unit, path, input_port, flags, debug_string)\
    _SHR_PBMP_PORT_REMOVE(path, input_port);
#define SHR_SW_STATE_PBMP_PORT_SET(_unit, path, input_port, flags, debug_string)\
    _SHR_PBMP_PORT_SET(path, input_port);
#define SHR_SW_STATE_PBMP_PORTS_RANGE_ADD(_unit, path, input_port, range, flags, debug_string)\
    _SHR_PBMP_PORTS_RANGE_ADD(path, input_port, range);
#define SHR_SW_STATE_PBMP_FMT(_unit, path, buffer, flags, debug_string)\
    _SHR_PBMP_FMT(path, buffer);

#else /*SHR_THIN_SW_STATE*/

#define SHR_SW_STATE_PBMP_NEQ(_unit, path, input_pbmp, result, flags, debug_string)\
    shr_sw_state_pbmp_operation(_unit, &path, SW_ST_PBMP_NEQ, &input_pbmp, NULL, -1, -1, result, NULL, NULL, flags, debug_string)
#define SHR_SW_STATE_PBMP_EQ(_unit, path, input_pbmp, result, flags, debug_string)\
    shr_sw_state_pbmp_operation(_unit, &path, SW_ST_PBMP_EQ, &input_pbmp, NULL, -1, -1, result, NULL, NULL, flags, debug_string)
#define SHR_SW_STATE_PBMP_MEMBER(_unit, path, input_port, result, flags, debug_string)\
    shr_sw_state_pbmp_operation(_unit, &path, SW_ST_PBMP_MEMBER, NULL, NULL, input_port, -1, result, NULL, NULL, flags, debug_string)
#define SHR_SW_STATE_PBMP_NOT_NULL(_unit, path, result, flags, debug_string)\
    shr_sw_state_pbmp_operation(_unit, &path, SW_ST_PBMP_NOT_NULL, NULL, NULL, -1, -1, result, NULL, NULL, flags, debug_string)
#define SHR_SW_STATE_PBMP_IS_NULL(_unit, path, result, flags, debug_string)\
    shr_sw_state_pbmp_operation(_unit, &path, SW_ST_PBMP_IS_NULL, NULL, NULL, -1, -1, result, NULL, NULL, flags, debug_string)
#define SHR_SW_STATE_PBMP_COUNT(_unit, path, result, flags, debug_string)\
    shr_sw_state_pbmp_operation(_unit, &path, SW_ST_PBMP_COUNT, NULL, NULL, -1, -1, NULL, result, NULL, flags, debug_string)
#define SHR_SW_STATE_PBMP_XOR(_unit, path, input_pbmp, flags, debug_string)\
    shr_sw_state_pbmp_operation(_unit, &path, SW_ST_PBMP_XOR, &input_pbmp, NULL, -1, -1, NULL, NULL, NULL, flags, debug_string)
#define SHR_SW_STATE_PBMP_REMOVE(_unit, path, input_pbmp, flags, debug_string)\
    shr_sw_state_pbmp_operation(_unit, &path, SW_ST_PBMP_REMOVE, &input_pbmp, NULL, -1, -1, NULL, NULL, NULL, flags, debug_string)
#define SHR_SW_STATE_PBMP_ASSIGN(_unit, path, input_pbmp, flags, debug_string)\
    shr_sw_state_pbmp_operation(_unit, &path, SW_ST_PBMP_ASSIGN, &input_pbmp, NULL, -1, -1, NULL, NULL, NULL, flags, debug_string)
#define SHR_SW_STATE_PBMP_GET(_unit, path, output_pbmp, flags, debug_string)\
    shr_sw_state_pbmp_operation(_unit, &path, SW_ST_PBMP_GET, NULL, output_pbmp, -1, -1, NULL, NULL, NULL, flags, debug_string)
#define SHR_SW_STATE_PBMP_AND(_unit, path, input_pbmp, flags, debug_string)\
    shr_sw_state_pbmp_operation(_unit, &path, SW_ST_PBMP_AND, &input_pbmp, NULL, -1, -1, NULL, NULL, NULL, flags, debug_string)
#define SHR_SW_STATE_PBMP_NEGATE(_unit, path, input_pbmp, flags, debug_string)\
    shr_sw_state_pbmp_operation(_unit, &path, SW_ST_PBMP_NEGATE, &input_pbmp, NULL, -1, -1, NULL, NULL, NULL, flags, debug_string)
#define SHR_SW_STATE_PBMP_OR(_unit, path, input_pbmp, flags, debug_string)\
    shr_sw_state_pbmp_operation(_unit, &path, SW_ST_PBMP_OR, &input_pbmp, NULL, -1, -1, NULL, NULL, NULL, flags, debug_string)
#define SHR_SW_STATE_PBMP_CLEAR(_unit, path, flags, debug_string)\
    shr_sw_state_pbmp_operation(_unit, &path, SW_ST_PBMP_CLEAR, NULL, NULL, -1, -1, NULL, NULL, NULL, flags, debug_string)
#define SHR_SW_STATE_PBMP_PORT_ADD(_unit, path, input_port, flags, debug_string)\
    shr_sw_state_pbmp_operation(_unit, &path, SW_ST_PBMP_PORT_ADD, NULL, NULL, input_port, -1, NULL, NULL, NULL, flags, debug_string)
#define SHR_SW_STATE_PBMP_PORT_FLIP(_unit, path, input_port, flags, debug_string)\
    shr_sw_state_pbmp_operation(_unit, &path, SW_ST_PBMP_PORT_FLIP, NULL, NULL, input_port, -1, NULL, NULL, NULL, flags, debug_string)
#define SHR_SW_STATE_PBMP_PORT_REMOVE(_unit, path, input_port, flags, debug_string)\
    shr_sw_state_pbmp_operation(_unit, &path, SW_ST_PBMP_PORT_REMOVE, NULL, NULL, input_port, -1, NULL, NULL, NULL, flags, debug_string)
#define SHR_SW_STATE_PBMP_PORT_SET(_unit, path, input_port, flags, debug_string)\
    shr_sw_state_pbmp_operation(_unit, &path, SW_ST_PBMP_PORT_SET, NULL, NULL, input_port, -1, NULL, NULL, NULL, flags, debug_string)
#define SHR_SW_STATE_PBMP_PORTS_RANGE_ADD(_unit, path, input_port, range, flags, debug_string)\
    shr_sw_state_pbmp_operation(_unit, &path, SW_ST_PBMP_PORTS_RANGE_ADD, NULL, NULL, input_port, range, NULL, NULL, NULL, flags, debug_string)
#define SHR_SW_STATE_PBMP_FMT(_unit, path, buffer, flags, debug_string)\
    shr_sw_state_pbmp_operation(_unit, &path, SW_ST_PBMP_FMT, NULL, NULL, -1, -1, NULL, NULL, buffer, flags, debug_string)
...
#define SW_STATE_NULL_CHECK(param) \
    do{ \
        if((param) == NULL){ \
            ...
            LOG_ERROR(BSL_LS_SHARED_SWSTATE, (BSL_META_U(_unit, "sw state ERROR: trying to free something that was never allocated\n")));\
...
#define SW_STATE_BITDCL_VERIFY(parent_verify_call, ptr, from_bit, to_bit, param_name) \
    do{	\
        ...
        shr_sw_state_shr_bitdcl_nof_bits_get(_unit, (uint8*)ptr, &nof_allocated_bits); \
