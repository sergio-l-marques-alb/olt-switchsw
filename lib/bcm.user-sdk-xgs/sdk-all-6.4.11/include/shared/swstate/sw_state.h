...
#define SHR_SW_STATE_BIT_OP(_unit, param)\
   SW_STATE_IF_ERR_EXIT(shr_sw_state_bit_op(_unit, param))
...
int shr_sw_state_alloc(int _unit, uint8** ptr, uint32 size, uint32 flags);
int shr_sw_state_memcpy(int _unit, uint8 *location, uint8 *input, uint32 length, uint32 flags);
int shr_sw_state_memset(int _unit, uint8 *location, int input, uint32 length, uint32 flags);
int shr_sw_state_pbmp_operation(int _unit, _shr_pbmp_t *path, SW_ST_PBMP op, _shr_pbmp_t *input_pbmp,
                            _shr_pbmp_t *output_pbmp, int input_port, int range, uint8 *result,
                            int *countr, char* buffer, uint32 flags, char *debug_string);
int shr_sw_state_free(int _unit, uint8 *ptr, uint32 flags);
