...
#define _BCM_OAM_NUMBER_OF_BITS_USED_BY_OUTLIF_PCP       SOC_PPC_OAM_NUMBER_OF_BITS_USED_BY_OUTLIF_PCP  
...
#define _BCM_OAM_BFD_DEFAULT_FWD_STRENGTH_GET(_unit) soc_property_get((_unit), spn_DEFAULT_TRAP_STRENGTH, 6)
...
typedef struct bcm_dpp_oam_info_s{
  ...
  uint32                      traps_ref_counter[SOC_PPD_NOF_TRAP_CODES];
  uint8                       y1731_opcode_to_intenal_opcode_map[SOC_PPD_OAM_ETHERNET_PDU_OPCODE_COUNT];
  uint32                      error_type_to_trap_id_map[SOC_PPD_OAM_OAMP_TRAP_TYPE_COUNT];
  uint32                      oamp_rx_trap_code_ref_count[SOC_PPD_NOF_TRAP_CODES];
  SOC_PPC_OAM_INIT_TRAP_INFO  trap_info;
  uint8                       is_internally_allocated_trap[SOC_PPD_NOF_TRAP_CODES];
  uint8                       cpu_trap_code_to_mirror_profile_map[SOC_PPD_NOF_TRAP_CODES];
  /*
   * Linked list heads
   */
...
int _bcm_dpp_oam_fifo_interrupt_handler(int unit, JER_PP_OAM_API_DMA_EVENT_TYPE event_type, SOC_PPC_OAM_INTERRUPT_GLOBAL_DATA * global_data);
