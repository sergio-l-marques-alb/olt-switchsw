...
typedef struct _sbZfC2PmProfileMemoryEntry {
  uint32 m_uType2;
  uint32 m_uLengthShift;
  uint32 m_uCouplingFlag;
  uint32 m_uRfcMode;
  uint32 m_uCommitedBucketSize;
  uint32 m_uColorBlind;
  uint32 m_uExcessBucketNoDecrement;
  uint32 m_uExcessInformationRate;
  uint32 m_uExcessBurstSize;
  uint32 m_uDropOnRed;
  uint32 m_uCommittedBucketNoDecrement;
  uint32 m_uCommittedInformationRate;
  uint32 m_uCommittedBurstSize;
...
#define SB_ZF_C2PMPROFILEMEMORYENTRY_GET_EXCESSBUCKETNODECREMENT(nToData,pFromData,nMaxFromDataIndex) \
          do { \
           (nToData) = (uint32) ((pFromData)[7] >> 6) & 0x01; \
          } while(0)
...
#define SB_ZF_C2PMPROFILEMEMORYENTRY_GET_EXCESSINFORMATIONRATE(nToData,pFromData,nMaxFromDataIndex) \
          do { \
           ...
           (nToData) |= (uint32) ((pFromData)[7] & 0x3f) << 12; \
          } while(0)
...
#define SB_ZF_C2PMPROFILEMEMORYENTRY_GET_COMMITTEDINFORMATIONRATE(nToData,pFromData,nMaxFromDataIndex) \
          do { \
           ...
           (nToData) |= (uint32) ((pFromData)[3] & 0x3f) << 12; \
          } while(0)
