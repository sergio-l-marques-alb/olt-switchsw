...
typedef struct _sbZfC2PrPortStateEntry {
  ...
  uint32 m_nLength;
  uint32 m_nNumPages;
  uint32 m_bStartOfPkt;
  uint32 m_nPageAddr;
  uint32 m_nLineAddr;
  uint32 m_nWordNumber;
  uint32 m_bDropPkt;
...
#define SB_ZF_C2PRPORTSTATEENTRY_GET_PAGE(nToData,pFromData,nMaxFromDataIndex) \
          do { \
           ...
           (nToData) |= (uint32) ((pFromData)[2] & 0x03) << 10; \
          } while(0)
