...
/*
 * $Id: sbZfCaPpCamRamConfigurationInstance0Entry.hx,v 1.4 Broadcom SDK $
...
typedef struct _sbZfCaPpCamRamConfigurationInstance0Entry {
  ...
  uint32 m_uLengthPtr;
  uint32 m_uStateMask4;
  uint32 m_uStateMask3;
  uint32 m_uStateMask2;
  uint32 m_uStateMask1;
  uint32 m_uStateMask0;
  uint32 m_uStateStreamMask;
  uint32 m_uState;
  uint32 m_uHashStart;
...
#define SB_ZF_CAPPCAMRAMCONFIGURATIONINSTANCE0ENTRY_GET_LENGTHMASK(nToData,pFromData,nMaxFromDataIndex) \
          do { \
           (nToData) = (uint32) ((pFromData)[17] >> 7) & 0x01; \
           (nToData) |= (uint32) ((pFromData)[18] & 0x03) << 1; \
          } while(0)

#define SB_ZF_CAPPCAMRAMCONFIGURATIONINSTANCE0ENTRY_GET_LENGTHPTR(nToData,pFromData,nMaxFromDataIndex) \
          do { \
           (nToData) = (uint32) ((pFromData)[17] >> 2) & 0x1f; \
          } while(0)
