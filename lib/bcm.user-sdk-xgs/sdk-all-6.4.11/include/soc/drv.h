...
#define SOC_IS_JERICHO(unit)      ((SOC_INFO(unit).chip_type  == SOC_INFO_CHIP_TYPE_JERICHO) || (SOC_IS_QMX(unit)) || (SOC_IS_QMX_B0(unit))  || (SOC_IS_QAX(unit))  || (SOC_IS_JERICHO_A0(unit))  || (SOC_IS_JERICHO_B0(unit))  )
...
#define SOC_IS_JERICHO_AND_BELOW(unit)  (SOC_IS_ARAD(unit) && !SOC_IS_QAX(unit) )
#define SOC_IS_JERICHO_B0_AND_ABOVE(unit) (SOC_IS_JERICHO_B0(unit) || SOC_IS_QMX_B0(unit))
...
#define SOC_IS_JERICHO_AND_BELOW(unit)  (0)
#define SOC_IS_JERICHO_B0_AND_ABOVE(unit) (0)
#endif

#ifdef BCM_QAX_SUPPORT
#define SOC_IS_QAX(unit)          (SOC_INFO(unit).chip_type   == SOC_INFO_CHIP_TYPE_QAX)
...
#define SOC_IS_DPP(unit) (SOC_IS_PETRAB(unit) || SOC_IS_ARAD(unit) || SOC_IS_ACP(unit) || SOC_IS_JERICHO_P3(unit) || SOC_IS_ARDON(unit) || SOC_IS_QAX(unit))
...
#define SOC_MAX_NUM_IECELL_BLKS           (1)
...
#define         SOC_DMA_UNLOCK(l)    (void)sal_spl(l)
    ...
    schan_op_func_t schan_op;

    soc_dma_mode_t  dma_mode;           /* Continuous or Chained DMA mode */

    LogList_t Hw_Log_List;              /* High Availability Journal */
    /*Miry*/
...
extern int
soc_mem_alpm_aux_table_correction(int unit, int pipe, int index);
