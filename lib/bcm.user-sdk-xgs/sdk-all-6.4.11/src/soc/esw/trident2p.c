                                            ...
                                            soc_td2p_info_t *si_info)
{
    ...
    if (bsl_check(bslLayerSoc, bslSourceTdm, bslSeverityVerbose, unit)) {
        _chip_pkg.soc_vars.td2p.tdm_chk_en=1;
    } else {
        _chip_pkg.soc_vars.td2p.tdm_chk_en=0;
    }
...
{
    int port_macro, pm_flex_enabled = 0, temp_num_hsp = 0;
    soc_info_t *si;
    si = &SOC_INFO(unit);
    ...
    *num_hsp = 0;
    ...
    if (si->frequency >= 760) {
        if (pipe == 0) {
            temp_num_hsp += soc_td2p_is_port_flex_enable(unit, 1);
            temp_num_hsp += soc_td2p_is_port_flex_enable(unit, 21);
            temp_num_hsp += soc_td2p_is_port_flex_enable(unit, 33);
            temp_num_hsp += soc_td2p_is_port_flex_enable(unit, 53);
        } else {
            temp_num_hsp += soc_td2p_is_port_flex_enable(unit, 65);
            temp_num_hsp += soc_td2p_is_port_flex_enable(unit, 85);
            temp_num_hsp += soc_td2p_is_port_flex_enable(unit, 97);
            temp_num_hsp += soc_td2p_is_port_flex_enable(unit, 117);
        }
    } else {
        ...
        if ((flexport_data[i].flags & SOC_PORT_RESOURCE_I_MAP) ||
            ...
            if (si->bandwidth >= 960000) {
                ...
                if (flexport_data[i].speed >= 40000) {
                    ...
                    /* If any port is capable of flexing above 40G,
                       then we've reserved HSP and T2OQ */
                    if(flexport_max_speed >= 40000) {
                        start_mmu_port = SOC_TD2_NUM_HSP_MMU_PORTS_PER_PIPE;
                        /* 53 MMU ports per pipe, MMU port 52 is CPU port,
                           MMU port 116 is Loopback port */
                        end_mmu_port = SOC_TD2_NUM_MMU_PORTS_PER_PIPE - 1;

                        /* Reserving 16 T2OQ per pipe to match with HSP */
                        t2oq_resrved = SOC_TD2_NUM_HSP_MMU_PORTS_PER_PIPE;
                    } else {
                        start_mmu_port = 0;
                        end_mmu_port = SOC_TD2_NUM_MMU_PORTS_PER_PIPE - 1;
                    }
            ...
            for(j = start_mmu_port; j < end_mmu_port; j++) {
                ...
                /* Only want to skip the amount of T2OQ needed to match with
                   ...
                   j < (36 + t2oq_resrved + (pipe * _TD2_PORTS_PER_PIPE)).
...
int soc_td2p_mmu_port_resource_set(int unit, soc_port_t port, int speed)
{
    ...
    SOC_IF_ERROR_RETURN(soc_trident2_lls_bmap_alloc_port(unit, port));
