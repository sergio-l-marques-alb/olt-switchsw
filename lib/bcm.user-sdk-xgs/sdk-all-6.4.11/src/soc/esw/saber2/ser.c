...
_soc_saber2_fifo_ser_info_t _soc_saber2_ip_ser_info[] = {
    ...
    /* SW2_EOP_BUFFER_A/B/C can not access */
...
STATIC 
soc_error_t soc_saber2_found_mem_in_mmu(soc_mem_t mem,
...
soc_saber2_parity_enable_mmu(int unit, soc_reg_t mmu_en_reg, int enable)
{
    ...
    _soc_saber2_mmu_sub_block_one_level_t * sub_block;
    _soc_saber2_mmu_sub_block_two_level_t * two_level_sub_block;
    ...
    for (index = 0; index < sub_block_num; index++) {
         sub_block = &_soc_saber2_mmu_sub_blocks_type_1[index];
    ...
    for (index = 0; index < sub_block_num; index++) {
        two_level_sub_block = &_soc_saber2_mmu_sub_blocks_type_2[index];
...
STATIC void
soc_saber2_ser_control_reg_get(CONST _soc_saber2_fifo_ser_info_t *fifo_ser_list,
                                        soc_mem_t   mem,
            ...
            if (regmem == _SOC_SABER2_SER_MEM) {
                ...
                if (non_sbus == 0) {
                    ...
                    if ((spci.mem == FP_STORM_CONTROL_METERSm) && (inst_type == 2))  {
                        ...
                        if (spci.mem != INVALIDm) {
                            soc_saber2_ser_control_reg_get(ser_info_table,
                                spci.mem,
                                &parity_enable_reg,
                                &parity_enable_field);
                        }
                      ...
                      _soc_saber2_mmu_sub_block_leaf_info_t * leaf_info)
{
    ...
    if(sub_block_type == _SOC_SABER2_MMU_SUBBLOCK_INFO2) {
        ...
        LOG_CLI((BSL_META_U(unit,
                        "\nleaf_parity: status reg:%d value 0x:%x mask reg:%d value 0x:%x\n"),
                    leaf_status_reg,stat_rval,
                    leaf_enable_reg,en_rval));
    }
                               ...
                               soc_reg_t mmu_stat_reg, char *prefix_str)
{
    ...
    uint32 sub_block_num;
    _soc_saber2_mmu_sub_block_one_level_t * sub_block;
    _soc_saber2_mmu_sub_block_two_level_t * sub_block_2;
    ...
    sub_block_num = COUNTOF(_soc_saber2_mmu_sub_blocks_type_2);
    for (index = 0; index < sub_block_num; index++) {
        sub_block_2 = &_soc_saber2_mmu_sub_blocks_type_2[index];
...
soc_saber2_ser_process_all(int unit)
{
    ...
    const      _soc_saber2_parity_route_block_t *rb;
    char       prefix_str[10];
    ...
    /* Loop through each place-and-route block entry */
    for (rbi = 0; ; rbi++) {
        rb = &_soc_saber2_parity_intr_route_blocks[rbi];
...
soc_saber2_ser_error_injection_support (int unit, soc_mem_t mem, int pipe) 
{
    ...
    /* MMU*/    
    rv = soc_saber2_found_mem_in_mmu(mem, &leaf_info);
    if (rv == SOC_E_NONE) {        
        return SOC_E_NONE;
    }
                        ...
                        int block, int index) 
{
    ...
    if (SOC_E_NONE != soc_saber2_found_mem_in_mmu(mem, &leaf_info)) { 
        LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META_U(unit,
                          ...
                          int pipe_target, int block, int index) 
{
    /* for  ser inject */    
     ...
     for (group = 0; _soc_saber2_parity_intr_route_blocks[group].cmic_bit; group++) {
         if(_soc_saber2_parity_intr_route_blocks[group].info == NULL) {
             continue;
         }
         info = _soc_saber2_parity_intr_route_blocks[group].info;

         if (_soc_saber2_parity_intr_route_blocks[group].blocktype == SOC_BLK_MMU) {
             continue;
         } else {
             SOC_BLOCK_ITER(unit, blk,_soc_saber2_parity_intr_route_blocks[group].blocktype) {
                            ...
                            int * pTestErrors) 
{
    ...
    soc_saber2_found_mem_in_mmu(mem, &leaf_info);
...
soc_saber2_ser_mmu_test(int unit, _soc_ser_test_t test_type, int *pTestErrors)
{
    ...
    sub_block_num = COUNTOF(_soc_saber2_mmu_sub_blocks_type_1);
    for (block_index = 0; block_index < sub_block_num; block_index ++) {
        sub_block = &_soc_saber2_mmu_sub_blocks_type_1[block_index];
    ...
    sub_block_num = COUNTOF(_soc_saber2_mmu_sub_blocks_type_2);
    for (block_index = 0; block_index < sub_block_num; block_index++) {
        sub_block_2 = &_soc_saber2_mmu_sub_blocks_type_2[block_index];
...
soc_saber2_ser_hw_test(int unit, _soc_ser_test_t test_type, int *pTestErrors)
{
    ...
    for (group = 0; _soc_saber2_parity_intr_route_blocks[group].cmic_bit; group++) {
        ...
        if ((_soc_saber2_parity_intr_route_blocks[group].blocktype == SOC_BLK_MXQPORT) ||
            (_soc_saber2_parity_intr_route_blocks[group].blocktype == SOC_BLK_XLPORT) ||
            (_soc_saber2_parity_intr_route_blocks[group].blocktype == SOC_BLK_TXLP) ||
            (_soc_saber2_parity_intr_route_blocks[group].blocktype == SOC_BLK_RXLP))            
        {
            continue;
        } 
...
STATIC soc_error_t
soc_saber2_found_mem_in_mmu(soc_mem_t mem,
                            _soc_saber2_mmu_sub_block_leaf_info_t  * mmu_leaf_info)
{
