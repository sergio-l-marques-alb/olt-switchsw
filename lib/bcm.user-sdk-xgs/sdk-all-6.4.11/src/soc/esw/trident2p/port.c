                                ...
                                soc_port_resource_t *resource)
{
    ...
    LOG_VERBOSE(BSL_LS_SOC_PORT,
                (BSL_META_U(unit,
                 ...
                 SOC_PBMP_FMT(phy_pbmp, phy_pfmt)));
    ...
    LOG_VERBOSE(BSL_LS_SOC_PORT,
     ...
     for (i = 0, pr = &resource[0]; i < nport; i++, pr++) {
        ...
        for (j = 0; j < _TD2_PORTS_PER_XLP; j++) {
            ...
            if (speed != sister_speed) {
                LOG_ERROR(BSL_LS_SOC_PORT,
                          (BSL_META_U(unit,
                                      "Speed must be the same on "
...
/*
 * Function:
 *      _soc_td2p_port_resource_encap_validate
 * Purpose:
 *      Validate that ports within the same Port Macro must
 *      have the same encapsulation mode.
 * Parameters:
 *      unit     - (IN) Unit number.
 *      nport    - (IN) Number of elements in array resource.
 *      resource - (IN) Port resource configuration array.
 ...
 */
STATIC int
_soc_td2p_port_resource_encap_validate(int unit,
                                       int nport,
                                       soc_port_resource_t *resource,
                                       soc_info_misc_t *post_si)
{
    ...
    for (i = 0, pr = &resource[0]; i < nport; i++, pr++) {
        ...
        /*
         * Check that ports within a Port Macro have the same
         * encapsulatio mode (either HG or IEEE).
         */
        encap = pr->encap;
        for (j = 0; j < _TD2_PORTS_PER_XLP; j++) {
            sister_port = TD2P_PHY_PORT(unit, phy_port).sisters_ports[j];
            ...
            sister_encap = post_si->port_encap[logic_port];
            if (encap != sister_encap) {
                LOG_ERROR(BSL_LS_SOC_PORT,
                          (BSL_META_U(unit,
                                      "Encapsulation mode must be same "
                                      "on physical ports %d %d %d %d\n"),
                           TD2P_PHY_PORT(unit, phy_port).sisters_ports[0],
                           TD2P_PHY_PORT(unit, phy_port).sisters_ports[1],
                           TD2P_PHY_PORT(unit, phy_port).sisters_ports[2],
                ...
                LOG_ERROR(BSL_LS_SOC_PORT,
                          (BSL_META_U(unit,
                                      ...
                                      "encap=%s\n"),
                           pr->logical_port, pr->physical_port,
                           (encap == SOC_ENCAP_HIGIG2) ? "HG" : "IEEE",
                           logic_port, sister_port,
                           (sister_encap == SOC_ENCAP_HIGIG2) ? "HG" : "IEEE"));
                                ...
                                soc_port_resource_t *resource)
{
    ...
    rv = _soc_td2p_port_resource_encap_validate(unit, nport, resource,
                                                post_si);
    if (SOC_FAILURE(rv)) {
        sal_free(post_si);
        return rv;
 ...
 */
STATIC int
_soc_td2p_soc_info_ptype_update(int unit)
{
    soc_info_t *si = &SOC_INFO(unit);
    int phy_port;
    int logic_port;
                                   ...
                                   soc_port_resource_t *post_resource)
{
    ...
    SOC_IF_ERROR_RETURN(_soc_td2p_soc_info_ptype_update(unit));
                             ...
                             soc_port_resource_t *post_resource)
{
    ...
    for (i = 0, pr = &post_resource[0]; i < post_count; i++, pr++) {
        ...
        if ((pr->speed >= 100000) ||
            ...
            soc_property_port_get(unit, logic_port, spn_PORT_SCHED_HSP, 0)) {
 ...
 */
STATIC int
_soc_td2p_misc_port_attach(int unit, soc_port_t port)
{
    soc_info_t *si;
    int phy_port;
    uint32 rval;
 ...
 */
STATIC int
_soc_td2p_misc_port_detach(int unit, soc_port_t port)
{
    soc_info_t *si;
    int phy_port;
    uint32 rval;
                                    ...
                                    soc_port_resource_t *resource)
{
    ...
    for (i = 0, pr = &resource[0]; i < nport; i++, pr++) {
        if (pr->flags & SOC_PORT_RESOURCE_ATTACH) {
            rv = _soc_td2p_misc_port_attach(unit, pr->logical_port);
            if (SOC_FAILURE(rv)) {
                LOG_ERROR(BSL_LS_SOC_PORT,
                          (BSL_META_U(unit,
                                    ...
                                    soc_port_resource_t *resource)
{
    ...
    for (i = 0, pr = &resource[0]; i < nport; i++, pr++) {
        if (pr->flags & SOC_PORT_RESOURCE_DETACH) {
            rv = _soc_td2p_misc_port_detach(unit, pr->logical_port);
            if (SOC_FAILURE(rv)) {
                LOG_ERROR(BSL_LS_SOC_PORT,
                          (BSL_META_U(unit,
...
/*
 * Function:
 *      _soc_td2p_port_macro_ports_delete
 * Purpose:
 ...
 */
STATIC int
_soc_td2p_port_macro_ports_delete(int unit,
                                  int pre_count,
                                  soc_port_resource_t *pre_resource)
{
    int i;
...
/*
 * Function:
 *      _soc_td2p_port_macro_ports_add
 * Purpose:
 ...
 */
STATIC int
_soc_td2p_port_macro_ports_add(int unit,
                               int nport,
                               soc_port_resource_t *resource)
{
                                ...
                                soc_td2p_info_t *pre_soc_info)
{
    ...
    /* Delete ports from Port Macro */
    SOC_IF_ERROR_RETURN
        (_soc_td2p_port_macro_ports_delete(unit, delete_count, pre_resource));
    ...
    /* Add ports to Port Macro */
    SOC_IF_ERROR_RETURN
        (_soc_td2p_port_macro_ports_add(unit, nport, resource));
