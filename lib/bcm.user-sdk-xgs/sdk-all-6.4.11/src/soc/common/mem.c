...
soc_mem_cache_set(int unit, soc_mem_t mem, int copyno, int enable)
{
    ...
    SOC_MEM_BLOCK_ITER(unit, mem, blk) {
        ...
        if (memState->cache[blk] != NULL) {
            if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "mem_cache_dma", 0))
                soc_cm_sfree(unit, memState->cache[blk]);
            else
        ...
        if (memState->vmap[blk] != NULL) {
            if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "mem_cache_dma", 0))
                soc_cm_sfree(unit, memState->vmap[blk]);
            else
    ...
    defined(BCM_GREYHOUND_SUPPORT)
        ...
        if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "mem_cache_dma", 0)) {
            /* Allocate new DMA-able cache */
            if ((cache = soc_cm_salloc(unit, cache_size, "table-cache")) == NULL) {
                MEM_UNLOCK(unit, mem);
                return SOC_E_MEMORY;
            }

            if ((vmap = soc_cm_salloc(unit, vmap_size, "table-vmap")) == NULL) {
                sal_free(cache);
                MEM_UNLOCK(unit, mem);
                return SOC_E_MEMORY;
            }
                             ...
                             int        entries_count)      /* number of entries to move (Range) */
{
    ...
    SOC_MEM_BLOCK_ITER(unit, mem, blk)
    {
        ...
        /* Copy cache vmap entries from old location to new location. We are doing it in 2 copies since the blocks can overlap. */
        sal_memcpy(tmp_cache_block, vmap + src_mem_array_vmap_offset+ (src_index_start * entry_dw), block_size_byte);
        sal_memcpy(vmap + dest_mem_array_vmap_offset + (dest_index_start * entry_dw), tmp_cache_block, block_size_byte);
                             ...
                             void **buffer)
{
    ...
    int index_min[TCAM_PROTECT_RANGES], index_max[TCAM_PROTECT_RANGES];
    int split_count[TCAM_PROTECT_RANGES];
...
#endif
    incr = soc_mem_index_count(unit, mem) / (TCAM_PROTECT_RANGES >> wide);
    for (i = 0; i < count; i++) {
        if (is_l3_defip) {
            idx = (index + i) / incr;
        } else {
        ...
        if (idx >= (TCAM_PROTECT_RANGES >> wide)) {
            return SOC_E_INTERNAL;
        }
    ...
    entry_ptr = entry;
    for (idx = 0; idx < TCAM_PROTECT_RANGES; idx++) {
        if (index_min[idx] == -1) {
            continue;
        }
                   ...
                   int index_begin, int index_end, void *buffer, void *cache_buffer)
{
    ...
    void          *shift_buffer[TCAM_PROTECT_RANGES] = {NULL};
#endif
...
#ifdef BCM_SBUSDMA_SUPPORT
    if (soc_feature(unit, soc_feature_sbusdma)) {
        rv = _soc_mem_array_sbusdma_write(unit, flags, mem, array_index, array_index,
...
/*
 * Function:
 *    _soc_mem_alpm_pipe_read_attempt
 * Purpose:
 ...
 */
STATIC int
_soc_mem_alpm_pipe_read_attempt(int unit, uint32 flags, soc_mem_t mem,
                          ...
                          int resp_word, int *rv, int *all_done)
{
    ...
    if (cache != NULL && CACHE_VMAP_TST(vmap, index)
        ...
        if (SOC_IS_TOMAHAWK(unit)) {
            if (_soc_mem_alpm_pipe_read_attempt(unit, flags, mem, index,
...
/*     Use crash recovery defined callback for access*/
    ...
    if (SOC_IS_DONE_INIT(unit))
    {
        ...
        {
            if(SOC_CONTROL(unit)->Hw_Log_List.Access_cb.mem_read)
            {

                return SOC_CONTROL(unit)->Hw_Log_List.Access_cb.mem_read(unit, mem, array_index, copyno, index, entry_data);
            }
        }
    }
 ...
 */
STATIC void
_soc_mem_write_ext_cache_set(uint32 flags, int *index, int *no_cache, int *no_hw)
{
    ...
    if ((flags & SOC_MEM_ONLY_CACHE) == SOC_MEM_ONLY_CACHE) {
        *no_hw = 1;
    } else {
        *no_hw = 0;
    }
...
#if defined(BCM_TRIDENT2_SUPPORT)
STATIC int
_soc_mem_write_nack_sram_correction_check(int unit, soc_mem_t mem) 
{
    uint32 l2_mod_fifo, rval;
    int count, memwr;
    /* 
     * memwr nack exception 
     * 1 mem is l2X
     * 2 L2_MOD_FIFO is enable for memwr
     * 3 L2_MOD_FIFO is full
     */
    if (SOC_IS_TD2_TT2(unit)) {
        switch (mem) {
            case L2Xm:
                SOC_IF_ERROR_RETURN(
                        READ_L2_MOD_FIFO_CNTr(unit, &l2_mod_fifo));
                count = soc_reg_field_get(unit, L2_MOD_FIFO_CNTr, 
                                          l2_mod_fifo, NUM_OF_ENTRIESf);

                if (SOC_REG_IS_VALID(unit,L2_MOD_FIFO_ENABLEr)) {
                    SOC_IF_ERROR_RETURN(
                            READ_L2_MOD_FIFO_ENABLEr(unit, &rval));
                    memwr = soc_reg_field_get(unit, L2_MOD_FIFO_ENABLEr,
                                              rval, L2_MEMWRf);
                } else {
                    SOC_IF_ERROR_RETURN(
                ...
                if ((count == soc_mem_index_count(unit, L2_MOD_FIFOm)) && 
                    (memwr == 1)) {
                    return SOC_E_INTERNAL;
                }
                break;
                              ...
                              int blk, int index2)
{
    ...
    do {
...
#if defined(BCM_TRIDENT2_SUPPORT)
            rv1 = _soc_mem_write_nack_sram_correction_check(unit, mem); 
            if (SOC_FAILURE(rv1)) {
                return rv;
            }
#endif
        ...
        if (SOC_E_FAIL == rv && soc_feature(unit, soc_feature_ignore_mem_write_nak)){
            /*
             * In bcm53400 series, when parity error happened in
...
_soc_mem_write_schan_msg_create(int unit, soc_mem_t mem, int copyno,
                                ...
                                uint32 *cache_entry_data,
                                uint32 *converted_entry_data,
                                int index, int no_cache, int no_hw)
{
    int blk;
    int index2;
    schan_msg_t schan_msg;
               ...
               int index, void *entry_data)
{
    ...
    _soc_mem_write_ext_cache_set(flags, &index, &no_cache, &no_hw);
              ...
              int index, void *entry_data)
{
    ...
    if (SOC_IS_DONE_INIT(unit))
    {
        if (BCM_UNIT_DO_HW_READ_WRITE(unit))
        {
            if(SOC_CONTROL(unit)->Hw_Log_List.Access_cb.mem_write)
            {
/*
...
*/
                ...
                return SOC_CONTROL(unit)->Hw_Log_List.Access_cb.mem_write(unit, mem, array_index, copyno, index, entry_data);
            }
        }
    }
...
#ifdef BCM_SBUSDMA_SUPPORT
        if (soc_feature(unit, soc_feature_sbusdma)) {
            rv = _soc_mem_array_sbusdma_write(unit, 0, mem, min_ar_index, max_ar_index,
...
STATIC INLINE int
_soc_mem_shared_hash_init(int unit, soc_mem_t mem, SHR_BITDCL **trace,
                          ...
                          int recurse_depth,_soc_hash_bank_info_t *bank_info)
{
    ...
    _soc_hash_bank_info_t bank_inf;
    uint8 num_ent = 0;
    ...
    if (NULL == bucket_trace) {
        SOC_IF_ERROR_RETURN(
            _soc_mem_shared_hash_init(unit, mem, &trace, &bank_inf));
    } else {
        trace = bucket_trace;
        if (NULL != bank_info) {
    ...
    for (bix = 0; bix < bank_inf.num_banks; bix++) {
        ...
        cb = bank_inf.bank_ids[cbix];
        if (banks == cb) {
            /* Not this bank */
            continue;
        }
        nbix = (cbix + 1) % bank_inf.num_banks; /* next bank index */
        db = bank_inf.bank_ids[nbix]; /* next destination bank */
#ifdef BCM_TOMAHAWK_SUPPORT
        if (SOC_IS_TOMAHAWK(unit)) {
                                ...
                                _soc_hash_bank_info_t *bank_info)
{
    ...
    for (bix = 0; bix < bank_info->num_banks; bix++) {
        ...
        bank_info->bank_bmp |= 1 << bank_info->bank_ids[bix];
    }
    ...
    /* Keep back trace of all buckets affected by recursion. */
    trace_size = SHR_BITALLOCSIZE(bank_info->numb * 10);
