...
_soc_egress_metering_freeze(int unit, soc_port_t port, void **setting)
{
    ...
    case SOC_CHIP_BCM56334:
        {
            int i;
            uint64 regval, temp_rval64;
            uint64 *buffer;
            int count;
            /* buffer required = (X+1), for store original register setting.
            * MAXBUCKETCONFIG_64r : X elements for a port
            * EGRMETERINGCONFIG_64r : 1 register value for a port
            */
            count = SOC_REG_NUMELS(unit, MAXBUCKETCONFIG_64r) + 1;
            buffer = sal_alloc(count * sizeof(uint64), "shaper buffer");
            if (buffer == NULL) {
                rv = SOC_E_MEMORY;
                break;
            }
            ...
            /* save the original configuration : MAXBUCKETCONFIG_64r */
            count = 0;
            for (i = 0; i < SOC_REG_NUMELS(unit, MAXBUCKETCONFIG_64r); i++) {
                rv = READ_MAXBUCKETCONFIG_64r(unit, port, i, &regval);
                if (SOC_FAILURE(rv)) {
                    sal_free(buffer);
                    break;
                }
                ...
                count ++;
                /* Disable egress metering for this port */
                soc_reg64_field32_set(unit, MAXBUCKETCONFIG_64r, &regval, MAX_REFRESHf, 0);
                soc_reg64_field32_set(unit, MAXBUCKETCONFIG_64r, &regval, MAX_THDf, 0);
                rv = WRITE_MAXBUCKETCONFIG_64r(unit, port, i, regval);
                if (SOC_FAILURE(rv)) {
                    sal_free(buffer);
                    break;
                }
            }
            /* save the original configuration : EGRMETERINGCONFIG_64r */
    ...
    case SOC_CHIP_BCM56334:
        {
            ...
            count = 0;
            for (i = 0; i < SOC_REG_NUMELS(unit, MAXBUCKETCONFIG_64r); i++) {
                rv = WRITE_MAXBUCKETCONFIG_64r(unit, port, i, buffer[count]);
                if (SOC_FAILURE(rv)) {
                    err_flag = 1;
                    break;
                }
