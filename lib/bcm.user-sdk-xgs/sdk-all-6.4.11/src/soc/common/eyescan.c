...
int is_eyescan_algorithm_legacy_mode(int unit, uint32 nof_ports, soc_port_t* ports, uint32 inst)
{
        ...
        if(PHY_DIAG_INST_DEV(inst) == PHY_DIAG_DEV_EXT) {
            ...
            /* Checking ID for tscf tsce phy */
            if ((pc->phy_id0 == 0x600d) && (pc->phy_id1 == 0x8770) && (pc->phy_model == 0x37)) {
                legacy = 0;
            }
            /* Checking id for furia */
            if ((pc->phy_id0 == 0xae02) && (pc->phy_id1 == 0x5230)) {
        ...
        /* internal phy is alwaus portmode and not legacy */  
        } else {
...
{
#ifdef PHYMOD_SUPPORT
    if (is_eyescan_algorithm_legacy_mode(unit, nof_ports, ports,inst) || !soc_feature(unit, soc_feature_portmod)) {
        ...
        if (params->type == 1) {
            rv = _soc_port_phy_eyescan_run_uc(
