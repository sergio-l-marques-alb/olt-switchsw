/*
 * $Id: sbZfCaPmTimerCheckerStateConsole.c,v 1.3 Broadcom SDK $
...
sbZfCaPmTimerCheckerState_Print(sbZfCaPmTimerCheckerState_t *pFromStruct) {
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("CaPmTimerCheckerState:: profile=0x%03x"), (unsigned int)  pFromStruct->m_uProfile));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" timerstart=0x%01x"), (unsigned int)  pFromStruct->m_bTimerStarted));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" timer=0x%05x"), (unsigned int)  pFromStruct->m_uTimer));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("\n")));

  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("CaPmTimerCheckerState:: seqnum=0x%08x"), (unsigned int)  pFromStruct->m_uSequenceNumber));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("\n")));
...
char *
sbZfCaPmTimerCheckerState_SPrint(sbZfCaPmTimerCheckerState_t *pFromStruct, char *pcToString, uint32 lStrSize) {
  uint32 WrCnt = 0x0;
