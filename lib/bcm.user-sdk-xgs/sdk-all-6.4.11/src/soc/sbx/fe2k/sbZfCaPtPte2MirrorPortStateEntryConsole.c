...
#include "sbZfCaPtPte2MirrorPortStateEntryConsole.hx"



/* Print members in struct */
void
sbZfCaPtPte2MirrorPortStateEntry_Print(sbZfCaPtPte2MirrorPortStateEntry_t *pFromStruct) {
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("CaPtPte2MirrorPortStateEntry:: hdrpresent=0x%01x"), (unsigned int)  pFromStruct->m_uHdrPresent));
  LOG_WARN(BSL_LS_SOC_COMMON,
  ...
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("CaPtPte2MirrorPortStateEntry:: numpageshi=0x%01x"), (unsigned int)  pFromStruct->m_uNumPagesHi));
  LOG_WARN(BSL_LS_SOC_COMMON,
  ...
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("CaPtPte2MirrorPortStateEntry:: sourcequeue=0x%02x"), (unsigned int)  pFromStruct->m_uSourceQueue));
  LOG_WARN(BSL_LS_SOC_COMMON,
...
char *
sbZfCaPtPte2MirrorPortStateEntry_SPrint(sbZfCaPtPte2MirrorPortStateEntry_t *pFromStruct, char *pcToString, uint32 lStrSize) {
  uint32 WrCnt = 0x0;

  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"CaPtPte2MirrorPortStateEntry:: hdrpresent=0x%01x", (unsigned int)  pFromStruct->m_uHdrPresent);
  WrCnt += SB_SPRINTF(&pcToString[WrCnt]," pktlength=0x%04x", (unsigned int)  pFromStruct->m_uPktLength);
  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"\n");

  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"CaPtPte2MirrorPortStateEntry:: numpageshi=0x%01x", (unsigned int)  pFromStruct->m_uNumPagesHi);
  ...
  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"CaPtPte2MirrorPortStateEntry:: sourcequeue=0x%02x", (unsigned int)  pFromStruct->m_uSourceQueue);
...
int
sbZfCaPtPte2MirrorPortStateEntry_Validate(sbZfCaPtPte2MirrorPortStateEntry_t *pZf) {
...
int
sbZfCaPtPte2MirrorPortStateEntry_SetField(sbZfCaPtPte2MirrorPortStateEntry_t *s, char* name, int value) {
