/*
 * $Id: sbZfCaPtPte5MirrorPortStateEntry.c,v 1.2 Broadcom SDK $
...
#include "sbTypes.h"
#include "sbZfCaPtPte5MirrorPortStateEntry.hx"
...
sbZfCaPtPte5MirrorPortStateEntry_Pack(sbZfCaPtPte5MirrorPortStateEntry_t *pFrom,
  ...
  int i;
  int size = SB_ZF_CAPTPTE5MIRRORPORTSTATEENTRY_SIZE_IN_BYTES;
  ...
  return SB_ZF_CAPTPTE5MIRRORPORTSTATEENTRY_SIZE_IN_BYTES;
}
...
sbZfCaPtPte5MirrorPortStateEntry_Unpack(sbZfCaPtPte5MirrorPortStateEntry_t *pToStruct,
  ...
  (pToStruct)->m_uHdrPresent =  (uint32)  ((pFromData)[6] >> 1) & 0x01;

  /* Unpack Member: m_uPktLength */
  (pToStruct)->m_uPktLength =  (uint32)  ((pFromData)[4] >> 3) & 0x1f;
  (pToStruct)->m_uPktLength |=  (uint32)  (pFromData)[5] << 5;
  (pToStruct)->m_uPktLength |=  (uint32)  ((pFromData)[6] & 0x01) << 13;

  /* Unpack Member: m_uNumPagesHi */
  (pToStruct)->m_uNumPagesHi =  (uint32)  ((pFromData)[4] ) & 0x07;

  /* Unpack Member: m_uNumPagesLo */
  (pToStruct)->m_uNumPagesLo =  (uint32)  ((pFromData)[3] >> 4) & 0x0f;

  /* Unpack Member: m_uBuffer */
  (pToStruct)->m_uBuffer =  (uint32)  (pFromData)[2] ;
  (pToStruct)->m_uBuffer |=  (uint32)  ((pFromData)[3] & 0x0f) << 8;

  /* Unpack Member: m_uSourceQueue */
  (pToStruct)->m_uSourceQueue =  (uint32)  (pFromData)[1] ;

  /* Unpack Member: m_uQueue */
  (pToStruct)->m_uQueue =  (uint32)  (pFromData)[0] ;
