/******************************************************************************
 *
 * $Id: fe2k-asm2-intd.c,v 1.10 Broadcom SDK $ 
...
int fe2kAsm2IntD__mwalign(fe2kAsm2IntD *a_p) {
    if (a_p->m_hwid < 3) {
        /* Inst in C2 are 10 octets, need this for alignment */
        return 16;
    }
    /* C3 is 12 octets, aligned naturally */
    return 0;
}
    ...
    for (i=0; i<a_p->m_qal; i++) {
        ...
        for (root = a_p->m_qat[i]; root; root = root->m_down) {
            ...
            if (nc) {
                ...
                align = fe2kAsm2IntD__mwalign(a_p);

                /* move bit stream to the offset of the field */ 
                Bsc__moveB (&bsc, 
                            a_p->m_onum * 8 - ((nc->m_bo-align) + nc->m_bl),
                            eBscBeg);
...
int fe2kAsm2IntD__getNc(fe2kAsm2IntD *a_p, char *a_n, unsigned int *a_nc) 
{
  ...
  for (i=0; i< a_p->m_ncnum; i++) {
	...
	Bsc__setLsof (&bsc, FALSE_VN); /*set appropriate bsc attributes*/
        align = fe2kAsm2IntD__mwalign(a_p);
	Bsc__moveB (&bsc, a_p->m_onum*8 - ( (a_e->m_bo-align) + a_e->m_bl), eBscBeg);/*move bit stream to the offset of the field */
