...
/*
 * $Id: sbFe2000UcodeLoad.c,v 1.21 Broadcom SDK $
 *
...
uint32 sbFe2000DisableLr(sbhandle userDeviceHandle)
{
  ...
  if (timeout == 0) {
    LOG_WARN(BSL_LS_SOC_COMMON,
             (BSL_META("ERROR Timed out waiting for lrp pending requests to finish\n")));
    return (-1);
  }
...
uint32
sbFe2000ReloadUcode(sbhandle userDeviceHandle, uint8 * ucode)
{
  int status;
 ...
 *----------------------------------------------------------------------------*/
uint32 sbFe2000GetNamedConstant(sbhandle userDeviceHandle, fe2kAsm2IntD *ucode, char *nc, uint32 *val)
{
  int status = SB_OK;
 ...
 * @return Named Constant value
 *----------------------------------------------------------------------------*/
uint32 sbFe2000SetNamedConstant(sbhandle userDeviceHandle, fe2kAsm2IntD *ucode, char *nc, uint32 val)
{
  int status = SB_OK;
...
{
  uint32 uBankSwapEvent, uTimeOut;
  ...
  if (SAND_HAL_IS_FE2KXT(userDeviceHandle)){
    ...
    if ( !uTimeOut ) {
      LOG_ERROR(BSL_LS_SOC_COMMON,
                (BSL_META("Timeout waiting for BANK_SWAP event from the LRP.\n")));
