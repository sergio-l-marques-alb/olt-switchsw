...
/*
 * $Id: sbFe2000CmuMgr.c,v 1.22 Broadcom SDK $
 *
...
struct sbFe2000CmuMgr_s {
  ...
  uint64 *ullCount[SB_FE2000_SEG_TBL_SEG_MAX];
  void* pSegMem[SB_FE2000_SEG_TBL_SEG_MAX];
  /* hold onto the bank and base */
  uint32  ulBank[SB_FE2000_SEG_TBL_SEG_MAX];
  uint32  ulBase[SB_FE2000_SEG_TBL_SEG_MAX];
  uint32 ulSize[SB_FE2000_SEG_TBL_SEG_MAX];
  /* pointer to per-segment host-side counters */
  sbFe2000SegTblType_e_t eType[SB_FE2000_SEG_TBL_SEG_MAX];
  /* segment in use count */
  uint32 ulUsedSegments;
  ...
  uint32  cmu_watermark;
    
  sbFe2000DmaMgr_t *pDmaState;
};
...
sbFe2000CmuHwInit(sbFe2000CmuMgr_t *pCtl, uint32 warmboot)
{
  ...
  uint32 ulRingSize = pCtl->ulRingSize - 1; /* power-of-2, expressed as mask */
  uint32 ulRingThresh = pCtl->ulRingThresh, ulMMClientCfg, reg;
                                ...
                                sbFe2000SegTblType_e_t eType)
{
  ...
  pBuf = thin_host_malloc(sizeof(uint64) * ulHostWords + 7);
  if (!pBuf)
    return SB_MALLOC_FAILED;
  ...
  SB_ASSERT((((uint32) pBuf) & 0x7) == 0);

  if(*ulSegId == -1) {
    /* map the segment for use */
    LOG_INFO(BSL_LS_SOC_COMMON,
             (BSL_META("Automatically Allocating Counter Seg %d\n"), *ulSegId));
    sts = sbFe2000CmuSegmentSet(pCtl, pCtl->ulUsedSegments, ulBank, ulBankBase,
                                (uint32) pBuf, ulNumCounters, eType,
    ...
    LOG_INFO(BSL_LS_SOC_COMMON,
             (BSL_META("Allocating Counter Seg %d\n"), *ulSegId));
...
sbFe2000CmuRingProcess(sbFe2000CmuMgr_t *pCtl, int flush)
{
  ...
  uint64 *pData;
  uint8 bWrapped = 0;
  uint32 consumer, producer, flushevent, count=0;
  ...
  while (1) {
    ...
    if ( !SOC_WARM_BOOT((uint32)pCtl->dev) && (!flush) ) {
...
{
  uint32      ulStride = 0;
  uint32      ulTimeout = CMU_TIMEOUT;
  uint64      *pullCount;
  uint32      i, base;
  uint32      flush_event, flush_event_done_mask;
  sbStatus_t    sts = SB_OK;
  uint32      bMutex = 0;

  if ( (bSync) && (!SOC_WARM_BOOT((uint32)pCtl->dev))) {
    sal_mutex_take(pCtl->cmu_flush_lock, sal_mutex_FOREVER);
    pCtl->bFlushing = 1;
    sts = sbFe2000CmuForceFlush(pCtl, ulSegment, ulOffset, ulEntries);
    if (sts != SB_OK) {
      LOG_WARN(BSL_LS_SOC_COMMON,
               (BSL_META("ERROR: CMU Force Flush already in progress. \n")));
  ...
  if (pCtl->cmu_watermark > ulTimeout) {
    LOG_WARN(BSL_LS_SOC_COMMON,
             (BSL_META("CMU force flush watermark = %d \n"), ulTimeout));
  ...
  if (!ulTimeout) {
      ...
      if (flush_event & flush_event_done_mask) {
          ...
          LOG_WARN(BSL_LS_SOC_COMMON,
                   (BSL_META("WARNING: did not receive CM_FORCE_FLUSH_EVENT interrupt "
                             "but done bit set. \n")));
          sts = SB_TIMEOUT_ERR_CODE;
      } else {
          /* cmu flush operation did not complete */
          LOG_WARN(BSL_LS_SOC_COMMON,
                   (BSL_META("CMU flush timed out!\n")));
          if (bSync) {
              sal_mutex_give(pCtl->cmu_flush_lock);
          }
...
{
  sbStatus_t sts;
  uint32 ulBank, ulMemSrc, ulData, ulBytes, ulPackets, ulBase, ulIdx,ulDataClear;
...
{
  uint32 ulOffset, ulCfgAddr, ulMmuAddr, ulPciAddr, ulLimitAddr;
  uint32 ulPciBaseDw, ulCfgData, ulMmuData, ulLimitData, ulPciData;
  uint32 ulSegEnable;
  ...
  ulOffset = ulSegment * (4 * sizeof(uint32));
...
sbFe2000CmuSegmentFlush(sbFe2000CmuMgr_t *pCtl,
                        ...
                        uint32 ulRateData)
{
  uint32 ulOffset, ulCtrlAddr, ulRateAddr, ulCtrlData;
  ...
  ulOffset = ulSegment * (2 * sizeof(uint32));
...
sbFe2000CmuSegmentFlushStatus(sbFe2000CmuMgr_t *pCtl,
                              ...
                              uint32 *ulCntrId)
{
  uint32 ulOffset, ulCtrlAddr, ulReg, ulCntrIdx;
  uint8 bInProgress;
 ...
 *----------------------------------------------------------------------------*/
sbStatus_t
sbFe2000CmuSegmentClear(sbFe2000CmuMgr_t *pCtl, uint32 ulSegment)
{
    uint32 uMmProtectionScheme0, uMmProtectionScheme0Stash;
    uint32 uMmProtectionScheme1, uMmProtectionScheme1Stash;
    uint32 uInstance;
    uint32 v, v0, i, ulAddr, ulMemSrc, ulBank, ulBase, ulWords;
    ...
    interrupt_mask = SOC_IRQ_MASK((uint32)pCtl->dev);
    SOC_IRQ_MASK((uint32)pCtl->dev) = 0x00001fff;
    SAND_HAL_FE2000_WRITE(pCtl->dev, PC_INTERRUPT_MASK, SOC_IRQ_MASK((uint32)pCtl->dev));
    ...
    SAND_HAL_FE2000_WRITE(pCtl->dev,  PC_DMA_CTRL0,
                   SAND_HAL_FE2000_FIELD_MASK(pCtl->dev, PC_DMA, CTRL0_ACK));
    if (i == 5000) {
      SOC_IRQ_MASK((uint32)pCtl->dev) = interrupt_mask;
      SAND_HAL_FE2000_WRITE(pCtl->dev, PC_INTERRUPT_MASK, SOC_IRQ_MASK((uint32)pCtl->dev));
    ...
    SB_MEMSET(pCtl->ullCount[ulSegment], 0, sizeof(uint64) * ulWords);
