/*
 * $Id: g2p3_eml_comp.c,v 1.22 Broadcom SDK $
 * 
...
uint32
g2p3_eml_l2_size_get(sbEMLCtx_p cstate, uint32 n)
{
                          ...
                          sbEMLPackedKeyPayload_p_t pkp)
{
    ...
    sal_memcpy(((uint8 *) pkp) + kbytes, p, emlc->psize);
}
                                 ...
                                 sbFe2000DmaSlab_t *slab)
{
    ...
    for (i = 0; i < emlc->nbanks; i++) {
        ...
        for (j = 0; j < b->nchunks; j++) {
            c = &b->chunks[j];
            sal_memcpy(((uint8 *) slab->data) + c->boffset,
                       ((uint8 *) e->payload) + c->offset,
                       c->size);
        }
                              ...
                              sbFe2000DmaSlab_t *slab)
{
    ...
    soc_sbx_g2p3_eml_comp_bank_t *b;
    soc_sbx_g2p3_eml_comp_bank_chunk_t *c;
    uint8 *dps[SOC_SBX_G2P3_EML_MAX_BANKS];
    int i, j, npay, off;
    uint32 addr, offset;
                          ...
                          int dontagepos, int align)
{
     ...
     * an entire payload (rather than this wacky #banks * payload
