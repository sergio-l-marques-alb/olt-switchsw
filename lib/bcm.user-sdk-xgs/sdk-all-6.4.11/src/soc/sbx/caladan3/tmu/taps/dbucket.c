					     ...
					     taps_dprefix_handle_t dph)
{
    ...
    rv = taps_command_enqueue_for_all_devices(unit, taps, wgroup, TAPS_DBUCKET_WORK, dpld_cmd);
    if (SOC_FAILURE(rv)) {
	LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META_U(unit,
                        ...
                        taps_dbucket_handle_t *dbh)
{
	...
	for (index = 0; index < taps->param.dbucket_attr.num_dbucket_pfx * 2; index++) {
	    rv = taps_dbucket_entry_debug_init(unit, taps, domain, bucket, index);
	}
    }
