...
_g2xx_isr(void *_unit, uint32 ireg)
{
    ...
    if ( interrupt & (SAND_HAL_FE2000_FIELD_MASK(sbx->sbhdl, PC_INTERRUPT_MASK, DMA_0_DONE_DISINT)
        ...
        LOG_INFO(BSL_LS_SOC_INTR,
                 (BSL_META_U(unit,
                             "%d:g2xx_isr: calling sbFe2000DmaIsr\n"),
                  unit));
...
_soc_sbx_g2xx_commons_init(int unit)
{
	...
	master_unit = SOC_SBX_MASTER(unit);
	assert(SOC_CONTROL(master_unit));
	assert(SOC_SBX_CONTROL(master_unit));
	master_fe = SOC_SBX_CONTROL(master_unit)->drv;
	assert(master_fe);
	assert(master_fe->pDmaMgr);
	BCM_SBX_IF_ERROR_RETURN(sbFe2000DmaInitAddSlave(master_unit, master_fe->pDmaMgr,
							unit, fe->pDmaMgr));
    }
        ...
        if (ucodestr){
            if ( (sal_strlen(ucodestr) == sal_strlen("c2_g2xx")) && (sal_strcmp(ucodestr, "c2_g2xx") == 0) ) {
                ...
                LOG_CLI((BSL_META_U(unit,
                                    "soc_sbx_fe2000xt_init() Unsupported ucode detected\n")));
                return SOC_E_FAIL;
            }
        }
    } else {
