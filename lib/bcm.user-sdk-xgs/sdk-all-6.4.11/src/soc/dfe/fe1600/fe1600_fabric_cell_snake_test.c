    ...
    if (!(flags & SOC_DFE_ENABLE_MAC_LOOPBACK) && !(flags & SOC_DFE_ENABLE_ASYNC_FIFO_LOOPBACK) ) 
    {
        ...
        if(!(flags & SOC_DFE_DONT_TOUCH_MAC_INTERRUPTS)) {
            for(p = 0 ; p < SOC_DFE_DEFS_GET(unit, nof_links) ; p++) {            
                ...
                SOCDNX_IF_ERR_EXIT(WRITE_FMAC_INTERRUPT_MASK_REGISTERr(unit,blk_id,0x0));
        ...
        if(!(flags & SOC_DFE_DONT_TOUCH_MAC_INTERRUPTS)) {
            for(p = 0 ; p < SOC_DFE_DEFS_GET(unit, nof_links) ; p++) {                
                blk_id = INT_DEVIDE(p, 4);
                /* first Clear */
                SOCDNX_IF_ERR_EXIT(WRITE_FMAC_INTERRUPT_REGISTERr(unit,blk_id,0xFFFFFFFF));
                ...
                SOCDNX_IF_ERR_EXIT(WRITE_FMAC_INTERRUPT_MASK_REGISTERr(unit,blk_id,0x181));
...
soc_fe1600_fabric_cell_snake_test_bcm887540_links_check(int unit, int reset_unstable_links)
{
    ...
    if (!reset_unstable_links)
    {
     ...
     {
        SOCDNX_IF_ERR_EXIT(WRITE_FMAC_INTERRUPT_REGISTERr(unit, fmac_index, -1));
    ...
    while (recheck_links)
    {
        ...
        {
            reset_links = 0;
            SOCDNX_IF_ERR_EXIT(READ_FMAC_INTERRUPT_REGISTERr(unit, fmac_index, &reg_val));
            if (reg_val != 0)
            {
                reset_links = 1;
            }
