/* $Id: ppc_api_frwrd_ipv6.c,v 1.11 Broadcom SDK $
...
/******************************************************************
*
* FILENAME:       DuneDriver/ppc/src/soc_ppc_api_frwrd_ipv6.c
*
  ...
  )
{
  ...
  sal_memset(info, 0x0, sizeof(SOC_PPC_FRWRD_IPV6_GLBL_INFO));
  ...
  )
{
  ...
  soc_sand_SAND_PP_IPV6_ADDRESS_clear(&(info->group));
  ...
  )
{
  ...
  LOG_CLI((BSL_META_U(unit,
                      "group:")));
  soc_sand_SAND_PP_IPV6_ADDRESS_print(&(info->group));
  if (info->inrif.mask)
  {
    LOG_CLI((BSL_META_U(unit,
