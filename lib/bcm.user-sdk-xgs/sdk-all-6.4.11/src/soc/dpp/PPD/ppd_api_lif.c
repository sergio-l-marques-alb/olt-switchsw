  ...
  )
{
  ...
  SOC_PPD_ARAD_TMP_DEVICE_CALL(l2_lif_pwe_add,(unit, in_vc_label, lif_index, pwe_additional_info, pwe_info, success));
  ...
  )
{
  ...
  SOC_PPD_ARAD_TMP_DEVICE_CALL(l2_lif_pwe_get,(unit, in_vc_label, lif_index, pwe_additional_info, pwe_info, found));
  ...
  )
{
  ...
  if (SOC_IS_PETRAB(unit)) {
      SOC_PB_PP_DEVICE_CALL(l2_lif_pwe_remove,(unit, in_vc_label, remove_from_sem, lif_index, pwe_additional_info));
  }
  else {
      SOC_PPD_ARAD_ONLY_DEVICE_CALL(l2_lif_pwe_remove,(unit, in_vc_label, remove_from_sem, is_gal, lif_index, pwe_additional_info));
  }
 ...
 *     Bind the extender-key to the LIF, and set the LIF attributes
 *     Details: in the H file. (search for prototype)
*********************************************************************/
    ...
    SOC_SAND_OUT SOC_SAND_SUCCESS_FAILURE           *success)
{
    ...
    SOC_PPD_ARAD_ONLY_DEVICE_CALL(l2_lif_extender_add,(unit, extender_key, lif_index, extender_info, success));

    SOC_PPD_DO_NOTHING_AND_EXIT;
exit:
  SOC_PPD_FUNC_PRINT(soc_ppd_l2_lif_extender_add_print,(unit, extender_key, lif_index, extender_info));
  ...
  )
{
  ...
  SOC_PPC_L2_LIF_PWE_ADDITIONAL_INFO_clear(info);
exit:
  SOC_SAND_VOID_EXIT_AND_SEND_ERROR(0, 0, 0);
}
    ...
    SOC_SAND_IN  SOC_PPD_L2_LIF_EXTENDER_INFO       *extender_info)
{
  ...
  LOG_CLI((BSL_META_U(unit, "extender_key:")));
  SOC_PPC_L2_LIF_EXTENDER_KEY_print((extender_key));

  LOG_CLI((BSL_META_U(unit, "lif_index: %lu\n\r"), lif_index));

  LOG_CLI((BSL_META_U(unit, "extender_info:")));
  SOC_PPC_L2_LIF_EXTENDER_INFO_print((extender_info));
