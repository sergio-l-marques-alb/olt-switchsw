...
#define SOC_PPD_API_TRAP_MGMT_FTMH_TRAFFIC_CLASS_MSB   (SOC_PPD_API_TRAP_MGMT_FTMH_MSB - 31)
#define SOC_PPD_API_TRAP_MGMT_FTMH_TRAFFIC_CLASS_NOF_BITS   (3)
...
/* coverity[uninit_use_in_call] */
      ARAD_PP_DEVICE_CALL(trap_packet_parse_unsafe,(unit, buff, buff_len,&packet_parse_info,packet_info,(soc_pkt_t *)dnx_pkt));
      goto exit_semaphore;
  }
  ...
  curr_dnx_hdr = &(dnx_pkt->dnx_header_stack[dnx_pkt->dnx_header_count]);
  ...
  curr_dnx_hdr->ftmh.packet_size = fld_val;

  /* Traffic class*/
  res = soc_sand_bitsteam_u8_ms_byte_first_get_field(
          unit,
          &hdr_buff[header_ptr],
          SOC_PPD_API_TRAP_MGMT_FTMH_TRAFFIC_CLASS_MSB,
          SOC_PPD_API_TRAP_MGMT_FTMH_TRAFFIC_CLASS_NOF_BITS,
          &fld_val
        );

  curr_dnx_hdr->ftmh.prio = fld_val;
  /* action type */
  res = soc_sand_bitsteam_u8_ms_byte_first_get_field(
          unit,
          &hdr_buff[header_ptr],
  ...
  case 0:
      packet_info->frwrd_type = SOC_PPD_TRAP_MGMT_PKT_FRWRD_TYPE_NORMAL;
      curr_dnx_hdr->ftmh.action_type = bcmPktDnxFtmhActionTypeForward;
  break;
  case 1:
      packet_info->frwrd_type = SOC_PPD_TRAP_MGMT_PKT_FRWRD_TYPE_SNOOP;
      curr_dnx_hdr->ftmh.action_type = bcmPktDnxFtmhActionTypeSnoop;
  break;
  case 2:
      packet_info->frwrd_type = SOC_PPD_TRAP_MGMT_PKT_FRWRD_TYPE_IN_MIRROR;
      curr_dnx_hdr->ftmh.action_type = bcmPktDnxFtmhActionTypeInboundMirror;
  break;
  case 3:
      curr_dnx_hdr->ftmh.action_type = bcmPktDnxFtmhActionTypeOutboundMirror;
  break;
  }
  ...
  curr_dnx_hdr->ftmh.src_sysport = fld_val;
  ...
  switch (packet_info->frwrd_type) {
     case SOC_PPD_TRAP_MGMT_PKT_FRWRD_TYPE_SNOOP:
     case SOC_PPD_TRAP_MGMT_PKT_FRWRD_TYPE_IN_MIRROR:
         break;
     default:
  ...
  if (packet_parse_info.internal_eep_ext)
  {
...
void
  SOC_PPD_TRAP_ETPP_INFO_clear(
    SOC_SAND_OUT SOC_PPD_TRAP_ETPP_INFO *info
  )
{
  SOC_SAND_INIT_ERROR_DEFINITIONS_NO_DEVID(0);
  SOC_SAND_CHECK_NULL_INPUT(info);

  SOC_PPC_TRAP_ETPP_INFO_clear(info);
exit:
  SOC_SAND_VOID_EXIT_AND_SEND_ERROR(0, 0, 0);
}
