...
int soc_jer_egr_lcm_above_sum_get(int unit, uint32 *numbers, uint32 nof_numbers, uint32 sum, uint32 *lcm){
    uint32 *curr_iter = NULL, current_lcm = 0, smallest_idx = 0;

    SOCDNX_INIT_FUNC_DEFS;
    SOCDNX_ALLOC(curr_iter, uint32, nof_numbers, "LCM workspace");
  ...
  )
{
    ...
    uint32 if_rate_mbps, if_rate_gbps, total_gig_sum = 0, total_lanes_sum = 0, lane_rate;
    soc_pbmp_t port_bm;
    ...
    uint32 rates[SOC_PBMP_PORT_MAX] = {0}, lanes[SOC_PBMP_PORT_MAX] = {0}, lcm_temp[SOC_PBMP_PORT_MAX] = {0};
    ...
    uint32 lanes_temp_oamp = 1, lanes_temp_rcy = 1;
    ...
    SOC_PBMP_ITER(port_bm, port) {
      ...
      if (SOC_DPP_CONFIG(unit)->arad->init.fqp_pqp_new_algo_enable) {  
        if (!(is_special_int)) 
        {
            /* get number of lanes */
            ...
            SOCDNX_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_qsgmii_offsets_remove, (unit, first_lane, &shifted_lane)));
            lane_rate = if_rate_mbps / num_of_lanes;

            if (shifted_lane < 25 && lane_rate > 20000) { /* falcon*/
    ...
    if(cal_to_update == socJerCalTypeFQP) { /*FQP*/
        ...
        */
        ...
        /* Replace dummies with species IFs */
