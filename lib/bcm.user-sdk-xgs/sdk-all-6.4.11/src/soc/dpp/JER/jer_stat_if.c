...
uint32 soc_jer_stat_if_init_set(int unit, ARAD_INIT_STAT_IF *stat_if)
{
    ...
    SOCDNX_IF_ERR_EXIT(soc_jer_stat_if_report_info_set(unit, nof_sif_ports, stat_if));
...
uint32 soc_jer_stat_if_report_info_set(int unit, int nof_sif_ports, ARAD_INIT_STAT_IF *stat_if)
{
    ...
    switch (stat_if->stat_if_info[0].mode)
    {
        ...
        case SOC_TMC_STAT_IF_REPORT_QSIZE:
            ...
            /* set report size */
            switch (stat_if->stat_if_info[0].report_size)
            {
                case SOC_TMC_STAT_IF_REPORT_SIZE_61b:
                    field_val = 1;
                    break;
                case SOC_TMC_STAT_IF_REPORT_SIZE_64b:
                    field_val = 0;
                    break;
                default:
    ...
    /* Config port 1, if needed, in an indentical way */
    if (nof_sif_ports == 2)
    {
        /* Config SIF port 1 */
        /* Verify valid info */
        ...
        switch (stat_if->stat_if_info[1].mode)
        {
            ...
            case SOC_TMC_STAT_IF_REPORT_QSIZE:
                ...
                /* set report size */
                switch (stat_if->stat_if_info[1].report_size)
                {
                    case SOC_TMC_STAT_IF_REPORT_SIZE_61b:
                        field_val = 1;
                        break;
                ...
                /* set min-max queue */
                SOCDNX_IF_ERR_EXIT(_jer_stat_if_set_queue_range(unit, 1, 0, stat_if->stat_if_info[0].if_qsize_queue_min, stat_if->stat_if_info[0].if_qsize_queue_max));
                break;
            default:
                SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG("Invalid statistics interface mode %d "), stat_if->stat_if_info[0].mode));
                break;
