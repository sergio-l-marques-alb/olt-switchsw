...
#define SOC_JER_NIF_ILKN_PML_INIT_ALL   -1
...
STATIC int soc_jer_portmod_ilkn_pml_init(int unit, int pml_index);
...
soc_jer_portmod_pml_init(int unit)
{
    ...
    int first_phy, pm_idx, idx, otp_bits, qsgmii_index, pml_index;
    ...
    pml_index = SOC_JER_NIF_IS_ILKN_OVER_FABRIC_ENABLED(unit) ? 0 : SOC_JER_NIF_ILKN_PML_INIT_ALL; 
    SOCDNX_IF_ERR_EXIT(soc_jer_portmod_ilkn_pml_init(unit, pml_index));
...
STATIC int
soc_jer_portmod_ilkn_pml_init(int unit, int pml_index) {
    ...
    int nof_fabric_ilkn_pms, first_pm, lane, first_phy, i;
    ...
    for(pml = 0; pml < SOC_QMX_NUM_OF_PMLS; ++pml) {
        ...
        if (pml_index != SOC_JER_NIF_ILKN_PML_INIT_ALL && pml != pml_index) {
            continue;
        }
        /* build ILKN PM info */
        ...
        for(pm = first_pm; pm < first_pm + MAX_NUM_OF_PMS_IN_ILKN; ++pm, ++fab_pm) {
            ...
            if (pm_over_fabric) {
                ...
                first_phy = SOC_JER_PML_BASE_LANE + pm * NUM_OF_LANES_IN_PM + 1;
...
soc_jer_portmod_fabric_init(int unit)
{
    ...
    if (SOC_JER_NIF_IS_ILKN_OVER_FABRIC_ENABLED(unit)) {
        SOCDNX_IF_ERR_EXIT(soc_jer_portmod_ilkn_pml_init(unit, 1));
    }
exit:
    SOCDNX_FUNC_RETURN;
}
...
soc_jer_portmod_register_external_phys(int unit)
{
    ...
    for (port_i = 0; port_i < SOC_MAX_NUM_PORTS; ++port_i) {
        ...
        SOC_PBMP_ITER(phy_ports, tmp_xphy_physical_port) {
            ...
            xphy_lane_connection.ss_lane = (xphy_physical_port-1) % 4;
            ...
            if (!IS_SFI_PORT(unit, port)) {
                ...
                if(is_master) {
                    ...
                    if (interface_type != SOC_PORT_IF_ILKN) {
                        ...
                        SOCDNX_IF_ERR_EXIT(soc_jer_port_ilkn_bypass_interface_enable(unit, port, 1));
                    }
                }
            }
        }
...
soc_jer_portmod_probe(int unit, pbmp_t pbmp, pbmp_t *okay_pbmp, int is_init_sequence)
{
    ...
    soc_pbmp_t phys, counter_pbmp,pbmp_nif;
    ...
    if (SOC_WARM_BOOT(unit)) {
       ...
       if(broadcast_load_fabric) {
        ...
        if (is_init_sequence) {
            /* Harinij: Iterate for each port if it has an external phy. Only if it has an external phy - invoke portmod_legacy_ext_phy_init on it */
            SOC_PBMP_ASSIGN(pbmp_nif, *okay_pbmp);
            SOC_PBMP_REMOVE(pbmp_nif, PBMP_SFI_ALL(unit));
            /* check if the port is legacy */
            SOC_PBMP_CLEAR(legacy_pbmp);

            PBMP_ITER(pbmp_nif, port) {
                SOCDNX_IF_ERR_EXIT(soc_port_sw_db_is_master_get(unit, port, &is_master));
                if (!is_master) {
                    continue;
                }
            ...
            SOCDNX_IF_ERR_EXIT(portmod_legacy_ext_phy_init(unit, legacy_pbmp));
        }
...
soc_jer_portmod_port_enable_set(int unit, soc_port_t port, uint32 mac_only, int enable)
{
    ...
    if (IS_IL_PORT(unit, port)) {
        SOCDNX_IF_ERR_EXIT(soc_jer_port_ilkn_bypass_interface_enable(unit, port, 0)); 
    }
...
soc_jer_port_open_nbi_path(int unit, int port) {
    ...
    int pm_index = 0, reg_port, qmlf_index, core;
    ...
    reg_port = (first_phy < SOC_JERICHO_NOF_PORTS_NBIH) ? REG_PORT_ANY : (first_phy / (SOC_JERICHO_NOF_PORTS_NBIH + SOC_JERICHO_NOF_PORTS_EACH_NBIL));
    pm_index = (first_lane % SOC_JERICHO_NOF_PORTS_NBIH) / NUM_OF_LANES_IN_PM; /*internal PM index inside NBIH/L (0-5)*/
...
soc_jer_port_nbi_ports_rstn(int unit, soc_port_t port, int enable){
    ...
    if (first_phy < SOC_JERICHO_NOF_PORTS_NBIH) {
        ...
        *nbih_ports_rstn = soc_reg_field_get(unit, NBIH_TX_PORTS_SRSTNr, reg_val, TX_PORTS_SRSTNf);
        ...
        COMPILER_64_BITCLR(nbil_ports_rstn, first_phy - SOC_JERICHO_NOF_PORTS_NBIH - (index*SOC_JERICHO_NOF_PORTS_EACH_NBIL));
        ...
        if (enable) {
            COMPILER_64_BITSET(nbil_ports_rstn, first_phy - SOC_JERICHO_NOF_PORTS_NBIH - (index * SOC_JERICHO_NOF_PORTS_EACH_NBIL)); 
            soc_reg64_field_set(unit, NBIL_RX_PORTS_SRSTNr, &reg64_val, RX_PORTS_SRSTNf, nbil_ports_rstn);
            SOCDNX_IF_ERR_EXIT(WRITE_NBIL_RX_PORTS_SRSTNr(unit, index, reg64_val));
        }
...
soc_jer_port_open_path(int unit, soc_port_t port) {
    ...
    if (is_master) {
        ...
        if (SOC_PORT_IF_ILKN == interface_type) {
            ...
            SOCDNX_IF_ERR_EXIT( soc_jer_port_sch_config(unit, port));
        }
    }
...
int soc_jer_port_prd_enable_set(int unit, soc_port_t port, uint32 flags, int enable)
{
    ...
    nof_lanes_nbi = SOC_JERICHO_NOF_LANES_PER_NBI;
    rv = sw_state_access[unit].dpp.soc.arad.tm.logical_ports_info.header_type_out.get(unit, port, &hdr_type);
    SOCDNX_IF_ERR_EXIT(rv);
    SOCDNX_IF_ERR_EXIT(soc_port_sw_db_is_hg_get(unit, port, &is_hg));
