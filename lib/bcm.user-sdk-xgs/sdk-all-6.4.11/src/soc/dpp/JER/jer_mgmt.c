        ...
        *fap_bitmap = soc_mem_field32_get(unit, IPST_CRVSm, &data, CR_VAL_BMPf);
        ...
        if (credit_value_type == SOC_TMC_FAP_CREDIT_VALUE_LOCAL) {
            if (remote_bit != SOC_TMC_FAP_CREDIT_VALUE_LOCAL) {
                ...
                --nof_remote_faps_with_remote_credit_value; /*Need to add support in WarmBot*/
            }
...
int jer_mgmt_nof_block_instances(int unit, soc_block_types_t block_types, int *nof_block_instances) 
{
    ...
    *nof_block_instances = 0;
    ...
    /* for new blocks apear in QAX*/
    if (*nof_block_instances == 0 && SOC_IS_QAX(unit)) {
        switch(block_types[0]) {

            case SOC_BLK_ILB:    
                *nof_block_instances = SOC_JER_NOF_INSTANCES_ILB;
                break;
            case SOC_BLK_IEP:    
                *nof_block_instances = SOC_JER_NOF_INSTANCES_IEP;
                break;
            case SOC_BLK_IMP:    
                *nof_block_instances = SOC_JER_NOF_INSTANCES_IMP;
                break;
            case SOC_BLK_SPB:    
                *nof_block_instances = SOC_JER_NOF_INSTANCES_SPB;
                break;
            case SOC_BLK_ITE:    
                *nof_block_instances = SOC_JER_NOF_INSTANCES_ITE;
                break;
            case SOC_BLK_DDP:    
                *nof_block_instances = SOC_JER_NOF_INSTANCES_DDP;
                break;
            case SOC_BLK_TXQ:    
                *nof_block_instances = SOC_JER_NOF_INSTANCES_TXQ;
                break;
            case SOC_BLK_TAR:    
                *nof_block_instances = SOC_JER_NOF_INSTANCES_TAR;
                break;
            case SOC_BLK_PTS:    
                *nof_block_instances = SOC_JER_NOF_INSTANCES_PTS;
                break;
            case SOC_BLK_SQM:    
                *nof_block_instances = SOC_JER_NOF_INSTANCES_SQM;
                break;
            case SOC_BLK_IPSEC_SPU_WRAPPER_TOP:    
                *nof_block_instances = SOC_JER_NOF_INSTANCES_IPSEC_SPU_WRAPPER_TOP;
                break;
            case SOC_BLK_IPSEC:    
                *nof_block_instances = SOC_JER_NOF_INSTANCES_IPSEC;
                break;
            case SOC_BLK_DQM:    
                *nof_block_instances = SOC_JER_NOF_INSTANCES_DQM;
                break;
            case SOC_BLK_ECGM:    
                *nof_block_instances = SOC_JER_NOF_INSTANCES_ECGM;
                break;
            case SOC_BLK_GCK:    
                *nof_block_instances = SOC_JER_NOF_INSTANCES_GCK;
                break;
            case SOC_BLK_IDB:    
                *nof_block_instances = SOC_JER_NOF_INSTANCES_IDB;
                break;
            case SOC_BLK_PEM:    
                *nof_block_instances = SOC_JER_NOF_INSTANCES_PEM;
                break;
        }
    }    
...
int jer_mgmt_revision_fixes (int unit)
{
    ...
    for (core_index = 0; core_index < JER_MGMT_NOF_CORES; ++core_index) {
        ...
        if (SOC_IS_JERICHO_B0_AND_ABOVE(unit)) {
             ...
             */
            SOCDNX_IF_ERR_EXIT(soc_reg32_set(unit, EPNI_COUNTER_RESOLUTION_FIX_ENABLEDr, core_index,  0/*index*/,  0/*data*/));
  ...
  )
{
    ...
    soc_IDR_CONTEXT_MRUm_field32_set(unit, entry, MAX_ORG_SIZEf, mru);
    soc_IDR_CONTEXT_MRUm_field32_set(unit, entry, MAX_SIZEf, mru);
    soc_IDR_CONTEXT_MRUm_field32_set(unit, entry, MIN_ORG_SIZEf, 0x20);
    soc_IDR_CONTEXT_MRUm_field32_set(unit, entry, MIN_SIZEf, 0x20);

    SOCDNX_SAND_IF_ERR_EXIT(arad_fill_table_with_entry(unit, IDR_CONTEXT_MRUm, MEM_BLOCK_ANY, entry));
