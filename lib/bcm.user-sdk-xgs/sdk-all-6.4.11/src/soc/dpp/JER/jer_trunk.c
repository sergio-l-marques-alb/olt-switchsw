...
int soc_jer_trunk_init_tables (int unit)
{
    ...
    {
        SOCDNX_IF_ERR_EXIT(arad_fill_table_with_variable_values_by_caching(unit, IRR_SMOOTH_DIVISIONm, core_index, MEM_BLOCK_ALL, -1, -1, _arad_mgmt_irr_tbls_init_dma_callback, NULL));
    }
...
int soc_jer_trunk_resolve_lb_key_ext_mode(int unit, ARAD_INIT_FABRIC *fabric)
{
    ...
    if(fabric->ftmh_lb_ext_mode == ARAD_MGMT_FTMH_LB_EXT_MODE_STANDBY_MC_LB) 
    {
        ...
        SOCDNX_IF_ERR_EXIT(soc_reg_above_64_field32_modify(unit, IHB_LBP_GENERAL_CONFIG_0r, SOC_CORE_ALL, 0, SWAP_LB_KEY_MSBS_ENABLEf, 1));
