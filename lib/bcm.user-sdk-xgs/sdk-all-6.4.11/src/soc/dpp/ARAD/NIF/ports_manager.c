/*
 * $Id: ports_manager.c,v 1.38 Broadcom SDK $
 *
...
soc_pm_serdes_ports_in_quad_get(int unit, uint32 quad, soc_pbmp_t *ports_in_quad) 
{
    ...
    /* This function relevant for Arad only */
...
soc_pm_last_initialized_in_serdes_quads_get(int unit, soc_port_t port, uint32 *last_in_use) 
{
    ...
    *last_in_use = 1;
    ...
    SOC_PBMP_ITER(valid_ports, port_i) {
        ...
        /*if same port (different channel)*/
        ...
        if (SOC_PBMP_NOT_NULL(quads_in_use_i)) {
            /*found other initialized port on the same quad*/
            *last_in_use = 0;
            break;
        }
    }
...
/*
 * Function:
 *      soc_pm_mac_ability_get
 * Purpose:
 *      get mac ability of specific port
 * Parameters:
 *      unit        - (IN) Unit number.
 *      port        - (IN) Port Number.
 *      mac_ability - (OUT) mac_ability structure
 * Returns:
 *      SOC_E_xxx
 */
soc_error_t
...
/*
 * Function:
 *      soc_pm_tx_remote_fault_enable_set
 * Purpose:
 *      enable/disable transmission of remote fault on the tx.
 *      valid for CAUI ports only
 * Parameters:
 *      unit        - (IN) Unit number.
 *      port        - (IN) Port Number.
 *      enable      - (IN) enable /disable transmission
 * Returns:
 *      SOC_E_xxx
 */
...
/*
 * Function:
 *      soc_pm_tx_remote_fault_enable_get
 * Purpose:
 *      get if transmission of remote fault on the tx.
 *      valid for CAUI ports only
 * Parameters:
 *      unit        - (IN) Unit number.
 *      port        - (IN) Port Number.
 *      enable      - (OUT) enabled /disabled transmission
 * Returns:
 *      SOC_E_xxx
 */
soc_error_t
...
/*
 * Function:
 *      soc_pm_software_init
 * Purpose:
 *      Ports manager SW initialization
 * Parameters:
 *      unit                - (IN) Unit number.
 *      all_phy_pbmp        - (IN) ALL phy ports of the device (include such that aren't in use)
 *      first_direct_port   - (IN) First direct port number
 *                              (OR the first port that is not in use)
 * Returns:
 *      SOC_E_xxx
 */
soc_error_t 
soc_pm_init(int unit, soc_pbmp_t all_phy_pbmp, uint32 first_direct_port)
{
    ...
    * This mechanisem allows access to port blocks (CLP\XLP) 
    * also when no port is defined for these ports 
    */
    ...
    SOC_PBMP_ITER(all_phy_pbmp, phy_port) {
        ...
        /*This entry might be ovveride according to real ports topology*/
        SOC_INFO(unit).block_port[blk] = logical_port;
        
        /*fill physical bitmap*/
...
/*
 * Function:
 *      soc_pm_mac_reset_set
 * Purpose:
 *      Set MAC in\out of reset
 * Parameters:
 *      unit                - (IN) Unit number.
 *      port                - (IN) Port Number.
 *      is_in_reset         - (IN) 1-In reset, 0-Out of reset.
 * Returns:
 *      SOC_E_xxx
 * Remarks: 
 *      This operation will influence other ports in the same MAC 
 */
soc_error_t
soc_pm_mac_reset_set(int unit, soc_port_t port, uint32 is_in_reset)
{
    ...
    if(SOC_PORT_IF_TM_INTERNAL_PKT == interface) {
        ...
        /*ilkn XMAC\CMAC always in reset*/
        SOC_PBMP_ITER(quads_in_use, quad){
                ...
                if(SOC_IS_ARADPLUS(unit) && (offset == 1) && (SOC_DPP_CONFIG(unit)->arad->init.elk.ext_interface_mode)){
                    /*no CMAC reset in this case of special ILKN defined*/
                }
                else
#endif
                {
...
#if defined(INCLUDE_KBP) && !defined(BCM_88030) && defined(BCM_88660_A0)
                        if(SOC_IS_ARADPLUS(unit) && (offset == 1) && (SOC_DPP_CONFIG(unit)->arad->init.elk.ext_interface_mode)){
                            /* no XMAC2 reset in this case of special ILKN defined*/
                        }
                        else
#endif
                        {
        ...
        if(SOC_BLK_CLP == SOC_PORT_TYPE(unit,port) || SOC_BLK_XLP == SOC_PORT_TYPE(unit,port)) {
            ...
            switch(internal_quad) {
                ...
                case 2:
                    ...
                    break;
                /* must have deault. Overwize, compilation error */
 ...
 * Parameters:
 *      unit                - (IN) Unit number.
 *      pbmp                - (IN) Ports to init.
 *      okay_ports          - (OUT) Ports succeeded
 * Returns:
 *      SOC_E_xxx
 */
soc_error_t
soc_pm_ports_init(int unit, soc_pbmp_t pbmp, soc_pbmp_t* okay_ports)
{
    ...
    SOC_PBMP_ITER(pbmp, port){
        ...
        /*validate all ports aren't initialized*/
        SOCDNX_IF_ERR_EXIT(soc_port_sw_db_is_initialized_get(unit, port, &is_initialized));
        if(is_initialized && !SOC_WARM_BOOT(unit) /*not need to init port sw db in warm boot*/) {
            ...
            SOC_PBMP_ITER(quads_in_use, quad){
                if(!SOC_PBMP_MEMBER(quads_oor, quad)) {
                    ...
                    if(SOC_PORT_IF_ILKN != interface){
                        /* We must always reset the first WC * in the block */
    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        SOC_PBMP_ITER(*okay_ports, port) {
            ...
            if (interface_type == SOC_PORT_IF_RXAUI || interface_type == SOC_PORT_IF_XAUI) {
                /*DFE must be turned off for XAUI and RXAUI*/
    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        SOC_PBMP_ITER(*okay_ports, port) {
            ...
            /*Make sure MAC block isn't in reset*/
            SOCDNX_IF_ERR_EXIT(soc_pm_mac_reset_set(unit, port, 0 /*out of reset*/));
        }
...
soc_pm_port_deinit(int unit, soc_port_t port)
{
    ...
    while (((interface_type == SOC_PORT_IF_CAUI) || (interface_type == SOC_PORT_IF_ILKN))) {
        ...
        SOCDNX_IF_ERR_EXIT(WRITE_PORT_MLD_CTRL_REGr(unit, SOC_BLOCK_PORT(unit, CLP_BLOCK(unit, clp)) /*CLP0/CLP1*/, reg32_val));
        sal_sleep(1);
        reg32_val = 0;
...
soc_pm_speed_set(int unit, soc_port_t port, int speed)
{
    ...
    /*If port is initialized - update HW*/
    SOCDNX_IF_ERR_EXIT(soc_port_sw_db_is_initialized_get(unit, port, &is_initialized));
    SOCDNX_IF_ERR_EXIT(soc_port_sw_db_interface_type_get(unit, port, &interface_type));
...
/*
 * Function:
 *      soc_pm_mac_enable_set
 * Purpose:
 *      Enable\Disable port's mac
 * Parameters:
 *      unit                - (IN) Unit number.
 *      port                - (IN) Port Number.
 *      enable              - (IN) Enable\Disable ports mac.
 * Returns:
 *      SOC_E_xxx
 */
soc_error_t 
...
/*
 * Function:
 *      soc_pm_mac_speed_set
 * Purpose:
 *      Set the Mac speed according to the port speed
 * Parameters:
 *      unit                - (IN) Unit number.
 *      port                - (IN) Port Number.
 *      speed               - (IN) The port speed.
 * Returns:
 *      SOC_E_xxx
 */
soc_error_t 
...
soc_pm_mac_loopback_set(int unit, soc_port_t port, uint32 lb)
{
    ...
    /*check if first port in quad in loopback*/
    ...
    /*reset loopback fifo if first port in quad in loopback*/
...
soc_error_t 
soc_pm_pfc_set(int unit, soc_port_t port, int is_rx /*1 for rx, 0 tx*/, int pfc)
{
    CONST mac_driver_t* md;
...
soc_error_t 
soc_pm_pfc_get(int unit, soc_port_t port, int is_rx /*1 for rx, 0 tx*/, int *pfc)
{
    CONST mac_driver_t* md;
    soc_mac_control_t control;
    SOCDNX_INIT_FUNC_DEFS;
...
soc_error_t 
soc_pm_llfc_set(int unit, soc_port_t port, int is_rx /*1 for rx, 0 tx*/, int llfc)
{
    CONST mac_driver_t* md;
    soc_mac_control_t control;
    SOCDNX_INIT_FUNC_DEFS;
...
soc_error_t 
soc_pm_llfc_get(int unit, soc_port_t port, int is_rx /*1 for rx, 0 tx*/, int *llfc)
{
    CONST mac_driver_t* md;
    soc_mac_control_t control;
    SOCDNX_INIT_FUNC_DEFS;
