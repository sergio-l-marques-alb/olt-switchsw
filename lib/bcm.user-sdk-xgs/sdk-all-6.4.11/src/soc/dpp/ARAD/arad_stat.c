...
soc_arad_stat_path_info_get(int unit, soc_dpp_stat_path_info_t *info)
{
    ...
    if (found)
    {
        ...
        if (found)
        {
            ...
            if (!found)
            {
                info->drop = soc_dpp_stat_path_drop_stage_egress_tm; /*packets wasn't transmitted from egress tm to egress pp*/
            }
        }
        else
        {
            info->drop = soc_dpp_stat_path_drop_stage_ingress_tm; /* packets wasn't transmitted to fabric */
        }
    }
    else
    {
        info->drop = soc_dpp_stat_path_drop_stage_ingress_no_packet; /* packets wasn't entered to ingress*/
    }
