...
#define ARAD_PP_FLP_PROG_SEL_CAM_IPV4MC_BRIDGEV4MC         10
#define ARAD_PP_FLP_PROG_SEL_CAM_IPV4MC_BRIDGE             11
...
#define ARAD_PP_FLP_PROG_SEL_CAM_IPV6UC_2PASS_FWD          28 /* IPv6 UC with RPF 2PASS, prog for second pass (DIP), 
                                                               ...
                                                               */
#define ARAD_PP_FLP_PROG_SEL_CAM_ETH_ING_LRN_FOR_IVL       28
#define ARAD_PP_FLP_PROG_SEL_CAM_IPV6UC_2PASS              29 /* IPv6 UC with RPF 2PASS, prog for First Pass (SIP), 
                                                               ...
                                                               */
#define ARAD_PP_FLP_PROG_SEL_CAM_IPV4MC_BRIDGEV4MC_FOR_IVL 29
#define ARAD_PP_FLP_PROG_SEL_CAM_IPV6UC_RPF                30
#define ARAD_PP_FLP_PROG_SEL_CAM_IPV4UC_L3VPN_RPF          31
   ...
   )
{
  ...
  if(is_routing_enabled) {
      ...
      prog_status |= 1 << PROG_FLP_IPV4MC_BRIDGE;
  }
   ...
   )
{
  ...
  if (SOC_DPP_PP_ENABLE(unit)) {
      ...
      ARAD_PP_FLP_SW_DB_FLP_PROGRAM_ID_SET(unit, PROG_FLP_TM, PROG_FLP_TM);
  }
   ...
   )
{
    ...
    ARAD_PP_IHB_FLP_PROGRAM_SELECTION_CAM_TBL_DATA prog_selection_cam_tbl;
    uint32 i = 0;
    int service_type[4]      = {0, 2, 4, 7}; /*AC to PWE & PWE to PWE for b00X*/
    int service_type_mask[4] = {1, 0, 0, 0};
    int num_of_selections = 4;

    SOC_SAND_INIT_ERROR_DEFINITIONS(0);

    /* For static, program id and app id is same */
    ARAD_PP_FLP_SW_DB_FLP_PROGRAM_ID_SET(unit, PROG_FLP_P2P, PROG_FLP_P2P);

#if defined(INCLUDE_KBP) && !defined(BCM_88030)
/* when ARAD_KBP_ENABLE_P2P_EXTENDED is enabled, we will use only service type 000 and 001 (AC to PWE & PWE to PWE) */
   ...
   )
{
  ...
  for (i = 0; i < 2; i++) { /* DHCP, static */
    ...
    program_id = (flp_prog_sel == ARAD_PP_FLP_PROG_SEL_CAM_ETH_TK_EPON_UNI_V4_DHCP) ? 
          PROG_FLP_ETHERNET_TK_EPON_UNI_V4_DHCP:PROG_FLP_ETHERNET_TK_EPON_UNI_V4_STATIC;
   ...
   )
{
  ...
  for (i = 0; i < 2; i++) { /* DHCP, static */
    ...
    program_id = (flp_prog_sel == ARAD_PP_FLP_PROG_SEL_CAM_ETH_TK_EPON_UNI_V6_DHCP) ? 
          PROG_FLP_ETHERNET_TK_EPON_UNI_V6_DHCP:PROG_FLP_ETHERNET_TK_EPON_UNI_V6_STATIC;
    ARAD_PP_FLP_SW_DB_FLP_PROGRAM_ID_SET(unit, program_id, program_id);
   ...
   )
{
  ...
  res = arad_pp_flp_get_next_avaialable(unit, PROG_FLP_PON_ARP_DOWNSTREAM, &cam_sel, prog_id);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
   ...
   )
{
  ...
  res = arad_pp_flp_get_next_avaialable(unit, PROG_FLP_PON_ARP_UPSTREAM, &cam_sel, prog_id);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
   ...
   )
{
  ...
  ARAD_PP_IHB_FLP_PROGRAM_SELECTION_CAM_TBL_DATA
    prog_selection_cam_tbl;
  int cam_sel;
  uint32 selection_offset = ARAD_PP_FLP_PROG_SEL_CAM_ETH_ING_LRN;

  SOC_SAND_INIT_ERROR_DEFINITIONS(0);

  /* For static, program id and app id is same */
  ARAD_PP_FLP_SW_DB_FLP_PROGRAM_ID_SET(unit, PROG_FLP_ETHERNET_ING_LEARN, PROG_FLP_ETHERNET_ING_LEARN);

  if ((SOC_DPP_CONFIG(unit)->pp.ivl_inlif_profile == 1)) {
	selection_offset = ARAD_PP_FLP_PROG_SEL_CAM_ETH_ING_LRN_FOR_IVL;
  }
  
  res = arad_pp_ihb_flp_program_selection_cam_tbl_get_unsafe(unit, selection_offset, &prog_selection_cam_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  ...
  prog_selection_cam_tbl.valid = 1;
  res = arad_pp_ihb_flp_program_selection_cam_tbl_set_unsafe(unit, selection_offset, &prog_selection_cam_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);

  if (mac_in_mac_enabled) {

      res = arad_pp_flp_get_next_avaialable(unit,PROG_FLP_MAC_IN_MAC_AFTER_TERMINATIOM,&cam_sel, mac_in_mac_prog_id);
      SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
   ...
   )
{
    ...
    res = arad_pp_flp_get_next_avaialable(unit, PROG_FLP_ETHERNET_ING_IVL_LEARN, &cam_sel, prog_id);
...
/*  res = arad_pp_flp_get_next_avaialable(unit, PROG_FLP_ETHERNET_ING_IVL_INNER_LEARN, &cam_sel_1, prog_id);*/
/*  SOC_SAND_CHECK_FUNC_RESULT(res, 200, exit);                                                                                 */
    ...
    for (idx=0;idx<2;idx++) {
        ...
        */
        ...
        if (idx == 0) {
            res = arad_pp_flp_get_next_avaialable(unit, PROG_FLP_ETHERNET_ING_IVL_INNER_LEARN, &cam_sel, prog_id);
            SOC_SAND_CHECK_FUNC_RESULT(res, 200, exit);
        }
        else {
            res = arad_pp_flp_cam_table_next_avaialable_get(unit, &cam_sel);
...
/*  res = arad_pp_flp_get_next_avaialable(unit, PROG_FLP_ETHERNET_ING_IVL_FWD_OUTER_LEARN, &cam_sel_1, prog_id);*/
/*  SOC_SAND_CHECK_FUNC_RESULT(res, 200, exit);                                                                                 */
    ...
    for (idx=0;idx<2;idx++) {
        ...
        */
        ...
        if (idx == 0) {
            ...
            res = arad_pp_flp_get_next_avaialable(unit, PROG_FLP_ETHERNET_ING_IVL_FWD_OUTER_LEARN, &cam_sel, prog_id);
            SOC_SAND_CHECK_FUNC_RESULT(res, 200, exit);
        }
        else {
            res = arad_pp_flp_cam_table_next_avaialable_get(unit, &cam_sel);
   ...
   )
{
  ...
  res = arad_pp_flp_get_next_avaialable(unit,PROG_FLP_OAM_STATISTICS,&cam_sel,prog_id);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
   ...
   )
{
  ...
  res = arad_pp_flp_get_next_avaialable(unit,PROG_FLP_OAM_DOWN_UNTAGGED_STATISTICS,&cam_sel,prog_id);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
   ...
   )
{
  ...
  res = arad_pp_flp_get_next_avaialable(unit,PROG_FLP_OAM_SINGLE_TAG_STATISTICS,&cam_sel,prog_id);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
   ...
   )
{
  ...
  res = arad_pp_flp_get_next_avaialable(unit,PROG_FLP_OAM_DOUBLE_TAG_STATISTICS,&cam_sel,prog_id);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
   ...
   )
{
  ...
  res = arad_pp_flp_get_next_avaialable(unit,PROG_FLP_BFD_MPLS_STATISTICS,&cam_sel,prog_id);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
   ...
   )
{
  ...
  res = arad_pp_flp_get_next_avaialable(unit,PROG_FLP_BFD_PWE_STATISTICS,&cam_sel,prog_id);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
   ...
   )
{
  ...
  res = arad_pp_flp_get_next_avaialable(unit,PROG_FLP_BFD_STATISTICS,&cam_sel,prog_id);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
   ...
   )
{
  ...
  res = arad_pp_flp_get_next_avaialable(unit,PROG_FLP_ETHERNET_PON_DEFAULT_UPSTREAM,NULL,prog_id);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
   ...
   )
{
  ...
  res = arad_pp_flp_get_next_avaialable(unit,PROG_FLP_ETHERNET_PON_DEFAULT_DOWNSTREAM,&cam_sel,prog_id);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
   ...
   )
{
  ...
  res = arad_pp_flp_get_next_avaialable(unit,PROG_FLP_ETHERNET_PON_LOCAL_ROUTE,&cam_sel,prog_id);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  ...
  if (mac_in_mac_enabled) {
      int32 mim_prog_id;
      res = arad_pp_flp_get_next_avaialable(unit,PROG_FLP_MAC_IN_MAC_AFTER_TERMINATIOM,&cam_sel,&mim_prog_id);
      SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
   ...
   )
{
  ...
  if(is_fcoe_enabled){      
      res = arad_pp_flp_get_next_avaialable(unit,PROG_FLP_FC_TRANSIT,&cam_sel,prog_id);
      SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
   ...
   )
{
  ...
  res = arad_pp_flp_get_next_avaialable(unit,PROG_FLP_FC_WITH_VFT_REMOTE,&cam_sel,prog_id);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
   ...
   )
{
  ...
  res = arad_pp_flp_get_next_avaialable(unit,PROG_FLP_FC_REMOTE,&cam_sel,prog_id);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
   ...
   )
{
  ...
  /* For static, program id and app id is same */
  ARAD_PP_FLP_SW_DB_FLP_PROGRAM_ID_SET(unit, PROG_FLP_ETHERNET_MAC_IN_MAC, PROG_FLP_ETHERNET_MAC_IN_MAC);
   ...
   )
{
  ...
  /* For dynamic, program id and app id is allocated dynamically and used for the next stages in the program creation */
  res = arad_pp_flp_get_next_avaialable(unit, PROG_FLP_IPV4UC_PUBLIC_RPF, NULL, prog_id);

  res = arad_pp_ihb_flp_program_selection_cam_tbl_get_unsafe(unit, ARAD_PP_FLP_PROG_SEL_CAM_IPV4UC_L3VPN_RPF, &prog_selection_cam_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  ...
  res = arad_pp_ihb_flp_program_selection_cam_tbl_set_unsafe(unit, ARAD_PP_FLP_PROG_SEL_CAM_IPV4UC_L3VPN_RPF, &prog_selection_cam_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
exit:
   ...
   )
{
  ...
  /* For static, program id and app id is same */
  ARAD_PP_FLP_SW_DB_FLP_PROGRAM_ID_SET(unit, PROG_FLP_IPV4UC_PUBLIC, PROG_FLP_IPV4UC_PUBLIC);
   ...
   )
{
  ...
  /* For static, program id and app id is same */
  /* ARAD_PP_FLP_SW_DB_FLP_PROGRAM_ID_SET(unit, PROG_FLP_IPV6UC_WITH_RPF_2PASS, PROG_FLP_IPV6UC_WITH_RPF_2PASS); */
  res = arad_pp_flp_get_next_avaialable(unit,PROG_FLP_IPV6UC_WITH_RPF_2PASS,NULL,prog_id);
   ...
   )
{
  ...
  /* For static, program id and app id is same */
  ARAD_PP_FLP_SW_DB_FLP_PROGRAM_ID_SET(unit, PROG_FLP_IPV4UC_RPF, PROG_FLP_IPV4UC_RPF);
   ...
   )
{
  ...
  /* For static, program id and app id is same */
  ARAD_PP_FLP_SW_DB_FLP_PROGRAM_ID_SET(unit, PROG_FLP_IPV4UC, PROG_FLP_IPV4UC);
...
uint32 arad_pp_flp_prog_sel_cam_bfd_single_hop_bridge( int unit)
{
    ...
    prog_selection_cam_tbl.ptc_profile  = 0;
    prog_selection_cam_tbl.forwarding_code = ARAD_PP_FWD_CODE_ETHERNET;
    prog_selection_cam_tbl.parser_leaf_context_mask = 0x0; /*Should be ARAD_PMF_PGM_SEL_PARSER_PROFILE_BFD_SINGLE_HOP */
    prog_selection_cam_tbl.cos_profile_mask = 0x00;
    prog_selection_cam_tbl.cos_profile_mask = 0x3F;
    prog_selection_cam_tbl.port_profile_mask = ARAD_PP_FLP_PORT_PROFILE_MASK_CONSIDER_PBP;
    prog_selection_cam_tbl.ptc_profile_mask = 0x03;
   ...
   )
{
  ...
  /* For static, program id and app id is same */
  /* Alreday done: ARAD_PP_FLP_SW_DB_FLP_PROGRAM_ID_SET(unit, PROG_FLP_IPV6UC, PROG_FLP_IPV6UC); */
   ...
   )
{
  ...
  /* For dynamic, program id and app id is allocated dynamically and used for the next stages in the program creation */
  res = arad_pp_flp_get_next_avaialable(unit, PROG_FLP_IPV6UC_RPF, NULL, prog_id);

  res = arad_pp_ihb_flp_program_selection_cam_tbl_get_unsafe(unit, ARAD_PP_FLP_PROG_SEL_CAM_IPV6UC_RPF, &prog_selection_cam_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  ...
  prog_selection_cam_tbl.program                   = *prog_id;
  prog_selection_cam_tbl.valid                     = 1;
  res = arad_pp_ihb_flp_program_selection_cam_tbl_set_unsafe(unit, ARAD_PP_FLP_PROG_SEL_CAM_IPV6UC_RPF, &prog_selection_cam_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
exit:
   ...
   )
{
  ...
  /* For static, program id and app id is same */
  ARAD_PP_FLP_SW_DB_FLP_PROGRAM_ID_SET(unit, PROG_FLP_IPV6UC, PROG_FLP_IPV6UC);
   ...
   )
{
    ...
    /* program selection */
    /* allocate resources */
    res = arad_pp_flp_get_next_avaialable(unit,PROG_FLP_IPV6UC_RPF,&cam_sel,&prog_id);
    SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
   ...
   )
{
  ...
  /* For static, program id and app id is same */
  ARAD_PP_FLP_SW_DB_FLP_PROGRAM_ID_SET(unit, PROG_FLP_IPV6MC, PROG_FLP_IPV6MC);
   ...
   )
{
  ...
  res = arad_pp_flp_get_next_avaialable(unit,PROG_FLP_VPLSOGRE,&cam_sel,prog_id);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
   ...
   )
{
  ...
  /* For static, program id and app id is same */
  ARAD_PP_FLP_SW_DB_FLP_PROGRAM_ID_SET(unit, PROG_FLP_LSR, PROG_FLP_LSR);
   ...
   )
{
  ...
  /* For static, program id and app id is same */
  ARAD_PP_FLP_SW_DB_FLP_PROGRAM_ID_SET(unit, PROG_FLP_COUPLING_LSR, PROG_FLP_COUPLING_LSR);
  ...
  prog_selection_cam_tbl.program                    = PROG_FLP_COUPLING_LSR;
  prog_selection_cam_tbl.valid                      = 1;
   ...
   )
{
  ...
  /* For static, program id and app id is same */
  ARAD_PP_FLP_SW_DB_FLP_PROGRAM_ID_SET(unit, PROG_FLP_TRILL_UC, PROG_FLP_TRILL_UC);
   ...
   )
{
  ...
  /* For static, program id and app id is same */
  ARAD_PP_FLP_SW_DB_FLP_PROGRAM_ID_SET(unit, PROG_FLP_TRILL_MC_ONE_TAG, PROG_FLP_TRILL_MC_ONE_TAG);
  ...
  /* TWO tags (FGL) */
  /* For static, program id and app id is same */
  ARAD_PP_FLP_SW_DB_FLP_PROGRAM_ID_SET(unit, PROG_FLP_TRILL_MC_TWO_TAGS, PROG_FLP_TRILL_MC_TWO_TAGS);
   ...
   )
{
  ...
  /* For static, program id and app id is same */
  ARAD_PP_FLP_SW_DB_FLP_PROGRAM_ID_SET(unit, PROG_FLP_TRILL_AFTER_TERMINATION, PROG_FLP_TRILL_AFTER_TERMINATION);
   ...
   )
{
  ...
  res = arad_pp_flp_get_next_avaialable(unit, PROG_FLP_VMAC_UPSTREAM, &cam_sel, prog_id);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
   ...
   )
{
  ...
  res = arad_pp_flp_get_next_avaialable(unit, PROG_FLP_VMAC_DOWNSTREAM, &cam_sel, prog_id);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
   ...
   )
{
  ...
  for (i = 0; i < 2; i++) { /* DHCP, static */
    ...
    flp_lookups_tbl.lem_2nd_lkp_and_value  = (flp_prog_sel == PROG_FLP_ETHERNET_TK_EPON_UNI_V4_DHCP) ? 0x7:0x0;
    flp_lookups_tbl.lem_2nd_lkp_or_value   = (flp_prog_sel == PROG_FLP_ETHERNET_TK_EPON_UNI_V4_DHCP) ? ARAD_PP_FLP_IP_SPOOF_DHCP_KEY_OR_MASK:ARAD_PP_FLP_IPV4_SPOOF_STATIC_KEY_OR_MASK;
   ...
   )
{
  ...
  arad_pp_dbal_qualifier_to_instruction(unit,SOC_PPC_FP_DATABASE_STAGE_INGRESS_FLP, 0, 0, SOC_PPC_FP_QUAL_IRPP_VRF,0, &vrf_ce_inst);
  arad_pp_dbal_qualifier_to_instruction(unit,SOC_PPC_FP_DATABASE_STAGE_INGRESS_FLP, 0, 16, SOC_PPC_FP_QUAL_IRPP_IN_RIF,0, &rif_ce_inst);
  ...
  if(ARAD_KBP_ENABLE_IPV6_UC || ARAD_KBP_ENABLE_IPV6_EXTENDED){
      ...
      if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "inrif_mapping_ipv6_uc", 0)) { /* add inrif to key-c */
   ...
   )
{
...
#endif /* INCLUDE_KBP */
  {
      ...
      flp_key_construction_tbl.instruction_1_16b     = (in_port) ?
                                                        (SOC_IS_JERICHO(unit) ? pp_port_ce_inst : ARAD_PP_FLP_16B_INST_P6_IN_PORT_D):
   ...
   )
{
  ...
  ARAD_PP_IHB_FLP_KEY_CONSTRUCTION_TBL_DATA
    flp_key_construction_tbl;
  SOC_SAND_INIT_ERROR_DEFINITIONS(0);
  res = arad_pp_ihb_flp_key_construction_tbl_get_unsafe(unit, PROG_FLP_COUPLING_LSR, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
  ...
  flp_key_construction_tbl.key_b_inst_0_to_5_valid = 0x0 /*6'b000000*/;
  flp_key_construction_tbl.key_c_inst_0_to_5_valid = 0x0 /*6'b000000*/;
  res = arad_pp_ihb_flp_key_construction_tbl_set_unsafe(unit, PROG_FLP_COUPLING_LSR, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
  res = arad_pp_ihb_flp_key_construction_tbl_get_unsafe(unit, PROG_FLP_COUPLING_LSR+ARAD_PP_FLP_INSTRUCTIONS_NOF, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
  flp_key_construction_tbl.key_a_inst_0_to_5_valid = 0x0 /*6'b000000*/;
  flp_key_construction_tbl.key_b_inst_0_to_5_valid = 0x0 /*6'b000000*/;
  flp_key_construction_tbl.key_c_inst_0_to_5_valid = 0x0 /*6'b000000*/;
  res = arad_pp_ihb_flp_key_construction_tbl_set_unsafe(unit, PROG_FLP_COUPLING_LSR+ARAD_PP_FLP_INSTRUCTIONS_NOF, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
exit:
   ...
   )
{
  ...
  SOC_SAND_CHECK_FUNC_RESULT(res, 42, exit);
  {
      ...
      if(ARAD_KBP_ENABLE_IPV6_UC || ARAD_KBP_ENABLE_IPV6_EXTENDED){
          ...
          flp_lookups_tbl.elk_key_c_valid_bytes = 16; 
              if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "inrif_mapping_ipv6_uc", 0)) {/* in-rif added to rop */
                  flp_lookups_tbl.elk_key_c_valid_bytes = 18; 
              }
      }
    #endif
  }
   ...
   )
{
  ...
  ARAD_PP_IHB_FLP_LOOKUPS_TBL_DATA
    flp_lookups_tbl;
  SOC_SAND_INIT_ERROR_DEFINITIONS(0);
  res = arad_pp_ihb_flp_lookups_tbl_get_unsafe(unit, PROG_FLP_COUPLING_LSR, &flp_lookups_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 42, exit);
  flp_lookups_tbl.lem_2nd_lkp_valid     = 1;
  flp_lookups_tbl.lem_2nd_lkp_key_select = 0;
  flp_lookups_tbl.lem_2nd_lkp_and_value  = 0x0;
  ...
  flp_lookups_tbl.learn_key_select      = ARAD_PP_FLP_LKP_KEY_SELECT_FID_FWD_MAC_KEY_HW_VAL;
  res = arad_pp_ihb_flp_lookups_tbl_set_unsafe(unit, PROG_FLP_COUPLING_LSR, &flp_lookups_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 42, exit);
exit:
   ...
   )
{
  ...
  res = arad_pp_ihb_flp_process_tbl_get_unsafe(unit, PROG_FLP_IPV4UC_PUBLIC, &flp_process_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 41, exit);
  ...
  res = arad_pp_ihb_flp_process_tbl_set_unsafe(unit, PROG_FLP_IPV4UC_PUBLIC, &flp_process_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 41, exit);
  tmp = 1; /* 0: Apply Ethernet traps on forwarding-header 1: Apply IPv4 traps on forwarding-header and L4 Traps on the following header 2: Apply IPv6 traps on forwarding-header and L4 Traps on the following header 3: Apply MPLS traps on forwarding-header 4: Apply FC traps on forwarding-header Else: Don't apply any protocol traps */
  SOC_REG_ABOVE_64_CLEAR(reg_val);
  res = READ_IHP_PROTOCOL_TRAPS_PROGRAM_SELECTr(unit, 0, reg_val);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 40, exit);
  SHR_BITCOPY_RANGE(reg_val,3*PROG_FLP_IPV4UC_PUBLIC,&tmp,0,3);
  res = WRITE_IHP_PROTOCOL_TRAPS_PROGRAM_SELECTr(unit, SOC_CORE_ALL, reg_val);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 45, exit);
exit:
   ...
   )
{
  ...
  if (SOC_IS_JERICHO(unit) && !(soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "l3_mc_use_tcam", 0))) {
      flp_process_tbl.include_lpm_1st_in_result_b        = 1;
      flp_process_tbl.lpm_1st_lkp_enable_default         = 1;
      flp_process_tbl.lpm_public_1st_lkp_enable_default  = 1;
      flp_process_tbl.include_lpm_2nd_in_result_a        = 1;
   ...
   )
{
  ...
  ARAD_PP_IHB_FLP_PROCESS_TBL_DATA
    flp_process_tbl;
  SOC_SAND_INIT_ERROR_DEFINITIONS(0);
  res = arad_pp_ihb_flp_process_tbl_get_unsafe(unit, PROG_FLP_COUPLING_LSR, &flp_process_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 41, exit);
  flp_process_tbl.include_lem_2nd_in_result_a    = 1;
  flp_process_tbl.include_lem_1st_in_result_b    = 1;
  flp_process_tbl.include_lpm_2nd_in_result_a    = 1;
  ...
  flp_process_tbl.enable_lsr_p2p_service         = 0;
  flp_process_tbl.not_found_trap_code = ARAD_PP_TRAP_CODE_INTERNAL_FLP_MPLS_UNKNOWN_LABEL;
  res = arad_pp_ihb_flp_process_tbl_set_unsafe(unit, PROG_FLP_COUPLING_LSR, &flp_process_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 41, exit);
  tmp = 3; /* 0: Apply Ethernet traps on forwarding-header 1: Apply IPv4 traps on forwarding-header and L4 Traps on the following header 2: Apply IPv6 traps on forwarding-header and L4 Traps on the following header 3: Apply MPLS traps on forwarding-header 4: Apply FC traps on forwarding-header Else: Don't apply any protocol traps */
  SOC_REG_ABOVE_64_CLEAR(reg_val);
  res = READ_IHP_PROTOCOL_TRAPS_PROGRAM_SELECTr(unit, 0, reg_val);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 40, exit);
  SHR_BITCOPY_RANGE(reg_val,3*PROG_FLP_COUPLING_LSR,&tmp,0,3);
  res = WRITE_IHP_PROTOCOL_TRAPS_PROGRAM_SELECTr(unit, SOC_CORE_ALL, reg_val);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 45, exit);
exit:
   ...
   )
{
  ...
  ARAD_PP_IHB_FLP_PROGRAM_SELECTION_CAM_TBL_DATA
    prog_selection_cam_tbl;
  uint32 selection_entry = ARAD_PP_FLP_PROG_SEL_CAM_IPV4MC_BRIDGEV4MC;
  ...
  if ((SOC_DPP_CONFIG(unit)->pp.ivl_inlif_profile == 1)) {
    selection_entry = ARAD_PP_FLP_PROG_SEL_CAM_IPV4MC_BRIDGEV4MC_FOR_IVL;
  }

  res = arad_pp_ihb_flp_program_selection_cam_tbl_get_unsafe(unit, selection_entry, &prog_selection_cam_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);
  ...
  prog_selection_cam_tbl.forwarding_code                = ARAD_PP_FWD_CODE_ETHERNET;
  prog_selection_cam_tbl.in_rif_uc_rpf_enable           = 0x0;
  prog_selection_cam_tbl.ipmc_bridge_v4mc_enable        = 0x0;
  prog_selection_cam_tbl.packet_is_compatible_mc        = 1;
  ...
  prog_selection_cam_tbl.ipmc_bridge_v4mc_enable_mask   = (SOC_DPP_CONFIG(unit)->pp.ipmc_l3mcastl2_mode == 1) ?0x0 : 0x1/*ignore*/;
  prog_selection_cam_tbl.packet_is_compatible_mc_mask   = 0x0; /* compatible MC only*/
  prog_selection_cam_tbl.packet_format_code_mask        = 0x3F; 
  prog_selection_cam_tbl.program                        = PROG_FLP_ETHERNET_ING_LEARN;
  prog_selection_cam_tbl.valid                          = 1;
  res = arad_pp_ihb_flp_program_selection_cam_tbl_set_unsafe(unit, selection_entry, &prog_selection_cam_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);
exit:
   ...
   )
{
  ...
  /* For static, program id and app id is same */
  ARAD_PP_FLP_SW_DB_FLP_PROGRAM_ID_SET(unit, PROG_FLP_IPV4MC_BRIDGE, PROG_FLP_IPV4MC_BRIDGE);

  res = arad_pp_ihb_flp_program_selection_cam_tbl_get_unsafe(unit, ARAD_PP_FLP_PROG_SEL_CAM_IPV4MC_BRIDGE, &prog_selection_cam_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);
  ...
  prog_selection_cam_tbl.forwarding_code                = ARAD_PP_FWD_CODE_ETHERNET;
  prog_selection_cam_tbl.in_rif_uc_rpf_enable           = 0x0;
  prog_selection_cam_tbl.ipmc_bridge_v4mc_enable        = 0x1;
  prog_selection_cam_tbl.packet_is_compatible_mc        = 1;
  ...
  prog_selection_cam_tbl.forwarding_code_mask           = 0x00;
  prog_selection_cam_tbl.in_rif_uc_rpf_enable_mask      = 0x1;/* ignore rpf */
  prog_selection_cam_tbl.ipmc_bridge_v4mc_enable_mask   = 0x0;
  prog_selection_cam_tbl.packet_is_compatible_mc_mask   = 0x0; /* compatible MC only*/
  prog_selection_cam_tbl.packet_format_code_mask        = 0x3F;
  prog_selection_cam_tbl.program                        = PROG_FLP_IPV4MC_BRIDGE;
  prog_selection_cam_tbl.valid                          = 1;

  res = arad_pp_ihb_flp_program_selection_cam_tbl_set_unsafe(unit, ARAD_PP_FLP_PROG_SEL_CAM_IPV4MC_BRIDGE, &prog_selection_cam_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);
exit:
   ...
   )
{
  ...
  if (SOC_DPP_CONFIG(unit)->pp.ipmc_l2_ssm_mode) {
      flp_key_construction_tbl.key_c_inst_0_to_5_valid = 0x21 /*hex 6'b100001*/; 
  }
  ...
  flp_key_construction_tbl.instruction_1_16b = 0;
  flp_key_construction_tbl.instruction_2_16b = 0;
  flp_key_construction_tbl.instruction_3_32b = arad_pp_ce_instruction_composer(32,ARAD_PP_CE_HEADER_AFTER_FWD,96,ARAD_PP_CE_IS_CE32);
  flp_key_construction_tbl.instruction_4_32b = 0;
  flp_key_construction_tbl.instruction_5_32b = 0;
  flp_key_construction_tbl.key_a_inst_0_to_5_valid = 0x0 /*6'b000000*/;
  flp_key_construction_tbl.key_b_inst_0_to_5_valid = 0x0 /*6'b000000*/;
  if (SOC_DPP_CONFIG(unit)->pp.ipmc_l2_ssm_mode) {
      flp_key_construction_tbl.key_c_inst_0_to_5_valid = 0x8 /*6'b001000*/; /* FID,DIP[27:0],SIP for forwarding */
  }
   ...
   )
{
  ...
  if (SOC_DPP_CONFIG(unit)->pp.ipmc_l2_ssm_mode == BCM_IPMC_SSM_TCAM_LPM) {
      /* TCAM (forward): {FID,DIP,SIP} - key-C */
      flp_lookups_tbl.tcam_lkp_key_select    = ARAD_PP_FLP_TCAM_LKP_KEY_SELECT_KEY_C_HW_VAL;
      flp_lookups_tbl.tcam_traps_lkp_db_profile_0 = 0x3F;
      flp_lookups_tbl.tcam_traps_lkp_db_profile_1 = 0x3F;
  }
   ...
   )
{
  ...
  /* For static, program id and app id is same */
  ARAD_PP_FLP_SW_DB_FLP_PROGRAM_ID_SET(unit, PROG_FLP_IPV4COMPMC_WITH_RPF, PROG_FLP_IPV4COMPMC_WITH_RPF);
   ...
   )
{
  ...
  res = arad_pp_flp_get_next_avaialable(unit, PROG_FLP_GLOBAL_IPV4COMPMC_WITH_RPF, &cam_sel, prog_id);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
...
/* FEC configurations */
  ...
  if (fec_res_found_bit_offset > 127 || fec_res_found_bit_offset < 120) {
      /* ERROR illegal value */
  ...
  fld_val_fec_res_found_offset = fec_res_found_bit_offset - 120;
   ...
   )
{
  ...
  if (SOC_DPP_L3_SRC_BIND_IPV6_ENABLE(unit)) {
   ...
   if(SOC_IS_JERICHO(unit)){
     arad_pp_flp_dbal_ethernet_tk_epon_uni_v6_program_tables_init(unit,sa_auth_enabled, slb_enabled,ARAD_TCAM_ACCESS_PROFILE_INVALID);
   }
  ...
  {      
      ...
      {
#endif
          arad_pp_flp_prog_sel_cam_ipv4uc_l3vpn(unit);
    #if defined(BCM_88675_A0) && defined(INCLUDE_KBP) && !defined(BCM_88030)
          if(JER_KAPS_ENABLE(unit)){
              arad_pp_flp_dbal_ipv4uc_l3vpn_program_tables_init(unit);
          }
          else
    #endif
          {
              arad_pp_flp_key_const_ipv4uc_l3vpn(unit);
              arad_pp_flp_lookups_ipv4uc_l3vpn(unit);
          }
          arad_pp_flp_process_ipv4uc_l3vpn(unit);
      }
...
#if defined(BCM_88675_A0) && defined(INCLUDE_KBP) && !defined(BCM_88030)
      if((JER_KAPS_ENABLE(unit)) && (!ARAD_KBP_ENABLE_IPV4_UC)) {
          arad_pp_flp_dbal_ipv4uc_rpf_program_tables_init(unit);
      }
      else 
#endif
      {
          arad_pp_flp_key_const_ipv4uc_rpf(unit);
          arad_pp_flp_lookups_ipv4uc_rpf(unit);
      }
      arad_pp_flp_process_ipv4uc_rpf(unit);
...
#if defined(BCM_88675_A0) && defined(INCLUDE_KBP) && !defined(BCM_88030)
      if((JER_KAPS_ENABLE(unit)) && (!ARAD_KBP_ENABLE_IPV4_UC)){
          arad_pp_flp_dbal_ipv4uc_program_tables_init(unit);
      }
      else 
#endif 
      {
      ...
      if (!SOC_DPP_CONFIG(unit)->pp.pon_application_enable) {
        ...
        /* ----------- IPv4 MC bridge -------------*/
        /* ----------------------------------------*/
        arad_pp_flp_prog_sel_cam_ipv4mc_bridge_v4mc(unit);
        ...
        if((JER_KAPS_ENABLE(unit)) && (SOC_DPP_CONFIG(unit)->pp.ipmc_l2_ssm_mode == BCM_IPMC_SSM_KAPS_LPM)) {
            arad_pp_flp_dbal_ipv4mc_bridge_program_tables_init(unit);
        }
        else
#endif
        {
...
#if defined(BCM_88675_A0) && defined(INCLUDE_KBP) && !defined(BCM_88030)
      if((JER_KAPS_ENABLE(unit)) && (!ARAD_KBP_ENABLE_IPV4_MC)) {
          arad_pp_flp_dbal_ipv4compmc_with_rpf_program_tables_init(unit);
      }
      else 
#endif 
      {
...
#if defined(BCM_88675_A0) && defined(INCLUDE_KBP) && !defined(BCM_88030)
      if ((SOC_IS_JERICHO(unit)) && (ARAD_KBP_ENABLE_IPV6_UC)) {
          arad_pp_flp_dbal_ipv6uc_kbp_table_init(unit);
      } else if (SOC_IS_JERICHO(unit))
#else
     if (SOC_IS_JERICHO(unit))
#endif
      {
          arad_pp_flp_dbal_ipv6uc_program_tables_init(unit);
      } else {
...
#if defined(BCM_88675_A0) && defined(INCLUDE_KBP) && !defined(BCM_88030)
          if ((SOC_IS_JERICHO(unit)) && (ARAD_KBP_ENABLE_IPV6_MC)) {
                  arad_pp_flp_dbal_ipv6mc_kbp_table_init(unit);
           }else if(JER_KAPS_ENABLE(unit)){
               arad_pp_flp_dbal_ipv6mc_program_tables_init(unit);
           }else
#endif 
           {
      ...
      if (SOC_IS_JERICHO(unit)) {
          ...
          if(!soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "l3_ipv6_uc_use_tcam", 0)) {
              int prog_id;
              arad_pp_flp_prog_sel_cam_ipv6uc_with_rpf(unit, &prog_id);

              if (ARAD_KBP_ENABLE_IPV6_UC) {
                  arad_pp_flp_dbal_ipv6uc_rpf_kbp_table_init(unit, prog_id);
              }else {
                  if (JER_KAPS_ENABLE(unit)){
                      arad_pp_flp_dbal_ipv6uc_with_rpf_program_tables_init(unit, prog_id);
                  }
              }
              arad_pp_flp_process_ipv6uc_with_rpf(unit, prog_id);
          }
      } else {
          if(ARAD_KBP_ENABLE_IPV6_UC){
      ...
      if (JER_KAPS_ENABLE(unit) && !ARAD_KBP_ENABLE_IPV4_UC) {
          int prog_id;
          arad_pp_flp_prog_sel_cam_ipv4uc_l3vpn_rpf(unit, &prog_id);
          arad_pp_flp_dbal_ipv4uc_l3vpn_rpf_program_tables_init(unit, prog_id);  
          arad_pp_flp_process_ipv4uc_l3vpn_rpf(unit, prog_id);
      }
#endif
  ...
  if ((SOC_DPP_CONFIG(unit)->pp.ivl_inlif_profile == 1)) {
      ...
      arad_pp_flp_prog_sel_cam_ethernet_ing_ivl_inner_learn(unit, &ivl_prog_id);
      /* Set Program Selection for L2 MC IVL using Inner-most VID - Must be called after UC */
      arad_pp_flp_prog_sel_cam_ethernet_mc_ing_ivl_inner_learn(unit, ivl_prog_id);
      /* Set database creation for L2 UC and MC IVL Inner-most VID KEY */
      arad_pp_dbal_flp_ethernet_ing_ivl_inner_learn_tables_create(unit, ivl_prog_id);
      /* Define FLP program process for all L2 UC and MC IVL databases */
      arad_pp_flp_process_ethernet_ing_all_ivl_learn(unit, TRUE, sa_auth_enabled, slb_enabled, ivl_prog_id);

      /* Load IVL L2 FWD OUTER V-TAG
         MUST be second, higher priority than untagged IVL L2 */
      ivl_prog_id=0;
      /* Set Program Selection for L2 UC IVL using Inner-most VID */
      arad_pp_flp_prog_sel_cam_ethernet_ing_ivl_fwd_outer_learn(unit, &ivl_prog_id);
      /* Set Program Selection for L2 MC IVL using Inner-most VID - Must be called after UC */
            
            /* arad_pp_flp_prog_sel_cam_ethernet_mc_ing_ivl_fwd_outer_learn(unit, ivl_prog_id); */
      /* Set database creation for L2 UC and MC IVL Inner-most VID KEY */
      arad_pp_dbal_flp_ethernet_ing_ivl_fwd_outer_learn_tables_create(unit, ivl_prog_id);
      /* Define FLP program process for all L2 UC and MC IVL databases */
      arad_pp_flp_process_ethernet_ing_all_ivl_learn(unit, TRUE, sa_auth_enabled, slb_enabled, ivl_prog_id);
      ...
      arad_pp_flp_prog_sel_cam_ethernet_ing_ivl_learn(unit, &ivl_prog_id);
      /* Set Program Selection for L2 MC IVL using IVEC VID - Must be called after UC */
      arad_pp_flp_prog_sel_cam_ethernet_mc_ing_ivl_learn(unit,ivl_prog_id);
      /* Set database creation for L2 UC and MC IVL IVEC VID KEY */
      arad_pp_dbal_flp_ethernet_ing_ivl_learn_tables_create(unit, ivl_prog_id);
  ...
  if (SOC_DPP_CONFIG(unit)->pp.pon_application_enable) {
      ...
      arad_pp_flp_lookups_ethernet_ing_learn(unit, ingress_learn_enable, sa_auth_enabled, slb_enabled);
  ...
  if (SOC_DPP_CONFIG(unit)->pp.bfd_ipv4_single_hop || (SOC_DPP_CONFIG(unit)->pp.bfd_ipv6_enable==SOC_DPP_ARAD_BFD_IPV6_SUPPORT_WITH_LEM)) {
      ...
      arad_pp_flp_prog_sel_cam_bfd_single_hop_bridge(unit);
   ...
   if (SOC_IS_ARADPLUS_A0(unit) && SOC_DPP_CONFIG(unit)->pp.oam_statistics) {
      ...
      arad_pp_flp_dbal_oam_statistics_program_tables_init(unit, oam_statistics_prog_id);
      ...
      arad_pp_flp_dbal_bfd_statistics_program_tables_init(unit, bfd_statistics_prog_id);
      ...
      arad_pp_flp_dbal_oam_down_untagged_statistics_program_tables_init(unit, oam_down_statistics_prog_id);
      ...
      arad_pp_flp_dbal_oam_single_tag_statistics_program_tables_init(unit, oam_single_tag_statistics_prog_id);
      ...
      arad_pp_flp_dbal_oam_double_tag_statistics_program_tables_init(unit, oam_double_tag_statistics_prog_id);
      ...
      arad_pp_flp_dbal_bfd_mpls_statistics_program_tables_init(unit, bfd_mpls_statistics_prog_id);
      ...
      arad_pp_flp_dbal_bfd_pwe_statistics_program_tables_init(unit, bfd_pwe_statistics_prog_id);
   ...
   )
{
  ...
  for (indx = 0;indx < nof_lines; line++,indx++) {
      ...
      if ((act_line == PROG_FLP_IPV6UC) || (act_line == PROG_FLP_IPV6MC)) {
          /* 0x0 - Use VRF Default Unicast
             0x1 - Use VRF Default Multicast
             Else: Use NotFoundTrapCode from program */
   ...
   )
{
  ...
  /* update program */
  prog_selection_cam_tbl.program      = (mode == 0)?PROG_FLP_ETHERNET_ING_LEARN:PROG_FLP_IPV4MC_BRIDGE;
   ...
   )
{
  ...
  res = arad_pp_ihb_flp_program_selection_cam_tbl_get_unsafe(unit, ARAD_PP_FLP_PROG_SEL_CAM_IPV4MC_BRIDGEV4MC, &prog_selection_cam_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);
   ...
   )
{
    ...
    /* program selection */
    /* allocate resources */
    res = arad_pp_flp_get_next_avaialable(unit,PROG_FLP_BIDIR,&cam_sel,&prog_id);
    SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
...
#else
    case PROG_FLP_PUNT:
      return "PROG_FLP_PUNT";
#endif
    case PROG_FLP_ETHERNET_TK_EPON_UNI_V4_DHCP:
      return "PROG_FLP_ETHERNET_TK_EPON_UNI_V4_DHCP";
    case PROG_FLP_ETHERNET_TK_EPON_UNI_V6_DHCP:
      return "PROG_FLP_ETHERNET_TK_EPON_UNI_V6_DHCP";
