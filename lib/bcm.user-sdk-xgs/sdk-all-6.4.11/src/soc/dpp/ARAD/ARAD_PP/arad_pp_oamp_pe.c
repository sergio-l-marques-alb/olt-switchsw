...
STATIC uint32 _arad_pp_oamp_pe_programs_usage_init(SOC_SAND_IN  int  unit) {
    ...
    if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
        ...
        programs[unit][ARAD_PP_OAMP_PE_PROGS_DEFAULT_CCM].mep_pe_prof_info.pe_var_16_nibble_select_0 = 38;
        programs[unit][ARAD_PP_OAMP_PE_PROGS_DEFAULT_CCM].mep_pe_prof_info.changed = 1;
        }
        ...
        if (SOC_IS_JERICHO(unit) && (SOC_DPP_CONFIG(unit)->pp.oam_statistics )) {
            ...
            programs[unit][ARAD_PP_OAMP_PE_PROGS_OAMP_SERVER_CCM].mep_pe_prof_info.pe_var_16_nibble_select_0 = 38;
            programs[unit][ARAD_PP_OAMP_PE_PROGS_OAMP_SERVER_CCM].mep_pe_prof_info.changed = 1;
        }
        ...
        if (SOC_DPP_CONFIG(unit)->pp.micro_bfd_support==SOC_DPP_ARAD_MICRO_BFD_SUPPORT_IPv4 || 
            ...
            if (SOC_IS_JERICHO(unit) && (SOC_DPP_CONFIG(unit)->pp.oam_statistics )) {
                programs[unit][ARAD_PP_OAMP_PE_PROGS_MICRO_BFD].mep_pe_prof_info.pe_var_16_nibble_select_0 = 38;
                programs[unit][ARAD_PP_OAMP_PE_PROGS_MICRO_BFD].mep_pe_prof_info.changed = 1;
            }
        }
        if (soc_property_get(unit, spn_OAM_MAID_11_BYTES_ENABLE, 0)) {
            ...
            programs[unit][ARAD_PP_OAMP_PE_PROGS_MAID_11B_END_TLV].pe_const = 0;
            if (SOC_IS_JERICHO(unit) && (SOC_DPP_CONFIG(unit)->pp.oam_statistics )) {
                programs[unit][ARAD_PP_OAMP_PE_PROGS_MAID_11B_END_TLV].mep_pe_prof_info.pe_var_16_nibble_select_0 = 38;
                programs[unit][ARAD_PP_OAMP_PE_PROGS_MAID_11B_END_TLV].mep_pe_prof_info.changed = 1;
            }
        }
        ...
        programs[unit][ARAD_PP_OAMP_PE_PROGS_BFD_PWE_BOS_FIX].pe_const = 0;
            if (SOC_IS_JERICHO(unit) && (SOC_DPP_CONFIG(unit)->pp.oam_statistics)) {
        programs[unit][ARAD_PP_OAMP_PE_PROGS_BFD_PWE_BOS_FIX].mep_pe_prof_info.pe_var_16_nibble_select_0 = 38;
        programs[unit][ARAD_PP_OAMP_PE_PROGS_BFD_PWE_BOS_FIX].mep_pe_prof_info.changed = 1;
            }
...
uint32 _arad_pp_oamp_pe_write_inst_bfd_pwe_bos_fix_jer(SOC_SAND_IN int unit) {
    ...
    /*
     ...
     Add OAM statistic support
    if (SOC_IS_JERICHO(unit) && (SOC_DPP_CONFIG(unit)->pp.oam_statistics))
    { 1 Write 2 MSB bytes to the buffer (PTCH-2) and store 2 LSB byte to TMP_REG3
      2 Write 2 bytes MEP-Index from PKT_VAR3 to buffer
      3 Prepare 8'h1 in TMP_VAR1
      4 Copy 2 bytes from TMP_REG3 to buffer
    }
...
/*#1 Write 2 MSB bytes (PTCH-2) to the buffer and store 2 LSB byte to TMP_REG3 */
    SOC_REG_ABOVE_64_CLEAR(reg_data);
    SOC_REG_ABOVE_64_CLEAR(reg_field);
    SET_OAMP_PE_IN_FIFO_RD_BITS(reg_field, 1);
    SET_OAMP_PE_FEM1_SEL_BITS(reg_field, 0Xb);
