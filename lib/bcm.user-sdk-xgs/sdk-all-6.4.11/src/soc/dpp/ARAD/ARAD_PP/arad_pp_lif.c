...
{
    soc_error_t rv;
    ARAD_PP_LIF_ID actual_ext = ext_lif_id;
    ...
    *is_wide_entry = FALSE;
    ...
    if (is_ingress) {
        ...
        *is_wide_entry = inlif_info.local_lif_flags & BCM_DPP_AM_IN_LIF_FLAG_WIDE ; 
        ...
        rv = sw_state_access[unit].dpp.bcm.alloc_mngr_local_lif.outlif_info.data_base.get(unit, lif_id, &outlif_info);
        SOCDNX_IF_ERR_EXIT(rv);
        ...
        *is_wide_entry = outlif_info.local_lif_flags & BCM_DPP_AM_OUT_LIF_FLAG_WIDE ;
        ...
        if (ext_type && (*is_wide_entry)) {
            ...
            rv = MBCM_PP_DRIVER_CALL(unit, mbcm_pp_eg_encap_extension_type_get,
                                     (unit, BCM_DPP_AM_LOCAL_OUTLIF_TO_BASE_BANK_ID(lif_id), ext_type));
            SOCDNX_IF_ERR_EXIT(rv);
        }
    }
  ...
  )
{
  ...
  res = ARAD_PP_L2_LIF_PWE_INFO_verify(unit, pwe_info);
  ...
  )
{
  ...
  if (ac_key->key_type == ARAD_PP_L2_LIF_AC_MAP_KEY_TYPE_FLEXIBLE_Q_IN_Q_PORT_INITIAL_VID_VID)
  {
   ...
   if (ac_key->key_type == ARAD_PP_L2_LIF_AC_MAP_KEY_TYPE_TEST2) {
       sem_key->key_type = ARAD_PP_L2_LIF_AC_MAP_KEY_TYPE_TEST2;
       sem_key->key_info.l2_eth.outer_vid = ac_key->outer_vid;
   }
...
uint32 arad_pp_l2_lif_ac_key_to_qualifier(SOC_SAND_IN  int                        unit,
                                          SOC_SAND_IN  ARAD_PP_L2_LIF_AC_KEY     *ac_key,
                                          SOC_SAND_OUT ARAD_PP_FP_QUAL_VAL qual_vals[])
{
    ...
    if (((ac_key->inner_vid == ARAD_PP_LIF_IGNORE_INNER_VID) && (ac_key->outer_vid != ARAD_PP_LIF_IGNORE_OUTER_VID)) ||
        ((ac_key->key_type == ARAD_PP_L2_LIF_AC_MAP_KEY_TYPE_PORT_COMP_VLAN) || (ac_key->key_type == ARAD_PP_L2_LIF_AC_MAP_KEY_TYPE_PORT_VLAN)))
    {    /*sem_key->key_type = ARAD_PP_ISEM_ACCESS_KEY_TYPE_VD_VID;*/
    ...
    if(ac_key->key_type  ==  ARAD_PP_L2_LIF_AC_MAP_KEY_TYPE_PORT_VLAN_VLAN)
    {
        qual_vals[qual_index].type = SOC_PPC_FP_QUAL_CMPRSD_OUTER_VID;
        qual_vals[qual_index].val.arr[0] = ac_key->outer_vid;
        qual_index++;
  ...
  if (!ignore_key)
  {
    ...
    else
    {
        if (ac_key->key_type == ARAD_PP_L2_LIF_AC_MAP_KEY_TYPE_TEST2) {
            ...
            res = arad_pp_dbal_vtt_sw_db_get(unit, &sem_key, &nof_tables, dbal_tables_id);
  ...
  if (*success == SOC_SAND_SUCCESS || *success == SOC_SAND_FAILURE_OUT_OF_RESOURCES_2)
  {
    ...
    if (ac_info->service_type == ARAD_PP_L2_LIF_AC_SERVICE_TYPE_AC2PWE)
    {
      ...
      if(ac_info->default_frwrd.default_forwarding.additional_info.eei.type != SOC_PPC_EEI_TYPE_EMPTY) 
      {
          ...
          if (ac_info->default_frwrd.default_forwarding.additional_info.outlif.type != ARAD_PP_OUTLIF_ENCODE_TYPE_NONE){
  ...
  )
{
  ...
  res = ARAD_PP_L2_LIF_AC_INFO_verify(unit, ac_info);
  SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
  ...
  if(!ignore_key)
  {
    ...
    else
    {
        ...
        else /* new implementation using DBAL*/
        {
           ...
           res = arad_pp_dbal_entry_get(unit, dbal_tables_id[0], qual_vals, &sem_entry, 0, &hit_bit, found);
           SOC_SAND_CHECK_FUNC_RESULT(res, 14, exit);
        }
    }
  }
  ...
  )
{
  ...
  res = ARAD_PP_L2_LIF_AC_GROUP_INFO_verify(unit, acs_group_info);
  SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
  ...
  if (!ignore_key) {
      ...
      else
      {
          ...
          else /* new implementation using DBAL*/
          {
             ...
             res = arad_pp_dbal_entry_get(unit, dbal_tables_id[0], qual_vals, &sem_entry, 0, &hit_bit, &found_in_isem);
             SOC_SAND_CHECK_FUNC_RESULT(res, 14, exit);
          }

        tmp_lif_index = arad_pp_sem_result_to_lif_index(unit, sem_entry.sem_result_ndx);
      }
  ...
  if (ignore_key || found_in_isem || found_vd) {
      ...
      if (found_in_isem)
      {
          if(nof_tables == 0){/* old implementation */
              ...
              res = arad_pp_dbal_entry_delete(unit, dbal_tables_id[0], qual_vals, &success);
          }
          SOC_SAND_CHECK_FUNC_RESULT(res, 17, exit);
      }
  ...
  )
{
  ...
  res = ARAD_PP_L2_LIF_ISID_INFO_verify(unit, isid_info);
  SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);
    ...
    SOC_SAND_OUT void** in_lif_data)
{
    ...
    if (extender_info->service_type == ARAD_PP_L2_LIF_AC_SERVICE_TYPE_AC2AC)
    {
    ...
    else if (extender_info->service_type == ARAD_PP_L2_LIF_AC_SERVICE_TYPE_MP) {
        ...
        *in_lif_data = in_lif_ac_mp_data;
        ...
        in_lif_ac_mp_data->vsi_assignment_mode = (extender_info->vsi_assignment_mode == ARAD_PP_VSI_EQ_IN_VID) ?
        ...
        in_lif_ac_mp_data->tt_learn_enable = SOC_SAND_BOOL2NUM(extender_info->learn_record.learn_type != ARAD_PP_L2_LIF_AC_LEARN_DISABLE);
        if (extender_info->learn_record.learn_type == ARAD_PP_L2_LIF_AC_LEARN_INFO)
        {
            /* fill the 16b learn info field according to the learn info (reverse forwarding decision info)
            ...
            res = arad_pp_fwd_decision_to_learn_buffer(unit, extender_info->learn_record.learn_info.type,
                                                       extender_info->learn_record.learn_info.dest_id, ARAD_PP_LIF_ENTRY_TYPE_EXTENDER,
                           ...
                           ARAD_PP_FP_QUAL_VAL                      qual_vals[])
{
    ...
    rv = arad_pp_dbal_vtt_sw_db_get(unit, &sem_key, &nof_dbal_tables, dbal_table_ids);
    SOCDNX_IF_ERR_EXIT(rv);
  ...
  )
{
    ...
    ARAD_PP_LIF_ENTRY_TYPE lif_type;
    ARAD_PP_ISEM_ACCESS_ENTRY sem_entry;
    SOC_DPP_DBAL_SW_TABLE_IDS dbal_table_ids[ARAD_PP_ISEM_ACCESS_NOF_TABLES];
    SOC_SAND_OUT ARAD_PP_FP_QUAL_VAL qual_vals[SOC_PPC_FP_NOF_QUALS_PER_PFG_MAX];
    uint8 hit_bit;
    ...
    case ARAD_PP_L2_LIF_AC_SERVICE_TYPE_MP:
        {
            ...
            if (data.type == 0x3) /* ac_mp */
            {
                ...
                extender_info->vsi_assignment_mode = (data.vsi_assignment_mode == ARAD_PP_L2_LIF_VSI_ASSIGNMENT_MODE_VSI_EQ_VLAN) ? 
                                                        ARAD_PP_VSI_EQ_IN_VID:data.vsi_assignment_mode;
                extender_info->oam_valid = data.oam_lif_set;
                extender_info->use_lif = data.use_in_lif;
                ...
                extender_info->default_frwrd.default_frwd_type = ARAD_PP_L2_LIF_DFLT_FRWRD_SRC_VSI;
                extender_info->global_lif = data.sys_in_lif;
                ...
                if (data.tt_learn_enable == 0)
                {
                    extender_info->learn_record.learn_type = ARAD_PP_L2_LIF_AC_LEARN_DISABLE;
                }
                else if (data.learn_info == 0)
                {
                    extender_info->learn_record.learn_type = ARAD_PP_L2_LIF_AC_LEARN_SYS_PORT;
                }
                else
                {
                    extender_info->learn_record.learn_type = ARAD_PP_L2_LIF_AC_LEARN_INFO;
                }
              
                if (data.learn_info != 0 && extender_info->learn_record.learn_type == ARAD_PP_L2_LIF_AC_LEARN_INFO)
                {
                    rv = arad_pp_fwd_decision_from_learn_buffer(
                        unit,
                        data.learn_info,
                        ARAD_PP_LIF_ENTRY_TYPE_EXTENDER, 
                    ...
                    if(extender_info->use_lif) {
                        ...
                        extender_info->learn_record.learn_info.additional_info.outlif.type = ARAD_PP_OUTLIF_ENCODE_TYPE_RAW;
                    }
                }
            } else{
                SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOC_MSG("Mismatch between lif SW DB and lif entry.")));
  ...
  )
{
    ...
    res = arad_pp_sw_db_lif_table_entry_use_set(
            unit, lif_index, ARAD_PP_LIF_ENTRY_TYPE_EXTENDER, extender_info->service_type);
    ...
    SOC_SAND_IN  ARAD_PP_L2_LIF_EXTENDER_INFO       *extender_info)
{
    ...
    res = ARAD_PP_L2_LIF_EXTENDER_KEY_verify(unit, extender_key);
    SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);

    SOC_SAND_ERR_IF_ABOVE_NOF(lif_index, SOC_DPP_DEFS_GET(unit, nof_local_lifs), ARAD_PP_LIF_ID_OUT_OF_RANGE_ERR, 20, exit);

    res = ARAD_PP_L2_LIF_EXTENDER_INFO_verify(unit, extender_info);
    SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
   ...
   )
{
  ...
  if (SOC_IS_JERICHO(unit))
  {
    ...
    else
    {
        ...
        uint32 data_to_set;
        int ext_offset;
        uint32 tbl_data[ARAD_PP_EG_ENCAP_DATA_INFO_MAX_SIZE];

       /*get the local inlif info into SW database*/
        res = sw_state_access[unit].dpp.bcm.alloc_mngr_local_lif.outlif_info.data_base.get(unit, lif_id, &outlif_info);
        SOCDNX_IF_ERR_EXIT(res);

      /*the 1/4 entry which need to be set, based on the lif id.*/
        ext_offset =  ( ARAD_PP_EG_ENCAP_ACCESS_OUT_LIF_TO_OFFSET(lif_id) % ARAD_PP_EG_ENCAP_NOF_TOP_BANKS_PER_EEDB_BANK ) * 20;
...
uint32 arad_pp_lif_protection_pointer_get_unsafe(
    SOC_SAND_IN  int                        unit,
    SOC_SAND_IN  ARAD_PP_LIF_ID             in_lif_ndx,
    SOC_SAND_IN  uint32                     *protection_pointer)
{
    uint32 res = SOC_SAND_OK, lif_sub_type;
    ARAD_PP_LIF_ENTRY_TYPE lif_type;
    ARAD_PP_LIF_ENTRY_INFO lif_entry_info;
    SOC_SAND_SUCCESS_FAILURE success;
    ARAD_PP_RIF_IP_TERM_KEY term_key;
  ...
  )
{
  ...
  {
    res = ARAD_PP_L2_LIF_AC_INFO_verify(unit, &(info->acs_info[ind]));
    SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  }
...
uint32 ARAD_PP_L2_LIF_EXTENDER_KEY_verify(
    SOC_SAND_IN  int                          unit,
    SOC_SAND_IN  ARAD_PP_L2_LIF_EXTENDER_KEY *info)
{
  SOC_SAND_INIT_ERROR_DEFINITIONS(0);
  SOC_SAND_CHECK_NULL_INPUT(info);
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in ARAD_PP_L2_LIF_EXTENDER_KEY_verify()",0,0);
}

uint32 ARAD_PP_L2_LIF_EXTENDER_INFO_verify(
    SOC_SAND_IN  int                          unit,
    SOC_SAND_IN  ARAD_PP_L2_LIF_EXTENDER_INFO *info)
{
  SOC_SAND_INIT_ERROR_DEFINITIONS(0);
  SOC_SAND_CHECK_NULL_INPUT(info);
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in ARAD_PP_L2_LIF_EXTENDER_INFO_verify()",0,0);
}
