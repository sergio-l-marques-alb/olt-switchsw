  ...
  )
{
  ...
  uint8
    app_id,
	trill_mode,
	is_disabling_unused_app,
	disable_mac_in_mac = 0,
	disable_trill = 0,
    enabled = 0,
  ...
  is_disabling_unused_app = soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "disable_lem_prefix_allocation_unused_apps", 0);
  trill_mode = soc_property_get(unit, spn_TRILL_MODE, 0);
  
  if((is_disabling_unused_app == 1) && (trill_mode == 0)){
	  disable_trill = 1;
  }
  ...
  if((is_disabling_unused_app == 1) && (enabled == 0)){
	  disable_mac_in_mac = 1;
  }
    ...
    )
{
  ...
  if(entry_type == ARAD_PP_LEM_ENTRY_TYPE_OUTLIF)
  {
      ...
      if(outlif_valid || fec_type == ARAD_PP_FEC_TYPE_FORWARD) 
      {   /* outlif value */
          if (SOC_IS_JERICHO(unit) && payload->is_learn_data) {
