...
#define ARAD_PP_OUTLIF_BASE                                 (0x80)
...
uint32 arad_pp_frwrd_mact_is_cpu_mode_enabled(SOC_SAND_IN int unit, SOC_SAND_OUT uint8 *cpu_enabled)
{
    ...
    if (dma_supported)
    {
        SOC_SAND_SOC_IF_ERROR_RETURN(res, 31, exit, READ_ECI_FIFO_DMA_SELr(unit, &reg64));
        dma_source_sel = COMPILER_64_LO(reg64);

        *cpu_enabled = (dma_source_sel == DMA_SRC_OLP_FIFO) ? 1 : 0;
    }
    else
    {
		reg_ndx = 0;
  ...
  )
{
  ...
  if (SOC_IS_ARADPLUS(unit)) {
      ...
      if (ARAD_PP_FRWRD_MACT_LEARN_LIMIT_MODE == ARAD_PP_FRWRD_MACT_LEARN_LIMIT_TYPE_LIF) {
          ...
          SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  20,  exit, ARAD_REG_ACCESS_ERR,READ_PPDB_B_LARGE_EM_COUNTER_DB_CNTR_PTR_CONFIGURATIONr_REG64(unit, &lif_limit_reg_value));
          ...
          SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  30,  exit, ARAD_REG_ACCESS_ERR,WRITE_PPDB_B_LARGE_EM_COUNTER_DB_CNTR_PTR_CONFIGURATIONr_REG64(unit, lif_limit_reg_value));
  ...
  for (handle_profile_ndx = 0; handle_profile_ndx < 1; ++handle_profile_ndx)
  {
    ...
    for (is_lag_ndx = 0; is_lag_ndx < ARAD_PP_FRWRD_MACT_EVENT_HANDLE_KEY_NOF_IS_LAGS; ++is_lag_ndx)
    {
      ...
      else /* Distributed mode */
      {
         ...
         */
        event_key.event_type = ARAD_PP_FRWRD_MACT_EVENT_TYPE_TRANSPLANT + ARAD_PP_FRWRD_MACT_EVENT_TYPE_LEARN + ARAD_PP_FRWRD_MACT_EVENT_TYPE_REFRESH;
        handle_info.self_learning = FALSE;
        handle_info.send_to_learning_fifo = TRUE;
        handle_info.send_to_shadow_fifo = FALSE;
        handle_info.send_to_cpu_dma_fifo = FALSE;
  ...
  )
{
  ...
  res = arad_pp_lem_access_parse_only_arad_format(
          unit,
          0, /* flags */
          event_buf->buff,
          event_buf->buff_len,
  ...
  output.payload.flags |= ARAD_PP_FWD_DECISION_PARSE_IS_LEARN;
  /*
   * Value
   */
    ...
    )
{
  ...
  {    
    ...
    offset += ARAD_PP_FRWRD_MACT_LEARN_MSG_EVENT_LEN;
    ...
    )
{
  ...
  if (SOC_DPP_CONFIG(unit)->pp.test2) {
      if (SOC_IS_ARADPLUS_AND_BELOW(unit)){
          SOC_SAND_SOC_IF_ERROR_RETURN(res, 29, exit, soc_reg_field32_modify(unit, EPNI_EEDB_OUTLIF_BASEr, REG_PORT_ANY, EEDB_OUTLIF_BASEf, ARAD_PP_OUTLIF_BASE));
      } else {
...
soc_error_t arad_pp_frwrd_mact_learning_dma_set( SOC_SAND_IN  int unit )
{
    ...
    if (!SAL_BOOT_PLISIM)
    {
        ...
        res = _soc_mem_sbus_fifo_dma_stop(unit, ARAD_LEARNING_DMA_CHANNEL_USED);
        SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 31, exit);
    }

    res = _soc_mem_sbus_fifo_dma_start_memreg(unit, ARAD_LEARNING_DMA_CHANNEL_USED,
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 31, exit, READ_CMIC_CMC0_FIFO_CH0_RD_DMA_HOSTMEM_THRESHOLDr(unit, &reg));

    /* Set threshold by soc property*/
    soc_reg_field_set(unit, CMIC_CMC0_FIFO_CH0_RD_DMA_HOSTMEM_THRESHOLDr, &reg, THRESHOLDf, dma_threshold);
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 31, exit, WRITE_CMIC_CMC0_FIFO_CH0_RD_DMA_HOSTMEM_THRESHOLDr(unit, reg)); 

    reg=0;
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 31, exit, READ_CMIC_CMC0_FIFO_CH0_RD_DMA_CFGr(unit, &reg));
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 31, exit, WRITE_CMIC_CMC0_FIFO_CH0_RD_DMA_CFGr(unit, reg));

    /* Set the DMA's source to be the OLP */
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 31, exit, READ_ECI_FIFO_DMA_SELr(unit, &reg64));
    COMPILER_64_SET(field64,0,DMA_SRC_OLP_FIFO);
    soc_reg64_field_set(unit, ECI_FIFO_DMA_SELr, &reg64, FIFO_DMA_0_SELf, field64);
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 31, exit, WRITE_ECI_FIFO_DMA_SELr(unit, reg64));

    /* Unmask fifo dma channle ARAD_LEARNING_DMA_CHANNEL_USED interrupts */
    soc_cmicm_intr0_enable(unit, IRQ_CMCx_FIFO_CH_DMA(ARAD_LEARNING_DMA_CHANNEL_USED));
...
soc_error_t arad_pp_frwrd_mact_learning_dma_unset(SOC_SAND_IN int unit)
{
    ...
    /* Disonnect DMA engine #0 from the OLP. */
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 31, exit, READ_ECI_FIFO_DMA_SELr(unit, &reg64));
    COMPILER_64_SET(field64,0,DMA_SRC_DEFAULT);
    soc_reg64_field_set(unit, ECI_FIFO_DMA_SELr, &reg64, FIFO_DMA_0_SELf, field64);
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 31, exit, WRITE_ECI_FIFO_DMA_SELr(unit, reg64));
      ...
      SOC_SAND_INOUT  void  *unused4)
{
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 31, exit, READ_CMIC_CMC0_FIFO_CH0_RD_DMA_NUM_OF_ENTRIES_VALID_IN_HOSTMEMr(unit, &field_32));

    num_events_in_host_memory = soc_reg_field_get(unit, CMIC_CMC0_FIFO_CH0_RD_DMA_NUM_OF_ENTRIES_VALID_IN_HOSTMEMr, field_32, ENTRYCOUNTf);
    ...
    res = _soc_mem_sbus_fifo_dma_set_entries_read(unit, ARAD_LEARNING_DMA_CHANNEL_USED, num_events_in_host_memory);
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 31, exit, WRITE_CMIC_CMC0_FIFO_CH0_RD_DMA_STAT_CLRr(unit, field_32));
