...
arad_pp_occ_mgmt_init_outlif_occupation (int unit)
{
    ...
    if (soc_property_get(unit, spn_SPLIT_HORIZON_FORWARDING_GROUPS_MODE, 1) == 1) {
        ...
        SHR_BITDCL mask;
        if (SOC_IS_JERICHO_B0_AND_ABOVE(unit) && !SOC_IS_QAX(unit) && soc_property_get(unit, "logical_port_mim_out", 0) && soc_property_get(unit, "bcm886xx_roo_enable", 0)) {
    ...
    /* OAM PCP */
    if (SOC_INFO(unit).oamp_port[0] == ARAD_OAMP_PORT_ID  && SOC_IS_JERICHO_B0_AND_ABOVE(unit)) { 
