     ...
     )
{
    ...
    if (SOC_IS_ARADPLUS(unit))  {
        soc_mem_field32_set(unit, EPNI_AC_HALF_ENTRY_FORMATm, table_entry, AC_0_ACTION_USE_AS_DATA_BITf, 0);
        ...
        soc_mem_field32_set(unit, EPNI_OUT_RIF_FORMATm, table_entry, OUT_RIF_0_ACTION_DROPf, 1);
        soc_mem_field32_set(unit, EPNI_OUT_RIF_FORMATm, table_entry, OUT_RIF_1_ACTION_DROPf, 1);
    }
...
{  
  ...
  if (key_prefix == ARAD_PP_EG_ENCAP_EEDB_ACCESS_TYPE_OUT_RIF 
       ...
       *        3.3 set the drop (in arad) / data (a+ and above) bits.
       */
      ...
      if (key_prefix2 == ARAD_PP_EG_ENCAP_EEDB_ACCESS_TYPE_NONE) {
          ...
          soc_mem_field32_set(unit, EPNI_AC_HALF_ENTRY_FORMATm, data, (is_location_at_start) ? AC_0_ENTRY_FORMATf : AC_1_ENTRY_FORMATf, 0);

          /* 3.3 - Set the drop / data bits. */
          if (SOC_IS_ARADPLUS(unit))  {
              soc_mem_field32_set(unit, EPNI_AC_HALF_ENTRY_FORMATm, data, (is_location_at_start) ? AC_0_ACTION_USE_AS_DATA_BITf : AC_1_ACTION_USE_AS_DATA_BITf, 0);
          } else if (SOC_IS_ARAD_B1_AND_BELOW(unit)) {
              soc_mem_field32_set(unit, EPNI_AC_FORMATm, data, (is_location_at_start) ? AC_0_ACTION_DROPf : AC_1_ACTION_DROPf, 1);
              soc_mem_field32_set(unit, EPNI_OUT_RIF_FORMATm, data, (is_location_at_start) ? OUT_RIF_0_ACTION_DROPf : OUT_RIF_1_ACTION_DROPf, 1);
          }
...
arad_pp_eg_encap_access_entry_location_info_get(int unit, int outlif, soc_mem_t *mem, uint32 *bank_id, uint32 *internal_offset){
    ...
    is_top_bank_entry = entry_offset >= ARAD_PP_EG_ENCAP_ACCESS_NOF_EEDB_ENTRIES;
  ...
  )
{
  ...
  push_command_upper_limit_val = (SOC_IS_JERICHO_B0_AND_ABOVE(unit) && soc_property_suffix_num_get(unit,-1,spn_CUSTOM_FEATURE,"push_profile_advanced_enable",0)) ? 
      ARAD_PP_EG_ENCAP_MPLS1_COMMAND_PUSH_QAX_UPPER_LIMIT_val : ARAD_PP_EG_ENCAP_MPLS1_COMMAND_PUSH_UPPER_LIMIT_val;
  ...
  if (tbl_data->mpls1_command == ARAD_PP_EG_ENCAP_MPLS1_COMMAND_POP_val) {
    ...
    if (soc_mem_field_valid(unit, EPNI_MPLS_SWAP_FORMATm, MPLS_ACTION_SWAP_REMARK_PROFILEf)) {
        tbl_data->mpls_swap_remark_profile = soc_mem_field32_get(unit, EPNI_MPLS_SWAP_FORMATm, data, MPLS_ACTION_SWAP_REMARK_PROFILEf);
    }
  ...
  )
{
  ...
  if (tbl_data->mpls1_command == ARAD_PP_EG_ENCAP_MPLS1_COMMAND_POP_val) {
    ...
    if (soc_mem_field_valid(unit, EPNI_MPLS_SWAP_FORMATm, MPLS_ACTION_SWAP_REMARK_PROFILEf)) {
        soc_mem_field32_set(unit, EPNI_MPLS_SWAP_FORMATm, data, MPLS_ACTION_SWAP_REMARK_PROFILEf, tbl_data->mpls_swap_remark_profile);
    }
  } else if (tbl_data->mpls1_command <= push_command_upper_limit_val) {
    /* Only in case of MPLS Push */
    /* MPLS-Push-0-7 */
  ...
  )
{
  ...
  if (SOC_IS_JERICHO(unit)) {
    mem = EPNI_TRILL_FORMAT_HALF_ENTRYm;
  } else {
  ...
  )
{
  ...
  tbl_data->next_vsi_lsb = (SOC_IS_JERICHO(unit)) ? outlif : 
        soc_mem_field32_get(unit, EPNI_OUT_RIF_FORMATm, data, (is_location_at_start) ? OUT_RIF_0_ACTION_VSI_LSBSf : OUT_RIF_1_ACTION_VSI_LSBSf);
  ...
  )
{
  ...
  soc_mem_field32_set(unit, EPNI_OUT_RIF_FORMATm, raw_data, (is_location_at_start) ? OUT_RIF_0_ENTRY_FORMATf : OUT_RIF_1_ENTRY_FORMATf, tmp);
  

  /* Set OUT-RIF data */
  soc_mem_field32_set(unit, EPNI_OUT_RIF_FORMATm, raw_data, (is_location_at_start) ? OUT_RIF_0_ACTION_DROPf : OUT_RIF_1_ACTION_DROPf, tbl_data->drop);       
  

  soc_mem_field32_set(unit, EPNI_OUT_RIF_FORMATm, raw_data, (is_location_at_start) ? OUT_RIF_0_ACTION_OAM_LIF_SETf : OUT_RIF_1_ACTION_OAM_LIF_SETf, tbl_data->oam_lif_set);
  

  soc_mem_field32_set(unit, EPNI_OUT_RIF_FORMATm, raw_data, (is_location_at_start) ? OUT_RIF_0_ACTION_REMARK_PROFILEf : OUT_RIF_1_ACTION_REMARK_PROFILEf, tbl_data->remark_profile);  
  ...
  if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
      soc_mem_field32_set(unit, EPNI_OUT_RIF_FORMATm, raw_data, (is_location_at_start) ? OUT_RIF_0_ACTION_VSI_LSBSf : OUT_RIF_1_ACTION_VSI_LSBSf, tbl_data->next_vsi_lsb);  
  }
  

  soc_mem_field32_set(unit, EPNI_OUT_RIF_FORMATm, raw_data, (is_location_at_start) ? OUT_RIF_0_ACTION_ARP_PTR_VALIDf : OUT_RIF_1_ACTION_ARP_PTR_VALIDf, tbl_data->next_outlif_valid);
  
  soc_mem_field32_set(unit, EPNI_OUT_RIF_FORMATm, raw_data, (is_location_at_start) ? OUT_RIF_0_ACTION_ARP_PTRf : OUT_RIF_1_ACTION_ARP_PTRf, tbl_data->next_outlif);

  if (SOC_IS_JERICHO(unit)) {
      soc_mem_field32_set(unit, EPNI_OUT_RIF_FORMATm, raw_data, (is_location_at_start) ? OUT_RIF_0_ACTION_OUTLIF_PROFILEf : OUT_RIF_1_ACTION_OUTLIF_PROFILEf, tbl_data->outlif_profile);
  }
  ...
  )
{
    ...
    if (SOC_IS_ARADPLUS(unit)) {
        ...
        if (tbl_data->use_as_data_entry)
        {
            ...
            previous_prefix = soc_mem_field32_get(unit, EPNI_AC_HALF_ENTRY_FORMATm, raw_data, AC_ENTRY_FORMATf);
            if (previous_prefix == ARAD_PP_EG_ENCAP_ACCESS_PREFIX_TYPE_OTHER) {
                is_big_ac_entry = soc_mem_field32_get(unit, EPNI_AC_HALF_ENTRY_FORMATm, raw_data, AC_0_ACTION_USE_AS_DATA_BITf);
            }
        } else { /* In case of odd entry, it's not a big AC entry. use_as_data_entry should be cleared. */
            soc_mem_field32_set(unit, EPNI_AC_HALF_ENTRY_FORMATm, raw_data, AC_0_ACTION_USE_AS_DATA_BITf, is_big_ac_entry);
        }
    }
#endif
    ...
    /* Key prefix set */
    tmp = ARAD_PP_EG_ENCAP_ACCESS_PREFIX_TYPE_OTHER;
    soc_mem_field32_set(unit, EPNI_AC_HALF_ENTRY_FORMATm, raw_data, AC_ENTRY_FORMATf, tmp);
    
   
    /* Key prefix 2_ENT set */
   
    soc_mem_field32_set(unit, EPNI_AC_HALF_ENTRY_FORMATm, raw_data, (is_location_at_start) ? AC_0_ENTRY_FORMATf : AC_1_ENTRY_FORMATf, ARAD_PP_EG_ENCAP_ACCESS_2_ENT_PREFIX_TYPE_OUT_AC);
   
    /* Set AC data */
    soc_mem_field32_set(unit, EPNI_AC_HALF_ENTRY_FORMATm, raw_data, (is_location_at_start) ? AC_0_ACTION_DROPf : AC_1_ACTION_DROPf, tbl_data->drop);
    
   
    soc_mem_field32_set(unit, EPNI_AC_HALF_ENTRY_FORMATm, raw_data, (is_location_at_start) ? AC_0_ACTION_OAM_LIF_SETf : AC_1_ACTION_OAM_LIF_SETf, tbl_data->oam_lif_set);
    
   
    soc_mem_field32_set(unit, EPNI_AC_HALF_ENTRY_FORMATm, raw_data, (is_location_at_start) ? AC_0_ACTION_VLAN_EDIT_PROFILEf : AC_1_ACTION_VLAN_EDIT_PROFILEf, tbl_data->vlan_edit_profile);
    
    
    soc_mem_field32_set(unit, EPNI_AC_HALF_ENTRY_FORMATm, raw_data, (is_location_at_start) ? AC_0_ACTION_VID_1f : AC_1_ACTION_VID_1f, (tbl_data->vid[0]));
    
   
    soc_mem_field32_set(unit, EPNI_AC_HALF_ENTRY_FORMATm, raw_data, (is_location_at_start) ? AC_0_ACTION_VID_2f : AC_1_ACTION_VID_2f, (tbl_data->vid[1]));
    
    
    soc_mem_field32_set(unit, EPNI_AC_HALF_ENTRY_FORMATm, raw_data, (is_location_at_start) ? AC_0_ACTION_PCP_DEI_PROFILEf : AC_1_ACTION_PCP_DEI_PROFILEf, tbl_data->pcp_dei_profile);
   
    if (SOC_IS_ARADPLUS(unit)) {
        soc_mem_field32_set(unit, EPNI_AC_HALF_ENTRY_FORMATm, raw_data, (is_location_at_start) ? AC_0_ACTION_USE_AS_DATA_BITf : AC_1_ACTION_USE_AS_DATA_BITf, tbl_data->use_as_data_entry);
    }
   
    if (SOC_IS_JERICHO(unit)) {
        soc_mem_field32_set(unit, EPNI_AC_HALF_ENTRY_FORMATm, raw_data, (is_location_at_start) ? AC_0_ACTION_OUTLIF_PROFILEf : AC_1_ACTION_OUTLIF_PROFILEf, tbl_data->outlif_profile);
    }
    ...
    if (is_big_ac_entry == 1)
    {
        ...
        soc_mem_field32_set(unit, EPNI_AC_ENTRY_WITH_DATA_FORMATm, raw_data, AC_ACTION_USE_AS_DATA_BITf, tbl_data->use_as_data_entry);

        soc_mem_field64_set(unit, EPNI_AC_ENTRY_WITH_DATA_FORMATm, raw_data, AC_ACTION_DATAf, data64);  
    }
#endif
