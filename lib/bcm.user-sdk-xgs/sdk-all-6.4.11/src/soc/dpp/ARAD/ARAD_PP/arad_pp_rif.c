  ...
  Arad_pp_procedure_desc_element_rif[] =
{
   ...
   */
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_RIF_MPLS_LABELS_RANGE_SET),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_RIF_MPLS_LABELS_RANGE_SET_PRINT),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_RIF_MPLS_LABELS_RANGE_SET_UNSAFE),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_RIF_MPLS_LABELS_RANGE_SET_VERIFY),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_RIF_MPLS_LABELS_RANGE_GET),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_RIF_MPLS_LABELS_RANGE_GET_PRINT),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_RIF_MPLS_LABELS_RANGE_GET_VERIFY),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_RIF_MPLS_LABELS_RANGE_GET_UNSAFE),
  ...
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_RIF_INFO_SET),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_RIF_INFO_SET_PRINT),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_RIF_INFO_SET_UNSAFE),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_RIF_INFO_SET_VERIFY),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_RIF_INFO_GET),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_RIF_INFO_GET_PRINT),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_RIF_INFO_GET_VERIFY),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_RIF_INFO_GET_UNSAFE),
    ...
    Arad_pp_error_desc_element_rif[] =
{
  ...
  },
  {
    ...
    "The range is: 0 - ARAD_PP_NOF_MPLS_TERM_MODEL_TYPES-1.\n\r ",
    SOC_SAND_SVR_ERR,
    FALSE
  },
  {
    ...
    "The range is: ARAD_PP_NOF_L3_NEXT_PRTCL_TYPES- ARAD_PP_NOF_L3_NEXT_PRTCL_TYPES.\n\r ",
    SOC_SAND_SVR_ERR,
    FALSE
  },
  {
...
typedef struct {
  ARAD_PP_FRWRD_FEC_RPF_MODE mode;
  ARAD_PP_LIF_ENTRY_INFO *info;
  uint32 dev_id;
} ARAD_PP_RIF_INFO_SET_IMP_UNSAFE_DATA;
  ...
  )
{
  ...
  uint32 res;
  ARAD_PP_RIF_INFO_SET_IMP_UNSAFE_DATA *data = opaque;
  uint32 *inlif_profile_ptr;

  SOC_SAND_INIT_ERROR_DEFINITIONS_NO_DEVID(0);

  /* Get the info */
  res = arad_pp_lif_table_entry_get(data->dev_id, lif_id, data->info);
  ...
  res = MBCM_PP_DRIVER_CALL(data->dev_id, mbcm_pp_occ_mgmt_app_set,(data->dev_id, SOC_OCC_MGMT_TYPE_INLIF, SOC_OCC_MGMT_INLIF_APP_STRICT_URPF, ((data->mode == ARAD_PP_FRWRD_FEC_RPF_MODE_UC_STRICT) ? 1 : 0), inlif_profile_ptr));
  SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);

  /* Update the entry */
  ARAD_DEVICE_CHECK(data->dev_id, exit);
  res = arad_pp_lif_table_entry_update(data->dev_id, lif_id, data->info);
  SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);
...
#endif /* BCM_88660_A0 */
STATIC uint32
  arad_pp_rif_routing_enablers_profile_configure(SOC_SAND_IN int unit, SOC_SAND_IN uint32 routing_enablers_bm, SOC_SAND_IN uint8 routing_enablers_bm_id)
{
  uint32
    res,
    entry_offset,
    j,
    nof_entries_to_insert_per_bm,
    tbl_data = 0;

  uint32 nof_routing_enablers_bm = SOC_DPP_CONFIG(unit)->l3.actual_nof_routing_enablers_bm;  
  /* RIF profile is 6 bits, the 2 MSB bits are used for urpf and default routing so we have 4 bits for routing enablers profiles and class ID
     the number of bits used for class ID is defined according to the number of routing enablers bitmaps.
     for example, if we have 8 routing enablers bitmaps we will use the following rif profile format:
     ...
     XXrrrX when X can be eather "0" or "1" (8 possibilities) the array saves all the possibilities according to the size of the routing profiles
     0-3 for 4 bit routing profile, 0-7 for 3 bit routing profile, 0-15 for 2 bit and 0-31 for 1 bit routing profile.*/
  const uint32 basic_entries_offset_for_routing_enablers[32] = {0,16,32,48,1,17,33,49,2,3,18,34,19,35,50,51,4,5,6,7,20,21,22,23,36,37,38,39,52,53,54,55};
  ...
  /* 
    nof_entries_to_insert_per_bm: the number of entries to insert is calculated by the number of bits in the RIF profile that are not relevant
    to the routing enablers bits if all birs assigned to the routing enablers (4 bits) that we have 2 bits that are not relevnt to the
    enablers it means that we need to add 4 entries.
    intf_class size| nof entries per BM
    ---------------|-------------------
    0                 4
    1                 8
    2                 16
    ...
    4                 32
    */
  nof_entries_to_insert_per_bm = 4<<(SOC_DPP_CONFIG(unit)->l3.bit_offset_of_the_routing_enablers_profile);
  ...
  for(j = 0; j < nof_entries_to_insert_per_bm; j++){
    entry_offset = basic_entries_offset_for_routing_enablers[j];
    entry_offset |= routing_enablers_bm_id<<(SOC_DPP_CONFIG(unit)->l3.bit_offset_of_the_routing_enablers_profile);      
    res = arad_pp_ihp_map_rif_profile_to_routing_enable_tbl_set_unsafe(unit, entry_offset, tbl_data);
    SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 40, exit);
  }

exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_pp_rif_routing_enablers_profile_configure()", nof_routing_enablers_bm, 0);
}


soc_error_t
 arad_pp_rif_global_urpf_mode_set(int unit, int is_strict)
{
    uint64 val64;
    ...
    if (is_strict) {
        COMPILER_64_SET(val64, SOC_SAND_U32_MAX, SOC_SAND_U32_MAX);
    }    

    SOCDNX_SAND_IF_ERR_EXIT(WRITE_IHB_IN_RIF_UC_RPF_MODEL_IS_STRICTr(unit, SOC_CORE_ALL, val64));
...
STATIC uint32
  arad_pp_rif_info_set_imp_unsafe(
    SOC_SAND_IN  int         unit,
    SOC_SAND_IN  ARAD_PP_RIF_ID    rif_id,
  ...
  )
{
  ...
  ARAD_PP_LIF_ENTRY_INFO *lif_info = NULL;  
  uint64 val64;
  uint8 is_strict;
  ...
  if (SOC_IS_JERICHO(unit)) {
    ...
    if (rif_info->is_configure_enabler_needed) {
      res = arad_pp_rif_routing_enablers_profile_configure(unit, rif_info->routing_enablers_bm, rif_info->routing_enablers_bm_id);
      SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
    }

    arad_pp_rif_info_to_rif_profile_convert(unit,rif_info->uc_rpf_enable,
                                            rif_info->enable_default_routing,
                                            rif_info->ipmc_bridge_v4mc_enable,
                                            rif_info->intf_class,
    ...
    if (soc_property_get(unit, spn_BCM886XX_L3_INGRESS_URPF_ENABLE, 0)) {
        ...
        if (is_strict) {
            COMPILER_64_BITSET(val64, tbl_data.rif_profile);
        }
        else {
            COMPILER_64_BITCLR(val64, tbl_data.rif_profile);
        }
  ...
  if (SOC_IS_ARADPLUS_A0(unit) && soc_property_get(unit, spn_BCM886XX_L3_INGRESS_URPF_ENABLE, 0)) {
    ARAD_PP_RIF_INFO_SET_IMP_UNSAFE_DATA data;
    uint8 urpf_mode;

    ARAD_ALLOC(lif_info, ARAD_PP_LIF_ENTRY_INFO, sizeof(ARAD_PP_LIF_ENTRY_INFO), "arad_pp_rif_info_set_imp_unsafe.lif_info");

    data.mode = rif_info->uc_rpf_mode;
    data.info = lif_info;
    /* We assume that the unit to set the lifs on is the same unit we got. */
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(0);
  SOC_SAND_CHECK_NULL_INPUT(rif_info);
  ARAD_PP_RIF_INFO_clear(rif_info);
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_PP_RIF_MPLS_LABELS_RANGE_SET_UNSAFE);
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_PP_RIF_MPLS_LABELS_RANGE_SET_VERIFY);

  ARAD_PP_STRUCT_VERIFY(ARAD_PP_RIF_MPLS_LABELS_RANGE, rif_labels_range, 10, exit);
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_PP_RIF_MPLS_LABELS_RANGE_GET_VERIFY);
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_PP_RIF_MPLS_LABELS_RANGE_GET_UNSAFE);

  SOC_SAND_CHECK_NULL_INPUT(rif_labels_range);

  ARAD_PP_RIF_MPLS_LABELS_RANGE_clear(rif_labels_range);
...
{
  uint32                    res = SOC_SAND_OK;
  ARAD_PP_MPLS_TERM_LKUP_INFO mpls_lkup_key;
  ...
  mpls_lkup_key.key_type = SOC_PPC_MPLS_TERM_KEY_TYPE_LABEL;
  if (mpls_lkup_key.key_type == ARAD_PP_MPLS_TERM_KEY_TYPE_LABEL_RIF)
  {
    if (mpls_key->flags != 0) {
    ...
    if (use_label_index)
    {
      ...
      case SOC_PPC_MPLS_LABEL_INDEX_FIRST:
        isem_key->key_type = ARAD_PP_ISEM_ACCESS_KEY_TYPE_MPLS_L1_IN_RIF;
        break;
      default:
  ...
  {
    if (mpls_key->flags & (SOC_PPC_RIF_MPLS_LABEL_FRR|SOC_PPC_RIF_MPLS_LABEL_COUPLING)) /* FRR & Coupling */
    {
      if (mpls_key->flags & SOC_PPC_RIF_MPLS_LABEL_FRR)
      {
        isem_key->key_type = ARAD_PP_ISEM_ACCESS_KEY_TYPE_MPLS_FRR;
      }
      if (mpls_key->flags & SOC_PPC_RIF_MPLS_LABEL_COUPLING) {
        isem_key->key_type = ARAD_PP_ISEM_ACCESS_KEY_TYPE_MPLS_COUPLING;
      }
      if (mpls_key->flags & SOC_PPC_RIF_MPLS_LABEL_ELI) { /* ELI invalid for FRR & Coupling */
        SOC_SAND_SET_ERROR_CODE(ARAD_PP_RIF_MPLS_LABEL_KEY_ILLEGAL_ERR, 20, exit);
      }     
    }
    else
    {
      if (use_label_index)
      {
        ...
        {      
          ...
          {
            case SOC_PPC_MPLS_LABEL_INDEX_FIRST:
              isem_key->key_type = (mpls_key->flags & SOC_PPC_RIF_MPLS_LABEL_ELI) ? ARAD_PP_ISEM_ACCESS_KEY_TYPE_MPLS_L1_ELI:ARAD_PP_ISEM_ACCESS_KEY_TYPE_MPLS_L1;
              break;
            case SOC_PPC_MPLS_LABEL_INDEX_SECOND:
              isem_key->key_type = (mpls_key->flags & SOC_PPC_RIF_MPLS_LABEL_ELI) ? ARAD_PP_ISEM_ACCESS_KEY_TYPE_MPLS_L2_ELI:ARAD_PP_ISEM_ACCESS_KEY_TYPE_MPLS_L2;
              break;
            case SOC_PPC_MPLS_LABEL_INDEX_THIRD:
              if ((mpls_key->flags & SOC_PPC_RIF_MPLS_LABEL_ELI)) {
                  /* Cant have ELI on third label */
                  SOC_SAND_SET_ERROR_CODE(ARAD_PP_RIF_MPLS_LABEL_KEY_ILLEGAL_ERR, 48, exit);
              }
          ...
          case SOC_PPC_MPLS_LABEL_INDEX_ALL:
              ...
              SOC_SAND_SET_ERROR_CODE(ARAD_PP_RIF_MPLS_LABEL_KEY_ILLEGAL_ERR, 60, exit);
              break;
            default:
              SOC_SAND_SET_ERROR_CODE(ARAD_PP_RIF_MPLS_LABEL_KEY_ILLEGAL_ERR, 62, exit);
          }      
        }         
      }
      else
      {
        isem_key->key_type = (mpls_key->flags & SOC_PPC_RIF_MPLS_LABEL_ELI) ? ARAD_PP_ISEM_ACCESS_KEY_TYPE_MPLS_ELI:ARAD_PP_ISEM_ACCESS_KEY_TYPE_MPLS;
      }
    }
  ...
  isem_key->key_info.mpls.is_bos = (mpls_key->flags & SOC_PPC_RIF_MPLS_LABEL_EXPECT_BOS) ? TRUE:FALSE;
  if (SOC_IS_JERICHO(unit) && soc_property_get(unit, spn_EVPN_ENABLE, 0)) {
      isem_key->key_info.mpls.flags =  (mpls_key->flags & SOC_PPC_RIF_MPLS_LABEL_EVPN_IML) ? ARAD_PP_MPLS_TERM_FLAG_EVPN_IML : 0;
  }

exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_pp_rif_mpls_key_to_isem_key_unsafe()", 0, 0);
}
  ...
  )
{
  ...
  res = arad_pp_sw_db_lif_table_entry_use_set(unit, lif_index, ARAD_PP_LIF_ENTRY_TYPE_MPLS_TUNNEL_RIF,0);
  SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);

  res = arad_pp_rif_mpls_label_map_add_internal_unsafe(
          unit,
          mpls_key,
          (mpls_key->flags & ARAD_PP_RIF_MPLS_LABEL_LOOKUP_NONE),
          lif_index,
          term_info,
          rif_info,
          success
  ...
  )
{
  ...
  sem_res_tbl_data.model_is_pipe = (term_info->processing_type == ARAD_PP_MPLS_TERM_MODEL_PIPE) ? 1 : 0;
  ...
  if (SOC_IS_ARADPLUS(unit) && soc_property_get(unit, spn_BCM886XX_MPLS_TERMINATION_KEY_MODE, 1)) {
    sem_res_tbl_data.expect_bos = (mpls_key->flags & ARAD_PP_RIF_MPLS_LABEL_EXPECT_BOS) ? 1 : 0;
  }
  ...
  if (SOC_IS_ARADPLUS_A0(unit) && soc_property_get(unit, spn_BCM886XX_L3_INGRESS_URPF_ENABLE, 0)) {
    ...
    res = MBCM_PP_DRIVER_CALL(unit, mbcm_pp_occ_mgmt_app_set,(unit, SOC_OCC_MGMT_TYPE_INLIF, SOC_OCC_MGMT_INLIF_APP_STRICT_URPF, 
	                                                                       ((urpf_mode == ARAD_PP_FRWRD_FEC_RPF_MODE_UC_STRICT) ? 1 : 0), &sem_res_tbl_data.in_lif_profile));
    SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);
  ...
  /* RIF table */
  if(term_info->rif != SOC_PPD_RIF_NULL && rif_info != NULL)
  {
...
{
  ARAD_PP_MPLS_TERM_LKUP_INFO lkup_info;
  uint32                    res = SOC_SAND_OK;

  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_PP_RIF_MPLS_LABEL_MAP_ADD_VERIFY);

  ARAD_PP_STRUCT_VERIFY(ARAD_PP_MPLS_LABEL_RIF_KEY, mpls_key, 10, exit);
  SOC_SAND_ERR_IF_ABOVE_NOF(lif_index, SOC_DPP_DEFS_GET(unit, nof_local_lifs), ARAD_PP_LIF_ID_OUT_OF_RANGE_ERR, 20, exit);
  res = ARAD_PP_MPLS_TERM_INFO_verify(unit, term_info);
  SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
  res = ARAD_PP_RIF_INFO_verify(unit, rif_info);
  SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);

  
  lkup_info.key_type = SOC_PPC_MPLS_TERM_KEY_TYPE_LABEL;
  if (lkup_info.key_type == ARAD_PP_MPLS_TERM_KEY_TYPE_LABEL && mpls_key->vsid != 0)
  {
    SOC_SAND_SET_ERROR_CODE(ARAD_PP_RIF_MPLS_LABEL_KEY_ILLEGAL_ERR, 50, exit);
  }
  ...
  )
{
  ...
  ignore_key = mpls_key->flags & ARAD_PP_RIF_MPLS_LABEL_LOOKUP_NONE;
  ...
  if(success == TRUE)
  {
    ...
    /* set entry type */
    res = arad_pp_sw_db_lif_table_entry_use_set(unit, *lif_index, ARAD_PP_LIF_ENTRY_TYPE_EMPTY,0);
    SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);
  }
  else
  {
  ...
  )
{
  ...
  ARAD_PP_MPLS_TERM_INFO_clear(term_info);
  ARAD_PP_RIF_INFO_clear(rif_info);
  ...
  *lif_index = arad_pp_sem_result_to_lif_index(unit,isem_entry.sem_result_ndx);
  ...
  term_info->processing_type = sem_res_tbl_data.model_is_pipe ? ARAD_PP_MPLS_TERM_MODEL_PIPE : ARAD_PP_MPLS_TERM_MODEL_UNIFORM;
  ...
  */
  if (*success == SOC_SAND_SUCCESS || *success == SOC_SAND_FAILURE_OUT_OF_RESOURCES_2)
  {
    res = arad_pp_sw_db_lif_table_entry_use_set(
            unit,
            lif_index,
            ARAD_PP_LIF_ENTRY_TYPE_IP_TUNNEL_RIF,
            0
          );
    SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
  }
  ...
  )
{
    ...
    /* IPMC BIDIR */
    if((term_key->flags &  ARAD_PP_RIF_IP_TERM_IPMC_BIDIR)){
    ...
    /* ipv6 */
    if((term_key->flags &  ARAD_PP_RIF_IP_TERM_FLAG_IPV6)){
    ...
    /* IP-LIF-Dummy */
    if(term_key->flags & ARAD_PP_RIF_IP_TERM_FLAG_USE_DIP_DUMMY) {
...
uint32
  arad_pp_rif_ip_tunnel_term_key_from_sem_key(
    SOC_SAND_IN  int                     unit,
    SOC_SAND_OUT ARAD_PP_RIF_IP_TERM_KEY    *term_key,
  ...
  )
{
    ...
    if(isem_key->key_type == ARAD_PP_ISEM_ACCESS_KEY_TYPE_IPV6_TUNNEL_DIP) {
        ...
        term_key->flags = ARAD_PP_RIF_IP_TERM_FLAG_IPV6;
    }
    /* ipv4 tcam */
  ...
  )
{
  ...
  if (term_key->flags & ARAD_PP_RIF_IP_TERM_FLAG_KEY_ONLY) {
      goto exit;
  }
  ...
  if(term_info->learn_enable && 
       ((term_info->learn_record.type == ARAD_PP_FRWRD_DECISION_TYPE_UC_FLOW) || 
       (term_info->learn_record.type == ARAD_PP_FRWRD_DECISION_TYPE_FEC))
    ){
  ...
  )
{
  ...
  if (flags & SOC_PPC_RIF_IP_TUNNEL_TERM_ADD_LIF_ID_IS_FEC) {
      ...
      SOC_SAND_ERR_IF_ABOVE_NOF(lif_index, SOC_DPP_DEFS_GET(unit, nof_fecs), ARAD_PP_LIF_ID_OUT_OF_RANGE_ERR, 10, exit);
  } 
      ...
      SOC_SAND_ERR_IF_ABOVE_NOF(soc_sand_msb_bit_on(lif_index), (SOC_DPP_CONFIG(unit))->pp.vxlan_tunnel_term_in_sem_my_vtep_index_nof_bits, ARAD_PP_LIF_ID_OUT_OF_RANGE_ERR, 11, exit);
  ...
  res = ARAD_PP_RIF_IP_TERM_INFO_verify(unit, term_info);
  SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
  if(rif_info) {
      res = ARAD_PP_RIF_INFO_verify(unit, rif_info);
      SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);
  ...
  )
{
  ...
  res = arad_pp_fwd_decision_from_learn_buffer(
      ...
      term_info->learn_record.additional_info.outlif.type = ARAD_PP_OUTLIF_ENCODE_TYPE_RAW;
  }
  term_info->oam_valid = sem_res_tbl_data.oam_lif_set;
  ...
  )
{
  ...
  SOC_SAND_ERR_IF_ABOVE_MAX(vsid_ndx, ARAD_PP_VSI_ID_MAX, ARAD_PP_VSI_ID_OUT_OF_RANGE_ERR, 10, exit);
  res = ARAD_PP_RIF_INFO_verify(unit, rif_info);
  SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_PP_RIF_INFO_SET_UNSAFE);
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_PP_RIF_INFO_SET_VERIFY);

  SOC_SAND_ERR_IF_ABOVE_NOF(rif_ndx, SOC_DPP_CONFIG(unit)->l3.nof_rifs, ARAD_PP_RIF_ID_OUT_OF_RANGE_ERR, 10, exit);
  res = ARAD_PP_RIF_INFO_verify(unit, rif_info);
  SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_PP_RIF_INFO_GET_VERIFY);
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_PP_RIF_INFO_GET_UNSAFE);
   ...
   )
{
    ...
    else if (SOC_IS_JERICHO(unit)) 
    {
        ...
        SHR_BITCOPY_RANGE(&data_32, ((intf_id %8) * 2), &(native_routing_vlan_tags_32), 0,2);
        data = data_32 & 0xffff; 
   ...
   )
{
    ...
    SOC_SAND_ERR_IF_ABOVE_MAX(native_routing_vlan_tags, ARAD_PP_VLAN_TAGS_MAX, ARAD_PP_VLAN_TAGS_OUT_OF_RANGE_ERR, 10, exit);
    SOC_SAND_ERR_IF_ABOVE_NOF(intf_id, SOC_DPP_CONFIG(unit)->l3.nof_rifs, ARAD_PP_RIF_ID_OUT_OF_RANGE_ERR, 10, exit);
   ...
   )
{
    ...
    else if (SOC_IS_JERICHO(unit)) 
    {
        ...
        *native_routing_vlan_tags = 0;
        ...
        SHR_BITCOPY_RANGE(&native_routing_vlan_tags_32, 0, &data_32, ((intf_id %8) * 2), 2);
        *native_routing_vlan_tags = native_routing_vlan_tags_32 & 0xffff; 
    }
...
uint32
  arad_pp_l2_rif_mpls_key_parse_unsafe(
    SOC_SAND_IN  int                               unit,
    SOC_SAND_IN  ARAD_PP_ISEM_ACCESS_KEY              *isem_key,
    SOC_SAND_OUT ARAD_PP_MPLS_LABEL_RIF_KEY           *rif_key
  )
{
  uint8 use_label_index = FALSE;
  uint32                    res = SOC_SAND_OK;
  ...
  ARAD_PP_MPLS_LABEL_RIF_KEY_clear(rif_key);
  ...
  case ARAD_PP_ISEM_ACCESS_KEY_TYPE_MPLS_COUPLING:
    rif_key->flags |= SOC_PPC_RIF_MPLS_LABEL_COUPLING;
    break;
  ...
  )
{
  ...
  case ARAD_PP_ISEM_ACCESS_KEY_TYPE_RPA:
    term_key->flags |= ARAD_PP_RIF_IP_TERM_IPMC_BIDIR;
  ...
  case ARAD_PP_ISEM_ACCESS_KEY_TYPE_IPV6_TUNNEL_DIP:
    sal_memcpy(&term_key->dip6, &isem_key->key_info.ip_tunnel.dip6, sizeof(SOC_SAND_PP_IPV6_SUBNET));
    term_key->flags |= ARAD_PP_RIF_IP_TERM_FLAG_IPV6 | SOC_PPC_RIF_IP_TERM_FLAG_USE_DIP;
    break;
...
uint32
  ARAD_PP_RIF_MPLS_LABELS_RANGE_verify(
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in ARAD_PP_RIF_MPLS_LABELS_RANGE_verify()",0,0);
}

uint32
  ARAD_PP_MPLS_LABEL_RIF_KEY_verify(
  ...
  )
{
  ...
  SOC_SAND_ERR_IF_ABOVE_MAX(info->label_index, SOC_PPC_NOF_MPLS_LABEL_INDEX-1, ARAD_PP_VSI_ID_OUT_OF_RANGE_ERR, 11, exit);

  SOC_SAND_MAGIC_NUM_VERIFY(info);

exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in ARAD_PP_MPLS_LABEL_RIF_KEY_verify()",0,0);
}

uint32
  ARAD_PP_RIF_INFO_verify(
    SOC_SAND_IN  int              unit,
  ...
  )
{
  ...
  if (info->vrf_id != 0)
  {
    SOC_SAND_ERR_IF_OUT_OF_RANGE(info->vrf_id, ARAD_PP_VRF_ID_MIN, SOC_DPP_DEFS_GET(unit, nof_vrfs) - 1, ARAD_PP_VRF_ID_OUT_OF_RANGE_ERR, 11, exit);
  }
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in ARAD_PP_RIF_INFO_verify()",0,0);
}

uint32
  ARAD_PP_RIF_IP_TERM_INFO_verify(
    SOC_SAND_IN  int                      unit,
  ...
  )
{
  ...
  SOC_SAND_ERR_IF_ABOVE_MAX_AND_NOT_NULL(info->rif, SOC_DPP_CONFIG(unit)->l3.nof_rifs - 1, ARAD_PP_RIF_NULL, ARAD_PP_RIF_ID_OUT_OF_RANGE_ERR, 10, exit);
  ...
  SOC_SAND_ERR_IF_ABOVE_MAX(info->vsi, ARAD_PP_VSI_ID_MAX, ARAD_PP_VSI_ID_OUT_OF_RANGE_ERR, 15, exit); 
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in ARAD_PP_RIF_IP_TERM_INFO_verify()",0,0);
}
