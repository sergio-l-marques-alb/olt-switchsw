  ...
  else if(fwd_decision->additional_info.eei.type == ARAD_PP_EEI_TYPE_OUTLIF) /* outlif */
  {
    ...
    if (SOC_IS_JERICHO(unit)) {
        ...
        temp = (system_headers_mode == ARAD_PP_SYSTEM_HEADERS_MODE_ARAD) ? 0x3
                : fwd_decision->additional_info.eei.val.outlif >> ARAD_PP_EEI_ENCAP_PTR_NOF_BITS_LSB;
        res = soc_sand_bitstream_set_any_field(
                &(temp),
...
arad_pp_init_orientation_maps(int unit){ 
    ...
    for(entry = 0; entry < 64; entry++){
        ...
        if ((entry > 31) && SOC_IS_JERICHO_B0_AND_ABOVE(unit) && !SOC_IS_QAX(unit) && soc_property_get(unit, "logical_port_mim_out", 0) && soc_property_get(unit, "bcm886xx_roo_enable", 0)) {
