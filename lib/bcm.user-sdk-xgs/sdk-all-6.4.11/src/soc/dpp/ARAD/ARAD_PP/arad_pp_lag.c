...
{
	/*Indexes in this table matches ARAD_PP_HASH_MASKS_* index*/
	/* Ethernet */
	/* MAC_SA*/
  ...
  )
{
  ...
  /* not part of symmetric: just take as is*/
	...
	case ARAD_PP_HASH_MASKS_MPLS_LABEL_4:
	case ARAD_PP_HASH_MASKS_MPLS_LABEL_5:
  ...
  )
{
  ...
  for (vc_indx = 0; vc_indx < sizeof(Arad_pp_lb_key_chunk_size) / sizeof(ARAD_PP_LB_HASH_CHUNK_SIZE); ++vc_indx)
  {
    ...
    for (mask_idx = 0; mask_idx < ARAD_PP_NOF_HASH_MASKS; ++mask_idx)
    {
	  ...
	  else if ((SOC_SAND_BIT(mask_idx) == ARAD_PP_HASH_MASKS_MPLS_LABEL_4) && (ARAD_PP_LB_MPLS_LBL4_VLD(map_index))) {
		skip = FALSE;
	  }
	  else if ((SOC_SAND_BIT(mask_idx) == ARAD_PP_HASH_MASKS_MPLS_LABEL_5) && (ARAD_PP_LB_MPLS_LBL5_VLD(map_index))) {
		skip = FALSE;
	  }
      if (skip)
      {
