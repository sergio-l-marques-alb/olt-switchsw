...
#define ARAD_PP_TRAP_MGMT_CPU_TRAP_QUALIFIER_MAX                 (ARAD_PP_NOF_TRAP_QUALIFIERS - 1)
...
Arad_pp_vtt_sb_to_internal[] =
{
  ...
  ARAD_PP_TRAP_CODE_INTERNAL_MPLS_ILLEGAL_LABEL,           
  ARAD_PP_TRAP_CODE_INTERNAL_INNER_IP_COMP_MC_INVALID_IP_0,          
  ARAD_PP_TRAP_CODE_INTERNAL_INNER_ETHERNET_MY_MAC_IP_DISABLED,  
 ...
 */
  ...
  case  ARAD_PP_TRAP_CODE_INNER_IP_COMP_MC_INVALID_IP_0:
    *trap_code_internal = ARAD_PP_TRAP_CODE_INTERNAL_INNER_IP_COMP_MC_INVALID_IP_0;
    break;
  case  ARAD_PP_TRAP_CODE_MPLS_ILLEGAL_LABEL:
    *trap_code_internal = ARAD_PP_TRAP_CODE_INTERNAL_MPLS_ILLEGAL_LABEL;
    ...
    break;
  case  ARAD_PP_TRAP_CODE_INNER_ETHERNET_MY_MAC_IP_DISABLED:
    *trap_code_internal = ARAD_PP_TRAP_CODE_INTERNAL_INNER_ETHERNET_MY_MAC_IP_DISABLED;
    break;
  default:
  ...
  )
{
  ...
  case  ARAD_PP_TRAP_CODE_INTERNAL_LLR_SA_EQUALS_DA:
    *trap_code = ARAD_PP_TRAP_CODE_SA_EQUALS_DA;
    break;
  case  ARAD_PP_TRAP_CODE_INTERNAL_LLR_8021X:
    *trap_code = ARAD_PP_TRAP_CODE_8021X;
    break;
 ...
 */
  ...
  default:
    ...
    else if (SOC_IS_JERICHO(unit) && is_jericho_ingress_trap)
    {
      switch (trap_code_internal) 
      {
        case ARAD_PP_TRAP_CODE_INTERNAL_INNER_IP_COMP_MC_INVALID_IP_0:
                *trap_code = ARAD_PP_TRAP_CODE_INNER_IP_COMP_MC_INVALID_IP_0;
                break;
        case ARAD_PP_TRAP_CODE_INTERNAL_MPLS_ILLEGAL_LABEL:
                *trap_code = ARAD_PP_TRAP_CODE_MPLS_ILLEGAL_LABEL;
                break;
        case ARAD_PP_TRAP_CODE_INTERNAL_INNER_ETHERNET_MY_MAC_IP_DISABLED:
                *trap_code = ARAD_PP_TRAP_CODE_INNER_ETHERNET_MY_MAC_IP_DISABLED;
                break;
      }
    }
    /* ARAD_PP_TRAP_CODE_USER_DEFINED_0-59 */
  ...
  )
{
    ...
    COMPILER_64_SET(curr_dnx_header->otsh.oam_ts_data, 0, fld_val);
  ...
  )
{
    ...
    if(soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "trap_packet_parse_test", 0)){
      res = arad_pp_oam_oamp_error_trap_id_and_destination_get(unit, SOC_PPD_OAM_OAMP_RMEP_ERR, &trap_id, &dest_info);
  ...
  )
{
    ...
    case ARAD_PP_TRAP_MGMT_PKT_FRWRD_TYPE_SNOOP:
    case ARAD_PP_TRAP_MGMT_PKT_FRWRD_TYPE_IN_MIRROR:
        break;
    default:
  ...
  is_routed = ((fld_val == ARAD_PP_TRAP_MGMT_IPV4_UC_ROUTE_VAL) ||
     ...
     */
     if ((soc_property_get(unit, "custom_feature_parse_oamp_punt_packet", 0) == src_sys_port) 
         &&
         (soc_property_get(unit, "custom_feature_parse_oamp_punt_packet", 0) != 0))
     {
         if (app_ext==3) {
             app_ext=1;
  ...
  )
{
  ...
  if ((soc_property_get(unit, "custom_feature_parse_oamp_punt_packet", 0) == _src_sys_port) 
      &&
      (soc_property_get(unit, "custom_feature_parse_oamp_punt_packet", 0) != 0)
       &&
  ...
  /* Check for packets with a double set of FTMH,internals*/

  if (
      ((soc_property_get(unit, "custom_feature_parse_oamp_punt_packet", 0) == _src_sys_port) 
      &&
      (soc_property_get(unit, "custom_feature_parse_oamp_punt_packet", 0) != 0))
      ||
      ((soc_property_get(unit, "custom_feature_oam_additional_FTMH_on_error_packets", 0) == 1)
       &&
       (_fwd_header_offset == 0) 
       &&
       ((_e_trap_id == SOC_PPC_TRAP_CODE_OAM_LEVEL) || (_e_trap_id == ARAD_PP_TRAP_CODE_OAM_PASSIVE))
       )
      )
  {
      res = arad_pp_trap_packet_parse_ftmh(unit,hdr_buff,packet_parse_info, packet_info, dnx_pkt->dnx_header_count, dnx_pkt);
...
uint32
  arad_pp_trap_mpls_illegal_label_write(
    ...
    ARAD_PP_TRAP_CODE_INTERNAL                          trap_code,
  ...
  )
{
  ...
  if (strength_fld_fwd.reg != INVALIDr) {
        ...
        if (trap_code_internal == ARAD_PP_TRAP_CODE_INTERNAL_MPLS_ILLEGAL_LABEL) {
  ...
  )
{
  ...
  if (strength_fld_snp.reg != INVALIDr)
  {
    ...
    if (trap_code_internal == ARAD_PP_TRAP_CODE_INTERNAL_MPLS_ILLEGAL_LABEL)
    {
         ...
         SOC_SAND_CHECK_FUNC_RESULT(res, 35, exit);/*need to ask tal*/
  ...
  )
{
  ...
  tbl_data.cos_profile_valid = ((profile_info->bitmap_mask & ARAD_PP_TRAP_ACTION_PROFILE_OVERWRITE_COS_PROFILE) ? 0x1 : 0x0);

  /* config per core section - Jericho and above */

  if (profile_info->bitmap_mask & ARAD_PP_TRAP_ACTION_PROFILE_USE_ARR) 
  { 
        BCM_DPP_CORES_ITER(_SHR_CORE_ALL, core)
        {
            res = arad_pp_trap_action_profile_set_tm_port(unit,core, profile_info->out_tm_port_arr[core], profile_info, &tbl_data);
            SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);

            tbl_data.cud_valid = ((profile_info->bitmap_mask & ARAD_PP_TRAP_ACTION_PROFILE_OVERWRITE_CUD) ? 0x1 : 0x0);
            if (tbl_data.cud_valid) 
            {
                 tbl_data.cud = profile_info->header_data_arr[core].cud;
            }

            if (!SOC_IS_ARADPLUS_AND_BELOW(unit))
            {
                  tbl_data.pp_dsp = profile_info->header_data_arr[core].dsp_ptr;
                  tbl_data.pp_dsp_valid =  ((profile_info->bitmap_mask & ARAD_PP_TRAP_ACTION_PROFILE_OVERWRITE_PP_DSP) ? 0x1 : 0x0);
            }
  ...
  )
{
  ...
  {
    profile_info->bitmap_mask |= ARAD_PP_TRAP_ACTION_PROFILE_OVERWRITE_COS_PROFILE;
  }
  ...
  else /* Jericho and above */
  {
          BCM_DPP_CORES_ITER(_SHR_CORE_ALL, core)
          {
                  ...
                  {
                      profile_info->bitmap_mask |= ARAD_PP_TRAP_ACTION_PROFILE_OVERWRITE_PP_DSP;
                  }
  ...
  )
{
  ...
  if (info->out_tm_port != ARAD_PP_TRAP_ACTION_PKT_DISCARD_ID && 
      info->out_tm_port != ARAD_PP_TRAP_ACTION_PKT_CPU_ID && 
      info->out_tm_port != ARAD_PP_TRAP_ACTION_PKT_DISCARD_AND_LEARN_ID)
  {
...
int 
arad_pp_trap_action_profile_set_tm_port(/*in*/ SOC_SAND_IN int unit,SOC_SAND_IN int core, SOC_SAND_IN SOC_PPC_TM_PORT tm_port, SOC_SAND_IN ARAD_PP_TRAP_EG_ACTION_PROFILE_INFO *profile_info, /*out*/ SOC_SAND_OUT ARAD_PP_EGQ_ACTION_PROFILE_TABLE_TBL_DATA *tbl_data)
{
    uint32 base_q_pair, reg_val, cpu_port_tm;

    SOCDNX_INIT_FUNC_DEFS;

    tbl_data->discard = (tm_port == ARAD_PP_TRAP_ACTION_PKT_DISCARD_ID ? 0x1 : 0x0);
    ...
    {
        if ( tm_port  == ARAD_PP_TRAP_ACTION_PKT_DISCARD_ID || tm_port == ARAD_PP_TRAP_ACTION_PKT_DISCARD_AND_LEARN_ID)
        {
        ...
        else if ( tm_port == ARAD_PP_TRAP_ACTION_PKT_CPU_ID)
        {
...
int 
arad_pp_trap_action_profile_get_tm_port_and_hdr_data(/*in*/ SOC_SAND_IN int unit, SOC_SAND_IN int core, SOC_SAND_IN ARAD_PP_EGQ_ACTION_PROFILE_TABLE_TBL_DATA *tbl_data,/*out*/  SOC_SAND_OUT SOC_PPC_TM_PORT *tm_port, SOC_SAND_OUT ARAD_PP_TRAP_EG_ACTION_HDR_DATA    *profile_hdr_data, SOC_SAND_OUT ARAD_PP_TRAP_EG_ACTION_PROFILE_INFO *profile_info)
{
     ...
     if (tbl_data->discard)
     {
       *tm_port = ARAD_PP_TRAP_ACTION_PKT_DISCARD_ID;
     }
     else if (!tbl_data->discard && tm_port_is_invalid_otm)
     {
      *tm_port = ARAD_PP_TRAP_ACTION_PKT_DISCARD_AND_LEARN_ID;
     }
     else
     {
