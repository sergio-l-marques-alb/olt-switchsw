  ...
  )
{
  ...
  if(mac_entry_key->key_type == SOC_PPC_FRWRD_MACT_KEY_TYPE_IPV4_MC && (SOC_IS_JERICHO(unit)) &&
                            (SOC_DPP_CONFIG(unit)->pp.ipmc_l2_ssm_mode == BCM_IPMC_SSM_KAPS_LPM))
  {
  ...
  )
{
    ...
    switch(mac_entry_key->key_type)
    {
        case SOC_PPC_FRWRD_MACT_KEY_TYPE_IPV4_MC:
            DBAL_QUAL_VAL_ENCODE_IPV4_DIP(&qual_vals[0], SOC_SAND_GET_BITS_RANGE(mac_entry_key->key_val.ipv4_mc.dip, ARAD_PP_LEM_ACCESS_KEY_PARAM0_IN_BITS_FOR_IPV4_MC-1, 0), 32);
