    ...
    arad_pp_init_dynamic_kbp_result(int unit)
{
    ...
    int results_valid[ARAD_PP_FLP_KBP_MAX_NUMBER_OF_RESULTS] = {0};
    ARAD_PMF_CE_IRPP_QUALIFIER_ATTRIBUTES kbp_qualifiers_values;
    int i, total_nof_bits_in_elk_res = 0,total_shift_bits_in_elk_res = 0 ;
    ...
    /* default result size in Jericho: 48 32 24 16, when result 0 can be used for FWD, and result 2 for RPF for UC programs, for MC programs
       RPF is located in result 1 (comp1)*/
    
    res_sizes[0] = soc_property_suffix_num_get(unit, 0, spn_EXT_TCAM_RESULT_SIZE_SEGMENT, "0", 48);/* FWD */
    res_sizes[1] = soc_property_suffix_num_get(unit, 0, spn_EXT_TCAM_RESULT_SIZE_SEGMENT, "1", 32);
    res_sizes[2] = soc_property_suffix_num_get(unit, 0, spn_EXT_TCAM_RESULT_SIZE_SEGMENT, "2", 24);/* FEC (RPF)*/
    res_sizes[3] = soc_property_suffix_num_get(unit, 0, spn_EXT_TCAM_RESULT_SIZE_SEGMENT, "3", 16);

    /*if fwd is enable in one of the tables the first result has to be 48 */
    ...
    if (total_nof_bits_in_elk_res != ARAD_PP_FLP_KBP_MAX_ROP_PAYLOAD_SIZE_IN_BITS) {
        SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG("total external result size has to be %d, total = %d, "
                                                              "update ext_tcam_result_size_segment_# according\n"), ARAD_PP_FLP_KBP_MAX_ROP_PAYLOAD_SIZE_IN_BITS, total_nof_bits_in_elk_res));
    }
    
    for (i = 0; i < ARAD_PP_FLP_KBP_MAX_NUMBER_OF_RESULTS; i++) {
        res_sizes_in_byte[i] = res_sizes[i]/8;
    }
    ...
    for(i = 0; i < SOC_DPP_DEFS_GET(unit, nof_flp_programs); i++){
        ...
        if (flp_lookups_tbl.elk_lkp_valid) {
            ...
            if((i == PROG_FLP_IPV4UC_RPF) || (i == ipv6rpf_prog_id) || (i == PROG_FLP_IPV6MC) || (i == PROG_FLP_IPV4COMPMC_WITH_RPF)){
                results_valid[2] = 1; /* RPF result for those program is located in result 2*/
                rpf_result_start_offset = res_sizes[3];
            ...
            results_valid[0] = 1;
			if ((i == PROG_FLP_IPV6MC)||(i == PROG_FLP_IPV4COMPMC_WITH_RPF)) {
				/* 	Enable result 1 for dummy lookup configuration
				 * 	IPV4MC and IPV6MC are using configured to have dummy lookup on lookup#1 in order to support
				 	RPF on lookup#2, as all other programs
				 */ 
				results_valid[1] = 1; 
			}
                            
            SOCDNX_SAND_IF_ERR_EXIT(arad_kbp_opcode_to_db_type(unit, flp_lookups_tbl.elk_opcode, &db_type, &ltr_id, &success));
            SOCDNX_SAND_IF_ERR_EXIT(arad_kbp_result_configurations_update(unit, db_type, res_sizes_in_byte, results_valid));
            results_valid[0] = 0;
            results_valid[1] = 0;
            results_valid[2] = 0;
        }        
    ...
    total_shift_bits_in_elk_res = 0;
    for (i = 0; i < ARAD_PP_FLP_KBP_MAX_NUMBER_OF_RESULTS; i++) {
        SOCDNX_SAND_IF_ERR_EXIT(arad_pmf_ce_internal_field_info_find(unit, SOC_PPC_FP_QUAL_ELK_LOOKUP_RESULT_0+i, 
                                                                     ARAD_FP_DATABASE_STAGE_INGRESS_PMF, 0, &found, &irpp_qual_info));

        total_shift_bits_in_elk_res += (irpp_qual_info.info.qual_nof_bits-res_sizes[i]);
        ...
        kbp_qualifiers_values.buffer_lsb    = irpp_qual_info.info.buffer_lsb + total_shift_bits_in_elk_res;        
