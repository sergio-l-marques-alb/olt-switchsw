...
#define ARAD_PP_MTR_MEMORY(unit, memory)				SOC_IS_ARADPLUS_AND_BELOW(unit)? IDR_##memory : MRPS_##memory
#define ARAD_PP_MTR_REG_PORT(unit, core_id)             SOC_IS_ARADPLUS_AND_BELOW(unit)? REG_PORT_ANY: core_id
#define ARAD_PP_MTR_MEM_BLOCK(unit, core_id)       		SOC_IS_ARADPLUS_AND_BELOW(unit)? MEM_BLOCK_ANY : MRPS_BLOCK(unit, core_id)
...
STATIC uint32 arad_pp_mtr_meter_ins_to_bw_profile_map_init_buckets(SOC_SAND_IN  int unit, SOC_SAND_IN  int core_id, SOC_SAND_IN ARAD_PP_MTR_METER_ID *meter_ins_ndx, SOC_SAND_IN uint32 bw_profile_id);
  ...
  )
{
	...
	if (bucket_size < total_rate) {
		*bucket_exp = (rate1_exp >= rate2_exp) ? rate1_exp + 1 : rate2_exp + 1;
...
uint32 arad_pp_metering_dp_map_entry_set(
	SOC_SAND_IN int                                  unit, 
	SOC_SAND_IN ARAD_PP_MTR_COLOR_RESOLUTION_INFO     *dp_map_entry)
{
	...
	SHR_BITCOPY_RANGE(&addr_idx, ARAD_PP_MTR_DP_MAP_MTR_DP_LSB,
					  (void *)&(dp_map_entry->policer_color), 0, 
...
uint32 arad_pp_metering_dp_map_entry_get(
	SOC_SAND_IN int                                  unit, 
	SOC_SAND_OUT ARAD_PP_MTR_COLOR_RESOLUTION_INFO     *dp_map_entry)
{
	uint32 addr_idx, dp_map_line;
	soc_mem_t mem;
	SOCDNX_INIT_FUNC_DEFS;
...
uint32 arad_pp_metering_pcd_entry_set(
	SOC_SAND_IN int                                  unit, 
	SOC_SAND_IN ARAD_PP_MTR_COLOR_DECISION_INFO      *pcd_entry)
{
...
uint32 arad_pp_metering_pcd_entry_get(
	SOC_SAND_IN int                                  unit, 
	SOC_SAND_OUT ARAD_PP_MTR_COLOR_DECISION_INFO      *pcd_entry)
{
	uint32 addr_idx, pcd_line;
	soc_mem_t mem;
	int core_index;
	 ...
	 SOC_SAND_IN uint8 sharing_mode)
{
	...
	if (ARAD_PP_MTR_SHARING_MODE_SERIAL == sharing_mode) {
	  ...
	  SOC_SAND_IN int unit)
{
	...
	for (addr_idx = 0; addr_idx < ARAD_PP_MTR_PCD_OPT_NUM; addr_idx++) {
		...
		if ((mcda_mtr_color == ARAD_PP_MTR_PCD_COL_INVALID) && (mcdb_mtr_color == ARAD_PP_MTR_PCD_COL_INVALID)) {
			...
			final_color = mcda_update_color = ARAD_PP_MTR_PCD_COL_YELLOW;
			mcdb_update_color = (mcdb_other_credits ? ARAD_PP_MTR_PCD_COL_YELLOW : ARAD_PP_MTR_PCD_COL_GREEN);
		}
  ...
  )
{
  ...
  if ((range_mode_64k && (sharing_mode != ARAD_PP_MTR_SHARING_MODE_NONE)) || 
      (!range_mode_64k && ((sharing_mode != ARAD_PP_MTR_SHARING_MODE_SERIAL) && (sharing_mode != ARAD_PP_MTR_SHARING_MODE_PARALLEL)))) {
  ...
  if (range_mode == 32) { /* sharing_mode is only relevent in 32K range_mode */
      soc_reg_field_set(unit, MRPS_GLBL_CFGr, &reg_val, MCQ_MCDS_PARALLELf, sharing_mode == ARAD_PP_MTR_SHARING_MODE_PARALLEL ? 1 : 0);
  }
  ...
  if (SOC_IS_ARADPLUS(unit)) {
      ...
      if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
          SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  __LINE__,  exit, ARAD_REG_ACCESS_ERR, soc_reg_above_64_field32_modify(unit, IDR_SPARE_REGISTER_2r, REG_PORT_ANY, 0, SYS_CONFIG_3f,  SOC_DPP_CONFIG(unit)->meter.color_resolution_mode == ARAD_PP_MTR_COLOR_RESOLUTION_MODE_RED_DIFF));
      }
      ...
      if ((range_mode == 32) && (sharing_mode != ARAD_PP_MTR_SHARING_MODE_NONE)){
  ...
  )
{
  ...
  if (bw_profile_info->is_sharing_enabled) {
	  ...
	  if (bw_profile_info->disable_cir && bw_profile_info->disable_eir 
			&& ARAD_PP_BW_PROFILE_ADD_VERIFY_IR_MIN == max_cir_for_calc 
			&& ARAD_PP_BW_PROFILE_ADD_VERIFY_IR_MIN == max_eir_for_calc) 
	  {
			/* all are disabled, no need to calculate anything */
      ...
      else 
	  {
		  ...
		  rates_idx = 0;
		  if (FALSE == bw_profile_info->disable_cir){
			  /* CIR used*/
			  rates[rates_idx] = bw_profile_info->cir;
			  ++rates_idx;
			  cir_valid_rate = 1;
		  }
		  if (FALSE == bw_profile_info->disable_eir){
			  /* EIR used*/
			  rates[rates_idx] = bw_profile_info->eir;
			  ++rates_idx;
			  eir_valid_rate = 1;
		  }
		  if (max_cir_for_calc > ARAD_PP_BW_PROFILE_ADD_VERIFY_IR_MIN){
			  /* Max-CIR used */
		  ...
		  if ((bw_profile_info->hierarchical_max_rate != 0) && (bw_profile_info->hierarchical_min_rate != 0)){
			  ...
			  rates[rates_idx] = bw_profile_info->hierarchical_max_rate;
			  rates[rates_idx + 1] = bw_profile_info->hierarchical_min_rate;
			  rates_idx += 2;
		  }
		  nof_rates = rates_idx;
  ...
  arad_pp_mtr_verify_valid_bucket_size(unit,
									   ...
									   prfcfg_tbl_data.max_eir_mantissa*max_eir_valid_rate,prfcfg_tbl_data.max_eir_mantissa*max_eir_valid_rate,
  ...
  if (SOC_IS_JERICHO(unit)) {
	   /* check which mem to config */
      if (mtr_group_ndx == 0) { /* group A */
         mem = MRPS_MCDA_PRFCFG_1m;
      } else { /* group B */
         mem = MRPS_MCDB_PRFCFG_1m;
      }
...
soc_error_t
  arad_pp_mtr_meter_ins_bucket_get(
    int                                  unit,
	int                                  core_id,
    ARAD_PP_MTR_METER_ID                 *meter_ins_ndx,
    int                                  *cbl,
	int                                  *ebl
  )
{
