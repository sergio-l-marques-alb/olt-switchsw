...
/* $Id: arad_pp_llp_filter.c,v 1.21 Broadcom SDK $
...
/* min entry in ARAD_PP_IHP_DESIGNATED_VLAN_TABLE_TBL_DATA which is not NULL */
  ...
  Arad_pp_error_desc_element_llp_filter[] =
{
  ...
  {
    ARAD_PP_PORTS_OUT_OF_RANGE_ERR,
    "ARAD_PP_PORTS_OUT_OF_RANGE_ERR",
...
uint32
  arad_pp_llp_filter_ingress_vlan_membership_set_unsafe(
    ...
    SOC_SAND_IN  uint32                                      ports[ARAD_PP_VLAN_MEMBERSHIP_BITMAP_SIZE]
  )
{
  uint32
    res = SOC_SAND_OK;
...
uint32
  arad_pp_llp_filter_ingress_vlan_membership_set_verify(
    SOC_SAND_IN  int                                     unit,
    SOC_SAND_IN  SOC_SAND_PP_VLAN_ID                           vid_ndx,
    SOC_SAND_IN  ARAD_PP_PORT                                ports[ARAD_PP_VLAN_MEMBERSHIP_BITMAP_SIZE]
  )
{
...
uint32
  arad_pp_llp_filter_ingress_vlan_membership_get_unsafe(
    ...
    SOC_SAND_IN  int                                   core_id,
    SOC_SAND_IN  SOC_SAND_PP_VLAN_ID                             vid_ndx,
    SOC_SAND_OUT ARAD_PP_PORT                                ports[ARAD_PP_VLAN_MEMBERSHIP_BITMAP_SIZE]
  )
{
  uint32
    res = SOC_SAND_OK;
  ...
  soc_sand_os_memset(ports, 0, sizeof(ARAD_PP_PORT) * ARAD_PP_VLAN_MEMBERSHIP_BITMAP_SIZE); /* Size of the ports array */
  soc_sand_os_memcpy(ports, &tbl_data.vlan_port_member_line, sizeof(uint32) * 8); /*sizeof(tbl_data.vlan_port_member_line)*/
  ...
  )
{
  ...
  SOC_SAND_ERR_IF_ABOVE_MAX(vid_ndx, SOC_SAND_PP_VLAN_ID_MAX, SOC_SAND_PP_VLAN_ID_OUT_OF_RANGE_ERR, 10, exit);
  SOC_SAND_ERR_IF_ABOVE_MAX(local_port, ARAD_PP_PORT_MAX, ARAD_PP_PORT_OUT_OF_RANGE_ERR, 20, exit);
  ...
  )
{
  ...
  ARAD_PP_STRUCT_VERIFY(ARAD_PP_ACTION_PROFILE, action_profile, 30, exit);
  SOC_SAND_SET_ERROR_CODE(ARAD_PP_FEATURE_NOT_SUPPORTED_ERR, 10, exit);
exit:
