...
STATIC uint32 arad_pp_frwrd_fec_is_protected_init(SOC_SAND_IN int unit);
  ...
  )
{
  ...
  uint32
    res;
  uint8
	is_protected, is_protected_init;

  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_PP_FRWRD_FEC_PROTECT_TYPE_GET);

  SOC_SAND_CHECK_NULL_INPUT(fec_protect_type);

  /*First check if the protected bitmap was initilized, if not rebuild it*/
  res = sw_state_access[unit].dpp.soc.arad.pp.fec.protected_bitmap_init.get(unit, &is_protected_init);
  SOC_SAND_CHECK_FUNC_RESULT(res, 5, exit);

  if (is_protected_init == 0) {
	  res = arad_pp_frwrd_fec_is_protected_init(unit);
	  SOC_SAND_CHECK_FUNC_RESULT(res, 5, exit);
  }
...
uint32
  arad_pp_frwrd_fec_is_protected_init(
    SOC_SAND_IN   int                       unit
  )
{
ARAD_PP_IHB_FEC_SUPER_ENTRY_TBL_DATA
...
ARAD_PP_FEC_TYPE
	...
	res = sw_state_access[unit].dpp.soc.arad.pp.fec.is_protected.bit_range_clear(unit, 0, ARAD_PP_FEC_PROTECT_BITMAP_SIZE);
	SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);

	/*Go over allocated fec's, set protected bit based on protection pointer configured in hw*/
	for(fec_ndx = 0; fec_ndx < SOC_DPP_DEFS_MAX(NOF_FECS); fec_ndx += 2) {

		res = sw_state_access[unit].dpp.soc.arad.pp.fec.fec_entry_type.get(unit, fec_ndx, &fec_type);
		SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
		...
		if (fec_type != ARAD_PP_NOF_FEC_TYPES) {
			...
			if (ihb_fec_super_entry_tbl_data.protection_pointer != ARAD_PP_FRWRD_FEC_PROTECT_PTR_VAL_NO_PROTECT){
				res = sw_state_access[unit].dpp.soc.arad.pp.fec.is_protected.bit_set(
					unit, 
					fec_ndx
					);
	...
	res = sw_state_access[unit].dpp.soc.arad.pp.fec.protected_bitmap_init.set(unit, 1);
	SOC_SAND_CHECK_FUNC_RESULT(res, 50, exit);

exit:
	SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_pp_frwrd_fec_is_protected_init()", 0, 0);
}
...
/* don't init this tables in Simulator */
  ...
  ARAD_ALLOC(glbl_info, ARAD_PP_FRWRD_FEC_GLBL_INFO, 1, "arad_pp_frwrd_fec_init_unsafe.glbl_info");
 /*
  ...
  )
{
  ...
  /*First check if the sw_state was initilized*/
  res = sw_state_access[unit].dpp.soc.arad.pp.fec.protected_bitmap_init.get(unit, &is_protected_init);
  SOC_SAND_CHECK_FUNC_RESULT(res, 5, exit);
  ...
  is_protected = (protect_type != ARAD_PP_FRWRD_FEC_PROTECT_TYPE_NONE) ? 1 : 0;
  ...
  )
{
  ...
  if (protect_fec != NULL && ARAD_PP_FRWRD_FEC_EVEN_FEC(fec_ndx) && protect_type != ARAD_PP_FRWRD_FEC_PROTECT_TYPE_NONE)
  {
  ...
  )
{
   ...
   * To prevent this state the left shift of key0 is set to 0 so bits [0:15] will be selected as key0 (for ARAD_PP_FEC_LB_KEY only).
   */
#ifdef BCM_88675_A0
  if (SOC_IS_JERICHO(unit)) {
      if (glbl_hash_info->hash_func_id == ARAD_PP_FEC_LB_KEY) {
...
void arad_pp_frwrd_fec_table_dma(int unit, uint32 start_index, uint32 nof_entries)
{
    soc_memstate_t *memState;
    int blk = PPDB_A_BLOCK(unit);
    int rv = BCM_E_NONE;
    int entry_dw = soc_mem_entry_words(unit, PPDB_A_FEC_SUPER_ENTRY_BANKm);

    memState = &SOC_MEM_STATE(unit, PPDB_A_FEC_SUPER_ENTRY_BANKm);
    rv = soc_mem_write_range(unit, PPDB_A_FEC_SUPER_ENTRY_BANKm, blk, start_index, (start_index + nof_entries -1), memState->cache[blk] + (start_index*entry_dw));
    if (rv != SOC_SAND_OK) {
        LOG_ERROR(BSL_LS_SOC_L3,
           (BSL_META_U(unit,
                       "%s(): Failed to dma write to PPDB_A_FEC_SUPER_ENTRY_BANK.\n"),
                       FUNCTION_NAME()));
    }
}
