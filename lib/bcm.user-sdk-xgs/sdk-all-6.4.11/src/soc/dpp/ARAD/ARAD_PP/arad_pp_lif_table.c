  ...
  )
{
  ...
  uint32
      sub_type;
  ARAD_PP_L2_LIF_PWE_ADDITIONAL_INFO	pwe_additional_info;
  ...
  switch (entry_type)
  {
    ...
    break;
    case ARAD_PP_LIF_ENTRY_TYPE_EXTENDER:
  ...
  )
{
  ...
  case ARAD_PP_LIF_ENTRY_TYPE_FIRST_AC_IN_GROUP:
  case ARAD_PP_LIF_ENTRY_TYPE_MIDDLE_AC_IN_GROUP:
    res = ARAD_PP_L2_LIF_AC_INFO_verify(unit, &(info->ac));
    SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  break;
  case ARAD_PP_LIF_ENTRY_TYPE_PWE:
    res = ARAD_PP_L2_LIF_PWE_INFO_verify(unit, &(info->pwe));
    SOC_SAND_CHECK_FUNC_RESULT(res, 11, exit);
  break;
  case ARAD_PP_LIF_ENTRY_TYPE_ISID:
    res = ARAD_PP_L2_LIF_ISID_INFO_verify(unit, &(info->isid));
  ...
  break;
  case ARAD_PP_LIF_ENTRY_TYPE_TRILL_NICK:
    res = ARAD_PP_L2_LIF_TRILL_INFO_verify(unit, &(info->trill));
    SOC_SAND_CHECK_FUNC_RESULT(res, 14, exit);
  break;
  case ARAD_PP_LIF_ENTRY_TYPE_EXTENDER:
    res = ARAD_PP_L2_LIF_EXTENDER_INFO_verify(unit, &(info->extender));
    SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
  break;
  default:
    break;
