...
#define ARAD_PP_LLP_PARSE_OUTER_TPID_MAX                         (ARAD_PP_NOF_LLP_PARSE_TPID_INDEXS-1)
#define ARAD_PP_LLP_PARSE_INNER_TPID_MAX                         (ARAD_PP_NOF_LLP_PARSE_TPID_INDEXS-1)
  ...
  Arad_pp_procedure_desc_element_llp_parse[] =
{
   ...
   */
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_LLP_PARSE_TPID_VALUES_SET),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_LLP_PARSE_TPID_VALUES_SET_PRINT),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_LLP_PARSE_TPID_VALUES_SET_UNSAFE),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_LLP_PARSE_TPID_VALUES_SET_VERIFY),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_LLP_PARSE_TPID_VALUES_GET),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_LLP_PARSE_TPID_VALUES_GET_PRINT),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_LLP_PARSE_TPID_VALUES_GET_VERIFY),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_LLP_PARSE_TPID_VALUES_GET_UNSAFE),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_LLP_PARSE_TPID_PROFILE_INFO_SET),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_LLP_PARSE_TPID_PROFILE_INFO_SET_PRINT),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_LLP_PARSE_TPID_PROFILE_INFO_SET_UNSAFE),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_LLP_PARSE_TPID_PROFILE_INFO_SET_VERIFY),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_LLP_PARSE_TPID_PROFILE_INFO_GET),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_LLP_PARSE_TPID_PROFILE_INFO_GET_PRINT),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_LLP_PARSE_TPID_PROFILE_INFO_GET_VERIFY),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_LLP_PARSE_TPID_PROFILE_INFO_GET_UNSAFE),
  ...
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_LLP_PARSE_PACKET_FORMAT_INFO_SET),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_LLP_PARSE_PACKET_FORMAT_INFO_SET_PRINT),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_LLP_PARSE_PACKET_FORMAT_INFO_SET_UNSAFE),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_LLP_PARSE_PACKET_FORMAT_INFO_SET_VERIFY),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_LLP_PARSE_PACKET_FORMAT_INFO_GET),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_LLP_PARSE_PACKET_FORMAT_INFO_GET_PRINT),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_LLP_PARSE_PACKET_FORMAT_INFO_GET_VERIFY),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_LLP_PARSE_PACKET_FORMAT_INFO_GET_UNSAFE),
  ...
  Arad_pp_error_desc_element_llp_parse[] =
{
  ...
  },
  {
    ...
    "The range is: 0 - ARAD_PP_NOF_LLP_PARSE_TPID_INDEXS-1.\n\r ",
    SOC_SAND_SVR_ERR,
    FALSE
  },
  {
   ...
   */
  {
    ...
    "ARAD_PP_LLP_PARSE_TPID_INFO.type must be SOC_SAND_PP_VLAN_TAG_TYPE_ANY.\n\r "
  ...
  },
  {
    ...
    "The range is: ARAD_PP_TRAP_CODE_ACCEPTABLE_FRAME_TYPE_DROP - .\n\r "
    "ARAD_PP_TRAP_CODE_ACCEPTABLE_FRAME_TYPE_CUSTOM_2.\n\r ",
    SOC_SAND_SVR_ERR,
    FALSE
  },
  {
...
STATIC uint8
  arad_pp_llp_parse_is_outer_s_tag(
    SOC_SAND_IN  ARAD_PP_LLP_PARSE_PACKET_FORMAT_INFO     *packet_format
  )
{
...
{ 
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_PP_LLP_PARSE_TPID_VALUES_SET_UNSAFE);
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_PP_LLP_PARSE_TPID_VALUES_SET_VERIFY);

  ARAD_PP_STRUCT_VERIFY(ARAD_PP_LLP_PARSE_TPID_VALUES, tpid_vals, 10, exit);
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_PP_LLP_PARSE_TPID_VALUES_GET_VERIFY);
...
{ 
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_PP_LLP_PARSE_TPID_VALUES_GET_UNSAFE);

  SOC_SAND_CHECK_NULL_INPUT(tpid_vals);

  ARAD_PP_LLP_PARSE_TPID_VALUES_clear(tpid_vals);
...
{ 
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_PP_LLP_PARSE_TPID_PROFILE_INFO_SET_UNSAFE);
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_PP_LLP_PARSE_TPID_PROFILE_INFO_SET_VERIFY);
  ...
  ARAD_PP_STRUCT_VERIFY(ARAD_PP_LLP_PARSE_TPID_PROFILE_INFO, tpid_profile_info, 20, exit);
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_PP_LLP_PARSE_TPID_PROFILE_INFO_GET_VERIFY);
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_PP_LLP_PARSE_TPID_PROFILE_INFO_GET_UNSAFE);

  SOC_SAND_CHECK_NULL_INPUT(tpid_profile_info);

  ARAD_PP_LLP_PARSE_TPID_PROFILE_INFO_clear(tpid_profile_info);
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_PP_LLP_PARSE_PACKET_FORMAT_INFO_SET_UNSAFE);
  ...
  llr_llvp_tbl.acceptable_frame_type_action = (format_info->action_trap_code - ARAD_PP_TRAP_CODE_ACCEPTABLE_FRAME_TYPE_DROP);
  ...
  if (!ARAD_PP_LLP_PARSE_ADVANCE_MODE(unit)) {
      ...
      llr_llvp_tbl.incoming_s_tag_exist = ARAD_PP_LLP_PARSE_IS_TAGGED(format_info->tag_format)  && 
          ((format_info->flags & ARAD_PP_LLP_PARSE_FLAGS_OUTER_C)==0);
  }
  ...
  for (parser_leaf_context = 0; parser_leaf_context < ARAD_PMF_PGM_SEL_PARSER_NOF_PARSER_LEAF_CONTEXT_VALS; parser_leaf_context++) 
  {
...
soc_error_t
  arad_pp_llp_parse_packet_format_eg_info_set(
   ...
   SOC_SAND_IN   uint32                                  port_profile_ndx,
   SOC_SAND_IN   ARAD_PP_LLP_PARSE_INFO                      *parse_key,
    SOC_SAND_INOUT  ARAD_PP_LLP_PARSE_PACKET_FORMAT_INFO        *format_info
  )
{
  ...
  if (!ARAD_PP_LLP_PARSE_ADVANCE_MODE(unit)) {
     ...
     is_outer_ctag = (format_info->flags & ARAD_PP_LLP_PARSE_FLAGS_OUTER_C)?1:0;
     is_inner_ctag = (format_info->flags & ARAD_PP_LLP_PARSE_FLAGS_INNER_C)?1:0;
     /* s-tag, if tagged and not ctag */
     is_outer_stag = ARAD_PP_LLP_PARSE_IS_TAGGED(format_info->tag_format)  && ((format_info->flags & ARAD_PP_LLP_PARSE_FLAGS_OUTER_C)==0);
  }
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_PP_LLP_PARSE_PACKET_FORMAT_INFO_SET_VERIFY);

  SOC_SAND_ERR_IF_ABOVE_MAX(port_profile_ndx, ARAD_PP_LLP_PARSE_PORT_PROFILE_NDX_MAX, ARAD_PP_LLP_PARSE_PORT_PROFILE_NDX_OUT_OF_RANGE_ERR, 10, exit);

  ARAD_PP_STRUCT_VERIFY(ARAD_PP_LLP_PARSE_INFO, parse_key, 20, exit);
  ARAD_PP_STRUCT_VERIFY(ARAD_PP_LLP_PARSE_PACKET_FORMAT_INFO, format_info, 30, exit);
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_PP_LLP_PARSE_PACKET_FORMAT_INFO_GET_VERIFY);
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_PP_LLP_PARSE_PACKET_FORMAT_INFO_GET_UNSAFE);

  SOC_SAND_CHECK_NULL_INPUT(parse_key);
  SOC_SAND_CHECK_NULL_INPUT(format_info);

  ARAD_PP_LLP_PARSE_PACKET_FORMAT_INFO_clear(format_info);
  ...
  format_info->action_trap_code = (ARAD_PP_TRAP_CODE)(llr_llvp_tbl.acceptable_frame_type_action + ARAD_PP_TRAP_CODE_ACCEPTABLE_FRAME_TYPE_DROP);

  res = arad_pp_ihp_vtt_llvp_tbl_get_unsafe(
          unit,
          ARAD_PP_TBL_IHP_VTT_LLVP_ENTRY_OFFSET(entry_offset,ARAD_PMF_PGM_SEL_PARSER_PROFILE_PP),
          &vtt_llvp_tbl
        );
  SOC_SAND_CHECK_FUNC_RESULT(res, 17, exit);
  ...
  )
{
  ...
  if (ARAD_PP_LLP_PARSE_ADVANCE_MODE(unit)) {
    ...
    if (epni_llvp_tbl.packet_has_c_tag == TRUE) {
       ...
       ARAD_PP_LLP_PARSE_FLAGS_OUTER_C : ARAD_PP_LLP_PARSE_FLAGS_INNER_C);
    }
  ...
  )
{
  ...
  /*
  for (ind = 0; ind < ARAD_PP_LLP_PARSE_NOF_TPID_VALS; ++ind)
  {
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in ARAD_PP_LLP_PARSE_TPID_VALUES_verify()",0,0);
}
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in ARAD_PP_LLP_PARSE_TPID_INFO_verify()",0,0);
}
  ...
  )
{
  ...
  ARAD_PP_STRUCT_VERIFY(ARAD_PP_LLP_PARSE_TPID_INFO, &(info->tpid1), 10, exit);
  ARAD_PP_STRUCT_VERIFY(ARAD_PP_LLP_PARSE_TPID_INFO, &(info->tpid2), 11, exit);

  SOC_SAND_MAGIC_NUM_VERIFY(info);

exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in ARAD_PP_LLP_PARSE_TPID_PROFILE_INFO_verify()",0,0);
}
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in ARAD_PP_LLP_PARSE_INFO_verify()",0,0);
}
  ...
  )
{
  ...
  SOC_SAND_ERR_IF_OUT_OF_RANGE(
    ...
    ARAD_PP_TRAP_CODE_ACCEPTABLE_FRAME_TYPE_DROP,
    ARAD_PP_TRAP_CODE_ACCEPTABLE_FRAME_TYPE_CUSTOM_2,
    ARAD_PP_LLP_PARSE_TRAP_CODE_OUT_OF_RANGE_ERR,
    20,
    exit
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in ARAD_PP_LLP_PARSE_PACKET_FORMAT_INFO_verify()",0,0);
}
