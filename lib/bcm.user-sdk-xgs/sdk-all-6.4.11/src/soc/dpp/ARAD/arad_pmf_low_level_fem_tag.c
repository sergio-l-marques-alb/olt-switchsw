...
/* { */
/*
 * Three arrays containing the INITIAL (default) values for the width per action.
...
/* } */
/* The following
 * three arrays contain the RUNTIME (current) values for the width per action.
 * Arrays are for Arad, Arad+ and Jericho.
 * For each action, there are three parameters:
  ...
  )
{
    ...
    *out_action_width = INTERNAL_TO_BCM_ACTION_NO_SPACE ;
    for (table_line = 0 ; table_line < num_actions_per_table ; table_line++) {
        ...
        if ((ARAD_PP_FP_ACTION_TYPE)loc_action == in_action_type) {
            found = TRUE ;
            /*
