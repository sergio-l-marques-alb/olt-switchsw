    ...
    if (oper == socDppScacheRetrieve) {
        ...
        if (storage_version < *recovered_ver) {
            ...
            if (flags & SOC_DPP_SCACHE_DOWNGRADE_INVALID) {
                LOG_ERROR(BSL_LS_SOC_SWDB,
                          (BSL_META_U(unit,
                                      SOC_DPP_MSG("Internal error.DOWNGRADE_INVALID.\n\r"))));
        ...
        if (alloc_size != allocated_size) {
            LOG_ERROR(BSL_LS_SOC_SWDB,
                      (BSL_META_U(unit,
                                  SOC_DPP_MSG("Not enough space in external storage \
                                              \n . \n\r"))));
			return(SOC_E_INTERNAL);
        }
        ...
        LOG_ERROR(BSL_LS_SOC_SWDB,
                  (BSL_META_U(unit,
                              SOC_DPP_MSG("Operation socDppScacheFreeCreate not supported.\n\r"))));
		return SOC_E_PARAM;
        ...
        LOG_ERROR(BSL_LS_SOC_SWDB,
                  (BSL_META_U(unit,
                              SOC_DPP_MSG("Operation not supported.\n\r"))));
		return SOC_E_PARAM;
		/* only re-alloc supported */
    }
