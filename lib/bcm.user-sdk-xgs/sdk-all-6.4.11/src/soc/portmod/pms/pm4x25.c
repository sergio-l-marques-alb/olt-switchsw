...
int pm4x25_wb_buffer_init(int unit, int wb_buffer_index, pm_info_t pm_info)
{
    ...
    SOC_WB_ENGINE_ADD_BUFF(SOC_WB_ENGINE_PORTMOD, wb_buffer_index, "pm4x25", NULL, VERSION(3), 1, SOC_WB_ENGINE_PRE_RELEASE);
    _SOC_IF_ERR_EXIT(rv);
    ...
    _SOC_IF_ERR_EXIT(portmod_next_wb_var_id_get(unit, &wb_var_id));
    SOC_WB_ENGINE_ADD_VAR(SOC_WB_ENGINE_PORTMOD, wb_var_id, "interface_config", wb_buffer_index, sizeof(portmod_port_interface_config_t), NULL, VERSION(3));
    _SOC_IF_ERR_EXIT(rv);
    pm_info->wb_vars_ids[interfaceConfig] = wb_var_id;

    _SOC_IF_ERR_EXIT(portmod_next_wb_var_id_get(unit, &wb_var_id));
    SOC_WB_ENGINE_ADD_VAR(SOC_WB_ENGINE_PORTMOD, wb_var_id, "port_init_config", wb_buffer_index, sizeof(portmod_port_init_config_t), NULL, VERSION(3));
    _SOC_IF_ERR_EXIT(rv);
                   ...
                   int wb_buffer_index, pm_info_t pm_info)
{
    ...
    if(SOC_WARM_BOOT(unit)){
        ...
        /*WB restore, dpp_mode (restoring at pm_init rather than at port_probe*/
            ...
            for(i = 0; i < MAX_PORTS_PER_PM4X25; i++) {
                ...
                if(tmp_port != -1){
                    ...
                    rv = SOC_WB_ENGINE_GET_VAR(unit, SOC_WB_ENGINE_PORTMOD,
                               pm_info->wb_vars_ids[interfaceConfig], &PM_4x25_INFO(pm_info)->port_config[i].interface_config);
                    _SOC_IF_ERR_EXIT(rv);

                    rv = SOC_WB_ENGINE_GET_VAR(unit, SOC_WB_ENGINE_PORTMOD,
                               pm_info->wb_vars_ids[portInitConfig], &PM_4x25_INFO(pm_info)->port_config[i].port_init_config);
                    _SOC_IF_ERR_EXIT(rv);
                }
            }
            for(i = 0 ; i < PM4X25_LANES_PER_CORE ; i++){
                rv = SOC_WB_ENGINE_GET_ARR (unit, SOC_WB_ENGINE_PORTMOD,
                              pm_info->wb_vars_ids[nofPhys], &(PM_4x25_INFO(pm_info)->nof_phys[i]), i);
                _SOC_IF_ERR_EXIT(rv);
            }
        }
        ...
        for(i = 0 ; i < PM4X25_LANES_PER_CORE ; i++){
            rv = SOC_WB_ENGINE_SET_ARR(unit, SOC_WB_ENGINE_PORTMOD,
                         ...
                         &(PM_4x25_INFO(pm_info)->nof_phys[i]), i);
             _SOC_IF_ERR_EXIT(rv);
        }
    }
...
STATIC
int _pm4x25_an_mode_update (int unit, int port, pm_info_t pm_info, int port_index, int num_lanes)
{
    portmod_port_init_config_t *init_config = &PM_4x25_INFO(pm_info)->port_config[port_index].port_init_config;
    soc_error_t rv = SOC_E_NONE;
    SOC_INIT_FUNC_DEFS;

    if(init_config->is_hg) {
        init_config->an_mode = phymod_AN_MODE_CL73BAM;
    } else if (init_config->an_cl73) {
        switch (init_config->an_cl73) {
            ...
            case PORTMOD_CL73_HPAM_VS_SW:
                init_config->an_mode = phymod_AN_MODE_HPAM;
                break ;  /* against TD+ */
        ...
        init_config->an_mode = phymod_AN_MODE_CL73;  /* default mode */
    }
    ...
    rv = SOC_WB_ENGINE_SET_VAR(unit, SOC_WB_ENGINE_PORTMOD,
         pm_info->wb_vars_ids[portInitConfig], &PM_4x25_INFO(pm_info)->port_config[port_index].port_init_config);
    _SOC_IF_ERR_EXIT(rv);
...
int pm4x25_port_attach(int unit, int port, pm_info_t pm_info, const portmod_port_add_info_t* add_info)
{
    ...
    if (!PORTMOD_PORT_ADD_F_AUTONEG_CONFIG_SKIP_GET(add_info))
    {
        ...
        SOC_IF_ERROR_RETURN(_pm4x25_an_mode_update (unit, port, pm_info, port_index, add_info->interface_config.port_num_lanes));
    }
    ...
    /*update wb_engine*/
    rv = SOC_WB_ENGINE_SET_VAR(unit, SOC_WB_ENGINE_PORTMOD,
         pm_info->wb_vars_ids[interfaceConfig], &PM_4x25_INFO(pm_info)->port_config[port_index].interface_config);
    _SOC_IF_ERR_EXIT(rv);
...
{
	SOC_INIT_FUNC_DEFS;
	SOC_PBMP_ASSIGN(*phys, PM_4x25_INFO(pm_info)->phys);
	SOC_FUNC_RETURN;
}
...
int pm4x25_port_enable_get(int unit, int port, pm_info_t pm_info, int flags, int* enable)
{
    ...
    if ((PORTMOD_PORT_ENABLE_MAC_GET(flags)||(0 == flags)) && (!is_bypassed)) {
        ...
        _SOC_IF_ERR_EXIT(clmac_enable_get(unit, port, CLMAC_ENABLE_SET_FLAGS_RX_EN, enable));
    } else if (PORTMOD_PORT_ENABLE_TX_GET(flags) && (!is_bypassed)) {
        _SOC_IF_ERR_EXIT(clmac_enable_get(unit, port, CLMAC_ENABLE_SET_FLAGS_TX_EN, enable));
    }
    if(0 == flags){
                                     ...
                                     int all_phy)
{
    ...
    if(!pm_is_bypassed){
        ...
        if (config->speed < 10000) {
            rv = clmac_strict_preamble_set (unit, port, 0);
            _SOC_IF_ERR_EXIT(rv);
        }
    ...
    if(!(PM_4x25_INFO(pm_info)->in_pm12x10)){
        ...
        _SOC_IF_ERR_EXIT(portmod_line_intf_from_config_get(config, &interface));
        _SOC_IF_ERR_EXIT(portmod_intf_to_phymod_intf(unit, config->speed, 
                interface, &phy_config.interface_type));
                            ...
                            const phymod_autoneg_control_t* an)
{
    ...
    _SOC_IF_ERR_EXIT(pm4x25_port_ability_local_get (unit, port, pm_info, &ability));
    ...
    if(PM_4x25_INFO(pm_info)->nof_phys == 0) {
        ...
        /* change cl72 config */
        if (PM_4x25_INFO(pm_info)->port_config[port_index].port_init_config.fs_cl72) {
...
{
    return (_clport_rx_control_set(unit, port, rx_ctrl->pass_control_frames));
}
...
{   
    ...
    rv = SOC_WB_ENGINE_SET_ARR(unit, SOC_WB_ENGINE_PORTMOD,
                 pm_info->wb_vars_ids[nofPhys],
                 &(PM_4x25_INFO(pm_info)->nof_phys[lane_index]), lane_index);
     _SOC_IF_ERR_EXIT(rv);
