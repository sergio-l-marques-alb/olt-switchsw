...
int pm4x10_wb_buffer_init(int unit, int wb_buffer_index, pm_info_t pm_info)
{
    ...
    SOC_WB_ENGINE_ADD_BUFF(SOC_WB_ENGINE_PORTMOD, wb_buffer_index, "pm4x10", NULL, VERSION(2), 1, SOC_WB_ENGINE_PRE_RELEASE);
    _SOC_IF_ERR_EXIT(rv);
    ...
    _SOC_IF_ERR_EXIT(portmod_next_wb_var_id_get(unit, &wb_var_id));
    SOC_WB_ENGINE_ADD_VAR(SOC_WB_ENGINE_PORTMOD, wb_var_id, "interface_config", wb_buffer_index, sizeof(portmod_port_interface_config_t), NULL, VERSION(2));
    _SOC_IF_ERR_EXIT(rv);
    pm_info->wb_vars_ids[interfaceConfig] = wb_var_id;

    _SOC_IF_ERR_EXIT(portmod_next_wb_var_id_get(unit, &wb_var_id));
    SOC_WB_ENGINE_ADD_VAR(SOC_WB_ENGINE_PORTMOD, wb_var_id, "port_init_config", wb_buffer_index, sizeof(portmod_port_init_config_t), NULL, VERSION(2));
    _SOC_IF_ERR_EXIT(rv);
    pm_info->wb_vars_ids[portInitConfig] = wb_var_id;

    _SOC_IF_ERR_EXIT(portmod_next_wb_var_id_get(unit, &wb_var_id));
    SOC_WB_ENGINE_ADD_VAR(SOC_WB_ENGINE_PORTMOD, wb_var_id, "phy_init_config", wb_buffer_index, sizeof(phymod_phy_init_config_t), NULL, VERSION(2));
    _SOC_IF_ERR_EXIT(rv);
    pm_info->wb_vars_ids[phyInitConfig] = wb_var_id;

    _SOC_IF_ERR_EXIT(portmod_next_wb_var_id_get(unit, &wb_var_id));
    SOC_WB_ENGINE_ADD_VAR(SOC_WB_ENGINE_PORTMOD, wb_var_id, "phy_interface_config", wb_buffer_index, sizeof(phymod_phy_inf_config_t), NULL, VERSION(2));
    _SOC_IF_ERR_EXIT(rv);
                   ...
                   int wb_buffer_index, pm_info_t pm_info)
{
   ...
   /* initialize num of phys for each lane */
    ...
    if(SOC_WARM_BOOT(unit)){
        ...
        /*WB restore, dpp_mode (restoring at pm_init rather than at port_attach*/
        if (SOC_IS_JERICHO(unit)) {
            for( i = 0 ; i < MAX_PORTS_PER_PM4X10; i++) {
                ...
                if(tmp_port != -1){
                    /*For dpp devices, that supports dynamic changes - restore here the port_config[port_index] db*/
                    rv = SOC_WB_ENGINE_GET_VAR(unit, SOC_WB_ENGINE_PORTMOD,
                               pm_info->wb_vars_ids[interfaceConfig], &PM_4x10_INFO(pm_info)->port_config[i].interface_config);
                    _SOC_IF_ERR_EXIT(rv);

                    rv = SOC_WB_ENGINE_GET_VAR(unit, SOC_WB_ENGINE_PORTMOD,
                               pm_info->wb_vars_ids[portInitConfig], &PM_4x10_INFO(pm_info)->port_config[i].port_init_config);
                    _SOC_IF_ERR_EXIT(rv);

                    rv = SOC_WB_ENGINE_GET_VAR(unit, SOC_WB_ENGINE_PORTMOD,
                               pm_info->wb_vars_ids[phyInitConfig], &PM_4x10_INFO(pm_info)->port_config[i].phy_init_config);
                    _SOC_IF_ERR_EXIT(rv);

                    rv = SOC_WB_ENGINE_GET_VAR(unit, SOC_WB_ENGINE_PORTMOD,
                               pm_info->wb_vars_ids[phyInterfaceConfig], &PM_4x10_INFO(pm_info)->port_config[i].phy_interface_config);
                    _SOC_IF_ERR_EXIT(rv);

                    rv = SOC_WB_ENGINE_GET_VAR(unit, SOC_WB_ENGINE_PORTMOD,
                               pm_info->wb_vars_ids[portDynamicState], &PM_4x10_INFO(pm_info)->port_config[i].port_dynamic_state);
                    _SOC_IF_ERR_EXIT(rv);
                }
            }
            for(i = 0 ; i < PM4X10_LANES_PER_CORE ; i++){
                rv = SOC_WB_ENGINE_GET_ARR (unit, SOC_WB_ENGINE_PORTMOD,
                              pm_info->wb_vars_ids[nofPhys], &(PM_4x10_INFO(pm_info)->nof_phys[i]), i);
                _SOC_IF_ERR_EXIT(rv);
            }
        ...
        for(i = 0 ; i < PM4X10_LANES_PER_CORE ; i++){
            rv = SOC_WB_ENGINE_SET_ARR(unit, SOC_WB_ENGINE_PORTMOD,
                         pm_info->wb_vars_ids[nofPhys],
                         &(PM_4x10_INFO(pm_info)->nof_phys[i]), i);
             _SOC_IF_ERR_EXIT(rv);
        }
    }
                                      ...
                                      phymod_phy_init_config_t *phy_init_config)
{
    ...
    if( nof_phys > 1 ){
        ...
        if (!is_legacy_present) {
            ...
            rv = phymod_phy_media_type_tx_get(&phy_access_arr[nof_phys-1], phymodMediaTypeChipToChip, &phymod_tx);
            if (SOC_FAILURE(rv)) return (rv);
        }
        /* overlap external phy  preemphasis and amp from config */
        if(init_config->ext_phy_tx_params_user_flag & PORTMOD_USER_SET_TX_PREEMPHASIS_BY_CONFIG) {
            phymod_tx.pre  = init_config->ext_phy_tx_params.pre ;
            phymod_tx.main = init_config->ext_phy_tx_params.main ;
            phymod_tx.post = init_config->ext_phy_tx_params.post ;
        }
        if(init_config->ext_phy_tx_params_user_flag & PORTMOD_USER_SET_TX_AMP_BY_CONFIG) {
            phymod_tx.amp  = init_config->ext_phy_tx_params.amp ;
        }
    ...
    rv = phymod_phy_media_type_tx_get(&phy_access_arr[0], phymodMediaTypeChipToChip, &phymod_tx);
    if (SOC_FAILURE(rv)) return (rv);
            ...
            if( serdes_lane != -1){    
                ...
                if(xphy_id != PORTMOD_XPHY_ID_INVALID){
                    ...
                    sal_memcpy (&phy_access[num_of_phys], &xphy_core_info.core_access,
                                sizeof(phymod_phy_access_t));
                    xphy_lane = ( params->sys_side == PORTMOD_SIDE_SYSTEM)? PM_4x10_INFO(pm_info)->lane_conn[xphy_idx][serdes_lane].ss_lane:
                                  PM_4x10_INFO(pm_info)->lane_conn[xphy_idx][serdes_lane].ls_lane;
                    if( xphy_lane != -1 ){
                ...
                if(xphy_id != PORTMOD_XPHY_ID_INVALID){
                    ...
                    if(num_of_phys == 0 ) {/* first one */
                        ...
                        xphy_lane = ( params->sys_side == PORTMOD_SIDE_SYSTEM)? 
                                    PM_4x10_INFO(pm_info)->lane_conn[xphy_idx][serdes_lane].ss_lane:
                                    PM_4x10_INFO(pm_info)->lane_conn[xphy_idx][serdes_lane].ls_lane;
                        if( xphy_lane != -1) {
...
*/
STATIC
void _pm4x10_port_tx_config_init_set(phymod_tx_t *tx_params_default, phymod_tx_t *tx_params_config,
                                            int config_flag, phymod_phy_init_config_t   *phy_init_config, int is_ext_phy)
{
    int i = 0;

    /* overlap preemphasis and amp from config */
    if(config_flag & PORTMOD_USER_SET_TX_PREEMPHASIS_BY_CONFIG) {
        tx_params_default->pre  = tx_params_config->pre ;
        tx_params_default->main = tx_params_config->main ;
        tx_params_default->post = tx_params_config->post ;
    }

    if(config_flag & PORTMOD_USER_SET_TX_AMP_BY_CONFIG) {
        tx_params_default->amp  = tx_params_config->amp ;
    }

    for(i=0 ; i<PM4X10_LANES_PER_CORE ; i++) {
        if(is_ext_phy) {
            phy_init_config->ext_phy_tx_params_user_flag[i] = config_flag;
            phy_init_config->ext_phy_tx[i].pre  = tx_params_default->pre  ;
            phy_init_config->ext_phy_tx[i].main = tx_params_default->main ;
            phy_init_config->ext_phy_tx[i].post = tx_params_default->post ;
            phy_init_config->ext_phy_tx[i].post2= tx_params_default->post2;
            phy_init_config->ext_phy_tx[i].post3= tx_params_default->post3;
            phy_init_config->ext_phy_tx[i].amp  = tx_params_default->amp  ;
        }
        else {
            phy_init_config->tx_params_user_flag[i] = config_flag;
            phy_init_config->tx[i].pre  = tx_params_default->pre  ;
            phy_init_config->tx[i].main = tx_params_default->main ;
            phy_init_config->tx[i].post = tx_params_default->post ;
            phy_init_config->tx[i].post2= tx_params_default->post2;
            phy_init_config->tx[i].post3= tx_params_default->post3;
            phy_init_config->tx[i].amp  = tx_params_default->amp  ;
        }
    }
}
                        ...
                        const portmod_port_interface_config_t* config)
{
    ...
    {   /* external phy. */
        ...
        _pm4x10_port_tx_config_init_set(&phymod_tx, &init_config->ext_phy_tx_params,
                                        init_config->ext_phy_tx_params_user_flag, phy_init_config, is_ext_phy);
    }
    else
    {   /* internal phy. */
    ...
    /* overlap serdes preemphasis and amp from config */
    _pm4x10_port_tx_config_init_set(&phymod_tx, &init_config->tx_params,
                                        init_config->tx_params_user_flag, phy_init_config, is_ext_phy);

    /* set all phys tx params to phymod_tx. internal and intermediary phy will be modify in portmod_port_phychain_phy_init*/
    ...
    /*update wb_engine*/
    rv = SOC_WB_ENGINE_SET_VAR(unit, SOC_WB_ENGINE_PORTMOD,
       pm_info->wb_vars_ids[phyInitConfig], &PM_4x10_INFO(pm_info)->port_config[port_index].phy_init_config);
    _SOC_IF_ERR_EXIT(rv);

    rv = SOC_WB_ENGINE_SET_VAR(unit, SOC_WB_ENGINE_PORTMOD,
       pm_info->wb_vars_ids[phyInterfaceConfig], &PM_4x10_INFO(pm_info)->port_config[port_index].phy_interface_config);
    _SOC_IF_ERR_EXIT(rv);
                ...
                for(i=0 ; i<nof_ext_phys ; i++) {
                    ...
                    if(xphy_id != PORTMOD_XPHY_ID_INVALID){
                        ...
                        if(SOC_E_NONE != (_pm4x10_core_probe(unit,port, pm_info, &xphy_core_info.core_access, &probe, 1))) {
                            ...
                            rv = SOC_WB_ENGINE_SET_ARR(unit, SOC_WB_ENGINE_PORTMOD,
                                         pm_info->wb_vars_ids[nofPhys],
                                         &(PM_4x10_INFO(pm_info)->nof_phys[temp]), temp);
                             _SOC_IF_ERR_EXIT(rv);
...
int _pm4x10_pm_core_init (int unit, pm_info_t pm_info, const portmod_port_add_info_t* add_info)
{
    ...
    uint32 core_is_initialized, is_initialized=0;
    int lane;
    int init_all;
	uint32 init_flags;
	soc_port_if_t interface = SOC_PORT_IF_NULL;
                        ...
                        const portmod_port_add_info_t* add_info)
{
    ...
    /*update wb_engine*/
    rv = SOC_WB_ENGINE_SET_VAR(unit, SOC_WB_ENGINE_PORTMOD,
         pm_info->wb_vars_ids[interfaceConfig], &PM_4x10_INFO(pm_info)->port_config[port_index].interface_config);
    _SOC_IF_ERR_EXIT(rv);

    rv = SOC_WB_ENGINE_SET_VAR(unit, SOC_WB_ENGINE_PORTMOD,
         pm_info->wb_vars_ids[portInitConfig], &PM_4x10_INFO(pm_info)->port_config[port_index].port_init_config);
    _SOC_IF_ERR_EXIT(rv);
...
int _pm4x10_an_mode_update (int unit, int port, pm_info_t pm_info)
{
    ...
    if (init_config->an_cl72 == 0) {
        init_config->an_cl72 = an_cl72;
    }
                                                  ...
                                                  phymod_phy_inf_config_t *phy_interface_config)
{
    ...
    if (interface_config->interface == _SHR_PORT_IF_ILKN){
        ...
        switch (interface_config->speed) {
            ...
            case 10000:
                if (port_num_lanes == 4) {
                   ...
                   if( fiber_pref) {
                    ...
                    } else if(interface_config->interface==SOC_PORT_IF_SFI) {
                        phy_interface_config->interface_type = phymodInterfaceSFI ;
                    ...
                    } else if(interface_config->interface==SOC_PORT_IF_LR) {
                        phy_interface_config->interface_type = phymodInterfaceLR;
                    } else if(interface_config->interface==SOC_PORT_IF_LRM) {
            ...
            case 40000:
                ...
                }else if(interface == SOC_PORT_IF_XGMII){
                    phy_interface_config->interface_type = phymodInterfaceXGMII;
                }else{
                ...
                if (port_is_higig) {
                    if (port_num_lanes == 2) {
                        if(interface_config->interface == SOC_PORT_IF_KR2) {
                            phy_interface_config->interface_type = phymodInterfaceKR2;
                        } else if (interface_config->interface == SOC_PORT_IF_SR2) {
            ...
            case 100000:
                ...
                break;
            case 106000:
                if(interface_config->interface == SOC_PORT_IF_CR10) {
                    phy_interface_config->interface_type = phymodInterfaceCR10;
                } else if(interface_config->interface == SOC_PORT_IF_SR4) {
                    phy_interface_config->interface_type = phymodInterfaceSR4;
    ...
    switch (interface_config->speed) {
        case 10000:
            if (port_num_lanes == 4) {
                ...
                if(interface==SOC_PORT_IF_SFI)  {
                    ...
                    phy_interface_config->interface_type = phymodInterfaceLRM;
                } else if(interface_config->interface==SOC_PORT_IF_ER) {
                    phy_interface_config->interface_type = phymodInterfaceER;
                } else if(interface_config->interface==SOC_PORT_IF_ER2) {
        ...
        case 40000:
            if(port_is_higig) {
                ...
                phy_interface_config->interface_type = phymodInterfaceCR2;
            }else if(interface_config->interface == SOC_PORT_IF_LR2){
                phy_interface_config->interface_type = phymodInterfaceLR2;
            }else if(interface_config->interface == SOC_PORT_IF_KR2){
                phy_interface_config->interface_type = phymodInterfaceKR2;
            }else if(interface_config->interface==SOC_PORT_IF_XLPPI) {
                phy_interface_config->interface_type = phymodInterfaceXLPPI;
            }else if(interface_config->interface==SOC_PORT_IF_XLAUI2) {
                phy_interface_config->interface_type = phymodInterfaceXLAUI2;
            } else{
                phy_interface_config->interface_type = phymodInterfaceXLAUI;
            }
        ...
        case 100000:
            if(interface == SOC_PORT_IF_SR4){
                ...
                phy_interface_config->interface_type = phymodInterfaceCR4;
            } else if(interface_config->interface == SOC_PORT_IF_ER4){
                        ...
                        int phy_init_flags)
{
    ...
    is_higig =  ((config->encap_mode == SOC_ENCAP_HIGIG2) ||  
                 (config->encap_mode == SOC_ENCAP_HIGIG)  ||   
                 PHYMOD_INTF_MODES_HIGIG_GET(config)) ? 1 : 0;
    _SOC_IF_ERR_EXIT(portmod_port_interface_type_verify(config->speed, config->port_num_lanes,
                                                        is_higig, config->interface));
                                   ...
                                   portmod_port_ability_t* ability)
{
     ...
     * COVERITY
     *
     * portmod_port_chain_phy_access_get already checks return value.
     */
                             ...
                             const phymod_autoneg_control_t* an)
{
    ...
    _SOC_IF_ERR_EXIT(pm4x10_port_ability_local_get (unit, port, pm_info, &ability));

    if ((ability.flags != SOC_PA_AUTONEG) && (pAn->enable != 0)) {
    ...
    /*update wb_engine*/
    rv = SOC_WB_ENGINE_SET_VAR(unit, SOC_WB_ENGINE_PORTMOD,
       pm_info->wb_vars_ids[portDynamicState], &PM_4x10_INFO(pm_info)->port_config[port_index].port_dynamic_state);
    _SOC_IF_ERR_EXIT(rv);
...
{
    return (_xlport_rx_control_set(unit, port, rx_ctrl->pass_control_frames));
}
...
{   
    ...
    if (PM_4x10_INFO(pm_info)->nof_phys[lane_index] <= phyn){
        ...
        rv = SOC_WB_ENGINE_SET_ARR(unit, SOC_WB_ENGINE_PORTMOD,
                     pm_info->wb_vars_ids[nofPhys],
                     &(PM_4x10_INFO(pm_info)->nof_phys[lane_index]), lane_index);
         _SOC_IF_ERR_EXIT(rv);
    }
