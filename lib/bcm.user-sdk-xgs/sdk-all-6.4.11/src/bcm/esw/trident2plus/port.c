...
#if defined(BCM_TRIUMPH3_SUPPORT) && defined(INCLUDE_L3)
    /* See bcm_tr_ipmc_detach, bcm_tr3_ipmc_repl_detach for more information */
...
_bcm_td2p_port_dscp_table_default_add(int unit, bcm_port_t port)
{
    ...
    entries[0] = &dscp_map_entries;
    BCM_IF_ERROR_RETURN
        (_bcm_dscp_table_entry_add(unit, entries, DSCP_CODE_POINT_CNT,
                                   &dscp_index));
    BCM_IF_ERROR_RETURN
        (_bcm_esw_port_tab_set(unit, port,
                               _BCM_CPU_TABS_ETHER,
                               TRUST_DSCP_PTRf,
                               dscp_index / DSCP_CODE_POINT_CNT));
...
_bcm_td2_port_basic_detach(int unit, bcm_port_t port)
{
    ...
    profile_index = value * DSCP_CODE_POINT_CNT;
    BCM_IF_ERROR_RETURN
        (_bcm_dscp_table_entry_delete(unit, profile_index));
...
/*
 * Function:
 *      bcm_td2p_port_vlan_priority_unmap_set
 * Description:
...
bcm_td2p_port_vlan_priority_unmap_set(int unit, bcm_port_t port,
                                     ...
                                     int pkt_pri, int cfi)
{
    egr_pri_cng_map_entry_t  pri_unmap_tab[_BCM_EGR_PRI_CNG_MAP_CHUNK];
    egr_map_mh_entry_t       egr_map;
    void         *entries[1];
    void         *entry;
    int          offset, index;
    int          old_pri_unmap_idx, new_pri_unmap_idx;

    if ((internal_pri < 0 || internal_pri >= (1 << 4)) ||
    ...
    sal_memset(pri_unmap_tab, 0, sizeof(pri_unmap_tab));
    sal_memset(&egr_map, 0, sizeof(egr_map));
    entries[0] = &pri_unmap_tab;
    BCM_IF_ERROR_RETURN(_bcm_esw_egr_port_tab_get(unit,
                            port, EGR_QOS_PROFILE_INDEXf, &old_pri_unmap_idx));

    old_pri_unmap_idx = _BCM_EGR_PRI_CNG_MAP_CHUNK * old_pri_unmap_idx;

    BCM_IF_ERROR_RETURN(soc_profile_mem_get(unit, egr_pri_cng_profile[unit],
                                            old_pri_unmap_idx,
                                            _BCM_EGR_PRI_CNG_MAP_CHUNK, entries));

    offset = (internal_pri << 2) | _BCM_COLOR_ENCODING(unit, color);
    entry = &pri_unmap_tab[offset];

    soc_mem_field32_set(unit, EGR_PRI_CNG_MAPm, entry, PRIf, pkt_pri);
    soc_mem_field32_set(unit, EGR_PRI_CNG_MAPm, entry, CFIf, cfi);

    BCM_IF_ERROR_RETURN(soc_profile_mem_add(unit, egr_pri_cng_profile[unit], entries,
                           _BCM_EGR_PRI_CNG_MAP_CHUNK, (uint32 *)&new_pri_unmap_idx));
    if ((new_pri_unmap_idx != old_pri_unmap_idx)) {
        BCM_IF_ERROR_RETURN(soc_profile_mem_delete(unit, egr_pri_cng_profile[unit],
                                                   old_pri_unmap_idx));
    }
    /* coverity[stack_use_callee] */
    /* coverity[stack_use_overflow] */
    BCM_IF_ERROR_RETURN(_bcm_esw_egr_port_tab_set(unit, port,
                 EGR_QOS_PROFILE_INDEXf,new_pri_unmap_idx/_BCM_EGR_PRI_CNG_MAP_CHUNK));
    BCM_IF_ERROR_RETURN(_bcm_esw_port_gport_validate(unit, port, &port));
    index = (port << 4) | (internal_pri);
...
/*
 * Function:
 *      bcm_td2p_port_vlan_priority_unmap_get
 * Description:
                                     ...
                                     int *pkt_pri, int *cfi)
{
    ...
    sal_memset(pri_unmap_tab, 0, sizeof(pri_unmap_tab));
    entries[0] = &pri_unmap_tab;
    BCM_IF_ERROR_RETURN(_bcm_esw_egr_port_tab_get(unit, port,
                                     EGR_QOS_PROFILE_INDEXf, &pri_unmap_idx));
    pri_unmap_idx = _BCM_EGR_PRI_CNG_MAP_CHUNK * pri_unmap_idx;
    BCM_IF_ERROR_RETURN(soc_profile_mem_get(unit, egr_pri_cng_profile[unit],
                                            pri_unmap_idx,
                                            _BCM_EGR_PRI_CNG_MAP_CHUNK,
                                            entries));
    offset = (internal_pri << 2) | _BCM_COLOR_ENCODING(unit, color);
    entry = &pri_unmap_tab[offset];
    *pkt_pri = soc_mem_field32_get(unit, EGR_PRI_CNG_MAPm, entry, PRIf);
    *cfi = soc_mem_field32_get(unit, EGR_PRI_CNG_MAPm, entry, CFIf);
    return SOC_E_NONE;
}
/*
 * Function:
 *      bcm_td2p_port_vlan_priority_unmap_init
 * Description:
 *      Initialize the EGR_PRI_CNG_MAP table. Since 
 *      the defult priority and cfi bit of the outgoing packets
 *      are the same, it only need to write the first part of 
 *      EGR_PRI_CNG_MAP table.
 * Parameters:
 *      unit         - (IN) Device number
 * Return Value:
 *      BCM_E_XXX
...
{
    egr_pri_cng_map_entry_t pri_unmap[_BCM_EGR_PRI_CNG_MAP_CHUNK];
    egr_map_mh_entry_t  egr_map;
    int index, priority, cfi, i, color;
    bcm_color_t  color_array[] = {bcmColorGreen, bcmColorYellow, bcmColorRed};
    bcm_port_t   port;
    void         *entries[1];
    bcm_pbmp_t   all_pbmp;
    ...
    /*Initialize EGR_PRI_CNG_MAP profile table*/
    if (egr_pri_cng_profile[unit] == NULL) {
        egr_pri_cng_profile[unit] = sal_alloc(sizeof(soc_profile_mem_t),
                                          "EGR_PRI_CNG_MAP Profile Mem");
        if (egr_pri_cng_profile[unit] == NULL) {
            return BCM_E_MEMORY;
        }
        soc_profile_mem_t_init(egr_pri_cng_profile[unit]);
    }
    mem = EGR_PRI_CNG_MAPm;
    entry_words = sizeof(egr_pri_cng_map_entry_t) / sizeof(uint32);
    BCM_IF_ERROR_RETURN(soc_profile_mem_create(unit, &mem, &entry_words, 1,
                                                egr_pri_cng_profile[unit]));

#ifdef BCM_WARM_BOOT_SUPPORT
    if (SOC_WARM_BOOT(unit)) {
       PBMP_ITER(all_pbmp,port) {
          BCM_IF_ERROR_RETURN
                    (_bcm_esw_egr_port_tab_get(unit,  port,
                          EGR_QOS_PROFILE_INDEXf, &tmp_index));
       index = (tmp_index * _BCM_EGR_PRI_CNG_MAP_CHUNK);
       for (i = 0; i < _BCM_EGR_PRI_CNG_MAP_CHUNK; i++)  {
              SOC_PROFILE_MEM_REFERENCE(unit, egr_pri_cng_profile[unit], index+i, 1);
              SOC_PROFILE_MEM_ENTRIES_PER_SET(unit, egr_pri_cng_profile[unit], index+i,
                          _BCM_EGR_PRI_CNG_MAP_CHUNK);
          }
...
#endif /* BCM_WARM_BOOT_SUPPORT */
    {
        ...
        for (i = 0; i < 3; i++) {
            ...
            for (priority = 0; priority <= 7; priority++) {
                cfi = (color == bcmColorRed) ? 1 : 0;
                index = (priority << 2) | _BCM_COLOR_ENCODING(unit, color);
                soc_mem_field32_set(unit, EGR_PRI_CNG_MAPm, &pri_unmap[index], PRIf,
                                    priority);
                soc_mem_field32_set(unit, EGR_PRI_CNG_MAPm, &pri_unmap[index],
                                    CFIf, cfi);
            }
        }
        PBMP_ITER(all_pbmp, port) {
            BCM_IF_ERROR_RETURN(soc_profile_mem_add(unit, egr_pri_cng_profile[unit], 
                                                    entries, _BCM_EGR_PRI_CNG_MAP_CHUNK,
                                                    (uint32 *)&index));
            BCM_IF_ERROR_RETURN(_bcm_esw_egr_port_tab_set(unit, port,
                              EGR_QOS_PROFILE_INDEXf,index/_BCM_EGR_PRI_CNG_MAP_CHUNK));
            /*Initialize EGR_MAP_MH table*/
            for (priority = 0; priority <= 7; priority++) {
                index = (port << 4) | priority;
                soc_mem_field32_set(unit, EGR_MAP_MHm, &egr_map, HG_TCf, priority);
                SOC_IF_ERROR_RETURN(WRITE_EGR_MAP_MHm(unit, MEM_BLOCK_ALL, index, &egr_map));
            }
        }
    }
                                        ...
                                        soc_mem_t mem, soc_field_t field, uint32 value)
{
    ...
    soc_mem_field32_set(0, mem, entries[table_id], field, value);
