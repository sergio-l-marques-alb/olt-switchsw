    ...
    *prio = 
    ...
    for (word_idx = 0; word_idx < word_count; word_idx++) {
        ...
        for (bit_idx = 0; bit_idx < bit_count; bit_idx++) {
           ...
           for (table_idx = 0; table_idx < enc_count; table_idx++) {
               for (idx = 0; idx < enc_table_len[table_idx] ; idx++) {
                   if ( cpu_cos_map_key[key_idx] == cpu_opcode_enc[table_idx][idx] )  {
                       if (maskbitset) { /* should always be set */
                           enc_mask[table_idx] |= (1 << idx);
                       }
                       if (keybitset) {
                           enc_key[table_idx] |= (1 << idx);
                       }
                   }
               }
           }
    ...
    for (table_idx = 0; table_idx < enc_count; table_idx++) {
        for (idx = 0; idx < enc_table_len[table_idx] ; idx++) {
            if ( enc_mask[table_idx] & (1 << idx) ) {
                BCM_RX_REASON_SET(*reasons_mask, cpu_opcode_enc[table_idx][idx]);
            }
            if ( enc_mask[table_idx] & (1 << idx) ) {
