...
_field_th_emstage_update(int unit, _field_stage_t *stage_fc)
{
    ...
    /* Point to Extractor Configuration array as ingress stage */
    stage_em->ext_cfg_arr = stage_fc->ext_cfg_arr;

    /* Point to Ibus qual sec info as ingress stage */
    stage_em->input_bus.size = stage_fc->input_bus.size;
    stage_em->input_bus.num_fields = stage_fc->input_bus.num_fields;
    stage_em->input_bus.qual_list = stage_fc->input_bus.qual_list;
...
_field_th_em_validate_view(int unit, uint16 key_size, _field_group_t *fg)
{
        ...
        if (BCM_FIELD_ASET_TEST(aset, a_idx)) {
            switch (a_idx) {
                ...
                case bcmFieldActionDoNotCutThrough:
                    aset_size[_FieldEmActionCutThrOverrideSet] +=
                                      EM_ACTION_CUT_THR_OVERRIDE_SET_SIZE;
                    aset_cnt[_FieldEmActionCutThrOverrideSet]++;
                    break;
    ...
    if (aset_cnt[_FieldEmActionL3SwChangeL2Set] ||
        aset_cnt[_FieldEmActionEmRedirectSet]) {
     ...
     */
    if (key_size <= EM_MODE128_KEY_SIZE) {
    ...
    } else if ((EM_MODE128_KEY_SIZE < key_size) 
                && (key_size <= EM_MODE160_KEY_SIZE)) {
        if (aset_key_size <= EM_MODE160_ACTION_DATA_SIZE) {
            fg->em_mode = _FieldExactMatchMode160;
        } else if ((EM_MODE160_ACTION_DATA_SIZE < aset_key_size)
                    && (aset_key_size <= EM_MODE320_ACTION_DATA_SIZE)) {
    ...
    } else if ((EM_MODE160_KEY_SIZE < key_size) 
                && (key_size <= EM_MODE320_KEY_SIZE)) {
                                                      ...
                                                      uint32 *buf)
{
    ...
    if ((f_ent->statistic.flags & _FP_ENTRY_STAT_VALID)
        ...
        if (_FP_INVALID_INDEX != f_st->hw_index) {
            ...
            rv = _bcm_field_th_val_set(buf, &hw_ctr_base_idx, 13, 12);
            BCM_IF_ERROR_RETURN(rv);
 ...
 *  BCM_E_XXX
 * Notes:
 *  This api will set exact match action profile bitmap
 *  in action profile table and also return action
 *  profile index and filled action data buffer.
 */
STATIC int
                                   ...
                                   uint32 *ap_idx)
{
    ...
    int level = 0;                    /* Meter Level.                 */
    int counter_present = 0;          /* Counter Present flag.        */
    int meter_present = 0;            /* Meter Present flag.          */
    void *entries[1];                 /* Profile Entry.               */
    soc_mem_t mem;                    /* Memory Identifier.           */
        ...
        if (_FP_ACTION_VALID & fa->flags) {
            switch (fa->action) {
                ...
                case bcmFieldActionIngSampleEnable:
                    ...
                    /* Cut Through Override */
                case bcmFieldActionDoNotCutThrough:
                    acts = &acts_buf[_FieldEmActionCutThrOverrideSet];
                    rv = _bcm_field_th_misc_action_set(unit, f_ent,
                                                       fa, &acts->data);
                    BCM_IF_ERROR_RETURN(rv);
                    soc_mem_field32_set(unit, mem, &act_prof_entry,
                                        DO_NOT_CUT_THROUGHf, 1);
                    acts->valid = 1;
                    break;
                case bcmFieldActionDoNotCheckUrpf:
     ...
     * Stat can be attached to entry only of it is
    ...
    if (counter_present) {
        acts = &acts_buf[_FieldEmActionCounterSet];
        rv = _field_th_em_flex_stat_action_set(unit, f_ent,
                                               &acts->data);
        BCM_IF_ERROR_RETURN(rv);
        soc_mem_field32_set(unit, mem, &act_prof_entry,
                            COUNTER_SET_ENABLEf, 1);
        acts->valid = 1;
    }

    /* Set Policer Action Data if present. */
    for (level = 0; level < _FP_POLICER_LEVEL_COUNT; level++) {
        if (f_ent->policer[level].flags & _FP_POLICER_VALID) {
            meter_present = 1;
        }
    }
    ...
    if (meter_present) {
        ...
        BCM_IF_ERROR_RETURN(rv);
        rv = _bcm_field_th_val_get(meterbuf, &acts->data,
                                   265, EM_ACTION_METER_SET_SIZE);
        BCM_IF_ERROR_RETURN(rv);
        soc_mem_field32_set(unit, mem, &act_prof_entry,
                            METER_SET_ENABLEf, 1);
        acts->valid = 1;
    ...
    if ((acts_buf[_FieldEmActionL3SwChangeL2Set].valid) ||
        (acts_buf[_FieldEmActionEmRedirectSet].valid)) {
        acts = &acts_buf[_FieldEmActionGreenToPidSet];
        soc_mem_field32_set(unit, mem, &act_prof_entry,
                                  GREEN_TO_PID_SET_ENABLEf, 1);
        acts->valid = 1;
                                                  ...
                                                  int *action_size)
{
    ...
    uint8 aset_cnt[_FieldEmActionSetCount] = {0};
                                     /* Action Per Set Count.             */
    uint8 aset_cum[_FieldEmActionSetCount] = {0};
                                     /* Action Per Set Cumulative Count.  */
...
_field_th_em_default_entry_install(int unit, _field_entry_t *f_ent)
{
    ...
    rv = _field_th_em_entry_action_size_get(unit, f_ent, &aset_size);
    BCM_IF_ERROR_RETURN(rv);
