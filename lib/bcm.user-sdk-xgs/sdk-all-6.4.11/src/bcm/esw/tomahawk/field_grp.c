                                 ...
                                 _field_group_t *fg)
{
    ...
    int ecmp2_p = 0;               /* Part info for ECMP2 postmux */
                             ...
                             uint16 qual_count, uint16 key_size)
{
     ...
     */
    if (stage_fc->stage_id == _BCM_FIELD_STAGE_EXACTMATCH) {
        rv = _field_th_em_validate_view(unit, key_size, fg);
        if (BCM_FAILURE(rv)) {
            LOG_VERBOSE(BSL_LS_BCM_FP, (BSL_META_U(unit,
...
/*
 * Function:
 *     _bcm_field_th_qual_hint_size_get
 * Purpose:
 *     Get the total qualifier size needs to be extracted after applying hints.
 * Parameters:
 *     unit      - (IN)   BCM unit number.
 *     qid       - (IN)    Qualifier.
 ...
 *     hints_size -    (OUT) Qualifier size to be extracted after 
 *                           hints are applied.
 * Returns:
 *     BCM_E_XXX
 */
STATIC int
_bcm_field_th_qual_hint_size_get(int unit, int qid,
                                 _field_group_t *fg,
                                 uint8 *hints_present,
                                 uint16 *hints_size)
{
    ...
    if (NULL == fg || NULL == hints_present || NULL == hints_size) {
        return BCM_E_PARAM;
    }
...
{
    int rv = BCM_E_NONE;      /* Operational Status. */
    uint8 hints_present = 0;  /* Flag to indicate hints are present 
                               * for the qualifier. */
    uint16 hints_size = 0;    /* Qualifier size after hints are applied. */
	...
	if (NULL != qual_list[qid]) {
		...
		default:
                    ...
                    if (fg->hintid) {
                        rv = _bcm_field_th_qual_hint_size_get(unit, qid, fg, 
                                                              &hints_present,
                                                              &hints_size);
                        BCM_IF_ERROR_RETURN(rv);
                    }
                                               ...
                                               _field_lt_slice_t *lt_fs)
{
    ...
    if (0 != part) {
        ...
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_FP, (BSL_META_U(unit,
            "LT entry part pri_tcam_idx:%d to tcam_idx get failed.\n\r"),
            pri_tcam_idx));
            return rv;
        }
        ...
        */
        ...
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_FP, (BSL_META_U(unit,
            "LT entry tcam_idx:%d to slice_offset failed.\n\r"),
            part_index));
           return rv;
        }
    }
                               ...
                               _bcm_field_qual_info_t **f_qual_arr)
{
    ...
    int ecmp2_off_0 = 0;    /* ECMP2 offset 0 */
    int ecmp2_off_1 = 0;    /* ECMP2 offset 1 */
