...
_field_action_software_recover(int unit, _field_entry_t *f_ent, uint32 *e_buf,
                               _field_action_t *act, int *val_array,
                               int *pos, uint8 action_param_type)
{
...
_field_entry_recover(int unit, _field_group_t **new_fg)
{
    ...
    uint8 action_param_type = -1;         /* Action Parameter changes are identified */
...
_field_group_recover(int unit, _field_group_t *fg)
{
    ...
    if (qual_count != -1) {
        for (idx = qual_count; idx < _bcmFieldQualifyCount; idx ++) {
            if (BCM_FIELD_QSET_TEST(fg->qset, idx)){
        ...
        for (idx = bcmFieldQualifyCount; idx < _bcmFieldQualifyCount; idx++) {
        ...
        case _bcmFieldInternalGroupEntry:
             ...
             for (idx = 0; idx < entry_count; idx++) {
                ...
                _field_entry_info_recover(unit, f_ent, flags, 1, new_fg,
                                         ...
                                         &action_count, &action_param_type);
               /* Actions recover for entry */
               if (f_ent->flags & _FP_ENTRY_INSTALLED) {
