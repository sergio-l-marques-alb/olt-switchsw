/*
 * $Id: vlan.c,v 1.471 Broadcom SDK $
...
bcm_esw_vlan_detach(int unit)
{
    ...
    BCM_IF_ERROR_RETURN(_bcm_vbmp_destroy(&vi->pre_cfg_bmp));
#if defined (BCM_EASY_RELOAD_SUPPORT)
    BCM_IF_ERROR_RETURN(_bcm_vbmp_destroy(&vi->egr_vlan_bmp));
    BCM_IF_ERROR_RETURN(_bcm_vbmp_destroy(&vi->vlan_tab_bmp));
#endif
...
#if defined(BCM_TRIDENT2PLUS_SUPPORT)
    if (soc_feature(unit, soc_feature_vlan_vfi_membership)) {
        BCM_IF_ERROR_RETURN(bcm_td2p_vp_group_detach(unit));
    } else
#endif
    if (soc_feature(unit, soc_feature_vp_group_ingress_vlan_membership) ||
        soc_feature(unit, soc_feature_vp_group_egress_vlan_membership)) {
...
_bcm_vlan_destroy(int unit, bcm_vlan_t vid)
{
    ...
    if (SOC_MEM_FIELD_VALID(unit, VLAN_TABm, VIRTUAL_PORT_ENf)) {
        ...
        if (soc_mem_field32_get(unit, VLAN_TABm, &vtab, VIRTUAL_PORT_ENf)) {
            ...
            bc_index = soc_mem_field32_get(unit, VLAN_TABm, &vtab, BC_IDXf);
            if (BCM_SUCCESS(
            ...
            umc_index = soc_mem_field32_get(unit, VLAN_TABm, &vtab, UMC_IDXf);
            ...
            uuc_index = soc_mem_field32_get(unit, VLAN_TABm, &vtab, UUC_IDXf);
                   ...
                   pbmp_t ing_pbmp, int flags)
{
    ...
    if (SOC_MEM_FIELD_VALID(unit, VLAN_TABm, VIRTUAL_PORT_ENf)) {
        ...
        if (soc_mem_field32_get(unit, VLAN_TABm, &vtab, VIRTUAL_PORT_ENf)) {
            ...
            ipmc_group[0] = soc_mem_field32_get(unit, VLAN_TABm, &vtab, BC_IDXf);
            ipmc_group[1] = soc_mem_field32_get(unit, VLAN_TABm, &vtab, UMC_IDXf);
            ipmc_group[2] = soc_mem_field32_get(unit, VLAN_TABm, &vtab, UUC_IDXf);
    ...
    if (SOC_MEM_FIELD_VALID(unit, VLAN_TABm, VIRTUAL_PORT_ENf)) {
        ...
        if (soc_mem_field32_get(unit, VLAN_TABm, &vtab, VIRTUAL_PORT_ENf)) {
            ...
            for (i = 0; i < 3; i++) {
                ...
                mc_index = soc_mem_field32_get(unit, VLAN_TABm, &vtab,
                        group_type[i]);
...
#ifdef BCM_KATANA2_SUPPORT
        if (SOC_IS_KATANA2(unit)) {
            rv = _bcm_kt2_vlan_port_default_action_set(unit,
                     local_port, action);
        } else
#endif
        {
...
#ifdef BCM_KATANA2_SUPPORT
        if (SOC_IS_KATANA2(unit)) {
            rv = _bcm_kt2_vlan_port_default_action_delete(unit, local_port);
        } else
#endif
        {
 ...
 */     
int
bcm_esw_vlan_port_egress_default_action_delete(int unit,
                                                           bcm_port_t port)
{
    int rv = BCM_E_UNAVAIL;
                                   ...
                                   void *user_data)
{
    ...
    if (SOC_IS_TRX(unit) && soc_feature(unit, soc_feature_vlan_action)) {
        ...
        rv = _bcm_esw_vlan_translate_traverse_mem(unit, VLAN_XLATEm, &trvs_st);
    }
#endif /* BCM_TRX_SUPPORT */
...
/*
 * Function:
 *      bcm_esw_vlan_control_port_vxlan_validate
 * Description:
 *      Checks that vlan port control is valid for a VXLAN GPORT.
 *      Resolves port to expected port value for the given port control type.
 * Parameters:
 *      unit      - (IN) Device number
 *      port      - (IN) Port to resolve
 ...
 */
STATIC int
bcm_esw_vlan_control_port_vxlan_validate(int unit, bcm_port_t port,
                                       bcm_vlan_control_port_t type,
                                       bcm_port_t *port_out)
{
#if defined(BCM_TRIDENT2_SUPPORT) && defined(INCLUDE_L3)
...
/*
 * Function:
 *      bcm_esw_vlan_control_port_mim_validate
 * Description:
 *      Checks that vlan port control is valid for a mim GPORT.
 *      Resolves port to expected port value for the given port control type.
 * Parameters:
 *      unit      - (IN) Device number
 *      port      - (IN) Port to resolve
 ...
 */
STATIC int
bcm_esw_vlan_control_port_mim_validate(int unit, bcm_port_t port,
                                       bcm_vlan_control_port_t type,
                                       bcm_port_t *port_out)
{
#if defined(BCM_TRIDENT2_SUPPORT) && defined(INCLUDE_L3)
...
/*
 * Function:
 *      bcm_esw_vlan_control_port_mpls_validate
 * Description:
 *      Checks that vlan port control is valid for a mpls GPORT.
 *      Resolves port to expected port value for the given port control type.
 * Parameters:
 *      unit      - (IN) Device number
 *      port      - (IN) Port to resolve
 ...
 */
STATIC int
bcm_esw_vlan_control_port_mpls_validate(int unit, bcm_port_t port,
                                       bcm_vlan_control_port_t type,
                                       bcm_port_t *port_out)
{
#if defined(BCM_TRIDENT2_SUPPORT) && defined(INCLUDE_L3)
...
/*
 * Function:
 *      bcm_esw_vlan_control_port_l2gre_validate
 * Description:
 *      Checks that vlan port control is valid for a l2gre GPORT.
 *      Resolves port to expected port value for the given port control type.
 * Parameters:
 *      unit      - (IN) Device number
 *      port      - (IN) Port to resolve
 ...
 */
STATIC int
bcm_esw_vlan_control_port_l2gre_validate(int unit, bcm_port_t port,
                                       bcm_vlan_control_port_t type,
                                       bcm_port_t *port_out)
{
#if defined(BCM_TRIDENT2_SUPPORT) && defined(INCLUDE_L3)
                                   ...
                                   bcm_port_t *port_out)
{
    ...
    if (BCM_GPORT_IS_SET(port)) {
        if (BCM_GPORT_IS_PROXY(port)) {
            ...
            BCM_IF_ERROR_RETURN
                (bcm_esw_vlan_control_port_vxlan_validate(unit, port,
                                                          type, port_out));
        } else if (BCM_GPORT_IS_MIM_PORT(port)) {
            BCM_IF_ERROR_RETURN
                (bcm_esw_vlan_control_port_mim_validate(unit, port,
                                                          type, port_out));
        } else if (BCM_GPORT_IS_MPLS_PORT(port)) {
            BCM_IF_ERROR_RETURN
                (bcm_esw_vlan_control_port_mpls_validate(unit, port,
                                                          type, port_out));
        } else if (BCM_GPORT_IS_L2GRE_PORT(port)) {
            BCM_IF_ERROR_RETURN
                (bcm_esw_vlan_control_port_l2gre_validate(unit, port,
                                                          type, port_out));
                    ...
                    if (soc_feature(unit, soc_feature_hgproxy_subtag_coe) &&
                        ...
                        SOC_IF_ERROR_RETURN(
                            bcm_esw_port_egr_lport_field_set(unit, port,
                                            EGR_LPORT_PROFILE_VLAN_CONTROL_1,
                                            VT_ENABLEf, arg));
                        return BCM_E_NONE;
                    }
#endif
           ...
           (arg >= bcmVlanTranslateEgressKeyCount)) {
            ...
            if (soc_feature(unit, soc_feature_egr_vlan_xlate_key_on_dvp)) {
                ...
                if (BCM_GPORT_IS_VXLAN_PORT(port) || BCM_GPORT_IS_MIM_PORT(port) ||
                    ...
                    if (arg == bcmVlanTranslateEgressKeyVlanPort) {
                    /* bcmVlanTranslateEgressKeyVlanPort default for local port */
                        return BCM_E_NONE;
                    } else {
                        return BCM_E_UNAVAIL;
                    }
            ...
            if (soc_feature(unit, soc_feature_egr_vlan_xlate_key_on_dvp)) {
                ...
                if (BCM_GPORT_IS_VXLAN_PORT(port) || BCM_GPORT_IS_MIM_PORT(port) ||
                    ...
                    if (!SOC_PORT_VALID(unit, local_port)) {
                        ...
                        *arg = bcmVlanTranslateEgressKeyVlanPort;
                        return BCM_E_NONE;
                    }
                } else if (SOC_PORT_VALID(unit, port)) {
                    *arg = bcmVlanTranslateEgressKeyVlanPort;
...
_xgs_vlan_ing_bitmap_get(int unit, vlan_tab_entry_t *vt, bcm_pbmp_t *ports)
{
    ...
    if (SOC_IS_XGS3_SWITCH(unit)) {
        if (SOC_MEM_FIELD_VALID(unit, VLAN_TABm, ING_PORT_BITMAPf)) {
            soc_mem_pbmp_field_get(unit, VLAN_TABm, vt, ING_PORT_BITMAPf,
                                   ports);
        }
    }
#endif
...
_xgs_vlan_ing_bitmap_set(int unit, vlan_tab_entry_t *vt, bcm_pbmp_t ports)
{
    ...
    if (SOC_IS_XGS3_SWITCH(unit)) {
        if (SOC_MEM_FIELD_VALID(unit, VLAN_TABm, ING_PORT_BITMAPf)) {
            soc_mem_pbmp_field_set(unit, VLAN_TABm, vt, ING_PORT_BITMAPf,
                                   &ports);
        } 
    }
#endif
                              ...
                              uint32 valid_fields, bcm_vlan_control_vlan_t *control)
{
    ...
    if (_BCM_VPN_VFI_IS_SET(vlan)) {
        ...
        _BCM_VPN_SET(vpn_min, _BCM_VPN_TYPE_VFI, 0);
        if ((vlan > (vpn_min+num_vfi-1)) || (vlan < vpn_min)) {
            return (BCM_E_PARAM);
        }
                                               ...
                                               int fs_idx, void *cookie)
{
    ...
    rv = soc_mem_search(unit, VLAN_XLATEm, MEM_BLOCK_ALL, &idx, 
                         &vent, &res_vent, 0);
    if (BCM_SUCCESS(rv)) {
        soc_mem_field32_set(unit, VLAN_XLATEm, &res_vent,
                            VINTF_CTR_IDXf, fs_idx);
        if (SOC_MEM_FIELD_VALID(unit, VLAN_XLATEm, USE_VINTF_CTR_IDXf)) {
            soc_mem_field32_set(unit, VLAN_XLATEm, &res_vent,
                                USE_VINTF_CTR_IDXf, fs_idx > 0 ? 1 : 0);
        }
        rv = soc_mem_insert(unit, VLAN_XLATEm, MEM_BLOCK_ALL, &res_vent);
        if (BCM_SUCCESS(rv)) {
                   ...
                   bcm_stat_flex_table_info_t *table_info)
{
    ...
    soc_mem_lock(unit, VLAN_XLATEm);
    rv = soc_mem_search(unit, VLAN_XLATEm, MEM_BLOCK_ALL,&index,&vent,
                        &vlan_xlate_entry,0);
    soc_mem_unlock(unit, VLAN_XLATEm);
    ...
    if (BCM_SUCCESS(rv)) {
        ...
        if (BCM_SUCCESS(rv)) {
            BCM_IF_ERROR_RETURN(_bcm_esw_vlan_translate_convert_to_extd_entry(
                                unit,&vlan_xlate_entry,&vlan_xlate_extd_entry));
           ...
           BCM_IF_ERROR_RETURN(_bcm_esw_vlan_translate_convert_to_extd_entry(
                               unit,&vent,&vlan_xlate_extd_entry));
        }
        ...
        if (table_info[table_count].direction == direction) {
            for (index_count=0; index_count < num_entries ; index_count++) {
                 BCM_IF_ERROR_RETURN(_bcm_esw_stat_counter_get(
                                     unit, sync_mode, 
                                     table_info[table_count].index,
                                     table_info[table_count].table, byte_flag,
                                     counter_indexes[index_count],
                                     &counter_values[index_count]));
            }
        }
            ...
            bcm_stat_flex_table_info_t *table_info)
{
    ...
    egr_vlan_xlate_entry_t return_vent={{0}};
    int                    index=0;
    ...
    BCM_IF_ERROR_RETURN (_bcm_trx_vlan_translate_egress_entry_assemble(
                         unit, &vent, port_class, outer_vlan,inner_vlan));
    soc_mem_lock(unit, EGR_VLAN_XLATEm);
    rv = soc_mem_search(unit,EGR_VLAN_XLATEm,MEM_BLOCK_ALL,&index,&vent,
                        &return_vent,0);
    if (BCM_SUCCESS(rv)) {
        ...
        if (soc_mem_field32_get(unit, VLAN_TABm, &vtab, VIRTUAL_PORT_ENf)) {
...
#endif /* BCM_TRIDENT2PLUS_SUPPORT */
            {
                if (soc_feature(unit, soc_feature_vp_group_ingress_vlan_membership) &&
                    ...
                    /* clear the vp group bitmap for this vlan */
                    rv = bcm_td_vlan_vp_group_set(unit,VLAN_TABm,vlan, -1,FALSE);
                    if (rv != BCM_E_NONE) {
                        soc_mem_unlock(unit, VLAN_TABm);
                        return rv;
                    }
        ...
        if (soc_feature(unit, soc_feature_vlan_vfi_membership)) {
            ...
            if (enable) {
                ...
                *flags |= _bcm_td2_stp_state_to_flags(unit, hw_stp_state, TRUE);
                ...
                if (!_BCM_VPN_VFI_IS_SET(vlan)) {
                    BCM_IF_ERROR_RETURN(
                        bcm_td2p_vp_group_ut_state_get(unit, vlan, port, &ut_state));
                    if (ut_state) {
...
#endif /* BCM_TRIDENT2PLUS_SUPPORT */
        {
            if (soc_feature(unit, soc_feature_vp_group_ingress_vlan_membership)) {
                BCM_IF_ERROR_RETURN(bcm_td_vlan_vp_group_get(
                            unit, VLAN_TABm, vlan, vpg, &enable));
                if (enable) {
                    *flags = BCM_VLAN_PORT_INGRESS_ONLY;
                }
            }
