...
STATIC int _bcm_tr3_l2gre_egr_xlate_entry_get(int unit, int vfi, egr_vlan_xlate_entry_t *vxlate_entry);
...
bcm_tr3_l2gre_vpn_create(int unit, bcm_l2gre_vpn_config_t *info)
{
    ...
    if (info->flags & BCM_L2GRE_VPN_WITH_VPNID) {
        ...
        /* Reset vpnid entry */
        rv = _bcm_tr3_l2gre_egr_xlate_entry_get(unit, vfi_index, &vxlate);
        if (BCM_E_NOT_FOUND == rv) {
            return BCM_E_NONE;
        } else if (BCM_E_NONE == rv) {
            vpnid = soc_EGR_VLAN_XLATEm_field32_get(unit, &vxlate, L2GRE_VFI__VPNIDf);
...
STATIC int
_bcm_tr3_l2gre_egr_xlate_entry_get( int unit, int vfi, egr_vlan_xlate_entry_t *vxlate_entry)
{
    int index=0;
    ...
    if (SOC_IS_TRIUMPH3(unit)) {
        soc_EGR_VLAN_XLATEm_field32_set(
           unit, vxlate_entry, KEY_TYPEf, _BCM_TR3_L2GRE_KEY_TYPE_LOOKUP_VFI);
    } else if (SOC_IS_TRIDENT2X(unit) || SOC_IS_TOMAHAWK(unit)) {
        soc_EGR_VLAN_XLATEm_field32_set(
           unit, vxlate_entry, ENTRY_TYPEf, _BCM_TD2_L2GRE_KEY_TYPE_LOOKUP_VFI);
    }
    soc_EGR_VLAN_XLATEm_field32_set(unit, vxlate_entry, VALIDf, 1);
    soc_EGR_VLAN_XLATEm_field32_set(unit, vxlate_entry, L2GRE_VFI__VFIf, vfi);

    BCM_IF_ERROR_RETURN(soc_mem_search(unit, EGR_VLAN_XLATEm,
...
bcm_tr3_l2gre_vpn_destroy(int unit, bcm_vpn_t l2vpn)
{
    ...
    rv = _bcm_tr3_l2gre_egr_xlate_entry_get(unit, vfi, &vxlate);
    if (BCM_E_NONE == rv) {
...
bcm_tr3_l2gre_vpn_get( int unit, bcm_vpn_t l2vpn, bcm_l2gre_vpn_config_t *info)
{
    ...
    if (BCM_E_NONE == rv) {
        ...
        info->vpnid = soc_EGR_VLAN_XLATEm_field32_get(unit, &vxlate, L2GRE_VFI__VPNIDf);
                                    ...
                                    int vp_nh_ecmp_index, bcm_l2gre_port_t *l2gre_port)
{
    ...
    if (mpath_flag) {
        intf = BCM_XGS3_MPATH_EGRESS_IDX_MIN + vp_nh_ecmp_index;
    } else {
        intf = BCM_XGS3_DVP_EGRESS_IDX_MIN + vp_nh_ecmp_index;
    }
    network_port = (l2gre_port->flags & BCM_L2GRE_PORT_EGRESS_TUNNEL) ? 1 : 0;
...
_bcm_tr3_l2gre_egr_xlate_entry_set(int unit, bcm_l2gre_vpn_config_t *vpn_info)
{
    ...
    sal_memset(&vxlate, 0, sizeof(egr_vlan_xlate_entry_t));
    ...
    /* Configurate EGR_VLAN_XLATE for Egress VPNID */
    if (SOC_IS_TRIUMPH3(unit)) {
        soc_EGR_VLAN_XLATEm_field32_set(unit, &vxlate, KEY_TYPEf, _BCM_TR3_L2GRE_KEY_TYPE_LOOKUP_VFI);
    } else if (SOC_IS_TRIDENT2X(unit) || SOC_IS_TOMAHAWK(unit)) {
        soc_EGR_VLAN_XLATEm_field32_set(unit, &vxlate, ENTRY_TYPEf, _BCM_TD2_L2GRE_KEY_TYPE_LOOKUP_VFI);
    }
    soc_EGR_VLAN_XLATEm_field32_set(unit, &vxlate, VALIDf, 0x1);
    soc_EGR_VLAN_XLATEm_field32_set(unit, &vxlate, L2GRE_VFI__VFIf, vfi_index);
    soc_EGR_VLAN_XLATEm_field32_set(unit, &vxlate, L2GRE_VFI__VPNIDf,  vpn_info->vpnid);
    ...
    rv = soc_mem_insert(unit, EGR_VLAN_XLATEm, MEM_BLOCK_ALL, &vxlate);
    if (rv < 0)
    {
        if (tpid_index != -1) {
...
_bcm_tr3_l2gre_egr_xlate_entry_reset(int unit, bcm_vpn_t vpn)
{
    ...
    action_present = soc_mem_field32_get(
        unit, EGR_VLAN_XLATEm, &key_ent, L2GRE_VFI__SD_TAG_ACTION_IF_PRESENTf);
    action_not_present = soc_mem_field32_get(
        unit, EGR_VLAN_XLATEm, &key_ent, L2GRE_VFI__SD_TAG_ACTION_IF_NOT_PRESENTf);
    if (1 == action_present || 4 == action_present ||
        7 == action_present || 1 == action_not_present) {
        tpid_index = soc_mem_field32_get(
            unit, EGR_VLAN_XLATEm, &key_ent, L2GRE_VFI__SD_TAG_TPID_INDEXf);
        BCM_IF_ERROR_RETURN(_bcm_fb2_outer_tpid_entry_delete(unit, tpid_index));
    }

    /* Delete the entry from HW */
    BCM_IF_ERROR_RETURN(soc_mem_delete(unit, EGR_VLAN_XLATEm, MEM_BLOCK_ALL, &key_ent));
...
_bcm_tr3_l2gre_match_vpnid_entry_set(int unit, bcm_l2gre_vpn_config_t *vpn_info)
{
    ...
    soc_MPLS_ENTRYm_field32_set(unit, &ment, L2GRE_VPNID__VPNIDf, vpn_info->vpnid);
...
STATIC int
_bcm_tr3_l2gre_trunk_table_reset(int unit, bcm_port_t port, bcm_trunk_t tgid)
{
    source_trunk_map_table_entry_t trunk_map_entry;/*Trunk table entry buffer.*/
    bcm_module_t my_modid;
    int    src_trk_idx = -1;
    ...
    BCM_IF_ERROR_RETURN(_bcm_esw_gport_resolve(unit, port,
                                 &my_modid, &port, &local_tgid, &local_id));
...
bcm_tr3_l2gre_port_add(int unit, bcm_vpn_t vpn, bcm_l2gre_port_t  *l2gre_port)
{
    ...
    if(l2gre_port->if_class) {
        bit_num_dvp = soc_mem_field_length(unit, EGR_DVP_ATTRIBUTE_1m, L2GRE__CLASS_IDf);
...
bcm_tr3_l2gre_port_learn_set(int unit, bcm_gport_t l2gre_port_id,
                                  uint32 flags)
{
    int vp, cml, rv = BCM_E_NONE, entry_type;
    source_vp_entry_t svp;
                ...
                int trunk_member_count, bcm_port_t *trunk_member_array)
{
    ...
    if (nh_index == 0) {
        for (idx = 0; idx < trunk_member_count; idx++) {
            rv += _bcm_tr3_l2gre_trunk_table_reset(unit, trunk_member_array[idx], trunk_id);
        }
        if (BCM_FAILURE(rv)) {
            return rv;
        }
