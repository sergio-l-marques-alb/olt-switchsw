...
#if defined(BCM_SABER2_SUPPORT)
    if (soc_sb2_5626x_devtype(unit)) {
        sal_memcpy(mpls_state, &source_vp_sb2_5626x,
                sizeof(source_vp_sb2_5626x));
        mpls_state += sizeof(source_vp_sb2_5626x);
    }
#endif /* BCM_SABER2_SUPPORT */
...
bcm_tr_mpls_init(int unit)
{
    ...
    if (soc_feature(unit, soc_feature_mpls_enhanced)) {
        ...
        size = SOC_MEM_TABLE_BYTES(unit, VFIm);
        mbuf = soc_cm_salloc(unit, size, "VFI");
        if (NULL == mbuf) {
            _bcm_tr_mpls_free_resource(unit, mpls_info);
            return BCM_E_MEMORY;
        }
        ...
        for (i = 0; i < num_vfi; i++) {
            ent = soc_mem_table_idx_to_pointer(unit, VFIm, vfi_entry_t *, mbuf, i);
            soc_VFIm_field32_set(unit, ent, BC_INDEXf, 0);
            soc_VFIm_field32_set(unit, ent, UUC_INDEXf, 0);
            soc_VFIm_field32_set(unit, ent, UMC_INDEXf, 0);
        }
        rv = soc_mem_write_range(unit, VFIm, MEM_BLOCK_ANY,
                                 soc_mem_index_min(unit, VFIm),
                                 soc_mem_index_max(unit, VFIm), mbuf);
...
bcm_tr_mpls_vpn_id_destroy (int unit, bcm_vpn_t vpn)
{
    ...
    if (_BCM_MPLS_VPN_IS_VPWS(vpn)) {
       ...
       _BCM_MPLS_VPN_GET(vfi, _BCM_MPLS_VPN_TYPE_VPLS, vpn);
        ...
        if (!soc_feature(unit, soc_feature_mpls_enhanced)) {
            if (SOC_MEM_FIELD_VALID(unit, VFIm, PFMf)) {
                soc_mem_field32_modify(unit, VFIm, vfi, PFMf, 3);
            }
        }
    } else if (_BCM_MPLS_VPN_IS_L3(vpn)) {
        int vrf;
    ...
    if (ing_nh_info->trunk == -1) {
        ...
        if (soc_feature(unit, soc_feature_linkphy_coe) ||
            ...
            if (modid_local == 0) {
                ...
                if (ing_nh_info->module == (my_modid + 1)) {
...
bcm_tr_mpls_port_untagged_profile_set (int unit, bcm_port_t port)
{
    ...
    rv = _bcm_trx_vlan_action_profile_entry_delete(unit, 0);
                                   ...
                                   int nh_index, _bcm_mpls_old_nh_info_t *old_nh_info)
{
    ...
    if (mpls_port->flags & BCM_MPLS_PORT_ENCAP_WITH_ID) {
         ...
         * With the flag BCM_MPLS_PORT_ENCAP_WITH_ID, there isn't an old NH,
         * so the _bcm_tr_mpls_l2_nh_info_delete() is not called.
         * Therefore we need to clear the old_vc_swap_idx.
         */
        if (old_nh_info->old_vc_swap_idx != -1) {
#if defined(BCM_TRIUMPH3_SUPPORT)
             if (soc_feature(unit, soc_feature_vc_and_swap_table_overlaid)) {
    ...
    *is_local = 0;
    ...
    if (BCM_GPORT_IS_SET(mpls_port->port) &&
        ...
        if (mod_out == (my_modid + 1)) {
            ...
            _BCM_MPLS_CLEANUP(rv);
            if ((phys_port >= min_subport) &&
                (phys_port <= max_subport)) {
                is_local_subport = 1;
            }
        }
    }
        ...
        if (BCM_GPORT_IS_TRUNK(mpls_port->port)) {
            ...
            if (BCM_GPORT_IS_SET(mpls_port->port) &&
                ...
                if (mod_out == (my_modid + 1)) {
                    ...
                    if ((local_port >= min_subport) &&
                        (local_port <= max_subport)) {
                        is_local_subport = 1;
                    }
                }
            }
            ...
            if (is_local_subport) {
                rv = soc_mem_field32_modify(unit, PORT_TABm, local_port,
                                        PORT_OPERATIONf, 0x1); /* L2_SVP */
                BCM_IF_ERROR_RETURN(rv);
...
int
_bcm_tr_mpls_match_delete(int unit, int vp, bcm_mpls_port_t *mpls_port, int sw_clear)
{
    _bcm_tr_mpls_bookkeeping_t *mpls_info = MPLS_INFO(unit);
    int rv=BCM_E_NONE;
    bcm_trunk_t trunk_id;
    ...
    if (soc_feature(unit, soc_feature_hgproxy_subtag_coe)) {
        ...
        if (is_local == 0) {
            if(_bcm_xgs5_subport_coe_mod_port_local(unit, mod_id_idx, port)) {
                is_local_subport = 1;
                BCM_IF_ERROR_RETURN(_bcmi_coe_subport_mod_port_physical_port_get
                                            (unit, mod_id_idx, port, &local_port));
            }
        }
    } else
#endif
        ...
        sal_memset(&ret_ment, 0, sizeof(mpls_entry_entry_t));
         ...
         if (rv == SOC_E_NONE) {
              ...
              if (soc_MPLS_ENTRYm_field32_get(unit, &ret_ment, PW_TERM_NUM_VALIDf)) {
                  ...
                  mpls_port->class_id = soc_MPLS_ENTRYm_field32_get(unit, &ment,
                    CLASS_IDf);
              }
#endif
        ...
        switch (vp_valid_flag) {
         ...
         case 0x2: /* Only VP2 is valid */
                             if (!(mpls_port->flags & BCM_MPLS_PORT_NETWORK)) {
                                 ...
                                 /* 
                                  * Since only VP2 is valid and the port being added is a network port,
                                  * its either existing network port (VP2) being replaced or a primary 
                                  * network port (VP3) being added
                                  */
                                 if (mpls_port->flags & BCM_MPLS_PORT_REPLACE) {
                                     if (active_vp != vp2) {
                                         return BCM_E_NOT_FOUND;
                             ...
                             if (rv >= 0) {
                                 ...
                                 if (!vp3_valid || (vp3_valid &&
                                    ...
                                    if (vp1 != -1) {
                                     ...
                                     rv = _bcm_esw_failover_prot_nhi_create(unit, 0, (uint32)prot_index,
                                                                            nh_index, mpls_port->failover_mc_group, mpls_port->failover_id);
    ...
    if (_BCM_MPLS_VPN_IS_VPWS(vpn)) {
       ...
       _BCM_MPLS_VPN_GET(vpn_id, _BCM_MPLS_VPN_TYPE_VPWS, vpn);
        ...
        if (vp == vp1) {
            ...
            /* If both failover port and primary port exist and share the same match
             * criteria, clear software record only
             */
            if ((vp3_read != -1) &&
                (mpls_info->match_key[vp].flags == _BCM_MPLS_PORT_MATCH_TYPE_LABEL) &&
                ((mpls_info->match_key[vp].flags == mpls_info->match_key[vp3_read].flags) &&
                (mpls_info->match_key[vp].match_label ==
                    mpls_info->match_key[vp3_read].match_label))) {
                (void) bcm_tr_mpls_match_clear (unit, vp);
            } else {
                rv = _bcm_esw_mpls_match_delete(unit,
                         vpless_failover_port == FALSE?
        ...
        if (rv >= 0) {
            ...
            if (vpless_failover_port == FALSE) {
                ...
                if (vp == vp1) {
                    ...
                    _bcm_tr_mpls_vpws_vp_map_set(unit, vpn_id, vp1, -1, vp3_read);
                } else if (vp == vp3) {
...
bcm_tr_mpls_swap_nh_info_add (int unit, bcm_l3_egress_t *egr, int nh_index, uint32 flags)
{
    ...
    rv =  _bcm_tr_mpls_get_vc_and_swap_table_index (unit, 0,
              NULL, NULL, egr, _BCM_MPLS_ACTION_SWAP, &vc_swap_index);
    _BCM_MPLS_CLEANUP(rv);
    /* Retrieve hardware index for mapping pointer */
    if ((egr->mpls_flags & BCM_MPLS_EGRESS_LABEL_EXP_SET) ||
        (egr->mpls_flags & BCM_MPLS_EGRESS_LABEL_PRI_SET)) {
...
bcm_tr_mpls_l3_nh_info_add(int unit, bcm_mpls_tunnel_switch_t *info, int *nh_index)
{
    ...
    rv =  _bcm_tr_mpls_get_vc_and_swap_table_index (unit, 0,
              NULL, info, NULL, _BCM_MPLS_ACTION_SWAP, 
              &egr_nh_info.vc_swap_index);
    _BCM_MPLS_CLEANUP(rv);
    /* Retrieve hardware index for mapping pointer */
    num_mpls_map = soc_mem_index_count(unit, EGR_MPLS_EXP_MAPPING_1m) / 64;
