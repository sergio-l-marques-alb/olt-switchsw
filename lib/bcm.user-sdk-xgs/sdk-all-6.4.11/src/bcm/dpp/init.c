...
_bcm_petra_modules_deinit(int unit, int silent)
{
    ...
    if (pp_enable) {
        ...
        SOC_PETRA_DETACH(unit, _bcm_petra_oam_detach, rv); /* init is done only in WB. Detach is an internal function, not currently supported through mkdispatch */ 
        SOC_PETRA_DETACH(unit, _bcm_petra_bfd_detach, rv); /* init is done only in WB. Detach is an internal function, not currently supported through mkdispatch  */
