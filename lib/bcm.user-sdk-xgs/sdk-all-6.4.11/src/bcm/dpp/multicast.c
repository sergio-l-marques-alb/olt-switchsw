...
_bcm_petra_multicast_group_open(int unit,BCM_DPP_MULTICAST_TYPE type,SOC_TMC_MULT_ID multicast_id_ndx)
{
    ...
    if (SOC_IS_PETRAB(unit)) {
        /* Open Ingress group */
        if (type == BCM_DPP_MULTICAST_TYPE_INGRESS_ONLY || type == BCM_DPP_MULTICAST_TYPE_INGRESS_AND_EGRESS) {
            int rv = bcm_dpp_am_mc_alloc(unit, BCM_DPP_AM_FLAG_ALLOC_WITH_ID, &multicast_id_ndx, FALSE);
            BCMDNX_IF_ERR_EXIT(rv);
        ...
        /* Open Egress group */
        if (type == BCM_DPP_MULTICAST_TYPE_EGRESS_ONLY || type == BCM_DPP_MULTICAST_TYPE_INGRESS_AND_EGRESS) {
            int rv = bcm_dpp_am_mc_alloc(unit, BCM_DPP_AM_FLAG_ALLOC_WITH_ID, &multicast_id_ndx, TRUE);
            BCMDNX_IF_ERR_EXIT(rv);
    ...
    uint8 is_delete)                        /* input: if non zero this is a delete operation, otherwise an add */
{
    ...
    if (is_in_range) { /* Handle bitmap groups. They do not have CUDs */
        ...
        rv = _bcm_petra_multicast_egress_vlan_membership_port_add_or_remove(unit, mcid, 0, &pbmp, is_delete);
    ...
    uint8 is_delete)        /* input: if non zero this is a delete operation, otherwise an add */
{
    ...
    BCM_PBMP_ITER(gport_info.pbmp_local_ports, local_port) { /* add/remove ports in bitmap */
        if (is_in_range) {
            ...
            rv = _bcm_petra_multicast_egress_vlan_membership_port_add_or_remove(unit, multicast_id_ndx, local_port, 0, is_delete);

            if (rv == BCM_E_NOT_FOUND) {
                LOG_VERBOSE(BSL_LS_BCM_L2, 
                            (BSL_META_U(unit,"bcm_l2_addr_delete address not found\n")));
            }
            BCMDNX_IF_ERR_NOT_E_NOT_FOUND_EXIT(rv);
        } else {
            SOC_TMC_ERROR mc_err = BCM_E_NONE;
            ...
            if (SOC_IS_ARAD(unit) && SAL_BOOT_QUICKTURN) {         
              ...
              for (multicast_id = 0; multicast_id < BCM_PETRA_MULTICAST_NOF_BRIDGE_ROUTE; ++multicast_id) {     
                  if (!SOC_IS_PETRAB(unit) || multicast_id < range_low || multicast_id > range_high) {
                      ...
                      rv = bcm_dpp_am_mc_alloc(unit, BCM_DPP_AM_FLAG_ALLOC_WITH_ID, &multicast_id_ndx, TRUE);
                        BCMDNX_IF_ERR_EXIT(rv);
                    }      
                }        
            } else {
