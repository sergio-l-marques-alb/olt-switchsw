        ...
        if (profile_type == _bcmDppFieldProfileTypeFlpProgramProfile) {
            ...
            if (entryType == _bcmDppFieldEntryTypeDirExt) {
                ...
                for (qual_ndx = 0; qual_ndx < SOC_PPD_FP_NOF_QUALS_PER_DB_MAX; qual_ndx++) {
                    ...
                    if (entryDe_hwType == SOC_PPD_FP_QUAL_FWD_PRCESSING_PROFILE) {
                 ...
                 */
                 for (qual_ndx = 0; qual_ndx < SOC_PPD_FP_NOF_QUALS_PER_DB_MAX; qual_ndx++) {
                    _BCM_DPP_FIELD_TCAM_ENTRY_QUALS_GET(unit,
                        ...
                        qual_ndx,
                        entryQual);
                    if (entryQual.hwType == SOC_PPD_FP_QUAL_FWD_PRCESSING_PROFILE) {
...
#endif  /* _BCM_SPECIAL_SHOW_FOR_RANGE */
    {
        if (BCM_FIELD_ENTRY_INVALID != qualData.qualType) {
            ...
            LOG_CLI((BSL_META_U(unit,
                     ...
                     qualData.qualType,
                     SOC_PPD_FP_QUAL_TYPE_to_string(qualData.hwType),
                     qualData.hwType));
                              ...
                              SOC_PPD_FP_ACTION_TYPE **ppdAct)
{
    ...
    (*ppdAct) = (SOC_PPD_FP_ACTION_TYPE*)(&(actMap[base]));
exit:
    BCMDNX_FUNC_RETURN;
}
                                ...
                                SOC_PPD_FP_QUAL_TYPE **ppdQual)
{
    ...
    /* return the pointer to it to the caller */
    SOC_SAND_QUALMAPS_CONVERT_HANDLE_TO_ELEMENT(unit,qualMaps_handle,qualMap) ;
    (*ppdQual) = (SOC_PPD_FP_QUAL_TYPE*)(&(qualMap[base]));
exit:
    BCMDNX_FUNC_RETURN;
}
...
{
    _bcm_dpp_field_qual_t *qualData;
    SHR_BITDCL pqset[_SHR_BITDCLSIZE(SOC_PPD_NOF_FP_QUAL_TYPES)];
    ...
    unit = unitData->unit;
    SHR_BITCLR_RANGE( pqset, 0, (_SHR_BITDCLSIZE(SOC_PPD_NOF_FP_QUAL_TYPES) * SHR_BITWID));
    ...
    if (_BCM_DPP_FIELD_ENTRY_IS_ENTRY(entry)) {
        ...
        FIELD_ACCESS.groupD.pqset.bit_range_read(unit, entryGroup, 0, 0, _SHR_BITDCLSIZE(SOC_PPD_NOF_FP_QUAL_TYPES) * (8 * sizeof(int)/*32bits in int*/), pqset);
     } else if (_BCM_DPP_FIELD_ENTRY_IS_PRESEL(entry)) {
        /*  *pqset = &(unitData->stageD[*stage].ppqset);*/
        FIELD_ACCESS.stageD.ppqset.bit_range_read(unit, stage, 0, 0, _SHR_BITDCLSIZE(SOC_PPD_NOF_FP_QUAL_TYPES) * (8 * sizeof(int)/*32bits in int*/), pqset);
    } else {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
                                    ...
                                    uint64 *mask)
{
    ...
    if (_BCM_DPP_FIELD_ENTRY_IS_ENTRY(entry)) {
            BCMDNX_IF_ERR_EXIT(FIELD_ACCESS.groupD.pqset.bit_range_read(unit, entryGroup, 0, 0, _SHR_BITDCLSIZE(SOC_PPD_NOF_FP_QUAL_TYPES) * (8 * sizeof(int))/*32bits in int*/, pqset));
    } else if (_BCM_DPP_FIELD_ENTRY_IS_PRESEL(entry)) {
/*            *pqset = &(unitData->stageD[*stage].ppqset);*/
            FIELD_ACCESS.stageD.ppqset.bit_range_read(unit, stage, 0, 0, _SHR_BITDCLSIZE(SOC_PPD_NOF_FP_QUAL_TYPES) * (8 * sizeof(int)/*32bits in int*/), pqset);
    }
