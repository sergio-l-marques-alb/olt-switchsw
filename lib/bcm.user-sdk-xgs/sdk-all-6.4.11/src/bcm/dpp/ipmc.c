  ...
  )
{
    ...
    SOC_PPD_FRWRD_IPV4_MC_ROUTE_KEY_clear(&ssm_route_key);
    SOC_PPD_FRWRD_IPV4_MC_ROUTE_INFO_clear(&ssm_route_info);
    ...
    if (data->s_ip_addr == 0) {
        ...
        if (ipmc_ssm_mode != BCM_IPMC_SSM_DISABLE) {
            ...
            if(ipmc_ssm_mode == BCM_IPMC_SSM_KAPS_LPM)
            {
                /*KAPS routing key */
                ...
                /* TCAM routing key */
                soc_sand_rv = (MBCM_PP_DRIVER_CALL(unit,mbcm_pp_ipmc_ssm_tcam_entry_add,(unit, &ssm_route_key, &ssm_route_info, &failure_indication)));
            }
            SOC_SAND_IF_FAIL_RETURN(failure_indication);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        }
    ...
    if (data->s_ip_addr == 0) {
        ...
        if (ipmc_ssm_mode != BCM_IPMC_SSM_DISABLE) {
            ...
            if(ipmc_ssm_mode == BCM_IPMC_SSM_KAPS_LPM)
            {
                ...
                /*TCAM get entry*/
                soc_sand_rv = (MBCM_PP_DRIVER_CALL(unit,mbcm_pp_ipmc_ssm_tcam_entry_get,(unit, &ssm_route_key, &ssm_route_info, &route_status, &found)));
            }
  ...
  )
{
    uint32                           soc_sand_rv;
    bcm_error_t                      rv;
    SOC_PPD_FRWRD_MACT_ENTRY_KEY     route_key;
    SOC_PPC_FRWRD_IPV4_MC_ROUTE_KEY  ssm_route_key;
    bcm_ipmc_addr_t                  data_find;
    uint8                            ipmc_ssm_mode;
    ...
    if (data->s_ip_addr == 0) {
        ...
        if (ipmc_ssm_mode != BCM_IPMC_SSM_DISABLE) {
            ...
            {
                /*MCAT remove entry*/
                soc_sand_rv = (MBCM_PP_DRIVER_CALL(unit,mbcm_pp_ipmc_ssm_tcam_entry_delete,(unit, &ssm_route_key)));
            }
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        }
        else { /* IPMC SSM disabled */
