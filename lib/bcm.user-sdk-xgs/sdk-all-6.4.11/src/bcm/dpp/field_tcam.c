...
int _bcm_dpp_field_tcam_entry_build_qualifiers_from_sw_state(bcm_dpp_field_info_OLD_t *unitData, SOC_PPD_FP_ENTRY_INFO *entryInfo, _bcm_dpp_field_ent_idx_t entry, uint8 isExternalTcam, unsigned int entryMaxQuals, _bcm_dpp_field_stage_idx_t stage);
                                           ...
                                           _bcm_dpp_field_ent_idx_t entry)
{
        ...
        if (_BCM_DPP_FIELD_PPD_ACTION_VALID(hwType)) {
            ...
            LOG_DEBUG(BSL_LS_BCM_FP,
                      (BSL_META_U(unit,
                       ...
                       _BCM_DPP_FIELD_TCAM_GLOBAL_ID(unit, entry, isExternalTcam),
                       index,
                       SOC_PPD_FP_ACTION_TYPE_to_string(hwType),
                       hwType,
                       hwFlags,
                       hwParam));
            switch (hwType) {
            case SOC_PPD_FP_ACTION_TYPE_EGR_TRAP:
                ...
                if (hwFlags &
                    ...
                    if (SOC_IS_ARAD(unit) 
                        ...
                        && (hwParam != SOC_PPD_TRAP_ACTION_PKT_DISCARD_ID)) {
    ...
    if (BCM_E_NONE == result) {
        ...
        if (BCM_E_NONE == result) {
            ...
            if (SOC_IS_PETRAB(unit)) {
                _BCM_DPP_FIELD_TCAM_ENTRY_HW_PRIORITY_GET(unit, isExternalTcam, entry, hwPriority);
                /* update the entry's hardware priority */
                hwPriority ^= 1;
                _BCM_DPP_FIELD_TCAM_ENTRY_HW_PRIORITY_SET(unit, isExternalTcam, entry, hwPriority);
            }
                               ...
                               const char *prefix)
{
    ...
    if (flags & _BCM_DPP_FIELD_ENTRY_IN_USE) {
        ...
        dumpedRanges = FALSE;
        {
            _bcm_dpp_field_qual_t qualData[SOC_PPD_FP_NOF_QUALS_PER_DB_MAX] ;
                                  ...
                                  _bcm_dpp_field_ent_idx_t entry)
{
    ...
    if (entryFlags & _BCM_DPP_FIELD_ENTRY_IN_USE) {        
        ...
        if (SOC_IS_ARAD(unit)) {
            ...
            if (orig_flags & _BCM_DPP_FIELD_ENTRY_NEW) {
                /*
...
/* coverity[uninit_use_in_call] */
            ...
            case bcmFieldActionIEEE1588: /*new action*/
                ...
                hwAct[ppdIndex].hwType = SOC_PPD_FP_ACTION_TYPE_IEEE_1588;
                hwAct[ppdIndex].hwParam = ( propActs[bcmIndex].bcmParam0 & 0xFFF );
                break;
            ...
            case bcmFieldActionInterfaceClassVPort: 
                ...
                hwAct[ppdIndex].hwType = SOC_PPD_FP_ACTION_TYPE_IN_LIF_PROFILE;
            ...
            case bcmFieldActionStat:
                ...
                if (SOC_IS_ARAD(unit)) {
                    /*
                     * Enter for ARAD, ARAD+,and 'JERICHO and up'.
                     */
                    uint32
                        stat_id, stat_id_mask,
			...
			if (param0 == BCM_ILLEGAL_ACTION_PARAMETER) {
                            ...
                            if(!soc_property_suffix_num_get(unit,-1,spn_CUSTOM_FEATURE,"lsr_cnt_egress",0)) {
                             if (param2 == BCM_ILLEGAL_ACTION_PARAMETER) {
    			                ...
    			                if (param1 == BCM_ILLEGAL_ACTION_PARAMETER) {
                                     ...
                                     *    param0 = Stat-ID
                                     */
                                    control_ndx.type = SOC_PPD_FP_CONTROL_TYPE_ACE_POINTER_ONLY ;
    			        ...
    			        if (param1 != BCM_ILLEGAL_ACTION_PARAMETER) {
                                 ...
                                 */
                                control_ndx.type = SOC_PPD_FP_CONTROL_TYPE_ACE_POINTER_OUT_LIF ;
                                control_ndx.val_ndx = cntr ;
                            ...
                            if(soc_property_suffix_num_get(unit,-1,spn_CUSTOM_FEATURE,"lsr_cnt_egress",0)) {
                                ...
                                hwAct[ppdIndex + 2].hwType = SOC_PPD_FP_ACTION_TYPE_ACE_TYPE ;
            ...
            case bcmFieldActionRedirect:
                ...
                if (SOC_IS_ARAD(unit)) {
                    ...
                    if (SOC_IS_JERICHO(unit)) {
                        /*
                         * For JERICHO, we need to specify ACE_TYPE as '0' to just do 'redirect'
                         */
                        hwAct[ppdIndex + 2].hwType = SOC_PPD_FP_ACTION_TYPE_ACE_TYPE ;
                        hwAct[ppdIndex + 2].hwParam = ACE_TYPE_VALUE_NULL ;
                        /*
                         * Indicate: Two actions are loaded into 'hwAct'
                         */
                        actCount = 2 ;
                    }
                }
        ...
        } else if (SOC_PPD_FP_DATABASE_STAGE_INGRESS_FLP == _BCM_DPP_FIELD_STAGE_DEV_INFO(unit, groupD_stage).hwStageId) {
            switch (propActs[bcmIndex].bcmType) {
...
exit:
    {
        _bcm_dpp_field_tc_p_act_t loc_tcActP[SOC_PPD_FP_NOF_ACTIONS_PER_DB_MAX];
                           ...
                           _bcm_dpp_field_ent_idx_t entry)
{
    ...
    for (index = 0; index < SOC_PPD_FP_NOF_ACTIONS_PER_DB_MAX; index++) {
         ...
         */
        hwType = SOC_PPD_FP_ACTION_TYPE_INVALID ;
                ...
                if (BCM_E_NONE == result) {
                    if (found) {
                        ...
                        if (index < SOC_PPD_FP_NOF_ACTIONS_PER_DB_MAX) {
                            ...
                            *param1 = 0;
                            if ((entInfo.actions[index].type == SOC_PPC_FP_ACTION_TYPE_FLP_ACTION_0) && (index < SOC_PPD_FP_NOF_ACTIONS_PER_DB_MAX-1)) {
...
int
    _bcm_dpp_field_tcam_entry_build_qualifiers_from_sw_state(bcm_dpp_field_info_OLD_t *unitData, SOC_PPD_FP_ENTRY_INFO *entryInfo, _bcm_dpp_field_ent_idx_t entry, uint8 isExternalTcam, unsigned int entryMaxQuals, _bcm_dpp_field_stage_idx_t stage)
{
    int index, offset;
   _bcm_dpp_field_qual_t qual ;
        ...
        if (_BCM_DPP_FIELD_PPD_QUAL_VALID(qual.hwType)) {
            LOG_DEBUG(BSL_LS_BCM_FP,(BSL_META_U(unit,"unit %d entry %u qual %d(%s)->%d(%s)"" %08X%08X/%08X%08X\n"),
                        ...
                        */
                        ...
                        qual.hwType,
                        SOC_PPD_FP_QUAL_TYPE_to_string(qual.hwType),
