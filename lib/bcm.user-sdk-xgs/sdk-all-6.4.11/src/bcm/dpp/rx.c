    ...
    int unit,SOC_PPD_TRAP_FRWRD_ACTION_PROFILE_INFO* prm_profile_info,bcm_rx_trap_config_t *config)
{
    ...
    if (prm_profile_info->bitmap_mask & SOC_PPD_TRAP_ACTION_PROFILE_OVERWRITE_DEST) {        
        if (prm_profile_info->dest_info.frwrd_dest.type == SOC_PPD_FRWRD_DECISION_TYPE_MC) {
            ...
            if (prm_profile_info->bitmap_mask & SOC_PPD_TRAP_ACTION_PROFILE_OVERWRITE_DEST_OUTLIF)
            {
    ...
    if (prm_profile_info->bitmap_mask & SOC_PPD_TRAP_ACTION_PROFILE_OVERWRITE_FRWRD_TYPE) {
    ...
    if (prm_profile_info->bitmap_mask & SOC_PPD_TRAP_ACTION_PROFILE_OVERWRITE_METER_0) 
    {
...
int _bcm_dpp_rx_egress_set_to_cpu_drop_info(int unit, SOC_PPD_TRAP_EG_ACTION_PROFILE_INFO *eg_cpu_drop_info)
{
    ...
    eg_cpu_drop_info->out_tm_port = SOC_PPD_TRAP_ACTION_PKT_CPU_ID;
...
_bcm_dpp_trap_root_mate_get(int unit, int trap_id)
{
	...
	case SOC_PPD_TRAP_CODE_MPLS_PWE_NO_BOS:
		rv = SOC_PPD_TRAP_CODE_MPLS_PWE_NO_BOS_LABEL_14;
		break;
	case SOC_PPD_TRAP_CODE_MPLS_TERM_TTL_0:
		rv = SOC_PPD_TRAP_CODE_MPLS_TTL0;
		break;
	case SOC_PPD_TRAP_CODE_MPLS_TERM_TTL_1:
		rv = SOC_PPD_TRAP_CODE_MPLS_TTL1;
		break;
	case SOC_PPD_TRAP_CODE_MPLS_TERM_CONTROL_WORD_DROP:
	case SOC_PPD_TRAP_CODE_MPLS_TERM_CONTROL_WORD_TRAP:
		rv = SOC_PPD_TRAP_CODE_MPLS_CONTROL_WORD_TRAP;
		break;
	case SOC_PPD_TRAP_CODE_IPV4_TERM_SIP_EQUAL_DIP:
		if (SOC_IS_PETRAB(unit)) {
			rv = SOC_PPD_TRAP_CODE_IPV4_SIP_EQUAL_DIP;
		}
		break;

	case SOC_PPD_TRAP_CODE_IPV4_TERM_DIP_ZERO:
		if (SOC_IS_PETRAB(unit)) {
			rv = SOC_PPD_TRAP_CODE_IPV4_DIP_ZERO;
		}
		break;
	case SOC_PPD_TRAP_CODE_IPV4_TERM_SIP_IS_MC:
		if (SOC_IS_PETRAB(unit)) {
			rv = SOC_PPD_TRAP_CODE_IPV4_SIP_IS_MC;
		}
		break;
	case SOC_PPD_TRAP_CODE_MC_USE_SIP_RPF_FAIL:
		if (SOC_IS_PETRAB(unit)) {
			rv = SOC_PPD_TRAP_CODE_MC_USE_SIP_AS_IS_RPF_FAIL;
		}
		break;
...
_bcm_dpp_trap_mate_get(int unit, int trap_id)
{
    ...
    case SOC_PPD_TRAP_CODE_MPLS_UNEXPECTED_BOS:
        ...
        break;
    /*Trap mates that are supported only in Jericho*/
    case SOC_PPD_TRAP_CODE_ETH_FL_IGMP_MEMBERSHIP_QUERY:
        if (SOC_IS_JERICHO(unit))
        {
              rv = SOC_PPD_TRAP_CODE_IGMP_MEMBERSHIP_QUERY;
              break;
        }
        else
        {
    ...
    case SOC_PPD_TRAP_CODE_ETH_FL_IGMP_REPORT_LEAVE_MSG:
        if (SOC_IS_JERICHO(unit))
        {
              rv = SOC_PPD_TRAP_CODE_IGMP_REPORT_LEAVE_MSG;
              break;
        }
        else
        {
    ...
    case SOC_PPD_TRAP_CODE_ETH_FL_IGMP_UNDEFINED:
        if (SOC_IS_JERICHO(unit))
        {
              rv = SOC_PPD_TRAP_CODE_IGMP_UNDEFINED;
              break;
        }
        else
        {
...
void _bcm_dpp_rx_trap_to_ppd_init(int unit)
{
    ...
    _bcm_rx_trap_to_ppd[bcmRxTrapMplsTerminationFail]           =    SOC_PPD_TRAP_CODE_MPLS_TERM_ERROR;
    ...
    _bcm_rx_trap_to_ppd[bcmRxTrapDfltDroppedPacket]                     =  SOC_PPD_TRAP_CODE_USER_DEFINED_DROP_TRAP;
    _bcm_rx_trap_to_ppd[bcmRxTrapDfltRedirectToCpuPacket]               =  SOC_PPD_TRAP_CODE_USER_DEFINED_DFLT_TRAP;
    _bcm_rx_trap_to_ppd[bcmRxTrapRedirectToCpuOamPacket]                =  SOC_PPD_TRAP_CODE_OAM_CPU;
    _bcm_rx_trap_to_ppd[bcmRxTrapSnoopOamPacket]                        =  SOC_PPD_TRAP_CODE_OAM_CPU_SNOOP;
    _bcm_rx_trap_to_ppd[bcmRxTrapRecycleOamPacket]                      =  SOC_PPD_TRAP_CODE_OAM_RECYCLE;
    _bcm_rx_trap_to_ppd[bcmRxTrapItmhSnoop0]                            =  SOC_PPD_TRAP_CODE_ITMH_SNOOP_0;  
    _bcm_rx_trap_to_ppd[bcmRxTrapItmhSnoop1]                            =  SOC_PPD_TRAP_CODE_ITMH_SNOOP_1;    
    _bcm_rx_trap_to_ppd[bcmRxTrapItmhSnoop2]                            =  SOC_PPD_TRAP_CODE_ITMH_SNOOP_2;    
    _bcm_rx_trap_to_ppd[bcmRxTrapItmhSnoop3]                            =  SOC_PPD_TRAP_CODE_ITMH_SNOOP_3;    
    _bcm_rx_trap_to_ppd[bcmRxTrapItmhSnoop4]                            =  SOC_PPD_TRAP_CODE_ITMH_SNOOP_4;    
    _bcm_rx_trap_to_ppd[bcmRxTrapItmhSnoop5]                            =  SOC_PPD_TRAP_CODE_ITMH_SNOOP_5;    
    _bcm_rx_trap_to_ppd[bcmRxTrapItmhSnoop6]                            =  SOC_PPD_TRAP_CODE_ITMH_SNOOP_6;    
    _bcm_rx_trap_to_ppd[bcmRxTrapItmhSnoop7]                            =  SOC_PPD_TRAP_CODE_ITMH_SNOOP_7;    
    _bcm_rx_trap_to_ppd[bcmRxTrapItmhSnoop8]                            =  SOC_PPD_TRAP_CODE_ITMH_SNOOP_8;    
    _bcm_rx_trap_to_ppd[bcmRxTrapItmhSnoop9]                            =  SOC_PPD_TRAP_CODE_ITMH_SNOOP_9;    
    _bcm_rx_trap_to_ppd[bcmRxTrapItmhSnoop10]                           =  SOC_PPD_TRAP_CODE_ITMH_SNOOP_10;   
    _bcm_rx_trap_to_ppd[bcmRxTrapItmhSnoop11]                           =  SOC_PPD_TRAP_CODE_ITMH_SNOOP_11;   
    _bcm_rx_trap_to_ppd[bcmRxTrapItmhSnoop12]                           =  SOC_PPD_TRAP_CODE_ITMH_SNOOP_12;   
    _bcm_rx_trap_to_ppd[bcmRxTrapItmhSnoop13]                           =  SOC_PPD_TRAP_CODE_ITMH_SNOOP_13;   
    _bcm_rx_trap_to_ppd[bcmRxTrapItmhSnoop14]                           =  SOC_PPD_TRAP_CODE_ITMH_SNOOP_14;   
    _bcm_rx_trap_to_ppd[bcmRxTrapItmhSnoop15]                           =  SOC_PPD_TRAP_CODE_ITMH_SNOOP_15;   
    _bcm_rx_trap_to_ppd[bcmRxTrapFcoeFcfPacket]                         =  SOC_PPD_TRAP_CODE_FCOE_WA_FIX_OFFSET;
    _bcm_rx_trap_to_ppd[bcmRxTrapVlanTagDiscard]                        =  SOC_PPD_TRAP_CODE_ACCEPTABLE_FRAME_TYPE_DROP;
    _bcm_rx_trap_to_ppd[bcmRxTrapVlanTagAccept]                         =  SOC_PPD_TRAP_CODE_ACCEPTABLE_FRAME_TYPE_ACCEPT;
    _bcm_rx_trap_to_ppd[bcmRxTrap1588Discard]                           =  SOC_PPD_TRAP_CODE_1588_PACKET_1;
    _bcm_rx_trap_to_ppd[bcmRxTrap1588Accepted]                          =  SOC_PPD_TRAP_CODE_1588_PACKET_2;            
    _bcm_rx_trap_to_ppd[bcmRxTrapBfdEchoOverIpv4]                       =  SOC_PPD_TRAP_CODE_BFD_ECHO; 
    _bcm_rx_trap_to_ppd[bcmRxTrapSat0]                                  =  SOC_PPD_TRAP_CODE_SAT_0;
    _bcm_rx_trap_to_ppd[bcmRxTrapSat1]                                  =  SOC_PPD_TRAP_CODE_SAT_1;
    _bcm_rx_trap_to_ppd[bcmRxTrapSat2]                                  =  SOC_PPD_TRAP_CODE_SAT_2;
    _bcm_rx_trap_to_ppd[bcmRxTrapOamMipSnoop2ndPass]                    =  SOC_PPD_TRAP_CODE_OAM_MIP_EGRESS_SNOOP_WITH_FTMH;
    _bcm_rx_trap_to_ppd[bcmRxTrapBfdOamDownMEP]                         =  SOC_PPD_TRAP_CODE_OAM_CPU; 
    _bcm_rx_trap_to_ppd[bcmRxTrapOamUpMEP]                              =  SOC_PPD_TRAP_CODE_OAM_CPU_MIRROR; 

    /*Jericho Ingress Traps*/
    _bcm_rx_trap_to_ppd[bcmRxTrapInnerIpCompMcInvalidIp]                =  SOC_PPD_TRAP_CODE_INNER_IP_COMP_MC_INVALID_IP_0;
    _bcm_rx_trap_to_ppd[bcmRxTrapIllegalReservedLabel]                  =  SOC_PPD_TRAP_CODE_MPLS_ILLEGAL_LABEL;
    _bcm_rx_trap_to_ppd[bcmRxTrapInnerMyMacAndIpDisabled]               =  SOC_PPD_TRAP_CODE_INNER_ETHERNET_MY_MAC_IP_DISABLED;     
    ...
    _bcm_rx_trap_to_ppd[bcmRxTrapOamBfdIpv6]                        = SOC_PPD_TRAP_CODE_TRAP_BFD_O_IPV6;
    ...
    /* ETPP traps */
    _bcm_rx_trap_to_ppd[bcmRxTrapEgTxFieldSnoop0]                    = _BCM_TRAP_ETPP_TO_ID(SOC_PPD_TRAP_CODE_ETPP_FIELD_SNOOP_0);
    _bcm_rx_trap_to_ppd[bcmRxTrapEgTxFieldSnoop1]                    = _BCM_TRAP_ETPP_TO_ID(SOC_PPD_TRAP_CODE_ETPP_FIELD_SNOOP_1);
    _bcm_rx_trap_to_ppd[bcmRxTrapEgTxFieldSnoop2]                    = _BCM_TRAP_ETPP_TO_ID(SOC_PPD_TRAP_CODE_ETPP_FIELD_SNOOP_2);
    _bcm_rx_trap_to_ppd[bcmRxTrapEgTxFieldSnoop3]                    = _BCM_TRAP_ETPP_TO_ID(SOC_PPD_TRAP_CODE_ETPP_FIELD_SNOOP_3);
    _bcm_rx_trap_to_ppd[bcmRxTrapEgTxFieldSnoop4]                    = _BCM_TRAP_ETPP_TO_ID(SOC_PPD_TRAP_CODE_ETPP_FIELD_SNOOP_4);
    _bcm_rx_trap_to_ppd[bcmRxTrapEgTxFieldSnoop5]                    = _BCM_TRAP_ETPP_TO_ID(SOC_PPD_TRAP_CODE_ETPP_FIELD_SNOOP_5);
    _bcm_rx_trap_to_ppd[bcmRxTrapEgTxFieldSnoop6]                    = _BCM_TRAP_ETPP_TO_ID(SOC_PPD_TRAP_CODE_ETPP_FIELD_SNOOP_6);
    _bcm_rx_trap_to_ppd[bcmRxTrapEgTxFieldSnoop7]                    = _BCM_TRAP_ETPP_TO_ID(SOC_PPD_TRAP_CODE_ETPP_FIELD_SNOOP_7);
    _bcm_rx_trap_to_ppd[bcmRxTrapEgTxFieldSnoop8]                    = _BCM_TRAP_ETPP_TO_ID(SOC_PPD_TRAP_CODE_ETPP_FIELD_SNOOP_8);
    _bcm_rx_trap_to_ppd[bcmRxTrapEgTxFieldSnoop9]                    = _BCM_TRAP_ETPP_TO_ID(SOC_PPD_TRAP_CODE_ETPP_FIELD_SNOOP_9);
    _bcm_rx_trap_to_ppd[bcmRxTrapEgTxFieldSnoop10]                   = _BCM_TRAP_ETPP_TO_ID(SOC_PPD_TRAP_CODE_ETPP_FIELD_SNOOP_10);
    _bcm_rx_trap_to_ppd[bcmRxTrapEgTxFieldSnoop11]                   = _BCM_TRAP_ETPP_TO_ID(SOC_PPD_TRAP_CODE_ETPP_FIELD_SNOOP_11);
    _bcm_rx_trap_to_ppd[bcmRxTrapEgTxFieldSnoop12]                   = _BCM_TRAP_ETPP_TO_ID(SOC_PPD_TRAP_CODE_ETPP_FIELD_SNOOP_12);
    _bcm_rx_trap_to_ppd[bcmRxTrapEgTxFieldSnoop13]                   = _BCM_TRAP_ETPP_TO_ID(SOC_PPD_TRAP_CODE_ETPP_FIELD_SNOOP_13);
    _bcm_rx_trap_to_ppd[bcmRxTrapEgTxFieldSnoop14]                   = _BCM_TRAP_ETPP_TO_ID(SOC_PPD_TRAP_CODE_ETPP_FIELD_SNOOP_14);
    _bcm_rx_trap_to_ppd[bcmRxTrapEgTxFieldSnoop15]                   = _BCM_TRAP_ETPP_TO_ID(SOC_PPD_TRAP_CODE_ETPP_FIELD_SNOOP_15);
    _bcm_rx_trap_to_ppd[bcmRxTrapOutVPortDiscard]                    = _BCM_TRAP_ETPP_TO_ID(SOC_PPD_TRAP_CODE_ETPP_OUT_VPORT_DISCARD);
    _bcm_rx_trap_to_ppd[bcmRxTrapEgTxStpStateFail]                   = _BCM_TRAP_ETPP_TO_ID(SOC_PPD_TRAP_CODE_ETPP_STP_STATE_FAIL);
    _bcm_rx_trap_to_ppd[bcmRxTrapEgTxProtectionPathUnexpected]       = _BCM_TRAP_ETPP_TO_ID(SOC_PPD_TRAP_CODE_ETPP_PROTECTION_PATH_UNEXPECTED);
    _bcm_rx_trap_to_ppd[bcmRxTrapOutVPortLookupFail]                 = _BCM_TRAP_ETPP_TO_ID(SOC_PPD_TRAP_CODE_ETPP_VPORT_LOOKUP_FAIL);
    _bcm_rx_trap_to_ppd[bcmRxTrapEgTxMtuFilter]                      = _BCM_TRAP_ETPP_TO_ID(SOC_PPD_TRAP_CODE_ETPP_MTU_FILTER);
    _bcm_rx_trap_to_ppd[bcmRxTrapEgTxDiscardFrameTypeFilter]         = _BCM_TRAP_ETPP_TO_ID(SOC_PPD_TRAP_CODE_ETPP_ACC_FRAME_TYPE);
    _bcm_rx_trap_to_ppd[bcmRxTrapEgTxSplitHorizonFilter]             = _BCM_TRAP_ETPP_TO_ID(SOC_PPD_TRAP_CODE_ETPP_SPLIT_HORIZON);


    _bcm_rx_trap_to_ppd[bcmRxTrapUserDefine]                        = SOC_PPD_TRAP_CODE_USER_DEFINED_0;
    _bcm_rx_trap_to_ppd[bcmRxTrapArpIPUnknown]                   = -1; /* Not supported by HW will be removed from 6.5*/
...
int _bcm_dpp_rx_ppd_trap_get(int unit,
    ...
    *soc_ppd_trap_id = _bcm_rx_trap_to_ppd[trap_type];
    ...
    if (SOC_IS_ARADPLUS_AND_BELOW(unit)) 
    {
        ...
        {
            case SOC_PPD_TRAP_CODE_INNER_IP_COMP_MC_INVALID_IP_0:
                *soc_ppd_trap_id = -1;
                break;
            case SOC_PPD_TRAP_CODE_MPLS_ILLEGAL_LABEL:
                *soc_ppd_trap_id = -1;
                break;
            case SOC_PPD_TRAP_CODE_INNER_ETHERNET_MY_MAC_IP_DISABLED:
                *soc_ppd_trap_id = -1;
                break;
        }
    }
    ...
    int trap_id)
{
    ...
    SOC_PPD_TRAP_ETPP_INFO                      default_etpp_trap_info;
    ...
    {
        SOC_PPD_TRAP_ETPP_INFO_clear(&default_etpp_trap_info); 
    ...
    bcm_rx_trap_config_t *config)
{
    ...
    SOC_PPD_TRAP_ETPP_INFO                     etpp_trap_info;
    bcm_dpp_rx_virtual_traps_t                 virtual_traps_p;
    uint32                                     oamp_error_trap_id;
    int                                        trap_id_sw; /*in case "use the hw id" soc is turned on - will be used to store the sw hw id */
    ...
    SOC_PPD_TRAP_ETPP_INFO_clear(&etpp_trap_info); 
    ...
    if (_BCM_TRAP_ID_IS_ETPP(trap_id_sw))
    {
        ...
        if ( (SOC_PPD_TRAP_CODE_USER_DEFINED_0 <= trap_id_sw) && (trap_id_sw <= _BCM_LAST_USER_DEFINE) ) { 
        ...
        if (config->flags & (BCM_RX_TRAP_UPDATE_DEST | BCM_RX_TRAP_DEST_MULTICAST)) {
            ...
            else
            {
                ...
                {
                    prm_profile_info.bitmap_mask |= SOC_PPD_TRAP_ACTION_PROFILE_OVERWRITE_DEST_OUTLIF;
                    ...
                    prm_profile_info.dest_info.frwrd_dest.additional_info.outlif.type = SOC_PPD_OUTLIF_ENCODE_TYPE_RAW;
        ...
        if (config->flags & BCM_RX_TRAP_UPDATE_POLICER) {
            ...
            prm_profile_info.bitmap_mask |= SOC_PPD_TRAP_ACTION_PROFILE_OVERWRITE_METER_0;
        ...
        if (config->flags & BCM_RX_TRAP_UPDATE_COUNTER_2) {
            prm_profile_info.bitmap_mask  |= SOC_PPD_TRAP_ACTION_PROFILE_OVERWRITE_COUNTER_1;
            prm_profile_info.count_info.counter_id = config->counter_num;
        }    
        ...
        if (config->flags & BCM_RX_TRAP_UPDATE_FORWARDING_TYPE) {
            prm_profile_info.bitmap_mask |= SOC_PPD_TRAP_ACTION_PROFILE_OVERWRITE_FRWRD_TYPE;
        ...
        if (_BCM_TRAP_ID_IS_VIRTUAL(trap_id_sw)) 
        {
         ...
         if (!SOC_IS_ARADPLUS_AND_BELOW(unit))
         {
            if (config->flags & BCM_RX_TRAP_UPDATE_DEST) 
            {
                ...
                prm_egr_profile_info.bitmap_mask |= SOC_PPD_TRAP_ACTION_PROFILE_OVERWRITE_PP_DSP;
                prm_egr_profile_info.bitmap_mask |= SOC_PPD_TRAP_ACTION_PROFILE_USE_ARR;

                /* Multi core device or a single core device with use of config_arr*/
                if (cores_num > 1)
                {
                    ...
                    if (config->dest_port == BCM_GPORT_BLACK_HOLE)
                    {
                        ...
                        {
                            prm_egr_profile_info.out_tm_port_arr[core] =  SOC_PPD_TRAP_ACTION_PKT_DISCARD_ID;
                        }
                    }
                    else
                    {                     
                        ...
                        BCM_DPP_CORES_ITER(_SHR_CORE_ALL, core)
                        {
                            _bcm_dpp_rx_fill_destination_info_egress_trap( unit,core, config->core_config_arr[core].dest_port, &prm_egr_profile_info, &prm_profile_info);
                           
                        }
                    }

                }
                else  /* single core device */
                {     

                    if (config->core_config_arr_len > 0)
                    {
                        dest_port = config->core_config_arr[0].dest_port;
                    }
                    /* we support the old way of transfering the destination gport  */
                    else
                    {
                        dest_port = config->dest_port;
                    }

                    _bcm_dpp_rx_fill_destination_info_egress_trap(unit, 0, dest_port ,&prm_egr_profile_info, &prm_profile_info);            
                }
            }
             /* if BCM_RX_TRAP_UPDATE_ENCAP_ID is set, destination (other than DROP) and outlif can be updated */
            if (config->flags & BCM_RX_TRAP_UPDATE_ENCAP_ID) 
            {
                 prm_egr_profile_info.bitmap_mask |= SOC_PPD_TRAP_ACTION_PROFILE_OVERWRITE_CUD;
                 prm_egr_profile_info.bitmap_mask |= SOC_PPD_TRAP_ACTION_PROFILE_USE_ARR;
                 /* Multi core device or a single core device with use of config_arr*/
                 if (cores_num > 1)
                 {
                    ...
                    BCM_DPP_CORES_ITER(_SHR_CORE_ALL, core)
                    {
                        if (BCM_GPORT_IS_SET(config->core_config_arr[core].encap_id)) 
                        {
                            rv = _bcm_dpp_field_gport_to_global_lif_convert(unit, config->encap_id, 0 /*out lif */, &out_lif_id);
                            BCM_IF_ERROR_RETURN(rv);
                        }
                        else
                        {
                            out_lif_id = config->core_config_arr[core].encap_id;
                        }
                ...
                {     
                    if (config->core_config_arr_len > 0)
                    {
                        rv = _bcm_dpp_field_gport_to_global_lif_convert(unit, config->core_config_arr[0].encap_id , 0 /*out lif */, &out_lif_id);
                        BCM_IF_ERROR_RETURN(rv);
                    }
                    /* we support the old way of transfering the destination gport  */
                    else
                    ...
                    prm_egr_profile_info.header_data_arr[0].cud = out_lif_id;
                }
            }
        ...
        if (config->flags & BCM_RX_TRAP_DROP_AND_LEARN) {
             prm_egr_profile_info.bitmap_mask |= SOC_PPD_TRAP_ACTION_PROFILE_OVERWRITE_DEST;
             if (SOC_IS_JERICHO(unit)) {
                prm_egr_profile_info.out_tm_port = SOC_PPD_TRAP_ACTION_PKT_DISCARD_ID; /* Mimic Drop configuration */
             } else {
                prm_egr_profile_info.out_tm_port = SOC_PPD_TRAP_ACTION_PKT_DISCARD_AND_LEARN_ID; 
             }             
        }
        ...
        if (config->flags & BCM_RX_TRAP_UPDATE_QOS_MAP_ID) 
        {
            ...
            prm_egr_profile_info.bitmap_mask |= SOC_PPD_TRAP_ACTION_PROFILE_OVERWRITE_COS_PROFILE;
...
{ 
    ...
    {    
        ...
        {
            if (prm_egr_profile_info.out_tm_port == SOC_PPD_TRAP_ACTION_PKT_DISCARD_AND_LEARN_ID ||
                (SOC_IS_JERICHO(unit) && prm_egr_profile_info.out_tm_port == SOC_PPD_TRAP_ACTION_PKT_DISCARD_ID /* mimic drop configuration */))
            {
            ...
            {      
                ...
                {      
                    ...
                    BCM_DPP_CORES_ITER(_SHR_CORE_ALL, core)
                    {
                        if(prm_egr_profile_info.out_tm_port_arr[core] == SOC_PPD_TRAP_ACTION_PKT_DISCARD_ID)
                        {
                            config->core_config_arr[core].dest_port = BCM_GPORT_BLACK_HOLE;
                        }
                        else
                        {
                            rv = soc_port_sw_db_tm_to_local_port_get(unit, core, prm_egr_profile_info.out_tm_port_arr[core] , &port_lcl);  
                            BCMDNX_IF_ERR_EXIT(rv);
                            BCM_GPORT_LOCAL_SET(config->core_config_arr[core].dest_port, port_lcl);
                        } 
                    }
                }
                else
        ...
        if (prm_egr_profile_info.bitmap_mask & SOC_PPD_TRAP_ACTION_PROFILE_OVERWRITE_CUD)
        {
            config->flags |= BCM_RX_TRAP_UPDATE_ENCAP_ID;
            if (!SOC_IS_ARADPLUS_AND_BELOW(unit))
            {
        ...
        if (prm_egr_profile_info.bitmap_mask & SOC_PPD_TRAP_ACTION_PROFILE_OVERWRITE_COS_PROFILE)
        {
...
/* is bcm886xx_rx_use_hw_trap_id SOC is turned on, trap_id_converted will store the HW trap id of trap_id_sw. if the SOC is turned off - will store the same value as the trap_id_sw*/
int
...
_bcm_dpp_rx_fill_destination_info_egress_trap(/*in*/ int unit, int core, int dest_port,
                                     /*out*/ SOC_PPD_TRAP_EG_ACTION_PROFILE_INFO *prm_egr_profile_info, SOC_PPD_TRAP_FRWRD_ACTION_PROFILE_INFO  *prm_profile_info)
{
    int rv = BCM_E_NONE;                             
    uint32   pp_port;
    uint32   tm_port;
