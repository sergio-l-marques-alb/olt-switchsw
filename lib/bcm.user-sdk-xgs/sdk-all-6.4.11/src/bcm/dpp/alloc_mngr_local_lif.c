...
_bcm_dpp_am_local_outlif_init(int unit)
{
    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        {
            /*
             * Initialize failover linked lists and their members.
             */
            uint32                       max_nof_list_members ;
  ...
  _bcm_dpp_am_local_outlif_extension_lif_id_get(int unit, int base_lif,  int *ext_lif)
{
    ...
    if (eedb_bank_info.is_extended)
    {
        ...
        ext_offset =  ( ( ( base_lif / 2) %_BCM_DPP_AM_EGRESS_LIF_NOF_ENTRIES_PER_HALF_BANK ) /4)* 2 + ( _BCM_DPP_AM_EGRESS_LIF_NOF_TOP_BANKS_ENTRIES_PER_TOP_BANK * ( eedb_bank_info.extension_bank % _BCM_DPP_AM_EGRESS_LIF_NOF_TOP_BANKS_PER_HALF_BANK ) );
