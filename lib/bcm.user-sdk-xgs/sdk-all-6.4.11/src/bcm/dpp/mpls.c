...
#define _BCM_DPP_MPLS_IS_FORBIDDEN_PUSH_PROFILE(push_profile) \
        ((push_profile) == ARAD_PP_EG_ENCAP_MPLS1_COMMAND_SWAP_val || (push_profile) == ARAD_PP_EG_ENCAP_MPLS1_COMMAND_POP_val) ? 1 : 0
    ...
    if (SOC_IS_JERICHO(unit) && SOC_DPP_CONFIG(unit)->pp.explicit_null_support
        ...
        && !SOC_WARM_BOOT(unit)
          ...
          if (bcm_dpp_am_l3_lif_is_allocated(unit, _BCM_DPP_AM_L3_LIF_IP_TERM, 0, SOC_DPP_CONFIG(unit)->pp.explicit_null_support_lif_id) == BCM_E_EXISTS) {
            ...
            rv = _bcm_dpp_lif_sw_resources_delete(unit, SOC_DPP_CONFIG(unit)->pp.explicit_null_support_lif_id, -1, _BCM_DPP_GPORT_SW_RESOURCES_INGRESS);
            BCMDNX_IF_ERR_EXIT(rv);
          }
    }
    ...
    int unit)
{
    ...
    */
    if (SOC_IS_JERICHO_B0_AND_ABOVE(unit) && soc_property_suffix_num_get(unit,-1,spn_CUSTOM_FEATURE,"push_profile_advanced_enable",0)) {
    ...
    if (SOC_IS_JERICHO_B0_AND_ABOVE(unit) && !SOC_WARM_BOOT(unit)) {
        uint32 null_value = 0;
        /* 2 */
    ...
    bcm_mpls_port_t *   mpls_port)
{
    ...
    if (_BCM_DPP_MPLS_IS_FORBIDDEN_PUSH_PROFILE(push_profile)) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Push command values 8-9 are reserved for swap and pop actions.")));
    }
...
int _bcm_mpls_port_delete(
   ...
   bcm_gport_t          port_id
    ...
    if ((BCM_GPORT_SUB_TYPE_IS_PROTECTION(port_id) || BCM_GPORT_SUB_TYPE_IS_FORWARD_GROUP(port_id)) && (gport_hw_resources.fec_id != -1)) {
        rv = bcm_dpp_am_fec_get_usage(unit, BCM_GPORT_SUB_TYPE_IS_FORWARD_GROUP(port_id)?gport_hw_resources.fec_id:_BCM_PPD_GPORT_FEC_TO_WORK_FEC(gport_hw_resources.fec_id), &usage);
        BCMDNX_IF_ERR_EXIT(rv);
    ...
    bcm_mpls_port_t * mpls_port)
{
    ...
    if ((_BCM_PPD_IS_VALID_FAILOVER_ID(mpls_port->failover_id) && _BCM_MULTICAST_IS_SET(mpls_port->failover_mc_group)) ||
        ...
        else if (phy_port.type == _bcmDppGportResolveTypeForwardFec) {
            /* Get failover port id */
            SOC_PPD_FEC_ID work_fec_id = _BCM_PPD_GPORT_WORK_TO_PROTECT_FEC(phy_port.phy_gport);
            BCM_GPORT_MPLS_PORT_ID_SET(mpls_port->failover_port_id, work_fec_id);
        }
    ...
    int nof_dummy_profiles)
{
    ...
    for (indx = 0; indx < nof_dummy_profiles; indx++) {
        rv = _bcm_mpls_tunnel_push_profile_alloc(unit,&dummy_profiles_array[indx],0,&label_entry,FALSE,TRUE,FALSE,TRUE,NULL);
        BCMDNX_IF_ERR_EXIT(rv);
        label_entry.exp++;
    }
    ...
    int *label_count)
{
    ...
    if (!SOC_DPP_CONFIG(unit)->arad->pp_op_mode.mpls_info.mpls_coupling_enable &&
         encap_entry_info[0].entry_type == SOC_PPD_EG_ENCAP_ENTRY_TYPE_DATA) {
        /* NULL data entry */
        label_array[0].action = BCM_MPLS_EGRESS_ACTION_NOP;
        *label_count = 1;
    }
    ...
    bcm_mpls_tunnel_switch_t * info)
{
    ...
    if ((working_fec.type != SOC_PPC_FEC_TYPE_TUNNELING_EEI_MPLS_COMMAND) && (working_fec.type != SOC_PPC_FEC_TYPE_TUNNELING_EEI_OUTLIF)) {
    ...
    bcm_gport_t port)
{
    ...
    qual_vals[0].val.arr[0]= (label&0xffff);
    qual_vals[0].val.arr[1]= (label&0xf0000)>>16;
    qual_vals[0].is_valid.arr[0]= 0xffff;/*16 bit*/
    qual_vals[0].is_valid.arr[1]= 0xf;/*4 bit*/
...
int _bcm_gport_mpls_is_valid_replace(int unit, bcm_mpls_port_t *new_mpls_port, bcm_mpls_port_t *old_mpls_port, bcm_mpls_label_t old_egress_vc, uint32 old_push_profile){
    ...
    if ((!n_is_protected) && (!(new_mpls_port->flags2 & BCM_MPLS_PORT2_EGRESS_ONLY)) && 
        (!(new_mpls_port->flags & BCM_MPLS_PORT_COUNTED)) && (!(new_mpls_port->flags & BCM_MPLS_PORT2_LEARN_ENCAP)) && 
    ...
    uint8 is_one_plus_one_protection)
{
    ...
    }  else if(mpls_port->egress_tunnel_if == 0) {
        intf_global_lif = 0;
    }
    ...
    if (update) {
        if (!with_id) {
            ...
            if (BCM_GPORT_SUB_TYPE_IS_FORWARD_GROUP(mpls_port->mpls_port_id) || BCM_GPORT_SUB_TYPE_IS_PROTECTION(mpls_port->mpls_port_id)) {
                ...
                /* Check if fec is allocated */
                if (gport_hw_resources.fec_id != -1) {
                    rv = bcm_dpp_am_fec_get_usage(unit, 
                                                  BCM_GPORT_SUB_TYPE_IS_FORWARD_GROUP(mpls_port->mpls_port_id) ? gport_hw_resources.fec_id : _BCM_PPD_GPORT_FEC_TO_WORK_FEC(gport_hw_resources.fec_id), 
                                                  &fec_usage);
                    BCMDNX_IF_ERR_EXIT(rv);
