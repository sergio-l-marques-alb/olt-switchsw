...
int static _bcm_dpp_switch_L2LearnMode_set(int unit, int arg)
{
    ...
    if ((arg & BCM_L2_LEARN_CPU) &&
        ...
        if (SOC_IS_JERICHO(unit)) {
            ...
            {
                BCMDNX_IF_ERR_EXIT(soc_reg_above_64_field32_modify(unit, PPDB_B_LARGE_EM_INTERRUPT_MASK_REGISTER_TWOr, REG_PORT_ANY, 0,  MACT_MASTER_EVENT_READY_MASKf, 1));
                BCMDNX_IF_ERR_EXIT(soc_reg_above_64_field32_modify(unit, PPDB_B_LARGE_EM_INTERRUPT_MASK_REGISTER_TWOr, REG_PORT_ANY, 0,  MACT_SLAVE_EVENT_READY_MASKf, 1));
            }
        }
        else
        {
                                      ...
                                      int arg)
{
	...
	max_hash_offset = SOC_IS_PETRAB(unit) ? 3 : 15;
	max_hash_seed = SOC_IS_PETRAB(unit) ? 255 : 65535;
    ...
    case bcmSwitchMeterAdjust:
        if (SOC_IS_ARAD(unit)) {
			if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
				...
				BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("Not supported on QAX")));
			}
		} else {
           BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("Supported only in Arad")));
        }
        ...
        if ((bcm_type == bcmSwitchPortHeaderType) || (SOC_IS_ARAD_B0_AND_ABOVE(unit))) {
            if (bcm_type == bcmSwitchPortHeaderType) {
                ...
                if (arg == BCM_SWITCH_PORT_HEADER_TYPE_TDM_RAW || arg == BCM_SWITCH_PORT_HEADER_TYPE_TDM) 
                {
                    ...
                    BCMDNX_IF_ERR_EXIT(rv);
                    if ((arg == BCM_SWITCH_PORT_HEADER_TYPE_TDM) && SOC_IS_JERICHO(unit))
                    {
...
int bcm_petra_switch_tpid_add(
    ...
    SOC_PPD_ADDITIONAL_TPID_VALUES additional_tpids;

    BCMDNX_INIT_FUNC_DEFS;
    DPP_SWITCH_UNIT_VALID_CHECK;
    BCMDNX_NULL_CHECK(tpid_info);

    SOC_PPD_ADDITIONAL_TPID_VALUES_clear(&additional_tpids);
    ...
    bcm_switch_lif_property_config_t *lif_config)
{
    ...
    if ( (lif_property == bcmLifPropertyEEDBBankPhase) || (lif_property == bcmLifPropertyEEDBBankExtension) ||  (lif_property == bcmLifPropertyEEDBBankExtensionType))
    {
         eedb_bank_id =  lif_config->key  ;
         if ( ((lif_property == bcmLifPropertyEEDBBankPhase) && !BCM_DPP_AM_LOCAL_OUT_LIF_BANK_IN_RANGE(eedb_bank_id) ) ||
              (((lif_property == bcmLifPropertyEEDBBankExtension ) || (lif_property == bcmLifPropertyEEDBBankExtensionType))  && !BCM_DPP_AM_LOCAL_OUT_LIF_REGULAR_BANK_IN_RANGE(eedb_bank_id) ))
         {
              BCMDNX_ERR_EXIT_MSG(BCM_E_FAIL, (_BSL_BCM_MSG("bank_id not in range")));
         }
    }

    else if (lif_property == bcmLifPropertyOutGlobalLifRangeType)
    {
         eedb_bank_id =  (lif_config->key) / 2 ;
         if (!BCM_DPP_AM_LOCAL_OUT_LIF_BANK_IN_RANGE(eedb_bank_id))
         {
              BCMDNX_ERR_EXIT_MSG(BCM_E_FAIL, (_BSL_BCM_MSG("bank_id not in range")));
         }
    }
    ...
    start_bank_id = (lif_property == bcmLifPropertyOutGlobalLifRangeType) ? lif_config->key :  eedb_bank_id * 2;
    ...
    for (bank_id_iter = start_bank_id ; bank_id_iter <= end_bank_id ; bank_id_iter++ ) {
        ...
        case bcmLifPropertyOutGlobalLifRangeType :
            ...
            if (lif_config->value == BCM_SWITCH_LIF_PROPERTY_ID_RANGE_TYPE_DIRECT ) 
            {
    ...
    bcm_switch_lif_property_config_t *lif_config)
{
    ...
    bank_id = (lif_property == bcmLifPropertyOutGlobalLifRangeType) ? eedb_bank_id : eedb_bank_id * 2;
    ...
    bcm_switch_control_info_t info)
{
    ...
    case bcmSwitchForwardingLayerMtuSize:
        soc_sand_rv = arad_pp_vsi_egress_mtu_set(unit, TRUE /*Forwarding Layer*/, key.index, info.value);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        break;
    case bcmSwitchLinkLayerMtuSize:
        soc_sand_rv = arad_pp_vsi_egress_mtu_set(unit, FALSE /*Link Layer*/, key.index, info.value);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        break;
    default:
        break;
    ...
    bcm_switch_control_info_t *info)
{
    ...
    case bcmSwitchLinkLayerMtuFilter:
        ...
        break;
    case bcmSwitchForwardingLayerMtuSize:
        soc_sand_rv = arad_pp_vsi_egress_mtu_get(unit, TRUE /*Forwarding Layer*/, key.index, (uint32*)&(info->value));
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        break;
    case bcmSwitchLinkLayerMtuSize:
        soc_sand_rv = arad_pp_vsi_egress_mtu_get(unit, FALSE /*Link Layer*/, key.index, (uint32*)&(info->value));
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        break;
    default:
        break;
