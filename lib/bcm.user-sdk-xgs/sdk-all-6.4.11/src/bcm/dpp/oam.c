...
do { \
   ...
   rv = bcm_petra_oam_endpoint_action_set_internal(unit,mep_id, &action,0,0); \
    BCMDNX_IF_ERR_EXIT(rv);\
} while (0); 
...
int _bcm_dpp_oam_set_counter(int unit, int counter_base, uint8 is_pcp, uint32 lif) {
	...
	if (is_pcp) {
		...
		if (counter_range_min==0) { /* First endpoint with counter */
			...
			counter_range_max = counter_base+1; /* Add one because the ingress side uses counter_base + 1. 
                                                                              Applicable only in Jericho. In Arad it doesn't matter.*/
		}
		else {
			if (counter_base > counter_range_max) {
				counter_range_max = counter_base +1; /* +1 for the same reason as above.*/
			}
...
int _bcm_dpp_oam_core_from_lif_get(int unit, uint32 global_lif, uint32 *core){
    ...
    rv = _bcm_dpp_global_lif_mapping_global_to_local_get(unit, _BCM_DPP_GLOBAL_LIF_MAPPING_INGRESS, global_lif, &local_lif);
    BCMDNX_IF_ERR_EXIT(rv);
    rv = _bcm_dpp_lif_to_sw_resources(unit, local_lif, -1, _BCM_DPP_GPORT_SW_RESOURCES_INGRESS, &gport_sw_resources);
    BCMDNX_IF_ERR_EXIT(rv);
    rv = MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_local_to_pp_port_get, (unit, gport_sw_resources.in_lif_sw_resources.port, &pp_port, (int*)core));
    BCMDNX_IF_ERR_EXIT(rv);
        ...
        if (profile_info->profile_data) {
            ...
            if (profile != old_profile) {
                ...
                if (endpoint_info->timestamp_format) {
                    classifier_mep_entry.flags |= SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_FLAG_DM_1588;
                } else {
                    classifier_mep_entry.flags &= ~SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_FLAG_DM_1588;
                }
...
STATIC int _bcm_oam_endpoint_server_store_data(int unit, SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY *classifier_mep_entry, bcm_oam_endpoint_info_t *endpoint_info) {
    BCMDNX_INIT_FUNC_DEFS;
...
STATIC int _bcm_oam_endpoint_local_create(int unit, bcm_oam_endpoint_info_t *endpoint_info_lcl) {
    ...
    if ((endpoint_info_lcl->gport == BCM_GPORT_INVALID) && ((endpoint_info_lcl->flags2 & BCM_OAM_ENDPOINT2_MPLS_EGRESS_ONLY) == 0)) {
        ...
        classifier_mep_entry.md_level = endpoint_info_lcl->level;
        if (is_upmep) {
            classifier_mep_entry.flags |= SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_FLAG_UPMEP;
        }
        classifier_mep_entry.src_mac_lsb = endpoint_info_lcl->src_mac_address[5];
        /* Store server data */
        ...
        for (is_passive = 0; is_passive <= 1; is_passive++) {
            ...
            if(SOC_IS_JERICHO(unit) && (endpoint_info_lcl->flags2 & BCM_OAM_ENDPOINT_FLAGS2_MPLS_EGRESS_ONLY) && !is_passive){
                continue;
            }
								...
								bcm_oam_endpoint_info_t *endpoint_info)  
{
    ...
    if (_BCM_OAM_MEP_INDEX_IS_REMOTE(endpoint)) {
        ...
        if (_BCM_OAM_DISSECT_IS_CLASSIFIER_MEP_ACCELERATED(&classifier_mep_entry)) {
            ...
            if (classifier_mep_entry.flags & SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_FLAG_DOWNMEP_TX_GPORT_IS_LAG) {
...
int bcm_petra_oam_endpoint_action_set_internal(
    ...
    if (BCM_OAM_OPCODE_GET(*action_lcl, SOC_PPD_OAM_ETHERNET_PDU_OPCODE_CCM)) {
        if (is_accelerated) {
    ...
    if (BCM_OAM_OPCODE_GET(*action_lcl, SOC_PPD_OAM_ETHERNET_PDU_OPCODE_LBR)) {
        if (is_accelerated) {
...
int bcm_petra_oam_endpoint_action_set(
    ...
    return bcm_petra_oam_endpoint_action_set_internal(unit,id,action,0,0);
}
...
int bcm_petra_oam_group_destroy(
    ...
    if ((soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "maid48_enabled", 0)) 
        ...
        int is_last, rv;
        rv = _bcm_dpp_am_template_oamp_pe_gen_mem_maid48_free(unit, ma_name.index, &is_last); 
		BCMDNX_IF_ERR_EXIT(rv);
    }
...
int _bcm_oam_set_classifier_action(int unit, uint32 mep_id, bcm_oam_endpoint_action_t action,int is_lm,int is_1dm_or_piggy_back,SOC_PPC_OAM_MEP_TYPE mep_type ,int is_slm) 
{
    ...
    *meter_disable = lif_profile.mep_profile_data.meter_disable;
    ...
    if (is_slm) {
        ...
        if (!SHR_BITGET(counter_disable, SOC_PPC_OAM_OPCODE_MAP_DMM)) {
            BCM_OAM_ACTION_SET(action, bcmOAMActionCountEnable); 
        }
        if (!SHR_BITGET(meter_disable, SOC_PPC_OAM_OPCODE_MAP_DMM)) {
            BCM_OAM_ACTION_SET(action, bcmOAMActionMeterEnable); 
        }
        is_piggy_backed =0;
    } else {
    ...
    rv = bcm_petra_oam_endpoint_action_set_internal(unit,mep_id, &action,is_piggy_backed,0);
    BCMDNX_IF_ERR_EXIT(rv);
...
STATIC int loss_delay_add_client_side(int unit,int endpoint_id, int is_single_ended_or_1dm, int is_lm, 
    ...
    *meter_disable = lif_profile.mep_profile_data.meter_disable;
    ...
    rv = bcm_petra_oam_endpoint_action_set_internal(unit,endpoint_id, &action,is_lm && !is_single_ended_or_1dm,0);
    BCMDNX_IF_ERR_EXIT(rv);
...
int bcm_petra_oam_loss_update_non_accelerated_internal(
    ...
    BCMDNX_IF_ERR_EXIT(bcm_petra_oam_endpoint_action_set_internal(unit,id,action,set_unset /* piggyback */,0/*sat-loopback*/));
...
int _bcm_oam_loopback_acc_packet_generation_add(int unit, bcm_oam_loopback_t *loopback_ptr,
    ...
    *new_flags |= (SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_FLAG_MEP_HAS_LOOPBACK
                   | (soc_loopback_info.is_period_in_pps ?
                      SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_FLAG_LOOPBACK_PERIOD_IN_PPS : 0));
...
int sat_loopback_add_rx(int unit, bcm_oam_loopback_t *loopback_ptr) 
{
    ...
    if ((classifier_mep_entry.flags & SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_FLAG_UPMEP) == 0) { /* Down mep */
        ...
        rv = bcm_petra_oam_endpoint_action_set_internal(unit, loopback_ptr->id, &action, 0/*piggyback(ignored)*/, 1/*sat_loopback*/);
        BCMDNX_IF_ERR_EXIT(rv);
...
int sat_loopback_add_tx(int unit, bcm_oam_loopback_t *loopback_ptr) 
{
    ...
    if (classifier_mep_entry.flags & SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_FLAG_UPMEP) { /* Up mep */
    ...
    if (loopback_ptr->period != 0) { /* Use LBM functionality as well*/
        ...
        if (classifier_mep_entry.flags & SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_FLAG_MEP_HAS_LOOPBACK) {
            ...
            remove_flags |= (SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_FLAG_MEP_HAS_LOOPBACK
                             | (is_period_in_kpps ? SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_FLAG_LOOPBACK_PERIOD_IN_PPS : 0));
        }
    }
...
int _bcm_oam_loopback_acc_packet_generation_get(int unit, bcm_oam_loopback_t *loopback_ptr,
    ...
    soc_loopback_info.is_period_in_pps =
        ((classifier_mep_entry->flags & SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_FLAG_LOOPBACK_PERIOD_IN_PPS) != 0);
...
int bcm_petra_oam_tst_rx_add(int unit,bcm_oam_tst_rx_t *tst_ptr)
{
    ...
    else 
    {
        ...
        /* Send TSTs to the OAMP port (This also sets the corrct OAM-subtype
           to select the Eg.PRGE program that sets the trap-code. */
        action.destination = oamp_port[0];
        rv = bcm_petra_oam_endpoint_action_set_internal(unit, tst_ptr->endpoint_id, &action, 0/*piggyback(ignored)*/, 1/*sat_loopback*/);
        BCMDNX_IF_ERR_EXIT(rv);
        /* Set the Eg.PRGE program variable to the correct trap code   */
      ...
      bcm_pkt_blk_t *packet_header)
{
    ...
    if (packet_header->len > SOC_PPD_OAM_MAX_PROTECTION_HEADER_SIZE) {
