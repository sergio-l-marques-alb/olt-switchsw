/*
 * $Id: oam_hw_db.c,v 1.148 Broadcom SDK $
...
do {\
    ...
    classifier_mep_ptr->counter = actual_counter; \
} while (0)


#define _BCM_OAM_COUNTER_BASE_ID_FROM_CLASSIFIER_COUNTER(endpoint_info_ptr, classifier_mep_ptr)\
do {\
   endpoint_info_ptr->lm_counter_base_id = classifier_mep_ptr->counter;\
...
int _bcm_oam_mep_db_entry_dealloc(int unit, uint32 mep_index, SOC_PPD_OAM_OAMP_MEP_DB_ENTRY *mep_db_entry, SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY * mep_info) {
    ...
    if (SOC_IS_ARAD_B0_AND_ABOVE(unit) && PPC_API_OAM_STORE_LOCAL_PORT_IN_MEP_DB(mep_db_entry->mep_type) &&
         ((!(mep_info->flags & SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY_FLAG_UPMEP)) || (mep_info->flags & SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_FLAG_SERVER && mep_info->port) || mep_db_entry->system_port==0xffff)) {
        ...
        if ((mep_info->flags & SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY_FLAG_UPMEP) && (mep_info->flags & SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_FLAG_SERVER && mep_info->port)) {
...
int _bcm_oam_mep_db_entry_struct_set(int unit, bcm_oam_endpoint_info_t *endpoint_info, SOC_PPD_OAM_OAMP_MEP_DB_ENTRY *mep_db_entry, int icc_ndx, uint8 is_11b_maid) {
        ...
        if (SOC_IS_ARAD_B0_AND_ABOVE(unit) && PPC_API_OAM_STORE_LOCAL_PORT_IN_MEP_DB(mep_db_entry->mep_type)) {
            if (_BCM_OAM_IS_SERVER_SERVER(endpoint_info) && _BCM_OAM_DISSECT_IS_ENDPOINT_UPMEP(endpoint_info)) {
                /* In this special case local port serves three purposes: 1) pointer to local_port_2_sys_port which will serve as the SSP on the ITMH
                   ...
                   2) Pointer to the OAMP_PE_GEN_MEM table from which the local port itself is taken.
                   This will be the SSP on the inner PTCH, and is retreived from the tx_gport.
                   3) Additionaly in Arad this serves as the LSB of the src mac address, which has already been verified to match the given value.*/
                ...
                BCM_DPP_UNIT_CHECK(unit);
                /*tm --> pp*/
                local_port_for_real = tm_port; /* Core will always be 0 in Arad anyways.*/
	...
	if (endpoint_info->type == bcmOAMEndpointTypeEthernet) {
        ...
        if((endpoint_info->flags2 & BCM_OAM_ENDPOINT_FLAGS2_MPLS_INGRESS_ONLY)&&(endpoint_info->intf_id == BCM_GPORT_INVALID)
...
int _bcm_oam_mep_db_entry_struct_get(int unit, bcm_oam_endpoint_info_t *endpoint_info, SOC_PPD_OAM_OAMP_MEP_DB_ENTRY *mep_db_entry, int original_tx_gport) {
    ...
    if (endpoint_info->type == bcmOAMEndpointTypeEthernet) {
        ...
        if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
            ...
            BCMDNX_IF_ERR_EXIT(ret);
            if (mep_db_entry->is_upmep && SOC_IS_ARADPLUS_A0(unit) && !_BCM_OAM_IS_SERVER_SERVER(endpoint_info)) {
                /* The LSB was set on the counter pointer (without the leftmost bit) and applied via the OAMP-PE.*/
                /* But this doesn't apply to the server*/
                endpoint_info->src_mac_address[5] = mep_db_entry->counter_pointer >>1; 
            }
            if (_BCM_OAM_IS_SERVER_SERVER(endpoint_info)) {
                /* in the server the src mac lsb is in the local port.*/
                endpoint_info->src_mac_address[5] = mep_db_entry->local_port; 
            }
        } else {
            uint8 data[5];
...
int _bcm_oam_classifier_default_mep_entry_struct_set(int unit, bcm_oam_endpoint_info_t *endpoint_info, SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY * classifier_mep_entry) {
    ...
    classifier_mep_entry->flags |= _BCM_OAM_DISSECT_IS_ENDPOINT_UPMEP(endpoint_info) ? SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_FLAG_UPMEP : 0;
...
int _bcm_oam_classifier_mep_entry_struct_set(int unit, bcm_oam_endpoint_info_t *endpoint_info, SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY * classifier_mep_entry) {
	...
	classifier_mep_entry->flags |= _BCM_OAM_DISSECT_IS_ENDPOINT_MIP(endpoint_info)? 0 : SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_FLAG_ENDPOINT;
	...
	classifier_mep_entry->flags |= _BCM_OAM_DISSECT_IS_ENDPOINT_ACCELERATED(endpoint_info) ? SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_FLAG_ACCELERATED : 0;
	classifier_mep_entry->flags |= _BCM_OAM_DISSECT_IS_ENDPOINT_UPMEP(endpoint_info) ? SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_FLAG_UPMEP : 0;
    ...
    if (endpoint_info->gport != BCM_GPORT_INVALID) {
        if (((endpoint_info->type == bcmOAMEndpointTypeBHHMPLS)||(endpoint_info->type == bcmOAMEndpointTypeBHHPwe))
           ...
           flags = (endpoint_info->mpls_out_gport == BCM_GPORT_INVALID) ? _BCM_DPP_GPORT_HW_RESOURCES_GLOBAL_LIF_INGRESS | _BCM_DPP_GPORT_HW_RESOURCES_LOCAL_LIF_EGRESS : _BCM_DPP_GPORT_HW_RESOURCES_GLOBAL_LIF_INGRESS;
        }
        ...
        /*For egress only MEP, use the outlif to get hw resouces since inlif is invalid */
        if((endpoint_info->flags2& BCM_OAM_ENDPOINT_FLAGS2_MPLS_EGRESS_ONLY) &&(endpoint_info->mpls_out_gport != BCM_GPORT_INVALID)){
          if((endpoint_info->type == bcmOAMEndpointTypeBHHMPLS)||(endpoint_info->type == bcmOAMEndpointTypeBHHPwe)){
              ...
              BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Error: Egress only flag  can support MPLS and PWE only")));
          }
        }
        else {
    ...
    if ((endpoint_info->type == bcmOAMEndpointTypeBHHMPLS)||(endpoint_info->type == bcmOAMEndpointTypeBHHPwe)){
       if(endpoint_info->flags2& BCM_OAM_ENDPOINT_FLAGS2_MPLS_INGRESS_ONLY){
          classifier_mep_entry->passive_side_lif = _BCM_OAM_INVALID_LIF;
       }
       else if(endpoint_info->flags2& BCM_OAM_ENDPOINT_FLAGS2_MPLS_EGRESS_ONLY){
...
int _bcm_oam_classifier_mep_entry_struct_get(int unit, bcm_oam_endpoint_info_t *endpoint_info, SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY * classifier_mep_entry) {
    ...
    if((classifier_mep_entry->mep_type ==SOC_PPD_OAM_MEP_TYPE_Y1731_O_MPLSTP)|| (classifier_mep_entry->mep_type ==SOC_PPD_OAM_MEP_TYPE_Y1731_O_PWE)){
        /*egress only mep */
       if (classifier_mep_entry->flags & SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY_FLAG_MEP_EGRESS_ONLY){
          endpoint_info->flags2= BCM_OAM_ENDPOINT_FLAGS2_MPLS_EGRESS_ONLY;
       }
       else if(classifier_mep_entry->passive_side_lif == _BCM_OAM_INVALID_LIF){/*ingress only mep*/
          endpoint_info->flags2= BCM_OAM_ENDPOINT_FLAGS2_MPLS_INGRESS_ONLY;
       }
    }
