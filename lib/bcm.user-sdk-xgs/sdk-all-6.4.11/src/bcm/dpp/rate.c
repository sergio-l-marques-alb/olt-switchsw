...
SOC_PPD_MTR_BW_PROFILE_INFO diag_pp_eth_policer_config;

STATIC uint32 _bcm_petra_rate_translate_kbits_to_packets(uint32 kbits)
{
	uint32 packets= kbits/_DPP_RATE_PACKETS_TO_BYTES_FACTOR;
	packets *= _DPP_RATE_KBITS_TO_BYTES_FACTOR;
	return packets;
}

STATIC uint32 _bcm_petra_rate_translate_packets_to_kbits(uint32 packets)
{
	uint32 kbits = packets/_DPP_RATE_KBITS_TO_BYTES_FACTOR;
	kbits *= _DPP_RATE_PACKETS_TO_BYTES_FACTOR;
	return kbits;
}
    ...
    BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_i) {
        for(type_indx = 0; type_indx < nof_types; ++type_indx) {
			...
			if (SOC_IS_JERICHO(unit)) {
				...
				soc_sand_rv = jer_pp_mtr_eth_policer_params_set(unit, pp_port, core_id, types[type_indx], &policer);
                           ...
                           uint32 *kbits_sec, uint32 *kbits_burst)
{
    ...
    BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_i) {
        ...
        for(type_indx = 0; type_indx < nof_types; type_indx++) {
			...
			if (SOC_IS_JERICHO(unit)) {
				BCMDNX_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_local_to_pp_port_get, (unit, port_i, &pp_port, &core_id)));
				soc_sand_rv = jer_pp_mtr_eth_policer_params_get(unit, pp_port, core_id, types[type_indx], &policer);
				SOC_SAND_IF_ERR_EXIT(soc_sand_rv);		
			} else{
