...
_bcm_dpp_in_lif_mim_match_add(int unit, bcm_mim_port_t *mim_port, int lif)
{
    ...
    BCMDNX_IF_ERR_EXIT(_bcm_dpp_lif_sw_resources_set(unit, lif, -1, _BCM_DPP_GPORT_SW_RESOURCES_INGRESS, &gport_sw_resources));
    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        if (!SOC_DPP_IS_VLAN_TRANSLATE_MODE_ADVANCED(unit)) { /* Standard vlan translate mode has a default ac for MIM, while AVT mode doesn't */
            ...
            if (SOC_IS_ARADPLUS_AND_BELOW(unit) || SOC_IS_PETRAB(unit)) {
                ...
                if (SOC_IS_PETRAB(unit)) {
                    rv = bcm_dpp_am_l2_ac_alloc(unit, 0, BCM_DPP_AM_FLAG_ALLOC_WITH_ID, &SOC_DPP_CONFIG(unit)->pp.mim_global_out_ac);
                } else {
    ...
    bcm_mim_vpn_t vpn)
{
    ...
    if (_BCM_DPP_VLAN_IS_BVID(vpn)) { /* B-VID */
        ...
        rv = _bcm_dpp_lif_sw_resources_delete(unit, isid_local_lif_id, -1, _BCM_DPP_GPORT_SW_RESOURCES_INGRESS);
        BCMDNX_IF_ERR_EXIT(rv); 
    ...
    bcm_mim_port_t * mim_port)
{
    ...
    if (mim_port->flags & BCM_MIM_PORT_ENCAP_OVERLAY) {
        ...
        if (!SOC_IS_JERICHO_B0_AND_ABOVE(unit)) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Flag: BCM_MIM_PORT_ENCAP_OVERLAY, MIM LL overlay is only available starting from Jericho_B0")));
        }
        ...
        * extract outer PCP DEI from outer-vlan */
        ...
        /* Update SW DB */
        rv = _bcm_dpp_lif_to_sw_resources(unit, -1, local_out_lif_id, _BCM_DPP_GPORT_SW_RESOURCES_EGRESS, &gport_sw_resources);
        BCMDNX_IF_ERR_EXIT(rv);
        ...
        rv = _bcm_dpp_lif_sw_resources_set(unit, -1, local_out_lif_id, _BCM_DPP_GPORT_SW_RESOURCES_EGRESS, &gport_sw_resources);
        BCMDNX_IF_ERR_EXIT(rv);
    } else {
        if (update) {
            ...
            BCMDNX_IF_ERR_EXIT(rv);
            BCMDNX_IF_ERR_EXIT( _bcm_dpp_lif_sw_resources_lif_usage_get(unit, -1, local_out_lif_id, NULL, &usage) );
    ...
    bcm_mim_port_t * mim_port)
{
    ...
    BCMDNX_IF_ERR_EXIT(_bcm_dpp_lif_sw_resources_lif_usage_get(soc_sand_dev_id, -1, local_outlif, NULL, &lif_usage));
    ...
    bcm_gport_t mim_port_id)
{
    ...
    BCMDNX_IF_ERR_EXIT(_bcm_dpp_lif_sw_resources_lif_usage_get(unit, -1, local_outlif_id, NULL, &lif_usage));
    if ((lif_usage != _bcmDppLifTypeMim) && (lif_usage != _bcmDppLifTypeOverlayLinkLayer)) {
    ...
    rv = _bcm_dpp_lif_sw_resources_delete(unit, -1, local_outlif_id, _BCM_DPP_GPORT_SW_RESOURCES_EGRESS);
    BCMDNX_IF_ERR_EXIT(rv); 
