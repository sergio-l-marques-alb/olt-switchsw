/*
 * $Id: port.c,v 1.200 Broadcom SDK $
...
int
_bcm_fe2000_port_egr_remark_idx_get(int unit, bcm_port_t port, uint32 *idx)
{
...
_bcm_fe2000_port_interface_init(int unit, int enable)
{
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_PORT,
                  (BSL_META_U(unit,
                              "bcm_port_init: Failed port probe unit=%d rv=%d(%s)\n"),
    ...
    LOG_VERBOSE(BSL_LS_BCM_PORT,
                (BSL_META_U(unit,
                            "bcm_port_init: Probed ports okay: %s of %s\n"),
                                 ...
                                 bcm_port_encap_config_t *config)
{
    ...
    LOG_VERBOSE(BSL_LS_BCM_PORT,
                (BSL_META_U(unit,
                            "bcm_fe2000_port_encap_config_get: unit=%d port=%d rv=%d\n"),
                 unit, gport, rv));
                                 ...
                                 const bcm_port_encap_config_t *config)
{
    ...
    LOG_VERBOSE(BSL_LS_BCM_PORT,
                (BSL_META_U(unit,
                            "bcm_fe2000_port_encap_config_set: unit=%d port=%d rv=%d\n"),
                 unit, gport, rv));
...
int 
bcm_fe2000_port_interface_config_get(
    int unit, 
    bcm_port_t port, 
...
int 
bcm_fe2000_port_interface_config_set(
    int unit, 
    bcm_port_t port, 
    bcm_port_interface_config_t *config)
{
...
_bcm_fe2000_port_link_get(int unit, bcm_port_t port, int hw, int *up)
{
    ...
    LOG_VERBOSE(BSL_LS_BCM_PORT,
                (BSL_META_U(unit,
                            "_bcm_port_link_get: unit=%d port=%d up=%d rv=%d\n"),
                 unit, port, *up, rv));
#endif 
...
bcm_fe2000_port_master_set(int unit, bcm_port_t port, int ms)
{
    ...
    LOG_VERBOSE(BSL_LS_BCM_PORT,
                (BSL_META_U(unit,
                            "bcm_port_master_set: unit=%d port=%d ms=%d rv=%d\n"),
                 unit, port, ms, rv));
                                      ...
                                      _bcm_fe2k_nvid_action_t action) 
{
    ...
    if (SOC_IS_SBX_G2P3(unit)) {
        ...
        if (BCM_FAILURE(result)) {
            result = _bcm_fe2000_mpls_port_info_get(unit, port, vid, 
                                                    pv2e.vlan, pv2e.vpws,
                                                    &keepUntaggedFlags);
                            ...
                            bcm_port_control_t type, int *value)
{
    ...
    LOG_VERBOSE(BSL_LS_BCM_PORT,
                (BSL_META_U(unit,
                            "bcm_fe2000_port_control_get: unit=%d, type=%d\n"),
                 unit, type));
    ...
    if (BCM_GPORT_IS_SET(port)) {
	    ...
	    case bcmPortControlFrameSpacingStretch:
	        if (INTERFACE_MAC(unit, port)) {
		  status = MAC_CONTROL_GET(PORT(unit, port).p_mac, unit, port,
		...
		break;
            ...
            default:
                LOG_VERBOSE(BSL_LS_BCM_PORT,
                            (BSL_META_U(unit,
                                        "bcm_fe2000_port_control_get: unit=%d, unsuppd type=%d\n"),
                             unit, type));
                status = BCM_E_UNAVAIL;
            }
        }
                            ...
                            bcm_port_control_t type, int value)
{
    ...
    LOG_VERBOSE(BSL_LS_BCM_PORT,
                (BSL_META_U(unit,
                            "bcm_fe2000_port_control_set: unit=%d, type=%d, value=0x%x\n"),
                 unit, type, value));
	...
	case bcmPortControlFrameSpacingStretch:
	    if (INTERFACE_MAC(unit, port)) {
	      status = (MAC_CONTROL_SET(PORT(unit, port).p_mac, unit, port,
        ...
        default:
            LOG_VERBOSE(BSL_LS_BCM_PORT,
                        (BSL_META_U(unit,
                                    "bcm_fe2000_port_control_set: unit=%d, unsuppd type=%d\n"),
                         unit, type));
            status = BCM_E_UNAVAIL;
        }
    }
...
bcm_fe2000_port_discard_set(int unit, bcm_port_t port, int mode)
{
    ...
    if (BCM_E_NONE == result) {
        ...
        if (BCM_E_NONE != result) {
            LOG_ERROR(BSL_LS_BCM_PORT,
                      (BSL_META_U(unit,
                                  "%s[%d]: unable to get port %d untagged VLAN: %d (%s)\n"),
                       FUNCTION_NAME(),
                       unit,
                       port,
                       result,
    ...
    if (BCM_E_NONE == result) {
        ...
        if (BCM_E_NONE != result) {
            LOG_ERROR(BSL_LS_BCM_PORT,
                      (BSL_META_U(unit,
                                  "%s[%d]: unable to set port %d tagged ingress drop:"
                                   " %d (%s)\n"),
                       FUNCTION_NAME(),
                       unit,
                       port,
                                ...
                                uint32 *kbits_sec, uint32 *kbits_burst)
{
    ...
    uint32 qid, connection;
    uint32 hpp_to_mac_thresh, spi_to_hpp_min_pages;
    uint32 hppFreq, epoch, contexts;
                                ...
                                uint32 kbits_sec, uint32 kbits_burst)
{
    ...
    uint32 qid, conn_idx;
    uint32 hppFreq, epoch, contexts, frame_max, frame_size;
    bcm_module_t modid;
    ...
    if (connection->bEgress == 0) {
        LOG_ERROR(BSL_LS_BCM_PORT,
                  (BSL_META_U(unit,
                              "%s: cannot set egress rates on non-egress paths\n"), 
                   FUNCTION_NAME()));
        return BCM_E_CONFIG;
    }
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_PORT,
                  (BSL_META_U(unit,
                              "%s: failed to convert port %d to a fabric port: %s\n"), 
