                                              ...
                                              uint32                    fte_idx)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_L3,
              (BSL_META(_SBX_D(l3_fe->fe_unit, "Enter (Fte 0x%x)\n")), fte_idx));
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_L3,
                  (BSL_META(_SBX_D(l3_fe->fe_unit, "error(%s) could not invalidate FTE 0x%x\n")),
    ...
    if (BCM_SUCCESS(rv)) {
        if (fte_hash_elem->ref_count > 1) {
            LOG_ERROR(BSL_LS_BCM_L3,
                      (BSL_META(_SBX_D(l3_fe->fe_unit, "fte 0x%x in use")), 
        ...
        LOG_VERBOSE(BSL_LS_BCM_L3,
                    (BSL_META(_SBX_D(l3_fe->fe_unit, "Freed fteHash for fteIdx 0x%x\n")), 
                     fte_idx));
    } else {
        LOG_ERROR(BSL_LS_BCM_L3,
                  (BSL_META(_SBX_D(l3_fe->fe_unit, "fte_idx 0x%x not found in hash table\n")), 
                   fte_idx));
    }
    ...
    if ((fte_idx >= SBX_LOCAL_GPORT_FTE_BASE(l3_fe->fe_unit)) &&
        ...
        res_type = SBX_GU2K_USR_RES_FTE_VPXS_PW_FO;
                                       ...
                                       uint32                  esmacIdx)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_L3,
              (BSL_META(_SBX_D(l3_fe->fe_unit, "Enter (ingrSmacIdx %d egrSmacIdx %d  "
               "%x:%x:%x:%x:%x:%x)\n")),
               ismacIdx, esmacIdx,
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_L3,
                  (BSL_META(_SBX_D(l3_fe->fe_unit, "Failed to set lsm idx=%d: %d %s\n")),
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_L3,
                  (BSL_META(_SBX_D(l3_fe->fe_unit, "Failed to set esmac idx=%d: %d %s\n")),
                            ...
                            soc_sbx_g2p3_etel2_t      *l2_hw_ete)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_L3,
              (BSL_META(_SBX_D(l3_fe->fe_unit, "Enter vidop %d, smac_idx %d vid %d mtu %d)\n")),
...

{
    ...
    LOG_DEBUG(BSL_LS_BCM_L3,
              (BSL_META(_SBX_D(l3_fe->fe_unit, "Enter : L2 ETE Index 0x%x\n")), ete_addr));

    status = soc_sbx_g2p3_etel2_set(l3_fe->fe_unit, ete_addr, l2_hw_ete);

    if (status != BCM_E_NONE) {
        LOG_ERROR(BSL_LS_BCM_L3,
                  (BSL_META(_SBX_D(l3_fe->fe_unit, "error %s in setting etel2[0x%x] in HW\n")),
                          ...
                          uint32                     change_flags)
{
    ...
    if (status != BCM_E_NONE) {
        LOG_ERROR(BSL_LS_BCM_L3,
                  (BSL_META(_SBX_D(l3_fe->fe_unit, "error %s in getting L2 ete (%d) in HW")),
                   bcm_errmsg(status), ete_addr));
    } else {
        ...
        if (status != BCM_E_NONE) {
            LOG_ERROR(BSL_LS_BCM_L3,
                      (BSL_META(_SBX_D(l3_fe->fe_unit, "error %s in setting L2 ete (%d) in HW")),
                       bcm_errmsg(status), ete_addr));
        }
    }
    return status;
                            ...
                            soc_sbx_g2p3_oi2e_t       *oi2e)
{
    ...
    LOG_VERBOSE(BSL_LS_BCM_L3,
                (BSL_META(_SBX_D(l3_fe->fe_unit, "set L3 ete(0x%x) ohi(0x%x) in hw\n")),
    ...
    if (status == BCM_E_NONE) {
        ...
        if (status != BCM_E_NONE) {
            LOG_ERROR(BSL_LS_BCM_L3,
                      (BSL_META(_SBX_D(l3_fe->fe_unit, "Could not set outhdrIndex2ete (%d)\n")),
                       l3_sw_ete->l3_ohi.ohi));
        }
        ...
        LOG_ERROR(BSL_LS_BCM_L3,
                  (BSL_META(_SBX_D(l3_fe->fe_unit, "Could not set l3 ete (%d)")),
                   l3_sw_ete->l3_ete_hw_idx.ete_idx));
    }
    return status;
}
                                 ...
                                 _fe2k_l3_ete_t            *l3_sw_ete)
{
    ...
    if (BCM_FAILURE(status)) {
        LOG_ERROR(BSL_LS_BCM_L3,
                  (BSL_META(_SBX_D(l3_fe->fe_unit, "Could not get Ipv4 ETE (%d)\n")), 
                   l3_sw_ete->l3_ete_hw_idx.ete_idx));
        return status;
    }
                                      ...
                                      bcm_l3_egress_t        *bcm_egr)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_L3,
              (BSL_META(_SBX_D(l3_fe->fe_unit, "Enter - encap-id(0x%x)\n")),
               bcm_egr->encap_id));
    ...
    if (!(bcm_egr->flags & BCM_L3_TGID)) {
        ...
        LOG_VERBOSE(BSL_LS_BCM_L3,
                    (BSL_META(_SBX_D(l3_fe->fe_unit, "soc_sbx_node_port_get mod(0x%x) port(%d)"
                     "-> node(0x%x) port (%d)\n")),
    ...
    if (status == BCM_E_NONE) {
        ...
        if (bcm_egr->flags & BCM_L3_TGID) {
            ...
            LOG_VERBOSE(BSL_LS_BCM_L3,
                        (BSL_META(_SBX_D(l3_fe->fe_unit, "LagEnable trunk=%d \n")), bcm_egr->trunk));
        } else {
	    fte.qid = SOC_SBX_NODE_PORT_TO_QID(l3_fe->fe_unit,
            ...
            LOG_VERBOSE(BSL_LS_BCM_L3,
                        (BSL_META(_SBX_D(l3_fe->fe_unit, "Non-Lag module %d "
        ...
        if (status != BCM_E_NONE) {
           LOG_ERROR(BSL_LS_BCM_L3,
                     (BSL_META(_SBX_D(l3_fe->fe_unit, "error %s in programming FTE 0x%x in HW\n")),
                       ...
                       bcm_l3_egress_t        *bcm_egr)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_L3,
              (BSL_META(_SBX_D(l3_fe->fe_unit, "Enter (Fte Index 0x%x)\n")), fte_idx));
    ...
    if (status != BCM_E_NONE) {
        LOG_ERROR(BSL_LS_BCM_L3,
                  (BSL_META(_SBX_D(l3_fe->fe_unit, "Could not get FTE (%d) from HW\n")), fte_idx));
    } else {
        ...
        if (fte.lag) {
            ...
            status = bcm_fe2000_trunk_get_old(l3_fe->fe_unit, bcm_egr->trunk, &trunk_info);

            if (status != BCM_E_NONE) {
                LOG_ERROR(BSL_LS_BCM_L3,
                          (BSL_META(_SBX_D(l3_fe->fe_unit, "Could not retreive trunk info for trunkId %d\n")),
                           bcm_egr->trunk));
                return status;
            }
            ...
            if (status != BCM_E_NONE) {
                LOG_ERROR(BSL_LS_BCM_L3,
                          (BSL_META(_SBX_D(l3_fe->fe_unit, "error(%s) Mapping "
                                              ...
                                              bcm_l3_egress_t        *bcm_egr)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_L3,
              (BSL_META(_SBX_D(l3_fe->fe_unit, "encap-id(0x%x)\n")), bcm_egr->encap_id));
    ...
    if (status != BCM_E_NONE) {
        LOG_ERROR(BSL_LS_BCM_L3,
                  (BSL_META(_SBX_D(l3_fe->fe_unit, "error(%s) could not get ete from encap-id(0x%x)\n")),
    ...
    if (status != BCM_E_NONE) {
        LOG_ERROR(BSL_LS_BCM_L3,
                  (BSL_META(_SBX_D(l3_fe->fe_unit, "error(%s) could not get interface for (0x%x)\n")),
    ...
    if ((l3_sw_ete->l3_ete_key.l3_ete_hk.type == _FE2K_L3_ETE__UCAST_IP) ||
        ...
        if (status != BCM_E_NONE) {
             LOG_ERROR(BSL_LS_BCM_L3,
                       (BSL_META(_SBX_D(l3_fe->fe_unit, "Could not get Ipv4 Ete (0x%x) from HW\n")),
                        l3_sw_ete->l3_ete_hw_idx.ete_idx));
            return status;
        }
        ...
        if (status != BCM_E_NONE) {
            LOG_ERROR(BSL_LS_BCM_L3,
                      (BSL_META(_SBX_D(l3_fe->fe_unit, "Could not get OutHeaderIndex (0x%x) "
        ...
        LOG_DEBUG(BSL_LS_BCM_L3,
                  (BSL_META(_SBX_D(l3_fe->fe_unit, "interfaceId= 0x%x V4-ete addr(SW)= 0x%x\n"
        ...
        if (l3_intf->if_flags & _FE2K_L3_MPLS_TUNNEL_SET) {
             ...
             */
             ...
             if (status != BCM_E_NONE) {
                 LOG_ERROR(BSL_LS_BCM_L3,
                           (BSL_META(_SBX_D(l3_fe->fe_unit, "Could not get mpls ete (0x%x) from HW\n")),
        ...
        LOG_ERROR(BSL_LS_BCM_L3,
                  (BSL_META(_SBX_D(l3_fe->fe_unit, "Internal Error\n"))));
...

{
    ...
    if (info->l3a_flags & BCM_L3_IP6) {
        LOG_DEBUG(BSL_LS_BCM_L3,
                  (BSL_META(_SBX_D(l3_fe->fe_unit, "Enter : V6 addr: " IPV6_FMT 
                   "\nprefix 0x%x (op %d)\n")),
        ...
        LOG_DEBUG(BSL_LS_BCM_L3,
                  (BSL_META(_SBX_D(l3_fe->fe_unit, "Enter : V4 addr 0x%08x len 0x%02x "
                   " ipctxt 0x%02x (op %d)\n")),
    ...
    case _fe2k_route_op__da_get:
        if (daPayload == NULL) {
            LOG_ERROR(BSL_LS_BCM_L3,
                      (BSL_META(_SBX_D(l3_fe->fe_unit, "NULL da-Payload\n"))));
            return BCM_E_PARAM;
        }
        break;
    ...
    case _fe2k_route_op__sa_get:
        if (saPayload == NULL) {
            LOG_ERROR(BSL_LS_BCM_L3,
                      (BSL_META(_SBX_D(l3_fe->fe_unit, "NULL sa-Payload\n"))));
            return BCM_E_PARAM;
        }
        break;
    }
        ...
        (op == _fe2k_route_op__da_mod)) 
    {
        ...
        if ((op == _fe2k_route_op__da_del) &&
            ...
            LOG_ERROR(BSL_LS_BCM_L3,
                      (BSL_META(_SBX_D(l3_fe->fe_unit, "Invalid ref_count %d on fte_idx 0x%x \n")),
    ...
    if (info->l3a_flags & BCM_L3_IP6) {
        ...
        case _fe2k_route_op__da_add:
            LOG_VERBOSE(BSL_LS_BCM_L3,
                        (BSL_META(_SBX_D(l3_fe->fe_unit, "IPv6 DA ADD: FT Index 0x%x, ecmpBits %d\n")),
        ...
        case _fe2k_route_op__da_del:
           LOG_VERBOSE(BSL_LS_BCM_L3,
                       (BSL_META(_SBX_D(l3_fe->fe_unit, "IPv6 DA DEL\n"))));
        ...
        case _fe2k_route_op__da_mod:
            LOG_VERBOSE(BSL_LS_BCM_L3,
                        (BSL_META(_SBX_D(l3_fe->fe_unit, "IPv6 DA UPDATE FT Index 0x%x, ecmpBits %d\n")),
                         daPayload->v6.ftidx, daPayload->v6.ecmpmask));
       ...
       case _fe2k_route_op__sa_add:
           LOG_VERBOSE(BSL_LS_BCM_L3,
                       (BSL_META(_SBX_D(l3_fe->fe_unit, "IPv6 SA ADD: PoE 0x%x, MODE %d Src Drop %d\n")),
        ...
        case _fe2k_route_op__sa_del:
            LOG_VERBOSE(BSL_LS_BCM_L3,
                        (BSL_META(_SBX_D(l3_fe->fe_unit, "IPv6 SA DEL\n"))));
       ...
       case _fe2k_route_op__sa_mod:
        ...
        case _fe2k_route_op__da_add:
            LOG_VERBOSE(BSL_LS_BCM_L3,
                        (BSL_META(_SBX_D(l3_fe->fe_unit, "IPv4 DA ADD: FT Index 0x%x, ecmpBits %d vid %d\n")),
        ...
        case _fe2k_route_op__da_del:
           LOG_VERBOSE(BSL_LS_BCM_L3,
                       (BSL_META(_SBX_D(l3_fe->fe_unit, "IPv4 DA DEL\n"))));
       ...
       case _fe2k_route_op__sa_add:
           LOG_VERBOSE(BSL_LS_BCM_L3,
                       (BSL_META(_SBX_D(l3_fe->fe_unit, "IPv4 SA ADD: PoE 0x%x, MODE %d Src Drop %d\n")),
        ...
        case _fe2k_route_op__sa_del:
            LOG_VERBOSE(BSL_LS_BCM_L3,
                        (BSL_META(_SBX_D(l3_fe->fe_unit, "IPv4 SA DEL\n"))));
    ...
    if (BCM_FAILURE(status)) {
        LOG_ERROR(BSL_LS_BCM_L3,
                  (BSL_META(_SBX_D(l3_fe->fe_unit, "Error in updating route in HW: %s\n")), 
                   bcm_errmsg(status)));
    }
                              ...
                              _soc_sbx_g2p3_route_commit_f v6_host_commit)
{
        ...
        if (commitV6) {
            if (v6_commit) {
                ...
                if (BCM_FAILURE(rv)) {
                   LOG_ERROR(BSL_LS_BCM_L3,
                             (BSL_META_U(unit,
                                         "v6 LPM route commit failed: %d %s\n"),
                              rv, bcm_errmsg(rv)));
                    /* keep trying */
                }
            } 
    ...
    if ( commitV4 && v4_commit) {
        ...
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_L3,
                      (BSL_META_U(unit,
                                  "v4 LPM route commit failed: %d %s\n"),
                       rv, bcm_errmsg(rv)));
        }
    }
                                 ...
                                 _fe2k_l3_route_op_t      da_op)
{
    ...
    if (info->l3a_flags & BCM_L3_RPF) {
        ...
        if (status == BCM_E_NONE) {
            ...
            LOG_ERROR(BSL_LS_BCM_L3,
                      (BSL_META(_SBX_D(l3_fe->fe_unit, "RPF error in pv2e[%d,0x%x]: %s\n")),
    ...
    if (status != BCM_E_NONE) {
        LOG_ERROR(BSL_LS_BCM_L3,
                  (BSL_META(_SBX_D(l3_fe->fe_unit, "error in ip route map: %s\n")), bcm_errmsg(status)));
        return status;
    }
    ...
    if (info->l3a_flags & BCM_L3_RPF) {
        ...
        if (BCM_FAILURE(status)) {
             LOG_ERROR(BSL_LS_BCM_L3,
                       (BSL_META(_SBX_D(l3_fe->fe_unit, "error in SA route set: %s\n")), 
                        bcm_errmsg(status)));
             return status;
        }
        if (_bcm_fe2000_l3_do_lpm_commit(l3_fe->fe_unit, info)) {
            ...
            if (status != BCM_E_NONE) {
                LOG_ERROR(BSL_LS_BCM_L3,
                          (BSL_META(_SBX_D(l3_fe->fe_unit, "error in SA route commit: %s\n")), 
                           bcm_errmsg(status)));
            }
        }
        /* sa commit failure ? */
       if (BCM_FAILURE(status)) {
            if (sa_op == _fe2k_route_op__sa_add) {
                ...
                if (BCM_FAILURE(tmp)) {
                    LOG_ERROR(BSL_LS_BCM_L3,
                              (BSL_META(_SBX_D(l3_fe->fe_unit, "Error %s in undoing SA route changes\n")),
                               bcm_errmsg(tmp)));
                }
            }
        }
    ...
    if (status != BCM_E_NONE) {
        LOG_ERROR(BSL_LS_BCM_L3,
                  (BSL_META(_SBX_D(l3_fe->fe_unit, "error in DA route set: %s\n")), bcm_errmsg(status)));
        return status;
    }
    ...
    if (_bcm_fe2000_l3_do_lpm_commit(l3_fe->fe_unit, info)) {
        ...
        if(status != BCM_E_NONE) {
            LOG_ERROR(BSL_LS_BCM_L3,
                      (BSL_META(_SBX_D(l3_fe->fe_unit, "error in DA route commit: %s\n")), 
                       bcm_errmsg(status)));
        }
    }
    if (BCM_FAILURE(status)) {
        if (da_op == _fe2k_route_op__da_add) {
            ...
            if (BCM_FAILURE(tmp)) {
                LOG_ERROR(BSL_LS_BCM_L3,
                          (BSL_META(_SBX_D(l3_fe->fe_unit, "Error %s in undoing DA route changes\n")),
                           bcm_errmsg(tmp)));
            }
        }
        return status;
                              ...
                              uint32                   ecmp_size)
{
    ...
    if (info->l3a_flags & BCM_L3_REPLACE) {
        ...
        if(status != BCM_E_NONE) {
            LOG_ERROR(BSL_LS_BCM_L3,
                      (BSL_META(_SBX_D(l3_fe->fe_unit, "Could not find route to update \n"))));
            return status;
        }        
        ...
        if(status != BCM_E_NONE) {
            LOG_ERROR(BSL_LS_BCM_L3,
                      (BSL_META(_SBX_D(l3_fe->fe_unit, "Could not find FTE hash element\n"))));
            return status;
        }
        ...
        if (BCM_SUCCESS(status)) { 
            LOG_ERROR(BSL_LS_BCM_L3,
                      (BSL_META(_SBX_D(l3_fe->fe_unit, "DA found on unit\n"))));
            return BCM_E_EXISTS;
        }
        if ((info->l3a_flags & BCM_L3_RPF) && BCM_SUCCESS(sa_status)) {
            LOG_ERROR(BSL_LS_BCM_L3,
                      (BSL_META(_SBX_D(l3_fe->fe_unit, "SA found on unit\n"))));
            return BCM_E_EXISTS;
        }
                                 ...
                                 bcm_l3_route_t *info)
{
    ...
    LOG_VERBOSE(BSL_LS_BCM_L3,
                (BSL_META(_SBX_D(l3_fe->fe_unit, "Enter\n"))));
    ...
    if (BCM_FAILURE(status)) {
        LOG_ERROR(BSL_LS_BCM_L3,
                  (BSL_META(_SBX_D(l3_fe->fe_unit, "error in DA route get for delete: %s\n")), 
                   bcm_errmsg(status)));
        return status;
    }
    ...
    if (info->l3a_flags & BCM_L3_RPF) {
        ...
        if (BCM_FAILURE(sa_status)) {
            LOG_ERROR(BSL_LS_BCM_L3,
                      (BSL_META(_SBX_D(l3_fe->fe_unit, "error in SA route get for delete: %s\n")),
                       bcm_errmsg(status)));
            return sa_status;
        }
    }
    ...
    if (BCM_FAILURE(status)) {
        LOG_ERROR(BSL_LS_BCM_L3,
                  (BSL_META(_SBX_D(l3_fe->fe_unit, "error in DA route delete: %s\n")),
                   bcm_errmsg(status)));
    } else {
        LOG_VERBOSE(BSL_LS_BCM_L3,
                    (BSL_META(_SBX_D(l3_fe->fe_unit, "Successfully removed Ipv4 DA\n"))));
        ...
        if (_bcm_fe2000_l3_do_lpm_commit(l3_fe->fe_unit, info) ) {
            ...
            if (BCM_FAILURE(status)) {
                LOG_ERROR(BSL_LS_BCM_L3,
                          (BSL_META(_SBX_D(l3_fe->fe_unit, "Error %s in DA Route Commit \n")), 
                           bcm_errmsg(status)));
                return status;
            } else {
                LOG_VERBOSE(BSL_LS_BCM_L3,
                            (BSL_META(_SBX_D(l3_fe->fe_unit, "Successfully Committed deleted Ipv4 routes\n"))));
            }
         }
         ...
         if (info->l3a_flags & BCM_L3_RPF) {
            ...
            if (BCM_FAILURE(status)) {
                    LOG_ERROR(BSL_LS_BCM_L3,
                              (BSL_META(_SBX_D(l3_fe->fe_unit, "error in SA route delete: %s\n")),
                               bcm_errmsg(status)));
            } else {
                if (_bcm_fe2000_l3_do_lpm_commit(l3_fe->fe_unit, info) ) {
                    ...
                    if (BCM_FAILURE(status)) {
                            LOG_ERROR(BSL_LS_BCM_L3,
                                      (BSL_META(_SBX_D(l3_fe->fe_unit, "Error %s in SA Route Commit \n")),
                                       bcm_errmsg(status)));
                    } else {
                            LOG_VERBOSE(BSL_LS_BCM_L3,
                                        (BSL_META(_SBX_D(l3_fe->fe_unit, "Successfully Committed "
