/*
 * $Id: recovery.c,v 1.9 Broadcom SDK $
...
fe2k_rcvr_default_stg_read_f(int unit, uint32 *val)
{
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_ATTACH,
                  (BSL_META_U(unit,
                              "failed to get flood ft[0x%04x]:%s\n"), 
                   fti, bcm_errmsg(rv)));
        return rv;   
    }
...
fe2k_rcvr_default_stg_write_f(int unit, uint32 val)
{
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_ATTACH,
                  (BSL_META_U(unit,
                              "failed to set flood ft[0x%04x]:%s\n"), 
                   fti, bcm_errmsg(rv)));
        return rv;   
    }
...
rc_non_tb_free(dq_t *elt, int unit)
{
    ...
    LOG_WARN(BSL_LS_BCM_ATTACH,
             (BSL_META_U(unit,
                         "Unprocessed non-tb vsi on pv2e[%d, 0x%03x]; vsi=0x%04x\n"),
...
fe2k_rcvr_fast_mems_get(int unit) 
{
    ...
    fast_pv2e->count = BCM_VLAN_COUNT * SBX_MAX_PORTS;

    fast_pv2e->lpi = sal_alloc(sizeof(uint32) * fast_pv2e->count,
                               "fast_lps");
    ...
    fast_pv2e->vlan = sal_alloc(sizeof(uint32) * fast_pv2e->count,
                                "fast_vlans");
...
#include <bcm_int/sbx/fe2000/recovery_vars.h>
    ...
    for (idx=0; idx < fast_pv2e->count; idx++) {
        if (fast_pv2e->lpi[idx] < SBX_MAX_PORTS) {
            continue;
        }

        LOG_VERBOSE(BSL_LS_BCM_ATTACH,
                    (BSL_META_U(unit,
                                "Found lpi 0x%x at idx %d\n"), 
        ...
        if (BCM_FAILURE(rv)) {
           LOG_ERROR(BSL_LS_BCM_ATTACH,
                     (BSL_META_U(unit,
                                 "Failed to allocate LP[0x%04x]:%s\n"), 
...
_bcm_fe2000_recovery_non_tb_vsi_insert(int unit, uint16 port, uint16 vid)
{
    ...
    LOG_VERBOSE(BSL_LS_BCM_ATTACH,
                (BSL_META_U(unit,
                            "Recording non-tb VSI at %d/0x%03x:  0x%04x\n"),
                 port, vid, 
