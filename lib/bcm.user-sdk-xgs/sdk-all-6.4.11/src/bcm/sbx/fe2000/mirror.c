/*
 * $Id: mirror.c,v 1.62 Broadcom SDK $
...
#define MIRROR_TABLE_LOCK_TAKE \
    /* claim the lock */ \
    LOG_DEBUG(BSL_LS_BCM_MIRROR, \
              (BSL_META_U(unit, "claim mirror table lock\n"))); \
                               ...
                               uint32 *destQueue)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_MIRROR,
              (BSL_META_U(unit, "get destination information\n")));
    ...
    if (BCM_E_NONE != result) {
        ...
        LOG_DEBUG(BSL_LS_BCM_MIRROR,
              (BSL_META_U(unit, "fabUnit %d, fabNode %d, fabPort %d\n"),
                           ...
                           const bcm_gport_t destGport)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_MIRROR,
              (BSL_META_U(unit, "initialise the mirror data\n")));

    /* handle appropriate mirror target type */
    /* mirror type is port */
    LOG_DEBUG(BSL_LS_BCM_MIRROR,
              (BSL_META_U(unit, "mirror is destined to mod %d port %d\n"),
                  destModId, destPort));
                            ...
                            const bcm_gport_t destGport)
{
    ...
    if (BCM_E_NONE == result) {
        LOG_DEBUG(BSL_LS_BCM_MIRROR,
              (BSL_META_U(unit, "using modPort FT entry %08X\n"),
                      portFte));
    ...
    if (SOC_E_NONE == result) {
        ...
        LOG_DEBUG(BSL_LS_BCM_MIRROR,
              (BSL_META_U(unit, "configure the mirror -> OI[0x%08X], QID[0x%08X]\n"),
        ...
        if (BCM_FAILURE(result)) {
            ...
            LOG_ERROR(BSL_LS_BCM_MIRROR,
                      (BSL_META_U(unit,
                                  "soc_sbx_g2p3_mirror_set(%08X,%d,&{0x%08X})"
                                   " returned -> %d (%s)\n"),
                                  ...
                                  const bcm_gport_t destGport)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_MIRROR,
              (BSL_META_U(unit, "search for match or free entry\n")));
                                       ...
                                       bcm_gport_t destGport)
{
    ...
    if (BCM_FAILURE(result)) {
        ...
        /* mirror target is a port, need to find the FTE for it */
        LOG_DEBUG(BSL_LS_BCM_MIRROR,
              (BSL_META_U(unit, "mirror is destined to port %x\n"), destGport));
                         ...
                         const bcm_gport_t destGport)
{
    ...
    if (BCM_E_NONE == result) {
        ...
        LOG_DEBUG(BSL_LS_BCM_MIRROR,
              (BSL_META_U(unit, "claim and fill %s mirror %d\n"),
                        ...
                        const unsigned int mirrorId)
{
    ...
    if (0 == (mirror->flags & FE2K_MIRROR_VALID)) {
        ...
        LOG_DEBUG(BSL_LS_BCM_MIRROR,
              (BSL_META_U(unit, "decrement use count for %s "
                             "mirror ID %d\n"),
                      dir_str[direction], mirrorId));
        mirror->refCount--;
        if (0 == mirror->refCount) {
            ...
            LOG_DEBUG(BSL_LS_BCM_MIRROR,
              (BSL_META_U(unit, "%s mirror ID %d no longer"
                       ...
                       bcm_gport_t *destPort)
{
    ...
    if (mirror->flags & FE2K_MIRROR_VALID) {
        ...
        LOG_DEBUG(BSL_LS_BCM_MIRROR,
              (BSL_META_U(unit, "get %s mirror information\n"), 
                      dir_str[direction]));
	if (destPort) {
            *destPort = mirror->dest_gport;
        }
        LOG_DEBUG(BSL_LS_BCM_MIRROR,
              (BSL_META_U(unit, "dest gport = %d\n"), mirror->dest_gport));
        result = BCM_E_NONE;
    } else {
                                 ...
                                 unsigned int *mirrorId)
{
    ...
    if (BCM_SUCCESS(result)) {
        *mirrorId = lp.mirror;
        ...
        LOG_ERROR(BSL_LS_BCM_MIRROR,
                  (BSL_META_U(unit,
                              "soc_sbx_g2p3_lp_get(*,%d,*) returned"
                               " -> %d (%s)\n"),
                   port, result, _SHR_ERRMSG(result)));
    }

    LOG_DEBUG(BSL_LS_BCM_MIRROR,
              (BSL_META_U(unit, "ingress mirror %d\n"), *mirrorId));
                                 ...
                                 unsigned int mirrorId)
{
    ...
    if (BCM_SUCCESS(result)) {
        ...
        if (lpi) {
            ...
            result = soc_sbx_g2p3_lp_get(unit, lpi, &tempLp);
            if (BCM_SUCCESS(result)) {
                ...
                if (localOldValues.mirror == tempLp.mirror) {
                    ...
                    result = soc_sbx_g2p3_lp_set(unit, lpi, &tempLp);
                }
            }
            if (BCM_FAILURE(result)) {
                LOG_ERROR(BSL_LS_BCM_MIRROR,
    ...
    if (BCM_FAILURE(result)) {
        ...
        if (BCM_FAILURE(result)) {
            LOG_ERROR(BSL_LS_BCM_MIRROR,
                      (BSL_META_U(unit,
                                  "soc_sbx_g2p3_lp_set(*,%d,*) returned"
                                   " -> %d (%s)\n"),
                       port, result, _SHR_ERRMSG(result)));
            return result;
        }
    ...
    while (BCM_SUCCESS(rc)) {
        if (iterPort == port) {
            ...
            lpi = pvv2e.lpi;
            if (SOC_SBX_CFG(unit)->mplstp_ena && pvv2e.vpws) {
                lpi += _BCM_FE2000_VPWS_UNI_OFFSET;
            }
             ...
             */
            if (lpi != 0) {
                result = soc_sbx_g2p3_lp_get(unit, lpi, &lp);
                if (BCM_FAILURE(result)) {
                    LOG_ERROR(BSL_LS_BCM_MIRROR,
                              (BSL_META_U(unit,
                                          "soc_sbx_g2p3_lp_get(0x%x) returned"
                                           " -> %d (%s)\n"),
                ...
                result = soc_sbx_g2p3_lp_set(unit, lpi, &lp);
                if (BCM_FAILURE(result)) {
                    LOG_ERROR(BSL_LS_BCM_MIRROR,
                              (BSL_META_U(unit,
                                          "soc_sbx_g2p3_lp_set(0x%x) returned"
                                           " -> %d (%s)\n"),
                               lpi, result, _SHR_ERRMSG(result)));
                    return result;
                }
                                ...
                                unsigned int *mirrorId)
{
    ...
    *mirrorId = ep2e.mirroridx;
    ...
    LOG_DEBUG(BSL_LS_BCM_MIRROR,
              (BSL_META_U(unit, "egress mirror %08X\n"), *mirrorId));
...
bcm_fe2000_mirror_init(int unit)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_MIRROR,
              (BSL_META_U(unit, "check primary globals\n")));
    tempLock = NULL;
    ...
    if (!_mirror_glob_lock) {
        ...
        LOG_DEBUG(BSL_LS_BCM_MIRROR,
              (BSL_META_U(unit, "create temp primary lock\n")));
        ...
        LOG_DEBUG(BSL_LS_BCM_MIRROR,
              (BSL_META_U(unit, "take primary lock\n")));
        ...
        LOG_DEBUG(BSL_LS_BCM_MIRROR,
              (BSL_META_U(unit, "set primary lock to temp lock\n")));
        _mirror_glob_lock = tempLock;
        LOG_DEBUG(BSL_LS_BCM_MIRROR,
              (BSL_META_U(unit, "clear unit data pointers\n")));
        sal_memset(&_mirror_glob, 0x00, sizeof(_mirror_glob));
        LOG_DEBUG(BSL_LS_BCM_MIRROR,
              (BSL_META_U(unit, "release temp primary lock\n")));
        if (0 != sal_mutex_give(tempLock)) {
            LOG_DEBUG(BSL_LS_BCM_MIRROR,
              (BSL_META_U(unit, "unable to release"
                                 " temp primary lock\n")));
            return BCM_E_INTERNAL;
        }
    } /* if (!_mirror_glob_lock) */
    ...
    if (tempLock) {
        ...
        if (_mirror_glob_lock != tempLock) {
            LOG_DEBUG(BSL_LS_BCM_MIRROR,
              (BSL_META_U(unit, "init collision; dispose of"
                                 " temporary lock\n")));
            sal_mutex_destroy(tempLock);
        }
    }
    ...
    LOG_DEBUG(BSL_LS_BCM_MIRROR,
              (BSL_META_U(unit, "check unit globals\n")));
    if (_mirror_glob[unit]) {
  
        LOG_DEBUG(BSL_LS_BCM_MIRROR,
              (BSL_META_U(unit, "unit globals exist; clear them\n")));
    ...
    LOG_DEBUG(BSL_LS_BCM_MIRROR,
              (BSL_META_U(unit, "create new unit globals\n")));    
    ...
    LOG_DEBUG(BSL_LS_BCM_MIRROR,
              (BSL_META_U(unit, "create new unit lock\n")));
    ...
    if (tempLock) {
        ...
        LOG_DEBUG(BSL_LS_BCM_MIRROR,
              (BSL_META_U(unit, "clear unit information\n")));
        ...
        LOG_DEBUG(BSL_LS_BCM_MIRROR,
              (BSL_META_U(unit, "invalidate mirror targets\n")));
        for (direction=0; 
             direction < _fe2k_mirror_direction_max;
             direction++) {
            for (index = 0; 
        ...
        for (direction=0; 
             ...
             direction++) {
            LOG_DEBUG(BSL_LS_BCM_MIRROR,
              (BSL_META_U(unit, "reserve low %d %s mirrors\n"),
        ...
        LOG_DEBUG(BSL_LS_BCM_MIRROR,
              (BSL_META_U(unit, "reserve entry zero of both ingress"
                             " and egress mirror tables\n")));
        /* commit lock to working unit descriptor */
        LOG_DEBUG(BSL_LS_BCM_MIRROR,
              (BSL_META_U(unit, "create port mirror table lock\n")));
    ...
    if (BCM_E_NONE != result) {
        ...
        LOG_DEBUG(BSL_LS_BCM_MIRROR,
              (BSL_META_U(unit, "error initialising unit; cleaning up\n")));

        if (tempUnit->tableLock) {
            LOG_DEBUG(BSL_LS_BCM_MIRROR,
              (BSL_META_U(unit, "destroying table lock\n")));
            sal_mutex_destroy(tempUnit->tableLock);
        }

        LOG_DEBUG(BSL_LS_BCM_MIRROR,
              (BSL_META_U(unit, "destroying unit data space\n")));
    ...
    LOG_DEBUG(BSL_LS_BCM_MIRROR,
              (BSL_META_U(unit, "release primary lock\n")));
    if (0 != sal_mutex_give(_mirror_glob_lock)) {
        LOG_DEBUG(BSL_LS_BCM_MIRROR,
              (BSL_META_U(unit, "unable to release primary lock\n")));
        return BCM_E_INTERNAL;
    }
...
_bcm_fe2000_mirror_destination_free(int unit, bcm_gport_t mirror_dest_id)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_MIRROR,
              (BSL_META_U(unit, "free mirror destination (ingress table) %08X\n"),
                   mirror_idx));
...
_bcm_fe2000_mirror_destination_create(int unit, bcm_mirror_destination_t *mirror_dest)
{
	...
	if (!SOC_IS_SBX_FE2KXT(unit) && !(SOC_SBX_MPLSTP_ENABLE(unit))) { 
            return (BCM_E_UNAVAIL);
        }
    }
...
_bcm_fe2000_mirror_port_ingress_dest_add(int unit, bcm_port_t port, bcm_gport_t mirror_dest_id)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_MIRROR,
              (BSL_META_U(unit, "add ingress mirror\n")));
...
_bcm_fe2000_mirror_port_egress_dest_add(int unit, bcm_port_t port, bcm_gport_t mirror_dest_id)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_MIRROR,
              (BSL_META_U(unit, "add egress mirror\n")));
...
_bcm_fe2000_mirror_port_ingress_dest_delete(int unit, bcm_port_t port, bcm_gport_t mirror_dest_id)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_MIRROR,
              (BSL_META_U(unit, "delete ingress mirror\n")));
