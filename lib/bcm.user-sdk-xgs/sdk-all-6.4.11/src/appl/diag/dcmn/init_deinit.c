    ...
    /* KBP application init for warmboot */
        ...
        if (JER_KAPS_ENABLE(unit)){
            if(init_param->warmboot) {
                ...
                if (init_param->kaps_arm) {
                    rv = dpp_kaps_arm_image_load_default(unit);
                    if (BCM_FAILURE(rv)) {
                        LOG_ERROR(BSL_LS_APPL_SHELL,
                                  (BSL_META_U(unit,
                                              "dpp_kaps_arm_image_load_default () Failed:\n")));
                        goto exit ;
                    }
                }
            }
...
appl_dcmn_init_usage_parse(int unit, args_t *a, appl_dcmn_init_param_t *init_param)
{
    ...
    if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "kaps_arm_enable", 0)) {
        init_param->kaps_arm = 1;
    }
