/****************************************************************
 * $Id: diag_signals_dump.c,v 1.0 28/10/2014 18:28 idanu Exp $
 ...
 * based on the resolved_data struct from the C-model
 *******************************************************************/
typedef struct  {
        ...
        char *str)
{
        ...
        sal_strncat(curr_line, &hex_digit_res_chr ,1);
        sal_strncat(curr_line, "x", 1);
    ...
    )
{
    ...
    SOC_PPD_DIAG_RECEIVED_PACKET_INFO info;
    ...
    /* the max size for C-model is 384 nibbles*/
    PRINT_TO_TARGET(ofp,("\n%d-original_pkt       = 0x%.384s", line_index, updated_packet),
                    (ofp, "\n%d-original_pkt       = 0x%.384s", line_index, updated_packet))

    /*adding nibbles (to match 384 nibbles) for the C-model to work*/
