...
cmd_result_t cmd_diag_load_balancing_cmp(int unit, args_t* a);
...
/*     --------------------------------------------------------------------------------
    ...
    { "Signals"               , "SIG"    , cmd_diag_pp_signals_get                    },
    ...
    { "Fec_Performance_Test"  , "FECPRF", cmd_diag_fec_performance                    }
#endif
#if defined(BCM_88650_A0) && defined(INCLUDE_KBP) && !defined(BCM_88030)
																					   ,
    ...
    { "ECMP_load_balancing"   , "ELB",     cmd_diag_load_balancing_cmp                }
#endif /* BCM_JERICHO_SUPPORT */
        };
 ...
 ********************************************************************/
extern SOC_PPD_MTR_BW_PROFILE_INFO diag_pp_eth_policer_config;
int 
...
{   
  ...
  int
	eth_mtr_bkt_lvl_mem = MTRPS_EM_MCDA_DYNAMICm;
  int
  ...
  parse_table_t    
	...
	for (i=0;i<SOC_PPC_NOF_MTR_ETH_TYPES;i++) {
		...
		ret = soc_mem_read(unit, eth_mtr_bkt_lvl_mem, MTRPS_EM_BLOCK(unit, core_id), bckLvl_tbl_entry, (void*)eth_mtr_bck_lvl_above_64_val); 
		if (ret != BCM_E_NONE) {
			return CMD_FAIL;
		}
	...
	aggregate_policer_valid = soc_mem_field32_get(unit, eth_mtr_config_mem, &eth_mtr_config_val, GLOBAL_METER_PROFILE_VALIDf);
	aggregate_policer_id    = soc_mem_field32_get(unit, eth_mtr_config_mem, &eth_mtr_config_val, GLOBAL_METER_PROFILEf);
...
{   
  ...
  int
    bucket_lvl_arr[SOC_PPC_NOF_MTR_ETH_TYPES] = {-1};
  int
	eth_mtr_bkt_lvl_mem = MTRPS_EM_MCDB_DYNAMICm;
  uint32
	bckLvl_tbl_offset,
	bckLvl_tbl_entry,
	cbl_bucket_fld[4]= {CBL_0f, CBL_1f, CBL_2f, CBL_3f};
    ...
    cmd_ppd_api_diag_rif_info_get(int unit, args_t* a)
{
  ...
  for (core=first_core; core < last_core; core++) {
      ...
      /* get last rif */
      arad_pp_signal_mngr_signal_get(soc_sand_dev_id, core, SOC_DPP_SIGNAL_MNGR_SIGNAL_TYPE_RIF, val, &size_of_signal_in_bits);
...
#ifndef __KERNEL__
static int diag_get_in_mem(int unit, SOC_PPD_LIF_ENTRY_INFO *lif_entry_p, char **table_name)
{
    int mem = 0;
    char *table_type = NULL;
    if((SOC_INFO(unit).chip_type == SOC_INFO_CHIP_TYPE_JERICHO) ||
                                    (SOC_INFO(unit).chip_type == SOC_INFO_CHIP_TYPE_QMX)) {
        /* Look for table and present appropriately */
        switch(lif_entry_p->type) {
            ...
            case SOC_PPC_LIF_ENTRY_TYPE_MIDDLE_AC_IN_GROUP:
                switch(lif_entry_p->value.ac.service_type) {
                    ...
                    case SOC_PPC_L2_LIF_AC_SERVICE_TYPE_AC2PBB:
                        ...
                        break;
                    default:
                        cli_out("Illegal AC Service Type:%d", lif_entry_p->value.ac.service_type);
                        break;
                }
                break;
            case SOC_PPC_LIF_ENTRY_TYPE_PWE:
                if(lif_entry_p->value.pwe.service_type == SOC_PPC_L2_LIF_PWE_SERVICE_TYPE_MP) {
                    mem = PPDB_B_LIF_TABLE_LABEL_PWE_MPm;
                    table_type = "LABEL_PWE_MP";
                }
                else if(lif_entry_p->value.pwe.service_type == SOC_PPC_L2_LIF_PWE_SERVICE_TYPE_P2P) {
                    mem = PPDB_B_LIF_TABLE_LABEL_PWE_P2Pm;
                    table_type = "LABEL_PWE_P2P";
                }
                else
                    cli_out("Illegal PWE Service Type:%d", lif_entry_p->value.pwe.service_type);
                break;
            case SOC_PPC_LIF_ENTRY_TYPE_ISID:
                if(lif_entry_p->value.isid.service_type == SOC_PPC_L2_LIF_ISID_SERVICE_TYPE_MP) {
                    mem = PPDB_B_LIF_TABLE_ISID_MPm;
                    table_type = "ISID_MP";
                }
                else if(lif_entry_p->value.isid.service_type == SOC_PPC_L2_LIF_ISID_SERVICE_TYPE_P2P) {
                    mem = PPDB_B_LIF_TABLE_ISID_P2Pm;
                    table_type = "ISID_P2P";
                }
                else
                    cli_out("Illegal ISID Service Type:%d", lif_entry_p->value.isid.service_type);
                break;
            ...
            case SOC_PPC_LIF_ENTRY_TYPE_EXTENDER:
                switch(lif_entry_p->value.extender.service_type) {
                    case SOC_PPC_L2_LIF_AC_SERVICE_TYPE_MP:
                        mem = PPDB_B_LIF_TABLE_AC_MPm;
                        table_type = "AC_MP";
                        break;
                    ...
                    case SOC_PPC_L2_LIF_AC_SERVICE_TYPE_AC2PBB:
                    default:
                        cli_out("Illegal Extender Service Type:%d", lif_entry_p->value.extender.service_type);
                        break;
                }
                break;
    ...
    else if((SOC_INFO(unit).chip_type == SOC_INFO_CHIP_TYPE_ARADPLUS) ||
                                         (SOC_INFO(unit).chip_type == SOC_INFO_CHIP_TYPE_QAX)) {
        /* Look for table and present appropriately */
        switch(lif_entry_p->type) {
            case SOC_PPC_LIF_ENTRY_TYPE_EMPTY:
                mem = IHP_LIF_TABLEm;
            ...
            case SOC_PPC_LIF_ENTRY_TYPE_MIDDLE_AC_IN_GROUP:
                switch(lif_entry_p->value.ac.service_type) {
                    case SOC_PPC_L2_LIF_AC_SERVICE_TYPE_MP:
                        mem = IHP_LIF_TABLE_AC_MPm;
                        table_type = "AC_MP";
                        break;
                    case SOC_PPC_L2_LIF_AC_SERVICE_TYPE_AC2AC:
                        mem = IHP_LIF_TABLE_AC_2_OUT_LIFm;
                        table_type = "AC_2_OUT_LIF";
                        break;
                    case SOC_PPC_L2_LIF_AC_SERVICE_TYPE_AC2PWE:
                    case SOC_PPC_L2_LIF_AC_SERVICE_TYPE_AC2PBB:
                        mem = IHP_LIF_TABLE_AC_2_EEIm;
                        table_type = "AC_2_EEI";
                        break;
                    default:
                        cli_out("Illegal AC Service Type:%d", lif_entry_p->value.ac.service_type);
            ...
            case SOC_PPC_LIF_ENTRY_TYPE_PWE:
                if(lif_entry_p->value.pwe.service_type == SOC_PPC_L2_LIF_PWE_SERVICE_TYPE_MP) {
                    mem = IHP_LIF_TABLE_LABEL_PWE_MPm;
                    table_type = "LABEL_PWE_MP";
                }
                else if(lif_entry_p->value.pwe.service_type == SOC_PPC_L2_LIF_PWE_SERVICE_TYPE_P2P) {
                    mem = IHP_LIF_TABLE_LABEL_PWE_P2Pm;
                    table_type = "LABEL_PWE_P2P";
                }
                else
                    cli_out("Illegal PWE Service Type:%d", lif_entry_p->value.pwe.service_type);
            ...
            case SOC_PPC_LIF_ENTRY_TYPE_ISID:
                if(lif_entry_p->value.isid.service_type == SOC_PPC_L2_LIF_ISID_SERVICE_TYPE_MP) {
                    mem = IHP_LIF_TABLE_ISID_MPm;
                    table_type = "ISID_MP";
                }
                else if(lif_entry_p->value.isid.service_type == SOC_PPC_L2_LIF_ISID_SERVICE_TYPE_P2P) {
                    mem = IHP_LIF_TABLE_ISID_P2Pm;
                    table_type = "ISID_P2P";
                }
                else
                    cli_out("Illegal ISID Service Type:%d", lif_entry_p->value.isid.service_type);
                break;
            case SOC_PPC_LIF_ENTRY_TYPE_IP_TUNNEL_RIF:
                mem = IHP_LIF_TABLE_IP_TTm;
                table_type = "IP_TT";
                break;
            case SOC_PPC_LIF_ENTRY_TYPE_MPLS_TUNNEL_RIF:
                mem = IHP_LIF_TABLE_LABEL_PROTOCOL_OR_LSPm;
                table_type = "LABEL_PROTOCOL_OR_LSP";
                break;
            case SOC_PPC_LIF_ENTRY_TYPE_TRILL_NICK:
                mem = IHP_LIF_TABLE_TRILLm;
                table_type = "TRILL";
                break;
            case SOC_PPC_LIF_ENTRY_TYPE_EXTENDER:
                switch(lif_entry_p->value.extender.service_type) {
        ...
        if(!SOC_MEM_IS_VALID(unit, mem)) {
            cli_out("Memory:%d does not exist defaulting to IHP_LIF_TABLE\n", mem);
            mem = IHP_LIF_TABLEm;
        }
    }
    else {
        cli_out("Command not supported for device:%s\n", soc_dev_name(unit));
    }
...
static int diag_get_out_mem(int unit, ARAD_PP_EG_ENCAP_EEDB_ACCESS_TYPE access_type, char **table_name)
{
    int mem = 0;
    char *table_type = NULL;
    if((SOC_INFO(unit).chip_type == SOC_INFO_CHIP_TYPE_JERICHO) ||
    ...
    else if(SOC_INFO(unit).chip_type == SOC_INFO_CHIP_TYPE_QAX) {
        /* Look for table and present appropriately */
        /* Look for table and present appropriately */
        switch(access_type) {
            case ARAD_PP_EG_ENCAP_EEDB_ACCESS_TYPE_NONE:
            ...
            case ARAD_PP_EG_ENCAP_EEDB_ACCESS_TYPE_DATA_ARP_OVERLAY:
            case ARAD_PP_EG_ENCAP_EEDB_ACCESS_TYPE_ROO_LINK_LAYER:
                mem  = EDB_LINK_LAYER_OR_ARP_FORMATm;
                table_type = "LINK_LAYER_OR_ARP_FORMAT";
                break;
            case ARAD_PP_EG_ENCAP_EEDB_ACCESS_TYPE_IP:
                mem = EDB_IPV4_TUNNEL_FORMATm;
                table_type = "IPV4_TUNNEL_FORMAT";
                break;
            case ARAD_PP_EG_ENCAP_EEDB_ACCESS_TYPE_TRILL:
        ...
        if(!SOC_MEM_IS_VALID(unit, mem)) {
            cli_out("Memory:%d does not exist defaulting to EDB_EEDB_BANK\n", mem);
            mem = EDB_EEDB_BANKm;
        }
    }
    else if(SOC_INFO(unit).chip_type == SOC_INFO_CHIP_TYPE_ARADPLUS) {
    ...
    cmd_ppd_api_diag_lif_show(int unit, args_t* a)
{
    ...
    if(!sal_strncasecmp(lif_type, "in", strlen(lif_type))) {
        ...
        mem = diag_get_in_mem(unit, &lif_entry_info, &table_type);
    }
        ...
        mem = diag_get_out_mem(unit, access_type, &table_type);
    }
    else {
    ...
    if((mem <= 0) || (bank_id < 0) || (internal_offset < 0)) {
        cli_out("Bad entry parameters - mem:%d bank_id:%d internal_offset:%d\n", mem, bank_id, internal_offset);
        return CMD_FAIL;
    }
    ...
    switch(frwrd_type)
    {
        ...
        case SOC_PPC_DIAG_FWD_LKUP_TYPE_ILM:
...
#endif /* defined(INCLUDE_KBP) && !defined(BCM_88030_A0) */
            {
                ...
                LOG_CLI((BSL_META_U(unit, "    |         key         |---------------------------------------------------|\n\r")));
                LOG_CLI((BSL_META_U(unit, "    |                     |        { %4d, %2d, %d, %7d (0x%6x) }        |\n\r"),
                              key->inrif, key->in_local_port, key->mapped_exp, key->in_label, key->in_label));
            }
            break;
    ...
    cmd_ppd_api_diag_pp_last_fec(int unit, int core, cmd_diag_pp_packet_action_t * action_resolution, int print_details)
{
    ...
    if (SOC_IS_JERICHO(unit)) {
        ARAD_PP_DIAG_FLD_READ(&fld,core,ARAD_IHB_ID, 4, 0, 150, 132, 125);
    } else {
        ARAD_PP_DIAG_FLD_READ(&fld,core,ARAD_IHB_ID,12, 0 , 147, 130,1251);
    }
    dest = regs_val[0];
    ...
    cmd_ppd_api_diag_pp_mc_info_print(int unit, int core, uint32 mc_id)
{
    ...
    if ( ((entry[0]) >> bit_offset) & 0x1 ) { /* ingress multicast */
        ...
        while ((next_entry > 0) && (next_entry != 0x3FFFF) /* temporary WA */) {
        ...
        case ARAD_PP_EEI_TYPE_MPLS:
            {
                switch (info->eei.val.mpls_command.command) {
                    case ARAD_PP_MPLS_COMMAND_TYPE_PUSH:
                        ...
                        break;
                    case ARAD_PP_EEI_IDENTIFIER_SWAP_VAL:
                        LOG_CLI((BSL_META_U(unit, "    EEI MPLS swap, label %d\n\r"), 
                                            info->eei.val.mpls_command.label));
                        break;
                    case ARAD_PP_EEI_IDENTIFIER_POP_VAL:
        ...
        case SOC_PPC_FRWRD_DECISION_TYPE_TRAP:
            {
                ...
                if (_bcm_dpp_rx_get_converted_trap_sw_id(unit, fwd_dest, &trap_id_sw) == BCM_E_NONE) {
    ...
    cmd_ppd_api_diag_pp_snp_mrr_info_print(int unit, int core, uint32 cmd, uint32 is_snoop)
{
    ...
    res = soc_mem_read(unit, IRR_SNOOP_MIRROR_DEST_TABLEm, MEM_BLOCK_ANY, cmd + is_snoop * 16, data);
...
{   
	...
	parse_table_add(&pt, "print_db" 	 	, PQ_DFL|PQ_NO_EQ_OPT|PQ_BOOL, 0, &print_db, NULL);
	...
	if (0 >= nof_arg) {
	    cli_out(
	            ...
	            "\n\t\t[iterator]               - Dumps all KBP current entries in all tables"
	            ...
	            "\n\t\t[print_db]               - Print out the current configuration of tables and DB."
	...
	if (all_entries == TRUE ) {
		BCMDNX_IF_ERR_EXIT(arad_kbp_print_diag_all_entries(unit));
	}
...
cmd_result_t
cmd_diag_load_balancing_cmp(int unit, args_t* a) {
    uint32 soc_rc;

    soc_rc = (MBCM_PP_DRIVER_CALL(unit, mbcm_pp_lag_print_ecmp_lb_data,(unit)));
    if(SOC_SAND_FAILURE(soc_rc)) {
        return CMD_FAIL;
    }
    return CMD_OK;
...
print_olp_warning(int unit, args_t* a){
    ...
    SOC_PPD_DIAG_RECEIVED_PACKET_INFO       prm_rcvd_pkt_info;
    ...
    /* Otherwise, skip OLP Warning print */
        if (skip_validation == 0) {    
            rv = arad_pp_diag_is_valid(unit, core, ARAD_PP_DIAG_RECEIVED_PACKET_INFO_GET, &ret);
...
print_pp_usage(int unit)
{
    ...
    "\nLAST - display diagnostic PP information for the last packet"
    "\n\tDetailed - give more detailed prints"
    ...
    "\nECMP_load_balancing - Displays the ECMP load balancing information"
#endif /* BCM_JERICHO_SUPPORT */
