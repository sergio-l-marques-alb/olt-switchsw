...
_phy_diag_portmod_dsc(int unit, bcm_pbmp_t pbmp,args_t *args)
{
    ...
    DPORT_BCM_PBMP_ITER(unit, pbmp, dport, port) {
        ...
        if (!config) {
            rv = phymod_diag_dsc_std(phy_access, nof_phys_structs);
            if(rv < 0 ){
                return CMD_FAIL;
            }
        } else {
            rv = phymod_diag_dsc_config(phy_access, nof_phys_structs);
            if(rv < 0 ){
                return CMD_FAIL;
            }
        }
...
_phy_jer_nif_measure(int unit, bcm_port_t port, uint32 *type_of_bit_clk, int *one_clk_time_measured_int, int *one_clk_time_measured_remainder, int *serdes_freq_int, int *serdes_freq_remainder, uint32 *lane) {
    ...
    if (rv != BCM_E_NONE) {
        cli_out("ERROR: Failed to get interface type: %s\n",
                bcm_errmsg(rv));
        return BCM_E_FAIL;
    }
    ...
    *one_clk_time_measured_remainder = ((((total_time_measured_int*10000) + (total_time_measured_remainder)) / (sync_sth_cnt * vco_div)) % 10000);
    ...
    if ((pll_type == SOC_JER_NIF_PLL_TYPE_PMH) && (interface_type != _SHR_PORT_IF_SGMII)){
        os_int = 1;
        os_remainder = 0;
    }
...
#define _call_pdi(unit, p, mp, s) \
    ...
    DPORT_BCM_PBMP_ITER(unit, pbm, dport, p) {
        ...
        if (!SOC_IS_DFE(unit) && !IS_SFI_PORT(unit, p)){
