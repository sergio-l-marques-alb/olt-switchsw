...
static int pp_get_ce_qualifier(uint16 ce_value, int ce_type, char *stage_n, qualifier_t *qualifier)
{
    ...
    if(!(ce_value & INTERNAL_BUFFER_QUALIFIER)) {
        ...
        /* CE is qual signal */
        if((buffer_size = dpp_stage_size(stage_n, qualifier->header)) < 0) {
        ...
        if(dpp_qual_signal_get(stage_n, qualifier->offset, qualifier->size, qualifier->header, qualifier->name) != _SHR_E_NONE)
...
static void pp_data_set_key(void *curIndex, char *stage_n, char *key_n, int valid, qualifier_t *qualifier, int qualifier_num)
{
    ...
    if((curKey = xml_add_node(curIndex, key_n, 4)) == NULL) {
    ...
    for(i = 0; i < qualifier_num; i++) {
        if(valid & (1 << i)) {
            if(pp_get_ce_qualifier(qualifier[i].ce, qualifier[i].ce_type, stage_n, &qualifier[i]) == -1)
                return;
        }
    }
    ...
    for(i = 0; i < qualifier_num; i++) {
        if(qualifier[i].valid == TRUE) {
            if((curQualifier = xml_add_node(curKey, qualifier[i].name, 5)) == NULL) {
...
static void pp_dump_tables(int unit, void *curStage, int stage, int depth)
{
   ...
   if((curSection = xml_add_node(curStage, "Tables", depth)) == NULL) {
   ...
   for (table_id = 0; table_id < SOC_DPP_DBAL_SW_NOF_TABLES; table_id++) {
       ...
       dataio_replace_white_space(tmp_str, table.table_name);
       if((curTable = xml_add_node(curSection, tmp_str, depth + 1)) == NULL) {
       ...
       for (i = 0; i < table.nof_table_programs; i++) {
           ...
           if((curIndex = xml_add_node(curTable, str, depth + 2)) == NULL) {
               cli_out("Failed to add VT selection entry:%d\n", i);
               return;
           }
           ...
           for (j = 0; j < table.nof_qualifiers; j++) {
               ...
               if((curQual = xml_add_node(curIndex, tmp_str, depth + 3)) == NULL) {
                   cli_out("Failed to add VT selection entry:%d\n", i);
                   return;
               }
               str = (char *)ARAD_PP_FP_QUAL_TYPE_to_string(table.qual_info[j].qual_type);
           ...
           xml_close_node(curIndex, depth + 2);
       }
       xml_close_node(curTable, depth + 1);
   }
   xml_close_node(curSection, depth);
exit:
   return;
}
...
static void pp_vt_ce_attribute_set(int unit, void *curKey, uint32 valid_1, uint32 valid_2, ARAD_PP_IHP_VTT1ST_KEY_CONSTRUCTION_TBL_DATA *vt_key_construction_p)
{
    ...
    for(i = 0; i < qualifier_num; i++) {
        if(valid & (1 << i)) {
            if(pp_get_ce_qualifier(qualifier[i].ce, qualifier[i].ce_type, "VT", &qualifier[i]) == -1)
                return;
        }
    }
...
static void pp_tt_ce_attribute_set(int unit, void *curKey, uint32 valid_1, uint32 valid_2, ARAD_PP_IHP_VTT2ND_KEY_CONSTRUCTION_TBL_DATA *tt_key_construction_p)
{
    ...
    for(i = 0; i < qualifier_num; i++) {
        if(valid & (1 << i)) {
            if(pp_get_ce_qualifier(qualifier[i].ce, qualifier[i].ce_type, "TT", &qualifier[i]) == -1)
                return;
        }
    }
...
static int pp_dump_vt_selection(int unit, void *curStage, int depth)
{
    ...
    /* Create VT sub-sections: Program-Selection */
    if((curSection = xml_add_node(curStage, "Program-Selection", depth)) == NULL) {
    ...
    for(i = 0; i < SOC_DPP_DEFS_GET(unit, nof_vtt_program_selection_lines); i++) {
        ...
        if((curIndex = xml_add_node(curSection, selection_name, depth + 1)) == NULL) {
               cli_out("Failed to add VT selection entry:%d\n", i);
            return CMD_FAIL;
        }
...
static int pp_dump_vt_programs(int unit, void *curStage, int depth)
{
    ...
    if((curSection = xml_add_node(curStage, "Programs", 2)) == NULL) {
    ...
    for(i = 0; i < SOC_DPP_DEFS_GET(unit, nof_vtt_programs); i++) {
        ...
        if((curIndex = xml_add_node(curSection, program_name, depth + 1)) == NULL) {
        ...
        if(vt_key_construction.vlan_translation_is_use_strength == 0) {
            ...
            /* Strength in use */
            if((curTable = xml_add_node(curIndex, "Priority-Decoder-0", depth + 2)) == NULL) {
...
/*            RHDATA_SET_HEX(curTable, "Bitmap", vt_key_construction.vlan_translation_0_pd_bitmap); */
            ...
            if((curTable = xml_add_node(curIndex, "Priority-Decoder-1", depth + 2)) == NULL) {
                   cli_out("Failed to add TT selection entry:%d\n", i);
                return CMD_FAIL;
            }
/*           RHDATA_SET_HEX(curTable, "Bitmap", vt_key_construction.vlan_translation_1_pd_bitmap); */
        ...
        if(vt_key_construction.isa_lookup_enable) {
            if((curTable = xml_add_node(curIndex, "ISEM_A", depth + 2)) == NULL) {
                   cli_out("Failed to add TT selection entry:%d\n", i);
                return CMD_FAIL;
            }
            ...
            xml_close_node(curTable, depth + 2);
        }
        if(vt_key_construction.isb_lookup_enable) {
            if((curTable = xml_add_node(curIndex, "ISEM_B", depth + 2)) == NULL) {
                   cli_out("Failed to add TT selection entry:%d\n", i);
                return CMD_FAIL;
            }
        ...
        if(vt_key_construction.tcam_db_profile != 0x3F) {
            if((curTable = xml_add_node(curIndex, "TCAM", depth + 2)) == NULL) {
                   cli_out("Failed to add TT selection entry:%d\n", i);
                return CMD_FAIL;
            }
        ...
        xml_close_node(curIndex, depth + 1);
    }
...
static int pp_dump_tt_selection(int unit, void *curStage, int depth)
{
    ...
    for(i = 0; i < SOC_DPP_DEFS_GET(unit, nof_vtt_program_selection_lines); i++) {
        ...
        if((curIndex = xml_add_node(curSection, tmp_str, depth + 1)) == NULL) {
               cli_out("Failed to add TT selection entry:%d\n", i);
            return CMD_FAIL;
        }
...
static int pp_dump_tt_programs(int unit, void *curStage, int depth)
{
    ...
    if((curSection = xml_add_node(curStage, "Programs", depth)) == NULL) {
...
static int pp_dump_flp_programs(int unit, void *curStage, int depth)
{
    ...
    for(i = 0; i < SOC_DPP_DEFS_GET(unit, nof_flp_programs); i++) {
        ...
        pp_data_set_key(curIndex, "FLP", "Key-A", valid_A, qualifier, k);
        pp_data_set_key(curIndex, "FLP", "Key-B", valid_B, qualifier, k);
        pp_data_set_key(curIndex, "FLP", "Key-C", valid_C, qualifier, k);
        pp_data_set_key(curIndex, "FLP", "Key-D", valid_D, qualifier, k);
        if(lookups_tbl.lem_1st_lkp_valid) {
            if((curTable = xml_add_node(curIndex, "LEM-1st", depth + 2)) == NULL) {
        ...
        if(lookups_tbl.lem_2nd_lkp_valid) {
            if((curTable = xml_add_node(curIndex, "LEM-2nd", depth + 2)) == NULL) {
        ...
        if(lookups_tbl.lpm_1st_lkp_valid) {
            if((curTable = xml_add_node(curIndex, "LPM-1st", depth + 2)) == NULL) {
                   cli_out("Failed to add LEM-1 program entry:%d\n", i);
                return CMD_FAIL;
            }
            ...
            if(lookups_tbl.lpm_public_1st_lkp_valid) {
                if((curPublicTable = xml_add_node(curTable, "Public", depth + 3)) == NULL) {
        ...
        if(lookups_tbl.lpm_2nd_lkp_valid) {
            if((curTable = xml_add_node(curIndex, "LPM-2nd", depth + 2)) == NULL) {
                   cli_out("Failed to add LEM-2 program entry:%d\n", i);
                return CMD_FAIL;
            }
        ...
        if (lookups_tbl.tcam_lkp_db_profile != ARAD_TCAM_ACCESS_PROFILE_INVALID ) {
            if((curTable = xml_add_node(curIndex, "TCAM-1st", depth + 2)) == NULL) {
                   cli_out("Failed to add LEM-1 program entry:%d\n", i);
                return CMD_FAIL;
            }
        ...
        if (lookups_tbl.tcam_lkp_db_profile_1 != ARAD_TCAM_ACCESS_PROFILE_INVALID ) {
            if((curTable = xml_add_node(curIndex, "TCAM-2nd", depth + 2)) == NULL) {
                   cli_out("Failed to add LEM-1 program entry:%d\n", i);
                return CMD_FAIL;
            }
        ...
        if(lookups_tbl.elk_lkp_valid) {
            if((curTable = xml_add_node(curIndex, "ELK", depth + 2)) == NULL) {
                   cli_out("Failed to add LEM-1 program entry:%d\n", i);
                return CMD_FAIL;
            }
        ...
        if((curTable = xml_add_node(curIndex, "Result-A", depth + 2)) == NULL) {
        ...
        if((curTable = xml_add_node(curIndex, "Result-B", depth + 2)) == NULL) {
               cli_out("Failed to add Result-A for program entry:%d\n", i);
            return CMD_FAIL;
        }
        ...
        if((curTable = xml_add_node(curIndex, "Misc", depth + 2)) == NULL) {
               cli_out("Failed to add Result-A for program entry:%d\n", i);
            return CMD_FAIL;
        }
...
static uint32 dpp_dump_flp_selection(int unit, dpp_export_pp_stage_info_t *stage)
{
    uint32 res = CMD_OK;
    ARAD_PP_IHB_FLP_PROGRAM_SELECTION_CAM_TBL_DATA    flp_prog_selection;
    int i;
...
static int dpp_dump_flp_programs(int unit,  dpp_export_pp_stage_info_t *stage)
{
    uint32         res = CMD_OK;
    int i;
    char *program_name;
...
static int dpp_dump_flp(int unit, dpp_export_pp_stage_info_t *stage)
{
    uint32         res = CMD_OK;

    /* Create FLP sub-sections: Program-Selection */
    res = dpp_dump_flp_selection(unit, stage);

    /* Create FLP sub-sections: Programs */
    res = dpp_dump_flp_programs(unit, stage);
...
static void pp_dump_parser_print_entry(int unit, void *curTop, int depth, int offset, int base, int macro, int first, int last)
{
    ...
    if((cur = xml_add_node(curTop, entry_str, depth + 1)) == NULL) {
    ...
    if((base == PARSER_PROGRAM_END) || (base == PARSER_PROGRAM_FIN)) {
        for(i = first; i <= last; i++)  {
            ...
            if(i == first) {
                ...
                dpp_sigparam_get("Packet_Format_Code", first_pfc, tmp_str);
                sprintf(entry_str, "%s(%d)", tmp_str, first_pfc);
            ...
            if(i == last) {
                ...
                dpp_sigparam_get("Packet_Format_Code", curr_pfc, tmp_str);
                sprintf(entry_str, "%s(%d)", tmp_str, curr_pfc);
...
static uint32 pp_dump_parser_ports(int unit, void *curTop, int depth)
{
    ...
    BCM_PBMP_ITER(pbmp_nif, port) {
        ...
        if((cur = xml_add_node(curTop, entry_n, depth)) == NULL) {
            cli_out("Failed to add Program Selection section to IRPP Parser\n");
            res = CMD_FAIL;
            goto exit;
        }
...
static uint32 pp_dump_parser_macros(int unit, void *curTop, int depth)
{
    ...
    /* Eth Protocols */
    if((cur = xml_add_node(curTop, "Eth-Protocols", depth)) == NULL) {
        cli_out("Failed to add Program Selection section to IRPP Parser\n");
        res = CMD_FAIL;
        goto exit;
    }
    ...
    /* IP Protocols */
    if((cur = xml_add_node(curTop, "IP-Protocols", depth)) == NULL) {
        cli_out("Failed to add Program Selection section to IRPP Parser\n");
        res = CMD_FAIL;
        goto exit;
    }
    ...
    /* Custom Protocols */
    if((cur = xml_add_node(curTop, "Macro-Protocols", depth)) == NULL) {
        cli_out("Failed to add Program Selection section to IRPP Parser\n");
        res = CMD_FAIL;
        goto exit;
    }
...
static int pp_dump_parser(int unit, void *curStage, int depth)
{
    ...
    if((curSection = xml_add_node(curStage, "Ports", depth)) == NULL) {
    ...
    if((curSection = xml_add_node(curStage, "Custom-Macros", depth)) == NULL) {
...
static int dpp_get_dict_top(int unit, char **top_name)
{
    int res = _SHR_E_NONE;
    switch(SOC_INFO(unit).chip_type) {
        ...
        case SOC_INFO_CHIP_TYPE_QAX:
            *top_name = "qax";
            break;
        default:
            LOG_WARN(BSL_LS_APPL_SHELL, (BSL_META_U(unit, "Not supported for %s\n"), soc_dev_name(unit)));
            res = _SHR_E_UNAVAIL;
            break;
    }
...
dpp_export_pp_build_db(int unit, dpp_export_pp_info_t *dpp_export_pp_info)
{
    ...
    int program_id, key, action, table_index, i;
    int program_selection;
    ARAD_PP_FP_RESOURCE_DIAG_INFO *fp_info = NULL;
    ...
    /* Dump selection and programm */
    dpp_dump_flp(unit, &dpp_export_pp_info->stages[SOC_PPC_FP_DATABASE_STAGE_INGRESS_FLP]);
    /* DBAL - FLP */
    ...
    for (table_index = 0; table_index < SOC_DPP_DBAL_SW_NOF_TABLES; table_index++) {
        ...
        if (!table.is_table_initiated || (table.table_programs[0].stage != ARAD_FP_DATABASE_STAGE_INGRESS_FLP)) continue;

        sal_strcpy(dpp_export_pp_info->stages[SOC_PPC_FP_DATABASE_STAGE_INGRESS_FLP].tables[table_index].name, table.table_name);  /* Using same name as in DBAL */
        sal_strncpy(dpp_export_pp_info->stages[SOC_PPC_FP_DATABASE_STAGE_INGRESS_FLP].tables[table_index].database, arad_pp_dbal_physical_db_to_string(table.physical_db_type), DPP_EXPORT_CHAR_MAX_LENGTH - 1);
        for (key = 0; key < table.nof_qualifiers; key++) {
            sal_strncpy(dpp_export_pp_info->stages[SOC_PPC_FP_DATABASE_STAGE_INGRESS_FLP].tables[table_index].keys[key].name, SOC_PPC_FP_QUAL_TYPE_to_string(table.qual_info[key].qual_type), DPP_EXPORT_CHAR_MAX_LENGTH - 1);
        }

        if (table.nof_actions) {
            for (action = 0; action < table.nof_actions; action++) {
                sal_strncpy(dpp_export_pp_info->stages[SOC_PPC_FP_DATABASE_STAGE_INGRESS_FLP].tables[table_index].actions[action].name, SOC_PPC_FP_ACTION_TYPE_to_string(table.action[action]), DPP_EXPORT_CHAR_MAX_LENGTH - 1);
            }
        }
        else {
            sal_strcpy(dpp_export_pp_info->stages[SOC_PPC_FP_DATABASE_STAGE_INGRESS_FLP].tables[table_index].actions[0].name, "ClassDestSet");
        }

        dpp_export_pp_table_priority_get(unit,
                                              table.table_programs[0].program_id, /* FLP - not unique per table, taking first! */
                                              table.table_programs[0].key_id,
                                              table.physical_db_type,
                                              &dpp_export_pp_info->stages[SOC_PPC_FP_DATABASE_STAGE_INGRESS_FLP].tables[table_index].priority);

        dpp_export_pp_info->stages[SOC_PPC_FP_DATABASE_STAGE_INGRESS_FLP].tables[table_index].is_static = 1; /* All FLP tables are static (not configured through Arrkis) */

        /* Programs information is taken from each table */
        for (i = 0; i < table.nof_table_programs; i++) {
            /* Only FLP is relevant here */
            if (table.table_programs[i].stage == SOC_PPC_FP_DATABASE_STAGE_INGRESS_FLP) {
                program_id = table.table_programs[i].program_id; /* program ID refers to the cam selection line*/
                if(ISEMPTY(dpp_export_pp_info->stages[SOC_PPC_FP_DATABASE_STAGE_INGRESS_FLP].bundles[program_id].name))
                    cli_out("No program name for program:%d", program_id);
                dpp_export_pp_info->stages[SOC_PPC_FP_DATABASE_STAGE_INGRESS_FLP].bundles[program_id].
                   tables[dpp_export_pp_info->stages[SOC_PPC_FP_DATABASE_STAGE_INGRESS_FLP].bundles[program_id].nof_tables].lookup =
                   table.table_programs[i].lookup_number;
                dpp_export_pp_info->stages[SOC_PPC_FP_DATABASE_STAGE_INGRESS_FLP].bundles[program_id].
                   tables[dpp_export_pp_info->stages[SOC_PPC_FP_DATABASE_STAGE_INGRESS_FLP].bundles[program_id].nof_tables].table =
                   &dpp_export_pp_info->stages[SOC_PPC_FP_DATABASE_STAGE_INGRESS_FLP].tables[table_index]; /* Link to the current table */
                dpp_export_pp_info->stages[SOC_PPC_FP_DATABASE_STAGE_INGRESS_FLP].bundles[program_id].nof_tables =
                   dpp_export_pp_info->stages[SOC_PPC_FP_DATABASE_STAGE_INGRESS_FLP].bundles[program_id].nof_tables + 1;
                dpp_export_pp_info->stages[SOC_PPC_FP_DATABASE_STAGE_INGRESS_FLP].bundles[program_id].is_static = 1;
            }
        }
    ...
    /* FP - PMF+FLP */
    BCMDNX_ALLOC(fp_info, sizeof(ARAD_PP_FP_RESOURCE_DIAG_INFO), "ARAD_PP_FP_RESOURCE_DIAG_INFO");
    if (fp_info == NULL) {
        return CMD_FAIL;
    }

    ARAD_PP_FP_RESOURCE_DIAG_INFO_clear(fp_info);
    ...
    for (table_index = 0; table_index < SOC_PPC_FP_NOF_DBS; table_index++) {
        if (fp_info->db[table_index].valid) {
            ...
            if (fp_info->db[table_index].type == SOC_PPC_FP_DB_TYPE_TCAM) {
                ...
                dpp_export_pp_info->stages[stage].tables[table_index].is_static = 1/*(arad_export_fp_field_group_id_to_name[stage][table_index] == NULL)*/; /* is configured through Arrkis */
...
static int pp_dump_export_data(int unit, dpp_export_pp_info_t * dpp_export_pp_info, SOC_PPC_FP_DATABASE_STAGE stage, void *curTop, int depth)
{
    uint32         res = CMD_OK;
    ...
    for (program_selection = 0; program_selection < DPP_EXPORT_PP_PRESEL_MAX; program_selection++) {
        if (!ISEMPTY(dpp_export_pp_info->stages[stage].presels[program_selection].name)) {
            if((curPresel = xml_add_node(curTop, "contextselect", depth)) == NULL) {
                   cli_out("Failed to add FLP selection entry:%d\n", program_selection);
                return CMD_FAIL;
            }

            RHDATA_SET_STR(curPresel, "name", dpp_export_pp_info->stages[stage].presels[program_selection].name);
            RHDATA_SET_STR(curPresel, "context", dpp_export_pp_info->stages[stage].presels[program_selection].bundle->name);
            RHDATA_SET_INT(curPresel, "user", !dpp_export_pp_info->stages[stage].presels[program_selection].is_static);
            RHDATA_SET_INT(curPresel, "priority", program_selection); /*ADD IMPLEMENTATION HERE!!! - I assume we can use TCAM index */

            if((curIndex = xml_add_node(curPresel, "field", depth + 1)) == NULL) {
                   cli_out("Failed to add FLP selection entry:%d\n", program_selection);
                return CMD_FAIL;
            }
            RHDATA_SET_STR(curIndex, "name", "AppType");
            RHDATA_SET_STR(curIndex, "value", dpp_export_pp_info->stages[stage].presels[program_selection].app_type);

            xml_close_node(curIndex, depth + 1);
            xml_close_node(curPresel, depth);
        }
    }

    /* Bundles */
    for (program_id = 0; program_id < DPP_EXPORT_PP_BUNDLE_TABLE_MAX; program_id++) {
        if (!ISEMPTY(dpp_export_pp_info->stages[stage].bundles[program_id].name)) {

            if((curBundle = xml_add_node(curTop, "context", depth)) == NULL) {
                   cli_out("Failed to add FLP selection entry:%d\n", program_selection);
                return CMD_FAIL;
            }

            RHDATA_SET_STR(curBundle, "name", dpp_export_pp_info->stages[stage].bundles[program_id].name);
            RHDATA_SET_INT(curBundle, "user", !dpp_export_pp_info->stages[stage].bundles[program_id].is_static);

            for (table_index = 0; table_index < dpp_export_pp_info->stages[stage].bundles[program_id].nof_tables; table_index++) {
                if (dpp_export_pp_info->stages[stage].bundles[program_id].tables[table_index].table) {

                    if((curTable = xml_add_node(curBundle, "table", depth + 1)) == NULL) {
                           cli_out("Failed to add FLP selection entry:%d\n", program_selection);
                        return CMD_FAIL;
                    }

                    RHDATA_SET_STR(curTable, "name", dpp_export_pp_info->stages[stage].bundles[program_id].tables[table_index].table->name);
                    RHDATA_SET_INT(curTable, "lookup", dpp_export_pp_info->stages[stage].bundles[program_id].tables[table_index].lookup);
                    RHDATA_SET_INT(curTable, "user", !dpp_export_pp_info->stages[stage].bundles[program_id].tables[table_index].is_static);

                    xml_close_node(curTable, depth + 1);
                }
            }

            xml_close_node(curBundle, depth);
        }
    }
    ...
    for (table_index = 0; table_index < DPP_EXPORT_PP_TABLE_MAX; table_index++) {
        if (!ISEMPTY(dpp_export_pp_info->stages[stage].tables[table_index].name)) {
            ...
            if((curTable = xml_add_node(curTop, "table", depth)) == NULL) {
                   cli_out("Failed to add FLP selection entry:%d\n", program_selection);
                return CMD_FAIL;
            }

            RHDATA_SET_STR(curTable, "name", dpp_export_pp_info->stages[stage].tables[table_index].name);
            RHDATA_SET_STR(curTable, "memory", dpp_export_pp_info->stages[stage].tables[table_index].database);
            RHDATA_SET_INT(curTable, "priority", dpp_export_pp_info->stages[stage].tables[table_index].priority);
            RHDATA_SET_INT(curTable, "user", !dpp_export_pp_info->stages[stage].tables[table_index].is_static);

            for (key = 0; key < SOC_PPC_FP_NOF_QUALS_PER_DB_MAX; key++) {
                if (!ISEMPTY(dpp_export_pp_info->stages[stage].tables[table_index].keys[key].name)) {
                    if((curKey = xml_add_node(curTable, "field", depth + 1)) == NULL) {
                           cli_out("Failed to add FLP selection entry:%d\n", program_selection);
                        return CMD_FAIL;
                    }

                    RHDATA_SET_STR(curKey, "value", dpp_export_pp_info->stages[stage].tables[table_index].keys[key].name);
                    xml_close_node(curKey, depth+1);
                }
            }

            for (action = 0; action < SOC_PPC_FP_NOF_ACTIONS_PER_DB_MAX; action++) {
                if (!ISEMPTY(dpp_export_pp_info->stages[stage].tables[table_index].actions[action].name)) {
                    if((curAction = xml_add_node(curTable, "action", depth + 1)) == NULL) {
                           cli_out("Failed to add FLP selection entry:%d\n", program_selection);
                        return CMD_FAIL;
                    }

                    RHDATA_SET_STR(curAction, "value", dpp_export_pp_info->stages[stage].tables[table_index].actions[action].name);
                    xml_close_node(curAction, depth+1);
                }
            }
            xml_close_node(curTable, depth);
        }
    }
...
static cmd_result_t pp_scheme_export(int unit, void *curTop)
{
    ...
    /* Stages */
    if ((Stages = xml_add_node(curTop, "stages", 1)) == NULL) {
    ...
    /* FLP */
    if ((curStage = xml_add_node(Stages, "stage", 2)) == NULL) {
        cli_out("Failed to add FLP section to dump\n");
        res = CMD_FAIL;
        goto exit;
    }

    RHDATA_SET_STR(curStage, "name", "IRPP.FLP");

    pp_dump_export_data(unit, dpp_export_pp_info, SOC_PPC_FP_DATABASE_STAGE_INGRESS_FLP, curStage, 3);
    xml_close_node(curStage, 2);
    ...
    RHDATA_SET_STR(curStage, "name", "IRPP.PMF");

    pp_dump_export_data(unit, dpp_export_pp_info, SOC_PPC_FP_DATABASE_STAGE_INGRESS_PMF, curStage, 3);
    sal_free(dpp_export_pp_info);
    xml_close_node(curStage, 2);
    xml_close_node(Stages, 1);
...
print_diag_pp_sig_usage(void)
{
    char diag_pp_sig_usage[] =
        "diag pp sig [name=Signal] [from=Stage] [to=Stage] [stage=Stage] [order=little/big] [core=0/1]\n"
        ...
        "        detail   - provides verilog name for specific signal and address ranges\n"
        "                   Disables expansion\n"
        ...
        "        internal - presents qualifiers per stage\n"
        ...
        "        double   - presents double signals - the ones that have same\n"
        "                   \"from\" \"to\" and \"attribute\" characteristics\n"
        ...
        "                   Case insensitive. Search include verilog names and sub-fields\n"
...
cmd_result_t dpp_export_pp(int unit, char *stage_n, char *filename)
{
    ...
    if(ISEMPTY(stage_n)) {
        ...
        if ((curTop = xml_create_top("top")) == NULL) {
        ...
        for(i = 0; stage_callback[i].name != NULL; i++) {
            if(!sal_strcasecmp(stage_n, stage_callback[i].name)) {
                if(stage_callback[i].dump_callback) {
                    if((curTop = xml_create_top(stage_callback[i].name)) == NULL) {
    ...
    if(ISEMPTY(filename)) {
        for(i = 0; i < 100; i++) {
            ...
            if(dataio_file_exists(filename) != _SHR_E_NONE)
    ...
    if(dataio_get_filepath(NULL, filename, filepath) != _SHR_E_NONE)
        goto exit;

    xml_save(curTop, filepath);
    xml_close_top(curTop);

    cli_out("Saving to:%s.xml\n", filename);
...
cmd_result_t appl_dpp_diag_sig_init(int unit)
{
#if !defined(__KERNEL__)
    ...
    if((dpp_get_dict_top(unit, &global_descr)) != _SHR_E_NONE) {
        res = CMD_NFND;
        goto exit;
    }

    if((dpp_signals_init(global_descr)) != _SHR_E_NONE) {
        res = CMD_FAIL;
        goto exit;
    }
...
cmd_result_t cmd_diag_pp_signals_get(int unit, args_t* a)
{
    ...
    SIG_COMMAND command;
    int print_flags = SIGNALS_PRINT_EXPAND;
    int prm_core = 0;
    ...
    if((res = appl_dpp_diag_sig_init(unit)) != CMD_OK) {
        LOG_WARN(BSL_LS_APPL_SHELL, (BSL_META_U(unit, "Signal DB was not initialized\n")));
        goto exit;
    }
    ...
    tokens = dataio_split_mstr(show_n, ",", 6, &realtokens);
    ...
    for(i = 0; i < realtokens; i++) {
        ...
        else if(!sal_strcasecmp(tokens[i], "detail"))
            print_flags |= SIGNALS_PRINT_VERILOG;
        else if(!sal_strcasecmp(tokens[i], "noexpand"))
            print_flags &= ~SIGNALS_PRINT_EXPAND;
        ...
        else if(!sal_strcasecmp(tokens[i], "double"))
            match_m.flags |= SIGNALS_MATCH_DOUBLE;
        ...
        else if(!sal_strcasecmp(tokens[i], "internal"))
            command = SIG_COMMAND_INTERNAL;
        else if(!sal_strcasecmp(tokens[i], "single"))
            command = SIG_COMMAND_DEBUG_SINGLE;
        else
    ...
    if(print_flags & SIGNALS_PRINT_VERILOG)
        print_flags &= ~SIGNALS_PRINT_FULL;
    ...
    switch(command) {
        ...
        case SIG_COMMAND_STRUCT:
            res = dpp_sigstruct_print(match_n);
            break;
        case SIG_COMMAND_PARAM:
            res = dpp_sigparam_print(match_n);
            break;
        case SIG_COMMAND_INTERNAL:
            res = dpp_qual_signal_print(&match_m, print_flags);
            break;
        case SIG_COMMAND_DEBUG:
            if((dsig_list = rhlist_create("prt_print", sizeof(signal_output_t), 0)) == NULL)
                return CMD_FAIL;

            dpp_dsig_get_list(unit, prm_core, &match_m, dsig_list);
            res = dpp_dsig_print(dsig_list, print_flags);
            dpp_dsig_free(dsig_list);
            break;
        case SIG_COMMAND_DEBUG_SINGLE:
            if(dpp_dsig_get(unit, prm_core, &match_m, &signal_output) != _SHR_E_NONE) {
                cli_out("Signal not found:%s\n", match_m.name);
            } else {
                cli_out("Block:%s Stage From:%s To:%s\n",
                       signal_output.debug_signal->block_n, signal_output.debug_signal->from, signal_output.debug_signal->to);
                cli_out("Signal:%s - %s\n", signal_output.debug_signal->attribute, signal_output.print_value);
            }
            break;
        default:
            break;
