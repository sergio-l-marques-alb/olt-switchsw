...
cmd_diag_test(int unit, args_t* a)
{
    ...
    if(!option) {
        ...
        return _bcm_petra_field_test_entry_traffic_perf(unit, i_stage, x, i_mode, do_warmboot);
        ...
        return _bcm_petra_field_test_full_tcam_perf(unit, i_stage, x, i_mode, do_warmboot);
        ...
        return _bcm_petra_field_test_full_tcam_diff_prio_perf(unit, i_stage, x, i_mode, do_warmboot);
...
_dpp_field_get_qual_list(SOC_PPC_FP_DATABASE_INFO *dbInfo_p, char *qual_str, int *qual_num_p)
{
    int i;
    int qual_num = 0;
    sal_memset(qual_str, 0, SOC_PPC_FP_NOF_QUALS_PER_DB_MAX * RHNAME_MAX_SIZE);
    for (i = 0; i < SOC_PPC_FP_NOF_QUALS_PER_DB_MAX; i++) {
        if (_BCM_DPP_FIELD_PPD_QUAL_VALID(dbInfo_p->qual_types[i])) {
            strcpy(qual_str, SOC_PPC_FP_QUAL_TYPE_to_string(dbInfo_p->qual_types[i]));
            qual_str += RHNAME_MAX_SIZE;
            qual_num++;
        }
    }
...
_dpp_field_get_action_list(SOC_PPC_FP_DATABASE_INFO *dbInfo_p, char *action_str, int *action_num_p)
{
    int i;
    int action_num = 0;
    sal_memset(action_str, 0, SOC_PPC_FP_NOF_ACTIONS_PER_DB_MAX * RHNAME_MAX_SIZE);
    for (i = 0; i < SOC_PPC_FP_NOF_ACTIONS_PER_DB_MAX; i++) {
        if (_BCM_DPP_FIELD_PPD_ACTION_VALID(dbInfo_p->action_types[i])) {
            strcpy(action_str, SOC_PPC_FP_ACTION_TYPE_to_string(dbInfo_p->action_types[i]));
            action_str += RHNAME_MAX_SIZE;
            action_num++;
        }
    }
    ...
    for (index = 0; ranges[index].qualifier < bcmFieldQualifyCount; index++) {
        for (range = ranges[index].rangeBase; range < ranges[index].rangeBase + ranges[index].count; range++) {
            ...
            if (BCM_E_NONE == result) {
                ...
                PRT_PRINT("%s(%d)", _bcm_dpp_field_qual_name[ranges[index].qualifier], ranges[index].qualifier);
                PRT_PRINT("%d", range);
                for (offset = 0; offset < actual; offset++) {
                    ...
                    PRT_PRINT("%s", flags_str);
                    PRT_PRINT("%d", rangeMin[offset]);
                    PRT_PRINT("%d", rangeMax[offset]);
                }
            }
...
cmd_dpp_field_table_list(int unit)
{
    ...
    int i;
    char qual_str[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX][RHNAME_MAX_SIZE];
    int qual_num;
    char action_str[SOC_PPC_FP_NOF_ACTIONS_PER_DB_MAX][RHNAME_MAX_SIZE];
    int action_num;
    int line_num;
    ...
    sal_memset(qual_str, 0, SOC_PPC_FP_NOF_QUALS_PER_DB_MAX * 64);
    sal_memset(action_str, 0, SOC_PPC_FP_NOF_ACTIONS_PER_DB_MAX * 64);
    ...
    for (db_id = 0; db_id < SOC_PPC_FP_NOF_DBS; db_id++) {
        ...
        _dpp_field_get_qual_list(&dbInfo, (char *)qual_str, &qual_num);
        _dpp_field_get_action_list(&dbInfo, (char *)action_str, &action_num);
        line_num = action_num > qual_num ? action_num : qual_num;
        PRT_PRINT("%d", db_id);
        for(i = 0; i < line_num; i++) {
            if(i == line_num - 1) {
                PRT_ALLOC_LINE(LINE_UNDERSCORE);
            }
            else {
                PRT_ALLOC_LINE(LINE_NONE);
            }
            if(i == 0) {
                PRT_PRINT("%d", db_id);
                PRT_PRINT("%s", SOC_PPC_FP_DATABASE_TYPE_to_string(dbInfo.db_type));
                PRT_PRINT("%s", SOC_PPC_FP_DATABASE_STAGE_to_string(stage));
                PRT_PRINT("%d", priority);
                PRT_PRINT("%d", key_size);
                PRT_PRINT("%d", fp_entry.nof_db_entries);
            }
            else {
                PRT_SKIP(6);
            }

            PRT_PRINT("%s", qual_str[i]);
            PRT_PRINT("%s", action_str[i]);
        }
    }
...
cmd_dpp_field_table_dump(int unit, int table_id, int start_entry_id, int output_num)
{
    ...
    PRT_SKIP(1);
    PRT_COPY("Actions");
    PRT_SKIP(action_num - 1);

    PRT_ALLOC_LINE(LINE_EQUAL);

    PRT_PRINT("%d", table_id);
    for (i = 0; i < qual_num; i++) {
        PRT_PRINT("%s", qual_str[i]);
    }

    PRT_SKIP(1);
    PRT_SKIP(1); /* for priority */
    PRT_SKIP(1);
    ...
    if ((SOC_PPC_FP_DB_TYPE_TCAM == dbInfo.db_type) ||
        ...
        for (soc_sandOffset = start_entry_id; soc_sandOffset < entryDeLimit; soc_sandOffset++) {
            ...
            if ((BCM_E_NONE == result) && okay) {
                ...
                PRT_SKIP(1);
                PRT_PRINT("%d", entInfoDe.priority);
                PRT_SKIP(1);
                for (i = 0; i < SOC_PPC_FP_NOF_ACTIONS_PER_DB_MAX; i++) {
                    if (_BCM_DPP_FIELD_PPD_ACTION_VALID(entInfoDe.actions[i].type)) {
                        PRT_PRINT("base  %08X", entInfoDe.actions[i].base_val);
