...
setup_itmh_tx(xd_t *xd)
{
    ...
    if (!SOC_IS_ARAD(xd->xd_tx_unit)) {
        ...
        if (SOC_IS_PETRAB(xd->xd_tx_unit)) { 
            int soc_sand_rv=0;
            soc_sand_rv = soc_pb_ports_itmh_extension_get_unsafe(soc_sand_dev_id, \
                0/* cpu port */, &ext_hdr_valid);
            rv = handle_sand_result(soc_sand_rv);
        }        
        if (BCM_FAILURE(rv)) {
            return;
        ...
        if (ext_hdr_valid) {
            ...
            sal_memcpy(&(pkt_info->_dpp_hdr[DPP_HDR_ITMH_BASE_LEN]), &itmh.ext.raw.bytes[0], \
...
xd->xd_tx_unit, bcm_errmsg(rv)));
    ...
    for (xd->xd_cur_cnt = 0;
        ...
        BCM_PBMP_ITER(pkt_info->tx_pbmp, port_ndx){
            ...
            dpp_tx_itmh_dest_port_set(xd->xd_tx_unit, pkt_info->pkt_data[0].data + DPP_HDR_PTCH_TYPE2_LEN, port_ndx);
