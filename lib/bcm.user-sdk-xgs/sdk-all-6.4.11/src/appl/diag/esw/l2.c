...
if_esw_l2(int unit, args_t *a)
{
    ...
    int bank0_hash_bucket = 0;
    int bank1_hash_bucket = 0;
    int bank0_offset = 0;
    int bank1_offset = 0;
#endif
    ...
    defined(BCM_TOMAHAWK_SUPPORT)
                    if (SOC_IS_TRIDENT2(unit) || SOC_IS_TOMAHAWK(unit)) {
                        ...
                        bank0_offset = 
                            soc_reg_field_get(unit, L2_TABLE_HASH_CONTROLr,
                                              regval, BANK0_HASH_OFFSETf);
                        bank1_offset = 
                            soc_reg_field_get(unit, L2_TABLE_HASH_CONTROLr,
                                              regval, BANK1_HASH_OFFSETf);
                        if (0 == hash_select2) {
            ...
            /* On TD2/TD2+/TH, there are two bits gap in the key of l2_entry between
             * vlan and mac. But the key that calculated by soc_tr_l2x_base_entry_to_key()
             * doesn't have such two bits gap, so re-calculate it*/
    ...
    defined(BCM_TRIDENT2_SUPPORT)
            if (SOC_IS_TRIDENT2X(unit) || SOC_IS_TOMAHAWK(unit)) {
                ...
                uint8 temp = 0;
                int i = 0;
                for (i = 2; i < XGS_HASH_KEY_SIZE; i++) {
                    temp = (key[i] >> 6) & 0x3;
                    if (2 == i) {
                        key[2] = (((key[2] >> 4) & 0x3) << 6) | (key[2] & 0xf);
                    } else {
                        key[i] = (key[i] << 2) | hibits;
                    }
                    hibits = temp;
                }
            }
    ...
    defined(BCM_TRIDENT2_SUPPORT)
            if (SOC_IS_TRIDENT2X(unit) || SOC_IS_TOMAHAWK(unit)) {
                hash_bucket = soc_td2x_th_l2x_hash(bits, key);
                bank0_hash_bucket = (hash_bucket >> bank0_offset) & 0xfff;
                bank1_hash_bucket = ((hash_bucket >> bank1_offset) & 0xfff) + 
                                    (4 * 1024);
            } else
#endif
            {
    ...
    if (SOC_IS_TRIDENT2X(unit) || SOC_IS_TOMAHAWK(unit)) {
        cli_out("Hash[%d] of key "
                    ...
                    "is bank0 bucket 0x%03x (%d) bank1 bucket 0x%03x (%d)\n",
                    hash_select,
                    key[7], key[6], key[5], key[4],
                    key[3], key[2], key[1], (key[0] >> 4) & 0xf,
                    bank0_hash_bucket, bank0_hash_bucket,
                    bank1_hash_bucket, bank1_hash_bucket);
    } else
#endif
    {
        cli_out("Hash[%d] of key "
