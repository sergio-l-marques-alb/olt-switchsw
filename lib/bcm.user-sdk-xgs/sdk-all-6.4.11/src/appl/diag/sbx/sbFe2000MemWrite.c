...
/*
 * $Id: sbFe2000MemWrite.c,v 1.13 Broadcom SDK $
   ...
   CAD_UINT                     uAddress,
    ...
    case FE2K_MEM_PRPORTTOQUEUEAG0:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          ...
          sbSwapWords(pData,1);
          sbZfCaPrPortToQueueAg0Entry_Unpack(&PrPortToQueueAg0Record,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaPrPortToQueueAg0Entry_SetField(&PrPortToQueueAg0Record,fieldname,val); 
          if(status != 0) return CMD_USAGE;
          sbZfCaPrPortToQueueAg0Entry_Pack(&PrPortToQueueAg0Record,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,1);
          status =
    ...
    case FE2K_MEM_PRPORTTOQUEUEAG1:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,1);
          sbZfCaPrPortToQueueAg1Entry_Unpack(&PrPortToQueueAg1Record,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaPrPortToQueueAg1Entry_SetField(&PrPortToQueueAg1Record,fieldname,val); 
          if(status != 0) return CMD_USAGE;
          sbZfCaPrPortToQueueAg1Entry_Pack(&PrPortToQueueAg1Record,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,1);
          status =
    ...
    case FE2K_MEM_PRPORTTOQUEUEPCI:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,1);
          sbZfCaPrPortToQueuePciEntry_Unpack(&PrPortToQueuePciRecord,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaPrPortToQueuePciEntry_SetField(&PrPortToQueuePciRecord,fieldname,val); 
          if(status != 0) return CMD_USAGE;
          sbZfCaPrPortToQueuePciEntry_Pack(&PrPortToQueuePciRecord,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,1);
          status =
    ...
    case FE2K_MEM_PRPORTTOQUEUESR0:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,1);
          sbZfCaPrPortToQueueSr0Entry_Unpack(&PrPortToQueueSr0Record,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaPrPortToQueueSr0Entry_SetField(&PrPortToQueueSr0Record,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaPrPortToQueueSr0Entry_Pack(&PrPortToQueueSr0Record,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,1);
          status =
    ...
    case FE2K_MEM_PRPORTTOQUEUESR1:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,1);
          sbZfCaPrPortToQueueSr1Entry_Unpack(&PrPortToQueueSr1Record,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaPrPortToQueueSr1Entry_SetField(&PrPortToQueueSr1Record,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaPrPortToQueueSr1Entry_Pack(&PrPortToQueueSr1Record,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,1);
          status =
    ...
    case FE2K_MEM_PRPORTTOQUEUEXG0:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,1);
          sbZfCaPrPortToQueueXg0Entry_Unpack(&PrPortToQueueXg0Record,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaPrPortToQueueXg0Entry_SetField(&PrPortToQueueXg0Record,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaPrPortToQueueXg0Entry_Pack(&PrPortToQueueXg0Record,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,1);
          status =
    ...
    case FE2K_MEM_PRPORTTOQUEUEXG1:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,1);
          sbZfCaPrPortToQueueXg1Entry_Unpack(&PrPortToQueueXg1Record,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaPrPortToQueueXg1Entry_SetField(&PrPortToQueueXg1Record,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaPrPortToQueueXg1Entry_Pack(&PrPortToQueueXg1Record,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,1);
          status =
    ...
    case FE2K_MEM_PDLRPOUTPUTHEADERCOPYBUFFER:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,8);
          sbZfCaPdLrpOutputHeaderCopyBufferEntry_Unpack(&PdLrpOutputHeaderCopyBufferRecord,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaPdLrpOutputHeaderCopyBufferEntry_SetField(&PdLrpOutputHeaderCopyBufferRecord,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaPdLrpOutputHeaderCopyBufferEntry_Pack(&PdLrpOutputHeaderCopyBufferRecord,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,8);
          status =
    ...
    case FE2K_MEM_PDPDOUTPUTHEADERCOPYBUFFER:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,8);
          sbZfCaPdPdOutputHeaderCopyBufferEntry_Unpack(&PdPdOutputHeaderCopyBufferRecord,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaPdPdOutputHeaderCopyBufferEntry_SetField(&PdPdOutputHeaderCopyBufferRecord,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaPdPdOutputHeaderCopyBufferEntry_Pack(&PdPdOutputHeaderCopyBufferRecord,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,8);
          status =
    ...
    case FE2K_MEM_LRLRPINSTRUCTIONMEMORY:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,3);
          sbZfCaLrLrpInstructionMemoryEntry_Unpack(&LrLrpInstructionMemoryRecord,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaLrLrpInstructionMemoryEntry_SetField(&LrLrpInstructionMemoryRecord,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaLrLrpInstructionMemoryEntry_Pack(&LrLrpInstructionMemoryRecord,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,3);
          status =
    ...
    case FE2K_MEM_QMBUFFERSTATE0:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,1);
          sbZfCaQmBufferState0Entry_Unpack(&QmBufferState0Record,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaQmBufferState0Entry_SetField(&QmBufferState0Record,fieldname,val);
          if(status != 0) return CMD_USAGE;

          sbZfCaQmBufferState0Entry_Pack(&QmBufferState0Record,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,1);
          status =
    ...
    case FE2K_MEM_QMBUFFERSTATE1:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,2);
          sbZfCaQmBufferState1Entry_Unpack(&QmBufferState1Record,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaQmBufferState1Entry_SetField(&QmBufferState1Record,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaQmBufferState1Entry_Pack(&QmBufferState1Record,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,2);
          status =
    ...
    case FE2K_MEM_QMBUFFERSTATE2:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,1);
          sbZfCaQmBufferState2Entry_Unpack(&QmBufferState2Record,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaQmBufferState2Entry_SetField(&QmBufferState2Record,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaQmBufferState2Entry_Pack(&QmBufferState2Record,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,1);
          status =
    ...
    case FE2K_MEM_QMBUFFERSTATE3:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,1);
          sbZfCaQmBufferState3Entry_Unpack(&QmBufferState3Record,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaQmBufferState3Entry_SetField(&QmBufferState3Record,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaQmBufferState3Entry_Pack(&QmBufferState3Record,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,1);
          status =
...
/* MCM was missing DequeueState0 code */
    case FE2K_MEM_QMDEQUEUESTATE0:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,2);
          sbZfCaQmDequeueState0Entry_Unpack(&QmDequeueState0Record,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaQmDequeueState0Entry_SetField(&QmDequeueState0Record,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaQmDequeueState0Entry_Pack(&QmDequeueState0Record,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,2);
          status =
    ...
    case FE2K_MEM_QMDEQUEUESTATE1:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,1);
          sbZfCaQmDequeueState1Entry_Unpack(&QmDequeueState1Record,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaQmDequeueState1Entry_SetField(&QmDequeueState1Record,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaQmDequeueState1Entry_Pack(&QmDequeueState1Record,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,1);
          status =
    ...
    case FE2K_MEM_QMFREEPAGEFIFO:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,1);
          sbZfCaQmFreePageFifoEntry_Unpack(&QmFreePageFifoRecord,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaQmFreePageFifoEntry_SetField(&QmFreePageFifoRecord,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaQmFreePageFifoEntry_Pack(&QmFreePageFifoRecord,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,1);
          status =
    ...
    case FE2K_MEM_QMNEXTBUFFER:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,1);
          sbZfCaQmNextBufferEntry_Unpack(&QmNextBufferRecord,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaQmNextBufferEntry_SetField(&QmNextBufferRecord,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaQmNextBufferEntry_Pack(&QmNextBufferRecord,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,1);
          status =
    ...
    case FE2K_MEM_QMNEXTPAGE:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,1);
          sbZfCaQmNextPageEntry_Unpack(&QmNextPageRecord,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaQmNextPageEntry_SetField(&QmNextPageRecord,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaQmNextPageEntry_Pack(&QmNextPageRecord,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,1);
          status =
    ...
    case FE2K_MEM_QMQUEUECONFIG:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,3);
          sbZfCaQmQueueConfigEntry_Unpack(&QmQueueConfigRecord,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaQmQueueConfigEntry_SetField(&QmQueueConfigRecord,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaQmQueueConfigEntry_Pack(&QmQueueConfigRecord,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,3);
          status =
    ...
    case FE2K_MEM_QMQUEUECOUNTERS:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,2);
          sbZfCaQmQueueCountersEntry_Unpack(&QmQueueCountersRecord,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaQmQueueCountersEntry_SetField(&QmQueueCountersRecord,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaQmQueueCountersEntry_Pack(&QmQueueCountersRecord,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,2);
          status =
    ...
    case FE2K_MEM_QMQUEUEHEADPTR:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,1);
          sbZfCaQmQueueHeadPtrEntry_Unpack(&QmQueueHeadPtrRecord,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaQmQueueHeadPtrEntry_SetField(&QmQueueHeadPtrRecord,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaQmQueueHeadPtrEntry_Pack(&QmQueueHeadPtrRecord,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,1);
          status =
    ...
    case FE2K_MEM_QMQUEUESTATE0:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,1);
          sbZfCaQmQueueState0Entry_Unpack(&QmQueueState0Record,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaQmQueueState0Entry_SetField(&QmQueueState0Record,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaQmQueueState0Entry_Pack(&QmQueueState0Record,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,1);
          status =
    ...
    case FE2K_MEM_QMQUEUESTATE0EN:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,1);
          sbZfCaQmQueueState0EnEntry_Unpack(&QmQueueState0EnRecord,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaQmQueueState0EnEntry_SetField(&QmQueueState0EnRecord,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaQmQueueState0EnEntry_Pack(&QmQueueState0EnRecord,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,1);
          status =
    ...
    case FE2K_MEM_QMQUEUESTATE1:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,1);
          sbZfCaQmQueueState1Entry_Unpack(&QmQueueState1Record,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaQmQueueState1Entry_SetField(&QmQueueState1Record,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaQmQueueState1Entry_Pack(&QmQueueState1Record,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,1);
          status =
    ...
    case FE2K_MEM_QMREPLICATIONSTATE:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,1);
          sbZfCaQmReplicationStateEntry_Unpack(&QmReplicationStateRecord,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaQmReplicationStateEntry_SetField(&QmReplicationStateRecord,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaQmReplicationStateEntry_Pack(&QmReplicationStateRecord,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,1);
          status =
 ...
 /* MCM no PmCounter zframe */
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,2);
          sbZfCaPbCounterEntry_Unpack(&PbCounterRecord,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaPbCounterEntry_SetField(&PbCounterRecord,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaPbCounterEntry_Pack(&PbCounterRecord,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,2);
          status =
    ...
    case FE2K_MEM_PMPROFILEMEMORY:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,2);
          sbZfCaPmProfileMemoryEntry_Unpack(&PmProfileMemoryRecord,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaPmProfileMemoryEntry_SetField(&PmProfileMemoryRecord,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaPmProfileMemoryEntry_Pack(&PmProfileMemoryRecord,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,2);
          status =
    ...
    case FE2K_MEM_SR0COUNTER:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,2);
          sbZfCaSrCounterEntry_Unpack(&SrCounterRecord,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaSrCounterEntry_SetField(&SrCounterRecord,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaSrCounterEntry_Pack(&SrCounterRecord,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,2);
          status =
    ...
    case FE2K_MEM_PPAGGREGATEHASHBITCONFIG:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          ...
          sbZfCaPpAggregateHashBitConfigEntry_Unpack(&PpAggregateHashBitConfigRecord,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaPpAggregateHashBitConfigEntry_SetField(&PpAggregateHashBitConfigRecord,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaPpAggregateHashBitConfigEntry_Pack(&PpAggregateHashBitConfigRecord,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,10);
          status =
    ...
    case FE2K_MEM_PPAGGREGATEHASHBYTECONFIG:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,2);
          sbZfCaPpAggregateHashByteConfigEntry_Unpack(&PpAggregateHashByteConfigRecord,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaPpAggregateHashByteConfigEntry_SetField(&PpAggregateHashByteConfigRecord,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaPpAggregateHashByteConfigEntry_Pack(&PpAggregateHashByteConfigRecord,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,2);
          status =
    ...
    case FE2K_MEM_PPCAMCONFIGURATIONINSTANCE0:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          ...
          sbZfCaPpCamConfigurationInstance0Entry_Unpack(&PpCamConfigurationInstance0Record,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaPpCamConfigurationInstance0Entry_SetField(&PpCamConfigurationInstance0Record,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaPpCamConfigurationInstance0Entry_Pack(&PpCamConfigurationInstance0Record,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,12);
          status =
 ...
 * Instance1 - 4 */
    ...
    case FE2K_MEM_PPCAMRAMCONFIGURATIONINSTANCE0:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,5);
          sbZfCaPpCamRamConfigurationInstance0Entry_Unpack(&PpCamRamConfigurationInstance0Record,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaPpCamRamConfigurationInstance0Entry_SetField(&PpCamRamConfigurationInstance0Record,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaPpCamRamConfigurationInstance0Entry_Pack(&PpCamRamConfigurationInstance0Record,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,5);
          status =
    ...
    case FE2K_MEM_PPHEADERRECORDSIZE:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,1);
          sbZfCaPpHeaderRecordSizeEntry_Unpack(&PpHeaderRecordSizeRecord,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaPpHeaderRecordSizeEntry_SetField(&PpHeaderRecordSizeRecord,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaPpHeaderRecordSizeEntry_Pack(&PpHeaderRecordSizeRecord,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,1);
          status =
    ...
    case FE2K_MEM_PPINITIALQUEUESTATE:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,3);
          sbZfCaPpInitialQueueStateEntry_Unpack(&PpInitialQueueStateRecord,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaPpInitialQueueStateEntry_SetField(&PpInitialQueueStateRecord,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaPpInitialQueueStateEntry_Pack(&PpInitialQueueStateRecord,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,3);
          status =
    ...
    case FE2K_MEM_PPPPOUTHEADERCOPY:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,8);
          sbZfCaPpPpOutHeaderCopyEntry_Unpack(&PpPpOutHeaderCopyRecord,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaPpPpOutHeaderCopyEntry_SetField(&PpPpOutHeaderCopyRecord,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaPpPpOutHeaderCopyEntry_Pack(&PpPpOutHeaderCopyRecord,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,8);
          status =
    ...
    case FE2K_MEM_PPQUEUEPRIORITYGROUP:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,1);
          sbZfCaPpQueuePriorityGroupEntry_Unpack(&PpQueuePriorityGroupRecord,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaPpQueuePriorityGroupEntry_SetField(&PpQueuePriorityGroupRecord,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaPpQueuePriorityGroupEntry_Pack(&PpQueuePriorityGroupRecord,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,1);
          status =
    ...
    case FE2K_MEM_PPRXPORTDATA:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,4);
          sbZfCaPpRxPortDataEntry_Unpack(&PpRxPortDataRecord,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaPpRxPortDataEntry_SetField(&PpRxPortDataRecord,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaPpRxPortDataEntry_Pack(&PpRxPortDataRecord,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,4);
          status =
    ...
    case FE2K_MEM_RC0DATA:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          ...
          sbZfCaRcDataEntry_Unpack(&RcDataRecord,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaRcDataEntry_SetField(&RcDataRecord,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaRcDataEntry_Pack(&RcDataRecord,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,36);
          status =
    ...
    case FE2K_MEM_ST0COUNTER:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,2);
          sbZfCaStCounterEntry_Unpack(&StCounterRecord,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaStCounterEntry_SetField(&StCounterRecord,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaStCounterEntry_Pack(&StCounterRecord,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,2);
          status =
    ...
    case FE2K_MEM_MM0INTERNAL0MEMORY:
        ...
        sbFe2000ImfDriver_Mm0Internal0MemoryRead(  uBaseAddress, 
                                                   ...
                                                   pData1); 
        if (status == SAND_DRV_CA_STATUS_OK) {
 cli_out("Mm0Internal0MemoryRead Read data0=0x%x data1=0x%x\n",*pData0, *pData1);
          sbSwapWords(pData,2);
 cli_out("Mm0Internal0MemoryRead SWAP data0=0x%x data1=0x%x\n",*pData0, *pData1);
          sbZfCaMmInternal0MemoryEntry_Unpack(&MmInternal0MemoryRecord,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaMmInternal0MemoryEntry_SetField(&MmInternal0MemoryRecord,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaMmInternal0MemoryEntry_Pack(&MmInternal0MemoryRecord,(uint8 *)dbuf,sizeof(dbuf));
 cli_out("Mm0Internal0MemoryRead UPdate data0=0x%x data1=0x%x\n",*pData0, *pData1);
          sbSwapWords(pData,2);
 cli_out("Mm0Internal0MemoryRead SwapUpdate data0=0x%x data1=0x%x\n",*pData0, *pData1);
          status =
    ...
    case FE2K_MEM_MM0INTERNAL1MEMORY:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,2);
          sbZfCaMmInternal1MemoryEntry_Unpack(&MmInternal1MemoryRecord,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaMmInternal1MemoryEntry_SetField(&MmInternal1MemoryRecord,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaMmInternal1MemoryEntry_Pack(&MmInternal1MemoryRecord,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,2);
          status =
    ...
    case FE2K_MEM_MM0NARROWPORT0MEMORY:
        ...
        sbFe2000ImfDriver_Mm0NarrowPort0MemoryRead(  uBaseAddress, 
                                                     ...
                                                     pData1); 
        if (status == SAND_DRV_CA_STATUS_OK) {
            cli_out("Mm0NarrowPort0MemoryRead Read data0=0x%x data1=0x%x\n",
                    *pData0, *pData1);
          sbSwapWords(pData,2);
          cli_out("Mm0NarrowPort0MemoryRead Swap data0=0x%x data1=0x%x\n",
                  *pData0, *pData1);
          sbZfCaMmNarrowPort0MemoryEntry_Unpack(&MmNarrowPort0MemoryRecord,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaMmNarrowPort0MemoryEntry_SetField(&MmNarrowPort0MemoryRecord,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaMmNarrowPort0MemoryEntry_Pack(&MmNarrowPort0MemoryRecord,(uint8 *)dbuf,sizeof(dbuf));
          cli_out("Mm0NarrowPort0MemoryRead UPDATdata0=0x%x data1=0x%x\n",
                  *pData0, *pData1);
          sbSwapWords(pData,2);
          cli_out("Mm0NarrowPort0MemoryRead UPDAT SWAPdata0=0x%x data1=0x%x\n",
                  *pData0, *pData1);
          status =
    ...
    case FE2K_MEM_MM0NARROWPORT1MEMORY:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,2);
          sbZfCaMmNarrowPort1MemoryEntry_Unpack(&MmNarrowPort1MemoryRecord,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaMmNarrowPort1MemoryEntry_SetField(&MmNarrowPort1MemoryRecord,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaMmNarrowPort1MemoryEntry_Pack(&MmNarrowPort1MemoryRecord,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,2);
          status =
    ...
    case FE2K_MEM_MM0WIDEPORTMEMORY:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,2);
          sbZfCaMmWidePortMemoryEntry_Unpack(&MmWidePortMemoryRecord,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaMmWidePortMemoryEntry_SetField(&MmWidePortMemoryRecord,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaMmWidePortMemoryEntry_Pack(&MmWidePortMemoryRecord,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,2);
          status =
    ...
    case FE2K_MEM_PB0DATA:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          ...
          sbZfCaPbDataEntry_Unpack(&PbDataRecord,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaPbDataEntry_SetField(&PbDataRecord,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaPbDataEntry_Pack(&PbDataRecord,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,24);
          status =
    ...
    case FE2K_MEM_PTMIRRORINDEX:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,1);
          sbZfCaPtMirrorIndexEntry_Unpack(&PtMirrorIndexRecord,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaPtMirrorIndexEntry_SetField(&PtMirrorIndexRecord,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaPtMirrorIndexEntry_Pack(&PtMirrorIndexRecord,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,1);
          status =
    ...
    case FE2K_MEM_PTPORTTOQUEUEAG0:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,1);
          sbZfCaPtPortToQueueEntry_Unpack(&PtPortToQueueRecord,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaPtPortToQueueEntry_SetField(&PtPortToQueueRecord,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaPtPortToQueueEntry_Pack(&PtPortToQueueRecord,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,1);
          status =
...
/* MCM - ? use PrPortXXX_Unpack - or - us PtPortToQueueEntry_Pack ??? */
    ...
    case FE2K_MEM_PTPTE0MIRRORPORTSTATE:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,2);
          sbZfCaPtPte0MirrorPortStateEntry_Unpack(&PtPte0MirrorPortStateRecord,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaPtPte0MirrorPortStateEntry_SetField(&PtPte0MirrorPortStateRecord,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaPtPte0MirrorPortStateEntry_Pack(&PtPte0MirrorPortStateRecord,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,2);
          status =
    ...
    case FE2K_MEM_PTPTE0PORTSTATE:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,3);
          sbZfCaPtPte0PortStateEntry_Unpack(&PtPte0PortStateRecord,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaPtPte0PortStateEntry_SetField(&PtPte0PortStateRecord,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaPtPte0PortStateEntry_Pack(&PtPte0PortStateRecord,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,3);
          status =
    ...
    case FE2K_MEM_PTPTE1MIRRORPORTSTATE:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,2);
          sbZfCaPtPte1MirrorPortStateEntry_Unpack(&PtPte1MirrorPortStateRecord,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaPtPte1MirrorPortStateEntry_SetField(&PtPte1MirrorPortStateRecord,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaPtPte1MirrorPortStateEntry_Pack(&PtPte1MirrorPortStateRecord,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,2);
          status =
    ...
    case FE2K_MEM_PTPTE1PORTSTATE:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,3);
          sbZfCaPtPte1PortStateEntry_Unpack(&PtPte1PortStateRecord,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaPtPte1PortStateEntry_SetField(&PtPte1PortStateRecord,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaPtPte1PortStateEntry_Pack(&PtPte1PortStateRecord,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,3);
          status =
    ...
    case FE2K_MEM_PTPTE2MIRRORPORTSTATE:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,2);
          sbZfCaPtPte2MirrorPortStateEntry_Unpack(&PtPte2MirrorPortStateRecord,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaPtPte2MirrorPortStateEntry_SetField(&PtPte2MirrorPortStateRecord,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaPtPte2MirrorPortStateEntry_Pack(&PtPte2MirrorPortStateRecord,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,2);
          status =
    ...
    case FE2K_MEM_PTPTE2PORTSTATE:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,3);
          sbZfCaPtPte2PortStateEntry_Unpack(&PtPte2PortStateRecord,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaPtPte2PortStateEntry_SetField(&PtPte2PortStateRecord,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaPtPte2PortStateEntry_Pack(&PtPte2PortStateRecord,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,3);
          status =
    ...
    case FE2K_MEM_PTPTE3MIRRORPORTSTATE:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,2);
          sbZfCaPtPte3MirrorPortStateEntry_Unpack(&PtPte3MirrorPortStateRecord,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaPtPte3MirrorPortStateEntry_SetField(&PtPte3MirrorPortStateRecord,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaPtPte3MirrorPortStateEntry_Pack(&PtPte3MirrorPortStateRecord,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,2);
          status =
    ...
    case FE2K_MEM_PTPTE3PORTSTATE:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,3);
          sbZfCaPtPte3PortStateEntry_Unpack(&PtPte3PortStateRecord,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaPtPte3PortStateEntry_SetField(&PtPte3PortStateRecord,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaPtPte3PortStateEntry_Pack(&PtPte3PortStateRecord,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,3);
          status =
    ...
    case FE2K_MEM_PTPTE4MIRRORPORTSTATE:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,2);
          sbZfCaPtPte4MirrorPortStateEntry_Unpack(&PtPte4MirrorPortStateRecord,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaPtPte4MirrorPortStateEntry_SetField(&PtPte4MirrorPortStateRecord,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaPtPte4MirrorPortStateEntry_Pack(&PtPte4MirrorPortStateRecord,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,2);
          status =
    ...
    case FE2K_MEM_PTPTE4PORTSTATE:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,3);
          sbZfCaPtPte4PortStateEntry_Unpack(&PtPte4PortStateRecord,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaPtPte4PortStateEntry_SetField(&PtPte4PortStateRecord,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaPtPte4PortStateEntry_Pack(&PtPte4PortStateRecord,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,3);
          status =
    ...
    case FE2K_MEM_PTPTE5MIRRORPORTSTATE:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,2);
          sbZfCaPtPte5MirrorPortStateEntry_Unpack(&PtPte5MirrorPortStateRecord,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaPtPte5MirrorPortStateEntry_SetField(&PtPte5MirrorPortStateRecord,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaPtPte5MirrorPortStateEntry_Pack(&PtPte5MirrorPortStateRecord,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,2);
          status =
    ...
    case FE2K_MEM_PTPTE5PORTSTATE:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,3);
          sbZfCaPtPte5PortStateEntry_Unpack(&PtPte5PortStateRecord,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaPtPte5PortStateEntry_SetField(&PtPte5PortStateRecord,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaPtPte5PortStateEntry_Pack(&PtPte5PortStateRecord,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,3);
          status =
    ...
    case FE2K_MEM_PTPTE6MIRRORPORTSTATE:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,2);
          sbZfCaPtPte6MirrorPortStateEntry_Unpack(&PtPte6MirrorPortStateRecord,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaPtPte6MirrorPortStateEntry_SetField(&PtPte6MirrorPortStateRecord,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaPtPte6MirrorPortStateEntry_Pack(&PtPte6MirrorPortStateRecord,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,2);
          status =
    ...
    case FE2K_MEM_PTPTE6PORTSTATE:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,3);
          sbZfCaPtPte6PortStateEntry_Unpack(&PtPte6PortStateRecord,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaPtPte6PortStateEntry_SetField(&PtPte6PortStateRecord,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaPtPte6PortStateEntry_Pack(&PtPte6PortStateRecord,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,3);
          status =
    ...
    case FE2K_MEM_PTQUEUETOPORT:
        ...
        if (status == SAND_DRV_CA_STATUS_OK) {
          sbSwapWords(pData,1);
          sbZfCaPtQueueToPortEntry_Unpack(&PtQueueToPortRecord,(uint8 *)dbuf,sizeof(dbuf));
          status = sbZfCaPtQueueToPortEntry_SetField(&PtQueueToPortRecord,fieldname,val);
          if(status != 0) return CMD_USAGE;
          sbZfCaPtQueueToPortEntry_Pack(&PtQueueToPortRecord,(uint8 *)dbuf,sizeof(dbuf));
          sbSwapWords(pData,1);
          status =
    ...
    if (status != SAND_DRV_CA_STATUS_OK) {
        cli_out("Error %d while reading Fe2000 memory\n",status);
    } 
    return 0;
}
...
sbFe2000MemSetTestAll(int unit, char *tempName, int addr, int val)
{
    ...
    for(x = 0; x < 410; x++) {
       ...
       /* memfield comes in as mem.field, so split memname and field name */
        ...
        cli_out("MemSetField mem= <%s>  field=<%s> addr=%d val=0x%x\n",
                memname,&memfield[3],addr,val);

        memindex = sbFe2000WhichMem(memname);
        if(memindex == -1) {
            cli_out("Error: unrecognized Fe2000 memory: %s\n",memname);
        }
        status = sbFe2000MemSetFieldEntry(unit, memindex,addr,memfield, val);
        if(status == CMD_USAGE) {
            cli_out("Error: unrecognized field <%s> for this Fe2000 memory: %s\n",&memfield[3],memname);
        }
    }
    return status;
}
...
sbFe2000MemSetField(int unit, char *memFieldName, int addr, int val)
{
   ...
   /* memfield comes in as mem.field, so split memname and field name */
    ...
    cli_out("MemSetField unit=%d mem= <%s>  field=<%s> val=0x%x\n",
...
#else
int sbFe2000MemWrite_not_empty;
