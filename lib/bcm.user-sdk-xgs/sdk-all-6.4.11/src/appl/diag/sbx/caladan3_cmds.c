...
cmd_sbx_caladan3_pd_hc(int unit, int queue, int on, int drop)
{
    ...
    for (index=0; index < 16; index++) {
        ...
        SOC_IF_ERROR_RETURN(soc_reg32_set(unit, copyreg[index], index, 0, regval));
        SOC_IF_ERROR_RETURN(soc_reg32_set(unit, copymask[index], index, 0, regval));
    }
...
cmd_sbx_caladan3_ped_show(int unit)
{
    ...
    /* Read data & mask */
    for (index=0; index < 16; index++) {
        SOC_IF_ERROR_RETURN(soc_reg32_get(unit, copyreg[15-index], index, 0, &regval));
        data = regval;
        SOC_IF_ERROR_RETURN(soc_reg32_get(unit, copymask[15-index], index, 0, &regval));
        mask = regval;
...
cmd_sbx_caladan3_ped_check(int unit, int word, int data, int data_mask)
{
    ...
    SOC_IF_ERROR_RETURN(soc_reg32_set(unit, copyreg[15-word], word, 0, data));
    SOC_IF_ERROR_RETURN(soc_reg32_set(unit, copymask[15-word], word, 0, data_mask));
...
cmd_sbx_caladan3_ped_clear(int unit)
{
    ...
    while(npedin-- > 0) {
       SOC_IF_ERROR_RETURN(WRITE_PD_COPY_BUF_CTRLr(unit, regval));
    }
    regval = 0;
