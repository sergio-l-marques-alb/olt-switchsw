...
#if defined(BCM_ARAD_SUPPORT) || defined(BCM_DFE_SUPPORT) || defined(BCM_CALADAN3_SUPPORT)
    ...
    if (rv_comp != 0) { 
        ...
        for (i = index_min; i <= index_max; ++i) {
            if (td_test_data.comp_fn(unit, d, s) != 0) {
                ...
                cli_out("\nMask");
                for (i = 0; i < ent_words; i++) {
                    cli_out("  = 0x%08x", (uint32)BSAND_BYTE_SWAP(td_test_data.datamask[i]));                    
                }
                cli_out("\n");
...
if (SOC_IS_ARAD(unit) ) {
    ...
    if (SOC_IS_SIRIUS(unit) || SOC_IS_CALADAN3(unit) || SOC_IS_TOMAHAWK(unit)) {
