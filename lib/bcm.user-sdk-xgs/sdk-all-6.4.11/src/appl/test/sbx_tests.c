...
sbx_linerate_mem_init(int unit, args_t *a, void **p)
{
  ...
  *p = pDiagsInfo;
  ...
  if (bad_arg) {
    ...
    cli_out("TR 116 MemExists=0x1  will test MM0.N0 only\n");
    cli_out("TR 116 MemExists=0x2  will test MM0.N1 only\n");
    cli_out("TR 116 MemExists=0x4  will test MM0.W only\n");
    cli_out("TR 116 MemExists=0x8  will test MM1.N0 only\n");
    cli_out("TR 116 MemExists=0x10 will test MM1.N1\n");
    cli_out("TR 116 MemExists=0x20 will test MM1.W\n");
    cli_out("Default is to test all memories\n");
    parse_eq_format(&pt);
  }
...
sbx_mem_test(int u, args_t *a, void *p)
{
  ...
  if (b_cMemTest) {
    ...
    if (SOC_INFO(u).chip_type == SOC_INFO_CHIP_TYPE_FE2000) {
#ifdef BCM_FE2000_SUPPORT
...
sbx_snake_init(int unit, args_t *a, void **p)
{
  ...
  if (start_port%2) {
    cli_out("start port needs to be even port#\n");
    goto done;
  }

  if (!(end_port%2)) {
    cli_out("end port needs to be odd port#\n");
    goto done;
  }

  if (end_port <= start_port) {
    cli_out("use end_port > start_port\n");
    goto done;
  }

  if (start_port > 23) {
    cli_out("Test only for 1G ports\n");
    goto done;
  }
