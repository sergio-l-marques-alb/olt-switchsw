...
{   
  ...
  SOC_PPD_RIF_MPLS_LABELS_RANGE_clear(&prm_rif_labels_range);
...
{   
  ...
  SOC_PPD_RIF_MPLS_LABELS_RANGE_print(&prm_rif_labels_range);
...
{   
  uint32 
    ret;   
  SOC_PPD_MPLS_LABEL_RIF_KEY   
    prm_mpls_key;
  SOC_PPD_LIF_ID   
    prm_lif_index=0;
  SOC_PPD_MPLS_TERM_INFO   
    prm_term_info;
  SOC_PPD_RIF_INFO   
    prm_rif_info;
  ...
  unit = soc_ppd_get_default_unit(); 
  SOC_PPD_MPLS_LABEL_RIF_KEY_clear(&prm_mpls_key);
  SOC_PPD_MPLS_TERM_INFO_clear(&prm_term_info);
  SOC_PPD_RIF_INFO_clear(&prm_rif_info);
...
{   
  ...
  SOC_PPD_MPLS_TERM_INFO_print(&prm_term_info);

  SOC_PPD_RIF_INFO_print(&prm_rif_info);
...
/******************************************************************** 
 *  Function handler: ip_tunnel_map_add (section rif)
 ********************************************************************/
int 
  ui_ppd_api_rif_ip_tunnel_map_add(
    CURRENT_LINE *current_line 
  ) 
{   
  uint32 
  ...
  UI_MACROS_INIT_FUNCTION("ui_ppd_api_rif"); 
  soc_sand_proc_name = "soc_ppd_rif_ip_tunnel_map_add"; 
 
  unit = soc_ppd_get_default_unit(); 
  SOC_PPD_RIF_IP_TERM_INFO_clear(&prm_term_info);
  SOC_PPD_RIF_INFO_clear(&prm_rif_info);
  SOC_PPD_RIF_IP_TERM_KEY_clear(&prm_term_key);
  ...
  /* DIP */
  if (!get_val_of(
    current_line,(int *)&match_index,SOC_PARAM_PPD_RIF_IP_TUNNEL_MAP_ADD_IP_TUNNEL_MAP_ADD_DIP_KEY_ID,1,  
    &param_val,VAL_IP,err_msg))                                                                
  {                                                                                                  
    prm_term_key.dip = param_val->value.ip_value;
    prm_term_key.flags |= SOC_PPC_RIF_IP_TERM_FLAG_USE_DIP;
  } 

  /* SIP */
  if (!get_val_of(
    current_line,(int *)&match_index,SOC_PARAM_PPD_RIF_IP_TUNNEL_MAP_ADD_IP_TUNNEL_MAP_ADD_SIP_KEY_ID,1,  
    &param_val,VAL_IP,err_msg))                                                                
  {                                                                                                  
    prm_term_key.sip = param_val->value.ip_value;
    prm_term_key.flags |= SOC_PPC_RIF_IP_TERM_FLAG_USE_SIP;
  } 


  if(!search_param_val_pairs(current_line,&match_index,SOC_PARAM_PPD_RIF_IP_TUNNEL_MAP_ADD_IP_TUNNEL_MAP_ADD_LIF_INDEX_ID,1)) 
  { 
    UI_MACROS_GET_NUMMERIC_VAL(SOC_PARAM_PPD_RIF_IP_TUNNEL_MAP_ADD_IP_TUNNEL_MAP_ADD_LIF_INDEX_ID);  
    UI_MACROS_CHECK_GET_VAL_OF_ERROR; 
    prm_lif_index = (uint32)param_val->value.ulong_value;
  } 

  if(!search_param_val_pairs(current_line,&match_index,SOC_PARAM_PPD_RIF_IP_TUNNEL_MAP_ADD_IP_TUNNEL_MAP_ADD_TERM_INFO_COS_PROFILE_ID,1)) 
  { 
    UI_MACROS_GET_NUMMERIC_VAL(SOC_PARAM_PPD_RIF_IP_TUNNEL_MAP_ADD_IP_TUNNEL_MAP_ADD_TERM_INFO_COS_PROFILE_ID);  
    UI_MACROS_CHECK_GET_VAL_OF_ERROR; 
    prm_term_info.cos_profile = (uint32)param_val->value.ulong_value;
  } 

  if(!search_param_val_pairs(current_line,&match_index,SOC_PARAM_PPD_RIF_IP_TUNNEL_MAP_ADD_IP_TUNNEL_MAP_ADD_TERM_INFO_RIF_ID,1)) 
  { 
    UI_MACROS_GET_NUMMERIC_VAL(SOC_PARAM_PPD_RIF_IP_TUNNEL_MAP_ADD_IP_TUNNEL_MAP_ADD_TERM_INFO_RIF_ID);  
    UI_MACROS_CHECK_GET_VAL_OF_ERROR; 
    prm_term_info.rif = (uint32)param_val->value.ulong_value;
  } 

  if(!search_param_val_pairs(current_line,&match_index,SOC_PARAM_PPD_RIF_IP_TUNNEL_MAP_ADD_IP_TUNNEL_MAP_ADD_RIF_INFO_ROUTING_ENABLERS_BM_ID,1)) 
  { 
    UI_MACROS_GET_NUMMERIC_VAL(SOC_PARAM_PPD_RIF_IP_TUNNEL_MAP_ADD_IP_TUNNEL_MAP_ADD_RIF_INFO_ROUTING_ENABLERS_BM_ID);  
    UI_MACROS_CHECK_GET_VAL_OF_ERROR; 
    prm_rif_info.routing_enablers_bm = (uint32)param_val->value.ulong_value;
  } 

  if(!search_param_val_pairs(current_line,&match_index,SOC_PARAM_PPD_RIF_IP_TUNNEL_MAP_ADD_IP_TUNNEL_MAP_ADD_RIF_INFO_TTL_SCOPE_INDEX_ID,1)) 
  { 
    UI_MACROS_GET_NUMMERIC_VAL(SOC_PARAM_PPD_RIF_IP_TUNNEL_MAP_ADD_IP_TUNNEL_MAP_ADD_RIF_INFO_TTL_SCOPE_INDEX_ID);  
    UI_MACROS_CHECK_GET_VAL_OF_ERROR; 
    prm_rif_info.ttl_scope_index = (uint32)param_val->value.ulong_value;
  } 

  if(!search_param_val_pairs(current_line,&match_index,SOC_PARAM_PPD_RIF_IP_TUNNEL_MAP_ADD_IP_TUNNEL_MAP_ADD_RIF_INFO_COS_PROFILE_ID,1)) 
  { 
    UI_MACROS_GET_NUMMERIC_VAL(SOC_PARAM_PPD_RIF_IP_TUNNEL_MAP_ADD_IP_TUNNEL_MAP_ADD_RIF_INFO_COS_PROFILE_ID);  
    UI_MACROS_CHECK_GET_VAL_OF_ERROR; 
    prm_rif_info.cos_profile = (uint32)param_val->value.ulong_value;
  } 

  if(!search_param_val_pairs(current_line,&match_index,SOC_PARAM_PPD_RIF_IP_TUNNEL_MAP_ADD_IP_TUNNEL_MAP_ADD_RIF_INFO_VRF_ID_ID,1)) 
  { 
    UI_MACROS_GET_NUMMERIC_VAL(SOC_PARAM_PPD_RIF_IP_TUNNEL_MAP_ADD_IP_TUNNEL_MAP_ADD_RIF_INFO_VRF_ID_ID);  
    UI_MACROS_CHECK_GET_VAL_OF_ERROR; 
    prm_rif_info.vrf_id = (uint32)param_val->value.ulong_value;
  } 

  if(!search_param_val_pairs(current_line,&match_index,SOC_PARAM_PPD_RIF_IP_TUNNEL_MAP_ADD_IP_TUNNEL_MAP_ADD_RIF_INFO_UC_RPF_ENABLE_ID,1)) 
  { 
    UI_MACROS_GET_NUMMERIC_VAL(SOC_PARAM_PPD_RIF_IP_TUNNEL_MAP_ADD_IP_TUNNEL_MAP_ADD_RIF_INFO_UC_RPF_ENABLE_ID);  
    UI_MACROS_CHECK_GET_VAL_OF_ERROR; 
    prm_rif_info.uc_rpf_enable = (uint8)param_val->value.ulong_value;
  } 
  ...
  /* Call function */
  ret = soc_ppd_rif_ip_tunnel_term_add(
          unit,
          0,
          &prm_term_key,
          prm_lif_index,
  ...
  if (soc_sand_get_error_code_from_error_word(ret) != SOC_SAND_OK) 
  { 
    send_string_to_screen(" *** soc_ppd_rif_ip_tunnel_term_add - FAIL", TRUE); 
    soc_sand_disp_result_proc(ret, "soc_ppd_rif_ip_tunnel_term_add");   
    goto exit; 
  } 
...
/******************************************************************** 
 *  Function handler: ip_tunnel_map_remove (section rif)
 ********************************************************************/
int 
  ui_ppd_api_rif_ip_tunnel_map_remove(
    CURRENT_LINE *current_line 
  ) 
{   
  uint32 
  ...
  UI_MACROS_INIT_FUNCTION("ui_ppd_api_rif"); 
  soc_sand_proc_name = "soc_ppd_rif_ip_tunnel_map_remove"; 
  ...
  /* Get parameters */ 
  if (!get_val_of(
    current_line,(int *)&match_index,SOC_PARAM_PPD_RIF_IP_TUNNEL_MAP_REMOVE_IP_TUNNEL_MAP_REMOVE_DIP_KEY_ID,1,  
    &param_val,VAL_IP,err_msg))                                                                
  {                                                                                                  
    prm_term_key.dip = param_val->value.ip_value;
    prm_term_key.flags |= SOC_PPC_RIF_IP_TERM_FLAG_USE_DIP;
  ...
  /* DIP */
  if (!get_val_of(
    current_line,(int *)&match_index,SOC_PARAM_PPD_RIF_IP_TUNNEL_MAP_REMOVE_IP_TUNNEL_MAP_REMOVE_SIP_KEY_ID,1,  
    &param_val,VAL_IP,err_msg))                                                                
  {                                                                                                  
    prm_term_key.dip = param_val->value.ip_value;
    prm_term_key.flags |= SOC_PPC_RIF_IP_TERM_FLAG_USE_DIP;
  ...
  /* Call function */
  ret = soc_ppd_rif_ip_tunnel_term_remove(
          unit,
          &prm_term_key,
          &prm_lif_index
        );
  if (soc_sand_get_error_code_from_error_word(ret) != SOC_SAND_OK) 
  { 
    send_string_to_screen(" *** soc_ppd_rif_ip_tunnel_map_remove - FAIL", TRUE); 
    soc_sand_disp_result_proc(ret, "soc_ppd_rif_ip_tunnel_map_remove");   
    goto exit; 
  } 
...
/******************************************************************** 
 *  Function handler: ip_tunnel_map_get (section rif)
 ********************************************************************/
int 
  ui_ppd_api_rif_ip_tunnel_map_get(
    CURRENT_LINE *current_line 
  ) 
{   
  uint32 
  ...
  UI_MACROS_INIT_FUNCTION("ui_ppd_api_rif"); 
  soc_sand_proc_name = "soc_ppd_rif_ip_tunnel_map_get"; 
  ...
  /* DIP */
  if (!get_val_of(
    current_line,(int *)&match_index,SOC_PARAM_PPD_RIF_IP_TUNNEL_MAP_GET_IP_TUNNEL_MAP_GET_DIP_KEY_ID,1,  
    &param_val,VAL_IP,err_msg))                                                                
  {                                                                                                  
    prm_term_key.dip = param_val->value.ip_value;
    prm_term_key.flags |= SOC_PPC_RIF_IP_TERM_FLAG_USE_DIP;
  ...
  /* SIP */
  if (!get_val_of(
    current_line,(int *)&match_index,SOC_PARAM_PPD_RIF_IP_TUNNEL_MAP_GET_IP_TUNNEL_MAP_GET_SIP_KEY_ID,1,  
    &param_val,VAL_IP,err_msg))                                                                
  {                                                                                                  
    prm_term_key.sip = param_val->value.ip_value;
    prm_term_key.flags |= SOC_PPC_RIF_IP_TERM_FLAG_USE_SIP;
  ...
  /* Call function */
  ret = soc_ppd_rif_ip_tunnel_term_get(
          unit,
          &prm_term_key,
          &prm_lif_index,
          &prm_term_info,
  ...
  if (soc_sand_get_error_code_from_error_word(ret) != SOC_SAND_OK) 
  { 
    send_string_to_screen(" *** soc_ppd_rif_ip_tunnel_map_get - FAIL", TRUE); 
    soc_sand_disp_result_proc(ret, "soc_ppd_rif_ip_tunnel_map_get");   
    goto exit; 
  } 

  cli_out("lif_index: %u\n\r",prm_lif_index);

  SOC_PPD_RIF_IP_TERM_INFO_print(&prm_term_info);
...
{   
  ...
  else if(!search_param_val_pairs(current_line,&match_index,SOC_PARAM_PPD_RIF_IP_TUNNEL_MAP_ADD_IP_TUNNEL_MAP_ADD_ID,1)) 
  { 
    ret = ui_ppd_api_rif_ip_tunnel_map_add(current_line); 
  } 
  else if(!search_param_val_pairs(current_line,&match_index,SOC_PARAM_PPD_RIF_IP_TUNNEL_MAP_REMOVE_IP_TUNNEL_MAP_REMOVE_ID,1)) 
  { 
    ret = ui_ppd_api_rif_ip_tunnel_map_remove(current_line); 
  } 
  else if(!search_param_val_pairs(current_line,&match_index,SOC_PARAM_PPD_RIF_IP_TUNNEL_MAP_GET_IP_TUNNEL_MAP_GET_ID,1)) 
  { 
    ret = ui_ppd_api_rif_ip_tunnel_map_get(current_line); 
  } 
