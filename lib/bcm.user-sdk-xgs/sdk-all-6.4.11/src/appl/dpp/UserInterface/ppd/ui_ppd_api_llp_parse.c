/* $Id: ui_ppd_api_llp_parse.c,v 1.7 Broadcom SDK $
...
{   
  ...
  SOC_PPD_LLP_PARSE_TPID_VALUES_clear(&prm_tpid_vals);
...
{   
  ...
  SOC_PPD_LLP_PARSE_TPID_VALUES_print(&prm_tpid_vals);
...
{   
  ...
  unit = soc_ppd_get_default_unit(); 
  SOC_PPD_LLP_PARSE_TPID_PROFILE_INFO_clear(&prm_tpid_profile_info);
...
{   
  ...
  SOC_PPD_LLP_PARSE_TPID_PROFILE_INFO_print(&prm_tpid_profile_info);
...
{   
  ...
  unit = soc_ppd_get_default_unit(); 
  SOC_PPD_LLP_PARSE_INFO_clear(&prm_parse_key);
  SOC_PPD_LLP_PARSE_PACKET_FORMAT_INFO_clear(&prm_format_info);
  ...
  { 
    ...
    UI_MACROS_CHECK_GET_VAL_OF_ERROR; 
    prm_format_info.action_trap_code = (SOC_PPD_TRAP_CODE)param_val->numeric_equivalent;
  } 
  */
  if(!search_param_val_pairs(current_line,&match_index,SOC_PARAM_PPD_LLP_PARSE_PACKET_FORMAT_INFO_SET_PACKET_FORMAT_INFO_SET_FORMAT_INFO_ACTION_TRAP_CODE_ID,1)) 
  { 
    ...
    UI_MACROS_CHECK_GET_VAL_OF_ERROR; 
    prm_format_info.action_trap_code = (SOC_PPD_TRAP_CODE)param_val->value.ulong_value;
  } 
...
{   
  ...
  SOC_PPD_LLP_PARSE_PACKET_FORMAT_INFO_print(&prm_format_info);
