/* $Id: ui_ppd_api_frwrd_fec.c,v 1.12 Broadcom SDK $
...
{   
  ...
  SOC_PPD_FRWRD_FEC_GLBL_INFO_clear(unit, &prm_glbl_info);
...
{   
  ...
  SOC_PPD_FRWRD_FEC_GLBL_INFO_print(unit, &prm_glbl_info);
...
{   
  ...
  unit = soc_ppd_get_default_unit(); 
  SOC_PPD_FRWRD_FEC_ENTRY_INFO_clear(&prm_working_fec);
  SOC_PPD_FRWRD_FEC_ENTRY_INFO_clear(&prm_protect_fec);
  SOC_PPD_FRWRD_FEC_PROTECT_INFO_clear(&prm_protect_info);
 
  prm_protect_fec.rpf_info.rpf_mode = SOC_PPD_FRWRD_FEC_RPF_MODE_UC_LOOSE;
  prm_working_fec.rpf_info.rpf_mode = SOC_PPD_FRWRD_FEC_RPF_MODE_UC_LOOSE;
  ...
  { 
    ...
    UI_MACROS_CHECK_GET_VAL_OF_ERROR; 
    prm_working_fec.trap_info.trap_code = (SOC_PPD_TRAP_CODE)param_val->value.ulong_value;
  } 
  ...
  { 
    ...
    UI_MACROS_CHECK_GET_VAL_OF_ERROR; 
    if(param_val->numeric_equivalent != SOC_PPD_FRWRD_FEC_RPF_MODE_UC_STRICT) {
  ...
  { 
    ...
    UI_MACROS_CHECK_GET_VAL_OF_ERROR; 
    prm_protect_fec.trap_info.trap_code = (SOC_PPD_TRAP_CODE)param_val->value.ulong_value;
  } 
...
{   
  ...
  unit = soc_ppd_get_default_unit(); 
  SOC_PPD_FRWRD_FEC_ENTRY_INFO_clear(&prm_fec_array[0]);

  prm_fec_array[0].rpf_info.rpf_mode = SOC_PPD_FRWRD_FEC_RPF_MODE_UC_LOOSE;
  ...
  { 
    ...
    UI_MACROS_CHECK_GET_VAL_OF_ERROR; 
    prm_fec_array[0].trap_info.trap_code = (SOC_PPD_TRAP_CODE)param_val->value.ulong_value;
  } 
  ...
  {
    sal_memcpy(&prm_fec_array[indx],&prm_fec_array[0],sizeof(SOC_PPD_FRWRD_FEC_ENTRY_INFO));
    if (inc_eep)
    {
...
{   
  ...
  unit = soc_ppd_get_default_unit(); 
  SOC_PPD_FRWRD_FEC_ENTRY_USE_INFO_clear(&prm_fec_entry_info);
  ...
  SOC_PPD_FRWRD_FEC_ENTRY_USE_INFO_print(&prm_fec_entry_info);
...
{   
  ...
  cli_out("protect_type: %s\n\r",SOC_PPD_FRWRD_FEC_PROTECT_TYPE_to_string(prm_protect_type));

  cli_out("working entry: \n\r");
  SOC_PPD_FRWRD_FEC_ENTRY_INFO_print(&prm_working_fec);
  if (prm_protect_type != SOC_PPD_FRWRD_FEC_PROTECT_TYPE_NONE)
  {
    cli_out("protecting entry: \n\r");
    SOC_PPD_FRWRD_FEC_ENTRY_INFO_print(&prm_protect_fec);
    if (prm_protect_type == SOC_PPD_FRWRD_FEC_PROTECT_TYPE_PATH)
    {
      SOC_PPD_FRWRD_FEC_PROTECT_INFO_print(&prm_protect_info);
    }
  }
...
{   
  ...
  {
    SOC_PPD_FRWRD_FEC_ENTRY_INFO_print(&prm_fec_array[indx]);
  }
...
{   
  ...
  unit = soc_ppd_get_default_unit(); 
  SOC_PPD_FRWRD_FEC_MATCH_RULE_clear(&prm_rule);
  soc_sand_SAND_TABLE_BLOCK_RANGE_clear(&prm_block_range);
 
  prm_rule.type = SOC_PPD_FRWRD_FEC_MATCH_RULE_TYPE_ALL;
  ...
  )
{
  ...
  unit = soc_ppd_get_default_unit(); 
  SOC_PPD_FRWRD_FEC_ENTRY_ACCESSED_INFO_clear(&prm_accessed_info);
  ...
  )
{
  ...
  SOC_PPD_FRWRD_FEC_ENTRY_ACCESSED_INFO_print(&prm_accessed_info);
...
{   
  uint32 
    ret;   
  SOC_PPD_FRWRD_FEC_ECMP_HASH_GLOBAL_INFO   
    prm_glbl_hash_info;
  ...
  unit = soc_ppd_get_default_unit(); 
  SOC_PPD_FRWRD_FEC_ECMP_HASH_GLOBAL_INFO_clear(&prm_glbl_hash_info);
...
{   
  ...
  SOC_PPD_FRWRD_FEC_ECMP_HASH_GLOBAL_INFO_print(&prm_glbl_hash_info);
...
{   
  ...
  unit = soc_ppd_get_default_unit(); 
  SOC_PPD_FRWRD_FEC_ECMP_INFO_clear(&prm_ecmp_info);
...
{   
  ...
  SOC_PPD_FRWRD_FEC_ECMP_INFO_print(&prm_ecmp_info);
...
{   
  ...
  SOC_PPD_PORT   
    prm_port_ndx;
  SOC_PPD_FRWRD_FEC_ECMP_HASH_PORT_INFO   
    prm_port_hash_info;
  ...
  unit = soc_ppd_get_default_unit(); 
  SOC_PPD_FRWRD_FEC_ECMP_HASH_PORT_INFO_clear(&prm_port_hash_info);
...
{   
  ...
  SOC_PPD_FRWRD_FEC_ECMP_HASH_PORT_INFO_print(&prm_port_hash_info);
