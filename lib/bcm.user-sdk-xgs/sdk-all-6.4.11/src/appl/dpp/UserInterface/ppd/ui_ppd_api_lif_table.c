/* $Id: ui_ppd_api_lif_table.c,v 1.6 Broadcom SDK $
...
{   
  uint32 
    ret;   
  SOC_PPD_LIF_TBL_TRAVERSE_MATCH_RULE   
    prm_rule;
  ...
  SOC_PPD_LIF_TBL_TRAVERSE_MATCH_RULE_clear(&prm_rule);
  SOC_SAND_TABLE_BLOCK_RANGE_clear(&prm_block_range);
  SOC_PPD_LIF_ENTRY_INFO_clear(&prm_entries_array);
  ...
  SOC_PPD_LIF_ENTRY_INFO_print(&prm_entries_array);
...
{   
  ...
  unit = soc_ppd_get_default_unit(); 
  SOC_PPD_LIF_ENTRY_INFO_clear(&prm_lif_entry_info);
  ...
  SOC_PPD_LIF_ENTRY_INFO_print(&prm_lif_entry_info);
...
{   
  ...
  { 
    ...
    UI_MACROS_CHECK_GET_VAL_OF_ERROR; 
    prm_lif_entry_info.value.isid.default_frwrd.default_forwarding.additional_info.trap_info.action_profile.trap_code = (SOC_PPD_TRAP_CODE)param_val->numeric_equivalent;
  } 
  ...
  { 
    ...
    UI_MACROS_CHECK_GET_VAL_OF_ERROR; 
    prm_lif_entry_info.value.pwe.default_frwrd.default_forwarding.additional_info.trap_info.action_profile.trap_code = (SOC_PPD_TRAP_CODE)param_val->numeric_equivalent;
  } 
  ...
  { 
    ...
    UI_MACROS_CHECK_GET_VAL_OF_ERROR; 
    prm_lif_entry_info.value.ac.default_frwrd.default_forwarding.additional_info.trap_info.action_profile.trap_code = (SOC_PPD_TRAP_CODE)param_val->numeric_equivalent;
  } 
