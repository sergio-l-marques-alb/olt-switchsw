/* $Id: ui_ppd_api_frwrd_mact.c,v 1.18 Broadcom SDK $
                                ...
                                )
{
  uint32
    ret;
  SOC_PPD_FRWRD_MACT_ADD_TYPE
    prm_add_type = SOC_PPD_FRWRD_MACT_ADD_TYPE_INSERT;
  ...
  SOC_PPD_FRWRD_MACT_ENTRY_KEY_clear(&prm_mac_entry_key);
  SOC_PPD_FRWRD_MACT_ENTRY_VALUE_clear(&prm_mac_entry_value);
    ...
    &param_val,VAL_TEXT,err_msg))
  {
    ...
    prm_mac_entry_key.key_type = SOC_PPD_FRWRD_MACT_KEY_TYPE_MAC_ADDR;
  }
  else if(!search_param_val_pairs(current_line,&match_index,SOC_PARAM_PPD_FRWRD_MACT_ENTRY_ADD_ENTRY_ADD_MAC_ENTRY_KEY_DIP_ID,1))
  {
    ...
    prm_mac_entry_key.key_type = SOC_PPD_FRWRD_MACT_KEY_TYPE_IPV4_MC;
  }
  else
  {
  ...
  if(!search_param_val_pairs(current_line,&match_index,SOC_PARAM_PPD_FRWRD_MACT_ENTRY_ADD_ENTRY_ADD_MAC_ENTRY_KEY_FID_ID,1))
  {
    ...
    UI_MACROS_CHECK_GET_VAL_OF_ERROR;
    if (prm_mac_entry_key.key_type == SOC_PPD_FRWRD_MACT_KEY_TYPE_IPV4_MC)
    {
                                ...
                                )
{
  ...
  SOC_PPD_FRWRD_MACT_ENTRY_VALUE_print(&prm_mac_entry_value);
                               ...
                               )
{
  uint32
    ret;
  SOC_PPD_FRWRD_MACT_TRAVERSE_MATCH_RULE
    prm_rule;
  SOC_PPD_FRWRD_MACT_TRAVERSE_ACTION
    prm_action;
  uint8
  ...
  unit = soc_ppd_get_default_unit();
  SOC_PPD_FRWRD_MACT_TRAVERSE_MATCH_RULE_clear(&prm_rule);
  SOC_PPD_FRWRD_MACT_TRAVERSE_ACTION_clear(&prm_action);

  /* Get parameters */
  prm_rule.key_type = SOC_PPD_FRWRD_MACT_KEY_TYPE_MAC_ADDR;
  prm_action.updated_val.frwrd_info.forward_decision.type = SOC_PPD_FRWRD_DECISION_TYPE_UC_PORT;
  prm_rule.value_rule.val.frwrd_info.forward_decision.type = SOC_PPD_FRWRD_DECISION_TYPE_UC_PORT;
  ...
  if(!search_param_val_pairs(current_line,&match_index,SOC_PARAM_PPD_FRWRD_MACT_TRAVERSE_TRAVERSE_RULE_VALUE_RULE_VAL_AGING_INFO_IS_DYNAMIC_ID,1))
  {
    ...
    prm_rule.value_rule.compare_mask |= SOC_PPD_FRWRD_MACT_TRAVERSE_MATCH_SELECT_IS_DYNAMIC;
  }
  ...
  if(!search_param_val_pairs(current_line,&match_index,SOC_PARAM_PPD_FRWRD_MACT_TRAVERSE_TRAVERSE_RULE_VALUE_RULE_VAL_FRWRD_INFO_DROP_WHEN_SA_IS_KNOWN_ID,1))
  {
    ...
    prm_rule.value_rule.compare_mask |= SOC_PPD_FRWRD_MACT_TRAVERSE_MATCH_SELECT_DROP_SA;
  }
  ...
  if(!search_param_val_pairs(current_line,&match_index,SOC_PARAM_PPD_FRWRD_MACT_TRAVERSE_TRAVERSE_RULE_VALUE_RULE_VAL_ACCESSED_ID,1))
  {
    ...
    prm_rule.value_rule.compare_mask |= SOC_PPD_FRWRD_MACT_TRAVERSE_MATCH_SELECT_ACCESSED;
  }
  ret = ui_ppd_frwrd_decision_set(
          current_line,
          1,
  ...
  {
    prm_rule.value_rule.compare_mask |= SOC_PPD_FRWRD_MACT_TRAVERSE_MATCH_SELECT_DEST_VAL;
  }
  ...
  {
    prm_rule.value_rule.compare_mask |= SOC_PPD_FRWRD_MACT_TRAVERSE_MATCH_SELECT_ADDITIONAL_INFO;
  }


  if ((prm_rule.value_rule.compare_mask & SOC_PPD_FRWRD_MACT_TRAVERSE_MATCH_SELECT_DEST_VAL) == 0
    && prm_rule.value_rule.val.frwrd_info.forward_decision.type != SOC_PPD_FRWRD_DECISION_TYPE_TRAP)
  {
    prm_rule.value_rule.val.frwrd_info.forward_decision.type = SOC_PPD_FRWRD_DECISION_TYPE_UC_PORT;
  }

  if (prm_rule.value_rule.compare_mask == SOC_PPD_FRWRD_MACT_TRAVERSE_MATCH_SELECT_ADDITIONAL_INFO && 
      prm_rule.value_rule.val.frwrd_info.forward_decision.additional_info.eei.type != SOC_PPD_EEI_TYPE_EMPTY) {
      prm_rule.value_rule.compare_mask |= SOC_PPD_FRWRD_MACT_TRAVERSE_MATCH_SELECT_DEST_TYPE;

      if (!search_param_val_pairs(current_line,&match_index,SOC_PARAM_PPD_FRWRD_DECISION_FD_LABEL_ID,1)) {
          prm_rule.value_rule.val.frwrd_info.forward_decision.type = SOC_PPD_FRWRD_DECISION_TYPE_FEC;
      }
      else{
          prm_rule.value_rule.val.frwrd_info.forward_decision.type = SOC_PPD_FRWRD_DECISION_TYPE_MC;
      }
  }
  ...
  if(!search_param_val_pairs(current_line,&match_index,SOC_PARAM_PPD_FRWRD_MACT_TRAVERSE_TRAVERSE_ACTION_UPDATED_VAL_AGING_INFO_IS_DYNAMIC_ID,1))
  {
    ...
    prm_action.update_mask |= SOC_PPD_FRWRD_MACT_TRAVERSE_MATCH_SELECT_IS_DYNAMIC;
  }
  ...
  if(!search_param_val_pairs(current_line,&match_index,SOC_PARAM_PPD_FRWRD_MACT_TRAVERSE_TRAVERSE_ACTION_UPDATED_VAL_FRWRD_INFO_DROP_WHEN_SA_IS_KNOWN_ID,1))
  {
    ...
    prm_action.update_mask |= SOC_PPD_FRWRD_MACT_TRAVERSE_MATCH_SELECT_DROP_SA;
  }
  ...
  {
    prm_action.update_mask |= SOC_PPD_FRWRD_MACT_TRAVERSE_MATCH_SELECT_DEST_VAL;
  }
  ...
  {
    prm_action.update_mask |= SOC_PPD_FRWRD_MACT_TRAVERSE_MATCH_SELECT_ADDITIONAL_INFO;
  }

  /* just as MACT has no drop */
  if ((prm_action.update_mask & SOC_PPD_FRWRD_MACT_TRAVERSE_MATCH_SELECT_DEST_VAL) == 0
     && prm_action.updated_val.frwrd_info.forward_decision.type != SOC_PPD_FRWRD_DECISION_TYPE_TRAP)
  {
    prm_action.updated_val.frwrd_info.forward_decision.type = SOC_PPD_FRWRD_DECISION_TYPE_UC_PORT;
  }
  ...
  /* prevent CPU stuck
  if (prm_action.type == SOC_PPD_FRWRD_MACT_TRAVERSE_ACTION_TYPE_RETRIEVE)
  {
  ...
  { 
    ...
    UI_MACROS_CHECK_GET_VAL_OF_ERROR; 
    if (prm_rule.key_type == SOC_PPD_FRWRD_MACT_KEY_TYPE_IPV4_MC)
    {
  ...
  )
{
  uint32
    ret;
  SOC_PPD_FRWRD_MACT_TRAVERSE_STATUS_INFO
    prm_status;
  ...
  unit = soc_ppd_get_default_unit();
  SOC_PPD_FRWRD_MACT_TRAVERSE_STATUS_INFO_clear(&prm_status);
  ...
  SOC_PPD_FRWRD_MACT_TRAVERSE_STATUS_INFO_print(&prm_status);
                                ...
                                )
{
  ...
  { 
    ...
    if (prm_rule.key_type == SOC_PPD_FRWRD_MACT_KEY_TYPE_IPV4_MC)
    {
      ...
      if(SOC_IS_PETRAB(unit)){
          prm_rule.key_rule.ipv4_mc.fid_mask = 0x3FFF;
      }
    ...
    else
    {
      prm_rule.key_rule.mac.fid = (uint32)param_val->value.ulong_value;
      if(SOC_IS_PETRAB(unit)){
          prm_rule.key_rule.mac.fid_mask = 0x3FFF;
      }
      else if(SOC_IS_ARAD(unit)){
          prm_rule.key_rule.mac.fid_mask = 0x7FFF;
      }
  ...
  prm_action.type = SOC_PPD_FRWRD_MACT_TRAVERSE_ACTION_TYPE_COUNT;
  /* print total */
  ...
  prm_mact_keys = (SOC_PPD_FRWRD_MACT_ENTRY_KEY*)sal_alloc(sizeof(SOC_PPD_FRWRD_MACT_ENTRY_KEY) * prm_block_range.entries_to_act, "prm_mact_keys");
  ...
  prm_mact_vals = (SOC_PPD_FRWRD_MACT_ENTRY_VALUE*)sal_alloc(sizeof(SOC_PPD_FRWRD_MACT_ENTRY_VALUE) * prm_block_range.entries_to_act, "prm_mact_vals");
  ...
  while(!SOC_SAND_TBL_ITER_IS_END(&prm_block_range.iter)) {
      ...
      ret = soc_ppd_frwrd_mact_get_block(
        unit,
        &prm_rule,
        SOC_PPD_FRWRD_MACT_TABLE_SW_HW,
        &prm_block_range,
        prm_mact_keys,
        prm_mact_vals,
        &prm_nof_entries
