  ...
  )
{
  ...
  else if(!search_param_val_pairs(current_line,&match_index,SOC_PARAM_PPD_FRWRD_IPV4_TEST_ID,1)) 
  { 
    ret = ui_ppd_frwrd_ipv4_test(current_line); 
  } 
  else if(!search_param_val_pairs(current_line,&match_index,SOC_PARAM_PPD_FRWRD_IPV4_TEST2_ID,1)) 
  { 
    ret = ui_ppd_frwrd_ipv4_test_pcp(current_line); 
  } 
