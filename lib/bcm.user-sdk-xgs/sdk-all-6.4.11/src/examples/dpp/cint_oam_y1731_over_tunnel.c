...
int l3_interface_init(int unit, int in_sysport, int out_sysport, int * _next_hop_mac_encap_id, int * encap_id_l2, int is_mpls){
    ...
    if (rv != BCM_E_NONE) {
        printf("Error, create egress object 1, out_sysport=%d, in unit %d\n", out_sysport, unit);
        return rv;
    }
    if(verbose >= 1) {
    ...
    *_next_hop_mac_encap_id = encap_id[1];
    if (rv != BCM_E_NONE) {
        printf("Error, create egress object 2, out_sysport=%d, in unit %d\n", out_sysport, unit);
        return rv;
    }
    if(verbose >= 1) {
        printf("created FEC-id =0x%08x, in unit %d\n", fec[1], unit);
...
int oam_example_over_tunnel(int unit,  bcm_oam_endpoint_type_t type, int use_mpls_out_gport) {
  ...
  */
  ...
  if(ingress_only_flag ==1){
    mep_acc_info.flags2= BCM_OAM_ENDPOINT_FLAGS2_MPLS_INGRESS_ONLY;
  }
  
  if(egress_only_flag ==1){
    mep_acc_info.flags2= BCM_OAM_ENDPOINT_FLAGS2_MPLS_EGRESS_ONLY;
  }
