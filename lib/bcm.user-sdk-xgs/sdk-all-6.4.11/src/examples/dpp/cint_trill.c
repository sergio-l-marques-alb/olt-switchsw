...
add_remote_rbridge(int unit, bcm_mac_t remote_mac_rb, bcm_vlan_t vsi, int port, int nickname, int flags, bcm_trill_port_t *trill_port) {
    ...
    if (!trill_bridge_mode) {
       ...
       /* Add remote RBridge
        ...
        /* Add LL-EEP:
           - Allocates an EEDB entry, and configures MAC + Out-VSI. */
        bcm_l3_egress_t_init(&ll_eep);
        sal_memcpy(ll_eep.mac_addr, remote_mac_rb, 6);
        ll_eep.vlan     = vsi; /* VSI < 4K */

        rv = bcm_l3_egress_create(unit, BCM_L3_EGRESS_ONLY, &ll_eep, &ll_eep_if_id);
        if (rv != BCM_E_NONE) {
            printf("Error, in bcm_l3_egress_create, \n");
            return rv;
        }
        ...
        trill_eep.egress_if  =  ll_eep.encap_id;
       ...
       /* Build Trill FEC:
           - Allocates a FEC, and point it to the allocated EEDB entry. */
        bcm_l3_egress_t_init(trill_fec);
        trill_gport_to_l3_intf(trill_eep.trill_port_id, &(trill_fec.intf)); /* fec point to trill eedb */
        trill_fec.port     = port;

        rv = bcm_l3_egress_create(unit, BCM_L3_INGRESS_ONLY, &trill_fec, &trill_fec_if_id);
        if (rv != BCM_E_NONE) {
            printf("Error, in bcm_l3_egress_create, \n");
            return rv;
        }
    ...
    if (is_device_or_above(unit, JERICHO)) {
        ...
        BCM_GPORT_TUNNEL_ID_SET(tunnel_gport, trill_bridge_mode ? ll_eep.encap_id : egr_obj_rb.encap_id); 
...
add_transit_rbridge(int unit, int port, int nickname, bcm_gport_t port_id) {
    ...
    /* Add Transit Trill FEC:
        - Allocates a FEC, and point it to the allocated EEDB entry. */
    bcm_l3_egress_t_init(&trill_fec);
    trill_fec.intf = trill_eep.egress_if;
    trill_fec.port = port;
