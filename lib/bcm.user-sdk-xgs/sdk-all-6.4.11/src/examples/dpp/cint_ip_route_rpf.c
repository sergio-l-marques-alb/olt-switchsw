...
int bcm_l3_ip6_mc_rpf(int unit, int port0, int port1, int port2, int port3) {
    ...
    for(i = 0; i < explicit_count + sip_count; i++) {
        ...
        sal_memcpy(l3egr.mac_addr, mac_l3_engress_add, 6);
        l3egr.intf = 0;
        l3egr.flags |= BCM_L3_RPF;

        if(i < explicit_count) {
            /*Set the first entry with an incorrect in-rif */
            l3egr.intf =  l3if[i].l3a_intf_id + (1-i)*5;
            l3egr.flags |=  BCM_L3_IPMC;
        }

        l3egr.port  = mc_gport_l;
        rc = bcm_l3_egress_create(unit, 0, &l3egr, &frwdl3egid);
    ...
    sal_memcpy(l3egr.mac_addr, mac_l3_engress_add, 6);
    l3egr.port  = port1;
    l3egr.intf = l3if[3].l3a_intf_id;/*Must have the in_rif value of the incoming MC packet*/
    printf("SIP search inrif is 0x%x",l3egr.intf);
    rc = bcm_l3_egress_create(unit, 0, &l3egr, &frwdl3egid);
    if (rc != BCM_E_NONE) {
      printf("Error, bcm_l3_egress_create \n");
      return rc;
    ...
    /*This is the host entry where the SIP is verified to be exists as part of SIP based RPF*/
    bcm_l3_host_t l3host;
    bcm_l3_host_t_init(&l3host);
    l3host.l3a_flags =  BCM_L3_IP6 | BCM_L3_RPF;
    sal_memcpy(l3host.l3a_ip6_addr,supported_sip,16);
    l3host.l3a_vrf = vlan_bais_in + (3*vlan_offset_in);;
    /* dest is FEC + OutLif */
    l3host.l3a_intf = frwdl3egid;     /* fec */
    l3host.encap_id = l3egr.encap_id; /* outlif */
