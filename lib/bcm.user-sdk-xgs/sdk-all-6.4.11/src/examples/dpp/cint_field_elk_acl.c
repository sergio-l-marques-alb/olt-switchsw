...
*   set the pmf to drop packets for a spesific hit bit options (0-3) and a specific application
*/ 
 int configure_pmf_action_by_hit_value_example(int unit, int hit_bit, uint64 hit_value, uint64 hit_mask, bcm_field_AppType_t app_type, bcm_field_action_t action_type, int param0, int param1) {
...
int acl_kbp_check_entry_priority_set_configuration(int unit, int size_change) {
	...
	if (size_change == 0) {
		rv = configure_pmf_action_by_hit_value_example(unit, HIT_BIT_1_QUAL, val, mask, bcmFieldAppTypeL2, bcmFieldActionDrop, 0, 0);
		if (rv) {
			printf("configure_pmf_action_by_hit_value_example bit 1 error\n");
			return  rv;
		}
	}else{
		rv = configure_pmf_action_by_hit_value_example(unit, HIT_BIT_3_QUAL, val, mask, bcmFieldAppTypeL2, bcmFieldActionDrop, 0, 0);
		if (rv) {
