...
int oam_lb_tst_sat_start(int unit, int gtf_id)
 {
     ...
     bw1.rate = 10;/*gtf_bw_cir_rate + gtf_bw_eir_rate;*/
     ...
     bw1.rate = 20;/*gtf_bw_cir_rate + gtf_bw_eir_rate;*/
  ...
  * @return int 
  */
 int oam_lb_tst_sat_run(int unit, int port1, int port2, int port3, int lb_tst_flag, int mep_is_up)
{
    bcm_error_t rv;
    bcm_oam_endpoint_info_t acc_endpoint; 
    bcm_oam_endpoint_info_t remote_endpoint;
    ...
    tst_tx_ptr.endpoint_id=tst_rx_ptr.endpoint_id=lb.id = 4100;/*tst &lb test down mep*/
