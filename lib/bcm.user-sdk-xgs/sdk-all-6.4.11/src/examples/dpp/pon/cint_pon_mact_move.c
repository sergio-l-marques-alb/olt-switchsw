/* $Id: cint_pon_mact_move.c,v 1.1 Broadcom SDK $
...
int pon_app_init(int unit, bcm_port_t port_pon_1, bcm_port_t port_pon_2, bcm_port_t port_nni)
{
    ...
    if (rv != BCM_E_NONE) {
        printf("Error, in bcm_vlan_control_port_set %s\n", pon_port_2, bcm_errmsg(rv));
    }

    /* Enable the L2 MAC move on pon_port_1 */
    rv = bcm_port_control_set(unit, pon_port_1, bcmPortControlL2Move, 1);
    if (rv != BCM_E_NONE) {
        printf("Error, in bcm_port_control_set %s\n", pon_port_1, bcm_errmsg(rv));
    }

    /* Enable the L2 MAC move on pon_port_2 */
    rv = bcm_port_control_set(unit, pon_port_2, bcmPortControlL2Move, 1);
    if (rv != BCM_E_NONE) {
        printf("Error, in bcm_port_control_set %s\n", pon_port_2, bcm_errmsg(rv));
    }
 ...
 */
int pon_mact_learning_setup(int unit, int dsp_parsing, int port, int aging_time)
{
    int rv = BCM_E_NONE;
    
    if (dsp_parsing) {
        rv = pon_mact_learning_msg_set(unit, port);
        if (rv != BCM_E_NONE) {
            printf("Error, pon_tunnel_mact_learning_msg_set failed $rv\n");
            return rv;
        }
    }
    rv = pon_mact_aging_time_set(unit, aging_time);
    if (rv != BCM_E_NONE) {
        printf("Error, pon_tunnel_mact_aging_time_set failed $rv\n");
        return rv;
    }
    rv = pon_mact_learning_mode_set(unit, dsp_parsing);
    if (rv != BCM_E_NONE) {
        printf("Error, pon_tunnel_mact_learning_mode_set failed $rv\n");
        return rv;
    }
...
int pon_mact_event_handle_register(int unit) {
    int rv = BCM_E_NONE;
    rv = bcm_l2_addr_register(unit,pon_mac_limit_mac_move_event_handle, &l2_addr);
    if (rv != BCM_E_NONE) {
