...
bcm_field_group_traverse_cb  field_group_destroy_callback(int unit, 
    bcm_field_group_t group, 
    void *user_data) {
    bcm_error_t rv = BCM_E_NONE;
    rv = bcm_field_group_remove(unit, group);
    if (rv != BCM_E_NONE) {
        printf("Error: bcm_field_group_remove failed (%s)\n", bcm_errmsg(rv));
    }

    rv = bcm_field_group_destroy(unit, group);
    if (rv != BCM_E_NONE) {
        printf("Error: bcm_field_group_destroy failed (%s)\n", bcm_errmsg(rv));
    }
}
...
bcm_error_t field_groups_destroy_all(int unit) {
    bcm_error_t rv = BCM_E_NONE;
    int ud = 0;
    rv = bcm_field_entry_destroy_all(unit);
    if (rv != BCM_E_NONE) {
        printf("Error: bcm_field_entry_destroy_all failed(%s)\n", bcm_errmsg(rv));
        return rv;
    }
