...
int tscf_phy_interface_config_set(const phymod_phy_access_t* phy, uint32_t flags, const phymod_phy_inf_config_t* config)
{
    ...
    firmware_lane_config.Cl72AutoPolEn = 1; 
...
{
    uint16_t baseP0, baseP1, baseP2, nextP1, nextP2;
    PHYMOD_IF_ERR_RETURN
       (tefmod_autoneg_lp_status_get(&phy->access, &baseP0, &baseP1, &baseP2, &nextP1, &nextP2));
