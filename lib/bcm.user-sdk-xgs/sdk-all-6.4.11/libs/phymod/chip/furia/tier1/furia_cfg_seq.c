                       ...
                       const phymod_phy_inf_config_t* config)
{
    ...
    if (pkg_side == SIDE_B) {
        ...
        if ((config_copy.interface_type == phymodInterfaceKR) ||
    ...
    if (FURIA_IS_SIMPLEX(chip_id)) {
        if(config_copy.data_rate == SPEED_40G || config_copy.data_rate == SPEED_42G || config_copy.data_rate == SPEED_11G) {
             ...
             config_copy.data_rate == SPEED_50G || config_copy.data_rate == SPEED_53G)) 
    {
        ...
        lane_map = FURIA_IS_DUPLEX(chip_id) ? 0xF : 0xFF;
    } else if ((link_type == FURIA_LINKTYPE_7 || link_type == FURIA_LINKTYPE_5) &&
            (config_copy.data_rate == SPEED_25G ||
    ...
    if ((curr_pll_mode[0] != pll_mode) ||
        (curr_pll_mode[1] != pll_mode)) {
        PHYMOD_IF_ERR_RETURN(
...
STATIC uint16_t _furia_get_phy_type(phymod_interface_t intf, uint32_t speed)
{
    ...
    switch (intf) {
        ...
        case phymodInterfaceXLAUI:
            if (((speed == SPEED_25G) || (speed == SPEED_27G) || (speed == SPEED_50G) || (speed == SPEED_53G)) && ((intf == phymodInterfaceCR) || (intf == phymodInterfaceCR2))) {
                phy_type = 0x1; 
            } else {
                phy_type = 0x4; 
            }
...
STATIC uint16_t _furia_get_link_type(phymod_interface_t intf, uint32_t speed)
{
    ...
    switch (intf) {
        ...
        break;
        case phymodInterfaceKR:
            if (speed == SPEED_10G || speed == SPEED_11G) {
                link_type = 0x2;
            } else if (speed == SPEED_20G || speed == SPEED_21G) {
                link_type = 0x6;
            } else {
                link_type = 0x8;
            }    
