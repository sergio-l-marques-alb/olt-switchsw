...
#define CGM_ITM_VSQ_WORDS_SIZE_MAX(unit)         (SOC_DPP_CONFIG(unit)->jer->dbuffs.dbuffs_bdries.fbc_mnmc_0.size \
                                                    * (SOC_DPP_CONFIG(unit)->arad->init.dram.dbuff_size / CGM_ITM_VSQ_WORDS_RESOLUTION))
...
/*
 * This enum is used to define each bit in the CGM_<..>_RJCT_MASKm and is corrently internal, 
...
STATIC uint32
  qax_itm_vsq_WRED_QT_DP_INFO_to_WRED_TBL_DATA(
...
STATIC uint32
  qax_itm_vsq_WRED_TBL_DATA_to_WRED_QT_DP_INFO(
     SOC_SAND_IN int                                                     unit,
...
STATIC int
  qax_itm_vsq_rjct_fadt_set(
    ...
    uint32                           entry_offset,
    SOC_TMC_ITM_VSQ_FADT_INFO        fadt_info,
    SOC_TMC_ITM_VSQ_FADT_INFO        *exact_fadt_info
  );
  ...
  )
{
    ...
    BCM_DPP_CORES_ITER(BCM_CORE_ALL, core_index) {
        ...
        /* Words */
        guaranteed_resources.guaranteed[SOC_TMC_INGRESS_THRESHOLD_TOTAL_BYTES].total = 
            (core_index == 0 ? 
             SOC_DPP_CONFIG(unit)->jer->dbuffs.dbuffs_bdries.fbc_mnmc_0.size  /* Number of buffers */
             : SOC_DPP_CONFIG(unit)->jer->dbuffs.dbuffs_bdries.fbc_mnmc_1.size)
...
_qax_itm_hw_data_to_wred_info (
        ...
        SOC_SAND_INOUT ARAD_ITM_WRED_QT_DP_INFO                           *wred_param
  )
{
    uint32
  ...
  )
{
     ...
     * min_avrg_th
     */
    min_avrg_th_16_byte = SOC_SAND_DIV_ROUND_UP(wred_param->min_avrg_th,SOC_TMC_ITM_WRED_GRANULARITY);
    ...
    *c3 = (wred_param->max_avrg_th == 0 ? 0 : soc_sand_log2_round_up(calc));
    ...
    *c1 = soc_sand_log2_round_down(calc);
    ...
    { /* This limit from above is HW restriction */
        max_val_c1 = SOC_SAND_DIV_ROUND_DOWN(0xFFFFFFFF, min_avrg_th_16_byte);
  ...
  )
{
    ...
    _qax_itm_wred_info_to_hw_data(unit, info, exact_info->min_avrg_th, exact_info->max_avrg_th, &c1, &c2, &c3);
    ...
    _qax_itm_hw_data_to_wred_info (unit, c1, c2, c3, exact_info);
  ...
  )
{
     ...
     * min_avrg_th
     */
    _qax_itm_mantissa_exp_field_get(unit, &voq_wred_ecn_min_th_mant_exp[is_ecn], &data, &info->min_avrg_th);
    
    /*
     * max_avrg_th
     */
    _qax_itm_mantissa_exp_field_get(unit, &voq_wred_ecn_max_th_mant_exp[is_ecn], &data, &info->max_avrg_th);
    ...
    _qax_itm_hw_data_to_wred_info (unit, c1, c2, c3, info);
  ...
  )
{
    ...
    if (pg_ndx >= SOC_TMC_ITM_VSQ_GROUPF_SZE(unit)) {
        SOCDNX_IF_ERR_EXIT(SOC_E_PARAM);
    }
        ...
        SOC_SAND_OUT SOC_TMC_ITM_WRED_QT_DP_INFO *info) 
{
    ...
    if (vsq_group_ndx == SOC_TMC_ITM_VSQ_GROUP_SRC_PORT) {
        ...
        entry_offset += pool_id * (SOC_TMC_ITM_VSQ_QT_RT_CLS_MAX(unit) * SOC_TMC_NOF_DROP_PRECEDENCE);
    }
    ...
    res = qax_itm_vsq_WRED_TBL_DATA_to_WRED_QT_DP_INFO(unit, &tbl_data, info);
    SOCDNX_IF_ERR_EXIT(res);
        ...
        SOC_SAND_OUT SOC_TMC_ITM_WRED_QT_DP_INFO *exact_info)
{
    ...
    res = qax_itm_vsq_WRED_QT_DP_INFO_to_WRED_TBL_DATA(unit, info, &tbl_data);
    SOCDNX_IF_ERR_EXIT(res);

    res = qax_itm_vsq_WRED_TBL_DATA_to_WRED_QT_DP_INFO(unit, &tbl_data, exact_info);
    SOCDNX_IF_ERR_EXIT(res);
  ...
  SOC_SAND_U64
    ...
    */
    ...
    res = arad_itm_man_exp_buffer_set(
          max_avrg_th_16_byte,
          SOC_TMC_ITM_VSQ_WRED_AVRG_TH_MNT_LSB,
          SOC_TMC_ITM_VSQ_WRED_AVRG_TH_MNT_NOF_BITS,
          SOC_TMC_ITM_VSQ_WRED_AVRG_TH_EXP_LSB,
          SOC_TMC_ITM_VSQ_WRED_AVRG_TH_EXP_NOF_BITS,
          FALSE,
  ...
  )
{
    ...
    arad_iqm_mantissa_exponent_get(unit, (0x1 << soc_mem_field_length(unit, CGM_VSQA_WORDS_RJCT_PRMSm, MAX_SIZE_TH_DP_0f)) - 1, CGM_ITM_VSQ_MANTISSA_BITS, &max_size_mnt, &max_size_exp);
    info->max_inst_q_size_th[SOC_TMC_INGRESS_THRESHOLD_TOTAL_BYTES] = (max_size_mnt * (0x1 << max_size_exp));
    arad_iqm_mantissa_exponent_get(unit, (0x1 << soc_mem_field_length(unit, CGM_VSQA_SRAM_BUFFERS_RJCT_PRMSm, MAX_SIZE_TH_DP_0f)) - 1, CGM_ITM_VSQ_MANTISSA_BITS, &max_size_mnt, &max_size_exp);
    info->max_inst_q_size_th[SOC_TMC_INGRESS_THRESHOLD_SRAM_BUFFERS] = (max_size_mnt * (0x1 << max_size_exp));
    arad_iqm_mantissa_exponent_get(unit, (0x1 << soc_mem_field_length(unit, CGM_VSQA_SRAM_PDS_RJCT_PRMSm, MAX_SIZE_TH_DP_0f)) - 1, CGM_ITM_VSQ_MANTISSA_BITS, &max_size_mnt, &max_size_exp);
  ...
  )
{
    ...
    if (info->size_fc[SOC_TMC_INGRESS_THRESHOLD_TOTAL_BYTES].clear > CGM_ITM_VSQ_WORDS_SIZE_MAX(unit)) {
        SOCDNX_IF_ERR_EXIT(SOC_E_PARAM);
    }
    if (info->size_fc[SOC_TMC_INGRESS_THRESHOLD_TOTAL_BYTES].set > CGM_ITM_VSQ_WORDS_SIZE_MAX(unit)) {
        SOCDNX_IF_ERR_EXIT(SOC_E_PARAM);
    }
    if (info->size_fc[SOC_TMC_INGRESS_THRESHOLD_SRAM_BUFFERS].clear > CGM_ITM_VSQ_SRAM_BUFFERS_SIZE_MAX) {
        SOCDNX_IF_ERR_EXIT(SOC_E_PARAM);
    }
    if (info->size_fc[SOC_TMC_INGRESS_THRESHOLD_SRAM_BUFFERS].set > CGM_ITM_VSQ_SRAM_BUFFERS_SIZE_MAX) {
        SOCDNX_IF_ERR_EXIT(SOC_E_PARAM);
    }
    if (info->size_fc[SOC_TMC_INGRESS_THRESHOLD_SRAM_PDS].clear > CGM_ITM_VSQ_SRAM_PDS_SIZE_MAX) {
        SOCDNX_IF_ERR_EXIT(SOC_E_PARAM);
    }
    if (info->size_fc[SOC_TMC_INGRESS_THRESHOLD_SRAM_PDS].set > CGM_ITM_VSQ_SRAM_PDS_SIZE_MAX) {
        SOCDNX_IF_ERR_EXIT(SOC_E_PARAM);
    }
    ...
    if (vsq_group_ndx == SOC_TMC_ITM_VSQ_GROUP_SRC_PORT) {
        /* VSQ-E table is doubled. The first half is for pool 0 and the second half is for pool 1 */
        entry_offset += pool_id * SOC_TMC_ITM_VSQ_QT_RT_CLS_MAX(unit);
    }
