...
kbp_status qax_kaps_register_read(void *xpt, uint32_t offset, uint32_t nbytes, uint8_t *bytes)
{
    ...
    if ((offset == 0) && (nbytes == 4) && (bytes != NULL)) {
        if (!SAL_BOOT_PLISIM && !(dcmn_device_block_for_feature(unit, DCMN_NO_EXTENDED_LPM_FEATURE))) {
            rv = READ_KAPS_KAPS_REVISIONr(unit,&reg_val);
            if (rv != SOC_SAND_OK) {
                return KBP_FATAL_TRANSPORT_ERROR;
            }
        } else {
            if (SOC_IS_QUX(unit)) {
                ...
                /* Replace with KAPS_QUMRAN_REVISION_REG_VALUE once added to the KBPSDK */
                reg_val = KAPS_QUMRAN_DEVICE_ID << 16;
            } else if (SOC_IS_JERICHO_PLUS(unit)) {
                if (dcmn_device_block_for_feature(unit, DCMN_NO_EXTENDED_LPM_FEATURE)) {
                    /* Use Jericho mode */
                    reg_val = KAPS_DEFAULT_DEVICE_ID;
                } else {
                 reg_val = KAPS_JERICHO_PLUS_REVISION_REG_VALUE;
                }
            } else {
               reg_val = KAPS_DEFAULT_DEVICE_ID;
            }
                                     ...
                                     uint8 *bytes)
{
    ...
    if (SOC_IS_QAX(unit)) {
        ...
        if (dcmn_device_block_for_feature(unit, DCMN_NO_EXTENDED_LPM_FEATURE)) {
            /* Jericho mode attempts unncessary write in Jericho+ */
            if (mem == KAPS_BUCKET_MAP_MEMORYm) {
                return KBP_OK;
            }
        }
    }
