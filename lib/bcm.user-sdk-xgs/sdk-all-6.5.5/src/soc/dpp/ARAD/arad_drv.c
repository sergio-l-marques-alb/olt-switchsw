...
soc_dpp_arad_str_prop_mc_tbl_mode(int unit)
{
    ...
    if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
        ...
        if (SOC_IS_QAX(unit)) {
            ...
            qax->nof_egress_bitmaps = qax->nof_ingress_bitmaps = 0;
...
soc_arad_default_config_general_get(int unit)
{
    ...
    if (SOC_IS_QAX(unit)) {
        dpp_pp->oam_hierarchical_lm = soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "oam_hierarchical_lm_enable", 0);
    }
    ...
    if (SOC_IS_QAX(unit) && soc_property_get(unit, spn_OAM_STATISTICS_PER_MEP_ENABLED, 0)) {
            SOCDNX_EXIT_WITH_ERR(SOC_E_UNAVAIL,
                                 (_BSL_SOCDNX_MSG(spn_OAM_STATISTICS_PER_MEP_ENABLED" currently is not supported for QAX device.")));
    }
    ...
    dpp_pp->custom_feature_mpls_port_termination_lif_id[0] = soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "defult_logical_interface_mpls_1_port_termination", -1);
    dpp_pp->custom_feature_mpls_port_termination_lif_id[1] = soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "defult_logical_interface_mpls_2_port_termination", -1);
    dpp_pp->custom_feature_mpls_port_termination_lif_id[2] = soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "defult_logical_interface_mpls_3_port_termination", -1);
    ...
    if ((dpp_pp->custom_feature_mpls_port_termination_lif_id[0] != -1) && (dpp_pp->custom_feature_mpls_port_termination_lif_id[1] != -1) && (dpp_pp->custom_feature_mpls_port_termination_lif_id[2] != -1))  {
        if (soc_property_get(unit, spn_MPLS_TERMINATION_LABEL_INDEX_ENABLE, 0) == 1){
            dpp_pp->custom_feature_mpls_port_termination = 1;
        } else {
            SOCDNX_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_SOCDNX_MSG(" For MPLS port Termination MPLS_TERMINATION_LABEL_INDEX_ENABLE must be set")));
        }
    } else if ((dpp_pp->custom_feature_mpls_port_termination_lif_id[0] + dpp_pp->custom_feature_mpls_port_termination_lif_id[1] + dpp_pp->custom_feature_mpls_port_termination_lif_id[2]) != -3) {
        SOCDNX_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_SOCDNX_MSG(" For MPLS port Termination 3 custom_feature_mpls_port_termination_lif_id must be set to valid LIF-ID")));
    }
...
soc_arad_str_prop_nif_elk_fwd_table_size_get(int unit, ARAD_INIT_ELK *elk)
{
    ...
    if((elk->fwd_table_size[ARAD_KBP_FRWRD_TBL_ID_IPV4_UC_RPF_0] == 0) && elk->fwd_table_size[ARAD_KBP_FRWRD_TBL_ID_IPV4_MC]) {
        SOCDNX_EXIT_WITH_ERR(SOC_E_FAIL, (_BSL_SOCDNX_MSG("KBP: IPv4 Multicast table requires the initialization of IPv4 Unicast table")));
    }
    if((elk->fwd_table_size[ARAD_KBP_FRWRD_TBL_ID_IPV6_UC_RPF_0] == 0) && elk->fwd_table_size[ARAD_KBP_FRWRD_TBL_ID_IPV6_MC]) {
        SOCDNX_EXIT_WITH_ERR(SOC_E_FAIL, (_BSL_SOCDNX_MSG("KBP: IPv6 Multicast table requires the initialization of IPv6 Unicast table")));
    }
...
soc_arad_info_config(int unit) 
{
    ...
    for (port_i = 0; port_i < SOC_MAX_NUM_PORTS; ++port_i) {
        ...
        if (!SOC_WARM_BOOT(unit)) {     
            ...
            if ((!SOC_PORT_IS_STAT_INTERFACE(flags)) && (!SOC_PORT_IS_LB_MODEM(flags)))
            {
    ...
    */
     ...
     */ 
      ...
      SOC_DPP_CORES_ITER(SOC_CORE_ALL, core) {
          for (i=0; i<(SOC_IS_QAX(unit)?QAX_COSQ_TOTAL_FLOW_REGIONS:SOC_TMC_COSQ_TOTAL_FLOW_REGIONS); i++) {
...
soc_arad_pp_init(int unit)
{
    ...
    if (SOC_IS_JERICHO(unit)) {
        ...
        dpp->pp.l3_route_vrf_update_ipv4 = (0x1 & soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "l3_vrf_update_enable", 0));
        dpp->pp.l3_route_vrf_update_ipv6 = ((0x1<<1) & soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "l3_vrf_update_enable", 0)) >> 1;
    }
