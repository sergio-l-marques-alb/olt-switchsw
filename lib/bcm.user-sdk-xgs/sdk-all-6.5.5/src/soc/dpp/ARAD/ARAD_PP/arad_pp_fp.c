        ...
        for (pd_ndx = 0; pd_ndx < ARAD_PP_FP_NOF_PDS; ++pd_ndx)
        {
            if (action_found[pd_ndx][1] == access_profile_id) {
                ...
                if (action_found[pd_ndx][0]) {
                    ...
                    if (SOC_IS_JERICHO_PLUS(unit) && (pd_ndx % SOC_TMC_TCAM_NOF_BANKS) == 0) {
                        /* Start of new cycle, reset fem_macro_ndx for JER+/QAX
                         * for filling macro_simple data
                         */
                        fem_macro_ndx = 0;
                    }
                    ...
                    if (key_id)
                    {
                      ...
                      {
                            if (info->tcam[cycle_ndx][bank_ndx].actions[ind].type == SOC_PPC_FP_ACTION_TYPE_CHANGE_KEY)
                            {
                    ...
                    /*In J+/QAX the LAST FEM/FES is corrupted so we read the action types from here*/
                    if(SOC_IS_JERICHO_PLUS(unit) == TRUE)
                    {
                        for (ind = 0; ind < SOC_PPC_FP_NOF_ACTIONS_PER_DB_MAX; ++ind)
                        {
                            if((info->tcam[cycle_ndx][bank_ndx].actions[ind].type != SOC_PPC_FP_ACTION_TYPE_INVALID) &&
                                (info->tcam[cycle_ndx][bank_ndx].actions[ind].type != SOC_PPC_FP_ACTION_TYPE_CHANGE_KEY))
                            {
  ...
  )
{
  ...
  for(db_idx = 0; db_idx < SOC_PPC_FP_NOF_DBS; db_idx++)
  {
    ...
    if(info->db[db_idx].valid)
    {
      ...
      for (action_idx = 0; action_idx < SOC_PPC_FP_NOF_ACTIONS_PER_DB_MAX; action_idx++) 
      {
          if(fp_db_info.action_types[action_idx] == SOC_PPC_FP_ACTION_TYPE_CHANGE_KEY) 
          {
              info->db[db_idx].db_tcam.action[action_idx].valid = 1;
              info->db[db_idx].db_tcam.action[action_idx].action_type = SOC_PPC_FP_ACTION_TYPE_CHANGE_KEY;
