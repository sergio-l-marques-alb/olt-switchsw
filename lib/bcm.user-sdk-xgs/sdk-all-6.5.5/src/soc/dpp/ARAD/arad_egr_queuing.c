  ...
  )
{
    ...
    */
    if (priority_mode == ARAD_EGR_PORT_ONE_PRIORITY || priority_mode == ARAD_EGR_PORT_TWO_PRIORITIES) 
    {
        ...
        if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
            data_sch = 0x7FFFFFF;
            SOCDNX_IF_ERR_EXIT(WRITE_SCH_CIR_SHAPERS_STATIC_TABEL_CSSTm(unit, SCH_BLOCK(unit, core_id), base_q_pair, &data_sch));  
        }
    }
  ...
  )
{
  ...
  /* CGM_DP_ELIGIBLE_TO_USE_RESOURCESr */
  if (SOC_IS_QAX(unit)) {
      SOC_SAND_SOC_IF_ERROR_RETURN(res, 1590, exit, READ_ECGM_CGM_DP_ELIGIBLE_TO_USE_RESOURCESr(unit, reg));
  } else {
      SOC_SAND_SOC_IF_ERROR_RETURN(res, 1590, exit, READ_CGM_CGM_DP_ELIGIBLE_TO_USE_RESOURCESr(unit, core, reg));
  }
  index = ((info->pool_id) << 2) | dp_ndx;
  ...
  )
{
  ...
  if (SOC_IS_QAX(unit)) {
      SOC_SAND_SOC_IF_ERROR_RETURN(res, 1870, exit, READ_ECGM_CGM_MAP_IF_2_THr(unit, reg_above_64));
      info->uc_profile = 0;
  ...
  )
{
    ...
    if (SOC_IS_QAX(unit)) {
        ...
        {
            SOC_SAND_SOC_IF_ERROR_RETURN(res, 1760, exit, READ_ECGM_CGM_MAP_IF_2_THr(unit, reg_above_64));
            SHR_BITCOPY_RANGE(reg_above_64, internal_interface_id*3, &profile_num, 0, 3);
            SOC_SAND_SOC_IF_ERROR_RETURN(res, 1770, exit, WRITE_ECGM_CGM_MAP_IF_2_THr(unit, reg_above_64));
  ...
  )
{
    ...
    if (SOC_IS_QAX(unit)) {
        SOCDNX_IF_ERR_EXIT(WRITE_ECGM_CGM_PD_TH_SP_OR_SHAREDr(unit, set_val));
        SOCDNX_IF_ERR_EXIT(WRITE_ECGM_CGM_DB_TH_SP_OR_SHAREDr(unit, set_val));
    } else {
