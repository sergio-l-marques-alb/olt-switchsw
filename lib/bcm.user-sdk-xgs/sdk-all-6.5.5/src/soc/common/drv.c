...
_soc_xgxs_reset_single_tsc(int unit, soc_port_t port, soc_reg_t reg) {
    ...
    SOC_IF_ERROR_RETURN(soc_reg_set(unit, reg, port, 0, rval64));
    /*
     * For TH+/Apache B0 default behaviour of CLPORT port IDDQ is changed
 ...
 * purpose:  Powerdown a single TSC associate with a TD2/TH/TD2+ port
 */
STATIC soc_error_t
 ...
 * purpose:  Powerup a single TSC associate with a TD2/TH/TD2+ port
 */
STATIC soc_error_t
...
int soc_sbusdma_lock_init(int unit)
{
    ...
    if (soc->tslam_enb != soc->tdma_enb && (SOC_IS_DPP(unit) || SOC_IS_DFE(unit))) {
