/*
 * $Id: xlmac.c,v 1.36 Broadcom SDK $
...
mac_xl_init(int unit, soc_port_t port)
{
    ...
    int mode;
    int encap = 0;
#if defined(BCM_TRIDENT2_SUPPORT) || defined(BCM_SABER2_SUPPORT)
    int runt;
#endif /* BCM_TRIDENT2_SUPPORT || BCM_SABER2_SUPPORT */
#if defined(BCM_SABER2_SUPPORT)
    int                 sgn_det = 0;
    uint32              sgn_crs;
#endif
...
#if defined(BCM_TRIDENT2_SUPPORT) || defined(BCM_SABER2_SUPPORT)
    if ((SOC_IS_TD2_TT2(unit) || SOC_IS_SABER2(unit)) && IS_HG_PORT(unit, port)) {
        runt = soc_property_port_get(unit, port, spn_HIGIG2_HDR_MODE,
               soc_feature(unit, soc_feature_no_higig_plus) ? 1 : 0) ?
                            XLMAC_RUNT_THRESHOLD_HG2 : XLMAC_RUNT_THRESHOLD_HG1;
    ...
    /* init IPG setting after port encap mode been established. */
    if (encap == 1) {
                ...
                soc_port_duplex_t duplex, int ifg)
{
    ...
    /* valid value: 8 to 63 bytes (i.e. multiple of 8 bits) */
...
#ifdef BCM_GREYHOUND_SUPPORT
    if (SOC_IS_GREYHOUND(unit) || SOC_IS_HURRICANE3(unit)) {
        soc_gh_xport_type_update(unit, port, hg_mode);
    } else 
#endif
    {
...
mac_xl_encap_set(int unit, soc_port_t port, int mode)
{
    ...
    if (SOC_IS_GREYHOUND(unit) || SOC_IS_HURRICANE3(unit)) {
        if (mode == SOC_ENCAP_HIGIG2_LITE) {
            /* GH specific :
             *  for HG2-LITE is sepecial case in PortMacro(PM).
    ...
    if (SOC_IS_TD2_TT2(unit) || (SOC_IS_SABER2(unit))) {
        runt = ((mode == SOC_ENCAP_HIGIG2) || (mode == SOC_ENCAP_HIGIG2_LITE)) ?
                    XLMAC_RUNT_THRESHOLD_HG2 : ((mode == SOC_ENCAP_HIGIG) ?
                        XLMAC_RUNT_THRESHOLD_HG1 : XLMAC_RUNT_THRESHOLD_XE);
        SOC_IF_ERROR_RETURN
