/* $Id: cint_flexible_hashing.c,v 1.3 Broadcom SDK $
...
int flexible_field_group_hash_two_keys(const int unit, bcm_field_qualify_t *hash_dq_qualifier, uint32 *hash_dq_length, uint16 *hash_dq_offset, bcm_field_data_qualifier_t *hash_dq_data_qual, uint8 hash_dq_nof, bcm_field_qualify_t *key_dq_qualifier, uint32 *key_dq_length, uint16 *key_dq_offset, bcm_field_data_qualifier_t *key_dq_data_qual, uint8 key_dq_nof, bcm_field_header_format_t preselector_header_format){
    ...
    result = bcm_field_qualify_Stage(unit, presel_id | BCM_FIELD_QUALIFY_PRESEL, bcmFieldStageHash);
    if (BCM_E_NONE != result) {
        printf("Error in bcm_field_qualify_Stage\n");
        return result;
    }

    result = bcm_field_qualify_HeaderFormat(unit, presel_id | BCM_FIELD_QUALIFY_PRESEL, preselector_header_format);
    if (BCM_E_NONE != result) {
...
int flexible_IP_L2_fwrd_with_UDP(int unit, int inPort, int outPort0, int outPort1, int outPort2) {
    ...
    for(i = 0; i < 2; i++) {
        ...
        */
        ...
        result = bcm_field_qualify_HeaderFormat(unit, presel_id | BCM_FIELD_QUALIFY_PRESEL, header_format[i]);
        if (BCM_E_NONE != result) {
            printf("Error in bcm_field_qualify_HeaderFormat\n");
            return result;
        }

        result = bcm_field_qualify_ForwardingType(unit, presel_id | BCM_FIELD_QUALIFY_PRESEL, ForwardingType[i]);
        if (BCM_E_NONE != result) {
            printf("Error in bcm_field_qualify_ForwardingType\n");
            return result;
        }

        result = bcm_field_qualify_IpProtocolCommon(unit, presel_id | BCM_FIELD_QUALIFY_PRESEL, bcmFieldIpProtocolCommonUdp);
        if (BCM_E_NONE != result) {
        ...
        result = bcm_field_qualify_IpProtocolCommon_get(unit, presel_id | BCM_FIELD_QUALIFY_PRESEL, &protocolGet);
        if (BCM_E_NONE != result) {
