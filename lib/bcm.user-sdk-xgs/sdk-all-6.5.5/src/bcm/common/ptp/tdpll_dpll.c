    ...
    bcm_tdpll_dpll_bindings_t *bindings)
{
    ...
    /* Reject output clock bindings for XGPLL2 and XGPLL3 if not supported by h/w */
	if (!soc_feature(unit, soc_feature_tdpll_outputclk_xgpll2)) {
        if (TDPLL_OUTPUT_CLOCK_SYNCE_XGPLL2_ENABLED( DPLL(dpll_index).bindings.output_clocks[0]) ) {
            return BCM_E_PARAM;
        }
	}
