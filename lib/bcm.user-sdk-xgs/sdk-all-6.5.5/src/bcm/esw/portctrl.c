    ...
    *okay = FALSE;
    ...
    if (valid) {
        if ((!init_flag) || (init_flag == PORTMOD_PORT_ADD_F_INIT_CORE_PROBE)) {
            rv = bcmi_esw_portctrl_speed_set(unit, port,
    ...
    if (!IS_HG_PORT(unit, port) || IS_GX_PORT(unit, port)) {
        ...
        if (an_status.enabled) {
            ...
            */
            ...
            /* BCM848xx PHY driver turns on AutoNeg for PHY loopback.
             * Compare the line speed and serdes speed only if port is not in loopback
             * This is because we dont want interface_config_set to be called
             * while the port is running traffic in loopback mode, since
             * interface_config_set invloves disabling the port first
             */
...
bcmi_esw_portctrl_autoneg_set(int unit, bcm_port_t port, int autoneg)
{
    ...
    BCM_IF_ERROR_RETURN(portmod_port_autoneg_get(unit, pport, &an));
    an.enable = autoneg;
...
bcmi_esw_portctrl_encap_xport_set(int unit, bcm_gport_t port, int mode)
{
    ...
    BCM_IF_ERROR_RETURN(portmod_port_autoneg_get(unit, port, &an_info));
...
bcmi_esw_portctrl_encap_set_execute(int unit, bcm_gport_t port, int mode, int force)
{
    ...
    if (BCM_SUCCESS(rv)) {
        ...
        rv = bcmi_esw_portctrl_frame_max_set(unit, port, SOC_INFO(unit).max_mtu);
    }
