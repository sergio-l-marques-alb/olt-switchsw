                     ...
                     uint32 *lpm_entry)
{
    ...
    /* validate if non-default VRF been assigned to LPM!
     *  - Some devices like HRx/GH can't support VRF feature and the internal 
     *    VRF must be treat as '0'.
     *  - SOC common design for those devices have no VRF supporting in LPM
     *    will report SW VRF value at SOC_L3_VRF_DEFAULT (0). 
     *    >> this design can be found in soc_xxx_lpm_vrf_get() functions.
     */
    if (lpm_cfg->defip_vrf != BCM_L3_VRF_DEFAULT) {
    ...
    /* L3_DEFIPm.GLOBAL_ROUTE0f for no VRF feature device is not used */
    return (BCM_E_NONE);
}
