...
int _bcm_trx_vp_tpid_add(int unit, bcm_gport_t vport, uint16 tpid, int color_select)
{
    ...
    if ((tgid == -1) && islocal && SOC_PORT_VALID(unit, port)) {
        ...
        if (soc_feature(unit, soc_feature_egr_vlan_control_is_memory)) {
            rv = READ_EGR_VLAN_CONTROL_1m(unit,
    ...
    if (tgid == -1) {
        ...
        if (islocal || (!islocal && pp_port_flag)) {
            ...
            if (soc_feature(unit, soc_feature_egr_vlan_control_is_memory)) {
                ...
                old_idx = soc_EGR_VLAN_CONTROL_1m_field32_get(unit, &vlan_control,
                        OUTER_TPID_INDEXf);
            } else
#endif
            {
...
_bcm_trx_port_dtag_mode_set(int unit, bcm_port_t port, int mode)
{
    ...
    if (!IS_HG_PORT(unit, port)) {
        ...
        if (SOC_IS_KATANA2(unit) || SOC_IS_TOMAHAWK(unit)) {
...
_bcm_trx_egr_lport_tab_default_entry_add(int unit, soc_profile_mem_t *prof) 
{
    ...
    /* Add LPORT default entry */
    entries[0] = &egr_lport_profile_entry; 
    entries[1] = &egr_vlan_control_1_entry;
    entries[2] = &egr_vlan_control_2_entry;
    entries[3] = &egr_vlan_control_3_entry;
    entries[4] = &egr_ipmc_cfg2_entry;
    entries[5] = &egr_mtu_entry;
    entries[6] = &egr_port_1_entry;
    entries[7] = &egr_counter_control_entry;
    entries[8] = &egr_shaping_control_entry;

    BCM_IF_ERROR_RETURN(soc_profile_mem_add(unit, prof, entries,
                                            1, &egr_lport_index));

    if (egr_lport_index != _bcm_trx_egr_lport_tab_default_entry_index_get(unit)) {
        /* Something went horribly wrong. */
        rv = BCM_E_INTERNAL;
    }
