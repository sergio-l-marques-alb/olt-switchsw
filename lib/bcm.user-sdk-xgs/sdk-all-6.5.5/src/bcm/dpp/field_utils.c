                                             ...
                                             uint64 *profile)
{
    ...
    if (_BCM_DPP_FIELD_ENTRY_IS_PRESEL(entry)) {
        ...
        if (_BCM_DPP_FIELD_ENTRY_IS_PRESEL_ADVANCED_MODE(entry)) {
            ...
            BCMDNX_IF_ERR_EXIT(_bcm_dpp_field_presel_to_bitmap(unit,entry, bcm_stage, &index, &stage_id)); 
...
_bcm_dpp_field_presel_to_bitmap(int unit,
                                ...
                                _bcm_dpp_field_stage_idx_t *stage_id)
{
    int ing_pmf_start_offset , eg_pmf_start_offset, slb_start_offset , start_offset;
    int presel_id  = presel & 0xFFF; /* mask the stage bits from presel-ids */

    BCMDNX_INIT_FUNC_DEFS;
    
   

    if ( (stage != bcmFieldStageIngress ) && (stage != bcmFieldStageEgress ) && (stage != bcmFieldStageHash) ) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
    ...
    slb_start_offset     = eg_pmf_start_offset +  SOC_DPP_DEFS_GET(unit,nof_egress_pmf_program_selection_lines);
    ...
    *presel_bitmap_id = presel_id  + start_offset;
...
_bcm_dpp_field_presel_index_to_pfg(int unit,
                             ...
                             int *pfg_ndx)
{
    int ing_pmf_start_offset , eg_pmf_start_offset, slb_start_offset ;
    ...
    if ( ( presel_id < ing_pmf_start_offset ) ||  ( presel_id > ( slb_start_offset + SOC_DPP_DEFS_GET(unit,nof_slb_program_selection_lines) ) ) )
    {
           BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
                             (_BSL_BCM_MSG_NO_UNIT("unit %d does not support"
                                               " preselector ID %d "),
    ...
    if (_BCM_DPP_FIELD_ENTRY_IS_ENTRY(entry)) {
        ...
        *qualLimit = SOC_PPC_FP_NOF_QUALS_PER_DB_MAX;
        ...
        if (_BCM_DPP_FIELD_ENTRY_IS_PRESEL_ADVANCED_MODE(entry)) {
            ...
            BCMDNX_IF_ERR_EXIT(_bcm_dpp_field_presel_to_bitmap(unit,entry, bcm_stage, &index, stage)); 
        }
