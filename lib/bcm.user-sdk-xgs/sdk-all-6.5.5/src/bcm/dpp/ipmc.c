  ...
  )
{
    ...
    if (data->flags & BCM_IPMC_IP6) {
        /* IPV6, allow default IPMC entries for jericho and above */
        if (!BCM_IP6_MULTICAST(data->mc_ip6_addr) && (SOC_IS_JERICHO(unit) && ((data->mc_ip6_mask[0] & 0xFF) != data->mc_ip6_addr[0]))) {
            LOG_ERROR(BSL_LS_BCM_IPMC,
                      (BSL_META_U(unit,
