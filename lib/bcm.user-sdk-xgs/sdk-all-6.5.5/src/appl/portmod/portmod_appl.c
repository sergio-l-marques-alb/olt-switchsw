...
int portmod_pm12x10_add(int unit, soc_pbmp_t phys, int core_addrs[3], int blk_ids[4], void **user_data)
{
    int rv;
    int i, is_sim;
...
portmod_appl_add_ports(int unit){
    ...
    for(phy=0 ; phy<12 ; phy++) {
        ...
        SOC_PBMP_PORT_ADD(add_info.phys, phy);
    ...
    for(phy=0 ; phy<10 ; phy++) {
        SOC_PBMP_PORT_ADD(add_info.phys, 12 + phy);
    }
    ...
    add_info.interface_config.interface = SOC_PORT_IF_CAUI;
    add_info.interface_config.interface_modes = 0;
    SOC_PBMP_PORT_ADD(add_info.phys, 24);
    SOC_PBMP_PORT_ADD(add_info.phys, 25);
    SOC_PBMP_PORT_ADD(add_info.phys, 26);
    SOC_PBMP_PORT_ADD(add_info.phys, 27);
