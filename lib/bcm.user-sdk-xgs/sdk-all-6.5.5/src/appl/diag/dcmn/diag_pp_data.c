/* 
 * $Id: diag_pp_data.c,v 1.15 Broadcom SDK $
 *
...
static uint32 pp_dump_parser_program(int unit, void *curTop, int offset, int depth);
...
static void pp_data_set_key(int unit, void *curIndex, char *stage_n, char *key_n, int valid, qualifier_t *qualifier, int qualifier_num)
{
    ...
    for(i = 0; i < qualifier_num; i++) {
        if(qualifier[i].valid == TRUE) {
            if((curQualifier = dbx_xml_child_add(curKey, qualifier[i].name, 5)) == NULL) {
                cli_out("Failed to add LSB qualifier:%s(%s) entry\n", qualifier[i].name,  qualifier[i].ce_name);
                return;
            }
...
static void pp_data_set_prefix(void *cur, uint32 and_value, uint32 or_value, int max_size)
{
    uint32         res = CMD_OK;
...
static void pp_attribute_set(void *node, char *attribute, uint32 data, uint32 mask, int data_size)
{
...
static void pp_dump_tables(int unit, void *curStage, int stage, int depth)
{
   int table_id;
   ...
   if((curSection = dbx_xml_child_add(curStage, "Tables", depth)) == NULL) {
       cli_out("Failed to add Table section to PP-Data\n");
       return;
   }
   ...
   for (table_id = 0; table_id < SOC_DPP_DBAL_SW_NOF_TABLES; table_id++) {
       ...
       if((curTable = dbx_xml_child_add(curSection, tmp_str, depth + 1)) == NULL) {
           cli_out("Failed to add VT selection entry:%d\n", i);
           return;
       }
       ...
       for (i = 0; i < table.nof_table_programs; i++) {
           arad_pp_dbal_program_to_string(unit, table.table_programs[i].stage, table.table_programs[i].program_id, (const char **)&str, &prog_id);
...
static void pp_vt_ce_attribute_set(int unit, void *curKey, uint32 valid_1, uint32 valid_2, ARAD_PP_IHP_VTT1ST_KEY_CONSTRUCTION_TBL_DATA *vt_key_construction_p)
{
    uint32 res;
...
static void pp_tt_ce_attribute_set(int unit, void *curKey, uint32 valid_1, uint32 valid_2, ARAD_PP_IHP_VTT2ND_KEY_CONSTRUCTION_TBL_DATA *tt_key_construction_p)
{
    uint32 res;
    qualifier_t qualifier[VTT_QUALIFIER_NUM];
    int i, k;
...
static int pp_dump_vt_selection(int unit, void *curStage, int depth)
{
    uint32         res = CMD_OK;
    ...
    if((curSection = dbx_xml_child_add(curStage, "Program-Selection", depth)) == NULL) {
           cli_out("Failed to add VTT section to PP-Data\n");
        return CMD_FAIL;
    }
    ...
    for(i = 0; i < SOC_DPP_DEFS_GET(unit, nof_vtt_program_selection_lines); i++) {
        ...
        res = arad_pp_ihp_isem_1st_program_selection_cam_tbl_get_unsafe(unit, i, &vt_prog_selection);
...
static int pp_dump_vt_programs(int unit, void *curStage, int depth)
{
    uint32         res = CMD_OK;
    ...
    if((curSection = dbx_xml_child_add(curStage, "Programs", 2)) == NULL) {
           cli_out("Failed to add VT section to PP-Data\n");
        return CMD_FAIL;
    }
    ...
    for(i = 0; i < SOC_DPP_DEFS_GET(unit, nof_vtt_programs); i++) {
        ...
        ARAD_CLEAR(&vt_key_construction, ARAD_PP_IHP_VTT1ST_KEY_CONSTRUCTION_TBL_DATA, 1);
        vt_key_construction.dbal = 1;
        res = arad_pp_ihp_vtt1st_key_construction_tbl_get_unsafe(unit, i, &vt_key_construction);
        SOC_SAND_CHECK_FUNC_RESULT(res, i*10, exit);

        if((curIndex = dbx_xml_child_add(curSection, program_name, depth + 1)) == NULL) {
               cli_out("Failed to add VT program entry:%d\n", i);
            return CMD_FAIL;
        }
        ...
        if(vt_key_construction.vlan_translation_is_use_strength == 0) {
            ...
            if((curTable = dbx_xml_child_add(curIndex, "Priority-Decoder-0", depth + 2)) == NULL) {
                   cli_out("Failed to add TT selection entry:%d\n", i);
                return CMD_FAIL;
            }
...
static int pp_dump_tt_selection(int unit, void *curStage, int depth)
{
    uint32         res = CMD_OK;
...
static int pp_dump_tt_programs(int unit, void *curStage, int depth)
{
    uint32         res = CMD_OK;
    ...
    if((curSection = dbx_xml_child_add(curStage, "Programs", depth)) == NULL) {
           cli_out("Failed to add TT-Programs section to PP-Data\n");
        return CMD_FAIL;
    }
    ...
    for(i = 0; i < SOC_DPP_DEFS_GET(unit, nof_vtt_programs); i++) {
        ...
        ARAD_CLEAR(&tt_key_construction, ARAD_PP_IHP_VTT2ND_KEY_CONSTRUCTION_TBL_DATA, 1);
        tt_key_construction.dbal = 1;
        res = arad_pp_ihp_vtt2nd_key_construction_tbl_get_unsafe(unit, i, &tt_key_construction);
        SOC_SAND_CHECK_FUNC_RESULT(res, i*10, exit);

        if((curIndex = dbx_xml_child_add(curSection, program_name, depth + 1)) == NULL) {
               cli_out("Failed to add TT program entry:%d\n", i);
            return CMD_FAIL;
        }
...
static int pp_dump_flp_selection(int unit, void *curStage, int depth)
{
    uint32         res = CMD_OK;
    ...
    for(i = 0; i < SOC_DPP_DEFS_GET(unit, nof_flp_program_selection_lines); i++) {
        ...
        res = arad_pp_ihb_flp_program_selection_cam_tbl_get_unsafe(unit, i, &flp_prog_selection);
        if (soc_sand_get_error_code_from_error_word(res) != SOC_SAND_OK)
            return CMD_FAIL;
        ...
        sprintf(tmp_str, "entry.%d", i);
        if((curIndex = dbx_xml_child_add(curSection, tmp_str, depth + 1)) == NULL) {
               cli_out("Failed to add FLP selection entry:%d\n", i);
            return CMD_FAIL;
        }
...
static int pp_dump_flp_programs(int unit, void *curStage, int depth)
{
    uint32         res = CMD_OK;
    ...
    for(i = 0; i < SOC_DPP_DEFS_GET(unit, nof_flp_programs); i++) {
        ...
        if((curIndex = dbx_xml_child_add(curSection, program_name, depth + 1)) == NULL) {
            cli_out("Failed to add FLP program entry:%d\n", i);
            return CMD_FAIL;
        }
        ...
        if(lookups_tbl.lem_1st_lkp_valid) {
            if((curTable = dbx_xml_child_add(curIndex, "LEM-1st", depth + 2)) == NULL) {
                   cli_out("Failed to add LEM-1 program entry:%d\n", i);
                return CMD_FAIL;
            }
        ...
        if(lookups_tbl.lem_2nd_lkp_valid) {
            if((curTable = dbx_xml_child_add(curIndex, "LEM-2nd", depth + 2)) == NULL) {
                   cli_out("Failed to add LEM-2 program entry:%d\n", i);
                return CMD_FAIL;
            }
        ...
        if(lookups_tbl.lpm_1st_lkp_valid) {
            ...
            if(lookups_tbl.lpm_public_1st_lkp_valid) {
                if((curPublicTable = dbx_xml_child_add(curTable, "Public", depth + 3)) == NULL) {
                       cli_out("Failed to add Public entry in :%d\n", i);
                    return CMD_FAIL;
                }
        ...
        if((curTable = dbx_xml_child_add(curIndex, "Result-A", depth + 2)) == NULL) {
               cli_out("Failed to add Result-A for program entry:%d\n", i);
            return CMD_FAIL;
        }
...
dpp_dump_stage_table_db(int unit, SOC_PPC_FP_DATABASE_STAGE  stage, dpp_export_pp_stage_info_t *stage_info)
{
    ...
    for (table_index = 0; table_index < SOC_DPP_DBAL_SW_NOF_TABLES; table_index++) {
        ...
        sal_strcpy(stage_info->tables[table_index].name, table.table_name);  /* Using same name as in DBAL */
...
static uint32 dpp_dump_flp_selection(int unit, int core, dpp_export_pp_stage_info_t *stage)
{
    ...
    for(i = 0; i < SOC_DPP_DEFS_GET(unit, nof_flp_program_selection_lines); i++) {
        ...
        if((stage->presels[i].qual_list = utilex_rhlist_create("qual_list", sizeof(signal_output_t), 0)) == NULL) {
            continue;
        }
...
static void pp_dump_parser_print_entry(int unit, void *curTop, int depth, int offset, int base, int macro, int first, int last)
{
    uint32         res = CMD_OK;
    char *entry_n;
    char entry_str[RHNAME_MAX_SIZE];
    char attr_str[RHNAME_MAX_SIZE];
    void *cur;
    int i;
    ...
    if(first == last)
        sprintf(entry_str, "%s:%d", entry_n, first);
    else
        sprintf(entry_str, "%s:%d-%d", entry_n, first, last);

    if((cur = dbx_xml_child_add(curTop, entry_str, depth + 1)) == NULL) {
         cli_out("Failed to add Program Selection section to IRPP Parser\n");
         return;
    }
    ...
    if(base != PARSER_PROGRAM_END) {
        ...
        if(base != PARSER_PROGRAM_FIN) {
            RHDATA_SET_HEX(cur, "Base-Address",  base << 4);
            pp_dump_parser_program(unit, cur, base << 4, depth + 1);
       }
    }

    RHDATA_SET_INT(cur, "Depth",  depth - 2);

    if((base == PARSER_PROGRAM_END) || (base == PARSER_PROGRAM_FIN)) {
        for(i = first; i <= last; i++)  {
            if(i + offset >= 0x800) {
                sprintf(entry_str, "Bad address on offset:0x%04x index:%d\n", offset, i);
                continue;
            }
            res = arad_pp_ihp_packet_format_table_tbl_get_unsafe(unit, i + offset, &packet_format);
            ...
            if(i == first) {
                ...
                dpp_sigparam_get(device, "Packet_Format_Code", first_pfc, entry_str, 1);

                sprintf(attr_str, "Packet_Format_Code:0x%03x-0x%03x", offset + first_addr, offset + (i -1));
                RHDATA_SET_STR(cur, attr_str, entry_str);
                sprintf(attr_str, "Leaf_Context:0x%03x-0x%03x", offset + first_addr, offset + (i -1));
                RHDATA_SET_HEX(cur, attr_str, first_plc);
                first_pfc = curr_pfc;
                first_plc = curr_plc;
                first_addr = i;
            }
            if(i == last) {
                ...
                dpp_sigparam_get(device, "Packet_Format_Code", curr_pfc, entry_str, 1);
                sprintf(attr_str, "Packet-Format-Code:0x%03x-0x%03x", offset + first_addr, offset + last);
                RHDATA_SET_STR(cur, attr_str, entry_str);
                sprintf(attr_str, "Leaf-Context:0x%03x-0x%03x", offset + first_addr, offset + last);
                RHDATA_SET_HEX(cur, attr_str, curr_plc);
            }
        }
    }
    else
        dbx_xml_node_end(cur, depth + 1);
...
static uint32 pp_dump_parser_program(int unit, void *curTop, int offset, int depth)
{
    ...
    if(offset >= 0x800) {
        cli_out("Offset:0x%04x is not eligible", offset);
        res = CMD_FAIL;
        goto exit;
    }

    for(i = 0; i < PARSER_PROGRAM_SEGMENT_SIZE; i++) {
        arad_pp_ihp_parser_program_tbl_get_unsafe(unit, 0, offset + i, &tbl_data);
        ...
        if(i == 0) {
            ...
            pp_dump_parser_print_entry(unit, curTop, depth, offset, prev_base, prev_macro, first, last);
            /* And start new one counting */
            last = first = i;
            prev_base = curr_base;
            prev_macro = curr_macro;
        }
        if(i == (PARSER_PROGRAM_SEGMENT_SIZE - 1))
            /* print anyway current */
            pp_dump_parser_print_entry(unit, curTop, depth, offset, curr_base, curr_macro, first, last);
   }
exit:
    return res;
}

static uint32 pp_dump_parser_ports(int unit, void *curTop, int depth)
{
    void *cur;
    bcm_pbmp_t pbmp_nif;
    bcm_port_t port;
    ...
    BCM_PBMP_ITER(pbmp_nif, port) {
        if(soc_port_sw_db_local_to_pp_port_get(unit, port, &pp_port, &core_id) != SOC_E_NONE) {
            cli_out("soc_port_sw_db_local_to_pp_port_get Failed\n");
            res = CMD_FAIL;
            goto exit;
        }
        res = arad_pp_ihp_pp_port_info_tbl_get_unsafe(unit, core_id, pp_port, &tbl_data);
...
static uint32 pp_dump_parser_macros(int unit, void *curTop, int depth)
{
    ...
    for(i = 0; i < 7; i++) {
        res = arad_pp_ihp_parser_eth_protocols_tbl_get_unsafe(unit, i, &eth_protocols);
    ...
    for(i = 0; i < 7; i++) {
        res = arad_pp_ihp_parser_ip_protocols_tbl_get_unsafe(unit, i, &ip_protocols);
    ...
    for(i = 0; i < 15; i++) {
        res = arad_pp_ihp_parser_custom_macro_protocols_tbl_get_unsafe(unit, i, &custom_macro_protocols);
        sprintf(entry_n, "Protocol:%d", i);
...
static int pp_dump_parser(int unit, void *curStage, int depth)
{
    ...
    pp_dump_parser_program(unit, curSection, 0, depth);
    dbx_xml_node_end(curSection, depth);

    if((curSection = dbx_xml_child_add(curStage, "Ports", depth)) == NULL) {
        cli_out("Failed to add Port section to IRPP Parser\n");
        res = CMD_FAIL;
        goto exit;
    }
    ...
    if((curSection = dbx_xml_child_add(curStage, "Custom-Macros", depth)) == NULL) {
        cli_out("Failed to add Custom-Macros section to IRPP Parser\n");
        res = CMD_FAIL;
        goto exit;
    }
...
static int pp_dump_export_data(int unit, dpp_export_pp_stage_info_t *stage_info, void *curTop, int depth)
{
    uint32         res = CMD_OK;
...
print_diag_pp_sig_usage(void)
{
    char diag_pp_sig_usage[] =
        ...
        "        full     - provides full value fo signal (by default only 256 are shown)\n"
...
cmd_result_t dpp_export_pp(int unit, char *stage_n, char *filename)
{
    ...
    if(ISEMPTY(stage_n)) {
        ...
        if(appl_dpp_diag_sig_init(unit) == NULL) {
            LOG_WARN(BSL_LS_APPL_SHELL, (BSL_META_U(unit, "Signal DB was not initialized\n")));
            goto exit;
        }
        ...
        for(i = 0; stage_callback[i].name != NULL; i++) {
            if(!sal_strcasecmp(stage_n, stage_callback[i].name)) {
                if(stage_callback[i].dump_callback) {
                    if((curTop = dbx_xml_top_create(stage_callback[i].name)) == NULL) {
                        cli_out("Fail to create dump for %s:\n", stage_callback[i].name);
                        return CMD_FAIL;
                    }
                    stage_callback[i].dump_callback(unit, curTop, 1);
                }
                break;
            }
        }
    ...
    if(ISEMPTY(filename)) {
        for(i = 0; i < 100; i++) {
            sprintf(auto_filename, "%s-%d", "PP-Data", i);
    ...
    dbx_xml_top_save(curTop, filepath);
...
{
#if !defined(__KERNEL__)
    return dpp_signals_init(SOC_CHIP_STRING(unit));
#else
    return NULL;
#endif
}
...
cmd_result_t cmd_diag_pp_signals_get(int unit, args_t* a)
{
    ...
    char *block_n = NULL, *stage_n = NULL, *from_n = NULL, *to_n = NULL, *show_n = NULL, *match_n = NULL, *order_n = "big";
    ...
    if(ret <= 0) {
        print_diag_pp_sig_usage();
        goto exit;
    }

    if((device = appl_dpp_diag_sig_init(unit)) == NULL) {
        LOG_WARN(BSL_LS_APPL_SHELL, (BSL_META_U(unit, "Signal DB for %s was not found\n"), SOC_CHIP_STRING(unit)));
        goto exit;
    }
    ...
    for(i = 0; i < realtokens; i++) {
        ...
        else if(!sal_strcasecmp(tokens[i], "full"))
            print_flags |= SIGNALS_PRINT_FULL;
    ...
    /* Avoid unsupported flag combinations */
    if(print_flags & SIGNALS_PRINT_DETAIL)
        print_flags &= ~SIGNALS_PRINT_FULL;

    switch(command) {
        case SIG_COMMAND_STAGE:
            res = dpp_stage_print(device, match_n, print_flags);
            break;
        case SIG_COMMAND_DEBUG:
            ...
            if(prm_core == -1) {
                ...
                SOC_DPP_CORES_ITER(SOC_CORE_ALL, prm_core) {
                    dpp_dsig_get_list(device, unit, prm_core, &match_m, dsig_list);
                }
            }
            else {
            ...
            res = dpp_dsig_print(dsig_list, print_flags);
            dpp_dsig_free(dsig_list);
            break;
        default:
            break;
...
exit:
    return res;
#else
    cli_out("This command is unavailable in Kernel mode\n");
    return CMD_OK;
#endif
}
