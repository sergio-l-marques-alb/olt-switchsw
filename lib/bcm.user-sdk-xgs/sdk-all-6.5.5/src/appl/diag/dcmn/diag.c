...
#ifdef BCM_JERICHO_SUPPORT
    uint32 physical_phy;
    int phy, nof_lanes_per_nbi = SOC_DPP_DEFS_GET(unit, nof_lanes_per_nbi);
    ...
    BCM_PBMP_ITER(pbmp_nif, port) {
        ...
        if (SOC_IS_JERICHO(unit)) {
            if (SOC_PORT_IF_ILKN == interface_type) {
                ...
                rv = soc_jer_nif_ilkn_phys_aligned_pbmp_get(unit, port, &phys_aligned, 0);
                if (rv != BCM_E_NONE) {
                    return CMD_FAIL;
                }
            }
...
dpp_diag_voq_details(int unit, int core_id, int voq_id)
{
   ...
   if (!isisq) {
     if (!SOC_TMC_ITM_PER_PACKET_COMPENSATION_ENABLED(unit)) {
       ...
       cli_out("\nPer Packet Compensation enabled.\n");
     }
