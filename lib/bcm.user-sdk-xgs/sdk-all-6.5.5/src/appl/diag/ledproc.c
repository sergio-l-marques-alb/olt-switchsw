...
ledproc_linkscan_cb(int unit, soc_port_t port, bcm_port_info_t *info)
{
    ...
    if (SOC_IS_TD_TT(unit) || SOC_IS_TRIUMPH3(unit)) {
        ...
        if ((phy_port > ledup0_pmax) && (!SOC_IS_TOMAHAWK(unit))) {
        ...
        if (soc_feature(unit, soc_feature_led_data_offset_a0)) {
            if (SOC_IS_HELIX4(unit)) {
            ...
            } else if ((SOC_IS_TRIDENT2X(unit) || SOC_IS_TITAN2PLUS(unit)) && !SOC_IS_APACHE(unit)) {
                int i, skip_count = 0;
                ...
                if (phy_port > 36) { /* Ports 37-72 in ledproc1 */
        ...
        PBMP_PORT_ITER(unit, port) {
            ...
            if (SOC_IS_TD_TT(unit)) {
...
#endif /* BCM_TOMAHAWK_SUPPORT */
                {
                    ...
                    portdata2 = portdata1; /* Save initial data */
                    if (portdata1 & 0x1) {
                        portdata |= 0x80;
                    } else {
                        portdata &= ~0x80;
