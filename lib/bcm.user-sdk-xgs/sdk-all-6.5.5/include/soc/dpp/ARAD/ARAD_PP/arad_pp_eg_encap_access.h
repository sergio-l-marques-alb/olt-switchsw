...
#define ARAD_PP_EG_ENCAP_LIF_ENTRY_TO_LOCAL_LIF(_unit, bank_id, offset)  \
    ((bank_id << SOC_DPP_DEFS_GET(_unit, nof_eg_encap_lsbs)) + offset)
...
#define ARAD_PP_EG_ENCAP_ACCESS_OUT_LIF_TO_BANK_ID(unit, outlif)    \
  (outlif >> (SOC_DPP_DEFS_GET(unit, nof_eg_encap_lsbs) + 1))
