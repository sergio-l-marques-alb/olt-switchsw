...
bcm_error_t
_bcm_extender_gport_get(int unit, bcm_gport_t glp,
                        int etag_vid, bcm_gport_t *ext_gport)
{
    int vp;
    if (NULL == ext_gport) {
        return BCM_E_PARAM;
    ...
    for (vp = soc_mem_index_min(unit, SOURCE_VPm);
        ...
        if (_bcm_vp_used_get(unit, vp, _bcmVpTypeExtender)) {
            if (!(EXTENDER_PORT_INFO(unit, vp)->flags &
                  BCM_EXTENDER_PORT_MULTICAST)) {
                if ((EXTENDER_PORT_INFO(unit, vp)->port == glp) &&
                    (EXTENDER_PORT_INFO(unit, vp)->extended_port_vid == etag_vid)) {
