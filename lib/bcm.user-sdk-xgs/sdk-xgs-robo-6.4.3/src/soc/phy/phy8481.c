...
#define PHY8485X_PRBS_ERR_COUNT_MSB_REGSTER                         0xD1DA
#define PHY8485X_PRBS_ERR_COUNT_LSB_REGSTER                         0xD1DB
...
_phy_8481_check_firmware(int unit,phy_ctrl_t *pc)  {
    ...
    LOG_CLI((BSL_META_U(unit,
                        "BCM8481X/8482X: u=%d p=%d: init. Chip Rev = %02d "
...
_phy_8481_copper_lb_set(int unit, soc_port_t port, int enable)
{
    ...
    if (PHY_IS_BCM8483_PLUS(pc)) {
        ...
        do {
            rv = READ_PHY84834_TOP_CONFIG_SCRATCH_8r(unit, pc, &data16);
            /* 
             *  If loobback enable, wait for MAC side Link UP state else 
             *  if loobback disable, wait for MAC side Link DOWN state.
             */
            if (((enable) && ((data16 & (1U << 13)) == 0x2000)) ||
                ((!enable) && (!(data16 & (1U << 13)))) ||
                SOC_FAILURE(rv)) {
                break;
            }
...
_phy84834_top_level_cmd_set_v2(int unit,phy_ctrl_t *pc, uint16 cmd, uint16 arg[], int size)
{
    ...
    if ((status & PHY8481_CMD_COMPLETE_PASS) == 0) {
        LOG_CLI((BSL_META_U(unit,
                                ...
                                soc_phy_control_t type, uint32 *value)
{
    ...
    case SOC_PHY_CONTROL_EEE_TRANSMIT_EVENTS:
        ...
        if (PHY_IS_BCM8484X(pc) || PHY_IS_BCM8485X(pc)) {
            if (speed == 10000) {
...
_phy_8481_copper_an_set(int unit, soc_port_t port, int an)
{
    ...
    if (pan != 0 && an == 0) {
        /* Workaround to avoid  1000BASET force mode which is 
           causing the PHY to misbehave  (0x2100 puts PHY in 100BT force mode )
         */
        SOC_IF_ERROR_RETURN
