int unit=0;
bcm_field_group_t gid_vcap=-1, gid_icap=-1;
...
int ptin_cap_init(void)
{
  ...
  printf("VCAP - bcm_field_group_create_mode: rv=%d\r\n", rv);
  if (rv != BCM_E_NONE)
  {
    return rv;
  }
  ...
  printf("ICAP - bcm_field_group_create_mode: rv=%d\r\n", rv);
  if (rv != BCM_E_NONE)
  {
    return rv;
  }
...
int ptin_create_vcap_rules(bcm_port_t bcm_port, int cos0, int cos1, int cos2, int cos3, int cos4, int cos5, int cos6, int cos7)
{
  ...
  for (prio=0; prio<7; prio++)
  {
      ...
      printf("prio=%u - bcm_field_entry_create: rv=%d\r\n", prio, rv);
      if (rv != BCM_E_NONE)
		return rv;
	  
	  /* Add qualifiers */
	  rv = bcm_field_qualify_InPort(unit, eid, bcm_port, 0xffffffff);
      printf("prio=%u - bcm_field_qualify_InPort: rv=%d\r\n", prio, rv);
      if (rv != BCM_E_NONE)
		return rv;
	  rv = bcm_field_qualify_OuterVlanPri(unit, eid, prio, 0x7);
      printf("prio=%u - bcm_field_qualify_OuterVlanPri: rv=%d\r\n", prio, rv);
      if (rv != BCM_E_NONE)
		return rv;
	  
	  /* Add actions */
	  rv = bcm_field_action_add(unit, eid, bcmFieldActionPrioIntNew, cos[prio], 0);
      printf("prio=%u - bcm_field_action_add(PrioIntNew): rv=%d\r\n", prio, rv);
      if (rv != BCM_E_NONE)
		return rv;
	  rv = bcm_field_action_add(unit, eid, bcmFieldActionClassDestSet, cos[prio], 0);
      printf("prio=%u - bcm_field_action_add(ClassDestSet): rv=%d\r\n", prio, rv);
      if (rv != BCM_E_NONE)
		return rv;
		
	  /* Install rule */
	  rv = bcm_field_entry_install(unit, eid);
      printf("prio=%u - bcm_field_entry_install(gid=%d, eid=%d): rv=%d\r\n", gid_vcap, eid, rv);
      if (rv != BCM_E_NONE)
		return rv;
  }
...
int ptin_create_icap_policer(bcm_port_t bcm_port, int cos, int cir, int cbs)
{
  ...
  printf("cos=%u - bcm_policer_create: rv=%d\r\n", cos, rv);
  if (rv != BCM_E_NONE)
    return rv;
  ...
  printf("cos=%u - bcm_field_qualify_DstClassField: rv=%d\r\n", cos, rv);
  if (rv != BCM_E_NONE)
    return rv;
	  
  /* Define actions */
  rv = bcm_field_action_add(unit, eid, bcmFieldActionRpDrop, 0, 0);
  printf("cos=%u - bcm_field_action_add(RpDrop): rv=%d\r\n", cos, rv);
  if (rv != BCM_E_NONE)
    return rv;
  
  /* Attach policer */
  rv = bcm_field_entry_policer_attach(unit, eid, 0, policer_id);
  printf("cos=%u - bcm_field_entry_policer_attach(policer_id=%d): rv=%d\r\n", cos, policer_id rv);
  if (rv != BCM_E_NONE)
    return rv;
