...
typedef struct
{
  ...
  UINT32          end_address;
  SOC_SAND_MULTI_SET_PTR    multi_set[CHIP_SIM_NOF_CHIPS];
  uint8*          base[CHIP_SIM_NOF_CHIPS] ;
} CHIP_SIM_EM_BLOCK;
...
void chip_sim_em_init_chip_specifics(SOC_SAND_DEVICE_TYPE device_type, CHIP_SIM_EM_BLOCK* indirect_blocks);
uint32 chip_sim_em_malloc(CHIP_SIM_EM_BLOCK* indirect_blocks, int block_cnt);
