/* $Id: chip_sim_mem.c,v 1.7 Broadcom SDK $
...
/* } LINK_FE200_LIBRARIES */
  ...
  case SOC_SAND_DEV_PCP:
    ...
    Chip_mem.core_size = 0xffffff; /* sizeof(PCP_REGS); */  /* Bigger than the actual size */ 
    Chip_mem.size = Chip_mem.core_size + sizeof(unsigned long) * CHIP_SIM_REMOTE_MEM_LAST;
    Chip_mem.desc = desc ;
    chip_sim_cntr_init_chip_specifics(Pcp_counters);
...
/* } */
  ...
  case SOC_SAND_DEV_PETRA:
    ...
    Chip_mem.size = SOC_PA_TOTAL_SIZE_OF_REGS * sizeof(uint32);
    Chip_mem.core_size = Chip_mem.size;

    Chip_mem.desc = desc;
    chip_sim_cntr_init_chip_specifics(Soc_petra_counters);

    chip_sim_interrupt_init_chip_specifics(Soc_petra_interrupts,
                                           INVALID_ADDRESS, /* SOC_PETRA do not general interrupt bit */
                                           0,
                                           INVALID_ADDRESS, /* SOC_PETRA do not general mask bit */
                                           0);

    chip_sim_indirect_init_chip_specifics(Soc_petra_indirect_blocks);
...
/* } */
  ...
  case SOC_SAND_DEV_PB:
    ...
    Chip_mem.size = SOC_PB_TOTAL_SIZE_OF_REGS * sizeof(uint32);
    Chip_mem.core_size = Chip_mem.size;

    Chip_mem.desc = desc;
    chip_sim_cntr_init_chip_specifics(Soc_pb_counters);

    chip_sim_interrupt_init_chip_specifics(Soc_pb_interrupts,
                                           INVALID_ADDRESS, /* PB do not general interrupt bit */
                                           0,
                                           INVALID_ADDRESS, /* PB do not general mask bit */
                                           0);

    chip_sim_indirect_init_chip_specifics(Soc_pb_indirect_blocks);
    chip_sim_em_init_chip_specifics(SOC_SAND_DEV_PB, Soc_pb_em_blocks);
    ...
    )
{
  ...
  {
    if (chip_sim_em_malloc(Soc_pb_em_blocks, SOC_PB_EM_TABLE_ID_LAST))
    {
      status = ERROR ;
      chip_sim_mem_free();
      goto chip_sim_mem_malloc_exit ;
