...
typedef struct portmod_default_user_access_s {
    ...
    int port; /**< Port that will be used for legacy phy access */
    int is_legacy_phy_present; /**< will be setif legacy phy is present in the phychain */
    int cmd_for_phy; /**< will be setif the cmd is targetted for this phy in  the phychain */
    int blk_id; /**< block id */
...
/*! 
 * @brief Flags for ELK port 
 */ 
#define PORTMOD_PORT_FLAGS_ELK 0x1

#define PORTMOD_PORT_FLAGS_ELK_SET(flags) ((flags) |= PORTMOD_PORT_FLAGS_ELK)

#define PORTMOD_PORT_FLAGS_ELK_CLR(flags) ((flags) &= ~PORTMOD_PORT_FLAGS_ELK)

#define PORTMOD_PORT_FLAGS_ELK_GET(flags) ((flags) & PORTMOD_PORT_FLAGS_ELK ? 1 : 0)
...
typedef struct portmod_port_init_config_s {
    ...
    int tx_params_user_flag;
    phymod_tx_t tx_params; /**< tx params from config */
...
typedef struct portmod_port_add_info_s {
    ...
    phymod_interface_t sys_interface; /**< ext phy sys side interface. */
...
#define PORTMOD_PM12x10_F_USE_PM4x25TD 0x2 /**< Use TD version of PM4x25 */

#define PORTMOD_PM12x10_F_EXTERNAL_TOP_MODE_SET(flags) ((flags) |= PORTMOD_PM12x10_F_EXTERNAL_TOP_MODE)
#define PORTMOD_PM12x10_F_USE_PM4x25TD_SET(flags) ((flags) |= PORTMOD_PM12x10_F_USE_PM4x25TD)

#define PORTMOD_PM12x10_F_EXTERNAL_TOP_MODE_CLR(flags) ((flags) &= ~PORTMOD_PM12x10_F_EXTERNAL_TOP_MODE)
#define PORTMOD_PM12x10_F_USE_PM4x25TD_CLR(flags) ((flags) &= ~PORTMOD_PM12x10_F_USE_PM4x25TD)

#define PORTMOD_PM12x10_F_EXTERNAL_TOP_MODE_GET(flags) ((flags) & PORTMOD_PM12x10_F_EXTERNAL_TOP_MODE ? 1 : 0)
#define PORTMOD_PM12x10_F_USE_PM4x25TD_GET(flags) ((flags) & PORTMOD_PM12x10_F_USE_PM4x25TD ? 1 : 0)
...
typedef struct portmod_ilkn_os_create_info_s {
    ...
    int wm_high; /**< Tx fifo watermark high value */
    int wm_low; /**< Tx fifo watermark low value */
 ...
 * @param [in]  all_phy         - 
 */
int portmod_port_interface_config_set(int unit, int port, const portmod_port_interface_config_t* config, int all_phy);
/*! 
 ...
 * @param [in]  first_phy_num   - First Phy Lane information.
 */
int portmod_ext_phy_attach(int unit, int phy, const phymod_core_access_t* ext_phy_access, uint32 first_phy_num);
 ...
 *
 * @brief Get/Set InterFrameGap Setting.  
 *
 * @param [in]  unit            - unit id
 * @param [in]  port            - logical port
 * @param [in]  speed           - the speed
 * @param [in]  duplex          - duplex mode. FULL or HALF.
 * @param [in]  ifg             - InterFrameGap.
 * @param [out]  real_ifg        - InterFrameGap.
 */
 ...
 */
int portmod_port_soft_reset_set(int unit, int port, int idx, int val);
 ...
 * @param [in]  flags           - 
 */
int portmod_port_pgw_reconfig(int unit, int port, const portmod_port_mode_info_t* pmode, int flags);
