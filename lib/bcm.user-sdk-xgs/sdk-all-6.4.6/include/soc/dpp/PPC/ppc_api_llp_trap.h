...
typedef enum
{
   ...
   *  see SOC_PPD_LLP_TRAP_PROG_TRAP_L2_INFO
   */
   ...
   *  SOC_PPD_LLP_TRAP_PROG_TRAP_L4_INFO.
   */
...
typedef struct
{
   ...
   *  SOC_PPD_LLP_TRAP_PORT_ENABLE to what is the usag of each
   *  bit. Example: to enable IGMP and ARP trap, set .
   *  trap_enable_mask = SOC_PPD_LLP_TRAP_PORT_ENABLE_ARP |
   *  SOC_PPD_LLP_TRAP_PORT_ENABLE_IGMP;Relevant only For Soc_petra-B,
   *  in T20E has to be SOC_PPD_LLP_TRAP_PORT_ENABLE_ALL
   */
...
typedef struct
{
   ...
   *  SOC_PPD_TRAP_CODE_MY_ARP. ARP-Requests that do not match any
   *  of these IPs will be trapped with trap-code =
   *  SOC_PPD_TRAP_CODE_ARP.
   */
   ...
   *  SOC_PPD_TRAP_CODE_MY_MAC_AND_ARP) if packet has my-MAC and
...
typedef struct
{
   ...
   *  SOC_PPD_L2_NEXT_PRTCL_TYPE enumeration to refer to these
   *  constant values
   */
...
typedef struct
{
   ...
   *  header) equal to this value. See SOC_PPD_L3_NEXT_PRTCL_TYPE
...
typedef struct
{
   ...
   *  SOC_PPD_LLP_TRAP_PROG_TRAP_COND_SELECT. For example, user
   ...
   *  compare failed. See SOC_PPD_LLP_TRAP_PROG_TRAP_COND_SELECT.
