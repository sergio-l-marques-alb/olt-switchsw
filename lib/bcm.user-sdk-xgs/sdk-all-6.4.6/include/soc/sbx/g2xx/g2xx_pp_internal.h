...
/*
 * $Id: g2xx_pp_internal.h,v 1.9 Broadcom SDK $
 *
...
#define SB_G2XX_FE_PP_BYTE_HASH_MASK_GEN(w, x)         \
...
#define SB_G2XX_FE_PP_BIT_HASH_MASK_GEN(w, x)          \
  { uint64 _uuTmp = COMPILER_64_INIT(0,1);             \
    COMPILER_64_SHL(_uuTmp, (39 - (x)));               \
    COMPILER_64_OR(w, _uuTmp);                         \
  }
...
*/
typedef uint32                        soc_sbx_g2xx_pp_layer4_filter_db_t;
typedef uint64                        soc_sbx_g2xx_pp_ip_tunnel_db_t;
typedef uint64                        soc_sbx_g2xx_pp_local_station_match_db_t;
...
typedef struct {
  ...
  uint32                 uBatchGroupRecordSize[SB_G2XX_FE_PP_NUM_OF_BATCH_GROUPS];
  ...
  uint32                 uEgrStartQid;
  uint32                 uEgrEndQid;
  uint32                 uQueueTpidState[SB_G2XX_FE_PP_NUM_OF_QUEUES];
  uint8                  bDebug;
...
sbStatus_t
soc_sbx_g2xx_pp_ipv6_str_sel_set(int unit, uint32 valid);

sbStatus_t
soc_sbx_g2xx_pp_ipv6mc_str_sel_set(int unit, uint32 valid);
