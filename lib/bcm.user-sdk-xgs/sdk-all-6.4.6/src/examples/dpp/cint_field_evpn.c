...
int evpn_egress_filter_db(int unit, int group_priority) {
    ...
    grp_conf.flags = BCM_FIELD_GROUP_CREATE_WITH_MODE | BCM_FIELD_GROUP_CREATE_WITH_ID | BCM_FIELD_GROUP_CREATE_WITH_PRESELSET | BCM_FIELD_GROUP_CREATE_WITH_ASET;
    ...
    for (cur_port = 0; cur_port < 3; cur_port++) {
        ...
        /* Learn Extention */
        result = bcm_field_qualify_LearnSrcPort(unit, ent, ports[cur_port]);
        if (result != BCM_E_NONE) {
            printf("Error, bcm_field_qualify_data $result\n");
            return result;
        }
