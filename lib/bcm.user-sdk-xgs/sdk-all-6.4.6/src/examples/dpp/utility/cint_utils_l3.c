...
int l3__intf_rif__create(int unit, create_l3_intf_s *l3_intf){
    ...
    if (l3_intf->vrf_valid || l3_intf->rpf_valid) {
        ...
        if (l3_intf->ingress_flags & BCM_L3_INGRESS_GLOBAL_ROUTE) {
...
struct create_l3_egress_s {
    ...
    /* FEC */
    bcm_if_t intf; /* Outgoing intf, can be tunnel/rif, relevant for FEC creation */
    bcm_gport_t gport; /* Outgoing port , relevant for FEC creation */
    ...
    bcm_if_t l3_egress_id; /* FEC ID */
    bcm_if_t encap_id; /* ARP ID, may need for allocation ID or for FEC creation */
};
...
int l3__egress__create(int unit, create_l3_egress_s *l3_egress) {
    ...
    /* FEC properties */
    l3eg.intf           = l3_egress->intf;
    l3eg.port           = l3_egress->gport;  
    l3eg.encap_id       = l3_egress->encap_id;
    ...
    l3_egress->l3_egress_id = l3egid;
    l3_egress->encap_id = l3eg.encap_id;
  
    if(verbose >= 1) {
        printf("unit %d: created FEC-id =0x%08x, ", unit, l3_egress->l3_egress_id);        
        printf("encap-id = %08x", l3_egress->encap_id );
    }

    if(verbose >= 2) {
        printf("outRIF = 0x%08x out-port =%d", l3_egress->intf, l3_egress->gport);
    }
...
int l3__egress_only_encap__create(int unit, create_l3_egress_s *l3_egress) {
    ...
    l3eg.flags    = BCM_L3_EGRESS_ONLY | l3_egress->l3_flags;
    l3eg.mac_addr = l3_egress->next_hop_mac_addr; 
    l3eg.vlan     = l3_egress->vlan;      
    l3eg.encap_id = l3_egress->encap_id;
    
    rc = bcm_l3_egress_create(unit, l3_egress->allocation_flags, &l3eg, &l3egid_null);
...
*/
int l3__update_rif_disable_ipmc(int unit, bcm_l3_intf_t* intf, int disable){
    ...
    /* ipmc will be enabled unless got flag "disable ipv4/ipv6 multicast" */
    if (disable) {
