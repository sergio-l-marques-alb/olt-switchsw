...
int erp_port_get(int unit, int *erp_sysport)
{
    int 
        ...
        count_erp = 0;
    bcm_gport_t 
        erp_local_port_gport, erp_sysport_gport;
        
    rv = bcm_port_internal_get(unit, BCM_PORT_INTERNAL_EGRESS_REPLICATION, 1, &erp_local_port_gport, &count_erp);
    if (rv != BCM_E_NONE) {
        printf("Error, bcm_port_internal_get(), rv=%d.\n", rv);
        return rv;
    }
    printf("count_erp=%d, erp_local_port_gport=0x%x\n", count_erp, erp_local_port_gport);
    
    rv = bcm_stk_gport_sysport_get(unit, erp_local_port_gport, &erp_sysport_gport);
    if (rv != BCM_E_NONE) {
        printf("Error, bcm_stk_gport_sysport_get(), rv=%d.\n", rv);
        return rv;
    }
    
    *erp_sysport = BCM_GPORT_SYSTEM_PORT_ID_GET(erp_sysport_gport);
        
    printf("count_erp=%d, erp_local_port_gport=0x%x, erp_sysport_gport=0x%x, *erp_sysport=0x%x,%d\n", count_erp, erp_local_port_gport, erp_sysport_gport, *erp_sysport, *erp_sysport);
