...
int set_wred_settings(int unit, bcm_gport_t gport, int is_enable, int drop_precedence, int max_thresh, int min_thresh, int max_prob, int gain)
{
    ...
    rv = bcm_cosq_gport_discard_set(0,gport,0,&discard);
    if (rv != BCM_E_NONE) {
 ...
 */
int set_vsq_tail_drop_settings(int unit,bcm_gport_t gport,int dp,int max_size)
{
    bcm_error_t rv = BCM_E_NONE;
    uint32 flags;
    bcm_cosq_gport_size_t gport_size;
    int cosq = 0;
    printf("Set Tail drop Settings unit %d, gport 0x%x, max_size %d \n",
        unit,gport,max_size);
 ...
 *  Example run: run_wred_example(0,2,8);
 *  
...
int run_wred_example(int unit,int destination_local_port, int base_queue)
{
    ...
    /* 5. disable tail drop settings so it will be affected by wred only. */
    for (drop_precedence = 0; drop_precedence < 4; drop_precedence++) {
        max_queue_size = 1000000;
        rv = set_vsq_tail_drop_settings(unit,vsqA,drop_precedence,max_queue_size);
        if (rv != BCM_E_NONE) {
            printf("(%s) \n",bcm_errmsg(rv));
            return rv;
        }   
 ...
 *
 * Example run: run_tail_drop_example(0,2,8); 
 *
 * Note: incoming base_queue parameter should be aligned with the destination port id. 
 * For example, when using diag init application, destination port id 2 is aligned with VOQ ID 8. 
 * Assumptions: destination_port_id exists, base_queue is created and connected to destination_port_id. 
...
int run_tail_drop_example(int unit,int destination_port_id,int base_queue)
{
    ...
    /* VSQ A is configured for Traffic class: 0-3. VSQ B is configured for Traffic class: 4-7. */
    BCM_GPORT_UNICAST_QUEUE_GROUP_SET(voq,base_queue);
    ...
    for (dp = 0; dp < 4; dp++) {
        max_queue_size = 500;
        rv = set_vsq_tail_drop_settings(unit,vsqA,dp,max_queue_size);
        if (rv != BCM_E_NONE) {
            printf("(%s) \n",bcm_errmsg(rv));
            return rv;
        }            
        max_queue_size = max_queue_size * 2;
        rv = set_vsq_tail_drop_settings(unit,vsqB,dp,max_queue_size);
        if (rv != BCM_E_NONE) {
            printf("(%s) \n",bcm_errmsg(rv));
            return rv;
        }
