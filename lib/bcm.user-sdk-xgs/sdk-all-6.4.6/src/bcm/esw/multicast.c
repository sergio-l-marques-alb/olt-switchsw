                             ...
                             int *port_count)
{
    ...
    PBMP_ITER(l2_pbmp, port_iter) {
        ...
        if (NULL != port_array) {
            ...
            if ((soc_feature(unit, soc_feature_linkphy_coe) ||
                soc_feature(unit, soc_feature_subtag_coe)) &&
                (port_iter >= min_subport) &&
                (port_iter <= max_subport)) {
...

{
    ...
    if (BCM_TRUNK_INVALID != trunk_id) {
        ...
        _bcm_esw_trunk_local_members_get(unit, trunk_id,
                                         ...
                                         &trunk_local_ports));
        /*
         * Tomahawk and Trident2Plus introduced REPL_PORT_AGG_MAP table.
...
#if defined (BCM_TOMAHAWK_SUPPORT)
                BCM_IF_ERROR_RETURN
                (_bcm_th_set_repl_port_agg_map(unit,local_trunk_member_ports,
                                               trunk_local_ports, trunk_id));
#endif /* BCM_TOMAHAWK_SUPPORT */
            } else if (SOC_IS_TRIDENT2PLUS(unit)) {
#if defined (BCM_TRIDENT2PLUS_SUPPORT)
                 BCM_IF_ERROR_RETURN
                (bcm_td2p_set_repl_port_agg_map(unit,local_trunk_member_ports,
                                               trunk_local_ports, trunk_id));
#endif /* BCM_TRIDENT2PLUS_SUPPORT */
            }
        } 
         ...
         * identical replication list as other members of this trunk
         */
        BCM_IF_ERROR_RETURN(bcm_esw_stk_my_modid_get (unit, &local_modid));
        BCM_IF_ERROR_RETURN
            (bcm_esw_trunk_find(unit, local_modid, port_out, &tgid));
        trunk_local_ports = 1;
        local_trunk_member_ports[0] = port_out;
        if (SOC_IS_TOMAHAWK(unit)) {
#if defined (BCM_TOMAHAWK_SUPPORT)
            BCM_IF_ERROR_RETURN
            (_bcm_th_set_repl_port_agg_map(unit,local_trunk_member_ports,
                                           trunk_local_ports, tgid));
#endif /* BCM_TOMAHAWK_SUPPORT */
        } else if (SOC_IS_TRIDENT2PLUS(unit)) {
#if defined (BCM_TRIDENT2PLUS_SUPPORT)
             BCM_IF_ERROR_RETURN
...
#if defined(BCM_TOMAHAWK_SUPPORT) || defined(BCM_TRIDENT2PLUS_SUPPORT)
            if (BCM_MC_PER_TRUNK_REPL_MODE(unit)) {
                 idx_max = 1;  /* For Tomahawk pick only first trunk member */
            } else 
#endif /* BCM_TOMAHAWK_SUPPORT || BCM_TRIDENT2PLUS_SUPPORT */
            {
...
#if defined (BCM_TOMAHAWK_SUPPORT) || defined (BCM_TRIDENT2PLUS_SUPPORT)
            if (BCM_MC_PER_TRUNK_REPL_MODE(unit)) {
                 /* when TH_MC_PER_TRUNK_REPL_MODE is set, pick only first trunk member */
                 idx_max = 1;
            } else
#endif /* BCM_TOMAHAWK_SUPPORT || BCM_TRIDENT2PLUS_SUPPORT */
            {
...

{
    ...
    if (BCM_MC_PER_TRUNK_REPL_MODE(unit)) {
        if (BCM_TRUNK_INVALID == trunk_id) {
            BCM_IF_ERROR_RETURN(bcm_esw_stk_my_modid_get (unit, &local_modid));
            rv = bcm_esw_trunk_find(unit, local_modid, port, &tgid);
            if (BCM_SUCCESS(rv)) {
                si = &SOC_INFO(unit);
                pipe = si->port_pipe[port_out]; 
#if defined (BCM_TOMAHAWK_SUPPORT)
                if (SOC_IS_TOMAHAWK(unit)) {
                    agg_id = get_free_aggregation_id(unit, pipe);
                }
#endif
#if defined (BCM_TRIDENT2PLUS_SUPPORT)
                if (SOC_IS_TRIDENT2PLUS(unit)) {
                    agg_id = bcm_td2p_get_free_aggregation_id(unit, pipe);
                }
#endif
                if (agg_id == 0xFF) {
                    mmu_port = SOC_INFO(unit).port_l2p_mapping[port_out];
...
#if defined(BCM_TOMAHAWK_SUPPORT) || defined(BCM_TRIDENT2PLUS_SUPPORT)
        if (BCM_MC_PER_TRUNK_REPL_MODE(unit)) {
            /* When per trunk replication mode is enabled, pick only first 
             * trunk member for which replication list has been created
             */
            idx_max = 1;
        } else 
#endif /* BCM_TOMAHAWK_SUPPORT || BCM_TRIDENT2PLUS_SUPPORT */
                            ...
                            bcm_gport_t *port_array, bcm_if_t *encap_id_array)
{
    ...
    for (i = 0; i < port_count ; i++) {
        ...
        if (BCM_TRUNK_INVALID != trunk_id) {
             ...
             * For all members of a trunk belonging to same pipe, REPL_PORT_AGG_MAP
             * table will be programmed with same L3MC-port-Agg-ID. This ID will 
             * be used with L3MC bitmap and will eventually result in single entry
             * in REPL_HEAD table representing all trunk memebers.
             */
            if (BCM_MC_PER_TRUNK_REPL_MODE(unit)) {
                if (SOC_IS_TOMAHAWK(unit)) {
#if defined (BCM_TOMAHAWK_SUPPORT)
                    rv = _bcm_th_set_repl_port_agg_map(unit,
                                              local_trunk_port_array[i],
                                              trunk_local_ports[i], trunk_id);
#endif /* BCM_TOMAHAWK_SUPPORT */
                } else if (SOC_IS_TRIDENT2PLUS(unit)) {
#if defined (BCM_TRIDENT2PLUS_SUPPORT)
                    rv = bcm_td2p_set_repl_port_agg_map(unit,
                                               local_trunk_port_array[i],
                                               trunk_local_ports[i], trunk_id);
#endif /* BCM_TRIDENT2PLUS_SUPPORT */
                }
...
#if defined (BCM_TOMAHAWK_SUPPORT) || defined (BCM_TRIDENT2PLUS_SUPPORT)
                        if (BCM_MC_PER_TRUNK_REPL_MODE(unit)) {
                            /* For Tomahawk/Td2P pick only first trunk member */
                            idx_max = 1;
                        } else
#endif /* BCM_TOMAHAWK_SUPPORT || BCM_TRIDENT2PLUS_SUPPORT */
                        {
...
#if defined (BCM_TOMAHAWK_SUPPORT) || defined (BCM_TRIDENT2PLUS_SUPPORT)
                                if (BCM_MC_PER_TRUNK_REPL_MODE(unit)) {
                                    /* Preserve all trunk members in l3_pbmp */                
                                    for (idx = 0; idx < trunk_local_ports[i]; idx++) {
                                        BCM_PBMP_PORT_ADD(l3_pbmp, local_trunk_port_array[i][idx]);
                                    }
                                }
#endif /* BCM_TOMAHAWK_SUPPORT || BCM_TRIDENT2PLUS_SUPPORT */
                                break;
...
_bcm_esw_fabric_multicast_destroy(int unit, bcm_multicast_t group)
{
    ...
    BCM_IF_ERROR_RETURN(
            _bcm_fabric_multicast_ipmc_group_type_set(unit, fabric_index));
