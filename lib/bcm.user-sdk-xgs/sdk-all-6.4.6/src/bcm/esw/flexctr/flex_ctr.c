...
bcm_stat_counter_map_t *stat_counter_map[BCM_MAX_STAT_COUNTER_IDS] = {0};
static bcm_error_t _bcm_esw_get_ingress_ifp_table_from_pipe(int pipe, soc_mem_t *ingress_table);
void _bcm_fill_stat_counter_map(int id, uint32 mode, uint32 pool, uint32 base_idx,
     ...
     if (!soc_feature(unit, soc_feature_flex_counter_opaque_stat_id)) {
        ...
        *stat_counter_id = 0;
        for(i = 1; i < BCM_MAX_STAT_COUNTER_IDS; ++i) {
            if (stat_counter_map[i] && stat_counter_map[i]->used) {
                if ((stat_counter_map[i]->group == group) && 
                (stat_counter_map[i]->object == object) &&
                (stat_counter_map[i]->mode == mode) && 
                (stat_counter_map[i]->pool == pool_number) && 
                (stat_counter_map[i]->base_idx == base_idx)) {
                    break;
                }
                continue;
            } else if (stat_counter_map[i]) {
                break;
            } else if(!(stat_counter_map[i])) {
                stat_counter_map[i] = 
                ...
                sal_memset(stat_counter_map[i], 0, sizeof(bcm_stat_counter_map_t));
        ...
        *stat_counter_id = i;
        if (stat_counter_map[i] != NULL) {
            stat_counter_map[i]->mode = mode;
            stat_counter_map[i]->group = group;
            stat_counter_map[i]->object = object;
            stat_counter_map[i]->pool = pool_number;
            stat_counter_map[i]->base_idx = base_idx;
            stat_counter_map[i]->used = 1;
        }
#endif
    }
}
...
bcm_field_qualify_t 
_bcm_esw_stat_get_field_stage_from_stat_ctr(uint32 stat_counter_id) 
{
#ifdef BCM_TOMAHAWK_SUPPORT
    if (!stat_counter_map[stat_counter_id]) {
        LOG_ERROR(BSL_LS_BCM_FLEXCTR,
            (BSL_META_U(0,
            "Failed to get counter id ")));
        return 0;
    ...
    switch(stat_counter_map[stat_counter_id]->object) {
     ...
     if (!soc_feature(unit, soc_feature_flex_counter_opaque_stat_id)) {
         ...
         *base_idx    = (stat_counter_id & BCM_STAT_FLEX_BASE_IDX_MASK);
         ...
         if ((stat_counter_id > BCM_MAX_STAT_COUNTER_IDS) ||
             (stat_counter_id <= 0) || (stat_counter_map[stat_counter_id] == NULL)) {
             LOG_ERROR(BSL_LS_BCM_FLEXCTR,
                      (BSL_META_U(0,
                                  "Failed to get counter id info %d"), stat_counter_id));
              return;
         }
         *mode        = (bcm_stat_flex_mode_t ) (stat_counter_map[stat_counter_id]->mode); 
         *group       = (bcm_stat_group_mode_t) (stat_counter_map[stat_counter_id])->group;
         *pool_number = (stat_counter_map[stat_counter_id]->pool); 
         *object      = (bcm_stat_object_t) (stat_counter_map[stat_counter_id]->object);
         *base_idx    = (stat_counter_map[stat_counter_id]->base_idx);
#endif
    }
}
/*
...
#ifdef BCM_TOMAHAWK_SUPPORT
        if (SOC_IS_TOMAHAWK(unit)) {
            if((stat_counter_map[stat_counter_id]) != NULL) {
                sal_free(stat_counter_map[stat_counter_id]);
                stat_counter_map[stat_counter_id] = NULL;
            }
        }
#endif
            }
        ...
        for (mode = 0; mode < BCM_STAT_FLEX_COUNTER_MAX_MODE; mode++) {
             if (_bcm_esw_stat_flex_get_ingress_mode_info(
                 ...
                 if ((flex_ingress_mode->num_selectors == num_selectors) &&
                     (flex_ingress_mode->total_counters == total_counters)) {
    ...
    uint32 *mode_id)
{
    ...
    *mode_id = 0; 
    ...
    if ((rv = _bcm_esw_stat_group_mode_id_check(unit, flags, total_counters, 
                                      num_selectors, attr_selectors, mode_id)) != BCM_E_NONE) {
         return rv;
    }
    ...
    uint32 mode_id)
{
    ...
    if (flags & BCM_STAT_GROUP_MODE_INGRESS) {
        if (soc_feature(unit, soc_feature_use_flex_ctr_oam_lm)) {
            memset(&flex_ingress_mode, 0, sizeof(flex_ingress_mode));
    ...
    uint32            *num_entries)
{
    ...
    rv= _bcm_esw_stat_flex_get_ingress_mode_info(
             unit,mode_id,flex_ingress_mode);
    if (!BCM_SUCCESS(rv)) {
        sal_free(flex_ingress_mode);
        return rv;     
    }
    ...
    if ((cmode.hint.type == bcmStatGroupAllocHintIngressFieldGroup)) {
       if ((flex_ingress_mode->attr_selectors[0].attr == bcmStatGroupModeAttrFieldIngressColor) &&
             (BCM_STAT_GROUP_MODE_ATTR_ALL_VALUES == flex_ingress_mode->attr_selectors[0].attr_value)) {
           ifp_counter_color.map[ctr_idx] = BCM_STAT_FLEX_COLOR_GREEN | BCM_STAT_FLEX_COLOR_YELLOW |
                                            BCM_STAT_FLEX_COLOR_RED;
       } else {
           for(i = 0; i < flex_ingress_mode->num_selectors; ++i) {
                if (flex_ingress_mode->attr_selectors[i].attr == bcmStatGroupModeAttrFieldIngressColor) {
                   ctr_idx = flex_ingress_mode->attr_selectors[i].counter_offset;
                   ifp_counter_color.map[ctr_idx] |= (1 << flex_ingress_mode->attr_selectors[i].attr_value);
    ...
    uint32 *mode_id)
{
    ...
    if (rv != BCM_E_EXISTS) {
        rv = _bcm_esw_stat_group_mode_id_create(unit, stat_config->flags, 
                                        stat_config->total_counters,
                                        nsel, attr_sel,
                                        mode_id);
    }
    ...
    if ((stat_config->hint.type == bcmStatGroupAllocHintIngressFieldGroup) ||
        ...
        if (_bcm_esw_stat_flex_get_ingress_mode_info(
           ...
           _bcm_esw_stat_flex_update_ingress_flex_info(
                unit, *mode_id, stat_config->flags, num_selectors,
                attr_sel);
        }
    ...
    uint32 *actual_num_selectors)
{
    ...
    if ((mode_id < BCM_STAT_FLEX_CUSTOM_INGRESS_COUNTER_MAX_MODE) &&
       (mode_id >= BCM_CUSTOM_INGRESS_MODE_START)) {
        rv = _bcm_esw_stat_flex_get_custom_ingress_mode_info(unit, ing_mode, 
                                                          &custom_ing_mode);
...
#ifdef BCM_TOMAHAWK_SUPPORT
void _bcm_fill_stat_counter_map(int id, uint32 mode, uint32 pool, 
                                uint32 base_idx, bcm_stat_object_t object,
                                bcm_stat_group_mode_t group)
{
            if (!stat_counter_map[id]) {
               stat_counter_map[id] = 
               sal_alloc(sizeof(bcm_stat_counter_map_t),"attr");
               
               if (stat_counter_map[id] == NULL) {
                   LOG_ERROR(BSL_LS_BCM_FLEXCTR,
                       (BSL_META_U(0,
                       "Memory alloc failed! ")));
                       return;
            ...
            stat_counter_map[id]->mode = mode;
            stat_counter_map[id]->group = group;
            stat_counter_map[id]->object = object;
            stat_counter_map[id]->pool = pool;
            stat_counter_map[id]->base_idx = base_idx;
            stat_counter_map[id]->used = 1;
}

void _bcm_get_stat_counter_map(int id, bcm_stat_counter_map_t *stat_ctr_map)
{
            if (stat_counter_map[id] == NULL) {
               stat_ctr_map->used = 0;
               return;
            }
            stat_ctr_map->mode = stat_counter_map[id]->mode;
            stat_ctr_map->group = stat_counter_map[id]->group;
            stat_ctr_map->object = stat_counter_map[id]->object;
            stat_ctr_map->pool = stat_counter_map[id]->pool;
            stat_ctr_map->base_idx = stat_counter_map[id]->base_idx;
            stat_ctr_map->used = stat_counter_map[id]->used;
}
#endif
