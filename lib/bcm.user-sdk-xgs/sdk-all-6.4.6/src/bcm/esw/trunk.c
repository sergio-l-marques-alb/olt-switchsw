...
bcm_esw_trunk_init(int unit)
{
	...
	if (!SOC_WARM_BOOT(unit) &&
        (SOC_CONTROL(unit)->soc_flags & SOC_F_RE_INITED)) {
        int rv;
#ifndef BCM_TH_LATENCY_EXCLUDE
        rv = _bcm_esw_trunk_hwmem_clear(unit, trunk_mem);
#else
        rv = _bcm_esw_trunk_hwmem_clear(unit, TRUNK_GROUPm);
#endif
        if (BCM_FAILURE(rv)) {
                ...
                if (SOC_IS_FBX(unit) || SOC_IS_RAVEN(unit)) { 
                    ...
                    if (soc_feature(unit, soc_feature_axp)) {
                        if (soc_mem_is_valid(unit, AXP_WTX_TRUNK_GROUP_BITMAPm)) {
                            SOC_IF_ERROR_RETURN
                                (soc_mem_clear(unit, AXP_WTX_TRUNK_GROUP_BITMAPm,
                                        MEM_BLOCK_ALL, 0));
                        }
                        if (soc_mem_is_valid(unit, AXP_WTX_TRUNK_BLOCK_MASKm)) {
                            SOC_IF_ERROR_RETURN
                                (soc_mem_clear(unit, AXP_WTX_TRUNK_BLOCK_MASKm,
                                    MEM_BLOCK_ALL, 0));
                        }
                    }
                        ...
                        bcm_switch_agm_id_t agm_id)
{
    ...
    BCM_IF_ERROR_RETURN(_bcm_th_trunk_agm_update(unit, tid, agm_mnt.attr.agm_type, 0, 0));
