...
_bcm_pfc_deadlock_recovery_end(int unit, int cos, bcm_port_t port)
{
    ...
    /* 3.a: For that port, set ignore_pfc_xoff = 1 (per port reg) */
    rval = 0;
    SOC_IF_ERROR_RETURN(
    ...
    SOC_IF_ERROR_RETURN(
     ...
     /* 3.b: Mask the Intr DD_TIMER_MASK_A|_B by setting 1 (pbmp) */
    COMPILER_64_ZERO(rval64);

    SOC_IF_ERROR_RETURN(
        soc_reg_get(unit, hw_res->timer_mask[cos], REG_PORT_ANY,
                    pipe, &rval64));

    COMPILER_64_NOT(temp_mask64);
    COMPILER_64_AND(rval64, temp_mask64);
    SOC_IF_ERROR_RETURN(
        soc_reg_set(unit, hw_res->timer_mask[cos], REG_PORT_ANY, pipe, rval64));

    /* 3.c: Turn timer off DD_TIMER_ENABLE_A|_B by setting 0 (pbmp) */
    COMPILER_64_ZERO(rval64);

    SOC_IF_ERROR_RETURN(
        soc_reg_get(unit, hw_res->timer_en[cos], REG_PORT_ANY,
                    pipe, &rval64));

    COMPILER_64_OR(rval64, temp_en64);
    SOC_IF_ERROR_RETURN(
        soc_reg_set(unit, hw_res->timer_en[cos], REG_PORT_ANY, pipe, rval64));
...
_bcm_pfc_deadlock_recovery_begin(int unit, int cos, int pipe, int pipe_mmu_port)
{
    ...
    SOC_IF_ERROR_RETURN(
        soc_reg_set(unit, hw_res->timer_mask[cos], REG_PORT_ANY,
                    pipe, rval64));
    ...
    SOC_IF_ERROR_RETURN(
        soc_reg_set(unit, hw_res->timer_en[cos], REG_PORT_ANY,
                    pipe, rval64));
    ...
    for (i = 0; i < pfc_deadlock_control->pfc_deadlock_cos_max; i++) {
        ...
        if (pfc_deadlock_control->hw_cos_idx_inuse[i] == TRUE) {
            ...
            for (pipe = 0; pipe < NUM_PIPE(unit); pipe++) {
                SOC_IF_ERROR_RETURN(
                    soc_reg_get(unit, hw_res->timer_status[i], REG_PORT_ANY,
                                0, &status64));
                SOC_IF_ERROR_RETURN(
                    soc_reg_get(unit, hw_res->timer_mask[i], REG_PORT_ANY,
                                0, &mask64));
                                  ...
                                  uint8 *enable_status)
{
    ...
    SOC_IF_ERROR_RETURN(soc_reg_get(unit, reg, 0, pipe, &rval64));

    if (operation == _bcmPfcDeadlockOperGet) { /* GET */
        if (enable_status) {
            *enable_status =
                PBMP_MEMBER(pfc_deadlock_pri_config->deadlock_ports,
                            local_port);
        }
        if (config) {
            if (mmu_port < 32) {
        ...
        SOC_IF_ERROR_RETURN(soc_reg_set(unit, reg, 0, pipe, rval64));
