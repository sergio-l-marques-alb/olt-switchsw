    ...
    for (chunk_index = 0; chunk_index < num_chunks; chunk_index++) {
        ...
        for (i = 0; i < (entry_index_max - entry_index_min + 1); i++) {
            ...
            if (soc_feature(unit, soc_feature_key_type_valid_on_vp_vlan_membership)) {
                ...
                is_glp_vlan = (key_type == _bcm_vp_vlan_mbrship_glp_vlan_type);
            }
            ...
            rv = bcm_td2_ing_vp_vlan_membership_delete(unit, vp, vlan, key_type);
            if (BCM_FAILURE(rv)) {
                goto cleanup;
            }
        }
...
bcm_td2_egr_vp_vlan_membership_delete_all(int unit, bcm_vlan_t vlan)
{
    ...
    for (chunk_index = 0; chunk_index < num_chunks; chunk_index++) {
        ...
        for (i = 0; i < (entry_index_max - entry_index_min + 1); i++) {
            ...
            rv = bcm_td2_egr_vp_vlan_membership_delete(unit, vp, vlan, key_type);
            if (BCM_FAILURE(rv)) {
                goto cleanup;
            }
        }
