...
_bcm_td2_vxlan_match_delete(int unit,  int vp , bcm_vxlan_port_t vxlan_port)
{
    ...
    if  (vxlan_info->match_key[vp].flags == _BCM_VXLAN_PORT_MATCH_TYPE_VLAN) {
        ...
        if (soc_feature(unit, soc_feature_vrf_aware_vxlan_termination)) {
            soc_MPLS_ENTRYm_field32_set(unit, &ment, VXLAN_SIP__OVIDf,
                              vxlan_info->vxlan_tunnel_term[tunnel_index].vlan);
        }
#endif
        soc_MPLS_ENTRYm_field32_set(unit, &ment, KEY_TYPEf, _BCM_VXLAN_KEY_TYPE_TUNNEL);
        soc_MPLS_ENTRYm_field32_set(unit, &ment, VALIDf, 1);
...
_bcm_td2_vxlan_match_get(int unit, bcm_vxlan_port_t *vxlan_port, int vp)
{
    ...
    }else if (_bcm_vp_used_get(unit, vp, _bcmVpTypeShare) &&
...
bcm_td2_vxlan_port_get(int unit, bcm_vpn_t vpn, bcm_vxlan_port_t *vxlan_port)
{
    ...
    if (vpn != BCM_VXLAN_VPN_INVALID) {
       ...
       if ((vp != default_vp) && (!_bcm_vp_used_get(unit, vp, _bcmVpTypeShare))) {
           return BCM_E_PARAM;
       }
    }
