...
#define _TD2P_NUM_PIPES 2
#define _TD2P_PORTS_PIPE 52
#define _TD2P_NUM_TRUNKS 1024

typedef struct _bcm_td2p_aggid_info_s {
    uint8 free_aggid[_TD2P_PORTS_PIPE];
} _bcm_td2p_aggid_info_t;

typedef struct _bcm_td2p_trunk_aggid_map_s {
    uint8 agg_id;
} _bcm_td2p_trunk_aggid_map_t;

typedef struct _bcm_td2p_trunk_aggid_info_s {
    _bcm_td2p_trunk_aggid_map_t trunk_aggid_map[_TD2P_NUM_TRUNKS];
} _bcm_td2p_trunk_aggid_info_t;


STATIC _bcm_td2p_aggid_info_t *td2p_free_aggid[BCM_MAX_NUM_UNITS];
STATIC _bcm_td2p_trunk_aggid_info_t *td2p_trunk_aggid_info[BCM_MAX_NUM_UNITS];

/*
 * Function:
 *      bcm_td2p_aggregation_id_list_detach
 * Purpose:
 ...
 */
int
bcm_td2p_aggregation_id_list_detach(int unit)
{
	if (td2p_free_aggid[unit] != NULL) {
		sal_free(td2p_free_aggid[unit]);
		td2p_free_aggid[unit] = NULL;
	}
    return BCM_E_NONE;
}
...
/*
 * Function:
 *      bcm_td2p_aggregation_id_list_init
 * Purpose:
...
int
bcm_td2p_aggregation_id_list_init(int unit)
{
    int   rv = BCM_E_NONE;
    ...
    if (td2p_free_aggid[unit] != NULL) {
        rv = bcm_td2p_aggregation_id_list_detach(unit);
        BCM_IF_ERROR_RETURN(rv);
    }

    td2p_free_aggid[unit] = sal_alloc((_TD2P_NUM_PIPES * 
                                 sizeof(_bcm_td2p_aggid_info_t)), "free_aggid");
    if (td2p_free_aggid[unit] == NULL) {
        return BCM_E_MEMORY;
    }
    sal_memset(td2p_free_aggid[unit], 0xFF, _TD2P_NUM_PIPES * 
               sizeof(_bcm_td2p_aggid_info_t));
...
/*
 * Function:
 *      bcm_td2p_aggid_trunk_map_detach
 * Purpose:
 *      Free aggregation ID list.
 * Parameters:
 *      unit  - (IN) Unit number.
 ...
 */
int
bcm_td2p_aggid_trunk_map_detach(int unit)
{
	if (td2p_trunk_aggid_info[unit] != NULL) {
		sal_free(td2p_trunk_aggid_info[unit]);
		td2p_trunk_aggid_info[unit] = NULL;
	}
    return BCM_E_NONE;
}
...
int
bcm_td2p_aggid_trunk_map_init(int unit)
{
    int   rv = BCM_E_NONE;
    ...
    if (td2p_trunk_aggid_info[unit] != NULL) {
        rv = bcm_td2p_aggid_trunk_map_detach(unit);
        BCM_IF_ERROR_RETURN(rv);
    }

    td2p_trunk_aggid_info[unit] = sal_alloc((_TD2P_NUM_PIPES * 
                                       sizeof (_bcm_td2p_trunk_aggid_map_t) *
                                       sizeof(_bcm_td2p_trunk_aggid_info_t)),
                                       "trunk_aggid_info");
    if (td2p_trunk_aggid_info[unit] == NULL) {
        return BCM_E_MEMORY;
    }
    sal_memset(td2p_trunk_aggid_info[unit], 0xFF, _TD2P_NUM_PIPES * 
               sizeof (_bcm_td2p_trunk_aggid_map_t) *
               sizeof(_bcm_td2p_trunk_aggid_info_t));
 ...
 *      BCM_E_XXX
 */
int bcm_td2p_get_free_aggregation_id(int unit, int pipe) {
    int aggid = 0xFF;
    _bcm_td2p_aggid_info_t *aggid_info;
    int id;

    aggid_info = &td2p_free_aggid[unit][pipe];

    for (id = 0; id < _TD2P_PORTS_PIPE; id++) {
...
static uint8 get_aggid_from_trunkid(int unit, int pipe, int tgid) {
     ...
     uint8 aggid;
    _bcm_td2p_trunk_aggid_info_t *aggid_map;
    
    aggid_map = &td2p_trunk_aggid_info[unit][pipe];
...
/*
 * Function:
 *      bcm_td2p_set_repl_port_agg_map
 * Purpose:
...
int bcm_td2p_set_repl_port_agg_map(int unit, const bcm_port_t *local_member_array,
    ...
    for (ltp_idx = 0; ltp_idx < local_member_count; ltp_idx++) {
        ...
        if (pipe != prev_pipe) {
            ...
            if (t_aggid != 0xFF) {
                ...
                SOC_IF_ERROR_RETURN
                    (soc_reg32_set(unit, MMU_TOQ_REPL_PORT_AGG_MAPr,
                                   port, 0, regval));
            }
