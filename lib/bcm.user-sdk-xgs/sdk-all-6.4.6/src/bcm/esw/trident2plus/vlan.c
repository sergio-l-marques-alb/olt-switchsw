...
bcm_td2p_vlan_vp_group_get(
      int unit,           /* unit number */
      soc_mem_t vlan_vfi_mem, /* either VLAN_TABm/EGR_VLANm or VFIm/EGR_VFIm */
      int vlan,           /* index to the above memory table */
      int vp_group,       /* the VP group number */
...
)
{
    ...
    soc_mem_t vlan_vfi_mbrship_mem;
    ing_vlan_vfi_membership_entry_t vlan_vfi_entry;
    ...
    if ((vlan_vfi_mem == VLAN_TABm) || (vlan_vfi_mem == VFIm)) {
        if((vlan_vfi_mem == VLAN_TABm)) {
            vlan_vfi_mem = VLAN_MPLSm;
            entry = &vlan_mpls_entry;
        } else {
            entry = &vfi_entry;
        }
        vlan_vfi_mbrship_mem = ING_VLAN_VFI_MEMBERSHIPm;

    } else {
        if(vlan_vfi_mem == EGR_VLANm) {
            entry = &egr_vlan_entry;
        } else {
            entry = &egr_vfi_entry;
        }
        vlan_vfi_mbrship_mem = EGR_VLAN_VFI_MEMBERSHIPm;

    }

    if ((rv = soc_mem_read(unit, vlan_vfi_mem, MEM_BLOCK_ANY, vlan,
                    entry)) != BCM_E_NONE) {
        return rv;
    }
    soc_mem_field_get(unit, vlan_vfi_mem, (uint32 *)entry,
            MEMBERSHIP_PROFILE_PTRf, &profile_idx);
    ...
    *enable = 0;
    ...
    if ((vlan_vfi_mem == VLAN_TABm) || (vlan_vfi_mem == VFIm)) {
        if ((rv = soc_mem_read(unit, vlan_vfi_mbrship_mem, MEM_BLOCK_ANY, profile_idx,
                        &vlan_vfi_entry)) != BCM_E_NONE) {
            return rv;
        }
    } else {
        if ((rv = soc_mem_read(unit, vlan_vfi_mbrship_mem, MEM_BLOCK_ANY, profile_idx,
                        &egr_vlan_vfi_entry)) != BCM_E_NONE) {
            return rv;
        }
    }

    if ((vlan_vfi_mem == VLAN_TABm) || (vlan_vfi_mem == VFIm)) {
        soc_mem_field_get(unit, vlan_vfi_mbrship_mem, (uint32 *)&vlan_vfi_entry,
                            VP_GROUP_BITMAPf, vp_group_bitmap);
    } else {
        soc_mem_field_get(unit, vlan_vfi_mbrship_mem, (uint32 *)&egr_vlan_vfi_entry,
                            VP_GROUP_BITMAPf, vp_group_bitmap);
    }
      ...
      int *port_cnt)      /* point to the first available array entry */
{
    ...
    for (vpg = 0; vpg < num_vp_groups; vpg++) {
        ...
        if (soc_feature(unit,
                  ...
                  bcm_td2p_ing_vp_group_unmanaged_get(unit)) {
            rv = bcm_td2p_vlan_vp_group_get(unit,
                                            (is_vfi ? VFIm :VLAN_TABm),
                                            vlan,
                                            vpg, &ing_enable);
                                              ...
                                              void *entry_data)
{
    ...
    if (!egress) {
        _bcm_vlan_vfi_membership_profile_entry_op(unit, &entry_data, 1, 
                                               BCMI_VP_GROUP_ADD, 
                                               BCMI_VP_GROUP_INGRESS,
             ...
             */
            if(cur_prof_index != 0) {
                _bcm_vlan_vfi_membership_profile_entry_op(unit, NULL, 1, 
                            ...
                            int add, int egress, int flags)
{
    ...
    if (!egress) {
        ...
        if(!_BCM_VPN_VFI_IS_SET(vlan_vfi)) {
            _bcm_td2p_vp_group_ut_state_get(unit, vlan_vfi, old_vp_group,
                    &ut_state);
        }

        /* Compare the existing vlan/vfi bitmaps and the ut_state
           on this group */
        /* Pick out the untagged flag first*/
        ut_flag_set = flags & BCM_VLAN_GPORT_ADD_UNTAGGED;
        /* Check if the user-preference matches with the group untagged state.*/
        if ((ut_flag_set && ut_state) || ((!ut_flag_set) && (!ut_state))) {
            /* Check if the vlan/vfi bitmap matches*/
            if (SHR_BITEQ_RANGE(new_vlan_vfi_bitmap,
                    EG_VP_GROUP(unit, old_vp_group)->vlan_vfi_bitmap,
                    0, (num_vlan + num_vfi))) {
                                        ...
                                        int *ifilter)
{
    ...
    uint32          *vp_entry = NULL, *gpp_stm_entry = NULL;
    _bcm_vp_group_port_type_t port_type;
    egr_dvp_attribute_entry_t dvp_entry;
        ...
        if(!egress) {
            ...
            */
            rv = bcm_esw_port_egr_lport_field_get(unit, port,
                                                  EGR_LPORT_PROFILE_LPORT_TAB,
                                                  gpp_enfilter_field, (uint32*)ifilter);
    ...
    if(port_type == _bcmVpGrpPortVp) {
        ...
        if (!egress) {
            ...
            /* Currently '_bcm_esw_egr_port_tab_set' is defined only for KT2,
               ...
               generic and call this LPORT config from there.
            */
            rv = bcm_esw_port_egr_lport_field_set(unit, port,
                                                  EGR_LPORT_PROFILE_LPORT_TAB,
                                                  gpp_enfilter_field, ifilter);
        }
    }
