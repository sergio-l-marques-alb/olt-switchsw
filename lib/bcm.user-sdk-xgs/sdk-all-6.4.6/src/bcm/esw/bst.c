                         ...
                         _bcm_bst_thread_oper_t operation)
{
    ...
    bcm_bst_stat_id_t bst_threshold_id[] = {bcmBstStatIdEgrPool, 
                                            bcmBstStatIdEgrMCastPool, 
                                            bcmBstStatIdMcast};   
                                            /* Need to set bst threshold id 
                                            when bst_trac_en is disabled or 
                                            enabled. Olny add bst id if relative
                                            bst id is set*/
    _bcm_bst_resource_info_t *resInfo;
    int ei, pipe;
    uint32 bid_index, index_max;
    uint32 cells = BST_THRESHOLD_MAX_HIGH;
    bcm_cosq_bst_profile_t profile;
    bst_info = _BCM_UNIT_BST_INFO(unit);
    ...
    case bcmSwitchBstEnable:
        ...
        /*db/qe bst_trigger shoud be blocked when bst_trac_en is disabled or 
          enabled*/
                ...
                for (ei = resInfo->index_min; ei <= index_max ; ei++) {
                    if (resInfo->flags & _BCM_BST_CMN_RES_F_PROFILEID) {
                        for (pipe = 0; pipe < NUM_PIPE(unit); pipe++) {
                            SOC_IF_ERROR_RETURN(
                                _bcm_bst_cmn_profile_set_hw(unit, pipe, ei,
                                                bst_threshold_id[bid_index], 
                                                &profile, NULL));
                        }
                    }
                    else {
                        for (pipe = 0; pipe < NUM_PIPE(unit); pipe++) {
                            ...
                            if ((resInfo->stat_mem[pipe] != INVALIDm) || 
                                ...
                                SOC_IF_ERROR_RETURN(
                                    _bcm_bst_cmn_io_op(unit, _bcmBstOpSet, ei,
                                               bst_threshold_id[bid_index],
                                               resInfo->stat_mem[pipe],
                                               resInfo->stat_reg[pipe],
                                               resInfo->stat_field,
                                               &cells));
                            }
                        }
...
#if defined(BCM_TRIUMPH3_SUPPORT) || defined(BCM_TRIDENT2_SUPPORT) || defined(BCM_TOMAHAWK_SUPPORT)
STATIC int
...
int _bcm_bst_detach(int unit)
{
    ...
    _BCM_BST_RESOURCE_ITER(bst_info, bid) {
        ...
        soc_profile_reg_destroy(unit, &resInfo->iprofile_r);
        resInfo->profile_r = NULL;
        resInfo->profile_m = NULL;
    }
                        ...
                        bcm_bst_stat_id_t bid, uint32 options, uint64 *pvalue)
{
    ...
    void *cb_data = NULL; 
    int bcm_rv, rcb = 0, ii;
    int start_hw_index = 0, end_hw_index = 0, pipe, pipe_offset;
