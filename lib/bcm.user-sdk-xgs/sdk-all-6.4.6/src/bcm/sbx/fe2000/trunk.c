/*
 * $Id: trunk.c,v 1.78 Broadcom SDK $
...
#ifdef BCM_FE2000_P3_SUPPORT
int
_bcm_fe2000_g2p3_trunk_outHdr_hw_set(int unit, uint32 eIndex,
                                     uint32 eteAddr)
{
    int                    rv = BCM_E_NONE;
 ...
 */
int
_bcm_fe2000_trunk_outHdr_set(int unit, uint32 eIndex, uint32 eteAddr)
{
...
#ifdef BCM_FE2000_P3_SUPPORT
int
_bcm_fe2000_g2p3_trunk_fte_hw_set(int unit, uint32 eIndex, bcm_trunk_t tid,
                                  uint32 outHdrIdx)
{
 ...
 */
int
_bcm_fe2000_trunk_fte_set(int unit, uint32 eIndex, bcm_trunk_t tid,
                          uint32 outHdrIdx)
{
    int rv = BCM_E_UNAVAIL;
 ...
 */
int
_bcm_fe2000_trunk_fte_inValidate(int unit, uint32 eIndex)
{
    int rv;
...
#ifdef BCM_FE2000_P3_SUPPORT
int
_bcm_fe2000_g2p3_trunk_ingLag_hw_set(int unit, uint32 eIndex, uint32 qid,
                                     uint32 outHdrIdx)
{
    int                  rv;
 ...
 */
int
_bcm_fe2000_trunk_ingLag_set(int unit, uint32 eIndex, uint32 qid,
                             uint32 outHdrIdx)
{
    int rv = BCM_E_UNAVAIL;
...
#ifdef BCM_FE2000_P3_SUPPORT
int
_bcm_fe2000_g2p3_trunk_port2Etc_hw_set(int unit, uint32 port, uint32 sid)
{
    int                 rv;
 ...
 */
int
_bcm_fe2000_trunk_port2Etc_set(int unit, uint32 port, uint32 sid)
{
    int rv = BCM_E_UNAVAIL;
...
#ifdef BCM_FE2000_P3_SUPPORT
int
_bcm_fe2000_g2p3_trunk_egrPort2Etc_hw_set(int unit, uint32 eIndex,
                                          uint32 sid)
{
    int                 rv;
 ...
 */
int
_bcm_fe2000_trunk_egrPort2Etc_set(int unit, uint32 eIndex, uint32 sid)
{
    int rv = BCM_E_UNAVAIL;
...
int
_bcm_fe2000_trunk_set(int unit, bcm_trunk_t tid, bcm_trunk_add_info_t *add_info, int *port_change)
{
    trunk_cntl_t       *tc;
    trunk_private_t    *ti;
    bcm_port_t          port;
...
bcm_fe2000_trunk_vlan_remove_port(int unit, bcm_vlan_t vid, bcm_port_t port)
{
    LOG_VERBOSE(BSL_LS_BCM_TRUNK,
                (BSL_META_U(unit,
                            "%s(unit=%d, vid=%d, port=%d) - This API is deprecated.\n"),
...
STATIC int
_bcm_fe2000_trunk_create_id(int unit, bcm_trunk_t tid)
{
    trunk_private_t        *ti;
    uint32                eIndex;
    uint32                outHdrIdx;
...
int
bcm_fe2000_trunk_create(int unit, uint32 flags, bcm_trunk_t *tid)
{
    trunk_cntl_t       *tc;
    trunk_private_t    *ti;
    ...
    if (flags & BCM_TRUNK_FLAG_WITH_ID) {
        return _bcm_fe2000_trunk_create_id(unit, *tid);
    }
...
_bcm_fe2000_g2p3_trunk_psc_set(int unit, bcm_trunk_t tid, int psc)
{
    ...
    if (rc != SOC_E_NONE){
        ...
        LOG_ERROR(BSL_LS_BCM_TRUNK,
                  (BSL_META_U(unit,
                              "soc_sbx_g2p3_pp_hash_templates_set failed: flags 0x%x, %s (0x%x)\n"),
    ...
    if (psc != BCM_TRUNK_PSC_VLANINDEX) {
        ...
        if (rc != SOC_E_NONE){
            ...
            LOG_ERROR(BSL_LS_BCM_TRUNK,
                      (BSL_META_U(unit,
                                  "soc_sbx_g2p3_pp_psc_set failed: flags 0x%x, %s (0x%x)\n"),
                       flags, bcm_errmsg(result), result));
            return BCM_E_INTERNAL;
        }   
    }
...
_bcm_fe2000_g2p3_trunk_psc_recover(int unit)
{
    ...
    if (sb_rv != SB_OK) {
        ...
        LOG_ERROR(BSL_LS_BCM_TRUNK,
                  (BSL_META_U(unit,
                              "soc_sbx_g2p3_pp_hash_templates_get failed: %s\n"),
                   bcm_errmsg(rv)));
        return BCM_E_INTERNAL;
    }
 ...
 */
int
bcm_fe2000_trunk_set_old(int unit, bcm_trunk_t tid, bcm_trunk_add_info_t *add_info)
{
    trunk_cntl_t       *tc;
    trunk_private_t    *ti;
    ...
    if (TRUNK_TID_VALID(unit, tid)) {
        ...
        if (TRUNK_PORTCNT_VALID(unit, add_info->num_ports)) {
            result = _bcm_fe2000_trunk_set(unit, tid, add_info, &port_change);
            if (BCM_E_NONE == result) {
                ti->in_use  = TRUE;
            }
            if (port_change == 0) {
 ...
 */
int 
bcm_fe2000_trunk_set(int unit, bcm_trunk_t tid, bcm_trunk_info_t *trunk_info,
        int member_count, bcm_trunk_member_t *member_array)
{
    ...
    status = bcm_fe2000_trunk_set_old(unit, tid, add_info);
    sal_free(add_info);
    return status;
}               
 ...
 */
int 
bcm_fe2000_trunk_get(int unit, bcm_trunk_t tid, bcm_trunk_info_t *trunk_info,
        int member_max, bcm_trunk_member_t *member_array, int *member_count)
{
    ...
    status = bcm_fe2000_trunk_get_old(unit, tid, add_info);
...
bcm_fe2000_trunk_destroy(int unit, bcm_trunk_t tid)
{
    ...
    if (ti->trunk_id != BCM_TRUNK_INVALID) {
        if (0 < ti->num_ports) {
            ...
            add_info->psc = tc->psc;
            (void)bcm_fe2000_trunk_set_old(unit, tid, add_info);
        }
...
_bcm_fe2000_trunk_recover(int unit)
{
    ...
    for (lagi = 0; lagi < lagi_max; lagi++) {
        ...
        LOG_VERBOSE(BSL_LS_BCM_TRUNK,
                    (BSL_META_U(unit,
                                "Adding port_num=%d mod/port=%d/%d "
                                 "from lag[0x%x]\n"),
                     ti->num_ports,
...
bcm_fe2000_trunk_init(int unit)
{
    ...
    tc->invalid_oi = soc_sbx_g2p3_oi2e_table_size_get(unit) + 1;
