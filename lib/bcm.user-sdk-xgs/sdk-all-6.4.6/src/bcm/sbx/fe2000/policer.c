/*
 * $Id: policer.c,v 1.63 Broadcom SDK $
                           ...
                           uint32 *profile_id)
{
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "Failed to get base rate for policer 0x%04x: %s\n"),
    ...
    if (LOG_CHECK(BSL_LS_BCM_POLICER | BSL_VERBOSE)) {
        LOG_VERBOSE(BSL_LS_BCM_POLICER,
                    (BSL_META_U(unit,
                                "Read PmConfig 0x%04x:\n"),
                     policer_id));
        LOG_VERBOSE(BSL_LS_BCM_POLICER,
                    (BSL_META_U(unit,
                                "rfcMode=%d rate=%d cbs=%d cir=%d ebs=%d eir=%d\n"), 
                     pm_config.uRfcMode, pm_config.uRate, pm_config.uCBS,
                     pm_config.uCIR, pm_config.uEBS, pm_config.uEIR));
        LOG_VERBOSE(BSL_LS_BCM_POLICER,
                    (BSL_META_U(unit,
        ...
        LOG_VERBOSE(BSL_LS_BCM_POLICER,
                    (BSL_META_U(unit,
                                "ckbits_sec=%d ckbits_burst=%d pkbits_sec=%d\n"),
 ...
 */
int
_bcm_policer_hw_create(int unit, uint32 policer_id,
                       bcm_policer_config_t *pol_cfg)
{
    ...
    if (pol_cfg->flags & BCM_POLICER_REPLACE) {
        LOG_ERROR(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "Un-supported flags BCM_POLICER_REPLACE\n")));
        return BCM_E_PARAM;
    }
    ...
    if (pol_cfg->mode != bcmPolicerModePassThrough) {
        ...
        if ((pol_cfg->ckbits_burst * 1000) < 
            ...
            LOG_WARN(BSL_LS_BCM_POLICER,
                     (BSL_META_U(unit,
                                 "%s: ckbits_burst (%d) less than recommended value "
        ...
        if ((pol_cfg->pkbits_burst * 1000) < 
            ...
            LOG_WARN(BSL_LS_BCM_POLICER,
                     (BSL_META_U(unit,
                                 "%s: pkbits_burst (%d) less than recommended value "
                            ...
                            bcm_policer_stat_t stat, uint64 *val)
{
    ...
    if (rv) {
        LOG_ERROR(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "Internal Error reading from PMU memory \n")));
        return BCM_E_INTERNAL;
    }
                            ...
                            bcm_policer_stat_t stat, uint64 val)
{
    ...
    if (rv) {
        LOG_ERROR(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "Internal error setting counters \n")));
        return BCM_E_INTERNAL;
    }
                                ...
                                uint32 *counter)
{
    ...
    bcm_policer_group_mode_t            grp_mode;
    int                                 num_ctrs = 0;
    int                                 counter_type = SOC_SBX_G2P3_INGCTR_ID;
	...
	soc_sbx_g2p3_counter_desc_t *cd;
	sbStatus_t s;
	
	cd = &tm->counters[counter_type];
	s = sbFe2000CmuCounterRead(fe->pCmuMgr, cd->bank, cd->base, num_ctrs,
				   NULL, 1, 1);
	rv = soc_sbx_translate_status(s);
        if (rv == BCM_E_NONE) {
            ...
            LOG_ERROR(BSL_LS_BCM_POLICER,
                      (BSL_META_U(unit,
                                  "%s: failed to clear counters %d through %d \n"), 
                       FUNCTION_NAME(), base_cntr, (base_cntr + num_ctrs)));
            _bcm_fe2000_stat_block_free(unit, counter_type, base_cntr);
            if (use_cookie) {
                sal_free(cookie);
            }
        }
                          ...
                          bcm_policer_t *policer_id)
{
    ...
    if (pol_cfg->flags & BCM_POLICER_WITH_ID) {
        ...
        if (_bcm_policer_id_assign(unit, policer_id) != BCM_E_NONE) {
            LOG_ERROR(BSL_LS_BCM_POLICER,
                      (BSL_META_U(unit,
                                  "%s: _bcm_policer_id_assign failed. \n"), 
                       FUNCTION_NAME()));
            POLICER_UNIT_UNLOCK(unit);
            return BCM_E_INTERNAL;
        }
...
bcm_fe2000_policer_destroy(int unit, bcm_policer_t policer_id)
{
    ...
    sb_status = sbFe2000PmPolicerDelete(pPm, (uint32) policer_id);
    if (sb_status != SB_OK) {
...
bcm_fe2000_policer_destroy_all(int unit)
{
    ...
    for (idx = 0; ((idx < pol_count) && (cur)); idx++) {
        id = cur->id;
        cur = cur->next;
        sb_status = sbFe2000PmPolicerDelete(pPm, (uint32) id);
        if (sb_status != SB_OK) {
            bcm_status = translate_sbx_result(sb_status);
            LOG_ERROR(BSL_LS_BCM_POLICER,
                      (BSL_META_U(unit,
                                  "sbFe2000PmPolicerDelete failed. return value %d (%s)"),
                       bcm_status, _SHR_ERRMSG(bcm_status)));
        } else {
                        ...
                        bcm_policer_config_t *policer_config)
{
    ...
    if (sbrv != SB_OK) {
        LOG_ERROR(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "Failed to recover policer 0x%04x from soc "
                               "layer: 0x%08x\n"),
                   id, sbrv));
    ...
    for (lpi = 0; lpi < num_lp; lpi++) {
        ...
        if (lp.policer) {
            ...
            if (BCM_FAILURE(rv)) {
                LOG_ERROR(BSL_LS_BCM_POLICER,
                          (BSL_META_U(unit,
                                      "Failed to decode policer 0x%04x from"
                                       "lp 0x%04x: %s\n"), 
            ...
            if (BCM_FAILURE(rv)) {
                LOG_ERROR(BSL_LS_BCM_POLICER,
                          (BSL_META_U(unit,
                                      "Failed to get mode %s policer count: %s\n"), 
            ...
            for (policer=0; policer < num_policers; policer++) {
                ...
                LOG_VERBOSE(BSL_LS_BCM_POLICER,
                            (BSL_META_U(unit,
                                        "Recovering policer 0x%x from lpi 0x%04x "
                                         "groupMode=%s\n"), plid, lpi, 
                             _bcm_fe2000_policer_group_mode_to_str(group_mode)));
                ...
                if (BCM_FAILURE(rv)) {
                    LOG_ERROR(BSL_LS_BCM_POLICER,
                              (BSL_META_U(unit,
                                          "Failed to recover policer id 0x%04x: %s\n"),
                               plid, bcm_errmsg(rv)));
                    return rv;
                }
...
bcm_fe2000_policer_init(int unit)
{
    ...
    if (BCM_SUCCESS(status)) {
        ...
        if (BCM_FAILURE(status)) {
            LOG_ERROR(BSL_LS_BCM_POLICER,
                      (BSL_META_U(unit,
                                  "%s: Failed to get policer params: %d %s\n"),
        ...
        if (BCM_SUCCESS(status)) {
            if (pp.size != (BCM_FE2000_XT_POLICERS + 1)) {
                LOG_ERROR(BSL_LS_BCM_POLICER,
                          (BSL_META_U(unit,
                                      "%s: Unexpected number of exception policers: %d\n"),
                ...
                LOG_ERROR(BSL_LS_BCM_POLICER,
                          (BSL_META_U(unit,
                                      "%s: Incorrect policer gruop, must be bank 0: %d\n"),
