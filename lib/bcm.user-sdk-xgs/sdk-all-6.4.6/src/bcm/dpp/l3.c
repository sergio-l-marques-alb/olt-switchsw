...
#else
#define BCM_DPP_MTU_VALID(_mtu) ((_mtu) == 0)
#endif
...
#define ARAD_PP_MYMAC_VSI_LSB_MAX0  ((1<<8)-1)
#define ARAD_PP_MYMAC_VSI_LSB_MAX1  ((1<<4)-1)
#define SOC_SAND_TRUE  1
#define SOC_SAND_FALSE 0

#define ARAD_PP_VRRP_MAC_DISABLE        (0x0)
#define ARAD_PP_VRRP_MAC_ENABLE         (0x1)
#define ARAD_PP_VRRP_MAC_IPV4_ENABLE    (0x1) /* default - same as enable */
#define ARAD_PP_VRRP_MAC_IPV6_ENABLE    (0x2)
#define ARAD_PP_VRRP_MAC_IS_IPV6        (0x2) /* same as ipv6_enable */
...
#define _BCM_PPD_EG_ENCAP_IS_TUNNEL_TYPE(unit,entry_type) \
    (entry_type == SOC_PPD_EG_ENCAP_ENTRY_TYPE_MPLS_ENCAP || entry_type == SOC_PPD_EG_ENCAP_ENTRY_TYPE_IPV4_ENCAP \
    || entry_type == SOC_PPD_EG_ENCAP_ENTRY_TYPE_IPV6_ENCAP \
    || entry_type == SOC_PPD_EG_ENCAP_ENTRY_TYPE_VSI || entry_type == SOC_PPD_EG_ENCAP_ENTRY_TYPE_SWAP_CMND \
    || entry_type == SOC_PPD_EG_ENCAP_ENTRY_TYPE_POP_CMND || entry_type == SOC_PPD_EG_ENCAP_ENTRY_TYPE_PWE)
...
#define _BCM_L3_SOURCE_BIND_IP6_COMPRESSION_SAV(unit, port)IS_PON_PORT((unit), (port))
  ...
  )
{
    ...
    alloc_flags = (egr->encap_id == 0) ? 0 : SHR_RES_ALLOC_WITH_ID;
    ...
    if(SOC_IS_ARAD(unit)){
        ...
        if (SOC_IS_JERICHO(unit)) {
            ...
            rv = _bcm_dpp_gport_alloc_global_and_local_lif(unit, alloc_flags, ll_global_lif_index, NULL, &local_out_lif_info, TRUE);
    ...
    if(alloc_flags & SHR_RES_ALLOC_WITH_ID) {
        ...
        if (soc_sand_get_error_code_from_error_word(ret) != SOC_SAND_OK)
        {
            LOG_ERROR(BSL_LS_BCM_L3,
                      (BSL_META_U(unit,
                                  "soc_sand error %x\n"), ret));
            BCMDNX_ERR_EXIT_MSG(BCM_E_INTERNAL, (_BSL_BCM_MSG("soc_ppd_eg_encap_range_info_get failed")));
        }       

        /* link layer?*/
        if(*local_lif_index <= eg_encap_range_info.ll_limit) {
            /* if the eep-id the user asked for (WITH_ID flag) already exists -
                   it can be used, no error should be returned */
            if (BCM_E_EXISTS != (bcm_dpp_am_l3_eep_is_alloced(unit,*ll_local_lif_index))) {
                rv = bcm_dpp_am_l3_eep_alloc(unit,_BCM_DPP_AM_L3_EEP_TYPE_DEFAULT, alloc_flags,ll_local_lif_index);
                BCMDNX_IF_ERR_EXIT(rv);
            }
        }        
        else if(*local_lif_index <= eg_encap_range_info.ip_tnl_limit) {
            if(_Bcm_dpp_sync_lif_eep) {
                lif_id = *local_lif_index;
                rv = bcm_dpp_am_l3_lif_ip_tnl_alloc(unit, alloc_flags, &lif_id);
                BCMDNX_IF_ERR_EXIT(rv);
        
                alloc_flags |= BCM_DPP_AM_FLAG_ALLOC_WITH_ID;
                *local_lif_index = lif_id;
            }
    
            rv = bcm_dpp_am_ip_tunnel_eep_alloc(unit, _BCM_DPP_AM_IP_TUNNEL_EEP_TYPE_DEFAULT, alloc_flags, local_lif_index);
            BCMDNX_IF_ERR_EXIT(rv);
        }
        else {
            ...
            if(_Bcm_dpp_sync_lif_eep) {
                lif_id = *local_lif_index;
                rv = bcm_dpp_am_l2_vpn_pwe_alloc(unit, _BCM_DPP_AM_L2_PWE_TERM_DEFAULT, alloc_flags, 2, &lif_id, NULL);
                BCMDNX_IF_ERR_EXIT(rv);
            ...
            rv = bcm_dpp_am_mpls_eep_alloc(unit,_BCM_DPP_AM_MPLS_EEP_TYPE_SIMPLE_MPLS_TUNNEL,alloc_flags,2,local_lif_index);
            BCMDNX_IF_ERR_EXIT(rv);
            tunneled = 1;
        }
    }/* at any place */
...
{
    int status = BCM_E_NONE;
    SOC_PPD_FEC_ID fec_id = (SOC_PPD_FEC_ID) *fec_index;
...
_bcm_l3_dealloc_fec(
    int unit,
    int flags,
    int usage,/*0-l3-route, 1: l3 ecmp, 2: Tunnel, 3: Protected tunnel, 4: Protected L2 interface*/
    int size,/*non-zero for protection or ECMP*/
    int fec_index    
  )
{
...
{
    BCMDNX_INIT_FUNC_DEFS;
    *fec_index = (SOC_PPD_FEC_ID)_BCM_PETRA_L3_ITF_VAL_GET(intf);
...
{
    BCMDNX_INIT_FUNC_DEFS;
    *eep = _BCM_PETRA_L3_ITF_VAL_GET(encap_id);
    BCM_EXIT;
exit:
    BCMDNX_FUNC_RETURN;
}
...
{
    _BCM_PETRA_L3_ITF_SET(*intf, _BCM_PETRA_L3_ITF_FEC, fec_index); 
}
...
{
    uint32 i;
    SOC_SAND_PP_IPV6_ADDRESS tmp_ipv6_addr;
    ...
    if (SOC_IS_JERICHO(unit)) {
        ...
        soc_sand_SAND_PP_IPV6_ADDRESS_clear(&tmp_ipv6_addr);

         /* reverse the uint32 order inside the array */
        for (i=0; i<SOC_SAND_PP_IPV6_ADDRESS_NOF_UINT32S; i++) {
             tmp_ipv6_addr.address[i] = soc_sand_ipv6_addr->address[SOC_SAND_PP_IPV6_ADDRESS_NOF_UINT32S-1-i];
        }
        sal_memcpy(ipv6_addr,&(tmp_ipv6_addr.address),sizeof(bcm_ip6_t));
    }
  ...
  )
{
    ...
    if (SOC_IS_JERICHO(unit)) {
        ...
        soc_sand_SAND_PP_IPV6_ADDRESS_clear(&tmp_ipv6_addr);
        sal_memcpy(&(tmp_ipv6_addr.address),ipv6_addr,sizeof(bcm_ip6_t));

        /* reverse the uint32 order inside the array */
        for (i=0; i<SOC_SAND_PP_IPV6_ADDRESS_NOF_UINT32S; i++) {
                soc_sand_ipv6_addr->address[i] = tmp_ipv6_addr.address[SOC_SAND_PP_IPV6_ADDRESS_NOF_UINT32S-1-i];
        }
    }
  ...
  )
{
  ...
  if (SOC_IS_ARAD(unit)) { /* ignore 12b' LSB */
      soc_ppd_mac.address[0] &= (uint8)(~(ARAD_PP_MYMAC_VSI_LSB_MAX0));
      soc_ppd_mac.address[1] &= (uint8)(~(ARAD_PP_MYMAC_VSI_LSB_MAX1));
  }
  else { /* ignore 6b' LSB */
      /* LCOV_EXCL_START */
      soc_ppd_mac.address[0] &= ~SOC_PB_PP_MYMAC_VSI_LSB_MAX;
      /* LCOV_EXCL_STOP */
  }
  ...
  if (is_set) { /* MyMac msb has been set already */
      ...
      if (SOC_IS_ARAD(unit)) { /* ignore 12b' LSB */
          soc_ppd_mac_org.address[0] &= (uint8)(~(ARAD_PP_MYMAC_VSI_LSB_MAX0));
          soc_ppd_mac_org.address[1] &= (uint8)(~(ARAD_PP_MYMAC_VSI_LSB_MAX1));
      }
      else { /* ignore 6b' LSB */
          /* LCOV_EXCL_START */
          soc_ppd_mac_org.address[0] &= ~SOC_PB_PP_MYMAC_VSI_LSB_MAX;
          /* LCOV_EXCL_STOP */
      }
  ...
  )
{
  ...
  if (SOC_IS_ARAD(unit))
  {
    soc_ppd_mac1.address[0] &= ARAD_PP_MYMAC_VSI_LSB_MAX0;
    soc_ppd_mac1.address[1] &= ARAD_PP_MYMAC_VSI_LSB_MAX1;
  }
  else
  {
    /* LCOV_EXCL_START */
    soc_ppd_mac1.address[1] = 0;
    soc_ppd_mac1.address[0] &= SOC_PB_PP_MYMAC_VSI_LSB_MAX;
    /* LCOV_EXCL_STOP */
  }
  ...
  )
{
  ...
  /* Arad uses 12 lsb of mymac for VSI, Soc_petra B uses 6*/
  if (SOC_IS_ARAD(unit)) {
  ...
  {
    /* LCOV_EXCL_START */
    soc_ppd_mac_org.address[0] = (0xa0 & soc_ppd_mac_org1.address[0]) | (0x3f & soc_ppd_mac_org2.address[0]);
  ...
  )
{
  ...
  int 
    old_profile, is_last, ingress_profile, egress_profile, is_allocated;
#ifdef BCM_ARAD_SUPPORT
  SOC_PPD_EG_ENCAP_VSI_ENCAP_INFO
      out_vsi_info;
#endif
  ...
  if (SOC_IS_ARAD(unit)) {
	  ...
	  ret = _bcm_dpp_am_template_vsi_ingress_profile_data_get(unit, vsi, &vsi_ingress_profile_data);
	  BCMDNX_IF_ERR_EXIT(ret);
      ret = _bcm_dpp_am_template_vsi_ingress_profile_mapping_get(unit, vsi, &ingress_profile);
	  BCMDNX_IF_ERR_EXIT(ret);
      ret = _bcm_dpp_am_template_vsi_egress_profile_data_get(unit, vsi, &vsi_egress_profile_data);
      BCMDNX_IF_ERR_EXIT(ret);
      ...
      if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "mtu_advanced_mode", 0)) 
      {
          ...
          /* If the Forwarding Layer MTU value is 0, then is it presumed to use the
           * port's configured MTU (if enabled) or none at all (if disabled) */
          if (intf->l3a_mtu_forwarding != 0) 
          {
              /* Find profile that matches the specified MTU values */
              soc_sand_rv = arad_pp_vsi_egress_mtu_profile_get(unit, TRUE /* Forwarding Layer MTU */, intf->l3a_mtu_forwarding, &mtu_forwarding_profile);
          ...
          {
              /* Find profile that matches the specified MTU values */
              soc_sand_rv = arad_pp_vsi_egress_mtu_profile_get(unit, FALSE /* Link Layer MTU */, intf->l3a_mtu, &mtu_profile);
              BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
              if (mtu_profile == 0)
              {
                  LOG_ERROR(BSL_LS_BCM_L3,
          ...
          if ((intf->l3a_mtu_forwarding != 0) && (intf->l3a_mtu != 0) && (mtu_forwarding_profile != mtu_profile))
          {
              LOG_ERROR(BSL_LS_BCM_L3,
                        (BSL_META_U(unit,
                                    "MTU profile of Forwarding Layer (%d) does not match Link Layer (%d)\n"), mtu_forwarding_profile, mtu_profile));
              BCMDNX_ERR_EXIT_MSG(BCM_E_INTERNAL, (_BSL_BCM_MSG("soc_ppd_vsi_info_set failed")));
          }
          ...
          {
              /* In this mode (custom_feature_mtu_advanced_mode) MTU profile for VSIs 1-3 
               * must be equal to the VSI value, and is destined to WA a hardware limitation. */
              if ((vsi >= 1) && (vsi <= 3) &&
                  (((intf->l3a_mtu != 0) && (vsi != mtu_profile))
                   || ((intf->l3a_mtu_forwarding != 0) && (vsi != mtu_forwarding_profile))))
              {
                  LOG_ERROR(BSL_LS_BCM_L3,
                            (BSL_META_U(unit,
                                        "MTU profile (%d) is invalid . For VSI %d MTU profile must be also be equal to VSI\n"), mtu_profile, vsi));
                  BCMDNX_ERR_EXIT_MSG(BCM_E_INTERNAL, (_BSL_BCM_MSG("soc_ppd_vsi_info_set failed")));
              }
          }

          /* The VSI profile is set according to Forwarding/Link Layer MTU profile */
          vsi_info.profile_egress = (intf->l3a_mtu != 0) ? mtu_profile : mtu_forwarding_profile;
      }
      else if ((vsi_egress_profile_data.mtu != intf->l3a_mtu) || !enable) 
      {
          ...
          BCMDNX_IF_ERR_EXIT(ret);
          if (is_allocated) {
              ret = _bcm_dpp_l2_vsi_profile_update(unit, egress_profile, ingress_profile, vsi_egress_profile_data, vsi_ingress_profile_data);
              BCMDNX_IF_ERR_EXIT(ret);
          }
          if (is_last && (old_profile != egress_profile)) {
              ...
              ret = _bcm_dpp_l2_vsi_profile_update(unit, old_profile, ingress_profile, vsi_egress_profile_data_old, vsi_ingress_profile_data);
              BCMDNX_IF_ERR_EXIT(ret);
          }
...
#ifdef BCM_ARAD_SUPPORT
  if(SOC_IS_ARAD(unit)) {
      if(enable && !(intf->l3a_flags & BCM_L3_TRILL_ONLY)) {
          SOC_PPD_EG_ENCAP_VSI_ENCAP_INFO_clear(&out_vsi_info);

          out_vsi_info.out_vsi = intf->l3a_vid;
          out_vsi_info.remark_profile = intf->dscp_qos.qos_map_id;
  ...
  )
{
  ...
  if (!rif_info.routing_enablers_bm) {
    BCMDNX_ERR_EXIT_MSG(BCM_E_NOT_FOUND, (_BSL_BCM_MSG("routing is not enabled on rif")));
  }
  ...
  )
{
  ...
  if (create) 
  {
      rif_info.routing_enablers_bm |= SOC_PPD_RIF_ROUTE_ENABLE_TYPE_IP_UC;
      rif_info.routing_enablers_bm |= SOC_PPD_RIF_ROUTE_ENABLE_TYPE_IP_MC;
      rif_info.routing_enablers_bm |= SOC_PPD_RIF_ROUTE_ENABLE_TYPE_MPLS;
      ...
      if (SOC_IS_JERICHO(unit)) {
		  SOC_PPD_RIF_INFO rif_info_to_delete;
		  int is_last_rout_enabler = 0;
	  	  
		  SOC_PPD_RIF_INFO_clear(&rif_info_to_delete);	  
      
		  rif_info_to_delete.routing_enablers_bm |= SOC_PPD_RIF_ROUTE_ENABLE_TYPE_IP_UC;
		  rif_info_to_delete.routing_enablers_bm |= SOC_PPD_RIF_ROUTE_ENABLE_TYPE_IP_MC;
		  rif_info_to_delete.routing_enablers_bm |= SOC_PPD_RIF_ROUTE_ENABLE_TYPE_MPLS;
		  rif_info_to_delete.uc_rpf_enable = (intf->l3a_flags & BCM_L3_RPF) ? 1 : 0;
		  rif_info_to_delete.vrf_id = intf->l3a_vrf;
  ...
  )
{
  ...
  soc_ppd_mac.address[5] = 0;
  soc_ppd_mac.address[4] = 0;
  soc_ppd_mac.address[3] = 0;
  soc_ppd_mac.address[2] = 0;
  soc_ppd_mac.address[1] = 0;
  soc_ppd_mac.address[0] &= SOC_PB_PP_MYMAC_VSI_LSB_MAX;
  ...
  )
{
  ...
  ac_ndx = ac_index;
    ...
    /*
     * Open AC per each vid of the vlan domain. All ACs point to the
     * vsi that is associated with the vlan domain.
     */
  SOC_PPD_L2_LIF_AC_KEY_clear(&ac_key);

  BCM_PBMP_ASSIGN(all_ports, PBMP_E_ALL(unit));

  /* as interface definition according to vlan only add to all ports*/
  
  PBMP_ITER(all_ports, port_i){

    BCMDNX_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_local_to_pp_port_get, (unit, port_i, &pp_port, &core)));

    ret = soc_ppd_port_info_get(unit, core, pp_port, &port_info);
    if (soc_sand_get_error_code_from_error_word(ret) != SOC_SAND_OK)
    { 
      LOG_ERROR(BSL_LS_BCM_L3,
                (BSL_META_U(unit,
                            "soc_sand error %x\n"), ret));
      BCMDNX_ERR_EXIT_MSG(BCM_E_INTERNAL, (_BSL_BCM_MSG("soc_ppd_port_info_get failed")));
    }

    
    /* then need to be maintained when port-vlan-domain mapping modified*/
    ...
    { 
      LOG_ERROR(BSL_LS_BCM_L3,
                (BSL_META_U(unit,
                            "soc_sand error %x\n"), ret));
      BCMDNX_ERR_EXIT_MSG(BCM_E_INTERNAL, (_BSL_BCM_MSG("soc_ppd_l2_lif_ac_add failed")));
    }
    ...
    int *ll_eep)
{
    ...
    itf_val = _BCM_PETRA_L3_ITF_VAL_GET(intf);
    ...
    else if(_BCM_PETRA_L3_ITF_IS_ENCAP(intf)) { /* OutLIF */
        *eep = itf_val;
        *vsi = 0;
    }
    ...
    if (SOC_IS_PETRAB(unit)) {
      /* LCOV_EXCL_START */
      soc_sand_rv = soc_ppd_eg_encap_range_info_get(soc_sand_dev_id, &range_info);
      BCM_SAND_IF_ERR_EXIT(soc_sand_rv);

      /* Entries (ip_tnl_limit+1)- 12K-1: are used for PWE, MPLS */
      
      if (*eep > range_info.ll_limit) {
          *tunneled = 1;
          *ll_eep = 0;
      } else if (*eep > range_info.ip_tnl_limit) {
          *tunneled = 1;
          *ll_eep = 0;
      } else {
          *tunneled = 0;
      ...
      /* LCOV_EXCL_STOP */
          ...
          if (_BCM_PPD_EG_ENCAP_IS_TUNNEL_TYPE(unit,entry_type)) { /* Entry is PWE, MPLS types,IP tunnel, or VSI */
              *tunneled = 1;
              *ll_eep = 0;
          } else if (entry_type == SOC_PPD_EG_ENCAP_ENTRY_TYPE_DATA) { /* DATA */
              BCMDNX_ALLOC(encap_entry_info, sizeof(SOC_PPD_EG_ENCAP_ENTRY_INFO)*SOC_PPD_NOF_EG_ENCAP_EEP_TYPES, "_bcm_tunnel_intf_is_tunnel.encap_entry_info");
              ...
              soc_sand_rv =
                  soc_ppd_eg_encap_entry_get(soc_sand_dev_id,
                                         SOC_PPD_EG_ENCAP_EEP_TYPE_TUNNEL_EEP, local_lif, 2,
              ...
              *ll_eep = 0;
              ...
              for (indx = 0; indx < nof_entries; ++indx) {
                  if (SOC_PPD_EG_ENCAP_DATA_COUPLING_SWAP_TYPE(soc_sand_dev_id,encap_entry_info[indx].entry_type,&(encap_entry_info[indx].entry_val.data_info))) {
                      *tunneled = 1;
                      break;
                  }
              }
          } else if (entry_type == SOC_PPD_EG_ENCAP_ENTRY_TYPE_LL_ENCAP) { /* Link layer */
              *tunneled = 0;
              *ll_eep = *eep;
              *eep = 0;
    ...
    if(*tunneled) {
        /* MPLS SWAP Coupling already allocates the memory and get the entry info. */
        if (encap_entry_info == NULL) {
            BCMDNX_ALLOC(encap_entry_info, sizeof(SOC_PPD_EG_ENCAP_ENTRY_INFO)*SOC_PPD_NOF_EG_ENCAP_EEP_TYPES, "_bcm_tunnel_intf_is_tunnel.encap_entry_info");
            if (encap_entry_info == NULL) {        
                BCMDNX_ERR_EXIT_MSG(BCM_E_MEMORY, (_BSL_BCM_MSG("failed to allocate memory")));
        ...
        for (indx = 0; indx < nof_entries; ++indx) {
            if (encap_entry_info[indx].entry_type == SOC_PPD_EG_ENCAP_ENTRY_TYPE_MPLS_ENCAP) {
            ...
            else if (encap_entry_info[indx].entry_type == SOC_PPD_EG_ENCAP_ENTRY_TYPE_IPV4_ENCAP) {
                *vsi = encap_entry_info[indx].entry_val.ipv4_encap_info.out_vsi;
                *ll_eep = next_eep[indx];
            }
            
            else if (encap_entry_info[indx].entry_type == SOC_PPD_EG_ENCAP_ENTRY_TYPE_POP_CMND) {
                *vsi = encap_entry_info[indx].entry_val.pop_info.ethernet_info.out_vsi;
                *ll_eep = next_eep[indx];
            }
            
            else if (encap_entry_info[indx].entry_type == SOC_PPD_EG_ENCAP_ENTRY_TYPE_PWE) {
                BCMDNX_ERR_EXIT_MSG(BCM_E_INTERNAL, (_BSL_BCM_MSG("not expected pwe type. shoud be mpls encap")));
            }
            else if (encap_entry_info[indx].entry_type == SOC_PPD_EG_ENCAP_ENTRY_TYPE_SWAP_CMND) {
                *vsi = encap_entry_info[indx].entry_val.swap_info.out_vsi;
                *ll_eep = next_eep[indx];
            }
            else if (encap_entry_info[indx].entry_type == SOC_PPD_EG_ENCAP_ENTRY_TYPE_VSI) {
...
int _bcm_l3_intf_eep_get(
    ...
    if (intf > 0) {
        ...
        } else if (_BCM_PETRA_L3_ITF_IS_ENCAP(intf)) {
  ...
  )
{
  ...
  *tunnel_idx = 0;
  ...
  itf_val = _BCM_PETRA_L3_ITF_VAL_GET(intf->l3a_intf_id);
  ...
  if (SOC_IS_PETRAB(unit)) {
      ...
      if (local_lif > range_info.ll_limit) {
            *is_tunnel = 1;
            *tunnel_idx = eep;
      }
      /* LCOV_EXCL_STOP */
      ...
      /* ARAD */
        BCMDNX_ALLOC(encap_entry_info, sizeof(SOC_PPD_EG_ENCAP_ENTRY_INFO)*SOC_PPD_NOF_EG_ENCAP_EEP_TYPES, "_bcm_ppd_l3_intf_is_tunneled.encap_entry_info");
        if (encap_entry_info == NULL) {        
            BCMDNX_ERR_EXIT_MSG(BCM_E_MEMORY, (_BSL_BCM_MSG("failed to allocate memory")));
        }
      soc_sand_rv =
 ...
 */
int
  _bcm_ppd_fec_entry_to_application_info( 
    int unit,
  ...
  )      
{
  ...
  if (SOC_IS_ARAD(unit)) {
      ...
      case SOC_PPD_FEC_TYPE_IP_UC: /* FEC is used for HOST */
        if (!BCM_VLAN_VALID(fec_entry->eep)) { /* EEP must be RIF index */
            BCMDNX_ERR_EXIT_MSG(BCM_E_INTERNAL, (_BSL_BCM_MSG("Internal error: in case fec type is IP_UC eep should represent a valid out_rif ")));
        }
        _BCM_PETRA_L3_ITF_SET(egr->intf,_BCM_PETRA_L3_ITF_RIF,fec_entry->eep); 
        break;
      case SOC_PPD_FEC_TYPE_DROP:      
      case SOC_PPD_FEC_TYPE_SIMPLE_DEST:
      case SOC_PPD_NOF_FEC_TYPES_ARAD:
        /* Nothing to do */
        if (fec_entry->eep != 0 || fec_entry->app_info.out_rif != 0) { /* must be null */
            BCMDNX_ERR_EXIT_MSG(BCM_E_INTERNAL, (_BSL_BCM_MSG("Internal error: in case fec type is Simple eep and out_rif must be 0")));
        }
       
        break;
      case SOC_PPD_FEC_TYPE_IP_MC: /* FEC points to multicast destination */
        /* Only intf */
        if (!BCM_VLAN_VALID(fec_entry->rpf_info.expected_in_rif)) { /* must be RIF index */
            BCMDNX_ERR_EXIT_MSG(BCM_E_INTERNAL, (_BSL_BCM_MSG("Internal error: in case fec type is IP_MC rif should represent a valid out_rif ")));
        }       
        _BCM_PETRA_L3_ITF_SET(egr->intf,_BCM_PETRA_L3_ITF_RIF,fec_entry->rpf_info.expected_in_rif);       
        break;
      case SOC_PPD_FEC_TYPE_TRILL_MC:
        
        if (fec_entry->app_info.out_rif != 0) { /* must be null */
            BCMDNX_ERR_EXIT_MSG(BCM_E_INTERNAL, (_BSL_BCM_MSG("Internal error: in case fec type is trill_mc out_rif must be 0")));
        }

        egr->flags |= BCM_L3_TRILL_ONLY;
        _BCM_PETRA_L3_ITF_SET(egr->encap_id,_BCM_PETRA_L3_ITF_ENCAP,fec_entry->eep);
        break;
      case SOC_PPD_FEC_TYPE_MPLS_LSR:
      case SOC_PPD_FEC_TYPE_MPLS_LSR_EEI_OUTLIF:
      case SOC_PPD_FEC_TYPE_TRILL_UC:
        if (fec_entry->app_info.out_rif != 0) { /* must be null */
            BCMDNX_ERR_EXIT_MSG(BCM_E_INTERNAL, (_BSL_BCM_MSG("Internal error: in case fec type is MPLS_LSR out_rif must be 0")));
        }
        _BCM_PETRA_L3_ITF_SET(egr->encap_id,_BCM_PETRA_L3_ITF_ENCAP,fec_entry->eep);
        if (fec_entry->type == SOC_PPD_FEC_TYPE_MPLS_LSR_EEI_OUTLIF) {
            egr->flags |= BCM_L3_ENCAP_SPACE_OPTIMIZED;
        }
        break;
      case SOC_PPD_FEC_TYPE_TUNNELING: /* Tunnel can be VSI-RIF or Tunnel-RIF. Decide according to range */
      case SOC_PPD_FEC_TYPE_TUNNELING_EEI_OUTLIF:
        if (fec_entry->app_info.out_rif != 0) { /* must be null */
            BCMDNX_ERR_EXIT_MSG(BCM_E_INTERNAL, (_BSL_BCM_MSG("Internal error: in case fec type is Tunneling out_rif must be 0")));
        }

        if (BCM_VLAN_VALID(fec_entry->eep)) {
          _BCM_PETRA_L3_ITF_SET(egr->intf,_BCM_PETRA_L3_ITF_RIF,fec_entry->eep);
        } else {
          _BCM_PETRA_L3_ITF_SET(egr->intf,_BCM_PETRA_L3_ITF_ENCAP,fec_entry->eep);
        }

        if (fec_entry->type == SOC_PPD_FEC_TYPE_TUNNELING_EEI_OUTLIF) {
            egr->flags |= BCM_L3_ENCAP_SPACE_OPTIMIZED;
        }
        break;
      case SOC_PPD_FEC_TYPE_TUNNELING_EEI_MPLS_COMMAND:
      {
          ...
          if (identifier < ARAD_PP_EEI_IDENTIFIER_SWAP_VAL) {
              BCMDNX_IF_ERR_EXIT(_bcm_mpls_tunnel_push_profile_info_get(unit, identifier, &has_cw, &label_entry));
          }
      ...
      case SOC_PPD_FEC_TYPE_ROUTING:
        if (!BCM_VLAN_VALID(fec_entry->app_info.out_rif)) { /* must be RIF index */
            BCMDNX_ERR_EXIT_MSG(BCM_E_INTERNAL, (_BSL_BCM_MSG("Internal error: in case fec type is Routing"
                                                       "rif should represent a valid out_rif. rif value is %d"), fec_entry->rpf_info.expected_in_rif));
        }
        _BCM_PETRA_L3_ITF_SET(egr->intf,_BCM_PETRA_L3_ITF_RIF,fec_entry->app_info.out_rif);
        _BCM_PETRA_L3_ITF_SET(egr->encap_id,_BCM_PETRA_L3_ITF_ENCAP,fec_entry->eep);
        break;
      default:
        BCMDNX_ERR_EXIT_MSG(BCM_E_INTERNAL, (_BSL_BCM_MSG("Internal error: unsupported fec_entry type %d"),fec_entry->type));
      }
  } else {
      /* LCOV_EXCL_START */
      /* Petra-B always the same */
      egr->intf = fec_entry->app_info.out_rif;
      egr->encap_id = fec_entry->eep;
      _BCM_PETRA_L3_ITF_SET(egr->encap_id,_BCM_PETRA_L3_ITF_ENCAP,fec_entry->eep);
      /* LCOV_EXCL_STOP */
  }
  ...
  )
{
  ...
  SOC_PPD_FRWRD_FEC_ENTRY_INFO_clear(&working_fec);
  SOC_PPD_FRWRD_FEC_ENTRY_INFO_clear(&protect_fec);
  ...
  {
      if (protect_type == SOC_PPD_FRWRD_FEC_PROTECT_TYPE_FACILITY) {
          egr->failover_id = protect_info.oam_instance_id;
      } else {
          DPP_FAILOVER_TYPE_SET(egr->failover_id, protect_info.oam_instance_id, DPP_FAILOVER_TYPE_FEC);
      }
  ...
  if ((working_fec.rpf_info.rpf_mode & SOC_PPD_FRWRD_FEC_RPF_MODE_MC_EXPLICIT)
      || (working_fec.rpf_info.rpf_mode & SOC_PPD_FRWRD_FEC_RPF_MODE_MC_USE_SIP_WITH_ECMP)) {
      
      egr->flags |= BCM_L3_RPF;

      if (working_fec.rpf_info.rpf_mode & SOC_PPD_FRWRD_FEC_RPF_MODE_MC_USE_SIP_WITH_ECMP) {
  ...
  if (is_local_conf)
  {
      if (_BCM_PETRA_L3_ITF_IS_ENCAP(egr->encap_id)) {

          rv = _bcm_dpp_global_lif_mapping_global_to_local_get(unit, _BCM_DPP_GLOBAL_LIF_MAPPING_EGRESS, _BCM_PETRA_L3_ITF_VAL_GET(egr->encap_id), &local_lif);
          BCMDNX_IF_ERR_EXIT(rv);
          ...
          if (usage == _bcmDppLifTypeLinkLayer) { /* EEDB entry is LL, fill egr with it's data */
              rv = _bcm_ppd_eg_encap_entry_get(unit, _BCM_PETRA_L3_ITF_VAL_GET(egr->encap_id), egr); 
              BCMDNX_IF_ERR_EXIT(rv);
          }
      }      
  }
  ...
  )
{
  ...
  */ 
  ...
  fec_ndx = fec_index;
  protect_type = SOC_PPD_FRWRD_FEC_PROTECT_TYPE_NONE;
  ...
  working_fec.rpf_info.rpf_mode = urpf_mode;
  ...
  if (SOC_IS_ARAD(unit)){
      ...
      switch(switch_rpf) {
          ...
          working_fec.rpf_info.rpf_mode = SOC_PPD_FRWRD_FEC_RPF_MODE_UC_LOOSE;
          break;
          case bcmL3IngressUrpfStrict:
          working_fec.rpf_info.rpf_mode = SOC_PPD_FRWRD_FEC_RPF_MODE_UC_STRICT;
          break;
          case bcmL3IngressUrpfDisable:
          default:
          working_fec.rpf_info.rpf_mode = SOC_PPD_FRWRD_FEC_RPF_MODE_NONE;
          break;
      }
      ...
      if(egr->flags & BCM_L3_RPF) {
          if(egr->flags & BCM_L3_IPMC) {
              working_fec.rpf_info.rpf_mode |= SOC_PPD_FRWRD_FEC_RPF_MODE_MC_EXPLICIT;
  ...
  if (SOC_IS_ARAD(unit)) {
      if (egr->flags & BCM_L3_TRILL_ONLY) {
          ...
          if (mc == -1) {
              working_fec.type = SOC_PPD_FEC_TYPE_TRILL_UC;
              working_fec.eep = eep_index;
          } else {
              working_fec.type = SOC_PPD_FEC_TYPE_TRILL_MC;
              working_fec.eep = eep_index;
          }
      } else if (is_tunnel_eei_mpls_command) {
          working_fec.type = SOC_PPD_FEC_TYPE_TUNNELING_EEI_MPLS_COMMAND;
          working_fec.eep = eei_val;
      } else if ((egr->flags & BCM_L3_IPMC)) {
          working_fec.type = SOC_PPD_FEC_TYPE_IP_MC;
          working_fec.eep = 0;
          working_fec.rpf_info.expected_in_rif = out_rif;
      } else if (out_rif == 0 && eep_index == 0) {
          working_fec.type = SOC_PPD_FEC_TYPE_SIMPLE_DEST;
          ...
          if (is_fec_eei_outlif) { /* Is tunnel eei means FEC should point to EEP using FEC EEI FORMAT C */
              working_fec.type = SOC_PPD_FEC_TYPE_TUNNELING_EEI_OUTLIF; 
          }
          else { /* Else FEC should point to EEP using FEC Outlif FORMAT B */
              working_fec.type = SOC_PPD_FEC_TYPE_TUNNELING;
          }
          ...
          if (is_fec_eei_outlif) { /* Is tunnel eei means FEC should point to EEP using FEC EEI FORMAT C */
              working_fec.type = SOC_PPD_FEC_TYPE_MPLS_LSR_EEI_OUTLIF; 
          }
          else { /* Else FEC should point to EEP using FEC Outlif FORMAT B */
              working_fec.type = SOC_PPD_FEC_TYPE_MPLS_LSR;
          }
          working_fec.eep = eep_index;
          working_fec.app_info.out_rif = 0;
      } else {
          working_fec.type = SOC_PPD_FEC_TYPE_ROUTING; /* normal routing with outlif (EEP) + outRIF*/
          working_fec.eep = eep_index;
  ...
  if (SOC_IS_PETRAB(unit)){
      ...
      if (egr->flags & BCM_L3_TRILL_ONLY) {
          ...
          working_fec.type = SOC_PPD_FEC_TYPE_IP_UC;
      }
      working_fec.eep = eep_index;
      working_fec.app_info.out_rif = out_rif;
      /* LCOV_EXCL_STOP */
  ...
  /* New FEC, init FEC_ENTRY_ACCESSED table record */
...
{ 
  ...
  ll_encap_info.out_vid_valid = SOC_SAND_FALSE;
  ll_encap_info.pcp_dei_valid = SOC_SAND_FALSE;

  if (egr->flags & BCM_L3_TRILL_ONLY  ) {
      ll_encap_info.out_vid_valid = SOC_SAND_TRUE;
      ll_encap_info.out_vid = egr->vlan;
      ll_encap_info.out_ac_valid = SOC_SAND_TRUE;  
  ...
  )
{
  ...
  /* Verify LIF is LL */
  BCMDNX_IF_ERR_EXIT( _bcm_dpp_lif_usage_get(unit, -1, eep_index, NULL, &usage) );
...
{   
  ...
  uint32
    tunnel_eep_ndx;
  SOC_PPD_EG_ENCAP_IPV4_ENCAP_INFO
    ipv4_encap_info;
  ...
  ipv4_encap_info.dest.enable_gre = SOC_SAND_FALSE;
  ...
  )
{
  ...
  _BCM_PETRA_L3_ITF_SET(egr->encap_id, _BCM_PETRA_L3_ITF_ENCAP, eep_index);
...
{  
  ...
  SOC_PPD_FEC_ID
    fec_id = 0xffffffff;
  SOC_PPD_FRWRD_IPV4_VRF_INFO
      vrf_info;
  SOC_PPD_FRWRD_DECISION_INFO 
      frwrd_decision;
  SOC_PPD_FRWRD_IPV4_ROUTER_DEFAULT_ACTION
  ...
  SOC_PPD_FRWRD_IPV4_VRF_INFO_clear(&vrf_info);
 ...
 /* set into MC or UC default depends on the flag */
  ...
  if(BCM_GPORT_IS_SET(info->l3a_port_tgid)){
      ...
      router_dflt_dest_ptr->type = SOC_PPD_FRWRD_IPV4_ROUTER_DEFAULT_ACTION_TYPE_ACTION_PROFILE;
      sal_memcpy(
          &(router_dflt_dest_ptr->value.action_profile),
          &(frwrd_decision.additional_info.trap_info.action_profile),
          sizeof(SOC_PPD_ACTION_PROFILE)
      );
  }
  else{
      /* otherwise default destination is FEC */
      router_dflt_dest_ptr->type = SOC_PPD_FRWRD_IPV4_ROUTER_DEFAULT_ACTION_TYPE_FEC;
...
{  
 ...
 /* get into MC or UC default depends on the flag */
  ...
  /* if port set then set default destination to forwarding decision according to port */
  if(router_dflt_dest_ptr->type == SOC_PPD_FRWRD_IPV4_ROUTER_DEFAULT_ACTION_TYPE_ACTION_PROFILE) {
...
{  
  ...
  SOC_PPD_FRWRD_IPV4_UC_ROUTE_KEY
    route_key;
  SOC_PPD_FEC_ID
    fec_id = 0xffffffff;
  SOC_SAND_SUCCESS_FAILURE
  ...
  BCMDNX_INIT_FUNC_DEFS;
  SOC_PPD_FRWRD_IPV4_UC_ROUTE_KEY_clear(&route_key);
  ...
  )
{
  ...
  if(SOC_IS_ARAD(unit)){
      ...
      if ((((_BCM_GPORT_IS_OVERLAY(info->l3a_port_tgid)) && (SOC_IS_JERICHO(unit))) /* check FEC: check is overlay gport */
         ...
         && (_BCM_PETRA_L3_ITF_IS_RIF(info->l3a_intf))  /* check outRIF */
      ...
      else if(!_BCM_PETRA_L3_ITF_IS_FEC(info->l3a_intf) && info->encap_id != 0) {
              frwrd_type = 1;
      }
      /* for tunnel: tunnel-itf + port */
      else if(_BCM_PETRA_L3_ITF_IS_ENCAP(info->l3a_intf)) {
          frwrd_type = 3;
      }
      ...
      if (_BCM_PETRA_L3_ITF_IS_FEC(info->l3a_intf)) {
         _bcm_l3_intf_to_fec(unit, info->l3a_intf, &fec_id);
         SOC_PPD_FRWRD_DECISION_FEC_SET(unit, &routing_info.frwrd_decision, fec_id, ret);
         BCM_SAND_IF_ERR_EXIT(ret);
      }
      else if(_BCM_PETRA_L3_ITF_IS_RIF(info->l3a_intf) || _BCM_PETRA_L3_ITF_IS_ENCAP(info->l3a_intf)) {/* l3-interface*/
          ...
          if (BCM_GPORT_IS_FORWARD_PORT(info->l3a_port_tgid)) {
              routing_info.frwrd_decision.type = SOC_PPD_FRWRD_DECISION_TYPE_FEC;
      ...
      if (frwrd_type == 3) {
         rif_entry_id = _BCM_PETRA_L3_ITF_VAL_GET(info->l3a_intf);
      
         routing_info.frwrd_decision.additional_info.outlif.type = SOC_PPD_OUTLIF_ENCODE_TYPE_RAW;
         routing_info.frwrd_decision.additional_info.outlif.val = rif_entry_id;
      }
      else if(frwrd_type == 1) {
         routing_info.frwrd_decision.additional_info.outlif.type = SOC_PPD_OUTLIF_ENCODE_TYPE_RAW;
         routing_info.frwrd_decision.additional_info.outlif.val = _BCM_PETRA_L3_ITF_VAL_GET(info->encap_id);
      }
      /* FEC + MAC --> forward decision = FEC + ARP (EEI)*/
      else if(frwrd_type == 2) {
         routing_info.frwrd_decision.additional_info.eei.type = SOC_PPD_EEI_TYPE_OUTLIF;
      
         _bcm_l3_encap_id_to_eep(unit, info->encap_id, &eep_val);
         routing_info.frwrd_decision.additional_info.eei.val.outlif = eep_val;
      
         /* FIX FEC to point to out-RIF only, as MAC outlif is set by host */
      
         /* LSR type used to say move out-RIF from out-LIF field to EEI field in FEC, at last it will be writing as tunnel format */
         rv = _bcm_dpp_gport_fix_fec_type(unit, _BCM_PETRA_L3_ITF_VAL_GET(info->l3a_intf), SOC_PPD_FEC_TYPE_IP_UC);
         BCMDNX_IF_ERR_EXIT(rv);
  ...
  /* PETRA-B */
      ...
      /* LCOV_EXCL_START */
      _bcm_l3_intf_to_fec(unit, info->l3a_intf, &routing_info.fec_id);
      routing_info.eep = SOC_PPD_EEP_NULL;
      /* LCOV_EXCL_STOP */
  }
...
{ 
  ...
  /* Route key */
  vrf_route.is_ipv6        = SOC_SAND_TRUE;
  ...
  )
{
  ...
  rv = _bcm_ppd_frwrd_ipv6_vrf_route_get(unit, &vrf_route);
  if ((rv != BCM_E_NONE) && (rv != BCM_E_NOT_FOUND)) {
    BCMDNX_ERR_EXIT_MSG(rv, (_BSL_BCM_MSG("get vrf route failed")));
  }
  BCMDNX_IF_ERR_EXIT(rv);

  /* Entry found - Fec id is resolved in vrf_route.vrf_intf */
  _bcm_l3_fec_to_intf(unit, vrf_route.vrf_intf, &(info->l3a_intf));
  *encap_id = vrf_route.encap_id;
...
{  
  ...
  SOC_PPD_FRWRD_IPV6_UC_ROUTE_KEY
    route_key;
  SOC_SAND_SUCCESS_FAILURE
    success;

  BCMDNX_INIT_FUNC_DEFS;
  SOC_PPD_FRWRD_IPV6_UC_ROUTE_KEY_clear(&route_key);
  ...
  )
{
  ...
  *encap_id = 0;
  if (route_info.additional_info.eei.type == SOC_PPD_EEI_TYPE_OUTLIF) {
      /* Retrieve the encap-id for Host table */
      _BCM_PETRA_L3_ITF_SET(*encap_id, _BCM_PETRA_L3_ITF_ENCAP, route_info.additional_info.eei.val.outlif); 
  }
  ...
  )
{
  ...
  if (SOC_IS_ARAD(unit)) {
     ...
     /* LCOV_EXCL_START */
     BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("IPV6 host is not supported")));
     /* LCOV_EXCL_STOP */
  }
  ...
  )
{
  ...
  SOC_PPD_FRWRD_DECISION_INFO_clear(&route_info);

  /* Different payload processing if host or not */
  /* destination must be FEC */
  _bcm_l3_intf_to_fec(unit, r_info->l3a_intf, &fec_id);
  SOC_PPD_FRWRD_DECISION_FEC_SET(unit, &route_info, fec_id, soc_sand_rv);
  BCM_SAND_IF_ERR_EXIT(soc_sand_rv);

  if (is_host) {
      /* EEP is valid this host points to: FEC + EEP */
      if (_BCM_PETRA_L3_ITF_IS_FEC(r_info->l3a_intf) && encap_id != 0) {

          /* setting host frwrd decision */
          /* FEC + MAC --> forward decision = FEC + ARP (EEI) */
          route_info.additional_info.eei.type = SOC_PPD_EEI_TYPE_OUTLIF;
  ...
  if (r_info->l3a_vrf == DPP_DEFAULT_VRF) {
  ...
  { 
    LOG_ERROR(BSL_LS_BCM_L3,
              (BSL_META_U(unit,
                          "soc_sand error %x\n"), ret));
    BCMDNX_ERR_EXIT_MSG(BCM_E_INTERNAL, (_BSL_BCM_MSG("_bcm_ppd_frwrd_ipv6_vrf_uc_route_add failed")));
  }
...
STATIC int _bcm_ppd_host_info_from_ppd_get(
   ...
   SOC_PPD_FRWRD_IP_ROUTE_STATUS       *route_status,
   bcm_l3_host_t                       *info
)
{
    int rv;
    ...
    /* FEC. For roo application in arad+, always a vxlan gport */
       ...
       rv = _bcm_dpp_gport_from_fwd_decision(unit,&(info->l3a_port_tgid),&frwrd_decision_local);
       BCMDNX_IF_ERR_EXIT(rv);
    }

    /* encap-id */
    if ( routing_info->frwrd_decision.additional_info.eei.type == SOC_PPD_EEI_TYPE_OUTLIF) {
        /* frwrd_type = 2 or 4 */
        _BCM_PETRA_L3_ITF_SET(info->encap_id, _BCM_PETRA_L3_ITF_ENCAP, routing_info->frwrd_decision.additional_info.eei.val.outlif); 
    }

    if (routing_info->frwrd_decision.additional_info.outlif.type == SOC_PPD_OUTLIF_ENCODE_TYPE_RAW) {
        if (routing_info->frwrd_decision.additional_info.outlif.val < SOC_DPP_CONFIG(unit)->l3.nof_rifs) {
            /* frwrd_type = 3 */
            _BCM_PETRA_L3_ITF_SET(info->l3a_intf, _BCM_PETRA_L3_ITF_ENCAP, routing_info->frwrd_decision.additional_info.outlif.val);
        }
        else {
            /* frwrd_type = 1 */
            _BCM_PETRA_L3_ITF_SET(info->encap_id, _BCM_PETRA_L3_ITF_ENCAP, routing_info->frwrd_decision.additional_info.outlif.val); 
        }      
    }
    ...
    if (routing_info->native_vsi != 0) {
        _BCM_PETRA_L3_ITF_SET(info->l3a_intf, _BCM_PETRA_L3_ITF_RIF, routing_info->native_vsi); 
    }
  ...
  )
{
  ...
  if (info->l3a_flags & BCM_L3_HIT_CLEAR) {
      host_flags  = SOC_PPD_FRWRD_IP_HOST_CLEAR_ON_GET;
  }
  ...
  )
{
  ...
  SOC_PPD_FEC_ID
    fec_id;
  SOC_PPD_FRWRD_IPV4_VPN_ROUTE_KEY
    route_key;
  SOC_SAND_SUCCESS_FAILURE
    success;

  BCMDNX_INIT_FUNC_DEFS;
  SOC_PPD_FRWRD_IPV4_VPN_ROUTE_KEY_clear(&route_key);
  ...
  )
{
  ...
  if (found == SOC_SAND_FALSE) {
    /* Entry not found */
    BCMDNX_ERR_EXIT_MSG(BCM_E_NOT_FOUND, (_BSL_BCM_MSG("soc_ppd_frwrd_ipv4_vrf_route_get failed")));
  } 
...
{ 
  ...
  /* Route key */
  vrf_route.is_ipv6        = SOC_SAND_FALSE;
  vrf_route.vrf_id         = info->l3a_vrf;
...
/* Function _ppd_xxxx */
int
  _bcm_ppd_frwrd_ipv6_glbl_info_get(
    int unit,
    bcm_l3_info_t *l3info
  )
{
  ...
  SOC_PPD_FRWRD_IPV6_GLBL_INFO
    glbl_info;
    
  BCMDNX_INIT_FUNC_DEFS;
    
  ret = soc_ppd_frwrd_ipv6_glbl_info_get(
          unit,
          &glbl_info
        );
  if (soc_sand_get_error_code_from_error_word(ret) != SOC_SAND_OK)
...
{  
    ...
    SOC_PPD_IP_ROUTING_TABLE_RANGE
      block_range;
    SOC_PPD_FRWRD_IPV6_UC_ROUTE_KEY *route_keys = NULL;
    SOC_PPD_FRWRD_IPV6_VPN_ROUTE_KEY *vpn_route_keys = NULL;
    SOC_PPD_FRWRD_DECISION_INFO
      *route_infos = NULL;
    SOC_PPD_FRWRD_IP_ROUTE_STATUS
    ...
    SOC_PPD_IP_ROUTING_TABLE_RANGE_clear(&block_range);

    BCMDNX_ALLOC(route_keys, 
	      sizeof(SOC_PPD_FRWRD_IPV6_UC_ROUTE_KEY) * DPP_FRWRD_IP_GET_BLOCK_NOF_ENTRIES, 
	      "UC Route Key Temp Storage");
    if (route_keys) {
        sal_memset(route_keys, 0, sizeof(SOC_PPD_FRWRD_IPV6_UC_ROUTE_KEY) * DPP_FRWRD_IP_GET_BLOCK_NOF_ENTRIES);
    } else {
        BCMDNX_ERR_EXIT_MSG(BCM_E_MEMORY, (_BSL_BCM_MSG("UC Route Keys Storage Unavailable")));
    }

    BCMDNX_ALLOC(vpn_route_keys, 
	      sizeof(SOC_PPD_FRWRD_IPV6_VPN_ROUTE_KEY) * DPP_FRWRD_IP_GET_BLOCK_NOF_ENTRIES, 
	      "VPN Route Key Temp Storage");
    if (vpn_route_keys) {
        sal_memset(vpn_route_keys, 0, sizeof(SOC_PPD_FRWRD_IPV6_VPN_ROUTE_KEY) * DPP_FRWRD_IP_GET_BLOCK_NOF_ENTRIES);
    } else {
    ...
    block_range.start.type = SOC_PPD_IP_ROUTING_TABLE_ITER_TYPE_PREFIX_IP_ORDERED;
    /* SOC_PPC_IP_ROUTING_TABLE_ITER_BEGIN(block_range.start.payload); */
    soc_sand_u64_clear(&block_range.start.payload);

    BCMDNX_ALLOC(route_infos, sizeof(SOC_PPD_FRWRD_DECISION_INFO) * DPP_FRWRD_IP_GET_BLOCK_NOF_ENTRIES, "_bcm_ppd_frwrd_ipv6_route_get_block.route_infos");
    if (route_infos == NULL) {        
        BCMDNX_ERR_EXIT_MSG(BCM_E_MEMORY, (_BSL_BCM_MSG("failed to allocate memory")));
    }
    for (vrf_ndx=BCM_L3_VRF_DEFAULT; vrf_ndx<256; vrf_ndx++)
    {
      soc_sand_u64_clear(&block_range.start.payload);
  ...
  )
{
  ...
  SOC_PPD_VRF_ID
    vrf_ndx;
  SOC_PPD_FRWRD_IPV6_VPN_ROUTE_KEY
    route_key;
  SOC_SAND_SUCCESS_FAILURE
    success;

  BCMDNX_INIT_FUNC_DEFS;
  SOC_PPD_FRWRD_IPV6_VPN_ROUTE_KEY_clear(&route_key);
  ...
  )
{
  ...
  soc_sand_rv = soc_ppd_frwrd_ipv6_vrf_route_get(unit,
                                         vrf_ndx,
                                         &route_key,
                                         get_flags,
                                         &route_info,
  ...
  _bcm_l3_fec_to_intf(unit, route_info.dest_id, &(info->vrf_intf));
  info->encap_id = 0;
  if (route_info.additional_info.eei.type == SOC_PPD_EEI_TYPE_OUTLIF) {
      /* Retrieve the encap-id for Host table */
      _BCM_PETRA_L3_ITF_SET(info->encap_id, _BCM_PETRA_L3_ITF_ENCAP, route_info.additional_info.eei.val.outlif); 
  }
  ...
  )
{
    ...
    SOC_PPD_FRWRD_FEC_MATCH_RULE_clear(&rule);
    ...
    rule.type = SOC_PPD_FRWRD_FEC_MATCH_RULE_TYPE_ALL;
    rule.value = 0;
...
{  
    ...
    BCMDNX_ALLOC(route_keys, sizeof(SOC_PPD_FRWRD_IPV4_UC_ROUTE_KEY)*DPP_FRWRD_IP_GET_BLOCK_NOF_ENTRIES, "_bcm_ppd_frwrd_ipv4_route_get_block.route_keys");
    if (route_keys == NULL) {        
        BCMDNX_ERR_EXIT_MSG(BCM_E_MEMORY, (_BSL_BCM_MSG("failed to allocate memory")));
    }
  ...
  )
{
    uint32
        ret,
        nof_entries = 0,
        host_flags=SOC_PPD_FRWRD_IP_HOST_GET_ACCESSS_STATUS;
    ...
    if (flags & BCM_L3_HIT) {
        host_flags |= SOC_PPD_FRWRD_IP_HOST_GET_ACCESSED_ONLY;
    }
    ...
    BCMDNX_ALLOC(routes_info, sizeof(SOC_PPD_FRWRD_IPV4_HOST_ROUTE_INFO)*DPP_FRWRD_IP_GET_BLOCK_NOF_ENTRIES, "_bcm_ppd_frwrd_ipv4_host_get_block.routes_info");
    if (routes_info == NULL) {        
        BCMDNX_ERR_EXIT_MSG(BCM_E_MEMORY, (_BSL_BCM_MSG("failed to allocate memory")));
    }

    BCMDNX_ALLOC(host_keys, sizeof(SOC_PPD_FRWRD_IPV4_HOST_ROUTE_INFO)*DPP_FRWRD_IP_GET_BLOCK_NOF_ENTRIES, "_bcm_ppd_frwrd_ipv4_host_get_block.host_keys");
    if (host_keys == NULL) {        
        BCMDNX_ERR_EXIT_MSG(BCM_E_MEMORY, (_BSL_BCM_MSG("failed to allocate memory")));
    }
  ...
  )
{
  ...
  SOC_PPD_EG_ENCAP_RANGE_INFO_clear(&eg_encap_range_info);

  eg_encap_range_info.ll_limit = L3_LL_EEP_MAX;

  /* The range between ll_limit and ip_tnl_limit 
   * will be associated with IP TUNNEL.
   * The range after ip_tnl_limit and 12K-1 
   * will be associated with MPLS and PWE 
   */
  eg_encap_range_info.ip_tnl_limit = (L3_LL_EEP_MAX * 2);
  ...
  if(SOC_IS_PETRAB(unit)){
      ...
      if (soc_sand_get_error_code_from_error_word(ret) != SOC_SAND_OK)
      {
        LOG_ERROR(BSL_LS_BCM_L3,
                  (BSL_META_U(unit,
                              "soc_sand error %x\n"), ret));
        BCMDNX_ERR_EXIT_MSG(BCM_E_INTERNAL, (_BSL_BCM_MSG("soc_ppd_eg_encap_range_info_set failed")));
      }
      /* LCOV_EXCL_STOP */
  }
  ...
  )
{
  ...
  SOC_PPD_MYMAC_VRRP_INFO
    vrrp_info;

  BCMDNX_INIT_FUNC_DEFS;

  SOC_PPD_MYMAC_VRRP_INFO_clear(&vrrp_info);
  ...
  )
{
  ...
  vrrp_info.enable = SOC_SAND_TRUE;
  ...
  if (SOC_IS_PETRAB(unit)) {
       ...
       *  The LSB of the MAC addresses is set per VSI. Up to 2 MAC
       *  addresses may be set per VSI.
       */

      /*
       * SOC_PPC_MYMAC_VRRP_MODE_256_VSI_BASED
       *  The LSB of the MAC addresses is set for each VSI-only
       *  for VSIs [0,256]. Up to 32 MAC addresses may be set for
       *  each such VSI.
       */
      ...
      if (flags & BCM_L3_IP6) {
          vrrp_info.vrid_my_mac_msb.address[1] = 2;
      } else {
...
/*
 * Function:
 *      bcm_dpp_l3_check_init
 * Purpose:
 *      Check if L3 is initialized
 * Parameters:
 *      unit     - Device Number
 * Returns:
 *      BCM_E_XXX
...
STATIC int _bcm_dpp_l3_check_init(int unit)
{
    int init;
    uint8 is_allocated;
 ...
 *      bcm_dpp_l3_lock
 * Purpose:
 *      Take the l3 software module Lock Sempahore
 * Parameters:
 *      unit     - Device Number
 * Returns:
 *      BCM_E_XXX
...
{
    BCMDNX_INIT_FUNC_DEFS;
    _rv = _bcm_dpp_l3_check_init(unit);
 ...
 *      bcm_dpp_l3_unlock
 * Purpose:
 *      Release  the L3 software module Lock Semaphore
 * Parameters:
 *      unit     - Device Number
 * Returns:
 *      BCM_E_XXX
    ...
    int unit)
{
    ...
    if (SOC_IS_ARAD(unit)) {
        ...
        switch(dpp_l3->vrrp_max_vid) {
          case 4096:
            vrrp_mode = SOC_PPD_MYMAC_VRRP_MODE_ALL_VSI_BASED;
            break;
          case 2*1024:
            vrrp_mode = SOC_PPD_MYMAC_VRRP_MODE_2K_VSI_BASED;
            break;
          case 1024:
            vrrp_mode = SOC_PPD_MYMAC_VRRP_MODE_1K_VSI_BASED;
            break;
          case 512:
            vrrp_mode = SOC_PPD_MYMAC_VRRP_MODE_512_VSI_BASED;
            break;
          case 256:
            vrrp_mode = SOC_PPD_MYMAC_VRRP_MODE_256_VSI_BASED;
            break;
        default:
        ...
        if (((vrrp_mode == SOC_PPD_MYMAC_VRRP_MODE_ALL_VSI_BASED) || (vrrp_mode == SOC_PPD_MYMAC_VRRP_MODE_2K_VSI_BASED))
    ...
    int unit)
{
    ...
    if (init == TRUE) {
        ...
        if (SOC_DPP_CONFIG(unit)->pp.ip_lif_dummy_id &&
            bcm_dpp_am_l3_lif_is_allocated(unit, _BCM_DPP_AM_L3_LIF_IP_TERM, 0, SOC_DPP_CONFIG(unit)->pp.ip_lif_dummy_id)) {
            rv = bcm_dpp_am_l3_lif_dealloc(unit, SOC_DPP_CONFIG(unit)->pp.ip_lif_dummy_id);
            BCMDNX_IF_ERR_EXIT(rv);
            rv = _bcm_dpp_in_lif_match_delete(unit, SOC_DPP_CONFIG(unit)->pp.ip_lif_dummy_id);
            BCMDNX_IF_ERR_EXIT(rv);
        }
    ...
    bcm_l3_intf_t *intf)
{
    ...
    if (intf->l3a_vid && (intf->l3a_vid >= SOC_DPP_CONFIG(unit)->l3.nof_rifs) && (SOC_DPP_CONFIG(unit)->l3.nof_rifs >= BCM_VLAN_DEFAULT)) {
        LOG_ERROR(BSL_LS_BCM_L3,
                  (BSL_META_U(unit,
                              "invalid vid(0x%x)\n"), intf->l3a_vid));
        BCM_RETURN_VAL_EXIT(BCM_E_PARAM);
  ...
  if (!BCM_DPP_NATIVE_ROUTING_VLAN_TAGS_VALID(intf->native_routing_vlan_tags)) {
        LOG_ERROR(BSL_LS_BCM_L3,
                  (BSL_META_U(unit,
    ...
    if (status == BCM_E_NONE)
    {
        ...
        if (status == BCM_E_NONE)
        {
            status = _bcm_ppd_rif_vsid_map_set(unit, intf, 1);
            if (status == BCM_E_NONE)
            {
 ...
 * Purpose:
 *      Create a new L3 interface for mpls usage
 * Parameters:
 *      unit - (IN) Unit number.
 *      intf - (IN/OUT) <UNDEF>
 * Returns:
    ...
    bcm_vlan_t vlan)
{
   ...
   */
    ...
    if (status == BCM_E_NONE)
    {
      status = _bcm_ppd_mymac_msb_set(unit, mac);

      if (status == BCM_E_NONE)
      {
        status = _bcm_ppd_vsi_info_set(unit, vlan, 0x1,&intf);
        if (status == BCM_E_NONE)
        {
          status = _bcm_ppd_mpls_rif_vsid_map_set(unit, vlan);
          if (status == BCM_E_NONE)
          {
            status = _bcm_ppd_mymac_vsi_lsb_set(unit,vlan, mac);
          }
        }
      }
    }
  ...
  if (status == BCM_E_NONE) {
      if (!vsi_info.enable_my_mac) {
        ...
        if (!(flags & _BCM_PETRA_L3_INTF_IGNORE_NOT_FOUND)) {
          BCMDNX_ERR_EXIT_MSG(BCM_E_NOT_FOUND, (_BSL_BCM_MSG("Intf not found")));
        }
      }
      ...
      if (is_found_tmp) {
        ...
        if (status == BCM_E_NONE)
        {
          status = _bcm_ppd_rif_vsid_map_set(unit, intf, 0);
          if (status == BCM_E_NONE)
          {
            if (SOC_IS_ARADPLUS(unit) &&  SOC_IS_ROO_ENABLE(unit)) 
            {
                status = soc_ppd_rif_native_routing_vlan_tags_set(unit, intf->l3a_intf_id, 0);
            }
          }
        }
      }
    ...
    bcm_l3_intf_t *intf)
{
    ...
    SOC_PPD_EG_ENCAP_ENTRY_INFO encap_entry_info[1];
    uint32 next_eep[1];
    uint32 nof_entries;
             ...
             if (SOC_IS_ARAD(unit)) {
                  ...
                  if (vsi_info.enable_my_mac) {
                      ...
                      if ((*encap_entry_info).entry_val.vsi_info.remark_profile != 0) {
                          intf->dscp_qos.qos_map_id = BCM_QOS_MAP_PROFILE_GET((*encap_entry_info).entry_val.vsi_info.remark_profile);
                      }                    
                 }
             }
	         if (!vsi_info.enable_my_mac) {
    ...
    int ll_eep)
{
    ...
    BCMDNX_ALLOC(encap_entry_info, sizeof(SOC_PPD_EG_ENCAP_ENTRY_INFO)*SOC_PPD_NOF_EG_ENCAP_EEP_TYPES, "_bcm_tunnel_intf_ll_eep_set.encap_entry_info");
    if (encap_entry_info == NULL) {        
        BCMDNX_ERR_EXIT_MSG(BCM_E_MEMORY, (_BSL_BCM_MSG("failed to allocate memory")));
    }
    ...
    if ((encap_entry_info[1].entry_type == SOC_PPD_EG_ENCAP_ENTRY_TYPE_MPLS_ENCAP)
        ...
        last_entry_type = SOC_PPD_EG_ENCAP_ENTRY_TYPE_MPLS_ENCAP;
        if (SOC_IS_JERICHO(unit)) {
            mpls_encap_info.outlif_profile = encap_entry_info[1].entry_val.mpls_encap_info.outlif_profile;
        }
    } else
        if ((encap_entry_info[0].entry_type == SOC_PPD_EG_ENCAP_ENTRY_TYPE_MPLS_ENCAP)
       ...
       if (encap_entry_info[0].entry_type == SOC_PPD_EG_ENCAP_ENTRY_TYPE_IPV4_ENCAP) {
        /* memcpy, cause override, magic-num, so here one by one copy */
         ...
         update_eep = tunnel_eep;
         last_entry_type = SOC_PPD_EG_ENCAP_ENTRY_TYPE_IPV4_ENCAP;
        if (SOC_IS_JERICHO(unit)) {
            ipv4_encap_info.outlif_profile = encap_entry_info[0].entry_val.ipv4_encap_info.outlif_profile;
              ...
              soc_sand_rv = soc_ppd_eg_encap_next_outlif_update(
                        soc_sand_dev_id,
                        local_lif,
                        SOC_PPD_EG_ENCAP_ENTRY_UPDATE_NEXT_LIF,
                        ll_eep
                    );
              BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
              /* done as pointer was updated */
              BCM_EXIT;
    } else if (encap_entry_info[0].entry_type == SOC_PPD_EG_ENCAP_ENTRY_TYPE_PWE) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_INTERNAL, (_BSL_BCM_MSG("3 labels are not supported")));
    } else if (encap_entry_info[0].entry_type == SOC_PPD_EG_ENCAP_ENTRY_TYPE_SWAP_CMND) {
        mpls_swap_info.swap_label = 
          encap_entry_info[0].entry_val.swap_info.swap_label;
        mpls_swap_info.out_vsi = vsi; 
        update_eep = tunnel_eep;
        last_entry_type = SOC_PPD_EG_ENCAP_ENTRY_TYPE_SWAP_CMND;
        if (SOC_IS_JERICHO(unit)) {
            mpls_swap_info.outlif_profile = encap_entry_info[0].entry_val.swap_info.outlif_profile;
        }
    } else if (encap_entry_info[0].entry_type == SOC_PPD_EG_ENCAP_ENTRY_TYPE_POP_CMND) {
        mpls_pop_info.ethernet_info.out_vsi = 
          vsi;
        ...
        update_eep = tunnel_eep;
        last_entry_type = SOC_PPD_EG_ENCAP_ENTRY_TYPE_POP_CMND;
        if (SOC_IS_JERICHO(unit)) {
            mpls_pop_info.outlif_profile = encap_entry_info[0].entry_val.pop_info.outlif_profile;
        }
    } else if (SOC_PPD_EG_ENCAP_DATA_COUPLING_SWAP_TYPE(soc_sand_dev_id,encap_entry_info[0].entry_type, &(encap_entry_info[0].entry_val.data_info))) {
        for (index = 0; index < SOC_PPD_EG_ENCAP_DATA_INFO_MAX_SIZE; index++) {
          mpls_data_info.data_entry[index] =  
            encap_entry_info[0].entry_val.data_info.data_entry[index];
        }
    ...
    bcm_if_t *if_id)
{
    ...
    /* Verify parameters */
    if (_BCM_PETRA_L3_ITF_IS_RIF(egr->intf)) {    
    ...
    if (egr->flags & BCM_L3_ENCAP_SPACE_OPTIMIZED) {
        if (SOC_IS_ARADPLUS_A0(unit) && (soc_property_get(unit, spn_BCM886XX_ROO_ENABLE, 0) || soc_property_get(unit, spn_BCM886XX_ERSPAN_TUNNEL_ENABLE, 1))) {
    ...
    if (update) {
        ...
        if (!is_ingress) {
            ...
            l3_if_val = _BCM_PETRA_L3_ITF_VAL_GET(egr->encap_id);
            _BCM_PETRA_L3_ITF_SET(if_get, _BCM_PETRA_L3_ITF_ENCAP, l3_if_val);
        }
        else {
            ...
            l3_if_val = _BCM_PETRA_L3_ITF_VAL_GET(*if_id);
            _BCM_PETRA_L3_ITF_SET(if_get, _BCM_PETRA_L3_ITF_FEC, l3_if_val);
        }
    ...
    bcm_if_t *if_id)
{
    ...
    SOC_PPD_FEC_ID fec_id = 0,protect_fec_id = 0;
    ...
    /* In case INGRESS ONLY unset we do egress settings */
    is_egress = (!(flags & BCM_L3_INGRESS_ONLY)) ? TRUE:FALSE;
    /* The CASCADED flag indicates that the FEC is group B (pointed FEC). */
    fec_usage = SOC_IS_JERICHO(unit) && (egr->flags & BCM_L3_CASCADED) ? _BCM_DPP_AM_FEC_ALLOC_USAGE_CASCADED : _BCM_DPP_AM_FEC_ALLOC_USAGE_STANDARD;
    ...
    if (egr->mpls_label != BCM_MPLS_LABEL_INVALID) {
        ...
        } else if (_BCM_PETRA_L3_ITF_IS_ENCAP(egr->intf)) {
            intf_tunnel_global_lif = _BCM_PETRA_L3_ITF_VAL_GET(egr->intf);
    ...
    if (is_ingress) {
        if (flags & BCM_L3_WITH_ID) {
            ...
            fec_index = (int)fec_id;
            _BCM_PETRA_L3_ITF_SET((*if_id), _BCM_PETRA_L3_ITF_FEC, fec_id);

            fec_flags |= SHR_RES_ALLOC_WITH_ID;
            if ((flags & BCM_L3_REPLACE)) {
                /* check it's already alloced otherwise error */
                status = bcm_dpp_am_fec_is_alloced(unit, 0, 0, 1, fec_index);
            ...
            /* protection, if protecting alloc two*/
                ...
                /* protection: working */
                fec_index = _BCM_PETRA_L3_ITF_VAL_GET(egr->failover_if_id);
                fec_index = _BCM_L3_FEC_TO_WORK_FEC(fec_index);
            }
            _BCM_PETRA_L3_ITF_SET(*if_id, _BCM_PETRA_L3_ITF_FEC, fec_index); 
        }     
    ...
    if (is_egress) {    
        ...
        if ((encap_with_id) && (flags & BCM_L3_REPLACE)) { /* second call */
            ...
            if (SOC_IS_JERICHO(unit)) {
                status = bcm_dpp_am_global_lif_is_alloced(unit, BCM_DPP_AM_FLAG_ALLOC_EGRESS, tunnel_global_lif);
                if (status != BCM_E_EXISTS) {
                ...
                status = _bcm_l3_alloc_eep(unit, egr, &tunnel_local_lif, &tunnel_global_lif, &ll_local_lif, &ll_global_lif); 
                /* Checking this eep was allocated already by expecting BCM_E_RESOURCE */
            ...
            fec_eep_pointer = tunnel_global_lif;
            _BCM_PETRA_L3_ITF_SET(egr->encap_id,_BCM_PETRA_L3_ITF_ENCAP,ll_global_lif); 
        }    
        ...
        */
        ...
        if(SOC_IS_ARAD(unit) && (egr->intf > 0) && _BCM_PETRA_L3_ITF_IS_RIF(egr->intf)) { /* update outrif to point to LL, needed only in ARAD */
            SOC_PPD_EG_ENCAP_ENTRY_INFO_clear(encap_entry_info);
            SOC_PPD_EG_ENCAP_VSI_ENCAP_INFO_clear(&out_vsi_info);

            rif_entry_id = _BCM_PETRA_L3_ITF_VAL_GET(egr->intf);
            ret = soc_ppd_eg_encap_entry_get(unit,SOC_PPD_EG_ENCAP_EEP_TYPE_VSI,rif_entry_id,0,encap_entry_info,next_eep,&nof_entries);
            if (soc_sand_get_error_code_from_error_word(ret) != SOC_SAND_OK)
            {
              LOG_ERROR(BSL_LS_BCM_L3,
                        (BSL_META_U(unit,
    ...
    if(is_ingress) {
        ...
        if (is_tunnel_eei_mpls_command) {
            ...
            if (label_entry.action == BCM_MPLS_EGRESS_ACTION_PUSH){
                status = _bcm_mpls_tunnel_push_profile_alloc(unit, &identifier, FALSE, &label_entry, FALSE, FALSE, FALSE, TRUE, &is_first);
                if (status != BCM_E_NONE) {
                    BCMDNX_IF_ERR_EXIT(status);
                }
            } else /* BCM_MPLS_EGRESS_ACTION_SWAP */ {
                identifier = ARAD_PP_EEI_IDENTIFIER_SWAP_VAL;
            }
            eei_val = SOC_PPD_EEI_ENCODING_MPLS_COMMAND(identifier, egr->mpls_label);
        }
    ...
    bcm_if_t intf)
{
    ...
    if (_BCM_PETRA_L3_ITF_IS_ENCAP(intf)) { /* LL */
      ...
      if (SOC_IS_JERICHO(unit)) {
        rv = _bcm_dpp_gport_delete_global_and_local_lif(unit, global_lif, _BCM_GPORT_ENCAP_ID_LIF_INVALID, local_lif, TRUE);
        BCMDNX_IF_ERR_EXIT(rv);
      ...
      rv = _bcm_l3_dealloc_fec(unit, 0, 1, 1, fec_index);
      BCMDNX_IF_ERR_EXIT(rv); 
    }
...
bcm_dpp_l3_intf_arr_to_fec_info_arr(
  ...
  bcm_if_t *intf_array,
  int intf_count,
  SOC_PPD_FRWRD_FEC_ENTRY_INFO *fec_array,
  ...
  )
{
  ...
  BCMDNX_INIT_FUNC_DEFS;
  /*
  SOC_PB_PP_FRWRD_FEC_PROTECT_TYPE protect_type;
  SOC_PB_PP_FRWRD_FEC_ENTRY_INFO protect_fec;
  SOC_PB_PP_FRWRD_FEC_PROTECT_INFO protect_info; 
  */ 
...
bcm_petra_l3_ecmp_base_fec_get(
    int unit, 
    bcm_if_t ecmp_index, 
    SOC_PPD_FEC_ID *base_fec)
{
#ifdef BCM_ARAD_SUPPORT

    uint32 soc_sand_rv;
    SOC_PPD_FRWRD_FEC_ECMP_INFO ecmp_info;
#endif
    BCMDNX_INIT_FUNC_DEFS;
...
/**
 * Check if the API is bcm_l3_egress_multipath_* (according to SOC property).
 * If yes return BCM_E_NONE. 
 * If no return BCM_E_CONFIG.
 */
int bcm_petra_l3_ecmp_check_api_is_multipath(int unit)
{
  ...
  if (SOC_IS_PETRAB(unit)) {
    ...
    BCM_EXIT;
    /* LCOV_EXCL_STOP */
  /* } else if (soc_property_get(unit, "custom_feature_ecmp_api_multipath", 0) != 1) {
    BCMDNX_ERR_EXIT_MSG(BCM_E_CONFIG, 
                     ("Error - The bcm_l3_egress_multipath_* APIs can only be used if custom_feature_ecmp_api_multipath==1.\n"))); */
  } else {
    LOG_WARN(BSL_LS_BCM_L3,
             (BSL_META_U(unit,
                         "Warning - The bcm_l3_egress_multipath_* APIs are deprecated.\n")));
    LOG_WARN(BSL_LS_BCM_L3,
             (BSL_META_U(unit,
                         "          It is recommneded to use the bcm_l3_egress_ecmp_* APIs instead.\n")));
    BCM_EXIT;
  }
...
/**
 * Check if the API is bcm_l3_egress_ecmp_* (according to SOC property).
 * If yes return BCM_E_NONE. 
 * If no return BCM_E_CONFIG.
 */
int bcm_petra_l3_ecmp_check_api_is_ecmp(int unit)
{
  BCMDNX_INIT_FUNC_DEFS;
  ...
  if (SOC_IS_PETRAB(unit)) {
    /* LCOV_EXCL_START */
    BCMDNX_ERR_EXIT_MSG(BCM_E_CONFIG, 
                     (_BSL_BCM_MSG_STR("Error - The bcm_l3_egress_ecmp_* APIs are unsupported for this device. Use the bcm_l3_egress_multipath_* APIs instead.\n")));
  /* } else if (soc_property_get(unit, "custom_feature_ecmp_api_multipath", 0) != 0) {
    BCMDNX_ERR_EXIT_MSG(BCM_E_CONFIG, 
                     (_BSL_BCM_MSG_STR("Error - The bcm_l3_egress_ecmp_* APIs can only be used if custom_feature_ecmp_api_multipath==0 or unspecified.\n")));
                     */
    /* LCOV_EXCL_STOP */
  } else {
    BCM_EXIT;
...
{  
  ...
  uint32 ret = 0;
  SOC_PPD_FRWRD_FEC_ENTRY_INFO *fec_array = NULL; 
  SOC_PPD_FRWRD_FEC_PROTECT_TYPE *protect_type_array = NULL; 
  SOC_PPD_FRWRD_FEC_PROTECT_INFO *protect_info_array = NULL; 
  SOC_PPD_FRWRD_FEC_ENTRY_INFO *protect_array = NULL; 
  ...
  SOC_PPD_FEC_ID old_start_fec_id = 0;
  ...
  BCMDNX_ALLOC(fec_array, DPP_L3_MAX_ENTRIES_IN_ECMP * sizeof(SOC_PPD_FRWRD_FEC_ENTRY_INFO), "fec array");
  if (fec_array == NULL) {
    BCMDNX_ERR_EXIT_MSG(BCM_E_MEMORY, (_BSL_BCM_MSG("failed to allocate memory")));
  }
  BCMDNX_ALLOC(protect_array, DPP_L3_MAX_ENTRIES_IN_ECMP * sizeof(SOC_PPD_FRWRD_FEC_ENTRY_INFO), "protect array");
  if (protect_array == NULL) {  
    BCMDNX_ERR_EXIT_MSG(BCM_E_MEMORY, (_BSL_BCM_MSG("failed to allocate memory")));
  }
  BCMDNX_ALLOC(protect_type_array, DPP_L3_MAX_ENTRIES_IN_ECMP * sizeof(SOC_PPD_FRWRD_FEC_PROTECT_TYPE), "protect type array");
  if (protect_type_array == NULL) { 
    BCMDNX_ERR_EXIT_MSG(BCM_E_MEMORY, (_BSL_BCM_MSG("failed to allocate memory")));
  }
  BCMDNX_ALLOC(protect_info_array, DPP_L3_MAX_ENTRIES_IN_ECMP * sizeof(SOC_PPD_FRWRD_FEC_PROTECT_INFO), "protect info array");
  ...
  if (SOC_IS_PETRAB(unit)) {
      ...
      else
      {
        ...
        rv = _bcm_l3_alloc_fec(unit, shr_flags, 1, max_paths, &ecmp_index);
        BCMDNX_IF_ERR_EXIT(rv);
      ...
      {
        bcm_sw_db_l3_add_fec_mapping_to_ecmp(unit, flags, intf_array[intf_indx], *mpintf, ecmp_index + intf_indx);
      }

      /* convert intf array to fec array to be used in soc_pb_pp_frwrd_fec_ecmp_add */
      ...
      if (!ecmp_copy_info.is_successive) {
          ...
          for (i=0; i<intf_count; i++) {
              ...
              /* FIXEM if update add only changed */
      ...
      SOC_PPD_FRWRD_FEC_ECMP_INFO_clear(&ecmp_info);
      ...
      if(old_hanlde == 1 ) {/* reuse only of copied*/
          ...
          for(i = identicals; i < old_ecmp_curr_size; ++i) {
                ...
                /* dealloc fec indx */
                rv = _bcm_l3_dealloc_fec(unit, 0, 1, 1 + old_protected, old_start_fec_id + i);
                BCMDNX_IF_ERR_EXIT(rv);
      ...
      if(old_hanlde == 2){ /* remove */
          ...
          if(!old_successive) {
              for (index = 0; index < old_ecmp_curr_size; index++ ) {
                ...
                /* dealloc fec indx */
                rv = _bcm_l3_dealloc_fec(unit, 0, 1, 1 + old_protected, old_start_fec_id + index);
                BCMDNX_IF_ERR_EXIT(rv);
    ...
    bcm_if_t *mpintf)
{
    ...
    BCMDNX_IF_ERR_EXIT(bcm_petra_l3_egress_multipath_max_create(unit,flags,SOC_DPP_CONFIG(unit)->l3.ecmp_max_paths,intf_count,intf_array,mpintf));
exit:
    _BCM_DPP_SWITCH_API_END(unit);
    BCMDNX_FUNC_RETURN;
}
    ...
    bcm_if_t mpintf)
{
  ...
  uint32 ret; 
  SOC_PPD_FEC_ID fec_id,base_fec_id;
  int rv;
  int index;
  int fec_index;
  SOC_PPD_FEC_ID ecmp_index;    
  int ecmp_max_size;
  int ecmp_curr_size;
  ...
  if (SOC_IS_ARAD(unit)) {
      if ((ecmp_max_size==-1)) {
          /* if ecmp was alloced but empty, mark it as unused and dealloc it */
          bcm_sw_db_l3_set_ecmp_sizes(unit, mpintf, 0, 0);
          rv = _bcm_l3_dealloc_ecmp(unit, ecmp_index);
          BCMDNX_IF_ERR_EXIT(rv);
          BCM_EXIT;
      }
  }
  ...
  /* detach fecs in sw_db */
  for (index = 0; index < ecmp_curr_size; index++)
  {
    bcm_sw_db_l3_fec_remove_fec(unit, base_fec_id+index);    
  } 

  /* reset ecmp sizes in sw_db */
  bcm_sw_db_l3_set_ecmp_sizes(unit,mpintf,0,0);

  if (SOC_IS_PETRAB(unit)) {
      /* LCOV_EXCL_START */
      /* calling fec remove on the ecmp should delete all fecs in the ecmp (?)*/
      ret = soc_ppd_frwrd_fec_remove(
        unit,
        ecmp_index
      );
      ...
      rv = _bcm_l3_dealloc_fec(unit, 0, 1, ecmp_max_size, fec_index);
      BCMDNX_IF_ERR_EXIT(rv);
      /* LCOV_EXCL_STOP */
  }
      ...
      rv = bcm_sw_db_l3_get_ecmp_is_successive(unit,mpintf,&is_successive);
      BCMDNX_IF_ERR_EXIT(rv);

      /* only if not successize remove FECs */
      if(!is_successive) {
          for (index = 0; index < ecmp_info.size; index++ ) {
            ret = soc_ppd_frwrd_fec_remove(
            unit,
            ecmp_info.base_fec_id + index
            );
            if (soc_sand_get_error_code_from_error_word(ret) != SOC_SAND_OK)
            { 
                BCMDNX_ERR_EXIT_MSG(BCM_E_INTERNAL, (_BSL_BCM_MSG("soc_sand error %x"), ret));
            ...
            /* dealloc fec indx */
            rv = _bcm_l3_dealloc_fec(unit, 0, 1, 1 + ecmp_info.is_protected, ecmp_info.base_fec_id + index);
            BCMDNX_IF_ERR_EXIT(rv);
      ...
      ecmp_info.size = 1;
      ecmp_info.base_fec_id = 0;
      ret = soc_ppd_frwrd_fec_ecmp_info_set(unit,ecmp_index,&ecmp_info);
      if (soc_sand_get_error_code_from_error_word(ret) != SOC_SAND_OK)
      { 
          BCMDNX_ERR_EXIT_MSG(BCM_E_INTERNAL, (_BSL_BCM_MSG("soc_sand error %x"), ret));
      }
    ...
    int *intf_count)
{
  ...
  if ((SOC_IS_PETRAB(unit))) {
      ...
      if (ecmp_curr_size < intf_size)
      {
        intf_size = ecmp_curr_size;
      }
      ...
      *intf_count = intf_size;
      ...
      for (index = 0; index < intf_size; index++)
      {
        ...
        /* given copy-FEC return ECMP and real-FEC */
        bcm_sw_db_l3_copy_fec_to_ecmps_get(unit, mpintf + index, &fec_to_ecmps);
        ...
        {
          BCMDNX_ERR_EXIT_MSG(BCM_E_INTERNAL, (_BSL_BCM_MSG("ecmp does not match multipath interface")));
        }
        /* real FEC of the given copy */
        intf_array[index] = fec_to_ecmps.real_fec;        
      }  
      /* LCOV_EXCL_STOP */
  }
  else if (SOC_IS_ARAD(unit)) {
    ...
    bcm_if_t intf)
{
  ...
  if (SOC_IS_PETRAB(unit)) {
      ...
      bcm_sw_db_l3_add_fec_mapping_to_ecmp(unit, 0, intf, mpintf, mpintf + ecmp_curr_size);
      ...
      /* re-add the ecmp with extra path to the same position */
      ret = soc_ppd_frwrd_fec_ecmp_add(
          unit,
          ecmp_index,
          fec_array,
  ...
  if (SOC_IS_ARAD(unit)) {
    ...
    if (!ecmp_info.is_protected && !protect_type_array[index]==SOC_PPD_FRWRD_FEC_PROTECT_TYPE_NONE) {
    ...
    bcm_if_t intf)
{
  ...
  for(index = 0; index < ecmp_curr_size; index++)
  {
    ...
    if (SOC_IS_PETRAB(unit)) {
        /* LCOV_EXCL_START */
        /* remove the fec from sw_db since it will be added in a different position */
        rv = bcm_sw_db_l3_fec_remove_fec(unit, base_fec_id + index);
        BCMDNX_IF_ERR_EXIT(rv);   
        /* LCOV_EXCL_STOP */ 
    }
  }
  ...
  /* convert the intf array into a fec array */
  /*
  rv = bcm_dpp_l3_intf_arr_to_fec_info_arr(unit,intf_array,ecmp_curr_size-found_flag,fec_array);
  BCMDNX_IF_ERR_EXIT(rv);    
  */ 
  ...
  if (SOC_IS_PETRAB(unit)) {
      ...
      /* LCOV_EXCL_START */
      /* re-add the ecmp with the path intf removed to the same mpintf position */
      ret = soc_ppd_frwrd_fec_ecmp_add(
          unit,
          ecmp_index,
          fec_array,
          ecmp_curr_size-found_flag,
          &success
      );
      if (soc_sand_get_error_code_from_error_word(ret) != SOC_SAND_OK)
      { 
        ...
        /* add the new duplicated fec's mappings to the sw_db */  
      for (index = 0; (index < (ecmp_curr_size-found_flag)); index++)
      {
          rv = bcm_sw_db_l3_add_fec_mapping_to_ecmp(unit, 0, intf_array[index], mpintf, ecmp_index + index);
          BCMDNX_IF_ERR_EXIT(rv);    
      }
      /* LCOV_EXCL_STOP */
  }

  if (ecmp_curr_size != found_flag) {
      rv = bcm_sw_db_l3_set_ecmp_sizes(unit,mpintf,ecmp_max_size,ecmp_curr_size-found_flag);  
      BCMDNX_IF_ERR_EXIT(rv);    
  }
    ...
    void *user_data)
{
  ...
  while(!SOC_SAND_TBL_ITER_IS_END(&(block_range.iter)))
  {
    ...
    for(idx = 0; idx < nof_entries; idx++)
    { 
      _bcm_l3_fec_to_intf(unit, fec_ids[idx], &mpintf);

      
      /*        From the `if (ecmp_max_size == 0)` line it seems possible. */
      /*        The impl' of this seems to be a straightforward array access, so... */
      /* get the current size of the ecmp group */
      bcm_sw_db_l3_get_ecmp_sizes(unit, mpintf, &ecmp_max_size, &ecmp_curr_size);
      ...
      {
        rv = bcm_petra_l3_egress_multipath_get(
          unit, 
          mpintf, 
          16, 
          intf_array, 
      ...
      /* Invoke user callback. */
      (*trav_fn)(unit, fec_ids[idx], intf_count, intf_array, user_data);             
    }  
  }
...
{
  SOC_PPC_FEC_ID fec_ndx = _BCM_PETRA_L3_ITF_VAL_GET(ecmp_intf);
  ...
  )
{
  ...
  fec_ndx = _BCM_PETRA_L3_ITF_VAL_GET(ecmp->ecmp_intf);
 ...
 * 1. Is intf protection_type == SOC_PPD_FRWRD_FEC_PROTECT_TYPE_NONE ? If yes ecmp_info.exists_protected == 0, o/w ecmp_info.exists_protected == 1. 
...
{
  uint32                          soc_sand_rv;
  SOC_PPD_FRWRD_FEC_ECMP_INFO     ecmp_hw_info;
  ...
  ecmp_fec_ndx = _BCM_PETRA_L3_ITF_VAL_GET(ecmp->ecmp_intf);
  ...
  ecmp_info->last_fec = ecmp_info->first_fec + ecmp_info->curr_size * (1 + (ecmp_info->exists_protected ? 1 : 0));
  ...
  )
{
  ...
  if ((old_info.first_fec != ecmp_info->first_fec) ||
    ...
    SOC_PPD_FRWRD_FEC_ECMP_INFO_clear(&ecmp_hw_info);
  ...
  _bcm_petra_l3_is_fec_protected(int unit, bcm_if_t intf, uint8 *set_for_yes)
{
  ...
  BCMDNX_VERIFY(_BCM_PETRA_L3_ITF_IS_FEC(intf));

  soc_sand_rv = soc_ppd_frwrd_fec_entry_get(
                  (unit),
                  _BCM_PETRA_L3_ITF_VAL_GET(intf),
                  &protect_type,
                  &working_fec,
                  &protect_fec,
                  &protect_info
  ...
  *set_for_yes = (protect_type == SOC_PPD_FRWRD_FEC_PROTECT_TYPE_NONE) ? 0 : 1;
  ...
  )
{
  ...
  *err = ECMP_SIMPLE_FEC_MGMT_ERR_NO_ERROR;
  ...
  for (intf_idx = 0; intf_idx < intf_count; intf_idx++) {
    ...
    if (!_BCM_PETRA_L3_ITF_IS_FEC(intf_array[intf_idx])) {
      if (print_errs) {
        LOG_ERROR(BSL_LS_BCM_L3,
                  (BSL_META_U(unit,
    ...
    if (intf_idx > 0) {
      SOC_PPD_FEC_ID prev_fec_ndx = _BCM_PETRA_L3_ITF_VAL_GET(intf_array[intf_idx-1]);
      SOC_PPD_FEC_ID curr_fec_ndx = _BCM_PETRA_L3_ITF_VAL_GET(intf_array[intf_idx]);
  ...
  )
{
  ...
  if (!SOC_IS_ARAD(unit)) {
     /* LCOV_EXCL_START */
    BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG_STR("Error - The bcm_l3_egress_ecmp APIs are unsupported for this device. "
                                                "Use the bcm_l3_egress_multipath APIs instead. ")));
     /* LCOV_EXCL_STOP */
  }
  ...
  if (intf_count == 0) {
    ...
    ecmp_info.first_fec = _BCM_PETRA_L3_ITF_VAL_GET(intf_array[0]);
  }
  ...
  )
{
  ...
  rv = bcm_petra_l3_ecmp_check_api_is_ecmp(unit);
  BCMDNX_IF_ERR_EXIT(rv);
  ...
  )
{
  ...
  if (!SOC_IS_ARAD(unit)) {
    /* LCOV_EXCL_START */
    BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG_STR("The bcm_l3_egress_ecmp APIs are unsupported for this device. "
                                              "Use the bcm_l3_egress_multipath APIs instead. ")));
    /* LCOV_EXCL_STOP */
  }
  ...
  for (intf_array_idx = 0; intf_array_idx < max_idx; intf_array_idx++) {
    SOC_PPD_FEC_ID curr_fec_ndx = ecmp_info.first_fec + intf_array_idx * (1 + (ecmp_info.exists_protected ? 1 : 0));
  ...
  )
{
  ...
  if (ecmp->flags & ~(possible_ecmp_flags)) {
  ...
  rv = _bcm_petra_l3_egress_ecmp_get_internal(unit, ecmp, 0, NULL, &curr_intf_count);
  BCMDNX_IF_ERR_EXIT(rv);
  ...
  rv = _bcm_petra_l3_egress_ecmp_get_internal(unit, ecmp, curr_intf_count, first_intf, &intf_count_get);
  BCMDNX_IF_ERR_EXIT(rv);
  ...
  )
{
  ...
  /* Get the current intf_array. */
  rv = _bcm_petra_l3_egress_ecmp_get_internal(unit, ecmp, curr_intf_count, intf_array, &intf_count_get);
  BCMDNX_IF_ERR_EXIT(rv);
  BCMDNX_VERIFY(intf_count_get == curr_intf_count);
...
typedef struct _bcm_petra_l3_egress_ecmp_find_traverse_callback_data_s {
  SOC_PPD_FEC_ID lowest_fec_idx;
  SOC_PPD_FEC_ID highest_fec_idx;
  bcm_l3_egress_ecmp_t ecmp;
  uint8 success;
    ...
    bcm_if_t *intf_id)
{
    ...
    if ((if_id >= SOC_DPP_CONFIG(unit)->l3.nof_rifs) && (if_id != SOC_PPD_RIF_NULL)) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Invalid interface")));
    }
    ...
    if (if_id != SOC_PPD_RIF_NULL) {
        ...
        if (SOC_IS_ARADPLUS(unit) && soc_property_get(unit, spn_BCM886XX_L3_INGRESS_URPF_ENABLE, 0)) {
            ...
            switch (rif_info.uc_rpf_mode) {
                case bcmL3IngressUrpfLoose:
                    ...
                    rif_info.uc_rpf_enable = SOC_SAND_TRUE;
                    break;
                case bcmL3IngressUrpfStrict:
                    ...
                    break;
                case bcmL3IngressUrpfDisable:
                    rif_info.uc_rpf_enable = SOC_SAND_FALSE;
                    break;
                default:
                    LOG_ERROR(BSL_LS_BCM_L3,
                              (BSL_META_U(unit,
        ...
        rif_info.routing_enablers_bm = SOC_PPD_RIF_ROUTE_ENABLE_TYPE_ALL;
        if (ing_intf->flags & BCM_L3_INGRESS_ROUTE_DISABLE_IP4_UCAST) {
            rif_info.routing_enablers_bm &= ~SOC_PPD_RIF_ROUTE_ENABLE_TYPE_IPV4_UC;
        }
        if (ing_intf->flags & BCM_L3_INGRESS_ROUTE_DISABLE_IP6_UCAST) {
            rif_info.routing_enablers_bm &= ~SOC_PPD_RIF_ROUTE_ENABLE_TYPE_IPV6_UC;
        }

        if (ing_intf->flags & BCM_L3_INGRESS_ROUTE_DISABLE_IP6_MCAST) {
            rif_info.routing_enablers_bm &= ~SOC_PPD_RIF_ROUTE_ENABLE_TYPE_IPV6_MC;
        }
        if (ing_intf->flags & BCM_L3_INGRESS_ROUTE_DISABLE_IP4_MCAST) {
            rif_info.routing_enablers_bm &= ~SOC_PPD_RIF_ROUTE_ENABLE_TYPE_IPV4_MC;
        }
        ...
        if (SOC_IS_JERICHO(unit)) {
            if (soc_property_get(unit, spn_BCM886XX_L3_INGRESS_URPF_ENABLE, 0)) {
                /* consider URPF mode for RIF ID allocation */
                if ((rif_info.uc_rpf_enable == SOC_SAND_TRUE) && (rif_info.uc_rpf_mode == SOC_PPC_FRWRD_FEC_RPF_MODE_UC_STRICT)) {
                    rif_info.routing_enablers_bm |= _BCM_L3_INGRESS_URPF_IS_STRICT;  
                }
            }
            
            /* assign profile to the current input flags if it is anew profile need to configure the HW */            
    ...
    bcm_if_t intf_id)
{
    ...
    BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("bcm_petra_l3_ingress_destroy is not available"))); 
exit:
    BCMDNX_FUNC_RETURN;
}
    ...
    bcm_l3_ingress_t *ing_intf)
{
...
#ifdef BCM_88660_A0
    if (SOC_IS_ARADPLUS(unit) && soc_property_get(unit, spn_BCM886XX_L3_INGRESS_URPF_ENABLE, 0) && rif_info.uc_rpf_enable != SOC_SAND_FALSE) {
        ing_intf->urpf_mode = rif_info.uc_rpf_mode;
    }
#endif /* BCM_88660_A0 */
    ing_intf->flags = 0;    
    if (!(rif_info.routing_enablers_bm & SOC_PPD_RIF_ROUTE_ENABLE_TYPE_IPV4_UC)) {
        ing_intf->flags |= (BCM_L3_INGRESS_ROUTE_DISABLE_IP4_UCAST);
    }
    if (!(rif_info.routing_enablers_bm & SOC_PPD_RIF_ROUTE_ENABLE_TYPE_IPV6_UC)) {
    ...
    if (!(rif_info.routing_enablers_bm & SOC_PPD_RIF_ROUTE_ENABLE_TYPE_IPV6_MC)) {
        ing_intf->flags |= (BCM_L3_INGRESS_ROUTE_DISABLE_IP6_MCAST);
    }

    if (!(rif_info.routing_enablers_bm & SOC_PPD_RIF_ROUTE_ENABLE_TYPE_MPLS)) {
    ...
    bcm_l3_route_t *ip_addr)
{
    ...
    BCM_DPP_UNIT_CHECK(unit);
    _BCM_DPP_SWITCH_API_START(unit);
    BCMDNX_IF_ERR_EXIT(bcm_petra_l3_route_delete(unit, ip_addr));
exit:
    _BCM_DPP_SWITCH_API_END(unit);
    BCMDNX_FUNC_RETURN;
}
    ...
    bcm_l3_host_t *info)
{
    ...
    /* there is not a ppd host table clear */
    mode = DPP_L3_TRAVERSE_DELETE_ALL;
    if(SOC_IS_PETRAB(unit)){
        /* LCOV_EXCL_START */
        status = _bcm_ppd_frwrd_ipv4_host_get_block(unit, mode, info->l3a_flags,0, 0, 0);
        BCMDNX_IF_ERR_EXIT(status);  
        /* LCOV_EXCL_STOP */  
    }
    else{
...

{
    ...
    if (info->l3a_flags & BCM_L3_IP6) {
        ...
        else if (info->l3a_vrf == DPP_DEFAULT_VRF) {
    ...
    bcm_l3_route_t *info)
{
    ...
    if (info->l3a_flags & BCM_L3_IP6) {
        ...
        /* check this is not default route */
        if(info->l3a_ip_mask == 0) {
            LOG_ERROR(BSL_LS_BCM_L3,
                      (BSL_META_U(unit,
    ...
    if (info->l3a_vrf == DPP_DEFAULT_VRF) {
    ...
    int enable)
{
    ...
    if (!SOC_IS_ARAD(unit)) {
        /* LCOV_EXCL_START */
        BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("Only ARAD support this function")));
        /* LCOV_EXCL_STOP */
    }
    ...
    soc_sand_dev_id = (unit);
    BCMDNX_ALLOC(lif_info, sizeof(SOC_PPD_LIF_ENTRY_INFO), "bcm_petra_l3_source_bind_enable_set.lif_info");
    if (lif_info == NULL) {        
        BCMDNX_ERR_EXIT_MSG(BCM_E_MEMORY, (_BSL_BCM_MSG("failed to allocate memory")));
    }
    ...
    if (enable == 1){
        ...
        lif_info->value.ac.cos_profile |= SOC_PPD_LIF_AC_SPOOF_MODE_ENABLE_SET;
    }else {
        /* Disable l3 source bind in AC */
        lif_info->value.ac.cos_profile &= SOC_PPD_LIF_AC_SPOOF_MODE_DISABLE_SET;
    }
    ...
    int *enable)
{
    ...
    if (lif_info.value.ac.cos_profile & SOC_PPD_LIF_AC_SPOOF_MODE_ENABLE_SET) {
        *enable = 1;
    } else {
        *enable = 0;
    }
...
STATIC int
  _bcm_petra_l3_ip6_compression_to_ppd(
    ...
    int lif_id,
    bcm_l3_source_bind_t *info,
    SOC_PPD_IPV6_COMPRESSED_ENTRY *ip6_compression_entry
  )
{
    bcm_error_t rv = BCM_E_NONE;
    _bcm_dpp_gport_info_t gport_info;
    ...
    if (lif_id != _BCM_GPORT_ENCAP_ID_LIF_INVALID) {
        ...
        ip6_compression_entry->lif_ndx = lif_id;
    }
    ...
    if (_BCM_L3_SOURCE_BIND_IP6_COMPRESSION_SAV(unit, gport_info.local_port)) {
        ...
        if (ip6_compression_entry->ip6_tt_compressed_result) {
            ip6_compression_entry->flags = SOC_PPD_IP6_COMPRESSION_DIP;
        } 
        else{
            ip6_compression_entry->flags = SOC_PPD_IP6_COMPRESSION_SIP;
        }
    ...
    if (_BCM_L3_IP6_COMPRESSION_TCAM_GET(info->flags)) {
        ip6_compression_entry->flags |= SOC_PPD_IP6_COMPRESSION_TCAM;
    }
    if (_BCM_L3_IP6_COMPRESSION_LEM_GET(info->flags)) {
        ip6_compression_entry->flags |= SOC_PPD_IP6_COMPRESSION_LEM;
    }
    ...
    bcm_l3_source_bind_t *info)
{
    ...
    rv = _bcm_dpp_gport_to_hw_resources(unit, info->port, _BCM_DPP_GPORT_HW_RESOURCES_LOCAL_LIF_INGRESS | _BCM_DPP_GPORT_HW_RESOURCES_STRICT_CHECK, 
                                                 &gport_hw_resources);
    BCMDNX_IF_ERR_EXIT(rv);    

    if ((info->flags & BCM_L3_SOURCE_BIND_IP6) == 0) {
        SOC_PPD_SRC_BIND_IPV4_ENTRY ipv4_bind_info;

        SOC_PPD_SRC_BIND_IPV4_ENTRY_clear(&ipv4_bind_info);
        /* Fill SOC_PPD_SRC_BIND_IPV4_ENTRY and call soc_ppd_src_bind_ipv4_add() */
        ...
        ipv4_bind_info.lif_ndx = gport_hw_resources.local_in_lif;
        ...
        SOC_PPD_SRC_BIND_IPV6_ENTRY ipv6_bind_info;
        SOC_PPC_IPV6_COMPRESSED_ENTRY ip6_compression_entry;

        SOC_PPD_SRC_BIND_IPV6_ENTRY_clear(&ipv6_bind_info);
        sal_memset(&ip6_compression_entry, 0, sizeof(SOC_PPD_IPV6_COMPRESSED_ENTRY));

        if (_BCM_L3_SOURCE_BIND_IP6_COMPRESSION(info->rate_id)) {
            rv = _bcm_petra_l3_ip6_compression_to_ppd(unit, gport_hw_resources.local_in_lif, info, &ip6_compression_entry);
            BCMDNX_IF_ERR_EXIT(rv);
            soc_sand_rv = MBCM_PP_DRIVER_CALL(unit, mbcm_pp_ip6_compression_add, (unit, &ip6_compression_entry, &success));
        } else {
            /* Fill SOC_PPD_SRC_BIND_IPV6_ENTRY and call soc_ppd_src_bind_ipv6_add() */
            if (BCM_MAC_IS_ZERO(info->mac)) {
            ...
            ipv6_bind_info.lif_ndx = gport_hw_resources.local_in_lif;
  ...
  )
{
    ...
    BCMDNX_ALLOC(src_bind_ipv4s, sizeof(SOC_PPD_SRC_BIND_IPV4_ENTRY)*DPP_FRWRD_IP_GET_BLOCK_NOF_ENTRIES, "_bcm_ppd_frwrd_ipv4_host_get_block.src_bind_ipv4s");
...
{  
    ...
    block_range.entries_to_act = DPP_FRWRD_IP_GET_BLOCK_NOF_ENTRIES;
    block_range.entries_to_scan = DPP_FRWRD_IP_ENTRIES_TO_SCAN;
    block_range.start.type = SOC_PPD_IP_ROUTING_TABLE_ITER_TYPE_IP_PREFIX_ORDERED;
    soc_sand_u64_clear(&block_range.start.payload);
    ...
    BCMDNX_ALLOC(route_keys, sizeof(SOC_PPD_FRWRD_IPV4_UC_ROUTE_KEY)*DPP_FRWRD_IP_GET_BLOCK_NOF_ENTRIES, "_bcm_ppd_l3_source_ipv4_subnet_get_block.route_keys");
    if (route_keys == NULL) {        
        BCMDNX_ERR_EXIT_MSG(BCM_E_MEMORY, (_ERR_MSG_MODULE_NAME, unit, _BCM_MSG("failed to allocate memory")));
    }
  ...
  )
{
    ...
    /* Block range key */
    block_range_key.start.type = SOC_PPD_IP_ROUTING_TABLE_ITER_TYPE_IP_PREFIX_ORDERED;
    ...
    BCMDNX_ALLOC(src_bind_ipv6s, sizeof(SOC_PPD_SRC_BIND_IPV6_ENTRY)*DPP_FRWRD_IP_GET_BLOCK_NOF_ENTRIES, "_bcm_ppd_l3_source_ipv6_get_block.src_bind_ipv6s");
    ...
    void *user_data)
{
    ...
    if (!SOC_IS_ARAD(unit)) {
        /* LCOV_EXCL_START */
        BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_ERR_MSG_MODULE_NAME, unit, _BCM_MSG("Only ARAD support this function")));
        /* LCOV_EXCL_STOP */
    }
...
bcm_petra_l3_vrrp_add(
    ...
    uint32 vrid)
{
    int rv = BCM_E_NONE, soc_sand_dev_id = 0;
    int vsi = 0;
    SOC_PPD_MYMAC_VRRP_INFO vrrp_info;
    ...
    if (!BCM_VLAN_VALID(vlan)) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Vlan is out of range")));
    }
    ...
    BCM_SAND_IF_ERR_EXIT(soc_ppd_mymac_vrrp_info_get(soc_sand_dev_id, &vrrp_info));

    if (vrrp_info.mode == SOC_PPD_MYMAC_VRRP_MODE_ALL_VSI_BASED) {
       vsi = 2;
    } else {
       vsi = 256;
    }
    ...
    rv = _bcm_ppd_mymac_vrrp_mac_set(unit, vlan, vrid, SOC_SAND_TRUE);
    DPP_L3_LOCK_RELEASE;
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    uint32 vrid)
{
    ...
    rv = _bcm_ppd_mymac_vrrp_mac_set(unit, vlan, vrid, SOC_SAND_FALSE);
    DPP_L3_LOCK_RELEASE;
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    bcm_vlan_t vlan)
{
    ...
    for (vrid = 0; vrid < vsi; vrid++) {
        rv = bcm_petra_l3_vrrp_delete(unit, vlan, vrid);
        if (rv != BCM_E_NONE) {
            LOG_ERROR(BSL_LS_BCM_L3,
                      (BSL_META_U(unit,
                                  "unit %d vlan %d vrid %d failed\n"), unit, vlan, vrid));
            break;
        }
    }
    DPP_L3_LOCK_RELEASE;
...
bcm_petra_l3_vrrp_get(
    ...
    int *count)
{
    int soc_sand_dev_id = 0, rv = BCM_E_NONE;
    uint32 vrid = 0, vsi = 0;
    uint8 enable = 0;
    SOC_PPD_MYMAC_VRRP_INFO vrrp_info;
   ...
   if (vrrp_info.mode == SOC_PPC_MYMAC_VRRP_MODE_ALL_VSI_BASED) {
    ...
    if ((alloc_size < 2) || (alloc_size > vsi)) {
    ...
    uint8* mode)
{
    ...
    /* All checks are irrelevant for Jericho. Jericho supports all combinations of flags, and these values are constant. */
    if (SOC_IS_JERICHO(unit)) {
        *vsi = 256;
        *mode = SOC_PPD_MYMAC_VRRP_MODE_ALL_VSI_BASED;
        BCM_EXIT;
    }
    ...
    if (SOC_IS_ARAD(unit)) {
        if (vrrp_info.mode == SOC_PPD_MYMAC_VRRP_MODE_ALL_VSI_BASED) {
            *vsi = 8;
            max_vlan = 4096;
            /* no need to check max_vlan, because it is always 4096 */
        }
        else {
            *vsi = 256;
            if (vrrp_info.mode == SOC_PPD_MYMAC_VRRP_MODE_2K_VSI_BASED) {
                max_vlan = 2048;
            }
            else if (vrrp_info.mode == SOC_PPD_MYMAC_VRRP_MODE_1K_VSI_BASED) {
                max_vlan = 1024;
            }
            else if (vrrp_info.mode == SOC_PPD_MYMAC_VRRP_MODE_512_VSI_BASED) {
                max_vlan = 512;
            }
            else {
                max_vlan = 256;
                *mode = SOC_PPD_MYMAC_VRRP_MODE_256_VSI_BASED;
            }
        }

#ifdef BCM_88660_A0
        /* In ARADPLUS, all modes except for 256_vsi_based are considered to be all vsi mode.
           And in all vsi mode, max vsi is 256 and max vlan is 4096. */
        if (SOC_IS_ARADPLUS(unit) && max_vlan != 256) {
            *vsi = 256;
            max_vlan = 4096;
            *mode = SOC_PPD_MYMAC_VRRP_MODE_ALL_VSI_BASED;
        } else if (SOC_IS_ARADPLUS(unit)) {
            /* Configuring all VSIs is not supported in SOC_PPD_MYMAC_VRRP_MODE_256_VSI_BASED*/
        ...
        *ipv6_distinct = vrrp_info.ipv6_enable;
        }
        if (vrrp_info.ipv6_enable) { /* IPV6 distinct */
        ...
        /* in Soc_petra-B: VRRP mode is always shared between IPV4 and IPV6 */
        if (ipv6_distinct != NULL) {
            *ipv6_distinct = FALSE;
        }
        if (!((flags & BCM_L3_VRRP_IPV6) && (flags & BCM_L3_VRRP_IPV4))) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("flags: IPV4, IPV6 must be both set, because VRRP mode is not IPV6 distinct")));
        }
        /* LCOV_EXCL_STOP */
    }
    ...
    uint32 vrid)
{
...
#ifdef BCM_88660_A0
    if (SOC_IS_ARADPLUS(unit) && mode == SOC_PPD_MYMAC_VRRP_MODE_ALL_VSI_BASED){
    ...
    if (SOC_IS_ARAD(unit)) {
        ...
        if (ipv6_distinct && (flags & BCM_L3_VRRP_IPV6)) {
            rv = _bcm_ppd_mymac_vrrp_mac_set(unit, vlan, vrid, ARAD_PP_VRRP_MAC_ENABLE | ARAD_PP_VRRP_MAC_IS_IPV6);
        }
        else { /* IPV4 only or shared */
           /* ipv4 is the default */
           rv = _bcm_ppd_mymac_vrrp_mac_set(unit, vlan, vrid, ARAD_PP_VRRP_MAC_ENABLE);
        }
    }
    else { /* Soc_petra-B */
        /* LCOV_EXCL_START */
...
_bcm_l3_vrrp_mac_to_global_mac(int unit, SOC_PPC_VRRP_CAM_INFO *cam_info, uint32 flags, int vrid){
    ...
    } else if ((cam_info->is_ipv4_entry) && (!(flags & BCM_L3_VRRP_IPV4)) && (flags & BCM_L3_VRRP_IPV6)) { /* ipv6 */
...
_bcm_l3_vrrp_setting_add(int unit, SOC_PPC_VRRP_CAM_INFO *cam_info, int vlan){
    ...
    if (_BCM_PETRA_L3_VRRP_VLAN_IS_ALL_VSI(vlan)) {
        ...
        rv = _bcm_dpp_am_template_vrrp_alloc_all(unit, cam_index, &alloc_count);
        BCMDNX_IF_ERR_EXIT(rv);
        alloc_count++; /* Add one allocated entry for the first single allocation above */
    } else {
    ...
    if (SOC_IS_JERICHO(unit)) {
        rv = bcm_dpp_switch_l3_protocol_group_change_count(unit, cam_info->protocol_group, cam_info->protocol_group_mask, alloc_count);
        BCMDNX_IF_ERR_EXIT(rv);
    }
    ...
    if (_BCM_PETRA_L3_VRRP_VLAN_IS_ALL_VSI(vlan)) {
        ...
        rv = _bcm_ppd_mymac_vrrp_mac_set(unit, vlan, cam_index, ARAD_PP_VRRP_MAC_ENABLE);
    }
...
_bcm_l3_vrrp_setting_delete(int unit, int profile_ndx, SOC_PPC_VRRP_CAM_INFO *cam_info, int vlan){
    ...
    if (_BCM_PETRA_L3_VRRP_VLAN_IS_ALL_VSI(vlan)) {
        /* Dealloc all*/
        rv = _bcm_dpp_am_template_vrrp_dealloc_all(unit, profile_ndx, &dealloc_count);
        BCMDNX_IF_ERR_EXIT(rv);
        ...
        /* Update protocol group counts */
    if (SOC_IS_JERICHO(unit)) {
        rv = bcm_dpp_switch_l3_protocol_group_change_count(unit, cam_info->protocol_group, cam_info->protocol_group_mask, -(dealloc_count));
        BCMDNX_IF_ERR_EXIT(rv);
    }
    ...
    if (_BCM_PETRA_L3_VRRP_VLAN_IS_ALL_VSI(vlan)) {
        ...
        rv = _bcm_ppd_mymac_vrrp_mac_set(unit, vlan, profile_ndx, ARAD_PP_VRRP_MAC_DISABLE);
        BCMDNX_IF_ERR_EXIT(rv);
    }
    ...
    uint32 *count)
{
    ...
    rv = _bcm_l3_vrrp_config_verify(unit, flags, vlan, &vsi, NULL, &mode);
    BCMDNX_IF_ERR_EXIT(rv);

#ifdef BCM_88660_A0

    if (SOC_IS_ARADPLUS(unit) && mode == SOC_PPD_MYMAC_VRRP_MODE_ALL_VSI_BASED){
        /* Since _bcm_l3_vrrp_config_verify is used in different context in every call,
           the vsi doesn't always mean the same thing and needs to be changed manually.*/
        vsi = _BCM_PETRA_L3_VRRP_CAM_MAX_NOF_ENTIRES(unit);
    }
#endif /*BCM_88660_A0*/

    if (SOC_IS_ARAD(unit)) { /* In case of Jericho, even though there can be 16 VRIDs, we don't want to return errors 
    ...
    if ((alloc_size < min_vsi) || (alloc_size > vsi)) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Invalid alloc_size")));
    }
...
#endif  /* BCM_88660_A0 */        
        if (SOC_IS_ARAD(unit)) {
            if (((flags & BCM_L3_VRRP_IPV4) && (enable & ARAD_PP_VRRP_MAC_IPV4_ENABLE)) ||
                ((flags & BCM_L3_VRRP_IPV6) && (enable & ARAD_PP_VRRP_MAC_IPV6_ENABLE)))
            {
                vrid_array[(*count)++] = vrid;
                if (*count == alloc_size) 
                    break;
    ...
    uint32 vrid)
{
    ...
    if (SOC_IS_ARAD(unit)) {
        ...
        if (ipv6_distinct && (flags & BCM_L3_VRRP_IPV6)) { /* IPV6 only */
            rv = _bcm_ppd_mymac_vrrp_mac_set(unit, vlan, vrid, ARAD_PP_VRRP_MAC_DISABLE | ARAD_PP_VRRP_MAC_IS_IPV6);
        }
        else { /* IPV4 only or shared */
            /* ipv4 is the default */
            rv = _bcm_ppd_mymac_vrrp_mac_set(unit, vlan, vrid, ARAD_PP_VRRP_MAC_DISABLE);
        }
    }
    else { /* Soc_petra-B: always shared */
        /* LCOV_EXCL_START */
       /* ipv4 is the default */
       rv = _bcm_ppd_mymac_vrrp_mac_set(unit, vlan, vrid, ARAD_PP_VRRP_MAC_DISABLE);
        /* LCOV_EXCL_STOP */
