    ...
    bcm_stat_counter_lif_stack_id_t *lif_stack_id_to_count)
{
    ...
    SOC_TMC_CNT_LIF_COUNTING_MASK_SET(lif_counting_mask, 
    ...
    BCMDNX_IF_ERR_EXIT(rv);
    if (*lif_stack_id_to_count == bcmStatCounterLifStackId0 && counter_source == SOC_TMC_CNT_SRC_TYPE_EPNI) {
        *lif_stack_id_to_count = bcmStatCounterLifStackIdNone;
    }
exit:
    BCMDNX_FUNC_RETURN;
}
    ...
    bcm_stat_counter_lif_stack_id_t lif_stack_id_to_count)
{
    ...
    if (lif_stack_id_to_count == bcmStatCounterLifStackIdNone && counter_source == SOC_TMC_CNT_SRC_TYPE_EPNI) {
        lif_stack_id_to_count = bcmStatCounterLifStackId0;
    } else if (lif_stack_id_to_count == bcmStatCounterLifStackId0 && counter_source == SOC_TMC_CNT_SRC_TYPE_EPNI) {
    ...
    bcm_stat_counter_lif_counting_range_t *lif_range)
{
    ...
    BCMDNX_NULL_CHECK(lif_range);
    if (lif_range->start >= SOC_DPP_DEFS_GET(unit, nof_global_lifs) || 
        lif_range->end >= SOC_DPP_DEFS_GET(unit, nof_global_lifs)) {
