...
/* Free all MEP-entry struct relevant info
   When this is called by endpoint_create (update) the tx_rate should not be freed*/
int _bcm_mep_db_entry_dealloc(int unit, uint32 mep_index, SOC_PPD_OAM_OAMP_MEP_DB_ENTRY *mep_db_entry, int free_tx_rate) {
    int is_last;
    uint32 soc_sand_rv;
    int rv;
    SOC_PPD_OAMP_TX_ITMH_ATTRIBUTES tx_itmh_attributes;
...
int bcm_petra_bfd_endpoint_create(int unit, bcm_bfd_endpoint_info_t *endpoint_info) {
    ...
    correct_flags |= BCM_BFD_ENDPOINT_UPDATE | BCM_BFD_ENDPOINT_WITH_ID | BCM_BFD_ENDPOINT_IN_HW | BCM_BFD_ENDPOINT_MPLS_TP_POLL_SEQUENCE_ENABLE |
                     ...
                     BCM_BFD_ECHO | BCM_BFD_ENDPOINT_EXPLICIT_DETECTION_TIME;
    ...
    if (!_BCM_BFD_ENDPOINT_IS_ACCELERATED_TO_OAMP(*endpoint_info_lcl)) {
        ...
        if (endpoint_info_lcl->flags & BCM_BFD_ENDPOINT_EXPLICIT_DETECTION_TIME) {
            ...
            if (endpoint_info_lcl->remote_detect_mult > 0) {
                BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
...

        {
            ...
            uint32 bfd_period_max = SOC_IS_ARADPLUS_AND_BELOW(unit) ? 3406 /* floor(3.33*0x3ff) */ : 1703 /* floor(1.67*0x3ff) */ ;
    ...
    if (endpoint_info_lcl->flags & BCM_BFD_ENDPOINT_UPDATE) {
		...
		if (endpoint_info->gport != BCM_GPORT_INVALID || existing_endpoint_info.gport != BCM_GPORT_INVALID) {
            ...
            rv = _bcm_dpp_gport_to_hw_resources(unit, endpoint_info->gport, _BCM_DPP_GPORT_HW_RESOURCES_GLOBAL_LIF_INGRESS|_BCM_DPP_GPORT_HW_RESOURCES_STRICT_CHECK, &gport_hw_resources);
            BCMDNX_IF_ERR_EXIT(rv);
        ...
        if (_BCM_BFD_ENDPOINT_IS_ACCELERATED_TO_OAMP(*endpoint_info_lcl)) { 
            ...
            */
            ...
            rv = _bcm_mep_db_entry_dealloc(unit, endpoint_info_lcl->id, &mep_db_entry,0 /* do not free tx rate*/);
            BCMDNX_IF_ERR_EXIT(rv);
            SOC_PPD_OAM_OAMP_MEP_DB_ENTRY_clear(&mep_db_entry);
...
int bcm_petra_bfd_endpoint_destroy(
   ...
   bcm_bfd_endpoint_t endpoint) {
    ...
    if (((classifier_mep_entry.flags & SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_FLAG_ACCELERATED) != 0) && 
        ...
        rv = _bcm_mep_db_entry_dealloc(unit, endpoint, &mep_db_entry, 1 /* free TX rate */);
        BCMDNX_IF_ERR_EXIT(rv);
