...
{   
    ...
    if (is_init) {
        ...
        if (SOC_DPP_CONFIG(unit)->pp.pon_application_enable) {
            for (port_i = 0; port_i < _BCM_PPD_NOF_PON_PP_PORT; port_i++) {
    ...
    int unit)
{
    ...
    if (SOC_DPP_CONFIG(unit)->pp.global_lif_index_drop != SOC_PPC_AC_ID_INVALID) {
        ...
        BCM_GPORT_VLAN_PORT_ID_SET(vlan_port.vlan_port_id, SOC_DPP_ENCAP_ID_INGRESS_ONLY_FROM_LIF(SOC_DPP_CONFIG(unit)->pp.global_lif_index_drop));
    ...
    if (SOC_DPP_CONFIG(unit)->pp.global_lif_index_simple != SOC_PPC_AC_ID_INVALID) {
        ...
        bcm_vlan_port_t_init(&vlan_port);
        vlan_port.criteria = BCM_VLAN_PORT_MATCH_PORT;
        BCM_GPORT_VLAN_PORT_ID_SET(vlan_port.vlan_port_id, SOC_DPP_ENCAP_ID_INGRESS_ONLY_FROM_LIF(SOC_DPP_CONFIG(unit)->pp.global_lif_index_simple));
    ...
    uint32 flags) /* flags : same as BCM_VLAN_GPORT_ADD_XXX */
{
    ...
    if(input_is_vsi && (vid >= SOC_DPP_DEFS_GET(unit, nof_vsi_lowers))) {
    ...
    SOC_DPP_CORES_ITER(SOC_CORE_ALL, core) {
        ...
        rv = pbmp_from_ppd_port_bitmap_convert(unit, core, ports, SOC_PPD_VLAN_MEMBERSHIP_BITMAP_SIZE, _BCM_DPP_CONVERT_FLAG_PP_TO_LOCAL, &core_pbmp);
        BCMDNX_IF_ERR_EXIT(rv);
        ...
        rv = pbmp_to_ppd_port_bitmap_convert(unit, core,  &core_pbmp, SOC_PPD_VLAN_MEMBERSHIP_BITMAP_SIZE, _BCM_DPP_CONVERT_FLAG_LOCAL_TO_PP,
                                             ports);
        BCMDNX_IF_ERR_EXIT(rv);
    ...
    if (update_egress && update_tag) {
      ...
      BCM_PBMP_ITER(pbmp, port_i) {
          ...
          if (SOC_PORT_IS_STAT_INTERFACE(port_flags))
          {
              continue;
          }                  
          BCMDNX_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_local_to_pp_port_get, (unit, port_i, &soc_ppd_port_i, &core)));

          if (SOC_DPP_CONFIG(unit)->pp.pon_application_enable && _BCM_PPD_IS_PON_PP_PORT(port_i) 
                && IS_PON_PORT(unit,_BCM_PPD_GPORT_PON_TO_PHY_PORT(port_i))) {
    ...
    bcm_pbmp_t pbmp)
{
    ...
    SOC_DPP_CORES_ITER(SOC_CORE_ALL, core) {
        ...
        /* 1. Update ingress vlan membership */
        rv = pbmp_to_ppd_port_bitmap_convert(unit, core, &core_pbmp, SOC_PPD_VLAN_MEMBERSHIP_BITMAP_SIZE, _BCM_DPP_CONVERT_FLAG_LOCAL_TO_PP, ports);
        BCMDNX_IF_ERR_EXIT(rv);
    ...
    int *flags)
{
    ...
    if (!(internal_flags & (_BCM_DPP_VLAN_GPORT_GET_VSI_INFO_ONLY))) {
        ...
        rv = pbmp_from_ppd_port_bitmap_convert(unit, core, ports, SOC_PPD_VLAN_MEMBERSHIP_BITMAP_SIZE, _BCM_DPP_CONVERT_FLAG_PP_TO_LOCAL, &pbmp);
        BCMDNX_IF_ERR_EXIT(rv);
            ...
            if (port_info.dflt_egress_ac == 0) {
                if (!is_alloc_ac) {
                    ...
                    if (SOC_IS_JERICHO(unit)) {
                        ...
                        /* Allocate the global and local lif */
                        rv = _bcm_dpp_gport_alloc_global_and_local_lif(unit, 0, &global_out_ac, NULL, &out_lif_info, TRUE);
                        BCM_IF_ERROR_RETURN(rv);
    ...
    bcm_port_t port)
{
    ...
    if (SOC_IS_ARAD(unit)) {
            ...
            if (local_out_ac != 0) {
                ...
                if (tmp_rv == BCM_E_EXISTS) {
                    ...
                    if (SOC_IS_JERICHO(unit)) {
                        ...
                        rv = _bcm_dpp_gport_delete_global_and_local_lif(unit, global_out_lif, _BCM_GPORT_ENCAP_ID_LIF_INVALID, local_out_ac, FALSE);
                        BCMDNX_IF_ERR_EXIT(rv);
        ...
        soc_sand_rv = soc_ppd_eg_ac_info_set(unit, local_out_ac, &ac_info);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    }
    ...
    if (found == FALSE && is_allocate_in_case_of_not_found) {
      /* Get a fresh Out-AC */
        if (SOC_IS_JERICHO(unit)) {
            ...
            /* Allocate the global and local lif */
            rv = _bcm_dpp_gport_alloc_global_and_local_lif(unit, 0, &global_lif_id, NULL, &out_lif_info, TRUE);
            BCM_IF_ERROR_RETURN(rv);
    ...
    bcm_vlan_action_set_t *action)
{
  ...
  if (found == FALSE) {
        ...
        if (SOC_IS_JERICHO(unit)) {
            ...
            /* Allocate the global and local lif */
            rv = _bcm_dpp_gport_alloc_global_and_local_lif(unit, 0, &global_lif, &inlif_info, NULL, FALSE);
            BCM_IF_ERROR_RETURN(rv);
  ...
  if (found == TRUE) {
    ...
    if (is_in_ac_to_remove == FALSE) {      
      ...
      if (SOC_IS_JERICHO(unit)) {
          ...
          BCMDNX_IF_ERR_EXIT(rv);
          rv = _bcm_dpp_gport_delete_global_and_local_lif(unit, global_lif, lif_index, _BCM_GPORT_ENCAP_ID_LIF_INVALID, FALSE);
          BCMDNX_IF_ERR_EXIT(rv);
      } else if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
    ...
    bcm_vlan_action_set_t *action)
{
    ...
    if (SOC_IS_PETRAB(unit)) {        
        ...
        /* core - unsued for PetraB */
        rv = _bcm_dpp_am_template_tpid_profile_data_get(unit, ppd_port, core, &profile_type, &accept_mode, &dtag_mode, &ac_key_map_type);
        BCMDNX_IF_ERR_EXIT(rv);

        if (ac_key_map_type == _bcm_petra_ac_key_map_type_normal) {
            soc_sand_rv = soc_ppd_port_info_get(unit, core, ppd_port, &port_info);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);            

            new_ac_key_map_type = _bcm_petra_ac_key_map_type_compressional;
            
            /* Remove old, and add new */
            rv = _bcm_dpp_am_template_tpid_profile_exchange(unit, ppd_port, core, profile_type, accept_mode, dtag_mode, new_ac_key_map_type, 
                            &old_tpid_profile, &last_appear, &new_tpid_profile, &first_appear);
            BCMDNX_IF_ERR_EXIT(rv);
            
            port_info.port_profile = new_tpid_profile;
            soc_sand_rv = soc_ppd_port_info_set(unit, SOC_CORE_INVALID, ppd_port, &port_info);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    ...
    bcm_vlan_t inner_vlan_high)
{
    ...
    if (SOC_IS_PETRAB(unit) && (is_port_comp_vlan_existed == FALSE)) {
        ...
        if (ac_key_map_type == _bcm_petra_ac_key_map_type_compressional) {
            /* core - not used*/
            soc_sand_rv = soc_ppd_port_info_get(unit, SOC_CORE_INVALID, ppd_port, &port_info);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);            
    ...
    )
{
    ...
    if (ing_command_info) {
        ...
        if ((rv == BCM_E_NONE) && (flags & BCM_VLAN_ACTION_SET_TPID_ALLOW_DESTROY)) {
            if(nof_tpids >= 1) {
                _rv = sw_state_access[unit].dpp.bcm.port.dpp_port_config.bcm_port_tpid_info.tpid_count.get(unit, 0, ing_command_info->tpid_profile, &tpid_count);
                if (_rv != BCM_E_NONE) { \
                    BCMDNX_ERR_EXIT_MSG(_rv, (_BSL_BCM_MSG("%s: get tpid_count failed, unit %d"),FUNCTION_NAME(), unit));
                }
               --tpid_count;
                _rv = sw_state_access[unit].dpp.bcm.port.dpp_port_config.bcm_port_tpid_info.tpid_count.set(unit, 0, ing_command_info->tpid_profile, tpid_count);
                if (_rv != BCM_E_NONE) { \
                  BCMDNX_ERR_EXIT_MSG(_rv, (_BSL_BCM_MSG("%s: set tpid_count failed, unit %d"),FUNCTION_NAME(), unit));
                }
            }
            if(nof_tpids >= 2) {
                _rv = sw_state_access[unit].dpp.bcm.port.dpp_port_config.bcm_port_tpid_info.tpid_count.get(unit, 1, ing_command_info->tpid_profile, &tpid_count);
                if (_rv != BCM_E_NONE) { \
                    BCMDNX_ERR_EXIT_MSG(_rv, (_BSL_BCM_MSG("%s: get tpid_count failed, unit %d"),FUNCTION_NAME(), unit));
                }
                --tpid_count;
                _rv = sw_state_access[unit].dpp.bcm.port.dpp_port_config.bcm_port_tpid_info.tpid_count.set(unit, 1, ing_command_info->tpid_profile, tpid_count);
                if (_rv != BCM_E_NONE) { \
                  BCMDNX_ERR_EXIT_MSG(_rv, (_BSL_BCM_MSG("%s: set tpid_count failed, unit %d"),FUNCTION_NAME(), unit));
                }
            }
            ...
            if(nof_tpids >= 1) {
                _rv = sw_state_access[unit].dpp.bcm.port.dpp_port_config.bcm_port_tpid_info.tpid_count.get(unit, 0, tpid_profile, &tpid_count);
                if (_rv != BCM_E_NONE) { \
                    BCMDNX_ERR_EXIT_MSG(_rv, (_BSL_BCM_MSG("%s: get tpid_count failed, unit %d"),FUNCTION_NAME(), unit));
                }
                ++tpid_count;
                _rv = sw_state_access[unit].dpp.bcm.port.dpp_port_config.bcm_port_tpid_info.tpid_count.set(unit, 0, tpid_profile, tpid_count);
                if (_rv != BCM_E_NONE) { \
                  BCMDNX_ERR_EXIT_MSG(_rv, (_BSL_BCM_MSG("%s: set tpid_count failed, unit %d"),FUNCTION_NAME(), unit));
                }
            }
            if(nof_tpids  >= 2) {
                _rv = sw_state_access[unit].dpp.bcm.port.dpp_port_config.bcm_port_tpid_info.tpid_count.get(unit, 1, tpid_profile, &tpid_count);
                if (_rv != BCM_E_NONE) { \
                    BCMDNX_ERR_EXIT_MSG(_rv, (_BSL_BCM_MSG("%s: get tpid_count failed, unit %d"),FUNCTION_NAME(), unit));
                }
                ++tpid_count;
                _rv = sw_state_access[unit].dpp.bcm.port.dpp_port_config.bcm_port_tpid_info.tpid_count.set(unit, 1, tpid_profile, tpid_count);
                if (_rv != BCM_E_NONE) { \
                  BCMDNX_ERR_EXIT_MSG(_rv, (_BSL_BCM_MSG("%s: set tpid_count failed, unit %d"),FUNCTION_NAME(), unit));
                }
            }
    ...
    int *forced_tpid_profile)
{
    ...
    if ((is_ingress) && (!forced_tpid_profile) && (action_id < SOC_DPP_NOF_INGRESS_VLAN_EDIT_RESERVED_ACTION_IDS(unit))) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("%s: IVE first #%d action IDs are reserved and can't be modified"),
                            FUNCTION_NAME(), SOC_DPP_NOF_INGRESS_VLAN_EDIT_RESERVED_ACTION_IDS(unit)));
    }
    ...
    int *arg)
{
    ...
    {          
        SOC_PPD_LIF_ID local_lif_index_drop; 
    ...
    PBMP_ITER(pbmp_local_ports,port) {
        ...
        case bcmVlanPortUseOuterPri:
        {
            if(SOC_DPP_CONFIG(unit)->pp.custom_feature_vt_tst2)
            {
                ...
                port_info.vlan_translation_profile = (arg == 0) ? SOC_PPD_PORT_DEFINED_VT_PROFILE_DEFAULT: SOC_PPD_PORT_DEFINED_VT_PROFILE_PCP;
...
_bcm_petra_vlan_flooding_per_lif_set(int unit, 
                                     bcm_port_t port,
                                     SOC_PPD_LIF_ID global_lif_index,
    ...
    bcm_vlan_control_vlan_t control)
{
    ...
    int update_aging = 1; /* This flag gets 0 when entropy_id and aging_cycles are 0. In this case the aging configuration is skipped */
    ...
    bcm_gport_t gport)
{
    ...
    */
    ...
    rv = _bcm_dpp_gport_to_hw_resources(unit, gport, _BCM_DPP_GPORT_HW_RESOURCES_LOCAL_AND_GLOBAL_LIF | _BCM_DPP_GPORT_HW_RESOURCES_FEC, 
                                        &gport_hw_resources);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    if ((is_ingress || is_egress) && (lif_usage != _bcmDppLifTypeVlan)) { /* VLAN */
    ...
    rv = _bcm_dpp_gport_has_remote_lif(unit,gport,&remote_lif);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    if (gport_hw_resources.fec_id != -1) {
        ...
        remove_fec = TRUE;
        if (SOC_DPP_ENCAP_TYPE_GET(BCM_GPORT_VLAN_PORT_ID_GET(gport)) == _DPP_ENCAP_ID_FORWARD_VAL) {
    ...
    if ((!remote_lif) && is_egress && 
        ((gport_hw_resources.fec_id == -1) 
         ...
         || (SOC_DPP_ENCAP_TYPE_GET(BCM_GPORT_VLAN_PORT_ID_GET(gport)) == _DPP_ENCAP_ID_FORWARD_VAL))
       ) {
    ...
    if (!remote_lif) {
        ...
        if (is_local) {
            ...
            /* Delete outlif resources if it's a port's default lif. */
        ...
        if (SOC_IS_JERICHO(unit)) {
            ...
            if (is_local) {
                ...
                if (is_egress && (SOC_DPP_CONFIG(unit)->pp.pon_application_enable) && (BCM_E_EXISTS == bcm_dpp_am_out_ac_is_alloced(unit, gport_hw_resources.local_out_lif))) { 
                ...
                /* Decide outlif type.
                   Inlif type shouldn't be decided because it has only one type. */
                if (is_3_tags_data) {
    ...
    bcm_vlan_port_t *vlan_port)
{
    ...
    if (BCM_GPORT_IS_VLAN_PORT(vlan_port->vlan_port_id) ) {
        ...
        if(!remote_lif) {
            if (is_ingress) {
                ...
                if (SOC_IS_JERICHO(unit)) {
                    ...
                    rv = MBCM_PP_DRIVER_CALL(unit,mbcm_pp_lif_is_wide_entry,(unit, in_lif_id, TRUE, &is_wide_entry));
                    BCMDNX_IF_ERR_EXIT(rv);
        ...
        soc_sand_rv = soc_ppd_l2_lif_ac_get(soc_sand_dev_id,&in_ac_key,(SOC_PPD_LIF_ID*)&in_lif_id,&ac_info,&found);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        ...
        if(ac_info.learn_record.learn_info.type == SOC_PPC_FRWRD_DECISION_TYPE_FEC) {
            ...
            BCMDNX_IF_ERR_EXIT(_bcm_dpp_gport_l2_intf_calculate_port_id(unit, act_fec, global_lif_id, vlan_port, BCM_GPORT_VLAN_PORT));
        }
    ...
    if (!vlan_port->failover_id && !vlan_port->ingress_failover_id  /* These three fields indicate that protection info was */
        ...
        gport_val = _bcm_dpp_in_lif_match_learn_gport_get(unit, in_lif_id);
    ...
    bcm_vlan_port_translation_t *vlan_port_translation)
{
    ...
    if (is_ingress) {
        ...
        soc_sand_rv = soc_ppd_lif_table_entry_update(soc_sand_dev_id, local_in_lif_id, lif_ing_info);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    } 
    /* Handle the Egress scenario */
        ...
        /* Update the HW with the LIF info that includes the VLAN editing info for the LIF */
        soc_sand_rv = soc_ppd_eg_ac_info_set(soc_sand_dev_id, local_out_lif_id, lif_eg_info);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    }
...
int _bcm_gport_vlan_is_valid_replace(int unit, bcm_vlan_port_t *new_vlan_port, bcm_vlan_port_t *old_vlan_port, int remote_lif) {
    ...
    if (!remote_lif) {
        ...
        if (_BCM_PPD_IS_VALID_FAILOVER_ID(new_vlan_port->ingress_failover_id)) {
            ...
            if (old_vlan_port->ingress_failover_id == _BCM_PROTECTION_POINTER_INVALID) {
    ...
    if (SOC_IS_JERICHO(unit) && 
        ...
        BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("a CASCADED FEC may not be replaced with a regular FEC (and vice versa)")));
    }
...
/*
 * Function:
 *      _bcm_dpp_get_free_eg_coupled_local_lif
 * Purpose:
 *      Retrieve a Local LIF to be used when allocating a Global LIF due to egress protection
 *      in Coupled mode.
 * Parameters: 
 *      egress_failover_id  : (IN)  Failover IF of the Encap
 *      flags               : (IN)  The VLAN Port flags.
 *      local_lif_id        : (OUT) The returned local LIF ID.
 *  
 * Returns:
 *      BCM_E_XXX
 */
int _bcm_dpp_get_free_eg_coupled_local_lif(bcm_failover_t egress_failover_id,
                                           uint32 flags,
                                           SOC_PPD_LIF_ID *local_lif_id)
{
    *local_lif_id = 0;
    return BCM_E_NONE;
}
...
/*
 * Function:
 *      _bcm_dpp_get_free_eg_decoupled_local_lif
 * Purpose:
 *      Retrieve a Local LIF to be used when allocating a Global LIF due to egress protection
 *      in De-coupled mode.
 * Parameters: 
 ...
 *      BCM_E_XXX
 */
int _bcm_dpp_get_free_eg_decoupled_local_lif(bcm_failover_t egress_failover_id,
                                            uint32 flags,
                                            SOC_PPD_LIF_ID *local_lif_id)
{
    *local_lif_id = 0;
...
int _bcm_dpp_vlan_port_create(int unit, bcm_vlan_port_t *vlan_port)
{
    ...
    SOC_PPD_LIF_ID egress_protection_local_out_lif_id = 0;
    ...
    if (encap_with_id) {
        if(!BCM_ENCAP_REMOTE_GET(vlan_port->encap_id)) {
            ...
            if (rv != BCM_E_NOT_FOUND) {
                BCMDNX_IF_ERR_EXIT(rv);
                if (is_ingress) {
                    rv = _bcm_dpp_lif_usage_get(unit, local_lif_id, _BCM_GPORT_ENCAP_ID_LIF_INVALID, &used_type, NULL);
                } else {
                    rv = _bcm_dpp_lif_usage_get(unit, _BCM_GPORT_ENCAP_ID_LIF_INVALID, local_lif_id, NULL, &used_type);
                }
        ...
        rv = _bcm_dpp_gport_l2_intf_get_lif_from_port_id(vlan_port, is_protected, is_mc_ingress, (SOC_PPD_LIF_ID*) &global_lif_id, BCM_GPORT_VLAN_PORT);
        BCMDNX_IF_ERR_EXIT(rv);
    ...
    if(!remote_lif) {        
        /* Egress protection AC LIFs are half entries that should be assigned from an egress protection EEDB bank,
           ...
           TBD - The support for this flag in the relevant allocation function is yet to be added.
                 This solution may change according to the actual implementation of Global LIF allocation. */
        if (is_egress_protection) {
            /* In Coupled mode, the backup Out-LIF allocation reserves two Global LIFs that use
               the same full EEDB entry. When the call is for the Working Out-LIF, the Global LIF is
               only retrieved, as it was already allocated by the backup Out-LIF call.
               Expected logic: The supplied LIF ID of the backup Out-LIF will be used by the
               LIF allocation mechanism to allocate the reserved Global LIF.
               TBD - Need to use the local LIF ID if found */
            if (SOC_DPP_IS_PROTECTION_EGRESS_COUPLED(unit)) {
                global_lif_flags |= BCM_DPP_AM_FLAG_ALLOC_EG_COUPLED_PAIR;

                if (_BCM_PPD_GPORT_IS_EGRESS_WORKING_PORT(vlan_port)) {
                    rv = _bcm_dpp_get_free_eg_coupled_local_lif(vlan_port->egress_failover_id,
                                                                vlan_port->flags, &egress_protection_local_out_lif_id);
                    BCMDNX_IF_ERR_EXIT(rv);
                }
            /* In Decoupled mode, a logic should be applied in order to lookup for an unused (but allocated)
               EEDB half entry according to the supplied failover ID */
            } else {
                global_lif_flags = ((vlan_port->egress_failover_id) % 2) ?
                    BCM_DPP_AM_FLAG_ALLOC_EG_DECOUPLED_ODD : BCM_DPP_AM_FLAG_ALLOC_EG_DECOUPLED_EVEN;
                rv = _bcm_dpp_get_free_eg_decoupled_local_lif(vlan_port->egress_failover_id,
                                                              vlan_port->flags, &egress_protection_local_out_lif_id);
                BCMDNX_IF_ERR_EXIT(rv);
            }
        }
           ...
           ((encap_with_id) && BCM_ENCAP_ID_GET(vlan_port->encap_id) != BCM_ENCAP_ID_GET(vlan_port_o.encap_id))) {
            ...
            if (SOC_IS_JERICHO(unit)) {
                ...
                if (update) {                    
                    ...
                    rv = _bcm_dpp_gport_delete_global_and_local_lif(unit, global_lif_id_o, in_lif_id_o, out_lif_id_o, TRUE);
                    BCMDNX_IF_ERR_EXIT(rv);
                }
                ...
                if (is_egress && !skip_egress_alloc) {   
                    ...
                    local_out_lif_info.app_alloc_info.application_type =  (is_3_tags_manipulated) ? bcm_dpp_am_egress_encap_app_out_big_ac : bcm_dpp_am_egress_encap_app_out_ac;
                 ...
                 */                
                rv = _bcm_dpp_gport_alloc_global_and_local_lif(unit, global_lif_flags, &global_lif_id, local_inlif_info_p, local_out_lif_info_p, TRUE);
                BCM_IF_ERROR_RETURN(rv);
                ...
                if ((!is_egress) || skip_egress_alloc) {
                    ...
                    rv = bcm_dpp_am_l2_ac_alloc(unit, lif_allocation_type, local_lif_flags, (SOC_PPD_LIF_ID*) &global_lif_id);
                    BCMDNX_IF_ERR_EXIT(rv);
                } 
                else if (!is_ingress) {   /* egress only */
                    if (!is_3_tags_manipulated) {   /* Not pon */
                    ...
                    rv = bcm_dpp_am_out_ac_alloc(unit, lif_allocation_type, local_lif_flags, (SOC_PPD_LIF_ID*) &global_lif_id);
                    BCMDNX_IF_ERR_EXIT(rv);
                } 
                    ...
                    rv = bcm_dpp_am_l2_vpn_ac_alloc(unit, lif_allocation_type, local_lif_flags, (SOC_PPD_LIF_ID*) &global_lif_id);
                    BCMDNX_IF_ERR_EXIT(rv);
                }
                ...
                if (is_3_tags_manipulated && SOC_IS_ARAD_B1_AND_BELOW(unit)) {
                    ...
                    rv = bcm_dpp_am_out_ac_alloc(unit, _BCM_DPP_AM_OUT_AC_TYPE_PON_3_TAGS_OUT_AC, out_ac_flags, (SOC_PPD_LIF_ID*)&local_pon_out_ac_id);
                    BCMDNX_IF_ERR_EXIT(rv);
                }
    ...
    if(is_forward_group) {
        ...
        if(!update) {
            if (with_id) {
                rv = _bcm_dpp_gport_l2_intf_get_lif_from_port_id(vlan_port, is_protected, is_mc_ingress, &fec_id, BCM_GPORT_VLAN_PORT);
                BCMDNX_IF_ERR_EXIT(rv);
    ...
    /* 1+1 protection store gport-id (MC + is-primary) to LIF id */
        ...
        phy_port->type = _bcmDppGportParseTypeMC;
        ...
        if (is_egress) {
            ...
            if (!update || phy_port->type != _bcmDppGportResolveTypeRing) {                
                sal_memset(phy_port, 0, sizeof(_BCM_GPORT_PHY_PORT_INFO));
                if (is_forward_group) {
                    phy_port->type = _bcmDppGportResolveTypeFec;
    ...
    if(is_protected || (is_forward_group)) {
        ...
        rv = bcm_dpp_gport_mgmt_fec_remote_lif_set(unit, act_fec, remote_lif);
        BCMDNX_IF_ERR_EXIT(rv);
    ...
    /* Add lif type to OutLIF SW DB, even if it's remote lif. */
    if (is_egress) {
        active_local_out_lif_id = (is_3_tags_manipulated && SOC_IS_ARAD_B1_AND_BELOW(unit)) ? local_pon_out_ac_id : local_out_lif_id;
        BCMDNX_IF_ERR_EXIT(_bcm_dpp_out_lif_usage_set(unit, active_local_out_lif_id, _bcmDppLifTypeVlan));
    }
    ...
    if (_BCM_DPP_GPORT_INFO_IS_LOCAL_PORT(gport_info) && !remote_lif) {
            ...
            SOC_PPD_FRWRD_DECISION_INFO_clear(&learn_info);
            SOC_PPD_L2_LIF_AC_KEY_clear(&in_ac_key);
            ...
            if(update) {
                ...
                if (is_ingress) {
                    ...
                    soc_sand_rv = soc_ppd_l2_lif_ac_get(unit, &in_ac_key_o, (SOC_PPD_LIF_ID*)&in_lif_id_o, &in_ac_info, &found);
                    SOC_SAND_IF_ERR_RETURN(soc_sand_rv);
                }
            ...
            if(vlan_port->vsi == BCM_VLAN_NONE || vlan_port->flags & BCM_VLAN_PORT_VSI_BASE_VID) {
                ...
                if(!update) {
                    ...
                    if (is_ingress) {
                        ...
                        _bcm_dpp_in_lif_match_learn_gport_set(unit, local_in_lif_id, learn_gport);
                    }
                }                
                if (vlan_port->flags & BCM_VLAN_PORT_VSI_BASE_VID) {                  
                    ...
                    if (!update){
                       ...
                       in_ac_info.service_type = SOC_PPD_L2_LIF_AC_SERVICE_TYPE_MP; /* Must be MP */
                    }
                    ...
                    if (is_ingress && is_egress) {
                        ...
                        SOC_PPD_FRWRD_DECISION_INFO_clear(&in_ac_info.learn_record.learn_info);
                        in_ac_info.learn_record.learn_type = SOC_PPD_L2_LIF_AC_LEARN_SYS_PORT;
                        in_ac_info.learn_record.learn_info.additional_info.outlif.type = SOC_PPD_OUTLIF_ENCODE_TYPE_NONE;
                        in_ac_info.learn_record.learn_info.additional_info.eei.type = SOC_PPD_EEI_TYPE_EMPTY;
                        ...
                        if(in_ac_info.learn_record.learn_info.type != SOC_PPC_FRWRD_DECISION_TYPE_UC_PORT) {
                            in_ac_info.learn_record.learn_type = SOC_PPD_L2_LIF_AC_LEARN_INFO;
                        }
                    }
                }
            } else 
            ...
            if (is_egress_protection) {
                /* TBD - There will be a need to define protection_pointer & protection_path in SOC_PPD_EG_AC_INFO
                eg_ac_info.protection_pointer = vlan_port->egress_failover_id; 
                eg_ac_info.protection_path = _BCM_PPD_GPORT_IS_EGRESS_WORKING_PORT(vlan_port) ? 0 : 1; 
                */ 
            }
            ...
            in_ac_info.is_extended = (vlan_port->flags & BCM_VLAN_PORT_INGRESS_WIDE) ? TRUE:FALSE;
            ...
            if (is_ingress) {
                ...
                if(update) {
                    ...
                    if(sal_memcmp(&in_ac_key_o, &in_ac_key,sizeof(SOC_PPD_L2_LIF_AC_KEY))){
                        soc_sand_rv = soc_ppd_l2_lif_ac_remove(unit, &in_ac_key_o, (SOC_PPD_LIF_ID*)&in_lif_id_o);
                        SOC_SAND_IF_ERR_RETURN(soc_sand_rv);
                    }

                    if(in_lif_id_o != local_in_lif_id) {
                        rv = _bcm_dpp_in_lif_match_delete(unit,in_lif_id_o);
                        BCMDNX_IF_ERR_EXIT(rv);
                        rv = _bcm_dpp_in_lif_match_vsi_set(unit,in_lif_id_o, 0);
                        BCMDNX_IF_ERR_EXIT(rv);
                    }
                }
            ...
            if (is_egress && !skip_egress_alloc) {
...
#endif
                    {
                        rv = _bcm_dpp_pon_encap_data_enty_add(unit, local_out_lif_id, pon_tunnel_tpid, pon_tunnel_pcp, vlan_port->egress_tunnel_value, local_pon_out_ac_id);
                        BCMDNX_IF_ERR_EXIT(rv);
                    }
                }
                ...
                if(SOC_IS_ARAD(unit) &&
                    ...
                    soc_sand_rv = soc_ppd_eg_filter_split_horizon_out_lif_orientation_set(unit,active_local_out_lif_id,in_ac_info.orientation);
                    SOC_SAND_IF_ERR_RETURN(soc_sand_rv);
                }
                    ...
                    if(update) {
                        if (!cep_port || 
                            ...
                            if(sal_memcmp(&ac_port_vsi_key_o, &ac_port_vsi_key,sizeof(SOC_PPD_EG_AC_VBP_KEY))){
                                soc_sand_rv = soc_ppd_eg_ac_port_vsi_info_remove(unit, &ac_port_vsi_key_o, (SOC_PPD_AC_ID*)&out_lif_id_o);
                                SOC_SAND_IF_ERR_EXIT(soc_sand_rv);
                            }
                        } else {
                                ...
                                if(sal_memcmp(&ac_port_cep_key_o, &ac_port_cep_key,sizeof(SOC_PPD_EG_AC_CEP_PORT_KEY))){
                                    soc_sand_rv = soc_ppd_eg_ac_port_cvid_info_remove(unit, &ac_port_cep_key_o, (SOC_PPD_AC_ID*)&out_lif_id_o);
                                    SOC_SAND_IF_ERR_EXIT(soc_sand_rv);
                            }
                        }
                    }
                ...
                if (!update) {
                    soc_sand_rv = soc_ppd_eg_encap_next_outlif_update(unit, active_local_out_lif_id, SOC_PPD_EG_ENCAP_ENTRY_UPDATE_DROP, 0);
                    SOC_SAND_IF_ERR_EXIT(soc_sand_rv);
                }
            }
    }
