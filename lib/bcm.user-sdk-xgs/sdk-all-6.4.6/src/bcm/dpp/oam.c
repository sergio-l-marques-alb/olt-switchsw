...
int _bcm_oam_stat_lem_entry_add(uint32 unit, SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY *classifier_mep_entry, bcm_oam_endpoint_info_t *endpoint_info) {
	int rv;
    ...
    if (endpoint_info->flags & BCM_OAM_ENDPOINT_UP_FACING) {
    	...
    	payload.flags = ARAD_PP_FWD_DECISION_PARSE_DEST;
    	payload.dest = endpoint_info->id;
    	qual_vals[0].val.arr[0] = (gport_info.sys_port | 0xC0000);
    	qual_vals[0].val.arr[1] = 0;
 ...
 */
void
_bcm_dpp_oam_htb_cast_key_f(shr_htb_key_t key, uint8 **key_bytes, uint32 *key_size)
{
...
int _bcm_dpp_hash_tbl_save_oam_ma_name(void *orig_data, uint8 *saved_data_ptr) {
	...
	ma_name_p = (_bcm_oam_ma_name_t*)orig_data;
	sal_memcpy(saved_data_ptr, ma_name_p, sizeof(_bcm_oam_ma_name_t));
	return BCM_E_NONE;
}
...
int _bcm_dpp_hash_tbl_load_oam_ma_name(uint8 *wb_data_ptr, void **data_ptr) {
	_bcm_oam_ma_name_t * ma_name_p = NULL;

    ma_name_p = sal_alloc(sizeof(_bcm_oam_ma_name_t), "oam_ma_name");
	sal_memcpy(ma_name_p, wb_data_ptr, sizeof(_bcm_oam_ma_name_t));
...
_bcm_dpp_sw_db_hash_oam_ma_name_create(int unit)
{
	...
	rv = dpp_hash_tbl_wb_init(unit, SOC_DPP_WB_HASH_TBLS_OAM_MA_NAME,
							  ...
							  sizeof(uint32), 
							  SOC_PPD_OAM_MAX_NUMBER_OF_MAS(unit), 
							  sizeof(_bcm_oam_ma_name_t),
							  _bcm_dpp_hash_tbl_save_oam_ma_name, 
							  _bcm_dpp_hash_tbl_load_oam_ma_name);
	BCMDNX_IF_ERR_EXIT(rv);
#endif /*BCM_WARM_BOOT_SUPPORT*/

	shr_htb_cast_key_func_set(_bcm_dpp_oam_ma_index_to_name_db[unit], (shr_htb_cast_key_f)_bcm_dpp_oam_htb_cast_key_f);
...
/* Maximal number of MEPS supported by HW is 8K x 7 MD-levels. However, this consumes proportional
   ...
   If up and down mep aded on same lif and level this should be increased by 2. */
	...
	shr_htb_cast_key_func_set(_bcm_dpp_oam_bfd_mep_info_db[unit], (shr_htb_cast_key_f)_bcm_dpp_oam_htb_cast_key_f);
...
_bcm_dpp_sw_db_hash_oam_ma_to_mep_create(int unit)
{
	...
	shr_htb_cast_key_func_set(_bcm_dpp_oam_ma_to_mep_db[unit], (shr_htb_cast_key_f)_bcm_dpp_oam_htb_cast_key_f);
exit:
    BCMDNX_FUNC_RETURN;
}
...
#endif /*BCM_WARM_BOOT_SUPPORT*/
#endif
	shr_htb_cast_key_func_set(_bcm_dpp_oam_bfd_mep_to_rmep_db[unit], (shr_htb_cast_key_f)_bcm_dpp_oam_htb_cast_key_f);
exit:
    BCMDNX_FUNC_RETURN;
}
...
_bcm_dpp_sw_db_hash_oam_bfd_rmep_info_create(int unit)
{
	...
	BCMDNX_IF_ERR_EXIT(rv);
#endif /*BCM_WARM_BOOT_SUPPORT*/
	shr_htb_cast_key_func_set(_bcm_dpp_oam_rmep_info_db[unit], (shr_htb_cast_key_f)_bcm_dpp_oam_htb_cast_key_f);
exit:
    BCMDNX_FUNC_RETURN;
}
...
_bcm_dpp_sw_db_hash_oam_mep_to_ais_id_create(int unit)
{
	...
	shr_htb_cast_key_func_set(_bcm_dpp_oam_mep_to_ais_id[unit], (shr_htb_cast_key_f)_bcm_dpp_oam_htb_cast_key_f);
...
int _bcm_dpp_oam_ma_db_insert(int unit, uint32 ma_index, _bcm_oam_ma_name_t * ma_name) {
	int rv;
	_bcm_oam_ma_name_t * ma_name_temp_p = NULL;
    ...
    if (rv == BCM_E_NOT_FOUND) {
		BCMDNX_ALLOC(ma_name_temp_p, sizeof(_bcm_oam_ma_name_t), "oam_ma_name");
...
int _bcm_dpp_oam_ma_db_get(int unit, uint32 ma_index, _bcm_oam_ma_name_t * ma_name, uint8 *found) {
	...
	_bcm_oam_ma_name_t * ma_name_temp = NULL;
...
int _bcm_dpp_oam_ma_db_delete(int unit, uint32 ma_index) {
	int rv;
	_bcm_oam_ma_name_t * name_dummy = NULL;
 ...
 * @return int 
 */
int _bcm_oam_mep_db_entry_dealloc(int unit, uint32 mep_index, SOC_PPD_OAM_OAMP_MEP_DB_ENTRY *mep_db_entry, SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY * mep_info) {
    int rv;
    int dont_care,is_last;
    uint32 soc_sand_rv;
    BCMDNX_INIT_FUNC_DEFS;

    if (SOC_IS_JERICHO(unit) && mep_db_entry->mep_type == SOC_PPD_OAM_MEP_TYPE_ETH_OAM) {
    ...
    if (SOC_IS_ARAD_B0_AND_ABOVE(unit) && PPC_API_OAM_STORE_LOCAL_PORT_IN_MEP_DB(mep_db_entry->mep_type) &&
         ((!(mep_info->flags & SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY_FLAG_UPMEP)) || (mep_info->lif == _BCM_OAM_INVALID_LIF && mep_info->port) || mep_db_entry->system_port==0xffff) ) {
        /* This table has been set by :
                1. Down MEPs
                2. Server up MEPs.
                3. When using illegal destination. Needed to disable CCM transmission with CCM interval !=0
            Only for Arad B0 and above (due to bug fix) and Ethernet (table is not used for MPLS/PWE).*/
        int ignored;
...
int _bcm_dpp_oam_bfd_sys_port_to_tx_gport(int unit, bcm_gport_t * tx_gport, uint32 sys_port) {
    ...
    uint32  lag_id , lag_member_id, sys_phys_port_id,soc_sand_rv;
    BCMDNX_INIT_FUNC_DEFS;
...
int _bcm_oam_mep_db_entry_struct_set(int unit, bcm_oam_endpoint_info_t *endpoint_info, SOC_PPD_OAM_OAMP_MEP_DB_ENTRY *mep_db_entry, int icc_ndx, uint8 is_11b_maid) {
    ...
    uint32      pp_port;
    int         core;
    int using_fake_destination = (SOC_DPP_CONFIG(unit)->pp.oam_ccm_rx_wo_tx &&   !endpoint_info->ccm_period && (endpoint_info->flags & BCM_OAM_ENDPOINT_REPLACE));
    ...
    if (SOC_IS_ARADPLUS(unit)) {
        mep_db_entry->is_11b_maid = is_11b_maid;
    }
    ...
    if ((endpoint_info->flags & BCM_OAM_ENDPOINT_UP_FACING) == 0 || _BCM_OAM_IS_SERVER_SERVER(endpoint_info)  ) {
        ...
        if (SOC_IS_ARAD_B0_AND_ABOVE(unit) &&  PPC_API_OAM_STORE_LOCAL_PORT_IN_MEP_DB(mep_db_entry->mep_type)) {
            if (_BCM_OAM_IS_SERVER_SERVER(endpoint_info)  && (endpoint_info->flags & BCM_OAM_ENDPOINT_UP_FACING)) {
                ...
                local_port_for_real  = pp_port; /* Core will always be 0 in Arad anyways.*/
    ...
    if (using_fake_destination) {
        ...
        mep_db_entry->system_port = 0xffff; /* Illegal destination*/
        if (SOC_IS_ARAD_B0_AND_ABOVE(unit) &&  PPC_API_OAM_STORE_LOCAL_PORT_IN_MEP_DB(mep_db_entry->mep_type)) {
 ...
 * @return int 
 */
int _bcm_oam_mep_db_entry_struct_get(int unit, bcm_oam_endpoint_info_t *endpoint_info, SOC_PPD_OAM_OAMP_MEP_DB_ENTRY *mep_db_entry, int original_tx_gport) {
...
int _bcm_oam_classifier_mep_entry_struct_set(int unit, bcm_oam_endpoint_info_t *endpoint_info, SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY * classifier_mep_entry) {
	...
	classifier_mep_entry->md_level = (classifier_mep_entry->mep_type== SOC_PPD_OAM_MEP_TYPE_Y1731_O_MPLSTP  &&
    ...
    if (endpoint_info->gport != BCM_GPORT_INVALID) {
        ...
        int global_lif, local_out_lif; 
		/* In Jericho the OAM-LIF in the ingress is the global LIF and at the egress it is the local LIF.*/		
        ret = _bcm_dpp_gport_to_hw_resources(unit, endpoint_info->gport, _BCM_DPP_GPORT_HW_RESOURCES_GLOBAL_LIF_INGRESS | _BCM_DPP_GPORT_HW_RESOURCES_LOCAL_LIF_EGRESS,
                                             &gport_hw_resources);
        BCMDNX_IF_ERR_EXIT(ret);
...
int _bcm_oam_classifier_mep_entry_struct_get(int unit, bcm_oam_endpoint_info_t *endpoint_info, SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY * classifier_mep_entry) {
    ...
    if (classifier_mep_entry->mep_type!= SOC_PPD_OAM_MEP_TYPE_Y1731_O_MPLSTP || soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "oam_over_mpls_ignore_mdl", 0)==0) {
        ...
        if ( (endpoint_info->opcode_flags & BCM_OAM_OPCODE_CCM_IN_HW)==0) {
            endpoint_info->level= 0l;
        } 
    }
    ...
    if (classifier_mep_entry->lif != _BCM_OAM_INVALID_LIF) {
        ...
        int local_lif;
        ret = _bcm_dpp_global_lif_mapping_global_to_local_get(unit, _BCM_DPP_GLOBAL_LIF_MAPPING_INGRESS, (endpoint_info->flags & BCM_OAM_ENDPOINT_UP_FACING)?
															  classifier_mep_entry->passive_side_lif :  classifier_mep_entry->lif, &local_lif);
        BCMDNX_IF_ERR_EXIT(ret);
        endpoint_info->gport = _bcm_dpp_in_lif_match_gport_get(unit, local_lif);
        if (classifier_mep_entry->flags & SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY_FLAG_USING_ASYMETRIC_MPLS_LIF) {
            ...
            global_outlif = SOC_DPP_ENCAP_ID_FROM_LIF(global_outlif); 
...
int _bcm_dpp_endpoint_counters_set(int unit, bcm_oam_endpoint_info_t *endpoint_info, SOC_PPC_LIF_ID lif) {
	...
	BCMDNX_INIT_FUNC_DEFS;
    sal_memset(&voq_cnt, 0x0, sizeof(SOC_TMC_CNT_VOQ_PARAMS));
    ...
    if (endpoint_info->lm_counter_base_id > 0) {
        ...
        for (crps_index = 0; crps_index < SOC_DPP_DEFS_GET(unit, nof_counter_processors); crps_index++) {           
            rv = bcm_dpp_counter_config_get(unit, crps_index, &enabled, &src_core, &src_type, &command_id, &eg_type, &format, &we_val, &custom_mode_params, &voq_cnt, &stag_lsb);
            BCMDNX_IF_ERR_EXIT(rv);
            if (!enabled) {
                continue;
            }
...
int _bcm_dpp_oam_bfd_trap_code_from_remote_gport(int unit, 
             ...
             if (!is_mep) {
                 if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "egress_snooping_advanced", 0)
                    ...
                    BCMDNX_IF_ERR_EXIT(rv); 
                     ...
                     BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                     wb_mirror_profile = (uint8)mirror_id;
                     rv = SOC_DPP_WB_ENGINE_SET_ARR(unit, SOC_DPP_WB_ENGINE_VAR_ARAD_PP_OAM_TRAP_CODE_TO_MIRROR_PROFILE_MAP, &wb_mirror_profile, *trap_code);
                     BCMDNX_IF_ERR_EXIT(rv);
                 }
                 else {
                 ...
                 if (is_upmep) {
                     ...
                     for (trap_code_i = SOC_PPD_TRAP_CODE_OAM_FTMH_MIRROR_TRAP_CODES_FIRST;
                         ...
                         if (bcm_dpp_am_oam_trap_code_upmep_ftmh_header_alloced_check(unit, trap_code_i) == BCM_E_EXISTS) {
                             ...
                             BCMDNX_IF_ERR_EXIT(rv);
                             rv = _bcm_dpp_gport_compare(unit, trap_config.dest_port, trap_i_config.dest_port, &is_equal);
                             BCMDNX_IF_ERR_EXIT(rv);
    ...
    if (is_accelerated) { /* If destination is OAMP, trap code should be one of the specially allocated in init */
        ...
        BCMDNX_IF_ERR_EXIT(rv);
        rv = _bcm_dpp_gport_compare(unit, oamp_port[0], remote_gport, &is_equal);
        BCMDNX_IF_ERR_EXIT(rv);
        if (is_equal) {
...
int _bcm_dpp_oam_fifo_interrupt_handler(int unit, JER_PP_OAM_API_DMA_EVENT_TYPE event_type) {
	uint32 rmeb_db_ndx; 
	...
	soc_sand_rv = soc_ppd_oam_event_fifo_read(unit, event_type, &rmeb_db_ndx_internal, event_id_bitmap, &valid, event_found.event_data);
	BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    ...
    while (valid) {
        if (event_type == JER_PP_OAM_API_DMA_EVENT_TYPE_EVENT) {
            ...
            if (!found) {
                BCMDNX_ERR_EXIT_MSG(BCM_E_INTERNAL,
                                 (_BSL_BCM_MSG("Internal error in FIFO event processing: MEP wasn't found. mep_db_ndx=%d"),
                                  event_found.mep_or_rmep_indx));
            }
...
STATIC int _bcm_oam_generic_endpoint_validity_checks(int unit, const bcm_oam_endpoint_info_t *endpoint_info) {
    ...
    if (((endpoint_info->flags & BCM_OAM_ENDPOINT_REMOTE) || is_accelerated)
        ...
        if (endpoint_info->port_state || endpoint_info->interface_state) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
                                (_BSL_BCM_MSG("Error: port_state, interface_state only available in Jericho and for accelerated/remote endpoints.")));
    ...
    if (endpoint_info->flags & BCM_OAM_ENDPOINT_REMOTE) {
        ...
        if (SOC_IS_ARADPLUS(unit)) {
			...
			if ((endpoint_info->sampling_ratio < 0) || (endpoint_info->sampling_ratio > 8)) {
				BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
        ...
        if (is_accelerated) {
            ...
            if (endpoint_info->type == bcmOAMEndpointTypeEthernet) {
                ...
                if ((endpoint_info->gport != BCM_GPORT_INVALID) && BCM_GPORT_IS_MPLS_PORT(endpoint_info->gport)) {
                    BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
                         (_BSL_BCM_MSG("Error: Can't create accelerated OAMoETHoPWE endpoint\n")));
                }
            } else {
			...
			if (endpoint_info->flags & BCM_OAM_ENDPOINT_RDI_TX && endpoint_info->flags2 &BCM_OAM_ENDPOINT2_RDI_FROM_RX_DISABLE ) {
                    BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
                                        (_BSL_BCM_MSG("When the flag BCM_OAM_ENDPOINT_RDI_TX is set, the flag2 BCM_OAM_ENDPOINT2_RDI_FROM_RX_DISABLE must not be set. ")));
			}
    ...
    if (_BCM_OAM_IS_SERVER_SERVER(endpoint_info) && !is_upmep) {
        ...
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
                            (_BSL_BCM_MSG("Error: tx-gport must be either invalid, LAG or a system port.\n")));
    }
...
STATIC int _bcm_oam_ethernet_endpoint_validity_checks(int unit, const bcm_oam_endpoint_info_t *endpoint_info) {
    ...
    if (endpoint_info->mpls_out_gport != BCM_GPORT_INVALID) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
                            (_BSL_BCM_MSG("Error: mpls_out_gport only applies to BHH endpoints.")));
    }
...
STATIC int _bcm_oam_mpls_pwe_endpoint_validity_checks(int unit, bcm_oam_endpoint_info_t *endpoint_info) {
    ...
    if (SOC_IS_ARADPLUS_AND_BELOW(unit) && endpoint_info->level !=7 && (endpoint_info->flags & BCM_OAM_ENDPOINT_REMOTE) ==0 
        && endpoint_info->type==bcmOAMEndpointTypeBHHMPLS && soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "oam_over_mpls_ignore_mdl", 0)==0) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
                            (_BSL_BCM_MSG("Error: Only level 7 supported for Y.1731 endpoints over MPLS.\n")));
    }
    
    if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "oam_over_mpls_ignore_mdl", 0) && (endpoint_info->opcode_flags & BCM_OAM_OPCODE_CCM_IN_HW) ==0 &&
        endpoint_info->level) {
                BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
                            (_BSL_BCM_MSG("Error: endpoint Level masked, should be 0 for non accelerated endpoints.\n")));
    }


    if (endpoint_info->mpls_out_gport != BCM_GPORT_INVALID ) {
        if (!BCM_GPORT_IS_MPLS_PORT(endpoint_info->mpls_out_gport)  && !BCM_GPORT_IS_TUNNEL(endpoint_info->mpls_out_gport)) {
			BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
						(_BSL_BCM_MSG("Error: mpls_out_gport must be an MPLS port or a tunnel gport\n")));
        }
	}
...
STATIC int _bcm_oam_endpoint_validity_checks_all(int unit, bcm_oam_endpoint_info_t *endpoint_info) {
    ...
    if ((endpoint_info->flags & BCM_OAM_ENDPOINT_REMOTE) == 0 && (endpoint_info->flags & BCM_OAM_ENDPOINT_INTERMEDIATE) == 0) {
        /* Local endpoint*/
        if (SOC_IS_JERICHO(unit) && (endpoint_info->lm_counter_base_id & 0x1)) {
            /* Given value must be even. The counter base id on the passive side is the given value + 1.*/
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
                                (_BSL_BCM_MSG("Error: lm_counter_base_id must be even (two counter sources are needed per endpoint)")));
        }
        if ( endpoint_info->lm_counter_base_id >= max_counter_range   || (endpoint_info->lm_counter_base_id < 0) ) {
            ...
            if (soc_property_get(unit, spn_OAM_DM_NTP_ENABLE, 1) == 0  && endpoint_info->timestamp_format != bcmOAMTimestampFormatIEEE1588v1) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
                                (_BSL_BCM_MSG("Error: NTP disabled, timestamp format must be bcmOAMTimestampFormatIEEE1588v1.")));
        ...
        if ((endpoint_info->flags & BCM_OAM_ENDPOINT_INTERMEDIATE) && (endpoint_info->flags & BCM_OAM_ENDPOINT_REPLACE)) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
        ...
        if (endpoint_info->flags & BCM_OAM_ENDPOINT_REMOTE){
			if ((!(endpoint_info->flags & BCM_OAM_ENDPOINT_REPLACE))
				...
				BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
									(_BSL_BCM_MSG("Only BCM_OAM_ENDPOINT_FAULT_CCM_TIMEOUT is allowed to be set for remote endpoint.")));
			}
		}
...
int bcm_petra_oam_endpoint_create(int unit, bcm_oam_endpoint_info_t *endpoint_info) {
    ...
    sal_memcpy(endpoint_info_lcl, endpoint_info, sizeof(bcm_oam_endpoint_info_t));  /* from here till the end of the function, the function uses the copy of the endpoint */
    ...
    if (BCM_GPORT_IS_TRAP(endpoint_info_lcl->remote_gport))
    {
 	...
 	is_accelerated = ((endpoint_info_lcl->opcode_flags & BCM_OAM_OPCODE_CCM_IN_HW) != 0);
    is_upmep = ((endpoint_info_lcl->flags & BCM_OAM_ENDPOINT_UP_FACING) != 0);
    ...
    if (is_default) {
        ...
        rv = _bcm_oam_default_endpoint_validity_checks(unit, endpoint_info_lcl, &default_id);
        BCMDNX_IF_ERR_EXIT(rv);
        ...
        if (endpoint_info_lcl->type == bcmOAMEndpointTypeEthernet) {
            ...
            BCMDNX_IF_ERR_EXIT(rv);
        } else if (endpoint_info_lcl->type == bcmOAMEndpointTypeBHHMPLS || endpoint_info_lcl->type == bcmOAMEndpointTypeBHHPwe) {
	...
	if (endpoint_info_lcl->flags & BCM_OAM_ENDPOINT_REPLACE) {
		...
		if (endpoint_info_lcl->id == -1 && SOC_IS_ARAD_B1_AND_BELOW(unit)) {
			BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
			 (_BSL_BCM_MSG("Error: Can not replace default mep.\n")));
		}

		bcm_oam_endpoint_info_t_init(&existing_endpoint_info);
		rv = bcm_petra_oam_endpoint_get(unit, endpoint_info_lcl->id, &existing_endpoint_info);
		BCMDNX_IF_ERR_EXIT(rv);
		...
		if (existing_endpoint_info.flags & BCM_OAM_ENDPOINT_REMOTE) {
		    ...
		    if (endpoint_info_lcl->ccm_period==0) {
				...
				soc_sand_rv = soc_ppd_oam_oamp_mep_db_entry_get(unit, endpoint_info_lcl->local_id, &mep_db_entry);
				BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
				SOC_PPD_OAM_CCM_PERIOD_FROM_CCM_INTERVAL_FIELD(ccm_period_ms, ccm_period_micro_s, mep_db_entry.ccm_interval);
                MULTIPLE_BY_3_5(ccm_period_ms, ccm_period_micro_s);
				endpoint_info_lcl->ccm_period = ccm_period_ms;
			}
			...
			rv = _bcm_dpp_oam_bfd_rmep_info_db_delete(unit, endpoint_info_lcl->id);
			BCMDNX_IF_ERR_EXIT(rv);
		    ...
		    soc_sand_rv = soc_ppd_oam_oamp_rmep_set(unit, rmep_id_internal, endpoint_info_lcl->name, 
												endpoint_info_lcl->local_id, endpoint_info_lcl->type, &rmep_db_entry, 1);
		    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
		}
		else {
            ...
            if ( (endpoint_info_lcl->flags & BCM_OAM_ENDPOINT_INTERMEDIATE) != (existing_endpoint_info.flags & BCM_OAM_ENDPOINT_INTERMEDIATE) ) {
			...
			if (((endpoint_info_lcl->opcode_flags & BCM_OAM_OPCODE_CCM_IN_HW)!=0) != ((classifier_mep_entry.flags & SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_FLAG_ACCELERATED)!= 0)) {
            ...
            if (is_accelerated){ 
                ...
                if  ((endpoint_info_lcl->vlan !=existing_endpoint_info.vlan) ||(endpoint_info_lcl->outer_tpid !=existing_endpoint_info.outer_tpid) 
                     ||  (endpoint_info_lcl->pkt_pri !=existing_endpoint_info.pkt_pri) || (endpoint_info_lcl->inner_tpid !=existing_endpoint_info.inner_tpid)  ||
                      endpoint_info_lcl->intf_id !=existing_endpoint_info.intf_id 
                     || (endpoint_info_lcl->inner_pkt_pri !=existing_endpoint_info.inner_pkt_pri) || (endpoint_info_lcl->inner_vlan !=existing_endpoint_info.inner_vlan) )   {
                    BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Error: encapsulation fields can not be updated.\n")));
                }
                if (endpoint_info_lcl->name > SOC_PPD_OAM_MAX_NUMBER_OF_LOCAL_MEPS(unit)) {
            ...
            if (endpoint_info_lcl->gport != BCM_GPORT_INVALID || existing_endpoint_info.gport != BCM_GPORT_INVALID) {
                if (endpoint_info_lcl->type == bcmOAMEndpointTypeEthernet) {
                    rv = _bcm_dpp_gport_compare(unit, endpoint_info_lcl->gport, existing_endpoint_info.gport, &port_is_equal); 
                    BCMDNX_IF_ERR_EXIT(rv);
                } else {
            ...
            if (endpoint_info_lcl->mpls_out_gport != BCM_GPORT_INVALID || existing_endpoint_info.mpls_out_gport != BCM_GPORT_INVALID) {
                /* gport_compare will not suffice for gports returned by MPLS APIs*/
                int lif1, lif2;
                _bcm_dpp_gport_hw_resources gport_hw_resources;

                rv = _bcm_dpp_gport_to_hw_resources(unit, endpoint_info_lcl->mpls_out_gport, _BCM_DPP_GPORT_HW_RESOURCES_GLOBAL_LIF_INGRESS|_BCM_DPP_GPORT_HW_RESOURCES_STRICT_CHECK, &gport_hw_resources);
                BCMDNX_IF_ERR_EXIT(rv);

                lif1 = gport_hw_resources.global_in_lif;

                rv = _bcm_dpp_gport_to_hw_resources(unit, existing_endpoint_info.mpls_out_gport, _BCM_DPP_GPORT_HW_RESOURCES_GLOBAL_LIF_INGRESS|_BCM_DPP_GPORT_HW_RESOURCES_STRICT_CHECK, &gport_hw_resources);
                BCMDNX_IF_ERR_EXIT(rv);
                ...
                port_is_equal = (lif1 == lif2);
                if (!port_is_equal) {
                    BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Error: mpls_out_gport can not be updated.\n")));
                }
            }
			...
			if ((endpoint_info_lcl->level != classifier_mep_entry.md_level) && (!is_default)) {
				BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Error: level can not be updated.\n")));
			}

			if (((endpoint_info_lcl->flags & BCM_OAM_ENDPOINT_UP_FACING)!=0) != ((classifier_mep_entry.flags & SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_FLAG_UPMEP) != 0)) {
			...
			if (is_accelerated) { /* TX - OAMP */
                ...
                if (SOC_IS_ARADPLUS(unit)) {
                    ...
                    if (classifier_mep_entry.flags & SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_FLAG_MEP_HAS_LOOPBACK ) {
                        ...
                        lb.period = period;
                        lb.id = endpoint_info_lcl->id;
                        if (endpoint_info_lcl->type == bcmOAMEndpointTypeEthernet) {
                            rv = _bcm_dpp_am_template_oam_lmm_oui_tables_get(unit, endpoint_info_lcl->id, &oui_profile, &msbs);
                            BCMDNX_IF_ERR_EXIT(rv);
                            rv = _bcm_dpp_am_template_oam_lmm_nic_tables_get(unit, endpoint_info_lcl->id, &nic_profile, &lsbs);
                            BCMDNX_IF_ERR_EXIT(rv);
            ...
            if (endpoint_info_lcl->gport != BCM_GPORT_INVALID) {
                ...
                if (update_counter) {
                    if (endpoint_info_lcl->lm_counter_base_id > 0) {
                        rv = _bcm_dpp_oam_set_counter(unit, endpoint_info_lcl->lm_counter_base_id, ((endpoint_info_lcl->lm_flags & BCM_OAM_LM_PCP)!=0),
                                                       is_upmep?new_classifier_mep_entry.passive_side_lif: new_classifier_mep_entry.lif);
                        BCMDNX_IF_ERR_EXIT(rv);
                    }
                    else if (!SOC_IS_JERICHO(unit)) { /* For Jericho the LIF must be set.*/
                        rv = _bcm_dpp_oam_lif_table_set(unit, is_upmep? new_classifier_mep_entry.passive_side_lif : new_classifier_mep_entry.lif, 0);
                        BCMDNX_IF_ERR_EXIT(rv);
                    }
                    soc_sand_rv = soc_ppd_oam_classifier_oem_mep_add(unit, endpoint_info_lcl->id, &new_classifier_mep_entry, 1/*update*/);
                    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                }

                rv = _bcm_dpp_oam_bfd_mep_info_db_delete(unit, endpoint_info_lcl->id);
                BCMDNX_IF_ERR_EXIT(rv);
                rv = _bcm_dpp_oam_bfd_mep_info_db_insert(unit, endpoint_info_lcl->id, &new_classifier_mep_entry);
                BCMDNX_IF_ERR_EXIT(rv);
            }
            else if (is_default) {
                SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY new_classifier_mep_entry;
                SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_clear(&new_classifier_mep_entry);
                rv = _bcm_oam_classifier_default_mep_entry_struct_set(unit, endpoint_info_lcl, &new_classifier_mep_entry);
                BCMDNX_IF_ERR_EXIT(rv);
                ...
                if (update_counter) {
                    if (endpoint_info_lcl->lm_counter_base_id > 0) {
                        rv = _bcm_dpp_oam_set_counter(unit, endpoint_info_lcl->lm_counter_base_id, ((endpoint_info_lcl->lm_flags & BCM_OAM_LM_PCP)!=0), _BCM_OAM_INVALID_LIF);
                        BCMDNX_IF_ERR_EXIT(rv);
                    }
                    else {
        ...
        if (is_default) {
            ...
            if (endpoint_info_lcl->flags & BCM_OAM_ENDPOINT_REMOTE) { /* Remote endpoint */
                ...
                if ((endpoint_info_lcl->gport == BCM_GPORT_INVALID) && (endpoint_info_lcl->tx_gport == BCM_GPORT_INVALID) &&
                ...
                if ((is_accelerated || _BCM_OAM_IS_SERVER_CLIENT(endpoint_info_lcl)) && (endpoint_info_lcl->flags & BCM_OAM_ENDPOINT_INTERMEDIATE)) {
                ...
                if (is_accelerated) { /* TX - OAMP */
                    ...
                    if (_BCM_OAM_IS_SERVER_SERVER(endpoint_info_lcl) && !is_upmep) {
                        ...
                        _BCM_OAM_BCM_MEP_TYPE_TO_MEP_DB_MEP_TYPE(endpoint_info_lcl->type, mep_type);
                        ...
                        rv = SOC_DPP_WB_ENGINE_SET_ARR(unit, SOC_DPP_WB_ENGINE_VAR_ARAD_PP_OAMP_SERVER_RX_TRAP, &server_trap_ref_counter, BCM_GPORT_TRAP_GET_ID(endpoint_info_lcl->remote_gport));
                        BCMDNX_IF_ERR_EXIT(rv);

                        if (server_trap_ref_counter == 1) {
                            /* This trap code hasen't been allocated yet */
                            soc_sand_rv = soc_ppd_oam_oamp_rx_trap_codes_set(unit, mep_type, BCM_GPORT_TRAP_GET_ID(endpoint_info_lcl->remote_gport));
                            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                        }
                    }
                ...
                if (endpoint_info_lcl->gport == BCM_GPORT_INVALID) {
                    ...
                    if ((endpoint_info_lcl->flags & BCM_OAM_ENDPOINT_UP_FACING) == 0 && BCM_GPORT_IS_TRUNK(endpoint_info_lcl->tx_gport))  {
                        classifier_mep_entry.flags |= SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY_FLAG_SERVER;
                    }
                    ...
                    if (SOC_DPP_CONFIG(unit)->pp.oam_statistics == 1 && (endpoint_info->opcode_flags & BCM_OAM_OPCODE_CCM_IN_HW) && is_accelerated) {
                    ...
                    for (is_passive = 0; is_passive <= 1; is_passive++) {
						...
						/* In Arad Mode Passive profile is fixed and configured separately.
						   In Jericho mode there is no need to configure the Passive profile.*/
                        ...
                        if (!is_passive && found) {
                            BCMDNX_ERR_EXIT_MSG(BCM_E_EXISTS,
                                                (_BSL_BCM_MSG("Error: Local endpoint with lif %d, mdlevel %d and direction %d(1-up,0-down) exists.\n"),
                                                 lif, endpoint_info_lcl->level,((classifier_mep_entry.flags & SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_FLAG_UPMEP) != 0)));
                        }
                            ...
                            if (SOC_PPD_OAM_IS_CLASSIFIER_ADVANCED_MODE(unit)) {
                                ...
                                if (!SOC_PPD_OAM_IS_CLASSIFIER_ADVANCED_MULTIPLE_MEPS_ON_LIF_MODE(unit)) {
                                    ...
                                    if (profile_data_pointer->mp_type_passive_active_mix && ((classifier_mep_entry.flags & SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_FLAG_ENDPOINT) == 0)) {
                                        ...
                                        _BCM_OAM_MEP_EXIST_ON_LIF_AND_DIRECTION(lif,
                                ...
                                if ((classifier_mep_entry.flags & SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_FLAG_ENDPOINT) != 0) {
                                    ...
                                    if (profile == _BCM_OAM_PASSIVE_PROFILE) {
                                        ...
                                        soc_sand_rv = soc_ppd_oam_classifier_find_mep_and_profile_by_lif_and_mdlevel(
                                            unit, lif, classifier_mep_entry.md_level,
                                            ((classifier_mep_entry.flags & SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_FLAG_UPMEP) != 0),
...
exit:
   sal_memcpy(endpoint_info, endpoint_info_lcl, sizeof(bcm_oam_endpoint_info_t));
	...
	if ((SOC_SAND_FAILURE(soc_sand_rv) || (BCM_FAILURE(rv))) && new_id_alloced) { 
		/* If error after new id was alloced we should free it */
		if (endpoint_info_lcl->flags & BCM_OAM_ENDPOINT_REMOTE) {
			rv = bcm_dpp_am_oam_rmep_id_dealloc(unit, endpoint_info_lcl->id);
			rv = _bcm_dpp_oam_bfd_mep_to_rmep_db_rmep_delete(unit, endpoint_info_lcl->local_id, endpoint_info_lcl->id);
			rv = _bcm_dpp_oam_bfd_rmep_info_db_delete(unit, endpoint_info_lcl->id);
		}
		else {
			if (!is_accelerated) { /* TX - OAMP */
				...
				rv = _bcm_dpp_oam_ma_to_mep_db_mep_delete(unit, endpoint_info_lcl->group, endpoint_info_lcl->id);
				rv = _bcm_dpp_oam_bfd_mep_info_db_delete(unit, endpoint_info_lcl->id);
			}
		}
	}
        ...
        /* local endpoint */
            ...
            if ((SOC_PPD_OAM_IS_CLASSIFIER_ADVANCED_MULTIPLE_MEPS_ON_LIF_MODE(unit) && (mep_info.flags & SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_FLAG_ENDPOINT) != 0) || 
				...
				_BCM_OAM_MEP_INDEX_IS_SERVER_CLIENT_SIDE(endpoint)) {
					...
					if (SOC_DPP_CONFIG(unit)->pp.oam_statistics == 1 && (mep_info.flags & SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_FLAG_UPMEP)) {
                ...
                _BCM_OAM_MEP_EXIST_ON_LIF_AND_DIRECTION(mep_info.lif, (mep_info.flags & SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_FLAG_UPMEP)==SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_FLAG_UPMEP
														, 0/*find_mip*/, found, is_mip_dummy, profile_dummy);

                if (!found) { /* Serch meps on the other directions */
                    _BCM_OAM_MEP_EXIST_ON_LIF_AND_DIRECTION(mep_info.passive_side_lif, (mep_info.flags & SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_FLAG_UPMEP)==0
															, 0/*find_mip*/, found, is_mip_dummy, profile_dummy);
                }

                if (!found) { /* No more meps on this lif */
                    rv = _bcm_dpp_oam_lif_table_set(unit,(mep_info.flags & SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_FLAG_UPMEP)?mep_info.passive_side_lif:  mep_info.lif, 0);
                    BCMDNX_IF_ERR_EXIT(rv);
                }
...
int _bcm_dpp_oam_clear(int unit) {
    ...
    {
        if ( i == 0 ) 
            trap_id_sw = _bcm_dpp_oam_trap_info[unit].trap_ids[SOC_PPD_OAM_TRAP_ID_OAMP];
        if ( i == 1 ) 
            trap_id_sw = _bcm_dpp_oam_trap_info[unit].trap_ids[SOC_PPD_OAM_TRAP_ID_OAMP_Y1731_MPLS];
        if ( i == 2 ) 
            trap_id_sw = _bcm_dpp_oam_trap_info[unit].trap_ids[SOC_PPD_OAM_TRAP_ID_OAMP_Y1731_PWE];
	...
	if (!is_bfd_init) {
        ...
        trap_id_sw =  _bcm_dpp_oam_trap_info[unit].trap_ids[SOC_PPD_OAM_TRAP_ID_CPU];
        BCMDNX_IF_ERR_EXIT(_bcm_dpp_rx_get_converted_trap_hw_id(unit,trap_id_sw, &trap_code_converted));
    	rv = bcm_petra_rx_trap_type_destroy(unit, trap_code_converted); 
		BCMDNX_IF_ERR_EXIT(rv);
	}
    ...
    trap_id_sw = _bcm_dpp_oam_trap_info[unit].trap_ids[SOC_PPD_OAM_TRAP_ID_SNOOP];
    BCMDNX_IF_ERR_EXIT(_bcm_dpp_rx_get_converted_trap_hw_id(unit,trap_id_sw, &trap_code_converted));
    rv = bcm_petra_rx_trap_get(unit, trap_code_converted, &trap_config);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    if (soc_property_get(unit, spn_OAM_RCY_PORT, -1)!=-1) {
        trap_id_sw = _bcm_dpp_oam_trap_info[unit].trap_ids[SOC_PPD_OAM_TRAP_ID_RECYCLE];
        BCMDNX_IF_ERR_EXIT(_bcm_dpp_rx_get_converted_trap_hw_id(unit,trap_id_sw, &trap_code_converted));
	    rv = bcm_petra_rx_trap_type_destroy(unit, trap_code_converted);
        BCMDNX_IF_ERR_EXIT(rv);
    }
    ...
    trap_id_sw =  _bcm_dpp_oam_trap_info[unit].upmep_trap_ids[SOC_PPD_OAM_UPMEP_TRAP_ID_RECYCLE]
    BCMDNX_IF_ERR_EXIT(_bcm_dpp_rx_get_converted_trap_hw_id(unit,trap_id_sw, &trap_code_converted));
	rv = bcm_petra_rx_trap_type_destroy(unit,trap_code_converted);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    {
        if ( i == 0 ) 
            trap_id_sw =  _bcm_dpp_oam_trap_info[unit].trap_ids[SOC_PPD_OAM_TRAP_ID_ERR_LEVEL];
        if ( i == 1 ) 
            trap_id_sw = _bcm_dpp_oam_trap_info[unit].trap_ids[SOC_PPD_OAM_TRAP_ID_ERR_PASSIVE];
...
int _bcm_oam_lm_dm_remove_shared(int unit,bcm_oam_endpoint_t endpoint_id , int is_lm, SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY * classifier_mep_entry) {
    ...
    if (old_profile.piggy_back_lm && !is_lm && (classifier_mep_entry->flags & SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_FLAG_UPMEP)==0 && SOC_IS_ARADPLUS_A0(unit) )  {
...
int _remove_loopback_reply(int unit, int endpoint_id, const SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY *classifier_mep_entry ) {
    ...
    if (classifier_mep_entry->flags &SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_FLAG_UPMEP ||(soc_property_get(unit, spn_OAM_RCY_PORT, -1)==-1) ) {
