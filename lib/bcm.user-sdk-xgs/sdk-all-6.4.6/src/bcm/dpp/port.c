...
bcm_petra_port_init(int unit)
{
            ...
            SOC_PBMP_ITER(ports_bm, port) {
                ...
                if (is_master) {
                    ...
                    if (interface != SOC_PORT_IF_ERP) {
                        _bcm_petra_port_nrdy_thresh_allocate(unit, port);
                    }
                }
            }
        }
  ...
  if (!SOC_WARM_BOOT(unit) && !SOC_DPP_PP_ENABLE(unit) && !SOC_IS_ARDON(unit)) 
  {
                                   ...
                                   bcm_port_control_t type, int value)
{
    ...
    BCMDNX_IF_ERR_EXIT(rv);
    prbs_mode = SOC_PBMP_MEMBER(prbs_mac_mode, port) ? portmodPrbsModeMac : portmodPrbsModePhy;
...
_bcm_petra_port_link_up_mac_update(int unit, bcm_port_t port){
    ...
    rv = MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_port_link_up_mac_update, (unit, port));
    BCMDNX_IF_ERR_EXIT(rv);
  ...
  BCM_PBMP_ITER(gport_info.pbmp_local_ports, local_port) {
    ...
    if (SOC_IS_ARAD(unit))
    {
        ...
        BCM_PBMP_ITER(gport_info.pbmp_local_ports, local_port) {  
            ...
            rv = MBCM_DPP_SOC_DRIVER_CALL(unit, mbcm_dpp_port_ability_local_get, (unit, local_port, &phy_ability));
            BCMDNX_IF_ERR_EXIT(rv);
...
int bcm_petra_port_add(int unit, bcm_port_t port, uint32 flags, bcm_port_interface_info_t *interface_info, bcm_port_mapping_info_t *mapping_info)  
{
    ...
    BCMDNX_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_port_nif_nof_lanes_get, (unit,  interface_info->interface, 
                                                     interface_info->phy_port, interface_info->num_lanes, &nof_lanes)));
...
int bcm_petra_port_remove(int unit, bcm_port_t port, uint32 flags) 
{
    ...
    int              base_q_pair, profile, rv, counter_interval, is_channelized, i;
    uint32           tm_port, nof_channels, out_port_priority;
    bcm_gport_t      gport;
    ...
    if (SOC_IS_JERICHO(unit)) {
        if (nof_channels == 1) {
            _bcm_petra_port_nrdy_thresh_free(unit, port);
        }
    }
                           ...
                           bcm_port_control_t type, int value)
{
    ...
    if (type != bcmPortControlOamDefaultProfile && type != bcmPortControlMplsEncapsulateExtendedLabel) {
    ...
    if (SOC_DPP_PP_ENABLE(unit)) {
            ...
            if (port_info.is_n_port == (!value)){
                ...
                if (soc_property_get(unit, spn_FCOE_NPV_BRIDGE_MODE, 0)){
                    ...
                    if (port_info.is_n_port) {
                        arad_egr_prog_editor_profile_get(unit, ARAD_PRGE_TM_SELECT_FCOE_N_PORT, &prge_prog_select);
                        port_info.tunnel_termination_profile = ARAD_PP_PORT_DEFINED_TT_PROFILE_FCOE_N_PORT;
                    }else {
                        arad_egr_prog_editor_profile_get(unit, SOC_TMC_PORT_HEADER_TYPE_NONE, &prge_prog_select);
        ...
        case bcmPortControlMacInMac:
              ...
              if (value == 0 || value == 1) { /* disable/enable MiM */ 
                  ...
                  SOC_PPD_LIF_ID mim_local_lif_ndx; 
        ...
        case bcmPortControlMplsEncapsulateExtendedLabel:
            {
                if (SOC_IS_JERICHO(unit))  {
                    if (soc_property_get(unit, spn_ROO_EXTENSION_LABEL_ENCAPSULATION, 0)) {
                        ...
                        uint32 next_eep[SOC_PPD_NOF_EG_ENCAP_EEP_TYPES], nof_entries = 0;
                        int out_lif_id = 0;
        ...
        case bcmPortControlExtenderType:
            ...
            if (SOC_IS_ARADPLUS(unit)) {
                ...
                case BCM_PORT_EXTENDER_TYPE_SWITCH:
                    ...
                    extender_info.ing_ecid_mode = SOC_PPC_EXTENDER_PORT_ING_ECID_KEEP;
                    ...
                    port_info.tunnel_termination_profile = SOC_PPC_PORT_DEFINED_TT_PROFILE_PORT_EXTENDER;
                            ...
                            bcm_port_control_t type, int *value)
{
    ...
    if (type != bcmPortControlMplsEncapsulateExtendedLabel) {
        rv = _bcm_dpp_gport_to_phy_port(unit, port, operations, &gport_info); 
        BCMDNX_IF_ERR_EXIT(rv);
    }
    ...
    if (is_ingress) {
        ...
        if (SOC_IS_JERICHO(unit)) {
            ...
            soc_sand_rv = soc_ppd_eg_encap_entry_get(
                unit,
                SOC_PPD_EG_ENCAP_EEP_TYPE_TUNNEL_EEP, gport_hw_resources.local_out_lif, 1,
                encap_entry_info, next_eep, &nof_entries);
            ...
            if (class_id == 0) {
                soc_sand_rv = soc_ppd_eg_filter_split_horizon_out_lif_orientation_set(unit, gport_hw_resources.local_out_lif, SOC_SAND_PP_HUB_SPOKE_ORIENTATION_SPOKE);
            } else {
                soc_sand_rv = soc_ppd_eg_filter_split_horizon_out_lif_orientation_set(unit, gport_hw_resources.local_out_lif, SOC_SAND_PP_HUB_SPOKE_ORIENTATION_HUB);
            }
    ...
    uint32 flags)
{
   ...
   if (learn_enable && (learn_info_ptr != NULL)) {
       learn_gport = _bcm_dpp_in_lif_match_learn_gport_get(unit, gport_hw_resources.local_in_lif);
   ...
   BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_i) {
      ...
      */
      ...
      if (!_BCM_DPP_TPID_PARSE_ADVANCED_MODE(unit)) {
         ...
         /* get number of tpids */
          tmp = 0;
          rv = PORT_ACCESS.tpids_count.get(unit, soc_ppd_port_i, &tmp);
          BCMDNX_IF_ERR_EXIT(rv);
      ...
      rv = PORT_ACCESS.dpp_port_config.bcm_port_tpid_info.tpid.get(unit, 0, port_info.tpid_profile, &old_tpids[0]);
      BCMDNX_IF_ERR_EXIT(rv);
      rv = PORT_ACCESS.dpp_port_config.bcm_port_tpid_info.tpid.get(unit, 1, port_info.tpid_profile, &old_tpids[1]);
      BCMDNX_IF_ERR_EXIT(rv);
      ...
      * get accept mode for this port
      ...
      if (!_BCM_DPP_TPID_PARSE_ADVANCED_MODE(unit)) {
          ...
          rv = PORT_ACCESS.tpids_count.set(unit, soc_ppd_port_i, tmp);
          BCMDNX_IF_ERR_EXIT(rv);
      }
...
{   
    ...
    if (lif_tpid_type != _bcm_petra_tpid_profile_none) { /* PWE/I-SID use an assigned tpid profile */
        ...
        rv = PORT_ACCESS.dpp_port_config.bcm_port_tpid_info.tpid.get(unit, 0, tpid_profile, &old_tpids[0]);
        BCMDNX_IF_ERR_EXIT(rv);
        rv = PORT_ACCESS.dpp_port_config.bcm_port_tpid_info.tpid.get(unit, 1, tpid_profile, &old_tpids[1]);
        BCMDNX_IF_ERR_EXIT(rv);
    } else { 
    ...
    )
{
   ...
   */
   ...
   if (gport_parse_info.type == _bcmDppGportParseTypeSimple) { /* Phy port */
       ...
       /* pwe/I-SID update */
       rv = _bcm_petra_tpid_profile_update(unit, gport_hw_resources.local_in_lif, tpid, oper);
       BCMDNX_IF_ERR_EXIT(rv);                
      } 
    ...
    bcm_port_encap_config_t *encap_config)
{
...
#ifdef BCM_ARAD_SUPPORT   
   {
       ...
       if(BCM_GPORT_IS_TUNNEL(gport)){
           ...
           rv = soc_ppd_eg_encap_entry_get(unit, SOC_PPD_EG_ENCAP_EEP_TYPE_TUNNEL_EEP, gport_hw_resources.local_out_lif, 1, encap_entry_info, next_eep, &nof_entries);
    ...
    bcm_port_match_info_t *match)
{
    ...
    rv = _bcm_dpp_lif_usage_get(unit, gport_hw_resources.local_in_lif, gport_hw_resources.local_out_lif, &in_lif_usage, &out_lif_usage);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    if (gport_hw_resources.local_in_lif != _BCM_GPORT_ENCAP_ID_LIF_INVALID) {
        rv = _bcm_petra_port_match_ingress_add(unit, port, gport_hw_resources.local_in_lif, match);
        BCMDNX_IF_ERR_EXIT(rv);
    }

    if (gport_hw_resources.local_out_lif != _BCM_GPORT_ENCAP_ID_LIF_INVALID) {
        rv = _bcm_petra_port_match_egress_add(unit, port, gport_hw_resources.local_out_lif, match);
        BCMDNX_IF_ERR_EXIT(rv);
    }    
    ...
    uint64 data)
{
    ...
    rv = (MBCM_PP_DRIVER_CALL(unit,mbcm_pp_lif_additional_data_set,(unit,lif_id, is_ingress, data)));
    BCMDNX_IF_ERR_EXIT(rv); 
