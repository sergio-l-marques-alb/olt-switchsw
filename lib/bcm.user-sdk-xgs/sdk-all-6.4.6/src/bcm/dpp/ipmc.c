  ...
  )
{
    ...
    if (found == SOC_SAND_FALSE) {
        /* Entry not found */
        if (data->flags & BCM_IPMC_REPLACE) {/*Only print message if we intended to find entry*/
  ...
  )
{
    ...
    SOC_PPD_FRWRD_MACT_ENTRY_VALUE
        route_info;
    SOC_PPC_FRWRD_IPV4_MC_ROUTE_KEY tcam_route_key;
    SOC_PPC_FRWRD_IPV4_MC_ROUTE_INFO tcam_route_info;
    SOC_SAND_SUCCESS_FAILURE
        failure_indication = SOC_SAND_SUCCESS;
    bcm_ipmc_addr_t data_find;
    uint8 is_ipmc_ssm_enabled;

    BCMDNX_INIT_FUNC_DEFS;
    unit = (unit);
    is_ipmc_ssm_enabled = SOC_DPP_CONFIG(unit)->pp.ipmc_l2_ssm_mode;

    SOC_PPD_FRWRD_MACT_ENTRY_KEY_clear(&route_key);
    SOC_PPD_FRWRD_MACT_ENTRY_VALUE_clear(&route_info);
    SOC_PPD_FRWRD_IPV4_MC_ROUTE_KEY_clear(&tcam_route_key);
    SOC_PPD_FRWRD_IPV4_MC_ROUTE_INFO_clear(&tcam_route_info);
    ...
    if (data->s_ip_addr == 0) {
        ...
        if (is_ipmc_ssm_enabled) {
            /* build TCAM routing key */
            tcam_route_key.fid = data->vid;
            tcam_route_key.group = data->mc_ip_addr;
            tcam_route_key.source.ip_address = data->s_ip_addr;
            tcam_route_key.source.prefix_len = bcm_ip_mask_length(data->s_ip_mask);

            tcam_route_info.dest_id.dest_type    = SOC_SAND_PP_DEST_MULTICAST;
            tcam_route_info.dest_id.dest_val     = _BCM_MULTICAST_ID_GET(data->group);

            soc_sand_rv = (MBCM_PP_DRIVER_CALL(unit,mbcm_pp_ipmc_ssm_tcam_entry_add,(unit, &tcam_route_key, &tcam_route_info, &failure_indication)));
            SOC_SAND_IF_FAIL_RETURN(failure_indication);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        }
  ...
  )
{
    ...
    if (data->s_ip_addr == 0) {
        ...
        if (is_ipmc_ssm_enabled) {
            ...
            soc_sand_rv = (MBCM_PP_DRIVER_CALL(unit,mbcm_pp_ipmc_ssm_tcam_entry_get,(unit, &tcam_route_key, &tcam_route_info, &route_status, &found)));
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        }
        else { /* IPMC SSM disabled */
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("when add IPMC entry as bridged SIP has to be seto to zero")));
    ...
    else if (tcam_route_info.dest_id.dest_type == SOC_SAND_PP_DEST_MULTICAST) {
        _BCM_MULTICAST_GROUP_SET(data->group,_BCM_MULTICAST_TYPE_L3,tcam_route_info.dest_id.dest_val);
    }
    else{
  ...
  )
{
    ...
    if (data->s_ip_addr == 0) {
        ...
        if (is_ipmc_ssm_enabled) {
            ...
            soc_sand_rv = (MBCM_PP_DRIVER_CALL(unit,mbcm_pp_ipmc_ssm_tcam_entry_delete,(unit, &tcam_route_key)));
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        }
        else { /* IPMC SSM disabled */
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("when add IPMC entry as bridged SIP has to be seto to zero")));
