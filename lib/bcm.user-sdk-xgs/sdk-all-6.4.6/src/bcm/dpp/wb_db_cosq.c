...
_bcm_dpp_wb_cosq_layout_init(int unit, int version)
{
    ...
    switch (version) {
        case BCM_DPP_WB_COSQ_VERSION_1_0:
            ...
            wb_info->max_voqs = BCM_DPP_WB_COSQ_MAX_VOQ_GROUPS(unit) * SOC_DPP_CONFIG(unit)->core_mode.nof_active_cores;
            wb_info->voq_off = 0;
            ...
            BCM_DPP_CORES_ITER(BCM_CORE_ALL, core) {
                ...
                entry_size = (entry_size + 4) & ~(0x3); /* next offset at a word boundary */
                wb_info->max_conns[core] = BCM_DPP_WB_COSQ_MAX_CONN_GROUPS(unit);
                wb_info->conn_off[core] = total_size;
                total_size += entry_size;

                entry_size = sizeof(bcm_dpp_wb_cosq_se_config_t) * BCM_DPP_WB_COSQ_MAX_SE(unit);
                entry_size = (entry_size + 4) & ~(0x3); /* next offset at a word boundary */
                wb_info->max_ses[core] = BCM_DPP_WB_COSQ_MAX_SE(unit);
                wb_info->se_off[core] = total_size;
                total_size += entry_size;

                entry_size = sizeof(bcm_dpp_wb_cosq_flow_config_t) * BCM_DPP_WB_COSQ_MAX_FLOW(unit);
                entry_size = (entry_size + 4) & ~(0x3); /* next offset at a word boundary */
                wb_info->max_flows[core] = BCM_DPP_WB_COSQ_MAX_FLOW(unit);
                wb_info->flow_off[core] = total_size;
                total_size += entry_size;

                /* E2E Flow Control Templates */
                entry_size = sizeof(bcm_dpp_wb_cosq_e2e_fc_data_t) * SOC_TMC_SCH_LOW_FC_NOF_AVAIL_CONFS;
                entry_size = (entry_size + 4) & ~(0x3); /* next offset at a word boundary */
                wb_info->max_e2e_fc_data[core] = SOC_TMC_SCH_LOW_FC_NOF_AVAIL_CONFS;
                wb_info->e2e_fc_data_off[core] = total_size;
                total_size += entry_size;

                /* CL Class Templates*/
                entry_size = sizeof(bcm_dpp_wb_cosq_class_data_t) * SOC_TMC_SCH_NOF_CLASS_TYPES;
                entry_size = (entry_size + 4) & ~(0x3); /* next offset at a word boundary */
                wb_info->max_class_data[core] = SOC_TMC_SCH_NOF_CLASS_TYPES;
                wb_info->class_data_off[core] = total_size;
                total_size += entry_size;
            }
            ...
            BCM_DPP_CORES_ITER(BCM_CORE_ALL, core) {
                ...
                entry_size = (entry_size + 4) & ~(0x3); /* next offset at a word boundary */
                wb_info->gbl_data_off[core] = total_size;
                total_size += entry_size;
            }
            /* IPF data*/
...
_bcm_dpp_wb_cosq_restore_voq_state(int unit)
{
    ...
    switch(wb_info->version) {
        case BCM_DPP_WB_COSQ_VERSION_1_0:
            ...
            for (nbr_entry = 0, element_index=0; nbr_entry < wb_info->max_voqs;
                           nbr_entry++, voq_state++, element_index += DPP_DEVICE_COSQ_COS_ALLOC_SZ) {

                /* check if voq group exists */
                if (voq_state->num_cos == 0) {
                    continue;
                }
                core = DPP_DEVICE_PETRA_VOQ_HD_CORE(unit, element_index);
                base_qid = DPP_DEVICE_PETRA_VOQ_HD_ID(unit, core, element_index);
                if (base_qid >= SOC_DPP_DEFS_GET(unit, max_queues)) {
                    continue;
                }
                /* update s/w state of cosq module */
                voq_config = (bcm_dpp_cosq_voq_config_t *)_bcm_petra_cosq_element_alloc(unit, &(cosq_config->voq_hd), element_index);
                if (voq_config == NULL) {
                ...
                rc = _bcm_petra_cosq_qid_type_get(unit, core, base_qid, &voq_gport,
                ...
                if (is_reserved == FALSE) {
                    rc = bcm_dpp_am_ingress_voq_allocate(unit, core, SHR_RES_ALLOC_WITH_ID,
...
_bcm_dpp_wb_cosq_restore_conn_state(int unit)
{
    ...
    int                                    base_cid, is_non_contiguous, is_composite, conn_id;
    int                                    actual_base_cid;
    int                                    is_reserved;
    ...
    int                                    num_cos_levels;
    bcm_dpp_cosq_config_t                 *cosq_config;
    uint32                                 flags, nof_remote_cores;
    uint8                                 src_modid = 0;
    bcm_dpp_wb_cosq_flow_config_t         *flow_state;
    bcm_dpp_cosq_flow_config_t            *flow_config;
    int                                   flow_region = 0, is_odd_even = 0;
    uint8                                 is_non_contiguous_uint8 = 0;
    int                                   core ;
    ...
    switch(wb_info->version) {
        case BCM_DPP_WB_COSQ_VERSION_1_0:
            ...
            BCM_DPP_CORES_ITER(BCM_CORE_ALL, core) {
                base_cid = 0 ;
                conn_state = (bcm_dpp_wb_cosq_connector_config_t *)(wb_info->scache_ptr + wb_info->conn_off[core]);
                flow_state = (bcm_dpp_wb_cosq_flow_config_t *)(wb_info->scache_ptr + wb_info->flow_off[core]);

                /* read all connector entries */
                for (nbr_entry = 0; nbr_entry < wb_info->max_conns[core];
                    ...
                    if (base_cid % 1024 == 0) {
                        ...
                        rc = _bcm_petra_flow_region_config_get(unit, core, flow_region, &is_non_contiguous_uint8, &is_odd_even);
                        BCMDNX_IF_ERR_EXIT(rc);
                    }
                    ...
                    /* update s/w state of cosq module */
                    conn_config = (bcm_dpp_cosq_connector_config_t *)_bcm_petra_cosq_element_alloc(unit, &(cosq_config->connector_hd[core]), actual_base_cid);
                    if (conn_config == NULL) {
                    ...
                    conn_config->flags = conn_state->flags ; 
                    ...
                    BCM_COSQ_GPORT_VOQ_CONNECTOR_CORE_SET(conn_gport, actual_base_cid,core);
                    ...
                    if (is_reserved == FALSE) {
                        /* update s/w state of resource manager */
                        nof_remote_cores = SOC_DPP_CONFIG(unit)->arad->region_nof_remote_cores[core][flow_region];
                        rc = bcm_dpp_am_cosq_scheduler_allocate(unit, core, nof_remote_cores, SHR_RES_ALLOC_WITH_ID,
                    ...
                    for (num_cos = 0; num_cos < conn_config->num_cos; num_cos++) {
                        ...
                        rc = _bcm_petra_flow_id_get(unit, core, actual_base_cid, num_cos, is_non_contiguous, is_composite, FALSE,&conn_id);
                        BCMDNX_IF_ERR_EXIT(rc);
    
                        flow_config = (bcm_dpp_cosq_flow_config_t *)_bcm_petra_cosq_element_alloc(unit, &(cosq_config->flow_hd[core]), conn_id);
                        if (flow_config == NULL) {
                            BCM_EXIT;
                        }
                        ...
                        if (is_composite) {
                            rc = _bcm_petra_composite_flow_id_get(unit, core, actual_base_cid, num_cos, is_non_contiguous, is_composite, &conn_id);
                            BCMDNX_IF_ERR_EXIT(rc);
...
_bcm_dpp_wb_cosq_restore_se_state(int unit)
{
    ...
    uint8                                 *se_info[SOC_DPP_DEFS_MAX(NOF_CORES)] ;
    int                                    num_cos_levels;
    uint32                                 flags;
    int                                    core ;
    ...
    BCM_DPP_CORES_ITER(BCM_CORE_ALL, core) {
        BCMDNX_ALLOC(se_info[core], sizeof(uint8) * ((wb_info->max_ses[core] / 8) + 1), "temp se info");
        if (se_info[core] == NULL) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_MEMORY, (_BSL_BCM_MSG("Unable to allocate temp memory")));
        }
        sal_memset(se_info[core], 0, sizeof(uint8) * ((wb_info->max_ses[core] / 8) + 1));
    }
    switch(wb_info->version) {
        case BCM_DPP_WB_COSQ_VERSION_1_0:

            BCM_DPP_CORES_ITER(BCM_CORE_ALL, core) {
                se_state = (bcm_dpp_wb_cosq_se_config_t *)(wb_info->scache_ptr + wb_info->se_off[core]);
                flow_state = (bcm_dpp_wb_cosq_flow_config_t *)(wb_info->scache_ptr + wb_info->flow_off[core]);

                /* read all se entries */
                for (se_id = 0; se_id < wb_info->max_ses[core]; se_id++) {
                    /* check that the element is not already processed */
                    if (se_info[core][(se_id / 8)] & (1 << (se_id % 8))) {
                        continue;
                    }
                    ...
                    if (flags & DPP_DEVICE_COSQ_CL_MASK) {
                        if (flags & DPP_DEVICE_COSQ_HR_MASK) { /* determines dual {CL, HR} mode */
                            rc = bcm_dpp_am_cosq_scheduler_allocate(unit, core, 1, SHR_RES_ALLOC_WITH_ID,
                            ...
                            rc = bcm_dpp_am_cosq_scheduler_allocate(unit, core, SOC_TMC_COSQ_ANY_NOF_REMOTE_CORES, SHR_RES_ALLOC_WITH_ID,
                    ...
                    se_config = (bcm_dpp_cosq_se_config_t *)_bcm_petra_cosq_element_alloc(unit, &(cosq_config->se_hd[core]), se_id);
                    if (se_config == NULL) {
                        goto err;
                    }
                    ...
                    se_info[core][(se_id / 8)] |= (1 << (se_id % 8));
    
                    flow_config = (bcm_dpp_cosq_flow_config_t *)_bcm_petra_cosq_element_alloc(unit, &(cosq_config->flow_hd[core]), flow_id);
                    if (flow_config == NULL) {
                        goto err;
                    }
                    ...
                    if (is_dual) {
                        se_config = (bcm_dpp_cosq_se_config_t *)_bcm_petra_cosq_element_alloc(unit, &(cosq_config->se_hd[core]), dual_se_id);
                        if (se_config == NULL) {
                            goto err;
                        }
                        ...
                        /* update temp state to indicate element processed */
                        se_info[core][(dual_se_id / 8)] |= (1 << (dual_se_id % 8));
                    }
    
                    if ( (is_composite) || (is_dual) ) {
                        flow_config = (bcm_dpp_cosq_flow_config_t *)_bcm_petra_cosq_element_alloc(unit, &(cosq_config->flow_hd[core]), flow_id2);
                        if (flow_config == NULL) {
                            goto err;
                        }
    ...
    if (!SOC_DPP_CONFIG(unit)->tm.is_port_tc_enable) {
        ...
        for (fap_port = 0; fap_port <= _BCM_PETRA_NOF_TM_PORTS(unit); fap_port++) {
            BCM_DPP_CORES_ITER(BCM_CORE_ALL, core) {
                ...
                rc = _bcm_dpp_am_template_cosq_port_hr_fc_profile_mapping_set(unit, core, fap_port, port_info.lowest_hp_class);
                BCMDNX_IF_ERR_EXIT(rc);
            } 
        }
    }
...
_bcm_dpp_wb_cosq_restore_class_data(int unit)
{
    ...
    BCM_DPP_CORES_ITER(BCM_CORE_ALL, core) {
        ...
        soc_sand_rc = (MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_sch_class_type_params_table_get,(unit, core, cl_class_table)));
        if(SOC_SAND_FAILURE(soc_sand_rc)) {
            LOG_ERROR(BSL_LS_BCM_INIT,
                      (BSL_META_U(unit,
        ...
        for (template = 0; template < wb_info->max_class_data[core]; template++) {
            switch(wb_info->version) {
                case BCM_DPP_WB_COSQ_VERSION_1_0:
                    class_state = (bcm_dpp_wb_cosq_class_data_t *)(wb_info->scache_ptr + wb_info->class_data_off[core]);
            ...
            rc = _bcm_dpp_am_template_cosq_sched_class_allocate_group(unit, core, flags, (int *)&class_data, ref_count, &is_allocated, &template);
            BCMDNX_IF_ERR_EXIT(rc);
...
_bcm_dpp_wb_cosq_restore_ingr_rate_class_data(int unit)
{
    ...
    if (SOC_IS_ARAD(unit) && (queue_to_rate_class_mapping_is_simple == ARAD_SW_DB_QUEUE_TO_RATE_CLASS_MAPPING_IS_SIMPLE)) /*template mngr is relevent only for simples queue to rate cls mapping mode*/
#endif /*BCM_ARAD_SUPPORT*/
    {
/*
    #ifdef BCM_ARAD_SUPPORT
            if (SOC_IS_ARAD(unit)) {
                rc = _bcm_dpp_am_template_queue_rate_cls_create_pool(unit);
                BCMDNX_IF_ERR_EXIT(rc);
            }
    #endif BCM_ARAD_SUPPORT 
*/ 
        for (template = 0; template < wb_info->max_ingr_rate_class_data; template++) {
            switch(wb_info->version) {
                case BCM_DPP_WB_COSQ_VERSION_1_0:
                    ...
                    /*        maintained in state table and persistent storage.                  */
                    /*        Following is a quick fix. The long term solution is to retrieve    */
                    /*        as much state from h/w.                                            */
        ...
        for (vsq_group = 0; vsq_group < SOC_TMC_NOF_VSQ_GROUPS; vsq_group++) {
            switch(wb_info->version) {
                case BCM_DPP_WB_COSQ_VERSION_1_0:
                    ...
                    ref_count = (vsq_rate_state + template)->ref_cnt[vsq_group];
                    if (ref_count == 0) {
                        continue;
                    }
                    ...
                    /*        Following is a quick fix. The long term solution is to retrieve    */
                    /*        as much state from h/w.                                            */
                    vsq_rate_class_state = (bcm_dpp_cosq_vsq_rate_class_info_t *) &((vsq_rate_state + template)->data);
            ...
            /* restore state  */
            rc = _bcm_dpp_am_template_vsq_rate_cls_allocate_group(unit, flags, vsq_group, &vsq_rate_class_info, ref_count, &is_allocated, &template);
            BCMDNX_IF_ERR_EXIT(rc);
        }
    }
...
_bcm_dpp_wb_cosq_restore_gbl_state(int unit)
{
    ...
    BCM_DPP_ASSYMETRIC_CORES_ITER(BCM_CORE_ALL, core) {
        switch(wb_info->version) {
            case BCM_DPP_WB_COSQ_VERSION_1_0:
                gbl_state = (bcm_dpp_wb_cosq_gbl_data_t *)(wb_info->scache_ptr + wb_info->gbl_data_off[core]);

                vsq_category = gbl_state->config.vsq_category_mode;
                ucast_qid_start = gbl_state->config.ucast_qid_start;
                ucast_qid_end = gbl_state->config.ucast_qid_end;
        ...
        rc = bcm_petra_cosq_fmq_vsq_category_mode_set(unit, core, (bcm_fabric_vsq_category_mode_t)vsq_category);
        BCMDNX_IF_ERR_EXIT(rc);

        rc = bcm_petra_fabric_multicast_scheduler_mode_get(unit, core, &is_run_allocation);
        BCMDNX_IF_ERR_EXIT(rc);
        queue_range.start = ucast_qid_start;
        queue_range.end = ucast_qid_end;
        if (!is_run_allocation && ucast_qid_start < DPP_DEVICE_COSQ_FMQ_NON_ENHANCED_QID_MAX) {
            queue_range.start = DPP_DEVICE_COSQ_FMQ_NON_ENHANCED_QID_MAX + 1;
        }
        rc = _bcm_petra_cosq_queue_range_set(unit, core, bcmCosqRangeFabricQueue, BCM_PETRA_COSQ_QUEUE_RANGE_SET_BOTH, TRUE, &queue_range);
        BCMDNX_IF_ERR_EXIT(rc);
        /*
         * set resource manager state
         */
        if (is_run_allocation) {
            sand_rc = (MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_mult_fabric_enhanced_get,(unit, core, &mcast_queue_range)));
        ...
        rc = _bcm_petra_cosq_queue_range_set(unit, core, bcmCosqRangeMulticastQueue, BCM_PETRA_COSQ_QUEUE_RANGE_SET_BOTH, TRUE, &mcast_queue_range);
        BCMDNX_IF_ERR_EXIT(rc);

        sand_rc = (MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_itm_ingress_shape_get,(unit, core, &isp_info)));
        if (SOC_SAND_FAILURE(sand_rc)) {
            rc = translate_sand_success_failure(sand_rc);
            BCMDNX_IF_ERR_EXIT(rc);
        }
        ...
        if (isp_info.enable) {
            ...
            rc = _bcm_petra_cosq_queue_range_set(unit, core, bcmCosqRangeShaperQueue, BCM_PETRA_COSQ_QUEUE_RANGE_SET_BOTH, TRUE, &queue_range);
            BCMDNX_IF_ERR_EXIT(rc);
        }
        BCMDNX_IF_ERR_EXIT(rc);
    }
            ...
            BCM_DPP_CORES_ITER(BCM_CORE_ALL, core) {
                ...
                for (nbr_entry = 0; nbr_entry < wb_info->max_conns[core] ;
                    ...
                    conn_config = (bcm_dpp_cosq_connector_config_t *)_bcm_petra_cosq_element_get(unit,
                                                          &(cosq_config->connector_hd[core]), actual_base_cid);
                    if (conn_config == NULL) {
                ...
                conn_state->flags = 0;
                    ...
                    conn_state->flags = conn_config->flags;
#ifdef BCM_DPP_COSQ_CONN_NO_MODID_OPTIMIZATION
            ...
            BCM_DPP_CORES_ITER(BCM_CORE_ALL, core) {
                ...
                for (se_id = 0; se_id < wb_info->max_ses[core]; se_id++) {
                    ...
                    flow_config = (bcm_dpp_cosq_flow_config_t *)_bcm_petra_cosq_element_get(unit,
                                                                &(cosq_config->flow_hd[core]), flow_id);
                    if (flow_config == NULL) {
                        goto err;
                    }
                    ...
                    if (is_dual) {
                        se_config = (bcm_dpp_cosq_se_config_t *)_bcm_petra_cosq_element_get(unit,
                                                                &(cosq_config->se_hd[core]), dual_se_id);
                        if (se_config == NULL) {
                            goto err;
                    }
                ...
                if ( (is_dual) || (is_composite) ) {
                    flow_config = (bcm_dpp_cosq_flow_config_t *)_bcm_petra_cosq_element_get(unit,
                                                            &(cosq_config->flow_hd[core]), flow_id2);
                    if (flow_config == NULL) {
                        goto err;
                    }
...
_bcm_dpp_wb_cosq_save_e2e_fc_data(int unit)
{
    ...
    BCM_DPP_CORES_ITER(BCM_CORE_ALL, core) {
        /* read all se entries */
        for (nbr_templates = 0; nbr_templates < wb_info->max_e2e_fc_data[core]; nbr_templates++) {
            switch(wb_info->version) {
                case BCM_DPP_WB_COSQ_VERSION_1_0:
                    ref_count = 0;
                    rc = _bcm_dpp_am_template_cosq_hr_fc_ref_get(unit, core, nbr_templates, &ref_count);
                    if (rc != BCM_E_NONE) {
                        ref_count = 0;
                        rc = BCM_E_NONE;
                    }

                    e2e_fc_state = (bcm_dpp_wb_cosq_e2e_fc_data_t *)(wb_info->scache_ptr + wb_info->e2e_fc_data_off[core]);
...
_bcm_dpp_wb_cosq_save_class_data(int unit)
{
    ...
    wb_info = BCM_DPP_WB_COSQ_INFO(unit);
    BCM_DPP_CORES_ITER(BCM_CORE_ALL, core) {
        for (nbr_templates = 0; nbr_templates < wb_info->max_class_data[core] ; nbr_templates++) {
            switch(wb_info->version) {
                case BCM_DPP_WB_COSQ_VERSION_1_0:
                    ref_count = 0;
                    rc = _bcm_dpp_am_template_cosq_sched_class_ref_get(unit, core, nbr_templates, &ref_count);
                    if (rc != BCM_E_NONE) {
                        ref_count = 0;
                        rc = BCM_E_NONE;
                    }
...
#ifdef BCM_ARAD_SUPPORT
    BCMDNX_IF_ERR_EXIT(sw_state_access[unit].dpp.soc.arad.tm.tm_info.queue_to_rate_class_mapping.is_simple_mode.get(unit, &queue_to_rate_class_mapping_is_simple));
    if (SOC_IS_ARAD(unit) && (queue_to_rate_class_mapping_is_simple == ARAD_SW_DB_QUEUE_TO_RATE_CLASS_MAPPING_IS_SIMPLE)) /*template mngr is relevant only for simples queue to rate cls mapping mode*/
#endif /*BCM_ARAD_SUPPORT*/
    {
    ...
    for (nbr_templates = 0; nbr_templates < wb_info->max_vsq_rate_class_data; nbr_templates++) {
        for (vsq_group = 0; vsq_group < SOC_TMC_NOF_VSQ_GROUPS; vsq_group++) {
            switch(wb_info->version) {
                case BCM_DPP_WB_COSQ_VERSION_1_0:
                    ref_count = 0;
                    rc = _bcm_dpp_am_template_vsq_rate_cls_ref_get(unit, nbr_templates, vsq_group, &ref_count);
                    if (rc != BCM_E_NONE) {
                        ref_count = 0;
                        rc = BCM_E_NONE;
                    }
                    ...
                    vsq_rate_state = (bcm_dpp_wb_cosq_vsq_rate_class_data_t *)(wb_info->scache_ptr + wb_info->vsq_rate_class_data_off);
                    (vsq_rate_state + nbr_templates)->ref_cnt[vsq_group] = ref_count;
                    ...
                    if (ref_count != 0) {
                        ...
                        /*        Following is a quick fix. THe long term solution is to store only  */
                        /*        the state that cannot be easily re-created from h/w.               */
                        vsq_rate_class_state = (bcm_dpp_cosq_vsq_rate_class_info_t *) &((vsq_rate_state + nbr_templates)->data);
...
_bcm_dpp_wb_cosq_save_gbl_state(int unit)
{
    ...
    BCM_DPP_ASSYMETRIC_CORES_ITER(BCM_CORE_ALL, core) {
        gbl_state = (bcm_dpp_wb_cosq_gbl_data_t *)(wb_info->scache_ptr + wb_info->gbl_data_off[core]);
        rc = _bcm_petra_cosq_ucast_qid_range_get(unit, core, &(gbl_state->config.ucast_qid_start), &(gbl_state->config.ucast_qid_end));
        BCMDNX_IF_ERR_EXIT(rc);

        rc = bcm_petra_cosq_fmq_vsq_category_mode_get(unit, core, (bcm_fabric_vsq_category_mode_t *)&(gbl_state->config.vsq_category_mode));
        BCMDNX_IF_ERR_EXIT(rc);
    }
exit:
    BCMDNX_FUNC_RETURN;
...
int
_bcm_dpp_wb_cosq_update_voq_state(int unit, int core, int base_qid)
{
    int                              rc = BCM_E_NONE;
    bcm_dpp_wb_cosq_info_t          *wb_info; 
    bcm_dpp_wb_cosq_voq_config_t    *voq_state;
    ...
    element_index = DPP_DEVICE_PETRA_VOQ_HD_ELEMENT(unit, core, base_qid);
    ...
    switch(wb_info->version) {
        case BCM_DPP_WB_COSQ_VERSION_1_0:
            ...
            voq_config = (bcm_dpp_cosq_voq_config_t *)_bcm_petra_cosq_element_get(unit,
                                         &(cosq_config->voq_hd), element_index);
            voq_state = (bcm_dpp_wb_cosq_voq_config_t *)(wb_info->scache_ptr + wb_info->voq_off);
            voq_state += (element_index / DPP_DEVICE_COSQ_COS_ALLOC_SZ);
 ...
 */
int
_bcm_dpp_wb_cosq_update_conn_all_state(int unit, int core, int base_conn, int is_non_contiguous, int is_composite, int src_modid)
{
    int                                  rc = BCM_E_NONE;
    bcm_dpp_wb_cosq_info_t              *wb_info; 
    bcm_dpp_cosq_config_t               *cosq_config;
    ...
    switch(wb_info->version) {
        case BCM_DPP_WB_COSQ_VERSION_1_0:
            /* update framework cache and peristent storage                       */
            conn_config = (bcm_dpp_cosq_connector_config_t *)_bcm_petra_cosq_element_get(unit,
                                         &(cosq_config->connector_hd[core]), base_conn);
            conn_state = (bcm_dpp_wb_cosq_connector_config_t *)(wb_info->scache_ptr + wb_info->conn_off[core]);
        ...
        conn_state->flags = conn_config->flags ;
            ...
            for (num_cos = 0; num_cos < conn_state->num_cos; num_cos++) {
                rc = _bcm_petra_flow_id_get(unit, core, base_conn, num_cos, is_non_contiguous, is_composite, FALSE,&conn_id);
                if (rc != BCM_E_NONE) {
                    goto err;
                }
                ...
                if (is_composite) {
                    rc = _bcm_petra_composite_flow_id_get(unit, core, base_conn, num_cos, is_non_contiguous, is_composite, &conn_id);
                    if (rc != BCM_E_NONE) {
                        goto err;
                    }
...
int
_bcm_dpp_wb_cosq_update_flow_state(int unit, int core, int flow_id)
{
    int                                  rc = BCM_E_NONE;
    bcm_dpp_wb_cosq_info_t              *wb_info; 
    bcm_dpp_cosq_config_t               *cosq_config;
...
int
_bcm_dpp_wb_cosq_update_se_all_state(int unit, int core, int se_id, int is_composite, int is_dual)
{   
    int                                  rc = BCM_E_NONE;
    bcm_dpp_wb_cosq_info_t              *wb_info; 
    bcm_dpp_cosq_config_t               *cosq_config; 
...
int
_bcm_dpp_wb_cosq_update_se_state(int unit, int core, int se_id, uint32 ref_cnt)
{
    int                                  rc = BCM_E_NONE;
    bcm_dpp_wb_cosq_info_t              *wb_info;
    bcm_dpp_cosq_config_t               *cosq_config;
...
int
_bcm_dpp_wb_cosq_update_e2e_fc_data(int unit, int core, int flags, int id1, int id2)
{
    int                                  rc = BCM_E_NONE;
    bcm_dpp_wb_cosq_info_t              *wb_info;
    soc_scache_handle_t                  wb_handle;
    ...
    for (index = 0; index < no_ids; index++) {
        switch(wb_info->version) {
            case BCM_DPP_WB_COSQ_VERSION_1_0:
                ref_count = 0;
                rc = _bcm_dpp_am_template_cosq_hr_fc_ref_get(unit, core, id[index], &ref_count);
                if (rc != BCM_E_NONE) {
                    ref_count = 0;
                    rc = BCM_E_NONE;
                }
...
int
_bcm_dpp_wb_cosq_update_class_data(int unit, int core, int flags, int id1, int id2)
{
    int                                  rc = BCM_E_NONE;
    bcm_dpp_wb_cosq_info_t              *wb_info;
    soc_scache_handle_t                  wb_handle;
    ...
    for (index = 0; index < no_ids; index++) {
        switch(wb_info->version) {
            case BCM_DPP_WB_COSQ_VERSION_1_0:
                ref_count = 0;
                rc = _bcm_dpp_am_template_cosq_sched_class_ref_get(unit, core, id[index], &ref_count);
                if (rc != BCM_E_NONE) {
                    ref_count = 0;
                    rc = BCM_E_NONE;
                }
...
int
_bcm_dpp_wb_cosq_update_vsq_rate_class_data(int unit, int flags, SOC_TMC_ITM_VSQ_GROUP vsq_group, int id1, int id2)
{
    int                                       rc = BCM_E_NONE;
    bcm_dpp_wb_cosq_info_t                   *wb_info;
    bcm_dpp_wb_cosq_vsq_rate_class_data_t    *vsq_rate_state;
    ...
    for (index = 0; index < no_ids; index++) {
        switch(wb_info->version) {
            case BCM_DPP_WB_COSQ_VERSION_1_0:
                ref_count = 0;
                rc = _bcm_dpp_am_template_vsq_rate_cls_ref_get(unit, id[index], vsq_group, &ref_count);
                if (rc != BCM_E_NONE) {
                    ref_count = 0;
                    rc = BCM_E_NONE;
                }
                ...
                vsq_rate_state = (bcm_dpp_wb_cosq_vsq_rate_class_data_t *)(wb_info->scache_ptr + wb_info->vsq_rate_class_data_off);
                (vsq_rate_state + id[index])->ref_cnt[vsq_group] = ref_count;
                ...
                if (ref_count != 0) {
                    ...
                    /*        Following is a quick fix. THe long term solution is to store only  */
                    /*        the state that cannot be easily re-created from h/w.               */
                    vsq_rate_class_state = (bcm_dpp_cosq_vsq_rate_class_info_t *) &((vsq_rate_state + id[index])->data);
                    (*vsq_rate_class_state) = vsq_rate_class_info;
                }
...
int
_bcm_dpp_wb_cosq_update_vsq_data(int unit, int core, int category_mode)
{
    int                                    rc = BCM_E_NONE;
    bcm_dpp_wb_cosq_info_t                *wb_info;
    bcm_dpp_wb_cosq_gbl_data_t            *gbl_state;
    ...
    BCM_DPP_ASSYMETRIC_CORES_ITER(core, core_index) {
        switch(wb_info->version) {
            case BCM_DPP_WB_COSQ_VERSION_1_0:
                gbl_state = (bcm_dpp_wb_cosq_gbl_data_t *)(wb_info->scache_ptr + wb_info->gbl_data_off[core_index]);
                gbl_state->config.vsq_category_mode = category_mode;

                data = (uint8 *)&(gbl_state->config.vsq_category_mode);
                offset = (uint32)(data - wb_info->scache_ptr);
...
int
_bcm_dpp_wb_cosq_update_ucast_data(int unit, int core, int qid_start, int qid_end)
{
    int                                    rc = BCM_E_NONE;
    bcm_dpp_wb_cosq_info_t                *wb_info;
    bcm_dpp_wb_cosq_gbl_data_t            *gbl_state;
    ...
    BCM_DPP_ASSYMETRIC_CORES_ITER(core, core_index) {
        switch(wb_info->version) {
            case BCM_DPP_WB_COSQ_VERSION_1_0:
                gbl_state = (bcm_dpp_wb_cosq_gbl_data_t *)(wb_info->scache_ptr + wb_info->gbl_data_off[core_index]);
                gbl_state->config.ucast_qid_start = qid_start;
                gbl_state->config.ucast_qid_end = qid_end;

                data = (uint8 *)&(gbl_state->config.ucast_qid_start);
                offset = (uint32)(data - wb_info->scache_ptr);
