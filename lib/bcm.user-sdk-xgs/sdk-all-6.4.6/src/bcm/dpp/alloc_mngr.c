...
#define _DPP_AM_TEMPLATE_OAM_MEP_PROFILE_ACC_COUNT (SOC_PPD_OAM_ACC_PROFILES_NUM) /* Highest profile is reseved for passive */
...
/*nof VSQFs*/
#define _DPP_AM_TEMPLATE_VSQ_PG_TC_MAPPING_MAX_ENTITIES(unit) (SOC_TMC_ITM_VSQ_GROUPF_SZE(unit))
...
#define _DPP_AM_TEMPLATE_VSQ_RATE_CLS_SIZE(vsq_group)                                                               (\
            ...
            (vsq_group == SOC_TMC_ITM_VSQ_GROUP_SRC_PORT) ? (sizeof(bcm_dpp_cosq_src_port_vsq_rate_class_info_t)) : (\
...
_bcm_dpp_pp_resource_setup(int unit, int res_id, int core_id, int pool_id, bcm_dpp_am_pool_info_t *p_info) {
    ...
    break;
        ...
        case dpp_am_res_vsq_src_port:
            if (!SOC_IS_ARADPLUS_AND_BELOW(unit)) {
                p_info->res_type = SHR_RES_ALLOCATOR_BITMAP;
                p_info->count = SOC_TMC_ITM_VSQ_GROUPE_SZE(unit);
                p_info->start = 0;
        ...
        case dpp_am_res_vsq_pg:
            if (!SOC_IS_ARADPLUS_AND_BELOW(unit)) {
                p_info->res_type = SHR_RES_ALLOCATOR_BITMAP;
                p_info->count = SOC_TMC_ITM_VSQ_GROUPF_SZE(unit);
                p_info->start = 0;
                desc_str = "VSQ PG ID";
        ...
        int *elem)
{
    ...
    *elem = trap_id;
    ...
    if ( (SOC_PPD_TRAP_CODE_USER_DEFINED_0 <= trap_id) && (trap_id <= _BCM_LAST_USER_DEFINE) ) { 
        ...
        */
        if( trap_id != SOC_PPD_TRAP_CODE_USER_DEFINED_0 )
        {
            flags |= SHR_RES_ALLOC_WITH_ID;
        }
		...
		if (meter_group < BCM_DPP_AM_METER_GROUP_ANY) {
			...
			*act_meter_group = meter_group;
			...
			if (nof_meters == 1) {
				...
				rv = dpp_am_res_alloc_align(unit, (*act_meter_core), dpp_am_res_meter_a + (*act_meter_group), flags | SHR_RES_BITMAP_ALLOC_ALIGN_ZERO, 
...
int _bcm_dpp_am_template_oam_local_port_2_sys_port_free(int unit, int profile, int *is_last)
{
    ...
    rc = dpp_am_template_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_oam_local_port_2_system_port, profile, is_last);
    BCMDNX_IF_ERR_EXIT(rc);
...
int
bcm_dpp_am_vsq_src_port_init(int unit) {
    int rv = BCM_E_NONE;
    int core_id;
    int src_pp_port;
    BCMDNX_INIT_FUNC_DEFS;
    BCM_DPP_CORES_ITER(BCM_CORE_ALL, core_id) {
        for (src_pp_port = 0; src_pp_port < SOC_MAX_NUM_PORTS; src_pp_port++) {
            rv = sw_state_access[unit].dpp.soc.arad.tm.tm_info.src_port_vsq.set(unit, core_id, src_pp_port, SOC_TMC_ITM_VSQ_GROUPE_SZE(unit));
            BCMDNX_IF_ERR_EXIT(rv);
        }
    }
exit:
    ...
    int *vsq_index)
{
    ...
    BCM_DPP_CORES_ITER(core_id, core_index) {
        rv = sw_state_access[unit].dpp.soc.arad.tm.tm_info.src_port_vsq.get(unit, core_index, src_pp_port, &vsq_index_get);
        BCMDNX_IF_ERR_EXIT(rv);
        if (vsq_index_get == SOC_TMC_ITM_VSQ_GROUPE_SZE(unit)) {
            BCMDNX_IF_ERR_EXIT(BCM_E_PARAM);
        }
    }
    ...
    BCM_DPP_CORES_ITER(core_id, core_index) {
        rv = sw_state_access[unit].dpp.soc.arad.tm.tm_info.src_port_vsq.set(unit, core_index, src_pp_port, *vsq_index);
        BCMDNX_IF_ERR_EXIT(rv);
    }
exit:
    BCMDNX_FUNC_RETURN;
    ...
    int vsq_index)
{
    ...
    BCM_DPP_CORES_ITER(core_id, core_index) {
        for (src_pp_port = 0; src_pp_port < SOC_MAX_NUM_PORTS; src_pp_port++) {
            rv = sw_state_access[unit].dpp.soc.arad.tm.tm_info.src_port_vsq.get(unit, core_index, src_pp_port, &vsq_index_get);
            BCMDNX_IF_ERR_EXIT(rv);
            if (vsq_index == vsq_index_get) {
                rv = sw_state_access[unit].dpp.soc.arad.tm.tm_info.src_port_vsq.set(unit, core_index, src_pp_port, SOC_TMC_ITM_VSQ_GROUPE_SZE(unit));
                BCMDNX_IF_ERR_EXIT(rv);
                break;
            }
        }
    ...
    )
{
    ...
    uint8 numq_get;
    BCMDNX_INIT_FUNC_DEFS;
    core_index = (core_id == BCM_CORE_ALL || SOC_DPP_CORE_MODE_IS_SYMMETRIC(unit)) ? 0 : core_id;
...
int _bcm_dpp_am_template_vsq_rate_cls_data_get(int unit, int core_id, int vsq_id, bcm_dpp_cosq_vsq_rate_class_info_t * data_rate_cls)
{
    ...
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, vsq_template[vsq_group], rate_cls, _bcm_dpp_am_template_vsq_rate_cls_data(vsq_group, data_rate_cls));
    BCMDNX_IF_ERR_EXIT(rc);
exit:
    BCMDNX_FUNC_RETURN;
}
...
int _bcm_dpp_am_template_vsq_rate_cls_exchange(int unit, int core_id, int vsq_id, bcm_dpp_cosq_vsq_rate_class_info_t * data_rate_cls, int *old_rate_cls, int *is_last, int *new_rate_cls, int *is_allocated)
{
    ...
    rc = dpp_am_template_exchange(unit, BCM_DPP_AM_DEFAULT_CORE_ID, vsq_template[vsq_group], 0, _bcm_dpp_am_template_vsq_rate_cls_data(vsq_group, data_rate_cls), tmp_old_rate_cls, &tmp_is_last, new_rate_cls, is_allocated);
    BCMDNX_IF_ERR_EXIT(rc);
...
_bcm_dpp_am_template_vsq_rate_cls_ref_get(int unit, int template, SOC_TMC_ITM_VSQ_GROUP vsq_group, uint32 *ref_count)
{
    ...
    rc = dpp_am_template_ref_count_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, vsq_template[vsq_group], template, ref_count);
    BCMDNX_IF_ERR_EXIT(rc);
exit:
    BCMDNX_FUNC_RETURN;
}
...
_bcm_dpp_am_template_vsq_rate_cls_allocate_group(int unit, uint32 flags, SOC_TMC_ITM_VSQ_GROUP vsq_group, bcm_dpp_cosq_vsq_rate_class_info_t *data_rate_cls, int ref_count, int *is_allocated, int *template)
{
    ...
    rc = dpp_am_template_allocate_group(unit, BCM_DPP_AM_DEFAULT_CORE_ID, vsq_template[vsq_group], flags, _bcm_dpp_am_template_vsq_rate_cls_data(vsq_group, data_rate_cls), ref_count, is_allocated, template);
    BCMDNX_IF_ERR_EXIT(rc);
...
_bcm_dpp_am_template_vsq_rate_cls_tdata_get(int unit, int template, bcm_dpp_cosq_vsq_rate_class_info_t *data_rate_cls)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    for (vsq_group = 0; vsq_group < SOC_TMC_NOF_VSQ_GROUPS; vsq_group++) {
        rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, vsq_template[vsq_group], template, _bcm_dpp_am_template_vsq_rate_cls_data(vsq_group, data_rate_cls));
        BCMDNX_IF_ERR_EXIT(rc);
    }
exit:
    BCMDNX_FUNC_RETURN;
...
int _bcm_dpp_am_template_meter_alloc(int unit, int core_id, int flags, int group, int high_rate, bcm_dpp_am_meter_entry_t *meter_profile_info, int *meter_profile,int *is_allocated)
{
    int rc = BCM_E_NONE;
    _dpp_am_template_t tmp_tmplt_type = dpp_am_template_meter_profile_a_low;
    ...
    rc = dpp_am_template_allocate(unit, core_id, tmp_tmplt_type, flags, meter_profile_info, is_allocated, meter_profile);
    BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_meter_free(int unit, int core_id, int meter_profile, int group, int high_rate, int *is_last)
{
    int rc = BCM_E_NONE;
    _dpp_am_template_t tmp_tmplt_type = dpp_am_template_meter_profile_a_low;
    ...
    rc = dpp_am_template_free(unit, core_id, tmp_tmplt_type, meter_profile, is_last);
    BCMDNX_IF_ERR_EXIT(rc);
...
{
    int core_id, rc = BCM_E_NONE;

    BCMDNX_INIT_FUNC_DEFS;

	_DPP_POLICER_METER_CORES_ITER(core_id){
		rc = dpp_am_template_clear(unit, core_id, dpp_am_template_meter_profile_a_low);
		BCMDNX_IF_ERR_EXIT(rc);

		rc = dpp_am_template_clear(unit, core_id, dpp_am_template_meter_profile_a_high);
		BCMDNX_IF_ERR_EXIT(rc);

		rc = dpp_am_template_clear(unit, core_id, dpp_am_template_meter_profile_b_low);
		BCMDNX_IF_ERR_EXIT(rc);

		rc = dpp_am_template_clear(unit, core_id, dpp_am_template_meter_profile_b_high);
		BCMDNX_IF_ERR_EXIT(rc);
	}
exit:
    BCMDNX_FUNC_RETURN;
...
{
  int rc = BCM_E_NONE;
  int profile_ndx = *profile_id;

  BCMDNX_INIT_FUNC_DEFS;

  rc = dpp_am_template_template_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_l3_rif_mac_termination_combination, &profile_info, &profile_ndx);
  BCMDNX_IF_ERR_EXIT(rc);
