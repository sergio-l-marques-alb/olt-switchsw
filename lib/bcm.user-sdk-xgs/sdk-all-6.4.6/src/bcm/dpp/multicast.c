    ...
    uint8 is_delete)                        /* input: if non zero this is a delete operation, otherwise an add */
{
    ...
    if (port_bm) { /* received port bitmap, used in non Petra devices */
        mbcm_dpp_mult_eg_vlan_membership_bm_add_f soc_func = is_delete ? dpp_mult_eg_bitmap_group_bm_remove : dpp_mult_eg_bitmap_group_bm_add;
...
_bcm_petra_multicast_egress_entries_set(int unit,SOC_TMC_MULT_ID multicast_id_ndx, int port_count, bcm_port_t *local_ports_arr,bcm_if_t *encap_id_arr)
{
    ...
    for (ndx = 0; ndx < port_count; ndx++)
    {
        ...
        entries[ndx].port = local_ports_arr[ndx];
        entries[ndx].cud  = encap_id_arr[ndx];
    }
    ...
    dpp_mc_replication_t **out_reps)        /* output: pointer to output replications array allocated by this function */
{
    ...
    for (i = 0; i < rep_count ; i++) {
        if (use_reps) { /* get the destination and CUD/s from the correct input source */
            ...
            if (r->flags & BCM_MUTICAST_REPLICATION_ENCAP2_VALID) {
                if (r->flags & BCM_MUTICAST_REPLICATION_ENCAP1_L3_INTF) {
                    ...
                    cud = r->encap1;
                    cud2 = r->encap2;
                    cud2_type_bit = DPP_MC_2ND_CUD_IS_EEI;
                }
            } else {
                cud = r->encap1;
        ...
        if (support_outlif_only_reps && dest == BCM_GPORT_INVALID) { /* outlif only replication (port mapped from outlif), discontinued in QAX */
            ...
            BCMDNX_IF_ERR_EXIT(_bcm_dpp_gport_to_phy_port(unit, dest, _BCM_DPP_GPORT_TO_PHY_OP_LOCAL_IS_MANDATORY, &gport_info)); 
    ...
    uint8 is_delete)                        /* input: if non zero this is a delete operation, otherwise an add */
{
    ...
    uint8 is_in_range, open, support_outlif_only_reps = TRUE, support_bitmap_reps = TRUE;
    ...
    if (is_in_range) { /* Handle bitmap groups. They do not have CUDs */
        ...
        BCMDNX_IF_ERR_EXIT(_bcm_petra_multicast_egress_vlan_membership_port_add_or_remove(unit, mcid, 0, &pbmp, is_delete));
    ...
    uint8 is_delete)        /* input: if non zero this is a delete operation, otherwise an add */
{
    ...
    BCM_PBMP_ITER(gport_info.pbmp_local_ports, local_port) { /* add/remove ports in bitmap */
        if (is_in_range) {
            BCMDNX_IF_ERR_EXIT(_bcm_petra_multicast_egress_vlan_membership_port_add_or_remove(unit, multicast_id_ndx, local_port, 0, is_delete));
        } else {
            SOC_TMC_ERROR mc_err = BCM_E_NONE;
            int existance_error;
            SOC_TMC_MULT_EG_ENTRY entry;
    ...
    bcm_if_t *encap_id)
{
    ...
    if(encap_type == _bcmDppGportResolveTypeAC || encap_type == _bcmDppGportResolveTypeFec) {
        encap_val = *encap_id;
    ...
    bcm_if_t *encap_id)
{
    ...
    else if (encap_type == _bcmDppGportResolveTypePhyEEP || encap_type == _bcmDppGportResolveTypeFec) {
        /* Translate AC to CUD */
