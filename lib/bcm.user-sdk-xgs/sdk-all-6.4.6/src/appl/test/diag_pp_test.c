...
int
diag_pp_received_packet_get(int unit, int core_id, diag_pp_received_packets_fields_t *pkt_info)
{
    int rv = 0;
    ...
    rv = soc_ppd_diag_received_packet_info_get(soc_sand_dev_id, core_id, &prm_rcvd_pkt_info);
...
diag_pp_parsing_info_get(int unit, int core_id, diag_pp_parsing_info_fields_t *pars_info)
{
    ...
    rv = soc_ppd_diag_parsing_info_get(soc_sand_dev_id, core_id, &prm_pars_info);
    if (soc_sand_get_error_code_from_error_word(rv) != SOC_SAND_OK) 
    { 
...
diag_pp_termination_info_get(int unit, int core_id, diag_pp_termination_info_fields_t *term_info)
{
    ...
    rv = soc_ppd_diag_termination_info_get(soc_sand_dev_id, core_id, &prm_term_info);
    if (soc_sand_get_error_code_from_error_word(rv) != SOC_SAND_OK) 
    { 
...
diag_pp_vlan_edit_info_get(int unit, int core_id, diag_pp_vlan_edit_info_fields_t *vlan_edit_info)
{
    ...
    rv = soc_ppd_diag_ing_vlan_edit_info_get(soc_sand_dev_id, core_id, &prm_vlan_edit_info);
    if (soc_sand_get_error_code_from_error_word(rv) != SOC_SAND_OK) 
    { 
...
diag_pp_pkt_tm_info_get(int unit, int core_id, diag_pp_pkt_tm_info_fields_t *pkt_tm_info)
{
    ...
    rv = soc_ppd_diag_pkt_associated_tm_info_get(soc_sand_dev_id, core_id, &prm_pkt_tm_info);
    if (soc_sand_get_error_code_from_error_word(rv) != SOC_SAND_OK) 
    { 
...
diag_pp_lif_lkup_info_get(int unit, int core_id, diag_pp_lif_lkup_info_fields_t *lif_lkup_info, SOC_PPC_DIAG_DB_USE_INFO *db_info)
{
    ...
    rv = soc_ppd_diag_db_lif_lkup_info_get(soc_sand_dev_id, core_id, db_info, &prm_lif_lkup_info);
    if (soc_sand_get_error_code_from_error_word(rv) != SOC_SAND_OK) 
    { 
...
int
diag_pp_frwrd_trace_info_get(int unit, int core_id, SOC_PPD_DIAG_FRWRD_DECISION_TRACE_INFO *frwrd_trace_info)
{
    int rv = 0;
    uint32 soc_sand_dev_id;
    ...
    rv = soc_ppd_diag_frwrd_decision_trace_get(soc_sand_dev_id, core_id, frwrd_trace_info);
    if (soc_sand_get_error_code_from_error_word(rv) != SOC_SAND_OK) 
    { 
...
int
diag_pp_traps_info_get(int unit, int core_id, SOC_PPD_DIAG_TRAPS_INFO *traps_info)
{
    int rv = 0;
    uint32 soc_sand_dev_id;
    ...
    rv = soc_ppd_diag_traps_info_get(soc_sand_dev_id, core_id, traps_info);
    if (soc_sand_get_error_code_from_error_word(rv) != SOC_SAND_OK) 
    { 
...
int
diag_pp_encap_info_get(int unit, int core_id, SOC_PPD_DIAG_ENCAP_INFO *encap_info)
{
    int rv = 0;
    uint32 soc_sand_dev_id;
    ...
    rv = soc_ppd_diag_encap_info_get(soc_sand_dev_id, core_id, encap_info);
    if (soc_sand_get_error_code_from_error_word(rv) != SOC_SAND_OK) 
    { 
...
int
diag_pp_frwrd_lkup_info_get(int unit, int core_id, SOC_PPD_DIAG_FRWRD_LKUP_INFO *frwrd_lkup_info)
{
    int rv = 0;
    uint32 soc_sand_dev_id;
    ...
    rv = soc_ppd_diag_frwrd_lkup_info_get(soc_sand_dev_id, core_id, frwrd_lkup_info);
    if (soc_sand_get_error_code_from_error_word(rv) != SOC_SAND_OK) 
    { 
...
int
diag_pp_egress_vlan_edit_info_get(int unit, int core_id, SOC_PPD_DIAG_EGRESS_VLAN_EDIT_RES *egress_vlan_edit_info)
{
    int rv = 0;
    uint32 soc_sand_dev_id;
    ...
    rv = soc_ppd_diag_egress_vlan_edit_info_get(soc_sand_dev_id, core_id, egress_vlan_edit_info);
...
diag_pp_received_packet_bridge_test(int unit, int core_id)
{
    ...
    if((rv = diag_pp_received_packet_get(unit, core_id, &received_pkt_info )) < 0)
    { 
...
diag_pp_lif_lkup_info_bridge_test(int unit, int core_id)
{
    ...
    expected_lif_lkup_info.key.ac.vlan_domain = 200;
    ...
    expected_lif_lkup_info.base_index = 4096;
...
diag_pp_frwrd_trace_info_bridge_test(int unit, int core_id)
{
    ...
    if((rv = diag_pp_frwrd_trace_info_get(unit, core_id, &received_frwrd_trace_info)) < 0)
    { 
...
diag_pp_encap_info_bridge_test(int unit, int core_id)
{
    ...
    if((rv = diag_pp_encap_info_get(unit, core_id, &received_encap_info)) < 0)
    { 
...
diag_pp_pkt_tm_info_mpls_test(int unit, int core_id)
{
    ...
    if (SOC_IS_JERICHO(unit)) {
        expected_pkt_tm_info.cud = 258048;
    } else {
...
diag_pp_lif_lkup_info_mpls_test(int unit, int core_id)
{
    ...
    expected_lif_lkup_info.found = 1; 
    expected_lif_lkup_info.base_index = SOC_IS_JERICHO(unit)?32768:16384;
    expected_lif_lkup_info.opcode_id = 0;
...
diag_pp_frwrd_trace_info_mpls_test(int unit, int core_id)
{
    ...
    expected_frwrd_trace_info.frwrd[SOC_PPC_DIAG_FRWRD_DECISION_PHASE_LKUP_FOUND].frwrd_decision.type = SOC_PPC_FRWRD_DECISION_TYPE_FEC;
    expected_frwrd_trace_info.frwrd[SOC_PPC_DIAG_FRWRD_DECISION_PHASE_LKUP_FOUND].frwrd_decision.dest_id = SOC_IS_JERICHO(unit)?4096:1024;
    expected_frwrd_trace_info.frwrd[SOC_PPC_DIAG_FRWRD_DECISION_PHASE_LKUP_FOUND].is_valid = 1;
    ...
    expected_frwrd_trace_info.frwrd[SOC_PPC_DIAG_FRWRD_DECISION_PHASE_PMF].frwrd_decision.type = SOC_PPC_FRWRD_DECISION_TYPE_FEC;
    expected_frwrd_trace_info.frwrd[SOC_PPC_DIAG_FRWRD_DECISION_PHASE_PMF].frwrd_decision.dest_id = SOC_IS_JERICHO(unit)?4096:1024;
...
diag_pp_frwrd_lkup_info_mpls_test(int unit, int core_id)
{
    ...
    expected_frwrd_lkup_info.lkup_res.frwrd_decision.dest_id = SOC_IS_JERICHO(unit)?4096:1024;
...
diag_pp_lem_lkup_info_mpls_test(int unit, int core_id)
{
    ...
    expected_lem_lkup_info.value.ilm.dest_id = SOC_IS_JERICHO(unit)?4096:1024;
...
diag_pp_encap_info_vpls_test(int unit, int core_id, uint32 ac_to_pwe)
{
    ...
    if (ac_to_pwe) {
        ...
        if (SOC_IS_JERICHO(unit)) {
            expected_encap_info.out_ac = 16384;
        } else {
...
diag_pp_frwrd_lkup_info_vpls_test(int unit, int core_id, uint32 ac_to_pwe)
{
    ...
    if (ac_to_pwe){   
        ...
        if (SOC_IS_JERICHO(unit)) {
            expected_frwrd_lkup_info.lkup_res.mact.frwrd_info.forward_decision.dest_id = 4096;
        } else {
        ...
        if (SOC_IS_JERICHO(unit)) {
            expected_frwrd_lkup_info.lkup_res.mact.frwrd_info.forward_decision.additional_info.outlif.val = 253952;
        } else {
...
diag_pp_lem_lkup_info_vpls_test(int unit, int core_id, uint32 ac_to_pwe)
{
    ...
    if (ac_to_pwe){
        ...
        if (SOC_IS_JERICHO(unit)) {
            expected_lem_lkup_info.value.mact.frwrd_info.forward_decision.dest_id = 4096;
        } else {
        ...
        if (SOC_IS_JERICHO(unit)) {
            expected_lem_lkup_info.value.mact.frwrd_info.forward_decision.additional_info.outlif.val  = 253952;
        } else {
...
diag_pp_frwrd_trace_info_vpls_test(int unit, int core_id, uint32 ac_to_pwe)
{
    ...
    if (ac_to_pwe) {
        expected_frwrd_trace_info.frwrd[SOC_PPC_DIAG_FRWRD_DECISION_PHASE_LKUP_FOUND].frwrd_decision.type = SOC_PPC_FRWRD_DECISION_TYPE_FEC;
        if (SOC_IS_JERICHO(unit)) {
            expected_frwrd_trace_info.frwrd[SOC_PPC_DIAG_FRWRD_DECISION_PHASE_LKUP_FOUND].frwrd_decision.dest_id = 4096;
        } else {
            expected_frwrd_trace_info.frwrd[SOC_PPC_DIAG_FRWRD_DECISION_PHASE_LKUP_FOUND].frwrd_decision.dest_id = 1024;
        }
        expected_frwrd_trace_info.frwrd[SOC_PPC_DIAG_FRWRD_DECISION_PHASE_LKUP_FOUND].is_valid = 1;
    ...
    if (ac_to_pwe) {
        expected_frwrd_trace_info.frwrd[SOC_PPC_DIAG_FRWRD_DECISION_PHASE_PMF].frwrd_decision.type = SOC_PPC_FRWRD_DECISION_TYPE_FEC;
        if (SOC_IS_JERICHO(unit)) {
            expected_frwrd_trace_info.frwrd[SOC_PPC_DIAG_FRWRD_DECISION_PHASE_PMF].frwrd_decision.dest_id = 4096;
        } else {
            expected_frwrd_trace_info.frwrd[SOC_PPC_DIAG_FRWRD_DECISION_PHASE_PMF].frwrd_decision.dest_id = 1024;
        }
        expected_frwrd_trace_info.frwrd[SOC_PPC_DIAG_FRWRD_DECISION_PHASE_PMF].frwrd_decision.additional_info.eei.type = SOC_PPC_EEI_TYPE_MPLS;
...
diag_pp_lif_lkup_info_vpls_test(int unit, int core_id, uint32 ac_to_pwe)
{
    ...
    if (!ac_to_pwe) {
        ...
        expected_lif_lkup_info.type = SOC_PPC_DIAG_LIF_LKUP_TYPE_MPLS_TUNNEL;
        if (SOC_IS_JERICHO(unit)) {
            expected_lif_lkup_info.base_index = 32768;
        } else {
    ...
    if (ac_to_pwe) {
        ...
        expected_lif_lkup_info.type = SOC_PPC_DIAG_LIF_LKUP_TYPE_AC;
        if (SOC_IS_JERICHO(unit)) {
            expected_lif_lkup_info.base_index = 2;
        } else {
...
diag_pp_test(int unit, args_t *a, void *p)
{
    ...
    if(dpt->bridge)
    {
        if((rv = diag_pp_received_packet_bridge_test(unit, dpt->core)) < 0)
        {
            cli_out("Error: diag_pp_received_packet_bridge_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_parsing_info_bridge_test(unit, dpt->core)) < 0)
        {
            cli_out("Error: diag_pp_parsing_info_bridge_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_termination_info_bridge_test(unit, dpt->core)) < 0)
        {
            cli_out("Error: diag_pp_termination_info_bridge_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_vlan_edit_info_bridge_test(unit, dpt->core)) < 0)
        {
            cli_out("Error: diag_pp_vlan_edit_info_bridge_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_pkt_tm_info_bridge_test(unit, dpt->core)) < 0)
        {
            cli_out("Error: diag_pp_pkt_tm_info_bridge_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_lif_lkup_info_bridge_test(unit, dpt->core)) < 0)
        {
            cli_out("Error: diag_pp_lif_lkup_info_bridge_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_frwrd_trace_info_bridge_test(unit, dpt->core)) < 0)
        {
            cli_out("Error: diag_pp_frwrd_trace_info_bridge_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_traps_info_bridge_test(unit, dpt->core)) < 0)
        {
            cli_out("Error: diag_pp_traps_info_bridge_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_encap_info_bridge_test(unit, dpt->core)) < 0)
        {
            cli_out("Error: diag_pp_encap_info_bridge_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_frwrd_lkup_info_bridge_test(unit, dpt->core)) < 0)
        {
            cli_out("Error: diag_pp_frwrd_lkup_info_bridge_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_frwrd_lpm_lkup_bridge_test(unit, dpt->core)) < 0)
        {
            cli_out("Error: diag_pp_frwrd_lpm_lkup_bridge_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_lem_lkup_info_bridge_test(unit, dpt->core)) < 0)
        {
            cli_out("Error: diag_pp_lem_lkup_bridge_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_egress_vlan_edit_info_bridge_test(unit, dpt->core )) < 0)
        {
            cli_out("Error: diag_pp_lem_lkup_bridge_test () Failed:\n");
            goto exit;
        }
    ...
    {
        if((rv = diag_pp_received_packet_router_test(unit, dpt->core)) < 0)
        {
            cli_out("Error: diag_pp_received_packet_router_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_parsing_info_router_test(unit, dpt->core)) < 0)
        {
            cli_out("Error: diag_pp_parsing_info_router_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_termination_info_router_test(unit, dpt->core)) < 0)
        {
            cli_out("Error: diag_pp_termination_info_router_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_vlan_edit_info_router_test(unit, dpt->core)) < 0)
        {
            cli_out("Error: diag_pp_vlan_edit_info_router_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_pkt_tm_info_router_test(unit, dpt->core)) < 0)
        {
            cli_out("Error: diag_pp_pkt_tm_info_router_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_lif_lkup_info_router_test(unit, dpt->core)) < 0)
        {
            cli_out("Error: diag_pp_lif_lkup_info_router_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_frwrd_trace_info_router_test(unit, dpt->core)) < 0)
        {
            cli_out("Error: diag_pp_frwrd_trace_info_router_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_traps_info_router_test(unit, dpt->core)) < 0)
        {
            cli_out("Error: diag_pp_traps_info_router_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_encap_info_router_test(unit, dpt->core)) < 0)
        {
            cli_out("Error: diag_pp_encap_info_router_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_frwrd_lkup_info_router_test(unit, dpt->core)) < 0)
        {
            cli_out("Error: diag_pp_frwrd_lkup_info_router_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_frwrd_lpm_lkup_router_test(unit, dpt->core)) < 0)
        {
            cli_out("Error: diag_pp_frwrd_lpm_lkup_router_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_lem_lkup_info_router_test(unit, dpt->core)) < 0)
        {
            cli_out("Error: diag_pp_lem_lkup_info_router_test () Failed:\n");
            goto exit;      
        }
        if((rv = diag_pp_egress_vlan_edit_info_router_test(unit, dpt->core)) < 0)
        {
            cli_out("Error: diag_pp_lem_lkup_bridge_test () Failed:\n");
            goto exit;
        }
    ...
    {
        if((rv = diag_pp_received_packet_mpls_test(unit, dpt->core)) < 0)
        {
            cli_out("Error: diag_pp_received_packet_mpls_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_parsing_info_mpls_test(unit, dpt->core)) < 0)
        {
            cli_out("Error: diag_pp_parsing_info_mpls_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_termination_info_mpls_test(unit, dpt->core)) < 0)
        {
            cli_out("Error: diag_pp_termination_info_mpls_test () Failed:\n");
            goto exit;
        }
        if (!SOC_IS_JERICHO(unit)) {
            if((rv = diag_pp_vlan_edit_info_mpls_test(unit, dpt->core)) < 0)
            {
                cli_out("Error: diag_pp_vlan_edit_info_mpls_test () Failed:\n");
                goto exit;
            }
        }
        if((rv = diag_pp_pkt_tm_info_mpls_test(unit, dpt->core)) < 0)
        {
            cli_out("Error: diag_pp_pkt_tm_info_mpls_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_lif_lkup_info_mpls_test(unit, dpt->core)) < 0)
        {
            cli_out("Error: diag_pp_lif_lkup_info_mpls_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_frwrd_trace_info_mpls_test(unit, dpt->core)) < 0)
        {
            cli_out("Error: diag_pp_frwrd_trace_info_mpls_test () Failed:\n");
            goto exit;
        } 
        if (!SOC_IS_JERICHO(unit)) {
            if((rv = diag_pp_traps_info_mpls_test(unit, dpt->core)) < 0)
            {
                cli_out("Error: diag_pp_traps_info_mpls_test () Failed:\n");
                goto exit;
            }
        }
        if((rv = diag_pp_encap_info_mpls_test(unit, dpt->core)) < 0)
        {
            cli_out("Error: diag_pp_encap_info_mpls_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_frwrd_lkup_info_mpls_test(unit, dpt->core)) < 0)
        {
            cli_out("Error: diag_pp_frwrd_lkup_info_mpls_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_lem_lkup_info_mpls_test(unit, dpt->core)) < 0)
        {
            cli_out("Error: diag_pp_lem_lkup_info_mpls_test () Failed:\n");
            goto exit;      
        }
        if ((rv = diag_pp_egress_vlan_edit_info_mpls_test(unit, dpt->core)) < 0) 
        {
    ...
    else if(dpt->vpls)
    {
        ...
        if((rv = diag_pp_received_packet_vpls_test(unit, dpt->core, ac_to_pwe)) < 0)
        {
            cli_out("Error: diag_pp_received_packet_vpls_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_parsing_info_vpls_test(unit, dpt->core, ac_to_pwe)) < 0)
        {
            cli_out("Error: diag_pp_parsing_info_vpls_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_termination_info_vpls_test(unit, dpt->core, ac_to_pwe)) < 0)
        {
            cli_out("Error: diag_pp_termination_info_vpls_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_vlan_edit_info_vpls_test(unit, dpt->core, ac_to_pwe)) < 0)
        {
            cli_out("Error: diag_pp_vlan_edit_info_vpls_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_lif_lkup_info_vpls_test(unit, dpt->core, ac_to_pwe)) < 0)
        {
            cli_out("Error: diag_pp_lif_lkup_info_vpls_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_frwrd_trace_info_vpls_test(unit, dpt->core, ac_to_pwe)) < 0)
        {
            cli_out("Error: diag_pp_frwrd_trace_info_vpls_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_encap_info_vpls_test(unit, dpt->core, ac_to_pwe)) < 0)
        {
            cli_out("Error: diag_pp_encap_info_vpls_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_frwrd_lkup_info_vpls_test(unit, dpt->core, ac_to_pwe)) < 0)
        {
            cli_out("Error: diag_pp_frwrd_lkup_info_vpls_test () Failed:\n");
            goto exit;
        }
        if((rv = diag_pp_lem_lkup_info_vpls_test(unit, dpt->core, ac_to_pwe)) < 0)
        {
            cli_out("Error: diag_pp_lem_lkup_info_vpls_test () Failed:\n");
            goto exit;      
        }
        if ((rv = diag_pp_egress_vlan_edit_info_vpls_test(unit, dpt->core, ac_to_pwe)) < 0) 
        {
