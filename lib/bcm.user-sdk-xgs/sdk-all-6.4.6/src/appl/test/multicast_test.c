...
int multicast_arad_test_init(int unit)
{
    ...
    if (SOC_IS_QAX(unit)) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_FAIL, (_BCM_MSG("access to registers should be fixed for QAX at places we used _REG(32|64) access routines")));
    }
...
int test_bcm_multicast_create(int unit, uint32 flags, bcm_multicast_t *group, int may_run_out_of_space)
{
    ...
    if ((flags & (BCM_MULTICAST_EGRESS_GROUP | BCM_MULTICAST_TYPE_TRILL)) || (~flags & BCM_MULTICAST_INGRESS_GROUP)) {
        ...
        mc_member_is_ingress_set(unit, data->egress_mc_groups + multicast_id, 1); /* mark group as open */
...
int test_bcm_multicast_verify_all(int unit)
{
    ...
    if (SOC_IS_JERICHO(unit)) {
        BCMDNX_IF_ERR_EXIT(soc_mem_read_range(unit, EDB_MAP_OUTLIF_TO_DSPm, MEM_BLOCK_ANY, 0, MC_NOF_OUTLIF_TO_PORT - 1, mcdb)); 
        for (i = 0; i < MC_NOF_OUTLIF_TO_PORT; ++i) {
            outlif2dsp[-(int32)i] = soc_mem_field32_get(unit, EDB_MAP_OUTLIF_TO_DSPm, mcdb + i, DSPf);
        }
    } else {
    ...
    irdb -=2;
    if (data->ingress_mc_groups) {
        for (i = 0; i < SOC_DPP_CONFIG(unit)->tm.nof_ingr_mc_ids - (SOC_IS_JERICHO(unit)?1:0); ++i) { /* Jericho reserved one ingress group for workaround */
