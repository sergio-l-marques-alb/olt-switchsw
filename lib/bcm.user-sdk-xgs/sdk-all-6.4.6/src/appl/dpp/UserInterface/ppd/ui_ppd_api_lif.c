...
static SOC_PPD_L2_LIF_AC_GROUP_INFO
  prm_acs_group_info;
                                 ...
                                 )
{
  ...
  SOC_PPD_L2_LIF_IN_VC_RANGE_clear(&prm_in_vc_range);
                                 ...
                                 )
{
  ...
  SOC_PPD_L2_LIF_IN_VC_RANGE_print(&prm_in_vc_range);
...
)
{
  ...
  SOC_PPD_TRAP_CODE
    trap_code = SOC_PPD_NOF_TRAP_CODES;
  uint32
    dest_id=0,
    frwrd_strength=0,
    snp_strength=0,
  ...
  */
  if(!search_param_val_pairs(current_line,&match_index,SOC_PARAM_PPD_FRWRD_DECISION_FD_TRAP_CODE_ID+offset,1))
  {
    ...
    UI_MACROS_CHECK_GET_VAL_OF_ERROR;
    trap_code = (SOC_PPD_TRAP_CODE)param_val->value.ulong_value;
  }
                       ...
                       )
{
  ...
  unit = soc_ppd_get_default_unit();
  SOC_PPD_L2_LIF_PWE_INFO_clear(&prm_pwe_info);
  ...
  if (prm_pwe_info.service_type != SOC_PPD_L2_LIF_PWE_SERVICE_TYPE_MP)
  {
	  cli_out("prm_pwe_info.service_type != SOC_PPD_L2_LIF_PWE_SERVICE_TYPE_MP\n");
  ...
  {
    if (prm_pwe_info.default_frwrd.default_forwarding.additional_info.eei.type == SOC_PPD_EEI_TYPE_MPLS)
    {
      prm_pwe_info.default_frwrd.default_forwarding.additional_info.eei.val.mpls_command.label += idx;
    }
    if (prm_pwe_info.default_frwrd.default_forwarding.additional_info.outlif.type == SOC_PPD_OUTLIF_ENCODE_TYPE_AC)
    {
                       ...
                       )
{
  ...
  SOC_PPD_L2_LIF_PWE_INFO_print(&prm_pwe_info);
                                    ...
                                    )
{
  ...
  unit = soc_ppd_get_default_unit();
  SOC_PPD_L2_VLAN_RANGE_KEY_clear(&prm_vlan_range_key);
                              ...
                              )
{
  ...
  SOC_PPD_L2_LIF_AC_KEY_QUALIFIER_clear(&prm_ac_key_qualifier);
                              ...
                              )
{
  ...
  cli_out("%s\n",SOC_PPD_L2_LIF_AC_MAP_KEY_TYPE_to_string(prm_ac_map_key));
...
********************************************************************/
int
ui_ppd_t20e_api_lif_ac_map_key_set(
                              CURRENT_LINE *current_line
                              )
{
  uint32
  ...
  UI_MACROS_INIT_FUNCTION("ui_ppd_api_lif");
  soc_sand_proc_name = "soc_ppd_t20e_l2_lif_ac_map_key_set";

  unit = soc_ppd_get_default_unit();

  /* Get parameters */
  if(!search_param_val_pairs(current_line,&match_index,SOC_PARAM_PPD_T20E_L2_LIF_AC_MAP_KEY_SET_AC_MAP_KEY_SET_PORT_PROFILE_NDX_ID,1))
  {
    UI_MACROS_GET_NUMMERIC_VAL(SOC_PARAM_PPD_T20E_L2_LIF_AC_MAP_KEY_SET_AC_MAP_KEY_SET_PORT_PROFILE_NDX_ID);
    UI_MACROS_CHECK_GET_VAL_OF_ERROR;
  ...
  if(!search_param_val_pairs(current_line,&match_index,SOC_PARAM_PPD_T20E_L2_LIF_AC_MAP_KEY_SET_AC_MAP_KEY_SET_VLAN_FORMAT_NDX_ID,1))
  {
    UI_MACROS_GET_SYMBOL_VAL(SOC_PARAM_PPD_T20E_L2_LIF_AC_MAP_KEY_SET_AC_MAP_KEY_SET_VLAN_FORMAT_NDX_ID);
    UI_MACROS_CHECK_GET_VAL_OF_ERROR;
  ...
  /* This is a set function, so call GET function first*/
  ret = soc_ppd_t20e_l2_lif_ac_map_key_get(
  unit,
  prm_port_profile_ndx,
  prm_vlan_format_ndx,
  &prm_ac_map_key
  ...
  if (soc_sand_get_error_code_from_error_word(ret) != SOC_SAND_OK)
  {
  send_string_to_screen(" *** soc_ppd_t20e_l2_lif_ac_map_key_get - FAIL", TRUE);
  soc_sand_disp_result_proc(ret, "soc_ppd_t20e_l2_lif_ac_map_key_get");
  goto exit;
  } 

  if(!search_param_val_pairs(current_line,&match_index,SOC_PARAM_PPD_T20E_L2_LIF_AC_MAP_KEY_SET_AC_MAP_KEY_SET_AC_MAP_KEY_ID,1))
  {
    UI_MACROS_GET_SYMBOL_VAL(SOC_PARAM_PPD_T20E_L2_LIF_AC_MAP_KEY_SET_AC_MAP_KEY_SET_AC_MAP_KEY_ID);
    UI_MACROS_CHECK_GET_VAL_OF_ERROR;
    prm_ac_map_key = param_val->numeric_equivalent;
  }

  /* Call function */
  ret = soc_ppd_t20e_l2_lif_ac_map_key_set(
    unit,
    prm_port_profile_ndx,
    prm_vlan_format_ndx,
    prm_ac_map_key
  ...
  if (soc_sand_get_error_code_from_error_word(ret) != SOC_SAND_OK)
  {
    send_string_to_screen(" *** soc_ppd_t20e_l2_lif_ac_map_key_set - FAIL", TRUE);
    soc_sand_disp_result_proc(ret, "soc_ppd_t20e_l2_lif_ac_map_key_set");
    goto exit;
  }
...
********************************************************************/
int
ui_ppd_t20e_api_lif_ac_map_key_get(
                              CURRENT_LINE *current_line
                              )
{
  uint32
  ...
  UI_MACROS_INIT_FUNCTION("ui_ppd_api_lif");
  soc_sand_proc_name = "soc_ppd_t20e_l2_lif_ac_map_key_get";

  unit = soc_ppd_get_default_unit();

  /* Get parameters */
  if(!search_param_val_pairs(current_line,&match_index,SOC_PARAM_PPD_T20E_L2_LIF_AC_MAP_KEY_GET_AC_MAP_KEY_GET_PORT_PROFILE_NDX_ID,1))
  {
    UI_MACROS_GET_NUMMERIC_VAL(SOC_PARAM_PPD_T20E_L2_LIF_AC_MAP_KEY_GET_AC_MAP_KEY_GET_PORT_PROFILE_NDX_ID);
    UI_MACROS_CHECK_GET_VAL_OF_ERROR;
    prm_port_profile_ndx = (uint32)param_val->value.ulong_value;
  }
  else
  {
    send_string_to_screen(" *** SW error - expecting parameter port_profile_ndx after ac_map_key_get***", TRUE);
    goto exit;
  }

  if(!search_param_val_pairs(current_line,&match_index,SOC_PARAM_PPD_T20E_L2_LIF_AC_MAP_KEY_GET_AC_MAP_KEY_GET_VLAN_FORMAT_NDX_ID,1))
  {
    UI_MACROS_GET_SYMBOL_VAL(SOC_PARAM_PPD_T20E_L2_LIF_AC_MAP_KEY_GET_AC_MAP_KEY_GET_VLAN_FORMAT_NDX_ID);
    UI_MACROS_CHECK_GET_VAL_OF_ERROR;
    prm_vlan_format_ndx = param_val->numeric_equivalent;
  }
  else
  {
    send_string_to_screen(" *** SW error - expecting parameter vlan_format_ndx after ac_map_key_get***", TRUE);
    goto exit;
  }
...
{   
  ...
  unit = soc_ppd_get_default_unit(); 
  SOC_PPD_L2_LIF_AC_MP_INFO_clear(&prm_info);
...
{   
  ...
  SOC_PPD_L2_LIF_AC_MP_INFO_print(&prm_info);
                      ...
                      )
{
  ...
  SOC_PPD_L2_LIF_AC_INFO_clear(&prm_ac_info);

  prm_ac_key.inner_vid = SOC_PPD_LIF_IGNORE_INNER_VID;
  prm_ac_key.outer_vid = SOC_PPD_LIF_IGNORE_OUTER_VID;
  prm_ac_info.learn_record.learn_type = SOC_PPD_L2_LIF_AC_LEARN_INFO;
  prm_ac_info.learn_record.learn_info.type = SOC_PPD_FRWRD_DECISION_TYPE_UC_PORT;
  prm_ac_info.learn_record.learn_info.additional_info.outlif.type = SOC_PPD_OUTLIF_ENCODE_TYPE_AC;
  prm_ac_info.orientation = SOC_SAND_PP_HUB_SPOKE_ORIENTATION_SPOKE;
  prm_ac_info.default_frwrd.default_frwd_type = SOC_PPD_L2_LIF_DFLT_FRWRD_SRC_LIF;
  ...
  { 
    ...
    prm_ac_info.service_type = param_val->numeric_equivalent;
	  if(prm_ac_info.service_type == SOC_PPD_L2_LIF_AC_SERVICE_TYPE_MP)
    {
      prm_ac_info.default_frwrd.default_frwd_type = SOC_PPD_L2_LIF_DFLT_FRWRD_SRC_VSI;
    }
    else
    {
      prm_ac_info.default_frwrd.default_frwd_type = SOC_PPD_L2_LIF_DFLT_FRWRD_SRC_LIF;
  ...
  for (idx = 0; idx < nof_reps; ++idx)
  {
    ...
    prm_ac_key.outer_vid = (prm_ac_key.inner_vid == SOC_PPD_LIF_IGNORE_INNER_VID) ? prm_ac_key.outer_vid + 1 : prm_ac_key.outer_vid + (prm_ac_key.inner_vid + 1) / 4096;
    prm_ac_key.inner_vid = (prm_ac_key.inner_vid == SOC_PPD_LIF_IGNORE_INNER_VID) ? SOC_PPD_LIF_IGNORE_INNER_VID : (prm_ac_key.inner_vid + 1) % 4096 + SOC_SAND_NUM2BOOL(prm_ac_key.inner_vid + 1== 4096);
  }
                      ...
                      )
{
  ...
  SOC_PPD_L2_LIF_AC_INFO_print(&prm_ac_info);
                               ...
                               )
{
  ...
  {
    SOC_PPD_L2_LIF_AC_INFO_clear(prm_acs_group_info.acs_info + prm_acs_info_index);
    ...
    if (prm_acs_group_info.acs_info[ prm_acs_info_index].service_type == (uint32) SOC_PPD_L2_LIF_PWE_SERVICE_TYPE_MP)
    {
      ret = ui_ppd_frwrd_decision_set(
              current_line,
              1,
    ...
    {
    ret = SOC_PB_PP_L2_LIF_AC_INFO_verify(&(prm_acs_group_info.acs_info[ prm_acs_info_index]));
    if (soc_sand_get_error_code_from_error_word(ret) != SOC_SAND_OK)
    {
      send_string_to_screen(" *** SOC_PB_PP_L2_LIF_AC_INFO_verify - FAIL", TRUE);
      soc_sand_disp_result_proc(ret, "SOC_PB_PP_L2_LIF_AC_INFO_verify");
      goto exit;
    }
    }
#endif
                               ...
                               )
{
  ...
  SOC_PPD_L2_LIF_AC_GROUP_INFO_clear(&prm_acs_group_info);
                               ...
                               )
{
  ...
  SOC_PPD_L2_LIF_AC_GROUP_INFO_print(&prm_acs_group_info);
...
{   
  uint32 
    ret;   
  SOC_PPD_L2_LIF_L2CP_KEY   
    prm_l2cp_key;
  SOC_PPD_L2_LIF_L2CP_HANDLE_TYPE   
    prm_handle_type;
  ...
  unit = soc_ppd_get_default_unit(); 
  SOC_PPD_L2_LIF_L2CP_KEY_clear(&prm_l2cp_key);
...
{   
  ...
  cli_out("handle_type: %s\n\r",SOC_PPD_L2_LIF_L2CP_HANDLE_TYPE_to_string(prm_handle_type));
                        ...
                        )
{
  ...
  SOC_PPD_SYS_VSI_ID   
    prm_vsi_ndx;
  SOC_PPD_L2_LIF_ISID_KEY
    prm_isid_key;
  SOC_PPD_LIF_ID   
    prm_lif_index=0;
  SOC_PPD_L2_LIF_ISID_INFO
    prm_isid_info;
  SOC_SAND_SUCCESS_FAILURE
    prm_success;
  ...
  unit = soc_ppd_get_default_unit();
  SOC_PPD_L2_LIF_ISID_KEY_clear(&prm_isid_key);
  SOC_PPD_L2_LIF_ISID_INFO_clear(&prm_isid_info);
                        ...
                        )
{
  ...
  SOC_PPD_L2_LIF_ISID_INFO_print(&prm_isid_info);
               ...
               )
{
  ...
  else if(!search_param_val_pairs(current_line,&match_index,SOC_PARAM_PPD_T20E_L2_LIF_AC_MAP_KEY_SET_AC_MAP_KEY_SET_ID,1))
  {
    ui_ret = ui_ppd_t20e_api_lif_ac_map_key_set(current_line);
  }
  else if(!search_param_val_pairs(current_line,&match_index,SOC_PARAM_PPD_T20E_L2_LIF_AC_MAP_KEY_GET_AC_MAP_KEY_GET_ID,1))
  {
    ui_ret = ui_ppd_t20e_api_lif_ac_map_key_get(current_line);
  }
