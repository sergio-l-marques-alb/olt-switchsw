...
static sal_usecs_t prev_ibod_interval[BCM_MAX_NUM_UNITS];
#endif
...
cmd_ibod_sync(int unit, args_t *a)
{
    ...
    if (usec > 0) {
        ...
        r = _bcm_esw_ibod_sync_recovery_running(unit,
                                                &prev_interval, NULL);
        if (r < 0 || prev_interval <= 0) {
            prev_interval = soc_property_get(unit, spn_PORTMON_INTERVAL,
                                             IBOD_SYNC_INTERVAL_MIN);
        }
        prev_ibod_interval[unit] = prev_interval;
