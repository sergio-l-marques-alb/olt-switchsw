...
void bcm_dpp_state_t_SW_DUMP_PRINT(void* element, char* variableName){ 
   ...
   SW_STATE_DUMP_SAFE_STRCPY(variableNameWithPath, variableName);     
   SW_STATE_DUMP_SAFE_STRCAT(variableNameWithPath, "sw_state.dpp.bcm.qos[0]"); 
   /* check the pointer is not null */ 
   if (sw_state[0]->dpp.bcm.qos) { 
      bcm_dpp_qos_state_t_SW_DUMP_PRINT((void*) sw_state[0]->dpp.bcm.qos, variableNameWithPath); 
   } else { 
       sal_fprintf(output_file, "%s: null \n ", variableNameWithPath); 
   } 
   SW_STATE_DUMP_SAFE_STRCPY(variableNameWithPath, variableName);     
