...
#if (defined BCM_PETRAB_SUPPORT || defined BCM_DPP_SUPPORT || defined BCM_DFE_SUPPORT)
    ...
    if (memp->flags & SOC_MEM_FLAG_MULTIVIEW) {
        ...
        if (SOC_IS_TRIUMPH2(unit) || SOC_IS_APOLLO(unit) ||
            ...
            if (sal_strlen(memp->views[key_type]) == 0) {
                LOG_INFO(BSL_LS_APPL_COMMON, (BSL_META_U(unit, "Wrong Key_type %d\n"), key_type));
                return;
            }
        }
#endif
...
_dpp_mem_write(int unit, args_t *a, int mod)
{
    ...
    for (index = start; index < start + count; index++) {
        ...
        if(1 == mod)
        {
            if (wide) {
                if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
                    r = _soc_arad_mem_array_wide_access(unit, mem, array_index, copyno, index, entry, 1);
                } else {
                    r = dcmn_mem_array_wide_access(unit, mem, array_index, copyno, index, entry, 1);
                }
            } else {
                r = soc_mem_array_read(unit, mem, array_index, copyno, index, entry);
            }  
            if (r < 0) {
        ...
        if (wide) {
            if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
                r = _soc_arad_mem_array_wide_access(unit, mem, array_index, copyno, no_cache ? -index : index, entry, 0);
            } else {
                r = dcmn_mem_array_wide_access(unit, mem, array_index, copyno, no_cache ? -index : index, entry, 0);
            }
        } else {
            r = soc_mem_array_write_extended(unit, no_cache ? SOC_MEM_DONT_USE_CACHE : SOC_MEM_NO_FLAGS, mem, array_index, copyno, index, entry);
        }           
        if (r < 0) {
