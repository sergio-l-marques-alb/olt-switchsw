/*
 * $Id: dispatch.c,v 1.28 Broadcom SDK $
...
char cmd_async_usage[] = 
    ...
    "async testx: submit an async bcmx call (vlan create)\n"
;
...
cmd_async(int unit, args_t *args)
{
    ...
    if (sal_strcasecmp(subcmd, "testx") == 0) {
#ifdef  INCLUDE_BCMX
        ++ cmd_async_count;
        sal_printf("[bcmx vlan create %d]\n", cmd_async_count);
        bcmx_vlan_create_async(cmd_async_count,
                               cmd_async_cb, INT_TO_PTR(cmd_async_count));
        return CMD_OK;
#else
        sal_printf("ERROR: bcmx support not included\n");
        return CMD_FAIL;
#endif  /* INCLUDE_BCMX */
    }
    return CMD_USAGE;
...
custom_stat_get(int unit, bcm_port_t port,
                int dosync, uint64 stats[snmpValCount])
{
    int     rv, argi, s, actual, op;
    uint32  args[snmpValCount*2];

    op = dosync ? CUSTOM_HANDLER_STAT_SYNC_GET : CUSTOM_HANDLER_STAT_GET;
    rv = bcm_custom_port_get(unit, port, op, snmpValCount*2, args, &actual);
    if (rv >= 0) {
        argi = 0;
...
#if 0  /* the bcmx version */
int
customx_stat_get(bcmx_lport_t port,
                 int dosync, uint64 stats[snmpValCount])
{
    int     rv, argi, s, op, actual;
    uint32  args[snmpValCount*2];

    op = dosync ? 2 : 1;
    rv = bcmx_custom_port_get(port, op, snmpValCount*2, args, &actual);
    if (rv >= 0) {
        argi = 0;
        for (s = 0; s < snmpValCount; s++) {
            COMPILER_64_SET(stats[s], args[argi], args[argi+1]);
...
cmd_dispatch(int unit, args_t *args)
{
    ...
    if (sal_strcasecmp(subcmd, "custom_stat") == 0) {
        ...
        rv = custom_stat_get(u, p, 0, stats);
        if (rv < 0) {
            sal_printf("ERROR: %s %s: %s\n", cmd, subcmd, bcm_errmsg(rv));
            return CMD_FAIL;
        }
    ...
    if (sal_strcasecmp(subcmd, "custom_stattime") == 0) {
        ...
        btime = sal_time_usecs();
        for (i = 0; i < count; i++) {
            (void)custom_stat_get(u, p, 0, stats);
        }
        etime = sal_time_usecs();
        sal_printf("%d api calls in %u usec (%d usec/call)\n",
                   count, etime-btime, (etime-btime)/count);
