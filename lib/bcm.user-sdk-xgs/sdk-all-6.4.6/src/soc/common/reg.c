    ...
    if (opcode != (is_mem ? READ_MEMORY_ACK_MSG : READ_REGISTER_ACK_MSG) || err != 0) {
        LOG_ERROR(BSL_LS_SOC_COMMON,
            (BSL_META_U(unit,
                  ...
                  "invalid S-Channel reply, expected %s:\n"),(is_mem ? "READ_MEM_ACK" : "READ_REG_ACK") ));
        soc_schan_dump(unit, &schan_msg, 2);
        return SOC_E_INTERNAL;
    }
...
soc_direct_reg_set(int unit, int cmic_block, uint32 addr, uint32 dwc_write, uint32 *data)
{
    ...
    if (SOC_IS_DONE_INIT(unit))
    {
        ...
        {
            if(SOC_CONTROL(unit)->Hw_Log_List.Access_cb.direct_reg_set)
            {
                return SOC_CONTROL(unit)->Hw_Log_List.Access_cb.direct_reg_set(unit, cmic_block, addr, dwc_write, data);
            }
        }
    }
...
soc_reg_get(int unit, soc_reg_t reg, int port, int index, uint64 *data)
{
    ...
    if (SOC_IS_DONE_INIT(unit))
    {
        if (BCM_UNIT_DO_HW_READ_WRITE(unit))
        {
            if(SOC_CONTROL(unit)->Hw_Log_List.Access_cb.soc_reg_get)
            {
                return SOC_CONTROL(unit)->Hw_Log_List.Access_cb.soc_reg_get(unit, reg, port, index, data);
            }
        }
    }
...
soc_reg_above_64_get(int unit, soc_reg_t reg, int port, int index, soc_reg_above_64_val_t data)
{
    ...
    if (SOC_IS_DONE_INIT(unit))
    {
        if (BCM_UNIT_DO_HW_READ_WRITE(unit))
        {
            if(SOC_CONTROL(unit)->Hw_Log_List.Access_cb.reg_above64_get)
            {
                return SOC_CONTROL(unit)->Hw_Log_List.Access_cb.reg_above64_get(unit, reg, port, index, data);
            }
        }
    }
...
soc_reg32_get(int unit, soc_reg_t reg, int port, int index, uint32 *data)
{
    ...
    if (SOC_IS_DONE_INIT(unit))
    {
        if (BCM_UNIT_DO_HW_READ_WRITE(unit))
        {
            if(SOC_CONTROL(unit)->Hw_Log_List.Access_cb.reg32_get)
            {
                return SOC_CONTROL(unit)->Hw_Log_List.Access_cb.reg32_get(unit, reg, port, index, data);
            }
        }
    }
...
soc_reg_set(int unit, soc_reg_t reg, int port, int index, uint64 data)
{
    ...
    if (SOC_IS_DONE_INIT(unit))
    {
        if (BCM_UNIT_DO_HW_READ_WRITE(unit))
        {
            if(SOC_CONTROL(unit)->Hw_Log_List.Access_cb.soc_reg_set)
            {
                return SOC_CONTROL(unit)->Hw_Log_List.Access_cb.soc_reg_set(unit, reg, port, index, data);
            }
        }
    }
...
soc_reg_set_nocache(int unit, soc_reg_t reg, int port, int index, uint64 data)
{
    ...
    if (SOC_IS_DONE_INIT(unit))
    {
        if (BCM_UNIT_DO_HW_READ_WRITE(unit))
        {
            if(SOC_CONTROL(unit)->Hw_Log_List.Access_cb.soc_reg_set_nocache)
            {
                return SOC_CONTROL(unit)->Hw_Log_List.Access_cb.soc_reg_set_nocache(unit, reg, port, index, data);
            }
        }
    }
...
soc_reg_above_64_set(int unit, soc_reg_t reg, int port, int index, soc_reg_above_64_val_t data)
{
    ...
    if (SOC_IS_DONE_INIT(unit))
    {
        if (BCM_UNIT_DO_HW_READ_WRITE(unit))
        {
            if(SOC_CONTROL(unit)->Hw_Log_List.Access_cb.reg_above64_set)
            {
                return SOC_CONTROL(unit)->Hw_Log_List.Access_cb.reg_above64_set(unit, reg, port, index, data);
            }
        }
    }
...
soc_reg32_set(int unit, soc_reg_t reg, int port, int index, uint32 data)
{
    ...
    if (SOC_IS_DONE_INIT(unit))
    {
        if (BCM_UNIT_DO_HW_READ_WRITE(unit))
        {
            if(SOC_CONTROL(unit)->Hw_Log_List.Access_cb.reg32_set)
            {
                return SOC_CONTROL(unit)->Hw_Log_List.Access_cb.reg32_set(unit, reg, port, index, data);
            }
        }
    }
...
soc_reg64_set(int unit, soc_reg_t reg, int port, int index, uint64 data)
{
    ...
    if (SOC_IS_DONE_INIT(unit))
    {
        if (BCM_UNIT_DO_HW_READ_WRITE(unit))
        {
            if(SOC_CONTROL(unit)->Hw_Log_List.Access_cb.reg64_set)
            {
                return SOC_CONTROL(unit)->Hw_Log_List.Access_cb.reg64_set(unit, reg, port, index, data);
            }
        }
    }
...
#define SOC_REG_ADDR_INVALID_PORT 0 /* for asserts */
    ...
    *blk = 0;/* not really needed, just to avoid coverity defect */
        ...
        if (SOC_BLOCK_IN_LIST(regblktype, portblktype)) {
            ...
            for (i = 0; i < port_num_blktype; i++) {
                ...
                if(SOC_IS_KATANA2(unit) &&
                    ...
                    (REG_FIRST_BLK_TYPE(regblktype) == SOC_BLK_IECELL)) {
                    soc_sb2_iecell_port_reg_blk_idx_get(unit, port,
                        SOC_BLK_IECELL, &block, &pindex);
                    break;
#endif
...
#if defined (BCM_SABER2_SUPPORT)
    if (SOC_IS_SABER2(unit)) {
        if ((SOC_BLK_IECELL == REG_FIRST_BLK_TYPE(regblktype)) &&
            (soc_portreg != SOC_REG_INFO(unit, reg).regtype)) {
            instance_mask = 1;
        }
    }
    ...
    if ((!block_id_mask) && (!schan_id_mask) && (REG_PORT_ANY == port || instance_mask || !SOC_BLOCK_IN_LIST(regblktype, portblktype))) {
        int blkport = port;
        ...
        case SOC_BLK_IMP:
            ...
            break;
        case SOC_BLK_SPB:
            block = SPB_BLOCK(unit);
            break;
        case SOC_BLK_ITE:
            block = ITE_BLOCK(unit);
            break;
        case SOC_BLK_DDP:
            block = DDP_BLOCK(unit);
            break;
        case SOC_BLK_TXQ:
            block = TXQ_BLOCK(unit);
            break;
        case SOC_BLK_TAR:
            block = TAR_BLOCK(unit);
            break;
        case SOC_BLK_PTS:
            block = PTS_BLOCK(unit);
            break;
        case SOC_BLK_SQM:
            block = SQM_BLOCK(unit);
            break;
        case SOC_BLK_IPSEC:
            block = IPSEC_BLOCK(unit);
            break;
        case SOC_BLK_IPSEC_SPU_WRAPPER_TOP:
            block = IPSEC_SPU_WRAPPER_TOP_BLOCK(unit,blkport);
            break;
        case SOC_BLK_DQM:
            block = DQM_BLOCK(unit);
            break;
        case SOC_BLK_ECGM:
            block = ECGM_BLOCK(unit);
            break;
        case SOC_BLK_GCK:
            block = GCK_BLOCK(unit);
            break;
        case SOC_BLK_IDB:
            block = IDB_BLOCK(unit);
            break;
        case SOC_BLK_PEM:
            block = PEM_BLOCK(unit);
            break;
        ...
        case SOC_BLK_MTRPS_EM:
            block = MRTPS_EM_BLOCK(unit, blkport);
            break;
        case SOC_BLK_NBIL:
...
soc_reg_port_valid(int unit, soc_reg_t reg, soc_port_t port) {
    if (SOC_BLOCK_IN_LIST(SOC_REG_INFO(unit, reg).block, SOC_BLK_XLPORT) &&
        SOC_BLOCK_IN_LIST(SOC_REG_INFO(unit, reg).block, SOC_BLK_MXQPORT)) {
