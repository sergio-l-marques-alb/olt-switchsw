...
#endif
    } else if (SOC_IS_ARADPLUS_A0(unit)&& (ch == SOC_MEM_FIFO_DMA_CHANNEL_3))  {
#ifdef BCM_DPP_SUPPORT
        uint8 oam_is_init=0;
        uint8 bfd_is_init=0;

        arad_pp_sw_db_oam_enable_get(unit, &oam_is_init); /* Return value ignored*/
        arad_pp_sw_db_bfd_enable_get(unit, &bfd_is_init); /* Return value ignored*/
