...
_sb2_cosq_counter_mem_map_t _sb2_cosq_mem_map[] =
{
    { CTR_FLEX_COUNT_0m, SOC_COUNTER_NON_DMA_COSQ_DROP_PKT },
    { CTR_FLEX_COUNT_1m, SOC_COUNTER_NON_DMA_COSQ_DROP_PKT },
    { CTR_FLEX_COUNT_2m, SOC_COUNTER_NON_DMA_COSQ_DROP_PKT_RED },
    { CTR_FLEX_COUNT_3m, SOC_COUNTER_NON_DMA_COSQ_DROP_PKT_RED },
    { CTR_FLEX_COUNT_4m, SOC_COUNTER_NON_DMA_EGR_PERQ_XMT_PKT },
                                 ...
                                 int *non_dma_entries)
{
    ...
    *non_dma_entries = 0;
    ...
    entry_words = soc_mem_entry_words(unit, CTR_FLEX_COUNT_0m);
    table_size = (soc_mem_index_count(unit, CTR_FLEX_COUNT_0m) * 2 *
                 entry_words * sizeof(uint32));
    buf = soc_cm_salloc(unit, table_size, "non_dma_counter");
    if (buf == NULL) {
        return SOC_E_MEMORY;
    ...
    for (id = 0; id < 6; id = id + 2) {
        ...
        non_dma0->entries_per_port = 8;
        non_dma0->mem = mem_map[id].mem;
        non_dma0->num_entries = 2 * soc_mem_index_count(unit, non_dma0->mem);
        non_dma0->reg = INVALIDr;
        non_dma0->field = COUNTf;
        non_dma0->cname = cname_pkt[id / 2];
        ...
        *non_dma_entries += non_dma0->num_entries;
        ...
        non_dma1->base_index = non_dma_start_index + *non_dma_entries;
        non_dma1->field = BYTE_COUNTf;
        non_dma1->cname = cname_byte[id / 2];
        *non_dma_entries += non_dma1->num_entries;
    }
    return SOC_E_NONE;
}
...
#ifdef BCM_CMICM_SUPPORT
        if (soc_feature(unit, soc_feature_cmicm) && SOC_IS_TRIDENT2X(unit)) {
            int i, skip_count = 0;
            ...
            PBMP_ITER(counter_pbmp, port) {
                ...
                if (ctr_reg >= NUM_SOC_REG) {
                    ...
                    if (non_dma->entries_per_port == 0) {
                        ...
                        if ((non_dma->id == SOC_COUNTER_NON_DMA_POOL_PEAK) ||
                ...
                for (i = 0; i < count; i++) {
                    ...
                    if (mmu_xpe_ctr == 0) {
                        ...
                        rv = soc_reg_get(unit, non_dma->reg, (i / NUM_XPE(unit)),
                                         (i % NUM_XPE(unit)), &ctr_new);
                    }
                    if (SOC_FAILURE(rv)) {
                        return;
                    }
