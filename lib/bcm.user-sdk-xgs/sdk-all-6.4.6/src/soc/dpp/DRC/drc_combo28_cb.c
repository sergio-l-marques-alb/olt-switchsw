...
int soc_dpp_drc_combo28_shmoo_vendor_info_get(int unit, int drc_ndx, combo28_vendor_info_t *info)
{
    ...
    info->manufacture_id  = SOC_SAND_GET_BITS_RANGE(reg_above64_val[0], 3, 0);
    info->revision_id     = SOC_SAND_GET_BITS_RANGE(reg_above64_val[0], 7, 4);
    info->dram_density    = SOC_SAND_GET_BITS_RANGE(reg_above64_val[0], 17, 16);
    info->fifo_depth      = SOC_SAND_GET_BITS_RANGE(reg_above64_val[0], 19, 18);
...
int soc_dpp_drc_combo28_shmoo_force_dqs(int unit, int drc_ndx , uint32 force_dqs_val, uint32 force_dqs_oeb)
{
    ...
    /* In SAMAR the only option was to control all 4 bytes toghether, here there is an option to control each one seperatly */
    /* but there must be a decision on the matter first, for now, forces the callback to control all 4 bytes together       */
    force_dqs_val = force_dqs_val ? 0xf : 0;
    force_dqs_oeb = force_dqs_oeb ? 0xf : 0;

    /* Setting of FORCE_DQS_VALf, DQS_OEB_VALf must come before setting of OVERRIDE_DQS_OEBf */
