...
int soc_jer_specific_info_config_direct(int unit)
{
    ...
    SOC_DPP_CONFIG(unit)->jer->nif.ilkn_over_fabric = soc_property_get(unit, "ilkn_over_fabric" , 0);
...
#endif
  {
      uint32 res = soc_property_get(unit, spn_BIST_ENABLE, 0);
      ...
      if (res) {
          ...
          SOCDNX_SAND_IF_ERR_EXIT(soc_bist_all_jer(unit, (res == 2 ? 1 : 0)));
      }
  }
    ...
    if (!SOC_WARM_BOOT(unit)) 
    {
        ...
        SOCDNX_IF_ERR_EXIT(soc_jer_ser_init(unit));
    }
