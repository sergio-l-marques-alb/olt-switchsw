   ...
   )
{
    ...
    JER_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHP_ID,30,2,155,0,5);
    ...
    JER_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHP_ID,30,1,255,252,5);
    rv = soc_sand_bitstream_get_any_field(regs_val, 0, 3, tmp_buffer);
    SOCDNX_SAND_IF_ERR_EXIT(rv);
    SHR_BITCOPY_RANGE(search_keys[0], 0, tmp_buffer, 0, 4);


    JER_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHP_ID,30,1,77,0,5);
