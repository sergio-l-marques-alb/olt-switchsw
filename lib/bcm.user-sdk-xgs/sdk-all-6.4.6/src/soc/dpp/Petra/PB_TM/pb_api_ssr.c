/* $Id: soc_pb_api_ssr.c,v 1.5 Broadcom SDK $
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(SOC_PB_SSR_BUFF_SIZE_GET);

  SOC_SAND_CHECK_NULL_INPUT(byte_size_ptr);

  res = soc_pb_ssr_PB_SW_DB_SSR_additional_buff_size_get(
      unit,
      byte_size_ptr
    );
  SOC_SAND_CHECK_FUNC_RESULT(res, 100, exit);

  *byte_size_ptr += sizeof(SOC_PB_SSR_BUFF);

exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("soc_pb_ssr_get_buff_size", 0, 0);
}
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in soc_pb_ssr_to_buff()",0,0);
}
  ...
  )
{
  ...
  res = soc_pb_ssr_from_buff_unsafe(
          unit,
          store_buff,
          buff_byte_size
        );
...
exit_semaphore:
  SOC_SAND_GIVE_DEVICE_SEMAPHORE;
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in soc_pb_ssr_from_buff()",0,0);
}
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(SOC_PB_SSR_IS_DEVICE_INIT_DONE);
  SOC_SAND_CHECK_DRIVER_AND_DEVICE;
  ...
  res = soc_pb_ssr_is_device_init_done_unsafe(
          unit,
          is_init_done
        );
...
exit_semaphore:
  SOC_SAND_GIVE_DEVICE_SEMAPHORE;
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in soc_pb_ssr_is_device_init_done()",0,0);
}
/* } */
