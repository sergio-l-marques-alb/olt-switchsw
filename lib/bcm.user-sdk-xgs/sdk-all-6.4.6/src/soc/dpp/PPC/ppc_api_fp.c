  ...
  )
{
  ...
  sal_memset(info, 0x0, sizeof(SOC_TMC_PMF_PFG_INFO));
  ...
  )
{
  ...
  SOC_TMC_PMF_PFG_INFO_clear(&(info->pfg_info));
  ...
  )
{
  ...
  info->type = SOC_PPC_NOF_FP_QUAL_TYPES;
  SOC_SAND_MAGIC_NUM_SET;
exit:
  SOC_SAND_VOID_EXIT_AND_SEND_ERROR(0, 0, 0);
}
  ...
  )
{
  ...
  {
    info->qual_types[ind] = SOC_PPC_NOF_FP_QUAL_TYPES;
  }
  ...
  )
{
  ...
  for (ind = 0; ind < SOC_PPC_FP_NOF_INGRESS_FLP_ACTIONS; ++ind)
  {
...
CONST static char*
  ...
  case SOC_PPC_FP_QUAL_FORWARDING_OFFSET_EXTENSION:
    str = "forwarding_offset_extension";
    break;
  case SOC_PPC_FP_QUAL_FORWARDING_HEADER_ENCAPSULATION:
    str = "forwarding_header_encapsulation";
    break;
  ...
  )
{
  ...
  LOG_CLI((BSL_META_U(unit,
                      "PFG info: ")));
  SOC_TMC_PMF_PFG_INFO_print(&(info->pfg_info));
  ...
  )
{
  ...
  for (ind = 0; ind < SOC_PPC_FP_NOF_QUALS_PER_DB_MAX; ++ind)
  {
    if (info->qual_types[ind] != SOC_PPC_NOF_FP_QUAL_TYPES)
    {
      LOG_CLI((BSL_META_U(unit,
  ...
  )
{
  ...
  if (info->type != SOC_PPC_NOF_FP_QUAL_TYPES)
  {
    LOG_CLI((BSL_META_U(unit,
                        "type: %s, "),SOC_PPC_FP_QUAL_TYPE_to_string(info->type)));
    LOG_CLI((BSL_META_U(unit,
  ...
  )
{
  ...
  {
    if (info->qual[ind2].type == SOC_PPC_NOF_FP_QUAL_TYPES)
    {
      continue;
    }
    is_to_print = TRUE;
