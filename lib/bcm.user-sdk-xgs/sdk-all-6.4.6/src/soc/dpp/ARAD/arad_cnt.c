  ...
  for (cnt_proc_ndx = 0; cnt_proc_ndx < ARAD_CNT_NOF_PROCESSOR_IDS(unit); cnt_proc_ndx++) {
      ...
      if (SOC_IS_ARADPLUS(unit)) {
          /* QPTS - "active counters" counter isn't accurate when using "Qsize" */ 
           SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res, 1241 + cnt_proc_ndx, exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, CRPS_CRPS_GENERAL_CFGr, REG_PORT_ANY,  cnt_proc_ndx, CRPS_N_ACT_CNT_VALIDATE_ENf, 0x1));
      }
  }
       ...
       )
{
    ...
    if (lif_stack_to_count < 0 || lif_stack_to_count > 3) {
    ...
    for (stack_level = 0; stack_level < ((source == SOC_TMC_CNT_SRC_TYPE_ING_PP) ? 4 : 3); stack_level++) {
        counting_mask[stack_level] = SOC_TMC_CNT_LIF_COUNTING_MASK_FIELD_VAL_GET(lif_counting_mask, stack_level);
    ...
    if (source == SOC_TMC_CNT_SRC_TYPE_ING_PP) {
        ...
        rv = READ_IHP_DEFAULT_COUNTER_SOURCEm(unit, IHP_BLOCK(unit, SOC_CORE_ALL), (index >> 3) ,data);
        ...
        SHR_BITCOPY_RANGE(&counter_sources, ((index & 0x7) * 4) + (command_id * 2), (uint32*)&lif_stack_to_count, 0, 2);

        soc_IHP_DEFAULT_COUNTER_SOURCEm_field32_set(unit, data, COUNTER_SOURCESf, counter_sources);

        rv = WRITE_IHP_DEFAULT_COUNTER_SOURCEm(unit, IHP_BLOCK(unit, SOC_CORE_ALL), (index >> 3) ,data);
        SOCDNX_IF_ERR_EXIT(rv);
        ...
        if (command_id == 0) {
            soc_EPNI_COUNTER_SOURCE_MAPm_field32_set(unit, data, COUNTER_0_SOURCEf, lif_stack_to_count);
        } else if (command_id == 1) {
            soc_EPNI_COUNTER_SOURCE_MAPm_field32_set(unit, data, COUNTER_1_SOURCEf, lif_stack_to_count);
        }
...
{   
    ...
    if (source == SOC_TMC_CNT_SRC_TYPE_ING_PP) {
        ...
        if (command_id == 0) {
            *lif_stack_to_count = (int)soc_IHP_DEFAULT_COUNTER_SOURCEm_field32_get(unit, &counter_sources, COUNTER_0_SOURCEf);
        } else if (command_id == 1) {
            *lif_stack_to_count = (int)soc_IHP_DEFAULT_COUNTER_SOURCEm_field32_get(unit, &counter_sources, COUNTER_1_SOURCEf);
        }
    } else if (source == SOC_TMC_CNT_SRC_TYPE_EPNI) {
        rv = READ_EPNI_PP_COUNTER_TABLEm(unit, EPNI_BLOCK(unit, SOC_CORE_ALL), 0 /*pp_port_ndx are configured symmetrically*/, data);
        SOCDNX_IF_ERR_EXIT(rv);
        ...
        if (command_id == 0) {
            *lif_stack_to_count = (int)soc_EPNI_COUNTER_SOURCE_MAPm_field32_get(unit, data, COUNTER_0_SOURCEf);
        } else if (command_id == 1) {
            *lif_stack_to_count = (int)soc_EPNI_COUNTER_SOURCE_MAPm_field32_get(unit, data, COUNTER_1_SOURCEf);
        }
    } else {
        LOG_ERROR(BSL_LS_SOC_STAT,(BSL_META_U(unit, "Invalid source type %d.\n"), source));
        SOCDNX_IF_ERR_EXIT(SOC_E_PARAM);
...
{   
    ...
    if (source == SOC_TMC_CNT_SRC_TYPE_ING_PP) {
        ...
        if (range_id == 0) {
             ...
             * IHP_COUNTER_IN_LIF_RANGE_0 is IHP_REG_01CD 
             *  COUNTER_IN_LIF_RANGE_0_LOW is FIELD_0_15
             *  COUNTER_IN_LIF_RANGE_0_HIGH is FIELD_16_31
             *  
             */
            if (range->start & ~((1 << soc_reg_field_length(unit, IHP_REG_01CDr, FIELD_0_15f)) - 1)) {
                SOCDNX_IF_ERR_EXIT(SOC_E_PARAM);
            }
            if (range->end & ~((1 << soc_reg_field_length(unit, IHP_REG_01CDr, FIELD_16_31f)) - 1)) {
                SOCDNX_IF_ERR_EXIT(SOC_E_PARAM);
            }            
            rv = READ_IHP_REG_01CDr(unit, &data32);
            SOCDNX_IF_ERR_EXIT(rv);

            soc_reg_field_set(unit, IHP_REG_01CDr, &data32, FIELD_0_15f,  range->start); 
            soc_reg_field_set(unit, IHP_REG_01CDr, &data32, FIELD_16_31f, range->end);

            rv = WRITE_IHP_REG_01CDr(unit, data32);
            SOCDNX_IF_ERR_EXIT(rv);
        } else if (range_id == 1) {
            /* 
             * in Arad Devices (not in Jericho) 
             * IHP_COUNTER_IN_LIF_RANGE_1 is IHP_REG_01CE
             *  COUNTER_IN_LIF_RANGE_1_LOW is FIELD_0_15
             *  COUNTER_IN_LIF_RANGE_1_HIGH is FIELD_16_31
             */
            if (range->start & ~((1 << soc_reg_field_length(unit, IHP_REG_01CEr, FIELD_0_15f)) - 1)) {
                SOCDNX_IF_ERR_EXIT(SOC_E_PARAM);
            }
            if (range->end & ~((1 << soc_reg_field_length(unit, IHP_REG_01CEr, FIELD_16_31f)) - 1)) {
                SOCDNX_IF_ERR_EXIT(SOC_E_PARAM);
            }            
            rv = READ_IHP_REG_01CEr(unit, &data32);
            SOCDNX_IF_ERR_EXIT(rv);

            soc_reg_field_set(unit, IHP_REG_01CEr, &data32, FIELD_0_15f,  range->start); 
            soc_reg_field_set(unit, IHP_REG_01CEr, &data32, FIELD_16_31f, range->end);

            rv = WRITE_IHP_REG_01CEr(unit, data32);
            SOCDNX_IF_ERR_EXIT(rv);
        }
    } else if (source == SOC_TMC_CNT_SRC_TYPE_EPNI) {
        uint64 data64;
...
{   
    ...
    if (source == SOC_TMC_CNT_SRC_TYPE_ING_PP) {
        ...
        if (range_id == 0) {
            ...
            range->start = soc_reg_field_get(unit, IHP_REG_01CDr, data32, FIELD_0_15f); 
            range->end =   soc_reg_field_get(unit, IHP_REG_01CDr, data32, FIELD_16_31f);
        } else if (range_id == 1) {
            /* 
             * in Arad Devices (not in Jericho) 
             * IHP_COUNTER_IN_LIF_RANGE_1 is IHP_REG_01CE 
            ...
            range->start = soc_reg_field_get(unit, IHP_REG_01CEr, data32, FIELD_0_15f); 
            range->end =   soc_reg_field_get(unit, IHP_REG_01CEr, data32, FIELD_16_31f);
        }
    } else if (source == SOC_TMC_CNT_SRC_TYPE_EPNI) {
        uint64 data64;
        COMPILER_64_ZERO(data64);
