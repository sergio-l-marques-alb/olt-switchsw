   ...
   )
{
    ...
    if (SOC_IS_ARADPLUS(unit) && (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "ext_rpf_fwd_parallel", 0) == 0)){
        opcode_info->rx_data_size += (search_id == 0) ? 6 : ((search_id == 1) ? 4 : ((search_id == 2) ? 2 : 3)); 
    }else{
    ...
    )
{
    ...
    if (search_id == 0) {
        kbp_table_config.tbl_asso_width = NLM_TBL_ADLEN_64B; /* if search ID is 0 it means we have 48bit for asso data in the ROP. */
    }else {
    ...
    int ilkn_rate)
{
   ...
   LOG_VERBOSE(BSL_LS_SOC_TCAM,
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 10, exit, READ_IHP_FLP_GENERAL_CFGr_REG64(unit, 0, &reg64_val));
    /* ***** Enables lookups to ELK ***** */
    fld_val = 0x1;
    soc_reg64_field_set(unit, IHP_FLP_GENERAL_CFGr, &reg64_val, ELK_ENABLEf, fld_val);
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 20, exit, WRITE_IHP_FLP_GENERAL_CFGr_REG64(unit, SOC_CORE_ALL, reg64_val));
