...
#define ARAD_PP_LIF_AC_P2P_TO_PWE_OUTLIF_VALID_JER               (1<<18)
#define ARAD_PP_LIF_AC_P2P_TO_PWE_OUTLIF_VALID_ARAD              (1<<16)
...
#define ARAD_PP_LIF_PROTECTION_POINTER_NOF_BITS_JER              (14)
#define ARAD_PP_LIF_PROTECTION_POINTER_MASK_JER                  (SOC_SAND_BITS_MASK((ARAD_PP_LIF_PROTECTION_POINTER_NOF_BITS_JER - 1), 0))
    ...
    uint32                 *sem_result) 
{
    ...
    if (SOC_IS_JERICHO(unit)) {
        if (actual_ext == 0) {
            ...
            rv = SOC_DPP_WB_ENGINE_GET_ARR(unit, SOC_DPP_WB_ENGINE_VAR_JER_LOCAL_INLIF_DB, &inlif_info, base_lif_id);
    ...
    SOC_SAND_IN  uint32                 sem_result) 
{
    ...
    if (SOC_IS_JERICHO(unit)) {
         ...
         res = SOC_DPP_WB_ENGINE_GET_ARR(unit, SOC_DPP_WB_ENGINE_VAR_JER_LOCAL_INLIF_DB, &inlif_info, sem_result);
         if (res == SOC_E_NONE ) 
         {
             return sem_result;
         }
         else if (res == SOC_E_NOT_FOUND )  
         {
             return sem_result+1;
         }
         goto exit;
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_pp_sem_result_to_lif_index()", 0, 0);
}
        ...
        SOC_SAND_OUT uint8 *is_wide_entry)
{
    ...
    *is_wide_entry = FALSE;
    ...
    if (is_ingress) {
        ...
        rv = SOC_DPP_WB_ENGINE_GET_ARR(unit, SOC_DPP_WB_ENGINE_VAR_JER_LOCAL_INLIF_DB, &inlif_info, lif_id);
        SOCDNX_IF_ERR_EXIT(rv);
        ...
        *is_wide_entry = inlif_info.local_lif_flags & BCM_DPP_AM_IN_LIF_FLAG_WIDE ; 
        ...
        rv = SOC_DPP_WB_ENGINE_GET_ARR(unit, SOC_DPP_WB_ENGINE_VAR_JER_LOCAL_OUTLIF_DB, &outlif_info, lif_id);
        SOCDNX_IF_ERR_EXIT(rv);
        ...
        SOC_SAND_OUT uint32 *hw_protection_path)
{
    ...
    *hw_protection_path = protection_pass_value;
    ...
    if (SOC_IS_JERICHO(unit)) {
        ...
        *hw_protection_pointer &= ARAD_PP_LIF_PROTECTION_POINTER_MASK_JER;
        if (!(SOC_DPP_IS_PROTECTION_INGRESS_COUPLED(unit))) {
            *hw_protection_path = protection_pointer >> ARAD_PP_LIF_PROTECTION_POINTER_NOF_BITS_JER;
        }
    }
}
        ...
        SOC_SAND_OUT uint8  *protection_pass_value)
{
    ...
    *protection_pass_value = hw_protection_path;
    ...
    if (SOC_IS_JERICHO(unit)) {
        ...
        if (!(SOC_DPP_IS_PROTECTION_INGRESS_COUPLED(unit))) {
            *protection_pointer |= hw_protection_path << ARAD_PP_LIF_PROTECTION_POINTER_NOF_BITS_JER;
        ...
        SOC_SAND_IN  int unit)
{
    if (SOC_IS_JERICHO(unit) && SOC_DPP_IS_PROTECTION_INGRESS_COUPLED(unit)) {
        return ARAD_PP_LIF_PROTECTION_POINTER_MASK_JER;
    }
  ...
  */
  if (*success == SOC_SAND_SUCCESS || *success == SOC_SAND_FAILURE_OUT_OF_RESOURCES_2)
  {
    if (pwe_info->service_type == ARAD_PP_L2_LIF_PWE_SERVICE_TYPE_P2P)
    {
      ...
      arad_pp_l2_lif_convert_protection_fields_to_hw(unit, pwe_info->protection_pointer, pwe_info->protection_pass_value,
    ...
    if (data.type == 0x0) /* pwe_p2p */
      {
        ...
        arad_pp_l2_lif_convert_protection_fields_from_hw(unit, data.protection_pointer, data.protection_path,
                                                         &(pwe_info->protection_pointer), &(pwe_info->protection_pass_value));
    ...
    if (ac_info->service_type == ARAD_PP_L2_LIF_AC_SERVICE_TYPE_AC2PWE)
    {
      ...
      if(ac_info->default_frwrd.default_forwarding.additional_info.eei.type != SOC_PPC_EEI_TYPE_EMPTY) 
      {
          ...
          if (ac_info->default_frwrd.default_forwarding.additional_info.outlif.type != ARAD_PP_OUTLIF_ENCODE_TYPE_NONE){
              ...
              if (SOC_IS_JERICHO(unit)) {
                  temp |= ARAD_PP_LIF_AC_P2P_TO_PWE_OUTLIF_VALID_JER;
              } else {
                  temp |= ARAD_PP_LIF_AC_P2P_TO_PWE_OUTLIF_VALID_ARAD;
              }
          } else {
  ...
  )
{
  ...
  SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_pp_l2_lif_extender_get_internal_unsafe()", 0, 0);
}
   ...
   )
{
  ...
  if (SOC_IS_JERICHO(unit))
  {
    if (is_ingress) {
        ...
        /*get the local inlif info into SW database*/
        res = SOC_DPP_WB_ENGINE_GET_ARR(unit, SOC_DPP_WB_ENGINE_VAR_JER_LOCAL_INLIF_DB, &inlif_info, lif_id);
    ...
    if (half_offset) 
    {
        tbl_data.lif_table_entry[2] = COMPILER_64_LO(data);
        tbl_data.lif_table_entry[3] = COMPILER_64_HI(data);
    }
    else
    {
        tbl_data.lif_table_entry[0] = COMPILER_64_LO(data);
        tbl_data.lif_table_entry[1] = COMPILER_64_HI(data);
    }
    ...
    else
    {
       ...
       /*get the local inlif info into SW database*/
        res = SOC_DPP_WB_ENGINE_GET_ARR(unit, SOC_DPP_WB_ENGINE_VAR_JER_LOCAL_OUTLIF_DB, &outlif_info, lif_id);
        SOCDNX_IF_ERR_EXIT(res);

      /*the 1/4 entry which need to be set, based on the lif id.*/
        ext_offset =  ( lif_id % ARAD_PP_EG_ENCAP_NOF_TOP_BANKS_PER_EEDB_BANK ) * 20;

        arad_pp_eg_enacp_access_additional_data_entry_get(unit,outlif_info.ext_lif_id,tbl_data);

        data_to_set = ( COMPILER_64_LO(data) & 0xFFFFF);

        SHR_BITCOPY_RANGE(tbl_data, ext_offset, &data_to_set, 0, 20);

        arad_pp_eg_enacp_access_additional_data_entry_set(unit,outlif_info.ext_lif_id,tbl_data);
    }
   ...
   )
{
  ...
  if (SOC_IS_JERICHO(unit))
  {
      if (is_ingress) {
          ...
          if (half_offset) 
          {
              COMPILER_64_SET(*data ,  tbl_data.lif_table_entry[3],  tbl_data.lif_table_entry[2]); 
          }
          else
          {
              COMPILER_64_SET(*data ,  tbl_data.lif_table_entry[1],  tbl_data.lif_table_entry[0]); 
          }
      }
      else
      {
