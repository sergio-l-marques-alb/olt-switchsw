  ...
  )
{
  ...
  ARAD_PP_EGQ_ACTION_PROFILE_TABLE_TBL_DATA
    tbl_data;
  uint32 
    base_q_pair, reg_val, cpu_port_tm;
  int
    core;
  ...
  if (profile_info->out_tm_port == ARAD_PP_TRAP_ACTION_PKT_DISCARD_ID)
  {
      /*Get invalid traffic port*/
      res = READ_EGQ_INVALID_OTMr(unit,0 , &reg_val);
      SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
      tbl_data.out_tm_port = soc_reg_field_get(unit, EGQ_INVALID_OTMr, reg_val, INVALID_OTMf);
  }
  else if (profile_info->out_tm_port == ARAD_PP_TRAP_ACTION_PKT_CPU_ID)
  {
      res = arad_pp_trap_get_first_cpu_port(unit,&cpu_port_tm, &core);
      SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
      res = soc_port_sw_db_tm_port_to_base_q_pair_get(unit, core, cpu_port_tm, &base_q_pair);
      SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
      tbl_data.out_tm_port = base_q_pair;
  }
  else
  {
      res = soc_port_sw_db_tm_port_to_base_q_pair_get(unit,0 ,profile_info->out_tm_port, &base_q_pair);
      SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
      tbl_data.out_tm_port = base_q_pair;
  }
  ...
  )
{
  ...
  {
    /*get the tm port from the queue*/
     res = arad_pp_trap_base_q_pair_to_port(unit,0 , tbl_data.out_tm_port, out_tm_port);
     SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
     profile_info->out_tm_port = *out_tm_port;
  }
  ...
  )
{
  ...
  if (info->out_tm_port != ARAD_PP_TRAP_ACTION_PKT_DISCARD_ID && info->out_tm_port != ARAD_PP_TRAP_ACTION_PKT_CPU_ID)
  {
...
/* get the first cpu port the core of that port*/
int
arad_pp_trap_get_first_cpu_port(/*in */ SOC_SAND_IN int unit, /*out*/ SOC_SAND_OUT  uint32 *cpu_port_tm, SOC_SAND_OUT  int *core )
{
    uint32 port_i;
    SOC_PBMP_ITER(SOC_INFO(unit).cmic_bitmap, port_i)
    {
        soc_port_sw_db_local_to_tm_port_get(unit, port_i, cpu_port_tm , core);
        return BCM_E_NONE;
    }
    return BCM_E_FAIL;
}
