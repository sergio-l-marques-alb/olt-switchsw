...
soc_error_t
arad_pp_extender_untag_fiter_set(int unit, int vlan_domain){
    int rv;
    SOC_DPP_DBAL_SW_TABLE_IDS  dbal_tables_id[ARAD_PP_ISEM_ACCESS_NOF_TABLES] = {0};
    ...
    rv = arad_pp_dbal_vtt_sw_db_get(unit, &sem_key, &nof_tables, dbal_tables_id);
    SOCDNX_IF_ERR_EXIT(rv);

    qual_vals[0].type = SOC_PPC_FP_QUAL_VLAN_DOMAIN;
    qual_vals[0].val.arr[0] = vlan_domain;

    qual_vals[1].type = SOC_PPC_FP_QUAL_PORT_EXTENDER_ETAG;
