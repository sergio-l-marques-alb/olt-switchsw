 ...
 */
#define GET_SIGNAL_FOR_DIRECTION(fld, core_id)                                   \
...
#define GET_SIGNAL_FOR_OAM_ID_UP(fld, core_id) \
      if (SOC_IS_JERICHO(unit)) {                                       \
...
#define GET_SIGNAL_FOR_OAM_ID_DOWN(fld, core_id) \
      if (SOC_IS_JERICHO(unit)) {                                           \
...
static dma_event_handler_cb_t dma_event_handler_cb[SOC_MAX_NUM_DEVICES]; /* No need for warmboot. */
  ...
  if (!is_second_init) {
	  ...
	  for (opcode = 0; opcode < SOC_PPD_OAM_ETHERNET_PDU_OPCODE_COUNT;  opcode++) {
		  ...
		  case SOC_PPD_OAM_ETHERNET_PDU_OPCODE_SLR:
			  ...
			  break;
          case SOC_PPD_BFD_PDU_OPCODE:
              internal_opcode = SOC_PPD_OAM_OPCODE_MAP_BFD;
              break;
          default:
	  ...
	  soc_reg_field_set(unit, IHP_OAM_COUNTER_NOT_IN_RANGE_CFGr, &reg, OAM_COUNTER_NOT_IN_RANGE_CFG_ADD_PCPf, 0);
                      ...
                      for (opcode = 1; opcode < SOC_PPD_OAM_ETHERNET_PDU_OPCODE_COUNT;  opcode++) {
                          ...
                          *meter_disable_lcl = profile_data->mip_profile_data.meter_disable;
                          ...
                          if (is_my_cfm_mac) {
                              ...
                              oam_payload.mirror_profile = cpu_trap_code_to_mirror_profile_map[profile_data->mep_profile_data.opcode_to_trap_code_unicast_map[internal_opcode]];
                          }
                          else {
                              _ARAD_PP_OAM_TRAP_CODE_TO_INTERNAL(unit,profile_data->mep_profile_data.opcode_to_trap_code_multicast_map[internal_opcode], oam_payload.cpu_trap_code);
                              oam_payload.mirror_profile = cpu_trap_code_to_mirror_profile_map[profile_data->mep_profile_data.opcode_to_trap_code_multicast_map[internal_opcode]];
                          }
                          res = arad_pp_oam_classifier_oam1_entry_set_on_buffer(unit, &oam1_key, &oam_payload, &oama_buffer);
                          SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
                      }
  ...
  )
{
                      ...
                      if (SOC_PPD_OAM_IS_CLASSIFIER_ARAD_MODE(unit) || (!profile_data->mp_type_passive_active_mix) || SOC_PPD_OAM_IS_CLASSIFIER_JERICHO_MODE(unit)) { /* MIP */
                          ...
                          *meter_disable_lcl = profile_data->mip_profile_data.meter_disable;
                          ...
                          if (SOC_IS_JERICHO(unit) && (oam_trap_code==SOC_PPC_TRAP_CODE_OAM_CPU_SNOOP)) {
                              ...
                              oam_payload.mirror_profile = cpu_trap_code_to_mirror_profile_map[SOC_PPC_TRAP_CODE_OAM_CPU_SNOOP_UP];
                          }
                          res = soc_ppd_trap_snoop_profile_info_get(unit, oam_trap_code, &snoop_profile_info);
                          SOC_SAND_CHECK_FUNC_RESULT(res, 52, exit);
  ...
  /* "Inject" entries: Only relevant for UpMep Ethernet.
        Active/passive entries handled together.*/
  if (is_upmep && !is_passive) {
      int mep_prof;
      oam1_key.inject = 1;
      oam1_key.is_bfd = SOC_PPD_OAM_IS_MEP_TYPE_BFD(classifier_mep_entry->mep_type); 
              ...
              if (SOC_PPD_OAM_IS_CLASSIFIER_ARAD_MODE(unit)) {
                  ...
                  for (cur_mp_type = 0; cur_mp_type < sizeof(possible_mp_types) / sizeof(SOC_PPC_OAM_MP_TYPE_JERICHO); ++cur_mp_type) {
                      ...
                      if (possible_mp_types[cur_mp_type] == SOC_PPC_OAM_MP_TYPE_JERICHO_ACTIVE_MATCH ||
                          ...
                          for (mep_prof = 0; mep_prof < SOC_PPD_OAM_NON_ACC_PROFILES_ARAD_PLUS_NUM; ++mep_prof) {
...
{
    uint32  res;
    SOC_PPD_OAM_OAMP_MEP_DB_ENTRY  lcl_mep_db_entry;
    const SOC_PPD_OAM_OAMP_MEP_DB_ENTRY  *p_mep_db_entry;
    soc_reg_above_64_val_t entry_above_64;
    uint32  entry_buffer[2];
    ...
    if (mep_db_entry == NULL) {
        SOC_PPD_OAM_OAMP_MEP_DB_ENTRY_clear(&lcl_mep_db_entry);
    ...
    if (!SOC_PPD_OAM_IS_MEP_TYPE_BFD(p_mep_db_entry->mep_type)) {
        ...
        if (is_last_mep && p_mep_db_entry->mep_type ==SOC_PPD_OAM_MEP_TYPE_ETH_OAM &&SOC_IS_ARADPLUS_AND_BELOW(unit) ) {
  ...
  )
{
  ...
  if (!update) {
      soc_OAMP_RMEP_DBm_field_set(unit, &entry, LAST_CCM_LIFETIMEf, &reg_val);
      reg_val = rmep_db_entry->last_ccm_lifetime_valid_on_create;
      soc_OAMP_RMEP_DBm_field_set(unit, &entry, LAST_CCM_LIFETIME_VALIDf, &reg_val);
  }
...
uint32 arad_pp_oam_event_fifo_get_interrupt_stat_mesage_register_internal(
   SOC_SAND_IN  int                                        unit) {
    ...
    if (last_interrupt_message_num[unit] == _ARAD_PP_OAM_NOF_INTERRUPT_MESSAGES_IN_FIFO) {
        ...
        soc_reg_above_64_field_get(unit, OAMP_STAT_INTERRUPT_MESSAGEr, data, STAT_INTERRUPT_MESSAGEf, interrupt_message[unit]);
        last_interrupt_message_num[unit] = 0;
    }
  ...
  ) 
{
    ...
    if (SOC_IS_JERICHO(unit) && (event_type == 1)) {
        /* Event is statistics report (LM/DM) */
        if (use_dma) {
            
            SOC_SAND_SET_ERROR_MSG((_BSL_SOCDNX_SAND_MSG(" Error: DMA for reports unavailable yet\n")));
            /*fill_interrupt_message = arad_pp_oam_event_fifo_get_interrupt_stat_mesage_dma_internal;*/
        }
        else {
    ...
    LOG_DEBUG(BSL_LS_SOC_OAM,
              (BSL_META_U(unit,
                          "Reading from the interrupt message. here are the first few bytes: 0x%08x%08x%08x%08x%08x%08x%08x\n"),interrupt_message[unit][6],interrupt_message[unit][5],interrupt_message[unit][4],
                          interrupt_message[unit][3],interrupt_message[unit][2],interrupt_message[unit][1],interrupt_message[unit][0]));
  ...
  )
{
	...
	header_size += _insert_itmh(pkt_header, dest_system_port,dp,tc ,0,0, 1 /*PPH type: 1 --> pph only*/  );
  ...
  )
{
  ...
  *trap_id = trap_above_64[0];
  ...
  res = arad_hpu_itmh_fwd_dest_info_parse(unit, dest, FALSE/*is_advanced_mode*/, FALSE/*is_extension*/, dest_info);
  SOC_SAND_CHECK_FUNC_RESULT(res, 5, exit);
  ...
  )
{
    ...
    if (!is_lm) { /* Reset the MEP_PE_PROFILE*/
        ...
        if (SOC_IS_ARADPLUS_A0(unit) && !exists_piggy_back_down) {
            /* For Arad, the MEP profile should this one of ARAD_PP_OAMP_PE_PROGS_DOWN_MEP_TLV_FIX(Down) or ARAD_PP_OAMP_PE_PROGS_UP_MEP_MAC(up):*/
       ...
       )
{
    ...
    soc_reg_above_64_field_set(unit,OAMP_TST_LBM_CTRLr, reg_data, TST_LBM_PE_KEYf, reg_field);
    ...
    if (mep_db_entry.mep_type == SOC_PPD_OAM_MEP_TYPE_ETH_OAM) {
        ...
        header_size += _insert_eth(pkt_header,dst_addr_in_uint32s,src_mac_for_real, vlans, tpids,mep_db_entry.tags_num, !(mep_db_entry.is_upmep), udh_size );
    } else {
        ...
        pph.fwd_header_offset = udh_size;
        pph.pkt_is_ctrl = 0;
       ...
       )
{
    ...
    if ((!SOC_IS_JERICHO(unit)) && (event_type != JER_PP_OAM_API_DMA_EVENT_TYPE_EVENT)) {
...
{
    return (dma_event_handler_cb[unit]) ? (dma_event_handler_cb[unit](unit, event_type)) : 0; /* Long way for one line... */
}
...
int
  arad_pp_oam_bfd_acc_endpoint_tx_disable(int unit, uint32 mep_index, SOC_PPC_OAM_OAMP_MEP_DB_ENTRY *mep_db_entry) {
    ...
    /* Read mep-db entry */
    SOCDNX_SAND_IF_ERR_EXIT(soc_mem_read(unit, mem, MEM_BLOCK_ANY, mep_index, &entry_above_64));
    ...
    SOCDNX_SAND_IF_ERR_EXIT(soc_mem_write(unit, mem, MEM_BLOCK_ANY, mep_index, &entry_above_64));
...
{ 
    ...
    res = arad_pp_diag_received_packet_info_get(unit, core_id, &rpi);
    ...
    if ((port_type == SOC_PORT_IF_OAMP) &&  (opaque_pt_attr != _ARAD_PP_OAM_PTCH_OPAQUE_VALUE)) {
        ...
        if (ll_found) {
            ...
            if (regs_val[0] == 0) { /*direction is down*/
                ...
                SUB_TYPE_MSG_FROM_CODE(sub_type, regs_val[0]);
                regs_val[0] = 0;
                GET_SIGNAL_FOR_OAM_ID_UP(fld, core_id);
                if (dir_printed == 0) {
                    LOG_CLI((BSL_META_U(unit,
                                        "Direction is up, ")));
                }
  ...
  arad_pp_oam_diag_mp_type_resolution_simulate(
    ...
    if (SOC_PPD_OAM_IS_CLASSIFIER_JERICHO_MODE(unit)) {
        ...
        */
        ...
        SOC_SAND_SOC_IF_ERROR_RETURN(res, 10, exit, READ_IHP_OAM_MP_TYPE_MAPr_REG64(unit, SOC_CORE_DEFAULT, &cfg_mp_type_map));
