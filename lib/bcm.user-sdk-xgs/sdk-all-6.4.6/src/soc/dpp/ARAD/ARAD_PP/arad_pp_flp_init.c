   ...
   )
{
    ...
    *o_val = 0;
    ...
    res = arad_pp_occ_mgmt_get_app_mask(unit, SOC_OCC_MGMT_TYPE_INLIF, SOC_OCC_MGMT_APP_USER, &occ_mask);
    SOC_SAND_CHECK_FUNC_RESULT(res, 200, exit);
    /* sal_printf ("occ-mng-mask = %d\n", occ_mask); */

    *o_mask = occ_mask ^ xor_mask;
    
    /* sal_printf ("set o_mask = %d\n", *o_mask); */
    res = arad_pp_occ_mgmt_app_set(unit,SOC_OCC_MGMT_TYPE_INLIF, SOC_OCC_MGMT_APP_USER, val, o_val);
   ...
   )
{
    ...
    /* sal_printf ("in_lif_profile_mask = %d %x, in_lif_profile_val = %d %x\n", in_lif_profile_mask, in_lif_profile_mask, in_lif_profile_val, in_lif_profile_val); */
...
uint32
   arad_pp_flp_process_ethernet_ing_ivl_learn(
     int unit,
...
exit:
 SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_pp_flp_process_ethernet_ing_ivl_learn", 0, 0);
}
   ...
   )
{
  ...
  if(ARAD_KBP_ENABLE_IPV4_UC || ARAD_KBP_ENABLE_IPV4_MPLS_EXTENDED){
      flp_key_construction_tbl.key_a_inst_0_to_5_valid = 0x0;
      flp_key_construction_tbl.key_b_inst_0_to_5_valid = 0x38; /*6'b001000 */
      flp_key_construction_tbl.key_c_inst_0_to_5_valid = 0x0; 
  }
  else
#endif
   ...
   )
{
  ...
  /* Load IVL L2 */
  if (/* (SOC_DPP_CONFIG(unit)->pp.ivl_cpu_port >= 0) && (*/(SOC_DPP_CONFIG(unit)->pp.ivl_inlif_profile == 1)) {
      int32 ivl_prog_id;
...
/*    arad_pp_flp_lookups_ethernet_ing_ivl_learn(unit, sa_auth_enabled, slb_enabled, ivl_prog_id); */
      arad_pp_flp_process_ethernet_ing_ivl_learn(unit, TRUE, sa_auth_enabled, slb_enabled, ivl_prog_id);
  }
