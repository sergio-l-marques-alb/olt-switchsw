 ...
 *      when using lag both in ingress and egress mc, this shouldn't be allowed.
 * Parameters:
 *      unit    - Device Number 
 * Returns:
 *      SOC_E_XXX 
...
int soc_arad_trunk_resolve_ingress_mc_destination_method(int unit, int8 use_trunk_as_ingress_mc_dest)
{
    ...
    {
        /* value if 0 will enable the fix that allows use if trunk as ingress mc destination */
