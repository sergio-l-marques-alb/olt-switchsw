  ...
  )
{
  ...
  {
  case ARAD_ITM_VSQ_GROUP_CTGRY:
    SOC_SAND_ERR_IF_ABOVE_MAX(vsq_in_group_ndx * arad_itm_vsq_is_ocb_max(unit), vsq_in_group_size - 1, ARAD_ITM_VSQ_NDX_OUT_OF_RANGE_ERR, 30, exit);
    break;
  case ARAD_ITM_VSQ_GROUP_CTGRY_TRAFFIC_CLS:
    SOC_SAND_ERR_IF_ABOVE_MAX(vsq_in_group_ndx * arad_itm_vsq_is_ocb_max(unit), vsq_in_group_size - 1, ARAD_ITM_VSQ_NDX_OUT_OF_RANGE_ERR, 32, exit);
    break;
  case ARAD_ITM_VSQ_GROUP_CTGRY_2_3_CNCTN_CLS:
    SOC_SAND_ERR_IF_ABOVE_MAX(vsq_in_group_ndx * arad_itm_vsq_is_ocb_max(unit), vsq_in_group_size - 1, ARAD_ITM_VSQ_NDX_OUT_OF_RANGE_ERR, 34, exit);
    break;
  case ARAD_ITM_VSQ_GROUP_STTSTCS_TAG:
    SOC_SAND_ERR_IF_ABOVE_MAX(vsq_in_group_ndx * arad_itm_vsq_is_ocb_max(unit), vsq_in_group_size - 1, ARAD_ITM_VSQ_NDX_OUT_OF_RANGE_ERR, 36, exit);
    break;
  case ARAD_ITM_VSQ_GROUP_SRC_PORT:
    SOC_SAND_ERR_IF_ABOVE_MAX(vsq_in_group_ndx * arad_itm_vsq_is_ocb_max(unit), vsq_in_group_size - 1, ARAD_ITM_VSQ_NDX_OUT_OF_RANGE_ERR, 38, exit);
    break;
  case ARAD_ITM_VSQ_GROUP_PG:
  ...
  )
{
    ...
    info->max_inst_q_size_bds = (0x1 << soc_mem_field_length(unit,IQM_VQPR_MAm,SET_THRESHOLD_BDf)) - 1;
...
#ifdef BCM_88660_A0
          if (SOC_IS_ARADPLUS(unit) && soc_property_get(unit, spn_POLICER_COLOR_RESOLUTION_MODE, 0)) {
            /* If the detailed color mode is active then Ethernet-Meter red (=1) becomes 2, Meter yellow (=2) becomes 1, and Meter (=3) stays 3. */
            if (mtr_res_indx == 1) {
