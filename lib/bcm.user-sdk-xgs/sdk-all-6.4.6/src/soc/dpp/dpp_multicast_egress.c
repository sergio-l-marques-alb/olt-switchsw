    ...
    if (SOC_DPP_CONFIG(unit)->tm.mc_mode & (DPP_MC_EGR_CORE_FDA_MODE | DPP_MC_EGR_CORE_MESH_MODE)) {
        ...
        if (group_mcid == SOC_DPP_CONFIG(unit)->tm.nof_mc_ids) { /* set core replication of all groups */
            ...
            if (groups_remainder > 0) {
                ...
                i = groups_remainder / SOC_SAND_NOF_BITS_IN_UINT32; /* offset in unit32s */
...
)
{
    ...
    if (SOC_IS_JERICHO(unit)) {
        ...
        if (port == _SHR_GPORT_INVALID) {
            ...
            DPP_CUD2CORE_SET_CORE(mcds, cud, DPP_CUD2CORE_UNDEF_VALUE);
        } else {
            SOCDNX_IF_ERR_EXIT(soc_port_sw_db_local_to_tm_port_get(unit, port, &tm_port, &core));
            previous_core = DPP_CUD2CORE_GET_CORE(mcds, cud); /*get core from sw*/
            if (core != previous_core) {
                ...
                DPP_CUD2CORE_SET_CORE(mcds, cud, core);
            }
        }
...
)
{
    ...
    if (SOC_IS_JERICHO(unit)) {
        ...
        core = DPP_CUD2CORE_GET_CORE(mcds, cud); /*get core from sw*/
    ...
    *out_err = _SHR_E_NONE;
    ...
    DPP_MC_FOREACH_CORE(cores_to_set, core_bm, core) {
        ...
        DPP_MC_ASSERT( /* there must not be unprocessed replications belonging to this 2nd CUD */
          nof_cud_reps_in_core == mcds->nof_egr_outlif_reps[core] && nof_bitmap_reps_in_core == mcds->nof_egr_bitmap_reps &&
...
)
{
    ...
    SOCDNX_IF_ERR_EXIT(dpp_mult_remove_replication(unit, DPP_MCDS_TYPE_VALUE_EGRESS_START, reps->dest, reps->cud, out_err, &changed_cores)); 
    ...
    /* recreate the group with the extra replication */
    {
        ...
        if (*out_err) { /* If failed due to lack of free entries */
            ...
            DPP_MC_FOREACH_CORE(changed_cores, core_bm, core) {
                ret1 = dpp_mcds_remove_replications_from_egress_group(unit, core, group_mcid, reps->dest, reps->cud); 
                if (ret2 == SOC_E_NONE) {
                    ret2 = ret1;
                }
            }
