...
)
{
    ...
    uint8 get_bm_reps_in_this_core, first_core = 1;
    int core;
    ...
    *group_size = 0;
    ...
    DPP_MC_FOREACH_CORE(cores_to_get, cores_bm, core) {
        ...
        SOCDNX_IF_ERR_EXIT(mcds->get_replications_from_entry(unit, core, get_bm_reps_in_this_core, first_core, cur_index,
        ...
        while (cur_index != end_index) {
            DPP_MC_ASSERT(cur_index > 0 && cur_index < MCDS_INGRESS_LINK_END(mcds));
            SOCDNX_IF_ERR_EXIT(mcds->get_replications_from_entry(unit, core, get_bm_reps_in_this_core, first_core, cur_index, entry_type, &cud2, &entries_remaining, group_size, &cur_index));
        ...
        first_core = 0; /* Don't increase bitmap replication count for a core which is not the first one */
    }
