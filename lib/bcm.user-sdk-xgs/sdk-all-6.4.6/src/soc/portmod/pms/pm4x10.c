...
#define PM4x10_PHY_ACCESS_GET(unit, port, pm_info, phy_acc) \
    do { \
        ...
        phy_acc = (is_bypass && PM_4x10_INFO(pm_info)->first_phy != -1) ? (PM_4x10_INFO(pm_info)->first_phy | SOC_REG_ADDR_PHY_ACC_MASK) : port; \
    } while (0)

extern int pm4x10_phy_control_set(phymod_phy_access_t *phy_access, int chain_len, soc_phy_control_t type, phymod_tx_t*, uint32, uint32 v);
/*extern int pm4x10_phy_control_get(int u, soc_port_t p, pm_info_t pm, soc_phy_control_t type, phymod_tx_t*, uint32, uint32 *v);*/
extern int pm4x10_phy_control_get(phymod_phy_access_t *phy_access, int chain_len, soc_phy_control_t type, phymod_tx_t*, uint32, uint32 *v);
extern int pm4x10_phy_link_mon_enable_set (phymod_phy_access_t *phy_access, int chain_len, uint32 value);
extern int pm4x10_phy_link_mon_status_get (phymod_phy_access_t *phy_access, int chain_len);
                   ...
                   int wb_buffer_index, pm_info_t pm_info)
{
    ...
    sal_memcpy(&(pm4x10_data->core_access[0].access), &info->access,
    ...
    if(SOC_WARM_BOOT(unit)){
        for( i = 0 ; i < MAX_PORTS_PER_PM4X10; i++) {
           rv = SOC_WB_ENGINE_GET_ARR (unit, SOC_WB_ENGINE_PORTMOD,
                         pm_info->wb_vars_ids[phy_type], &pm4x10_data->core_access[i].type, i);
           _SOC_IF_ERR_EXIT(rv);
        }
    }
...
pm4x10_default_bus_update(int unit, pm_info_t pm_info, const portmod_bus_update_t* update) 
{
    ...
    if(PM_4x10_INFO(pm_info)->default_fw_loader_is_used) {
        PM_4x10_INFO(pm_info)->external_fw_loader = update->external_fw_loader;
        user_acc = (portmod_default_user_access_t*)PM_4x10_INFO(pm_info)->core_access[0].access.user_acc;
        user_acc->blk_id = update->blk_id;
    }

    if(PM_4x10_INFO(pm_info)->default_bus_is_used) {
        PM_4x10_INFO(pm_info)->core_access[0].access.bus = update->default_bus; 
        user_acc = (portmod_default_user_access_t*)PM_4x10_INFO(pm_info)->core_access[0].access.user_acc;
        user_acc->blk_id = update->blk_id;
    }
...
static int _xlport_lag_failover_status_toggle(int unit, int phy_acc)
{
    uint32_t rval;
...
{
    int rv = SOC_E_NONE, phy_acc;

    PM4x10_PHY_ACCESS_GET(unit, port, pm_info, phy_acc);

    if (SOC_REG_IS_VALID(unit, PGW_CNTMAXSIZEr)) {
        rv = READ_PGW_CNTMAXSIZEr(unit, phy_acc, (uint32_t *)val);
    } else if (SOC_REG_IS_VALID(unit, XLPORT_CNTMAXSIZEr)) {
        rv = READ_XLPORT_CNTMAXSIZEr(unit, phy_acc, (uint32_t *)val);
    }
    return rv;
}
...
int pm4x10_port_cntmaxsize_set(int unit, int port, pm_info_t pm_info, int val)
{
    ...
    if (SOC_REG_IS_VALID(unit, PGW_CNTMAXSIZEr)) {
        rv = WRITE_PGW_CNTMAXSIZEr(unit, phy_acc, (uint32_t)val);
    } else if (SOC_REG_IS_VALID(unit, XLPORT_CNTMAXSIZEr)) {
        rv = WRITE_XLPORT_CNTMAXSIZEr(unit, phy_acc, (uint32_t)val);
    }
    return rv;
}
                                    ...
                                    int* is_most_ext)
{
    ...
    if(params->phyn > PM_4x10_INFO(pm_info)->nof_phys){
        _SOC_EXIT_WITH_ERR(SOC_E_PARAM,
               (_SOC_MSG("phyn exceeded. max allowed %d. got %d"),
               PM_4x10_INFO(pm_info)->nof_phys - 1, params->phyn));
    }

    phyn = params->phyn;

    if(phyn < 0) {
        phyn = PM_4x10_INFO(pm_info)->nof_phys - 1;
    }

    sal_memcpy (&phy_access[0], &(PM_4x10_INFO(pm_info)->core_access[phyn]),
                sizeof(PM_4x10_INFO(pm_info)->core_access[phyn]));
    ...
    phy_access[0].access.lane_mask <<= PM_4x10_INFO(pm_info)->first_phy_lane[phyn];

    *nof_phys = 1;

    if (is_most_ext) {
        if (phyn == PM_4x10_INFO(pm_info)->nof_phys-1) {
            *is_most_ext = 1;
        } else {
            *is_most_ext = 0;
        }
...
STATIC
int _pm4x10_pm_enable (int unit, int port, pm_info_t pm_info, int port_index, 
                        const portmod_port_add_info_t* add_info, int enable)
{
    ...
    if(enable) {
        ...
        if(PM4x10_QUAD_MODE_IF(config->interface)) {
            port_mode = XLMAC_4_LANES_TOGETHER;
        } else if(PM4x10_DUAL_MODE_IF(config->interface)){
            if(!three_ports_mode) {
                port_mode = XLMAC_2_LANES_DUAL;
            } else {
                port_mode = (port_index < 2) ?  XLMAC_3_TRI_0_0_2_3 :
                                                XLMAC_3_TRI_0_1_2_2;
            }
        } else { /*single mode*/
            if(!three_ports_mode) {
                ...
                port_mode = (port_index < 2) ?  XLMAC_3_TRI_0_1_2_2 :
                                                XLMAC_3_TRI_0_0_2_3;
            }
        }
        ...
        soc_reg_field_set (unit, XLPORT_MODE_REGr, &reg_val, 
                           XPORT0_CORE_PORT_MODEf, port_mode);
        soc_reg_field_set (unit, XLPORT_MODE_REGr, &reg_val, 
                           XPORT0_PHY_PORT_MODEf,  port_mode);
        ...
        if(PM_4x10_INFO(pm_info)->portmod_phy_external_reset) {
            if(!((config->interface == SOC_PORT_IF_CAUI)&&(PM_4x10_INFO(pm_info)->core_num_int))) {
        ...
        nof_phys = PM_4x10_INFO(pm_info)->nof_phys;
        core_access = (PM_4x10_INFO(pm_info)->core_access);

        if (nof_phys > 1) {
            
            PORTMOD_USER_ACC_LPORT_SET(&(core_access[nof_phys-1].access), port); 
        }
        _SOC_IF_ERR_EXIT(portmod_port_phychain_core_probe(core_access,
                                                              nof_phys));
                         ...
                         const portmod_port_add_info_t* add_info, int enable)
{
    ...
    if(enable) {
        ...
        if (SOC_REG_IS_VALID(unit, PGW_MAC_RSV_MASKr)) {
            ...
            _SOC_IF_ERR_EXIT(WRITE_XLPORT_MAC_RSV_MASKr(unit, port, rsv_mask));
        }
        ...
        /* Counter MAX size */
        if (SOC_REG_IS_VALID(unit, PGW_CNTMAXSIZEr)) {
            _SOC_IF_ERR_EXIT(READ_PGW_CNTMAXSIZEr(unit, phy_acc, &reg_val));
            soc_reg_field_set(unit, PGW_CNTMAXSIZEr, &reg_val, CNTMAXSIZEf, 1518);
            _SOC_IF_ERR_EXIT(WRITE_PGW_CNTMAXSIZEr(unit, phy_acc, reg_val));
        } else if (SOC_REG_IS_VALID(unit, XLPORT_CNTMAXSIZEr)) {
            _SOC_IF_ERR_EXIT(READ_XLPORT_CNTMAXSIZEr(unit, phy_acc, &reg_val));
            soc_reg_field_set(unit, XLPORT_CNTMAXSIZEr, &reg_val, CNTMAXSIZEf, 1518);
            _SOC_IF_ERR_EXIT(WRITE_XLPORT_CNTMAXSIZEr(unit, phy_acc, reg_val));
        }
 
        /* Reset MIB counters */
        if (SOC_REG_IS_VALID(unit, PGW_MIB_RESETr)) {
            ...
            _SOC_IF_ERR_EXIT(WRITE_PGW_MIB_RESETr(unit, reg_val));
        } else if (SOC_REG_IS_VALID(unit, XLPORT_MIB_RESETr)) {
            _SOC_IF_ERR_EXIT(READ_XLPORT_MIB_RESETr(unit, phy_acc, &reg_val));
            SHR_BITSET(&reg_val, internal_port);
            _SOC_IF_ERR_EXIT(WRITE_XLPORT_MIB_RESETr(unit, phy_acc, reg_val));
            SHR_BITCLR(&reg_val, internal_port);
            _SOC_IF_ERR_EXIT(WRITE_XLPORT_MIB_RESETr(unit, phy_acc, reg_val));
        }
    } else {
                        ...
                        const portmod_port_interface_config_t* config)
{
    ...
    int    i, nof_phys = 0, rv = 0, phy_acc, flags = 0;
    ...
    if(!pm_is_bypassed){
        ...
        rv = xlmac_encap_set (unit, port, flags,
                              PHYMOD_INTF_MODES_HIGIG_GET(config) ?
                              SOC_ENCAP_HIGIG2 : SOC_ENCAP_IEEE);
        _SOC_IF_ERR_EXIT(rv);

        if (config->speed < 10000) {
            rv = xlmac_strict_preamble_set (unit, port, 0);
            _SOC_IF_ERR_EXIT(rv);
        }
    }
    ...
    is_legacy_phy = ((portmod_default_user_access_t *)(phy_access_lane.access.user_acc))->is_legacy_phy_present;
    if (!is_legacy_phy)
...
{
    int        port_index = -1, rv = 0, phy, i, phys_count = 0, nof_phys, port_i;
    uint32     pm_is_active = 0, bitmap;
    uint32 pm_is_bypassed = 0;
    soc_pbmp_t port_phys_in_pm;
    int my_i, first_phy = -1, phy_acc;
    ...
    pm_is_bypassed = (add_info->interface_config.interface == SOC_PORT_IF_ILKN)? 1: 0;
    rv = SOC_WB_ENGINE_SET_VAR(unit, SOC_WB_ENGINE_PORTMOD, 
                    pm_info->wb_vars_ids[isBypassed], &pm_is_bypassed);
    _SOC_IF_ERR_EXIT(rv);
    ...
    if(!pm_is_active){
        ...
        /*init the PM*/
        rv = _pm4x10_pm_enable (unit, port, pm_info, port_index, add_info, 1);
        _SOC_IF_ERR_EXIT(rv);
    ...
    /* initalize port */
    rv = _pm4x10_pm_port_init(unit, port, phy_acc, port_index, add_info, 1);
    _SOC_IF_ERR_EXIT(rv);
...
int pm4x10_port_detach(int unit, int port, pm_info_t pm_info)
{
    ...
    rv = _pm4x10_pm_port_init(unit, port, phy_acc, port_index, NULL, 0);
    _SOC_IF_ERR_EXIT(rv);

    /*deinit PM in case of last one*/
    if (is_last_one) {
       rv = _pm4x10_pm_enable(unit, port, pm_info, port_index, NULL, 0);
       _SOC_IF_ERR_EXIT(rv);
...
int pm4x10_pm_bypass_set(int unit, pm_info_t pm_info, int bypass_enable)
{
    ...
    if(pm_is_active) {
        _SOC_EXIT_WITH_ERR(SOC_E_PARAM, 
                    (_SOC_MSG("can't chenge bypass mode for active pm")));
    }
                            ...
                            int flags, int enable)
{
    ...
    if(enable){
        ...
        if((PORTMOD_PORT_ENABLE_MAC_GET(actual_flags))  && (!is_bypassed)){
            rv = xlmac_enable_set(unit, port, 0, 0);
            _SOC_IF_ERR_EXIT(rv);
        }
    }
    ...
    switch (interface_config->speed) {
        ...
        case 10000:
            if (port_num_lanes == 4) {
                ...
                if( fiber_pref ||
                   (interface_config->interface==SOC_PORT_IF_SFI) ||
                   (interface_config->interface==SOC_PORT_IF_SR)) {
                    phy_interface_config->interface_type = phymodInterfaceSFI ;
                } else if(interface_config->interface==SOC_PORT_IF_CR) {
        ...
        case 20000: 
            if (port_num_lanes == 4) {
                ...
                if (port_is_higig) {
                    ...
                    if( (interface_config->interface == SOC_PORT_IF_KR2) ||   
                        (interface_config->interface == SOC_PORT_IF_KR) ) {
                        phy_interface_config->interface_type = phymodInterfaceKR2;
                    } else if( (interface_config->interface == SOC_PORT_IF_CR2) || 
        ...
        case 40000:
            if(port_is_higig) {
                if(interface_config->interface & SOC_PORT_IF_KR4){
                ...
                phy_interface_config->interface_type = phymodInterfaceKR4;
            }else if(interface_config->interface == SOC_PORT_IF_SR4){
    ...
    switch (interface_config->speed) {
        case 10000:
            if (port_num_lanes == 4) {
                ...
                if(interface_config->interface==SOC_PORT_IF_SFI)  {
                    phy_interface_config->interface_type = phymodInterfaceSFI ;
                } else if(interface_config->interface==SOC_PORT_IF_CR) {
                    phy_interface_config->interface_type = phymodInterfaceCR;
                } else if(interface_config->interface==SOC_PORT_IF_SR) {
                    phy_interface_config->interface_type = phymodInterfaceSR;
                } else if(interface_config->interface==SOC_PORT_IF_KR) {
        ...
        case 40000:
            if(port_is_higig) {
                ...
                phy_interface_config->interface_type = phymodInterfaceSR4;
            }else if(interface_config->interface == SOC_PORT_IF_CR4){
                phy_interface_config->interface_type = phymodInterfaceCR4;
            }else if(interface_config->interface == SOC_PORT_IF_LR4){
                phy_interface_config->interface_type = phymodInterfaceLR4;
            }else if(interface_config->interface == SOC_PORT_IF_KR4){
                phy_interface_config->interface_type = phymodInterfaceKR4;
            }else{
                phy_interface_config->interface_type = phymodInterfaceXLAUI;
            }
            break;
        case 100000:
            if(interface_config->interface == SOC_PORT_IF_SR4){
                phy_interface_config->interface_type = phymodInterfaceSR4;
            } else if(interface_config->interface == SOC_PORT_IF_LR4){
                phy_interface_config->interface_type = phymodInterfaceLR4;
            } else if(interface_config->interface == SOC_PORT_IF_KR4){
                        ...
                        int all_phy)
{
    ...
    phymod_tx = &phy_init_config->tx[0] ; 
    ...
    if (!(config->flags & PHYMOD_INTF_F_SET_SPD_TRIGGER)) {
            _SOC_IF_ERR_EXIT(portmod_port_phychain_tx_set(phy_access_arr, nof_phys, phymod_tx));
    }
                  ...
                  portmod_loopback_mode_t loopback_type, int enable)
{
    ...
    if (enable)
        _SOC_IF_ERR_EXIT(_xlport_lag_failover_status_toggle(unit, phy_acc));
                                   ...
                                   portmod_port_ability_t* ability)
{
    ...
    int                 max_speed = 11000;
    int                 is_dual_lane_port = 1, num_of_lanes = 0 ;
    uint32              bitmap, tmpbit ;
    phymod_phy_access_t phy_access[1+MAX_PHYN];
    int                 nof_phys;

   
    /* If legacy external phy present retrive the local ability from the 
    ...
    if (nof_phys > 1) {
        ...
        if(PORTMOD_IS_LEGACY_PHY_GET(&(phy_access[nof_phys-1].access))) {
            /* call portmod phy chain function to retrive the ability */
            return portmod_port_phychain_local_ability_get(phy_access,
                                                           nof_phys,
                                                           ability);
        } else {
    ...
    if (_pm4x10_port_index_get (unit, port, pm_info, &port_index, &bitmap))
        return (SOC_E_PARAM);
    ...
    is_dual_lane_port = (interface_config->interface ==  SOC_PORT_IF_RXAUI)? 1 : 0;
    ...
    if ((num_of_lanes==1)||(num_of_lanes==2)) {
...
static int _pm4x10_nof_lanes_get(int unit, int port, pm_info_t pm_info)
{
    int port_index, rv;
                                 ...
                                 int* is_most_ext)
{
    ...
    if(phyn > PM_4x10_INFO(pm_info)->nof_phys){
        _SOC_EXIT_WITH_ERR(SOC_E_PARAM, 
               (_SOC_MSG("phyn exceeded. max allowed %d. got %d"), 
                       PM_4x10_INFO(pm_info)->nof_phys - 1, phyn));
    }
    if(phyn < 0)
    {
        phyn = PM_4x10_INFO(pm_info)->nof_phys - 1;
    ...
    sal_memcpy(&core_access_arr[0], &(PM_4x10_INFO(pm_info)->core_access[phyn]),
                     sizeof(PM_4x10_INFO(pm_info)->core_access[phyn]));
    *nof_cores = 1;
...
int pm4x10_ext_phy_attach_to_pm(int unit, pm_info_t pm_info, const phymod_core_access_t *ext_phy_access, uint32 first_phy_lane)
{
    ...
    if (MAX_PHYN <= PM_4x10_INFO(pm_info)->nof_phys){
    ...
    candidate_phy_access = &(PM_4x10_INFO(pm_info)->core_access[PM_4x10_INFO(pm_info)->nof_phys]);
    sal_memcpy(candidate_phy_access, ext_phy_access , sizeof(phymod_core_access_t));

    PM_4x10_INFO(pm_info)->first_phy_lane[PM_4x10_INFO(pm_info)->nof_phys] = first_phy_lane;
    ++PM_4x10_INFO(pm_info)->nof_phys;

    if(SOC_WARM_BOOT(unit)){
        rv = SOC_WB_ENGINE_GET_ARR(unit, SOC_WB_ENGINE_PORTMOD,
                     pm_info->wb_vars_ids[phy_type], &(candidate_phy_access->type),PM_4x10_INFO(pm_info)->nof_phys - 1);
        _SOC_IF_ERR_EXIT(rv);
    }
exit:
   SOC_FUNC_RETURN;
...
int pm4x10_ext_phy_detach_from_pm(int unit, pm_info_t pm_info, phymod_core_access_t *ext_phy_access)
{
    ...
    /* Validate parameters */
    if (0 == PM_4x10_INFO(pm_info)->nof_phys){
        _SOC_EXIT_WITH_ERR(SOC_E_PARAM, (_SOC_MSG("Port macro has no external phys connected to it")));
    }
    if (NULL == ext_phy_access){
        _SOC_EXIT_WITH_ERR(SOC_E_PARAM, (_SOC_MSG("ext_phy_access output parameter is NULL")));
    }
...
{
    return(xlmac_pfc_config_set(unit, port, pfc_cfg));
}
...
int pm4x10_port_pfc_config_get (int unit, int port,pm_info_t pm_info, 
                           const portmod_pfc_config_t* pfc_cfg)
{
    return (xlmac_pfc_config_get (unit, port, pfc_cfg));
}
...
{
    return (xlmac_speed_get (unit, port, speed));
}
...
int pm4x10_port_phy_reg_read(int unit, int port, pm_info_t pm_info, int flags, int reg_addr, uint32* value)
{
    ...
    _SOC_IF_ERR_EXIT
        (portmod_port_chain_phy_access_get(unit, port, pm_info, 
                                           ...
                                           &nof_phys));
    _SOC_IF_ERR_EXIT
        (portmod_port_phychain_reg_read(phy_access, nof_phys, reg_addr,value));
...
int pm4x10_port_phy_reg_write(int unit, int port, pm_info_t pm_info, int flags, int reg_addr, uint32 value)
{
    ...
    _SOC_IF_ERR_EXIT
        (portmod_port_chain_phy_access_get(unit, port, pm_info, 
                                           ...
                                           &nof_phys));
    _SOC_IF_ERR_EXIT
        (portmod_port_phychain_reg_write(phy_access, nof_phys, reg_addr,value));
...
{
    return (xlmac_pass_control_frame_set(unit, port, rx_ctrl->pass_control_frames));
}
...
{
    int link, phy_acc, flags = 0;   
    phymod_autoneg_status_t autoneg;
    SOC_INIT_FUNC_DEFS;

    PM4x10_PHY_ACCESS_GET(unit, port, pm_info, phy_acc);

    memset(&autoneg, 0, sizeof(autoneg));
    ...
    if(!link) {
        ...
        if(autoneg.enabled && autoneg.locked) {
            ...
            _SOC_IF_ERR_EXIT(xlmac_enable_set(unit, port, 0, 0));
            flags = XLMAC_SPEED_SET_FLAGS_SOFT_RESET_DIS;
            _SOC_IF_ERR_EXIT(xlmac_speed_set(unit, port, flags, autoneg.data_rate));
...
int pm4x10_port_lag_failover_status_toggle (int unit, soc_port_t port, pm_info_t pm_info)
{
    ...
    return(_xlport_lag_failover_status_toggle(unit, phy_acc));
}
                      ...
                      uint32 inst, int op_type, int op_cmd, const void *arg) 
{
    ...
    /* This is to select between line side and sys side.  The bit definition
       is misleading.  Need to fix. */
    if(PHY_DIAG_INST_INTF(inst) == PHY_DIAG_INTF_SYS ){
        phy_access[0].access.flags |= PHYMOD_ACC_DEVAD_0_OVERRIDE_MASK; 
    } else { /* line side is default */
        phy_access[0].access.flags &= ~PHYMOD_ACC_DEVAD_0_OVERRIDE_MASK; 
    }

    /* can get lane#  with this PHY_DIAG_INST_LN(inst); */
    ...
    switch(op_cmd) {
        ...
        case PHY_DIAG_CTRL_LINKMON_MODE:
            ...
            SOC_IF_ERROR_RETURN
                (pm4x10_phy_link_mon_enable_set(phy_access, nof_phys, PTR_TO_INT(arg)));
            break;
        ...
        case PHY_DIAG_CTRL_LINKMON_STATUS:
            ...
            SOC_IF_ERROR_RETURN
                (pm4x10_phy_link_mon_status_get(phy_access, nof_phys));
            break;
        ...
        default:
            if(op_type == PHY_DIAG_CTRL_SET) {
                ...
                if (!SAL_BOOT_SIMULATION) {
                    SOC_IF_ERROR_RETURN(pm4x10_phy_control_set(phy_access, nof_phys, op_cmd, &ln_txparam, lane_map, PTR_TO_INT(arg)));
                }
                ...
                if (!SAL_BOOT_SIMULATION) {
                    SOC_IF_ERROR_RETURN(pm4x10_phy_control_get(phy_access,nof_phys,op_cmd, &ln_txparam, lane_map, (uint32 *)arg));
                }
            } else {
...
int pm4x10_port_ifg_set(int unit, int port, pm_info_t pm_info, int speed, soc_port_duplex_t duplex, int ifg,  int *real_ifg)
{
    ...
    *real_ifg = 0;
    ...
    _SOC_IF_ERR_EXIT
        (pm4x10_port_ability_local_get(unit, port, pm_info, &ability));
    ...
    *real_ifg = ifg < 64 ? 64 : (ifg + 7) & (0x1f << 3);
    ...
    _SOC_IF_ERR_EXIT(pm4x10_port_duplex_get(unit, port, pm_info,&cur_duplex));
    _SOC_IF_ERR_EXIT(pm4x10_port_speed_get(unit, port,pm_info, &cur_speed));
    ...
    if (cur_speed == speed &&
        cur_duplex == (duplex == SOC_PORT_DUPLEX_FULL ? TRUE : FALSE)) {
            ...
            pm4x10_port_tx_average_ipg_set (unit, port, pm_info, *real_ifg);
...
{
    return (xlmac_e2ecc_hdr_set (unit, port, e2ecc_hdr->words));
}
...
int pm4x10_port_soft_reset_set(int unit, int port, pm_info_t pm_info, int idx, int val)
{
    return (_pm4x10_port_soft_reset_set(unit, port, pm_info, idx, val));
}
...
int pm4x10_port_pgw_reconfig (int unit, int port, pm_info_t pm_info, 
                              const portmod_port_mode_info_t *pmode, int flags)
{
    int rv, idx;
    int phy_acc;
    uint32 reg_val;
