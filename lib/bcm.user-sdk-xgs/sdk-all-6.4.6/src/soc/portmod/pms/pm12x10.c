                         ...
                         const portmod_port_add_info_t* add_info)
{
    ...
    for(i = 0 ; i < 3 ; i++)
    {
        rv = PM_4X10_DRIVER(pm_info, i)->f_portmod_port_interface_config_set(unit, port, PM_4X10_INFO(pm_info,i), 
                                          &add_info->interface_config, TRUE); /* TRUE mean apply to all phys */
        _SOC_IF_ERR_EXIT(rv);
    }
                             ...
                             int flags, int enable)
{
    ...
    if(enable) {
        ...
        /* make sure MAC is also disabled (in case it was enabled) */
                               ...
                               int all_phy)
{
    ...
    rv = PM_4X25_DRIVER(pm_info)->f_portmod_port_interface_config_set (unit, 
                                  port, PM_4X25_INFO(pm_info), config, all_phy);
    _SOC_IF_ERR_EXIT(rv);
                              ...
                              const phymod_autoneg_control_t* an)
{
    ...
    if (!an->enable) {
       ...
       if_cfg.flags |= PHYMOD_INTF_F_SET_SPD_DISABLE;
        ...
        {
            rv = PM_4X10_DRIVER(pm_info, i)->f_portmod_port_interface_config_set(
                           unit, port, PM_4X10_INFO(pm_info, i), &if_cfg, FALSE);
            _SOC_IF_ERR_EXIT(rv);
        }
       if_cfg.flags &= ~PHYMOD_INTF_F_SET_SPD_DISABLE;
       if_cfg.flags |= PHYMOD_INTF_F_SET_SPD_TRIGGER;
        ...
        {
            rv = PM_4X10_DRIVER(pm_info, i)->f_portmod_port_interface_config_set(
                           unit, port, PM_4X10_INFO(pm_info, i), &if_cfg, FALSE); /* only ext phy */
            _SOC_IF_ERR_EXIT(rv);
        }
    }
...
int pm12x10_port_soft_reset_set(int unit, int port, pm_info_t pm_info, int idx, int val)
{
    ...
    for(i = 0 ; i < 3 ; i++)
    {
        rv = PM_4X10_DRIVER(pm_info, i)->f_portmod_port_soft_reset_set(unit, port, 
                                             PM_4X10_INFO(pm_info,i), idx, val);
        _SOC_IF_ERR_EXIT(rv);
    }
                              ...
                              const portmod_port_mode_info_t *pmode, int flags)
{
    ...
    _SOC_IF_ERR_EXIT((PM_4X25_DRIVER(pm_info)->f_portmod_port_pgw_reconfig(unit,
                            port, PM_4X25_INFO(pm_info), pmode, flags)));

    for(i = 0 ; i < 3 ; i++)
    {
        rv = PM_4X10_DRIVER(pm_info, i)->f_portmod_port_pgw_reconfig(unit, port, 
                                             PM_4X10_INFO(pm_info,i), pmode, flags);
        _SOC_IF_ERR_EXIT(rv);
    }
