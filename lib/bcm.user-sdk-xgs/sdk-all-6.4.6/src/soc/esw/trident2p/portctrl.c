...
static portmod_pm_instances_t td2p_pm_types[] = {
    ...
    {portmodDispatchTypePm4x10,  SOC_TD2P_PM4X10_COUNT},
#endif
...
_soc_td2p_portctrl_pm4x10_portmod_init(int unit, int num_of_instances) 
{
    ...
    for (pmid = 0; PORTMOD_SUCCESS(rv) && (pmid < num_of_instances); pmid++) {
       ...
       rv = phymod_polarity_t_init
        ...
        for (core_num = 0; core_num < core_cnt; core_num++) {
            ...
            bonding_swap_tx = _soc_td2p_portctrl_is_chip_bonding_swap(unit, pmid, core_num, portmodDispatchTypePm4x10, 1) ;
            txlane_map = bonding_swap_tx ?
                             SOC_TD2P_NIB_TRX(txlane_map) : txlane_map;
            
            bonding_swap_rx = _soc_td2p_portctrl_is_chip_bonding_swap(unit, pmid, core_num, portmodDispatchTypePm4x10, 0) ;
    ...
    if ((ref_clk_prop == 125) ||( ref_clk_prop == 1))
        ...
        SOC_IF_ERROR_RETURN(
            soc_esw_portctrl_setup_ext_phy_add(unit, first_port, pm_info.type));
    }
    ...
    for (pmid = 0; PORTMOD_SUCCESS(rv) && (pmid < num_of_instances); pmid++) {
        ...
        for (lane = 0; lane < SOC_PM4X25_NUM_LANES; lane++) {
            ...
            if (rx_polarity  == 0xFFFFFFFF) {
                ...
                pm_info.pm_specific_info.pm4x25.polarity.rx_polarity
                            |= rx_polarity;
            } else {
                pm_info.pm_specific_info.pm4x25.polarity.rx_polarity
                            |= ((rx_polarity & 0x1) << lane);
            ...
            if (tx_polarity  == 0xFFFFFFFF) {
                ...
                pm_info.pm_specific_info.pm4x25.polarity.tx_polarity
                            |= tx_polarity;
            } else {
                pm_info.pm_specific_info.pm4x25.polarity.tx_polarity
                            |= ((tx_polarity & 0x1) << lane);
...
_soc_td2p_portctrl_pm12x10_portmod_init(int unit, int num_of_instances) 
{
    ...
    for (pmid = 0; PORTMOD_SUCCESS(rv) && (pmid < num_of_instances); pmid++) {
        ...
        for (core_num = 0;
            ...
            for (lane = 0; lane < SOC_PM4X10_NUM_LANES; lane++) {
                ...
                if (rx_polarity  == 0xFFFFFFFF) {
                    ...
                    pm4x10_info->polarity.rx_polarity |=
                        ((rx_polarity & 0x1) << lane);
                }
                ...
                if (tx_polarity  == 0xFFFFFFFF) {
                    ...
                    pm4x10_info->polarity.tx_polarity |=
                          ((tx_polarity & 0x1) << lane);
                }
            }
        ...
        {
            SOC_IF_ERROR_RETURN(
                soc_esw_portctrl_setup_ext_phy_add(unit, ext_port_num, pm_info.type));
            ext_port_num += SOC_PM4X10_NUM_LANES ;
        }
    }
