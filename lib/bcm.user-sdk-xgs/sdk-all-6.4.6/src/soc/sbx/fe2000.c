/*
 * $Id: fe2000.c,v 1.123 Broadcom SDK $
...
_soc_sbx_fe2000_ped_work_around(int unit)
{
    ...
    if (v) {
        LOG_CLI((BSL_META_U(unit,
                            "sbFe2000SetQueueParams() failed with error code:0x%x\n"),
                 v));
        return SOC_E_INIT;
    }
    ...
    if (v) {
        LOG_CLI((BSL_META_U(unit,
                            "sbFe2000Init() failed with error code:0x%x\n"), v));
        return SOC_E_INIT;
    }
    /* set up queues here */
    ...
    if (v) {
        LOG_CLI((BSL_META_U(unit,
                            "sbFe2000InitQueues() failed with error code:0x%x\n"), v));
        return SOC_E_INIT;
    }
    ...
    if (v < 4) {
        LOG_CLI((BSL_META_U(unit,
                            "PED work-around TX failed (only saw %d packets)\n"), v));
        return SOC_E_INIT;
    }
...
soc_sbx_fe2000_fte_partition(int unit)
{
...
#ifdef BCM_SBX_C1_C2_INTEROP
    if (maxFtes > 192*1024) {
       /* C1mplstp for interop has only 192K ftes */
       maxFtes = 192*1024;      
    }
#endif
    ...
    SOC_SBX_CFG_FE2000(unit)->fteMap[SOC_SBX_FSEG_VPLS_COLOR] = fteBase;
    ...
    if (fteBase < maxFtes) {
        if(SOC_SBX_CFG(unit)->mplstp_ena) {
            ...
            SOC_SBX_CFG_FE2000(unit)->fteMap[SOC_SBX_FSEG_PW_FO] = fteBase;
            fteBase = fteBase + 32 * 1024;
        } else {
            SOC_SBX_CFG_FE2000(unit)->fteMap[SOC_SBX_FSEG_PW_FO] = fteBase;
        }
    }
...
/* FIX: please leave this in the code until SAL_BOOT_BCMSIM is working */
    ...
    sbFe2000InitDefaultParams(feip);
    feip->lr.bLrpBypass = soc_property_get(unit, spn_LRP_BYPASS, 0);
...
/* FIX: this crashes bcmsim */
    ...
    if (SOC_WARM_BOOT(unit) == FALSE) {
        ...
        if (rv) {
            LOG_CLI((BSL_META_U(unit,
                                "sbFe2000InitClearMems() failed with error code:0x%x\n"), rv));
            return SOC_E_INIT;
        }
    }
#endif
    ...
    if (SOC_FAILURE(rv)) {
        LOG_CLI((BSL_META_U(unit,
                            "Failed to partition FTE space: %d %s\n"),
                 rv, soc_errmsg(rv)));
        return rv;
    }
...
/* FIX: please leave this in the code until SAL_BOOT_BCMSIM is working */
    ...
    if (!sTuningParams) {
        if (feip->ddrConfigRw) {
            LOG_CLI((BSL_META_U(unit,
                                "Loading DDR Tuning Parameters from NV\n")));
            rv = feip->ddrConfigRw(unit, 0, feip->ddrconfig);
            if (rv) {
                /* error no valid tuning paramters */
                LOG_CLI((BSL_META_U(unit,
                                    "ERROR:No DDR tuning parameters found.  Run exttsbx and ddrconfig\n")));
    ...
    if (rv) {
        LOG_CLI((BSL_META_U(unit,
                            "sbFe2000XtInit() failed with error code:0x%x\n"), rv));
        return SOC_E_INIT;
    }
...
/* FIX: this crashes bcmsim */
    ...
    if (rv) {
        LOG_CLI((BSL_META_U(unit,
                            "sbFe2000XtEnablePr() failed with error code:0x%x\n"), rv));
        return SOC_E_INIT;
    }
                            ...
                            soc_cm_isr_func_t isr)
{
    ...
    default:
        LOG_CLI((BSL_META_U(unit,
                            "soc_sbx_fe2000_register_isr invalid isrtype: %d\n"),
                 isrtype));
    }
    SOC_SBX_WARM_BOOT_OBSERVE(unit, wb);
}
...
soc_sbx_fe2000_isr(void *_unit)
{
    ...
    for (;;) {
        ...
        LOG_INFO(BSL_LS_SOC_INTR,
                 (BSL_META_U(unit,
                             "%d:fe2000_isr: unit interrupts: 0x%08x unit_int_mask: 0x%08x\n"),
        ...
        if (v & (SAND_HAL_FE2000_FIELD_MASK(unit, PC_INTERRUPT, DMA_0_DONE)
         ...
         LOG_INFO(BSL_LS_SOC_INTR,
                  (BSL_META_U(unit,
                              "%d:fe2000_isr: link status change interrupt NOT IMPLEMENTED\n"),
                   unit));
          ...
          LOG_INFO(BSL_LS_SOC_INTR,
                   (BSL_META_U(unit,
                               "%d:fe2000_isr:MIIM op complete NOT IMPLEMENTED"),
                    unit));
          SAND_HAL_FE2000_WRITE(unit, PC_INTERRUPT,
          ...
          SAND_HAL_FE2000_WRITE(unit, PC_INTERRUPT_MASK, SOC_IRQ_MASK(unit));
            ...
            LOG_INFO(BSL_LS_SOC_INTR,
                     (BSL_META_U(unit,
                                 "%d:fe2000_isr:Counter Ring FIFO Data avail\n"),
                      unit));
#endif
            (sbx->lib_isr)(_unit, v);
            SAND_HAL_FE2000_WRITE(unit, PC_INTERRUPT,
          ...
          LOG_INFO(BSL_LS_SOC_INTR,
                   (BSL_META_U(unit,
                               "%d:fe2000_isr:unimplemented PCI error0 "
                               "interrupt 0x%x\n"),
                    unit,
          ...
          LOG_INFO(BSL_LS_SOC_INTR,
                   (BSL_META_U(unit,
                               "%d:fe2000_isr:unimplemented PCI error1 "
                               "interrupt 0x%x\n"),
                    unit,
          ...
          SAND_HAL_FE2000_WRITE(unit, PC_ERROR1_MASK, 0xffffffff);
            ...
            if ((unit_v & SAND_HAL_FE2000_FIELD_MASK(unit, PC_UNIT_INTERRUPT, PM))
                ...
                LOG_INFO(BSL_LS_SOC_INTR,
                         (BSL_META_U(unit,
                                     "%d:fe2000_isr:PMU unit interrupt\n"),
                          unit));
                ...
                LOG_INFO(BSL_LS_SOC_INTR,
                         (BSL_META_U(unit,
                                     "%d:fe2000_isr:PMU unit interrupt (unsupported) 0x%x\n"),
                          unit, v));
                ...
                LOG_INFO(BSL_LS_SOC_INTR,
                         (BSL_META_U(unit,
                                     "%d:fe2000_isr:CM unit interrupt\n"),
                          unit));
                (sbx->lib_isr)(_unit, v);
                ...
                LOG_INFO(BSL_LS_SOC_INTR,
                         (BSL_META_U(unit,
                                     "%d:fe2000_isr:unimplemented unit interrupt 0x%x "
                                     "(0x%x/0x%x)\n"),
                          unit,
          ...
          LOG_INFO(BSL_LS_SOC_INTR,
                   (BSL_META_U(unit,
                               "%d:fe2000_isr:unimplemented interrupt 0x%x "
                               "(0x%x/0x%x)\n"),
                    unit,
    ...
    if (!(v & SAND_HAL_FE2000_FIELD_MASK(unit, PC_INTERRUPT_MASK, UNIT_DISINT) )) {
        ...
        LOG_INFO(BSL_LS_SOC_INTR,
                 (BSL_META_U(unit,
                             "%d:fe2000_isr:re-enable unit_interrupt_mask: 0x%08x\n"),
                  unit, unit_v_mask));
    ...
    LOG_INFO(BSL_LS_SOC_INTR,
             (BSL_META_U(unit,
                         "%d:fe2000_isr:re-enable pc_interrupt_mask: 0x%08x\n"),
              unit, v));
...
_soc_sbx_fe2000_port_info_load(int unit)
{
    ...
    if (num_ports_found != ucode_num_ports) {
        LOG_WARN(BSL_LS_SOC_COMMON,
                 (BSL_META_U(unit,
                             "Found %d ucode_port connections, "
