...
soc_sbx_g2xx_constant_shell_print(int unit, int cid)
{
    ...
    if (rv) {
        LOG_CLI((BSL_META_U(unit,
                            "constant %s get timeout\n"),
                 tm->constants[cid].name));
        return rv;
    }

    LOG_CLI((BSL_META_U(unit,
                        "%s="), tm->constants[cid].name));
...
soc_sbx_g2xx_global_shell_print(int unit, int gid)
{
    ...
    if (rv) {
        LOG_CLI((BSL_META_U(unit,
                            "global %s get timeout\n"),
                 tm->globals[gid].name));
        return rv;
    }

    LOG_CLI((BSL_META_U(unit,
                        "%s="), tm->globals[gid].name));
         ...
         int mid, int argc, char **argv)
{
    ...
    if (rv) {
        LOG_CLI((BSL_META_U(unit,
                            "memory read from %s[0x%x] failed\n"),
    ...
    LOG_CLI((BSL_META_U(unit,
                        "%s[0x%x]="),  tm->memories[mid].name, addr));
    if (name) {
        LOG_CLI((BSL_META_U(unit,
                            "%s[0x%x]="), name, offset));
    }
    ...
    if (name) {
        for (i = 0; i < SOC_SBX_G2XX_TABLE_MAX_ID; i++) {
            ...
            if (!sal_strcasecmp(td->name, name)) {
                ...
                LOG_CLI((BSL_META_U(unit,
                                    "Bank  Memory    Port  Width  Base     Size\n")));
                LOG_CLI((BSL_META_U(unit,
                                    "-----------------------------------------"
                                    "-----\n")));
                for (j = 0; j < td->nbanks; j++) {
                    LOG_CLI((BSL_META_U(unit,
                                        "%-6d%-11s%-5d%-7d0x%06x 0x%06x\n"),
