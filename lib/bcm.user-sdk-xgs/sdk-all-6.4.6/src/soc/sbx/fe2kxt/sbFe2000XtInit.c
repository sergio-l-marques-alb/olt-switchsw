...
/*****************************************************************************
 * $Id: sbFe2000XtInit.c,v 1.133 Broadcom SDK $
 *
...
static uint32 CalculateLrLoaderEpoch(sbFe2000XtInitParams_t *pInitParams);
static uint32 sbFe2000XtInitRegDone(sbhandle userDeviceHandle, uint32 uCtlReg,uint32 uDone, uint32 uTimeOut);
static uint32 sbFe2000XtInitQm(sbhandle userDeviceHandle, sbFe2000XtInitParams_t *pInitParams);
...
static void sbFe2000XtInitRc(uint32 uInstance,sbhandle userDeviceHandle, sbFe2000XtInitParams_t *pInitParams);
static uint32 sbFe2000XtInitMm(uint32 uInstance,sbhandle userDeviceHandle, sbFe2000XtInitParams_t *pInitParams);
static void sbFe2000XtInitPm(sbhandle userDeviceHandle, sbFe2000XtInitParams_t *pInitParams);
static uint32 sbFe2000XtInitLr(sbhandle userDeviceHandle, sbFe2000XtInitParams_t *pInitParams);
static uint32 sbFe2000XtInitCoCounters(sbhandle userDeviceHandle, sbFe2000XtInitParams_t *pInitParams);
static uint32 sbFe2000XtInitPt(sbhandle userDeviceHandle, sbFe2000XtInitParams_t *pInitParams);
static uint32 sbFe2000XtInitPr(sbhandle userDeviceHandle, sbFe2000XtInitParams_t *pInitParams);
static uint32 sbFe2000XtInitPp(sbhandle userDeviceHandle, sbFe2000XtInitParams_t *pInitParams);
static uint32 sbFe2000XtInitSpi(sbhandle userDeviceHandle, uint16 nNum, sbFe2000XtInitParams_t *pInitParams);
static uint32 sbFe2000XtInitSpiTx(sbhandle userDeviceHandle, uint16 nSpi, sbFe2000XtInitParams_t *pInitParams);
static uint32 sbFe2000XtInitSpiRx(sbhandle userDeviceHandle, uint16 nSpi, sbFe2000XtInitParams_t *pInitParams);
static uint32 sbFe2000XtInitXgm(sbhandle userDeviceHandle, uint16 nMac, sbFe2000XtInitParams_t *pInitParams);
static uint32 sbFe2000XtInitAgmPortRx(sbhandle userDeviceHandle, uint16 nAgmNum, uint16 nPort, sbFe2000XtInitParams_t *pInitParams);
static uint32 sbFe2000XtInitAgmPortTx(sbhandle userDeviceHandle, uint16 nAgmNum, uint16 nPort, sbFe2000XtInitParams_t *pInitParams);
static int agmMacConfigCalcAddr(sbhandle userDeviceHandle, int nAgmNum, int nPort);
static uint32 sbFe2000XtInitAgm(sbhandle userDeviceHandle, uint16 nAgmNum, sbFe2000XtInitParams_t *pInitParams);
static uint32 sbFe2000XtInitAgms(sbhandle userDeviceHandle, sbFe2000XtInitParams_t *pInitParams);
static uint32 sbFe2000XtTakeXgXsOutOfReset(sbhandle userDeviceHandle, uint32 nMac);
...
static void sbFe2000XtInitCleanUpMm(sbhandle userDeviceHandle, uint16 nNum);
static void sbFe2000XtInitCleanUpCm(sbhandle userDeviceHandle);
static void sbFe2000XtInitCleanUpRc(sbhandle userDeviceHandle, uint16 nNum);
...
static void sbFe2000XtInitCleanUpAgm(sbhandle userDeviceHandle, uint16 nNum);
static void sbFe2000XtInitCleanUpXgm(sbhandle userDeviceHandle, uint16 nNum);
static void sbFe2000XtInitCleanUpSpi4(sbhandle userDeviceHandle, uint16 nNum);
static void sbFe2000XtInitCleanUpAgmPortRx(uint16 nNum, uint16 nPort, sbhandle userDeviceHandle);
static void sbFe2000XtInitCleanUpAgmPortTx(uint16 nNum, uint16 nPort, sbhandle userDeviceHandle);
static uint32 sbFe2000XtInitPLL(sbhandle userDeviceHandle,sbFe2000XtInitParams_t *pInitParams);
static uint32 sbFe2000XtInitPsyncSlipBuffers(sbhandle userDeviceHandle,sbFe2000XtInitParams_t *pInitParams);
...
sbFe2000XtInitDefaultParams(int unit, sbFe2000XtInitParams_t *pInitParams)
{
    ...
    for ( nXgmii = 0; nXgmii < SB_FE2000XT_NUM_XG_INTERFACES; nXgmii++ ) {
        ...
        COMPILER_64_SET(pInitParams->xgt[nXgmii].uuE2eccMacDa, 0xFFFF, 0xFFFFFFFF);
        COMPILER_64_ZERO(pInitParams->xgt[nXgmii].uuE2eccMacSa);
    ...
    for( uIndex = SB_FE2000_IF_PTYPE_AGM0; uIndex <=SB_FE2000_IF_PTYPE_XGM1; uIndex++ ) {
        ...
        pInitParams->pr.bCcUsePortSteering[uIndex - 2] = FALSE;
        pInitParams->pr.bCcUsePacketSteering[uIndex - 2] = FALSE;
    }
...
CalculateLrLoaderEpoch(sbFe2000XtInitParams_t *pInitParams)
{
  ...
  uint64 fIngressLoadTime;
  uint64 fEgressLoadTime;
#if 0
  int64 fXferTime;
  int64 fFirstTime;
  int64 fIntermediate;
...
/*uint64 fClockScale = (SB_FE2000XT_PIPELINE_CLOCK_RATE / SB_FE2000XT_CORE_CLOCK_RATE);*/
  ...
  COMPILER_64_ADD_32(fEgressLoadTime, pInitParams->lr.uLoadCount);
  COMPILER_64_ADD_32(fEgressLoadTime, 4);

  if( pInitParams->lr.bPairedMode == TRUE ) {
    uint64 uuTmp;
    uuTmp = fIngressLoadTime;
    COMPILER_64_ADD_64(uuTmp,fEgressLoadTime);
    COMPILER_64_UMUL_32(uuTmp, ClockScale); 
    ...
    uuTmp = fIngressLoadTime;
    COMPILER_64_UMUL_32(uuTmp, ClockScale);
    uLoaderEpoch = COMPILER_64_LO(uuTmp) + 14;
  }
             ...
             sbFe2000XtInitParams_t *pInitParams)
{
  ...
  if (status != SB_FE2000_STS_INIT_OK_K)
    {
        LOG_CLI((BSL_META("sbFe2000XtInitPLL() failed with error code:0x%x\n"), status));
        return status;
    }
  ...
  if (status != SB_FE2000_STS_INIT_OK_K)
    {
        LOG_CLI((BSL_META("sbFe2000XtInitQm() failed with error code:0x%x\n"), status));
        return status;
    }
  ...
  if (status != SB_FE2000_STS_INIT_OK_K)
    {
        LOG_CLI((BSL_META("sbFe2000XtInitSpi[0] failed with error code:0x%x\n"), status));
        return status;
    }
  ...
  if (status != SB_FE2000_STS_INIT_OK_K)
    {
        LOG_CLI((BSL_META("sbFe2000XtInitSpi[1] failed with error code:0x%x\n"), status));
        return status;
    }
  ...
  for(i=0;i< SB_FE2000XT_NUM_XG_INTERFACES;i++) {
    if (pInitParams->xg[i].bBringUp) {
        ...
        if (status != SB_FE2000_STS_INIT_OK_K)
        {
            LOG_CLI((BSL_META("sbFe2000XtInitXgm [interface %d] failed with error code:0x%x\n"), i, status));
            return status;
        }
    }
  }
  ...
  if (status != SB_FE2000_STS_INIT_OK_K)
  {
      LOG_CLI((BSL_META("sbFe2000XtInitAgms failed with error code:0x%x\n"), status));
      return status;
  }
  ...
  if (status != SB_FE2000_STS_INIT_OK_K)
    {
        LOG_CLI((BSL_META("sbFe2000XtInitPt failed with error code:0x%x\n"), status));
        return status;
    }
  ...
  if (status != SB_FE2000_STS_INIT_OK_K)
    {
        LOG_CLI((BSL_META("sbFe2000XtInitPr failed with error code:0x%x\n"), status));
        return status;
    }
  ...
  if (status != SB_FE2000_STS_INIT_OK_K)
    {
        LOG_CLI((BSL_META("sbFe2000XtInitPp failed with error code:0x%x\n"), status));
        return status;
    }
  ...
  for(i=0;i<SB_FE2000_NUM_MM_INSTANCES;i++) {
    ...
    if (status != SB_FE2000_STS_INIT_OK_K)
    {
        LOG_CLI((BSL_META(" sbFe2000XtInitMm instance %d failed with error code:0x%x\n"),i, status));
        return status;
    }
  }
  ...
  if (status != SB_FE2000_STS_INIT_OK_K) {
      LOG_CLI((BSL_META("sbFe2000XtInitLr failed with error code:0x%x\n"), status));
      return status;
  }
...
#define SB_FE2000_CALENDAR_ENTRIES_PER_REGISTER 4
  ...
  if (!pInitParams->bSimulation) {
    ...
    if ( !uTimeOut ) {
      ...
      LOG_ERROR(BSL_LS_SOC_COMMON,
                (BSL_META("ST%d PLL failed to lock!  Status 0x%x"), nSpi, uStStatus));
    } else {
      LOG_INFO(BSL_LS_SOC_COMMON,
               (BSL_META("ST%d PLL locked."), nSpi));
    }
...
static uint32
sbFe2000XtInitXgm(sbhandle userDeviceHandle, uint16 nMac,
                sbFe2000XtInitParams_t *pInitParams)
{
  ...
  uint8 bClause45 = TRUE;
  uint32 uXG_debug = 0;
#if 1
  uint8 bInternalPhy = TRUE;
  uint32 uDevAddr = 1;
  uint32 uMdioData;
#endif
  DENTER();
  ...
  for ( nNum = 0; nNum < SB_FE2000XT_NUM_XG_INTERFACES; ++nNum ) {
    ...
    pClk->SetClock(nNum, (uint32)fClkXgXtorRef,  50,  0, 0, 0);
  }
  pClk->ProgramClocks();
#endif
  ...
  if (!pInitParams->bSimulation) {
    ...
    sbFe2000UtilXgmMiimRead( userDeviceHandle, bInternalPhy, bClause45, uDevAddr, uPhyOrPortAddr, (uint32) 0xffde, &uMdioData);
    ...
    sbFe2000UtilXgmMiimRead( userDeviceHandle, bInternalPhy, bClause45, uDevAddr, uPhyOrPortAddr, (uint32) 0x8000, &uMdioData);
    sbFe2000UtilXgmMiimWrite(userDeviceHandle, bInternalPhy, bClause45, uDevAddr, uPhyOrPortAddr, (uint32) 0x8000, (uMdioData & 0xdfff));
    /* Disable auto-neg */
    sbFe2000UtilXgmMiimRead( userDeviceHandle, bInternalPhy, bClause45, uDevAddr, uPhyOrPortAddr, (uint32) 0xffe0, &uMdioData);
    sbFe2000UtilXgmMiimWrite(userDeviceHandle, bInternalPhy, bClause45, uDevAddr, uPhyOrPortAddr, (uint32) 0xffe0, (uMdioData & 0xefff));
    ...
    sbFe2000UtilXgmMiimRead( userDeviceHandle, bInternalPhy, bClause45, uDevAddr, uPhyOrPortAddr, (uint32) 0x8308, &uMdioData);
    ...
    sbFe2000UtilXgmMiimWrite(userDeviceHandle, bInternalPhy, bClause45, uDevAddr, uPhyOrPortAddr, (uint32) 0x8308, uMdioData);
    if ( pInitParams->xg[nMac].bSerdesLocalLoopback == TRUE ) {
      sbFe2000UtilXgmMiimWrite(userDeviceHandle, bInternalPhy, bClause45, uDevAddr, uPhyOrPortAddr, (uint32) 0xffde, 0x0800);
      sbFe2000UtilXgmMiimRead(userDeviceHandle,  bInternalPhy, bClause45, uDevAddr, uPhyOrPortAddr, (uint32) 0x0000, &uMdioData);
      sbFe2000UtilXgmMiimWrite(userDeviceHandle, bInternalPhy, bClause45, uDevAddr, uPhyOrPortAddr, (uint32) 0x0000, (uMdioData | 0x0001));
      sbFe2000UtilXgmMiimWrite(userDeviceHandle, bInternalPhy, bClause45, uDevAddr, uPhyOrPortAddr, (uint32) 0xffde, 0x0000);
    }
    /* Set start sequencer bit.*/
    sbFe2000UtilXgmMiimRead( userDeviceHandle, bInternalPhy, bClause45, uDevAddr, uPhyOrPortAddr, (uint32) 0x8000, &uMdioData);
    sbFe2000UtilXgmMiimWrite(userDeviceHandle, bInternalPhy, bClause45, uDevAddr, uPhyOrPortAddr, (uint32) 0x8000, (uMdioData | 0x2000));
  }
...
sbStatus_t
sbFe2000XtInitXgmRxTx(sbhandle userDeviceHandle, uint16 nMac,
                 sbFe2000XtInitParams_t *pInitParams)
{
  uint32 uDataLo;
  uint32 uDataHi;
...
static uint32
sbFe2000XtInitAgmPortRx(sbhandle userDeviceHandle, uint16 nAgmNum, uint16 nPort,
              sbFe2000XtInitParams_t *pInitParams)
{
  ...
  uStatus = sbFe2000UtilAgmWrite(userDeviceHandle, nAgmNum, nPort,
                 ...
                 ((COMPILER_64_HI(pInitParams->ag[nAgmNum].common[nPort].uuMacAddr) & 0xffff << 16) |
                  (COMPILER_64_LO(pInitParams->ag[nAgmNum].common[nPort].uuMacAddr) >> 16)));
  if(SB_FE2000_STS_INIT_OK_K != uStatus) {
    return uStatus;
  }
...
static uint32
sbFe2000XtInitAgmPortTx(sbhandle userDeviceHandle, uint16 nAgmNum, uint16 nPort,
              sbFe2000XtInitParams_t *pInitParams)
{
...
static uint32
sbFe2000XtTakeXgXsOutOfReset(sbhandle userDeviceHandle, uint32 nMac) {
  uint32 uDataLo, uDataHi, uStatus;
...
static uint32
sbFe2000XtInitAgm(sbhandle userDeviceHandle, uint16 nAgmNum, sbFe2000XtInitParams_t *pInitParams)
{
  ...
  uStatus = sbFe2000UtilAgmWrite(userDeviceHandle, nAgmNum, SB_FE2000_MAX_AG_PORTS, SAND_HAL_REG_OFFSET(C2, GPORT_MAX_PKT_SIZE_STATUS_UPDATE),
    ...
    if ( (!pInitParams->bSimulation) &&
      ...
      sbFe2000UtilXgmMiimRead( userDeviceHandle, bInternalPhy, bClause45, uDevAddr, uPhyOrPortAddr, (uint32) 0x8000, &uMdioDataSave);
      uMdioDataSave &= 0xd0ff;
      uMdioDataSave |= 0x0600;
      sbFe2000UtilXgmMiimWrite( userDeviceHandle, bInternalPhy, bClause45, uDevAddr, uPhyOrPortAddr, (uint32) 0x8000, uMdioDataSave);
      /* Enable lanes via XGXS BLOCK1 "laneCtrl0" register */
      sbFe2000UtilXgmMiimRead(  userDeviceHandle, bInternalPhy, bClause45, uDevAddr, uPhyOrPortAddr, (uint32) 0x8015, &uMdioData);
      ...
      sbFe2000UtilXgmMiimWrite( userDeviceHandle, bInternalPhy, bClause45, uDevAddr, uPhyOrPortAddr, (uint32) 0x8015, uMdioData);
    ...
    /* Set start sequencer bit.*/
    sbFe2000UtilXgmMiimWrite(userDeviceHandle, bInternalPhy, bClause45, uDevAddr, uPhyOrPortAddr, (uint32) 0x8000, (uMdioDataSave | 0x2000));
           ...
           sbFe2000XtInitParams_t *pInitParams)
{
  ...
  {
    uint32 uPdConfig = SAND_HAL_READ(userDeviceHandle, C2,PD_CONFIG);
  ...
  {
  uint32 uPdDebug = SAND_HAL_READ(userDeviceHandle, C2,PD_DEBUG);
...
static void
sbFe2000XtInitRc(uint32 uInstance, sbhandle userDeviceHandle,
           sbFe2000XtInitParams_t *pInitParams)
{
  DENTER();
  if (uInstance == 0) {
           ...
           sbFe2000XtInitParams_t *pInitParams)
{
  ...
  for( uGroup = 0; uGroup < SB_FE2000_PM_TOTAL_NUM_GROUPS; uGroup++ ) {
    uint32 uBaseRegAddress = SAND_HAL_REG_OFFSET(C2,PM_GROUP0_CONFIG0)+(16*uGroup);
  ...
  {
    uint32 uPmConfig = SAND_HAL_READ(userDeviceHandle, C2, PM_CONFIG0);
           ...
           sbFe2000XtInitParams_t *pInitParams)
{
    ...
    int i, j;
    int nLrImemSize;
    uint32 nClocksPerUs = 0;

#ifdef PLISIM
    nLrImemSize = SB_FE2000XT_LR_NUMBER_OF_STREAMS * SB_FE2000XT_LR_NUMBER_OF_INSTRS;
#else
    nLrImemSize = SAND_HAL_C2_LR_IMEM_ACC_CTRL_ADDRESS_MASK + 1;
#endif
    ...
    SAND_HAL_WRITE(userDeviceHandle, C2,  LR_BA_IMEM0_DEBUG, 0);
    SAND_HAL_WRITE(userDeviceHandle, C2,  LR_BA_IMEM1_DEBUG, 0);
    SAND_HAL_WRITE(userDeviceHandle, C2,  LR_BA_IMEM2_DEBUG, 0);
    SAND_HAL_WRITE(userDeviceHandle, C2,  LR_BA_IMEM3_DEBUG, 0);
    SAND_HAL_WRITE(userDeviceHandle, C2,  LR_BA_IMEM4_DEBUG, 0);
    SAND_HAL_WRITE(userDeviceHandle, C2,  LR_BA_IMEM5_DEBUG, 0);
    SAND_HAL_WRITE(userDeviceHandle, C2,  LR_BA_IMEM6_DEBUG, 0);
    SAND_HAL_WRITE(userDeviceHandle, C2,  LR_BA_IMEM7_DEBUG, 0);
...
#undef UPDATE
    ...
    /*
    nClocksPerUs = (int32) floor( (((uint64) 0xffffffff) * SB_FE2000XT_NSEC_PER_PIPELINE_CLOCK) / \
                             ((uint64) 1000)  );
    */

    /*
    nClocksPerUs = (int32) floor( (((uint64) 0xffffffff) * (1000000 / pInitParams->uHppFreq)) / \
                             ((uint64) 1000)  );
    */

    {uint64 uuTmp;
        COMPILER_64_SET(uuTmp, 0, 0xffffffff);
        COMPILER_64_UMUL_32(uuTmp, (1000000 / pInitParams->uHppFreq));
        if (soc_sbx_div64(uuTmp, 1000, &nClocksPerUs) == SOC_E_PARAM) return SOC_E_INTERNAL;
    }
#else
        
#endif

    SAND_HAL_WRITE( userDeviceHandle, C2, LU_CLOCKS_PER_US, (int32) nClocksPerUs);
...
static int
_MemDiagUpdateLrpEpoch(sbhandle userDeviceHandle, uint32 epoch)
{
  uint32 uTimeOut;
  uint32 uUpdateEvent;
...
static
uint32 _MemDiagWaitForPhase(sbhandle userDeviceHandle, uint32 phase)
{
  uint32 uTimeout = 0;
  uint32 uDone = 0;
...
static
uint32 _MemDiagEnableProcessing(sbhandle userDeviceHandle)
{
...
_MemDiagCheckForErrors(sbhandle userDeviceHandle) {
  uint32 uError = SAND_HAL_READ(userDeviceHandle, C2, LU_ERROR);
  if (uError) {
    /* LOG_ERROR(BSL_LS_SOC_COMMON,
             (BSL_META("MemDiagCheckForErrors LU ERRORS 0x%x \n"), uError)); */
  }
  return uError;
}
static void 
           ...
           sbFe2000XtInitParams_t *pInitParams)
{
  ...
  sbZfC2LrLrpInstructionMemoryBank0Entry_t zLrpInstructionMemoryBank0ClearEntry;
    ...
    LOG_INFO(BSL_LS_SOC_COMMON,
             (BSL_META("start Clearing Coherent Counters\n")));
  ...
  if (soc_sbx_fe2000_ucode_load_from_buffer((int)userDeviceHandle, soc_sbx_fe2kxt_clearcc_fe2kAsm2UcodeEc, sizeof (soc_sbx_fe2kxt_clearcc_fe2kAsm2UcodeEc)) != 0) {
    LOG_ERROR(BSL_LS_SOC_COMMON,
              (BSL_META("Couldnt load clearCc ucode.\n")));
    return SB_FAILED;
  }
  ...
  if(fe2kAsm2IntD__getNc(ucodeBytes, "elen", &elen) > 0){
    LOG_ERROR(BSL_LS_SOC_COMMON,
              (BSL_META("Couldnt read epoch loength from clearCc ucode.\n")));
    SB_ASSERT(0);
  }
  ...
  if (status != SB_OK) {
    LOG_ERROR(BSL_LS_SOC_COMMON,
              (BSL_META("Couldnt write clearCc ucode.\n")));
    return status;
  }

  if (_MemDiagUpdateLrpEpoch(userDeviceHandle, pInitParams->lr.uEpochLengthInCycles) != 0) {
    LOG_ERROR(BSL_LS_SOC_COMMON,
              (BSL_META("Couldnt upate lrp epoch for clearCc ucode.\n")));
    return SB_FAILED;
  }
  ...
  if ( _MemDiagEnableProcessing(userDeviceHandle) != SB_OK) {
    LOG_ERROR(BSL_LS_SOC_COMMON,
              (BSL_META("Couldnt start clearCc ucode.\n")));
    return SB_FAILED;
  }
  ...
  if ( _MemDiagWaitForPhase(userDeviceHandle, 0) ) {
    LOG_ERROR(BSL_LS_SOC_COMMON,
              (BSL_META("Clear Coherent Counters timeout,  FAIL\n")));
  } else if ( _MemDiagCheckForErrors(userDeviceHandle) ) {
    LOG_INFO(BSL_LS_SOC_COMMON,
             (BSL_META("Clear Coherent Counters  ERRORS!\n")));
    status = SB_FAILED;
  } else {
    LOG_INFO(BSL_LS_SOC_COMMON,
             (BSL_META("Cleared Coherent Counters\n")));
  }
...
static uint32
sbFe2000XtInitMm(uint32 uInstance, sbhandle userDeviceHandle,
           sbFe2000XtInitParams_t *pInitParams)
{
  ...
  uint32 uValidDelay, uTurnAroundDelay, uPhyConfig1, uPhyConfig2, uPhyConfig3, uAddressShift;
  uint32 uMmError;
  uint8 bPvtFailed;
  uint32 uPvtCompAttempt;
  ...
  while ( (uPvtCompAttempt < FE2K_MAX_PVT_COMPENSATION_ATTEMPTS) && (bPvtFailed == 1) ) {
    uPvtCompAttempt++;
    LOG_INFO(BSL_LS_SOC_COMMON,
             (BSL_META("MM%d PVT compensation attempt: %d\n"),
  ...
  if (bPvtFailed) {
    LOG_ERROR(BSL_LS_SOC_COMMON,
              (BSL_META("MM%d PVT compensation: All %d calls to PvtCompensation ended in failure\n"),
               uInstance,uPvtCompAttempt));
  } else {
    LOG_INFO(BSL_LS_SOC_COMMON,
             (BSL_META("MM%d PVT compensation succeeded after %d calls to PvtCompensation\n"),
              uInstance,uPvtCompAttempt));
  }
   ...
   */
    ...
    LOG_WARN(BSL_LS_SOC_COMMON,
             (BSL_META("Invalid memory configuration\n")));
    SB_ASSERT(0);
  }
  ...
  if (sTuningParams) {
      char tmp[31];
      sal_strncpy(tmp, sTuningParams, sizeof(tmp)-1);
      /* if any parameters are present, then all must be present */
      _GET_NEXT_VALUE(tmp, pDdrConfig->uValidDelay[0]);
      ...
      if (sTuningParams == NULL){
          LOG_ERROR(BSL_LS_SOC_COMMON,
                    (BSL_META("Tuning parameter: %s missing\n"), sPropName));
          SB_ASSERT(0);
          return SOC_E_PARAM;
      }
      sal_strncpy(tmp, sTuningParams, sizeof(tmp)-1);
      _GET_NEXT_VALUE(tmp, pDdrConfig->uValidDelay[1]);
      ...
      sal_strncpy(tmp, sTuningParams, sizeof(tmp)-1);
      _GET_NEXT_VALUE(tmp, pDdrConfig->uValidDelay[2]);
  ...
  if (pDdrConfig->bValid){
      ...
      LOG_INFO(BSL_LS_SOC_COMMON,
               (BSL_META("MMInstance%d-NarrowPort0 VALID_DELAY=%d, PHASE_SELECT=%d,\n"), uInstance, pDdrConfig->uValidDelay[0], pDdrConfig->uPhaseSelect[0] ));
      ...
      LOG_INFO(BSL_LS_SOC_COMMON,
               (BSL_META("MMInstance%d-NarrowPort1 VALID_DELAY=%d, PHASE_SELECT=%d,\n"), uInstance, pDdrConfig->uValidDelay[1], pDdrConfig->uPhaseSelect[1] ));
      ...
      LOG_ERROR(BSL_LS_SOC_COMMON,
                (BSL_META("%s:DDR Tuning Parameters not found for MMU 0x%x.  Run exttsbx to perform DDR tuning\n"), FUNCTION_NAME(), uInstance ));
           ...
           sbFe2000XtInitParams_t *pInitParams)
{
  ...
  if ( (pInitParams->ag[0].bBringUp) || (pInitParams->xg[2].bBringUp) ){
      ...
      if( sbFe2000XtInitRegDone(userDeviceHandle,
          ...
          LOG_ERROR(BSL_LS_SOC_COMMON,
                    (BSL_META("%s:Waiting for PT (AG0/XG2) init_done timed out..\n"),
                     FUNCTION_NAME()));
           ...
           sbFe2000XtInitParams_t *pInitParams)
{
  ...
  SAND_HAL_RMW_FIELD(userDeviceHandle, C2, PR_CONFIG, PED_PRIORITY, (uint32)pInitParams->pr.PipelinePriority);
  ...
  if( pInitParams->pr.bCcEnable[0] ) {
      ...
      sbZfC2PrCcTcamConfigEntry_Pack(&zTcamConfigEntry, (uint8*)uCfgBuffer,
  ...
  {
      LOG_CLI((BSL_META("%s: invalid config for PRE6\n"), FUNCTION_NAME()));
...
sbFe2000XtInitPp(sbhandle userDeviceHandle,
...

  {
    /* LRP By Pass Mode configuration */
     uint32 uDebugPedHeader = SAND_HAL_READ(userDeviceHandle, C2,PP_DEBUG_PED_HDR);
     uint32 uPpDebugConfig;
     uint32 uPpMakePedHeader;
     uint32 uPpAddRouteHeader;
     uint32 uPpDeleteFirstHeader;
...
uint32 sbFe2000XtInitConvertPbEnables(sbFe2000XtInitParams_t *pInitParams, uint8 bPre, uint32 uPre)
{
  uint32 uPbEnable = 0;
...
void sbFe2000XtInitCleanUpPt(sbhandle userDeviceHandle)
{
  ...
  uint32 uError = SAND_HAL_READ(userDeviceHandle, C2, PT_PTE0_ERROR0);
  DENTER();
...
void sbFe2000XtInitCleanUpQm(sbhandle userDeviceHandle)
{
  ...
  SAND_HAL_READ(userDeviceHandle, C2, QM_ERROR0);
  DENTER();
  SAND_HAL_READ(userDeviceHandle, C2, QM_ERROR1);
...
void sbFe2000XtInitCleanUpPr(sbhandle userDeviceHandle)
{
  ...
  uint32 uError = SAND_HAL_READ(userDeviceHandle, C2, PR_ERROR);
  DENTER();
...
{
  uint32 uError = SAND_HAL_READ(userDeviceHandle, C2, PP_EVENT0);
  DENTER();
...
static
void sbFe2000XtInitCleanUpMm(sbhandle userDeviceHandle, uint16 nNum)
{
  uint32 uError = SAND_HAL_READ_STRIDE(userDeviceHandle, C2, MM, nNum, MM_ERROR);
  DENTER();
...
{
  uint32 uError = SAND_HAL_READ(userDeviceHandle, C2, CM_SEGMENT_DISABLE_ERROR);
  uint32 uCmLfsrEnable = 0;
  DENTER();
...
static
void sbFe2000XtInitCleanUpRc(sbhandle userDeviceHandle, uint16 nNum)
{
  uint32 uError = SAND_HAL_READ(userDeviceHandle, C2, RC_ERROR);
  DENTER();
...
static
void sbFe2000XtInitCleanUpAgm(sbhandle userDeviceHandle, uint16 nNum)
{
  uint16 nPort;
  uint32 uData;
  DENTER();
...
static
void sbFe2000XtInitCleanUpAgmPortRx(uint16 nNum, uint16 nPort, sbhandle userDeviceHandle)
{
  DENTER();
#ifdef DEBUG_PRINT
  LOG_INFO(BSL_LS_SOC_COMMON,
...
static
void sbFe2000XtInitCleanUpAgmPortTx(uint16 nNum, uint16 nPort, sbhandle userDeviceHandle)
{
  DENTER();
...
static
void sbFe2000XtInitCleanUpXgm(sbhandle userDeviceHandle, uint16 nNum)
{
  DENTER();
  if ( nNum == 0 ) {
  } else if ( nNum == 1 ) {
...
static
void sbFe2000XtInitCleanUpSpi4(sbhandle userDeviceHandle, uint16 nNum)
{
  DENTER();
  DEXIT();
}
                ...
                sbFe2000XtInitParams_t *pInitParams)
{
    ...
    uint32 bLocked;
    uint32 uIndex;
    uint32 uMDivSws, uMDivHpp;
    uint32 uNDivSws, uNDivHpp;
    uint32 uVRngSws, uVRngHpp;
...
void sbFe2000XtStartBubbleTimer(uint32 unit, uint32 startBubble, uint32 size)
{
  ...
  /* vn-0909101655+*/ if (SAL_BOOT_BCMSIM) {
   LOG_VERBOSE(BSL_LS_SOC_COMMON,
               (BSL_META("Skipping %s for sbx bcmsim initialization\n"), FUNCTION_NAME()));
  ...
  SAND_HAL_WRITE( SOC_SBX_SBHANDLE(unit), C2, LU_RTC_SUB_NS_INC, 0xD5555555);
                   ...
                   sbFe2000Queues_t *queues)
{
    ...
    int i, iface, many2one, pbsel, egress, port;
    uint32 mirrorData, uPtConfig, uPtData;
    uint32 unit = UNIT_FROM_HANDLE(userDeviceHandle);
    ...
    for (i = 0; i < queues->n; i++) {
        ...
        if (!egress) {
            ...
            case SB_FE2000_IF_PTYPE_AGM0:
                if (sbFe2000XtImfDriver_PrCc2QueueRamWrite((uint32)userDeviceHandle,
                                                            fromPort,
                                                            fromQid)){
                    return SB_FAILED;
                }
            ...
            case SB_FE2000_IF_PTYPE_AGM1:
                if (sbFe2000XtImfDriver_PrCc3QueueRamWrite((uint32)userDeviceHandle,
                                                            fromPort,
                                                            fromQid)){
                    return SB_FAILED;
                }
            ...
            case SB_FE2000_IF_PTYPE_XGM0:
                if (sbFe2000XtImfDriver_PrCc4QueueRamWrite((uint32)userDeviceHandle,
                                                            fromPort,
                                                            fromQid)){
                    return SB_FAILED;
                }
            ...
            case SB_FE2000_IF_PTYPE_XGM1:
                if (sbFe2000XtImfDriver_PrCc5QueueRamWrite((uint32)userDeviceHandle,
                                                            fromPort,
                                                            fromQid)){
                    return SB_FAILED;
                }
...
sbFe2000XtSetQueueParams(int unit, sbFe2000XtInitParams_t *ip)
{
	...
	/* This is a quick change to manipulate the SPI allocation FIFO
	   through a SOC property. The aggregate interface bandwidth is
	   based on the front panel port speed. Here we just want to effect
	   for ingress into fabric through SPI. */
            ...
            for (; portspergroup; i++) {
                ...
                if (queues->bw[con] >= 10000) {
                    ...
                    if (queues->ifexpmode[c->from.ulInterfacePortType]) {
                        LOG_INFO(BSL_LS_SOC_COMMON,
                                 (BSL_META_U(unit,
                                             "    qid: 0x%x set for expanded mode FIFO\n"), qid));
                    }
                } else {
                    queuesused = 1;
...
sbFe2000XtFlushEccOnSpi(int unit)
{
    ...
    uint32 preConfig, pbPages, pbEnable, prEnable;
    ...
    if (!txBuffer) {
        LOG_CLI((BSL_META_U(unit,
                            "failed to allocate pkt tx buffer\n")));
        return SOC_E_MEMORY;
    }
    ...
    if (!rxBuffer) {
        ...
        LOG_CLI((BSL_META_U(unit,
                            "failed to allocate pkt rx buffer\n")));
        return SOC_E_MEMORY;
    }
    ...
    for (spi=0; spi<2 && SOC_SUCCESS(rv); spi++) {
        ...
        if (SOC_FAILURE(rv)) {
            LOG_CLI((BSL_META_U(unit,
                                "Failed to send ecc flush packet: %d %s\n"),
                     rv, soc_errmsg(rv)));
        }
        ...
        if (SOC_FAILURE(rv)) {
            LOG_CLI((BSL_META_U(unit,
                                "Failed to receive ecc flush packet on spi %d: "
...
sbFe2000XtEccClear(int unit)
{
    ...
    /*vn-0909210830+*/ if (SAL_BOOT_BCMSIM) {
     LOG_VERBOSE(BSL_LS_SOC_COMMON,
                 (BSL_META_U(unit,
                             "Skipping %s for sbx bcmsim initialization\n"), FUNCTION_NAME()));
     ...
     * through the interfaces.*/
        ...
        if (rv) {
            LOG_CLI((BSL_META_U(unit,
                                "Failed to flush ecc on spi: %d %s\n"), 
                     rv, soc_errmsg(rv)));
        }
