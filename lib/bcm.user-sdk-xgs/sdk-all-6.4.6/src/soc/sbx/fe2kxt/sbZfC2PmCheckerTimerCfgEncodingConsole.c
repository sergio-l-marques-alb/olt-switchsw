...
sbZfC2PmCheckerTimerCfgEncoding_Print(sbZfC2PmCheckerTimerCfgEncoding_t *pFromStruct) {
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("C2PmCheckerTimerCfgEncoding:: type=0x%01x"), (unsigned int)  pFromStruct->m_uType));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" interrupt=0x%01x"), (unsigned int)  pFromStruct->m_uInterrupt));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" reset=0x%01x"), (unsigned int)  pFromStruct->m_uReset));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" strict=0x%01x"), (unsigned int)  pFromStruct->m_uStrict));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("\n")));

  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("C2PmCheckerTimerCfgEncoding:: mode32=0x%01x"), (unsigned int)  pFromStruct->m_uMode32));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" deadline=0x%05x"), (unsigned int)  pFromStruct->m_uDeadline));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("\n")));
...
char *
sbZfC2PmCheckerTimerCfgEncoding_SPrint(sbZfC2PmCheckerTimerCfgEncoding_t *pFromStruct, char *pcToString, uint32 lStrSize) {
  uint32 WrCnt = 0x0;
