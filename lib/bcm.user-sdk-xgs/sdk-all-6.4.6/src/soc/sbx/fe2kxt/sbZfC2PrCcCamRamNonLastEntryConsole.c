...
sbZfC2PrCcCamRamNonLastEntry_Print(sbZfC2PrCcCamRamNonLastEntry_t *pFromStruct) {
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("C2PrCcCamRamNonLastEntry:: reserve3=0x%04x"), (unsigned int)  pFromStruct->m_uReserve3));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" reserve2=0x%08x"), (unsigned int)  pFromStruct->m_uReserve2));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("\n")));

  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("C2PrCcCamRamNonLastEntry:: reserve1=0x%03x"), (unsigned int)  pFromStruct->m_uReserve1));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" shift=0x%02x"), (unsigned int)  pFromStruct->m_uShift));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" nextstate=0x%04x"), (unsigned int)  pFromStruct->m_uNextState));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("\n")));

  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("C2PrCcCamRamNonLastEntry:: e2ecc=0x%01x"), (unsigned int)  pFromStruct->m_uE2ECC));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" last=0x%01x"), (unsigned int)  pFromStruct->m_uLast));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("\n")));
...
char *
sbZfC2PrCcCamRamNonLastEntry_SPrint(sbZfC2PrCcCamRamNonLastEntry_t *pFromStruct, char *pcToString, uint32 lStrSize) {
  uint32 WrCnt = 0x0;
