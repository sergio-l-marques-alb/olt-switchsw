...
sbZfFe2000PmProfileSeqGenMemory_Print(sbZfFe2000PmProfileSeqGenMemory_t *pFromStruct) {
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("Fe2000PmProfileSeqGenMemory:: resv0=0x%01x"), (unsigned int)  pFromStruct->uResv0));
  LOG_WARN(BSL_LS_SOC_COMMON,
  ...
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("Fe2000PmProfileSeqGenMemory:: resv1=0x%07x"), (unsigned int)  pFromStruct->uResv1));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" resv2=0x%08x"), (unsigned int)  pFromStruct->uResv2));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("\n")));
...
sbZfFe2000PmProfileSeqGenMemory_SPrint(sbZfFe2000PmProfileSeqGenMemory_t *pFromStruct, char *pcToString, uint32 lStrSize) {
  ...
  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"Fe2000PmProfileSeqGenMemory:: resv0=0x%01x", (unsigned int)  pFromStruct->uResv0);
  ...
  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"Fe2000PmProfileSeqGenMemory:: resv1=0x%07x", (unsigned int)  pFromStruct->uResv1);
