...
sbZfFe2000PmOamTimerConfig_Print(sbZfFe2000PmOamTimerConfig_t *pFromStruct) {
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("Fe2000PmOamTimerConfig:: rate=0x%01x"), (unsigned int)  pFromStruct->uRate));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" deadline=0x%08x"), (unsigned int)  pFromStruct->uDeadline));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" started=0x%01x"), (unsigned int)  pFromStruct->bStarted));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("\n")));

  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("Fe2000PmOamTimerConfig:: strict=0x%01x"), (unsigned int)  pFromStruct->bStrict));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" reset=0x%01x"), (unsigned int)  pFromStruct->bReset));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" interrupt=0x%01x"), (unsigned int)  pFromStruct->bInterrupt));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("\n")));
