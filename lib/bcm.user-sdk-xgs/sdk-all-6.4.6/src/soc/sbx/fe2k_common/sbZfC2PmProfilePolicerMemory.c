...
sbZfC2PmProfilePolicerMemory_Unpack(sbZfC2PmProfilePolicerMemory_t *pToStruct,
  ...
  /* Unpack Member: uType */
  (pToStruct)->uType =  (uint32)  ((pFromData)[2] >> 2) & 0x01;

  /* Unpack Member: uLengthShift */
  (pToStruct)->uLengthShift =  (uint32)  ((pFromData)[3] >> 7) & 0x01;
  (pToStruct)->uLengthShift |=  (uint32)  ((pFromData)[2] & 0x03) << 1;

  /* Unpack Member: bCouplingFlag */
  (pToStruct)->bCouplingFlag =  (uint32)  ((pFromData)[3] >> 6) & 0x01;

  /* Unpack Member: bRFC2698Mode */
  (pToStruct)->bRFC2698Mode =  (uint32)  ((pFromData)[3] >> 5) & 0x01;

  /* Unpack Member: uBktCSize */
  (pToStruct)->uBktCSize =  (uint32)  ((pFromData)[3] ) & 0x1f;

  /* Unpack Member: bBlind */
  (pToStruct)->bBlind =  (uint32)  ((pFromData)[4] >> 7) & 0x01;

  /* Unpack Member: bBktENoDecrement */
  (pToStruct)->bBktENoDecrement =  (uint32)  ((pFromData)[4] >> 6) & 0x01;

  /* Unpack Member: uEIR */
  (pToStruct)->uEIR =  (uint32)  ((pFromData)[6] >> 4) & 0x0f;
  (pToStruct)->uEIR |=  (uint32)  (pFromData)[5] << 4;
  (pToStruct)->uEIR |=  (uint32)  ((pFromData)[4] & 0x3f) << 12;

  /* Unpack Member: uEBS */
  (pToStruct)->uEBS =  (uint32)  (pFromData)[7] ;
  (pToStruct)->uEBS |=  (uint32)  ((pFromData)[6] & 0x0f) << 8;

  /* Unpack Member: bDropOnRed */
  (pToStruct)->bDropOnRed =  (uint32)  ((pFromData)[8] >> 7) & 0x01;

  /* Unpack Member: bBktCNoDecrement */
  (pToStruct)->bBktCNoDecrement =  (uint32)  ((pFromData)[8] >> 6) & 0x01;

  /* Unpack Member: uCIR */
  (pToStruct)->uCIR =  (uint32)  ((pFromData)[10] >> 4) & 0x0f;
  (pToStruct)->uCIR |=  (uint32)  (pFromData)[9] << 4;
  (pToStruct)->uCIR |=  (uint32)  ((pFromData)[8] & 0x3f) << 12;

  /* Unpack Member: uCBS */
  (pToStruct)->uCBS =  (uint32)  (pFromData)[11] ;
  (pToStruct)->uCBS |=  (uint32)  ((pFromData)[10] & 0x0f) << 8;
...
uint32 sbZfC2PmProfilePolicerMemory_Pack32( sbZfC2PmProfilePolicerMemory_t *pFrom, 
                                                ...
                                                uint32 nMaxToDataIndex)
{
  uint8 uBuffer[SB_ZF_C2_PM_PROFPOLICERMEMORY_SIZE_IN_WORDS*4];
  uint32 uIndex =0;
  uint32 *pBuffer = NULL;
  uint32 uNumberOfWords = SB_ZF_C2_PM_PROFPOLICERMEMORY_SIZE_IN_WORDS;
  sbZfC2PmProfilePolicerMemory_Pack(pFrom , &uBuffer[0], SB_ZF_C2_PM_PROFPOLICERMEMORY_SIZE_IN_BYTES);
  pBuffer = (uint32 *) &uBuffer[0];
                                                 ...
                                                 uint32 nMaxToDataIndex)
{
  ...
  uint32 uBuffer[SB_ZF_C2_PM_PROFPOLICERMEMORY_SIZE_IN_WORDS*4];
  uint32 uNumberOfWords = SB_ZF_C2_PM_PROFPOLICERMEMORY_SIZE_IN_WORDS;
  uint32 uIndex =0;
  ...
  sbZfC2PmProfilePolicerMemory_Unpack(pToData , (uint8 *) &uBuffer[0], SB_ZF_C2_PM_PROFPOLICERMEMORY_SIZE_IN_BYTES);
  return uNumberOfWords;
}
