...
#define PHY82780_SET_SYS_SIDE(x)    (x).access.flags |= (1 << INTERFACE_SIDE_SHIFT)
#define PHY82780_CLEAR_SYS_SIDE(x)    (x).access.flags &= ~(1 << INTERFACE_SIDE_SHIFT)
...
STATIC int
phy_82780_firmware_set(int unit, int port, int offset, uint8 *array,int datalen)
{
...
phy_82780_per_lane_control_set(int unit, soc_port_t port, int lane, soc_phy_control_t type, uint32 value)
{
    ...
    case SOC_PHY_CONTROL_RX_VGA_RELEASE:
        rv = phy_82780_per_lane_rx_vga_set(pmc, intf, lane, 0 /* release */, 0x8000);
        break;
    /* TX LANE SQUELCH */
    case SOC_PHY_CONTROL_TX_LANE_SQUELCH:
