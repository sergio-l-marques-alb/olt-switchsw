  ...
  )
{
    ...
    if ((SOC_IS_JERICHO(unit)) && SOC_DPP_CONFIG(unit)->emulation_system && (mem_size > 256)
...
exit:
    ...
    if (((unsigned_i = sal_time() - start_time)) >= 5) {
        LOG_INFO(BSL_LS_SOC_INIT, ("==> dcmn_fill_table_with_entry(%s) ran for %u:%2.2u\n",
          SOC_MEM_NAME(unit, mem), unsigned_i / 60, unsigned_i % 60));
    }
    if (dcmn_runtime_debug_per_device[unit].run_stage <= dcmn_runtime_debug_state_initializing) {
        ...
        if (unsigned_i >= soc_mem_entry_words(unit, mem)) {
            LOG_INFO(BSL_LS_SOC_INIT, ("==> dcmn_fill_table_with_entry(%s) received a zero entry\n", SOC_MEM_NAME(unit, mem)));
        }
    }
#endif /* DCMN_RUNTIME_DEBUG */
#if !defined(PALLADIUM_BACKDOOR) && !defined(JERICHO_EMULATION_OLD_ACCELERATION_BEHAVIOR)
    fast_exit:
#endif
    SOCDNX_FUNC_RETURN;
}
...
#ifdef DCMN_RUNTIME_DEBUG
    if (((unsigned_i = sal_time() - start_time)) >= 5) {
        LOG_INFO(BSL_LS_SOC_INIT, ("==> dcmn_fill_partial_table_with_entry(%s) ran for %u:%2.2u\n",
          SOC_MEM_NAME(unit, mem), unsigned_i / 60, unsigned_i % 60));
    }
    if (dcmn_runtime_debug_per_device[unit].run_stage <= dcmn_runtime_debug_state_initializing) {
        for (unsigned_i = 0; unsigned_i < soc_mem_entry_words(unit, mem); ++unsigned_i) {
        ...
        if (unsigned_i >= soc_mem_entry_words(unit, mem)) {
            LOG_INFO(BSL_LS_SOC_INIT, ("==> dcmn_fill_partial_table_with_entry(%s) received a zero entry\n", SOC_MEM_NAME(unit, mem)));
        }
    }
#endif /* DCMN_RUNTIME_DEBUG */
    SOCDNX_FUNC_RETURN;
...
#ifdef BCM_88950_A0
    if (SOC_IS_FE3200(unit) && mem == DCL_CPU_Hm)
    {
        MEM_LOCK(unit, mem);
        rv = soc_fe3200_fabric_cell_dcl_cpu_wide_memory_read(unit, copyno, entry_data);
        if (rv != SOC_E_NONE)
        {
            cli_out("unit %d: soc_fe3200_fabric_cell_dcl_cpu_wide_memory_read failed", unit);
        }
        goto done;
    }
#endif
