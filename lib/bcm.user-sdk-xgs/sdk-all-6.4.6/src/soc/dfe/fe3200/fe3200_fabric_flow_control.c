    ...
    if (SOC_DFE_IS_FE13(unit) && SOC_DFE_CONFIG(unit).fabric_local_routing_enable)
    {
        switch (nof_pipes)
        {
            case 1:
                ...
                break;
            case 2:
                dcm_thresholds_default_values-> local_switch_fifo_size[0] = (SOC_DFE_FABRIC_PIPES_CONFIG(unit).mapping_type == soc_dcmn_fabric_pipe_map_dual_tdm_non_tdm) ? SOC_FE3200_DCM_DEFAULT_FIFO_SIZE_LOCAL_SWITCH_IP_TDM_DUAL_PIPE : SOC_FE3200_DCM_DEFAULT_FIFO_SIZE_LOCAL_SWITCH_UC_MC_DUAL_PIPE;
                dcm_thresholds_default_values-> local_switch_fifo_size[1] = (SOC_DFE_FABRIC_PIPES_CONFIG(unit).mapping_type == soc_dcmn_fabric_pipe_map_dual_tdm_non_tdm) ? SOC_FE3200_DCM_DEFAULT_FIFO_SIZE_LOCAL_SWITCH_IP_TDM_DUAL_PIPE : 0;
                dcm_thresholds_default_values-> local_switch_fifo_size[2] = 0;
