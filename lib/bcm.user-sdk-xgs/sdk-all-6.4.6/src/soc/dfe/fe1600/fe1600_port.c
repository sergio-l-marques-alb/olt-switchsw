/*
 * $Id: fe1600_port.c,v 1.47 Broadcom SDK $
 *
...
soc_fe1600_port_soc_init(int unit)
{
    ...
    for (port_i = 0; port_i < SOC_DFE_DEFS_GET(unit ,nof_links); ++port_i) {
        ...
        if(SHR_BITGET(&puc,7)) {
            ...
            if(SOC_FE1600_LCPLL_CONTROL1_125_VAL == pll_control_1) { /*control word for 125MHz*/
                SOC_INFO(unit).port_refclk_int[port_i] = 125;
            } else if(SOC_FE1600_LCPLL_CONTROL1_156_25_VAL == pll_control_1){ /*control word for 156.25MHz*/
                SOC_INFO(unit).port_refclk_int[port_i] = 156;
            } else {
    ...
    if (!SOC_WARM_BOOT(unit)) {
        SOC_PBMP_ITER(pbmp, port) {
            ...
            if (SOC_IS_FE1600_B0_AND_ABOVE(unit)) {
                ...
                {
#ifdef BCM_88754_A0
                    SOCDNX_IF_ERR_EXIT(soc_fe1600_drv_bcm88754_async_fifo_set(unit, quad, lane, ITEM_6f /*RX full rate enable*/, val));
                    SOCDNX_IF_ERR_EXIT(soc_fe1600_drv_bcm88754_async_fifo_set(unit, quad, lane, ITEM_13f /*TX full rate enable*/, val));
#endif /*BCM_88754_A0*/
                } else {
...
soc_fe1600_port_control_pcs_set(int unit, soc_port_t port, soc_dcmn_port_pcs_t pcs)
{
    ...
    SOCDNX_IF_ERR_EXIT(READ_FMAC_FPS_RX_FEC_CONFIGURATIONr(unit, blk_id, inner_port, &reg_val32));       /* if we want to disable fec, we will write 0 to disable rx*/
    soc_reg_field_set(unit,FMAC_FPS_RX_FEC_CONFIGURATIONr,&reg_val32,FPS_N_RX_FEC_FEC_ENf,rx_fec_fec_en); 
    SOCDNX_IF_ERR_EXIT(WRITE_FMAC_FPS_RX_FEC_CONFIGURATIONr(unit, blk_id, inner_port, reg_val32));


    SOCDNX_IF_ERR_EXIT(READ_FMAC_FPS_TX_CONFIGURATIONr(unit, blk_id, inner_port, &reg_val32)); /* if we want to disable fec, we will write 0 to disable tx*/
 ...
 *      unit     -  (IN) Unit number.
 *      port     -  (IN) fabric port #
 *      flags    -  (IN) relevant flags: SOC_DFE_PORT_CONTROL_RX_SERDES_IGNORE
 *      power    -  (IN) on/off
 * Returns:
 *      SOC_E_xxx
 * Notes:
...
soc_fe1600_port_loopback_get(int unit, soc_port_t port, soc_dcmn_loopback_mode_t* loopback)
{
    ...
    /*soc_dfe_loopback_mode_mac_async_fifo*/
...
soc_error_t 
soc_fe1600_port_prbs_tx_enable_set(int unit, soc_port_t port, portmod_prbs_mode_t mode, int value)
{
...
soc_error_t 
soc_fe1600_port_prbs_tx_enable_get(int unit, soc_port_t port, portmod_prbs_mode_t mode, int* value)
{
    int blk_id, inner_port;
    uint32 reg_val32;
    soc_dcmn_port_pcs_t pcs;
...
soc_error_t 
soc_fe1600_port_prbs_rx_enable_set(int unit, soc_port_t port, portmod_prbs_mode_t mode, int value)
{
    int blk_id, inner_port;
    uint32 reg_val32;
    soc_dcmn_port_pcs_t pcs;
...
soc_error_t 
soc_fe1600_port_prbs_rx_enable_get(int unit, soc_port_t port, portmod_prbs_mode_t mode, int* value)
{
    int blk_id, inner_port;
    uint32 reg_val32;
    soc_dcmn_port_pcs_t pcs;
...
soc_error_t 
soc_fe1600_port_prbs_rx_status_get(int unit, soc_port_t port, portmod_prbs_mode_t mode, int* value)
{
    int blk_id, inner_port;
...
soc_error_t 
soc_fe1600_port_prbs_polynomial_set(int unit, soc_port_t port, portmod_prbs_mode_t mode, int value)
{
    SOCDNX_INIT_FUNC_DEFS;
    SOC_FE1600_ONLY(unit);
...
soc_error_t 
soc_fe1600_port_prbs_polynomial_get(int unit, soc_port_t port, portmod_prbs_mode_t mode, int* value)
{
    SOCDNX_INIT_FUNC_DEFS;
    SOC_FE1600_ONLY(unit);
...
soc_error_t 
soc_fe1600_port_prbs_tx_invert_data_set(int unit, soc_port_t port, portmod_prbs_mode_t mode, int value)
{
    SOCDNX_INIT_FUNC_DEFS;
    SOC_FE1600_ONLY(unit);
...
soc_error_t 
soc_fe1600_port_prbs_tx_invert_data_get(int unit, soc_port_t port, portmod_prbs_mode_t mode, int* value)
{
    SOCDNX_INIT_FUNC_DEFS;
    SOC_FE1600_ONLY(unit);
...
soc_fe1600_port_speed_set(int unit, soc_port_t port, int speed)
{
    ...
    if (SOC_IS_FE1600_B0_AND_ABOVE(unit)) {
        ...
        if (SOC_IS_BCM88754_A0(unit))
        {               
            SOCDNX_IF_ERR_EXIT(soc_fe1600_drv_bcm88754_async_fifo_set(unit, quad, lane, ITEM_6f /*RX full rate enable*/, 0x1));
            SOCDNX_IF_ERR_EXIT(soc_fe1600_drv_bcm88754_async_fifo_set(unit, quad, lane, ITEM_13f /*TX full rate enable*/, 0x1));
        } else
#endif /*BCM_88754_A0*/
        {
            SOCDNX_IF_ERR_EXIT(READ_FMAC_ASYNC_FIFO_CONFIGURATIONr(unit, quad, lane, &reg64_val));
...
/*
 * Function:
 *      soc_fe1600_port_wcmod_ucode_load
 * Purpose:
...
soc_fe1600_port_wcmod_ucode_load(int unit , int port, uint8 *arr,int arr_len) {
    ...
    soc_timeout_t to;
    int clause = 22; /*FE1600 access intenal phy with CL22*/
...
soc_fe1600_port_serdes_power_disable(int unit , soc_pbmp_t disable_pbmp) {
    ...
    /*in case of fe1600_b0 there is no need to power on and power off SerDers in order to save power ==> do nothing*/
#ifdef BCM_88750_B0
    if (SOC_IS_FE1600_B0_AND_ABOVE(unit)) {
        SOC_EXIT;
    }
