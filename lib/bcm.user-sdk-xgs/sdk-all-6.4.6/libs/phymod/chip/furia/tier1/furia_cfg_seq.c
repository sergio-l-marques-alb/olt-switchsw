                       ...
                       const phymod_phy_inf_config_t* config)
{
    ...
    if (pkg_side == SIDE_B) {
        ...
        if (/*(config_copy.interface_type == phymodInterfaceCAUI4) ||
            (config_copy.interface_type == phymodInterfaceXLAUI4) ||*/
...
chip to chip module */
    ...
    if (FURIA_IS_SIMPLEX(chip_id)) {
        ...
        if(config_copy.data_rate == SPEED_50G || config_copy.data_rate == SPEED_100G) {
    ...
    phy_type = _furia_get_phy_type(config_copy.interface_type, higig_mode);
    ...
    for(lane_index = 0; lane_index < num_lanes; lane_index++) {
        if (((lane_map >> lane_index) & 1) == 0x1) {
            ...
            */
            PHYMOD_IF_ERR_RETURN
                (_furia_set_intf_type(pa, config_copy.interface_type, lane_index));
        }
    }
    ...
    for(lane_index = 0; lane_index < num_lanes; lane_index++) {
        if (((lane_map >> lane_index) & 1) == 0x1) {
            ...
            if((config_copy.interface_type == actual_intf) &&
               (config_copy.data_rate == actual_speed) &&
