                 ...
                 int *blk, uint8 *acc_type)
{
    ...
    int               phy_port, i;
    int               instance = -1;
    int               instance_mask = 0, block_id_mask = 0;
...
#define SOC_REG_ADDR_INVALID_PORT 0 /* for asserts */
    ...
    *blk = 0;/* not really needed, just to avoid coverity defect */
    ...
    regblktype = SOC_REG_INFO(unit, reg).block;
    if (!block_id_mask && !instance_mask && port >= 0) {
        if (SOC_BLOCK_IN_LIST(regblktype, portblktype)) {
            assert(SOC_PORT_VALID(unit, port));
            if (soc_feature(unit, soc_feature_logical_port_num)) {
                /*
    ...
    if ((!block_id_mask) && (REG_PORT_ANY == port || instance_mask || !SOC_BLOCK_IN_LIST(regblktype, portblktype))) {
        int blkport = port;
 ...
 * registers and memories operaiotns.
 */
int 
