...
)
{
  ...
  uint32 
    ...
    cell_size_in_bits = 0,
    cur_cell_offset; /*In Bytes*/
#ifdef BCM_ARAD_SUPPORT
  uint32
    buf_arad[(ARAD_DATA_CELL_NOT_INBAND_BYTE_SIZE/4) + 1];
#endif
#ifdef BCM_PETRAB_SUPPORT
  uint32
    buf_petra[SOC_PETRA_SR_DATA_CELL_PAYLOAD_LENGTH_FE1600/32 +1];
#endif
  ...
  rv = soc_dpp_fabric_route2sr_link_list(
    unit,
    route,
    &sr_link_list
  );
  ...
  /* Determining the max cell size*/
  if(flags & SOC_DCMN_SR_CELL_FLAG_FE600_FORMAT) {
    cell_size_in_bits = SOC_SAND_DATA_CELL_PAYLOAD_IN_BYTES * 8;
  }
  else{
#ifdef BCM_ARAD_SUPPORT
    if(SOC_IS_ARAD(unit))
    {
      cell_size_in_bits = ARAD_DATA_CELL_NOT_INBAND_BYTE_SIZE*8;
    }
#endif
#ifdef BCM_PETRAB_SUPPORT
    if(SOC_IS_PETRAB(unit))
    {
      cell_size_in_bits = SOC_PETRA_SR_DATA_CELL_PAYLOAD_LENGTH_FE1600;
    }
#endif
  }

  for(cur_cell_offset = 0; cur_cell_offset < data_in_size; cur_cell_offset += cell_size_in_bits/8)
  {
    /*Copying the data to the buffer*/
    if( (data_in_size - cur_cell_offset) >= (cell_size_in_bits/ 8) ) {
#ifdef BCM_ARAD_SUPPORT
      if(SOC_IS_ARAD(unit)){
        sal_memcpy(buf_arad, (uint8*)data_in + cur_cell_offset, cell_size_in_bits/8);
      }
#endif
#ifdef BCM_PETRAB_SUPPORT
      if(SOC_IS_PETRAB(unit)){
        sal_memcpy(buf_petra, (uint8*)data_in + cur_cell_offset, cell_size_in_bits/8);
      }
#endif
    }
    else
    {/*The data doesn't fit the whole cell. Pad the MSB*/
#ifdef BCM_ARAD_SUPPORT
      if(SOC_IS_ARAD(unit)){
        sal_memset((uint8*)buf_arad, '\0', cell_size_in_bits/8);
        sal_memcpy(buf_arad, (uint8*)data_in + cur_cell_offset, data_in_size - cur_cell_offset);
      }
#endif
#ifdef BCM_PETRAB_SUPPORT
      if(SOC_IS_PETRAB(unit))
      {
        sal_memset((uint8*)buf_petra, '\0', cell_size_in_bits/8);
        sal_memcpy(buf_petra, (uint8*)data_in + cur_cell_offset, data_in_size - cur_cell_offset);
      }
#endif
    }
    if (!(flags & SOC_DCMN_SR_CELL_FLAG_FE600_FORMAT))
...
#ifdef BCM_PETRAB_SUPPORT
     if(SOC_IS_PETRAB(unit)) {
        soc_sand_rv = soc_petra_cpu2cpu_write(
          soc_sand_dev_id,
          &sr_link_list,
          cell_size_in_bits,
          buf_petra
...
#ifdef BCM_ARAD_SUPPORT
   if (SOC_IS_ARAD(unit)) {
        soc_sand_rv = arad_cpu2cpu_write_unsafe(
          soc_sand_dev_id,
          &sr_link_list,
          cell_size_in_bits,
          buf_arad
...
int 
soc_dpp_fabric_receive_sr_cell( 
  int unit, 
  uint32 flags, 
...
)
{
  ...
  soc_reg_above_64_val_t
    temp_data_out[((ARAD_DATA_CELL_NOT_INBAND_BYTE_SIZE*8)/128) + 1];
#endif /* BCM_ARAD_SUPPORT */
  ...
  if(flags & SOC_DCMN_SR_CELL_FLAG_FE600_FORMAT) {
    max_cell_size = SOC_SAND_DATA_CELL_PAYLOAD_IN_BYTES;
  }
  else{
#ifdef BCM_ARAD_SUPPORT
    if(SOC_IS_ARAD(unit))
    {
      max_cell_size = ARAD_DATA_CELL_NOT_INBAND_BYTE_SIZE;
    }
#endif
#ifdef BCM_PETRAB_SUPPORT
    if(SOC_IS_PETRAB(unit))
    {
      max_cell_size = SOC_PETRA_SR_DATA_CELL_PAYLOAD_LENGTH_FE1600/8;
    }
#endif
  }

  if (!SOC_IS_ARAD(unit) && data_out_max_size < max_cell_size)
  {
    rv = SOC_E_LIMIT;
    goto soc_dpp_attach_error;
  }

  *data_out_size = max_cell_size;
...
/* 
* COVERITY * 
* Coverity note: Overrun of static array "temp_data_out" of size 400 bytes by passing it as an argument to a function which indexes it at byte position 10160. 
...
/* coverity[overrun-buffer-val] */
      ...
      rv = handle_sand_result(soc_sand_rv);
      SOCDNX_IF_ERR_EXIT(rv);
      if (data_out_max_size < cell_size)
      {
        rv = SOC_E_LIMIT;
        goto soc_dpp_attach_error;
      }
      /* Copyingb the data if cell_size < max_cell_size need to do lsb alignment instead of msb alignment*/
      modulo = (cell_size*8)%128;
      for(i=0; i < (cell_size*8)/128; ++i) {
        if(modulo) {
          SHR_BITCOPY_RANGE(data_out, i*128, temp_data_out[((ARAD_DATA_CELL_NOT_INBAND_BYTE_SIZE*8)/128)- (cell_size*8/128) +i -1], 128 - modulo, modulo);
          SHR_BITCOPY_RANGE(data_out, i*128 + modulo, temp_data_out[((ARAD_DATA_CELL_NOT_INBAND_BYTE_SIZE*8)/128)-(cell_size*8/128) + i], 0, 128 - modulo);   
        }
        else
        {
          SHR_BITCOPY_RANGE(data_out, i*128, temp_data_out[((ARAD_DATA_CELL_NOT_INBAND_BYTE_SIZE*8)/128) - (cell_size*8/128) + i], 0, 128);
        }
      }
      if(modulo) {
        SHR_BITCOPY_RANGE(data_out, i*128, temp_data_out[((ARAD_DATA_CELL_NOT_INBAND_BYTE_SIZE*8)/128)- (cell_size*8/128) +i -1], 128 - modulo, modulo);
      }
      *data_out_size = cell_size;
    }
#endif /* BCM_ARAD_SUPPORT */
...
#ifdef BCM_PETRAB_SUPPORT
    if(SOC_IS_PETRAB(unit)) {
      soc_sand_rv = soc_petra_cpu2cpu_read(
        soc_sand_dev_id,
        data_out
      );
    }
...
#ifdef BCM_ARAD_SUPPORT
   if (SOC_IS_ARAD(unit)) {
      soc_sand_rv = arad_cpu2cpu_read_unsafe(
        soc_sand_dev_id,
        data_out
      );
    }
