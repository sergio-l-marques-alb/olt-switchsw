...
)
{
    ...
    if (DPP_MCDS_TYPE_IS_INGRESS(entry_type)) { /* set ingress entry pointer */
        ...
        uint32 entry_type = soc_mem_field32_get(unit, IRR_MCDB_EGRESS_FORMAT_1m, mcdb_entry, ENTRY_FORMATf);
        switch (entry_type >> 1) { /* select memory format based on the format type */
          case 0:
            mem = IRR_MCDB_EGRESS_FORMAT_1m;
            break;
          case 1:
            if (entry_type != DPP_MCDS_TYPE_VALUE_EGRESS_START || next_entry != DPP_MC_EGRESS_LINK_PTR_END || (entry_type & 1)) {
                mem = -1;
          ...
          case 3:
            ...
            break;
          default:
            if (entry_type != DPP_MCDS_TYPE_VALUE_EGRESS_START || next_entry != DPP_MC_EGRESS_LINK_PTR_END || (entry_type & 4)) {
                mem = -1;
            } /* We allow the first egress group entry to have no pointer if it marks the end of the group */
        }
    } else {
          ...
          case 1:
            {
                ...
                } else if (bitmap != DPP_MC_EGR_BITMAP_DISABLED &&  ++found <= max) { /* replication not disabled for this entry */
