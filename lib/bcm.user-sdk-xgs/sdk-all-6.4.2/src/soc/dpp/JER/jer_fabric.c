  ...
  )
{
    ...
    int i, nof_cores, tdm_pipe_field_val;
...
uint32
  soc_jer_init_fabric_flow_control(
    SOC_SAND_IN  int                 unit,
...
uint32
  soc_jer_init_fabric_scheduler_adaptation( /*needs fixing*/
    SOC_SAND_IN int    unit
  )
{
    uint32 reg_val32;
    ...
    /*if mode is dedicated, need to mask unwanted links*/
...
*/
uint32
  soc_jer_init_fabric_repeater_config(
    SOC_SAND_IN  int  unit
  )
{
    SOCDNX_INIT_FUNC_DEFS
  ...
  )
{
    ...
    if (!is_mesh) {
         ...
         SOCDNX_IF_ERR_EXIT(soc_reg_above_64_field32_modify(unit, FDT_LOAD_BALANCING_SWITCH_CONFIGURATIONr, REG_PORT_ANY, 0, SWITCHING_NETWORK_LFSR_CNTf, 0));
         ...
         SOCDNX_IF_ERR_EXIT(soc_reg_above_64_field32_modify(unit, FDT_LOAD_BALANCING_SWITCH_CONFIGURATIONr, REG_PORT_ANY, 0, SWITCHING_NETWORK_LFSR_CNTf, 1));
    }
    ...
    SOCDNX_IF_ERR_EXIT(soc_jer_init_fabric_flow_control(unit, ARAD_CONNECTION_DIRECTION_BOTH, &fc)); /*empty*/

    SOCDNX_IF_ERR_EXIT(soc_jer_init_fabric_scheduler_adaptation(unit)); 

    /* Configure repeater remote links */ 
    SOCDNX_IF_ERR_EXIT(soc_jer_init_fabric_repeater_config(unit)); /*empty*/
    ...
    for (i = 0; i < fap_id_count; ++i) {
        ...
        if (offset < 32) {
            ...
            *dqcq_map = soc_reg64_field32_get(unit, ECI_GLOBAL_DQCQ_MAP_CFG_2r, reg64_val, DEV_ID_TO_DQCQ_MAP_MSBf);
            SHR_BITCOPY_RANGE(dqcq_map, offset, &group_ctx, 0, SOC_JER_FABRIC_GROUP_CTX_LENGTH);
            soc_reg64_field32_set(unit, ECI_GLOBAL_DQCQ_MAP_CFG_1r, &reg64_val, DEV_ID_TO_DQCQ_MAP_MSBf, *dqcq_map);
