/* $Id: tmc_api_end2end_scheduler.c,v 1.11 Broadcom SDK $
  ...
  )
{
  ...
  info->is_valid = 0;
  SOC_TMC_SCH_SUB_FLOW_SHAPER_clear(&(info->shaper));
  ...
  )
{
  ...
  {
    SOC_TMC_SCH_SUBFLOW_clear(&(info->sub_flow[ind]));
  }
  ...
  )
{
    ...
    SOC_TMC_SCH_FLOW_clear(&(info->sch_consumer));
...
device_sch->rates[ind].rci_level = ind % SOC_TMC_SCH_NOF_RCI_LEVELS;
device_sch->rates[ind].num_active_links = ind / SOC_TMC_SCH_NOF_RCI_LEVELS;
*/
  ...
  )
{
  ...
  for (ind_i = 0; ind_i < SOC_TMC_SCH_NOF_RCI_LEVELS + 1; ++ind_i)
  {
    LOG_CLI((BSL_META_U(unit,
                        "|--------")));
  }
  ...
  LOG_CLI((BSL_META_U(unit,
                      "|LNK\\RCI ")));
  for (ind_i = 0; ind_i < SOC_TMC_SCH_NOF_RCI_LEVELS; ++ind_i)
  {
    LOG_CLI((BSL_META_U(unit,
                        "| %6d "), ind_i));
  }
  ...
  for (ind_j = 0; ind_j < SOC_DPP_DEFS_GET(unit, nof_fabric_links) + 1; ++ind_j)
  {
    ...
    {
      LOG_CLI((BSL_META_U(unit,
                          "| %6u "), info->rates[ind_j * SOC_TMC_SCH_NOF_RCI_LEVELS + ind_i].rate));
    }
    LOG_CLI((BSL_META_U(unit,
                        "|\n\r")));
  }
...
/* 
 * Print structure for Soc_petra-B only. For ARAD call arad_ARAD_SCH_PORT_INFO_print
 */
void
