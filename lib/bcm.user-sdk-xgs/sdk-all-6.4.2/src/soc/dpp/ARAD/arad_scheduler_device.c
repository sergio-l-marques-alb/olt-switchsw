  ...
  )
{
    ...
    SOCDNX_IF_ERR_EXIT(READ_SCH_FC_MAP__FCMm(unit, SCH_BLOCK(unit, core), egress_offset, &tbl_data));
    sch_offset = soc_SCH_FC_MAP__FCMm_field32_get(unit,&tbl_data,FC_MAP__FCMf);
  ...
  )
{
    ...
    else
    {
        ...
        offset = ARAD_SCH_OFFSET_TO_NON_CHANNELIZED_OFFSET(unit, sch_offset);
        /* Obtain max credit rate nof bits */
        credit_rate_nof_bits = soc_mem_field_length(unit, SCH_ONE_PORT_NIF_CONFIGURATION__OPNCm, PORT_NIF_MAX_CR_RATEf);
    ...
    {
        SOCDNX_IF_ERR_EXIT(READ_SCH_CH_NIF_RATES_CONFIGURATION__CNRCm(unit, SCH_BLOCK(unit, core), offset, tbl_data));
        soc_SCH_CH_NIF_RATES_CONFIGURATION__CNRCm_field32_set(unit,tbl_data,CH_NI_FXX_MAX_CR_RATEf,credit_rate);
        SOCDNX_IF_ERR_EXIT(WRITE_SCH_CH_NIF_RATES_CONFIGURATION__CNRCm(unit, SCH_BLOCK(unit, core), offset, tbl_data));
    }
    else
    {
        SOCDNX_IF_ERR_EXIT(READ_SCH_ONE_PORT_NIF_CONFIGURATION__OPNCm(unit, SCH_BLOCK(unit, core), offset, tbl_data));
        soc_SCH_ONE_PORT_NIF_CONFIGURATION__OPNCm_field32_set(unit,tbl_data,PORT_NIF_MAX_CR_RATEf,credit_rate);
        SOCDNX_IF_ERR_EXIT(WRITE_SCH_ONE_PORT_NIF_CONFIGURATION__OPNCm(unit, SCH_BLOCK(unit, core), offset, tbl_data));
    }
  ...
  )
{
  ...
  /* Get scheduler offset */
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 1260, exit, READ_SCH_FC_MAP__FCMm(unit,MEM_BLOCK_ANY,egress_offset,&data));
  ...
  )
{
  ...
  /* Get scheduler offset */
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 1380, exit, READ_SCH_FC_MAP__FCMm(unit,MEM_BLOCK_ANY,egress_offset,&data));
  sch_offset = soc_SCH_FC_MAP__FCMm_field32_get(unit,&data,FC_MAP__FCMf);
  ...
  )
{
  ...
  offset = (nof_active_links_ndx * ARAD_SCH_NOF_RCI_LEVELS) + rci_level_ndx;
...
{
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_SCH_DEVICE_RATE_ENTRY_VERIFY);

  SOC_SAND_ERR_IF_ABOVE_MAX(
    nof_active_links_ndx, ARAD_SCH_NOF_LINKS,
    ARAD_SCH_FABRIC_LINK_ID_OUT_OF_RANGE_ERR,10,exit
  );

  SOC_SAND_ERR_IF_ABOVE_MAX(
    rci_level_ndx, ARAD_SCH_NOF_RCI_LEVELS-1,
    ARAD_SCH_FABRIC_LINK_ID_OUT_OF_RANGE_ERR,20,exit
  );

exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_sch_device_rate_entry_verify()",0,0);
}
