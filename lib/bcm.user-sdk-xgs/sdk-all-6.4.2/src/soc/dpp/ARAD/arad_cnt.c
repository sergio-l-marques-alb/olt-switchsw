      ...
      )
{
    ...
    for(pp_port_ndx = 0;  pp_port_ndx < ARAD_PORT_NOF_PP_PORTS;  pp_port_ndx++)
    {
		SOC_SAND_SOC_IF_ERROR_RETURN(res, 29, exit, READ_EPNI_PP_COUNTER_TABLEm(unit, MEM_BLOCK_ANY, pp_port_ndx, data));
		...
		SOC_SAND_SOC_IF_ERROR_RETURN(res, 30, exit, WRITE_EPNI_PP_COUNTER_TABLEm(unit, MEM_BLOCK_ANY, pp_port_ndx, data));
    }
exit:
...

{
 ...
 if (!SOC_UNIT_NUM_VALID(unit)) {
  ...
  if((info->src_type >= ARAD_CNT_SRC_TYPE_EPNI_A) && (info->src_type <= ARAD_CNT_SRC_TYPE_EPNI_D))
  {
	  ...
	  SOC_SAND_SOC_IF_ERROR_RETURN(res, 70, exit, READ_EPNI_PP_COUNTER_TABLEm(unit, MEM_BLOCK_ANY, 0, &data));
  ...
  )
{
  ...
  if (direction == SOC_TMC_PORT_DIRECTION_INCOMING) {
      ...
      SOC_SAND_SOC_IF_ERROR_RETURN(res, 1020, exit, READ_EPNI_PP_COUNTER_TABLEm(unit, MEM_BLOCK_ANY, port_ndx, data));

      soc_EPNI_PP_COUNTER_TABLEm_field32_set(unit, data, COUNTER_COMPENSATIONf, fld_val);

      SOC_SAND_SOC_IF_ERROR_RETURN(res, 1030, exit, WRITE_EPNI_PP_COUNTER_TABLEm(unit, MEM_BLOCK_ANY, port_ndx, data));
