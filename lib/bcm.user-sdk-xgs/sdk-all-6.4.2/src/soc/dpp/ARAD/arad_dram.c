...
#define SOC_ARAD_DRAM_BUFFER_NUM_MAX 0x1fffff
...
#define ARAD_DRAM_VAL_TO_CLOCKS_DOWN(value)                     \
          (ARAD_DRAM_IS_VAL_IN_CLOCKS(value) ? (ARAD_DRAM_VAL(value)) : SOC_SAND_DIV_ROUND_DOWN(dram_freq * ARAD_DRAM_VAL(value), 1000000))
  ...
  )
{
 ...
 LOG_DEBUG(BSL_LS_SOC_DRAM,
           (BSL_META_U(unit,
                       "%s(): bist_err_occur=0x%x, bist_full_err_cnt=0x%x,  bist_single_err_cnt=0x%x, bist_global_err_cnt=0x%x\n"), FUNCTION_NAME(), *bist_err_occur, *bist_full_err_cnt, *bist_single_err_cnt, *bist_global_err_cnt)); 
    ...
    SOC_SAND_IN ARAD_ITM_DBUFF_SIZE_BYTES dbuff_size)
{
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 10, exit, READ_MMU_GENERAL_CONFIGURATION_REGISTERr(unit, &reg_val));
   ...
   SOC_DPP_CONFIG(unit)->arad->init.dram.interleaving_bits = interleaving_bits;
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 20, exit, WRITE_MMU_GENERAL_CONFIGURATION_REGISTERr(unit, reg_val));
...
#if defined(ARAD_DRAM_MMU_ARAD_PLUS_FIX) && defined (BCM_88660_A0)
    if (!SOC_IS_ARADPLUS(unit)) {
        ...
        SOC_SAND_SOC_IF_ERROR_RETURN(res, 50, exit, READ_DRCA_AC_OPERATING_CONDITIONS_2r(unit, &reg_val));
        dd_rt_rcdw = soc_reg_field_get(unit, DRCA_AC_OPERATING_CONDITIONS_2r, reg_val, DD_RT_RCDWf);
        dd_rt_rcdr = soc_reg_field_get(unit, DRCA_AC_OPERATING_CONDITIONS_2r, reg_val, DD_RT_RCDRf);

        SOC_SAND_SOC_IF_ERROR_RETURN(res, 50, exit, READ_DRCA_AC_OPERATING_CONDITIONS_3r(unit, &reg_val));
        cnt_wrapprd = soc_reg_field_get(unit, DRCA_AC_OPERATING_CONDITIONS_3r, reg_val, CNT_WRAPPRDf);
        cnt_rdapprd = soc_reg_field_get(unit, DRCA_AC_OPERATING_CONDITIONS_3r, reg_val, CNT_RDAPPRDf);
        ...
        if (SOC_DPP_CONFIG(unit)->arad->init.dram.bist_enable == 0x1) {
            for (instance_idx=0; instance_idx < SOC_DPP_DEFS_GET(unit, hw_dram_interfaces_max) ;instance_idx++) {
                if (is_valid[instance_idx]) {
                    ...
                    LOG_DEBUG(BSL_LS_SOC_DRAM,
                              (BSL_META_U(unit,
                                          "%s(): instance_idx=%d, bist_err_occur=0x%x, bist_full_err_cnt=0x%x, bist_single_err_cnt=0x%x, bist_global_err_cnt=0x%x\n"), 
                    ...
                    if ((bist_err_occur != 0) && (bist_full_err_cnt != 0) && (bist_single_err_cnt != 0) && (bist_global_err_cnt != 0)) {
                       LOG_ERROR(BSL_LS_SOC_DRAM,
                                 (BSL_META_U(unit,
                                             "%s(): ERROR: dram_idx=%d Failed BIST. bist_err_occur=0x%x, bist_full_err_cnt=0x%x, bist_single_err_cnt=0x%x, bist_global_err_cnt=0x%x.\n"), 
                                             FUNCTION_NAME(), instance_idx, bist_err_occur, bist_full_err_cnt, bist_single_err_cnt, bist_global_err_cnt));
...
{        
    ...
    /* Senity check */
    if (buf_num > SOC_ARAD_DRAM_BUFFER_NUM_MAX) {
        LOG_ERROR(BSL_LS_SOC_DRAM,
                  (BSL_META_U(unit,
                              "%s(): Buffer number=%d (0x%x), Exceeded max size=%d (0x%x). \n"), 
                              FUNCTION_NAME(), buf_num, buf_num, SOC_ARAD_DRAM_BUFFER_NUM_MAX, SOC_ARAD_DRAM_BUFFER_NUM_MAX));
    ...
    )
{
   ...
   /* set the logical mode */
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 10, exit, soc_reg32_get(unit, MMU_BANK_ACCESS_CONTROLLER_CONFIGURATIONSr, REG_PORT_ANY, 0, &reg_val));

    soc_reg_field_set(unit, MMU_BANK_ACCESS_CONTROLLER_CONFIGURATIONSr, &reg_val, INDIRECT_LOGICAL_MODEf, logical_mod);

    SOC_SAND_SOC_IF_ERROR_RETURN(res, 20, exit, soc_reg32_set(unit, MMU_BANK_ACCESS_CONTROLLER_CONFIGURATIONSr, REG_PORT_ANY, 0, reg_val));

    /* set the address of reading */
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 50, exit, soc_reg32_get(unit, MMU_INDIRECT_COMMANDr, REG_PORT_ANY, 0, &reg_val));

    soc_reg_field_set(unit, MMU_INDIRECT_COMMANDr, &reg_val, INDIRECT_COMMAND_TRIGGERf, 0x1);

    SOC_SAND_SOC_IF_ERROR_RETURN(res, 60, exit, soc_reg32_set(unit, MMU_INDIRECT_COMMANDr, REG_PORT_ANY, 0, reg_val));    
  
    res = arad_polling(
          unit,
          ARAD_TIMEOUT,
          ARAD_MIN_POLLS,
          MMU_INDIRECT_COMMANDr,
          REG_PORT_ANY, 
          0, /* index */
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 80, exit, soc_reg_above_64_get(unit, MMU_INDIRECT_COMMAND_RD_DATAr, REG_PORT_ANY, 0, data));
    ...
    )
{
   ...
   /* set the logical mode */
    ...
    /* set date */
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 50, exit, soc_reg_above_64_set(unit, MMU_INDIRECT_COMMAND_WR_DATAr, REG_PORT_ANY, 0, data)); 

    /* triger indirect command */ 
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 60, exit, soc_reg32_get(unit, MMU_INDIRECT_COMMANDr, REG_PORT_ANY, 0, &reg_val));

    soc_reg_field_set(unit, MMU_INDIRECT_COMMANDr, &reg_val, INDIRECT_COMMAND_TRIGGERf, 0x1);

    SOC_SAND_SOC_IF_ERROR_RETURN(res, 70, exit, soc_reg32_set(unit, MMU_INDIRECT_COMMANDr, REG_PORT_ANY, 0, reg_val));    
    ...
    SOC_SAND_IN     int nbytes)
{
    ...
    if (offset + nbytes > (SOC_DPP_CONFIG(unit)->arad->init.dram.dram_user_buffer_size_mbytes * 1000000)) {
        LOG_ERROR(BSL_LS_SOC_DRAM,
                  (BSL_META_U(unit,
                              "%s(): Exceeded dram buffer size. offset=%d, nbytes=%d, dram_user_buffer_size_mbytes=%d\n"), 
    ...
    if ((access_type != SOC_ARAD_DRAM_USER_BUFFER_ACCESS_TYPE_READ) && (access_type != SOC_ARAD_DRAM_USER_BUFFER_ACCESS_TYPE_WRITE)) {
        LOG_ERROR(BSL_LS_SOC_DRAM,
                  (BSL_META_U(unit,
                              "%s(): undefined access type. access_type=%d\n"), FUNCTION_NAME(), access_type));
        SOC_SAND_SET_ERROR_CODE(SOC_SAND_GEN_ERR, 20, exit);
    }

    LOG_DEBUG(BSL_LS_SOC_DRAM,
              (BSL_META_U(unit,
                          "%s(): flags=0x%x, access_type=0x%x, buf=%p, offset=%d, nbytes=%d\n"), FUNCTION_NAME(), flags ,access_type, buf, offset, nbytes));
    ...
    /* General calculation */
    max_index  = dbuff_size / SOC_ARAD_DRAM_MMU_IND_DATA_LEN;

    /* Find buff number from offset */
    buf_num_offset = (offset / dbuff_size);
    buf_ndx        = (offset % dbuff_size) / SOC_ARAD_DRAM_MMU_IND_DATA_LEN;
    buf_num        = SOC_DPP_CONFIG(unit)->arad->init.dram.dram_user_buffer_start_ptr + buf_num_offset;

    LOG_DEBUG(BSL_LS_SOC_DRAM,
              (BSL_META_U(unit,
                          "%s(): dbuff_size=%d, max_index=%d, buf_num_offset=%d, buf_ndx=%d, buf_num=%d, dram_user_buffer_start_ptr=%d, SOC_ARAD_DRAM_MMU_IND_DATA_LEN=%d\n"), 
                          FUNCTION_NAME(), dbuff_size, max_index, buf_num_offset, buf_ndx, buf_num, SOC_DPP_CONFIG(unit)->arad->init.dram.dram_user_buffer_start_ptr, SOC_ARAD_DRAM_MMU_IND_DATA_LEN));

    nbytes_done = 0;
    while (nbytes > nbytes_done) {

        loop_offset = 0;
        len = SOC_ARAD_DRAM_MMU_IND_DATA_LEN;
        ...
        loop_offset = (offset + nbytes_done) % SOC_ARAD_DRAM_MMU_IND_DATA_LEN;
        len = SOC_SAND_MIN(SOC_ARAD_DRAM_MMU_IND_DATA_LEN - loop_offset, nbytes - nbytes_done);

        LOG_DEBUG(BSL_LS_SOC_DRAM,
                  (BSL_META_U(unit,
                              "%s(): nbytes_done=%d, loop_offset=%d, len=%d, buf_num=%d, buf_ndx=%d, addr_full=0x%x\n"), 
                              FUNCTION_NAME(), nbytes_done ,loop_offset, len, buf_num, buf_ndx, addr_full));

        if (write == 0x1) {
            if ((loop_offset != 0x0) || (len != SOC_ARAD_DRAM_MMU_IND_DATA_LEN)) {
                res = arad_dram_mmu_indirect_read_unsafe(unit, logical_mod, addr_full, data);
                SOC_SAND_CHECK_FUNC_RESULT(res, 60, exit);
            }

            sal_memcpy((uint8 *)data + loop_offset, 
                       buf + nbytes_done, 
                       len);
            ...
            sal_memcpy(buf + nbytes_done,
                       (uint8 *)data + loop_offset, 
                       len);
        }
        ...
        LOG_DEBUG(BSL_LS_SOC_DRAM,
                  (BSL_META_U(unit,
                              "%s(): nbytes_done=%d, buf_logical=0x%x, buf_logical_offset=%d, phy_addr=0x%x\n"), 
    ...
    if (nbytes != nbytes_done) {
        LOG_ERROR(BSL_LS_SOC_DRAM,
                  (BSL_META_U(unit,
                              "%s(): nbytes=%d should be equal to nbytes_done=%d\n"), FUNCTION_NAME(), nbytes ,nbytes_done));
           ...
           SOC_SAND_OUT uint32 *pass_test)
{
    ...
    uint32 addr;
    uint32 buf_max_index = SOC_DPP_CONFIG(unit)->arad->init.dram.dbuff_size / SOC_ARAD_DRAM_MMU_IND_DATA_LEN;
    soc_reg_above_64_val_t data;
    soc_reg_above_64_val_t known_patern;

    SOC_SAND_INIT_ERROR_DEFINITIONS(0);
 
    sal_memset((void*)known_patern, ARAD_DRAM_BUFFER_TEST_PATTERN, SOC_ARAD_DRAM_MMU_IND_DATA_LEN);
    ...
    *pass_test = 1;   
    ...
    for (i=0; i < buf_max_index; ++i) {
        ...
        if (sal_memcmp(data, known_patern, SOC_ARAD_DRAM_MMU_IND_DATA_LEN) == 0x1) {
            *pass_test = 0;
             break;
        }
    }
...
int soc_dpp_ardon_dram_mmu_set(int unit, soc_dpp_drc_combo28_info_t *drc_info) 
{
    ...
    field32_val = 0x0;
    soc_reg_field_set(unit, MMU_GENERAL_CONFIGURATION_REGISTERr, &reg32_val, STOP_MMU_PIPE_INSIDE_DPRCf, field32_val);
   ...
   LOG_DEBUG(BSL_LS_SOC_DRAM,
    ...
    case 0:
        ...
        break; 
    case 1:
        /* MMU uses A. We need to use C. ==> Map Dram0 to Dram2 */
        field32_val = 0x2;
        soc_reg_field_set(unit, MMU_DRAM_NUMBER_SWAPPINGr, &reg32_val, DRAM_0_MAPPINGf, field32_val);
        break; 
    case 2:
        /* MMU uses A, D. We need to use C, D. ==> Map Dram0 to Dram2 */
        field32_val = 0x2;
        soc_reg_field_set(unit, MMU_DRAM_NUMBER_SWAPPINGr, &reg32_val, DRAM_0_MAPPINGf, field32_val);
        break;
    case 3:
        /* MMU uses A, B, C. We need to use A, C, D. ==> Map Dram1 to Dram3 */
        field32_val = 0x3;
        soc_reg_field_set(unit, MMU_DRAM_NUMBER_SWAPPINGr, &reg32_val, DRAM_1_MAPPINGf, field32_val);
        break;
    case 4:
        /* MMU uses A, B, D, E. We need to use A, B, C, D. ==> Map Dram4 to Dram2 */
        field32_val = 0x2;
        soc_reg_field_set(unit, MMU_DRAM_NUMBER_SWAPPINGr, &reg32_val, DRAM_4_MAPPINGf, field32_val);
        break;
    default:
        /* Verified already in dram_info_verify*/
        SOCDNX_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_SOC_MSG("Error in %s: Number of dram_num=%d is not supported."), FUNCTION_NAME(), drc_info->dram_num));
...
int soc_dpp_drc_combo28_dram_init(int unit, soc_dpp_drc_combo28_info_t *drc_info)
{
    ...
    SOCDNX_IF_ERR_EXIT(soc_dpp_drc_combo28_info_struct_convert(unit, drc_info, &shmoo_info));

    SOCDNX_IF_ERR_EXIT(soc_combo28_shmoo_dram_info_set(unit, &shmoo_info));
   
    SOC_PBMP_ITER(drc_info->dram_bitmap, drc_ndx) {
