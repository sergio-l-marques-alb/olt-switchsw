...
/* Build EM payload according to routing info.frwrd_decision and routing_info.mac_lsbs
    ...
    )
{
  ...
  /* to extend ARP table, write MAC address (DA) 4 LSBs in add_info */
  ...
  res = soc_sand_bitstream_set_any_field(
            &mac_lsb,
            16,
            ARAD_PP_FRWRD_IPV4_HOST_TABLE_PAYLOAD_ASD_HI_NOF_BITS,
            add_info_lcl
          );
    ...
    )
{
  ...
  *accessed = FALSE;
  ...
  /* to extend ARP table, MAC address (DA) 4 LSBs is written in add_info[19:16] (unused bits of EEI)
     EEI = 24 bits = {out_lif[15:0], unused[19:16], EEI type[23:20]} */
  ARAD_PP_LEM_ACCESS_HI_FROM_EEI(routing_info->mac_lsb, payload.asd);
