...
#define ARAD_PP_RIF_ROUTING_ENABLER_UC           0x1
#define ARAD_PP_RIF_ROUTING_ENABLER_MC           0x2
#define ARAD_PP_RIF_ROUTING_ENABLER_MPLS         0x4
#define ARAD_PP_RIF_ROUTING_ENABLERS_BM_MAX      (ARAD_PP_RIF_ROUTING_ENABLER_MC | ARAD_PP_RIF_ROUTING_ENABLER_UC | ARAD_PP_RIF_ROUTING_ENABLER_MPLS)
  ...
  )
{
  ...
  tbl_data.enable_routing_mc = (rif_info->routing_enablers_bm & ARAD_PP_RIF_ROUTING_ENABLER_MC) ? 1 : 0;
  tbl_data.enable_routing_uc = (rif_info->routing_enablers_bm & ARAD_PP_RIF_ROUTING_ENABLER_UC) ? 1 : 0;
  tbl_data.enable_routing_mpls = (rif_info->routing_enablers_bm & ARAD_PP_RIF_ROUTING_ENABLER_MPLS) ? 1 : 0;
  ...
  if (SOC_IS_ARADPLUS(unit) && soc_property_get(unit, spn_BCM886XX_L3_INGRESS_URPF_ENABLE, 0)) {
    ...
    rv = SOC_DPP_WB_ENGINE_SET_ARR(unit, SOC_DPP_WB_ENGINE_VAR_ARAD_PP_RIF_URPF_MODE, &urpf_mode, rif_id);
    SOC_SAND_CHECK_FUNC_RESULT(rv, 45, exit);
  ...
  )
{
  ...
  rif_info->routing_enablers_bm =
    ((tbl_data.enable_routing_mc == 0) ? 0 : ARAD_PP_RIF_ROUTING_ENABLER_MC) |
    ((tbl_data.enable_routing_uc == 0) ? 0 : ARAD_PP_RIF_ROUTING_ENABLER_UC) |
    ((tbl_data.enable_routing_mpls == 0) ? 0 : ARAD_PP_RIF_ROUTING_ENABLER_MPLS);
  ...
  )
{
  ...
  if (SOC_IS_ARADPLUS(unit) && soc_property_get(unit, spn_BCM886XX_L3_INGRESS_URPF_ENABLE, 0)) {
    ...
    res = arad_pp_sw_db_rif_to_lif_group_map_add_lif_to_rif(
       unit,
       sem_res_tbl_data.vsi,
       lif_index
       );
    SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);

    /* Now set the proper bit in the inlif profile according to the rpf mode of the rif. */
    res = SOC_DPP_WB_ENGINE_GET_ARR(unit, SOC_DPP_WB_ENGINE_VAR_ARAD_PP_RIF_URPF_MODE, &urpf_mode, sem_res_tbl_data.vsi);
  ...
  )
{
  ...
  SOC_SAND_ERR_IF_ABOVE_MAX(info->routing_enablers_bm, ARAD_PP_RIF_ROUTING_ENABLERS_BM_MAX, ARAD_PP_RIF_ROUTING_ENABLERS_BM_OUT_OF_RANGE_ERR, 14, exit);
#ifdef BCM_88660_A0
