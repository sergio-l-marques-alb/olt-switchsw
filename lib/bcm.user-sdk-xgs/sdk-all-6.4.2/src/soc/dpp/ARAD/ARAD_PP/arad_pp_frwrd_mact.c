  ...
  )
{
  ...
  /* invalid SW state */
  res = arad_pp_sw_db_mact_traverse_flush_use_set(unit, 0);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  ...
  )
{
  ...
  case ARAD_PP_FRWRD_MACT_TRAVERSE_STATE_AGGREGATE:
      res = arad_pp_sw_db_mact_traverse_mode_set(unit,mode->state);
      SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  break;
  ...
  )
{
  ...
  res = arad_pp_sw_db_mact_traverse_mode_get(unit, &mode_lcl);
  ...
  )
{
  ...
  if (SAL_BOOT_PLISIM) {
      ...
      res = arad_pp_lem_traverse_bitmap_build(unit, &rule_key, &rule_key_mask, &(rule->value_rule), &tbl_data);
      SOC_SAND_CHECK_FUNC_RESULT(res, 35, exit);
 
      arad_pp_lem_block_get(unit, rule, (void *)&tbl_data, block_range, lem_key_array, payload_array, &nof_total_entries);
  ...
  LOG_INFO(BSL_LS_SOC_FORWARD,
           (BSL_META_U(unit,
            ...
            /* if fifo full and it's enoguh for this read then stop flush db */
    ...
    /* read and parse reply to lem structs */
      ...
      /* ultra check that only relevenat keys returned */
...
#ifdef PLISIM
  if (SAL_BOOT_PLISIM) {
      if (payload_array != NULL) sal_free(payload_array);
      if (lem_key_array != NULL) sal_free(lem_key_array);
  }
#endif
  ...
  )
{
 ...
 /* compare for key in bits 69-54 (FID)*/
  ...
  res = arad_pp_sw_db_mact_traverse_mode_get(unit, &traverse_mode);
  ...
  )
{
  ...
  if(action->type == ARAD_PP_FRWRD_MACT_TRAVERSE_ACTION_TYPE_COUNT && block_range->iter == 0) {
      ...
      /* if action is to count, and no mask then just read HW register*/
        /* check payload if select is not noe */
      if (rule_val != NULL) {
