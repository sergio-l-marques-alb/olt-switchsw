  ...
  )
{
    ...
    SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_FABRIC_PRIORITY_BITS_MAPPING_TO_FDT_INDEX_GET);
    *index = 0;
  ...
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_fabric_priority_bits_mapping_to_fdt_index_get()", tc, dp);
}
  ...
  )
{
    ...
    SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_FABRIC_PRIORITY_SET);
    ...
    if (!SOC_DPP_CONFIG(unit)->tdm.is_tdm_over_primary_pipe) {
        if (fabric_priority >= ARAD_FBC_PRIORITY_NOF-1 /*last one for tdm only*/) {
            SOC_SAND_SET_ERROR_CODE(ARAD_FABRIC_PRIORITY_OUT_OF_RANGE, 5, exit);
        }
    } else {
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_fabric_priority_set()", fabric_priority, 0);
  ...
  )
{
    ...
    SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_FABRIC_PRIORITY_GET);
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_fabric_priority_get()", 0, 0);
}
...
arad_fabric_reachability_status_get(int unit, int moduleid, int links_max, uint32 *links_array, int *links_count)
{
    ...
    SOCDNX_IF_ERR_EXIT(READ_RTP_DRMAm(unit, MEM_BLOCK_ANY, moduleid/2, rtp_reg_val));
   ...
   SOC_SAND_IN  int                                 unit)
{
    ...
    if (SOC_DPP_CONFIG(unit)->arad->init.fabric.is_fe600) {
        ...
        max_cell_size = ARAD_FABRIC_VSC256_MAX_CELL_SIZE;
    }
