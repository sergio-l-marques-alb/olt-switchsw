...
{
    uint32 is_valid, is_initialized, quad, last_in_use;
    ...
    SOCDNX_IF_ERR_EXIT(soc_port_sw_db_is_valid_port_get(unit, port, &is_valid));
    if(!is_valid) {
        SOCDNX_EXIT_WITH_ERR(SOC_E_PORT, (_BSL_SOCDNX_MSG("Can't deinit invalid port %d"), port));
    }

    /*make sure port is initialized*/
    SOCDNX_IF_ERR_EXIT(soc_port_sw_db_is_initialized_get(unit, port, &is_initialized));
    if(!is_initialized) {
        SOCDNX_EXIT_WITH_ERR(SOC_E_PORT, (_BSL_SOCDNX_MSG("Port isn't initialized %d"), port));
    }
...
soc_pm_speed_set(int unit, soc_port_t port, int speed)
{
    ...
    if(is_initialized && !is_virtual) {
        /*disable port*/
        soc_pm_enable_get(unit, port,&enabled);
        if(enabled) {
...
soc_pm_default_speed_get(int unit, soc_port_t port, int* speed)
{
    ...
    switch(interface) {
        ...
        case SOC_PORT_IF_CAUI:
            SOCDNX_IF_ERR_EXIT(soc_port_sw_db_num_lanes_get(unit, port, &nof_lanes));
            *speed = nof_lanes == 10 ? 100000 : 120000;
            break;
        case SOC_PORT_IF_ILKN:
            *speed = 10312;
            break;
 ...
 */
soc_error_t
soc_pm_port_clp_reset(int unit, soc_port_t port)
{
    uint32 is_valid, is_initialized;
