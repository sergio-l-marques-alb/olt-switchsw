...
soc_fe3200_port_soc_init(int unit)
{
    ...
    for (port_i = 0; port_i < SOC_FE3200_NOF_LINKS ; ++port_i) {
        ...
        if(SHR_BITGET(&puc,SOC_FE3200_PORT_ECI_POWER_UP_CONFIG_DYNAMIC_PLL_BIT) || SAL_BOOT_PLISIM) {
            /*if PUC[SOC_FE3200_PORT_ECI_POWER_UP_CONFIG_DYNAMIC_PLL_BIT]==1 then LC PLL config is taken from CMIC regs*/
            switch(lcpll) {
            case 0:
            ...
            /*if PUC[SOC_FE3200_PORT_ECI_POWER_UP_CONFIG_DYNAMIC_PLL_BIT]==0 then LC PLL config is taken from the PUC*/
            switch(lcpll) {
            case 0:
        ...
        if (SOC_DFE_IS_FE13_ASYMMETRIC(unit)) /* if fe13 is in asymmetric mode, need to change the clk frequence of changed quads to the values configured*/
        {
            if (port_i >= 104 && port_i <= 107 && SOC_DFE_CONFIG(unit).fabric_clk_freq_quad_26 != -1)
            {
                SOC_INFO(unit).port_refclk_int[port_i] = SOC_DFE_CONFIG(unit).fabric_clk_freq_quad_26;
            }
            else if (port_i >=140 && port_i <= 143 && SOC_DFE_CONFIG(unit).fabric_clk_freq_quad_35 != -1)
            {
                SOC_INFO(unit).port_refclk_int[port_i] = SOC_DFE_CONFIG(unit).fabric_clk_freq_quad_35;
            }
    ...
    sync_e_master_divider =  soc_dfe_property_suffix_num_get(unit, 0, spn_SYNC_CLK_DIVIDER_CLK, "", 0, SOC_DFE_PROPERTY_UNAVAIL);
...
soc_fe3200_port_probe(int unit, pbmp_t pbmp, pbmp_t *okay_pbmp, int is_init_sequence)
{
    ...
    SOC_PBMP_ITER(pbmp, port) {
        ...
        rc= soc_dcmn_fabric_port_probe(unit, port, &port_config);
        SOCDNX_IF_ERR_EXIT(rc);
...
soc_error_t
soc_fe3200_port_update_fsrd_block(int unit, bcm_port_t port, int enable)
{
    int fsrd_block;
    uint32 field_val;
    uint64 reg_val64;
...
soc_error_t
soc_fe3200_port_first_link_in_fsrd_get(int unit, bcm_port_t port, int *is_first_link, int enable)
{
    int fsrd_block, i;
    int counter_mask_fsrd_bmp = 0;
    int counter_mask_port_bmp = 0;
    pbmp_t mask_fsrd_bmp;
    pbmp_t mask_port_bmp;

    SOCDNX_INIT_FUNC_DEFS;

    SOC_PBMP_CLEAR(mask_fsrd_bmp);
    SOC_PBMP_CLEAR(mask_port_bmp);

    SOC_PBMP_PORT_ADD(mask_port_bmp, port);
    ...
    SOC_PBMP_AND(mask_port_bmp, PBMP_SFI_ALL(unit)); /* mask the current port */

    SOC_PBMP_COUNT(mask_fsrd_bmp, counter_mask_fsrd_bmp);
    SOC_PBMP_COUNT(mask_port_bmp, counter_mask_port_bmp);

    /* Count the number of links activated in that bmp */

    if (enable && (counter_mask_fsrd_bmp==0)) /* all the ports in the fmac are off, and we want to turn on a port */
    {
        *is_first_link = 1;
    }
    else if ((!enable) && (counter_mask_port_bmp == 1) && (counter_mask_fsrd_bmp == 1)) /* the port we are about to turn off is the only port in the fsrd */
    {
        *is_first_link = 1;
    }
    else
...
soc_fe3200_port_init(int unit)
{
    ...
    rc = portmod_create(unit, SOC_FE3200_NOF_LINKS, SOC_FE3200_NOF_LINKS, 1 , pm_types_and_instances);
    SOCDNX_IF_ERR_EXIT(rc);
...
soc_error_t 
soc_fe3200_port_control_power_set(int unit, soc_port_t port, uint32 flags, soc_dfe_port_power_t power)
{
    int portmod_enable_flags = 0;
    SOCDNX_INIT_FUNC_DEFS;

    if(flags & SOC_DFE_PORT_CONTROL_FLAGS_RX_SERDES_IGNORE){
        portmod_enable_flags = PORTMOD_PORT_ENABLE_MAC | PORTMOD_PORT_ENABLE_RX;
        SOCDNX_IF_ERR_EXIT(portmod_port_enable_set(unit, port, portmod_enable_flags, soc_dfe_port_power_on == power ? 1 : 0));
        portmod_enable_flags = PORTMOD_PORT_ENABLE_TX;
        SOCDNX_IF_ERR_EXIT(portmod_port_enable_set(unit, port, portmod_enable_flags, soc_dfe_port_power_on == power ? 1 : 0));
    }
    else{
        SOCDNX_IF_ERR_EXIT(portmod_port_enable_set(unit, port, portmod_enable_flags, soc_dfe_port_power_on == power ? 1 : 0));
...
soc_error_t 
soc_fe3200_port_control_power_get(int unit, soc_port_t port, soc_dfe_port_power_t* power)
{
    int enable;
    int portmod_enable_flags = 0;
    SOCDNX_INIT_FUNC_DEFS;
    ...
    *power =  enable ? soc_dfe_port_power_on : soc_dfe_port_power_off;
exit:
    SOCDNX_FUNC_RETURN;
}
 ...
 */
soc_error_t 
soc_fe3200_port_control_rx_enable_set(int unit, soc_port_t port, uint32 flags, int enable)
{
 ...
 */
soc_error_t 
soc_fe3200_port_control_tx_enable_set(int unit, soc_port_t port, int enable)
{
 ...
 */
soc_error_t 
soc_fe3200_port_control_rx_enable_get(int unit, soc_port_t port, int* enable)
{
 ...
 */
soc_error_t 
soc_fe3200_port_control_tx_enable_get(int unit, soc_port_t port, int* enable)
{
 ...
 */
soc_error_t 
soc_fe3200_port_loopback_set(int unit, soc_port_t port, soc_dcmn_loopback_mode_t loopback)
{
    ...
    SOCDNX_IF_ERR_EXIT(soc_fe3200_port_loopback_get(unit, port, &current_lb));
    ...
    if(current_lb != soc_dcmn_loopback_mode_none){
        /*open exist loopback*/
        SOCDNX_IF_ERR_EXIT(soc_dcmn_to_portmod_lb(unit, current_lb, &portmod_lb_mode));
    ...
    if(loopback != soc_dcmn_loopback_mode_none){
        ...
        SOCDNX_IF_ERR_EXIT(soc_dcmn_to_portmod_lb(unit, loopback, &portmod_lb_mode));
 ...
 */
soc_error_t 
soc_fe3200_port_loopback_get(int unit, soc_port_t port, soc_dcmn_loopback_mode_t* loopback)
{
    portmod_loopback_mode_t supported_lb_modes[] = { portmodLoopbackMacAsyncFifo, portmodLoopbackMacOuter, portmodLoopbackMacPCS,
        portmodLoopbackPhyGloopPMD,portmodLoopbackPhyRloopPMD};
    soc_dcmn_loopback_mode_t mapped_moeds[] = {soc_dcmn_loopback_mode_mac_async_fifo, soc_dcmn_loopback_mode_mac_outer, soc_dcmn_loopback_mode_mac_pcs,
        soc_dcmn_loopback_mode_phy_gloop, soc_dcmn_loopback_mode_phy_rloop};
    int i = 0;
    int enable = 0;
    SOCDNX_INIT_FUNC_DEFS;
    ...
    *loopback = soc_dcmn_loopback_mode_none;
    for(i = 0 ; i < COUNTOF(supported_lb_modes); i++){
        SOCDNX_IF_ERR_EXIT(portmod_port_loopback_get(unit, port, supported_lb_modes[i], &enable));
        if(enable){
            break;
        }
    }
...
soc_error_t 
soc_fe3200_port_control_pcs_set(int unit, soc_port_t port, soc_dfe_port_pcs_t pcs)
{
    uint32 properties = 0;
    SOCDNX_INIT_FUNC_DEFS;
    if (pcs == soc_dfe_port_pcs_64_66_fec)
    {
        PORTMOD_ENCODING_EXTRCT_CIG_FROM_LLFC_SET(properties); /*CIG From llfc is enabled by default*/
    } else if (pcs == soc_dfe_port_pcs_64_66_rs_fec|| pcs == soc_dfe_port_pcs_64_66_ll_rs_fec)
    {
        PORTMOD_ENCODING_LOW_LATENCY_LLFC_SET(properties);
    }
    SOCDNX_IF_ERR_EXIT(portmod_port_encoding_set(unit, port, properties, pcs));
exit:
    SOCDNX_FUNC_RETURN;
}

soc_error_t 
soc_fe3200_port_control_pcs_get(int unit, soc_port_t port, soc_dfe_port_pcs_t* pcs)
{
    uint32 properties = 0;
    SOCDNX_INIT_FUNC_DEFS;
...
soc_error_t 
soc_fe3200_port_prbs_tx_enable_set(int unit, soc_port_t port, int mode, int value)
{
    int flags = 0;
    SOCDNX_INIT_FUNC_DEFS;
...
soc_error_t 
soc_fe3200_port_prbs_rx_enable_set(int unit, soc_port_t port, int mode, int value)
{
    int flags = 0;
    SOCDNX_INIT_FUNC_DEFS;
...
soc_error_t 
soc_fe3200_port_prbs_tx_enable_get(int unit, soc_port_t port, int mode, int *value)
{
    int flags = 0;
    SOCDNX_INIT_FUNC_DEFS;
...
soc_error_t 
soc_fe3200_port_prbs_rx_enable_get(int unit, soc_port_t port, int mode, int *value)
{
    int flags = 0;
    SOCDNX_INIT_FUNC_DEFS;
...
soc_error_t 
soc_fe3200_port_prbs_rx_status_get(int unit, soc_port_t port, int mode, int* value)
{
...
soc_error_t 
soc_fe3200_port_prbs_tx_invert_data_get(int unit, soc_port_t port, int mode, int *invert)
{
    int flags = 0;
...
soc_error_t 
soc_fe3200_port_prbs_tx_invert_data_set(int unit, soc_port_t port, int mode, int invert)
{
    int flags = 0;
    phymod_prbs_t config;
    SOCDNX_INIT_FUNC_DEFS;
...
soc_error_t 
soc_fe3200_port_prbs_polynomial_set(int unit, soc_port_t port, int mode, int value)
{
    int flags = 0;
...
soc_error_t 
soc_fe3200_port_prbs_polynomial_get(int unit, soc_port_t port, int mode, int *value)
{
    int flags = 0;
    phymod_prbs_t config;
    SOCDNX_INIT_FUNC_DEFS;
...
soc_fe3200_port_burst_control_set(int unit, soc_port_t port, soc_dcmn_loopback_mode_t loopback)
{
    ...
    if (loopback == soc_dcmn_loopback_mode_mac_outer)
    {
...
soc_error_t
soc_fe3200_port_control_fec_error_detect_set(int unit, soc_port_t port, int value)
{
    uint32 properties = 0;
    portmod_port_pcs_t encoding;
    SOCDNX_INIT_FUNC_DEFS;
...
soc_error_t
soc_fe3200_port_control_fec_error_detect_get(int unit, soc_port_t port, int *value)
{
    uint32 properties = 0;
    portmod_port_pcs_t encoding;
    SOCDNX_INIT_FUNC_DEFS;
...
soc_error_t
soc_fe3200_port_control_low_latency_llfc_set(int unit, soc_port_t port, int value)
{
    uint32 properties = 0;
    portmod_port_pcs_t encoding;
    SOCDNX_INIT_FUNC_DEFS;
...
soc_error_t
soc_fe3200_port_control_low_latency_llfc_get(int unit, soc_port_t port, int *value)
{
    uint32 properties = 0;
    portmod_port_pcs_t encoding;
    SOCDNX_INIT_FUNC_DEFS;
...
soc_error_t
soc_fe3200_port_phy_get(int unit, soc_port_t port, uint32 flags,
                 uint32 phy_reg_addr, uint32 *phy_data)
{
    ...
    if ((flags & SOC_PHY_NOMAP) == 0){
        rv = portmod_port_phy_lane_access_get(unit, port, &params, 1, &phy_access, &nof_phys_structs);
        ...
        if (reg_flag & SOC_PHY_REG_INDIRECT) {
            ...
            if (flags & SOC_PHY_NOMAP) {
                ...
                *phy_data = phy_rd_data;
                ...
                rv = PHYMOD_BUS_READ(&phy_access.access, phy_reg_addr, phy_data);
                SOCDNX_IF_ERR_EXIT(rv);
            }
        }
    }
...
soc_error_t
soc_fe3200_port_phy_set(int unit, soc_port_t port, uint32 flags,
                 uint32 phy_reg_addr, uint32 phy_data)
{
    uint16 phy_wr_data;
    uint32  phy_id;
    ...
    if (flags & (SOC_PHY_I2C_DATA8 | SOC_PHY_I2C_DATA16)) {
        ...
        if (reg_flag & SOC_PHY_REG_INDIRECT) {
            ...
            if (flags & BCM_PORT_PHY_NOMAP) {
                ...
                rv = PHYMOD_BUS_WRITE(&phy_access.access, phy_reg_addr, phy_data);
                SOCDNX_IF_ERR_EXIT(rv);
            }
        }
    }
...
soc_fe3200_port_pump_enable_set(int unit, soc_port_t port, int enable)
{
    ...
    SOCDNX_IF_ERR_EXIT(READ_FMAC_FMAL_TX_GENERAL_CONFIGURATIONr(unit, blk, inner_link, &reg32_val));
...
soc_fe3200_port_dynamic_soc_init(int unit, soc_port_t port, int is_first_link)
{
    ...
    *field = soc_reg_field_get(unit, FMAC_LINK_LEVEL_FLOW_CONTROL_ENABLE_REGISTERr, reg32_val, LNK_LVL_FC_RX_ENf);
    ...
    /*Link pump*/
    SOCDNX_IF_ERR_EXIT(READ_FMAC_FMAL_TX_GENERAL_CONFIGURATIONr(unit, fmac_block, fmac_inner_link, &reg32_val));
    ...
    *field = soc_reg_field_get(unit, FMAC_REG_0102r, reg32_val, FIELD_0_3f);
    ...
    if (is_first_link)
    {
        for (blk = fmac_first_block; blk < fmac_first_block +  SOC_DFE_DEFS_GET(unit, nof_quads_in_fsrd) ; blk++)
        {
            if (SOC_DFE_CONFIG(unit).fabric_clk_freq_quad_26 != -1 && blk == 26)
            {
                SOCDNX_IF_ERR_EXIT(READ_FSRD_SRD_QUAD_CTRLr(unit, 2, 2, &reg32_val)); /* 2*12 + 2 */
                soc_reg_field_set(unit, FSRD_SRD_QUAD_CTRLr, &reg32_val, SRD_QUAD_N_LCREF_ENf, 0);
                SOCDNX_IF_ERR_EXIT(WRITE_FSRD_SRD_QUAD_CTRLr(unit, 2, 2, reg32_val));       
            }
            if (SOC_DFE_CONFIG(unit).fabric_clk_freq_quad_35 != -1 && blk == 35)
            {
                SOCDNX_IF_ERR_EXIT(READ_FSRD_SRD_QUAD_CTRLr(unit, 11, 2, &reg32_val)); /* 2*12 + 11 */
                soc_reg_field_set(unit, FSRD_SRD_QUAD_CTRLr, &reg32_val, SRD_QUAD_N_LCREF_ENf, 0);
