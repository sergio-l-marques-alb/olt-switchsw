/*
 * $Id: fe3200_diag.c,v 1.2.12.2 Broadcom SDK $
 *
...
{
	int nof_pipes, curr_max =0, temp, i , j;
	uint32 reg_val;
	uint64 reg_val64;
	...
	for (i = 0 ; i < SOC_DFE_DEFS_GET(unit, nof_instances_dcl) ; i++)
	{
		...
		for (j = 0; j < 4; j++) /* iterate over all possible priorities - as array elements*/
		{
...
soc_fe3200_queues_get_info(int unit, soc_dfe_queues_info_t* fe_queues_info)
{
	...
	for (i = 0; i < SOC_DFE_DEFS_GET(unit, nof_instances_dch); i++)
	{
		/* A Links 0-11, B Links 12-23, C Links 24-35 D Links 36-39*/
		...
		fe_queues_info->dch_queues_info[0].nof_dch[i].nof_link_b = i*SOC_DFE_DEFS_GET(unit, nof_links_in_dcq) + SOC_DFE_DEFS_GET(unit, nof_links_in_dcq)/3 + nof_fifo; /* Start from relevant DCH block + Start from B links (12) + offset in "B Links" */
		...
		fe_queues_info->dch_queues_info[0].nof_dch[i].nof_link_c = i*SOC_DFE_DEFS_GET(unit, nof_links_in_dcq) + 2*(SOC_DFE_DEFS_GET(unit,nof_links_in_dcq)/3) + nof_fifo; /* Start from relevant DCH block + Start from C Links(24) + offset in "C Links" */
	}
	...
	if (nof_pipes >= 2)
	{
		for (i = 0 ; i < SOC_DFE_DEFS_GET(unit, nof_instances_dch); i++)
		{
			...
			fe_queues_info->dch_queues_info[1].nof_dch[i].nof_link_b = i*SOC_DFE_DEFS_GET(unit, nof_links_in_dcq) + SOC_DFE_DEFS_GET(unit, nof_links_in_dcq)/3 + nof_fifo; /* Start from relevant DCH block + Start from B links (12) + offset in "B Links" */
			...
			fe_queues_info->dch_queues_info[1].nof_dch[i].nof_link_c = i*SOC_DFE_DEFS_GET(unit, nof_links_in_dcq) + 2*(SOC_DFE_DEFS_GET(unit,nof_links_in_dcq)/3) + nof_fifo; /* Start from relevant DCH block + Start from C Links(24) + offset in "C Links" */
		}
	}
	/* Pipe 2 */
	if (nof_pipes == 3)
	{
		for (i = 0 ; i < SOC_DFE_DEFS_GET(unit, nof_instances_dch); i++)
		{
			...
			fe_queues_info->dch_queues_info[2].nof_dch[i].nof_link_b = i*SOC_DFE_DEFS_GET(unit, nof_links_in_dcq) + SOC_DFE_DEFS_GET(unit, nof_links_in_dcq)/3 + nof_fifo; /* Start from relevant DCH block + Start from B links (12) + offset in "B Links" */
			...
			fe_queues_info->dch_queues_info[2].nof_dch[i].nof_link_c = i*SOC_DFE_DEFS_GET(unit, nof_links_in_dcq) + 2*(SOC_DFE_DEFS_GET(unit,nof_links_in_dcq)/3) + nof_fifo; /* Start from relevant DCH block + Start from C Links(24) + offset in "C Links" */
		}
	}
