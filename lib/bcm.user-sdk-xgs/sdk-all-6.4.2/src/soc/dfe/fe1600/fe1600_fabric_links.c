...
#define SOC_FE1600_FABRIC_LINKS_ALDWP_MIN                   (4)
#define SOC_FE1600_FABRIC_LINKS_ALDWP_MAX                   (0xf)
...
soc_fe1600_fabric_links_aldwp_config(int unit, soc_port_t port)
{
    ...
    uint32 reg32_val;
    soc_dfe_port_pcs_t encoding = 0;
    int enable = 0;

    SOCDNX_INIT_FUNC_DEFS;

    
    if (SOC_IS_FE3200(unit) && SAL_BOOT_PLISIM)
    {
        SOC_EXIT;
    }
    ...
    highest_aldwp = 0;
    dch_instance = port / SOC_FE1600_NOF_LINKS_IN_DCQ;
    SOCDNX_IF_ERR_EXIT(MBCM_DFE_DRIVER_CALL(unit,mbcm_dfe_fabric_link_device_mode_get,(unit,port, 1/*rx*/, &device_mode)));
    for (port_index = dch_instance * SOC_FE1600_NOF_LINKS_IN_DCQ; 
          port_index < (dch_instance + 1) * SOC_FE1600_NOF_LINKS_IN_DCQ; 
          port_index++)
    {
        ...
        rv = MBCM_DFE_DRIVER_CALL(unit, mbcm_dfe_port_control_pcs_get, (unit, port, &encoding));
        if (encoding == soc_dfe_port_pcs_8_10)
        {
    ...
    } else if (highest_aldwp > SOC_FE1600_FABRIC_LINKS_ALDWP_MAX)
    {
        highest_aldwp =SOC_FE1600_FABRIC_LINKS_ALDWP_MAX;
    }
