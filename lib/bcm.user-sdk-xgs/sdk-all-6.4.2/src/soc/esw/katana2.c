/*
 * $Id: katana2.c,v 1.162 Broadcom SDK $
    ...
    if (_soc_kt2_tcam_ser_info[unit] != NULL) {
        for (i = 0; _soc_kt2_tcam_ser_info[unit][i].mem != INVALIDm; i++) {
            if (_soc_kt2_tcam_ser_info[unit][i].mem == mem) {
                ...
                rv = ser_test_mem(unit, &test_data, test_type, &testErrors);
                ...
                for (table = 0; info[table].type != _SOC_PARITY_TYPE_NONE; table++) {
                    ...
                    if (memTable == mem) {
                        soc_ser_create_test_data(unit, tmp_entry, field_data,
                                                 ...
                                                 &test_data);
                        rv = ser_test_mem(unit, &test_data, test_type, 
                                          &testErrors);
                        if (rv != SOC_E_NONE) {
                            LOG_CLI((BSL_META_U(unit,
                                                "Error during H/W test.  Aborting.\n")));
    ...
    if (_soc_kt2_tcam_ser_info[unit] != NULL) {
        for (i = 0; _soc_kt2_tcam_ser_info[unit][i].mem != INVALIDm; i++) {
            rv = ser_test_mem_pipe(unit, SER_RANGE_ENABLEr, i, -1,
                                   _soc_kt2_tcam_ser_info[unit][i].mem, INVALIDf, 
...
static uint32 kt2_tdm_A2_56450_14[216]={0};
...
static uint32 kt2_tdm_A2_56450_15[216]={0};
...
static uint32 *bcm56450_tdm[]={
               ...
               kt2_tdm_A1_56450_9,               /*56248 Cfg=9:IntCfg=40 */
               ...
               kt2_tdm_A2_56450_14,              /*56248 Cfg=14:IntCfg=45 */
               kt2_tdm_A2_56450_15,              /*56248 Cfg=15:IntCfg=46 */
...
static bcm56450_tdm_info_t bcm56450_tdm_info[]={
	       ...
	       {205, 88, 8, 11},                     /*56450 Cfg=4:IntCfg=4 */
               ...
               {205, 216, 12, 9},                    /*56450 Cfg=14:IntCfg=14 */
               {205, 216, 12, 9},                    /*56450 Cfg=15:IntCfg=15 */
               ...
               {205, 108, 12, 9},                    /*56248 Cfg=8:IntCfg=39 */
               ...
               {205, 216, 24, 9},                    /*56248 Cfg=14:IntCfg=45 */
               {205, 216, 24, 9},                    /*56248 Cfg=15:IntCfg=46 */
...
/*    } */
    ...
    for (sub_port = KT2_MIN_SUBPORT_INDEX;
         sub_port <= KT2_MAX_SUBPORT_INDEX;
         sub_port ++) {
    ...
    case BCM56458_DEVICE_ID:
         ...
         case BCM56450_DEVICE_ID_OFFSET_CFG + 14: /* kt2_tdm_A2_56450_14 */
                  ...
                  sal_memcpy(arr,
                             &kt2_tdm_56450A2_ref[0],
                             sizeof(kt2_tdm_56450A2_ref));
                  kt2_tdm_replace(arr,tdm_size,21,KT2_IDLE,0);
                  kt2_tdm_replace(arr,tdm_size,17,KT2_IDLE,0);
                  olp_port_flag=1;
                  break;
         ...
         case BCM56450_DEVICE_ID_OFFSET_CFG + 15: /* kt2_tdm_A2_56450_15 */
                  ...
                  memcpy(arr,
                         ...
                         sizeof(kt2_tdm_56450A2_ref));
                  kt2_tdm_replace(arr,tdm_size,21,KT2_IDLE,0);
                  kt2_tdm_replace(arr,tdm_size,13,KT2_IDLE,0);
                  kt2_tdm_replace(arr,tdm_size,9,60,0);
                  tdm_index=0;
                  for (outer_index=0;outer_index < 2; outer_index++) {
                       for (port=9; port <=12 ; port ++) {
                            while(arr[tdm_index] != 60) {
                                  tdm_index++;
                            }
                            arr[tdm_index]= port;
                       }
                       ...
                       arr[tdm_index]= KT2_IDLE;
                       while(arr[tdm_index] != 60) {
                             tdm_index++;
                       }
                       arr[tdm_index]= KT2_IDLE;
                       for (port=13; port <=16 ; port ++) {
                            while(arr[tdm_index] != 60) {
                                  tdm_index++;
                            }
                            arr[tdm_index]= port;
...
int _soc_katana2_misc_init(int unit)
{
    ...
    *  2 instances one each for evlan and ehcpm stages of epipe */
    ...
    if (SOC_REG_IS_VALID(unit, FP_CAM_CONTROL_TM_13_THRU_0r)) {
        SOC_IF_ERROR_RETURN(READ_FP_CAM_CONTROL_TM_13_THRU_0r(unit, &rval));
        soc_reg_field_set(unit, FP_CAM_CONTROL_TM_13_THRU_0r,
                          &rval, ALL_TCAMS_TM_13_0f, val);
        soc_reg_field_set(unit, FP_CAM_CONTROL_TM_13_THRU_0r,
                          &rval, ALL_GLOBAL_MASK_TCAMS_TM_13_0f, val);
        SOC_IF_ERROR_RETURN(WRITE_FP_CAM_CONTROL_TM_13_THRU_0r(unit, rval));
    }
...
/* Used in mmu reconfigure path for flex Io ports */ 

STATIC int
_soc_katana2_mmu_init_phase4(int unit) 
{
    int    port=0;
    int    mxqblock=0;
    int    subport=0;
    ...
    for (mxqblock=0;mxqblock<KT2_MAX_MXQBLOCKS ; mxqblock++) {
         for (subport=0;subport<KT2_MAX_MXQPORTS_PER_BLOCK; subport++) {
              port = (*kt2_mxqblock_ports[unit])[mxqblock][subport];
              SOC_IF_ERROR_RETURN(_soc_katana2_mmu_init_phase4_port(
                                  unit,port)); 
         }
    }
    return SOC_E_NONE;
...
/* Just splitted _soc_katana2_mmu_init() for flex IO feature */
STATIC int
_soc_katana2_mmu_init_phase3(int unit)
{
    int    port=0;
    uint32 rval=0;
    for (port=0;port<42;port++) {
         ...
         if (((port > KT2_CMIC_PORT) && (port < KT2_LPBK_PORT) &&
             ...
             if (soc_reg_field_valid(unit, MMU_ENQ_IP_PRI_TO_PG_PROFILE_0r, 
                 ...
                 if (!((port == KT2_OLP_PORT) && 
                       (SOC_INFO(unit).olp_port[0] == 1))) {
                     LOG_CLI((BSL_META_U(unit,
                                         "WARN: Suspicious Config:%s:%d Port=%d\n"),
...
_soc_katana2_mmu_init(int unit)
{
    ...
    SOC_IF_ERROR_RETURN(_soc_katana2_mmu_init_phase3(unit));
...
{
  SOC_IF_ERROR_RETURN(_soc_katana2_mmu_init_phase4(unit)); 
  SOC_IF_ERROR_RETURN(_soc_katana2_mmu_init_phase3(unit)); 
  SOC_IF_ERROR_RETURN(_soc_katana2_mmu_init_helper(unit));
  LOG_CLI((BSL_META_U(unit,
                      "MMU is reconfigured \n")));
                ...
                if (si->port_cosq_base[i] + si->port_num_uc_cosq[i] >
                    ...
                    LOG_ERROR(BSL_LS_SOC_COMMON,
                              (BSL_META_U(unit,
                               ...
                               port, max_queue_num,
                               si->port_cosq_base[i],
                               si->port_cosq_base[i] + si->port_num_uc_cosq[i]));
                    return SOC_E_CONFIG;
                }
                /*if (soc_feature(unit, soc_feature_mmu_packing)) { */
                if (packing_mode) {
...
/* #ifdef BROADCOM_SVK  */
   ...
   if (READ_TOP_MISC_CONTROL_1r(unit, &top_misc_control_1_val) != SOC_E_NONE) {
       return;
   }
   wc_xfi_mode_sel_val[0] = soc_reg_field_get(unit, TOP_MISC_CONTROL_1r,
                                              top_misc_control_1_val,
                                              wc_xfi_mode_sel_fld[0]);
   wc_xfi_mode_sel_val[1] = soc_reg_field_get(unit, TOP_MISC_CONTROL_1r,
                                              top_misc_control_1_val,
   ...
   if (auto_polarity_flip) {
       if (soc_mem_config_set) {
           LOG_ERROR(BSL_LS_SOC_COMMON,
                     (BSL_META_U(unit,
                                 "Broadcom SVK..Adding required polarity flips \n")));
       } else {
           LOG_ERROR(BSL_LS_SOC_COMMON,
                     (BSL_META_U(unit,
                                 "=====================================\n")));
           LOG_ERROR(BSL_LS_SOC_COMMON,
                     (BSL_META_U(unit,
                                 "soc_mem_config_set() NULL so cannot set"
                                 "PolarityFlips automatically.\n"
                                 "## Suggesting to ## Update config.bcm with\n")));
           LOG_ERROR(BSL_LS_SOC_COMMON,
                     (BSL_META_U(unit,
                                 "=====================================\n")));
       }
       for (mxqblock = 0; mxqblock < 8; mxqblock++) {
            for (loop=0; loop < KT2_MAX_MXQPORTS_PER_BLOCK; loop++) {       
                 ...
                 if (wc_xfi_mode_sel_val[0]) {
                     if (((*kt2_mxqblock_ports[unit])[mxqblock][loop] == 25) ||
                         ((*kt2_mxqblock_ports[unit])[mxqblock][loop] == 36)) {
                         _soc_kt2_mem_config_set(unit,config_str,NULL);
                         continue;
                     }  
                 }
                 if (wc_xfi_mode_sel_val[1]) {
                     if (((*kt2_mxqblock_ports[unit])[mxqblock][loop] == 26) ||
                         ((*kt2_mxqblock_ports[unit])[mxqblock][loop] == 39)) {
                         _soc_kt2_mem_config_set(unit,config_str,NULL);
                         continue;
                     }  
                 }
   ...
   if (auto_portgroup) {
       if (SOC_WARM_BOOT(unit)) {
           LOG_CLI((BSL_META_U(unit,
                               "Broadcom SVK..Skipping required port groups \n")));
           return;
       }
      if (soc_mem_config_set) {
           LOG_ERROR(BSL_LS_SOC_COMMON,
                     (BSL_META_U(unit,
                                 "Broadcom SVK..Adding required port groups \n")));
       } else {
           LOG_ERROR(BSL_LS_SOC_COMMON,
                     (BSL_META_U(unit,
                                 "=====================================\n")));
           LOG_ERROR(BSL_LS_SOC_COMMON,
                     (BSL_META_U(unit,
                                 "soc_mem_config_set() NULL so cannot set "
                                 "WarpCore PortGroups automatically.\n"
                                 "## Suggesting to ## Update config.bcm with \n")));
           LOG_ERROR(BSL_LS_SOC_COMMON,
                     (BSL_META_U(unit,
                                 "=====================================\n")));
