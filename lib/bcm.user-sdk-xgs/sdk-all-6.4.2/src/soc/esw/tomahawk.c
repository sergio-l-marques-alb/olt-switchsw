...
{
    switch (dev_id) {
    /* Note change this to 56965 later and set the corresponding value below */
...
soc_tomahawk_port_speed_update(int unit, soc_port_t port, int speed)
{
    ...
    SOC_IF_ERROR_RETURN
        (_soc_tomahawk_mc_toq_cfg(unit, port, (speed >= 40000) ? TRUE : FALSE));
...
_soc_tomahawk_clear_all_memory(int unit)
{
    ...
    SOC_PBMP_ITER(PBMP_ALL(unit), port) {
        SOC_IF_ERROR_RETURN
            (WRITE_EGR_1588_LINK_DELAY_64r(unit, REG_PORT_ANY, rval64));
    }
