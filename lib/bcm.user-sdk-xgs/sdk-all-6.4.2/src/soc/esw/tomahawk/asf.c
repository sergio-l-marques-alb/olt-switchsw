    ...
    soc_th_asf_mode_e   mode)
{
    ...
    /* init CT class */
    if (SOC_E_NONE !=
        (ret = _soc_th_port_asf_class_init(unit, port, speed_encoding)) ) {
        return SOC_E_INIT;
    }
    ...
    int        port_speed)
{
    ...
    LOG_CLI(("%-4s     %-11s      ", SOC_PORT_NAME(unit, port), mode_str[mode]));

    if (_SOC_TH_ASF_MODE_SAF == mode) {
        LOG_CLI(("    .. NA ..\n"));
    } else if (_SOC_TH_ASF_MODE_FAST_TO_SLOW == mode) {
        ret = _soc_th_port_asf_speed_limits_get(unit, port, port_speed,
    ...
    int        port_speed)
{
    ...
    LOG_CLI(("%-4s %-3d %-3d %-6d %-6d %-3d %-3d   ", SOC_PORT_NAME(unit, port),
