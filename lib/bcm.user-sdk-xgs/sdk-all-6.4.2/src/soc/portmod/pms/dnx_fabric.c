...
int dnx_fabric_wb_buffer_init(int unit, int wb_buffer_index, pm_info_t pm_info)
{
    ...
    SOC_WB_ENGINE_ADD_VAR(SOC_WB_ENGINE_PORTMOD, wb_var_id, "is_initialzed", wb_buffer_index, sizeof(uint32), NULL, VERSION(1));
    SOCDNX_IF_ERR_EXIT(rv);
...
STATIC
int dnx_fabric_core_init_phase1(int unit, int port, pm_info_t pm_info, const portmod_port_interface_config_t* config)
{
    ...
    fsrd_schan_id = pm_info->pm_data.dnx_fabric->fsrd_schan_id | SOC_REG_ADDR_BLOCK_ID_MASK;
...
{
    int rv, fsrd_schan_id, fsrd_internal_quad;
    uint32 is_core_initialized, reg_val;
    SOCDNX_INIT_FUNC_DEFS;
...
int dnx_fabric_port_enable_set(int unit, int port, pm_info_t pm_info, int flags, int enable)
{
    ...
    if(enable){
        if(PORTMOD_PORT_ENABLE_MAC_GET(actual_flags)){
            ...
            SOCDNX_IF_ERR_EXIT(READ_FMAC_RECEIVE_RESET_REGISTERr(unit, fabric_data->fmac_schan_id | SOC_REG_ADDR_BLOCK_ID_MASK, &reg_val));
            if(PORTMOD_PORT_ENABLE_RX_GET(actual_flags)){
            ...
            SOCDNX_IF_ERR_EXIT(WRITE_FMAC_RECEIVE_RESET_REGISTERr(unit, fabric_data->fmac_schan_id | SOC_REG_ADDR_BLOCK_ID_MASK, reg_val));
        }    
        if(PORTMOD_PORT_ENABLE_PHY_GET(actual_flags)){
...
int dnx_fabric_port_interface_config_set(int unit, int port, pm_info_t pm_info, const portmod_port_interface_config_t* config)
{
    ...
    if(!is_core_initialized){
        SOCDNX_IF_ERR_EXIT(dnx_fabric_core_init_phase1(unit, port, pm_info , config));
    }
    ...
    if(!is_core_initialized){
        SOCDNX_IF_ERR_EXIT(dnx_fabric_core_init_phase2(unit, port, pm_info , config));
    }
...
int dnx_fabric_port_interface_config_get(int unit, int port, pm_info_t pm_info, portmod_port_interface_config_t* config)
{
    ...
    phy_config.ref_clock = fabric_data->ref_clk; /*in field*/
    SOCDNX_IF_ERR_EXIT(phymod_phy_interface_config_get(&phy, config->flags, &phy_config));
...
int dnx_fabric_port_encoding_set(int unit, int port, pm_info_t pm_info, uint32 properties, portmod_port_pcs_t encoding)
{
    ...
    int cig_ignore = 0;
    dnx_fabric_t fabric_data = pm_info->pm_data.dnx_fabric;
    int fmac_blk_id_masked = fabric_data->fmac_schan_id | SOC_REG_ADDR_BLOCK_ID_MASK;
    uint32 phy_index;
    uint32 reg_val;
    uint64 reg64_val;
    SOCDNX_INIT_FUNC_DEFS;
...
_dnx_fabric_force_signal_detect_set(int unit, pm_info_t pm_info)
{
    ...
    dnx_fabric_t fabric_data = pm_info->pm_data.dnx_fabric;
    int fmac_blk_id_masked = fabric_data->fmac_schan_id | SOC_REG_ADDR_BLOCK_ID_MASK;
    portmod_loopback_mode_t mac_loopbacks[]= {portmodLoopbackMacAsyncFifo, portmodLoopbackMacOuter, portmodLoopbackMacPCS};
    int lb_enable = FALSE;
    SOCDNX_INIT_FUNC_DEFS;

    for(port_index = 0 ; port_index < MAX_PORTS_PER_DNX_FABRIC_PM; port_index++){
        SOC_WB_ENGINE_GET_ARR(unit, SOC_WB_ENGINE_PORTMOD, pm_info->wb_vars_ids[wb_ports], &ports_db_entry, port_index);
        SOCDNX_IF_ERR_EXIT(rv);
        if(ports_db_entry < 0){
            continue;
        }
...
int dnx_fabric_mac_loopback_set(int unit, int port, pm_info_t pm_info, portmod_loopback_mode_t loopback_type, int enable)
{
    uint32 reg_val = 0;
    uint32 field[1] = {0};
    uint32 fmac_blk_id_masked = pm_info->pm_data.dnx_fabric->fmac_schan_id | SOC_REG_ADDR_BLOCK_ID_MASK;
    uint32 phy_index;
    SOCDNX_INIT_FUNC_DEFS;
...
int dnx_fabric_port_link_get(int unit, int port, pm_info_t pm_info, int* link)
{
    ...
    if (enable) {
        ...
        SOCDNX_IF_ERR_EXIT(WRITE_FMAC_FMAC_INTERRUPT_REGISTER_2r(unit,  fabric_data->fmac_schan_id | SOC_REG_ADDR_BLOCK_ID_MASK,reg_val));
        
        /*check rxlos interrupt - link is up*/

        SOCDNX_IF_ERR_EXIT(READ_FMAC_FMAC_INTERRUPT_REGISTER_2r(unit, fabric_data->fmac_schan_id | SOC_REG_ADDR_BLOCK_ID_MASK, &reg_val));
...
int dnx_fabric_port_prbs_config_set(int unit, int port, pm_info_t pm_info, int mode, int flags, const phymod_prbs_t* config)
{
    ...
    if(mode == 0){ /*phy*/
        ...
        if(PHYMOD_PRBS_DIRECTION_RX_GET(flags) || (flags == 0)){
            SOCDNX_IF_ERR_EXIT(READ_FMAC_KPCS_TEST_RX_CONFIGURATIONr(unit, fabric_data->fmac_schan_id | SOC_REG_ADDR_BLOCK_ID_MASK, phy_index, &reg_val));
            soc_reg_field_set(unit, FMAC_KPCS_TEST_RX_CONFIGURATIONr, &reg_val, KPCS_N_TST_RX_PTRN_SELf, poly_val);
            SOCDNX_IF_ERR_EXIT(WRITE_FMAC_KPCS_TEST_RX_CONFIGURATIONr(unit, fabric_data->fmac_schan_id | SOC_REG_ADDR_BLOCK_ID_MASK, phy_index, reg_val));
        }
        if(PHYMOD_PRBS_DIRECTION_TX_GET(flags) || (flags == 0)){
            SOCDNX_IF_ERR_EXIT(READ_FMAC_KPCS_TEST_TX_CONFIGURATIONr(unit, fabric_data->fmac_schan_id | SOC_REG_ADDR_BLOCK_ID_MASK, phy_index, &reg_val));
            soc_reg_field_set(unit, FMAC_KPCS_TEST_TX_CONFIGURATIONr, &reg_val, KPCS_N_TST_TX_PTRN_SELf, poly_val);
            SOCDNX_IF_ERR_EXIT(WRITE_FMAC_KPCS_TEST_TX_CONFIGURATIONr(unit, fabric_data->fmac_schan_id | SOC_REG_ADDR_BLOCK_ID_MASK, phy_index, reg_val));
        }
    }
exit:
    SOCDNX_FUNC_RETURN; 
...
int dnx_fabric_port_prbs_status_get(int unit, int port, pm_info_t pm_info, int mode, int flags, phymod_prbs_status_t* status)
{
    ...
    if(mode == 0){ /*phy*/
        ...
        SOCDNX_IF_ERR_EXIT(dnx_fabric_port_phy_index_get(unit, port, &phy_index));
        SOCDNX_IF_ERR_EXIT(READ_FMAC_FPS_RX_STATUSr(unit, fabric_data->fmac_schan_id | SOC_REG_ADDR_BLOCK_ID_MASK, phy_index, &reg_val));
        status->prbs_lock = soc_reg_field_get(unit, FMAC_FPS_RX_STATUSr, reg_val, FPS_N_RX_SYNC_STATUSf);
        status->prbs_lock_loss = soc_reg_field_get(unit, FMAC_FPS_RX_STATUSr, reg_val, FPS_N_RX_SYNC_STATUS_CHf);
        /*error counter*/
        SOCDNX_IF_ERR_EXIT(READ_FMAC_KPCS_TEST_RX_STATUSr(unit, fabric_data->fmac_schan_id | SOC_REG_ADDR_BLOCK_ID_MASK, phy_index, &reg_val));
