...
dcmn_intr_handler_deinit(int unit)
{
    ...
    soc_event_unregister(unit, dcmn_intr_switch_event_cb, NULL);
...
#ifdef BCM_JERICHO_SUPPORT
    if (SOC_IS_JERICHO(unit)) {
        rc = jer_interrupts_array_init(unit); 
        SOCDNX_IF_ERR_EXIT(rc);
...
#ifdef BCM_88950
    if (SOC_IS_FE3200(unit)) {
        rc = fe3200_interrupts_array_init(unit); 
        SOCDNX_IF_ERR_EXIT(rc);
...
#endif
    rc = soc_event_register(unit, dcmn_intr_switch_event_cb, NULL);
    SOCDNX_IF_ERR_EXIT(rc);
