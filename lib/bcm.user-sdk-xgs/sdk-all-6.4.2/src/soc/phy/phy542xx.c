                                 ...
                                 soc_port_phy_timesync_config_t *conf)
{
    ...
    data16 = mask16 = 0;
    if ( PHY_IS_BCM5429X(pc) && (conf->validity_mask &
                     SOC_PORT_PHY_TIMESYNC_VALID_PHY_1588_INBAND_TS_CONTROL) ) {
        phy_bcm542xx_direct_reg_read(unit, pc,
    ...
    if ( (conf->validity_mask &
        ...
        if (data16 & PHY_BCM542XX_TOP_1588_INBAND_CNTL_RESV0_ID_CHECK) {
            conf->inband_ts_control.flags |=
                SOC_PORT_PHY_TIMESYNC_INBAND_TS_RESV0_ID_CHECK;
        }
        if (data16 & PHY_BCM542XX_TOP_1588_INBAND_CNTL_SYNC_EN ) {
            conf->inband_ts_control.flags |=
                SOC_PORT_PHY_TIMESYNC_INBAND_TS_SYNC_ENABLE;
        }
        if (data16 & PHY_BCM542XX_TOP_1588_INBAND_CNTL_DELAY_REQ_EN ) {
            conf->inband_ts_control.flags |=
                SOC_PORT_PHY_TIMESYNC_INBAND_TS_DELAY_RQ_ENABLE;
        }
        if (data16 & PHY_BCM542XX_TOP_1588_INBAND_CNTL_PDELAY_REQ_EN ) {
            conf->inband_ts_control.flags |=
                SOC_PORT_PHY_TIMESYNC_INBAND_TS_PDELAY_RQ_ENABLE;
        }
        if (data16 & PHY_BCM542XX_TOP_1588_INBAND_CNTL_PDELAY_RESP_EN ) {
            conf->inband_ts_control.flags |=
                SOC_PORT_PHY_TIMESYNC_INBAND_TS_PDELAY_RESP_ENABLE;
        }

        if (data16 & PHY_BCM542XX_TOP_1588_INBAND_CNTL_RESV0_ID_UPDATE) {
            conf->inband_ts_control.flags |=
                SOC_PORT_PHY_TIMESYNC_INBAND_TS_RESV0_ID_UPDATE;
        }
        ...
        if (data16 & PHY_BCM542XX_TOP_1588_CAP_SRC_PORT_CLK_ID) {
            conf->inband_ts_control.flags |=
                SOC_PORT_PHY_TIMESYNC_INBAND_TS_CAP_SRC_PORT_CLK_ID;
        }
                              ...
                              int offset, uint32 value)
{
    ...
    /* RDB 0x83c bit[3:0], REC_CLK1 primary recovery clodk */
    if ( value) {
        /* set bit[3] = 0 to enable clock output, also clear bit[2:0] */
        data &= ~(PHY_BCM542XX_SYNCE_RECOVERY_CLK_REG_REC_CLK1_DIS |
                  PHY_BCM542XX_SYNCE_RECOVERY_CLK_REG_REC_CLK1_SEL);
        /* select port which recovered clock REC_CLK1 is deprived from */
        data |= (offset & PHY_BCM542XX_SYNCE_RECOVERY_CLK_REG_REC_CLK1_SEL);
    } else {
        /* set bit[3] = 1 to disable recovered clock output */
        data |=  (PHY_BCM542XX_SYNCE_RECOVERY_CLK_REG_REC_CLK1_DIS);
    }
 ...
 * Purpose:
 *    SyncE secondary recovery clodk
 * Parameters:
 * Returns:
 */
int
phy_bcm542xx_clock_secondary_enable_set(int unit, phy_ctrl_t *pc,
                                        int offset, uint32 value)
{
    uint16  data;
    ...
    SOC_IF_ERROR_RETURN(
        phy_bcm542xx_direct_reg_read(unit, pc,
                PHY_BCM542XX_SYNCE_RECOVERY_CLK_REG_OFFSET, &data) );
    /* RDB 0x83c bit[7:4], REC_CLK2 secondary recovery clodk */
    if ( value) {
        /* set bit[7] = 0 to enable clock output, also clear bit[6:4] */
        data &= ~(PHY_BCM542XX_SYNCE_RECOVERY_CLK_REG_REC_CLK2_DIS |
                  PHY_BCM542XX_SYNCE_RECOVERY_CLK_REG_REC_CLK2_SEL);
        /* select port which recovered clock REC_CLK2 is deprived from */
        data |= (offset & PHY_BCM542XX_SYNCE_RECOVERY_CLK_REG_REC_CLK2_SEL);
    } else {
        /* set bit[7] = 1 to disable recovered clock output */
        data |=  (PHY_BCM542XX_SYNCE_RECOVERY_CLK_REG_REC_CLK2_DIS);
    }
    /* write to SYNCE Recovery Clock Registe, RDB 0x83c */
    SOC_IF_ERROR_RETURN(
 ...
 */
int
phy_bcm542xx_clock_secondary_enable_get(int unit, phy_ctrl_t *pc,
                                        int offset, uint32 *value)
{
    uint16  data;
    *value = TRUE;
 ...
 */
int
phy_bcm542xx_eee_enable(int unit, soc_port_t port,
                        soc_port_ability_t *ability, int enable)
{
    phy_ctrl_t  *pc;
                                          ...
                                          uint32 *value)
{
    ...
    phy_bcm542xx_direct_reg_read(unit, pc,  /* bit[0] MII_BUFFER_ENABLE */
                     PHY_BCM542XX_TOP_MISC_MII_BUFF_CNTL_PHYN(dev_port), 
                     &temp16);
                             ...
                             soc_phy_control_t type, uint32 *value)
{
    ...
    case SOC_PHY_CONTROL_EEE_TRANSMIT_EVENTS:
        ...
        _phy_bcm542xx_eee_counter_get(unit, pc,
                         PHY_BCM542XX_EEE_STAT_TX_EVENTS_REG_OFFSET,
                         value);
        break;
    ...
    case SOC_PHY_CONTROL_EEE_TRANSMIT_DURATION:
        ...
        _phy_bcm542xx_eee_counter_get(unit, pc,
                         PHY_BCM542XX_EEE_STAT_TX_DURATION_REG_OFFSET,
                         value);
        break;
    ...
    case SOC_PHY_CONTROL_EEE_RECEIVE_EVENTS:
        ...
        _phy_bcm542xx_eee_counter_get(unit, pc,
                         PHY_BCM542XX_EEE_STAT_RX_EVENTS_REG_OFFSET,
                         value);
        break;
    ...
    case SOC_PHY_CONTROL_EEE_RECEIVE_DURATION:
        ...
        _phy_bcm542xx_eee_counter_get(unit, pc,
                         PHY_BCM542XX_EEE_STAT_RX_DURATION_REG_OFFSET,
                         value);
        break;
                         ...
                         soc_phy_control_t type, uint32 value)
{
    ...
    case SOC_PHY_CONTROL_CLOCK_ENABLE:
        phy_bcm542xx_clock_enable_set(unit, pc,
                                (int)PHY_BCM542XX_DEV_PHY_SLICE(pc), value);
        break;
    case SOC_PHY_CONTROL_CLOCK_SECONDARY_ENABLE:
        phy_bcm542xx_clock_secondary_enable_set(unit, pc,
                                (int)PHY_BCM542XX_DEV_PHY_SLICE(pc), value);
        break;
    case SOC_PHY_CONTROL_CLOCK_MODE_AUTO:
        rv = _phy_bcm542xx_clock_control_set(unit, pc, value,
                         ...
                         soc_phy_control_t type, uint32 *value)
{
    ...
    /* clock controls */
    case SOC_PHY_CONTROL_CLOCK_ENABLE:
        phy_bcm542xx_clock_enable_get(unit, pc,
                                (int)PHY_BCM542XX_DEV_PHY_SLICE(pc), value);
        break;
    case SOC_PHY_CONTROL_CLOCK_SECONDARY_ENABLE:
        phy_bcm542xx_clock_secondary_enable_get(unit, pc,
                                (int)PHY_BCM542XX_DEV_PHY_SLICE(pc), value);
        break;
    case SOC_PHY_CONTROL_CLOCK_MODE_AUTO:
    ...
    case SOC_PHY_CONTROL_EEE:
        ...
        _phy_bcm542xx_get_eee_control_status(unit, pc,value);
        break;
    case SOC_PHY_CONTROL_EEE_AUTO:
        if ( ! PHY_FLAGS_TST(unit, port, PHY_FLAGS_EEE_CAPABLE) ) {
            rv = SOC_E_UNAVAIL;
        ...
        _phy_bcm542xx_get_auto_eee_control_status(unit, port, value);
        break;
    case SOC_PHY_CONTROL_EEE_AUTO_FIXED_LATENCY:
        if ( ! PHY_FLAGS_TST(unit, port, PHY_FLAGS_EEE_CAPABLE) ) {
            rv = SOC_E_UNAVAIL;
...
phy_bcm542xx_probe(int unit, phy_ctrl_t *pc) {
    ...
    case (PHY_BCM54292_MODEL | PHY_BCM54295_MODEL | PHY_BCM54296_MODEL):
        if ( rev & 0x8 ) {
            ...
            pi->phy_name = (hidden_rev & 0x8) ? "BCM54296" : "BCM54295";
        } else {
