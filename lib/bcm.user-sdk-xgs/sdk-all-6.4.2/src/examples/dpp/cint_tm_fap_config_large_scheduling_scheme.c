...
{
    int rv;
    bcm_gport_t cir_gport, eir_gport;
    bcm_cosq_gport_info_t gport_info;
    if ((rv = bcm_cosq_gport_add(unit, 0, 1, flags, &cir_gport)) != BCM_E_NONE) {
                  ...
                  int num_l2_se)
{
    ...
    */
    ...
    BCM_PBMP_ITER(port_set,port) {
         for (se_elem = 0; se_elem < L4_NUM_ELEMENTS; se_elem++) {
             ...
             cir_gport = cint_composite_sched_config (unit, flags, L4_CIR_RATE,
    ...
    BCM_PBMP_ITER(port_set,port) {
        for (l4_index = 0; l4_index < L4_NUM_ELEMENTS; l4_index++) {
            for (se_elem = 0; se_elem < L3_NUM_ELEMENTS; se_elem++) {
                ...
                cir_gport = cint_composite_sched_config (unit, flags, L3_CIR_RATE,
    ...
    */
        ...
        for (l4_index = 0; l4_index < L4_NUM_ELEMENTS; l4_index++) {
            for (l3_index = 0; l3_index < L3_NUM_ELEMENTS; l3_index++) {
                for (se_elem=0; se_elem < L2_NUM_ELEMENTS; se_elem++) {
                    ...
                    cir_gport = cint_composite_sched_config (unit, flags,
    ...
    BCM_PBMP_ITER(port_set,port) {
        /* create gport  */
        BCM_GPORT_MODPORT_SET(portmod_gport, my_modid, port);
        ...
        for (l4_index = 0; l4_index < L4_NUM_ELEMENTS; l4_index++) {
            ...
            for (l3_index = 0; l3_index < L3_NUM_ELEMENTS; l3_index++) {
                for (se_elem = 0; se_elem < L2_NUM_ELEMENTS; se_elem++) {
                    ...
                    BCM_COSQ_GPORT_VOQ_CONNECTOR_SET(vc_group,
			...
			/* attach VOQ connector to level 2 CL */
                    ...
                    */
                    ...
                    BCM_GPORT_UNICAST_QUEUE_GROUP_SET(gvoq_base,
                                                      (voq_index * 4) + voq_base);
                         ...
                         int  rate)
{
    ...
    BCM_COSQ_GPORT_VOQ_CONNECTOR_SET(vc_group, voq_bundle_id);
