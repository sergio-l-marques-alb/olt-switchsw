...
int config_ports_rate(int unit, bcm_pbmp_t pbmp, int kbits_sec_max, int header_type ,int is_port_add)
{
    ...
    bcm_port_interface_info_t interface_info;
    bcm_port_mapping_info_t   mapping_info;
    int                   local_port_speed, new_interface_speed;
    ...
    BCM_PBMP_ITER(pbmp, port) /*configure each port in pbmp*/
    {
        ...
        if (is_port_add) 
        {
            ...
            if (is_channelized) {
                if ((kbits_sec_max_get + kbits_sec_max) <= (port_phy_speed*interface_info.num_lanes*1000 /* maximal interface speed */)) {
                ...
                {
                    rv = bcm_cosq_gport_bandwidth_set(unit,fap_port,0,0,(port_phy_speed*interface_info.num_lanes*1000 /* maximal interface speed */),0);
                    if (rv != BCM_E_NONE) {
                        printf("Error, bcm_cosq_gport_bandwidth_set,unit $unit fap gport: %x, $rv\n", fap_port);
                        return rv;
                    } 
