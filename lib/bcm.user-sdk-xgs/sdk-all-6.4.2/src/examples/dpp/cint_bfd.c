...
create_tunnel_initiator(int unit, int* ingress_intf, int *tunnel_id, int extend_example) {
    ...
    if (!is_aradplus || pipe_mode_exp_set) {
...
int bfd_echo_example(int unit, int port1, int port2, int is_acc, int remote_port){
    ...
    rv =  bfd_ipv4_example(unit, is_acc, remote_gport);
    if (rv != BCM_E_NONE) {
...
int bfd_ipv4_example(int unit, int endpoint_acc, bcm_gport_t remote_gport) {
    int rv;
...
int bfd_mpls_example(int unit) {
  ...
  */
  ...
  bfd_endpoint_info.local_flags = 2;
  bfd_endpoint_info.egress_label.label = tunnel_switch.label;
...
int bfd_pwe_example(int unit) {
  ...
  */
  ...
  bfd_endpoint_info.type = is_router_alert ? bcmBFDTunnelTypePweRouterAlert : bcmBFDTunnelTypePweControlWord;
...
int bfd_ipv4_run_with_defaults(int unit, int port1, int port2, int is_acc, int remote_port) {
  bcm_error_t rv;
...
int bfd_pwe_run_with_defaults(int unit, int port1, int port2) {
  bcm_error_t rv;
