...
int bcm_l3_ip6_4_label_push_protection(int unit,  int in_port, int out_port, int ttl_threshold, int is_protection) {
  ...
  if (is_protection==0) {
      rc = create_tunnel_initiator_4_label_push(unit, &ingress_intf[1], &tunnel_id); 
    	if (rc != BCM_E_NONE) {
  ...
  /* create fec for UC IPV6 entry */
  rc = create_l3_egress(unit,CINT_NO_FLAGS, out_port, out_vlan, tunnel_id, mac_l3_egress, &egress_intf[0], &encap_id[0]);
