...
*/
/*
*  This CINT is meant to provide an example of building an ARAD scheduling scheme in 8 priorities mode.
...
* 
* For this example to run you must bring up the ARAD device with the following SOC properties:
* diag_cosq_disable=1
*/ 
cint_reset();
...
int
port_internal_id_get(int modid, int port)
{
    int port_i;
...
convert_modport_to_sysport_id(int modid,int port)
{
    ...
    if (internal_port_idx != -1) {
        return (g_info.offset_start_of_sys_port * modid + internal_port_idx 
 ...
 */
int
bcm_arad_tm_fap_cosq(int unit, int modid, int nof_units, int* unit_arr, int* modid_arr)
{
    int rv = BCM_E_NONE;
    int cfg_2nd_unit;    
    int unit_idx;
