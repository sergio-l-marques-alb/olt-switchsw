...
/*
 * $Id: diag_ocb.c,v 1.4 Broadcom SDK $
 ...
 *
 * File:    diag_ocb.c
 * Purpose: Manages ocb diagnostics functions 
 */
...
cmd_result_t
cmd_dpp_diag_ocb_static(int unit, args_t *a) {
    ...
    cli_out("\tFBC_MNMC size = %d\n", dbuffs->fbc_mnmc.size);
    cli_out("\tFBC_FMC size = %d\n", dbuffs->fbc_fmc.size);
    cli_out("\n");
    ...
    if (soc_reg_above_64_get(unit, IDR_FBC_EXTERNAL_CONFIGURATIONr, REG_PORT_ANY, 0, reg_above64_val) < 0) return CMD_FAIL;
    ...
    cli_out("\tIDR_FBC_EXTERNAL_CONFIGURATION, FULL_MULTICAST. Low=0x%x, High=0x%x (size=%d)\n", field_start, field_end, field_end - field_start);
    ...
    cli_out("\tIDR_FBC_EXTERNAL_CONFIGURATION, MINI_MULTICAST. Low=0x%x, High=0x%x (size=%d)\n", field_start, field_end, field_end - field_start);
    cli_out("\n");
...
cmd_result_t
cmd_dpp_diag_ocb_dynamic(int unit, args_t *a) {
...
/*
 * Print diag ocb usage
*/
void cmd_dpp_diag_ocb_usage(int unit) {
    char cmd_dpp_diag_ocb_usage[] =
       "Usage (DIAG ocb):"
       "\n\tDIAGnotsics OCB commands\n\t"
       "Usages:\n\t"
       "DIAG oam <OPTION>"
#ifdef __PEDANTIC__
    ...
    cli_out(cmd_dpp_diag_ocb_usage);
}
...
*/
cmd_result_t
cmd_dpp_diag_ocb(int unit, args_t *a) {
    char      *function;

    if (!SOC_IS_JERICHO(unit)) {
        cli_out("Error: OCB Diag is not supported for this unit=%d\n", unit);
        return CMD_USAGE;
    }
    function = ARG_GET(a); 
    if ((!function) || (!sal_strncasecmp(function, "static", strlen(function)))) {
#ifdef BCM_JERICHO_SUPPORT
        return cmd_dpp_diag_ocb_static(unit, a);
#endif /* BCM_JERICHO_SUPPORT */
    } else if (!sal_strncasecmp(function, "dynamic", strlen(function))) {
        return cmd_dpp_diag_ocb_dynamic(unit, a);
    } else {
        return CMD_USAGE;
    }
}
