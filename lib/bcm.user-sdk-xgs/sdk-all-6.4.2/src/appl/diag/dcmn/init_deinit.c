...
"  ModID=<value>          Module id for attach. (default=0). BaseModID should be added when BaseModID is not 0.\n\n"
"  BaseModID=<value>      Base module id. (default=0). Should be added to ModID.\n\n"
...
"                           Test sequence: deinit, sample memory allocations, init, deinit, smaple memory allocation.\n"
...
appl_dcmn_init(int unit, appl_dcmn_init_param_t* init_param) {
    ...
    if(!init_param->no_attach) {
        ...
        LOG_INFO(BSL_LS_BCM_INIT,
                 (BSL_META_U(unit,
                             "%d: Attach unit.\n"), unit));
        rv = soc_cm_device_create(dev->device, dev->rev, NULL);
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_APPL_SHELL,
                      (BSL_META_U(unit,
                                  "soc_cm_device_create () Failed:\n")));
            return rv;                
        }
    ...
    if (SOC_IS_DPP(unit) && SOC_MAC_LIMIT_PER_TUNNEL_ENABLE_GET(unit)) {
        if (SOC_IS_ARAD_B1_AND_BELOW(unit)) {
		    sh_process_command(unit, "mcsload 0 BCM88650_A0_0.srec ResetUC=true InitMCS=true StartUC=true StartMSG=true;");
        } else if (SOC_IS_ARADPLUS_A0(unit)) {
            sh_process_command(unit, "mcsload 0 BCM88660_A0_0.srec ResetUC=true InitMCS=true StartUC=true StartMSG=true;");
        }
    }
#endif    
    ...
    if( (!init_param->no_rx_los) && ((SOC_IS_ARAD(unit) && !SOC_IS_ARDON(unit)) || SOC_IS_FE1600(unit)) ) {
...
appl_dcmn_init_usage_parse(int unit, args_t *a, appl_dcmn_init_param_t *init_param)
{
    ...
    parse_table_add(&pt, "NofDevices", PQ_INT, (void *)0,
