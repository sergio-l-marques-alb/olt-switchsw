...
typedef struct xd_s {                   /* TX/RX description */
    ...
    int         visibility_pkt;         /* indication of internal pkt trace */
#endif
} xd_t;
...
#if defined(BCM_TOMAHAWK_SUPPORT)
    if (soc_feature(u, soc_feature_visibility)) {
        parse_table_add(&pt, "PacketTraceSrcPort",      PQ_DFL|PQ_INT, 0,
                        &xd->pkt_info.pkt_trace_src_port,      NULL);

        parse_table_add(&pt, "PacketTrace",    PQ_DFL|PQ_INT, 0,
                        &xd->visibility_pkt, NULL);
    }
...
#endif
#if defined(BCM_TOMAHAWK_SUPPORT)
    if(xd->visibility_pkt) { 
        pkt_info->flags2 |= BCM_PKT_F2_VISIBILITY_PKT;
    } else {
        pkt_info->flags2 =  pkt_info->flags2 & (~BCM_PKT_F2_VISIBILITY_PKT);
    }
#endif
