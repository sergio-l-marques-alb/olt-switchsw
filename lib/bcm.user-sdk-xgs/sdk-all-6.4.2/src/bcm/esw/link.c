...
_bcm_esw_link_reload(int unit)
{
    ...
    if (BCM_SUCCESS(rv)) {
        ...
        BCM_PBMP_AND(SOC_CONTROL(unit)->link_mask2, sop->link_fwd);
...
_bcm_esw_link_fault_get(int unit, int port, int *fault)
{
    ...
    if (SOC_IS_TOMAHAWK(unit)) {
        /* this entire piece of code should live on top of MAC driver */
        return BCM_E_NONE;
    }
...
_bcm_esw_linkscan_update_port(int unit, uint32 flags, int port)
{
    ...
    if (soc_feature(unit, soc_feature_port_lag_failover)) {
        if (cur_failed) {
            ...
            new_failed = soc_reg64_field32_get(unit, lag_failover_status_reg,
                                               lag_fail_status,
    ...
    */
    ...
    if (cur_link != new_link) {
        if (new_link) {
            ...
            LOG_INFO(BSL_LS_BCM_LINK,
                     (BSL_META_U(unit,
                                 "Link up processing took %d usecs\n"),
