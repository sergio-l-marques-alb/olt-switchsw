                               ...
                               uint32 *entry, void *ma_rmep)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_OAM,
              (BSL_META_U(unit,
                          "OAM(unit %d) Info: _bcm_kt2_oam_read_clear_faults index=%d "
                           "Table=%d.\n"), unit, index, mem));
                           ...
                           bcm_oam_endpoint_t ep_id)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_OAM,
              (BSL_META_U(unit,
                          "OAM(unit %d) Info: _bcm_kt2_oam_group_ep_list_add" 
                           " (GID=%d) (EP=%d).\n"), unit, group_id, ep_id));
    return (BCM_E_NONE);
}
    ...
    if (BCM_SUCCESS
        ...
        if (hash_data->tx_ctr == _BCM_OAM_INVALID_INDEX) {
            ...
            if (BCM_FAILURE(rv)) {
                LOG_ERROR(BSL_LS_BCM_OAM,
                          (BSL_META_U(unit,
                                      "OAM(unit %d) Error: L3_ENTRY table update failed "
        ...
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                                  "OAM(unit %d) Error: L3_ENTRY table write failed "
                                  "for EP=%d %s.\n"),
                       unit, ep_info_p->id, bcm_errmsg(rv)));
            return (rv);
        }
    ...
    if (BCM_SUCCESS(_bcm_kt2_oam_find_egr_lmep(unit, hash_data,
        ...
        if (hash_data->tx_ctr == _BCM_OAM_INVALID_INDEX) {
            ...
            if (BCM_FAILURE(rv)) {
                LOG_ERROR(BSL_LS_BCM_OAM,
                          (BSL_META_U(unit,
                                      "OAM(unit %d) Error: EGR_MP_GROUP table update "
                          ...
                          bcm_oam_endpoint_info_t *ep_info_p)
{
    ...
    if (BCM_SUCCESS
        ...
        if (BCM_FAILURE(rv)) {
            ...
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                                  "OAM(unit %d) Error: L3_ENTRY table update failed for "
                                  "EP=%d" "  %s.\n"), unit, ep_p->id, bcm_errmsg(rv)));
            return (rv);
        }
    } else {
        ...
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
                                  "OAM(unit %d) Error: L3_ENTRY table insert failed for "
                                  "EP=%d" "  %s.\n"), unit, ep_p->id, bcm_errmsg(rv)));
            L3_UNLOCK(unit);
            return (rv);
        }
                          ...
                          l3_entry_1_entry_t *l3_entry)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_OAM,
              (BSL_META_U(unit,
                          "OAM(unit %d) Info: _bcm_kt2_oam_rmep_recover.\n"),
               unit));
    /*
     * Get OAM control structure.
   ...
   /* Has to be a valid RMEP, return error otherwise. */
    ...
    LOG_DEBUG(BSL_LS_BCM_OAM,
              (BSL_META_U(unit,
                          "OAM(unit %d) Info: _bcm_kt2_oam_rmep_recover - done.\n"),
               unit));
...
cleanup:
    ...
    LOG_DEBUG(BSL_LS_BCM_OAM,
              (BSL_META_U(unit,
                          "OAM(unit %d) Info: _bcm_kt2_oam_rmep_recover - error_done.\n"),
               unit));
    return (rv);
}
                         ...
                         port_tab_entry_t *port_entry)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_OAM,
              (BSL_META_U(unit,
                          "OAM(unit %d) Info: _bcm_kt2_oam_port_lmep_rx_config_recover .\n"),
               unit));
    /*
     * Get OAM control structure.
     *     Note: Lock taken by calling routine.
    ...
    LOG_DEBUG(BSL_LS_BCM_OAM,
              (BSL_META_U(unit,
                          "OAM(unit %d) Info: _bcm_kt2_oam_lmep_rx_config_recover"
                           " - done.\n"), unit));
    return (BCM_E_NONE);
                         ...
                         egr_mp_group_entry_t *egr_mp_grp_entry)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_OAM,
              (BSL_META_U(unit,
                          "OAM(unit %d) Info: _bcm_kt2_oam_lmep_upmep_rx_config_recover .\n"),
               unit));
    /*
     * Get OAM control structure.
     *     Note: Lock taken by calling routine.
...
_bcm_kt2_oam_lmep_tx_config_recover(int unit)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_OAM,
              (BSL_META_U(unit,
                          "OAM(unit %d) Info: _bcm_kt2_oam_lmep_tx_config_recover.\n"),
               unit));
    ...
    if (!SOC_WARM_BOOT_SCACHE_IS_LIMITED(unit) && (stable_size > 0)) {
        if (recovered_ver < BCM_WB_VERSION_1_1) {
            ...
            if (SOC_FAILURE(rv)) {
                LOG_ERROR(BSL_LS_BCM_OAM,
                        (BSL_META_U(unit,
                                    "OAM(unit %d) Error: scache alloc failed  - %s.\n"),
                         unit, bcm_errmsg(rv)));
                goto cleanup;
            }
        }
                      ...
                      bcm_oam_group_info_t *group_info)
{
    ...
    if (BCM_FAILURE(rv)) {
        ...
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "OAM(unit %d) Error: bcm_kt2_oam_group_get Group ID=%d "
                               "- Failed.\n"), unit, group));
        return (rv);
    }
                          ...
                          bcm_oam_group_t group)
{
    ...
    if (BCM_FAILURE(rv)) {
        ...
        LOG_ERROR(BSL_LS_BCM_OAM,
                  (BSL_META_U(unit,
                              "OAM(unit %d) Error: bcm_kt2_oam_endpoint_destroy_all"
                               " (GID=%d) - %s.\n"), unit, group, bcm_errmsg(rv)));
        return (rv);
    }
    ...
    for (grp_idx = 0; grp_idx < oc->group_count; grp_idx++) {
        ...
        if (BCM_E_EXISTS
            ...
            if (BCM_FAILURE(rv)) {
                ...
                LOG_ERROR(BSL_LS_BCM_OAM,
                          (BSL_META_U(unit,
                                      "OAM(unit %d) Error: _bcm_kt2_oam_get_group "
                                       "(GID=%d) - %s.\n"), unit, grp_idx, bcm_errmsg(rv)));
                return (rv);
            }
...
bcm_kt2_oam_endpoint_create(int unit, bcm_oam_endpoint_info_t *endpoint_info)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_OAM,
              (BSL_META_U(unit,
                          "OAM(unit %d) Info: bcm_kt2_oam_endpoint_create "
                                ...
                                _bcm_oam_hash_key_t  *hash_key)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_OAM,
              (BSL_META_U(unit,
                          "BHH(unit %d) Info: bcm_kt2_oam_bhh_endpoint_create"
                           "Endpoint ID=%d.\n"), unit, endpoint_info->id));
