/*
 * $Id: vswitch.c,v 1.46 Broadcom SDK $
...
#define VSWITCH_LOCK_RELEASE \
    if (sal_mutex_give(_vswitch_state[unit]->lock)) { \
        ...
        LOG_ERROR(BSL_LS_BCM_VLAN,     \
                  (BSL_META_U(unit,                            \
                              "unable to release vswitch lock" \
                              " on unit %d\n"),                \
                   unit));                                     \
        return BCM_E_INTERNAL; \
    }
 ...
 */
int 
_bcm_fe2000_is_vswitch_inuse(int unit,
                             bcm_vlan_t vsi) 
{
