...
_bcm_xgs3_tx_pipe_bypass_header_setup(int unit, bcm_pkt_t *pkt)
{
    ...
    if (soc_feature(unit, soc_feature_visibility)) {
        if ((port == -1) && (pkt->flags2 & BCM_PKT_F2_VISIBILITY_PKT)) {
            /* Set the masquerad source port */
            port = pkt->rx_port;
            _bcm_esw_pkt_trace_src_port_set(unit, port);
        } else if (pkt->flags2 & BCM_PKT_F2_VISIBILITY_PKT) {
            if (pkt->rx_port != 0) {
                port = pkt->rx_port;
            } 
            _bcm_esw_pkt_trace_src_port_set(unit, port);
...
if ((soc_feature(unit, soc_feature_visibility)) && 
    (pkt->flags2 & BCM_PKT_F2_VISIBILITY_PKT)) {
        ...
        soc_loopback_hdr_field_set(unit, &loopback_hdr, LBMH_pkt_profile,VISIBILITY_PKT_PROFILE_NUM);
