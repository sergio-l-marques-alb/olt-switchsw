...
#define _BCM_DPP_AM_EGRESS_GLOBAL_LIF_BANK_ID_TO_RES_ID(_unit, _bank_id) (_BCM_DPP_AM_GLOBAL_LIF_EGRESS_RES_ID_START(_unit) + _bank_id)
#define _BCM_DPP_AM_INGRESS_GLOBAL_LIF_BANK_ID_TO_RES_ID(_unit, _bank_id) (_BCM_DPP_AM_GLOBAL_LIF_INGRESS_RES_ID_START(_unit) + _bank_id)
...
STATIC uint32
_bcm_dpp_am_global_lif_bank_type_get(int unit, int bank_id, uint8 is_ingress, bcm_dpp_am_global_lif_bank_type_t *new_bank_type);
...
STATIC uint32
_bcm_dpp_am_global_lif_get_ingress_count(int unit, int *count);

STATIC uint32
_bcm_dpp_am_global_lif_update_egress_count(int unit, int diff);

STATIC uint32
_bcm_dpp_am_global_lif_get_egress_count(int unit, int *count);

STATIC uint32
_bcm_dpp_am_global_lif_update_count(int unit, int engine_id, int diff);

STATIC uint32
_bcm_dpp_am_global_lif_get_count(int unit, int engine_id, int *count);
...
STATIC uint32 
_bcm_dpp_am_global_lif_bank_is_dynamically_legal(int unit, int bank_id, uint8 is_ingress, uint8 *is_legal);
...
#define _BCM_DPP_AM_EGRESS_ENCAP_BANK_ID_TO_POOL_ID(bank_id) (dpp_res_arad_pool_egress_encap_bank_start + bank_id)

#define _BCM_DPP_AM_INGRESS_LIF_OBJECT_ID_TO_BANK_ID(object_id) (object_id / _BCM_DPP_AM_INGRESS_LIF_NOF_ENTRIES_PER_BANK)
#define _BCM_DPP_AM_INGRESS_LIF_BANK_ID_TO_POOL_ID(bank_id) (dpp_res_arad_pool_ingress_lif_bank_start + bank_id)
...
#ifdef ALLOC_MNGR_DEALLOC_BANK
STATIC uint32
_bcm_dpp_am_sync_lif_unset_bank(int unit, int bank_id)
{
    bcm_dpp_am_sync_lif_t *sync_lif_i;
    uint32 type;
    ...
    /* Valid */
    SHR_BITCLR(sync_lif_i->sync_lif_banks,bank_id * _BCM_DPP_AM_SYNC_LIF_NOF_BITS_PER_BANK);
...
/* if we do memory optimization pool werent set at init, set them now */
#if (BCM_ARAD_OPTIMIZE_MEMORY_ALLOCATION)
    /* Allocate bank in A.M. */
    rv = dpp_am_res_set_pool(unit,
                          BCM_DPP_AM_DEFAULT_CORE_ID,
                          _BCM_DPP_AM_INGRESS_LIF_BANK_ID_TO_POOL_ID(new_bank),
                          SHR_RES_ALLOCATOR_BITMAP,
                          _BCM_DPP_AM_INGRESS_LIF_BANK_LOW_ID(new_bank),
                          _BCM_DPP_AM_INGRESS_LIF_NOF_ENTRIES_PER_BANK,
                          NULL,
                          "Ingress LIF bank allocation");
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    *bank_id = new_bank;
    ...
    if (new_bank == 0 && (SOC_DPP_CONFIG(unit)->pp.global_lif_index_simple != 0 && SOC_DPP_CONFIG(unit)->pp.global_lif_index_drop != 0)) {
        int object = 0;        
        rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID,_BCM_DPP_AM_INGRESS_LIF_BANK_ID_TO_POOL_ID(new_bank),SHR_RES_ALLOC_WITH_ID,1,&object);
        BCMDNX_IF_ERR_EXIT(rv);
    }
    BCM_EXIT;
exit:
...
_bcm_dpp_am_ingress_lif_internal_alloc(int unit, bcm_dpp_am_ingress_lif_alloc_info_t* allocation_info, uint8 is_sync, uint32 flags, int count, int* object)
{
    ...
    if (flags & (SHR_RES_ALLOC_WITH_ID)) {
        ...
        rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID,_BCM_DPP_AM_INGRESS_LIF_BANK_ID_TO_POOL_ID(bank_id),flags,count,object);
        BCMDNX_IF_ERR_EXIT(rv);
    } else {
        uint8 alloc_success;
        ...
        for (bank_id = 0; bank_id < nof_valid_banks; ++bank_id) {
            rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID,_BCM_DPP_AM_INGRESS_LIF_BANK_ID_TO_POOL_ID(banks_ids[bank_id]),flags,count,object);
            if (rv == BCM_E_NONE) {
                alloc_success = TRUE;
        ...
        if (!alloc_success) {
            ...
            if (is_success) {
                ...
                /* Allocate object */
                rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID,_BCM_DPP_AM_INGRESS_LIF_BANK_ID_TO_POOL_ID(new_bank),flags,count,object);
                BCMDNX_IF_ERR_EXIT(rv);
                alloc_success = TRUE;
            } else {
...
_bcm_dpp_am_ingress_lif_internal_alloc_align(int unit, bcm_dpp_am_ingress_lif_alloc_info_t* allocation_info, uint8 is_sync, uint32 flags, int align, int offset, int count, int* object)
{
    ...
    if (flags & (SHR_RES_ALLOC_WITH_ID)) {
        ...
        if(flags & BCM_DPP_AM_FLAG_ALLOC_REPLACE) {
            rv = dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID, _BCM_DPP_AM_INGRESS_LIF_BANK_ID_TO_POOL_ID(bank_id), offset, count);
            if(rv == BCM_E_EXISTS) {
                rv = BCM_E_NONE;
                BCM_EXIT;
            }
        }
        rv = dpp_am_res_alloc_align(unit, BCM_DPP_AM_DEFAULT_CORE_ID,_BCM_DPP_AM_INGRESS_LIF_BANK_ID_TO_POOL_ID(bank_id),flags,align,offset,count,object);
        BCMDNX_IF_ERR_EXIT(rv);
        ...
        alloc_success = FALSE;
        for (bank_id = 0; bank_id < nof_valid_banks; ++bank_id) {
            rv = dpp_am_res_alloc_align(unit, BCM_DPP_AM_DEFAULT_CORE_ID,_BCM_DPP_AM_INGRESS_LIF_BANK_ID_TO_POOL_ID(banks_ids[bank_id]),flags,align,offset,count,object);
            if (rv == BCM_E_NONE) {
                alloc_success = TRUE;
        ...
        if (!alloc_success) {
            ...
            if (is_success) {
                ...
                /* Allocate object */
                rv = dpp_am_res_alloc_align(unit, BCM_DPP_AM_DEFAULT_CORE_ID,_BCM_DPP_AM_INGRESS_LIF_BANK_ID_TO_POOL_ID(new_bank),flags,align,offset,count,object);
                BCMDNX_IF_ERR_EXIT(rv);
...
_bcm_dpp_am_ingress_lif_internal_dealloc(int unit, bcm_dpp_am_ingress_lif_alloc_info_t* allocation_info, uint8 is_sync, int count, int object)
{
    ...
    rv = dpp_am_res_free_and_status(unit, BCM_DPP_AM_DEFAULT_CORE_ID,_BCM_DPP_AM_INGRESS_LIF_BANK_ID_TO_POOL_ID(bank_id),count,object,&flags);
    BCMDNX_IF_ERR_EXIT(rv);
    
    if (flags & (SHR_RES_FREED_TYPE_LAST_ELEM | SHR_RES_FREED_POOL_LAST_ELEM)) {
#ifdef ALLOC_MNGR_DEALLOC_BANK
        ingress_lif_i = &INGRESS_LIF_CNTL(unit);
        /* remove bank in case all not in used */
        /* Set SW DB bank information */
        /* Valid */
        SHR_BITCLR(ingress_lif_i->ingress_lif_banks,bank_id * _BCM_DPP_AM_INGRESS_LIF_NOF_BITS_PER_BANK);

        req_phase_numbers = 0;
        /* Required Phase number */
        SHR_BITCOPY_RANGE(ingress_lif_i->ingress_lif_banks, (bank_id*_BCM_DPP_AM_INGRESS_LIF_NOF_BITS_PER_BANK+_BCM_DPP_AM_INGRESS_LIF_PHASE_BITS_MIN), &req_phase_numbers, 0, (_BCM_DPP_AM_INGRESS_LIF_PHASE_NOF_BITS));    
    
        /* Count */
        ingress_lif_i->ingress_lif_count--;    
        
        /* Free bank in A.M. */
        rv = dpp_am_res_unset_pool(unit, BCM_DPP_AM_DEFAULT_CORE_ID,
                              _BCM_DPP_AM_INGRESS_LIF_BANK_ID_TO_POOL_ID(bank_id));
        BCMDNX_IF_ERR_EXIT(rv);
#endif
...
_bcm_dpp_am_ingress_lif_is_allocated(int unit, bcm_dpp_am_ingress_lif_alloc_info_t* allocation_info, int count, int object)
{
    ...
    BCM_RETURN_VAL_EXIT(dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID,_BCM_DPP_AM_INGRESS_LIF_BANK_ID_TO_POOL_ID(bank_id),count,object));
...
#define _BCM_DPP_AM_EGRESS_ENCAP_NOF_BITS (SOC_PPD_EG_ENCAP_NOF_BANKS*_BCM_DPP_AM_EGRESS_ENCAP_NOF_BITS_PER_BANK)
...
_bcm_dpp_am_egress_encap_allocate_new_bank(int unit, uint8 is_sync, uint32 flags, SOC_PPD_EG_ENCAP_ACCESS_PHASE phase_access, int *bank_id, uint8 *is_success)
{
    ...
    *is_success = TRUE;
    ...
    if (flags & SHR_RES_ALLOC_WITH_ID) {
        ...
        for (tmp_bank_id = 0; tmp_bank_id < SOC_PPD_EG_ENCAP_NOF_BANKS; ++tmp_bank_id) {
...
/* if we do memory optimization then pool werent set at init, set them now */
#if (BCM_ARAD_OPTIMIZE_MEMORY_ALLOCATION)
    /* Allocate bank in A.M. */
    rv = dpp_am_res_set_pool(unit,
                          BCM_DPP_AM_DEFAULT_CORE_ID,
                          _BCM_DPP_AM_EGRESS_ENCAP_BANK_ID_TO_POOL_ID(new_bank),
                          SHR_RES_ALLOCATOR_BITMAP,
                          _BCM_DPP_AM_EGRESS_ENCAP_BANK_LOW_ID(new_bank),
                          _BCM_DPP_AM_EGRESS_ENCAP_NOF_ENTRIES_PER_BANK,
                          NULL,
                          "Egress encapsulation bank allocation");
    BCMDNX_IF_ERR_EXIT(rv);
#endif /*(BCM_ARAD_OPTIMIZE_MEMORY_ALLOCATION)*/
    ...
    *bank_id = new_bank;
    ...
    if (new_bank == 0) {
        ...
        int object = 0;        
        rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID,_BCM_DPP_AM_EGRESS_ENCAP_BANK_ID_TO_POOL_ID(new_bank),SHR_RES_ALLOC_WITH_ID,1,&object);
        BCMDNX_IF_ERR_EXIT(rv);
...
{    
    ...
    case dpp_am_res_eep_ip_tnl: /* IP tunnel */
      *phase_number = (allocation_info->application_type == bcm_dpp_am_egress_encap_app_ip_tunnel_roo) ? SOC_PPD_EG_ENCAP_ACCESS_PHASE_ONE:SOC_PPD_EG_ENCAP_ACCESS_PHASE_TWO;
      break;
...
_bcm_dpp_am_egress_encap_banks_by_access_phase(int unit, uint8 is_sync, SOC_PPD_EG_ENCAP_ACCESS_PHASE phase_access, int* nof_valid_banks, int* banks_ids)
{
    ...
    *nof_valid_banks = 0;
    for (bank_id = 0; bank_id < SOC_PPD_EG_ENCAP_NOF_BANKS; ++bank_id) {
        rv = _bcm_dpp_am_egress_encap_is_exist_bank(unit, bank_id, &is_exist);
        BCMDNX_IF_ERR_EXIT(rv);
...
_bcm_dpp_am_egress_encap_internal_alloc(int unit, bcm_dpp_am_egress_encap_alloc_info_t* allocation_info, uint8 is_sync, uint32 flags, int count, int* object)
{
    ...
    if (flags & (SHR_RES_ALLOC_WITH_ID)) {
        ...
        rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID,_BCM_DPP_AM_EGRESS_ENCAP_BANK_ID_TO_POOL_ID(bank_id),flags,count,object);
        BCMDNX_IF_ERR_EXIT(rv);
        ...
        for (bank_id = 0; bank_id < nof_valid_banks; ++bank_id) {
            rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID,_BCM_DPP_AM_EGRESS_ENCAP_BANK_ID_TO_POOL_ID(banks_ids[bank_id]),flags,count,object);
            if (rv == BCM_E_NONE) {
                alloc_success = TRUE;
                /* Allocation succeded no need to find other banks */
        ...
        if (!alloc_success) {
            ...
            if (is_success) {
                ...
                /* Allocate object */
                rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID,_BCM_DPP_AM_EGRESS_ENCAP_BANK_ID_TO_POOL_ID(new_bank),flags,count,object);
                BCMDNX_IF_ERR_EXIT(rv);
...
_bcm_dpp_am_egress_encap_internal_alloc_align(int unit, bcm_dpp_am_egress_encap_alloc_info_t* allocation_info, uint8 is_sync, uint32 flags, int align, int offset, int count, int* object)
{
    ...
    if (flags & (SHR_RES_ALLOC_WITH_ID)) {
        ...
        /* if replace flag is set and exist then we are done */
        if(flags & BCM_DPP_AM_FLAG_ALLOC_REPLACE) {
            rv = dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID, _BCM_DPP_AM_EGRESS_ENCAP_BANK_ID_TO_POOL_ID(bank_id), offset, count);
            if(rv == BCM_E_EXISTS) {
                rv = BCM_E_NONE;
                BCM_EXIT;
            }
        }
        rv = dpp_am_res_alloc_align(unit, BCM_DPP_AM_DEFAULT_CORE_ID,_BCM_DPP_AM_EGRESS_ENCAP_BANK_ID_TO_POOL_ID(bank_id),flags,align,offset,count,object);
        BCMDNX_IF_ERR_EXIT(rv);
        ...
        alloc_success = FALSE;
        for (bank_id = 0; bank_id < nof_valid_banks; ++bank_id) {
            rv = dpp_am_res_alloc_align(unit, BCM_DPP_AM_DEFAULT_CORE_ID,_BCM_DPP_AM_EGRESS_ENCAP_BANK_ID_TO_POOL_ID(banks_ids[bank_id]),flags,align,offset,count,object);
            if (rv == BCM_E_NONE) {
                alloc_success = TRUE;
        ...
        if (!alloc_success) {
            ...
            if (is_success) {
                ...
                /* Allocate object */
                rv = dpp_am_res_alloc_align(unit, BCM_DPP_AM_DEFAULT_CORE_ID,_BCM_DPP_AM_EGRESS_ENCAP_BANK_ID_TO_POOL_ID(new_bank),flags,align,offset,count,object);
                BCMDNX_IF_ERR_EXIT(rv);
...
_bcm_dpp_am_egress_encap_internal_dealloc(int unit, bcm_dpp_am_egress_encap_alloc_info_t* allocation_info, uint8 is_sync, int count, int object)
{
    ...
    rv = dpp_am_res_free_and_status(unit, BCM_DPP_AM_DEFAULT_CORE_ID,_BCM_DPP_AM_EGRESS_ENCAP_BANK_ID_TO_POOL_ID(bank_id),count,object, &flags);
    BCMDNX_IF_ERR_EXIT(rv);

    if (flags & (SHR_RES_FREED_TYPE_LAST_ELEM | SHR_RES_FREED_POOL_LAST_ELEM)) {
#ifdef ALLOC_MNGR_DEALLOC_BANK
        encap_i = &EGRESS_ENCAP_CNTL(unit);
        /* last entry remove bank settings */
        /* Set SW DB bank information */
        /* Valid */
        SHR_BITCLR(encap_i->egress_encap_banks,bank_id * _BCM_DPP_AM_EGRESS_ENCAP_NOF_BITS_PER_BANK);

        /* Phase number */
        phase_number = 0;
        SHR_BITCOPY_RANGE(encap_i->egress_encap_banks, (bank_id*_BCM_DPP_AM_EGRESS_ENCAP_NOF_BITS_PER_BANK+_BCM_DPP_AM_EGRESS_ENCAP_PHASE_BITS_MIN), &phase_number, 0, (_BCM_DPP_AM_EGRESS_ENCAP_PHASE_NOF_BITS));    
        ...
        /* Free bank in A.M. */
        rv = dpp_am_res_unset_pool(unit, BCM_DPP_AM_DEFAULT_CORE_ID,
                              _BCM_DPP_AM_EGRESS_ENCAP_BANK_ID_TO_POOL_ID(bank_id));
        BCMDNX_IF_ERR_EXIT(rv);
#endif
...
_bcm_dpp_am_egress_encap_is_allocated(int unit, bcm_dpp_am_egress_encap_alloc_info_t* allocation_info, int count, int object)
{
    ...
    BCM_RETURN_VAL_EXIT(dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID,_BCM_DPP_AM_EGRESS_ENCAP_BANK_ID_TO_POOL_ID(bank_id),count,object));
...
_bcm_dpp_am_sync_lif_dealloc(int unit, bcm_dpp_am_sync_lif_alloc_info_t* allocation_info, int count, int object)
{
...
#ifdef ALLOC_MNGR_DEALLOC_BANK
    /* 
     * In case Bank is invalid for both sides then remove also in Sync LIF 
     */
    is_exist = 0;
    rv = _bcm_dpp_am_ingress_lif_is_exist_bank(unit, bank_id, &is_exist);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    if (!is_exist) {
        ...
        if (!is_exist) {
            /* Remove also in Sync */
            rv = _bcm_dpp_am_sync_lif_unset_bank(unit, bank_id);
            BCMDNX_IF_ERR_EXIT(rv);
        }
    }
#endif
...
_bcm_dpp_am_global_lif_init(int unit){
    ...
    for (bank_id_iter = 0 ; bank_id_iter < _BCM_DPP_AM_MAX_NOF_GLOBAL_LIF_BANKS ; bank_id_iter++) {

        /* 
         *  Set egress bank.
         */

        if (bank_id_iter < _BCM_DPP_AM_GLOBAL_LIF_EGRESS_LEGAL_BANKS_START(_unit)) {
    ...
    count = 0;
    rv = SOC_DPP_WB_ENGINE_SET_VAR(unit, SOC_DPP_WB_ENGINE_VAR_ALLOC_MNGR_MULTI_CORE_GLOBAL_LIF_EGRESS_COUNT, &count);
    BCMDNX_IF_ERR_EXIT(rv);
    
    rv = SOC_DPP_WB_ENGINE_SET_VAR(unit, SOC_DPP_WB_ENGINE_VAR_ALLOC_MNGR_MULTI_CORE_GLOBAL_LIF_INGRESS_COUNT, &count);
    BCMDNX_IF_ERR_EXIT(rv);
...
_bcm_dpp_am_global_lif_internal_dealloc(int unit, uint32 sync_flags, int global_lif_id){
    ...
    if (sync_flags & _BCM_DPP_AM_SYNC_LIF_EGRESS) {
        rv = dpp_am_res_free_and_status(unit, BCM_DPP_AM_DEFAULT_CORE_ID, _BCM_DPP_AM_EGRESS_GLOBAL_LIF_BANK_ID_TO_RES_ID(unit, bank_id), 1, global_lif_id, &flags);
        BCMDNX_IF_ERR_EXIT(rv);
    ...
    if (sync_flags & _BCM_DPP_AM_SYNC_LIF_INGRESS) {
        rv = dpp_am_res_free_and_status(unit, BCM_DPP_AM_DEFAULT_CORE_ID, _BCM_DPP_AM_INGRESS_GLOBAL_LIF_BANK_ID_TO_RES_ID(unit, bank_id), 1, global_lif_id, &flags);
        BCMDNX_IF_ERR_EXIT(rv);
    ...
    /* Update bank type if necessary. */
    if (clear_bank) {
        rv = _bcm_dpp_am_global_lif_clear_bank_and_type(unit, _BCM_DPP_AM_SYNC_LIF_INGRESS, bank_id);
        BCMDNX_IF_ERR_EXIT(rv);
    }    
...
_bcm_dpp_am_global_lif_internal_is_alloc(int unit, uint32 sync_flags, int global_lif_id){
    ...
    /* Find matched bank */
    res_id = (sync_flags & _BCM_DPP_AM_SYNC_LIF_EGRESS) ?   _BCM_DPP_AM_EGRESS_GLOBAL_LIF_BANK_ID_TO_RES_ID(unit, bank_id)
                                                        :   _BCM_DPP_AM_INGRESS_GLOBAL_LIF_BANK_ID_TO_RES_ID(unit, bank_id);

    rv = dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID, res_id, 1, global_lif_id);
    BCM_RETURN_VAL_EXIT(rv);
...
_bcm_dpp_am_global_lif_bank_is_sync_bank(int unit, int bank_id, uint8 *is_sync){
    ...
    rv = _bcm_dpp_am_global_lif_bank_type_get(unit, bank_id, TRUE, &bank_type);
    BCMDNX_IF_ERR_EXIT(rv);
...
/*
 * Function:
 *     _bcm_dpp_am_global_lif_bank_is_dynamically_legal
 * Purpose:
...
_bcm_dpp_am_global_lif_is_legal_bank_by_type(int unit, int bank_id, uint8 is_ingress, uint8 is_sync, uint32 bank_alloc_flags, uint8 *is_legal) {
    ...
    /* Get bank type. */
    rv = _bcm_dpp_am_global_lif_bank_type_get(unit, bank_id, is_ingress, &bank_type);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    case bcm_dpp_am_global_lif_bank_type_none:
        if (allocated_bank_only) {
            ...
            *is_legal = FALSE;
            ...
            rv = _bcm_dpp_am_global_lif_bank_is_dynamically_legal(unit, bank_id, is_ingress, is_legal);
            BCMDNX_IF_ERR_EXIT(rv);
        }
        break;
    default:
...
_bcm_dpp_am_global_lif_allocate_new_bank(int unit, uint32 sync_flags, int *bank_id, uint8 *is_success) {
    ...
    *is_success = FALSE;
    ...
    for (bank_id_iter = _BCM_DPP_AM_MAX_NOF_GLOBAL_LIF_BANKS - 1 ; bank_id_iter >= 0 ; bank_id_iter--) {
        if (is_ingress) {
            rv = _bcm_dpp_am_global_lif_bank_type_get(unit, bank_id_iter, TRUE, &ingress_bank_type);
            BCMDNX_IF_ERR_EXIT(rv);
        }

        if (is_egress) {
            rv = _bcm_dpp_am_global_lif_bank_type_get(unit, bank_id_iter, FALSE, &egress_bank_type);
            BCMDNX_IF_ERR_EXIT(rv);
        }
        ...
        if (ingress_bank_type == bcm_dpp_am_global_lif_bank_type_none
            ...
            rv = _bcm_dpp_am_global_lif_bank_is_dynamically_legal(unit, bank_id_iter, FALSE, &is_egress_legal);
            BCMDNX_IF_ERR_EXIT(rv);

            rv = _bcm_dpp_am_global_lif_bank_is_dynamically_legal(unit, bank_id_iter, TRUE, &is_ingress_legal);
            BCMDNX_IF_ERR_EXIT(rv);
...
/*
 * Function:
 *     _bcm_dpp_am_global_lif_bank_type_get
 * Purpose:
 *      Get ingress/ egress bank type from the SW DB.
 *  
 * Parameters:
 *     unit             - (IN) Device number
 *     bank_id          - (IN) The bank id to get.
 *     is_ingress       - (IN) Wether to get the ingress bank (FALSE means get the egress bank).
 *     bank_type        - (OUT)Will be filled with the bank's type.
 * Returns: 
 *     BCM_E_NONE       - If the allocation was succesful.
 *     BCM_E_*          - Otherwise.
 */
STATIC uint32
_bcm_dpp_am_global_lif_bank_type_get(int unit, int bank_id, uint8 is_ingress, bcm_dpp_am_global_lif_bank_type_t *bank_type){
    uint32 rv;
    int engine_id;
    BCMDNX_INIT_FUNC_DEFS;

    engine_id = (is_ingress) ? SOC_DPP_WB_ENGINE_VAR_ALLOC_MNGR_MULTI_CORE_GLOBAL_LIF_INGRESS_BANK_TYPES
                             : SOC_DPP_WB_ENGINE_VAR_ALLOC_MNGR_MULTI_CORE_GLOBAL_LIF_EGRESS_BANK_TYPES;

    rv = SOC_DPP_WB_ENGINE_GET_ARR(unit, engine_id, bank_type, bank_id);
    BCMDNX_IF_ERR_EXIT(rv);
...
_bcm_dpp_am_global_lif_bank_type_set(int unit, int bank_id, uint8 is_ingress, bcm_dpp_am_global_lif_bank_type_t new_bank_type){
    ...
    rv = _bcm_dpp_am_global_lif_bank_type_get(unit, bank_id, is_ingress, &old_bank_type);
    ...
    rv = SOC_DPP_WB_ENGINE_SET_ARR(unit, engine_id, &new_bank_type, bank_id);
    BCMDNX_IF_ERR_EXIT(rv);
...
_bcm_dpp_am_global_lif_res_alloc(int unit, uint32 alloc_flags, uint32 sync_flags, int bank_id, int *global_lif){
    ...
    if (is_egress) {
        rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID, _BCM_DPP_AM_EGRESS_GLOBAL_LIF_BANK_ID_TO_RES_ID(unit, bank_id), alloc_flags, 1, global_lif);
        BCMDNX_IF_ERR_EXIT(rv);
    ...
    if (is_ingress) {
        rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID, _BCM_DPP_AM_INGRESS_GLOBAL_LIF_BANK_ID_TO_RES_ID(unit, bank_id), alloc_flags, 1, global_lif);
        ...
        if (rv != BCM_E_NONE && is_egress) {
            ...
            rc = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID, _BCM_DPP_AM_EGRESS_GLOBAL_LIF_BANK_ID_TO_RES_ID(unit, bank_id), 1, *global_lif);
...
_bcm_dpp_am_global_lif_internal_alloc_by_type(int unit, uint32 alloc_flags, uint32 sync_flags, int *global_lif, uint8 *alloc_success) {
    uint32 rv;
    int banks_ids[_BCM_DPP_AM_MAX_NOF_GLOBAL_LIF_BANKS];
    int nof_valid_banks, bank_id;
    BCMDNX_INIT_FUNC_DEFS;
...
_bcm_dpp_am_global_lif_ingress_alloc_verify(int unit){
    ...
    rv = _bcm_dpp_am_global_lif_get_ingress_count(unit, &count);
    BCMDNX_IF_ERR_EXIT(rv);
...
_bcm_dpp_am_global_lif_egress_alloc_verify(int unit){
    ...
    rv = _bcm_dpp_am_global_lif_get_egress_count(unit, &count);
    BCMDNX_IF_ERR_EXIT(rv);
...
_bcm_dpp_am_global_lif_update_ingress_count(int unit, int diff){
    ...
    rv = _bcm_dpp_am_global_lif_update_count(unit, SOC_DPP_WB_ENGINE_VAR_ALLOC_MNGR_MULTI_CORE_GLOBAL_LIF_INGRESS_COUNT, diff);
    BCMDNX_IF_ERR_EXIT(rv);
...
_bcm_dpp_am_global_lif_update_egress_count(int unit, int diff){
    ...
    rv = _bcm_dpp_am_global_lif_update_count(unit, SOC_DPP_WB_ENGINE_VAR_ALLOC_MNGR_MULTI_CORE_GLOBAL_LIF_EGRESS_COUNT, diff);
    BCMDNX_IF_ERR_EXIT(rv);
...
_bcm_dpp_am_global_lif_update_count(int unit, int engine_id, int diff) {
    ...
    /* Get old count. */
    rv = SOC_DPP_WB_ENGINE_GET_VAR(unit, engine_id, &count);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    rv = SOC_DPP_WB_ENGINE_SET_VAR(unit, engine_id, &count);
    BCMDNX_IF_ERR_EXIT(rv);
...
/*
 * Function:
 *     _bcm_dpp_am_global_lif_get_ingress_count
 *     _bcm_dpp_am_global_lif_get_egress_count
 *     _bcm_dpp_am_global_lif_get_count
 * Purpose:
 *      Returns the global ingress/egress lif counter. 
 *  
 * Parameters:
 *     unit             - (IN) Device number
 *     count            - (OUT) The number of global lifs. 
 * Returns: 
 *     BCM_E_NONE       - If the allocation was succesful.
 *     BCM_E_INTERNAL   - If the new count is less than 0.
 *     BCM_E_*          - Otherwise.
...
_bcm_dpp_am_global_lif_get_ingress_count(int unit, int *count){
    ...
    rv = _bcm_dpp_am_global_lif_get_count(unit, SOC_DPP_WB_ENGINE_VAR_ALLOC_MNGR_MULTI_CORE_GLOBAL_LIF_INGRESS_COUNT, count);
    BCMDNX_IF_ERR_EXIT(rv);
...
_bcm_dpp_am_global_lif_get_egress_count(int unit, int *count){
    ...
    rv = _bcm_dpp_am_global_lif_get_count(unit, SOC_DPP_WB_ENGINE_VAR_ALLOC_MNGR_MULTI_CORE_GLOBAL_LIF_EGRESS_COUNT, count);
    BCMDNX_IF_ERR_EXIT(rv);
