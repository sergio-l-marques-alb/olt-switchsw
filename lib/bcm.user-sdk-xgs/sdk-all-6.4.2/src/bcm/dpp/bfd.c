...
#define _BCM_BFD_IS_CLASSIFICATION_DONE_USING_YOUR_DISC(type) (type == bcmBFDTunnelTypeUdp)
 ...
 *  PFD    |--->  bPF00D0
 *  
 *  In addition the flags PF, may not be used together, so the
...
#define _BCM_BFD_FLAGS_PROFILE_INDEX_TO_FLAGS(profile_index, flags) \
	flags = (profile_index >=6)? 0 : (profile_index==0)? -1 : \
 ...
 * P, F may not be used together. 
 */
#define _BCM_BFD_FLAG_COMBINATION_IS_NOT_SUPPORTED(flags) \
	(flags & 0xd  ||  (flags &0x10 && flags &0x20 ))
...
int _bcm_bfd_mep_db_entry_struct_set(int unit, bcm_bfd_endpoint_info_t *endpoint_info, SOC_PPD_OAM_OAMP_MEP_DB_ENTRY *mep_db_entry) {
    ...
    if ((endpoint_info->type == bcmBFDTunnelTypeUdp) || (endpoint_info->type == bcmBFDTunnelTypeMpls)) {
        ...
        if (endpoint_info->flags & BCM_BFD_ENDPOINT_MULTIHOP ||  (endpoint_info->type == bcmBFDTunnelTypeMpls) ) {
            ...
            mep_db_entry->dst_ip_add = endpoint_info->dst_ip_addr * ( (endpoint_info->flags & BCM_BFD_ENDPOINT_MULTIHOP) !=0);
    ...
    if ((endpoint_info->type != bcmBFDTunnelTypeUdp)) {
        ...
        if (((endpoint_info->type == bcmBFDTunnelTypeMplsTpCc) || (endpoint_info->type == bcmBFDTunnelTypePweGal)) && SOC_IS_ARADPLUS(unit)) {
...
int _bcm_bfd_mep_db_entry_struct_get(int unit, bcm_bfd_endpoint_info_t *endpoint_info, SOC_PPD_OAM_OAMP_MEP_DB_ENTRY *mep_db_entry) {
    ...
    case SOC_PPD_OAM_MEP_TYPE_BFD_O_PWE:
        ...
        if (SOC_IS_ARADPLUS(unit) && (mep_db_entry->label == SOC_PPD_MPLS_TERM_RESERVED_LABEL_GAL)) {
...
int _bcm_bfd_classifier_mep_entry_struct_get(int unit, bcm_bfd_endpoint_info_t *endpoint_info, SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY *classifier_mep_entry, int *dip_ndx) {
    ...
    if ((!_BCM_BFD_IS_CLASSIFICATION_DONE_USING_YOUR_DISC(endpoint_info->type)) && (classifier_mep_entry->lif != _BCM_OAM_INVALID_LIF)) {
        BCMDNX_IF_ERR_EXIT(_bcm_dpp_lif_to_gport(unit, classifier_mep_entry->lif, 1, &endpoint_info->gport));
    }
    else {
...
int bcm_petra_bfd_init(int unit) {
    ...
    bcm_gport_t oamp_port, cpu_port;
    int count_erp;
...
/* In warmboot oam init is called first and inits DBs, so we don't need to init them from BFD */
...
#endif /*BCM_WARM_BOOT_SUPPORT*/    
    {
        ...
        if (!oam_is_init) {
            rv = bcm_petra_rx_trap_type_create(unit, BCM_RX_TRAP_WITH_ID, bcmRxTrapUserDefine, (int *)&_bcm_dpp_bfd_trap_info[unit].trap_ids[SOC_PPD_BFD_TRAP_ID_CPU]);
            BCMDNX_IF_ERR_EXIT(rv);

            trap_config_cpu.flags = (BCM_RX_TRAP_UPDATE_DEST | BCM_RX_TRAP_TRAP);
            trap_config_cpu.dest_port = cpu_port;
            rv = _bcm_dpp_rx_trap_set(unit, _bcm_dpp_bfd_trap_info[unit].trap_ids[SOC_PPD_BFD_TRAP_ID_CPU], &trap_config_cpu, _BCM_OAM_TRAP_FWD_OFFSET_INDEX);
            BCMDNX_IF_ERR_EXIT(rv);
            rv = _bcm_oam_bfd_common_cpu_trap_code_set(unit, _bcm_dpp_bfd_trap_info[unit].trap_ids[SOC_PPD_BFD_TRAP_ID_CPU]);
            BCMDNX_IF_ERR_EXIT(rv);
		...
		if (soc_property_get(unit, spn_BFD_ECHO_ENABLED, 0) == 1) { 
			int trap_code_tmp = SOC_PPC_TRAP_CODE_BFD_ECHO;
			rv = bcm_petra_rx_trap_type_create(unit, BCM_RX_TRAP_WITH_ID, bcmRxTrapUserDefine, &trap_code_tmp);
			BCMDNX_IF_ERR_EXIT(rv);
			/*TRAP_FWD_OFFSET_INDEX = 4*/
			rv = _bcm_dpp_rx_trap_set(unit, SOC_PPD_TRAP_CODE_BFD_ECHO, &trap_config_oamp, 4);
			BCMDNX_IF_ERR_EXIT(rv);

		} 

        _bcm_dpp_bfd_trap_info[unit].trap_ids[SOC_PPD_BFD_TRAP_ID_OAMP_IPV4] = SOC_PPD_TRAP_CODE_TRAP_BFD_O_IPV4;
        rv = bcm_petra_rx_trap_type_create(unit, 0, bcmRxTrapOamBfdIpv4, (int *)&_bcm_dpp_bfd_trap_info[unit].trap_ids[SOC_PPD_BFD_TRAP_ID_OAMP_IPV4]);
        BCMDNX_IF_ERR_EXIT(rv);
        rv = _bcm_dpp_rx_trap_set(unit, _bcm_dpp_bfd_trap_info[unit].trap_ids[SOC_PPD_BFD_TRAP_ID_OAMP_IPV4], &trap_config_oamp, _BCM_OAM_TRAP_FWD_OFFSET_INDEX);
        BCMDNX_IF_ERR_EXIT(rv);

        _bcm_dpp_bfd_trap_info[unit].trap_ids[SOC_PPD_BFD_TRAP_ID_OAMP_MPLS] = SOC_PPD_TRAP_CODE_TRAP_BFD_O_MPLS;
        rv = bcm_petra_rx_trap_type_create(unit, 0, bcmRxTrapOamBfdMpls, (int *)&_bcm_dpp_bfd_trap_info[unit].trap_ids[SOC_PPD_BFD_TRAP_ID_OAMP_MPLS]);
        BCMDNX_IF_ERR_EXIT(rv);
        rv = _bcm_dpp_rx_trap_set(unit, _bcm_dpp_bfd_trap_info[unit].trap_ids[SOC_PPD_BFD_TRAP_ID_OAMP_MPLS], &trap_config_oamp, _BCM_OAM_TRAP_FWD_OFFSET_INDEX);
        BCMDNX_IF_ERR_EXIT(rv);

        _bcm_dpp_bfd_trap_info[unit].trap_ids[SOC_PPD_BFD_TRAP_ID_OAMP_PWE] = SOC_PPD_TRAP_CODE_TRAP_BFD_O_PWE;
        rv = bcm_petra_rx_trap_type_create(unit, 0, bcmRxTrapOamBfdPwe, (int *)&_bcm_dpp_bfd_trap_info[unit].trap_ids[SOC_PPD_BFD_TRAP_ID_OAMP_PWE]);
        BCMDNX_IF_ERR_EXIT(rv);
        rv = _bcm_dpp_rx_trap_set(unit, _bcm_dpp_bfd_trap_info[unit].trap_ids[SOC_PPD_BFD_TRAP_ID_OAMP_PWE], &trap_config_oamp, _BCM_OAM_TRAP_FWD_OFFSET_INDEX);
        BCMDNX_IF_ERR_EXIT(rv);

        _bcm_dpp_bfd_trap_info[unit].trap_ids[SOC_PPD_BFD_TRAP_ID_OAMP_CC_MPLS_TP] = SOC_PPD_TRAP_CODE_TRAP_BFDCC_O_MPLS_TP;
        rv = bcm_petra_rx_trap_type_create(unit, 0, bcmRxTrapOamBfdCcMplsTp, (int *)&_bcm_dpp_bfd_trap_info[unit].trap_ids[SOC_PPD_BFD_TRAP_ID_OAMP_CC_MPLS_TP]);
        BCMDNX_IF_ERR_EXIT(rv);
        rv = _bcm_dpp_rx_trap_set(unit, _bcm_dpp_bfd_trap_info[unit].trap_ids[SOC_PPD_BFD_TRAP_ID_OAMP_CC_MPLS_TP], &trap_config_oamp, _BCM_OAM_TRAP_FWD_OFFSET_INDEX);
        BCMDNX_IF_ERR_EXIT(rv);
        ...
        if (SOC_IS_ARADPLUS(unit)) {
    		...
    		for (prof_ind = 1;  prof_ind < 8 /* 16 profiles */; ++prof_ind) {
    ...
    /* Init local_discr reference counter */
    BCM_SAND_IF_ERR_EXIT(soc_dpp_wb_engine_init_buffer(unit, SOC_DPP_WB_ENGINE_BUFFER_ARAD_PP_BFD_YDISCR_RANGE_REF_COUNTER));
...
int bcm_petra_bfd_endpoint_create(int unit, bcm_bfd_endpoint_info_t *endpoint_info) {
    ...
    uint32 ref_counter; /*used for static registers - bfd_pdu and bfd_cc_packet*/
    uint8 is_bfd_init;
    int correct_flags = 0;
    uint8 port_is_equal;
	uint8 found_profile;
    ...
    is_default = _BCM_BFD_IS_MEP_ID_DEFAULT(unit, endpoint_info->id);
	...
	if ((endpoint_info->flags & BCM_BFD_ECHO) && (SOC_IS_ARAD_B1_AND_BELOW(unit))) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
                         (_BSL_BCM_MSG("Error: BCM_BFD_ECHO is supported only on ARAD+ and above devices.")));
    }

	if ((endpoint_info->type != bcmBFDTunnelTypeUdp) && (endpoint_info->flags & BCM_BFD_ECHO) )
    {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
                         (_BSL_BCM_MSG("Error: BFD ECHO suppoerted only on IPv4 type.")));
    }

	if ((endpoint_info->flags & BCM_BFD_ENDPOINT_HW_ACCELERATION_SET) && (endpoint_info->flags & BCM_BFD_ECHO)) {
		 BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
                         (_BSL_BCM_MSG("Error: BCM_BFD_ENDPOINT_HW_ACCELERATION_SET is not supported with BCM_BFD_ECHO")));
	}

	if ((endpoint_info->flags & BCM_BFD_ECHO) && ((!(endpoint_info->flags & BCM_BFD_ENDPOINT_IN_HW)) || (!(endpoint_info->flags & BCM_BFD_ENDPOINT_MULTIHOP)))) {
		 BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
                         (_BSL_BCM_MSG("Error: BCM_BFD_ECHO flag must be set with BCM_BFD_ENDPOINT_IN_HW and BCM_BFD_ENDPOINT_MULTIHOP flags")));
	}

	if ((endpoint_info->flags & BCM_BFD_ENDPOINT_HW_ACCELERATION_SET) && (SOC_IS_ARAD_B1_AND_BELOW(unit))) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
                         (_BSL_BCM_MSG("Error: BCM_BFD_ENDPOINT_HW_ACCELERATION_SET is supported only on ARAD+ and above devices.")));

    }

    if ((endpoint_info->local_state != bcmBFDStateAdminDown) && (endpoint_info->local_state != bcmBFDStateDown) && (endpoint_info->local_state != bcmBFDStateUp)
        && (endpoint_info->local_state != bcmBFDStateInit)) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
    ...
    if ((endpoint_info->type != bcmBFDTunnelTypeUdp) && (endpoint_info->type != bcmBFDTunnelTypeMpls) && (endpoint_info->type != bcmBFDTunnelTypePweControlWord)
        && (endpoint_info->type != bcmBFDTunnelTypeMplsTpCc) && (endpoint_info->type != bcmBFDTunnelTypeMplsTpCcCv) && (endpoint_info->type != bcmBFDTunnelTypePweGal)) {
        if (SOC_IS_ARAD_B1_AND_BELOW(unit) || (endpoint_info->type == bcmBFDTunnelTypePweRouterAlert) || (endpoint_info->type == bcmBFDTunnelTypePweTtl)) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
    ...
    if (SOC_IS_JERICHO(unit) && ((endpoint_info->type == bcmBFDTunnelTypePweControlWord) || (endpoint_info->type == bcmBFDTunnelTypePweRouterAlert) || (endpoint_info->type == bcmBFDTunnelTypePweTtl))) {
    ...
    if (((endpoint_info->type != bcmBFDTunnelTypeUdp) && (endpoint_info->type != bcmBFDTunnelTypeMpls) )
        && (endpoint_info->src_ip_addr != 0 || endpoint_info->udp_src_port != 0)) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
    ...
    if (endpoint_info->type == bcmBFDTunnelTypePweTtl) {
        if (endpoint_info->egress_label.ttl != 1) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
                             (_BSL_BCM_MSG("Error: In PWE TTL type egress_label.ttl must be 1.")));
        }
    }

    if ( (endpoint_info-> flags & BCM_BFD_ENDPOINT_IN_HW) ==0 || (endpoint_info->type == bcmBFDTunnelTypeUdp)) {
        /* NON acc endpoint or IP endpoint. Either way TX MPLS fields must be 0. */
        if (endpoint_info->egress_label.ttl ||  endpoint_info->egress_label.flags || endpoint_info->egress_label.tunnel_id  || endpoint_info->egress_label.qos_map_id
             || endpoint_info->egress_label.l3_intf_id   || endpoint_info->egress_label.pkt_cfi  || endpoint_info->egress_label.label ) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
    ...
    if (!_BCM_BFD_ENDPOINT_IS_ACCELERATED_TO_OAMP(*endpoint_info)) {
        ...
        if ( endpoint_info->int_pri || endpoint_info->local_diag || endpoint_info->remote_discr || endpoint_info->remote_flags || endpoint_info->local_flags || endpoint_info->loc_clear_threshold
			  || endpoint_info->remote_state || endpoint_info->remote_diag || endpoint_info->local_state || endpoint_info->local_min_echo || endpoint_info->local_min_tx || endpoint_info->local_detect_mult 
			  || endpoint_info->dst_ip_addr || endpoint_info->bfd_period || endpoint_info->remote_detect_mult ) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
        ...
        if (endpoint_info->tx_gport != BCM_GPORT_INVALID &&  !BCM_GPORT_IS_SYSTEM_PORT(endpoint_info->tx_gport) &&
            !BCM_GPORT_IS_MODPORT(endpoint_info->tx_gport) ) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
                             (_BSL_BCM_MSG("Error: tx-gport must be either invalid or a system port.")));
        }
		if (endpoint_info->int_pri >0x1f) {
			BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
        ...
        if (SOC_IS_ARAD_B1_AND_BELOW(unit) || soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "bfd_flags_manual_mgmt", 0)) {
            if ( (endpoint_info->remote_flags& ~0x3f) || (endpoint_info->local_flags& ~0x3f) ) {
                BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
                     (_BSL_BCM_MSG("Error: remote_flags, local_flags 6 bit fields.")));
            }
        } else {
            if (_BCM_BFD_FLAG_COMBINATION_IS_NOT_SUPPORTED(endpoint_info->remote_flags) || 
                _BCM_BFD_FLAG_COMBINATION_IS_NOT_SUPPORTED(endpoint_info->local_flags)) {
                            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
                                 (_BSL_BCM_MSG("Error: Only the flags P, F, D are supported. Given local_flags: %d, remote_flags: %d"),
                                  endpoint_info->local_flags , endpoint_info->remote_flags ));
            }
        }
		if (endpoint_info->local_diag > 16) {
						BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
							 (_BSL_BCM_MSG("Error: local_diag field must be between 0 and 16.")));
		}
		if (endpoint_info->remote_diag > 16) {
						BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
							 (_BSL_BCM_MSG("Error: remote_diag field must be between 0 and 16.")));
		}
        if (endpoint_info->loc_clear_threshold > 3) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
                 (_BSL_BCM_MSG("Error: loc_clear_threshold may not be greater than 3")));
        }
		if (endpoint_info->local_min_rx * endpoint_info->remote_detect_mult > _BCM_BFD_MAX_LIFETIME_IN_MS) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
                 (_BSL_BCM_MSG("Error: local_min_rx * remote_detect_mult is too big")));
        }

        if (endpoint_info->bfd_period > 3406 /* floor(0x3ff * 3.33) is 3406 */) { /* writing on a 10 bit field, in 3.33 ms units*/
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
    ...
    if ( (endpoint_info->type != bcmBFDTunnelTypeUdp && endpoint_info->type != bcmBFDTunnelTypeMpls ) && (endpoint_info->ip_ttl || endpoint_info->ip_tos)) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
    ...
    /* Currently BFD over LSP and IPV4 single hop are mutually exclusive.*/
    if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "bfd_lsp_support", 0)==0) {
        if (endpoint_info->type ==bcmBFDTunnelTypeMpls ) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
                                (_BSL_BCM_MSG("Error: BFDoIPoMPLS not supported without suitable soc property."))); 
        }
    } else {
        if (endpoint_info->type ==bcmBFDTunnelTypeUdp && (endpoint_info->flags & BCM_BFD_ENDPOINT_MULTIHOP)==0 ) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
                                (_BSL_BCM_MSG("Error: BFD over IP Single hop unsupported together with BFD BFDoIPoMPLS."))); 
        }
    }
    ...
    if (endpoint_info->flags & BCM_BFD_ENDPOINT_UPDATE) {
        ...
        rv = bcm_petra_bfd_endpoint_get(unit, endpoint_info->id, &existing_endpoint_info);
        BCMDNX_IF_ERR_EXIT(rv);
		...
		if (endpoint_info->gport != BCM_GPORT_INVALID || existing_endpoint_info.gport != BCM_GPORT_INVALID) {
			...
			rv = _bcm_dpp_gport_to_lif(unit, endpoint_info->gport, &new_lif_id, &dont_care, &dont_care, &dont_care);
			BCMDNX_IF_ERR_EXIT(rv); 
			rv = _bcm_dpp_gport_to_lif(unit, existing_endpoint_info.gport, &existing_lif_id, &dont_care, &dont_care, &dont_care);
			BCMDNX_IF_ERR_EXIT(rv); 
        ...
        if (endpoint_info->remote_id != existing_endpoint_info.remote_id) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Error: remote_id can not be updated.\n")));
        }

        if (((endpoint_info->flags & BCM_BFD_ENDPOINT_IN_HW) != 0) != ((existing_endpoint_info.flags & BCM_BFD_ENDPOINT_IN_HW) != 0)) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Error: Acceleration in HW may not be added / removed.\n")));
        }

        if ((endpoint_info->remote_discr) != (existing_endpoint_info.remote_discr)) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Error: Remote discriminator may not be updated.\n")));
        }

        if ((endpoint_info->flags & BCM_BFD_ENDPOINT_IN_HW) || _BCM_BFD_IS_CLASSIFICATION_DONE_USING_YOUR_DISC(endpoint_info->type)) { /* check discriminator */
            soc_sand_rv = soc_ppd_oam_bfd_discriminator_range_registers_get(unit, &range);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
            if ((endpoint_info->local_discr & SOC_PPD_BFD_DISCRIMINATOR_TO_LIF_START_MASK ) != (existing_endpoint_info.local_discr & SOC_PPD_BFD_DISCRIMINATOR_TO_LIF_START_MASK)) {
                BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Error: LSBs of Local discriminator may not be updated.\n")));
            }
            if (range != endpoint_info->local_discr >> SOC_PPD_BFD_DISCRIMINATOR_RANGE_BIT_START) {
                BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Error: MSBs of Local discriminator may not be updated.\n")));
            }
        }
            
        if ((endpoint_info->udp_src_port) != (existing_endpoint_info.udp_src_port)) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Error: udp_src_port may not be updated.\n")));
        }

        if ((endpoint_info->dst_ip_addr) != (existing_endpoint_info.dst_ip_addr)) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Error: dst_ip_addr may not be updated.\n")));
        }

        if ((endpoint_info->int_pri) != (existing_endpoint_info.int_pri)) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Error: int_pri can not be updated.\n")));
        }

        if ((endpoint_info->src_ip_addr) != (existing_endpoint_info.src_ip_addr)) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Error: src_ip_addr can not be updated.\n")));
        }

        if ((endpoint_info->ip_tos) != (existing_endpoint_info.ip_tos)) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Error: ip_tos can not be updated.\n")));
        }

        if ((endpoint_info->ip_ttl) != (existing_endpoint_info.ip_ttl)) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Error: ip_ttl can not be updated.\n")));
        }

        if ((endpoint_info->egress_label.ttl) != (existing_endpoint_info.egress_label.ttl)) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Error: egress_label.ttl can not be updated.\n")));
        }

        if ((endpoint_info->egress_label.exp) != (existing_endpoint_info.egress_label.exp)) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Error: egress_label.exp can not be updated.\n")));
        }

        if ((endpoint_info->egress_label.label) != (existing_endpoint_info.egress_label.label)) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Error: egress_label.label can not be updated.\n")));
        }

        if ((endpoint_info->egress_if) != (existing_endpoint_info.egress_if)) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Error: egress_if can not be updated.\n")));
        }

        
        rv = _bcm_bfd_classifier_mep_entry_struct_set(unit, endpoint_info, &new_classifier_entry, classifier_mep_entry.dip_profile_ndx);
        BCMDNX_IF_ERR_EXIT(rv); 
        ...
        if (_BCM_BFD_ENDPOINT_IS_ACCELERATED_TO_OAMP(*endpoint_info)) { 
            ...
            SOC_DPP_WB_ENGINE_VAR wb_eng_var = (endpoint_info->type == bcmBFDTunnelTypeMplsTpCc) ? SOC_DPP_WB_ENGINE_VAR_ARAD_PP_BFD_MPLS_TP_CC_REF_COUNTER :
                                                                                                   SOC_DPP_WB_ENGINE_VAR_ARAD_PP_BFD_PDU_REF_COUNTER;
            rv = SOC_DPP_WB_ENGINE_GET_VAR(unit, wb_eng_var, &ref_counter);
            BCMDNX_IF_ERR_EXIT(rv);
            rv = _bcm_bfd_manage_static_registers(unit, endpoint_info, ref_counter, (endpoint_info->type == bcmBFDTunnelTypeMplsTpCc), 1);
            BCMDNX_IF_ERR_EXIT(rv);
            ...
            */
            ...
            soc_sand_rv = soc_ppd_oam_oamp_mep_db_entry_get(unit, endpoint_info->id, &mep_db_entry);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);

            rv = _bcm_mep_db_entry_dealloc(unit, &mep_db_entry);
            BCMDNX_IF_ERR_EXIT(rv);
            SOC_PPD_OAM_OAMP_MEP_DB_ENTRY_clear(&mep_db_entry);
            rv = _bcm_bfd_mep_db_entry_struct_set(unit, endpoint_info, &mep_db_entry);
            BCMDNX_IF_ERR_EXIT(rv);
            /*set entry in mep db*/
            soc_sand_rv = soc_ppd_oam_oamp_mep_db_entry_set(unit, endpoint_info->id, &mep_db_entry, 0, name);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
            ...
            */
            ...
            rv = _bcm_bfd_rmep_db_entry_struct_set(unit, endpoint_info, &rmep_db_entry);
            BCMDNX_IF_ERR_EXIT(rv);

            /* Update entry in rmep db and rmep index db*/
            _BCM_OAM_REMOTE_MEP_INDEX_TO_INTERNAL(rmep_index_internal, endpoint_info->remote_id); 
            soc_sand_rv = soc_ppd_oam_oamp_rmep_set(unit, rmep_index_internal, 0, endpoint_info->id, mep_db_entry.mep_type, &rmep_db_entry, 1);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
		...
		/*update sw db mep_info with new mep */
		rv = _bcm_dpp_oam_bfd_mep_info_db_insert(unit, endpoint_info->id, &new_classifier_entry);
    	BCMDNX_IF_ERR_EXIT(rv);
    } /* END OF UPDATE */
    else {
        if (is_default) {
            ...
            if (SOC_IS_ARAD_B1_AND_BELOW(unit)) {
                ...
                rv = _bcm_dpp_oam_bfd_trap_code_from_remote_gport(unit, endpoint_info->remote_gport, &trap_code, TRUE/*meta_data*/, &classifier_mep_entry);
                BCMDNX_IF_ERR_EXIT(rv);
                ...
                rv = _bcm_bfd_default_id_from_ep_id(unit, endpoint_info->id, &default_id);
                BCMDNX_IF_ERR_EXIT(rv);
            ...
            if (endpoint_info->flags & BCM_BFD_ENDPOINT_WITH_ID) {
                rv = _bcm_dpp_oam_bfd_mep_to_rmep_db_get(unit, endpoint_info->id, &rmep_list_p, &found);
                BCMDNX_IF_ERR_EXIT(rv);
                /* Validate that mep with same id does not exist */
                if (found) {
                    BCMDNX_ERR_EXIT_MSG(BCM_E_NOT_FOUND,
                                     (_BSL_BCM_MSG("Error: Endpoint with id %d already exist."), endpoint_info->id));
                }

                /* Id should be equal to lowest 13 bits of your discriminator in case packet is accelerating to OAMP */
                if ((endpoint_info->flags & BCM_BFD_ENDPOINT_IN_HW) &&
                    ((endpoint_info->remote_gport == BCM_GPORT_INVALID) || (endpoint_info->flags & BCM_BFD_ENDPOINT_HW_ACCELERATION_SET))) {
                    if (endpoint_info->id != (endpoint_info->local_discr & SOC_PPD_BFD_DISCRIMINATOR_TO_ACC_MEP_ID_START_MASK)) {
                        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
                                         (_BSL_BCM_MSG("Error: Error in id %d: id should be equal to lowest 13 bits of local_discr"), endpoint_info->id));
                    }
                }
            }
            ...
            if (_BCM_BFD_ENDPOINT_IS_ACCELERATED_TO_OAMP(*endpoint_info) ||
                ...
                new_disc_range = endpoint_info->local_discr >> SOC_PPD_BFD_DISCRIMINATOR_RANGE_BIT_START;
                rv = SOC_DPP_WB_ENGINE_GET_VAR(unit, SOC_DPP_WB_ENGINE_VAR_ARAD_PP_BFD_YDISCR_RANGE_REF_COUNTER, &ref_counter);
                BCMDNX_IF_ERR_EXIT(rv);
                if (ref_counter > 0) {
                ...
                ref_counter++;
                rv = SOC_DPP_WB_ENGINE_SET_VAR(unit, SOC_DPP_WB_ENGINE_VAR_ARAD_PP_BFD_YDISCR_RANGE_REF_COUNTER, &ref_counter);
                BCMDNX_IF_ERR_EXIT(rv);
            }

            if (_BCM_BFD_ENDPOINT_IS_ACCELERATED_TO_OAMP(*endpoint_info) &&
                ((endpoint_info->type == bcmBFDTunnelTypeMpls) || (endpoint_info->type == bcmBFDTunnelTypeUdp) ||
                 (endpoint_info->type == bcmBFDTunnelTypePweControlWord) || (endpoint_info->type == bcmBFDTunnelTypePweTtl) ||
                 (endpoint_info->type == bcmBFDTunnelTypePweRouterAlert) ) ) {
                rv = SOC_DPP_WB_ENGINE_GET_VAR(unit, SOC_DPP_WB_ENGINE_VAR_ARAD_PP_BFD_PDU_REF_COUNTER, &ref_counter);
                BCMDNX_IF_ERR_EXIT(rv);
                rv = _bcm_bfd_manage_static_registers(unit, endpoint_info, ref_counter, 0, 0);
                BCMDNX_IF_ERR_EXIT(rv);
                /*Update the counter only when we are sure an endpoint will be created*/
                if (endpoint_info->type == bcmBFDTunnelTypeMpls)  {
                    if ((endpoint_info->udp_src_port < _BCM_BFD_UDP_SRC_PORT_MIN) || (endpoint_info->udp_src_port > _BCM_BFD_UDP_SRC_PORT_MAX)) {
                        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
                                         (_BSL_BCM_MSG("Error: udp_src_port value should be between %d and %d. Given value: %d"),
                                          _BCM_BFD_UDP_SRC_PORT_MIN, _BCM_BFD_UDP_SRC_PORT_MAX, endpoint_info->udp_src_port));
                    }
                    soc_sand_rv = soc_ppd_oam_bfd_mpls_udp_sport_get(unit, &udp_sport);
                    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                    if (udp_sport == 0) {
                        soc_sand_rv = soc_ppd_oam_bfd_mpls_udp_sport_set(unit, endpoint_info->udp_src_port);
                        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                    } else if (udp_sport != endpoint_info->udp_src_port) {
                        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
                    ...
                    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                    if (udp_sport == 0) {
                        soc_sand_rv = soc_ppd_oam_bfd_ipv4_udp_sport_set(unit, endpoint_info->udp_src_port);
                        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                    } else if (udp_sport != endpoint_info->udp_src_port) {
                        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
                    ...
                    if ((endpoint_info->flags & BCM_BFD_ENDPOINT_MULTIHOP) == 0) {
                        if ((endpoint_info->ip_ttl != 255) && (endpoint_info->ip_ttl != 1)) {
                            /*TTL&TOS*/
                            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
                                                (_BSL_BCM_MSG("Error: For IP One Hop supporting only ttl 1 and 255. Given value: %d"), endpoint_info->ip_ttl));
                        }
                        if (endpoint_info->dst_ip_addr) {
                            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
                                                (_BSL_BCM_MSG("Error: For IP One Hop supporting dest ip address field should remain blank")));
                        }
                    }
                }
                ...
                BCMDNX_IF_ERR_EXIT(rv);
            } else if (endpoint_info->type == bcmBFDTunnelTypeMplsTpCc) {
                if (_BCM_BFD_ENDPOINT_IS_ACCELERATED_TO_OAMP(*endpoint_info)) {
                    rv = SOC_DPP_WB_ENGINE_GET_VAR(unit, SOC_DPP_WB_ENGINE_VAR_ARAD_PP_BFD_MPLS_TP_CC_REF_COUNTER, &ref_counter);
                    BCMDNX_IF_ERR_EXIT(rv);
                    rv = _bcm_bfd_manage_static_registers(unit, endpoint_info, ref_counter, 1, 0);
                    BCMDNX_IF_ERR_EXIT(rv);
                    ref_counter++;
            ...
            SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_clear(&classifier_mep_entry);
            rv = _bcm_bfd_classifier_mep_entry_struct_set(unit, endpoint_info, &classifier_mep_entry, dip_ndx);
            BCMDNX_IF_ERR_EXIT(rv);
            ...
            if (endpoint_info->flags & BCM_BFD_ENDPOINT_HW_ACCELERATION_SET) {
                ...
                if (endpoint_info->flags & BCM_BFD_ENDPOINT_WITH_ID) {
                    ...
                    endpoint_info->id = endpoint_info->local_discr & SOC_PPD_BFD_DISCRIMINATOR_TO_ACC_MEP_ID_START_MASK;

                    /* Checking same id does not collide with existig endpoints */
                    SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_clear(&classifier_mep_entry_temp);
                    rv = _bcm_dpp_oam_bfd_mep_info_db_get(unit, endpoint_info->id, &classifier_mep_entry_temp, &found);
                    BCMDNX_IF_ERR_EXIT(rv);
                    if (found) {
                        BCMDNX_ERR_EXIT_MSG(BCM_E_EXISTS, (_BSL_BCM_MSG("Error: Local endpoint with id %d exists."), endpoint_info->id));
                    }
                    flags = SHR_RES_ALLOC_WITH_ID;
                }
                rv = _bcm_dpp_bfd_mep_id_alloc(unit, flags, (uint32 *)&endpoint_info->id);
                BCMDNX_IF_ERR_EXIT(rv);
                ...
                if (server_trap_ref_counter == 1) {
                    ...
                    soc_sand_rv = soc_ppd_oam_oamp_rx_trap_codes_set(unit, classifier_mep_entry.mep_type, BCM_GPORT_TRAP_GET_ID(endpoint_info->remote_gport));
                    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                }
                ...
                if (endpoint_info->flags & BCM_BFD_ENDPOINT_IN_HW) {
                    ...
                    if (classifier_mep_entry.lif==_BCM_OAM_INVALID_LIF) {
                        endpoint_info->id = _BCM_BFD_UNACCELERATED_MEP_INDEX_GET(classifier_mep_entry.your_discriminator);
                    }
                    else {
                        endpoint_info->id = _BCM_BFD_UNACCELERATED_MEP_INDEX_GET(classifier_mep_entry.lif);
                    }
                }

                /*in case of BFD echo we don't use The OAM classification the PMF does it*/
                if (!(endpoint_info->flags & BCM_BFD_ECHO)) {
                    SOC_PPD_OAM_LIF_PROFILE_DATA_clear(&profile_data);
                    ...
                    } else  if (BCM_GPORT_IS_SYSTEM_PORT(endpoint_info->remote_gport)  || BCM_GPORT_IS_MODPORT(endpoint_info->remote_gport) ||
                                BCM_GPORT_IS_LOCAL(endpoint_info->remote_gport)) {
                    ...
                    } else if (endpoint_info->remote_gport == BCM_GPORT_INVALID) {
                    ...
                    if (endpoint_info->flags & BCM_BFD_ENDPOINT_MULTIHOP) {
                        ...
                        rv = _bcm_dpp_am_template_bfd_ipv4_dip_alloc(unit, 0 /*flags*/, &(endpoint_info->src_ip_addr), &is_allocated, &dip_ndx);
                        BCMDNX_IF_ERR_EXIT(rv);
                        if (is_allocated) {
                            soc_sand_rv = soc_ppd_oam_bfd_my_bfd_dip_ipv4_set(unit, dip_ndx, endpoint_info->src_ip_addr);
                            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                        }
                    }
            ...
            if (_BCM_BFD_ENDPOINT_IS_ACCELERATED_TO_OAMP(*endpoint_info) ) {
                ...
                */
                ...
                if (endpoint_info->flags & BCM_BFD_ENDPOINT_REMOTE_WITH_ID) {
                    if (!_BCM_OAM_MEP_INDEX_IS_REMOTE(endpoint_info->remote_id)) {
                        _BCM_OAM_REMOTE_MEP_INDEX_FROM_INTERNAL(endpoint_info->remote_id, rmep_index);
                    }
                    else {
                        rmep_index = endpoint_info->remote_id;
                    }
                ...
                endpoint_info->remote_id = rmep_index;
                ...
                rmep_info.mep_index = endpoint_info->id;
                ...
                soc_sand_rv = soc_ppd_oam_oamp_rmep_set(unit, rmep_index_internal, 0, endpoint_info->id, mep_db_entry.mep_type, &rmep_db_entry, 0);
                BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
            ...
            rv = _bcm_dpp_oam_bfd_mep_to_rmep_db_insert(unit, endpoint_info->id, rmep_index);
            BCMDNX_IF_ERR_EXIT(rv);
            ...
            rv = _bcm_dpp_oam_endpoint_list_member_add(unit, &_bcm_bfd_endpoint_list[unit], endpoint_info->id);
            BCMDNX_IF_ERR_EXIT(rv);
        }
    }
   ...
   bcm_bfd_endpoint_info_t *endpoint_info) {
    ...
    if (endpoint_info->type == bcmBFDTunnelTypeMpls) {
        ...
        if ((endpoint_info->flags & (BCM_BFD_ENDPOINT_MULTIHOP | BCM_BFD_ENDPOINT_HW_ACCELERATION_SET | BCM_BFD_ECHO))
            ...
            rv = _bcm_dpp_am_template_bfd_ipv4_dip_data_get(unit, dip_ndx, &(endpoint_info->src_ip_addr));
            BCMDNX_IF_ERR_EXIT(rv);
        }
    }
   ...
   bcm_bfd_endpoint_t endpoint) {
    ...
    if ((!(classifier_mep_entry.flags & SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_FLAG_SERVER))
        ...
        if (classifier_mep_entry.mep_type == SOC_PPD_OAM_MEP_TYPE_BFD_O_IPV4_M_HOP) {
            rv = _bcm_dpp_am_template_bfd_ipv4_dip_free(unit, classifier_mep_entry.dip_profile_ndx, &is_last);
            BCMDNX_IF_ERR_EXIT(rv);
            if (is_last) {
                soc_sand_rv = soc_ppd_oam_bfd_my_bfd_dip_ipv4_set(unit, classifier_mep_entry.dip_profile_ndx, 0);
                BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
            }
        }
...
int bcm_petra_bfd_clear(int unit) {
    ...
    if (!oam_is_init) {
        ...
        rv = bcm_petra_rx_trap_type_destroy(unit, _bcm_dpp_bfd_trap_info[unit].trap_ids[SOC_PPD_BFD_TRAP_ID_CPU]);
        BCMDNX_IF_ERR_EXIT(rv);
    }

    rv = bcm_petra_rx_trap_type_destroy(unit, _bcm_dpp_bfd_trap_info[unit].trap_ids[SOC_PPD_BFD_TRAP_ID_OAMP_IPV4]);
    BCMDNX_IF_ERR_EXIT(rv);

    rv = bcm_petra_rx_trap_type_destroy(unit, _bcm_dpp_bfd_trap_info[unit].trap_ids[SOC_PPD_BFD_TRAP_ID_OAMP_MPLS]);
    BCMDNX_IF_ERR_EXIT(rv);

    rv = bcm_petra_rx_trap_type_destroy(unit, _bcm_dpp_bfd_trap_info[unit].trap_ids[SOC_PPD_BFD_TRAP_ID_OAMP_PWE]);
    BCMDNX_IF_ERR_EXIT(rv);

    rv = bcm_petra_rx_trap_type_destroy(unit, _bcm_dpp_bfd_trap_info[unit].trap_ids[SOC_PPD_BFD_TRAP_ID_OAMP_CC_MPLS_TP]);
    BCMDNX_IF_ERR_EXIT(rv);
...
bcm_dpp_bfd_endpoint_diag_print(bcm_bfd_endpoint_info_t *endpoint_info) {
    ...
    case bcmBFDTunnelTypePweControlWord:
        LOG_CLI((BSL_META("BFD over PWE control word"))); break;
    case bcmBFDTunnelTypeMplsTpCc:
