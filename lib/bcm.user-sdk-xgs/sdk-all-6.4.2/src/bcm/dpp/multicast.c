 ...
 */
int
 _bcm_petra_multicast_ppd_port_bitmap_from_mult_vlan_pbmp(int unit, uint32 *ports, const SOC_TMC_MULT_EG_VLAN_MEMBERSHIP_GROUP *bitmap_group)
{
    int i, nof_local_ports, nof_longs_in_bitmap; 
    BCMDNX_INIT_FUNC_DEFS;
    ...
    if (SOC_IS_JERICHO(unit)) { /* Jericho needs conversion from TM ports + core to local ports */
        ...
        int core = 0;
        for (i = 0; i < nof_longs_in_bitmap; i++) { /* init output bitmap to no ports */
            ports[i] = 0;
        }
        ...
        do { /* loop over all the TM ports in the bitmap */
            ...
            do { /* loop over the TM ports in the current word */
                if (bits & 1) { /* convert the found TM port to local port */
                    ...
                    soc_error_t res = soc_port_sw_db_tm_to_local_port_get(unit, tm_port, core, &local_port);
                    if (res == SOC_E_NOT_FOUND) {
                        ...
                        ports[local_port / SOC_SAND_NOF_BITS_IN_UINT32] |= 1 << (local_port % SOC_SAND_NOF_BITS_IN_UINT32);
                    }
                }
                bits >>= 1;
            } while (++tm_port % SOC_SAND_NOF_BITS_IN_UINT32);
            if (--words_left % (SOC_TMC_NOF_FAP_PORTS_PER_CORE / SOC_SAND_NOF_BITS_IN_UINT32)) {
                ++core; /* If finished going over the TM ports of a core, increase the core ID */
                tm_port = 0;
            }
        } while (words_left);

    } else { /* No need to convert ports, as tm port = local port in the device */
        for (i = 0; i < nof_longs_in_bitmap; i++) {
            ports[i] = bitmap_group->bitmap[i];
        }
    }
...
/*
 * Convert an egress bitmap group from a local port bitmap to a SOC_TMC TM ports bitmap.
 */
int
_bcm_petra_multicast_ppd_port_bitmap_to_mult_vlan_pbmp(int unit, const uint32 *ports, SOC_TMC_MULT_EG_VLAN_MEMBERSHIP_GROUP *bitmap_group)
{
    int i, nof_local_ports, nof_longs_in_bitmap; 
    BCMDNX_INIT_FUNC_DEFS;
    BCMDNX_NULL_CHECK(ports);
    ...
    if (SOC_IS_JERICHO(unit)) { /* Jericho needs conversion from TM ports + core to local ports */
        ...
        for (i = 0; i < nof_longs_in_bitmap; i++) { /* loop over all the local ports in the bitmap */
            bits = ports[i];
            do { /* loop over the local ports in the current word */
                if (bits & 1) { /* convert the found local port to a TM port+core ID */
                ...
                bits >>= 1;
            } while (++local_port % SOC_SAND_NOF_BITS_IN_UINT32);
        }

    } else { /* No need to convert ports, as tm port = local port in the device */
        for (i = 0; i < nof_longs_in_bitmap; i++) { 
            bitmap_group->bitmap[i] = ports[i];
        }
    }
