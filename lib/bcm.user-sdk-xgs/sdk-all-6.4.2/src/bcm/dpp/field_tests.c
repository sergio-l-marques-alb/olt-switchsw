...
#define _BCM_PETRA_FIELD_MAX_QUALIFY_LENGTH           256
...
#define _BCM_PETRA_FIELD_GROUP_PRIO_HIGHEST           122 /*127 (higest priority - 5 proiorities used for ITMH programmable mode)*/
...
#define _BCM_PETRA_FIELD_TEST_MAXIMAL_GROUP_ID        122 /*127 (higest group id - 5 groups used for ITMH programmable mode)*/
...
#define _BCM_PETRA_FIELD_TEST_PACKET_SIZE_BYTES       160             
...
#define _BCM_PETRA_FIELD_TEST_MAX_DATA_QUALIFIER_ID                  (SOC_PPC_FP_QUAL_HDR_USER_DEF_LAST - SOC_PPC_FP_QUAL_HDR_USER_DEF_0 - 4 ) /*4 last dataquals are used for ITMH programmable mode setting*/
#define _BCM_PETRA_FIELD_TEST_DATA_QUALIFIER_MAX_OFFSET              504
...
/* Takes the randoum value x the number of iteration i and returns the rand value*/
...
STATIC bcm_error_t _bcm_petra_field_test_qualify_hardware_length(int unit, _bcm_dpp_field_stage_idx_t stage, uint32 type, bcm_field_qualify_t qualify, uint32 *length_best, uint32 *length_worst)
{
    ...
    for(ppd_qual_ndx = 0; ppd_qual_ndx < _BCM_PETRA_FIELD_QUAL_CHAIN; ++ppd_qual_ndx) {
        if(soc_ppd_qual_ptr[ppd_qual_ndx] < SOC_PPD_NOF_FP_QUAL_TYPES) {
            rv = _bcm_dpp_ppd_qual_bits(unitData, stage, soc_ppd_qual_ptr[ppd_qual_ndx], &exposed, &hardwareBestCase, &hardwareWorstCase);
            BCM_IF_ERROR_RETURN(rv);
            if(length_best) {
 ...
 * Needed to choose rendom mask and value for the qualifier
 */
STATIC bcm_error_t _bcm_petra_field_test_qualify_exposed_length(int unit, _bcm_dpp_field_stage_idx_t stage, uint32 type, bcm_field_qualify_t qualify, uint32 *length)
{
    ...
    if(unitData->qualMaps[qualify] == NULL)
    {
    ...
    if (unitData->qualMaps[qualify][_BCM_DPP_FIELD_QUALMAP_OFFSET_LENGTH] > 0) {
        *length = unitData->qualMaps[qualify][_BCM_DPP_FIELD_QUALMAP_OFFSET_LENGTH];
    } else {
        for (ppd_qual_ndx = 0, *length = 0;
             (BCM_E_NONE == rv) &&
             (ppd_qual_ndx < unitData->devInfo->qualChain);
             ppd_qual_ndx++) {
            if (ppdQual[ppd_qual_ndx] < SOC_PPD_NOF_FP_QUAL_TYPES) {
                rv = _bcm_dpp_ppd_qual_bits(unitData,
                                            stage,
...
{
    _bcm_dpp_field_info_t *unitData;
    SOC_PPD_FP_ACTION_TYPE *soc_ppd_action_ptr;
    bcm_error_t rv;
    ...
    *length = 0;
    ...
    for(ppd_action_ndx = 0; ppd_action_ndx < _BCM_PETRA_FIELD_ACTION_CHAIN; ++ppd_action_ndx) {
        if(soc_ppd_action_ptr[ppd_action_ndx] < SOC_PPD_NOF_FP_ACTION_TYPES) {
            rv = _bcm_dpp_ppd_action_bits(unitData, stage, soc_ppd_action_ptr[ppd_action_ndx], &action_length);
            BCM_IF_ERROR_RETURN(rv);
...
STATIC bcm_error_t _bcm_petra_field_test_action_length_with_valid_bit(int unit, _bcm_dpp_field_stage_idx_t stage, uint32 type, bcm_field_action_t action, uint32 *length) {
    bcm_error_t rv;
    rv = _bcm_petra_field_test_action_length(unit, stage, action, length);
    if(rv != BCM_E_NONE) {
        return rv;
    }
    ++(*length); /* adding the valid bit*/
    return BCM_E_NONE;
}

/* 
 * This function gets the length of each one of the qulifiers and put the value in the global array qualifiers_info
 */
STATIC cmd_result_t _bcm_petra_field_test_set_qualifiers_info(int unit, _bcm_dpp_field_stage_idx_t stage, uint32 type)
{
    ...
    for(qual_ndx = 0; qualifiers_info[qual_ndx].qualify != bcmFieldQualifyCount; ++qual_ndx) {
...
/*
 * This function gets unit, entry, qualifier, arries that contains the data and the mask (if neeeded mask).
...
STATIC bcm_error_t _bcm_petra_field_test_operate_set_function(int unit, _bcm_dpp_field_stage_idx_t stage, uint32 type, bcm_field_entry_t entry, bcm_field_qualify_t qualifier, uint32 *data, uint32 *mask)
{
    ...
    for(qual_index=0; qualifiers_info[qual_index].qualify != bcmFieldQualifyCount; ++qual_index) {
    ...
    switch(prototype) {
        ...
        case 15:
        {
            ...
            /* bcm_field_IpProtocolCommon_t is enumorator - we assume 32 bits*/
            s_data = data[0];
        ...
        case 17:
        {
            ...
            /* bcm_field_IpType_t is enumorator - we assume 32 bits*/
            s_data = data[0];
        ...
        case 18:
        {
            ...
            /* bcm_field_TunnelType_t is enumorator - we assume 32 bits*/
            s_data = data[0];
        ...
        case 19:
        {
            ...
            /* bcm_field_LoopbackType_t is enumorator - we assume 32 bits*/
            s_data = data[0];
        ...
        case 20:
        {
            ...
            /* bcm_field_ForwardingType_t is enumorator - we assume 32 bits*/
            s_data = data[0];
        ...
        case 21:
        {
            ...
            /* bcm_field_stage_t is enumorator - we assume 32 bits*/
            s_data = data[0];
        ...
        case 23:
        {
            ...
            /* bcm_field_IpFrag_t is enumorator - we assume 32 bits*/
            s_data = data[0];
        ...
        case 24:
        {
            ...
            /* bcm_field_L2Format_t is enumorator - we assume 32 bits*/
            s_data = data[0];
        ...
        case 29:
        {
            ...
            /* bcm_field_FibreChan_t is enumorator - we assume 32 bits*/
            s_data = data[0];
        ...
        case 31:
        {
            ...
            /* bcm_field_header_format_t is enumorator - we assume 32 bits*/
            s_data = data[0];
        ...
        case 33:
        {
            ...
            /* bcm_rx_trap_t is enumorator - we assume 32 bits*/
            s_data = data[0];
        ...
        case 35:
        {
            ...
            /* bcm_field_ArpOpcode_t is enumorator - we assume 32 bits*/
            s_data = data[0];
        ...
        case 37:
        {
            ...
            /* bcm_field_AppType_t is enumorator - we assume 32 bits*/
            s_data = data[0];
...
/*
 * This function gets unit, entry, qualifier, arries that contains the data and the mask to be get.
    ...
    if(prototype == 15) {
        if(stage == _BCM_DPP_FIELD_STAGE_INDEX_INGRESS
            ...
            i = _bcm_petra_field_rand_val(x, second_rand_param++) %
                (sizeof(supported_IpProtocolCommon_Ingress)/sizeof(bcm_field_IpProtocolCommon_t));
            data[0] = supported_IpProtocolCommon_Ingress[i];
        } else {
            i = _bcm_petra_field_rand_val(x, second_rand_param++) %
                (sizeof(supported_IpProtocolCommon_Egress)/sizeof(bcm_field_IpProtocolCommon_t));
        ...
        if(stage == _BCM_DPP_FIELD_STAGE_INDEX_INGRESS
            || stage == _BCM_DPP_FIELD_STAGE_INDEX_ING_EXTERNAL) {
            i = _bcm_petra_field_rand_val(x, second_rand_param++) %
                (sizeof(supported_IpType_Ingress)/sizeof(bcm_field_IpType_t));
            data[0] = supported_IpType_Ingress[i];
        } else {
            i = _bcm_petra_field_rand_val(x, second_rand_param++) %
                (sizeof(supported_IpType_Egress)/sizeof(bcm_field_IpType_t));
        ...
        if(stage == _BCM_DPP_FIELD_STAGE_INDEX_INGRESS
            || stage == _BCM_DPP_FIELD_STAGE_INDEX_ING_EXTERNAL) {
            i = _bcm_petra_field_rand_val(x, second_rand_param++) %
                (sizeof(supported_TunnelType_Ingress)/sizeof(bcm_field_TunnelType_t));
            data[0] = supported_TunnelType_Ingress[i];
        } else {
            i = _bcm_petra_field_rand_val(x, second_rand_param++) %
                (sizeof(supported_TunnelType_Egress)/sizeof(bcm_field_TunnelType_t));
        ...
        if(stage == _BCM_DPP_FIELD_STAGE_INDEX_INGRESS
            || stage == _BCM_DPP_FIELD_STAGE_INDEX_ING_EXTERNAL) {
            i = _bcm_petra_field_rand_val(x, second_rand_param++) %
                (sizeof(supported_ForwardingType_Ingress)/sizeof(bcm_field_ForwardingType_t));
            data[0] = supported_ForwardingType_Ingress[i];
        } else {
            i = _bcm_petra_field_rand_val(x, second_rand_param++) %
                (sizeof(supported_ForwardingType_Egress)/sizeof(bcm_field_ForwardingType_t));
        ...
        if(stage == _BCM_DPP_FIELD_STAGE_INDEX_INGRESS
            || stage == _BCM_DPP_FIELD_STAGE_INDEX_ING_EXTERNAL) {
            i = _bcm_petra_field_rand_val(x, second_rand_param++) %
                (sizeof(supported_stage_Ingress)/sizeof(bcm_field_stage_t));
            data[0] = supported_stage_Ingress[i];
        } else {
            i = _bcm_petra_field_rand_val(x, second_rand_param++) %
                (sizeof(supported_stage_Egress)/sizeof(bcm_field_stage_t));
        ...
        if(stage == _BCM_DPP_FIELD_STAGE_INDEX_INGRESS
            || stage == _BCM_DPP_FIELD_STAGE_INDEX_ING_EXTERNAL) {
            i = _bcm_petra_field_rand_val(x, second_rand_param++) %
                (sizeof(supported_IpFrag_Ingress)/sizeof(bcm_field_IpFrag_t));
            data[0] = supported_IpFrag_Ingress[i];
        } else {
            i = _bcm_petra_field_rand_val(x, second_rand_param++) %
                (sizeof(supported_IpFrag_Egress)/sizeof(bcm_field_IpFrag_t));
        ...
        if(stage == _BCM_DPP_FIELD_STAGE_INDEX_INGRESS
            || stage == _BCM_DPP_FIELD_STAGE_INDEX_ING_EXTERNAL) {
            i = _bcm_petra_field_rand_val(x, second_rand_param++) %
                (sizeof(supported_L2Format_Ingress)/sizeof(bcm_field_L2Format_t));
            data[0] = supported_L2Format_Ingress[i];
        } else {
            i = _bcm_petra_field_rand_val(x, second_rand_param++) %
                (sizeof(supported_L2Format_Egress)/sizeof(bcm_field_L2Format_t));
        ...
        extern void _bcm_dpp_rx_ppd_trap_get(int, bcm_rx_trap_t, int*);
        int soc_ppd_trap_id;
        ...
        if(qualifier == bcmFieldQualifyPacketRes) {
            ...
            data[0] = _BCM_PETRA_L3_ITF_FEC << _BCM_PETRA_L3_ITF_SHIFT;
            ...
            /* in all if this case the ethertype will be identified as vlan and not ethrtype */
            while(data[0] == 0x8100
                || data[0] == 0x88a8
                || data[0] == 0x9100
                || data[0] == 0x9200
            ...
            if(qualifiers_info[qual_index].size == 0) {
                ...
                if(_BCM_PETRA_FIELD_TEST_DEBUG_CHECK_ERR) {
                    LOG_INFO(BSL_LS_BCM_FP,
                             (BSL_META_U(unit,
                                         "The qualifier in not supported \n")));
                }
                _BCM_PETRA_FIELD_TEST_FAIL_MSG;
                return CMD_FAIL;
            }
            ...
            /* mask if needed */
            if((prototype  >= 0 && prototype <=9) || prototype == 11 || prototype == 12 || prototype == 14 || prototype == 28 || prototype == 34) {
                for(i = 0; i < qualifiers_info[qual_index].size; i+=32) {
                    if(qualifiers_info[qual_index].size - i >= 32) {
 ...
 * fills it with max_items items and return vector of the selceted items
 */
    ...
    )
{
    ...
    /* The idea is to take randomally one of the items in valid_items[0 ... nof_valid_items -item_idx],
 ...
 * Then it takes randomally _BCM_PETRA_FIELD_SET_AND_REMOVE_TEST_NOF_ITEMS_TO_TEST and check if they are in the set. 
...
/*
 * The function gets the set of valid qualifiers and actions for specific type selected randommaly 
 * Return 0 in success, 1 in failure 
 * if type is 0 the type will be choosen randomally, else it will take the required type
 */
...
uint8 _bcm_petra_field_test_data_qualifier_get_valid_set(int unit, _bcm_dpp_field_stage_idx_t stage, uint32 x, bcm_field_qset_t *valid_set)
{
    ...
    for(data_qual_id = 0; data_qual_id <= _BCM_PETRA_FIELD_TEST_MAX_DATA_QUALIFIER_ID; ++data_qual_id) {
...
{
    int qual_arr[_BCM_PETRA_FIELD_TEST_MAX_DATA_QUALIFIER_ID + 1];
    int qual_count;
    int qual_idx;
    bcm_error_t rv;
    ...
    rv = bcm_field_qset_data_qualifier_get(data_qualifier_add_remove_test_unit,
                                           *set,
                                           _BCM_PETRA_FIELD_TEST_MAX_DATA_QUALIFIER_ID + 1,
                                           qual_arr,
                                           &qual_count);
    if(rv != BCM_E_NONE) {
        LOG_INFO(BSL_LS_BCM_FP,
                 (BSL_META("Failur in bcm_field_qset_data_qualifier_get\n")));
    }
...
STATIC void _bcm_petra_field_data_qualifier_set_print(const bcm_field_qset_t *qset)
{
    ...
    if(rv != BCM_E_NONE) {
        LOG_CLI((BSL_META_U(unit,
                            "Failur in bcm_field_qset_data_qualifier_get\n")));
    }
    for(qual_idx = 0; qual_idx < qual_count; ++qual_idx) {
        LOG_CLI((BSL_META_U(unit,
                            "%d "), qual_arr[qual_idx]));
...
#else
        is_lsb_lcl[ppd_qual_idx] = is_msb_lcl[ppd_qual_idx] = TRUE;
#endif
    }
    if(is_lsb) {
        *is_lsb = TRUE;
...
/* avilable resources for single program */
...
/* availablr resources for single field group */
 ...
 * it take its hardware length for each of the posible hardware qualifiers, and any 16 bits or less may take 1 c.e. 
 * it deyermine also the number of hardware qualifiers 
 */
STATIC bcm_error_t _bcm_petra_field_test_qualify_get_nof_resorces(int unit, _bcm_dpp_field_stage_idx_t stage, uint32 type, bcm_field_qualify_t qualify, uint32 *nof_ces, uint32 *nof_hw_quals)
{
    ...
    for(ppd_qual_idx = 0; ppd_qual_idx < _BCM_PETRA_FIELD_QUAL_CHAIN; ++ppd_qual_idx) {
        if(soc_ppd_qual_ptr[ppd_qual_idx] < SOC_PPD_NOF_FP_QUAL_TYPES) {
            if(nof_hw_quals) {
                *nof_hw_quals = ppd_qual_idx + 1;
            }
            rv = _bcm_dpp_ppd_qual_bits(unitData, stage, soc_ppd_qual_ptr[ppd_qual_idx], &exposed, &hardwareBestCase, &hardwareWorstCase);
            BCM_IF_ERROR_RETURN(rv);
            if(nof_ces) {
...
/* In this case the limit is not tp take more than 4 ces. */
    ...
    /* verify no excessing of the lsb and msb buffer.
     * if the qualifier can be nor lsb either msb, we can be confident
     * because of the check that the total length doen't excess the maximal key lengbth
     */
...
)
{
    ...
    for(nof_items_in_set = 0; nof_items_in_set < max_items && length < max_length ; ++nof_items_in_set) {
        do
        {
            ...
            to_continue = FALSE;
            /* check that the new item can be with all the prevoius */
...
{
    _bcm_dpp_field_info_t *unitData;
    SOC_PPD_FP_QUAL_TYPE *soc_ppd_qual_ptr1, *soc_ppd_qual_ptr2;
    bcm_error_t rv;
    ...
    for(ppd_qual_idx1 = 0; ppd_qual_idx1 < _BCM_PETRA_FIELD_QUAL_CHAIN; ++ppd_qual_idx1) {
        for(ppd_qual_idx2 = 0; ppd_qual_idx2 < _BCM_PETRA_FIELD_QUAL_CHAIN; ++ppd_qual_idx2) {
            if(soc_ppd_qual_ptr1[ppd_qual_idx1] < SOC_PPD_NOF_FP_QUAL_TYPES &&
...
{
    _bcm_dpp_field_info_t *unitData;
    SOC_PPD_FP_ACTION_TYPE *soc_ppd_action_ptr1, *soc_ppd_action_ptr2;
    bcm_error_t rv;
    ...
    for(ppd_action_idx1 = 0; ppd_action_idx1 < _BCM_PETRA_FIELD_ACTION_CHAIN; ++ppd_action_idx1) {
        for(ppd_action_idx2 = 0; ppd_action_idx2 < _BCM_PETRA_FIELD_ACTION_CHAIN; ++ppd_action_idx2) {
            if(soc_ppd_action_ptr1[ppd_action_idx1] < SOC_PPD_NOF_FP_ACTION_TYPES &&
...
/*
 * This function get qset and aset randomally for field group 
                                                                        ...
                                                                        uint32 *sum_actions)
{
    ...
    if(rnd == 0 && max_bank_size >= 320) {
        ...
        max_qualifiers_length = 80;/* must be in onle single zone - no split overheadr */
    }
    ...
    if(_bcm_petra_field_test_qualify_and_action_get_valid_set(unit, stage, x, type, &valid_qset, &valid_aset)) {
        if(_BCM_PETRA_FIELD_TEST_DEBUG_CHECK_ERR) {
            LOG_INFO(BSL_LS_BCM_FP,
                     (BSL_META_U(unit,
                                 "Falied to get valid qset and aset\n")));
        }
        _BCM_PETRA_FIELD_TEST_FAIL_MSG;
        return CMD_FAIL;
    }
                                                               ...
                                                               int nof_groups)
{
    ...
    for(fg_idx = 0; fg_idx < nof_groups; ++fg_idx) {
        ...
        if(maximal_key_length == SOC_DPP_DEFS_GET(unit, tcam_big_bank_key_nof_bits)) {
            field_groups[fg_idx].flags |= BCM_FIELD_GROUP_CREATE_LARGE;
        } else {
                                                                 ...
                                                                 uint8 print_anyway)
{
    ...
    /* qset comparing */
    /* we just verify that what we have insetred is included. We may have another qualifiers */
    rv = _bcm_dpp_field_qset_subset(qset_verify, field_group->qset);
    if(rv != BCM_E_NONE) {
        if(_BCM_PETRA_FIELD_TEST_DEBUG_CHECK_ERR || print_anyway) {
            LOG_INFO(BSL_LS_BCM_FP,
                     (BSL_META_U(unit,
                                 "group id %d - the qset retrived is not including the qset set  \n"), field_group->group));
        }
        _BCM_PETRA_FIELD_TEST_FAIL_MSG;
        return CMD_FAIL;
    }
    ...
    /* we just verify that what we have insetred is included. We may have another actions */
    rv = _bcm_dpp_field_aset_subset(aset_verify, field_group->aset);
    if(rv != BCM_E_NONE) {
        if(_BCM_PETRA_FIELD_TEST_DEBUG_CHECK_ERR || print_anyway) {
            LOG_INFO(BSL_LS_BCM_FP,
                     (BSL_META_U(unit,
                                 "group id %d - the aset retrived is not including the aset set \n"), field_group->group));
        }
        _BCM_PETRA_FIELD_TEST_FAIL_MSG;
        return CMD_FAIL;
    }
           ...
           && ((entry->id < _BCM_DPP_FIELD_ENT_BIAS(unit, DirExt) 
            ...
            if((!entry->quals_info[qual_idx].is_undefined)) {
				...
				if(stage != _BCM_DPP_FIELD_STAGE_INDEX_ING_EXTERNAL) {
                    ...
                    if(sal_memcmp(entry->quals_info[qual_idx].data, data_verify, sizeof(data_verify))) {
                        if(_BCM_PETRA_FIELD_TEST_DEBUG_CHECK_ERR || print_anyway) {
                            LOG_INFO(BSL_LS_BCM_FP,
                                     (BSL_META_U(unit,
                    ...
                    if(sal_memcmp(entry->quals_info[qual_idx].mask, mask_verify, sizeof(data_verify))) {
                        if(_BCM_PETRA_FIELD_TEST_DEBUG_CHECK_ERR || print_anyway) {
                            LOG_INFO(BSL_LS_BCM_FP,
                                     (BSL_META_U(unit,
                                                 "entry %d qualifier %s mask mismatch.\n") , entry->id, qualifiers_names[entry->quals_info[qual_idx].qualify]));
                        }
                        _BCM_PETRA_FIELD_TEST_FAIL_MSG;
                        return CMD_FAIL;
                    }
				...
				if(stage != _BCM_DPP_FIELD_STAGE_INDEX_ING_EXTERNAL) {
                    /* we do it to avoid msb allignment */
...
/*
 * get randomally param0 and param1 for action
 */
...
/* If we wand to use the qulifier MplsLabel2XXX, we can't expect MplsLabel1Bos to be true.
                                                                       ...
                                                                       uint32 *buffer)
{
    ...
    sal_memset(buffer, 0xa5, _BCM_PETRA_FIELD_TEST_PACKET_SIZE_BYTES);
    ...
    ipv6_header.payload_length = _BCM_PETRA_FIELD_TEST_PACKET_SIZE_BYTES - 60 - 14 - 4 * ethernet_header.nof_vlans; 
    ...
    SHR_BITCOPY_RANGE(
        buffer,
        _BCM_PETRA_FIELD_TEST_PACKET_SIZE_BYTES*8 -
    ...
    for(qual_idx = 0; qual_idx < _BCM_PETRA_FIELD_TEST_MAX_QUALIFIERS_IN_ENTRY; ++qual_idx) {
        if(entry->quals_info[qual_idx].is_valid) {
            ...
            if(dq.flags & BCM_FIELD_DATA_QUALIFIER_OFFSET_NEGATIVE) {
                SHR_BITCOPY_RANGE(buffer, (_BCM_PETRA_FIELD_TEST_PACKET_SIZE_BYTES + dq.offset - dq_offset_addition - dq.length)*8,
                                                       ...
                                                       uint32 *buffer)
{
                    ...
                    if(!is_predefined) {
                        ...
                        if(ethernet_header.nof_vlans == 1 && entry->quals_info[qual_idx].data[0] != 0x8100 && should_hit) {
                            if(_BCM_PETRA_FIELD_TEST_DEBUG_CHECK_VERB) {
                                LOG_INFO(BSL_LS_BCM_FP,
                                         (BSL_META_U(unit,
                                                     "Changing the data of the qulifier OuterTpid to 0x8100.\n")));
                            }
                            ...
                            if(_BCM_PETRA_FIELD_TEST_DEBUG_CHECK_VERB) {
                                LOG_INFO(BSL_LS_BCM_FP,
                                         (BSL_META_U(unit,
                                                     "Changing the data of the qulifier OuterTpid to 0x9100.\n")));
                            }
                    ...
                    if(is_ipv4) {
                        if((entry->quals_info[qual_idx].data[0] & entry->quals_info[qual_idx].mask[0]) !=
                            ...
                            if(_BCM_PETRA_FIELD_TEST_DEBUG_CHECK_VERB) {
                                LOG_INFO(BSL_LS_BCM_FP,
                                         (BSL_META_U(unit,
                                                     "Changing the data of the qulifier EtherType to 0x800 and the mask to 0xffff.\n")));
                            }
                        ...
                        if((entry->quals_info[qual_idx].data[0] & entry->quals_info[qual_idx].mask[0]) !=
                            ...
                            if(_BCM_PETRA_FIELD_TEST_DEBUG_CHECK_VERB) {
                                LOG_INFO(BSL_LS_BCM_FP,
                                         (BSL_META_U(unit,
                                                     "Changing the data of the qulifier EtherType to 0x86dd and the mask to 0xffff.\n")));
                            }
                            if(bcm_field_qualify_EtherType(unit,
                                                           entry->id,
                                                           entry->quals_info[qual_idx].data[0],
                        ...
                        if((entry->quals_info[qual_idx].data[0] & entry->quals_info[qual_idx].mask[0]) !=
                            ...
                            if(_BCM_PETRA_FIELD_TEST_DEBUG_CHECK_VERB) {
                                LOG_INFO(BSL_LS_BCM_FP,
                                         (BSL_META_U(unit,
                                                     "Changing the data of the qulifier EtherType to 0x8847 and the mask to 0xffff.\n")));
                            }
                            if(bcm_field_qualify_EtherType(unit,
                                                           entry->id,
                                                           entry->quals_info[qual_idx].data[0],
        ...
        for(qual_idx = 0; qual_idx < _BCM_PETRA_FIELD_TEST_MAX_QUALIFIERS_IN_ENTRY; ++qual_idx)
        {
            if(entry->quals_info[qual_idx].is_valid) {
                switch(entry->quals_info[qual_idx].qualify)
                {
                    ...
                    case bcmFieldQualifyMplsLabel1:
                        if(entry->quals_info[qual_idx].mask[0]) {
                            uint32 tmp_data = entry->quals_info[qual_idx].data[0];
                            tmp_data ^= 1 << first_set_bit_ndx(entry->quals_info[qual_idx].mask[0]);
                            _bcm_petra_field_test_buffer_to_mpls_header(tmp_data, &mpls_header);
                            may_not_hit = TRUE;
                        }
                        break;
                    case bcmFieldQualifyMplsLabel1Ttl:
                    ...
                    case bcmFieldQualifyMplsLabel2:
                        if(entry->quals_info[qual_idx].mask[0]) {
                            uint32 tmp_data = entry->quals_info[qual_idx].data[0];
                            tmp_data ^= 1 << first_set_bit_ndx(entry->quals_info[qual_idx].mask[0]);
                            _bcm_petra_field_test_buffer_to_mpls_header(tmp_data, &mpls_header2);
                            may_not_hit = TRUE;
                        }
                        break;
                    case bcmFieldQualifyMplsLabel2Ttl:
                    ...
                    case bcmFieldQualifyMplsLabel3:
                        if(entry->quals_info[qual_idx].mask[0]) {
                            uint32 tmp_data = entry->quals_info[qual_idx].data[0];
                            tmp_data ^= 1 << first_set_bit_ndx(entry->quals_info[qual_idx].mask[0]);
                            _bcm_petra_field_test_buffer_to_mpls_header(tmp_data, &mpls_header3);
                            may_not_hit = TRUE;
                        }
                        break;
                    case bcmFieldQualifyMplsLabel3Ttl:
    ...
    if(is_ipv4){
        ...
        ipv6_header.version = 6;
        ipv6_header.payload_length = _BCM_PETRA_FIELD_TEST_PACKET_SIZE_BYTES - 40 - 14 - 4 * ethernet_header.nof_vlans; 
...
#ifdef BCM_ARAD_SUPPORT
/* verify that no action occured */
STATIC uint8 no_actions_verification(SOC_PPD_FP_PACKET_DIAG_INFO *info, uint8 is_de)
{
    int32 cycle, macro;
    for(cycle = ARAD_PP_FP_NOF_CYCLES - 1; cycle >= 0; --cycle){
        if(!is_de) {
            for(macro = _BCM_PETRA_FIELD_TEST_PP_FP_NOF_MACRO_SIMPLES - 1; macro >= 0; --macro) {
                if(info->macro_simple[cycle][macro].action.type != SOC_PPD_FP_ACTION_TYPE_INVALID
                    && info->macro_simple[cycle][macro].action.type != SOC_PPD_FP_ACTION_TYPE_NOP) {
                    return FALSE;
                }
            }
        } else {
            for(macro = _BCM_PETRA_FIELD_TEST_PP_FP_NOF_MACROS - 1; macro >= 0; --macro) {
                if(info->macro[cycle][macro].action.type != SOC_PPD_FP_ACTION_TYPE_INVALID
                   && info->macro[cycle][macro].action.type != SOC_PPD_FP_ACTION_TYPE_NOP) {
                    return FALSE;
                }
            }
        }
...
STATIC uint8 _bcm_petra_field_test_check_hit(SOC_PPD_FP_PACKET_DIAG_INFO *info, _bcm_dpp_field_stage_idx_t stage, uint8 is_de, uint8 is_dt)
{
    uint32 cycle, bank;
    ...
    if(stage == _BCM_DPP_FIELD_STAGE_INDEX_INGRESS) {
        for(cycle = 0; cycle < ARAD_PP_FP_NOF_CYCLES; ++cycle) {
            for(bank = 0; bank < ARAD_PP_FP_TCAM_NOF_BANKS; ++bank) {
        ...
        for(egress_action = 0; egress_action < SOC_DPP_DEFS_MAX(NOF_EGRESS_PMF_ACTIONS); ++egress_action) {
            if(info->egress_action[egress_action].type == SOC_PPD_FP_ACTION_TYPE_COS_PROFILE &&
 ...
 */ 
STATIC cmd_result_t _bcm_petra_field_test_arad_pp_fp_ire_traffic_send(int unit,
                                                                      ARAD_PP_FP_PACKET *packet,
                                                                      uint8 resend)
{
    static ARAD_PP_FP_PACKET last_packet;
    uint32 soc_sand_res;
    
    if(resend) {
        soc_sand_res = arad_pp_fp_ire_traffic_send(unit, &last_packet);
    ...
    sal_memcpy(&last_packet, packet, sizeof(ARAD_PP_FP_PACKET));
...
STATIC cmd_result_t _bcm_petra_field_send_packet_and_get_information(int unit,
                                                                     uint32 buffer[_BCM_PETRA_FIELD_TEST_PACKET_SIZE_BYTES/sizeof(uint32)],
                                                                     uint32 local_port_src,
                                                                     SOC_PPD_FP_PACKET_DIAG_INFO **info_out)
{
    ARAD_PP_FP_PACKET *packet = NULL;
    uint32 index;
    uint32 soc_sand_res;
    /* NOTE: The static varible info may cause critical section problem if 2 threads will run test in the same time.
      We assume that it may not be happend. If it may be happend we should cahnge the code.
      The reason for this is stack overflow problem. The structure is very big */
    static SOC_PPD_FP_PACKET_DIAG_INFO info;
    
    packet = sal_alloc(sizeof(ARAD_PP_FP_PACKET), "_bcm_petra_field_send_packet_and_get_information.packet");
    if(!packet) {
        LOG_INFO(BSL_LS_BCM_FP,
                 (BSL_META_U(unit,
                             "Memory allocation failure.\n")));
    ...
    ARAD_PP_FP_PACKET_clear(packet);
            
    packet->local_port_src = local_port_src;
    packet->size_bytes = _BCM_PETRA_FIELD_TEST_PACKET_SIZE_BYTES;
            
    /* we want the buffer to be opposite to send it */
    for(index = 0; index < _BCM_PETRA_FIELD_TEST_PACKET_SIZE_BYTES / sizeof(uint32); ++index)
    {
        SHR_BITCOPY_RANGE(packet->buffer, index*32, buffer, _BCM_PETRA_FIELD_TEST_PACKET_SIZE_BYTES*8 - 32*(index + 1), 32);
    }
            
    if(_bcm_petra_field_test_arad_pp_fp_ire_traffic_send(unit, packet, FALSE) != CMD_OK) {
        sal_free(packet);
        _BCM_PETRA_FIELD_TEST_FAIL_MSG;
        return CMD_FAIL;
    }
    ...
    /* get the information */
    soc_sand_res = soc_ppd_fp_packet_diag_get(unit, &info);
    if(SOC_SAND_FAILURE(soc_sand_res)) {
        sal_free(packet);
        _BCM_PETRA_FIELD_TEST_FAIL_MSG;
        return CMD_FAIL;
...
/* This function take entry data of predefinded qualifiers data and mask and tranlates it to regular qualifiers
 ...
 */
STATIC bcm_error_t _bcm_petra_field_test_to_ppd_actions(int unit, _bcm_dpp_field_stage_idx_t stage, bcm_field_action_t action, SOC_PPD_FP_ACTION_TYPE **ppd_actions)
{
    _bcm_dpp_field_info_t *unitData;
    bcm_error_t rv;
                                                                ...
                                                                SOC_PPD_FP_ACTION_TYPE *ppd_actions)
{
    ...
    for(action_idx = 0; action_idx < nof_actions; ++action_idx) {
        ...
        for(ppd_action_idx = 0; ppd_action_idx < _BCM_PETRA_FIELD_ACTION_CHAIN; ++ppd_action_idx) {
            if(cur_action_ppd_ptr[ppd_action_idx] < SOC_PPD_NOF_FP_ACTION_TYPES) {
...
#if defined BCM_ARAD_SUPPORT && ! defined(PLISIM)  
STATIC uint8 actions_verification(SOC_PPD_FP_PACKET_DIAG_INFO *info,
                                  const SOC_PPD_FP_ACTION_TYPE *ppd_actions,
                                  uint32 nof_actions,
...
#endif /* defined BCM_ARAD_SUPPORT && ! defined(PLISIM) */  
                                                       ...
                                                       _bcm_petra_field_test_entry_info_t *entry)
{
    ...
    uint32 action_idx;
    SOC_PPD_FP_PACKET_DIAG_INFO* info;
    uint8 hit;
#endif
    ...
    buffer = sal_alloc(_BCM_PETRA_FIELD_TEST_PACKET_SIZE_BYTES, "_bcm_petra_field_test_traffic_test.buffer");
    if(!buffer) {
        LOG_INFO(BSL_LS_BCM_FP,
                 (BSL_META_U(unit,
                             "Memory allocation failure.\n")));
    ...
    {
        sal_memset(buffer, 0x0, _BCM_PETRA_FIELD_TEST_PACKET_SIZE_BYTES);
            ...
            if(is_predefined || !should_hit) {
                if(_BCM_PETRA_FIELD_TEST_DEBUG_CHECK_VERB) {
                    if(!should_hit) {
                        LOG_INFO(BSL_LS_BCM_FP,
                                 (BSL_META_U(unit,
                                             "Skiping the test of not hit. All the masks are 0.\n")));
                    } else {
                        LOG_INFO(BSL_LS_BCM_FP,
                                 (BSL_META_U(unit,
                                             "Skiping the test of hit. Can\'t build adequate packets.\n")));
                    }
                }
            }
            continue;
...
#ifdef BCM_ARAD_SUPPORT
        if(SOC_IS_ARAD(unit)) {
            result = _bcm_petra_field_send_packet_and_get_information(unit, buffer, 1, &info);
            if(result != CMD_OK){
                _BCM_PETRA_FIELD_TEST_FAIL_MSG;
                goto fail;
            }
            ...
            if(hit && !should_hit) {
                LOG_INFO(BSL_LS_BCM_FP,
                         (BSL_META_U(unit,
                                     "Hit occured although it shouldn\'t occur\n")));
                _BCM_PETRA_FIELD_TEST_FAIL_MSG;
                goto fail;
            ...
            if(is_dt && hit) {
                ...
                SOC_PPD_FP_ACTION_TYPE *ppd_actions = NULL;
                ...
                if(nof_actions > 0) {
                    ...
                    ppd_actions = sal_alloc(sizeof(SOC_PPD_FP_ACTION_TYPE) * nof_actions, "_bcm_petra_field_test_traffic_test.ppd_actions");
                    if(!ppd_actions) {
                        LOG_INFO(BSL_LS_BCM_FP,
                                 (BSL_META_U(unit,
                                             "Memory allocation faliure.\n")));
...
STATIC cmd_result_t _bcm_petra_field_test_get_unavailable_ports(int unit, _bcm_dpp_field_profile_type_t profile_type, uint32 unavail_ports[SOC_TMC_NOF_FAP_PORTS_MAX_IN_LONGS])
{
    uint32 profile_ports[SOC_TMC_NOF_FAP_PORTS_MAX_IN_LONGS];
    uint32 profile;
    bcm_error_t rv;

    sal_memset(unavail_ports, 0x0, SOC_TMC_NOF_FAP_PORTS_MAX_IN_LONGS * sizeof(uint32));
                
    for(profile = 1; profile < _BCM_DPP_PRESEL_NOF_PORT_PROFILES; ++profile) {
        rv = _bcm_dpp_field_presel_port_profile_get(unit, profile_type, profile, profile_ports);
        if(rv != BCM_E_NONE) {
            _BCM_PETRA_FIELD_TEST_FAIL_MSG;
            return CMD_FAIL;
        }
        SHR_BITOR_RANGE(unavail_ports, profile_ports, 0, _BCM_DPP_NOF_LOCAL_PORTS(unit), unavail_ports);
    }
                                                     ...
                                                     uint32 *mask)
{
    ...
    switch(qualifier) {
        ...
        case bcmFieldQualifyInPort:
            {
                uint32 unavail_ports[SOC_TMC_NOF_FAP_PORTS_MAX_IN_LONGS];
                result = _bcm_petra_field_test_get_unavailable_ports(unit, _bcmDppFieldProfileTypeInPort, unavail_ports);
                /* we assume that there is an available port */
                do{
                    *data = 1 + _bcm_petra_field_rand_val(x, second_rand_param++) % (_BCM_DPP_NOF_LOCAL_PORTS(unit) - 1);
                } while(SHR_BITGET(unavail_ports, *data));
            }
            *mask = 0xffffffff;
            break;
        case bcmFieldQualifyInPorts:
            {
                ...
                for(index = 0; index < _BCM_DPP_NOF_LOCAL_PORTS(unit); index += 32) {
                    if(index + 32 <= _BCM_DPP_NOF_LOCAL_PORTS(unit)) {
                        mask[index/32] = 0xffffffff;
                        data[index/32] = _bcm_petra_field_rand_val(x, second_rand_param++) & ~(unavail_ports[index/32]);
                        if(data[index/32]) {
                            have_port = TRUE;
                        }
                    } else {
                        mask[index/32] = ( 1 << (_BCM_DPP_NOF_LOCAL_PORTS(unit) - index) ) - 1;
                        data[index/32] = ( _bcm_petra_field_rand_val(x, second_rand_param++) %
                            (1 << (_BCM_DPP_NOF_LOCAL_PORTS(unit) - index)) ) 
                                & ~(unavail_ports[index/32]);
                    }
                    if(index == 0) {
                        /* don't include port 0 */
                        data[index/32] &= ~1;
                    }
                    if(data[index/32]) {
                        have_port = TRUE;
                ...
                if(!have_port) {
                    ...
                    do{
                        index = 1 + _bcm_petra_field_rand_val(x, second_rand_param++) % (_BCM_DPP_NOF_LOCAL_PORTS(unit) - 1);
                    } while(SHR_BITGET(unavail_ports, index));
                    SHR_BITSET(data, index);
                }
            }
            break;
        case bcmFieldQualifyOutPort:
            {
                uint32 unavail_ports[SOC_TMC_NOF_FAP_PORTS_MAX_IN_LONGS];
                result = _bcm_petra_field_test_get_unavailable_ports(unit, _bcmDppFieldProfileTypeOutPort, unavail_ports);
                /* we assume that there is an available port */
                do{
                    *data = 1 + _bcm_petra_field_rand_val(x, second_rand_param++) % (_BCM_DPP_NOF_LOCAL_PORTS(unit) - 1);
                } while(SHR_BITGET(unavail_ports, *data));
...
STATIC cmd_result_t _bcm_petra_field_test_set_entry_qualifiers(int unit, _bcm_dpp_field_stage_idx_t stage, CONST bcm_field_qset_t *qset, uint8 with_traffic, uint32 x, uint8 is_scan_mode, _bcm_petra_field_test_entry_info_t *entry)
{
    bcm_error_t rv;
    cmd_result_t result;
    uint32 second_rand_param = 0;
                    ...
                    if(qualify == bcmFieldQualifyInPorts) {
                        for(qual_idx_2 = 0; qual_idx_2 < qual_index; ++qual_idx_2) {
                            if(entry->quals_info[qual_idx_2].is_valid
                                && entry->quals_info[qual_idx_2].qualify == bcmFieldQualifyInPort){
                                entry->quals_info[qual_index].is_valid = FALSE;
                                break;
                            }
                        }
    ...
    for(; qual_index < _BCM_PETRA_FIELD_TEST_MAX_QUALIFIERS_IN_ENTRY; ++qual_index) {
        entry->quals_info[qual_index].qualify = bcmFieldQualifyCount +1;/* i.e invalid qualify */
        entry->quals_info[qual_index].is_valid = FALSE;
    }
...
STATIC cmd_result_t _bcm_petra_field_test_set_entry_data_qualifiers(int unit, _bcm_dpp_field_stage_idx_t stage, bcm_field_qset_t *qset, uint8 with_traffic, uint32 x, uint8 is_scan_mode, _bcm_petra_field_test_entry_info_t *entry)
{
    ...
    int qual_arr[_BCM_PETRA_FIELD_TEST_MAX_DATA_QUALIFIER_ID + 1];
    uint8 can_be_valid[_BCM_PETRA_FIELD_TEST_MAX_DATA_QUALIFIER_ID + 1];
    int qual_count;
    bcm_field_data_qualifier_t dq[_BCM_PETRA_FIELD_TEST_MAX_DATA_QUALIFIER_ID + 1];
    uint32 qual_length;
...
static const bcm_field_aset_t* get_set_get_action_failure(void){
    const static bcm_field_action_t set_get_action_failure[] = {
        ...
        /* This action causes failure in the entry instalation*/
...
STATIC cmd_result_t _bcm_petra_field_test_set_entry_actions(int unit, _bcm_dpp_field_stage_idx_t stage, bcm_field_aset_t *aset, uint8 with_traffic, uint8 is_scan_mode, uint32 x, _bcm_petra_field_test_entry_info_t *entry)
{
    bcm_error_t rv;
    cmd_result_t result;
    bcm_field_action_t action;
    ...
    for(action = 0; action < bcmFieldActionCount; ++action) {
        if(BCM_FIELD_ASET_TEST(*aset, action)) {
            ...
            if((stage == _BCM_DPP_FIELD_STAGE_INDEX_EGRESS || is_scan_mode) && with_traffic) {
...
STATIC cmd_result_t _bcm_petra_field_entry_scan_test(int unit, _bcm_dpp_field_stage_idx_t stage, bcm_field_group_config_t *field_group, uint8 with_traffic, uint8 is_predefined, uint8 do_warmboot)
{
    ...
    if(is_predefined) {
        ...
        result = _bcm_petra_field_test_set_entry_qualifiers(unit, stage, &field_group->qset, with_traffic, sal_rand(), TRUE, entry);
        if(result != CMD_OK) {
            _BCM_PETRA_FIELD_TEST_FAIL_MSG;
            goto fail;
        }
    ...
    result = _bcm_petra_field_test_set_entry_actions(unit, stage, &field_group->aset, with_traffic, TRUE, sal_rand(), entry);
    if(result != CMD_OK) {
        _BCM_PETRA_FIELD_TEST_FAIL_MSG;
        goto fail;
    }
    ...
    if(with_traffic) {
        result = _bcm_petra_field_test_traffic_test(unit, stage, is_predefined, FALSE, (field_group->mode == bcmFieldGroupModeDirect), field_group->group, entry);
        if(result != CMD_OK) {
            _BCM_PETRA_FIELD_TEST_FAIL_MSG;
            goto fail;
        }
                                                          ...
                                                          uint32 nof_presels)
{
    ...
    for(presel_idx = 0;
        ...
        ++presel_idx) {
        rv = bcm_field_qualify_AppType(unit, (presel_idx + 3) | BCM_FIELD_QUALIFY_PRESEL, supported_app_type[presel_idx]);
        if(rv != BCM_E_NONE) {
            _BCM_PETRA_FIELD_TEST_FAIL_MSG;
            goto fail;
        }
    ...
    if(presel_idx < (sizeof(supported_app_type) / sizeof(supported_app_type[0])) ){
        for(presel_idx_2 = --presel_idx;
            ...
            ++presel_idx_2) {
            rv = bcm_field_qualify_AppType(unit, (presel_idx + 3) | BCM_FIELD_QUALIFY_PRESEL, supported_app_type[presel_idx_2]);
            if(rv != BCM_E_NONE) {
                _BCM_PETRA_FIELD_TEST_FAIL_MSG;
                goto fail;
            }
 ...
 * In the field group test it creates field grouop with one qualifier and one action install, retrieve and destroy the field group
 * For the entry test it chooses randomally data and mask for the qualifier, randomally param0 and param1 for the action, and retrieves them
...
STATIC cmd_result_t _bcm_petra_field_test_field_group_and_entry_scan(int unit, _bcm_dpp_field_stage_idx_t stage, uint8 is_direct_table, uint8 entry_also, uint8 with_traffic, uint8 is_predefined, uint8 do_warmboot)
{
    ...
    return CMD_OK;
#if (defined (BCM_ARAD_SUPPORT) && defined(BCM_WARM_BOOT_SUPPORT)) || (defined(INCLUDE_KBP) && !defined(BCM_88030))
fail:
    return CMD_FAIL;
#endif /* (defined (BCM_ARAD_SUPPORT) && defined(BCM_WARM_BOOT_SUPPORT)) || (defined(INCLUDE_KBP) && !defined(BCM_88030)) */
}
                                                                   ...
                                                                   SOC_PPD_FP_PACKET_DIAG_INFO **info)
{
    ...
    sal_memset(buffer, 0xa5, sizeof(buffer));
    SHR_BITCOPY_RANGE(
        buffer,
        _BCM_PETRA_FIELD_TEST_PACKET_SIZE_BYTES*8 -
            (112 + 32*ethernet_header->nof_vlans) /* ethernet header length */,
    ...
    return _bcm_petra_field_send_packet_and_get_information(unit, buffer, local_port_src, info);
}
#endif

/*  
 * Assumptions: 
 * 1. each aset in the field group has no more then 2 actions, and the number of the actions in all of the field groups is no nmore then 32 
 * 2. each qset in the field group has no more then 6 qualifiers, and the number of the qualifiers in all of the field groups is no nmore then 24 
...
STATIC cmd_result_t _bcm_petra_field_test_field_group_tag(int unit, _bcm_dpp_field_stage_idx_t stage, uint32 nof_fgs, uint32 x, _bcmPetraFieldTestSpeedMode mode, uint8 with_destroy, uint8 with_traffic, uint8 has_de, uint8 do_warmboot)
{
    ...
    /* These varible may be set only once. Done to avoid coverity stack use.*/
    static bcm_field_qset_t qset;
    static bcm_field_aset_t aset;
    static uint8 is_qset_and_ast_set = FALSE;
    
    /* We need qset_verify only for checking the existance of field group. We don't use this varible */
    ...
    SOC_PPD_FP_ACTION_TYPE ppd_actions[_BCM_PETRA_FIELD_TEST_FIELD_GROUP_DESTROY_WITH_TRAFFIC_NOF_ACTIONS];
    ...
    for(iteration_idx = 0; iteration_idx < nof_iterations; ++iteration_idx) { 
        ...
        if(with_destroy && with_traffic) {
            ...
            for(app_type_idx = 0; app_type_idx < _BCM_PETRA_FIELD_TEST_NOF_APP_TYPES_MAX; ++app_type_idx) {
                ...
                result = _bcm_petra_field_test_fill_field_groups(unit, stage, x + second_rand_param++, FALSE, FALSE, 0, _BCM_PETRA_FIELD_TEST_FIELD_GROUP_BANK_MAX_SIZE, field_groups[app_type_idx], nof_fgs);
                if(result != CMD_OK) {
                    if(_BCM_PETRA_FIELD_TEST_DEBUG_CHECK_VERB) {
                        LOG_INFO(BSL_LS_BCM_FP,
                                 (BSL_META_U(unit,
        ...
        for(app_type_idx = 0; app_type_idx < _BCM_PETRA_FIELD_TEST_NOF_APP_TYPES_MAX; ++app_type_idx) {
            ...
            for(fg_idx = 0; fg_idx < nof_fgs; ++fg_idx) {
                ...
                /* verifying no re use of previos priority */
                if(app_type_idx > 0) {
                    do{
                        uint32 fg_idx3;
        ...
        if(with_destroy) {
            /* creat many field groups till there is no place to add another field group */
            for(fg_idx = 0; fg_idx < _BCM_FIELD_GROUP_TEST_NOF_GROUPS_DESTROY; ++fg_idx) {
                do{
                    does_exist_already = FALSE;
            ...
            /* Taking existing group and check the retrived atributtes */
            if(stage == _BCM_DPP_FIELD_STAGE_INDEX_ING_EXTERNAL) {
        ...
        if(with_destroy) {
            for(fg_idx = 0; fg_idx < _BCM_FIELD_GROUP_TEST_NOF_GROUPS_DESTROY; ++fg_idx) {
                bcm_field_group_destroy(unit, destroy_fg_ids[fg_idx]);
            }
        }
...
cmd_result_t _bcm_petra_field_test_field_group_destroy_with_traffic(int unit, _bcm_dpp_field_stage_idx_t stage, uint32 x, _bcmPetraFieldTestSpeedMode mode, uint8 do_warmboot) {
    if(stage != _BCM_DPP_FIELD_STAGE_INDEX_INGRESS) {
        LOG_INFO(BSL_LS_BCM_FP,
                 (BSL_META_U(unit,
                             "The test is valid only for the ingerss stage!\n")));
        return CMD_FAIL;
    }
    ...
    return _bcm_petra_field_test_field_group_tag(unit, stage, _BCM_PETRA_FIELD_TEST_NOF_GROUPS_ID(stage), x, mode, TRUE, TRUE, FALSE, do_warmboot);
}
...
#ifdef BCM_ARAD_SUPPORT
    db_id_ndx = unitData->groupD[group].hwHandle;
    if(SOC_IS_ARAD(unit)) {
        tcam_db_id = ARAD_PP_FP_DB_ID_TO_TCAM_DB(db_id_ndx);
        soc_sand_res = arad_sw_db_tcam_db_entry_size_get(unit, tcam_db_id, &entry_size);
        if(SOC_SAND_FAILURE(soc_sand_res)) {
            _BCM_PETRA_FIELD_TEST_FAIL_MSG;
        ...
        LOG_INFO(BSL_LS_BCM_FP,
                 (BSL_META_U(unit,
                             "This function is not supported for this unit.\n")));
        return BCM_E_UNAVAIL;
#ifdef BCM_ARAD_SUPPORT
    }
#endif /* BCM_ARAD_SUPPORT */
 ...
 * it create field groups. Then fill's randomally entries and retrieves and compare some of them choosed randomally.
 * If we have traffic only one entry is created, and according to the entry calls to the traffic test. */
STATIC cmd_result_t _bcm_petra_field_test_entry_and_traffic(int unit, _bcm_dpp_field_stage_idx_t stage, uint32 x, _bcmPetraFieldTestSpeedMode mode, uint8 with_traffic, uint8 is_direct_table, uint8 do_warmboot)
{
    uint32 nof_iterations;
    uint32 iteration_idx;
    uint32 fg_idx, fg_idx2;
    ...
    for(iteration_idx = 0; iteration_idx < nof_iterations ; ++iteration_idx) {
        ...
        if(is_direct_table) {
            ...
            result = _bcm_petra_field_test_fill_field_groups(unit, stage, x + second_rand_param++, FALSE, FALSE, 0, _BCM_PETRA_FIELD_TEST_FIELD_GROUP_BANK_MAX_SIZE, field_groups, _BCM_PETRA_FIELD_TEST_ENTRY_NOF_GROUPS_ID(stage));
            if(stage == _BCM_DPP_FIELD_STAGE_INDEX_ING_EXTERNAL) {
                for(fg_idx = 0; fg_idx < _BCM_PETRA_FIELD_TEST_ENTRY_NOF_GROUPS_ID(stage); ++fg_idx) {
                    BCM_FIELD_PRESEL_INIT(field_groups[fg_idx].preselset);
        ...
        for(fg_idx = 0; fg_idx < _BCM_PETRA_FIELD_TEST_ENTRY_NOF_GROUPS_ID(stage); ++fg_idx) {
            ...
            for(entry_idx = 0; entry_idx < nof_entries; ++entry_idx){
                ...
                /* set the qualifiers */
                result = _bcm_petra_field_test_set_entry_qualifiers(unit, stage, &field_groups[fg_idx].qset, with_traffic, x + second_rand_param++, FALSE, &entries[fg_idx][entry_idx]);
                if(result != CMD_OK) {
                    _BCM_PETRA_FIELD_TEST_FAIL_MSG;
                    goto fail;
                }
                ...
                if(is_direct_table) {
                    ...
                    /* if the entry is emtry - remove it but not in the traffic test. In the traffic test don't remove */
                    if(!with_traffic) {
                    ...
                    for(entry_idx2 = 0; entry_idx2 < entry_idx; ++entry_idx2) {
                        if(can_be_inserted) {
                            if(entries_have_intersection(&entries[fg_idx][entry_idx], &entries[fg_idx][entry_idx2])) {
                                ...
                                for(qual_index = 0; qual_index < _BCM_PETRA_FIELD_TEST_MAX_QUALIFIERS_IN_ENTRY; ++qual_index) {
                                    entries[fg_idx][entry_idx].quals_info[qual_index].qualify = bcmFieldQualifyCount +1;/* i.e invalid qualify */
                ...
                result = _bcm_petra_field_test_set_entry_actions(unit, stage, &field_groups[fg_idx].aset, with_traffic, FALSE, x + second_rand_param++, &entries[fg_idx][entry_idx]);
                if(result != CMD_OK) {
                    _BCM_PETRA_FIELD_TEST_FAIL_MSG;
                    goto fail;
                }
        ...
        if(!with_traffic) {
            ...
            /* traffic test - we have only 1 entry */
                for(fg_idx = 0; fg_idx < _BCM_PETRA_FIELD_TEST_ENTRY_NOF_GROUPS_ID(stage); ++fg_idx) {
                    result = _bcm_petra_field_test_traffic_test(unit, stage, FALSE, FALSE, is_direct_table, field_groups[fg_idx].group, &entries[fg_idx][0]);
                    if(result != CMD_OK) {
                        _BCM_PETRA_FIELD_TEST_FAIL_MSG;
                        goto fail;
                }
...
cmd_result_t _bcm_petra_field_test_entry(int unit, _bcm_dpp_field_stage_idx_t stage, uint32 x, _bcmPetraFieldTestSpeedMode mode, uint8 do_warmboot)
{
    ...
    result = _bcm_petra_field_test_entry_and_traffic(unit, stage, x, mode, FALSE, FALSE, do_warmboot);
...
cmd_result_t _bcm_petra_field_test_entry_traffic(int unit, _bcm_dpp_field_stage_idx_t stage, uint32 x, _bcmPetraFieldTestSpeedMode mode, uint8 do_warmboot)
{
    ...
    result = _bcm_petra_field_test_entry_and_traffic(unit, stage, x, mode, TRUE, FALSE, do_warmboot);
    if(result == CMD_OK) {
        LOG_INFO(BSL_LS_BCM_FP,
                 (BSL_META_U(unit,
                             "The test finished successfully\n")));
...
/* This function indicates the index of the entry in witch the hit should occour */
 ...
 * it takes randomally value that may hit at leaast in one of the entries and check if the hit occoured in the right entry
 */
    ...
    for(iteration_idx = 0; iteration_idx < nof_iterations; ++iteration_idx) { 
        ...
        if(nof_fgs > 1) {
            if(stage == _BCM_DPP_FIELD_STAGE_INDEX_INGRESS) {
                ...
                rv = bcm_field_qualify_HeaderFormat(unit, 0 | BCM_FIELD_QUALIFY_PRESEL, bcmFieldHeaderFormatL2);
                if(rv != BCM_E_NONE) {
                    _BCM_PETRA_FIELD_TEST_FAIL_MSG;
                    goto destroy_presels_only;
                }
                ...
                rv = bcm_field_qualify_HeaderFormat(unit, 1 | BCM_FIELD_QUALIFY_PRESEL, bcmFieldHeaderFormatIp4);
                if(rv != BCM_E_NONE) {
                    _BCM_PETRA_FIELD_TEST_FAIL_MSG;
                    goto destroy_presels_only;
                }
                ...
                rv = bcm_field_qualify_HeaderFormat(unit, 2 | BCM_FIELD_QUALIFY_PRESEL, bcmFieldHeaderFormatIp6);
                if(rv != BCM_E_NONE) {
                    _BCM_PETRA_FIELD_TEST_FAIL_MSG;
                    goto destroy_presels_only;
                }
        ...
        for(entry_idx = 0; entry_idx < nof_entries*nof_fgs; ++entry_idx){
            ...
            for(qual_idx = 1; qual_idx < _BCM_PETRA_FIELD_TEST_MAX_QUALIFIERS_IN_ENTRY; ++qual_idx) {
                entries[fg_idx][cur_entry_idx[fg_idx]].quals_info[qual_idx].qualify = bcmFieldQualifyCount + 1;/* i.e invalid qualify */
    	                    ...
    	                    for(bank = 0; bank < ARAD_PP_FP_TCAM_NOF_BANKS; ++bank) {
    	                        if(info->tcam[cycle][bank].is_match) {
    	                            if(info->tcam[cycle][bank].actions[0].val != 
    	                                ...
    	                                LOG_INFO(BSL_LS_BCM_FP,
                                                 (BSL_META_U(unit,
                                                             "Hit ocuured in the wrong entry\n")));
    	                                _BCM_PETRA_FIELD_TEST_FAIL_MSG;
    	                                goto fail;
    	                            } else {
    	                                success = TRUE;
...
STATIC cmd_result_t _bcm_petra_field_test_field_group_priority_tag(int unit, uint32 x, _bcmPetraFieldTestSpeedMode mode, uint8 do_warmboot)
{
    ...
    SOC_PPD_FP_ACTION_TYPE ppd_actions[_BCM_PETRA_FIELD_TEST_FIELD_GROUP_PRIORITY_NOF_ACTIONS];
...
cmd_result_t _bcm_petra_field_test_cascaded(int unit, _bcm_dpp_field_stage_idx_t stage, uint32 x, _bcmPetraFieldTestSpeedMode mode, uint8 do_warmboot)
{
    ...
    for(iteration_idx = 0; iteration_idx < nof_iterations; ++iteration_idx) { 
        ...
        /* Take another 3 actions randomally */
                ...
                for(cycle = 0; cycle < ARAD_PP_FP_NOF_CYCLES; ++cycle) {
                    for(bank = 0; bank < ARAD_PP_FP_TCAM_NOF_BANKS; ++bank) {
                        if(info->tcam[cycle][bank].is_match) {
                            LOG_INFO(BSL_LS_BCM_FP,
                                     (BSL_META_U(unit,
                                                 "Hit occured altoght it shouldn\'t occour\n")));
                            _BCM_PETRA_FIELD_TEST_FAIL_MSG;
                            goto fail;
                        }
                    }
                    ...
                    for(bank = 0; bank < ARAD_PP_FP_TCAM_NOF_BANKS; ++bank) {
                        if(info->tcam[cycle][bank].is_match) {
                            for(action_idx = 0; action_idx < ARAD_PP_FP_NOF_ACTIONS_PER_DB_MAX; ++action_idx) {
                                if(info->tcam[cycle][bank].actions[action_idx].type != SOC_PPD_FP_ACTION_TYPE_INVALID){
                                    ++nof_hits;
                                    if(info->tcam[cycle][bank].actions[action_idx].type == SOC_PPD_FP_ACTION_TYPE_CHANGE_KEY && 
                                    ...
                                    if(info->tcam[cycle][bank].actions[action_idx].type == SOC_PPD_FP_ACTION_TYPE_LEARN_FID && 
 ...
 * It fills randomally prisels and retrieves and compare some of them choosed randomally. 
 */
    ...
    for(iteration_idx = 0; iteration_idx < nof_iterations; ++iteration_idx) { 
        ...
        if(mode != _bcmPetraFieldTestScan) {
            for(presel_idx = 0; presel_idx < _BCM_PETRA_FIELD_TEST_NOF_PRESELS; ++presel_idx){
                ...
                result = _bcm_petra_field_test_set_entry_qualifiers(unit, stage, &valid_qset, FALSE, x + second_rand_param++, FALSE, &presels[presel_idx]);
                if(result != CMD_OK) {
                    _BCM_PETRA_FIELD_TEST_FAIL_MSG;
                    goto fail;
                }
            ...
            for(qualify = 0; qualify < bcmFieldQualifyCount; ++qualify) {
                ...
                result = _bcm_petra_field_test_set_entry_qualifiers(unit, stage, &qset, FALSE, sal_rand(), TRUE, &presels[0]);
                if(result != CMD_OK) {
                    _BCM_PETRA_FIELD_TEST_FAIL_MSG;
                    goto fail;
                }
                ...
                rv = bcm_field_presel_destroy(unit, presels[0].id & (~BCM_FIELD_QUALIFY_PRESEL));
                if(rv != BCM_E_NONE) {
                    _BCM_PETRA_FIELD_TEST_FAIL_MSG;
                    goto fail;
                }
...
fail:
    if(nof_iterations > 1) {
        for(presel_idx = 0; presel_idx < _BCM_PETRA_FIELD_TEST_NOF_PRESELS; ++presel_idx) {
            bcm_field_presel_destroy(unit, presels[presel_idx].id & (~BCM_FIELD_QUALIFY_PRESEL));
        }
    }
...
cmd_result_t _bcm_petra_field_test_field_group_presel(int unit, _bcm_dpp_field_stage_idx_t stage, uint32 x, _bcmPetraFieldTestSpeedMode mode, uint8 do_warmboot)
{
    ...
    SOC_PPD_FP_ACTION_TYPE ppd_actions[3];
    _bcm_petra_field_test_ethernet_header_t ethernet_header;
    uint32 unavail_ports[SOC_TMC_NOF_FAP_PORTS_MAX_IN_LONGS];
    ...
    if(stage == _BCM_DPP_FIELD_STAGE_INDEX_INGRESS) {
        ...
        for(cur_avail_port = 0; cur_avail_port < _BCM_DPP_NOF_LOCAL_PORTS(unit); ++cur_avail_port) {
            if(!SHR_BITGET(unavail_ports, cur_avail_port)) {
                break;
            }
        }
        rv = bcm_field_qualify_OutPort(unit, 0 | BCM_FIELD_QUALIFY_PRESEL, cur_avail_port, 0xffffffff);
    }
    if(rv != BCM_E_NONE) {
        _BCM_PETRA_FIELD_TEST_FAIL_MSG;
        goto destroy_presels_only;
    ...
    if(stage == _BCM_DPP_FIELD_STAGE_INDEX_INGRESS) {
        ...
        for(; cur_avail_port < _BCM_DPP_NOF_LOCAL_PORTS(unit); ++cur_avail_port) {
            if(!SHR_BITGET(unavail_ports, cur_avail_port)) {
                BCM_PBMP_PORT_ADD(data, cur_avail_port);
                break;
            }
        }
        for(; cur_avail_port < _BCM_DPP_NOF_LOCAL_PORTS(unit); ++cur_avail_port) {
        ...
        rv = bcm_field_qualify_OutPorts(unit, 1 | BCM_FIELD_QUALIFY_PRESEL, data, mask);
    }
    if(rv != BCM_E_NONE) {
        _BCM_PETRA_FIELD_TEST_FAIL_MSG;
        goto destroy_presels_only;
...
#ifdef BCM_ARAD_SUPPORT        
    if(SOC_IS_ARAD(unit)) {
        SOC_PPD_FP_PACKET_DIAG_INFO *info = NULL;
        uint8 success;
...
#ifndef PLISIM
        {
            ...
            /* action verification - actions[0] and action[2] should be occured*/
            for(cycle = ARAD_PP_FP_NOF_CYCLES - 1; cycle >= 0; --cycle){
                for(macro = _BCM_PETRA_FIELD_TEST_PP_FP_NOF_MACRO_SIMPLES - 1; macro >= 0; --macro) {
                    if(info->macro_simple[cycle][macro].action.type != SOC_PPD_FP_ACTION_TYPE_INVALID) {
                        if(info->macro_simple[cycle][macro].action.type == ppd_actions[0]) {
                            if(info->macro_simple[cycle][macro].action.val ==  1) {
                                ...
                                LOG_INFO(BSL_LS_BCM_FP,
                                         (BSL_META_U(unit,
                                                     "The action of D.B. 0 occured with a wrong value\n")));
                                _BCM_PETRA_FIELD_TEST_FAIL_MSG;
                                goto fail;
                            }
                            ...
                            if(info->macro_simple[cycle][macro].action.val ==  1) {
                                ...
                                LOG_INFO(BSL_LS_BCM_FP,
                                         (BSL_META_U(unit,
                                                     "The action of D.B. 2 occured with a wrong value\n")));
                                _BCM_PETRA_FIELD_TEST_FAIL_MSG;
                                goto fail;
                            }
                        } else {
    ...
    if(SOC_IS_ARAD(unit)) {
...
#ifndef PLISIM
        {
            ...
            /* action verification - actions[1] and action[2] should be occured*/
            for(cycle = ARAD_PP_FP_NOF_CYCLES - 1; cycle >= 0; --cycle){
                for(macro = _BCM_PETRA_FIELD_TEST_PP_FP_NOF_MACRO_SIMPLES - 1; macro >= 0; --macro) {
                    if(info->macro_simple[cycle][macro].action.type != SOC_PPD_FP_ACTION_TYPE_INVALID) {
                        if(info->macro_simple[cycle][macro].action.type == ppd_actions[1]) {
                            if(info->macro_simple[cycle][macro].action.val ==  1) {
                                ...
                                LOG_INFO(BSL_LS_BCM_FP,
                                         (BSL_META_U(unit,
                                                     "The action of D.B. 1 occured with a wrong value\n")));
                                _BCM_PETRA_FIELD_TEST_FAIL_MSG;
                                goto fail;
                            }
                        } else if(info->macro_simple[cycle][macro].action.type == ppd_actions[2]) {
    ...
    rv = bcm_field_presel_destroy(unit, 0);
    if(rv != BCM_E_NONE) {
        _BCM_PETRA_FIELD_TEST_FAIL_MSG;
        result = CMD_FAIL;
    }
    rv = bcm_field_presel_destroy(unit, 1);
    if(rv != BCM_E_NONE) {
        _BCM_PETRA_FIELD_TEST_FAIL_MSG;
        result = CMD_FAIL;
    }
...
cmd_result_t _bcm_petra_field_test_field_group_presel_1(int unit, _bcm_dpp_field_stage_idx_t stage, uint32 x, _bcmPetraFieldTestSpeedMode mode, uint8 do_warmboot)
{
    ...
    SOC_PPD_FP_ACTION_TYPE ppd_actions[_BCM_PETRA_FIELD_TEST_DB_PRESEL_1_NOF_DBS];
    _bcm_petra_field_test_ethernet_header_t ethernet_header;
    uint8 does_exist_already;
    CONST uint32 local_port_src = 1;
    ...
    for(iteration_idx = 0; iteration_idx < nof_iterations; ++iteration_idx) { 
        ...
        for(cur_port = 1; cur_port < _BCM_DPP_NOF_LOCAL_PORTS(unit); ++cur_port) {
            if(!SHR_BITGET(unavail_ports, cur_port)) {
        ...
        /* creating half of the presels randomally */
        ...
        for(presel_id = 0; presel_id < _BCM_PETRA_FIELD_TEST_DB_PRESEL_1_NOF_PRESELS/2; ++presel_id) {
            ...
            if(stage == _BCM_DPP_FIELD_STAGE_INDEX_INGRESS) {
                    ...
                    switch(rnd){
                        case 0:
                            rv = bcm_field_qualify_HeaderFormat(unit, presel_id | BCM_FIELD_QUALIFY_PRESEL, bcmFieldHeaderFormatL2);
                            break;
                        case 1:
                            rv = bcm_field_qualify_HeaderFormat(unit, presel_id | BCM_FIELD_QUALIFY_PRESEL, bcmFieldHeaderFormatIp4);
                            break;
                        case 2:
                            rv = bcm_field_qualify_HeaderFormat(unit, presel_id | BCM_FIELD_QUALIFY_PRESEL, bcmFieldHeaderFormatIp6);
                            break;
                        default:;
                    }
                    if(rv != BCM_E_NONE) {
                ...
                rv = bcm_field_qualify_OutPort(unit, presel_id | BCM_FIELD_QUALIFY_PRESEL, out_ports[out_ports_idx], 0xffffffff);
                if(rv != BCM_E_NONE) {
                    _BCM_PETRA_FIELD_TEST_FAIL_MSG;
                    goto destroy_presels_only;
                }
                                    ...
                                    for(action_idx = 0; action_idx < _BCM_PETRA_FIELD_TEST_DB_PRESEL_1_NOF_DBS; ++action_idx) {
                                        if(info->macro_simple[cycle][macro].action.type == ppd_actions[action_idx]) {
                                            if(info->macro_simple[cycle][macro].action.val ==  1) {
                                                ...
                                                LOG_INFO(BSL_LS_BCM_FP,
                                                         (BSL_META_U(unit,
                                                                     "The action of D.B. %d occured with a wrong value\n"), action_idx));
                                                _BCM_PETRA_FIELD_TEST_FAIL_MSG;
                                                goto fail;
                                            }
                                        }
        ...
        /* dfestroying the residual presels */
        for(presel_id = 0; presel_id < _BCM_PETRA_FIELD_TEST_DB_PRESEL_1_NOF_PRESELS/2; ++presel_id) {
            rv = bcm_field_presel_destroy(unit, presel_id);
            if(rv != BCM_E_NONE) {
                _BCM_PETRA_FIELD_TEST_FAIL_MSG;
...
STATIC cmd_result_t _bcm_petra_field_test_data_qualifiers_scan(int unit, _bcm_dpp_field_stage_idx_t stage)
{
    ...
    for(cur_qualify = 0; cur_qualify < bcmFieldQualifyCount; ++cur_qualify) {
        ...
        if(dq2.qual_id != dq_verify.qual_id) {
            LOG_INFO(BSL_LS_BCM_FP,
                     (BSL_META_U(unit,
                                 "Retriveing failure in qual_id. set %d, get %d\n"), dq2.qual_id, dq_verify.qual_id));
            bcm_field_data_qualifier_destroy(unit, 0);
        ...
        if(dq2.offset_base != dq_verify.offset_base) {
            LOG_INFO(BSL_LS_BCM_FP,
                     (BSL_META_U(unit,
                                 "Retriveing failure in offset_base. set %s, get %s\n"), 
        ...
        if(dq2.offset != dq_verify.offset) {
            LOG_INFO(BSL_LS_BCM_FP,
                     (BSL_META_U(unit,
                                 "Retriveing failure in offset. set %d, get %d\n"), dq2.offset, dq_verify.offset));
            bcm_field_data_qualifier_destroy(unit, 0);
            bcm_field_data_qualifier_destroy(unit, dq2.qual_id);
            _BCM_PETRA_FIELD_TEST_FAIL_MSG;
            return CMD_FAIL;
        ...
        if(dq2.length != dq_verify.length) {
            LOG_INFO(BSL_LS_BCM_FP,
                     (BSL_META_U(unit,
                                 "Retriveing failure in length. set %d, get %d\n"), dq2.length, dq_verify.length));
            bcm_field_data_qualifier_destroy(unit, 0);
            bcm_field_data_qualifier_destroy(unit, dq2.qual_id);
            _BCM_PETRA_FIELD_TEST_FAIL_MSG;
            return CMD_FAIL;
        ...
        if(dq2.stage != dq_verify.stage) {
            LOG_INFO(BSL_LS_BCM_FP,
                     (BSL_META_U(unit,
                                 "Retriveing failure in stage. set %s, get %s\n"),
    ...
    for(iteration_idx = 0; iteration_idx < nof_iterations; ++iteration_idx) { 
        ...
        if(rnd) {
            ...
            dq2.offset = _bcm_petra_field_rand_val(x, second_rand_param++) % 
                (_BCM_PETRA_FIELD_TEST_DATA_QUALIFIER_MAX_OFFSET + 1) / 8;
...
STATIC cmd_result_t _bcm_petra_field_test_create_data_qualifiers(int unit, _bcm_dpp_field_stage_idx_t stage, uint32 x, uint8 predefined_also, uint8 predefined_only, uint8 is_de_large)
{
        ...
        for(qualify = 0; qualify < bcmFieldQualifyCount; ++qualify) {
            if(BCM_FIELD_QSET_TEST(*_bcm_petra_field_test_supported_qualifiers_for_data_qualifier(unit, stage), qualify)){
                if(is_de_large) {
                    ...
                    /* remove qualifier whose length is samller than 4 */
    ...
    for(qual_idx = 0; qual_idx <= _BCM_PETRA_FIELD_TEST_MAX_DATA_QUALIFIER_ID; ++qual_idx) {
        ...
        if(predefined_only ||
            ...
            if(dq[qual_idx].offset_base == bcmFieldDataOffsetBaseThirdHeader) {
                ...
                dq[qual_idx].offset = _bcm_petra_field_rand_val(x, second_rand_param++) % 
                    ((_BCM_PETRA_FIELD_TEST_DATA_QUALIFIER_MAX_OFFSET + 1) / 8 - 1) + 1;
            } else {
            dq[qual_idx].offset = _bcm_petra_field_rand_val(x, second_rand_param++) % 
                (_BCM_PETRA_FIELD_TEST_DATA_QUALIFIER_MAX_OFFSET + 1) / 8;
            }
...
STATIC cmd_result_t _bcm_petra_field_test_data_qualifiers_entry_and_traffic(int unit, _bcm_dpp_field_stage_idx_t stage, uint32 x, _bcmPetraFieldTestSpeedMode mode, uint8 with_traffic, uint8 do_warmboot)
{
    ...
    for(iteration_idx = 0; iteration_idx < nof_iterations; ++iteration_idx) { 
...
#endif
#endif
        result = _bcm_petra_field_test_fill_field_groups(unit, stage, x + second_rand_param++, TRUE, FALSE, 0, _BCM_PETRA_FIELD_TEST_FIELD_GROUP_BANK_MAX_SIZE, field_groups, _BCM_PETRA_FIELD_TEST_ENTRY_NOF_GROUPS_ID(stage));
        if(result != CMD_OK) {
            if(_BCM_PETRA_FIELD_TEST_DEBUG_CHECK_VERB) {
                LOG_INFO(BSL_LS_BCM_FP,
                         (BSL_META_U(unit,
        ...
        if(!with_traffic) {
            ...
            /* traffic test - we have only 1 entry */
            for(fg_idx = 0; fg_idx < _BCM_PETRA_FIELD_TEST_ENTRY_NOF_GROUPS_ID(stage); ++fg_idx) {
                result = _bcm_petra_field_test_traffic_test(unit, stage, FALSE, FALSE, FALSE, field_groups[fg_idx].group, &entries[fg_idx][0]);
                if(result != CMD_OK) {
                    _BCM_PETRA_FIELD_TEST_FAIL_MSG;
                    goto fail;
                }
        ...
        if(stage != _BCM_DPP_FIELD_STAGE_INDEX_ING_EXTERNAL) {
            for(qual_id = 0; qual_id <= _BCM_PETRA_FIELD_TEST_MAX_DATA_QUALIFIER_ID; ++qual_id) {
...
/* There in no nned to test of the predefined data qualifiers without entries traffic - the test of the data qualifiers with entries tests also predefined data qualifiers */
cmd_result_t _bcm_petra_field_test_predefined_data_qualifiers_entry_traffic(int unit, _bcm_dpp_field_stage_idx_t stage, uint32 x, _bcmPetraFieldTestSpeedMode mode, uint8 do_warmboot)
{
    ...
    dq = sal_alloc(sizeof(bcm_field_data_qualifier_t) * (_BCM_PETRA_FIELD_TEST_MAX_DATA_QUALIFIER_ID + 1), "_bcm_petra_field_test_predefined_data_qualifiers_entry_traffic.dq");
    if(!dq) {
        LOG_INFO(BSL_LS_BCM_FP,
                 (BSL_META_U(unit,
                             "Memory allocation failure.\n")));
    ...
    for(iteration_idx = 0; iteration_idx < nof_iterations; ++iteration_idx) { 
        ...
        if(nof_qualifiers > _BCM_PETRA_FIELD_TEST_MAX_DATA_QUALIFIER_ID + 1) {
            nof_qualifiers = _BCM_PETRA_FIELD_TEST_MAX_DATA_QUALIFIER_ID + 1;
        }
        ...
        if(stage == _BCM_DPP_FIELD_STAGE_INDEX_EGRESS) {
            ...
            result = _bcm_petra_field_test_fill_field_groups(unit, stage, x + second_rand_param++, TRUE, FALSE, type, _BCM_PETRA_FIELD_TEST_FIELD_GROUP_BANK_MAX_SIZE, field_groups, _BCM_PETRA_FIELD_TEST_ENTRY_NOF_GROUPS_ID(stage));
            if(result != CMD_OK) {
                if(_BCM_PETRA_FIELD_TEST_DEBUG_CHECK_VERB) {
                    LOG_INFO(BSL_LS_BCM_FP,
                             (BSL_META_U(unit,
        ...
        for(fg_idx = 0; fg_idx < _BCM_PETRA_FIELD_TEST_ENTRY_NOF_GROUPS_ID(stage); ++fg_idx) {
            ...
            /* actions of the entries */
            result = _bcm_petra_field_test_set_entry_actions(unit, stage, &field_groups[fg_idx].aset, TRUE, FALSE, x + second_rand_param++, &entries[fg_idx]);
            if(result != CMD_OK) {
                _BCM_PETRA_FIELD_TEST_FAIL_MSG;
                goto fail;
            }
...
#endif
        for(fg_idx = 0; fg_idx < _BCM_PETRA_FIELD_TEST_ENTRY_NOF_GROUPS_ID(stage); ++fg_idx) {
            result = _bcm_petra_field_test_traffic_test(unit, stage, TRUE, FALSE, FALSE, field_groups[fg_idx].group, &entries[fg_idx]);
            if(result != CMD_OK) {
                _BCM_PETRA_FIELD_TEST_FAIL_MSG;
                goto fail;
            }
                                                                     ...
                                                                     bcm_field_qset_t *valid_qset_msb_only)
{
    ...
    for(type =0; type < _BCM_PETRA_FIELD_NOF_TYPES; ++type) {
        ...
        BCM_IF_ERROR_RETURN(rv);
        BCM_FIELD_QSET_REMOVE(valid_qset_for_type, bcmFieldQualifyIp4);
        BCM_FIELD_QSET_REMOVE(valid_qset_for_type, bcmFieldQualifyIp6);
        BCM_FIELD_QSET_REMOVE(valid_qset_for_type, bcmFieldQualifyMpls);
        BCM_FIELD_QSET_REMOVE(valid_qset_for_type, bcmFieldQualifyStageIngress);
        BCM_FIELD_QSET_REMOVE(valid_qset_for_type, bcmFieldQualifyStageEgress);
        BCM_FIELD_QSET_REMOVE(valid_qset_for_type, bcmFieldQualifyStageExternal);
        BCM_FIELD_QSET_REMOVE(valid_qset_for_type, bcmFieldQualifyIsEqualValue);
        ...
        for(cur_qualify = 0; cur_qualify < bcmFieldQualifyCount; ++cur_qualify) {
            ...
            if(SOC_IS_ARAD(unit)) {
                ...
                for(ppd_qual_idx = 0; ppd_qual_idx < _BCM_PETRA_FIELD_QUAL_CHAIN; ++ppd_qual_idx) {
                    if(soc_ppd_qual_ptr[ppd_qual_idx] < SOC_PPD_NOF_FP_QUAL_TYPES) {
                        rv = _bcm_dpp_ppd_qual_bits(unitData, _BCM_DPP_FIELD_STAGE_INDEX_INGRESS, soc_ppd_qual_ptr[ppd_qual_idx], &exposed, &hardware, NULL);
                        BCM_IF_ERROR_RETURN(rv);
                        length += hardware;
                    }
                }
                                                              ...
                                                              int nof_groups)
{
    ...
    for(fg_idx = 0; fg_idx < nof_groups; ++fg_idx) {
        ...
        if(SOC_IS_ARADPLUS_AND_BELOW(unit) && ((nof_groups - fg_idx) >= _BCM_PETRA_FIELD_TEST_DIRECT_EXTRACTION_MAX_LSB_FIELD_GROUPS - fg_idx/2)) {
            /* in this case we have no freedom to chooose msb or lsb randomally - not needed in Jericho, since all the qualifiers are both LSB and MSB */
       ...
       ((total_length < action_length) && (_bcm_petra_field_rand_val(x, second_rand_param++) % 2)) ) {
        ...
        for(qual_index = 0; qual_index < nof_qualifiers; ++qual_index) {
            if(_bcm_petra_field_rand_val(x, second_rand_param++) % 2) {
                ...
                (*de_fields)[de_fields_index].qualifier = bcmFieldQualifyCount;
                total_length += (*de_fields)[de_fields_index].bits;
                ++de_fields_index;
                                                          ...
                                                          int nof_fields)
{
    ...
    /* setting the other qualifiers randomally */
    for(field_idx = 0; field_idx < nof_fields; ++field_idx) {
...
#ifndef PLISIM
        {
            SOC_PPD_FP_ACTION_TYPE ppd_action;
            bcm_error_t rv;
    ...
    for(iteration_idx = 0; iteration_idx < nof_iterations ; ++iteration_idx) {
        ...
        if(!with_traffic) {
            ...
            for(fg_idx = 0; fg_idx < nof_fgs; ++fg_idx) {
                ...
                if(!is_large) {
                    result = _bcm_petra_field_test_traffic_test(unit, stage, FALSE, TRUE, FALSE, field_groups[fg_idx].group, &entries[fg_idx][0]);
                    if(result != CMD_OK) {
                        _BCM_PETRA_FIELD_TEST_FAIL_MSG;
                        goto fail;
                    }
    ...
    for(iteration_idx = 0; iteration_idx < nof_iterations; ++iteration_idx) { 
        ...
        for(fg_idx = 0; fg_idx < SOC_DPP_DEFS_GET(unit, nof_tcam_big_banks); ++fg_idx){
            if((fg_idx % 3) == 0) {
                ...
                rv = bcm_field_qualify_HeaderFormat(unit, (fg_idx/3) | BCM_FIELD_QUALIFY_PRESEL, fg_idx/3);
...
#endif
            /* create entries in order that the field groups will be writen in the hardware */
        ...
        for(fg_idx = 0; fg_idx < SOC_DPP_DEFS_GET(unit, nof_tcam_big_banks); ++fg_idx) {
            ...
            if((fg_idx % 3) == 2) {
                rv = bcm_field_presel_destroy(unit, fg_idx/3);
                if(rv != BCM_E_NONE) {
                    _BCM_PETRA_FIELD_TEST_FAIL_MSG;
                    result = CMD_FAIL;
                }
...
destroy_presels_only:
    for(fg_idx = 0; fg_idx < SOC_DPP_DEFS_GET(unit, nof_tcam_big_banks)/3 + 1; ++fg_idx) {
        bcm_field_presel_destroy(unit, fg_idx);
    }
...
cmd_result_t _bcm_petra_field_test_direct_table_entry(int unit, _bcm_dpp_field_stage_idx_t stage, uint32 x, _bcmPetraFieldTestSpeedMode mode, uint8 do_warmboot)
{
    ...
    result = _bcm_petra_field_test_entry_and_traffic(unit, stage, x, mode, FALSE, TRUE, do_warmboot);
...
cmd_result_t _bcm_petra_field_test_direct_table_entry_traffic(int unit, _bcm_dpp_field_stage_idx_t stage, uint32 x, _bcmPetraFieldTestSpeedMode mode, uint8 do_warmboot)
{
    ...
    result = _bcm_petra_field_test_entry_and_traffic(unit, stage, x, mode, TRUE, TRUE, do_warmboot);
    if(result == CMD_OK) {
        LOG_INFO(BSL_LS_BCM_FP,
                 (BSL_META_U(unit,
                             "The test finished successfully\n")));
...
/* The test creates filed group and use all the posible entries till there is no plcae to insert more.
 ...
 */
STATIC cmd_result_t test_full_tcam(int unit, _bcm_dpp_field_stage_idx_t stage, uint32 x, _bcmPetraFieldTestSpeedMode mode, uint8 diff_prio, uint8 compress, uint8 do_warmboot)
{
    cmd_result_t result;
    bcm_error_t rv;
    uint32 second_rand_param = 0;
    ...
    uint32 nof_entries;
    uint32 nof_priorities_sets;
    uint32 priority_set_size;
    ...
    for(iteration_idx = 0; iteration_idx < nof_iterations; ++iteration_idx) { 
        ...
        result = _bcm_petra_field_test_fill_field_groups(unit, stage, x + second_rand_param++, FALSE, FALSE, 0, _BCM_PETRA_FIELD_TEST_FIELD_GROUP_BANK_MAX_SIZE, field_groups, _BCM_PETRA_FIELD_TEST_FULL_TCAM_NOF_GROUPS);
        if(result != CMD_OK) {
            if(_BCM_PETRA_FIELD_TEST_DEBUG_CHECK_VERB) {
                LOG_INFO(BSL_LS_BCM_FP,
                         (BSL_META_U(unit,
        ...
        if(stage == _BCM_DPP_FIELD_STAGE_INDEX_ING_EXTERNAL) {
            _BCM_PETRA_TEST_DEINIT_INIT_KBP(unit, rv);
            rv = _bcm_petra_field_test_kbp_set_presels(unit, _BCM_PETRA_FIELD_TEST_FULL_TCAM_NOF_GROUPS);
            if(rv != BCM_E_NONE) {
                _BCM_PETRA_FIELD_TEST_FAIL_MSG;
                return CMD_FAIL;
            }
        ...
        for(fg_idx = 0; fg_idx < _BCM_PETRA_FIELD_TEST_FULL_TCAM_NOF_GROUPS; ++fg_idx) {
...
#if _BCM_PETRA_FIELD_TEST_FULL_TCAM_NOF_GROUPS > 1 /* to avoid coverity defect */
            if((fg_idx == 0) || (key_hw_size[fg_idx] < key_hw_size_min) ) {
                key_hw_size_min = key_hw_size[fg_idx];
            }
#endif /* _BCM_PETRA_FIELD_TEST_FULL_TCAM_NOF_GROUPS > 1 */
            if(stage == _BCM_DPP_FIELD_STAGE_INDEX_INGRESS) {
            ...
            for(entry_idx = 0; entry_idx < nof_entries; ++entry_idx) {
                ...
                result = _bcm_petra_field_test_set_entry_qualifiers(unit, stage, &field_groups[fg_idx].qset, FALSE, x + second_rand_param++, FALSE, &entries[fg_idx][entry_idx]);
                if(result != CMD_OK) {
                    _BCM_PETRA_FIELD_TEST_FAIL_MSG;
                    goto fail;
                }
                ...
                /* actions of the entries */
                result = _bcm_petra_field_test_set_entry_actions(unit, stage, &field_groups[fg_idx].aset, TRUE, FALSE, x + second_rand_param++, &entries[fg_idx][entry_idx]);
                if(result != CMD_OK) {
                    _BCM_PETRA_FIELD_TEST_FAIL_MSG;
                    goto fail;
                }
        ...
        if(compress) {
            ...
            result =  _bcm_petra_field_test_fill_field_groups(unit, stage, x + second_rand_param++, FALSE, FALSE, 0, key_hw_size_min, &second_field_group, 1);
            if(result != CMD_OK) {
                _BCM_PETRA_FIELD_TEST_FAIL_MSG;
                goto fail;
            }
...
cmd_result_t _bcm_petra_field_test_full_tcam(int unit, _bcm_dpp_field_stage_idx_t stage, uint32 x, _bcmPetraFieldTestSpeedMode mode, uint8 do_warmboot)
{
    if(stage == _BCM_DPP_FIELD_STAGE_INDEX_EGRESS) {
        LOG_INFO(BSL_LS_BCM_FP,
                 (BSL_META_U(unit,
                             "The test is not valid for the engerss stage!\n")));
        return CMD_FAIL;
    }
    ...
    return test_full_tcam(unit, stage, x, mode, FALSE, FALSE, do_warmboot);
}
...
cmd_result_t _bcm_petra_field_test_full_tcam_diff_prio(int unit, _bcm_dpp_field_stage_idx_t stage, uint32 x, _bcmPetraFieldTestSpeedMode mode, uint8 do_warmboot)
{
    ...
    return test_full_tcam(unit, stage, x, mode, TRUE, FALSE, do_warmboot);
}
...
cmd_result_t _bcm_petra_field_test_compress(int unit, _bcm_dpp_field_stage_idx_t stage, uint32 x, _bcmPetraFieldTestSpeedMode mode, uint8 do_warmboot)
{
    if(stage != _BCM_DPP_FIELD_STAGE_INDEX_INGRESS) {
        LOG_INFO(BSL_LS_BCM_FP,
                 (BSL_META_U(unit,
                             "The test is valid for the ingerss stage only!\n")));
        return CMD_FAIL;
    }
    ...
    return test_full_tcam(unit, stage, x, mode, FALSE, TRUE, do_warmboot);
}
                                                            ...
                                                            uint32 *buffer)
{
    ...
    /* PTCH */
    val = 0;
    SHR_BITCOPY_RANGE(buffer, _BCM_PETRA_FIELD_TEST_PACKET_SIZE_BYTES*8 - 16, &val, 0, 16);

    /* ITMH */
    val = 0x000c0001;
    SHR_BITCOPY_RANGE(buffer, _BCM_PETRA_FIELD_TEST_PACKET_SIZE_BYTES*8 - 16 - 32, &val, 0, 32);

    /* allign the offset to refers the ethernet */
    dq.offset += 6;/* 2 for PTCH, 4 for ITMH */
    SHR_BITCOPY_RANGE(buffer, (_BCM_PETRA_FIELD_TEST_PACKET_SIZE_BYTES - dq.offset - dq.length)*8, data, 0, 8 * dq.length);
...
STATIC cmd_result_t _bcm_petra_field_test_itmh_field_group_tag(int unit, _bcm_dpp_field_stage_idx_t stage, uint32 x, _bcmPetraFieldTestSpeedMode mode, uint8 with_traffic, uint32 nof_fgs, uint8 do_warmboot)
{
    ...
    SOC_PPD_FP_ACTION_TYPE ppd_actions[_BCM_PETRA_FIELD_TEST_ITMH_NOF_ACTIONS];
    ...
    if(rv != BCM_E_NONE) {
        _BCM_PETRA_FIELD_TEST_FAIL_MSG;
        goto destry_data_qualifiers_only;
    }

    rv = bcm_field_qualify_ForwardingType(unit, BCM_FIELD_QUALIFY_PRESEL, bcmFieldForwardingTypeTrafficManagement);
    if(rv != BCM_E_NONE) {
        _BCM_PETRA_FIELD_TEST_FAIL_MSG;
        goto destry_data_qualifiers_and_presel;
    }
...
#ifdef BCM_ARAD_SUPPORT
                if(SOC_IS_ARAD(unit)) {
                    result = _bcm_petra_field_send_packet_and_get_information(unit, buffer, 0, &info);
                    if(result != CMD_OK) {
                        _BCM_PETRA_FIELD_TEST_FAIL_MSG;
                        goto fail;
                    }
                            ...
                            for(macro = _BCM_PETRA_FIELD_TEST_PP_FP_NOF_MACRO_SIMPLES - 1; macro >= 0; --macro) {
                                if(info->macro_simple[cycle][macro].action.type != SOC_PPD_FP_ACTION_TYPE_INVALID) {
                                    ...
                                    if(info->macro_simple[cycle][macro].action.val == data[0]) {
                                        ...
                                        LOG_INFO(BSL_LS_BCM_FP,
                                                 (BSL_META_U(unit,
                                                             "The action of D.B. occured with a wrong value expected 0x%x but got 0x%x\n"), data[0], info->macro_simple[cycle][macro].action.val));
                                        _BCM_PETRA_FIELD_TEST_FAIL_MSG;
                                        goto fail;
                                    }
                                }
...
destry_data_qualifiers_and_presel:
    bcm_field_presel_destroy(unit, 0);
destry_data_qualifiers_only:
    bcm_field_data_qualifier_destroy(unit, 0);
...
#ifdef BCM_ARAD_SUPPORT
STATIC cmd_result_t
_bcm_petra_field_test_itmh_parsing_test_get_signal(int unit, uint32 blk, ARAD_PP_DIAG_REG_FIELD *fld, uint32 *signal)
{
    uint32 soc_sand_res;
    ...
    soc_sand_res = arad_pp_diag_dbg_val_get_unsafe(unit, blk, fld, val);
    if(SOC_SAND_FAILURE(soc_sand_res)) {
        return CMD_FAIL;
    }
    *signal = val[0];
...
STATIC cmd_result_t
_bcm_petra_field_test_itmh_parsing_test_get_multicast_id(int unit, uint32 *multicast_id)
{
    cmd_result_t result;
    ...
    result = _bcm_petra_field_test_itmh_parsing_test_get_signal(unit, 21, &fld, multicast_id);
    if(result == CMD_OK) {
        *multicast_id &= (1 << 19) - 1;
    }
    return result;
...
STATIC cmd_result_t
_bcm_petra_field_test_itmh_parsing_test_get_user_header_2(int unit, uint32 *user_header_2)
{
    /* it is the same signal */
    return _bcm_petra_field_test_itmh_parsing_test_get_multicast_id(unit, user_header_2);
}

/* taking the signal -> lbp2dbg_ire_eq_bytes_to_strip [6 : 0] */
STATIC cmd_result_t
_bcm_petra_field_test_itmh_parsing_test_get_bytes_to_remove(int unit, uint32 *bytes)
{
    cmd_result_t result;
    ARAD_PP_DIAG_REG_FIELD fld;
    ...
    result = _bcm_petra_field_test_itmh_parsing_test_get_signal(unit, 21, &fld, bytes);
    if(result == CMD_OK) {
        *bytes &= (1 << 7) - 1;
    }
    return result;
...
STATIC cmd_result_t
_bcm_petra_field_test_itmh_parsing_test_get_snoop(int unit, uint32 *snoop)
{
    cmd_result_t result;
    ARAD_PP_DIAG_REG_FIELD fld;
    ...
    result = _bcm_petra_field_test_itmh_parsing_test_get_signal(unit, 21, &fld, snoop);
    if(result == CMD_OK) {
        *snoop &= (1 << 4) - 1;
    }
    return result;
...
STATIC cmd_result_t _bcm_petra_field_test_itmh_parsing_test_tag(int unit, _bcm_dpp_field_stage_idx_t stage, uint32 x, _bcmPetraFieldTestSpeedMode mode, uint8 do_warmboot)
{
#ifdef BCM_ARAD_SUPPORT
    /*bcm_error_t rv;*/
    cmd_result_t result;
    ...
    uint32 soc_sand_res;
    ARAD_PP_FP_PACKET packet;
    SOC_PPD_DIAG_PKT_TM_INFO packet_tm_info;
    ...
    if(SOC_IS_ARAD(unit)) {
        ...
        for(iteration_idx = 0; iteration_idx < nof_iterations; ++iteration_idx) { 
            ...
            for(flip = 0; flip < _BCM_PETRA_FIELD_TEST_ITMH_PARSING_TEST_NOF_FLIPS; ++flip) {
                ...
                for(buffer_idx = 3; buffer_idx < _BCM_PETRA_FIELD_TEST_PACKET_SIZE_BYTES / sizeof(uint32); ++buffer_idx) {
                ...
                soc_sand_res = soc_ppd_diag_pkt_associated_tm_info_get(unit, &packet_tm_info);
                if(SOC_SAND_FAILURE(soc_sand_res)) {
                    _BCM_PETRA_FIELD_TEST_FAIL_MSG;
                    return CMD_FAIL;
                }
                ...
                if(itmh_data.base.snoop_cmd_ndx != packet_tm_info.snoop_id) {
                    if(packet_tm_info.snoop_id) {
                        ...
                        result = _bcm_petra_field_test_itmh_parsing_test_get_snoop(unit, &snoop_verify);
                        if(result != CMD_OK) {
                            _BCM_PETRA_FIELD_TEST_FAIL_MSG;
                            return CMD_FAIL;
                        }
                ...
                if(itmh_data.base.destination.type != ARAD_DEST_TYPE_ISQ_FLOW_ID && itmh_data.base.destination.type != ARAD_DEST_TYPE_OUT_LIF) {
                    ...
                    if(itmh_data.base.destination.type == ARAD_DEST_TYPE_ISQ_FLOW_ID) {
                        ...
                        result = _bcm_petra_field_test_itmh_parsing_test_get_user_header_2(unit, &user_header_2);
                        if(result != CMD_OK) {
                            _BCM_PETRA_FIELD_TEST_FAIL_MSG;
                            return result;
                        }
                ...
                if(itmh_data.base.destination.type == ARAD_DEST_TYPE_MULTICAST_FLOW_ID) {
                    result = _bcm_petra_field_test_itmh_parsing_test_get_multicast_id(unit, &multicast_id_verify);
                    if(result != CMD_OK) {
                        _BCM_PETRA_FIELD_TEST_FAIL_MSG;
                        return result;
                    }
                ...
                /* bytes to remove */
                result = _bcm_petra_field_test_itmh_parsing_test_get_bytes_to_remove(unit, &bytes_to_remove);
                if(result != CMD_OK) {
                    _BCM_PETRA_FIELD_TEST_FAIL_MSG;
                    return result;
                }
        ...
        LOG_INFO(BSL_LS_BCM_FP,
                 (BSL_META_U(unit,
                             "The test is not valid to this unit.\n")));
        return CMD_FAIL;
#ifdef BCM_ARAD_SUPPORT
    }
#endif
...
cmd_result_t _bcm_petra_field_test_itmh_parsing_test(int unit, _bcm_dpp_field_stage_idx_t stage, uint32 x, _bcmPetraFieldTestSpeedMode mode, uint8 do_warmboot)
{
#ifdef BCM_ARAD_SUPPORT
    cmd_result_t result;
    bcm_error_t rv = BCM_E_NONE;
    ...
    result = _bcm_petra_field_test_itmh_parsing_test_tag(unit, stage, x, mode, do_warmboot);
    if(result == CMD_OK) {
        LOG_INFO(BSL_LS_BCM_FP,
                 (BSL_META_U(unit,
...
cmd_result_t _bcm_petra_field_test_resend_last_packet(int unit, _bcm_dpp_field_stage_idx_t stage, uint32 x, _bcmPetraFieldTestSpeedMode mode, uint8 do_warmboot)
{
    ...
    if(SOC_IS_ARAD(unit)) {
        cmd_result_t result;
        result = _bcm_petra_field_test_arad_pp_fp_ire_traffic_send(unit, NULL, TRUE);
        if(result == CMD_OK) {
            LOG_INFO(BSL_LS_BCM_FP,
                     (BSL_META_U(unit,
...
#endif
    {
        LOG_INFO(BSL_LS_BCM_FP,
                 (BSL_META_U(unit,
                             "The optin is unavailable to this device\n")));
        return CMD_FAIL;
    }
}
        ...
        if(SOC_IS_ARAD(unit)) {
...
#ifndef PLISIM
            {
                ...
                for(cycle = 0; cycle < ARAD_PP_FP_NOF_CYCLES; ++cycle) {
                    for(bank = 0; bank < ARAD_PP_FP_TCAM_NOF_BANKS; ++bank) {
                        if(info->tcam[cycle][bank].is_match) {
                            for(action = 0; action < ARAD_PP_FP_NOF_ACTIONS_PER_DB_MAX; ++action) {
                                if(info->tcam[cycle][bank].actions[action].type != SOC_PPD_FP_ACTION_TYPE_INVALID){
                                    ++nof_hits;
                                    if(info->tcam[cycle][bank].actions[action].type == SOC_PPD_FP_ACTION_TYPE_USER_HEADER_1 && 
                                    ...
                                    if(info->tcam[cycle][bank].actions[action].type == SOC_PPD_FP_ACTION_TYPE_USER_HEADER_2 && 
