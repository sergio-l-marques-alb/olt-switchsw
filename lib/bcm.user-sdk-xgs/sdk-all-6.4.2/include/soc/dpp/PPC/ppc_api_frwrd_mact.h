...
typedef enum
{
   ...
   *  additional_info at SOC_PPD_FRWRD_DECISION_INFO.
   */
...
typedef enum
{
  ...
  /*
   *  when call traverse function, then the call will
   *  - set travese rule in next available place if exist
   */
...
typedef struct
{
   ...
   *  DIF and FID (compatible MC packets briding)
   */
  SOC_PPC_FRWRD_MACT_KEY_TYPE key_type;
  /*
   *  MACT value, <MACT,FID> (normal bridging) or <DIP,FID>
   *  (IPv4 compatbilbe MC) or <Is-Destination-FEC,Destination,Flow-Label> (Stateful load balancing)
   *  
   */
...
typedef struct
{
   ...
   *  how fresh is the entry. Higher valule indicates a more
...
typedef struct
{
   ...
   *  compared. See SOC_PPD_FRWRD_MACT_TRAVERSE_MATCH_SELECT.
   */
  uint32 compare_mask;
...
typedef struct
{
   ...
   *  matched entries. See
   *  SOC_PPD_FRWRD_MACT_TRAVERSE_MATCH_SELECT
   */
  uint32 update_mask;
...
typedef struct
{
  ...
  /*
   *  Number of Traverse availale entries
   *  in ARAD: traverse can act on up to 8 rules togather
   *  ARAD only.
