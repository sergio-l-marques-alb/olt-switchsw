...
typedef enum
{
    ...
    ARAD_EGR_PROG_EDITOR_PROG_NULL, /* NULL program */
    #endif
...
typedef enum
{
    ...
    ARAD_EGR_PROG_EDITOR_LFEM_ZERO_4_MSB,         /*4_label_push*/
    ...
    ARAD_EGR_PROG_EDITOR_LFEM_BUILD_PTCH_2B,
...
#ifdef  ERASE_OR_DONT_BUT_REMOVE_THIS_MACRO
uint32
  arad_egr_prog_editor_stack_prune_bitmap_set(
    SOC_SAND_IN int unit,
    SOC_SAND_IN uint32 local_stack_port_ndx,
    SOC_SAND_IN uint32  bitmap[ARAD_STACK_PRUN_BMP_LEN]
  );

uint32
  arad_egr_prog_editor_stack_prune_bitmap_get(
    SOC_SAND_IN  int unit,
    SOC_SAND_IN  uint32 stack_profile,
    SOC_SAND_OUT uint32  bitmap[ARAD_STACK_PRUN_BMP_LEN]
  );
#endif
