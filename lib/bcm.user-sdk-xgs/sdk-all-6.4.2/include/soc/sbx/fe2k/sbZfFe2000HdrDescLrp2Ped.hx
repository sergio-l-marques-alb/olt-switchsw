...
/*
 * $Id: sbZfFe2000HdrDescLrp2Ped.hx,v 1.4 Broadcom SDK $
...
typedef struct _sbZfFe2000HdrDescLrp2Ped {
  ...
  uint32 ulStr;
  uint32 ulBuffer;
  uint32 ulExceptIndex;
  uint32 ulSQueue;
  uint32 ulTc;
  uint32 ulHc;
  uint32 ulMirrorIndex;
  uint32 ulCopyCnt;
  uint32 ulCM;
  uint32 ulD;
  uint32 ulEnq;
...
#define SB_ZF_FE2000HDRDESCLRP2PED_GET_XFERLEN(nToData,pFromData,nMaxFromDataIndex) \
          do { \
           (nToData) = (uint32) ((pFromData)[26] >> 2) & 0x3f; \
           (nToData) |= (uint32) (pFromData)[27] << 6; \
          } while(0)
...
#define SB_ZF_FE2000HDRDESCLRP2PED_GET_FRAMELEN(nToData,pFromData,nMaxFromDataIndex) \
          do { \
           ...
           (nToData) |= (uint32) ((pFromData)[25] & 0x3f) << 8; \
          } while(0)
