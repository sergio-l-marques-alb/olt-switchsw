/*
 * $Id: wb_db_qos.h,v 1.2 Broadcom SDK $
 ...
 *
 * File:        wb_db_qos.h
 * Purpose:     WarmBoot - Level 2 support (QOS Module)
 */

#ifndef _BCM_DPP_WB_DB_QOS_H_
#define _BCM_DPP_WB_DB_QOS_H_
 ...
 *
 * Currently there is a single handle for QOS module.
...
#define BCM_DPP_WB_QOS_VERSION_1_0            SOC_SCACHE_VERSION(1,0)
#define BCM_DPP_WB_QOS_CURRENT_VERSION        BCM_DPP_WB_QOS_VERSION_1_0


#define BCM_DPP_WB_QOS_INFO(unit)             (_dpp_wb_qos_info_p[unit])
#define BCM_DPP_WB_QOS_INFO_INIT(unit)        ( (_dpp_wb_qos_info_p[unit] != NULL) &&          \
                                                 (_dpp_wb_qos_info_p[unit]->init_done == TRUE) )
...
#define BCM_DPP_WB_DEV_QOS_DIRTY_BIT_SET(unit)                                       \
                        SOC_CONTROL_LOCK(unit);                                       \
                        SOC_CONTROL(unit)->scache_dirty = 1;                          \
                        BCM_DPP_WB_QOS_INFO(unit)->is_dirty = 1;                     \
                        SOC_CONTROL_UNLOCK(unit);

#define BCM_DPP_WB_QOS_DIRTY_BIT_CLEAR(unit)                                         \
                        SOC_CONTROL_LOCK(unit);                                       \
                        BCM_DPP_WB_QOS_INFO(unit)->is_dirty = 0;                     \
                        SOC_CONTROL_UNLOCK(unit);

#define BCM_DPP_WB_QOS_IS_DIRTY(unit)                                                \
                        (BCM_DPP_WB_QOS_INFO(unit)->is_dirty == 1)
 ...
 * runtime information
 */
typedef struct bcm_dpp_wb_qos_info_s {
    int            init_done;
    int            is_dirty;
    ...
    int            ing_lif_cos_mpls_bitmap_off;
    int            ing_lif_cos_ipv4_bitmap_off;
    int            ing_lif_cos_ipv6_bitmap_off;
    int            ing_lif_cos_l2_inner_bitmap_off;
    int            ing_lif_cos_l2_outer_bitmap_off;
    int            ing_lif_cos_l2_untag_bitmap_off;

    int            ing_pcp_vlan_ctag_bitmap_off;
    int            ing_pcp_vlan_utag_bitmap_off;
    int            ing_pcp_vlan_stag_bitmap_off;    

    int            egr_remark_encap_mpls_bitmap_off;
    int            egr_remark_encap_ipv6_bitmap_off;
    int            egr_remark_encap_ipv4_bitmap_off;
    int            egr_remark_encap_l2_bitmap_off;
    int            egr_remark_mpls_bitmap_off;
    int            egr_remark_ipv6_bitmap_off;
    int            egr_remark_ipv4_bitmap_off;    

    int            egr_mpls_php_ipv6_bitmap_off;
    int            egr_mpls_php_ipv4_bitmap_off;    

    int            egr_pcp_vlan_ctag_bitmap_off;
    int            egr_pcp_vlan_utag_bitmap_off;
    int            egr_pcp_vlan_stag_bitmap_off;
    int            egr_pcp_vlan_mpls_bitmap_off;
    int            egr_pcp_vlan_ipv6_bitmap_off;
    int            egr_pcp_vlan_ipv4_bitmap_off;    

    int            egr_l2_i_tag_bitmap_off;    
#ifdef BCM_88660
    int            egr_dscp_exp_marking_bitmap_off;
#endif /* BCM_88660 */
    int            ing_cos_opcode_off;
    int            opcode_bmp_off;

} bcm_dpp_wb_qos_info_t;

extern bcm_dpp_wb_qos_info_t   *_dpp_wb_qos_info_p[BCM_MAX_NUM_UNITS] ;
