...
\section tsce reference clock configuration 

TSCE can configured to use 156.25MHz or 125MHz reference clock. 

For PCS Configuration, The reference clock freq only matters for AN. And used for tick generation, that is used in AN timers.
 ...
 - In the case of 125MHz reflck only:  set the tick override enable
   ...
   and tick_denominator = 1.  For 1256.25MHz refclk, the design takes casre of the tick 
          Register: Main0_tick_control_1(Adr:0x9007) and Main0_tick_control_0(Adr:9008)
           
For PMD configuration,
 - The pll_mode and the osr_mode are to be programmed as required, based on the refclk.
        Register: PLL_CAL_COM_CTL_7(Adr:0xd127) and CKRST_CTRL_OSR_MODE_CONTROL - OSR_MODE_CONTROL(Adr:0xd080)
 - For 125MHz, the heartbeat counter should be set to 'd500. For 156.25MHz refclk, the default setting of `d625 is good
        Register: DIG_COM_TOP_USER_CONTROL_0(Adr:0xd0f4)
 - Do not change either refclk_divcnt  refclk_divcnt_sel registers. The default values will work fine for 125MHz or 156.25 MHz refclk. 
   This clock divider is used by the internal calibration logic to generate an internal slow clock that is used for calibration. 
   If you switch to 125MHz refclk, this internal clock is a little slower and total calibration time will take a little longer but 
   calibration will work fine with this. These registers should only be used for debug to tweak calibration if needed.
       Register: PLL_CAL_COM_CTL_5(Adr:0xd125) and PLL_CAL_COM_CTL_6(Adr:0xd126)
 - The PMD ucode should support the configuration of pll_mode and osrmode for AN speeds for the corresponding refclk.

\section  tsce-pmd fractional plldiv configuration

  The PMD API "eagle_tsc_configure_pll", should be used for fractional plldiv configuration. More details goto eagle API  #eagle_tsc_configure_pll


\section  TSCE frequently asked questions (FAQ)

  - What are the different firmware load methods?

   The two firmware load methods are external and internal. External is fast load. Internal load is slow load. External load is recommended way as it is faster
