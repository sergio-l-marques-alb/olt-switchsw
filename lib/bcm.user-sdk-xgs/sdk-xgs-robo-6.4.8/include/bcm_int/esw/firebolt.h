/*
 * $Id: firebolt.h,v 1.326 Broadcom SDK $
...
#define BCM_XGS3_L3_L2CPU_NH_IDX    (1) /* Reserved trap to cpu nh index.   */
#define BCM_XGS3_L3_INVALID_INDEX  (-1) /* Generic invalid table index.     */
#define BCM_XGS3_EGRESS_IDX_MIN   _SHR_L3_EGRESS_IDX_MIN /* Egress object start index    */
...
#define BCM_XGS3_MPLS_IF_EGRESS_IDX_MIN (0x493E0) /* MPLS interface egress  */
                                                  /* object start index.    */
#define BCM_XGS3_DVP_EGRESS_IDX_MIN (0x61A80) /* DVP next hop egress object */
#define BCM_XGS3_PROXY_EGRESS_IDX_MIN (0x7A120) /* Proxy next hop egress object */
#define BCM_XGS3_L3_BLACK_HOLE_NH_IDX   (0) /* BLACK_HOLE nhop idx */
 ...
 * This flag is used only while creating ecmp RH group on 
 * device which stores RH set in member table(Tomahawk) and reset when done.
 * The flag need not be saved and is not used for any state recovery later.
 * This is internal flag for now. However, if another group flag with
 * this value is defined, this will need to be redefined to another
 * unused value
 */         
 ...
 * Only for TH with Resilient Hashing, max paths is 16k
 */
...
typedef struct _bcm_l3_tbl_op_s {
    ...
    int  value;          /* Parameter to be passed */
    ...
    int (*add_func) (int, int, void *, int);
                             /* Entry deletion routine.                  */
    int (*delete_func) (int, int, int);
} _bcm_l3_tbl_op_t;
...
typedef struct _bcm_l3_ecmp_info_s {
    ...
    int ecmp_flags[2 * BCM_L3_MAX_ECMP_GROUPS]; /* ECMP group flags for upto 2K groups. */
...
typedef struct _bcm_l3_hw_calls_s {
    ...
    /* Add next hop entry. */
    int (*nh_add)(int unit, int idx, void *nh_entry, int max_paths);
    /* Delete next hop entry. */
    int (*nh_del)(int unit, int idx, int max_paths);
    ...
    /* Add ecmp group. */
    int (*ecmp_grp_add)(int unit, int ecmp_grp, void *buf, int max_paths);
    /* Delete ecmp group. */
    int (*ecmp_grp_del)(int unit, int ecmp_grp, int max_paths);
    ...
    int (*ing_intf_get)(int unit,  iif_entry_t *hw_entry,
    ...
    int (*tnl_init_add)(int unit, int idx, void *buf, int max_paths);
    /* Delete tunnel initiator entry. */
    int (*tnl_init_del)(int unit, int idx, int max_paths);
    ...
    int (*proxy_nh_add)(int unit, int idx, void *nh_entry, int max_paths);
} _bcm_l3_hw_calls_t;
...
#define BCM_XGS3_DVP_EGRESS_IDX_VALID(_u_, _idx_)                     \
            ((_idx_ >= BCM_XGS3_DVP_EGRESS_IDX_MIN ) &&               \
             (_idx_ < BCM_XGS3_DVP_EGRESS_IDX_MIN + BCM_XGS3_L3_NH_TBL_SIZE(_u_)))

/* Proxy next hop egress object index valid. */
#define BCM_XGS3_PROXY_EGRESS_IDX_VALID(_u_, _idx_)                     \
            ((_idx_ >= BCM_XGS3_PROXY_EGRESS_IDX_MIN ) &&               \
             (_idx_ < BCM_XGS3_PROXY_EGRESS_IDX_MIN + BCM_XGS3_L3_NH_TBL_SIZE(_u_)))
...
extern int _bcm_fb2_port_tpid_set(int unit, bcm_port_t port, uint16 tpid);
...
extern int bcm_xgs3_l3_egress_ecmp_max_paths_get(int unit, bcm_if_t mpintf,
                                                 int *max_paths);
extern int _bcm_xgs3_l3_ecmp_grp_info_get(int unit, void *l3_ecmp_count, 
                                                  int *group_size, int *base_idx);
extern int _bcm_xgs3_l3_ecmp_grp_info_set(int unit, void *l3_ecmp_count,     
...
extern int _bcm_xgs3_l3_ecmp_egress_if_get(int unit, bcm_l3_egress_ecmp_t *ecmp_info, 
                                           int intf_size, bcm_if_t *intf_array, int *intf_count);
...
extern int _bcm_xgs3_nh_entry_parse(int unit, uint32 *ing_entry_ptr, 
                                    uint32 *egr_entry_ptr, uint32 index, 
...
extern int _field_fb_tcam_policy_mem_get(int unit, _field_stage_id_t stage_id,
                                         soc_mem_t *tcam_mem,
                                         soc_mem_t *policy_mem);
extern int _field_fb_tcam_policy_clear(int unit, _field_stage_id_t stage_id,
                                       int tcam_idx);
