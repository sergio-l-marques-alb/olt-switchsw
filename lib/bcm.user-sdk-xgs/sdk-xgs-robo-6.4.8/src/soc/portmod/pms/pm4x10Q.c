...
int pm4x10q_wb_buffer_init(int unit, int wb_buffer_index, pm_info_t pm_info)
{
    ...
    SOC_WB_ENGINE_ADD_BUFF(SOC_WB_ENGINE_PORTMOD, wb_buffer_index, "pm4x10Q", NULL, VERSION(1), 1, SOC_WB_ENGINE_PRE_RELEASE);
    _SOC_IF_ERR_EXIT(rv);
...
int pm4x10Q_port_attach(int unit, int port, pm_info_t pm_info, const portmod_port_add_info_t* add_info)
{
    ...
    SOC_IF_ERROR_RETURN(pm4x10_default_bus_update(unit, PM_4x10Q_INFO(pm_info)->pm4x10, &pm_update));
    ...
    if (pm_is_active == 0) {
        ...
        SOC_IF_ERROR_RETURN(WRITE_CHIP_SWRSTr(unit, reg_port, reg_val));
    ...
    SOC_IF_ERROR_RETURN(WRITE_GPORT_RSV_MASKr(unit, port, 0x78)); 
    SOC_IF_ERROR_RETURN(WRITE_GPORT_STAT_UPDATE_MASKr(unit, port, 0x78));

    SOC_IF_ERROR_RETURN(READ_GPORT_CONFIGr(unit, port, &reg_val));
    soc_reg_field_set(unit, GPORT_CONFIGr, &reg_val, CLR_CNTf, 1);
    SOC_IF_ERROR_RETURN(WRITE_GPORT_CONFIGr(unit, port, reg_val));
    ...
    SOC_IF_ERROR_RETURN(WRITE_GPORT_MODE_REGr(unit, port, reg_val));
     ...
     /* init MAC */
    _SOC_IF_ERR_EXIT(unimac_init(unit, port, 0));
...
{
    int enable, phy_index, sub_phy, total_ports_left, i, reg_port, invalid_port = -1, temp_port, rv;
    uint32 reg_val, phy_num_ports, phy_acc_port;
    SOC_INIT_FUNC_DEFS;
    ...
    if (phy_acc_port == port && phy_num_ports == 0) {
        _SOC_IF_ERR_EXIT(pm4x10_pm_bypass_set(unit, PM_4x10Q_INFO(pm_info)->pm4x10, 0)); 

        _SOC_IF_ERR_EXIT(pm4x10_port_detach(unit, port, PM_4x10Q_INFO(pm_info)->pm4x10));
...
{
    int phy_index, sub_phy, temp_flags, rv;
    soc_port_t phy_acc_port;
    SOC_INIT_FUNC_DEFS;
    
    if (PORTMOD_PORT_ENABLE_MAC_GET(flags) || flags == 0) {
        _SOC_IF_ERR_EXIT(unimac_enable_set(unit, port, enable));
    }

    if (PORTMOD_PORT_ENABLE_PHY_GET(flags) || flags == 0) {
        temp_flags = flags;
        PORTMOD_PORT_ENABLE_MAC_CLR(temp_flags);
        PORTMOD_PORT_ENABLE_PHY_SET(temp_flags);
        ...
        _SOC_IF_ERR_EXIT(pm4x10_port_enable_set(unit, phy_acc_port, PM_4x10Q_INFO(pm_info)->pm4x10, temp_flags, enable));
...
{
    int temp_flags, phy_enable, mac_enable, phy_index, sub_phy, rv;
    soc_port_t phy_acc_port;
    SOC_INIT_FUNC_DEFS;
    
    if (PORTMOD_PORT_ENABLE_MAC_GET(flags) || flags == 0) {
       _SOC_IF_ERR_EXIT(unimac_enable_get(unit, port, &mac_enable));
       *enable = mac_enable;
    }
    ...
    if (PORTMOD_PORT_ENABLE_PHY_GET(flags) || flags == 0) {
        ...
        _SOC_IF_ERR_EXIT(pm4x10_port_enable_get(unit, phy_acc_port, PM_4x10Q_INFO(pm_info)->pm4x10, temp_flags, &phy_enable));
        *enable = phy_enable;
    }
...
int pm4x10Q_port_interface_config_set(int unit, int port, pm_info_t pm_info, const portmod_port_interface_config_t* config, int all_phy)
{
    ...
    /* call pm4x10 below */
    _SOC_IF_ERR_EXIT(pm4x10_port_interface_config_set(unit, phy_acc_port, PM_4x10Q_INFO(pm_info)->pm4x10, config, all_phy));
...
int pm4x10Q_port_interface_config_get(int unit, int port, pm_info_t pm_info, portmod_port_interface_config_t* config)
{
    ...
    /* call pm4x10 below */
    _SOC_IF_ERR_EXIT(pm4x10_port_interface_config_get(unit, phy_acc_port, PM_4x10Q_INFO(pm_info)->pm4x10, config));
...
int pm4x10Q_port_loopback_set(int unit, int port, pm_info_t pm_info, portmod_loopback_mode_t loopback_type, int enable)
{
    ...
    case portmodLoopbackPhyRloopPMD:
        ...
        /* call pm4x10 below */
        _SOC_IF_ERR_EXIT(pm4x10_port_loopback_set(unit, phy_acc_port, PM_4x10Q_INFO(pm_info)->pm4x10, loopback_type, enable));
        break;
    default:
...
int pm4x10Q_port_loopback_get(int unit, int port, pm_info_t pm_info, portmod_loopback_mode_t loopback_type, int* enable)
{
    ...
    case portmodLoopbackPhyRloopPMD:
        ...
        /* call pm4x10 below */
        _SOC_IF_ERR_EXIT(pm4x10_port_loopback_get(unit, phy_acc_port, PM_4x10Q_INFO(pm_info)->pm4x10, loopback_type, enable));
        break;
    default:
        _SOC_EXIT_WITH_ERR(SOC_E_UNAVAIL, (_SOC_MSG("unsupported loopback type %d"), loopback_type));
    }
...
int pm4x10Q_port_link_get(int unit, int port, pm_info_t pm_info, int* link)
{
    int rv;
    int phy_index, sub_phy;
    soc_port_t phy_acc_port;
    ...
    rv = pm4x10_port_link_get(unit, phy_acc_port, PM_4x10Q_INFO(pm_info)->pm4x10, link);
    _SOC_IF_ERR_EXIT(rv);
...
int pm4x10Q_port_autoneg_set(int unit, int port, pm_info_t pm_info, const phymod_autoneg_control_t* an)
{
    int rv;
    int phy_index, sub_phy;
    soc_port_t phy_acc_port;
    ...
    rv = pm4x10_port_autoneg_set(unit, phy_acc_port, PM_4x10Q_INFO(pm_info)->pm4x10, an);
    _SOC_IF_ERR_EXIT(rv);
...
int pm4x10Q_port_autoneg_get(int unit, int port, pm_info_t pm_info, phymod_autoneg_control_t* an)
{
    int rv;
    int phy_index, sub_phy;
    soc_port_t phy_acc_port;
    ...
    rv = pm4x10_port_autoneg_get(unit, phy_acc_port, PM_4x10Q_INFO(pm_info)->pm4x10, an);
    _SOC_IF_ERR_EXIT(rv);
...
int pm4x10Q_port_polarity_update_set(int unit, int port, pm_info_t pm_info, int lane)
{
    int rv;
    int phy_index, sub_phy;
    soc_port_t phy_acc_port;
    ...
    rv = pm4x10_port_polarity_update_set(unit, phy_acc_port, PM_4x10Q_INFO(pm_info)->pm4x10, phy_index);
    _SOC_IF_ERR_EXIT(rv);
