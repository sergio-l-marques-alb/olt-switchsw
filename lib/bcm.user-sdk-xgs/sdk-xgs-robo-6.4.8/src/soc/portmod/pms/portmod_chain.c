                                               ...
                                               const uint32 phy_init_flags)
{
    ...
    if(phy_init_flags == PORTMOD_INIT_F_ALL_PHYS) { 
        ...
        rv = phymod_phy_interface_config_set(&phy_access[0], flags,config);
    }
    return(rv);
}
...
int phy_index;
    ...
    while((PHYMOD_E_UNAVAIL== rv) && (phy_index >= 0)) {
        if (phy_index && PORTMOD_IS_LEGACY_PHY_GET(&(phy_access[phy_index].access))) {
            ...
            rv = phymod_phy_intr_enable_set(&phy_access[phy_index], intr_src, enable);
        }
        phy_index--;                             
    }
    return(rv);
...
int phy_index;
    ...
    while((PHYMOD_E_UNAVAIL== rv) && (phy_index >= 0)) {
        if (phy_index && PORTMOD_IS_LEGACY_PHY_GET(&(phy_access[phy_index].access))) {
            ...
            rv = phymod_phy_intr_enable_get(&phy_access[phy_index],intr_src, enable);
        }
        phy_index--;
    }
    return(rv);
...
int phy_index;
    ...
    while((PHYMOD_E_UNAVAIL== rv) && (phy_index >= 0)) {
        if (phy_index && PORTMOD_IS_LEGACY_PHY_GET(&(phy_access[phy_index].access))) {
            ...
            rv = phymod_phy_intr_status_get(&phy_access[phy_index],intr_src, enable);
        }
        phy_index--;
    }
    return(rv);
...
int phy_index;
    ...
    while((PHYMOD_E_UNAVAIL== rv) && (phy_index >= 0)) {
        if (phy_index && PORTMOD_IS_LEGACY_PHY_GET(&(phy_access[phy_index].access))) {
            ...
            rv = phymod_phy_intr_status_clear(&phy_access[phy_index],intr_src );
        }
        phy_index--;
    }
    return(rv);
