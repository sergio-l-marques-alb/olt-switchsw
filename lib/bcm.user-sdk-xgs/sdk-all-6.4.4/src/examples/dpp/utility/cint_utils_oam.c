...
int NUMBER_OF_COUNTERS_PER_COUNTER_SOURCE = 0x2000; /* Assuming double packets per source */
...
int set_counter_source_and_engines(int unit, int * counter_base) {
	...
	*counter_base = 0;
	...
	if ( current_lm_counter_base % NUMBER_OF_COUNTERS_PER_COUNTER_SOURCE == 0  ) {
		...
		counter_config.format.format_type = bcmStatCounterFormatDoublePackets; /* This gives us 8K counter ids per engine, but only 29 bits for the counter.
																														29 bits is enough since TXfcF, RxfcF on LMM/Rs only include 32 bits.*/
		...
		calls_to_counter_config_set +=2;
	}

	/* At this stage the counter source has been allocated. Return the counter base and get out.*/
	current_lm_counter_base += 8; /* Increment by 8 to allow for PCP based LM.*/
