...
int create_l3_intf(int unit, uint32 flags, uint8 open_vlan, bcm_gport_t port, int vlan, int vrf, bcm_mac_t mac_addr, int *intf) {
  ...
  rc = double_tag_hook(unit, port, vlan);
  if (rc != BCM_E_NONE) {
	  printf("Error, in double_tag_hook, used to add support for d_tagged/untagged packets\n"); 
	  return rc;
  }
  ...
  l3if.l3a_vid = vlan;
  l3if.l3a_mtu_forwarding = default_mtu_forwarding;
  l3if.l3a_mtu = default_mtu;
...
int default_route_config(int unit, int in_port, int out_port, int is_trap){
    ...
    if (rv != BCM_E_NONE) {
        printf("Error, create_l3_intf_global_route\n", in_port);
    }    
