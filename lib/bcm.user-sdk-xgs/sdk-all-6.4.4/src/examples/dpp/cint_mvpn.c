...
int mvpn_l2gre_ip_tunnel(int unit, int *encap) {
    ...
    rv = ipv4_tunnel_build_tunnels(unit, cint_mvpn_info.provider_port, out_tunnel_intf_ids,out_tunnel_gports);
    if (rv != BCM_E_NONE) {
    ...
    rv = bcm_multicast_l3_encap_get(unit, -1 /* mc: not used */, out_tunnel_gports[0], out_tunnel_intf_ids[0], encap); 
    if (rv != BCM_E_NONE) {
...
int mvpn_run(int unit) {
    ...
    rv = bcm_port_control_set(0,cint_mvpn_info.recycle_port, bcmPortControlOverlayRecycle, 1);  
    if (rv != BCM_E_NONE) {
