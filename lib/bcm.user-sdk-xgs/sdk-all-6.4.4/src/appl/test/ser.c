...
{
    uint32 mem_buff[SOC_MAX_MEM_WORDS], mask[SOC_MAX_MEM_WORDS], mem_buff_restored_from_cache[SOC_MAX_MEM_WORDS];
    uint32 interrupt_count, new_interrupt_count, sleep_for_a_bit = 20000;
    ...
    if (new_interrupt_count != 1 + interrupt_count) {
        LOG_INFO(BSL_LS_APPL_TESTS,(BSL_META_U(unit, "Memory %s didn't triger interrupt %d\n, old count is %d, new count is %d\n"), SOC_MEM_NAME(unit, mem), ser_test_params->interrupt, interrupt_count, new_interrupt_count));
        test_failed = 1;
        rv = BCM_E_FAIL;
        goto done;
    }
