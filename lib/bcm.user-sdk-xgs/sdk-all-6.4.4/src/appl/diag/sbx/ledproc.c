/*
 * $Id: ledproc.c,v 1.12 Broadcom SDK $
...
static void _ledproc_write_addr(int unit, int addr, uint8 value)
{
    ...
    if(iterations >= 10) {
        cli_out("Error reading addr 0x%X\n", addr);
    }
}

static uint8 _ledproc_read_addr(int unit, int addr)
{
    uint32 reg = 0;
    int done;
    int iterations = 0;
 ...
 */
{
    ...
    if (ARG_CNT(a) == 0) {
        cli_out("%s: LED Micro-controller is %s (PC=0x%x State=%s%s); "
                "auto updating is %s.\n",
                ARG_CMD(a),
        ...
        LOG_VERBOSE(BSL_LS_APPL_COMMON,
                    (BSL_META_U(unit,
                                "LED_CTRL=0x%02X LED_STATUS=0x%02X\n"),
                     led_ctrl, led_status));
        return(CMD_OK);
    }
	...
	for (offset = 0; offset <= max; offset++) {
	    cli_out(" %02x", _ledproc_read_addr(unit, FE2K_LED_PROGRAM_RAM(offset)));
	    if ((offset % 16) == 15) {
            cli_out("\n");
	    }
	}
