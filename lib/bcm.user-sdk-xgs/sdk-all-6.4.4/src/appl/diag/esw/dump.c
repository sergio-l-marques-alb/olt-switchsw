...
do_dump_table(int unit, soc_mem_t mem,
              ...
              int enable_ifp_slice_mode_check)
{
    int       k, i;
    ...
    for (k = index; k < index + count; k++) {
        ...
        if (SOC_IS_TOMAHAWK(unit)) {
            if (enable_ifp_slice_mode_check) {
                ...
                if (soc_th_ifp_index_invalid(unit, tmp_mem, k)) {
                    continue;
                }
            }
        }
...
#endif
        {
...
#ifdef BCM_TOMAHAWK_SUPPORT
                if (SOC_IS_TOMAHAWK(unit)) {
                    view = _soc_tomahawk_alpm_bkt_view_get(unit, k);
                } else 
#endif
                {
            ...
            if (flags & DUMP_TABLE_PIPE_X) {
                i = soc_mem_pipe_select_read(unit, SOC_MEM_NO_FLAGS, mem, copyno,
                ...
                i = soc_mem_read_no_cache(unit, mem, copyno, k, entry);
            } else {
                i = soc_mem_read(unit, mem, copyno, k, entry);
            }
        }
...
cmd_esw_dump(int unit, args_t *a)
{
    ...
    for (;;) {
         if (arg1 != NULL && !sal_strcasecmp(arg1, "raw")) {
             ...
             arg1 = ARG_GET(a);
         } else if (arg1 != NULL && !sal_strcasecmp(arg1, "nocheck_ifp_slice_mode")) {
