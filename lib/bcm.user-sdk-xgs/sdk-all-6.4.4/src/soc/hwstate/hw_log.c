...
STATIC void _soc_hw_log_check_overflow(int unit);
...
static LogList_t Hw_Log_List[SOC_MAX_NUM_DEVICES];
...
{
    return Hw_Log_List[unit].IsEmpty ? TRUE : FALSE;
}
...
soc_hw_log_is_enabled(int unit)
{
    ...
    if ((Hw_Log_List[unit].HwLogSize == 0) || Hw_Log_List[unit].WorkingMode == HW_LOG_MODE_Disabled)
        return FALSE;
    else
        return TRUE;
}
...
{
    if (sal_thread_self() == Hw_Log_List[unit].TID)  /* Allow Commits only from main thread */
        return TRUE;
    else
        return FALSE;
}
...
soc_hw_log_register_callbacks(int unit)
{
    ...
    SOC_INFO(unit).ha_hw_log_access = crash_rec_access;

    return SOC_E_NONE;
}

void
_soc_hw_log_list_init(int unit, HwLogEntry_t *ptr)
{
    Hw_Log_List[unit].Unit         = unit;
    Hw_Log_List[unit].Head         = ptr;
    Hw_Log_List[unit].Current      = NULL;
    Hw_Log_List[unit].End          = ptr + Hw_Log_List[unit].NumOfElements;
    Hw_Log_List[unit].EntriesCounter = 0;
    Hw_Log_List[unit].IsEmpty      = TRUE;
}
...
_soc_hw_log_get_soc_properties(int unit)
{
    ...
    /* GET SOC PROPERTY*/
    Hw_Log_List[unit].WorkingMode = soc_property_get(unit, spn_HA_HW_JOURNAL_MODE, HW_LOG_MODE_Disabled);
    Hw_Log_List[unit].HwLogSize = soc_property_get(unit, spn_HA_HW_JOURNAL_SIZE, 0);

/*
    LOG_ERROR(BSL_LS_SOC_HWLOG,
            (BSL_META_U(unit, "hwlog_mode %d, hwlog_size %d\n"), Hw_Log_List[unit].WorkingMode, Hw_Log_List[unit].HwLogSize));
*/
...
soc_hw_log_init(int unit)
{
...
#endif
    {
        ...
        {
            Hw_Log_List[unit].NumOfElements = Hw_Log_List[unit].HwLogSize / sizeof(HwLogEntry_t);

            
            mem_ptr = sal_alloc(Hw_Log_List[unit].HwLogSize, "HW_LOG_Mem_Pool");
            if (!mem_ptr)
            {
                LOG_ERROR(BSL_LS_SOC_HWLOG,
                                    (BSL_META_U(unit, "HW LOG Allocation failed for unit %d. LogSize %d\n"), unit, Hw_Log_List[unit].HwLogSize));
                return SOC_E_MEMORY;
            }
            memset(mem_ptr, 0, Hw_Log_List[unit].HwLogSize);

            Hw_Log_List[unit].TID = sal_thread_self();

            /* Init Hw Log List*/
            _soc_hw_log_list_init(unit, (HwLogEntry_t *) mem_ptr);
            Hw_Log_List[unit].ImmediateAccess = FALSE;

            LOG_ERROR(BSL_LS_SOC_HWLOG,
                    (BSL_META_U(unit, "HW LOG Enabled for unit %d. LogSize %d, WorkingMode %s\n"), unit, Hw_Log_List[unit].HwLogSize,
                            (Hw_Log_List[unit].WorkingMode == HW_LOG_MODE_Disabled) ? "Disabled" : "Enabled"));
        }
        else /* HW LOG is disabled */
        {
            Hw_Log_List[unit].ImmediateAccess = TRUE;
            _soc_hw_log_list_init(unit, NULL);
...
{
    if (Hw_Log_List[unit].WorkingMode != HW_LOG_MODE_Disabled)
    {
        Hw_Log_List[unit].ImmediateAccess = TRUE;
        Hw_Log_List[unit].WorkingMode = HW_LOG_MODE_Disabled;

        if (Hw_Log_List[unit].Head)
            sal_free(Hw_Log_List[unit].Head);

        Hw_Log_List[unit].Head = NULL;
        _soc_hw_log_list_init(unit, NULL);
/*
    LOG_ERROR(BSL_LS_SOC_HWLOG,
...
{
    HwLogEntry_t *entry = Hw_Log_List[unit].Head;
...
soc_hw_log_get_working_mode(int unit)
{
    ...
    return Hw_Log_List[unit].WorkingMode;
}
...
soc_hw_log_purge(int unit)
{
    ...
    if (soc_hw_log_get_working_mode(unit) == HW_LOG_MODE_Commit_After_Each_Api)
        _soc_hw_log_list_init(unit, Hw_Log_List[unit].Head);
    return SOC_E_NONE;
}
...
_soc_hw_log_get_next_list_entry(int unit, HwLogEntry_t** hwLogEntry)
{
    ...
    if (Hw_Log_List[unit].Head == NULL) /* The List is not initialized */
    {
        LOG_ERROR(BSL_LS_SOC_HWLOG,
    ...
    if (Hw_Log_List[unit].IsEmpty == TRUE) /*The list is empty, this is the first element */
    {
        /*        LOG_ERROR( BSL_LS_SOC_HWLOG, (BSL_META_U(unit, "Unit %d: First Entry in Hw Log List.\n"), unit) );*/
        entry = Hw_Log_List[unit].Head;
        Hw_Log_List[unit].IsEmpty = FALSE;
    }
    else if (Hw_Log_List[unit].Current <= Hw_Log_List[unit].End) /* If this is not the last element */
    {
        entry = Hw_Log_List[unit].Current + 1; /* The list is not empty, insert the data to the tail. */
        Hw_Log_List[unit].Current->Next = entry;
    }
    ...
    Hw_Log_List[unit].Current = entry;
    Hw_Log_List[unit].Current->Next = NULL;

    Hw_Log_List[unit].EntriesCounter++;
...
STATIC int _soc_hw_log_insert_entry_to_log(int unit, LogEntry_t *data)
{
    ...
    /* Check overflow. in case HW LOG is full --> Commit the changes */
    _soc_hw_log_check_overflow(unit);
...
/*    _soc_hw_log_print_list_memory_entry(unit, &entry.Data);*/

    /*Insert the log entry*/
    _soc_hw_log_insert_entry_to_log(unit, &entry);
        ...
        unsigned array_index, int copyno, int index, void *entry_data)
{
    ...
    if ((Hw_Log_List[unit].Head == NULL) || _soc_hw_log_log_list_is_empty(unit)) /* The List is not initialized or empty */
    {
        goto not_found;
    }
...
{
    if (Hw_Log_List[unit].EntriesCounter >= Hw_Log_List[unit].NumOfElements - 1)
    {
        LOG_ERROR(BSL_LS_SOC_HWLOG, (BSL_META_U(unit, "-------------FORCE COMMIT-------------\n")));
...
soc_hw_log_diagshow(int unit)
{
    LOG_ERROR(BSL_LS_SOC_HWLOG,
            (BSL_META_U(unit, "\nHW LOG Diagnostics:\n"
                    ...
                    (Hw_Log_List[unit].WorkingMode == HW_LOG_MODE_Disabled) ?
                            "" :
                            ((Hw_Log_List[unit].WorkingMode == HW_LOG_MODE_Commit_After_Each_Api) ?
                                    "Commit After Each Api" :
                                    "Commit Per User Demand")), Hw_Log_List[unit].HwLogSize, Hw_Log_List[unit].NumOfElements, Hw_Log_List[unit].EntriesCounter));
    return SOC_E_NONE;
}
