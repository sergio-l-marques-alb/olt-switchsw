...
soc_mem_dmaable(int unit, soc_mem_t mem, int copyno)
{
    ...
    if(SOC_IS_ARAD(unit)) {
        ...
        if (SOC_IS_DONE_INIT(unit) && BCM_UNIT_DO_HW_READ_WRITE(unit)){
            LOG_ERROR(BSL_LS_SOC_SOCMEM,
                      (BSL_META_U(unit,
                                  "Attempt to write DMA to %d (%s) table!!!!!!!!!!!!!!!!!!!!!!!! Not implemented in HW LOG!!!!!!!!!\n"),
                       mem,
                                ...
                                void *entry_data, void *entry_data_cache)
{
    ...
    if (soc_feature(unit, soc_feature_xy_tcam_28nm) &&
       (meminfo->flags & SOC_MEM_FLAG_CAM) &&
        ...
        } else if (SOC_MEM_FIELD_VALID(unit, mem, KEYf)) {
            key_field = KEYf;
            mask_field = MASKf;
        }
...
/*     Use crash recovery defined callback for access*/
    ...
    if (SOC_IS_DONE_INIT(unit))
    {
        ...
        {
            if(SOC_INFO(unit).ha_hw_log_access.mem_read)
            {

                return SOC_INFO(unit).ha_hw_log_access.mem_read(unit, mem, array_index, copyno, index, entry_data);
            }
        }
    }
 ...
 */
STATIC int
_soc_mem_write_sanity_check(int unit, soc_mem_t mem, int index)
{
    soc_mem_info_t *meminfo = &SOC_MEM_INFO(unit, mem);
                              ...
                              int blk, int index2)
{
    ...
    do {
        ...
        if (SOC_FAILURE(rv) && soc_feature(unit, soc_feature_shared_hash_mem)) {
            int rv1 = soc_ser_sram_correction(unit, SOC_PIPE_ANY,
    ...
    if (mem == L3_DEFIP_ALPM_IPV4m || mem == L3_DEFIP_ALPM_IPV4_1m ||
        ...
        if (soc_mem_field32_get(unit, mem, entry_data, VALIDf)) {
                       if (SOC_IS_TRIDENT2(unit)) {
                               ...
                               _soc_tomahawk_alpm_bkt_view_set(unit, index, mem);
                       }
#endif
        }
    }
 ...
 */
STATIC int
_soc_mem_write_schan_msg_create(int unit, soc_mem_t mem, int copyno,
               ...
               int index, void *entry_data)
{
    ...
    int rv = SOC_E_NONE;
    void *entry_data_ptr;
    static uint32 converted_entry_data[SOC_MAX_MEM_WORDS];
    static uint32 cache_entry_data[SOC_MAX_MEM_WORDS];

    SOC_IF_ERROR_RETURN(_soc_mem_write_sanity_check(unit, mem, index));
    ...
    rv = _soc_mem_write_schan_msg_create(unit, mem, copyno, array_index,
              ...
              int index, void *entry_data)
{
    ...
    if (SOC_IS_DONE_INIT(unit))
    {
        if (BCM_UNIT_DO_HW_READ_WRITE(unit))
        {
            if(SOC_INFO(unit).ha_hw_log_access.mem_write)
            {
/*
                LOG_CLI((BSL_META_U(unit, "soc_mem_array_write unit %d: %s.[%d] : 0x%X \n"),
                         unit, SOC_MEM_NAME(unit, mem), index, *(int *)entry_data));
*/

                return SOC_INFO(unit).ha_hw_log_access.mem_write(unit, mem, array_index, copyno, index, entry_data);
            }
        }
    }
...
_retry:
    ...
    if (SOC_IS_TOMAHAWK(unit)) {
        ...
        schan_msg.gencmd.address |= 
            (((banks_disable & 0xf) << 13) | (bucket_index & SOC_TH_ALPM_BKT_MASK));
    } else 
#endif /* BCM_TOMAHAWK_SUPPORT */
    {
...
_retry:
    ...
    if (SOC_IS_TOMAHAWK(unit)) {
        schan_msg.gencmd.address &= ~(0x0001ffff);
        schan_msg.gencmd.address |= 
            (((banks_disable & 0xf) << 13) | (bucket & SOC_TH_ALPM_BKT_MASK));
    } else 
#endif
    {
        schan_msg.gencmd.address &= ~(0x0003ffff);
...
#ifdef BCM_TOMAHAWK_SUPPORT
            if (SOC_IS_TOMAHAWK(unit)) {
                rv1 = (soc_tomahawk_hash_bank_number_get(unit, mem, bix,
                                                         &bank_ids[bix]));
            } else
#endif /* BCM_TOMAHAWK_SUPPORT */
            {
...
#endif /* BCM_TOMAHAWK_SUPPORT */
            {
                return soc_td2_vlan_xlate_entry_hash(unit, hash_sel,
                                                     entry_data);
            }
        } else
#endif /* BCM_TRIDENT2_SUPPORT */
                          ...
                          int recurse_depth)
{
    ...
    if (banks != SOC_MEM_HASH_BANK_ALL) {
        ...
        if (SOC_IS_TOMAHAWK(unit)) {
            SOC_IF_ERROR_RETURN
                (soc_tomahawk_hash_seq_number_get(unit, mem, banks,
                                                  &banks_seq_num));
        } else 
#endif /* BCM_TOMAHAWK_SUPPORT */
        {
            SOC_IF_ERROR_RETURN
                (soc_trident2_hash_seq_number_get(unit, mem, banks,
                                                  &banks_seq_num));
        }
    } else {
...
_soc_mem_shared_hash_init(int unit, soc_mem_t mem,
                          uint32 *num_bkt, SHR_BITDCL **trace,
                                  ...
                                  int recurse, int recurse_depth)
{
    ...
    if (NULL == bucket_trace) {
        SOC_IF_ERROR_RETURN(
            _soc_mem_shared_hash_init(unit, mem, &numb, &trace,
