...
soc_direct_reg_set(int unit, int cmic_block, uint32 addr, uint32 dwc_write, uint32 *data)
{
    ...
    if (SOC_IS_DONE_INIT(unit))
    {
        ...
        {
            if(SOC_INFO(unit).ha_hw_log_access.direct_reg_set)
            {
                return SOC_INFO(unit).ha_hw_log_access.direct_reg_set(unit, cmic_block, addr, dwc_write, data);
            }
        }
    }
...
soc_reg_get(int unit, soc_reg_t reg, int port, int index, uint64 *data)
{
    ...
    if (SOC_IS_DONE_INIT(unit))
    {
        if (BCM_UNIT_DO_HW_READ_WRITE(unit))
        {
            if(SOC_INFO(unit).ha_hw_log_access.soc_reg_get)
            {
                return SOC_INFO(unit).ha_hw_log_access.soc_reg_get(unit, reg, port, index, data);
            }
        }
    }
...
soc_reg_above_64_get(int unit, soc_reg_t reg, int port, int index, soc_reg_above_64_val_t data)
{
    ...
    if (SOC_IS_DONE_INIT(unit))
    {
        if (BCM_UNIT_DO_HW_READ_WRITE(unit))
        {
            if(SOC_INFO(unit).ha_hw_log_access.reg_above64_get)
            {
                return SOC_INFO(unit).ha_hw_log_access.reg_above64_get(unit, reg, port, index, data);
            }
        }
    }
...
soc_reg32_get(int unit, soc_reg_t reg, int port, int index, uint32 *data)
{
    ...
    if (SOC_IS_DONE_INIT(unit))
    {
        if (BCM_UNIT_DO_HW_READ_WRITE(unit))
        {
            if(SOC_INFO(unit).ha_hw_log_access.reg32_get)
            {
                return SOC_INFO(unit).ha_hw_log_access.reg32_get(unit, reg, port, index, data);
            }
        }
    }
...
soc_reg_set(int unit, soc_reg_t reg, int port, int index, uint64 data)
{
    ...
    if (SOC_IS_DONE_INIT(unit))
    {
        if (BCM_UNIT_DO_HW_READ_WRITE(unit))
        {
            if(SOC_INFO(unit).ha_hw_log_access.soc_reg_set)
            {
                return SOC_INFO(unit).ha_hw_log_access.soc_reg_set(unit, reg, port, index, data);
            }
        }
    }
...
soc_reg_set_nocache(int unit, soc_reg_t reg, int port, int index, uint64 data)
{
    ...
    if (SOC_IS_DONE_INIT(unit))
    {
        if (BCM_UNIT_DO_HW_READ_WRITE(unit))
        {
            if(SOC_INFO(unit).ha_hw_log_access.soc_reg_set_nocache)
            {
                return SOC_INFO(unit).ha_hw_log_access.soc_reg_set_nocache(unit, reg, port, index, data);
            }
        }
    }
...
soc_reg_above_64_set(int unit, soc_reg_t reg, int port, int index, soc_reg_above_64_val_t data)
{
    ...
    if (SOC_IS_DONE_INIT(unit))
    {
        if (BCM_UNIT_DO_HW_READ_WRITE(unit))
        {
            if(SOC_INFO(unit).ha_hw_log_access.reg_above64_set)
            {
                return SOC_INFO(unit).ha_hw_log_access.reg_above64_set(unit, reg, port, index, data);
            }
        }
    }
...
soc_reg32_set(int unit, soc_reg_t reg, int port, int index, uint32 data)
{
    ...
    if (SOC_IS_DONE_INIT(unit))
    {
        if (BCM_UNIT_DO_HW_READ_WRITE(unit))
        {
            if(SOC_INFO(unit).ha_hw_log_access.reg32_set)
            {
                return SOC_INFO(unit).ha_hw_log_access.reg32_set(unit, reg, port, index, data);
            }
        }
    }
...
soc_reg64_set(int unit, soc_reg_t reg, int port, int index, uint64 data)
{
    ...
    if (SOC_IS_DONE_INIT(unit))
    {
        if (BCM_UNIT_DO_HW_READ_WRITE(unit))
        {
            if(SOC_INFO(unit).ha_hw_log_access.reg64_set)
            {
                return SOC_INFO(unit).ha_hw_log_access.reg64_set(unit, reg, port, index, data);
            }
        }
    }
