...
#define SOC_AVS_SABER2_NUMBER_OF_CENTRALS 20
...
typedef struct sb2_avs_info_s {
    void *regulator_p;
    void *regulator_vcfg;
} sb2_avs_info_t;

static sb2_avs_info_t SB2_AVS_INFO[SOC_MAX_NUM_DEVICES];

/* The unit in Algorithm is 0.1mV while the unit in Regulator is uV*/
#define SB2_REGULATOR_SCALE  (100)
...
_soc_saber2_avs_voltage_set(int unit, uint32 val)
{
    ...
    uint32  dac_val, bg_val, rval;
    float dac_val_temp, set_volt;

    set_volt = (float)val / 10000;
    ...
    SOC_IF_ERROR_RETURN(WRITE_AVS_REG_PVT_MNTR_CONFIG_DAC_CODEr(unit, dac_val));
    SOC_IF_ERROR_RETURN(WRITE_AVS_REG_PVT_MNTR_CONFIG_MIN_DAC_CODEr(unit, dac_val));
    SOC_IF_ERROR_RETURN(WRITE_AVS_REG_PVT_MNTR_CONFIG_MAX_DAC_CODEr(unit, dac_val));
...
{
    uint32 reg_val[10], rval;
    int i, sum = 0;

    /* Take 10 samples and average */
    for(i = 0; i< 10; i++) {
        SOC_IF_ERROR_RETURN(READ_AVS_REG_RO_REGISTERS_0_PVT_1V_0_MNTR_STATUSr(
                    unit, &rval));
        reg_val[i] = soc_reg_field_get(unit, AVS_REG_RO_REGISTERS_0_PVT_1V_0_MNTR_STATUSr,
                rval, DATAf);
        sal_usleep(10000);
        sum += reg_val[i];
    }
    ...
    /* Calculate voltage */
    *val = ((sum / 10) * 880 * 1000) / (1024 * 70);

    return SOC_E_NONE;
}

#ifdef SB2_AVS_I2C
static i2c_regulator_volt_dac_param_t sb2_core_dac_param = {
    .uV_max = 530000,
    .uV_min = 600000,
};
static i2c_regulator_volt_config_t sb2_avs_ltc3880[] = {
    {"Core",                /* Voltage source to configure */
     I2C_LTC3880_CH0,       /* A/D channel # */
     "ltc3880_41",          /* I2C_LTC3880_41: ADC device name for volt get */
     "ltc3880_41",          /* I2C_LTC3880_41  ADC device name for volt set */ 
     I2C_MUX_3 ,            /* MUX device name */
     &sb2_core_dac_param    /* dac_param for Volt Limitation */
    }   
};
...
_soc_saber2_avs_voltage_set(int unit, uint32 val)
{
    ...
    hndl = SB2_AVS_INFO[unit].regulator_p;
    vcfg = SB2_AVS_INFO[unit].regulator_vcfg;  

    if ((hndl== NULL) || (vcfg == NULL)) {
        return SOC_E_INIT;
    }
    uV = val * (SB2_REGULATOR_SCALE / SOC_AVS_S1); 

    /* Calculating voltage based on feedback circuit */
    uV = uV * 549 / 1000; /* For 1V output, feedback output is .549V */
...
_soc_saber2_avs_voltage_get(int unit, uint32 *val)
{
    ...
    /* Calculating voltage based on feedback circuit */
    uV = uV * 1000 / 549; /* For 1V output, feedback output is .549V */

    *val = uV /(SB2_REGULATOR_SCALE/SOC_AVS_S1);
    return 0;
}

STATIC int 
_soc_saber2_regulator_volt_init(int unit, regulator_config_t *rcfg)
{
    regulator_volt_t *volt;
    int rv = SOC_E_NONE;

    volt = sal_alloc(sizeof(regulator_volt_t), "sb2_regulator_volt");
    sal_memset(volt, 0, sizeof(regulator_volt_t));
    if (volt != NULL) {
        volt->voltcfg_table = sb2_avs_ltc3880;
...
STATIC int 
_soc_saber2_regulator_volt_deinit(int unit, regulator_config_t *rcfg)
{
...
STATIC int
_soc_saber2_regulator_callback(int unit, regulator_func_t func,
        regulator_type_t type, regulator_config_t *rcfg)
{
    ...
    if (type == REGULATOR_TYPE_VOLT) {
        if (func == REGULATOR_FUNC_INIT) {
            rv = _soc_saber2_regulator_volt_init(unit, rcfg);
        } else if (func == REGULATOR_FUNC_DEINIT) {
            rv = _soc_saber2_regulator_volt_deinit(unit, rcfg);
        }
    }
    return rv;
}
...
STATIC int sb2_dac_devs_init (int unit, char *dev_name)
{
    ...
    if ((dac = bcm_i2c_open(unit, I2C_LTC3880_41,
                            flags, 0)) < 0) {
...
{    
    ...
    /* LTC 3880 device */
    sal_memset(&SB2_AVS_INFO[unit], 0, sizeof(sb2_avs_info_t));
    SOC_IF_ERROR_RETURN(
        soc_regulator_init(unit, REGULATOR_TYPE_VOLT, 
                           bb_saber2_regulator_callback, 
                           &(SB2_AVS_INFO[unit].regulator_p)));    
    hndl = SB2_AVS_INFO[unit].regulator_p;
    SOC_IF_ERROR_RETURN(
        soc_regulator_get_voltcfg_by_name(unit, 
            hndl, "Core", 
            &SB2_AVS_INFO[unit].regulator_vcfg));
#endif
