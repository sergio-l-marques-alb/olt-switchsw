/*
 * $Id: enduro.c,v 1.126 Broadcom SDK $
                                       ...
                                       char *msg, soc_block_t block)
{
    ...
    soc_reg_field_set(unit, group_reg, &group_status, info[table].error_field, 0);
    addr = soc_reg_addr(unit, group_reg, block_port, 0);
    SOC_IF_ERROR_RETURN(soc_reg32_write(unit, addr, group_status)); 
    return SOC_E_NONE;
}
                                          ...
                                          soc_port_t block_port, soc_block_t block)
{
    ...
    size = soc_reg_field_length(unit, mem_id_reg, STATUSf);
                 
    index_reg = info->intr_status0_reg;
    for (index = 0; index < size; index++, mem_bitmap >>= 1) {
        if (!(mem_bitmap & 1)) {
            continue;
        }
        addr = soc_reg_addr(unit, index_reg, block_port, index);
    ...
    soc_reg_field_set(unit, MEM_FAIL_INT_STATr, &status, TOQ0_IPMC_TBL_PAR_ERRf, 0);
    SOC_IF_ERROR_RETURN(WRITE_MEM_FAIL_INT_STATr(unit, status)); 
    return SOC_E_NONE;
}
                                          ...
                                          soc_port_t block_port, soc_block_t block)
{
    ...
    soc_reg_field_set(unit, MEM_FAIL_INT_STATr, &status, E2EFC_PAR_ERRf, 0);
    SOC_IF_ERROR_RETURN(WRITE_MEM_FAIL_INT_STATr(unit, status));
                                          ...
                                          soc_port_t block_port, soc_block_t block)
{
    ...
    soc_reg_field_set(unit, MEM_FAIL_INT_STATr, &status, MTRO_PAR_ERRf, 0);
    SOC_IF_ERROR_RETURN(WRITE_MEM_FAIL_INT_STATr(unit, status)); 
    /* Enable parity */
    SOC_IF_ERROR_RETURN(READ_MISCCONFIGr(unit, &val));
    soc_reg_field_set(unit, MISCCONFIGr, &val, PARITY_CHK_ENf, 1);
                                          ...
                                          soc_port_t block_port, soc_block_t block)
{
    ...
    soc_reg_field_set(unit, MEM_FAIL_INT_STATr, &status, WRED_PAR_ERRf, 0);
    SOC_IF_ERROR_RETURN(WRITE_MEM_FAIL_INT_STATr(unit, status)); 
    return SOC_E_NONE;
}
...
_soc_enduro_process_parity_error(int unit)
{
    ...
    uint32 group_enable, cpi_blk_bit;
            ...
            for (table = 0; info[table].error_field != INVALIDf; table++) {
                ...
                case _SOC_PARITY_INFO_TYPE_GENERIC:
                    if (info[table].enable_field == START_BY_START_ERR_ENf) {
                        ...
                        SOC_IF_ERROR_RETURN
						    (READ_MEM_FAIL_INT_STATr(unit, &rr));
                        rr &= ~0x200;
                        SOC_IF_ERROR_RETURN
						    (WRITE_MEM_FAIL_INT_STATr(unit, rr)); 
                    }
