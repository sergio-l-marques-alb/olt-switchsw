                              ...
                              uint32 *pspmap)
{
    ...
    *pspmap = 0;
    ...
    if (port > 41  && level == SOC_KT2_NODE_LVL_ROOT) {
        mem = LLS_S1_CONFIGm ;
    }
    if (mem == INVALIDm) {
...
_soc_kt2_dump_sched_at(int unit, int port, int level, int offset, int hw_index)
{
    ...
    int num_spri, first_child, first_mc_child, rv, cindex, cindex1;
    uint32 ucmap, spmap;
    soc_kt2_sched_mode_e sched_mode;
    soc_mem_t mem, mem1;
    int index_max, index_max1, ii, jj, ci, child_level, child_l1_level;
    int wt = 0, num_child, num_l1_child = 0;
    uint32 entry[SOC_MAX_MEM_WORDS], entry1[SOC_MAX_MEM_WORDS];
    ...
    for (ii = 0, ci = 0; ii <= index_max; ii++) {
        ...
        /* When LLS of CPU port is dumped, Index is 0 which is the same as
           invalid parent index. Validity of the index for CPU alone is checked
           by computing the number of children at the node at parent index 0 */
             
        if (port == 0) {
            if ((child_level == SOC_KT2_NODE_LVL_L0) || 
                    (child_level == SOC_KT2_NODE_LVL_L1)) {
            soc_kt2_get_child_type(unit, port, child_level, &child_l1_level);
            mem1 = _SOC_KT2_NODE_PARENT_MEM(child_l1_level);
            index_max1 = soc_mem_index_max(unit, mem1);
            num_l1_child = 0 ;
                               
            for (jj = 1; jj <= index_max1; jj++) {
                SOC_IF_ERROR_RETURN(
                           soc_mem_read(unit, mem1, MEM_BLOCK_ALL, jj, &entry1));
                cindex1 = soc_mem_field32_get(unit, mem1, entry1, C_PARENTf);
       ...
       if (cindex == hw_index)  {
            if (child_level == SOC_KT2_NODE_LVL_L2)  {
                if ((cindex == 0) && (num_child >= num_spri)) {
                    break;
                }
