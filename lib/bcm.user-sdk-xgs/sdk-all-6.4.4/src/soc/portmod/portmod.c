...
int _portmod_port_macro_internal_add(int unit , const portmod_pm_create_info_internal_t* pm_add_info, int *pm_created_id)
{
    ...
    _SOC_IF_ERR_EXIT(portmod_pm_init(unit, pm_add_info, pm_id + 1, &_pmm_info[unit]->pms[pm_id]));
    pm_initialized = TRUE;
...
#ifdef PORTMOD_PM4X25TD_SUPPORT
    if (PORTMOD_PM12x10_F_USE_PM4x25TD_GET(pm_add_info->pm_specific_info.pm12x10.flags)) {
...
int portmod_port_add(int unit, int port, const portmod_port_add_info_t *port_add_info)
{
    ...
    if(!PHYMOD_CORE_INIT_F_RESUME_AFTER_FW_LOAD_GET(port_add_info)) {
...
int portmod_ext_phy_attach(int unit, int phy, const phymod_core_access_t* ext_phy_access) {
    ...
    for( i = 0 ; i < nof_pms ; i++){
        _SOC_IF_ERR_EXIT(portmod_ext_phy_attach_to_pm(unit, pms_info[i], ext_phy_access));
    }
    SOC_FUNC_RETURN;
exit:
    ...
    --i;
    for( ; i >= 0 ; --i) {
        portmod_ext_phy_detach_from_pm(unit, pms_info[i], NULL);
    }
    SOC_FUNC_RETURN;
}
...
int portmod_ext_phy_detach(int unit, int phy)
{
    int i = 0, nof_pms = 0;
    pm_info_t pms_info[MAX_PMS_PER_PHY] = {NULL,};
    phymod_core_access_t ext_phys_access[MAX_PMS_PER_PHY];
    SOC_INIT_FUNC_DEFS;
    ...
    for( i = 0 ; i < nof_pms ; i++){
        _SOC_IF_ERR_EXIT(portmod_ext_phy_detach_from_pm(unit, pms_info[i], &ext_phys_access[i]));
    }
    SOC_FUNC_RETURN;
exit:
    /* Rollingback the detach operations because of an error */
    --i;
    for( ; i >= 0 ; --i) {
        portmod_ext_phy_attach_to_pm(unit, pms_info[i], &ext_phys_access[i]);
    }
    SOC_FUNC_RETURN;
}
