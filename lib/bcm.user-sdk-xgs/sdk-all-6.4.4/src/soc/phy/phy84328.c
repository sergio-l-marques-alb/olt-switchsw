...
_phy84328_init_ucode_bcst_load(int unit, int port)
{
	...
	sal_udelay(10); /* Wait for 10us */
        ...
        /* Write the address register with the value of the register adddress so that 
           we dont need to read for consecutive writes */
        SOC_IF_ERROR_RETURN(
                soc_esw_miimc45_addr_write(unit, pc->phy_id, PHY84328_C45_DEV_PMA_PMD, PHY84328_DEV1_MSG_IN));
	...
	for (j = 0; j < num_words; j+=2) {
	    ...
	    data16 = (fw_ptr[j] << 8) | fw_ptr[j+1];
            /* As the address register knows which register to write, just write the data consecutively */
            SOC_IF_ERROR_RETURN(
                    soc_esw_miimc45_data_write(unit, pc->phy_id, PHY84328_C45_DEV_PMA_PMD, data16));
