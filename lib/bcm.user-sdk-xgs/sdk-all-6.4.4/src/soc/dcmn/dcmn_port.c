...
soc_dcmn_port_phy_control_set(int unit, soc_port_t port, int phyn, int lane, int is_sys_side, soc_phy_control_t type, uint32 value)
{
    ...
    SOCDNX_IF_ERR_EXIT(portmod_port_phy_lane_access_get(unit, port, &params, 1, &phys, &phys_returned, NULL));
    switch (type)
    {
                 ...
                 uint32 phy_reg_addr, uint32 *phy_data)
{
    ...
    if ((flags & SOC_PHY_NOMAP) == 0){
        rv = portmod_port_phy_lane_access_get(unit, port, &params, 1, &phy_access, &nof_phys_structs, NULL);
        SOCDNX_IF_ERR_EXIT(rv);
    }
