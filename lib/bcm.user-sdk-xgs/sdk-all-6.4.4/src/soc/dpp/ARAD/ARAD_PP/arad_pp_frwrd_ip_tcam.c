    ...
    )
{
    ...
    *success = SOC_SAND_SUCCESS;
    ...
    if ((frwrd_table_id == ARAD_KBP_FRWRD_TBL_ID_LSR_IP_SHARED) || (frwrd_table_id == ARAD_KBP_FRWRD_TBL_ID_EXTENDED_IPV6) ||
		 (frwrd_table_id == ARAD_KBP_FRWRD_TBL_ID_EXTENDED_P2P) || (frwrd_table_id == ARAD_KBP_FRWRD_TBL_ID_IPV4_MC) ) {
        location.db_entry = db_entry;
        res = arad_sw_db_frwrd_ip_kbp_location_tbl_set(unit, data_indx, &location);
        SOC_SAND_CHECK_FUNC_RESULT(res, 90, exit);
    }
