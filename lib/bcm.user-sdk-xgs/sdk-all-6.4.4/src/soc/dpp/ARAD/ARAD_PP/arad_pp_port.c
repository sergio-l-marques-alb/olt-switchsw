  ...
  if (SOC_IS_JERICHO(unit)) {
	...
	SOC_DPP_CORES_ITER(SOC_CORE_ALL, core) {
		...
		for (pp_port = 0; pp_port < MAX_PORTS_IN_CORE ; pp_port++) {
			...
			fld_val = ( (core << ARAD_MAPPED_PP_PORT_CORE_BIT) & 0x100 ) | (pp_port & 0xff);
			...
			soc_IHP_VTT_IN_PP_PORT_CONFIGm_field_set(unit, in_port_tbl_data, MAPPED_PP_PORTf, &fld_val);
			...
			soc_IHP_PINFO_FLP_0m_field_set(unit, in_port_tbl_data, MAPPED_PP_PORTf, &fld_val);
			...
			soc_IHB_PINFO_PMFm_field_set(unit, in_port_tbl_data, MAPPED_PP_PORTf, &fld_val);
