     ...
     SOC_SAND_OUT SOC_SAND_SUCCESS_FAILURE   *success)
{
    ...
    ARAD_SW_DB_FP_ENTRY       sw_db_fp_entry;
  ...
  )
{
            ...
            if ((fp_database_info.flags & SOC_PPC_FP_DATABASE_INFO_FLAGS_USE_DBAL)) {
                if ((fp_database_info.flags & SOC_PPC_FP_DATABASE_INFO_FLAGS_HANDLE_ENTRIES_BY_KEY)) {                    
                    ...
                    if (physical_db_type == SOC_DPP_DBAL_PHYSICAL_DB_TYPE_LEM) {
                      ...
                      lem_payload.dest = action.value[0];
    ...
    if ((*success == SOC_SAND_SUCCESS) && (!entry.is_for_update)) {
        ...
        res = arad_sw_db_fp_db_entries_get(unit, stage, db_id_ndx, &sw_db_fp_entry);
        SOC_SAND_CHECK_FUNC_RESULT(res, 100, exit);

        sw_db_fp_entry.nof_db_entries++;

        res = arad_sw_db_fp_db_entries_set(unit, stage, db_id_ndx, &sw_db_fp_entry);
        SOC_SAND_CHECK_FUNC_RESULT(res, 110, exit);
    }
  ...
  arad_pp_fp_direct_table_entry_get(int unit, ARAD_FP_DATABASE_STAGE stage, uint32 db_id_ndx, uint32 entry_id_ndx, ARAD_PP_FP_ENTRY_INFO* info, uint8* is_found, uint32 value_in[ARAD_PP_FP_TCAM_ENTRY_SIZE], uint32 mask_in[ARAD_PP_FP_TCAM_ENTRY_SIZE], ARAD_TCAM_ACTION* action)
{
  ...
  res = arad_sw_db_tcam_db_action_bitmap_ndx_get(unit, ARAD_PP_FP_DB_ID_TO_TCAM_DB(db_id_ndx), &action_bitmap_ndx);
  SOC_SAND_CHECK_FUNC_RESULT(res, 15, exit);
  ...
  /* Find which bank this database uses */
  for (bank_ndx = 0; bank_ndx < SOC_DPP_DEFS_GET_TCAM_NOF_BANKS(unit); bank_ndx++) {
    res = arad_sw_db_tcam_db_bank_used_get(unit, ARAD_PP_FP_DB_ID_TO_TCAM_DB(db_id_ndx), bank_ndx, &is_used);
    SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);
    if (is_used) {
      break;
    }
  ...
  *is_found = FALSE;
  ...
  if (fp_database_info.db_type == ARAD_PP_FP_DB_TYPE_DIRECT_TABLE) {
    ...
    if (fp_database_info.flags & SOC_PPC_FP_DATABASE_INFO_FLAGS_HANDLE_ENTRIES_BY_KEY) {
      SOC_DPP_DBAL_PHYSICAL_DB_TYPES  physical_db_type;
      arad_pp_dbal_table_physical_db_get(unit, fp_database_info.internal_table_id, &physical_db_type);
      if((physical_db_type == SOC_DPP_DBAL_PHYSICAL_DB_TYPE_SEM_A) || (physical_db_type == SOC_DPP_DBAL_PHYSICAL_DB_TYPE_SEM_B)) {
        ...
        res = arad_pp_dbal_entry_get(unit, fp_database_info.internal_table_id, (info->qual_vals), (void *)(&action), &(info->priority), &hit_bit, is_found);
      }
    } else{      
