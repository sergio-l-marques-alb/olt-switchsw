      ...
      )
{
    ...
    *fec_id = ((payload >> (32 - JER_KAPS_AD_WIDTH_IN_BITS)) & ~(1 << 17));
    ...
    )
{
    ...
    dest_val = (payload >> (32 - JER_KAPS_AD_WIDTH_IN_BITS));
    if (dest_val & (1 << 17)) {
        route_info->dest_id.dest_type = SOC_SAND_PP_DEST_FEC;
        route_info->dest_id.dest_val = dest_val & ~(1 << 17);
    }
    else if (dest_val & (1 << 18)) {
        route_info->dest_id.dest_type = SOC_SAND_PP_DEST_MULTICAST;
        route_info->dest_id.dest_val = dest_val & ~(1 << 18);
    }
  ...
  )
{
  ...
  SOC_SAND_ERR_IF_ABOVE_NOF(info->inrif, SOC_DPP_DEFS_GET(unit, nof_rifs), ARAD_PP_RIF_ID_OUT_OF_RANGE_ERR, 12, exit);
