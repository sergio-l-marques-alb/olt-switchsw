...
#define ARAD_PP_LB_MPLS_LBL1_VLD(vc_indx) \
  ((vc_indx== ARAD_PP_LB_VECTOR_INDEX_MPLSX1)||(vc_indx== ARAD_PP_LB_VECTOR_INDEX_MPLSX2_LABEL1)||(vc_indx==ARAD_PP_LB_VECTOR_INDEX_MPLSX3_LABEL1))

#define ARAD_PP_LB_MPLS_LBL2_VLD(vc_indx) \
  ((vc_indx >= ARAD_PP_LB_VECTOR_INDEX_MPLSX2_LABEL1) && (vc_indx <= ARAD_PP_LB_VECTOR_INDEX_MPLSX3_LABEL2))

#define ARAD_PP_LB_MPLS_LBL3_VLD(vc_indx) \
  ((vc_indx >= ARAD_PP_LB_VECTOR_INDEX_MPLSX3_LABEL1) && (vc_indx <= ARAD_PP_LB_VECTOR_INDEX_MPLSX3_LABEL3))
...
CONST STATIC uint8 Arad_pp_lb_key_chunk_size[ARAD_PP_LB_NOF_VECTOR_INDEXES] =
  ...
  )
{
  ...
  {
      ihb_lb_pfc_profile_tbl.lb_vector_index_1   = Arad_pp_lb_hash_map_table[idx].index1;
      ihb_lb_pfc_profile_tbl.lb_vector_index_2   = Arad_pp_lb_hash_map_table[idx].index2;
      ihb_lb_pfc_profile_tbl.lb_vector_index_3   = Arad_pp_lb_hash_map_table[idx].index3;
      ihb_lb_pfc_profile_tbl.lb_vector_index_4   = Arad_pp_lb_hash_map_table[idx].index4;
      ihb_lb_pfc_profile_tbl.lb_vector_index_5   = Arad_pp_lb_hash_map_table[idx].index5;
  ...
  )
{
  ...
  for (entry_idx = 0; entry_idx < soc_ppd_lag->nof_entries; entry_idx++)
  {
    ...
    tm_lag->lag_member_sys_ports[entry_idx].flags     = soc_ppd_lag->members[entry_idx].flags;
  }
}
  ...
  )
{
  ...
  ARAD_ALLOC(tm_lag_info, ARAD_PORTS_LAG_INFO, 1, "arad_pp_lag_set_unsafe.tm_lag_info");
  ...
  )
{
  ...
  for (vc_indx = ARAD_PP_LB_VECTOR_INDEX_NONE; vc_indx < ARAD_PP_LB_NOF_VECTOR_INDEXES; ++vc_indx)
  {
    /*
     * update Ethernet key
     */
    ...
    lb_vector_program_map_tbl_data.chunk_size = Arad_pp_lb_key_chunk_size[vc_indx];
    ...
    for (mask_idx = 0; mask_idx < ARAD_PP_NOF_HASH_MASKS; ++mask_idx)
    {
      ...
      if(Arad_pp_lag_hash_field_info[field_info_offset].key_index == vc_indx)
      {
        skip = FALSE;
      }
      if ((SOC_SAND_BIT(mask_idx) == ARAD_PP_HASH_MASKS_MPLS_LABEL_1) && (ARAD_PP_LB_MPLS_LBL1_VLD(vc_indx)))
      {
        skip = FALSE;
      }
      else if ((SOC_SAND_BIT(mask_idx) == ARAD_PP_HASH_MASKS_MPLS_LABEL_2) && (ARAD_PP_LB_MPLS_LBL2_VLD(vc_indx)))
      {
        skip = FALSE;
      }
      else if ((SOC_SAND_BIT(mask_idx) == ARAD_PP_HASH_MASKS_MPLS_LABEL_3) && (ARAD_PP_LB_MPLS_LBL3_VLD(vc_indx)))
      {
        skip = FALSE;
      }
