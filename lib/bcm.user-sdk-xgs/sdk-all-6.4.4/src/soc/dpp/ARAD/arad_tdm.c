...
STATIC uint32
arad_tdm_local_to_reassembly_context_get(int unit, uint32 port)
{
    if (SOC_IS_JERICHO(unit)) {
      return port;
    } else {
      return arad_sw_db_local_to_reassembly_context_get(unit,port);
    }
}
  ...
  )
{
  ...
  reassembly_context = arad_tdm_local_to_reassembly_context_get(unit,port_ndx);
  ...
  )
{
    ...
    /* Tables retreive by reassembly context */
    reassembly_context = arad_tdm_local_to_reassembly_context_get(unit, port);
  ...
  )
{
  ...
  if (configure_ingress) {
      ...
      uint32 reassembly_context = arad_tdm_local_to_reassembly_context_get(unit, port_ndx);
