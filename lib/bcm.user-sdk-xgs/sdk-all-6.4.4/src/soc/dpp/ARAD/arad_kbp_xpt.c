...
)
{
    ...
    res = arad_kbp_rop_read(aradxpt->unit, &rd_data);
    if (SOC_SAND_KBP_ROP_FAILURE(res)) {
        *o_reason = NLMRSC_OPR_FAILURE;
        return;
    }   
...
)
{
    ...
    res = arad_kbp_rop_write(aradxpt->unit, &wr_data);
    if (SOC_SAND_KBP_ROP_FAILURE(res)) {
        *o_reason = NLMRSC_OPR_FAILURE;
        return;
    }
...
)
{
    ...
    res = arad_kbp_rop_blk_write(aradxpt->unit, &wr_data);
    if (SOC_SAND_KBP_ROP_FAILURE(res)) {
        *o_reason = NLMRSC_OPR_FAILURE;
        return;
    }
...
{
    return arad_kbp_xpt_init(unit, alloc,max_rqt_count,max_rslt_count,chan_id);
}
