    ...
    )
{
  ...
  key[1] = (uint8) ((location->bank_id << ARAD_TCAM_LOCATION_ENCODING_BANK_LSB_IN_SECOND_UINT8) | (location->entry >> 8));
    ...
    )
{
  ...
  location->entry   = (((uint32) key[1] & ((1 << (ARAD_TCAM_LOCATION_ENCODING_BANK_LSB_IN_SECOND_UINT8))-1)) << 8) | key[0];
    ...
    )
{
    ...
    res = arad_sw_db_tcam_db_access_profile_id_get(unit, tcam_db_id, 0, &access_profile_id);
    SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
    ...
    SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
    res = arad_sw_db_tcam_no_insertion_priority_order_get(unit, tcam_db_id, &no_insertion_priority_order);
    ...
    )
{
    ...
    res = arad_sw_db_tcam_sparse_priorities_get(unit, tcam_db_id, &sparse_priorities);
    ...
    )
{
  ...
  res = arad_sw_db_tcam_bank_entry_size_get(
          unit,
          bank_id,
          &entry_size
        );
    ...
    )
{
  ...
  res = arad_sw_db_tcam_db_valid_get(
          unit,
          tcam_db_id,
          &valid
        );
  ...
  for (bank_iter = 1; (bank_iter < nof_banks_to_consider) && (*found == FALSE); bank_iter++)
  {
      ...
      if ((bank_curr < bank_min) || (bank_curr > bank_max))
      {
          ...
          *found = FALSE;
          ...
          res = arad_sw_db_tcam_db_bank_nof_entries_get(unit, tcam_db_id, bank_curr, &nof_entries);
          SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
    ...
    )
{
  ...
  if (has_db_entries)
  {
      res = arad_sw_db_tcam_db_bank_nof_entries_get(unit, tcam_db_id, location->bank_id, &nof_entries);
      SOC_SAND_CHECK_FUNC_RESULT(res, 5, exit);
      if(nof_entries == 0) {
  ...
  *found = FALSE;
  ...
  if (is_found == TRUE)
  {
      ...
      for (bank_ndx = 0; (bank_ndx < nof_banks) && (is_found == FALSE); bank_ndx++) {
          ...
          res = arad_sw_db_tcam_db_bank_nof_entries_get(unit, tcam_db_id, bank_id, &nof_entries);
          SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
          if (nof_entries == 0) {
          ...
          if (is_found)
          {
              ...
              SOC_SAND_CHECK_FUNC_RESULT(res, 5, exit);
              res = arad_sw_db_tcam_global_location_tbl_get(unit, global_location_id, &global_location);
              SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
   ...
   )
{
  ...
  if (!is_bank_share_update) {
      ...
      entries_free--;
      res = arad_sw_db_tcam_bank_nof_entries_free_set(
        unit, 
        bank_id, 
        entries_free);
      SOC_SAND_CHECK_FUNC_RESULT(res, 60, exit);
    ...
    )
{
  ...
  res = arad_sw_db_tcam_global_location_tbl_set(unit, global_location_id, &global_location);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  ...
  if (!prio_node_found) {
    ...
    if ((prio_location.entry_id_first != entry_id) && (prio_location.entry_id_last != entry_id)) {
        ...
        if (is_update_prio) {
            ...
            res = soc_sand_sorted_list_entry_remove_by_iter(prio_list, iter);
    ...
    )
{
  ...
  res = soc_sand_occ_bm_is_occupied(bank_bm, location->entry, &bit_set);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
    ...
    )
{
  ...
  res = soc_sand_occ_bm_occup_status_set(bank_bm, location->entry, bit_set);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
    ...
    )
{
  ...
  res = arad_sw_db_tcam_db_location_tbl_set(unit, data_indx, location);
  SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
  ...
  )
{
  ...
  if (*found)
  {
      ...
      res = arad_sw_db_tcam_db_location_tbl_get(unit, data_indx, location);
      SOC_SAND_CHECK_FUNC_RESULT(res, 12, exit);
    ...
    else
    {
      ...
      *distance = arad_tcam_bank_entry_size_to_entry_count_get(unit, entry_size, low->bank_id) - low->entry;
      for (bank_ndx = (low->bank_id + 1); bank_ndx < high->bank_id; bank_ndx++) {
          res = arad_sw_db_tcam_db_bank_used_get(unit, tcam_db_id, bank_ndx, &is_used);
          SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
          if (is_used) {
    ...
    )
{
  ...
  SOC_SAND_CHECK_FUNC_RESULT(res, 5, exit);
  res = arad_sw_db_tcam_global_location_tbl_get(unit, global_location_id_source, &global_location);
  ...
  SOC_SAND_CHECK_FUNC_RESULT(res, 7, exit);
  res = arad_sw_db_tcam_global_location_tbl_set(unit, global_location_id_dest, &global_location);
  SOC_SAND_CHECK_FUNC_RESULT(res, 8, exit);
  ARAD_TCAM_GLOBAL_LOCATION_clear(&global_location_clear);
  res = arad_sw_db_tcam_global_location_tbl_set(unit, global_location_id_source, &global_location_clear);
  SOC_SAND_CHECK_FUNC_RESULT(res, 9, exit);
  ...
  /* Update the location table */
  res = arad_sw_db_tcam_db_location_tbl_set(unit, data_indx, destination);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  ...
  if (set_hw_also) {
      ...
      res = arad_sw_db_tcam_db_bank_nof_entries_get(unit, tcam_db_id, source->bank_id, &bank_nof_entries);
      SOC_SAND_CHECK_FUNC_RESULT(res, 77, exit);
      bank_nof_entries --;
      res = arad_sw_db_tcam_db_bank_nof_entries_set(unit, tcam_db_id, source->bank_id, bank_nof_entries);
      SOC_SAND_CHECK_FUNC_RESULT(res, 77, exit);

      res = arad_sw_db_tcam_db_bank_nof_entries_get(unit, tcam_db_id, destination->bank_id, &bank_nof_entries);
      SOC_SAND_CHECK_FUNC_RESULT(res, 77, exit);
      bank_nof_entries ++;
      res = arad_sw_db_tcam_db_bank_nof_entries_set(unit, tcam_db_id, destination->bank_id, bank_nof_entries);
    ...
    )
{
  ...
  for (bank_ndx = 0; bank_ndx < nof_banks; bank_ndx++) {
      ...
      /* Use the old method for IP TCAM and 80b and Direct table because more efficient */
      res = arad_sw_db_tcam_bank_has_direct_table_get(unit, bank_id, &has_direct_table);
      SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
      if(has_direct_table) {
        move_per_block = TRUE;
      } else {
  ...
  )
{
  ...
  *bank_owner = ARAD_TCAM_NOF_BANK_OWNERS;
  ...
  {
      res = arad_sw_db_tcam_db_bank_used_get(unit, tcam_db_ndx, bank_id, &is_used);
      SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);

      if(is_used)
      {
          /* tcam db entry size is required in order to get the access profile array index */
          res = arad_sw_db_tcam_db_entry_size_get(unit, tcam_db_ndx, &entry_size);
          SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
          ...
          res = arad_sw_db_tcam_db_access_profile_id_get(unit, tcam_db_ndx, access_array_ndx, &access_profile_id);
          SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
  ...
  )
{
   ...
   */
  res = arad_sw_db_tcam_bank_valid_get(
          unit,
          bank_id,
          &valid
        );
  ...
  else
  {
    ...
    for (is_inverse = 0; is_inverse < ARAD_TCAM_NOF_OCCUPATIONS_BITMAPS; is_inverse++) {
            ...
            && entry_size == old_entry_size)
        {
           ...
           */
          soc_sand_occ_bm_clear(occ_bm[is_inverse]);
        }
        else
        {
          ...
          soc_sand_occ_bm_destroy(occ_bm[is_inverse]);
          create_bitmap = TRUE;
        }
    }
  }
  ...
  nof_entries = arad_tcam_bank_entry_size_to_entry_count_get(
                    ...
                    bank_id);
  res = arad_sw_db_tcam_bank_nof_entries_free_set(unit, bank_id, nof_entries);
  SOC_SAND_CHECK_FUNC_RESULT(res, 23, exit);
  ...
  res = arad_sw_db_tcam_bank_valid_set(
    unit,
    bank_id,
    TRUE
  );
  ...
  )
{
   ...
   */
  priorities = arad_sw_db_tcam_db_priorities_get(unit, tcam_db_id);
  ...
  priorities->init_info.key_size      = ARAD_TCAM_DB_LIST_KEY_SIZE * sizeof(uint8);
  priorities->init_info.data_size     = ARAD_TCAM_DB_LIST_DATA_SIZE * sizeof(uint8);
  ...
  priorities->init_info.wb_var_index  = SOC_DPP_WB_ENGINE_VAR_TCAM_DB_PRIORITY + (tcam_db_id * WB_ENGINE_SORTED_LIST_INNER_VARS_NUM); 
  priorities->init_info.unit     = unit;

  res = soc_sand_os_memset(&sorted_list_restore_info, 0, sizeof(sorted_list_restore_info));
  SOC_SAND_CHECK_FUNC_RESULT(res, 666, exit);

  /* save info that is used for wb restoration */
  sorted_list_restore_info.init_info = priorities->init_info;
  ...
  res = soc_dpp_wb_engine_enable_dynamic_sorted_list(unit, SOC_DPP_WB_ENGINE_VAR_TCAM_DB_PRIORITY + (tcam_db_id * WB_ENGINE_SORTED_LIST_INNER_VARS_NUM), *priorities);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 25, exit);

  res = arad_sw_db_tcam_db_entry_size_set(
    unit,
    tcam_db_id,
    entry_size
  );
  ...
  res = arad_sw_db_tcam_db_prefix_size_set(
    unit,
    tcam_db_id,
    prefix_size
  );
  ...
  res = arad_sw_db_tcam_db_is_direct_set(
    unit, 
    tcam_db_id, 
    is_direct
  );
  ...
  res = arad_sw_db_tcam_use_small_banks_set(
    unit, 
    tcam_db_id, 
    use_small_banks
  );
  ...
  res = arad_sw_db_tcam_no_insertion_priority_order_set(
    unit, 
    tcam_db_id, 
    no_insertion_priority_order
  );
  ...
  res = arad_sw_db_tcam_sparse_priorities_set(
    unit, 
    tcam_db_id, 
    sparse_priorities
  );
  ...
  for (bank_id = 0; bank_id < SOC_DPP_DEFS_GET_TCAM_NOF_BANKS(unit); ++bank_id)
  {
    res = arad_sw_db_tcam_db_bank_used_set(
      unit,
      tcam_db_id,
      bank_id,
      FALSE
  ...
  res = arad_sw_db_tcam_db_valid_set(
    unit,
    tcam_db_id,
    TRUE
  );
  ...
  )
{
  ...
  res = soc_sand_sorted_list_destroy(prio_list);
  SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);

  res = arad_sw_db_tcam_db_valid_set(unit, tcam_db_id, FALSE);
  SOC_SAND_CHECK_FUNC_RESULT(res, 50, exit);
  ...
  )
{
   ...
   */
  res = arad_sw_db_tcam_db_entry_size_get(unit, tcam_db_id, &db_entry_size);
  SOC_SAND_CHECK_FUNC_RESULT(res, 25, exit);
  res = arad_sw_db_tcam_bank_entry_size_get(unit, bank_id, &bank_entry_size);
   ...
   */
  if(is_direct) {
      res = arad_sw_db_tcam_bank_has_direct_table_set(
        unit, 
        bank_id, 
        is_direct
      );
  ...
  res = arad_sw_db_tcam_db_prefix_set(
    unit,
    tcam_db_id,
    prefix
  );
  ...
  if (access_profile_array_id == 0) {
      ...
      res = soc_sand_occ_bm_create(&occ_bm_init_info, &occ_bm);
      SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
  ...
  )
{
  ...
  if (is_used)
  {
     ...
     */
    arad_sw_db_tcam_db_prefix_get(unit, tcam_db_id, &prefix);
    ...
    {
      res = arad_sw_db_tcam_managed_bank_prefix_db_set(unit, bank_id, base | ndx, ARAD_TCAM_MAX_NOF_LISTS);
      SOC_SAND_CHECK_FUNC_RESULT(res, 6, exit);
    }

      res = arad_sw_db_tcam_db_bank_used_set(unit, tcam_db_id, bank_id, FALSE);
      SOC_SAND_CHECK_FUNC_RESULT(res, 6, exit);
      ...
      if(has_direct_table)
      {
          ...
          if(is_direct) {
              res = arad_sw_db_tcam_bank_has_direct_table_set(unit, bank_id, FALSE);
              SOC_SAND_CHECK_FUNC_RESULT(res, 7, exit);
          }
      }

      /* Reset the SW DB for the number of entries per bank */
      res = arad_sw_db_tcam_db_bank_nof_entries_set(unit, tcam_db_id, bank_id, 0 /* bank_nof_entries */);
      SOC_SAND_CHECK_FUNC_RESULT(res, 8, exit);

      /*
       *  If the bank is not used at all now, mark it uninitialized
       */
      res = arad_sw_db_tcam_managed_bank_nof_db_get(unit, bank_id, &nof_dbs);
      SOC_SAND_CHECK_FUNC_RESULT(res, 8, exit);
      res = arad_sw_db_tcam_managed_bank_nof_db_set(unit, bank_id, --nof_dbs);
      SOC_SAND_CHECK_FUNC_RESULT(res, 8, exit);
      if (nof_dbs == 0)
      {
        res = arad_sw_db_tcam_bank_valid_set(unit, bank_id, FALSE);
        SOC_SAND_CHECK_FUNC_RESULT(res, 9, exit);
  ...
  )
{
  ...
  if (*success == SOC_SAND_SUCCESS)
  {
     ...
     */
      ...
      /* Update the SW DB for the number of entries per bank */
      res = arad_sw_db_tcam_db_bank_nof_entries_get(unit, tcam_db_id, location.bank_id, &bank_nof_entries);
      SOC_SAND_CHECK_FUNC_RESULT(res, 53, exit);
      bank_nof_entries ++;
      res = arad_sw_db_tcam_db_bank_nof_entries_set(unit, tcam_db_id, location.bank_id, bank_nof_entries);
      SOC_SAND_CHECK_FUNC_RESULT(res, 55, exit);
  ...
  )
{
  ...
  *found = FALSE;
  ...
  {
    res = arad_sw_db_tcam_db_bank_used_get(unit, tcam_db_id, current.bank_id, &is_used);
    SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
    if (is_used)
    {
      /*
  ...
  )
{
  ...
  while (found)
  {
      ...
      res = arad_sw_db_tcam_bank_nof_entries_free_get(unit, bank_id_to, &nof_entries_free);
      SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(0);
    ...
    res = arad_sw_db_tcam_bank_nof_entries_free_get(unit, bank_id, &nof_entries_free);
    SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
    for (bank_idx = 0; bank_idx < SOC_DPP_DEFS_GET_TCAM_NOF_BANKS(unit); ++bank_idx)
    {
        ...
        res = arad_sw_db_tcam_db_bank_used_get(unit, tcam_db_id, bank_idx, &is_used);
        SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
        if (!is_used) {
        ...
        res = arad_sw_db_tcam_db_bank_nof_entries_get(unit, tcam_db_id, bank_idx, &(nof_entries_db[is_bank_upper]));
        SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
    }
    ...
    )
{
  ...
  res = arad_sw_db_tcam_db_bank_used_get(unit, tcam_db_id, location_curr.bank_id, &is_used);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit); 
  if(is_used){
      res = arad_tcam_db_bank_occ_bitmap_entry_get(
                unit,
