    ...
    SOC_SAND_OUT  uint8  *enable)
{
    ...
    {
        SOCDNX_IF_ERR_EXIT(READ_EGQ_GENERAL_RQP_DEBUG_CONFIGr(unit, core, &reg_val32));
        enable_curr = soc_reg_field_get(unit, EGQ_GENERAL_RQP_DEBUG_CONFIGr, reg_val32, DBG_FDA_STOPf);
        enable_all = enable_all && SOC_SAND_NUM2BOOL(enable_curr);
    }
    

    /*IRR enable*/
    SOCDNX_IF_ERR_EXIT(READ_IRR_DYNAMIC_CONFIGURATIONr(unit, &reg_val32));

    enable_curr = soc_reg_field_get(unit, IRR_DYNAMIC_CONFIGURATIONr, reg_val32, ENABLE_DATA_PATH_IDR_0f);
    enable_all = enable_all && SOC_SAND_NUM2BOOL(enable_curr);

    enable_curr = soc_reg_field_get(unit, IRR_DYNAMIC_CONFIGURATIONr, reg_val32, ENABLE_DATA_PATH_IDR_1f);
    enable_all = enable_all && SOC_SAND_NUM2BOOL(enable_curr);

    enable_curr = soc_reg_field_get(unit, IRR_DYNAMIC_CONFIGURATIONr, reg_val32, ENABLE_DATA_PATH_IQM_0f);
    enable_all = enable_all && SOC_SAND_NUM2BOOL(enable_curr);

    enable_curr = soc_reg_field_get(unit, IRR_DYNAMIC_CONFIGURATIONr, reg_val32, ENABLE_DATA_PATH_IQM_1f);
    enable_all = enable_all && SOC_SAND_NUM2BOOL(enable_curr);

    enable_curr = soc_reg_field_get(unit, IRR_DYNAMIC_CONFIGURATIONr, reg_val32, ENABLE_DATA_PATH_0f);
    enable_all = enable_all && SOC_SAND_NUM2BOOL(enable_curr);

    enable_curr = soc_reg_field_get(unit, IRR_DYNAMIC_CONFIGURATIONr, reg_val32, ENABLE_DATA_PATH_1f);
    enable_all = enable_all && SOC_SAND_NUM2BOOL(enable_curr);
    

    /*IDR enable*/
    SOCDNX_IF_ERR_EXIT(READ_IDR_DYNAMIC_CONFIGURATIONr(unit, &reg_val32));
    enable_curr = soc_reg_field_get(unit, IDR_DYNAMIC_CONFIGURATIONr, reg_val32, ENABLE_DATA_PATHf);
    enable_all = enable_all && SOC_SAND_NUM2BOOL(enable_curr);
