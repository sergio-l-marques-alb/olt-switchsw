    ...
    int unit)
{
    ...
    if (SOC_DFE_CONFIG(unit).fabric_cell_fifo_dma_enable != SOC_DFE_PROPERTY_UNAVAIL && SOC_DFE_CONFIG(unit).fabric_cell_fifo_dma_enable)
    {
        for (channel = SOC_DFE_DEFS_GET(unit, fifo_dma_fabric_cell_first_channel);
              ...
              channel++)
        {
            rc = soc_dfe_fifo_dma_channel_init(unit, channel);
            BCMDNX_IF_ERR_EXIT(rc);
        }
       ...
       case bcmFabricIsolate:
        ...
        case bcmFabricShutdown:
            if (arg) /* shutdown */
            {
                ...
                /*Shut down device*/
                rc = soc_dfe_drv_graceful_shutdown_set(unit, pbmp_active_links, 1, pbmp_unisolated_links, 0); 
                BCMDNX_IF_ERR_EXIT(rc);            
                ...
                {
                    rc = soc_dfe_drv_graceful_shutdown_set(unit, pbmp_active_links, 0, pbmp_unisolated_links, device_isolation_status == soc_dcmn_isolation_status_isolated ? 1:0);
                    BCMDNX_IF_ERR_EXIT(rc);
                       
                } else {
                    rc = soc_dfe_drv_graceful_shutdown_set(unit, PBMP_SFI_ALL(unit), 0, PBMP_SFI_ALL(unit), 0);
                    BCMDNX_IF_ERR_EXIT(rc);                
                }
                isolate_type = soc_dfe_fabric_isolate_type_none;
    ...
    int *value)
{
    ...
    for (i = 0 ; i<count ; i++)
    {
        switch (type[i])
        {
            ...
            case bcmFabricLinkMidFifoSize:
                rc = MBCM_DFE_DRIVER_CALL(unit, mbcm_dfe_fabric_flow_control_mid_fifo_size_threshold_set, (unit, type_idx, fabric_pipe, is_fe1, is_fe3, value[i]));
                BCMDNX_IF_ERR_EXIT(rc);
                break; 
            ...
            case bcmFabricLinkTxAlmostFull:
                rc = MBCM_DFE_DRIVER_CALL(unit, mbcm_dfe_fabric_flow_control_tx_almost_full_threshold_set, (unit, type_idx, fabric_pipe, is_fe1, is_fe3, value[i]));
                BCMDNX_IF_ERR_EXIT(rc);
                break;

            case bcmFabricLinkTxFifoSize:
                rc = MBCM_DFE_DRIVER_CALL(unit, mbcm_dfe_fabric_flow_control_tx_fifo_size_threshold_set, (unit, type_idx, fabric_pipe, is_fe1, is_fe3, value[i]));
                BCMDNX_IF_ERR_EXIT(rc);
                break;
    ...
    bcm_fabric_link_remote_pipe_mapping_t *mapping_config)
{
    ...
    else
    {
        ...
        {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Size of buffer shold be bigger than the number of remote pipes")));
        }
