    ...
    soc_field_t *base_idx_field)
{
    ...
    *base_idx_field=FLEX_CTR_BASE_COUNTER_IDXf;
    ...
    case EGR_IP_TUNNEL_MPLSm:
         *offset_mode_field = 
                       ip_tunnel_mpls_entry_counter_fields[index].offset_mode;
         *pool_number_field = 
                       ip_tunnel_mpls_entry_counter_fields[index].pool_number;
         *base_idx_field = 
                       ip_tunnel_mpls_entry_counter_fields[index].base_idx;
         break;
#endif /* BCM_TRIDENT2PLUS_SUPPORT */
    default:
         break;
       ...
       ingress_entry_data = soc_mem_table_idx_to_pointer(unit, ingress_table,
        ...
        if (SOC_IS_TOMAHAWK(unit)) {
            if ((IFP_POLICY_TABLEm == ingress_table) ||
                ...
                uint32 count, stat_counter_id, index;
                ...
                /* bcmStatGroupModeFlex1 is used as a dummy place holder arg.
                 * color_map_set() below needs a valid "custom" stat_counter_id
                 * in order to program the color map databse. The only sanity
                 * check that is done by color_map_set() on the group field of the
                 * stat id is group >= bcmStatGroupModeFlex1 &&
                 * group <= bcmStatGroupModeFlex4
                 */
                _bcm_esw_stat_get_counter_id(bcmStatGroupModeFlex1,
 ...
 */
static void
_bcm_esw_stat_flex_check_ingress_ifp_policy_table(int unit)
{
    uint32    index=0;
    ...
    rv = _bcm_field_th_group_oper_mode_get(unit, bcmFieldQualifyStageIngress, &oper_mode);
    if (BCM_FAILURE(rv)) {
       LOG_ERROR(BSL_LS_BCM_FLEXCTR,
                 (BSL_META_U(unit,
                             "Failed to get Group Operation Mode[%d]"),
                             rv));
       return;
    }
...
bcm_error_t _bcm_esw_stat_flex_cleanup(int unit)
{
    ...
    for (direction=bcmStatFlexDirectionIngress;direction<2;direction++) {
         ...
         if (soc_feature(unit, soc_feature_use_flex_ctr_oam_lm)) {
             if (direction == bcmStatFlexDirectionIngress) {
                 for (pool_group_id=0;
                      pool_group_id < BCM_STAT_FLEX_COUNTER_MAX_GROUPS;
                      pool_group_id++) {
                     if (BCM_E_PARAM ==  
                     ...
                     flex_pool_group[unit][pool_group_id].group_type =
                          BCM_FLEX_POOL_GROUP_USED_BY_NONE;
                     flex_pool_group[unit][pool_group_id].free_pools  =
                          BCM_STAT_FLEX_COUNTER_POOLS_PER_GROUP;
                     flex_pool_group[unit][pool_group_id].base_pool = 
              ...
              if (SOC_IS_TOMAHAWK(unit)) {
                  ...
                  if (bcmStatFlexDirectionIngress == direction) {
                      if (NULL != ifp_color_map[unit][pool_id]) {
                          soc_cm_sfree(unit, ifp_color_map[unit][pool_id]);
                          ifp_color_map[unit][pool_id] = NULL;
                      }
                  }
              }
#endif
...
bcm_error_t _bcm_esw_stat_flex_init(int unit)
{
    ...
    for (direction = bcmStatFlexDirectionIngress;
         ...
         for (pool_id = 0; pool_id < num_pools[direction]; pool_id++) {
              ...
              flex_pool_stat[unit][direction][pool_id].used_entries = 0;
              flex_pool_stat[unit][direction][pool_id].attached_entries = 0;
              SHR_BITCLR_RANGE(flex_pool_stat[unit][direction][pool_id].used_by_objects,
                               0, bcmStatObjectMaxValue);
#if defined(BCM_TOMAHAWK_SUPPORT)
              if (SOC_IS_TOMAHAWK(unit)) {
                  /* Ingress pools 12 - 16 are reserved for use by IFP only.
                  ...
                  if (bcmStatFlexDirectionIngress == direction &&
                      ...
                      flex_pool_stat[unit][direction][pool_id].used_by_tables =
                          FLEX_COUNTER_POOL_USED_BY_IFP_POLICY_TABLE;
                      SHR_BITSET(flex_pool_stat[unit][direction][pool_id].used_by_objects,
                                  bcmStatObjectIngFieldStageIngress);
                  }
                  ...
                  if (bcmStatFlexDirectionIngress == direction) {
                      ...
                      sal_memset(ifp_color_map[unit][pool_id], 0,
              ...
              sal_memset(flex_base_index_reference_count[unit][direction]
                         [pool_id], 0, sizeof(uint16) * num_ctr);
    ...
    if (soc_mem_read(unit, egress_table, MEM_BLOCK_ANY,
        ...
        if (flex_base_index_reference_count[unit][bcmStatFlexDirectionEgress]
            ...
            if (_bcm_esw_stat_flex_insert_stat_id(
                ...
                LOG_VERBOSE(BSL_LS_BCM_FLEXCTR,
                            (BSL_META_U(unit,
                                        "WARMBOOT: Could not add entry in scache memory."
                                        "Attach it\n")));
            }
        }
        return BCM_E_NONE;
            ...
            uint32               pool_number)
{
    ...
    if (flex_pool_stat[unit][bcmStatFlexDirectionIngress][pool_number].
...
#endif
        {
             ...
             */
            if (soc_feature(unit, soc_feature_use_flex_ctr_oam_lm)) {
                pool_group_id = pool_number/BCM_STAT_FLEX_COUNTER_MAX_GROUPS;
                flex_pool_group[unit][pool_group_id].free_pools++; 
                if(flex_pool_group[unit][pool_group_id].free_pools
                            ==  BCM_STAT_FLEX_COUNTER_POOLS_PER_GROUP) {
                    BCM_IF_ERROR_RETURN(
                           _bcm_esw_stat_flex_oam_enable_ingress_group(
                                                              unit,
            ...
            void                 *ingress_entry_data1)
{
    ...
    if (SOC_IS_TOMAHAWK(unit) &&
        ...
        for (ctr_idx = 0;
             ctr_idx < flex_ingress_modes[unit][mode].total_counters;
             ctr_idx++) {
            if (0 != ifp_color_map[unit][pool_number][base_idx + ctr_idx]) {
                for (color_mask = 0; color_mask < BCM_MAX_IFP_COLORS; color_mask++) {
                    switch (ifp_color_map[unit][pool_number][base_idx + ctr_idx]
                        ...
                        default:
                            if (0 != (color_map_mask[color_mask] &
                                ifp_color_map[unit][pool_number]
                                [base_idx + ctr_idx])) {
                                return BCM_E_PARAM;
                            }
                            break;
            ...
            uint32               *pool_number)
{
    ...
    if (soc_feature(unit, soc_feature_use_flex_ctr_oam_lm)) {
        ...
        for ( pool_group_id = 0;                      
              pool_group_id < BCM_STAT_FLEX_COUNTER_MAX_GROUPS; 
              pool_group_id++) {                      
            if ((flex_pool_group[unit][pool_group_id].group_type == 
                                                    allocate_group_type) &&
                  flex_pool_group[unit][pool_group_id].free_pools) {
                 default_pool_number = pool_number_l =  
                                flex_pool_group[unit][pool_group_id].base_pool;
        ...
        _bcm_ifp_color_map_t  *color_map)
{
    ...
    if (base_index >= soc_mem_index_count(unit, _ctr_counter_table
                                                [bcmStatFlexDirectionIngress]
                                                [pool_number])) {
        return BCM_E_PARAM;
    }
    ...
    if ((0 == total_counters) ||
        (total_counters != flex_ingress_modes[unit]
        [offset_mode].total_counters)) {
    ...
    for (ctr_idx = 0; ctr_idx < total_counters; ctr_idx++) {
        if (NULL == ifp_color_map[unit][pool_number]) {
            /* This should not happen. Error during init ? */
            return BCM_E_INTERNAL;
        }
        ifp_color_map[unit][pool_number][base_index + ctr_idx] =
        ...
        _bcm_ifp_color_map_t  *color_map)
{
    ...
    for (ctr_idx = 0; ctr_idx < total_counters; ctr_idx++) {
        color_map->map[ctr_idx] = ifp_color_map[unit][pool_number]
                                  [base_index + ctr_idx];
    }
    return BCM_E_NONE;
}
