...
_bcm_sb2_ing_service_pri_map_create(int unit, uint32 flags, int *map_id)
{
    ...
    sal_memset(service_pri_map_table, 0, sizeof(service_pri_map_table));
    entries[0] = &service_pri_map_table;
    ...
    case _BCM_QOS_MAP_TYPE_SERVICE_PRI_MAP_TABLE:
        if (soc_feature(unit, soc_feature_oam_service_pri_map)) {
            if (!_BCM_QOS_SERVICE_PRI_MAP_TABLE_USED_GET(unit, id)) {
                ...
                /* Modify what's needed */
                entry = &service_pri_map_table[map->int_pri];
                soc_ING_SERVICE_PRI_MAPm_field32_set(unit, entry, 
                        OFFSETf, offset);
                ...
                /* Add new chunk and store new HW index */
                entries[0] = service_pri_map_table;
                rv = _bcm_service_pri_map_table_entry_add(unit, entries, 
                        _BCM_QOS_MAP_CHUNK_SERVICE_PRI_MAP,
                        (uint32 *)&index);
...
int _bcm_sb2_ing_service_pri_map_info_get(int unit, int pri_map_id,
                                          int *hw_idx, int *pri_mode)
{
    if (soc_feature(unit, soc_feature_oam_service_pri_map)) {
        if (_BCM_QOS_SERVICE_PRI_MAP_TABLE_USED_GET(unit, pri_map_id)) {
            *hw_idx = QOS_INFO(unit)->service_pri_map_hw_idx[pri_map_id]; 
            *pri_mode = _BCM_QOS_SERVICE_PRI_MAP_TABLE_FLAGS_GET(unit, pri_map_id); 
            return BCM_E_NONE;
        }
    } else {
        return BCM_E_UNAVAIL;
