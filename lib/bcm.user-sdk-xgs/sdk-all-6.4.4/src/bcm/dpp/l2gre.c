...
_bcm_petra_l2_gre_resources_get(
    ...
    *fec_index = -1;
    ...
    if(ingress) {
        ...
        rv = _bcm_dpp_gport_to_global_and_local_lif(unit,l2gre_port->match_tunnel_id,global_in_lif, local_in_lif,NULL,NULL,&is_local);
        BCMDNX_IF_ERR_EXIT(rv);
    }
    ...
    if(egress) {
        ...
        if (l2gre_port->egress_tunnel_id != 0) {
            /* get match tunnel (terminator) */
        rv = _bcm_dpp_gport_to_global_and_local_lif(unit,l2gre_port->egress_tunnel_id,global_out_lif,NULL,local_out_lif,NULL,&is_local);
            BCMDNX_IF_ERR_EXIT(rv);
        }
    }
...
bcm_petra_l2gre_port_sw_state_add(
    ...
    int update)
 {
    ...
    /* update lif usage */
    if (local_in_lif != -1) {
        rv = _bcm_dpp_in_lif_usage_set(unit, local_in_lif, _bcmDppLifTypeL2Gre);
        BCMDNX_IF_ERR_EXIT(rv);
    ...
    if (local_out_lif != -1) {
        /* Update outLIF usage */
        rv = _bcm_dpp_out_lif_usage_set(unit,local_out_lif,_bcmDppLifTypeL2Gre);
        BCMDNX_IF_ERR_EXIT(rv);
    }
