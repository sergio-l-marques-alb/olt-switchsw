...
#define TRUNK_CHECK_INIT(u_, il_)                                               \
    do {                                                                        \
        ...
        if (_init_state < (il_)) return BCM_E_INIT;                             \
    } while (0);
                    ...
                    if (!TRUNK_PORTCNT_VALID(lag_info->nof_entries + 1, nports)) {
                        ...
                        if (BCM_FAILURE(rv)) {
                            ...
                            LOG_ERROR(BSL_LS_BCM_TRUNK,
                                    ...
                                    BCMDNX_IF_ERR_EXIT(TRUNK_ACCESS.t_info.in_use.set(unit, tid, TRUE)); /* set trunk in use for a case it dosn't set yet */
                        }
                    }
                }            
        } else {
...
bcm_petra_trunk_detach(int unit)
{
    ...
    BCMDNX_IF_ERR_EXIT(TRUNK_ACCESS.ngroups.set(unit, 0));
    BCMDNX_IF_ERR_EXIT(TRUNK_ACCESS.stk_ngroups.set(unit, 0));
    BCMDNX_IF_ERR_EXIT(TRUNK_ACCESS.nports.set(unit, 0));
    ...
    BCMDNX_IF_ERR_EXIT(TRUNK_ACCESS.t_info.free(unit));
    ...
    BCMDNX_IF_ERR_EXIT(TRUNK_ACCESS.free(unit));
...
#endif
    {
        BCMDNX_IF_ERR_EXIT(TRUNK_ACCESS.ngroups.set(unit, _ngroups));
        BCMDNX_IF_ERR_EXIT(TRUNK_ACCESS.nports.set(unit, _nports));
    }
