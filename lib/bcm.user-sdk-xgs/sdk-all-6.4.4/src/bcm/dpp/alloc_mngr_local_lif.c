...
_bcm_dpp_local_lif_is_wide_entry(int unit, int local_lif, int is_ingress, int *is_wide_entry)
{
    ...
    if (!SOC_IS_JERICHO(unit)) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_CONFIG, (_BSL_BCM_MSG("wide entries can be configured in the lif table only in jericho and above devices")));
    ...
    if (is_ingress) {
        ...
        if (inlif_info.local_lif_flags & BCM_DPP_AM_IN_LIF_FLAG_WIDE){
            *is_wide_entry = 1;
        }
    } else {
        bcm_dpp_am_local_out_lif_info_t outlif_info;
        ...
        if (outlif_info.local_lif_flags & BCM_DPP_AM_OUT_LIF_FLAG_WIDE){
            *is_wide_entry = 1;
        }
    }
