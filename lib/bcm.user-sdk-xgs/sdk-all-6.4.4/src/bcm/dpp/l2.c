                                   ...
                                   SHR_LLM_SERVICE_TYPE *serv_type)
{
    ...
    rv = _bcm_dpp_gport_to_global_and_local_lif(unit, gport_id, NULL, (int*)&lif_id, NULL, &fec_id, &is_local);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    *restore = 0;
    ...
    if (trvrs_flags & _BCM_PETRA_TRAVERSE_MATCH_PORT) {
        ...
        if (!_BCM_DPP_PORT_ENCAP_INVALID(match_l2addr->encap_id)) {
			...
			if(BCM_GPORT_IS_MODPORT(match_l2addr->port)){ /* mod-port*/
                             ...
                             bcm_l2_learn_limit_t *limit)
{
    ...
    /* In case of LIF limit, the existing VSI limit table is used. Therefore, the LIF value should be
       mapped to the VSI value, according to exact mapping that is done by the HW */
        ...
        rv = _bcm_dpp_gport_to_global_and_local_lif(unit, limit->port, NULL, &lif_id, NULL, NULL, &is_local);
        BCMDNX_IF_ERR_EXIT(rv);
...
bcm_petra_l2_egress_create(int unit, bcm_l2_egress_t *egr)
{
    ...
    /*if (SOC_IS_JERICHO(unit) && (egr->flags & BCM_L2_EGRESS_WIDE)) {
        local_lif_flags |= BCM_DPP_AM_OUT_LIF_FLAG_WIDE;
    }*/
    ...
    if (!update) {
        ...
        if (SOC_IS_JERICHO(unit)) {
            ...
            if (alloc_flags &  SHR_RES_ALLOC_WITH_ID) {
                rv = _bcm_dpp_lif_id_is_mapped(unit, FALSE, global_outlif, &is_mapped);
                BCMDNX_IF_ERR_EXIT(rv);
            }
            if (is_mapped) {
                ...
                local_out_lif_info.glif = global_outlif;
    ...
    /* 3.5 Write global lif mapping to the glem. */
    if (SOC_IS_JERICHO(unit) && is_mapped && !update) {
    ...
    bcm_l2_egress_t *egr)
{
    ...
    _bcm_lif_type_e usage;
    int global_outlif = 0, local_outlif = 0;  /* used to get allocated outlif/eedb index */
    int is_wide_entry = 0; /* indicates whether lif is in wide entry */
    ...
    if (SOC_IS_JERICHO(unit)) {
        rv = _bcm_dpp_local_lif_is_wide_entry(unit, local_outlif, 0, &is_wide_entry);
        BCMDNX_IF_ERR_EXIT(rv);
