...
err_code_t blackhawk_tsc_stop_rx_adaptation(srds_access_t *sa__, uint8_t enable) {
  ...
  if (enable) {
      ...
      if(err_code) {
          ...
          blackhawk_tsc_INTERNAL_stop_micro(sa__,0,&err_code);
      }
      return(err_code);
  }
  else {
...
err_code_t blackhawk_tsc_read_rx_afe(srds_access_t *sa__, enum srds_rx_afe_settings_enum param, int8_t *val) {
  /* Assumes the micro is not actively tuning */
    ...
    switch(param) {
    case RX_AFE_PF:
        EFUN(blackhawk_tsc_INTERNAL_get_rx_pf_main(sa__, val));
        break;
    case RX_AFE_PF2:
        EFUN(blackhawk_tsc_INTERNAL_get_rx_pf2(sa__, val));
        break;
    case RX_AFE_VGA:
        EFUN(blackhawk_tsc_INTERNAL_get_rx_vga(sa__, val));
      break;
    ...
    case RX_AFE_DFE14:
        ...
        break;
    case RX_AFE_PF3:
        return(blackhawk_tsc_INTERNAL_get_rx_pf3(sa__, val));
    default:
        return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
    }
...
err_code_t blackhawk_tsc_ext_loop_timing(srds_access_t *sa__, uint8_t enable)
{
    ...
    EFUN(wr_tx_pi_sampler_max_cnt(0x7));
...
/*********************************/
static err_code_t _blackhawk_tsc_get_micro_num_uc_cores(srds_access_t *sa__, uint8_t *num_micros) {
    ESTM(*num_micros = rdc_micro_num_uc_cores());
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk_tsc_uc_reset_with_info(srds_access_t *sa__, uint8_t enable, ucode_info_t ucode_info) {
  if (enable) {
    ...
    EFUN(wrc_uc_active(0));
    {
        uint8_t micro_orig, num_micros, micro_idx;
        ESTM(micro_orig = blackhawk_tsc_get_micro_idx(sa__));
        EFUN(_blackhawk_tsc_get_micro_num_uc_cores(sa__, &num_micros));
        for (micro_idx = 0; micro_idx < num_micros; micro_idx++) {
            EFUN(blackhawk_tsc_set_micro_idx(sa__, micro_idx));
    ...
    EFUN(blackhawk_tsc_pmd_wr_reg(sa__, 0xD228, 0x0000)); /* RAM CONFIG */
    EFUN(blackhawk_tsc_pmd_wr_reg(sa__, 0xD229, 0x0000));
    EFUN(blackhawk_tsc_pmd_wr_reg(sa__, 0xD22B, 0x0001));
...
err_code_t blackhawk_tsc_init_blackhawk_tsc_info(srds_access_t *sa__) {
    ...
    ENULL_MEMSET(blackhawk_tsc_info_ptr, 0, sizeof(blackhawk_tsc_info_t));

    if (blackhawk_tsc_info_ptr == 0) {
        EFUN_PRINTF(("ERROR:  Info table pointer is null.\n"));
        return (_error(ERR_CODE_INFO_TABLE_ERROR));
    }

    EFUN(blackhawk_tsc_rdblk_uc_prog_ram(sa__, (uint8_t *)info_table, INFO_TABLE_RAM_BASE, INFO_TABLE_END));
