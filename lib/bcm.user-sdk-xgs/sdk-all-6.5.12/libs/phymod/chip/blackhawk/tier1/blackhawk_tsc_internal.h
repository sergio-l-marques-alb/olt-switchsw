...
/** Base of core variable block, BLACKHAWK variant. */
#define CORE_VAR_RAM_BASE (0x400)
/** Base of lane variable block, BLACKHAWK variant. */
#define LANE_VAR_RAM_BASE (0x480)
/** Size of lane variable block, BLACKHAWK variant. */
#define LANE_VAR_RAM_SIZE (0x340)
 ...
 * @param sa__ is an opaque state vector passed through to devide access functions.
 ...
 * @param timeout_value is the 5 bits value to program the hardware timeout configuration register
 * @return Error code, if generated (returns ERR_CODE_NONE if no errors)
 */
 ...
 * @param *div Divider value (enumerated), or BLACKHAWK_TSC_PLL_DIV_UNKNOWN to auto-compute.
 ...
 * @param sa__ is an opaque state vector passed through to device access functions.
 * @param *div Divider value, in the same encoding as enum #blackhawk_tsc_pll_div_enum.
 * @return Error Code, if generated (returns ERR_CODE_NONE if no errors)
 */
err_code_t blackhawk_tsc_INTERNAL_read_pll_div(srds_access_t *sa__, uint32_t *div);
...
static const int blackhawk_tsc_endian_const = 1;
#define is_big_endian() ((*(char*)&blackhawk_tsc_endian_const) == 0)
