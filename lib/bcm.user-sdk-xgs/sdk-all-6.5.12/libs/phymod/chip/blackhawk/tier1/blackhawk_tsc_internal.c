...
static blackhawk_tsc_info_t blackhawk_tsc_info;

blackhawk_tsc_info_t *blackhawk_tsc_INTERNAL_get_blackhawk_tsc_info_ptr(void) {
    return &blackhawk_tsc_info;
}

err_code_t blackhawk_tsc_INTERNAL_verify_blackhawk_tsc_info(blackhawk_tsc_info_t const *test_info)
{
...
err_code_t blackhawk_tsc_INTERNAL_get_BER_string(srds_access_t *sa__, uint16_t time_ms, char *string) {
    ...
    PHYMOD_MEMSET(&ber_data_local, 0, sizeof(struct ber_data_st));

    if(string == NULL) {
        return(ERR_CODE_BAD_PTR_OR_INVALID_INPUT);
    }

    EFUN(blackhawk_tsc_INTERNAL_get_BER_data(sa__, time_ms, &ber_data_local));
    ...
    COMPILER_64_SET(val64, 0, (uint32_t)3);
    /* if((ber_data_local.num_errs < 3) && (ber_data_local.lcklost == 0)) { */  /* lcklost = 0 */
    if ((COMPILER_64_LT(ber_data_local.num_errs, val64)) && (ber_data_local.lcklost == 0)) {
        USR_STRNCAT(string2,"<",1);
        /* ber_data_local.num_errs = 3; */
        COMPILER_64_SET(ber_data_local.num_errs, 0, (uint32_t)3);
    } else {
        USR_STRNCAT(string2," ",1);
    }
...
err_code_t blackhawk_tsc_INTERNAL_get_BER_data(srds_access_t *sa__, uint16_t time_ms, struct ber_data_st *ber_data) {
    uint8_t lcklost = 0;
    uint32_t err_cnt= 0;
    ...
    /* Only get lock lost status using 10ms data */
    ...
    if(ber_data->lcklost == 0) {
        ...
        uint8_t prbs_chk_burst_err_cnt_en = 0;
        uint8_t prbs_chk_en_timer_mode = 0;
        uint8_t prbs_chk_en_timeout = 0;
        uint16_t time_ms_adjusted = 0;

        /* save prbs hardware timer config registers */
        ESTM(prbs_chk_burst_err_cnt_en = rd_prbs_chk_burst_err_cnt_en());
        ESTM(prbs_chk_en_timer_mode = rd_prbs_chk_en_timer_mode());
        ESTM(prbs_chk_en_timeout = rd_prbs_chk_en_timeout());
        ...
        EFUN(blackhawk_tsc_INTERNAL_get_prbs_timeout_count_from_time(sa__, time_ms, &time_ms_adjusted, &timeout_value));
        ...
        /* wait time_ms + 10ms for hardware timeout */
        EFUN(USR_DELAY_MS(time_ms_adjusted+10));
        EFUN(blackhawk_tsc_prbs_err_count_state(sa__, &err_cnt,&lcklost));

        /* restore prbs hardware timer config registers */
        EFUN(wr_prbs_chk_burst_err_cnt_en(prbs_chk_burst_err_cnt_en));
        EFUN(wr_prbs_chk_en_timer_mode(prbs_chk_en_timer_mode));
        EFUN(wr_prbs_chk_en_timeout(prbs_chk_en_timeout));
        ...
        COMPILER_64_SET(ber_data->num_bits, 0, (uint32_t)1);
        COMPILER_64_UMUL_32(ber_data->num_bits, time_ms_adjusted);
        COMPILER_64_UMUL_32(ber_data->num_bits, num_bits_per_ms);
...
err_code_t blackhawk_tsc_INTERNAL_get_prbs_timeout_count_from_time(srds_access_t *sa__, uint16_t time_ms, uint16_t * time_ms_adjusted, uint8_t * timeout_value) {
    uint8_t iter = 0;

    if(time_ms > 384) {
        *timeout_value = 31;
        *time_ms_adjusted = 448;
        return ERR_CODE_NONE;
    }
...
err_code_t blackhawk_tsc_INTERNAL_pam4_to_bin(char var, char bin[]) {
    if(!bin) {
  ...
  switch (var) {
    ...
    default :  ENULL_STRCPY(bin,"");
               ...
               return (_error(ERR_CODE_CFG_PATT_INVALID_PAM4));
  }
  return (ERR_CODE_NONE);
}

err_code_t blackhawk_tsc_INTERNAL_compute_bin(char var, char bin[]) {
    if(!bin) {
        return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
    }
...
err_code_t blackhawk_tsc_INTERNAL_compute_hex(char bin[],uint8_t *hex) {
  if(!hex) {
    return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
  }
...
err_code_t blackhawk_tsc_INTERNAL_read_event_log_with_callback(srds_access_t *sa__,
    ...
    EFUN(blackhawk_tsc_INTERNAL_verify_blackhawk_tsc_info(blackhawk_tsc_info_ptr));
...
uint32_t blackhawk_tsc_INTERNAL_float12_to_uint32(uint8_t byte, uint8_t multi) {
...
err_code_t blackhawk_tsc_INTERNAL_get_rx_pf_main(srds_access_t *sa__, int8_t *val) {
...
err_code_t blackhawk_tsc_INTERNAL_get_rx_pf2(srds_access_t *sa__, int8_t *val) {
...
err_code_t blackhawk_tsc_INTERNAL_get_rx_pf3(srds_access_t *sa__, int8_t *val) {
    ESTM(*val = (int8_t)blackhawk_tsc_INTERNAL_gray_to_uint8(rd_ams_rx_pf3_ctrl()));
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk_tsc_INTERNAL_get_rx_vga(srds_access_t *sa__, int8_t *val) {
    ESTM(*val = (int8_t)rd_rx_vga_status());
    return (ERR_CODE_NONE);
}
...
#if defined(BLACKAWK_SINAI)
    EFUN(wr_rx_afe_override_sel(3));                     /* Select data23 slicer */
#else
...
err_code_t blackhawk_tsc_INTERNAL_get_rx_pam_mode(srds_access_t *sa__, enum blackhawk_tsc_rx_pam_mode_enum *pmode) {
    ...
    if (rx_pam4_mode == 0) {
        ...
        *pmode = (rx_pam4_es_mode) ? PAM4_ES : PAM4_NS;
        ...
        return(_error(ERR_CODE_INVALID_RX_PAM_MODE));
    }
...
err_code_t blackhawk_tsc_INTERNAL_read_lane_state(srds_access_t *sa__, blackhawk_tsc_lane_state_st *istate) {  
  ...
  EFUN(blackhawk_tsc_INTERNAL_get_rx_pf_main(sa__, &state.pf_main));
  state.pf_hiz = 0;
  EFUN(blackhawk_tsc_INTERNAL_get_rx_pf2(sa__, &state.pf2_ctrl));
  EFUN(blackhawk_tsc_INTERNAL_get_rx_pf3(sa__, &state.pf3_ctrl));
  EFUN(blackhawk_tsc_INTERNAL_get_rx_vga(sa__, &state.vga));
  ...
  ESTM(state.heye_left = blackhawk_tsc_INTERNAL_eye_to_mUI(sa__, rdv_usr_sts_heye_left()));
  ESTM(state.heye_right = blackhawk_tsc_INTERNAL_eye_to_mUI(sa__, rdv_usr_sts_heye_right()));
  ESTM(state.veye_upper = blackhawk_tsc_INTERNAL_eye_to_mV(sa__, rdv_usr_sts_veye_upper(), ladder_range));
  ESTM(state.veye_lower = blackhawk_tsc_INTERNAL_eye_to_mV(sa__, rdv_usr_sts_veye_lower(), ladder_range));
...
err_code_t blackhawk_tsc_INTERNAL_calc_patt_gen_mode_sel(uint8_t *mode_sel, uint8_t *zero_pad_len, uint8_t patt_length) {
...
static err_code_t _blackhawk_tsc_get_refclk_in_hz(enum blackhawk_tsc_pll_refclk_enum refclk, uint32_t *refclk_in_hz) {
    switch (refclk) {
 ...
 */
static err_code_t _blackhawk_tsc_get_divisor_ratio(enum blackhawk_tsc_pll_div_enum div, uint16_t *numerator, uint16_t *denominator)
{
    switch (div) {
...
static err_code_t _blackhawk_tsc_get_refclk_from_div_vco(uint32_t *refclk_freq_hz, enum blackhawk_tsc_pll_div_enum div, uint32_t vco_freq_khz, enum blackhawk_tsc_pll_option_enum pll_option) {
    uint16_t numerator, denominator;
    EFUN(_blackhawk_tsc_get_divisor_ratio(div, &numerator, &denominator));
...
err_code_t blackhawk_tsc_INTERNAL_get_vco_from_refclk_div(uint32_t refclk_freq_hz, enum blackhawk_tsc_pll_div_enum div, uint32_t *vco_freq_khz, enum blackhawk_tsc_pll_option_enum pll_option) {
    uint16_t numerator, denominator;
    EFUN(_blackhawk_tsc_get_divisor_ratio(div, &numerator, &denominator));
    if (pll_option == BLACKHAWK_TSC_PLL_OPTION_REFCLK_DOUBLER_EN) refclk_freq_hz *= 2;
    if (pll_option == BLACKHAWK_TSC_PLL_OPTION_REFCLK_DIV2_EN)    refclk_freq_hz /= 2;
...
static err_code_t _blackhawk_tsc_check_div(enum blackhawk_tsc_pll_div_enum div) {
    uint8_t i, found = 0;
    for (i=0; i<_blackhawk_tsc_div_candidates_count; i++) {
        found |= div == _blackhawk_tsc_div_candidates[i];
    }
    if (!found) {
...
static err_code_t _blackhawk_tsc_check_vco_freq_khz(uint32_t vco_freq_khz) {
...
static err_code_t _blackhawk_tsc_get_div(uint32_t refclk_freq_hz, uint32_t vco_freq_khz, enum blackhawk_tsc_pll_div_enum *div, enum blackhawk_tsc_pll_option_enum pll_option) {
    int32_t vco_freq_khz_min_error = 0x7FFFFFFF;
    uint8_t i;
    for (i=0; i<_blackhawk_tsc_div_candidates_count; i++) {
        uint32_t actual_vco_freq_khz = 0;
        int32_t  vco_freq_khz_error;
        EFUN(blackhawk_tsc_INTERNAL_get_vco_from_refclk_div(refclk_freq_hz, _blackhawk_tsc_div_candidates[i], &actual_vco_freq_khz, pll_option));
        ...
        if (vco_freq_khz_min_error > vco_freq_khz_error) {
            ...
            *div = _blackhawk_tsc_div_candidates[i];
        }
    }
...
err_code_t blackhawk_tsc_INTERNAL_resolve_pll_parameters(enum blackhawk_tsc_pll_refclk_enum refclk,
                                                  uint32_t *refclk_freq_hz,
                                                  enum blackhawk_tsc_pll_div_enum *div,
    ...
    const uint8_t given_param_count = (((refclk == BLACKHAWK_TSC_PLL_REFCLK_UNKNOWN) ? 0 : 1)
                                       + ((*div == BLACKHAWK_TSC_PLL_DIV_UNKNOWN) ? 0 : 1)
                                       + ((*vco_freq_khz == 0) ? 0 : 1));
    ...
    if (*div != BLACKHAWK_TSC_PLL_DIV_UNKNOWN) {
        EFUN(_blackhawk_tsc_check_div(*div));
    }

    /* Verify that the requested VCO frequency is allowed. */
    if (*vco_freq_khz != 0) {
        EFUN(_blackhawk_tsc_check_vco_freq_khz(*vco_freq_khz));
    }

    if (refclk == BLACKHAWK_TSC_PLL_REFCLK_UNKNOWN) {
        /* Determine refclk from vco frequency and div */
        EFUN(_blackhawk_tsc_get_refclk_from_div_vco(refclk_freq_hz, *div, *vco_freq_khz, pll_option));
    } else {
        EFUN(_blackhawk_tsc_get_refclk_in_hz(refclk, refclk_freq_hz));
    }

    if (*vco_freq_khz == 0) {
        /* Determine VCO frequency from refclk and divider */
        EFUN(blackhawk_tsc_INTERNAL_get_vco_from_refclk_div(*refclk_freq_hz, *div, vco_freq_khz, pll_option));
    }
     ...
     */
    EFUN(_blackhawk_tsc_get_div(*refclk_freq_hz, *vco_freq_khz, &auto_div, pll_option));
    if (*div == BLACKHAWK_TSC_PLL_DIV_UNKNOWN) {
    ...
    if ((original_vco_freq_khz != 0)
        ...
        EFUN_PRINTF(("ERROR: Could not achieve requested VCO frequency of %d kHz.\n       Refclk is %d Hz, %s option enabled, and auto-determined divider is 0x%08X, yielding a VCO frequency of %d kHz.\n",
                     original_vco_freq_khz, *refclk_freq_hz, pll_option_e2s[pll_option], *div, *vco_freq_khz));
        return (_error(ERR_CODE_VCO_FREQUENCY_INVALID));
    }
    ...
    if (auto_div != *div) {
        EFUN_PRINTF(("ERROR: Conflicting PLL parameters:  refclk is %d Hz, %s option enabled, divider is 0x%08X, and VCO frequency is %d kHz.\n       Expected divider is 0x%08X\n",
                     *refclk_freq_hz, pll_option_e2s[pll_option], *div, *vco_freq_khz, auto_div));
        return (_error(ERR_CODE_CONFLICTING_PARAMETERS));
    }
...
err_code_t blackhawk_tsc_INTERNAL_display_pll_to_divider(srds_access_t *sa__, uint32_t div) {
...
#ifdef EMULATION_EN
        if (is_diag_emu_ready(sa__->diag_em_sts)) {

            /* Read the leading byte, if starting at an odd address. */
            if ((addr & 1) == 1) {
                read_val = trace_mem[addr];
                addr++;
            ...
            while (block_cnt >= 2) {
                read_val = (trace_mem[addr]) | (trace_mem[addr+1] << 8);
                addr+=2;
