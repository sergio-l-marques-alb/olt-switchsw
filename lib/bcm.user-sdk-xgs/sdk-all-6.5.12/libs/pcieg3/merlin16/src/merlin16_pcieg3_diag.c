...
static err_code_t merlin16_pcieg3_poll_diag_data(srds_access_t *sa__, const merlin16_pcieg3_info_t *merlin16_pcieg3_info_ptr, uint16_t *status, uint8_t *diag_rd_ptr, uint8_t byte_count, uint32_t timeout_ms) {
    const uint32_t lane_diag_size = merlin16_pcieg3_info_ptr->diag_mem_ram_size;
    uint16_t loop;
...
err_code_t merlin16_pcieg3_read_eye_scan_stripe(srds_access_t *sa__, uint32_t *buffer, uint16_t *status) {
    ...
    EFUN(merlin16_pcieg3_INTERNAL_verify_merlin16_pcieg3_info(merlin16_pcieg3_info_ptr));
        ...
        lane_diag_base = merlin16_pcieg3_info_ptr->diag_mem_ram_base + ((lane%merlin16_pcieg3_info_ptr->lane_count) * merlin16_pcieg3_info_ptr->diag_mem_ram_size) + 
            merlin16_pcieg3_info_ptr->grp_ram_size*merlin16_pcieg3_INTERNAL_grp_idx_from_lane(merlin16_pcieg3_get_lane(sa__));

    EFUN(merlin16_pcieg3_poll_diag_data(sa__, merlin16_pcieg3_info_ptr, status, &diag_rd_ptr, stripe_size, 400));
        
    arg.buffer_ptr = buffer;
    ESTM(merlin16_pcieg3_INTERNAL_rdblk_uc_generic_ram(sa__,
                                              lane_diag_base,
...
err_code_t merlin16_pcieg3_display_core_state_hdr(void) {
  ...
  EFUN_PRINTF(("CORE  ST   PLL_PWDN   COM_CLK   UCODE_VER  AFE_VER   LIVE_TEMP   AVG_TMON   RESCAL   RATE ANA_VCO_RANGE PLL_LOCK"));
  EFUN_PRINTF(("\n"));
  return (ERR_CODE_NONE);
}
...
err_code_t merlin16_pcieg3_display_diag_data(srds_access_t *sa__, uint16_t  diag_level) {
    ...
    EFUN(merlin16_pcieg3_version(&api_version));
    ...
    if((diag_level & SRDS_DIAG_REG_CORE)
       || (diag_level & SRDS_DIAG_REG_LANE)) {
        EFUN(merlin16_pcieg3_reg_dump(sa__));
    }
    if(diag_level & SRDS_DIAG_UC_CORE) {
        EFUN(merlin16_pcieg3_uc_core_var_dump(sa__));
    }
    if(diag_level & SRDS_DIAG_UC_LANE) {
        EFUN(merlin16_pcieg3_uc_lane_var_dump(sa__));
    }
...
err_code_t merlin16_pcieg3_diag_access(srds_access_t *sa__, enum srds_diag_access_enum type, uint16_t addr, uint16_t data, uint16_t param) {
    ...
    case SRDS_LANE_RAM_READ_BYTE: {
                             if(data > 1) {
                                 ...
                                 EFUN(merlin16_pcieg3_INTERNAL_rdblk_uc_generic_ram(sa__, (addr + merlin16_pcieg3_info_ptr->lane_var_ram_base + (merlin16_pcieg3_get_lane(sa__) * merlin16_pcieg3_info_ptr->lane_var_ram_size)),
                                                                           data,
                                                                           0,
                                                                           data,
                                                                           &state,
