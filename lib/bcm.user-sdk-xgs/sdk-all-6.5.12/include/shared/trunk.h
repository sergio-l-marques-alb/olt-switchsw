...
#define _SHR_TRUNK_CORE_ALL_BIT         23
#define _SHR_TRUNK_CORE_ALL_MASK        1 << _SHR_TRUNK_CORE_ALL_BIT
#define _SHR_TRUNK_CORE_ALL             -1
#define _SHR_TRUNK_CORE_OFFSET          20
#define _SHR_TRUNK_POOL_OFFSET          17
#define _SHR_TRUNK_GROUP_OFFSET         0          

#define _SHR_TRUNK_ID_SET(trunk_id, core, pool, group)  \
    (trunk_id = (_SHR_TRUNK_ID_MASK) | \
    (((core) == (_SHR_TRUNK_CORE_ALL)) ? (_SHR_TRUNK_CORE_ALL_MASK) : ((core) << (_SHR_TRUNK_CORE_OFFSET))) | \
    ((pool) << (_SHR_TRUNK_POOL_OFFSET)) | \
    ((group) << (_SHR_TRUNK_GROUP_OFFSET)))
    
#define _SHR_TRUNK_ID_CORE_GET(core, trunk_id) \
    (core = ((trunk_id) & (_SHR_TRUNK_CORE_ALL_MASK)) ? \
    (_SHR_TRUNK_CORE_ALL) : \
    ((trunk_id) & (~(_SHR_TRUNK_ID_MASK))) >> (_SHR_TRUNK_CORE_OFFSET))

#define _SHR_TRUNK_ID_POOL_GET(pool, trunk_id)  \
    (pool = ((trunk_id) & ((1 << (_SHR_TRUNK_CORE_OFFSET)) - 1)) >> (_SHR_TRUNK_POOL_OFFSET))
    
#define _SHR_TRUNK_ID_GROUP_GET(group, trunk_id)  \
    (group = (trunk_id) & ((1 << (_SHR_TRUNK_POOL_OFFSET)) - 1))
...
#define _SHR_TRUNK_ID_IS_STACKING (trunk_id) \
    (((trunk_id) & ((_SHR_TRUNK_STACKING_MASK) | (_SHR_TRUNK_ID_MASK))) != 0)
