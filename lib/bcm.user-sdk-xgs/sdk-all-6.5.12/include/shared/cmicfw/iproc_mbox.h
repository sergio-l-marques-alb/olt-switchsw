...
#define IPROC_MBOX_HEAD(chan, mbox)         (soc_m0ssq_sram_read32(chan->unit, ((unsigned long)&mbox->head - soc_iproc_sram_membase_get())))
#define IPROC_MBOX_TAIL(chan, mbox)         (soc_m0ssq_sram_read32(chan->unit, ((unsigned long)&mbox->tail - soc_iproc_sram_membase_get())))

#define MOVE_IPROC_MBOX_HEAD(chan, mbox, size)  \
    { \
        soc_m0ssq_sram_write32(chan->unit, ((unsigned long)&mbox->head - soc_iproc_sram_membase_get()), \
...
#define MOVE_IPROC_MBOX_TAIL(chan, mbox, size)  \
    { \
        soc_m0ssq_sram_write32(chan->unit, ((unsigned long)&mbox->tail - soc_iproc_sram_membase_get()), \
...
#define IPROC_MBOX_FULL(chan, mbox, size) ((IPROC_MBOX_TAIL(chan, mbox) < IPROC_MBOX_HEAD(chan, mbox)) ? ((IPROC_MBOX_TAIL(chan, mbox) + size) >= IPROC_MBOX_HEAD(chan, mbox)) : ((IPROC_MBOX_TAIL(chan, mbox) - IPROC_MBOX_HEAD(chan, mbox) + size) >= (IPROC_MBOX_MAX_ENT(chan) - IPROC_MBOX_START_IDX)))
...
#define IPROC_MBOX_FULL(chan, mbox, size) ((mbox->tail < mbox->head) ? ((mbox->tail + size) >= mbox->head) : ((mbox->tail - mbox->head + size) >= (chan->maxent - IPROC_MBOX_START_IDX)))
