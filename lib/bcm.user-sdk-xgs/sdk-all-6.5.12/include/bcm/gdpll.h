...
typedef enum bcm_gdpll_input_event_e {
    ...
    bcmGdpllInputEventRP1IF = 14,       /* RP1 interface input event type */
    bcmGdpllInputEventRP1RF = 15,       /* BCN counter input event type */
...
typedef struct bcm_gdpll_chan_dpll_config_s {
    uint16 k1[BCM_GDPLL_NUM_STATES];    /* DPLL config parameter */
    uint8 k1Shift[BCM_GDPLL_NUM_STATES]; /* left or right sight, 48 places max */
    uint16 k1k2[BCM_GDPLL_NUM_STATES]; 
    uint8 k1k2Shift[BCM_GDPLL_NUM_STATES]; 
    uint16 k1k3[BCM_GDPLL_NUM_STATES]; 
    uint8 k1k3Shift[BCM_GDPLL_NUM_STATES]; 
    uint32 lockDetThres[BCM_GDPLL_NUM_STATES]; 
    uint8 dwell_count[BCM_GDPLL_NUM_STATES]; 
    uint64 phase_error_limiter_thres[BCM_GDPLL_NUM_STATES]; 
    uint8 fll_decim_factor; 
    uint8 nco_bits; 
    uint8 phase_counter_ref;            /* Boolean */
    uint8 dpll_num_states; 
    uint8 dual_loop_flag;               /* Boolean */
    uint64 phase_counter_delta;         /* 52-bit */
...
typedef struct bcm_gdpll_chan_dpll_state_s {
    ...
    uint64 loop_filter;                 /* filter parameter */
    uint32 dwell_counter;               /* filter dwell counter */
    uint32 lockDetFilter;               /* Filter parameter */
    uint64 prev_perror;                 /* DPLL channel prev perror, 52 bits */
    uint64 ferror;                      /* DPLL channel ferror, 52 bits */
    uint8 fll_decim_counter; 
    uint8 first_order_loop_transition;  /* Initialize to FALSE */
