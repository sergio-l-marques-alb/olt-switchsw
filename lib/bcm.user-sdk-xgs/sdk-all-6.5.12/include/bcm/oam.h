...
typedef enum bcm_oam_control_type_e {
    ...
    bcmOAMControlCount = 24             /* Always Last. Not a usable value. */
...
/* Initialize an OAM AIS (alarm indication signal) structure */
extern void bcm_oam_ais_t_init(
    bcm_oam_ais_t *ais_ptr);

/* Initialize an OAM OPCODE ACTION structure */
extern void bcm_oam_endpoint_action_t_init(
    bcm_oam_endpoint_action_t *action_ptr);

#ifndef BCM_HIDE_DISPATCHABLE

/* Add an OAM AIS (alarm indication signal) object */
extern int bcm_oam_ais_add(
    int unit, 
    bcm_oam_ais_t *ais_ptr);

/* Get an OAM AIS (alarm indication signal) object */
extern int bcm_oam_ais_get(
    int unit, 
    bcm_oam_ais_t *ais_ptr);

/* Delete an OAM AIS (alarm indication signal) object */
...
/* Initialize a signal degradation/ signal fault detection structure */
...
/* Add an OAM Y.1711 alarm  object */
extern int bcm_oam_y_1711_alarm_add(
    int unit, 
    bcm_oam_y_1711_alarm_t *alarm_ptr);

/* Get an OAM Y.1711 alarm object */
...
/* Initialize an OAM csf structure */
extern void bcm_oam_csf_t_init(
    bcm_oam_csf_t *csf_ptr);

#ifndef BCM_HIDE_DISPATCHABLE

/* Add an OAM CSF (Client Signal Failure) object */
extern int bcm_oam_csf_add(
    int unit, 
    bcm_oam_csf_t *csf_ptr);

/* Get an OAM CSF (Client Signal Failure) object */
extern int bcm_oam_csf_get(
    int unit, 
    bcm_oam_csf_t *csf_ptr);

/* Delete an OAM CSF (alarm indication signal) object */
...
/* Unregister a callback for handling OAM performance events */
...
/* Enable/Disable drop conditions for OAM lookup types */
...
/* Get the drop condition that is ignored for an OAM lookup type */
...
/* Attach a PM profile to an endpoint. */
...
/* Get the PM profile attached to this endpoint. */
...
/* Get PM stats for a given endpoint. */
...
/* 
 * Create a new LM count profile for opcodes with all opcodes having
 * counting disabled. The id of the profile created is returned in
 * lm_count_profile variable.
 */
...
/* 
 * Enable/disable counting for the opcodes in the opcodes_bitmap in the
 * profile pointed by lm_count_profile variable.
 */
...
/* 
 * Get the opocdes in opcodes_bitmap for given count enable  as part of
 * lm_count_profile.
 */
...
/* 
 *  Delete the count profile with the id lm_count_profile.The counting
 * for all opocdes will be destroyed in the profile.
 */
...
/* 
 * Add ports (specified by port_arr) to trunk (specified by trunk_gport)
 * mapping to port-trunk database in OAM
 */
...
/* 
 * Delete ports (specified by port_arr) to trunk (specified by
 * trunk_gport) mapping from port-trunk database in OAM
 */
...
/* 
 * Get ports (into port_arr) that are mapped to trunk (specified by
 * trunk_gport) from port-trunk database in OAM
 */
...
typedef enum bcm_oam_mpls_tp_channel_type_e {
    ...
    bcmOamMplsTpChannelTypeCount = 3    /* Always Last. Not a usable value. */
...
/* 
 * Delete a list of values for MPLS TP OAM/BFD channel types that can be
 * received in a packet
 */
...
#define BCM_OAM_PROFILE_WITH_ID 0x1        /* Used by oam action profile create
                                              profile create with id. */
...
/* Used to identify Loss Measurement type. */
typedef enum bcm_oam_loss_type_e {
    bcmOAMLossTypeSingleEnded = 0,  /* PDU used will be Eth-LM. */
    bcmOAMLossTypeSLM = 1, 
    bcmOAMLossTypeDualEnded = 2,    /* PDU used will be CCM. */
    bcmOAMLossTypeCount = 3 
} bcm_oam_loss_type_t;
...
typedef struct bcm_oam_action_result_s {
    ...
    bcm_oam_loss_type_t lm_type; 
    bcm_oam_timestamp_format_t time_format; 
