...
#define PRT_COLUMN_WIDTH_SPL 12
#define PRT_COLUMN_MAX_NUM  40
...
typedef struct
{
    ...
    /**
     * dont_show - if this flag is TRUE don't show column in screen table
     * Does not influence XML output
     */
    int dont_show;
    /**
...
/*
 * Internal Facilities for pretty print not to be used iutside this include file
 * {
 */
...
#define PRT_COLUMN_ITERATOR(k_mc, prt_ctr_mc)                                                                          \
    for (k_mc = 0; k_mc < prt_ctr->col_num; k_mc++)                                                                    \
        if(prt_ctr_mc->col[k_mc].dont_show == FALSE)
...
#define _PRT_COMMITX(prt_ctr_mc) {                                                                                    \
                                      diag_sand_prt(prt_ctr_mc, sand_control, NULL);                                  \
...
#define _PRT_COMMITF(prt_ctr_mc, filename_str) {                                                                      \
                                                  diag_sand_prt(prt_ctr_mc, sand_control, filename_str);              \
                                                  _PRT_FREE(prt_ctr_mc);                                              \
                                               }
...
{                                                                                                                      \
    diag_sand_prt(prt_ctr_mc, NULL, NULL);                                                                             \
    _PRT_FREE(prt_ctr_mc);                                                                                             \
}
...
{                                                                                                                     \
    ...
    prt_row_offset = (char *)prt_ctr_mc->row_cur + sizeof(rhentry_t) + prt_ctr_mc->col[prt_ctr_mc->col_cur].cell_offset;    \
    PRT_FORMAT(prt_row_offset, prt_ctr_mc->col[prt_ctr_mc->col_cur].max_width, __VA_ARGS__);                          \
    if (prt_ctr_mc->col[prt_ctr_mc->col_cur].flex_type == PRT_FLEX_ASCII)                                             \
    {                                                                                                                 \
        utilex_str_shrink(prt_row_offset);                                                                            \
    }                                                                                                                 \
    prt_row_offset[prt_ctr_mc->col[prt_ctr_mc->col_cur].max_width - 1] = 0;                                           \
    prt_ctr_mc->col_cur++;                                                                                            \
}
...
{                                                                                                                      \
    ...
    {                                                                                                                  \
        ...
        while(shift_num--)                                                                                             \
        {                                                                                                              \
            PRT_FORMAT(prt_row_offset + strlen(prt_row_offset), prt_ctr_mc->col[prt_ctr_mc->col_cur].max_width, "  "); \
        }                                                                                                              \
    }                                                                                                                  \
    PRT_FORMAT(prt_row_offset + strlen(prt_row_offset), prt_ctr_mc->col[prt_ctr_mc->col_cur].max_width, __VA_ARGS__);  \
    prt_row_offset[prt_ctr_mc->col[prt_ctr_mc->col_cur].max_width - 1] = 0;                                            \
    prt_ctr_mc->col_cur++;                                                                                             \
}
...
{                                                                                                                      \
    ...
    if(prt_ctr_mc->info_list == NULL)                                                                                  \
    {                                                                                                                  \
        if((prt_ctr_mc->info_list = utilex_rhlist_create("prt_print", sizeof(rhentry_t), 0)) == NULL)                  \
        {                                                                                                              \
...
{                                                                                                                      \
    ...
    if(prt_ctr_mc->col_num  >= PRT_COLUMN_MAX_NUM)                                                                     \
    {                                                                                                                  \
        SHR_CLI_EXIT(_SHR_E_INTERNAL, "Number of columns exceeded maximum:%d\n", PRT_COLUMN_MAX_NUM);                  \
    }                                                                                                                  \
    prt_ctr_mc->col[prt_ctr_mc->col_num].cell_offset = prt_ctr_mc->row_width;                                          \
    prt_ctr_mc->col[prt_ctr_mc->col_num].max_width = width;                                                            \
    prt_ctr_mc->col[prt_ctr_mc->col_num].flex_type = flag;                                                             \
    PRT_FORMAT(prt_ctr_mc->col[prt_ctr_mc->col_num].name, prt_ctr_mc->col[prt_ctr_mc->col_num].max_width, __VA_ARGS__);\
    prt_ctr_mc->row_width += prt_ctr_mc->col[prt_ctr_mc->col_num].max_width;                                           \
    prt_ctr_mc->col_num++;                                                                                             \
}

#define _PRT_COLUMN_ALIGN(prt_ctr_mc)            prt_ctr_mc->col[prt_ctr_mc->col_num].align = TRUE
...
{                                                                                                                      \
    ...
    {                                                                                                                  \
        ...
        prt_ctr_mc->col[prt_ctr_mc->col_num - 1].type      = prt_type;                                                 \
        prt_ctr_mc->col[prt_ctr_mc->col_num - 1].parent_id = prt_parent_id;                                            \
 ...
 * \param [in] prt_ctr_mc - pointer to the control structure
 * \par INDIRECT OUTPUT
 *         prt_ctr_mc->col[PRT_TITLE_ID] - title prt column is updated
 */
...
{                                                                                                                      \
    ...
    prt_ctr_mc = sal_alloc(sizeof(prt_control_t), "prt_control");                                                      \
    memset(prt_ctr_mc, 0, sizeof(prt_control_t));                                                                      \
    prt_ctr_mc->unit = unit;                                                                                           \
    prt_ctr_mc->col[PRT_TITLE_ID].depth = 1;                                                                           \
    PRT_FORMAT(prt_ctr_mc->col[PRT_TITLE_ID].name, PRT_COLUMN_WIDTH, __VA_ARGS__);                                     \
}
...
/**
 * \brief
 *    Returns current column number, is useful to calculate how many columns should be skipped when deailng with table,
