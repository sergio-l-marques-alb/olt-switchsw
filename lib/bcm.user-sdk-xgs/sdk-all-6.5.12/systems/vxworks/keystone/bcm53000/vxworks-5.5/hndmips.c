 ...
 *
 * $Id: hndmips.c,v 1.2 Broadcom SDK $
 */
...
static void
si_init_timer_irq(si_t *sih, uint irq)
{
	osl_t *osh;
	void *regs;
	uint32 flag = 31;
...
BCMINITFN(si_mips_init)(si_t *sih, uint shirqmap)
{
	...
	/* PR21489: do not optimise timings for 4712 */
	/* XXX: this PR is still under investigation */
	/* Set timing for the flash */
	/* PR14142/14140: Make sure that w1+w3 < 7 */
	tmp = CEIL(10, ns) << FW_W3_SHIFT;	/* W3 = 10nS */
	tmp |= CEIL(10, ns) << FW_W1_SHIFT;	/* W1 = 10nS */
	tmp |= CEIL(120, ns);			/* W0 = 120nS */
	if (sih->ccrev < 9)
		W_REG(osh, &cc->pflash_waitcount, tmp);
	...
	if ((sih->ccrev < 9) ||
		...
		/* XXX in the case of 5350 this is a bogus register in Chip Common */
		W_REG(osh, &cc->extif_cs2_waitcount, tmp);
	}
    ...
    case BCM53000_CHIP_ID:
		...
		si_setirq(sih, 1, GMAC_CORE_ID, 1);
		si_setirq(sih, 3, PCIE_CORE_ID, 0);
		si_setirq(sih, 4, PCIE_CORE_ID, 1);
    	
		si_init_timer_irq(sih, 5);
		break;
...
BCMINITFN(mips_pmu_setclock)(si_t *sih, uint32 mipsclock, uint32 ddrclock, uint32 axiclock)
{
	...
	for (i = PMU5_PLL_P1P2_OFF; i <= PMU5_PLL_FMAB_OFF; i++) {
		W_REG(osh, &cc->pllcontrol_addr, PMU5_MAINPLL_PLL0 + i);
		(void)R_REG(osh, &cc->pllcontrol_addr);
		pmureg = R_REG(osh, &cc->pllcontrol_data);
		if (pmureg != pll4716_table[idx][i + 3])
			break;
	}
