/*
 * $Id: smbus.c,v 1.10 Broadcom SDK $
...
cmicx_smbus_timeout_recovery(int unit)
{
    ...
    READ_IPROCPERIPH_SMBUS1_SMBUS_BIT_BANG_CONTROLr(unit, &rval);
    if (soc_reg_field_get(unit, IPROCPERIPH_SMBUS1_SMBUS_BIT_BANG_CONTROLr, rval, SMBDAT_INf) == 0) {
        ...
        WRITE_IPROCPERIPH_SMBUS1_SMBUS_BIT_BANG_CONTROLr(unit, rval);
        clk = 1;
    ...
    READ_IPROCPERIPH_SMBUS1_SMBUS_MASTER_COMMANDr(unit, &rval);
    if (soc_reg_field_get(unit, IPROCPERIPH_SMBUS1_SMBUS_MASTER_COMMANDr, rval, MASTER_START_BUSY_COMMANDf) != 0 ||
        ...
        READ_IPROCPERIPH_SMBUS1_SMBUS_MASTER_FIFO_CONTROLr(unit, &rval);
        ...
        WRITE_IPROCPERIPH_SMBUS1_SMBUS_MASTER_FIFO_CONTROLr(unit, rval);
...
retry_cmicx:
        ...
        rv = cmicx_smbus_start_wait(unit);
        if (rv == SOC_E_NONE) {
            READ_IPROCPERIPH_SMBUS1_SMBUS_MASTER_DATA_READr(unit, &rval);
            *data = (uint8) (rval & 0xff);
        } else if (rt-- > 0) {
            goto retry_cmicx;
        } else {
