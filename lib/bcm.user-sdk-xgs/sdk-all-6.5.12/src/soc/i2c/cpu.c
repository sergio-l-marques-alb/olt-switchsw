...
{
    int ret = 0, i, dlen = p_i2c_access->alen & 0xf, alen = (p_i2c_access->alen >> 4) & 0xf;
    uint32 value;
    unsigned nmsgs = alen ? 2 : 1;
    unsigned char result[4] = {0}, addr[4];
    ...
    switch (p_i2c_access->alen)
    {
        ...
        default:
            ...
            LOG_ERROR(BSL_LS_SOC_I2C,
              (BSL_META("error - unsupported address and data length given = 0x%x"),
              (unsigned)p_i2c_access->alen));
            ret = 1;
            goto exit;
            break;
    } /* switch*/
    ...
    rw_data.nmsgs = nmsgs--;                 /* WRITE + READ */
  
    i2c_rw_msg[0].addr    = i2c_rw_msg[1].addr = p_i2c_access->chip;
    i2c_rw_msg[0].flags   = 0;                                            /* I2C_M_WR */
    i2c_rw_msg[0].buf     = &addr[0];
    i2c_rw_msg[0].len     = alen;
    i2c_rw_msg[nmsgs].flags   = I2C_M_RD;
    i2c_rw_msg[nmsgs].buf     = &result[0];
    i2c_rw_msg[nmsgs].len     = dlen;

    /* build address according to endianess */
    value = p_i2c_access->addr;
    for (i = alen - 1 ; i >= 0 ; --i) {
        addr[i] = value & 0xff;
        value >>= 8;
    }
    ...
    while (ioctl(p_i2c_access->file_desc, I2C_RDWR, &rw_data) < 0) {
        LOG_INFO(BSL_LS_SOC_I2C, (BSL_META("ioctl I2C_RDWR failed in %s, %d, %s "
    ...
    /* build results value according to endianess */
    value = result[0];
    for (i = 1 ; i < dlen; ++i) {
        value = (value << 8) | result[i];
    }
    p_i2c_access->data = value;
...
{
    unsigned char addr_n_data[8], *data;
    int ret = 0, i, dlen = p_i2c_access->alen & 0xf, alen = (p_i2c_access->alen >> 4) & 0xf;
    uint32 value;
    struct i2c_rdwr_ioctl_data rw_data;
    ...
    i2c_rw_msg.buf = &addr_n_data[0];
    i2c_rw_msg.len = alen + dlen;
    ...
    /* build data according to endianess */
    data = addr_n_data + alen;
    value = p_i2c_access->data;
    for (i = dlen - 1 ; i >= 0 ; --i) {
        data[i] = value & 0xff;
        value >>= 8;
    }
...
int cpu_i2c_read_extended(int chip, int addr, CPU_I2C_BUS_LEN alen, int* p_val, int donot_print_errors)
{
  ...
  if (ret)
  {
    if (!donot_print_errors) {
      LOG_ERROR(BSL_LS_SOC_I2C,
               (BSL_META("cpu_i2c_read() returned with error.\n")));
    }
    ret = 1;
    goto exit;
  }
...
int cpu_i2c_read_extended(int chip, int addr, CPU_I2C_BUS_LEN alen, int* p_val, int donot_print_errors)
{
    return cpu_i2c_read(chip, addr, alen, p_val);
}
#endif /* __KERNEL__ */
#endif /* LINUX */
