...
{
    qax_mcds_t *mcds = dpp_get_mcds(unit);
    DPP_MC_ASSERT(DPP_MCDS_TYPE_IS_NORMAL(list_type) && ((0 == in_block_size) == DPP_MCDS_TYPE_IS_START(list_type))); /* assumes IS_START returns 0/1 */
...
)
{
    ...
    if (to_delete) {
        /* Unused MCDB entries need to be defined as egress NULL value. Therefore after deleting ingress multicast group (qax_mcds_set_linked_list) and moving it to be egress (qax_mcid_type_set_unsafe) mcdb unused entry need to 
         * be set to egress NULL value. */
        mcds->start_entry = mcds->free_value;
