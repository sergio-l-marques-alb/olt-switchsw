        ...
        if (kbp_cache_mode == FALSE) {
...
#endif
            {
                ...
                if(ARAD_KBP_TO_SOC_RESULT(res) != SOC_SAND_OK) {
                    ...
                    *success = (res == KBP_OUT_OF_UDA) ? SOC_SAND_FAILURE_OUT_OF_RESOURCES : SOC_SAND_FAILURE_INTERNAL_ERR;
                    SOC_SAND_SET_ERROR_CODE(SOC_SAND_GEN_ERR, 80, exit);
                }
            }
        }
    ...
    *success = SOC_SAND_SUCCESS;
    ...
    if(nof_range_quals_in_table) {
        ...
        for(i = 0; i < nof_range_quals_in_table; i++) {
            ...
            /* read the range index from the entry data */
            ...
            if(mask == 0xffff) {
                /* set min and max available range */
                range.low = 0;
                range.high = 0xffff;
            } else {
    ...
    )
{
    ...
    /* route add for LPM DBs */
    res = arad_pp_tcam_kbp_lpm_route_add(unit, table.db_prefix, 0 /*data_indx*/, prefix_len, data, assoData, success);
    SOC_SAND_CHECK_FUNC_RESULT(res, 200, exit);
