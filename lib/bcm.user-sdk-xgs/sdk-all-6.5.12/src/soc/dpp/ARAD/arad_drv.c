...
soc_arad_default_config_general_get(int unit)
{
    ...
    /* L2 Source MAC encap supported for Jericho and above */
    ...
    /* enabling adding UDH for down mep packets bound to the CPU */
    ...
    /* SDK-126418: These soc properties are only for QUX due to a HW LM Bug */
    if (SOC_IS_QUX(unit)) {
        /* Default will be 0 - LM mode */
        dpp_pp->qux_slm_lm_mode = soc_property_get(unit, spn_OAM_SLM_LM_MODE, 0);

        /* Counting disabled for these subtypes */
        dpp_pp->qux_dm_1588_counting_enable = 0;
        dpp_pp->qux_dm_ntp_counting_enable = 0;
        dpp_pp->qux_lb_counting_enable = 0;

        /* Default will be 1 - counting enabled for CCM */
    ...
    if (SOC_IS_JERICHO(unit) && dpp_pp->explicit_null_support && SOC_DPP_CONFIG(unit)->pp.ip_lif_dummy_id &&
       (SOC_DPP_CONFIG(unit)->pp.ip_lif_dummy_id != SOC_DPP_CONFIG(unit)->pp.explicit_null_support_lif_id)) {
        SOCDNX_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_SOCDNX_MSG("explicit null and ip lif dummy LIF should identical when they are both validated")));
    }
    ...
    if ((SOC_IS_QAX(unit)) && (dpp_pp->oamp_flexible_da != 0)) {
        /* UDH cannot be enabled when custom feature flexible da is set. */
        ...
        /* OAM statistics per MEP cannot be enabled when custom feature flexible da is set */
...
soc_arad_str_prop_nif_elk_core_port_mapping_get(int unit)
{
    ...
    int max_kbp_port = ARAD_KBP_OP_IS_DUAL_MODE ? 4 : 2;
    int kbp_port;
    ARAD_INIT_ELK *elk = &SOC_DPP_CONFIG(unit)->arad->init.elk;
    SOCDNX_INIT_FUNC_DEFS;

    propkey = spn_EXT_TCAM_START_LANE;
    for (kbp_port = 0; kbp_port < max_kbp_port; kbp_port ++) {
            ...
            switch (kbp_port) {
                ...
                default:
                    SOCDNX_EXIT_WITH_ERR
                        (SOC_E_FAIL, (_BSL_SOCDNX_MSG("Bad KBP port ID %d!\n"), kbp_port));
            }
        } else {
...
soc_arad_str_prop_nif_elk_fwd_table_size_get(int unit, ARAD_INIT_ELK *elk)
{
    ...
    /* Public tables are only supported on OP */
    if (!ARAD_KBP_IS_OP && (
        elk->fwd_table_size[ARAD_KBP_FRWRD_TBL_ID_IPV4_UC_PUBLIC_RPF_0] ||
    ...
    /* Check that both public and private tables are initialized, in order for the public+private program to work */
...
soc_arad_str_prop_parse_mpls_termination_database_mode(int unit, uint8 mpls_termination_label_index_enable, uint8 mpls_termination_rif_enable, uint8 *mpls_database, SOC_PPC_MPLS_TERM_NAMESPACE_TYPE  *mpls_namespace)
{
    ...
    switch (mode) {
        ...
        case 10:
            ...
            break;
        case 11:
            /* 11: MPLS_1 refers to label namespace L1L3 and located in SEM-A. MPLS_2 refers to label namespace L2 and located in SEM-B. */
            mpls_database[0] = 0;
            mpls_database[1] = 1;
            mpls_namespace[0] = SOC_PPC_MPLS_TERM_NAMESPACE_L1_L3;
            mpls_namespace[1] = SOC_PPC_MPLS_TERM_NAMESPACE_L2;
            break;
        case 12:
            /* 11: MPLS_1 refers to label namespace L1L2 and located in SEM-A. MPLS_2 refers to label namespace L3 and located in SEM-B. */
            mpls_database[0] = 0;
            mpls_database[1] = 1;
            mpls_namespace[0] = SOC_PPC_MPLS_TERM_NAMESPACE_L1_L2;
            mpls_namespace[1] = SOC_PPC_MPLS_TERM_NAMESPACE_L3;
...
#endif /* BCM_88675_A0 */
    elk->is_master = TRUE;
    if (dpp_arad->init.elk.tcam_dev_type == ARAD_NIF_ELK_TCAM_DEV_TYPE_BCM52311) {
...
#else
        /* In case KBP lib is not used, force L3 MC to TCAM */
        l3_mc_use_tcam = 1;
#endif
        if (l3_mc_use_tcam > 2) {
...
soc_arad_default_ofp_rates_set(int unit) {
    ...
    SOC_DPP_CORES_ITER(SOC_CORE_ALL, core) {
        ...
        SOC_PBMP_ITER(ports_bm, port_i) {
            ...
            /* in jericho for dedicated queuing mode we need to update IF interface rate
             * for both dedicated queuing IF and non-TDM IF
             */
            if (SOC_IS_JERICHO(unit) && (interface == SOC_PORT_IF_ILKN) &&
               (SOC_DPP_CONFIG(unit)->arad->init.ilkn_tdm_dedicated_queuing) ) {
               /*check if ILK and get low egr IF */
