...
*  2. soc_sand_code_hamming_get_p_no_check() and soc_sand_code_hamming_get_p()
*     have same functionality as: soc_sand_code_hamming_get_p().
...
/* $Id: sand_code_hamming.c,v 1.3 Broadcom SDK $
 * refer to soc_sand_code_hamming_get_p()
 */
SOC_SAND_RET
  soc_sand_code_hamming_get_p_no_check(
    SOC_SAND_IN    uint32  data[],
...
*  SOC_SAND_DIRECT:
*    SOC_SAND_RET -
*      If error code is not SOC_SAND_OK then
*        specific error codes:
  ...
  for(p_i=0; p_i<p_bit_wide; ++p_i)
  {
    ...
    for(bit_i=0x0; bit_i<data_bit_wide; ++bit_i)
    {
      ...
      if( ( bit_i+1 == data_bit_wide) /*last loop*/ ||
          ((bit_i+1)%SOC_SAND_NOF_BITS_IN_UINT32 == 0) /*32 bit boundary*/
        )
      {
        print_flag = TRUE;
      }
      ...
      {
        /*
        soc_sand_print_u_long_binary_format(data_to_print, SOC_SAND_NOF_BITS_IN_UINT32);
        LOG_CLI((BSL_META(" ")));
        */
