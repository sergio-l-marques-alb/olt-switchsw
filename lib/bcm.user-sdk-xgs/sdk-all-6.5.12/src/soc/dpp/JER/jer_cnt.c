...
int soc_jer_stat_egress_receive_tm_pointer_format_set(
    ...
    /* if tc_mask  the lsb bit is not set, the shift will be zero, no matter if the cast bit is valid or not */
    traffic_class_shift_local = (((traffic_class_mask & 0x1) == FALSE) && (traffic_class_mask != 0)) ? 0 : cast_bits;
    /* find the last bit set in the tc mask */
    ...
    /** for q_pair, find the first bit set and place it in the closest free bit in the pointer format */
 ...
 *      stif_counter_id
 *      is_double_entry
 *      engine_range_offset - offset from the profile range to the engine range (can be positive or negative. used in QAX/QUX only
 * Returns:
 *      SOC_E_XXX
 * Notes:
    ...
    )
{
    ...
    /*   calculate the offset. In QAX, we would like to create the adjust the counter pointer to the engine range. this is given by parameter engine_range_offset. (negative or positive)
               For that we need to add an offset. pointer=Out-Lif-Id(18b)>>double_entry(1b) + offset(6b)<<11 + port_offset(8b)<<9 
               the offset is in 2K resolution, and can reach upto 128K. the ptr is 17 bits, so with the offset, we can reach any pointer we want.
               In Jer+, there is no capability to do so, because the ptr is 20 bits, and the offset range is not enough to reach each counter pointer that we want.
               Therefore, in Jer+, we do not use the offset in the same way as in QAX.
          */
    if(SOC_IS_QAX(unit))
    {
        base_counter_pointer = profile_range->start + engine_range_offset;

        /* calc the offset base on the next_counter_pointer */
