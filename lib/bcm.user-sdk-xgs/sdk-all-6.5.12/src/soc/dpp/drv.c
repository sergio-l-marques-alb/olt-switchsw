...
/* To be called after soc_sand_info_config_blocks() to invalidate non used DRAM blocks */
...
soc_dpp_info_config(int unit)
{
    ...
    SOCDNX_IF_ERR_EXIT(soc_sand_info_config_blocks(unit, SOC_DPP_DEFS_GET(unit, nof_cores))); /* configure blocks data structure */
...
soc_dpp_chip_type_set(int unit, uint16 dev_id)
{
...
#endif
#ifdef BCM_JERICHO_SUPPORT
    case JERICHO_DEVICE_ID: /* BCM88675_DEVICE_ID: */ 
    case BCM88670_DEVICE_ID:
    case BCM88671_DEVICE_ID:
    case BCM88671M_DEVICE_ID:
    case BCM88672_DEVICE_ID:
    ...
    case BCM88679_DEVICE_ID:
        ...
        break;
    case QMX_DEVICE_ID: /* BCM88375_DEVICE_ID */
    case BCM88370_DEVICE_ID:
    case BCM88371_DEVICE_ID:
    case BCM88371M_DEVICE_ID:
    case BCM88376_DEVICE_ID:
...
soc_dpp_attach(int unit)
{
    ...
    /* Add list of ARAD errors to all-system errors pool                                                  */
    ...
    /* If fabric connect mode is single_fap, mesh_topology block should not be enabled. */
    if (SOC_IS_QAX(unit)) {
...
{    
    ...
    /* should be called from soc_dpp_attach, but moved here since
     * soc_dpp_info_config moved here
     */
...
/* Get the bitmap value for the device's admission precedence preference - one bit ber DP */
...
/* Parse to configure maximum IDs of ST-VSQs, maximum IDs of TM-ports, and enabling/disabling header compensation. */
int
...
soc_dpp_init(int unit, int reset)
{
    ...
    if (stable_location == 4) {
        ...
        SOCDNX_IF_ERR_RETURN(ha_init(unit, 1 /* HA enabled */, ha_name, stable_size, SOC_WARM_BOOT(unit) ? 0 : 1));
    }
#endif
#endif
...
/* } */
    ...
    if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
        ...
        DISPLAY_MEM ;
        DISPLAY_MEM_PRINTF(("%s(): unit %d: After %s\r\n",__FUNCTION__,unit,"soc_dpp_wb_engine_init")) ;
    }
    ...
    DISPLAY_MEM ;
    DISPLAY_MEM_PRINTF(("%s(): unit %d: After %s\r\n",__FUNCTION__,unit,"shr_sw_state_init")) ;
