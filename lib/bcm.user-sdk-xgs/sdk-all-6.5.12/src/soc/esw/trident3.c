...
{
    _soc_td3_alpm2_bnk_fmt_map[0][index] = fmt;
}

int8
_soc_trident3_alpm2_bnk_fmt_get(int unit, int index)
{
    return _soc_td3_alpm2_bnk_fmt_map[0][index];
}
...
_soc_td3_port_flex_init(int unit, int is_any_cap)
{
    ...
    if (SOC_PBMP_NOT_NULL(SOC_PORT_DISABLED_BITMAP(unit, all)) || is_any_cap) {
        SHR_BITSET_RANGE(si->flexible_pm_bitmap, 1, _TD3_PBLKS_PER_DEV(unit));
    } else {
...
static uint32 _soc_td3_temp_mon_mask[SOC_MAX_NUM_DEVICES];


/*
 * Function:
 *      _soc_td3_set_idb_dpp_ctrl
 * Purpose:
 ...
 */
static int
_soc_td3_set_idb_dpp_ctrl(int unit)
{
    soc_reg_t reg;
...
_soc_trident3_idb_init(int unit)
{
    ...
    _soc_td3_set_idb_dpp_ctrl(unit);
...
soc_trident3_uft_uat_config(int unit) {
    ...
    for(index = 0; index < vlan_xlate_2_16k_banks; index++) {
        ...
        soc_mem_field32_set(unit, VLAN_XLATE_2_HASH_CONTROLm,
                                hash_control_entry,
                                logical_to_physical_fields[index], index +
                                mpls_16k_banks + vlan_xlate_2_16k_banks + 4);
        soc_mem_field32_set(unit, IP_UAT_16K_SHARED_BANKS_CONTROLm,
                            shared_hash_control_entry, shared_fields[index +
                            mpls_16k_banks + vlan_xlate_1_16k_banks],
                            shared_hash_offset);
...
#define _SOC_TD3_DEF_TEMP_MAX   90
    ...
    _soc_td3_temp_mon_mask[unit] = temp_mask;
...
STATIC int
_soc_trident3_latency_config(int unit)
{
    soc_reg_t reg;
    uint32 rval;
...
subport_id_to_sgpp_map_key_attributes_entry_t sgpp_map_attr;
    ...
    if (soc_feature(unit, soc_feature_robust_hash)) {
        ...
        sal_memset
            (hash_control_entry, 0, sizeof(vlan_xlate_1_hash_control_entry_t));
        ...
        sal_memset
            (hash_control_entry, 0, sizeof(vlan_xlate_2_hash_control_entry_t));
        ...
        sal_memset
            (hash_control_entry, 0, sizeof(egr_vlan_xlate_1_hash_control_entry_t));
        ...
        sal_memset
            (hash_control_entry, 0, sizeof(egr_vlan_xlate_2_hash_control_entry_t));
        ...
        sal_memset
            (hash_control_entry, 0, sizeof(ing_vp_vlan_membership_hash_control_entry_t));
        ...
        sal_memset
            (hash_control_entry, 0, sizeof(egr_vp_vlan_membership_hash_control_entry_t));
        ...
        sal_memset
            (hash_control_entry, 0, sizeof(ing_dnat_address_type_hash_control_entry_t));
        ...
        sal_memset(hash_control_entry, 0, 
                    sizeof(subport_id_to_sgpp_map_hash_control_entry_t));
...
STATIC int
_soc_trident3_tscx_firmware_set(int unit, int port, uint8 *array, int datalen)
{
...
_soc_trident3_misc_init(int unit)
{
    ...
    _soc_trident3_latency_config(unit);
    ...
    _phy_tsce_firmware_set_helper[unit] = _soc_trident3_tscx_firmware_set;
    _phy_tscf_firmware_set_helper[unit] = _soc_trident3_tscx_firmware_set;
...
STATIC int
_soc_trident3_age_timer_get(int unit, int *age_seconds, int *enabled)
{
    soc_control_t *soc = SOC_CONTROL(unit);
...
STATIC int
_soc_trident3_age_timer_max_get(int unit, int *max_seconds)
{
...
STATIC int
_soc_trident3_age_timer_set(int unit, int age_seconds, int enable)
{
    soc_control_t *soc = SOC_CONTROL(unit);
                              ...
                              void *d3, void *d4)
{
    ...
    for (i = 0; i < 12; i++) {
        if (i & _soc_td3_temp_mon_mask[unit]) {
...
soc_functions_t soc_trident3_drv_funs = {
    ...
    _soc_trident3_age_timer_get,
    _soc_trident3_age_timer_max_get,
    _soc_trident3_age_timer_set,
    _soc_trident3_tscx_firmware_set,
