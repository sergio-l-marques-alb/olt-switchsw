...
*
* $File:  trident3_mmu_port_resequence.c
*/
...
/*! @fn int soc_trident3_mmu_set_mmu_to_phy_port_mapping(int unit,
    ...
    int unit,soc_port_resource_t *port_resource_t)
{
    ...
    if (port_resource_t->physical_port == -1) {
        phy_port_new = TD3_INVALID_PHY_PORT_NUMBER; /* All 1's */
    }
    else {
...
/*! @fn int soc_trident3_mmu_vbs_port_flush(int unit,
 ...
 */
int
soc_trident3_mmu_vbs_port_flush(int unit,soc_port_schedule_state_t *port_schedule_state_t,
    ...
    for (port=0; port<port_schedule_state_t->nport; port++) {
      if (port_schedule_state_t->resource[port].physical_port == -1) {
	    if (port_schedule_state_t->resource[port].pipe == pipe) {
	      ...
	      COMPILER_64_ADD_32(temp64, TD3_MMU_FLUSH_ON);
...
/*! @fn int soc_trident3_mmu_rqe_port_flush(int unit,
 ...
 */
int
soc_trident3_mmu_rqe_port_flush(int unit,int pipe,
                                 uint64 set_val)
{
...
/*! @fn int soc_trident3_mmu_mtro_port_flush(int unit,
 *              soc_port_resource_t *port_resource_t, int set_val)
 *  @param unit Device number
 *  @param *port_resource_t Port Resource Struct
 *  @param set_val Value to be set to the snapshot register
 ...
 */
int
soc_trident3_mmu_mtro_port_flush(int unit,soc_port_resource_t *port_resource_t,
                                  uint64 set_val)
{
...
/*! @fn int soc_trident3_mmu_disable_ct(int unit,
 *              soc_port_resource_t *port_resource_t)
 *  @param unit Device number
 *  @param *port_resource_t Port Resource Struct
 ...
 */
int
soc_trident3_mmu_disable_ct(int                  unit,
                            soc_port_resource_t  *port_resource_t)
{
