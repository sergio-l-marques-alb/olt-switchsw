...
#define MN_FLEX_SPEED_MAX_DEFAULT    42000
...
if (0) {
    ...
    MN_INFO(unit)->port_max = ports_max;

} else {

    MN_INFO(unit)->speed_max = 106000;
    MN_INFO(unit)->port_max = 74;
 ...
 */
int
soc_mn_port_oversub_get(int unit, int phy_port, soc_port_t logical_port,
                          int *oversub)
{
    soc_pbmp_t pbmp;
    int oversub_mode = 0;
...
_soc_mn_speed_valid(int unit, int phy_port, int lanes, int speed)
{
    ...
    switch (speed) {
        ...
        case 20000:
            ...
            break;
        case 25000:
            speed_mask = MN_PHY_PORT_RATE_25;
            break;
        case 40000:
            speed_mask = MN_PHY_PORT_RATE_40;
            break;
        case 50000:
            speed_mask = MN_PHY_PORT_RATE_50;
            break;
        case 100000:
            speed_mask = MN_PHY_PORT_RATE_100;
            break;
        case 11000:
        ...
        case 21000:
            ...
            break;
        case 27000:
            speed_mask = MN_PHY_PORT_RATE_27;
            break;
        case 42000:
            speed_mask = MN_PHY_PORT_RATE_42;
            break;
        case 53000:
            speed_mask = MN_PHY_PORT_RATE_53;
            break;
        case 106000:
            speed_mask = MN_PHY_PORT_RATE_106;
            break;
        case 127000:
            speed_mask = MN_PHY_PORT_RATE_127;
            break;
        default:
            LOG_ERROR(BSL_LS_SOC_PORT,
                        (BSL_META_U(unit,
 ...
 */
STATIC int
_soc_mn_logic_ports_max_validate(int unit, pbmp_t phy_pbmp)
{
    int phy_port;
    int ports_max;
    int port_count = 0;
...
/*
 * Function:
 *      _soc_mn_port_resource_oversub_validate
 * Purpose:
 ...
 */
STATIC int
_soc_mn_port_resource_oversub_validate(int unit,
                                         int nport,
                                         soc_port_resource_t *resource,
                                         soc_info_misc_t *post_si)
{
    ...
    for (i = 0, pr = &resource[0]; i < nport; i++, pr++) {
        ...
        rv = soc_mn_port_oversub_get(unit, phy_port, pr->logical_port,
                                       &oversub);
        if (SOC_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_SOC_PORT,
                      (BSL_META_U(unit,
                                 ...
                                 soc_port_resource_t *resource)
{
    ...
    for (i = 0, pr = &resource[0]; i < nport; i++, pr++) {
        ...
        if (!(pr->flags & SOC_PORT_RESOURCE_SPEED)) {
            ...
            SOC_IF_ERROR_RETURN
                (_soc_mn_phy_port_flex_valid(unit, pr->physical_port));

            /* Check lane assignment */
            SOC_IF_ERROR_RETURN
                (_soc_mn_phy_port_lanes_valid(unit, pr->physical_port,
                                              pr->num_lanes));
            /* Check speed (port rate) */
            SOC_IF_ERROR_RETURN
                (_soc_mn_speed_valid(unit, pr->physical_port,
    ...
    /* Check port mappings */
    SOC_IF_ERROR_RETURN
        (_soc_mn_port_mapping_validate(unit, nport, resource));
...
/*
 * Function:
 *      _soc_mn_port_resource_mixed_sisters_validate
 * Purpose:
...
int
soc_mn_port_resources_to_portlist(int unit,
                                  int nport, soc_port_resource_t *resource,
                                  int *pre_count, int *pre_port_list,
                                  int *post_count, int *post_port_list, int *post_port_flags)
{
    int i;
    int rv;
    soc_port_resource_t *pre_resource;
    ...
    rv = soc_mn_port_resource_validate(unit, nport, resource);
    if (SOC_FAILURE(rv)) {
    ...
    for (i = 0; i < *pre_count; i++) {
        (pre_port_list[i]) = pre_resource[i].logical_port;
    }

    for (i = 0; i < *post_count; i++) {
        (post_port_list[i]) = post_resource[i].logical_port;
