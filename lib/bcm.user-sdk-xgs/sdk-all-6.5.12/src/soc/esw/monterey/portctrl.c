...
static int monterey_def_pkg_rx_polarity_flip[64] = {0};
static int monterey_def_pkg_tx_polarity_flip[64] = {0};

/* Indexed by direction (rx=0; tx=1) and TSC instance */
static int monterey_def_pkg_rx_lane_swap[16] = {0};
static int monterey_def_pkg_tx_lane_swap[16] = {0};
...
soc_mn_portctrl_pm_port_config_get(int unit, soc_port_t port, void *port_config)
{
    ...
    if (!pm_found) {
        for (i=0; i<SOC_MN_PM4X25_COUNT; i++) {
            if (core_index == _mn_pm4x25_core_num[i]) {
    ...
    if (!pm_found) {
        for (i=0; i<SOC_MN_CPM4X25_COUNT; i++) {
            if (core_index == _mn_cpm4x25_core_num[i]) {
    ...
    for (lane = 0; lane < SOC_PM4X10_NUM_LANES; lane++) {
        ...
        if ((logical_port == -1) || SOC_PBMP_MEMBER(SOC_INFO(unit).all.disabled_bitmap, logical_port)) {
            ...
            if (SOC_PBMP_MEMBER(SOC_INFO(unit).cpri.bitmap, logical_port)) {
                cpri_mode++;
            } else {
                eth_mode++;
            }
