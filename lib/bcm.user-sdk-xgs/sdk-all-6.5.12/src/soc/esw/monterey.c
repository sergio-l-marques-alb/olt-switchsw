...
#define _MN_MMU_EXPRESS_BUFFER_TOTAL_1 11550 
#define _MN_MMU_EXPRESS_BUFFER_TOTAL_2 12170 
...
static _soc_buffer_ser_en_info_t _soc_monterey_ep_buffer_ser_info[] = {
    ...
    { "CLP10_CELL_BUF",  EGR_EDATABUF_PARITY_CONTROLr, CLP10_ECC_ENf  },
    ...
    { "EP_CLP10__DATABUF_CONTROL_INFO", EGR_EDB_CTRL_PARITY_ENr, CLP10_ECC_ENf  },
...
int IS_ROE_ENABLED(int unit) { 
    ...
    soc_info_t *si;
    si = &SOC_INFO(unit);
    if ((si->frequency == 815 || si->frequency == 816)) {
         return TRUE;          
    ...
    if (lossless) {
        ...
        for (index = 0; index < COUNTOF(offset_ob_prio); index++) {
             if ((index == 1 ||  index == 2 || index == 3 ) 
                  && !(SOC_PBMP_MEMBER(pbmp_xport_roe_mcu, port))) { 
                  if (index == 3) {  
...
_soc_monterey_ser_enable_pm (int unit, int enable)
{
    ...
    uint64    rval64;
    int       rbi, block_info_idx, port, rv;
                               ...
                               _soc_ser_sram_info_t *sram_info)
{
    ...
    case L2_ENTRY_ONLY_ECCm:
        ...
        if (index < SOC_MONTEREY_NUM_ENTRIES_L2_BANK) {
            ...
            sram_info->ram_count = SOC_MONTEREY_NUM_EL_SHARED;
            entries_per_ram = SOC_MONTEREY_RAM_OFFSET_L2_SHARED_BANK;
            base_index = SOC_MONTEREY_NUM_ENTRIES_L2_BANK;
            entries_per_bank = entries_per_ram * sram_info->ram_count;
            offset = base_index + (index % entries_per_ram);
            base = offset;
            for (i = 1; i < SOC_MONTEREY_NUM_EL_L2; i++) {
    ...
    case L3_ENTRY_ONLY_ECCm:
        if (index < SOC_MONTEREY_NUM_ENTRIES_L3_BANK) {
            /* dedicated L3 entries - hash table */
            sram_info->ram_count = 1;
            entries_per_ram = SOC_MONTEREY_NUM_ENTRIES_L3_BANK;
            base = index;
        } else {
            sram_info->ram_count = SOC_MONTEREY_NUM_EL_SHARED;
            entries_per_ram = SOC_MONTEREY_RAM_OFFSET_L3_NARROW;
            base_index = SOC_MONTEREY_NUM_ENTRIES_L3_BANK;
            entries_per_bank = entries_per_ram * sram_info->ram_count;
            offset = base_index + (index % entries_per_ram);
            base = offset;
            for (i = 1; i < SOC_MONTEREY_NUM_EL_L3_IPMC; i++) {
                if (index >= (base_index + (entries_per_bank * i))) {
                    base = offset + (entries_per_bank * i);
                }
            }
    ...
    case L3_DEFIP_ALPM_ECCm:
        num_uft_banks = soc_monterey_get_alpm_banks(unit);
        ...
        sram_info->ram_count = SOC_MONTEREY_NUM_EL_SHARED;
        base_bucket = ((index >> ((num_uft_banks + 1) / 2)) & SOC_MONTEREY_ALPM_BKT_MASK);
        base = index & ((num_uft_banks == 4) ? 0x3 : 0x1);
        LOG_VERBOSE(BSL_LS_SOC_COMMON,
                    (BSL_META_U(unit,
                                "reported bucket: 0x%08x, bank:%d\n"), base_bucket, base));
        base_bucket = base_bucket % SOC_MONTEREY_ALPM_BKT_OFFFSET;
        LOG_VERBOSE(BSL_LS_SOC_COMMON,
                    (BSL_META_U(unit,
                                "base bucket: 0x%08x\n"), base_bucket));
        sram_info->ram_count = SOC_MONTEREY_NUM_EL_SHARED;
        ...
        for (i = 0; i < SOC_MONTEREY_NUM_EL_SHARED; i++) {
            /* Retreive views for all 8 buckets */
            bkt_offset = base_bucket + SOC_MONTEREY_ALPM_BKT_OFFFSET * i;
            view = _soc_monterey_alpm_bkt_view_map[unit][bkt_offset];
            if (INVALIDm != view) {
                LOG_VERBOSE(BSL_LS_SOC_COMMON,
                            (BSL_META_U(unit,
            ...
            switch (view) {
            case L3_DEFIP_ALPM_RAWm:
                sram_info->index_count[i] = SOC_MONTEREY_L3_DEFIP_ALPM_PER_BKT_COUNT;
                sram_info->mem_indexes[i][0] = (bkt_offset <<
                                                ((num_uft_banks + 1) / 2)) | base;
                break;
            case L3_DEFIP_ALPM_IPV6_128m:
                sram_info->index_count[i] = SOC_MONTEREY_L3_DEFIP_ALPM_IPV6_128_PER_BKT_COUNT;
                sram_info->mem_indexes[i][0] = (bkt_offset <<
                                                ((num_uft_banks + 1) / 2)) | base;
                sram_info->mem_indexes[i][1] = (1 <<
                                                _SER_TEST_MEM_MN_ALPM_SHIFT(num_uft_banks)) |
                                                (bkt_offset << ((num_uft_banks + 1) / 2)) | base;
                break;
            case L3_DEFIP_ALPM_IPV6_64_1m:
                sram_info->index_count[i] = SOC_MONTEREY_L3_DEFIP_ALPM_IPV6_64_1_PER_BKT_COUNT;
                sram_info->mem_indexes[i][0] = (bkt_offset <<
                                                ((num_uft_banks + 1) / 2)) | base;
                sram_info->mem_indexes[i][1] = (1 <<
                                                _SER_TEST_MEM_MN_ALPM_SHIFT(num_uft_banks)) |
                ...
                break;
            case L3_DEFIP_ALPM_IPV4m:
                sram_info->index_count[i] = SOC_MONTEREY_L3_DEFIP_ALPM_IPV4_PER_BKT_COUNT;
                for (j = 0; j < SOC_MONTEREY_L3_DEFIP_ALPM_IPV4_PER_BKT_COUNT; j++) {
            ...
            case L3_DEFIP_ALPM_IPV4_1m:
            case L3_DEFIP_ALPM_IPV6_64m:
                sram_info->index_count[i] = SOC_MONTEREY_L3_DEFIP_ALPM_IPV6_64_PER_BKT_COUNT;
                for (j = 0; j < SOC_MONTEREY_L3_DEFIP_ALPM_IPV6_64_PER_BKT_COUNT; j++) {
                    sram_info->mem_indexes[i][j] = (j <<
                                                    _SER_TEST_MEM_MN_ALPM_SHIFT(num_uft_banks)) |
                                                    (bkt_offset << ((num_uft_banks + 1) / 2)) |
                        base;                
                                 ...
                                 int port_lanes, int port_bandwidth)
{
    ...
    case 15000:
        /*  Eagles only - 2,4 lanes */
        return (!soc_monterey_port_is_falcon(unit, phy_port) && ((port_lanes == 2) || (port_lanes == 4)));
    case 20000:
    ...
    for (pgw = 0; pgw < MONTEREY_PGWS_PER_DEV; pgw++) {
        ...
        for (index = 0; index < MONTEREY_PORTS_PER_PGW; index++) {
            ...
            if (port == -1) {
                ...
                pgw_port_bandwidth[index] = SOC_PORTCTRL_HG2_TO_IEEE_BW_GET(SOC_INFO(unit).port_speed_max[port]);
            }
        }
...
soc_monterey_port_config_init(int unit, uint16 dev_id, uint8 rev_id)
{
    ...
    int freq_list_len;
    int const *freq_list;
    static const int monterey_freq_list[] = { 862, 861, 817, 818, 819, 820, 
                                                 816, 815, 705, 575, 510, 435 };
    int int_ports[3][3] = {
...
/*zte sku*/
    case 817:
        si->io_bandwidth = 721000;
        si->bandwidth = 515000;
        break;
/*CPRI_A*/
    case 819:
        si->io_bandwidth = 640000;
        si->bandwidth = 485000;
        break;
...
/*CPRI_B*/
	 ...
	 */
                ...
                if (*sub_str == 'i') {
                    ...
                    if (*sub_str != '\0') {
                        ...
                        if (fallback_core > 2 || fallback_core < 0) {
                            ...
                            for (i = 0; soc_monterey_valid_an_cores[i].lane_config != SOC_INVALID_LANE_CONFIG; i++) {
                                if (soc_monterey_valid_an_cores[i].lane_config == lane_config) {
                                    if (!soc_monterey_valid_an_cores[i].valid_lanes[fallback_core]) {
                                        fallback_core = -1;
                                        break;
                                    }
                                }
        ...
        if ((port_bandwidth >= 100000) && ((phy_port == 17) || (phy_port == 53))) {
            ...
            if (SOC_SUCCESS(soc_monterey_port_lane_config_get(unit,
                                                            phy_port,
                                                            &mode_lane,
                                                            &found))) {
                if (found) {
            ...
            if (SOC_SUCCESS(soc_monterey_port_autoneg_core_get(unit,
                                                             phy_port,
                                                             &an_core,
                                                             &found))) {
                if (found) {
...
if (!SOC_IS_MONTEREY(unit)) {
    if (soc_feature(unit, soc_feature_untethered_otp)) {
        /* Apache Maverick will Support LR or OS only no mixed mode operation and
         * Firebolt5 only supports mixed LR + OS at device level.
         *
         * No Mixed mode LR + OS at a single port macro level anyway.
         * */
        if (SOC_BOND_INFO_FEATURE_GET(unit, socBondInfoFeatureFamilyFirebolt5) && oversub_mode != 2) {
            LOG_ERROR(BSL_LS_SOC_COMMON,
                      (BSL_META_U(unit, "Device supports only mixed LR and OS port configs\n")));
            rv = SOC_E_FAIL;
        }
        if (!SOC_BOND_INFO_FEATURE_GET(unit, socBondInfoFeatureFamilyFirebolt5) && oversub_mode == 2) {
            LOG_ERROR(BSL_LS_SOC_COMMON,
                      (BSL_META_U(unit, "Device does not support mixed LR and OS ports\n")));
            rv = SOC_E_FAIL;
        }
    }
}
 ...
 * */
STATIC int
_soc_monterey_tsc_disable(int unit)
{
...
if (!SOC_IS_MONTEREY(unit)) {
    ...
    if (soc_property_get(unit, "disable_unused_port_blocks", TRUE)) {
        SOC_IF_ERROR_RETURN(_soc_monterey_tsc_disable(unit));
    }
...
{
    switch (speed) {
        case 5000:   *sp_spacing = 0x14; *spg = mn_ovs_speed_5G;   break;
        case 10000:  *sp_spacing = 0x14; *spg = mn_ovs_speed_10G;  break;
        case 20000:  *sp_spacing = 0x0a; *spg = mn_ovs_speed_20G;  break;
        case 25000:  *sp_spacing = 0x0a; *spg = mn_ovs_speed_25G;  break;
        case 40000:  *sp_spacing = 0x0a; *spg = mn_ovs_speed_40G;  break;
        case 50000:  *sp_spacing = 0x0a; *spg = mn_ovs_speed_50G;  break;
        case 100000: *sp_spacing = 0x04; *spg = mn_ovs_speed_100G; break;
        default:     *sp_spacing = 0x00;  *spg = mn_ovs_speed_None; break;
    }
...
static const soc_reg_t mn_mmu_ovs_group_wt_regs[MONTEREY_MMU_OVS_WT_GROUP_COUNT] = {
...
static const soc_mem_t mn_mmu_tdm_mem[] = {ES_PIPE0_TDM_TABLE_0m, ES_PIPE1_TDM_TABLE_1m};
...
soc_monterey_tdm_calc(int unit, soc_monterey_tdm_t *monterey_tdm)
{
   ...
   monterey_chip_pkg.soc_vars.mn.customer10G_tdm = 1;
    ...
    switch (si->frequency) {
        case 705 : 
            monterey_chip_pkg.clk_freq =  705;
            break; 
        default:
    ...
    rv = tdm_mn_set_iarb_tdm(si->frequency,
                            monterey_tdm->is_oversub,
                            &monterey_tdm->iarb_tdm_wrap_ptr,
                            monterey_tdm->iarb_tdm);
                      ...
                      soc_monterey_tdm_t *monterey_tdm)
{
    ...
    for (group = 0; group < MONTEREY_MMU_OVS_GROUP_COUNT; group++) {
        ...
        MONTEREY_TDM_MMU_SPEED_GET(unit, speed_class);
        _soc_monterey_mmu_ovs_speed_class_map_get(unit, speed_class, &speed_group, &speed_spacing);

        if (speed_group == mn_ovs_speed_None) {
            continue;
        }
    ...
    for (wt_group = 0; wt_group < MONTEREY_MMU_OVS_WT_GROUP_COUNT; wt_group++) {
        ...
        for (index = 0; index < MONTEREY_MMU_OVS_GROUP_TDM_LENGTH; index++) {
        ...
        for (slot = 0; slot < MONTEREY_MMU_OVS_GROUP_TDM_LENGTH; slot++) {
            ...
            if (mmu_port == 0x7f) {
                ...
                if (speed_class == 0) {
                    ...
                    _soc_monterey_mmu_ovs_speed_class_map_get(unit, speed_class,
                                                            &speed_group, &speed_spacing);
                    MONTEREY_TDM_WT_GRP_RESERVE(unit, wt_group, speed_group);
                }
            }
...
/* temp fix for tdm issue once new tdm is in place this check shold be removed*/
    ...
    values[0] = monterey_tdm->iarb_tdm_wrap_ptr;
    fields[1] = DISABLEf;
    values[1] = 0;
    SOC_IF_ERROR_RETURN
                                ...
                                _soc_mmu_device_info_t *devcfg, int lossless)
{
    ...
    rqe_entry_shared_total = _MONTEREY_MMU_TOTAL_RQE_ENTRY(unit) - 160 - 88;
 ...
 */
    if (IS_ROE_ENABLED(unit)) { 
        pool0_share = total_pool_size - _MN_MMU_EXPRESS_BUFFER_TOTAL_2 ; 
        pool1_share = _MN_MMU_EXPRESS_BUFFER_TOTAL_2;
    } else { 
    ...
    for (idx = 0; idx < _MONTEREY_MMU_NUM_POOL; idx++) {
        ...
        if (idx == 0) {  /* 43 % */
         ...
         }  else if (idx == 1 && IS_ROE_ENABLED(unit)) {  /* 56 % of total is reservef for express traffic */
    ...
    PBMP_ALL_ITER(unit, port) {
        ...
        for (idx = 0; idx < _MONTEREY_MMU_NUM_PG; idx++) {
             if (IS_ROE_ENABLED(unit) && (idx == 5 || idx == 7)) {
        ...
        for (idx = 0; idx < _MONTEREY_MMU_NUM_POOL; idx++) {
            ...
            if (idx == 0) {
                buf_port_pool->pool_limit = total_pool_size -  _MN_MMU_EXPRESS_BUFFER_TOTAL_2 ;
                buf_port_pool->pool_resume =
                            total_pool_size - (default_mtu_cells * 2) - _MN_MMU_EXPRESS_BUFFER_TOTAL_2;
            } else if (idx == 1) {
                buf_port_pool->pool_limit =  _MN_MMU_EXPRESS_BUFFER_TOTAL_2 ;
                buf_port_pool->pool_resume =
                            _MN_MMU_EXPRESS_BUFFER_TOTAL_2 - (default_mtu_cells * 2) ;
            }
        ...
        for (idx = 0; idx < si->port_num_uc_cosq[port]; idx++) {
            ...
            if (lossless) {
                ...
                if (IS_ROE_ENABLED(unit) && (idx == 1 || idx == 3))  {
        ...
        for (idx = 0;
            ...
            if(IS_ROE_ENABLED(unit) && ((idx == 15 || idx == 17) 
               && !IS_CPU_PORT(unit, port))) { 
    ...
    PBMP_ALL_ITER(unit, port) {
        ...
        for (idx = 0; idx < _MONTEREY_MMU_NUM_PG; idx++) {
             ...
             if (!lossless) {
                 if (IS_ROE_ENABLED(unit) && (idx == 5 || idx ==7)) 
                       buf_prigroup->pool_limit =_MN_MMU_EXPRESS_BUFFER_TOTAL_2 ;
                 else {  
                       buf_prigroup->pool_limit = total_pool_size - in_reserved - 
                                           _MN_MMU_EXPRESS_BUFFER_TOTAL_2 - 16;
                 }     
             }
       } 
    }
...
_soc_monterey_mmu_init(int unit)
{
    ...
    if (soc_feature(unit, soc_feature_untethered_otp)) {
        if (SOC_BOND_INFO_FEATURE_GET(unit, socBondInfoFeatureMmu12Mb)) {
            ...
            _soc_mn_skew_index = 1;
        } else {
            /*socBondInfoFeatureMmu6Mb*/
            _soc_mn_skew_index = 2;
        }
    } else {
                      ...
                      int lossless)
{
    ...
    for (i = 0; i < post_num_res; i++) {
        ...
        SOC_IF_ERROR_RETURN(
            soc_esw_portctrl_pgw_reconfigure(unit,
                                             post_res[i].logical_port,
                                             post_res[i].physical_port, 
                                             &data));
    }
...
const ser_monterey_skipped_mem_t monterey_skipped_mems[] = {
    ...
    /* Apache REG file doesn't have fields defined (eg. ECC) 
     * for these below memories, so by-pass these.
     */
    {MMU_INTFO_QCN_CNM_TIMER_TBLm, _SOC_ACC_TYPE_PIPE_ANY},
    {MMU_QCN_CNM_COUNTERm, _SOC_ACC_TYPE_PIPE_ANY},
    {MMU_QCN_QFBTBm, _SOC_ACC_TYPE_PIPE_ANY},
    {FP_GLOBAL_MASK_TCAMm, _SOC_ACC_TYPE_PIPE_ANY},
    {MMU_QCN_ENABLE_0m, _SOC_ACC_TYPE_PIPE_ANY},
    {MMU_MTRO_S1_MEM_0m, _SOC_ACC_TYPE_PIPE_ANY},
    /* Below SER_SPECIAL mems get recovered from alternate pipe in td2p,
     * not possible in AP */
    {L2_ENTRY_LPm, _SOC_ACC_TYPE_PIPE_ANY},
    {L3_DEFIP_ALPM_ECCm, _SOC_ACC_TYPE_PIPE_ANY},
    {L3_ENTRY_LPm, _SOC_ACC_TYPE_PIPE_ANY},
    /* Till we get resolution on HW JIRA 1736 */
    {LLS_L0_CONFIGm, _SOC_ACC_TYPE_PIPE_ANY},
    {LLS_L0_PARENTm, _SOC_ACC_TYPE_PIPE_ANY},
    {LLS_L1_CONFIGm, _SOC_ACC_TYPE_PIPE_ANY},
    {LLS_L1_PARENTm, _SOC_ACC_TYPE_PIPE_ANY},
    {LLS_L2_PARENTm, _SOC_ACC_TYPE_PIPE_ANY},
    {LLS_S1_CONFIGm, _SOC_ACC_TYPE_PIPE_ANY},
    {LLS_S1_PARENTm, _SOC_ACC_TYPE_PIPE_ANY},
    {INVALIDm}
};
...
STATIC soc_error_t
_soc_monterey_ser_mem_info_get (int unit, soc_mem_t mem, void** ser_mem_info)
{
    int i, j;
    _soc_mem_ser_en_info_t *ser_en_info = NULL;
...
soc_monterey_ser_tcam_test (int unit, _soc_ser_test_t test_type) {
    ...
    for (i = 0; tcams[i].mem != INVALIDm; i++) {
        ...
        _soc_monterey_perform_ser_test(unit, &test_data, test_type,
                                   ...
                                   int inject_only, int cmd, uint32 flags)
{
    ...
    if (((mem == VFP_TCAMm) || (mem == EFP_TCAMm)) &&
        (soc_mem_test_skip(unit,mem,index) == TRUE)) {
        return SOC_E_PARAM;
    }
