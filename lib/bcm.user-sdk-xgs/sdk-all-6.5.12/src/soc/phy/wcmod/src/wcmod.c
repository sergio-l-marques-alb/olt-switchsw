    ...
    if (!SOC_PBMP_MEMBER(PBMP_IL_ALL(unit), pc->port)) {
        if (SOC_INFO(unit).port_num_lanes[pc->port] >= 10) {
            ...
            *link = ws->accData; 
            ...
            /* Check CL49 PCS to see if transitioned through the expected states */
            SOC_IF_ERROR_RETURN
                (WC40_REG_READ(unit, ws, 0x00, 0x000081de, &data16));
            data16 &= GP2_REG_GP2_E_CL49_EN_MASK;
            data16 >>= GP2_REG_GP2_E_CL49_EN_SHIFT;

            if (*link == TRUE && data16 & (1 << pc->lane_num)) {
                SOC_IF_ERROR_RETURN
                    (WC40_REG_READ(unit, ws, 0x00, 0x00008366, &data16));
                /*CL49 RX FSM status register:
                               Bit 1: Current receive control symbol state
                               Bit 4: Latched receive control symbol state
                                       It will be set to 1 when current status is from control symbol to other symbols,
                                       until it is cleared after software read.
                   data16 != 0:   PCS is active.
                   data16 & 0x1:  The current code group is control symbol state.
                   data16 & 0x10: Between the last read operation and current read operation, at least one control symbol is received.
                   So this valid check is used to check if at least one control symbol is received and the PCS is active since the last SW read.
                   If no control symbol, it means the link is invalid.
                 */
                if (data16 != 0 && !(data16 & 0x1) && !(data16 & 0x10)) {
                    *link = FALSE;
                }
            }
