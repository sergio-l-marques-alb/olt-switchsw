...
#endif
                SOC_EXIT;
            } else if ((mem == FDA_FDA_MCm) && SOC_IS_JERICHO(unit) && (type == DCMN_ECC_ECC2B_DC)) {
                if (cache_enable) {
    ...
    char* msg)
{
    ...
    if (SOC_MEM_FIELD_VALID(unit, shadow_correct_info_p->mem, ECCf)) {
        SOC_REG_ABOVE_64_CLEAR(value);
        SOCDNX_IF_ERR_EXIT(dcmn_disable_block_ecc_check(unit,interrupt_id, shadow_correct_info_p->mem,block_instance,value,orig_value));
    }
    ...
    if (SOC_MEM_FIELD_VALID(unit, shadow_correct_info_p->mem, ECCf)) {
        SOCDNX_IF_ERR_EXIT(dcmn_disable_block_ecc_check(unit,interrupt_id,shadow_correct_info_p->mem,block_instance,orig_value,NULL));
    }

exit:
    if ((shadow_correct_info_p->mem == FDA_FDA_MCm) && SOC_IS_JERICHO(unit)) {
        /* restore the value*/
        SOCDNX_IF_ERR_EXIT(WRITE_FDA_ENABLE_DYNAMIC_MEMORY_ACCESSr(unit, val_bak));
    }
    ...
    char* msg)
{
    ...
    if (SOC_IS_DPP(unit) &&
        (dcmn_tbl_is_dynamic(unit, ecc_1b_correct_info_p->mem) &&
