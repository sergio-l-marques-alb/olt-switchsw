...
uint32 jer_entry_index[JER_PP_EM_SER_DB_TYPE_NOF_TYPES][JER_PP_EM_SER_NOF_HASH_RESULTS] =
            {{0, 0, 0, 0, 0, 0, 0, 0},            /* LEM    */
             {0, 0, 0, 0, 0, 0, 0, 0},            /* ISEM_A */
             {0, 0, 0, 0, 0, 0, 0, 0},            /* ISEM_B */
             {0, 0, 0, 0, 0, 0, 0, 0},            /* GLEM   */
             {0, 0, 0, 0, 0, 0, 0, 0},            /* ESEM_0 */
             {0, 0, 0, 0, 0, 0, 0, 0},            /* ESEM_1 */
             {0, 0, 0, 0, 0, 0, 0, 0},            /* OEMA   */
             {0, 0, 0, 0, 0, 0, 0, 0},            /* OEMB   */
             {0, 0, 0, 0, 0, 0, 0, 0}             /* RMAPEM */
            };
                                         ...
                                         uint32 val)
{
    ...
    switch (mem) {
        ...
        case KEYT_PLDT_MEM(OAMP, REMOTE_MEP_EXACT_MATCH):
            SOC_SAND_SOC_IF_ERROR_RETURN(res, 110, exit, READ_OAMP_ECC_ERR_2B_MONITOR_MEM_MASKr(unit, &reg_64_val));
            MASK_SER_INTERRUPTS_64B_SET(RMAPEM, RMAPEM_ERR_MEM_MASK_REGISTER, &reg_64_val, mask_val_64);
            SOC_SAND_SOC_IF_ERROR_RETURN(res, 120, exit, WRITE_OAMP_ECC_ERR_2B_MONITOR_MEM_MASKr(unit, reg_64_val));
            break;
                                            ...
                                            int val)
{
    ...
    switch (mem) {
        ...
        case KEYT_PLDT_MEM(PPDB_A, ISEM):
            SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  30,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, MANAGEMENT_UNIT_CONFIGURATION_REGISTER(PPDB_A,ISEM), REG_PORT_ANY, 0, MNGMNT_UNIT_ENABLE_FIELD(ISEM),  val));
            break;

        case KEYT_PLDT_MEM(PPDB_B, ISEM):
            SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  40,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, MANAGEMENT_UNIT_CONFIGURATION_REGISTER(PPDB_B,ISEM), REG_PORT_ANY, 0, MNGMNT_UNIT_ENABLE_FIELD(ISEM),  val));
            break;

        case KEYT_PLDT_MEM(EDB, GLEM):
            SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  50,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, MANAGEMENT_UNIT_CONFIGURATION_REGISTER(EDB,GLEM), REG_PORT_ANY, 0, MNGMNT_UNIT_ENABLE_FIELD(GLEM),  val));
            break;

        case KEYT_PLDT_MEM(EDB, ESEM):
            SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  60,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, MANAGEMENT_UNIT_CONFIGURATION_REGISTER(EDB,ESEM), REG_PORT_ANY, 0, MNGMNT_UNIT_ENABLE_FIELD(ESEM),  val));
            break;

        case KEYT_PLDT_MEM(EDB, ESEM_0):
            SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  70,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, MANAGEMENT_UNIT_CONFIGURATION_REGISTER(EDB,ESEM_0), REG_PORT_ANY, 0, MNGMNT_UNIT_ENABLE_FIELD(ESEM_0),  val));
            break;

        case KEYT_PLDT_MEM(EDB, ESEM_1):
            SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  80,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, MANAGEMENT_UNIT_CONFIGURATION_REGISTER(EDB,ESEM_1), REG_PORT_ANY, 0, MNGMNT_UNIT_ENABLE_FIELD(ESEM_1),  val));
            break;

        case KEYT_PLDT_MEM(PPDB_A, OEMA):
            SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  90,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, MANAGEMENT_UNIT_CONFIGURATION_REGISTER(PPDB_A,OEMA), REG_PORT_ANY, 0, MNGMNT_UNIT_ENABLE_FIELD(OEMA),  val));
            break;

        case KEYT_PLDT_MEM(PPDB_A, OEMB):
            SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  100,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, MANAGEMENT_UNIT_CONFIGURATION_REGISTER(PPDB_A,OEMB), REG_PORT_ANY, 0, MNGMNT_UNIT_ENABLE_FIELD(OEMB),  val));
            break;

        case KEYT_PLDT_MEM(OAMP, REMOTE_MEP_EXACT_MATCH):
            SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  110,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, MANAGEMENT_UNIT_CONFIGURATION_REGISTER(OAMP,REMOTE_MEP_EXACT_MATCH), REG_PORT_ANY, 0, MNGMNT_UNIT_ENABLE_FIELD(REMOTE_MEP_EXACT_MATCH),  val));
            break;
...
uint32 jer_pp_em_ser_lem_fix_counters(int unit)
{
    ...
    res = jer_pp_em_ser_management_enable_set(unit, KEYT_PLDT_MEM(PPDB_B, LARGE_EM), 1);
    SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);
...
uint32 jer_pp_em_ser_em_fix_counters(int unit, JER_PP_SER_EM_TYPE_INFO* ser_info)
{
    ...
    /* Enable the management unit */
    res = jer_pp_em_ser_management_enable_set(unit, ser_info->keyt_pldt_mem, 1);
    SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
...
int jer_pp_em_ser_lem(int unit, soc_mem_t mem, uint32 array_index, uint32 entry_offset)
{
    ...
    res = jer_pp_em_ser_management_enable_set(unit, mem, 0);
    SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
    addr = soc_mem_base(unit, mem) + soc_mem_index_count(unit, mem) * array_index + entry_offset;
    res = soc_mem_array_read(unit,
                             mem,
                             array_index,
                             MEM_BLOCK_ANY,
    ...
    char* msg)
{
    ...
    if (SOC_MEM_TYPE(unit,mem) == SOC_MEM_TYPE_XOR) {
        ...
        res = dcmn_interrupt_handles_corrective_action_for_xor(unit, block_instance, interrupt_id, shadow_correct_info_p, msg, 1);
        SOC_SAND_CHECK_FUNC_RESULT(res, 5, exit);

        /* Enable the management unit */
        res = jer_pp_em_ser_management_enable_set(unit, mem, 1);
        SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
    }
    ...
    char* msg)
{
    ...
    /* Disable the management unit */
    rc = jer_pp_em_ser_management_enable_set(unit, mem, 0);
    SOCDNX_IF_ERR_EXIT(rc);

    if (SOC_MEM_TYPE(unit,mem) == SOC_MEM_TYPE_XOR) {
        rc = dcmn_interrupt_handles_corrective_action_for_xor(unit, block_instance, interrupt_id, ecc_1b_correct_info_p, msg, 0);
        SOCDNX_IF_ERR_EXIT(rc);
    ...
    /* Enable the management unit */
    rc = jer_pp_em_ser_management_enable_set(unit, mem, 1);
    SOCDNX_IF_ERR_EXIT(rc);
