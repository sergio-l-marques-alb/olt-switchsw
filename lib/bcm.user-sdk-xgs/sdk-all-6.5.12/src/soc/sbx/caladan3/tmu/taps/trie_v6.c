/*
 * $Id: trie_v6.c,v 1.6 Broadcom SDK $
...
int _trie_v6_split_len_get(trie_node_t  *trie,
		       unsigned int *pivot,
		       unsigned int *length,
		       ...
		       unsigned int *bpm)
{
	...
	if (((*length > max_split_len) && (trie->count != max_count))) {
	    ...
	    rv = _trie_v6_split_len_get(trie->child[bit].child_node, 
...
int tmu_trie_v6_split_ut(unsigned int seed) 
{
    ...
    for (id=0; id < 4; id++) {
		...
		do {
			...
			if (dup) {                    
			    LOG_CLI((BSL_META("\n Duplicate at index[%d]:key[0x%08x:0x%08x:0x%08x:0x%08x:0x%08x] Retry!!!\n"), 
                                     index, pyld[index].key[0], pyld[index].key[1], pyld[index].key[2],
        ...
        if (SOC_SUCCESS(rv)) {
            ...
            for(index=0; index < numkey && rv == SOC_E_NONE; index++) {
                ...
                if (rv != SOC_E_NONE) {
                    ...
                    if (rv != SOC_E_NONE) {
                        LOG_CLI((BSL_META("SEARCH: Key=0x%x 0x%x 0x%x 0x%x  0x%x len %d SEARCH idx:%d failed on both trie!!!!\n"), 
                                 pyld[index].key[0], pyld[index].key[1], pyld[index].key[2], pyld[index].key[3],
...
#ifdef LOG
        LOG_CLI((BSL_META("+ Inserted Key=0x%x 0x%x 0x%x 0x%x 0x%x 0x%x vpn=0x%x Len=%d idx:%d\n"), 
                 pyld[index].key[0], pyld[index].key[1], pyld[index].key[2], pyld[index].key[3],
                 pyld[index].key[4], vrf, pyld[index].len, index));
#endif
        rv = trie_insert(trie, &pyld[index].key[0], NULL, pyld[index].len, &pyld[index].node);
        if (rv != SOC_E_NONE) {
            LOG_CLI((BSL_META("FAILED to Insert Key=0x%x 0x%x 0x%x 0x%x 0x%x vpn=0x%x Len=%d idx:%d\n"), 
                     pyld[index].key[0], pyld[index].key[1], pyld[index].key[2], pyld[index].key[3],
                     pyld[index].key[4], vrf, pyld[index].len, index));
        }
...
#endif
            if (rv != SOC_E_NONE) {
                LOG_CLI((BSL_META("SEARCH: Key=0x%x 0x%x 0x%x 0x%x 0x%x len %d SEARCH idx:%d failed!!!!\n"), 
                         pyld[index].key[0], pyld[index].key[1], pyld[index].key[2], pyld[index].key[3],
                         pyld[index].key[4], pyld[index].len, index));
                break;
            } else {
...
#ifdef VERBOSE
                LOG_CLI((BSL_META("Lkup[%d] key/len: 0x%x 0x%x 0x%x 0x%x 0x%x/%d Found Key/len: 0x%x 0x%x 0x%x 0x%x 0x%x/%d \n"),
                         index, pyld[index].key[0], pyld[index].key[1], pyld[index].key[2],
                         pyld[index].key[3], pyld[index].key[4], pyld[index].len, result_key[0],
                         result_key[1], result_key[2], result_key[3], result_key[4], result_len));
#endif
#endif
            }
        }
    ...
    while(num_deleted < numkey && rv == SOC_E_NONE) {
        ...
        if (pyld[index].len != 0xFFFFFFFF) {
            ...
            if (rv == SOC_E_NONE) {
                ...
                if (rv == SOC_E_NONE) {
#ifdef LOG
                    LOG_CLI((BSL_META("Deleted Key=0x%x 0x%x 0x%x 0x%x 0x%x Len=%d idx:%d Num-Key:%d\n"), 
                             pyld[index].key[0], pyld[index].key[1], pyld[index].key[2], pyld[index].key[3],
                             pyld[index].key[4], pyld[index].len, index, num_deleted));
#endif
                    ...
                    for (index=0; index < numkey; index++) {
                        ...
                        rv = trie_search(trie, &pyld[index].key[0], pyld[index].len, &pyldptr);
                        if (rv != SOC_E_NONE) {
                            LOG_CLI((BSL_META("ALL SEARCH after delete: Key= 0x%x 0x%x 0x%x 0x%x 0x%x len %d SEARCH idx:%d failed!!!!\n"), 
                                     pyld[index].key[0], pyld[index].key[1], pyld[index].key[2], pyld[index].key[3], pyld[index].key[4],
                    ...
                    LOG_CLI((BSL_META("Deleted Key=0x%x 0x%x 0x%x 0x%x 0x%x Len=%d idx:%d FAILED!!!\n"), 
                             pyld[index].key[0], pyld[index].key[1], pyld[index].key[2], pyld[index].key[3], pyld[index].key[4], 
                             pyld[index].len, index));
                    break;
                }
    ...
    if (datum) {
        ...
        DQ_TRAVERSE(&datum->list, elem) {
            ...
            LOG_CLI((BSL_META(" Pivot: 0x%x 0x%x 0x%x 0x%x 0x%x Len: %d \n"), 
                     velem->key[0], velem->key[1], velem->key[2], velem->key[3], 
...
int tmu_taps_bpm_trie_v6_ut(int id, unsigned int seed)
{
    ...
    do {
        ...
        if (rv != SOC_E_NONE) {
            LOG_CLI((BSL_META("FAILED to Insert PIVOT Key=0x%x 0x%x 0x%x 0x%x 0x%x Len=%d idx:%d\n"), 
                     pivot_pyld[pivot].key[0], pivot_pyld[pivot].key[1], pivot_pyld[pivot].key[2], pivot_pyld[pivot].key[3], pivot_pyld[pivot].key[4], 
			...
			pyld[index].key[4] &= MASK(pyld[index].len);
                    ...
                    if (!dup) {
                        ...
                        if (SOC_FAILURE(rv)) {
                            LOG_CLI((BSL_META("\n !! Failed to find LPM pivot for index[%d]:key[0x%08x:0x%08x:0x%08x:0x%08x:0x%08x] !!!!\n"),
                                     pfx,pyld[pfx+index].key[0],pyld[pfx+index].key[1],pyld[pfx+index].key[2],
                                     pyld[pfx+index].key[3],pyld[pfx+index].key[4]));
                        } 
                    }
                ...
                if (SOC_SUCCESS(rv)) {
                    ...
                    if (SOC_FAILURE(rv)) {
                        LOG_CLI((BSL_META("\n !! Failed insert prefix into pivot trie"
                                 ...
                                 pfx+index,pyld[pfx+index].key[0],pyld[pfx+index].key[1],pyld[pfx+index].key[2],
                                 pyld[pfx+index].key[3],pyld[pfx+index].key[4]));
                    } else {
            ...
            for (i=0; i <= pivot; i++) {
                LOG_CLI((BSL_META("\n --- TRIE domain dump: Pivot: 0x%x 0x%x 0x%x 0x%x 0x%x len=%d ----- \n"),
                         pivot_pyld[i].key[0], pivot_pyld[i].key[1], pivot_pyld[i].key[2],
                         pivot_pyld[i].key[3], pivot_pyld[i].key[4], pivot_pyld[i].len));
            ...
            if (SOC_SUCCESS(rv) && ++pivot < _MAX_TEST_PIVOTS_) {
                ...
                if (SOC_SUCCESS(rv)) {
                    ...
                    LOG_CLI((BSL_META("BPM for split pivot: 0x%x 0x%x 0x%x 0x%x 0x%x / %d = [0x%x 0x%x 0x%x 0x%x 0x%x] \n"),
                             pivot_pyld[pivot].key[0], pivot_pyld[pivot].key[1], pivot_pyld[pivot].key[2],
                             pivot_pyld[pivot].key[3], pivot_pyld[pivot].key[4],
                             pivot_pyld[pivot].len, bpm[0], bpm[1], bpm[2], bpm[3], bpm[4]));
                } else {
    ...
    do {
        ...
        if (!dup) {
            ...
            if (SOC_FAILURE(rv)) {
                LOG_CLI((BSL_META("\n !! Failed to find LPM pivot for index[%d]:key[0x%08x:0x%08x:0x%08x:0x%08x:0x%08x] !!!!\n"),
                         pfx,pyld[pfx].key[0],pyld[pfx].key[1],pyld[pfx].key[2],pyld[pfx].key[3],pyld[pfx].key[4]));
            } 
        } else {
            pyld[pfx].len++;
        }
	...
	if (pyld[pfx].len >= 1) {
		...
		LOG_CLI((BSL_META("!!!!! BPM search Test FAILED to propagate add \n"
                                  " index[%d]:key[0x%08x:0x%08x:0x%08x:0x%08x:0x%08x] len=%d!!!!\n"),
                         pfx, pyld[pfx].key[0], pyld[pfx].key[1], pyld[pfx].key[2],
                         pyld[pfx].key[3], pyld[pfx].key[4], pyld[pfx].len));
		assert(0);
	    }
