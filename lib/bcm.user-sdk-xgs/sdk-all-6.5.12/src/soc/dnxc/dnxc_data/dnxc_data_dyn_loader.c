...
dnxc_data_dyn_loader_parse_init(
    int unit)
{
    void *cur_top, *cur_module, *cur_sub_module, *node;
    char file_path[RHFILE_MAX_SIZE], full_file_path[RHFILE_MAX_SIZE];
    char file_name[RHFILE_MAX_SIZE];
    const char *chip_name;
    char module_name[RHSTRING_MAX_SIZE];
    char sub_module_name[RHSTRING_MAX_SIZE];
    int is_file_exists, rv;
     ...
     * 1. check if chip folder exists
     * 2.  -check if file exists. If file exists - try to open it.
     *     - If file not exists - Do nothing (since dynamic data is optional).
     */
    rv = dbx_file_get_full_path(SOC_CHIP_STRING(unit), file_path, full_file_path);
    if (rv == _SHR_E_NOT_FOUND)
    {
        /** chip folder doesn't exist, go to exit */
        SHR_EXIT();
    }

    is_file_exists = dbx_file_exists(full_file_path);

    if (is_file_exists == TRUE)
    {
        /** get pointer to top of the XML file */
        cur_top = dbx_file_get_xml_top(SOC_CHIP_STRING(unit), file_path, "device_data", 0);
