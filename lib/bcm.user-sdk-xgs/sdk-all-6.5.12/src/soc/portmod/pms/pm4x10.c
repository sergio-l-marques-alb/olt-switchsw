...
STATIC
int _pm4x10_txpi_sdm_scheme_set(int unit, int port, pm_info_t pm_info, portmod_txpi_sdm_type_t type);
...
int _pm4x10_port_tsc_is_master_pll(int unit, int port, pm_info_t pm_info)
{
   ...
   _SOC_IF_ERR_EXIT(_pm4x10_pm_version_get(unit, port, pm_info, &pm_version));
...
STATIC
int _pm4x10_port_ts_adjust_get(int unit, int port, pm_info_t pm_info,
                        ...
                        phymod_tx_t ext_phy_tx_params[])
{
    ...
    if(!pm_is_bypassed){
        ...
        rv = _pm4x10_port_ts_adjust_get(unit, port, pm_info, config->speed, &ts_adjust);
        rv = xlmac_timestamp_delay_set(unit, port, ts_adjust, config->speed);
        _SOC_IF_ERR_EXIT(rv);
...
int _pm4x10_pm_xlport_init (int unit, int port, pm_info_t pm_info,
                            ...
                            int timestamp_mode, uint32 txpi_sdm_scheme)
{
    uint32 reg_val, port_mode;
    int    three_ports_mode, rv;
    uint32 txpi_lane_select[portmodPortModeCount][PM4X10_LANES_PER_CORE]= {
        {0, 1, 2, 3}, {0, 1, 2, 2}, {0, 0, 2, 3}, {0, 0, 2, 2}, {0, 0, 0, 0}
    };
    pm_version_t pm_version;
    SOC_INIT_FUNC_DEFS;
    ...
    if (PORTMOD_PM4x10_IS_GEN3_28nm(pm_version)) {
        /* Set txpi sdm scheme */
        _SOC_IF_ERR_EXIT(_pm4x10_txpi_sdm_scheme_set(unit, port, pm_info, txpi_sdm_scheme));
        /* Set txpi lane select */
        _SOC_IF_ERR_EXIT(_pm4x10_txpi_lane_select_set(unit, port, pm_info, (uint32_t *)txpi_lane_select[port_mode]));
    }
                        ...
                        const portmod_port_add_info_t* add_info)
{
    ...
    if(!pm_is_active){
        ...
        rv = _pm4x10_pm_xlport_init (unit, port, pm_info, port_index,
                 ...
                 add_info->init_config.txpi_sdm_scheme);
        _SOC_IF_ERR_EXIT(rv);
...
/* Following code applies to the phy 54290 */
    ...
    if(!link) {
        ...
        if(autoneg.enabled && autoneg.locked) {
            ...
            _SOC_IF_ERR_EXIT(_pm4x10_port_ts_adjust_get(unit, port, pm_info, interface_config.speed, &ts_adjust));
                     ...
                     pm_info_t pm_info, const portmod_port_mode_info_t *mode)
{
    ...
    if (PORTMOD_PM4x10_IS_GEN3_28nm(pm_version)) {
        /* Set txpi lane select */
        _SOC_IF_ERR_EXIT(_pm4x10_txpi_lane_select_set(unit, port, pm_info, (uint32_t *)txpi_lane_select[mode->cur_mode]));
    }
...
{
    return (xlmac_timesync_tx_info_get(unit, port, tx_info));
}
...
int _pm4x10_txpi_sdm_scheme_set(int unit, int port, pm_info_t pm_info, portmod_txpi_sdm_type_t type)
{
    ...
    _SOC_IF_ERR_EXIT(READ_XLPORT_TXPI_CSR_CTRL1r(unit, phy_acc, &reg_val));
    soc_reg_field_set(unit, XLPORT_TXPI_CSR_CTRL1r, &reg_val, TXPI_CSR_CTRL_SDM_TYPEf, type);
    _SOC_IF_ERR_EXIT(WRITE_XLPORT_TXPI_CSR_CTRL1r(unit, phy_acc, reg_val));
...
int pm4x10_port_txpi_override_set (int unit, int port, pm_info_t pm_info, const portmod_txpi_override_t *override)
{
    ...
    if (override->enable == TRUE) {
        /*
         *Need to use COMPILER_64_XXX macros to get 
         *reg_value = txpi_dsm_value_1ppm * override->value;
         */
        COMPILER_64_SET(reg_value, TXPI_DSM_VALUE_1PPM_HI, TXPI_DSM_VALUE_1PPM_LO);
        COMPILER_64_UMUL_32(reg_value, (uint32) override->value);

        for (i = 0; i < MAX_PORTS_PER_PM4X10; i++) {
            if (bitmap & (0x1 << i)) {
                soc_reg32_set(unit, over_reg_low[i], phy_acc, 0, COMPILER_64_LO(reg_value));
                soc_reg32_set(unit, over_reg_high[i], phy_acc, 0, (COMPILER_64_HI(reg_value) & 0xffff));
            }
        }
    }
...
int pm4x10_port_txpi_override_get (int unit, int port, pm_info_t pm_info, portmod_txpi_override_t *override)
{
    ...
    uint64 tmp_reg_val64, txpi_dsm_value_1ppm;
    ...
    if (override->enable == TRUE) {
        ...
        override->value = (uint16_t)COMPILER_64_LO(tmp_reg_val64);
    } else {
        override->value = 0;
    }
...

{
    ...
    COMPILER_64_UDIV_64(tmp_reg_val64,txpi_dsm_value_1ppm);
    *value = (uint16_t)COMPILER_64_LO(tmp_reg_val64);
