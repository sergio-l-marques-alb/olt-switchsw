    ...
    int threshold_value)
{
    ...
    uint32 action_required = SOC_DNXF_CGM_ACTION_NONE;
    uint32 in_soft_reset = 0;
    _shr_dnxf_cgm_index_type_t index_type;
    soc_dnxc_isolation_status_t device_isolation_status = soc_dnxc_isolation_status_active;
    ...
    action_required = threhold_to_reg_info_binding_table[threshold_type].action_required;

    /*
     * Several thresholds require soft init in order to write to them properly
     */
    if (action_required & SOC_DNXF_CGM_ACTION_APPLY_SOFT_RESET)
    {
        rc = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_fabric_topology_isolate_get, (unit, &device_isolation_status));
        if (device_isolation_status != soc_dnxc_isolation_status_isolated)
        {
            SHR_ERR_EXIT(_SHR_E_UNAVAIL, "Soft reset should be initiated only in isolation mode!");
        }
        ...
        case soc_dnxf_cgm_reg_structure_1:
            ...
            SHR_IF_ERR_EXIT(soc_dnxf_cgm_handle_register_structure_1
                            (unit, start_index, end_index, block_start_indx, block_end_indx, index_type, lr_nlr_offset,
                             profile_id, action_required, threshold_type, _SHR_FABRIC_FLAG_SET, &threshold_value));
            break;
...
exit:
    /*
     * In order to cover all of the error cases the procedure of exiting the soft reset is after the exit label
     */
    if ((action_required & SOC_DNXF_CGM_ACTION_APPLY_SOFT_RESET) && in_soft_reset)
    {
        rc = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_drv_soft_init, (unit, SOC_DNXC_RESET_ACTION_OUT_RESET));
        /*
         * Change the return value if error but will not go to exit label
         */
        SHR_IF_ERR_CONT(rc);
    }
    SHR_FUNC_EXIT;
    ...
    int *threshold_value)
{
    ...
    switch (threshold_handle_case)
    {
        case soc_dnxf_cgm_reg_structure_1:
            ...
            SHR_IF_ERR_EXIT(soc_dnxf_cgm_handle_register_structure_1
                            (unit, start_index, end_index, block_start_indx, block_end_indx, index_type, lr_nlr_offset,
                             profile_id, action_required, threshold_type, _SHR_FABRIC_FLAG_GET, threshold_value));
            break;
        case soc_dnxf_cgm_reg_structure_2:     /* Special case when pipe dimension is in the registers field */
            SHR_IF_ERR_EXIT(soc_dnxf_cgm_handle_register_structure_2
                            (unit, start_index, end_index, block_start_indx, block_end_indx, lr_nlr_offset, profile_id,
    ...
    int *pipe)
{
    ...
    if (!(threshold_id & _SHR_FABRIC_TH_INDEX_PIPE_LEVEL_INDICATION))
    {
    ...
    /** Decode Level */
    if (((threshold_id & _SHR_FABRIC_TH_INDEX_LEVEL_MASK) >> (_SHR_FABRIC_TH_INDEX_LEVEL_NOF_BITS - 1)) != 0)
    {
        *level = -1;
    }
    else
    {
        *level = threshold_id & _SHR_FABRIC_TH_INDEX_LEVEL_MASK;
    }

    /** Decode Pipe */
    if (((threshold_id >> (_SHR_FABRIC_TH_INDEX_LEVEL_NOF_BITS + _SHR_FABRIC_TH_INDEX_PIPE_NOF_BITS - 1)) &
         _SHR_FABRIC_TH_INDEX_PIPE_MASK) != 0)
    {
        *pipe = -1;
    }
    else
    {
        *pipe = (threshold_id >> _SHR_FABRIC_TH_INDEX_LEVEL_NOF_BITS) & _SHR_FABRIC_TH_INDEX_PIPE_MASK;
    }
        ...
        for (dim0 = start_index[0]; dim0 <= end_index[0]; dim0++)
        {
                 ...
                 dim1++)
            {
                ...
                if (set_or_get == _SHR_FABRIC_FLAG_SET)
                {
                    ...
                    if ((action_required & SOC_DNXF_CGM_ACTION_APPLY_OFFSET) && i == 1)
                    {
                        /*
                         * Check if the threshold value is big enough to apply offset
                         */
                        if (threshold_value_tmp - SOC_DNXF_FULL_FIFO_OFFSET < 0)
                        {
                            SHR_ERR_EXIT(_SHR_E_PARAM, "Value for this threshold cannot be smaller than %d",
                                         SOC_DNXF_FULL_FIFO_OFFSET);
                        }
                        threshold_value_tmp -= SOC_DNXF_FULL_FIFO_OFFSET;
                    }
