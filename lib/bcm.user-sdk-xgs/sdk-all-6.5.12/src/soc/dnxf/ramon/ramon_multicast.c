...
soc_ramon_multicast_egress_add(int unit, soc_multicast_t group, soc_gport_t port)
{
    ...
    SHR_IF_ERR_EXIT(READ_MCT_MCTm(unit, 0, MEM_BLOCK_ANY, group, data));
#else /* ifndef SOC_RAMON_MULTICAST_TABLE_UPDATE_READ_ENABLED */
...
soc_ramon_multicast_egress_get(int unit, soc_multicast_t group, int port_max, soc_gport_t *port_array, int *port_count)
{
    ...
    SHR_IF_ERR_EXIT(soc_ramon_fabric_multicast_read_info(unit, group, &table, &index, &is_first_half));
    SHR_IF_ERR_EXIT(READ_MCT_MCTm(unit, table, MEM_BLOCK_ANY, index, data));
