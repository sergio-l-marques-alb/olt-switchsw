...
*/
    ...
    case IFP_POLICY_TABLEm:
        if (SOC_IS_TRIDENT3X(unit)) {
            ...
            /* If the error is inject to wide view, and the reported mem
               is IFP_POLICY_TABLE, but the index is still based on wide
               view.*/
            mem_list[1] = IFP_POLICY_TABLE_WIDEm; iratio[0] = 1;
        } else {
            return SOC_E_UNAVAIL;
        }
         break;
...
#define _SOC_IF_ERR_L2X_THAW(rv, mem)                                          \
    ...
    /* Determine RAM count and indexes for all RAMs */
        ...
        if (SOC_IS_MONTEREY(unit)) {
            SOC_IF_ERROR_RETURN
                (_soc_monterey_mem_sram_info_get(unit, mem, index, &sram_info));
        } else 
#endif /* BCM_MONTEREY_SUPPORT */
#if defined(BCM_APACHE_SUPPORT)
        if (SOC_IS_APACHE(unit)) {
