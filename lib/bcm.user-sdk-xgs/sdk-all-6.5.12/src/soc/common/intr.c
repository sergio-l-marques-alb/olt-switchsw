...
soc_get_interrupt_id_specific(int unit, int reg_adress, int reg_block, int field_bit, int* interrupt_id)
{
    ...
    *interrupt_id = -1;
    ...
    for(i=0; i < nof_interrupts; i++) {
        /* check block */
        if(soc_interrupt_is_supported(unit, SOC_BLOCK_INFO(unit, blk_indx).number, i) == 0x0) {
            continue; 
        }
...
{
    int index = port;
#if defined(BCM_PETRA_SUPPORT) || defined(BCM_DNXF_SUPPORT)
    soc_interrupt_db_t *interrupts_arr;
    soc_block_types_t block_types;
    ...
    if(SOC_IS_ARAD(unit) || SOC_IS_DNXF(unit)){
        if(SOC_BLOCK_IN_LIST(block_types, SOC_BLK_CLP)) {
            ...
            index = (SOC_IS_JERICHO(unit) || SOC_IS_DNXF(unit)) ? SOC_PORT_BLOCK_NUMBER(unit, port) : SOC_PORT_BLOCK_NUMBER(unit, port) + SOC_MAX_NUM_CLP_BLKS;
        } 
    }
#endif /* BCM_PETRA_SUPPORT */
...
int soc_interrupt_is_all_mask(int unit, int *is_all_mask){
    ...
    if (soc_feature(unit, soc_feature_cmicx)) {
        /* interrupt number is CHIP_INTR_LOW_PRIORITY=119 */
        return soc_cmic_intr_is_mask(unit, CHIP_INTR_LOW_PRIORITY, is_all_mask);
    }
#endif
