...
#ifdef BCM_DNX_SUPPORT
    if (SOC_IS_DNX(unit)) {
       /* Since Jer2 has the dummy blocks CDMAC defined in the regsfile, we need
          to assign them to the CDPORT block */
        if (regblktype[0] == SOC_BLK_CDMAC && SOC_REG_INFO(unit, reg).regtype == soc_portreg) {
            regblktype = SOC_REG_INFO(unit, CDPORT_SW_FLOW_CONTROLr).block;
        }
    }
#endif
