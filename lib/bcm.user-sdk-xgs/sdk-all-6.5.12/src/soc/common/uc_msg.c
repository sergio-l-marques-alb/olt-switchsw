...
extern uint32 ft_firmware_version;
...
soc_cmicx_sw_intr(int unit, void *data)
{
    ...
    uint32 uc0_addr = soc_reg_addr(unit, ICFG_MHOST0_SW_PROG_INTRr, REG_PORT_ANY, 0);
    uint32 uc1_addr = soc_reg_addr(unit, ICFG_MHOST1_SW_PROG_INTRr, REG_PORT_ANY, 0);

    /* Bit-0 = PCIe Host */
    int intr0 = soc_cmic_mcs_read(unit, uc0_addr) & 1;
    int intr1 = soc_cmic_mcs_read(unit, uc1_addr) & 1;
    ...
    /* Bit-0 = PCIe Host */
    m0_u0_sw_prg_intr = u0_val & 1;
    m0_u1_sw_prg_intr = u1_val & 1;
    m0_u2_sw_prg_intr = u2_val & 1;
    m0_u3_sw_prg_intr = u3_val & 1;

    /* Clear Interrupts */
    if (intr0) 
    {
        soc_cmic_mcs_write(unit, uc0_addr, 0);
    }
    if (intr1) 
    {
        soc_cmic_mcs_write(unit, uc1_addr, 0);
    }
                          ...
                          soc_cmic_uc_appl_cb_t *shutdown_cb, void *cb_data)
{
    ...
    if(MOS_MSG_CLASS_1588 == msg_class) {
        ptp_firmware_version = soc_ntohl(rcv.s.data);
    }
#endif
    /* BS Msg Class */
    if(MOS_MSG_CLASS_BS == msg_class) {
        bs_firmware_version =  soc_ntohl(rcv.s.data);
    }
    ...
    if(msg_class == MOS_MSG_CLASS_FT) {
        ft_firmware_version = soc_ntohl(rcv.s.data);
    }
#endif /* INCLUDE_FLOWTRACKER */
    return (SOC_E_NONE);
}
