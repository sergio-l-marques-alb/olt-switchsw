...
soc_device_reset(int unit, int mode, int action)
{
    ...
    if (SOC_IS_DNX(unit)) {
        return (soc_dnx_device_reset(unit, mode, action));
    }
#endif
...
#if defined(LE_HOST)
        if ((!SAL_BOOT_PLISIM) && (!SAL_BOOT_XGSSIM) && (!SAL_BOOT_BCMSIM)) {
            cmicx_pktdma_ep_to_cpu_hdr_endian_set(unit, 0, 0);
        }
#endif
        ...
        for (cmc = cmc_start; cmc <= cmc_end; cmc++) {
            if (big_packet) {
                for(y = 0; y < CMICX_N_DMA_CHAN; y++) {
                    cmicx_pktdma_data_endian_set(unit, cmc, y);
                }
            }
            if (big_other) {
                /* Set Packet DMA Descriptor and EP_TO_CPU_HDR Endianness */
                for(y = 0; y < CMICX_N_DMA_CHAN; y++) {
                    cmicx_pktdma_desc_endian_set(unit, cmc, y);
                    cmicx_pktdma_ep_to_cpu_hdr_endian_set(unit, cmc, y);
                }
#ifdef BCM_SBUSDMA_SUPPORT
                if (soc_feature(unit, soc_feature_sbusdma)) {
                    for(y = 0; y < SOC_SBUSDMA_CH_PER_CMC; y++) {
...
#ifdef BCM_SBUSDMA_SUPPORT
                if (soc_feature(unit, soc_feature_sbusdma)) {
                    for(y=0; y < CMIC_CMCx_SBUSDMA_CHAN_MAX ; y++) {
                        /* dma/slam */
...
soc_autoz_get(int unit, soc_port_t port, int *enable)
{
...
#endif /* defined(BCM_ESW_SUPPORT) || defined(BCM_SIRIUS_SUPPORT) || defined(BCM_SAND_SUPPORT) */
  ...
  defined(BCM_SAND_SUPPORT) || defined(BCM_CALADAN3_G3P1_SUPPORT) || defined(PORTMOD_SUPPORT)
...
#if defined(BCM_XGS3_SWITCH_SUPPORT) || defined(BCM_SIRIUS_SUPPORT) || defined(BCM_PETRA_SUPPORT)
/*
...
#if defined(BCM_TRIDENT_SUPPORT) || defined(BCM_PETRA_SUPPORT) || defined(BCM_CALADAN3_SUPPORT)
int
...
#if defined(BCM_TRIDENT_SUPPORT) || defined(BCM_TRIUMPH3_SUPPORT) || defined(BCM_TRIDENT2_SUPPORT)
    int         led_num;
