...
STATIC intr_data_t _intr_data[CMIC_CMC_NUM_MAX][CMIC_SBUSDMA_NUM_MAX];
...
{
    if (vchan < 0 ||
         vchan >= SOC_PCI_CMCS_NUM(unit) * CMIC_SBUSDMA_NUM_MAX) {
        return SOC_E_PARAM;
    }
    *cmc = vchan / CMIC_SBUSDMA_NUM_MAX;
    *ch =  vchan % CMIC_SBUSDMA_NUM_MAX;
...
cmicx_sbusdma_intr_init(int unit)
{
    ...
    handle = sal_alloc(sizeof(soc_cmic_intr_handler_t)*CMIC_CMC_NUM_MAX*
                       CMIC_SBUSDMA_NUM_MAX, "sbusdma_interrupt");
    if (handle == NULL) {
        return SOC_E_MEMORY;
    }
    hitr = handle;
    ...
    rv = soc_cmic_intr_register(unit, handle,
                           CMIC_CMC_NUM_MAX*CMIC_SBUSDMA_NUM_MAX);
    sal_free(handle);
