...
typedef struct sal_thread_journaling_not_allowed_ll_entry_d {
    struct sal_thread_journaling_not_allowed_ll_entry_d *next;
    sal_thread_t tid;
} sal_thread_journaling_not_allowed_ll_entry_t;

typedef struct sal_thread_journaling_not_allowed_d {
    sal_thread_journaling_not_allowed_ll_entry_t *head;
} sal_thread_journaling_not_allowed_t;
 ...
 * see .h file
 */
int sal_thread_is_journaling_allowed(void)
{
    int result = 1;
    sal_thread_journaling_not_allowed_ll_entry_t *entry;
    sal_thread_t current_thread = sal_thread_self();

    entry = sal_thread_journaling_not_allowed.head;

    while(NULL != entry)
    {
        if(current_thread == entry->tid) {
            result = 0;
            break;
        }
...
/**
 * \brief - add the current thread to the thread array
 */
STATIC void sal_thread_journaling_thread_not_allowed_remove(sal_thread_t thread)
{
    sal_thread_journaling_not_allowed_ll_entry_t *prev = NULL;
    sal_thread_journaling_not_allowed_ll_entry_t *cur = sal_thread_journaling_not_allowed.head;

    while(NULL != cur) {
        if(thread == cur->tid) {
            /* check if head entry is to be removed */
            if(cur == sal_thread_journaling_not_allowed.head) {
                sal_thread_journaling_not_allowed.head = cur->next;
            } else {
        ...
        /* move previous and current entry ptrs */
        prev = cur;
        cur = cur->next;
    }
}
 ...
 * \brief - add the current thread to the thread array
 */
STATIC int sal_thread_journaling_thread_not_allowed_add(sal_thread_t thread)
{
    sal_thread_journaling_not_allowed_ll_entry_t *new = NULL;
    sal_thread_journaling_not_allowed_ll_entry_t *last = sal_thread_journaling_not_allowed.head;

    new = (sal_thread_journaling_not_allowed_ll_entry_t *)malloc(sizeof(sal_thread_journaling_not_allowed_ll_entry_t));
    ...
    /* insert the entry at the end of the linked list */
    if(NULL == last) {
        sal_thread_journaling_not_allowed.head = new;
        return 0;
    }
...
#if defined(BCM_DNX_SUPPORT) || defined(BCM_DNXF_SUPPORT)
    if(sal_thread_journaling_thread_not_allowed_add((sal_thread_t)id) != 0) {
        return (SAL_THREAD_ERROR);
    }
#endif
...
#if defined(BCM_DNX_SUPPORT) || defined(BCM_DNXF_SUPPORT)
    sal_thread_journaling_thread_not_allowed_remove(thread);
#endif
