...
extern utilex_sw_state_occ_bitmap_t *utilex_occ_bitmap_data[SOC_MAX_NUM_DEVICES];

#include <soc/dnx/swstate/auto_generated/access/utilex_occupation_bitmap_access.h>
/*
 * { 
 */
 ...
 */
const static uint8 utilex_bitmap_val_to_first_one[UTILEX_OCC_BM_ARRAY_MAP_LEN] = {
 ...
 * Total number of meaningful entries is 256 (= number of bit combinations in a byte)
 */
const static uint8 utilex_bitmap_val_to_first_zero[UTILEX_OCC_BM_ARRAY_MAP_LEN] = {
    0,
    1, 0,
    2, 0, 1, 0,
    3, 0, 1, 0, 2, 0, 1, 0,
 ...
 * Total number of meaningful entries is 256 (= number of bit combinations in a byte)
 */
const static uint8 utilex_bitmap_val_to_last_zero[UTILEX_OCC_BM_ARRAY_MAP_LEN] = {
    ...
    uint32 max_nof_dss)
{
    ...
    SHR_IF_ERR_EXIT(UTILEX_OCC_BITMAP_ACCESS.init(unit));
    ...
    /*
     * This is a good place to invoke utilex_occ_bm_tests() if unit testing is required.
     */
exit:
    SHR_FUNC_EXIT;
}
    ...
    UTILEX_OCC_BM_T ** bit_map_ptr_ptr)
{
    ...
    *bit_map_ptr_ptr = occ_bitmap_data[unit]->dss_array[bit_map_index];
exit:
    SHR_FUNC_EXIT;
}
/*
    ...
    int with_deinit)
{
    ...
    /*
     * Run through all occupied ds (a set bit in 'occupied_dss') and destroy them one by one
     */
    for (bit_map_index = 0; bit_map_index < max_nof_dss; bit_map_index++)
    {
        SHR_IF_ERR_EXIT(UTILEX_OCC_BITMAP_ACCESS.occupied_dss.bit_get(unit, bit_map_index, &bit_val));
        ...
        {
            /*
             * 'bit_map_index' is now the index of an occupied bit map.
             */
            UTILEX_OCC_BM_CONVERT_BITMAP_INDEX_TO_HANDLE(bit_map, bit_map_index);
            SHR_IF_ERR_EXIT(utilex_occ_bm_destroy(unit, bit_map));
        }
    }
    if (with_deinit)
    {
        SHR_IF_ERR_EXIT(UTILEX_OCC_BITMAP_ACCESS.occupied_dss.free(unit));
        SHR_IF_ERR_EXIT(UTILEX_OCC_BITMAP_ACCESS.dss_array.ptr_free(unit));
        /*
    ...
    uint8 *found)
{
    ...
    SHR_IF_ERR_EXIT(utilex_occ_bm_get_next_range_helper(unit, bit_map, val, 0, 0, start, end, forward, place, found));
    SHR_IF_ERR_EXIT(UTILEX_OCC_BITMAP_ACCESS.dss_array.size.get(unit, bit_map_index, &size));
    if (*place >= size)
    {
        *place = 0;
    ...
    UTILEX_OCC_BM_PTR * bit_map)
{
    ...
    SHR_IF_ERR_EXIT(utilex_occ_bm_init_info_clear(&bm_restored));
    /*
     * copy init info 
     */
    ...
    for (indx = 0; indx < (num_bits + 2); ++indx)
    {
        ...
        if (indx == 19)
        {
            ...
            for (ii = 0; ii < 2; ii++)
            {
                ...
                utilex_ret =
                    utilex_occ_bm_get_next_in_range(unit, bitmap, start, end, forward[ii], 0, &local_place,
                                                    &local_found);
                if (SHR_FAILURE(utilex_ret))
                {
                    goto exit;
    ...
    uint8 silent)
{
    ...
    pass = TRUE;
    utilex_ret = UTILEX_OCC_BITMAP_ACCESS.max_nof_dss.get(unit, &max_nof_dss);
    if (SHR_FAILURE(utilex_ret))
    {
        goto exit;
    }
    ...
    utilex_ret = utilex_occ_bm_destroy_all(unit, FALSE);
    ...
    uint8 silent)
{
    ...
    utilex_pass = utilex_occ_bm_test_5(unit, silent);
    if (!utilex_pass)
    {
        fail = TRUE;
    }
...
shr_error_e
utilex_occ_bm_dummy_empty_function_to_make_compiler_happy(
    int unit)
{
    SHR_FUNC_INIT_VARS(unit);
    SHR_FUNC_EXIT;
