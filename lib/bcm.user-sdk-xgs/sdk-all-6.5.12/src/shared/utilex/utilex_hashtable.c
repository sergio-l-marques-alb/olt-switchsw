...
#include <soc/dnx/swstate/auto_generated/access/utilex_hash_table_access.h>
/*
 * { 
 */
    ...
    uint8 **tmp_buf_ptr_ptr)
{
    ...
    *tmp_buf_ptr_ptr =
        ((utilex_sw_state_hash_table_t *)
         sw_state_roots_array[unit][UTILEX_HASH_TABLE_MODULE_ID])->hashs_array[hash_table_index]->hash_data.tmp_buf;
exit:
    SHR_FUNC_EXIT;
}
    ...
    uint8 **tmp_buf2_ptr_ptr)
{
    ...
    *tmp_buf2_ptr_ptr =
        ((utilex_sw_state_hash_table_t *)
         sw_state_roots_array[unit][UTILEX_HASH_TABLE_MODULE_ID])->hashs_array[hash_table_index]->hash_data.tmp_buf2;
exit:
    SHR_FUNC_EXIT;
}
uint32
    ...
    uint32 max_nof_hashs)
{
    ...
    SHR_IF_ERR_EXIT(UTILEX_HASH_TABLE_ACCESS.init(unit));
    ...
    /*
     * If unit test is required, this is a good place for it.
     * utilex_hash_table_test_1(unit) ;
     */
    ...
    UTILEX_HASH_TABLE_INIT_INFO init_info)
{
    ...
    *hash_table_ptr = hash_table;
     ...
     */
    SHR_IF_ERR_EXIT(UTILEX_HASH_TABLE_ACCESS_DATA.memory_use.create(unit, hash_table_index, &btmp_init_info));
    ...
    UTILEX_HASH_TABLE_PTR hash_table)
{
    ...
    SHR_IF_ERR_EXIT(UTILEX_HASH_TABLE_ACCESS_DATA.memory_use.destroy(unit, hash_table_index));
    ...
    int with_deinit)
{
    ...
    for (hash_table_index = 0; hash_table_index < max_nof_hashs; hash_table_index++)
    {
        ...
        {
            /*
             * 'hash_table_index' is now the index of an occupied hash table.
             */
            UTILEX_HASH_TABLE_CONVERT_HASHTABLE_INDEX_TO_HANDLE(hash_table, hash_table_index);
    ...
    if (with_deinit)
    {
        SHR_IF_ERR_EXIT(UTILEX_HASH_TABLE_ACCESS.occupied_hashs.free(unit));
        SHR_IF_ERR_EXIT(UTILEX_HASH_TABLE_ACCESS.hashs_array.ptr_free(unit));
        /*
    ...
    uint8 *success)
{
    ...
    SHR_IF_ERR_EXIT(UTILEX_HASH_TABLE_ACCESS_DATA.
                    memory_use.is_occupied(unit, hash_table_index, data_indx, &indx_in_use));
    ...
    UTILEX_HASH_TABLE_KEY * const key)
{
     ...
     */
    SHR_IF_ERR_EXIT(UTILEX_HASH_TABLE_ACCESS_DATA.memory_use.status_set(unit, hash_table_index, entry_offset, FALSE));
    ...
    uint32 data_indx)
{
     ...
     */
    SHR_IF_ERR_EXIT(UTILEX_HASH_TABLE_ACCESS_DATA.memory_use.is_occupied(unit, hash_table_index, data_indx, &in_use));
    if (!in_use)
    {
        /*
    ...
    if (*ptr_long == null_ptr)
    {
        if (get_first_empty)
        {
            if (alloc_by_index)
            {
                 ...
                 */
                SHR_IF_ERR_EXIT(UTILEX_HASH_TABLE_ACCESS_DATA.
                                memory_use.status_set(unit, hash_table_index, *entry, TRUE));
            ...
            {
                SHR_IF_ERR_EXIT(UTILEX_HASH_TABLE_ACCESS_DATA.
                                memory_use.alloc_next(unit, hash_table_index, ptr_long, &found_new));
            }
            if (!found_new)
            {
    ...
    *found = FALSE;
    if (get_first_empty)
    {
        if (alloc_by_index)
        {
             ...
             * don't check if this index already in use 
             */
            SHR_IF_ERR_EXIT(UTILEX_HASH_TABLE_ACCESS_DATA.memory_use.status_set(unit, hash_table_index, *entry, TRUE));
            found_new = TRUE;
            next_node = *entry;
        }
        else
        {
            SHR_IF_ERR_EXIT(UTILEX_HASH_TABLE_ACCESS_DATA.
                            memory_use.alloc_next(unit, hash_table_index, &next_node, &found_new));
        }
        if (!found_new)
        {
            *entry = UTILEX_HASH_TABLE_NULL;
    ...
    uint8 *found)
{
     ...
     */
    SHR_IF_ERR_EXIT(UTILEX_HASH_TABLE_ACCESS_DATA.memory_use.is_occupied(unit, hash_table_index, data_indx, found));
    /*
     * check if the entry 'entry_offset' is in use.
     */
    if (!*found)
    ...
    UTILEX_HASH_TABLE_PTR hash_table)
{
     ...
     * initialize the data to be mapped to 
     */
    SHR_IF_ERR_EXIT(UTILEX_HASH_TABLE_ACCESS_DATA.memory_use.clear(unit, hash_table_index));
exit:
    SHR_FUNC_EXIT;
}
    ...
    uint32 *data_indx)
{
    ...
    {
        SHR_IF_ERR_EXIT(UTILEX_HASH_TABLE_ACCESS_DATA.memory_use.is_occupied(unit, hash_table_index, indx, &occupied));
        if (occupied)
        {
            *data_indx = indx;
            *iter = indx + 1;
    ...
    int unit)
{
...

    {
        ...
        SHR_IF_ERR_EXIT(utilex_hash_table_destroy_all(unit, FALSE));
    }
exit:
    if (SHR_FUNC_ERR())
    {
 ...
 */
shr_error_e
utilex_hash_table_dummy_empty_function_to_make_compiler_happy(
    int unit)
{
    SHR_FUNC_INIT_VARS(unit);
    SHR_FUNC_EXIT;
