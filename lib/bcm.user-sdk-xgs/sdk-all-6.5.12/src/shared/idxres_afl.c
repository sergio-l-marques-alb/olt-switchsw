/*
 * $Id: idxres_afl.c,v 1.13 Broadcom SDK $
 ...
 *
 *  Set _SHR_AIDXRES_SELF_LOCKING to TRUE to impose a lock on created lists.
 ...
 *
 *  Set _SHR_AIDXRES_SELF_LOCKING to FALSE to not do this. Not doing it will
 *  avoid setting up a lock, and skip the overhead of tracking the lock,
 *  reducing resource usage marginally.
 */
#define _SHR_AIDXRES_SELF_LOCKING FALSE
...
#if defined(BCM_ESW_SUPPORT) && (defined(BCM_KATANA_SUPPORT) || \
    defined(BCM_TRIUMPH3_SUPPORT) || defined(BCM_GREYHOUND2_SUPPORT))
#define AIDXLIST_BASE 16
#else /* defined(BCM_ESW_SUPPORT) && (defined(BCM_KATANA_SUPPORT) || \
         defined(BCM_TRIUMPH3_SUPPORT) || defined(BCM_GREYHOUND2_SUPPORT)) */
#define AIDXLIST_BASE 8
#endif /* defined(BCM_ESW_SUPPORT) && (defined(BCM_KATANA_SUPPORT) || \
          defined(BCM_TRIUMPH3_SUPPORT) || defined(BCM_GREYHOUND2_SUPPORT)) */

#endif
#undef AIDXRES_BITS_ENTRY
#if (32 == AIDXLIST_BASE)
typedef uint32 _aidxres_list_entry_t;           /* an element in a list bank */
...
#endif
#if (16 == AIDXLIST_BASE)
typedef uint16 _aidxres_list_entry_t;           /* an element in a list bank */
...
#endif
#if (8 == AIDXLIST_BASE)
typedef uint8 _aidxres_list_entry_t;            /* an element in a list bank */
 ...
 */
typedef struct _aidxres_list_elemdesc_s {
 ...
 */
typedef struct _aidxres_list_bank_sublist_s {
 ...
 */
typedef struct _aidxres_list_bank_s {
 ...
 */
typedef struct _aidxres_list_sublist_s {
 ...
 */
typedef struct _shr_aidxres_list_s {
#if _SHR_AIDXRES_SELF_LOCKING
    sal_mutex_t                     lock;           /* lock for this list */
#endif /* _SHR_AIDXRES_SELF_LOCKING */
 ...
 *  _aidxres_sanity_settings, defined below (and also see the defines in the
 *  header file for _AIDXRES_SANITY_POINT_* and _AIDXRES_SANITY_FUNC_*) will
 ...
 *  the penalty of a test+branch.  _AIDXRES_DEFAULT_SANITY here sets the
 *  default value of _aidxres_sanity_settings, which can be manipulated by
 *  other code, such as the example 'afl paranoia' command on SBX devices.
 */
#undef AIDXRES_SANITY_CHECKING
#if 0
#define _AIDXRES_DEFAULT_SANITY \
    (_AIDXRES_SANITY_POINT_ENTRY | \
     _AIDXRES_SANITY_POINT_RETURN | \
     _AIDXRES_SANITY_FUNC_ALLOC | \
     _AIDXRES_SANITY_FUNC_FREE | \
     _AIDXRES_SANITY_FUNC_CREATE | \
     _AIDXRES_SANITY_DUMP_FAULTS)
#else
#define _AIDXRES_DEFAULT_SANITY 0
#endif
...
#define AIDXRES_DUMP_LIST(list) _aidxres_dump_list(list)
...
static void
_aidxres_dump_list(const shr_aidxres_list_handle_t list){
...
/* NOT static */ uint32 _aidxres_sanity_settings = _AIDXRES_DEFAULT_SANITY;
...
_aidxres_list_check(const shr_aidxres_list_handle_t list)
{
    ...
    if ((BCM_E_NONE != result) &&
        (_aidxres_sanity_settings & _AIDXRES_SANITY_DUMP_FAULTS)) {
        LOG_CLI((BSL_META("aidxres %p appears corrupt; dumping it\n"), (void*)list));
        _aidxres_dump_list(list);
    }
    return result;
}
...
/*
 *  Function
 *      _add_block_to_sublist
 *  Purpose
 *      Prepare a block and insert it into the specified sublist, then be sure
 *      the bank is also in the sublist.
 *  Parameters
 *      (in) shr_aidxres_list_handle_t list = list to manipulate
 *      (in) shr_aidxres_element_t first = first element of block
 *      (in) shr_aidxres_element_t sublist = sublist for insertion
 *  Returns
 *      (void)
 *  Notes
...
/*
 *  Function
 *      _remove_first_from_sublist
 *  Purpose
 *      Remove the first block from a sublist in preparation for allocate.
 *  Parameters
 *      (in) shr_aidxres_list_handle_t list = list to manipulate
 *      (in) shr_aidxres_element_t sublist = sublist to manipulate
 *      (out) shr_aidxres_element_t *element = place to put grabbed first elem
 *  Returns
 *      (void)
 *  Notes
 *      No locking or parameter checking is performed.  This is used internally
...
/*
 *  Function
 *      _remove_block_from_sublist
 *  Purpose
 *      Remove a specific block from a sublist during defragmentation
 *  Parameters
 *      (in) shr_aidxres_list_handle_t list = list to manipulate
 *      (in) shr_aidxres_element_t first = first element of block
 *      (in) shr_aidxres_element_t sublist = sublist for insertion
 ...
 *
 *  Function
 *      _prep_block_for_alloc
 *  Purpose
 *      Prepare a block to be returned to the caller after successful alloc.
 *  Parameters
 *      (in) shr_aidxres_list_handle_t list = list to manipulate
 *      (in) shr_aidxres_element_t first = first element of block
...
/*
 *  Function
 *      _shr_aidxres_list_alloc
 *  Purpose
 *      Allocate the next available block of appropriate size from a list
 *  Parameters
 *      (in) shr_aidxres_list_handle_t list = list from which to allocate
 *      (in) shr_aidxres_element_t count = number of elements in block
 *      (out) shr_aidxres_element_t *element = where to put alloced block base
 *  Returns
...
#ifdef AIDXRES_SANITY_CHECKING
    if ((_aidxres_sanity_settings & (_AIDXRES_SANITY_POINT_ENTRY |
                                     _AIDXRES_SANITY_FUNC_ALLOC)) ==
        (_AIDXRES_SANITY_POINT_ENTRY | _AIDXRES_SANITY_FUNC_ALLOC)) {
...
#ifdef AIDXRES_SANITY_CHECKING
    if ((_aidxres_sanity_settings & (_AIDXRES_SANITY_POINT_RETURN |
                                     _AIDXRES_SANITY_FUNC_ALLOC)) ==
        (_AIDXRES_SANITY_POINT_RETURN | _AIDXRES_SANITY_FUNC_ALLOC)) {
        auxRes = _aidxres_list_check(list);
        if (BCM_E_NONE != auxRes) {
            return auxRes;
        }
...
/*
 *  Function
 *      _shr_aidxres_list_free
 *  Purpose
 *      Free a block back to a list
 *  Parameters
 *      (in) shr_aidxres_list_handle_t list = list from which block was alloced
 *      (in) shr_aidxres_element_t = first element in block to free
 *  Returns
                       ...
                       shr_aidxres_element_t element)
{
    ...
    if ((_aidxres_sanity_settings & (_AIDXRES_SANITY_POINT_ENTRY |
                                     _AIDXRES_SANITY_FUNC_FREE)) ==
        (_AIDXRES_SANITY_POINT_ENTRY | _AIDXRES_SANITY_FUNC_FREE)) {
    ...
    if ((_aidxres_sanity_settings & (_AIDXRES_SANITY_POINT_RETURN |
                                     _AIDXRES_SANITY_FUNC_FREE)) ==
        (_AIDXRES_SANITY_POINT_RETURN | _AIDXRES_SANITY_FUNC_FREE)) {
        result = _aidxres_list_check(list);
        if (BCM_E_NONE != result) {
            return result;
        }
                        ...
                        char *name)
{
    ...
    if ((_aidxres_sanity_settings & (_AIDXRES_SANITY_POINT_RETURN |
                                     _AIDXRES_SANITY_FUNC_CREATE)) ==
        (_AIDXRES_SANITY_POINT_RETURN | _AIDXRES_SANITY_FUNC_CREATE)) {
 ...
 *      (out) shr_aidxres_element_t *element = where to put alloced elem num
 *   Returns
 *      int = BCM_E_NONE if element allocated successfully
 *                    BCM_E_* as appropriate otherwise
 *   Notes
...
shr_aidxres_list_reserve(shr_aidxres_list_handle_t list,
                         ...
                         shr_aidxres_element_t last)
{
    bcm_error_t           result;     /* value to be returned to caller */
                               ...
                               shr_aidxres_element_t count)
{
    ...
    if ((!list) ||
        (first < list->valid_low) ||
        (last > list->valid_high) ||
        (count > ((uint32)1 << (list->sublist_count - 1)))) {
        return BCM_E_PARAM;
    }
