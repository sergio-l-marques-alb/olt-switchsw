...
#endif
            th_alpm_table_ent_dump(unit, mem, entry, copyno, k, fmt,
                                   -1, flags, fmt_pfx[fmt]);
        }
#endif
#endif
...
cmd_esw_dump(int unit, args_t *a)
{
    ...
    if (SOC_IS_TRIDENT3(unit)) {
        if (flags  & DUMP_TABLE_FORMAT) {
           ...
           buf = ARG_GET(a);
           if (buf == NULL) {
                cli_out ("Enter the value for format.\n");
                rv = CMD_FAIL;
                goto done;
           }
           tmp = buf;
           buf_count = 0;
           buf_len = sal_strlen(buf);

           do {
               format_val[i] = sal_ctoi(tmp, 0);
               tmp = tmp + sizeof(uint32);
               buf_count += BYTES2BITS(sizeof(uint32));
           } while ((tmp != '\0') &&
                   ( i < SOC_MAX_MEM_WORDS) &&
                   (buf_count <buf_len));
