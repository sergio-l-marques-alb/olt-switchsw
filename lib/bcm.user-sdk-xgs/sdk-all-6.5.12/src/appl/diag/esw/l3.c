...
_ipmc_cmd_entry_print(int unit, bcm_ipmc_addr_t *info, void *cookie)
{
    ...
    if (info->flags & BCM_IPMC_IP6) {
        ...
        cli_out("%s     %s  %s  %s\n", "Group", "VRF", "CLASS", "HIT");
        cli_out("%3d %4d %5d %2d %3d %3d   0x%x %2d,%5d,%4s\n",
...
_l3_cmd_egress_obj_print(int unit, int index, bcm_l3_egress_t *info, void *cookie)
{
    ...
    if (soc_feature(unit, soc_feature_ecmp_dlb_optimized)) {
        ...
        for (i = 0; i < soc_mem_index_count(unit, ECMP_DLB_CONTROLm); i++) {
            rv = READ_ECMP_DLB_CONTROLm(unit, MEM_BLOCK_ANY, i,
                                        &ecmp_group_entry);
            if (BCM_FAILURE(rv)) {
...
/* Dumps perf stats */
void
_l3_route_perf_data_print(_l3_route_perf_data_t *perf, const char *label)
{
    if( perf->count > 0) {
        cli_out("Performance Dump: %s :\n", label);
        cli_out("  Min: %d uSec\n", perf->min);
        cli_out("  Max: %d uSec\n", perf->max);
        cli_out("  Avg: %d uSec\n", (perf->total / perf->count));
        cli_out("  Total: %d.%3d Sec\n", (perf->total / 1000000), (perf->total%1000000/1000));
        cli_out("  Count: %d\n", perf->count);
    } else {
        cli_out("Performance Dump: %s : NO DATA!\n", label);
    }
}
...
STATIC void
_l3_route_perf_v4_op_test(int unit, int op, _l3_route_perf_data_t *route_perf)
{
    int         rv;
    int         inx, idx;
    ...
    for (inx = 0; inx < loop; inx++) {
        base_route.l3a_vrf = _l3_route_perf_vrf;
...
STATIC void
_l3_route_perf_v4_test(int unit)
{
    int run;
    bcm_l3_route_t base_route;

    _l3_route_perf_data_t route_add_perf;
    _l3_route_perf_data_t route_get_perf;
    _l3_route_perf_data_t route_del_perf;
    _l3_route_perf_data_t route_upd_perf;

    _l3_route_perf_data_init(&route_add_perf);
    _l3_route_perf_data_init(&route_get_perf);
    _l3_route_perf_data_init(&route_del_perf);
    _l3_route_perf_data_init(&route_upd_perf);
    ...
    for (run = 0; run < 10; run ++) {
        ...
        cli_out("\nRUN # %d\n", run);
        cli_out("Insert default IPv4 route\n");
        bcm_l3_route_t_init(&base_route);
        base_route.l3a_intf = _l3_route_perf_obj_id[unit];
        base_route.l3a_vrf = _l3_route_perf_vrf;
        ...
        /* Insert entry */
        _l3_route_perf_v4_op_test(unit, _L3_ROUTE_PERF_OP_ADD, &route_add_perf);
        /* Update entry */
        _l3_route_perf_v4_op_test(unit, _L3_ROUTE_PERF_OP_UPD, &route_upd_perf);
        /* Lookup entry */
        _l3_route_perf_v4_op_test(unit, _L3_ROUTE_PERF_OP_GET, &route_get_perf);
        /* Delete entry */
        _l3_route_perf_v4_op_test(unit, _L3_ROUTE_PERF_OP_DEL, &route_del_perf);

        _l3_route_perf_data_print(&route_add_perf, "Insert 8K L3_IPV4_UC_ROUTE routes");
        _l3_route_perf_data_print(&route_upd_perf, "Update 8K L3_IPV4_UC_ROUTE routes");
        _l3_route_perf_data_print(&route_get_perf, "Lookup 8K L3_IPV4_UC_ROUTE routes");
        _l3_route_perf_data_print(&route_del_perf, "Delete 8K L3_IPV4_UC_ROUTE routes");
...
STATIC void
_l3_route_perf_v6_op_test(int unit, int op, _l3_route_perf_data_t *route_perf)
{
    int         i, rv;
    int         start, end, duration;
    bcm_l3_route_t base_route;
    ...
    /* Insert entry */
    cli_out("%s %d IPv6 Routes\n", _l3_route_perf_op_str[op], count);
    start = sal_time_usecs();
...
STATIC void
_l3_route_perf_v6_test(int unit)
{
    int run;
    bcm_l3_route_t base_route;
    ...
    for (run = 0; run < 10; run ++) {
        ...
        cli_out("Insert default IPv6 route\n");
        bcm_l3_route_t_init(&base_route);
        base_route.l3a_intf = _l3_route_perf_obj_id[unit];
        base_route.l3a_vrf = _l3_route_perf_vrf;
        base_route.l3a_flags = BCM_L3_IP6;
        ...
        /* Insert entry */
        _l3_route_perf_v6_op_test(unit, _L3_ROUTE_PERF_OP_ADD, &route_add_perf);
        /* Update entry */
        _l3_route_perf_v6_op_test(unit, _L3_ROUTE_PERF_OP_UPD, &route_upd_perf);
        /* Lookup entry */
        _l3_route_perf_v6_op_test(unit, _L3_ROUTE_PERF_OP_GET, &route_get_perf);
        /* Delete entry */
        _l3_route_perf_v6_op_test(unit, _L3_ROUTE_PERF_OP_DEL, &route_del_perf);

        _l3_route_perf_data_print(&route_add_perf, "L3_IPV6_UC_ROUTE_VRF ADD");
        _l3_route_perf_data_print(&route_upd_perf, "L3_IPV6_UC_ROUTE_VRF UPDATE");
        _l3_route_perf_data_print(&route_get_perf, "L3_IPV6_UC_ROUTE_VRF GET");
        _l3_route_perf_data_print(&route_del_perf, "L3_IPV6_UC_ROUTE_VRF DELETE");
...
{
    _l3_route_perf_v4_test(unit);
    _l3_route_perf_v6_test(unit);
...
_l3_cmd_alpm_debug_show(int unit, args_t *a)
{
    ...
    if (c != NULL && sal_strcasecmp(c, "brief") == 0) {
        ...
        if (soc_feature(unit, soc_feature_alpm2)) {
            rv = bcm_esw_alpm_debug_brief_show(unit, bucket, index);
        } else
#endif
    ...
    if (soc_feature(unit, soc_feature_alpm2)) {
        rv = bcm_esw_alpm_debug_show(unit, vrf, flags);
...
#if defined(BCM_TOMAHAWK_SUPPORT)
    if (soc_feature(unit, soc_feature_alpm2)) {
        rv = bcm_esw_alpm_sanity_check(unit, mem, index, check_sw);
    } else
#endif
#if defined(BCM_TOMAHAWK_SUPPORT) || defined(BCM_APACHE_SUPPORT) || defined(BCM_TRIDENT3_SUPPORT)
...
char cmd_esw_alpm_usage[] =
    ...
    "alpm trace sanity <freq_cnt>\n\t"
    "alpm trace wrap <enable|disable>\n\t"
    "alpm trace dump [showflags]\n\t"
    "alpm table dump [raw|chg|all|nocache] <mem> <index> <fmt> [<ent>]\n\t"
...
cmd_esw_alpm(int unit, args_t *a)
{
    ...
    int   subcmd = 0;
    int   acb_bmp = 3;
    static sal_usecs_t usec[SOC_MAX_NUM_DEVICES] = {0};
    int   flags = 0;
    int   index = 0;
    int   fmt = 1;
    int   ent_idx = 0;
    ...
    if (sal_strcasecmp(table, "hitbit") == 0) {
        ...
        if (sal_strcasecmp(subcmd_s, "disable") == 0 ||
            ...
            if (SAL_BOOT_BCMSIM || SAL_BOOT_QUICKTURN) {
                ...
                itv = ALPM_HIT_INTERVAL_DEF;
            }
            usec[unit] = itv;
            alpm_dist_hitbit_enable_set(unit, usec[unit]);
        } else {
            return CMD_USAGE;
        }
        return CMD_OK;
    ...
    if (sal_strcasecmp(table, "trace") == 0) {
        ...
        if (sal_strcasecmp(subcmd_s, "enable") == 0) {
            (void)alpm_trace_set(unit, TRUE);
        } else if (sal_strcasecmp(subcmd_s, "disable") == 0) {
            (void)alpm_trace_set(unit, FALSE);
        } else if (sal_strcasecmp(subcmd_s, "clear") == 0) {
            (void)alpm_trace_clear(unit);
        } else if (sal_strcasecmp(subcmd_s, "sanity") == 0) {
            if ((subcmd_s = ARG_GET(a)) != NULL && isint(subcmd_s)) {
                index = parse_integer(subcmd_s); /* freq_cnt */
            } else {
                index = -1; /* for show trace sanity */
            }
            (void)alpm_trace_sanity(unit, index);
        } else if (sal_strcasecmp(subcmd_s, "dump") == 0) {
            if ((subcmd_s = ARG_GET(a)) == NULL) {
                (void)alpm_trace_dump(unit, 0);
            } else if (sal_strcasecmp(subcmd_s, "showflags") == 0) {
                (void)alpm_trace_dump(unit, 1);
            } else {
                return CMD_USAGE;
            }
            ...
            if (sal_strcasecmp(subcmd_s, "enable") == 0) {
                (void)alpm_trace_wrap_set(unit, TRUE);
            } else if (sal_strcasecmp(subcmd_s, "disable") == 0) {
                (void)alpm_trace_wrap_set(unit, FALSE);
            } else {
                return CMD_USAGE;
            }
        } else {
    ...
    if (!sal_strcasecmp(table, "table")) {
        ...
        if (!sal_strcasecmp(subcmd_s, "dump")) {
            ...
            if (soc_feature(unit, soc_feature_alpm2)) {
                (void)alpm_table_dump(unit, mem, copyno, index, fmt, ent_idx, flags);
            } else {
...
#define IP6_STR_CNT 6
    ...
    if (sal_strcasecmp(table, "pvt") == 0) {
        ...
        switch (subcmd) {
            ...
            case L3_SHOW:
                ...
                alpm_cb_pvt_dump(unit, acb_bmp, bkt);
                return CMD_OK;
            default:
                return (CMD_USAGE);
        }
    ...
    if (sal_strcasecmp(table, "pvtlen") == 0) {
        switch (subcmd) {
            /* alpm pvtlen show */
            case L3_SHOW:
                alpm_cb_pvtlen_dump(unit, acb_bmp);
                return CMD_OK;
            default:
                return (CMD_USAGE);
        }
    ...
    if (sal_strcasecmp(table, "stat") == 0) {
        switch (subcmd) {
            /* alpm stat show */
            case L3_SHOW:
                alpm_cb_stat_dump(unit, acb_bmp);
                return CMD_OK;
            default:
                return (CMD_USAGE);
        }
    ...
    if (sal_strcasecmp(table, "config") == 0) {
        switch (subcmd) {
            /* alpm config show */
            case L3_SHOW:
                alpm_cb_conf_dump(unit, acb_bmp);
                return CMD_OK;
            default:
                return (CMD_USAGE);
        }
