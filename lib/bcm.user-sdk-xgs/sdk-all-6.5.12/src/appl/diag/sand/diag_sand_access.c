...
static sh_sand_option_t access_read_options[] = {
    ...
    {"changed", SAL_FIELD_TYPE_BOOL, "Print only when value is differen from reset one", "No"},
    ...
    {"overwrite", SAL_FIELD_TYPE_BOOL, "flag used to overwrite auxilary no read", "No"},
...
static sh_sand_invoke_t access_read_invokes[] = {
    {"vtt_firts_entry", "name=vtt count=1", 0},
    {NULL}
};
    ...
    for (reg = 0; reg < NUM_SOC_REG; reg++)
    {
        ...
        if (separate_flag == TRUE && print_flag == TRUE)
        {
            ...
            if (sal_strcmp(folder_name, ""))
            {
                ...
                sal_strcat(help_filename, folder_name);
                sal_strcat(help_filename, SOC_REG_NAME(unit, reg));
                sal_strcat(help_filename, ".xml");
            }
        }
        ...
        SOC_MEM_BLOCK_ITER(unit, mem, i_bl)
        {
            ...
            if (separate_flag == TRUE && print_flag == TRUE)
            {
                ...
                if (sal_strcmp(folder_name, ""))
                {
                    sal_memset(help_filename, 0, sizeof(char) * 512);
                    sal_strcat(help_filename, folder_name);
                    sal_strcat(help_filename, SOC_MEM_NAME(unit, mem));
                    sal_strcat(help_filename, ".xml");
                }
            }
...
static sh_sand_man_t access_write_man = {
    "Set registers/memories value.",
    "Command allows to set a value to a register or memory field.\n"
        "Fields not specified in the list are left unaffected.\n",
    "ACCess write [reg|mem] <REG/MEM Name> [block=<COPY>] [index=<INDEX>] [entry=<ENTRY>] [count=<ENTRYCOUNT>] [value=<FIELD>(<FIELDVALUE>),...| <DW0> .. <DWN>]\n",
    "acc write mem TCAM_TCAM_BANK_COMMAND block=TCAM0 entry=0 count=1 value=TCAM_CPU_CMD_WR(3),TCAM_CPU_CMD_VALID(3),TCAM_CPU_CMD_RD(0),TCAM_CPU_CMD_CMP(0)\n"
        "acc write mem IPPC_PMF_KBR_PASS_1 entry=0 count=1 value=\"0x0 0x0 0x0 0x0 0x1 0x0 0x0 0x0 0x0 0x0 0x0 0x5 0x0 0x0 0x0 0x0 0x8 0x0 0x0 0x11 0x3 0x0\"\n"
        "acc write reg TCAM_TCAM_BANK_BLOCK_OWNER value=0\n",
};

static sh_sand_option_t access_write_options[] = {
    {"variable", SAL_FIELD_TYPE_STR, "Name of the reg/mem to be set.", ""},
    ...
    {"offset", SAL_FIELD_TYPE_STR, "Offeset.", ""},
...
static sh_sand_invoke_t access_write_invokes[] = {
    {"TCAM_TCAM_BANK_BLOCK_OWNER", "reg TCAM_TCAM_BANK_BLOCK_OWNER value=0", 0},
    {NULL}
};
...
static sh_sand_man_t access_modify_man = {
    "Modify registers/memories field value.",
    "Command allows to modify register/memory field(s) value(s).\n"
        "Fields not specified in the list are left unaffected.\n",
    "ACCess Modify [reg|mem] <REG/MEM Name> [block=<COPY>] [index=<INDEX>] [entry=<ENTRY>] [count=<ENTRYCOUNT>] [value=<FIELD>(<FIELDVALUE>),...]",
    "acc mod mem IPPC_PMF_KBR_PASS_1 entry=0 count=1 value=KEY_A_APP_DB(0x0)\n",
};
...
static sh_sand_invoke_t access_modify_invokes[] = {
    {"IPPC_PMF_KBR_PASS_1", "mem IPPC_PMF_KBR_PASS_1 entry=0 count=1 value=KEY_A_APP_DB(0x0)", 0},
    {NULL}
};
...
static sh_sand_man_t access_hit_man = {
    "Obtain hit entries from adapter server and read the content",
    "Command allows to display or to save in XML file value of registers&memories hitted in cmodle processing",
    "ACCess hit [name=<regular expr>] [block=<substring>] [field] [all] [property=<regular expr>]",
    "acc hit name=RIP block=SCH\n" "acc hit property=mem.*cachable block=PPDB_B0\n",
};
...
static sh_sand_option_t access_hit_options[] = {
    ...
    {"dbal", SAL_FIELD_TYPE_BOOL, "Ignore filters and print all hit regs/mems, exampel for DBAL usage", "No"},
    {"field", SAL_FIELD_TYPE_BOOL, "Include fields in regs/mems search and output", "No"},
    ...
    {"clear", SAL_FIELD_TYPE_BOOL, "Flag used to tells to delete the hit bit indication record.", "No"},
    {NULL}
};
            ...
            for (reg = 0; reg < NUM_SOC_REG; reg++)
            {
                ...
                switch (reginfo->regtype)
                {
                    ...
                    default:
                        ...
                        if (read_flag)
                        {
                            access_reg_read(unit, reg, i_bl, 0, offset, reg_value, &address);
                        }
                        else
                        {
                            uint8 acc_type;
                            ...
                            SOC_REG_ABOVE_64_CLEAR(reg_value);
                            address =
                                soc_reg_addr_get(unit, reg, SOC_BLOCK_PORT(unit, i_bl), offset,
                                                 SOC_REG_ADDR_OPTION_NONE, &blk, &acc_type);
                        }
                        access_reg_print(prt_ctr, unit, reg, SOC_BLOCK_NAME(unit, i_bl), offset, address, reg_value,
                                         field_flag, changed, fld_match_list);
                        break;
                }
...
sh_sand_cmd_t sh_sand_access_cmds[] = {
    ...
    ,
    {"list", access_list_cmd, NULL, access_list_options, &access_list_man, NULL, sh_sand_all_invokes}
    ,
    {"read", access_read_cmd, NULL, access_read_options, &access_read_man, NULL, access_read_invokes}
    ,
#ifdef ADAPTER_SERVER_MODE
    ...
    ,
    {"write", access_write_cmd, NULL, access_write_options, &access_write_man, NULL, access_write_invokes}
    ,
    {"modify", access_modify_cmd, NULL, access_modify_options, &access_modify_man, NULL, access_modify_invokes}
    ,
    {NULL}
};
...
{
    cmd_result_t result;
    SH_SAND_VERIFY(sh_sand_access_cmds, result);

    sh_sand_act(unit, args, sh_sand_access_cmds, sh_sand_sys_cmds);
    ARG_DISCARD(args);
    /*
