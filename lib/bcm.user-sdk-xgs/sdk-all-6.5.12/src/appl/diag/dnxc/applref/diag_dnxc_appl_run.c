 ...
 */
shr_error_e
static sh_dnxc_appl_run_name_cmd(
    int unit,
    char *appl_name,
    int  is_deinit)

{
    ...
    if(sal_strcasecmp(appl_name, "all") == 0) /** run full init or full deinit depending on "deinit" argument */
    {
        ...
        else /** if (is_deinit == FALSE), run full init sequence*/
        {
            appl_params.no_deinit = 1;
            appl_params.no_init = 0;
        }
        SHR_IF_ERR_EXIT(appl_dnxc_init_step_list_run(unit, NULL, &appl_params));
    }
    else /** run a specific application init or deinit*/
    {
 ...
 */
shr_error_e
static sh_dnxc_appl_run_id_cmd(
    int unit,
    int appl_id,
    int is_deinit)
    ...
    sh_sand_control_t * sand_control)
{
    ...
    /* 
     *  Run applications according to provided arguements.
    ...
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Both application ID and name can't be given together at the same time\n");
    }
...
sh_sand_option_t sh_dnxc_appl_run_options[] = {
    ...
    {"id",        SAL_FIELD_TYPE_INT32, "Invoke application by id",      "-1",        NULL},
    {"name",      SAL_FIELD_TYPE_STR,   "Invoke application by name",    "",          NULL},
    {"deinit",    SAL_FIELD_TYPE_BOOL,  "Whether to run init or deinit", "false",     NULL},
    {NULL}
};

sh_sand_man_t sh_dnxc_appl_run_man = {
    "Diagnostic pack for running init/deinit of reference applications by name or by id",
    "Diagnostic pack for running init/deinit of reference applications by name or by id"
    "To obtain a list of application names and its IDs use 'dnx application list'",
    "dnx application run [name=<application_name|'cosq'|'l2'|'sysports'|'all'>]"
    "[id=<application_id|1|8|>] [init]",
    "* 'dnx application run' - run full deinit and init sequence\n"
