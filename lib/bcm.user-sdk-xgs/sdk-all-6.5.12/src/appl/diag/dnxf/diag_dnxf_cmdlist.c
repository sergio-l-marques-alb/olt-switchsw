/** \file diag_dnxf_cmdlist.c
 *
 * Purpose: List of commands specific for SOC_IS_DNXF devices
 */

#include <appl/diag/shell.h>
#include <appl/diag/cmdlist.h>
#include <appl/diag/sand/diag_sand_framework.h>
#include <appl/diag/sand/diag_sand_access.h>
#include <appl/diag/dnxc/diag_dnxc_diag.h>

#ifdef INCLUDE_CTEST
#include <appl/ctest/dnxf/ctest_dnxf_root.h>
#endif

#include <soc/dnxf/swstate/auto_generated/diagnostic/swstate_commandline.h>

#include "diag_dnxf_cmdlist.h"

#ifdef INCLUDE_AUTOCOMPLETE
#include <sal/appl/editline/autocomplete.h>
#endif

#define BSL_LOG_MODULE BSL_LS_APPL_SHELL

extern sh_sand_man_t sh_dnxc_appl_man;
extern sh_sand_cmd_t sh_dnxc_appl_cmds[];
extern sh_sand_man_t sh_dnxc_clear_man;
extern sh_sand_cmd_t sh_dnxc_clear_cmds[];
extern sh_sand_man_t sh_dnxc_data_man;
extern sh_sand_cmd_t sh_dnxc_data_cmds[];
extern sh_sand_man_t sh_dnxc_fabric_man;
extern sh_sand_cmd_t sh_dnxc_fabric_cmds[];
extern sh_sand_man_t sh_dnxc_show_man;
extern sh_sand_cmd_t sh_dnxc_show_cmds[];
extern sh_sand_man_t sh_dnxc_diag_man;
extern sh_sand_cmd_t sh_dnxc_diag_cmds[];
extern sh_sand_man_t sh_dnxc_diag_avs_man;
extern sh_sand_cmd_t sh_dnxc_avs_cmds[];
extern sh_sand_cmd_t sh_dnxc_linkscan_cmds[];
extern sh_sand_man_t sh_dnxc_linkscan_man;
extern sh_sand_man_t sh_dnxc_intr_man;
extern sh_sand_cmd_t sh_dnxc_intr_cmds[];

sh_sand_cmd_t sh_dnxf_commands[] = {
    {"access", NULL, sh_sand_access_cmds, NULL, &sh_sand_access_man},
    {"application", NULL, sh_dnxc_appl_cmds, NULL, &sh_dnxc_appl_man},
    {"avs", NULL, sh_dnxc_avs_cmds, NULL, &sh_dnxc_diag_avs_man},
    {"clear", NULL, sh_dnxc_clear_cmds, NULL, &sh_dnxc_clear_man},
    {"data", NULL, sh_dnxc_data_cmds, NULL, &sh_dnxc_data_man},
/** diag is for backward compatibility commands only, no new functionality should use it */
    {"diag", NULL, sh_dnxc_diag_cmds, NULL, &sh_dnxc_diag_man},
    {"fabric", NULL, sh_dnxc_fabric_cmds, NULL, &sh_dnxc_fabric_man},
    {"show", NULL, sh_dnxc_show_cmds, NULL, &sh_dnxc_show_man},
    {"linkscan", NULL, sh_dnxc_linkscan_cmds, NULL, &sh_dnxc_linkscan_man},
    {"interrupt",   NULL, sh_dnxc_intr_cmds,    NULL, &sh_dnxc_intr_man},
    {NULL}      /* This line should always stay as last one */
};

cmd_result_t
cmd_dnxf_invoke(
    int unit,
    args_t * args)
{
    /*
     * Take args back to first command 
     */
    ARG_PREV(args);
    sh_sand_act(unit, args, sh_dnxf_commands, sh_sand_sys_cmds);
    ARG_DISCARD(args);
    /*
     * Always return OK - we provide all help & usage from inside framework
     */
    return CMD_OK;
}

/**
 * \brief Routine allowing to treat all new commands in unified way.
 * All commands may be invoked through dnx - e.g. "dnx access list..."
 */
cmd_result_t
cmd_dnxf_all(
    int unit,
    args_t * args)
{
    cmd_result_t result;
    /*
     * Start from highest point in the tree
     */
    result = diag_sand_error_get(sh_sand_act(unit, args, sh_dnxf_commands, sh_sand_sys_cmds));
    ARG_DISCARD(args);
    return result;
}

#if defined(DNX_SW_STATE_DIAGNOSTIC)
/**
 * \brief Routine allowing to treat all new commands in unified way.
 * All commands may be invoked through swstate - e.g. "swstate utilex ..."
 */
static cmd_result_t
cmd_dnxf_swstate_all(
    int unit,
    args_t * args)
{
    cmd_result_t result;
    /*
     * Start from highest point in the tree
     */
    result = diag_sand_error_get(sh_sand_act(unit, args, sh_dnxf_swstate_cmds, sh_sand_sys_cmds));
    ARG_DISCARD(args);
    return result;
}
#endif /* DNX_SW_STATE_DIAGNOSTIC */

shr_error_e
cmd_dnxf_sh_init(
    int unit)
{
    char command[SH_SAND_MAX_TOKEN_SIZE];

    SHR_FUNC_INIT_VARS(unit);

    SHR_CLI_EXIT_IF_ERR(sh_sand_cmd_add_to_dyn_list(unit, sh_dnxf_commands, cmd_dnxf_invoke), "");

#ifdef INCLUDE_AUTOCOMPLETE
    sh_sand_cmd_autocomplete_init(unit, NULL, sh_dnxf_commands);
#ifdef DNX_SW_STATE_DIAGNOSTIC
    sh_sand_cmd_autocomplete_init(unit, NULL, (sh_sand_cmd_t[2])
                                  {
                                  {
                                  "swstate", NULL, sh_dnxf_swstate_cmds}
                                  ,
                                  {
                                  NULL}
                                  }
    );
#endif /* DNX_SW_STATE_DIAGNOSTIC */
#endif /* INCLUDE_AUTOCOMPLETE */

    command[0] = 0;
    SHR_CLI_EXIT_IF_ERR(sh_sand_init(unit, sh_dnxf_commands, command, SH_SAND_VERIFY_ALL),
                        "DNXF shell commands verification failed\n");
    command[0] = 0;
    SHR_CLI_EXIT_IF_ERR(sh_sand_init(unit, sh_sand_sys_cmds, command, SH_SAND_VERIFY_ALL),
                        "DNXF shell system command verification failed\n");
    command[0] = 0;
#if defined(DNX_SW_STATE_DIAGNOSTIC)
    SHR_CLI_EXIT_IF_ERR(sh_sand_init(unit, sh_dnxf_swstate_cmds, command, 0),
                        "SWState system command verification failed\n");
#endif

exit:
    SHR_FUNC_EXIT;
}

shr_error_e
cmd_dnxf_sh_deinit(
    int unit)
{
    SHR_FUNC_INIT_VARS(unit);

    SHR_CLI_EXIT_IF_ERR(sh_sand_cmd_remove_from_dyn_list(unit, sh_dnxf_commands, cmd_dnxf_invoke), "");

#ifdef INCLUDE_AUTOCOMPLETE
    autocomplete_delete_all(unit);
#endif
    SHR_CLI_EXIT_IF_ERR(sh_sand_deinit(unit, sh_dnxf_commands), "DNXF shell commands deinit failed\n");
#if defined(DNX_SW_STATE_DIAGNOSTIC)
    SHR_CLI_EXIT_IF_ERR(sh_sand_deinit(unit, sh_dnxf_swstate_cmds), "DNXF shell commands deinit failed\n");
#endif

exit:
    SHR_FUNC_EXIT;
}

const char cmd_dnxf_usage[] =
    "Allows to invoke any combination of commands, get and search in usage for all dnx specific commands\n";

#if defined(DNX_SW_STATE_DIAGNOSTIC)
const char cmd_dnxf_swstate_usage[] = "Root command for swstate tree. Use \"swstate usage\" for details\n";
#endif

cmd_t bcm_dnxf_cmd_list[] = {

    {"Attach", sh_attach, sh_attach_usage, "Attach SOC device(s)"},
#if defined(BCM_DFE_SUPPORT) || defined(BCM_DPP_SUPPORT)
    {"CLEAR", cmd_dpp_clear, cmd_dpp_clear_usage, "Clear a memory table or counters"},
    
#endif /* defined(BCM_DFE_SUPPORT) || defined(BCM_DPP_SUPPORT) */
#if defined(BCM_DFE_SUPPORT)
    {"CounTeR", cmd_dpp_counter, cmd_dpp_counter_usage, "Enable/disable counter collection"},
    
#endif /* BCM_DFE_SUPPORT */
#if defined(__DUNE_GTO_BCM_CPU__) || defined(__DUNE_WRX_BCM_CPU__) || defined(__DUNE_SLK_BCM_CPU__)
    {"cpu_i2c", cmd_sand_cpu_i2c, cmd_sand_cpu_i2c_usage, "Read/Write I2C via cpu"},
    {"cpu_regs", cmd_sand_cpu_regs, cmd_sand_cpu_regs_usage, "Read/Write function to cpu regs"},
#endif
#ifdef INCLUDE_CTEST
    {"CTest", cmd_dnxf_ctest_invoke, cmd_dnxf_ctest_usage, "Run tests"},        /* Infra */
#endif
    {"DEInit", sh_deinit, sh_deinit_usage, "Deinit SW modules"},
    {"DETach", sh_detach, sh_detach_usage, "Detach SOC device(s)"},
    {"DEviceReset", sh_device_reset, sh_device_reset_usage, "Perform different device reset modes/actions."},
    {"DMA", cmd_sand_dma, cmd_sand_dma_usage, "DMA Facilities Interface"},
    {"DNXF", cmd_dnxf_all, cmd_dnxf_usage, "Invocation point for dnxf specific commands"},      /* Infra */
    {"Dump", cmd_sand_mem_get, cmd_sand_mem_get_usage, "Dump an address space or registers"},
    {"Getreg", cmd_sand_reg_get, cmd_sand_reg_get_usage, "Get register"},
#if defined(BCM_DFE_SUPPORT)
    {"GPort", cmd_dpp_gport, cmd_dpp_gport_usage, "Show the current queue gports set up in the system"},
#endif /* BCM_DFE_SUPPORT */
#ifdef INCLUDE_I2C
    {"I2C", cmd_i2c, cmd_i2c_usage, "Inter-Integrated Circuit (I2C) Bus commands"},
#endif
    {"INIT", sh_init, sh_init_usage, "Initialize SOC and S/W"},
#if defined(BCM_DFE_SUPPORT)
    {"init_dnx", cmd_dnxf_init_dnx, appl_dnxf_init_usage, "Initialize/deinitialize DNX S/W"},
#endif /* BCM_DFE_SUPPORT */
    {"LISTmem", cmd_sand_mem_list, cmd_sand_mem_list_usage, "List the entry format for a given table"},
    {"Listreg", cmd_sand_reg_list, cmd_sand_reg_list_usage, "List register fields"},
    {"MODify", cmd_sand_mem_modify, cmd_sand_mem_modify_usage, "Modify table entry by field names"},
    {"Modreg", cmd_sand_reg_modify, cmd_sand_reg_modify_usage, "Register Read/Modify/Write"},
    {"PacketWatcher", pw_command, pw_usage, "Monitor ports for packets"},
#if defined(BCM_DFE_SUPPORT)
    {"PBMP", cmd_dpp_pbmp, cmd_dpp_pbmp_usage, "Convert port bitmap string to hex"},
#endif /* BCM_DFE_SUPPORT */
    {"PCIC", cmd_sand_pcic_access, cmd_sand_pcic_access_usage, "Access to PCI configuration space"},
    {"pcie", cmd_sand_pcie_reg, cmd_sand_pcie_reg_usage, "Read/Write form devices via pcie"},
    {"PCIEPHY", cmd_pciephy, pciephy_usage, "PCIe PHY specific utilities"},
#if defined(BCM_DNXF_SUPPORT)
    {"PHY", if_dnx_phy, if_dnx_phy_usage, "Set/Display phy characteristics"},
    {"PORT", if_dnx_port, if_dnx_port_usage, "Set/Display port characteristics"},
#endif /* BCM_DNXF_SUPPORT */
#ifdef PORTMOD_DIAG
    {"PortMod", cmd_portmod_diag, portmod_diag_usage, "portmod diagnostics"},
#endif
#if defined(BCM_DNXF_SUPPORT)
    {"PortStat", if_dnx_port_stat, if_dnx_port_stat_usage, "Display port status in table"},
#endif /* BCM_DNXF_SUPPORT */
    {"PROBE", sh_probe, sh_probe_usage, "Probe for available SOC units"},
    {"Setreg", cmd_sand_reg_set, cmd_sand_reg_set_usage, "Set register"},
#if defined(BCM_DFE_SUPPORT)
    {"SOC", cmd_dpp_soc, cmd_dpp_soc_usage, "Print internal driver control information"},
#endif /* BCM_DFE_SUPPORT */
    {"STKMode", cmd_stkmode, cmd_stkmode_usage, "Hardware Stacking Mode Control"},
#if defined(BCM_DFE_SUPPORT)
    {"SwitchControl", cmd_sand_switch_control, cmd_sand_switch_control_usage, "General switch control"},
#endif /* BCM_DFE_SUPPORT */
#if defined(DNX_SW_STATE_DIAGNOSTIC)
    {"SWState", cmd_dnxf_swstate_all, cmd_dnxf_swstate_usage, "Invocation point for swstate specific commands"},
#endif
#ifdef INCLUDE_TEST
    {"TestClear", test_clear, test_clear_usage, "Clear run statisistics for a test"},
    {"TestList", test_print_list, test_list_usage, "List loaded tests and status"},
    {"TestMode", test_mode, test_mode_usage, "Set global test run modes"},
    {"TestParameters", test_parameters, test_parameters_usage, "Set test Parameters"},
    {"TestRun", test_run, test_run_usage, "Run a specific or selected tests"},
    {"TestSelect", test_select, test_select_usage, "Select tests for running"},
#endif
#ifdef BCM_WARM_BOOT_SUPPORT
    {"WARMBOOT", sh_warmboot, sh_warmboot_usage, "Optionally boot warm"},
#endif /* BCM_WARM_BOOT_SUPPORT */
    {"Write", cmd_sand_mem_write, cmd_sand_mem_write_usage, "Write entry(s) into a table"},
#if defined(BCM_EASY_RELOAD_SUPPORT) || defined(BCM_EASY_RELOAD_WB_COMPAT_SUPPORT)
    {"XXReload", cmd_xxreload, cmd_xxreload_usage, "\"Easy\" Reload control"}
    ,
#endif /* defined(BCM_EASY_RELOAD_SUPPORT) || defined(BCM_EASY_RELOAD_WB_COMPAT_SUPPORT) */
};

int bcm_dnxf_cmd_cnt = COUNTOF(bcm_dnxf_cmd_list);
