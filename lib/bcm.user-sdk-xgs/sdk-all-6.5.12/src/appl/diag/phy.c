...
_if_esw_phy_raw(int u, args_t *a)
{
    ...
    if (is_sbus || is_sim) {
        ...
        /* need to check if BH 8 lane */
        if ((phy_devad == 0) && (is_bh)) {
            phy_aer = (phy_devad << 11) | (phy_lane / 4) << 4 | (phy_lane % 4) | (phy_pindex << 8);
        } else {
...
int bfcmap88060_show_fc_config(int unit, int port)
{
    ...
    /* Setting cos to default 3 */
    pCfg.cos_to_pri.cos = 3;
    ...
    cli_out("    default COS value                                                        | cos_to_pri.cos                      : %#x\n",    pCfg.cos_to_pri.cos);
    cli_out("    default priority value                                                   | cos_to_pri.pri                      : %#x\n",    pCfg.cos_to_pri.pri);
