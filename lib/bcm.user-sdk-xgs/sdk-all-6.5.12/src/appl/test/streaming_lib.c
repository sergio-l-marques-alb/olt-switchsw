...
stream_turn_off_fc(int unit, pbmp_t pbmp)
{
    ...
    stream_turn_off_idb_fc(unit, idb_fcc_regs, obm_per_pipe);
    ...
    PBMP_ITER(PBMP_PORT_ALL(unit), port) {
        if (port < SOC_MAX_NUM_PORTS) {
            ...
            if (SOC_PBMP_MEMBER(PBMP_OVERSUB(unit), port)) {
                ...
                if (rate_calc->traffic_load != 0 && (1000 * 100 / rate_calc->traffic_load) > ovs_ratio_x1000_64) {
                    COMPILER_64_UDIV_64(port_rate[port], (1000 * 100 / rate_calc->traffic_load));
                }
                else {
