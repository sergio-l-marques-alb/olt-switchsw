                   ...
                   void *data)
{
    ...
    for (i = 0; i < SOC_PCI_CMCS_NUM(unit) * SOC_SBUSDMA_CH_PER_CMC; i++) {
        if (sbusdma_desc_handle[i] == handle) {
            sbusdma_desc_handle[i] = 0;
            cmc = i / SOC_SBUSDMA_CH_PER_CMC;
            ch  = i % SOC_SBUSDMA_CH_PER_CMC;
                      ...
                      int index_max, int frag, int write)
{
    ...
    int vchan = cmc * SOC_SBUSDMA_CH_PER_CMC + ch;
            ...
            while (sb_test_data.run[n]) {
                for (frag = 0; frag < sb_test_mem[n].frag_count; frag++) {
                    ...
                    if (sb_test_data.dma_wr_en) {
                        ...
                        avail_bm |= 1 << (cmc * SOC_SBUSDMA_CH_PER_CMC);
                    }
                        ...
                        if (sb_test_data.dma_wr_en && sb_test_data.desc_mode > 0) {
                            ...
                            do {
                                ret = soc_sbusdma_desc_run(unit,
                                          sbusdma_desc_handle[cmc * SOC_SBUSDMA_CH_PER_CMC + ch]);
                            ...
                            do {
                                 sal_usleep(10);
                            } while (sbusdma_desc_handle[cmc * SOC_SBUSDMA_CH_PER_CMC + ch] != 0);
                        }
                        if (sb_test_data.dma_wr_en) {
                            avail_bm = sb_test_mem[n].mem_cmc_ch_bm;
                        }
...
sb_test_write(int unit)
{
    ...
    int n, done = FALSE;
    char str[80];
    /* sal_thread_t pid_sbusdma[SOC_PCI_CMCS_NUM(unit) * SOC_SBUSDMA_CH_PER_CMC]; */
    int ch_max;
    sal_thread_t *pid_sbusdma;

    ch_max = SOC_PCI_CMCS_NUM(unit) * SOC_SBUSDMA_CH_PER_CMC;
...
sb_test_init(int unit, args_t *a, void **pa)
{
    ...
    if (soc_feature(unit, soc_feature_cmicm) &&
        ...
        if ((sb_test_data.num_mem < 1) ||
            (sb_test_data.num_mem > SOC_PCI_CMCS_NUM(unit) * SOC_SBUSDMA_CH_PER_CMC)) {
    ...
    frag = 0;
    for (i = 0; i < SOC_PCI_CMCS_NUM(unit) * SOC_SBUSDMA_CH_PER_CMC + 1; i++) {
        if (sb_test_data.cmc_ch_bm & (0x1 << i)) {
            LOG_INFO(BSL_LS_APPL_TESTS,
                 (BSL_META_U(unit,
                             "Testing CMC %0d channel %0d\n"), i / SOC_SBUSDMA_CH_PER_CMC,
                             i % SOC_SBUSDMA_CH_PER_CMC));
            frag++;
        }
    }
    ...
    if (soc_feature(unit, soc_feature_cmicm_multi_dma_cmc) &&
        ...
        for (cmc = 1; cmc < SOC_PCI_CMCS_NUM(unit); cmc++) {
            if ((sb_test_data.cmc_ch_bm &
                 (CMIC_CMC0_SBUSDMA_CH_MASK << (cmc * SOC_SBUSDMA_CH_PER_CMC))) != 0) {
