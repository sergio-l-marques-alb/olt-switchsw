    ...
    if (dfe != DNXC_PORT_SERDES_LANE_CONFIG_INVALID_VAL)
    {
        switch (dfe)
        {
            ...
            case soc_dnxc_port_lp_dfe:
                ...
                break;
            default:
                SHR_ERR_EXIT(_SHR_E_PARAM, "port %d: Invalid serdes_lane_config_dfe SoC property value. Allowed values are on|off|lp", port);
        }
    }
    ...
    if (media_type != DNXC_PORT_SERDES_LANE_CONFIG_INVALID_VAL)
    {
        switch (media_type)
        {
            ...
            case BCM_PORT_RESOURCE_PHY_LANE_CONFIG_MEDIUM_OPTICS:
                ...
                break;
            default:
                SHR_ERR_EXIT(_SHR_E_PARAM, "port %d: Invalid serdes_lane_config_media_type SoC property value. Allowed values are backplane|copper|optics", port);
        }
    }
    ...
    if (channel_mode != DNXC_PORT_SERDES_LANE_CONFIG_INVALID_VAL)
    {
        switch (channel_mode)
        {
            ...
            case soc_dnxc_port_force_er:
                ...
                break;
            default:
                SHR_ERR_EXIT(_SHR_E_PARAM, "port %d: Invalid serdes_lane_config_channel_mode SoC property value. Allowed values are force_nr|force_er", port);
        }
    }
...
bcm_dnxf_port_probe(int unit, pbmp_t pbmp, pbmp_t *okay_pbmp) {
    ...
    BCM_PBMP_ITER(pbmp, port) {
        ...
        if(!(BCM_PBMP_MEMBER(dnxf_data_port.general.supported_phys_get(unit)->pbmp, port))){
            SHR_ERR_EXIT(_SHR_E_PARAM, "Fabric link %d is disabled for device %s", port, soc_dev_name(unit));
    ...
    if (!is_init_sequence)
    {
        ...
        BCM_PBMP_ITER(pbmp, port) 
        {
            ...
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "link %d is already probed", port);
            }
        }
    }
...
bcm_dnxf_port_init(int unit)
{
    ...
    if (!SOC_WARM_BOOT(unit)) {
        PBMP_ITER(okay_ports, p) {
            ...
            {
                SHR_ERR_EXIT(_SHR_E_CONFIG, "port %d: speed must be set for an active port", p);
            }
            ...
            { /* resource set only in init sequence and only if port speed is defined */
                ...
                /* set precoder and lp precoder according to SoC properties */
            ...
            /* set TX FIR params according to SoC property value */
...
bcm_dnxf_port_loopback_set(int unit, bcm_port_t port, int loopback)
{
    ...
    case BCM_PORT_LOOPBACK_MAC_REMOTE:
        ...
        break;
    default:
        SHR_ERR_EXIT(_SHR_E_UNAVAIL, "Unsupported loopback %d", loopback); 
        break;
    }
    ...
    bcm_port_resource_t *resource)
{
    ...
    bcm_gport_t port;
    /* we assume that pm_ids of fabric ports of an FE device start from 0 and are sequential. in case of ramon pm_ids are expected to be 0 - 23 */
    _bcm_dnxf_port_resource_multi_pm_config_t pm_config[DNXF_DATA_MAX_PORT_GENERAL_NOF_PMS];
    ...
    for(resource_index = 0; resource_index < nport; ++resource_index)
    {
     ...
     /* pm_config array initialization */
     sal_memset(pm_config, 0, sizeof(pm_config));
     ...
     for(resource_index = 0; resource_index < nport; ++resource_index)
     {
         ...
         {
             SHR_ERR_EXIT(_SHR_E_INTERNAL, "port %d: pm_id %d is not between 0 and %d", resource[resource_index].port, pm_id, DNXF_DATA_MAX_PORT_GENERAL_NOF_PMS-1);
         }
         ...
         {
             SHR_ERR_EXIT(_SHR_E_PARAM, "port %d configuration is given twice", resource[resource_index].port);
         }
     ...
     for (pm_id = 0; pm_id < DNXF_DATA_MAX_PORT_GENERAL_NOF_PMS; ++pm_id)
     {
         ...
         SHR_IF_ERR_EXIT(portmod_pm_speed_config_validate(unit, pm_id, &pbmp, 0, &vco_setting));
     ...
     for (pm_id = 0; pm_id < DNXF_DATA_MAX_PORT_GENERAL_NOF_PMS; ++pm_id)
     {
         ...
         {
             vco[0] = pm_config[pm_id].ovco;
             vco[1] = pm_config[pm_id].tvco;
             nof_plls_per_pm = dnxf_data_port.pll.nof_plls_per_pm_get(unit);
             
             SHR_IF_ERR_EXIT(portmod_pm_vco_reconfig(unit, pm_id, 0 /* no serdes reset */, nof_plls_per_pm, vco));
         }
         ...
         for(port_index = 0; port_index < DNXC_PORT_FABRIC_LANES_PER_CORE; ++port_index)
         {
             if (pm_config[pm_id].ports_to_set[port_index] == 1)
             {
                 portmod_speed_config_t speed_config_get;
                 uint32 lp_prec;

                 port = BCM_DNXF_PORT_GET(pm_id, port_index);
                 
                 SHR_IF_ERR_EXIT(portmod_port_speed_config_get(unit, port, &speed_config_get));
                 lp_prec = PORTMOD_PORT_PHY_LANE_CONFIG_LP_PREC_EN_GET(speed_config_get.lane_config);
                 if (lp_prec)
                 {
