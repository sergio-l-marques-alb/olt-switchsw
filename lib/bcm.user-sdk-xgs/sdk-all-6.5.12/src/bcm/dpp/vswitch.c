    ...
    bcm_gport_t    port)
{
    ...
    if(lif_info->type & (SOC_PPC_LIF_ENTRY_TYPE_AC|SOC_PPC_LIF_ENTRY_TYPE_FIRST_AC_IN_GROUP|SOC_PPC_LIF_ENTRY_TYPE_MIDDLE_AC_IN_GROUP)){
        ...
        /* if ac is multicast port, then learn info should be FEC which destination is MC group*/
        /*
              * if FEC created when creating Vlan port,
              *      then do nothing
              * else
              *     if backup ac,
              *        then allocate FEC and fill it
              *     else
              *        get backup ac's fec
              *     set learn infor
        */
        if (BCM_GPORT_SUB_TYPE_IS_MULTICAST(port)) {
            SOC_PPC_FEC_ID fec_id = _BCM_GPORT_ENCAP_ID_LIF_INVALID;

            if(lif_info->value.ac.learn_record.learn_info.type == SOC_PPC_FRWRD_DECISION_TYPE_FEC) {
                fec_id = lif_info->value.ac.learn_record.learn_info.dest_id;
            }

            if (fec_id == _BCM_GPORT_ENCAP_ID_LIF_INVALID) {
                if (BCM_GPORT_SUB_TYPE_MULTICAST_PS_GET(port)){
                    bcm_gport_t gport;
                    int gport_id;

                    BCM_GPORT_SUB_TYPE_MULTICAST_SET(gport_id, 0, BCM_GPORT_SUB_TYPE_MULTICAST_VAL_GET(port));
                    BCM_GPORT_VLAN_PORT_ID_SET(gport, gport_id);
                    rv = bcm_petra_vlan_port_get_mc_fec(unit, gport, &fec_id);
                    BCMDNX_IF_ERR_EXIT(rv);
                    if (fec_id == _BCM_GPORT_ENCAP_ID_LIF_INVALID) {
                        BCMDNX_ERR_EXIT_MSG(BCM_E_INTERNAL, (_BSL_BCM_MSG(" should create backup AC and to VSI first!")));
                    }
                } else {
                    ...
                    rv = bcm_dpp_am_fec_alloc(unit,0,_BCM_DPP_AM_FEC_ALLOC_USAGE_STANDARD,1,&fec_id);
                    BCMDNX_IF_ERR_EXIT(rv);
                    ...
                    fec_entry.dest.dest_val = BCM_GPORT_SUB_TYPE_MULTICAST_VAL_GET(port);
                ...
                lif_info->value.ac.learn_record.learn_info.dest_id = fec_id;
            }
        } else {
