...
{
    sal_memset((uint8 *)_bcm_rx_trap_to_ppd, 0xFF, sizeof(_bcm_rx_trap_to_ppd));
    ...
    _bcm_rx_trap_to_ppd[bcmRxTrapEgTxPortNotVlanMember]              = _BCM_TRAP_ETPP_TO_ID(SOC_PPC_TRAP_CODE_ETPP_PORT_NOT_VLAN_MEMBER);
...
{ 
    ...
    SOC_PPC_TRAP_SNOOP_ACTION_PROFILE_INFO      snoop_profile_info;
    bcm_dpp_rx_virtual_traps_t*                 virtual_traps_p = NULL;
    uint32                                      trap_index; /* for the allocation manager only, not PPD calls */
    int                                         trap_id_egress;
    uint32                                      oamp_error_trap_id;
    int                                         core;
    ...
    {    
        ...
        if (_BCM_TRAP_ID_IS_VIRTUAL(trap_id_sw)) 
        {
            ...
            /*Clears virtual bit before allocation,range 0-11*/
            virtual_trap_id = _BCM_TRAP_ID_TO_VIRTUAL(trap_id_sw);
            BCMDNX_ALLOC(virtual_traps_p, sizeof(bcm_dpp_rx_virtual_traps_t),
                "bcm_petra_rx_trap_get.virtual_traps_p");
        ...
        if (found) {
                ...
                (header_type_out == SOC_TMC_PORT_HEADER_TYPE_STACKING) )
            {
                ...
                if (packetInfo->is_trapped) {
                    ...
                    _bcm_dpp_rx_trap_type_get(unit, TRUE, packetInfo->cpu_trap_code, &found, &pkt->rx_reason);        
                    if (!found) {
                        LOG_VERBOSE(BSL_LS_BCM_RX,
                                    (BSL_META_U(unit,
...
{
    BCMDNX_INIT_FUNC_DEFS;
    BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("bcm_petra_rx_trap_protocol_set not supported ")));
exit:
    BCMDNX_FUNC_RETURN;
}
...
{
    BCMDNX_INIT_FUNC_DEFS;
    BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("bcm_petra_rx_trap_protocol_get not supported ")));
exit:
    BCMDNX_FUNC_RETURN;
}
...
{
    BCMDNX_INIT_FUNC_DEFS;
    BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("bcm_petra_rx_trap_protocol_clear not supported ")));
exit:
    BCMDNX_FUNC_RETURN;
}
...
{
    BCMDNX_INIT_FUNC_DEFS;
    BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("bcm_petra_rx_protocol_profiles_set not supported ")));
exit:
    BCMDNX_FUNC_RETURN;
}
...
{
    BCMDNX_INIT_FUNC_DEFS;
    BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("bcm_petra_rx_protocol_profiles_get not supported ")));
exit:
    BCMDNX_FUNC_RETURN;
}
