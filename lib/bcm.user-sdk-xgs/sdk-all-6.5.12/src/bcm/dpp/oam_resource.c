...
int _bcm_dpp_bfd_mep_id_alloc(int unit, uint32 flags, uint32 *mep_index) {
    ...
    if (flags == SHR_RES_ALLOC_WITH_ID) {
        ...
        rv = _bcm_dpp_oam_mep_id_alloc(unit, flags, 1, 0, mep_index);
    }
 ...
 */
int _bcm_dpp_oam_mep_id_alloc(int unit, uint32 flags, uint8 is_short, uint8 type, uint32 *mep_index) {
    int rv;
...
int _bcm_dpp_oam_bfd_trap_code_get_and_prepare_for_action(int unit, SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY* classifier_mep_entry,
    ...
    is_advanced_egress_snooping = soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "egress_snooping_advanced", 0) && SOC_IS_ARADPLUS_AND_BELOW(unit);
...
int _bcm_oam_trap_and_mirror_profile_set(int unit,
	...
	if (set_upmep) {
        ...
        if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "egress_snooping_advanced", 0) && trap_code_upmep == SOC_PPC_TRAP_CODE_OAM_CPU_SNOOP
