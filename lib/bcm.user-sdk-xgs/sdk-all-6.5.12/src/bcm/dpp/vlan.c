    ...
    bcm_gport_t gport)
{
    ...
    */
    ...
    if (gport_hw_resources.fec_id != -1) {
    ...
    }else if (is_ingress && vlan_port.failover_mc_group) { /*1+1 protection get FEC id*/
        int in_lif_id =0;
        uint8 found;
        uint32 fec_id=-1;
        ...
        soc_sand_rv = soc_ppd_l2_lif_ac_get(unit,&in_ac_key,(SOC_PPC_LIF_ID*)&in_lif_id,&ac_info,&found);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);

        if(ac_info.learn_record.learn_info.type == SOC_PPC_FRWRD_DECISION_TYPE_FEC) {
            fec_id = ac_info.learn_record.learn_info.dest_id;
        }
        if(fec_id != _BCM_GPORT_ENCAP_ID_LIF_INVALID && (BCM_GPORT_SUB_TYPE_MULTICAST_PS_GET(gport))){
    ...
    SOC_PPC_FEC_ID *fec_id)
{
    ...
    *fec_id = _BCM_GPORT_ENCAP_ID_LIF_INVALID;
    /* Determine if it's an Egress only LIF according to the gport encapsulation */
    hw_resources_flags = _BCM_DPP_GPORT_HW_RESOURCES_LOCAL_AND_GLOBAL_LIF | _BCM_DPP_GPORT_HW_RESOURCES_FEC;
    rv = _bcm_dpp_gport_to_hw_resources(unit, gport, hw_resources_flags, &gport_hw_resources);
    BCMDNX_IF_ERR_EXIT(rv);

    in_lif_id = gport_hw_resources.local_in_lif;
    if (in_lif_id != _BCM_GPORT_ENCAP_ID_LIF_INVALID ){
        soc_sand_rv = soc_ppd_lif_table_entry_get(unit, in_lif_id, &lif_entry_info);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        if(lif_entry_info.type != SOC_PPC_LIF_ENTRY_TYPE_AC) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_INTERNAL, (_BSL_BCM_MSG("LIF entry %d is not AC, type - %d."),
                                                gport, lif_entry_info.type));/* expected to be AC type*/
        }
        /* Get the learn data (fec) from here */
        ac_info = &(lif_entry_info.value.ac);
        if(ac_info->learn_record.learn_info.type == SOC_PPC_FRWRD_DECISION_TYPE_FEC) {
            *fec_id = ac_info->learn_record.learn_info.dest_id;
        }
    }
...
int _bcm_dpp_vlan_port_create(int unit, bcm_vlan_port_t *vlan_port)
{
    ...
    is_ac_with_pppoe = SOC_DPP_PPPOE_IS_ENABLE(unit) && vlan_port->egress_tunnel_value;
    ...
    if(is_protected) {
        ...
        if(!update && vlan_port->vsi != BCM_VLAN_NONE) {
            /*if this is active ac, then get fec_id from backup ac, else*/
            if (vlan_port->failover_port_id != BCM_GPORT_TYPE_NONE) {
                rv = bcm_petra_vlan_port_get_mc_fec(unit, vlan_port->failover_port_id, &fec_id);
                BCMDNX_IF_ERR_EXIT(rv);
                if (fec_id == _BCM_GPORT_ENCAP_ID_LIF_INVALID) {
                    BCMDNX_ERR_EXIT_MSG(BCM_E_INTERNAL, (_BSL_BCM_MSG(" get fec id is invalid")));
                }
            } else {
    ...
    } else if (is_1_plus_1  && vlan_port->failover_mc_group  && vlan_port->vsi != BCM_VLAN_NONE) {
        /*only fill fec when ac is backup*/
        if (!update && (vlan_port->failover_port_id == BCM_GPORT_TYPE_NONE)) {
    ...
    if (_BCM_DPP_GPORT_INFO_IS_LOCAL_PORT(gport_info) && !remote_lif) {
            ...
            if(vlan_port->vsi == BCM_VLAN_NONE || vlan_port->flags & BCM_VLAN_PORT_VSI_BASE_VID) {
                ...
                if(!update) {
                    ...
                    if (is_ingress) {
                        ...
                        if (!is_protected && (vlan_port->failover_port_id != BCM_GPORT_TYPE_NONE)) {
            ...
            if (is_1_plus_1 || is_1_plus_1_o) {
                ...
                /*1+1 proctection, fec's destination is mc_group*/
