                              ...
                              bcm_proxy_server_t *proxy_server, soc_reg_t hg_reg, int enable)
{
    ...
    if (proxy_server->server_gport_mask == BCM_PROXY_SERVER_GPORT_ALL_MASK) {
        ...
        BCM_IF_ERROR_RETURN
            (_bcm_esw_proxy_gport_resolve(unit, proxy_server->server_gport_mask,
                                          &server_port_mask, &modid_mask, 0));
...
{
    int rv = BCM_E_NONE;
    int port_size = NUM_HG_PORT(unit);
