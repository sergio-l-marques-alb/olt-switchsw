...
#define BCM_WB_DEFAULT_VERSION            BCM_WB_VERSION_1_15
...
_bcm_esw_l3_sync(int unit)
{
   ...
   if (soc_feature(unit, soc_feature_nh_for_ifp_actions)) {
    ...
    if (soc_feature(unit, soc_feature_ecmp_dlb)) {
        ...
        if (soc_feature(unit, soc_feature_ecmp_dlb_optimized)) {
            BCM_IF_ERROR_RETURN
                (bcm_th2_ecmp_dlb_sync(unit, &l3_scache_ptr));
        } else
#endif /* BCM_TOMAHAWK2_SUPPORT */
        {
    ...
    if (soc_feature(unit, soc_feature_riot)) {
        BCM_IF_ERROR_RETURN
            (bcm_td2p_l3_extended_view_sync(unit, &l3_scache_ptr));
    }
#endif /* BCM_RIOT_SUPPORT */
...
#ifdef BCM_TRIDENT3_SUPPORT
    if ((l3->l3_ecmp_levels > 1) && (l3->l3_ecmp_rh_overlay_entries == 0) && (SOC_IS_TRIDENT3X(unit))) {
        l3->l3_ecmp_rh_overlay_entries = 32768;
    }
...
bcm_esw_l3_egress_get (int unit, bcm_if_t intf, bcm_l3_egress_t *egr) 
{
    ...
    if (SOC_IS_XGS3_SWITCH(unit) && soc_feature(unit, soc_feature_l3)) {
...
#endif
        {
            ...
            if ((intf - BCM_XGS3_EGRESS_IDX_MIN) == BCM_XGS3_L3_BLACK_HOLE_NH_IDX(unit)) {
    ...
    if (SOC_IS_XGS3_SWITCH(unit) && soc_feature(unit, soc_feature_l3)) {
        ...
        if (!BCM_XGS3_L3_MPATH_EGRESS_IDX_VALID(unit, mpintf)) {
            ...
            return (BCM_E_PARAM);
                   ...
                   bcm_opt_ecmp_group_is_rh(unit, mpintf - BCM_XGS3_MPATH_EGRESS_IDX_MIN)) {
            rv = bcm_opt_ecmp_rh_multipath_get(unit, mpintf - BCM_XGS3_MPATH_EGRESS_IDX_MIN,
                                              intf_size,
                                              intf_array,
                                              intf_count,
                                              NULL);
                           ...
                           int agm_index)
{
    ...
    if (soc_feature(unit, soc_feature_dgm) &&
        ...
        bcm_l3_egress_ecmp_t ecmp_info;
        bcm_l3_egress_ecmp_t_init(&ecmp_info);
        ecmp_info.ecmp_intf = ecmp_group_id + BCM_XGS3_MPATH_EGRESS_IDX_MIN;
        rv = bcm_xgs3_l3_ecmp_get(unit, &ecmp_info, 0, NULL, &count);

        if (SOC_SUCCESS(rv) &&
            ecmp_info.dynamic_mode == BCM_L3_ECMP_DYNAMIC_MODE_DGM) {
            if (agm_index) { /* attach */
                soc_L3_ECMP_COUNTm_field32_set(unit, &ecmpc_entry, COUNTf, count - 1);
            } else { /* detach */
