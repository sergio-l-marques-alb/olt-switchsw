...
/*
 * Function:
 *  _bcm_tr3_mpls_match_failover_update
 * Purpose:
...
int
_bcm_tr3_mpls_match_failover_update(int unit, int vp, int fo_vp)
{
    _bcm_tr_mpls_bookkeeping_t *mpls_info = MPLS_INFO(unit);
    int rv = BCM_E_NONE;
    uint32 key_type = 0x10;
    int vpless_failover_port = FALSE;
    mpls_entry_entry_t mpls_hw_entry = {{0}};
    int m_index = -1;
    ...
    if  ((mpls_info->match_key[vp].flags ==
        ...
        rv = soc_mem_search(unit, mem, MEM_BLOCK_ANY, &m_index,
                &ment, hw_buf, 0);
    }

    /* If mpls entry found, update with failover vp */
    if (BCM_SUCCESS(rv) && m_index != -1) {
        rv = soc_mem_read(unit, MPLS_ENTRYm, MEM_BLOCK_ANY, m_index,
                                                (void *)&mpls_hw_entry);
        soc_MPLS_ENTRYm_field32_set(unit, &mpls_hw_entry, MPLS__SOURCE_VPf, fo_vp);
        rv = soc_mem_write(unit, MPLS_ENTRYm, MEM_BLOCK_ALL, m_index,
                                                (void *)&mpls_hw_entry);;
    }
