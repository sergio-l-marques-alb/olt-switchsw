...
#if 0
    if (BCM_FIELD_ASET_TEST(fg->aset, bcmFieldActionErspan3HdrSgtAdd)) {
       BCM_FIELD_ASET_ADD(fg->aset, _bcmFieldActionOpaque1);
    }

    if (BCM_FIELD_ASET_TEST(fg->aset, bcmFieldActionErspan3HdrVsmIdAdd)) {
...
_field_th_group_add_aset_validate(int unit, _field_group_add_fsm_t *fsm_ptr)
{
    ...
    /* Flowtracker stage has only Flowtracker GroupId as Policy */
#if defined(BCM_FLOWTRACKER_SUPPORT)
    if ((soc_feature(unit, soc_feature_field_flowtracker_support)) &&
        (_BCM_FIELD_STAGE_FLOWTRACKER == fg->stage_id)) {
        for (a_idx = 0; a_idx < _bcmFieldActionCount; a_idx++) {
            if ((BCM_FIELD_ASET_TEST(aset, a_idx)) &&
                (a_idx != bcmFieldActionFlowtrackerGroupId)) {
                                 ...
                                 _field_group_t *fg)
{
    ...
    for (part = 0; part < parts_count; part++) {
        ...
        if (arr_sz > 0) {
            ...
            _FP_XGS3_ALLOC(qid_arr, (arr_sz * sizeof(uint32)), "Group qual id");
            if (NULL == qid_arr) {
                LOG_ERROR(BSL_LS_BCM_FP, (BSL_META_U(unit,
                                    ...
                                    _field_ext_info_t *ext_info)
{
    ...
    int chunk = 0;                      /* Qualifier Chunk index.     */
    int p, l, g, e_num;                 /* part, level, gran & ext number. */
    ...
    for (level = 1; level < _FP_EXT_LEVEL_COUNT; level++) {
        ...
        for (idx = 0; idx < (_NUM_CHUNKS_PER_GRAN * _NUM_GRAN) && (3 != level); idx++) {
			...
			continue;
                    ...
                    if (chunk == ibus_field->num_chunks) {
                        continue;
                    }
...
/*
 * Function:
 *     _field_th_ingress_ext_code_get_old
 * Purpose:
 *     Finds extractor select encodings that will satisfy the
 *     requested qualifier set (Qset).
 * Parameters:
 ...
 */
STATIC int
_field_th_ingress_ext_code_get_old(int unit, _field_group_add_fsm_t *fsm_ptr)
{
                          ...
                          _field_group_add_fsm_t *fsm_ptr)
{
    ...
    if (SOC_IS_TRIDENT2(unit)) {
        _field_th_ingress_ext_code_get_old(unit, fsm_ptr);
    }
...
_field_th_group_add_slice_allocate(int unit, _field_group_add_fsm_t *fsm_ptr)
{
    ...
    /*    validate the group to make sure the group with same priority
     *    and mode doesnot exist in other slices
     */
    slice = -1;
    fsm_ptr->rv = _bcm_field_th_group_add_mode_validate(unit, fg);
    if (BCM_FAILURE(fsm_ptr->rv)) {
        fsm_ptr->rv = (BCM_E_RESOURCE);
        fsm_ptr->fsm_state = _BCM_FP_GROUP_ADD_STATE_END;
        return BCM_E_NONE;
    ...
    while (NULL != fs) {
       ...
       if (empty) {
           ...
           for (fs_ptr = fg->slices; fs_ptr != NULL;
               fs_ptr = fs_ptr->next) {
               if (fs_ptr == fs) {
                  _field_slice_t *fs_ct_ptr;
                  for (slice = 0; slice < count; slice++) {
                      fs_ct_ptr = fs_ptr + slice;

                      if (NULL == fs_ct_ptr->prev) {
                         if (slice == 0) {
                         ...
                         fs_ct_ptr->prev->next = fs_ct_ptr->next;
                      }

                      if (NULL != fs_ct_ptr->next) {
                         fs_ct_ptr->next->prev = fs_ct_ptr->prev;
                      }
                      fs_ct_ptr->next = fs_ct_ptr->prev = NULL;
                  }
               }
           }
       }
                                  ...
                                  _field_group_t *fg)
{
    ...
    for (idx = parts_count - 1; idx >= 0; idx--) {
        ...
        while (NULL != fs) {
            /* Check whether slice is shared by more than 1 conflicting groups */
                        ...
                        bcm_field_group_config_t *gc)
{
    ...
    switch (stage) {
        ...
        case _BCM_FIELD_STAGE_EXACTMATCH:
            ...
            if (BCM_SUCCESS(rv) &&
                soc_property_get(unit, spn_FLOWTRACKER_ENABLE, 0) &&
                (flags & _FP_GROUP_CONFIG_FLOWTRACKER_SPECIFIC)) {
                /*
        ...
        case _BCM_FIELD_STAGE_FLOWTRACKER:
            if ((!soc_feature(unit, soc_feature_field_flowtracker_support)) ||
                 (soc_property_get(unit, spn_FLOWTRACKER_ENABLE, 1) == 0)) {
                if (pc != NULL) {
                    sal_free(pc);
                }
                return (BCM_E_UNAVAIL);
