                              ...
                              bcmi_keygen_md_t *keygen_md)
{
    ...
    uint32 section_gran_bmp[BCMI_KEYGEN_EXT_SECTION_COUNT] = { 0 };
    ...
    for (level = 1; level < (keygen_md->num_ext_levels + 1); level++) {
        for (ext_idx = 0;
            ...
            section_gran_bmp[in_sec] |= (1 << bit_position);
            _func_rv =
            ...
            if (SHR_BITGET(ext_cfg->ext_attrs.w, BCMI_KEYGEN_EXT_MULTI_GRAN)) {
    ...
    for (in_sec = 0; in_sec < BCMI_KEYGEN_EXT_SECTION_COUNT; in_sec++) {
         sec_cfg = keygen_md->ext_cfg_db->sec_cfg[in_sec];
         if ((section_gran_bmp[in_sec] == 0x7) && sec_cfg != NULL) {
             sec_cfg->flags |= BCMI_KEYGEN_EXT_SECTION_MULTI_GRAN_1_2_4;
         }
    }
    BCMI_KEYGEN_RETURN_VAL_EXIT(BCM_E_NONE);
