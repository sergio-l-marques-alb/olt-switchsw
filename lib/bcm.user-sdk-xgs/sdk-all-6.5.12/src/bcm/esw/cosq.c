...
int
_bcm_cosq_tcb_queue_resolve(int unit, int pipe, int queue, int *index)
{
#ifdef BCM_TCB_SUPPORT
    int rv = BCM_E_UNAVAIL;
...
#ifdef BCM_TOMAHAWK2_SUPPORT
    if (SOC_IS_TOMAHAWK2(unit)) {
        rv = bcm_th2_cosq_tcb_queue_resolve(unit, pipe, queue, index);
    }
#endif
#ifdef BCM_TRIDENT3_SUPPORT
    if (SOC_IS_TRIDENT3(unit)) {
        rv = bcm_td3_cosq_tcb_queue_resolve(unit, pipe, queue, index);
    }
#endif
    return rv;
#endif
