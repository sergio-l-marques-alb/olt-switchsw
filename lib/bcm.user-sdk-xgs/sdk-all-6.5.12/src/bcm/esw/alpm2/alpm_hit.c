...
alpm_rte_hit_get(int u, int vrf_id, _alpm_tbl_t tbl, int idx, int ent)
{
    ...
    rv = ALPM_DRV(u)->alpm_bkt_hit_get(u, vrf_id, ACB_BTM(u), tbl, idx, ent);
    return rv;
}
...
{
    int rv = BCM_E_UNAVAIL;
    rv = ALPM_DRV(u)->alpm_bkt_hit_set(u, vrf_id, ACB_BTM(u), tbl, idx, ent, 1);
    return rv;
}
...
{
    int         rv = BCM_E_NONE;
    int         pkm, tcam_idx, ent_idx, vrf_id;
...
bcm_esw_alpm_hit_init(int u)
{
    ...
    usec = soc_property_get(u, "alpm_dist_hit_interval_us",
                            ALPM_HIT_INTERVAL_DEF);
