...
#define PRINT_PVT_BKT_ITEM(u, vrf, pkm, dc) \
    do { \
        LOG_CLI((BSL_META_U(u, \
                 "%-4d %-7s %-7d %-7d %-7d %-7d |   %-7s\n"), (vrf), \
                 alpm_util_pkm_str[(pkm)], (dc)->pvt_cnt, (dc)->bkt_cnt, \
                 (dc)->add_cnt, (dc)->del_cnt, VRF_NAME((u), (vrf)) )); \
    } while (0)

#define PRINT_PVT_BKT_TOT(u, pkm, all) \
    do { \
        LOG_CLI((BSL_META_U(u, \
...
STATIC void
alpm_debug_count_dump(int u, int cb_bmp, int vrf_id, uint32 flags)
{
    int found = FALSE;
    _alpm_pvt_bkt_cnt_t alpm_dc[ALPM_PKM_CNT];
    _alpm_pvt_bkt_cnt_t all_dc[ALPM_PKM_CNT];
    int                 i, vrf, pkm;
    _alpm_cb_t          *acb;
    trie_t              *pvt_trie;
    ...
    for (i = 0; i < ACB_CNT(u); i++) {
        ...
        for (vrf = 0; vrf <= ALPM_VRF_ID_MAX(u); vrf++) {
            ...
            for (pkm = 0; pkm < ALPM_PKM_CNT; pkm++) {
                if (!ACB_VRF_INITED(u, acb, vrf, pkm)) {
                    continue;
                }
                pvt_trie = ACB_PVT_TRIE(acb, vrf, pkm);

                if ((pvt_trie == NULL) || (pvt_trie->trie == NULL)) {
                    continue;
                }
                alpm_dc[pkm].pvt_cnt = pvt_trie->trie->count;
                alpm_dc[pkm].add_cnt = ACB_PVT_CTRL(acb, vrf, pkm).cnt_add;
                alpm_dc[pkm].del_cnt = ACB_PVT_CTRL(acb, vrf, pkm).cnt_del;
                alpm_dc[pkm].route_cnt = ACB_PVT_CTRL(acb, vrf, pkm).route_cnt;
                alpm_dc[pkm].db_type = ACB_PVT_CTRL(acb, vrf, pkm).db_type;

                (void)bcm_esw_alpm_pvt_traverse(u, i, vrf, pkm,
                           alpm_pvt_bkt_cnt_cb, (void *)&alpm_dc[pkm]);
                found = TRUE;

                if (flags & BCM_ALPM_DEBUG_SHOW_FLAG_BKT) {
                    PRINT_PVT_BKT_ITEM(u, vrf, pkm, &alpm_dc[pkm]);
                }
                ALPM_PVT_BKT_CNT_ADD(&all_dc[pkm], &alpm_dc[pkm]);
            }
        }
        ...
        if (found == FALSE) {
            ...
            PRINT_SPLIT_LINE(u);
            for (pkm = 0; pkm < ALPM_PKM_CNT; pkm++) {
                PRINT_PVT_BKT_TOT(u, pkm, &all_dc[pkm]);
            }
        }
    }
}
                  ...
                  _alpm_trv_data_t *trv_data)
{
    ...
    if (PVT_BKT_TRIE(pvt_node) == NULL) {
        if (BCM_SUCCESS(rv)) {
            ALPM_INFO((" --PVT %p->BKT %p: idx:%d.%d Sublen:%d IP:%s w/o bkt\n",
                       pvt_node, PVT_BKT_DEF(pvt_node),
                       idx, sub_idx, pvt_node->key_len, ip_buf));
        } else {
            ALPM_ERR((" --PVT %p->BKT %p: idx:%d.%d Sublen:%d IP:%s w/o bkt"
                      " Error=(VRF:%d PKM:%d)\n",
                      pvt_node, PVT_BKT_DEF(pvt_node),
                      idx, sub_idx, pvt_node->key_len, ip_buf,
                      vrf_id, pkm));
        ...
        if (BCM_SUCCESS(rv)) {
            ALPM_INFO(("\n --PVT %p->BKT %p: idx:%d.%d Sublen:%d IP:%s w/o def_rte\n",
                       pvt_node, PVT_BKT_DEF(pvt_node),
                       idx, sub_idx, pvt_node->key_len, ip_buf));
        } else {
            ALPM_ERR(("\n --PVT %p->BKT %p: idx:%d.%d Sublen:%d IP:%s w/o def_rte"
        ...
        if (BCM_SUCCESS(rv)) {
            ALPM_INFO(("\n --PVT %p->BKT %p: idx:%d.%d Sublen:%d bpmDest:%d IP:%s\n",
                       pvt_node, PVT_BKT_DEF(pvt_node),
...
STATIC void
alpm_trie_debug_dump(int u, int cb_bmp, int vrf_id)
{
    int         i, vrf, pkm;
    _alpm_cb_t  *acb;
    _alpm_trv_data_t trv_data;
    ...
    for (i = 0; i < ACB_CNT(u); i++) {
        ...
        for (vrf = 0; vrf <= ALPM_VRF_ID_MAX(u); vrf++) {
            ...
            for (pkm = 0; pkm < ALPM_PKM_CNT; pkm++) {
                ...
                trv_data.pkm = pkm;
                ALPM_INFO(("\n%s VRF:%d PKM:%s\n",
                           alpm_util_acb_str[i], vrf, alpm_util_pkm_str[pkm]));
                (void)bcm_esw_alpm_pvt_traverse(u, i, vrf, pkm,
                                                alpm_pvt_debug_cb, &trv_data);

                if (ACB_HAS_RTE(acb)) {
                    ALPM_INFO(("\n"));
                    (void)bcm_esw_alpm_pfx_traverse(u, vrf, pkm,
 ...
 */
int
bcm_esw_alpm_debug_show(int u, int vrf_id, uint32 flags)
{
    int rv = BCM_E_NONE;
    ...
    if (flags & (BCM_ALPM_DEBUG_SHOW_FLAG_CNT | BCM_ALPM_DEBUG_SHOW_FLAG_BKT)) {
        alpm_debug_count_dump(u, ALPM_CB_ALL_BMP, vrf_id, flags);
    }

    if (flags & (BCM_ALPM_DEBUG_SHOW_FLAG_INTDBG)) {
        alpm_trie_debug_dump(u, ALPM_CB_ALL_BMP, vrf_id);
    }
                    ...
                    _alpm_bkt_adata_t *adata, _alpm_dbg_data_t *dp)
{
    ...
    char ip_buf[IP6ADDR_STR_LEN];
    _bcm_defip_cfg_t lpm_cfg;
    _alpm_tbl_t bkt_tbl = ACB_BKT_TBL(acb, vrf_id, pkm);
    _alpm_tbl_t pvt_tbl = (ACB_HAS_TCAM(acb) ? acb->pvt_tbl[pkm] :
                           ACB_BKT_TBL(ACB_UPR(u, acb), vrf_id, pkm));
    ...
    /**** Pivot level ****/
    alpm_util_bkt_info_get(u, vrf_id, pkm, acb, fent, sub_idx, &bkt_info,
                           &sublen, &def_miss);
    ...
    if (ALPMC(u)->_alpm_dbg_info_bmp & _ALPM_DBG_INFO_PVT) {
        alpm_util_cfg_construct(u, pkm, vrf_id, key, key_len, adata, &lpm_cfg);
        if (ALPM_IS_IPV4(pkm)) {
            alpm_util_fmt_ipaddr(ip_buf, lpm_cfg.defip_ip_addr);
        } else {
            alpm_util_fmt_ip6addr(ip_buf, lpm_cfg.defip_ip6_addr);
    ...
    for (bnk = 0; bnk < ACB_BNK_PER_BKT(acb, vrf_id); bnk++) {
        ...
        if ((fmt == 0) || (fmt > ACB_FMT_MAX(acb))) {
            if (fmt > ACB_FMT_MAX(acb)) {
        ...
        ALPM_IER(alpm_bkt_entry_read(u, ACB_BKT_TBL(acb, vrf_id, pkm),
                                     acb, bkt_entry, idx1));

        for (eid = 0; eid < ACB_FMT_ENT_MAX(acb, fmt); eid++) {
            uint32 key2[4] = {0};
            int len2 = 0;
            int sub_bkt_ptr = 0;

            /* process bucket entry Prefix */
            alpm_util_ent_ent_get(u, acb, bkt_entry, fmt, eid, ftmp);
            alpm_util_bkt_pfx_get(u, acb, ftmp, fmt, key2, &len2, &valid);
            alpm_util_bkt_adata_get(u, pkm, acb, ftmp, fmt, &adata1, &sub_bkt_ptr);
            ...
            if (!valid || BI_SUB_BKT_IDX(&bkt_info) != sub_bkt_ptr) {
                ...
                if (ACB_BKT_FIXED_FMT(acb, 1)) {
                    ...
                    alpm_util_pfx_cat(u, pkm, key, sublen, key2, len2, new_key, &new_len);
                }
            }
            ...
            if (ACB_HAS_RTE(acb)) {
                ...
                if (ALPMC(u)->_alpm_dbg_info_bmp & _ALPM_DBG_INFO_RTE) {
                    alpm_util_cfg_construct(u, pkm, vrf_id, new_key, new_len,
                                            &adata1, &lpm_cfg);
                    if (ALPM_IS_IPV4(pkm)) {
                        alpm_util_fmt_ipaddr(ip_buf, lpm_cfg.defip_ip_addr);
                    } else {
                    ...
                    ALPM_INFO(("VRF:%d PKM:%s %s: Route(%d) idx:%d.%d Sublen:%d"
                               " fmt:%d Dest:%d IP:%s\n",
                               vrf_id, alpm_util_pkm_str[pkm],
                               ALPM_TBL_NAME(bkt_tbl),
                               dp->route_cnt, idx1, eid,
            ...
            alpm_util_ent_data_get(u, pkm, acb, ftmp, fmt, fent1);
            ALPM_IER(alpm_cb_dbg_process(u, vrf_id, pkm, ACB_DWN(u, acb),
 ...
 */
int
bcm_esw_alpm_debug_brief_show(int u, int phy_bkt, int index)
{
    int ipv6, key_mode;
    int idx, sub_idx;
    int vrf_id;
    ...
    uint32 key[4];
    int key_pfx_len;
    int i, j, bit128, pid, alloc_sz, k, ent_used_tot;
    ...
    for (i = 0; i < ACB_CNT(u); i++) {
        ...
        for (pid = 0; pid < ALPM_BKT_PID_CNT; pid++) {
            ...
            dp->dc[i].ent_max = dp->dc[i].bank_max * ACB_FMT_ENT_MAX(acb, 1);
            ...
            if (ACB_HAS_TCAM(acb)) {
                for (pkm = 0; pkm < ALPM_PKM_CNT; pkm++) {
                    dp->dc[i].pvt_max[pkm] =
                       (pkm == ALPM_PKM_32B ? 2 : 1) * tcam_table_size(u, pkm);
                }
            }
        }
    }
    ...
    for (pk = 0; pk < ALPM_PKM_CNT; pk++) {
        ...
        for (idx = 0; idx < idx_end; idx++) {
            ALPM_IEG(tcam_entry_read(u, pk, lpm_entry, idx, idx));
            ...
            for (sub_idx = 0; sub_idx < ALPM_TCAM_SUBCNT; sub_idx += step_size) {
                ...
                pid = (ALPM_TCAM_ZONED(u) ? ALPM_VRF_ID_TO_BPC_PID(u, vrf_id) : 0);
                if (pid == 1 &&
                    ACB_BKT_POOL(acb, 0) == ACB_BKT_POOL(acb, pid)) {
                    pid = 0;
                }
                dp = &dbg[pid];
                ...
                if (!use_alpm) {
                    ...
                    if (ALPMC(u)->_alpm_dbg_info_bmp &
                        ...
                        alpm_util_cfg_construct(u, pkm, vrf_id, key, key_pfx_len,
                                                &adata0, &lpm_cfg);
                        if (ALPM_IS_IPV4(pkm)) {
                            alpm_util_fmt_ipaddr(ip_buf, lpm_cfg.defip_ip_addr);
                        } else {
                        ...
                        ALPM_INFO(("VRF:%d PKM:%s TCAM: Direct Route(%d)"
                                   " idx:%d.%d Sublen:%d Dest:%d IP:%s\n",
                                   vrf_id, alpm_util_pkm_str[pkm],
                                   dp->route_cnt, idx, sub_idx, key_pfx_len,
    ...
    if (ALPMC(u)->_alpm_dbg_info_bmp & _ALPM_DBG_INFO_CNT) {
        bit128 = !!tcam_table_size(u, ALPM_PKM_128);
        /* count bkt sharing */
        for (i = 0; i < ACB_CNT(u); i++) {
            acb = ACB(u, i);
            for (pid = 0; pid < ALPM_BKT_PID_CNT; pid++) {
...
#if defined(BCM_TRIDENT3_SUPPORT)
        if (SOC_IS_TRIDENT3(u)) { /* for TD3 specific only */
                       ...
                       _alpm_bkt_adata_t *adata, _alpm_sanity_t *sanity)
{
    ...
    trie_t *pvt_trie, *pfx_trie, *bkt_trie;
    ...
    rv = BCM_E_NONE;
    alpm_trie_key_to_pfx(u, pkm, key, key_len, pfx);
    pvt_trie = ACB_PVT_TRIE(acb, vrf_id, pkm);
    if (pvt_trie != NULL) {
        rv = trie_search(pvt_trie, pfx, key_len, (trie_node_t **)&pvt_node);
        if (BCM_SUCCESS(rv)) {
    ...
    if (BCM_FAILURE(rv)) {
        ...
        if (ACB_HAS_TCAM(acb)) {
            ALPM_ERR(("VRF:%d PKM:%s %s: TCAM idx:%d.%d Sublen:%d bpmDest:%d IP:%s\n ==> %s\n",
                      vrf_id, alpm_util_pkm_str[pkm], ALPM_TBL_NAME(pvt_tbl),
                      idx, sub_idx, key_len, adata->defip_ecmp_index,
                      ip_buf, err_msg));
        } else {
            ALPM_ERR(("VRF:%d PKM:%s %s: PVT idx:%d.%d Sublen:%d fmt:%d bpmDest:%d IP:%s\n ==> %s\n",
                       vrf_id, alpm_util_pkm_str[pkm], ALPM_TBL_NAME(pvt_tbl),
                       idx, sub_idx, key_len, pvtfmt,
    ...
    if ((def_miss == FALSE) && (pfx_trie != NULL)) {
        rv = trie_find_lpm(pfx_trie, pfx, key_len,
        ...
        if (BCM_FAILURE(rv)) {
            ...
            if (pvt_node != NULL) {
                if (PVT_BKT_DEF(pvt_node) != pfx_node->bkt_ptr) {
                    sanity->error++;
                    ALPM_ERR(("%s: Pivot idx:%d.%d DEF_PFX %p"
                              " diff than BPM BKT %p\n",
                              ALPM_TBL_NAME(pvt_tbl), idx, sub_idx,
    ...
    for (bnk = 0; bnk < ACB_BNK_PER_BKT(acb, vrf_id); bnk++) {
        ...
        /**** Bucket Level ****/
        idx1 = ALPM_TAB_IDX_GET_BKT_BNK(acb, BI_ROFS(&bkt_info), BI_BKT_IDX(&bkt_info), bnk);
        ALPM_IER(alpm_bkt_entry_read_no_cache(u, ACB_BKT_TBL(acb, vrf_id, pkm),
                                              acb, bkt_entry, idx1));
        ...
        for (eid = 0; eid < ACB_FMT_ENT_MAX(acb, fmt); eid++) {
            ...
            if (BCM_SUCCESS(rv)) {
                /* process bucket entry ASSOC_DATA */
                alpm_trie_key_to_pfx(u, pkm, new_key, new_len, pfx);

                /* HW bucket key vs SW bkt trie */
                if (pvt_node != NULL) {
                    bkt_trie = PVT_BKT_TRIE(pvt_node);
                    if (bkt_trie != NULL) {
                        rv = trie_search(bkt_trie, pfx, new_len,
            ...
            if (BCM_FAILURE(rv)) {
                ...
                ALPM_ERR(("VRF:%d PKM:%s %s: BKT idx:%d(bkt%d bnk%d).%d Sublen:%d"
                          " fmt:%d Dest:%d IP:%s\n ==> %s\n",
                          vrf_id, alpm_util_pkm_str[pkm],
                          ALPM_TBL_NAME(bkt_tbl),
            ...
            if (ACB_HAS_RTE(acb)) {
                ...
                if (BCM_SUCCESS(rv)) { /* if failed on bkt, bypass pfx check */
                    if (pfx_trie != NULL) {
                        rv = trie_search(pfx_trie, pfx, new_len,
                                         (trie_node_t **)&pfx_node);
                        if (BCM_SUCCESS(rv)) {
                            bkt_idx = idx1 | ((eid & ALPM_IDX_ENT_MASK)
                                              << ALPM_IDX_ENT_SHIFT);
                    ...
                    if (BCM_FAILURE(rv)) {
                        ...
                        ALPM_ERR(("VRF:%d PKM:%s %s: RTE idx:%d.%d Sublen:%d"
                                  " fmt:%d Dest:%d IP:%s\n ==> %s\n",
                                  vrf_id, alpm_util_pkm_str[pkm],
                                  ALPM_TBL_NAME(bkt_tbl),
            ...
            /* get ALPM Data entry ready for next level */
            alpm_util_ent_data_get(u, pkm, acb, ftmp, fmt, fent1);
            ALPM_IER(alpm_cb_sanity_process(u, vrf_id, pkm, ACB_DWN(u, acb),
 ...
 *      index    - (In)Memory index (not in used)
 *      check_sw - (In)Check SW tables as well.
 * Returns:
 *      BCM_E_XXX
 */
int
bcm_esw_alpm_sanity_check(int u, soc_mem_t mem, int index, int check_sw)
{
    int ipv6, key_mode;
    int idx, sub_idx;
    int vrf_id;
...
bad:
    ...
    if (rv == BCM_E_NONE) {
        ...
        if (BCM_SUCCESS(rv)) {
            LOG_CLI(("L3 ALPM sanity check: passed!\n"));
        }
    }
 ...
 */
STATIC int
alpm_cb_bm_lookup(int u, int vrf_id, int pkm, _alpm_cb_t *acb,
                    void *fent, uint32 *key, int key_len, int idx,
                    int sub_idx, _bcm_defip_cfg_t *lpm_cfg,
                    _alpm_lkup_res_t *hit_res, uint32 *sw_key, int sw_len)
{
    ...
    for (bnk = 0; bnk < ACB_BNK_PER_BKT(acb, vrf_id); bnk++) {
        ...
        for (eid = 0; eid < ACB_FMT_ENT_MAX(acb, fmt); eid++) {
            ...
            if (ACB_BKT_FIXED_FMT(acb, 1)) {
                ...
                alpm_util_pfx_cat(u, pkm, key, sublen, key2, len2, hw_key, &hw_len);
            }

            if (hw_len > sw_len) { /* invalid or not match */
                continue;
            }
            alpm_util_len_to_mask(pkm, hw_len, hw_mask);

            /* new_len <= sw_len: compare all hw_masked key */
            key_ent_cnt = ALPM_KEY_ENT_CNT(pkm);
    ...
    if (bm_len != -1) {
        ...
        if (ACB_HAS_RTE(acb)) {
            *hit_res = ALPM_HIT;
            ALPM_VERB(("Hit route in %s idx:%d.%d\n",
            ...
            alpm_util_ent_data_get(u, pkm, acb, bm_fent, bm_fmt, ftmp);
            ALPM_IER(alpm_cb_bm_lookup(u, vrf_id, pkm, ACB_DWN(u, acb),
                                       ftmp, bm_key, bm_len, bm_idx, bm_ent,
                                       lpm_cfg, &hit_res1, sw_key, sw_len));
        ...
        alpm_util_bkt_adata_get(u, pkm, acb, bm_fent, bm_fmt, &adata1, NULL);
        alpm_util_cfg_construct(u, pkm, vrf_id, bm_key, bm_len, &adata1, lpm_cfg);
        lpm_cfg->defip_index =
            alpm_util_ent_phy_idx_get(u, acb, pkm, vrf_id, bm_idx); /* ALPM mem index */

        /* Get HIT bit from best matched entry */
        if (!ALPM_HIT_SKIP(u)) {
            hit_val = alpm_rte_hit_get(u, vrf_id, bkt_tbl, bm_idx, bm_ent);
                     ...
                     int sw_len, _bcm_defip_cfg_t *lpm_cfg)
{
    ...
    if (do_urpf) {
        idx_min += tcam_table_size(u, pkm);
    }
    idx_end = idx_min + idx_cnt;

    /* 1) Scanning for Global High routes first */
    for (idx = idx_min; idx < idx_end; idx++) {
        ALPM_IER(tcam_entry_read(u, pkm, lpm_entry, idx, idx));

        for (sub_idx = 0; sub_idx < ALPM_TCAM_SUBCNT; sub_idx += step_size) {
            rv_tcam = tcam_valid_entry_mode_get(u, pkm, lpm_entry,
            ...
            tcam_entry_vrf_id_get(u, pkm, lpm_entry, sub_idx, &hw_vrf_id);
            ...
            hw_len = 0;
            tcam_entry_to_key(u, pkm, lpm_entry, sub_idx, hw_key);
            tcam_entry_pfx_len_get(u, pkm, lpm_entry, sub_idx, &hw_len);
            ...
            alpm_util_key_to_cfg(u, pkm, hw_key, lpm_cfg);
    ...
    for (idx = idx_min; idx < idx_end; idx++) {
        ...
        for (sub_idx = 0; sub_idx < ALPM_TCAM_SUBCNT; sub_idx += step_size) {
            ...
            if (!use_alpm) {
                ...
                tcam_entry_bdata_get(u, pkm, lpm_entry, sub_idx, fent);
                ALPM_IER(alpm_cb_bm_lookup(u, hw_vrf_id, pkm, acb, fent,
                                           hw_key, hw_len, idx, sub_idx,
 ...
 *      nh_ecmp_idx - (OUT)Next hop or ecmp group index of bpm route
 * Returns:
 *      BCM_E_XXX
 */
int
bcm_esw_alpm_find(int u, _bcm_defip_cfg_t *lpm_cfg, int *nh_ecmp_idx)
{
    ...
    alpm_util_cfg_to_key(u, lpm_cfg, sw_key); /* get sw_key[4] */
