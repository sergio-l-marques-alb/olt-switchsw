...
#define GH2_VXLAN_RIOT_CH_PORT_MASK          0x7fff
...
#define GH2_VXLAN_RIOT_CH_ENCAP_VALUE(_ipmc_, _port_)                      \
    ...
    int *index)
{
    ...
    if (BCM_XGS3_L3_EGRESS_IDX_VALID(unit, ecmp_member_array[0])) {
        *index = (ecmp_member_array[0] - BCM_XGS3_EGRESS_IDX_MIN);
    } else {
        rv = BCM_E_PARAM;
    }
    ...
    void *user_data)
{
    ...
    nh_ecmp_intf = (*nh_ecmp_index + BCM_XGS3_EGRESS_IDX_MIN);
    found = 0;
    ...
    if (found != 0) {
        ...
        nhi = (nh_ecmp_intf - BCM_XGS3_EGRESS_IDX_MIN);
        BCM_IF_ERROR_RETURN(
            soc_mem_read(unit, EGR_L3_NEXT_HOPm, MEM_BLOCK_ANY,
                         nhi, &egr_nh_entry));
    ...
    if (vp_info->vp_type == GH2_VXLAN_DEST_VP_TYPE_TUNNEL) {
        ...
        if (ecmp) {
            ...
            vxlan_port->egress_if =
                (nh_ecmp_index + BCM_XGS3_MPATH_EGRESS_IDX_MIN);
            ...
            vxlan_port->egress_if =
                (nh_ecmp_index + BCM_XGS3_EGRESS_IDX_MIN);
        }
    ...
    int vp)
{
    ...
    for (port_idx = 0; port_idx < local_member_count; port_idx++) {
        if (vxlan_port->flags & BCM_VXLAN_PORT_LOOPBACK) {
            ...
            encap_info.encap_service = BCM_SWITCH_ENCAP_SERVICE_CUSTOM_HEADER;
            encap_info.value32 =
                GH2_VXLAN_RIOT_CH_ENCAP_VALUE(0, local_port_out[port_idx]);
            ...
            for (i = 0; i < 2; i++) {
                ...
                match_info.value32 =
                    GH2_VXLAN_RIOT_CH_ENCAP_VALUE((is_ipmc) ? 1 : 0,
                                                  local_port_out[port_idx]);
