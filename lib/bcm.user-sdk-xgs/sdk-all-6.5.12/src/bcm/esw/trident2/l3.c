...
#ifdef BCM_TRIDENT3_SUPPORT
            if (SOC_IS_TRIDENT3X(unit)) {
                /* TD3 is different from previous chips.
                   0 in KEY_TYPE was used to distinguish regular from extended
                   entries in previous chips.
                   In TD3 DATA_TYPE is 1 for extended entries */
                ...
                soc_mem_field32_set(unit, mem, buf_p, BASE_VALID_0f, 1); 
            } else
#endif
            {
                                        ...
                                        int *nh_idx, void *l3x_entry, int key_type)
{
    ...
    if (soc_feature(unit, soc_feature_flex_flow) &&
        (key_type >= TD3_L3_HASH_KEY_TYPE_COUNT)) {
                                ...
                                int idx, _bcm_l3_cfg_t *l3cfg, int *nh_idx)
{
    ...
    case L3_ENTRY_SINGLEm:
        ...
        if  (!(soc_feature(unit, soc_feature_flex_flow)) &&
             ...
             ((mem == L3_ENTRY_SINGLEm) && (data_type != 0)))) {
    ...
    case L3_ENTRY_DOUBLEm:
        ...
        if (key_type == TD2_L3_HASH_KEY_TYPE_V4MC) {
            ...
            l3cfg->l3c_flags = BCM_L3_IPMC | BCM_L3_L2ONLY;
              ...
              (((mem == L3_ENTRY_DOUBLEm) && (data_type == 2))
               || (mem != L3_ENTRY_DOUBLEm))){
                             ...
                             _bcm_l3_cfg_t *l3cfg)
{
    ...
    if (ipv6) {
        ...
        if (soc_feature(unit, soc_feature_base_valid)) {
            ...
            /* Set entry valid bit. */
            soc_mem_field32_set(unit, mem, buf_p, BASE_VALID_0f, 5);
            soc_mem_field32_set(unit, mem, buf_p, BASE_VALID_1f, 6);
            soc_mem_field32_set(unit, mem, buf_p, BASE_VALID_2f, 6);
            soc_mem_field32_set(unit, mem, buf_p, BASE_VALID_3f, 7);
        } else {
        for (idx = 0; idx < 4; idx++) {
            /* Set key type */
                           ...
                           int idx, _bcm_l3_cfg_t *l3cfg)
{
    ...
    if (soc_feature(unit, soc_feature_base_valid)) {
        if (ipv6) {
            if (soc_mem_field32_get(unit, mem, buf_p, BASE_VALID_0f) != 5 ||
                soc_mem_field32_get(unit, mem, buf_p, BASE_VALID_1f) != 6 ||
                soc_mem_field32_get(unit, mem, buf_p, BASE_VALID_2f) != 6 ||
                soc_mem_field32_get(unit, mem, buf_p, BASE_VALID_3f) != 7) {
                return (BCM_E_NOT_FOUND);
            }
        } else {
            if (soc_mem_field32_get(unit, mem, buf_p, BASE_VALID_0f) != 3 ||
                soc_mem_field32_get(unit, mem, buf_p, BASE_VALID_1f) != 4) {
                return (BCM_E_NOT_FOUND);
            }
        }
...
bcm_th_alpm_add(int unit, _bcm_defip_cfg_t * lpm_cfg, int nh_ecmp_idx)
{
    ...
    alpm_trace_log(unit, ALPM_TRACE_OP_ADD, lpm_cfg, nh_ecmp_idx);
...
bcm_th_alpm_del(int unit, _bcm_defip_cfg_t * lpm_cfg)
{
    ...
    alpm_trace_log(unit, ALPM_TRACE_OP_DELETE, lpm_cfg, -1);
