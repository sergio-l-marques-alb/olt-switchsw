                       ...
                       bcm_rtag7_base_hash_t *hash_res)
{
    ...
    uint64 rtag7_hash_control, rtag7_hash_control_2_64;
    ...
    if (SOC_REG_IS_VALID(unit, RTAG7_HASH_CONTROL_2_64r)) {
        SOC_IF_ERROR_RETURN
            (READ_RTAG7_HASH_CONTROL_2_64r(unit, &rtag7_hash_control_2_64));
    } else if (SOC_REG_IS_VALID(unit, RTAG7_HASH_CONTROL_2r)) {
        SOC_IF_ERROR_RETURN
    ...
    if (SOC_REG_FIELD_VALID(unit, RTAG7_HASH_CONTROL_2r,
        ...
        rtag7_en_flow_label_ipv6_a =
            soc_reg_field_get(unit, RTAG7_HASH_CONTROL_2r,
                              ...
                              ENABLE_FLOW_LABEL_IPV6_Af);
    } else if (SOC_REG_FIELD_VALID(unit, RTAG7_HASH_CONTROL_2_64r,
                                   ENABLE_FLOW_LABEL_IPV6_Af)) {
        rtag7_en_flow_label_ipv6_a =
 ...
 */
STATIC int
compute_td2_ecmp_hash(int unit, uint32 flags, bcm_rtag7_base_hash_t *hash_Base,
                  uint32 *hash_value)
{
    /*regular var declaration*/
    uint32 hash_sub_sel;
    ...
    int concat_f              = CONCATENATE_HASH_FIELDS_ECMPf ;
    int ecmp_hash_control_f   = ECMP_HASH_USE_RTAG7f;
    int rtag7_ecmp_hash_sel_f = USE_FLOW_SEL_ECMPf;
    rtag7_port_based_hash_entry_t
           port_based_hash_entry;
    rtag7_flow_based_hash_entry_t 
           flow_based_hash_entry;
    ...
    if (((soc_feature(unit, soc_feature_riot) ||
        ...
        flags & BCM_SWITCH_PKT_INFO_HASH_OVERLAY)) {

        sub_sel_f             = SUB_SEL_ECMP_LEVEL1f;
        ecmp_offset_f         = OFFSET_ECMP_LEVEL1f ;
        concat_f              = CONCATENATE_HASH_FIELDS_ECMP_LEVEL1f ;
        ecmp_hash_control_f   = ECMP1_HASH_USE_RTAG7f;
        rtag7_ecmp_hash_sel_f = USE_FLOW_SEL_ECMP1f;
    }
#endif
    ...
    SOC_IF_ERROR_RETURN
        (READ_RTAG7_HASH_SELr(unit, &rtag7_hash_sel));
    if (soc_reg_field_valid(unit, RTAG7_HASH_SELr, rtag7_ecmp_hash_sel_f)) {
        rtag7_ecmp_use_macro_flow_hash =
            soc_reg_field_get(unit, RTAG7_HASH_SELr,
                              rtag7_hash_sel, rtag7_ecmp_hash_sel_f);
    } else {
    ...
    if (rh_mode == RTAG7_RH_MODE_ECMP || rh_mode == RTAG7_RH_MODE_LAG) {
        ...
        if (soc_feature(unit, soc_feature_td3_style_dlb_rh)){
            rh_flowset_table_config = soc_reg_field_get(unit, ENHANCED_HASHING_CONTROLr,
                       enhanced_hash_control, ECMP_FLOWSET_TABLE_CONFIGf);
        } else {
            rh_flowset_table_config = soc_reg_field_get(unit, ENHANCED_HASHING_CONTROLr,
...
check_td2_ecmp_rh_enable (int unit, int ecmp_group, uint32 ethertype) 
{
    ...
    if (soc_feature(unit, soc_feature_td3_style_dlb_rh)){
        rh_ecmp_enable  = soc_L3_ECMP_COUNTm_field32_get(unit, &ecmpc_entry, LB_MODEf);
    } else {
...
check_td2_lag_rh_enable (int unit, int tgid, uint32 ethertype) 
{
    ...
    uint8  rh_lag_group_enable = 0, rh_lag_ethertype_enable, lag_rh_enable=0;
    uint32 rh_flowset_table_config, enhanced_hash_control;
    ...
    SOC_IF_ERROR_RETURN(READ_ENHANCED_HASHING_CONTROLr(unit, &enhanced_hash_control));
    if (soc_feature(unit, soc_feature_td3_style_dlb_rh)){
        rh_flowset_table_config = soc_reg_field_get(unit, ENHANCED_HASHING_CONTROL_2r, enhanced_hash_control,
                                                HGT_LAG_FLOWSET_TABLE_CONFIGf);
        rh_lag_group_enable = (soc_TRUNK_GROUPm_field32_get(unit, &dst_tg_entry,
                                             TRUNK_MODEf)) ==  3 ? 1 : 0;
    } else {
        rh_flowset_table_config = soc_reg_field_get(unit, ENHANCED_HASHING_CONTROLr, enhanced_hash_control,
                                                RH_FLOWSET_TABLE_CONFIG_ENCODINGf);
...
check_td2_hgt_rh_enable (int unit, int hgtid, uint32 ethertype) 
{
    ...
    uint8  rh_hgt_group_enable = 0, rh_hgt_ethertype_enable, hgt_rh_enable = 0;
    uint32 rh_hgt_enable = 0, enhanced_hash_control;
    ...
    if (soc_feature(unit, soc_feature_td3_style_dlb_rh)){
        SOC_IF_ERROR_RETURN(READ_HG_TRUNK_MODEm(unit, MEM_BLOCK_ANY, hgtid,
                    &hg_trunk_mode_entry));
        rh_hgt_group_enable  = soc_HG_TRUNK_MODEm_field32_get(unit, &hg_trunk_mode_entry,
                                                         HG_TRUNK_LB_MODEf);
        SOC_IF_ERROR_RETURN(READ_ENHANCED_HASHING_CONTROL_2r(unit, &enhanced_hash_control));
        rh_hgt_enable = soc_reg_field_get(unit, ENHANCED_HASHING_CONTROL_2r, enhanced_hash_control,
                                                RH_DLB_SELECTIONf);
                   ...
                   uint32 hash_index, bcm_gport_t *dst_gport)
{
    ...
    if (BCM_PBMP_IS_NULL(local_pbmp)) {
        ...
        BCM_IF_ERROR_RETURN
            (bcm_esw_trunk_get(unit, tgid, &trunk_info, BCM_TRUNK_MAX_PORTCNT,
                ...
                uint8 lag_rh_enable, uint32 ethertype)
{
    ...
    if (lag_rh_enable) {
        ...
        if (soc_feature(unit, soc_feature_td3_style_dlb_rh)){
            hgt_rh_flow_set_base = soc_HG_TRUNK_GROUPm_field32_get(unit,
                                           &hg_tg_entry, RH_FLOW_SET_BASEf);
            hgt_rh_flow_set_size = soc_HG_TRUNK_GROUPm_field32_get(unit,
                                           &hg_tg_entry, RH_FLOW_SET_SIZEf);
        } else
#endif
        {
            BCM_IF_ERROR_RETURN
                                   ...
                                   bcm_if_t *dst_intf)
{
    ...
    if (0 != (pkt_info->flags & BCM_SWITCH_PKT_INFO_HASH_MULTIPATH)) {
        ...
        if (0 != (check_td2_ecmp_rh_enable(unit, 
            ...
            ecmp_rh_enable = TRUE;
            hash_value = 0;
            rc = compute_td2_ecmp_rh_hash(unit, &hash_res, &hash_rh_value);
            if (rc != BCM_E_NONE) {
                return rc;
            }
            LOG_VERBOSE(BSL_LS_BCM_COMMON,
            ...
            hash_rh_value = 0;
            rc = compute_td2_ecmp_hash(unit, pkt_info->flags, &hash_res, &hash_value);
            if (rc != BCM_E_NONE) {
                return rc;
            }
            LOG_VERBOSE(BSL_LS_BCM_COMMON,
