...
int
bcm_td3_cosq_tcb_queue_resolve(int unit, int pipe, int queue, int *index)
{
    *index = queue & _TD3_TCB_UCQ_SCOPE_QUEUE_MASK;
    return BCM_E_NONE;
}
                                    ...
                                    bcm_cosq_tcb_config_t *config)
{
    ...
    int type, entity_num, pipe, id, propability;
    ...
    switch(type) {
        case _TCB_SCOPE_UCQ:
            ...
            entity_num = soc_reg64_field32_get(unit, TCB_CONFIGr, rval64,
                                               SCOPE_ENTITYf);
            pipe = entity_num >> _TD3_TCB_UCQ_SCOPE_PIPE_OFFSET;
            id = (entity_num & _TD3_TCB_UCQ_SCOPE_QUEUE_MASK) %
                 _BCM_TD3_NUM_UCAST_QUEUE_PER_PORT;
            local_mmu_port = (entity_num & _TD3_TCB_UCQ_SCOPE_QUEUE_MASK) /
                             _BCM_TD3_NUM_UCAST_QUEUE_PER_PORT;
