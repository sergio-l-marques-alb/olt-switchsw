        ...
        if (pfc_deadlock_control->hw_cos_idx_inuse[cos] == TRUE) {
            ...
            PBMP_ALL_ITER(unit, port) {
                ...
                SOC_IF_ERROR_RETURN(
                    soc_td3_mmu_bmp_reg_pos_get(unit, port, &pipe, &split, &pos));
                SOC_IF_ERROR_RETURN(
                    soc_trident3_xpe_reg_get(unit, hw_res->timer_status[split], -1,
                                             pipe, cos, &status64));
                SOC_IF_ERROR_RETURN(
                ...
                if (COMPILER_64_IS_ZERO(status64) == 0) {
                    ...
                    BCM_IF_ERROR_RETURN(
                        _bcm_td3_pfc_deadlock_recovery_begin(unit, cos, port));
                }
            }
            ...
            if (SOC_PBMP_NOT_NULL(pfc_deadlock_pri_config->deadlock_ports)) {
                BCM_IF_ERROR_RETURN(_bcm_td3_pfc_deadlock_recovery_update(unit, cos));
            }
        }
    }
    return BCM_E_NONE;
                                      ...
                                      uint8 *enable_status)
{
    ...
    SOC_IF_ERROR_RETURN(
        soc_td3_mmu_bmp_reg_pos_get(unit, local_port, &pipe, &split, &pos));
