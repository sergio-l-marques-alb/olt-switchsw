...
int ipmc_flows_ipmc_disabled_l3_mcast_l2_per_rif(int unit, int inP, int outP, int outP2 ,int outP3 ,int vlan1, int vlan2, int vlan3, int vlan4){
    ...
    rv = bcm_multicast_create(unit, BCM_MULTICAST_INGRESS_GROUP | BCM_MULTICAST_TYPE_L3 | BCM_MULTICAST_WITH_ID, &(ipmc_flows_info1.ipmc_mc_groups[0]));
    if (rv != BCM_E_NONE) {
    ...
    rv = bcm_multicast_create(unit, BCM_MULTICAST_INGRESS_GROUP | BCM_MULTICAST_TYPE_L2 | BCM_MULTICAST_WITH_ID, &(ipmc_flows_info1.ipmc_mc_groups[1]));
    if (rv != BCM_E_NONE) {
        printf("Error, bcm_multicast_create \n");
        return rv;
    }
    ...
    rv = bcm_multicast_create(unit, BCM_MULTICAST_INGRESS_GROUP | BCM_MULTICAST_TYPE_L3 | BCM_MULTICAST_WITH_ID, &(ipmc_flows_info1.ipmc_mc_groups[2]));
    if (rv != BCM_E_NONE) {
        printf("Error, bcm_multicast_create \n");
        return rv;
    }
    ...
    rv = add_ingress_multicast_forwarding_with_encap(unit, ipmc_flows_info1.ipmc_mc_groups[0], ipmc_flows_info1.forwarding_ports_1, 3, vlans[0],0);
    if (rv != BCM_E_NONE) {
    ...
    rv = add_ingress_multicast_forwarding_with_encap(unit, ipmc_flows_info1.ipmc_mc_groups[1], ipmc_flows_info1.forwarding_ports_2, 2, vlans[1],0);
    if (rv != BCM_E_NONE) {
        printf("Error, add_ingress_multicast_forwarding_with_encap \n");
        return rv;
    }
...
int ipmc_force_bridge(int unit,int inport, int outP1, int outP2, int outP3 ) {
    ...
    bcm_mac_t MyMac  = {0x00, 0x0c, 0x00, 0x02, 0x00, 0x00};
    int vlan = 400;
    int ipmc_group_start = diag_test_mc(unit,"get_nof_egress_bitmap_groups");
    int rv,i;
    int interfaceID = 50;
    int ports[3];
    int ipmcGroups[3];
    ...
    int flags = BCM_MULTICAST_INGRESS_GROUP | BCM_MULTICAST_WITH_ID | BCM_MULTICAST_TYPE_L3;


    for (i=0;i<3;i++) {
        ipmcGroups[i] = ipmc_group_start + i;
