...
/* $Id: cint_ipmc_example.c,v 1.24 Broadcom SDK $
...
int run_with_prefix_ipv4_mc_example(int unit, int iport, int oport, int sip_prefix_len, int dip_prefix_len, int nof_dest_ports, int ipmc_index, int use_inrif){
    bcm_error_t rv = BCM_E_NONE;
    int ingress_intf;
    ...
    if (use_inrif && sip_prefix_len == 32) {
        rv = add_ip4mc_prefix_entry(unit, mc_ip, src_ip, vlan, mc_id, s_ip_mask, d_ip_mask);
    } else {
        rv = add_ip4mc_prefix_entry(unit, mc_ip, src_ip, 0/*vlan*/, mc_id, s_ip_mask, d_ip_mask);
    }
...
*/
int run_with_ports_ipv6_mc_example(int unit, int iport, int oport){
	/* src: 12CD:0000:0000:0000:0000:0000:0000:0001 */
	/* dst: FF00:0000:0000:0000:0000:0000:0000:0001 */
    ...
    for(port_ndx = 0; port_ndx < nof_dest_ports; ++port_ndx) {
    	...
    	rv = add_ip6mc_entry(unit, mc_ip6, s_ip6, vlan + port_ndx, mc_id, intf.vrf);
    	if (rv != BCM_E_NONE) return rv;
    }
...
/* A example how to use bcm_ipmc_traverse() to get all IPv4 MC entries */
int ipmc_prefixes_test(int unit,int outP)
{
    int rv = BCM_E_NONE;
	int i;
	...
	int vlan =100;
	int ipmcGroup[2] = {33569161,33569162};
	...
	for (i=0;i<2;i++) {
		...
		egress_intf[i].port	    = outP;
		egress_intf[i].intf 	= l3Intf_for_egress[i].l3a_intf_id;
		bcm_l3_egress_create(unit,0,&egress_intf[i],&eg_int[i]);
		if (rv != BCM_E_NONE) {
		    printf("Error, bcm_l3_egress_create %d\n",i);
		    return -1;
		}

		rv = bcm_multicast_create(unit, BCM_MULTICAST_INGRESS_GROUP | BCM_MULTICAST_TYPE_L3 | BCM_MULTICAST_WITH_ID, &ipmcGroup[i]);
		if (rv != BCM_E_NONE) {
			printf("Error, bcm_multicast_create %d\n",i);
			return rv;
		}
		rv = bcm_multicast_ingress_add(unit, ipmcGroup[i], outP, egress_intf[i].encap_id&0x7ffff);
		if (rv != BCM_E_NONE) {
			printf("Error, bcm_multicast_ingress_add %d\n",i);
			return rv;
		}
        bcm_ipmc_addr_t_init(&data);
		data.vrf = vlan;
