...
#define POLAR_QUEUE_DSCP2TC  0x0
#define POLAR_QUEUE_PCP2TC   0x1
#define POLAR_QUEUE_DA2TC    0x2
#define POLAR_QUEUE_PID2TC   0x3

#define POLAR_QUEUE_LEVEL1_ALL_WRR     0x0
#define POLAR_QUEUE_LEVEL1_1_STRICT    0x1
#define POLAR_QUEUE_LEVEL1_2_STRICT    0x2
#define POLAR_QUEUE_LEVEL1_ALL_STRICT  0x3

#define POLAR_QUEUE_LEVEL2_ALL_WRR     0x0
#define POLAR_QUEUE_LEVEL2_1_STRICT    0x1
#define POLAR_QUEUE_LEVEL2_ALL_STRICT  0x2

/*
 *  Function : drv_polar_queue_port_prio_to_queue_set
 *
 *  Purpose :
 ...
 */
int
drv_polar_queue_port_prio_to_queue_set(int unit, uint8 port, uint8 prio, 
    uint8 queue_n)
{
    uint32  reg_value, temp, internal_cpu_port_num;
    ...
    soc_cm_debug(DK_VERBOSE, "drv_polar_queue_port_prio_to_queue_set: \
    ...
    if (IS_GE_PORT(unit, port) && (port == internal_cpu_port_num)) {
...
/*
 *  Function : drv_polar_queue_port_prio_to_queue_get
 *
 *  Purpose :
 ...
 */
int
drv_polar_queue_port_prio_to_queue_get(int unit, uint8 port, uint8 prio, 
    uint8 *queue_n)
{
    uint32  reg_value, temp, internal_cpu_port_num;
    /* Using 32b port variable to compare against SOC_MAX_NUM_PORTS,
    ...
    *queue_n = temp;
    ...
    soc_cm_debug(DK_VERBOSE, "drv_polar_queue_port_prio_to_queue_get: \
        unit %d, port = %d, prio = %d, *queue_n = %d\n", 
        unit, port, prio, *queue_n);
...
/*
 *  Function : drv_polar_queue_mode_set
 *
 *  Purpose :
 ...
 */
int 
drv_polar_queue_mode_set(int unit, soc_pbmp_t bmp, uint32 flag, 
    uint32 mode)
{
    uint32  reg_value, temp, p, internal_cpu_port_num;
    
    soc_cm_debug(DK_VERBOSE, "drv_polar_queue_mode_set: \
    ...
    PBMP_ITER(bmp, p) {
        if (IS_CPU_PORT(unit, p)) {
            SOC_IF_ERROR_RETURN(REG_READ_IMP_QOS_PRI_CTLr
                (unit, &reg_value));
        } else if (p == internal_cpu_port_num) {
        ...
        if (flag & DRV_QUEUE_FLAG_LEVLE2) {
            switch (mode) {
                case DRV_QUEUE_MODE_WRR:
                    temp = POLAR_QUEUE_LEVEL1_ALL_WRR;
                    break;
                case DRV_QUEUE_MODE_1STRICT:
                    temp = POLAR_QUEUE_LEVEL1_1_STRICT;
                    break;
                case DRV_QUEUE_MODE_2STRICT:
                    temp = POLAR_QUEUE_LEVEL1_2_STRICT;
                    break;
                case DRV_QUEUE_MODE_STRICT:
                    temp = POLAR_QUEUE_LEVEL1_ALL_STRICT;
                    break;                
                default:
            ...
            switch (mode) {
                case DRV_QUEUE_MODE_WRR:
                    temp = POLAR_QUEUE_LEVEL2_ALL_WRR;
                    break;
                case DRV_QUEUE_MODE_1STRICT:
                    temp = POLAR_QUEUE_LEVEL2_1_STRICT;
                    break;
                case DRV_QUEUE_MODE_STRICT:
                    temp = POLAR_QUEUE_LEVEL2_ALL_STRICT;
                    break;                
                default:
                    return SOC_E_UNAVAIL;
            }
...
/*
 *  Function : drv_polar_queue_mode_get
 *
 *  Purpose :
 ...
 */  
int 
drv_polar_queue_mode_get(int unit, uint32 port, uint32 flag, 
    uint32 *mode)
{
    uint32  reg_value, temp2 = 0, temp1 = 0, internal_cpu_port_num;
    ...
    if (IS_CPU_PORT(unit, port)) {
        SOC_IF_ERROR_RETURN(REG_READ_IMP_QOS_PRI_CTLr
            (unit, &reg_value));
    } else if (port == internal_cpu_port_num) {
        SOC_IF_ERROR_RETURN(REG_READ_P7_QOS_PRI_CTLr
            (unit, &reg_value));
    } else {
        SOC_IF_ERROR_RETURN(REG_READ_PN_QOS_PRI_CTLr
    ...
    if(((flag & DRV_QUEUE_FLAG_LEVLE2) && 
         (temp1 == POLAR_QUEUE_LEVEL1_ALL_STRICT)) || 
         ((!flag) & (temp2 == POLAR_QUEUE_LEVEL2_ALL_STRICT))) {
    ...
    soc_cm_debug(DK_VERBOSE, "drv_polar_queue_mode_get: \
...
/*
 *  Function : drv_polar_queue_WRR_weight_set
 *
 *  Purpose :
 ...
 */
int 
drv_polar_queue_WRR_weight_set(int unit, uint32 port_type, 
    soc_pbmp_t bmp, uint8 queue, uint32 weight)
{
    uint32  reg_value, p;
    uint32  max_weight = 0, internal_cpu_port_num;

    soc_cm_debug(DK_VERBOSE, "drv_polar_queue_WRR_weight_set: \
...
/*
 *  Function : drv_polar_queue_WRR_weight_get
 *
 *  Purpose :
 ...
 */
int 
drv_polar_queue_WRR_weight_get(int unit, uint32 port_type, 
    uint32 port, uint8 queue, uint32 *weight)
{
    uint32  reg_value, internal_cpu_port_num;
    ...
    soc_cm_debug(DK_VERBOSE, "drv_polar_queue_WRR_weight_get: \
...
/*
 *  Function : drv_polar_queue_prio_set
 *
 *  Purpose :
 ...
 */
int 
drv_polar_queue_prio_set(int unit, uint32 port, uint8 prio, uint8 queue_n)
{
    uint32  p;
    soc_pbmp_t  pbmp;

    soc_cm_debug(DK_VERBOSE, "drv_polar_queue_prio_set: \
...
/*
 *  Function : drv_polar_queue_prio_get
 *
 *  Purpose :
 ...
 */
int 
drv_polar_queue_prio_get(int unit, uint32 port, uint8 prio, uint8 *queue_n)
{
    uint32  p;
    uint8   new_prio = 0;
    ...
    soc_cm_debug(DK_VERBOSE, "drv_polar_queue_prio_get: \
        unit %d, port = %d, priority = %d, queue = %d\n", 
        unit, port, prio, *queue_n);
...
/*
 *  Function : drv_polar_queue_mapping_type_set
 *
 *  Purpose :
 ...
 */
int 
drv_polar_queue_mapping_type_set(int unit, soc_pbmp_t bmp, 
    uint32 mapping_type, uint8 state)
{
    uint32	reg_value;
    uint32	temp, p, temp1, internal_cpu_port_num;

    SOC_IF_ERROR_RETURN(DRV_DEV_PROP_GET
        (unit, DRV_DEV_PROP_INTERNAL_CPU_PORT_NUM, &internal_cpu_port_num));

    soc_cm_debug(DK_VERBOSE, "drv_polar_queue_mapping_type_set: \
    ...
    switch (mapping_type) {
        ...
        case DRV_QUEUE_MAP_DFSV:
            ...
            PBMP_ITER(bmp, p) {
                ...
                if (state) {
                    temp = POLAR_QUEUE_DSCP2TC;
                    temp1 = POLAR_QUEUE_PCP2TC;
                } else {
                    temp = POLAR_QUEUE_PID2TC;
                    temp1 = POLAR_QUEUE_PID2TC;
                }
                SOC_IF_ERROR_RETURN(soc_TC_SEL_TABLEr_field_set
        ...
        case DRV_QUEUE_MAP_PORT:
            /* DRV_QUEUE_MAP_PORT is not needed for Polar.
...
/*
 *  Function : drv_polar_queue_mapping_type_get
 *
 *  Purpose :
 ...
 */
int 
drv_polar_queue_mapping_type_get(int unit, uint32 port, 
    uint32 mapping_type, uint8 *state)
{
    uint32  reg_value;
    uint32  temp, internal_cpu_port_num;
    ...
    switch (mapping_type) {
        ...
        case DRV_QUEUE_MAP_DFSV:
            ...
            if(mapping_type == DRV_QUEUE_MAP_PRIO) {
                if (temp == POLAR_QUEUE_PCP2TC) {
                    *state = TRUE;
                } else {
                    *state = FALSE;
                }
            } else {
                if (temp == POLAR_QUEUE_DSCP2TC) {
                    *state = TRUE;
                } else {
                    *state = FALSE;
                }
    ...
    soc_cm_debug(DK_VERBOSE, "drv_polar_queue_mapping_type_get: \
...
/*
 *  Function : drv_polar_queue_count_set
 *
 *  Purpose :
 ...
 */
int 
drv_polar_queue_count_set(int unit, uint32 port_type, uint8 count)
{
    uint32  sch2_num_cosq;

    soc_cm_debug(DK_VERBOSE, "drv_polar_queue_count_set: \
...
/*
 *  Function : drv_polar_queue_count_get
 *
 *  Purpose :
 ...
 */
int 
drv_polar_queue_count_get(int unit, uint32 port_type, uint8 *count)
{
    uint32  sch2_num_cosq;
    ...
    *count = (uint8) sch2_num_cosq;
    ...
    soc_cm_debug(DK_VERBOSE, "drv_polar_queue_count_get: \
        unit %d, port type = %d, queue count = %d\n", unit, port_type, *count);
