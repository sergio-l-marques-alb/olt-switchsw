                               ...
                               uint32 tab_index, uint32 old_index, int pfx)
{
    ...
    if (mode) {
        ...
        if (entry_data_upr == NULL) {
            soc_cm_debug(DK_ERR, "upper data is NULL for pfx - %d\n", pfx);
            return;
        }
                              ...
                              int pfx, int *key_index)
{
    ...
    if (is_ipv6 == 3) {
        ...
        soc_cm_debug(DK_ERR, "LPM128 hash lookup for pfx - %d failed\n", pfx);
        return SOC_E_PARAM;
    }
...
soc_lpm128_group_index_map_table_dump(int u) 
{
    ...
    for (i = 0; i < defip_table_size; i++) {
        ...
        soc_cm_print("[%05d, %05d]  ", i, SOC_LPM128_INDEX_TO_PFX_GROUP(u, i));
    }
    soc_cm_print("\n");
}
...
void print_pfx_info(int u, soc_lpm128_state_p lpm_state_ptr, int pfx) 
{
    ...
    if (type == socLpmEntryTypeV4) {
        soc_cm_debug(DK_ERR, "------------V4 pfx - %d ---------------------\n", 
                     pfx);
    } else if(type == socLpmEntryType64BV6) {
        soc_cm_debug(DK_ERR, "------------ 64B V6 pfx - %d ----------------\n", 
                     pfx);
    } else {
        soc_cm_debug(DK_ERR, "------------ 128B V6 pfx - %d ---------------\n", 
                     pfx);
    }
    soc_cm_debug(DK_ERR, "Start1: %d End1: %d\n", start1, end1);
    soc_cm_debug(DK_ERR, "Start2: %d End2: %d\n", start2, end2);
    soc_cm_debug(DK_ERR, "Fent: %d Vent: %d\n", fent, vent);
    soc_cm_debug(DK_ERR, "Prev: %d Next: %d\n", prev, next);
    soc_cm_debug(DK_ERR, "------------ END ---------------------\n");
}
...
int test_lpm128_verify_algorithm(int u)
{
    ...
    if (is_reserved) {
        ...
        if (NULL == lpm_state_ptr) {
            soc_cm_debug(DK_ERR, "reserved and unreserved state ptr is NULL\n");
            return SOC_E_MEMORY;
        }
    }

    if (lpm_state_ptr == NULL) {
        soc_cm_debug(DK_ERR, "lpm128 state ptr is NULL, just return\n");
        return SOC_E_MEMORY; 
    }

    pfxmap = sal_alloc(defip_table_size * sizeof(int32), "debug pfxmap");
    if (pfxmap == NULL) {
        return SOC_E_MEMORY;             
    } 
    ...
    while (curr_pfx != -1) {
        ...
        if (end1 == -1) {
            soc_cm_debug(DK_ERR, "pfx: %d end1 is -1\n", curr_pfx);
        ...
        if (start2 != -1) {
            ...
            if (end2 == -1) {
                soc_cm_debug(DK_ERR, "pfx: %d end2 is -1\n", curr_pfx);
                print_pfx_info(u, lpm_state_ptr, curr_pfx);
            }
            ...
            for (i = start2; i <= end_idx; i++) {
                if (pfxmap[i] != -1) {
                    ...
                    soc_cm_debug(DK_ERR, "S2 onflict for pfx: %d with pfx: %d\n",
                                 curr_pfx, pfxmap[i]);
        ...
        for (i = start1; i <= end_idx; i++) {
            ...
            if (pfxmap[i] != -1) {
                /* There is a overwrite */
                soc_cm_debug(DK_ERR, "Conflict for pfx: %d with pfx: %d\n",
                             curr_pfx, pfxmap[i]);
    ...
    for (i = 0; i < is_reserved && max_v6_entries; i++) {
        if (pfxmap[i] != -1) {
            soc_cm_debug(DK_ERR, "overflow into reserved space pfx: %d\n", 
                         pfxmap[i]);
...
dump_lpm128_prefix_info(int u)
{
    ...
    while (lpm_state_ptr && curr_pfx > 0) {
        ...
        soc_cm_print("pfx: %d start1: %d start2: %d end1: %d end2: %d \n",
                     curr_pfx, start1, start2, end1, end2);
        soc_cm_print("vent1: %d fent1: %d  prev: %d next: %d \n",
                     vent1, fent1, prev, next);
        curr_pfx = next;
        total_prefixes += vent1; 
    }
    if (lpm_state_ptr) {
        soc_cm_print("Total number of prefixes - %d\n", total_prefixes);
    }
    ...
    soc_cm_print("reserved prefixes=====================\n");
    ...
    while (lpm_state_ptr && curr_pfx > 0) {
        ...
        soc_cm_print("pfx: %d start1: %d start2: %d end1: %d end2: %d \n",
                     curr_pfx, start1, start2, end1, end2);
        soc_cm_print("vent: %d fent: %d prev: %d next: %d \n",
                     vent1, fent1, prev, next );
        curr_pfx = next;
        total_prefixes += vent1;
    }
                                       ...
                                       soc_lpm128_state_p lpm_state_ptr)
{
    ...
    if (!is_reserved) {
        ...
        if (other_index >= paired_tcam_size) {
            soc_cm_debug(DK_ERR, "finding up index: to_index: %d"\
    ...
    if (type == socLpmEntryTypeV4) {
        ...
        if ((tcam_number & 1) && (start_index % tcam_depth == 0)) {
           ...
           if (start_index < paired_tcam_size) {
            ...
            if (start_index > paired_tcam_size) {
                soc_cm_debug(DK_ERR, "create new group, start_index(%d) "\
                                     ...
                                     int *index)
{
   ...
   if (to_index_tcam_number > to_ent_tcam_number) {
       soc_cm_debug(DK_ERR, "finding up index: lowest v6 is at higher index "\
...
_lpm128_fb_shift_v4_pfx_up(int u, soc_lpm128_state_p lpm_state_ptr, int pfx)
{
    ...
    if (from_ent < 0) {
       soc_cm_debug(DK_ERR, "V4 move up: Invalid start start1: %d pfx: %d\n", 
                    from_ent, pfx);
       return SOC_E_INTERNAL; 
    }
                           ...
                           int *defip_table_size)
{
     ...
     * L3_DEFIP table is not divided into 2 to support URPF.
     */
    if (tcam_pair_count && SOC_URPF_STATUS_GET(unit) &&
        !soc_feature(unit, soc_feature_l3_defip_advanced_lookup)) {
        switch(tcam_pair_count) {
            case 1:
            case 2:
...
soc_lpm_free_64bv6_route_get(int u, int *entries)
{
    ...
    if (soc_feature(u, soc_feature_l3_lpm_scaling_enable)) {
        ...
        paired_v4_count = (paired_v4_count + 1) >> 1;
