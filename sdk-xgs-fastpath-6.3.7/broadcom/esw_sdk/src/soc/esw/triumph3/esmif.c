...
nl_mdio_prbs_chk_err(int unit, uint16 mdio_portid, uint16 crx)
{
    ...
    for(mdio_devid = mdio_devid_min; mdio_devid <= mdio_devid_max; ++mdio_devid) {
        ...
        if((prbs_ctrl & 0x1) == 0x0) {
            SOC_DEBUG_PRINT
                ((DK_WARN, "PRBS is not enabled for NL TCAM mdio_portid=%d\n", mdio_portid));
            return SOC_E_FAIL;
        }
        SOC_IF_ERROR_RETURN
        ...
        if(err_count_l0) {
			SOC_DEBUG_PRINT((DK_WARN, "\t%d PRBS errors seen on lane %d of %s interface\n", err_count_l0, if_lane_base + 0, if_name));
	    }		
        if(err_count_l1) {
    ...
    if(prbs_pass) {
        SOC_DEBUG_PRINT((DK_WARN, "\tPRBS OK \n"));
    }
    _SHR_RETURN(SOC_E_NONE);
}
...
#if 0                           /* Skip for now, since db test fails */
                          ...
                          unsigned *list_cost210)
{
     ...
     * integer value help ?
     */
    soc_cm_debug(DK_VERBOSE, "esm_calc_new_step_size_ps: ");
    soc_cm_debug(DK_VERBOSE, "worst_req_proc_cost_ns=%d", 
                 worst_req_proc_cost_ns);
    soc_cm_debug(DK_VERBOSE, " step_size_ps=%d", step_size_ps);
    soc_cm_debug(DK_VERBOSE, "(num_bits_to_represent_worst_req_proc_cost=8)");
                            ...
                            unsigned percent_hysteresis) 
{
    ...
    soc_cm_debug(DK_VERBOSE, "esm_prog_drop_all_costs: drop_all_cost2=%d, ",
                drop_all_cost2);
    soc_cm_debug(DK_VERBOSE, "drop_all_reset_cost2=%d (is ", drop_all_reset_cost2);
    soc_cm_debug(DK_VERBOSE, "%d %% of drop_all_cost2", percent_hysteresis);


    soc_cm_debug(DK_VERBOSE, "esm_prog_drop_all_costs: drop_all_cost1=%d, ",
                drop_all_cost1);
    soc_cm_debug(DK_VERBOSE, "drop_all_reset_cost1=%d (is ", drop_all_reset_cost1);
    soc_cm_debug(DK_VERBOSE, "%d %% of drop_all_cost1", percent_hysteresis);

    soc_cm_debug(DK_VERBOSE, "esm_prog_drop_all_costs: drop_all_cost0=%d, ",
                drop_all_cost0);
    soc_cm_debug(DK_VERBOSE, "drop_all_reset_cost0=%d (is ", drop_all_reset_cost0);
    soc_cm_debug(DK_VERBOSE, "%d %% of drop_all_cost0", percent_hysteresis);
    return SOC_E_NONE;
}
...
hwtl_kill_endless (int unit) 
{
    ...
    if (!SOC_SUCCESS(rc)) {
        ...
        soc_cm_debug(DK_ERR, "unit: %d HWTL did not complete, will not ", unit);
        soc_cm_debug(DK_ERR, "verify packet, byte counters\n");
        return rc;
    } else {
        ...
        if (!SOC_SUCCESS(status)) {
            soc_cm_debug(DK_ERR,"unit %d: HWTL test PASSED (completed without",
                         unit);
            soc_cm_debug(DK_ERR, "mismatches), but packet, byte counters");
            soc_cm_debug(DK_ERR, "check FAILED\n");
	    } else {
...
*/
        soc_cm_debug(DK_VERBOSE, "unit %d: WARNING: will not check for hwtl_done,", unit);
        soc_cm_debug(DK_VERBOSE, "will not verify packet, byte counters");
        soc_cm_debug(DK_VERBOSE, "(ENDLESS=1), HWTL is still running\n");
        return SOC_E_NONE;
    } else {
        /* wait for hwtl_done */
        rc = hwtl_poll_done(unit);
        if (rc != SOC_E_NONE) {
            /* hwtl did not complete */
            soc_cm_debug(DK_ERR, "unit %d: HWTL did not complete !!, will not",
                         unit);
            soc_cm_debug(DK_ERR, "verify packet, byte counters");
	        return rc;
        } else {
            ...
            unsigned skip_exp_txrx_pbc_chk;
            /* hwtl had completed without mismatch */
            soc_cm_debug(DK_VERBOSE, "unit %d: HWTL PASSED (completed", unit);
            soc_cm_debug(DK_VERBOSE, "without mismatch), will now verify ");
            soc_cm_debug(DK_VERBOSE, "packet, byte counters\n");
	        ...
	        if (rc != SOC_E_NONE) {
                soc_cm_debug(DK_ERR, "unit: %d HWTL PASSED (completed without ",
                             unit);
                soc_cm_debug(DK_ERR, "mismatches), but packet, byte counters ");
                soc_cm_debug(DK_ERR, "check FAILED !!");
	        } else {
                soc_cm_debug(DK_VERBOSE, "unit: %d Packet, byte counters check",
                            unit);
                soc_cm_debug(DK_VERBOSE, " also PASSED");
	        }
	        return rc;
        } 
   }
                               ...
                               unsigned *latency_ns)
{
    ...
    soc_cm_debug(DK_VERBOSE,  "test_runtime_seconds=%d\n",test_runtime_seconds);
    ...
    if (failed) {
        ...
        *latency_ns = esmif_clk_period_ns * esmif_max_latency_recorded;
        ...
        if (esmif_max_latency_recorded > ESMIF_MAX_FIFO_DEPTH) {
         soc_cm_debug(DK_ERR, "unit %d: ESMIF.MAX_LATENCY_RECORDED = ", unit);
         soc_cm_debug(DK_ERR, "%d !! - number ", esmif_max_latency_recorded);
         soc_cm_debug(DK_ERR, "greater than %d means ", ESMIF_MAX_FIFO_DEPTH );
         soc_cm_debug(DK_ERR, "catastrophy !!\n");
         return SOC_E_FAIL; 
        } else if (esmif_max_latency_recorded > ESMIF_ALLOWED_FIFO_DEPTH) {
         soc_cm_debug(DK_VERBOSE, "unit %d: ESMIF.MAX_LATENCY_RECORDED = ",
                      unit);
         soc_cm_debug(DK_VERBOSE, "%d !! - which ",esmif_max_latency_recorded );
         soc_cm_debug(DK_VERBOSE, "is greater than budgeted %d !!", 
         ...
         soc_cm_debug(DK_VERBOSE,  "ESMIF.MAX_LATENCY_RECORDED = %d ", 
                      esmif_max_latency_recorded);
         soc_cm_debug(DK_VERBOSE, "(num_esmif_clks) @ %d MHz, this ",
                      esmif_clk_freq_khz(unit)/1000);
         soc_cm_debug(DK_VERBOSE, "corresponds \n to latency of %d nS\n", 
                      *latency_ns);
         return SOC_E_NONE;;
        }
    }
...
#if 0                           /* Skip for now, since it fails */
...
*/
    ...
    if (llength != 3) {
        soc_cm_debug(DK_ERR, "unit %d: %s: ptr_fme_req=%d, could not",
                     unit, proc_name, ptr_fme_req);
        soc_cm_debug(DK_ERR, "compute cost in terms of num_rsp_words,");
        soc_cm_debug(DK_ERR, "num_req_words, num_nl_cclks\n");
        /* failure */
        return SOC_E_FAIL; 
    }
    cost2_num_rsp_words = list_cost210[0]; 
    ...
    if (num_rsp_words) {
        ...
        soc_cm_debug(DK_ERR, "unit %d: %s: ptr_fme_req=%d, num_rsp_words",
                     unit, proc_name, ptr_fme_req);
        soc_cm_debug(DK_ERR, "must be non-zero, exitting\n");
    ...
    if (num_req_words) {
        ...
        soc_cm_debug(DK_ERR, "unit %d: %s: ptr_fme_req=%d, num_req_words",
                     unit, proc_name, ptr_fme_req);
        soc_cm_debug(DK_ERR, "must be non-zero, exitting\n");
        return SOC_E_FAIL; /* failed */
    }
    ...
    if (num_nl_cclks) {
        ...
        soc_cm_debug(DK_VERBOSE, "unit %d: %s: ptr_fme_req=%d, num_nl_cclks",
                     unit, proc_name, ptr_fme_req);
        soc_cm_debug(DK_VERBOSE, "must be non-zero, exitting\n");
        return SOC_E_FAIL; /* failed */
    }
                       ...
                       unsigned step_size_ps_x) 
{
    ...
    if (llength != 3) {
        soc_cm_debug(DK_ERR, "unit %d: esm_prog_new_fme_costs: will not ",
                             unit);
        soc_cm_debug(DK_ERR, "to program costs in FIELDMAPPER table");
        return SOC_E_FAIL;
    }
    ...
    soc_cm_debug(DK_VERBOSE, "unit %d: esm_prog_new_fme_costs: ptr_fme_req=%d",
                              unit, ptr_fme_req);
    soc_cm_debug(DK_VERBOSE, "fme_cost2=%d, fme_cost1=%d, fme_cost0=%d\n",
...
*/
    if (*idle_req_rsp_latency_ns_x == 0) {
        ...
        SOC_IF_ERROR_RETURN(rc);
        soc_cm_print("config add idle_req_rsp_latency_ns.%d=%d\n", 
                        unit, *idle_req_rsp_latency_ns_x);
    }
    if (*idle_req_rsp_latency_ns_x == 0) {
        soc_cm_debug(DK_ERR, "unit %d: ptr_fme_req=%d: idle_req_rsp_latency",
                     unit, idle_req_rsp_ptr_fme_req);
        soc_cm_debug(DK_ERR, "measurement FAILED, will quit\n");
        return SOC_E_FAIL;
    }
...
{
    const unsigned max_init1_attempts = 5,   /* Check this */
        en_nl_parity_scan  = 1,
...
esm_chk_giol(int unit)
{
    ...
    if (giol0) {
       soc_cm_debug(DK_ERR, "GIOL0 is asserted on unit %d!.\n", unit);
    }
    if (giol1) {
       soc_cm_debug(DK_ERR, "GIOL1 is asserted on unit %d!.\n", unit);
    }
    return giol0 | giol1;
}  
...
nl_clear_giol(int unit, unsigned num_nl)
{
    ...
    for (dev_id = 0; dev_id < num_nl; ++dev_id) {
        ...
        if (nl_chk_err_status_reg(unit, dev_id)) {
            soc_cm_debug(DK_ERR, "Failed to clear the NL11K error register on "
                             "device %d on unit %d.\n", dev_id, unit);
        }
    }
    ...
    for (dev_id = 0; dev_id < num_nl; ++dev_id) {
        ...
        if (rv < SOC_E_NONE) {
            soc_cm_debug(DK_ERR, "Status check failed on NL11K "
                             "device %d on unit %d.\n", dev_id, unit);
        }
    }
    
    if (esm_chk_giol(unit)) {
        soc_cm_debug(DK_ERR, "Failed to clear GIOL status on unit %d.\n", 
                              unit);
        return SOC_E_FAIL;
    }
    return SOC_E_NONE;
...
esm_err_recovery_seq(int unit, unsigned num_nl)
{
    ...
    if (ipipe_req_count0 != ipipe_req_count1) {
        soc_cm_debug(DK_ERR, "Not successful in stopping Esmif lookup "
                             "requests to Etu on unit %d.\n", unit);
        error++;
    }
    if (COMPILER_64_NE(tx_pkt_count0, tx_pkt_count1)) {
        soc_cm_debug(DK_ERR, "Not successful in stopping ETU requests "
                             "to NL11K on unit %d.\n", unit);
        error++;
    }
    if (COMPILER_64_NE(rx_pkt_count0, rx_pkt_count1)) {
        soc_cm_debug(DK_ERR, "Not successful in stopping NL11K responses "
                             "to ETU on unit %d.\n", unit);
        error++;
    }  
    if (ipipe_rsp_count0 != ipipe_rsp_count1) {
        soc_cm_debug(DK_ERR, "Not successful in stopping Etu responses to "
                             "to Esmif on unit %d.\n", unit);
        error++;
    }
    ...
    if (chk_fifos(unit)) {
        soc_cm_debug(DK_ERR, "ESM FIFO check failed on unit %d.\n", unit);
        error++;
    }
    if (chk_etu_counters(unit)) {
        soc_cm_debug(DK_ERR, "ESM counter check failed on unit %d.\n", unit);
        error++;
    }
    if (chk_all_intr_sts(unit, num_nl)) {
        soc_cm_debug(DK_ERR, "ESM interrupt check failed on unit %d.\n", unit);
        error++;
    }    
    if (chk_esmif_adc(unit)) {
        soc_cm_debug(DK_ERR, "ESM ADC check failed on unit %d.\n", unit);
        error++;
    }
    for (dev_id = 0; dev_id < num_nl; ++dev_id) {
        if (nl_chk_errors(unit, dev_id)) {
            soc_cm_debug(DK_ERR, "Failed to clear errors on NL11K "
                                 "device %d on unit %d.\n", dev_id, unit);
            error++;
        }
    }
...
soc_tr3_esm_interrupt_process(int unit)
{
    ...
    if (etu_chk_errors(unit)) {
        count++;
        soc_cm_debug(DK_ERR, "Recovering from ETU alignment error on "
                             "unit %d (incident #%d).\n", unit, count);
        rv = esm_err_recovery_seq(unit, num_nl);
        if (rv < SOC_E_NONE) {
            soc_cm_debug(DK_ERR, "ETU alignment recovery failed on "
                                 "unit %d.\n", unit);
        }
    }
    return;
...
soc_tr3_esm_recovery_start(int unit)
{
    ...
    if (soc->esm_recovery_pid == SAL_THREAD_ERROR) {
        soc_cm_debug(DK_ERR, "soc_tr3_esm_recovery_start: Could not start esm recovery thread\n");
	      return SOC_E_MEMORY;
    }
