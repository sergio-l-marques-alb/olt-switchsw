...
soc_bcm56830_mem_config(int unit)
{
    ...
    index_max = 8191;
    sop->memState[L2MCm].index_max = index_max;
...
soc_trident2_mem_config(int unit)
{
    ...
    if ((dev_id == BCM56830_DEVICE_ID) || (dev_id == BCM56834_DEVICE_ID)) { 
        l2_entries = 160 * 1024;
    }
...
soc_trident2_l3_defip_index_map(int unit, soc_mem_t mem, int index)
{
    ...
    if (alpm_mode == 1) {
        ...
        if (index < soc_mem_index_count(unit, mem)/2) {
            ...
            /* make index relative to upper half */
            index -= (soc_mem_index_count(unit, mem)/2);
            return index + 2 * _TD2_INDEX_2K_ENTRIES + 
                   defip128_tbl_sz;
       }
    }
...
soc_trident2_port_config_init(int unit, uint16 dev_id, uint8 rev_id)
{
    ...
    frequency = rev_id == BCM56850_A0_REV_ID ? 415 : frequency;
    ...
    if ((si->io_bandwidth > si->bandwidth) || (BCM56855_DEVICE_ID == dev_id)) {
        oversub_pbm =
    ...
    for (port = 0; port < num_port; port++) {
        ...
        /* Logical ports 73-86 of 56855 are initialized as loopback ports */
        if ((BCM56855_DEVICE_ID == dev_id) 
            && (port > SOC_TD2_56855_LOOPBACK_HIGIG_PORT_START - 1)) { 
            break;
        }
    ...
    /* Logical ports 73-86 of 56855 are initialized as loopback ports */
    port = SOC_TD2_56855_LOOPBACK_HIGIG_PORT_START;
    if (BCM56855_DEVICE_ID == dev_id){ 
        for (tsc_id = 0; tsc_id < _TD2_XLPS_PER_PGW * _TD2_PGWS_PER_DEV; 
            tsc_id++) {
            if (!(tsc_map & (1 << tsc_id))) {              
                phy_port = tsc_id * _TD2_PORTS_PER_XLP + 1;                

                if (phy_port > (num_phy_port - 2)){
                    break;
                }
                ...
                /* The port speed of lb port is 40Gbps*/
                port_bandwidth = 40 * 1000; 
                                 ...
                                 int enable, uint8 flag)
{
    ...
    if (!enable) { 
        ...
        if (*thdo_drop_bmp == 0) {
            ...
            if (SOC_FAILURE(rv)) {
                soc_cm_debug(DK_ERR,"Clear the registers fail.");
                FWD_CTRL_UNLOCK(unit);
                return rv;
            }
        }
        ...
        *thdo_drop_bmp |= flag;
        ...
        rv = _soc_td2_thdo_hw_set(unit, port, enable);
        if (SOC_FAILURE(rv)) {
            soc_cm_debug(DK_ERR,"Set the registers fail.");
            FWD_CTRL_UNLOCK(unit);
            return rv;
        }
