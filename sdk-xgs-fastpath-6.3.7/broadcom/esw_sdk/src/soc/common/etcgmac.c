/*
 * $Id: etcgmac.c 1.12 Broadcom SDK $
...
static char* chipdumpregs(ch_t *ch, gmac0regs_t *regs, char *b);
static void gmac_mf_cleanup(ch_t *ch);
static int gmac_speed(ch_t *ch, uint32 speed);
static void gmac_miiconfig(ch_t *ch);

#define BCM53001_ROBO_MII_CORE_UNIT    0
#define BCM53001_ROBO_MDC_PORT         0
#define BCM53001_ROBO_PHY_ADDR         EPHY_NOREG


static char *cpu_mac = "00-10-18-53-00-01";

struct chops bcmgmac_et_soc_chops = {
    chipid,
    chipattach,
    chipdetach,
    chipreset,
...
chipattach(etc_soc_info_t *etc, void *dev, void *regsva)
{
    ...
    if (ch->sih->chiprev == GMAC_VOYAGER_REVISION_ID) {
        /* using GMAC 1 */
        /* The Voyager is connected with GMAC core 1 */
        coreunit = 1;
    } else {
        /* using GMAC 0 */
        coreunit = 0;
    ...
    if ((regs = (gmac0regs_t *)ai_soc_setcore(ch->sih, GMAC_CORE_ID, coreunit)) == NULL) {
    ...
    etc->phyaddr = BCM53001_ROBO_PHY_ADDR; /* CHECK ME */
    /* reset the gmac core */
    chipreset(ch);
    ...
    {                
        ...
        /* TX: TC_BK, RX: RX_Q0 */
        ch->di[0] = dma_soc_attach(etc->et, name, 0, ch->dev, 1,
        ...
        /* TX: TC_BE, RX: RX_Q1 */
        ch->di[1] = dma_soc_attach(etc->et, name, 1, ch->dev, 1,
        ...
        /* TX: TC_CL, RX: RX_Q2 */
        ch->di[2] = dma_soc_attach(etc->et, name, 2, ch->dev, 1,
                               ...
                               etc->en_rxsephdr[2]);
        /* TX: TC_VO, RX: RX_Q3 */
        ch->di[3] = dma_soc_attach(etc->et, name, 3, ch->dev, 1,
        ...
        for (i = 0; i < NUMTXQ; i++)
            if (ch->di[i] != NULL)
                etc->txavail[i] = (uint*)dma_soc_getvar(ch->di[i], "&txavail");
    ...
    etc->mdcport = BCM53001_ROBO_MDC_PORT;
...
chipdetach(ch_t *ch)
{
    ...
    /* put the keystone gmac0 reset will cause gmac common in reset state
       before the Voyager CSS version.
       Skip the reset step for those before the voyager CSS version.*/
    if (ch->sih->chiprev == GMAC_VOYAGER_REVISION_ID) {  
        if (ch->sih){
...
static char*
chipdumpregs(ch_t *ch, gmac0regs_t *regs, char *buf)
{
    uint phyaddr;
    ...
    /* mib registers */
    PRREG(tx_good_octets); PRREG(tx_good_octets_high); PRREG(tx_good_pkts);
    buf += sprintf(buf, "\n");
    PRREG(tx_octets); PRREG(tx_octets_high); PRREG(tx_pkts);
    buf += sprintf(buf, "\n");
    PRREG(tx_broadcast_pkts); PRREG(tx_multicast_pkts); PRREG(tx_uni_pkts);
    buf += sprintf(buf, "\n");
    PRREG(tx_len_64); PRREG(tx_len_65_to_127); PRREG(tx_len_128_to_255);
    buf += sprintf(buf, "\n");
    PRREG(tx_len_256_to_511); PRREG(tx_len_512_to_1023); PRREG(tx_len_1024_to_max);
    buf += sprintf(buf, "\n");
    PRREG(tx_len_max_to_jumbo); PRREG(tx_jabber_pkts); PRREG(tx_oversize_pkts);
    buf += sprintf(buf, "\n");
    PRREG(tx_fragment_pkts); PRREG(tx_underruns); PRREG(tx_total_cols);
    buf += sprintf(buf, "\n");
    PRREG(tx_single_cols); PRREG(tx_mulitple_cols); PRREG(tx_excessive_cols);
    buf += sprintf(buf, "\n");
    PRREG(tx_late_cols); PRREG(tx_defered); PRREG(tx_pause_pkts);
    buf += sprintf(buf, "\n");

    PRREG(rx_good_octets); PRREG(rx_good_octets_high); PRREG(rx_good_pkts);
    buf += sprintf(buf, "\n");
    PRREG(rx_octets); PRREG(rx_octets_high); PRREG(rx_pkts);
    buf += sprintf(buf, "\n");
    PRREG(rx_broadcast_pkts); PRREG(rx_multicast_pkts); PRREG(rx_uni_pkts);
    buf += sprintf(buf, "\n");
    PRREG(rx_len_64); PRREG(rx_len_65_to_127); PRREG(rx_len_128_to_255);
    buf += sprintf(buf, "\n");
    PRREG(rx_len_256_to_511); PRREG(rx_len_512_to_1023); PRREG(rx_len_1024_to_max);
    buf += sprintf(buf, "\n");
    PRREG(rx_len_max_to_jumbo); PRREG(rx_jabber_pkts); PRREG(rx_oversize_pkts);
    buf += sprintf(buf, "\n");
    PRREG(rx_fragment_pkts); PRREG(rx_missed_pkts); PRREG(rx_undersize);
    buf += sprintf(buf, "\n");
    PRREG(rx_crc_errs); PRREG(rx_align_errs); PRREG(rx_symbol_errs);
    buf += sprintf(buf, "\n");
    PRREG(rx_pause_pkts); PRREG(rx_nonpause_pkts); PRREG(rxq0_irc_drop);
    buf += sprintf(buf, "\n");
    PRREG(rxq1_irc_drop); PRREG(rxq2_irc_drop); PRREG(rxq3_irc_drop);
    buf += sprintf(buf, "\n");
    PRREG(rx_cfp_drop);
    buf += sprintf(buf, "\n");
...
gmac_clearmib(ch_t *ch)
{
    ...
    OR_REG(ch->dev, &ch->regs->devcontrol, DEVCONTROL_MIB_RESET_ON_READ_MASK);
...
{    
    OR_REG(ch->dev, &ch->regs->command_config, COMMAND_CONFIG_SW_RESET_MASK);
...
{
    AND_REG(ch->dev, &ch->regs->command_config, ~COMMAND_CONFIG_SW_RESET_MASK);
    OSL_DELAY(GMAC_RESET_DELAY);
}
...
gmac_reset(ch_t *ch)
{
    ...
    ocmdcfg = cmdcfg = R_REG(ch->dev, &ch->regs->command_config);
    ...
    cmdcfg &= ~(COMMAND_CONFIG_TX_ENA_MASK | COMMAND_CONFIG_RX_ENA_MASK | 
        ...
        COMMAND_CONFIG_ENA_EXT_CONFIG_MASK);
    ...
    if (cmdcfg != ocmdcfg)
        W_REG(ch->dev, &ch->regs->command_config, cmdcfg);
...
gmac_promisc(ch_t *ch, bool mode)
{
    ...
    cmdcfg = R_REG(ch->dev, &ch->regs->command_config);
#endif /* GMAC_PROMISC_BY_CFP */    
    ...
    switch (mode) {
...
#endif
            {
                dma_soc_fifoloopbackenable(ch->di[TX_Q0], TRUE);
            }
            break;
        ...
        case LOOPBACK_MODE_NONE:
...
#endif
            {
                ...
                dma_soc_fifoloopbackenable(ch->di[TX_Q0], FALSE);
            }
            break;
...
gmac_enable(ch_t *ch)
{
    ...
    W_REG(ch->dev, &regs->command_config, cmdcfg);
    ...
    /* request ht clock */
    W_REG(ch->dev, &regs->clk_ctl_st, 0x2);

    /* init the mac data period to 9. this is the optimal value for
     * gmac to work at all backplane clock speeds
     */
    rxqctl = R_REG(ch->dev, &regs->rxqctl);
    rxqctl &= ~RXQCTL_RESERVED0_MASK;
    W_REG(ch->dev, &regs->rxqctl, rxqctl | (RC_MAC_DATA_PERIOD << RXQCTL_RESERVED0_SHIFT));
...
gmac_miiconfig(ch_t *ch)
{
     ...
     */
    devstatus = R_REG(ch->dev, &regs->devstatus);
    mode = ((devstatus & 
...
chipinreset:
    ...
    /* Drive mdc clock */
    origidx = ai_soc_coreidx(ch->sih);
    com_regs = ai_soc_setcore(ch->sih, GMAC_COM_CORE_ID, 0);
    ASSERT(regs != NULL);
    OR_REG(ch->dev, &com_regs->phycontrol, PHYCONTROL_MDC_TRANSITION_EN_MASK);
    ai_soc_setcoreidx(ch->sih, origidx);
...
chipinit(ch_t *ch, bool full)
{
    ...
    W_REG(ch->dev, &regs->intrecvlazy, (1 << INTRECVLAZY_FRAME_COUNT_SHIFT));
    ...
    {
        for (i = 0; i < NUMRXQ; i++) {
            dma_soc_rxsephdrctrl(ch->di[i], etc->en_rxsephdr[i], 
    ...
    /* Set TX IPG length */
    W_REG(ch->dev, &regs->tx_ipg_length, 12);

    if (!etc->promisc) {
        /* set our local address */
        W_REG(ch->dev, &regs->mac_0,
              hton32(*(uint32 *)&etc->cur_etheraddr.octet[0]));
        W_REG(ch->dev, &regs->mac_1,
    ...
    W_REG(ch->dev, &regs->frm_length, ETHER_MAX_LEN + 32);
...
chiprx(ch_t *ch)
{
     ...
     */
    for (i = 0; i < NUMRXQ; i++) {
        while ((p = dma_soc_rx(ch->di[NUMRXQ - i - 1])) != NULL) {
            /* skip the rx header */
    ...
    ch->intstatus &= ~(INTMASK_RCVINTERRUPT_0_MASK | INTMASK_RCVINTERRUPT_1_MASK |
...
chipgetintrevents(ch_t *ch)
{
    ...
    intstatus = R_REG(ch->dev, &ch->regs->intstatus);
...
chipstatsupd(ch_t *ch)
{
     ...
     * counters.
     */
    OR_REG(ch->dev, &regs->devcontrol, DEVCONTROL_MIB_RESET_ON_READ_MASK);
...
chipdumpmib(ch_t *ch, char *buf)
{
    ...
    buf += sprintf((char *) buf, "tx_broadcast_pkts %d tx_multicast_pkts %d \
        tx_uni_pkts %d\n",
        m->tx_broadcast_pkts, m->tx_multicast_pkts, m->tx_uni_pkts);
    ...
    buf += sprintf((char *) buf, "tx_len_max_to_jumbo %d\n",
        m->tx_len_max_to_jumbo);
    ...
    buf += sprintf((char *) buf, "tx_defered %d tx_pause_pkts %d\n",
        m->tx_defered, m->tx_pause_pkts);

    buf += sprintf((char *) buf, "rx_good_octets %d rx_good_octets_high %d \
        rx_good_pkts %d\n",
        m->rx_good_octets, m->rx_good_octets_high, m->rx_good_pkts);
    
    buf += sprintf((char *) buf, "rx_octets %d rx_octets_high %d \
        rx_pkts %d\n",
        m->rx_octets, m->rx_octets_high, m->rx_pkts);
        
    buf += sprintf((char *) buf, "rx_broadcast_pkts %d rx_multicast_pkts %d \
        rx_uni_pkts %d\n",
        m->rx_broadcast_pkts, m->rx_multicast_pkts, m->rx_uni_pkts);
    ...
    buf += sprintf((char *) buf, "rx_len_max_to_jumbo %d\n",
        m->rx_len_max_to_jumbo);
    
    buf += sprintf((char *) buf, "rx_jabber_pkts %d rx_oversize_pkts %d \
        rx_fragment_pkts %d\n",
        m->rx_jabber_pkts, m->rx_oversize_pkts, m->rx_fragment_pkts);
    
    buf += sprintf((char *) buf, "rx_missed_pkts %d rx_undersize %d\n",
        m->rx_missed_pkts, m->rx_undersize);
    ...
    buf += sprintf((char *) buf, "rxq0_irc_drop %d rxq1_irc_drop %d \
        rxq2_irc_drop %d\n",
        m->rxq0_irc_drop, m->rxq1_irc_drop, m->rxq2_irc_drop);
    
    buf += sprintf((char *) buf, "rxq3_irc_drop %d rx_cfp_drop %d\n",
        m->rxq3_irc_drop, m->rx_cfp_drop);
...
chipphyrd(ch_t *ch, uint phyaddr, uint reg)
{
    ...
    origidx = ai_soc_coreidx(ch->sih);
    regs = ai_soc_setcore(ch->sih, GMAC_COM_CORE_ID, 0); 
    ASSERT(regs != NULL);

    /* set phyaccess for read/write */
    W_REG(ch->dev, &regs->phyaccess,
          (PHYACCESS_TRIGGER_MASK | (phyaddr << PHYACCESS_CPU_PHY_ADDR_SHIFT) | 
          (reg << PHYACCESS_CPU_REG_ADDR_SHIFT)));

    /* wait for it to complete */
    SPINWAIT((R_REG(ch->dev, &regs->phyaccess) & PHYACCESS_TRIGGER_MASK), 1000);
    tmp = R_REG(ch->dev, &regs->phyaccess);
...
chipphywr(ch_t *ch, uint phyaddr, uint reg, uint16 v)
{
    ...
    /* wait for it to complete */
    SPINWAIT((R_REG(ch->dev, &regs->phyaccess) & PHYACCESS_TRIGGER_MASK), 1000);
    if (R_REG(ch->dev, &regs->phyaccess) & PHYACCESS_TRIGGER_MASK) {
...
{
    dma_soc_rxreset(ch->di[id]);
    dma_soc_rxreclaim(ch->di[id]);
}

static void 
chiprxinit(ch_t *ch, int id)
{
    if(!dma_soc_rxenabled(ch->di[id])) {
        dma_soc_rxinit(ch->di[id]);
    }
}
...
chipcfprd(ch_t *ch, void *arg)
{
    ...
    /* Read tcam */
    tmp = R_REG(ch->dev, &regs->cfp_access);
    ...
    W_REG(ch->dev, &regs->cfp_access, tmp);
    ...
    for (i=0; i < CFP_TCAM_ENTRY_WORD; i++) {
        buf->cfp_entry.tcam[i] = R_REG(ch->dev, (&(regs->cfp_tcam_data0) + i ));
        buf->cfp_entry.tcam_mask[i] = R_REG(ch->dev, (&(regs->cfp_tcam_mask0) + i ));
    }
    ...
    buf->cfp_entry.action= R_REG(ch->dev, &regs->cfp_action_data);
...
chipcfpwr(ch_t *ch, void *arg)
{
    ...
    for (i=0; i < CFP_TCAM_ENTRY_WORD; i++) {
        W_REG(ch->dev, (&(regs->cfp_tcam_data0) + i), buf->cfp_entry.tcam[i]);
        W_REG(ch->dev, (&(regs->cfp_tcam_mask0) + i), buf->cfp_entry.tcam_mask[i]);
    }
    ...
    /* Write Action ram */
    W_REG(ch->dev, &regs->cfp_action_data, buf->cfp_entry.action);
...
chipcfpudfrd(ch_t *ch, void *arg)
{
    ...
    switch (buf->l3_framing) {
        case CFP_L3_FRAMING_IPV4:
            ...
            reg_off = (uint32)(&regs->udf_0_a3_a0 + (buf->slice_id * 4) + \
                (buf->field_idx /4));
            tmp = R_REG(ch->dev, reg_off);
        ...
        case CFP_L3_FRAMING_IPV6:
            ...
            if (buf->slice_id == CFP_IPV6_MAX_SLICE_ID) {
                reg_off = (uint32)(&regs->udf_0_d3_d0 + (buf->slice_id * 4) + \
                    (buf->field_idx /4));
            } else {
                reg_off = (uint32)(&regs->udf_0_b3_b0 + (buf->slice_id * 4) + \
                    (buf->field_idx /4));
            }
            tmp = R_REG(ch->dev, reg_off);
            shift = (buf->field_idx % 4) * 8;
        ...
        case CFP_L3_FRAMING_NONIP:
            ...
            reg_off = (uint32)(&regs->udf_0_c3_c0 + (buf->slice_id * 4) + \
                (buf->field_idx /4));
            tmp = R_REG(ch->dev, reg_off);
            shift = (buf->field_idx % 4) * 8;
            buf->field_value = (tmp >> shift) & CFP_UDF_OFFSET_BASE_VALUE_MASK;
...
chipcfpudfwr(ch_t *ch, void *arg)
{
    ...
    switch (buf->l3_framing) {
        case CFP_L3_FRAMING_IPV4:
            ...
            tmp &= ~mask;
            tmp |= (((buf->field_value/2) | buf->flags) & \
                UDF_0_A3_A0_CFG_UDF_0_A0_MASK) << shift;
            W_REG(ch->dev, reg_off, tmp);
            break;
        case CFP_L3_FRAMING_IPV6:
            /* slice id 3 means Chain slice */
            if (buf->slice_id > CFP_IPV6_MAX_SLICE_ID) { 
            ...
            if (buf->slice_id == CFP_IPV6_MAX_SLICE_ID) {
                reg_off =  (uint32)(&regs->udf_0_d3_d0 + \
                    (buf->slice_id * 4) + (buf->field_idx /4));
            } else {
                reg_off =  (uint32)(&regs->udf_0_b3_b0 + \
                    (buf->slice_id * 4) + (buf->field_idx /4));
            }
...
static int chiprxrateset(ch_t *ch, uint channel, uint pps)
{
    ...
    reg_val = R_REG(ch->dev, &regs->irc_cfg);
    ...
    if (tmp != map_val) {
        ...
        reg_val |= ((map_val & IRC_CFG_IRC0_IDX_MASK) << 
            (channel * IRC_CFG_IRC1_IDX_SHIFT));
        W_REG(ch->dev, &regs->irc_cfg, reg_val);
    }
...
static int chiptxrateset(ch_t *ch, uint channel, uint rate, uint burst)
{
    ...
    reg_val = R_REG(ch->dev, &regs->erc0_cfg + channel);
    if (rate){
        /* Set enable bit */
    ...
    W_REG(ch->dev, &regs->erc0_cfg + channel, reg_val);
...
static int chipflowctrlmodeset(ch_t *ch, uint mode)
{
    ...
    reg_val = R_REG(ch->dev, &regs->devcontrol);
    ...
    W_REG(ch->dev, &regs->devcontrol, reg_val);
...
static int chipflowctrlautoset(ch_t *ch, uint on_th, uint off_th)
{
    ...
    reg_val = R_REG(ch->dev, &regs->flowctlthresh);
    ...
    W_REG(ch->dev, &regs->flowctlthresh, reg_val);
...
static int chipflowctrlrxchanset(ch_t *ch, uint chan, uint on_th, uint off_th)
{
    ...
    reg_val = R_REG(ch->dev, &regs->rx_ch0_flow_ctrl + chan);
    ...
    W_REG(ch->dev, &regs->rx_ch0_flow_ctrl + chan, reg_val);
...
chiptpidset(ch_t *ch, uint index, uint tpid)
{
    ...
    /* Set TPID */
    reg_val = R_REG(ch->dev, &regs->stag0 + index);
    reg_val &= ~STAG0_TPID_MASK;
    reg_val |= tpid & STAG0_TPID_MASK;
    W_REG(ch->dev, &regs->stag0 + index, reg_val);
...
static int chiptxqosmodeset(ch_t *ch, uint mode)
{
    ...
    reg_val = R_REG(ch->dev, &regs->txqos);
    ...
    W_REG(ch->dev, &regs->txqos, reg_val);
