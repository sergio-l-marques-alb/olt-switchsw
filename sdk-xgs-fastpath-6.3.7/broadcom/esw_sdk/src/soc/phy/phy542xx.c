...
{
    PHY_BCM542XX_EN_RDB_MODE(unit, pc);   /* set RDB register access mode */
    reg_addr &= PHY_BCM542XX_RDB_ADDR_REG_ADDR;

    SOC_IF_ERROR_RETURN(    /* MDIO write to reg 0x1E[15:0] for the address */
        PHY_BCM542XX_WRITE_PHY_REG(unit, pc,
                                   PHY_BCM542XX_RDB_ADDR_REG_OFFSET, reg_addr));

    SOC_IF_ERROR_RETURN(    /* MDIO read from reg 0x1F[15:0] for the data */
        PHY_BCM542XX_READ_PHY_REG(unit, pc,
                                  PHY_BCM542XX_RDB_DATA_REG_OFFSET, data ));
    PHY_BCM542XX_DIS_RDB_MODE(unit, pc);   /* legacy register access mode */
    return SOC_E_NONE;
}
...
int phy_bcm542xx_direct_reg_write(int unit, phy_ctrl_t *pc, uint16 reg_addr, uint16 data)
{
    ...
    SOC_IF_ERROR_RETURN(    /* MDIO write to reg 0x1F[15:0] for the data */
        PHY_BCM542XX_WRITE_PHY_REG(unit, pc,
                                   PHY_BCM542XX_RDB_DATA_REG_OFFSET, data ));
                                   ...
                                   uint16 data, uint16 mask)
{
    ...
    SOC_IF_ERROR_RETURN(    /* MDIO write to reg 0x1E[15:0] for the address */
        PHY_BCM542XX_WRITE_PHY_REG(unit, pc,
                                   PHY_BCM542XX_RDB_ADDR_REG_OFFSET, reg_addr));
    SOC_IF_ERROR_RETURN(    /* MDIO modify reg 0x1F[15:0] for the data & mask */
        PHY_BCM542XX_MODIFY_PHY_REG(unit, pc,
                                    PHY_BCM542XX_RDB_DATA_REG_OFFSET,
                                    data, mask));
    PHY_BCM542XX_DIS_RDB_MODE(unit, pc);   /* legacy register access mode */
    return SOC_E_NONE;
}
 ...
 */
STATIC int
_phy_bcm54280_init(int unit, soc_port_t port, int automedium)
{
    phy_ctrl_t  *pc;
    pc = EXT_PHY_SW_STATE(unit, port);
 ...
 */
STATIC int
_phy_bcm54282_init(int unit, soc_port_t port, int automedium) 
{
    phy_ctrl_t  *pc;
    pc = EXT_PHY_SW_STATE(unit, port);
                   ...
                   int fiber_enable   , int copper_enable )
{
    ...
    /*----------- COPPER INTERFACE ---------- */
    data = ( PHY_IS_BCM54210_220(pc) ) ? PHY_BCM542XX_MODE_SEL_SGMII_2_COPPER
                                       : PHY_BCM542XX_MODE_SEL_COPPER_2_SGMII;
    ...
    if ( fiber_enable && (automedium || fiber_preferred) ) {
        ...
        SOC_IF_ERROR_RETURN
            (PHY_MODIFY_BCM542XX_1000X_MII_ANAr(unit, pc, data, data));
        ...
        if ( PHY_IS_BCM54210_220(pc) ) {
            ...
            /* External Serdes Control Reg., DIGX_SHD_1C_14, RDB_0x234 */
            SOC_IF_ERROR_RETURN
                    (PHY_WRITE_BCM542XX_EXT_SERDES_CTRLr(unit, pc, data)); /* 0xd1cf */
            /* SGMII Slave Control Register, DIGX_SHD_1C_15, RDB_0x235 */
            SOC_IF_ERROR_RETURN
                (PHY_MODIFY_BCM542XX_SGMII_SLAVEr(unit, pc, 0,
                                      PHY_BCM542XX_SGMII_SLAVE_AUTO));
            ...
            /* Second SERDES MII CONTROL Register, RDB_0xb00 */
            SOC_IF_ERROR_RETURN
                (phy_bcm542xx_direct_reg_write(unit, pc,
                            PHY_BCM542XX_2ND_SERDES_MII_CTRL_REG_OFFSET,
                            PHY_BCM542XX_2ND_SERDES_MII_CTRL_INIT));
    ...
    /*   and  Enable LEDs to indicate traffic status                            */
    SOC_IF_ERROR_RETURN
        (PHY_MODIFY_BCM542XX_MII_ECRr(unit, pc,
                                      PHY_BCM542XX_MII_ECR_EN_LEDT,
                                      PHY_BCM542XX_MII_ECR_EN_LEDT));
...
phy_bcm542xx_init(int unit, soc_port_t port)
{
    ...
    PHY_BCM542XX_DIS_RDB_MODE(unit, pc);
    ...
    /* fiber_capable? */
    if ( PHY_IS_BCM54240(pc) || PHY_IS_BCM54285(pc) || PHY_IS_BCM54210(pc) ||
         PHY_IS_BCM54294(pc) || PHY_IS_BCM54295(pc) || PHY_IS_BCM54220(pc) ) {
        fiber_capable = 1;
    }
   ...
   /* Init PHYS and MACs to defaults */
    ...
    if ( SOC_WARM_BOOT(unit) ) {
        ...
        if ( fiber_capable == 1 ) {
            ...
            pc->fiber_detect     = ( val16 & (1 << 8) ) ? -4 : 0;
            pc->fiber.preferred  = ( val16 & (1 << 1) ) ? TRUE : FALSE;
...
phy_bcm542xx_link_get(int unit, soc_port_t port, int *link)
{
    ...
    *link = FALSE;      /* Default return */
    ...
    if ( PHY_IS_BCM54240(pc) || PHY_IS_BCM54285(pc) ) {
        ...
        if ( (!(temp16 & (1U << 5))) && ((temp16 & 0x1f) == 0xb) ) {
                         ...
                         soc_phy_control_t type, uint32 value)
{
    ...
    case SOC_PHY_CONTROL_EEE_AUTO:
        ...
        if ( value == 1 ) {    /* Enable AutoGrEEEn */
            SOC_IF_ERROR_RETURN
                (phy_bcm542xx_eee_auto_enable(unit, port, 1));
        } else {               /* Disable AutoGrEEEn */
            SOC_IF_ERROR_RETURN
