/*
 * $Id: phy84793.c 1.7.6.2 Broadcom SDK $
...
_phy_84793_dsc_diag(int32 unit, soc_port_t port, int32 intf, int32 lane)
{
    ...
    soc_cm_print("LaneN  PPM PPM_XFR clk90_ofs PF SL_TRGT VGA BIAS DFE1 DFE2 "
    ...
    switch (op_cmd) {
        ...
        default:
            if (op_type == PHY_DIAG_CTRL_SET) {
                SOC_IF_ERROR_RETURN
                (phy_84793_per_lane_control_set(unit, port, lane,
                                        op_cmd, PTR_TO_INT(arg)));
            } else if (op_type == PHY_DIAG_CTRL_GET) {
                SOC_IF_ERROR_RETURN
                (phy_84793_per_lane_control_get(unit, port, lane,
                                        op_cmd, (uint32 *)arg));
            } else {
    ...
    *value = 0;
    switch (mode) {
        ...
        case PHY84793_MODE_10X10G_TO_4X25G_OTL:
            ...
            if ((rddata & (1 << lane)) != (1<<lane)) { /* If not locked */
                SOC_IF_ERROR_RETURN
                 (_phy_84793_modify_pma_pmd_reg (unit, port, pc, 
