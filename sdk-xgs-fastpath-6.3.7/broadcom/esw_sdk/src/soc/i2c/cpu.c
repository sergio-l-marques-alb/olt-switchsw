...
#define CPU_I2C_DEVICE_FILE_NAME_PREFIX "/dev/i2c-" /* i2c device is CPU_I2C_DEVICE_FILE_NAME_PREFIX + device number */
#define CPU_I2C_MAX_NAME_LENGTH 256
...
int cpu_i2c_init(int* p_fd, char dev_num)
{
  ...
  {
    soc_cm_debug(DK_I2C,"ERROR in %s: Failed to open() I2C device num (%d)\n", FUNCTION_NAME(), (int)dev_num);
    ret = 1;
    goto exit;
  }
...
int cpu_i2c_read_int(cpu_i2c_access_t* p_i2c_access)
{
  ...
  case CPU_I2C_ALEN_NONE_DLEN_WORD: /* No internal adress */
	  ...
	  while (ioctl(p_i2c_access->file_desc, I2C_RDWR, &rw_data) < 0) {
		  soc_cm_debug(DK_I2C,"ioctl I2C_RDWR failed in %s, %d, %s - error reading from chip 0x%02x, address 0x%02x, error = %d\n",
					   __FILE__, __LINE__, FUNCTION_NAME(), p_i2c_access->chip, p_i2c_access->addr,errno);
		  if (errno != EINTR) {
			  ret = 1;
			  goto exit;
		  }
  ...
  case CPU_I2C_ALEN_BYTE_DLEN_LONG: /* address is 1 bytes long, data is 4 byte long*/
	  ...
	  while (ioctl(p_i2c_access->file_desc, I2C_RDWR, &rw_data) < 0) {
		  soc_cm_debug(DK_I2C,"ioctl I2C_RDWR failed in %s, %d, %s - error reading from chip 0x%x, address 0x%x, error = %d\n",
					   __FILE__, __LINE__, FUNCTION_NAME(), p_i2c_access->chip, p_i2c_access->addr,errno);
		  if (errno != EINTR) {
			  ret = 1;
			  goto exit;
  ...
  case CPU_I2C_ALEN_LONG_DLEN_LONG: /* address is 4 bytes long */
      ...
      default:
          ...
          soc_cm_debug(DK_I2C, "error - unsupported address length given = %d", p_i2c_access->alen);
          ret = 1;
          goto exit;
          break;
    } /* switch*/
...
int cpu_i2c_write_int(cpu_i2c_access_t* p_i2c_access)
{
    ...
    /* soc_cm_debug(DK_I2C,"%s(): p_i2c_access->alen=%x, p_i2c_access->chip=%x, p_i2c_access->addr=%x, p_i2c_access->data=%x\n", FUNCTION_NAME(), p_i2c_access->alen, p_i2c_access->chip, p_i2c_access->addr, p_i2c_access->data); */
	...
	case CPU_I2C_ALEN_NONE_DLEN_BYTE: /* No internal adress */
		...
		while (ioctl(p_i2c_access->file_desc, I2C_RDWR, &rw_data) < 0) {
			soc_cm_debug(DK_I2C, "ioctl I2C_RDWR failed in %s, %d, %s- error to write to chip 0x%x, address 0x%x, error = %d\n",
						  __FILE__, __LINE__, FUNCTION_NAME(), p_i2c_access->chip, p_i2c_access->addr, errno);
			if (errno != EINTR) {
				ret = 1;
				goto exit;
...
int cpu_i2c_write(int chip, int addr, CPU_I2C_BUS_LEN alen, int val)
{
  ...
  {
    soc_cm_debug(DK_I2C,"ERROR in %s: cpu_i2c_init() returned with error.\n", FUNCTION_NAME());
    return 1;
  }
  soc_cm_debug(DK_I2C, "cpu_i2c_write: bus:%d dev:@%02x reg:%0x data:%x\n", dev_num, chip, addr, val);
  ...
  if (ret)
  {
    soc_cm_debug(DK_I2C,"Error in %s: cpu_i2c_write() returned with error.\n", FUNCTION_NAME());
    ret = 1;
    goto exit;
  }
...
int cpu_i2c_read(int chip, int addr, CPU_I2C_BUS_LEN alen, int* p_val)
{
  ...
  soc_cm_debug(DK_I2C, "cpu_i2c_read: bus:%d dev:@%02x reg:%0x\n", dev_num, chip, addr);

  ret = cpu_i2c_read_int(&i2c_access);
  if (ret)
  {
    soc_cm_debug(DK_I2C,"Error in %s: cpu_i2c_read() returned with error.\n", FUNCTION_NAME());
    ret = 1;
    goto exit;
  }
  *p_val = i2c_access.data;
  soc_cm_debug(DK_I2C, "cpu_i2c_read: bus:%d dev:@%02x reg:%0x data:%x\n", dev_num, chip, addr, *p_val);
                   ...
                   uint8 reg, uint8 *data, uint8 *len)
{
    ...
    if (*len == 0) {
        soc_cm_debug(DK_I2C,"ERROR in cpu_i2c_block_read: invalid len specified (%d)", *len);
        return SOC_E_PARAM;
    }
    ...
    if (rv < 0) {
        soc_cm_debug(DK_I2C,"\nFailed reading %d bytes data from %x at device saddr %x", *len, reg, saddr);
        return SOC_E_PARAM;
    }
    return SOC_E_NONE;
}
...
int cpu_i2c_write(int chip, int addr, CPU_I2C_BUS_LEN alen, int val)
{
    ...
    soc_cm_debug(DK_I2C,"%s: not implemented for KERNEL mode \n", FUNCTION_NAME());
    return -1;
}
...
#define DEBUG_PRINT         if (DEBUG_PRINT_ENABLE) soc_cm_print
...
int cpu_i2c_write(int chip, int addr, CPU_I2C_BUS_LEN alen, int val)
{
    ...
    DEBUG_PRINT("%s: chip:%d addr:%x alen:%d val:%d \n", 
               FUNCTION_NAME(), chip, addr, alen, val);
    switch (alen) {
    ...
    case CPU_I2C_ALEN_BYTE_DLEN_LONG:
        ...
        break;
    default:
        soc_cm_debug(DK_I2C,"%s: alen:%x is not supported\n", FUNCTION_NAME(), alen);
    ...
    case 4:
        ...
        break;
    default:
        soc_cm_debug(DK_I2C,"%s: buf_len:%x not supported\n", FUNCTION_NAME(), 
                     buf_len);
        return SOC_E_INTERNAL;
    }
    ...
    DEBUG_PRINT("%s: success. wrote val:%x to addr:%x\n", FUNCTION_NAME(), 
               val, addr);
    return SOC_E_NONE;
}
...
int cpu_i2c_read(int chip, int addr, CPU_I2C_BUS_LEN alen, int* p_val)
{
    ...
    DEBUG_PRINT("%s: chip:%d addr:%x alen:%d \n", FUNCTION_NAME(), chip, 
                addr, alen);
    if (p_val == NULL) {
        return SOC_E_PARAM;
    }
    *p_val = 0;
    ...
    DEBUG_PRINT("%s: success chip:%d addr:%x alen:%d read-val:0x%0x \n", 
               FUNCTION_NAME(), chip, addr, alen, *p_val);
    return SOC_E_NONE;
}
