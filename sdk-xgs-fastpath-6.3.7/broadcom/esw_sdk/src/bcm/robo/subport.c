/*
 * $Id: subport.c 1.22 Broadcom SDK $
        ...
        int vpgrp_id, int svlan)
{
    ...
    if (op == _BCM_TB_SUBPORT_VM_OP_CREATE){
        ...
        if (rv != BCM_E_NONE) {
            soc_cm_debug(DK_WARN, "%s,%d, Allocating Flow-ID failed!\n", 
                    FUNCTION_NAME(), __LINE__);
        ...
        if (rv_temp != BCM_E_NONE) {
             ...
             */
            soc_cm_debug(DK_WARN, "%s,%d, Releasing IVM/EVM entries failed!\n", 
                        FUNCTION_NAME(), __LINE__);
        }
        ...
        if (down_vm_flow != _BCM_ROBO_INVALID_ID){
            if (DRV_VM_FLOW_FREE(unit, down_vm_flow)){
                soc_cm_debug(DK_WARN, "%s,%d, Releasing Flow-ID failed!\n", 
                        FUNCTION_NAME(), __LINE__);
            }
        }
    ...
    for (i = 0; i < _TB_SUBPORT_NUM_MCASTREP_GROUP; i++){
        ...
        if (t_vpgrp_act_svlan_db->evm_eid != _BCM_ROBO_INVALID_ID){
            ...
            if ((rv != BCM_E_NONE) && (rv != BCM_E_NOT_FOUND)){
                /* warning message only, no error return to finish all clear 
                 * process. */
                soc_cm_debug(DK_WARN, "%s,%d, Destroy EVM failed!\n", 
                        FUNCTION_NAME(), __LINE__);
                rv = BCM_E_NONE;
            }
        }
        if (t_vpgrp_act_svlan_db->ivm_eid != _BCM_ROBO_INVALID_ID){
            ...
            if ((rv != BCM_E_NONE) && (rv != BCM_E_NOT_FOUND)){
                /* warning message only, no error return to finish all clear 
                 * process. */
                soc_cm_debug(DK_WARN, "%s,%d, Destroy IVM failed!\n", 
                        FUNCTION_NAME(), __LINE__);
                rv = BCM_E_NONE;
            }
        }
                             ...
                             bcm_gport_t *group)
{
    ...
    if (config->flags & BCM_SUBPORT_GROUP_WITH_ID) {
        ...
        if (vpgrp_id < 0 || vpgrp_id > (_TB_SUBPORT_NUM_GROUP_PER_PORT - 1)){
            ...
            soc_cm_debug(DK_PORT, "%s, invalid support group id 0x%x\n", 
                    FUNCTION_NAME(), *group);
            _SUBPORT_VP_UNLOCK(unit);
            return BCM_E_PARAM;
        } else {
...
bcm_robo_subport_group_destroy(int unit, bcm_gport_t group)
{
    ...
    temp_vpgrp = _tb_vpgrp_db[unit] + vpgrp_mem_id;
    if (!SUBPORT_GROUP_IS_VALID(temp_vpgrp)){
        soc_cm_debug(DK_PORT, "%s, not existed support group id 0x%x\n", 
                FUNCTION_NAME(), group);
    ...
    if (t_mcrep_act_svlan->valid == FALSE) {
        soc_cm_debug(DK_WARN, "Internal problem on SW inforamtion.\n");
    } else {
        if (valid_cnt == 0){     
                          ...
                          bcm_subport_group_config_t *config)
{
    ...
    port_in = _TB_SUBPORT_GROUP_ID_2PORT_ID(vpgrp_mem_id);
    soc_cm_debug(DK_PORT, "%s, GPORT=0x%08x:vp_group=%d,port=%d\n", 
 ...
 *              - 2.5.1.3 VLAN Tagged UNI V Single VC ATM or Native Ethernet 
                         ...
                         bcm_gport_t *port)
{
    ...
    if (config->pkt_vlan > BCM_VLAN_MAX) {
        soc_cm_debug(DK_WARN, "CVLAN(%d) out of range\n", config->pkt_vlan);
        return BCM_E_PARAM;
    }
    ...
    if (active_flags & _BCM_SUBPORT_FLAG_EGR_SPCP_MAPPED){
        if ((config->int_pri < 0) || 
            ...
            soc_cm_debug(DK_WARN,"The int_pri0=0x%02x is invalid!\n",
                    config->int_pri);
            return BCM_E_PARAM;
        }
    } else {
        ...
        if (config->int_pri < 0 || config->int_pri > 7){
            soc_cm_debug(DK_WARN,"The int_pri0=%d is invalid!\n",
                    config->int_pri);
            return BCM_E_PARAM;
        }
    }    
...
bcm_robo_subport_port_delete(int unit, bcm_gport_t port)
{
    ...
    soc_cm_debug(DK_PORT, "%s, port(GPORT)=0x%x\n",FUNCTION_NAME(),port);
    if (!BCM_GPORT_IS_SUBPORT_PORT(port)){
        soc_cm_debug(DK_PORT, "%s, port 0x%x is not at subport type.\n", 
                FUNCTION_NAME(), port);
        return BCM_E_PARAM;
    }
    ...
    if (vp_id < 0 || vp_id > _TB_SUBPORT_SYSTEM_ID_MAX){
        soc_cm_debug(DK_PORT, "%s, port 0x%x is in the valid range\n", 
                FUNCTION_NAME(), port);
        return BCM_E_PARAM;
    }
    ...
    if (!VPBMP_BITGET(temp_vpgrp->vp_bmp, vport)){
        soc_cm_debug(DK_PORT,"%s,subport%d is not existed in this group\n", 
                FUNCTION_NAME(), vport);
                         ...
                         bcm_subport_config_t *config)
{
    ...
    if (DRV_MCREP_VPORT_CONFIG_GET(unit, port_out, 
               DRV_MCREP_VPORT_OP_VID, &vport, &vid)){
        soc_cm_debug(DK_PORT,"%s,failed on get subport vid!\n", 
                FUNCTION_NAME());
            ...
            for (j = 0; j < _TB_SUBPORT_NUM_VPORT_PER_GROUP; j++){
                ...
                if (VPBMP_BITGET(temp_vpgrp->vp_bmp, j)){
                    ...
                    if (bcm_subport_port_get(unit, gport, &config)){
                        soc_cm_debug(DK_PORT, "[Debug]%s,failed on getting" 
