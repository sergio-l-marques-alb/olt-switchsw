/*
 * $Id: l3.c 1.326.2.5 Broadcom SDK $
...
#if defined(BCM_KATANA_SUPPORT) || defined(BCM_TRIUMPH3_SUPPORT)
    if (soc_feature(unit,soc_feature_advanced_flex_counter)) {
        return _bcm_esw_l3_egress_stat_counter_get(unit, sync_mode, intf_id, 
                                                   stat, num_entries, 
                                                   counter_indexes,
                                                   counter_values);
    } else
                        ...
                        uint64 *val)
{
   ...
   if (!soc_feature(unit,soc_feature_advanced_flex_counter)) {
    ...
    rv = _bcm_esw_flex_stat_get(unit, 0, _bcmFlexStatTypeVrf, vrf,
                          ...
                          bcm_l3_vrf_stat_t stat, uint32 *val)
{
   ...
   if (!soc_feature(unit,soc_feature_advanced_flex_counter)) {
    ...
    rv = _bcm_esw_flex_stat_get32(unit, 0, _bcmFlexStatTypeVrf, vrf,
                          _bcm_esw_l3_vrf_stat_to_flex_stat(stat), val);
    L3_UNLOCK(unit);
    return rv;
   }
...
#if defined(BCM_TRIDENT2_SUPPORT)
    if (SOC_IS_TD2_TT2(unit)) { 
        return _bcm_td2_l3_host_stat_counter_get(unit, sync_mode, info, stat, 
                    num_entries, counter_indexes, counter_values);
    } else
#endif
#if defined(BCM_TRIUMPH3_SUPPORT)
    if (SOC_IS_TRIUMPH3(unit)) {
        return _bcm_tr3_l3_host_stat_counter_get(unit, sync_mode, info, stat, 
                    num_entries, counter_indexes, counter_values);
    } else
#endif
    {
