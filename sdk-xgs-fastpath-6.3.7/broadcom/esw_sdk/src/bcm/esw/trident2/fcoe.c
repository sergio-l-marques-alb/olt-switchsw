...
_bcm_td2_fcoe_sync(int unit)
{
    ...
    if (rv == BCM_E_NONE) {
        ...
        fcoe_scache_ptr += sizeof(_vsan_id_reserve_state);
    }
                         ...
                         _bcm_defip_cfg_t *lpm_cfg)
{
    ...
    if (route->flags & BCM_FCOE_COPY_TO_CPU) {
        lpm_cfg->defip_flags |= BCM_L3_COPY_TO_CPU;
    }
    if (route->flags & BCM_FCOE_DST_DISCARD) {
                         ...
                         bcm_fcoe_route_t *route)
{
    ...
    if (lpm_cfg->defip_flags & BCM_L3_COPY_TO_CPU) {
...
_bcm_td2_fcoe_route_delete_prefix(int unit, uint32 *lookup)
{
    ...
    if (soc_L3_DEFIPm_field32_get(unit, lookup, VALID0f)
        ...
        _bcm_fb_lpm_ent_parse(unit, &lpm_cfg, &nh_idx, (defip_entry_t *)lookup, 
                              NULL);
                                    ...
                                    soc_mem_t mem, uint32 *lookup) 
{
    ...
    if (SOC_MEM_FIELD_VALID(unit, mem, FCOE__NEXT_HOP_INDEXf)) {
        next_hop_index = soc_mem_field32_get(unit, mem, lookup, 
...
_bcm_td2_mem_get_vrf(int unit, soc_mem_t mem, uint32 *buf, int *vrf)
{
    ...
    } else if (mem == L3_ENTRY_IPV4_UNICASTm) {
    ...
    if (mem == L3_DEFIPm) {
        ...
        switch (key_type) {
            ...
            case TD2_L3_HASH_KEY_TYPE_FCOE_HOST_EXT:
                id_key = (mem == L3_ENTRY_IPV4_MULTICASTm)
 ...
 */
STATIC int
_bcm_td2_key_type_is_fcoe(soc_mem_t mem, int key_type)
{
    if (mem == L3_ENTRY_IPV4_MULTICASTm) {
                             ...
                             bcm_fcoe_route_t *route)
{
    ...
    for (idx = 0; idx < idx_max; idx++) {
        ...
        BCM_IF_ERROR_RETURN(BCM_XGS3_MEM_READ(unit, mem, idx, buf));
        ...
        if (!_bcm_td2_key_type_is_fcoe(mem, key_type_0)
                             ...
                             bcm_fcoe_route_t *route)
{
    ...
    for (idx = 0; idx < idx_max; idx++) {
        ...
        if (   (soc_mem_field32_get(unit, mem, buf, VALID0f) && 
            ...
            || (soc_mem_field32_get(unit, mem, buf, VALID1f) && 
                soc_mem_field32_get(unit, mem, buf, ENTRY_TYPE1f))
            ) {

            if (traverse_func(unit, mem, buf, route) != BCM_E_NONE) {
                break;
            }
        }
...
_bcm_td2_fcoe_traverse_tables(int unit, fcoe_traverse_cb_f traverse_func,
                              bcm_fcoe_route_t *route)
{
    int rv = _bcm_td2_fcoe_traverse_table(unit, L3_ENTRY_IPV4_UNICASTm,
                                          traverse_func, route);
    if (BCM_SUCCESS(rv)) {
        rv = _bcm_td2_fcoe_traverse_table(unit, L3_ENTRY_IPV4_MULTICASTm,
                                          traverse_func, route);
    }
                                  ...
                                  bcm_fcoe_route_t *route)
{
    ...
    if (mem == L3_ENTRY_IPV4_MULTICASTm) {
        ...
        int nexthop = soc_mem_field32_get(unit, mem, buf,
                                          FCOE__NEXT_HOP_INDEXf);
...
STATIC int
_bcm_fcoe_vsan_param_validate (bcm_fcoe_vsan_t *vsan, 
                                 ...
                                 bcm_fcoe_fc_header_type_t *hdr_type)
{
    ...
    if (rv == BCM_E_NONE) {
        if (flags & BCM_FCOE_FC_HEADER_TYPE_INGRESS) {
            rv = _bcm_td2_fcoe_fc_read_header(unit, ING_FC_HEADER_TYPEm, 
                                              r_ctl, &encoded_value);
        } else if (flags & BCM_FCOE_FC_HEADER_TYPE_EGRESS) {
            rv = _bcm_td2_fcoe_fc_read_header(unit, EGR_FC_HEADER_TYPEm, 
                                              r_ctl, &encoded_value);
        } else {
            rv = BCM_E_PARAM;
        }
                                   ...
                                   bcm_stat_flex_table_info_t *table_info)
{
    ...
    /* Ingress FCOE */
    table_info[*num_of_tables].table = L3_ENTRY_IPV4_MULTICASTm;
    ...
    if (soc_feature(unit, soc_feature_fcoe)) {
        ...
        switch (key->key_type) {
            ...
            case bcmFcoeVsanTranslateKeyInnerTag:
                ...
                /* vlan id should also have PRI/CFI bits */
                soc_VLAN_XLATEm_field32_set(unit, vent, XLATE__ITAGf,
                                            key->inner_vlan);
                break;
            ...
            case bcmFcoeVsanTranslateKeyDoubleVsan:
                ...
                soc_VLAN_XLATEm_field32_set(unit, vent, XLATE__IVIDf,
                                            key->inner_vlan);
                soc_VLAN_XLATEm_field32_set(unit, vent, XLATE__VSAN_IDf,
                                            key->vsan);
                break;
        ...
        if (use_glp) {
            ...
            if (SOC_MEM_FIELD_VALID(unit, VLAN_XLATEm, SOURCE_TYPEf)) {
                soc_VLAN_XLATEm_field32_set(unit, vent, SOURCE_TYPEf, 0);
            }
                                ...
                                bcm_fcoe_vsan_translate_key_config_t *key)
{
    ...
    soc_EGR_VLAN_XLATEm_field32_set(unit, vent, OVIDf, key->outer_vlan);
    soc_EGR_VLAN_XLATEm_field32_set(unit, vent, IVIDf, key->inner_vlan);
    soc_EGR_VLAN_XLATEm_field32_set(unit, vent, VSAN_IDf, key->vsan);
                                    ...
                                    bcm_fcoe_vsan_action_set_t *action)
{
    ...
    if (action->flags & BCM_FCOE_VSAN_ACTION_INGRESS) {
        ...
        /* update table index */
        soc_VLAN_XLATEm_field32_set(unit, &vent, XLATE__TAG_ACTION_PROFILE_PTRf,
                                    profile_idx);
        ...
        soc_VLAN_XLATEm_field32_set(unit, &vent, XLATE__FCOE_VSAN_IDf, 
                                    action->new_vsan);
        soc_VLAN_XLATEm_field32_set(unit, &vent, XLATE__FCOE_VSAN_PRIf, 
                                    action->new_vsan_pri);
        ...
        if (search_rv == SOC_E_NONE) {
            ...
            rv = soc_mem_write(unit, VLAN_XLATEm, MEM_BLOCK_ALL, index, &vent);
    ...
    if (action->flags & BCM_FCOE_VSAN_ACTION_EGRESS) {
        ...
        search_rv = soc_mem_search(unit, EGR_VLAN_XLATEm, MEM_BLOCK_ANY, &index,
                                   vent_ptr, &vent_old, 0);
        ...
        if (search_rv == SOC_E_NONE) {
            ...
            */
            ...
            old_vft_profile_index = 
                soc_EGR_VLAN_XLATEm_field32_get(unit, vent_ptr,
                                               FCOE_VFT_FIELDS_PROFILE_INDEXf); 
        ...
        /* new VSAN & PRI */
        soc_EGR_VLAN_XLATEm_field32_set(unit, vent_ptr, FCOE_VSAN_IDf, 
                                        action->new_vsan);
        soc_EGR_VLAN_XLATEm_field32_set(unit, vent_ptr, FCOE_VSAN_PRIf, 
                                        action->new_vsan_pri);
                                    ...
                                    bcm_fcoe_vsan_translate_key_config_t *key) 
{
    ...
    BCM_IF_ERROR_RETURN(
        _bcm_td2_fcoe_ing_vlan_translate_entry_assemble(unit, &ing_vent, 
                                                        key)); 
    ...
    if (search_rv == SOC_E_NONE) {   /* exist */
        ...
        soc_VLAN_XLATEm_field32_set(unit, &ing_vent, VALIDf, 0);
        rv = soc_mem_write(unit, VLAN_XLATEm, MEM_BLOCK_ALL, index, &ing_vent);
    ...
    if (rv == SOC_E_NONE) {
        ...
        egr_profile_idx = soc_EGR_VLAN_XLATEm_field32_get(unit, &egr_vent,
                                                     TAG_ACTION_PROFILE_PTRf); 
        egr_vft_profile_idx =  soc_EGR_VLAN_XLATEm_field32_get(unit, &egr_vent,
                                               FCOE_VFT_FIELDS_PROFILE_INDEXf); 

        /* mark entry as invalid */
        soc_VLAN_XLATEm_field32_set(unit, &egr_vent, VALIDf, 0);
        rv = soc_mem_write(unit, EGR_VLAN_XLATEm, MEM_BLOCK_ALL, index, 
                           &egr_vent);
...
_bcm_td2_fcoe_sw_dump(int unit)
{
    ...
    soc_cm_print("VSAN ID Creation State : \n");
    for (vsan_id = 0; vsan_id <= FCOE_MAX_VSAN_ID; vsan_id++) {
        if ((vsan_id % 16) == 0) {
            soc_cm_print("Vsan ID (from %04d): ", vsan_id); 
        }
        soc_cm_print("%d ", _bcm_td2_vsan_id_creation_state_get(unit, vsan_id));
        if ((vsan_id %16) == 15) {
            soc_cm_print("\n");
        }
    }
...
STATIC int
_bcm_td2_fcoe_correct_key_type_for_flags(uint32    flags, 
                                         soc_mem_t mem, 
                                         int       key_type)
{
    int rv = BCM_E_NONE;
                                    ...
                                    void *user_data)
{
    ...
    for (idx = 0; idx < idx_max; idx++) {
        ...
        if (_bcm_td2_fcoe_correct_key_type_for_flags(flags, mem, key_type_0) != 
             BCM_E_NONE) {
            continue;
        }
