/*
 * $Id: flex_ctr_common.c 1.84.2.6 Broadcom SDK $
...
#define FLEX_SET_BYTE_COUNTER_UPDATE \
    do { \
        ...
        COMPILER_64_AND(flex_values[i].byte_counter_value, max_byte_mask); \
        /* Update Soft Copy */ \
        COMPILER_64_SET(flex_byte_counter[unit][dir][pool_id][start+i], \
                        COMPILER_64_HI(flex_values[i].byte_counter_value), \
                        COMPILER_64_LO(flex_values[i].byte_counter_value)); \
        /* Change Read Hw Copy */ \
        hw_val[0] = COMPILER_64_HI(flex_values[i].byte_counter_value); \
        hw_val[1] = COMPILER_64_LO(flex_values[i].byte_counter_value); \
...
#define FLEX_SET_PACKET_COUNTER_UPDATE \
    do { \
        flex_values[i].pkt_counter_value &= max_packet_mask; \
        /* Update Soft 64-bits Copy */ \
        COMPILER_64_SET(flex_packet64_counter[unit][dir][pool_id][start+i], \
                    0, flex_values[i].pkt_counter_value); \
        /* Update Soft Copy */ \
        flex_packet_counter[unit][dir][pool_id][start+i] = \
                    flex_values[i].pkt_counter_value; \
        /* Change Read Hw Copy */ \
        soc_mem_field_set(unit, mem, (uint32 *)&entry_temp[i], \
                          PACKET_COUNTERf, &flex_values[i].pkt_counter_value); \
...
#define FLEX_SET_PACKET_COUNTER_UPDATE_TD2 \
    ...
    /* If entry_cnt less than 3, memory range operation is not efficient */ 
    if (entry_cnt < 3) {
    ...
    if (soc_mem_read(unit,table,MEM_BLOCK_ANY,
        ...
        *flex_entries = total_entries;
        ...
        if (BCM_FAILURE(_bcm_esw_stat_flex_set_mem_update(unit, table, index, 
                                ...
                                pool_number, byte_flag, flex_values))) {
            BCM_STAT_FLEX_COUNTER_UNLOCK(unit);
            sal_free(entry_data);
            return BCM_E_INTERNAL;
        }
...
bcm_error_t _bcm_esw_stat_flex_init(int unit)
{
    ...
    if (config_str != NULL) {
        ...
        if (rv != BCM_E_NONE) {
            FLEXCTR_ERR(("ERR: Parsing property string failed"));
            _bcm_esw_stat_flex_cleanup(unit);
            return rv;
        }
    }
