...
cmd_result_t
sh_rcload_silent(int u, args_t *a)
/*
 * Function:     sh_rcload
 * Purpose:    Load commands from a file.
 * Parameters:    u - unit number
...
appl_warmboot_storage_requirements(int unit)
{
    ...
    if (SOC_SUCCESS(rv)) {
        ...
        soc_cm_print("ERROR: Failed to get stable size!!\n");
        return;
    }
        ...
        if (!sal_strcasecmp(c, "on")) {
		   ...
		   if (0 == ARG_CNT(a)) {
		    ...
		    if (!sal_strcasecmp(test_status, "on")) {  
			     _bcm_dpp_switch_warmboot_test_mode_set(unit,WB_TEST_MODE_AFTER_EVERY_API);
			}else if (!sal_strcasecmp(test_status, "dvapi")){
                        _bcm_dpp_switch_warmboot_test_mode_set(unit,WB_TEST_MODE_END_OF_DVAPIS);
			}else if (!sal_strcasecmp(test_status, "off")){
                        _bcm_dpp_switch_warmboot_test_mode_set(unit,WB_TEST_MODE_NONE);
 ...
 */
{
    ...
    if ((bcm_rv = system_init(u)) < 0) {
        ...
        rv = CMD_FAIL;
            ...
            option = ARG_GET(a);        
            if (option) {
                diag_init_warmboot = sal_ctoi(option,0);
            }
    
            bcm_rv = appl_dpp_bcm_diag_init(u, diag_init_mymodid, diag_init_nof_devices, diag_init_cosq_disable, diag_init_warmboot);
