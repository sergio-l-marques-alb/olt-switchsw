...
cmd_esw_disabled_pbmp(int unit, args_t *a)
{
    ...
    if (!c) {
        printk("Current disabled bitmaps:\n");
        printk("     FE   ==> %s\n",
...
cmd_cmicm_intr(int unit,args_t *a)
{
    ...
    if (!(SOC_REG_IS_VALID (unit, CMIC_IRQ_STATr)) && (ARG_CNT(a) <= 4)) {
        printk ("Options cmc,stat must be provided\n");
        return CMD_USAGE;
    }
    ...
    if (parse_cmp("Enable", cmd_option, '\0')) {
        if (parse_irq_list(a, irq_names, irq_masks, count, &mask)) {
            printk("Invalid Interupt Name/Value.\n");
            return CMD_USAGE;
        }
...
cmd_intr(int unit, args_t *a)
{
    ...
    if (parse_cmp("Enable", subcmd, '\0')) {
        if (parse_irq_list(a, irq_names, irq_masks, count, &mask)) {
            printk("Invalid Interrupt Name/Value.\n");
            printk("Note: cmc,stat options are not supported in this device\n");
            return CMD_USAGE;
        }
