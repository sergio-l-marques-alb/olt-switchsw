...
ptp_show_port(int unit, bcm_ptp_stack_id_t stack_id, int clock_num, int port_number)
{
    ...
    printk("   portDS.peerMeanPathDelay           : %s\n", uint64_decimal_string);
...
STATIC char *ptp_clock_accuracy_get(bcm_ptp_clock_accuracy_t *clock_accuracy, char *clock_accuracy_str)
{
    if (*clock_accuracy >= bcmPTPClockAccuracy25ns &&
        *clock_accuracy <= bcmPTPClockAccuracyL10s) {
            strcpy(clock_accuracy_str, 
                    bcm_ptp_clock_accuracy_str[(int)(*clock_accuracy 
                    - bcmPTPClockAccuracy25ns)]);
    }
    else {
        strcpy(clock_accuracy_str, 
                "bcmPTPClockAccuracyUnknown/bcmPTPClockAccuracyReserved");
    }
...
ptp_show_foreign_master_dataset(int unit, bcm_ptp_stack_id_t stack_id, int clock_num, int port_num)
{
    ...
    for (index = 0; index < dataset.num_foreign_masters; index++) {
        ...
        printk("    Offset scaled log variance: %u\n", 
                    dataset.foreign_master[index].offset_scaled_log_variance);
        printk("    Clock accuracy: 0x%02x %s\n", 
...
cmd_ptp_channels(int unit, args_t *a)
{
    ...
    if (parse_cmp("Set", arg, 0)) {
        ...
        printk(PTP_CHANNELS_USAGE_1);
        printk(PTP_CHANNELS_USAGE_2);
...
#define PTP_TODOUT_USAGE\
    "PTP ToDOut <setting> [OffsetNS=<offsetNS>] [OffsetSec=<offsetSec>]\n"    \
    "                     [Delay=<delayNS>] [CustomFormat=<format_string>]\n" \
    "                     [PPSJitter=<nsec>]\n"                               \
    "    where <setting> = | Off | NMEAzda | ISO8601 | NTP | UBlox |\n"       \
    "                      | ChinaMobile | ChinaTelecom | CUSTom |\n"
...
cmd_ptp_servo_config_show(int unit, bcm_ptp_stack_id_t stack_id, int clock_num)
{
    ...
    printk("\t Freq. Correction Timestamp   : %s.%09u sec.\n",
...
cmd_ptp_servo(int unit, args_t *a)
{
    ...
    printk(PTP_SERVO_USAGE_1);
    printk(PTP_SERVO_USAGE_2);
    PTP_CLI_RESULT_RETURN(CMD_FAIL);
}
...
cmd_ptp_telecom(int unit, args_t *a)
{
    ...
    if (!arg) {
        ...
        if (!arg) {
            ...
            printk("Scaled AVAR Threshold      : %s (nsec-sq)\n", uint64_decimal_string);
...
tdpll_instance_show(int unit, int low_idx, int high_idx)
{
    ...
    printk("#  GP0  GP1  GP2  GP3  GP4  GP5  SE1  SE2  1588  BS0  BS1 S-PLL M-PLL  BW  PBO\n");
    ...
    for (dpll_index = low_idx; dpll_index < high_idx; ++dpll_index) {
        ...
        for (i = 0; i < num_io; ++i) {
            ...
            if (input_status) {
                if (output_status) {
                    printk("%s%c/%c%c", padding[i], input_char[input_status], output_char[output_status],
                           reference_char[reference_idx]);
                } else {
                    printk("%s %c%c ", padding[i], input_char[input_status], reference_char[reference_idx]);
                }
            } else {
                printk("%s %c%c ", padding[i], output_char[output_status], reference_char[reference_idx]);
            }
        }
        ...
        printk("%3u.%02u", (unsigned)bwhz_dec, (unsigned)bwhz_frac);
...
cmd_tdpll_instance(int unit, args_t *a)
{
    ...
    if (arg) {
        ...
        if (dpll_index_str) {
            if (!isint(dpll_index_str)) {
                printk("%s INSTance show/Get/Set: Invalid DPLL index '%s'\n", ARG_CMD(a), dpll_index_str);
                printk(TDPLL_INSTANCE_USAGE_1);
                printk(TDPLL_INSTANCE_USAGE_2);
                return CMD_FAIL;
            }
    ...
    if (!arg || parse_cmp("show", arg, 0)) {
        ...
        if (arg_dpll_index < 0) {
            printk("%s INSTance Get: No DPLL index specified\n", ARG_CMD(a));
            return(CMD_FAIL);
        }
        ...
        printk("\n");
        printk("T-DPLL DPLL Instance Configuration.\n");
        printk("Unit: %d   Stack: %d\n",
               unit, 0);
        printk("-----------------------------------------------------------"
               "---------------------\n");
        printk("DPLL       : %d\n", arg_dpll_index);
        printk("\n");
        printk("Inputs     : 0x%08x\n", input_mask);
        ...
        if (arg_dpll_index < 0) {
            printk("%s INSTance Set: No DPLL index specified\n", ARG_CMD(a));
            return(CMD_FAIL);
        }
        parse_table_init(unit, &pt);
...
tdpll_inputclk_show(int unit, int low_idx, int high_idx)
{
    ...
    printk("ID     En?   Offset   Valid    QL     Prio   LkO Mux   DPLLS\n");
    ...
    for (inputclk_idx = low_idx; inputclk_idx < high_idx; ++inputclk_idx) {
        ...
        printk("%s ", input_name[inputclk_idx]);

        PTP_IF_ERROR_RETURN(bcm_tdpll_input_clock_enable_get(
            unit, 0, inputclk_idx, &enabled));

        printk(" %c", enabled ? 'Y' : 'N');
        ...
        if (!valid && freq_error_ppb == -65249626) {
            ...
            printk("%10d   ", freq_error_ppb);
        }

        if (valid) {
            printk("  %c   ", (soft_warn) ? 'W' : 'Y');
        } else {
            printk("  %c   ", (hard_reject) ? 'R' : 'I');
        }
...
cmd_tdpll_inputclk(int unit, args_t *a)
{
    ...
    if (arg) {
        ...
        if (clock_index_str) {
            ...
            if (!arg_index_str[arg_clock_index]) {
                printk("%s INputclk show/Get/Set: Invalid Clock index '%s'\n", ARG_CMD(a), clock_index_str);
                printk(TDPLL_INPUTCLK_USAGE_1);
                printk(TDPLL_INPUTCLK_USAGE_2);
                return CMD_FAIL;
            }
        }
    }
    ...
    if (!arg || parse_cmp("show", arg, 0)) {
        ...
        if (arg_clock_index < 0) {
            printk("Inputclk index required\n");
            return CMD_USAGE;
        }
        ...
        printk("\n");
        printk("T-DPLL Input Clock Data and Metadata.\n");
        printk("Unit: %d   Stack: %d\n",
               unit, 0);
        printk("-----------------------------------------------------------"
               "---------------------\n");
        printk("Index    : %d (%s)   L1 MUX: %d.%d\n", arg_clock_index,
        ...
        printk("Enable   : [%s]\n", arg_clk_en ? "x":" ");
        printk("\n");
        printk("Clock f  : %u Hz TS f: %u Hz (Q: %d)\n", (unsigned)arg_clock_frequency_rv,
        ...
        printk("\n");
        printk("Valid    : [%s]\n", arg_valid ? "x":" ");
        printk("| Over Soft-Warn [%s] | Under Hard-Accept [%s] | Over Hard-Reject [%s] |\n",
               arg_over_softwarn ? "x":" ", arg_under_hardaccept ? "x":" ", arg_over_hardreject ? "x":" ");
        printk("f Error  : %d ppb\n", arg_freq_error_ppb);
        printk("\n");
        printk("DPLLs    : | DPLL 0 [%s] | DPLL 1 [%s] | DPLL 2 [%s] | DPLL 3 [%s] | DPLL 4 [%s] |\n",
        ...
        if (arg_ql_index != invalid_value) {
            ...
            if (bcm_esmc_ql_unresolvable == arg_ql) {
                printk("TDPLL INPUTCLK SET ... : Unresolvable QL.\n");
            } else {
...
cmd_tdpll_outputclk(int unit, args_t *a)
{
    ...
    if (arg) {
        ...
        if (clock_index_str) {
            ...
            if (!arg_index_str[arg_clock_index]) {
                printk("%s OUTputclk Get/Set: Invalid Clock index '%s'\n", ARG_CMD(a), clock_index_str);
                printk(TDPLL_OUTPUTCLK_USAGE);
                return CMD_FAIL;
            }
        }
    }
    ...
    if (!arg) {
        ...
        printk("\n");
        printk("T-DPLL Output Clock Data and Metadata.\n");
        printk("Unit: %d   Stack: %d\n",
               unit, 0);
        printk("-----------------------------------------------------------"
               "---------------------\n");
        ...
        printk("Enable     : [%s]\n", arg_clk_en ? "x":" ");
        printk("\n");        
        printk("Synth f    : %u Hz TS f: %u Hz (Q: %d)\n", (unsigned)arg_synth_frequency_rv,
               (unsigned)arg_tsevent_frequency_rv, arg_tsevent_quotient_rv);
        printk("Deriv f    : %u Hz (Q: %d)\n",
...
cmd_tdpll_monitor(int unit, args_t *a)
{
    ...
    if (!arg) {
        ...
        printk("\n");
        printk("T-DPLL Input Clock Monitoring Parameters and Thresholds.\n");
        printk("Unit: %d   Stack: %d\n",
               unit, PTP_STACK_ID_DEFAULT);
        printk("-----------------------------------------------------------"
               "---------------------\n");
        printk("Interval   : %u sec\n", (unsigned)arg_monitor_interval_rv);
        printk("Soft-Warn  : %u ppb\n", (unsigned)arg_softwarn_ppb_rv);
        printk("Hard-Accept: %u ppb\n", (unsigned)arg_hardaccept_ppb_rv);
        printk("Hard-Reject: %u ppb\n", (unsigned)arg_hardreject_ppb_rv);
        printk("\n");
...
cmd_tdpll_select(int unit, args_t *a)
{
    ...
    if (dpll_index_str) {
        if (!isint(dpll_index_str)) {
            printk("%s SELect Get/Set: Invalid DPLL index '%s'\n", ARG_CMD(a), dpll_index_str);
            printk(TDPLL_SELECT_USAGE);
            return CMD_FAIL;
        }
        arg_dpll_index = parse_integer(dpll_index_str);
    }
    ...
    if (parse_cmp("Get", arg, 0)) {
        ...
        printk("\n");
        printk("T-DPLL Reference Selection Configuration and State Data.\n");
        printk("Unit: %d   Stack: %d\n",
               unit, 0);
        printk("-----------------------------------------------------------"
               "---------------------\n");
        printk("DPLL  : %d\n", arg_dpll_index);
        printk("Modes : QL-Enabled [%s]\n", arg_ql_enabled ? "x":" ");
        printk("Best  : %d (%s)\n", arg_best_clock,
...
cmd_tdpll_switch(int unit, args_t *a)
{
    ...
    if (dpll_index_str) {
        if (!isint(dpll_index_str)) {
            printk("%s SWitch Get/Set: Invalid DPLL index '%s'\n", ARG_CMD(a), dpll_index_str);
            printk(TDPLL_SWITCH_USAGE);
            return CMD_FAIL;
        }
        arg_dpll_index = parse_integer(dpll_index_str);
    }
    ...
    if (parse_cmp("Get", arg, 0)) {
        ...
        printk("\n");
        printk("T-DPLL Reference Switching Configuration and State Data.\n");
        printk("Unit: %d   Stack: %d\n",
               unit, 0);
        printk("-----------------------------------------------------------"
               "---------------------\n");
        printk("DPLL  : %d\n", arg_dpll_index);
        printk("Modes : | Revertive [%s] |\n",
               arg_revertive ? "x":" ");
        printk("Ref.  : %d %s (%d %s)\n",
...
cmd_tdpll_holdover(int unit, args_t *a)
{
    ...
    if (arg) {
        ...
        if (clock_index_str) {
            ...
            if (!arg_index_str[arg_index]) {
                printk("%s HOLDover Get/Set: Invalid Output '%s'\n", ARG_CMD(a), clock_index_str);
                printk(TDPLL_HOLDOVER_USAGE_1);
                printk(TDPLL_HOLDOVER_USAGE_2);
                return CMD_FAIL;
            }
        }
    }
    ...
    if (!arg) {
        ...
        printk("\n");
        printk("T-DPLL Holdover State and Configuration Data.\n");
        printk("Unit: %d   Stack: %d\n",
               unit, 0);
        printk("-----------------------------------------------------------"
               "---------------------\n");
        ...
        printk("Mode       : | [%s] Instantaneous | [%s] 1sec | [%s] Manual | [%s] Fast-Avg | [%s] Slow-Avg |\n",
...
cmd_tdpll_esmc(int unit, args_t *a)
{
    ...
    if (dpll_index_str) {
        if (!isint(dpll_index_str)) {
            printk("%s ESMC Get/Set: Invalid DPLL index '%s'\n", ARG_CMD(a), dpll_index_str);
            printk(TDPLL_ESMC_USAGE_1);
            printk(TDPLL_ESMC_USAGE_2);
            return CMD_FAIL;
        }
        arg_dpll_index = parse_integer(dpll_index_str);
    }
    ...
    if (!arg) {
        ...
        printk("\n");
        printk("T-DPLL Ethernet Synchronization Messaging Channel (ESMC).\n");
        printk("Unit: %d   Stack: %d\n",
               unit, 0);
        printk("-----------------------------------------------------------"
               "---------------------\n");
        ...
        if (invalid_value != arg_ql_index) {
            ...
            if (bcm_esmc_ql_unresolvable == arg_ql) {
                printk("TDPLL ESMC SET ... : Incompatible QL and ITU-T G.781 networking option.\n");
            } else {
        ...
        if (invalid_value != arg_holdover_ql_index) {
            ...
            if (bcm_esmc_ql_unresolvable == arg_holdover_ql) {
                printk("TDPLL ESMC SET ... : Incompatible Holdover QL and ITU-T G.781 networking option.\n");
            } else {
    ...
    bcm_ptp_ctdev_alarm_data_t *ctdev_alarm_data)
{
     ...
     */
    soc_cm_print("C-TDEV psec @T=%u: ", (unsigned)sal_time());
    for(i = 0; i < 11; ++i) {
        format_uint64_decimal(uint64_decimal_string, ctdev_alarm_data->tdev_psec[i], ',');
        soc_cm_print("| (%d) %s - %s ", ctdev_alarm_data->tau_sec[i], uint64_decimal_string, g823_mask_exceed[i] ? "F":"P");
    }
    soc_cm_print("\n");
...
diag_tdpll_input_clock_monitor_callback(
    ...
    bcm_tdpll_input_clock_monitor_cb_data_t *cb_data)
{
    printk("T-DPLL Input Clock Monitoring Event\n");
    printk("Unit: %d   Stack: %d\n", unit, stack_id);
    printk("-----------------------------------------------------------"
           "---------------------\n");
    printk("Index : %d\n", cb_data->index);
...
diag_tdpll_input_clock_selector_callback(
    ...
    bcm_tdpll_input_clock_selector_cb_data_t *cb_data)
{
    printk("T-DPLL Reference Selection Event\n");
    printk("Unit: %d   Stack: %d\n", unit, stack_id);
    printk("-----------------------------------------------------------"
           "---------------------\n");
    printk("DPLL : %d\n", cb_data->dpll_index);
