 ...
 *
 * File:        test.c
 * Purpose:     Quickturn Verification Driver
 */
...
#endif
#if defined(BCM_ROBO_SUPPORT)
    case SOC_CHIP_BCM5324:      return(T5324);
    case SOC_CHIP_BCM5396:      return(T5396);
    case SOC_CHIP_BCM5389:      return(T5389);
    case SOC_CHIP_BCM5398:      return(T5398);
    case SOC_CHIP_BCM5397:      return(T5397);
    case SOC_CHIP_BCM5348:      return(T5348);
    case SOC_CHIP_BCM5347:      return(T5347);
    case SOC_CHIP_BCM5395:      return(T5395);
    case SOC_CHIP_BCM53242:      return(T53242);
    case SOC_CHIP_BCM53262:      return(T53262);
    case SOC_CHIP_BCM53115:      return(T53115);
    case SOC_CHIP_BCM53118:      return(T53118);
    case SOC_CHIP_BCM53101:      return(T53101);
    case SOC_CHIP_BCM53280:      return(T53280);
    case SOC_CHIP_BCM53125:      return(T53125);
    case SOC_CHIP_BCM53128:      return(T53118);
    case SOC_CHIP_BCM53600:      return(T53280);
    case SOC_CHIP_BCM89500:      return(T89500);
    case SOC_CHIP_BCM53010:      return(T53010);
    case SOC_CHIP_BCM53020:      return(T53020);
#endif
#if defined(BCM_SBX_SUPPORT)
    case SOC_CHIP_BCM88020:
	if (SOC_IS_SBX_FE2KXT(unit)) {
	    return(T88025);
	} else { 
	    return(T88020);
	}
    case SOC_CHIP_BME3200:      return(T88100);
    case SOC_CHIP_QE2000:       return(T88200);
    case SOC_CHIP_BM9600:       return(T88130);
    case SOC_CHIP_BCM88230:     return(T88230);
    case SOC_CHIP_BCM88030:     return(T88030);
#endif
    default:                    return(0);
    }
}
...
test_test_done(int u, test_t *test, void *fp)
{
    ...
    if (test->t_done_f) {               /* Call done function */
        if (test->t_done_f(u, fp)) {
            printk("Test: ERROR: Done function for test %d (%s) failed\n",
                   test->t_test, test->t_name);
            rv = -1;
        }
    }
 ...
 */
{
    ...
    if (!(test->t_flags & _test_chip(u))) {
        if (test_options & TEST_O_OVERRIDE) {
            printk("Warning: Running test %d (%s) not supported on %s\n", 
                   test->t_test, test->t_name, SOC_UNIT_GROUP(u));
        }
    }

    if (test_options & TEST_O_RUN) {
        printk("Test %d (%s) Started\n", test->t_test, test->t_name);
    }
    ...
    if (TEST_RUNNING != (error = setjmp(jmp))) { /* Control C handler */
        if (test_active) {
            if (cleanup) {
                printk("Warning: Cleanup aborted - "
                       "continue at your own risk\n");
            } else {
                cleanup = TRUE;
                printk("Warning: cleaning up active test: %s\n", 
                       test_active->t_name);
    ...
    if (TEST_RUNNING != (error = setjmp(test_active_jmp))) {
        ...
        for (i = 0; i < vloops; i++) {
            if (vloops > 1) {
                soc_cm_debug(DK_VERBOSE + DK_TESTS,
...
{
    if (status_report) {
        printk("-----+----+--- TEST STATUS REPORT ----+"
               "-----+-----+-----+-----+-----------\n");
    }

    printk("U/A/S|Test|            Test           |Loop | Run |Pass |Fail |"
           "  Arguments\n");
    printk("     | #  |            Name           |Count|Count|Count|Count|"
           "\n");
}
 ...
 */
{
    ...
    printk(" %c%c%c |%4d| %-26s|%5d|%5d|%5d|%5d| %s\n", 
 ...
 */
{
    ...
    while ((s = ARG_GET(a)) != NULL) {
        test = test_find(s);
        if (!test) {
            printk("%s: Unable to locate test: %s\n", ARG_CMD(a), s);
            result = CMD_FAIL;
            continue;
        } 
        if (!header) {
...
test_run_selected(int u)
{
    ...
    if (test_options & TEST_O_RANDOM) {
        printk("Test: Random mode not supported yet\n");
        return(TEST_STOP);
    }
 ...
 */
{
    ...
    if (!ARG_CNT(a)) {
        ...
        if (NULL == (test = test_find(ARG_CUR(a)))) {
            printk("%s: Unknown test: %s\n", ARG_CMD(a), ARG_CUR(a));
            return(CMD_FAIL);
        }

        if (!(test->t_flags & _test_chip(u)) && 
            !(test_options & TEST_O_OVERRIDE)) {
            printk("Error: Test %d (%s) not supported on %s\n"
                   "Error: use \"testmode +override\" to override and run\n",
                   test->t_test, test->t_name, SOC_UNIT_GROUP(u));
            return(CMD_FAIL);
        }
        if (SOC_IS_KATANA2(u) && (test->t_test == 53)) {
            printk("Error: Test %d (%s)"
                   "not supported on %s\n"
                   "Error: use \"TestRun 140 (DDR bist test)\" instead\n",
                   test->t_test, test->t_name, SOC_UNIT_GROUP(u));
            return(CMD_FAIL);
        }          
...
_test_select(int u, test_t *t, int only_if_available)
{
    ...
    if (only_if_available) {            /* Only select available tests */
        ...
        if (!(t->t_flags & chip)) {
            if (test_options & TEST_O_OVERRIDE) {
                printk("Warning: Selecting test %d (%s) "
                       "not supported on %s\n", 
 ...
 */
{
    ...
    while ((c = ARG_GET(a)) != NULL) {
        if (parse_mask(c, options, &test_options)) {
            printk("%s: Error: invalid option ignored: %s\n", ARG_CMD(a), c);
        }
    }
    return(CMD_OK);
}
