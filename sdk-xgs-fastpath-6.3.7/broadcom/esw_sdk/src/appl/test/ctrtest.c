/*
 * $Id: ctrtest.c 1.41 Broadcom SDK $
...
#if defined(BCM_ROBO_SUPPORT) || defined(BCM_ESW_SUPPORT) || defined(BCM_SIRIUS_SUPPORT) || defined(BCM_ARAD_SUPPORT) || defined(BCM_CALADAN3_SUPPORT)
    int			r;
...
#if defined(BCM_ESW_SUPPORT) || defined(BCM_SIRIUS_SUPPORT) || defined(BCM_ARAD_SUPPORT) || defined(BCM_CALADAN3_SUPPORT)
		...
		continue;
            ...
            for (i = 0; i < SOC_CTR_MAP_SIZE(unit, ctype); i++) {
                ...
                if (!SOC_COUNTER_INVALID(unit, reg) && 
                    ...
                    addr = soc_reg_addr_get(unit, reg, port, 0, &block, &at);
