                            ...
                            soc_info_misc_t *post_si)
{
    ...
    sal_memset(post_si, 0, sizeof(*post_si));

    /* Get current information */
    for (i = 0; i < SOC_MAX_NUM_PORTS; i++) {
        post_si->port_l2p_mapping[i] = si->port_l2p_mapping[i];
        post_si->port_p2l_mapping[i] = si->port_p2l_mapping[i];
        post_si->port_speed_max[i] = si->port_speed_max[i];
        post_si->port_speed[i] = -1;
        post_si->port_encap[i] = -1;
        ...
        if (IS_HG_PORT(unit, logic_port)) {
            post_si->port_encap[logic_port] = SOC_ENCAP_HIGIG2;
        } else {
            post_si->port_encap[logic_port] = SOC_ENCAP_IEEE;
        }
    }
    SOC_PBMP_ASSIGN(post_si->oversub_pbm, si->oversub_pbm);
    SOC_PBMP_ASSIGN(post_si->disabled_bitmap, si->all.disabled_bitmap);
                                ...
                                soc_port_resource_t *resource)
{
     ...
     */
    SOC_IF_ERROR_RETURN
        (_soc_td2p_post_soc_info_get(unit, nport, resource,
                                     &post_si));
    ...
    for (i = 0, pr = &resource[0]; i < nport; i++, pr++) {
        ...
        SOC_IF_ERROR_RETURN
            (_soc_td2p_phy_port_flex_valid(unit, pr->physical_port));
        ...
        SOC_IF_ERROR_RETURN
            (_soc_td2p_speed_valid(unit, pr->physical_port,
    ...
    SOC_IF_ERROR_RETURN
        (_soc_td2p_port_resource_oversub_validate(unit, nport, resource,
                                                  &post_si));

    /* Check mixed speed on sister ports */
    SOC_IF_ERROR_RETURN
        (_soc_td2p_port_resource_mixed_sisters_validate(unit, nport, resource,
                                                        &post_si));
 ...
 *
 * Pre-FlexPort array
 *     Contains current information for ports that are configured in the
 *     system and are to be modified after the FlexPort operation.
 *     This should include all mappings where ports are deleted
 ...
 *
 * Post-FlexPort array
 *     Contains information on new configuration for ports that will
    ...
    for (i = 0, pr = &post_resource[0]; i < post_count; i++, pr++) {
        ...
        for (port_idx = 0; port_idx < SOC_DRIVER(unit)->port_num_blktype;
            ...
            case SOC_BLK_XLPORT:
                ...
                if (si->port_speed_max[logic_port] < 10000) {    /* GE */
 ...
 *
 *      Only initialize those ports that are flagged as 'new' or 'remapped'.
 *
 *      If a port currently exists and the mapping is not changed after flex,
 *      the port is not reset.  This is done to preserve legacy behavior.
 *
                                    ...
                                    soc_port_resource_t *resource)
{
    ...
    for (i = 0, pr = &resource[0]; i < nport; i++, pr++) {
        ...
        if ((pr->flags & SOC_PORT_RESOURCE_NEW) ||
 ...
 *      It covers HW programming that is done during SOC MISC.
 *
 *      Only clear those 'delete' ports (physical port is -1) that are
 *      flagged as 'destroyed' or 'remapped' (i.e. NOT all 'delete' ports
 *      are detached).
 *
 *      If a port currently exists and the mapping is not changed after flex,
 *      the port is not reset.  This is done to preserve legacy behavior.
 ...
 *                   but is mapped to a different physical port.
 *
 *      Note that a port resource to be deleted (physical is -1) may not
 *      necessarily be destroyed.  For example, a logical port will
 *      continue to exist and the logical to physical port mapping is same:
 *
 *      See L1 where mapping is SAME
 *               L1 --> -1 (current mapped to P1, and set to 1 lane)
 *               L1 --> P1 (set to 4 lanes)
 *               ...
 * Parameters:
 *      unit           - (IN) Unit number.
 *      nport          - (IN) Number of elements in array resource.
                                    ...
                                    soc_port_resource_t *resource)
{
    ...
    for (i = 0, pr = &resource[0]; i < nport; i++, pr++) {
        ...
        /* Skip ports that are not for deletion */
        if (pr->physical_port != -1) {
            continue;
        }
    ...
    for (i = 0, pr = &resource[0]; i < nport; i++, pr++) {
        ...
        LOG_VERBOSE(BSL_LS_SOC_PORT,
                    (BSL_META_U(unit,
                                ...
                                "logical_port=%d physical_port=%d\n"),
                     pr->logical_port, pr->physical_port));
        SOC_IF_ERROR_RETURN(soc_esw_portctrl_add(unit, pr->logical_port, 0));
    }
                                ...
                                soc_td2p_info_t *pre_soc_info)
{
     ...
     *
     *   - Delete ports in SOC layer:
     *       + Clear ports in HW MISC
     *         (Detach ONLY for destroyed or remapped ports)
     ...
     *         (Attach ONLY for new or remapped ports)
     ...
     * - Port Mapping Delete:
     *   This is indicated when the physical port is set to (-1).
     ...
     * Remapped:  Logical port will continue to exist after FlexPort
     *            but is mapped to a different physical port.
     *            Ports are 'detached' and 'attached'.
     * New:       Logical port did not exist before FlexPort.
     *            Ports are 'attached'
     * Same:      Logical to physical port mapping remains the same.
     *            Port will NOT be 'detached' nor 'attached'.
     *
     * The 'detach' and 'attach' sequences re-programs a lot of PORT related
     * HW and SW data.  In order to preserve legacy API behavior
     * the 'detach' and  'attach' steps will be applied ONLY to port
     * mapping cases as explained above:
     * (i.e. SAME port mappings are not detached or attached):
     *     - DETACH:  DESTROY and REMAPPED port mappings
     *     - ATTACH:  NEW and REMAPPED port mappings
     *
     * In addition, NOTE that the 'PortMacro' module handles things
     * a bit different.  The Port Macro requires ALL ports, including
     * those with the SAME port mappings, to always be 'deleted'
     * and 'added' back.
     *
     * ------------------
     * SOC_INFO Update
...
soc_flexport_sw_dump(int unit) {
    ...
    LOG_CLI((BSL_META_U(unit,
                        "\n    eq_pbm Bitmap: %s"),
       SOC_PBMP_FMT(SOC_INFO(unit).eq_pbm,pfmt)));
