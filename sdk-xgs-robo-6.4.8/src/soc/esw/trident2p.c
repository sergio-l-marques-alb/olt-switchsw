...
soc_td2p_mem_config(int unit, uint16 dev_id, uint8 rev_id)
{
    ...
    switch (dev_id) {
       ...
       case BCM56868_DEVICE_ID:
            ...
            memState[EGR_MPLS_PRI_MAPPINGm].index_max = -1;
            memState[EGR_MPLS_EXP_MAPPING_1m].index_max = -1;
            memState[EGR_MPLS_EXP_MAPPING_2m].index_max = -1;
            memState[EGR_MPLS_EXP_PRI_MAPPINGm].index_max = -1;
...
soc_td2p_set_mmu_credit_limit(int unit, soc_port_t port)
{
   ...
   soc_EGR_MMU_CREDIT_LIMITm_field_set(unit, entry, ENABLEf, &enable);
   soc_EGR_MMU_CREDIT_LIMITm_field_set(unit, entry, CELL_COUNTSf, &credit_limit);   

   SOC_IF_ERROR_RETURN(
      WRITE_EGR_MMU_CREDIT_LIMITm(unit, MEM_BLOCK_ALL, phy_port, entry));
                                     ...
                                     _soc_mmu_rsvd_buffer_t *rsvd_buffer)
{
    ...
    for (i = start_pm; i < end_pm; i++) {
        ...
        if (pm_flex_enabled == 1) {
            ...
            start_port = (i * TD2P_NUM_PORTS_PER_TSC4) + 1;
