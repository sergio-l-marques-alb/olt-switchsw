...
#if defined(BCM_WARM_BOOT_SUPPORT) && defined(BCM_TRIDENT2PLUS_SUPPORT)
    if (SOC_IS_TRIDENT2PLUS(unit) && SOC_WARM_BOOT(unit) &&
        soc_td2p_is_flex_enable(unit) && (!upgrade_from_nosupport)) {
        LOG_VERBOSE(BSL_LS_SOC_COMMON,
               (BSL_META_U(unit,
...
_soc_trident2_tdm_init(int unit)
{
    ...
    PBMP_PORT_ITER(unit, port) {
        ...
        num_of_subport = 0;
        if (SOC_IS_TRIDENT2PLUS(unit) &&
            (_soc_td2_tdm.speed[phy_port] >= 120000)) {
            num_of_subport = 11;
        ...
        for (i = 0; i < num_of_subport; i++) {
            _soc_td2_tdm.port_state[phy_port + i + 1] = PORT_STATE_SUBPORT;
        }
    }
...
_soc_td2_ledup_init(int unit)
{
    ...
    if (SOC_IS_TRIDENT2PLUS(unit)) {
        rval = 0;
        for (pgw = 0; pgw < TD2P_PGWS_PER_DEV; pgw++) {
            block = PGW_CL_BLOCK(unit, pgw);
            port = SOC_BLOCK_PORT(unit, block);
            if (port < 0) { /* the PGW_CL block is not in use */
                continue;
            ...
            if (pgw % 2){
                SOC_IF_ERROR_RETURN
                        (soc_reg_get(unit, PGW_LED_CHAIN_OUT_CONFIGr, 
                                     port, 0, &rval64));
 
                soc_reg64_field32_set(unit, PGW_LED_CHAIN_OUT_CONFIGr,
                                  &rval64, DELAYf, 0x1);
                SOC_IF_ERROR_RETURN
                    (soc_reg_set(unit, PGW_LED_CHAIN_OUT_CONFIGr, 
                                 port, 0, rval64));
            } else {
                SOC_IF_ERROR_RETURN
                        (soc_reg_get(unit, PGW_LED_CHAIN_OUT_CONFIGr, 
    ...
    PBMP_PORT_ITER(unit, port) {
        ...
        if ((phy_port % 32) / 16) {
            ...
            if (si->port_num_lanes[port] >= 10) {
                phy_port -= 4;
            } else if (phy_port % 16 < 4) {
                phy_port += 12;
            } else if (phy_port % 16 < 8) {
                phy_port += 4;
            } else if (phy_port % 16 < 12) {
                phy_port -= 4;
            } else {
                phy_port -= 12;
            }
        ...
        if (phy_port >= 64) {
            phy_port = phy_port - 64;
            remap_reg = remap_led1_regs[phy_port / 4];
            remap_field = remap_fields[phy_port];
            data_ram_loc = &y_data_ram_loc;
        } else {
            remap_reg = remap_led0_regs[phy_port / 4];
            remap_field = remap_fields[phy_port];
...
_soc_trident2_misc_init(int unit)
{
    ...
    higig2 = soc_property_port_get(unit, LB_PORT(unit), spn_HIGIG2_HDR_MODE,
                                   soc_feature(unit,
                                   soc_feature_no_higig_plus) ? 1: 0)
                                   ? 1 : 0;
    soc_mem_field32_set(unit, EGR_ING_PORTm, entry, HIGIG2f, higig2);
                               ...
                               _soc_mmu_device_info_t *devcfg, int lossless)
{
    ...
    for (idx = 0; idx < 11; idx++) {
        ...
        if (SOC_IS_TRIDENT2PLUS(unit) && !lossless) {
            soc_reg_field_set(unit, MMU_THDR_QE_CONFIG1_PRIQr, 
                    &rval, DYNAMIC_ENABLEf, 1);
            soc_reg_field_set(unit, MMU_THDR_QE_CONFIG1_PRIQr, 
                    &rval, COLOR_LIMIT_DYNAMICf, 1);
    ...
    if (SOC_IS_TRIDENT2PLUS(unit)) {
        ...
        if (!soc_feature(unit, soc_feature_mpls)) {
            ...
            if ((MPLS_ENTRYm == mem)
                ...
                || (EGR_MPLS_PRI_MAPPINGm == mem)
                || (EGR_MPLS_EXP_MAPPING_1m == mem)
                || (EGR_MPLS_EXP_MAPPING_2m == mem)
                || (EGR_MPLS_VC_AND_SWAP_LABEL_TABLEm == mem)) return 1;
        }
    } else
#endif
    {
