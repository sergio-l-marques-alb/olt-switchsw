...
tdm_th_chk_get_cal_len(tdm_mod_t *_tdm, int cal_id)
{
    ...
    if (idx_s!=0 && idx_e!=0 && cal_main!=NULL){
        ...
        if (higig_port==BOOL_FALSE && 
            ...
            higig_boot==BOOL_FALSE ){
            switch (clk_freq){
                case 850: cal_len = LEN_850MHZ_EN; break;
                case 766:
                case 765: cal_len = LEN_765MHZ_EN; break;
                case 672: cal_len = LEN_672MHZ_EN; break;
                case 645: cal_len = LEN_645MHZ_EN; break;
                case 545: cal_len = LEN_545MHZ_EN; break;
                default:  cal_len = 0; break;
            }
        }
        else {
            switch (clk_freq){
                case 850: cal_len = LEN_850MHZ_HG; break;
                case 766:
                case 765: cal_len = LEN_765MHZ_HG; break;
                case 672: cal_len = LEN_672MHZ_HG; break;
                case 645: cal_len = LEN_645MHZ_HG; break;
                case 545: cal_len = LEN_545MHZ_HG; break;
                default:  cal_len = 0; break;
            }
        }
    }
                ...
                if (slot_req>1){
                    ...
                    for (j=0; j<cal_main_len && j<512; j++) {
                        ...
                        if (port_i==port_j){
                            ...
                            /* min */
                            if (tracker[j]<(space_min-1) && tracker[j]>0){
                                result = FAIL;
                                TDM_ERROR6("%s, %s, port %d slot %d, %d < %d\n",
                                   "[Checker: Linerate Jitter (MIN)]",
                                   pipe_str,
