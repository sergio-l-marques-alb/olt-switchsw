        ...
        if (soc_mem_field32_get(unit, VLAN_TABm, &vtab, VIRTUAL_PORT_ENf)) {
            ...
            for (i = 0; i < 3; i++) {
                ...
                BCM_IF_ERROR_RETURN(_bcm_esw_multicast_ipmc_read(unit, ipmc_group[i],
                                                                 ...
                                                                 &l3_pbmp));
                if (do_not_add[i]) {
                    SOC_PBMP_REMOVE(l2_pbmp, orig_pbmp);
                } else {
        ...
        if (soc_feature(unit, soc_feature_vlan_vfi_membership)){
            if ((flags & BCM_VLAN_PORT_INGRESS_ONLY)) {
                ...
                rv = bcm_td2p_vlan_vp_group_set(unit, vlan,
                    BCMI_VP_GROUP_INGRESS, BCM_GPORT_VP_GROUP_GET(port),TRUE);
                BCM_IF_ERROR_RETURN(rv); 
            }
            if ((flags & BCM_VLAN_PORT_EGRESS_ONLY)) {
                ...
                BCM_IF_ERROR_RETURN(rv); 
                rv = bcm_td2p_vlan_vp_group_set(unit, vlan,
                    BCMI_VP_GROUP_EGRESS, BCM_GPORT_VP_GROUP_GET(port),TRUE);
                BCM_IF_ERROR_RETURN(rv); 
            }
        } else
#endif /* BCM_TRIDENT2PLUS_SUPPORT */
...
bcm_esw_vlan_gport_delete(int unit, bcm_vlan_t vlan, bcm_gport_t port)
{
    ...
    if (soc_feature(unit, soc_feature_wlan) && BCM_GPORT_IS_WLAN_PORT(port)) {
        ...
        if (soc_feature(unit, soc_feature_vlan_vfi_membership)){
            rv = bcm_td2p_vlan_vp_group_set(unit, vlan,
                BCMI_VP_GROUP_INGRESS, BCM_GPORT_VP_GROUP_GET(port), FALSE);
            BCM_IF_ERROR_RETURN(rv);

            rv = bcm_td2p_vlan_vp_group_set(unit, vlan,
                BCMI_VP_GROUP_EGRESS, BCM_GPORT_VP_GROUP_GET(port), FALSE);
            BCM_IF_ERROR_RETURN(rv);
        } else
#endif /* BCM_TRIDENT2PLUS_SUPPORT */
        {
            ...
            if (soc_feature(unit, soc_feature_vlan_vfi_membership)){
                if (bcm_td2p_ing_vp_group_unmanaged_get(unit)) {
                    ...
                    rv = bcm_td2p_vlan_vp_group_set(unit,
                            vlan, BCMI_VP_GROUP_INGRESS, -1, FALSE);
                    if (rv != BCM_E_NONE) {
                        soc_mem_unlock(unit, VLAN_TABm);
                        return rv;
                    }
                ...
                if (bcm_td2p_egr_vp_group_unmanaged_get(unit)) {
                    /* clear the vp group bitmap for this vlan */
                    rv = bcm_td2p_vlan_vp_group_set(unit,
                            vlan, BCMI_VP_GROUP_EGRESS, -1, FALSE);
                    if (rv != BCM_E_NONE) {
                        soc_mem_unlock(unit, VLAN_TABm);
                        return rv;
                    }
        ...
        *flags=0;
        ...
        if (soc_feature(unit, soc_feature_vlan_vfi_membership)) {
            rv = bcm_td2p_vlan_vp_group_get(unit, vlan,
                BCMI_VP_GROUP_INGRESS, BCM_GPORT_VP_GROUP_GET(port), &enable);
            BCM_IF_ERROR_RETURN(rv);
            if (enable) {
                *flags = BCM_VLAN_PORT_INGRESS_ONLY;
            }
            rv = bcm_td2p_vlan_vp_group_get(unit, vlan,
                BCMI_VP_GROUP_EGRESS, BCM_GPORT_VP_GROUP_GET(port), &enable);
            BCM_IF_ERROR_RETURN(rv);
            if (enable) {
