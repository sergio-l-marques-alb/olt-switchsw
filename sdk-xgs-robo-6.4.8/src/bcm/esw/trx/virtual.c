...
#define _BCM_NETWORK_VP_USED_GET(_u_, _vp_) \
        SHR_BITGET(VIRTUAL_INFO(_u_)->vp_network_vp_bitmap, (_vp_))
...
#define _BCM_VLAN_VFI_USED_GET(_u_, _vfi_) \
        SHR_BITGET(VIRTUAL_INFO(_u_)->vlan_vfi_bitmap, (_vfi_))
...
_bcm_vfi_init(int unit, int vfi)
{
    ...
    if (soc_feature(unit, soc_feature_vlan_vfi_membership)) {
        ...
        soc_VFIm_field32_set(unit, &vfi_entry, STGf, BCM_STG_DEFAULT);

        soc_EGR_VFIm_field32_set(unit, &egr_vfi_entry, STGf, BCM_STG_DEFAULT);
