...
bcm_sb2_failover_sync(int unit)
{
    ...
    for (index = 0; index < (2 * soc_mem_index_count (unit, MMU_INITIAL_NHOP_TBLm));
            index++)
    {
        *(((uint32 *)ptr)) = failover_keeper[index].primary_nhi;
        ptr += sizeof(uint32);
        *(((uint32 *)ptr)) = failover_keeper[index].secondary_nhi;
        ptr += sizeof(uint32);
...
bcm_sb2_failover_reinit(int unit)
{
    ...
    for (index = 0; index < (2 * soc_mem_index_count (unit, MMU_INITIAL_NHOP_TBLm));
            index++) {
        failover_keeper[index].primary_nhi = *(((uint32 *)ptr));
        ptr += sizeof(uint32);
        failover_keeper[index].secondary_nhi = *(((uint32 *)ptr));
        ptr += sizeof(uint32);
        if (*(((uint32 *)ptr)) == 1) {
...
bcm_sb2_failover_destroy(int unit, int failover_id) 
{
    ...
    if (failover_keeper[failover_id].primary_nhi != -1) {
        ...
        BCM_IF_ERROR_RETURN (soc_mem_read(unit, MMU_INITIAL_NHOP_TBLm,
                    MEM_BLOCK_ANY, 
                    failover_keeper[failover_id].primary_nhi / 4,
                    &mmu_prot_nhi_entry));

        soc_mem_field32_set(unit, MMU_INITIAL_NHOP_TBLm,
                &mmu_prot_nhi_entry,
                field[failover_keeper[failover_id].primary_nhi % 4] , 0);

        BCM_IF_ERROR_RETURN (soc_mem_write(unit, MMU_INITIAL_NHOP_TBLm,
                    MEM_BLOCK_ALL, failover_keeper[failover_id].primary_nhi / 4 ,
                    &mmu_prot_nhi_entry));

        BCM_IF_ERROR_RETURN (soc_mem_read(unit, MMU_INITIAL_NHOP_TBLm,
                    MEM_BLOCK_ANY, 
                    failover_keeper[failover_id].secondary_nhi / 4,
                    &mmu_prot_nhi_entry));

        soc_mem_field32_set(unit, MMU_INITIAL_NHOP_TBLm,
                &mmu_prot_nhi_entry,
                field[failover_keeper[failover_id].secondary_nhi % 4] , 0);

        BCM_IF_ERROR_RETURN (soc_mem_write(unit, MMU_INITIAL_NHOP_TBLm,
                    MEM_BLOCK_ALL, failover_keeper[failover_id].secondary_nhi / 4 ,
                    &mmu_prot_nhi_entry));

        failover_keeper[failover_id].primary_nhi = -1;
        failover_keeper[failover_id].secondary_nhi = -1;
    }
    return BCM_E_NONE;
}
/*
                                     ...
                                     int value)
{
    ...
    if ((failover_keeper[failover->failover_id].secondary_nhi == -1) ||
        (failover_keeper[failover->failover_id].primary_nhi == -1)) {
