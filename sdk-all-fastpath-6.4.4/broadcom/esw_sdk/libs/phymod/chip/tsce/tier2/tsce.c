...
int tsce_core_info_get(const phymod_core_access_t* phy, phymod_core_info_t* info)
{
    ...
    if ((serdes_id & 0x3f) == TSCE4_MODEL) {
        ...
        strncpy(info->name, "Tsce4A0", strlen("Tsce4A0")+1); 
    } else {
        info->core_version = phymodCoreVersionTsce12A0;
        strncpy(info->name, "Tsce4A0", strlen("Tsce12A0")+1); 
    }
...
{
    PHYMOD_IF_ERR_RETURN
        (eagle_tsc_read_rx_afe(&phy->access, RX_AFE_PF, (int8_t *)&rx->peaking_filter.value));
    PHYMOD_IF_ERR_RETURN
        (eagle_tsc_read_rx_afe(&phy->access, RX_AFE_PF2, (int8_t *)&rx->low_freq_peaking_filter.value));
    PHYMOD_IF_ERR_RETURN
        (eagle_tsc_read_rx_afe(&phy->access, RX_AFE_VGA, (int8_t *)&rx->vga.value));
    PHYMOD_IF_ERR_RETURN
        (eagle_tsc_read_rx_afe(&phy->access, RX_AFE_DFE1, (int8_t *)&rx->dfe[0].value));
    PHYMOD_IF_ERR_RETURN
        (eagle_tsc_read_rx_afe(&phy->access, RX_AFE_DFE2, (int8_t *)&rx->dfe[1].value));
    PHYMOD_IF_ERR_RETURN
        (eagle_tsc_read_rx_afe(&phy->access, RX_AFE_DFE3, (int8_t *)&rx->dfe[2].value));
    PHYMOD_IF_ERR_RETURN
        (eagle_tsc_read_rx_afe(&phy->access, RX_AFE_DFE4, (int8_t *)&rx->dfe[3].value));
    PHYMOD_IF_ERR_RETURN
        (eagle_tsc_read_rx_afe(&phy->access, RX_AFE_DFE5, (int8_t *)&rx->dfe[4].value));
    return PHYMOD_E_NONE;
}
...
{  
    ...
    if (flags & PHYMOD_INTF_F_SET_SPD_TRIGGER) {
        if ((config->interface_type == phymodInterfaceCR10)  ||
            ...
            spd_intf =  (config->data_rate == 120000) ?
                        TEMOD_SPD_120G_CR12 : TEMOD_SPD_100G_CR10;
            PHYMOD_IF_ERR_RETURN
    ...
    if  (config->interface_type == phymodInterfaceSGMII) {
        ...
        switch (config->data_rate) {
            ...
            if (PHYMOD_INTF_MODES_HIGIG_GET(config)) {
               if (config->data_rate == 5000) {
                ...
                } else if (config->data_rate == 12000) {
                    spd_intf = TEMOD_SPD_10000_XFI;
                } else if (config->data_rate == 10000) {
                    spd_intf = TEMOD_SPD_10000_XFI;
                } else {
        ...
        firmware_lane_config.MediaType = phymodFirmwareMediaTypePcbTraceBackPlane;
        spd_intf = TEMOD_SPD_40G_XLAUI;  /* should rename as 40G_MLD */
        dfe_adjust = 0 ;
    } else if (config->interface_type == phymodInterfaceKR4) {
        if(config->data_rate==42000) {
    ...
    } else if(config->interface_type == phymodInterfaceCR10) {
            spd_intf =  (config->data_rate == 120000) ?
                        TEMOD_SPD_120G_CR12 : TEMOD_SPD_100G_CR10;
    } else if(config->interface_type == phymodInterfaceCAUI) {
            spd_intf =  (config->data_rate == 120000) ?
                        TEMOD_SPD_120G_CR12 : TEMOD_SPD_100G_CR10;
    }
...
int tsce_phy_autoneg_set(const phymod_phy_access_t* phy, const phymod_autoneg_control_t* an)
{
    ...
    if (an->enable) {
        ctrl_port =1 ;
        PHYMOD_IF_ERR_RETURN
            (temod_set_an_port_mode(&phy->access, num_lane_adv_encoded, start_lane, ctrl_port));
    } else {
        ctrl_port =0 ;
        PHYMOD_IF_ERR_RETURN
            (temod_set_an_port_mode(&phy->access, num_lane_adv_encoded, start_lane, ctrl_port));
...
int tsce_core_init(const phymod_core_access_t* core, const phymod_core_init_config_t* init_config, const phymod_core_status_t* core_status)
{
    ...
    if (init_config->interface.ref_clock == phymodRefClk125Mhz) {
        PHYMOD_IF_ERR_RETURN
            (temod_refclk_set(&core_copy.access, TEMODREFCLK125MHZ)) ;
    } else {
        PHYMOD_IF_ERR_RETURN
            (temod_refclk_set(&core_copy.access, TEMODREFCLK156MHZ)) ;
    }
  ...
  /* NEED TO REVISIT */
    ...
    PHYMOD_IF_ERR_RETURN
        (temod_mld_am_timers_set(&core->access));
    PHYMOD_IF_ERR_RETURN
...
int tsce_phy_eee_set(const phymod_phy_access_t* phy, uint32 enable)
{
    PHYMOD_IF_ERR_RETURN(temod_eee_control_set(&phy->access,enable));
    return PHYMOD_E_NONE;
}

int tsce_phy_eee_get(const phymod_phy_access_t* phy, uint32* enable)
{
    uint32 enable_local;
