 ...
 *  Revision      :  $Id: falcon_tsc_dv_functions.c 869 2015-01-23 00:24:44Z kirand $ *
 *                                                                                    *
...
err_code_t falcon_tsc_config_shared_tx_pattern( const phymod_access_t *pa, uint8_t patt_length, const char pattern[]) {
  ...
  if ((str_len > 2) && ((USR_STRNCMP(pattern, "0x", 2)) == 0)) {
    ...
    for (i=2; i < str_len; i++) {
      ...
      USR_STRNCAT(patt_mod,bin,USR_STRLEN(bin));  
      if (pattern[i] != '_') {
  ...
  for (i=zero_pad_len; i + patt_length < 241; i = i + patt_length) {  
    USR_STRNCAT(patt_final,patt_mod,USR_STRLEN(patt_mod));
    j++; 
  }
...
err_code_t falcon_tsc_core_pwrdn( const phymod_access_t *pa, enum srds_core_pwrdn_mode_enum mode) {
  ...
  switch(mode) {
    ...
    case PWRDN:
            ...
            err_code = falcon_tsc_delay_ns(500);                     /* wait for more than 50 comclk cycles  */
...
/***********************************************/
  ...
  err_code_t falcon_tsc_ucode_mdio_load( const phymod_access_t *pa, uint8_t *ucode_image, uint16_t ucode_len) {
    ...
    wrc_micro_ra_wrdatasize(0x1);                         /* Select 16bit transfers */
    ...
    wrc_micro_ra_wrdatasize(0x2);                         /* Select 32bit transfers as default */
  ...
  err_code_t falcon_tsc_ucode_load_verify( const phymod_access_t *pa, uint8_t *ucode_image, uint16_t ucode_len) {
    ...
    wrc_micro_ra_rddatasize(0x1);                         /* Select 16bit transfers */
    ...
    wrc_micro_ra_rddatasize(0x2);                         /* Select 32bit transfers ad default */
...
/*************************************************/
  ...
  err_code_t falcon_tsc_wrw_uc_ram( const phymod_access_t *pa, uint16_t addr, uint16_t wr_val) {        
    ...
    wrc_micro_autoinc_wraddr_en(0);
    wrc_micro_ra_wrdatasize(0x1);                       /* Select 16bit write datasize */    
    wrc_micro_ra_wraddr_msw(0x2000);                    /* Upper 16bits of RAM address to be written to */
  ...
  uint16_t falcon_tsc_rdw_uc_ram( const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr) {      
   ...
   if(!err_code_p) {
    ...
    wrc_micro_autoinc_rdaddr_en(0);
    wrc_micro_ra_rddatasize(0x1);                       /* Select 16bit read datasize */
    wrc_micro_ra_rdaddr_msw(0x2000);                    /* Upper 16bits of RAM address to be read */
  ...
  uint8_t falcon_tsc_rdb_uc_ram( const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr) {      
    ...
    wrc_micro_autoinc_rdaddr_en(0);
    wrc_micro_ra_rddatasize(0x0);                       /* Select 8bit read datasize */
    wrc_micro_ra_rdaddr_msw(0x2000);                    /* Upper 16bits of RAM address to be read */
    wrc_micro_ra_rdaddr_lsw(addr);                      /* Lower 16bits of RAM address to be read */
  ...
  err_code_t falcon_tsc_rdblk_uc_ram( const phymod_access_t *pa, uint8_t *mem, uint16_t addr, uint16_t cnt) {      
      ...
      do {
          mem[i] = falcon_tsc_rdb_uc_ram( pa, &err_code,addr+i);         /* Value read from the required Data RAM Address */
          if(err_code) {
              return(_error(err_code));
          }
          i++;
