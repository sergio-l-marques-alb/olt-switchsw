/*
 * $Id: vswitch.h,v 1.21 Broadcom SDK $
...
typedef struct _bcm_tr_vswitch_bookkeeping_s {
    ...
    PARSER_HINT_ARR uint32      *vsi_usage;
...
#define _BCM_DPP_VPN_BVID_MASK         0xf000
...
#define _BCM_DPP_VLAN_TO_BVID(_vpn_) \
    ((_vpn_) | _BCM_DPP_VPN_BVID_MASK)

#define _BCM_DPP_VLAN_IS_BVID(_vpn_) \
    (((_vpn_) & _BCM_DPP_VPN_BVID_MASK) == _BCM_DPP_VPN_BVID_MASK)
...
#define _BCM_DDP_VPN_TO_VSI(vpn) ((_BCM_DDP_IS_MP_VPN(vpn))?vpn:SOC_PPD_VSI_P2P_SERVICE)
...
int bcm_petra_vswitch_destroy_internal(int unit,bcm_vlan_t vsi, int is_port_delete);
