...
typedef enum
{
   ...
   */
  SOC_PPC_NOF_PORT_DEFINED_VT_PROFILES = 8
...
typedef struct
{
  ...
  /*
   *  Enable Extended P2P service for traffic enters from this
   *  port. When lookup in the LIF table fails, then the
   *  behavior depends on this field:- If FALSE, then the
   *  packet will be assigned a default AC see
   *  soc_ppd_l2_lif_ac_add(), and the further processing of the
   *  packet will be identical if the lookup in the LIF table
   *  succeeded. - If TRUE, then the packet will be identified
   *  as part of Extended P2P service, and identical lookup
   *  will be performed in the LEM.
   *  REMARKS:
   *    1. even when extended-p2p is enabled in port, it's legal
   *       that lookup to LIF table success. and then packet
   *       handling will be according to LIF attributes
   *    2. when lookup into LIF table fails, then the IVE
   *       and service-type (p2p/MP) are taken from the port default lif-index
   *       and in order to preform extend p2p lookup in LEM
   *       service type has to be MP.
   */
  uint8 enable_incoming_extend_p2p;
  /*
   *  Enable Extended P2P service for traffic outgoing from
   *  this port. When packet arrives at the egress port with
   *  no AC, then port x VSI lookup will be performed only if
   *  this is FALSE; otherwise, the VSI is global for all
   *  extended P2P service and has no meaning in this context.
   */
  uint8 enable_outgoing_extend_p2p;

} SOC_PPC_PORT_EXTEND_P2P_INFO;
...
typedef struct
{
   ...
   *  soc_ppd_t20e_l2_lif_ac_map_key_set()Range: 0 - 7.for T20E only
   *  - TPID selection see
   ...
   *    SOC_PPD_TRAP_CODE_SAME_INTERFACE trap code is raised
   *  - if this filter occurs at egress (For unicast/multicast packets) then
   *    SOC_PPD_TRAP_EG_TYPE_HAIR_PIN is raised
  ...
  /*
   *  Per port Extended P2P information. Not relevant for T20E
   *  and may be ignored.
   */
  SOC_PPC_PORT_EXTEND_P2P_INFO extend_p2p_info;
  /*
   ...
   *  size in bytes.
   *  check is performed for bridged, routed packets
...
void
  SOC_PPC_PORT_EXTEND_P2P_INFO_clear(
    SOC_SAND_OUT SOC_PPC_PORT_EXTEND_P2P_INFO *info
  );
...
void
  SOC_PPC_PORT_EXTEND_P2P_INFO_print(
    SOC_SAND_IN  SOC_PPC_PORT_EXTEND_P2P_INFO *info
  );
