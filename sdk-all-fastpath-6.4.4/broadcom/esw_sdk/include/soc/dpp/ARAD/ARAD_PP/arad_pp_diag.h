...
#define ARAD_PP_DIAG_FLD_READ(prm_fld, prm_blk, prm_addr_msb, prm_addr_lsb, prm_fld_msb, prm_fld_lsb, prm_err_num)  \
...
#define ARAD_PP_DIAG_FLD_GET(blk, addr_msb, addr_lsb, fld_msb, fld_lsb, fld_val)  \
    do { \
        ...
        fld.msb = fld_msb; \
        fld.lsb = fld_lsb; \
        res = arad_pp_diag_dbg_val_get_unsafe(unit, blk, &fld, tmp_buff);  \
...
typedef enum
{
   ...
   * Auto generated. Do not edit following section {
   */
  ARAD_PP_DIAG_PKT_TRACE_OUT_OF_RANGE_ERR = ARAD_PP_ERR_DESC_BASE_DIAG_FIRST,
...
typedef struct
{
  uint32 trap_dest[ARAD_PP_NOF_TRAP_CODES * 4];

  uint8 already_saved;

  ARAD_PP_DIAG_MODE_INFO mode_info;
} ARAD_PP_SW_DB_DIAG;
...
#define ARAD_PP_DIAG_DBG_VAL_LEN        (24)
...
uint32
  ARAD_PP_DIAG_IPV6_VPN_KEY_verify(
    SOC_SAND_IN  int                       unit,
...
uint32
  arad_pp_diag_prge_first_instr_get(int unit, uint32 *first_instruction);
