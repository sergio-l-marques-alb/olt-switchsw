...
#define SOC_JER_FABRIC_SR_CELL_RECEIVED_NOF_SIDES        (4)
...
typedef struct {
    int dest_0[SOC_DPP_MAX_NOF_FAP_ID_MAPPED_TO_DEST_LIMIT];
    int dest_0_count;
    int dest_1[SOC_DPP_MAX_NOF_FAP_ID_MAPPED_TO_DEST_LIMIT];
    int dest_1_count;
    int dest_2[SOC_DPP_MAX_NOF_FAP_ID_MAPPED_TO_DEST_LIMIT];
    int dest_2_count;
} JER_MODID_GROUP_MAP;
...
* FUNCTION:
*     Enable Fabric SyncE configuration
* INPUT:
*  SOC_SAND_IN  int                                  unit -
*     Identifier of the device to access.
*  SOC_SAND_IN  int                                  is_master -
*     Configure master port/ slave port
*  SOC_SAND_IN/OUT  int/int*                         port_id-
*     port to be configured syncE.
*     (on get function the function returns an
*     indication wheather faric syncE is enabled)  
* RETURNS:
*   OK or ERROR indication.
* REMARKS: 
*   None. 
