...
uint32 dpp_mcds_copy_replications_from_arrays(
    ...
    SOC_SAND_IN  uint32    arrays_size,    /* size of output arrays */
    SOC_SAND_IN  uint32    *port_array,    /* array containing input logical ports/destinations */
    SOC_SAND_IN  soc_if_t  *encap_id_array /* array containing input encapsulations/CUDs/outlifs */
)
{
  dpp_mcds_base_t *mcds = dpp_get_mcds(unit);
...
)
{
  ...
  int failed = 1;
  uint32 recycle_workaround = SOC_IS_JERICHO(unit) ? 1 : 0; /* must be 0 or 1, do we need a recycle interface workaround for Jericho */
  ...
  if (recycle_workaround) { /* create the last ingress group empty for the work around */
    SOC_TMC_ERROR out_err = _SHR_E_NONE;
    SOCDNX_IF_ERR_EXIT(dpp_mult_ing_group_open(unit, SOC_DPP_CONFIG(unit)->tm.nof_ingr_mc_ids - recycle_workaround, NULL, 0, &out_err)); /* create the workaround group */
    if (out_err != _SHR_E_NONE) {
      DPP_MC_ASSERT(0);
      SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG("recycle workaround creation error")));
    }
  }
