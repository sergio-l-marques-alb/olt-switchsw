...
#define ARAD_ITM_VSQ_MIN_WRED_AVRG_TH_MNT_MSB       SOC_TMC_ITM_VSQ_MIN_WRED_AVRG_TH_MNT_MSB     
#define ARAD_ITM_VSQ_MIN_WRED_AVRG_TH_MNT_LSB       SOC_TMC_ITM_VSQ_MIN_WRED_AVRG_TH_MNT_LSB     
#define ARAD_ITM_VSQ_MIN_WRED_AVRG_TH_MNT_NOF_BITS  SOC_TMC_ITM_VSQ_MIN_WRED_AVRG_TH_MNT_NOF_BITS
#define ARAD_ITM_VSQ_MIN_WRED_AVRG_TH_EXP_MSB       SOC_TMC_ITM_VSQ_MIN_WRED_AVRG_TH_EXP_MSB     
#define ARAD_ITM_VSQ_MIN_WRED_AVRG_TH_EXP_LSB       SOC_TMC_ITM_VSQ_MIN_WRED_AVRG_TH_EXP_LSB     
#define ARAD_ITM_VSQ_MIN_WRED_AVRG_TH_EXP_NOF_BITS  SOC_TMC_ITM_VSQ_MIN_WRED_AVRG_TH_EXP_NOF_BITS

                                                                                                 
#define ARAD_ITM_VSQ_MAX_WRED_AVRG_TH_MNT_MSB       SOC_TMC_ITM_VSQ_MAX_WRED_AVRG_TH_MNT_MSB     
#define ARAD_ITM_VSQ_MAX_WRED_AVRG_TH_MNT_LSB       SOC_TMC_ITM_VSQ_MAX_WRED_AVRG_TH_MNT_LSB     
#define ARAD_ITM_VSQ_MAX_WRED_AVRG_TH_MNT_NOF_BITS  SOC_TMC_ITM_VSQ_MAX_WRED_AVRG_TH_MNT_NOF_BITS
#define ARAD_ITM_VSQ_MAX_WRED_AVRG_TH_EXP_MSB       SOC_TMC_ITM_VSQ_MAX_WRED_AVRG_TH_EXP_MSB     
#define ARAD_ITM_VSQ_MAX_WRED_AVRG_TH_EXP_LSB       SOC_TMC_ITM_VSQ_MAX_WRED_AVRG_TH_EXP_LSB     
#define ARAD_ITM_VSQ_MAX_WRED_AVRG_TH_EXP_NOF_BITS  SOC_TMC_ITM_VSQ_MAX_WRED_AVRG_TH_EXP_NOF_BITS
...
uint32
arad_itm_vsq_in_group_size_get(SOC_SAND_IN  ARAD_ITM_VSQ_GROUP       vsq_group_ndx, 
                               SOC_SAND_OUT uint32                   *vsq_in_group_size)
{
  ...
  case ARAD_ITM_VSQ_GROUP_CTGRY:
    *vsq_in_group_size = ARAD_ITM_VSQ_GROUPA_SZE;
    break;
  case ARAD_ITM_VSQ_GROUP_CTGRY_TRAFFIC_CLS:
    *vsq_in_group_size = ARAD_ITM_VSQ_GROUPB_SZE;
    break;
  case ARAD_ITM_VSQ_GROUP_CTGRY_2_3_CNCTN_CLS:
    *vsq_in_group_size = ARAD_ITM_VSQ_GROUPC_SZE;
    break;
  case ARAD_ITM_VSQ_GROUP_STTSTCS_TAG:
    *vsq_in_group_size = ARAD_ITM_VSQ_GROUPD_SZE;
    break;
  case ARAD_ITM_VSQ_GROUP_SRC_PORT:
    *vsq_in_group_size = ARAD_ITM_VSQ_GROUPE_SZE;
    break;
  case ARAD_ITM_VSQ_GROUP_PG:
    *vsq_in_group_size = ARAD_ITM_VSQ_GROUPF_SZE;
    break;
  default:
  ...
  )
{
  ...
  SOC_SAND_ERR_IF_ABOVE_MAX(
    is_cob_only, arad_itm_vsq_is_ocb_max(unit) - 1,
    SOC_SAND_VALUE_OUT_OF_RANGE_ERR, 15, exit
  );
  res = arad_itm_vsq_in_group_size_get(vsq_group_ndx, &vsq_in_group_size);
  SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
  ...
  )
{
  ...
  fld = FC_SET_FR_BDB_TH_LPf;
  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  40,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_read(unit, reg, REG_PORT_ANY, 0, FC_SET_FR_BDB_TH_LPf, &exp_man));
  arad_iqm_mantissa_exponent_get(unit, exp_man, bdbs_th_mnt_nof_bits, &mnt_val, &exp_val);
  ...
  )
{
  ...
  if (hw_q_type == ARAD_SW_DB_QUEUE_TYPE_NOT_AVAILABLE) {
      ...
      if (SOC_IS_JERICHO(unit)) {
        ...
        res = arad_sw_db_queue_type_ref_count_get(unit, hw_q_type, &ref_count);
  ...
  )
{
  ...
  entry_offset = (is_ocb_only * vsq_in_group_size) + vsq_in_group_ndx;
  ...
  )
{
  ...
  reg_val = 0;
  fld_val = (vsq_in_group_size * is_ocb_only) + vsq_in_group_ndx;
  ...
  )
{
  ...
  if (SOC_IS_JERICHO(unit)) {
      ...
      SOC_SAND_CHECK_FUNC_RESULT(arad_sw_db_queue_type_map_get(unit, info->old_cr_req_type_ndx, &old_hw_q_type), 44 ,exit);

      res = arad_sw_db_queue_type_ref_count_exchange(unit, core, old_hw_q_type, hw_q_type);
      ...
      line_index = queue_ndx / 32;
      line_shift = queue_ndx % 32;
      SOC_DPP_CORES_ITER(core, core_index) {
          SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res, 55, exit, ARAD_REG_ACCESS_ERR,
                                                   READ_IRR_QUEUE_IS_OCB_COMMITTEDm(unit, core_index, MEM_BLOCK_ANY, line_index, is_ocb_only_line));
          if (is_ocb_only) {
              SHR_BITSET(is_ocb_only_line, line_shift);
          } else {
              SHR_BITCLR(is_ocb_only_line, line_shift);
          }
          SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res, 55, exit, ARAD_REG_ACCESS_ERR,
                                                   WRITE_IRR_QUEUE_IS_OCB_COMMITTEDm(unit, core_index, MEM_BLOCK_ANY, line_index, is_ocb_only_line));
      }
  }
  ...
  )
{
  ...
  info->cr_req_type_ndx = info->old_cr_req_type_ndx = tolerance_level;
    ...
    for (iqm_dp_indx = 0; iqm_dp_indx <= ARAD_PP_DP_MAX_VAL; ++iqm_dp_indx)  {
      ...
      for (mtr_res_indx = 0; mtr_res_indx <= ARAD_PP_DP_MAX_VAL; ++mtr_res_indx) {
        ...
        if (SOC_DPP_CONFIG(unit)->arad->init.pp_enable) {
          ingress_mapped_mtr_res_indx = mtr_res_indx;
          egress_mapped_mtr_res_indx = mtr_res_indx;
          ...
          if (SOC_IS_ARADPLUS(unit) && soc_property_get(unit, spn_POLICER_COLOR_RESOLUTION_MODE, 0)) {
            ...
            if (mtr_res_indx == 1) {
              ingress_mapped_mtr_res_indx = 2;
              egress_mapped_mtr_res_indx = 2;
            }
            ...
            if (mtr_res_indx == 3) {
              ingress_mapped_mtr_res_indx = 3;
              egress_mapped_mtr_res_indx = 3;
            }
