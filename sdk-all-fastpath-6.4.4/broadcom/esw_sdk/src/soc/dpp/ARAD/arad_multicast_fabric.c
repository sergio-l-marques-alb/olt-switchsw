    ...
    )
{
    ...
    if (multicast_class_valid)
    {
        ...
        */
        ...
        rv = arad_port_to_interface_map_get(
                unit,
                core,
                mcast_class_port_id,
                &if_id,
        ...
        if (if_id != ARAD_IF_ID_NONE)
        {
            ...
            rv = arad_ofp_rates_sch_single_port_rate_sw_set(unit, core, mcast_class_port_id, 0);
            SOCDNX_IF_ERR_EXIT(rv);

            rv = arad_ofp_rates_sch_single_port_rate_hw_set(unit, core, mcast_class_port_id); 
            SOCDNX_IF_ERR_EXIT(rv);
        }
    }
