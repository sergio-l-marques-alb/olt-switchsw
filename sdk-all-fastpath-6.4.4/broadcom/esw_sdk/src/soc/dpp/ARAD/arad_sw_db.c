  ...
  )
{
  ...
  for (core = 0; core < SOC_DPP_DEFS_MAX(NOF_CORES); core++) {
      Arad_sw_db.arad_device_sw_db[unit]->arad_sw_db_egr_ports.erp_interface_id[core] = ARAD_IF_ID_NONE;
  }
...
{
  return Arad_sw_db.arad_device_sw_db[unit]->arad_sw_db_egr_ports.erp_interface_id[core];
}
  ...
  )
{
    ...
    ARAD_SW_DB_FIELD_SET(res, 
                         unit,
                         arad_sw_db_egr_ports.erp_interface_id[core],
                         &erp_interface_id
                         );
    ...
    res = arad_wb_db_egr_port_update_erp_interface_state(unit, core); 
    SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  ...
  )
{
    ...
    res = arad_wb_db_egr_port_update_calcal_length_state(unit, core); 
    SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
...
{
  return Arad_sw_db.arad_device_sw_db[unit]->arad_sw_db_egr_ports.calcal_length[core];
}
  ...
  )
{
    ...
    ARAD_SW_DB_FIELD_SET(res, 
                         unit,
                         arad_sw_db_egr_ports.chan_arb[core][chan_arb_id].nof_calcal_instances,
                         &nof_instances
                         );
    ...
    res = arad_wb_db_egr_port_update_nof_calcal_instances_state(unit, core, chan_arb_id); 
    SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  ...
  )
{
  ...
  return Arad_sw_db.arad_device_sw_db[unit]->arad_sw_db_egr_ports.chan_arb[core][chan_arb_id].nof_calcal_instances;
  ...
  )
{
  ...
  ARAD_SW_DB_FIELD_GET(
    unit,
    arad_sw_db_egr_ports.rates[use_core][base_q_pair],
    val
  );
}
   ...
   )
{
  ...
  *rate = Arad_sw_db.arad_device_sw_db[unit]->arad_sw_db_egr_ports.rates[use_core][base_q_pair].sch_rates;
  return SOC_SAND_OK;
}
   ...
   )
{
  ...
  for(i=0 ; i < nof_cores ; i++) {
      Arad_sw_db.arad_device_sw_db[unit]->arad_sw_db_egr_ports.rates[use_core + i][base_q_pair].sch_rates = rate;
        ...
        res = arad_wb_db_egr_port_update_rates_state(unit, use_core + i, base_q_pair); 
        SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
   ...
   )
{
  ...
  *rate = Arad_sw_db.arad_device_sw_db[unit]->arad_sw_db_egr_ports.rates[use_core][base_q_pair].egq_rates;
  return SOC_SAND_OK;
}
   ...
   )
{
  ...
  for(i=0 ; i < nof_cores ; i++) {
      Arad_sw_db.arad_device_sw_db[unit]->arad_sw_db_egr_ports.rates[use_core+i][base_q_pair].egq_rates = rate;
   ...
   )
{
  ...
  *is_valid = Arad_sw_db.arad_device_sw_db[unit]->arad_sw_db_egr_ports.rates[use_core][base_q_pair].valid;
  return SOC_SAND_OK;
}
   ...
   )
{
  ...
  for(i=0 ; i< nof_cores ; i++) {
        Arad_sw_db.arad_device_sw_db[unit]->arad_sw_db_egr_ports.rates[use_core+i][base_q_pair].valid = is_valid;
  ...
  )
{
  ...
  {
    val->queue_rate[idx].valid = Arad_sw_db.arad_device_sw_db[unit]->arad_sw_db_egr_ports.port_priority_cal[core].queue_rate[idx].valid;
    if (Arad_sw_db.arad_device_sw_db[unit]->arad_sw_db_egr_ports.port_priority_cal[core].queue_rate[idx].valid)
    {
      val->queue_rate[idx].egq_bursts = Arad_sw_db.arad_device_sw_db[unit]->arad_sw_db_egr_ports.port_priority_cal[core].queue_rate[idx].egq_bursts;
      val->queue_rate[idx].egq_rates = Arad_sw_db.arad_device_sw_db[unit]->arad_sw_db_egr_ports.port_priority_cal[core].queue_rate[idx].egq_rates;
    }
  }
  ...
  )
{
    ...
    ARAD_SW_DB_FIELD_SET(res, 
                         unit,
                         arad_sw_db_egr_ports.port_priority_cal[core].queue_rate[q_pair],
                         val
                         );
    ...
    res = arad_wb_db_egr_port_update_queue_rate_state(unit, core); 
    SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  ...
  )
{
  ...
  for (ps_idx = 0; ps_idx < ARAD_EGR_NOF_PS; ++ps_idx)
  {
    ...
    {
      val->tcg_rate[ps_idx][tcg_idx].valid = Arad_sw_db.arad_device_sw_db[unit]->arad_sw_db_egr_ports.tcg_cal[core].tcg_rate[ps_idx][tcg_idx].valid;
      if (Arad_sw_db.arad_device_sw_db[unit]->arad_sw_db_egr_ports.tcg_cal[core].tcg_rate[ps_idx][tcg_idx].valid)
      {
        val->tcg_rate[ps_idx][tcg_idx].egq_bursts = Arad_sw_db.arad_device_sw_db[unit]->arad_sw_db_egr_ports.tcg_cal[core].tcg_rate[ps_idx][tcg_idx].egq_bursts;
        val->tcg_rate[ps_idx][tcg_idx].egq_rates = Arad_sw_db.arad_device_sw_db[unit]->arad_sw_db_egr_ports.tcg_cal[core].tcg_rate[ps_idx][tcg_idx].egq_rates;
      }
    }    
  }
  ...
  )
{
    ...
    ARAD_SW_DB_FIELD_SET(res, 
                         unit,
                         arad_sw_db_egr_ports.tcg_cal[core].tcg_rate[ps][tcg_ndx],
                         val
                         );
    ...
    res = arad_wb_db_egr_port_update_tcg_rate_state(unit, core); 
    SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  ...
  )
{
  ...
  res = arad_sw_db_tm_queue_to_rate_class_mapping_is_simple_set(unit, ARAD_SW_DB_QUEUE_TO_RATE_CLASS_MAPPING_IS_UNDEFINED);
  SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
  ...
  )
{
    ...
    /* no overrun. stage must be at least 0. nof_levels can get -1 onlt if stage is at least ARAD_NOF_FP_DATABASE_STAGES.
     * In this case we will exit in the condition above. */
    /* coverity [overrun-buffer-arg:FALSE} */
    sal_memset(Arad_sw_db.arad_device_sw_db[unit]->pmf[stage].psl_info.levels_info,0x0, sizeof(ARAD_SW_DB_PMF_PSL_LEVEL_INFO) * 2 * nof_levels);
    for(indx = 0; indx < nof_levels; ++ indx) 
    {
  ...
  )
{
  ...
  SOC_SAND_CHECK_FUNC_RESULT(res, 140, exit);
    ...
    if (SOC_DPP_CONFIG(unit)->pp.l3_source_bind_mode != SOC_DPP_L3_SOURCE_BIND_MODE_DISABLE)
    {
		...
		res = soc_dpp_wb_engine_init_buffer(unit, SOC_DPP_WB_ENGINE_BUFFER_ARAD_SW_DB_SRC_BIND_SPOOF_ID_REF_CNT);
    ...
    res = sw_state_access[unit].dpp.soc.arad.tm.jer_modid_group_map.alloc(unit);
    SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 310, exit);

    if (SOC_IS_ARADPLUS(unit)) {
        res = sw_state_access[unit].dpp.soc.arad.tm.arad_plus.alloc(unit);
  ...
  )
{
  ...
  if (SOC_IS_ARADPLUS(unit)) {
    res = sw_state_access[unit].dpp.soc.arad.tm.arad_plus.free(unit);
    SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 7, exit);
  }

  res = sw_state_access[unit].dpp.soc.arad.tm.op_mode.free(unit);
  ...
  res = sw_state_access[unit].dpp.soc.arad.tm.jer_modid_group_map.free(unit);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 310, exit);

  res = sw_state_access[unit].dpp.soc.arad.tm.free(unit);
  ...
  )
{
  ...
  sal_memcpy(ce_instr, pgm_ce[cycle_ndx], sizeof(ARAD_SW_DB_PMF_CE)*ARAD_PMF_LOW_LEVEL_NOF_CE_IN_PROG_MAX_ALL_LEVELS);
...
uint32
  arad_sw_db_src_bind_spoof_id_ref_count_get(
    SOC_SAND_IN  int               unit,
    SOC_SAND_IN  uint16            spoof_id,
    SOC_SAND_OUT uint16           *ref_count
  )
{
    soc_error_t
        rv;
    ...
    rv  = SOC_DPP_WB_ENGINE_GET_ARR(unit,
                                    SOC_DPP_WB_ENGINE_VAR_ARAD_SW_DB_SRC_BIND_SPOOF_ID_REF_CNT,
                                    ref_count,
                                    spoof_id);
    SOC_SAND_SOC_CHECK_FUNC_RESULT(rv, 20, exit);
exit:
    SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_sw_db_src_bind_spoof_id_ref_count_get()",0,0);
}

uint32
  arad_sw_db_src_bind_spoof_id_ref_count_set(
    SOC_SAND_IN  int               unit,
    SOC_SAND_IN  uint16            spoof_id,
    SOC_SAND_IN  uint8             increase
  )
{
  uint16 ref_count = 0;
  soc_error_t
  ...
  rv = arad_sw_db_src_bind_spoof_id_ref_count_get(unit, spoof_id, &ref_count);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(rv, 10, exit);
  ...
  SOC_SAND_SOC_CHECK_FUNC_RESULT(rv, 20, exit);
exit:
    SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_sw_db_src_bind_spoof_id_ref_count_set()",0,0);
}
...
arad_sw_db_sw_dump(int unit) 	 
{
    ...
    for (i = 0; i < SOC_DPP_DEFS_GET(unit, nof_cores); i++) {
        LOG_VERBOSE(BSL_LS_SOC_SWDB,
                    (BSL_META_U(unit,
                                "\n calcal_length:        %u\n"),  db->arad_sw_db_egr_ports.calcal_length[i]));
    }

    for (i = 0; i < SOC_DPP_DEFS_GET(unit, nof_cores); i++) {
        for (j = 0; j < ARAD_EGR_NOF_Q_PAIRS; j++) {
            if(db->arad_sw_db_egr_ports.port_priority_cal[i].queue_rate[j].valid) {
                LOG_VERBOSE(BSL_LS_SOC_SWDB,
                            (BSL_META_U(unit,
                                        "\n queue_rate (%03d): valid %hhu egq_rates %u egq_bursts %u\n"), 
                                        i, 
                             db->arad_sw_db_egr_ports.port_priority_cal[i].queue_rate[j].valid, 
                             db->arad_sw_db_egr_ports.port_priority_cal[i].queue_rate[j].egq_rates, 
                             db->arad_sw_db_egr_ports.port_priority_cal[i].queue_rate[j].egq_bursts));
            }
        }
    }
    ...
    for (i = 0; i < SOC_DPP_DEFS_GET(unit, nof_cores); i++) {
        for (j = 0; j < SOC_DPP_DEFS_GET(unit, nof_channelized_calendars); j++) {
            LOG_VERBOSE(BSL_LS_SOC_SWDB,
                        (BSL_META_U(unit,
                                    "\n nof_calcal_instances (%02d):  %u\n"), i, db->arad_sw_db_egr_ports.chan_arb[i][j].nof_calcal_instances));
        }
    }
    ...
    for (core = 0; core < SOC_DPP_DEFS_GET(unit, nof_cores); core++) {
        for (i = 0; i < ARAD_EGR_NOF_PS; i++) {
            for (j = 0; j < ARAD_NOF_TCGS; j++) {
                if(db->arad_sw_db_egr_ports.tcg_cal[core].tcg_rate[i][j].valid) {
                    LOG_VERBOSE(BSL_LS_SOC_SWDB,
                                (BSL_META_U(unit,
                                            "\n eg_mult_nof_vlan_bitmaps (%02d, %01d): valid %hhu egq_rates %u egq_bursts %u\n"), 
                                            i, j, 
                                 db->arad_sw_db_egr_ports.tcg_cal[core].tcg_rate[i][j].valid, 
                                 db->arad_sw_db_egr_ports.tcg_cal[core].tcg_rate[i][j].egq_rates, 
                                 db->arad_sw_db_egr_ports.tcg_cal[core].tcg_rate[i][j].egq_bursts));
                }
            }
        }
    }
    ...
    )
{
    ...
    if (!SOC_DPP_CORE_MODE_IS_SYMMETRIC(unit)) {
        ARAD_ALLOC(Arad_sw_db.arad_device_sw_db[unit]->tm.sysport2basequeue, ARAD_SW_DB_SYSPORT_TO_BASE_QUEUE, ARAD_NOF_SYS_PHYS_PORTS_GET(unit) * SOC_DPP_CONFIG(unit)->core_mode.nof_active_cores, "sysport2basequeue");
    }
   ...
   )
{
    ...
    if (core_id != SOC_CORE_ALL){
        ...
        SHR_BITSET_RANGE(&core_bitmap, 0, SOC_DPP_CONFIG(unit)->core_mode.nof_active_cores);
    }

    sysport2basequeue = Arad_sw_db.arad_device_sw_db[unit]->tm.sysport2basequeue;
    SOCDNX_NULL_CHECK(sysport2basequeue);

    for (core_offset = 0 ;core_offset < SOC_DPP_CONFIG(unit)->core_mode.nof_active_cores ;core_offset++) {
...
uint32
  arad_sw_db_queue_type_ref_count_get(
    SOC_SAND_IN  int            unit,
    SOC_SAND_IN  uint8          q_type,
    SOC_SAND_OUT uint32*        ref_count)
{
    SOCDNX_INIT_FUNC_DEFS;
    if (!SOC_UNIT_NUM_VALID(unit)) {
    ...
    if (q_type >= SOC_TMC_ITM_NOF_QT_NDXS) {
        SOCDNX_EXIT_WITH_ERR(SOC_E_PARAM,(_BSL_SOCDNX_MSG("Invalid queue type: %d"), q_type));
    }
    *ref_count = Arad_sw_db.arad_device_sw_db[unit]->q_type_ref_count[q_type];

exit:
    SOCDNX_FUNC_RETURN;
}
uint32
  arad_sw_db_queue_type_ref_count_set(
    SOC_SAND_IN  int            unit,
    SOC_SAND_IN  uint8          q_type,
    SOC_SAND_IN  uint32         ref_count)
{
    ...
    Arad_sw_db.arad_device_sw_db[unit]->q_type_ref_count[q_type] = ref_count;
#ifdef BCM_WARM_BOOT_SUPPORT
    /* update warm boot file */
    SOCDNX_SAND_IF_ERR_EXIT(arad_wb_db_update_q_type_ref_count_state(unit, q_type));
#endif /* BCM_WARM_BOOT_SUPPORT */
...
{    
    ...
    if (orig_q_type != SOC_TMC_ITM_QT_NDX_INVALID) {
        ...
        rv = arad_sw_db_queue_type_ref_count_get(unit, orig_q_type, &orig_q_type_ref_count);
        ...
        rv = arad_sw_db_queue_type_ref_count_set(unit, orig_q_type, orig_q_type_ref_count);
        SOCDNX_IF_ERR_EXIT(rv);
    }

    /*increase original q type ref count*/
    rv = arad_sw_db_queue_type_ref_count_get(unit, new_q_type, &new_q_type_ref_count);
    SOCDNX_IF_ERR_EXIT(rv);

    new_q_type_ref_count += nof_queue_remaped;

    rv = arad_sw_db_queue_type_ref_count_set(unit, new_q_type, new_q_type_ref_count);
    SOCDNX_IF_ERR_EXIT(rv);
  ...
  )
{
  ...
  if (user_q_type == SOC_TMC_ITM_QT_NDX_INVALID) {
      ...
      *mapped_q_type = Arad_sw_db.arad_device_sw_db[unit]->q_type_map[user_q_type - ARAD_SW_DB_1ST_DYNAMIC_QUEUE_TYPE(unit)];
      if (!*mapped_q_type) {
  ...
  )
{
  ...
  if (ARAD_SW_DB_QUEUE_TYPE_IS_DYNAMIC(unit, user_q_type)) {
      uint8 *mapping = &Arad_sw_db.arad_device_sw_db[unit]->q_type_map[-ARAD_SW_DB_1ST_DYNAMIC_QUEUE_TYPE(unit)];
      if (SOC_IS_JERICHO(unit)) {
          ...
          if (!mapping[user_q_type]) {
              mapping[user_q_type] = user_q_type;
      ...
      res = arad_wb_db__update_q_type_map_state(unit, user_q_type - ARAD_SW_DB_1ST_DYNAMIC_QUEUE_TYPE(unit)); 
      SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
...
{
    uint32 i, nof_indices = SOC_DPP_CONFIG(unit)->tm.nof_mc_ids / SOC_SAND_NOF_BITS_IN_UINT32;
    uint32 remainder_bits =  SOC_DPP_CONFIG(unit)->tm.nof_mc_ids % SOC_SAND_NOF_BITS_IN_UINT32;
    uint32 bits = is_open ? 0xffffffff : 0;
    uint32* word;
    ...
    word = Arad_sw_db.arad_device_sw_db[unit]->arad_sw_db_multicast.egress_groups_open_data; /* get the pointer to the first word */
    for (i = 0; i < nof_indices; ++i, ++word) {
#ifdef BCM_WARM_BOOT_SUPPORT
        if (*word != bits) {
#endif /* BCM_WARM_BOOT_SUPPORT */
            *word = bits;
#ifdef BCM_WARM_BOOT_SUPPORT
        /* update warm boot file */
            SOCDNX_SAND_IF_ERR_EXIT(arad_wb_db_multicast_update_egress_groups_open_data_state(unit, i));
        }
#endif /* BCM_WARM_BOOT_SUPPORT */
    }
    if (remainder_bits) {
        bits &= (0xffffffff >> (SOC_SAND_NOF_BITS_IN_UINT32 - remainder_bits));
#ifdef BCM_WARM_BOOT_SUPPORT
        if (*word != bits) {
#endif /* BCM_WARM_BOOT_SUPPORT */
            *word = bits;
