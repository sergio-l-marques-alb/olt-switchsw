  ...
  )
{
  ...
  if (DPP_MCDS_TYPE_IS_INGRESS(entry_type)) { /* set ingress entry pointer */
    ...
    case 0:
      {
        ...
        if (port != DPP_MULT_EGRESS_PORT_INVALID) { /* replication not disabled for this entry part */
          if (++found <= max) {
            dpp_add_egress_replication_port_cud(mcds, 0, cud, port);
          }
        }
    ...
    case 3:
      {
        ...
        if (cud != DPP_MC_EGR_OUTLIF_DISABLED) { /* replication not disabled for this entry part */
          if (++found <= max) {
            dpp_add_egress_replication_cud(mcds, 0, cud);
          }
        }
    ...
    case 5:
      {
        ...
        if (port != DPP_MULT_EGRESS_PORT_INVALID) { /* replication not disabled for this entry part */
          if (++found <= max) {
            dpp_add_egress_replication_port_cud(mcds, 0, soc_mem_field32_get(unit, IRR_MCDB_EGRESS_FORMAT_4m, mcdb_entry, OUTLIF_1f), port);
          }
        }
        port = soc_mem_field32_get(unit, IRR_MCDB_EGRESS_FORMAT_4m, mcdb_entry, PP_DSP_2f);
        if (port != DPP_MULT_EGRESS_PORT_INVALID) { /* replication not disabled for this entry part */
          if (++found <= max) {
            dpp_add_egress_replication_port_cud(mcds, 0, soc_mem_field32_get(unit, IRR_MCDB_EGRESS_FORMAT_4m, mcdb_entry, OUTLIF_2f), port);
          }
        }
      }
      break;
    ...
    if (port != DPP_MULT_EGRESS_PORT_INVALID) { /* replication not disabled for this entry part */
          if (++found <= max) {
        dpp_add_egress_replication_cud(mcds, 0, port);
      }
    }
