...
)
{
    ...
    /*
    *    Each aging profile has 16 entries. Start from the last entry of the given aging profile
    */
    entry_offset = (aging_profile + 1) << ARAD_PP_FRWRD_MACT_AGE_NOF_BITS;
    ...
    for (age_ndx = (ARAD_PP_FRWRD_MACT_ENTRY_NOF_AGES-1); age_ndx >= 0; --age_ndx)
    {
        entry_offset = entry_offset - 2; /* every age has two entries (is_owned = TRUE/FALSE */
