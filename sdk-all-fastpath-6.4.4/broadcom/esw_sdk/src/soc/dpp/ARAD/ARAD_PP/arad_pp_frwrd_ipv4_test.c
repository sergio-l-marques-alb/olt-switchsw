...
/* $Id: arad_pp_frwrd_ipv4_test.c,v 1.22 Broadcom SDK $
  ...
  )
{
  ...
  if (reset_info->vrf == ARAD_PP_FRWRD_IP_ALL_VRFS_ID)
  {
    res = arad_pp_frwrd_ipv4_vrf_all_routing_tables_clear(unit,0);
  ...
  if (reset_info->vrf == 0)
  {
    res = arad_pp_frwrd_ipv4_uc_routing_table_clear(unit);
    SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
  }
  else
  {
    res = arad_pp_frwrd_ipv4_vrf_routing_table_clear(unit,reset_info->vrf);
    SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
  }
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(0);
    ...
    for (indx = 1; indx < lpm_mngr.init_info.nof_mems; ++indx)
    {
      ...
      res = arad_pp_frwrd_ipv4_mem_status_get(unit,indx,&mem_status);
      SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);

      ARAD_PP_FRWRD_IPV4_MEM_STATUS_print(&mem_status);
    }
  ...
  )
{
  ...
  ARAD_PP_IPV4_LPM_MNGR_INFO
    lpm_mngr;
  ARAD_PP_FRWRD_IPV4_MEM_DEFRAG_INFO
    defrag_info;
  uint32
    res;
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(0);
    ...
    for (indx = 1; indx < lpm_mngr.init_info.nof_mems; ++indx)
    {
      ...
      ARAD_PP_FRWRD_IPV4_MEM_DEFRAG_INFO_clear(&defrag_info);
      LOG_INFO(BSL_LS_SOC_FORWARD,
               (BSL_META_U(unit,
                           "\n\r\n\r Defrage bank: %u \n\r"),indx));

      res = arad_pp_frwrd_ipv4_mem_defrage (unit,indx,&defrag_info);
      SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
    }
...
/* lookup in SW-sim*/
  ...
  fec_id = arad_pp_ipv4_lpm_mngr_lookup0(&lpm_mngr, unit, lkup_info->key.vrf, lkup_info->key.key.subnet.ip_address);
  LOG_INFO(BSL_LS_SOC_FORWARD,
           (BSL_META_U(unit,
  ...
  )
{
  ...
  uint32
    ...
    ARAD_PP_FRWRD_IPV4_UC_ROUTE_KEY             route_key;
...
/************************************************************************/
  ...
  ARAD_PP_FRWRD_IPV4_UC_ROUTE_KEY_clear(&route_key);
  ...
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
    ...
    for(indx = 0; indx < nof_routes && rule_count < 10000 ; indx++) {
      ...
      if (!in_cache && (till_cache == 0 ||((indx - last_commit_index) % till_cache == till_cache-1) ))
      {
        ...
        arad_pp_frwrd_ip_routes_cache_mode_enable_set(unit,ARAD_PP_FRWRD_IP_ALL_VRFS_ID,ARAD_PP_FRWRD_IP_CACHE_MODE_IPV4_UC_LPM);
        in_cache = TRUE;
      }
      else if (in_cache &&  ((last_cached_index + till_commit == indx)))
      {
        ...
        /*arad_pp_frwrd_ip_routes_cache_mode_enable_set(0,0,0);*/
        arad_pp_frwrd_ip_routes_cache_commit(unit,/*vrf*/ARAD_PP_FRWRD_IP_CACHE_MODE_IPV4_UC_LPM,ARAD_PP_FRWRD_IP_ALL_VRFS_ID,&fail_inidcation);
        arad_pp_frwrd_ip_routes_cache_mode_enable_set(unit,ARAD_PP_FRWRD_IP_ALL_VRFS_ID,ARAD_PP_FRWRD_IP_CACHE_MODE_NONE);
        if (print_level > 0)
        {
          LOG_INFO(BSL_LS_SOC_FORWARD,
                   (BSL_META_U(unit,
      ...
      {
        res = arad_pp_frwrd_ipv4_uc_route_add(unit,&route_key,add_sys_fec_id,&fail_inidcation);
        SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
      ...
      if(rule_count > 0 && soc_sand_os_rand() % 101 >= 100 - remove_prob ) {
        ...
        if(selected_rule != 0) {
          ...
          /*arad_pp_ipv4_lpm_mngr_prefix_remove(Lpm_mngr, 0, &key, 0, &success);*/
          route_key.subnet.ip_address = key.ip_address;
          route_key.subnet.prefix_len = key.prefix_len;
          res = arad_pp_frwrd_ipv4_uc_route_remove(unit,&route_key,&fail_inidcation);
          SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
          if (fail_inidcation != SOC_SAND_SUCCESS)
          {
            LOG_INFO(BSL_LS_SOC_FORWARD,
      ...
      if (in_cache &&  ((indx == nof_routes-1 &&till_cache==0)))
      {
        ...
        /*arad_pp_frwrd_ip_routes_cache_mode_enable_set(0,0,0);*/
        arad_pp_frwrd_ip_routes_cache_commit(unit,/*vrf*/ARAD_PP_FRWRD_IP_CACHE_MODE_IPV4_UC_LPM,0,&fail_inidcation);
        arad_pp_frwrd_ip_routes_cache_mode_enable_set(unit,ARAD_PP_FRWRD_IP_ALL_VRFS_ID,ARAD_PP_FRWRD_IP_CACHE_MODE_NONE);
        in_cache = FALSE;
      }
      ...
      for(i=0; i < rule_count; i++) {
        ...
        if ((test & 2))
        {
          sys_fec_id = arad_pp_ipv4_lpm_mngr_lookup0(Lpm_mngr, 0, 0, key.ip_address);
        }
  ...
  )
{
  ...
  info->vrf = 0;
  ARAD_PP_FRWRD_IPV4_VPN_ROUTE_KEY_clear(&(info->key));
  SOC_SAND_MAGIC_NUM_SET;
exit:
  SOC_SAND_VOID_EXIT_AND_SEND_ERROR(0, 0, 0);
}
  ...
  )
{
  ...
  LOG_CLI((BSL_META_U(unit,
                      "key:\n\r")));
  ARAD_PP_FRWRD_IPV4_VPN_ROUTE_KEY_print(&(info->key));
exit:
  SOC_SAND_VOID_EXIT_AND_SEND_ERROR(0, 0, 0);
}
