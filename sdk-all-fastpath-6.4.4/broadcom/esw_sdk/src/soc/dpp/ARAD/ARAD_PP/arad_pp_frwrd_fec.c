  ...
  )
{
  ...
  res = arad_pp_ihb_fec_super_entry_tbl_get_unsafe(
          unit,
          ARAD_PP_FRWRD_FEC_SUPER_ENTRY_INDEX(fec_ndx),
          &ihb_fec_super_entry_tbl_data
        );
  SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
  ...
  )
{
  ...
  /* In Jericho, the LSB is ignored (FEC within a Super FEC) */
  if (SOC_IS_JERICHO(unit)) {
      oam_instance_ndx = fec_instance_ndx >> 1;
  }
  ...
  )
{
  ...
  SOC_SAND_ERR_IF_ABOVE_NOF(info->expected_in_rif, SOC_DPP_DEFS_GET(unit, nof_rifs), ARAD_PP_RIF_ID_OUT_OF_RANGE_ERR, 11, exit);
  ...
  )
{
  ...
  {
    SOC_SAND_ERR_IF_ABOVE_NOF(info->out_rif, SOC_DPP_DEFS_GET(unit, nof_rifs), ARAD_PP_RIF_ID_OUT_OF_RANGE_ERR, 11, exit);
  }
  ...
  )
{
  ...
  if (info->type == ARAD_PP_FEC_TYPE_IP_MC && info->dest.dest_type != SOC_SAND_PP_DEST_MULTICAST)
  {
    SOC_SAND_SET_ERROR_CODE(ARAD_PP_FRWRD_FEC_IPMC_DEST_NOT_MC_ERR, 35,exit);
  }
