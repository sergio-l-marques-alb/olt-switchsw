...
arad_fast_reg_get(int unit, soc_reg_t reg, int acc_type, int addr, int block, soc_reg_above_64_val_t data)
{
    ...
    if (SOC_IS_DONE_INIT(unit))
    {
        ...
        {
            if(SOC_INFO(unit).ha_hw_log_access.fast_reg_get)
            {
                return SOC_INFO(unit).ha_hw_log_access.fast_reg_get(unit, reg, acc_type, addr, block, data);
            }
        }
    }
    ...
    arad_fast_reg_set(int unit, soc_reg_t reg, int acc_type, int addr, int block, soc_reg_above_64_val_t data)
{
    ...
    if (SOC_IS_DONE_INIT(unit))
    {
        if (BCM_UNIT_DO_HW_READ_WRITE(unit))
        {
            if(SOC_INFO(unit).ha_hw_log_access.fast_reg_set)
            {
                return SOC_INFO(unit).ha_hw_log_access.fast_reg_set(unit, reg, acc_type, addr, block, data);
            }
        }
    }
  ...
  )
{
  ...
  if (SOC_IS_DONE_INIT(unit))
  {
      if (BCM_UNIT_DO_HW_READ_WRITE(unit))
      {
          ...
          {
              return SOC_INFO(unit).ha_hw_log_access.polling(
                   unit,
                   time_out,
                   min_polls,
                   reg,
