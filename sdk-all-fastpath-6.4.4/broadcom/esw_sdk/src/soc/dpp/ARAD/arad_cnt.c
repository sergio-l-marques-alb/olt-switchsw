    ...
    )
{
  ...
  case ARAD_CNT_SRC_TYPE_ING_PP_2_LSB:
      fld_val = ARAD_CNT_SRC_TYPE_ING_PP_FLD_VAL(unit, src_core, command_id, _LSB);
      break;
  case ARAD_CNT_SRC_TYPE_ING_PP_MSB:
  case ARAD_CNT_SRC_TYPE_ING_PP_2_MSB:
      fld_val = ARAD_CNT_SRC_TYPE_ING_PP_FLD_VAL(unit, src_core, command_id, _MSB);
      break;
  ...
  case ARAD_CNT_SRC_TYPE_EGR_PP:
      fld_val = ARAD_CNT_SRC_TYPE_EGR_PP_FLD_VAL(unit, src_core, command_id, _LSB);
      break;
  case ARAD_CNT_SRC_TYPE_EG_PP_MSB:
      fld_val = ARAD_CNT_SRC_TYPE_EGR_PP_FLD_VAL(unit, src_core, command_id, _MSB);
      break;
  ...
  case ARAD_CNT_SRC_TYPE_OAM_ING_D:
    fld_val = ARAD_CNT_SRC_TYPE_OAM_FLD_VAL(unit, src_core, (src_type - ARAD_CNT_SRC_TYPE_OAM_ING_A));
    break;
  case ARAD_CNT_SRC_TYPE_EPNI_A:
  case ARAD_CNT_SRC_TYPE_EPNI_B:
      fld_val = ARAD_CNT_SRC_TYPE_EPNI_FLD_VAL(unit, src_core, command_id, _LSB);
      break;
  case ARAD_CNT_SRC_TYPE_EPNI_C:
  case ARAD_CNT_SRC_TYPE_EPNI_D:
      fld_val = ARAD_CNT_SRC_TYPE_EPNI_FLD_VAL(unit, src_core, command_id, _MSB);
      break;
  case ARAD_CNT_NOF_SRC_TYPES_EGQ_TM:
      fld_val = ARAD_CNT_SRC_TYPE_EGQ_TM_FLD_VAL(unit,src_core);
      break;
  case ARAD_CNT_NOF_SRC_TYPES_IPT_LATENCY:
       ...
       SOC_SAND_IN  ARAD_CNT_COUNTERS_INFO *info)
{
   ...
   if (info->src_type == ARAD_CNT_SRC_TYPE_OAM_ING_A ||
       info->src_type == ARAD_CNT_SRC_TYPE_OAM_ING_B ||
       info->src_type == ARAD_CNT_SRC_TYPE_OAM_ING_C ||
       info->src_type == ARAD_CNT_SRC_TYPE_OAM_ING_D ||
       info->src_type == ARAD_CNT_NOF_SRC_TYPES_IPT_LATENCY){
       /* Always 1 count for OAM */
       /* Always 1 count for IPT latency*/
    ...
    if (SOC_IS_JERICHO(unit)) {
        ...
        res = _arad_cnt_counter_bmap_mem_by_src_type_get(unit, proc_id, info->src_type, &src_btmap_mem, bmap_fields);
        SOC_SAND_CHECK_FUNC_RESULT(res, 19, exit);
        /*entries iteration- iter over all including the filter entry*/
        for(entry_index = 0 ; entry_index < ARAD_CNT_BMAP_OFFSET_COUNT; entry_index++){
          /*iter on bmap*/
      ...
      )
{
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  25,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field64_modify(unit, base_val_reg, src_core, 0, base_val_fld + (COUNTER_0_MODE_0_BASEf - COUNTER_0_MODE_0_BASEf),  field64));
    COMPILER_64_SET(field64,0,base_val_tm);    
    SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  26,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field64_modify(unit, base_val_reg, src_core, 0, base_val_fld + (COUNTER_0_MODE_1_BASEf - COUNTER_0_MODE_0_BASEf),  field64));
    COMPILER_64_SET(field64,0,base_val_vsi);
    SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  27,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field64_modify(unit, base_val_reg, src_core, 0, base_val_fld + (COUNTER_0_MODE_2_BASEf - COUNTER_0_MODE_0_BASEf),  field64));
    COMPILER_64_SET(field64,0,base_val_outlif);
    SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  28,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field64_modify(unit, base_val_reg, src_core, 0, base_val_fld + (COUNTER_0_MODE_3_BASEf - COUNTER_0_MODE_0_BASEf),  field64));

    
    for(pp_port_ndx = 0;  pp_port_ndx < SOC_TMC_NOF_FAP_PORTS_PER_CORE;  pp_port_ndx++)
    {
        SOC_SAND_SOC_IF_ERROR_RETURN(res, 29, exit, READ_EPNI_PP_COUNTER_TABLEm(unit, EPNI_BLOCK(unit, src_core), pp_port_ndx, data));
        ...
        SOC_SAND_SOC_IF_ERROR_RETURN(res, 30, exit, WRITE_EPNI_PP_COUNTER_TABLEm(unit, EPNI_BLOCK(unit, src_core), pp_port_ndx, data));
    }
exit:
...
if (!SAL_BOOT_PLISIM ) {
  ...
  if (info->src_type == SOC_TMC_CNT_SRC_TYPE_ING_PP || info->src_type == SOC_TMC_CNT_SRC_TYPE_ING_PP_2_LSB || 
      info->src_type == SOC_TMC_CNT_SRC_TYPE_ING_PP_MSB || info->src_type == SOC_TMC_CNT_SRC_TYPE_ING_PP_2_MSB) {
  ...
  )
{
  ...
  if (SOC_IS_JERICHO(unit) && info->we_val > SOC_DPP_DEFS_GET(unit, nof_counter_processors)) {
      /*The Wtite Enable bitmap is limited to the number of regular counter processors*/
  ...
  )
{
 ...
 if (!SOC_UNIT_NUM_VALID(unit)) {
  ...
  ARAD_CNT_SRC_TYPE_LSB_GET(unit, fld_val, lsb);
  ...
  if (ARAD_CNT_SRC_TYPE_IS_EGR_PP_FLD(unit,fld_val)) {
      ...
      if (lsb == _LSB && info->command_id == 0) {
          info->src_type = ARAD_CNT_SRC_TYPE_ING_PP;
      } else if (lsb == _LSB && info->command_id == 1) {
          info->src_type = ARAD_CNT_SRC_TYPE_ING_PP_2_LSB;
      } else if (lsb == _MSB && info->command_id == 0) {         
          info->src_type = ARAD_CNT_SRC_TYPE_ING_PP_MSB;
      } else if (lsb == _MSB && info->command_id == 1) {
          info->src_type = ARAD_CNT_SRC_TYPE_ING_PP_2_MSB;
      }
  } else if (ARAD_CNT_SRC_TYPE_IS_OAM_FLD(unit, fld_val)) {
      if (info->command_id == 0) {
          info->src_type = ARAD_CNT_SRC_TYPE_OAM_ING_A;
      } else if (info->command_id == 1) {
            info->src_type = ARAD_CNT_SRC_TYPE_OAM_ING_B;
      } else if (info->command_id == 2) {
            info->src_type = ARAD_CNT_SRC_TYPE_OAM_ING_C;
      } else if (info->command_id == 3) {
            info->src_type = ARAD_CNT_SRC_TYPE_OAM_ING_D;
      }
  } else if (ARAD_CNT_SRC_TYPE_IS_EPNI_FLD_VAL(unit, fld_val)) {
      if (lsb == _LSB && info->command_id == 0) {
          info->src_type = ARAD_CNT_SRC_TYPE_EPNI_A;
      } else if (lsb == _LSB && info->command_id == 1) {
          info->src_type = ARAD_CNT_SRC_TYPE_EPNI_B;
      } else if (lsb == _MSB && info->command_id == 0) {
          info->src_type = ARAD_CNT_SRC_TYPE_EPNI_C;
      } else if (lsb == _MSB && info->command_id == 1) {
          info->src_type = ARAD_CNT_SRC_TYPE_EPNI_D;
      }
  } else if (ARAD_CNT_SRC_TYPE_IS_EGQ_TM_FLD(unit, fld_val)) {
