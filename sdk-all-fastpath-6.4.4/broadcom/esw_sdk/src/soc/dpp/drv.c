        ...
        switch(blktype)
        {
            ...
            case SOC_BLK_CLP:
                if (blknum >= SOC_MAX_NUM_CLP_BLKS) {
                    SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG("soc_dpp_info_config_blocks: soc_dpp_info_config: too many CLP blocks %d"),blknum));
                }
            ...
            case SOC_BLK_MTRPS_EM:
                if (blknum >= SOC_MAX_NUM_MTRPS_EM_BLKS) {
                    SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG("soc_dpp_info_config_blocks: soc_dpp_info_config: too many MRPS_EM blocks")));
                }
                si->mtrps_em_blocks[blknum] = blk;
                break;
            case SOC_BLK_EDB:
                si->edb_block = blk;
                break;
            ...
            case SOC_BLK_ILKN_PML:
                if (blknum >= SOC_MAX_NUM_ILKN_PML_BLKS) {
                    SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG("soc_dpp_info_config_blocks: soc_dpp_info_config: too many ILKN_PML blocks")));
                }
                si->ilkn_pml_block[blknum] = blk;
                si->block_port[blk] = blknum | SOC_REG_ADDR_INSTANCE_MASK;
                break;
            case SOC_BLK_IQMT:
                si->iqmt_block = blk;
...
soc_dpp_prop_parse_dtm_nof_remote_cores(int unit, int core, int region)
{
    ...
    /* set nof_remote_cores=1 for HR region*/
...
soc_dpp_deinit(int unit)
{
    ...
    /* DeInit the HW-Log mechanism */
    soc_hw_log_deinit(unit);
 ...
 *      SOC_E_XXX
 * Notes:
 *      SOC property can be retrieve only after this function.
 */
int
...
soc_dpp_info_config_common_tm(int unit) 
{
    ...
    /* flow control properties - PP Inband RX Calendar */
    ...
    for (port = 0; port < SOC_DPP_CONFIG(unit)->tm.max_interlaken_ports; port++) {
        ...
        if ( ((SOC_DPP_CONFIG(unit)->tm.fc_inband_intlkn_channel_mub_enable_mask[port][SOC_TMC_CONNECTION_DIRECTION_RX] != SOC_DPP_FC_INBAND_INTLKN_CHANNEL_MUB_DISABLE) ||
              (SOC_DPP_CONFIG(unit)->tm.fc_inband_intlkn_channel_mub_enable_mask[port][SOC_TMC_CONNECTION_DIRECTION_TX] != SOC_DPP_FC_INBAND_INTLKN_CHANNEL_MUB_DISABLE)) &&
             (SOC_DPP_CONFIG(unit)->tm.fc_inband_intlkn_llfc_mub_enable_mask[port] != SOC_DPP_FC_INBAND_INTLKN_LLFC_MUB_DISABLE) ) {
            LOG_ERROR(BSL_LS_SOC_INIT,
                      (BSL_META_U(unit,
                                  "disabling interlaken port(%d) LLFC via multi use bits\n"), port));
    ...
    if (SOC_IS_ARADPLUS_AND_BELOW(unit)) /* For Jericho, Remove swapping option between calendar IF and OOB IF */
    {
    ...
    /* consistency checks - PP Inband RX Calendar */
    if (SOC_IS_JERICHO(unit)) {
        /* 1. Only one fc calendar interface is needed to map for PP calendar.
         *    The connection between fc calendar interface and PP calendar is fixed. 
        ...
        /* If more than one calendar interface are used for PP calendar, 
         * we choose one clendar interface. 
         */
        ...
        /* 2. Check if PP COE Pause counter rate is bigger than max rate. */        
...
soc_dpp_petra_stk_modid_config_set(int unit, int fap_id) {
  ...
  if (fap_id == DPP_TDM_FAP_DEVICE_ID_UNIQUE && /* Special FAP Device ID */
      (SOC_DPP_CONFIG(unit)->tdm.is_bypass || /* TDM bypass mode on */
       (SOC_IS_PETRAB(unit) && (SOC_IS_DPP_TDM_STANDARD(unit) || SOC_IS_DPP_TDM_OPTIMIZE(unit))))) { /* Or Petra-B is TDM optimize or standard */
...
#ifdef BCM_ARAD_SUPPORT
    if ((SOC_IS_ARAD(unit)) && (enable) && !(SOC_IS_JERICHO(unit)))  {
        soc_error_t rv;
...
soc_dpp_parse_device_core_mode(int unit) {
    ...
    if (SOC_DPP_DEFS_GET(unit, nof_cores) > 1) {
        ...
        if (core_mode_propval) {
            ...
            /*Cannot be undefined, return failure*/
...
soc_dpp_init(int unit, int reset)
{
    ...
    if (SOC_IS_ARAD(unit)) {
        ...
        /* Retrieve SOC properties which are relevant to WB (user dram buffer) */
	...
	/* Init the HW-Log mechanism */
    soc_hw_log_init(unit);
    soc_hw_log_register_callbacks(unit);
