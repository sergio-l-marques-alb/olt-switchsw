                                       ...
                                       soc_lpm128_state_p lpm_state_ptr)
{
    ...
    if (!is_reserved) {
        ...
        /* 
         * start_index points to the entry 'next' to the ones allocated for
         * curr_pfx. Hence the below loop should include start_index
         */
       ...
       /* Last entry is half full -> keep it last. */
        if (entry_count > 1) {
            ...
            if (erase) {
                sal_memcpy(e, soc_mem_entry_null(u, L3_DEFIPm),
                            soc_mem_entry_words(u,L3_DEFIPm) * 4);
                LPM128_HASH_INSERT(u, e, NULL, end_ent);
                SOC_IF_ERROR_RETURN(WRITE_L3_DEFIPm(u, MEM_BLOCK_ANY, end_ent, e));
                SOC_IF_ERROR_RETURN(_lpm128_fb_urpf_entry_replicate(u, end_ent,
                                                                    e, NULL));
                SOC_LPM128_INDEX_TO_PFX_GROUP(u, end_ent) = -1;
            }
        } else {
            SOC_IF_ERROR_RETURN(_lpm128_fb_entry_shift(u, lpm_state_ptr, 
                                         ...
                                         int start_index, int count)
{
    ...
    if (socLpmEntryType64BV6 == prev_pfx_type && 
        ...
        if (other_pfx == -1 ||
            ...
            if ((SOC_LPM128_STATE_START1(u, lpm_state_ptr, prev_pfx) /
                tcam_depth) ==
                (SOC_LPM128_STATE_START1(u, lpm_state_ptr, other_pfx) /
                tcam_depth)) {
                other_start = SOC_LPM128_STATE_START1(u, lpm_state_ptr, prev_pfx)
                              + tcam_depth;
            } else {
            ...
            SOC_IF_ERROR_RETURN(
                _lpm128_move_v4_entry_down_for_v6(u, count % tcam_depth,
                    other_start, other_pfx, lpm_state_ptr, prev_pfx));
