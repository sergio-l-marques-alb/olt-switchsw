...
tdm_mem_transpose( tdm_mod_t *_tdm )
{
	...
	for (i=0; i<_tdm->_core_data.vmap_max_len; i++) {
		memcpy( &(_tdm->_core_data.vmap[_tdm->_core_data.vars_pkg.m_tdm_mem_transpose.dst][i]), &(_tdm->_core_data.vmap[_tdm->_core_data.vars_pkg.m_tdm_mem_transpose.src][i]), sizeof(unsigned char) );
	}
	/* Clear source vector */
	for (i=0; i<_tdm->_core_data.vmap_max_len; i++) {
...
**/
void
tdm_vector_rotate_step(unsigned char vector[], int size, int step)
{
	int i,j, last;
