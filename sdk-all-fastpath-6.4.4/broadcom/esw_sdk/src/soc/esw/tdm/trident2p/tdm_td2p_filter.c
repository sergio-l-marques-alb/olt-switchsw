...
/**
@name: tdm_td2p_filter_vector_shift
@param:
...
**/
int
tdm_td2p_filter_vector_shift(int *tdm_tbl, int port, int dir)
{
	...
	TD2P_TOKEN_CHECK(port) {
		if (dir==DN) {
			for (i=0; i<256; i++) {
				if (tdm_tbl[i]==port && tdm_tbl[i+1]!=TD2P_OVSB_TOKEN) {
					abort=BOOL_TRUE;
					break;
				}
			}
...
/**
@name: tdm_td2p_filter_local_slice_dn
@param:

**/
void
tdm_td2p_filter_local_slice_dn(int port, int *tdm_tbl, int **tsc, enum port_speed_e *speed)
{
	int i=port, j, pp_prox=PASS;

	if (tdm_tbl[i]<125 && tdm_tbl[i]>0) {
		for (j=1; j<4; j++) {
			if (tdm_td2p_legacy_which_tsc(tdm_tbl[i+1+j],tsc)==tdm_td2p_legacy_which_tsc(tdm_tbl[i],tsc)) {
				pp_prox=FAIL;
				break;
			}
		}
...
/**
@name: tdm_td2p_filter_local_slice_up
@param:

**/
void
tdm_td2p_filter_local_slice_up(int port, int *tdm_tbl, int **tsc, enum port_speed_e *speed)
{
	int i, j, pp_prox;
...
/**
@name: tdm_td2p_filter_ancillary_smooth
@param:

**/
void
tdm_td2p_filter_ancillary_smooth(int port, int *tdm_tbl, int lr_idx_limit, int accessories)
{
	int i, j, pool=0, smallest_slice_idx=0, slice_limit=1, timeout=32000;
	
	for (i=0; i<(lr_idx_limit+accessories); i++) {
		if (tdm_tbl[i]==TD2P_ANCL_TOKEN) {
	...
	while ( (pool>0) && ((--timeout)>0) ) {
		for (i=0; i<lr_idx_limit; i++) {
			...
			TD2P_TOKEN_CHECK(tdm_tbl[i]) {
				if ( (tdm_td2p_slice_size_local(i,tdm_tbl,lr_idx_limit)<=slice_limit) && ( (tdm_td2p_check_same_port_dist_dn_port(TD2P_ANCL_TOKEN,i,tdm_tbl,lr_idx_limit)>11) ) && ( (tdm_td2p_check_same_port_dist_up_port(TD2P_ANCL_TOKEN,i,tdm_tbl,lr_idx_limit)>11) ) ) {
					...
					tdm_tbl[smallest_slice_idx]=TD2P_ANCL_TOKEN;
					pool--;
				}
			}
		}
...
/**
@name: tdm_td2p_filter_slice_dn
@param:

**/
int
tdm_td2p_filter_slice_dn(int port, int *tdm_tbl, int lr_idx_limit, int **tsc, enum port_speed_e *speed)
{
	int i, j, k, slice_translation_done;
	
	i=port; slice_translation_done=BOOL_FALSE;
	for (j=3; (i+j)<(lr_idx_limit-1); j++) {
		if (tdm_tbl[i+j]==TD2P_OVSB_TOKEN && 
			tdm_tbl[i+1+j]==TD2P_OVSB_TOKEN &&
			(tdm_tbl[i+j-1]==TD2P_OVSB_TOKEN || (tdm_td2p_legacy_which_tsc(tdm_tbl[i+j-1],tsc)!=tdm_td2p_legacy_which_tsc(tdm_tbl[i+j+3],tsc))) &&
			(tdm_tbl[i+j-2]==TD2P_OVSB_TOKEN || (tdm_td2p_legacy_which_tsc(tdm_tbl[i+j-2],tsc)!=tdm_td2p_legacy_which_tsc(tdm_tbl[i+j+2],tsc)))) {
			for (k=(i+j); k>i; k--) {
...
/**
@name: tdm_td2p_filter_slice_up
@param:

**/
int
tdm_td2p_filter_slice_up(int port, int *tdm_tbl, int lr_idx_limit, int **tsc, enum port_speed_e *speed)
{
	int i, j, k, slice_translation_done;
	...
	for (j=3; j<(lr_idx_limit-1); j++) {
		if ((i-j-3)>=0) {
			if (tdm_tbl[i-j]==TD2P_OVSB_TOKEN && 
				...
				(tdm_tbl[i-j+1]==TD2P_OVSB_TOKEN || (tdm_td2p_legacy_which_tsc(tdm_tbl[i-j+1],tsc)!=tdm_td2p_legacy_which_tsc(tdm_tbl[i-j-3],tsc))) &&
				(tdm_tbl[i-j+2]==TD2P_OVSB_TOKEN || (tdm_td2p_legacy_which_tsc(tdm_tbl[i-j+2],tsc)!=tdm_td2p_legacy_which_tsc(tdm_tbl[i-j-2],tsc))) &&
				(tdm_td2p_check_lls_flat_up(i,tdm_tbl,speed))) {
				for (k=(i-j); k<i; k++) {
					tdm_tbl[k]=tdm_tbl[k+1];
					slice_translation_done=BOOL_TRUE;
				}
...
{
	int g, i, j, k=1, l=TD2P_NUM_EXT_PORTS;
	unsigned char dither_shift_done=BOOL_FALSE, dither_done=BOOL_FALSE, dither_slice_counter=0;

	dither_shift_done=BOOL_FALSE;
	for (i=0; i<(lr_idx_limit+accessories); i++) {
		if (tdm_tbl[i]==TD2P_OVSB_TOKEN && tdm_td2p_slice_size_local(i,tdm_tbl,(lr_idx_limit+accessories))==tdm_td2p_slice_size(TD2P_OVSB_TOKEN,tdm_tbl,(lr_idx_limit+accessories))) {
	...
	if (dither_shift_done) {
		for (i=1; i<(lr_idx_limit+accessories); i++) {
			...
			while (tdm_tbl[i]!=TD2P_OVSB_TOKEN && tdm_td2p_slice_size_local(i,tdm_tbl,(lr_idx_limit+accessories))==tdm_td2p_slice_size(1,tdm_tbl,(lr_idx_limit+accessories)) && i<(lr_idx_limit+accessories) ) {
...
tdm_td2p_filter_fine_dither(int port, int *tdm_tbl, int lr_idx_limit, int accessories, int **tsc)
{
	...
	for (j=2; j<(lr_idx_limit+accessories-4); j++) {
		if ( tdm_tbl[j]!=TD2P_OVSB_TOKEN && tdm_tbl[j-2]!=TD2P_OVSB_TOKEN && tdm_tbl[j+2]!=TD2P_OVSB_TOKEN && tdm_tbl[j+4]!=TD2P_OVSB_TOKEN && 
			 tdm_td2p_slice_size_local((j-2), tdm_tbl, (lr_idx_limit+accessories))==1 && 
			 tdm_td2p_slice_size_local(j, tdm_tbl, (lr_idx_limit+accessories))==1 && 
			 tdm_td2p_slice_size_local((j+2), tdm_tbl, (lr_idx_limit+accessories))==1 && 
			 tdm_td2p_slice_size_local((j+4), tdm_tbl, (lr_idx_limit+accessories))==1 &&
...
tdm_td2p_filter_chain( tdm_mod_t *_tdm )
{
	...
	if (_tdm->_core_data.vars_pkg.lr_enable && _tdm->_core_data.vars_pkg.os_enable && !(_tdm->_core_data.vars_pkg.refactor_done)) {
		for (i=1; i<_tdm->_chip_data.soc_pkg.lr_idx_limit; i++) {
			TD2P_TOKEN_CHECK(tdm_pipe_main[i]) {
				if (tdm_arr_exists(tdm_pipe_main[i],ARR_FIXED_LEN,vec_trans_port_list)) {
					...
					if (tdm_arr_append(vec_filter_port,ARR_FIXED_LEN,vec_trans_port_list,_tdm->_chip_data.soc_pkg.num_ext_ports)==BOOL_TRUE) {
						last_smooth = tdm_td2p_check_fit_smooth(tdm_pipe_main,vec_filter_port,_tdm->_chip_data.soc_pkg.lr_idx_limit,2);
						if (tdm_td2p_filter_vector_shift(tdm_pipe_main,vec_filter_port,DN)) {
							...
							if (tdm_td2p_check_fit_smooth(tdm_pipe_main,vec_filter_port,_tdm->_chip_data.soc_pkg.lr_idx_limit,2)>=last_smooth) {
								vec_filter_abort=BOOL_TRUE;
							}
							if (vec_filter_abort) {
								vec_filter_result=tdm_td2p_filter_vector_shift(tdm_pipe_main,vec_filter_port,UP);
		...
		for (i=4; i<_tdm->_chip_data.soc_pkg.lr_idx_limit; i++) {
			TD2P_TOKEN_CHECK(tdm_pipe_main[i]) {
				if (tdm_arr_exists(tdm_pipe_main[i],ARR_FIXED_LEN,vec_trans_port_list)) {
					...
					if (tdm_arr_append(vec_filter_port,ARR_FIXED_LEN,vec_trans_port_list,_tdm->_chip_data.soc_pkg.num_ext_ports)==BOOL_TRUE) {
						last_smooth = tdm_td2p_check_fit_smooth(tdm_pipe_main,vec_filter_port,_tdm->_chip_data.soc_pkg.lr_idx_limit,2);
						if (tdm_td2p_filter_vector_shift(tdm_pipe_main,vec_filter_port,UP)) {
							vec_filter_abort=BOOL_FALSE;
							...
							if (vec_filter_abort) {
								vec_filter_result=tdm_td2p_filter_vector_shift(tdm_pipe_main,vec_filter_port,DN);
		...
		for (i=1; i<(_tdm->_chip_data.soc_pkg.lr_idx_limit-4); i++) {
			if (tdm_pipe_main[i]!=_tdm->_chip_data.soc_pkg.soc_vars.ovsb_token && tdm_pipe_main[i-1]!=_tdm->_chip_data.soc_pkg.soc_vars.ovsb_token && tdm_pipe_main[i+1]==_tdm->_chip_data.soc_pkg.soc_vars.ovsb_token && tdm_pipe_main[i+2]==_tdm->_chip_data.soc_pkg.soc_vars.ovsb_token) {
				tdm_td2p_filter_local_slice_dn(i,tdm_pipe_main,_tdm->_chip_data.soc_pkg.pmap,_tdm->_chip_data.soc_pkg.speed);
			}
		}
		for (i=4; i<(_tdm->_chip_data.soc_pkg.lr_idx_limit-1); i++) {
			if (tdm_pipe_main[i]!=_tdm->_chip_data.soc_pkg.soc_vars.ovsb_token && tdm_pipe_main[i+1]!=_tdm->_chip_data.soc_pkg.soc_vars.ovsb_token && tdm_pipe_main[i-1]==_tdm->_chip_data.soc_pkg.soc_vars.ovsb_token && tdm_pipe_main[i-2]==_tdm->_chip_data.soc_pkg.soc_vars.ovsb_token) {
				tdm_td2p_filter_local_slice_up(i,tdm_pipe_main,_tdm->_chip_data.soc_pkg.pmap,_tdm->_chip_data.soc_pkg.speed);
			}
		}
		if (((_tdm->_core_data.vars_pkg.cap+_tdm->_chip_data.soc_pkg.tvec_size)/tdm_td2p_num_lr_slots(tdm_pipe_main))<3) {
			tdm_td2p_filter_ancillary_smooth(i,tdm_pipe_main,_tdm->_chip_data.soc_pkg.lr_idx_limit,_tdm->_chip_data.soc_pkg.tvec_size);
			for (i=0; i<(_tdm->_chip_data.soc_pkg.lr_idx_limit+_tdm->_chip_data.soc_pkg.tvec_size); i++) {
				TD2P_TOKEN_CHECK(tdm_pipe_main[i]) {
					if ( (_tdm->_chip_data.soc_pkg.speed[tdm_pipe_main[i]]<=(SPEED_40G+SPEED_DELIMITER)) && (_tdm->_chip_data.soc_pkg.speed[tdm_pipe_main[i]]>=(SPEED_40G-SPEED_DELIMITER)) ) {
						if (tdm_td2p_check_same_port_dist_dn(i,tdm_pipe_main,(_tdm->_chip_data.soc_pkg.lr_idx_limit+_tdm->_chip_data.soc_pkg.tvec_size))<_tdm->_core_data.rule__same_port_min) {
		...
		while (i<_tdm->_chip_data.soc_pkg.lr_idx_limit) {
			...
			filter_slice_size=tdm_td2p_slice_size_local(i,tdm_pipe_main,_tdm->_chip_data.soc_pkg.lr_idx_limit);
				...
				for (j=4; j<_tdm->_chip_data.soc_pkg.lr_idx_limit; j++) {
					if (tdm_pipe_main[i+j]==_tdm->_chip_data.soc_pkg.soc_vars.ovsb_token && tdm_pipe_main[i+j+1]==_tdm->_chip_data.soc_pkg.soc_vars.ovsb_token) {
						...
						for (k=filter_slice_end; k>=i; k--) {
							TD2P_TOKEN_CHECK(tdm_pipe_main[k]) {
								if (tdm_pipe_main[k]<=SPEED_42G_HG2) {
									filter_slice_dist=tdm_td2p_check_same_port_dist_dn(k,tdm_pipe_main,(_tdm->_chip_data.soc_pkg.lr_idx_limit+_tdm->_chip_data.soc_pkg.tvec_size));
		...
		for (i=1; i<(_tdm->_chip_data.soc_pkg.lr_idx_limit+_tdm->_chip_data.soc_pkg.tvec_size); i++) {
			if (tdm_pipe_main[i]!=_tdm->_chip_data.soc_pkg.soc_vars.ovsb_token && tdm_pipe_main[i-1]!=_tdm->_chip_data.soc_pkg.soc_vars.ovsb_token && tdm_pipe_main[i+1]==_tdm->_chip_data.soc_pkg.soc_vars.ovsb_token && tdm_pipe_main[i+2]==_tdm->_chip_data.soc_pkg.soc_vars.ovsb_token) {
				if ((tdm_pipe_main[i]>SPEED_42G_HG2) || (tdm_td2p_check_same_port_dist_dn(i,tdm_pipe_main,(_tdm->_chip_data.soc_pkg.lr_idx_limit+_tdm->_chip_data.soc_pkg.tvec_size))>_tdm->_core_data.rule__same_port_min)) {
		...
		for (i=(_tdm->_chip_data.soc_pkg.lr_idx_limit+_tdm->_chip_data.soc_pkg.tvec_size-1); i>=4; i--) {
			if (tdm_pipe_main[i]!=_tdm->_chip_data.soc_pkg.soc_vars.ovsb_token && tdm_pipe_main[i+1]!=_tdm->_chip_data.soc_pkg.soc_vars.ovsb_token && tdm_pipe_main[i-1]==_tdm->_chip_data.soc_pkg.soc_vars.ovsb_token && tdm_pipe_main[i-2]==_tdm->_chip_data.soc_pkg.soc_vars.ovsb_token) {
				if ((tdm_pipe_main[i]>SPEED_42G_HG2) || (tdm_td2p_check_same_port_dist_up(i,tdm_pipe_main,(_tdm->_chip_data.soc_pkg.lr_idx_limit+_tdm->_chip_data.soc_pkg.tvec_size))>_tdm->_core_data.rule__same_port_min)) {
...
/* #endif */
			for (i=1; i<(_tdm->_chip_data.soc_pkg.lr_idx_limit-4); i++) {
				if (tdm_pipe_main[i]!=_tdm->_chip_data.soc_pkg.soc_vars.ovsb_token && tdm_pipe_main[i-1]!=_tdm->_chip_data.soc_pkg.soc_vars.ovsb_token && tdm_pipe_main[i+1]==_tdm->_chip_data.soc_pkg.soc_vars.ovsb_token && tdm_pipe_main[i+2]!=_tdm->_chip_data.soc_pkg.soc_vars.ovsb_token) {
					i=tdm_td2p_filter_slice_dn(i,tdm_pipe_main,_tdm->_chip_data.soc_pkg.lr_idx_limit,_tdm->_chip_data.soc_pkg.pmap,_tdm->_chip_data.soc_pkg.speed);
				}
			}
			for (i=4; i<(_tdm->_chip_data.soc_pkg.lr_idx_limit-1); i++) {
				if (tdm_pipe_main[i]!=_tdm->_chip_data.soc_pkg.soc_vars.ovsb_token && tdm_pipe_main[i+1]!=_tdm->_chip_data.soc_pkg.soc_vars.ovsb_token && tdm_pipe_main[i-1]==_tdm->_chip_data.soc_pkg.soc_vars.ovsb_token && tdm_pipe_main[i-2]!=_tdm->_chip_data.soc_pkg.soc_vars.ovsb_token) {
					i=tdm_td2p_filter_slice_up(i,tdm_pipe_main,_tdm->_chip_data.soc_pkg.lr_idx_limit,_tdm->_chip_data.soc_pkg.pmap,_tdm->_chip_data.soc_pkg.speed);
				}
			}
/* #ifdef _LLS_SCHEDULER */
		}
/* #endif */
		timeout=DITHER_PASS;
		while ( tdm_td2p_slice_size(_tdm->_chip_data.soc_pkg.soc_vars.ovsb_token,tdm_pipe_main,(_tdm->_chip_data.soc_pkg.lr_idx_limit+_tdm->_chip_data.soc_pkg.tvec_size))>1 && tdm_td2p_slice_size(_tdm->_chip_data.soc_pkg.soc_vars.ovsb_token,tdm_pipe_main,(_tdm->_chip_data.soc_pkg.lr_idx_limit+_tdm->_chip_data.soc_pkg.tvec_size))>=tdm_td2p_slice_size(1,tdm_pipe_main,(_tdm->_chip_data.soc_pkg.lr_idx_limit+_tdm->_chip_data.soc_pkg.tvec_size)) && ((--timeout)>0) ) {
			tdm_td2p_filter_dither(tdm_pipe_main,_tdm->_chip_data.soc_pkg.lr_idx_limit,_tdm->_chip_data.soc_pkg.tvec_size,_tdm->_chip_data.soc_pkg.pmap,DITHER_THRESHOLD,_tdm->_chip_data.soc_pkg.speed);
		}
		timeout=DITHER_PASS;
		while ( tdm_td2p_slice_size(_tdm->_chip_data.soc_pkg.soc_vars.ovsb_token,tdm_pipe_main,(_tdm->_chip_data.soc_pkg.lr_idx_limit+_tdm->_chip_data.soc_pkg.tvec_size))>1 && tdm_td2p_slice_size(_tdm->_chip_data.soc_pkg.soc_vars.ovsb_token,tdm_pipe_main,(_tdm->_chip_data.soc_pkg.lr_idx_limit+_tdm->_chip_data.soc_pkg.tvec_size))>=tdm_td2p_slice_size(1,tdm_pipe_main,(_tdm->_chip_data.soc_pkg.lr_idx_limit+_tdm->_chip_data.soc_pkg.tvec_size)) && ((--timeout)>0) ) {
			tdm_td2p_filter_dither(tdm_pipe_main,_tdm->_chip_data.soc_pkg.lr_idx_limit,_tdm->_chip_data.soc_pkg.tvec_size,_tdm->_chip_data.soc_pkg.pmap,(DITHER_THRESHOLD-DITHER_SUBPASS_STEP_SIZE),_tdm->_chip_data.soc_pkg.speed);
		}
		timeout=DITHER_PASS;
		while ( tdm_td2p_slice_size(_tdm->_chip_data.soc_pkg.soc_vars.ovsb_token,tdm_pipe_main,(_tdm->_chip_data.soc_pkg.lr_idx_limit+_tdm->_chip_data.soc_pkg.tvec_size))>1 && tdm_td2p_slice_size(_tdm->_chip_data.soc_pkg.soc_vars.ovsb_token,tdm_pipe_main,(_tdm->_chip_data.soc_pkg.lr_idx_limit+_tdm->_chip_data.soc_pkg.tvec_size))>=tdm_td2p_slice_size(1,tdm_pipe_main,(_tdm->_chip_data.soc_pkg.lr_idx_limit+_tdm->_chip_data.soc_pkg.tvec_size)) && ((--timeout)>0) ) {
			tdm_td2p_filter_dither(tdm_pipe_main,_tdm->_chip_data.soc_pkg.lr_idx_limit,_tdm->_chip_data.soc_pkg.tvec_size,_tdm->_chip_data.soc_pkg.pmap,(DITHER_THRESHOLD-DITHER_SUBPASS_STEP_SIZE-DITHER_SUBPASS_STEP_SIZE),_tdm->_chip_data.soc_pkg.speed);
		}
		if (tdm_td2p_slice_size(1, tdm_pipe_main, (_tdm->_chip_data.soc_pkg.lr_idx_limit+_tdm->_chip_data.soc_pkg.tvec_size))==2) {
			for (i=3; i<(_tdm->_chip_data.soc_pkg.lr_idx_limit+_tdm->_chip_data.soc_pkg.tvec_size); i++) {
				if (tdm_pipe_main[i-3]!=_tdm->_chip_data.soc_pkg.soc_vars.ovsb_token && tdm_pipe_main[i]!=_tdm->_chip_data.soc_pkg.soc_vars.ovsb_token && tdm_pipe_main[i+3]!=_tdm->_chip_data.soc_pkg.soc_vars.ovsb_token && tdm_pipe_main[i+6]!=_tdm->_chip_data.soc_pkg.soc_vars.ovsb_token && tdm_td2p_slice_size_local((i-3), tdm_pipe_main, _tdm->_chip_data.soc_pkg.lr_idx_limit)==2 && tdm_td2p_slice_size_local(i, tdm_pipe_main, _tdm->_chip_data.soc_pkg.lr_idx_limit)==2 && tdm_td2p_slice_size_local((i+3), tdm_pipe_main, _tdm->_chip_data.soc_pkg.lr_idx_limit)==2 && tdm_td2p_slice_size_local((i+6), tdm_pipe_main, _tdm->_chip_data.soc_pkg.lr_idx_limit)==2) {
					if (tdm_td2p_filter_fine_dither(i,tdm_pipe_main,_tdm->_chip_data.soc_pkg.lr_idx_limit,_tdm->_chip_data.soc_pkg.tvec_size,_tdm->_chip_data.soc_pkg.pmap)==FAIL) {
						break;
					}
				}
			}
			for (i=3; i<(_tdm->_chip_data.soc_pkg.lr_idx_limit+_tdm->_chip_data.soc_pkg.tvec_size); i++) {
				if (tdm_pipe_main[i-3]!=_tdm->_chip_data.soc_pkg.soc_vars.ovsb_token && tdm_pipe_main[i]!=_tdm->_chip_data.soc_pkg.soc_vars.ovsb_token && tdm_pipe_main[i+3]!=_tdm->_chip_data.soc_pkg.soc_vars.ovsb_token && tdm_td2p_slice_size_local((i-3), tdm_pipe_main, _tdm->_chip_data.soc_pkg.lr_idx_limit)==2 && tdm_td2p_slice_size_local(i, tdm_pipe_main, _tdm->_chip_data.soc_pkg.lr_idx_limit)==2 && tdm_td2p_slice_size_local((i+3), tdm_pipe_main, _tdm->_chip_data.soc_pkg.lr_idx_limit)==2) {
					if (tdm_td2p_filter_fine_dither(i,tdm_pipe_main,_tdm->_chip_data.soc_pkg.lr_idx_limit,_tdm->_chip_data.soc_pkg.tvec_size,_tdm->_chip_data.soc_pkg.pmap)==FAIL) {
						break;
					}
				}
