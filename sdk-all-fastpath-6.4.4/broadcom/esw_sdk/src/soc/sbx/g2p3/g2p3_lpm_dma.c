...
/*
 * $Id: g2p3_lpm_dma.c,v 1.5 Broadcom SDK $
 *
...
g2p3_lpmDmaInit(g2p3_lpmDmaHandle_t *pHandle,
                    ...
                    uint32 maxReqs, uint32 slabWords)
{
    sbStatus_t status;
    g2p3_lpmDmaHandle_t dma;
    void *vp;
    uint8 *p;
    uint32 hostAllocBytes;
                     ...
                     g2p3_lpmCallback_f_t cb, void *cbUserData)
{
    ...
    LOG_WARN(BSL_LS_SOC_COMMON,
             (BSL_META("g2p3_lpmDmaWrite: calling Flush\n")));
#endif
...
flush(g2p3_lpmDmaHandle_t dma)
{
    ...
    while ((req = nextReq(dma))) {
        ...
        if (slabWords >= SB_FE2000_SLAB_SIZE(1)) {
...
#if LPM_DMA_DEBUG
            LOG_WARN(BSL_LS_SOC_COMMON,
                     (BSL_META("dma flush: slab opcode write, @0x%06x : %3d x 0x%08x\n"),
            ...
            dma->status = sbFe2000DmaSlabRequestMult(dma->pDmaCtxt, 
    ...
    if (slab != slabOp->slab) { /* finish off last slab */
        ...
        dma->status = sbFe2000DmaSlabRequestMult(dma->pDmaCtxt, dma->dmaReqToken, 
                        slabOp);
    }
...
#if LPM_DMA_DEBUG
    LOG_WARN(BSL_LS_SOC_COMMON,
             (BSL_META("g2p3_lpmDmaFlush: nReqs == %d\n"), dma->nReqs));
#endif
    if (dma->busy)
