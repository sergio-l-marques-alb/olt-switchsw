                        ...
                        int field_count, soc_field_t *fields, uint32 *values)
{
    ...
    for (idx = 0; idx < field_count; idx++) {
        if ((NULL == fields + idx) || (NULL == values + idx)) {
           /*
            * Coverity
            * coverity complains the control can not reach here
            * This is kept for future use or as defensive statement.
            *
...
#endif /* BCM_KATANA2_SUPPORT || BCM_GREYHOUND_SUPPORT */
    }
}
...
soc_addr_to_mem_extended(int unit, uint32 block, int acc_type, uint32 address)
{
...
#endif
    if (SOC_IS_DPP(unit) || SOC_IS_DFE(unit) || SOC_IS_GREYHOUND(unit)) {
    ...
    for (mem = 0; mem < NUM_SOC_MEM; mem++) {
        if (soc_mem_is_valid(unit, mem) &&
            ...
            if (SOC_IS_DFE(unit) || SOC_IS_DPP(unit) || SOC_IS_GREYHOUND(unit)) {
        ...
        if (!SOC_MEM_IS_ARRAY(unit, mem)) {
            ...
            if((address < (SOC_MEM_INFO(unit, mem).base + SOC_MEM_INFO(unit, mem).index_min)) || 
                ...
                *index  = (address - (SOC_MEM_INFO(unit, mem).base + SOC_MEM_INFO(unit, mem).index_min)) % \
                          (SOC_MEM_INFO(unit, mem).index_max - SOC_MEM_INFO(unit, mem).index_min + 1);
                return SOC_E_NONE;
            }
        }
    }
...
#endif /* BCM_ESW_SUPPORT || BCM_SBX_SUPPORT || BCM_PETRA_SUPPORT || BCM_DFE_SUPPORT || defined(PORTMOD_SUPPORT)*/
