                         ...
                         const portmod_port_add_info_t* add_info)
{
    ...
    ((portmod_port_add_info_t*)add_info)->interface_config.flags |= PHYMOD_INTF_F_SET_CORE_MAP_MODE;
    ...
    for(i = 0 ; i < 3 ; i++)
    {
        rv = PM_4X10_DRIVER(pm_info, i)->f_portmod_port_interface_config_set(unit, port, PM_4X10_INFO(pm_info,i), 
    ...
    if(enable) {
        ...
        for(i = 0 ; i < 3; i++) {
            rv = PM_4X10_DRIVER(pm_info, i)->f_portmod_port_enable_set(unit, 
                                   port, PM_4X10_INFO(pm_info, i), flags, 1);
            _SOC_IF_ERR_EXIT(rv);
        }
    } else {
        for(i = 0 ; i < 3; i++){
            rv = PM_4X10_DRIVER(pm_info, i)->f_portmod_port_enable_set(unit, 
                                   port, PM_4X10_INFO(pm_info, i), flags, 0);
            _SOC_IF_ERR_EXIT(rv);
        }
        rv = PM_4X25_DRIVER(pm_info)->f_portmod_port_enable_set(unit, port, 
                             ...
                             int flags, int* enable)
{
    ...
    *enable = 0;
    if (PORTMOD_PORT_ENABLE_PHY_GET(flags)) {
        rv = PM_4X10_DRIVER(pm_info, 0)->f_portmod_port_enable_get(unit, 
                         port, PM_4X10_INFO(pm_info, 0), flags, enable);
    } else {
                               ...
                               const portmod_port_interface_config_t* config)
{
    ...
    rv = PM_4X25_DRIVER(pm_info)->f_portmod_port_interface_config_set (unit, 
                                  port, PM_4X25_INFO(pm_info), config);
    _SOC_IF_ERR_EXIT(rv);

    /* Make sure to set the mode first on all cores before programming rest */
    for ( i = 0 ; i < 3 ; i++)
    {
                  ...
                  portmod_loopback_mode_t loopback_type, int* enable)
{
   ...
   switch(loopback_type) {
    ...
    case portmodLoopbackPhyGloopPCS: /*slide*/
    case portmodLoopbackPhyRloopPCS: /*slide*/
        rv = PM_4X10_DRIVER(pm_info, 0)->f_portmod_port_loopback_get(unit, port,
                                PM_4X10_INFO(pm_info, 0), loopback_type, enable);
        _SOC_IF_ERR_EXIT(rv);
        break;
    default:
        _SOC_EXIT_WITH_ERR(SOC_E_UNAVAIL, (
                              ...
                              const phymod_autoneg_control_t* an)
{
    ...
    if (port_fallback_lane == 1)
        core_seq = 2; /* seq number */
    else if (port_fallback_lane == 2)
        core_seq = 1; /* seq number */
    else
        core_seq = 0; /* seq number */
    ...
    /*
     * For 100G ports -- the speed should be triggered, so all the 3 cores
     * will establish the link again. Otherwise the link goes down on
     * commands like autoneg=off or fullduplex=true
     */
    if (IS_C_PORT(unit, port)) {
       portmod_port_interface_config_t if_cfg;

       _SOC_IF_ERR_EXIT(PM_4X10_DRIVER(pm_info, 2)->f_portmod_port_interface_config_get(
                           unit, port, PM_4X10_INFO(pm_info, 2), &if_cfg));
       ...
       if_cfg.flags |= PHYMOD_INTF_F_SET_SPD_TRIGGER;
        ...
        {
            rv = PM_4X10_DRIVER(pm_info, i)->f_portmod_port_interface_config_set(
                           unit, port, PM_4X10_INFO(pm_info, i), &if_cfg);
            _SOC_IF_ERR_EXIT(rv);
        }
    }
exit:
...
{      
    return (PM_4X10_DRIVER(pm_info, 2)->f_portmod_port_link_get(
                unit, port, PM_4X10_INFO(pm_info, 2), link));
}
...
int pm12x10_port_phy_lane_access_get (int unit, int port, pm_info_t pm_info, 
                                      ...
                                      int max_phys, phymod_phy_access_t* access,
                                      int* nof_phys,
                                      int* is_most_ext)
{
    int i = 0;
                                          ...
                                          portmod_port_ability_t *ability)
{
    ...
    rv = PM_4X10_DRIVER(pm_info, 2)->f_portmod_port_ability_advert_get (
             unit, port, PM_4X10_INFO(pm_info,2), ability);
    _SOC_IF_ERR_EXIT(rv);
                      ...
                      uint32 inst, int op_type, int op_cmd, const void *arg)
{
    ...
    for(i = 0 ; i < 3 ; i++)
    {
        rv = PM_4X10_DRIVER(pm_info, i)->f_portmod_port_diag_ctrl(unit, port, PM_4X10_INFO(pm_info,i), 
                          inst, op_type, op_cmd, arg);
        _SOC_IF_ERR_EXIT(rv);
    }
...
{
    return (PM_4X25_DRIVER(pm_info)->f_portmod_port_ref_clk_get(unit,
                 port, PM_4X25_INFO(pm_info), ref_clk));
}
