 ...
 * vxlan_roo_run(unit, in_port, out_port, vpn_id, add_routing_table_entry);                            
 * 
 * 
...
/* feature: add routing table entry */
 0, 
/* feature: enable ecmp for vxlan tunnel*/
 0
};
 ...
 * you don't want to configure global my mac msbs.
 * you want to specify the native vsi compensation (per l3_intf)
 * 
...
int vxlan_roo_ipv4_tunnel_build_tunnels(int unit, int out_port, bcm_if_t ll_eep_id, bcm_if_t* intf_ids, bcm_gport_t *tunnel_gports){
    ...
    int flags = 0;
    int flags1 = 0;
    int out_vlan = g_vxlan_roo.overlay_out_vlan;
 ...
 */
int vxlan_roo_run(int unit, int in_port, int out_port, int vpn_id, int add_routing_table_entry){
    int rv;
    int vpn; 
    ...
    l2__global_mac__set(unit, tunnel_2.sa); 
    ...
    l2_egress_overlay.outer_vlan = g_vxlan_roo.overlay_out_vlan; /* PCP DEI (0) + outer_vlan (96=0x60)  */
  ...
  rv = vxlan_add_port(unit,vpn,in_port,in_tunnel_gports[1],out_tunnel_gports[1],out_tunnel_ecmp_int_ids,vxlan_port_flags,& g_vxlan_roo.vxlan_port_id);
  if (rv != BCM_E_NONE) {
      printf("Error, vxlan_add_port 2, in_gport=0x%08x --> out_intf=0x%08x \n", in_tunnel_gports[1],out_tunnel_intf_ids[1]);
  }


  /* Create fec for native routing. Build the native link layer (part of ROO encapsulation). 
   * - create outlif using  next hop
   * - FEC contains: port
   * -               outLif and outRif: to build native LL.
   *   Note: This fec is the same as routing fec.
  ...
  rv = create_l3_egress(unit, 
                   ...
                   &g_vxlan_roo.native_ll_outlif /* LL eedb index */
                   );
  if (rv != BCM_E_NONE) {
      printf("Error, create_l3_egress\n");
  }
  ...
  if (g_vxlan_roo.enable_add_entry_routing_table) {
      int native_hi_fec_encap_id; 
      printf("Create FEC point to cascaded FEC \n");
      printf("cascaded FEC: 0x%08x \n", g_vxlan_roo.vxlan_port_id);
      rv = create_l3_egress(unit, 
                       0,  /* flags             */
                       g_vxlan_roo.vxlan_port_id, /* dest: fec-id instead of port */
                       out_vlan, /* out-vlan */
                       native_ing_intf, /* l3_intf  */
                       native_next_hop, /* next-hop */
                       &native_hi_fec_fec_id, /* output: fec id. Point to LL eedb entry  */
                       &native_hi_fec_encap_id /* LL eedb index */
                       );
      if (rv != BCM_E_NONE) {
          printf("Error, create_l3_egress\n");
      }
  ...
  rv = l2__port_vlan__create(unit, 0, out_port, out_vlan, out_vlan, &vlan_port_id);
  if (rv != BCM_E_NONE) {
  ...
  if (g_vxlan_roo.enable_add_entry_routing_table) {
      int route =  g_vxlan_roo.native_dip;
      int mask  = 0xfffffff0;
      add_route(unit, 
                route,               /* key of LPM entry: ip               */
                mask,                /*                   mask for route   */
                g_vxlan_roo.vrf,     /*                   router instance */
