 ...
 * 0. Init oam on the device by calling bcm_oam_init (This cint assumes that you already did).
...
int oam_endpoint_over_lag_main(int unit, uint8 test_level){
    ...
    if (test_level != 2) {
        ...
        /* 5. Create the Oam endpoint (MEP) with the vlan gport. */
        rv = oam_endpoint_over_lag_create_endpoint(unit);
        if (rv != BCM_E_NONE) {
 ...
 * BCM_E_*:     If something went wrong. 
 */
int oam_endpoint_over_lag_create_endpoint(int unit){
    int rv = BCM_E_NONE;
