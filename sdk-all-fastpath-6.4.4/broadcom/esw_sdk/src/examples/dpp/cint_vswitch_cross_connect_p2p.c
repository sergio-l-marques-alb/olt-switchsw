/* 
 * $Id: cint_vswitch_cross_connect_p2p.c,v 1.18 Broadcom SDK $
...
mpls_tunnels_config(
    ...
    rv = create_l3_intf(unit, 0, port, vlan, cross_connect_info.my_mac, &ingress_intf);    
    if (rv != BCM_E_NONE) {
    ...
    rv = create_l3_intf(unit, 0, port, eg_vlan, cross_connect_info.my_mac, &ingress_intf);
    if (rv != BCM_E_NONE) {
        printf("Error, in create_l3_intf\n");
        print rv;
        return rv;
    ...
    rv = create_l3_egress(unit, 0, port, eg_vlan, ingress_intf, cross_connect_info.nh_mac, egress_intf, &encap_id);
    if (rv != BCM_E_NONE) {
...
run(int unit, int port1, int type1, int protection1, int second_port1 /* used if protection1 is set */, 
    ...
    case 1:
        ...
        if (protection1 == 1 || protection1== 2) {
            ...
            if(verbose >= 1) {
                printf("created vlan_port_3 (protection for 1) : 0x0%8x\n", cross_connect_info.vlan_port_3);
            }
            ...
            if(protection1 == 2) {
                ...
                rv = multicast_vlan_port_add(unit,cross_connect_info.protection_mc1,cross_connect_info.port_1,cross_connect_info.vlan_port_1);
                if (rv != BCM_E_NONE) {
                    printf("Error, in multicast_vlan_port_add with port %d\n", cross_connect_info.port_1);
                    return rv;
                }
                rv = multicast_vlan_port_add(unit,cross_connect_info.protection_mc1,cross_connect_info.port_3,cross_connect_info.vlan_port_3);
                if (rv != BCM_E_NONE) {
                    printf("Error, in multicast_vlan_port_add with port %d\n", cross_connect_info.port_1);
                    return rv;
                }
        ...
        if (protection1) {
           ...
           if (!decoupled_mode) {
            ...
            if(protection1 == 2) {
                ...
                /* add gport to protection MC */
                rv = multicast_mpls_port_add(unit,cross_connect_info.protection_mc1,cross_connect_info.port_1,cross_connect_info.mpls_port_1.mpls_port_id);
                if (rv != BCM_E_NONE) {
                    printf("Error, in multicast_vpls_port_add with port %d\n", cross_connect_info.port_1);
                    return rv;
                }
                rv = multicast_mpls_port_add(unit,cross_connect_info.protection_mc1,cross_connect_info.port_3,cross_connect_info.mpls_port_2.mpls_port_id);
                if (rv != BCM_E_NONE) {
                    printf("Error, in multicast_vpls_port_add with port %d\n", cross_connect_info.port_1);
                    return rv;
                }
