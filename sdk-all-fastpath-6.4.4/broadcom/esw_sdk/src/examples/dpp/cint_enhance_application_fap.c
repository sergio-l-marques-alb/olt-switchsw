...
/*
 * $Id: cint_enhance_application_fap.c,v 1.2 Broadcom SDK $
 *
 ...
 * Create FQ element
 */
int create_fq(int unit,bcm_gport_t* gport_fq)
{
    bcm_gport_t physical_gport = 0;
    bcm_error_t rv;
    int num_cos_level = 1;
    uint32 flags = BCM_COSQ_GPORT_SCHEDULER | BCM_COSQ_GPORT_SCHEDULER_FQ;

    rv = bcm_cosq_gport_add(unit,physical_gport,num_cos_level,flags,gport_fq);
    if (rv != BCM_E_NONE) {
 ...
 */ 
int get_enhance_hrs(int unit, bcm_gport_t *hrs_classes)
{
    bcm_error_t rv = BCM_E_NONE;
    int index;
    bcm_cosq_gport_type_t gport_type;
    ...
    for (index = 0; index < 4; index++) {
        switch (index) {
        case 0:
            gport_type = bcmCosqGportTypeGlobalFmqClass1;
            break;
        case 1:
            gport_type = bcmCosqGportTypeGlobalFmqClass2;
            break;
        case 2:
            gport_type = bcmCosqGportTypeGlobalFmqClass3;
            break;
        case 3:
            gport_type = bcmCosqGportTypeGlobalFmqClass4;
            break;
        default:
        ...
        hrs_classes[index] = gport_info.out_gport;
    }
 ...
 */
int create_voq_connector(int unit, int num_cos_levels,bcm_gport_t *voq_connector)
{
    bcm_error_t rv;
    int my_modid;
    ...
    BCM_GPORT_MODPORT_SET(src_gport, my_modid, 0);

    rv = bcm_cosq_gport_add(unit,src_gport,num_cos_levels,BCM_COSQ_GPORT_VOQ_CONNECTOR,voq_connector);
    if (rv != BCM_E_NONE) {
...
{    
    ...
    rv = get_enhance_hrs(unit,hr_classes);
    if (rv != BCM_E_NONE) {
        print rv;
        printf("(%s) \n",bcm_errmsg(rv));
        return rv;
    ...
    rv = bcm_cosq_gport_add(unit,0,1,BCM_COSQ_GPORT_SCHEDULER
    ...
    /*  1. Create VOQ connector */
    rv = create_voq_connector(unit,num_cos_level,&mg1_voq_connector);
    if (rv != BCM_E_NONE) {
        print rv;
        printf("(%s) \n",bcm_errmsg(rv));
        return rv;
    ...
    rv = create_voq_connector(unit,num_cos_level,&mg2_voq_connector);
    if (rv != BCM_E_NONE) {
        print rv;
        printf("(%s) \n",bcm_errmsg(rv));
        return rv;
    ...
    rv = create_voq_connector(unit,num_cos_level,&mg3_voq_connector);
    if (rv != BCM_E_NONE) {
        print rv;
        printf("(%s) \n",bcm_errmsg(rv));
        return rv;
    ...
    connect_info.remote_modid = my_modid;
    connect_info.flags = BCM_COSQ_GPORT_CONNECTION_EGRESS;    
    BCM_GPORT_MCAST_QUEUE_GROUP_SET(connect_info.voq,base_queues[0]);
    ...
    BCM_GPORT_MCAST_QUEUE_GROUP_SET(connect_info.voq,base_queues[1]);
    rv = bcm_cosq_gport_connection_set(unit,&connect_info);
    if (rv != BCM_E_NONE) {
        print rv;
        printf("(%s) \n",bcm_errmsg(rv));
    ...
    BCM_GPORT_MCAST_QUEUE_GROUP_SET(connect_info.voq,base_queues[2]);
    rv = bcm_cosq_gport_connection_set(unit,&connect_info);
    if (rv != BCM_E_NONE) {
        print rv;
        printf("(%s) \n",bcm_errmsg(rv));
    ...
    {        
        ...
        for (index = 0; index < 3; index++) {
            ...
            bcm_multicast_create(unit,flags,&start_of_multicast_ingress);
