...
int evpn_pe_config(int unit,
    ...
    rv = l2__global_mac__set(unit, cint_evpn_info.global_mac); /* 1.3 */
    if (rv != BCM_E_NONE) {
        print rv;
        printf("Error, in l2__global_mac__set\n");
        return rv;
    }
...
int evpn_create_rif(int unit, int vsi, bcm_mac_t my_mac, bcm_if_t *rif){
    ...
    rv = l2__my_mac_ipv4_vsi__set(unit, my_mac,vsi);
    if (rv != BCM_E_NONE) {
        print rv;
        printf("Error, in l2__my_mac_ipv4_vsi__set\n");
        return rv;
    }
