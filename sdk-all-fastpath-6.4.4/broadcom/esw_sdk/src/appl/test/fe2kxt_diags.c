/*
 * $Id: fe2kxt_diags.c,v 1.24 Broadcom SDK $
...
const uint64 g_Fe2kxtNarrowPortpattern = COMPILER_64_INIT(0xA, 0xAAAAAAAA);
const uint64 g_Fe2kxtWidePortpattern = COMPILER_64_INIT(0xAAAAAAAA, 0xAAAAAAAA);
const uint64 g_Fe2kxtNarrowPortantipattern = COMPILER_64_INIT(0x5, 0x55555555);
const uint64 g_Fe2kxtWidePortantipattern = COMPILER_64_INIT(0x55555555, 0x55555555);

static uint32 XgmToPtPr[SB_FE2000XT_NUM_XG_INTERFACES] = { 4, 5, 2, 3 };
...
extern uint32 sbFe2000UtilXgmMiimRead(sbhandle userDeviceHandle, uint8 bInternalPhy, uint8 bClause45,
					uint32 uDevAddr, uint32 uPhyOrPortAddr, uint32 uRegAddr,
					uint32 *pData);

extern uint32 sbFe2000UtilXgmMiimWrite(sbhandle userDeviceHandle,uint8 bInternalPhy, uint8 bClause45,
					 uint32 uDevAddr, uint32 uPhyOrPortAddr, uint32 uRegAddr, 
					 uint32 uData);
...
static uint64 uXG_TxCount0 = COMPILER_64_INIT(0,0);
static uint64 uXG_RxCount0 = COMPILER_64_INIT(0,0);
static uint64 uXG_TxCount1 = COMPILER_64_INIT(0,0);
static uint64 uXG_RxCount1 = COMPILER_64_INIT(0,0);
...
static uint32 g_AGTxPorts[2 /*SB_FE2000XT_NUM_AG_INTERFACES*/][12/*SB_FE2000XT_MAX_AG_PORTS*/] ={{0}};
static uint32 g_AGRxPorts[2 /*SB_FE2000XT_NUM_AG_INTERFACES*/][12/*SB_FE2000XT_MAX_AG_PORTS*/] ={{0}};
...
int sbFe2kxtDiagsSnakeTest(sbxDiagsInfo_t *pDiagsInfo)
{
...
#ifndef __KERNEL__
  signal(SIGINT,sigcatcher_fe2kxt);
#endif
  ...
  for(i=0;i<soc_ndev;i++) {
    if (SOC_IS_SBX_FE2KXT(SOC_NDEV_IDX2DEV(i))) {
      if (fe_1 == -1) {
  ...
  if (!pDiagsInfo->bSnakeLoopOn) {
	...
	if (uDataTx == 0) {
	  cli_out("ERROR port:%s did not transmit any packets\n",SOC_PORT_NAME(unit,port));
	  rv = -1;
	}
      } else {
	cli_out("WARN: Could not read ge%d tx stats (%s)\n",port,bcm_errmsg(ge_stat));
      }
      if (( ge_stat = soc_sbx_fe2000_am_read(unit,nAgm,port%12,REG_OFFSET_C2(AM_RX_VALID_PKT_CNT),&uDataRx)) == 0) {
	if (uDataRx == 0) {
	  cli_out("ERROR port:%s did not recieve any packets\n",SOC_PORT_NAME(unit,port));
	  rv = -1;
	}
      } else {
	cli_out("WARN: Could not read ge%d rx stats (%s)\n",port,bcm_errmsg(ge_stat));
      }
    }
...

{
  ...
  for(vlan=SNAKE_VLAN_START;vlan<=end_vlan;vlan++,port+=2) {
    ...
    if (( rv = bcm_vlan_create(unit,vlan)) < 0 ) {
      cli_out("ERROR: bcm_vlan_create returned(%d) (%s) when creating vlan %d \n",
    ...
    if (dbg) {
      cli_out("vlan add %d pbm=%s,%s ubm=%s,%s\n",vlan,SOC_PORT_NAME(unit,port),
    ...
    if ((rv = bcm_vlan_port_add(unit,vlan,port_pbm,port_pbm_untag)) < 0) {
      cli_out("ERROR: Could not add vlan:%d to ports %s,%s: (%s)\n",vlan,
...
int sbFe2kxtDiagsSnakeTest_l2_create(sbxDiagsInfo_t *pDiagsInfo)
{
  ...
  for (i = 0; i< 2; i++ ) {
    ...
    if ((rv = bcm_l2_clear(unit)) < 0) {
      cli_out("ERROR: bcm_l2_clear (%d) (%s)\n",rv,
              bcm_errmsg(rv));
      return (-1);
    }
  }
  ...
  for(port=start_port;port<=end_port;port++) {
    ...
    if (dbg) {
      cli_out("l2 add mac=%s vlanid=%d module=%d pbm=%s static=true\n",
...

{
  ...
  while (pvar) {
      ...
      if (BCM_FAILURE(rv)) {
          cli_out("Failed to convert front panel port %d to fabric port: %s\n",
      ...
      if (dbg) {
          cli_out("Converted front panel port %d to fabric port %d\n",
  ...
  sal_sprintf (rh_cmd[0],"rh=0x%03x0%04x0000000000000000",qid_start/4,
  ...
  sal_sprintf (pay_cmd[0],"pat=0x%02d%02d%02d%02d%02d%02d%02d%02d%02d%02d%02d%02d9000",
  ...
  sal_sprintf (rh_cmd[1],"rh=0x%03x0%04x0000000000000000",qid_end/4,
	  pDiagsInfo->nInjectPacketsLen-8);

  parse_macaddr(l2_macDAaddrPkt2_str,l2_macDAaddr);
  parse_macaddr(l2_macSAaddrPkt2_str,l2_macSAaddr);

  sal_sprintf (pay_cmd[1],"pat=0x%02d%02d%02d%02d%02d%02d%02d%02d%02d%02d%02d%02d9000",
	  l2_macDAaddr[0],l2_macDAaddr[1],l2_macDAaddr[2],l2_macDAaddr[3],l2_macDAaddr[4],l2_macDAaddr[5],
	  l2_macSAaddr[0],l2_macSAaddr[1],l2_macSAaddr[2],l2_macSAaddr[3],l2_macSAaddr[4],l2_macSAaddr[5]);

  if (dbg >= 1) {
    cli_out("Injecting %d packets into %s PCI port..to go out ge%d\n",pDiagsInfo->nInjectPackets,
  ...
  for (i = 0; i < pDiagsInfo->nInjectPackets*2; i++) {
    ...
    sal_sprintf (len_cmd,"len=%d",pDiagsInfo->nInjectPacketsLen);
    pArgs->a_cmd="SBX_TX";
    pArgs->a_argc = 5;
    pArgs->a_arg=1;
...

{
  ...
  uint32 prbs_direction = 0;
  uint32 uField = 0;
  uint8 bRxInSync = TRUE;
  uint8 bRxEnable = TRUE;
  uint8 bTxEnable = TRUE;
  uint32 st_config0, st_error;
  int cal;
  ...
  if ( qe == -1 ) {
    cli_out("ERROR Could not QE on board for this FE\n");
    return (-1);
  }
  ...
  if (soc_cm_interrupt_disconnect(1) < 0) {
      LOG_ERROR(BSL_LS_APPL_COMMON,
                (BSL_META("soc_detach: could not disconnect interrupt line\n")));

  }

  SAND_HAL_WRITE(fehdl, C2, PC_INTERRUPT_MASK, 0xffffffff);

  SAND_HAL_RMW_FIELD(qehdl, KA, PC_RESET, SOFT_RESET, 1);
  SAND_HAL_RMW_FIELD(fehdl, C2, PC_RESET, SOFT_RESET, 1);
  SAND_HAL_RMW_FIELD(qehdl, KA, PC_RESET, SOFT_RESET, 0);
  SAND_HAL_RMW_FIELD(fehdl, C2, PC_RESET, SOFT_RESET, 0);
  ...
  /* enable  and force training at tx, verify TX is enabled */
  if (prbs_direction == PRBS_TO_FE) {
    ...
    if (nSpi == 0) {
      uData = SAND_HAL_READ(qehdl,KA,ST0_CONFIG0);
      bTxEnable = SAND_HAL_GET_FIELD(KA,ST0_CONFIG0,TX_ENABLE,uData);
    } else {
      uData = SAND_HAL_READ(qehdl,KA,ST1_CONFIG0);
      bTxEnable = SAND_HAL_GET_FIELD(KA,ST1_CONFIG0,TX_ENABLE,uData);
    }
    if (bTxEnable == 0) {
      cli_out("Error: %s SPI%d TX is not enabled, ST%d_CONFIG0(0x%08x)\n",SOC_CHIP_STRING(fe), nSpi, nSpi, uData);
      return -1;
    }
  } else {
    ...
    if (nSpi == 0) {
      uData = SAND_HAL_READ(fehdl,C2,ST0_CONFIG0);
      bTxEnable = SAND_HAL_GET_FIELD(C2,ST0_CONFIG0,TX_ENABLE,uData);
    } else {
      uData = SAND_HAL_READ(fehdl,C2,ST1_CONFIG0);
      bTxEnable = SAND_HAL_GET_FIELD(C2,ST1_CONFIG0,TX_ENABLE,uData);
    }
    if (bTxEnable == 0) {
      cli_out("Error: %s SPI%d TX is not enabled, ST%d_CONFIG0(0x%08x)\n",SOC_CHIP_STRING(fe), nSpi, nSpi, uData);
      return -1;
  ...
  /* enable rx at the reciever, and then read to verify */
  if (prbs_direction == PRBS_TO_FE) {
      SAND_HAL_RMW_FIELD_STRIDE(fehdl,C2,SR,nSpi,SR_CONFIG0,RX_ENABLE,1);
      cli_out("Testing SPI%d interface QE(UNIT %d) --> FE(UNIT %d)\n",
              nSpi,qe,fe);
      if (nSpi == 0) {
	uData = SAND_HAL_READ(fehdl,C2,SR0_CONFIG0);
	bRxEnable = SAND_HAL_GET_FIELD(C2,SR0_CONFIG0,RX_ENABLE,uData);
      } else {
	uData = SAND_HAL_READ(fehdl,C2,SR1_CONFIG0);
	bRxEnable = SAND_HAL_GET_FIELD(C2,SR1_CONFIG0,RX_ENABLE,uData);
      }
      if (bRxEnable == 0) {
	cli_out("Error: %s SPI%d RX is not enabled, SR%d_CONFIG0(0x%08x)\n",SOC_CHIP_STRING(fe), nSpi, nSpi, uData);
	return -1;
      }
  } else {
      cli_out("Testing SPI%d interface FE(UNIT %d) --> QE(UNIT %d)\n",
              nSpi,fe,qe);
      if (nSpi == 0) {
	SAND_HAL_RMW_FIELD(qehdl,KA,SR0_CONFIG0,RX_ENABLE,1);
	uData = SAND_HAL_READ(qehdl, KA, SR0_CONFIG0);
	bRxEnable = SAND_HAL_GET_FIELD(KA, SR0_CONFIG0, RX_ENABLE, uData);
      } else {
	SAND_HAL_RMW_FIELD(qehdl,KA,SR1_CONFIG0,RX_ENABLE,1);
	uData = SAND_HAL_READ(qehdl, KA, SR1_CONFIG0);
	bRxEnable = SAND_HAL_GET_FIELD(KA, SR1_CONFIG0, RX_ENABLE, uData);
      }
      if (bRxEnable == 0) {
	cli_out("Error: %s SPI%d RX is not enabled, SR%d_CONFIG0(0x%08x)\n",SOC_CHIP_STRING(qe), nSpi, nSpi, uData);
	return -1;
      }
  }
  ...
  if (prbs_direction == PRBS_TO_FE) {
    /* clear errors */
    if (nSpi==0) {
      SAND_HAL_WRITE (qehdl, KA, ST0_ERROR, 0xffffffff);
    } else {
      SAND_HAL_WRITE (qehdl, KA, ST1_ERROR, 0xffffffff);
    }
    sal_usleep(50000);
    if (nSpi == 0)
      st_error = SAND_HAL_READ(qehdl, KA, ST0_ERROR);
    else
      st_error = SAND_HAL_READ(qehdl, KA, ST1_ERROR);
    if (st_error != 0x0) {
      cli_out("QE-2000 SPI%d error 0x%08x\n", nSpi, st_error);
      goto spi_data_path_test;
    }

    /* change calendar */
    if (nSpi==0) {
      st_config0 = SAND_HAL_READ (qehdl, KA, ST0_CONFIG0);
    } else {
      st_config0 = SAND_HAL_READ (qehdl, KA, ST1_CONFIG0);
    }
    cal = SAND_HAL_GET_FIELD(KA,ST0_CONFIG0, TX_CAL_LEN, st_config0);
    if (nSpi == 0)
      SAND_HAL_RMW_FIELD(qehdl, KA,  ST0_CONFIG0, TX_CAL_LEN, cal+1);
    else
      SAND_HAL_RMW_FIELD(qehdl, KA, ST1_CONFIG0, TX_CAL_LEN, cal+1);    
    sal_usleep(200000);

    /* check whether it can detect DIP2 error */    
    if (nSpi == 0)
    st_error = SAND_HAL_READ(qehdl, KA, ST0_ERROR);
    else
      st_error = SAND_HAL_READ(qehdl, KA, ST1_ERROR);
    if (SAND_HAL_GET_FIELD(KA,ST0_ERROR, DIP2_FRAME_ERR, st_error) == 0) {
      cli_out("QE-2000 SPI%d STATUS test failed\n", nSpi);
      status = -1;
    }
    
    /* restore calendar and clear errors */
    if (nSpi == 0)
      SAND_HAL_RMW_FIELD(qehdl, KA, ST0_CONFIG0, TX_CAL_LEN, cal);
    else
      SAND_HAL_RMW_FIELD(qehdl, KA, ST1_CONFIG0, TX_CAL_LEN, cal);
    if (nSpi==0) {
      SAND_HAL_WRITE (qehdl, KA, ST0_ERROR, 0xffffffff);
    } else {
      SAND_HAL_WRITE (qehdl, KA, ST1_ERROR, 0xffffffff);
    ...
    /* clear errors */
    SAND_HAL_WRITE_STRIDE(fehdl, C2, ST, nSpi, ST0_ERROR, 0xffffffff);
    st_error = SAND_HAL_READ_STRIDE(fehdl, C2, ST, nSpi, ST0_ERROR);
    if (st_error) {
      sal_printf ("FE-2000 st%derror=0x%08x\n", nSpi, st_error);
      goto spi_data_path_test;
    }

    /* change calendar */
    st_config0 = SAND_HAL_READ_STRIDE(fehdl, C2, ST, nSpi, ST0_CONFIG0);
    cal = SAND_HAL_GET_FIELD(C2,ST0_CONFIG0, TX_CAL_LEN, st_config0);
    SAND_HAL_RMW_FIELD_STRIDE(fehdl, C2, ST, nSpi, ST0_CONFIG0, TX_CAL_LEN, cal+1);
    sal_usleep(200000);

    /* check whether it can detect DIP2 error */
    st_error = SAND_HAL_READ_STRIDE(fehdl, C2, ST, nSpi, ST0_ERROR);
    if (st_error == 0) {
      sal_printf ("FE-2000 SPI%d STATUS test failed\n", nSpi);
      status = -1;
    }
    /* restore calendar */
    SAND_HAL_RMW_FIELD_STRIDE(fehdl, C2, ST, nSpi, ST0_CONFIG0, TX_CAL_LEN, cal);
    SAND_HAL_WRITE_STRIDE(fehdl, C2, ST, nSpi, ST0_ERROR, 0xffffffff);
  }

 spi_data_path_test:
  /* clear errors */
  COMPILER_REFERENCE(uStatus0);
  ...
  if (prbs_direction == PRBS_TO_FE) {
    ...
    cli_out("%s sr%d_prbs_status0 = 0x%x ",SOC_CHIP_STRING(fe),nSpi,uData);
    if (uField != 0) {
        cli_out(" *prbs_error*\n");
    } else {
        cli_out("\n");
	...
	cli_out("%s sr%d_prbs_status0 = 0x%x ",SOC_CHIP_STRING(qe),nSpi,uData);
	if (uField != 0) {
            cli_out(" *prbs_error*\n");
        } else {
            cli_out("\n");
	...
	uData = SAND_HAL_READ(qehdl,KA,SR1_PRBS_STATUS1);
      ...
      if (uData != 0) {
	cli_out("QE SPI%d ERROR SR_PRBS_STATUS1 Mismatch Errors 0x%08x \n",nSpi,uData);
	status = -1;
	/* dump the fields for easier debug */
	if (nSpi == 0) {
	...
	uData = SAND_HAL_READ(qehdl,KA,SR1_PRBS_STATUS2);
      ...
      if (uData != 0) {
	LOG_ERROR(BSL_LS_APPL_COMMON,
                  (BSL_META("QE SPI%d: ERROR SR_PRBS_STATUS2 Mismatch Errors 0x%08x\n"),nSpi, uData));
	status = -1;
	if (nSpi == 0) {
  ...
  if (status == 0) {
	...
	/* cli_out("C2_prbs0_status0=0x%x\n",uData); */
	uField = SAND_HAL_GET_FIELD(C2,SR0_PRBS_STATUS0,PRBS_ERROR_OCCURRED,uData);
	if (uField == 0) {
	  cli_out("WARN: Could not force error in QE-->FE direction for spi0 interface\n");
	} else {
	  cli_out("Good force error ok for spi0(%d)\n",uField);
	}
      } else {
	uData = SAND_HAL_READ(fehdl,C2,SR1_PRBS_STATUS0);
	/* cli_out("C2_prbs1_status0=0x%x\n",uData); */
	uField = SAND_HAL_GET_FIELD(C2,SR1_PRBS_STATUS0,PRBS_ERROR_OCCURRED,uData);
	if (uField == 0) {
	  cli_out("WARN: Could not force error in QE-->FE direction for spi1 interface\n");
	} else {
	  cli_out("Good force error ok for spi1(%d)\n",uField);
	}
      }
	...
	uField = SAND_HAL_GET_FIELD(KA,SR0_PRBS_STATUS0,PRBS_ERROR_OCCURRED,uData);
	if (uField == 0) {
	  cli_out("WARN: Could not force error in FE-->QE direction for spi0 interface\n");
	} else {
	  cli_out("Good force error ok for spi0(%d)\n",uField);
	}
      } else {
	...
	uField = SAND_HAL_GET_FIELD(KA,SR1_PRBS_STATUS0,PRBS_ERROR_OCCURRED,uData);
	if (uField == 0) {
	  cli_out("WARN: Could not force error in FE-->QE direction for spi1 interface\n");
	} else {
	  cli_out("Good force error ok for spi1(%d)\n",uField);
	}
      }
...
int sbFe2kxtDiagsSramMemTestAll(sbxDiagsInfo_t *pDiagsInfo) {
  ...
  for(mem_test=0;mem_test<SB_FE2KXT_MEM_TEST_LAST;mem_test++) {
    ...
    cli_out("Running Test[%2d..%2d] %-45s  .. ",mem_test,nMemTests,sbFe2kxtDiagsMemoryTestStr[mem_test]);
    if(sbFe2kxtDiagsSramMemTest(pDiagsInfo) != 0) {
      stat = -1;
      if (strstr(sbFe2kxtDiagsMemoryTestStr[mem_test],"RANDOM") != NULL) {
	cli_out("FAILED: To re-run use MemTest=%d Verbose=1 Seed=%d \n", mem_test,pDiagsInfo->seed);
      } else {
...

{
  ...
  if ((sbFe2kxtDiagsMemType_t) pDiagsInfo->e_mem_type == SB_FE2KXT_NARROW_PORT_0) {
  ...
  loopOnReadError:
	...
	cli_out("Read Timeout MM%d,MEM_TYPE:%s, retry attempt %d .. -- ctrl-c to stop\r",
...
int sbFe2kxtDiagsSramAddressBusTest(sbxDiagsInfo_t *pDiagsInfo)
{
  ...
  if (!pDiagsInfo->bEndAddr) {
      ...
      } else if ((sbFe2kxtDiagsMemType_t) pDiagsInfo->e_mem_type == SB_FE2KXT_NARROW_PORT_1) {
 ...
 loopOnError0:
  ...
  if (status != SB_FE2KXT_SUCCESS_E) {
    if (pDiagsInfo->bLoopOnError) {
      cli_out("Write timeout, trying again .. ctrl-c to stop\r");
      goto loopOnError0;
    }
    return status;
  }
  ...
  loopOnError1:
    ...
    if (status != SB_FE2KXT_SUCCESS_E) {
      if (pDiagsInfo->bLoopOnError) {
	cli_out("Read timeout, trying again .. ctrl-c to stop\r");
	goto loopOnError1;
      }
      return status;
    }
	...
	if (pDiagsInfo->bLoopOnError) {
	  LOG_ERROR(BSL_LS_APPL_COMMON,
                    (BSL_META("\r%s:%d MM%d Data mismatch: exp 0x%x%08x got 0x%x%08x at address: 0x%lx -- looping on error ctrl-c to stop"), FUNCTION_NAME(),__LINE__,
	  ...
	  LOG_ERROR(BSL_LS_APPL_COMMON,
                    (BSL_META("\nERROR:%s:%d MM%d Data mismatch: exp 0x%x%08x got 0x%x%08x at address: 0x%lx\n"), FUNCTION_NAME(),__LINE__,
                     mmu,COMPILER_64_HI(g_antipattern), COMPILER_64_LO(g_antipattern),COMPILER_64_HI(ullData_rd), COMPILER_64_LO(ullData_rd),ulStartAddr));
	  return (-1);
	}
      } else { /* for debug */
	if (pDiagsInfo->debug_level >=1 ) {
	  cli_out("%s:%d GOOD MM%d Got:0x%x%08x Exp:0x%x%08x at address: 0x%lx\n",FUNCTION_NAME(),__LINE__,
                  mmu,COMPILER_64_HI(g_antipattern), COMPILER_64_LO(g_antipattern),COMPILER_64_HI(ullData_rd), COMPILER_64_LO(ullData_rd),ulStartAddr);
	}
	...
	if (pDiagsInfo->bLoopOnError) {
	  ...
	  LOG_ERROR(BSL_LS_APPL_COMMON,
                    (BSL_META("\nERROR:%s:%d MM%d Data mismatch: exp 0x%x%08x got 0x%x%08x at address: 0x%lx"), FUNCTION_NAME(),__LINE__,
                     mmu,COMPILER_64_HI(g_pattern), COMPILER_64_LO(g_pattern),COMPILER_64_HI(ullData_rd), COMPILER_64_LO(ullData_rd),ulWalkAddr));
	  /* return (-1); */
	}
      } else { /* for debug */
...
int sbFe2kxtDiagsSramDataBusWidePortTest(sbxDiagsInfo_t *pDiagsInfo)
{
  ...
  if (pDiagsInfo->bLoopOnError) {
    cli_out("Loop on error not supported for this test.\n");
    return(-1);
  }
  ...
  for(uPhase=0;uPhase<3;uPhase++) {
	...
	if (sbFe2kxtDiagsMemRead(userDeviceHandle, mmu,uOffset, &uActData,
	  ...
	  if ( COMPILER_64_EQ(uActData, ullExpData)) {
	    if (dbgLevel >= 1) {
	      cli_out("%s:GOOD MM%d, MemType:%s Act=0x%x%08x Exp:0x%x%08x at Addr:0x%08x\n",FUNCTION_NAME(),
                      mmu,sbFe2kxtDiagsMemoryStr[mem],COMPILER_64_HI(uActData), COMPILER_64_LO(uActData),COMPILER_64_HI(ullExpData), COMPILER_64_LO(ullExpData),uOffset);
	    }
	    ...
	    LOG_ERROR(BSL_LS_APPL_COMMON,
                      (BSL_META("\nERROR:%s:%d, MM%d, MemType:%s Exp:0x%x%08x, Got:0x%x%08x, at address:0x%08x\n"),FUNCTION_NAME(),__LINE__,
                       mmu,sbFe2kxtDiagsMemoryStr[mem],COMPILER_64_HI(ullExpData), COMPILER_64_LO(ullExpData),COMPILER_64_HI(uActData), COMPILER_64_LO(uActData),uOffset));
	    return (-1);
	  }
	}
      } else if (uPhase == 2) {
	  ...
	  if ( COMPILER_64_NE(uActData, ullExpDataInv)) {
	    LOG_ERROR(BSL_LS_APPL_COMMON,
                      (BSL_META("ERROR:%s:%d, MM%d, MemType:%s Exp:0x%x%08x, Got:0x%x%08x, at address:0x%08x\n"),FUNCTION_NAME(),__LINE__,
                       mmu,sbFe2kxtDiagsMemoryStr[mem],COMPILER_64_HI(ullExpDataInv), COMPILER_64_LO(ullExpDataInv),COMPILER_64_HI(uActData), COMPILER_64_LO(uActData),uOffset));
	    return (-1);
	  } else if (dbgLevel >=1) {
	    cli_out("%s:GOOD MM%d, MemType:%s Act=0x%x%08x Exp:0x%x%08x at Addr:0x%08x\n",FUNCTION_NAME(),
                    mmu,sbFe2kxtDiagsMemoryStr[mem],COMPILER_64_HI(uActData), COMPILER_64_LO(uActData),COMPILER_64_HI(ullExpDataInv), COMPILER_64_LO(ullExpDataInv),uOffset);
	  }
	}
      }
    }
...
int sbFe2kxtDiagsMemRead(sbhandle userDeviceHandle,
  ...
  case SB_FE2KXT_NARROW_PORT_0:
	...
	return (SB_FE2KXT_MEM_ACC_READ_TIMEOUT_E);
      ...
      COMPILER_64_SET(*pData,0,SAND_HAL_READ_STRIDE(userDeviceHandle,C2,MM,uInstance,MM_NARROW_PORT0_MEM_ACC_DATA0));
      /* get the remaining 4 bits of 36 bits of data */
      COMPILER_64_SET(tmp,SAND_HAL_READ_STRIDE(userDeviceHandle,C2,MM,uInstance,MM_NARROW_PORT0_MEM_ACC_DATA1) & 0xf, 0);
  ...
  case SB_FE2KXT_NARROW_PORT_1:
	...
	return (SB_FE2KXT_MEM_ACC_READ_TIMEOUT_E);
      ...
      /* get the first 32bits of 36 bits data */
      COMPILER_64_SET(*pData,0,SAND_HAL_READ_STRIDE(userDeviceHandle,C2,MM,uInstance,MM_NARROW_PORT1_MEM_ACC_DATA0));
      COMPILER_64_SET(tmp,SAND_HAL_READ_STRIDE(userDeviceHandle,C2,MM,uInstance,MM_NARROW_PORT1_MEM_ACC_DATA1) & 0xf,0);
      /* get the remaining 4 bits of 36 bits of data */
      COMPILER_64_OR(*pData, tmp);
    }
    break;
  ...
  case SB_FE2KXT_WIDE_PORT:
	...
	return (SB_FE2KXT_MEM_ACC_READ_TIMEOUT_E);
      ...
      COMPILER_64_SET(*pData,0,SAND_HAL_READ_STRIDE(userDeviceHandle,C2,MM,uInstance,MM_WIDE_PORT_MEM_ACC_DATA0));
      COMPILER_64_SET(tmp,SAND_HAL_READ_STRIDE(userDeviceHandle,C2,MM,uInstance,MM_WIDE_PORT_MEM_ACC_DATA1),0);
...
int sbFe2kxtDiagsMemWrite(sbhandle userDeviceHandle,
  ...
  case SB_FE2KXT_NARROW_PORT_0:
    ...
    {
      uint32 offset,ack_mask;
      SAND_HAL_WRITE_STRIDE(userDeviceHandle,C2,MM,uInstance,MM_NARROW_PORT0_MEM_ACC_DATA0,COMPILER_64_LO(ullData));    /* Least significant 32 bits of 36 bit data to be written */
      SAND_HAL_WRITE_STRIDE(userDeviceHandle,C2,MM,uInstance,MM_NARROW_PORT0_MEM_ACC_DATA1,COMPILER_64_HI(ullData)&0xf);   /* Most significant 4 bits of 36 bit data to be written */
  ...
  case SB_FE2KXT_NARROW_PORT_1:
    ...
    {
      uint32 offset,ack_mask;
      SAND_HAL_WRITE_STRIDE(userDeviceHandle,C2,MM,uInstance,MM_NARROW_PORT1_MEM_ACC_DATA0,COMPILER_64_LO(ullData));
      SAND_HAL_WRITE_STRIDE(userDeviceHandle,C2,MM,uInstance,MM_NARROW_PORT1_MEM_ACC_DATA1,COMPILER_64_HI(ullData)&0xf);
      /* initiate the write request */
  ...
  case SB_FE2KXT_WIDE_PORT:
    {
      uint32 offset,ack_mask;
      SAND_HAL_WRITE_STRIDE(userDeviceHandle,C2,MM,uInstance,MM_WIDE_PORT_MEM_ACC_DATA0,COMPILER_64_LO(ullData));    /* least significant 32 bits of 64 bit data */
      SAND_HAL_WRITE_STRIDE(userDeviceHandle,C2,MM,uInstance,MM_WIDE_PORT_MEM_ACC_DATA1,COMPILER_64_HI(ullData));       /* most significant 32 bits of 64 bit data */
      /* initiate the write request */
...
int sbFe2kxtDiagsInitMem(sbxDiagsInfo_t *pDiagsInfo) {
  ...
  switch (pDiagsInfo->e_mem_type)
	...
	cli_out("%s: Initializing MM%d Narrow Port0 Memory..\n",FUNCTION_NAME(),mmu);
      }
    ...
    case SB_FE2KXT_NARROW_PORT_1:
      if (pDiagsInfo->debug_level) {
	cli_out("%s: Initializing MM%d Narrow Port1 Memory..\n",FUNCTION_NAME(),mmu);
      }
	...
	if (pDiagsInfo->debug_level) {
	  cli_out("%s: Initializing MM%d Wide Port Memory..\n",FUNCTION_NAME(),mmu);
	}
...
int
sbFe2kxt1GLoopbackSet(uint32 unit,uint32 loopback)
{
  int rv = 0;
  uint32 port;
...
int sbFe2kxtDiags10GQELoopback(sbxDiagsInfo_t *pDiagsInfo)
{
  ...
  uint32 unit;
  uint32 i;
  uint32 port_10g_start,port_10g_end;
  args_t *pArgs = NULL;
  int timer_sec = 0;
  int port = 0;
  int port_status;
  ...
  if (dbg) {
    cli_out("vlan add %d pbm=%d,%d ubm=%d,%d\n",vlan,
            port_10g_start,
            port_10g_end,
            port_10g_start,
            port_10g_end);
 ...
 inject_packets:
  ...
  COMPILER_64_ZERO(uXG_TxCount0); COMPILER_64_ZERO(uXG_RxCount0);  /* start port counters */
  COMPILER_64_ZERO(uXG_TxCount1); COMPILER_64_ZERO(uXG_RxCount1);  /* end port counters */
  cli_out("Looping packet between %s <---> %s for roughly %d seconds ..\n",SOC_PORT_NAME(unit,port_10g_start),
...
int sbFe2kxtPoll10GCounters(uint32 unit,uint32 port_10g_start, uint32 port_10g_end)
{

    uint32 uRetry = 3000;
    uint32 bXG_start_Incrementing = 0;
    uint32 bXG_end_Incrementing = 0;
    uint32 uDataTx = 0;
    uint32 uDataRx = 0;
    uint32 uXgmNum_start;
    uint32 uXgmNum_end;
    int rv = 0;
    sbhandle userDeviceHandle;
    ...
    while ( --uRetry && !bXG_start_Incrementing) {
      ...
      COMPILER_64_SET(uuTmp, 0, uDataTx);
      COMPILER_64_ADD_64(uuTmp, uXG_TxCount0);
      if (COMPILER_64_LE(uuTmp, uXG_TxCount0)) { /* if this count <= last count, pkt stopped */
	thin_delay(300); continue;
      }  else {
	COMPILER_64_ADD_32(uXG_TxCount0, uDataTx);
	bXG_start_Incrementing = 1;
      }
    }

    if (uRetry == 0) {
      cli_out("ERROR: XG%d looping packet stopped unexpectedly\n",uXgmNum_start);
	rv = -1;
    }
    ...
    while ( --uRetry && !bXG_end_Incrementing) {
      ...
      COMPILER_64_SET(uuTmp, 0, uDataTx);
      COMPILER_64_ADD_64(uuTmp, uXG_TxCount1);
      if (COMPILER_64_LE(uuTmp, uXG_TxCount1)) { /* if this count <= last count, pkt stopped */
	thin_delay(300); continue;
      }  else {
	COMPILER_64_ADD_32(uXG_TxCount1, uDataTx);
    ...
    COMPILER_64_ADD_32(uXG_RxCount0, uDataRx);

    uDataRx = sbFe2kXtDiagsReadRxPktCount(userDeviceHandle,XgmToPtPr[uXgmNum_end]);
    COMPILER_64_ADD_32(uXG_RxCount1, uDataRx);

    cli_out("XG%d Rx:0x%x%08x Tx:0x%x%08x XG%d Rx:0x%x%08x Tx:0x%x%08x\r",uXgmNum_start,
            COMPILER_64_HI(uXG_RxCount0), COMPILER_64_LO(uXG_RxCount0),
            COMPILER_64_HI(uXG_TxCount0), COMPILER_64_LO(uXG_TxCount0),
            uXgmNum_end,
            COMPILER_64_HI(uXG_RxCount1), COMPILER_64_LO(uXG_RxCount1),
            COMPILER_64_HI(uXG_TxCount1), COMPILER_64_LO(uXG_TxCount1));
    return rv;
}
...
int sbFe2kxtDiags10GLoopback(sbxDiagsInfo_t *pDiagsInfo)
{
  ...
  uint32 uPktCnt;
  sbhandle userDeviceHandle;
  uint32 uXgm_start,uXgm_end = 0;
  uint32 uXgm = 0;
  uint32 unit;
  int rv = 0;
  uint32 stat = 0;
  uint32 uDataRx,uDataTx = 0;
  uint32 dbg;
  char *s = NULL;
  uint32 port_10g = 0;
  uint32 uQueue = 120;
  uint32 xe_start_port = 0;
  uint32 xe_end_port = 0;
  int nPrPt = 0;
  ...
  cli_out("10GE%d - 10GE%d  %s  Loopback Test\n",xe_start_port-23,xe_end_port-23,s);
  ...
  if (pDiagsInfo->bInternalLpk) {
	...
	cli_out("Error setting loopback for port:%s (%s)\n",SOC_PORT_NAME(unit,port_10g),
                bcm_errmsg(rv));
	return (-1);
      }
    }
   ...
   */
    ...
    if (uDataTx != uPktCnt) {
      LOG_ERROR(BSL_LS_APPL_COMMON,
                (BSL_META("ERROR: PCI TxExp:%d TxAct:%d\n"),uPktCnt,uDataTx));
      stat = -1;
    } else if (dbg) {
      cli_out("GOOD: PCI TxExp:%d TxAct:%d\n",uPktCnt,uDataTx);
    }
    ...
    if (uDataRx != uPktCnt) {
      LOG_ERROR(BSL_LS_APPL_COMMON,
                (BSL_META("ERROR: SWS(PCI) RxExp:%d RxAct:%d\n"),uPktCnt,uDataRx));
      stat = -1;
    } else if (dbg) {
      cli_out("GOOD: SWS(PCI) RxExp:%d RxAct:%d\n",uPktCnt,uDataRx);
    }
	...
	cli_out("GOOD: XGM%d TxExp:%d TxAct:%d\n",uXgm_start,uPktCnt,uDataTx);
      }
	...
	cli_out("GOOD XGM%d RxExp:%d RxAct:%d\n",uXgm_end,uPktCnt,uDataRx);
      }
    }
    ...
    if (uDataTx != uPktCnt) {
      LOG_ERROR(BSL_LS_APPL_COMMON,
                (BSL_META("ERROR: SWS.PTE6 TxExp:%d TxAct:%d\n"),uPktCnt,uDataTx));
      stat = -1;
    } else if (dbg) {
      cli_out("GOOD: SWS.PTE6 TxExp:%d TxAct:%d\n",uPktCnt,uDataTx);
    }

    if (stat == 0) {
      cli_out("Packet looped back to PCI ok. Continuity test passed\n");
    } else {
      cli_out("Packet was not recieved back to PCI. Test failed\n");
    }
...
int sbFe2kxtDiagsUnimacLoopback(sbxDiagsInfo_t *pDiagsInfo)
{
  ...
  DENTER();
  cli_out("Running 1G Internal(Unimac) Loopback Test\n");

    /* connect PCI.p0 -> AGM0.p0 */
  if (dbg) {
    cli_out("Creating SWS connection from %s.p%d to %s.p%d using queue:%d\n",
  ...
  if (stat != SB_FE2KXT_SUCCESS_E) {
    LOG_ERROR(BSL_LS_APPL_COMMON,
              (BSL_META("ERROR:%s sbFe2kxtDiagsInjectPciPacket failed with status %d\n"),FUNCTION_NAME(),stat));
    return(-1);
  }
  ...
  if (uData != nPacketsInjected) {
    ...
    cli_out("GOOD SWS.PCI(PRE6) Exp:%d Act:%d\n",nPacketsInjected,uData);
  }
  ...
  if (stat != SB_FE2KXT_SUCCESS_E) {
    LOG_ERROR(BSL_LS_APPL_COMMON,
              (BSL_META("ERROR:%s sbFe2kxtDiagsRecievePciPacket did not rx expected pkt(s) %d\n"),FUNCTION_NAME(),stat));
    return (-1);
  }
  ...
  if (counter_stat !=0 ) {
    cli_out("Counter mismatch error occured. FAILED\n");
    rv = -1;
  } else {
    cli_out("Packet looped back to PCI ok. PASSED\n");
    rv = 0;
  }
...
int sbFe2kxtDiagsSerdesExternalLoopback(sbxDiagsInfo_t *pDiagsInfo)
{
  ...
  cli_out("Running 1G External Loopback Test\n");
  ...
  if (uData != nPacketsInjected) {
    ...
    return (-1);
      ...
      if (dbg) {
	  cli_out("GOOD PCI->FE2k TxExp:%d TxAct:%d\n",nPacketsInjected,uData);
      }
  }
  ...
  if (stat != SB_FE2KXT_SUCCESS_E) {
    LOG_ERROR(BSL_LS_APPL_COMMON,
              (BSL_META("ERROR:%s sbFe2kxtDiagsRecievePciPacket not all pkt(s) recieved to PCI\n"),FUNCTION_NAME()));
    return stat;
  } else {
    cli_out("Packet looped back to PCI ok. Continuity test passed\n");
  }
...
void sbFe2kxtCreate1GLoopbackQueues(sbxDiagsInfo_t *pDiagsInfo)
{
    ...
    if (dbg) {
    cli_out("Creating SWS connection from %s.%d to %s.%d using queue:%d\n",
  ...
  if (bExternal) {
	...
	if (g_AGTxPorts[nAgm][nPort]) {
	  ...
	  if (uData != nPackets) {
	    ...
	    if (nDbg) {
	      cli_out("GOOD AGM%d.p%d TxExp:%d got TxAct:%d\n",nAgm,nPort,nPackets,uData);
	    }
	  }
	}
	/* check port rx */
	if (g_AGRxPorts[nAgm][nPort]) {
	  ...
	  if (uData != nPackets) {
	    ...
	    if (nDbg) {
	      cli_out("GOOD AGM%d.p%d RxExp:%d got RxAct:%d\n",nAgm,nPort,nPackets,uData);
	    }
	  }
	}
      }
...
int sbFe2kxtDiagsSerdesInternalLoopback(sbxDiagsInfo_t *pDiagsInfo)
{
  ...
  cli_out("Running 1G Internal(PHY) Loopback Test\n");
  ...
  if (stat == 0) {
    ...
    cli_out("Packet was not recieved back to PCI. FAILED\n");
  }
...
sbFe2kxtDiagsInjectPciPacket(sbxDiagsInfo_t *pDiagsInfo)
{
    ...
    if (dbg) {
	cli_out("Injecting %d packets into PCI\n",nPackets);
    }

    sal_memset(pArgs,0x0,sizeof(args_t));
    sal_sprintf (cmd_1,"%d",nPackets);
    sal_sprintf (cmd_2,"len=%d",len);
    if (dbg) {
	cli_out("sbx_tx %d len=%d\n",nPackets,len);
    }
    pArgs->a_cmd="SBX_TX";
    pArgs->a_argc=3;
    pArgs->a_arg=1;
...
sbFe2kxtDiagsReceivePciPacket(sbxDiagsInfo_t *pDiagsInfo)
{
    ...
    if (uData != nPkts) {
      LOG_ERROR(BSL_LS_APPL_COMMON,
                (BSL_META_U(unit,
                            "ERROR:%s -> PCI TxExp:%d got TxAct:%d\n"),SOC_CHIP_STRING(unit),nPkts,uData));
       stat = -1;
    } else {
      if (dbg) {
	cli_out("GOOD %s_%d -> PCI TxExp:%d got TxAct:%d\n",SOC_CHIP_STRING(unit)
                ,unit,nPkts,uData);
      }
    }
...
sbFe2kxtLineRateMemTestRun(sbxDiagsInfo_t *p)
{
  ...
  cli_out("%s: Testing with seed=0x%x\n",FUNCTION_NAME(),seed);
  ...
  if (rv != SB_OK) {
    cli_out("ERROR sbFe2kxtLineRateMemTestSetup failed(%d)\n",rv);
    return (-1);
  }
  ...
  cli_out("Resetting LR registers.. \n");
  if (0 != (rv = fe2kxt_set_lrp_default_regs(unit))) {
    cli_out("Setting LR registers failed(%d)\n",rv);
...
#define UPDATE _SB_UPDATE_SAVE
  ...
  /* phase 0 */
  cli_out("Loading phase0 microcode.. ");
  for (i = 0 ; i <= 1; i++) {
    status = sbFe2000UcodeLoad(hdl,(uint8*)&fe2kxtdiag_phase0);
    if(status != SB_OK) {
      cli_out("ERROR loading phase0 microcode (%d) %s\n",
  ...
  if (timeout) {
    cli_out("ERROR: first phase timed out, bailing\n");
    test_error(unit,"%s: failed LRP memory test. \n",SOC_CHIP_STRING(unit));    
    return (-1);
  }
  ...
  if (err) {
    cli_out("MEMORY ERRORS DETECTED = %d\n",err);
    cli_out("Skipping second phase, since phase one had errors\n");
    test_error(unit,"%s: failed LRP memory test. \n",SOC_CHIP_STRING(unit));    
    return (-1);
  }

  /* phase 1 */
  cli_out("Loading phase1 microcode.. ");
  fe2kXtAsm2IntD__initDefaultPhase1(&fe2kxtdiag_phase1);

  for (i = 0; i <= 1; i++ ) {
    status = sbFe2000UcodeLoad(hdl,(uint8*)&fe2kxtdiag_phase1);
    if(status != SB_OK) {
      cli_out("ERROR loading phase1 microcode (%d) %s \n",
              status,bcm_errmsg(status));
      test_error(unit,"%s: failed LRP memory test. \n",SOC_CHIP_STRING(unit));
      return status;
    }
  ...
  if (timeout) {
    cli_out("ERROR: second phase timed out, bailing\n");
    test_error(unit,"%s: failed LRP memory test. \n",SOC_CHIP_STRING(unit));    
    return (-1);
  }
  ...
  if (err) {
    cli_out("MEMORY ERRORS DETECTED = %d\n",err);
    cli_out("Skipping third phase, since phase two had errors\n");
    test_error(unit,"%s: failed LRP memory test. \n",SOC_CHIP_STRING(unit));    
    return (-1);
  }

  /* phase 2 */
  cli_out("Loading phase2 microcode.. ");
  fe2kXtAsm2IntD__initDefaultPhase2(&fe2kxtdiag_phase2);

  for (i = 0; i <= 1; i++ ) {
    status = sbFe2000UcodeLoad(hdl,(uint8*)&fe2kxtdiag_phase2);
    if(status != SB_OK) {
      cli_out("ERROR loading phase2 microcode (%d) %s\n",
              status,bcm_errmsg(status));
      test_error(unit,"%s: failed LRP memory test. \n",SOC_CHIP_STRING(unit));
      return status;
    }
  ...
  if (timeout) {
    cli_out("ERROR: Third phase timed out, bailing\n");
    test_error(unit,"%s: failed LRP memory test. \n",SOC_CHIP_STRING(unit));    
    return (-1);
  }
...
Fe2kxt_SetupCtlRegs(sbxDiagsInfo_t *pDiagsInfo)
{
  ...
  cli_out("Setting up ctrl regs using seed=0x%x\n",seed);
  ...
  for(uLrp = 0; uLrp < 48; uLrp++) {
    /* cli_out("Writing MM0 Internal0 memory entry at address=0x%08x -- 0x%08x\n",uLrp,ulCtlReg); */
  ...
  /* cli_out("Writing MM0 Internal1 memory entry at address 0x%08x -- 0x%08x\n",0,ulSharedReg); */
...
/* write internal */
void
Fe2kxt_WrI(sbhandle hdl, uint32 mem, uint32 offs, uint32 data)
{
...
/* read internal */
uint32
Fe2kxt_RdI(sbhandle hdl,uint32 mem, uint32 offs)
{
  switch (mem) {
  case 0:
    {
...
uint32
Fe2kxt_WaitForPhase(sbhandle hdl,uint32 phase)
{
  ...
  while (1) {
    ...
    if (ulDone) {
      ...
      cli_out("Finished (%d of 3) stages\n",ulStage);
      if (ulStage != phase)
	cli_out("ERROR: Register failed to show correct phase expt %d actual %d\n", phase, ulStage);
      return 0;
    }
    if (ulTimeout++ > 100000) {
      cli_out("ERROR: Timed out waiting for test to finish\n");
      return 1;
    }
  }
}
...
uint32
Fe2kxt_CheckErrors(sbhandle hdl,int32 seed,uint32 phase)
{
  uint32 err = 0;
  uint32 ulCtlReg, i;
  ...
  for ( i = 0; i < MAX_PES; i++) {
	...
	ulD1 = 0;
      ...
      cli_out("ERROR: PE %d encountered an error\n", i);
      cli_out("ERROR: Addr = 0x%08x\n", ulAddr);
      cli_out("ERROR: Memory = %s \n", MemStr[ulPort]);
      if (ulPort < 4) {
	cli_out("ERROR: read 0x%08x expt 0x%08x xor 0x%08x\n",
                ulD0, ulE0, ulD0 ^ ulE0);
      } else {
	cli_out("ERROR: read 0x%08x%08x expt 0x%08x%08x xor 0x%08x%08x\n",
...
uint32
Fe2kxt_HashData(int32 seed, uint32 ulOffs, uint32 Inv)
{
  int32 seedEx;
...
fe2kxt_set_lrp_default_regs(int unit)
{
  ...
  for(i = 0; i < Fe2k_lrp_regs_l->count; i++) {
	...
	regval |= (reg_info->fields[j]->default_val << reg_info->fields[j]->shift);
      ...
      /* cli_out("Setting Reg:%s to 0x%x\n",reg_info->name,regval); */
      rv = sbx_diag_write_reg(unit,reg_info,regval);
      if (rv) {
	cli_out("sbx_diag_write_reg failed(%d)\n",rv);
	sal_free(Fe2k_lrp_regs_l);
	return (rv);
      }
      regval = 0;
...
int Fe2kXtDiagsSetErrorRegList(sbxDiagsInfo_t *pDiagsInfo,uint32 unit) {
  int idx = 0;
  int i = 0;
  soc_sbx_chip_info_t *chip_info = NULL;
  soc_sbx_reg_info_t *reg_info = NULL;
...
int Fe2kXtDiagsCheckForErrors(uint32 unit) {
  ...
  for(i = 0; i < gFe2kxt_reginfo_l->count; i++) {
    ...
    if (idx >= 0) {
      ...
      rv = sbx_diag_read_reg(unit,reg_info,&regval);
      if (rv != CMD_OK) {
	cli_out("ERROR: sbx_diag_read_reg returned(%d)\n",rv);
	return (-1);
      }
      if (regval != 0) {
	...
	rv = sbx_diag_write_reg(unit, reg_info, regval);
	if (rv != CMD_OK) {
	  cli_out("ERROR: sbx_diag_reg_write returned(%d)\n",rv);
	  return (-1);
	}
	return (-1);
      }
...
int Fe2kXtDiagsPollForErrors(uint32 sp, uint32 ep,uint32 unit) {
  sbhandle hdl;
  int rv = 0;
  uint32 uData = 0;
  uint32 uRetry;
  ...
  if (sp == XE0_PORT ||
    ...
    cli_out("AGM0 Rx:0x%x%08x Tx:0x%x%08x, AGM1 Rx:0x%x%08x Tx:0x%x%08x\r",
...
uint32  sbFe2kXtDiagsReadTxPktCount(sbhandle hdl,int pt) {
 uint32 uData = 0;
 switch(pt) {
 case 2:
   uData = SAND_HAL_READ(hdl,C2,PT_PTE2_PKT_CNT);
...
uint32  sbFe2kXtDiagsReadRxPktCount(sbhandle hdl,int pr) {
  uint32 uData = 0;
  switch(pr) {
  case 2:
    uData = SAND_HAL_READ(hdl,C2,PR_PRE2_PKT_CNT);
...
int32 sbFe2kxtDiagsBistStart(sbxDiagsInfo_t *pDiagsInfo)
{
  ...
  if (status == 0) {
    cli_out("Bist tests passed.\n");
  }
  return status;
}

/* bist test, CC2,CC3,CC4,CC5 functions */
int32 sbFe2kxtDiagsCC2Bist(sbxDiagsInfo_t *pDiagsInfo)
{
  ...
  if (uData !=0) {
    cli_out("ERROR: CC2_CAMBIST_STATUS != 0 at start of test\n");
    return (-1);
  }
    
  uData = SAND_HAL_READ(userDeviceHandle,C2,PR_CC2_CAMBIST_CONFIG);
  if (uData !=0) {
    cli_out("ERROR: CC2_CAMBIST_CONFIG != 0 at start of test\n");
    return (-1);
  }

  uData = SAND_HAL_READ(userDeviceHandle,C2,PR_CC2_CAMBIST_DATA);
  if (uData !=0) {
    cli_out("ERROR: CC2_CAMBIST_DATA != 0 at start of test\n");
    return (-1);
  }
  ...
  if (bistdone) {
    ...
    cli_out("CC2 Bist test done in %d usec,  ",soc_timeout_elapsed(&to));
    uData = SAND_HAL_READ(userDeviceHandle,C2,PR_CC2_CAMBIST_STATUS);
    if (uData & 2) {
      cli_out("PASSED\n");
    } else {
    ...
    cli_out("CC2 bist test timedout\n");
    return status = -1;
  }
...
int32 sbFe2kxtDiagsCC3Bist(sbxDiagsInfo_t *pDiagsInfo)
{
  ...
  if (uData !=0) {
    cli_out("ERROR: CC3_CAMBIST_STATUS != 0 at start of test\n");
    return (-1);
  }
    
  uData = SAND_HAL_READ(userDeviceHandle,C2,PR_CC3_CAMBIST_CONFIG);
  if (uData !=0) {
    cli_out("ERROR: CC3_CAMBIST_CONFIG != 0 at start of test\n");
    return (-1);
  }

  uData = SAND_HAL_READ(userDeviceHandle,C2,PR_CC3_CAMBIST_DATA);
  if (uData !=0) {
    cli_out("ERROR: CC3_CAMBIST_DATA != 0 at start of test\n");
    return (-1);
  }
  ...
  if (bistdone) {
    /* check for pass status */
    cli_out("CC3 Bist test done in %d usec,  ",soc_timeout_elapsed(&to));
    uData = SAND_HAL_READ(userDeviceHandle,C2,PR_CC3_CAMBIST_STATUS);
    if (uData & 2) {
      cli_out("PASSED\n");
    } else {
    ...
    cli_out("CC3 Bist test timedout\n");
    status = -1;
  }
...
int32 sbFe2kxtDiagsCC4Bist(sbxDiagsInfo_t *pDiagsInfo)
{
  ...
  if (uData !=0) {
    cli_out("ERROR: CC4_CAMBIST_STATUS != 0 at start of test\n");
    return (-1);
  }
    
  uData = SAND_HAL_READ(userDeviceHandle,C2,PR_CC4_CAMBIST_CONFIG);
  if (uData !=0) {
    cli_out("ERROR: CC4_CAMBIST_CONFIG != 0 at start of test\n");
    return (-1);
  }

  uData = SAND_HAL_READ(userDeviceHandle,C2,PR_CC4_CAMBIST_DATA);
  if (uData !=0) {
    cli_out("ERROR: CC4_CAMBIST_DATA != 0 at start of test\n");
    return (-1);
  }
  ...
  if (bistdone) {
    /* check for pass status */
    cli_out("CC4 Bist test done in %d usec,  ",soc_timeout_elapsed(&to));
    uData = SAND_HAL_READ(userDeviceHandle,C2,PR_CC4_CAMBIST_STATUS);
    if (uData & 2) {
      cli_out("PASSED\n");
    } else {
    ...
    cli_out("CC4 Bist test timedout\n");
    status = -1;
  }
...
int32 sbFe2kxtDiagsCC5Bist(sbxDiagsInfo_t *pDiagsInfo)
{
  ...
  if (uData !=0) {
    cli_out("ERROR: CC5_CAMBIST_STATUS != 0 at start of test\n");
    return (-1);
  }
    
  uData = SAND_HAL_READ(userDeviceHandle,C2,PR_CC5_CAMBIST_CONFIG);
  if (uData !=0) {
    cli_out("ERROR: CC5_CAMBIST_CONFIG != 0 at start of test\n");
    return (-1);
  }

  uData = SAND_HAL_READ(userDeviceHandle,C2,PR_CC5_CAMBIST_DATA);
  if (uData !=0) {
    cli_out("ERROR: CC5_CAMBIST_DATA != 0 at start of test\n");
    return (-1);
  }
  ...
  if (bistdone) {
    /* check for pass status */
    cli_out("CC5 Bist test done in %d usec,  ",soc_timeout_elapsed(&to));
    uData = SAND_HAL_READ(userDeviceHandle,C2,PR_CC5_CAMBIST_STATUS);
    if (uData & 2) {
      cli_out("PASSED\n");
    } else {
    ...
    cli_out("CC5 Bist test timedout\n");
    status = -1;
  }
...
int32 sbFe2kxtDiagsPPCAM0Bist(sbxDiagsInfo_t *pDiagsInfo)
{
  ...
  if (uData !=0) {
    cli_out("ERROR: PP_CAMBIST_STATUS0 != 0 at start of test\n");
    return (-1);
  }
    
  uData = SAND_HAL_READ(userDeviceHandle,C2,PP_CAMBIST_CONFIG0);
  if (uData !=0) {
    cli_out("ERROR: PP_CAMBIST_CONFIG0 != 0 at start of test\n");
    return (-1);
  }

  uData = SAND_HAL_READ(userDeviceHandle,C2,PP_CAMBIST_DATA0);
  if (uData !=0) {
    cli_out("ERROR: PP_CAMBIST_DATA0 != 0 at start of test\n");
    return (-1);
  }
  ...
  if (bistdone) {
    /* check for pass status */
    cli_out("PP CAM0 Bist test done in %d usec,  ",soc_timeout_elapsed(&to));
    uData = SAND_HAL_READ(userDeviceHandle,C2,PP_CAMBIST_STATUS0);
    if (uData & 2) {
      cli_out("PASSED\n");
    } else {
    ...
    cli_out("PP CAM0 bist test timedout\n");
    status = -1;
  }
...
int32 sbFe2kxtDiagsPPCAM1Bist(sbxDiagsInfo_t *pDiagsInfo)
{
  ...
  if (uData !=0) {
    cli_out("ERROR: PP_CAMBIST_STATUS1 != 0 at start of test\n");
    return (-1);
  }
    
  uData = SAND_HAL_READ(userDeviceHandle,C2,PP_CAMBIST_CONFIG1);
  if (uData !=0) {
    cli_out("ERROR: PP_CAMBIST_CONFIG1 != 0 at start of test\n");
    return (-1);
  }

  uData = SAND_HAL_READ(userDeviceHandle,C2,PP_CAMBIST_DATA1);
  if (uData !=0) {
    cli_out("ERROR: PP_CAMBIST_DATA1 != 0 at start of test\n");
    return (-1);
  }
  ...
  if (bistdone) {
    /* check for pass status */
    cli_out("PP CAM1 Bist test done in %d usec,  ",soc_timeout_elapsed(&to));
    uData = SAND_HAL_READ(userDeviceHandle,C2,PP_CAMBIST_STATUS1);
    if (uData & 2) {
      cli_out("PASSED\n");
    } else {
    ...
    cli_out("PP CAM1 bist test timedout\n");
    status = -1;
  }
...
int32 sbFe2kxtDiagsPPCAM2Bist(sbxDiagsInfo_t *pDiagsInfo)
{
  ...
  if (uData !=0) {
    cli_out("ERROR: PP_CAMBIST_STATUS2 != 0 at start of test\n");
    return (-1);
  }
    
  uData = SAND_HAL_READ(userDeviceHandle,C2,PP_CAMBIST_CONFIG2);
  if (uData !=0) {
    cli_out("ERROR: PP_CAMBIST_CONFIG2 != 0 at start of test\n");
    return (-1);
  }

  uData = SAND_HAL_READ(userDeviceHandle,C2,PP_CAMBIST_DATA2);
  if (uData !=0) {
    cli_out("ERROR: PP_CAMBIST_DATA2 != 0 at start of test\n");
    return (-1);
  }
  ...
  if (bistdone) {
    /* check for pass status */
    cli_out("PP CAM2 Bist test done in %d usec,  ",soc_timeout_elapsed(&to));
    uData = SAND_HAL_READ(userDeviceHandle,C2,PP_CAMBIST_STATUS2);
    if (uData & 2) {
      cli_out("PASSED\n");
    } else {
    ...
    cli_out("PP CAM2 bist test timedout\n");
    status = -1;
  }
...
int32 sbFe2kxtDiagsPPCAM3Bist(sbxDiagsInfo_t *pDiagsInfo)
{
  ...
  if (uData !=0) {
    cli_out("ERROR: PP_CAMBIST_STATUS3 != 0 at start of test\n");
    return (-1);
  }
    
  uData = SAND_HAL_READ(userDeviceHandle,C2,PP_CAMBIST_CONFIG3);
  if (uData !=0) {
    cli_out("ERROR: PP_CAMBIST_CONFIG3 != 0 at start of test\n");
    return (-1);
  }

  uData = SAND_HAL_READ(userDeviceHandle,C2,PP_CAMBIST_DATA3);
  if (uData !=0) {
    cli_out("ERROR: PP_CAMBIST_DATA3 != 0 at start of test\n");
    return (-1);
  }
  ...
  if (bistdone) {
    /* check for pass status */
    cli_out("PP CAM3 Bist test done in %d usec,  ",soc_timeout_elapsed(&to));
    uData = SAND_HAL_READ(userDeviceHandle,C2,PP_CAMBIST_STATUS3);
    if (uData & 2) {
      cli_out("PASSED\n");
    } else {
    ...
    cli_out("PP CAM3 bist test timedout\n");
    status = -1;
  }
...
void Fe2kxt_EnableProcessing(sbxDiagsInfo_t *pDiagsInfo) {
  ...
  if (rv != TRUE) {
    cli_out("LRP Memory Bank0 write failed \n");
  }

  /* do a bank swap */
  rv = sbFe2000SwapInstructionMemoryBank(hdl);
  if (rv != 0) {
    cli_out("Timeout waiting for bank swap event from LRP\n");
  }
}

void sigcatcher_fe2kxt(int signal)
{
  cli_out("\n");
  gStopSnakeTest = 1;
  return;
...
void sbFe2000DiagUpdateLrpEpoch(sbhandle hdl, uint32 epoch) {
  UINT uTimeOut;
  UINT uUpdateEvent;
  ...
  if ( !uTimeOut ) {
    cli_out("Timeout waiting for UPDATE event from the LRP.\n");
  }
...
fe2kxt_clear_lrp_banks(int unit) {
  ...
  if (status != SB_OK) {
    LOG_ERROR(BSL_LS_APPL_COMMON,
              (BSL_META_U(unit,
                          "%s: swap instruction bank failed\n"),FUNCTION_NAME()));
    return status;
  }
