/*
 * $Id: arad_test.c,v 1.65 Broadcom SDK $
...
"                          To run the test for all of the drams set this value to -1\n"
...
arad_bist_test_init(int unit, args_t *a, void **p)
{
    ...
    if((abt->run_info.write_weight == 0) && (abt->run_info.read_weight == 0)) {
        LOG_INFO(BSL_LS_APPL_TESTS,
                 (BSL_META_U(unit,
                             "WriteWeight=0 and ReadWeight=0 is illegal configuration\n")));
        return -1; 
    }

    if (*dram_ndx_srt == '\0') {
        /* set by default - dram index = 0 */
        BCM_PBMP_CLEAR(abt->dram_ndx);
        BCM_PBMP_PORT_ADD(abt->dram_ndx, 0);

    } else if (parse_small_integers(unit, dram_ndx_srt, &abt->dram_ndx) < 0) {
        LOG_INFO(BSL_LS_APPL_TESTS,
                 (BSL_META_U(unit,
...
int arad_bist_test(int unit, args_t *a, void *p)
{
    ...
    SOC_PBMP_ITER(abt->dram_ndx, dram_ndx) {
        ...
        if (SOC_IS_DPP_DRC_COMBO28(unit)) 
        {
            ...
            if ((SOC_IS_JERICHO(unit) || SOC_IS_ARDON(unit)) && (!SOC_PBMP_MEMBER(SOC_DPP_CONFIG(unit)->arad->init.drc_info.dram_bitmap, dram_ndx)))
            {
        ...
        if (abt->run_info.arad_bist_flags & SOC_TMC_DRAM_BIST_FLAGS_ALL_ADDRESS) {
            ...
            if(SOC_IS_DPP_DRC_COMBO28(unit) == FALSE){
                ...
                if (SOC_SAND_FAILURE(soc_sand_rc)) {
                    LOG_INFO(BSL_LS_APPL_TESTS,
                             (BSL_META_U(unit,
                                         "Error: First arad_dram_bist_test_start () Failed:\n")));
                    arad_bist_fail = 0x1;
                    goto done;
                }
            } else {
                ...
                if (rv < 0) {
                    LOG_INFO(BSL_LS_APPL_TESTS,
                             (BSL_META_U(unit,
                                         "Error: soc_dpp_drc_combo28_bist_test_start () Failed:\n")));
                    arad_bist_fail = 0x1;
                    goto done;
                }            
            }
            ...
            if (SOC_IS_DPP_DRC_COMBO28(unit) == FALSE) {
                ...
                if (SOC_SAND_FAILURE(soc_sand_rc)) {
                    LOG_INFO(BSL_LS_APPL_TESTS,
                             (BSL_META_U(unit,
                                         "Error: Secound arad_dram_bist_test_start () Failed:\n")));
                    arad_bist_fail = 0x1;
                    goto done;
                }
            } else {
            ...
            if (SOC_IS_DPP_DRC_COMBO28(unit) == FALSE) {
                ...
                if (SOC_SAND_FAILURE(soc_sand_rc)) {
                    LOG_INFO(BSL_LS_APPL_TESTS,
                             (BSL_META_U(unit,
                                         "Error: arad_dram_bist_test_start () Failed:\n")));
                    arad_bist_fail = 0x1;
                    goto done;
                }
            } else {
        ...
        } else
        {
            ...
            if ((combo28_bist_err_cnt.bist_err_occur != 0x0)          || (combo28_bist_err_cnt.bist_full_err_cnt != 0x0)   || 
                ...
                (is_gddr5 && ((combo28_bist_err_cnt.bist_edc_global_err_cnt != 0x0) || (combo28_bist_err_cnt.bist_edc_err_occur != 0x0))))
            {
                arad_bist_fail = 0x1;
            }
        }
