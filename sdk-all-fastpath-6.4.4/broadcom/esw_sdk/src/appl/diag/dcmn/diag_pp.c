    ...
    cmd_ppd_api_diag_rif_info_get(int unit, args_t* a)
{
  ...
  /* get last rif */
  arad_pp_signal_mngr_signal_get(soc_sand_dev_id, SOC_DPP_SIGNAL_MNGR_SIGNAL_TYPE_RIF, val, &size_of_signal_in_bits);
  ...
  if (SOC_IS_JERICHO(unit)) {      
      arad_pp_signal_mngr_signal_get(soc_sand_dev_id, SOC_DPP_SIGNAL_MNGR_SIGNAL_TYPE_RIF_PROFILE, val, &size_of_signal_in_bits);
...
{ 
     ...
     */
    if (last_program) {
        res = arad_pp_isem_access_print_last_vtt_program_data(soc_sand_dev_id, 1, &prog_id_vt, &prog_id_tt);
...
{ 
     ...
     */
    if (last_program) {
        res = arad_pp_flp_access_print_last_programs_data(soc_sand_dev_id, 1, &prog_id);
        /* check if previous function return an error value */
        if (soc_sand_get_error_code_from_error_word(res) != SOC_SAND_OK) { 
            return CMD_FAIL; 
        }
...
cmd_diag_kaps_lkup_info_get(int unit, args_t* a) {
    ...
    soc_rc = (MBCM_PP_DRIVER_CALL(unit, mbcm_pp_diag_kaps_lkup_info_get, (unit)));
