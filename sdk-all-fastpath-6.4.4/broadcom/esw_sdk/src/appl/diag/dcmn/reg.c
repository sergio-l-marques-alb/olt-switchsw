...
#if (defined(BCM_PETRA_SUPPORT) || defined(BCM_DFE_SUPPORT)) && (defined(LINUX) || defined(UNIX)) && (defined(__DUNE_GTO_BCM_CPU__) || defined(__DUNE_WRX_BCM_CPU__))
extern int _cpu_write(int d, uint32 addr, uint32 *buf);
extern int _cpu_read(int d, uint32 addr, uint32 *buf);
#endif


#define SOC_PETRAB_REG_MASK             (0x1ff)
#define SOC_PETRAB_BLOCK_MASK           (0x7e00)
#define SOC_PETRAB_BLOCK_SIZE           (0x200)
#define SOC_PETRAB_NIF_BLOCK_SIZE       (0x400)
#define SOC_PETRAB_NIF_MAL_BLOCK_SIZE   (0x40)
#define SOC_PETRAB_NIF_MAL_MASK         (0x200)
#define SOC_PETRAB_NIF_IDX_MASK         (0x400)
#define SOC_PETRAB_MALS_PER_NIF         (0x8)

typedef struct soc_dpp_reg_addr_info_s {
    int     block;      /* block number */
    uint32  start_addr; /* base address of first block of this type */
    int     num_blocks; /* number of instances of the block */
} soc_dpp_reg_addr_info_t;

static soc_dpp_reg_addr_info_t soc_petrab_reg_addr_info [] = {
    {SOC_BLK_CFC,       0x4600,     1},
    {SOC_BLK_DPI,       0x0c00,     6},
    {SOC_BLK_DRC,       0x1800,     6},
    {SOC_BLK_ECI,       0x0000,     1},
    {SOC_BLK_EGQ,       0x5800,     1},
    {SOC_BLK_EPNI,      0x3a00,     1},
    {SOC_BLK_FCR,       0x3200,     1},
    {SOC_BLK_FCT,       0x3000,     1},
    {SOC_BLK_MMU,       0x0a00,     1},
    {SOC_BLK_FDR,       0x2e00,     1},
    {SOC_BLK_FDT,       0x2c00,     1},
    {SOC_BLK_MESH_TOPOLOGY,       0x0000,     1},
    {SOC_BLK_IDR,       0x2800,     1},
    {SOC_BLK_IHB,       0x6400,     1},
    {SOC_BLK_IHP,       0x6000,     1},
    {SOC_BLK_IPS,       0x0200,     1},
    {SOC_BLK_IPT,       0x0800,     1},
    {SOC_BLK_IQM,       0x0400,     1},
    {SOC_BLK_IRE,       0x2400,     1},
    {SOC_BLK_IRR,       0x2a00,     1},
    {SOC_BLK_MAC,       0x3c00,     3}, /* fabric MAC */
    {SOC_BLK_MBU,       0x0000,     1},
    {SOC_BLK_MCC,       0x4e00,     1},
    {SOC_BLK_MSW,       0x4800,     1},
    {SOC_BLK_NBI,       0x4a00,     1},
    {SOC_BLK_NIF,       0x5000,     1},
    {SOC_BLK_NIF_MAL,   0x5200,     16},
    {SOC_BLK_OLP,       0x4400,     1},
    {SOC_BLK_QDR,       0x0600,     1},
    {SOC_BLK_RTP,       0x3600,     1},
    {SOC_BLK_SCH,       0x4200,     1}
};

#define SOC_PETRAB_REG_ADDR_INFO_LEN    (sizeof(soc_petrab_reg_addr_info)/ \
                                         sizeof(soc_dpp_reg_addr_info_t))
 ...
 */
STATIC void
collect_comma_args_dcmn(args_t *a, char *valstr, char *first)
{
    char           *s;
...
{
    int                     i, reg, block;
    uint32                  block_addr, end_addr, lkup_addr, block_size;
    soc_dpp_reg_addr_info_t *addr_info = NULL;
    ...
    if (SOC_IS_PETRAB(unit)) {
        for (i = 0; i < SOC_PETRAB_REG_ADDR_INFO_LEN; i++) {
            addr_info = &soc_petrab_reg_addr_info[i];
            block_addr = addr & SOC_PETRAB_BLOCK_MASK;
            /*block_size = ((addr_info->block == SOC_BLK_NIF) ? 
                          SOC_PETRAB_NIF_BLOCK_SIZE : SOC_PETRAB_BLOCK_SIZE);*/
            block_size = SOC_PETRAB_BLOCK_SIZE;
            end_addr = addr_info->start_addr +
                       addr_info->num_blocks * block_size;
            if ((block_addr >= addr_info->start_addr) && 
                (block_addr < (end_addr))) {
                block = addr_info->block;
                break;
            }
        }
        if ((i >= SOC_PETRAB_REG_ADDR_INFO_LEN) || (addr_info == NULL)) {
        ...
        /* address associated to a valid block. Find the reg */
        lkup_addr = addr_info->start_addr + (addr & SOC_PETRAB_REG_MASK);
    }
    else {
        int idx = 0, schan_blk; 
    ...
    for (reg = 0; reg < NUM_SOC_REG; reg++) {
        ...
        if (lkup_addr == (SOC_REG_INFO(unit, reg).offset & (~0x80000))) {
            ...
            if (SOC_IS_PETRAB(unit)
                || SOC_BLOCK_IN_LIST(regblktype, ainfo->block)) {
                ainfo->valid = 1;
                ainfo->reg = reg;
    ...
    if (ainfo->valid && SOC_IS_PETRAB(unit)) {
        ainfo->addr = addr;
        if (block == SOC_BLK_NIF) {
            if (addr & SOC_PETRAB_NIF_MAL_MASK) {
                /* Reg belongs to NIF_MAL */
                ainfo->block = SOC_BLK_NIF_MAL;
                ainfo->idx = (addr & SOC_PETRAB_REG_MASK)/
                              SOC_PETRAB_NIF_MAL_BLOCK_SIZE;
                if (addr & SOC_PETRAB_NIF_IDX_MASK) {
                    ainfo->idx += SOC_PETRAB_MALS_PER_NIF; /* its on 2nd NIF */
                }
            } else {
                ainfo->block = block;
                ainfo->idx = 0;
                if (addr & SOC_PETRAB_NIF_IDX_MASK) {
                    ainfo->idx = 1; /* belongs to 2nd NIF */
                }
            }
        } else {
            ainfo->idx = ((addr & SOC_PETRAB_BLOCK_MASK) - 
                          addr_info->start_addr)/ SOC_PETRAB_BLOCK_SIZE;
            ainfo->block = block;
        }
    }
}
 ...
 */
STATIC  int
dpp_reg_print_one_field(int unit, soc_regaddrinfo_t *ainfo, soc_reg_above_64_val_t val, uint32 flags,
      char *fld_sep, int wrap, char *field_name )
{
    soc_reg_info_t *reginfo = &SOC_REG_INFO(unit, ainfo->reg);
    int             f;
    int             rv = 0;
...
dpp_reg_print_all(int unit, soc_regaddrlist_t *alist, uint32 flags)
{
    ...
    for (j = 0; j < alist->count; j++) {
        ...
        if(soc_cpureg == SOC_REG_INFO(unit,ainfo->reg).regtype) {
            addr = soc_reg_addr_get(unit, ainfo->reg, ainfo->block, ainfo->idx, FALSE, &blk, &acc_type);
 ...
 */
STATIC int
dpp_reg_print_field(int unit, soc_regaddrlist_t *alist, uint32 flags, char *field_name)
{
    int                 j;
    soc_reg_above_64_val_t value; 
    int                 r, rv = 0, array_index, blk;
    ...
    for (j = 0; j < alist->count; j++) {
        ...
        if (soc_cpureg == SOC_REG_INFO(unit,ainfo->reg).regtype) {
            ...
            SOC_REG_ABOVE_64_CLEAR(value);
            value[0] = val32;
            rv = dpp_reg_print_one_field(unit, ainfo, value, flags, ",", 62,field_name);
        }
#ifdef BCM_IPROC_SUPPORT
        else if (soc_iprocreg == SOC_REG_INFO(unit,ainfo->reg).regtype) {
            addr = soc_reg_addr_get(unit, ainfo->reg, ainfo->block, ainfo->idx, FALSE, &blk, &acc_type);
 ...
 */
STATIC int
_dpp_reg_get_by_type(int unit, uint32 regaddr, uint32 cmic_blk, soc_regtype_t regtype,
         soc_reg_above_64_val_t outval, uint32 flags, int array_index)
{
    int                 rv = CMD_OK;
    int                 r, i;
    soc_regaddrinfo_t   ainfo;
    ...
    case soc_genreg:
    case soc_portreg:
        dpp_soc_regaddrinfo_get(unit, &ainfo, regaddr, cmic_blk);
        if (!ainfo.valid) {
        ...
        if(SOC_IS_PETRAB(unit))
          r = soc_reg_above_64_get(unit, ainfo.reg, REG_PORT_ANY, array_index, outval);
        else
    ...
    if ((rv == CMD_OK) && (flags & REG_PRINT_DO_PRINT)) {
        if (flags & REG_PRINT_HEX) {
            ...
            char buf[256];
            bsl_log_add(BSL_APPL_SHELL, bslSeverityNormal, bslSinkIgnore, unit, "%s[0x%x] = ", soc_regtypenames[regtype], regaddr);
            if(SOC_REG_IS_ABOVE_64(unit,ainfo.reg)) {
 ...
 */
cmd_result_t
cmd_dpp_reg_get(int unit, args_t *a)
{
    soc_reg_above_64_val_t          regval;
    ...
    if(1 == all_regs) {
        ...
        return dpp_all_reg_get(unit, is_debug);
    }
    ...
    if ((rt = dpp_regtype_lookup_name(name)) != 0) {
/*        if ((name = ARG_GET(a)) == 0) {
            return CMD_USAGE;
        }*/
    } else {
        rt = dpp_regtype_lookup_name("soc");
    }
    if (0 == rt) {
    ...
    if (isint(name)) {    /* Numerical address given */
        ...
        else if (1 == direct_mode)
        {
            if (array_index >=0) {
                ...
                bsl_log_add(BSL_APPL_SHELL, bslSeverityNormal, bslSinkIgnore, unit, "dwc_read max size is 20 , received %d\n", dwc_read);
                rv = CMD_FAIL;
            } else {
                uint32 get_data[20];
        ...
        {
          rv = _dpp_reg_get_by_type(unit, regaddr, cmic_blk, rt->type, regval, flags, array_index);

        }
    } else {
        if (array_index >=0) {
            cli_out("ind should not be used for registers specified symbolically, specify array index (or range) in the register description\n");
            rv = CMD_FAIL;
        } else {
            ...
            if (parse_symbolic_reference(unit, &alist, name) < 0) {
                ...
                rv = CMD_FAIL;
                  ...
                  next_name = ARG_GET(a);
                  if (next_name != NULL ) {
                      if ( dpp_reg_print_field(unit, &alist, flags,next_name) < 0) {
                          rv = CMD_FAIL;
                          cli_out("Syntax error parsing \"%s\" field\n",next_name);
                      }
                  } else {
                      if (dpp_reg_print_all(unit, &alist, flags) < 0) {
                        rv = CMD_FAIL;
                      }
                  }
            }
 ...
 */
STATIC int
_dpp_reg_set_by_type(int unit, uint32 regaddr, uint32 cmic_blk, soc_regtype_t regtype,
                     soc_reg_above_64_val_t regval, int array_index)

{
    ...
    case soc_portreg:
        ...
        if(SOC_IS_PETRAB(unit))
          r = soc_reg_above_64_set(unit, ainfo.reg, REG_PORT_ANY, array_index, regval);
        else
...
modify_reg_fields(int unit, soc_reg_t regno,
          soc_reg_above_64_val_t val, soc_reg_above_64_val_t* mask /* out param */, char *mod)
{
    soc_field_info_t *fld;
    char           *fmod, *fval;
    char           *modstr;
 ...
 */
STATIC cmd_result_t
dpp_do_reg_set(int unit, args_t *a, int mod)
{
    soc_reg_above_64_val_t regval;
    ...
    if ((0 == mod) && (rt = dpp_regtype_lookup_name(name)) != 0) {
        /*        if ((name = ARG_GET(a)) == 0) {
                  return CMD_USAGE;
                  }*/
    } else {
    ...
    collect_comma_args_dcmn(a, valstr, s);
    ...
    if (!mod && isint(name)) {
        ...
        rv = _dpp_reg_set_by_type(unit, regaddr, cmic_blk, rt->type, regval, array_index);
            ...
            for (i = 0; i < alist.count; i++) {
                ...
                if (!isint(valstr)) {  /* Must modify registers */
                    ...
                    if (mod) {
                        rv = CMD_OK;
                        if (soc_cpureg == SOC_REG_INFO(unit,ainfo->reg).regtype) {
                            addr = soc_reg_addr_get(unit, ainfo->reg, ainfo->block, ainfo->idx, TRUE, &blk, &acc_type);
                            ...
                            rv = soc_reg_above_64_get(unit, ainfo->reg, ainfo->port,  SOC_REG_IS_ARRAY(unit, ainfo->reg) ? ainfo->idx : 0, regval);
                        }
                    } else {
                ...
                if (soc_cpureg == SOC_REG_INFO(unit,ainfo->reg).regtype) {
                    ...
                    rv = soc_custom_reg_above_64_set(unit, ainfo->reg, ainfo->port,  SOC_REG_IS_ARRAY(unit, ainfo->reg) ? ainfo->idx : 0, regval);
                } else if (rv == CMD_OK) {
                    rv = soc_reg_above_64_set(unit, ainfo->reg, ainfo->port,  SOC_REG_IS_ARRAY(unit, ainfo->reg) ? ainfo->idx : 0, regval);
                }
 ...
 */
cmd_result_t
cmd_dpp_reg_set(int unit, args_t *a)
{
    return dpp_do_reg_set(unit, a, 0);
}

char cmd_dpp_reg_modify_usage[] =
 ...
 */
cmd_result_t
cmd_dpp_reg_modify(int unit, args_t * a)
{
    return dpp_do_reg_set(unit, a, 1);
}


#if defined(__DUNE_GTO_BCM_CPU__) || defined(__DUNE_WRX_BCM_CPU__)
char cmd_dpp_cpu_i2c_usage[] =
#ifdef COMPILER_STRING_CONST_LIMIT
    "Full documentation cannot be displayed with -pendantic compiler\n";
#else
...
cmd_result_t
cmd_dpp_cpu_i2c(int unit, args_t* a)
{
#ifndef __KERNEL__
    ...
    if (!function) {
        ...
        if (!alen) {
            return CMD_USAGE;
        } else if (!sal_strncasecmp(alen, "byte", strlen(alen))) {
            int_alen = CPU_I2C_ALEN_BYTE_DLEN_BYTE;
        } else if (!sal_strncasecmp(alen, "word", strlen(alen))) {
            int_alen = CPU_I2C_ALEN_WORD_DLEN_WORD;
        } else if (!sal_strncasecmp(alen, "long", strlen(alen))) {
        ...
        cpu_i2c_write(int_chip_addr, int_data_addr,
                      (CPU_I2C_BUS_LEN )int_alen, int_value);
        ...
        cpu_i2c_read(int_chip_addr, int_data_addr,
                     (CPU_I2C_BUS_LEN )int_alen, &int_value);
...
cmd_result_t
cmd_dpp_cpu_regs(int unit, args_t* a)
{
...
char cmd_dpp_pcie_reg_usage[] =
    "\n\tRead/Write By pcie\n\t"
    "Usages:\n\t"
    "pcie write <reg_address> <value>\n\t"
    "pcie read <reg_address>\n\t"
    "Notice:\n\t"
...
cmd_result_t
cmd_dpp_pcie_reg(int unit, args_t* a)
{
    char *function, *addr, *value;
    uint32 int_addr, int_value;
    int swap=0;
    ...
    if ((!function) || (!addr)) {
        return CMD_USAGE;
    } else if (!sal_strncasecmp(function, "write", strlen(function)) || !sal_strncasecmp(function, "w", strlen(function))) {

        if (!sal_strncasecmp(addr, "swap", strlen(function)) || !sal_strncasecmp(addr, "s", strlen(function))) {
            swap = 1;
            addr = ARG_GET(a);
            if(!addr) {
                return CMD_USAGE;
        ...
        CMVEC(unit).write(&CMDEV(unit).dev, int_addr, int_value);
        ...
        int_value = CMVEC(unit).read(&CMDEV(unit).dev, int_addr);
...
#endif /* (defined(BCM_PETRA_SUPPORT) || defined(BCM_DFE_SUPPORT)) && (defined(LINUX) || defined(UNIX)) && (defined(__DUNE_GTO_BCM_CPU__) || defined(__DUNE_WRX_BCM_CPU__)) */
