  ...
  )
{
    ...
    if(SOC_IS_ARAD(unit)){
        ...
        if (SOC_IS_JERICHO(unit)) {
            ...
            /* Check if the lif id indicates a global lif (mapped lif) or local lif (direct lif) */
            if (alloc_flags &  SHR_RES_ALLOC_WITH_ID) {
                rv = _bcm_dpp_lif_id_is_mapped(unit, FALSE, *ll_global_lif_index, &is_mapped);
                BCMDNX_IF_ERR_EXIT(rv);
            }
            if (is_mapped) {
                /* First, allocate the global lif */
                rv = bcm_dpp_am_global_lif_alloc(unit, alloc_flags | BCM_DPP_AM_FLAG_ALLOC_EGRESS, ll_global_lif_index);
                BCMDNX_IF_ERR_EXIT(rv);
                /* Then, allocate the egress local lif */
                local_out_lif_info.glif = *ll_global_lif_index;
                local_out_lif_info.local_lif_flags = BCM_DPP_AM_OUT_LIF_FLAG_COUPLED;
            } else {
                local_lif_flags = alloc_flags;
                ll_local_lif_index = ll_global_lif_index;
                local_out_lif_info.glif = *ll_local_lif_index;
                local_out_lif_info.local_lif_flags = BCM_DPP_AM_OUT_LIF_FLAG_DIRECT;
            }
            ...
            *ll_local_lif_index = local_out_lif_info.base_lif_id;
            ...
            if (SOC_IS_JERICHO(unit) && is_mapped) {
                rv = MBCM_PP_DRIVER_CALL(unit, mbcm_pp_lif_glem_access_entry_add, (unit, *ll_local_lif_index, *ll_global_lif_index));
                BCMDNX_IF_ERR_EXIT(rv);
            }
    ...
    bcm_if_t *if_id)
{
    ...
    if (is_egress) {    
        ...
        */
        ...
        /* Update GLEM entries */
        if (SOC_IS_JERICHO(unit)) {
            status = MBCM_PP_DRIVER_CALL(unit, mbcm_pp_lif_glem_access_entry_add, (unit, tunnel_global_lif, tunnel_local_lif));
            if (status != BCM_E_NONE) {
                DPP_L3_LOCK_RELEASE;
                BCMDNX_IF_ERR_EXIT(status);
            }
    ...
    bcm_l3_source_bind_t *info)
{
    ...
    rv = _bcm_dpp_gport_to_global_and_local_lif(unit, info->port, NULL, &lif_id, NULL, &fec_id, &is_local);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    if ((info->flags & BCM_L3_SOURCE_BIND_IP6) == 0) {
        ...
        if (_BCM_L3_SOURCE_BIND_IP6_COMPRESSION(info->rate_id)) {
            rv = _bcm_petra_l3_ip6_compression_to_ppd(unit, lif_id, info, &ip6_compression_entry);
            BCMDNX_IF_ERR_EXIT(rv);
