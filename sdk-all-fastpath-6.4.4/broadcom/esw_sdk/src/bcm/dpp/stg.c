...
#define STG_CHECK_INIT(unit)                                           \
    do {                                                               \
        int init;                                                      \
        BCMDNX_IF_ERR_EXIT(STG_ACCESS.init.get(unit, &init));\
        BCM_DPP_UNIT_CHECK(unit);                                      \
        if (unit >= BCM_MAX_NUM_UNITS) return BCM_E_UNIT;              \
        if (init == FALSE) return BCM_E_INIT;           \
 ...
 */
#define STG_BITMAP_TST(_stg)     SHR_BITGET(sw_state[unit]->dpp.bcm.stg->stg_bitmap, _stg)
#define STG_BITMAP_SET(_stg)     SHR_BITSET(sw_state[unit]->dpp.bcm.stg->stg_bitmap, _stg)
#define STG_BITMAP_CLR(_stg)     SHR_BITCLR(sw_state[unit]->dpp.bcm.stg->stg_bitmap, _stg)
...
_bcm_petra_stg_sw_dump(int unit)
{
    ...
    STG_ACCESS.stg_min.get(unit, &stg_min);
    STG_ACCESS.stg_max.get(unit, &stg_max);
    STG_ACCESS.stg_defl.get(unit, &stg_defl);
    STG_ACCESS.stg_count.get(unit, &stg_count);
    STG_ACCESS.init.get(unit, &init);
    ...
    for (stg = stg_min; stg <= stg_max; stg++) {
        ...
        if (STG_BITMAP_TST(stg)) {
            ...
            for (vid = vlan_first;
                ...
                cnt++;
                STG_ACCESS.vlan_next.get(unit, vid, &vlan_next);
            }
...
_bcm_petra_stg_stp_get(int unit, bcm_stg_t stg, bcm_port_t port, int *stp_state)
{
    ...
    if (BCM_PBMP_MEMBER(sw_state[unit]->dpp.bcm.stg->stg_enable[stg], port)) {
        if (BCM_PBMP_MEMBER(sw_state[unit]->dpp.bcm.stg->stg_state_h[stg], port)) {
            state |= 0x2;
        }
        if (BCM_PBMP_MEMBER(sw_state[unit]->dpp.bcm.stg->stg_state_l[stg], port)) {
            state |= 0x1;
        }
        switch (state) {
...
_bcm_petra_stg_stp_set(int unit, bcm_stg_t stg, bcm_port_t port, int stp_state)
{
    ...
    if (BCM_E_NONE == result) {
        if (stp_state == BCM_STG_STP_DISABLE) {
            BCM_PBMP_PORT_REMOVE(sw_state[unit]->dpp.bcm.stg->stg_enable[stg], port);

        } else {
            BCM_PBMP_PORT_ADD(sw_state[unit]->dpp.bcm.stg->stg_enable[stg], port);
            
            if ((stp_state == BCM_STG_STP_LEARN) ||
                (stp_state == BCM_STG_STP_FORWARD)) {
                BCM_PBMP_PORT_ADD(sw_state[unit]->dpp.bcm.stg->stg_state_h[stg], port);
            } else {
                BCM_PBMP_PORT_REMOVE(sw_state[unit]->dpp.bcm.stg->stg_state_h[stg], port);
            }
            if ((stp_state == BCM_STG_STP_LISTEN) ||
                (stp_state == BCM_STG_STP_FORWARD)) {
                BCM_PBMP_PORT_ADD(sw_state[unit]->dpp.bcm.stg->stg_state_l[stg], port);
            } else {
                BCM_PBMP_PORT_REMOVE(sw_state[unit]->dpp.bcm.stg->stg_state_l[stg], port);
            }
        }
    }
...
_bcm_petra_stg_vlan_add(int unit, bcm_stg_t stg, bcm_vlan_t vid)
{
    ...
    } else if (STG_BITMAP_TST(stg)) { /* STG must already exist */
...
bcm_petra_stg_vlan_remove_all(int unit, bcm_stg_t stg)
{
    ...
    if (stg != stg_defl) {
        ...
        if (!STG_BITMAP_TST(stg)) {   /* STG must already exist */
...
bcm_petra_stg_detach(int unit)
{
    ...
    BCMDNX_IF_ERR_EXIT(STG_ACCESS.stg_bitmap.free(unit));
    BCMDNX_IF_ERR_EXIT(STG_ACCESS.stg_enable.free(unit));
    BCMDNX_IF_ERR_EXIT(STG_ACCESS.stg_state_h.free(unit));
    BCMDNX_IF_ERR_EXIT(STG_ACCESS.stg_state_l.free(unit));
    BCMDNX_IF_ERR_EXIT(STG_ACCESS.vlan_first.free(unit));
    BCMDNX_IF_ERR_EXIT(STG_ACCESS.vlan_next.free(unit));
    BCMDNX_IF_ERR_EXIT(STG_ACCESS.free(unit));
...
{
    int                 sz_bmp, sz_e, sz_sh, sz_sl, sz_vfirst;
    bcm_port_t          port;
    int                 result = BCM_E_NONE;
    bcm_stg_t           stg_max;
    bcm_stg_t           stg_min;
    ...
    sz_bmp = SHR_BITALLOCSIZE(stg_max+1);

    BCMDNX_IF_ERR_EXIT(STG_ACCESS.stg_bitmap.alloc(unit, sz_bmp/sizeof(SHR_BITDCL)));
    ...
    BCMDNX_IF_ERR_EXIT(STG_ACCESS.vlan_next.alloc(unit, BCM_VLAN_COUNT));
