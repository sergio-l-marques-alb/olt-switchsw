...
STATIC int
_bcm_field_presel_qualifiers_dump(int unit, bcm_field_presel_t presel);
...
_field_th_presel_qualifiers_init(int unit, _field_stage_t *stage_fc)
{
    ...
    _FP_PRESEL_QUAL_ADD(unit, stage_fc, bcmFieldQualifyEMLogicalTableId0,
        _bcmFieldSliceSelDisable, 0, _bcmFieldQualifierTypePresel,
        33 - key_base_off, 5);
    /* EXACT_MATCH_LOGICAL_TABLE_ID_1 - 5 */
    _FP_PRESEL_QUAL_ADD(unit, stage_fc, bcmFieldQualifyEMLogicalTableId1,
        _bcmFieldSliceSelDisable, 0, _bcmFieldQualifierTypePresel,
        38 - key_base_off, 5);
#endif
    /* RESERVED_0 - 1 */
...
/*
 * Function:
 *     _field_th_presel_info_init
 * Purpose:
 *     Initialize Field control Preselector information and qualifiers.
 * Parameters:
 *     unit     - (IN) BCM device number.
...
STATIC int
_field_th_presel_info_init(int unit,
                           _field_control_t *fc,
                           _field_stage_t *stage_fc)
{
    int                   rv;                   /* Operation return status.  */
    int                   qual_idx;             /* Qualifier index. */
    _field_presel_info_t  *presel_info = NULL;  /* Reference to Presel Info. */
    ...
    /* Allocate preselector lookup hash. */
    _FP_XGS3_ALLOC(fc->presel_hash, _FP_HASH_SZ(fc) * \
                   sizeof(_field_presel_entry_t *), "Presel hash");
    if (NULL == fc->presel_hash) {
        return (BCM_E_MEMORY);
    }

    /* Allocate preselector information structure. */
    _FP_XGS3_ALLOC(presel_info, sizeof(_field_presel_info_t),
                   "Field Control Presel Info Alloc.");
    if (NULL == presel_info) {
        LOG_ERROR(BSL_LS_BCM_FP, (BSL_META_U(unit,
               "Error: Field Control Preselector Info Alloc failed.\n")));
        return (BCM_E_MEMORY);
    }
  
    /* Update the Preselector Max Limit */
    presel_info->presel_limit = _BCM_FIELD_PRESEL_ID_MAX;
 ...
 *    - Preselection field control initialization.
 *
 * Parameters:
 *    unit      - (IN) BCM device number.
                                     ...
                                     _field_stage_t *stage_fc)
{
    ...
    if (_BCM_FIELD_STAGE_PRESEL != stage_fc->stage_id) {
        return (BCM_E_NONE);
    }

    /* Initialize Preselection control information and qualifiers */
    rv = _field_th_presel_info_init(unit, fc, stage_fc);
    if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_FP, (BSL_META_U(unit,
...
/*
 * Function:
 *     _field_th_presel_info_free
 * Purpose:
 *     De-Initialize Preselector Information Structure.
 * Parameters:
 *     unit     - (IN) BCM device number.
 *     fc       - (IN) Reference to Field Control structure.
 * Returns:
...
STATIC int
_field_th_presel_info_free(int unit,
                           _field_control_t *fc)
{
    /* Input parameters check. */
    if (NULL == fc) {
    ...
    /* Free presel lookup hash. */
    if (NULL != fc->presel_hash) sal_free(fc->presel_hash);

    /* Free presel information. */
    if (NULL != fc->presel_info) sal_free(fc->presel_info);
                                 ...
                                 _field_stage_t *stage_fc)
{
    ...
    /* Deallocate Field Control Preselector information */
    _field_th_presel_info_free(unit, fc);
                            ...
                            bcm_field_presel_t *presel)
{
    ...
    if (flags & _bcmFieldPreselCreateWithId) {
       ...
       while (max_count--) {
          ...
          if (id == presel_info->presel_limit) {
             presel_info->last_allocated_id = _FP_ID_BASE;
          }
    ...
    /* Insert Presel into Preselector hash */
    _FP_HASH_INSERT(fc->presel_hash, f_presel, (id & _FP_HASH_INDEX_MASK(fc)));
                             ...
                             bcm_field_presel_t presel_id)
{
    ...
    /* Remove preselector for lookup hash. */
    _FP_HASH_REMOVE(fc->presel_hash, _field_presel_entry_t, f_presel,
                    (presel_id & _FP_HASH_INDEX_MASK(fc)));
...
{
   uint32                hash_index; /* Entry hash index */
   _field_control_t      *fc;        /* Field control structure */
   ...
   hash_index = presel_id & _FP_HASH_INDEX_MASK(fc);
   f_presel = fc->presel_hash[hash_index];
   for (;f_presel != NULL; f_presel = f_presel->next) {
      /* Match entry id */
      if (f_presel->presel_id == presel_id) {
         *presel = f_presel;
         return BCM_E_NONE;
      }
   }
...
{
    /* Input parameters check. */
    if ((NULL == f_presel) || (NULL == tcam_idx)) {
        return (BCM_E_PARAM);
    }
    ...
    *tcam_idx = f_presel->lt_fs->start_tcam_idx + f_presel->hw_index;
...
/*
 * Function:
 *     _field_th_presel_tcam_key_mask_get
 * Purpose:
 ...
 */
STATIC int
_field_th_presel_tcam_key_mask_get(int unit,
                                   _field_stage_t *stage_fc,
                                   int inst,
                                   _field_presel_entry_t *f_presel,
                                  ...
                                  _field_presel_data_t *p_data)
{
    ...
    if (!(f_presel->flags & _FP_ENTRY_DIRTY)) {
       ...
       rv = _bcm_field_presel_entry_tcam_idx_get(unit, f_presel, &data_idx);
       if (BCM_FAILURE(rv)) {
          goto bcm_error;
       }
...
#define DATA_TOO_BIG_FOR_FIELD ((p_data[idx] & ~u32_mask) != 0)
    ...
    /* Allocate memory for TCAM Key and Mask. */
    rv = _field_th_presel_tcam_key_mask_get(unit, stage_fc, inst,
                              ...
                              uint32 *mask)
{
    ...
    BCM_IF_ERROR_RETURN
        (_field_stage_control_get(unit, _BCM_FIELD_STAGE_PRESEL, &stage_fc));
...
#if 1
rv = _bcm_field_presel_qualifiers_dump(unit, presel);
if (BCM_FAILURE(rv))
{
  LOG_ERROR(BSL_LS_BCM_FP, (BSL_META_U(unit, "Qualifiers Dump Failed .. \n\r")));
...
_bcm_field_presel_qualifiers_dump(int unit, bcm_field_presel_t presel)
{
    ...
    LOG_CLI((BSL_META_U(unit, "Preselector Entry Id: %d\n\r"), presel));
    ...
    for (qual_id = 0; qual_id < _bcmFieldQualifyCount; qual_id++) {
        ...
        LOG_CLI((BSL_META_U(unit, "%-10s: "), _field_qual_name(qual_id)));
        /* Initialize q_offset to NULL */
        sal_memset(&q_offset, 0x0, sizeof(q_offset));
        ...
        LOG_CLI((BSL_META_U(unit, "Data:[0x%x] Mask:[0x%x]\n\r"),
                            q_data[0], q_mask[0]));
        /* Reference count */
        num_qual = 1;
    }
...
_bcm_field_presel_group_prio_sort(int unit, _field_group_t *fg)
{
   ...
   int i,j;
    ...
    for (i = 0; i < _FP_PRESEL_ENTRIES_MAX_PER_GROUP; i++) {
       if (f_presel[i]) {
           LOG_CLI((BSL_META_U(unit, "arr[%d]:priority[%d]\n\r"),
                    i, f_presel[i]->priority));
       }
    }
    ...
    for (idx = valid_entries-1; idx >= 0; idx--) {
       ...
       if (f_presel_tmp != NULL) {
          if (((move_down == 1) && ((prio >= f_presel_tmp->priority) ||
             ...
             rv = _bcm_field_th_lt_entry_install(unit, f_presel_tmp);
             if (BCM_FAILURE(rv)) {
                LOG_ERROR(BSL_LS_BCM_FP, (BSL_META_U(unit,
                                  ...
                                  uint8 valid)
{
    ...
    BCM_IF_ERROR_RETURN(_field_stage_control_get(unit, _BCM_FIELD_STAGE_PRESEL,
                                                 &stage_fc));
...
_bcm_field_th_lt_entry_install(int unit, _field_presel_entry_t *f_presel)
{
    ...
    _field_presel_entry_t *f_presel_p; /* Preselector Entry Part.         */
    ...
    for (idx = parts_count -1; idx >= 0; idx--) {
       ...
       /* Install all parts of the entry in hardware. */
        rv = _bcm_field_th_lt_entry_hw_install(unit, lt_tcam_mem, tcam_idx,
                                               f_presel_p->lt_tcam.key,
                                               f_presel_p->lt_tcam.mask,
                                               f_presel_p->lt_data.data,
                                               valid);
        if (BCM_FAILURE(rv)) {
            break;
        }
