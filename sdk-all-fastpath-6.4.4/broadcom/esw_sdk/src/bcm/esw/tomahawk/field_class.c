...
STATIC int _field_th_class_type_qset_get(int unit,
                                         bcm_field_qset_t *qset,
                                         _field_class_type_t *ctype);

STATIC int _field_th_class_max_entries(int unit, 
                                       int cpipe,
                                       _field_class_type_t ctype,
                                       int *max_entries);
STATIC int
...
STATIC int
_field_th_class_size_get(int unit,
                         _field_class_type_t ctype,
                         int *memsize);
 ...
 */
STATIC int
_field_th_class_type_qset_get(int unit, bcm_field_qset_t *qset,
                                                _field_class_type_t *ctype)
{
...
_field_th_class_group_validate(int unit, _field_group_t *class_fg) 
{
    ...
    if (stage_fc->oper_mode == bcmFieldGroupOperModeGlobal) {
         ...
         */
        if (BCM_PBMP_NEQ(pc.all, class_fg->pbmp)) {
 ...
 */
STATIC int
_field_th_class_size_get(int unit, _field_class_type_t ctype,
                                                  int *memsize) 
{
    int rv = BCM_E_NONE; /* Operation Return Status */
                                                ...
                                                bcm_field_aset_t aset_class)
{
    ...
    rv = _bcm_field_group_status_init(unit, &fg->group_status);
    if (BCM_FAILURE(rv)) {
        sal_free(fg);
        return rv;
    }
    ...
    rv = _field_th_class_type_qset_get(unit, &fg->qset, &ctype);
    if (BCM_FAILURE(rv)) {
        sal_free(fg);
        return rv;
    }
...
_bcm_field_th_class_entry_dump(int unit, _field_entry_t *class_ent)
{
    ...
    while (fa != NULL) {
        LOG_CLI((BSL_META("{act=%d, class=0x%x%08x}"),
                fa->action, fa->param[1], fa->param[0]));
        fa = fa->next;
    }
    LOG_CLI((BSL_META_U(unit,
                        "\n")));
        ...
        case _FieldClassSrcCompression:
            ...
            if (keytype == _FP_KEY_IPv4_TYPE) {
                ...
                for (i = 0; i < sizeof(bcm_ip6_t); i++) {
                    LOG_CLI((BSL_META("0x%x:"), cip6_addr[i]));
                }
                LOG_CLI((BSL_META(", ")));
                LOG_CLI((BSL_META_U(unit,"Ipv6 Mask=")));
                for (i = 0; i < sizeof(bcm_ip6_t); i++) {
                    LOG_CLI((BSL_META("0x%x:"), cip6_addr_mask[i]));
                }
                LOG_CLI((BSL_META(", ")));
            } else {
                                                    ...
                                                    bcm_field_entry_t entry)
{
    ...
    rv = _field_th_class_size_get(unit, ctype, &tcam_sz);
    if (BCM_FAILURE(rv)) {
        sal_free(f_ent);
        return (rv);
    }
 ...
 * Returns:
 *    BCM_E_XXX
 * Notes: This api should not delete
 *    entry if install flag is set. This check happens
 *    in calling function.
 */
int
                                    ...
                                    int qual, uint32 *data, uint32 *mask) 
{
    ...
    /* Retreive Class Type from entry qset */ 
    rv = _field_th_class_type_qset_get(unit,
    ...
    switch(ctype) {
        ...
        case _FieldClassSrcCompression:
            ...
            if (qual == bcmFieldQualifySrcIp) {
                ...
                /*
                 * Validate VRF Id,
                 * should not be more than 11 bits.
                 */
                if ((*data & ~(0x7FF)) ||
                    (*mask & ~(0x7FF))) {
                    return (BCM_E_PARAM);
                                    ...
                                    bcm_field_class_info_t *class_info)
{
    ...
    COMPILER_64_TO_32_LO(fa->param[0], class_info->class);
    COMPILER_64_TO_32_HI(fa->param[1], class_info->class); 
                                ...
                                bcm_field_class_info_t *class_info)
{
    ...
    COMPILER_64_SET(class_info->class, fa->param[1], fa->param[0]);
 ...
 * Returns:
 *    BCM_E_XXX:
 * Notes: No Null Check for entry, group and key.
...
_field_th_class_ethertype_install(int unit, _field_entry_t *entry)
{
    ...
    soc_reg_t reg;              /* SOC Register */ 
    static const soc_reg_t etype_class_regs[] = {
        ETHERTYPE_MAP_PIPE0r,
        ETHERTYPE_MAP_PIPE1r,
        ETHERTYPE_MAP_PIPE2r,
        ETHERTYPE_MAP_PIPE3r
    };                          /* SOC Per Pipe Register */
    uint32 *cdata = NULL;       /* Class Data */
    ...
    if (stage_fc->oper_mode == bcmFieldGroupOperModeGlobal) {
        ...
        reg = etype_class_regs[fg->instance]; 
    }
 ...
 * Returns:
 *    BCM_E_XXX
 * Notes: No Null Check for entry, group and classdata buffer.
...
_field_th_class_l4srcport_install(int unit, _field_entry_t *entry) 
{
    ...
    uint32 rval = 0;            /* Register Value */
    soc_reg_t reg;              /* SOC Register */
    static const soc_reg_t l4srcport_class_regs[] = {
        L4_SRC_PORT_MAP_PIPE0r,
        L4_SRC_PORT_MAP_PIPE1r,
        L4_SRC_PORT_MAP_PIPE2r,
        L4_SRC_PORT_MAP_PIPE3r
    };                          /* SOC Per Pipe Register */
    uint32 *cdata = NULL;       /* Class Data */
    ...
    if (stage_fc->oper_mode == bcmFieldGroupOperModeGlobal) {
        ...
        reg = l4srcport_class_regs[fg->instance];
    }
...
_field_th_class_l4dstport_install(int unit, _field_entry_t *entry) 
{
    ...
    uint32 rval = 0;            /* Register Value */
    soc_reg_t reg;              /* SOC Register */
    static const soc_reg_t l4dstport_class_regs[] = {
        L4_DST_PORT_MAP_PIPE0r,
        L4_DST_PORT_MAP_PIPE1r,
        L4_DST_PORT_MAP_PIPE2r,
        L4_DST_PORT_MAP_PIPE3r
    };                          /* SOC Per Pipe Register */
    uint32 *cdata = NULL;       /* Class Data */
    ...
    if (stage_fc->oper_mode == bcmFieldGroupOperModeGlobal) {
        ...
        reg = l4dstport_class_regs[fg->instance];
    }
...
_field_th_class_srccompression_install(int unit, _field_entry_t *entry)
{
    ...
    _field_stage_t *stage_fc = NULL;
                                /* Stage field control info */
    soc_mem_t mem;              /* SOC Memory */
    static const soc_mem_t src_class_mem[] = {
...
_field_th_class_ttl_install(int unit, _field_entry_t *entry)
{
    ...
    _field_stage_t *stage_fc = NULL;
                                /* Stage field control info */
    soc_mem_t mem;              /* SOC Memory */
    static const soc_mem_t ttl_class_mem[] = {
        TTL_FN_PIPE0m,
        TTL_FN_PIPE1m,
        TTL_FN_PIPE2m,
        TTL_FN_PIPE3m
...
_field_th_class_tos_install(int unit, _field_entry_t *entry) 
{
    ...
    _field_stage_t *stage_fc = NULL;
                                /* Stage field control info */
    soc_mem_t mem;              /* SOC Memory */
    static const soc_mem_t tos_class_mem[] = {
        TOS_FN_PIPE0m,
        TOS_FN_PIPE1m,
        TOS_FN_PIPE2m,
        TOS_FN_PIPE3m
...
_field_th_class_ipproto_install(int unit, _field_entry_t *entry)
{
    ...
    _field_stage_t *stage_fc = NULL;
                                /* Stage field control info */
    soc_mem_t mem;              /* SOC Memory */
    static const soc_mem_t ipproto_class_mem[] = {
        IP_PROTO_MAP_PIPE0m,
        IP_PROTO_MAP_PIPE1m,
        IP_PROTO_MAP_PIPE2m,
        IP_PROTO_MAP_PIPE3m
    ...
    if (stage_fc->oper_mode == bcmFieldGroupOperModeGlobal) {
        ...
        mem = ipproto_class_mem[fg->instance];
    }
...
_field_th_class_tcp_install(int unit, _field_entry_t *entry)
{
    ...
    _field_stage_t *stage_fc = NULL;
                                /* Stage field control info */
    soc_mem_t mem;              /* SOC Memory */
    static const soc_mem_t tcp_class_mem[] = {
        TCP_FN_PIPE0m,
        TCP_FN_PIPE1m,
        TCP_FN_PIPE2m,
        TCP_FN_PIPE3m
