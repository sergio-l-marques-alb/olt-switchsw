            ...
            uint32 stat_counter_id)
{
    ...
    switch(object) {
         ...
         case bcmStatObjectIngFieldStageIngress:
              rv = _bcm_esw_stat_flex_destroy_ingress_table_counters(
                        unit, IFP_POLICY_TABLEm, object, offset_mode,
                        base_index, pool_number);
              break;
...
void _bcm_esw_stat_flex_init_pkt_attr_bits(int unit)
{
   ...
   if (SOC_IS_KATANA2(unit)) {
       ...
       ing_pkt_attr_cmprsd_bits_g.inner_dot1p_pos = 25 - offset_adj ;
    ...
    uint32            *num_entries)
{
    ...
    case bcmStatObjectIngFieldStageIngress:
          ...
          if (-1 != *stat_counter_id) {
              uint32 l_pool, l_mode, l_baseidx;
              bcm_stat_object_t l_object;
              bcm_stat_group_mode_t l_group;

              _bcm_esw_stat_get_counter_id_info(*stat_counter_id, &l_group,
                                                &l_object, &l_mode, &l_pool,
                                                &l_baseidx);
              if (l_pool < SOC_INFO(unit).num_flex_ingress_pools) {
                  pool_number = l_pool;
              } else {
          ...
          rv = _bcm_esw_stat_flex_create_ingress_table_counters(
                    unit, IFP_POLICY_TABLEm, object, mode_id,
                    &base_index, &pool_number);
          break;
