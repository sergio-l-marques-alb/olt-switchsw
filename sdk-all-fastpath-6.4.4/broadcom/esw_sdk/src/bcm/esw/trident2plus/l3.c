...
{   
    ...
    if (num_ol_banks > BCMI_L3_INTF_BANKS_MAX) {
        num_ol_banks = BCMI_L3_INTF_BANKS_MAX ;
    }
    /* Setup the bits for overlay banks. */
    for (iter = 0; iter < num_ol_banks; iter++) {
        ol_bank_bit_value |= (1 << iter) ;
    }
    /* Keep the overlay entries in first banks */
    ol_bank_bit_value = (ol_bank_bit_value <<
        (BCMI_L3_INTF_BANKS_MAX - num_ol_banks));
    ...
    if (num_ol_banks > BCMI_L3_NH_BANKS_MAX) {
        num_ol_banks = BCMI_L3_NH_BANKS_MAX ;
    }
    for (iter=0; iter < num_ol_banks; iter++) {
        ol_bank_bit_value |= (1 << iter) ;
    }
    /* Keep the overlay entries in first banks */
    ol_bank_bit_value = (ol_bank_bit_value <<
        (BCMI_L3_NH_BANKS_MAX - num_ol_banks));
