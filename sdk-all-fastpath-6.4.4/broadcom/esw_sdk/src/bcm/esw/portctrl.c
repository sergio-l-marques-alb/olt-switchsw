...
/*
 * Function:
 *      _bcm_esw_portctrl_to_portmod_ability
 * Purpose:
 *      Converts the _port_ability_t structure from BCM to
 *      PortMod _port_ability_t
 * Parameters:
 *      port_ability - (IN) BCM bcm_port_ability_t to convert.
 *      portmod_ability - (OUT) Returns information in PortMod _ability_ structure.
 * Returns:
 *      None
 */
STATIC void
_bcm_esw_portctrl_to_portmod_ability(bcm_port_ability_t *port_ability,
...
_bcm_esw_portctrl_pm4x10_portmod_init(int unit, int num_of_instances) 
{
    ...
    for (pmid = 0; PORTMOD_SUCCESS(rv) && (pmid < num_of_instances); pmid++) {
        ...
        for (core_num = 0; core_num < core_cnt; core_num++) {
            ...
            if(SOC_IS_TRIDENT2PLUS(unit)||1) { /* use TD2 legacy notion system */
                rxlane_map_l = 0 ;   
    ...
    if ((ref_clk_prop == 125) ||( ref_clk_prop == 1))
        ...
        tmp_ext_phy_access.access.addr = soc_property_port_get(unit, xphy_logical_port, spn_PORT_PHY_ADDR, 0xFF);
        tmp_ext_phy_access.type = phymodDispatchTypeCount; /* Make sure it is invalid. */

        if(tmp_ext_phy_access.access.addr != 0xFF) {
            ...
            rv = portmod_ext_phy_attach(unit, first_port, &tmp_ext_phy_access);
        }
...
_bcm_esw_portctrl_pm12x10_portmod_init(int unit, int num_of_instances) 
{
    ...
    for (pmid = 0; PORTMOD_SUCCESS(rv) && (pmid < num_of_instances); pmid++) {
        ...
        xphy_logical_port = SOC_INFO(unit).port_p2l_mapping[first_port+4];
        tmp_ext_phy_access.access.addr = soc_property_port_get(unit, xphy_logical_port, spn_PORT_PHY_ADDR, 0xFF);
        if(tmp_ext_phy_access.access.addr != 0xFF) {
            local_user_access = sal_alloc(sizeof(portmod_default_user_access_t), "pm4x10_specific_db");
            sal_memset(local_user_access, 0, sizeof(portmod_default_user_access_t));
            ...
            rv = portmod_ext_phy_attach(unit, first_port+4, &tmp_ext_phy_access);
        }

        xphy_logical_port = SOC_INFO(unit).port_p2l_mapping[first_port+8];
        tmp_ext_phy_access.access.addr = soc_property_port_get(unit, xphy_logical_port, spn_PORT_PHY_ADDR, 0xFF);
        if(tmp_ext_phy_access.access.addr != 0xFF) {
            local_user_access = sal_alloc(sizeof(portmod_default_user_access_t), "pm4x10_specific_db");
            sal_memset(local_user_access, 0, sizeof(portmod_default_user_access_t));
            ...
            rv = portmod_ext_phy_attach(unit, first_port+8, &tmp_ext_phy_access);
        }
    }
                                     ...
                                     portmod_port_init_config_t* init_config)
{
    ...
    interface_config->max_speed = SOC_INFO(unit).port_speed_max[port];
    ...
    if(is_hg) {
       ...
       if(an_cl37) {
           ...
           init_config->an_mode = soc_property_port_get(unit, port,
                                               spn_PHY_AN_C73, init_config->an_mode);
       }
    }
    ...
    /* init_config->user_set_tx = PORTMOD_USER_SET_TX_BY_STD ;*/
    if(preemphasis) {
        /* init_config->user_set_tx |= PORTMOD_USER_SET_TX_PREEMPHASIS_BY_CONFIG ; 
        p_tx->pre  =  preemphasis & 0xff;
        p_tx->main = (preemphasis & 0xff00) >> 8;
        p_tx->post = (preemphasis & 0xff0000) >> 16;*/
    }
    if(driver_current){
        /* init_config->user_set_tx |= PORTMOD_USER_SET_TX_AMP_BY_CONFIG ; 
        p_tx->amp = driver_current;*/
    }
                                     ...
                                     portmod_port_init_config_t* init_config)
{
    ...
    if ((SOC_IS_TRIDENT2PLUS(unit)) || (SOC_IS_GREYHOUND(unit))) {
                                       ...
                                       portmod_port_interface_config_t *if_cfg)
{
    ...
    if (IS_C_PORT(unit, port)) {
       portmod_port_interface_config_t triger_cfg;
      
       PORTMOD_IF_ERROR_RETURN
              (portmod_port_interface_config_get(unit, pport, &triger_cfg));

       triger_cfg.flags |= PHYMOD_INTF_F_SET_SPD_DISABLE;
       PORTMOD_IF_ERROR_RETURN
              (portmod_port_interface_config_set(unit, pport, &triger_cfg));
       triger_cfg.flags ^= PHYMOD_INTF_F_SET_SPD_DISABLE;
       
       triger_cfg.flags |= PHYMOD_INTF_F_SET_SPD_TRIGGER;
       PORTMOD_IF_ERROR_RETURN
              (portmod_port_interface_config_set(unit, pport, &triger_cfg));
    }
...
bcmi_esw_portctrl_loopback_set(int unit, bcm_gport_t port, int loopback)
{
    ...
    if (IS_C_PORT(unit, port)) {
       ...
       PORTMOD_IF_ERROR_RETURN
              (portmod_port_interface_config_set(unit, port, &if_cfg));
    }
                                     ...
                                     bcm_port_abil_t ability_mask)
{
    ...
    bcm_port_ability_t given_ability;
    portmod_port_ability_t portmod_ability;
    ...
    sal_memset(&given_ability, 0, sizeof(bcm_port_ability_t));
    sal_memset(&portmod_ability, 0, sizeof(portmod_port_ability_t));
    ...
    if (port_ability != NULL) {
        sal_memcpy(&given_ability, port_ability, sizeof(bcm_port_ability_t));
    } else {
        BCM_IF_ERROR_RETURN
    ...
    soc_port_ability_mask(&given_ability, &local_ability);
    ...
    _bcm_esw_portctrl_to_portmod_ability(&given_ability, &portmod_ability);

    PORT_LOCK(unit);
    rv = portmod_port_ability_advert_set(unit, pport, &portmod_ability);
    PORT_UNLOCK(unit);
    ...
    LOG_VERBOSE(BSL_LS_BCM_PORT,
                (BSL_META_UP(unit, port,
                     ...
                     given_ability.speed_half_duplex,
                     given_ability.speed_full_duplex,
                     given_ability.pause, given_ability.interface,
                     given_ability.medium, given_ability.loopback,
                     given_ability.flags));
    ...
    if (SOC_MEM_IS_VALID(unit, EGR_PORTm)) {
        ...
        if(soc_mem_field_valid(unit, EGR_PORTm, HG_TYPEf)) {
            SOC_IF_ERROR_RETURN(soc_mem_field32_modify(unit, EGR_PORTm, port,
                                ...
                                BCM_PORT_HG_TYPE_HGOE_TRANSPORT: 0));
        }
    } else {
...
bcmi_esw_portctrl_encap_set(int unit, bcm_gport_t port, int mode)
{
    ...
    if (BCM_SUCCESS(rv) && soc_feature(unit, soc_feature_higig_over_ethernet)) {
        ...
        if (BCM_SUCCESS(rv) && SOC_MEM_FIELD_VALID(unit, PORT_TABm, HG_TYPEf)) {
            /*rv = soc_mem_field32_modify(unit, PORT_TABm, port, HG_TYPEf, mode ==
                                        BCM_PORT_ENCAP_HIGIG_OVER_ETHERNET?
                                        BCM_PORT_HG_TYPE_HGOE_TRANSPORT: 0);*/

            rv = _bcm_esw_port_tab_set(unit, port,
                                       _BCM_CPU_TABS_NONE, HG_TYPEf, mode ==
                                       BCM_PORT_ENCAP_HIGIG_OVER_ETHERNET?
                                       BCM_PORT_HG_TYPE_HGOE_TRANSPORT: 0);
        }
    }
