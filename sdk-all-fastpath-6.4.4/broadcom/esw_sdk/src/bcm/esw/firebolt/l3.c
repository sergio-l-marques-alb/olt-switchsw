    ...
    if (stable_size > 0) { /* Limited and Extended recovery only */
        ...
        if (BCM_SUCCESS(rv)) {
            ...
            if (recovered_ver >= BCM_WB_VERSION_1_9) {
               if (soc_feature(unit, soc_feature_l3)) {
...
#endif /* BCM_TOMAHAWK_SUPPORT */
                    {
                        sal_memcpy(egflgbitmap, l3_scache_ptr,
                                   SHR_BITALLOCSIZE(BCM_L3_MAX_ECMP_GROUPS));
                        l3_scache_ptr += SHR_BITALLOCSIZE(BCM_L3_MAX_ECMP_GROUPS);
                        for (i = 0; i < BCM_L3_MAX_ECMP_GROUPS; i++) {
                            if (SHR_BITGET(egflgbitmap, i)) {
                                BCM_XGS3_L3_ECMP_GROUP_FLAGS_SET(unit, i,
                                                 BCM_L3_ECMP_PATH_NO_SORTING);
                            } else {
                                BCM_XGS3_L3_ECMP_GROUP_FLAGS_RESET(unit, i,
                ...
                if (soc_feature(unit, soc_feature_l3) &&
                    ...
                    /* ECMP Level 1 flags */
                    additional_scache_sz += SHR_BITALLOCSIZE(BCM_L3_ECMP_MAX_2K_GROUPS);
                    /* ECMP Level 2 flags */
                    additional_scache_sz += SHR_BITALLOCSIZE(BCM_L3_ECMP_MAX_2K_GROUPS);
                }
            ...
            if (soc_feature(unit, soc_feature_hierarchical_ecmp)) {
                sal_memcpy(eglevel1flgbitmap, l3_scache_ptr,
                                   SHR_BITALLOCSIZE(BCM_L3_ECMP_MAX_2K_GROUPS));
                l3_scache_ptr += SHR_BITALLOCSIZE(BCM_L3_ECMP_MAX_2K_GROUPS);
                sal_memcpy(eglevel2flgbitmap, l3_scache_ptr,
                                   SHR_BITALLOCSIZE(BCM_L3_ECMP_MAX_2K_GROUPS));
                l3_scache_ptr += SHR_BITALLOCSIZE(BCM_L3_ECMP_MAX_2K_GROUPS);
                for (i = 0; i < BCM_L3_ECMP_MAX_2K_GROUPS; i++) {
                    if (SHR_BITGET(eglevel1flgbitmap, i)) {
                        BCM_XGS3_L3_ECMP_GROUP_FLAGS_SET(unit, i,
                                                 BCM_L3_ECMP_OVERLAY);
                    } else {
                        BCM_XGS3_L3_ECMP_GROUP_FLAGS_RESET(unit, i,
                    ...
                    if (SHR_BITGET(eglevel2flgbitmap, i)) {
                        BCM_XGS3_L3_ECMP_GROUP_FLAGS_SET(unit, i,
...
#if defined(BCM_TRIDENT2_SUPPORT)
    if (SOC_MEM_FIELD_VALID(unit, BCM_XGS3_L3_MEM(unit, defip), GLOBAL_HIGH0f)) {
        if (BCM_L3_VRF_OVERRIDE == lpm_cfg->defip_vrf) {
            soc_mem_field32_set(unit, BCM_XGS3_L3_MEM(unit, defip),
                                lpm_entry, GLOBAL_ROUTE0f, 0x1);
            soc_mem_field32_set(unit, BCM_XGS3_L3_MEM(unit, defip),
                                lpm_entry, GLOBAL_HIGH0f, 0x1);
        }
    }
#endif /* BCM_TRIDENT2_SUPPORT */
    return (BCM_E_NONE);
    ...
    if (SOC_IS_TD_TT(unit)) {
        if (SOC_IS_TOMAHAWK(unit)) {
            tbl_entry_sz = WORDS2BYTES(soc_mem_entry_words(unit,
                               L3_DEFIP_HIT_ONLYm));
            BCM_IF_ERROR_RETURN
                (bcm_xgs3_l3_tbl_range_dma(unit, 
                                           L3_DEFIP_HIT_ONLYm,
                                           tbl_entry_sz,
                                           "lpm_hit_only_tbl",
                                           idx_start, idx_end - 1,
...
_bcm_xgs3_l3_black_hole_nh_setup(int unit)
{
    ...
    if (BCMI_L3_NH_OVERLAY_VALID(unit)) {
        ...
        if (BCM_XGS3_L3_BLACK_HOLE_NH_IDX > 
            BCM_XGS3_L3_TBL(unit, next_hop).split_maxused) {

            BCM_XGS3_L3_TBL(unit, next_hop).split_maxused = 
                BCM_XGS3_L3_BLACK_HOLE_NH_IDX;
        }
    } else 
#endif
    {
...
_bcm_xgs3_l3_l2cpu_nh_setup(int unit)
{
    ...
    if (BCMI_L3_NH_OVERLAY_VALID(unit)) {
        ...
        if (BCM_XGS3_L3_L2CPU_NH_IDX > 
            BCM_XGS3_L3_TBL(unit, next_hop).split_maxused) {

            BCM_XGS3_L3_TBL(unit, next_hop).split_maxused = 
                BCM_XGS3_L3_L2CPU_NH_IDX;
        }
    } else 
#endif
    {
...
_bcm_xgs3_l3_ecmp_group_init(int unit)
{
    ...
    if (soc_feature(unit, soc_feature_hierarchical_ecmp)) {
        if (BCM_XGS3_L3_TBL(unit, ecmp_info).ecmp_mode == 
            ...
            tbl_ptr->idx_max = tbl_ptr->idx_max / 2;
        }
    }
#endif /* BCM_TOMAHAWK_SUPPORT */
    ...
    if (soc_feature(unit, soc_feature_vp_lag)) {
        ...
        if (max_vp_lags > 0) {
            tbl_ptr->idx_maxused = max_vp_lags + vp_lag_idx_offset - 1;
        }
    }
#endif /* BCM_TRIDENT2_SUPPORT */
...
_bcm_xgs3_l3_ecmp_table_init(int unit)
{
    ...
    if (soc_feature(unit, soc_feature_hierarchical_ecmp)) {
        ...
        if (rval == ECMP_MODE_SINGLE_LOW_LATENCY) {
            /* Set max table size to half of ECMP table */
            tbl_ptr->idx_max = tbl_ptr->idx_max/2;
        }
    } else 
#endif /* BCM_TOMAHAWK_SUPPORT */
...
_bcm_xgs3_egress_l3_intf_id_alloc(int unit, _bcm_l3_intf_cfg_t *intf_info)
{
    ...
    {
        min_idx = tbl_ptr->idx_min;
        max_idx = BCM_XGS3_L3_IF_TBL_SIZE(unit);
    }
...
_bcm_xgs3_tbl_del(int unit,  _bcm_l3_tbl_op_t *data)
{
...
#ifdef BCM_RIOT_SUPPORT
    if (BCMI_RIOT_IS_ENABLED(unit) && (tbl_ptr->split_idx > 0)) {
    ...
    /* Check index sanity. */
    if ((index < idx_min) || (index > *maxused_idx)) {
        return (BCM_E_NONE);
    }
   ...
   int width;
    ...
    if ((flags & BCM_L3_MULTIPATH)) {
        ...
        BCM_XGS3_L3_ENT_REF_CNT_DEC
                (BCM_XGS3_L3_TBL_PTR(unit, ecmp_grp), nh_ecmp_index, width);
...
bcm_xgs3_ecmp_group_del(int unit, int ecmp_group_id)
{
    ...
    if (BCMI_L3_ECMP_IS_MULTI_LEVEL(unit)) {
        ol_ecmp_in_use = 1;
        split_idx = data.tbl_ptr->split_idx;

        if ((data.tbl_ptr->split_maxused == (data.tbl_ptr->idx_min)) && 
            (!BCM_XGS3_L3_ENT_REF_CNT(
            data.tbl_ptr, data.tbl_ptr->split_maxused))) {

            ol_ecmp_in_use = 0; 
        }
    } 
#endif
    /* if max_vp_lags has valid value or if this is RIOT flow then 
       idx_maxused is not 0 and has to be checked with offset value and not 0. */
    if ((max_vp_lags > 0) || (split_idx)) {
        if (max_vp_lags > 0) {
            if (((max_vp_lags + split_idx - 1) == data.tbl_ptr->idx_maxused) &&
                (ol_ecmp_in_use == 0)) {

                if (!BCM_XGS3_L3_ENT_REF_CNT
                    (data.tbl_ptr, (max_vp_lags + split_idx))) {
            ...
            if ((split_idx == data.tbl_ptr->idx_maxused) &&
                (ol_ecmp_in_use == 0)) {

                if (!BCM_XGS3_L3_ENT_REF_CNT
                    (data.tbl_ptr, split_idx)) {
        ...
        if (entry_type == 1) {
            if (egr.flags &  BCM_L3_ROUTE_LABEL) {
                ...
                if (egr.mpls_label != BCM_MPLS_LABEL_INVALID) {
                    ...
                    bcm_tr_mpls_egress_entry_modify(unit, nh_idx, 0, 0);
                }
            }
        }
    }
...
_bcm_xgs3_ecmp_group_nh_delete(int unit, int *ecmp_group, int ecmp_count)
{
    ...
    for (idx = 0; idx < ecmp_count; idx++) {
         /* If ECMP member is another ECMP group, skip NH delete */
         if (soc_feature(unit, soc_feature_hierarchical_ecmp)) {
            ...
            if (BCM_XGS3_L3_TBL(unit, ecmp_info).ecmp_mode == 
                ...
                if ((ecmp_group_flags & BCM_L3_ECMP_OVERLAY) &&
                    ...
                    if (BCM_XGS3_L3_ECMP_GROUP_FLAGS_ISSET(unit,
                        (intf_array[idx] - BCM_XGS3_MPATH_EGRESS_IDX_MIN),
                        BCM_L3_ECMP_UNDERLAY)) {
                        continue;
                    } else {
                        return (BCM_E_PARAM);
                                    ...
                                    int intf_count, bcm_if_t *intf_array, bcm_if_t *mpintf)
{
    ...
    if (soc_feature(unit, soc_feature_ecmp_resilient_hash_optimized) &&
        ...
        rv = bcm_xgs3_l3_egress_multipath_max_create(unit, flags, ecmp_flags,
                                                     BCM_XGS3_L3_ECMP_RH_MAX_PATHS(unit),
                          ...
                          void *user_data)
{
    ...
    for (idx = 0; idx <= tbl_ptr->idx_maxused; idx++) {
        ...
        if (soc_feature(unit, soc_feature_vp_lag)) {
             ...
             */ 
            if ((idx >= tbl_ptr->split_idx) 
                && (BCMI_RIOT_IS_ENABLED(unit)) &&
                (idx <= tbl_ptr->split_idx + max_vp_lags)) {

                 idx = tbl_ptr->split_idx + max_vp_lags - 1; 
                 continue;
            } else 
#endif
            {
    ...
    int *intf_count)
{
    ...
    if (ecmp_info->dynamic_mode == BCM_L3_ECMP_DYNAMIC_MODE_RESILIENT &&
        ...
        rv = bcm_th_ecmp_rh_get_multipath(unit, 
                        ...
                        intf_size, intf_array, intf_count, &ecmp_info->max_paths);
    } else
#endif /* BCM_TOMAHAWK_SUPPORT */
    {
...
_bcm_xgs3_lpm_get_nh_info(int unit, _bcm_defip_cfg_t *lpm_cfg, int nh_idx)
{
    ...
    if (BCM_XGS3_L3_EGRESS_MODE_ISSET(unit)) {
        if (lpm_cfg->defip_flags & BCM_L3_MULTIPATH) {
            ...
            if (nh_info.encap_id > 0 &&
                nh_info.encap_id < BCM_XGS3_EGRESS_IDX_MIN) {
                lpm_cfg->defip_intf = nh_idx + BCM_XGS3_DVP_EGRESS_IDX_MIN;
            } else {
...
bcm_xgs3_max_ecmp_set(int unit, int max_paths)
{
    ...
    if (soc_feature(unit, soc_feature_hierarchical_ecmp)) {
        ...
        if ((BCM_XGS3_L3_TBL(unit, ecmp_info).ecmp_mode ==
                     ECMP_MODE_SINGLE_LOW_LATENCY) && (max_paths > 128)) {
            return (BCM_E_PARAM);
        }
    ...
    for (grp_idx = ecmp_tbl_ptr->idx_min; grp_idx <= grp_max_idx;) {
        ...
        */
        if (soc_feature(unit, soc_feature_vp_lag)) {
             ...
             */
            if ((BCMI_RIOT_IS_ENABLED(unit)) 
                && (ecmp_tbl_ptr->split_idx > 0)) {

                if ((grp_idx >= ecmp_tbl_ptr->split_idx) && 
                    (grp_idx <= ecmp_tbl_ptr->split_idx + max_vp_lags - 1)) {
                    grp_idx = ecmp_tbl_ptr->split_idx + max_vp_lags - 1;
                    continue;
                }
            } else 
...
_bcm_xgs3_l3_intf_get(int unit, _bcm_l3_intf_cfg_t *intf_info)
{
    ...
    if (soc_feature(unit, soc_feature_riot)) {
        intf_info->l3i_source_vp = 
            soc_mem_field32_get(unit, mem, l3_if_entry_p, SVPf);

        if (soc_mem_field32_get(unit, mem, l3_if_entry_p, 
...
_bcm_xgs3_l3_intf_add(int unit, _bcm_l3_intf_cfg_t *intf_info)
{
    ...
    if (BCMI_L3_INTF_OVERLAY_VALID(unit)) {
        ...
        soc_mem_field32_set(unit, mem, l3_if_entry_p,
                            SVPf, intf_info->l3i_source_vp);

        soc_mem_field32_set(unit, mem, l3_if_entry_p, VLAN_VFI_OVERLAY_TYPEf,
            BCMI_RIOT_VPN_VFI_IS_SET(unit, intf_info->l3i_vid) ? 1 : 0 ); 

        soc_mem_field32_set(unit, mem, l3_if_entry_p,
            (BCMI_RIOT_VPN_VFI_IS_SET(unit, intf_info->l3i_vid) ? 
            VFIf : fld->vlan_id),
            (intf_info->l3i_vid & _BCM_VPN_ID_MASK));
    } else 
#endif
    {
        /* Set vlan id. */
